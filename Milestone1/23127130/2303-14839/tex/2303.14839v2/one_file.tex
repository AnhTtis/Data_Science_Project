...
\end{figure}
 We display our numerical OTOCs for increasing particle $N=10^3, ~10^4, ~5\cdot 10^4$ in Fig.~\ref{fig:OTOC_FP_numeric}, where we observe the predicted $2\ls-\ls$ transition, precisely following the heuristic arguments of Sec.~\ref{sec:OTOC_theory} and the %exact 
 more refined classical analysis of Sec.~\ref{sec:peter}. In particular, the analytical result Eq.~\eqref{eq:exact.clOTOC} for the classical OTOC follows perfectly the quantum OTOC $\mathbf{C}(t)$, i.e., it captures both regimes and the transition.
...
\begin{figure*}[!ht]
    ...
    \caption{Time-evolution of the Husimi-distribution for the state $|\vec{\xi}\, \rangle$ centered at the hyperbolic off-phase FP with $N=10^3$ particles. We observe scrambling along the unstable manifold on the separatrix until $t\approx \tLeak$.}
    \label{fig:husimiFP}
\end{figure*}
...
This confirms that the~$2\ls-\ls$ transition is independent of the specific value of~$\Theta$, i.e., it is a robust signal of a dynamical transition.
%\FloatBarrier
...
Exemplary, choosing $t_{0} = -3\tEhr/4$ (with $\tEhr$ the Ehrenfest time for the non-squeezed coherent state), we effectively transform $\Delta u \sim \hbar_{{\rm eff}}^{1/2}$ into $\Delta u \sim \hbarE$.
...
We supported our picture of the dynamical transition by means of extensive simulations on the experimentally accessible, and integrable,  Bose-Hubbard dimer. The extremely clean fixed point and separatrix structure of this systems allows us for a detailed study of the mechanism, and the   analytical expectations of Sec.~\ref{sec:OTOC_theory} are verified to an excellent degree.  
