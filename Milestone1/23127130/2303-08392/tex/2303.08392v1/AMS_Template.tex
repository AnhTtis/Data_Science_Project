...
%\title{Mathematical aspects and simulated annealing for the Digital Annealer's Algorithm}
...
\begin{figure}
        ...
        \begin{subfigure}[b]{0.49\textwidth}
                \includegraphics[width=\textwidth]{SA_diagram.pdf}
                \caption{SA}
        \end{subfigure}
        \begin{subfigure}[b]{0.49\textwidth}
                \includegraphics[width=\textwidth]{DA_diagram.pdf}
                \caption{DA}
        \end{subfigure}
    \caption{Comparison between the spin updates performed by SA and DA.}
    \label{fig:SAvsDA}
\end{figure} 

The algorithm works as follows. Given a state $X_{k-1} = \sigmavec$ at time $k-1$, we propose a parallel-trial where each vertex $x$ is assigned as eligible to flip its spin value following the Metropolis criterion $\exp(-\beta_k E_x(\sigmavec)^+)$ corresponding to temperature ${\beta_k}^{-1}$. If the set $S$ of all eligible vertices contains at least one element, then a vertex $x$ is chosen uniformly at random from $S$, and we place $X_{k} = \sigmavec^{x}$; otherwise, nothing is done, and we consider $X_{k} = \sigmavec$. Note that, differently from \cite{DA2017,MatsubaraDA}, we are disregarding the escape mechanism so as to guarantee the Markov property holds. See Figure \ref{fig:SAvsDA} for a comparison between the spin update rule performed by SA compared to the DA. According to practical simulations \cite{DA2019,TISCIE2022,IEEE2023,DA2017,Fujitsu2017}, it is expected that, by properly decreasing the temperature toward zero at each step, the updated configuration approaches a ground state. Therefore, a good approximation for a ground state can be achieved, provided we run the algorithm for a sufficiently large number of steps. In this paper, we provide a mathematical foundation for these observations.
...
\end{equation}
where the first term in the right-hand side of the equation (\ref{eq:relation}) coincides with the standard single site spin-flip Metropolis dynamics transition probability from $\sigmavec$ to $\sigmavec^{x}$.  Therefore, as discussed in \cite{DA2019}, the DA, in fact, provides us with a larger spin-flip rate compared to the Metropolis dynamics, which may prove to be a decisive property that can guarantee greater effectiveness in relation to traditional methods.
...
On the other hand, note that in the proof of Theorem \ref{DAannealing2} the assumption that $H$ is given as in equation (\ref{eq:Hamiltonian}) was not necessary, therefore,  we do not have to restrict ourselves only to QUBO problems and Theorem \ref{DAannealing} can also be applied to search for ground states of Hamiltonians other than those written in this form. Furthermore, recall that we dealt with results regarding the asymptotic convergence to the ground states, and, in a realistic scenario, especially when dealing with problems that involve thousands of variables, one does not have infinite time to perform a simulation. Relying on ideas from Freidlin and Wentzel \cite{freidlin2012}, O. Catoni \cite{Catoni1991,Catoni1992,Cot1998} obtained rigorous results regarding finite time simulation, large deviation principles, and optimal cooling schedules for the Metropolis algorithm and Markov chains with rare transitions. Investigations on such topics specifically for the DA and other algorithms, such as those discussed in \cite{SCA21}, are still in progress.

Another topic that is also worth investigating is the behavior of the corresponding time-homogeneous Markov chain at a fixed temperature in order to provide a complete characterization of its stationary distribution and obtain estimates of its mixing time. As the reader can verify, providing an upper bound for the mixing time at high temperatures such as in \cite{SCA21,TISCIE2022}, where it was possible to show that the mixing times for certain probabilistic cellar automata is upper bounded by a quantity proportional to $\log |V|$, is not straightforward. Hence, there are still interesting directions to be explored.

\section*{Acknowledgements}
This work was supported by JST CREST Grant Number PJ22180021, Japan. 
