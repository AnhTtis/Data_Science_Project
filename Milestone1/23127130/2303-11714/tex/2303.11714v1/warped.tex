...
\begin{equation}
R_{abcd}\epsilon^{ab}{}_e\epsilon^{cd}{}_f=\frac{\Lambda^2}{\ell^2}g_{ef}+\frac{4(1-\Lambda^2)}{\Lambda^2\ell^4}W_eW_f,\label{first}
\end{equation}
...
&=&(\nabla_a\nabla^a-m^2+\frac{1}{8}R_{abcd}\gamma^{ab}\gamma^{cd})\psi,
\end{eqnarray}
...
Although the self-dual warped AdS black hole is more involved than the simplest BTZ black hole, our work demonstrates that the strategy previously developed in \cite{Zhang} for the algebraic approach to the spectrum quasinormal modes in the BTZ black hole turns out to be utterly applicable to the self-dual warped AdS black hole. As we know, there are other warped AdS black hole solutions with the $SL(2,R)\times U(1)$ isometry\cite{MCL,BC,ALPSS,CMN}
, where a variety of quasinormal modes have been calculated out mainly by solving the equations of motion analytically\cite{CX1,CX2,CN,CNX,LR}. With our present work, we are convinced that the derivation of these quasinormal modes can also be made as simple as possible by our algebraic approach.
...
\bibitem{KLS} D. Kapec, A. Lupsasca, and A. Strominger, arXiv: 2211.01674[gr-qc].
