...
furthermore, as in the Dijkgraaf-Witten model \cite{DW}, starting from a finite group $G$, we can
...
Finally, we attempt to make an application from the skew-racks above;
%As a modification of Property FR, we introduce skew-racks which yield link-invariants that are link-homotopic (Appendix \ref{SS233}). In another application, 
Precisely, Section \ref{SCG211} suggests elementary approaches to find some 3-manifolds, which are not the results of surgery of any knot in $S^3$; however, unfortunately, in this paper,
...
The author is grateful to Nozomu Sekino, Kimihiko Motegi and Motoo Tange for giving him valuable comments on Dehn surgery.
...
We introduce skew-racks, as a special class of biracks (see \cite{FR, CEGN} for the definition of biracks). We define {\it a skew-rack} to be a triple of a set $X$, a binary operation $\lhd: X \times X \ra X $, and a bijection $\kappa : X \ra X$ satisfying
\begin{enumerate}[(SR1)]
...
Define $(x,a) \lhd (y,b)$ by $ (f(x) y^{-1} by,f(a) ) $, and $\rho(x,a)= ( f(x), f(a)^{-1})$.
...
\begin{prop}\label{propss2}Let $(X,\lhd, \kappa) $ be a skew-rack satisfying $ \kappa^2 = \mathrm{id}_X $ as in (SS2).
Define the map $\mathrm{Tw}: X \ra X$ by setting $ \mathrm{Tw}(x) = \kappa(x)\lhd^{-1}\kappa(x)$ .
Then, the map is bijective, where the 
inverse is the map $X \ra X $ that sends $ x $ to $\kappa (x) \lhd x$. % is of $ $. 
%with an involution $\rho:X \ra X$ satisfying (SS1) and (SS2). Then, the map $\mathrm{Tw}$ is injective. In particular, if $X$ is of finite order, then $X$ satisfies (SS3).
\end{prop}
\begin{proof}
% We will show that the map $X \ra X $ that sends $ x $ to $\kappa (x) \lhd x$ is of $ $. 
Letting $y$ be $ \mathrm{Tw} ( \kappa (x)\lhd x  )$, % \lhd^{-1} ( \kappa (x)\lhd x )$,
we may show $y= x$. Notice that $x\lhd \kappa (x)= y \lhd (x \lhd \kappa (x))$, which is equal to 
\[  \bigl(( y\lhd^{-1}x) \lhd x \bigr) \lhd (x \lhd \kappa (x))= \bigl(( y\lhd^{-1}x) \lhd x \bigr)\lhd \kappa (x) =y \lhd \kappa (x). \] 
Thus, by (SS2), we have $y=x$. Similarly, we can easily check $\kappa (\mathrm{Tw} ( x))\lhd \mathrm{Tw} ( x) = x$, leading to the 
proof. 
%For this, we may show
...
%\end{equation}
%To show the injectivity of $\mathrm{Tw}$, suppose $ x \lhd^{-1} \kappa(x)= y \lhd^{-1} \kappa(y)$.Using \eqref{qq21}, we notice that %claim that implies $x=y$. Indeed, since\[ x \lhd^{-1} ( x \lhd^{-1} \kappa(x)) = x \lhd^{-1} \kappa(x) = y \lhd^{-1} \kappa(y )=y \lhd ( y \lhd^{-1} \kappa(y))= y \lhd^{-1} ( x \lhd^{-1} \kappa(x)) . \]
...
Finally, we should emphasize that the map $ \mathrm{Tw}^{ \pm 1}$ in Proposition \ref{propss2} corresponds
with an addition of a $(\mp 1)$-framing in an arc as in the Reidemeister move of type I. 
\begin{figure}[htpb]
\begin{center}
...
\begin{equation}\label{pp033} \mathrm{Ann}^{-1}(A_{a_1, \dots, a_n }) := \{ x \in X \mid \kappa^{n+1} (x) \lhd  \kappa \bigl( A_{a_1, \dots, a_n } (x) \bigr) = A_{a_1, \dots, a_n } (x) \}. \end{equation}
As the case $n=0$, we define $\Ann (X) $ to be the subset $\{ x \in X \mid x \lhd \kappa (x) = \kappa(x) \}$

%for $\mathrm{Ann}^{\pm 1}(A_{a_1, \dots, a_n }) $ for simplicity.
%$$ .$$%
...
\begin{enumerate}[(FR1)]
\item The subset $\Ann (X) $ is not empty, and 
bijective to $\mathrm{Ann}^{ \varepsilon} (A_{a_1, \dots, a_n }) $ for any $ a_1,\dots, a_n \in X$ and $ \varepsilon \in \{ \pm 1\}$.
% The cardinality of  is not zero, and does not depend on the choice of . % and $g_1, \dots, g_n \in \mathrm{Inn}^{\rm even}_{\kappa }(X) $.
\item For any $ a_1,\dots, a_n \in X $ and $x \in \mathrm{Ann}^{+1}(A_{a_1, \dots, a_n }) , y \in \mathrm{Ann}^{-1}(A_{a_1, \dots, a_n }) $, the equalities
\begin{equation}\label{pp0} \kappa^{n+i} (a_i)= A_{a_1, \dots, a_n } \bigl( \kappa^{i+1} (a_i) \lhd x \bigr), \end{equation}
\begin{equation}\label{pp03}
 \kappa^{n+i}(a_i) \lhd \kappa^{ n+1 }(y) = A_{a_1 \kappa (y), a_2 \lhd \kappa^2 (y), \dots, a_n \lhd \kappa^n (y) }(\kappa^{i+1} (a_i) ) \end{equation}
...
is $ \kappa^{n+1} (x) = A_{a_1, \dots, a_n }(x) \lhd \kappa^{n+1} (x)$; hence, $x \in \Ann (A_{a_1, \dots, a_n }) $.
...
if $ x \lhd^{\varepsilon} \kappa (x) = x \lhd^{\varepsilon } (x \cdot g)$ holds for any $x \in X, g \in \mathrm{Inn}^{\rm even}_{\kappa }(X) ,\varepsilon \in \{\pm 1 \} $.
...
\[ \mathcal{C}(\gamma) = \mathrm{Tw}^{-1}( \kappa (a \cdot g)) = (a \cdot g ) \lhd \kappa ( a \cdot g) , \ \ \ %and
\mathcal{C}(\delta) = \mathrm{Tw}^{-1}( \kappa (a ) ) \lhd ( a \cdot g) = (a \lhd \kappa (a) ) \lhd (a \cdot g). \]
...
\[ A_{a_1, \dots, a_n} (\kappa^{i-1} (a_i) \lhd x) = \kappa^{n+i}(a_i) \delta(\kappa^{n+i} (x)) \delta(\kappa^{n+i+1}(a_1)) \delta(\kappa^{n+i}(a_2)) \cdots \delta( \kappa^i (a_n)) = \kappa^{n+i+1}(a_i) . \]
...
Since $ \mathrm{Im}(\delta)$ is commutative, \eqref{key1} means $\delta (a ) =\delta( a \delta (b))$.
Thus,
$$ (z \lhd^{\varepsilon} \kappa(a))\lhd^{- \varepsilon } (a \cdot g) = z \delta (a )^{\varepsilon}   \delta( a \delta (b_1) \cdots \delta (b_n) )^{- \varepsilon}  =z. $$
...
the fixed-point subgroup $\{ h\in G \mid f ( h)=h\}$; see, e.g., \cite{BNN}.
...
Then, for a concrete group $K$, it is not so hard to determine the set $\mathrm{Col}_X(D_q^{\pm }) $ with the help of the computer program. %For example, we can check the following:\begin{prop}\label{prop333} Let $p, q$ be primes with $p,q <100$, and $K$ be $ \mathrm{SL}_2(\F_p)$ with $f= \id_K $. Then, the cardinality of $|\mathrm{Col}_X(D_q^{\pm}) | $ is equal to $ p^2 | \mathrm{SL}_2(\F_p)|$ if and only ifv$p= 2q \pm 1$.\end{prop}It is reasonable to hope that this proposition holds for any primes $p,q$.
For example, we give a list of some computation of $|\mathrm{Col}_X(D_q^{\pm }) | $; see Table \ref{444}.
...
3 & $ |K|$ & & $ |K|$ & & $ |K|$ & & $|K|$ & & $ |K|$ & & $ |K|$ \\ \hline
5 & $ 121|K|$ & & $ |K|$ & & $ 121|K|$ & & $ |K|$ & & $ |K|$ & & $ |K|$ \\ \hline
7 & $ |K|$ & & $ 337|K|$ & & $ |K|$ & & $ |K|$ & & $ |K|$ & & $ 57|K|$ \\ \hline
11 & $2641 |K|$ & & $ |K|$ & & $ 2641|K|$ & & $ 2641|K|$ & & $ |K|$ & & $ |K|$ \\ \hline
13 & $|K|$ & & $ 6553|K|$ & & $ |K|$ & & $ |K|$ & & $ |K|$ & & $ |K|$ \\ \hline
\end {tabular}
...
Furthermore, $\phi$ is said to be {\it $f$-link homotopic} if $X$ is $f$-link homotopic and the following holds for any
$a \in X$ and $g \in \mathrm{Inn}^{\rm even}_{\kappa}(X)$:
%and only if be
\begin{equation}\label{kkk4} \phi( ( a \cdot g) \lhd \kappa(a), a \cdot g ) + \phi( a, a\lhd \kappa(a)  ) = \phi(\kappa (a) , a \cdot g ) + \phi ( ( a \cdot g) \lhd \kappa(a), a  ) .\end{equation}
\end{defn}
...
These maps $\tilde{\lhd},\tilde{\kappa}$ define a skew-rack on $\X= X \times A $,
...
\begin{equation}\label{hhh} \phi (a\lhd b, \rho(b)) +\phi( (a\lhd b) \lhd c,\rho(c))= \phi( a \lhd \kappa (c) , \rho(\kappa(c)))+ \phi((a\lhd b)\lhd c, \rho(b\lhd c)). \end{equation}
Replace $ (a\lhd b) \lhd c , \rho (b \lhd c) $ and $, \rho (\kappa (c))$ by $a,b$, and $c$ respectively.
...
\[=\bigl((f(x) z^{-1} wz , f(a) +d+ \theta ( f(x), z^{-1})+ \theta ( f(x) z^{-1}, w z)+\theta(w,z) -\theta(z,z^{-1}) ), (y,b) \bigr) \in \tilde{G}. \]
...
As an application of the cocycle invariant, we will give two criteria to detect some 3-manifolds, which are not the results of surgery of any knot in $S^3$; see, e.g., \cite{HKMP}, \cite[Section 7.1]{AFW} and references therein for the details of such 3-manifolds, and such other criteria.
For this, as in Example \ref{exa28}, we fix groups $N \unlhd K$, and $X= K \times N$ with $f= \id_K $;
