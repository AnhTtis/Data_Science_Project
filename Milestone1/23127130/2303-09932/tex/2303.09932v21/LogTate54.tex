...
By the $p$-adic crystalline comparison isomorphism, we get $\alpha=[Z_{\mathbb C_p}]\in H_{et}^{2d}(X_{\mathbb C_p},\mathbb Q_p)$.
By standard Hilbert scheme argument (the Hilbert schemes are defined over $k$), 
$\alpha=[Z_{\mathbb C_p}]=[Z'_{\mathbb C_p}]\in H_{et}^{2d}(X_{\mathbb C_p},\mathbb Q_p)$, with $Z'\in\mathcal Z^d(X_{\bar k})\otimes\mathbb Q_p$.
Since $\alpha$ is $G$ invariant, we get
\begin{equation*}
\alpha=[Z']=[Z'']\in H_{et}^{2d}(X_{\bar k},\mathbb Q_p), 
...
Recall that a $p$ adic field $K$ is not algebraicaly closed, so we do not have a formula relating $NS^1(C\times C')$ to $\Hom_{\AbVar(K)}(C,C')$.
...
\item For $X\in\Sch$ a scheme, we denote by $\Pic(X)$ its Picard group of line bundle (or equivalently rational classes of Cartier divisors)
...
\end{equation*} 
Assume that moreover $\Pic(U^{\mathcal O}/p)=0$.
\begin{itemize}
...
\end{equation*}
where $r_1:V_1\to U^{\mathcal O}/p$ and $r_2:V_2\to U^{\mathcal O}/p$ are etale, $V_1$ and $V_2$ are affine, 
and $r_1:V_1\to r_1(V_1)$ and $r_2:V_2\to r_2(V_2)$ are finite etale.
...
Then, since $\Pic(U^{\mathcal O}/p)=0$, $U^{\mathcal O}/p\backslash r_1(V_1)=V(f_1)$, $U^{\mathcal O}/p\backslash r_2(V_2)=V(f_2)\subset U^{\mathcal O}/p$ 
are given by a single equation, this gives 
\begin{equation*}
...
\item[(i)']Let $r=(r_i:V_i\to U^{\mathcal O}/p)_{i\in I}$ be an etale cover in $\Var(O_K/p)$ with $V_i,i\in I$ affine and $r_i:V_i\to r_i(V_i)$ finite etale. Denote 
\begin{equation*}
...
Then, since $\Pic(U^{\mathcal O}/p)=0$, $U^{\mathcal O}/p\backslash r_i(V_i)=V(f_i)\subset U^{\mathcal O}/p$ are given by a single equation for each $i\in I$, this gives 
for each $i\in I$, $W_i\backslash V_i=V(f_i)\subset W_i$ and $O(W_i)=O_K/p[x_1,\cdots,x_d][k_1^{-1},\cdots,k_d^{-1}][e][e_i]$.
We choose (non canonical) splittings of $O_K/p$-vector spaces
\begin{eqnarray*}
...
\end{eqnarray*}
For each $p\in\mathbb N$ we get a (non canonical) splitting of complexes of $\mathbb Z/p$ modules
...
\end{eqnarray}
\item[(ii)]Consider a cartesian square in $\Sch^{int,sm}/O_K$
\begin{equation*}
\xymatrix{V^{\mathcal O}_1\times_{U^{\mathcal O}}V^{\mathcal O}_2\ar[d]^{p_1}\ar[rr]^{p_2} & \, & \ar[d]^{r_2}V^{\mathcal O}_2 \\
V^{\mathcal O}_1\ar[rr]^{r_1} & \, & U}
\end{equation*}
where $r'_1:V^{\mathcal O}_1\to U^{\mathcal O}$ and $r_2:V^{\mathcal O}_2\to U^{\mathcal O}$ are etale, $V^{\mathcal O}_1$ and $V^{\mathcal O}_2$ are affine, 
and $r'_1:V^{\mathcal O}_1\to r'_1(V^{\mathcal O}_1)$, $r'_2:V^{\mathcal O}_2\to r'_2(V^{\mathcal O}_2)$ are finite etale.
We denote $V_1:=V^{\mathcal O}_1\otimes_{O_K}K$ and $V_2:=V^{\mathcal O}_2\otimes_{O_K}K$.
Then the cotangent bundles $\Omega^1_{V_1}\in\Shv_O(V_1)$, $\Omega^1_{V_2}\in\Shv_O(V_2)$, 
...
We also take a (non canonical) splitting of $O_K/p$-vector spaces
\begin{equation*}
\Omega^1(\hat U)=\tilde\Omega^1_{U^{\mathcal O},\log}(\hat U)\oplus NL(\hat U)
\end{equation*}
...
Then, since $\Pic(U^{\mathcal O}/p)=0$,
\begin{equation*}
...
W^{\mathcal O}_1\times_{U^{\mathcal O}}W^{\mathcal O}_2\backslash V^{\mathcal O}_1\times_{U^{\mathcal O}}V^{\mathcal O}_2=
V(f_1f_2)\subset W^{\mathcal O}_1\times_{U^{\mathcal O}}W^{\mathcal O}_2, 
\end{equation*}
are given by a single equation, and $f_1=f_{1,1}\cdots f_{1,r}$, $f_{1,j}\in O(W^{\mathcal O}_1)$ irreducible, 
$f_2=f_{2,1}\cdots f_{2,s}$, $f_{2,j}\in O(W^{\mathcal O}_1)$ irreducible, and
We have then
\begin{eqnarray*}
...
We then have
\begin{eqnarray*}
r^*\Omega^{1,an}_{/O_K}(\hat V_1)=r^*\tilde\Omega^{1,an}_{/K,\log,\mathcal O}(V_1)\oplus NL(V_1), \; 
r^*\Omega^{1,an}_{/O_K}(\hat V_2)=r^*\tilde\Omega^{1,an}_{/K,\log,\mathcal O}(V_2)\oplus NL(V_2) , \\
r^*\Omega^{1,an}_{/O_K}(\hat V_1\times_{\hat U}\hat V_2)=r^*\tilde\Omega^{1,an}_{/K,\log,\mathcal O}(V_1\times_{U}V_2)\oplus NL(V_1\times_UV_2),  \\
\Omega(p_1)(NL(V_1))\subset NL(V_1\times_UV_2),  \; \Omega(p_2)(NL(V_2))\subset NL(V_1\times_UV_2)
\end{eqnarray*}
and
\begin{eqnarray*}
r^*\Omega^1_{\hat U^{\mathcal O,et}}(V_1)=r^*\tilde\Omega^1_{\hat U,\log,\mathcal O}(V_1)\oplus NL(V_1), \;
r^*\Omega^1_{\hat U^{\mathcal O,et}}(V_2)=r^*\tilde\Omega^1_{\hat U,\log,\mathcal O}(V_2)\oplus NL(V_2) , \\
r^*\Omega^1_{\hat U^{\mathcal O,et}}(V_1\times_UV_2)=r^*\tilde\Omega^1_{\hat U,\log,\mathcal O}(V_1\times_{U}V_2)\oplus NL(V_1\times_UV_2),  \\
\Omega(p_1)(NL_a(V_1))\subset NL(V_1\times_UV_2),  \; \Omega(p_2)(NL(V_2))\subset NL(V_1\times_UV_2),
\end{eqnarray*}
where $p_1:V_1\times_{U}V_2\to V_1$ and $p_2:V_1\times_{U}V_2\to V_2$ are the morphisms given by base change.
\item[(ii)']Let $r=(r_i:V^{\mathcal O}_i\to U^{\mathcal O})_{i\in I}$ be an etale cover in $\Sch^{int,sm}/O_K$, 
with $V^{\mathcal O}_i$ affine and $r_i:V^{\mathcal O}_i\to r_i(V^{\mathcal O}_i)$ finite etale for each $i\in I$. Denote 
\begin{equation*}
V^{\mathcal O}_{\bullet}\in\Fun(\Delta(I),\Sch^{\int,sm}/O_K), \; 
V^{\mathcal O}_{i_1,\cdots,i_l}:=V^{\mathcal O}_{i_1}\times_{U^{\mathcal O}}\cdots\times_{U^{\mathcal O}}V^{\mathcal O}_{i_l}
\end{equation*}
and $V_{\bullet}:=V^{\mathcal O}_{\bullet}\otimes_{O_K}K\in\Fun(\Delta(I),\SmVar(K))$ the associated Cech simplicial schemes. 
Take compactification for each $i\in I$,
\begin{equation*}
r_1:V^{\mathcal O}_i\xrightarrow{j_i}W^{\mathcal O}_i\xrightarrow{\bar r_i} U^{\mathcal O},
\end{equation*}
where $j_i$ are open embeddings and $\bar r_i$ finite (and surjective) morphisms.
Since $\Pic(U^{\mathcal O}/p)=0$, for each $i\in I$, $W^{\mathcal O}_i\backslash V^{\mathcal O}_i=V(f_i)\subset W^{\mathcal O}_i$ are given by a single equation 
and 
\begin{equation*}
O(\hat W_i)=K\{x_1,\cdots,x_d\}[e,e_i][k_1^{-1},\cdots,k_d^{-1}].
\end{equation*}
We choose (non canonical) splittings of $O_K/p$-vector spaces
\begin{eqnarray*}
...
\begin{equation*}
NL(V_{\bullet})\subset\Omega^1_{\hat U^{et}}(V_{\bullet})=r^*\Omega^{1,an}_{/O_K}(V_{\bullet})
\end{equation*}
satisfaying
\begin{eqnarray*}
r^*\Omega^1_{\hat U^{et}}(V_{\bullet})=r^*\tilde\Omega^1_{\hat U,\log,\mathcal O}(V_{\bullet})\oplus NL(V_{\bullet}), \;
r^*\Omega^{1,an}_{/O_K}(V_{\bullet})=r^*\tilde\Omega^{1,an}_{/K,\log,\mathcal O}(V_{\bullet})\oplus NL(V_{\bullet}).
\end{eqnarray*}
For each $p\in\mathbb N$ we get (non canonical) splittings of complexes of $\mathbb Z_p$ modules
\begin{eqnarray}\label{splitO1}
r^*\Omega^p_{\hat U^{\mathcal O,et}}(V_{\bullet})=r^*\tilde\Omega^p_{\hat U,\log,\mathcal O}(V_{\bullet})\oplus\Omega^p_{\hat U^{et},nl}(V_{\bullet}), \\ 
\Omega^1_{\hat U^{et},nl}(V_{\bullet}):=NL(V_{\bullet}), \, 
\Omega^p_{\hat U^{et},nl}(V_{\bullet}):=NL(V_{\bullet})\wedge r^*\Omega^{p-1}_{\hat U^{\mathcal O,et}}(V_{\bullet}).
\end{eqnarray}
and
\begin{eqnarray}\label{splitO2}
r^*\Omega^{p,an}_{/O_K}(V_{\bullet})=r^*\tilde\Omega^{p,an}_{/K,\log,\mathcal O}(V_{\bullet})\oplus\Omega^p_{\hat U^{et},nl}(V_{\bullet}), \\ 
\Omega^1_{\hat U^{et},nl}(V_{\bullet}):=NL(V_{\bullet}), \, 
\Omega^p_{\hat U^{et},nl}(V_{\bullet}):=NL(V_{\bullet})\wedge\Omega^{p-1}_{\hat U^{et}}(V_{\bullet}).
\end{eqnarray}
\end{itemize}
...
with $e:U^{\mathcal O}\to e(U^{\mathcal O})$ finite etale and $\Pic(U^{\mathcal O}/p)=0$.
\end{lem}
...
Then the complementary open subset $U^{\mathcal O}:=\bar{V^{\mathcal O}}\backslash R$ is finite etale over $e(U^{\mathcal O})$. 
Moreover since $O_K/p$ is a finite field, $\Pic(Y)$ is finite for any $Y\in\SmVar(O_K/p)$ 
since $\Pic^s(Y)(O_K/p)$ is a finite group where $\Pic^s(Y)$ is the Picard scheme. Hence, up to shrinking $U^{\mathcal O}$, 
we may assume that $\Pic(U^{\mathcal O}/p)=0$.
\end{proof}
...
is an integral model of $U$, in particular $U^{\mathcal O}\otimes_{O_K}K=U$ and such that $\Pic(U^{\mathcal O}/p)=0$.
...
\begin{proof}
\noindent(i0): 
By proposition \ref{oetetprop}, we have
\begin{equation*}
H^q_{et}(U,r^*\Omega^{p,an}_{/K,\log,\mathcal O})=H^q_{et}(U^{\mathcal O},\Omega^{p,an}_{/K,\log,\mathcal O})=H^q_{et}(U^{\mathcal O}/p,\Omega^{p,an}_{/K,\log,\mathcal O}), 
\end{equation*}
and
\begin{equation*}
H^q_{et}(U,r^*\Omega^p_{\hat U,\log,\mathcal O})=H^q_{et}(U^{\mathcal O},\Omega^p_{\hat U,\log,\mathcal O})=H^q_{et}(U^{\mathcal O}/p,\Omega^p_{\hat U,\log,\mathcal O}). 
\end{equation*}
Hence it is enough to show that $H^q_{et}(U^{\mathcal O}/p,\Omega^{p,an}_{/K,\log,\mathcal O})=0$ and $H^q_{et}(U^{\mathcal O}/p,\Omega^p_{\hat U,\log,\mathcal O})=0$
for $q\neq 0$.
...
\end{eqnarray*}
where $s.et;cov$ means standard etale vover (i.e $r_i$ are etale, $X_i$ are affine and $r_i:X_i\to r_i(X_i)$ finite etale) 
that is Chech etale cohomology coincide with etale cohomology.
Since $U^{\mathcal O}\in\Sch^{\int}/O_K$ is affine, 
we have for $(r_i:V^{\mathcal O}_i\to U^{\mathcal O})_{i\in I}$ a standard etale cover in $\Sch^{\int}/O_K$, $H^q\Omega^p_{\hat U^{et}}(V_{\bullet})=0$, 
since $\Omega^p_{\hat U^{et}}\in\PSh_O(U^{et})$ a (quasi)coherent $O(\hat U^{et})$-module.
Let $(r_i:V^{\mathcal O}_i\to U^{\mathcal O})_{i\in I}$ be a standard etale cover in $\Sch^{int}/O_K$. Denote  
\begin{equation*}
V^{\mathcal O}_{\bullet}\in\Fun(\Delta(I),\Sch^{\int,sm}/O_K), \; 
V^{\mathcal O}_{i_1,\cdots,i_l}:=V^{\mathcal O}_{i_1}\times_{U^{\mathcal O}}\cdots\times_{U^{\mathcal O}}V^{\mathcal O}_{i_l}
\end{equation*}
and $V_{\bullet}:=V^{\mathcal O}_{\bullet}\otimes_{O_K}K\in\Fun(\Delta(I),\SmVar(K))$ the associated Cech simplicial schemes.
Since the cotangent bundle $\Omega^1_{U^{\mathcal O}}\in\Shv_O(U^{\mathcal O})$ of $U^{\mathcal O}$ is trivial 
as $U^{\mathcal O}$ admits an etale map to $\mathbb A_{O_K}^{d_U}$ and since $\Pic(U^{\mathcal O}/p)=0$, 
we have the (non canonical) splittings of $O_K$ modules (\ref{splitO1}) and (\ref{splitO2})
\begin{eqnarray*}
r^*\Omega^p_{\hat U^{\mathcal O,et}}(V_{\bullet})=r^*\tilde\Omega^p_{\hat U,\log,\mathcal O}(V_{\bullet})\oplus\Omega^p_{\hat U^{et},nl}(V_{\bullet}), \; 
...
\begin{eqnarray*}
H^qr^*\Omega^p_{\hat U^{\mathcal O,et}}(V_{\bullet})=H^qr^*\tilde\Omega^p_{\hat U,\log,\mathcal O}(V_{\bullet})\oplus H^q\Omega^p_{\hat U^{et},nl}(V_{\bullet}), \\ 
...
and
\begin{eqnarray*}
H^qr^*\Omega^{p,an}_{/O_K}(V_{\bullet})=H^qr^*\tilde\Omega^{p,an}_{/K,\log,\mathcal O}(V_{\bullet})\oplus H^q\Omega^p_{\hat U^{et},nl}(V_{\bullet}), \\ 
...
\noindent(i):By a standard result of Bhatt and Scholze (proposition 5.6.2 of \cite{BSch}), 
we have for $X\in\Var(K)$ and $X^{\mathcal O}\in\Sch^{int}/O_K$ and integral model of $X$,
\begin{equation*}
...
is surjective locally for the etale topology (take a finite cover of $X$ by w-contractile schemes and use Mitag-Lefter).
...
as $U^{\mathcal O}$ admits an etale map to $\mathbb A_{O_K}^{d_U}$ and since $\Pic(U^{\mathcal O}/p)=0$, we have (non canonical) splittings (\ref{splitO})
\begin{eqnarray*}
...
such that there exists an etale map $e_i:X^{\mathcal O}_i\to G_m^{d_{X_i}}\subset\mathbb A_{O_K}^{d_{X_i}}$ 
with $e_i:X^{\mathcal O}_i\to e_i(X^{\mathcal O}_i)$ finite etale and such that $\Pic(X_i^{\mathcal O}/p)=0$. 
...
(or take $X_i$ such that there exists an etale map $e_i:X_i\to\mathbb A^{d_X}$ and such that $\Pic(X_i)=0$ 
which works for all $w$ as in proposition \ref{UXpet} : see remark \ref{UDR} ),
...
Let $X\in\SmVar(k)$. Then for each $x\in X$ there exists a Zariski open affine subset $U\subset X$ such that 
\begin{itemize}
\item there exists an etale map $e:U\to V$ where $V\subset\mathbb A^d_k$ be a Zariski open subset (consider a finite morphism $\bar e:X\to\mathbb P^{d_X}$). 
Then the tangent bundle is trivial of $U$ is trivial.
\item $\Pic(U)=0$ by taking for $U$ an elementary fibration so that $U$ is a $K(\pi,1)$ and $\Pic(U)=\Pic(U_{\mathbb C}^{an})=0$ by the exponential exact sequence
after taking an embedding $k\hookrightarrow\mathbb C$.
\end{itemize}
Then we can see using a (non canonical) splitting as in the proof of proposition \ref{UXpet} that 
for each $p,q\in\mathbb Z$, $q\neq 0$, $p\neq 0$, $H_{et}^q(U,\Omega^p_{U,\log})=0$. In particular, for $Z\in\mathcal Z^k(U)$, $[Z]=0\in H^{2k}_{DR}(U)$. 
\end{rem}
...
such that there exists a finite etale map $e:X^{\mathcal O}\to e(X^{\mathcal O})\subset\mathbb A^{d_X}_{O_K}$
to an open subset of the affine space and such that $\Pic(X^{\mathcal O}/p)=0$.
...
with $e:U^{\mathcal O}\to e(U^{\mathcal O})$ finite etale and such that $\Pic(U^{\mathcal O}/p)=0$, 
...
with $e_i:X^{\mathcal O}_i\to e_i(X^{\mathcal O}_i)$ finite etale and $\Pic(X_i^{\mathcal O}/p)=0$. 
...
\end{equation*}
of the diagram (\ref{Frob}) are exact. Hence, since the rows of this diagram (\ref{Frob}) are exact,
the remaining columns are also exact. This proves the proposition.
\end{proof}
...
such that $w(\alpha)=[Z]\in H^{2d}_{DR}(X_{\hat k_{\sigma_p}})$. 
Since the Hilbert schemes of $X$ are defined over $k$, there exists $Z'\in\mathcal Z^d(X_{\bar k})\otimes\mathbb Q_p$
such that $w(\alpha)=[Z]=[Z'_{\hat k_{\sigma_p}}]\in H^{2d}_{DR}(X_{\hat k_{\sigma_p}})$.
By \cite{ChinoisCrys},
...
\ar[rrr]^{B_{cris,X}(\gamma^{\vee}_{Z'})}\ar[d]^{Ra_{X*}\Gamma_{Z'}\alpha(X)} & \, & \, &
...
H^{2d}_{DR,Z'_{\hat k_{\sigma_p}}}(X_{\hat k_{\sigma_p}})\otimes_{\hat k_{\sigma_p}}\mathbb B_{cris}
\ar[rrr]^{DR(X)(OB_{cris,X})(\gamma^{\vee}_{Z'})} & \, & \, &
...
\begin{rem}\label{NS}
Let $K\subset\bar{\mathbb Q_p}$ be a $p$-adic field. 
...
Denote $(E^{2d}_{DR}(\mathcal X/S),F):=H^{2d}\int_f(O_{\mathcal X},F_b)\in\Vect_{fil\mathcal D}(S)$ and
\begin{equation*}
HL^{d,2d}(\mathcal X/S):=
