...
\begin{itemize}
    ...
    \item In Section~\ref{subsec:supp_cost}, we investigate the computational cost for our \SPTl{} during fine-tuning and inference.
...
\vspace{-0.75em}
\item\adapter{}-$k$~\cite{houlsby2019parameter}: freezes all the backbone parameters while adding a down projection, a ReLU~\cite{hendrycks2016gaussian} non-linearity, and an up projection layer sequentially in the feed-forward network (FFN) of each visual Transformer block. We follow the training details of~\cite{zhang2022neural} to achieve better performance.
\vspace{-0.75em}
\item \lora{}-$k$~\cite{hu2022lora}: freezes all the backbone parameters while adding a concurrent branch including two low-rank matrices to the weight matrices in the multi-head self-attention layers to approximate efficiently updating them. The low-rank matrices can be merged into the backbone weights after fine-tuning. We follow the training details of~\cite{zhang2022neural} to achieve better performance.
\vspace{-0.75em}
...
\label{subsec:supp_cost}

We investigate the computational cost of \SPTl{} by comparing with full fine-tuning, addition-based method \deepprompt{}, and reparameterization-based method \lora{}-16. 
The results are presented in Table~\ref{tab:comsumption}, which is evaluated with image resolution 224$\times$224 on a single GeForce 3090 GPU. We observe that \deepprompt{} has higher inference latency and inference GPU memory due to the additional prompts. In contrast, since the updated parameters after fine-tuning can be reparameterized and merged into the pre-trained model, our \SPTl{}, \SPTl{} w/o unstructured tuning, and \lora{}-16 are more efficient than \deepprompt{} during inference. 
However, we observe that our \SPTl{} has slightly higher fine-tuning memory than the full fine-tuning and \lora{}-16 which is taken up by masking the gradients in Eq.~(5) of the main paper. Therefore, we argue that slightly more fine-tuning memory (0.6G higher than the full fine-tuning) is affordable and saving fine-tuning memory is not the main purpose of parameter-efficient tuning. Nevertheless, for circumstances that require a low fine-tuning memory, one can employ any structured tuning method such as LoRA to the sensitive weight matrices identified by our sensitivity criterion to achieve similar fine-tuning memory but much higher performance compared with the task-agnostic VPET methods, as shown in Figure 3 (b) and Table 4 of the main paper.
...
\centering
    ...
    \toprule
     \bf{Method}  &  \bf{Inference Latency (ms/img)}  & \bf{Inference Memory (GB)} & \bf{Fine-tuning Memory (GB)} \\ 
    \midrule
    ...
    \SPTl{}  & \textbf{2.8} & \textbf{1.3} & 12.5 \\
    \bottomrule
    \end{tabular}%
    }
        
\caption{Cost comparisons with ViT-B/16 backbone on the \vtab{}~\cite{zhai2019vtab} benchmark. The VPET methods are evaluated with around 0.70\% fractions of the trainable parameters. We report the latency (ms/img) and the peak memory usage (GB).
}
...
We show more parameter sensitivity patterns for ViT-B/16 with different pre-training strategies on six sampled tasks from VTAB-1k. We visualize the proportions of the sensitive parameters under 0.4M trainable parameter budget. Visualizations of block-wise sensitivity are shown in Figures~\ref{fig:sensitive_sup},~\ref{fig:sensitive_mae}, and~\ref{fig:sensitive_moco} and visualizations of operation-wise sensitivity are shown in Figure~\ref{fig:sens_ssl_op}. We observe that 1) given a pre-trained backbone, the patterns of the sensitive parameter proportions diverse greatly; and 2) query and key projections $\mW_{q}$ and $\mW_{k}$ are the most insensitive weights in all cases. The findings align with our observation in Section 4.4 of the main paper, suggesting that our observations are general for ViT-B/16 with various pre-training strategies.
\vspace{0.5em}

We also compare the variances of the parameter sensitivities across the \vtab{} tasks for ViT-B/16 with different pre-training strategies. The results are visualized in Figure~\ref{fig:variance}. We observe that the self-supervised pre-trained backbones have higher sensitivity variances than the supervised pre-trained one across the 19 downstream tasks. In particular, the variance of ViT-B/16 pre-trained with MAE~\cite{he2022masked} is twice as large as that of the supervised pre-trained ViT-B/16. We speculate that our SPT variants can better handle the large variances for self-supervised pre-trained backbones (Table 2 of the main paper) by identifying task-specific positions to introduce the trainable parameters.

\begin{figure}[htb]
\begin{center}
    \includegraphics[width=\linewidth]{pics/sensitive_sup.pdf}
\end{center}
...
\begin{figure}[htb]
\begin{center}
    \includegraphics[width=\linewidth]{pics/sensitive_mae.pdf}
\end{center}
...
\begin{figure}[tb]
\begin{center}
    \includegraphics[width=\linewidth]{pics/sensitive_moco.pdf}
\end{center}
...
\begin{figure}[tb]
\begin{center}
    \includegraphics[width=\linewidth]{pics/sens_ssl_op.pdf}
\end{center}
\caption{The proportions of the sensitive
parameters for each operation with 0.4M trainable parameter budget for \mae{}~\cite{he2022masked} (left) and \moco{}~\cite{chen2021empirical} (right) pre-trained ViT-B/16 on six sampled tasks from VTAB-1k~\cite{zhai2019vtab}. We show the proportions of the sensitive
...
\label{fig:sens_ssl_op}
\end{figure}

\begin{figure}[htb]
\begin{center}
    \includegraphics[width=0.5\linewidth]{pics/variance.pdf}
\end{center}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.6\linewidth]{pics/natural_structured.pdf}
\end{center}
