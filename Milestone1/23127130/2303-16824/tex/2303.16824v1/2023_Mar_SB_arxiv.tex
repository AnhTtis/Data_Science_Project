...
\begin{equation}
	\tilde{\kappa}=\tilde{\kappa} (x, y):={n \choose 2}^{-1}\sum_{1\leq i < j \leq n}\tilde{h}_{\hat{F}_1}(X_i, X_j)\tilde{h}_{\hat{F}_2}(Y_i, Y_j), \ \ \text{and}\ \ \tilde{\rho}=\tilde{\rho}(x,y):=\frac{\tilde{\kappa}(x,y)}{\sqrt{\tilde{\kappa}(x,x)\tilde{\kappa}(y,y)}},\label{rhou}
\end{equation}
...
\begin{equation}\label{kappahathdecom}n^{1/2}\big(\tilde{\kappa}-\kappa\big)=n^{-1/2}\sum_{i=1}^{T}H_1(X_i, Y_i)+R_n, \ \text{where}\ \  
...
Indeed, \begin{equation}
	\tilde{\kappa}={n \choose 2}^{-1}\sum_{1\leq i<j \leq n}h_{F_1}(X_i, X_j)h_{F_2}(Y_i, Y_j)+{R}_n, \ \text{where}\ \ n{R}_n\xrightarrow{P}0.
\end{equation}\label{hdecom2}
...
\section{The $S_B$ measure of association}\label{sec:sb} Now suppose there are $R$ regions and let $X_i$ denote a variable corresponding to the region $i=1,\ldots, R$. Then we can use the similarity measure $\rho$, to define a global spatial association measure (\textbf{Spatial Bergsma}) as 
\begin{equation}
...
\end{equation}
Define an ${R \choose 2} \times 1$ column vector $\mathbf{C}$, and a $1\times {R \choose 2}$  row vector $\mathbf{d}$ as
...
\begin{proof}
	...
	\begin{eqnarray}
		...
		&=&{T \choose 2}^{-1}\sum_{1\leq m < n\leq T}\tilde{h}_{\hat{F}_i}(x_{im},x_{in})\tilde{h}_{\hat{F}_j}(x_{jm},x_{jn}).
	\end{eqnarray}
	...
	%Theorem \ref{MUCLT},
	\begin{equation}
		\tilde{\mathbf{C}}_{{R \choose 2}\times 1}=\big(\sqrt{T}\big(\tilde{\kappa}^{(ij)}-{\kappa}^{(ij)}, 1\leq i < j \leq R\big) \xrightarrow{D} N\big(\mathbf{0},\Sigma_{{R \choose 2} \times {R \choose 2}}\big), \label{numsb}
	\end{equation}
...
\subsection{Asymptotic distribution of $S_B$ under spatial pairwise independence}\label{sec:sbasymnull}
...
Recall from \eqref{sbdef} that,
	...
	Further we have
	\begin{equation}
		\tilde{\kappa}^{(ij)}=\tilde{\kappa}(X_i^{T},X_j^{T})={T \choose 2}^{-1}\sum_{1\leq m<n \leq T}\tilde{h}_{\hat{F}_i}(x_{im},x_{in})\tilde{h}_{\hat{F}_j}(x_{jm},x_{jn}). \label{kappaij}
	\end{equation}
As mentioned earlier, due to independence, the first projections $H_1^{(ij)}$ in \eqref{hdecom1} are zero. We now use the second projections from Equation \eqref{hdecom2}. 
	...
	%		 with $R_n=O(n^{-3})$
	\begin{equation*}
		\tilde{\kappa}^{(ij)}={T \choose 2}^{-1}\sum_{1\leq m<n \leq T}{h}_{{F_i}}(x_{im},x_{in}){h}_{{F_j}}(x_{jm},x_{jn})+{R}^{(ij)}_T,
...
\caption{Simulated null distribution, $T=50$, $R=14$ for $10,000$ replicates, for six distributions. Proximity matrices $W$: lag-$1$ adjacency (left), 
...
\caption{Comparison of simulated null (black-solid line) and asymptotic null distributions (red-dashed-line) of $T*S_B$ with $10,000$ replicates, based on finite discrete approximation using 100 eigenvalues for aymptotic distribution, with $W$ matrices: lag-$1$ adjacency (left), inverse distance (middle), and linearly arranged spatial units (right). }
...
\caption{Empirical distributions of $S_{B}$, $R$=14, $T$=50, $10,000$ replicates, with sample mean overlayed, dependence parameter $\theta$ on $x$-axis under SMA model (top), SAR model (bottom). Proximity matrices: lag-$1$ adjacency (left), inverse distance (middle), and linearly arranged spatial units (right).}
\label{fig:boxplots}
\end{figure}
...
In this section we present an application of $\tilde{S}_B$ to the COVID19 data from the Indian state of Kerala in the southern part of India. We consider district-level monthly COVID-19 data for the 14 districts of Kerala as of October 31, 2021. The data is obtained from a crowd-sourced database \url{https://data.incovid19.org}. We have the number of districts $R=14$ and the number of time points $T=19$ (months). 
...
Let $x_{it}$=number of new cases for the $i$th district during the time period $t=1, \ldots, 19 (=T)$, $i=1, \ldots, 14 (=R)$. Then the models are defined as follows.\vskip5pt
...
\begin{equation}
\log (\mu_{it})=\beta^{(i)}_{0}+\beta^{(i)}_{1} x_{i(t-1)}+\sum_{j \in N_i}\beta^{(i)}_{{2,j}}w_{ij}x_{j(t-1)},\hspace{0.5cm}t=1,\ldots,19,\hspace{0.5cm}j=1,\ldots,14. \label{SpAR}
\end{equation}
...
\end{equation}
The first part of the model with parameters $\beta_{1,k}$ is adopted in the spirit of gravity models and explains the effect of the volume of air passenger traffic on the district level COVID-19 incidence. The (inverse) effect distance between airports and district headquarters is captured by a distance decay function. Based on the results from \cite{Bhattacharjeeetal}, we use two decay function, the inverse decaly $f(d)=1/d$, and exponential decay $f(d)=\exp(-d)$. 
%\vskip-10pt
\begin{figure}[ht]
\centering
...
spatio-temporal modelling of COVID-19 data on the monthly time series data for the 14 districts of the Indian state of Kerala.  
