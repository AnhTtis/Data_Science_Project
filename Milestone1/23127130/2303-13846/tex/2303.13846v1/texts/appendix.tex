...
As shown by the results in Table~\ref{table:tinyimagenet}, we observed 2.08\% improvement on average for Ensemble robustness compared to vanilla methods, which is significantly higher than that of CIFAR-100 (0.67\%, Table~\ref{table:cifar100}) and on par with CIFAR-10 (2.20\%, Table~\ref{table:robustness-resnet}) and SVHN (2.30\%, Table~\ref{table:robustness-vgg}). 
...
In addition to ResNet-18 and VGG16, we also evaluate our FSR module on WideResNet-34-10.
...
As explained in Sec.~\ref{ssec:3.1}, in order to disentangle the non-robust activations through the separation loss $\mathcal{L}_{sep}$ (Eq.~\ref{eq:l_sep}):
\begin{equation}
...
\begin{table}
	...
	\end{center}
        \vspace{-3mm}
	\caption{
	    Comparison of accuracy (\%) for different design choices of the separation loss $\mathcal{L}_{sep}$ (Eq.~\ref{eq:l_sep}).
	}
	\label{table:label}
\end{table}
...
We compare the robustness upon replacing $m$ with a binary mask $b$ (Sec.~\ref{ssec:3.1}) that divides the activations in a discrete manner.
...
In order to verify whether our Separation Net is learning appropriate robustness scores for each feature activaiton, we tried replacing the output mask $m$ from the Separation Net (Eq.~\ref{eq:mask}) with different strategies.
...
Table~\ref{table:separation} shows that both strategies significantly lag behind our method without Separation, which is equivalent to recalibrating all activations (refer to Table~\ref{table:abl-sep}). 
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[t]{0.33\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/lam_sep.pdf}
        \caption{$\lambda_{sep}$}
        \label{fig:lam-sep}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{0.33\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/lam_rec.pdf}
        \caption{$\lambda_{rec}$}
        \label{fig:lam-rec}
    \end{subfigure}
    \caption{
...
We also compare the robustness as we vary the temperature $\tau$ (Eq.~\ref{eq:mask}) that controls how ``discrete" the mask is.
...
In Fig.~\ref{fig:lam-sep} and Fig.~\ref{fig:lam-rec}, we visualize the trends of model robustness as we vary the weights on our proposed loss functions $\mathcal{L}_{sep}$ (Eq.~\ref{eq:l_sep}) and $\mathcal{L}_{rec}$ (Eq.~\ref{eq:l_rec}).
...
Tables~\ref{table:robustness-resnet} and~\ref{table:black-box} show the robustness of our method under both white-box and black-box attacks when applied to ResNet-18 on the CIFAR-10 dataset.
