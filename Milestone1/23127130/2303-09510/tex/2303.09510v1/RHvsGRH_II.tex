...
\thanks{MSC Numbers: 11M06, 11M26.}

\thanks{Keywords: Generalized Riemann Hypothesis, Riemann zeta function.}
...
\bigskip\noindent{\sc Remark.} In place of \eqref{eq:superbound},
...
Thus, statement $(ii)$ is consistent the 
results of \cite{Banks}.

\section{Preliminaries}

\subsection{Notation}

As is customary, we put $\e(u)\defeq \er^{2\pi iu}$
for all $u\in\R$. We use $\fl{u}$ to denote the
largest integer not exceeding $u$, and $\{u\}\defeq u-\fl{u}$
is the fractional part of $u$.

As in  Theorem~\ref{thm:RHvsGRH}, $C_\euB$ always
denotes the constant $\int_0^\infty\euB(u)\,du$.
...
$\psi(x)\defeq\sum_{n\le x}\Lambda(u)$ is known to satisfy
\be\label{eq:Chebest}
...
Assume RH. For any $\xi\in\R^+$ and $T\ge 100$, we have
$$
\ssum{\rho=\frac12+i\gamma\\0<\gamma\le T}\xi^{-\rho}\cX(1-\rho)
...
For $u>1$, let
$$
\Sigma_1(u)\defeq\ssum{\rho=\frac12+i\gamma\\0<\gamma\le u}
...
\ee
In proving \eqref{eq:Sigsum}, it suffices to assume 
 $T$ has this property. Indeed, using \eqref{eq:X1-s}
we see that $\cX(1-\rho)\ll 1$ for all complex zeros of $\zeta(s)$
(under RH), so that
$$
\big|\Sigma_1(T_*)-\Sigma_1(T)\big|\le
\ssum{\rho=\frac12+i\gamma\\T_*<\gamma\le T_*+1}
\big|\xi^{-\rho}\cX(1-\rho)\big|\,\mathop{\ll}\limits_\xi\,\log T,
$$
and we also have
$$
\big|\Sigma_2(T_*/2\pi\xi)-\Sigma_2(T/2\pi\xi)\big|\le
\ssum{T_*/2\pi\xi<n\le (T_*+1)/2\pi\xi}\big|\Lambda(n)\e(-n\xi)\big|\,\mathop{\ll}\limits_\xi\,\log T.
$$
Consequently, if the bound \eqref{eq:Sigsum} holds 
for all $T$ satisfying \eqref{eq:LD-horiz}, then
$$
\Sigma_1(T_*)+\Sigma_2(T_*/2\pi\xi)
\,\mathop{\ll}\limits_\xi\,T_*^{1/2}\log^2T_*
$$
holds for arbitrary $T_*\ge 100$.

From now on, we assume that $T$ satisfies \eqref{eq:Sigsum}.
...
-\xi^{-c}\sum_n\frac{\Lambda(n)}{n^c}T^{c-1/2}
-\xi^{-c}\sum_n\frac{\Lambda(n)}{n^c}
\frac{T^{c+1/2}}{|T-2\pi n\xi|+T^{1/2}}.
$$
...
\,\mathop{\ll}\limits_\xi\,\frac{\log T}{T}
\sum_{n\in S_2}\frac{1}{|n-T_\circ|+T^{1/2}}.
$$
...
Denote
$$
F_1(u)\defeq\ssum{\rho=\frac12+i\gamma\\0<\gamma\le u}
\xi^{-\rho}\cX(1-\rho),\qquad
...
bound \eqref{eq:stick} under RH, and using \eqref{eq:superbound},
