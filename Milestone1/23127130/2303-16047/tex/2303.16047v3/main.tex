...
\author{%
  Chudi Zhong\textsuperscript{\rm 1}\thanks{Equal Contribution} \quad Zhi Chen\textsuperscript{\rm 1}\footnotemark[1] \quad Jiachang Liu\textsuperscript{\rm 1} \quad Margo Seltzer\textsuperscript{\rm 2} \quad Cynthia Rudin\textsuperscript{\rm 1}\\
  \\
  \textsuperscript{1} Duke University \textsuperscript{2} The University of British Columbia\\
  \texttt{\{chudi.zhong, zhi.chen1, jiachang.liu\}@duke.edu}\\
...
We use four datasets: a recidivism dataset (COMPAS)
%\footnote{We acknowledge that there are several recent calls for refraining from using the COMPAS dataset. However, we have used it in our study as a benchmark with awareness of its limitations and the ongoing debate surrounding its usage.} 
\cite{LarsonMaKiAn16}, the Fair Isaac (FICO) credit risk dataset \cite{competition}, a Diabetes dataset \cite{smith1988using}, and an ICU dataset MIMIC-II \cite{saeed2002mimic}.
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{figures/box_plot_compas_0.0005_0.001_1.01_merge_bins_13_camera_ready.png}
    \includegraphics[width=0.45\textwidth]{figures/box_plot_fico_0.0005_0.001_1.01_merge_bins_24_camera_ready.png}
    \caption{Box plots of the volume ratio and precision ratio of Method 1 over Method 2 across different $\tilde{K}$, for two datasets. It shows the precision and volume ratios are close to 1, indicating that Method 2 performs similarly to Method 1.} %\vspace*{-6pt}
...
\centering
\includegraphics[width=1\textwidth]{figures/diabetes_mcr.png}
...
\begin{figure*}
    ...
    \caption{Different shape functions (in red) of ``PFratio'' in MIMIC-II sampled from $\hat{R}$. }
...
We thank the anonymous reviewers for their suggestions and insightful questions. We gratefully acknowledge support from grants DOE DE-SC0023194, NIH/NIDA R01 DA054994, NSF IIS-2130250, NSF DGE-2022040, and DOE DE-SC0021358. We acknowledge the support of the Natural Sciences and Engineering Research Council of Canada (NSERC).
