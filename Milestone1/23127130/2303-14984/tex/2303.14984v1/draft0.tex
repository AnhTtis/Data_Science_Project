...
\begin{equation}
(\g C:S)_{ij}=\sum_{k\ell}\g C_{ijk\ell}S_{k\ell},\qquad (S:\g C)_{k\ell}=\sum_{ij}S_{ij}\g C_{ijk\ell}S_{k\ell},\qquad S:T=\sum_{ij}S_{ij}	{T}_{ij}.
\end{equation}
for any $S,T\in S_3(\R)$. 

The space $H^1(\Omega)$ is the Sobolev space of complex valued square integrable functions whose the gradient is also square integrable. The space $H^1(\Omega)^3$ is three dimensional vectorial valued version of $H^1(\Omega)$. 
...
The body is fixed on  $\Gamma_{\text{Dir}}$ and some boundary stress $\g g$ is applied on $\Gamma_{\text{Neu}}:=\partial \Omega\bs \overline{\Gamma_{\text{Dir}}}$. A internal stress $\g f$ is also applied inside $\Omega$. Hence the displacement $\g u$ satisfies the following system of equations/boundary conditions:
...
\begin{equation}\nonumber
\inner{\g u,\g v}{\cH}:=\int_\Omega\cE(\g u):\cE(\overline{\g v})
\end{equation}
...
\int_\Omega(\g C:\cE(\g u)):\cE(\overline{\g v})=\int_\Omega\g f\cdot\overline{\g v} + \int_{\Gamma_\text{Neu}}\g g\cdot \overline{\g v},\qquad \forall \g v\in \cH.
\end{equation}
...
\begin{equation}\nonumber
|a(\g u,\g u)| =  \int_\Omega \cE(\g u):\g C:\cE(\overline{\g u})\geq \alpha \int_\Omega |\cE(\g u)|^2\geq \alpha \norm{\g u}{\cH}^2. 
\end{equation}
...
The specific case $\g f=\g 0$ and $\g g=\g 0$ is called the homogenous harmonic problem and, as we will see, it is an eigenvalue problem. The following theorem states the existence of an orthogonal modal basis in a weighed space that depends on the density. 
...
\begin{theorem} There exists a unique non decreasing sequence of eigenvalues $(\lambda_k)_{k\in\N}$ satisfying $\lambda_0>0$ and $\lim_{n\to +\infty}\lambda_k=+\infty$ and there exists a sequence of eigenmodes $(\g u_k)_{k\in\N}$ in $\cH$ such that 
\begin{equation}\nonumber
...
\begin{equation}\nonumber
\int_\Omega\rho\, \g u_k\cdot \overline{\g u}_\ell = \delta_{k\ell},\qquad\forall k,\ell\in\N. 
\end{equation}
...
\begin{equation}\nonumber
\inner{\g u,\g v}{\rho}:=\int_\Omega\rho\, \g u\cdot\overline{\g v}.
\end{equation}
This Hilbert space is isomorphic to the classic Hilbert space $L^2(\Omega)^3$ as $0<\beta\leq \rho\leq \norm{\rho}{L^\infty(\Omega)}<+\infty$. We define now a linear operator $A_\rho$ by 
\begin{equation}
...
\begin{equation}\nonumber
a(\g u,\g v) = \int_\Omega\rho\, \g f\cdot\overline{\g v},\qquad\forall \g v\in\cH.
\end{equation}
Then for any $\g \ph\in L^2_\rho(\Omega)^3$,
\begin{equation}\nonumber
\begin{aligned}
\inner{A_\rho\g f,\g \ph}\rho &= \inner{\g u,\g \ph}\rho = \int_\Omega\rho\, \g u\cdot \overline{\g \ph} = \overline{a(A(\rho\g \ph),\g u)} = \overline{a(A_\rho\g \ph,\g u)}  = a(\g u,A_\rho\g \ph) = \int_\Omega\rho\, \g f\cdot \overline{A_\rho\g \ph}\\
&=\inner{\g f,A_\rho\g \ph}\rho.
\end{aligned}
\end{equation}
...
\end{equation}
Moreover, $\norm{\g u_n}{L^2_\rho(\Omega)} = 1$ and for any $n\neq m$, $\int_\Omega\rho\, \g u_n\cdot \overline{\g u_m}=0$ because
\begin{equation}\nonumber
\begin{aligned}
\int_\Omega\rho\, A_\rho\g f_n\cdot \overline{A_\rho\g f_m} = \mu_n\mu_m \int_\Omega\rho\,  \g f_n\cdot \overline{\g f_m} = \inner{\g f_n\cdot \g f_m}\rho = 0.
\end{aligned}
\end{equation}
...
\end{aligned}
\end{equation}
Choosing $\g v=\overline{\g u_n}$ we get from the orthogonallity of the modes in $L^2_\rho(\Omega)^3$,
\begin{equation}\nonumber
\alpha_n\lambda_n=\int_\Omega\g f\cdot\overline{\g u_n} + \int_{\Gamma_\text{Neu}}\g g\cdot \overline{\g u_n},\qquad \forall n\in\N.
\end{equation}
If we call $f_n:=\int_\Omega\g f\cdot\overline{\g u_n}$ and $g_n:=\int_{\Gamma_\text{Neu}}\g g\cdot \overline{\g u_n}$, we get the formula 
\begin{equation}
...
\end{aligned}
\end{equation}
Choosing $\g v=\overline{\g u_n}$ we get from the orthogonallity of the modes:
\begin{equation}\nonumber
\alpha_n(\omega)(\lambda_n-\omega^2)=\int_\Omega\g f(.,\omega)\cdot\overline{\g u_n} + \int_{\Gamma_\text{Neu}}\g g(.,\omega)\cdot \overline{\g u_n},\qquad \forall n\in\N.
\end{equation}
If we call $f_n(\omega):=\int_\Omega\g f(.,\omega)\cdot\overline{\g u_n}$ and $g_n(\omega):=\int_{\Gamma_\text{Neu}}\g g(.,\omega)\cdot \overline{\g u_n}$, we get the formula 
