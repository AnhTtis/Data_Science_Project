...
%
\begin{abstract}
%, which can be decomposed into three computationally-intensive sub-tasks: skull-stripping, intensity normalization and spatial normalization. 
% To train a learning-based method, we construct a large brain pre-processing dataset of 10000 scans from publicly available datasets. 
% However, in this dataset, sub-tasks training data are not available. 
...
Brain magnetic resonance imaging (MRI) is widely-used in clinical practice and neuroscience. %, and pre-processing of these images is a ubiquitous task. 
% Today, large-scale studies can include tens of thousands of brain MRIs~\cite{Bookheimer2019}. 
% However, these scans can vary in resolution, contrast, and spatial orientation/alignment, and furthermore may contain artifacts such as bias fields which can impact the accuracy of downstream analyses. 
% Additionally, raw images will contain non-brain content, such as the neck and face, which must be removed for privacy reasons and to focus on the target region of interest.
Many popular toolkits for pre-processing brain MRI scans exist, e.g., FreeSurfer~\cite{fischl2012freesurfer}, FSL~\cite{Smith2004}, AFNI~\cite{cox1996afni}, and ANTs~\cite{Avants2008}. 
...
%While effective, these methods can become computationally burdensome especially when applied on large datasets.
...
%due to the efficiency of forward passes, and rely on curated training datasets for specific tasks. 
% However, there is a small amount of this data easily available, compared to large amount of public available pre-processed brain MRI scans. 
%For example, SynthStrip a skull-stripping model, use a dataset of 80 subjects for training, while public ADNI contains more than 4000 subjects. 
% To achieve this, we first created a large ($N > 10,000$) and diverse brain MRI dataset, containing raw images and their pre-processed counterparts obtained with FreeSurfer. 


% for \modelname. By relaxing the requirement of manually annotated datasets for sub-tasks, we then make use of publicly available pre-processed MRI scans generated using Freesurfer to scale the training dataset to more than 10000 scans. However, this dataset only contained raw images and their pre-processed brains. Thus, we have to train \modelname~for learning three individual mappings (skull-stripping, intensity normalization and spatial normalization) in absence of sub-tasks supervision. 
...
% However, because mappings for \modelname~is highly under-constrained, simply disentangling the whole pre-processing procedure into image translation and spatial transformation often leads to the well known problem of mode collapse, where all images maps to similar output images. Thus, it calls us to propose \modelname~which encourages the translation to be geometric-preserving.
 
% In this work, we introduce \modelname, a novel learning-based method for efficiently pre-processing structural brain MRI volumes. \modelname~shows superiority over conventional (non-learning-based) and learning-based baselines on both efficiency and performance. In addition, at inference time, the end user has the flexibility to turn on/off different pre-processing steps.

\section{Methods}
% In this work, we consider a standard MRI pre-processing pipeline composed of the following sub-tasks: skull-stripping, intensity normalization, and spatial normalization (affine registration to a standard coordinate space). 
...
%The input image $x$ is first fed through GPT, and then spatially transformed to the target coordinates space using ST. 
...
% However, having the U-Net simply output the skull-stripped, intensity-normalized image directly often leads to the well known problem of mode collapse, where all images maps to similar output images. 
% To solve the issue, 
...
% Thus the skull stripped and intensity-normalized image is computed as:
\begin{equation}
...
% There is another important advantage of having $f_\theta$ output a multiplier field $\chi$, instead of the final image directly. 
...
The first term is a reconstruction loss $L_{rec}$. %which encourages the output of the end-to-end model to be close to the ground truth $x_{gt}$.
In this paper, we use SSIM~\cite{Wang2004} for $L_{rec}$.
...
% Note that as $\lambda$ increases, the importance of the total variation loss increases. In the extreme where $\lambda \rightarrow \infty$, the total loss will be minimized as TV goes to zero, which is achieved when $\chi$ is a constant valued image and the mapping is fully geometry-preserving.
...
% This strategy of conditioning on $\lambda$ allows us to train a model that is agnostic to the value of the hyperparameter. In particular, since $\lambda$ is an input to the model, it allows the user to efficiently produce a range of outputs at inference time that corresponds to setting the hyperparameter to different values, obviating the need for hyperparameter tuning.
...
% Evaluation datasets differed for different experiments and are listed under the corresponding sections.
...
%Skull-stripping (SS), intensity normalization (IN) and spatial normalization (SN).
...
\begin{figure}[t]%
    \centering
    \subfloat[\centering ]{{\includegraphics[width=0.5\textwidth,height=4.5cm]{figure/affine_examples.pdf}}}%
    %\qquad 
    \subfloat[\centering ]{{\includegraphics[width=0.47\textwidth,height=5cm]{figure/dice_results.pdf}}}%
    \caption{(a) Representative examples for spatial normalization. Rows depict sagittal, axial and coronal view. For each view, from left to right: input image, atlas, \modelname~results, and FreeSurfer results. (b) Boxplots illustrating Dice scores of each anatomical structure for \modelname~and FreeSurfer in the atlas-based registration with the PPMI dataset. We combine left and right hemispheres of the brain into one structure for visualization.}%
    \label{fig_ab}%
\end{figure}
...
%as well as models where a U-Net model for intensity-normalization and skull-tripping and a STN for affine registration are trained separately?
% In this ablation study, we investigated four baselines: naive U-Net, U-Net+STN, STN+U-Net and UMIRGPIT. U-Net+STN first applies a U-Net trained to skull strip and intensity normalize, followed by a separately trained STN for affine registration. STN+U-Net reverses the order of the two networks.
...
% We consider this U-Net baseline as an ablation of \modelname.
For evaluation, we use the test images from the HCP dataset.%, which was also used for the evaluation of ``Intensity Normalization'' in Section~\ref{sec:in}.
\\
...
%As our method does not require subtaks supervision, it can utilize widely available fully pre-processed target images. 
