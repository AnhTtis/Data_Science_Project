...
% \ck{Image two: picture of qumode-qubit device}
\begin{figure}
    \centering
...
% In the case of photons, the qumode stores the configuration of the electromagnetic field, and in the case of phonons, the qumode stores the vibrational state. 
...
% It is often common to speak of the momentum operator $\hat{p}$, which can be found through the Fourier transform of $\hat{x}$.  
...
Truncation of the Hilbert space is required to provide error bounds, otherwise the remainder terms become undefined.
% In this work, if unbounded operators are present, the remainder terms in the expansions that we consider become undefined and some form of a truncation is needed to ensure that the mathematics is well defined. 
...
% The final aspect that we need to talk about in our description of the state is the qubits. 
To incorporate the qubit's state, we 
% In our model of computing we assume that the vector space is 
...
% Our work will assume only a single qubit with a qumode in the Fock-space encoding.



\subsubsection{Representing operations and measurements via block encodings}
% Wwe will discuss the bosonic operations that act on the qumodes for the system.
...
% In order to introduce this, we need to review some notation surrounding bosonic creation and annihilation operators (otherwise known as raising and lowering or Fock operators). 
Our techniques can be applied in both spaces because
% In the following, we demonstrate how some operations can be analytically decomposed, regardless of whether they are defined with 
...
% There are many gate operations that can be considered as part of an instruction set. 
To illustrate the \qq~ISA, we define three types of operations with examples:
\begin{enumerate}
    \item \textbf{Qubit-exclusive}: these include typical qubit gates like the \textit{Pauli} ($X, Y, Z$) gates, \textit{Hadamard} ($H$) gate
     \begin{align}
...
% qubit or qumode-exclusive or entangle across the qubit and qumode. For example, we can still perform single-qubit operations like $H$ and $R_z$.


% Finally, 
% Here, for convenience, we focus on a different gate, which we name 
...
% , which we assume we can implement without error.
% While this gate can be approximated with arbitrary fidelity using 
...
\end{align}
\end{define}
% Notice that this gate operates simultaneously on the qumode and qubit (producing, in the language of ion-trap quantum computers, a `red-sideband' transition). 
...
% in our application, we demonstrate how these formulas can be used as a control scheme for our device.
...
\begin{theorem}[Informal BCH theorem from \cite{Childs_2013}]\label{thm:BCH}
    ...
    \begin{align}
        ...
        % &= e^{A \lambda} e^{B \lambda} e^{-A \lambda} e^{-B \lambda}\\
    \end{align}
