...
\author{Jinfeng Zhou\footnotemark[1],~~Xian-Ming Gu$^{\S,}$\footnotemark[1],~~Jinye Shen\footnotemark[1],~~Yong-Liang Zhao\footnotemark[2],~~Hu Li\footnotemark[3]}
...
{\tt guxm@swufe.edu.cn} (corresponding author); {\tt shenjy@swufe.edu.cn}}
\footnotetext[2]
...
the compact operator with a tempered L1 approximation with nonuniform time
...
\noindent
{\bf Keywords:} Tempered time-fractional B-S model; nonuniform time steps; exponential transformation; compact difference scheme; regularity.
...
with the compact difference operator in space and the tempered L1 formula with graded time steps will be derived for solving the
...
The numerical method is constructed by the compact difference operator and the (fast) tempered $L1$ formula with 
nonuniform time steps. The unconditional stability and convergence of $\min\left\{\gamma\alpha,2-\alpha\right\}$-order 
in time and fourth-order in space are rigorously proved by mathematical induction. Numerical examples are included
and the numerical results indicated that the proposed numerical methods are very efficient in aspects of 
both high accuracy and low computational cost.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\appendix
\section{Appendix}
