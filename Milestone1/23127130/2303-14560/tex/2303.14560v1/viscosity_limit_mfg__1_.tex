...
it is known that as $\nu \to 0^+$, 
the solutions $(u_\nu,m_\nu)$ converge to $(u,m)$, 
where $(u,m)$ are solutions to first-order MFGs \cite{note,CGPT}. 
...
or there are extra requirements on $H$ and $f$ \cite{car, GM19}.
Relying on \cite{CG, CGPT},
we show that the condition can be dropped without further restrictions (see Theorem \ref{T.3.2}).
...
We thank Andrzej \'{S}wi\c{e}ch for remarks which lead to the first part of Section \ref{S61}.
We also thank Fran\c{c}ois Delarue, Dan Lacker and Jianfeng Zhang for helpful discussions.
