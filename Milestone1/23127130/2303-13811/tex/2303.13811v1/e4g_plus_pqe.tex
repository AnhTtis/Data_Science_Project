...
proving redundancy of $C$ for the case when \cof{C}{y} is not implied
...
than \Eg. To avoid re-examining subspace $\pnt{y}^*$, \egp generates a
\ti{shorter} plugging clause $D = \overline{y}_1$.
\end{example}
...
than QE.  The first feature is that one can ignore the subspaces
\pnt{y} where $F \setminus \s{C}$ implies $C$. The second feature is
that when \cof{F}{y} is satisfiable, one only needs to prove
redundancy of the clause $C$ alone.  Among the three algorithms we run
in experiments, namely, \dpqe, \Eg, and \egp only \egp exploits both
features. (In addition to using \dpqe inside \egp we also ran it as a
stand-alone PQE solver.)  \dpqe does not use the first
feature~\cite{hvc-14} and \Eg does not exploit the second one. As we
show in Sections~\ref{sec:fifo_exper} and~\ref{sec:inv_gen_exper},
this affects the performance of \dpqe and \Eg.
