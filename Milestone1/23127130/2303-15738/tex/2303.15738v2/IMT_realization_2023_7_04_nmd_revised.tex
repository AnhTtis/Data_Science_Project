...
\newtheorem*{lemma:alpha_non-commute}{Lemma~\ref{alpha_non-commute}}
...
For any hyperbolic knot, $\mathcal{S}_K(g)$ is known to be a finite set for all non-trivial elements $g \in G(K)$. 

The Property P conjecture, settled by Kronheimer and Mrowka, 
asserts that every non-trivial knot satisfies Property P: 
for any $r \in \mathbb{Q}$, 
there exists a non-trivial element $g \in G(K)$ such that $\mathcal{S}_K(g) \not\ni r$. 
The purpose of this article is to prove somewhat surprising flexibilities--
Separation Property and Realization Property--
of the set $\mathcal{S}_K(g)$, 
which are refinements of Property P in the context of Dehn filling trivialization. 

We prove Separation Theorem that can be seen as a Dehn filling analogue of various separability properties of 3--manifold groups: 
...
We develop this to establish Realization Theorem asserting that 
for any hyperbolic knot $K$ without exceptional surgery slope, 
every finite set of slopes can be realized as the set $\mathcal{S}_K(g)$ for infinitely many, 
mutually non-conjugate elements $g \in G(K)$.
This result can be generalized to more general setting, but we provide some examples showing that Separation Theorem and Realization Theorem do not hold unconditionally. 
\end{abstract}
...
%Primary: 57M05, Secondary: 57K10; 57K30; 57M07; 20F65
%
%\textit{Key words and phrases:}\ 
%Dehn filling, knot group, Property P, peripheral Magnus property, Dehn filling trivialization, slope, stable commutator length, hyperbolic length, residual finiteness}
%}
...
\footnote[0]{
MSC2020\, Primary: 57M05, Secondary: 57K10; 57K30; 57M07; 20F65
...
\label{Introduction}
In \cite{Dehn} Dehn introduced \textit{Dehn surgery} and made important connections between group theory and topology. 
...
The Property P conjecture due to Bing and Martin \cite{BingMartin} 
had been one of the leading problems in $3$--manifold topology until it was settled by Kronheimer and Mrowka \cite{KM}, 
and still suggests several new developments concerning a relationship between Dehn filling and knot group as discussed below. 
 
The motivation of this article is to present a new perspective on a study of Dehn surgery (filling) from group theoretic viewpoint,  
which takes a root in the Property P conjecture. 
...
\[
\pi_1(K(p/q)) = G(K)/ \langle\!\langle \mu^p \lambda^q \rangle\!\rangle =  G(K)/\langle\!\langle p/q \rangle\!\rangle = G(K)/ \langle\!\langle r \rangle\!\rangle,
\] 
and we obtain a natural epimorphism
\[
...
Precisely $r$--Dehn filling trivializes elements in
\[
...
Bing and Martin \cite{BingMartin} introduced the following: 

\medskip 

\noindent
\textbf{Property P}. \quad 
\textit{$\pi_1(K(r)) \ne 1$ for any non-trivial knot $K$ and $r \in \mathbb{Q}$. }

\medskip
In \cite{BingMartin} it had been conjectured that every non-trivial knot satisfies Property P, 
and Kronheimer and Mrowka \cite{KM} settled this conjecture affirmatively using gauge theory and symplectic topology. 
We state Property P ``Theorem'' in the context of knot group. 
...
This is achieved in the previous paper \cite{IMT_Magnus} based upon Theorem~\ref{propertyP} and topological and group theoretical arguments. 
...
Let us capture Property P in the context of Dehn filling trivialization 
by introducing the set valued function on a knot group. 
...
Using the function $\mathcal{S}_K$, 
Property P is equivalent to the following. 

\bigskip


\noindent
\textbf{Property P (paraphrased).}\quad 
...
As a consequence of this we have $\mathcal{S}_K(\mu) = \emptyset$. 

\medskip

Let us recall some fundamental facts about the function $\mathcal{S}_K \colon G(K) \to 2^{\mathbb{Q}}$. 
...
It should be noted that Theorem~\ref{unbound} does not say anything about slopes $s$ for which 
$p_s(g) \ne 1$. 
...
there exists an element $g \in G(K)$ such that 
$r \in \mathcal{S}_K(g)$ and $s \not\in \mathcal{S}_K(g)$.}
\item 
...
In (ii) as we mention in Theorem~\ref{S_K_hyperbolic}, 
for any hyperbolic knot $K$ and any non-trivial element $g \in G(K)$,  
$\mathcal{S}_K(g)$ is a finite set, 
and hence $\mathcal{R}$ must be also finite. 
Furthermore, as we observe below, in general, 
Separation Property does not hold.  
To explain this we prepare some terminologies, which will be used throughout this paper. 

Thanks to the Geometrization theorem \cite{Pe1,Pe2,Pe3} we may classify slopes into 
...
there is no element $g \in G(K)$ such that $\mathcal{R} \subset \mathcal{S}_K(g) \subset \mathbb{Q} -  \mathcal{S}$. 
\end{example}
...
$\frac{18}{5} \in \mathcal{S}_K(g)$ and 
$18  \not\in \mathcal{S}_K(g)$. 
\end{example}
...
immediately implies Separation Property (i). 
\medskip


\begin{proposition}
\label{separation_two_slopes}
For given two distinct slopes $r$ and $s$ in $\mathbb{Q}$, 
...
We may  generalize Proposition~\ref{separation_two_slopes} to Separation Theorem below which 
asserts that generically Separation Property does hold. 
...
Then there exists an element $g \in [G(K), G(K) ]\subset G(K)$ such that $\mathcal{R} \subset \mathcal{S}_K(g) \subset \mathbb{Q} - \mathcal{S}$. 
\end{theorem}

Now let us choose $\mathcal{S} = \mathbb{Q} - \mathcal{R}$ in Separation Property (ii). 
Then $\mathcal{S}_K(g) = \mathcal{R}$, 
which leads us Realization Property. 
Taking Examples~\ref{torus_knot_pq}, \ref{torus_knot_cyclic}, \ref{pretzel_non_separation} into account  
it is reasonable to formulate Realization Property as in the following form. 
Recall that $\mathcal{S}_K(g)$ is always a finite set for any hyperbolic knot $K$ and for all non-trivial element $g \in G(K)$. 


\bigskip

\noindent
\textbf{Realization Property.}\quad 
{\it Let $K$ be a non-trivial knot without finite surgery. 
For a given finite subset $\mathcal{R} = \{ r_1, \ldots, r_n \} \subset \mathbb{Q}$, 
there exists a non-trivial element $g \in G(K)$ such that $\mathcal{S}_K(g) =\mathcal{R}$. 
In other words, 
we may find a non-trivial element $g \in G(K)$ so that it becomes trivial after prescribed $r_i$--Dehn fillings, 
...
The next theorem proves that generically hyperbolic knots satisfy Realization Property, 
...
We address a kind of injectivity of the function $\mathcal{S}_K \colon G(K) \to 2^{\mathbb{Q}}$. 
...
Using the residual finiteness of $3$--manifold groups, 
...
but in general it is not clear that varying elements $\alpha$ we may actually obtain 
...
$\mathcal{S}_K(h) = \mathcal{S}_K(g)$, and prove Theorem~\ref{non_rigid} below for hyperbolic knots without torsion surgery. 
...
when $K$ has no torsion surgery, 
$\mathcal{S}_K(g)=\mathcal{S}_K(g^{n})$ holds, 
...
we need to prove that there exists a non-trivial element $g$ such that $\mathcal{S}_K(g) \subset \mathbb{Q} - \mathcal{S}$. 
This is the opposite situation of Theorem~\ref{unbound}, 
and leads us to the extreme case where $\mathcal{R} = \emptyset$ and $\mathcal{S} = \mathbb{Q}$ in the Separation property (ii) (Realization Property). 
...
we need to prove the following result which is regarded as an improvement of Proposition~\ref{cup_cap}(i). 
...
The former heavily depends on the residual finiteness of $3$--manifold groups and the latter depends on Theorem~\ref{S_K_intersection} above. 
...
Let $K$ be a hyperbolic knot and $g \in G(K)$ a peripheral element other than longitudinal one. 
...
\[
\begin{split}
g_n & = g^{g^{w_n}} g^{-2} = g^{W_n} g^{-2} = (W_n)^{-1} g W_n\, g^{-2} \\
...
$h^{-1} g h$ is null-homologous, and thus it is $\lambda_K^m$ for some integer $m$. 
...
So there exists an element $\gamma \in G(K)$ such that $\gamma^{-1} y \gamma = \lambda^{\ell}$.  
...
Then the inclusion $\Sigma \to \widehat{\Sigma}$ induces a monomorphism on homology.  
Since $[y] \ne 0 \in H_1(\Sigma)$, 
...
$\widehat{\Sigma}$ is a Thurston norm minimizing surface in $K(0)$. 
Since $K$ is non-trivial, $\widehat{\Sigma}$ is not a $2$--sphere. 
If $\widehat{\Sigma}$ has genus greater than one, 
it is incompressible. 
For otherwise, a compression of $\widehat{\Sigma}$ reduces the Thurston norm. 
Assume that $\widehat{\Sigma}$ is a torus. 
If it is compressible, 
then after compression we have a non-separating $2$--sphere in $K(0)$. 
This then implies $K$ is a trivial knot (\cite[Corollary~8.3]{GabaiIII}), a contradiction. 
Thus $\widehat{\Sigma}$ is incompressible in $K(0)$. 
Hence $y$ is non-trivial in $\pi_1(K(0))$. 

On the other hand, since $\gamma^{-1} y \gamma = \lambda^{\ell}$ in $G(K)$, 
...
$h^{-1}(\lambda^{-q_0}y^{p_i}) h$ is null-homologous, 
...
\begin{proof}
\[
p_{r_i}(g_a) = [p_{r_i}(a)^{-1}p_{r_i}(g_k)p_{r_i}(a),\ p_{r_i}(r_{k+1})] = [1,\ p_{r_i}(r_{k+1})] = 1 
\]
for $i = 1, \dots, k$,
and 
\begin{align*}
 p_{r_{k+1}}(g_a) &=  [p_{r_{k+1}}(a)^{-1}p_{r_{k+1}}(g_k)p_{r_{k+1}}(a),\ p_{r_{k+1}}(r_{k+1})]\\
&= [p_{r_{k+1}}(a)^{-1} p_{r_{k+1}}(g_k)p_{r_{k+1}}(a),\ 1] = 1.
\end{align*}
...
Note that 
\[
p_s(g_a)=1\quad \textrm{if and only if}\quad [p_s(a^{-1} g_k a),\ p_s(r_{k+1})] = 1. 
\] 

Claim~\ref{alpha_non-commute} below is crucial in the following procedure, 
and we will apply this several times.  
The proof of this claim is a little bit long, so we defer its proof until we finish the proof of Theorem~\ref{separation}. 
In fact, we present Lemma~\ref{conjugate_non-commute} (in slightly generalized form) after completing the proof of Theorem~\ref{separation}, from which Claim~\ref{alpha_non-commute} immediately follows.  

\begin{claim}
\label{alpha_non-commute}
There exists $\alpha \in \pi_1(K(s))$ such that $[\alpha^{-1} p_s(g_k)\alpha,\ p_s(r_{k+1})] \ne 1$. 
\end{claim}
...
${\bf (1)}$\ Following Lemma~\ref{alpha_non-commute} 
there exists $\alpha_1 = a(s_1) \in \pi_1(K(s_1))$ such that 
\[
[\alpha_1^{-1} p_{s_1}(g_k) \alpha_1,\ p_{s_1}(r_{k+1})] \ne 1. 
\] 
Since $p_{s_1} \colon G(K) \to \pi_1(K(s_1))$ is surjective, 
we have $a_1 \in G(K)$ such that  $p_{s_1}(a_1) = \alpha_1$. 
Then we have 
\[
p_{s_1}( [a_1^{-1} g_k a_1, r_{k+1}] ) \ne 1. 
\]

Let us write 
\[
g_{k, a_1} = [a_1^{-1} g_k a_1, r_{k+1}] \in G(K), 
\]
...
\[
[\alpha_2^{-1} p_{s_2}(g_k) \alpha_2,\ p_{s_2}(r_{k+1})] \ne 1,\ \textrm{equivalently}\ p_{s_2}([a_2^{-1} g_k a_2, r_{k+1}]) \ne 1. 
\]

Then put 
\[
g_{k, a_2} = [a_2^{-1} g_k a_2, r_{k+1}] \in G(K), 
\] 
which is non-trivial and satisfies $p_{s_2}(g_{k, a_2}) \ne 1$. 
Since $p_r(g_k) = 1$ for $r \in \mathcal{R}_k$ and obviously $p_{r_{k+1}}(r_{k+1}) = 1$, 
\[
p_r(g_{k, a_2}) = [p_r(a_2)^{-1}p_r(g_k)p_r(a_2), p_r(r_{k+1})] = 1\  \textrm{for}\  r \in \mathcal{R}_{k+1}. 
\] 
...
Again, following Lemma~\ref{alpha_non-commute}, 
...
\[
[\alpha_3^{-1} p_{s_3}(g_k) \alpha_3,\ p_{s_3}(r_{k+1})] \ne 1,\ \textrm{equivalently}\ p_{s_3}( [a_3^{-1} g_k a_3, r_{k+1}] ) \ne 1. 
\]
Then put 
\[
g_{k, a_3} = [a_3^{-1}g_k a_3,\ r_{k+1}] \in G(K), 
\]
...
As above, using Lemma~\ref{alpha_non-commute} we construct $g_{k, a_m}$ in $G(K)$ such that $p_{s_m}(g_{k, a_m}) \ne 1$. 
...
which immediately implies Claim~\ref{alpha_non-commute} used in the proof of Theorem~\ref{separation}. 
...
This then implies 
\[
[\alpha^{-1} p_s(g)^{-1} \alpha,\  p_s(r)] = [(\alpha^{-1} p_s(g) \alpha)^{-1},\ p_s(r)] =1, \,\textrm{and}
\]
\[
\left[\prod_{i=1}^d \alpha_i^{-1} p_s(g)^{\varepsilon_i} \alpha_i,\ p_s(r)) \right]  = 1\quad (\varepsilon_i = \pm 1)
\] 
for any $\alpha_i$ and $d$. 
...
By \cite[Corollary~4.1.6]{MKS} (cf. \cite[p.196, 28]{MKS}), 
...
write $\langle \! \langle p_s(g) \rangle\!\rangle = \langle p_s(g) \rangle$. 
...
This shows that $\langle \! \langle p_s(g) \rangle\!\rangle = \langle p_s(g) \rangle$ is not normal, a contradiction. 
...
%If $K(s)$ is sufficiently large, i.e. it contains an incompressible surface, then \cite[Theorem II.6.3]{JS} shows that $K(s)$ is also Seifert fibered, a contradiction. 
%Otherwise, the Geometrization theorem says that $K(s)$ is either hyperbolic or Seifert fibered. 
%Since $\pi_1(K(s)) \supset \mathbb{Z} \oplus \mathbb{Z}$, $K(s)$ is not hyperbolic, 
%and thence it is Seifert fibered, a contradiction. 
...
We apply Theorem~\ref{separation} again to get an element $g_2 \in [G(K), G(K)] \subset G(K)$ which satisfies
\[
...
let us write
 \[ G_w := F_n \slash \langle\!\langle w \rangle \! \rangle = \langle a_1,a_2,\ldots, a_n\ | \: w \rangle,
  \]
which is the one-relator group whose relator is $w$. 
...
\end{bmatrix}$
as $n \to \infty$. 
This contradicts the discreteness of $\rho(G(K))$. 
\end{proof}
...
\begin{figure}[htb]
\centering
\includegraphics[bb=0 0 121 113,width=0.2\textwidth]{figure-eight_knot_mu_h.pdf}
...
\begin{proof}
Assume for a contradiction that 
$g^{p+pm-1} s^{-1} g^{-p+1} s$ is conjugate to $g^{\ell_m}$ for some integer $\ell_m$.
Then 
$p_s(g^{p+pm-1} s^{-1} g^{-p+1} s)$ is also conjugate to $p_s(g)^{\ell_m}$ in $\pi_1(K(s))$. 
Since 
\[
...
On the other hand, 
$\varphi(g^{p+pm-1} s^{-1} g^{-p+1} s)$ is conjugate to $\varphi(g^{\ell_m}) = \varphi(g^{\pm pm})$. 
By the choice of $p$, 
...
%Topology Appl.\ \textbf{110} (2001), 113--118. 
%}
