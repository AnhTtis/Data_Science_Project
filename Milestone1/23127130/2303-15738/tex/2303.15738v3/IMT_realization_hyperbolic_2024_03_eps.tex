...
\address{International Institute for Sustainability with Knotted Chiral Meta Matter (WPI-SKCM$^2$), Hiroshima University, 
...
\begin{abstract}    
Each $r$--Dehn filling of the exterior $E(K)$ of a knot $K$ in $S^3$ produces a $3$--manifold $K(r)$, 
and induces an epimorphism 
from the knot group $G(K) = \pi_1(E(K))$ to $\pi_1(K(r))$, 
which trivializes elements in its kernel. 
To each element $g \in G(K)$, 
consider all the non-trivial Dehn fillings and assign 
$\mathcal{S}_K(g) = \{ r \in \mathbb{Q} \mid  \textrm{$r$--Dehn filling trivializes}\ g \}$ $\subset \mathbb{Q}$. 
{\it Which subsets of $\mathbb{Q}$ can occur as $\mathcal{S}_K(g)$?} 
Property P concerns this question and gives a fundamental result which asserts that the emptyset $\emptyset$ can be realized by $\mathcal{S}_K(\mu)$ for the meridian $\mu$ of $K$. 
Suppose that $K$ is a hyperbolic knot. 
Then $\mathcal{S}_K(g)$ is known to be finite for all non-trivial elements $g \in G(K)$. 
We prove that generically, for instance, 
if $K$ has no exceptional surgery, then 
any finite (possibly empty) family of slopes $\mathcal{R} = \{ r_1, . . . , r_n \}$ can be realized by $\mathcal{S}_K(g)$ for some element $g \in G(K)$. 
Furthermore, there are infinitely many, 
mutually non-conjugate such elements, each of which is not conjugate to any power of $g$.  
We also provide an example showing that the above realization property does not hold unconditionally. 
\end{abstract}
...
This is the first in a series of papers where we study Dehn filling from group theoretic point of view. 
Each Dehn filling of a knot exterior naturally induces an epimorphism from the knot group to the fundamental group of the resulting $3$--manifold, 
which trivializes elements in the kernel of the epimorphism. 
Such a typical element is a slope element represented by a simple closed curve in the boundary of the knot exterior identifying the meridian of the filled solid torus. 
We investigate such trivialization arising via Dehn fillings parametrized by rational numbers. 
To make precise we start with some terminologies and notations. 

Let $K$ be a non-trivial knot in the $3$--sphere $S^3$ with a tubular neighborhood $N(K)$. 
Removing the interior of $N(K)$ we obtain the exterior $E(K)$ of $K$, which has a torus boundary. 
Denote by $G(K)$ the {\em knot group} $\pi_1(E(K))$. 
Let $(\mu, \lambda)$ be a preferred meridian-longitude pair of $K$.  
Then every simple closed curve on $\partial E(K)$ represents $\mu^p \lambda^q$ for some coprime integers $p$ and $q$. 
$p/q$--{\em Dehn filling} is an operation gluing a solid torus $V$ to $E(K)$ along their boundaries so that 
the meridian of $V$ represents $\mu^p \lambda^q$ \cite{Dehn}. 
The resulting $3$--manifold is denoted by $K(p/q)$. 

Each $p/q$--Dehn filling of $E(K)$ induces an epimorphism 
\[
...
$\langle\!\langle r \rangle\!\rangle$ using the rational number $r = p/q$. 
Then algebraically, $r$--Dehn filling is regarded as an operation trivializing elements in $\langle\!\langle r \rangle\!\rangle$. 
We call this trivialization a {\it Dehn filling trivialization}.   

Concerning Dehn filling trivialization, the most important and fundamental result is the Property P Theorem, 
which was conjectured by Bing and Martin \cite{BingMartin} and settled by Kronheimer and Mrowka \cite{KM}(cf. \cite{GLu2} and \cite{Pe1,Pe2}). 
Originally, Property P Theorem asserts that $\pi_1(K(r))$ is non-trivial for all non-trivial slopes $r \in \mathbb{Q}$. 
Since the meridian $\mu$ normally generates the knot group $G(K)$, 
$\pi_1(K(r))$ is also normally generated by a single element $p_r(\mu)$. 
Hence, if $p_r(\mu) = 1 \in \pi_1(K(r))$, then $\pi_1(K(r))$ is trivial, 
contradicting Property P Theorem. 
Thus we have the following paraphrase of Property P Theorem. 

\begin{theorem}[Paraphrase of Property P Theorem]
\label{propertyP_2}
For any non-trivial knot $K$, the meridian $\mu$ remains non-trivial after
$r$--Dehn filling for all $r \in \mathbb{Q}$. 
\end{theorem}


On the other hand, in general, 
a given element $g \in G(K)$ becomes trivial for some Dehn fillings and remains non-trivial for other Dehn fillings. 
To describe this we introduce the following set valued function on $G(K)$. 

\begin{definition}[Dehn filling trivializing slope set]
\label{D}
...
We call $\mathcal{S}_K(g)$ the {\it Dehn filling trivializing slope set} of $g$, 
or the {\it trivializing slope set} of $g$ for short. 
\end{definition}
...
Determination of $\mathcal{S}_K(g)$ requires to solve ``parametrized word problem'' (parametrized over $\mathbb{Q}$), 
and seems to be very hard. 
The Property P Theorem says $\mathcal{S}_K(\mu) = \emptyset$. 
As far as we know, the emptyset is the unique explicitly known subset realized by the trivializing slope set. 
We are intrigued by the following question. 

\begin{question}
\label{motivation}
Which subset $\mathcal{R}$ of $\mathbb{Q}$ is realized by the Dehn filling trivializing slope set 
 $\mathcal{S}_K(g)$ for some element $g \in G(K)$?
 \end{question}

Since $G(K)$ is countable, 
$\mathcal{S}_K \colon G(K) \to 2^{\mathbb{Q}}$ is not surjective. 
Furthermore, for hyperbolic knots we have the following finiteness property. 
 

\begin{theorem}[Finiteness Theorem]
\label{finiteness}
For any hyperbolic knot $K$,  
the Dehn filling trivializing slope set $\mathcal{S}_K(g)$ is always a finite subset of $\mathbb{Q}$ for any non-trivial element $g \in G(K)$
\footnote{Any torus knot does not satisfy the finiteness property, and for satellite knots, it is expected that they satisfy finiteness property, but it remains open.}.
\end{theorem}

Although this result was shown in \cite{IchiMT} along the same line with the proof of a deep result \cite{Agol}, 
we will give an alternative simpler proof in this article. 
Following Theorem~\ref{finiteness} we focus on finite subsets $\mathcal{R}$ in Question~\ref{motivation}. 


Before stating our main result (Realization Theorem), 
we exhibit examples of finite subsets $\mathcal{R}$ which cannot be realized 
by the Dehn filling trivializing slope set. 
...
Let us take a closer look at Example~\ref{pretzel_non_separation}.  
We recall some terminologies. 
A slope $r$ is a {\em finite surgery slope} if $\pi_1(K(r))$ is finite. 
Then the Geometrization Theorem \cite{Pe1,Pe2,Pe3} says that $K(r)$ is a Seifert fiber space, so $r$ is also a {\em Seifert surgery slope}. 
Obviously, every element $g \in G(K)$ becomes a torsion element after a finite surgery slope. 
...
In fact, any hyperbolic knot is conjectured to have no reducing surgery \cite{GS}, and known to have at most two such surgeries \cite{GLreducible}. 


In Example~\ref{pretzel_non_separation},  
the slope $18$ is a finite surgery slope for $K$. 
It should be noted here that the complement of $\{ \frac{18}{5} \}$, more generally the complement of $\mathcal{R}$, 
contains a finite (Seifert) surgery slope $18$. 
...
Then any finite \(possibly empty\) subset $\mathcal{R} = \{ r_1, \ldots, r_n\} \subset \mathbb{Q}$
is realized by $\mathcal{S}_K(g)$ for some element $g \in [G(K), G(K)] \subset G(K)$ 
whenever $\mathcal{R} = \emptyset$ or its complement contains neither a Seifert surgery slope nor two reducing surgery slopes. 
\end{theorem}

Theorem~\ref{realization} immediately implies that for most hyperbolic knots, every finite subset can be realized by the trivializing slope set. 

\begin{corollary}
\label{no_Seifert}
Let $K$ be a hyperbolic knot which admits neither a Seifert surgery nor two reducing surgeries. 
Then any finite \(possibly empty\) subset $\mathcal{R}$ is realized by $\mathcal{S}_K(g)$ for some element $g$. 
\end{corollary}

\medskip

Thus, for any hyperbolic knot without {\em exceptional} (meaning non-hyperbolic) surgery, 
every finite (possibly empty) subset  is realized by $\mathcal{S}_K(g)$ for some element $g$. 
...
and for a given knot there are infinitely many such crossing changes \cite{MM_crossing}. 

\medskip

\begin{example}
Let $K$ be a knot depicted by Figure~\ref{knot_without_exceptional}, which is the simplest hyperbolic knot without exceptional surgery. 
Then every finite (possibly empty) subset $\mathcal{R} \subset \mathbb{Q}$ is realized as 
$\mathcal{S}_K(g)$ for some element $g \in G(K)$. 

For instance, let $a_n$ be the rational number up to $n$-th decimal place of $\pi$, 
and put 
\[\mathcal{R}_n = \{ 3,\ 3.1,\ 3.14,\ 3.141,\ 3.1415,\; \dots,\; a_n \}.\]
Then for any integer positive integer $n$ we have an element $g_n$ which satisfies 
$\mathcal{S}_K(g_n) = \mathcal{R}_n$. 
\end{example}

\begin{figure}[htb]
\centering
%\includegraphics[width=0.2\textwidth]{knot_without_exceptional.eps}
\includegraphics[width=0.2\textwidth]{knot_without_exceptional.pdf}
\caption{The simplest hyperbolic knot without exceptional surgery} 
...
Theorem~\ref{realization} (Corollary~\ref{no_Seifert}) and Remark~\ref{Realization_theorem_non_peripheral} show that the Dehn filling  trivializing slope set does not detect peripherality of elements, 
...
\begin{theorem}[Identical trivializing slope set]
\label{non_rigid}
...
We close the introduction by translating Theorems~\ref{finiteness} (Finiteness Theorem) and \ref{realization} (Realization Theorem) purely in the language of knot group (without using Dehn filling). 
Theorem~\ref{finiteness} says that 
for any hyperbolic knot 
$\displaystyle \bigcap_{r \in \mathcal{R}}   \langle\!\langle r \rangle\!\rangle = \{ 1 \}$ whenever $\mathcal{R}$ is an ``infinite'' subset of $\mathbb{Q}$. 
On the other hand, 
as a consequence of Theorem~\ref{realization} we have the following result. 
...
In Section~\ref{function_S_K} we will collect elementary properties of Dehn filling trivializing slope sets, 
...
In the case where $K$ is a hyperbolic knot without torsion surgery, 
we may simplify the statement as follows. 

\begin{lemma}[Shrinking Lemma I']
\label{S_K_intersection_no_torsion}
Let $K$ be a hyperbolic knot without torsion surgery. 
Let $g$ be a non-peripheral element and $h$ a non-trivial element in $[G(K),G(K)]$. 

Then for each non-zero integer $n$, we have a constant $N_n > 0$ such that  
\[
\mathcal{S}_K(g) \cap \mathcal{S}_K(h) = \mathcal{S}_K(g^mh^n) 
\] 
for all integers $m \ge N_n$ for each $n$.
\end{lemma}

Lemma~\ref{S_K_intersection_no_torsion} plays a key role in the proof of Theorem~\ref{non_rigid} (Section~\ref{identical trivialization}). 


\medskip


\section{Elementary properties of Dehn filling trivializing slope sets}
\label{function_S_K}
Let us collect some elementary properties of Dehn filling trivializing slope sets. 
...
Dehn filling trivializing slope set is almost $\mathrm{Aut}(G(K))$-invariant in the following sense. 
...
This cannot happen by Theorem~\ref{propertyP_2} (Paraphrase of Property P Theorem). 
...
We close this subsection with a proof of Theorem~\ref{finiteness}. 

\begin{proof}[Proof of Theorem~\ref{finiteness}]
Let $K$ be a hyperbolic knot and $g$ a non-trivial element of $G(K)$. 
Assume that $g$ is peripheral, 
i.e. it is conjugate to a power of a slope element $\gamma$. 
If $p_r(g) = 1$ in $\pi_1(K(r))$, 
then by Proposition~\ref{slope} $r$ is represented by $\gamma$ or $r$ is a finite surgery slope. 
Since the number of such slopes is finite, we finish the proof. 
Suppose that $g$ is not peripheral. 
Then the argument in the proof of Proposition~\ref{L>0} shows that 
there are at most finitely many $r$ such that $p_r(g) = 1$. 
...
Hence, for any hyperboic surgery slope $s$ with $p_s(g) \ne 1$, 
...
\section{Elements whose Dehn filling trivializing slope set is empty} 
...
because it is equivalent to Property P Theorem; see Theorem~\ref{propertyP_2}. 
...
\includegraphics[bb=0 0 121 113,width=0.2\textwidth]{figure-eight_knot_a_b.pdf}
...
Assume for a contradiction that $\widehat{\Sigma}$ is compresiible. 
...
The goal of this section is to establish ``Separation Property'' for Dehn filling trivializing slope set. 
...
$\emptyset \subset \mathcal{S}_K(g_{0,\, m}) \subset \mathbb{Q} - \mathcal{S}$ (Step 1). 
...
If $p_{s_1}((g_{k+1,\, 1})^{q_1} h_{k+1,\, a_2}) = p_{s_1}(g_{k+1,\, 1})^{q_1} p_{s_1}( h_{k+1,\, a_2}) = 1$, 
then 
$p_{s_1}(g_{k+1,\, 1})^{q_1-1} 
= 1 \in \pi_1(K(s_1))$. 
Since $p_{s_1}(g_{k+1,\, 1}) \ne 1$, 
$p_{s_1}(g_{k+1,\, 1})$ is a torsion element of order $t_{1}  \ge 2$ and 
$q_1 - 1$ is a multiple of the order $t_{1}$. 

So take $q_1$ so that $q_1-1$ is not a multiple of $t_1$. 
...
${\bf (i+1, 1)}\ \mathcal{R}_{k+1} = \{r_1,\ldots,r_{k}, r_{k+1} \} \subset \mathcal{S}_K(g_{k+1,\, i+1}) \subset \mathbb{Q} - \{ s_1, \dots, s_i, s_{i+1} \}$
...
${\bf (i+1, 2)}$\ If $s_{i+1} \in \mathcal{S}_K(g_{k+1, i})$, i.e. $p_{s_{i+1}}(g_{k+1, i}) = 1$, 
...
Since we assume that is not a Seifert surgery slope, the former does not happen.
...
\renewcommand{\labelenumi}{(\roman{enumi})}
\item 
$Z(r) \cong \mathbb{Z}, \mathbb{Z} \oplus \mathbb{Z}$, or, \mbox{the Klein bottle group}. 
\item There is a Seifert piece $M$ of the torus decomposition \cite{JS,Jo,Hat} of $K(s)$ such that, 
up to conjugation of $\pi_1(K(s))$, 
$p_s(r)$ is a power of a regular fiber in $M$ and $Z(p_s(r))$ is the canonical subgroup of $\pi_1(M)$.
\end{enumerate} 
...
It follows from the last paragraph of p.35 in \cite{Scott} that $K(s)$ is also Seifert fiber space. 
...
In the second case (b), due to a homological reason, $s=0$. 
...
$[p_0(r)^a] = [p_0(\mu)^{ma}] = ma [p_0(\mu)] = 0$ in $H_1(K(0)) =  \langle [\mu] \rangle = \mathbb{Z}$. 
...
\noindent
{\bf Case (ii)}.\\
In this case $\langle \! \langle p_s(g) \rangle\!\rangle \subset \pi_1(M)$.
Recall that $K(s)$ is not a Seifert fiber space by the assumption, and thus the Seifert piece $M$ is not the whole $K(s)$.  
Let $T$ be a component of $\partial M$ and $X$ a decomposing piece $X$ other than $M$ and $\partial X \supset T$. 
...
Then we find an element $\gamma \in \langle \! \langle p_s(g) \rangle\!\rangle$ which does not belong to $ \pi_1(T)$, 
i.e. $\gamma \in \langle \! \langle p_s(g) \rangle\!\rangle -\pi_1(T) \subset  \pi_1(M) - \pi_1(T)$.  
...
\section{Shrinking trivializing slope sets}
\label{shrink}

Recall that 
in general we have the following inequality (Propositions~\ref{cup_cap} and \ref{S_K(g)_S_K(g^n)}) for Dehn filling trivializing slope sets. 
\[
...
for any $m, n \ne 0$. 
In general, 
it is difficult to find elements with smaller Dehn filling trivializing slope set; see also Proposition~\ref{cup_cap} (2). 

The purpose in this Section is prove that the inequality may be replaced by the equality under some conditions, 
which enables us to ``shrink'' Dehn filling trivializing slope set. 
Actually we will prove two kinds of Shrinking Lemma; 
the first one (Lemma~\ref{S_K_intersection}) bridges the gap between Separation Lemma and Realization Theorem, 
and the second one (Lemma~\ref{S_K_intersection_no_torsion}) will be used in the proof of Theorem~\ref{non_rigid}. 

\begin{lemma_S_K_intersection}[Shrinking Lemma I]
Let $K$ be a hyperbolic knot. 
Let $g$ be a non-peripheral element and $h$ a non-trivial element in $[G(K),G(K)]$. 
Assume that $\mathbb{Q} - \mathcal{S}_K(g)$ contains neither a finite surgery slope nor two reducing surgery slopes. 
...
$p_s(g)^{m_0} = p_s(h^{-n_0})$. 
If an integer $m$ also satisfies $p_s(g)^{m} = p_s(h^{-n_0})$. 
...
Lemmas~\ref{scl_product} and \ref{monotonicity} and the choice of $m$ implies 
\begin{align*}
\mathrm{scl}_{\pi_1(K(s))}(p_s(g^{m}h^{n_0})) &> m\, \mathrm{scl}_{\pi_1(K(s))}(p_s(g)) - n_0\,\mathrm{scl}_{\pi_1(K(s))}(p_s(h))-\frac{1}{2}\\
...
Following Claim~\ref{s=0}, 
if $g \not\in [G(K), G(K)]$, 
we may assume $s \ne 0$, 
thus $K(s)$ is a rational homology $3$--sphere and $\pi_1(K(s))/[\pi_1(K(s)),\ \pi_1(K(s))]$ is finite, 
so we may still apply Lemma~\ref{scl_product} 
to follow the same discussion in Case 1. 
...
we may observe that there are infinitely many integers integer $m$ with $C_{n_0} < m \in M_{n_0}$ for each prime integer $n_0$ coprime to $a_1, \dots, a_k$. 
...
In the case where $K$ is a hyperbolic knot without torsion surgery, 
we may simplify the statement of Lemma~\ref{S_K_intersection} as follows. 

\begin{lemma_S_K_intersection_no_torsion}[Shrinking Lemma I']
Let $K$ be a hyperbolic knot without torsion surgery. 
Let $g$ be a non-peripheral element and $h$ a non-trivial element in $[G(K),G(K)]$. 
Then for each non-zero integer $n$, we have a constant $N_n > 0$ such that  
\[
...
\] 
for all integers $m \ge N_n$ for each $n$.
\end{lemma_S_K_intersection_no_torsion}

\begin{proof} 
Just follow the argument in the proof of Lemma~\ref{s_not_in_S_hyperbolic} skipping the discussion for 
torsion elements. 
\end{proof}
...
Since $\mathcal{S}_K(g_1)$ is a finite subset of $\mathbb{Q}$ (Theorem~\ref{finiteness}), 
we may write 
\[
...
\renewcommand{\labelenumi}{(\arabic{enumi})}
\item 
An element $g$ with $\mathcal{S}_K(g) = \emptyset$ is necessarily 
non-peripheral. 
Indeed, if $g$ is conjugate to a power of a slope element $\mu^p\lambda^q$, 
then $\mathcal{S}_K(g)$ contains $p/q$. 
...
\section{Elements with identical trivializing slope set } 
...
\begin{thm_non_rigid}[Identical Dehn filling trivializing slope set]
Let $K$ be a hyperbolic knot without torsion surgery slope. 
...
Since $\mathcal{S}_K(g)$ is a finite set (Theorem~\ref{finiteness}), 
...
\]
where $m \in \mathbb{Z}$ and $h = s^{-1} g s$.  

Note that since $g \in [G(K), G(K)]$, $h$ also belongs to $[G(K), G(K)]$. 
It follows from Lemma~\ref{S_K_intersection_no_torsion} that for a given $-p+1$, 
...
TI has been partially supported by JSPS KAKENHI Grant Number JP19K03490, 21H04428 and 23K03110. 
...
Geom.\ Topol.\ \textbf{4} (2000), 431--449. 
