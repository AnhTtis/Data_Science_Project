...
Each robot $i \in \{1, \dots, N_A\}$ also has a sensor (measurement) model described by
\begin{equation}
...
\label{sec:Est Cont Motion Plan}
...
$x^i_k \sim b(x_k^i)=\N(\hat{x}_k^i,\Sigma_k^i)$, where $\hat{x}_k^i \in \X^i$ is the state estimate (mean), and $\Sigma_k^i \in \reals^{n_i \times n_i}$ is the covariance matrix.  

We define \emph{motion plan} for robot $i$ to be a pair $(\check{U}^i,\check{X}^i)$ of nominal controls $\check{U}^i=(\check{u}_0^i, \check{u}_1^i, \ldots, \check{u}_{T-1}^i)$ and the corresponding trajectory of nominal states $\check{X}^i=(\check{x}_0^i, \check{x}_1^i, \ldots, \check{x}_T^i)$, where $\check{X}^i$ is obtained via propagation of the dynamics in~\eqref{eq: dynamics model} under $\check{U}^i$ in the absence of process noise.  Then, robot $i$ executes motion plan $(\check{U}^i, \check{X^i})$ by using a proportional stabilizing feedback controller 
$u_k^i = \check{u}_{k}^i - K(\hat{x}_k^i - \check{x}_k^i),$ 
...
Given $N_A$ robots with uncertain dynamics in \eqref{eq: dynamics model}, noisy measurements in \eqref{eq: measurement model}, and controller from Sec.~\ref{sec:Est Cont Motion Plan} in workspace $\W$ with static obstacles $\W_O$, 
...
% \textcolor{blue}{Note that most related work, e.g., \cite{Patwardhan2023_DistMRMP,AlonsoMora2018_CoopCollAvoid,Senbaslar2019_MultiRobotDistributed,VanParys2016_OnlineDistMultiVehicle}, solves the uncertain MRMP problem using \emph{online} (iterative) approaches with finite horizon predictions. These do not provide safety or reachability guarantees over the entire plan, contrary to the above stated problem. This implies the plans must be computed \emph{offline} while the safety guarantees must hold at run time. The advantage to this approach is that the computed plans are sound, whereas the online methods may produce incorrect solutions, e.g., run into a deadlock. In this work, we seek an offline MRMP algorithm that is both sound and has probabilistic completeness properties, i.e., generates a valid plan almost-surely if one exists.
% }

Note that, unlike most related work, e.g., \cite{Patwardhan2023_DistMRMP,AlonsoMora2018_CoopCollAvoid,Senbaslar2019_MultiRobotDistributed,VanParys2016_OnlineDistMultiVehicle}, that solve the uncertain MRMP problem using \emph{online} (iterative) approaches with finite horizon predictions, the above problem asks for the entire plan with safety guarantees.  This implies that the plans must be computed \emph{offline} while the chance-constraint guarantees must hold at run time. The advantage to this approach is that the computed plans are ensured to be sound, i.e., hold hard guarantees on safety and reachability, before deployment, whereas the online methods may produce incorrect solutions, e.g., run into a deadlock.  
% In this work, we seek an offline MRMP algorithm that, in addition to soundness, has probabilistic completeness properties, i.e., generates a vaxlid plan almost-surely if one exists.
In this work, we seek an offline MRMP algorithm that guarantees soundness \emph{and} exhibits probabilistic completeness, i.e., generates a valid plan almost-surely 
if one exists.

% \textcolor{blue}{
% Note that, unlike most related work, e.g., \cite{Patwardhan2023_DistMRMP,AlonsoMora2018_CoopCollAvoid,Senbaslar2019_MultiRobotDistributed,VanParys2016_OnlineDistMultiVehicle}, that uses \emph{online} approaches with finite horizon predictions to solve the uncertain MRMP problem, the above problem requires \emph{offline} computation of the entire plan with chance-constraint guarantees at run time. This ensures that the computed plans are sound and hold hard guarantees on safety and reachability before deployment. In this work, we specifically seek an offline MRMP algorithm that not only guarantees soundness but also has probabilistic completeness property, i.e., generates a valid plan almost-surely if one exists.
% }