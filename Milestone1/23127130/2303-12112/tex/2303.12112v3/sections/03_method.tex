...
\begin{equation}
    \label{eq:emscore}
    \text{Score}(t, V) = \frac{\text{Score}(t, V)_c + \text{Score}(t, V)_f}{2},
\end{equation}
...
\begin{equation}
    ...
    \text{Ref-Score}(t, V, r) = \frac{\text{Score}(t, V) + \max_{r \in R}\text{Score}(t, r)}{2},
\end{equation}
where $\text{Score}(t, r)$ is computed as defined in Eq.~\ref{eq:emscore} by using the word-level embeddings of the reference caption.
