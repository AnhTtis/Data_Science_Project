...
In Subsection~\ref{drei} (resp. Subsection~\ref{zwei}), we prove Theorem~\ref{archard1} (resp. Theorem~\ref{verhard}) in the remaining cases, that is for every $t\geq 2$ and every positive integer $k$.  
  Finally, in Subsection~\ref{vier} we prove Theorems~\ref{approx1} and~\ref{approx2}.
Note that all the studied decision problems are in NP since a $k$-strenghtening (resp. $k$-arc-strenghthening) family ${\cal X}$ is a certificate as checking whether $\Inv(D; {\cal X})$ is $k$-strong (resp. $k$-arc-strong) can be done in polynomial time, e.g. using the algorithms cited in the proof of Corollary 1.17. Therefore we just need to prove their hardness.


\subsection{One inversion}\label{eins}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{proof}
    ...
    Before giving the main proof that our reduction works, we show the following intermediate result.  \begin{claim}\label{force}
    ...
    By Claim~\ref{force}, we have $\phi'(y_1)\neq \phi'(z_1)$.
By symmetry, we may assume that $\phi'(y_1)=\true$ and $\phi'(z_1)=\false$.
...
For any fixed positive integer $k$, deciding whether $\sinv'_k(D)\leq 1$ for a given oriented graph $D$ is NP-complete.
\end{theorem}


\begin{theorem}\label{ver1}
For any fixed positive integer $k$, deciding whether $\sinv_k(D)\leq 1$ for a given $k$-strengthenable oriented graph $D$ is NP-complete.
\end{theorem}
...
is a reduction from ME$k$SAT which is NP-hard by Proposition~\ref{equ}. Fix some integer $k \geq 1$ and let $(X,\mathcal{C})$ be an instance of ME$k$SAT. We construct an oriented graph $D(X,\mathcal{C})$ as follows. 
We start from the disjoint union of a $k$-strong tournament $T$ with vertex set $S = \{s_1,\ldots,s_{2k+1}\}$ and two stable sets 
...
Now, for every $C \in \mathcal{C}$, we reverse $k$ arcs between $w_C$ and its $2k+1$ in-neighbours in $V$. Then each $w_C$ has at least $k$ in-neighbours and $k$ out-neighbours in $S\cup V$. So, by Proposition~\ref{lem:kstrong+} again, the resulting digraph is $k$-strong.
...
The {\bf cut covering number} of $G$, denoted by $\cc(G)$, is the minimum integer $t$ such that there is a cut cover of $G$ of size $t$. The following well-known result proved in \cite{https://doi.org/10.1002/jgt.3190010208} shows a close relationship between the cut covering number and the classical chromatic number.

\begin{proposition}[Harary et al.~\cite{https://doi.org/10.1002/jgt.3190010208}]\label{prop:cc-chi}
...
The main technical part of the reduction is contained in the following Lemma that will be reused in Section~\ref{vier}.
...
Let $S \subseteq V(D)$ be non-empty and suppose that $\min\{d_{D'}^-(S),d_{D'}^{+}(S)\}<k$ .
...
As $D'\langle Z_e\rangle=D\langle Z_e\rangle$ and $D\langle Z_e\rangle$ is $k$-arc-strong by construction, we obtain that either $Z_e \subseteq S$ or $Z_e \cap S=\emptyset$.
...
Further, $A(D')$ contains the arcs $uz_e^{i}$ and $z_e^{i}v$ for all $i \in \{2,\ldots,k\}$.
...
\end{claim}
\begin{proofclaim}
Suppose otherwise. As $D'$ is $k$-arc-strong and $d_{D}(Z_e)=2k$, we have $d_{D'}^-(Z_e)= d_{D'}^+(Z_e)=k$. By assumption, for every $i \in[t]$ either both $u$ and $v$ or neither $u$ and $v$ are contained in $X_i$. Thus we deduce that for every $j \in [k]$, either both the arcs corresponding to edges in $\partial_{D}(Z_e)\cap \partial_{D}(z_e^{j})$ are inverted or neither of them are. Hence, in $D'$,  there is exactly one arc entering $Z_e$ incident to $z_e^{j}$ for $j=2,\ldots,k$, and there are either zero or two arcs entering $Z_e$ incident to $z_e^{1}$. Thus $d_{D'}^-(Z_e)\in \{k-1,k+1\}$, a contradiction.
\end{proofclaim}
...
\begin{proof}
We let $V(D)$ be the union of $V(G)$, a set of vertices  $Z_e=\{z^1_e,\ldots,z_e^{2k+1}\}$ for every $e \in E(G)$ and a set of vertices $W=\{w_1,\ldots,w_{2k+1}\}$. 
...
\end{claim}
\begin{proofclaim}
Suppose otherwise, that is for every $i \in [t]$ either both $u$ and $v$ or neither $u$ and $v$ are in $X_i$.
Thus both the arcs corresponding to edges in $\partial_{D}(Z_e)\cap \partial_{D}(z_e^{1})$ are inverted or none of them. This yields that in $D'- \{w_1,\ldots,w_{k-1}\}$,  there is either no arc entering $Z_e$ or no arc leaving $Z_e$. Hence $D'-\{w_1,\ldots,w_{k-1}\}$ is not strongly connected, a contradiction.
\end{proofclaim}
...
\subsection{Inapproximability}\label{vier}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
