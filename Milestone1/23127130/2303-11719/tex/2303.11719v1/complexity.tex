...
%For both parameters, we obtain hardness results for all possible values of $k$ and all possible numbers $t$ of inversions. More concretely, we prove the following results:
...
In Subsection \ref{zwei} (resp. Subsection~\ref{drei}), we prove Theorem~\ref{verhard} (resp. Theorem~\ref{archard1}), in the remaining cases, that is for all $t\geq 2$ and all positive integer $k$.  
...
\begin{proof}
    ...
    Let $(X,\mathcal{C})$ be an instance of ME$k$SAT. We now create an instance $(X',\mathcal{C}')$ of ME$(k+1)$SAT. First, we let $Y=\{y_1,\ldots,y_{k+3}\}$ and $Z=\{z_1,\ldots,z_{k+3}\}$ be sets of new variables. We now let $\mathcal{C}_1$ contain all clauses $C \subseteq Y \cup Z$ with $|C|=2k+3$ and $k+1\leq |C \cap Y|\leq k+2$. Further, we let $\mathcal{C}_2 = \{C\cup \{y_1, z_1\} \mid C \in \mathcal{C}\}$. Finally, we set $X'=X\cup Y \cup Z$ and $\mathcal{C}'=\mathcal{C}_1\cup \mathcal{C}_2$. We now show that $(X',\mathcal{C}')$ is a positive instance of ME$(k+1)$SAT if and only if $(X,\mathcal{C})$ is a positive instance of ME$k$SAT.

    First suppose that $(X,\mathcal{C})$ is a positive instance of ME$k$SAT, so there is an assignment $\phi:X \rightarrow \{\true,\false\}$ such that every clause in $\mathcal{C}$ contains at least $k$ true and at least $k$ false variables with respect to $\phi$. Define $\phi':X' \rightarrow \{\true,\false\}$ by $\phi'(x)=\phi(x)$ for all $x \in X$ and $\phi'(y_i)=\true$ and $\phi'(z_i)=\false$ for $i\in[k+3]$. By construction, every clause in $\mathcal{C}_1$ contains at least $k+1$ true and at least $k+1$ false variables with respect to $\phi'$. Further, for every clause $C\cup \{y_1, z_1\} \in \mathcal{C}_2$, the clause $C$ contains at least $k$ true and at least $k$ false variables with respect to $\phi$. As $\phi'(y_1)=\true$ and $\phi'(z_1)=\false$, we obtain that $C\cup \{y_1, z_1\}$ contains at least $k+1$ true and at least $k+1$ false variables with respect to $\phi'$. Hence $(X',\mathcal{C}')$ is a positive instance of ME$(k+1)$SAT.
    ...
    \begin{claim}\label{force}
        We have $\phi'(y_1)=\ldots=\phi'(y_{k+3})\neq \phi'(z_1)=\ldots=\phi'(z_{k+3})$.
    \end{claim}
    \begin{proofclaim}
        First suppose that both $X$ and $Y$ contain both true and false variables with respect to $\phi'$, say $\phi'(y_1)=\phi'(z_1)=\true$ and $\phi'(y_2)=\phi'(z_2)=\false$. Let $C_1=Y \cup Z \setminus \{y_2,z_2,z_3\}$ and $C_2= Y \cup X \setminus \{y_1,z_1,z_3\}$. Further, let $\alpha_1,\alpha_2$ be the number of true variables with respect to $\phi'$ in $C_1$ and $C_2$, respectively. As $C_1,C_2 \in \mathcal{C}_1$, we have $k+1\leq \alpha_i\leq k+2$ for $i=1,2$. On the other hand, by assumption, we have $\alpha_2=\alpha_1-2$. This yields $k+1 \leq \alpha_2=\alpha_1-2\leq (k+2)-2=k$, a contradiction.
    ...
    \end{proofclaim}
By symmetry and Claim \ref{force}, we may suppose that $\phi'(y_i)=\true$ and $\phi'(z_i)=\false$ for all $i\in[k+3]$. Let $\phi$ be the restriction of $\phi'$ to $X$. For every $C \in \mathcal{C}$, the clause $C\cup \{y_1, z_1\}$ contains at least $k+1$ true and at least $k+1$ false literals with respect to $\phi'$. As $\phi'(y_1)=\true$ and $\phi'(z_1)=\false$, we obtain that $C$ contains at least $k$ true and at least $k$ false literals with respect to $\phi$. Hence $(X,\mathcal{C})$ is a positive instance of ME$k$SAT.
\end{proof}
We here prove the following results.


\begin{theorem}\label{arc1}
Deciding whether $\sinv'_k(D)\leq 1$ for a given ($k$-strengthenable) oriented graph $D$ is NP-complete.
\end{theorem}
...
is a reduction from ME$k$SAT which is NP-hard by Proposition \ref{equ}. Fix some $k \geq 2$ and let $(X,\mathcal{C})$ be an instance of ME$k$SAT. We now create an oriented graph $D(X,\mathcal{C})$ as follows. 
...
Indeed $D \langle S\rangle$ is $k$-strong and hence $S$ is $k$-strong in $D(X,\mathcal{C})$.
...
In order to prove Theorem~\ref{verhard} and \ref{archard1} for $t\geq 2$ and all $k$, we shall use another well-studied graph parameter. Let $G$ be a graph. For any set $X\subset V(G)$, we denote by 
$\partial_G(X)$, the {\bf cut associated to} $X$, that is the set of edges having exactly one end-vertex in $X$.
A {\bf cut cover} of $G$ is a collection $X_1,\ldots,X_t$ of subsets of $V(G)$ such that $\cup_{i=1}^t \partial_G(X_i)=E(G)$.
The {\bf cut covering number} of $G$, denoted by $\cc(G)$, is the minimum integer $t$ such that there is a cut cover of size $t$. The following result shows a close relationship between the cut covering number and the classical chromatic number.

\begin{proposition}[Folklore]\label{prop:cc-chi}
...
Given a digraph $D$ and a set $X$ of vertices in $D$, we denote by $\delta_D(X)$ the set of edges of $\UG(D)$ with exactly one end-vertex in $X$.
...
Observe that either $vz_e^1,z_e^1u \in A(D')$ (if $u \in X_i$) or $z_e^1v,uz_e^1 \in A(D')$ (if $v \in X_i$).
...
\end{claim}
\begin{proofclaim}
Suppose otherwise. As $D'$ is $k$-arc-strong and $d_{D}(Z_e)=2k$, we have $d_{D'}^-(Z_e)= d_{D'}^+(Z_e)=k$. For $j\in[k]$, observe that in any iteration $i$, as either both $u$ and $v$ or none of $u$ and $v$ are contained in $X_i$ either both the arcs corresponding to edges in $\delta_{D}(Z_e)\cap \delta_{D}(z_e^{j})$ are inverted or none of them. Hence, in $D'$,  there is exactly one arc entering $Z_e$ incident to $z_e^{j}$ for $j=2,\ldots,k$, and there are either zero or two arcs entering $Z_e$ incident to $z_e^{1}$. Thus $d_{D'}^-(Z_e)\in \{k-1,k+1\}$, a contradiction.
\end{proofclaim}
By Claim~\ref{par1}, we obtain that $(X_1',\ldots,X_t')$ is a cut cover for $G$. We hence have $\cc(G)\leq t=\sinv'_k(D)$.
\end{proof}
...
We now show a reduction from the cut covering problem to the one of deciding whether $\sinv_k(D) \leq t$  for a given oriented graph $D$.



\begin{lemma}\label{lem:reduc3}
Given a graph $G$ and a positive integer $k$, one can construct in polynomial time some $k$-strengthenable oriented graph $D$ such that $\sinv_k(D)=\cc(G)$ and $|V(D)|=|V(G)|+3k|E(G)|+2k+1$.
\end{lemma}

\begin{proof}
We let $V(D)$ contain $V(G)$, a set of vertices  $Z_e=\{z^1_e,\ldots,z_e^{2k+1}\}$ and a set of vertices $X_e=\{x^1_e,\ldots,x_e^{k-1}\}$ for every $e \in E(G)$ and a set of vertices $X=\{x_1,\ldots,x_{2k+1}\}$. 
We add arcs to $D$ such that $D\langle X\cup \bigcup_{e \in E(G)}X_e\rangle$ is a $k$-strong tournament.
We let $A(D)$ contain an arc from $v$ to $x_i$ for every $v \in V(G)$ and every $i\in[k]$ and an arc from $x_i$ to $v$ for every $v \in V(G)$ and every $i\in \{k+1,\ldots,2k\}$.
Next for every $e \in E(G)$, we add arcs to $D$ such that $D\langle Z_e\rangle$ is a $k$-strong tournament. 
Further, for every $e \in E(G)$, $i\in [k]$ and $j\in [k-1]$, we add an arc from $z_e^{i}$ to $x_e^{j}$ and for every $e \in E(G)$, $i\in\{k+1,\ldots,2k\}$ and $j\in [k-1]$, we add an arc from  $x_e^{j}$ to $z_e^{i}$. 
Finally, for every $e=uv \in E(G)$, we add the arcs $u z_e^{1}$ and $v z_e^{1}$. This finishes the description of $D$. 
Observe that $|V(D)|=|V(G)|+3k|E(G)|+2k+1$. 
...
Now for every $e=uv \in E(G)$, there is a smallest $\alpha(e)$ such that exactly one of $u$ and $v$ is contained in $X_i$. 
...
As $D\langle X\cup \bigcup_{e \in E(G)}X_e\rangle$ is $k$-strong, we obtain that $X\cup \bigcup_{e \in E(G)}X_e-Y$ is contained in a single strongly connected component $S$ of $D'-Y$. Next observe that every $v \in V(G)-Y$ has an in-neighbour and an out-neighbour in $S$, so $V(G)-Y \subseteq S$. 
Now consider some $e=uv \in E(G)$. As $D'\langle Z_e\rangle=D\langle Z_e\rangle$ is $k$-strong, we obtain that $Z_e-Y$ is contained in a single connected component of $D'-Y$. 
If $Y\neq X_e$, then there is at least one arc from $Z_e-Y$ to $X_e-Y$ and there is at least one arc from $X_e-Y$ to $Z_e-Y$, so $Z_e-Y \subseteq S$. 
Finally, if $Y=X_e$, observe that by construction, either $D'$ contains the arcs $uz_e^1$ and $z_e^1v$ or the arcs $vz_e^1$ and $z_e^1u$. 
Again, we obtain $Z_e-Y \subseteq S$. This yields that $D'$ is $k$-strong. We hence have $\sinv_k(D)\leq t=\cc(G)$.
...
Let $e=uv \in E(G)$. Then there is some $i \in \{1,\ldots,t\}$ such that $X'_i$ contains exactly one of $u$ and $v$.
\end{claim}
\begin{proofclaim}
Suppose otherwise. Observe that in any iteration $i$, as either both $u$ and $v$ or none of $u$ and $v$ are contained in $X_i$ either both the arcs corresponding to edges in $\delta_{D}(Z_e)\cap \delta_{D}(z_e^{1})$ are inverted or none of them. This yields that in $D'-X_e$  there are either zero or two arcs entering $Z_e$. This yields that $D'-X_e$ is not strongly connected, a contradiction.
\end{proofclaim}
By Claim \ref{par2}, we obtain that $(X_1',\ldots,X_t')$ is a cut cover for $G$. We hence have $\cc(G)\leq t=\sinv_k(D)$.
Hence $\cc(G) = \sinv_k(D)$.
\end{proof}
...
In order to prove Theorems \ref{approx1} and \ref{approx2}, we need the following result of Zuckerman \cite{v003a006}.
\begin{proposition}\label{zuck}
Unless P=NP, there is no polynomial-time $\bigO(n^{1-\epsilon})$-approximation algorithm for computing $\chi(G)$ for any $\epsilon >0$.
\end{proposition}

We first use this to obtain a negative result concerning the cut covering number of a given graph.
...
Now Proposition \ref{approcc} and Lemma \ref{lem:reduc3} imply Theorem \ref{approx1} and Proposition \ref{approcc} and Lemma \ref{lem:reduc2} imply Theorem \ref{approx2}.
