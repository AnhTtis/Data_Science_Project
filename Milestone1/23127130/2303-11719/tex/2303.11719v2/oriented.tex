...
Let $G$ be an $n$-vertex $d$-degenerate graph $G$.
...
For sake of completeness, we give the proof of this theorem.
We denote by $\mathbb{F}_2$ the field with two elements $0$ and $1$.
Given two vectors $\vec{x},\vec{y} \in \mathbb{F}_2^t$, we denote by $\vec{x} \cdot \vec{y}$ their inner product, that is $\sum_{i=1}^t \vec{x}_i \vec{y}_i \in \mathbb{F}_2$.

\begin{proof}
    Set $t= \log n +2d-1$.
    Let $\vec{G}_1,\vec{G}_2$ be two orientations of $G$.
    For every edge $uv \in E(G)$, we define $c(uv) \in \mathbb{F}_2$ such that $c(uv)=0$ if and only if $uv$ is oriented the same in $\vec{G}_1$ and in $\vec{G}_2$.
    Let $(v^1, \dots, v^n)$ be a $d$-degenerate ordering of $G$.
    We will iteratively build vectors $\vec{v}^1, \dots, \vec{v}^n \in \mathbb{F}_2^t$ such that, for every $i \in [n]$,
   % when $\vec{v}^1, \dots, \vec{v}^i$ are constructed, 
   the following conditions hold:
    \begin{enumerate}
        \item[$A^i$]:\label{item:lin_indep}  for every $\ell >i$, the vectors of $\{\vec{v}^j \mid 1\leq j \leq i \mbox{~and~} v^j \in N(v^\ell)\}$ are linearly independent, and 
        \item[$B^i$]:\label{item:correct_orientation} for every edge $v^j v^{j'}$ with $j,j' \leq i$, $\vec{v}^j \cdot \vec{v}^{j'} = c(v^j v^{j'})$. 
    \end{enumerate}
    This will prove the theorem.
    Indeed, given such vectors, we can define the inversions $X_i = \{u \mid \vec{u}_i=1\}$ for every $i\in\{1, \dots, t\}$.
    Then the arc $uv$ is reversed by $X_1, \dots, X_t$ if and only if $\vec{u} \cdot \vec{v} = 1$.
    Hence Condition $B^n$ implies that $\Inv(\vec{G}_1; X_1, \dots, X_t) = \vec{G}_2$.

    We now build this family of vectors as follows.
    For $i=1$, we take $\vec{v}^1 \in \mathbb{F}_2^t \setminus \{\vec{0}\}$ arbitrarily.
    Now suppose that we have $\vec{v}^1, \dots, \vec{v}^i$ satisfying $A^i$ and $B^i$. We shall now choose $\vec{v}^{i+1}$.
    Satisfying $A^{i+1}$ forbids a vector space of dimension at most $d-1$ for each neighbour $v^\ell$ of $v^{i+1}$ with $\ell>i+1$, because $v^\ell$ has at most $d-1$ neighbours %distinct from $v^{i+1}$
    in $\{v^1, \dots , v^{i}\}$ by $d$-degeneracy of $G$. So, in total, satisfying $A^{i+1}$ forbids the union of those vector spaces, whose size is at most $1+n(2^{d-1}-1)$.
    Satisfying $B^{i+1}$ allows at least $2^{t-d}$ values for $\vec{v}^{i+1}$. 
    Indeed, according to $B^i$ the vectors of $\{\vec{v}^j \mid 1\leq j\leq i \mbox{~and~} v^j \in N(v^{i+1})\}$ are linearly independent, so the affine space
    of the solutions of the system of equations $\vec{v}^j \cdot \vec{v}^{i+1} = c(v^j v^{i+1})$ for the at most $d$ indices $j$ such that $1\leq j\leq i$ and $v^j \in N(v^{i+1})$,  has dimension at least $t-d$.
    Since $2^{t-d} = 2^{d-1+\log n} > 1+n(2^{d-1}-1)$,
    there is a vector $\vec{v}^{i+1} \in \mathbb{F}_2^t$ such that both $A^{i+1}$ and $B^{i+1}$ are satisfied.
    This concludes the proof of the theorem.
\end{proof}

In a multigraph $G$, for every pair $(u,v)$ of vertices, a {\bf $(u,v)$-cut} is 
a set $F$ of edges such that every $(u,v)$-path in $G$ intersects $F$. A {\bf cut} is a $(u,v)$-cut for some $u,v$.
...
\begin{lemma}\label{lemma:min_k_connected_mad}
    Let $k$ be a positive integer and let $G$ be an $n$-vertex multigraph.
...
\begin{lemma}\label{sizet}
    For every positive integer $t$, there exists a 2-edge-connected digraph $D$ on $2^{t-1}+1+\binom{2^{t-1}+1}{2}$ vertices with a specified vertex $s$ such that in each digraph obtained from $D$ by applying at most $t-1$ inversions contains a sink or source distinct from $s$.
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{extreminf}
    For every pair of positive integers $n,k$ with $n \geq 2k+1$, there is a $2k$-edge-connected digraph $D$ on $n$ vertices such that $\sinv'_k(D) \geq \frac{1}{2}\log(n -k+1)$.
\end{lemma}

\begin{proof}
By assumption, we have $n-k+1\geq k+2\geq 3$.
    Hence, by Lemma \ref{arbn1}, there exists a 2-edge-connected digraph $D_0$ on $n-k+1$ vertices such that any digraph obtained from $D_0$ by inverting less than $\frac{1}{2} \log(n-k+1)$ sets has a sink or a source. %There are infinitely many such oriented graphs by Lemma~\ref{thm:inf_bound_sinv'_1}.
    ...
    Consider now a family of subsets $\cal X$ such that
