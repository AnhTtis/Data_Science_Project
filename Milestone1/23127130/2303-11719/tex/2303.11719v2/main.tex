...
\usepackage{times,amsmath,amsthm,amssymb,dsfont,graphicx,xspace,epsfig,xcolor}
...
\item[(ii)] for any fixed positive integers $k$ and $t$, deciding whether a given oriented graph $\vec{G}$ satisfies $\sinv'_k(\vec{G}) \leq t$ (resp. $\sinv_k(\vec{G}) \leq t$) is NP-complete ; 
\item[(iii)] if $T$ is a tournament of order at least $2k+1$, then
$\sinv'_k(T) \leq \sinv_k(T) \leq 2k$, 
and $\sinv'_k(T) \leq \frac{4}{3}k+o(k)$;

\item[(iv)] $\frac{1}{2}\log(2k+1) \leq \sinv'_k(T) \leq \sinv_k(T)$ for some tournament $T$ of order $2k+1$; 

\item[(v)] if $T$ is a tournament of order at least $19k-2$ (resp. $11k-2$), then
$\sinv'_k(T) \leq \sinv_k(T) \leq 1$ (resp. $\sinv_k(T) \leq 3$);
\item[(vi)] for every $\epsilon>0$, there exists $C$ such that $\sinv'_k(T) \leq \sinv_k(T) \leq C$ for every tournament $T$ on at least $2k+1 + \epsilon k$ vertices.
\end{itemize}
