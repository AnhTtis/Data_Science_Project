...
In this section, we prove Theorem~\ref{thm:exttournoi}, that is $\frac{1}{2} \log(2k+1) \leq M_k \leq 2k$ for all sufficently large $k$.
...
% &\leq \left(1+o(1)+2(c-1)\exp(-\frac{1+o(1)}{2n})\right)^n \\
...
\subsection{Upper bounds on \texorpdfstring{$M_k$}{Mk} and transitive tournaments}
%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we give some upper bounds on $M_k$ for all positive intergs $k$. Most notably, we prove the upper bound in Theorem \ref{thm:exttournoi}.
...
\begin{proof}
    Let $(v_1, \dots, v_n)$ be an ordering of $V(TT_n)$ such that
...
\subsection{Upper bound on \texorpdfstring{$M'_k$}{M'k}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{proposition}\label{extend}
    Let $T$ be a tournament on $2k+1$ vertices and $X \subseteq V(T)$ with $|X|\leq \frac{2}{3}k$ such that $d_T^-(x)=d_T^+(x)=k$ for all $x \in X$. Then there exists a $k$-arc-strong tournament $T'$ on $V(T)$ such that all the edges in $\UG(T)\langle X \rangle \cup \delta_{\UG(T)}(X)$ have the same orientation in $T$ and $T'$.
\end{proposition}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &=\left(d_H^+(S)-d_H^+(x,V(H)\setminus(S \cup \{x\}))+d_H^+(S\setminus \{x\},x)\right)\\
        &\qquad -\left(d_H^-(S)-d_H^-(x,V(H)\setminus(S \cup \{x\}))+d_H^-(S\setminus \{x\},x)\right)\\
        &=d_H^+(S)-d_H^+(S)\\
        &\qquad +d_H^+(S\setminus \{x\},x)+d_H^+(V(H)\setminus(S \cup \{x\}),x)\\
        &\qquad -d_H^+(x,V(H)\setminus(S \cup \{x\}))-d_H^+(x,S\setminus \{x\}))\\
        &=d_H^+(S)-d_H^+(S)+d_H^-(x)-d_H^+(x)\\
        &=d_H^+(S)-d_H^-(S).
    \end{align*}
    Repeatedly applying this argument, we obtain $d_H^+(S)-d_H^-(S)=d_H^+(S \setminus X)-d_H^-(S \setminus X) \leq d_H^+(S \setminus X) \leq |S \setminus X| \cdot |X|$ since every arc in $H$ have an extremity in $X$. Symmetrically, we obtain $d_H^+(S)-d_H^-(S)=d_H^-(V(H)\setminus S)-
...
We choose $k_0$ large enough so that for all $k \geq k_0$, the  two following inequalities hold:
...
$16 \log k \lceil \frac{1}{4}\log k\rceil \leq \sqrt{k}$.
...
For some $b \in B$ and a tournament $\tilde{T}$ on $V(T)$, we denote $|k-d_{\tilde{T}}^-(b)|$ by {\boldmath $\defect_{\tilde{T}}(b)$.} Let $q=\lceil\frac{1}{4}\log k\rceil$ and $q^* = \lceil \log^2 k\rceil$. We now choose $q^*$ sets $X_1,\ldots,X_{q^*}\subseteq V(T)$ independently and uniformly at random. 
...
\begin{enumerate}[label=\Alph*]
    \item[$E_1$]: There is some $b \in B$ with $\defect_{T'}(b)\geq 2 \log k \sqrt{k}$.
    \item[$E_2$]: There are some $b_1,\ldots,b_q \in B$ such that $\vec{b}_1,\ldots,\vec{b}_q$ are linearly dependent.
    \item[$E_3$]: There are some $b_1,\ldots,b_q \in B$ such that $\vec{b}_1,\ldots,\vec{b}_q$ are linearly independent and a sequence $(\star_1,\ldots,\star_q)\in \{+,-\}^q$ such that $|\bigcap_{i=1}^q N_{T'}^{\star_i}(b_i)\cap A|\leq 5 \log k \sqrt{k}$.
\end{enumerate}
...
\end{claim}
\begin{proofclaim}
    Let $p=\max\{\defect_{\Inv(T';B_i)}(b) \mid b \in B_i\}$. Observe that $p\leq  \max\{\defect_{T'}(b)+q-1 \mid b \in B_i\}$. Hence, as $E_1$ does not occur and $q=\lceil\frac{1}{4}\log k\rceil\leq \log k \sqrt{k}$, we have $p\leq 2 \log k \sqrt{k}+ \log k \sqrt{k}=3 \log k \sqrt{k}$. We shall now build a sequence of sets $Y_i^0,\ldots,Y_i^p$ with the following properties:
    ...
    As $E_2$ does not occur, $\vec{b}_1,\ldots,\vec{b}_q$ are linearly independent. Moreover, as $E_3$ does not occur, $|\bigcap_{\mu=1}^q N_{T'}^{\star_\mu}(b_\mu)\cap A|\geq 5 \log k \sqrt{k}$. As $p \leq 3 \log k \sqrt{k}$, and $q \leq \log k \sqrt{k}$, we have $|Y_i^j|\leq p+q \leq 4 \log k \sqrt{k}$. 
    Thus $(\bigcap_{\mu=1}^q N_{T'}^{\star_\mu}(b_\mu)\cap A)\setminus Y_i^j\neq \emptyset$. We can hence choose some $y \in (\bigcap_{\mu=1}^q N^{\star_\mu}(b_\mu)\cap A)\setminus Y_i^j$ and define $Y_i^{j+1}=Y_i^j\cup \{y\}$. By definition, $Y_i^{j+1}$ is obtained from $Y_i^j$ by adding one vertex in $A\setminus Y_i^j$. 
    In order to see that the last property holds, let $\mu \in [q]$. If $\max\{\defect_{\Inv(T';Y_i^j)}(b)\mid b \in B_i\}=d_{\Inv(T';Y_i^j)}^-(b_\mu)-k$, we have $\star_\mu=-$ and hence $y \in N^-_{\Inv(T';Y_i^j)}(b_\mu)$. 
    As $d_{\Inv(T';Y_i^j)}^-(b_\mu)-k >0$, we obtain $\defect_{\Inv(T';Y_i^{j+1})}(b_\mu)=d_{\Inv(T';Y_i^{j+1})}^-(b_\mu)-k=d_{\Inv(T';Y_i^j)}^-(b_\mu)-k-1=p-j-1=p-(j+1)$. Similarly, if $\max\{\defect_{\Inv(T';Y_i^j)}(b)\mid b \in B_i\}=k-d_{\Inv(T';Y_i^j)}^-(b_\mu)$, then $\defect_{\Inv(T';Y_i^{j+1})}(b)=p-(j+1).$ 
    Otherwise, let $b' \in B$ with $\defect_{\Inv(T';Y_i^j)}(b')=p-j$.  As $\defect_{T'}(b_\mu) \equiv \defect_{T'}(b') \mod 2$, we have $\defect_{\Inv(T';Y_i^j)}(b_\mu)\equiv \defect_{T'}(b_\mu)+|Y_i^j|-1\equiv \defect_{T'}(b')+|Y_i^j|-1\equiv \defect_{\Inv(T';Y_i^j)}(b') \mod 2$, hence $\defect_{\Inv(T';Y_i^j)}(b_\mu)\leq \defect_{\Inv(T';Y_i^j)}(b')-2$. This yields $\defect_{\Inv(T';Y_i^{j+1})}(b_\mu)\leq \defect_{\Inv(T';Y_i^j)}(b_\mu)+1\leq \defect_{\Inv(T';Y_i^j)}(b')-2+1=p-(j+1)$.
\end{proofclaim}
...
Now suppose that $\vec{b}\neq \vec{0}$. Once $\vec{b}$ revealed, for every $v \in V(T)$, let $\Gamma_v$ be a random variable which is 1 if $T'$ contains the arc $bv$ and 0 otherwise and let $\Gamma=\sum_{v \in V(T-b)}\Gamma_v$. Observe that, due to the independent and uniform choice of $(X_1,\ldots,X_{q^*})$, we have $\Pr(\Gamma_v=1)=\frac{1}{2}$ for all $v \in V(T-b)$ and the $\Gamma_v$ are independent. This yields that $\Gamma\sim \Bin(2k,\frac{1}{2})$. Further, observe that $\defect_{T'}(b)\leq 2 \log k \sqrt{k}$ if and only if $k-2 \log k \sqrt{k}\leq \Gamma \leq k+2 \log k \sqrt{k}$. By Chernoff's Bound (Proposition \ref{chernoff}), we obtain 
...
\begin{align*}
    ...
    & = \frac{1}{k^2}.
\end{align*} 
Similarly, since $2k-\Gamma \sim \Bin(2k,\frac{1}{2})$, we obtain that $\Pr[\Gamma>k+2 \log k \sqrt{k}]\leq \frac{1}{k^2}$.
This yields that 
\begin{align*}
    \Pr[\defect_{T'}(b)&>2 \log k \sqrt{k}]\\&=\Pr[\vec{b}=\vec{0}] \cdot \Pr[\defect_{T'}(b)>2 \log k \sqrt{k}\mid \vec{b}=\vec{0}]+\Pr[\vec{b}\neq 0] \cdot \Pr[\defect_{T'}(b)>2 \log k \sqrt{k}\mid \vec{b}\neq \vec{0}]\\
...
%Since $10 \log k \sqrt{k}\leq k^{\frac{2}{3}}$ and $\lceil\frac{1}{4}\log k\rceil\leq \frac{1}{3}\log k$, we have 
Now, since $10 \log k \leq k^{\frac{1}{8}}$ and $q = \lceil\frac{1}{4}\log k\rceil \leq \frac{3}{8}\log k$, we obtain $5 \log k \sqrt{k} \leq \frac{1}{2}k^{\frac{5}{8}}=\frac{1}{2}k2^{-\frac{3}{8}\log k} \leq \frac{1}{2}k2^{-q}$.
By Chernoff's Bound (Proposition~\ref{chernoff}), we obtain 

\begin{align*}
\Pr[\Gamma<5 \log k \sqrt{k}]&\leq \Pr\left[\Gamma<\left(1-\frac{1}{2}\right)k2^{-q}\right]\\
&\leq \exp\left(-\frac{1}{8}k2^{-q}\right)\\
&\leq \exp\left(-\frac{1}{8}k^{\frac{1}{2}}\right)\\
...
\begin{case}
$2k+1+6\log \sqrt{k}\leq n \leq 4k-2$. 
\end{case}
...
Due to the uniform choice of $X$, we have $\Pr[ab \in A(T)]=\frac{1}{2}$ and that the $\Gamma_a$ are independent. This yields $\Gamma \sim \Bin(2k+1+6\log k \sqrt{k},\frac{1}{2})$. By Chernoff's Bound (Proposition~\ref{chernoff}), we obtain 

\begin{align*}
\Pr[\Gamma<k]&\leq \Pr\left[\Gamma<\left(1-\frac{ \log k \sqrt{k}}{k}\right)\frac{1}{2}\left(2k+1+6\log k \sqrt{k}\right)\right]\\
&\leq \exp(-\log^2(k)/2)\\
&\leq \exp(-2 \log k) ~~~~~~~ \mbox{because $k\geq 16$}\\
&\leq \exp(-2 \ln k)\\
& = \frac{1}{k^2}.
\end{align*} 
Similarly, we obtain $\Pr[|N^+_{T'}(b)\cap A|<k]\leq \frac{1}{k^2}$. As $B$ contains at most $(4k-3)-(2k+1+2\log k \sqrt{k})\leq 2k$ elements, the Union Bound (Proposition \ref{union}) yields that the probability that there is at least one $b \in B$ with $\min\{|N^-_{T'}(b)\cap A|,|N^+_{T'}(b)\cap A|\}\leq k$ is at most $\frac{2}{k^2}2k=\frac{4}{k}<1$, as $k\geq 5$.
...
Further, by the minimality of $T$, there is a collection $\mathcal{X}$ of $\sinv'_k(T_0\langle A\rangle)\leq (\frac{4}{3}+\frac{\epsilon}{2})k+6\log k\sqrt{k}$ sets such that $\Inv(T'_0\langle A \rangle)$ is $k$-arc-strong. Hence, by Proposition~\ref{lem:kstrong+}, we obtain that $\Inv(T;\{X_0\} \cup \mathcal{X})$ is $k$-arc-strong, a contradiction.  

\begin{case}
$2k+2\leq n \leq 2k+6\log k \sqrt{k}$. 
\end{case}
Let $v \in V(T)$ be an arbitrary vertex. We can then find a set $X \subseteq V(T)$ such that for $T'=\Inv(T;X)$, we have $\min\{d_{T'}^+(v),d_{T'}^-(v)\}\geq k$. Further, by the minimality of $T$, there is a collection $\mathcal{X}$ of $\sinv'_k(T)\leq (\frac{4}{3}+\frac{\epsilon}{2})k+n-(2k+1)-1$ sets such that $\Inv(T'\langle A \rangle; \mathcal{X})$ is 2-arc-strong. Hence by Proposition~\ref{lem:kstrong+}, we obtain that $\Inv(T;\{X\}\cup \mathcal{X})$ is $k$-arc-strong, a contradiction.
\end{proof}

\section{Upper bounds on \texorpdfstring{$m_k(n)$}{mk(n)}\label{sec:upper_bound_Mkn}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we prove several results showing that tournaments on significantly more than $2k$ vertices can be made $k$-strong by a small number of inversions. More precisely, in Subsection \ref{first}, we prove Theorem \ref{thm:s1}, in Subsection~\ref{nk10}, we prove Theorem \ref{nk1}, in Subsection \ref{nk3sec}, we prove Theorem \ref{nk3}, and in Subsection \ref{epsgross}, we prove Theorem \ref{thm:2+eps}.
...
\item[\rm (M2)] the vertex $v_i$ dominates at least half of the vertices
  $v_{i+1},v_{i+2},\ldots,v_j$, and the vertex $v_j$ is dominated by at least half of the vertices $v_i,v_{i+1},\ldots,v_{j-1}$.  In particular, each vertex $v_i$, $1 \leq i <n$, dominates its successor $v_{i+1}$. 
...
\begin{proof}
    ...
    Let $A_0$ be the set of vertices in $\{a_{4k+1},\ldots,a_{6k}\}$ which have less than $k$ out-neighbours in $B$ in $T$. Further, let $A_1=\{a_1,\ldots,a_{2k-|A_0|}\}$. 
    Observe that $|A_0 \cup A_1|=|A_0|+|A_1|=2k$. 
    Similarly, let $B_0$ be the set of vertices in $\{b_1,\ldots,b_{2k}\}$ that have less than $k$ in-neighbours in $A$ and let $B_1=\{b_{4k+|B_0|+1},\ldots,b_{6k}\}$. 
...
By construction, there is some $i \in \{1,\ldots,6k\}$ such that $b=v_i$. Let $T'=T\langle\{v_{i},\ldots,v_{n-6k}\}\rangle$. By (M2), every vertex in $((X \cap B)\cup F)\cap \{v_i,\ldots,v_{n-6k}\}$ and every vertex in $C\setminus F$ is reachable from $b$ in $T'$. On the other hand, there is obviously no vertex in $((X \cap B)\cup F)\cap \{v_i,\ldots,v_{n-6k}\}$ reachable from $b$ in $T'-((X \cap B)\cup F)$. % Let $F=Y \cup \{v_{i+1},\ldots,v_{6k}\}$.
By Lemma \ref{median order bounds}, this yields  
\begin{align*}
    2|((X \cap B)\cup F)\cap \{v_i,\ldots,v_{n-6k}\}|&\geq |R^+_{T'}(b)|-|R^+_{T'-((X \cap B)\cup F)}(b)|\\
    &= |R^+_{T'}(b)\cap B|-|R^+_{T'-((X \cap B)\cup F)}(b)\cap B|\\
    & \qquad +|R^+_{T'}(b)\cap (C \cap F)|-|R^+_{T'-((X \cap B)\cup F)}(b)\cap (C \cap F)|\\
    & \qquad +|R^+_{T'}(b)\cap (C \setminus F)|-|R^+_{T'-((X \cap B)\cup F)}(b)\cap (C \setminus F)|\\
    &\geq  |((X \cap B)\cup F)\cap \{v_i,\ldots,v_{n-6k}\}|\\
    & \qquad +|C \setminus F|-|R^+_{T'-((X \cap B)\cup F)}(b)\cap (C \setminus F)|.
\end{align*}
For every $v \in C \setminus F$, if $v$ is reachable from $b$ in $T'-((X \cap B) \cup F)$, then $v$ is clearly reachable from $b$ in $T_1-F$. Since $|X \cap B|\leq 2k$ and $|F|\leq k-1$, we obtain 
\begin{align*}
    |R^+_{T_1-F}(b)\cap (C \setminus F)|&\geq  |R^+_{T'-((X \cap B)\cup F)}(b)\cap (C \setminus F)|\\
    &\geq |C \setminus F|-|((X \cap B)\cup F)\cap \{v_i,\ldots,v_{n-6k}\}|\\
...
A similar argument shows that $ |R^-_{T_1-F}(a)\cap (C \setminus F)|\geq |C \setminus F|-(3k-1)$. As $|C|\geq n-12k\geq 7k-2$, we obtain
 \begin{align*}
    ...
    & \geq 1.
\end{align*}
Hence there is a vertex $v^* \in \{v_{6k+1},\ldots,v_{n-6k}\}\cap (R^+_{T_1}(b)\cap (C \setminus F)) \cap (R^-_{T_1}(a)\cap (C \setminus F))$. By definition, $T_1-F$ contains a directed path from $b$ to $v^*$ and a directed path from $v^*$ to $a$. Hence $T_1-F$ contains a directed path from $b$ to $a$.
\end{proofclaim}
...
By (iii), there is a directed path from $x'$ to a vertex $u$ in $B\setminus F$ in $T_1-F$.
...
Finally, we give the proof of Proposition \ref{nk1unter}, which states $N_k(1)\geq 5k-2$.
...
Lemma~\ref{lem:n-2F} asserts that from any vertex of $(A \cup C)\setminus F$ one can reach a vertex of $A\setminus F$ in $T_1-F$. Thus there exists a vertex $x' \in A\setminus F$ reachable from $x$ in $T_1-F$. By (iii), there is a directed path from $x'$ to a vertex of $u$ in $B\setminus F$ in $T'-F$.
Hence there is a directed path $P_x$ from $x$ to $u$ in $T_1-F$.
Similarly, by directional duality, in $T_1-F$, there is a directed path $P_y$ from a vertex $w\in A\setminus F$ to $y$.
...
By Lemma~\ref{lem:n-2F}, $|R^+_{T\langle B \cup C\rangle - F}(u) \cap (C\setminus F)| \geq |C|-2|F| = |C\setminus F| -|F| >  |C\setminus F|/2$ because $|C|\geq 3k-2$. Similarly, we obtain $|R^-_{T\langle A \cup C\rangle - F}(w) \cap (C\setminus F)|  >|C\setminus F|/2$. Thus there is a vertex of $C\setminus F$ in $R^+_{T\langle B \cup C\langle - F}(u)\cap R^-_{T\langle A \cup C\rangle - F}(w)$, and so there exists a directed path $Q$ from $u$ to $w$ in $T_1 - F$. Then
$P_x Q P_y$ is a directed path from $x$ to $y$ in $T_1-F$.
\end{proof}
...
$m_k(n) \leq \left\{ \begin{array}{ll}
      2k &\text{if~ $ 2k+1 \leq n < 11k - 2$,}\\
      3  &\text{if~ $ 11k - 2 \leq n < 19k - 2$,}\\
...
% \CR{Je crois que c'est bon: $k \geq 8t/\epsilon$ donc $t/2 \leq \epsilon k/16$. Le $t \geq 16$ est utilisé pour obtenir l'avant derniere ligne.}\FHO{Pardon, tu as raison. Il faudrait peut-etre mentionner ca explicitement en haut. Par contre, le probleme en fin de la demonstration persiste.}
    ...
    \begin{split}
        ...
        &\leq 2^{-t \epsilon k/64} + 2^{-t\epsilon k/16}\leq 2\cdot 2^{-t \epsilon k/64}%=2^{-t \epsilon k/64+1}.\\\leq 2^{-t\epsilon k/128}\\
    \end{split}
    \]
...
\begin{claim}\label{claim:decompose_into_events_A_B_C_}
    ...
    \item[$E_3$]: there are disjoint sets $A,B \subseteq V(T')$ with $|A|,|B| \geq (1+\epsilon/4)\frac{k}{2}$ 
...
$\epsilon  \leq \frac{1}{3}$ and $t\geq 2$, we have $k \leq \frac{1}{2}n\leq \frac{2}{3}\cdot\frac{5}{6}n\leq(1-\frac{1}{2^t-1})(1-\epsilon/2)n$. By Chernoff's Bound (Proposition~\ref{chernoff}), and because $t \geq 2$, we have
\begin{align*}
...
\begin{proofclaim}
    ...
    We have $(1+\epsilon/4)k/2 \leq \frac{1+\epsilon/4}{2+\epsilon}(n-2)/2 = \frac{1}{2}(1-\frac{\epsilon/4}{1+\epsilon/2}) (n-2)/2 \leq \frac{1}{4}(1-\epsilon/8)(n-2)$ since $n \geq (2+\epsilon)k+2$.
    ...
    Hence $(1+\epsilon/4)k/2 \leq (1-\epsilon/16)\left(\frac{1}{4}-\frac{3}{4(2^t-1)}\right)(n-2)$, and by Chernoff's Bound (Proposition~\ref{chernoff})
     \begin{align*}
   ...
   \end{align*}
    ...
    Hence, by the Union Bound (Proposition~\ref{union}), $\Pr[\min\{|A|,|B|,|C|\} \leq (1+\epsilon/4)\frac{k}{2}] \leq \sum_{X \in \{A,B,C\}}\Pr[|X| \leq (1+\epsilon/4)\frac{k}{2}]\leq 3\exp\left(-\epsilon^2\frac{n-2}{4096}\right)$.
\end{proofclaim}
...
\begin{proofclaim}
    ...
    \begin{array}{r c l}
       ...
       & \leq & 2^{2\lceil (1+\epsilon/4)k/2 \rceil} 2^{-t\epsilon k/128} \hspace{1cm} \text{ by Lemma~\ref{lemma:proba_3} and the Union Bound} \\
       & \leq & 2^{(1+\epsilon/4)k+2} 2^{-t\epsilon k/128} \hspace{1.325cm}  \\
       & \leq & 2^{-(2C+1)k} \\
    \end{array}
    \]
    
    as $t \geq \frac{128}{\epsilon}(2C+2+\epsilon/4) + 16$ and $\epsilon k/64 \geq \frac{1}{8}$.
...
\begin{problem}
  ...
  Find a good lower and upper bounds on $\sinv_k(n)$.
\end{problem}
...
\begin{problem}
    Is there an integer $t$ such that for every $\epsilon>0$, there is an integer $k_0$ with the property that for every $k \geq k_0$, if $T$ is a tournament on at least $(2+\epsilon)k$ vertices, then we have $\sinv_k(T)\leq t$?
\end{problem}
