...
\begin{abstract}
Kerr-cat qubits are a promising candidate for fault-tolerant quantum computers owing to the biased nature of errors.
...
We gradually modulate $\Delta_1$ in order to perform an $R_{zz}$ gate.
...
When the nonlinear terms in Eq.~\eqref{eq:H_eff_c_same_sign_1} can be neglected, the tensor product of coherent states $\ket{(-1)^{i+1}\alpha_{1}(t),(-1)^{i+1}\alpha_{2}(t)}_{\rm c}$ is the eigenstate of Hamiltonian $H_{\mathrm{c}}^{\Bar{i},\Bar{i}}(t)$ with eigenenergy
\begin{align}
    E_\mathrm{c}^{\bar{i},\bar{i}}(t):=
    -2g\alpha[\alpha_{1}(t)+\alpha_{2}(t)]
    +K\alpha^4.
...
If we set $|\Delta_{k}(0)|\gg g\alpha$ for $k=1,2$ so that the approximation $\ket{(-1)^{i+1}\alpha_{1}(0),(-1)^{i+1}\alpha_{2}(0)}_{\rm c}\simeq\ket{0,0}_{\mathrm{c}}$ is valid, $\ket{0,0}_{\mathrm{c}}$ is not only the eigenstate of $H_{\mathrm{c}}^{\Bar{i},\Bar{j}(\ne \bar{i})}(0)$ but also the approximate eigenstate of $H_{\mathrm{c}}^{\bar{i},\bar{i}}(0)$.
Hence, if we change $\Delta_{k}(t)$ slowly enough so that the couplers evolve adiabatically, the system can evolve as
\begin{align}
...
-\delta_{i,j}\int_0^{t}2g\alpha[\alpha_{1}(t')+\alpha_{2}(t')]\,\mathrm{d}t'\nonumber\\
...
&=\delta_{i,j}\ket{(-1)^{i+1}\alpha_{1}(t),(-1)^{i+1}\alpha_{2}(t)}_{\mathrm{c}} 
\notag \\
&\quad\mbox{}+(1-\delta_{i,j})\ket{0,0}_{\mathrm{c}},
\end{align}
and $\mathcal{N}(t)$ is for normalization.
After the gate time $t_f$, the detunings of the couplers 
return to the initial values so that $\ket{\psi(t_f)}_{\rm c}^{\bar{i},\bar{j}}\simeq \ket{0,0}_{\rm c}$.
Defining $\Theta$ as
\begin{align}
...
\end{align}
is an $R_{zz}$ gate with rotation angle $\Theta$ on the Kerr-cat qubits with $I_c$ being the identity operator on the couplers.
From \eqref{eq:final_state}, we find that the $R_{zz}(\Theta)$ gate is approximately realized. 
...
In order to verify that the states of the Kerr-cat qubits remain $\ket{\bar{i},\bar{j}}_{\rm q}$ during the control of the detuning, we consider the effective drive Hamiltonians of the KPOs conditioned by the state of the couplers, which is either $\ket{(-1)^{i+1}\alpha_{1}(t),(-1)^{i+1}\alpha_{2}(t)}_{\rm c}$ or $\ket{0,0}_{\rm c}$.
We define the effective drive Hamiltonians corresponding to $\ket{(-1)^{i+1}\alpha_{1}(t),(-1)^{i+1}\alpha_{2}(t)}_{\rm c}$ and $\ket{0,0}_{\rm c}$ as
\begin{widetext}
\begin{align}
    \hat{H}_{I,\mathrm{KPO}}^{\bar{i},\bar{i}}(t):&={\vphantom{\Ket{(-1)^{i+1}}}}_{\mathrm{c}}\Braket{(-1)^{i+1}\alpha_{1}(t),(-1)^{i+1}\alpha_{2}(t)|\hat{H}_I|(-1)^{i+1}\alpha_{1}(t),(-1)^{i+1}\alpha_{2}(t)}_{\mathrm{c}}
    \notag \\
    &=(-1)^{i+1}g[\alpha_{1}(t)+\alpha_{2}(t)]\sum_{j=1}^2\left(\hat{a}_j^{\dag}+\hat{a}_j
    \right)
\end{align}
\end{widetext}
and
...
That is, the four basis states are equally weighted.
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{2transmon_coupler_2KPO_alpha_max_optimization_noFF_reduced_dim_off_alpha004_32_option_6.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{2transmon_coupler_2KPO_alpha_max_optimization_noFF_reduced_dim_off_alpha004_42_deco.pdf}
...
In contrast, when the decoherence cannot be neglected, we see in Fig.~\ref{fig:off_infidelity_deco_1} the tendency of the infidelity to increase, so we must perform the quantum error correction, which is left as our future work.
...
\begin{align}
    ...
    \alpha_{2}(t)=-\alpha^{\rm min}_{\rm c}.
\end{align}
Note that we have
\begin{gather}
...
Typical time dependence of the detunings is presented in Fig.~\ref{fig:Delta_c12t}.
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{2transmon_coupler_2KPO_alpha_max_optimization_noFF_reduced_dim_alpha_min_004_data2_3_infidelity_with_deco3.pdf}
