...
Of note, the residual coupling can be eliminated when the detunings of the two couplers are set to opposite values.
...
In Sec.~\ref{sec:coupling}, we explain how to switch {\scshape on} and {\scshape off} the $ZZ$ coupling between the qubits.
...
The energy difference between the two sets of levels can be regarded as the effect of the $ZZ$-coupling Hamiltonian of the form  $E\hat{\sigma}_{z}^{\mathrm{q}1}\hat{\sigma}_{z}^{\mathrm{q}2}\TAred{/2}$, where
\TAred{$\hat{\sigma}_{z}^{\mathrm{q}j}=\ket{\bar{0}}_{\mathrm{q}j}\bra{\bar{0}}-\ket{\bar{1}}_{\mathrm{q}j}\bra{\bar{1}}$} and $E$ is the energy difference.
...
\begin{align}
    \TAred{\hat{R}}_{zz}(\Theta):=\mathcal{N}(t_f)\sum_{i,j=0}^1e^{-i\Theta\delta_{i,j}}\ket{\bar{i},\bar{j}}_{\mathrm{q}}\bra{\bar{i},\bar{j}}\otimes I_{\mathrm{c}}
\end{align}
is the $R_{zz}$ gate with rotation angle $\Theta$ on the Kerr-cat qubits with $I_{\mathrm{c}}$ being the identity operator on the couplers.
From Eq.~\eqref{eq:final_state}, we find that the $R_{zz}(\Theta)$ gate is approximately realized. 
Of note, we can eliminate \TAred{the} unwanted residual coupling by imposing $\Delta_{1}(t)=-\Delta_{2}$, which leads to $\Theta=0$ in Eq.~\eqref{Theta_2_15_23}.
...
\begin{figure}
    ...
    \caption{Infidelity $1-F(\hat{\rho}(t),\ket{\Psi(0)})$ when the effective $ZZ$ coupling should be switched {\scshape off} in the cases (a) without and (b) with decoherence.
    The red solid line labeled ``Numerical'' is calculated numerically with $\hat{\rho}(t)$ under master equation \eqref{eq:gksl1}.
...
\end{figure}
We evaluate the performance of our scheme of \TAred{the} effective coupling when it is switched {\scshape off} with the infidelity defined as \cite[Sec.~9.2.2]{wilde_2017}
\begin{align}
...
\end{figure}
The $R_{zz}$ gate for the Kerr-cat qubits can be realized by switching {\scshape on} their effective coupling.
...
The authors thank Yuichiro Matsuzaki, Toyofumi Ishikawa, and Hiroomi Chono for useful discussions.
...
\begin{figure}
    ...
    \caption{A circuit diagram to describe Hamiltonian \eqref{eq:H_R_t_main1}.}
    \label{fig:system2}
\end{figure}
...
-Let us derive Hamiltonian \eqref{eq:H_1}, which describes Fig.~\ref{fig:system2}, by canonical quantization \cite{PRXQuantum.2.040204}.
...
\begin{gather}
    ...
    x=\frac{\tilde{C}}{C}.
\end{gather}
Then, Hamiltonian \eqref{eq:classical_H} is rewritten as
\begin{align}
...
\begin{align}
    ...
    \label{eq:H_lambda1} \\
    \hat{H}_I&=8vE_C\sum_{j=1}^2\hat{n}_{\mathrm{KPO}j}\hat{n}_{\mathrm{c}j}
    \notag \\
    &\quad\mbox{}-E_L\hat{\varphi}_{\mathrm{KPO}1}\hat{\varphi}_{\mathrm{c}2}-E_L\hat{\varphi}_{\mathrm{KPO}2}\hat{\varphi}_{\mathrm{c}1}.
    \label{eq:H_I1}
\end{align}

Then, let us derive Hamiltonian \eqref{eq:H_R_t_main1} by conventional methods \cite{doi:10.7566/JPSJ.88.061015_2019_3_1,Masuda2021,PhysRevB.84.174521}. 
In Eq.~\eqref{eq:H_lambda1}, we assume $uE_C\ll E_J^{\lambda}\cos{[\phi_{\lambda}(t)/2]}$ $\forall t$.
Then, expanding $\cos{\hat{\varphi}_{\lambda}}$ to the fourth order in $\hat{\varphi}_{\lambda}$ is a good approximation:
\begin{align}
...
\begin{figure}
    ...
    \caption{A circuit diagram to describe Hamiltonian \eqref{eq:H_R_t_main2}.
...
\begin{figure}
    ...
    \caption{Infidelity $1-F(\hat{\rho}(t),\ket{\Psi(0)})$ when the effective $ZZ$ coupling should be switched {\scshape off} in the case without decoherence:
...
\begin{table}
    ...
    The bold values are design values, from which the other values (only the first three digits are shown) are calculated.
...
The counterparts to Eqs.~\eqref{eq:H_eff_c_same_sign_1}--\eqref{eq:eigenenrgy1}, and \eqref{eq:eigenenrgy2} are
\begin{align}
...
\label{app:mapping}
If the interaction Hamiltonian $\hat{H}_I^{\mathrm{R}}$ can be neglected, master equation \eqref{eq:gksl1} and the initial state \eqref{eq:initial2} give
\begin{align}
...
\end{align}
Since the initial state of coupler $k$, $\ket{0}_{\mathrm{c}k}$, is the steady state of master equation \eqref{eq:me_coupler1}, we have
\begin{align}
    \hat{\rho}_{\mathrm{c}k}(t)=\ket{0}_{\mathrm{c}k}\bra{0}.
\end{align}

We map master equation \eqref{eq:me_KPO1} of KPO $j$
...
Using Eqs.~\eqref{eq:me_KPO1}, \eqref{eq:aj_matrix1}--\eqref{eq:rho_matrix1}, the master equation of qubit $j$ is written as
\begin{align}
