...
\[\tr(f(sD))\stackrel{s\rightarrow0}{\sim}\insum{k}c(f,k)a_k(D^2)s^{k-\frac{d}{2}},\]
...
\[L(x,s)\stackrel{s\rightarrow0}{\sim}\insum{k}c(f,k)\tr(V_x^k(x))s^{k-\delta}\]
...
\begin{proof}
	...
	Thus $r_U\circ G^\pm|_{\Gamma_{K}(E)}$ extends continuously to a map $\Gamma^m_A(E)\rightarrow \Gamma^n(E|_U)$. As $C^k$- convergence implies distributional convergence, this continuous extension must coincide with $r_U\circ G^\pm|_{\Gamma^m_{K}(E)}$, as the latter is distributionally continuous.
\end{proof}
...
\begin{proof}
	...
	The former holds, as $V_0(z)=V_0$. For the latter, we calculate using the transport equation for $P$:
	\begin{align*}
		&(\rho_x^U-2k)V_k(z)\\
...
\begin{lemma}
	...
	\[G^{\pm m}_{\square-z,0}\sim\insum{m}\binom{k+m-1}{m}z^mR_\pm(2m+2k).\]
\end{lemma}
\begin{proof}
	As $R_\pm(2m+2k)=R^{\R^d}_\pm(2m+2k,0)$, Theorem \ref{PowExp} applied to $\square-z$ gives us the asymptotic expansion
	\[G^{\pm m}_{\square-z,0}\sim\insum{m}\binom{k+m-1}{m}V_m(z)R_\pm(2m+2k),\]
	...
	\[\square z^d=0,\]
	so we obtain
	\[G^{\pm m}_{\square-z,0}\sim\insum{m}\binom{k+m-1}{m}z^mR_\pm(2m+2k).\qedhere\]
\end{proof}
This carries over from Minkowski space to $M$ in a straightforward way:
\begin{lemma}
	\label{Rzex}
	We have the asymptotic expansion
	\[R_\pm(z,2m,x)\sim_x \insum{m}\binom{k+m-1}{m}z^mR^{U}_\pm(2m+2k,x).\]
\end{lemma}
\begin{proof}
	...
	\[\phi_x^{-1*}(R^{\R^ d}_\pm(2m+2k))=R^{T_x M}_\pm(2m+2k)\]
		and
	\[\phi_x^{-1*}(G^{\pm m}_{\square_{\R^d}-z,0})=G^{\pm m}_{\square_{T_xM}-z,0}\]
	...
	\[G^{\pm m}_{\square_{\R^d}-z,0}=\sum\limits_{m=0}^{N}\binom{k+m-1}{m}z^mR^{\R^ d}_\pm(2m+2k)+F\]
	with $F\in C^n(\R^d)$,
	we have
	\begin{align*}
	&R_\pm(z,2m,x)\\
	&=(\exp_x)^{-1*}(G^{\pm m}_{\square_{T_xM}-z,0})\\
	&=(\exp_x\circ\phi_x)^{-1*}(G^{\pm m}_{\square_{\R^d}-z,0})\\
...
\begin{thm}
	...
	More generally,
	\[G^{\pm m}_{P-z,x}|_U\sim_x \insum{k}\binom{k+m-1}{m}V^{k,U}_xR^U_\pm(z,2k+2m,x).\]
\end{thm}
\begin{proof}
