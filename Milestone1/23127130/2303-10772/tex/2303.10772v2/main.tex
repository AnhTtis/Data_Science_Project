...
%
{There are also some traditional methods, such as~\cite{ball2012unsupervised,cola2015unsupervised,rida2015unsupervised} employed unsupervised learning to facilitate the development of gait recognition. 
%
...
%
{To address the two challenges, we propose a new method called \textbf{Selective Fusion}, to gradually pull cross-view and cross-cloth sequences together.}
...
%
When the situations become complex, other cloth augmentation methods can be employed, such as sheer the bottom part of the silhouettes to simulate wearing a dress, adding an oval above the head to simulate wearing a hat, {which is another promising research direction in the future research.}
%
...
%
{Therefore, the clothing augmentation via dilation or erosion used in our method just provides the opportunity to let the cross-cloth sequences have the chance to be closer to each other. 
%
...
We train and test the performance of our method on three popular datasets, CASIA-BN~\cite{yu2006framework}, Outdoor-Gait~\cite{song2019gaitnet} and GREW~\cite{lin2014effects}. Note that during training, we do not use the true label in the training set of CASIA-BN, Outdoor-Gait, GREW.
...
{Here we show the structure and optimization setting we used in pre-training and unsupervised learning in Table~\ref{unsupervise_param}. 
...
\begin{table}[ht]
\centering
\caption{{The Rank-1 and Rank-5 accuracy (\%) on GREW. Trained on the GREW train set and evaluated on test set.}}
\begin{tabular}{ccccc}
\hline
{Backbone} & {Method} &{Rank-1} &{Rank-5} \\ \hline
\multirow{4}{*}{{GaitSet}} & {Upper} & \textcolor{BrickRed}{48.40} & \textcolor{BrickRed}{63.58}  \\
...
\label{tab:SC-Fusion}
\end{table}
{\textbf{We also include additional experiments in the supplementary materials},  evaluating the influence of hyperparameters, the curriculum rate, and presenting visualizations of t-SNE to illustrate the effectiveness of our method.}
