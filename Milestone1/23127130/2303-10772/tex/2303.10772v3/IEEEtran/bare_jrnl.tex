...
%
However, gait recognition in a realistic situation will be affected by many factors such as occlusion, dirty labels, labeling, and more. 
%
\textcolor{blue}{Labeling, especially, is a major challenge, requiring intensive manual effort for pairwise data. Therefore, training on unlabeled datasets becomes crucial to save resources and address these challenges.}
...
%
\textcolor{blue}{There are also some traditional methods, such as~\cite{ball2012unsupervised,cola2015unsupervised,rida2015unsupervised} employed unsupervised learning to facilitate the development of gait recognition. 
%
...
%
\textcolor{blue}{To address the two challenges, we propose a new method called \textbf{Selective Fusion}, to gradually pull cross-view and cross-cloth sequences together.}


\textcolor{blue}{Our method comprises two techniques: Selective Cluster Fusion (SCF), which is used to narrow the distance of cross-cloth clusters, and Selective Sample Fusion (SSF), which is used to pull cross-view pairs nearly, especially helpful for outliers near $0^{\circ}/180^{\circ}$.}
%
\textcolor{blue}{To be specific, first, in SCF, we use a support set selection module to generate a support set for each cluster.
%
In the support set, there are selected candidate clusters of each corresponding cluster that are potentially of the same person but in different clothes.
%
...
% \textcolor{blue}{It is worth noting that our method focuses on clustering the features and learning the interior correlations of clusters based on pseudo labels. We do not specifically focus on learning the domain-invariant features or addressing domain shift in particular.
...
\begin{itemize}
    \item[$\bullet$] \textcolor{blue}{We focus on Unsupervised Gait Recognition (UGR) using a cluster-based method with contrastive learning. Despite its practicality, it requires careful consideration. To address this task, we establish a baseline using cluster-level contrastive learning.}
    
    \item[$\bullet$]  \textcolor{blue}{We deeply explore the characteristics of UGR, finding the two main challenges: clustering sequences with different clothes and with front/back views. To address these challenges, we propose a \textbf{Selective Fusion} method. This method involves selecting potentially matched cluster/sample pairs to help them fuse gradually.}
...
%
Despite lots of gait recognition research, there is still a need for further exploration of its practical applications.
%
...
%
JointsGait~\cite{li2020jointsgait} extracts spatio-temporal features from 2D joints by GCN~\cite{yan2018spatial}, then maps them into discriminative space according to the human body structure and walking pattern.
%
GaitGraph~\cite{gaitgraph} uses human pose estimation to extract robust pose from RGB images, then encode the keypoints as nodes, and encode skeletons as joints in the Graph Convolutional Network to extract gait information.
...
%----------------------------------------
Clustering-based methods~\cite{zeng2020hierarchical,wang2021camera,chen2021ice,xuan2021intra,zhang2023camera} first estimate a pseudo label for each sequence and train the network with sequence similarity.
%
...
%
We do not use the same method in SCF, since each cluster mainly contains sequences with one cloth type, and it's better to pull clusters as a whole.
%
...
\textcolor{blue}{3) With the pseudo labels, we compute the centroid of each cluster $\mathcal{C}_k, k \in \{0,1,...,Q\}$, $Q$ is the number of clusters, and then initialize a memory bank $\mathcal{M}$ at the cluster-level to store these centers $\mathcal{M} = \{\mathcal{C}_0, \mathcal{C}_1, ..., \mathcal{C}_Q\}$.}
%

\textcolor{blue}{4) During each iteration, a mini-batch $b$ will be randomly selected from pseudo clusters, and during gradient propagation, we update the backbone with a ClusterNCE loss~\cite{dai2021cluster}.}
\begin{equation}
\label{ClusterNCE}
    \textcolor{blue}{\mathcal{L}_{q_{CNCE}}=-\log \frac{\exp \left( q\cdot \mathcal{C}_+/\tau \right)}{\sum_{k=1}^Q{\exp}\left( q\cdot \mathcal{C}_k/\tau \right)}}
\end{equation}
...
\textcolor{blue}{In our method, at the beginning of each epoch, we use a Cloth Augmentation (CA) method to randomly generate an augmented variant for each sequence in the training dataset, then put them into the same backbone to extract features, named $f_{i}$ and $f_{i_{CA}}$.
%
...
%
When the situations become complex, other cloth augmentation methods can be employed, such as sheer the bottom part of the silhouettes to simulate wearing a dress, adding an oval above the head to simulate wearing a hat, \textcolor{blue}{which is another promising research direction in the future research.}
%
However, since such real conditions don't appear in these common datasets, adding more cloth augmentation methods will hinder us from verifying the effectiveness of our method.
%
\textcolor{blue}{Therefore, the clothing augmentation via dilation or erosion used in our method just provides the opportunity to let the cross-cloth sequences have the chance to be closer to each other. 
%
This will facilitate our method to utilize the close chance to further pull the clusters near through the support set and multi-cluster update strategy.}
...
\textbf{Support Set:} \textcolor{blue}{The input of the support set selection module is the centroids of $\mathcal{C}_{ak}$ and $\mathcal{C}_{k}$.
%
...
\begin{equation}
\label{momentum}
   \textcolor{blue}{\forall \mathcal{C}_{ak} \in \mathcal{S}_k, \forall q \in \mathcal{C}_{ak}, \mathcal{C}_{ak} \leftarrow m \mathcal{C}_{ak} + (1-m) q}
\end{equation}
...
% %
% \textcolor{blue}{We load the pre-trained model and evaluate the performance of our method on three popular datasets, CASIA-BN~\cite{yu2006framework}, Outdoor-Gait~\cite{song2019gaitnet} and GREW~\cite{lin2014effects}.}
\textcolor{blue}{We train and test the performance of our method on three popular datasets, CASIA-BN~\cite{yu2006framework}, Outdoor-Gait~\cite{song2019gaitnet} and GREW~\cite{lin2014effects}. Note that during training, we do not use the true label in the training set of CASIA-BN, Outdoor-Gait, GREW.}
...
\subsubsection{\textcolor{blue}{GREW}}
...
%
Furthermore, this dataset includes a train set with 20,000 identities and 102,887 sequences, a validation set with 345 identities and 1,784 sequences, a test set with 6,000 identities and 24,000 sequences. In the test phase, we strictly follow the official test protocols.
%2 sequences per subject are treated as probes and another 2 sequences as the gallery.
%This evaluation strictly following the official test protocols.
}
...
\textcolor{blue}{In this section, we provide comprehensive details about the implementation of our method.}  
...
% \textcolor{blue}{We include details such as the structure, optimization settings, and hyperparameter configurations in the supplementary materials.}
\setlength{\tabcolsep}{1pt}
\begin{table*}[ht]
\centering
\caption{\textcolor{blue}{The parameters used in unsupervised learning on OU-MVLP, CASIA-BN, Outdoor-Gait dataset, and GREW.}}
%\resizebox{\columnwidth}{!}{%
\begin{tabular}{ccccccc}
\hline
Param                           & Backbone  &OU-MVLP & CASIA-BN                 & Outdoor-Gait   &\textcolor{blue}{GREW}   \\  
\hline
\multirow{2}{*}{Model Channel}  & GaitSet   & -        & (32, 64, 128)(128, 256)  & (32, 64, 128)(128, 256) &\textcolor{blue}{(32, 64, 128,256)(256, 256)} \\
                                & GaitGL    & -        & (32, 64, 128)(128, 128)  & -                       &\textcolor{blue}{-} \\
Batch Size                      & Both      & (32, 16) & (8, 16)                  & (8, 8)                  & \textcolor{blue}{(8,16)}                 \\
Weight Decay                    & Both      & 5e-4     & 5e-4                     & 5e-4                    &\textcolor{blue}{5e-4}                   \\
Start LR                        & Both      & 1e-1     & 1e-4                     & 1e-4                    &\textcolor{blue}{1e-4}                  \\
Milestones                      & Both      & -        & {[}3.5k, 8.5k{]}         & {[}3.5k, 8.5k{]}        & \textcolor{blue}{{[}3.5k, 8.5k{]}}       \\
Epoch                           & Both      & -        & Baseline: 50, SF: 50     & Baseline: 50, SF: 50    & \textcolor{blue}{Baseline: 50, SF: 50}  \\
Iteration                       & Both      & -        & Baseline: 50, SF: 100    & Baseline: 50, SF: 100   &\textcolor{blue}{Baseline: 1000, SF: 1000} \\
\multirow{2}{*}{\begin{tabular}[c]{@{}l@{}}Upper bound Milestones\end{tabular}} 
                                 & GaitSet  &{[}50k, 100k, 125k{]}      & {[}20k, 40k, 60k, 80k{]} & {[}10k, 20k, 30k, 35k{]} & \textcolor{blue}{{[}10k, 20k, 30k, 35k{]}}\\
                                & GaitGL    & {[}150k, 200k, 210k{]}    & {[}70k, 80k{]}           & -  &\textcolor{blue}{-}\\ \hline
\end{tabular}%
%}
...
% \subsubsection{Hyper-Parameters Setting} 
 \textcolor{blue}{Here we show the structure and optimization setting we used in pre-training and unsupervised learning in Table~\ref{unsupervise_param}. 
 % We leverage the GaitSet baseline from OpenGait~\cite{fan2022opengait} for the GaitSet backbone and similarly utilize the GaitGL implementation from OpenGait for the GaitGL backbone. 
% The input and output channel dimensions for each module in the GaitSet and GaitGL backbones are outlined in the first row of Table~\ref{unsupervise_param}.
% Consistently, the backbone employed remains unchanged for both CASIA-BN and Outdoor-Gait datasets. 
Recognizing individuals on GREW proves more challenging due to its larger scale and diverse, real-world settings. Consequently, we employ convolutional layers with increased channel sizes (32, 64, 128, 256).
}During training, we select 30 frames and use all the frames for evaluation. 
%
For the hyper-parameters, we set $s_{up}=0.7$ for baseline, and $s_{up}=0.3$ for Selective Fusion to enlarge the boundary, others are set as $n=40$, $\tau=0.05$, $k=2$, $\lambda_{base}=0.005$, $c_{low} = 0.8$, $s_{o} = 0.7$. Since the sequences in Outdoor-Gait is fewer, we change $n$ to 20 to avoid overfitting. Each frame has been normalized to $64\times44$. We also provide additional hyperparameter configurations in the supplementary materials.
%
...
%
With the SCF method, not only the accuracy of CL condition improved, but also the accuracy of NM, and BG, which means features from CL sequences can also provide useful information when recognizing a person, and they can not be neglected.
%
...
%
\subsubsection{\textcolor{blue}{GREW}}
\textcolor{blue}{To test the generalization of our method, we apply it to a large wild dataset GREW ~\cite{lin2014effects}. Our method excels in further separating the narrowed features within the feature space as comprehensively as possible. The results, as shown in Table~\ref{tab:grew}, illustrate that our proposed method outperforms the baseline under both conditions (Rank-1 + 1.40\%; Rank-5 + 1.68\%). This affirms that our Selective Fusion method can effectively guide the updating of clusters and finally get the clusters with different conditions. Additionally, it validates the generalization of our method in outdoor scenarios.} 
...
% \begin{table}[h]
% \centering
% \caption{\textcolor{blue}{Our method v.s. other unsupervised State-of-the-Art.}}
% \begin{tabular}{ccccc}
% \hline
% \textcolor{blue}{Backbone} & \textcolor{blue}{Method} &\textcolor{blue}{Rank-1} &\textcolor{blue}{Rank-5} \\ \hline
% \textcolor{blue}{GaitBase} & \textcolor{blue}{GaitSSB} & \textcolor{blue}{16.60} &\textcolor{blue}{-}\\ \hline
% \multirow{2}{*}{\textcolor{blue}{GaitSet}} 
%  % & \textcolor{blue}{Pretrain} &\textcolor{blue}{ 16.96} & \textcolor{blue}{ 28.46}  \\
%  & \textcolor{blue}{Base (CC)} &\textcolor{blue}{ 18.26} & \textcolor{blue}{30.42}  \\
%  & \textcolor{blue}{Ours (SCF)} & \textbf{\textcolor{blue}{19.66}} & \textbf{\textcolor{blue}{32.10}} \\ \hline
% \end{tabular}
% \label{tab:grew1}
% \end{table}
...
\begin{table}[ht]
\centering
\caption{\textcolor{blue}{The Rank-1 and Rank-5 accuracy (\%) on GREW. Trained on the GREW train set and evaluated on test set.}}
\begin{tabular}{ccccc}
\hline
\textcolor{blue}{Backbone} & \textcolor{blue}{Method} &\textcolor{blue}{Rank-1} &\textcolor{blue}{Rank-5} \\ \hline
\multirow{4}{*}{\textcolor{blue}{GaitSet}} & \textcolor{blue}{Upper} & \textcolor{BrickRed}{48.40} & \textcolor{BrickRed}{63.58}  \\
 & \textcolor{blue}{Pretrain} &\textcolor{blue}{ 16.96} & \textcolor{blue}{ 28.46}  \\
 & \textcolor{blue}{Base (CC)} &\textcolor{blue}{ 18.26} & \textcolor{blue}{30.42}  \\
 & \textcolor{blue}{Ours (SCF)} & \textbf{\textcolor{blue}{19.66}} & \textbf{\textcolor{blue}{32.10}}\\ \hline
\end{tabular}
...
In this section, we demonstrate that both SCF and SSF are indispensable in our Selective Fusion method.
%
...
\label{tab:SC-Fusion}
\end{table}
\textcolor{blue}{\textbf{We also include additional experiments in the supplementary materials},  evaluating the influence of hyperparameters, the curriculum rate, and presenting visualizations of t-SNE to illustrate the effectiveness of our method.}
...
%
Then, we find two problems Unsupervised Gait Recognition faced: sequences in different clothes cannot be gathered into a single cluster, and sequences captured from front/back views are hard to collect with sequences in other views.
%
...
\begin{IEEEbiographynophoto}{Shaopeng Yang}
