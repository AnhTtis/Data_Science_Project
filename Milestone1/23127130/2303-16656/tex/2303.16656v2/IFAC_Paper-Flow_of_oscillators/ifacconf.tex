...
A total of ${N = 30}$ trajectories are generated,
...
The learning rate is reduced 5-fold for every 5~epochs in which the 
validation loss is not reduced,
and the training is stopped when the validation loss does not
decrease more than $5\times 10^{-4}$ for 30~consecutive~epochs.
...
The optimal initial learning rate was found to be $1\times 10^{-2}$
and the training took 186~seconds on a cluster node with an NVIDIA T4 GPU and
an Intel(R) Xeon(R) Gold 6226R CPU @ 2.90GHz.
The RNN is a single-layer LSTM network with 8~hidden states.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.9\linewidth]{fig/ifac_mse_time_horizon.pdf} 
    \vspace{-0.5em}
    \caption{
...
on a different set of $100$ trajectories on $[0, t]$
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{fig/mse_new_inputs_62b22dac34824da3a16253ba649211ef.pdf}
    \vspace{-0.5em}
...
\begin{equation*}
    u_k = A\, {\sin}{\left( {\Omega}k\Delta \right)},
\end{equation*}
where ${A\sim\mathrm{LogNormal}(0, 1)}$ and ${\Omega\sim\mathrm{Uniform}(0, 2\pi)}$.
This corresponds to sinusoidal signals with a maximum frequency of $1$~Hz.
...
computed on 100~trajectories for each of the two input distributions $P_u$ and $Q_u$.
