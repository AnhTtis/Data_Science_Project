...
% Taking $\lambda_A = P_A$ gives
% $$\dot{\nu}_P^\ast(h)(y,a,x) = (y-\mu_P(a,x)) \frac{p_A(a)}{g_P(a \mid x)} h(a) + \int [\mu_P(a,x)-\nu(P)(a)]h(a) P_A(da).$$
...
\begin{align*}
\left\|\bar{\nu}_n^{\beta_n}-\nu(P_0) - P_n\phi_0^{\beta_n} - \frac{1}{2}\sum_{j=1}^2 \mathcal{B}_n^{j,\beta_n}\right\|_{\mathcal{H}}&\le \max_j \|\mathcal{R}_n^{j,\beta_n}\|_{\mathcal{H}} + \max_j \|\mathcal{D}_n^{j,\beta_n}\|_{\mathcal{H}}, % \label{eq:regDecomp}
\end{align*}
