...
Recent works~\cite{wu2021self, lin2022improving, xia2022hypergraph} propose to address the noise and the sparsity problems of CF by providing self-supervision signals using contrastive learning (CL) techniques. We show that our KD approach can provide even more additional supervisions. Specifically, we list the pull-close gradients from both InfoNCE-based CL loss, and our KD loss $\mathcal{L}_1$, \wrt, a single node embedding $\textbf{h}_i$, as follows:
\begin{align}
