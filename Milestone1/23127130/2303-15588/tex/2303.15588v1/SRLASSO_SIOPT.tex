...
authors~\cite{berk2022lasso}, we inspect the \srlasso{} through the lens of
...
sensitivity. For instance, \cref{fig:motivation-new} portrays the Lipschitz
...
\begin{figure}[t]
  ...
  \hfill
  \begin{subfigure}[t]{.49\linewidth}
    \includegraphics[width=\linewidth]{fig/sr_uc_lipschitz_comparison.pdf}
    \caption{Lipschitz behavior for each program; $L_{\text{SR}}$ as
      in~\cref{eq:lipschitz_ub_lamda}, $L_{\text{UC}}$ as
      in~\cref{eq:Lipschitz_lambda_LASSO}. $V$ as in~\cref{cor:Lipschitz_lambda}
      gives
      $L_{\text{UC}} \approx L_{\text{SR}} |1-V|$.\label{fig:motivation-new}}
  \end{subfigure}
  \caption{Comparison between \srlasso{} (SR) and (unconstrained) \lasso{} (UC):
    ...
    solution and $x^\sharp$. See~\cref{sec:sr-vs-uc} for further details and
    discussion.}
...
\cref{fig:empirical-lipschitz-bound} supports the notion that our theoretical
bounds on the Lipschitz constant for \srlasso{} are relatively tight (at least
in the regime considered in the experiment).
...
carried out by the authors of this paper~\cite{berk2022lasso}. Stability
...
Bolte \etal{}~\cite{BPS21, BPS22}. A sensitivity analysis of the proximal
...
$\proj_{C}(x) := \argmin_{z \in \reals^{n}} \| z - x \|$. Write the orthogonal
...
\begin{proof}
 ...
 \[
   ...
   \quad b-Ax\in\cS.
 \]
 Using \cref{cor:Invar}(b) and the fact that $0\in N_{\cS}(b-Ax)$, we see that
 ...
 \Cref{lem:SD1Norm} (b), and the last equivalence uses \Cref{cor:Equiv}.
...
\begin{proof}
  ...
  from~\cite[Lemma~4.9]{berk2022lasso}. The first two conditions, for $J^{C}$
...
\begin{lemma}
  ...
  \cref{ass:intermediate} holds at $\bar x_k$ (for $(A_k,b_k,\lambda_k)$).
\end{lemma}
\begin{proof}
  ...
  $A_J$ has full rank, so does $A_{J_k}$ for all $k$ sufficiently large.
\end{proof}

\noindent
We are now in a position to state the main result of this section. Recall that
we already know from \cref{prop:intermed-implies-weak} and
...
\begin{proof}
  We apply~\cite[Proposition~4.10]{berk2022lasso} for 
  $f : (\reals^{m} \times \reals) \times \reals^{n} \to \reals^{n}$  with 
  \begin{align*}
    f((b, \lambda), x) %
    = \frac{1}{\lambda} A^{\top} \partial \|\cdot\|_{2}(Ax - b), %
  ...
  by~\cite[Proposition~4.10(b)]{berk2022lasso} and showing the validity of the first claim in part (a). 
  ...
  by~\cite[Proposition~4.10(c)]{berk2022lasso}, the graphical derivative
...
\end{align}
Let $I = \supp(\bar x)$ and recall~\cite[Lemma~4.9]{berk2022lasso}, namely,
\begin{align}
...
$(\bar b,\bar \lambda)$. Hence, by~\cite[Proposition~4.10(c)]{berk2022lasso},
...
\begin{proof} Set $\bar{z} := (\bar{x}, \| \bar{x} \|_{1})$,
  ...
  of~\cite[Lemma~4.7]{berk2022lasso}, and all that needs to be observed is the
...
\begin{remark}[\cref{ass:Strong} is local property]
...
In this section we compare the Lipschitz behavior for the \srlasso{} solution map with that of the (unconstrained) \lasso{}. We draw theoretical comparisons (in \cref{sec:theoretical-comparison}) using the Lipschitz bound in~\cref{cor:Lipschitz_lambda} and an analogous bound derived in~\cite{berk2022lasso}; and numerical comparisons in~\cref{sec:numerical-lipschitz-comparison}.
...
An analogous version of this bound for the \lasso{} can be derived from results in \cite{berk2022lasso}. Under \cite[Assumption~4.4]{berk2022lasso} with associated $(\bar b, \bar \lambda) \in \mathbb{R}^m \times \mathbb{R}_{++}$ (which is the analogue of \Cref{ass:Strong} for the \lasso{} case), \ie{} for $\bar x_{\mathrm{UC}}$ solving \cref{eq:unconstrained-LASSO} with $(b,\lambda) = (\bar b, \bar \lambda)$ such that
$$
A_I \text{ has full column rank} \quad \text{and} \quad \left\|A^\top_{I^C}(\bar b - A \bar x_{\text{UC}})\right\|_{\infty} < \lambda,
$$
where $I_{\text{UC}} := \supp(\bar x_{\text{UC}})$. In this case, an inspection of the proof of \cite[Corollary~4.16]{berk2022lasso} reveals that the derivative of the \lasso{} solution map $S_{\mathrm{UC}}$ at $\bar \lambda$ satisfies 
$
...
\end{equation}
We are now in a position to compare the two Lipschitz bounds \cref{eq:Lipschitz_lambda_SR-LASSO} and \cref{eq:Lipschitz_lambda_LASSO}. Under the respective ``strong'' assumptions (\ie{} \Cref{ass:Strong} and \cite[Assumption~4.4]{berk2022lasso}) and supposing that $\bar x_{\mathrm{SR}} \approx \bar{x}_{\mathrm{UC}}$ (which, in turn, implies $I_{\mathrm{SR}} \approx I_{\mathrm{UC}}$ and $\bar r_{\mathrm{SR}} \approx \bar r_{\mathrm{UC}}$), the only difference between the two bounds is the multiplicative term
...
$x^\sharp \in \reals^{n}$, $n = 200$, and measurements $b \in \reals^{m}$ are
given by
\begin{align}
\label{eq:CS_model}
  x^\sharp_{j} :=
...
in~\cref{fig:motivation-new} we set $(m, n, s, \gamma) = (100, 200, 5,
...
\begin{align*}
  \bar \lambda_{\text{best}}^{P} &:= \argmin_{\lambda \in ( \lambda_{i}^{P})_{i \in [\Lambda]}} \|\bar{x}^{P}(\lambda) - x^{\sharp}\|_{2};  %
  & %
...
On the other hand, in~\cref{fig:motivation-new} we plot the empirical local
...
of~\cite[Theorem~4.13]{berk2022lasso}). Interestingly, there is a clear
...
\cref{fig:motivation-new}), which appears to serve as a good characterization
...
\begin{figure}[t]
  ...
  \caption{Visualizing uniqueness sufficiency for
    ...
    $\|\bar x(\lambda) - x^\sharp\|_{2}$ where $\bar x(\lambda)$
...
$\|\bar x(\lambda) - x^\sharp\|_{2}$. The horizontal position of the vertical
...
\begin{figure}[t]
  ...
  \caption{Background: uniqueness sufficiency heatmap displaying the proportion
    ...
    data. Foreground: error $\|\bar x(\lambda) - x^\sharp\|_{2}$ (left axis) as
...
matrix $A$ is a parameter. Moreover, similarly to~\cite{berk2022lasso}, our
...
solutions~\cite{foucart2023sparsity}. Finally, in the LASSO case it is
