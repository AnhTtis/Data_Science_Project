...
\begin{remark}
\label{rmk:pcaproj}
We note that under more fine-grained information on the underlying measure $\mu$, considerable improvements to the upper bound of Proposition \ref{prop:pcaproj} are possible; this has e.g. been achieved in \cite{milbradt2020high} for a different notion of a ``sub-Gaussian'' distribution (strictly stronger than \eqref{eq:momentbd}). In that setting, it can in fact be shown that the empirical PCA projection error is of order $\mathcal{O}\left(\cR^\opt_d(\mu)\right)$ whenever $N \gtrsim d \log(1/\delta)$, thereby achieving essentially optimal PCA convergence rates. In the present context, PCA will be applied for dimension reduction on both the input and output spaces under a non-linear mapping $\Psi^\dagger$. Unfortunately, it is unclear whether the required property on $\mu$ would be preserved under the push-forward of $\mu$ by $\Psi^\dagger$. In contrast, the bound \eqref{eq:momentbd} is robust under such a push-forward. Potential improvements of Proposition \ref{prop:pcaproj}, potentially building on \cite{milbradt2020high}, are left as a challenge for future work.
\end{remark}
...
Fix $k,d\in \N$. There exists $c_{k,d}>0$ and $f\in F_{k,d}$ depending only on $k$ and $d$, and an absolute constant $\lambda > 0$ independent of both $d$ and $k$, such that for any neural network $\psi$, we have the lower bound
\begin{align}
\label{eq:lower2}
