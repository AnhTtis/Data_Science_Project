...
Fix $k,d\in \N$. There exists  $f\in F_{k,d}$, a constant $c_{k,d}>0$ depending only on $k$ and $d$, and an absolute constant $\lambda > 0$, independent of both $d$ and $k$, such that for any neural network $\psi$, we have the lower bound
\begin{align}
\label{eq:lower2}
