...
Upper complexity bounds are the subject of the remaining sections: In section \ref{sec:quant-enc}, it a smoothness criterion is shown to rule out the first potential obstacle to efficient operator learning.
...
We then finally show, in section \ref{sec:overcoming}, how PCA-Net can overcome the curse of dimensionality for two prototypical PDE operators arising in the context of the Darcy flow and Navier-Stokes equations, respectively.
...
\paragraph{PCA}
Given a Hilbert space $\cH$, a probability measure $\mu$ on $\cX$, and a projection dimension $d$, PCA aims to minimize the average reconstruction error $\E_{u\sim \mu}[\Vert u - P u \Vert^2]$ over the set $\Pi_d$ of orthogonal projections $P: \cH \to \cH$ of rank $d$. It is well-known (e.g. \cite{jolliffe2002principal,pca,reisswahl2020}) that this can be achieved by considering the covariance operator $\Sigma = \E_{u\sim \mu}[u \otimes u]$, which is diagonalizable; there exists a sequence ${\lambda}_1\ge {\lambda}_2 \ge \dots \ge 0$ of eigenvalues and corresponding orthonormal eigenvectors ${\phi}_1,{\phi}_2,\dots \in \cH$, such that $\Sigma {\phi}_j = {\lambda}_j {\phi}_j$ for all $j$. The optimal PCA projection of dimension $d$, ${P}_{\le d}: \cH \to \cH$, can then be written as a composition ${P}_{\le d} = \cD_\cH^\opt \circ \cE_\cH^\opt$, where the optimal PCA encoder $\cE_\cH^\opt$ is given by,
\begin{align}
\label{eq:EH}
\cE_\cH^\opt: \cH \to \R^d, \quad \cE_\cH^\opt(u) := (\langle u, \phi_1\rangle, \dots, \langle u, \phi_d\rangle),
\end{align}
and the corresponding PCA decoder $\cD_\cH$ is the mapping,
\begin{align}
\label{eq:DH}
\cD_\cH^\opt: \R^d \to \cH, \quad \cD_\cH^\opt(\eta) = \sum_{j=1}^d \eta_j \phi_j.
\end{align}
It can be shown that ${P}_{\le d} = \cD_\cH^\opt\circ \cE_\cH^\opt$ defines an orthogonal projection on $\cH$, such that
\[
...
\end{align}
this optimal PCA projection error. One can show that $\cR^\opt_d(\mu)$ is related to the PCA eigenvalues, by
\begin{align}
\label{eq:Rd1}
...
\end{align}
According to this definition \eqref{eq:momentbd}, $\mu$ is sub-Gaussian if the real-valued random variable $\Vert u \Vert_{\cH}$, with $u \sim \mu$, is sub-Gaussian in the conventional sense; there exist many equivalent characterizations of real-valued sub-Gaussian random variables (see e.g. \cite[Sect. 2.5.1]{vershynin2018high}). We then have:
...
Proposition \ref{prop:pcaproj} above is a natural high-probability analogue of a previous result on empirical PCA of \cite{pca}, there derived in expectation. Proposition \ref{prop:pcaproj} uses the same bound on the PCA excess risk as Proposition \ref{prop:pcaproj-qual}, but combines it with a general Bernstein concentration bound for $\cH$-valued random variables to derive quantitative rates. We include the details in Appendix \ref{app:pcaproj}.

\begin{remark}
\label{rmk:pcaproj}
We note that under more fine-grained information on the underlying measure $\mu$, considerable improvements to the upper bound of Proposition \ref{prop:pcaproj} are possible; this has e.g. been achieved in \cite{milbradt2020high} for a different notion of a ``sub-Gaussian'' distribution (strictly stronger than \eqref{eq:momentbd}). In that setting, it can in fact be shown that the empirical PCA projection error is $\mathcal{O}\left(\cR^\opt_d(\mu)\right)$ whenever $N \gtrsim d \log(1/\delta)$, thereby achieving essentially optimal PCA convergence rates. In the present context, PCA will be applied for dimension reduction on both the input and output spaces under a non-linear mapping $\Psi^\dagger$. Unfortunately, it is unclear whether the required property on $\mu$ is preserved under the push-forward of $\mu$ by $\Psi^\dagger$. In contrast, the bound \eqref{eq:momentbd} is robust under such a push-forward. Potential improvements of Proposition \ref{prop:pcaproj} are left as a challenge for future work.
\end{remark}
...
Ideally, we would like to choose the neural network as a minimizer of the expected loss
\begin{align*}
...
In practice, the neural network $\psi$ in \eqref{eq:pcanet} is usually trained to minimizing the following empirical loss,
\begin{align*}
...
\begin{remark}
In the present work, we will not address the practical training of the neural network $\psi$, analysis of which is a notoriously difficult problem. Instead, we will contend ourselves with an ``approximation theoretic'' approach; in the following analysis, it will be shown that certain $\psi$ exists, but no a priori guarantee is given that this $\psi$ can be found by the numerical optimization of the empirical loss $\hat{\cL}$ in practice. Thus, our analysis will focus on the approximation error for $\psi$; Additional generalization and optimization error contributions will not be discussed. The samples $u_1,\dots, u_N$ will, however, enter our analysis of the empirical PCA encoder $\cE_\cX$ and decoder $\cD_\cY$.
\end{remark}
...
Fix $k,d\in \N$. There exists $c_{k,d}>0$ and $f\in F_{k,d}$ depending only $k$ and $d$, and an absolute constant $\lambda > 0$ independent of both $d$ and $k$, such that for any neural network $\psi$, we have the lower bound
\begin{align}
\label{eq:lower2}
...
\label{sec:darcy}
Let $D \subset \R^n$ be a bounded domain. We consider the operator $\Psi^\dagger: a \mapsto w$, mapping the coefficient field $a$ to the solution of the following elliptic problem:
...
\right.
\end{align}
on a domain $D \subset \R^n$, for fixed right-hand side $f$, and with Dirichlet boundary conditions. Let $H^1_0(D)$ be the Sobolev space consisting of weakly differentiable functions $w: D \to \R$ which vanish on the boundary $\partial D$, and whose gradient is square-integrable. It is well-known, e.g. \cite[Chapt. 6]{evans2022partial}, that if the coefficient field $a\in L^\infty(D)$, satisfies two-sided (coercivity and upper) bounds 
\begin{align}
\label{eq:twosided}
...
\[
a(x;y) \ge \bar{a}_{\mathrm{min}} - \sum_{\ell=1}^\infty \gamma_\ell \Vert \rho_\ell \Vert_{L^\infty} \ge \frac{1}{1+\kappa} \bar{a}_{\mathrm{min}} =: \lambda >0,
\]
for any $z \in U := [-1,1]^\N$. On the other hand, the assumed embedding $\cX \embeds L^\infty(D)$ implies that there exists a constant $C>0$, such that $\Vert \slot \Vert_{L^\infty} \le C \Vert \slot \Vert_{\cX}$, and hence \eqref{eq:alpha} ensures a uniform upper bound,
\begin{align*}
a(x;z) 
&\le 
...
\begin{remark}
We do not assume any \emph{(explicit) knowledge} of the functions $\bar{a}$, $\rho_\ell$, the parameters $\gamma_\ell$, $\alpha$, $\lambda$, $\Lambda$ or indeed any information on the law of the joint random variable $z = (z_1,z_2,\dots) \in [-1,1]^\N$. In particular, the  random variables $z_1, z_2,\dots$ need not be independent. For the following arguments, it is sufficient that an expansion of the form \eqref{eq:expansion} exists.
\end{remark}

Given this setting, we prove the following theorem:

\begin{theorem} \label{thm:darcy}
Under the setting and the prevailing assumptions of this section and with $\mu\in \cP(\cX)$ the law of $a(\slot;z)$ given by \eqref{eq:expansion}. For any $\delta,\eta > 0$ and $\epsilon > 0$, there exists a PCA-Net $\Psi = \cD_\cY \circ \psi \circ \cE_\cX$ satisfying the error bound,
\[
\E_{a\sim \mu}\left[\Vert \Psi^\dagger(a) - \Psi(a) \Vert^2_{H^1_0} \right]
\le 
C\epsilon,
\]
with probability at least $1-\delta$, and with constant $C = C(\mu,\eta)>0$ depending only on $\mu$ and $\eta$. With the same implied constant, the required PCA dimensions are at most $\dx = \dy = d \sim \epsilon^{-\frac1{2\alpha} - \eta}$ and the required number samples for PCA is at most $N \sim d^{1+4\alpha} \log(1/\delta)$. Furthermore, the following complexity bounds hold for the ReLU network $\psi$,
\[
...
\begin{remark}
In particular, Theorem \ref{thm:darcy} shows that approximation of the Darcy flow operator $\Psi^\dagger$ is possible with algebraic bounds on the PCA dimensions $\dx$, $\dy$, the number of required PCA samples $N$, and the number of neural network parameters, $\size(\psi)$. In fact, even under a mild decay rate $\alpha > 1$, the required size of $\psi$ is at most \emph{linear} in the desired accuracy.
\end{remark}
...
\begin{align}
\label{eq:F}
\cF(z) := \Psi^\dagger\left(\bar{a} + \textstyle\sum_{\ell=1}^\infty \gamma_\ell z_\ell \rho_\ell\right).
\end{align}
This mapping has been studied in a series of papers \cite{cohen2010convergence,CDS2011,SchwabZech2019,OSZ2019,OSZ2020}, and is known to allow for a convergent Taylor series expansion (in the variables $\bm{z}\in U$). To state the next Lemma, on a suitable convergence of Taylor series, we recall that $\bm{\nu}$ is called a multi-index in this infinite-dimensional context, if $\bm{\nu}=(\nu_1,\nu_2,\dots)$ is a sequence of non-negative integers, such that $\nu_\ell =0$ for almost all $\ell$. For $\bm{z} \in U$ and a multi-index $\bm{\nu}$, we define the monomial $\bm{z}^{\bm{\nu}} = \prod_{\nu_\ell\ne 0} z_\ell^{\nu_\ell}$. In particular, the following lemma follows immediately from \cite[Thm. 1.3]{CDS2011}.
\begin{lemma}
\label{lem:Taylor}
...
Under the prevailing assumption and with $\cF$ defined by \eqref{eq:F}. Let $\eta>0$ be a small (fudge) constant. There exists a constant $C=C(\cF, \mu, \eta)>0$, depending only on $\cF$, on the decay rate $\alpha$ and on $\eta$, such that for any bounded linear map $\cE_\cY: \cY \to \R^{d_{\cY}}$, and for every $m\in \N$, there exists a ReLU network $\psi^\star: \R^m \to \R^{d_{\cY}}$, $y\mapsto \psi^\star(z_1,\dots, z_m)$ with
...
and such that
\begin{gather*}
\size(\psi^\star) \le Cm \left(\log(m)^2 + d_{\cY}\right), 
\\
...
\right.
\end{align}
Here $u: \T^n\times [0,T] \to \R^n$ is the flow vector field, $p: \T^n \times [0,T] \to \R$ is the scalar pressure and $\nu\ge 0$ is the viscosity (we allow $\nu=0$, corresponding to the incompressible Euler equations). We have denoted by $\bar{u}: \T^n\to \R^n$ the (divergence-free) initial data. Since the solution operator $\Psi^\dagger$ associated with \eqref{eq:ns} is only known to be well-defined in two spatial dimensions, we focus on this case. We however point out that all results readily extend to the three-dimensional case, under additional (as of yet unproven) smoothness assumptions. 

\begin{remark}
Classical well-posedness results for the Navier-Stokes ($\nu > 0$) and Euler ($\nu = 0$) equations imply in the two-dimensional case $n=2$, that if the random initial data $\bar{u} \sim \mu$, is uniformly bounded $\Vert \bar{u}\Vert_{H^r}\le \bar{M}$ for $r>n/2+1$, then the corresponding solution $u(t)$ at a fixed later time $t\in [0,T]$ satisfies a similar bound
...
\end{proposition}
In fact, Proposiiton \ref{prop:ns-nn} is the two-dimensional case of a general $d$-dimensional result (cf.  Proposition \ref{prop:nsd-nn}). Given this neural network emulation result, the remaining issue is that the empirical PCA encoder $\cE_\cX$ and decoder $\cD_\cY$ do \emph{not} act on Fourier coefficients. And hence, additional work is necessary to overcome this incompatibility and suitably adapt the construction of the neural network in Proposition \ref{prop:ns-nn}, which ultimately results in an efficient PCA-Net approximation $\Psi = \cD_\cY \circ \psi \circ \cE_\cX$ in Section \ref{sec:PCA-error}, summarized in the following lemma:
\begin{lemma}
\label{lem:NS-approx-err}
Given the above setting, for any $K\in \N$ (corresponding to Fourier modes with wavenumber $\vert k\vert_{\infty} \le K$), there exists a PCA-Net $\Psi = \cD_\cY \circ \psi \circ \cE_\cX$, such that 
\begin{align*}
\E_{u\sim \mu}\left[
\Vert \Psi(u) - \Psi^\dagger(u) \Vert_{L^2(\mu)}^2
\right]^{1/2}
&\le
C \epsilon + C\E_{u\sim \mu}\left[\Vert u - P_\cX u \Vert_{L^2}^2\right]^{1/2} 
\\
&\qquad + \E_{v \sim \Psi^\dagger_\#\mu}\left[\Vert v - P_\cY v \Vert_{L^2}^2\right]^{1/2},
\end{align*}
...
To prove this claim, it is sufficient to prove that such $\Psi^\dagger_\gamma: \cX \to \cY$ exists with the \emph{additional constraint} that $\im(\Psi^\dagger_\gamma)$ belongs to a one-dimensional subspace of $\cY$. In this case, we can clearly identify $\Psi^\dagger_\gamma$ with a mapping $\cX \to \R$, and thus, our goal is to prove Proposition in the (most constrained) case, where the output space $\cY = \R$ is one-dimensional. This will be the goal of the remainder of this section.
...
\le 1,
\end{align*}
so that that $f\in F_{k,d}$. This is irrespective of the specific choice of $f_s \in F_{k,d}$. We are going to  determine the $f_s$ recursively, formally starting from the empty sum, i.e. $f_0=0$. We fix $2\lambda := 1/25$ throughout the following construction. In the recursive step, given $f_1,\dots, f_{s-1}$, we distinguish two cases:
...
\end{align*}
The claimed result now easily follows by noticing that $\eta>0$ in the last estimate is arbitrary; we can thus repeat the argument with $\eta/2$ in palce of $\eta$ to arrive at the claimed result.
\end{proof}
...
\]
Next, we recall that the $\mu$ and $\Psi^\dagger_\#\mu$ are concentrated on bounded sets; indeed, this is the case by assumption for $\mu$, and is a consequence of the a priori bound \eqref{eq:apriori} in the case of $\Psi^\dagger_\#\mu$. Invoking Proposition \ref{prop:pcaproj}, we can therefore estimate
\begin{align*}
...
\right),
\end{align}
for any $r\ge n/2+1$, and where the constant $C>0$ depends only
...
\]
The fixed-point function $F(\hat{w}) = F(\hat{u}^m;\hat{w})$  (see \eqref{eq:Ffixedpt}) applied in the inner-most loop of Algorithm \eqref{alg:emulation}, lines 11-13, can clearly be represented by a a neural network of a size $\sim \size(\NL_{\bar{M},\epsilon})$, as it only differs from $\NL_{\bar{M},\epsilon}$ by a linear input and a linear output. Composing $L \lesssim_{r,n} \log(MTK)$ of these networks, we obtain a representation $\psi^{\ast}: \hat{u}^m \mapsto \hat{u}^{m+1}$, corresponding to lines 9-14 of Algorithm \ref{alg:emulation}, and such that 
\[
