...
\begin{document} 
   ...
   \title{Near-IR and optical radial velocities of the active M-dwarf star Gl~388 (AD~Leo)  with SPIRou at CFHT and SOPHIE at 
   	  OHP\thanks{Based on observations obtained with the spectro-polarimeter SPIRou at the Canada-France-Hawaii Telescope (CFHT)
	  and the spectrograph SOPHIE 
          at the Observatoire de Haute-Provence (OHP) in France.}:
          }

   \subtitle{A 2.23 days rotation period and no evidence for a co-rotating planet.
          }
   ...
   \institute{Universit\'e Grenoble Alpes, CNRS, IPAG, 38000 Grenoble, France\\
    ...
    ECyT-UNSAM, Campus Miguelete, 25 de Mayo y Francia, (1650), Buenos Aires, Argentina
          \and
     Canada France Hawaii Telescope Corporation, CNRS/INSU
     \and
...
% 5 {} token are mandatory
  ...
  {The search for extrasolar planets around the nearest M-dwarfs is a crucial step towards identifying the nearest Earth-like planets.
  ...
  that could mimic those of a planet. 
   ...
   is due to stellar activity or to a planet which co-rotates with the star as suggested in the past.}
  % methods heading (mandatory)
   ...
   {
   The SOPHIE RV time-series (precision of  3 to 5 m s$^{-1}$ per visit) displays a periodic signal with 2.23$\pm$0.01 days period and 
   ...
   is due to stellar activity rather than to a co-rotating planet. 
...
is to identify and characterise rocky planets orbiting the nearest stars which 
...
Furthermore, as M-dwarfs have low-masses, Earth-like planets in their habitable zone can induce a RV signal of a few m/s, 
...
Late type M-dwarfs, even when nearby, are faint in the optical domain, which makes obtaining the high S/N high 
resolution spectra required for precise RV challenging with moderate aperture telescopes. 
...
the RV signal has the same amplitude and phase in optical and near-IR observations.
The RV signature of a planet is achromatic, 
while activity induces a different RV signature in near-infrared (near-IR).
Furthermore,
...
which has mostly been interpreted as due to activity but which \cite{Tuomi2018} claimed is caused by a 0.24 M$_J$ (76 M$_\oplus$) planet that co-rotates with the star.
...
However, these works have not been able to firmly reject the hypothesis of the co-rotating planet.
\citet[][]{Kossakowski2022} finds for instance that a mixed model of a stable ($K=$16.6 m~s$^{-1}$, 3$\sigma$ upper limit on the putative planet of 27 M$_\oplus$) 
plus a quasi-periodic red-noise  model best explains their RV data.
Near-IR RV measurements with higher accuracy can settle
the issue of the co-rotating planet around Gl 388 for good, 
which is what we set to do in this paper. 


We have obtained quasi-simultaneous RV measurements in 2019 to 2021
...
with SPIRou\footnote{Spectro-Polarim\`etre~InfraRouge, meaning, infrared spectro-polarimeter.} at the CFHT.
SPIRou observations were obtained in the framework of the SPIRou Legacy Survey, 
a large observing programme of 310 nights with SPIRou dedicated to study planetary 
systems around nearby M dwarfs  and magnetized star / planet formation.
We use the SOPHIE and SPIRou radial velocities (RVs) and activity indicators, 
and SPIRou measurements of
the longitudinal magnetic field (B$_\ell$), to probe whether the RV
signal detected in the optical is indeed due to a planet, or instead reflects stellar activity. 
...
Its optical spectra displays strong chromospheric emission lines, including a prominent H$\alpha$ emission with central absorption, 
...
\citet[][]{Morin2008} studied Gl 388 with the ESPaDOnS\footnote{\'Echelle SpectroPolarimetric Device for the Observation of Stars.}  spectropolarimeter, and reported 
an effective Rossby number of  4.7$\times10^{-2}$, a $R~{\rm sin}~i$ of 0.13 R$_\odot$ and an inclination of 20$^\circ$. 
They found a strong negative polarimetric signature (i.e. a longitudinal field directed towards the object)
...
ZDI  revealed an average magnetic flux $B\simeq$ 0.2 kG, a strong polar spot with a radial field with a maximum magnetic flux $B\simeq$ 1.3 kG. 
The prominent component observed is the radial component of a dipole almost 
aligned with the rotational axis which indicated a strongly axisymmetric magnetic topology with 90\% of the large-scale energy in the $m=$~0 modes.


The \citet[][]{Bonfils2013} M-dwarf survey 
with the HARPS\footnote{High-Accuracy Radial velocity Planet Searcher \citep[][]{Mayor2003}.} instrument reported 
high-precision RVs of Gl~388. Their RV time-series periodogram contains significant power excess at 1.8 and 2.2 days periods, 
which are 1-day aliases of each other, and the latter of which is consistent with the stellar rotational period previously identified through ZDI. 
The authors performed a one planet fit to the data obtaining a period of $2.2267\pm0.0001$ days, 
a semi-amplitude $K=30.5\pm1.3$ s$^{-1}$ and eccentricity of $0.1$.
\citet[][]{Bonfils2013} showed that the  bisector span has periodogram with broad power at shorter periods 
and that it is anti-correlated with the RV, which strongly suggests that stellar activity is responsible 
for the periodic RV variation.
\citet[][]{Reiners2013} analyzed the same HARPS data set with an alternative template matching technique \citep[HARPS-TERRA\footnote{Template-Enhanced Radial velocity Re-analysis Application.} software,][]{Anglada-Escude2012}, 
and independently confirmed the conclusions of \citet[][]{Bonfils2013}. 
Analyzing the RV behaviour in seven sections of the spectra, centered at 419, 450, 486, 528, 583, 631, and 665~nm, 
they found the same 2.2~days period for each section (as would also be expected for a Doppler signal) 
but that the semi-amplitude $K$ decreases with increasing wavelength, 
providing independent evidence that the periodic RV signal is due to stellar activity.
When analyzing the full spectrum, \citet[][]{Reiners2013} also find that the cross-correlation function (CCF) and the template matching technique 
...
They analysed HARPS and HIRES\footnote{HIgh Resolution Echelle Spectrometer \citep[][]{Vogt1994}.} 
...
which is consistent with previous measurements, and like \citet[][]{Bonfils2013} report a correlation 
between the RV signal and the bisector span (BIS). 
Contrary to \citet[][]{Reiners2013}, \citet[][]{Tuomi2018} reported the RV signal to be independent of the wavelength range, 
besides the bluest 24 HARPS orders, which the authors claimed to be heavily contaminated by activity.
Their photometric time-series displayed the 2.23 day period too, but with considerable variability: 
...
The photometric signal therefore appears to vary on short time scales, 
while the RV signal is stable over thousands of days.
\citet[][]{Tuomi2018} tested multiple star-spots scenarios and found simultaneously explaining a rapidly 
varying photometric signal and a stable RV signal very challenging. 
They interpreted the RV signal as a 0.24 M$_J$ planet because they find the signal independent of the wavelength range 
and because is stable and invariant over time. They considered unlikely that this is caused by stellar activity 
and stellar spots co-rotating on the stellar surface.
...
they found their HARPS-N optical RVs to vary with a period of 2.2231 days
and a semi-amplitude $K$ of 33 m s$^{-1}$ and that the RV signal and the BIS are strongly anti-correlated.
Like \citet[][]{Reiners2013}, they report different RV amplitudes for optical RVs measured by a cross-correlation method  
(DRS\footnote{Data Reduction System.} pipeline) and by a template matching method (TERRA pipeline), with a smaller amplitude for the template matching method.
...
With a Gaussian prior of 2.225 $\pm$ 0.02 days on the period and 0.3 the eccentricity,
\citet[][]{Carleo2020} set a 3$\sigma$ upper-limit of 23~m~s$^{-1}$ on the near-IR semi-amplitude, 
...
{In a recent work \citet[][]{Kossakowski2022} have investigated Gl~388 at optical and near-IR wavelengths 
with CARMENES\footnote{Calar Alto high-Resolution search for M dwarfs with Exoearths with Near-infrared and optical \'Echelle Spectrographs.} \citep[][]{Quirrenbach2014}.
The 2.23 day periodicity was recovered in the CARMENES optical RVs and a
strong anti-correlation between the chromatic index (CRX), the bisector (BIS) and 
...
They found that the RV signal exhibit amplitude fluctuations and phase shifts in time,
a behaviour that is not compatible with a Keplerian origin of the signal.
...
has a semi-amplitude $K$=16.6$\pm$2.2~m~s$^{-1}$,
which translates in a 3$\sigma$ upper-limit to the candidate planet of 27 M$_\oplus$ (=0.084 M$_J$) in a 2.23 day period.
...
Measurements in the near-infrared with higher precision are required to 
test whether the stable component suggested by the Gaussian process analysis 
...
To investigate variations on longer time scales,
we also include archival optical data obtained  from 2005 to 2016 with HARPS.
In the following subsections we describe each data-set.

\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{HARPS_SOPHIE_SPIROU_time_coverage_v2.pdf}
\caption{The new SPIRou and SOPHIE RVs, together with the archival HARPS data used in this work.
}
...
on the 3.6m telescope at La Silla Observatory in Chile.
The HARPS data-set consists of 51 measurements, of which
39 were acquired between 2005 and 2006 (HARPS1 dataset) 
and previously reported in  \citet[][]{Bonfils2013}
and 12 are new measurements obtained between 2013 and 2016 (HARPS2 dataset). 
Each spectrum covers the 380-690 nm spectral range with 71 orders at a 
median resolving power of 115000. Spectra
are wavelength calibrated to a high accuracy using a simultaneous ThAr hollow cathode 
...
Data was reduced with the SOPHIE data-reduction system \citep[][]{Bouchy2009a,Heidari2022thesis}.
...
with the SPIRou near-IR spectropolarimeter  \citep[][]{Donati2020} 
installed on the 3.6m Canada-France-Hawaii Telescope (CFHT) 
atop Mauna Kea (Hawaii). The SPIRou spectra cover from 0.96 to 2.50 $\mu$m at a resolving power of $R\sim70000$.
Figure~\ref{HARPS_SOPHIE_SPIROU_time_coverage} illustrates the time coverage of the HARPS, SOPHIE and SPIRou observations.
...
Details of the wavelength calibration algorithm which uses both Uranium-Neon lamp and Fabry-P\'erot exposures is described in 
\citet[][]{Hobson2021} and \citet[][]{Cook2022}.
The 1D spectrum is then corrected for telluric absorption using a principal component analysis (PCA) method 
...
RVs measurements were extracted from the spectra with a line by line (LBL) algorithm 
described in \citet[][]{Artigau2022}.
They were referred to the barycenter of the Solar System using the  astropy routine barycorrpy, and corrected for the drift of the Fabry-P\'erot and for changes in the RV zero-point.
The RV zero-point correction was derived from observations of set of RV RV standards that are observed by SPIRou nightly since 2018.
...
to those spectra to extract average Stokes $I$ (unpolarised) and $V$ (circularly polarised) profiles. 
The line mask for Least Squares Deconvolution (LSD) was generated via the Vienna Atomic Line Database (VALD) \citep{Gustafsson2008} from a synthetic spectrum 
with $T_{\mathrm{eff}}=3500$ K, $\log g=$ 5.0 cm~s$^{-2}$ and $v_{\mathrm{micro}}=$ 1 km~s$^{-1}$, 
and contains 1400 atomic lines between 950 and 2600~nm deeper than than 3\% of the continuum level and with a known 
...
The integral in Eq.~\ref{eq:Bl} was computed within $\pm$ 50 km s$^{-1}$ of the profile center, to encompass the full line width in both Stokes $I$ and $V$.
We exclude six February 2019 observations from the magnetic analysis presented here, because an optical component of the instrument was dysfunctioning 
at very low temperatures and affected those six circular polarisation spectra.
...
on-line interface hosted by the University of Geneva\footnote{https://dace.unige.ch/radialVelocities/},
we calculated Least-Square Periodograms 
for the HARPS1, HARPS2  and SOPHIE time-series, both individually and combined.
Figure~\ref{Periodograms_HARPS_SOPHIE} summarizes the periodograms obtained.
All optical data-sets produce a prominent peak at 2.23 days 
...
We used DACE to fit a Keplerian model to each dataset, using the 2.23 days peak to set
the initial conditions\footnote{The initial conditions of the Keplerian model are computed using the formalism of \citet[][]{Delisle2016}.}.
...
Even though the 2013$-$2016 HARPS2 dataset has a larger dispersion than 
the 2005$-$2006 HARPS1 dataset (see Figure~\ref{HARPS_SOPHIE_SPIROU_time_coverage}),
the combined HARPS1 + HARPS2 dataset can be described by a similar period and semi-amplitude to that of HARPS1, 
with the main difference being on, 
...
with a  2.23 days period retrieved in both data-sets, but
with a  2.3$\pm$0.6~m~s$^{-1}$ difference 
between the semi-amplitudes of the Keplerian fits.
...
However, the 2.227, 2.230 and 2.231~days periods are aliases of each other 
for samplings of $\sim$3300 and $\sim$5000 days, which respectively correspond to the average interval 
between the HARPS1 and HARPS2 acquisition periods and between the HARPS1 and SOPHIE ones 
(Figure~\ref{HARPS_SOPHIE_SPIROU_time_coverage}). 
...
a single global Keplerian solution can be describe all HARPS and SOPHIE RVs. 
...
we will use only the SOPHIE data in our comparison with the near-IR RVs.
...
We can clearly see that the infrared RVs do not match the periodic signal observed in the optical data, 
...
it clearly excludes  the periodic signal of 24~m~s$^{-1}$ semi-amplitude observed in the optical RVs
...
\begin{figure}
   ...
   \includegraphics[width=0.5\textwidth]{SOPHIE_SPIROU_phase_folded.pdf}
      ...
      The grey line represents the Keplerian model of period 2.23 days and $K=$ 23.6 m s$^{-1}$ derived from the SOPHIE data (Table~\ref{table:HARPS_SOPHIE_periodogram}).
      }
...
and have shown a flat RVs at a precision of 2.6~m~s$^{-1}$ rms in Barnard's star 
\citep[][]{Artigau2022}, 
...
inject a sinusoidal Keplerian signal at 12 equi-spaced phases.
...
amplitude (and the corresponding projected mass) 
as a function of  period. 
The 1\% FAP confidence level is computed from the levels of highest peaks of 1000 permuted data sets, 
and the 1\% upper limits function is obtained by sorting the 
...
The SPIRou observations would have detected a 2.23~days period planet of 8 and 4 Earth-masses
with 99\% and 75\% confidence levels respectively. 
   ...
   \centering  
    ...
    {\it Left panels: } upper-limits in m s$^{-1}$ of the semi-amplitude $K$ of the sinusoidal signal.
     {\it Right panels:} upper-limits in terms of the projected mass m~{\it sin(i)}. 
     Objects with projected mass above the limit are ruled out by the observed GLS periodogram 
     ...
     {\it Lower panels:} zoom on the 0.8 to 10~days range. 
     The red star indicates the location of the 0.24 M$_J$ (76 M$_\oplus$) planet in co-rotation claimed by \citet[][]{Tuomi2018}.
...
chromatic dependence of the RV signal inside the optical or the infrared domain.
...
We selected a 400K temperature difference between the photosphere and the spot as it is typical 
for a mid-M star \citep[][]{berdyugina2005, barnes2015}.  
To analyse the SOPHIE data, 
we constructed two cross-correlation masks starting from the M-star mask used to compute the RVs: 
a "high-contrast" mask which contains the lines that fall in the regions where the flux ratio is {\it higher} than its median value,
and a  "low-contrast" mask with the lines that fall in the complementary regions where the flux ratio is {\it below} its median value.
The "high" and "low" contrast  regions of SOPHIE are illustrated in  Figure~\ref{SOPHIE_high_low_regions} in the Appendix.
To analyse the SPIRou data, we defined in an analogous manner the "low" and "high" contrast regions as illustrated in 
Figure~\ref{SPIROU_spot_3400_3000} in the Appendix.
The line-by-line method calculates a RV for each "line" in the spectrum 
...
we find largely equivalent results for the three masks. 
All of them produce periodograms with a peak at the same 2.23 days period and have a similar phase for the signal.
The amplitude, however, does differ by a small but significant 2.5 m s$^{-1}$ between the RVs calculated with the "low" and the "high" mask.
Figure \ref{SOPHIE_high_low_RV} in the Appendix displays the phase-folded SOPHIE RV time-series obtained with the three masks, 
and they differ little. 

The properties of SPIRou's "high", "low" and "full" RV time-series are summarised in Table~\ref{table:SPIROU_chromatic}. 
...
\hline\hline       
   & full & high & low \\
    \hline                    
median RV [km/s]			& 12.505 	                         & 12.515                             & 12.493\\
mean $\sigma_{\rm RV}$ [m/s] & 3.0	                                  & 2.8		                  & 3.2 \\
...
\begin{table}
\centering  
\caption{1$\sigma$ confidence interval on the semi-amplitude K for a sinusoidal fit with fixed P=2.23 days and phase in the RV time-series
per band in the optical (SOPHIE) and near-IR (SPIRou). }
...
but this time we selected the orders used to built the CCF.
For the B band we selected the orders 3 to 18 ($4008.5 - 4962.7$ \AA), 
for the V band the orders 19 to 31 ($4934.9 - 6092$ \AA),
and for the R band the orders 32 to 38 ($6077 - 6943$ \AA).
For SPIRou,
we analysed the RV time-series obtained by applying the LBL algorithm to individual near-IR bands within the SPIRou spectral domain:
Y ($900.0 - 1113.40$ nm),
...
In all data sets, 
we used the same T$_0$ such that the SOPHIE RVs have phase = 0 at $v=0$~km~s$^{-1}$.
In each RV time-series, 
we fitted a sinusoidal signal with fixed  period of 2.23 days and phase=0.
Table~\ref{table:SOPHIE_SPIROU_K_sinfit} shows the retrieved best-fit semi-amplitude $K$ in m s$^{-1}$ 
with a $1\sigma$ error-bar.
   
In the case of the SOPHIE data, 
the B, V, and R radial velocity time-series display, within 1$\sigma$, the same sinusoidal signal with a very similar amplitude.
A slight decrease of $-2$~m~s$^{-1}$ in the semi-amplitude with respect to the B band is seen 
the V and R bands, however, the difference is smaller than the $3 - 4$~m~s$^{-1}$ $1\sigma$ uncertainty in the semi-amplitude.
...
The sinusoidal fits in the Y and J band hint a periodic signal similar to that observed in 
the optical but with a weaker amplitude.
However, 
when the periodogram analysis is performed (see Figure$~\ref{Activity:Periodograms}$ in the Appendix), 
none of near-IR RV time-series per band display a significant peak at $P$=2.23 days.
The semi-amplitudes error shows that $K$ is compatible with 0 m s$^{-1}$ within the 1$\sigma$ uncertainty.
The sinusoidal model is in fact fitting the noise dispersion.
The sinusoidal fit enables, nevertheless, to lower the upper-limit of the semi-amplitude to 17, 6, 1, 5, and 2 m s$^{-1}$ 
for the Y, J, H, K and the full near-IR SPIRou domain respectively (see Table~\ref{table:SOPHIE_SPIROU_K_sinfit}).
The difference in semi-amplitude between the H and K bands could suggest a difference in RV dispersion due to the onset 
...
The increase on RV semi-amplitude at $\lambda>1500$ nm of tens of m s$^{-1}$ predicted by some 
...
however,  we don't observe strong chromatic effects within the individual photometry bands.
A small but significant difference is found in the RV analysis between 
"high" and "low" CCF masks of SOPHIE. No difference was found in other tests.
...
\end{figure*}
  ...
  \begin{figure}
   \includegraphics[width=0.5\textwidth]{Bl_LSP_19a19b20a20b_notrend.pdf}
    ...
    A long-term variation of the $B_\ell$ data set is filtered out beforehand, to prevent it from perturbing the period analysis. 
...
which a future publication (Bellotti et al. in prep) will use for a full Zeeman Doppler Imaging analysis.
Table~\ref{SPIRouRVBlTable} summarizes the $B_\ell$ measurements  obtained from those profiles.
$B_\ell$ is detected at every epoch and varies from $-55$~G to $-277$~G,
with a median of $-194$~G, and its median measurement noise is 15~G.
Figure~\ref{fig:mask} displays the generalised Lomb-Scargle periodogram of the $B_\ell$ time-series. 
Although stellar activity signals sometimes lose  coherency over long time spans, the periodogram here shows a single, clear, narrow, and highly significant (FAP$<$ 0.01\%) peak at 2.23$\pm$0.01 days.
This period is consistent with those derived from both the optical spectropolarimetric observations of \citet{Morin2008} and the SOPHIE, HARPS, HARPS-N and HIRES RV measurements.
The periodogram structure at longer periods is likely due to aliases of the observation window.
%
Previous studies have shown that  of $B_\ell$ is a reliable magnetic activity tracer \citep{Folsom2016, Hebrard2016} and that it varies on the stellar rotation period. 
...
\subsection{Gaussian Process analysis of the optical HARPS and SOPHIE data using SPIRou's $B_\ell$ as activity proxy.}

\subsubsection{Gaussian Process modeling with a variable decay-time.}
Gaussian Process (GP) modelling of RV time-series is nowadays a standard procedure to correct for 
...
$B_\ell$ provides therefore an excellent proxy of stellar activity for modelling the optical RVs within the GP framework.
The  use of $B_\ell$ as stellar activity indicator is motivated by the close physical link between stellar magnetic activity and
the presence of spots and plages, and the results of spectropolarimetry 
Zeeman Doppler Imaging studies in M dwarfs \citep[e.g.][]{EmilieH2014,EmilieH2016} 
...
\begin{table*}[]
\centering
  \caption[]{Prior and posterior distributions of the quasi-periodic GP model of the SPIRou $B_\ell$ measurements and the HARPS and SOPHIE RV time-series.}
  \label{GP_table}
...
We modelled the combined HARPS and SOPHIE RVs 
running two sequential  Quasi-Periodic Gaussian Processes (QP GP).
We run first a GP on the SPIRou $B_\ell$ time-series. 
Then, using the decay-time, smoothing and cycle length (i.e. rotation period) 
posterior distributions of the $B_\ell$ GP as priors, we run a second GP on the optical RV-series. 
For the $B_\ell$ GP, 
we used uniform distributions for the priors of the amplitude, decay-time, smoothing, cycle-length and uncorrelated noise (see further details in Table~\ref{GP_table}). 
This modelling strategy has the underlying assumption that the RV effects and the magnetic field have the same temporal behaviour.
Our principal aim is to check how well the optical RVs
could be described by QP GP model in which  the stable component is 
associated to rotation and the variable component is associated to changes in magnetic activity,
and determine the typical time-scale of that variable component.
...
and $\sigma$ the uncorrelated noise (s in the corner plots).
...
It is the similar value retrieved using the periodogram analysis,
however, with a smaller uncertainty. The decay time obtained in $B_\ell$ time-series is $216^{+82}_{-54}$ days.

\begin{figure}
    \centering
    \includegraphics[width=0.49\textwidth]{gl388_bell.pdf}
    \caption{
    Longitudinal magnetic field ($B_\ell$) time-series (black dots) and its quasi-periodic Gaussian 
    Process model fit (green-line). Details of the GP are given in Table~\ref{GP_table}. 
...
The decay time obtained in the radial velocity GP is $205^{+36}_{-32}$ days, 
a value that is also consistent within the error bars with the decay time obtained in $B_\ell$ GP.
...
in the RV jitter.
The similarity on the period and decay time of the $B_\ell$ GP and RV GP 
...
as activity proxy provides a good description the observed optical RV time-series variability.
...
\begin{figure*}
   ...
   \includegraphics[width=0.98\textwidth]{GL388_HARPS_SOPHIE_RV_GP.pdf}
    \caption{Summary of the GP analysis of the optical RV time-series using SPIRou's $B_\ell$ as activity proxy. 
    {\it Upper left:} Quasi-periodic Gaussian Process (QP GP) fit (green line) to the HARPS and SOPHIE optical RV time-series (black points). 
    ...
    {\it Lower left:} Residuals in the optical RVs after the subtraction of the QP GP fit.
     ...
     In the top of each periodogram, the period and FAP of the strongest peak is given.
...
the GP by construction does not keep constant the phase between the various sets of optical RV data. 
This analysis leaves unanswered the question whether is it still possible to fit all the 
optical RV data with a slowly evolving pattern that does not lose phase from run to run 
(i.e. can the RV time-series be described with a GP with a long decay time?),
nor the question of how long can the decay time be before the uncorrelated noise and RV rms start to increase. 
To explore the influence of the decay-time in the uncorrelated noise and RV rms,
and to test whether a very long decay-time ($>$1000 days) could  describe the data,
we run a series of QP GP on the optical RV data-sets, 
by fixing in each the decay time to different values from 1 to 10,000 days.
Our results are summarised in Figure~\ref{fig:rms_vs_decay}
where we show the evolution of the uncorrelated noise 
and the RV rms as a function of the decay time.
We  observe that as soon the decay time becomes 
larger than 10 days the uncorrelated 
noise and the RV rms increase.
...
The decay-time of the order of hundred days suggested both by the variable and fixed decay-time Gaussian Process modelling 
indicates that the RV jitter is evolving with time on a timescale of less than a year, conversely to what \citet[][]{Tuomi2018} claimed.
\begin{figure}
   \begin{center}
   \includegraphics[width=0.5\textwidth]{uncorr_and_rms_versus_decay_time.pdf}
    ...
    The horizontal line in grey dots shows the typical accuracy of SOPHIE RVs (3 m s$^{-1}$). The vertical line in grey dots indicates the decay time of 80 days. 
     }
...
for the SOPHIE dataset.
The two quantities are slightly anti-correlated  with a Pearson coefficient $R$ of $-$0.23 (p-value = 0.05).
 This anti-correlation is weaker than the $R$ of $-$0.81 obtained by \citet[][]{Bonfils2013} with HARPS. 
This difference may be due to the higher spectral resolution of HARPS (R=120\,000) in comparison with SOPHIE (R=70\,000) 
since the BIS depends strongly on this spectrograph feature \citep{Desort2007}.
...
Within the LBL framework which we use for the SPIRou spectra, 
we adopt the second and third derivatives of the LBL line profiles as line-shape diagnostics \citep[][Cortez-Zuleta, in prep.]{Artigau2022}. 
...
\begin{figure}
   \centering         
		...
		\caption{BIS versus RV for the SOPHIE data. The red line indicates the linear fit between these variables. The Pearson's coefficient is $-$0.23, which indicates a weak anti-correlation.}
                 \label{SOPHIE:RV:BIS}
 \end{figure} 
 \begin{figure}
   \centering     	
		\includegraphics[width=0.49\textwidth]{SOPHIE_periodogram_BIS.pdf}
	         \caption{Generalized Lomb-Scargle periodogram of the SOPHIE bisector time-series. The grey dashed line marks the 1\% of false alarm probability (FAP). The highest peak in the periodogram corresponds to a 2.4~days period. The red vertical line indicates the location of the 2.23 days period.}
...
None of the peaks in either periodogram rises to the 1\% FAP line. We note that one of the highest peaks of the $ \rm H\alpha $ 
periodogram is located at 4.2 days, which is close to the $\rm 2\times P$ subharmonic of the 2.23~days rotation period, but this might be coincidental.

Activity indicators are not as well established in the near-infrared domain.
We analyzed  the temporal evolution of time-series of the 
pseudo-equivalent width (pEW) of four atomic lines that have been previously tested in the optical domain and/or for Sun-like stars: 
...
The periodograms of Iron, Potassium, and Titanium not exhibit any high significant periodicity that are not related to the spectral window function. 
...
Activity features, such as spots, close to the observed pole will be in view for their full life time; 
thus, their pEW and projected velocity change little with rotation.} 
Most of the stellar activity indicators tested in this work indeed do not vary on the 2.23~days stellar rotation period, 
...
\begin{figure*}
   \centering         
   		\includegraphics[width=0.98\textwidth]{Activity_indicators_SOPHIE_SPIRou.pdf}
		\caption{Generalized Lomb-Scargle periodograms of the activity  indicators measured in the SOPHIE and SPIRou spectra. 
		The dashed line marks the 1\% false alarm probability (FAP). The vertical red-line indicates the 2.23 days period.}
                 \label{Activity:Periodograms}
 \end{figure*} 		
 
\section{Conclusions}
   
RV searches for planets around nearby M-dwarfs are our best chance to identify rocky planets in the habitable zone that can
be characterised by future instruments \citep[e.g.][]{doyon2014, beichman2014, snellen2015, lovis2017}.
...
However, many nearby M-dwarfs are magnetically active. The spots and plages in their atmospheres present a challenge to RV planet searches, since they can induce RV signatures that mimic those of a planet. 

In this paper, we have presented contemporaneous  new RV and spectro-polarimetry observations of the magnetically active M-dwarf Gl~388 obtained between 2019 and 2021,
with the optical high-resolution spectrometer SOPHIE at OHP 
and the near-IR high-resolution spectro-polarimeter  SPIRou at CHFT.
Our observations were obtained ten to fifteen years after 
previous RV measurements with HARPS at la Silla (2005 and 2006)  
and  optical spectro-polarimetry  observations with ESPaDOnS at CFHT  (2007 and 2008).

The new  SOPHIE RVs vary on a  2.23$\pm$0.01~days period with 23.6$\pm$0.5~m~s$^{-1}$ amplitude, which is consistent with the earlier HARPS observations.
The two optical data-sets can be jointly fit with a single Keplerian solution, which however has larger residuals than separate fits to the individual data-sets. This
is explainable by subtle amplitude or phase change between the HARPS and SOPHIE epochs due to interactions between stellar differential rotation and changes 
in the average latitude of the spots. 
The HARPS data alone have long provided
strong evidence that the RV signal was due to stellar activity \citep[][]{Bonfils2013, Reiners2013}. 
The new SPIRou data displays no periodic signal and their 5~m~s$^{-1}$ dispersion
is well below the 23.6$\pm$0.5 amplitude of the optical RV signal, removing any possible doubt that the latter is indeed due to activity, and cannot be from the Doppler effect.
The HARPS and SOPHIE data demonstrate that activity can induce a RV signal which is largely
stable well over a decade, even in a star as active as Gl~388.
The long-term stability of the Gl~388 RV signal shows how easily an activity signature in optical RV measurements can be misinterpreted  
as evidence for a planet \citep[e.g.,][]{Tuomi2018}.

The SPIRou campaign on Gl 388 demonstrates spectacularly the lower sensitivity of the near-IR RVs (at least those obtained by an LBL method)  
to stellar magnetic activity. This opens the way to perform exoplanet searches around stars which so far have been little studied or excluded from 
studies in the optical because of their activity. 
...
For Gl~205, a star quieter than AD Leo, \citet[][]{Cortes-Zuleta2023} demonstrated that the RV jitter can 
...
With lower planetary masses, the combination of data from several instruments and the need of activity filtering,
a growing number of spurious signals is possible. 
To use near-IR RVs in addition to optical RVs is important to consolidate optical RV surveys with long-term follow-up campaigns. 
...
We measure a rotation period of 2.2305$\pm$0.0016 days from the SPIRou $B_\ell$ time-series.
ESPaDOnS and SPIRou, using the same technique at different wavelengths, produce essentially identical  measurements of the stellar rotation period. 
Spectro-polarimetry can determine the rotation period independently of photometry, which
is particularly relevant for stars with complex photometric time-series behaviour, and can independently measure the rotation period and the inclination.
...
\begin{figure}[h]
   \centering         
   		\includegraphics[width=0.5\textwidth]{SOPHIE_wavelenght_coverage.pdf}
		\caption{ {\it Upper panel:} theoretical PHOENIX medium resolution models for stellar atmospheres with $T_{\rm eff}=3400$ and $3000$ K in the 
                 SOPHIE wavelength range. {\it Lower panel:}  Flux ratio of the stellar atmospheres models of the upper panel. The horizontal red line is the median flux ratio.
                 Lines of the SOPHIE M-star mask in the regions over the median flux ratio constitute the "high-contrast" mask, the lines located below the median constitute the "low-contrast" mask.
                 }
                 \label{SOPHIE_high_low_regions}
\end{figure} 
\begin{figure}[h]
                 \includegraphics[width=0.5\textwidth]{SOPHIE_chromaticity.pdf}
                 \caption{Phase-folded RVs measured with SOPHIE using the ``full'', ``high-contrast", and ``low-contrast"  SOPHIE M-type star masks.
                 The model is the Keplerian model derived from the SOPHIE's full-mask RVs.}
   ...
   \centering         
                 ...
                 \caption{{\it Upper panel:} theoretical PHOENIX medium resolution models for stellar atmospheres with $T_{\rm eff}=3400$ and $3000$ K in the 
                 SPIRou wavelength range. {\it Lower panel:}  Flux ratio of the stellar atmospheres models of the upper panel. The regions highlighted in light-green 
                 are the ``high-contrast" flux-ratio regions used for filtering the LBL RV measurements. }
...
\begin{figure}[h]
   \centering         
                 ...
                 \caption{Phase-folded SPIRou RVs for the LBL calculation including the ``full" , ``high-contrast", and ``low-contrast" lines.
...
\begin{figure*}[]
   \centering         
                 ...
                 \caption{Phase folded RVs in the B, V, R bands for SOPHIE (yellow points) and in the Y, J, H, K bands for SPIRou (red points).
                 The best sinusoidal fit per band is given in a continuous line following the same color convention.
                 For comparaison, in each panel, we include (blue color) the phase folded RVs and sinusoidal fit obtained in the full SOPHIE range. 
                 For the sinusoidal fit and RV phase-folding the phase (i.e. T$_0$ ) and period (2.23 days) is the same of all the time-series.   
                 Note that although a sinusoidal fit is possible in the SPIRou data, the periodograms show that there is no signal at 2.23 days period 
                 (see Figure~\ref{SPIROU_periodogram_bands}). The SPIRou data is compatible with a straight line with $K=0$~km~s$^{-1}$. 
                 The sinusoidal fit to the SPIRou data enables us to set an upper limit of the RV dispersion that is smaller than the rms value.}
...
\centering    
                 ...
                 \caption{Periodograms of the SPIRou RVs in the Y, J, H, K bands. The horizontal lines mark FAP levels of 0.1, 1, and 10\%. No periodic signal is observed in any of the bands at 2.23 days. }
...
\begin{figure}[h]
    ...
    \caption{Corner-plot of the quasi-periodic Gaussian-process fit of the SPIRou longitudinal magnetic field (B$_\ell$) time-series. 
...
\begin{figure}
    ...
    \caption{Corner-plot of the quasi-periodic Gaussian-process fit of HARPS and SOPHIE RV time-series. 
    The description of  the meaning of the variables is given in Table~\ref{GP_table}.}
    \label{fig:corner_rv}
\end{figure}
