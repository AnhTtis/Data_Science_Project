...
Euclidean is the only surface to have constant right ratios.
It can replace the parallel postulates, to enable local definition of 2-dimensional Euclidean space.
It is more generic and much simpler than Riemann curvature tensor.
\end{abstract}
...
Using sets to define topological or geometric objects is already a common practice \cite{Differential Geometry} \cite{Isometric}.
...
It uses the right angle as the unit of measurement \cite{Mistakes}, but it does not provide any way to arbitrarily divide a right angle: Such a unit of angle measurement cannot be applied generally. 
Without angle measurement, two angles cannot be compared or equalized.  
...
\item specifies the different applicable scope of each axiom, and

\item progresses by narrowing down the scopes.
\end{enumerate}
This paper will provide a toy model for such a testable and progressive approach.
...
Another goal is to introduce a new and simpler implicit measurement called right ratio to replace Riemann curvature tensor \cite{Differential Geometry} which requires second derivative on a Riemannian manifold \cite{Differential Geometry}.  
In contrast, the new measurement of flatness does not require the surface to be differentiable.
Like Gaussian curvature \cite{Non-Euclidean Geometry}, it is a measurement for flat, hyperbolic, elliptic. 
Unlike Gaussian curvature, the right ratio is implicit rather than explicit, and it can measure the combination of flat, hyperbolic, elliptic at different measuring distance.
This paper also put forth a conjecture that the smoothness of right ratio may lead to Riemannian manifolds.
...
As in all approaches in studying geometry \cite{Euclid} \cite{Plane} \cite{Mistakes} \cite{Hilbert} \cite{Differential Geometry} \cite{Non-Euclidean Geometry}, the concept of the point is the necessary presumption for topology and geometry.  
The definition of geometric object is not the common geometric object, but only as a necessary condition for geometric objects which is limited to the context of this paper.
...
As the embodiment of the original axiom \textsl{``To draw a straight line from any point to any point''} in the original Euclidean geometry \cite{Euclid}:
...
Axiom \ref{axm:continuous} allows more than one straight line segments of the same length to connect any two points, such as between a pair of poles on a spherical surface, or on a conical surface.
...
As the embodiment of the original axiom \textsl{``To describe a circle with any center and distance''} \cite{Euclid} in the original Euclidean geometry but not limited to 2-dimensional:
...
As the embodiment of the original axiom \textsl{``To extend a finite straight line continuously in a straight line''} \cite{Euclid} in the original Euclidean geometry:
...
On a conical surface, any straight line segment with the sharp point of the cone as one end cannot be extended at that end.
...
$C \in \overline{AB*} \Longrightarrow \overline{AC*} = \overline{AB*}$.
\end{thm}
...
$C,D \in \overline{*AB*} \Longrightarrow \overline{*CD*} = \overline{*AB*}$.
\end{thm}
...
One conclusion of Theorem \ref{thm:local unique segment} is Theorem \ref{thm:line intersect line}, which says that any 2-dimensional space can also have a small enough 2-side area if the surface satisfied Axiom \ref{axm:continuous}.
\begin{thm}
...
\end{enumerate}
The above procedure constructs the only $\overline{*AE*}$.
\end{proof}
...
The secret to let the first four Euclidean postulates to specify Euclidean surfaces lies in how to interpret the fourth Euclidean postulate \cite{Euclid} \textsl{``That all right angles are equal to one another''}:
\begin{itemize}
\item If it means to be equal in all distance-scales, so that a right angle equals to itself in all different distance-scales, then the space is similar and it is Euclidean.  
...
One major problem of the Parallel Postulates is that it adds global requirement to 2-dimensional Euclidean surface.
In contrast, Definition \ref{def: Euclidean flat} is local.
Using Definition \ref{def: Euclidean flat} to replace the Parallel Postulates not only enables local establishment of Euclidean surface, but also provides easy local identification of elliptic or hyperbolic surface.
...
One example of this comparison is Archimedes' axiom in the enhanced axiom set \cite{Hilbert}.
...
Each axiom has its own applicable range, and the axiom set narrows down the applicable ranges progressively, e.g., the 5 axioms apply to non-Euclidean surfaces \cite{Non-Euclidean Geometry} as well.
...
Mathematically, it is much simpler than Riemann curvature tensor \cite{Differential Geometry} to measure flatness.
It does not require Riemannian manifolds \cite{Differential Geometry} so that it is more generic.
It can measure the curvature at different measuring distance, and it can measure the combination of different surface curvature, such as at different direction or different measuring distance.
It is even possible that the mathematical smoothness of local right ratio will lead to Riemannian manifolds.
