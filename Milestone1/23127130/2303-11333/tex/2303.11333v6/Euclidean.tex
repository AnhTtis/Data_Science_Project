...
Euclidean geometry \cite{Euclid}\cite{Plane}\cite{Old and New} is the origin of modern mathematics.
The original Euclidean axiom set contains 4 axioms. 
Since the discovery of parallel postulate \cite{Plane}, it is regarded as insufficient \cite{Old and New}\cite{Mistakes}, so it was enhanced by the Hilbert axiom set of 21 axioms in 5 groups \cite{Hilbert}.
...
Through its Archimedes' axiom \cite{Hilbert}, the Hilbert axiom set should have avoided this difficulty by defining the right ratio as proposed in this paper, but it has not, to allow the circular definition of angle measurement.
...
Also, Birkhoff's axiom set is too simple to be mathematically rigorous.

Both Hilbert axiom set and the Birkhoff axiom set belong to the second-order logic which is based on relations of point sets \cite{Old and New}. 
Tarski's axiom set \cite{Tarski} is a first-order logic \cite{Formal System}.
...
For example, in Tarski's axiom set, among the three alternative forms of the Axiom of Euclid \cite{Tarski}:
...
\end{enumerate}
a) and b) are also permissible on spherical surface, but c) is not, so it is hard to understand how these three alternatives are equivalent in that logic system, and how a) and b) can  unequally identify the surface to be Euclidean.
There is even no need to interpret meaning in Tarski's axiom set because the formalism of the first-order logic will prove or disprove all statements on the subject matter \cite{Old and New}\cite{Tarski}.
Tarski's axiom set is not suitable for common understanding of Euclidean geometry.

In this paper, the Hilbert axiom set is treated as the standard axiom set for Euclidean geometry.


\subsection{Localness}

Euclidean geometry does not use infinitive geometric objects such as straight lines as its foundation, but instead states \textsl{``if the line is extended to a sufficient length"} \cite{Euclid}.
...
Other existing methods in the Hilbert axiom set for this purpose such as \textsl{``The sum of the angles of a triangle is two right angles''} \cite{Hilbert} is not truly local because it cannot measure the curvature at a point on a manifold \cite{Differential Geometry}.
...
Axiom \ref{axm:monotonic} seems an implicit assumption in all the existing axiom sets for Euclidean geometry.
Even it were not, it can replace all the related axioms in the Hilbert axiom set.
...
Logically, when an axiom contains one or more primitive notions, it is no longer testable by itself.
...
Thus, the number of primitive notions for an axiom set has to be minimized.

One problem of Hilbert's axiom set \cite{Hilbert} is that it contains a few primitive notions without prior descriptions, such as the axiom \textsl{``I.3. Three points not situated in the same straight line always completely determine a plane.''} hides three primitive notion: dimension, flat plane, and the uniqueness of the flat plane. 
...
Tarski's axiom set contains point as the only primitive notion \cite{Tarski} so it is better.

Like the Hilbert axiom set, Tarski's axiom set states that three noncollinear points determine a 2-dimensional surface, but it lacks the actual definition of the 2-dimensional surface.
...
Both the Hilbert axiom set and Tarski axiom sets have a few axioms to describe ordered ``betweeness'' of points in a straight line \cite{Old and New}\cite{Hilbert}\cite{Tarski}.
...
For example, in Tarski's axiom set, ``betweenness'' can be deduced from ``congruence'' when $\leq$ relation is defined \cite{Tarski}, but such specification is weaker than using distance-based ordered set of points to specify continuity of the space, including different types of continuity  \cite{Isometric}.
...
Instead of stating all axioms of an axiom set together, this new axiom set specifies the different applicable scopes of each axiom, and progresses by narrowing down the scopes.

 

\subsection{Congruence} 

All three existing axiom sets contain congruence relations \cite{Hilbert}\cite{Birkhoff}\cite{Tarski}, which limits the space to be either Euclidean, or spherical, or hyperbolic \cite{Non-Euclidean Geometry}.
Instead,  this new axiom set needs no congruence, and conclude both the congruence and the similarity of Euclidean space, such that without its Axiom \ref{axm:flat}, it can cover much more types of spaces.
...
Along with this trend, and also to satisfy the AP preference in US college admissions, the best high school students in mathematics have focused on learning advanced mathematics such as advanced calculus and functional analysis, rather than thoroughly understanding basic mathematics such as Euclidean geometry.
...
In another word, it is only a necessary condition for defining distance in normal sense.
...
Axiom \ref{axm:measurable} states that the distance between any two points is constant.
...
\end{align}
\end{defn}
The definition of $\overline{AB}$ excludes when $A$ and $B$ are two opposite poles on a spherical surface, because one of such poles can be regarded as infinitively far away from the other pole so that they can only be connected by a straight line \cite{Non-Euclidean Geometry}.
...
the Tarski axiom set does not need any geometric object including $\overline{AB}$.
...
\ifdefined\VERBOSE
\begin{proof}
Theorem \ref{thm:segment inequality} leads to Theorem \ref{thm:inclusive outside round} and \ref{thm:inclusive inside round}.
Theorem \ref{thm:segment inequality} and Theorem \ref{thm:split segment} lead to Theorem \ref{thm:inclusive outside round} and {thm:inclusive inside round}.
The exception of Theorem \ref{thm:outside rounds}, \ref{thm:inside round}, \ref{thm:inclusive outside round} and \ref{thm:inclusive inside round} leads Theorem \ref{thm:intersecting rounds}.
\end{proof}
\fi
...
$\forall C \in \overline{AB} \Longrightarrow \overline{*AB*} = \overline{*AC} + \overline{CB*}$.
\end{thm}
...
\subsection{Surface $\iff$ 2-Sided 2-Dimensional Space}
...
However, this relation may not hold on to more complicated surfaces.
...
In contrast, Section \ref{sec:Non Euclidean} will show that non-Euclidean surfaces are not flat.
...
\begin{itemize}
\item 
Because Euclidean surface can be divided into identical squares, geometric objects from different parts of the surface can be compared for equality directly. 
This means that Euclidean surfaces are \emph{homogeneous} \cite{Non-Euclidean Geometry} and \emph{isometric} \cite{Non-Euclidean Geometry}.
...
This means that Euclidean surfaces are \emph{isotropic} \cite{Non-Euclidean Geometry} and \emph{similar} \cite{Non-Euclidean Geometry}.
...
This means that Euclidean surfaces are \emph{reflective} \cite{Non-Euclidean Geometry}.
...
Because Euclidean surfaces are isometric, isotropic, and reflective, the length of a straight segment is invariant under translation, rotation, and reflection.  
Because Euclidean surfaces are also similar, an angle distance ratio is invariant under translation, rotation, reflection, and scaling.
...
\caption{ 
\textsl{Left:} Normalized transitional distance of right ratio vs. conical cut angle from 0 to 180 degrees, and measuring angle from 5 to 85 degrees on a conical surface.
\textsl{Right:} Normalized transitional derivative of right ratio vs. conical cut angle from 0 to 180 degrees, and measuring angle from 5 to 85 degrees on a conical surface.
}
...
To show the extent of the Euclidean region of the right ratio measurement, Figure \ref{fig:transition} \textit{Left} shows $|FB|/|FA|$ which is defined as the \emph{normalized transitional distance of right ratio} vs. the conical cut angles and the measuring angles.
...
Another difference is that all other surfaces need parameters to characterize them, such as the radius of a spherical surface, but Euclidean surfaces are free from any such parameter.  
...
Each axiom has its own applicable range, and the axiom set narrows down the applicable ranges progressively, e.g., the first five axioms apply to non-Euclidean surfaces \cite{Non-Euclidean Geometry} as well.

Although Axiom \ref{axm:continuous} contains Axiom \ref{axm:measurable}, these two axioms are presented separately to be compatible with the Euclidean axiom set.
Also, Axiom \ref{axm:measurable} has wider applicable range than Axiom \ref{axm:continuous}.
...
Compared with the Hilbert axiom set \cite{Hilbert}, this new set is much simpler, because the new axiom set has much less primitive notions by utilizing real number set to define primitive notions in the Hilbert axiom set, and betweeness.  
The parallel postulate, the congruence relation, and the similarity relation are all the consequences of Axiom \ref{axm:flat}.
As shown in this paper, the new axiom set can be conclude each 2-dimensional related axioms in the Hilbert axiom set.
...
It may also be possible to prove that if a surface is isometric and isotropic, it is either Euclidean or spherical.
...
Prof. David Ebin from University of Stony Brook suggested that the axiom set needs point as primary notion.
...
Prof Jeremy Avigad from Carnegie Mellon University suggested that the axiom set should be compatible with current mathematical code such as lean, which resulted in dropping one axiom from the axiom set.
