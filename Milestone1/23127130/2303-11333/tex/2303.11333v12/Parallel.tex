...
\item $\forall E \in \underline{DA*}: \exists! F \in \underline{*AB*} \bigwedge F \not \in \underline{DA*} \iff \{E, F\} = \underline{C:|CE|} \cap \underline{*AB*}$.
\end{enumerate}
\end{axm}
Immediately from Axiom (strictly monotonic):
\begin{thm}[strictly monotonic]
...
According to Theorem (strictly monotonic), $|AC| < |BC|$ unless $D = E$.
\end{proof}
