...
\begin{abstract}
This paper shows that the 4 axioms in the original Euclidean axiom set can be modernized as 6 axioms.  
...
It is regarded as insufficient \cite{Old and New}\cite{Mistakes}, so it was enhanced by Hilbert axiom set of 21 axioms in 5 groups \cite{Hilbert}.
For common usage, a parallel postulate is added to Euclidean axiom set as the fifth axiom in modern time  \cite{Euclid}\cite{Plane}\cite{Old and New}.
In this paper, Euclidean axiom set does not contain the fifth axiom.

Euclidean axiom set \cite{Euclid} has one mathematical difficulty: the measurement of angles.  
...
Through its Archimedes' axiom \cite{Hilbert}, Hilbert axiom set should have avoided this difficulty by defining the right ratio as proposed in this paper, but it has not, to allow the circular definition of angle measurement.
Perhaps to overcome this difficulty of angle measurement, later Birkhoff axiom set for Euclidean geometry \cite{Birkhoff} requires both distance and angle to be independently measurable, which is different from the common practice to derive angle measurement from distance measurement.  
Also, Birkhoff's axiom set is too simple to be mathematically rigorous.

Both Hilbert's and Birkhoff's axiom sets belong to the second-order logic which is based on relations of point sets \cite{Old and New}. 
...
In this paper, Hilbert axiom set is treated as the standard axiom set for Euclidean geometry.
...
One problem of Hilbert's axiom set \cite{Hilbert} is that it contains a few primitive notions without prior descriptions, such as:
\begin{itemize}
\item 
The axiom \textsl{``I.1. Two distinct points always completely determine a straight line.''}, mentions a straight line without defining it.  

\item 
The axiom \textsl{``I.3. Three points not situated in the same straight line always completely determine a plane.''} hides three primitive notion: dimension, flat plane, and the uniqueness of the flat plane. 
...
Like Hilbert axiom set, Tarski's axiom set states that three noncollinear points determine a 2-dimensional surface, but it lacks the actual definition of the 2-dimensional surface.
...
For example, using a marked ruler, it is now possible to achieve what Euclidean axiom set cannot achieve, such as trisecting an angle \cite{Old and New}.
...
As a mathematical insufficiency, Euclidean axiom set contains no ordering of points in a straight line \cite{Old and New}.
Both Hilbert axiom set and Tarski axiom sets have a few axioms to describe ordered ``betweeness'' of points in a straight line \cite{Old and New}\cite{Hilbert}\cite{Tarski}.
...
However, the first primitive notion in Hilbert axiom set is straight line \cite{Hilbert}, which is infinitive in extend.
The parallel postulate \cite{Hilbert} in Hilbert axiom set, which differentiates Euclidean surfaces from non-Euclidean surfaces \cite{Plane}\cite{Old and New}\cite{Non-Euclidean Geometry}, is also infinitive in extend, so that it cannot be applied locally.
...
Other existing methods in Hilbert axiom set for this purpose such as \textsl{``The sum of the angles of a triangle is two right angles''} \cite{Hilbert} is not truly local because it cannot measure the curvature at a point on a manifold \cite{Differential Geometry}.
...
Besides the trivial Axiom \ref{axm:stable}, Axiom \ref{axm:monotonic} seems an implicit assumption in all the existing axiom sets for Euclidean geometry.
...
The violation of Axiom \ref{axm:stable} is used to detect gravitational waves \cite{Gravitational Waves}.
...
It is also possible to define the distance as another number set other than the whole real number set, such as the construable field \cite{Old and New} for Hilbert axiom set.
...
It is implicit in Hilbert axiom set \cite{Hilbert} and Tarski axiom set \cite{Tarski} by asserting ``congruence'' relation between any two points.
...
Tarski axiom set does not need any geometric object including $\overline{AB}$.
...
As the embodiment of the original axiom \textsl{``To extend a finite straight line continuously in a straight line''} \cite{Euclid} in Euclidean axiom set:
...
When $|CD| \ll |DE|$, $|CE|$ probably increases monotonically with $|DE|$ on a differentiable surface, but in other cases, such monotonic relation is not guaranteed.
...
For example, the monotonic relation belongs to neither congruence relation, nor similarity relation, nor to be covered by Archimedes axiom, nor any other axiom in Hilbert axiom set.
...
The fourth axiom in Euclidean axiom set \cite{Euclid} is \textsl{``All right angles are equal to one another''}. This requirement can be quantified by:
\begin{defn}[flat]
...
Using geometric properties, Axiom \ref{axm:stable} can be applied to Euclidean surface:
...
All axioms of group IV in Hilbert axiom set \cite{Hilbert} are special cases for the general congruence relation defined by Theorem \ref{thm:repeated} and Theorem \ref{thm:reflective}.
...
Through Archimedes Axiom, Hilbert axiom set introduces similarity relations.
...
\item Because the surface is smooth, $\beta = \alpha^2/4$.
...
This paper uses six axioms to establish 2-dimensional Euclidean geometry progressively:
\begin{itemize}
...
Compared with Euclidean axiom set \cite{Euclid}, the new axiom set is mathematically more rigorous, because it is built upon the modern mathematical formal system \cite{Formal System}.  It can be viewed as a modern update of the original axioms:
\begin{itemize}
...
Compared with Hilbert axiom set \cite{Hilbert}, this new set is much simpler, because the new axiom set has much less primitive notions by utilizing real number set to define primitive notions in Hilbert axiom set, and betweeness.  
The parallel postulate, the congruence relation, and the similarity relation are all the consequence of Axiom \ref{axm:flat}.
As shown in this paper, the new axiom set can be conclude each 2-dimensional related axioms in Hilbert axiom set.
