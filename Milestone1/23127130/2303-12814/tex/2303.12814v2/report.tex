...
\begin{abstract}
In this paper, we establish a sharp upper bound on the the number of fixed points a certain class of neural networks can have. The networks under study (autoencoders) can be viewed as discrete dynamical systems whose nonlinearities are given by the choice of activation functions. To this end, we introduce a new class $\F$ of $C^1$ activation functions that is closed under composition, and contains e.g. the logistic sigmoid function. We use this class to show that any 1-dimensional neural network of arbitrary depth with activation functions in $\F$ has at most three fixed points. Due to the simple nature of such networks, we are able to completely understand their fixed points, providing a foundation to the much needed connection between application and theory of deep neural networks.
\end{abstract}
...
Autoencoders have been part of the deep learning literature since the 1980s \citep{Goodfellow-et-al-2016, 1993hinton}. Initially popular in dimensionality reduction and image processing, in the last decade they have been used in speech recognition \citep{2020yang} and as generative models \citep{2014kingma}.

As the name suggests, an {\em autoencoder} is a neural network designed to automatically encode (and decode) important features from a data set. This is achieved by composing an {\em encoder}, $f$, and a {\em decoder}, $g$, where $f$ maps its input to a {\em code} (usually of a lower dimension than the input), and $g$ maps the code back to the dimension of the original input. The composition $g \circ f$ is then trained to approximate the identity function on the input data set. Since the raw data does not need to be labelled, autoencoders are an example of an unsupervised learning model.

Designing an autoencoder involves many decisions: what architecture to use, what loss function and optimisation algorithm to use in training, what activation function to use, and choosing values to assign to other hyperparameters, such as the learning rate. Understanding the effect that these choices have on the dynamics of the training algorithm and the autoencoder itself could greatly assist the design process, leading to more reliable autoencoders in realistic applications.

There are few studies analysing the dynamics of autoencoders, and those that do are largely experimental and less theoretical \citep{2021piotrowski, 2022dacruz}. Understanding the number and nature of the fixed points and periodic points of an autoencoder is fundamental in understanding its long term dynamics.

In this paper, we study the dynamics of very simple autoencoders in the form of neural networks of arbitrary depth, but containing only one neuron in each layer. The complexity of such neural networks is far from that of any autoencoders used in realistic applications. However, limiting our scope to this simpler problem means that we are able to solve it completely. In doing so, we discover a large class $\F$ of $C^1$ functions that is closed under composition, and this paper is largely about understanding this family. One property of this family is that if an element $f$ in $\F$ has two fixed points $x$ and $y$, then the derivatives at these points must satisfy $f'(x)f'(y) \leq 1$, and so we call $\F$ the family of \textit{nowhere coexpanding functions}.

Our main result states that the set of fixed points of any function in $\F$ is either an interval, or has cardinality at most three. Furthermore, we show that the intersection $\F \cap C^3$ is the class of functions with non-positive Schwarzian derivative. Hence, this generalises the work of \citep{MR494306}, which studies the dynamics of $C^3$ functions with negative Schwarzian derivative. For some applications, functions in $\F$ can be {\em glued} together to make new $C^1$ functions in $\F$ which are in general not in $C^3$; we give examples of this in Section~\ref{sec:example}.


\section{Statement of main results and definitions}

To state our main result, we first introduce our function class $\F$ -- a subset of scalar $C^1$ functions with no critical points: $f'(x) \neq 0$ for all $x \in \R$. Unless otherwise stated, all functions in this paper will be of this form.
...
We may generalise this definition to any two distinct points $x, y \in \R$. Let $\Delta =\{(x,x)\in\R^2\}$, and define the function $\chi_f: \R^2 \setminus \Delta \to \R$ as follows:
\begin{equation*}
...
In other words, $x$ and $y$ are coexpanding if there is an affine function $A:\R \to \R$ such that $x$ and $y$ are coexpanding fixed points of $A \circ f$, where an {\bf affine function} is a function $A: \R \to \R$ of the form
\[A(x) \coloneqq a x + b,\]
\noindent
for constants $a, b \in \R$.

If $f$ does not have coexpanding points, it is {\bf nowhere coexpanding}. We now define $\F$ to be the set of all nowhere coexpanding functions:
\begin{equation*}
\F \coloneqq \{f\in C^1(\R, \R) \colon f'(x) \neq 0 \textrm{ and }\chi_f(x,y) \le 1 \textrm{ for all } x\neq y\}.
\end{equation*}

Observe that all non-constant affine functions are in $\F$, since $\chi_A(x, y) = 1$ for any such $A$ and distinct $x, y \in \R$.

Section \ref{sec:coexpanding} shows that $\F$ is closed under composition, and classifies all possible sets of fixed points for functions in $\F$. Let $\mathrm{Fix}(f) \coloneqq \{x \in \mathbb{R}: x = f(x)\}$. Our main result is as follows:

\begin{theorem} \label{thm:fix}
If $f \in \F$, then either $\fix(f)$ is a closed interval, or $f$ has at most three fixed points.
\end{theorem}

A neural network whose layers are all one neuron wide is a composition of two classes of functions: affine functions and non-linear activation functions. 

\sloppy
\begin{corollary} \label{corr:neural_basic}
For any choice of affine functions $A_1, \ldots, A_n$ and activation functions $\sigma_1, \ldots, \sigma_n \in \F$, a function $\Phi$ of the form
\[\Phi \coloneqq A_n \circ \sigma_n \circ A_{n-1} \circ \sigma_{n-1} \circ \cdots \circ \sigma_1 \circ A_1\]
has at most three isolated fixed points.
\end{corollary}
\fussy
This result follows immediately from Theorem~\ref{thm:fix}, where we exclude the possibility of $\fix(\Phi)$ being an interval by demanding that the the fixed points of $\Phi$ be isolated. We include this corollary in order to emphasise the special structure of neural networks that we are ultimately interested in understanding.

One popular choice of activation function is the \textbf{logistic sigmoid function} $\sigma$, defined by
\[\sigma(x) \coloneqq \frac 1 {1 + e^{-x}}.\]
\noindent
An important, yet even more special case of our main result is the following.

\begin{corollary} \label{corr:neural}
For any choice of affine functions $A_1, \ldots, A_n$, a function $\Phi$ of the form
\[\Phi \coloneqq A_n \circ \sigma \circ A_{n-1} \circ \sigma \circ \cdots \circ \sigma \circ A_1\]
has at most three fixed points.
\end{corollary}

To prove Corollary \ref{corr:neural}, it suffices to show that the logistic sigmoid function $\sigma$ is in $\F$ and that $\fix(\Phi)$ has no interior. 

A key observation of this work is that the class $\F$ is a natural generalisation of the Schwarzian derivative (see Section~\ref{sec:schwarzian} for a precise definition). This fact is made clear in the following result:
...
The class of functions having negative Schwartzian derivatives is well studied in the field of Dynamical Systems. Our class $\F$ allows us to work with a larger class of activation functions, of weaker regularity. Nevertheless, in Section \ref{sec:example}, we conclude with an example showing that $\F$ is not closed under addition. This demonstrates that more work is needed to study the fixed points of more general neural networks.
...
In this section, we use Schwarzian derivatives to characterise $C^3$ functions in $\F$. We further show that the logistic sigmoid, $\sigma$ is in $\F$, from which it follows that $\Phi$ in Corollary \ref{corr:neural} is in $\F$. We then use this to prove Corollary \ref{corr:neural}.
...
The following lemma is adapted from Chapter 9.4 of \citep{MR3012659}.
...
With this, the proof of Corollary \ref{corr:neural} is straightforward.

\begin{proof}[Proof of Corollary \ref{corr:neural}]
...
Consider the function $f(x) \coloneqq \tanh(4 x) + \tanh(x/4)$, shown in Figure \ref{fig:tanh_counterexample}.
