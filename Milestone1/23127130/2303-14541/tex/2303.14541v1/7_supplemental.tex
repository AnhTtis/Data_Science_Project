...
\begin{comment}
\subsection{Main Paper Evaluation Details}

Quantitative evaluation reported in the main paper were provided for multiple different runs (resulting in minor differences between our results in Tables 1 and 4); we will average them for a revised version.

Furthermore, ablations provided in the main paper in Tables 2 and 3 were reported for the results of 1 self-training cycle, as the results are expected to carry to the rest of the self-training cycles.
As this may be confusing, we will provide the results of both the first and last self-training cycles (averaged over multiple runs) for a revised version.
\end{comment}
...
We show our method as a strong pretraining strategy in Figure~\ref{fig:limited_data}, illustrating AP@50, with full results in Table~\ref{tab:data_efficient}.
Our approach notably outperforms both training from scratch as well as the state-of-the-art 3D pretraining of CSC.
...
We note that the 3D contrastive pre-training of CSC, similar to other 3D pre-training methods developed for non-transformer backbones \cite{xie2020pointcontrast, hou2021exploring, zhang_depth_contrast,nunes2022segcontrast}, was not beneficial for a transformer-based model. 
A similar observation was also reported in a recent pretraining method \cite{hou2023mask3d}. 
...
Finally, we also ablate the effect of our physical connectivity-based foreground separation introduced in Section \ref{sec:dataset_gen}. In our main method we separate all set of connected components in the foreground, but only keep the component with the highest eigenvector activation (\textit{Max}). As an alternative we also test a method where we calculate the highest average activation in the connected component (\textit{Avg.}), a method where we keep the component with the largest surface value (\textit{Largest}) and finally, to test the effect of this module, without any kind of connectivity-based separation (\textit{No Sep.}). 
...
\While{$j \le max\_inst\_num$}{
  ...
  $ v_{max} = max(\mathbf{v}) $ \\
  $ \mathbf{m}' = sep(\mathbf{v}, v_{max}, \mathcal{C})$ \\
  $M \gets M \cup \{\mathbf{m}'\}$
}
\end{algorithm}
...
We report comparative results of the masked \textit{NCut} algorithm and our FreeMask 3D adaptation after self-training in Table \ref{tab:pseudo_performance} of the main paper and in Table \ref{tab:freemask_vs_ncut} of the initial pseudo mask scores. 
