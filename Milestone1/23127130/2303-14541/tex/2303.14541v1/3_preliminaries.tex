...
We first introduce the recent methodologies in unsupervised 2D instance segmentation and revisit the Normalized Cut algorithm \cite{shi2000normalized_cut} used for our pseudo mask generation. 

\paragraph{Self-training for 2D unsupervised instance segmentation}

Recent advances in 2D unsupervised instance segmentation have used two-stage training pipelines to achieve remarkable segmentation results \cite{wang2022freesolo,wang2023cut}.
These methods first generate a set of coarse pseudo masks and then refine them with a series of self-training iterations. 
%
In particular, FreeSolo~\cite{wang2022freesolo} uses multi-branch feature extraction to obtain self-similar regions as mask proposals. 
This results in a dense set of initial pseudo-annotated instances.
%
CutLER~\cite{wang2023cut} then use the normalized cut (NCut) algorithm~\cite{shi2000normalized_cut} with deep self-supervised features from DINO~\cite{caron2021emerging_dino} to identify multiple prominent regions as pseudo masks.  

Inspired by such approaches, we also develop a two-stage pipeline for unsupervised 3D instance segmentation.
We employ the principle of the NCut approach for pseudo-generation, and lift it to support high-resolution 3D segmentation by operating on segment-level geometric primitives from self-supervised 2D and 3D features.


\paragraph{Normalized cut algorithm}
...
Normalizing the cut cost function with the size of the partitions can solve the problem of single outlier node removal, which is one of the biggest difficulties of other graph cut algorithms.
