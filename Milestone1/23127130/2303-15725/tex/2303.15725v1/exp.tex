...
\end{align*}
where the first step follows from the differential chain rule, and the second step follows from $\frac{ \d ( \exp(Ax) - b ) }{ \d t } = \exp(Ax) \circ \frac{A \d x }{ \d t}$ in Part 1.
...
Then we have
    \begin{align*}
        \| H(x) - H(y) \| \leq \exp(6 R^2) \cdot \| x - y \|_2
    \end{align*}
\end{lemma}
\begin{proof}
...
, the second step follows from Fact~\ref{fac:vector_norm}, the third step follows from $\|x\|_2 \leq R,\|y\|_2 \leq R$, the fourth step follows from $\|b\|_2 \leq R$, the last step follows from $R \geq 2$.
...
\end{align}
where the first step follows  from $\| A (y-x) \|_{\infty} < 0.01$ and Fact~\ref{fac:vector_norm},  the fourth step follows from Fact~\ref{fac:vector_norm}, the fifth step follows from Fact~\ref{fac:matrix_norm}, the last step follows from $\|A\| \leq R$.
