...
\end{align*}
where the first step follows from the differential chain rule, and the second step follows from $\frac{ \d ( \sinh(Ax) - b ) }{ \d t } = \cosh(Ax) \circ \frac{A \d x }{ \d t}$ in Part 1.
...
, the second step follows from Fact~\ref{fac:vector_norm}, the third step follows from $\| \cdot \|_{\infty} \leq \| \cdot \|_2$ (Fact~\ref{fac:vector_norm}),  the fourth step follows from Fact~\ref{fac:vector_norm}, the fifth step follows from $\| x \|_2, \| y \|_2 \leq R$ , the sixth step follows from $\|b\|_2 \leq R$, the seventh step follows from $R \geq 2$.
%\Zhihang{I modified the reasons above a little bit, please double check.} \ZHao{I re-polished again. Please check.}
...
\end{align}
where the first step follows  from $\| A (y-x) \|_{\infty} < 0.01$ and Fact~\ref{fac:vector_norm}, the second step follows from Fact~\ref{fac:vector_norm}, the third step follows from Fact~\ref{fac:vector_norm}, the forth  step follows from Fact~\ref{fac:matrix_norm}, the last step follows from $\|A\| \leq R$.
%\Zhihang{I added the reasons for step 2-4, please double check.}  \Zhao{Sure, I re-polished the reason.}
