...
In this paper, we treat the key matrix as a learnable parameter. During training, our primary goal is to tackle the issue of multiple attention regression by utilizing the exponential function and its corresponding equation: $\min_{X} \|D^{-1} \exp(AX) - B \|_2$, where $D^{-1}$ denotes the normalization factor.
