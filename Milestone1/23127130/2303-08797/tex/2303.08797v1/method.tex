...
\begin{equation}
    ...
    \begin{aligned}
        ...
        \end{aligned}
\end{equation}
\item $x_0$,  $x_1$, and $z$ are random variables drawn independently from $\rho_0$, $\rho_1$, and ${\sf N}(0,\text{\it Id})$, respectively;
\item $\gamma: [0,1] \to \RR $  satisfies: $\gamma(0)=\gamma(1) = 0$; $\gamma(t) > 0$ for all $t \in (0, 1)$; $\gamma\in C^2((0,1))$; and $\gamma^2\in C^1([0,1])$
\end{enumerate}
\end{definition}
Eq.~\eqref{eq:bound:dI} requires that $I(t, x_0, x_1)$ does not move too fast along the way from $x_0$ at $t=0$ to $x_1$ at $t=1$, and as a result does not wander too far from either of these endpoints -- this assumption is made for convenience and is not necessary for most arguments below.
...
In Section~\ref{sec:practical} we discuss how to design the  stochastic interpolant in~\eqref{eq:stochinterp} and give some of the properties of the corresponding process~$x_t$. Examples of stochastic interpolants are also shown in Figure~\ref{fig:example:xt} for various choices of $I$ and $\gamma$ in~\eqref{eq:stochinterp}.

\begin{remark}[Comparison with~\cite{albergo2023building}]
    The difference between the stochastic interpolant defined in~\eqref{eq:stochinterp} and the one originally introduced in~\cite{albergo2023building} is the inclusion of the latent variable $\gamma(t) z$. The objective of the present paper is to elucidate the advantages that this additional term provides. We note that we could generalize the construction by making $\gamma(t)$ a tensor; here we focus on the scalar case for simplicity.
\end{remark}
...
\begin{figure}[t!]
    ...
    \caption{A illustration of the flexibility of designing stochastic interpolants geared toward one's aims. All examples show one realization of $x_t$ with one $x_0\sim\rho_0$, and one $x_1\sim \rho_1$ (the flowers at the left and right of the figures) and one $z\sim {\sf N}(0,\Id )$. \textit{Top}: interpolating with no latent variable, which reduces to the method of \cite{albergo2023building}. \textit{Upper middle}: interpolating with the latent variable included, giving a straightforward way of performing image-to-image translation with diffusive processes. \textit{Lower middle}: interpolating with Gaussian encoding-decoding, i.e. turning the signal into noise at mid-point. \textit{Bottom}: one-sided interpolant which reduces to the Gaussian latent variable, with connections to score-based diffusion models. }
...
\end{equation}
where $x_t$ is defined in~\eqref{eq:stochinterp} and the expectation  is taken independently over $x_0\sim\rho_0$, $x_1\sim\rho_1$, and $z\sim {\sf N}(0,\text{\it Id})$.
...
$$
and in this notation we also have $\rho(t,x) = \EE\delta(x-x_t)$.}, as described in the following definition.

\begin{definition}
    \label{def:cond:expect}
    Given any $f\in C^\infty_0([0,1]\times \RR^d\times \RR^d\times\RR^d)$, its conditional expectation $\EE\left(f(t,x_0,x_1,z)| x_t=x\right)$ is the function of~$x$ such that
\begin{equation}
    \label{eq:cond:e}
    \int_{\RR^d} \EE\left(f(t,x_0,x_1,z)| x_t=x\right) \mu(t,dx) = \EE f(t,x_0,x_1,z)
\end{equation}
where $\mu(t,dx)$ is the time-dependent distribution of $x_t$ defined by~\eqref{def:cond:expect}, and the expectation on the right-hand side is taken independently over $x_0\sim \rho_0$, $x_1\sim\rho_1$, and $z\sim {\sf N}(0,\text{\it Id})$.
\end{definition}
Vector-valued functions have conditional expectations that are define analogously.

\begin{remark}
    Another seemingly more general way to define the stochastic interpolant is via
\begin{equation}
    \label{eq:stoch:interp:gen}
    x'_t = I(t,x_0,x_1)+ N_t
\end{equation}
where $N: [0,1]\to \RR^d$ is a zero-mean Gaussian stochastic process constrained to satisfy $N_{t=0}=N_{t=1}=0$. As we will show below, our construction only depends on the single-time properties of~$N_t$, which are completely specified by $\EE [Z_t Z^\T_t]$. That is, if we take $\gamma(t)$ in~\eqref{eq:stochinterp} such that $\EE [N_t N^\T_t] = \gamma^2(t)\Id$, then the probability distribution of $x_t$ will coincide with that of $x'_t$ defined in~\eqref{eq:stoch:interp:gen}, $x_t \stackrel{\text{d}}{=} x'_t$, which is all that matters in our construction.
For example, taking $\gamma(t) = \sqrt{t(1-t)}$ in~\eqref{eq:stochinterp} -- a choice we will consider below -- is equivalent to choosing $N_t$ to be a Brownian bridge in~\eqref{eq:stoch:interp:gen}, i.e. the stochastic process realizable in terms of the Wiener process $W_t$ as $N_t = W_t - t W_1$. This observation will also help us draw an analogy between our approach and the construction used in score-based diffusion models. As we will show below, it is simpler for both analysis and practical implementation to work with the definition~\eqref{eq:stochinterp} for $x_t$.
\end{remark}

To proceed, we will also make the following assumption on the PDFs $\rho_0$, $\rho_1$, and their relation to the function~$I$:
\begin{assumption}
\label{as:rho:I}
...
The probability distribution of the stochastic interpolant $x_t$ defined in~\eqref{eq:stochinterp} is absolutely continuous with respect to the Lebesgue measure at all times $t\in[0,1]$ and has a time-dependent probability density function (PDF) $\rho(t)$ that satisfies $\rho(0) = \rho_0$, $\rho(1)=\rho_1$, $\rho \in C^1([0,1];C^p(\RR^d))$ for any $p\in\NN$, and $\rho(t, x) > 0$ for all $(t, x) \in [0, 1]\times \RR^d$. In addition, $\rho$ solves the transport equation
\begin{equation}
    \label{eq:transport}
...
\begin{equation}
    \label{eq:b:ode:def}
    b_\ODE(t,x) = \EE ( \partial_t I(t,x_0,x_1) + \dot \gamma(t) z| x_t = x).
\end{equation}
...
\end{equation}
where $x_t$ is defined in~\eqref{eq:stochinterp} and the expectation $\EE$ is taken independently over $x_0\sim \rho_0$, $x_1\sim\rho_1$, and $z\sim {\sf N}(0,\text{\it Id}).$
...
The proof of Theorem~\ref{thm:score} is given in Appendix~\ref{app:proof:interpolate}. Having access to the score immediately allows us to rewrite the TE~\eqref{eq:transport} as forward and backward Fokker-Planck equations, which we state as:

\begin{restatable}[Fokker Planck equations]{corollary}{interpolationfpe}
\label{prop:interpolate_fpe}
For any $\eps\ge 0$, the probability density $\rho$ specified in Theorem~\ref{prop:interpolate} satisfies:
\begin{enumerate}[leftmargin=0.15in]
...
\begin{equation}
    \label{eq:b:def}
    b_\fwd(t,x) = b_\ODE(t,x) + \eps s(t,x).
\end{equation}
...
\begin{equation}
    \label{eq:b:r:def}
    b_\rev(t,x) = b_\ODE(t,x) - \eps s(t,x).
\end{equation}
...
\begin{remark}
    If we set $\gamma(t)=0$ in $x_t$ (i.e we remove the latent variable), the stochastic interpolant~\eqref{eq:stochinterp} reduces to the one originally considered in~\cite{albergo2023building}. In this setup, the results above formally stand except that we cannot guarantee the spatial regularity of $b_\ODE(t,x)$ and $s(t,x)$ in general, since this regularity relies on the presence of the latent variable (as shown in the proof of Theorem~\ref{prop:interpolate}). Therefore,  the introduction  latent variable $\gamma(t) z$ will help at the level of generative models  (see Section~\ref{sec:generative}), where the solution to ODE/SDE will be better behaved, and at the level of statistical approximation, since the target $b$ and $s$ will be more regular. We will see in Section~\ref{sec:practical} that it also gives us much greater flexibility in the way we can bridge $\rho_0$ and $\rho_1$ and therefore design generative models with appealing properties.
\end{remark}
\begin{remark} 
    We will see in Section~\ref{sec:likelihood_bounds} that the forward and backward FPE in~\eqref{eq:fpe} and \eqref{eq:fpe:tr} are more robust  than the TE in~\eqref{eq:transport} against approximation errors in the velocity $b_\ODE$ and the score $s$, which has practical implications for the generative models based on these equations. 
\end{remark}
...
\begin{remark}
    The objectives in~\eqref{eq:obj:v} and \eqref{eq:obj:s} (as well as the one in~\eqref{eq:obj:vv}) are amenable to empirical estimation if we have samples from $\rho_0$ and $\rho_1$, since in that case we can easily generate samples of $x_t= I(t,x_0,x_1) + \gamma(t) z$ at any time $t\in[0,1]$. We will use this feature in the numerical experiments presented below.
\end{remark}
\begin{remark}
    Since $s$ is the score of $\rho$, an alternative objective to estimate it is~\cite{hyvarinen05a} 
\begin{equation}
    \label{eq:ob:w:alt}
     \int_0^1 \EE\left(|\hat s(t,x_t)|^2 +2\nabla \cdot \hat s(t,x_t) \right)dt.
\end{equation}
The derivation of~\eqref{eq:ob:w:alt} is standard:
for the reader's convenience we recall it at the end of Appendix~\ref{app:proof:interpolate}. The advantage of using~\eqref{eq:obj:s} over~\eqref{eq:ob:w:alt} is that it does not requires us to take the divergence of $\hat s$.
\end{remark}
...
\end{equation}
solved backward in time from the final data~$X^\rev_{t=1}\sim\rho_1$ independent of $W^\rev$.
\end{enumerate}
\end{restatable}

\paragraph{Reverse It\^o Calculus.}
Here, the solution of the reverse-time SDE~\eqref{eq:sde:R} is by definition $X^\rev_t= Z^\fwd_{1-t}$ where $Z^\fwd_t$ satisfies
\begin{equation}
    \label{eq:sde:R:Y}
        dZ^\fwd_t = -b_\rev(1-t,Z^\fwd_t)dt+ \seps  dW_t,
\end{equation}
solved forward in time from the initial data~$Z^\fwd_{t=0}\sim \rho_1$ independent of $W$. To avoid repeated application of the transformation $t\to1-t$, it is convenient to work with \eqref{eq:sde:R} directly using
\begin{enumerate}
...
\end{equation}
    \item For any $g\in C^0([0,1];(C_0^0(\RR_d))^d)$ and $t\in[0,1]$, the following backward It\^o isometries hold:
\begin{equation}
    \label{eq:ito:iso}
    \begin{aligned}
...
\begin{remark}
The generative models based on $X_t$ to the ODE~\eqref{eq:ode:1}, and the solutions $X^\fwd_t$ and $X^\rev_t$ of the forward and backward SDEs~\eqref{eq:sde:1} and \eqref{eq:sde:R} will typically involve drifts $b$, $b_\fwd$, and $b_\rev$ that are, in practice, imperfectly estimated via minimization of~\eqref{eq:obj:v} and~\eqref{eq:obj:s}. It is therefore important to estimate the error such imperfections induces, and how they depend on the generative model used, which is the object of our next section.
\end{remark}

\subsection{Likelihood control}
\label{sec:likelihood_bounds}
In this section, we demonstrate that jointly minimizing the objective functions~\eqref{eq:obj:vv} and~\eqref{eq:obj:s} (or minimizing the single loss~\eqref{eq:obj:v}) minimizes the $\mathsf{KL}$-divergence from the target density $\rho_1$ to the model density $\hat{\rho}_1$. The derivation is based on a simple and exact characterization of the $\mathsf{KL}$-divergence between two transport equations or two Fokker-Planck equations with different drifts. Remarkably, we find that the presence of a diffusive term determines whether or not it is sufficient to learn the drift to control $\mathsf{KL}$. This can be seen as a generalization of the result for score-based diffusion models described in~\cite{song2021mle} to arbitrary generative models described by ODEs or SDEs. The proofs of the statements in this section are provided in Appendix~\ref{app:proof:kl}.
...
\end{equation}
\end{restatable}
Lemma~\ref{lemma:kl_fpe} shows that, unlike for transport equations, the $\mathsf{KL}$-divergence between the solutions of two Fokker-Planck equations is controlled by the error in their drifts. The diffusive term in each Fokker-Planck equations provides an additional negative term in the $\mathsf{KL}$-divergence, which eliminates the need for explicit control on the Fisher divergence. 
...
\label{thm:kl:bound}
Let $\rho$ denote the solution of the Fokker-Planck equation~\eqref{eq:fpe}. Given two velocity fields $\hat{b}, \hat{s} \in C^0([0,1], C^1(\RR^d, \RR^d))$, define 
\begin{equation}
    \label{eq:bhat:vhat}
    \hat b_\fwd(t,x) = \hat b(t,x) + \eps \hat s(t,x), \qquad \hat v(t,x) = \hat b(t,x) + \gamma(t) \dot \gamma(t) \hat s(t,x)
\end{equation} 
where the function $\gamma$ satisfies the propoerties listed in Definition~\ref{def:interp}. Let $\hat \rho$ denote the solution to the Fokker-Planck equation
\begin{equation}
     \partial_t \hat \rho + \nabla \cdot (\hat b_\fwd\hat \rho) = \eps  \Delta \hat \rho, \qquad \hat \rho(0) = \rho_0.
\end{equation}
Then,
...
\end{equation}
so that $\eps^* > 1$ if the score is learned to higher accuracy than $\hat{b}$ and  $\eps^*<1$ in the opposite situation. Note that \eqref{eq:optimal:eps} suggests to take $\eps= 0$ if $\hat{b}$ is learned perfectly but $\hat s$ is not, and send $\eps \to \infty$ in the opposite situation. We stress however that, while taking $\eps=0$ is achievable in practice and simply leads to the ODE~\eqref{eq:ode:1}, taking $\eps\to\infty$ is not, as increasing $\eps$  speeds-up the time scales in the SDE~\eqref{eq:sde:1} and~\eqref{eq:sde:R}, thereby making their numerical integration increasingly costly as the time-step must be decreased inverse-proportionally to $\eps$. 
\end{remark}

\subsection{Density estimation and cross-entropy calculation}
\label{sec:density}

It is well-known that the solution of the TE~\eqref{eq:transport} can be expressed in terms of the solution to probability flow ODE~\eqref{eq:ode:1}. Specifically we have:
...
\end{restatable}
This lemma is proven in Appendix~\ref{app:prrof:de}.  Interestingly, we have similar results for the solution of the forward and backward FPE in~\eqref{eq:fpe} and~\eqref{eq:fpe:tr}. These results use auxiliary forward and backward SDE in which the role of the forward and backward drifts are switched:
...
\end{equation}
and let $Y^\fwd_t$ and $Y^\rev_t$ be the solution of the following forward and backward SDE:
\begin{equation}
    \label{eq:sde:y:1}
...
\end{equation}
to be solved backward in time from the final condition~$Y^\rev_{t=1}=x$ independent of $W^\rev$. Then, given the PDFs $\rho_0$ and $\rho_1$:
    \begin{enumerate}[leftmargin=0.15in]
\item The solution to the forward FPE
\begin{equation}
    \label{eq:fpe:f:hat}
    \partial \hat \rho_\fwd + \nabla \cdot (\hat b_\fwd \hat \rho_\fwd ) = \eps \Delta \hat \rho_\fwd , \qquad \hat \rho_\fwd (0) = \rho_0,
\end{equation}
...
\begin{equation}
    \label{eq:sde:h:R}
    d\hat X^\rev_t = b_\rev(t,\hat X^\rev_t)dt  + \seps  dW^\rev_t, \quad W_t^\rev = -W_{1-t},
\end{equation}
...
The results of Lemma~\ref{lem:tesol} and Theorem~\ref{prop:sde:rho} can then be used to test the quality of  samples generated either by the  ODE~\eqref{eq:ode:1}, or by the forward and backward SDE~~\eqref{eq:sde:1} and~\eqref{eq:sde:R}, using the cross entropy as  measure.  In particular, the following two results are direct consequences of Lemma~\ref{lem:tesol} and Theorem~\ref{prop:sde:rho}, respectively:

\begin{restatable}{corollary}{crossentode}
    \label{prop:ce:ode}
    In the same conditions as in Lemma~\ref{lem:tesol}, if $\hat \rho(0) = \rho_0$, the cross-entropy of $\hat \rho(1)$ relative to $\rho_1$ is given by
\begin{equation}
    \label{eq:ce:ode}
    \begin{aligned}
   ...
   \end{aligned}
\end{equation}
where $\EE_1$ denotes the expectation on $x_1\sim\rho_1$. Similarly, if $\hat \rho(1) = \rho_1$, the cross-entropy of $\hat \rho(0)$ relative to $\rho_0$ is given by
\begin{equation}
    \label{eq:ce:ode:r}
    \begin{aligned}
    H(\rho_0|\hat\rho(0)) &= - \int_{\RR^d} \log \hat\rho(0,x) \rho_0(x) dx\\
   & = -\EE_0 \int_0^1 \nabla \cdot b_{\ODE}(\tau, X_{0,\tau}(x_0)) d\tau - \EE_0 \log \rho_1( X_{0,1}(x_0))
   \end{aligned}
\end{equation}
 where $\EE_0$ denotes the expectation on $x_0\sim\rho_0$.
\end{restatable} 

\begin{restatable}{corollary}{crossentsde}
    \label{prop:ce:sde}
    In the same conditions as in Theorem~\ref{prop:sde:rho}, the cross-entropy of $\hat \rho_\fwd(1)$ relative to $\rho_1$ is given by
\begin{equation}
    \label{eq:ce:sde}
    \begin{aligned}
    H(\rho_1|\hat \rho_\fwd(1)) &= - \int_{\RR^d} \log \hat \rho_\fwd(1,x) \rho_1(x) dx\\
   & = -\EE_1 \log \EE_\rev^{x_1} \left(\exp\left(- \int_0^1\nabla \cdot b_\fwd(t, Y^\rev_t) dt\right)  \rho_0(Y_{t=0}^\rev)\right),
   \end{aligned}
\end{equation}
where  $\EE_\rev^{x_1}$ denotes the expectation on $Y^\rev_t$ conditional on $Y^\rev_{t=1}=x_1$, and $\EE_1$ denotes the expectation on $x_1\sim\rho_1$.
Similarly, the cross-entropy of $\hat \rho_\rev(0)$ relative to $\rho_0$ is given by
\begin{equation}
    \label{eq:ce:sde:r}
    \begin{aligned}
    H(\rho_0|\hat \rho_\rev(0)) &= - \int_{\RR^d} \log \hat \rho_\rev(0,x) \rho_0(x) dx\\
   & = -\EE_0 \log \EE_\fwd^{x_0} \left(\exp\left( \int_0^1\nabla \cdot b_\rev(t, Y^\fwd_t)  dt\right)\rho_1(Y^\fwd_{t=1})\right),
   \end{aligned}
\end{equation}
where $\EE_\rev^{x_0}$ denotes the expectation on $Y^\fwd_t$ conditional on $Y^\fwd_{t=0}=x_0$, and $\EE_0$ denotes the expectation on $x_0\sim\rho_0$.
  
\end{restatable}

If in~\eqref{eq:ce:ode}, \eqref{eq:ce:ode:r}, \eqref{eq:ce:sde}, and \eqref{eq:ce:sde:r}  we approximate the expectations $\EE_0$ and $\EE_1$  over $\rho_0$ and $\rho_1$ by empirical expectations over  data sets from these densities, these equations allow us to cross-validate different approximations of $\hat b$ and $\hat s$. They also allow us to compare  the cross-entropies of densities evolved by the TE~\eqref{eq:TE:hat}, or by the forward and backward FPE~\eqref{eq:fpe:f:hat} and~\eqref{eq:fpe:r:hat}.

\begin{remark}
    When using  \eqref{eq:ce:sde} and \eqref{eq:ce:sde:r} in practice, taking the log of the expectations $\EE_\rev^{x_1}$ and $\EE_\fwd^{x_0}$ may create difficulties. For example, if we want to use Hutchinson's trace estimator to compute the divergences of $b_\fwd$ and $b_\rev$, this will introduce a bias.  One way to remove this bias is to use Jensen's inequality to exchange the log and the expectation and get the upper bounds
\begin{equation}
    \label{eq:ce:sde:bound}
    \begin{aligned}
    H(\rho_1|\hat \rho_\fwd(1)) \le  \int_0^1 \EE_1 \EE_\rev^{x_1}\nabla \cdot b_\fwd(t, Y^\rev_t) dt - \EE_1 \EE_\rev^{x_1} \log  \rho_0(Y_{t=0}^\rev),
   \end{aligned}
\end{equation}
and
\begin{equation}
    ...
    \begin{aligned}
    H(\rho_0|\hat \rho_\rev(0)) \le -\EE_0 \EE_\fwd^{x_0} \int_0^1\nabla \cdot b_\rev(t, Y^\fwd_t)  dt- \EE_0 \EE_\fwd^{x_0} \log \rho_1(Y^\fwd_{t=1}).
   \end{aligned}
\end{equation}
It should be stressed, however, that these bounds are not sharp in general. In fact, using calculations similar to the one presented in the proof of Theorem~\ref{prop:ce:sde}, we can derive the exact expressions
\begin{equation}
    \label{eq:ce:sde:exact}
    \begin{aligned}
    H(\rho_1|\hat \rho_\fwd(1))=   \int_0^1 \EE_1 \EE_\rev^{x_1}\left(\nabla \cdot b_\fwd(t, Y^\rev_t) -\eps |\nabla \log \hat \rho_\fwd(t,Y_t^\rev)|^2\right)  dt - \EE_1 \EE_\rev^{x_1} \log  \rho_0(Y_{t=0}^\rev),
   \end{aligned}
\end{equation}
and
\begin{equation}
    ...
    \begin{aligned}
    H(\rho_0|\hat \rho_\rev(0)) = -\EE_0 \EE_\fwd^{x_0} \int_0^1\left(\nabla \cdot b_\rev(t, Y^\fwd_t) +\eps |\nabla \log \hat \rho_\rev(t,Y_t^\fwd)|^2\right) dt- \EE_0 \EE_\fwd^{x_0} \log \rho_1(Y^\fwd_{t=1}).
   \end{aligned}
\end{equation}
Unfortunately, since $\nabla \log \hat\rho_\fwd \not= \hat s$ and $\nabla \log \hat\rho_\rev \not= \hat s$ in general due to approximation errors, we do not know how to estimate the extra terms at the right hand side of~\eqref{eq:ce:sde:exact} and \eqref{eq:ce:sde:r:exact}. A possibility is to use~$\hat s$ as proxy to approximate $\nabla \log \rho_\fwd$ and $\nabla \log \rho_\rev $ but this approximation is uncontrolled. 
