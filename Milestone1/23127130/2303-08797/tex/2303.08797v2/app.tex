...
\end{equation}
by Assumption~\ref{as:rho:I}. As a result, the integral in~\eqref{eq:v:L2} is continuous at $t=0$ and $t=1$, $b$ must be integrable on $[0,1]$, and~\eqref{eq:bt:bounded} holds. 
\end{proof}
...
\begin{equation}
    \label{eq:EL:s}
    \begin{aligned}
     \mathcal{L}_s[\hat s] &= \int_{\RR^d} \left( \tfrac12|\hat s(t,x)|^2 + \gamma^{-1}(t)\EE\left(z|x_t=x\right)\cdot \hat s(t,x)\right) \rho(t,x) dx,\\
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where we used $\EE (z| y_t = x) = - \beta(t) s(t,x) $ from \eqref{eq:obj:v:os} as well as the definition of~$D(t)$ in~\eqref{eq:sig:def}. This means that the density of $y_t$ satisfies
\begin{equation}
    \label{eq:pdf:lin:os}
