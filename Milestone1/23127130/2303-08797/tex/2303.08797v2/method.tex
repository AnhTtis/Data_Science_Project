...
Vector-valued functions have conditional expectations that are defined analogously.
...
To proceed, we will make the following assumption on the densities $\rho_0$, $\rho_1$, and their relation to the function~$I$:
\begin{assumption}
\label{as:rho:I}
...
In Section~\ref{sec:generative} we will use the results of this theorem to design generative models based on forward and backward stochastic differential equations. Note that if we define $\rho_\rev(t_\rev,x) = \rho(1-t_\rev,x)$, the reversed FPE~\eqref{eq:fpe:tr} can be written as
\begin{equation}
    \label{eq:fpe:tr:r}
    \partial_{t_\rev} \rho_\rev({t_\rev},x) - \nabla \cdot \left(b_\rev(1-{t_\rev},x)\rho_\rev({t_\rev},x)\right) = \eps  \Delta \rho_\rev({t_\rev},x), \qquad \rho_\rev({t_\rev}=0) = \rho_1,
\end{equation}
which is now well-posed forward in (reversed) time~$t_\rev$. To have only one definition of time~$t$ it is more convenient to work with~\eqref{eq:fpe:tr}.  
...
To avoid repeated applications of the transformation $t\mapsto1-t$, it is convenient to work with \eqref{eq:sde:R} directly using the reversed It\^o calculus rules stated in the following lemma, proven in Appendix~\ref{app:proof:generative}:
...
\begin{equation}
    ...
    df(t,X^\rev_t) = \partial_t f(t,X^\rev_t) dt+ \nabla f(X^\rev_t) \cdot dX^\rev_t - \eps \Delta f(t,X^\rev_t) dt.
\end{equation}
...
\label{thm:kl:bound}
Let $\rho$ denote the solution of the Fokker-Planck equation~\eqref{eq:fpe}. Given two velocity fields $\hat{b}, \hat{s} \in C^0([0,1], (C^1(\RR^d))^d)$, define 
\begin{equation}
    \label{eq:bhat:vhat}
