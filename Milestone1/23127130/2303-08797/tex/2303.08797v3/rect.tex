...
\end{equation}
We can use the map $X_{t=1}: \RR^d \to \RR^d$ to define a new stochastic interpolant with $z\sim {\sf N} (0,\Id)$
\begin{equation}
    \label{eq:new:os}
    x^\REC_t = \alpha(t) z + \beta(t) X_{t=1}(z),
\end{equation}
where $\alpha^2,\beta\in C^2([0,1])$ satisfy $\alpha(0)=\beta(1) = 1$, $\alpha(1) = \beta(0) = 0$, and $\alpha(t) >0$ for all $t\in[0,1)$.
%
Clearly, we then have $x^\REC_{t=0} = z\sim \rho_0$ since $X_{t=0}(z)=z$ and $x^\REC_{t=1} = X_{t=1}(z) \sim \rho_1$ by definition of the probability flow equation. 
%
We can define a new probability flow equation associated with the velocity field
\begin{equation}
    \label{eq:b:rec}
    b^\REC(t,x) =  \EE[ \dot x^\REC_t | x^\REC_t =x] = \dot\alpha(t) \EE[z | x^\REC_t =x] + \dot\beta(t) \EE[X_{t=1}(z)| x^\REC_t =x].
\end{equation}
It is easy to see that this velocity field is amenable to estimation, since it is the unique minimizer of 
\begin{equation}
    \label{eq:obj:brec}
    \mathcal L_{b^\REC}[\hat b^\REC] = \int_0^1 \EE \big[ \tfrac12 |\hat b^\REC(t,x^\REC_t) |^2 - (\dot\alpha(t) z + \dot\beta(t) X_{t=1}(z)) \cdot \hat b^\REC(t,x^\REC_t)\big] dt,
\end{equation}
where $x^\REC_t$ is given in~\eqref{eq:new:os} and the expectation is now only on $z\sim {\sf N}(0,\Id)$. Our next result show that the probability flow equation associated with the velocity field \eqref{eq:b:rec} has straight line solutions, but ultimately it leads to a generative model that is identical to the one based on~\eqref{eq:ode:1:os:x0}. 
%
...
\begin{restatable}{theorem}{rec}
    ...
    \end{equation}
    Then, all solutions are such that $X^\REC_{t=1}(z)\sim \rho_1$ if $z\sim {\sf N}(0,\Id)$. In addition, 
...
\begin{remark}[Consistency models]
    Recent work has introduced the notion of \textit{consistency models}~\cite{song_consistency_2023}, which distill a velocity field learned via score-based diffusion into a single-step map.
    %
