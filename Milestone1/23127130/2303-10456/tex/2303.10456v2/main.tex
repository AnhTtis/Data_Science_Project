...
In addition, we examine the example by Leditzky \etal
...
%
we show that our capacity formula is exhausted with a finite-dimensional  ancilla, and in Section~\ref{app:etMAC}, we prove the capacity theorem.
...
As an example, we consider the channel that was introduced by Leditzki \etal \cite{LeditzkyAlhejjiLevinSmith:20p}.
...
\end{itemize}
Leditzki \etal \cite{LeditzkyAlhejjiLevinSmith:20p} defined a MAC such that the channel is ideal for input strategies that win the game, and pure-noise otherwise. 
The precise definition is given below.
...
%
Leditzki \etal \cite{LeditzkyAlhejjiLevinSmith:20p} defined a classical MAC $P_{Y|X_1,X_2}$, with
\begin{align}
...
In general, every distribution $p(i,j)$ that is obtained from local measurements on a state $\ket{\phi_{A_1 A_2}}$ can also be obtained by 
...
$\overline{P}_e^{(n)}(\code)< \frac{1}{9}\varepsilon_0$, for a sufficiently large $n$. Based on the considerations in \cite{Cai:14p}, it follows that the rate pair $(R_1,R_2)$ is also achievable with a maximal error criterion, \ie, such that $P_e^{(n)}(\code)\leq \varepsilon_0$.
...
Furthermore, since \mbox{Alice 1} performs a measurement that depends on her message $m_1$ alone,
the channel input $X_{1}[i]$ can be obtained as the outcome of a measurement  
...
We have also  shown that the  example by Leditzky \etal \cite{LeditzkyAlhejjiLevinSmith:20p} is a special case and follows from our main result in 
Theorem~\ref{theo:etMAC}.

We have established that our capacity formula is
a single-letter formula,  involving auxiliary variables and ancillas of finite dimensions.
As a consequence, we obtained a sufficient entanglement rate to achieve the capacity region.
%
...
$\Mset_{2}'$ is at least as large as half the original set, \ie
\begin{align}
