...
such that  sharing entanglement between transmitters strictly increases the achievable sum rate (see also \cite{Notzel:20p,NotzelDiAdamo:20c}).
...
%
the transmitters can now communicate with each other over either classical or quantum noise-free links at a fixed limited rate. 
...
\leq  2^{nC_{12}}
\text{ , and }\;
|\Omega^{T}_{2\to 1}|\leq  2^{nC_{21}}
%
%
%
\label{eq:Permissible}
\end{align}
where $\Omega^{T}_{k\to j}=%
...
\sum_{y^n: g(y^n)\neq (m_1,m_2)} 
  ...
  f(x_1^n,x_2^n|m_1,m_2,s_{1\to 2}^T(m_1,m_2),s_{2\to 1}^T(m_1,m_2))
\\ \cdot
P_{Y|X_1,X_2}^n(y^n|x_1^n,x_2^n) %
\end{multline}
for $(m_1,m_2)\in [1:2^{nR_1}]\times [1:2^{nR_2}]$, where $\omega_{1\to 2}^T\equiv s_{1\to 2}^T(m_1,m_2)$ and $\omega_{2\to 1}^T\equiv s_{2\to 1}^T(m_1,m_2)$ are the $T$-sequences of conferencing messages, sent from Alice 1 to Alice 2, and from Alice 2 to Alice 1, respectively.
...
\begin{itemize}
\item
an entangled state $\Psi_{E_1 E_2 \bar{E}_1 \bar{E}_2}\in\mathscr{D}(\Hset_{E_1 E_2 \bar{E}_1 \bar{E}_2})$ that is shared between the transmitters.
\item 
two message sets  $[1:2^{nR_1}]$ and $ [1:2^{nR_2}]$, %
\item 
...
The senders share the entangled pair of 
...
\begin{remark}
We focus here on two conferencing models. In the discussion section, we will consider other variations  (see Subsection~\ref{Subsection:Open}). 
\end{remark}
...
\end{multline}
The subscript `ET' stand for entangled transmitters.
...
%
 ...
 of the region $\mathcal{R}^{\text{in}}_{\text{ET}}(P_{Y|X_1,X_2})$  %
%
for a given channel.
\begin{lemma}
\label{lemm:CardCl}
The union of the inner bound in (\ref{eq:inRetx_In}) is exhausted by %
...
%
\end{lemma}
The proof of Lemma~\ref{lemm:CardCl} is based on the Fenchel-Eggleston-Carath\'eodory lemma \cite{Eggleston:66p}, using similar arguments as 
...
We note that the dimension of the entangled systems $A_1$ and $A_2$ in the rate formula (\ref{eq:inRetx_In}) is unbounded. That is, the rate formula
$\mathcal{R}^{\text{in}}_{\text{ET}}(P_{Y|X_1,X_2})$ involves a union over the limit of finite-dimensional quantum correlations
...
Let us denote the capacity region 
%
%
with CR-assisted transmitters by $\mathcal{C}_{\text{CRT}}(P_{Y|X_1,X_2})$.
We note that if we choose the measurements in (\ref{eq:inRetx_In}) such that 
...
%
%
The union of the outer bound in (\ref{eq:inRetx_Out_1}) is exhausted by an
 auxiliary variables and pure states as in  Lemma~\ref{lemm:CardCl}, following the same considerations. The details are thus omitted.
The lack of a dimension bound for the entangled state extends to the outer bound as well (see Remark~\ref{Remark:Dimension}).
...
%
%
As for the direct part, consider the region in (\ref{eq:inRetx_In}). We choose an entangled state, classical variables, and POVMs as follows. Let the entangled state $\ket{\phi_{A_1 A_2}}$ be as in (\ref{eq:Magic_A1A2}), where
...
In this sense, conferencing is stronger than entanglement. Thereby, one may ask, for example, is conferencing at a low rate better than entanglement at a high rate? Based on Theorem~\ref{theo:etMAC_Conferencing}, the answer is no. It can be easily seen that when the values of $C_{12}$ and $C_{21}$ are very low, the rate increase is negligible as well. Whereas, we have seen that high-rate entanglement without conferencing can lead to a significant increase.
\end{remark}
...
\section{Proof of Lemma~\ref{lemm:CardCl} %
}
\label{app:CardCl}
...
Next, we bound the alphabet size for the auxiliary variable $V_1$.
...
\begin{align}%
T_{1|v_0} \,:\; 
\left( p_{V_1|V_0}(\cdot|v_0) \,,\; p_{V_2|V_0}(\cdot|v_0) \right) & \mapsto 
\sum_{(v_1,v_2)\in\Vset_1\times \Vset_2} p_{V_1|V_0}(v_1|v_0)p_{V_2|V_0}(v_2|v_0) \tau_{12|v_0}(v_1,v_2)= \nonumber\\& \Big(  
...
\label{eq:thetaU}
\end{align} 
Since the map $T_{0}$ is bilinear, it maps  the set of distribution pairs on $\Vset_1 \times \Vset_2$ to a connected compact set in $\mathbb{R}^{|\mathcal{X}_1||\mathcal{X}_2|+2}$.
Thus, by the Fenchel-Eggleston-Carath\'eodory lemma \cite{Eggleston:66p}, for every  $\left( p_{V_1|V_0}(\cdot|v_0) , p_{V_2|V_0}(\cdot|v_0)\right)$, 
there exists a pair of probability distributions $\left(p_{\bar{V}_1|V_0}(\cdot|v_0),p_{\bar{V}_2|V_0}(\cdot|v_0)\right)$ on a subset $\bar{\Vset}_1\times \bar{\Vset}_2\subseteq \Vset_1\times \Vset_2$ of size $%
|\Xset_1| |\Xset_2|+2$, such that 
$%
T_{12|v_0}(p_{\bar{V}_1|V_0}(\cdot|v_0),p_{\bar{V}_2|V_0}(\cdot|v_0))=T_{12|v_0}(p_{V_1|V_0}(\cdot|v_0),p_{V_2|V_0}(\cdot|v_0))) %
$. %
...
We show that for every $\delta_1,\delta_2,\eps_0>0$, there exists a $(2^{n(R_1-\delta_1)},2^{n(R_2-\delta_2)},n)$ code for $P_{Y|X_1,X_2}$ with entangled transmitters and $\eps_0$-error, provided that $(R_1,R_2)\in \mathcal{R}_{\text{ET}}(P_{Y|X_1,X_2})$. 
...
\cite{CsiszarKorner:82b} \cite[Th. 1.3]{Kramer:08n}. %
...
%
We now show the outer bound. 
%
 Suppose that Alice 1 and Alice 2 share an entangled state $\Psi_{E_1 E_2}$. Then, Alice $k$ chooses a message uniformly at random from $[1:2^{nR_k}]$, for $k=1,2$. 
%
%
...
Notice that $V_{1}[i]$ and $V_{2}[i]$ are \emph{not}  conditionally independent given $V_{0}[i]$.
...
Let $\Psi_{E_1 E_2}=\sum_{j,\ell} Q'_j\otimes Q''_\ell$ be an arbitrary decomposition of the entangled state. Hence, by linearity,
\begin{align}
&\left(
\widetilde{\mathcal{F}}^{(i,m_1)}_{E_1\to X_1}\otimes 
...
fix a joint distribution $p_{V_0} p_{V_1|V_0} p_{V_2|V_0}$, an entangled state $\varphi_{A_1 A_2}$, and collection of POVMs $\Lset_k(v_0,v_k)%
%
$ for $k=1,2$. 
...
%
As the sum within the square brackets is bounded by $2^{-n(I(V_1;Y|V_0 V_2)-\eps_1(\delta))}$ (see \cite{CsiszarKorner:82b} \cite[Th. 1.3]{Kramer:08n}), we have 
%
%
%
\begin{align}
...
\begin{align}
R_2&<I(V_2;Y| V_0 V_1)+R_2'-\eps_2(\delta)
\intertext{and}
R_1+R_2&<I(V_1 V_2;Y|V_0)+R_1'+R_2'-\eps_3(\delta)
 \,.
\end{align}
...
$R_1'+R_2'+R_1''<I(V_1 V_2;Y)-\eps_4(\delta)$,
$R_1'+R_2'+R_2''<I(V_1 V_2;Y)-\eps_5(\delta)$, and
$R_1'+R_2'+R_1''+R_2''<I(V_1 V_2;Y)-\eps_6(\delta)$.
Thus, it suffices to require
\begin{align}
R_1+R_2&<I(V_1 V_2;Y)-\max_{j\in\{4,5,6\}}\eps_j(\delta)
\,.
\end{align}
...
%
 Suppose that Alice 1 and Alice 2 share an entangled state $\Psi_{E_1 E_2}$. Then, Alice $k$ chooses a message $m_k$, uniformly at random from $[1:2^{nR_k}]$, for $k=1,2$. They perform a conferencing protocol, whereby Alice 1 sends 
...
for every pair of operators $Q'$ and $Q''$ on $\Hset_{E_1}$
and $\Hset_{E_2}$, respectively. 
Let $\Psi_{E_1 E_2}=\sum_{j,\ell} Q'_j\otimes Q''_\ell$ be an arbitrary decomposition of the entangled state. Hence, following the same steps as in (\ref{eq:Converse_Measurement}), we have %
\begin{align}
&\left(
\widetilde{\mathcal{F}}^{(i,m_1)}_{E_1\to X_1}\otimes 
\widetilde{\mathcal{F}}^{(i,m_2)}_{E_2\to X_2}\right)(\Psi_{E_1 E_2})
...
Our main results are inner and outer bounds, as well as a regularized  characterization, for the capacity region for the \emph{general} MAC with entangled transmitters.
...
We note that if the systems were classical, then the three classes would have been identical based on the Fenchel-Eggleston-Carath\'eodory lemma \cite{Eggleston:66p} (see Section~\ref{subsection:Cardinality_Proof}). However, in the quantum case, the relation between those classes is not so obvious.
...
$\mathcal{R}_{\text{ET}}(P_{Y|X_1,X_2})$ and $\mathcal{O}_{\text{ET}}(P_{Y|X_1,X_2})$ involves a union over  the approximate family
$C_{qa}$. 
...
\item
In Case~\hyperlink{Case6}{\textcolor{blue!80!black}{6}}, the transmitters can use pre-shared entanglement in the encoding stage as well.
