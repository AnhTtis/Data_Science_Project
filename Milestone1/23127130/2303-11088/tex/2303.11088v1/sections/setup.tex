...
We run our experiments with version 0.8.5 %
of our Theodolite scalability benchmarking framework.\footnote{\url{https://www.theodolite.rocks/}}
...
For Flink, we always have one additional Pod running the JobManager.
...
We repeat most of our experiments 3~times, which we experimentally observed to be a good trade-off between overall execution time and statistical grounding~\cite{EMSE2022}.
Due to limited resources, we run the single-nodes and public-cloud experiments only once, but provide a corresponding replication package~\cite{ReplicationPackage}.
