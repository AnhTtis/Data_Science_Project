...
In this section we apply our approach to more datasets derived from the US census database. We discuss what are the differences on three (plus the one in the main body of the paper (cf.Section\ref{sec:experiments})) prediction tasks based on the US census data: ACS Income (main body), ACS Travel Time, ACS Employment and ACS Mobility\cite{DBLP:conf/nips/DingHMS21}.
...
and, the \enquote{Equal Treatment Inspector} $g_\psi$ on $\{\Ss(f_\beta,X^{val}),Z^{val}\}$. Both methods are evaluated on $\{X^{te},Z^{te},y^{te}\}$. For the type of models, $f_\beta$, as we are in tabular data we focus on we choose $f_\theta$ to be a \texttt{xgboost}\cite{DBLP:conf/kdd/ChenG16}  that achieve state-of-the-art model performance~\cite{DBLP:conf/nips/GrinsztajnOV22,DBLP:journals/corr/abs-2101-02118,BorisovNNtabular},and for the inspector $g_\psi$ a logistic regression. The final explanations are given by the coefficients of the logistic regression.
...
\caption{In the left figure, comparison of the performance of \textit{Equal Treatment Inspector}, over the state of CA14 for the ACS Employment dataset. For this dataset most of different protected attributes comparison have different AUC distributions in terms of mean and variance. The group that receives the most comparison is Other-Black. This difference in the model behaviour is due to features such as \enquote{Education} and \enquote{Citizenship}. 
...
\end{figure*}
Compared to other prediction tasks based on the US census dataset, the measured domographic parity violation for this task  is smallest. The AUC of the \enquote{Equal Treatment Inspector} is ranging from $0.55$ to $0.70$. For Asian vs Black un-equal treatment we see that there is a lot of variation of the AUC, indicating that the method achieves different values on the bootstrapping folds. 
If we look for the features driving the equal treatment gap, we see particularly high values when comparing \enquote{Asian} and \enquote{Black} populations, and for features \enquote{Citizenship} and \enquote{Employment}. 
...
\caption{In the left figure, comparison of the performance of \textit{Equal Treatment Inspector}, in different pair-wise ethnicities for the ACS Travel Time dataset. In the right image, the features that are more probable to be originating the equal treatment violation (higher implies more probable).}
\label{fig:xai.traveltime}
\end{figure*}

In general, the values of the AUC of the \enquote{Equal Treatment Inspector} are in the range of $0.50$ to $0.60$. 
By inspecting the features we can see that our proposed methods highlight different equal treatment drivers depending on the pair-wise comparison made. 
...
The goal of this task is to predict whether an individual had the same residential address one year ago, only including individuals between the ages of 18 and 35. This filtering increases the difficulty of the prediction task, as the base rate of staying at the same address is above $90\%$ for the general population~\cite{DBLP:conf/nips/DingHMS21}.
...
\caption{In the left figure, comparison of the performance of \textit{Equal Treatment Inspector}, in different pair-wise ethnicities for the ACS Mobility dataset. In the right image, the features that are more probable to be originating the demographic parity violation (higher implies more probable).}
\label{fig:xai.mobility}
\end{figure*}

In general, the values of the AUC of the \enquote{Equal Treatment Inspector} are in the range of $0.55$ to $0.80$. 
