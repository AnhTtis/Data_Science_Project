...
\title{Ext-Bessel model vanishes for tempered representations}
\author{Rui Chen} 

\address{Institute for Advanced Study in Mathmatics, ZheJiang University, East No.7 Building, Zijingang Campus, Hangzhou 310058, China }
\email{rchenmat@zju.edu.cn}
%\dedicatory{to Steve Kudla, \\ in admiration and appreciation\\ on the occasion of his 70th birthday}
\begin{document}

\begin{abstract}
In this paper we prove that the Ext-analogue of Bessel model vanishes for tempered representations.
\end{abstract}

\maketitle

\section{Problem and set up}
Let $F$ be a non-Archimedean local field of characteristic $0$ and $E$ either $F$ itself or a quadratic field extension of $F$. Let $V=V_{n+1}$ be an $n+1$-dimensional Hermitian space over $E$, and $W=W_n$ an $n$-dimensional non degenerate subspace of $V$. Let $G=G(V)$ and $H=H(W)$ be the isometry groups of $V$ and $W$. We shall identify $H$ as a subgroup of $G$ fixing some anisotropic vector. For an irreducible representation $\pi$ of $G$ and $\sigma$ of $H$, one can define the Bessel model
\[
    \Hom_H(\pi,\sigma).
\]
The study of this model has led to many important results, for example the local Gan-Gross-Prasad conjecture. In recent years, Prasad pointed out that, we should not only study the Hom space, but also the Ext-analogue of Bessel model  
\[
    \Ext^i_H(\pi,\sigma),
\]
...
\]
It has been shown by Prasad \cite[Thm. 6.1]{MR3966813} that these Ext groups are all finite dimensional, so the $\EP_H(\pi,\sigma)$ is well defined. He also verified that $\EP_H(\pi,\sigma)$ has some good properties, such as the compatibility with parabolic inductions. Comparing with the Riemann-Roch theorem, he conjectured that the Waldspurger's integral formula gives the Euler-Poincar\'e characteristic \cite[Conj. 7.1]{MR3966813}. 

\vskip 5pt

As a first test of Prasad's conjecture, one can consider the Bessel model (and its Ext-analogue) for tempered representations. In this case, the Waldspurger's integral formula gives the multiplicity of the Bessel model \cite{MR2684300} \cite{MR3202558} \cite{MR4146145}, and Prasad predicted in \cite[Conj. 7.1(2)]{MR3966813} that higher Ext are all vanishing. The main purpose of this paper is to verify this:
...
\,
\begin{enumerate}
\item Recently in his IH\'ES lecture note \cite[Thm. 9.3]{prasad2023homological}, Prasad showed that indeed Theorem \ref{T:Main} implies \cite[Conj. 7.1]{MR3966813}!

\vskip 5pt

\item In \cite{MR4069188}, Chan-Savin studied the Iwahori component of the Bessel model and obtained some Ext-vanishing results for the Steinberg representation.
\end{enumerate}
\end{Rmk}

\vskip 5pt

The main tool that we will use is Atobe's work on Jacquet modules \cite{MR4055180}. In that paper Atobe systematically studied the interaction of Jacquet modules and the local Langlands correspondence (LLC) for classical groups, based on some results of M{\oe}glin, Jantzen and B. Xu. Using Atobe's result, we can embed a tempered representation into a small standard module, and then appealing to the Mackey theory to study the Hom and Ext spaces. 
...
\[
    WD_E=W_E\times \SL_2
\]
the Weil-Deligne group of $E$. An irreducible representation of $WD_E$ will be typically written as $\rho S_a$, where $\rho$ stands for an irreducible representation of $W_E$, and $S_a$ stands for the unique $a$-dimensional irreducible representation of $\SL_2$. Let $\nu$ be the absolute value of $E^\times$, and also regarded as a character of each $\GL_k = \GL_{k/E}$ through the determinant map. We shall use 
\[
    \tau_1\times \tau_2 \quad \text{and} \quad \tau\rtimes\pi
\]
to denote the nomarlized parabolic induction of general linear groups and classical groups. For a group $G$, its modulus character will be denoted by $\Delta_G$. The hyperbolic plane over $E$ will be denoted by $\calH$.
...
Let $\rho$ be an irreducible unitary supercuspidal representation of $\GL_k$. By the LLC for general linear groups, $\rho$ corresponds to an irreducible $k$-dimensional representation of $W_E$. We shall often use the same notation to denote $\rho$ and its L-parameter. If $\{x,\cdots,y\}$ is a segment, we denote by 
\[
    \langle\rho;x,\cdots,y\rangle
\]
the unique irreducible subrepresentation of $\rho\nu^x\times\cdots\times\rho\nu^y$. In particular, if $x\geq y$, then this is a Steinberg representation and will be also denoted by $\delta_\rho(x,y)$; if $x<y$, then this is a Speh representation and will be also denoted by $\zeta_\rho(x,y)$.\\
...
\]
for some irreducible representations $\tau_i$ and $\sigma_i$ of $\GL_k$ and $G_0$, then we define the partial Jacquet module
\[
...
\]
To simplify notations, when $\rho$ is clear in the context, we omit it from the subscript. The following lemma is worth noting:
...
\begin{proof}
This is a simple application of the Tadi\'c's formula.
...
Let $\pi$ be an irreducible tempered (resp. discrete series) representation of $G(V)$, $V_0\subset V$ is a non degenerate subspace such that
\[
    V\simeq V_0+\calH^k,
\]
...
\]
Since $\sigma$ is tempered, by Lemma \ref{L:CasselmanCri} any irreducible consitituent of the normalized Jacquet module $\Jac_{\overline{Q}}\sigma$ has non positive exponents. Hence
\[
...
\]
where $M_{bp}$ is the summation of all irreducible constituent which is conjugate self-dual of the same parity as $M$. Assume that:

\begin{itemize}
    \item[(\textbf{A.0})] $M_{bp}$ is a discrete L-parameter, and $M_{np}$ is a summation of non conjugate self-dual characters of $W_E$.
\end{itemize}
\vskip 5pt
We also impose the same condition on $N$. In this section we show that:
...
Given an L-parameter $M$ satisfies (\textbf{A.0}), we can write
\[
    M_{np} = m_1\chi_1 +\cdots + m_r\chi_r,
\]
where $\chi_i$ is a non conjugate self-dual character of $\GL_1$ with multiplicity $m_i$, such that for any $i\neq j$, we have
\[
    \chi_i \neq \chi_j \quad \text{and} \quad \chi_i \neq \left(\chi_j^c\right)^\vee.
\]
Then by the property of the LLC, we know that any $\pi\in\Pi_M(G)$ can be written as an irreducible parabolic induction
\[
...
\[
    \Jac_{\rho,x}\pi_{bp} \neq 0
\]
for some $x\in\R$. Then by \cite[Lem. 4.1]{MR4055180} we have $x\in\frac{1}{2}\Z_{>0}$, and $\rho S_{2x+1}\subset M_{bp}$. The following two lemmas, which were originally stated in Atobe's paper for discrete/ good partity L-parameters, are key ingredients of our proof of the main theorem.
...
Now suppose that we have proved the lemma for L-parameters $M'$ with $\dim M'_{np}<m$. Let
\[
...
\]
Then the L-parameter associated to $\pi_1$ is $M_1$. By the induction hypothesis, we have a short exact sequence
...
\]
where $V_{an}$ and $W_{an}$ are the anisotropic kernel of $V$ and $W$. Let 
\[
...
\[
    \Ext^i_H\left(\pi,\sigma\right) \simeq  \Ext^i_{G_0}\left(\sigma^\vee,\pi_0^\vee\right)
\]
for all $i\geq 0$. Doing this repeatly, finaly we can ``peel off'' all of the bad parity part, and our target $\Ext_H^i\left(\pi,\sigma\right)$ is isomorphic to some Ext group over $G(V_{an})$ or $H(W_{an})$. As both $G(V_{an})$ and $H(W_{an})$ are compact, we know that
\[
...
If both $\pi_{bp}$ and $\sigma_{bp}$ are supercuspidal, then applying Lemma \ref{L:Reduction} as in the basic case, one can show that
\[
...
\[
    \Jac_{\rho, x}\pi_{bp} \neq 0.
\]
Again we can further assume that:
\vskip 5pt
\begin{itemize}
...
\end{itemize}
\vskip 5pt
Because otherwise we can exchange the role of $\pi$ and $\sigma$, thanks to Corollary \ref{C:Switch}. We put $a=2x+1 \geq 2$. Finally, combining Corollary \ref{C:Switch} with the LLC, we can assume that:
\vskip 5pt
\begin{itemize}
\item[(\textbf{A.3})] Any character $\chi$ occuring in $N_{np}$ or $\left(N^c_{np}\right)^\vee$ is not an unramified twist of $\rho$.
\end{itemize}
\vskip 5pt
...
\end{equation*}
for all $i\geq 1$. Similar to the basic case, we use the Mackey theory to compute this Ext group. The quotient $\Pi_c$ of $\rho\nu^x\rtimes \pi_0$ corresponding to the closed $H$-orbit of $G/P$ is
\[
    \Pi_c = \rho\nu^{x+\frac{1}{2}}\rtimes \left(\pi_0~\big|_{H_0} \right),
\]
which does not make any contribution to the Ext group since $\sigma$ is tempered and $x+\frac{1}{2}>0$. Similar to \cite[Thm. 15.1]{MR3202556}, the $H$-subrepresentation $\Pi_{op}$ corresponding to the open $H$-orbit of $G/P$ is
\[
...
\]
Let $\chi_1,\cdots,\chi_k$ be a sequence of unitary non conjugate self-dual character of $\GL_1$, such that $\chi_j$ does not occur in the cuspidal component of $\sigma$ for all $j\in\{1,\cdots,k\}$. Set
\[
...
\]
On the other hand it can be computed using the Mackey theory as well. The quotient $\Pi^\flat_c$ of $\xi\rtimes \pi_0$ corresponding to the closed $H$-orbit of $G/P$ is
\[
    \Pi^\flat_c = \xi\nu^{\frac{1}{2}}\rtimes \left(\pi_0~\big|_{H_0} \right),
\]
which will not contribute to the Ext group since $\sigma$ is tempered. The $H$-subrepresentation $\Pi^\flat_{op}$ corresponding to the open $H$-orbit of $G/P$ is
\[
...
\]
where $H_\alpha=H(W_\alpha)$ for some subspace $W_\alpha\subset W$ such that $W=W_\alpha+\calH^{k-\alpha}$, $B_\alpha$ is the Bessel subgroup of $H_\alpha$ associated to the pair $(W_\alpha,V_0)$, and $\mu_\alpha$ is the distinguished character attached to $B_\alpha$. 
When $\alpha<k$, $\xi^{(\alpha)}$ is a product of $\chi_j$'s; since we have assumed that $\chi_j$ does not occur in the cuspidal component of $\sigma$, it will not contribute to the Ext group. Thus
\[
...
\[
    \Pi'_c = \rho\nu^{-x+\frac{1}{2}}\rtimes \left(\pi_0~\big|_{H_0} \right).
\]
To see $\Pi'_c$ does not contribute to the Ext groups, we use the second Frobenius reciprocity
...
\]
which does not make any contribution to the Ext group by the temperedness of $\sigma$. The $H$-subrepresentation $\Pi_{op}$ corresponding to the open $H$-orbit of $G/P$ is 
\[
...
\]
To see $\Pi'_c$ will not contribute to Ext groups, again we appeal to the second Frobenius reciprocity and K\"unneth's formula. We have 
\[
...
So now we have completed the proof of Propostion \ref{P:discCase}.
...
In this section we shall finish the proof of Theorem \ref{T:Main} based on Proposition \ref{P:discCase}. Let $M$ be a tempered L-parameter for $G$ and $N$ of $H$. If $M$ does not satisfy (\textbf{A.0}), then there is an irreducible $k$-dimensional representation $\rho$ of $W_E$ and $a\in\Z_{>0}$, such that
\[
...
\]
Same as the argument in Section \ref{SS:Induction} Subcase 2, one can choose a sequence of non conjugate self-dual characters $\chi_1,\cdots,\chi_a$ suitably, such that
\[
    \Ext^i_H\left(\delta_\rho\left(\frac{a-1}{2}, -\frac{a-1}{2}\right)\rtimes \pi_0,\sigma\right) = \Ext^i_H\left(\chi_1\times\cdots\times\chi_a\rtimes \pi_0,\sigma\right).
\]
Note that $\chi_1\times\cdots\times\chi_a\rtimes \pi_0$ has L-parameter
\[
    \left(\chi_1+\cdots+\chi_a\right) + M_0 + \left(\chi_1^c+\cdots+\chi_a^c\right)^\vee.
\]
...
The author would like to thank Prof. Kei Yuen Chan, Dr. Chuijia Wang and Dr. Jialiang Zou for helpful discussions.
