...
\begin{equation}
    \overline{\Delta}_{\text{CFTL}}: x\in[0,1]^p, y\in [0,1]^p \mapsto \overline{\Delta}_{\text{CTL}}^{\frac{1}{\gamma}}
\end{equation}
where $\gamma \geq 1$ is the focal term. With $\gamma > 1$, CFTL focuses more on less accurate predictions that have been misclassified. Note that we can also add the focal term to JMLs and DMLs.
