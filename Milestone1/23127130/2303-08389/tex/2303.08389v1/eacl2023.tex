...
\newcommand{\ours}{PR-MCS}
% \newcommand{\dataset}{FineGCC}
\newcommand{\dataset}{M-FineCapEval\xspace}

%\newcommand{\yong}[1]{\textcolor{cyan}{#1}}
\newcommand*{\yong}{\textcolor{cyan}}

\title{PR-MCS: Perturbation Robust Metric for MultiLingual Image Captioning}

\author{Yongil Kim$^{1}$, Yerin Hwang$^{1}$, Hyeongu Yun$^{1}$,\\\bf Seunghyun Yoon$^{2}$, 
 Trung Bui$^{2}$, ~\and Kyomin Jung$^{1}$ \\
  $^{1}$Dept. of Electrical and Computer Engineering, Seoul National University \\
  $^{2}$Adobe Research\\
  \texttt{\{miles94, dpfls589, youaredead\}@snu.ac.kr},
  \texttt{\{syoon, bui\}@adobe.com},
...
\input{texts/2_Relatedworks.tex}

\input{resource/figure2}
\section{Perturbation Robust Multi-Lingual CLIPScore}
\label{sec:methods}
\input{texts/3_Methods.tex}

\input{resource/figure4}
\section{\dataset~ Dataset}
\label{sec:Mfinecapeval}
\input{texts/4_MFineCapEval.tex}

\input{resource/figure5}
\input{resource/figure6}
\section{Experiments}
\label{sec:experiments}
\input{texts/5_Experiments.tex}

\section{Conclusion}
\label{sec:conclusion}
\input{texts/6_Conclusion.tex}

\clearpage

\section*{Limitations}
\label{sec:limitations}
\input{texts/Limitations.tex}
...
\appendix
\label{sec:appendix}
\input{texts/7_appendix.tex}

\end{document}

\iffalse
\title{Instructions for EACL 2023 Proceedings}
