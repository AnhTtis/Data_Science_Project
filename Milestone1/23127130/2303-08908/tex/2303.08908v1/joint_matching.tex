...
\title{Online Bipartite Matching in the Probe-Commit Model}
...
\begin{table}[H]
	...
	\begin{tabular}{c|c|c} \hline
		...
		\hline
	\end{tabular}
	\caption{New competitive ratios are \textbf{bolded}.  "$\ge$" refers to lower bounds on the \textit{optimal} competitive ratio (algorithmic results), "$\le$" refers to upper bounds (impossibility/hardness results), and arrows indicate improvement from the state of the art. ``$-$" indicates that no constant competitive ratio is attinable, and ``$?$" means that no previous competitive ratio was known.}
	\label{tab:my_label}
\end{table}
...
when $G$ is known and has unbounded patience, Derakhshan and Farhadi \cite{Derakhshan2022} very recently proved an approximation
...
is edge-weighted and known to the online probing algorithm). Afterwards, we explain how our techniques apply when $G$ is unknown and draw from a known i.d. input or generated by an adversary.
...
\textbf{Proving Theorem \ref{thm:new_LP_relaxation}:} In order to prove Theorem \ref{thm:new_LP_relaxation},
the natural approach is to view $x_{v}(\bm{e})$ as the probability that the offline adaptive benchmark
...
regarding the offline adaptive benchmark:
\begin{enumerate}[label=(\subscript{P}{{\arabic*}})]
	\item If $e=(u,v)$ is probed and $\sta(e)=1$, then $e$ is included in the matching, provided $v$ is currently unmatched. \label{eqn:single_vertex_committal}
...
we could upper bound $\OPT(G)$ by $\LPOPT(G)$. Now, if we were working
with the \textit{online} adaptive benchmark, then it is clear that we could assume \ref{eqn:single_vertex_committal} and \ref{eqn:single_vertex_non_adaptivity} simultaneously\footnote{It is clear that we may assume
	the offline adaptive benchmark satisfies \ref{eqn:single_vertex_committal} w.l.o.g., but not \ref{eqn:single_vertex_non_adaptivity}.}
w.l.o.g.  On the other hand, if a probing algorithm does \textit{not} respect an adaptive vertex ordering
on $V$ (i.e., does not probe edges in $\partial(v)$ consecutively), then the probes involving $v \in V$ will in general depend on $(\sta(e))_{e \in \partial(v)}$. For instance, if $e \in \partial(v)$ is probed and inactive, then perhaps the offline adaptive benchmark next probes $e' =(u,v') \in \partial(v')$ for some $v' \neq v$. If $e'$ is active and thus added to the matching by \ref{eqn:single_vertex_committal}, then the offline adaptive benchmark can never subsequently probe $(u,v)$ without violating \ref{eqn:single_vertex_committal}, as $u$ is now unavailable to be matched to $v$. Alternatively, one could define $x_{v}(\bm{e})$ as the probability that the offline adaptive benchmark probes $\bm{e}$ in order,
...
Our solution is to introduce an alternative interpretation of $(x_{v}(\bm{e}))_{v \in V, \bm{e} \in \scr{C}_v}$
...
If edge $e_t=(u_t,v_t)$ is returned, and $u_t$ is unmatched, then we add $e_t$ to the current matching. This
online algorithm attains a competitive ratio of $1/e$, and its analysis follows a similar
...
\subsubsection{Vertex-weighted Worst-case Instances}
We analyze our vertex-weighted algorithm using \ref{LP:DP}, an LP introduced by Brubach et al.,
...
\begin{proof}[Proof of Theorem \ref{thm:known_id_relaxation}]
	...
	variable $\bm{1}_{[b_{t} = v]}$, then we get that 
	\[
...
\begin{proof}[Proof of Theorem \ref{thm:ROM_edge_weights}]	
	...
	\]
	Moreover, Lemma \ref{lem:availability_lower_bound} implies that
	\[    
	...
	\end{align*}
	As a result, using \eqref{eqn:value_of_matching}, we get that
	\begin{align*}
...
of $\OPT$ when restricted to the \textbf{induced stochastic graph}\footnote{Given $R \subseteq U, V' \subseteq  V$, the induced stochastic graph 
	...
	in $R \times \{s\}$.} $G[\{v\} \cup R]$. We denote the performance of $\OPT$ on $G[\{s\} \cup R]$ by
$\OPT(R,s)$.
...
If $R$ consists of the unmatched vertices when $v$ it arrives,
then suppose that Algorithm \ref{alg:dynamical_program} matches $v$ to $u$ while making its probes to
a subset of the edges of $R \times \{v\}$. In this case, we \textbf{charge} $ 2 w_{u}$ to $\alpha_{u}$ and
$2 w_{u}/ \OPT(v,R)$ to $\phi_{v,R}$.
...
\begin{example} \label{example:bad_OPT_behaviour}
	...
	if $R_{2} = R_{1} \setminus \{v_{2}\}$, then $\OPT$ does \textit{not} probe $(u_{1},v)$. Specifically,
...
\begin{proof}[Proof of Proposition \ref{prop:offline_dual_variable_lower_bound_general}]
	...
	and recall the definitions of $p_{v_0}:=\max_{e \partial(v_0)}p_e$ and $c_{v_0}:=\max_{\bm{e} \in \scr{C}_{v_0}} |\bm{e}|$. 
...
\begin{proof}[Proof of Lemma \ref{lem:dual_feasibility_constraints_general}]
	...
	Proposition \ref{prop:offline_dual_variable_lower_bound_general} ensures that
	\[
...
\begin{align*}\label{LP:config_dual}
	...
	&\text{subject to} & \beta_{v} + \sum_{j=1}^{|\bm{e}|} p_{e_j} \cdot g(\bm{e}_{< j}) \cdot \alpha_{u_j} \ge \sum_{j=1}^{|\bm{e}|} p_{e_j} \cdot w_{e_j} \cdot g( \bm{e}_{ < j}) &&
...
with substring-closed probing constraints $(\scr{C}_v)_{v \in V}$. 
...
\begin{proof}
	...
	\begin{align}
		\mb{P}[\text{$u$ matched by $\scr{A}(G)$}] &= \mb{P}\left[ \cup_{\substack{v \in V: \\ x_{u,v} =1}} \sta(u,v)=1 \right] \\
		&\ge 1 - \prod_{v \in V} (1 - p x_{u,v})
	\end{align}
...
where
\[
\til{x}_{u,v_t}:= \sum_{\substack{\bm{e}' \in \scr{C}_{v_t}: \\ e \in \bm{e}'}} g(\bm{e}_{ < e}') \cdot x_{v_t}( \bm{e}'),
\]
...
\[
    \mb{E}[ w(e_t)  \, | \, V_{t}]  = \frac{\LPOPT(G_t)}{t},
\]
...
where
\[
	\til{x}^{(t)}_{u,v} := \sum_{\substack{\bm{e}' \in \scr{C}_{v_t}: \\ e \in \bm{e}'}} g(\bm{e}_{ < e}') \cdot x_{v_t}^{(t)}( \bm{e}')
\]
...
Moreover, define $\til{E}:= E \setminus \partial(v_0)$. We claim the following inequality:
\[
...
charged $w_{u}$. In other words, $\alpha_{u_0} \ge w_{u} \cdot \bm{1}_{[u_{0} \notin R_{v_0}]}$.
As a result,
\[
