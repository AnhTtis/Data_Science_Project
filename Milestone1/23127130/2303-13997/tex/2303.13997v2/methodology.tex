...
bits, respectively, for a $64 \times 64$ systolic array.  $\times 2$ is due to
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.45\linewidth}
         \centering
         \includegraphics[width=0.72\linewidth]{Fig/ActivationDistribution_revised.pdf}
         \caption{}
         \label{fig:actDis}
     \end{subfigure}
     \begin{subfigure}[b]{0.45\linewidth}
         \centering
         \includegraphics[width=0.72\linewidth]{Fig/partialSumDistribution_revised.pdf}
         \caption{}
...
the switching activities of partial sums from one bin to another bin should be maintained as similar as possible. 
To achieve this goal,
we group partial sums 
according to the similarity in their bit numbers.
%with similar bit numbers into one bin. % with a heuristic method.
%To construct the bins we utilize a simple heuristic approach.
Specifically, a predetermined number of bins is first specified. Afterwards, partial sums are randomly selected and assigned into each bin. 
%we decide on the number of bins and assign one randomly selected partial sum to each bin.
%Third, 
The remaining partial sums are iteratively assigned into the most matching bins by
measuring the similarity between a specific partial sum and a bin. This similarity is evaluated 
by counting the number of different bits between this partial sum and those in the bins. 
The partial sum is assigned to
the bin where the partial sum has the least number of different bits in average. 
In the experiments, 50 bins were used.
%are iteratively assigned to the bin where the partial sums inside have the least number of different bits 
%and the bin whose partial sums 
%to find the most suitable bin which has the highest similarity 
%Next, we randomly iterate through all remaining partial sums and assign it to the bin
%which best matches the bit-characteristic of the selected partial sum.
%We measure the similarity between a specific partial sum and a bin
%by counting the number of differing bits between that partial sum and all assigned partial sums of the bin.
%The partial sum is then added to the bin with the least number of differing bits.
%In our experiment, we used 50 bins to partition the whole range of the partial sums.
...
the bins. It can be observed that 
the partial sum transitions are not evenly distributed.  
%partial sums also exhibit a special transition distribution.
For example, the bright diagonal line from the upper left corner to the lower right corner indicates that there are many transitions between partial sums with similar values. In addition, the bright vertical and horizontal lines also demonstrate intensive partial sum transitions. 
%This observation emphasizes the importance of using real data for power analysis
%to get results which closely reflect the real operating condition of the systolic array.

% It can be observed that the full value range of the partial sums are
