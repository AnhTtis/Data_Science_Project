...
\begin{enumerate}[(1)]
	\item Start with $r=R$.
	\item Perform breadth-first search from $c$ in $G$ to compute the ball $\ball(c, 4r)$.
...
We say that a vertex $v$ is \emph{captured} by a ball $\ball(c, r)$ if $\dist_{G}(c,v)\le r/2$.
%
