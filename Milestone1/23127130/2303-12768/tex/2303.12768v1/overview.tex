...
The basic tool of our algorithms is a clustering algorithm in \cite{bodwin2016better}. Let $G = (V, E)$ be the input undirected unweighted graph. Roughly speaking, for any radius parameter $R$, we can decompose the graph into a set $\bset$ of balls, with the following properties.
\begin{itemize}
...
Our starting point is a spanner with stretch function $f(d) = d + O(d^{1/2})$ and $\tilde{O}(n^{8/7})$ edges, which improves upon the previous sparsity bound of $\tilde{O}(n^{20/17})$ in \cite{chechik2013new}. Consider any pair of vertices $s, t\in V$, and let $\pi$ be a shortest path between them of length $D$. We apply the clustering algorithm from \cite{bodwin2016better} with radius parameter $R = D^{1/2}$ to $G$, and obtain a set $\bset$ of balls. Intuitively, by the coverage property, and recalling that every ball in $\balls$ has radius roughly $R=D^{1/2}$, we can choose a subset of $O(D / R) = O(D^{1/2})$ balls from $\bset$ whose union contains the entire shortest path $\pi$.
...
To see why this is true, assume for simplicity that $\ball(c_1, r_1)$ and $\ball(c_{l-1}, r_{l-1})$ are the first and the last large ball, respectively. Then, by construction of $S$, there exist $u\in \ball(c_1, r_1)\cap S$ and $v\in \ball(c_{l-1}, r_{l-1})\cap S$, and it is easy to verify using triangle inequality that the following path connecting $s$ to $t$ in $H$ has length at most $\dist_G(s,t)+O(R)$; the path is the concatenation of (see \Cref{overview-hitset} for an illustration):
	\begin{itemize}[leftmargin=*]
		...
		\item a path from $w_1$ to $u$ of length at most $2R$, as we can afford to add to $H$ a breath-first search tree of $G[\ball(c_1, r_1)]$ beforehand; similarly, a path from $w_l$ to $v$ of length at most $2R$; and
...
then computing a sparse subgraph $H_{c_i}\subseteq G[\ball(c_i, r_i)]$ in order to preserve the distance between $w_i, w_{i+1}$ to within a constant additive error.
...
\begin{itemize}[leftmargin=*]
	...
	%
	Then we still need to zoom into the intervals $[1, x]$ and $[y, l+1]$, and recurse. Now, since all the level-$1$ balls have already been assigned their demand pairs within these two intervals, we have now reduced to the uniform size case within those two intervals.
\end{itemize}

Intuitively, in the general case where all balls in $\bset$ have different sizes, we will first partition them into $O(1/\eps)$ groups according to their sizes, and then generalize the above approach for two ball sizes to multiple ball sizes. This will eventually introduce an additional factor $2^{O(k/\eps)}n^{O(k\eps)}$ in the sparsity.
...
We now provide an overview for our additive spanner construction. In what follows we will mainly highlight the difference between the construction by \cite{bodwin2016better} and ours.

First, we apply the clustering algorithm from \cite{bodwin2016better} with radius $R$ to $G$ and obtain a set $\bset$ of balls. 
...
$O(n/R^{5/3})=O(n^{2/7})\le (R^{4/3})^{1/2}\le O(|\ball|^{1/2})$ pairs.

We proceed differently. We start by showing via the above framework that any graph $G$ and any subset $S\subseteq V(G)$ has a $O(|S|^{3/2})$-additive subset spanner of linear size with respect to $S$, which is a subgraph $H\subseteq G$, such that for every pair $s,s'\in S$, $\dist_{H}(s,s')\le \dist_{G}(s,s')+O(|S|^{3/2})$. The algorithm is simple. Set $R=|S|^{3/2}$ and compute a clustering (and we can ignore balls $\ball$ with $|\ball|\le (|S|^{3/2})^{4/3}=|S|^2$); iteratively go through all shortest paths connecting pairs in $S$ and buy them whenever their endpoints are unsettled. Since whenever a cluster collects a new demand pair, it is settled with a new vertex in $S$, a ball $\ball$ gets at most $|S|=O(|\ball|^{1/2})$ demand pairs. Now instead of planting a $+O(|\ball|^{1/2})$ additive spanner inside each ball, we plant a $+O(|S|^{3/2})$ subset spanner inside each ball with respect to its boundary vertices. The stretch between them is now $O((|\ball|/R)^{3/2})$, which is less than $|\ball|^{1/2}$ when $R^{4/3}<|\ball|<R^{3/2}$, this small advantage, together with some further utilization of the subset spanner, allows us to eventually improve their bound $O(n^{3/7+\eps})$ to $O(n^{0.403})$.
