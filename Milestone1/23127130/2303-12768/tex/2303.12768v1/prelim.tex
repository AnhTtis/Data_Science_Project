...
Let $\balls$ be a collection of balls. We say that a vertex $v$ is \emph{covered} by $\balls$ iff $v$ belongs to some ball in $\balls$.
...
We will use the following lemma, whose proof is a simple implementation of the breath-first search (BFS) algorithm, and is omitted.
...
One of the building blocks in the construction of $+O(n^{3/7+\eps})$ additive spanner in \cite{bodwin2021better} is a clustering procedure, which takes the input graph and computes a collections of balls with certain coverage and disjointness properties.
