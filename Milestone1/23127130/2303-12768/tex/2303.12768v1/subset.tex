...
For a graph $G = (V, E)$, a subgraph $H\subseteq G$ and a subset $U\subseteq V$, we say that $H$ is a \emph{subset spanner of $G$ on $U$ with additive error $f(n)$}, iff for every pair $u,u'\in U$, $\dist_{H}(u,u')\le \dist_{G}(u,u')+f(n)$.

\begin{lemma}\label{subset}
For any $\epsilon> 0$, there is an algorithm that, given an unweighted undirected graph $G$ on $n$ vertices and $m$ edges, and a subset $U\subseteq V(G)$, in time $O\big(m\big(|U| + 2^{O(1/\epsilon)}\big)\big)$, computes a subset spanner $H$ of $G$ on $U$ with additive error $O(|U|^{3/2}\cdot n^\epsilon)$, such that $|E(H)|=2^{O(1/\epsilon)}\cdot n$. 
\end{lemma}
...
$$\sum_{c} |E(L_c)|= \sum_{c} O\left(|\ball(c, 4r)| + \sqrt{|\ball(c, 4r)|}\cdot \left(\frac{|\ball(c, 4r)|}{R}\right)^2\right) = \sum_c O(2^{15/\epsilon}|\ball(c, 4r)|)=O(2^{25/\epsilon}n/\eps).$$
...
\begin{enumerate}[(1),leftmargin=*]
	\item Starting with $i=0$, and set $u_0=v_0=s$. If $u_i = t$ then we terminate the process and set $l=i+1$.
...
Let $\pi_{s, t}[u, v]$ be a maximal subpath of $\pi_{s, t}$ consisting of only vertices covered by $\bset_1$, such that $u\ne s$ and $v\ne t$. Then both $u, v$ are covered at boundary by $\bset_1$.
\end{claim}
\begin{proof}
...
Recall that path $\pi_{s, t}$ was partitioned into sub-paths in sets $\Sigma$ and $\Sigma^\prime$. From the algorithm description, all subpaths in $\Sigma'$ are entirely contained in $H$.
...
Hence, $$E\left(\bigcup_{\ball(c, r)\in \bset\text{ is large}} \Pi_c\right)\leq O\left(\sum_{\ball(c, r)\in \bset\text{ is large}}|\ball(c, 4r)|\right) = 2^{O(1/\eps)}n.$$
\end{proof}
