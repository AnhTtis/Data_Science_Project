...
$^{\dagger}$\{vibhoothi, akatsenou, pitief, anil.kokaram\}@tcd.ie, 
...
Although some of these metrics have been tested with HDR images, or, in isolation in HEVC encodings, these have not yet been fully compared as optimisation functions inRate-Distortion Optimisation (RDO).
...
{\noindent \bf{wPSNR}} is calculated by weighting the mean squared error of the pixel values (wMSE) in YCbCr according to the power of their luma value. High luma values are assigned larger weights (see details in~\cite{jvethdrctc}). 
...
{\noindent \bf{PSNRL100}} measures the distortion of the luma component $L$ of CIELAB colour space. Similarly to DE100, a normalisation factor of 100 is applied before applying the $L^*$  transformation. The PSNR100 score is computed as the PNSR of the mean absolute error (MAE) of the normalised $L^*$.  
...
%\vspace{-.5em}
\subsection{HDR Metrics in Rate-Control}
%\vspace{-.3em}
...
This change in the luma-chroma characteristics suggests that we will observe discrepancies when optimising for a luma-based or a chroma-based HDR metric. Note that the balance between chroma and luma in the encoder is a fundamental issue that also affects SDR material. Indeed, a somehow similar problem was also reported by Barbato et al.~\cite{rav1e2019spie} for the rav1e AV1 encoder, on BT.709 SDR videos. It was found that optimising the BD-Rate for CIEDE2000, a quality metric that targets both luma and chroma, induced significant losses in terms of the luma-based PSNR-Y. Similarly to H.Sup15, they proposed to adjust the chroma quantisers, but this time for SDR. The exact formula for the offset is slightly different in rav1e compared to the ITU proposed method for HEVC, however the idea is the same. They ran an offline experiment to tune the chroma quantisers, so as to optimise an average of both CIEDE2000 and PSNR-Y over 1000 images.
...
During optimisation, at each iteration of $(k_1,k_2)$, the  cost function is expressed by the clip's average BD-Rate~\cite{bdrate,bdrate_extended} gains over the default configuration ($(k_1,k_2)=(1,1)$, see \cite{vibspie2022}). The BD-Rate for a particular clip $m$ is defined as: 
\begin{multline}
    C^m(k_1,k_2)  = \\ \frac{1}{Q_2 - Q_1} \int_{Q_1}^{Q_2} \left(R^m_{(k_1,k_2)}(Q)-R^m_{(1,1)}(Q)\right) dQ\,,
\label{eq:bdr_single}
\end{multline}
where the range $[Q_1,Q_2]$ is set as in~\cite{aomctc}. 
In  practice, we encode each video clip $m$ for five values of $qp$ ($qp\in\{27, 39, 49, 59, 63\}$). Evaluating the quality metric for each of these five encodes gives us five operating RD points, which can then be interpolated using Piecewise Cubic Hermite Interpolating Polynomial (PCHIP)~\cite{pchippaper} for the BD-Rate.%to compute the BD-Rate.
...
\subsection{Dataset}
%%\vspace{-.5em}
We use a subset of the HDR dataset utilised in~\cite{vibspie2022}. It consists of 35 clips (4550 frames) curated from public resources. Videos are normalized to BT.2020 colour primaries with SMPTE2084 Perceptual Quantizer (PQ) transfer function represented in YUV space inside the YUV-Y4M container. All sequences have a native spatial resolution of 3840x2160 at \{24, 50, 59.94, 60\} fps and are downsampled with Lanczos-5 filter to 1920x1080. More details on the dataset sources and characteristics can be found in the project page~\cite{supplymaticme2023}.
...
\toprule
\multicolumn{2}{c}{Evaluation Metrics} & \multicolumn{5}{c}{Optimisation Metric} & \multicolumn{3}{c}{Chroma Offsets (CO)}\\ \cmidrule(r){1-2}\cmidrule(r){3-7}\cmidrule{8-10}
...
A first obseervation is that some of the metrics are strongly correlated. As anticipated, the wPSNR and PNSR are virtually identical 
...
%\vspace{-.4em}
\subsection{The Issue with Luma-Based BD-Rates} 
%\vspace{-.5em}
