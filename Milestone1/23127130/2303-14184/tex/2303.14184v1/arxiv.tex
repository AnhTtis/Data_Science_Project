...
A concurrent work NeRDi~\cite{deng2022nerdi}   also leverages the generative prior for single-view NeRF. 
Yet the reconstructed 3D model is not well aligned with the input image.
In contrast, we propose a two-stage 3D synthesis framework equipped with a relaxed SDS loss, yielding high-quality 3D representation faithful to the given input image.
