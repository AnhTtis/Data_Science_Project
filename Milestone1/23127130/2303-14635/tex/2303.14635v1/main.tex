\documentclass[reprint,aps,prl,twocolumn,superscriptaddress]{revtex4-1}
\usepackage{graphicx}
\usepackage{mathrsfs}
...
far infrared Kerr and 
Faraday responses of thin 
films of MnBi$_2$Te$_4$ with up to $N=10$ septuple layers with the goal of clarifying 
the relationship between these convenient magneto-optical observable and the TME.
We find that for even $N$ the linear Kerr and Faraday responses
to an electric field vanish in the low-frequency limit, 
even though the the magnetoelectric response is large and approximately quantized.  
\end{abstract}
...
% \item Using Eq. \ref{eq:efield} and Eq. \ref{eq:faraday_kerr}, the Faraday and Kerr rotation angles {\it vs.} 
% optical frequencies are calculated, shown as in Fig. \ref{fig:Kerr_Faraday}.
% The Faraday and Kerr rotations for AF thin films are shown in Fig. \ref{fig:Kerr_Faraday} (a) and (c), from which we see that for thin films with even-number-layers (corresponds to dashed curves), there is no Faraday and Kerr signals as $\sigma_{xy} (\omega) = 0$. For odd N thin films, however, there is a large Faraday and Kerr rotation angle ($\theta_{F/K}$) at finite frequencies even though the MnBi$_2$Te$_4$ thin films are trivial insulators, see $N =1$ or $N = 3$ thin films for example, although $\theta_{F/K}$ is still 0 in the DC limit. Otherwise, the Faraday and Kerr rotation angles in the DC limit approach to a value of around 1.4 rad and 0.16 rad correspondingly. The Faraday and Kerr rotation signals, although not distinguished for the QAH insulators with the same Chern number, have observable differences at finite frequency due to different Berry curvature distributions in momentum space.
...
response \cite{Qi2008,Wang2015,Morimoto2015} properties.
The TME effect occurs only when the Dirac cones are gapped by introducing magnetic dopants at the 
surface or by using magnetic TIs like MnBi$_2$Te$_4$, and has been proven to be difficult to measure directly. 
Partly for this reason there has been interest\cite{Tse2011,Tse2010a,Tse2010b,Wu_Exp_2016,Dziom2017,Armitage_RMP_2018,Armitage_2019,he2022topological,Molenkamp_2022,Ahn_2022} 
in using magneto-optical Kerr or Faraday effects as a proxy for magnetization.
In this Letter we point out that both the Kerr and the Faraday responses to an electric field 
in magnetic TI thin films differ qualitatively from the magnetization response. 
We illustrate the difference using a simplified coupled Dirac-cone model\cite{Lei2020} of the magnetic TI MnBi$_2$Te$_4$.
...
%   surfaces of a mangnetic TI.  At each surface the magnetization depends linearly on the local chemical potential relative to the local electric potential.  
%   When an electric field $E_z$ is applied across the bulk of the thin film it produces a relative energetic shift between the gapped Dirac cones.  
%   The total equilibrium orbital magnetization is then 
%   proportional to $E_z$ and to the film volume.
%     }\label{fig:schematic}
% \end{figure}
% \fi

In the thin-film limit
Kerr and Faraday optical response coefficients
and orbital magnetization are closely related quantities 
that are normally present or absent together.
All three quantities require broken time reversal symmetry.
The Kerr and Faraday effects are easily measured \cite{Fu2007,Brune2011,Kozlov2014,Xu_2014,Yoshimi_2015} when external fields are 
...
non-zero even in the absence of an electric field. One of this cases is MnBi$_2$Te$_4$ films with an odd number of septuple layers $N$ in which
the surface magnetizations are parallel. 
Our interest here is the magnetization response to electric field in films with an even number of septuple layers, which is quantized.  

% Kerr effect measurements are routinely 
% used as a proxy for magnetization measurements.
% As explained below, the even septuple layer compensated magnetic topological insulator case is an 
% exception in which the Kerr and Faraday effects 
% behave in a qualitatively different manner than the magnetization.  
% In even $N$ MnBi$_2$Te$_4$ films, however,  
% the Kerr and Faraday effects to an electric field vanishes at low frequencies
% under circumstances where the TME is robust.
% Below we first explain the origin of this difference, and then explore it quantitatively using a simplfied model of magnetic topological insulator thin films.
% We focus on the case of magnetic topological insulators 
% (and on MnBi$_2$Te$_4$ in particular),
% since these seem at present to have more reproducible 
% magnetism than surface-magnetized non-magnetic TIs,
% although our conclusions apply to both cases.


% \section{Hall, Faraday, and Kerr Effects {\it vs.} Orbital Magnetization}

% The Kerr and Faraday effects of TI thin films are easily measured \cite{Fu2007,Brune2011,Kozlov2014,Xu_2014,Yoshimi_2015} when external fields are applied or the magnetization orientations on top and bottom surfaces are parallel.  The device Hall conductivity 
% is then quantized \cite{Yu2010,Liu2008,Chang2013} at a non-zero value and the magnetization is 
% non-zero even in the absence of an electric field.  In MnBi$_2$Te$_4$ the surface magnetizations are 
% parallel in films with an odd number of septuple layers.  Our interest here is 
% therefore in the magnetization response to electric field in films with an even number of septuple layers. 
...
% or equivalently of the oribital magnetizations that they produce $dM_{edge}/d\mu = \sigma A e^2/h$,
...
% either at the top and bottom surfaces or, as in the case a magnetic topological insulator
...
% The magnetic configuration with opposite magnetization orientations on opposite surfacesis often referred to in the literature as the {\it axion insulator} configuration.  
...
% insulator with thickness $t$, the local electrical potentials on the top and bottom surfaces differ by $e E_z t$, moving the 
% local Dirac bands relative to the chemical potential as illustrated in Fig.~\ref{fig:schematic}.
% This difference yields a net magnetization that is linear in $E_z$ and proportional to the system volume - the topological magnetoelectric effect.  
...
The TME effect is a {\it dc} response property of TIs, but also holds approximately at 
...
% \end{pmatrix} = \frac{1}{\mathcal{N}} 
%     \begin{pmatrix}
% 1 - (1+2\alpha \sigma_{xx})^2 - (2\alpha \sigma_{xy})^2 \\
% 4 \alpha \sigma_{xy}
% \end{pmatrix},
% \end{aligned}
% \end{equation}
...
by symmetry \cite{Lei2021_metamagnetism,lei2022quantum} at electric field 
$E_z=0$ at all frequencies.  Because of quantization, the {\em dc} Hall 
conductivity vanishes identically at finite $E_z$ until the field is strong enough to close the gap; 
The linear response of the Hall conductivity to $E_z$ is therefore zero in the {\em dc} limit.
Under the same conditions, the linear response of the magnetization is quantized at the topologically protected value.  
We anticipate that the linear response of the Kerr and Faraday effects to $E_z$
is continued to be strongly suppressed at finite frequencies provided they are well below the gap.  
We now use a simplified model of MnBi$_2$Te$_4$ thin films to 
test this expectation quantitatively.  
...
Dirac velocity $v_D = 5 \times 10^5 m/s $,
...
% \begin{figure}[htb]
%   \centering
%   \includegraphics[width=0.9 \linewidth ]{Optical_Response.pdf}
%   \caption{Schematic illustration of Kerr and Faraday effects in MnBi$_2$Te$_4$ thin films.
%   (a) Kerr and Faraday effect in thin film MnBi$_2$Te$_4$ with magnetic configuration illustrated in (b) and (c). 
%   The electric fields in our calculations are along the $\hat{z}$ direction, {\it i.e.} perpendicular to the thin film.
%   }\label{fig:one}
% \end{figure}
% \fi
...
\begin{figure}[htp!]
  ...
  (a) Bandstructure of $N=4$ MnBi$_2$Te$_4$.  Even $N$ bands 
  are doubly degenerate due to the $\mathcal{T} \mathcal{I}$ symmetry discussed in the text.
...
$E_z=0$ systems by plotting 
the bandstructure and the frequency dependent longitudinal and Hall 
optical conductivities  ($\sigma_{xx}(\omega)$ and $\sigma_{xy}(\omega)$) of antiferromagnetic MnBi$_2$Te$_4$ thin films with septuple-layer numbers $N=4$ and $N=5$ (The optical conductivities for other antiferromagnetic thin films as shown in Fig. S1 of the supplemental material \cite{SI}).  
The ground states for $N=4$ and other even $N$ films are referred as axion insulators in the literature, and have similar conductivities.  
...
which leads, via a generalized Kramers theorem, to the doubly degenerate 2D bands \cite{Lei2021_metamagnetism} shown in Fig. \ref{fig:band_conductivity} (a).
...
In Fig. S2 of the supplementary material \cite{SI} we summarize the properties of the conductivity tensor in thin films with 
...
\begin{figure}[htp!]
  ...
  \caption{Comparison of the electric field dependence of the orbital magnetization, and the Kerr (a) and Faraday (b) rotation angles
  ...
  % In the plot of (c) and (d) the optical frequency is 25 meV which is below the gap and the electric field is set to be 25 meV/nm which is close but smaller than the critical electric field.
...
is broken by an electric field $E_z$ applied
across the film.  The orbital magnetism response to $E_z$, plotted as blue curves in Fig. \ref{fig:kerr_field})(a) and (b),
...
%response to $E_z$ to be weak since there is no {\em dc} response. 
%Even-layer MnBi$_2$Te$_4$ thin films in AF state are axion insulators, the Hall conductivities are 0 and thus have no Faraday or Kerr signals as discussed. However, axion insulators do have strong magnetoelectric response with a quantized coeffecient in bulk limit\cite{Lei2021_ME}. 
%This orbital magnetism response to electric field is difficult to measure directly as it is still small. In this section we will discuss the possibility to measure this orbital magneto-electric response via magneto-optical effects, i.e. Faraday or Kerr rotations of polarized light.

We now combine the simplified coupled Dirac cone model and self-consistent Sch\"odinger-Possion equations\cite{Lei2021_QAH} 
to model the effect of $E_z$ on the magneto-optical response. 
In Fig. \ref{fig:kerr_field} we show typical 2D limit Kerr and Faraday rotation angles for 
...
they remain small even when the vertical electric field $E_z$ \cite{Lei2021_QAH} 
is near the critical value at which the film gap vanishes.
We attribute these very small values to approximate locality of the Hall response at 
top and bottom surfaces.  Because the surface magnetizations are opposite the total Hall conductivities 
nearly vanish at all frequencies even for $E_z \ne 0$.
The Kerr and Faraday angles in Fig.~\ref{fig:kerr_field} ($\sim 10^{-11}$ rad) 
lie below current Kerr angle detection limits\cite{Xia2006_Kerr,Rowe2017_Faraday}, to the best of our knowledge.
The frequency and electric fields dependence of the underlying conductivities are 
presented in Fig. S4-S6 of the 
supplemental material \cite{SI}.  Because these response coefficients 
calculated in the 2D limit are practically zero even at $E_z \ne 0$, the finite 
...
film as two Dirac surfaces that support half-quantized Hall effects of opposite sign and are 
...
can be composed \cite{Tse2011} from the top ($T$) and bottom ($B$) single-interface scattering matrices: 
...
In contrast, the Kerr rotation angle is non-zero when $n_s = n_0$, as illustrated in Fig. S7 of supplemental material \cite{SI}. 
...
The Kerr signal can also depend on the substrate thickness $d_s$. 
We can account for both the thickness and the index of refraction ($n_s$) of the 
substrate, as illustrated in Fig. \ref{fig:Kerr_Faraday_Thickness} (a), by replacing 
...
This $E_z$ is smaller than the critical electric field, 
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=0.9 \linewidth ]{Kerr_Faraday_Substrate.pdf}
  \caption{
  ...
  % (d) Kerr rotation {\it vs.} dielectric constant of substrate with various thickness.
  These results were calculated for a light frequency of 25 meV, which is below the gap at $E_z=0$, 
  an electric field $E_z =25$ meV/nm, which is close to but smaller 
  ...
  sample thickness at $E_z=0$ with the substrate thickness set to $d=200\lambda$.
  The range of thickness to wavelength ration corresponds to photon energies $\sim 10$ meV and 
  films with fewer than $\sim 10$ septuple layers.
  (e)-(f): Dependence of Kerr and Faraday angles on total Hall conductivity 
...
the total Hall conductivity $\sigma_{xy}^{T}$ remains extremely small (as illustrated in Fig. S5 of supplemental material \cite{SI}) even for 
...
In Fig. \ref{fig:Kerr_Faraday_Thickness}(e) and (f) we plot the Kerr and 
...
Kerry and angles to an electric field is extremely weak, and what survives might be difficult to disentangle.
...
This work was sponsored by the Army Research Office under 
Grant Number W911NF-16-1-0472. 
...
\appendix
\begin{center}
{\bf{Supplementary Material for \\''Gate-Tunable Quantum Anomalous Hall Effects in MnBi$_2$Te$_4$ Thin Films''}}
\end{center}
%%%%%%%%%% Merge with supplemental materials %%%%%%%%%%
...
\section{Optical conductivities}
...
For MnBi$_2$Te$_4$ thin films in ferromagnetic (FM) state, the frequency-dependence of longitudinal optical conductivity $\sigma_{xx}(\omega)$ is similar with the ones in AF state. In Fig. \ref{fig:conductivity_FM} (a) and (b) we show the plots of real and imaginary parts of $\sigma_{xx}(\omega)$ for MnBi$_2$Te$_4$ thin films with thickness from 5 SLs to 10 SLs, the step-like behaviors of $\Re \sigma_{xx}(\omega)$ origin from the accumulated exited subbands when the frequency increases. The Hall conductivities, shown in Fig. \ref{fig:conductivity_FM} (c) and (d) where we use the same color to label the same thickness as in panel (a) and (b), have different dependence on optical conductivity compared with the AF state. In the DC limit, a topological phase transition happens when the thickness increases to $N \ge 9$, above the critical thickness the Chern number increase from 1 to 2. 
$\Im \sigma_{xy}(\omega)$ shown in Fig. \ref{fig:conductivity_FM} (d) indicates that the dependence on frequency differs between $N \ge 9$ and $N < 9$ thin films. Note that $\Im \sigma_{xy}(\omega) < 0$ at low frequencies for $N < 9$ thin films, this is due to the negative Berry curvature close to $\Gamma$ point in 2D bands. The negative Berry curvature also lead to a decrease of $\Re \sigma_{xy}(\omega)$ shown in Fig. \ref{fig:conductivity_FM} (c).
...
\section{Magneto-optical Response}

Based on the optical conductivities, the magneto-optical responses of MnBi$_2$Te$_4$ thin films are estimated as shown in Fig. \ref{fig:Kerr_Faraday}, 
here we estimate the magneto-optical rotation angles in the 2D limit. For AF/FM thin films,the Kerr rotation angles are quantized to be $-\pi/2$ when the Chern numbers are non-zero. However, in the $dc$ limit, Faraday rotations are proportional with Chern numbers and thus distinguish the topological phases of thin films.
...
\section{Electric field dependence of optical conductivity}
...
As a comparison, the Faraday and Kerr rotation angles in 2D limit for N = 5 MnBi$_2$Te$_4$ thin film as am example of odd-layer case are shown in Fig. \ref{fig:kerr_field} (c) and (d). We see that the Faraday rotation angles maximize at the DC limit in the absence of electric fields. In the presence of external field the Faraday rotation angles decrease and minimize when the thin film is driven to a semimetal phase, i.e. at the electric field of around 20 meV/nm for 5-layer thin film. Note that this critical field depends on the optical frequencies, that is a positive dependence at low-frequencies. Kerr rotation angles have similar dependence on electric field compared with Faraday rotation angles. The Faraday and Kerr rotation signals can thus be used as a detection of the topological phase transition induced by external electric fields.

\section{Thickness and diffraction corrections on Kerr and Faraday rotation angles}

\ifpdf
\begin{figure}[htp!]
  \centering
  \includegraphics[width=0.9 \linewidth ]{SI_Faraday_Kerr_dielectric.pdf}
  \caption{ Dependence of magneto-optical Faraday and Kerr rotation angles {\it vs.} the relative dielectric constant of substrate.
...
The incoming field $[\Tilde{E^{ti}} ~~ \Tilde{E^{rj}}]^T $ and outgoing field $[\Tilde{E^{ri}} ~~ \Tilde{E^{tj}}]^T $ at the interface  are connected with the scattering matrix $S$ which reads as:
\begin{equation}
    S = 
    \begin{pmatrix}
\Bar{r} & \Bar{t'}\\
...
\end{pmatrix},
\end{equation}
with $\bar r'$ and $\bar t'$ obtained from $\bar r$ and ($\bar t$) by reversing the wavevector direction and interchanging the dielectric constants on opposite sides of the interface, and $\Bar{r}$, $\Bar{t}$ are defined as:
\begin{equation*}
    \Bar{r} = \begin{pmatrix}
        r_{xx} & r_{xy} \\
        -r_{xy} & r_{yy}
...
\end{aligned}
\end{equation}
Here $r_{SB}$ and $t_{SB}$ are calculated as:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Here $r_B$/$t_B$ is the scattering matrice in the interfaces between substrate and sample and $r_S$/$t_S$ is the one between substrate and vacuum.
