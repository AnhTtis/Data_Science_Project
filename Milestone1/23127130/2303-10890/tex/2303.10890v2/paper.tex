...
\begin{split}
 ...
 \end{split}
\end{align}
where $f$ is the linear growth rate, $a$ is the scale factor, $H$ is the Hubble parameter, and $\alpha \equiv d \ln n_{\rm av}(x) / d \ln x +  2$ is the selection function of a given galaxy sample. We note that $\bar{\delta}$ corresponds to the isotropy-conserving component, and $\delta_{\rm ani}^{\rm std}$ and $\delta_{\rm ani}^{\rm new}$ denote the isotropy-breaking ones depending on $b_{\rm g}$ and $b_{\rm g}^{(2)}$, respectively.
...
In this section, we shall build a computation methodology for the galaxy 2PCFs sourced from a general type of the isotropy-breaking matter matter power spectrum \eqref{eq:Pm_GLM}.
