...
% (2) Previous evaluation protocols may yield wrong assessment w.r.t. model performance, since they only considered limited scope of target datasets.
(2) The previous evaluation protocols are constrained to downstream datasets that resemble Kinetics-400. However, the high performance on these datasets does not necessarily transfer to other application domains.
...
% To quantify the progress in video action recognition, several large-scale video datasets are introduced over the past few years which have significantly advanced the field, including the Kinetics family~\cite{kay2017kinetics,k600,k700}, Sports1M~\cite{sports1m}, Something-something-v1/v2~\cite{goyal2017something} and many others~\cite{miech2019howto100m,shao2020finegym,sigurdsson2018charades,epickitchen}. 
% However, their limitations also emerge. 
% First, each of these datasets has its own characteristics (e.g., appearance-focused, motion-focused, long-tail, fine-grained, egocentric). Without evaluating on a suite of datasets, we cannot fully diagnose a model and make further improvements.
% Second, most of these datasets contain daily or sports actions because these categories are easier to collect from the web. Yet many important real-world applications, such as anomaly detection and industrial inspection, are rarely included in these datasets. 
% Hence, it is questionable whether we can directly use the ``best'' model trained on existing datasets for out-of-domain use cases. 
% Third, the held-out test set for these datasets either does not exist or is not commonly adopted. 
% This will affect the transfer performance because models tuned on a test set using hyperparameter optimization or neural architecture search might achieve quite good performance but cannot translate to other datasets due to overfitting.