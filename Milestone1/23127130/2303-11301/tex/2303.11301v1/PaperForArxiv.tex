...
\def\cvprPaperID{3650} % *** Enter the CVPR Paper ID here
\def\confName{CVPR}
\def\confYear{2023}
...
\label{sec:intro}
% Introduction on existing dense-head 3D Detectors
...
% nuScenes/Waymo accuracy, FPS
% nuSCenes/Waymo tracking
% nuScenes multi-modal, FPS
...
%SECOND~\cite{second}                  & 50.6                 & 62.3                 & 62.9 G   & 64.1 G  & \textbf{64 ms} \\
...
\multirow{2}{*}{\em Method} & \multirow{2}{*}{mAP} & \multirow{2}{*}{NDS} & \multicolumn{2}{c|}{FLOPs} \\
                        &                      &                      & Sparse CNN    & Head  \\ \hline
%SECOND~\cite{second}                  & 50.6                 & 62.3                 & 62.9 G   & 64.1 G \\
...
\begin{center}
   ...
   \caption{Detailed structure of VoxelNeXt framework. Circled numbers in the figure correspond to the paragraphs in Sections~\ref{sec:backbone-adaptation} and \ref{sec:sparse-prediction-head}. %\small{\textcircled{\scriptsize{1}}}$\,$Additional down-samplings. \small{\textcircled{\scriptsize{2}}}$\,$Sparse height compression. \small{\textcircled{\scriptsize{3}}}$\,$Voxel selection. \small{\textcircled{\scriptsize{4}}}$\,$Box regression. 
   1 - Additional down-samplings. 2 - Sparse height compression. 3 - Voxel selection. 4 - Box regression. 
...
% Latency & 31 ms & - & 80ms & 96ms & 122ms & - & - & 138ms & 94ms & 132ms & 78ms & 66ms \\
...
%TransFusion~\cite{transfusion}                & 65.5  & 70.2 & 86.2 & 56.7 & 66.3 & 58.8 & 28.2 & 86.1 & 68.3 & 44.2 & 82.0 & 78.2 \\ 
...
%PillarNet-34~\cite{pillarnet}$^{\dagger}$  & 66.0 & \textbf{71.4} & 87.6 & 57.5 & 63.6 & 63.1 & 27.9 & 87.3 & 70.1 & 42.3 & 83.3 & 77.2 \\ 
\hline \hline
...
%$^*$ means that we follow FSD~\cite{fsd} to decrease the number of pasted instances in the ground-truth sampling augmentation and increase training epochs by 6 epochs. 
...
%Voxel R-CNN~\cite{voxel-rcnn}                                        &  -  & 75.6 / -     & 66.6 / -    &  - &  -       &  -      &  -     \\
VoxSeT~\cite{voxelset}                                        &  -  & 76.0 / -     & 68.2 / -    &  - &  -       &  -      &  -     \\
VoTr-TSD~\cite{voxeltransformer}                                        &  -  & 74.9 / 74.3     & 65.9 / 65.3     & -  &  -       &  -      &  -     \\
%Pyramid R-CNN~\cite{pyramid-rcnn}                                        &  -  &  76.3 / 75.7  & 67.2 / 66.7    &  -  &  -   &  -  &  - \\
...
%Graph R-CNN~\cite{graph-rcnn}                                        & 73.1 / 70.9   & 80.8 / 80.3    & 72.5 / 72.1      &  82.3 / 76.6      &  74.4 / 69.0     & 75.3 / 74.2   &  72.5 / 71.5  \\
FSD$_{spconv}$~\cite{fsd}                                      & 71.9 / 69.7 & 77.8 / 77.3     & 68.9 / 68.5    & 81.9 / 76.4      & 73.2 / 68.0      & 76.5 / 75.2     & 73.8 / 72.5    \\
%FSD$_{spconv}^*$~\cite{fsd}                                      & 72.9 / 70.8 & {79.2} / 78.8     & 70.5 / 70.1    & {82.6} / {77.3}      & 73.9 / 69.1      & 77.1 / 76.0     & 74.4 / 73.3    \\
\hline
...
%VoxelNeXt$^*$                                      &     \textbf{73.3} / \textbf{71.1}      &     79.1 / {79.0} &  70.8 / {70.5}  &  82.2 / 77.2   &     {74.2} / {69.3}   &  {77.3} / {76.3}  &    {74.8} / {73.6}        \\ 
\hline
\end{tabular}
}
\label{tab:waymo-val}
...
Results denoted as $\dagger$~\cite{vista, uvtr, pillarnet} are reported with the double-flip testing augmentation~\cite{centerpoint}. Both lines of results are better than previous ones. %Results on the validation set are provided in the appendix.
We compare VoxelNeXt with other 3D object detectors on the Waymo validation split in Tab.~\ref{tab:waymo-val} and on Argoverse2~\cite{argo2} in Tab.~\ref{tab:argo2-allclasses}. %Following FSD~\cite{fsd}, we also decrease the number of pasted instances in the ground-truth sampling data augmentation with longer training epochs, which is denoted as $^*$. VoxelNeXt also works decently on {\em Pedestrian} and {\em Cyclist}. 
We present latency comparison in Tab.~\ref{tab:nuscenes-test} and Fig.~\ref{fig:argo2-efficiency-comparison}.
...
\end{tabular}%}
\label{tab:analysis-data-amount}
\end{center}
\end{table}
...
We visualize the results of adjacent frames in Fig.~A~-~\ref{fig:frames-box-query-voxels}. The corresponding query voxels are depicted as red squares. %We also provide a sequence of video frames, in both BEV and perspective views.
