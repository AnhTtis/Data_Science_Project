...
Recently, focus has shifted to optimization-based algorithms, which implement partial steps or approximations of the full Suzuki-Trotter step~\cite{barison_pvqd_2021, barratt_parallel_2021, benedetti_evolution_2021, slattery_ublock_2022}. 
...
Another line of work directly focuses on the preparation of thermal states by minimizing the free energy of a variational ansatz \cite{sbahi_mirror_2022}. This, approach however, does not implement general quantum time evolution.
...
\begin{equation}\label{eq:argmin_dual}
    \begin{aligned}
   \delta\theta 
   &\approx \argmin{\delta\theta} \frac{1 - F(\theta, \theta+\delta\theta)}{2(\delta\tau)^2} - \frac{b^T(\theta) \delta\theta}{\delta\tau} \\
...
\mathcal{L}(\delta\theta) = \frac{1 - F(\theta, \theta+\delta\theta)}{2} - \delta\tau \cdot b^T(\theta) \delta\theta.
\end{equation}
...
J.G. thanks Christa Zoufal and Stefano Barison for insightful discussions, as well as Elena Peña Tapia and Alexander Miessen for helpful comments on this manuscript.
