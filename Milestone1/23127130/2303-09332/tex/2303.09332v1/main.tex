...
The set of separations induced by a tree-decomposition in this way is itself tree-like~\cites{TreelikeSpaces,TreeSets} in that the separations are pairwise nested.
...
they constructed a locally finite graph that does not have a tree-decomposition that efficiently distinguishes all its tangles, canonical or not~\cite{InfiniteSplinters}*{Example~4.9}.
On the other hand, they proved that every locally finite graph still admits a \emph{tree of tangles}, a nested set~$N$ of separations such that every two distinguishable tangles are efficiently distinguished by some element of~$N$ and every separation in~$N$ efficiently distinguishes some two tangles: \looseness=-1

\begin{theorem}[\cite{InfiniteSplinters}*{Theorem 6.6}] \label{thm:ToT}
...
Then some end of~$G$ is `tied to'~$A \cap B$, and it will be easy to see that this end~$\omega$ is unique.
...
With this structure at hand, we then deduce that the unique end~$\omega$ tied to~$A \cap B$ must be thick.
...
\begin{mainresult} \label{main:Technical}
    ...
    If~$B \neq \emptyset$, then~$G$ has a unique end that is tied to~$A \cap B$, and this end is thick.
\end{mainresult}
...
\cref{sec:InterlacingMethod} deals with the interlacing theorem.
We finally combine our results on limit separations with the interlacing theorem to prove our main results, \cref{main:Theorem,main:Technical}, in~\cref{sec:ProofMainTheorem}.
...
\end{equation*}
Two separations are \emph{nested} if they have two orientations which are comparable with respect to~$\le$, and a set of separations is \emph{nested} if its elements are pairwise nested.
...
The next lemma shows that~$(A, B)$ is again a separation of~$G$.
...
If two sequences of separations both dominate each other, then they are \emph{interlaced}.
Note that a sequence of separations is interlaced with each of its subsequences.
...
\begin{lemma}[\cite{GraphDec}*{Lemma~7.3}] \label{lem:InterlacingLemma}
    ...
    If~$((A_i, B_i))_{i \in \N}$ dominates~$((C_j, D_j))_{j \in \N}$, then~$(A, B) \le (C, D)$.
    In particular, if~$((A_i, B_i))_{i \in \N}$ and~$((C_j, D_j))_{j \in \N}$ are interlaced, then~$(A, B) = (C, D)$.
\end{lemma}
...
In order to prove that a nested set~$N$ of separations is exhaustive, it is sufficient to consider certain `representative' sequences in~$\vN$ with strong structural properties instead of just arbitrary ones.
...
As an important example of this, we will often use~\cref{lem:InterlacingLemma} in combination with the next~\cref{lem:BoundedOrder} to obtain that if a strictly increasing sequence of `tight' separations is non-exhaustive, then the orders of its elements cofinitely exceed every integer; in particular, we can find a subsequence with strictly increasing orders.
So if~$N$ consists of tight separations, then in order to determine whether~$\vN$ is exhaustive, it suffices to investigate sequences in~$\vN$ with strictly increasing orders.

To state~\cref{lem:BoundedOrder}, recall that, given a set~$X$ of vertices of a graph~$G$, a component~$K$ of~$G - X$ \emph{tight} if~$N_G(K) = X$, and a separation~$\{A, B\}$ of~$G$ is \emph{tight} if~$A \setminus B$ and~$B \setminus A$ each contain a tight component of~$G - (A \cap B)$~(cf.~\cite{InfiniteSplinters}*{Section~6}).
...
\begin{lemma} \label{lem:limitsepispseudotight}
    Let~$((A_i, B_i))_{i \in \N}$ be a sequence of tight separations of a locally finite graph~$G$, and let~$(A, B)$ be its supremum.
    If~$B$ is non-empty, then every~$v \in A \cap B$ has a neighbour~$w \in B \setminus A$ such that, for infinitely many~$i \in \N$, $w$ is contained in a tight component~$K_i$ of~$G - (A_i \cap B_i)$ with~$K_i \subseteq B_i \setminus A_i$.
    In particular, we have~$N_G(B\setminus A) = A \cap B$.
\end{lemma}
\begin{proof}
    Consider an arbitrary vertex~$v \in A \cap B$.
    For every~$i \in \N$, the separation~$\{A_i, B_i\}$ is tight by assumption and there hence exists a tight component~$K_i$ of~$G-(A_i \cap B_i)$ with~$K_i \subseteq B_i \setminus A_i$.
    ...
    Then we have $w \in \bigcap_{i \in \cI} K_i \subseteq \bigcap_{i \in \cI} B_i \setminus A_i = B \setminus A$, where the last equality follows from~\cref{lem:InterlacingLemma} since~$((A_i,B_i))_{i \in \cI}$ is a subsequence of~$((A_i,B_i))_{i \in \N}$.
    The `in particular'-part follows immediately.
\end{proof}

\noindent We briefly remark that the above proof of~\cref{lem:limitsepispseudotight} did not use the fact that there also exists a tight component of~$G - (A_i \cap B_i)$ in~$A_i \setminus B_i$ for all~$i \in \N$.
Indeed, it is enough to assume that the separation~$\{A_i, B_i\}$ is proper and that there is a tight component in~$B_i \setminus A_i$.

With~\cref{lem:LimitSeparatorIsNonEmpty} and~\cref{lem:limitsepispseudotight} at hand, we are ready to prove~\cref{lem:LimitSepHasInfiniteOrder} asserts.
...
\begin{proof}[Proof of~\cref{lem:LimitSepHasInfiniteOrder}]
    ...
    Furthermore, $A \cap B \neq \emptyset$ by~\cref{lem:LimitSeparatorIsNonEmpty}, and~\cref{lem:limitsepispseudotight} thus yields a neighbour~$w \in B\setminus A$ of~$A \cap B$ and an infinite set~$\cI \subseteq \N$ such that~$w$ is contained in a tight component~$K_i$ of~$G - (A_i \cap B_i)$ with~$K_i \subseteq B_i \setminus A_i$ for all~$i \in \cI$.
...
As we will see in~\cref{sec:ProofMainTheorem}, this is already the first step towards~\cref{main:Technical} which asserts that the graph has a thick end which is tied to~$A \cap B$.
Indeed, for every infinite set of vertices in a connected locally finite graph, such as~$A \cap B$, there exists an end which is tied to this set.
...
We will frequently use the following observation which can easily be derived from the definition of the partial order of oriented separations.
...
\end{lemma}
\begin{proof}
    By~\cref{lem:NestedWithCorners}, we need to find~$I \in \N$ such that, for every~$i \ge I$, the four sets~$A_i \cap C$, $A_i \cap D$, $B_i \cap C$ and~$B_i \cap D$ are non-empty after removing~$S_i := (A_i \cap B_i) \cap (C \cap D)$.
    We show that there is~$I_{B \cap D} \in \N$ such that for every~$i \ge I_{B \cap D}$ the set~$B_i \cap D$ is non-empty after removing~$S_i$.
    Similarly, one can obtain suitable~$I_{A \cap C}, I_{A \cap D}, I_{B \cap C} \in \N$.
    Then~$I := \max\{I_{A \cap C}, I_{A \cap D}, I_{B \cap C}, I_{B \cap D}\}$ will be as desired. 

    We now show that the desired~$I_{B \cap D}$ exists.
    Let~$S := (A \cap B) \cap (C \cap D)$.
    As~$\{C, D\}$ crosses~$\{A, B\}$, the set~$(B \cap D) \setminus S = (B \cap D) \setminus (A \cap C)$ is non-empty by~\cref{lem:NestedWithCorners}, and we fix an arbitrary vertex~$v$ in this set; in particular, $v \in B$.
    We distinguish the two cases~$v \in B \setminus A$ and~$v \in A \cap B$.
    
    If~$v \in B\setminus A$, then the definition of the limit~$(A, B)$ immediately yields that~$v \in B_i \setminus A_i$ for all~$i \in \N$ and thus~$v \in (B_i \cap D) \setminus (A_i \cap C) = (B_i \cap D) \setminus S_i$.
    In particular, $I_{B \cap D} := 0$ is a suitable choice in this case.
    Otherwise, we have~$v \in A \cap B$.
    \cref{lem:PushingLemma} then yields~$I_{B \cap D} \in \N$ such that~$v \in A_i \cap B_i$ for all~$i \ge I_{B \cap D}$.
    This~$I_{B \cap D}$ now is as desired:
    Since~$v \in (A \cap B \cap D) \setminus S = (A \cap B \cap D) \setminus C$, we in particular have~$v \in D \setminus C$.
    Thus, $v \in (A_i \cap B_i) \cap (D \setminus C) = (A_i \cap B_i \cap D) \setminus S_i$ for~$i \ge I_{B \cap D}$.
    In particular, $(B_i \cap D) \setminus S_i$ is non-empty for~$i \ge I_{B \cap D}$.
\end{proof}
...
\begin{lemma} \label{lem:SeparationAboveLimit}
    Let~$((A_i, B_i))_{i \in \N}$ be a sequence of separations of a graph~$G$, and let~$(A, B)$ be its supremum.
    Then~$(A_i, B_i) \le (A, B)$ for all~$i \in \N$.
    In particular, if~$(A, B) \le (C, D)$ for a separation~$(C, D)$ of~$G$, then~$(A_i, B_i) \le (C, D)$ for all~$i \in \N$. \qed
\end{lemma}
...
\begin{lemma} \label{lem:FiniteOrderBelowLimit}
    ...
    If~$((A_i, B_i))_{i \in \N}$ is non-exhaustive and~$(C, D) \le (A, B)$, then there exists an~$I \in \N$ such that~$(C, D) \le (A_i, B_i)$ for all~$i \ge I$.
\end{lemma}
\begin{proof}
    ...
    More precisely, we successively find conditions on~$i \in \N$ for the emptiness of each of the three sets~$((C \cap B_i) \setminus S_i) \cap D$, $((C \cap B_i) \setminus S_i) \cap A_i$ and~$((C \cap B_i) \setminus S_i) \setminus (A_i \cup D)$, which partition~$(C \cap B_i) \setminus S_i$, and finally give some~$I \in \N$ satisfying all these conditions.

    First, we consider the set~$((C \cap B_i) \setminus S_i) \cap D$.
    ...
    This immediately implies $((C \cap B_i) \setminus S_i) \cap D = (C \cap D) \cap (B_i \setminus A_i) = \emptyset$ for all~$i \ge I_1$.
    
    Next, we turn to~$((C \cap B_i) \setminus S_i)  \cap A_i$.
    Recall that~$A \cap B \neq \emptyset$ by~\cref{lem:LimitSeparatorIsNonEmpty}.
    So~\cref{lem:limitsepispseudotight} yields a neighbour~$w \in B \setminus A$ of~$A \cap B$ and an infinite set~$\cI \subseteq \N$ such that~$w$ is contained in a tight component~$K_i$ of~$G - (A_i \cap B_i)$ with~$K_i \subseteq B_i \setminus A_i$ for all~$i \in \cI$.
    Now let~$i \in \cI$ with~$i \ge I_1$, and suppose for a contradiction that~$((C \cap B_i) \setminus S_i) \cap A_i$ is non-empty.
    Since~$N_G(K_i) = A_i \cap B_i$, there thus exists a path~$P$ from~$w$ to the set~$((C \cap B_i) \setminus S_i) \cap A_i \subseteq A_i \cap B_i$ whose vertices lie in~$K_i$ except its last vertex~$u$.
    Note that~$u \in C \setminus D$, as~$u \in ((C \cap B_i) \setminus S_i) \cap A_i = (C \cap B_i \cap A_i) \setminus D$, and~$w \in D \setminus C$, as~$w \in B \setminus A$ and~$(C, D) \le (A, B)$.
    Moreover, the path~$P$ does not meet~$C \cap D$ as~$(C \cap D) \cap K_i \subseteq (C \cap D) \cap (B_i \setminus A_i) = ((C \cap B_i) \setminus S_i) \cap D$ is empty for~$i \ge I_1$, as shown above.
    ...
    It remains to consider~$((C \cap B_i) \setminus S_i) \setminus (A_i \cup D) = (C \cap B_i) \setminus (A_i \cup D)$.
    We first prove that if $(C \cap B_i) \setminus (A_i \cup D)$ is non-empty for~$i \in \cI$ with~$i \ge I_1$, then it must contain a neighbour of~$S_i$.
    Secondly, we show that~$(C \cap B_i) \setminus (A_i \cup D)$ cannot contain a neighbour of~$S_i$ for large enough~$i$, which yields the desired emptiness of~$(C \cap B_i) \setminus (A_i \cup D)$ for large enough~$i \in \cI$.
    
    First, let us prove that if~$(C \cap B_i) \setminus (A_i \cup D)$ is non-empty for~$i \in \cI$ with~$i \ge I_1$, then it contains a neighbour of~$S_i$.
    It is easy to check that $(A_i \cup D, C \cap B_i)$ is again a separation of~$G$ with separator~$(C \cap B_i) \cap (A_i \cup D)$.
    As shown above, the two sets~$((C \cap B_i) \setminus S_i) \cap A_i$ and~$((C \cap B_i) \setminus S_i) \cap D$ are empty since~$i \in \cI$ with~$i \ge I_1$.
    Thus, the separator of $(A_i \cup D, C \cap B_i)$ is in fact~$S_i$.
    Both~$(C \cap B_i) \setminus (A_i \cup D)$ and~$(A_i \cup D) \setminus (C \cap B_i)$ are non-empty, where the latter follows from~$(A_i, B_i)$ being tight and hence~$A_i \setminus B_i$ being non-empty.
    So since~$(A_i \cup D, C \cap B_i)$ is a separation of the connected graph~$G$, its separator~$S_i$ has to be non-empty and~$(C \cap B_i) \setminus (A_i \cup D)$ contains a neighbour of~$S_i$.
    
    Secondly, let us show that no neighbour of $S_i$ is contained in~$(C \cap B_i) \setminus (A_i \cup D)$ for large enough~$i$.
    ...
    Thus, it suffices to argue that no neighbour of~$S$ is contained in~$(C \cap B_i) \setminus (A_i \cup D)$ for large~$i$.
    Consider any neighbour~$u$ of~$S$ which is contained in~$C$, and hence in~$A$, since~$(C, D) \le (A, B)$.
    Then there exists~$I_u \in \N$ such that~$u \in A_i$ for all~$i \ge I_u$, and hence~$u \notin B_i \setminus A_i \supseteq (C \cap B_i) \setminus (A_i \cup D)$ for~$i \ge I_u$.
    ...
    Then~$(C \cap B_i) \setminus (A_i \cup D)$ contains no neighbour of~$S$ for all~$i \ge I_3$.
...
\begin{proof}
    ...
    Since~$\vs_i \in P_i$ and~$\sv_{j-1} \in P_j$ by~\cref{item:Orientation}, the consistency of~$P_i$ and~$P_j$ implies that~$\sv_\ell \in P_i$ and~$\vs_\ell \in P_j$, so~$s_\ell$ distinguishes~$P_i$ and~$P_j$.
    ...
    So such~$P_k$ orient~$r$, and both~$P_k$ and~$P_{k+1}$ contain the same orientation of~$r$ by~\cref{item:DistinguishingPairwise}.
...
\end{lemma}
\begin{proof}
    We recursively find a function~$f\colon \N \to \N$ and a strictly increasing sequence~$(\vs_i')_{i \in \N}$ in~$\vN$ which is strongly~$\cP$-relevant such that for every~$k \in \N$, we have
    \begin{enumerate}
    ...
    We now proceed by defining~$f(\ell+1)$ and~$\vs_i'$ for the remaining~$i = f(\ell) + 1, \dots, f(\ell+1)$ such that~$(\vs_i')_{i \le f(\ell+1)}$ satisfies the properties \cref{prop:siissubsequencestrong} and~\cref{prop:stronglyOrelevant} for every~$k \le \ell+1$.
    ...
    We cannot have~$\rv < \vs_{\ell+1}$, as we would otherwise have a contradiction to the consistency of the pre-tangle~$P_{\ell+1}$ in~$G$ which contains both~$\vr$ and~$\vs_{\ell+1}$.
    For~$\vs_{\ell+1} < \vr$, observe that~$r$ cannot distinguish~$P_{\ell+1}$ and~$Q_{\ell+1}$ as they are efficiently distinguished by~$s_{\ell+1}$ and~$|r| < |s_{\ell+1}|$.
    Thus, $\vr \in P_{\ell+1}$ implies~$\vr \in Q_{\ell+1}$.
    But by the choice of $Q_{\ell+1}$ it also contains~$\sv_{\ell+1}$ which then contradicts the consistency of~$Q_{\ell+1}$.
...
Consequently, we say that an end~$\omega$ of~$G$ is \emph{tied} to a set~$U \subseteq V(G)$ if there exists a comb with teeth in~$U$ whose spine is in~$\omega$.\footnote{An end is tied to a set~$U$ if and only if it is contained in its topological closure~$\bar{U}$ in~$|G|$ (cf.~\cite{DiestelBook16}*{Section~8.6}).}
It is immediate from this definition that if~$\omega$ is tied to~$U$ and~$(A, B)$ is a separation in the pre-tangle~$P_\omega$ in~$G$ induced by~$\omega$, then infinitely many vertices of~$U$ are contained in~$B$.
Similarly, if~$\{A, B\}$ is a finite-order separation of~$G$ and the end~$\omega$ is tied to~$B$, then~$(A, B) \in P_\omega$.
...
Then \cref{lem:StarComb} implies that some end of the graph is tied to the separator of such a limit separation.
The next lemma shows that this end is unique if the sequence is contained in a nested set distinguishing all pre-tangles.
...
\begin{lemma} \label{lem:UniqueEnd}
    ...
    If~$((A_i, B_i))_{i \in \N}$ is non-exhaustive and~$N$ distinguishes all pre-tangles in~$G$ that are induced by ends tied to~$A \cap B$, then precisely one end of~$G$ is tied to~$A \cap B$.
\end{lemma}
\begin{proof}
    Since~$((A_i, B_i))_{i \in \N}$ is non-exhaustive, $A \cap B$ is infinite by~\cref{lem:LimitSepHasInfiniteOrder}.
    So at least one end of~$G$ is tied to~$A \cap B$ by~\cref{lem:StarComb}.
    Suppose for a contradiction that two distinct ends~$\omega$ and~$\omega'$ of~$G$ are tied to~$A \cap B$, and let~$P_\omega$ and~$P_{\omega'}$ be the pre-tangles in~$G$ that are induced by~$\omega$ and~$\omega'$, respectively.
    ...
    To show that~$\{C, D\}$ crosses~$\{A, B\}$, observe that there exist infinitely many elements of~$A \cap B$ in~$C$, since~$\omega$ is tied to~$A \cap B$.
...
It asserts that given a non-exhaustive strictly increasing sequence in a nested set distinguishing all pre-tangles of a locally finite graph, the (by~\cref{lem:UniqueEnd} unique) end which is tied to the separator of its limit separation cannot be thin.
...
\begin{theorem} \label{thm:ThickEnd}
    ...
    If~$((A_i, B_i))_{i \in \N}$ is non-exhaustive and~$\cP$ contains the pre-tangle in~$G$ which is induced by the unique end~$\omega$ of~$G$ tied to~$A \cap B$, then~$\omega$ is thick.
\end{theorem}
\begin{proof}
    ...
    Let~$P_\omega$ be the pre-tangle in~$G$ which is induced by the (unique) end~$\omega$ tied to~$A \cap B$.
    ...
    \begin{claimproof}
        ...
        To see~$(A_i', B_i') \in P_\omega$, recall that~$(A_i', B_i') \le (A, B)$ by~\cref{lem:SeparationAboveLimit}.
        Thus, $A \cap B$ is contained in~$B_i'$.
        Since~$\omega$ is tied to~$A \cap B$, it is thus tied to~$B_i'$ as well, and this implies~$(A_i', B_i') \in P_\omega$ by definition.
        ...
        Then~$(A_i', B_i') < (C, D) \le (B, A)$, but we also have~$(A_i', B_i') \le (A,B)$ by~\cref{lem:SeparationAboveLimit} which contradicts the fact that~$\{A_i', B_i'\}$ is tight and hence proper.
...
\begin{lemma}[\cite{InfiniteSplinters}*{Lemma~6.1 and its proof}] \label{lem:EfficientDistinguishersAreTight}
    ...
    Then there exists a tight component~$K$ of~$G - (A \cap B)$ with~$K \subseteq B \setminus A$.
...
\begin{proof}[Proof of \cref{main:Technical}]
    ...
    Since every tangle in~$G$ is also a pre-tangle in~$G$, \cref{thm:ThickEnd} implies that the unique end tied to~$A \cap B$ is thick, as desired.
\end{proof}
...
Since profiles are also pre-tangles, one can then obtain direct analogues of~\cref{main:Theorem} and~\cref{main:Technical} for sets~$\cP$ of profiles with the same proofs.
...
In this appendix we prove a structurally refined version of~\cref{thm:ThickEnd}:
while~\cref{thm:ThickEnd} asserts that the (unique) end~$\omega$ of~$G$ tied to the separator of the limit separation~$(A, B)$ is thick, we here show in~\cref{prop:ThicknessBeyondLimit} that there are infinitely many disjoint rays in~$G[B \setminus A]$ which are contained in the end~$\omega$; in other words, these rays witness the thickness of~$\omega$ `beyond' the limit separation~$(A, B)$.
...
\begin{proposition} \label{prop:ThicknessBeyondLimit}
    ...
    If~$((A_i, B_i))_{i \in \N}$ is non-exhaustive, then the unique end~$\omega$ of~$G$ which is tied to~$A \cap B$ contains an infinite family of disjoint rays in~$G[B \setminus A]$.
\end{proposition}
\begin{proof}
    ...
    By the ubiquity of rays~\cite{halin65}*{Satz~1},
    it suffices to show that for every~$k \in \N$, there exist~$k$~disjoint rays in~$G[B \setminus A]$ which belong to~$\omega$ in~$G$.
    ...
    Thus, König's infinity lemma~\cite{InfLemma}*{VI. Kapitel~\S\,2,~Satz~6}
    ...
    Hence, every ray in~$\cR_i$ is contained in~$G[B'_i \setminus A'_i]$ except its startvertex, since~$\omega$ is tied to~$A \cap B \subseteq B'_i$. 
    ...
    We now inductively exhibit infinitely many disjoint paths in~$G$ joining~$R$ and~$Q$ which then yields that they are in same end of~$G$ and, in particular, that $R \in \omega$.
    ...
    We then let~$P_{j+1}$ be the (unique) path with first follows~$Q_{j+1} - R^*$ and then~$P_{j+1}'$.
...
\begin{example} \label{ex:RaysNotEquivalent}
    ...
    For every~$n \in \N$, we pick a copy of the complete graph~$K^{2^{n+2}}$ on~$2^{n+2}$ vertices.
    In this~$K^{2^{n+2}}$, we fix~$2^n$ vertices~$u_1^n, \dots, u_{2^n}^n$ and~$2^{n+1}$ vertices~$v_1^n, \dots, v_{2^{n+1}}^n$ all of which are distinct.
    We then identify~$u_i^{n+1}$ with~$v_i^n$ for all~$n \in \N$ and $i \leq 2^{n+1}$.
    Furthermore, we pick a ray~$R^n = r_1^n r_2^n \dots$ for every~$n \in \N$ such that these rays are disjoint from each other and from the graph constructed before.
    Then for every~$n \in \N$ and~$m \le n$, we add an edge joining~$r_n^m$ and~$u_1^{n+1} = v_1^n$.
    This completes the construction of the graph~$G$.

    Each of the~$K^{2^{n+2}}$ induces a tangle~$P_n$ in~$G$.
    It is easy to check that the tight separation~$s_n$ with separator~$\{v_1^i \mid i < n\} \cup \{v_j^n \mid j \le 2^{n+1}\}$ efficiently distinguishes~$P_n$ and~$P_{n+1}$.
...
The idea is to join each of the infinitely many disjoint rays~$R^n$ to a cofinal subset of~$A \cap B$ in such a way that every vertex in~$A \cap B$ is connected to finitely many~$R^n$ and that every two rays~$R^n$ and~$R^m$ share only finitely many neighbours in~$A \cap B$.
