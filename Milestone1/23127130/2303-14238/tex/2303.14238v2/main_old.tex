...
Chemical reaction network (CRN) theory offers a versatile mathematical framework in which to model complex systems, ranging from biochemistry and game theory to the origins of life \cite{veloz2014reaction,smith2016origin}. The usefulness of CRNs in modelling these phenomena stems from their ability to exhibit a wide range of nonlinear dynamics and it is widely recognized that \textit{autocatalysis} can be seen as a basis for many of them \cite{epstein1998introduction,schuster2019special}. Broadly, autocatalysis can be framed as the ability of a given chemical species to make more of itself or catalyze its own formation. Thus, from the perspective of kinetics, one would expect autocatalysts to show a regime of super-linear growth \cite{hordijk2004detecting,andersenadefining}. For some examples of nonlinear kinetics obtained by composing two autocatalytic cycles and their relevance to ecological dynamics, see Figure \ref{fig:Peng_summary}.    
...
For the above mentioned reasons, a comprehensive understanding of autocatalysis in CRNs has great practical values. However, the mathematical treatment of autocatalysis is also a primary challenge in CRN theory. In \cite{andersenadefining}, it is conjectured that it is impossible for a CRN to show a temporary speed-up of the reaction before settling down to reach an equilibrium if, at least, it is not \textit{formally} autocatalytic. Also, it is suggested in \cite{deshpande2014autocatalysis} that understanding the interactions between autocatalytic and drainable subnetworks might provide a basis for obtaining a proof for the long-standing \textit{persistence conjecture} \cite{feinberg1989necessary}. 

In a recent work \cite{blokhuis2020universal}, Blokhuis et al.\ have defined a notion of autocatalysis strictly in terms of the stoichiometric matrix without referring to the underlying CRN. While their notion is more restrictive than an intuitive notion of autocatalysis discussed in \cite{andersenadefining,deshpande2014autocatalysis}, it yields a richer mathematical analysis. Using their notion of autocatalysis, Blokhuis et al. show that there are only five types of minimal autocatalytic motifs. They also found that autocatalysis is more abundant than previously thought, and simple reaction networks with a few reactions can generically have a lot of minimal autocatalytic motifs contained in them. 

In this paper, we will extend their definition to subnetworks and provide tools for exhaustively enumerating them and understanding their combinatorics. The layout of the paper is as follows. In Section \ref{sec:CRN_AC}, we briefly review chemical reaction networks (CRNs) and a few different notions of autocatalysis. In Section \ref{sec:organization}, we develop a formalism for analyzing minimal autocatalytic subnetworks (MASs) that are detectable from the stoichiometric matrix and provide a linear-programming algorithm to exhaustively enumerate them.\footnote{An implementation can be found online at \cite{Blanco_Python_Codes_for_2023}.} We also propose a visualization scheme in which to represent their combinatorics and polyhedral geometry. In Section \ref{sec:applications}, we define the cluster chemical reaction network (CCRN) framework and argue that it is provides a natural framework in which to analyze realistic chemical reaction networks, i.e.\ those arising from chemistry. We then explore the combinatorics of MASs in this framework and provide a worked example to demonstrate the polyhedral geometry of autocatalysis in the stoichiometric subspace of the species concentrations. Multiple examples spread throughout this work are subnetworks from the CCRN framework. We conclude with a discussion in Section \ref{sec:discussion} of our contributions and possible avenues for future investigation. 


%----------------section end -----------------------------
\section{Chemical reaction networks and autocatalytic cycles}
\label{sec:CRN_AC}
The mathematical theory of chemical reaction networks (CRNs), pioneered by Horn, Jackson, and Feinberg \cite{horn1972general,feinberg2019foundations,yu2018mathematical}, has ubiquitous applications in understanding Nature. In particular, for biological and ecological applications, the formalism captures a notion of self-replication or \textit{autocatalysis} where a subset of species, when present, increase their own population. It is then natural to rigorously understand the organization of a collection or \textit{ecology} of autocatalytic cycles, and forms the basis for the investigation pursued in this work.   

In \cite{feinberg2019foundations}, Feinberg defines a CRN as a triple of the set of species, complexes, and reactions $(\mathcal{S},\mathcal{C},\mathcal{R})$, which we review in Appendix \ref{app:deficiency}. It has been argued in \cite{deshpande2014autocatalysis}, however, that the data of the set $\mathcal{C}$ of complexes is redundant as it can be recovered from the reaction set $\mathcal{R}$ and thus a CRN can be defined more economically as $(\mathcal{S},\mathcal{R})$. Since the two representations are equivalent, we will pick either, dictated by convenience. On the other hand, the concept of autocatalysis, dating back to the late 1800s \cite{ostwald1890autokatalyse,peng2022wilhelm}, has several distinct formulations in CRN theory, for a review of which we refer the readers to Andersen et al.\ \cite{andersenadefining}. While in our work, we closely follow the formulation of Blokhuis et al.\ \cite{blokhuis2020universal}, we contrast it against other formulations in Section \ref{sec:AC-motif}.  
...
\begin{align*}
    ...
    \mathcal{R} &= \{ \ce{$y$ -> $y'$}  \}
\end{align*}
The \textit{reaction hypergraph} of the CRN $(\mathcal{S},\mathcal{R})$ is the directed graph with vertex set $\mathcal{C}:= \{y,y' | y\to y'\in \mathcal{R}\}$ and edge set $\mathcal{R}$. The vertices, also referred to as \textit{complexes}, are multisets of species, and are denoted by a column vector $y$ representing the stoichiometry or multiplicity of each species in the complex. Note that this is purely topological data, and no reference is made to the kinetics or dynamics of a system specified by the CRN.    
...
$\mathcal{R}' \subset \mathcal{R}$ by $\mathbb{S}^{\mathcal{R}'}$. In general, for a subgraph $\mathcal{G}' = (\mathcal{S}',\mathcal{R}')\subset \mathcal{G}$, we denote the restriction of $\mathbb{S}$ to $\mathcal{G}'$ by $\mathbb{S}|_{\mathcal{G}'} := \mathbb{S}^{\mathcal{R}'}_{\mathcal{S}'}$. We define a \textbf{subnetwork} of a graph $\mathcal{G}$ to be the subgraph $(\mathcal{S}|_{\mathcal{R}'},\mathcal{R}')$ where $\mathcal{R}' \subset \mathcal{R}$ and $\mathcal{S}|_{\mathcal{R}'}$ indicates that there is no omission of species in the subnetwork that participates in the set of reactions $\mathcal{R}'$ of the original graph $\mathcal{G}$. 
...
\begin{align*}
    \sum^{\mathcal{R}}_{r=1} v_r \cdot   y_r &\to \sum^{\mathcal{R}}_{r=1} v_r \cdot y'_r. 
\end{align*}


The left null space of $\mathbb{S}$, i.e.\ vectors $\textbf{x}$ such that $\textbf{x}\cdot\mathbb{S} = 0$, correspond to conservation laws $\textbf{x} \cdot \Delta \textbf{n} = 0$, and $\textbf{x} \cdot \textbf{n}$ is an invariant for the network under an arbitrary flow. The vector $\textbf{x}$ is called a positive conservation law if \textit{all} its coefficients are nonnegative. Similarly, the right null space of $\mathbb{S}$, i.e. vectors $\textbf{w}$ such that $\mathbb{S}\cdot \textbf{w} = 0$, correspond to null flows which leave the population (concentration) of the species unchanged ($\Delta \textbf{n} = 0$). A careful accounting of the right null space of $\mathbb{S}$ provides a topological classification of a CRN, and we review it in Appendix \ref{app:deficiency}. 
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.8\linewidth]{figures/E-graph example 2_1.png}
  %\caption{E-graph}
  \label{fig:sub1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{figures/flow example 2_1.png}
...
\begin{align*}
    ...
    \ce{A + B -> 2B}.
\end{align*}
Then the species set $\mathcal{S} = \{ A, B\}$ and the reaction set $\mathcal{R} = \{ A + B \to 2 A , 2A \to A + B ,A + B \to 2B \}$. As can be read from $\mathcal{R}$, the complex set $\mathcal{C} = \{A + B, 2A, 2B \}$, and their corresponding vectors are  
...
\end{bmatrix},
\] with left null space (conservation laws) spanned by $\textbf{x} = [1, 1]$ and right null space (null flows) spanned by the basis $\{ [1, 1, 0 ]^T, [1, 0, 1 ]^T\}$. The stoichiometric subspace, defined as the span of the columns of the stoichiometric matrix, is the subspace of the change in species concentration under an arbitrary flow on $\mathcal{G}$. Notice that for this example it is one-dimensional and perpendicular to the conservation law. For a visual representation of this example, see Figure \ref{fig:test}. 
\end{example}
...
\begin{enumerate}
    ...
    \begin{align*}
        \text{(F)} + \textbf{m} \cdot \mathcal{M} \to \textbf{n} \cdot \mathcal{M} + \text{(W)}, 
    \end{align*}
...
\begin{enumerate}
    ...
    \item for every reaction $y \to y' \in \mathcal{G}$, the input complex $y$ contains at least one species from the set $\mathcal{M}$. This ensures that the flow is inadmissible, or there is no flow, if the population of any species in the set $\mathcal{M}$ is zero. 
\end{enumerate}
...
Note that this definition is consistent with an intuitive notion of autocatalysis and is cognate with those of Gopalkrishnan, Deshpande, Barenholz et al.\ \cite{deshpande2014autocatalysis,gopalkrishnan2011catalysis,barenholz2017design}, and others. 
...
\begin{enumerate}
    ...
    \item For every reaction $y \to y' \in \mathcal{A}_R$, at least one species from the set $\mathcal{A}_S$ is net consumed and another is net produced. Equivalently, each column in $\overline{\mathbb{S}}:=\mathbb{S}|_{\mathcal{A}}$ has at least one positive and one negative coefficient.
\end{enumerate}
Notice that the first condition is identical to that of \textit{exclusive autocatalysis}. However, the second condition, also termed \textit{autonomy} of submatrix $\overline{\mathbb{S}}$, ensures that the production of any species \textit{consumes} at least one other species of the motif, and disallows unconditional growth. The condition that there needs to be at least one autocatalytic species in the reactants distinct from the products makes this definition more restrictive than exclusive autocatalysis, due to which we term their criterion as \textbf{stoichiometric autocatalysis}. Stoichiometric autocatalysis can be simply inferred from the stoichiometric matrix without referring to the underlying graph. In the rest of our work we will adhere to their criterion and will simply use \textit{autocatalysis} to mean stoichiometric autocatalysis, unless specified otherwise. 

Following the terminology in \cite{deshpande2014autocatalysis}, if there exists a nonnegative flow such that $\overline{\mathbb{S}} \cdot \textbf{g} \prec 0$, we would term the subgraph as a \textit{drainable motif}, where the change in concentration of all the species in the subset is strictly negative. While we focus our attention on autocatalytic motifs, the results in our paper are easily extended to their drainable counterpart. In \cite{deshpande2014autocatalysis}, the authors prove that a network is persistent if it has no drainable motifs and also remark on the importance of these motifs for understanding the dynamics of a network in ecological terms. 

\subparagraph{Autocatalytic core:} An \textit{autocatalytic core} is a minimal autocatalytic motif that does not contain a smaller autocatalytic motif. In \cite{blokhuis2020universal}, it is shown that if $\mathcal{A} = (\mathcal{A}_C, \mathcal{A}_R)$ is an autocatalytic core, it has the following properties:
\begin{enumerate}
    ...
    \item Every forward reaction in $\mathcal{A}_R$ contains only one species from the core set $\mathcal{A}_C$ (possibly with multiplicity higher than one).
\end{enumerate}
Furthermore, the authors provide a geometric characterization of the autocatalytic cores and show that they can only be of five types. We label the autocatalytic subnetworks in Figure \ref{fig:cluster_n4} using their taxonomy, and for details we refer the readers to \cite{blokhuis2020universal}. 
...
\end{bmatrix}.
\] 
Notice that there is no autocatalytic core in this network, and thus a naive characterization by the Blokhuis et al.\ criteria would regard the network to be non-autocatalytic. On the other hand, from Example \ref{eg:CRN-exclusive}, we know that the network is exclusively autocatalytic. To remedy this, we \textbf{modify} the network by replacing each reaction whose reactant and product complex shares a species with two new reactions with a fictitious distinct intermediate species. This then yields a new network $\mathcal{G}^*$ given by 
\[
\ce{  2 B <- D <- A + B  <=> C <=> 2 A },
\]
where $C,D$ are the newly added fictitious species. The stoichiometric matrix for $\mathcal{G}^*$ is given by
\[ \mathbb{S}^* = \begin{bmatrix}
    -1              &  \phantom{-}2  &    -2        & \phantom{-}1 & -1         & \phantom{-}0\\
    -1              &   \phantom{-}0 & \phantom{-}0 & \phantom{-}1 & -1         & \phantom{-}2\\
     \phantom{-}1   &           -1   & \phantom{-}1 & -1           & \phantom{-}0 &\phantom{-}0\\
     \phantom{-}0   &   \phantom{-}0 & \phantom{-}0 &  0           & \phantom{-}1 & -1
\end{bmatrix}.
\] 
...
Even mildly complicated CRNs can exhibit an abundance of autocatalytic cores \cite{personalconv}. Recall from Section \ref{sec:definitions} that we define a \textit{subnetwork} of a graph $\mathcal{G} = (\mathcal{S},\mathcal{R})$ to be a subgraph $(\mathcal{S}|_{\mathcal{R}'},\mathcal{R}')$ where $\mathcal{R}' \subset \mathcal{R}$ and there is no omission of species in the subnetwork that participate in $\mathcal{R}$ in the original graph $\mathcal{G}$. Then, we define a \textit{minimal autocatalytic subnetwork} to be a subnetwork that contains exactly one autocatalytic core. In Section \ref{sec:definitions}, we demonstrate that firstly these subnetworks organize within equivalence classes, and secondly the subnetworks within an equivalence class can also be organized by analyzing their, what we call as, \textit{species-productive cones}. We prove some results about autocatalytic subnetworks in Section \ref{sec:results}, provide algorithms to exhaustively enumerate the minimal autocatalytic subnetworks as well as identify their species-productive cones in Section \ref{sec:algorithms}, and finally discuss their geometric structure and visualization scheme in Section \ref{sec:visualization}. It must be noted that, to simplify construction, we have assumed that the graph $\mathcal{G}$ is reversible throughout this section, i.e.\ for every reaction $y_1\to y_2 \in \mathcal{G}$, the reverse reaction $y_2 \to y_1$ is also in the reaction set of $\mathcal{G}$.  
...
\end{enumerate}
Notice that both these subnetworks have empty waste species and non-core member species sets.
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.8\linewidth]{figures/flow productive eg 3_2.png}
  %\caption{Flow-productive cone}
  %\label{fig:sub1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{figures/species productive eg 3_2.png}
...
\end{align}
By definition, an element in the interior of the flow-productive cone of $A$ corresponds to a flow vector for which $A$ is productive, i.e.\ the core member species of $A$ are strictly produced, and we refer to it as the \textit{flow productive region} of $A$. It must be noted that that the inverse of $\overline{\mathbb{S}}_C$ might contain negative coefficients. Since we are only considering reversible networks, negative flows on a reaction simply correspond to running the reverse reaction. In general, however, for non reversible networks the productive cone must be defined as \[\mathcal{F}(A) = \text{cone}(\text{cols}(\overline{\mathbb{S}}_C^{-1})) \cap \mathbb{R}_{\geq 0}^{A_{C}}.\]
...
\end{align}
Therefore, each vector in the interior of the species-productive cone for a MAS, which we refer to as the \textit{species-productive region}, is the change in concentration of each participating species for which the subnetwork is productive in the core member species. We remark that the \textit{species-consumptive cone} for the \textit{drainable subnetwork} $A^\text{op}$, obtained by reversing the edges of the autocatalytic subnetwork $A$, is the opposite of species-productive cone, i.e.\ 
...
\begin{example}
\label{eg:all-cones}
    In this example, we will identify the flow-productive, species-productive and partition-productive cone for the MAS $A_1$ from Example \ref{eg:FWMC} (shown in Figure \ref{fig:test}). For $A_1 = (\{A,B,C\}, \{A+B \to C, C \to 2 A \}$,
    \begin{align*}
        \overline{\mathbb{S}} &= 
        \begin{bmatrix}
            -1  & \phantom{-}2 \\
...
\begin{example}
    ...
    Consider two MASs (from Section \ref{sec:CCRN-1constituent} Figure \ref{fig:cluster_n4}) 
    \begin{align*}
...
\begin{remark}
    The converse statement, if the reversible graph of the union of two MASs contains a null cycle then their species-productive cones must intersect, is not true. For example, consider the MASs (from Section \ref{sec:CCRN-1constituent} Figure \ref{fig:cluster_n4})
    \begin{align*}
...
We propose an mathematical optimization-based approach to generate MASs in a CRN and to identify their intersecting productive cones. A flowchart of our procedure to construct the MAS is shown in Fig. \ref{fig:flowchart}. Given the stoichiometric matrix, the algorithm, in its first iteration computes an autocatalytic subnetwork by adequately selecting the core species and the reactions taking part of the subnetwork with minimum number of reactions. The obtained subnetwork is saved, and its FWMC partition is computed. Its tuple of  reactions is added to the set of \emph{incompatible} reactions, \texttt{Incomp}, and the procedure is repeated, by filtering the reactions in the subsequent obtained subnetworks to not contain any of the tuples in \texttt{Incomp}, until no feasible subnetworks with the \emph{compatibility} conditions are possible. Minimality  is assured by the minimization of the number of reactions in the subnetwork in each run, and also by the compatibility conditions.
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
Notice that any asymmetry in entries across the diagonal indicates that only one of the partition-productive cones completely contain the other. This matrix can then be visualized as a heat map, for example see Figure \ref{fig:intersection_partition_5}.

\begin{figure}
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/food_waste_L_5.png}
    \caption{Projections of species-productive cones.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/Fig_Class27_n5.png}
    \caption{Intersection information of MASs.}
    \end{subfigure}    
    \caption{All MASs belonging to the $\{\{\overline{4}\},\{\overline{1}\},\{\{\overline{2},\overline{3},\overline{5}\}\}\}$ class in the complete $L=5$ 1-constituent CCRN are considered. The projections of their species-productive cones on the food and waste species are shown in the top panel. The bottom panel displays the intersection information of MASs within a class analogous to Figure \ref{fig:intersection_partition_5}. Note that there are no MASs in the class that are disjoint, and thus the `disjoint' label is omitted. }
\label{fig:cone_intersection}
\end{figure}

Each equivalence class can contain several MASs. The species-productive cones of the MASs in a class are not always identical. While they will share the same projection on the core species, the productive regions in the non-core species can be very different. For example, let us pick the equivalence class $\{\{\overline{4}\},\{\overline{1}\},\{\{\overline{2},\overline{3},\overline{5}\}\}\}$ from the list of classes shown in Figure \ref{fig:intersection_partition_5}. From the figure, we know that it contains $5$ MASs. We plot the projection of the species-productive region in the non-core species in the top panel of Figure \ref{fig:cone_intersection}. In higher dimensions when more non-core members are involved, this representation can get rather cumbersome. Thus, we employ a similar visualization as $C_\text{pp}$ to depict the intersection of MASs within an equivalence class. Whether or not there is an intersection between the species-productive cones can be ascertained using our algorithm outlined in the previous subsection. For an example of the resulting visualization for the same equivalence class considered above, see the bottom panel of Figure \ref{fig:cone_intersection}. In the same manner, a visualization for the information of pairwise intersection of the productive cones of all MASs for a CRN can also be obtained, for example see Figure \ref{fig:all_intersect}. 
...
\begin{example}
    Consider the complete $L=5$ 1-constituent CCRN of order two. The list of all the FWMC classes, the number of MASs they contain, and their intersection information is summarized in Figure \ref{fig:intersection_partition_5}. Consider the class $\{\{\overline{4}\},\{\overline{1}\},\{\{\overline{2},\overline{3},\overline{5}\}\}\}$. It contains the MASs:
    \begin{align*}
    ...
    \end{align*}
\end{example}
The projection of the species-productive cones for the above MASs and their intersection information is summarized in Figure \ref{fig:cone_intersection}.
...
All realistic chemical reactions, in the absence of nuclear reactions, conserve the number of atoms of each type along a reaction. Often, but not always, reactions involving polymers also conserve the number of each monomer along every reaction. This motivates taking a \textit{coarse-grained} perspective, where we forget the bond structure of each molecule or polymer and only count the number of atoms or monomers, respectively. Following terminology from Chapter 7 of \cite{kelly2011reversibility}, we will refer to this coarse-grained description of each species as a \textit{cluster} (see Figure \ref{fig:clusterization}). This also allows us to coarse-grain a realistic CRN into a new CRN which has clusters as species and reactions between multisets of clusters induced from the reactions in the original CRN. We will call the resulting CRN as a \textit{cluster chemical reaction network} (CCRN) of the original CRN.


There are three advantages of considering CCRN descriptions of CRNs. First, consider a CRN whose species set consists of all polymers formed of $D$ monomers up to size $N$. Then, the CCRN induced by the CRN would reduce the number of species from exponential ($D^N$) to polynomial ($N^D$) in the size of the polymer, providing a significant computational advantage. Secondly, since each reaction in the original CRN induces a reaction in the CCRN with the same topology, the mapping would preserve autocatalytic subnetworks. In other words, if a CRN is autocatalytic, it's induced CCRN will also be autocatalytic. However, since it is a many-to-one map, multiple autocatalytic subnetworks of the CRN might map to the same autocatalytic subnetwork in its CCRN or new autocatalytic subnetworks in the CCRN might emerge. Thirdly, one can re-introduce structure into the CCRN by adding more species with the same conserved quantities. In this way, the coarse-graining can be systematically refined to recover the original CRN, and allows a gradual complexification of the model.
...
We formally define a CCRN in Section \ref{sec:CCRN-formalism}, and systematically explore the properties of CCRNs with one conserved quantity (type of atom or monomer) in Section \ref{sec:CCRN-complete}. We also provide the statistics of autocatalytic subnetworks found in such reaction networks and present a worked-out-example in Section \ref{sec:L_3_example}. We briefly discuss the computational challenges in scaling the algorithm for fully connected models in Section \ref{sec:comp_challenge} and introduce rule-generated CCRNs in Section \ref{sec:rule_gen}.  


\subsection{Formalism}
\label{sec:CCRN-formalism}
A \textit{cluster chemical reaction network (CCRN)} is a CRN with the additional structure that, upon excluding the inflow and outflow reactions, the network has at least one nonnegative integer conservation law (for the definition of a conservation law, see Section \ref{sec:hypergraphs}). Each conservation law corresponds to a \textit{constituent} that is conserved. We denote the number of distinct constituents by $D$ and label them as $A_1, \ldots, A_D$. The species of a CCRN, termed \textit{clusters}, are multisets of constituents and denoted by an overline (notation chosen to be consistent with \cite{liu2018mathematical}) over a vector of nonnegative integers representing the number of each constituent in the cluster. For instance, \[\overline{\textbf{n}} := \overline{n_1,\ldots,n_D}\] denotes the cluster comprising $n_1, \ldots, n_D$ of constituents $A_1, \ldots, A_D$, respectively. Notice that a unit of a constituent represents a unit of conserved quantity, thus a cluster is labelled by its set of conserved quantities. In case of multiple clusters with the same conserved quantities, we demarcate them with an asterisk $^*$. For example, in Table \ref{table:cluster-framework-rules}, $\overline{1}$ and $\overline{1}^*$ refer to a monomer and an activated monomer, respectively. Finally, we define the \textit{length} of a cluster to be the sum of its conserved quantities, \[l(\textbf{n}) := \sum_{i=1}^D n_i.\]       


In a CCRN, \textit{complexes} are multisets of clusters and denoted by \[c^\alpha := \sum_\textbf{n} c^\alpha_\textbf{n} \overline{\textbf{n}}.\] As an abuse of notation, we denote the stoichiometry of complex $c^\alpha$ also by $c^\alpha$, where now it is the column vector with entries $c^\alpha_\textbf{n}$. We define the \textit{size} of a complex to be its vector sum of conserved quantities of each type, and denote it as \[|c^\alpha| = \sum_\textbf{n} c^\alpha_\textbf{n}  \textbf{n}.\] We define the \textit{width} of a complex to be total number of clusters in the multiset, and denote it by \[w(c^\alpha):= \sum_\textbf{n} c^\alpha_\textbf{n}.\] In the CCRN, every reaction must be such that the size of the source and target complexes are identical. Thus, a reaction $c^\alpha \to c^\beta$ is allowed only if \[|c^\alpha| = \sum_\textbf{n} c^\alpha_\textbf{n}  \textbf{n}= \sum_\textbf{n} c^\beta_\textbf{n}  \textbf{n} = |c^\beta|.\] This restriction consistently ensures that the number of constituents across each reaction are conserved, as 
\[\sum_\textbf{n} \textbf{n}(c^\beta_\textbf{n} - c^\alpha_\textbf{n}) = 0.\] 
...
We define a \textit{complete CCRN} of length $L$ and order $w$ to consist of all species up to length $L$ and all possible reactions that are allowed by the conservation laws with complexes of width up to $w$. Since a reaction consisting of more than two reactants or products can be written as a chain of second-order reactions, for our analysis \textit{we will restrict to CCRNs of order two}.
...
\begin{tabular}{c|ccccc|ccccc}
   & \multicolumn{5}{c|}{CCRN}  & \multicolumn{5}{c}{$\#$ reactions}  \\
   Max.\ size & $|\mathcal{C}|$ & $|\mathcal{R}|$ & $\ell$ & $\imath$ & $\delta$ & 2 & 3 & 4 & 5 &6  \\
   \hline 
...
%    10 & 62 & 190 & 17 & 145 & 36 & & & &&\\
\end{tabular}
\caption{\label{table:1cluster} The number of minimal autocatalytic subnetworks for the complete 1 constituent cluster model of order two (also, see Figure \ref{fig:1_const_MASs_data}). For deficiency calculations, see Appendix \ref{app:kernel}.}
\end{table}
...
\begin{figure}
    ...
    \centering
    \includegraphics[width=\textwidth]{figures/graphics_4cluster_1.png}
    \caption{All FWMC classes and their MASs for $L=4$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/Fig_IntersectionPartitionCones_L4.png}
    \caption{Intersection information of FWMC classes.}
    \end{subfigure}    
    \caption{All minimal autocatalytic subnetworks (MASs) with for the complete 1-constituent CCRN with $L=4$ are shown in the top panel. The core types are also color coded using the typology of Blokhuis et al.\ in \cite{blokhuis2020universal}. The list of the equivalence classes with the information of their intersection is shown in the bottom panel. }
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{figures/L_3 basis.png}
    \caption{Stoichiometric subspace with the autocatalytic productive cones for L=3}
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{figures/L_3 fixed points.png}
...
Catalyzed activation &    \ce{$\overline{C} + \overline{1}$ <=>  $\overline{1}^*+\overline{C}$}       \\ \\
  ...
  \end{cases}$\\ \\
  \hline \\
  Vesicle fusion & \ce{$\overline{j} + \overline{k}$ <=>  $\overline{j+k}$ }\\
  Vesicle fission & \ce{$\overline{j}$ <=>  $\overline{j-k} + \overline{k}$ } \\
  \\
  \hline
    \end{tabular}
...
\begin{align*}
    \ce{$\overline{j,k}+\overline{1,0}$ <=>  $\overline{j+1,k}$},\\
        \ce{$\overline{j,k}+\overline{0,1}$ <=>  $\overline{j,k+1}$}.
\end{align*}
In realistic chemistry, however, a monomer addition to a polymer is only done by an activated monomer and not an unactivated monomer. Thus, we want to distinguish activated and unactivated monomers in our model, for which we can add extra species $\overline{1,0}^*$ and $\overline{0,1}^*$. The resulting polymerization reactions and other examples of rule generated CCRNs are shown in Table \ref{table:cluster-framework-rules}. 
...
In this work, we began by reviewing the notions of autocatalysis in literature in Section \ref{sec:AC-motif}. We term the autocatalysis that can be detected from the stoichiometric matrix as \textit{stoichiometric autocatalysis}, and show that it is more constrained than \textit{formal} or \textit{exclusive} autocatalysis. Any system showing stoichiometric autocatalysis is formally and exclusively autocatalytic, which means that enumeration of stoichiometrically autocatalytic subnetworks can only underestimate the autocatalytic structure in a CRN. 

In \cite{blokhuis2020universal}, it was shown that there are exactly five types of minimal stoichiometrically autocatalytic cores, where a core is obtained by only selecting a few species from the complete set of species that participate in the autocatalytic reactions. In Section \ref{sec:organization}, we extend their work by defining stoichiometric autocatalysis for autocatalytic subnetworks defined on the complete set of participating species in the reaction network. We show that for any such subnetwork, the species set can be uniquely assigned a \textit{food-waste-member-core} partition, and define geometrically relevant quantities for them. We then derive mathematical results about these quantities, and provide an algorithm for exhaustively enumerating all minimal autocatalytic subnetworks (MASs). We also show how to take the list of MASs and obtain a visual summary of their combinatorics. An implementation can be found online at \cite{Blanco_Python_Codes_for_2023}.

In Section \ref{sec:applications}, we define the \textit{cluster chemical reaction network} (CCRN) framework. Since CCRNs are defined using conservation laws, we argue that this a natural framework for real CRNs. We then use the tools developed in Section \ref{sec:organization} to organize a list of MASs for maximally connected 1-constituent CCRNs with $L$ species and show that their population increases exponentially with $L$. While the fully connected CCRNs might not have physical relevance, in section \ref{sec:rule_gen} we explain how to obtain sparse CCRNs using rules induced by real polymer or biological chemistry.  


Our work opens up a few lines of future research, namely:
\begin{enumerate}
    \item For a simple network in Example \ref{eg:CRN-1}, proven to be exclusively autocatalytic in Example \ref{eg:CRN-exclusive}, we proposed a modification such that the resulting graph was stoichiometrically autocatalytic in Example \ref{eg:CRN-extension}. What is a general algorithm to modify any exclusively autocatalytic motif to a stoichiometrically autocatalytic motif? Having such an algorithm would allow us to use the typology of \cite{blokhuis2020universal} and the results in this work to understand the autocatalytic properties of any CRN. 
    \item While in the CCRN formalism we restricted the clusters to have nonnegative constituents, one can also consider the clusters defined on the complete integer lattice. The resulting reaction network could then be used to model real nuclear reactions. For example, if the positive conserved quantity corresponds to positive charge or subatomic particle type, the negative conserved quantity would refer to a negative charge or anti-particle, respectively.    
    \item The generation of MASs, for general CRNs, and in particular for CCRNs, is computationally costly. Our optimization-based approach requires solving a series of binary mathematical programming problems with an increasing number of linear constraints. Further research includes the particular study of these optimization problems, in particular, its polyhedral properties that will allow to strengthen the formulation and solve them more efficiently. Exploiting the algebraic properties behind conservation laws of sequences of integer numbers would also lead us to understand the mathematical insights of these networks.
    \item Realistic CRNs will not coarse-grain to a fully connected CCRN, thus appreciably reducing the number of autocatalytic cycles. As a future investigation, similar to \cite{anderson2022prevalence}, can we consider randomly generated CCRNs in an Erdos-Renyi framework and compute a bound on the number of autocatalytic cycles which persist as a function of the probability of a reaction being allowed in the network? 
    \item CCRNs are defined in terms of their conservation laws, and have more degrees of symmetry than generic CRNs. Having exhaustively enumerated the stoichiometrically autocatalytic subnetworks of a complete CCRN, can we fully characterize the kinetics that result from their compositions (also see Figure \ref{fig:Peng_summary})?        
\end{enumerate}
...
We want to thank Connor Simpson for help with combinatorics and programming, Alex Plum, Tymofii Sokolskyi, and Zhen Peng for biological discussions, and Asvin G., Dave Auckly, Diego Rojas, Gautam Neelakantham, Jeff Linderoth, and Vladimir Sotirov for mathematical discussions. This work was partially funded by the National Science Foundation, Division of Environmental Biology (Grant No: 2218817). V. Blanco acknowledges the financial support by the Spanish Ministerio de Ciencia e Innovaci\'on AEI/FEDER grant number PID2020-114594GBC21 and IMAG-Maria de Maeztu grant CEX2020-001105-M /AEI /10.13039/501100011033, Junta de Andalucía projects P18-FR-1422/2369, AT 21\_00032, FEDERUS-1256951, B-FQM-322-UGR20, and UE-NextGenerationEU (ayudas de movilidad para la recualificaci\'on del profesorado universitario).

\appendix

\section{Deficiency theory and multistability in CRNs}
\label{app:deficiency}
In \cite{feinberg2019foundations}, a CRN is defined by the triple $\{\mathcal{S},\mathcal{C},\mathcal{R}\}$, where $\mathcal{S}, \mathcal{C},$ and $\mathcal{R}$ are the set of species, complexes, and reactions respectively. Recall from Section \ref{sec:hypergraphs} that a CRN is a hypergraph with vertices in the set $\mathcal{S}$, multisets, or what we term, hypervertices in the set $\mathcal{C}$, and hyperedges in $\mathcal{R}$. The stoichiometric matrix $\mathbb{S}$ is defined such that (see Eq. \ref{eq:stoich_mat})
\begin{align*}
    \text{cols}(\mathbb{S}) &= \{y'-y|y\to y' \in \mathcal{R}\}. \nonumber
\end{align*}


Alternatively, $\mathbb{S}$ can be written as the product
$$\mathbb{S} = Y \phantom{.} \mathbb{M},$$
where the columns of $Y$ are the stoichiometries of the hypervertices 
\begin{align*}
...
Let $\mathcal{H}_1^L = (\mathcal{S},\mathcal{C},\mathcal{R})$ denote a complete 1-constituent CCRN of length $L$ and order two. 1-constituent CCRN of length $L$ means that the species set consists of clusters up to length $L$, denoted as $\mathcal{S} = \{ \overline{1}, \overline{2}, \ldots, \overline{L}\}$ (for a detailed definition, see Section \ref{sec:CCRN-1constituent}.) For a diagrammatic representation of the reaction network, see Figure \ref{fig:1-cluster-complexes}. Here, following Appendix \ref{app:deficiency}, we will perform the necessary calculations to calculate the kernel of the stoichiometric matrix for a general length $L$.
