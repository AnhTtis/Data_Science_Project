...
\thanks{C.G. is supported by a Klarman Postdoctoral Fellowship at Cornell University.}
...
\email{\href{mailto:gaoyibo@umich.edu}{{\tt gaoyibo@umich.edu}}}
...
\end{abstract}
\keywords{Kazhdan--Lusztig polynomial, Schubert variety, singular locus, intersection cohomology}
\maketitle
...
\begin{align*}
    ...
    &= \min_{y \leq w} \min \{i>0 \mid IH^i(X_w)_y \neq 0\}.
\end{align*}
...
Our first main theorem is a refinement and proof of Conjecture~\ref{conj:bp-conjecture}.
\begin{theorem}
...
Let $G$ be simply-laced, then the Schubert variety $X_w \subset G/B$ is smooth if and only if $w$ avoids the following patterns:
\begin{enumerate}
...
The following is a translation of Theorem~\ref{thm:bp-smoothness} in light of our conventions for patterns.
...
As we split these roots according to Lemma~\ref{lem:split-inversion-all}, $e_a$ does not appear on the left hand side because $\pm a$ do not belong to the interval $(b,c)$ and $w(\pm a)$ do not belong to the interval $(w(\bar d),w(c))$. Thus, we obtain a nontrivial relation among labels below $w$, so $h(w)=1$.
...
If polynomials $f_1,f_2\in\Z_{\geq0}[q]$ are both top-heavy, then we have $h(f_1f_2)=\min(h(f_1),h(f_2))$.
\end{lemma}
...
\end{figure}
Let us now consider \[(e_d-e_{\bar a})+(e_{\bar a}-e_{\bar b})+(e_{\bar b}-e_{c})=(e_d-e_{b})+(e_{b}-e_a)+(e_a-e_{\bar c})\] that corresponds to the red dashed line in Figure~\ref{fig:21bar4bar3},
and split these roots according to Lemma~\ref{lem:split-inversion-all}. On the left hand side, there is a term $e_k-e_{\bar c}$ for $\bar c<k\leq\bar b$ where $w\gtrdot wr_{e_k-e_{\bar c}}$. However, on the right hand side, $c$ or $\bar c$ can only appear in the form of $e_l-e_{\bar c}$ for $l>\bar b$. As a result, we obtain a nontrivial linear relations among the positive roots that label covers below $w$. Lemma~\ref{lem:relation-corank1} implies that $h(w)=1$.
...
\end{figure}
Let $J=\{0,1,2,\ldots,n-2\}\subset S$. If $h(w_J)\leq2$, then $h(w)\leq h(w_J)\leq 2$ and we are done, so assume $h(w_J)\geq3$. Recall that group multiplication is an injection $[e,w^J]^J\times [e,w_{J}]\hookrightarrow [e,w]$. Let \[U=[e,w]\setminus [e,w^J]^J\times[e,w_J].\] In other words, $u\in U$ if and only if $u\leq w$ and $u_J\nleq w_J$. 

Since $w(n)=\bar n$, $L^J(w^J)=L^J(w_0^J)$ is palindromic, as the Poincar\'{e} polynomial of $G/P_J$, and thus $h(L^J(w^J)L(w_J))=h(w_J)\geq3$. For any $u\in U$, we have $u_J\neq w_J$, and as $w_J(n{-}1)=\overline{n{-}1}$, Claim~\ref{claim:greater-than-rank2} implies that $\ell(u_J)\geq3$, and thus $\ell(u)\geq3$. Now consider an explicit element $v=ws_{n-2}s_{n-1}$ with length $\ell(w)-2$. Note that the index $\pm(n{-}1)$ is either $\pm b$ or inside the region $A$. As $v(\bar n)=w(\overline{n{-}2})\leq n-4$, $\ell(v^J)\leq 2n-6$ and $\ell(v_J)=\ell(v)-\ell(v^J)\geq \ell(w)-2n+4$. At the same time, $\ell(w_J)=\ell(w)-\ell(w^J)=\ell(w)-2n+2$. Via this calculation on length, $v_J\nleq w_J$ so $v\in U$. Consider the Poincar\'{e} polynomial
...
\end{claim}
\begin{proof}
Since $w$ is singular, but avoids the specified patterns, $w$ must contain $3412$ by Theorem~\ref{thm:bp-smoothness}. Since $w_K$ and $\prescript{}{K}{w}$ are smooth, any occurrence of $3412$ must use an index $\pm n$ as well as a value $\pm n$. Any such occurrence is related to one of the desired form by the symmetry $w(-i)=-w(i)$ or by inversion of $w$ except one of the form $w(-n)w(i)w(j)w(k)=cnab$. In this case, if $-c<a$, then $nab\bar{c}$ is an occurrence of $4231$ in $w$, and if $-k>i$, then $cab\bar{n}$ is an occurrence of $4231$. Since this contradicts our assumption, we must instead have $-c>a$ and $-k>i$; in this case $\bar{b}na\bar{c}$ is a justified occurrence of $3412$. 
\end{proof}

\begin{claim}
\label{claim:j-b-pm-1}
Let $cnab=w(i)w(j)w(k)w(n)$ be a justified occurrence of $3412$ in $w$, then $j= \pm1$ and $b=\pm 1$.
\end{claim}
\begin{proof}
Assume we have chosen a justified occurrence with $c$ minimal and $a$ maximal among occurrences with $w(i)=c$. 
...
\node at (2.5,2.5) {$D$};
\end{tikzpicture}
\caption{The permutation diagram of $w$ and the indicated regions, as discussion in the proof of Lemma~\ref{lem:typeA-3412-lower}.}
\label{fig:3412-lower-bound}
\end{figure}
Since $w$ avoids $4231$, the elements in $A$ must be in decreasing.

If $B=\emptyset$, letting $J=\{2,3,\ldots,n-1\}$ we have $w^J=d123\cdots n=s_{d-1}s_{d-2}\cdots s_2s_1$ where $d=w(1)$. At the same time, when $B=\emptyset$, $w_J(1)=1$ and $2,3,\ldots,d$ appear in reversed order in $w_J$, meaning that $s_2,\ldots,s_{d-1}\in D_L(w_J)$. Thus, $\supp(w^J)\cap J\subset D_L(w_J)$, and $w=w^Jw_J$ is a BP-decomposition, so $L(w)=L^J(w^J)L(w_J)$ by Theorem~\ref{thm:bp-decomposition-fiber-bundle}. As $L^J(w^J)=1+q+\cdots+q^{d-1}$ is palindromic, by Lemma~\ref{lem:min-of-h} we have $h(w)=h(w_J)$. Moreover, $B=\emptyset$ implies that no $3412$ uses $(1,w(1))$ so $\mh(w)=\mh(w_J)$. By induction, $h(w)=h(w_J)\geq\mh(w_J)=\mh(w)$. Similarly, if $C=\emptyset$, we can apply the induction hypothesis to $w^{-1}$.
...
\end{claim}
\begin{proof}
If $A=\emptyset$, then $D=\emptyset$. Clearly $\ell(w)-\ell(u)\geq1$ since $u\leq w$ and $w\notin U$. 
...
\end{align*} as shown in Figure~\ref{fig:3412-upper-bound}. Take $r=9$ with $m=2$, \begin{align*}
...
Theorem~\ref{thm:type-A-formula} now follows from Proposition~\ref{prop:h-equals-1}(i), Lemma~\ref{lem:typeA-3412-upper} and Lemma~\ref{lem:typeA-3412-lower}.


\section*{Acknowledgements}
We are grateful for our very fruitful conversations with Alexander Woo and with Sara Billey.
