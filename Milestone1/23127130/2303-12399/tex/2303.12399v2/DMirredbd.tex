...
Since when we fix a finite extension $K/F$ and a Drinfeld module $\phi$, the numbers $N_d$ and $\Omega_\phi$ are fixed. Elementary Calculus can tell us that the fraction $\frac{q^{\deg_T\ell}}{\deg_T\ell^{N_d}}$ tends to infinity as $\deg_T\ell$ goes to infinity. Thus we can always find a real number $C_{\phi,d}$ such that $\deg_T\ell>C_{\phi.d}$ implies $\frac{q^{\deg_T\ell}}{\deg_T\ell^{N_d}}>q^{\Omega_\phi}$. Now we try to solve $C_{\phi,d}$ explicitly:


\begin{lem}\label{num}

Let $a, b,$ and $c$ be positive real numbers such that $c^{1/b}\cdot\frac{b}{{\rm ln}a}\geqslant e$, here $e$ is the Euler's number. Then 
$$x>\frac{-b\cdot W_{-1}(\frac{-{\rm ln}a}{c^{1/b}\cdot b})}{{\rm ln}\ a}$$
is a solution to the inequality $$\frac{a^x}{x^b}>c.$$

Here ${\ln(\cdot)}:={\rm log}_e(\cdot)$ and $W_{-1}$ is the negative brach of the real-valued Lambert $W$-function, i.e. the inverse function of the complex valued function $f(y)=ye^y$.
...
\begin{proof}
First of all, we solve the equation
$$\frac{a^x}{x^b}=c.$$
We have
$$\begin{array}{lll}
&&\frac{a^x}{x^b}=c\\
\ \\
&\Rightarrow&x{\rm ln} (a)={\rm ln}(c)+b{\rm ln }(x)\\
\ \\
&\Rightarrow&x\cdot\frac{{\rm ln}(a)}{b}={\rm ln} (c^{1/b})+{\rm ln }(x) \\
\ \\
 (\textrm{set } \tilde{x}=x\frac{{\rm ln}(a)}{b}) &\Rightarrow& \tilde{x}={\rm ln} (c^{1/b}\cdot \frac{b}{{\rm ln}(a)}) +{\rm ln}(\tilde{x})\\
\ \\
(\textrm{set } \tilde{c}=-{\rm ln}(c^{1/b}\frac{b}{{\rm ln}(a)}) )&\Rightarrow& -\tilde{x}+{\rm ln}(\tilde{x})=\tilde{c} \\
\ \\
&\Rightarrow& -\tilde{x}e^{-\tilde{x}}=-e^{\tilde{c}}\\
\ \\
(\textrm{Here we use condition } c^{1/b}\cdot\frac{b}{{\rm ln}(a)}\geqslant e)&\Rightarrow& -\tilde{x}=W_{-1}(-e^{\tilde{c}})\\
\ \\
&\Rightarrow& x=\frac{-bW_{-1}(\frac{-{\rm ln}(a)}{c^{1/b}b})}{{\rm ln}(a)}

\end{array}$$


Now from elementary Calculus, we can see that the function $g(x)=\frac{a^x}{x^b}$ is increasing whenever $x>\frac{b}{{\rm ln}(a)}$. On the other hand, the output value of $-W_{-1}(\frac{-{\rm ln}(a)}{c^{1/b}\cdot b})$ is at least $1$. Thus we have
$$x>\frac{-b\cdot W_{-1}(\frac{-{\rm ln}(a)}{c^{1/b}\cdot b})}{{\rm ln}\ a}\geqslant \frac{b}{{\rm ln}(a)}.$$

Thus we get
$$\frac{a^x}{x^b}>c, \textrm{ whenever } x>\frac{-b\cdot W_{-1}(\frac{-{\rm ln}a}{c^{1/b}\cdot b})}{{\rm ln} (a)}.$$
\end{proof}
...
and $$C_{\phi,d}=\frac{-b\cdot W_{-1}(\frac{-{\rm ln}(a)}{c^{1/b}\cdot b})}{{\rm ln}(a)}.$$ 
Where  $a=q$, $b=10(d+1)^7$, and $c=q^{\Omega_\phi}$
\end{cor}

\section*{Acknowledgement}

The author would like to thank Sophie Marcques for inspiring discussions, and Wei-Hung Su for showing me how to solve the inequality in Lemma \ref{num}.
