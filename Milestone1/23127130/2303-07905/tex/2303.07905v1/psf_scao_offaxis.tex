...
\usepackage[backend=bibtex,defernumbers=true,sorting=nty,maxbibnames=99]{biblatex}
...
\addbibresource{psf_scao_offaxis}
...
\end{equation}
where $S$ is the telecsope area, $P({\bf x})$ is the pupil aperture function and the structure function of the residual incoming phase 
...
Since the AO system can only correct a limited number of modes, a split of $\phi$ into a part in the space of mirror modes, $\phi_\|$, and a part orthogonal to these modes, $\phi_\perp$, was proposed in \cite{VeRiMaRo97PSF,WaHoRa18}. Clearly, $\phi = \phi_\| + \phi_\perp$ and thus from \eqref{eq:sf} we get:
\begin{equation}\label{eq:sf_split}
D_\phi ({\bf x},{\boldsymbol \rho})= D_{\phi_\|}({\bf x},{\boldsymbol \rho}) + D_{\phi_\perp} ({\bf x},{\boldsymbol \rho})+ 2 \langle [\phi_\|({\bf x},t) -\phi_\| ({\bf x} + {\boldsymbol \rho},t)][\phi_\perp({\bf x},t) - \phi_\perp ({\bf x} + {\boldsymbol \rho},t) ] \rangle_t.
\end{equation}
Note that in \cite{VeRiMaRo97PSF} this splitting was done after interchanging spatial average and the exponential in \eqref{OTF}.\\

Please note that the last term of \eqref{eq:sf_split} is not an inner product but the time average of a point-wise product of two orthogonal functions. Therefore, this term does not vanish in general. However, we follow the assumption of \cite{VeRiMaRo97PSF} and neglect it. \\	

In this case the OTF can be rewritten as	
\begin{equation*}
B({\boldsymbol \rho}/\lambda) =  \frac{1}{S} \iint_\mathcal{P} P({\bf x}) P({\bf x}+{\boldsymbol \rho})\cdot \text{exp}\left(-\frac{1}{2}D_{\phi_\|}({\bf x},{\boldsymbol \rho})\right) \cdot \text{exp}\left(-\frac{1}{2}D_{\phi_\perp}({\bf x},{\boldsymbol \rho}) \right)\,  d{\bf x}.
\end{equation*}\\

The orthogonal part of the residual phase cannot be computed from the actual on-sky data, but only simulated by using sophisticated atmospheric models. Therefore, an instantaneous calculation of $D_{\phi_\perp}({\bf x},{\boldsymbol \rho})$ gives no meaningful contribution and we follow the suggestion of \cite{VeRiMaRo97PSF} to replace it by its mean over the variable ${\bf x}$
\begin{align}\label{eq:D_avg}
\bar{D}_{\phi_\perp}({\boldsymbol \rho}) = \frac{\iint_{\mathcal{P}} P({\bf x}) P({\bf x}+{\boldsymbol \rho}) D_{\phi_\perp}({\bf x},{\boldsymbol \rho})\, d{\bf x}}{\iint_{\mathcal{P}} P({\bf x}) P({\bf x}+{\boldsymbol \rho})\, d{\bf x}}.
\end{align}
Similarly, we replace $D_{\phi_\|}$ by $\bar{D}_{\phi_\|}$ to reduce the computational load. This is feasible as several numerical experiments have shown that the result is not influenced by this simplification, see, e.g., \cite{WaHoRa18}.\\

Using the above averages, \eqref{OTF} simplifies to
...
\end{equation}
As \eqref{eq:otf_prod} is a product of three independent terms, we define 
\begin{align}
...
{\bold A} {\boldsymbol \Phi} := \begin{pmatrix}{\bold A}_{k-K} {\boldsymbol \Phi} \\ {\bold A}_{k-K+1} {\boldsymbol \Phi} \\ \vdots \\ {\bold A}_k {\boldsymbol \Phi}     \end{pmatrix} = \begin{pmatrix}\varphi_{k-K} \\ \varphi_{k-K+1} \\ \vdots \\ \varphi_k  \end{pmatrix} := {\boldsymbol \varphi}.
\end{align}
...
\begin{align*}
({\bold A}_k {\boldsymbol \Phi})({\bold r}) := \sum_{l=1}^L \phi^{(l)} ({\bold r} + v_l k \Delta_T), \qquad {\bold r} \in \bar{\Omega},
\end{align*}
...
Second, the solution to \eqref{eq:tomo-rtc}, the full atmospheric turbulence ${\boldsymbol \Phi}$, is projected onto the telescope pupil along the direction of interest $\beta$, using the projection operator from \cite{Fusco}, defined as
\begin{equation}
\label{eq:project}
P_\beta{\boldsymbol \Phi} := \sum_{l=1}^L \Phi^{(l)}({\bold r} + \beta h_l),
\end{equation}
...
{\bold A} {\boldsymbol \Phi} := \begin{pmatrix}{\bold A}_{k-K} {\boldsymbol \Phi} \\ {\bold A}_{k-K+1} {\boldsymbol \Phi} \\ \vdots \\ {\bold A}_k {\boldsymbol \Phi} \\ \vdots \\ {\bold A}_{k+K-1} {\boldsymbol \Phi} \\ {\bold A}_{k+K} {\boldsymbol \Phi}      \end{pmatrix} = \begin{pmatrix}\varphi_{k-K} \\ \varphi_{k-K+1} \\ \vdots \\ \varphi_k \\ \vdots \\ \varphi_{k+K-1} \\ \varphi_{k+K} \\ \end{pmatrix} := {\boldsymbol \varphi}.
\end{align}
...
Let us now describe how the three terms of \eqref{eq:Dphipar} can be computed. First, the structure function $\bar{D}_{\phi_{rec,\beta_i}}$ relies on the computed pseudo wavefronts $\varphi_{rec,\beta_i}$, via the corresponding phase $\phi_{rec,\beta_i} = \frac{2\pi}{\lambda}\varphi_{rec,\beta_i}$ in the desired directions and can be directly computed using \eqref{eq:remove-mirror} and \eqref{eq:D_avg}.\\

As in \cite{WaHoRa18}, the structure functions for noise and aliasing $ \bar{D}_{\phi_n}$ and $ \bar{D}_{\phi_r}$ are spatially averaged, from realizations of $\phi_n$ and $\phi_r$ from Monte Carlo simulations. We assume a Gaussian white noise covariance matrix on the wavefront sensor $C_{WFS} = \frac{1}{n_{photons}}I$. To obtain $\phi_r$, we simulate $\phi_\perp$, compute the respective WFS measurements $\Gamma\phi_\perp$ and use the same AO control algorithm $R$ as before to obtain $\phi_r$. Note that our simulations have shown that we can neglect the further propagation of $\phi_n$ and $\phi_r$ through the tomographic reconstruction and the projection step in direction $\beta_i$. Furthermore, note that the noise and aliasing structure functions are computed only once as a starting point and updates can be performed offline, so one could use available covariance matrices together with the $U_{ij}$-functions as proposed in \cite{VeRiMaRo97PSF}. However, when using a matrix-free AO control algorithm this would mean that one needs to set up the matrix which can easily be done by computing the response of the algorithm to only one non-zero measurement.

\subsection{Algorithm for off-axis PSF reconstruction in an SCAO system}
We summarize the algorithm for off-axis PSF reconstruction in an SCAO system in Algorithm~\ref{alg:psfr-scao}, consisting of three steps. One step, computing $B_\perp$ and $B_{tel}$, can be done in simulation only, the second step, computing $B_{\|,\beta_i}$, has to be performed after the observation. The third step of combining the first two is post processing after the exposure time. As an additional input the directions of interest for the PSF reconstruction have to be defined. Note that computing $B_{\|,\beta_i}$ during the observation would not work as we want to use the time series of AO data to perform the tomography step. Therefore, the storage and dataflow environment needs to be suited for saving all AO telemetry data.\\
...
\begin{table}[H]
\caption{weighted 3-layer atmosphere} 
\label{tab:eso3layer}
\begin{center}       
...
\hline
\rule[-1ex]{0pt}{3.5ex} windspeed (m/s) & 15 & 10 & 28\\ 
\end{tabular}
\end{center}
\end{table} 
...
We obtain on-axis a FVU of $0.14\%$ and off-axis $4.22\%$. Using the method from \cite{WaHoRa18}, we obtain a FVU of $0.08\%$ on-axis.
  ...
  \begin{figure}[!htb]
  \begin{center}
\includegraphics[width=0.8\linewidth]{psf_3l_weight_onaxis.png}
\end{center}
...
\label{fig:psfr-onaxis-3l-standard}
 \end{figure}
   ...
   \begin{figure}[!htb]
  \begin{center}
\includegraphics[width=0.8\linewidth]{psf_3l_weight_offaxis.png}
\end{center}
...
\hline
\rule[-1ex]{0pt}{3.5ex} windspeed (m/s) & 15 & 13 & 13 & 9 & 9 & 15 & 25 & 40 & 21\\ 
\end{tabular}
\end{center}
\end{table} 
...
In Figure~\ref{fig:psfr-onaxis-3l-9l} and \ref{fig:psfr-offaxis-3l-9l}, we show the resulting PSFs on- and off-axis, respectively. We reconstruct an on-axis Strehl ratio of $65.0\%$, being $0.8\%$ above the true Strehl ratio. The off-axis Strehl of our reconstructed PSF is reconstructed to an accuracy of $5.2\%$.  Again, we compute the FVU of the OTF using \eqref{eq:fvu}. We obtain on-axis a FVU of $0.13\%$ and off-axis $4.33\%$. For the on-axis PSF, we can compare to the method from \cite{WaHoRa18} which underestimates the Strehl ratio by $0.26\%$ and gives a FVU of $0.07\%$.
  ...
  \begin{figure}[!htb]
  \begin{center}
\includegraphics[width=0.8\linewidth]{psf_9l_3l_onaxis.png}
\end{center}
...
\label{fig:psfr-onaxis-3l-9l}
 \end{figure}
   ...
   \begin{figure}[!htb]
  \begin{center}
\includegraphics[width=0.8\linewidth]{psf_9l_3l_offaxis.png}
\end{center}
...
\hline
\rule[-1ex]{0pt}{3.5ex} windspeed (m/s) & 12 & 15 & 30\\ 
\end{tabular}
\end{center}
\end{table} 
  ...
  \begin{figure}[!htb]
  \begin{center}
\includegraphics[width=0.8\linewidth]{psf_3l_high_onaxis.png}
\end{center}
   \caption[PSF reconstruction on-axis 3 layer atmosphere]{Comparison of the true PSF (blue) and the reconstructed PSF using tomography (red) and using the DM commands (yellow) for a 3-layer atmosphere with strong high layer on-axis.}
\label{fig:psfr-onaxis-3l-high}
 \end{figure}
 
   \begin{figure}[!htb]
  \begin{center}
\includegraphics[width=0.8\linewidth]{psf_3l_high_offaxis.png}
\end{center}
...
For all scenarios, the error of the Strehl ratio of reconstructed on-axis PSF is always below 2\%. Therefore, we only present the error of the Strehl ratio of the off-axis PSF in Table~\ref{tab:sensitivity} for a perturbation of $C_n^2$ values and layer heights simultaneously.  The results indicate that a too high $C_n^2$ fraction in the highest layer has the biggest influence when no other errors occur. However, the combinations of different errors show that a too high and too strong top layer results in an underestimation of the Strehl ratio while a too low top layer and a too strong ground layer give an overestimation of similar magnitude. The exact other combinations (too low and too strong top layer, or, too high top layer and too strong ground layer) seem to almost cancel their influence as they give even better results than just perturbing layer heights or $C_n^2$ values. \\
...
\caption{Error of the Strehl ratio of the reconstructed off-axis PSF for input errors on $C_n^2$-values (columns) and layer heights (rows). GL: ground layer, ML: mid layer, TL: top layer}
...
Altogether, our tests show that even with imprecise knowledge of the atmospheric parameters a good quality in the reconstructed PSF can be obtained. E.g., for errors less than 10\% on each of $C_n^2$-values, layer heights and wind speeds, the typical error on the Strehl ratio is below 4\%. For bigger errors on at least one quantity, the error increases.  This means that at least an atmospheric profile with few layers and their wind vectors needs to be known in order to reconstruct an off-axis PSF. 

\subsubsection{Sensitivity analysis for a reduced number of reconstructed layers}\label{sect:numerics-sensitivity-2layer}
As in practice a real-time atmospheric monitor might be not available, we want to investigate whether tomography on two layers, i.e., one ground layer and one higher layer, could be enough. This reduces the computational complexity of the tomographic step and could still give good results. From the ESO standard 9-layer atmosphere profile, we compute an averaged 2-layer profile, shown in Table~\ref{tab:eso2layer}, by using similar formulae as \eqref{eq:layer_calc}.\\
...
\hline
\rule[-1ex]{0pt}{3.5ex} windspeed (m/s) & 14 & 20 \\ 
\end{tabular}
\end{center}
\caption{weighted 2-layer atmosphere} 
\label{tab:eso2layer}
\end{table} 
...
\caption{Error of the Strehl ratio of the reconstructed off-axis PSF for input errors on $C_n^2$-values (columns) and layer heights (rows). GL: ground layer, TL: top layer}
...
In this work we presented a new algorithm for off-axis PSF reconstruction in an SCAO system for the upcoming generation of GSMTs. Our approach uses reconstructed atmospheric layers to overcome the problem of field dependent PSFs in wide field AO systems through anisoplanatism. One can easily compute PSFs for different view directions within the field of view. Simulations show qualitatively good reconstruction of the PSF compared to the PSF calculated directly from the simulated incoming wavefront and stability with respect to imprecise knowledge of atmospheric parameters. Furthermore, the used algorithm has a reasonable run time and memory consumption.\\

In forthcoming work, we want to validate this algorithm with on-sky data. However, obtaining synchronized AO data, atmospheric parameters and science images with single stars is a big challenge.\\ 

A future goal is to use the reconstructed PSFs as input to an algorithm for image improvement, e.g., \cite{DyRaReSoWa21,BeCoRaJoNeFuWi19PSF}. Such an approach leads to a further improvement of the quality of the reconstructed PSF and simultaneously improves the quality of the observed image. However, it is only possible for certain dedicated science images. 
...
The authors thank Carmelo Arcidiacono (INAF Osservatorio Astronomico di Padova) for suggesting the test case in Section~\ref{sect:numerics-sensitivity} and Miska Le Louarn (ESO) for suggesting the compression to two layers presented in Section~\ref{sect:numerics-sensitivity-2layer}.
