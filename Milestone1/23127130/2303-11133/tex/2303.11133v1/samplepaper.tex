...
%
\begin{abstract}
Given an $\omega$-automaton and a set of word homomorphisms, we look at which accepted words have such a substitutive structure, and in particular if there is at least one.
...
Two classical methods to generate words are the following: on the one hand, sofic shifts are the set of walks on a labeled graph (which can be considered as an $\omega$-automaton) \cite{lind_marcus_1995}; 
...
It is then natural to ask whether a given $\omega$-automaton accepts a word with a given substitutive structure.
...
The method is flexible enough to allow for additional constraints on the directive sequences of substitutions, which is powerful enough for example to decide whether an $\omega$-automaton accepts a Sturmian word.
A consequence is the decidability of whether a given set of finite words codes some Sturmian word (or from any family with an $S$-adic characterization).
...
%A finite word on $\mathcal{A}$ is a concatenation of finitely many symbols of $\mathcal{A}$.
The set of finite words on $\mathcal{A}$ is denoted as $\mathcal{A}^*$.
%A (mono)infinite word is an infinite concatenation of symbols of $\mathcal{A}$, which can be modeled as a function $x : \mathbb{N} \rightarrow \mathcal{A}$.
...
%A factor of a word $x$ is a contiguous subword of $x$.
%$u \preceq_f x$ means that $u$ is a factor of $x$ (the $f$ meaning \emph{factor}).
For a word $x$ and $0 \leq j \leq k < |x|$, $x_{\llbracket j,k \rrbracket}$ is the word $x_j x_{j+1} x_{j+2} \dots x_{k-1} x_{k}$ and is called a \emph{factor} of $x$.
For a finite word $w$ and $x$ a (possibly infinite word), $w \preceq_f x$ means that $w$ is a factor of $x$, and $w \sqsubseteq_p x$ means that $w$ is a prefix of $x$.
%A finite word $w$ is a prefix of a (possibly infinite) word $x$ if $w = x_{\llbracket 0, k \rrbracket}$ for some $k \leq |x|$: we then write this relation as $w \sqsubseteq_p x$.
%For $n \geq 1$, $p_n(x)$ is the number of different factors of $x$ of length $n$ : $p_n(x) = Card(\{u \preceq_f x\ |\ |u| = n \})$.
%The set of infinite words on the alphabet $\mathcal{A}$ is denoted as $\mathcal{A}^\mathbb{N}$.
...
Another fundamental class of shifts is the one of sofic shifts.
Although the traditional way to define them uses labeled graphs (see \cite{lind_marcus_1995}), we will here use the notion of $\omega$-automata, which is equivalent.

\begin{definition}[$\omega$-automaton]
An $\omega$-automaton $\mathfrak{A}$ over an alphabet $\mathcal{A}$ is a tuple $(Q, I, T, Acc)$, where $Q$ is a finite set of states, $I \subseteq Q$ is the set of initial states, $T \subseteq Q \times \mathcal{A} \times Q$ is the set of transitions of $\mathfrak{A}$ and $Acc  \subseteq Q^\mathbb{N} \cup Q^\ast$ is an acceptance condition (see the following definition).
\end{definition}
...
For $n \geq 1$ (or $n = \infty$), a sequence $(q_k)_{0 \leq k \leq n}$ with $q_k \in Q$ is a \emph{walk} in $\mathfrak{A}$ if there is $(a_k)_{1\leq k\leq n} \subseteq \mathcal{A}$ such that for all $k$, $q_k \xrightarrow[]{a_{k+1}} q_{k+1}\in T$. We then write $q_0 \xrightarrow[]{a_1} q_1 \xrightarrow[]{a_2} q_2 \xrightarrow[]{a_3} \cdots \xrightarrow[]{a_n} q_{n}$.
The word $w = (a_k)_{1 \leq k \leq n}$ \emph{labels} the  walk.
...
However, in contrast to much of the literature, in this paper, when $Acc$ is not specified, we will consider that $Acc = Q^\mathbb{N} \cup Q^\ast$, i.e. that every  walk is accepted.
...
If all states of $\mathfrak A$ are initial ($I = Q$), its language of infinite words is a shift (called a \emph{sofic shift}) .

\subsection{Substitutions}
\begin{definition}[Homomorphisms and substitutions]
A homomorphism is a function $\sigma : \mathcal{A}^\ast \rightarrow \mathcal{A}^\ast$ such that $\sigma(uv) = \sigma(u)\sigma(v)$ for all $u,v \in \mathcal{A}^\ast$.
$\sigma$ is extended to $\mathcal{A}^\mathbb{N} \rightarrow \mathcal{A}^\mathbb{N}$ by $\sigma(x_0 x_1 x_2 x_3 \dots) = \sigma(x_0) \sigma(x_1) \sigma(x_2) \sigma(x_3) \dots$
...
\begin{itemize}
    ...
    \item a \emph{purely substitutive word} generated by $\sigma$ if there is a letter $a \in \mathcal{A}$ such that $x = \lim\limits_{n \rightarrow \infty} \sigma^n(a)$ and the limit is well-defined;
    \item a \emph{morphic word} generated by $\sigma$ and $\tau$ if $x = \tau(y)$, where $y$ is a purely substitutive word generated by $\sigma$;
    \item a \emph{substitutive word} generated by $\sigma$ if $x$ is a morphic word with $\tau$ being a coding, i.e. $\tau(\mathcal{A}) \subseteq \mathcal{A}$.
\end{itemize}
\end{definition}

It is now possible to extend these definitions in the case we use multiple homomorphisms.
...
\begin{definition}[Infinitely desubstitutable words and directive sequences]
    Let $\mathcal{S}$ be a finite set of substitutions, and let $(\sigma_n)_{n \in \mathbb N} \subseteq \mathcal{S}$.
...
Then $x$ is infinitely desubstitutable by $(\sigma_n)_{n \in \mathbb N}$ if, and only if, there is a sequence of infinite words $(x_n)_{n \in \mathbb N}$ such that $x = \sigma_{\llbracket 0, n \rrbracket}(x_{n+1}) $ for all $n \geq 0$.
...
Let $\mathfrak{A} = (Q, I, T)$ be an $\omega$-automaton, and $\sigma$ a homomorphism.
Then, we define $\sigma^{-1}(\mathfrak{A})$ as the $\omega$-automaton $(Q, I, T')$ where, for all $q_1,q_2 \in Q$ and $a\in\mathcal A$, $q_1 \xrightarrow[]{a} q_2 \in T'$ iff $\trans{q_1}{\sigma(a)}{q_2}$ is a computation in $\mathfrak{A}$.
\end{definition}
...
It is a classical construction in the theory of finite automata, and it satisfies the following property :
...
This defines an accepting walk labeled by $u$ in $\sigma^{-1}(\mathfrak{A})$.
\end{proof}

There is a more precise equivalent for finite words:

\begin{proposition}
\label{WordSub}
    Let $w$ be a finite word, $\mathfrak{A}$ be an $\omega$-automaton and $\sigma$ a homomorphism.
...
When desubstituting automata, the following property is crucial:

%$\mathfrak{A}$ and $\sigma^{-1}(\mathfrak{A})$ share the same set of states:
%only the set of transitions changes.
%In other words, 

\begin{fact}\label{fct:DesubFinite}
Let $\mathfrak{A}$ be an $\omega$-automaton on the alphabet $\mathcal{A}$,
let $\mathfrak{S}(\mathfrak{A})$ be the set of all $\omega$-automata which have the same set of states and the same initial states as $\mathfrak{A}$. 
For any homomorphism $\sigma$ on $\mathcal{A}$, $\sigma^{-1}(\mathfrak{A})$ is an element of $\mathfrak{S}(\mathfrak{A})$.
\end{fact}

In particular, remark that $\mathfrak{S}(\mathfrak{A})$ is finite: given $\mathfrak{A} = (Q,I,T)$, an element of $\mathfrak{S}(\mathfrak{A})$ is identified by its transitions, which form a subset of $(Q \times \mathcal{A} \times Q)$, so $\text{Card}(\mathfrak{S}(\mathfrak{A})) = 2^{|Q|^2 \times |\mathcal{A}|}$.
...
Then there exist $n < m \in \mathbb{N}$ such that $\sigma^{-n}(\mathfrak{A}) = \sigma^{-m}(\mathfrak{A})$.
\end{fact}

In the remainder of the section, we prove that, given an $\omega$-automaton $\mathfrak{A}$ and a substitution $\sigma$, the problems of finding a fixed point of $\sigma$ or a purely substitutive walk generated by $\sigma$ accepted by $\mathfrak{A}$ are decidable.

A purely substitutive word generated by an erasing homomorphism $\sigma$ is also generated by a non-erasing homomorphism $\tau$ (that is, a substitution) that can be effectively constructed.
Thus, we assume $\sigma$ itself is a substitution.

\begin{proposition}\label{prop:FixedPointPower}
Let $\mathfrak{A}$ be an $\omega$-automaton, let $\sigma$ be a substitution and let $n < m \in \mathbb{N}$ be as in Fact \ref{fct:LoopAutomata}. 
Then, $\mathfrak{A}$ accepts a fixed point for $\sigma^k$ for some $k \geq 1$ iff  $\mathcal{L}_\infty(\sigma^{-n}(\mathfrak{A}))$ is nonempty.
\end{proposition}

\begin{proof}
We have $n < m \in \mathbb{N}$ as in Fact~\ref{fct:LoopAutomata}, so that $\sigma^{-n}(\mathfrak{A}) = \sigma^{-m}(\mathfrak{A})$.

If $\mathcal{L}_\infty(\sigma^{-n}(\mathfrak{A}))$ is empty, then $\mathcal{L}_\infty(\sigma^{-p}(\mathfrak{A}))$ is empty for every $p \geq n$.
...
Because $\sigma^{-n}(\mathfrak{A}) = \sigma^{-m}(\mathfrak{A}) = \sigma^{-(m-n)}(\sigma^{-n}(\mathfrak{A}))$, $x$ is accepted by $\sigma^{-j(m-n)}(\sigma^{-n}(\mathfrak{A})) = \sigma^{-(n + j(m-n))}(\mathfrak{A})$ for all $j \in \mathbb{N}$.
...
By compacity of the language of an $\omega$-automaton, $\Tilde{x} \in \mathcal{L}_{\infty}(\sigma^{-n}(\mathfrak{A}))$. 


Let $NG_{\sigma^{m-n}} \subseteq \mathcal{A}$ be the set of non-growing letters for $\sigma^{m-n}$, i.e. $\forall a \in NG_{\sigma^{m-n}}, \forall j \geq 1, |\sigma^{j(m-n)}(a)| = 1$. Let $k = \inf \{i \in \mathbb{N}\ |\ \Tilde{x}_i \notin NG_{\sigma^{m-n}}\}$ ($k$ may be infinite).
Then, for $i < k$, because $\sigma$ is a (nonerasing) substitution and every letter in $\Tilde{x}_{\llbracket 0,k-1\rrbracket}$ is non-growing, $\sigma^{(m-n)}(\Tilde{x})_i = \sigma^{(m-n)}(\Tilde{x}_i)$.
In addition, because $\Tilde{x}$ is an adherence value of $(\sigma^{n+j(m-n)}(x))_{j \in \mathbb{N}}$, there is $p(\Tilde{x}_i) \geq 1$ such that $\sigma^{p(\Tilde{x}_i)\cdot(m-n)}(\Tilde{x}_i) = \Tilde{x}_i$ for every position $i < k$. Since $\mathcal A$ is finite, $(p(\Tilde{x}_i))_{0\leq i<k}$ contains only finitely many values, so we can define $p = \text{lcm} \{p(\Tilde{x}_i)\}$.
...
Then, by concatenation, $\Tilde{x}_{\llbracket 0,k \rrbracket} \sqsubseteq_p \sigma^{pq(m-n)}(\Tilde{x}_{\llbracket 0,k \rrbracket})$.
Thus, $(\sigma^{jpq(m-n)}(\Tilde{x}))_{ j \in \mathbb{N}}$ has a limit, which is a fixed point for $\sigma^{pq(m-n)}$, and by compacity of $\mathcal{L}_{\infty}(\mathfrak{A})$, is accepted by $\mathfrak{A}$.
\end{proof}
...
%Thus, to decide whether there is a purely substitutive word accepted by an $\omega$-automaton $\mathfrak{A}$, we only need to refine the previous method:

\begin{proposition}\label{prop:PureSubsWord}
    Let $\mathfrak{A}$ be an $\omega$-automaton, $\sigma$ a substitution and $n < m \in \mathbb{N}$ be as in Fact~\ref{fct:LoopAutomata}.
    Let $RP_\sigma \subseteq \mathcal{A}$ be the set of letters that are right-prolongable for $\sigma$, i.e. $\forall b \in RP_\sigma, b \sqsubsetneq_p \sigma(b)$.
...
\begin{proof}
    ...
    Then, $\sigma^{m-n}(b)$ labels a accepting walk on $\sigma^{-m}(\mathfrak{A}) = \sigma^{-n}(\mathfrak{A})$.
    By iteration, for every $k \geq 1$, we have that $\sigma^{k(m-n)}(b)$ labels a accepting walk on $\sigma^{-n}(\mathfrak{A})$, so $\sigma^{n + k(m-n)}(b)$ always labels an accepting walk on $\mathfrak{A}$. By compacity, $u = \lim\limits_{k \rightarrow \infty} \sigma^{n + k(m-n)}(b)$ is accepted by $\mathfrak{A}$.
...
This result extends to morphic words: a morphic word generated by $\sigma$ and $\tau$ in $\mathfrak{A}$ is a purely substitutive word generated by $\sigma$ in $\tau^{-1}(\mathfrak{A})$.
...
\begin{proof}
    Let $x$ a fixed point for $\sigma$ and define $FP_\sigma = \{b \in \mathcal{A}\ |\ \sigma(b) = b\}$ be the set of letters which are fixed points under $\sigma$.
    There are two cases:
...
\item there is a letter $a$ appearing in $x$ such that $\sigma(a) \neq a$.
    ...
    So $a \sqsubseteq_p \sigma(a)$: $a$ is right-prolongable for $\sigma$, so $\lim\limits_{n \rightarrow \infty} \sigma^n(a)$ exists. Since $x = \sigma^n(x) = p \sigma^n(a) \sigma^n(x')$ for every $n \in \mathbb{N}$, by compacity, $x = p \lim\limits_{n \rightarrow \infty} \sigma^n(a)$.
\end{enumerate}
...
In this section, we suppose that $\mathfrak A$ is an $\omega$-automaton and $\mathcal{S}$ is a finite set of substitutions (i.e. nonerasing homomorphisms), as is usual when studying multiple homomorphisms. We prove that the problem of finding an infinitely desubstitutable (infinite) word is decidable. To study this question, we introduce a meta-$\omega$-automaton:
...
Define the $\omega$-automaton $\mathcal{S}^{-\infty}(\mathfrak{A}) = (D(\mathfrak{A}), \{\mathfrak{A}\}, \mathcal{T})$ on the alphabet $\mathcal{S}$, with the set of states $D(\mathfrak{A}) = \{\sigma^{-1}(\mathfrak{A}),\ \sigma \in \mathcal{S}^\ast\}$, $\mathfrak A$ the only initial state and set of transitions $\mathcal{T} = \{ \mathfrak{B} \xrightarrow[]{\sigma} \sigma^{-1}(\mathfrak{B})\ |\ \mathfrak{B} \in D(\mathfrak{A}), \sigma \in \mathcal{S}\}$.
\end{definition}
...
%We now prove that $\mathcal{S}^{-\infty}(\mathfrak{A})$ contains all the information needed to decide our problem:
...
\begin{corollary}\label{prop:SAdicLanguage}
    Let $Rep(\mathfrak A, \mathcal{S}) = \{(\sigma_n)_{n \in \mathbb N}\ |\ \exists x \in \mathcal{L}_\infty(\mathfrak A), x \text{ is inf. desub. by } (\sigma_n) \}$ be the set of directive sequences of words accepted by $\mathfrak A$.
    Then $Rep(\mathfrak A, \mathcal{S})$ is the language of some $\omega$-automaton.
    In particular, $Rep(\mathfrak A, \mathcal{S})$ is closed as a subset of $\mathcal{S}^\mathbb{N}$.
\end{corollary}

%We show Proposition~\ref{prop:InfDesub} one direction at a time.
...
%In order to use a topological result, we recall that, given an $\omega$-automaton $\mathfrak{B}$, $\mathcal{L}_\infty(\mathfrak{B})$ is closed for the prodiscrete topology.
...
This point $x$ is desubstituable by any $\sigma_1 \circ \dots \circ \sigma_k$, thus it is infinitely desubstituable by the sequence $(\sigma_n)_{n \in \mathbb{N}}$.
\end{proof}

With Proposition \ref{prop:InfDesub}, we can deduce the decidability of the existence of an infinitely desubstitutable walk in an $\omega$-automaton $\mathfrak A$.
...
%The classical $\mathcal S$-adic characterization of Sturmian words is the following:
...
In this section, we generalize Theorem~\ref{prop:InfDesubWalk} to Sturmian words and more general restrictions on the directive sequence.
...
\begin{proof}
    ...
    According to Proposition~\ref{prop:InfDesub} combined with Theorem~\ref{thm:SturmArnoux},  there is a Sturmian infinite word accepted by $\mathfrak{A}$ if, and only if, there is an infinite walk accepted by $\mathcal{S}_{St}^{-\infty}(\mathfrak{A})$ labeled by a word $(\sigma_n)_{n \in \mathbb{N}}$ which alternates infinitely in type.
...
\begin{proof}
   % Let $\mathcal{S}^{-\infty}(\mathfrak{A})$.
    ...
    The intersection between a Büchi $\omega$-automaton and an $\omega$-automaton is a Büchi $\omega$-automaton that can be effectively constructed, and checking the non-emptiness of a Büchi $\omega$-automaton is decidable.
\end{proof}
...
We also characterize the set of allowed directive sequences akin to Corollary~\ref{prop:SAdicLanguage}: the set of directive sequences on $\mathcal{S}$ accepted by the Büchi $\omega$-automaton $\mathfrak R$ that define a word accepted by $\mathfrak A$ is itself recognized a Büchi $\omega$-automaton.
...
% Let $\mathfrak{A} = (Q,I,T,Acc)$ be an $\omega$-automaton.
% $\mathfrak{A}$ is \emph{pruned} if all states appear as source and as target of some transition.

% Any $\omega$-automaton $\mathfrak{A}$ can be pruned by successively removing states that do not appear as source and as target of some transition. 
% Denote $p(\mathfrak{A})$ the resulting pruned $\omega$-automaton.
% \end{definition}

% \begin{proposition}\label{prop:Pruning}
% For any $\sigma$ and $\mathfrak A$, $\sigma^{-1}(p(\mathfrak A)) = p(\sigma^{-1}(\mathfrak A))$.
% \end{proposition}

%We are now ready to prove Theorem~\ref{prop:SturmianOmega}.

\begin{proof}
    Let $x$ be a Sturmian word accepted by $\mathfrak{A}$.
    Consider the transformation of $\omega$-automata $\text{forget} : (Q,I,T) \mapsto (Q,Q,T)$ which makes all states initial.
    Then, $\text{forget}(\mathfrak{A})$ also accepts $x$, and its language is shift-invariant: therefore, $\mathcal{O}(x) \subseteq \mathcal{L}_\infty(\text{forget}(\mathfrak{A}))$.
    Let $\chi(x)$ be the Sturmian characteristic word associated with  $x$ (see \cite{perrin_restivo_2012}): it belongs to $\mathcal{O}(x)$, so it is accepted by $\text{forget}(\mathfrak{A})$.
    ...
    But, because $\chi(x)$ is a characteristic word, it represents the orbit of zero when considering the point of view of circle rotation (see \cite{perrin_restivo_2012}): when combined with Proposition~2.7 of \cite{berthe_holton_zamboni_2006}, it yields that
    ...
    Thus, we can find a substitution $\tau$ such that $\mathfrak{B} = \tau^{-1}(\mathfrak{B})$ and $\tau \in \mathcal{S}_{St}^\ast \setminus (L_0^\ast \cup L_1^\ast)$.
...
If all states of $\mathfrak A$ have property (H), there are two possibilities: if there is no infinite computation starting on an initial state, the infinite language of $\mathfrak{A}$ is empty; otherwise, $\mathfrak{A}$ is total.
    ...
    \begin{proof}[of Lemma \ref{lem:TechnicalLemma}]
        Let $\mathfrak C = (Q_\mathfrak{C}, I_\mathfrak{C}, T_\mathfrak{C})$, and $q \in Q_\mathfrak{C}$.
        ...
        So the last computation can be written $q \xrightarrow[]{0} \trans{q_u}{m}{q_t} \transo{}{}{\cdots}$\hfill$\square$
    \end{proof}
...
By applying the proof of Theorem \ref{prop:SturmianOmega}, we obtain an equivalent statement for the Fibonacci word:
...
\begin{corollary}
    A sofic subshift contains $f$ iff it contains some $\sigma_f^k(\mathcal{A}^\mathbb{N})$.
    
%    Let $\mathbb{S}$ be a sofic subshift.
%    Then, it contains the Fibonacci word iff it contains some $\sigma_f^k(\mathcal{A}^\mathbb{N})$.
\end{corollary}
...
\section{Appendix}
% Mettre les images cote à cote
% Spécifier à quelle section on fait référence
