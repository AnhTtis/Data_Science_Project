...
\be\label{eq:412}
\l_{t+1} = \frac{\l_t}{\mu_t} - 1 ,
\eta_t = (1 + \l_{t+1}) \al_t 
\ee
In the above, in principle the quantity $\l_0$ is not specified and 
...
\ee
\be\label{eq:414}
\bth_{t+1} = \frac{ \l_{t+1} }{ 1 + \l_{t+1} } \bth_t 
+ \frac{1}{1 + \l_{t+1} } \w_{t+1} ,
\ee
then $\bth_{t+1}$ satisfies \eqref{eq:111}.
Note that one can write \eqref{eq:414} as
\bd
\bth_{t+1} = \bth_t + \frac{1}{1 + \l_{t+1} } ( \w_{t+1} - \bth_t )
= \bth_t + \frac{1}{1 + \l_{t+1} } (\w_t - \bth_t - \eta_t \h_{t+1} ) ,
\ed
or equivalently as
\be\label{eq:415}
\bth_{t+1} 
= \bth_t + \frac{1}{1 + \l_{t+1} } (\w_t - \bth_t ) - \al_t \h_{t+1}  .
\ee 
...
In \cite{Sebbouh-et-al-CoLT21}, the authors choose $\l_0 = 0$, and
\be\label{eq:415b}
\l_t = \frac{S_{t-1}}{\eta_t} , \mbox{ where } 
...
In this case, both $\l_t$ and $\eta_t$ are also constants, and
\eqref{eq:412} simplifies to
\bd
\l = \frac{\mu}{1-\mu} , 1 + \l = \frac{1}{1 - \mu} , 
\frac{\l}{1+\l} = \mu ,
...
\end{split}
\ee
Note that we have replaced $0.5 B_t$ by $B_t$ in the above, to avoid
...
is also summable.
Finally, since $\ell_2$ is a subset of $\ell_1$, the summability of
$\al_t B_t$ implies that $\al_t^2 B_t^2$ is also summable.
