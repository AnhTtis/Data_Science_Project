...
in~\citeyear{Barnes1900}, by Barnes~\cite{Barnes1900} as the integer values of
the ``G-function''.
...
to~$D_{n,1}$ and that~$D_n = \sum_k D_{n,k}$.
...
\begin{proof}
  ...
  the same property.
  Moreover, given a size-$(n-1)$ vector~$(p_1, p_2, \ldots, p_{n-1})$ of
  non-negative integers such that for all~$i$,~$p_i \leq i$, the number of such
  variation matrices with exactly~$p_{n-i}$ zeros in the~$i$-th line is
  \begin{equation*}
