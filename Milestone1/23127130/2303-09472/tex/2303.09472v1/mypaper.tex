...
Apart from the above scheme and architectural novelties, we show the effectiveness of joint optimization. In the second stage, we observe that the estimated IPR may still have minor errors, which will affect the performance of the DIRformer. However, the previous DMs need many iterations, which is unavailable to optimize DM with the decoder together. Since our DiffIR requires few iterations, we can run all iterations and obtain the estimated IDR to optimize with DIRformer jointly. As shown in Fig.~\ref{fig:head}, our DiffIR achieves SOTA performance consuming much less computation than other DM-based methods (\eg, RePaint~\cite{repaint} and LDM~\cite{LDM}). In particular, DiffIR is 1000$\times$ more efficient than RePaint. Our main contributions are threefold:
 \begin{itemize}
...
\label{sec:pretrain}
Before introducing pretraining DiffIR, we would like to introduce two networks in the first stage, including a compact IR prior extraction network (CPEN) and a dynamic IRformer (DIRformer). The structure of CPEN is shown in Fig.~\ref{fig:method} yellow box, which is mainly stacked with residual blocks and linear layers to extract the compact IR prior representation (IPR). After that, DIRformer can use the extracted IDR to restore LQ images. The structure of the DIRformer is shown in Fig.~\ref{fig:method} pink box, which is stacked with dynamic transformer blocks in the Unet shape. The dynamic transformer blocks consist of dynamic multi-head transposed attention (DMTA, Fig.~\ref{fig:method} green box) and dynamic gated feed-forward network (DGFN, Fig.~\ref{fig:method} nattier blue box), which can use IDR as dynamic modulation parameters to add restoration details into feature maps. 
...
Then, after $T$ times iterations, we obtain the final estimated IPR $\mathbf{\hat{Z}} \in\mathbb{R}^{4C^{\prime}}$. We joint train DPEN$_{S2}$, denoising network, and DIRformer using $\mathcal{L}_{all}$:
\begin{equation}
\label{eq:diff}
\vspace{-1mm}
...
\begin{table*}[htbp]
  ...
  \caption{Quantitative comparison ( LPIPS/DISTS). for \textbf{Single image super-resolution} on benchmark datasets. Best and second best performance are marked in bold and underlined, respectively. The bottom two methods marked in gray adopt the diffusion model.}
   \resizebox{1\linewidth}{!}{
    ...
    \toprule[0.2em]
    \multirow{2}[2]{*}{\textbf{Method}} & \multicolumn{2}{c}{\textbf{Set5}~\cite{Set5}} & \multicolumn{2}{c}{\textbf{Set14}~\cite{Set14}} & \multicolumn{2}{c}{\textbf{General100}~\cite{general100}} & \multicolumn{2}{c}{\textbf{Urban100}~\cite{Urban100}} & \multicolumn{2}{c}{\textbf{DIV2K100}~\cite{DIV2K}} \\
          & LPIPS $\downarrow$& DISTS $\downarrow$& LPIPS $\downarrow$& DISTS $\downarrow$& LPIPS $\downarrow$& DISTS $\downarrow$& LPIPS $\downarrow$& DISTS $\downarrow$& LPIPS $\downarrow$& DISTS $\downarrow$\\
    \midrule
    SFTGAN~\cite{SFTGAN} & 0.0800  & 0.1085 & 0.1313 & 0.1133 & 0.0947 & 0.0992 & 0.1343 & 0.1062 & 0.1331 & 0.0736 \\
    SRGAN~\cite{SRGAN} & 0.0753 & 0.1003 & 0.1327 & 0.1067 & 0.0964 & 0.0982 & 0.1439 & 0.1081 & 0.1257 & 0.0663 \\
    ESRGAN~\cite{ESRGAN} & 0.0758 & 0.0949 & 0.1241 & 0.0951 & 0.0879 & 0.0874 & 0.1229 & 0.0880 & 0.1154 & 0.0593 \\
    USRGAN~\cite{USRGAN} & 0.0795 & 0.1045 & 0.1347 & 0.0997 & 0.0937 & 0.0931 & 0.1330 & 0.0975 & 0.1325 & 0.0645 \\
    SPSR~\cite{SPSR}  & \underline{0.0647} & 0.0921 & 0.1207 & 0.0920 & 0.0862 & 0.0884 & 0.1184 & 0.0849 & 0.1099 & 0.0546 \\
    BebyGAN~\cite{BebyGAN} & \textbf{0.0627} & \underline{0.0902} & \underline{0.1157} & \underline{0.0865} & \underline{0.0778} & \underline{0.0797} & \underline{0.1096} & \underline{0.0797} & \underline{0.1022} & \underline{0.0499} \\
    \rowcolor{lightgray}
    LDM~\cite{LDM}   & 0.1485 & 0.1535 & 0.2034 & 0.1358 & 0.1655 & 0.1422 & 0.1816 & 0.1284 & 0.1939 & 0.0963 \\
    \rowcolor{lightgray}
    DiffIR$_{S2}$ (Ours) & \textbf{0.0627} & \textbf{0.0749} & \textbf{0.1022} & \textbf{0.0772} & \textbf{0.0657} & \textbf{0.0643} & \textbf{0.0921} & \textbf{0.0734} & \textbf{0.1007} & \textbf{0.0498} \\
    \bottomrule[0.2em]
    \end{tabular}%
    }
  \label{tab:SR}%
