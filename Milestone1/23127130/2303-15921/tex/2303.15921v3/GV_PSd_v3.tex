...
\title{\boldmath Pole-skipping of gravitational waves in the backgrounds of four-dimensional massive black holes}
...
\begin{enumerate}
    ...
    \label{apparent}
    \begin{align}
        \psi_\text{in}(r)&=(r-r_0)^{-\frac{i\omega}{4 \pi T}}g_\text{in}(r)+c \ln(r-r_0) \psi_\text{out}(r), \\ \psi_\text{out}(r)&=(r-r_0)^{\frac{i\omega}{4 \pi T}}g_\text{out}(r).  \label{apparent1}
    \end{align}
    ...
    \begin{align}
        \psi_\text{in}(r)&=(r-r_0)^{-\frac{i\omega}{4 \pi T}}g_\text{in}(r),\\ \psi_\text{out}(r)&=(r-r_0)^{\frac{i\omega}{4 \pi T}}g_\text{out}(r)+c \ln(r-r_0) \psi_\text{in}(r).  \label{apparent2}
    \end{align}
        \end{subequations}
...
where
\begin{equation}
    N_n(\omega)=(n-1)! \left(2i\omega-f_1\right) \left(2i\omega-2f_1\right)\ldots  \left(2i\omega-(n-1)f_1\right),
\end{equation}
...
\begin{equation}
     a_\pm= 4\pi n T \tilde\psi^{\pm}_0 \qty(\tilde\psi^{\pm}_n-\frac{\partial_\omega D_n(\omega,\mu)}{2iN_n(\omega)}\tilde\psi^{\pm}_0) \left.\qty(\pm\frac{\dd \tilde\omega}{\dd\mu}\frac{\delta\mu}{\delta\omega}-1)\right|_{(\omega,\mu)=(\omega_n,\mu_n)}. \label{new_a}
\end{equation}
...
The authors would like to thank Richard Davison and Pavel Kovtun for valuable discussions. S.G. was supported by the STFC Ernest Rutherford Fellowship ST/T00388X/1. The work is also supported by the research programme P1-0402 and the project N1-0245 of Slovenian Research Agency (ARIS). M.V. was supported by the STFC Studentship ST/X508366/1.
