...
\section{Method}
In this section, we investigate how the displacement field and inverse consistency loss~\cite{greer2021icon, tian2022gradicon} of Eq.~\eqref{eq.inv_loss} work. $\mathbf{z}$ is omitted in the derivations because it stays the same in every equation.
...
Considering that $\mathbf{p}(0)=\mathbf{d}_i + \mathbf{p}(c_i)$,  we can use the inverse consistency loss~\cite{greer2021icon, tian2022gradicon}  of Eq.~\eqref{eq.inv_loss_supp} for the displacements $\{g_i(\cdot)\}$ to encourage $g_i$ to compatible with the requirements for integrating an underlying velocity field $\mathbf{v_i}$.
