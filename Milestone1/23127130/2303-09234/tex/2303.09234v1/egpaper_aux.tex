\begin{filecontents}{egpaper_for_review.aux}
...
%%%%%%%%% TITLE
\begin{comment}
\title{\texttt{\textcolor{magenta}{N}\textcolor{orange}{A}\textcolor{green}{I}\textcolor{cyan}{S}\textcolor{violet}{R}}: A 3D Version of \textcolor{magenta}{N}eural \textcolor{orange}{A}dditive Model for \textcolor{green}{I}nterpretable \textcolor{cyan}{S}hape \textcolor{violet}{R}epresentation}
\end{comment}
...
\begin{comment}
In most cases, deep implicit functions model 3D shapes effectively as parametric zero-level set defining the surface of interest \cite{park2019deepsdf, novello2022exploring, michalkiewicz2019deep}.  

However, when we want to represent a set of different shapes sharing some similarities, e.g., an atlas of pediatric airway, we expect an implicit neural representation to reflect the difference of the shape population coming from different covariates, e.g., age, weight, gender. That is to say, given two different ages, we want our shape representation to give two corresponding shapes at the two ages so we know how age influences the shape.

However, in 


We present a framework called \texttt{NAISR} (Neural Additive Interpretable Shape Representation) to represent a shape atlas with different covariates. The simplest example is the 2D ellipsoid with different principal semiaxes. Given such a set of shapes covaritates, we expect \texttt{DNIE} to disentangle the influence of different covariates to shapes. The disentanglement is represented by different level-set evolutions so we know for example if we make the age bigger, the airway is getting bigger.
Compared to traditional representations, DIFs show expressive and flexible capacity for representing complex shapes and fine geometric details, even
in challenging tasks like human digitization [43].
\end{comment}
...
Further, we use an approximate inverse consistency loss to regularize the displacement fields. The basic idea is that the approximate inverse transformation $g_i^{-1}(\cdot)$ of $g_i(\cdot)$ should be able to map a point in the template space back to the point in the source space where
\begin{equation}
\begin{aligned}
...
\begin{figure}[ht]
\vskip-0.2in
\centerline{\includesvg[width=0.9\columnwidth]{figs/summary_test_metrics_transfer_volume.svg}}
\label{fig.transfer}
...
\begin{tabular}{lrr}
\toprule
Methods &  Vol. mean &  Vol. median\\
\midrule
A-SDF(with Cov.) &  46.24 &        38.52 \\
A-SDF & 51.72 &        51.18 \\
NAISR(with Cov.) &  \textbf{11.39} &         9.48  \\
NAISR  &  12.61 &         \textbf{8.50}\\
\bottomrule
\end{tabular}
\end{center}
\caption{Difference in the volume of transfered shapes and grountruth shapes. Bold scores perform the best across all methods. The blue bar in the boxplot represents the reconstruction step that follows~\eqref{eq.infer_z}. The orange bar represents the reconstruction step follows~\eqref{eq.infer_cz}. The unit is cm^3.  }
\label{tab.transfer}
\end{figure}
\end{comment}
...
\vskip-0.4in
\begin{center}
\centerline{\includesvg[width=2.\columnwidth]{figs/NAISR transfer.svg}}
\caption{Shape transfer of a patient. Blue shapes are the groundtruth shapes and the red shapes are the reconstructed shapes. The table below lists the covariates for the shape above. est. volume represents the volume of the reconstructed shape. ori. volume represents the volume of the groundtruth shape.}
\end{center} 
\end{figure*}
\end{comment}
...
\vskip-0.2in
\centerline{\includesvg[width=0.9\columnwidth]{figs/summary_test_metrics_transfer_volume.svg}}
\caption{Volume difference of transfered shapes and grountruth shapes. The blue bars in the boxplot represents the reconstruction step that follows~\eqref{eq.infer_z}. The orange bars represent the reconstruction step follows~\eqref{eq.infer_cz}. The unit is $cm^3$.  }
\label{fig.transfer}
\end{figure}
...
\end{tabular}
\end{center}
\caption{Volume of difference of transfered shapes and grountruth shapes. Bold scores perform the best across all methods. The unit is $cm^3$.  }
\label{tab.transfer}
\end{figure}
...
\vskip 0.2in
\begin{center}
\centerline{\includesvg[width=2.2\columnwidth]{figs/1379.svg}}
\caption{Manifold}
\label{fig.transfer}
\end{center}
\vskip -0.2in
...
\vskip 0.2in
\begin{center}
\centerline{\includesvg[width=2.2\columnwidth]{figs/manifold3.svg}}
\caption{Manifold}
\label{fig.transfer}
\end{center}
\vskip -0.2in
