					...
					\begin{prop}\label{nmaxthobs1 bd} 
						...
						\begin{prop}\label{nmaxthobs1 bd_vel} 
							...
							{\subsection{Interior control} In this section, we study the lack of exact controllability of the system \eqref{nmaxeq3} when the localized interior control is acting on the density equation. Similar result holds true for the remaining two cases (control in velocity or control in stress). We establish the result by violating the observability inequality on an appropriate solution of the corresponding adjoint system. Our proof is in the same spirit of \cite[Section 3]{Beauchard}. We first find a candidate function as a terminal data of the transport equation and exploit the lack of controllability of the same in small time.
								...
								\end{theorem}
								\begin{proof}
									Since $|\beta|T<\max\{l_1, 2\pi-l_2\}$, there exists a nontrivial function $\hat \sigma_T \in C_c^{\infty}(0,2\pi)$ 
									...
									\begin{equation}\label{tr}
										\begin{cases}
											\hat{\sigma}^N_t-\beta\hat{\sigma}^N_x-\omega_1\hat \sigma^N=0, &  \mbox{ in } (0,T) \times (0, 2\pi),\\	\hat{v}^N_t-\beta\hat{v}^N_x-\omega_2\hat v^N=0, &  \mbox{ in } (0,T) \times (0, 2\pi),\\	\hat{S}^N_t-\beta\hat{S}^N_x-\omega_3\hat S^N=0, &  \mbox{ in } (0,T) \times (0, 2\pi),\\
									...
									\noindent
									Let us denote the $3\times 3$ matrix $R:=\text{diag}(\omega_1, \omega_2, \omega_3).$
								...
								\begin{theorem}
									Let $0<T<\frac{2\pi}{\beta}$, where $\beta$ is define in \Cref{lack theorem}. Then the system  \eqref{nmaxeq3 bd} is not controllable in small time by means of boundary control acting in the boundary term in the velocity.
								\end{theorem} Let us first consider the following transport equations
								\begin{equation}\label{trnsprt_b1}
									\begin{cases}
										\check{\sigma}_t+\beta\check{\sigma}-\omega_1\check{\sigma}=0,\ & \text{ in } (0,T)\times(0,2\pi),\\
								...
								such that the solutions $\check{\sigma}$  of \eqref{trnsprt_b1} satisfy %$\text{supp}(\tilde{\sigma})\subset\blue\{(t,x)\ :\ \bar{u}t<x<\bar{u}t+(2\pi-\bar{u}T),\ 0<t<T\}$. {\blue In particular, we have 
									...
									%Since $\hat{\sigma}_T^N$ is the image of $\hat{\sigma}_T$ by a differential operator, we have $\text{supp}({\hat{\sigma}_T^N})\subset \text{supp}(\hat{\sigma}_T)$. Let us write
									$$
									\check{\sigma}_T=\sum_{n \in \mathbb{Z}} c_n e^{ in   x} \text {, where } c_n=\int_{0}^{2\pi} e^{ in   x} \check{\sigma}_T(x) d x, \forall n \in \mathbb{Z} .
									$$
									...
									\begin{equation}\label{tr1}
										\begin{cases}
											...
											\check{v}^N_t-\beta\check{v}^N_x-\omega_2\check v^N=0, &  \mbox{ in } (0,T) \times (0, 2\pi),\\
											\check{S}^N_t-\beta\check{S}^N_x-\omega_3\check S^N=0, &  \mbox{ in } (0,T) \times (0, 2\pi),\\
									...
									We write the solutions of the systems \eqref{tr} and \eqref{nmaxeqadj lack} respectively as
									\begin{align}
										\left(\check{\sigma}^N, 	\check{v}^N, 	\check{S}^N\right)^\top(t,x)&=\sum_{|{n}|\geq N+1}a^N_n \, e^{-\beta in(T-t)}e^{inx}e^{-tR}\left(1,  -\frac{ \beta_l+u_s }{ \rho_s}, \frac{\mu\left( \beta_l+u_s\right) }{\kappa \rho_s \beta_l}\right)^{\top},\\
										\left(\sigma^N,v^N,\tilde {S}^N\right)^\top(t,x)&=\sum_{|{n}|\geq N+1}a^N_n \, e^{\overline{\lambda_n^1}(T-t)}e^{inx}\left(1,  \alpha_{n,1}^2,   \alpha_{n,1}^3 \right)^{\top}.
									\end{align}
									%\begin{align}
