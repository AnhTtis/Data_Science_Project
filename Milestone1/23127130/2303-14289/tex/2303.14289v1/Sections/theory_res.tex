...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we prove theoretical convergence guarantees for our proposed algorithmic framework (\texttt{GTA}). 
%In this section, we analyse the theoretical convergence of our proposed algorithm. 
The analysis is divided into three subsections. 
In \cref{sec.mult comms}, we analyze the effect of multiple communications, i.e., $n_c \geq 1$ (and $n_g = 1$), on \texttt{GTA} and the three cases \texttt{GTA-1}, \texttt{GTA-2} and \texttt{GTA-3}. 
...
%We do this as the analysis is simpler to understand without multiple computation ($n_g = 1$). 
...
%for a fully connected network as it's a special case not covered by the analysis of previous subsections. 
...
\bassumption    \label{asum.convex and smooth}
    ...
    \begin{align*}
        ...
        &\|\nabla f_i(z) - \nabla f_i(z')\|_2 \leq L\|z - z'\|_2,    \qquad \forall \; i = 1, \dots, n.
    \end{align*}
\eassumption
...
% \end{align*}
where $\beta \in [0, 1)$ because $\Wmbf$ is a mixing matrix for a connected network and $\beta_i \in [0,1]$ because $\Wmbf_i$ are symmetric doubly stochastic for $ i = 1, 2, 3, 4$. Using the definitions of $\Zmbf^{n_c} = \Wmbf^{n_c}\otimes I_{d}$ and $\Zmbf_i^{n_c} = \Wmbf_i^{n_c}\otimes I_{d}$ for $ i = 1, 2, 3, 4$, it follows that
...
\end{align}
where $x_{i,k,j}$, denotes the local copy of the $i$th node, in outer iteration $k$ and inner iteration $j$. 
...
\input{Sections/Proof subsections/mult_comms.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Deterministic Analysis for g > 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{Sections/Proof subsections/mult_grads.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Fully connected Graph Analysis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{Sections/Proof subsections/fully_connected.tex}
