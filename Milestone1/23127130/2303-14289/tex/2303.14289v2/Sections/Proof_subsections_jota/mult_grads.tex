...
We now define the explicit form of $B(n_c, n_g)$ 
...
\bremark 
Linear convergence can be established for the iterates generated by \cref{alg : Deterministic} by treating inner iterations as a special case of of time-varying networks and following the analysis techniques in \cite{nedic2017geometrically, nedic2017achieving, nguyen2022performance}. Such analysis would ensure descent in each inner iteration and require that the step size satisfy a pessimistic $\mathcal{O}(\frac{1}{n_g^2})$ condition. Our analysis takes a different approach; we quantify the error across outer iterations, and as a result, the condition on the step size is less pessimistic, i.e., $\mathcal{O}(\frac{1}{n_g})$. That said, the analysis involves a complicated error recursion that makes it difficult to explicitly quantify the rate of convergence of \cref{alg : Deterministic}.
%\sg{Alternatively, linear convergence to the solution for \cref{alg : Deterministic} can be established by considering the inner iterations as a special case of time-varying networks and following the analysis in works like \cite{nedic2017geometrically, nedic2017achieving, nguyen2022performance}. This analysis would ensure descent in each inner iteration resulting in a $\mathcal{O}(\frac{1}{n_g^2})$ condition on the step size $\alpha$. Our analysis goes one step further by quantifying error not for each inner iterate but between iterates obtained after each outer iteration, resulting in a less pessimistic condition for the step size, i.e., $\mathcal{O}(\frac{1}{n_g})$. A downside of this analysis is a highly complex error recursion making it difficult to quantify the convergence rate for \cref{alg : Deterministic}}
% \rb{
% While linear convergence can be achieved by adopting the techniques found in works such as \cite{nedic2017geometrically, nedic2017achieving, nguyen2022performance} and treating inner iterations as a specialized form of time-varying networks, our analysis takes a different approach. The alternative approach enforces stronger conditions, like descent in each inner iteration, which results in a more pessimistic condition of $\mathcal{O}(\frac{1}{n_g^2})$ on the step size $\alpha$. In contrast, our approach involves quantifying the error between iterates obtained after each outer iteration rather than the inner iterations, leading to a more favorable conditions on the step size, i.e., $\mathcal{O}(\frac{1}{n_g})$. }
\eremark

Similar to \cref{th. general g=1 rate bound}, the only constraint \cref{th. alpha bound g > 1} imposes on the system is $\beta_1, \beta_3 < 1$. This implies the communication matrices $\Wmbf_1$ and $\Wmbf_3$ must represent connected networks (not necessarily the same network) even when multiple communication and multiple computation steps are performed. \cref{th. alpha bound g > 1} does not impose any restrictions on the relation among $\Wmbf_1$, $\Wmbf_2$, $\Wmbf_3$ and $\Wmbf_4$. Thus, it allows for more flexibility than the structures considered in the literature even when multiple communication and multiple computation steps are performed.  
...
Based on \cref{th. alpha bound g > 1}, the step size conditions for methods described in \cref{tab: Algorithm Def} can be derived. We omit these conditions as they are complex and do not offer any additional insights. 
We also omit the counterpart to \cref{th. general g=1 rate bound} as the matrix $B(n_c, n_g)$ is now a dense matrix, thus any such bounds are again highly complex and do not offer strong insights into the effects of communication and computation on the convergence rate. If $B(n_c, n_g)$ is a reducible matrix, the analysis for the progression of $r_k$ can be further simplified from \cref{lem:lyapunov g > 1}. The analysis for this case is presented in \cref{sec.full graph res} with the examples of \texttt{GTA-2} and \texttt{GTA-3} when $\Wmbf = \frac{1_n1_n^T}{n}$, i.e., $\beta= 0$. 
