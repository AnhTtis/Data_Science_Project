...
Chemical potentials, denoted as $\mu_\sigma$, are used to control particle numbers $N_\sigma = \int n_\sigma\,(\bm{r}) d\bm{r}$ associated with a given spin component.
...
For the computation we use W-SLDA Toolkit \cite{PhysRevLett.120.253002,PhysRevLett.112.025301,WSLDAToolkit}. All technical parameters, needed to restore the calculations presented below are included into reproducibility packs~\cite{SM}.
...
such that $a(t)$ and the first derivative $\partial_t a(t)$ are continuous, leading to $t_1 - t_0 = {\pi}/{\omega_1}$ and $t_q - t_1 = {\pi}/{\omega_2}$. Using this parametrization, the limit $\omega_1 \ll \eF$ corresponds to an adiabatic process, i.e. a small energy transfer to the system. On the other hand $\omega_2 \gtrsim \eF$ generates the rapid quench.
    ...
    \end{enumerate}
    where $\epsilon\ll\eF$ and $\sigma\approx\eF$. In numerical realization the Dirac function $\delta(x)$ is approximated by narrow Gaussian function.
...
\end{equation}
where elliptic modulus parameter is related to maximum and minimum value of the paring field during the oscillations $k\approx 1-\left(\frac{\min[\Delta(t)]}{\max[\Delta(t)]}\right)^2$.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{dn-fit2.png}
\caption{Time evolution of the pairing field (order parameter) magnitude in the center of the simulation domain $\Delta(x=0,t)$ for times corresponding to the Higgs oscillation regime. (a)~Large amplitude Higgs oscillations (blue line), induced by rapidly quenching the interaction from $a_1 k_F=-0.1$ to $a_0 k_F=-10$. They are well reproduced by elliptic function~(\ref{eq:dn}) with $\Omega=0.95\Delta_0$ and $k=0.993$ (orange line). (b)~Small amplitude Higgs mode (blue line), induced by rapidly quenching the interaction from $a_1 k_F=-5$ to $a_0 k_F=-10$. The oscillation frequency is $\omega=2\Delta_0$ (orange line).}
\label{fig:omega}
\end{figure}
...
Clearly, there exist optimal value of $k$ (resonance) that trigger the instability almost immediately. In Fig.~\ref{fig:decay_k}(b) we show the Fourier spectra of the order parameter after applying the perturbation, but before the decay. It is seen that the uniform Higgs mode ($k=0$) first decays into two modes with $+k_{d}$ and $-k_{d}$. The diagram representing schematically the decay of small amplitude mode is shown in inset of Fig.~\ref{fig:decay_k}(b).
\begin{figure}[t]
\centering
\includegraphics[width=0.49\textwidth]{kmode-inset.png}
\includegraphics[width=0.49\textwidth]{kmode04.png}
\caption{Panel (a): Decay time as a function of the modulation wave vector $k/k_F$. Panel (b): Fourier spectra of the order parameter just before the decay induced by the external modulation $k/k_F=0.4$. Inset: The decay diagram for the small amplitude Higgs mode.}
\label{fig:decay_k}
\end{figure}
...
\includegraphics[width=\textwidth]{fig6-alt2.png}
    ...
    e) 3D configuration for $P=5\%$. Upper colorbar indicates local spin polarization $p(\bm{r})$ on the right panel; colorbar on the right shows the absolute value of the order parameter $|\Delta(\bm{r})|$, normalized to $e_F$. Inner inset shows the contour surface for which the order parameter is zero. The dimensionless size of the box is $L\kF=64$ in each direction.}
    \label{fig:glass}
\end{figure*}
...
In ordered to reproduce numerical results presented in this paper, use reproducibility packs of W-SLDA Toolkit, provided through the arxiv preprint system~\cite{SM}. They contain detailed information about the numerical setup and computation process.
...
The calculations were executed on Piz Daint supercomputer based in Switzerland at Swiss National Supercomputing Centre (CSCS), PRACE allocation No. 2021240031. We also acknowledge the Global Scientific Information and Computing Center, Tokyo Institute of Technology for resources at TSUBAME3.0 (project ID: hp210079).
...
J.~Tokimoto, S.~Tsuchiya, and T.~Nikuni, ``{H}iggs mode in a trapped superfluid
  fermi gas,'' {\em J. Low Temp. Phys.}, vol.~187, p.~765, 6 2017.

\bibitem{PhysRevA.90.023621}
G.~M. Bruun, ``Long-lived higgs mode in a two-dimensional confined fermi
...
J.~Tokimoto, S.~Tsuchiya, and T.~Nikuni, ``Excitation of {H}iggs mode in
  superfluid fermi gas in bcs–bec crossover,'' {\em J. Phys. Soc. Japan},
...
J.~Kombe, J.-S. Bernier, M.~K\"ohl, and C.~Kollath, ``Finite-duration
  interaction quench in dilute attractively interacting fermi gases: Emergence
...
M.~H. Szyma\'{n}ska, B.~D. Simons, and K.~Burnett, ``Dynamics of the bcs-bec
  crossover in a degenerate fermi gas,'' {\em Phys. Rev. Lett.}, vol.~94,
  p.~170402, May 2005.

\bibitem{PhysRevA.73.013609}
W.~Yi and L.-M. Duan, ``Dynamic response of an ultracold fermi gas near the
...
M.~Dzero, E.~A. Yuzbashyan, and B.~L. Altshuler, ``Cooper pair turbulence in
  atomic fermi gases,'' {\em EPL}, vol.~85, p.~20004, 2009.
...
B.~T\"{u}zemen, T.~Zawi{\'{s}}lak, G.~Wlaz{\l}owski, and P.~Magierski,
  ``Disordered structures in ultracold spin-imbalanced fermi gas,'' {\em New J.
...
P.~Magierski, B.~T\"uzemen, and G.~Wlaz\l{}owski, ``Spin-polarized droplets in
  the unitary fermi gas,'' {\em Phys. Rev. A}, vol.~100, p.~033613, Sep 2019.
...
B.~T\"{u}zemen, P.~Kukli{\'{n}}ski, P.~Magierski, and G.~Wlaz{\l}owski,
  ...
  unitary fermi gas,'' {\em Acta Phys. Pol. B}, vol.~51, no.~3, p.~595, 2020.

\bibitem{PhysRevA.104.033304}
P.~Magierski, B.~T\"uzemen, and G.~Wlaz\l{}owski, ``Dynamics of spin-polarized
  impurity in ultracold fermi gas,'' {\em Phys. Rev. A}, vol.~104, p.~033304,
  Sep 2021.
...
A.~Boulet, G.~Wlaz\l{}owski, and P.~Magierski, ``Local energy density
  functional for superfluid fermi gases from effective field theory,'' {\em
  Phys. Rev. A}, vol.~106, p.~013306, 7 2022.

\bibitem{PhysRevLett.101.215301}
A.~Bulgac and M.~M. Forbes, ``Unitary fermi supersolid: The larkin-ovchinnikov
...
\bibitem{SM}
``{See ancillary files added to arXiv preprint version}.''

\bibitem{arxiv.2210.09829}
G.~Lyu, K.-T. Xi, S.~Yoon, Q.~Chen, and G.~Watanabe, ``Exciting long-lived
  {H}iggs mode in superfluid fermi gases with particle removal,'' 2022.
...
K.~Hossain, K.~Kobuszewski, M.~M. Forbes, P.~Magierski, K.~Sekizawa, and
  G.~Wlaz\l{}owski, ``Rotating quantum turbulence in the unitary fermi gas,''
