\documentclass[twoside,twocolumn,aps,10pt,prb,floatfix,showpacs,citeautoscript,superscriptaddress]{revtex4-2}
...
The local density approximation (LDA), two generalized gradient approximations (PBE and PBEsol) as well as a functional including non-local correlation effects (CX) all give a double minimum PES for the R-tilt mode coordinate using the experimental value for the lattice constant.
Using the corresponding theoretical lattice constants, LDA gives deeper potential minima, while for PBE the double minimum feature disappear.
Two hybrid functionals (HSE and CX0p) show in general better agreement for the various vibrational frequencies and both show an anharmonic PES for the R-tilt mode, quite similar to PBE at its theoretical lattice constant. The computational cost for the hybrid functionals is substantial compared with generalized gradient approximations and as a compromise between accuracy and computational cost we have therefore chosen to base this study on PBE \cite{PerBurErn1996} using its theoretical lattice constant. For comparison, PBEsol \cite{perdewRestoringDensityGradientExpansion2008} has also been used, and the corresponding results are presented in \autoref{sec:discussion}.
...
For more details see Supplemental Material at [URL will be inserted by publisher].
...
At the R-point the lowest branch at \SI{3.8}{\milli\electronvolt} corresponds to out-of-phase tilts of sequential ZrO$_6$ octahedra. This will here be denoted the {\bf R-tilt mode}.
...
The phonon dispersions in \autoref{fig:harmonic_dispersion} are very similar to what is obtained from the standard small displacement method, see e.g., Ref.~\cite{perrichonUnravelingGroundStateStructure2020}. However, the R-tilt mode frequency is sensitive to the theoretical treatment due to anharmonicity. Here, we have developed a harmonic model based on displacements from \gls{md} simulations carried out at 10, 150, 300 and \SI{500}{K}. The obtained R-tilt mode frequency is \SI{3.8}{\milli\electronvolt}, slightly larger than \SI{2.3}{\milli\electronvolt}, obtained using the standard small displacement method with the default displacement 0.01 \AA~\cite{perrichonUnravelingGroundStateStructure2020}.
...
By comparing with DFT calculations and the calculated phonon dispersions in Fig.~\ref{fig:harmonic_dispersion} the low-frequency peak can be attributed to the out-of-phase oxygen tilt oscillations, the R-tilt mode, while the higher-frequency peak corresponds to the acoustic branch and is connected to barium displacements, the R-acoustic mode. We also checked that the R-tilt mode vanishes at the R point (1.5 1.5 1.5) while the R-acoustic mode is still present, which is consistent with the neutron selection rules expected for modes of $R_4^+$ et $R_5^+$ symmetry, respectively~\cite{ExperimentIN8}.

INS spectra of BaZrO$_3$ are proportional to the neutron coherent cross-section: $(\Bar{b})^2 \frac{k_f}{k_i} S(\boldsymbol{Q}, E)$, where $b$ is the nuclear scattering length, $k_i$ (respectively $k_f$) the wave vector of the incident (respectively scattered) neutron beam and $S(\boldsymbol{Q}, E)$ the neutron scattering function. For oxygen and barium, the coherent scattering lengths are quite similar ($b_\textrm{O}=5.803$ fm and $b_\textrm{Ba}=5.07$ fm) while zirconium has a higher one ($b_\textrm{Zr}=7.16$ fm) \cite{Neutron-scattering}. Hence, for given $k_i$ and $k_f$ and with the proper selection rules, neutron intensity should be of the same order of magnitude for the oxygen-dependent R-tilt and for barium-dependent R-acoustic modes. 
...
\begin{figure}[htpb]
    ...
    \caption{Measured dispersion curves along the $\Gamma$--R--M path, measured at \SI{2}{\kelvin} by inelastic neutron scattering (INS) and at \SI{80}{\kelvin} by inelastic x-ray scattering (IXS). Indicative errorbars have been estimated and added, dependant of the scattering methods and ease of fit. Open symbols indicate data points that we could not identify by comparison to phonon calculations.
...
To reconstruct phonon dispersions, we have also measured spectra away from the R-point, in the directions  R(0.5 0.5 2.5)--M(0.5 0.5 2) and R(0.5 0.5 2.5)--$\Gamma$(1 1 2) at \SI{2}{\kelvin} for INS. For IXS, we measured in the directions  R(0.5 0.5 3.5)--M(0.5 0.5 4) and R(0.5 0.5 3.5)--$\Gamma$(1 1 4) at \SI{80}{\kelvin}. When both R-tilt and R-acoustic modes were resolved, INS spectra were fitted as previously. However, further from R, it becomes very difficult to resolve two peaks as the R-tilt mode broadens and crosses the R-acoustic mode. Then, spectra were fitted by a single-damped harmonic oscillator to extract the frequency of the R-acoustic mode. For the IXS experiments, spectra were fitted as previously by the sum of damped harmonic oscillators for the R-tilt mode and R-acoustic mode, and by a Lorentzian lineshape for the pseudo-elastic peak centered around \SI{0}{\milli\electronvolt}. For select spectra, higher frequencies features appear above \SI{20}{\milli\electronvolt} and up to the edge of the frequency range ($\approx$ \SI{30}{\milli\electronvolt}). When these features are present, they have been fitted by additional damped harmonic oscillators. 

Frequencies extracted from the fits of INS and IXS spectra are represented in Fig.~\ref{fig:Exp_dispersion}. The spectra and their fits are given in Supp. Mat. [Supplementary information]. We can first observe that the frequency of the R-acoustic mode is very consistent between the INS and IXS measurements, the difference in temperature for the two measurements being here negligible. The R-acoustic mode reaches its maximum frequency of around \SI{13}{\milli\electronvolt} at the R-point. This frequency then decreases towards $\Gamma$ and tends towards a value of around \SI{11}{\milli\electronvolt} at the M-point.

The R-point soft-mode is at its lowest energy at R (in INS $\approx$ \SI{5.6}{\milli\electronvolt}, in IXS $\approx$ \SI{5.2}{\milli\electronvolt})  and rises quickly in energy as we probe further from R. While the spectra are particularly unambiguous at the R point itself, the picture becomes much less clear as we move away from the zone boundary. Along the R-M direction, we observe a single branch attributed to oxygen, whereas phonon band calculations and symmetry considerations predict a splitting into two oxygen branches, see Fig.~\ref{fig:harmonic_dispersion}. The tilt modes at M also could not be identified with certainty. It is not clear at this stage why this is the case. No strict selection rules are expected but it is probable that the polarization (atomic displacements) for these modes are incompatible with the geometry of the scattering experiment, resulting in negligible intensity. Further experiments assisted with accurate calculations of scattering cross sections will be needed to clarify the modes at M and the dispersion along the R-M direction. 

In addition, in the IXS data we observe extra modes at higher frequency (above \SI{20}{\meV}, shown in open symbols on Fig.~\ref{fig:Exp_dispersion}) both in the middle of the R-$\Gamma$ direction and in the R-M direction. It is tempting to attribute these modes to the Zr-dominated phonon branch seen in Fig.~\ref{fig:harmonic_dispersion}, the quantitative agreement of the phonon energies remains unsatisfactory in comparison to the acoustic and tilt branches. Further studies will be necessary to rule out a spurious origin for these modes and clarify their assignment. However, they should not impact the low-frequency study of the R-tilt mode and acoustic mode and will therefore not be discussed any further.
%%% TEXEXPAND: END FILE ./tex-files/experimental_methods_and_results.tex
...
We consider a 8x8x8 supercell ($N$=2560 atoms) with periodic boundary conditions and the \gls{md} simulations are carried out using the \textsc{GPUMD}~\cite{fanEfficientMolecularDynamics2017} package with the \gls{fcp} \cite{brorssonEfficientCalculationLattice2022}.
...
\begin{align}
    F(\boldsymbol{q},t) = \sum_i^N \sum_j^N \left < b_i b_j \mathrm{exp} \left [ i \boldsymbol{q} \cdot ( \boldsymbol{r}_i(t)-\boldsymbol{r}_j(0)) \right ] \right >_{\textrm{time}}
\end{align}
is determined at the R point, for the wavevector $\boldsymbol{q}=(2\pi/a)[0.5,0.5,2.5]$, as a time average 
$\langle \cdots \rangle_{\textrm{time}}$
...
\end{align}
is then obtained by a Fourier transformation and $S(\boldsymbol{q},\omega)$ is averaged over 50 independent \gls{md} simulations.
...
In order to extract the frequencies, we fit the spectra to a damped harmonic oscillator model (see Supplemental Material at [URL will be inserted by publisher]). The obtained theoretical frequencies are compared with our experimental data points in \autoref{fig:rmod_frequency}.

\begin{figure}[ht]
\centering
\includegraphics{fig6.pdf}
\caption{
    The temperature dependency of the dynamical structure factor $S(\boldsymbol{q},\omega)$ at the R point ($\boldsymbol{q}=(2\pi/a)[0.5,0.5,2.5]$) from an MD simulation based on PBE.
}
...
Next, we consider the \gls{scp} method. The modelling is based on a 6x6x6 supercell, and we use M=80 structures in each iteration.
...
and the iteration in \eq{eq:scph} is stopped once the free energy per atom is smaller than $\SI{0.001}{meV}$ for two successive iterations.
...
\caption{
    The phonon dispersions at different temperatures obtained from the self consistent phonon (\gls{scp}) model based on PBE.
}
...
The R-tilt mode shows a strong temperature dependence. From the INS data we obtain a softening from \SI{9.4}{\milli\electronvolt} at \SI{296}{\kelvin} to \SI{5.6}{\milli\electronvolt} at \SI{2}{\kelvin}, while the IXS data decrease from \SI{8.3}{\milli\electronvolt} at \SI{300}{\kelvin} to \SI{5.2}{\milli\electronvolt} at \SI{80}{\kelvin}. The INS frequencies are therefore consistently higher by about \SI{1}{\milli\electronvolt} but also more reliable since the R-tilt mode is weak in the IXS spectra. Our theoretical results, the data based on the self-consistent phonon (SCP) method and the result obtained using the dynamic simulation technique (MD), agree nicely with the experimental data.

\begin{figure}[ht]
\centering
\includegraphics{fig8.pdf}
\caption{
    The temperature dependency for the R-tilt and R-acoustic modes, both from experiments (INS and IXS) and theory (SCP and MD). HP is the harmonic approximation, which is temperature independent. The theoretical data are based on PBE. For experimental data, indicative errorbars have been estimated depending of the scattering method and ease of fit.
}
...
However, using improved approximations for the exchange-correlation approximations within \gls{dft} we expect the agreement to be improved. It was shown in Ref. \cite{granhedBaZrOStabilityPressure2020} that inclusion of Fock exchange as implemented in the two hybrid functionals, HSE \cite{heydErratumHybridFunctionals2006} and CX0p \cite{jiaoExtentFockexchangeMixing2018}, stiffens the atomic bonds and thus increases the vibrational frequencies. It then follows that the \gls{msrd} will decrease.
...
A 4th order \gls{fcp} is constructed based on the PBEsol functional (for details, see Supplemental Material at [URL will be inserted by publisher]).
...
We have here presented a combined experimental and theoretical study of the temperature dependence of the antiferrodistortive R-tilt mode in barium zirconate (BZO). 
...
These displacements are measured in EXAFS and the fluctuations of the Ba-O distance are a sensitive local measure of the out-of-phase tilt oscillations of the oxygen octahedra. 
...
Altogether, our work provides a robust description of the lattice dynamics and its temperature dependence at the R point, but future work will be needed to clarify open questions on the lattice dynamics away from R. This includes the exact position and temperature dependence of the tilt branch along the R-M direction, as well as a proper deconvolution of the inelastic spectrum, in order to understand in details the strong asymmetry of the phonon mode and extract intrinsic widths, possibly in the relation to the proposed presence of nanodomains. We anticipate that further inelastic scattering experiments or diffuse scattering experiment will help to address this issue and that extended computational modeling may uncover the presence or absence of the proposed nanodomains.
\\%%% TEXEXPAND: END FILE ./tex-files/conclusion.tex


\section{Acknowledgements}
Rapha\"el Haumont and Romuald Saint-Martin from the ICMMO - UMR 8182, Université Paris Saclay, are acknowledged for their help in the preparation of the single crystals used for the INS and IXS experiments.
...
\bibitem [{\citenamefont {Glazer}(2011)}]{glazerBriefHistoryTilts2011}%
  ...
  {Glazer}},\ }\href {https://doi.org/10.1080/01411594.2010.544732} {\bibfield
...
\bibitem [{\citenamefont {Howard}\ and\ \citenamefont
  ...
  {Stokes}},\ }\href {https://doi.org/10.1107/S0108767304024493} {\bibfield
  {journal} {\bibinfo  {journal} {Acta Crystallogr. A}\ }\textbf {\bibinfo
  {volume} {61}},\ \bibinfo {pages} {93} (\bibinfo {year} {2005})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Sai}\ and\ \citenamefont
  ...
  {Vanderbilt}},\ }\href {https://doi.org/10.1103/PhysRevB.62.13942} {\bibfield
   {journal} {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume}
  {62}},\ \bibinfo {pages} {13942} (\bibinfo {year} {2000})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont
  ...
  {Knight}},\ }\href {https://doi.org/10.1007/s10853-020-04453-5} {\bibfield
  {journal} {\bibinfo  {journal} {J. Mater. Sci.}\ }\textbf {\bibinfo {volume}
  {55}},\ \bibinfo {pages} {6417} (\bibinfo {year} {2020})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Sebastian}(2010)}]{sebastian2010dielectric}%
  ...
  for Wireless Communication}}},\ Dielectric Materials for Wireless
  Communication\ (\bibinfo  {publisher} {{Elsevier Science}},\ \bibinfo {year}
...
\bibitem [{\citenamefont {Vassen}\ \emph {et~al.}(2000)\citenamefont {Vassen},
  ...
  {\bibfnamefont {D.}~\bibnamefont {St{\"o}ver}},\ }\href
  {https://doi.org/10.1111/j.1151-2916.2000.tb01506.x} {\bibfield  {journal}
  {\bibinfo  {journal} {J. Am. Ceram. Soc.}\ }\textbf {\bibinfo {volume}
  {83}},\ \bibinfo {pages} {2023} (\bibinfo {year} {2000})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Kreuer}(2003)}]{KreuerProtonConductingOxides2003}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {K.}~\bibnamefont
  {Kreuer}},\ }\href {https://doi.org/10.1146/annurev.matsci.33.022802.091825}
...
\bibitem [{\citenamefont {Dobal}\ \emph {et~al.}(2001)\citenamefont {Dobal},
  ...
  {\bibfnamefont {A.~S.}\ \bibnamefont {Bhalla}},\ }\href
...
\bibitem [{\citenamefont {Zhong}\ and\ \citenamefont
  ...
  {Vanderbilt}},\ }\href {https://doi.org/10.1103/PhysRevLett.74.2587}
  {\bibfield  {journal} {\bibinfo  {journal} {Phys. Rev. Lett.}\ }\textbf
  {\bibinfo {volume} {74}},\ \bibinfo {pages} {2587} (\bibinfo {year}
...
\bibitem [{\citenamefont {Akbarzadeh}\ \emph {et~al.}(2005)\citenamefont
  ...
  {author} {\bibfnamefont {J.~M.}\ \bibnamefont {Kiat}},\ }\href
...
\bibitem [{\citenamefont {Bili{\'c}}\ and\ \citenamefont
  ...
  {Gale}},\ }\href {https://doi.org/10.1103/PhysRevB.79.174107} {\bibfield
  {journal} {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume}
  {79}},\ \bibinfo {pages} {174107} (\bibinfo {year} {2009})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Chen}\ \emph {et~al.}(2018)\citenamefont {Chen},
  ...
  {\bibfnamefont {J.}~\bibnamefont {{\'I}{\~n}iguez}},\ }\href
...
\bibitem [{\citenamefont {Amoroso}\ \emph {et~al.}(2018)\citenamefont
  ...
  {https://doi.org/10.1103/PhysRevB.97.174108} {\bibfield  {journal} {\bibinfo
  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume} {97}},\ \bibinfo
  {pages} {174108} (\bibinfo {year} {2018})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Perrichon}\ \emph {et~al.}(2020)\citenamefont
  ...
  {\bibfnamefont {M.}~\bibnamefont {Karlsson}},\ }\href
  {https://doi.org/10.1021/acs.chemmater.9b04437} {\bibfield  {journal}
...
\bibitem [{\citenamefont
  ...
  {Evarestov}},\ }\href {https://doi.org/10.1103/PhysRevB.83.014105} {\bibfield
   {journal} {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume}
  {83}},\ \bibinfo {pages} {014105} (\bibinfo {year} {2011})}\BibitemShut
  {NoStop}%
...
\bibitem [{\citenamefont {Levin}\ \emph {et~al.}(2021)\citenamefont {Levin},
  ...
  {\bibfnamefont {R.~A.}\ \bibnamefont {Maier}},\ }\href
  {https://doi.org/10.1103/PhysRevB.104.214109} {\bibfield  {journal} {\bibinfo
   {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume} {104}},\ \bibinfo
  {pages} {214109} (\bibinfo {year} {2021})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Xin}\ \emph {et~al.}(2019)\citenamefont {Xin},
  ...
  and\ \bibinfo {author} {\bibfnamefont {J.}~\bibnamefont {Kreisel}},\ }\href
...
\bibitem [{\citenamefont {Hellman}\ \emph {et~al.}(2011)\citenamefont
  ...
  {Simak}},\ }\href {https://doi.org/10.1103/PhysRevB.84.180301} {\bibfield
  {journal} {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume}
  {84}},\ \bibinfo {pages} {180301} (\bibinfo {year} {2011})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Monacelli}\ \emph {et~al.}(2021)\citenamefont
  ...
  \bibinfo {pages} {363001} (\bibinfo {year} {2021})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Fransson}\ \emph {et~al.}(2021)\citenamefont
  ...
  {journal} {Adv. Theory Simul.}\ }\textbf {\bibinfo {volume} {4}},\ \bibinfo
  {pages} {2000240} (\bibinfo {year} {2021})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Perdew}\ \emph {et~al.}(2008)\citenamefont {Perdew},
  ...
  {\bibfnamefont {K.}~\bibnamefont {Burke}},\ }\href
...
\bibitem [{\citenamefont {Gonze}\ \emph {et~al.}(1994)\citenamefont {Gonze},
  ...
  \bibinfo {author} {\bibfnamefont {D.}~\bibnamefont {Allan}},\ and\ \bibinfo
  {author} {\bibfnamefont {M.}~\bibnamefont {Teter}},\ }\href
...
\bibitem [{\citenamefont {Gonze}\ and\ \citenamefont
  ...
  }\href {https://doi.org/10.1103/PhysRevB.55.10355} {\bibfield  {journal}
  {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo {volume} {55}},\
  \bibinfo {pages} {10355} (\bibinfo {year} {1997})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Togo}\ and\ \citenamefont
  {Tanaka}(2015)}]{TOGO20151}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
  {Togo}}\ and\ \bibinfo {author} {\bibfnamefont {I.}~\bibnamefont {Tanaka}},\
  }\href {https://doi.org/https://doi.org/10.1016/j.scriptamat.2015.07.021}
...
\bibitem [{\citenamefont {Piovano}\ \emph {et~al.}(2020)\citenamefont
  ...
  {author} {\bibfnamefont {F.}~\bibnamefont {Charpenay}},\ }\href@noop {}
  {\bibfield  {journal} {\bibinfo  {journal} {ILL Annual report 2020}\ ,\
  \bibinfo {pages} {76}} (\bibinfo {year} {2020})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Guennou}\ \emph {et~al.}(2021)\citenamefont
  ...
  report}} (\bibinfo {year} {2021}),\ \bibinfo {note} {{I}nstitut Laue-Langevin
  (ILL)}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Guennou}\ \emph {et~al.}(2019)\citenamefont
  ...
  report}} (\bibinfo {year} {2019}),\ \bibinfo {note} {{I}nstitut Laue-Langevin
  (ILL)}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Krisch}\ and\ \citenamefont
  ...
  }in\ \href {https://doi.org/10.1007/978-3-540-34436-0_5} {\emph {\bibinfo
  {booktitle} {Light Scattering in Solid IX}}},\ \bibinfo {editor} {edited by\
  \bibinfo {editor} {\bibfnamefont {M.}~\bibnamefont {Cardona}}\ and\ \bibinfo
  {editor} {\bibfnamefont {R.}~\bibnamefont {Merlin}}}\ (\bibinfo  {publisher}
  {Springer Berlin Heidelberg},\ \bibinfo {address} {Berlin, Heidelberg},\
  \bibinfo {year} {2007})\ Chap.~\bibinfo {chapter} {5}, pp.\ \bibinfo {pages}
  {317--370}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Girard}\ \emph {et~al.}(2019)\citenamefont {Girard},
  ...
  {A.}~\bibnamefont {Bosak}},\ }\href
  {https://doi.org/10.1107/S1600577518016132} {\bibfield  {journal} {\bibinfo
  {journal} {J. Synchrotron Radiat.}\ }\textbf {\bibinfo {volume} {26}},\
  \bibinfo {pages} {272} (\bibinfo {year} {2019})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Sears}(1992)}]{Neutron-scattering}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {V.~F.}\ \bibnamefont
  {Sears}},\ }\href {https://doi.org/10.1080/10448639208218770} {\bibfield
  {journal} {\bibinfo  {journal} {Neutron News}\ }\textbf {\bibinfo {volume}
  {3}},\ \bibinfo {pages} {26} (\bibinfo {year} {1992})},\ \Eprint
  {https://arxiv.org/abs/https://doi.org/10.1080/10448639208218770}
  {https://doi.org/10.1080/10448639208218770} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {Weber}\ \emph {et~al.}(2016)\citenamefont {Weber},
  ...
  and\ \bibinfo {author} {\bibfnamefont {P.}~\bibnamefont {Böni}},\ }\href
...
\bibitem [{\citenamefont {Weber}(2017)}]{Weber2017}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {T.}~\bibnamefont
  {Weber}},\ }\href {https://doi.org/10.1016/j.softx.2017.06.002} {\bibfield
  {journal} {\bibinfo  {journal} {SoftwareX}\ }\textbf {\bibinfo {volume}
  {6}},\ \bibinfo {pages} {148} (\bibinfo {year} {2017})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Weber}(2021)}]{Weber2021}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {T.}~\bibnamefont
  {Weber}},\ }\href {https://doi.org/10.1016/j.softx.2021.100667} {\bibfield
  {journal} {\bibinfo  {journal} {SoftwareX}\ }\textbf {\bibinfo {volume}
  {14}},\ \bibinfo {pages} {100667} (\bibinfo {year} {2021})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Brorsson}\ \emph {et~al.}(2022)\citenamefont
  ...
  {\bibfnamefont {P.}~\bibnamefont {Erhart}},\ }\href
...
\bibitem [{\citenamefont {Metsanurk}\ and\ \citenamefont
  ...
  {\bibfield  {journal} {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo
  {volume} {99}},\ \bibinfo {pages} {184304} (\bibinfo {year}
...
\bibitem [{\citenamefont {Fornasini}\ and\ \citenamefont
  ...
  {Grisenti}},\ }\href {https://doi.org/10.1107/S1600577515010759} {\bibfield
  {journal} {\bibinfo  {journal} {J. Synchrotron Radiat.}\ }\textbf {\bibinfo
  {volume} {22}},\ \bibinfo {pages} {1242} (\bibinfo {year}
...
\bibitem [{\citenamefont {Lebedev}\ and\ \citenamefont
  ...
  {Sluchinskaya}},\ }\href {https://doi.org/10.1134/S1063783413090229}
  {\bibfield  {journal} {\bibinfo  {journal} {Phys. Solid State}\ }\textbf
  {\bibinfo {volume} {55}},\ \bibinfo {pages} {1941} (\bibinfo {year}
...
\bibitem [{\citenamefont {Granhed}\ \emph {et~al.}(2020)\citenamefont
  ...
  {Hyldgaard}},\ }\href {https://doi.org/10.1103/PhysRevB.101.224105}
  {\bibfield  {journal} {\bibinfo  {journal} {Phys. Rev. B}\ }\textbf {\bibinfo
  {volume} {101}},\ \bibinfo {pages} {224105} (\bibinfo {year}
  {2020})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Heyd}\ \emph {et~al.}(2006)\citenamefont {Heyd},
  \citenamefont {Scuseria},\ and\ \citenamefont
  {Ernzerhof}}]{heydErratumHybridFunctionals2006}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {J.}~\bibnamefont
  {Heyd}}, \bibinfo {author} {\bibfnamefont {G.~E.}\ \bibnamefont {Scuseria}},\
  and\ \bibinfo {author} {\bibfnamefont {M.}~\bibnamefont {Ernzerhof}},\ }\href
  {https://doi.org/10.1063/1.2204597} {\bibfield  {journal} {\bibinfo
  {journal} {J. Chem. Phys.}\ }\textbf {\bibinfo {volume} {124}},\ \bibinfo
  {pages} {219906} (\bibinfo {year} {2006})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Jiao}\ \emph {et~al.}(2018)\citenamefont {Jiao},
...
\bibitem [{\citenamefont {Tadano}\ \emph {et~al.}(2014)\citenamefont {Tadano},
  ...
  {26}},\ \bibinfo {pages} {225402} (\bibinfo {year} {2014})}\BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Fransson}\ \emph {et~al.}(2022)\citenamefont
  ...
  from underdamped to overdamped dynamics in {{CsPbBr3}}}} (\bibinfo {year}
