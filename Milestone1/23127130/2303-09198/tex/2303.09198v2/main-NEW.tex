%\documentclass[aihp]{imsart}      % onecolumn (second format)
...
Many real-world networks were found to have degree distributions that can be approximated by a power-law distribution, where the fraction of vertices of degree $k$ scales as a power law with infinite variance~\cite{vazquez2002}. Therefore, random graph models that serve as benchmarks for these real-world networks often focus on networks with power-law degree distributions. These random graphs are constructed to have similar degree distributions as real-world networks, but the model does not prescribe other graph properties. The behavior of network properties of random graph models with a prescribed degree sequence has therefore been an object of intensive study~\cite{dhara2019,friedrich2015a,heydari2017,janssen2019,hofstad2017,yin2019a}.
...
\paragraph{Notation.} Let $f(n) \sim g(n)$ denote $f(n)/g(n)\rightarrow 1$. We say that $U$ is a regularly varying function with index $\rho$ when for every $\lambda>0$, $\lim_{x\to\infty}U(\lambda x)/U(x)=\lambda^\rho$. Furthermore, we say that a sequence of events $\{\mathcal{E}_n\}_{n\geq 1}$ happens with high probability when $\lim_{n\to\infty}\Prob{\mathcal{E}_n}=1.$ We denote the indicator function by $I(\cdot)$.
...
We denote  $\mu = \Exp{W_i}$. Then, every pair of vertices with weights $(h,h')$ is independently connected with probability $p(h,h')$. In this paper, we take
...
%\bz{als we voor 2 aparte secties gaan intro/results denk ik niet dat we in sectie 1 met subsecties hoeven te werken, maar ik ben er nog niet over uit, is ook kwestie van uitproberen} 
 ...
 \end{equation}
	The next result, proven in Appendix \ref{app:sec14appendix}, describes the growth rate of $m_n=\Exp{\triangle_n}$, extending previous work ~\cite{stegehuis2019b, hofstad2017d} on the pure power law case.
...
As we will see, an unusually large number of triangles will be caused by one hub. To determine how large a hub needs to be, we define for $a>0$,

\begin{equation}
\label{definition-can}
c_a(n) := \inf \Big\{ c: n^2 \frac 12 \int_0^\infty \int_0^\infty f_n(x,y,c) dF(x)dF(y) \geq m_n a \Big\}.
\end{equation}
Intuitively, $c_a(n)$ is the smallest size of a hub needed to create $a m_n$ additional triangles. The next lemma estimates its order of magnitude. Its proof is given in Appendix \ref{app:sec14appendix}. 
\begin{lemma}
\label{lemma-can}
...
By applying a concentration result of  Chatterjee~\cite{chatterjee2012missing}, we show in Section \ref{sec:prooftaularge} that, to establish the behavior of  $\Prob{\triangle_n > (1+a)m_n}$, it suffices to establish the behavior of $\Prob{G_n > (1+a)m_n}$, with 
\begin{equation}
\label{eq:def:gn}
...
\begin{align}
  ...
  %\nonumber\\
   %& \leq \limsup_{n\to\infty}\frac {\log  \Prob{\triangle_n>n^{3-3\alpha/2}(H+a)}}{n^{1-\alpha3/4}\log(n)} \leq 
...
\paragraph*{Behavior at the boundary $\alpha=4/3$: multiple hubs} The above two theorems show a stark contrast in the way additional triangles are generated: if $\alpha>4/3$ they consist of two regular nodes and one hub of the order of~\eqref{definition-can}, and if $\alpha<4/3$, they consist one regular node, and two hubs of the order of~\eqref{definition-can}. At the boundary $\alpha=4/3$ both may occur.
%, as is shown next. 
%
...
To avoid technical complications with slowly varying functions that can arise on the boundary (for example, $m_n/n$ could be oscillating between 0 and $\infty$), we assume that $\Prob{W>n} = (1+o(1))C n^{-\alpha}$, in which case Lemma \ref{lemma-mean} $m_n \sim C^3Hn$.
...
leading to $nl\mu/2$ additional triangles consisting of a single hub and two regular nodes. In addition, each of the $l(l-1)/2$ pairs of hubs form $n$ triangles with the regular nodes. Therefore, if 
we wish to exceed the number of triangles with a factor $a m_n$ we need $k(a)$  hubs where $k(a)$ is defined as
\begin{equation}
\label{eq-def-k(a)}
    k(a) := \inf \{l: l \mu/2 + l(l-1)/2 > a C^3 H \}.
\end{equation}
...
As shown in Lemma \ref{lem-lb-additionaltriangles} below, (see also Proposition \ref{prop-hublimit}), $nK_l(z_1,...,z_l)$ can be interpreted as the expected number of additional triangles (up to a term of $o(n)$) caused by $l$ hubs of size $z_i n, i=1,...,l$. 
...
\begin{equation}
\label{def:eta(a)}
    \eta(a) = \inf \{ \eta: K_{k(a)}(\eta, \infty, ...,\infty) \geq  C^3Ha\}.
\end{equation}
Note that 
$\eta(a)>0$ if $K_{k(a)-1}(\infty, ...,\infty)  = (k(a)-1) \mu + (k(a)-1)(k(a)-2)/2 <  a C^3 H$. 
\begin{theorem}
\label{thm-triangleboundary}
    Suppose that $P(W>x)\sim C x^{-4/3}$,
and suppose that  $(k(a)-1) \mu + (k(a)-1)(k(a)-2)/2 <  a C^3 H$.
     Then 
     \begin{equation}
...
Combining the upper bound $(3/2)\alpha-2$ for $\theta$ with the fact that $\Exp{\triangle_n}$ is regularly varying with index $n^{3-3\alpha/2}$ shows that the theorem applies until deviations of order $n$, as $n^{3-3\alpha/2}n^{(3/2)\alpha-2}=n$. Intuitively, this is because the inhomogeneous random graph has on average $\mu n$ edges. A single high-degree vertex can therefore only create $\mu n$ triangles. However, the maximum possible number of triangles in a graph on $n$ vertices scales as $n^3$. %Thus, larger deviations still call for a different result. 
...
Suppose that $\Prob{W\geq 1}=1$ and $\Prob{W>x} \sim C x^{-\alpha}$ for $x\geq 1$. For $\gamma\in(\max(1,3-3\alpha/2),3)$ and $\alpha>1$ and $a>0$,
\begin{equation}
...
\end{equation}
be the empirical distribution function associated with the weights $W_1,\ldots, W_n$ and observe that
\begin{equation}
\label{definition-gn}
...
The proof of Theorem \ref{thm-gn} consists of the following steps.
\begin{enumerate}
...
Set $F_n^U$ as the empirical distribution function of the $U_i$ variables and $\bar{F}_n$ as their complementary comulative distribution function. Define 
...
\leq 1-\Prob{E_n(A,c,\delta)}
\end{equation}
as $\bar F_n(x)$ is non-increasing.
Finally, we investigate the range $[b(n),\infty)$. Let $c>0$ and note that
\[
\Prob{ \sup_{x\in  [b(n), \infty)} \bar F_n(x)> c/n} = \Prob{\bar F_n(b(n))> c/n} = \Prob{ \sum_{i=1}^n I(U_i <\bar F(b(n))) > c}.
\]
We can use Lemma 2.3 from \cite{stegehuis2022scale} to upper bound the right-hand side and get 
\begin{equation}
\label{cbtail3}
\Prob{ \sup_{x\in  [b(n), \infty)} \bar F_n(x)> c/n} \leq % e^{n \bar F(b(n))} 
(n\bar F(b(n)))^{\lceil c \rceil}.
\end{equation}
...
If $\alpha>1$ and $\delta>0$ is such that $1/\alpha +\delta<1$, the upper bound in (\ref{cbtail3}) is regularly varying with index $-\lceil c \rceil \delta \alpha$, which can be made to go to $0$ at any desired polynomial rate by picking $c$ appropriately large. We summarize our findings in the following proposition. 
...
Let $\beta>0$. Then 
\begin{equation}
    1-  \Prob{E_n(A,c,\delta)} \leq 2 e^{- n \bar F(a(n)) h(A+1)}  + (n\bar F(b(n)))^{\lceil c \rceil},
\end{equation}
which is $o(n^{-\beta})$ if $\lceil c \rceil > \beta / \alpha \delta$ where $E_n(A,c,\delta)$ is as in~\eqref{eq:En}.
\end{proposition}
...
\end{equation} 
or in other words: the probability that a jump of at most $\varepsilon n$ creates the desired deviation is small.
...
\end{align}
%Informally, the random variable with distribution function $F_n$ truncated at $\varepsilon \cs{n}$ is stochastically bounded by a random variable 
...
\begin{equation}
\int_0^{\varepsilon n} x d F_n(x) \leq (1+A) \int_0^{\infty} x d F(x) + (1+A)  b(n) \bar F(a(n)) + (c/n) (\varepsilon n).
\end{equation}
...
The desired result (\ref{rs99}) now follows by choosing $\varepsilon$ small enough so that $c\varepsilon < a$. %\cs{We do not actually use this result, right? We actually only need (2.20), and I believe that this one is still ok, as it is basically a triple integral from 0 to infinity, so just an expectation of a non-decreasing function.}
...
\end{equation}
where $c_a(n)$ is as in~\eqref{definition-can}.
\end{proposition}


\begin{proof}
On the set $E_n(A,c,d)\cap \{L_n(\varepsilon c_a(n))=0\}$, the following upper bound holds for $\bar F_n$ for all $x\geq 0$
%on $x\in [0,\varepsilon c_a(n)]$: 
\begin{equation}
\bar F_n(x) \leq \bar F_n^*(x),
\end{equation}
with $\bar F_n^*(x)$ defined by
\begin{equation}
\label{eq-deffnstar}
(1+A) \bar F(x) I(x<a(n)) + (1+A) \bar F(a(n)) I(a(n)\leq x<b(n)) + \frac{c}{n} I(x \in [b(n), \varepsilon c_a(n)]).
\end{equation}
%\bz{I removed the stochastic dominance story, it was only meant as a way to provide intuition and it is confusing as the mass of the upper bound is $1+A$}
%Again, on the set \cs{$E_n(A,c,d)$}, the random variable with distribution function $F_n$ truncated at $\varepsilon \cs{c_a(n)}$ is stochastically bounded by a random variable 
%with \cs{complementary} distribution $(1+A) \bar F(x)$ on $[0,a(n))$,  an atom of size $\cs{(1+A)}\bar F(a(n))-c/n$ at  $b(n)$, and  an atom of size $c/n$ at $\varepsilon c_a(n)$. 
We have that $b(n)/c_a(n)\rightarrow 0$ [for $\delta$ small enough, to be specified later].
Since $f_n$ is nondecreasing in each coordinate for fixed $n$, we can use the property $\bar F_n(x) \leq \bar F_n^*(x)$ to conclude that, on the set $E_n(A, \delta,  c) \cap \{ L_n(\varepsilon c_a(n))=0\}$, 
\begin{equation}
\label{eq:Gnub}
  G_n \leq  n^3 \int_0^\infty \int_x^\infty\int_y^\infty   f_n(x,y,z) dF_n^*(z)dF_n^*(y)dF_n^*(x).
\end{equation}
%\textcolor{red}{a direct analytic proof seems preferable, for example by writing $f_n$ as a triple integral w.r.t. a function $h_n$ [can be defined in terms of $h_1$ as is done now in appendix], and then switch order of integration. The fact that the RHS is integrating up to infinity will help I think.}\cs{Can we not use stochastic domination here, as we basically integrate from 0 to infinity (with a factor 1/6?)}

The next step is to evaluate the integral on the right-hand side of (\ref{eq:Gnub}), where we need to keep track of the value of $x,y,z$ in each of the $3$ terms in $F_n^*$: they may be of (s)mall ($<a(n)$), (m)edium ($b(n)$) or (l)arge ($\varepsilon c_a(n)$) value. 
...
\begin{align}
\frac{G_n}{(1+A)^3}&\leq  m_n +  n^3\bar F(a(n))\int_0^\infty\int_x^\infty  f_n(x,y,b(n)) dF(y)dF(x) \nonumber \\ 
...
&+n^3 (\bar F(a(n))) (c/n)^2f_n(\varepsilon c(n),\varepsilon c(n),b(n))+ n^3(c/n)^3f_n(\varepsilon c_a(n),\varepsilon c_a(n),\varepsilon c_a(n)).
\label{eq-tenterms}
\end{align}
Apart from the main term $m_n$, we need to bound 9 terms in total.
In what follows, we often use that $a_n$ is regularly varying of index $1/\alpha -\delta$, that $\bar F(a_n)$ is regularly varying of index $\alpha \delta - 1$,  that $b(n)$ is regularly varying of index $1/\alpha+\delta$, and that $c(n)=c_a(n)$ is regularly varying of index $\alpha_c = \frac 14 \frac{\alpha}{\alpha-1}$. 
...
\begin{equation}
n^3 \frac{c}{n}\int_0^\infty\int_x^\infty f_n(x,y,\varepsilon c_a(n)) dF(x)dF(y)  \sim c \varepsilon^{(\alpha-1)2} am_n,
\end{equation}
for every $\varepsilon>0$.\\

\noindent
{\bf Term 4:}
invoking Lemma \ref{lemma-sbc} with both sequences equal to $b(n)$, and $\alpha_b= \alpha_c=1/\alpha+\delta$ gives that \\
...
\noindent
{\bf Term 6:}
invoking Lemma \ref{lemma-sbc} with sequences $b(n)$ and $\varepsilon c_a(n)$, such that $\alpha_b=1/\alpha+\delta$ and $\alpha_c= \frac 14 \frac{\alpha}{\alpha-1}$, it follows that this term behaves like
...
\begin{equation}
    \Prob{G_n > (1+a)m_n ; L_n(\varepsilon c_a(n))\geq 2}\leq  \Prob{L_n(\varepsilon c_a(n))\geq 2} = o( n \Prob{W> c_a(n)}). 
\end{equation}
...
\Prob{G_n> (1+a) m_n, W_n > \varepsilon c_a(n) > W_i, i<n} = (1+o(1))  \Prob{W> c_a(n)}. 
\end{equation}
...
\begin{equation}
     \Prob{G_n> (1+a) m_n \mid D_n} \sim \Prob{W_n > c_a(n) \mid D_n},
\end{equation}
...
The idea of the proof is we represent our random graph by a random graph with $n-1$ vertices and truncated weights, and then add a single vertex with weight $yc_a(n)$.  
We first prove (\ref{eq-ub}) for $y>1$.
For $W_i^z=W_iI(W_i<z)$, define the truncated mean $m_{n-1}(z) = \Exp{G_{n-1}(W_1^z,...,W_{n-1}^z)}$, and observe that (e.g.\ by inspecting the proof of Lemma \ref{lemma-mean}), for $a(n) = \bar F^{-1}(n) n^{-\delta}$,
\begin{equation}
\label{eq-truncatedmean}
m_{n-1} (a(n)) = (1+o(1))m_n
\end{equation}
if $\delta>0$ is small enough. 
On the event $D_n$ and when $W_n=yc_a(n)$ we can write, since $a(n) \geq \varepsilon c_a(n)$ for $n$ large enough, and $f_n(x,y,z)$ is non-decreasing in each coordinate for fixed $n$,
\begin{align}
    G_{n}(W_1^{a(n)}, ..., W_{n-1}^{a(n)}, y c_n(a)) &\geq  n^3\int_0^{a(n)} \int_u^{a(n)}\int_v^{a(n)} f_n(u,v,w) dF_{n-1}(w)dF_{n-1}(v)dF_{n-1}(u) \nonumber \\
    &+ n^2\int_0^{a(n)} \int_u^{a(n)}f_n(u,v,yc_a(n)) dF_{n-1}(v)dF_{n-1}(u)
    \label{Gnlowerbound}
%    \nonumber 
    %\\
    %&\geq (1-A)^3 m_{n-1}(a(n)) + (1-A)^2 m_n a y^{3-\alpha 3/2} (1+o(1)),
\end{align}
To bound the first term on the right-hand side of this expression, 
 observe that, for any constant $A>0$, with high probability, (\ref{cbtail1}) implies that $\bar F_{n-1}(x) \geq (1-A) \bar F(x)$ on $x\in [0,a(n)]$.
 Now, observe that, for any constant $A>0$, with high probability, (\ref{cbtail1}) implies that $\bar F_{n-1}(x) \geq (1-A) \bar F(x)$ on $x\in [0,a(n)]$.  Therefore, we see that, with high probability on the set $D_n$, given $W_n=yc_a(n)$,
\begin{align*}
   n^3\int_0^{a(n)} \int_u^{a(n)}\int_v^{a(n)} f_n(u,v,w) dF_{n-1}(w)dF_{n-1}(v)dF_{n-1}(u) &\geq  (1-A)^3 m_{n-1}(a(n))\\
  &\sim (1-A)^3 m_{n}.
   \end{align*}
To bound the second term in (\ref{Gnlowerbound}), we use again the fact
that $\bar F_{n-1}(x) \geq (1-A) \bar F(x)$ on $x\in [0,a(n)]$ with high probability, to obtain 
\begin{align*}
   & n^2\int_0^{a(n)} \int_u^{a(n)}f_n(u,v,yc_a(n)) dF_{n-1}(v)dF_{n-1}(u)\\
&\geq (1-A)^2n^2\int_0^{a(n)} \int_u^{a(n)}f_n(u,v,yc_a(n)) dF(v)dF(u)\\
&\sim (1-A)^2 n^2 K_2 \frac{2}{(yc_a(n))^2} \bar F\left(\frac{n}{yc_a(n)}\right)^2\\
&\sim (1-A)^2 K_2 n^3 y^{-2}y^{2\alpha} c_a(n)^{2\alpha}\\
&\sim (1-A)^2 K_2 y^{2\alpha-2} am_n.
\end{align*}
In the last 4 steps, 
 we applied Lemma \ref{lemma-sb} with $y c_a(n)$, the defining property of $c_a(n)$, and regular variation.  Since $y>1$, there exists an $A>0$ such that the RHS of (\ref{Gnlowerbound}) is larger
...
\begin{align}
    G_{n}(W_1^{\varepsilon c_a(a)}, ..., W_n^{\varepsilon c_a(a)}, y c_n(a)) &\leq \mbox{ RHS of (\ref{eq-tenterms}}) \nonumber\\
...
The second term in the right-hand side of the last display can be analyzed in a way similar to (\ref{eq-tenterms}). In particular, we can upper bound it with 
\begin{align*}
     & O(n^{2\alpha \delta}) + n^2 (1+A)^2S_{yc_a(n)}(n)+ n^2(1+A) \frac cn S_{\varepsilon c_a(n), yc_a(n)} \nonumber\\
     & \quad + n^2(1+A) \bar F(a(n)) S_{b(n), yc_a(n)}(n),
\end{align*}
with $S_{b,c}(n)$ and $S_b(n)$ as in~\eqref{eq:Sbc} and~\eqref{eq:Sbn}.
...
\label{thm-gnboundary}
    Suppose that $\Prob{W>x}\sim C x^{-4/3}$ and suppose that  $(k(a)-1) \mu + (k(a)-1)(k(a)-2)/2 <  a C^3 H$.
     Then 
    \begin{equation}
    \label{eq-asymptoticsboundary2}
...
The limit equals $(z_i/\mu)\Exp{W^2I(W\leq \mu/z_i)}+\Exp{WI(W>\mu/z_i)}$. This leads to the desired upper bound. 

A lower bound follows by bounding $\min \{xy/\mu, n\}$
...
\begin{lemma}
    ...
    Let $\triangle_n(\delta,a)$ be the number of triangles $\{i,j,n\}$ in a graph with $n$ vertices where node $i,j<n$ have weight $W_i I(W_i\leq n^{1/2+\delta})$ and node $n$ weight $c_a(n)$.
    %    conditionally on containing} node $n$ with $W_n=c_a(n)$, and only %containing other nodes $i$ such that $W_i \leq n^{1/2+\delta}$ for %$i<n$. \cs{As the referee mentioned, this should maybe be %conditionally on $W_n=c_a(n)$?}
...
% \cs{Deze begrijp ik niet helemaal. Moet het $i>l$ zijn misschien?} \bz{klopt, ik pas het aan} 
 Let $\triangle_{n,l}(\delta,z_1,...,z_l)$ be the number of triangles 
 in a graph with $n$ vertices where node $i\leq n-l$ has weight $W_i I(W_i\leq n^{1/2+\delta})$ and node $n-i+1, i=1,...,l$ has weight $nz_i.$
...
\label{eq-additionaltrianglesmultiplehubs}
    \triangle_{n,l}(\delta,z_1,...,z_l)/n \rightarrow C^3H+ K_l(z_1,...,z_l),
\end{equation}
with $K_l, H$ as in~\eqref{eq-def-k(a)},~\eqref{eq:h} and $C$ as in Theorem~\ref{thm:smalldevs}.
\end{lemma}


\begin{proof}[Proof of Theorem \ref{thm-triangle-singlebigjump} and Theorem \ref{thm-triangleboundary}.]
The proofs of both theorems are similar. We first prove an asymptotic upper bound. Write for $0<\zeta<1$,
\begin{align*}
...
\end{align*}
By Lemma \ref{lem-lb-additionaltriangles}, $\Prob{\triangle_{n}(\delta,a+\eta) > (1+a)m_n}\rightarrow 1$ for sufficiently small $\delta$,
...
Next, consider  $\alpha=4/3$. Write, for $\zeta>0$ and sufficiently large $n$
\begin{align*}
    \Prob{\triangle_{n} > (1+a)m_n} &\geq n^{k(a)} \Prob{K_{k(a)}(W_1/n,...,W_{k(a)}/n) > C^3H(a+\zeta)}\\
    & \hspace{1cm} \cdot \Prob{\triangle_{n} > (1+a)m_n \mid K_{k(a)}(W_1/n,...,W_{k(a)}/n) > C^3H(a+\zeta)}.
\end{align*}
%Define $Z_k=\{z=(z_1,...,z_k): C_k(z) \geq (a+\eta)C^3H\}$.
By conditioning on $(W_1/n,...,W_{k(a)}/n)$ and applying the second part of Lemma \ref{lem-lb-additionaltriangles}, we see that (using 
 $m_n \sim C^3Hn$)
$\Prob{\triangle_{n} > (1+a)m_n \mid K_{k(a)}(W_1/n,...,W_{k(a)}/n) > C^3H(a+\zeta) }\rightarrow 1$. 
Since, for sufficiently small $\zeta>0$, $k(a)=k(a+\zeta)$,  we have that 
\begin{align}& \Prob{K_{k(a)}(W_1/n,...,W_{k(a)}/n) > C^3H(a+\zeta)} \nonumber\\& \sim 
 \prob( K_{k(a)}(X_1^{\eta(a+\zeta)}, \ldots, X_{k(a)}^{\eta(a+\zeta)}) \geq C^3H(a+\zeta)) (\Prob{W> \eta(a+\zeta) n})^{k(a)}.\end{align}
% \cs{Bovenstaande zie ik zelf ook niet helemaal}\bz{ik heb het nu verbeterd,de $m_n$ moest $C^3H$ zijn}
...
\begin{align}
      ...
      \frac{\prob( K_{k(a)}(X_1^{\eta(a+\zeta)}, \ldots, X_{k(a)}^{\eta(a+\zeta)}) \geq C^3H(a+\zeta)) (\Prob{W> \eta(a+\zeta) n})^{k(a)}}{\prob( K_{k(a)}(X_1^{\eta(a)}, \ldots, X_{k(a)}^{\eta(a)}) \geq C^3Ha) (\Prob{W> \eta (a) n})^{k(a)}}.
\end{align}
...
% with probability at least $1-\exp(-\delta_2)$, all edges between vertices of weights $W_iW_j<\mu n$ are in at most $Kn^{2-\alpha}$ triangles.
 ...
 \end{equation}
 for some $c_1:=c_1(\alpha,\delta)>0$ and $n$ sufficiently large. 
\end{lemma}
...
Define the event $E_w = \{W_1=w_1,...,W_n=w_n\}$ and set $g_n= \sum_{i<j<k} f_n(w_i,w_j,w_k)$ as the expected number of triangles conditional on specific values of the weights. Let $\triangle_n(w)$ be the conditional probability of the number of triangles conditionally on $E_w$.
...
We set $X_{ijk}=X'_{ijk}$ as the indicator that a triangle is present between vertices $i,j,k$. Furthermore, we set $X_{ijk(uvw)}=X_{ijk}$ when $|\{i,j,k,u,v,w\}|\geq 5$. When $|\{i,j,k,u,v,w\}|= 4$, we set $X_{ijk(uvw)}=X_{ijk}$ when the overlap of $ijk$ and $uvw$ occurs at an edge with $w_uw_v>\mu n$ (where we assumed w.l.o.g. that the overlap occurs at $u,v$), and we set $X_{ijk(uvw)}= 0$ otherwise or when $\{i,j,k\}=\{u,v,w\}$. Then, $\sum_{i,j,k}X_{ijk(uvw)}$ and $X_{uvw}$ are independent. Indeed, when two triangles do not overlap at an edge, their presence is independent conditionally on the weights, as the edge indicators are independent conditionally on the weights. When the edge overlap occurs at an edge that is present with probability one, the presence of the two triangles is still independent conditionally on the weights. In all other cases, $X_{ijk(uvw)}=0$, which is also independent of $X_{uvw}$. 


On the event $\mathcal{E}'$,
\begin{equation}
	\triangle_n =\frac{1}{6}\sum_{i,j,k}X_{ijk}\leq \sum_{ijk}X_{ijk(uvw)}+3n^{2-\alpha}.
\end{equation}
...
\begin{align}\label{eq:exptrianglb}
    G_n& \geq n^3 \int_{a\sqrt{\mu n}}^{\sqrt{\mu n}}\int_{a\sqrt{\mu n}}^{\sqrt{\mu n}}\int_{a\sqrt{\mu n}}^{\sqrt{\mu n}}f_n(u,v,w) dF_n(u)dF_n(v)dF_n(w)\nonumber\\
    & = \mu^{-3}\Big(\int_{a\sqrt{\mu n}}^{\sqrt{\mu n}}w^2dF_n(w)\Big)^3 \geq  \varepsilon_2 n^{3-3\alpha/2}L(\sqrt{n})^3
\end{align}
...
%
%By ~\eqref{eq:exptrianglb} $G_n\geq \varepsilon_ 2n^{3-3\alpha/2}L(\sqrt{n})^3$ on the event that the weight sequence satisfies $\mathcal{E}_1$. In that %case, and on the event $\mathcal{E}'$, we can use Lemma~\ref{lem:chatterjeelem} with $a=3n^{2-\alpha}$,
%\begin{align}
...
Note first that
\begin{align*}
    \Exp{ \triangle_n (\delta,a)} &= (n-1)^3\int_{0<x<y<z<n^{1/2+\delta}} f_{n}(x,y,z) dF(x)dF(y)dF(z)\\
    & \hspace{1cm}+(n-1)^2\int_{0<x<y<n^{1/2+\delta}} f_{n}(x,y,c_a(n)) dF(x)dF(y).
\end{align*}
...
\end{align*}
we see that $\triangle_n (\delta,a) < (1+\zeta)^3(1+a)m_n$ with high probability on the event $E_n$, while outside of this event $\triangle_n (\delta,a)\leq n^3$, and therefore
\begin{equation}
...
To prove Theorems~\ref{thm:smalldevs} and~\ref{thm:triangldptausmall}, we distinguish several types of vertices, based on the vertex weights. Specifically, fix $\zeta>2$ and $\varepsilon>0$ and let $\gamma$ as in Theorem~\ref{thm:triangldptausmall}, then
\begin{itemize}
...
%For the proof of Theorem~\ref{thm:triangldptausmall}, we will always set $D=1$ and $\gamma$ as the $\gamma$ of Theorem~\ref{thm:triangldptausmall}, while for Theorem~\ref{thm:smalldevs}, we will often choose $D=\sqrt{2 a}$, and we set $\gamma = 3-3\alpha/2$. 
...
\end{lemma}
We next provide an elementary lemma that bounds the probability that polynomially many vertices have at least a given weight:
...
& \leq \exp\Big(-un^{\gamma} \log(n^{\gamma-1+\alpha\beta})\Big)(1+o(1)).
 \end{align}
...
Suppose that $\Prob{W>x} \sim C x^{-\alpha}$. Let $B_3$ be the set of vertices with weights at most 
 ...
 \begin{align}\label{eq:triangb3b3b3}
 	...
 	%&
 	%\leq \max\Bigg(\exp\Big(-\frac{K}{3M}\log\Big(\frac{K}{24n^{3-3\alpha/2}}\Big)\Big),\exp(-Dn^{(\gamma-1)/2}\log(n^{(\gamma-3)/2+\alpha}))\Bigg)\nonumber\\
 	& \leq n\exp\Big(-\sqrt{2a}n^{(\gamma-1)/2}\log(n^{(\gamma-3)/2+\alpha}))\Big)
 \end{align}
when $Kn^\gamma >(1+\varepsilon)^3C^3Hn^{3-3\alpha/2}$. %, where $Hn^{3-3\alpha/2}$ denotes the average number of triangles. 
%and where  
%\begin{equation}
...
% \textcolor{red}{Rough idea:
% \begin{align*}
% 	\Prob{\triangle_{B_3,B_3,B_3}\geq K } &= \int_0^{Q_n} \int_x^{Q_n} \int_y^{Q_n} 
%  \Prob{\triangle_{B_3,B_3,B_3} (w_1,....,w_n) \geq K} dF_n(x)dF_n(y)dF_n(z)\\
% &\leq (1+\varepsilon)^3  \int_0^{Q_n} \int_x^{Q_n} \int_y^{Q_n} 
%  \Prob{\triangle_{B_3,B_3,B_3} (w_1,....,w_n) \geq K} dF(x)dF(y)dF(z).
% \end{align*}
% To justify last inequality we need another lemma to address another main concern of the referee, see my comment in the list of remaining issues.\\
% UPDATE, I realize this is not enough. To be continued. But I think we need to use an explicit representation for $\triangle_{B_3,B_3,B_3}$ in terms 
% of the uniform random variables and the weights.
% }
% \cs{This does not make much sense to me. The probability that there are at least $K$ triangles does not depend on 3 vertices. Can we not just apply stochastic domination on the probability conditionally on being at most $Q_n$? As in fact, these triangles do not depend at all on the higher mass than $Q_n$, in contrast to the mean that we had before. }
...
Given any weight distribution satisfying the event $\mathcal{E}_1$, the edge count is a sum of independent Bernoulli random variables with mean at most $(1+\varepsilon)\mu n$. Thus, on $\mathcal{E}_1$, Lemma~\ref{lem-concentration} yields that for $K>(1+\varepsilon)^2\mu n$,
%\cs{Volgens mij was hier stochastische dominantie niet nodig, omdat wat hieronder staat voor elke $F$ geldt in de set waar ik toch al op conditioneerde.}
\begin{align}
		\Prob{E_{B_3,B_3}\geq K \mid \mathcal{E}_1 }& \leq \Prob{E_{B_3,B_3}\geq K \mid \mathcal{E}_1} \nonumber\\
...
\begin{align}
	\Prob{\triangle_{B_3,B_3,B_3}\geq Kn^\gamma}& \leq \exp\Big(-\frac{Kn^\gamma}{3n}\log\Big(\frac{Kn^\gamma}{(1+\varepsilon)^3H n^{3-3\alpha/2}}-1+\frac{(1+\varepsilon)^3H n^{3-3\alpha/2}}{Kn^\gamma}\Big)\Big)\nonumber\\
 &   \leq \exp(-Dh(1+\varepsilon)n^{(\gamma-1)/2}\log(n^{(\gamma-3)/2+\alpha}))
\end{align}
for $\gamma\geq 1$ and $n$ sufficiently large, as on $\mathcal{E}_1$, the mean number of triangles is bounded by $(1+\varepsilon)^3Hn^{3-3\alpha/2}(1+o(1))$. Plugging in the value of $D$ then proves the lemma.
\end{proof}
...
\begin{lemma}\label{lem:b2edgetriang}
	...
	\begin{equation}
		\Prob{E_{A,B_2}>n^{(\gamma+1)/2}\mid N(B_2)}\leq \exp\Bigg(-\frac{n^{(\gamma+1)/2}}{M} \log\Big(\frac{n^{(\gamma-1)/2}}{K_1 N(B_2)\log(n)^{-\zeta}}\Big)\Bigg),
	\end{equation}
for some $K_1>0$,
where $N({B_2})$ denotes the number of $B_2$ vertices. 
Furthermore,
\begin{equation}
	\Prob{ \triangle_{A,B_2,B_2}> n^\gamma\mid N(B_2)} \leq \exp\Big(-\frac{n^\gamma}{M^2}\log\Big(\frac{n^{\gamma-1}}{3K_1 N(B_2)^2\log(n)^{-\zeta}}\Big)\Big).
\end{equation}
\end{lemma}
...
%By~\cite{stegehuis2019b}, $\Exp{\triangle_n}=Hn^{3-3\alpha/2}(1+o(1))$.
...
\end{align}
Now, $\sqrt{2 a}n^{1-\alpha3/4}$ vertices of weight at least $\mu n$ generate $n(\sqrt{2 a}n^{1-\alpha3/4})^2/2=a n^{3-3\alpha/2}$ triangles with probability one. Indeed, every pair of 2 vertices of weight at least $\mu n$ forms a triangle with any of the other $n$ vertices, creating $a n^{3-3\alpha/2}$ triangles. Furthermore, the vertices of weight at most $n^{1/\alpha}$ generate $C^3Hn^{3-3\alpha/2}(1+o(1))$ 
%\cs{ refer to part computed by Bert instead, as that one has asymptotic power-law pdf instead of cdf} 
triangles with high probability~\cite{hofstad2017b}. As these triangles use different edges, their presence is independent from the presence of the triangles with the $\mu n$ weight vertices.
...
\begin{equation}\label{eq:b3deltasmall}
    \Prob{\triangle_{B_3B_3B_3}>(C^3H+a)n^{3-3\alpha/2}}\leq \exp\Big(-{\sqrt{2 a}n^{1-\alpha3/4}}\log\big(n^{\alpha/4}\big)\Big).
\end{equation}
%Note that $B_3,B_3,B_3$ triangles in this case also include AAA and AA$B_3$ triangles. 


\textit{$B_3B_3B_i$ triangles for $i \in \{1,2\}$. }
We bound the number of $B_3B_3B_i$ triangles for $i=1,2$ by the number of $B_i$ vertices times the number of edges between $B_3$ vertices. 
By~\eqref{eq:Nb1b2} with $\gamma=3-3\alpha/2$ and $D =\sqrt{2 a}/h(1+\varepsilon)$,
...
\end{equation}
Then, by~\eqref{eq:Nb1b2A},
\begin{align}\label{eq:triangab2b3deltasmall}
    \Prob{\triangle_{B_3B_3B_i}>\varepsilon n^{3-3\alpha/2}\mid E_{B_3,B_3}< n^{1-3\alpha/8}}& \leq \Prob{N(B_1\cup B_2)>\varepsilon n^{2-9\alpha/8}}\nonumber\\
...
\begin{align}\label{eq:triangab1b1deltasmall}
    \Prob{\triangle_{B_iB_1B_1}>an^{3-3\alpha/2}}& \leq \Prob{N(B_1)>\sqrt{2 a}n^{1-\alpha3/4}}\nonumber\\
    %& \leq \exp\Big(-n^{(\gamma-1)/2} \log(n^{(\gamma-3)/2+\alpha})-n(1-n^{(\gamma-1)/2-1})\log\Big(\frac{1-n^{(\gamma-1)/2-1}}{1-n^{-\alpha/2}}\Big)\Big)\nonumber\\
...
Thus, 
\begin{equation}
    \limsup_{n\to\infty}\frac{\log\Big(\Prob{\triangle_{B_iB_1B_1}>an^{3-3\alpha/2}}\Big)}{n^{1-\alpha3/4}\log(n)}\leq -\sqrt{2a}(1-\frac{3\alpha}{4})+(1-\alpha) = \sqrt{2a}\frac{-\alpha}{4}.
\end{equation}
...
\begin{equation}\label{eq:triangbbbdeltasmall}
    \Prob{\triangle_{B_iB_jB_k}>\varepsilon n^{3-3\alpha/2}}\leq \exp\Big(-{\sqrt{2 a}n^{1-\alpha3/4}}\log\big(n^{\alpha/4}\big)\Big)
\end{equation}
for $i,j,k\in\{1,2\}$.
...
%\end{equation}
%Let $A_{BBB}n^{3-3\alpha/2}$ denote the average number of triples of vertices with weights at least $\sqrt{\mu n}$. 
...
%& \leq \exp\Big(-n^{\gamma/3} \log(n^{\gamma/3-(2-\alpha)/2})-n(1-n^{\lambda-1})\log\Big(\frac{1-n^{\lambda-1}}{1-n^{-\alpha/2}}\Big)\Big)\nonumber\\
   % & \leq \exp\Big(-(A_{BBB}+\varepsilon)^{1/3}n^{(2-\alpha)/2} \log\Big(\frac{(A_{BBB}+\varepsilon)^{1/3}n^{(2-\alpha)/2}}{A_{BBB}^{1/3}n^{(2-\alpha)/2}}\Big)\Big)\nonumber\\
    %& = \exp\Big(-(A_{BBB}+\varepsilon)^{1/3}n^{(2-\alpha)/2} \log\Big((1+\varepsilon/A_{BBB})^{1/3}\Big)\Big).
\end{align}
where $N(\geq \sqrt{\mu n})$ denotes the number of vertices with weight at least $\sqrt{\mu n}$.
Now by~\eqref{eq:Nb1b2A} 
\begin{align}\label{eq:Nb2detailed}
	\Prob{\mathcal{F}}:=\Prob{N(B_2\cup B_1)>n^{1-\alpha3/4}\log(n)^{\zeta}}
%	& \leq \exp\Big(-n^{(\gamma-1)/2}\log(n)^{\zeta}\log\Big(n^{(\gamma-1)/2}(\log(n)^{\zeta}-\log(n^{(\gamma-3)/2+\alpha}))\Big)\Big)\nonumber\\
	& \leq \exp\Big(-\sqrt{2 a} n^{1-\alpha3/4} \log(n^{\alpha/4})\Big),
\end{align}
when $\zeta>1$ and $n$ is sufficiently large. 
...
\begin{align}
&\Prob{\triangle_{B_3'B_iB_j}>\varepsilon n^{(2-\alpha)3/2}\mid \bar{\mathcal{F}}} 
%\leq     \Prob{N(B_2\cup B_1)N(\geq \sqrt{\mu n})^2 n^{(2-\alpha)/2}  }\nonumber\\
%& \leq \exp\Big(-n^{\gamma/3} \log(n^{\gamma/3-(2-\alpha)/2})-n(1-n^{\lambda-1})\log\Big(\frac{1-n^{\lambda-1}}{1-n^{-\alpha/2}}\Big)\Big)\nonumber\\
 \leq \Prob{N(B)^2 >\varepsilon n^{1-3\alpha/8}\log(n)^{-\zeta}\mid \bar{\mathcal{F}}}\nonumber\\
...
\begin{equation}\label{eq:triangbbb3deltasmall}
    \Prob{\triangle_{B_3'B_iB_j}>\varepsilon n^{3-3\alpha/2}}\leq 2\exp\Big(-{\sqrt{2 a}n^{1-\alpha3/4}}\log\big(n^{\alpha/4}\big)\Big).
\end{equation}
...
% \begin{align}
%     \Prob{\sum_{u,v,w}Y_{uvw}>(A_{AAA}+\delta)n^{3-3\alpha/2}} & \leq \exp(-\frac{(A_{AAA}+\delta)n^{3-3\alpha/2}}{K^2n}\log((A_{AAA}+\delta)n^{3-3\alpha/2}/(3A_{AAA}n^{3-3\alpha/2})))\nonumber\\
...
\begin{align}\label{eq:b2deltasmall}
    \Prob{ \triangle_{AB_2B_2}> \varepsilon n^{3-3\alpha/2}}& \leq \exp\Big(-\varepsilon \frac{n^{3-3\alpha/2}}{K^2n}\log\Big(\frac{\varepsilon n^{3-3\alpha/2}}{3K_1 N(B_2)^2n\log(n)^{-\zeta}}\Big)\Big)\nonumber\\
...
\begin{align}\label{eq:triangab2b1deltasmall}
    \Prob{\triangle_{AB_1B_2}>\varepsilon n^{3-3\alpha/2}}& \leq \Prob{N(B_1)E_{AB_2}>\varepsilon n^{3-3\alpha/2}}\nonumber\\
...
%     \Prob{N(\triangle_{AAB_i})>\varepsilon n^{3-3\alpha/2}} & \leq \Prob{N(B_2'\cup B_1')>\varepsilon n^{2-3/2\alpha}/K}\nonumber\\
...
\begin{align}
    & \Prob{\sum_{\{i,j,k\}\neq \{3,3,3\}, \{i,1,1\}}\triangle_{B_iB_jB_k}>\varepsilon n^{3-3\alpha/2}} %\nonumber\\
    %& 
...
\begin{align}
\Prob{\triangle_n>(H+a) n^{3-3\alpha/2}}& \leq \Prob{\triangle_{B_3B_3B_3}+\triangle_{B_1B_1B_i}>(C^3H+a-\varepsilon) n^{3-3\alpha/2}}\nonumber\\
& \quad +K \exp\Big(-\sqrt{2 a} n^{1-\alpha3/4} \log(n^{\alpha/4})\Big)\nonumber\\
    %& \Prob{N(\triangle_{B_iB_jB_k})>(H+a) n^{3-3\alpha/2}} \nonumber\\
    & \leq (K+1) \exp\Big(-\sqrt{2 a} n^{1-\alpha3/4} \log(n^{\alpha/4})\Big) \nonumber\\
    & \quad + \Prob{\triangle_{B_iB_1B_1}>(C^3H+a-2\varepsilon) n^{3-3\alpha/2}}.
\end{align}
Thus,
\begin{equation}\label{eq:triangabbfinaldeltasmall}
     \limsup_{n\to\infty}\frac{\log\Big(\Prob{\triangle_n>(C^3H+a) n^{3-3\alpha/2}}\Big)}{n^{1-\alpha3/4}\log(n)}\leq \sqrt{2a-2\varepsilon}\Big(\frac{-\alpha}{4}\Big).
\end{equation}
%To conclude,~\eqref{eq:triangbbbdeltasmall},~\eqref{eq:triangabbfinaldeltasmall} and~\eqref{eq:triangaabdeltasmall} yield that
% \begin{equation}
%     \log\big(\Prob{\triangle_n>(\delta +A)n^{3-3\alpha/2}}\big)\leq \sqrt{\delta-24-6\varepsilon} n^{1-\alpha3/4} \log(n^{\alpha/4})(1+o(1)).
% \end{equation}
...
%\begin{funding}
\paragraph{Acknowledgements.} C. Stegehuis was supported by NWO VENI grant 202.001 and NWO M2 grant 0.379.
...
Let $$h(u,v,w)= \begin{cases}
    \frac{8uvw}{\mu^3} & uv, vw, uw<\mu \\
    \frac{2v}{\mu^2} & uv, vw<\mu , uw\geq \mu \\
    \frac{2u}{\mu2} & uv, uw<\mu , vw\geq \mu \\
    \frac{2w}{\mu^2} & uw, vw<\mu , uv\geq \mu \\
    0 & \text{else}.
\end{cases}$$
%Note that $f_1$ is absolutely continous in each variable, so there exists a function $h$ such that $f_1(x,y,z) = \int_0^x\int_0^y\int_0^z h(u,v,w) dudvdw$.
Then, we can write 
\begin{align*}
  ...
  %  &= \frac {1}{\bar F(\sqrt{n})^3} \int_{x=0}^\infty\int_{y=0}^\infty\int_{z=0}^\infty f_n( \sqrt{n} x,\sqrt{n} y, \sqrt{n} z) dF(x)dF(y)dF(z)\\
    ...
    &= \int_{u=0}^\infty\int_{v=0}^\infty\int_{w=0}^\infty h(u,v,w) \frac{\bar F(u\sqrt{n})}{\bar F(\sqrt{n})} \frac{\bar F(v\sqrt{n})}{\bar F(\sqrt{n})} \frac{\bar F(w\sqrt{n})}{\bar F(\sqrt{n})} dudvdw,
\end{align*}
...
%    \Prob{\triangle_n>n^\gamma ; n^\lambda \text{ vertices of weight }>\sqrt{n} } \leq \Prob{ n^\lambda \text{ vertices of weight }>\sqrt{n} }.
%\end{equation}
By Lemma~\ref{lem:nvertices}, when $\gamma>3-3\alpha/2,$
\begin{align}
...
We first investigate the number of $AB_3'B_3'$ triangles. When $B\cap B_3=\emptyset$, we are done. Otherwise, $A\subseteq B_3$ and by~\eqref{eq:triangb3b3b3}, for $\gamma>3-3\alpha/2$ and $D=\sqrt{2a}/h(1+\varepsilon)$,
...
\subsection{$A+\delta$ deviations}
	Guess:
	\begin{itemize}
	    \item For $\alpha<4/3$: $\sqrt{2 a}n^{1-\alpha3/4}$ hubs of order $n$. In pairs, they form triangles with all other vertices, creating $n\Big(\sqrt{2 a}n^{1-\alpha3/4}\Big)^2=\delta n^{3-3\alpha/2}$ triangles. PROVEN NOW
	    ...
	    or (making change of variables and already taking a limit in the integration limits)
	     \begin{equation}
	        \mu^{3-3\alpha/2}\int_0^{\infty}\int_0^{\infty}c^2(xy)^{-\alpha}\min(xf(\delta),1)\min(yf(\delta),1) dx dy = \delta .
	    \end{equation}
	    which gives
	     \begin{equation}
	        \mu^{3-3\alpha/2}\Bigg(\int_0^{\infty}cx^{-\alpha}\min(xf(\delta),1)dx\Bigg)^2 = \delta .
	    \end{equation}
	    or
	    \begin{equation}
	        c^2\mu^{3-3\alpha/2}f(\delta)^{2\tau-4}\Bigg(\frac{1}{(2-\alpha)(\tau-2)}\Bigg)^2 = \delta,
	    \end{equation}
	    yielding $f(\delta)=\tilde{c}\delta^{1/(2\tau-4)}$.
	    Such a hub would on average create $\delta n^{3-3\alpha/2}$ triangles. This gives as conjecture:
     \begin{equation}
         \Prob{\triangle_n>n^{3-3\alpha/2}(H+a)}\sim \delta^{-\alpha/(2\tau-4)} n^{\frac{(2-\alpha)^2}{4(1-\alpha)}}
     \end{equation}
