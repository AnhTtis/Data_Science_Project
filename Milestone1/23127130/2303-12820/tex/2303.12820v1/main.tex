...
\title{The horizontal chord set \\ to CIRM and back }
\begin{abstract}
...
We are interested in the following question: for which
...
\begin{proof}
Fix a mountain range $f|_{[s_1,s_2]}$, and consider the shifted mountain range $f(x-\ell)$ (see Figure \ref{fig:shift}). For any shift $\ell\in[0,s_2-s_1]$,
...
First, we show that the chord set may not be the entire interval $[0,1]$:
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.1\textwidth]{unslidable-chord.png}
    \caption{A chord that can be slid neither up nor down.}
    \label{fig:slide}
\end{figure}
...
\begin{figure}[H]
    ...
    \textwidth]{four-bumps.png} 
    \hspace{0.25in}
    \includegraphics[height=0.25\textwidth]{countable-chords.png}
    \vspace{0.15in}
    \caption{A function with $S(f) = [0,1/2] \cup \{3/4,1\}$ and one where $S(f)$ has many accumulation points.}
    \label{fig:isolated}
\end{figure}
...
In this section, we will use Hopf's classical theorem:
{\it for the horizontal chord set $S(f)$, the set $S^c  := \R^+ \setminus S(f)$
is open and additive; i.e., if $a,b \in S^c$ then \mbox{$a + b \in S^c$}.}
The other main ingredient in this section is  the central symmetry $1 \to 1 - \ell$ of the interval $[0,1]$ .
The image of $S(f)$ by this symmetry is the set $T(f) := \{\ell: 1-\ell \in S(f)\}$.
Denote the Lebesgue measure on $[0,1]$ by $\lambda$.
We have the following result:

\begin{theorem}
Let $f:[0,1]\to\mathbf{R}$ be a continuous function with $f(0)=f(1)=0$. Then the length  of $S(f)$ satisfies
$$\lambda(S(f)) 
\ge \frac{1}{2}. $$
This bound is sharp.
\end{theorem}
...
\begin{proof} 
Since the map $\ell \to 1- \ell$ preserves the Lebsegue measure we have 
$$\lambda(T(f)) = \lambda(S(f)).$$

By Hopf's theorem if  $a + b \not \in S^*$ then we must have either $a$ or $b$ not in $S^c$, i.e., at least one of $a$ or $b$ belongs to $S(f)$.  We apply this to $a = \ell$ and $b = 1 - \ell$, since we know $1 \in S(f)$ we conclude that either $\ell$ or $1 - \ell$ is contained in  $S(f)$, or equivalently either $\ell \in S(f)$ or $\ell \in T(f)$; i.e.,
$$S(f) \cup T(f) = [0,1].$$
If $\lambda(S(f)) < 1/2$ the two display equations yield an immediate contradiction, thus the first statement of the theorem holds.
...
Our proof leads to the following natural question, does there exist a example of an $f$
...
We thank CIRM for an excellent working environment during our \guillemotleft~recherche en binomes~\guillemotright\  collaboration in July 2022. We used Desmos \cite{D} to experiment, explore all of the many cases, and create our figures.


\begin{thebibliography}{99}
\bibitem[BDD17]{BDD} Keith Burns, Orit Davidovich, Diana Davis, {\it Average pace and horizontal chords}, The Mathematical Intelligencer, 39(4)  (2017) 41-45.

\bibitem[D]{D} {\it Desmos}, \url{https://www.desmos.com/calculator/}, accessed 2022--2023.

\bibitem[H36]{H} Heinz Hopf, {\it \"Uber die Sehnen ebener Kontinuen und 
die Schleifen geschlossener Wege}, Comment.~Math.~Helv., 9 (1936) 303-319.

\bibitem[L34]{L} Paul L\'evy, {\it Sur une G\'en\'eralisation du Th\'eor\'eme de Rolle}, C.R.~Acad.~Sci., Paris, 198 (1934), 424--425.
