...
\newcommand\cec{}
\newcommand\peter{}
\newcommand\UB{\color{ForestGreen}}
...
\title[A two-table theorem for a disordered
  ...
  {\peter A two-table theorem for a disordered
  Chinese restaurant process}}
...
{We next give a formal definition of our model (Section~\ref{sub:def}) and state our main results (Section~\ref{sub:main_results}).} %\bigskip
...
\item[(iv)] {The proportion of customers on the largest table
\[
...
{\peter We thus give the proofs of Theorems~\ref{thm:A} and~\ref{thm:B} at the end of Section~\ref{sec:CT}, in which we introduce the embedding of our process into continuous time and state their continuous-time analogues.}
...
$\beta$, independently of each other.  Writing
 ...
 parameter~1,  independent also of $(W_i)_{i\geq 1}$, we define
\be\label{eq:yule}
...
elementary properties of the Yule process,  for any fixed $i\geq 1$,
\be\label{eq:cv_yule}
Z_i(t) \sim \zeta_i \exp(W_i(t-\tau_i))\quad\text{ almost surely as }t\uparrow\infty,
\ee 
where $(\zeta_i)_{i\geq 1}$ is a sequence of i.i.d.\ random variables of 
...
{\peter In the Weibull and Gumbel cases, to control the size of high-weighted tables that are created late in the process, we need the convergence of~\eqref{eq:maxdomain-2} to also hold in $L^1$.
This holds if $(n\mu\big((A(n)+uB(n),M))_{n\geq 1}$ is uniformly integrable, which is the case in all explicit examples we have considered, see also Appendix~B.}
...
implies that the function $f(u)= uA(u)/B(u)$ is increasing to infinity. This and continuity imply that, for all large enough~$t$, \eqref{eq:cond_u} has a unique solution $u_t=f^{-1}(t)$. 
By Assumption \ref{H}, we also have $B(u) = o(A(u))$ as $u\uparrow\infty$,
which implies that $f(u)\to\infty$ as $u\uparrow\infty$.
 Thus $u_t=f^{-1}(t)\uparrow \infty$ as $t\to\infty$.
...
$(u_t)$ is regularly varying with index $\frac\alpha{\alpha+1}$. And as $w_t=B(u_t)$ we get that $(w_t)$ is regularly varying with index $\frac{-1}{\alpha+1}$.
\smallskip
...
where $L(x)$ is slowly varying at $x=\infty$ and $\alpha> 0$.
\end{assumption}


In the Gumbel case, {\cec the assumption needed for the two-table theorem} is more complicated.
Recall the function $\Phi_t(x)$ %=u_t \mu(v_t+x w_t,M)$
in \eqref{eq:Phi_t-def} %which converges pointwise to $\Phi(x)$,
and that $\Phi(x)=\e{-x}$ in the Gumbel case.

%we introduce the following assumption:

\begin{assumption}\label{G}{\bf (Gumbel)}
{\cec In addition to~\eqref{eq:maxdomain-2}}, 
...
$\exp(tv_t+\Theta(tw_t))$.  
Indeed, the mass of all points with $\tau_n/u_t\to 0$ concentrates asymptotically on the subset of $\mathcal S$ where the first coordinate is zero. As this set has no mass under the intensity measure of the limiting Poisson process, these points must leave every compact subset of~$\mathcal S$ and this can only happen by their third coordinate going to $-\infty$. Hence none of these points corresponds to the largest table. This argument also applies when 
$\tau_n/u_t\to \infty$, or $\frac{W_n-v_t}{w_t}$ goes to infinity, or to zero in the 
Fr\'{e}chet, or $-\infty$ in the Weibull or Gumbel case. It
crucially rests on the compactification of the intervals in  the
...
\]
Since $M(t)$ is Poisson-distributed with parameter $\theta t$, 
...
we prove convergence of $\Gamma_t$ \eqref{eq:Gamma}
...
Secondly, in Subsection~\ref{sub:young_and_unfit}, 
...
where 
\[\mathrm d\zeta(z,y,z) = \begin{cases}
\theta\mathrm ds\otimes 
-\Phi'(y)\mathrm dy \otimes \delta_{y-s}(\mathrm dz),
&\text{ in the Weibull and Gumbel cases,}\\
\theta\mathrm ds\otimes 
...
Equation~\eqref{eq:cv_Psit} is a direct consequence of the following lemma:
...
For all $\varepsilon, \kappa>0$, there exists $x<x_{\max}$ such that, for all sufficiently large $t$, 
...
\ee
This may be proved using the maximal inequality for the submartingale 
...
\Big)\\
&\leq  \theta\, \int_{xu_t}^t \mathrm ds\,  \int_0^{\infty}  \mathrm d\mu(w) \, \mathbb P\big(A\geq \exp\big(\ell_\kappa(t) - w(t-s)\big)\\
...
= \theta\Phi_t\bigg(\frac{\kappa}{2(1-x)}\bigg)\\
&=(\theta+o(1)) \Phi\bigg(\frac{\kappa}{2(1-x)}\bigg),
\ea
...
For all $\varepsilon, \kappa>0$, there exists $y$ such that, for all sufficiently large $t$, 
...
Fix $\varepsilon>0$. 
By Lemma~\ref{lem:young}, there exists $x = x(\kappa, \eta)$ such that
\be\label{eq:B}
...
Furthermore, by Lemma~\ref{lem:unfit}, there exists $y = y(\kappa, \eta)$ such that
\be\label{eq:C}
...
and~\ref{sub:proof_main}, we conclude the 
proof of Theorem~\ref{th:two_tables} once Proposition~\ref{prop:two_tables_summable} has been established.  
We wish to show that for any $j\geq 3$,
the fraction ${Z_{m_j}(t)}/{Z_{m_1}(t)}$ converges to zero
almost surely, and then deduce the same result for 
${Z_{n_3}(t)}/{Z_{n_1}(t)}$,
...
exponent).    
We start by considering the times $(t_k)_{k\geq 1}$. 
Equation \eqref{eq:theta-sep-sum} 
implies that, at these times, the largest and third-largest exponents are well-separated. 
We expect the fraction ${Z_{m_j}(t_k)}/{Z_{m_1}(t_k)}$
to behave roughly like 
$\exp(-(\Theta^{(1)}(t_k)-\Theta^{(j)}(t_k)))\leq
\exp(-(\Theta^{(1)}(t_k)-\Theta^{(3)}(t_k)))$, which by
\eqref{eq:theta-sep-sum} %and the Borel--Cantelli lemma 
is smaller than
$\exp(-\lambda_{t_k}t_kw_{t_k})$ almost surely for all $k$ large enough.%
\smallskip

To show that $Z_{m_j}(t)$ indeed
grows like $\exp(\Theta^{(j)}(t))$, we 
need to establish that
it is unlikely that the largest exponents  were
created late.   This is done in Lemma~\ref{lem:new-tables}.
For tables created early, a large deviations estimate, 
see Lemma \ref{lem:Peter},
gives that, with high probability, the size of the table is close
to the exponential of its exponent.
...
\ee
and such that whenever $\xi-\lambda\geq -x_t$ we have
\be\label{eq:pi-A-diff-W}
\pi(A_t(\xi-\lambda))-\pi(A_t(\xi))
\leq C\lambda x_t^{\alpha-\delta} 
t^{\delta(1+\alpha+\delta+\frac1{1+\alpha})}.
\ee
\item In the Fr\'echet case, let $x_t>0$ with $x_tw_t\to\oo$.
{\cec For any $\delta, C>0$, for all $t$ large enough,}
\be\label{eq:pi-A-F}
%c x_t^{1+\alpha-\delta}\leq
\pi(A_t(x_t))
...
let $x_t>0$ with $x_t w_t/v_t\to 0$, 
and in the case $M=1$ also $\frac{x_t w_t}{1-v_t}\to0$.
Then, as $t\uparrow\infty$,
\be\label{eq:pi-A-G}
\pi(A_t(-x_t))
\geq \big(\theta+o(1)\big) \int_{-x_t}^{x_t} \Phi_t(z) \dd z
\ee
and if $\xi-\lambda\geq -x_t$
...
$w_t=t^{-\frac{1}{1+\alpha}} L(t)$.  This, combined with
...
\ee
Similarly to~\eqref{eq:similar}, using the Potter's bounds
\eqref{eq:potter-infty} 
\be
\Phi_t(x_t)\leq C_4 x_t^{-\alpha+\delta} 
t^{\delta(1/\alpha+\alpha-\delta)},
\ee
which gives \eqref{eq:pi-A-diff-F}.
...
\end{split}\ee
The intuition behind the first equality is that we integrate over all possible values $\xi$ for $\xi^{(1)}(t)$: for this renormalised exponent to be the maximum one, there needs to be one point of the point process at $\xi$, and none larger (hence the term $\mathbb{P}(\Pi(A_t(\xi))=\varnothing)$); for 
...
for $0<\eps<\tfrac1{1+\alpha}$ and in the decomposition
\eqref{eq:two_terms} we set $y_t=-\tfrac12x_t$.  
Since \smash{$w_t=t^{-\frac1{1+\alpha}}L(t)$} we have $x_tw_t\to0$.
...
$y_t=x_t$.  Since $w_t=t^{\frac1\alpha}L(t)$, we have $x_tw_t\to\oo$.
...
the lower bound in \eqref{eq:G2-Phi_t}, this gives that, % holds 
for all~$t$ large enough,
\be\begin{split}
...
Since $\e{2x_t(1+o(1))}=(\log t)^{4(1+o(1))}\leq t^\gamma$, for all $t$ large enough, this concludes the proof. \qedhere
\end{itemize}
\end{proof}
...
If $\tau_{m_1(t_k)}>t_{k-1}$, then $\Theta^{(1)}(t_k)\leq W_{m_1(t_k)}(t_k-t_{k-1})$ and we indeed have~\eqref{eq:ineq_third_expo}.
Otherwise, $W_{m_1(t_k)}(t_{k-1}-\tau_{m_1(t_k)})$ is at most equal to the largest exponent at time~$t_{k-1}$, which is, by definition, $\Theta^{(1)}(t_{k-1})$. This indeed implies~\eqref{eq:ineq_third_expo}.
%as can easily be seen by considering the slopes of the two affine
%functions involved. 
\end{proof}

{\cec{In the Gumbel case, we need the following facts about slowly varying functions:
...
\begin{proof}
{\it (1)} By Potter's bounds, for any $\delta>0$, there exists $x_0 = x_0(\delta)$ such that, for all $x\geq x_0$,
$L(x)\leq x^\delta$.
...
\ee
Then, by Potter's bounds, $\sum_{n\geq1} \PP\big(\max_{1\leq i\leq n}  W_i>
...
that, for all $k\geq k_0$ and $t\in[t_k,t_{k+1})$,
...
In the unbounded case $M=\oo$, for any $\rho>0$ and any $\eps>0$,
\be\label{eq:maxW-G}
...
{\cec Because, in the Gumbel case, $A$ is increasing}, we get
...
$j$-th largest table.  
This is true for tables that are `old', hence
we need to show that `new' tables can be dismissed.

\subsection{New tables do not contribute}

The following lemma makes precise a sense in which new tables cannot be too large. 
Recall that 
$m_j(t)$ denotes the index of the $j$-th largest exponent
$\Theta^{(j)}(t)=\Theta_{m_j(t)}(t)$ at time $t$ and
that $\tau_{m_j(t)}$ denotes the time at which the corresponding table
was created.\smallskip

For all $t>0$, set
\be\label{eq:def_st}
s(t) =\begin{cases}
\frac{t}{5(j+1)} & \text{ in the Weibull case,}\\
t^\rho \text{ where }\rho>0\text{ satisfies }1-\tfrac1\eta<\rho<\frac\alpha{1+\alpha}
& \text{ in the Fr\'echet case,}\\
\tfrac{t}{5(j+1)A(t)^2}
& \text{ in the Gumbel case.}
\end{cases}
\ee
For all $k\geq 1$, set $s_k = s(t_k)$.
With this definition, for all $k$ large enough,
\be\label{eq:prop_sk1}
t_k-s_k<t_{k-1}\ee
and
\be\label{eq:prop_sk2}
s_k<\frac{t_k}{j+1}.\ee
Indeed,~\eqref{eq:prop_sk1} holds because $t_k-t_{k-1} = \eta t_k^{\nicefrac1\eta-1}$ as $k\uparrow\infty$, and~\eqref{eq:prop_sk2} holds because $\rho<1$ in the Fr\'echet case,
and because $A(t)$ converges to either~$1$ (in the bounded Gumbel case) or infinity (in the unbounded Gumbel case) as $t\uparrow\infty$.


\begin{lemma}\label{lem:new-tables}
Fix $j\geq 1$.
Let $t_k=k^\eta$ with $\eta$ chosen as in Proposition
\ref{prop:two_tables_summable}.
Under Assumption \ref{H} in the Weibull and Fr\'echet cases,
and Assumption \ref{G} in the Gumbel case,
we have
\be\label{eq:old-exp-sum}
\sum_{k=1}^\oo \PP(
%\exists\; i\in\{1,2,\dotsc,j\}:
\exists\; t\in[t_{k-1},t_k]:
\tau_{m_j(t)}>t_k-s_{k}
)<\oo.
\ee
\end{lemma}
Fix $j\geq 1$. 
For all $k\geq 1$, set
\[\mathcal{E}_k = \mathcal{E}_k^{(j)}=
\{\exists t\in[t_{k-1},t_k]:
\tau_{m_j(t)}>t_k-s_{k}\}.\] 
On $\mathcal{E}^c_k$, for all times~$t\in [t_{k-1},t_k]$,
the $j$-th largest exponent is `old' in the sense that the table
it corresponds to was created before time $t_k-s_k$.
As noticed in~\eqref{eq:prop_sk1}, $t_k-s_k<t_{k-1}$ for all $k$ large enough: 
in particular any table created within the interval $[t_{k-1},t_k]$ is considered as `new'.

\begin{proof}
%{\cec Make it clear we look at exponents at time $t_k$.} Not true!
For $i\in\{1, 2, \ldots, j\}$, we set
\be
I_i:=\Big[\frac{(i-1)t_k}{j+1},\frac{i t_k}{j+1}\Big).
\ee
The goal is to show that, with high probability,  in
each of the intervals $I_1,\dotsc,I_j$ there is a table created whose
exponent  is larger than the exponent of any table created between time $t_k-s_k$ and time $t_k$. 
Then none
of the $j$ largest exponents can have been created after time $t_k-s_k$.
We first note that, if $k$ is large enough,
the intervals $I_1, \ldots, I_j$, and $[t_k-s_{k}, t_k]$ are disjoint; this follows from~\eqref{eq:prop_sk2}, which is equivalent to
i.e.\ $\tfrac{j}{j+1} t_k< t_k-s_k$.\smallskip

First note that, for all $k\geq 1$,
\be\ %label{eq:start}
\PP(\mathcal{E}_k)
\leq \PP\Big(\exists t\in[t_{k-1},t_k]:
\max_{\tau_n>t_k-s_{k}} W_n(t-\tau_n)\geq
\min_{1\leq i\leq j}
\max_{n:\tau_i\in I_i} W_n(t-\tau_n) 
\Big).\ee
Indeed, if $\max_{\tau_n>t_k-s_{k}} W_n(t-\tau_n)<
\min_{1\leq i\leq j}
\max_{n:\tau_i\in I_i} W_n(t-\tau_n)$,
then all exponents $W_n(t-\tau_n)$ such that $\tau_n>t_k-s_k$ 
are smaller than at least $j$ other exponents at time $t$;
and if this is true for all $t\in[t_{k-1},t_k]$, then $\mathcal E_k^c$ holds.
For $t\in[t_{k-1},t_k]$ we have the following bounds on exponents:
\be\label{eq:ifthen1}
\mbox{if } \tau_n>t_k-s_k\mbox{ then, }
W_n(t-\tau_n)\leq W_n(t_k-(t_k-s_k))=s_k W_n,
\ee
and for $k$ large enough,
\be\label{eq:ifthen2}
\mbox{if } \tau_n\in\bigcup_{i=1}^j I_i \mbox{ then, }
W_n(t-\tau_n)\geq W_n(t_{k-1}-\tfrac{j}{j+1} t_k)
\geq \tfrac{t_k}{2(j+1)} W_n.
\ee
We used that $t_{k-1}/t_k=(\frac{k-1}{k})^\eta\to1$
as $k\to\oo$ so $t_{k-1}\geq\frac{j+1/2}{j+1}t_k$ for all $k$ large enough.  
We thus get
\ban
\PP(\mathcal{E}_k)
&\leq \PP\Big( \exists t\in[t_{k-1},t_k]:
s_{k}
\max_{n:\tau_n>t_k-s_{k}} W_n  > 
\frac {t_{k}}{2(j+1)}
\min_{1\leq i\leq j}
\max_{n:\tau_n\in I_i}  W_n 
\Big)\notag\\
&=\PP\big(\tfrac{2(j+1)s_{k}}{t_{k}}  X > \min_{1\leq i\leq j} Y_i\big), %\label{eq:start}
\ean
where we have set
\be\label{eq:def_XY}
X:=\max_{n:t_k-s_{k}<\tau_n<t_k} W_n \quad
\text{ and }\quad
Y_i:=\max_{n:\tau_n\in I_i}  W_n,
\mbox{ where } i\in\{1,\dotsc,j\}.
\ee
%the $j$-th largest exponent at time $t$ cannot correspond to a table born after time $t_k-s_k$.
Using that $Y_1, \ldots, Y_j$ are i.i.d., see~\eqref{eq:def_XY}, 
we get that, for all $k$ large enough,
\be\begin{split}\label{eq:PEk}
\PP(\mathcal E_k)
&\leq\PP\big(\tfrac{2(j+1)s_{k}}{t_{k}}  X > \min_{1\leq i\leq j} Y_i\big)
\leq j \PP\big(Y_1<2(j+1)\tfrac{s_k}{t_k}X\big)\\
&= j \EE\Big[\PP\big(W_1<2(j+1)\tfrac{s_k}{t_k}X\big)^{M(I_1)}\Big]\\
&=j\e{-\frac{\theta}{j+1}t_k}
\exp\big(
\tfrac{\theta}{j+1} t_k \PP\big(W_1<2(j+1)\tfrac{s_k}{t_k}X\big)
\big),
\end{split}\ee
where we used that the number $M(I_1)$ of tables created in the
interval $I_1$ has Poisson distribution with parameter
$\frac{\theta}{j+1}t_k$. 
We look at the three different cases.\smallskip

\begin{itemize}[leftmargin=*]
\item Weibull and bounded Gumbel: in these cases, almost surely,  $X\leq 1$. 
Furthermore, by assumption, for all $k$ large enough,
$s_{k}/t_{k}\leq \tfrac{1}{4(j+1)}$. 
We thus get
\be
\PP(\mathcal E_k)
\leq j\exp(-\tfrac\theta{j+1} t_k + \tfrac{\theta}{j+1}t_k \mu(0,\tfrac12))
= j\exp(-\tfrac\theta{j+1}\mu(\tfrac12,1)t_k).
\ee
Because $t_k = k^\eta$ with $\eta>0$, and because $\mu(\tfrac12,1)>0$,
the probabilities $\mathbb P(\mathcal E_k)$ are indeed summable.\smallskip
\item Fr\'echet:   in this case $\mu(x,\oo)=x^{-\alpha}L(x)$
where $L(x)$ is slowly varying at infinity.
For any $\sigma>0$,
\be
\PP\big(W_1<2(j+1)\tfrac{s_k}{t_k}X\big)\leq
\PP\Big(2(j+1)\tfrac{s_{k}}{t_{k}}  X\geq
t_{k}^{\nicefrac1\alpha-\sigma}\Big) 
+{\PP}\big(W_1<t_{k}^{\nicefrac1\alpha-\sigma}\big)
\ee
Using $s_k=t_k^\rho$ and conditioning on the number of tables created
after time $t_k-s_k$ we get
\be\begin{split}
\PP\Big(2(j+1)\tfrac{s_{k}}{t_{k}}  X\geq
t_{k}^{\nicefrac1\alpha-\sigma}\Big) &\leq
\theta s_k \PP(W_1\geq \tfrac{1}{2(j+1)} t_k^{1+1/\alpha-\sigma-\rho})
\\
&\leq \tfrac{\theta 2^\alpha(j+1)^\alpha}{t_k}t_k^{-\alpha(1-\sigma)+\rho(1+\alpha)}
L(\tfrac{1}{2(j+1)} t_k^{1+1/\alpha-\sigma-\rho})
\end{split}\ee
Since $\rho<\frac\alpha{1+\alpha}$ we can choose $\sigma>0$
so that $\alpha(1-\sigma)>\rho(1+\alpha)$.
Then
\be
\PP\Big(2(j+1)\tfrac{s_{k}}{t_{k}}  X\geq
t_{k}^{\nicefrac1\alpha-\sigma}\Big)=o(t_k^{-1})
\ee
so that, using  \eqref{eq:PEk}, 
\be
\PP(\mathcal E_k)\leq j\exp\big(o(1)-
\tfrac{\theta}{j+1} t_k \PP\big(W_1\geq t_k^{1/\alpha-\sigma})\big).
\ee
Since 
\be
 t_k \PP\big(W_1\geq t_k^{1/\alpha-\sigma})=
t_k(t_k^{1/\alpha-\sigma})^{-\alpha} L(t_k^{1/\alpha-\sigma})
=t_k^{\alpha\sigma} L(t_k^{1/\alpha-\sigma}),
\ee
we indeed get that
$\sum_{k\geq 1} \PP(\mathcal E_k)<\oo$ in this case as well.\smallskip
\item {\cec 
Unbounded Gumbel:
We fix $\eps\in(0,1)$ and split 
depending on whether $X\leq A(t_k)^{1+\eps}$
or $X> A(t_k)^{1+\eps}$:
\be\begin{split}
\PP\big(W_1<2(j+1)\tfrac{s_k}{t_k}X\big)
&\leq \PP\big(W_1<2(j+1)\tfrac{s_k}{t_k}A(t_k)^{1+\eps}\big)+
\PP\big(X> A(t_k)^{1+\eps}\big)\\
&\leq \PP\big(W_1<\tfrac{2(j+1)}{A(t_k)^{1-\eps}}\big)+
\theta s_k \PP\big(W_1> A(t_k)^{1+\eps}\big)\\
&\leq t_k \mu\big(A(t_k)^{1+\eps},\infty\big).
\end{split}\ee
By Assumption~\ref{H}, $A^{-1}(A(t)^{1+\eps})\mu(A(t)^{1+\eps},\oo)\to 1$ as $t\uparrow\infty$.
By Lemma~\ref{lem:XXX}, $t\ll A^{-1}(A(t)^{1+\eps})$ as $t\uparrow\infty$. 
Thus, $t\mu(A(t)^{1+\eps},\oo)\to 0$ as $t\uparrow\infty$,
which implies that $\PP\big(W_1<2(j+1)\tfrac{s_k}{t_k}X\big)\to 0$ as $k\uparrow\infty$, as needed.

\noindent 
Using \eqref{eq:PEk} we get, for large $k$, that
$\PP(\mathcal E_k)\leq j\e{-\frac{\theta}{2(j+1)}t_k}$
which is summable.}
\qedhere
\end{itemize}
\end{proof}
...
Fix $j\geq 1$
and work under Assumption \ref{H} in the Weibull and Fr\'echet cases,
respectively Assumption \ref{G} in the Gumbel case.
Let $t_k=k^\eta$ be as in Proposition \ref{prop:two_tables_summable},
and let $\varphi\in(0,1)$ be chosen as follows:
\begin{itemize}[leftmargin=*]
\item Weibull:  $\varphi<1/\eta$
\item Fr\'echet:  $(1-\varphi)\rho>2/\alpha+1-1/\eta$
where $\rho$ is 
as in~\eqref{eq:def_st}.
\item Gumbel:  $\varphi<1/\eta$
\end{itemize}
Consider the event
\be\label{eq:Mk}
\mathcal M_k=
...
|\log Z_{m_j}(t)-\Theta_{m_j}(t)|\leq t_k^{1-\varphi}
\big\}.
\ee
Then, almost surely, $\mathcal M_k$ holds for all $k$ large enough.  
\end{proposition}
\begin{proof}
We aim at using the Borel--Cantelli 
lemma, and thus prove that $\mathbb P(\mathcal M^c_k)$ is summable. 
Using Lemma \ref{lem:new-tables} we may assume that 
$\tau_{m_j(t)}\leq t_k-s_{k}$.  Then,
\be
\begin{split}
\PP(\mathcal M_k^c)
&\leq \PP\big(\exists \tau_m\leq t_k-s_{k}:
\sup_{t\in [t_{k-1},t_k]}  
|\log Z_{m}(t)-\Theta_{m}(t)|> t_k^{1-\varphi}\big)\\
&\leq \EE\Big[
\sum_{\tau_m\leq t_k-s_{k}} \PP\big( \sup_{t\in [t_{k-1},t_k]}  
|\log Z_{m}(t)-\Theta_{m}(t)|> t_k^{1-\varphi}\mid W_m, \tau_m\big)
\Big] \\
& \leq \EE\Big[
\sum_{\tau_m\leq t_k-s_{k}} \PP\big( \sup_{t\in [t_{k-1},t_k]} \\
&\phantom{ladidadida}
|\log Z_{m}(t)-W_{m}(t-\tau_m)|> (t_k-\tau_m)^{1-\varphi}\mid W_m, \tau_m\big)
\Big].
\end{split}
\ee
%where the expectation is over the $\tau_m$'s and the $W_m$'s.
We now use Lemma \ref{lem:Peter} with $\lambda = W_m$, $a=t_{k-1}$, $b=t_k$, and $y = t_k^{-1} (t_k-\tau_m)^{1-\varphi}$; we get that there exists a constant $C>0$ such that
\be\begin{split}
&\PP\big(\sup_{t\in [t_{k-1},t_k]}  
|\log Z_{m}(t)-W_{m}(t-\tau_m)|> (t_k-\tau_m)^{1-\varphi}\mid W_m, \tau_m\big)\\
&\leq (2 + t_{k-1} W_m ) \exp\big(-(t_k-\tau_m)^{1-\varphi}+(t_k-t_{k-1}) W_m
\big) \\
&\leq (2 + t_{k-1} W_m ) \exp\big(-s_k^{1-\varphi}+(t_k-t_{k-1}) W_m
\big).
\end{split}\ee
Thus, because $\#\{m\colon \tau_m\leq t_k-s_{k}\}$ is
Poisson-distributed of parameter $\theta(t_k-s_{k})$, 
\be\label{eq:Mk-stretch}
\PP(\mathcal M_k^c)
\leq \theta (t_k-s_{k})
\, \mathrm e^{-s_{k}^{1-\varphi}} \,
\EE\big[ (2 + t_{k-1} \overline W(t_k) ) \, \mathrm e^{ (t_k-t_{k-1}) \overline W(t_k) } \big],
\ee
where we recall that $\overline W(t) = \max_{m\leq M(t)} W_m$
and $t_k-t_{k-1}=\mathcal O(t_k^{1-1/\eta})$ as $k\uparrow\infty$.
\begin{itemize}[leftmargin=*]
\item In the case when $M=1$, i.e.\ the Weibull and bounded Gumbel cases, 
$\overline W(t_k)\leq 1$ while $s_k$ equals $t_k$ times some slowly
varying function, see~\eqref{eq:def_st} and recall that $s_k = s(t_k)$.
Thus, by \eqref{eq:Mk-stretch} and using Potter's bounds to control the slowly varying function $A(t_k)$ in the Gumbel case, we get that
$\PP(\mathcal M_k^c)$ decays in~$k$ as fast as a stretched exponential in $t_k$, and
hence in $k$, as soon as $1-\varphi>1-1/\eta$. 
Thus, $\sum_{k\geq1} \PP(\mathcal M_k^c)<\oo$ as required.
\smallskip
\item In the Fr\'echet case, we use Lemma~\ref{lem:max_as_F}
and its consequence \eqref{eq:overlline-W-bound}
 which gives that, for any $\eps>0$,
$\overline W(t_k) \leq t_k^{\nicefrac2\alpha+\eps}$
almost surely for large enough $k$.  
Also, in this case, $s_k=t_k^\rho$, see~\eqref{eq:def_st},
with $0<\rho<\tfrac\alpha{1+\alpha}$.
We thus get that, again, by~\eqref{eq:Mk-stretch}, 
$\PP(\mathcal M_k^c)$ decays as fast as a stretched exponential in $t_k$, and
hence in $k$, as soon as 
$(1-\varphi)\rho>\frac2\alpha+1-\tfrac1\eta+\eps$,
which can be satisfied for $\eps$ small enough, 
since we have assumed $(1-\varphi)\rho>\tfrac2\alpha+1-\tfrac1\eta$.
\smallskip
\item In the unbounded Gumbel case, arguing similarly to~\eqref{eq:maxW-G} and below it, we get that, for all $\eps>0$,
$\overline W(t_k)\leq t_k^\eps$ almost
surely for all~$k$ large enough. 
Thus, if $t_k^{1-1/\eta+\eps} = o(s_k^{1-\varphi})$ as $k\uparrow\infty$, then $\sum_{k\geq 1}\PP(\mathcal E_k)<\infty$ as desired.  
Since $s_k=\tfrac{t_k}{5(j+1)A(t_k)^2}$ and $A(t)$ is slowly varying, 
this is satisfied when $1-\varphi>1-\tfrac1\eta+\eps$,
which is true for $\eps$ small enough, 
since we have assumed 
$\varphi<\tfrac1\eta$. \qedhere
\end{itemize}
\end{proof}
...
Fix any $j\geq3$ and recall that $m_j=m_j(t)$ denotes the index of the
$j$-th largest exponent $\Theta$.  Our first aim is to prove that 
\be\label{eq:aim}
\sup_{t\in[t_{k-1},t_k]} \frac{M(t) Z_{m_j}(t)}{Z_{m_1}(t)}
\to 0, \quad\mbox{ almost surely as }k\to\oo.
\ee
We first prove~\eqref{eq:aim}, 
...
Thus, almost surely for all $k$ large enough,
\be\begin{split}\label{eq:onMk}
\sup_{t\in[t_{k-1},t_k]} \frac{M(t) Z_{m_j}(t)}{Z_{m_1}(t)}
&\leq \sup_{t\in[t_{k-1},t_k]}  
\frac{M(t) \exp(\Theta_{m_j}(t)+t_k^{1-\varphi})}
{\exp(\Theta_{m_1}(t)-t_k^{1-\varphi})}\\
&= \sup_{t\in[t_{k-1},t_k]}  M(t)
\exp\big(
-(\Theta_{m_1}(t)-\Theta_{m_j}(t))+2t_k^{1-\varphi}
\big)\\
&\leq  M(t_k)
...
\ee
by Proposition~\ref{prop:interpolate}.
Using the fact that $M(t_k)\leq 2\theta t_k$ almost surely for all $k$ large enough, by a large deviation estimate for Poisson random variables, because $M(t_k)$ is a Poisson random variable of parameter $\theta t_k$, we get that, almost surely for all $k$ large enough,
\be
\sup_{t\in[t_{k-1},t_k]} \frac{M(t) Z_{m_j}(t)}{Z_{m_1}(t)}
\leq 2\theta t_k
\exp\big(-\tfrac12 t_k\lambda_{t_k} w_{t_k}+2t_k^{1-\varphi}
\big)
\ee
We need to check that the right-hand-side of \eqref{eq:onMk} converges
to zero almost surely, i.e. that
...
$\kappa+\tfrac1{1+\alpha}<\tfrac1\eta<2\kappa$.
For  Proposition \ref{prop:exponents-vs-tables}, 
we need $\varphi\in(0,1)$
with $\varphi<\tfrac1\eta$.  And for \eqref{eq:onMk-W}, we need
$\varphi>\kappa+\tfrac{1}{1+\alpha}$.  These inequalities can
...
$\kappa>\tfrac1{1+\alpha}$ 
satisfying $\tfrac2{1+\alpha}<\kappa+\tfrac1{1+\alpha}<1$. 
We can then pick $\varphi$ satisfying
$\kappa+\tfrac1{1+\alpha}<\varphi<\min\{1,2\kappa\}$, 
and finally let $\eta$
satisfy $\varphi<\tfrac1\eta<2\kappa$.  
\item Fr\'echet:  For Proposition \ref{prop:interpolate}, we need
$\kappa+\tfrac1{\alpha}<\tfrac1\eta<2\kappa$.
For Lemma \ref{lem:new-tables}, we need $\rho>0$
with $1-\tfrac1\eta<\rho<\tfrac\alpha{1+\alpha}$.
For  Proposition \ref{prop:exponents-vs-tables}, 
we need $\varphi\in(0,1)$
with $(1-\varphi)\rho>\tfrac2\alpha+1-\tfrac1\eta$.  
And for \eqref{eq:onMk-F}, we need
$\varphi>\kappa-\tfrac{1}{\alpha}$.  These inequalities are consistent
...
$\frac1\alpha<\kappa<\frac{1+\alpha}\alpha$.  Note that we then have 
$2\kappa>\kappa+\frac1\alpha>\frac2\alpha>\frac1{1+\alpha}$.  Next
pick $\eta$ such that $\kappa+\frac1\alpha<\frac1\eta<2\kappa$.  Since
...
$1-(\kappa-\frac1\alpha)=\frac{1+\alpha}{\alpha}-\kappa>0$ so we can
find $\varphi$ such that
$\kappa-\frac1\alpha<\varphi<\frac\alpha{1+\alpha}$.  It remains to
...
are at their extreme values, for some $\kappa\in (\tfrac1\alpha, \tfrac{1+\alpha}\alpha)$.  
...
\item Gumbel:  Proposition \ref{prop:interpolate} places no
  restrictions on the parameters in this case, nor does Lemma
  \ref{lem:new-tables}.  
For  Proposition \ref{prop:exponents-vs-tables}, 
we need $\varphi\in(0,1)$
with $\varphi<\tfrac1\eta$,  and for \eqref{eq:onMk-G}, we need
$\varphi>\kappa$.  In this case we simply pick any
$\kappa,\varphi,\eta$ such that $0<\kappa<1$ and 
$\kappa<\varphi<\frac1\eta<2\kappa$, with $\varphi<1$.
\end{itemize}

Having shown that the various inequalities can all be simultaneously 
satisfied, we conclude that the right-hand-side of
\eqref{eq:onMk} goes to 0, which means that, for$j\geq 3$,
\be \label{eq:interpolate}
\mathbb{P}\Big(
\frac{M(t) Z_{m_j}(t)}{Z_{m_1(t)}}\to 0
 \text{ as }t\to\oo\Big)=1.
\ee
...
By countable subadditivity, \eqref{eq:interpolate} 
implies that $\PP(\mathcal G)=1$, where
\be
\mathcal G=\Big\{
\mbox{for all }j\geq3,\; \lim_{t\uparrow\infty}\frac{M(t) Z_{m_j}(t)}{Z_{m_1(t)}}=0
\Big\}.
\ee
...
large enough $k$. Otherwise,  there exists an arbitrarily large $k$ such that 
$n_1(t_k)=m_j(t_k)$ for some $j\geq 3$,
giving
\be
...
\[\lim_{t\to \infty}\frac{M(t)}{\log N(t)}= \frac{\theta}{{\rm essup }\,  \mu}.\]
...
\ea
We aim to use the dominated convergence theorem: note that, for all $x\leq u\leq 1+\log t$,
...
Thus, because $u\mapsto \mathrm e^{-u}$ is integrable on $[x,\infty)$,
...
{\cec Note that we are unable to prove that Assumption~\ref{G} 
...
However, the following function $m$ corresponds to a weight distribution $\mu$ that is in the domain of attraction of the Gumbel distribution and does not satisfy Assumption~\ref{G} (this distribution does not satisfy the von Mises condition): $m(x) = \log\big(\tfrac{\mathrm e}{1-x}\big)\log\log\big(\tfrac{\mathrm e}{1-x}\big)$, for all $x\in[0,1)$.}

% (f) does not (but still belongs to the domain of attraction of the Gumbel distribution). 
(a--e) are all examples of bounded weight distributions.
The following is an
unbounded example:
\begin{itemize}
\item[(f)]
...
\bigg(1-\frac{xB(t)}{\hat A(t)}\bigg)^{-\alpha}\bigg].
\]
Now note that, for all $y<1$, $(1-y)^{-\alpha}\geq 1 + \alpha y$.
Thus, for all $x< \hat A(t)/B(t)=\alpha(1+\log t)$, we have 
\[
t\mu(A(t)+xB(t), 1) 
\leq t\exp\bigg[1-\hat A(t)^{-\alpha} \bigg(1+\alpha\frac{xB(t)}{\hat A(t)}\bigg)\bigg]
...
\bigg(1+\frac{\alpha xB(t)}{\hat A(t)}+C \frac{x^2B(t)^2}{\hat A(t)^2}
\bigg)\bigg]\\
