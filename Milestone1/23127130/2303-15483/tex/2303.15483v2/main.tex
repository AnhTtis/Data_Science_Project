...
\begin{abstract}
Smithson (1971) ``Fixed points of order preserving multifunctions'' gives a fixed point theorem for a self correspondence on a partially ordered set, but its proof has an error.
This paper points it out by constructing a counterexample and proves the theorem correctly.
\end{abstract}
...
For a subset $S$ of $X,$ the \textit{supremum} of $S,$ denoted by $\sup S,$ is the least upper bound of $S.$
...
In the proof, \cite{smithson1971fixed} considers a family $\cS$ of subsets of $X$ defined as
\begin{align*}
    \cS
    \coloneqq
    \left\{
...
\begin{align*}
    \begin{cases}
        M \geq x \ \text{for} \ x \in X \\
        x \geq m \ \text{for} \ x \in X \\
        (a, i) \geq (b, j) \Leftrightarrow a \geq b, \ i = j \ \text{for} \ a, b \in (0, 1) \ \text{and} \ i, j \in \{1, 2\}
    \end{cases}
    .
\end{align*}
...
\begin{tikzpicture}[scale=1.8]
  ...
  \draw (0.3,\lensHeight) node {$M$};
  \draw (-0.8,0) node {$(0, 1) \times \{1\}$};
  \draw (0.8,0) node {$(0, 1) \times \{2\}$};
\end{tikzpicture}
...
\begin{align*}
    \begin{cases}
        F(M) = \{M\} \\
        F(a, 1) = \{(a, 1)\} \ \text{for} \ a \in (0, 1) \\
        F(b, 2) = \{m\}  \ \text{for} \ b \in (0, 1) \\
        F(m) = \{m\}
    \end{cases}
    .
\end{align*}
...
An insightful observation is that one cannot add $M$ to $X_0$ because of the ``convexity'' condition (2).
...
\textbf{Step 2}:
In the same way as \cite{smithson1971fixed}, there exist a subset $C_0$ of $C$ and an isotone function $g_0 : C_0 \to X$ such that (i) $x_0 = \sup C_0$ and (ii) $g_0 (x) \in F(x)$ and $g_0(x) \geq x$ for all $x \in C_0.$
