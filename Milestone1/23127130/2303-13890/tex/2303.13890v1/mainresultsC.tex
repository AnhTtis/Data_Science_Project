...
\begin{rmk}\label{rem:mainL1}
	The asymptotic \eqref{asymp} is in fact uniform. Indeed,  since  $\phi'$ is decreasing then $a_*$ in \eqref{eq:a*} is monotone in $t/x$. Hence, if $t/x -\mathfrak{b}\leq h(x)$, with $\limo{x}h(x)=0$, then \eqref{asymp} is valid uniformly in $t$ on this region with $a_*$ evaluated for $t=\mathfrak{b}x+xh(x)$. Even more, for this theorem and all others concerning the large asymptotic behaviour one can track explicitly the constants in the speed of convergence, see \eqref{asymp}, and thereby obtain strict bounds on the densities and its derivatives. This, however, is a matter of another study, as it has to be done for classes of subordinators.
\end{rmk}
...
The most recent results are Theorems 1-2 of \cite{DonRiv}. They deal only with densities and offer no estimates on the speed of convergence. When $t(x)/x\downarrow \mathfrak{b}$ it is required that
