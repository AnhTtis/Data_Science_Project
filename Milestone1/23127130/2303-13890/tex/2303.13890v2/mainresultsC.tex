...
In this section we present results concerning $\fP{x,t}$, see \eqref{def:f}, which in the case of $\mathfrak{b}=0$ is the density of $L(t)$. The results are in two directions - series expansion and behaviour at zero, and large asymptotics together with speed of convergence for $\fP{x,t}$ and its derivatives. We complement the latter results by providing information about $f_\phi^c(x,t)$ (see \eqref{def:fc}). We also furnish  results for the density of $\sigma(t)$ and its derivatives. 
...
\end{equation}
plays a role only in some results. We offer discussion of these conditions in Section \ref{subsubsec:EX}, we present alternative formulation of \eqref{def:condiB} and we demonstrate that they are milder than those in the literature.
First of all, let us underline that condition $\eqref{def:condiA}$ is related to the regularity of the function $f_\Phi$ defined in \eqref{def:f}.
\begin{thm}\label{thm:regularityfphi1}
	Let $\Phi$ be the Laplace exponent of some potentially killed subordinator and assume that condition $\eqref{def:condiA}$ holds. Then, for any $n \ge 0$ there exists $x_0(n,L) \ge 0$ such that, for any $k,l \ge 0$ with $k+l \le n$ and $(x,t) \in \mathbb{D}$ with $x>x_0(n,L)$ $\displaystyle \frac{\partial^k}{\partial x^k}\frac{\partial^l}{\partial t^l}f_\phi(x,t)$ is well-defined and
	\begin{equation}
		...
		\label{intreprder1}
	\end{equation}
	where the integral is absolutely convergent. If $L=\infty$ in \eqref{def:condiA}, then for any $n \ge 0$, $x_0(n,\infty)=0$.
%	
...
\end{thm}
Theorem \ref{thm:regularityfphi1} is proved in Section \ref{sec:LT} and it is needed for the study of the asymptotic behaviour of $f_\Phi$. The first result in this direction considers the behaviour of $f_\Phi(x,t)$ when $t/x\downarrow \mathfrak{b},$ as $x\to\infty$.
\begin{thm}\label{thm:mainL}
	Let $\Phi$ be the Laplace exponent of some potentially killed subordinator and assume that  conditions $\eqref{def:condiA}$ and $\eqref{def:condiB}$ hold true. If $t=t(x)$ is such that $t(x)/x\in\lbrb{\mathfrak{b},\Phi'(0^+)}$ and  $\limi{x}t(x)/x=\mathfrak{b}$ from above, then, for any $k\geq0,l\geq0$, we have
	...
	\end{equation}
	Furthermore, for any $M<L$, $a_*\geq Me^{-1}(t/x-\mathfrak{b})^{-1}$ for all $x$ large enough. 
\end{thm}
\begin{rmk}\label{rem:mainL} In Section \ref{subsubsec:EX} we prove that \eqref{def:condiA} and \eqref{def:condiB} are milder than the respective one in \cite{DonRiv}. $\eqref{def:condiB}$ is very mild as it holds even for some Compound Poisson processes, e.g., when $\nu_\Phi(dy)=e^{-y}dy$. It cannot be deduced from the more stringent \eqref{def:condiA} as it implies $\Phi(\infty)=\infty$. \eqref{def:condiA} typically holds with $L=\infty$ as in the regular variation case treated as a standing example in Section \ref{subsubsec:EX}. 
\end{rmk}
\begin{rmk}\label{rem:mainL1}
	The asymptotic \eqref{asymp} is in fact uniform. Indeed,  since  $\phi'$ is decreasing then $a_*$ in \eqref{eq:a*} is monotone in $t/x$. Hence, if $t/x -\mathfrak{b}\leq h(x)$, with $\limo{x}h(x)=0$, then \eqref{asymp} is valid uniformly in $t$ on this region with $a_*$ evaluated for $t=\mathfrak{b}x+xh(x)$. Even more, for this theorem and all others concerning the large asymptotic behaviour one can track explicitly the constants in the speed of convergence, see \eqref{asymp}, and thereby obtain strict upper bounds on the densities and its derivatives. Those upper bounds will be in the line of studies of \cite{ChoKim21,GrLTr21} but under milder assumptions. For lower bounds as in \cite{ChoKim21,GrLTr21} a further study is needed but the saddle point method is poised to deliver these too. For more information see the discussion in Section \ref{subsubsec:EX} and note that the same reasoning applies for our other main results below.
\end{rmk}
...
\begin{thm}\label{thm:main1}
	Let $\Phi$ be the Laplace exponent of some potentially killed subordinator and assume that \eqref{def:condiA} holds. If $t=t(x)$ is such that $t(x)/x\in\lbrb{\mathfrak{b},\Phi'(0^+)}$ and  $\limi{x}t(x)/x=\bar{x}\in\lbrb{\mathfrak{b},\Phi'(0^+)}$, then, for any $k\geq0,l\geq0$, we have 
	\begin{equation}\label{def:fin}
		\begin{split}
			&\frac{\partial^k\partial ^l }{\partial x^k\partial t^l}\fP{x,t}=\frac{(-1)^k\Phi^\dagger(\bar{a})\Phi^{k}(\bar{a})}{\sqrt{2\pi}\bar{a}^{1-l}\sqrt{-\Phi''(\bar{a})}}\frac{e^{\bar{a}t-x\Phi(\bar{a})}}{\sqrt{x}} \lbrb{1+\bo{\sqrt{\ln(x)}x^{-\frac12}}}, \ \mbox{ as }x \to \infty,
		\end{split}
	\end{equation}
...
\begin{thm}\label{thm:main2}
	Let $\Phi$ be the Laplace exponent of some potentially killed subordinator and assume that  conditions $\eqref{def:condiA}$ and $\eqref{def:condiB'}$ hold true. If $t=t(x)$ is such that $t(x)/x\in\lbrb{\mathfrak{b},\Phi'(0^+)}$ and  $\limi{x}t(x)/x=\Phi'(0^+)$ from below in such a way that the solution to $\eqref{eq:a*}$, that is $a_*=a_*(x)$ satisfies
	\begin{equation}\label{eq:addCondi}
		\limi{x} -x\phi''(a_*)a^2_*=\infty, \quad \limsupi{x}\frac{\ln\lbrb{\frac{1}{a_*}}}{x}<\infty,\quad \forall \delta>0 \limi{x}e^{-\delta x}x\phi''(a_*)a^2_*=0,
	\end{equation}
...
\begin{rmk}\label{rem:main2}
 We mention that if $\phi''(0^+)<\infty$ then the last two conditions in \eqref{eq:addCondi} follow from the first, which actually becomes $\limi{x}xa^2_*=\infty$. Also, it is not hard to get that the latter implies \eqref{def:condiB'}.
\end{rmk}
...
\[\IntOI e^{-z t}G_\Phi(x,t)dt=\frac{e^{-x\Phi(z)}}{z}=\frac{z}{\Phi^\dagger\lbrb{z}}\int_{0}^\infty f_\Phi(x,t)e^{-z t}dt\]
...
\begin{thm}\label{thm:mainS}
	Let $\Phi$ be the Laplace exponent of some potentially killed subordinator and assume that condition \eqref{def:condiA} holds. Then, for any $n \ge 0$ there exists $x_0(n,L) \ge 0$ such that, for any $k,l\ge 0$ with $k+l \le n$ and $(x,t) \in \mathbb{D}$ with $x>x_0(n,L)$, $\displaystyle \frac{\partial^k}{\partial x^k}\frac{\partial^l}{\partial t^l}G_\Phi(x,t)$ is well-defined. If $L=\infty$ in \eqref{def:condiA}, then $x_0(n,\infty)=0$. In particular, the density $g_\Phi$ is well-defined for $x > x_0(1,L)$ and $\displaystyle \frac{\partial^k}{\partial x^k}\frac{\partial^l}{\partial t^l}g_\Phi(x,t)$ is well-defined for $x>x_0(k+l+1,L)$. Under the conditions and the notation of Theorem \ref{thm:mainL}, if $t/x=t(x)/x \downarrow \mathfrak{b}$, then
	\begin{equation}\label{asympS1}
		\begin{split}
	\frac{\partial^k\partial ^l }{\partial x^k\partial t^l} G_{\Phi}\lbrb{x,t}=\frac{\partial^k\partial ^l }{\partial x^k\partial t^l} f_{\Phi}\lbrb{x,t}\frac{1}{\Phi^\dagger(a_*)}\lbrb{1+\bo{\frac{\sqrt{\ln\lbrb{a_*\sqrt{-\Phi''(a_*)x}}} }{a_*\sqrt{-\Phi''(a_*)x}}}}, \quad \mbox{ as }x \to \infty.
		\end{split}
	\end{equation}
	If $\limi{x}t/x=\bar{x}\in\lbrb{\mathfrak{b},\Phi'(0^+)}$, under the conditions of Theorem \ref{thm:main1} we have 
	\begin{equation}\label{asympS}
		\begin{split}
			\frac{\partial^k\partial ^l }{\partial x^k\partial t^l} G_{\Phi}\lbrb{x,t}
			=\frac{\partial^k\partial ^l }{\partial x^k\partial t^l} f_{\Phi}\lbrb{x,t}\frac{1}{\Phi^\dagger(\bar{a})}\lbrb{1+\bo{\ln(x)x^{-\frac12}}}, \quad \mbox{ as }x \to \infty.
		\end{split}
	\end{equation}
...
\begin{coro}\label{cor:mainS}
	Under the conditions of Theorem \ref{thm:mainS}, then \eqref{asympS1} and \eqref{asympS} hold with a factor $q$ for $f^{\rm k}_{\Phi}$. Furthermore, \eqref{asympS1} hold with a factor $\mathfrak{b}a_\ast$ and \eqref{asympS} with a factor $\mathfrak{b}\bar{a}$ for $f^{\rm c}_\Phi$.
\end{coro}

\subsubsection{Comparison to existing results and examples}\label{subsubsec:EX}
The most recent results are Theorems 1-2 of \cite{DonRiv} and Theorem 4.17 of \cite{GrLTr21}. Since \cite{GrLTr21} together with \cite{ChoKim21} develop bounds too and their conditions from Lemma \ref{lem:condi} below are in the setting of \cite{DonRiv}  we do not discuss them at the level of asymptotics. \cite{DonRiv} deals only with densities and offers no estimates on the speed of convergence. When $t(x)/x\downarrow \mathfrak{b}$ it is required that
\begin{equation}\label{condi:DR}
	\begin{split}
		&\liminfi{y}\frac{y^2\Delta(y)}{\bar{\nu}_\Phi(y^{-1})}=\liminfo{y}\frac{\Delta(y^{-1})}{y^2\bar{\nu}_\Phi(y)}>0, 
	\end{split}
\end{equation}
which implies $\Phi(\infty)=\infty$ and resembles the well-known positive increase condition, see e.g. \cite[eq (6)]{MinSav23+}. However, choosing $\nu_{\Phi}(dy)=y^{-1}\ln\lbrb{y^{-1}}\ind{y\in\lbrb{0,1}}dy$ we see that \eqref{condi:DR} is not satisfied whereas  \eqref{def:condiA} and \eqref{def:condiB} hold since at infinity $x\Phi'''(x)\asymp x^{-2}\ln(x)\asymp -\Phi''(x)$ and $x^2\Delta(x)\asymp\ln\lbrb{x}$, as $x \to \infty$.

Before proceeding with a specific example we prove that \eqref{condi:DR} implies \eqref{def:condiA} and \eqref{def:condiB}. For this purpose we need a technical lemma.
\begin{lem}\label{lem:phi''}
	Let $\phi$ be a Bernstein function. Then, for some absolute constant $C$ it holds that
	\begin{equation}\label{eq:phi''}
		\begin{split}
			& e^{-1}\Delta(x)\leq-\phi''(x)\leq \Delta(x)+\frac{C}{x^2}\bar{\mu}_\phi\lbrb{\frac{1}{x}}.
		\end{split}
	\end{equation}
...
\begin{proof}
	...
	and the lower bound in \eqref{eq:phi''} follows. The upper bound follows from
	\begin{equation*}
		\begin{split}
			-\phi''(x)	&\leq \int_{0}^{\frac{1}{x}}y^2\mu_{\Phi}(dy)+\sum_{n\geq 1}\int_{\frac{n}{x}}^{\frac{n+1}{x}}e^{-xy}y^2\mu_{\Phi}(dy)\leq \Delta(x)+\frac{1}{x^2}\sum_{n\geq 1}(n+1)^2e^{-n}\int_{\frac{n}{x}}^{\frac{n+1}{x}}\mu_{\Phi}(dy)\\
			&\leq \Delta(x)+\frac{C}{x^2} \bar{\mu}_\phi\lbrb{\frac{1}{x}}.
		\end{split}
	\end{equation*}
\end{proof}
\begin{lem}\label{lem:condi}
	...
	\end{equation} 
then \eqref{def:condiB} holds true. Subsequently, condition \eqref{condi:DR} implies  \eqref{def:condiB} and \eqref{def:condiA} with $L=\infty$.
\end{lem}
\begin{proof}
	From Item \ref{it:phi'} of Lemma \ref{lem:Bern} we have that $\phi'''$ is positive and non-increasing and therefore
	\[-\phi''(x)\geq \int_x^{2x}\phi'''(y)dy\geq x\phi'''(2x).\]
	Hence \eqref{def:condiB''} implies \eqref{def:condiB}. Let us show that \eqref{condi:DR} triggers \eqref{def:condiB''}. From \eqref{eq:phi''} we get at infinity
	...
	\begin{equation}\label{eq:12}
		...
		\end{split}
	\end{equation}
and we arrive for small $x$ at
\begin{equation}\label{eq:relation}
	\begin{split}
		&\int_0^x w\bar{\mu}_\phi(w)dw\asymp-\phi''(x^{-1})\asymp \Delta(x^{-1}).
	\end{split}
\end{equation}
However, from \textbf{[SC']} at page 8 of \cite{DonRiv} we have, for any $\lambda>1$, some $c\geq 1$ and $\alpha\in\lbrbb{0,2}$,  that
\begin{equation*}
	\begin{split}
...
\[-\phi''(y)\asymp -\phi''(2y)\]
and \eqref{def:condiB''}, \eqref{def:condiB} follow.
From \cite[Definition 2.0.6]{BGT87} and \eqref{eq:relation} it follows that $-\phi''(x)$ is O-regularly varying function at infinity, see \cite[Corollary 2.0.5]{BGT87}. Then from \cite[2.1.9 of Theorem 2.1.8]{BGT87} it follows that the lower Matuszewska index  of $-\phi''(x)$ is larger or equal to $\alpha-2>-2$ and the upper index is clearly $0$. Therefore, $-\phi''(x)$ is of bounded decrease as in \cite[p.71]{BGT87} and from \cite[Proposition 2.2.1]{BGT87} we get that $-\phi''(x)\geq x^{-2+\alpha+\epsilon}$ for all $0<\epsilon<2-\alpha$  and $x$ large enough. Therefore, from \eqref{eq:relation} we obtain that
\[\Delta\lbrb{x}\geq Cx^{-2+\alpha+\epsilon}\]
for some $C>0$ and all $x$ large enough and clearly \eqref{def:condiA} holds with $L=\infty$.
\end{proof}
...
%\ga{Let us also underline that \eqref{def:condiA} and \eqref{def:condiB} are verified if $\Phi$ is a Bernstein function that is regularly varying at infinity with index $\alpha>0$. Indeed, on the one hand \eqref{def:condiB} follows by a direct application of the monotone density theorem \cite[Theorem 1.7.2]{bingham}. On the other hand, \eqref{def:condiA} follows with $L=\infty$ once we notice that $\mu_\Phi(dy)=m(y)dy$ where $m$ is regularly varying at $0$ with index $-\alpha-1$ (see \cite[Proposition 5.23]{pottheory}). Actually, \eqref{def:condiA} is verified even if $\Phi$ is a Bernstein function such that $\Phi(x)\asymp x^{\alpha}\mathfrak{l}(x)$ as $x \to \infty$ for some slowly varying function $\mathfrak{l}$, as a consequence of \cite[Theorem 13.2.10]{ksv12}.}
	...
	Let us provide an example. Take $q=\mathfrak{b}=0$ and a L\'evy measure with a compactly supported density that is regularly varying at $0$ with index $-1-\alpha$ for some $\alpha \in (0,1)$, as for instance
...
\[x^2\Delta(x)=x^2\int_{0}^{\frac1x}y^2 m(y)dy\simi Cx^\alpha\]
and  \eqref{def:condiA} holds with $L=\infty$. It is clear that, actually, assumptions \eqref{def:condiA} and \eqref{def:condiB} hold for any Bernstein function $\Phi$ that is regularly varying at $\infty$ with index $\alpha \in (0,1)$.

Going back to the example, let $t(x)/x\downarrow 0$. Using $-\Phi''(x)\sim C_2x^{\alpha-2}$, as $x \to \infty$, we establish, for any $k,l\geq 0$, 
\begin{equation}\label{asymp1ex}
	\begin{split}
		& \frac{\partial^k\partial ^l }{\partial x^k\partial t^l}\fP{x,t}=\frac{(-1)^kC^{k+1}_0}{\sqrt{2\pi C_2}}e^{a_*t-x\Phi(a_*)}\frac{a^{k+l+1-\frac{\alpha}{2}}_*}{\sqrt{x}}\lbrb{1+\bo{\frac{\ln\lbrb{a_*x} }{a^{\frac{\alpha}{2}}_*\sqrt{x}}}}, \quad \mbox{ as }x \to \infty,
	\end{split}
\end{equation}
where we have used Theorem \ref{thm:mainL}  with $\Phi(y)=\phi^{\dagger}(y)\sim C_0 y^{\alpha}$ as $y \to \infty$. Finally, employing that $\Phi'(y)\sim C_1y^{\alpha-1}$ as $y \to \infty$ we get from \eqref{eq:a*1} that 
\[C_1a^{\alpha-1}_*\sim \frac{t}{x} \quad \mbox{ as }x \to \infty \iff a_* \sim \lbrb{\frac{1}{C_1}\frac{t}{x}}^{\frac{1}{\alpha-1}}, \quad \mbox{ as }x \to \infty,\]
and we can plug in this expression above. The speed of convergence is of the form $(t/x)^{\alpha/(2(1-\alpha))}x^{-\frac12}$ which offers faster decay when $\alpha$ approaches $1$. We can choose $t=t(x)$ as we wish as long as $t/x\to 0$.

When $t(x)/x\to \bar{x}\in\lbrb{\mathfrak{b},\infty}$ we are in the setting of Theorem \ref{thm:main1} whose condition is only \eqref{def:condiA}. We have discussed the relation between \eqref{def:condiA} and \eqref{condi:DR} in \cite{DonRiv}. We emphasize that there they have some additional hypothesis called (G) and (H). Condition  (H) is more stringent than ours at least in the case when $L$ is finite since then the integral in \cite{DonRiv} may not be finite. To illustrate our results in the example of regular variation above take $t(x)/x=1$, $\phi'(\bar{a})=1$ and note that from \eqref{def:fin} we deduce that
\begin{equation*}
	\begin{split}
		&\frac{\partial^k\partial ^l }{\partial x^k\partial t^l}\fP{x,t}=\frac{(-1)^k\Phi^\dagger(\bar{a})\Phi^{k}(\bar{a})}{\sqrt{2\pi}\bar{a}^{1-l}\sqrt{-\Phi''(\bar{a})}}\frac{e^{x\lbrb{\bar{a}-\Phi(\bar{a})}}}{\sqrt{x}} \lbrb{1+\bo{\ln(x)x^{-\frac12}}}, \quad \mbox{ as }x \to \infty,
	\end{split}
\end{equation*}
and the dependence on $x$ is explicit. Since $x\phi'(x)\leq \phi(x)$, see \eqref{it:ineq} of Lemma \ref{lem:Bern}, we conclude that $\bar{a}-\phi\lbrb{\bar{a}}\leq 0$ as required for the asymptotic above. 

Consider the case when $\limi{x}t(x)/x=\infty=\phi'(0^+)$. Then as above $a_*\sim C (t/x)^{-\frac{1}{1-\alpha}}$  and $x=\so{t(x)}$, as $x \to \infty$, thus
\[-a^2_*\phi''(a_*)x\sim Ca^{\alpha}_*x\sim C't^{-\frac{\alpha}{1-\alpha}}x^{\frac{1}{1-\alpha}}, \quad \mbox{ as }x \to \infty,\]
which goes to infinity if $t=\so{x^{\frac{1}{\alpha}}}$ and the first requirement of \eqref{eq:addCondi} is fulfilled. Also, for any $\delta>0$,
\[\limi{x}e^{-\delta x}t^{-\frac{\alpha}{1-\alpha}}x^{\frac{1}{1-\alpha}}=0,\]
for $t=\so{x^{\frac{1}{\alpha}}}$, as $x \to \infty$, and the third condition in \eqref{eq:addCondi} holds. Under the same restrictions $\ln(1/a_*)$ is at most of logarithmic growth and the second imposition of \eqref{eq:addCondi} is valid.
 ...
 where $\log_2x=\log\log x, c>0$, see \cite[Chapter III]{bertoinb}. Set $t=c\frac{x^2}{\log_2x}$. Clearly, $t/x\to \infty$ and from above \eqref{def:condiA},\eqref{def:condiB} and \eqref{def:condiB'} hold. Also, 
 \[\phi'(a_*)=\frac{t}{x} \Rightarrow a_*\sim C\frac{\log^2_2 x}{x^2}, \quad \mbox{ as }x \to \infty.\]
 Furthermore, as $x \to \infty$,
 \[-xa_*\phi''(a_*)\sim Cxa^{-\frac12}_*\sim C\log_2(x)\to \infty \]
 and the first requirement of \eqref{eq:addCondi} is fulfilled. The second and the third impositions of \eqref{eq:addCondi} are then obvious and Theorem \ref{thm:mainS} holds true and by virtue of its claims it yields local estimates for the densities and all derivatives of the probabilities above.
 
Assume that  $\abs{\phi''(0^+)}<\infty$ and hence $\phi'(0^+)<\infty$. Let us determine the speed by which $t/x$ may approach $\phi'(0^+)$. First, we note that since $a_*\to 0$ then, as $x \to 0$,
\[-a_*\phi''(0^+)\sim \phi'(0)-\phi'(a_*)=\phi'(0)-\frac{t}{x}.\]
From Remark \ref{rem:main2} we have to ensure that  $\limi{x}a_*\sqrt{x}=\infty$. Hence, from the last relation we must have
\[\limi{x}\frac{\sqrt{x}}{x\phi'(0)-t}=0.\]
...
\[\Pbb{\sigma(x)\in\lbbrbb{a,b}}=\int_a^b g(x,t)dt.\]

The works \cite{ChoKim21,GrLTr21} develop upper and lower bounds for $g_\phi$. The most general condition seems to be in \cite[Theorem 3.3]{GrLTr21} which assumes that  $-x^{2-\alpha}\phi''(x), \alpha<2,$ is eventually almost increasing, see \cite[page 6]{GrLTr21}. The conditions on $\phi$ in their main Theorem 4.17 lead to the aforementioned one. From \cite[Lemma 2.8]{GrLTr21} it follows that \eqref{eq:relation} holds and $\liminfi{x}x^{2-\alpha}\Delta(x)>0$. Therefore, \eqref{def:condiA} is true with $L=\infty$. Also \eqref{def:condiB''} and hence \eqref{def:condiB} are valid. This could be seen from \cite[Prop 2.3 and Prop. 2.4 with $b=0$]{GrLTr21} which together with item \ref{it:ineq} of Lemma \ref{lem:Bern} show that $-x^2\phi''(x)\asymp \phi(x)$ which gives \eqref{def:condiB''} using the subadditivity of $\phi$. Henceforth, we deduce that our main conditions are milder. Then upper bounds for $g_\phi$ and $f_\phi$ can be obtained by tracking the constants and terms in the application of the saddle point method for the proofs of our main results. This can be done explicitly as the inversion is done for set of values of $t$ and $x$. The lower bounds will follow the same route by separating out the main term and then subtracting the contribution of the negligible. The latter may worsen the estimates but it will provide increasingly (with the growth of $t,x$) tighter bounds.


\subsection{A power series representation of densities and their derivatives and their behaviour at zero}
...
%		&	\int_1^{+\infty} s^{-1} \bar{\nu}_{\Phi}(s) ds \, < \, +\infty. 	\label{eq:integrlog} \tag{$\mathbb{A}_6$}
	\end{align}
We remark that \eqref{eq:extensionA3} and \eqref{eq:uniformlimcond} are very general. They are satisfied by a wide class of Bernstein functions (see the discussion in Section \ref{discussionassumptions} below). Upon the validity of \eqref{eq:extensionA3} and \eqref{eq:uniformlimcond} we have the following regularity result.
...
\end{thm}
Theorem \ref{thm:smoothfmu} is proved in Section \ref{sec:LT}. Under its conditions the  following  power series representation holds.
...
\begin{rmk}\label{rmk:seriesexp}
	Note that the series \eqref{eq:seriesder} which contains the tail, its convolutions and their derivatives is similar to the series expansion for the potential density of a subordinator with drift, see \cite{DoeSav_10}.
\end{rmk}
\begin{rmk}
...
\begin{prop}\label{prop:extcont}
		...
		\end{equation*}
		Then, for any $\varepsilon>0$ and any $t>0$, denoting by $\gamma_\varepsilon$ the parametrized curve $\gamma_\varepsilon: z=\varepsilon e^{i\xi}$ for $\xi \in [-\pi,\pi]$,
