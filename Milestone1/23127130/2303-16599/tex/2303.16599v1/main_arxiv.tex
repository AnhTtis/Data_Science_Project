...
\newcommand{\WC}{\textcolor{red}}

\newcommand{\WCC}{\textcolor{orange}}
% \newcommand{\WC}{\todo[color=red!40]}
\newcommand{\LJ}{\textcolor{blue}}
\newcommand{\anno}{\textcolor{ashgrey}} 
\newcommand{\nb}{\lf nb_n \rf}
%previous comment
\newcommand{\checkit}{\textcolor{orange}} 
...
\maketitle
\begin{abstract}
  Long-run covariance matrix estimation is the building block of time series inference problems. The corresponding difference-based estimator,
...
improving  the performance via the residual-free formula of the proposed estimator.  The effectiveness of the proposed method  is justified theoretically and demonstrated by superior performance in simulation studies, while its usefulness is elaborated by means of real data analysis.
\end{abstract}
\textbf{Keywords}:
debias; difference-based; locally stationary;  long-run variance; monotonic power;  time-varying linear model; 
...
The long-run variance plays a central role in the statistical inference of time series linear models.  Consider the following functional linear model (\cite{zhou2010simultaneous}) for the time series observations $(y_{i,n},x_{i,n})$, $1\leq i\leq n$,
\begin{align}
    y_{i,n} =   x^{\top}_{i,n} \beta_{i,n} + e_{i,n},\quad \beta_{i,n}=\beta(i/n), \quad 1 \leq i \leq n, \label{eq:tvlinear}
\end{align}
where $ x_{i,n}$ is a $p$-dimensional covariate process whose first element is $1$ and $e_{i,n}$ is the error process, both of which can be non-stationary, $(\beta_{i,n})$ are the time-varying regression coefficients and $\beta(\cdot):[0,1]\to \mathbb R^p$ is the regression function. If  $\hat \beta_{i,n}$ is assumed to be constant or smoothly changing,  it can be estimated by ordinary least squares  or nonparametric methods (see \cite{chan2010local}, \cite{li2011non}, \cite{su2019}) 
...
Classic estimation of the long-run covariance matrix in linear models, see for instances \cite{newey1987} and \cite{andrews1991heteroskedasticity},  
...
which is difficult to achieve under structural changes. A prevalent approach to overcome the obstacle is the difference-based estimation, which is built on $y_{i+m,n}-y_{i,n}$ where $m$ is a diverging tuning parameter such that $m=o(n)$ so that most $y_{i+m,n}-y_{i,n}$ are approximately zero-mean under both smooth and abrupt structural changes. Most of the existing results assume stationarity, see for example, 
 \cite{gomez2017sjs} and \cite{chan2021optimal}. Exceptions include \cite{dette2019detecting},  which allows non-stationary errors. To the best of the author's knowledge, the existing difference-based long-run variance estimators for time series data are designed for $p=1$. When $(e_{i,n})$ are $i.i.d.$,  the difference-based estimators for variance has been studied by for example \cite{muller1987estimation}, \cite{hall1990bio} and \cite{brown2007aos}.
% \WC{Lj  add some citaiton. I think there is an AOS from a UPenn big prof. }. 

For $p>1$, the long-run variance is referred as the long-run covariance matrix, whose estimation is much more involved, see \cite{jansson_2002}, \cite{HIRUKAWA2021} and reviews therein. %but cannot be avoided in contrast to the stationary case ( see \cite{shao2015self} and review therein).
% \WC{Lj: please check the reference. I think \cite{chan2021optimal},\cite{dette2019detecting},\cite{BeranLongmemory} are for $p=1$. We should focus on $p>1$.}
Most existing methods are plug-in methods that utilize estimated residuals  which have been widely applied to the goodness of fit tests, tests of structural breaks (\cite{aue2013}, \cite{wu2018gradient}, \cite{kao2018}), detecting gradual changes (\cite{vogt2015detecting}), simultaneous confidence bands for coefficient functions (\cite{zhou2010simultaneous}), tests for long memory (\cite{BeranLongmemory}, \cite{bai2021}), etc. Since they crucially depend on the pre-estimation of regression coefficients, they are inconsistent under abrupt structural breaks, causing the notorious non-monotonic power when applied to structural stability tests. Moreover,  most nonparametric specification tests involve both $\hat \beta_{i,n}$ and the  long-run covariance matrix,  while  $\hat \beta_{i,n}$ is also used in the formula of the plug-in estimator for the long-run covariance matrix. Therefore, the plug-in estimate tends to sensitize those tests to the tuning parameters chosen for $\hat \beta_{i,n}$. 

In this paper, we develop a new difference-based estimator of the time-varying long-run covariance matrix to overcome the aforementioned difficulties for functional linear models with non-stationary dependent errors. Our estimator has an extra debiased term compared to their univariate counterpart because the extension of the latter estimator to linear models with $p>1$ is asymptotically biased. We derive the consistency of the new estimator under time series nonstationarity allowing flexible predictor-error relationships in Section \ref{sec:theory}. In particular, our difference-based estimator achieves faster convergence rates under certain moment conditions than that of the plug-in estimator in \cite{zhou2010simultaneous}.  We then apply the new estimator to the structural stability tests and the long-memory tests, studying the theoretical and numerical performance in Section \ref{sec:app} and \ref{sec:sim}, respectively. An empirical illustration using public health data can be found in Section \ref{sec:data}. %demonstrate the usefulness of our approach by means of real data analysis.
...
In this paper, we consider a general form of non-stationary called local stationarity for the covariate and the error processes of \eqref{eq:tvlinear}, which has received  substantial attention in the literature.  We employ the definition of locally stationary processes  based on Bernoulli shift processes, see \cite{wu2005nonlinear} and 
\cite{zhou2010simultaneous}, while there are also many other formulations, see \cite{dahlhaus1997}, \cite{nason2000wavelet}, and 
 \cite{dahlhaus2019bej} for a comprehensive review. \par We start by introducing necessary notation that will be used in the rest of the paper.
Define $\lf a \rf$ as the largest integer smaller than $a$, and $\lambda_{\min}(A)$ as the smallest eigenvalue of any symmetric squared matrix $A$. Let $|\cdot|$ denote the absolute value for scalars and the Frobenius norm for vectors. Let  $\F_i = (\cdots, \eta_{i-1}, \eta_i)$, where $(\eta_i)_{i \in \mathbb Z}$ are $i.i.d.$ random elements, and the couple process $\F_{i,\{0\}}=(\cdots,\eta_{-1},\eta_{0}^{\prime},\eta_{1},\cdots,\eta_{i})$, where $(\eta_{i}^{\prime})_{i\in \mathbb Z}$ is the $i.i.d.$ copy of $(\eta_{i})_{i\in \mathbb Z}$. We write $f(\cdot) \in C^q[0,1]$ if $f(\cdot)$ has $q$-th order continuous derivative.  Consider the data generating mechanism $L(t,\F_i) \in R^{p}$, where $L$ is a filter function such that $L(t, \F_i)$ is well-defined. We say $L(t,\F_0) \in \mathrm{Lip}_s(I)$ if there exists a constant $c > 0$ such that for any $t_1, t_2 \in I, t_1 < t_2$,
\begin{align}
    \|L(t_1,\F_0)-L(t_2,\F_0) \|_s  \leq c|t_1-t_2|,
\end{align}
where $\|X\|_s = (\E|X|^s)^{1/s}$. To measure the dependence of time series,  we adopt the physical dependence (see \cite{wu2005nonlinear}).  For  $L(t,\F_i)$ on interval $I$, the physical dependence in $\mathcal L^r$ norm is defined by
...
The following definition gives the definition of locally stationarity and short-range dependence. 
\begin{definition}
The process $G(t, \F_i)$ is of $r$-order $(r\geq 1)$ short-range dependence on interval $I$ if $\sup_{t \in I} \| G(t, \F_0) \|_{r} <\infty $, $\delta_{r}(H, k, I) = O(\chi^k)$,  and $s$-order ($s \geq 2$) locally stationary on interval $I$, if
 $G(t, \F_0) \in \mathrm{Lip}_{s}(I)$.
\end{definition}
In the functional linear model \eqref{eq:tvlinear}, we consider  $ x_{i,n}= W(t_i, \F_i)$, $e_{i,n}=H(t_i, \F_i)$, where  $t_i=i/n$ , $W$ and $H$ are measurable nonlinear filters mapping from $[0, 1]\times \mathbb R^{\mathbb Z}$ to $\mathbb R^p$ and $(-\infty, 1]\times \mathbb R^{\mathbb Z}$ to $\mathbb R$, respectively. Permitting $i$ to approach $-\infty$ enables us to develop our proposed method under long memory, see \cref{sec:app2} for detailed discussion. We further assume $e_{i,n}$ has the following state-heteroscedastic form, i.e., 
...
admits the heteroscedastic errors considered in \cite{lackoffit} and \cite{kulik2012conditional}, where they assume $\tilde H(t_i, \mathcal H_i)$ to have the form of  $s( x_{i,n})$ for some unknown smooth function $s(\cdot)$. Moreover, $\tilde G(t_i,\mathcal G_i)$ allows the conditional heteroscedasticity  as considered by assumption 1(b) of \cite{cavaliere2017quasi}. 
 Further, let $ {U}\left(t_i, \mathcal{F}_{i}\right) = W(t_i, \F_i)H(t_i, \F_i) $ such that $x_{i,n}e_{i,n}= {U}\left(t_i, \mathcal{F}_{i}\right)$. The (time-varying) long-run covariance function for the functional linear model \eqref{eq:tvlinear} is defined by
\begin{align}
 {  \Sigma}(t):=\sum_{j=-\infty}^{\infty} \operatorname{Cov}\left\{{U}\left(t, \mathcal{F}_{0}\right), {U}\left(t, \mathcal{F}_{j}\right)\right\}.\label{eq:long-run variance}
\end{align}
% which can be shown to be equivalent to the definition in \eqref{eq:long-run variance0}, see Lemma 3- Lemma 5 of \cite{zhou2010simultaneous}.  
For stationary $x_{i,n}$ and $e_{i,n}$, $\Sigma(t)$ will be time invariant. In this case for linear models with no structural changes, \cite{zhoushao2013} proposes a self-normalization method for statistical inference so that the estimation of the long-run covariance matrix can be avoided. However, their method relies crucially on strict stationarity. Therefore, for the statistical inference of non-stationary time series linear models, the accurate and robust estimation of  $\Sigma(t)$ is essential. 


%is a crucial ingredient of inference problems of \eqref{eq:tvlinear} such as testing goodness of fit, change point tests (\cite{wu2018gradient}, \cite{chan2021optimal}), simultaneous confidence bands for coefficient functions (\cite{zhou2010simultaneous}), tests for long memory (\cite{BeranLongmemory}), mass excess tests of relevant mean changes (\cite{dette2019detecting}), etc.  A common approach to estimate $  \Sigma(u)$ is the  the plug-in estimator utilizing estimated residuals $\hat e_{i,n}'s$ (see for instance \cite{zhou2010simultaneous} \cite{vogt2015detecting}, \cite{wu2018gradient}). However, we find through our extensive numerical studies that the tests using the plug-in estimator 
...
%The difference-based estimator, which circumvents the pre-estimation of  complex trends, has attracted considerable interest due to its  robustness to both smooth and abrupt structural breaks and outstanding finite sample performance for the stationary series  (see  \cite{muller1987estimation}, \cite{hall1990bio}, \cite{vogt2012} \cite{gomez2017sjs}, \cite{zhou2010simultaneous}, \cite{dette2019detecting}, and  \cite{chan2021optimal}).  However,
...
%To bridge this gap,
 %we propose a debiased difference-based estimator for the long-run covariance matrix, which is the first consistent difference-based estimator for the long-run covariance matrix in regression models with locally stationary  time series covariates and  errors. Our difference-based estimator achieves faster convergence rates under certain moment conditions compared to the plug-in estimator in \cite{zhou2010simultaneous},
...
%high largely alleviating the non-monotonic power problem in the change point tests (see \cite{JUHL200914}, and \cite{wu2018gradient} ).

% As emphasized in \cite{chan2021optimal}, 
...
%  \cite{muller1987estimation}, \cite{hall1990bio}, \cite{gomez2017sjs} and  \cite{dette2019detecting} where the estimation of a time-varying trend or change points is avoided. 
...
%  see for instance \cite{zhou2010simultaneous} and   \cite{vogt2015detecting}. Such plug-in estimate will yield test results that are sensitive to the choices of the smoothing parameter $b_n$ under null, and even more sensitive under alternative hypothesis.
...
% The difference-based variance and long-run variance estimators have attracted considerable research attention; see for example \cite{muller1987estimation}, \cite{hall1990bio}, \cite{dette2019detecting}, and \cite{chan2021optimal} among others.
% \WC{Cite the latest AOS paper?}\edit{done. It hasn't been published yet}. %The difference-based variance and long-run variance estimators have been investigated by \cite{muller1987estimation}, \cite{hall1990bio}, \cite{gomez2017sjs} and  \cite{dette2019detecting} among others to circumvent the need for the pre-estimation and to achieve faster convergence rate.
...
% Difference-based estimators have been successful in removing deterministic trend without pre-estimation, see for instance
 %\cite{muller1987estimation}, \cite{hall1990bio}, %\cite{gomez2017sjs}
 
%We present the difference-based long-run covariance estimator for $p\geq 2$. For $p=1$ we recommend the difference statistic proposed by \cite{dette2019detecting} for $\sigma^2_H(t)$ which is built on the difference of $y_{i,n}$. %Nevertheless, the estimation of ${  \Sigma}(t)$ is much more involved since $(  x_{i,n} e_{i,n})$ is not directly observable and its magnitude of stochastic order is different under null and the alternatives.
%For $p \geq 2$, a direct extension of the estimator in 
When $p=1$,
\cite{dette2019detecting}   proposes the following   difference-based estimator $\acute{{  \Sigma}}(t)$ based on the difference of $  x_{i,n}y_{i,n}$ for the long run variance of $e_{i,n}$. Let $  Q_{k, m}=\sum_{i=k}^{k+m-1}   x_{i,n}y_{i,n}$, and  for $t \in [m/n,1-m/n]$, 
\begin{align}
...
where for some bandwidth $\tau_n$ and the kernel function $K(\cdot)$ with support $(-1,1)$,
     \begin{align}
      \omega(t, i)= K_{\tau_n}\left(t_i-t\right) / \sum_{i=1}^{n} K_{\tau_{n}} \left(t_i-t\right).
     \end{align}
For $t \in [0,m/n)$,  $\acute{{  \Sigma}}(t) = \acute{{  \Sigma}}(m/n)$ and for $t \in (1-m/n,1]$, $\acute{{  \Sigma}}(t) = \acute{{  \Sigma}}(1-m/n)$. \par
Unfortunately, in \cref{proof:thm7.1}, we find that the difference-based estimator $\acute {  \Sigma}(t)$ is asymptotically biased for $p\ge 2$, %Hence, bias correction for $\acute{{  \Sigma}}(t)$ is required and it is desired to use a $b_n$-free statistic so that \cref{algorithm}  will be less sensitive to the smoothing parameter $b_n$ and hence more stable.
where we also show that $\E (\acute {  \Sigma}(t))-  \Sigma(t)$ can be uniformly approximated by the expectation of the following $ {\Sigma}^A(t)$:
     \begin{align}
         \Sigma^A (t) = \sum_{j=m}^{n-m} \frac{m   A_{j,m}   A_{j,m}^{\T}}{2}\omega(t, j),~\text{for}~t \in [m/n,1-m/n],\nonumber
    \end{align}
where for $j \geq m$,
...
define
    ...
    \end{align}
      Since $\E \tilde{  E}_{j,m} = 0$, the  random vector $\surd{m} \tilde{  E}_{j,m}$ is  $O_p(1)$.  Treating $\sum_{i=j-m+1}^j \tilde{  Y}_{i,m}/\surd{m}$ as the response variable, $ \sum_{i=j-m+1}^j \tilde{  X}_{i,m}/\surd{m} $ as covariates and  $\surd m\tilde{  E}_{j,m}$ as errors, \eqref{Motivation} motives us to approximate $  \beta(t)$ in $  A_{j,m}$ by 
    \begin{align}
        \breve {  \beta}(t) =   \Omega^{-1}(t)  \varpi (t),\label{eq:brevebeta}
    \end{align}
    where $ \Omega(t)$ and $  \varpi (t)$ are the smoothed versions of $\acute{  \Delta}_{j}/2 := \frac{1}{2m}\sum_{i=j-m+1}^{j} \tilde{  X}_{i,m}\tilde{  X}_{i,m}^{\T}$, and $\breve {  \Delta}_{j}/2 := \frac{1}{2m}\sum_{i=j-m+1}^{j} \tilde{  X}_{i,m}^{\T}\tilde{  Y}_{i,m}$, i.e., for $t \in [m/n, 1-m/n]$,
    \begin{align}
...
and the corresponding debiased difference-based estimator $\hat {  \Sigma}(t)$ for $t \in [0,1]$: 
     \begin{align}
         \hat {  \Sigma}(t) = \acute {  \Sigma}(t) - \breve {  \Sigma}(t), ~~~ \text{where}~~\breve {  \Sigma}(t)= \sum_{j=m}^{n-m} \frac{m\hat{  A}_{j,m} \hat{  A}_{j,m}^{\T}}{2}\omega(t, j)\label{eq:diff_correct},
     \end{align}
 which is robust to structural breaks in regression coefficient functions due to differencing, and except for $m$, $\tau$ used for $\acute{\Sigma}(t)$, the correction $\breve \Sigma(t)$ does not involve additional tuning parameters.    %We emphasize that $\acute{\Sigma}(t)$, $\breve{\Sigma}(t)$ and $\hat \Sigma(t)$ use the same tuning parameters so that $\hat \Sigma(t)$ enjoys the similar advantages to the univariate difference-based long-run variance estimator in \cite{dette2019detecting} (namely $\acute{\Sigma}(t)$ with $p=1$).
 Thanks to its residual-free formula, we can obtain more accurate type I error rates for the nonparametric specification tests by using  $\hat \Sigma(t)$, see Section \ref{sec:theory}. %\WC{Why is $\breve{\beta}(t)$ robust to abrupt changes}\LJ{Not robust to abrupt changes (unless in the intercept)}
...
%    \label{A:K}\\
%    (C2)  
    Each coordinate of $  \beta(\cdot)$ lies in $C^3[0,1]$.\label{A:beta}
%  \end{enumerate}
  \label{A:nonpar}
 \end{assumption}
...
% and
  The value $\lambda_{\min}(\Sigma(t))$ is bounded from $0$ uniformly on $[0,1]$, and each element of ${  \Sigma}(t) \in  C^2[0,1]$.
%     \label{A:U_long-run variance}
%\end{enumerate}
\end{assumption}
Assumption \ref{Ass-U} guarantees the non-degeneracy of the long-run covariance matrix of the process of $ U(t,  \F_i)$ and each component of ${  \Sigma}(t)$ is smooth,  which are common in the analysis of functional linear models of locally stationary time series, see for example \cite{zhou2010simultaneous}.   Let $ J(t, \F_0) =  W(t, \F_0) W^{\T}(t, \F_0)$.
Define $\mu_W (\cdot) = \mathbb{E}(W(t, \F_0))$, $M(t) := \mathbb{E} (J(t, \F_0))$, $t \in [0,1]$. 
\begin{assumption}\label{Ass-W}   
 The functions
 {$ M(\cdot) \in C^1[0,1]$, $ \mu_W (\cdot) \in C^1[0,1]$, and $\inf_{t \in [0,1]} M(t) > 0$}. The covariate process $W(t, \F_i)$ is of $4$-order locally stationarity and $16\kappa$-order short-range dependence on $[0,1]$ for  some $\kappa \geq 1$.\label{E:WW} \label{A:Mt}
...
Assumption \ref{Ass-W} requires that $ M(t)$ is non-degenerate, implies that  $\sup_{t\in [0,1]} \| J(t, \F_0)\|_{8\kappa} < \infty$, $\delta_{8\kappa}( J, k) = O(\chi_1^{k})$ for some $\chi_1\in(0,1)$, and ensures that each element of $( x_{i,n}  x_{i,n}^{\top})$ is $2$-order locally stationary. 
...
The covariance function $\mathrm{Cov} \left({ J}(t, \F_0),{ J}(t, \F_0)\right) \in C^2 [0,1]$ and its smallest eigenvalue is strictly positive on $[0,1]$.
...
\begin{assumption} The process $H(\cdot, \F_i)$ satisfies $16\kappa$-order of  short-range dependence and $4$-order of locally stationarity on $[0, 1]$.
...
\end{assumption}
 ...
 Assumptions \ref{E:WW} and \ref{E:U} ensure that $U(t, \F_i)$ is of $2$-order local stationarity and $8\kappa$-order of short-range dependence.
The existence of  $16\kappa_{th}$ moments of covariates and errors are assumed for technical convenience, and it is satisfied by sub-exponential random variables. We conjecture that it can be relaxed by substantially more involved mathematical arguments, see the simulation study of \cite{bai2021}. 
 Assumption \ref{Ass-E} is mild, and it excludes the scenario in which all of the time series covariates reduce to deterministic smooth trends; in this case, we recommend using the direct multivariate extension of the difference-based long-run covariance estimator of \cite{dette2019detecting}, namely $\acute \Sigma(t)$.
 %The smooth condition \ref{A:U_smooth} is required for in obtaining uniform consistency, see Theorem 4 in \cite{zhou2010simultaneous}.  %Condition \ref{E:U} strengthens condition \ref{A:U} and \ref{A:U_delta}.


% \begin{remark}
% % Notice that (i) of \cref{thm:bootstrapA} assumes \cref{Ass-E} that allows the driving shocks to have sub-exponential tails.
% \cref{Ass-E} allows the driving shocks to have sub-exponential tails.
% Although this assumption requires the existence of high moments for driving shocks, it is made for technical convenience.
% \LJ{Keep? By evidence from extensive simulation studies, we conjecture that under weaker moment assumptions our results in this paper (including long-run variance estimation and algorithms) still hold, see also \cref{heavy} for empirical evidence.}
%   \end{remark}
% \begin{remark}
%  It's worth noting that the moment conditions in condition \ref{E:WW} and \ref{E:U} are assumed for the convenience of proof. In the unreported experiments, the proposed testing procedure is effective even when the error term has heavy tails, for example, it may follow Student-t distribution. \edit{Discuss the moment condition.}
% \end{remark}

Let $\mathcal{I} = [\gamma_n, 1-\gamma_n] \subset (0,1)$, where $\gamma_{n}=\tau_{n}+(m+1) / n$. The following theorem justifies the uniform consistency of  $\hat{  \Sigma}(t)$ over $\mathcal I$. 
% \WC{will double check the following results after checking the proofs}
\begin{theorem}  \label{thm:lrv_diff}
    Under Assumptions \ref{A:nonpar}, \ref{Ass-U},  \ref{Ass-W}, \ref{Ass-E} and \ref{B:H_delta} with constant $\kappa \geq 1$, suppose
    $m = O(n^{1/3})$, $m \to \infty$, $\tau_n \to 0$, $n\tau_n^3 \to \infty$, $m / (n \tau_n^{2}) \to 0$, $\tau_n^{2-1/\kappa}\surd{m} \to 0$, we have
    \begin{equation}
      \sup _{t \in \mathcal{I}}\left|\hat{{ \Sigma}}(t)-{  \Sigma}(t)\right|= \Op\left(\frac{m^{1/2}}{n^{1/2} \tau_n^{1/2+1/\kappa}} + \frac{1}{m}+m^{1/2}\tau_n^{2-1/{\kappa}} \right).\nonumber
      \end{equation}
  \end{theorem}

%\LJ{Hi Weichi, I think the rate here is better than that in \cite{zhou2010simultaneous}.}
%\edit{confirm: 1. attaining better rates 2. a wider range for the tuning parameter selection? Check the my rewriting.}
The above equation and the bandwidth conditions for Theorem \ref{thm:lrv_diff} ensure the uniform consistency of $\hat{\Sigma}(t)$. We compare our results with \cite{zhou2010simultaneous} which proposes a plug-in estimator of long-run covariance matrix estimator using nonparametric residuals. %, i.e, using  nonparametric residuals in the infeasible long-run covariance matrix estimator.
% which has been proposed and investigated in  Theorem 5 of \cite{zhou2010simultaneous}.  
According to Theorem 5 in \cite{zhou2010simultaneous}, %their rate is optimized at $b_n \asymp n^{-1/5}$, $m \asymp n^{1/4}$, $\tau_n \asymp n^{-1/8}$,  depending on the moment conditions. 
their best approximation rate can be  close to but not faster than $n^{-1/4}$.
In contrast, our uniform rate in \cref{thm:lrv_diff} is $n^{-4/15+1/(5\kappa)}$ by
taking $m \asymp n^{4/15}$, $\tau_n \asymp n^{-1/5}$, which is better  when $\kappa$ is sufficiently large. %\LJ{The moment condition of $\kappa$ is due to technical convenience. We conjecture that under weaker moment assumptions our results still hold, see the simulation study of \cite{bai2021} for covariates and errors that have heavier tail.}
% \WC{reminder: conditions are for techinical convenience; simulations in \cite{bai2021} supports the validity of the method with covariates and errors that have much more heavier tail.}
%Moreover, the approximation rate in  the rate in Theorem 5 of \cite{zhou2010simultaneous} involve $b_n$,  Consequently, large and small $b_n$ will deteriorate the accuracy of the long-run variance estimates, which has an impact ont the tunning parameter  the two aspects.
...
%For non-stationary time series nonparametric regression, a common approach for the selection of $b_n$ is the Generalized Cross Validation which selects $b_n$ from a wide range according to certain criterion, see \cref{sel} in the supplement and also Section 4.4 of \cite{zhou2010simultaneous}. 
%Thus, the use of plug-in estimator in \cite{zhou2010simultaneous} will yield a much smaller searching range since both large and small $b_n$ will deteriorate the approximation accuracy, see Theorem 5 there for more details.  

%The discussion there demonstrate that their convergence rate cannot be smaller than $\Op(n^{-1/4})$.  %when $b_n \asymp n^{-1/5}$. 

%Thanks to the difference-based approach, the above rate is better than the rate of Theorem 5 of \cite{zhou2010simultaneous}, which estimated the long-run covariance matrix via plugging in the nonparametric residuals.
%\edit{check the rewriting}
  % Finally, many time series bootstrap procedures utilize $\hat {  \Sigma}^{-1/2}(t)$, see for instance,  \cite{zhou2010simultaneous} and \cite{zhang2012}. However, they only investigated the consistency of $\hat{  \Sigma}(t)$, from which it is, indeed, non-trivial to derive the uniform consistency of $\hat{  \Sigma}^{-1/2}(t)$. As a complement to the previous work, we establish  the uniform consistency of the latter estimator in \cref{lm:davis} of the supplement, which will be used in \cref{algorithm} of \cref{sec:bootstrap} and \cref{algorithms} in the supplement. 
...
% %   \end{align}
%   where $  \beta_{j}(t) \in \mathcal C^3[a_j, a_{j+1}]$, $\sup_{0 \leq j \leq q_n}|  \beta_{j}(a_{j}) -   \beta_j(a_{j}^{-})| < \infty$, and $q_n$ is the number of abrupt changes and is allowed to diverge at the rate of $n\tau_n^3/m_n$. In fact, the robustness to change points is a well-known advantage of difference-based estimator,  see, e.g., \cite{gomez2017sjs} and \cite{chan2021optimal}. 
%   \end{remark}
...
We are interested in the structural changes in the stochastic linear regression model 
\begin{align}
     y_{i,n} = x_{i,n}^{\top}  \beta_{i,n} + e_{i,n}, \quad 1\leq i\leq n,
\end{align}
% \WC{Please change to $\beta_{i,n}$}
where $\{x_{i,n}\}_{i=1}^n$ is the $p$-dimensional covariate process and $\{e_{i,n}\}_{i=1}^n$ is the error process. The test for structural changes  of $(\beta_{i,n})$ considers the hypothesis $H_0: \beta_{1,n}=\beta_{2,n}=...=\beta_{n,n}$. Allowing  general non-stationarity in the covariates and the errors, \cite{wu2018gradient} proposes to use the test statistic 
\begin{align}
    T_n = \max_{1 \leq j \leq n}\left| \sum_{i=1}^j \hat e_{i,n} x_{i,n} / \surd{n} \right|,\quad \text{where} ~ \hat e_{i,n} = y_{i,n} - x_{i,n}^{\top} \hat{ \beta}_n, \label{eq:cpstat}
\end{align}
where $\hat{ \beta}_n = \mathrm{argmin}_{\beta} \sum_{i=1}^n (y_{i,n} - x_{i,n}^{\top}  \beta )^2$.
For the alternative hypothesis,  we consider
% \WC{Lujia, the following has identification problems. Maybe set $g(0)=0$}
\begin{align}
y_{i,n}=  x^{\top}_{i,n} \beta(i/n) + e_{i,n}, \quad  \beta(t) =  \beta +  L_ng(t),\label{eq:alt}
\end{align}
where $\beta, g(t) \in \mathbb R^p$, $g(0) = 0$, $L_n$ is a positive real sequence, $\beta(t)$ has potential abrupt changes, i.e., $  \beta(t)=\sum_{j=0}^{q_n}  b_{j}(t) \mathbb I_{\left\{a_{j} \leq t<a_{j+1}\right\}}$, $0 = a_0 < a_1 < \cdots < a_{q_n} < a_{q_n+1} =1$,
%   \begin{align}
%       \beta_1(t))=\sum_{j=0}^{d} \beta_{1,j}(t) \mathbb I_{\left\{a_{j} \leq t<a_{j+1}\right\}},
%   \end{align}
   $ b_{j}(t) \in \mathcal C^3[a_j, a_{j+1}]$, $\sup_{0 \leq j \leq q_n}| b_{j}(a_{j}) -  b_j(a_{j}^{-})| < \infty$, and $q_n$ is the number of abrupt changes.
   % The robustness to smooth and abrupt changes is a well-known advantage of difference-based estimator,  see, e.g., \cite{gomez2017sjs} and \cite{chan2021optimal}.  
\cite{wu2018gradient} proposes a general bootstrap statistics according to \eqref{eq:ncpdist}, which relies on the residual $\hat e_{i,n}=y_{i,n}-x_{i,n}^{\T}\hat \beta_n$. Their test can be applied to piecewise locally stationary covariates and errors, and is unified for testing structural changes in general $M$ estimation. They show their power approaches $1$ under local alternatives $L_n=n^{-1/2}$ or $L_n=o(1)$, $\surd n L_n\rightarrow \infty$. However,  if $L_n$ does not vanish, $\hat \beta_n$ is not consistent and the power of the test in \cite{wu2018gradient} in this case is not theoretically guaranteed.
If we focus on the locally stationary process which is quite general and on least squares regression which is arguably  the most widely applied M-estimator in practice, we could improve the power of the test for structural changes via our proposed long-run covariance matrix estimator $\hat \Sigma(t)$ and an alternative bootstrap procedure to \cite{wu2018gradient}.
...
$
    \Psi_{i,m}^{(r)} = n^{-1/2}\sum_{j=1}^i \hat { \Sigma}^{1/2}(t_j) R_j^{(r)}$, $\hat{ \Lambda}(i/n) = \sum_{j=1}^i x_{j,n} x_{j,n}^{\top}/n,
$
  $\{ R_j^{(r)}\}_{j=1}^n$ are the $i.i.d.$ standard norm random variables independent of data and are independently generated  in the $r_{th}$ bootstrap iteration, and $ \hat { \Sigma}(t) $ is the proposed difference-based estimator of $ \Sigma(t)$. Let $B$ denote the number of bootstrap iterations.
...
%     \end{align}
% \end{proposition}
% The uniform consistency of the difference-based estimator of \eqref{eq:diff_correct} is guaranteed by \cref{thm:lrv_diff} and an application of \cite{yu2015useful}.
% \WC{delete}Specifically, we can include the method of \WC{I don't think this is correct}\cite{wu2018gradient} as an example by letting $\hat { \Sigma}^{1/2}(t_j) =  \sum_{r = j}^{j + m - 1} \hat e_{r,n} x_{r, n}/\surd{m} $, $q_n = \surd{n-m+1}$.
Let $F_{(1)} \leq F_{(2)} \leq \cdots \leq F_{(B)}$ be the order statistics of $(F_r)$. We reject the structural stability test at the significance level of $\alpha$ if $T_n$ is greater than the $F_{\lfloor (1-\alpha)B \rfloor}$. % \LJ{Maybe we can omit the algorithm} \WC{I think so}
% The algorithm of the structure stability test equipped with the difference-based estimator \eqref{eq:diff_correct} is listed in Algorithm \ref{alg:cp}. 
% \begin{algo}\label{alg:cp}
% Structural change detection
% %\begin{algorithmic}
% \begin{tabbing}
% \qquad \enspace Require:  A consistent estimator of $\hat{ \Sigma}^{1/2}(t)$ and $l_n$\\
% \qquad \enspace Compute the test statistic $T_n$ defined in \eqref{eq:cpstat}.\\
% \qquad \enspace Compute $\hat{ \Lambda}(i/n) = \sum_{j=1}^i x_j x_j^{\top}/n$, $i = 1 \cdots n$.\\
% \qquad \enspace For $r = 1 \cdots B$\\
% \qquad  \qquad Generate $i.i.d.$ standard normal random variables $\{ R_j^{(r)}\}_{j=1}^n$.\\
% \qquad  \qquad  Calculate $F_r$ as defined in \eqref{eq:bootstrap}.\\
%  \qquad \enspace Let $F_{(1)} \leq F_{(2)} \leq \cdots \leq F_{(B)}$ be the order statistics of $F_r$.\\ 
% \qquad \enspace  Reject the structural stability test if $T_n$ is greater than the $F_{\lfloor (1-\alpha)B \rfloor}$.
% \end{tabbing}
% %\end{algorithmic}
% \end{algo}
% Suppose the number of iterations of the bootstrap test is $B$.
% We will reject the structural stability test if $T_n$ is greater than the $(1-\alpha)$ of $(F_r)_{r=1}^B$. 
%\LJ{In Appendix ?, we also  compare tests with the difference-based estimator \eqref{eq:diff_correct}  with the  plug-in estimator of ordinary least squares residuals as in \cite{wu2018gradient}.}
% \WC{Theorems of behavior under alternavie}\
We proceed to relax Assupmtion \ref{A:nonpar} in \cref{thm:lrv_diff} allowing for the possible presence of abrupt changes and discuss the property of the bootstrap procedure.
 Let $\Lambda(t) = \int_0^t M(s) ds$. %The following result ensures the consistency and the robustness of  the difference-based estimator  \eqref{eq:diff_correct} w.r.t. piecewise smooth mean functions.
\begin{theorem}\label{thm:cp}
Assume  $q_n = o(\min\{n\tau_n^3/m, (n\tau_n)^2/m\})$ under the alternative hypothesis \eqref{eq:alt}. Then, under the Assumptions \ref{Ass-U},  \ref{Ass-W}, \ref{Ass-E} and \ref{B:H_delta} with constant $\kappa \geq 1$, and the bandwidth conditions
    $m = O(n^{1/3})$, $m \to \infty$, $\tau_n \to 0$, $n\tau_n^3 \to \infty$, $m / (n \tau_n^{2}) \to 0$, $\tau_n^{2-1/\kappa}\surd{m} \to 0$, we have
  \begin{equation}
      \sup _{t \in \mathcal{I}}|\hat{{ \Sigma}}(t)-{ \Sigma}(t)|= \op\left(1 \right),
      \end{equation}
and as a result there exists a $p$-dimensional zero-mean Gaussian process $Z(t)$ with covariance function $\gamma(t,s) = \int_0^{\min(t,s)}  \Sigma(r)dr$ such that 
 \begin{align}
        F_r \Rightarrow   \sup_{t \in (0,1]}|G(t)| := \sup_{t \in (0,1]}|Z(t) -  \Lambda(t)  \Lambda^{-1}(1) Z(1)|.
    \end{align}
\end{theorem}
\Cref{thm:cp} shows that our difference-based estimator $\hat \Sigma(t)$ is consistent under the alternatives of both smooth and abrupt changes. Taking $m \asymp n^{4/15}$, $\tau_n \asymp n^{-1/5}$, it follows that $\min\{n\tau_n^3/m, (n\tau_n)^2/m\} \asymp n^{2/15}$ which allows the number of abrupt jumps diverges to infinity as $n \to \infty$. The following proposition gives the limiting distribution of the test statistic $T_n$ under the null hypothesis and ensures the monotonic power of the bootstrap procedure under $H_A:  \beta(t) =  \beta + L_n  g(t)$. Define 
\begin{align}
   \Lambda(t) = \int_0^t M(s) ds, \quad \Lambda(s,  g(\cdot)) =\int_0^s M(r)  g(r) dr,
\end{align}
and $F(t, g(\cdot)) =    \Lambda(t, g(\cdot)) -   \Lambda(t) \Lambda^{-1}(1) \Lambda(1,  g(\cdot))$.
...
\cref{thm:lrv_diff} and the null hypothesis of no structural changes, we have
   ...
   $T_n \to \infty$ in probability at the rate $\sqrt{n}L_n$, and
   $$\big|\mathbb P(T_n \geq \hat q_{1-\alpha}) -\mathbb P \big(\sup_{t \in [0,1]} |G(t)+ n^{1/2} L_n F(t,  g(\cdot))| \geq \hat q_{1-\alpha} \big)\big|=o(1),$$
...
\cref{thm:cp} and \cref{prop:stat_alt} guarantee the asymptotic correctness  of the bootstrap procedure with the difference-based estimator and that its asymptotic power approaches $1$ under the fixed alternative. Meanwhile, the bootstrap procedure with the difference-based estimator can detect the local alternatives at the parametric rate $\surd{n}$ in the sense that if $L_n = n^{-1/2}$, $T_n \Rightarrow \sup_{t\in [0,1]} |G(t) + F(t,  g(\cdot))|.$ Notably, 
$F_r$ shares the same limiting distribution as the limiting distribution of $T_n$  under the null hypothesis, while  Proposition B.1 of \cite{wu2018gradient} shows that with the ordinary least squares residuals, the magnitude of $F_r$ using their bootstrap procedure is
$\surd{m}\max(L_n,  \log^2 n /\surd{n}
) \log n$ when $\surd{m} L_n \to \infty$, $L_n \to 0$, implying power loss due to the divergence of $F_r$. Therefore, the bootstrap procedure equipped with the difference-based estimator will be more powerful than that with ordinary least squares residuals and overcome non-monotonic power caused by the inflation of the bootstrap statistics in \cite{wu2018gradient}. In earlier work on the remedy of the non-monotonic power for the test of smooth structural changes, \cite{JUHL200914} proposes to estimate the long-run variance via plugging in the nonparametric residuals  for $p=1$. However, the improvement in the power of their approach does not carry over in the presence of abrupt structure changes.  % However, plugging in nonparametric residuals is not satisfactory in the regression setting for obtaining accurate sizes in finite samples.
...
\begin{align}\label{model1}
    y_{i,n}=x_{i,n}^{\T} \beta(t_i)+(1-\mathcal B)^de_{i,n},%\quad e_{i,n}=(1-\mathcal B)^{d}u_{i,n},
    ~~~~  i=1,2,\cdots n,
\end{align}
...
\cite{bai2021} proposes to test $H_0$ using the jackknife corrected nonparametric residuals. They obtain the local linear estimate of $\beta(\cdot)$, i.e., 
\begin{equation}\label{eq:loclin}
(\hat{{\beta}}_{b_{n}}(t), \hat{{\beta}}_{b_{n}}^{\prime}(t))=\underset{ \eta_{0}, \eta_{1} \in \mathbb{R}^{p}}{\arg \min}\sum_{i=1}^{n}\{y_{i, n}-{x}_{i,n}^{\T}  \eta_{0}- {x}_{i,n}^{\T}   \eta_{1}(t_{i}-t)\}^{2} K_{b_{n}}(t_{i}-t),
\end{equation}
where $K(\cdot)$ is a kernel function with finite support $(-1,1)$, $b_n$ is a bandwidth and $K_{b_n}(\cdot)=K(\cdot/b_n)$. Then, they consider the jackknife  estimator $\tilde{{\beta}}_{b_{n}}(t)=2 \hat{{\beta}}_{b_{n} / \surd{2}}(t)-\hat{{\beta}}_{b_{n}}(t)$ of which the asymptotic bias terms involving $ \beta^{\prime \prime }(\cdot)$ in the formula of $\hat \beta_{b_n}$ and $\hat \beta_{b_n/\surd 2}$ are canceled. Let $K^*(\cdot)$ denote the jackknife equivalent kernel $2 \surd{2} K(\surd{2}x) - K(x)$.  For the sake of simplicity, we write $n^{\prime}$ short for $\lf nb_n \rf$.
Define the nonparametric residuals and their partial sum as $
  \tilde e_{i,n} = y_{i,n} - x_{i,n}^{\T} \tilde{\beta}_{b_n}(t_i)$ and $\tilde S_{r,n} =\sum_{i=n^{\prime}+1}^r \tilde{e}_{i,n}$, $r=n^{\prime}+1,\cdots n-n^{\prime}$, respectively. The KPSS, R/S, V/S and K/S-type test statistics of \cite{bai2021} are\\
...
\cite{bai2021} proposes to implement the above tests via the following bootstrap-assisted procedure. Define $\hat{{M}}(t) = \frac{1}{n \eta_n}\sum_{i=1}^n   {x}_{i,n} {x}_{i,n}^{\T} K_{\eta_n}(t_i - t^*)$,
where $t^* = \max\{\eta_n,\min(t,1-\eta_n)\}$ for some bandwidth  $\eta_n \to 0$, $n\eta_n^2 \to \infty$. Let $\hat{ \Sigma}^*(\cdot)$ be any consistent long-run covariance matrix estimator  satisfying the regularity condition 5.1 in their paper,  and $\hat \sigma^{*2}_{H}(t)=(\hat { \Sigma}^*(t))_{1,1} $.  Generate $B$ $i.i.d.$ copies of $N(0, I_p)$ vectors $V^{(r)}_i=(V^{(r)}_{i,1},...,V^{(r)}_{i,p})^\top$, $1\leq r\leq B$, and for each $r$ calculate 
        \begin{align}
...
%   $ \par
     Let $ \widetilde{ \mathrm{K}}_{n,(1)} \leq \widetilde{ \mathrm{K}}_{n,(2)} \leq \cdots  \leq \widetilde{ \mathrm{K}}_{n,(B)}$ be the ordered statistics of $\widetilde{ \mathrm{K}}_{n}^{(r)}, r = 1,2,\cdots,B$, and $B^* = \max\{r: \widetilde{K}_{n,(r)} \leq K_n\}$. 
...
% \edit{
  ...
  \begin{assumption}\label{assumptionHp}
   Assumption \ref{E:HW} holds over $(-\infty,1]$, $H(t,\F_i)$ is of $2\kappa$-order locally stationarity on $(-\infty, 1]$, and its
...
and \ref{assumptionHp}, assuming $m\tau_n^{3/2}/\log n \to \infty$, $\tau_n \to 0$, $n\tau_n^3 \to \infty$, $m / (n \tau_n^{2}) \to 0$, $\tau_n^{2-1/\kappa}\surd{m} \to 0$, $m = O(n^{1/3})$, $\kappa \geq \max\{\frac{4}{1/2-d},\frac{2}{3d},4\}$, it follows that under $H_A$
\begin{align}
\sup_{t \in \I}\left| m^{-2d}\hat{{ \Sigma}}(t) -\kappa_2(d)\sigma_H^2(t)  \mu_W(t) \mu^{\T}_W(t)\right| = \op(1),\nonumber
\end{align}
...
In the following \cref{lm:Sigma_dn}, we investigate the performance of the estimator $\hat { \Sigma}(t)$ under the local alternatives $d_n=c/ \log n$ for some constant $c>0$. For this purpose, we define the long-run {\it cross} covariance vector between the locally stationary processes $ (U(t,\F_i))$ and $( H(t,\F_j))$.
...
%  \WC{say, Lujia please explain the independence using (a) in page 515 of \cite{zhou2010simultaneous}}\edit{done}, 
...
\begin{theorem}\label{lm:Sigma_dn}
  ...
  \ref{assumptionHp} and \ref{C:s} be satisfied. If $ m \tau_n^{3/2} \to \infty$, $\tau_n \to 0$, $n\tau_n^3 \to \infty$, $m / (n \tau_n^{2}) \to 0$, $\tau_n^{2-1/\kappa} \surd{m}\to 0$, $m = \lf n^{\alpha_1}\rf$, $\alpha_1 \in (0,1/3)$, we have
  %$m = O(n^{1/3})$,
  \begin{align}   
   \sup_{t \in \I}\left|\hat{{ \Sigma}}(t) - \check { \Sigma}(t)\right|   = \op(1).\nonumber
 \end{align}
...
% \WC{Move the following comments to supplement and focus on $p\geq 2$ in the main!}  If $p=1$,  $ \mu_W(t) = 1$, $ \Sigma(t) = s_{UH}(t) =  \sigma_H^2(t)$, then $\check { \Sigma}(t)$ coincides with $e^{2c\alpha_1}\sigma_H^2(t)$, which differs from the limit of $\hat {\Sigma}(t)$ under the null hypothesis, i.e., $\sigma_H^2(t)$, by only a multiplicative factor $e^{2c\alpha_1}$. For $p\geq 2$, the difference between the limits of $\hat { \Sigma}(t)$ under the null hypothesis and local alternatives can be no longer captured by  a multiplicative factor.\par
  \cref{lm:Sigma_d} and \cref{lm:Sigma_dn} lead to the desired limiting distribution of bootstrap statistics under the fixed and local alternatives achieving satisfactory power performance in finite samples, see \cite{bai2021} for theoretical justification and numerical evidence. In finite samples, we demonstrate that long memory tests with difference-based long-run covariance matrix estimates can achieve sizes closer to the nominal level and are more powerful than their counterparts using plug-in estimates, see \cref{sec:sim}.
 
\section{Simulation} \label{sec:sim}
We investigate the finite sample power performance of the tests introduced in \cref{sec:app} and compare the results using difference-based estimator \eqref{eq:diff_correct} with the results equipped with existing long-run variance estimators. Let $(\varepsilon_{l})_{l\in \mathbb Z}, (\zeta_{l})_{l\in \mathbb Z}, (\eta_{l})_{l\in \mathbb Z}$ be $i.i.d.$ $ N(0,1)$, $\vartheta_i = (\eta_i+ \varepsilon_i)/2$,  and consider the filtrations
\begin{align}
    \F_j =( \cdots, \zeta_{j-1}, \zeta_{j}), \quad \mathcal{G}_j = (\cdots, \varepsilon_{j-1}, \varepsilon_{j}),\quad \mathcal{H}_j = (\cdots, \varepsilon_{j}, \eta_{j}),\quad j=-\infty, \cdots, n. \quad \nonumber
\end{align}
...
\end{align}
where the function $m_{i,n}$ includes the following scenarios: \par
CP1: $m_{i,n} = 2\delta \sin(2 \pi t_i)x_{i,n,1} 1(0.5 \leq t_i \leq 1)$.\par
   ...
   CP4: $m_{i,n} = 1.5\delta \sin(2 \pi t) 1(0 \leq t \leq 0.2~\text{or}~0.4 \leq t \leq 0.6~\text{or}~0.8 \leq t \leq 1)$.\par
We conduct our simulation with sample size $300$.
As shown in \cref{fig:cptest}, when there are $4$ change points, the block bootstrap tests based on ordinary least squares residuals (\cite{wu2018gradient}) suffer from low and non-monotonic power that can not approach $1$. By contrast, the newly proposed difference-based long-run covariance matrix estimator enhances the simulated power significantly and addresses the non-monotonic power issue by taking the difference. To further illustrate the role of long-run covariance matrix estimators in bootstrap tests of structural breaks, we investigate the estimation accuracy of both long-run covariance matrix estimators and find that our proposed estimator halves the empirical mean square error in the presence of change points,  see \cref{sec:lrv} of the supplement for extra simulation results. \par
% \LJ{Maybe postpone to the supplement. Estimation accuracy of lrv matrix} \WC{Mention this in the main}
% 
\begin{figure}
    \centering
    \includegraphics[width = 0.9\textwidth]{cpcolor.jpg}
    % \figuresize{0.35}
% \figurebox{15pc}{20pc}{}[cpcolor.jpg]
 \caption{The power performance of gradient-based structural change point tests as $\delta$ increases from $0$ to $1$ with sample size $n = 300$ and simulation times $2000$, under three scenarios CP1(blue), CP2(orange), CP4(red). Dotted curves denote simulated rejection rates of bootstrap tests using blocks of ordinary least squares residuals,  and solid curves denote simulated rejection rates of those equipped with the difference-based long-run covariance matrix estimator.  Left panel: nominal size 0.05; Right panel: nominal size 0.1.}
...
\begin{align}
    y_{i, n}=\beta_{1}(t_i)+\beta_{2}(t_i) x_{i, n}+(1-\B)^de_{i,n}, \quad i=1, \ldots, n,
\label{M1}
\end{align}
 where $\B$ is the lag operator, $\beta_{1}(t)=4 \sin (\pi t) $, $\beta_{2}(t)=4 \exp \{-2 \left(t-0.5\right)^{2}\} $, $x_{i, n}=W(t_i, \mathcal{F}_{i})$, and $ e_{j,n} = H(t_j,\mathcal{F}_j, \mathcal{G}_j)$, where $$
    H(t,\mathcal{F}_{i},\mathcal{G}_{i}) =  B\left(t,\mathcal{G}_{i}\right)(1+W^2(t, \mathcal{F}_{i}))^{1/2},$$ where $
...
$ and $B(t, \mathcal{G}_{i})=(0.3 - 0.4(t-0.5)^2) B(t, \mathcal{G}_{i-1})+ 0.8\varepsilon_{i}$. 
 \cref{fig:lrd} compares the power performance of the KPSS, V/S, R/S, and K/S tests using difference-based and plug-in long-run covariance matrix estimators for model \eqref{M1} of sample size $1500$ with nominal level $0.05$ (left) and $0.1$ (right) with long memory parameter $d$ increasing from $0$ to $0.5$. The difference-based long-run covariance matrix estimator leads to consistent improvement for the power of KPSS, V/S, R/S, and K/S tests against $0< d \leq 1/2$ in finite samples. Notably, equipped with the difference-based estimator the simulated power of K/S, R/S, and KPSS tests can reach $1$ with the sample size $1500$ as $d$ increases to $0.5$, while using the plug-in long-run covariance matrix estimator the power is much lower and stays far below $1$, except for  the V/S test.
\begin{figure}
\centering
     \includegraphics[width = 0.9\textwidth]{lrdcolor.jpg}
\caption{Power performance of KPSS(orange), K/S(blue), R/S(red), and  V/S(green) tests with sample size $1500$. Dotted curves denote simulated rejection rates of tests using the plug-in method, while solid curves denote simulated rejection rates of tests using the difference-based method. Left panel: nominal size 0.05; Right panel: nominal size 0.1.}
...
We apply our newly proposed long-run covariance matrix estimator \eqref{eq:diff_correct} to the analysis of Hong Kong hospital data, including structural change detection and tests for long memory. The data set consists of daily hospital admissions in Hong Kong as well as daily measurements of pollutants between January 1, 1994, and December 31, 1995, whose sample size is $730$. Consider the functional linear model for this data set, i.e., 
\begin{equation}\label{eq:hk_model}
   y_{i,n}=\beta_{1}(t_i)+\sum_{p=2}^{4} \beta_{p}(t_i) x_{i, p, n}+\varepsilon_{i, n}, \quad \quad i=1, \ldots, n,
 \end{equation}
 where $(y_{i,n})$ is the series of daily total number of hospital admissions of circulation and respiration and $(x_{i, p, n})$, $p = 2, 3, 4$, are the series of daily levels  (in micrograms per cubic meter) of $\text{SO}_2$, $\text{NO}_2$ and dust, respectively.  %\par We consider the test for the long memory of $\varepsilon_i$ of the whole time span between January 1, 1994, and December 31, 1995 with sample size $730$.


As illustrated in Section 5 of \cite{wu2018gradient}, it is of practical concern to test whether $\beta(\cdot):=(\beta_p(\cdot),1\leq p\leq 4)^\top$ is a constant vector. The test for structural changes equipped with the difference-based estimator yields $p$-value $0.0318$, which rejects the null hypothesis of no structural change, while the test procedure proposed in  \cite{wu2018gradient} based on ordinary least squares residuals yields $p$-value greater than $0.1$. The different testing results can be attributed to the power loss of \cite{wu2018gradient} under structural change, as shown in \cref{fig:cptest}.


% \LJ{Compare \cite{zhou2013heteroscedasticity}}
 We then consider the test for the long memory of $\varepsilon_{i,n}$. 
\cite{bai2021} performs long-memory tests on each covariate process and concludes that they are short-range dependent. Therefore, we could apply the tests introduced in \cref{sec:app2} to this data, and compare the $p$-values 
 of the tests equipped with the difference-based estimator \eqref{eq:diff_correct} and with the plug-in estimator of \cite{zhou2010simultaneous}, respectively.  The bandwidth $b_n$ in \cref{sec:app2} are selected by the GCV  method advocated by \cite{zhou2010simultaneous} and \cite{bai2021}.
 \begin{table}[ht]
\caption{$p$-values of tests for long memory.}
\centering
\begin{tabular}{rrrrr|rrrrr}
  \hline
 & KPSS & RS & VS & KS & & KPSS & RS & VS & KS \\ 
  \hline
plug & 0.353 & 0.276 & 0.126 & 0.411 &
  diff & 0.672 & 0.895 & 0.705 & 0.840 \\ 
   \hline
\end{tabular} \label{tb:pvalue}
\end{table}
...
\section{Conclusion}
In this paper, we propose a debiased difference-based long-run covariance matrix estimator for the functional linear model  under complex dynamics. We establish the uniform consistency of the proposed estimator and justify theoretically its application to structural change tests and detection of long memory. We conduct extensive simulation studies and achieve significant improvements in the finite samples. In particular, the new difference-based statistic is robust to both smooth and abrupt changes in the regression coefficients, largely eliminating the non-monotonic issue arising in the change point test. Finally, we apply our new estimators to the nonparametric analysis of Hong Kong circulatory and respiratory data, providing new uncertainty on structural changes and long memory. The optimal long-run variance for time series with stationary errors has been thoroughly discussed recently by \cite{chan2021optimal}. However, the approach therein is not applicable when non-stationarity is present. We leave the optimal estimation of the long-run covariance matrix under time series non-stationarity as rewarding future work. In addition, the generalization of our method beyond linear models will also be of great importance.
%\WC{Furthermore, the reuslts yielded by the plugin methods are more  sensitive to the tuning parameter $b_n$ } %The optimality of difference statistics \cite{chan2021optimal} for the random-design locally stationary time series regression model is left for rewarding future work.

\section{Acknowledgement}
Weichi Wu is the corresponding author and is supported by NSFC 12271287.
\section{Supplementary material}
In the supplement, we present extra simulation studies and the proofs of results in this paper as well as auxiliary technical results. 
...
We organize the supplementary material as follows: We investigate the simulated sizes of tests for long memory and the magnitude of long-run covariance matrix estimates in the presence of smooth and abrupt changes in \cref{sec:lrv}. \cref{diff} presents proofs of the results in the main paper. \cref{sec:aux} provides auxiliary results which are used in the proofs.  \par 
...
plug-in  & 7.60 & 15.00 & 14.90 & 28.60 & 18.00 & 36.20 & 7.70 & 16.50 \\ 
  diff & 4.80 & 8.90 & 3.70 & 6.20 & 3.60 & 5.90 & 4.50 & 9.40 \\ 
   \hline
\end{tabular}
\caption{Simulated rejection rates of KPSS, R/S, V/S and K/S-type tests for long memory when $d=0$, $n = 750$.}
\label{tb:size}
\end{table}
\subsection{Long-run covariance estimates with change points}
To further illustrate the role of long-run variance estimators in bootstrap tests, we conduct $1000$ times simulation, where we find the MSE of long-run variance estimators using ordinary least square residuals is $695.33$, more than twice the magnitude of the MSE of  that of difference-based method $313.72$ when there are four change points in the time series structure (scenario CP4 with $
...
\subsection{Notation}
We first introduce some notation that will be frequently used in the mathematical argument of this section. In the following proofs, we will omit the index $n$ in $e_{i,n}, x_{i,n}, y_{i,n}$ for simplicity. Define filtration $\FF_i=(\varepsilon_{-\infty},...,\varepsilon_i)$ for $i.i.d.$ random variables $(\varepsilon_{i})_{i\in \mathbb Z}$. For a random vector $(v_i)_{i=1}^n \in \F_s$, let $v_{i,\{s\}}$ denote the series replacing the $\varepsilon_{s}$ with its $i.i.d.$ copy.  For a random matrix $(A_i)_{i=1}^n \in \F_s$, 
define $A_{j,\{s\}}$ as the random matrix replacing  $\varepsilon_{s}$ in $A_{j}$ with its $i.i.d.$ copy. Recall that $ e_{i,n}^{(d)} = \sum_{j=0}^{\infty}\psi_j(d) e_{i-j, n}$, $ e_{i,n}^{(d_n)} = \sum_{j=0}^{\infty}\psi_j(d_n) e_{i-j, n}$. For the sake of simplicity, we use $\psi_j$ to represent $\psi_j(d)$ when we discuss the fixed alternatives and $\psi_j(d_n)$ for the theory of the local alternatives. Recall $t_i = i/n$, and $K^*(x)$ denotes the jackknife equivalent kernel $2 \sqrt{2} K(\sqrt{2}x) - K(x)$. Let $0 \times \infty = 0$, $a_n \sim b_n$ denote $\lim_{n \to \infty} a_n/b_n = 1$ for real sequences $a_n$ and $b_n$.  Let $\mathcal{I} = [\gamma_n, 1-\gamma_n] \subset (0,1)$, $\gamma_{n}=\tau_{n}+(m+1) / n.$ Recall  $$A_{j,m} = \frac{1}{m} \sum_{i=j-m+1}^j\{x_i x_i^{\T} { \beta}(t_i) - x_{i+m} x_{i+m}^{\T} { \beta}(t_{i+m})\},\quad { \Sigma}^A (t) = \sum_{j=m}^{n-m} \frac{m A_{j,m} A_{j,m}^{\T}}{2}\omega(t, j),$$
...
\end{align}
 ...
 We define below the counterparts of $Q_{k,m}$, ${ \Delta}_j$ and $\hat{{ \Sigma}}(\cdot)$ in \cref{sec:diff}. Define for $m \geq 2$, $t \in [m/n, 1 - m/n]$,
    ...
    \end{align}
\subsection{Proof of \texorpdfstring{ \cref{thm:lrv_diff}}{Proof of Theorem 1} }
...
\begin{align}
\sup_{t \in \I}\lt| \breve { \Sigma}(t) - { \Sigma}^A (t)\rt|= \Op\left( \sqrt{m}\tau_n^{2-1/\kappa} + \sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} \right).
\end{align}
...
Define $A_n = \{ \sup_{t \in  \I} |{ \Omega}(t) - M^+(t)| \leq l_n ((m\tau_n)^{-1/2} + m/(n\tau_n^{1/2}) + \tau^{1/2}_n)\}$. By \eqref{eq:remark5.2}, $\lim_{n\to\infty} \pp(A_n) = 1.$ Since $0 \leq \sum_{j=m}^{n-m} \omega(t,j) \leq 1$, we have
  \begin{align}
 ...
 Define $ B_{j,m} = \frac{1}{m}\sum_{i=j-m+1}^j x_i x_i^{\T}.$
    ...
    \end{align}
    Similar to  Lemma 6 in \cite{zhou2010simultaneous}, using rectangular kernel with bandwidth $m/n$, under Assumption \ref{E:WW}, we have
    \begin{align}
...
\begin{align}
\sup_{t \in \I} \|( { \beta}(t) - \breve{ \beta}(t))1(A_n) \|_{4\kappa} = O\left(\tau_n^2\right).\label{eq:betabreve}
\end{align}

Let  $M^+(t) =\E\{\bar{J}(t, \F_0)\bar{J}^{\T}(t, \F_0)\}$. 
Following similar arguments in Lemma 6 of \cite{zhou2010simultaneous} and Theorem 5.2 of \cite{dette2019detecting}, under Assumptions \ref{E:WW} and \ref{Ass-E}, we have 
\begin{align}
   \sup_{m \leq j\leq n} \|\acute { \Delta}_j/2 - M^+(t_j)\| = O(m^{-1/2} + m/n).
\end{align}
Then, it follows that 
\begin{align}
      \sup_{t \in \I } \|{ \Omega}(t) - M^+(t)\| = O(m^{-1/2} + m/n + \tau_n).
\end{align}
By the chaining argument in Propostion B.1 in Section B.2 in \cite{dette2018change}, we have
\begin{align}
      \sup_{t \in \I} |{ \Omega}(t) - M^+(t)| = \Op((m\tau_n)^{-1/2} + m/(n\tau_n^{1/2}) + \tau^{1/2}_n).\label{eq:remark5.2}
\end{align}
...
\begin{align}
\sup_{t \in \I} \|{ \varpi}(t) - { \Omega}(t) { \beta}(t)\|_{4\kappa} = O\left(\tau_n^2\right).
\end{align}
...
Observe that 
\begin{align}
{ \varpi}(t)& = \sum_{j=m}^{n-m} \frac{ \omega(t, j)}{2} \grave{A}_{j,m} + \sum_{j=m}^{n-m} \frac{\omega(t, j)}{2} \grave { \Delta}_j : =  W_1 + W_2,\label{eq:varpi}
\end{align}
where $W_1, W_2$ are defined in the obvious way. Recall that 
$\acute { \Delta}_{j} = \frac{1}{m}\sum_{i=j-m+1}^{j} (x_{i,n} x_{i,n}^{\T}-x_{i+m, n} x_{i+m, n}^{\T})^2$.
By triangle inequality, we have 
\begin{align}
\sup_{t \in \I} \left\| W_1 - { \Omega}(t) { \beta}(t) \right\|_{4\kappa} 
&\leq \sup_{t \in \I} \left\|W_1 - \sum_{j=m}^{n-m} \frac{ \acute { \Delta}_j\omega(t, j)}{2} { \beta}(t_j) \right\|_{4\kappa}  +  \sup_{t \in \I} \left\| \sum_{j=m}^{n-m} \frac{\acute { \Delta}_j  \omega(t, j)}{2} ({ \beta}(t_j)-{ \beta}(t)) \right\|_{4\kappa}\\ 
& := W_{11} + W_{12}.
\end{align}
...
% W_{11} = O(m/n).
% \end{align}
Under Assumption \ref{E:WW}, we have $\max_{m \leq j \leq n-m} \|\acute { \Delta}_j\|_{4\kappa} = O(1).$ Then, by simlar argments in Lemmas 3 of \cite{zhou2010simultaneous} and the continuity of $M^{+}(t)$, we obtain  
\begin{align}
 W_{12} 
% & \leq \sup_{t\in \I}\sum_{j=m}^{n-m} \frac{ \omega(t, j)|{ \beta}(t_j) -{ \beta}(t)|}{2}\max_{m \leq j \leq n-m}\|\acute { \Delta}_j\|_{4\kappa} = O(\tau_n).
& = \sup_{t \in \I} \left\|\sum_{j=m}^{n-m} \frac{\acute { \Delta}_j  \omega(t, j)}{2} ({ \beta}^{\prime}(t)(t_j - t) + O(\tau_n^2))  \right\|_{4\kappa}\\ 
& \leq \sup_{t \in \I} \left\|\sum_{j=m}^{n-m} (\acute { \Delta}_j/2 - M^{+}(t_j))  \omega(t, j) { \beta}^{\prime}(t)(t_j - t) \right\|_{4\kappa}\\ &+ \sup_{t \in \I} \left\|\sum_{j=m}^{n-m}  M^{+}(t_j) \omega(t, j) { \beta}^{\prime}(t)(t_j - t)  \right\|_{4\kappa} + O(\tau_n^2) \\ 
& = \sup_{t \in \I} \left\|\sum_{j=m}^{n-m}   \omega(t, j) { \beta}^{\prime}(t)M^{+}(t)(t_j - t)  \right\|_{4\kappa} + O(\tau_n((n\tau_n)^{-1/2} + m/n) + \tau_n^2) \\ 
&= O\left(\tau_n^2\right).
\label{eq:W12}
\end{align}

Therefore, combining \eqref{eq:W11}  and \eqref{eq:W12}, since $n\tau_n^3 \to \infty$, $m/(n\tau_n^2) \to 0$, we have
\begin{align}
\sup_{t \in \I} \| W_1 - \Omega(t) { \beta}(t)\|_{4\kappa}  = O\left(\tau_n^2 \right).\label{eq:W1}
\end{align}

To proceed, we shall show that 
\begin{align}
\sup_{t \in \I} \|W_2\|_{4\kappa} = O((n \tau_n)^{-1/2} + \chi^m).\label{eq:W2}
\end{align} 
...
\begin{align}
\left\|  \sum_{j=m}^{n-m} \frac{ \omega(t, j)}{2} \lt(\grave { \Delta}_j -  \E \grave { \Delta}_j\rt) \right\|_{4\kappa} &= \left\| \sum_{s = -m}^{\infty}\sum_{j=m}^{n-m} \frac{ \omega(t, j)}{2} \proj_{j-s} \grave{ \Delta}_j \right\|_{4\kappa}\\
 &\leq C  \sum_{s = -m}^{\infty} \left\{ \sum_{j=m}^{n-m} \frac{ \omega^2(t, j)}{4} \lt\|\proj_{j-s} \grave{ \Delta}_j \rt\|_{4\kappa}^2\right\}^{1/2}.\label{eq:W2-1}
\end{align}
Under Assumptions \ref{E:WW} and \ref{B:H_delta}, using similar techniques in Lemma 3 of \cite{zhou2010simultaneous}, we have
\begin{align}
...
Hence, by \eqref{eq:fixtA}, under conditions $m = O(n^{1/3}), n\tau_n^3 \to \infty$, we have
\begin{align}
 \sup_{t \in \I}\left\| (\breve { \Sigma}(t) - { \Sigma}^A (t))1(A_n) \right\|_{\kappa}= O\left(\sqrt{m}\tau_n^2+ \sqrt{\frac{m}{n\tau_n}}\right).
\end{align}
%  By the chaining argument in Proposition B.1 in Section B.2 in \cite{dette2018change}, we have
%  \begin{align}
%   \left \| \sup_{t \in \I}| (\breve { \Sigma}(t) - { \Sigma}^A (t))1(A_n)|  \right\|_{\kappa}= O\left( \sqrt{m\tau_n^{2-2/\kappa}}\right).
% \end{align}
% Finally, by Proposition A.1 in \cite{wu2018gradient}, we have 
% \begin{align}
%      \sup_{t \in \I} \lt| \breve { \Sigma}(t) - { \Sigma}^A (t)\rt|= \Op\left( \sqrt{m\tau_n^{2-2/\kappa}}\right).
% \end{align}
Finally, the result follows from the chaining argument in Proposition B.1 in Section B.2 in \cite{dette2018change} and Proposition A.1 in \cite{wu2018gradient}.
\end{proof}



\subsubsection{Proof of \texorpdfstring{\cref{thm:lrv_diff}}{lrvdiff}}\label{proof:thm7.1}
% \WC{Checked, please change green to dark}
% \begin{proof}
Recall the definition of $\tilde{ \Sigma}(t)$ in \eqref{eq:tildeSigma}. Observe that
    ...
    we have
    \begin{align}
      \sup_{t \in \I}\lt|{ \Sigma}^A (t) -  \breve { \Sigma}(t)\rt| = \Op\left(\sqrt{m}\tau_n^{2-1/\kappa} + \sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}}\right).\label{eq:SigmaA}
  \end{align}
  To proceed, define 
    \begin{align}
...
%   $$\delta_4(\tilde{ \Delta}(m),k) = \sup_{1 \leq j \leq n} \| \tilde{ \Delta}_j - \tilde{ \Delta}_{j,\{j-k\}}\|_4.$$ 
  Under Assumptions \ref{Ass-E} and \ref{B:H_delta}, similar to (36) in Lemma 3 in \cite{zhou2010simultaneous}, we have
   \begin{align}
  ...
  where  M is a sufficiently large positive constant.
   Following similar arguments in Theorem 1 of \cite{wu2007strong}, $\sup_j \|\tilde{ \Delta}_j\|_{2\kappa} = O(m^{-1/2})$. Similar to \eqref{eq:Ajm}, we have uniformly for $m \leq j\leq n- m$, 
   \begin{align}
  ...
  \begin{align}
    ...
    = O\left(\sqrt{\frac{m}{n \tau_n}}\right).
    \label{eq:Sigmat0}
  \end{align}
  By the chaining argument in Propostion B.1 in Section B.2 in \cite{dette2018change}, we have
  \begin{equation}
    \sup _{t \in \mathcal{I}} \left| m \sum_{j=m}^{n-m} \omega(t, j)\tilde { \Delta}_{j}A_{j,m}^{\T} \right| = \Op\left(\sqrt{\frac{m}{n \tau_{n}^{1+2/\kappa}}}\right).\label{eq:Sigmat}
   \end{equation} 
  Combining \eqref{eq:Sigma_decomp},  \eqref{eq:SigmaA} and \eqref{eq:Sigmat}, we obtain
  \begin{equation}
    \sup _{t \in \mathcal{I}} \lt| \hat{{ \Sigma}}(t)-\tilde{{ \Sigma}}(t) \rt| = \Op\left(\sqrt{m}\tau_n^{2-1/\kappa} +  \sqrt{\frac{m}{n\tau_n^{1+2\kappa}}}\right).
    \label{eq:Sigmatsup}
  \end{equation}
 By Lemma 3 in \cite{zhou2010simultaneous}, Proposition B.1 in Section B.2 in \cite{dette2018change}, we have 
  \begin{equation}
   \sup _{t \in \mathcal{I}} \lt| \tilde{{ \Sigma}}(t)-\mathbb{E}\tilde{{ \Sigma}}(t)\rt|=\Op\left(\sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}}\right). 
    \label{eq:Sigmat0sup}
  \end{equation}
  Using similar techniques in Lemma 4 and Lemma 5 of \cite{zhou2010simultaneous}, which hold uniformly for $t\in(0,1)$,  \eqref{eq:Sigmat0sup} leads to 
  \begin{equation}
    \sup _{t \in \mathcal{I}} \lt| \tilde{{ \Sigma}}(t) -{ \Sigma}(t)\rt|  = 
    \Op\left(\sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} + \frac{1}{m}+\sqrt{m}\tau_n^{2-1/\kappa}\right),
    \label{eq:Sigmatsup0}
  \end{equation}
  With \eqref{eq:Sigmatsup} and \eqref{eq:Sigmatsup0}, the supreme bound is thus proved. \hfill $\Box$
  \subsection{Proof of \texorpdfstring{\cref{thm:cp}}{Proof of Theorem 2}}
  Let $\mathcal C_m = \{j-m \leq i \leq j+m, \beta^{\prime\prime\prime}(t_i) ~\text{exists and continuous} \}$. Similar to \eqref{eq:fixtA},
  \begin{align}
    &\sup_{t \in \I} \lt\| (\breve { \Sigma}(t) - { \Sigma}^A (t))1(A_n)\rt\|_{\kappa}\\
    &\leq \sup_{t \in \I} \sum_{j=m}^{n-m}\frac{m\omega(t,j)}{2} \lt\|(\hat{A}_{j,m} -  A_{j,m})1(A_n)\rt\|_{2\kappa}\left(\lt\|(\hat{A}_{j,m} -  A_{j,m})1(A_n)\rt\|_{2\kappa} + 2\|A_{j,m}\|_{2\kappa}\right) \\ 
    & \leq \sup_{t \in \I} \left(\sum_{j\in \mathcal C_m, m \leq j \leq n-m} + \sum_{j \notin \mathcal C_m, m \leq j \leq n-m}\right) \frac{m\omega(t,j)}{2} \lt\|(\hat{A}_{j,m} -  A_{j,m})1(A_n)\rt\|_{2\kappa} \\ & \times \left(\lt\|(\hat{A}_{j,m} -  A_{j,m})1(A_n)\rt\|_{2\kappa} + 2\|A_{j,m}\|_{2\kappa}\right).\label{eq:fixAcp}
  \end{align}
For $j \notin \mathcal C_m$, we have $\| A_{j,m} \|_{2\kappa} = O(1)$ and $\lt\|(\hat{A}_{j,m} -  A_{j,m})1(A_n)\rt\|_{2\kappa} = O(1)$. We shall  show that,
  $$\max_{j \in  \mathcal C_m}\|(\hat{  A}_{j,m} -   A_{j,m})1(A_n) \|_{2\kappa} = O(\tau_n^2). $$
  It suffices to show that 
  \begin{align}
      \sup_{j \in  \mathcal C_m}\|( \beta(t_j
      ) - \check { \beta}(t_j))1(A_n) \|_{4\kappa} = O(\tau_n^2).\label{eq:beta_cp} 
  \end{align}
...
\begin{align}
{ \varpi}(t)& = \sum_{j=m}^{n-m} \frac{ \omega(t, j)}{2} \grave{A}_{j,m} + \sum_{j=m}^{n-m} \frac{\omega(t, j)}{2} \grave { \Delta}_j : =  W_1 + W_2,
\end{align}
where $W_1, W_2$ are defined in the obvious way.
Following \eqref{eq:W11}, \eqref{eq:W12} and \eqref{eq:W2}, we have 
\begin{align}
 & \sup_{r \in C_m} \| W_1 - { \Omega}(t_r) { \beta}(t_r)\| \\ &\leq   \sum_{j=m}^{n-m} \frac{ \omega(t_r, j)}{2m}\left\{\sum_{i = j-m+1}^{j} \left\|x_i x_i^{\T} - x_{i+m} x_{i+m}^{\T}\right\|_{8\kappa}\right.\\ & 
\times \left. \|x_i x_i^{\T}({\beta}(t_i) - { \beta}(t_r)) - x_{i+m} x_{i+m}^{\T}({\beta}(t_{i+m}) - { \beta}(t_r))\|_{8\kappa} \right\}\\ 
   &\leq   \sup_{r \in C_m} \sum_{j=m}^{n-m} \frac{ \omega(t_r, j)}{2m} \left(\sum_{j \in  \mathcal C_m, m \leq j \leq n-m} + \sum_{j \notin  \mathcal C_m, m \leq j \leq n-m}\right)\{\sum_{i = j-m+1}^{j} \left\|x_i x_i^{\T} - x_{i+m} x_{i+m}^{\T}\right\|_{8\kappa} \\ & 
\times  \|x_i x_i^{\T}({\beta}(t_i) - { \beta}(t_r)) - x_{i+m} x_{i+m}^{\T}({\beta}(t_{i+m}) - { \beta}(t_r))\|_{8\kappa}\}\\ 
   &=  O\left(\frac{mq_n}{n\tau} + \frac{m}{n}\right), \label{eq:W11cp}
\end{align}
as well as 
\begin{align}
  \sup_{r \in C_m} \|  W_2\| = O((n\tau)^{-1/2} + \chi^m).\label{eq:W2cp}
\end{align}
Since $q_n = o(n\tau_n^3 / m)$, following similar arguments of \cref{lm:bias}, combining \eqref{eq:W11cp} and \eqref{eq:W2cp}, we have proved \eqref{eq:beta_cp}. Then, by \eqref{eq:fixAcp} and the chaining argument of Proposition B.1 of \cite{dette2018change}, we still have 
\begin{align}
\sup_{t \in \I}\lt| \breve { \Sigma}(t) - { \Sigma}^A (t)\rt| & = \Op\left( \sqrt{m}\tau_n^{2-1/\kappa} + \sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} + \frac{m^2q_n}{n\tau^{1+1/\kappa}} \right) \\ 
& = \Op\left( \sqrt{m}\tau_n^{2-1/\kappa} + \sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} + \frac{m q_n}{n\tau^{3}} \right). \label{eq:cpcorrect}
\end{align}
...
\begin{align}
  ...
  \end{align}
   By the chaining argument in Propostion B.1 in Section B.2 in \cite{dette2018change}, similar to \eqref{eq:Sigmat}, we have
  \begin{equation}
    \sup _{t \in \mathcal{I}} \left| m \sum_{j=m}^{n-m} \omega(t, j)\tilde { \Delta}_{j}A_{j,m}^{\T} \right| = \Op\left(\sqrt{\frac{m}{n \tau_{n}^{1+2/\kappa}}}\right). \label{eq:cpmain}
   \end{equation} 
  ...
  shown the first part of the results. The second part follows from \cite{yu2015useful} and Theorem 3.4 of \cite{wu2018gradient}. \hfill  $\Box$.
...
% %    Under Assumptions \ref{A:K}, \ref{A:beta}, \ref{Ass-W} and \ref{Ass-E} under the bandwidth conditions $m = O(n^{1/3})$, $m / (n \tau^2_n) \to 0$, $n\tau_n^3 \to \infty$, and $\sqrt{m} \tau_n^{2-1/\kappa} \to 0$, for $d_n = c/\log n$, we have 
% %   \begin{align}
% %   \sup_{t \in \I}\lt| \breve { \Sigma}_{d_n}(t) - { \Sigma}^A (t)\rt|= \Op\left( \sqrt{m}\tau_n^{2-1/\kappa} + \sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}}\right)=\op(1).
% %   \end{align}
% %   \end{lemma}
...
% %       \begin{align}
% %       \sup_{1 \leq k \leq n - m + 1}\left\|\sum_{i=k}^{k+m-1} x_i e_i^{(d_n)}\right\|_4 = O(\sqrt{m}).
% %       \end{align}
% %     \end{lemma}
% %     \begin{proof}[Proof]
...
%    \sup_{t \in \I} \lt| \hat{{ \Sigma}}_{d_n}(t) - \tilde{{ \Sigma}}_{d_n}(t) \rt| = \Op\left( \sqrt{m}\tau_n^{2-1/\kappa} + \sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} \right) = \op(1).\label{eq:local_step1}
%    \end{align}
% %   To show this, define 
...
% % %   \begin{align}
% % %   \sup_{t \in \I} \lt| \breve { \Sigma}_{d_n}(t) - { \Sigma}^A(t)\rt| = \Op\left(\sqrt{m \tau_n^{2-2/\kappa}}\right).\label{eq:supdn1}
% % %   \end{align}
% %   Define $
...
% %   by chaining argument in Proposition B.1 in  \cite{dette2018change},  we have 
% %    \begin{align}
% %   \sup_{t \in \I} \left |\sum_{j=m}^{n-m} m\omega(t, j) \tilde { \Delta}^{(d_n)}_{j}A_{j,m}^{\T} \right| = \Op\left(\sqrt{\frac{m}{n\tau^{3/2}_n}}\right).
% %   \label{eq:supdn2}
% %   \end{align}
...
%     Define 
%    $$\check{Q}^{(d_n)}_{k, m} = : \sum_{i=k}^{k+m-1} x_{i}\check e_{i, L}^{(d_n)},\quad \check { \Delta}_{j}^{(d_n)}=\frac{\check{{Q}}^{(d_n)}_{j-m+1, m}- \check{{Q}}^{(d_n)}_{j+1, m}}{m}, \quad \check{{ \Sigma}}_{d_n}(t)=\sum_{j=m}^{n-m} \frac{m \check { \Delta}_{j}^{(d_n)}\{\check { \Delta}^{(d_n)}_{j}\}^{\T}}{2}\omega(t, j).$$
...
%   \begin{align}
%   \lt\| \check{Q}^{(d_n)}_{k, m} - \tilde{Q}^{(d_n)}_{k, m}\rt\|  \leq  m \max_{1 \leq i \leq n}\lt\| x_i \tilde{e}^{(d_n)}_{i,L}\rt\| \leq m \max_{1 \leq i \leq n} \| x_i \|_4 \lt\| \tilde{e}^{(d_n)}_{i,L} \rt\|_4  = O(m/\sqrt{L})\label{eq:diffQdn}.
%   \end{align}
% By \cref{lm:psum_dn} and \eqref{eq:diffQdn}, we  have  
%   \begin{align}
%   \left\| \check { \Sigma}_{d_n} (t) - \tilde { \Sigma}_{d_n} (t)\right\|\leq m \max_{m\leq j \leq n-m}\left\| \tilde  { \Delta}_{j}^{(d_n)} -  \check{ \Delta}_{j}^{(d_n)}\right\|_4 ( \left\| \tilde  { \Delta}_{j}^{(d_n)} -  \check{ \Delta}_{j}^{(d_n)}\right\|_4  + 2\left\| \tilde{ \Delta}_{j}^{(d_n)}\right\|_4) = O\left(\sqrt{m/L}\right).
%   \end{align}
%   By Proposition B.1 in  \cite{dette2018change}, since $m/(L\tau_n)  = m^{-1}\tau_n^{-3/2} \to 0$, \eqref{eq:local_step2} is proved.
%   \par
%   \textbf{Step 3}	: 
%     Define 
%    $\bar{Q}^{(d_n)}_{k, m} = : \sum_{i=k}^{k+m-1}( x_{i}e_i  + { \mu}_W(t_i) (\check e_{i,L}^{(d_n)} - e_i))$,$$\bar { \Delta}_{j}^{(d_n)}=\frac{\bar{{Q}}^{(d_n)}_{j-m+1, m}- \bar{{Q}}^{(d_n)}_{j+1, m}}{m}, \quad \bar{{ \Sigma}}_{d_n}(t)=\sum_{j=m}^{n-m} \frac{m \bar { \Delta}_{j}^{(d_n)}\{\bar { \Delta}^{(d_n)}_{j}\}^{\T}}{2}\omega(t, j). $$
%   We shall show that 
%    \begin{align}
%    \sup_{t \in \I }\left|\check { \Sigma}_{d_n}(t) - \bar { \Sigma}_{d_n}(t)\right|  = \Op\left(\sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} + d_n\right) = \op(1).\label{eq:local_step3}
%    \end{align}
%   Observe that 
%   \begin{align}
%   \check {Q}^{(d_n)}_{k, m}
...
%     Then, it follows that 
%    \begin{align}
%        \check { \Sigma}_{d_n}(t) - \bar { \Sigma}_{d_n}(t) =  \sum_{j=m}^{n-m} \frac{m\omega(t, j)}{2}( \check { \Delta}_{j}^{(d_n)}\{\check { \Delta}^{(d_n)}_{j}\}^{\T} - \bar { \Delta}_{j}^{(d_n)}\{\bar { \Delta}^{(d_n)}_{j}\}^{\T}),\label{eq:varstep3}
%    \end{align}
%    and \begin{align}
%     \check { \Delta}_{j}^{(d_n)}\{\check { \Delta}^{(d_n)}_{j}\}^{\T} - \bar { \Delta}_{j}^{(d_n)}\{\bar { \Delta}^{(d_n)}_{j}\}^{\T}  = { \vartheta}_{k,m} { \vartheta}_{k,m}^{\T} +  { \vartheta}_{k,m}\{\bar { \Delta}^{(d_n)}_{j}\}^{\T} + \bar { \Delta}^{(d_n)}_{j}{ \vartheta}_{k,m}^{\T}.\label{eq:vartheta}
%    \end{align}
...
%     Under Assumptions \ref{assumptionHp} and \ref{Ass-W}, by Lemma 7 in \cite{zhou2014vstat} we have for $l, j>0$,
%     \begin{align}
%     |\E(\bar{x}_ie_{i-j}\bar{x}^{\T}_{i+k}e_{i+k-l} )|
%     = O(\chi^{\rho^*}),
%     \end{align}
%     where
%   following the lines in the proof of Theorem 2 in \cite{zhou2014vstat}, we have 
%   \begin{align}
...
%   %  \end{align}
%   % where the last equality follows from a careful check of Lemma 3.2 in \cite{KOKOSZKA199519}.
%   % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    \begin{align}
%   \sup_{t \in \I }\left|\check { \Sigma}_{d_n}(t) - \bar { \Sigma}_{d_n}(t) - \E(\check { \Sigma}_{d_n}(t) - \bar { \Sigma}_{d_n}(t))\right| =\Op\left(\sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}}\right)= \op(1).
...
%   & = O\left(\sqrt{\frac{m}{n\tau_n}}\right),
%   \end{align}
%   where in the last equality, we consider $0<i+s<L$, and $i+s \geq L$ separately and use the fact $\sum_{i=1}^L i^{-1} = O(\log L).$ Then, \eqref{eq:local_step32} follows from the chaining argument in Proposition B.1 in  \cite{dette2018change}.
...
%    By \eqref{eq:step3_2} and similar arguments in Theorem 1(ii) of \cite{wu2007strong}, we have uniformly for $1 \leq k \leq n-m+1$,
%    \begin{align}
...
%   \begin{align}
%     \sup_{t \in \I}| { \Sigma}_{d_n}^{\circ}(t)- \E { \Sigma}_{d_n}^{\circ}(t)|  = \Op\left(\sqrt{\frac{1}{m \tau_n^{3/2}}}\right) = \op(1),\label{eq:local_step6}
%   \end{align}
%   \par
...
%   \begin{align}
%   \sup_{t \in \I} |\hat { \Sigma}_{d_n}(t) - \check { \Sigma}(t)| =\Op\left(\sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} + \sqrt{m}\tau_n^{2-1/\kappa} +\sqrt{\frac{1}{m\tau_n^{3/2}}} + (\log n)^{-1}\right) = \op(1).
%   \end{align}
...
\cref{prop:stat_alt} follows from similar but easier arguments of Theorem 3.1 and  Theorem 3.2 of \cite{wu2018gradient}.
...
For  the clarity of presentation, write $\hat{ \Sigma}_d(t)$ for $\hat{ \Sigma}(t)$ defined in \eqref{eq:diff_correct} under the fixed alternative.  Define $${ \Sigma}_d(t) = \kappa_2(d)\sigma_H^2(t)  \mu_W(t) \mu^{\T}_W(t),\quad t \in [0,1].$$
 ...
 $, $\tilde{{ \Sigma}}_d(t)=\sum_{j=m}^{n-m} \frac{m \tilde { \Delta}_{j}^{(d)}\{\tilde { \Delta}^{(d)}_{j}\}^{\T}}{2}\omega(t, j)$.
 $  \grave { \Delta}_{j}^{(d)} = \frac{1}{m}\sum_{i=j-m+1}^{j} (x_i x_i^{\T}-x_{i+m} x_{i+m}^{\T}) (x_i e_i^{(d)}-x_{i+m} e_{i+m}^{(d)})
...
The following proposition is from Theorem 4.2 of \cite{bai2021}.
...
\begin{align}
  \max_{1 \leq k \leq n - l + 1}\left\|\sum_{i=k}^{k+l-1} (x_i - \mu_W(t_i)) e_i^{(d)}\right\| = O(\sqrt{l}(\log n)^d).
\end{align}
\end{corollary}
\begin{proof}[Proof]
  The corollary follows from a careful check of the proof of Theorem 4.2 of \cite{bai2021}.
\end{proof}
...
%Under Assumptions \ref{A:beta}, \ref{Ass-W}, \ref{Ass-E}, suppose $\kappa  \geq \max \{4, 2/(1-2d)\}$, $m/(n\tau_n^2) \to 0$, $m = O(n^{1/3})$,  $\sqrt m \tau_n^{2-1/\kappa} \to 0$, $m \to \infty$, $n \tau_n^3 \to \infty$, we have 
Under the conditions of  \cref{lm:Sigma_d}, we have
\begin{align}
\sup_{t \in \I}\lt| \breve { \Sigma}_d(t) - { \Sigma}^A (t)\rt|= \Op\left( \sqrt{m} ((n\tau_n)^{d-1/2} + \tau_n^2)\tau_n^{-1/\kappa} +  m ((n\tau_n)^{2d-1}+\tau_n^4)\tau_n^{-1/\kappa}\right) = \op(m^{2d}).
\end{align}
\end{lemma}
...
%     \end{align}
After a careful check of the proof of \cref{lm:correction}, the behavior of $W_1$ is unchanged under the fixed alternatives and  it's sufficient to show that $W_2^{(d)}$, $W_2$ under the fixed alternatives, s.t.
    \begin{align}
    \sup_{t \in \I} \lt\|W_2^{(d)}\rt\|_{4\kappa} = O((n\tau_n)^{d-1/2}).\label{eq:W2d}
    \end{align} 
...
\end{align}
Let $N = N_n =\lf  n \tau_n\rf$. Note that under Assumption \ref{E:HW}, we can write 
\begin{align}
    W_2^{(d)} &= \sum_{s=0}^N \sum_{j=m}^{n-m}\frac{\omega(t,j)}{2} \proj_{j-s} \grave { \Delta}_j^{(d)} + \sum_{j=m}^{n-m} \frac{\omega(t,j)}{2} \sum_{s=N+1}^{\infty} \proj_{j-s} \grave { \Delta}_j^{(d)} = W_{21}^{(d)} + W_{22}^{(d)}.
\end{align}
Since $\proj_{j-s}\grave { \Delta}_j^{(d)}$ are martingale differences with respect to $j$, for $0 \leq s\leq N$, we haves
\begin{align}
    \left\|\sum_{j=m}^{n-m}\frac{\omega(t,j)}{2} \proj_{j-s} \grave { \Delta}_j^{(d)} \right\|^2_{4\kappa} = \sum_{j=m}^{n-m}\frac{\omega^2(t,j)}{4} \lt\|\proj_{j-s} \grave { \Delta}_j^{(d)} \rt\|^2_{4\kappa} = O\left(\frac{1}{n\tau_n m^2}\left(\sum_{i=-m+1}^{m} (i+s)^{d-1}\right)^2 \right).\label{eq:W21d1}
\end{align}
Therefore, by \eqref{eq:W21d1}, we have 
\begin{align}
     \left\| W_{21}^{(d)} \right\|_{4\kappa} = O(N^{d}/\sqrt{n\tau_n}) = O((n\tau_n)^{d-1/2}).\label{eq:W21d}
\end{align}
...
\begin{align}
    \lt\|W_{22}^{(d)} \rt\|_{4\kappa} = O(N^{d-1/2}) = O((n\tau_n)^{d-1/2}).\label{eq:W22d}
\end{align}
Therefore, \eqref{eq:W2d} follows from \eqref{eq:W21d} and \eqref{eq:W22d}.
...
% \begin{align}
% \sup_{t \in \I}\lt\|(\breve { \Sigma}_d(t) - { \Sigma}^A (t))1 (A_n)\rt\|_{\kappa} = O(m(n\tau_n)^{2d-1} + \sqrt{m\tau_n^2} + \sqrt{m} (n\tau_n)^{d-1/2}).
% \end{align}
% Finally, by Proposition B.1 in  \cite{dette2018change} and Proposition A.1 in \cite{wu2018gradient}, the lemma is proved.
...
% \begin{proof}
  Recall $\tilde{{Q}}_{k,m}^{(d)} = \sum_{i=k}^{k+m-1}x_i e_i^{(d)}$, $\tilde { \Delta}_{j}^{(d)}=\frac{\tilde{{Q}}^{(d)}_{j-m+1, m}- \tilde{{Q}}^{(d)}_{j+1, m}}{m}$,  $\tilde{{ \Sigma}}_d(t)=\sum_{j=m}^{n-m} \frac{m \tilde { \Delta}_{j}^{(d)}\{\tilde { \Delta}^{(d)}_{j}\}^{\T}}{2}\omega(t, j)$.
  We break the proof into 6 steps. 

\textbf{Step 1}: 
We shall prove that under the bandwidth conditions $\kappa \geq 4/(1/2-d)$, $m/(n\tau_n^2) \to 0$, $\sqrt{m}\tau_n^{2-1/k} \to 0$, $m = O(n^{1/3})$,  $m \to \infty$, $n \tau_n^3 \to \infty$,
\begin{align}
 \sup_{t \in [0,1]} \lt| \hat{{ \Sigma}}_d(t) - \tilde{{ \Sigma}}_d(t) \rt| = \Op\left( \sqrt{m} ((n\tau_n)^{d-1/2} + \tau_n^2)\tau_n^{-1/\kappa} +  m ((n\tau_n)^{2d-1}+\tau_n^4)\tau_n^{-1/\kappa}\right) = \op(m^{2d}).\label{eq:Sigmad_step1}
\end{align}
...
%   \cref{lm:delta_xed} and a careful inspection of Corollary 3 (ii) in \cite{wu2007strong} show that $\sup_j\|\tilde{ \Delta}^{(d)}_j\|_{2\kappa} = O(m^{d-1/2})$. By \cref{physical}, \cref{lm:delta_xed}, \eqref{eq:Ajm} and \eqref{eq:lrv3}, similar to \eqref{eq:h_null}, we have 
% \begin{align}
%   \|h_{s,j}^{(d)}(t) \|_{\kappa}
...
%   Combining \eqref{eq:correctiond_sup}, \cref{lm:correctiond} and \eqref{eq:supd2},  we have the result in \eqref{eq:Sigmad_step1}
 ...
 $
  ...
  by chaining argument in Proposition B.1 in  \cite{dette2018change},  we have 
   \begin{align}
  \sup_{t \in \I} \left |\sum_{j=m}^{n-m} m\omega(t, j) \tilde { \Delta}^{(d)}_{j}A_{j,m}^{\T} \right| = \Op\left(\sqrt{\frac{m}{(n\tau_n)^{1-2d}}}\tau_n^{-1/\kappa}\right).
  \label{eq:supdn2}
  \end{align}
  Combining \cref{lm:correctionlocal} and \eqref{eq:supdn2}, we have shown \eqref{eq:local_step1}.
\par
\par
\textbf{Step 2}:
Define $\bar{{Q}}_{k,m}^{(d)} = \sum_{i=k}^{k+m-1}{ \mu}_W(t_i) e_i^{(d)}$,  $1 \leq k \leq n - m + 1$, $\bar { \Delta}_{j}^{(d)} = \frac{\bar{{Q}}^{(d)}_{j-m+1, m}- \bar{{Q}}^{(d)}_{j+1, m}}{m}$,  $\bar{{ \Sigma}}_d(t)=\sum_{j=m}^{n-m} \frac{m \bar { \Delta}_{j}^{(d)}\{\bar { \Delta}^{(d)}_{j}\}^{\T}}{2} \omega(t, j)$. We shall show that under the bandwidth condition $m \tau_n^{3/2}/\log n\to \infty$, $\kappa \geq 2/(3d)$, \begin{align}
...
\begin{align}
  \max_{1 \leq k \leq n - m + 1}\lt\|\bar{{Q}}_{k,m}^{(d)} - \tilde{{Q}}_{k,m}^{(d)} \rt\|_{2\kappa} = O(\sqrt{m} (\log n)^d).
...
\begin{align}
  \left\| \tilde { \Delta}_{j}^{(d)}\lt\{\tilde { \Delta}_{j}^{(d)}\rt\}^{\T} -  \bar { \Delta}_{j}^{(d)}\lt\{\bar { \Delta}_{j}^{(d)}\rt\}^{\T} \right\|_{\kappa} &\leq \left\| \tilde { \Delta}_{j}^{(d)} -  \bar { \Delta}_{j}^{(d)}\right\|_{2\kappa} \left\|\lt\{\bar { \Delta}_{j}^{(d)}\rt\}^{\T} \right\|_{2\kappa}  + \left\| \tilde { \Delta}_{j}^{(d)} \right\|_{2\kappa} \left\|\lt\{\tilde { \Delta}_{j}^{(d)} - \bar { \Delta}_{j}^{(d)}\rt\}^{\T} \right\|_{2\kappa}\\ &= O(m^{d-1}(\log n)^d).
\end{align}
Since $m \tau_n^{3/2}/\log n \to \infty$, \eqref{eq:Sigmad_step2} follows from triangle inequality and Proposition B.1 in  \cite{dette2018change}.
\par
\par
\textbf{Step 3}:
Let $\zeta_j = \sum_{i=j}^{\infty} \proj_j e_{i}$, $\zeta_j^{\circ} = \zeta_j (t_j) = \sum_{i=j}^{\infty} \proj_j H(t_j,\FF_i)$. Define
\begin{align}
  \overline{Z}_{k,m} = \sum_{j=0}^L \psi_j  \sum_{i=k}^{k+m-1}{ \mu}_W(t_i) \zeta^{\circ}_{i-j},\quad { \Delta}_j^{(d), \circ} = \frac{\overline{{Z}}_{j-m+1,m} -\overline{{Z}}_{j+1,m}}{m}, \quad { \Sigma}_d^{\circ}(t) = \sum_{j=m}^{n-m} \frac{m { \Delta}_j^{(d), \circ} \{{ \Delta}_j^{(d), \circ }\}^{\T}}{2}\omega(t,j).
\end{align}
 Let $L= M m^{1 + \frac{1}{2d+1}} \tau_n^{1/2}$, where $M$ is a sufficiently large constant. We will show that
\begin{align}
  \sup_{t \in \I}|\bar{{ \Sigma}}_d(t) - { \Sigma}_d^{\circ}(t)|  = \Op\left\{m^{2d}\lt(m^{- \frac{1/2-d}{2d+1}}  \tau_n^{d/2-1/4-1/\kappa}\rt) \right\} = \op(m^{2d}).\label{eq:Sigmad_step3}
\end{align}
...
After a careful check on Lemma 2 in \cite{wu2011gaussian}, we have
\begin{align}
...
Following the proof of  Corollary 2  in \cite{wu2011gaussian}, under Assumption  \ref{A:H_long}, we obtain
\begin{align}
...
\eqref{eq:Sigmad_step3} then follows from  Proposition B.1 in \cite{dette2018change}.
\par
\par
\textbf{Step 4}: 
...
\end{align}
 Following similar arguments in the proof of Theorem 3.1 \cite{wu2006invariance}, 
for $0 \leq k \leq \lf \frac{n - m}{2L}\rf$,  let
$D_{k,i} =  \Delta_{2kL+i}^{(d), \circ} \{ \Delta_{2kL+i}^{(d), \circ }\}^{\T}- \E( \Delta_{2kL+i}^{(d), \circ} \{ \Delta_{2kL+i}^{(d), \circ }\}^{\T}|\F_{2kL+i - 2L})$, $i = 0,1,2,\cdots, 2L-1$, and 
$
E_{h} = \E( \Delta_{h}^{(d), \circ} \{ \Delta_{h}^{(d), \circ }\}^{\T}|\F_{h- 2L}) - \E( \Delta_{h}^{(d), \circ} \{ \Delta_{h}^{(d), \circ }\}^{\T})$,  $m \leq h \leq n-m$.
...
Recall that $ \Delta_h^{(d), \circ} = \frac{\overline{{Z}}_{h-m+1,m} -\overline{{Z}}_{h+1,m}}{m}$, in which $$\overline{Z}_{h,m} = \sum_{j=0}^L \psi_j  \sum_{i=h}^{h+m-1}{ \mu}_W(t_i) \zeta^{\circ}_{i-j} = \sum_{j=0}^{L+m-1} p_{j,h + m -1,m} \zeta^{\circ}_{h + m -1 -j},$$
 where $p_{j, h + m - 1,m} = \sum_{i=(j-L)_+}^{(m-1) \wedge j} \psi_{j-i}{ \mu}_W\left(\frac{h + m -1 -i}{n}\right)$,  $\{\zeta^{\circ}_j\}$ are martingale differences. \par Under the geometric measure contraction condition, for $0 \leq j \leq L$, we have
 \begin{align}
...
By Burkholder's inequality, uniformly for all $i$,
\begin{align}
...
\end{align}
where $C$ is a sufficiently large constant. Therefore, since $L /(n\tau_n) = m^{1 + \frac{1}{2d+1}} /(n \tau_n^{1/2}) = O(1/(m\tau_n^{1/2}))$,  by \eqref{eq:Sigmao}, \eqref{eq:Eki} and \eqref{eq:Dki},  and Proposition B.1 in  \cite{dette2018change},  we have shown \eqref{eq:Sigmad_step4}.
...
\begin{align}
  ...
  \begin{cases}
    \sum_{i=0}^{j-1} \psi_{j-i}{ \mu}_W\left(\frac{k-i}{n}\right) + { \mu}_W(\frac{k-j}{n}), & j \leq m - 1\\ 
    \sum_{i=0}^{m-1} \psi_{j-i}{ \mu}_W\left(\frac{k-i}{n}\right), & m \leq j \leq L\\ 
    \sum_{i=j-L}^{m-1} \psi_{j-i}{ \mu}_W\left(\frac{k-i}{n}\right)= O(mL^{d-1}), & j \geq L+1.\\ 
  \end{cases}
\end{align}

Then, approximate $p_{j,k,m}$ by integrals. When $j \leq m - 1$, by the continuity of ${ \mu}_W$ and \cref{lm:basic},
\begin{align}
...
\par
\textbf{Step 6}:
Let $g_{\kappa,n} = m^{-d} (\log n)^{d} \tau_n^{-1/\kappa} +\sqrt{m}( (n\tau_n)^{d-1/2} + \tau_n^2)\tau_n^{-1/\kappa} +  m ((n\tau_n)^{2d-1} + \tau_n^4)\tau_n^{-1/\kappa} + m^{- \frac{1/2-d}{2d+1}}  \tau_n^{d/2-1/4-1/\kappa} +  (m\tau_n^{3/2})^{-1/2} + f_n$.
...
%  \end{align}
% %  where $\mathcal{I} = [\gamma_n, 1-\gamma_n] \subset (0,1)$, $\gamma_{n}=\tau_{n}+(m+1) / n$.
% \end{corollary}
...
\subsubsection{Proof of \texorpdfstring{\cref{lm:Sigma_dn}}{Theorem 4}}
    ...
    Recall that
    $$\tilde{{ \Sigma}}_{d_n}(t)=\sum_{j=m}^{n-m} \frac{m \tilde { \Delta}_{j}^{(d_n)}\{\tilde{ \Delta}^{(d_n)}_{j}\}^{\T}}{2}\omega(t, j),$$ where $\tilde { \Delta}_{j}^{(d_n)}=\frac{\tilde{Q}^{(d_n)}_{j-m+1, m}- \tilde{Q}^{(d_n)}_{j+1, m}}{m}$, $\tilde{Q}^{(d_n)}_{k, m}=\sum_{i=k}^{k+m-1} x_{i}e_i^{(d_n)}$.\par We break the proof in the following 8 steps.
...
By \cref{lm:psum_dn} and \eqref{eq:diffQdn}, we  have  
  ...
  % Using similar techniques in proposition 5.3(just as corollary of propoisition 5.2)  cannot yield o(1) after chaining argument.
  % In the step 2 of Lemma 5(\cref{lm:Sigma_d}), we actually do not tackle problem. We assume the existence $kappa$ moment.
    Under Assumptions \ref{Ass-W} and  \ref{assumptionHp}, by Lemma 7 in \cite{zhou2014vstat} we have for $l, j>0$,
    \begin{align}
...
%   & = O\left(m^{-3/2} d_n  \sum_{i=-m+1}^{m}\min\left\{ \chi^{i-L+s}L^{d_n-1}, (i+s)^{d_n-1}\right\} 1(i+s > 0 )\right).
 & = O\left(m^{-1/2} r_{i,s,n}\right).
  \label{eq:Pvv}
  \end{align}
   Under \cref{Ass-E}, similar to (36) in Lemma 3 in \cite{zhou2010simultaneous}, we have
   \begin{align}
...
%   where $$\check { \Sigma}(t) =  { \Sigma}(t) + (e^{c\alpha}-1)^2\sigma_H^2(t) { \mu}_W(t) \mu^{\T}_W(t) + (e^{c\alpha}-1)s_{UH}(t) { \mu}_W^{\T}(t) + (e^{c\alpha}-1){ \mu}_W(t)s^{\T}_{UH}(t).$$
  ...
  Summarizing Step 1 - Step 7, we have 
  \begin{align}
  \sup_{t \in \I} |\hat { \Sigma}_{d_n}(t) - \check { \Sigma}(t)| =\Op\left(\sqrt{\frac{m}{n\tau_n^{1+2/\kappa}}} + \sqrt{m}\tau_n^{2-1/\kappa} +\sqrt{\frac{1}{m\tau_n^{3/2}}} + (\log n)^{-1}\right) = \op(1).\label{eq:localdrate}
  \end{align} \hfill $\Box$
...
%   \begin{align}   
%    \sup_{t \in \I}\left|\hat{\sigma}^2_{d_n}(t) - e^{2c\alpha_1}\sigma_H^2(t)\right|  = \Op\left(\sqrt{\frac{m}{n\tau_n^2}}  +\sqrt{\frac{1}{m\tau_n^{3/2}}} + \log^{-1} n\right) = \op(1).
%  \end{align}
%   \end{corollary}
% \begin{proof}[Proof]
...
%      %   \end{proposition}
%      %   \begin{proof}
%      %     The proposition follows after a careful investigation of Theorem 1 in \cite{wu2005nonlinear}.
%      %   \end{proof}
...
%      %    \begin{align}
%      %     \frac{\Gamma(j+d)}{\Gamma(j+1)}= \frac{\sqrt{\frac{2\pi}{j+d}}(\frac{j+d}{e})^{j+d}(1+O(\frac{1}{j+d}))}{\sqrt{\frac{2\pi}{j+1}}(\frac{j+1}{e})^{j+1}(1+O(\frac{1}{j+1}))} &= \frac{j^{d-1}(1 + d/j)^j(1 + d/j)^{d-1/2}(1+O(1/j))}{e^{d-1}(1 + 1/j)^j(1 + 1/j)^{1/2}(1+O(1/j))}\\
%      %     & = j^{d-1} + O(j^{d-2}).
%      %    \end{align}
%      %      Since $\ln \Gamma(z) \sim z \ln z-z+\frac{1}{2} \ln \frac{2 \pi}{z}+\sum_{n=1}^{N-1} \frac{B_{2 n}}{2 n(2 n-1) z^{2 n-1}}$, the constant in the big $O$ of $\Gamma(z)=\sqrt{\frac{2 \pi}{z}}\left(\frac{z}{e}\right)^{z}\left(1+O\left(\frac{1}{z}\right)\right)$ is always $B_2/2 = 1/12$.
%      %   \end{proof}
...
%      %   \begin{remark}
%      %     These two results correspond to the conclusions in Lemma 2 of \cite{shao2007local}. 
%      %   \end{remark}
...
%      %     Suppose ${ \Sigma}(t)$ has $q$ distinct eigenvalues, $\tilde \lambda_1(t) > \cdots > \tilde \lambda_q(t)$. Let $\mathbb{Q}(t) = \{k: \exists j \neq i, \lambda_j(t) = \lambda_i(t)  = \tilde \lambda _k(t)\}$. 
%      %       Let
%      %      \begin{align}
...
%      %      \end{align}
%      %      By Corollary 1 in \cite{yu2015useful}, if $k \not \in \mathbb{Q}(t)$, suppose $\lambda_i(t) = \tilde \lambda_k(t)$. Then, we have
%      %     \begin{align}
...
%      %     \end{align}
%      %     If $j \in \mathbb{Q}(t)$, suppose  $\lambda_{r-1}(t) > \lambda_r(t) = \cdots = \tilde \lambda_j(t)  = \cdots = \lambda_s(t) > \lambda_{s+1}(t) $, and let ${V}_j(t) = ({v}_r(t), \cdots, {v}_s(t))$. Let $\hat{V}_j(t) = (\hat {v}_r(t), \cdots , \hat {v}_s(t))$.
%      %       By Theorem 2 in \cite{yu2015useful}, $\exists \hat{O}_j(t) \in \R^{(s-r+1) \times (s-r+1)}$ which is orthogonal, s.t.
%      %     \begin{align}
...
\begin{proof}
Under \cref{assumptionHp}, by Lemma 3.2 of \cite{KOKOSZKA199519} and \cref{physical}, we have
\begin{align}
...
\begin{proof}[Proof]
  ...
  \begin{align}
    \|H^{(d)}(t_i,\FF_i)\|^2_{2p}
    &\leq  M\left\|  \sum_{j\in \mathbb{Z}} \left(\mathcal{P}_j H^{(d)} (t_i, \mathcal{F}_i) \right)^2 \right\|_{p} \leq  M \sum_{j\in \mathbb{Z}} \left\| \mathcal{P}_j H^{(d)} (t_i, \mathcal{F}_i) \right\|_{2p}^2  = O(1),
  \end{align}
  where $M$ is a sufficiently large constant.
