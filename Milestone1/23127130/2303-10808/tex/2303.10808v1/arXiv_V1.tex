...
\end{equation*}
where $k_0 = \lfloor n\varepsilon_0\rfloor $ denotes the unknown location of the change point and $\varepsilon_0 \in (0,1)$.
% List classical literature here
Change point testing is a classical statistical problem which dates back to \cite{page1954continuous,page1955test} and there is a vast amount of literature in both statistics and econometrics for fixed-dimensional or low-dimensional data. For low-dimensional time series, we refer the readers to  \cite{aue2013structural} and \cite{casini2019} 
for excellent reviews of the subject and the huge literature cited therein.

%\cite{aue2009break}, %\cite{shao2010testing},\cite{kirch2015detection}, %\cite{zhang2018unsupervised} for some 
%For
...
%(among many others) for some recent work and Perron (2006) and Aue and Horv´ath (2013) for excellent reviews and the huge literature cited therein

%For example, \cite{andrews93} proposes a classical method that uses the maximum of the likelihood ratio test over a pre-specified values set of the parameter. The counterpart using Wald test or the LM tests is also discussed, and all their limiting distributions are shown to be dependent on the parameter set. We refer the readers to the survey by \cite{aue2013structural} for more relevant work for the low-dimensional data.


% List recent literature here

In the past decade, there is a surge of interest in developing change point testing/estimation methods for high-dimensional data, as motivated by the increasing need for the analysis of  high-dimensional data with change points from many scientific areas, such as genomics, neuroimaging and economics. Here we shall mention recent contributions by \cite{horvath2012change}, \cite{chan2013darling}, \cite{jirak2015uniform}, \cite{cho2016change}, \cite{wang2018high}, \cite{enikeeva2019high}, \cite{dette2020likelihood}, \cite{wang2020dating}, \cite{wang2022inference}, \cite{yu2021finite}, \cite{zhang2021adaptive}, among others. Note that some of the above-mentioned works specifically target high-dimensional independent data, and do not allow for temporal dependence.
For change point testing/estimation of high-dimensional time series (i.e., with temporal dependence), see \cite{jirak2015uniform}, \cite{dette2020likelihood}, \cite{wang2022inference}, \cite{chen2021inference}, to name a few.

A common feature of the methods developed for high-dimensional time series is that they need to handle the estimation of high-dimensional long run variance (LRV) matrix. Consistent estimation of LRV is a thorny issue in practice, as the choice of bandwidth is a notoriously difficult one and becomes especially challenging with the presence of change points and high-dimensionality. Recently, self-normalization (SN, hereafter) based inference, which avoids  direct consistent estimation of LRV,   has been extended to high-dimensional change point detection problem in \cite{wang2022inference} and \cite{zhang2021adaptive}.  We shall refer to \cite{liu2021high} for a timely review of the recent literature on high-dimensional change point testing and estimation.



%Change point detection is still a very active research field nowadays because of its broad application in signal processing, biological sciences and many other fields that usually involve high-dimensional data, which leads a surge of interest in change point detection in the high-dimensional regime. \cite{horvath2012change} targets at the panel data where the errors admit a linear process and investigates the limiting null distribution of a test that is based on the quasi-maximum likelihood argument. \cite{jirak2015uniform} constructs a test based on the maximum of coordinate-wise CUSUM-statistics and established its limiting null distribution as both the sample size and dimensionality tend to infinity. A different way to borrow coordinates' information is proposed in \cite{wang2018high}, which is a two-stage procedure called `inspect'. The core idea is that a good projection direction can be obtained as the leading left singular vector of the matrix that solves a convex optimization problem derived from the cumulative sum transformation of the time series. Thus the first stage of `inspect' is to project the data along this direction whereas the second step is to apply the existing univariate change point estimation algorithm to the projected series. More recently, \cite{wang2022inference} proposed a new test which is based on U-statistic in \cite{chen2010two} and the self-normalization principle in \cite{shao2010self}. To accommodate the weak temporal dependence within the high-dimensional time series, a trimming parameter is further introduced. The asymptotic theory of the proposed test is established under appropriate technical assumptions. As an extension, \cite{zhang2021adaptive} integrates the U-statistic based method in \cite{wang2022inference} and the $L_q$-norm based high-dimensional test in \cite{he2021asymptotically} to proposed an adaptive test, which is shown to be powerful against both sparse and dense alternatives.     Also, 
% Discuss the inherent drawback of all the existing dimension-specific methods


For change point testing of multivariate time series,  the literature is naturally divided into two categories: methods developed and justified for low-dimensional time series, and methods that can accommodate high dimension and allow (or require) the dimension $p$ to be comparable to or exceeds sample size $n$. Most of the existing methods are designed for a specific dimensional regime (i.e., either low/fixed dimension or high/growing dimension).
%it is obvious that most existing method targets at a specific dimensional regime and the established asymptotic theory holds only when some specific assumptions of the data dependence are satisfied. Usually 
%Specifically, the methods developed for the high-dimensional setting are substantially different from those proposed for low/fixed dimensional problems
On one hand, the methods developed for low/fixed-dimensional problems may not be theoretically justified or even applicable when $p > n$. For example, the SN-based test in \cite{shao2010testing} works quite well when $p$ is relatively small compared to $n$, but it is no longer applicable when $p >n$ due to the non-invertibility of self-normalizer and can exhibit serious size distortion when $p$ is moderate relative to $n$ (e.g., $p=10$ and $n=100$) in the presence of moderate/strong temporal dependence. On the other hand, the method developed in the high-dimensional setting may not work for the low-dimensional data,  as the approximation accuracy of some test statistics highly relies on the central limit effect from the high dimension and the size can be quite distorted for data of low or moderate dimension. In addition, for the same test statistic, the limiting null distributions under different asymptotic regimes are usually different, which results in practical difficulty in using these tests as the calibration usually depends on the regimes, which are unknown in practice. This naturally motivates the question whether there is a change point testing procedure that can work for weakly dependent time series of low, medium and high dimensions, that is, be dimension-agnostic? 


%Motivated by these observations, it is natural to ask whether there exists an  SN-based methodology that can work for all settings, including  low, medium and high dimensions, weak and strong panel dependence? In this proposal, we aim to propose a new class of methodology based on sample splitting and self-normalization (SS-SN, hereafter) that satisfactorily address the above question. Our proposal is inspired by recent work by \cite{kim2020dimensionagnostic}, who first proposed this notion of dimension agnostic inference in nonparametric testing problems. 


The dimension-agnostic property of an inference procedure has been paid attention to in \cite{paindaveine2016high}, \cite{wasserman2020universal} but it was not until \cite{kim2020dimensionagnostic} who formalized the notion of dimension-agnostic inference, and proposed a sample splitting  approach to several nonparametric testing problems. As argued in their paper, many test statistics developed in the literature often have a different limiting distribution in a fixed-dimensional regime ($p$ is fixed as $n\rightarrow\infty$) or a high-dimensional regime ($\min\{n,p\}\rightarrow\infty$ together at some relative rate). This typically leads to different calibration threshold (or critical values) and the overall rejection rule is different in two regimes. But in practice, suppose we are given a dataset with (say) 120 samples in $30$ dimensions, should we calibrate assuming $p$ is fixed, or $n \gg p$, or $p/n\rightarrow 0.25$? 
The goal of dimension-agnostic inference is to develop a test statistic whose limiting null distribution is the same regardless of how the dimensionality $p$ scales with respect to sample size $n$. A major benefit of the dimension-agnostic inference is being consistent against various dimensional regimes, and the selection of dimension-dependent calibration threshold can be avoided.



%The goal of dimension-agnostic inference is to develop a test statistic whose limiting null distribution is the same regardless of how $p$ scales with respect to $n$. 


%However, there is no exact boundary dividing the dimensional regimes apart in real world. With limited prior knowledge of a random data, we may hardly determine whether the high-dimensional method or the low-dimensional method works better. Motivated by these observations, it is natural to ask whether there exists some methodology that can apply for all settings, including low, medium and high dimensions, weak and strong panel dependence? Inspired by recent work of \cite{kim2020dimensionagnostic}, we are able to propose a dimension-agnostic change point test by incorporating the idea of sample splitting in that article and the self-normalization technique by \cite{shao2010self}. 


% Introduce \cite{kim2020dimensionagnostic} and dimension-agnostic

%\hg{duplicated paragraphs above and below?}



% Such property of an inference procedure has been paid attention to in \cite{paindaveine2016high}, \cite{wasserman2020universal} but it was not until \cite{kim2020dimensionagnostic} who formalized the notion of dimension-agnostic inference, and proposed sample splitting based approach to several inference problems in both low-dimensional and high-dimensional statistics. 

%As argued in their paper, many test statistics developed in the literature often have a different limiting distribution in a fixed-dimensional regime ($p$ is fixed as $n\rightarrow\infty$) or a high-dimensional regime ($\min\{n,p\}\rightarrow\infty$ together at some relative rate). This typically leads to different calibration threshold (or critical values) and the overall rejection rule is different in two regimes. 


% Introduce the key idea of the proposed method; illustrate the novelty and significance of the proposed method

In this article, we advance the dimension-agnostic inference to change point testing in time series for the first time.
%borrow the idea of sample splitting introduced in \cite{kim2020dimensionagnostic} to find an appropriate direction along which to project the multivariate data. With the univariate projected data, we incorporate the trimming technique utilized in \cite{wang2022inference} and the one-dimension self-normalizer proposed in \cite{shao2010self} to construct the test statistic. As will be shown later, we consider both the dense and sparse alternatives, and will generalize the test for the single change point problem to the multiple change points scenario through wild binary segmentation by \cite{fryzlewicz2014wild}. To the best of our knowledge, this is the first time a dimension-agnostic procedure for change point testing is developed.
Although our work builds on \cite{kim2020dimensionagnostic} and follows their sample splitting and projection approach, it differs from their work in several fundamental ways. First, we deal with the time series data, whereas independent data is the sole focus in \cite{kim2020dimensionagnostic}. Note that time series has a natural ordering which implies the relatively limited way of sample splitting. For the iid data, sample splitting is not unique and it remains to develop a way of combining different sample splits for the purpose of dimension-agnostic inference, whereas for time series, there is no ambiguity with sample splits (e.g., an equal-sized sample split with time series is unique) so the concern over practical replicability due to randomness of sample splits is minimal. It is thus natural to use sample splitting for the purpose of dimension-agnostic inference in the time series setting. Second, we need to make some methodological adjustments to accommodate temporal dependence and a broad range of dimensionality. In particular, we need to introduce a trimming parameter to control the bias incurred by weak temporal dependence . Also the self-normalizer used in the mean inference problem of \cite{kim2020dimensionagnostic}
is the classical studentizer in the formation of $t$ statistic, whereas we adopt the self-normalizer for change point testing for time series as used in \cite{shao2010testing}.
%Self-normalization for time series has been developed initially in \cite{shao2010self} and was extended to change point testing by \cite{shao2010testing}. 
%This leads to we use is different across problems, which is a feature in SN-based time series inference whereas the $t$ statistic was used in \cite{kim2020dimensionagnostic}.
Third, the theoretical argument is very different. The argument in \cite{kim2020dimensionagnostic}, which relies on the pointwise Berry-Esseen bound conditional on the half of the sample, no longer applies to the time series setting. To alleviate the difficulty, we develop some new conditioning argument to study the large sample behavior of our proposed test statistic. 
Fourth, an added benefit is that our proposed test is not only dimension-agnostic, but also agnostic to the degree of panel (i.e., cross-sectional) dependence. The robustness to the dimension and degree of panel dependence does not come for free and there is indeed a price to pay. As we demonstrate in simulation studies, there is a certain degree of power loss in some settings, which is in part due to the use of sample splitting, but the efficiency loss seems moderate in many scenarios. These findings are in general agreement with 
and an important complement to those in \cite{kim2020dimensionagnostic}.
%may be alleviated by combining multiple sample splits.




% Organization of the paper
The rest of the article is organized as follows. In Section \ref{Sec:Methodology}, we introduce our dimensional-agnostic test statistic for a single change point alternative and state the main assumptions for three data generating processes that encompass both fixed/growing dimensional regimes and weak/strong cross-sectional dependence. Section~\ref{Sec:Theory-null} and  Section~\ref{Sec:Theory-power}  present the main theory for our single change point test under the null hypothesis and alternative hypothesis, respectively. Some extensions, including multiple change point testing, are discussed in Section~\ref{Sec:Extension}. Numerical results are gathered in Section~\ref{Sec:NumResults-simul}. 
Section~\ref{Sec:Discussion} concludes. 
All technical proofs are relegated to the supplement. 

%start from the case with a single change point and introduce the procedure of sample splitting. The test statistics targeting at the dense and sparse alternatives are proposed respectively. Then we generalize the testing procedure to the scenario of multiple change points by incorporating the method of wild binary segmentation. The theoretical results are presented in Section \ref{Sec:NumResults-simul}. Finally, some discussion and potential future work are summarized in Section \ref{Sec:Discussion}.
...
%\hg{Most of the technical assumptions are not yet provided in this section, maybe change the section title as ``Methodology and Data Structures"? Additionally, maybe we can make the methodology into a subsection?}

In this section, we introduce the ``SS-SN" (i.e., sample splitting and self-normalization) methodology  for single change point testing in the mean of a multivariate time series. Our test  targets the dense alternative in the mean, and  we shall leave the tests for sparse alternative or multiple change points  to Section~\ref{Sec:Extension}.

%\xs{add a brief review of the method in \cite{kim2020dimensionagnostic} for one sample mean inference}
...
Based on $f_{\mbox{mean}}(\cdot)$, \cite{kim2020dimensionagnostic} define a bilinear statistic
\begin{equation*}
    \bar{f}_{\mbox{mean}}  
  = \lrp{\frac{1}{n_1} \sum\limits_{i=1}^{n_1} X_i}^{\top} \lrp{\frac{1}{n_2} \sum\limits_{j=n_1+1}^{n} X_j},
\end{equation*}
...
%Based on the asymptotic theory established in \cite{kim2020dimensionagnostic}, under mild moment condition on the Lyapunov ratio, , regardless of $p$. The asymptotic power expression is also derived and it is shown that the power is asymptotically maximized when $m_1/n \rightarrow 1/2$. Their main assumpti Assumption 2.2




%One case to be discussed is that has a single change point in mean. In this case, we propose the tests targeting at the dense and sparse alternative respectively. Another case is that has multiple change points, and we generalize the proposed method to accommodate this case as well.


%----------------------------------------------------------------
%\subsection{Proposed Test Targeting at Single Dense Alternative}\label{Sec:Methodology-dense}

%We first look into the scenario with a single change point that is dense in mean. 

%----------------------------------------------------------------
\subsection{Methodology}\label{Sec:Methodology-dense}

Our test statistic is formed by following the steps below:
...
\end{equation*}
%where $m = \lfloor{ n\varepsilon \rfloor}$ and $N = n-2m$ denote the size of middle block. 
...
Initially, we plan to use $\cx_1$ and $\cx_3$ to form an estimate of $\mu_1-\mu_n$, say $m^{-1}\sum_{t=1}^{m}X_t-m^{-1}\sum_{t=n-m+1}^{n}X_t$, and then project the data in the middle block $\cx_2$ onto this direction. This would be a direct extension of the splitting and projection idea presented in \cite{kim2020dimensionagnostic}. However, as demonstrated in \cite{wang2020hypothesis} and \cite{wang2022inference}, temporal dependence of high-dimensional time series may result in a bias term for the U-statistic based estimate. It turns out that to make the bias asymptotically negligible in our setting,  it requires a very stringent assumption on the growth rate of $p$ as a function of $n$. To this end, we introduce a trimming parameter $\eta \in (0, \varepsilon)$. 
%As investigated by \cite{wang2022inference}, the temporal dependence of the high-dimensional data may lead to severe bias in subsequent analysis, we thus incorporate the trimming technique to accommodate such dependence. To be specific, we denote by $\eta \in (0,\varepsilon)$ the trimming ratio, then
Define $m_1 = \lfloor{ n(\varepsilon-\eta) \rfloor}$ and $m_2 = m - m_1$, then $\cx_1$ and $\cx_3$ can be respectively partitioned into two smaller pieces, i.e. $\cx_1 = \cx_{11} \cup \cx_{12}$ with $\cx_{11} = \lrcp{X_1, \cdots, X_{m_1}}$ and $\cx_{12} = \lrcp{X_{m_1+1}, \cdots, X_{m}}$, whereas $\cx_3 = \cx_{31} \cup \cx_{32}$ with $\cx_{31} = \lrcp{X_{n-m+1}, \cdots, X_{n-m_1}}$ and $\cx_{32} = \lrcp{X_{n-m_1+1}, \cdots, X_{n}}$. 
We shall use the data in $\cx_{11}$ and $\cx_{32}$ to estimate $\mu_1-\mu_n$ and then project the resulting estimate to the middle block $\cx_2$. For convenience, we shall call $\varepsilon$ splitting parameter and $\eta$ trimming parameter. 

\item  {\bf Projection:} 
%As a direct application of trimming, $\cx_{12}$ and $\cx_{31}$ will be excluded for subsequent analysis. 
...
\item {\bf Forming a studentized test statistic:} After dimension reduction and projection, we end up with a univariate series $\{Y_t\}_{t=1}^{N}$. Following the insights provided in \cite{kim2020dimensionagnostic}, studentization is a key for dimension-agnosticness of their test. For mean testing, the classical $t$ statistic was used in \cite{kim2020dimensionagnostic} for the projected data since the data are iid and the testing problem is one sample. By contrast, we are dealing with time series and a change point testing problem, so some modification needs to be made. Specifically, we shall apply the SN-based test statistic in \cite{shao2010testing} to the projected data $\{Y_t\}$.
%Based on the obtained scalars, it is natural for us to apply the self-normalization method, which is designed for the univariate change point testing problem in \cite{shao2010self}, to construct the test statistic. 
For $1\le a\le b\le N$, with the cumulative sum defined as $S_{a,b} = \sum_{j=a}^{b} Y_j$, we further define
\begin{equation*}
    T_n(k)
...
\begin{rmk}{\rm 
Self-normalization for time series \cite{shao2010self} is an inference technique  that has been developed for low and fixed-dimensional parameter in a low dimensional time series, following some early developments by \cite{kvb2000} and \cite{lobato2001testing}. It uses an inconsistent variance estimator to yield an asymptotically pivotal statistic, and does not involve any tuning parameter or involves less number of tuning parameters compared to traditional procedures. See \cite{shao2015self} 
for a comprehensive review of recent developments for low dimensional time series. There have been two recent extensions to the high-dimensional setting: \cite{wang2020hypothesis} adopted  a one sample U-statistic with trimming and extended self-normalization to inference for the mean of high-dimensional time series; \cite{wang2022inference} used a two sample U-statistic and extended the self-normalization (SN)-based change point test in \cite{shao2010testing} to high-dimensional independent and dependent data.  Note that the test in \cite{wang2022inference}  is not expected to work for low dimensional time series as the theory requires the dimension to grow and the approximation by the limiting null distribution may be inadequate when the dimension is low or moderate. Additionally, the applicability of their test is limited to time series with weak cross-sectional dependence. 
%In particular, for the mean change point testing of high-dimensional time series, \cite{wang2022inference} developed their test statistic based on a trimmed U-statistic and self-normalization. The performance of their test can be sensitive to the choice of trimming parameter, which does not seem to have practical guideline. 
We shall present some simulation comparison in Section~\ref{Sec:NumResults-simul}.
}
\end{rmk}

\begin{rmk}

{\rm  Sample splitting has been widely used in statistics and machine learning but it seems that most of its use is for independent data; see  \cite{shafer2008tutorial}, \cite{wasserman2009high}, \cite{rinaldo2019bootstrapping}, \cite{wasserman2020universal}, among others. In the context of time series, we are only aware of  \cite{lunde2019sample} and \cite{chang2022testing}. Specifically, sample splitting was used for the post-selection inference in the time series setting in \cite{lunde2019sample} and for unit root testing in \cite{chang2022testing}. The scope and property of our proposed SS-SN inference are very different from these papers and have no overlap with the existing literature.
}
\end{rmk}
...
%\xs{comment on computationally complexity, and compare with \cite{wang2022inference}, the latter of which is of order $O(pn^2)$; see Section 2.1 of \cite{wang2022inference}.}

%Note that the proposed can be written in terms of $\{S_{1,k}\}_{k=1}^{N}$, thus we compute $\{S_{1,k}\}_{k=1}^{N}$ first. The calculation of $\hat\mu_1,\hat\mu_n$ can be done with the cost $O(n)$ and afterwards, the calculation of $\{Y_j\}_{j=1}^{N}$ can be done with the cost $O(np)$. Once all the $Y_j$'s are calculated, $\{S_{1,k}\}_{k=1}^{N}$ can be obtained at the cost of $O(n^2)$. Finally, the test statistic can be computed with the cost $O(n^2)$. In summary, the total computational complexity of the proposed method is of order $O(n (n+p))$, which is significantly efficient than the method with computation cost $O(n^2p)$ proposed in \cite{wang2022inference}, especially when the sample size is large and the data dimensionality is high.
...
Since our main goal is to show that our test statistic works for both fixed-dimensional and growing-dimensional time series, and for time series with both weak cross-sectional dependence and strong cross-sectional dependence, we shall consider three types of data generating processes in this paper.
As summarized in 
%Given the observations $\{X_t\}_{t=1}^{n}$, we consider three different data structures throughout this paper, which are summarized as 
Table \ref{Tab:DataStructure}, we consider three cases for $\{X_t-\mu_t\}$: (1) stationary weakly dependent time series with fixed $p$ and arbitrary cross-sectional dependence; (2) linear process with growing $p$ that allows for weak temporal and cross-sectional dependence. This is similar to the setting in \cite{wang2020hypothesis}, where nonlinear casual process was assumed. The results are expected to hold under the more general nonlinear process but at the expense of more complicated technical arguments; (3) static factor model for high-dimensional time series which can accommodate strong cross-sectional dependence and weak temporal dependence. 
...
%the three structures to be discussed range across 
...
\begin{table}[h!]
    ...
    \hline\hline        
    \end{tabular}
    \caption{Three types of data generating processes}
    \label{Tab:DataStructure}
\end{table}

Below we shall present the exact requirements for each data generating process. 

%Specifically, the first is a stationary sequence with a fixed dimensionality $p$, as stated in the following definition.

%\hg{We name the following statement a definition or an assumption?}

\begin{defi}[DGP1]\label{Def:Model_Fixedp}
Assume that $X_t-\mu_t \in \br^p, t\in \fZ$ is a stationary sequence with $\be[X_t] = \mu_t$ and that the long-run variance $\Omega^{(1)} = \sum\limits_{k=-\infty}^{\infty} \Cov(X_t,X_{t+k})$ is a positive definite matrix. Further assume that $p$ is fixed and  $\frac{1}{\sqrt{n}} \sum\limits_{t=1}^{\lrfl{nr}} (X_t-\mu_t) \leadsto (\Omega^{(1)})^{1/2} B_p(r)$ in $D[0,1]$ as $n\rightarrow\infty$.
\end{defi}
Throughout the paper, we use $D[0,1]$ (or more generally, $D[a,b]$) to denote the space of $R^{p}$-valued ($p\in\fN$) functions on $[0,1]$ (or $[a,b]$) which are right continuous with left limits, endowed with the Skorokhod topology \cite{billingsley2008probability}. 
The assumption that the long-run variance matrix $\Omega^{(1)}$ is positive definite is a common assumption in SN-based inference \cite{shao2010self, shao2015self} and is widely used in the literature of time series analysis. The functional central limit theorem (or invariance principle) is a high-level assumption that can be verified for weakly dependent time series that satisfy certain mixing or near epoch dependence assumptions; see  \cite{lobato2001testing} and \cite{shao2010self} for related discussions. Note that there is no particular restriction on the cross-sectional dependence other than the positive definiteness of $\Omega^{(1)}$. 

%which is mild and reasonable.

Next, we introduce the second data generating process which requires the dimension $p$ to grow with sample size $n$. 

%The second data structure to be considered is the linear process with a diverging dimensionality $p$, see Definition \ref{Def:Model_lp}.

\begin{defi}[DGP2]\label{Def:Model_lp} 
...
The sequence of coefficient matrices $\{a_j\}_{j=0}^{\infty}$ determines the temporal dependence of the series, whereas the cross-sectional dependence of $X_t$ is jointly determined by  the covariance matrix $\Gamma^{(2)}$ and the coefficient matrices $\{a_j\}_{j=0}^{\infty}$. Additional assumptions will be imposed later to 
make sure that both temporal dependence and  cross-sectional dependence are weak.
...
%The last and the most involved data structure discussed in this paper is the factor structure with the dimensionality $p$ growing to infinity, as defined in Definition \ref{Def:Model_factor}.

\begin{defi}[DGP3]\label{Def:Model_factor}
Assume that 
...
The factor model is commonly used to fit large-dimensional time series from economics and finance, when strong cross-sectional dependence is expected. Mathematically, 
$\cov(X_t)=\Lambda \cov(F_t) \Lambda^T +\cov(Z_t)$, thus the strong cross-sectional dependence of $X_t$ originates from the low-rank coefficient matrix $\Lambda$. Since both $\{F_t\}$ and $\{Z_t\}$ are weakly dependent over time and they are mutually independent,  the original series $\{X_t\}$ also has weak temporal dependence. 

% and attains weak temporal dependence from the component of the linear process. 

%\hg{Clarification of $\Sigma$,$\Gamma$,$\Omega$.

%In summary, $\Omega$ occurs in DGP1\&3, $\Gamma$ only occurs in DGP2 and $\Sigma$ occurs in DGP2\&3.
%\begin{enumerate}[label=(\roman*)]
 %   \item $\fOmega$: For DGP1, we use $\Omega \in \br^{p\times p}$ (fixed $p$) to denote the LRV of the stationary sequence $\{X_t\}_{t=1}^{n}$. For DGP3, we use $\Omega \in \br^{s\times s}$ (small fixed $s$) to denote the variance of the low-dimensional factor series $\{F_t\}_{t=1}^{n}$. 
 %   \item $\fGamma$: In DGP2, we use $\Gamma \in \br^{p\times p}$ (growing $p$) to denote the variance of the iid sequence $\{\varepsilon_t\}_{t\in\bz}$ in the linear process.
 %   \item $\fSigma$: To deal with DGP2, we start from an iid sequence $\{X_t\}_{t=1}^{n}$ with variance $\Sigma \in \br^{p \times p}$ (growing $p$). In DGP3, we use $\Sigma \in \br^{p \times p}$ (growing $p$) to denote the variance of the linear process $\{Z_t\}_{t=1}^{n}$.
%\end{enumerate}
%In general, $\Omega$ is used for variance/long-run variance of the low/fixed dimensional sequence, $\Sigma$ is used for variance of linear-process-related high-dimensional sequence. $\Gamma$ has no duplicated usage.

%If needed, maybe we can use $\Omega^{(1)}$/$\Omega^{(3)}$, $\Sigma^{(2)}$/$\Sigma^{(3)}$ to distinguish the notations, or $\Omega$/$\tilde{\Omega}$, $\Sigma$/$\tilde{\Sigma}$.}

%\xs{I think this would be a good idea, maybe use $\Omega_1$, $\Omega_2$, $\Omega_3$ etc. For DGP$_j$, all the involved quantities are $\Omega_j$, $\Sigma_j$ etc. }
...
\begin{table}[h!]
    ...
    \hline\hline
    \end{tabular}
    \caption{Simulated critical values for $G$}
    \label{Tab:critval}
\end{table}


%\xs{is the critical values from shao and zhang, or simulated by you? We need to present the critical values in a table if you simulated it.}




%theoretical results. The three data structures with different dimensional settings and dependence structure are introduces at the beginning. Based on the three structures of interest, we establish the limiting distribution of the proposed test statistic under the null hypothesis, which turns out to be pivotal and and verifies that the proposed test is dimension-agnostic. Under the scenario of a single dense alternative, we additionally provide the power analysis. The power analyses of the case with a single sparse alternative as well as the scenario of multiple change points are left as future work due to the foreseeable technical difficulties.
...
%\xs{can we illustrate the key steps used in the conditioning argument in the fixed-dimensional setting.  i think this might help to convey the novelty of our theory.}

In this section, we present the limiting theory under  the first DGP and a sketch of the proof. The main novelty of the proof lies in the use of a conditioning argument on the asymptotic null distribution. To be specific, we can first apply the Functional CLT in Definition \ref{Def:Model_Fixedp} and the continuous mapping theorem to obtain that 
\begin{equation*}
    G_n = \sup\limits_{k=1,\cdots,N-1} T_n(k) V_n^{-1/2}(k) \stra{d}   M(b(\varepsilon,\eta), \cb),
\end{equation*}
where 
$b(\varepsilon,\eta) = \frac{1}{\varepsilon-\eta} \lrp{B_p(\varepsilon-\eta) + B_p(1) - B_p(1-\varepsilon+\eta)}$,  $\cb = \{B_p(s)\}_{\varepsilon \le s \le 1-\varepsilon}$ and 
{\small \BEqn
& & M(b(\varepsilon,\eta),\cb) \\
&=& \sup_{r\in[0,1]}
    \frac{b^{\top}(\varepsilon,\eta) \Omega^{(1)} \bigp{B_p(\varepsilon+(1-2\varepsilon)r) - B_p(\varepsilon) - r\lrp{B_p(1-\varepsilon) - B_p(\varepsilon)}}}
    {\lrp{
     \begin{array}{l}
     ~ ~ \int_{0}^{r} \lrp{b^{\top}(\varepsilon,\eta) \Omega^{(1)} \bigp{B_p(\varepsilon+(1-2\varepsilon)s) - B_p(\varepsilon) - \frac{s}{r}\lrp{B_p(\varepsilon+(1-2\varepsilon)r) - B_p(\varepsilon)}}}^2 ds \\[2mm]
...
Using the independent increment property of the Brownian motion, it is easy to see that $b(\varepsilon,\eta)$ is independent of $\cb$, thus if we can show that 
\begin{equation}\label{eq:conddist}
    M(b(\varepsilon,\eta), \cb) |_{b(\varepsilon,\eta)=b_0} 
=^d \sup\limits_{r\in[0,1]} \lrp{B(r) - r B(1)} V^{-1/2}(r)=G,
\end{equation}
which does not depend on $b_0$, then the unconditional distribution of $M(b(\varepsilon,\eta), \cb)$ coincides with $G$. A key observation in proving ~(\ref{eq:conddist}) is that conditioning on $b(\varepsilon,\eta)=b_0$, the processes $\{b_0^{\top} \Omega^{(1)} B_p(s)\}_{\varepsilon\le s\le 1-\varepsilon}$ and $\{\omega B(s)\}_{\varepsilon\le s\le 1-\varepsilon}$ are equal in distribution, where $\omega=\sqrt{b_0^{\top} (\Omega^{(1)})^2 b_0}$. Then we obtain that conditioning on $b(\varepsilon,\eta)=b_0$, 
\BEqn
    \{b(\varepsilon,\eta)^{\top} \Omega^{(1)} \lrp{B_p(\varepsilon+(1-2\varepsilon)r) - B_p(\varepsilon)}\}_{0\le r\le 1}
&=^d& \{\sqrt{1-2\varepsilon}~ \omega B(r)\}_{r\in [0,1]}, \\[2mm]
    b(\varepsilon,\eta)^{\top} \Omega^{(1)} \lrp{B_p(1-\varepsilon) - B_p(\varepsilon)}
&=^d& \sqrt{1-2\varepsilon}~ \omega B(1), \\[2mm]
    \{b(\varepsilon,\eta)^{\top} \Omega^{(1)} \lrp{B_p(1-\varepsilon) - B_p(\varepsilon+(1-2\varepsilon)r)}\}_{0\le r\le 1}
&=^d& \{\sqrt{1-2\varepsilon}~ \omega \lrp{B(1) - B(r)}\}_{r\in [0,1]},
\EEqn 
and the above equality in distribution holds jointly. This leads to the desired result formally summarized in the following theorem.

    
%With the continuous mapping theorem (CMT), to find the limiting distribution of the test statistic $G_n = \max_{k=1,\cdots,N-1} T_n(k)V_n^{-1/2}(k)$, it suffices to find the limiting distribution of $T_n(k)$ and $V_n(k)$ respectively. Note that both $T_n(k)$ and $V_n(k)$ can be expressed in terms of the pre-defined cumulative sum $S_{a,b}$ with some $1\le a\le b\le N$, then we start from the asymptotic analysis of the cumulative sum. 

%Let $\{a_n\},\{b_n\}$ denote two sequences in $\{1,\cdots,N\}$ satisfying that $a_n\rightarrow a$ and $b_n\rightarrow b$, it follows from the functional central limit theorem that 
%\begin{equation*}
%    S_{a_n,b_n} 
%\leadsto \frac{1}{\varepsilon-\eta} \lrp{B_p(\varepsilon-\eta) + B_p(1) - B_p(1-\varepsilon+\eta)}^{\top} \Omega \lrp{B_p(\varepsilon+b) - B_p(\varepsilon+a)}
%\quad \mbox{as }n\rightarrow\infty.
%\end{equation*}
%By the natural property of the Brownnian motion, $B_p(\varepsilon-\eta) + B_p(1) - B_p(1-\varepsilon+\eta)$ is independent of the process $\cb = \{B_p(s)\}_{\varepsilon<s<1-\varepsilon}$. If conditioning on  it holds that $S_{a_n,b_n}$ asymptotically equals in distribution to $\sqrt{b^{\top}(\varepsilon,\eta) \Omega^2 b(\varepsilon,\eta)} \lrp{B(\varepsilon+b) - B(\varepsilon+a)}$.

%For any $k\in\{1,\cdots,N-1\}$, we assume that $k/N \rightarrow r \in[0,1]$ as $n\rightarrow\infty$. Consequently, if conditioning on $b(\varepsilon,\eta)$, we have that
%\begin{equation*}
 %   \sqrt{N} T_n(k) \leadsto \sqrt{(1-2\varepsilon) b^{\top}(\varepsilon,\eta) \Omega^2 b(\varepsilon,\eta)} \lrp{B(r) - r B(1)},
%\end{equation*}
%and
%\BEqn
%& & \sqrt{N} V_n^{1/2}(k) \\
%&\leadsto& \sqrt{(1-2\varepsilon) b^{\top}(\varepsilon,\eta) \Omega^2 b(\varepsilon,\eta)}
%\lrp{\int_{0}^{r} \lrp{B(s)-\frac{s}{r}B(r)}^2 ds + \int_{r}^{1}\lrp{\lrp{B(1)-B(s)} - \frac{1-s}{1-r}\lrp{B(1)-B(r)}}^2ds}^{1/2} \\
%&=^d& \sqrt{(1-2\varepsilon) b^{\top}(\varepsilon,\eta) \Omega^2 b(\varepsilon,\eta)} V^{1/2}(r).
%\EEqn

%Conditioning on $b(\varepsilon,\eta)$, the term $\sqrt{(1-2\varepsilon) b^{\top}(\varepsilon,\eta) \Omega^2 b(\varepsilon,\eta)}$, which appears in the asymptotic distributions of both $\sqrt{N} T_n(k)$ and $\sqrt{N} V_n(k)$,  can thus be cancelled out. This leads to the following theorem. 


%\begin{equation*}
%    G_n  = \max\limits_{k=1,\cdots,N-1} T_n(k)V_n^{-1/2}(k)
%  \leadsto \sup\limits_{r\in[0,1]} \lrp{B(r) - r B(1)} V^{-1/2}(r),
%\end{equation*}
%which is independent of $b(\varepsilon,\eta)$ and thus arrives at the desired result.

%Recall that for our test statistic $G_n$, both the numerator $T_n(k)$ and the denominator $V_n(k)$ can be expressed in terms of the cumulative sum $S_{a,b}$ with $1\le a\le b\le N$. By applying the functional central limit theorem, it holds that 
%\begin{equation*}
%    S_{a,b} 
%\leadsto \frac{1}{\varepsilon-\eta} \lrp{B_p(\varepsilon-\eta) + B_p(1) - B_p(1-\varepsilon+\eta)}^{\top} \Omega \lrp{B_p(\varepsilon+b_0) - B_p(\varepsilon+a_0)},
%\end{equation*}
%where $a_0 = \lim a/n$ and $b_0 = \lim b/n$. It follows from the natural property of the Brownnian motion that $b(\varepsilon,\eta) = B_p(\varepsilon-\eta) + B_p(1) - B_p(1-\varepsilon+\eta)$ is independent of the process $\cb = \{B_p(s)\}_{\varepsilon<s<1-\varepsilon}$. Consequently, conditioning on $b(\varepsilon,\eta)$, the asymptotic distribution of $S_{a,b}$ can be expressed as $\frac{1}{\varepsilon-\eta} \sqrt{b^{\top}(\varepsilon,\eta) \Omega^2 b(\varepsilon,\eta)} \lrp{B(b_0) - B(a_0)}$. In other words, the original involved analysis of the $p$-dimensional Brownian motion is now simplified to that of a univariate Brownian motion. Note that the conditioned term appear in both the numerator and the denominator, then it can be cancelled out after applying CMT, which implies that the limiting distribution for DGP1 is independent of $b(\varepsilon,\eta)$ and we thus arrive at the desired result.

%elaborate the theory case by case. When the observed data is a stationaty sequence, we need the following assumption.

%\begin{assumpt}\label{Assumpt:NullDist_Wn_Fixedp}
%Assume that $\frac{1}{\sqrt{n}} \sum\limits_{t=1}^{\lrfl{nr}} X_t \leadsto \Omega^{1/2} B_p(r)$ in $D[0,1]$ as $n\rightarrow\infty$.
%\end{assumpt}


%\begin{rmk}
%Assumption \ref{Assumpt:NullDist_Wn_Fixedp} is similar to Donsker's theorem, which is obtainable under some common and mild conditions.
%\end{rmk}

%With the Donsker-styled condition, we establish the following theorem.


%\xs{I think the whole logic is incorrect! In my opinion, it should be like this. Using the continuous mapping theorem, we obtain $G_n \longrightarrow G$, where $G=G(\{B_p(r), r\in [0,1]\})$ is complex. Then you show that $G|b(\varepsilon,\eta)=b\sim \sup\limits_{r\in[0,1]} \lrp{B(r) - r B(1)} V^{-1/2}(r)$  which does not depend on $b$, so the unconditional distribution of $G$ is the same as the conditional distribution, i.e., 
%$G=^d\sup\limits_{r\in[0,1]} \lrp{B(r) - r B(1)} V^{-1/2}(r)  $.}


\begin{thm}\label{Thm:NullDist_Gn_Fixedp}
...
%Under suitable conditions, we show that when the data structure of the data admits either of the aforementioned data structure, the limiting distribution of the test statistic $G_n$ is the same under the null hypothesis, i.e.
...
%\begin{defi}\label{Def:Model_iid_Growingp}
%Suppose $\{X_t\}_{t=1}^{n}$ is an iid sequence of $p$-dimensional random vectors with mean $\mu = \be[X_t] = 0$ and covariance matrix $\Sigma = (\Sigma_{ij})_{p \times p}$.
%\end{defi}
...
%\hg{(Sketch of proof for the iid case.)

%In fact, it follows from the iid proporty of $\{X_t\}_{t=1}^{n}$ that $\{Y_j\}_{j=1}^{N}$ forms a martingale difference sequence with respect to $\cf_j=\sigma(X_1,\cdots,X_{j+m})$ under the null.\xs{Is this true?} Consequently, by applying the martingale CLT established as Theorem 35.12 of \cite{billingsley2008probability}, we can show for any integer $k$ and any $\varepsilon \le r_1<\cdots<r_k \le 1-\varepsilon$ that 
%\begin{equation*}
%    \frac{1}{N_n}(W_n(r_1),\cdots,W_n(r_k)) 
%\rightarrow (B(r_1)-B(\varepsilon),\cdots,B(r_k)-B(\varepsilon)).
%\end{equation*}
%By using Lemma 9.8 of \cite{wang2020hypothesis}, we can further show that the process $\{\frac{W_n(r)}{N_n}\}_{\varepsilon\le r\le 1-\varepsilon}$ is tight under the null, which leads to the statement in Proposition \ref{Prop:NullDist_Wn}.}

%\xs{I did not mean to ask you to explain the following proposition}

\begin{prop}\label{Prop:NullDist_Wn}
...
%\xs{I meant to ask you to explain how to derive the limiting null distribution of our test statistic based on the above proposition, is it just CMT?}
%\begin{equation*}
%    \lrp{\frac{W_n(r_1)}{N_n}, \cdots, \frac{W_n(r_k)}{N_n}}
%\Longrightarrow \lrp{B(r_1)-B(\varepsilon), \cdots, B(r_k) - B(\varepsilon)}
%\end{equation*}
%where $\{B(r)\}_{r\in[0,1]}$ denotes the standard Brownian motion.
\end{prop}

%By applying Proposition \ref{Prop:NullDist_Wn}, we can obtain the limiting distribution of the test statistic $G_n$ under the null as $p\rightarrow\infty$.

%\begin{thm}\label{Thm:NullDist_Gn_Growingp}
%Assume that $\{X_t\}_{t=1}^{n}$ is an iid sequence defined as Definition \ref{Def:Model_iid_Growingp}. Under Assumption \ref{Assumpt:NullDist_Wn_iid_Growingp}, it holds under the null that $G_n \stra{d} G$ as $\min\{n,p\}\rightarrow\infty$.
%\end{thm}

With the auxiliary result summarized in Proposition \ref{Prop:NullDist_Wn} for the iid sequence, we are ready to establish a counterpart for the linear process via the Beveridge-Nelson decomposition \cite{phillips1992asymptotics}. To be specific, we 
define 
$ A^{(j)} = \sum\limits_{\ell=j}^{\infty} a_\ell, j\in\bn$
and 
rewrite $\{X_t\}_{t=1}^{n}$ as
\begin{equation*}
    X_t = D_t - R_t, 
\end{equation*}
...
%For notational simplicity, we further 
%then it follows that $D_t = A^{(0)} \varepsilon_t$ and $\widetilde{D}_t = \sum\limits_{j=0}^{\infty} A^{(j+1)} \varepsilon_{t-j}$. 
...
where $A_{\cdot k}$ denotes the $k$-th column of the matrix $A$ and $\|\Gamma^{(2)}\|_F^2 = \sum\limits_{1\le j,k\le p} (\Gamma^{(2)}_{jk})^2$. Thus $\|\Gamma^{(2)}\|_F = O\lrp{\|A^{(0)} \Gamma^{(2)} (A^{(0)})^{\top}\|_F}$ if the matrix $A^{(0)}$ is banded with fixed bandwidth.
...
%is neither too dense nor too sparse, and this constraint will be used to ensure the validity of the normalizer.
%\xs{what do you mean by validity}
%\hg{Inappropriate wording, I meant to say the normalizer cannot hold without this assumption.}


%Explanation of Assumption \ref{Assumpt:NullDist_Wn_lp}\ref{Assumpt:NullDist_Wn_Lp-3}.

%When deriving the limiting distribution of $W_n(r)$ for the linear process, we decompose $W_n(r)$ as a leading process $L_n(r)$ and remainder process $R_n(r)$. With the normalizers $\tilde{N}_n \sim n\|A^{(0)}\Gamma(A^{(0)})^{\top}\|_F$ and $N_n \sim n\|\Gamma^{(2)}\|_F$, we have shown that $L_n(r)/\tilde{N}_n \leadsto B(r)-B(\varepsilon)$ and $R_n(r)/N_n \leadsto 0$. 

%It is trivial that $\tilde{N}_n \lesssim n\|A^{(0)}\|^2\|\Gamma\|_F \lesssim n\|\Gamma\|_F \sim N_n$ under Assumption \ref{Assumpt:NullDist_Wn_lp}\ref{Assumpt:NullDist_Wn_Lp-7}, and the original purpose of Assumption \ref{Assumpt:NullDist_Wn_lp}\ref{Assumpt:NullDist_Wn_Lp-3} is to exclude the case when $\tilde{N}_n = o(N_n)$. In fact, when $\tilde{N}_n \sim N_n$, we can show that $W_n(r)/\tilde{N}_n \leadsto B(r)-B(\varepsilon)$. The problem occurs when $\tilde{N}_n = o(N_n)$. In this case, to make $R_n(r)$ negligible, the normalizer needs to be at least $N_n$, but with such normalizer, the expected leading term $L_n(r)/N_n$ also converges to zero.


%To prevent $\tilde{N}_n$ from being dominated by $N_n$, the intuition is that, the $L_2$ norm of most of the columns of $A^{(0)}$ cannot be too close to zero, or equivalently, $A^{(0)}$ cannot have too many sparse columns.

%A possibility is that, when $A^{(0)}$ is really sparse, we can find a sharper upper bound of the remainder term $R_n(r)$ so that the normarlizer to make $R_n(r)$ negligible does not need to be $N_n$, but this seems nontrivial.

%\hg{(Sketch of proof of the linear process)

%Define $\widetilde{N}_n = \sqrt{2n m_1} \|A^{(0)} \Gamma (A^{(0)})^{\top}\|_F$ as the normalizer for the linear process, then we can derive the process limit of $\{\frac{W_n(r)}{\widetilde{N}_n}\}$ under the null. 

%Specifically, for each fixed $r\in [\varepsilon,1-\varepsilon]$, $W_n(r)$ consists of four different types of inner products, namely, $D_i^{\top} D_j$, $D_i^{\top} R_j$, $R_i^{\top} D_j$ and $R_i^{\top} R_j$. It follows from the decomposition of $\{X_t\}_{t=1}^{n}$ that $\{D_t\}_{t=1}^{n}$ is an iid sequence of leading terms whereas $\{R_t\}_{t=1}^{n}$ is a sequence of remainder terms, thus it directly follows from Proposition \ref{Prop:NullDist_Wn} that 
%\begin{equation*}
%    \frac{1}{\widetilde{N}_n} \sum\limits_{i=1}^{m_1}(D_i-D_{n+1-i})^{\top} \lrp{\sum\limits_{j=1}^{\lrfl{nr}-\lrfl{n\varepsilon}} D_{j+m}}
%\leadsto B(r)-B(\varepsilon)
%\qquad \mbox{in } D[0,1]
%\end{equation*}
%Using the maximal inequality established in Proposition 1 of \cite{wu2007strong}, we can show all the remainder terms converge to zero, that is,
%\BEqn
%& & \left\|\sup\limits_{r\in[\varepsilon,1-\varepsilon]} \lrabs{\frac{1}{\widetilde{N}_n} \sum\limits_{i=1}^{m_1} (D_i-D_{n+1-i})^{\top} \lrp{\sum\limits_{j=1}^{\lrfl{nr} - \lrfl{n\varepsilon}} R_{j+m}}}\right\|_4 = o(1), \\
%& & \left\|\sup\limits_{r\in[\varepsilon,1-\varepsilon]} \lrabs{\frac{1}{\widetilde{N}_n} \sum\limits_{i=1}^{m_1} (R_i-R_{n+1-i})^{\top} \lrp{\sum\limits_{j=1}^{\lrfl{nr} - \lrfl{n\varepsilon}} D_{j+m}}}\right\|_4 = o(1), \\
%& & \left\|\sup\limits_{r\in[\varepsilon,1-\varepsilon]} \lrabs{\frac{1}{\widetilde{N}_n} \sum\limits_{i=1}^{m_1} (R_i-R_{n+1-i})^{\top} \lrp{\sum\limits_{j=1}^{\lrfl{nr} - \lrfl{n\varepsilon}} R_{j+m}}}\right\|_4 = o(1).
%\EEqn
%Therefore, Proposition \ref{Prop:NullDist_Wn_lp} is a direct result by combining the limiting processes of the leading terms and remainder terms.}
%\xs{again, this is not what I meant}
...
\begin{prop}\label{Prop:NullDist_Wn_lp}
...
For any $\varepsilon \le u \le 1-\varepsilon$, we observe that $m_1 S_{1, \lrfl{nu}-\lrfl{n\varepsilon}} = W_n(u)$, then it follows from Proposition \ref{Prop:NullDist_Wn_lp} that
\begin{equation*}
    \frac{m_1}{\widetilde{N}_n} S_{1, \lrfl{nu}-\lrfl{n\varepsilon}}
\leadsto B(u) - B(\varepsilon)~\mbox{in}~D[\varepsilon,1-\varepsilon].
\end{equation*}
Now for any $u\in[\varepsilon,1-\varepsilon]$, we let $k = \lrfl{nu}-\lrfl{n\varepsilon}$, then it directly follows from the continuous mapping theorem that
\BEqn
& & G_n \\
...
&=:& M(\{B(s)\}_{\varepsilon\le s\le 1-\varepsilon}).
\EEqn

Furthermore, we can show that $M(\{B(s)\}_{\varepsilon\le s\le 1-\varepsilon})=^d G$ by
changing the variables $r = (u-\varepsilon)/(1-2\varepsilon)$ and $s = (v-\varepsilon)/(1-2\varepsilon)$ and 
using the fact that the process $\{B(\varepsilon + (1-2\varepsilon)r) - B(\varepsilon)\}_{0\le r\le 1}$ is equal in distribution with the process $\{\sqrt{1-2\varepsilon} B(r)\}_{0\le r\le 1}$. 
...
%With Proposition \ref{Prop:NullDist_Wn_lp}, we obtain the limiting null distribution of the proposed test statistic when the data is a linear process.

\begin{thm}\label{Thm:NullDist_Gn_Growingp_lp}
...
%However, with the exponential term $\rho^{m_2/4}$, such constraint is mild and reasonable.
...
%\xs{maybe move this paragraph here?}
...
\begin{rmk}
Assumption \ref{Assumpt:NullDist_Wn_factor}\ref{Assumpt:NullDist_Wn_factor-1} basically requires that both $\Lambda^{\top} Z_i$ and $F_i$ satisfy the functional central limit theorem. The joint process convergence result stated in Assumption \ref{Assumpt:NullDist_Wn_factor}\ref{Assumpt:NullDist_Wn_factor-1} then follows since we assume the full independence between $\{Z_i\}$ and $\{F_i\}$. 
%\xs{do we need to assume that $\Lambda^{\top} \Sigma \Lambda$ is positive definite.}
%\hg{yes, added in Assumption \ref{Assumpt:NullDist_Wn_factor}}
%is a generalized version of that assumed in Assumption \ref{Assumpt:NullDist_Wn_Fixedp}, which is required by Donsker's theorem. 
...
%by and remain unchanged in their structures except for 
\end{rmk}
...
\begin{thm}\label{Thm:NullDist_Gn_factor}
...
\begin{prop}\label{Prop:PowerExpression}
...
\begin{thm}\label{Thm:DensePower_Fixedp}
...
\begin{enumerate}[label=(\roman*)]
 ...
 %   {\color{BrickRed} Original:
 %   $M(\Delta,b_0) := \sup\limits_{r\in[0,1]} T(r,\Delta,b_0) V^{-1/2}(r,\Delta,b_0)$ with $u = u(b_0,\Delta) = (\Omega^{(1)})^{1/2}b_0 - c\Delta_0$,
  %  \begin{equation*}
  %      T(r,\Delta,b_0) 
  %    = B(r)-rB(1) - c \lrp{r_0\wedge r} \lrp{(1-r_0)\wedge(1-r)} \sqrt{\frac{1-2\varepsilon}{u^{\top}\Omega^{(1)} u}} u^{\top} \Delta_0
   % \end{equation*}
   % and
   % {\small
   % \BEqn
...
%    \begin{equation*}
%        \blrp{G_n > G_{1-\alpha}}
%    \rightarrow \int_{x\in\br^{p}} \lrp{\pi(\varepsilon-\eta)}^{-p/2} \exp\lrp{-\frac{1}{4}(\varepsilon-\eta) x^{\top}x} \blrp{G(\Delta|x) > G_{1-\alpha}} dx,
%    \end{equation*}
%    where $G(\Delta|x) := \sup\limits_{r\in[0,1]} T(r,\Delta;x) V^{-1/2}(r,\Delta;x)$, with
%    \BEqn
%    & & T(r,\Delta;x) \\
%    &:=& \lrp{(\Omega^{(1)})^{1/2} x - c\Delta_0}^{\top} (\Omega^{(1)})^{1/2} 
...
%          \lrp{(\Omega^{(1)})^{1/2} x - c\Delta_0}^{\top} (c\Delta_0),
%    \EEqn
%    and 
%    {\small
%    \BEqn
...
%    &:=& \int_{0}^{r}
%        \Large\left(
%          \lrp{(\Omega^{(1)})^{1/2}x - c\Delta_0}^{\top}
%          (\Omega^{(1)})^{1/2}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=120mm,width=135mm]{PowerPlot-DGP1.pdf}
    \caption{Power curves of SS-SN and SN test in \cite{shao2010testing} under DGP1}
    \label{Fig:PowerPlot-DGP1}
\end{figure}

The three rows of Figure \ref{Fig:PowerPlot-DGP1} correspond to the three types of $\Omega^{(1)}$ whereas the three columns refer to $p\in\{3,5,10\}$. %In each figure, we use the red solid curve to represent the power of our proposed methods and use the green dotted curve for the \texttt{SN} proposed by \cite{shao2010testing}. 
When compared to \texttt{SN}, our proposed method has some mild power loss when there is no componentwise dependence within the data, i.e., when $\Omega^{(1)}=I_p.$ 
Somewhat surprisingly, our method gains some advantage in the AR and compound symmetric cases and the power again seems to grow with respect to dimension. It is 
worth mentioning that when $p=1$, the univariate data has the same distribution under all the three $\Omega^{(1)}$'s, and the power behavior in this case matches those with $\Omega^{(1)}$ being ID. Therefore, contrary to the phenomenon discovered by \cite{kim2020dimensionagnostic}, our dimension agnostic test statistic does not always incur power loss, and when it does lose/gain 
power and by how much very much depend on the dimension and the  dependence structure in the data. 


%{\color{BrickRed}
%Another possible way to display results is shown in Figure \ref{Fig:PowerPlot-DGP1-2}. The three sub-figures correspond to the three choices of $\Omega^{(1)}$. In each plot, the solid and dotted lines refer to the proposed method and \texttt{SN} respectively, whereas the three colors represent different dimensionality.

%\begin{figure}[h!]
%    \centering
%    \includegraphics[height=60mm,width=160mm]{PowerPlot-3.pdf}
%    \caption{Power curves against the magnitude of the mean shift.}
%    \label{Fig:PowerPlot-DGP1-2}
%\end{figure}
%}
...
\begin{thm}\label{Thm:DensePower_lp}
...
\begin{enumerate}[label=(\roman*)]
  ...
  %  {\color{BrickRed} Original:
  %  $M(c):= \sup\limits_{r\in[0,1]} T(r,c) V^{-1/2}(r,c)$ with
   % \begin{equation*}
   %     T(r,c)
   %  := B(r) - rB(1) + c \sqrt{\frac{(1-2\varepsilon)(\varepsilon-\eta)}{2}} \lrp{r_0\wedge r}\lrp{(1-r_0)\wedge(1-r)}
   % \end{equation*}
   % and
   % {\small
   % \BEqn
...
%    we have that $\blrp{G_n > G_{1-\alpha}} \rightarrow \blrp{G(\Delta) > G_{1-\alpha}}$, where $G(\Delta):= \sup\limits_{r\in[\varepsilon,1-\varepsilon]} T(r,\Delta) V^{-1/2}(r,\Delta)$ with
%    \begin{equation*}
%        T(r,\Delta)
%     := \sqrt{\frac{2(1-2\varepsilon)}{\varepsilon-\eta}} \lrp{B(r) - rB(1)}
%    + c(1-2\varepsilon)(r \wedge r_0) \lrp{(1-r)\wedge(1-r_0)}
%    \end{equation*}
%    and
%    {\small
%    \BEqn
%    & & V(r,\Delta) \\
%    &:=& \int_{0}^{r} 
%         \lrp{ \sqrt{\frac{2(1-2\varepsilon)}{\varepsilon-\eta}} \lrp{B(s) - \frac{s}{r}B(r)} 
%              + c(1-2\varepsilon) \frac{(s \wedge r_0) \lrp{(r-s)\wedge(r-r_0)}\vee0}{r} }^2 ds \\
%    &+& \int_{r}^{1} 
%        \lrp{ \sqrt{\frac{2(1-2\varepsilon)}{\varepsilon-\eta}} \lrp{B(1-s) - \frac{1-s}{1-r}B(1-r)} 
%             - c(1-2\varepsilon) \frac{\lrp{(1-s)\wedge(1-r_0)} \lrp{(s-r)\wedge(r_0-r)}\vee0}{1-r} }^2 ds.
%    \EEqn}}
\end{enumerate}
\end{thm}
%\xs{what is $r_0$?}
%\hg{introduced the notation at the beginning of Section 4}
...
%o compare the power curve in the intermediate case, we perform the simulations...
%\xs{can we present some power comparison based on the power formula above, say $(n,p)=(100,100), (400,400)$, $\varepsilon=0.1$, $\eta=0.02$.}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=65mm,width=70mm]{PowerPlot-DGP2.pdf}
    \caption{Power curves of SS-SN and $T(\eta_0)$ in \cite{wang2022inference} under DGP2}
...
%To some extent, we use the two-tailed observations to determine the direction of the projection and the testing is mainly based on the $n-2m$ observations in the middle of the sequence, thus our method adopts a heavier trimming, and this might explain the noticeable power loss of the proposed method.





%Similar to the case when $\{\tilde{X}_t\}_{t=1}^{n}$ is a stationary sequence with fixed $p$, the asymptotic behavior of the proposed test statistic can be divided into three regimes based on the determinant factor $\frac{\|\Delta\|_2}{\|A^{(0)} \Gamma (A^{(0)})^{\top}\|_F^{1/2}}$. A subtle difference is that, for the nontrivial case when the limit of the determinant is a strictly positive constant, we can derive the explicit limiting distribution of the test statistic in this case, whereas it cannot be obtained when $\{\tilde{X}\}_{t=1}^{n}$ is a stationary sequence.
...
% there exists some $L_2\in\br^{s\times s}$ that is independent of $p$, such that $\frac{\Lambda \Omega \Lambda^{\top}}{\|\Lambda \Omega \Lambda^{\top}\|_F} \rightarrow L_2$ as $p \rightarrow \infty$.
\end{assumpt}

%\xs{the dimension does not match here, $\Lambda^{\top} \Omega \Lambda$? Is it $\Sigma^{(3)}$ or $\Omega$?}

%\hg{this assumption is no longer needed for the revised proof of the factor model, i have removed it.}

%Under Assumption \ref{Assumpt:NullDist_Wn_factor} and Assumption \ref{Assumpt:DensePower_factor}, we are ready to derive the limiting behavior of $G_n$, as stated in Theorem \ref{Thm:DensePower_factor}.

\begin{thm}\label{Thm:DensePower_factor}
...
\begin{enumerate}[label=(\roman*)]
   ...
   %     {\color{BrickRed} Original:
   %     where $M(\Delta):= \sup\limits_{r\in[0,1]} T(r,\Delta) V^{-1/2}(r,\Delta)$ with
    %    \begin{equation*}
    %        T(r,\Delta)
    %    := B(r) - rB(1) + c \sqrt{\frac{(1-2\varepsilon)(\varepsilon-\eta)}{2}} \lrp{r_0\wedge r}\lrp{(1-r_0)\wedge(1-r)}
     %   \end{equation*}
   ...
   %     {\color{BrickRed} Original:
    %    where $M(\Delta,b_0) := \sup\limits_{r\in[0,1]} T(r,\Delta,b_0) V^{-1/2}(r,\Delta,b_0)$ with 
     %   \begin{equation*}
     %       T(r,\Delta,b_0)
     %     = B(r)-rB(1) - \omega_3\sqrt{\frac{1-2\varepsilon}{\omega_1^2+\omega_2^2}} \lrp{r_0\wedge r} \lrp{(1-r_0)\wedge(1-r)},
     %   \end{equation*}
     %   and 
     %   {\small
     %   \BEqn
     ...
     %   \EEqn}
      ...
      %  \begin{equation*}
       ...
       %   = \sqrt{\lrp{b_0^{\top} ((\Omega^{(3)})^{1/2})^{\top} L_0 - c_2 \lambda_0^{\top}} \Omega^{(3)} \lrp{b_0^{\top} ((\Omega^{(3)})^{1/2})^{\top} L_0 - c_2 \lambda_0^{\top}}^{\top}},
       % \end{equation*}
        %and $\displaystyle\omega_2 = \frac{c_1}{\varepsilon-\eta}$, $\omega_3 = c_2 \lambda_0^{\top} (\Omega^{(3)})^{1/2}$.}
    \end{enumerate}
...
%    \begin{enumerate}[label=(\arabic*)]
%        \item 
%        when $\|\Lambda\| = o\lrp{\|\Gamma^{(3)}\|_F^{1/2}}$, if we additionally assume that 
%        \begin{equation*}
%            \frac{\|\Delta\|_2^2}{\sqrt{2(\varepsilon-\eta)} \|A^{(0)} \Gamma^{(3)} (A^{(0)})^{\top}\|_F}
%        \rightarrow c:=c(\Delta),
%        \end{equation*}
%        then it holds that $\blrp{G_n > G_{1-\alpha}} \rightarrow \blrp{G(\Delta) > G_{1-\alpha}}$, where $G(\Delta):= \sup\limits_{r\in[\varepsilon,1-\varepsilon]} T(r,\Delta) V^{-1/2}(r,\Delta)$ with
%        \begin{equation*}
%            T(r,\Delta)
%        := \sqrt{\frac{2(1-2\varepsilon)}{\varepsilon-\eta}} \lrp{B(r) - rB(1)}
%        + c(1-2\varepsilon)(r \wedge r_0) \lrp{(1-r)\wedge(1-r_0)}
...
%        & & + \int_{r}^{1} 
%            \left(
%              \sqrt{\frac{2(1-2\varepsilon)}{\varepsilon-\eta}} \lrp{B(1-s) - \frac{1-s}{1-r}B(1-r)} 
%            \right. \\
%        & & \hspace{4em}
%            \left.
%              - c(1-2\varepsilon) \frac{\lrp{(1-s)\wedge(1-r_0)} \lrp{(s-r)\wedge(r_0-r)}\vee0}{1-r} 
%            \right)^2 ds.
%        \EEqn}
...
%        \begin{equation*}
%            \blrp{G_n > G_{1-\alpha}}
%        \rightarrow \int_{x\in\br^{s}} \lrp{\pi(\varepsilon-\eta)}^{-s/2} \exp\lrp{-\frac{1}{4}(\varepsilon-\eta) x^{\top}x} \blrp{G(\Delta|x) > G_{1-\alpha}} dx,
%        \end{equation*}
%        where $G(\Delta|x) := \sup\limits_{r\in[0,1]} T(r,\Delta;x) V^{-1/2}(r,\Delta;x)$, with
%        \BEqn
%        & & T(r,\Delta;x) \\
%        &:=& \frac{1}{\varepsilon-\eta} \lrp{h((1-2\varepsilon)r+\varepsilon, x) - r h(1-\varepsilon, x)}     - c_2 \sqrt{(1-2\varepsilon)} \lambda_0^{\top} (\Omega^{(3)})^{1/2} \lrp{B_s(r) - rB_s(1)} \\
%        & & - (1-2\varepsilon) \lrp{r_0 \wedge r} \lrp{(1-r_0) \wedge (1-r)} 
%              \lrp{c_2 \lambda_0^{\top} (\Omega^{(3)})^{1/2} x - c_3},
%        \EEqn
%        and 
%        {\small
%        \BEqn
...
%        &:=& \int_{0}^{r}
%            \Large\left(
%              \frac{1}{\varepsilon-\eta} \lrp{h((1-2\varepsilon)s+\varepsilon, x) - \frac{s}{r} h((1-2\varepsilon)r+\varepsilon, x)} 
%            - c_2 \sqrt{(1-2\varepsilon)} \lambda_0^{\top} (\Omega^{(3)})^{1/2} \lrp{B_s(s) - \frac{s}{r} B_s(r)} \right. \\
%        & & \hspace{3em} 
%            \left.
%              - (1-2\varepsilon) \frac{\lrp{s \wedge r_0} \lrp{(r-s) \wedge (r-r_0)} \vee 0}{r}
%                \lrp{c_2 \lambda_0^{\top} (\Omega^{(3)})^{1/2} x - c_3}
...
%              \lrp{
%                ( h(1-\varepsilon, x) - h((1-2\varepsilon)s+\varepsilon, x) )
%                 - \frac{1-s}{1-r} ( h(1-\varepsilon, x) - h((1-2\varepsilon)r+\varepsilon, x) )}
%              \right) \\
%        & & \hspace{3em}
%            - c_2 \sqrt{(1-2\varepsilon)} \lambda_0^{\top} (\Omega^{(3)})^{1/2} \lrp{B_s(1-s) - \frac{1-s}{1-r} B_s(1-r)} \\
%        & & \hspace{3em} 
%            \left.
%              + (1-2\varepsilon) \frac{\lrp{(1-s) \wedge (1-r_0)} \lrp{(s-r) \wedge (s-r_0)} \vee 0}{1-r}
%                \lrp{c_2 \lambda_0^{\top} (\Omega^{(3)})^{1/2} x - c_3}
%            \Large\right)^2 ds.
%        \EEqn}
%        Here, we define $h(r,x)$ as
%        \begin{equation*}
%            h(r,x)
%        := (\varepsilon-\eta) x^{\top} ((\Omega^{(3)})^{1/2})^{\top} L_0 (\Omega^{(3)})^{1/2} \lrp{B_s(r)-B_s(\varepsilon)} + c_1 \lrp{\tilde{B}(r) - \tilde{B}(\varepsilon)}
%        \end{equation*}
%        where $\cb_s:=\{B_s(r):0\le r \le 1\}$ is a standard $s$-dimensional Brownian motion and $\tilde\cb:=\{\tilde{B}(r):0\le r\le 1\}$ is an independent one-dimensional standard Brownian motion.
%    \end{enumerate}}
...
(1) %can be further based on the dominant term between $\|\Lambda\|$ and $\|\Gamma\|_F^{1/2}$. 
When $\|\Gamma^{(3)}\|_F^{1/2}$ dominates $\|\Lambda\|$, the noise component $Z_t$ becomes the leading term, so it is not surprising to find that the limiting distribution of $G_n$ in this case matches that in Theorem \ref{Thm:DensePower_lp}\ref{Thm:DensePower_lp-3}. (2) When $\|\Gamma^{(3)}\|_F^{1/2} = O\lrp{\|\Lambda\|}$, this corresponds to the case the noise component is not the leading term, the expression is more complicated since both the low rank part $\Lambda F_t$ and the noise part $Z_t$ can be non-negligible. 
...
%whereas our test statistic directly uses the CUMSUM process of the projected data and employs the self-normalizer. The second difference is about the implementation of the test. Note that the limiting distribution of the test statistic in \cite{horvath2022} includes some unknown variables, hence the authors propose some new bootstrapping methods to simulate the critical values. As a contrast, the limiting null distribution of our proposed is shown to be pivotal and its critical value can be easily simulated using the Monte Carlo replication. Additionally, we also establish some theoretical results of the power against the local alternative, which is not investigated in \cite{horvath2022}.


%xs{provide some description and comparison of the methods in \cite{horvath2022}}

%\hg{In the latter paper, the authors assume that the observed data $\{y_{i}\}_{i=1}^{n}$ with $p$ cross sections admit the factor model $y_{ij} = \mu_{\ell j} + f_i^{\top} \lambda_j + e_{ij}$ for $k_{\ell-1} < i \le k_{\ell}$, $1\le \ell\le R+1$ and $1\le k_1<k_2<\cdots<k_R\le n$, where $\{f_i\}_{i=1}^{n}\in \br^{s}$ are the common factors, $\{\lambda_j\}_{j=1}^{p}\in\br^{s}$  are the corresponding loadings, and $\{e_{ij}\}_{1\le i\le n,1\le j\le p}$ are the error terms. This article considers testing $H_0: \mu_{1j}=\cdots=\mu_{Rj},\ 1\le j\le p$ versus the alternative $H_1: \mu_{ij}\neq\mu_{i+1,j}, i\in\{1,\cdots,R\}, \mbox{ for some } j\in\{1,\cdots,p\}$. The main contribution of \cite{horvath2022} is to develop a new centring and normalizing method for the CUMSUM process based on the observations. Specifically, their test statistics are based on functionals of 
%\begin{equation*}
 %  V_{np}(u) = \sum\limits_{j=1}^{p} \lrp{S_{j}^2(u) - \frac{\lrfl{nu}(n-\lrfl{nu})}{\lrfl{n\tau }(n-\lrfl{n\tau})} S_{j}^2(\tau)}\bone\{nu \geq 1\},
 %\end{equation*} 
%where the tuning parameter $\tau\in(0,1)$ and $S_j(u) = \sum\limits_{i=1}^{\lrfl{nu}}(y_{ij}-\bar{y}_{nj})$ with $\bar{y}_{nj}=\frac{1}{n}\sum\limits_{i=1}^{n} y_{ij}$. Under the assumption that the idiosyncratic errors are serially uncorrelated, the authors investigate the asymptotic behavior of the $\{V_{np}(u)\}_{0\le u\le 1}$ and it is shown that the limiting distribution depends crucially on to what extent the common factors are asymptitcally dominant. Based on these findings, the authors propose an appropriate estimator of the normalizing sequence and derive the limiting distribution of the proposed test statistic under both the null and the alternative, which are later generalized to the case that allows for serial correlation within the idiosyncratic errors. 

%Though both are applicable for high-dimensional panel data, there are several differences between \cite{horvath2022} and ours. The first difference lies in the construction of the test statistic. \cite{horvath2022} develops a random centering technique for the original CUMSUM process and the normalizer is based on the empirical cross covariances along with a predefined kernel function, whereas our test statistic directly uses the CUMSUM process of the projected data and employs the self-normalizer. The second difference is about the implementation of the test. Note that the limiting distribution of the test statistic in \cite{horvath2022} includes some unknown variables, hence the authors propose some new bootstrapping methods to simulate the critical values. As a contrast, the limiting null distribution of our proposed is shown to be pivotal and its critical value can be easily simulated using the Monte Carlo replication. Additionally, we also establish some theoretical results of the power against the local alternative, which is not investigated in \cite{horvath2022}.
...
%the magnitude of the mean shift is minimal in terms of $\|\Delta\|_2$, and contrarily, it achieves asymptotically power one when the mean shift is strong enough. Suppose that the magnitude of the mean shift is moderate, the asymptotic power can be expressed as a function in terms of $\Delta$, whose range lies between $\alpha$ and one. 
...
\begin{table}[h!]
    ...
    \hline\hline
    \end{tabular}
    \caption{Summary of asymptotic power analysis for three DGPs}
...
\section{Some Extensions}\label{Sec:Extension}

%\xs{give a brief summary about what to come next}



So far, we have developed a test statistic for a single (dense) change point in Section \ref{Sec:Methodology} and established its asymptotic theories under the null and the alternative in Section \ref{Sec:Theory-null} and \ref{Sec:Theory-power} respectively. In this section, we generalize the proposed SS-SN method to target the multiple change points alternative or the single sparse change. Specifically, in Section \ref{Sec:Extension-multipleCP}, we incorporate the proposed SS-SN methodology with the scanning test statistic of  \cite{zhang2018unsupervised} and develop a new test statistic to test for multiple change points. The asymptotic theory is also presented in this section. Furthermore, we present a variant of our SS-SN test statistic for the sparse alternative in Section \ref{Sec:Extension-sparse} and offer some discussion.
%, though we leave the theoretical justification for future work.


%----------------------------------------------------------------
\subsection{Multiple Change Points Testing}\label{Sec:Extension-multipleCP}

%\xs{add a paragraph of multiple change points testing, (splitting, trimming, projection and one-dimensional multiple change points testing), refer to \cite{zhang2018unsupervised}.}



We first consider the multiple change points alternative. Mathematically, we aim to test
\begin{equation*}
    H_0: \mu_1=\cdots=\mu_n
    \quad\mbox{versus}\quad
...
%Specifically, we require the following assumption on the alternative.
...
%According to the formulation of $\{Y_j\}_{j=1}^{N}$ as defined in Section \ref{Sec:Methodology}, we have that $Y_j = \lrag{\hat\mu_1-\hat\mu_n,X_{j+m}}$ with $\hat\mu_1 = \frac{1}{m_1} \sum_{i=1}^{m_1} X_i$ and $\hat\mu_n = \frac{1}{m_1} \sum_{i=1}^{m_1} X_{n+1-i}$. 
...
%\{(\lrfl{nr_1}-\lrfl{n\varepsilon},\lrfl{nr_2}-\lrfl{n\varepsilon}): (r_1,r_2)\in\Xi(\varepsilon)\}.
\EEqn

%{\color{BrickRed}
%Need $\Xi_n(\varepsilon) = \{(k_1,k_2): k_1,k_2\in\{2,\cdots,N-1\}, k_1<k_2\}$, may need to slightly modify the original notation and proof; to be revisited.}


%Here $\tau$ is another trimming parameter different from the trimming and splitting parameters we used during the sample splitting process. As argued in \cite{zhang2018unsupervised}, $\tau$ controls the minimal length of partial sums, and its effect is accounted for in the limiting distribution and its approximation. From the definition of $\Omega(\tau)$, for any $(t_1,t_2)\in\Omega(\tau)$, the pairwise distances among $\{1,t_1,t_2,n\}$ all exceed $\tau$, so that all the partial sums involved in the subsequent steps contain enough elements. In this case, the rule of thumb choice of $\tau=0.1$.

% Then the test statistic is defined as
% \begin{equation*}
%     G_n^{M}
%   = \max\limits_{(\ell_1,\ell_2)\in\Xi_n(\varepsilon)} \frac{T_n^{f}(1,\ell_1,\ell_2)}{\lrp{V_n^{f}(1,\ell_1,\ell_2)}^{1/2}} 
%   + \max\limits_{(\ell_1,\ell_2)\in\Xi_n(\varepsilon)} \frac{T_n^{b}(\ell_1,\ell_2,N)}{\lrp{V_n^{b}(\ell_1,\ell_2,N)}^{1/2}}.
% \end{equation*}
...
%According to the expression of $G_n^M$, it contains two self-normalized parts. The first part is expected to search for change points over all possible intervals that starts from the head of the sequence, whereas the latter part searches over all possible intervals that ends at the tail of the sequence. $G_n^M$ combines these two self-normalized terms and searches for the most significant change points in both the forward and backward directions simultaneously.
...
%\begin{rmk}
%The overall computational expense of $G_n^M$ is at the order of $O(n(n^2+p))$. Specifically, after computing $\{Y_j\}_{j=1}^{N}$ at the cost of $O(np)$, we can calculate $\{S_k\}_{k=1}^{N}$ at the cost of $O(n^2)$, where $S_k = S_{1,k} = \sum_{j=1}^{k}Y_j$. Furthermore, let $U = (u_{k1,k2})_{N\times N}$ and $V = (v_{k1,k2})_{N\times N}$ be two upper-triangular matrices in $\br^{N\times N}$, where $u_{k_1,k_2} = S_{k_2}-S_{k_1-1}$ and $v_{k_1,k_2} = k_2-k_1+1$, then both matrices can be calculated with a cost of $O(n^2)$. Observe that $T_n^f(1,\ell_1,\ell_2) = (u_{1,\ell_1}-\frac{v_{1,\ell_1}}{v_{1,\ell_2}} u_{1,\ell_2}) / \sqrt{v_{1,\ell_2}}$, and
%\begin{equation*}
 %   V_n^{f}(1,\ell_1,\ell_2)
 % = \frac{1}{v^2(1,\ell_2)} \sum\limits_{i=1}^{\ell_1} \lrp{u_{1,i} - \frac{v_{1,i}}{v_{1,\ell_1}} u_{1,\ell_1}}^2
 % + \frac{1}{v^2(1,\ell_2)} \sum\limits_{i=\ell_1+1}^{\ell_2} \lrp{u_{i,\ell_2} - \frac{v_{i,\ell_2}}{v_{\ell_1,\ell_2}} u_{\ell_1,\ell_2}}^2,
%\end{equation*}
%we can calculate $\max\limits_{(\ell_1,\ell_2)\in\Xi_n(\varepsilon)} \sqrt{r_2} T^{f}(r_1,r_2) \lrp{V^{f}(r_1,r_2)}^{-1/2}$ at the order of $O(n^3)$. Similarly, we can compute $\max\limits_{(\ell_1,\ell_2)\in\Xi_n(\varepsilon)} \sqrt{1-r_1} T^{b}(r_1,r_2) \lrp{V^{b}(r_1,r_2)}^{-1/2}$ at the same cost, and consequently, the overall computational expense is of order $O(n(n^2+p))$.

%Note that the most computationally expensive step is to find the maximum of $\sqrt{r_2} T^{f}(r_1,r_2) \lrp{V^{f}(r_1,r_2)}^{-1/2}$ over the set $\Xi_n(\varepsilon)$ where $|\Xi_n(\varepsilon)| = O_s(n^2)$. One possible remedy is to adopt the discretised idea proposed in \cite{zhang2018unsupervised} and to eases the computational burden by finding the maximum over a set of the cardinality $O(n)$, for which we spare the details.
%\end{rmk}
...
\begin{thm}\label{Thm:NullDist_multiple}
...
%     G_n^{M} 
% \stra{d} G^{M}
% := \sup\limits_{(r_1,r_2)\in\Xi} \sqrt{r_2} T^{f}(r_1,r_2) \lrp{V^{f}(r_1,r_2)}^{-1/2}
%  + \sup\limits_{(r_1,r_2)\in\Xi} \sqrt{1-r_1} T^{b}(r_1,r_2) \lrp{V^{b}(r_1,r_2)}^{-1/2},
% \end{equation*}
%{\color{PineGreen}
\BEqn
    G_n^{M} 
...
%{\color{BrickRed} formulation and theory to be added.} \xs{there is no need to pursue this!}

%{\color{BrickRed} in \cite{zhang2018unsupervised} an trimming is used so that all pairs $(t_1,t_2)\in\Xi(\varepsilon)$ has distance no smaller than $\varepsilon$. This assumption is only needed to guarantee the consistency under the alternative, i.e. the test statistic diverges to infinity. The consistency is not a focus in our article and their consistency results depend on some assumptions that cannot be easily generalized to our setting, thus we remove the additional trimming in our case. 
...
\centering
    ...
    \hline\hline
        Quantile Level & 90\% & 95\% & 97.5\% & 99\% & 99.5\% & 99.9\% \\ \hline
%(one-sided,10000)       Critical value & 18.12 & 19.83 & 21.93 & 29.56 & 37.44 & 73.19 \\
%(one-sided,20000)       Critical value & 18.13 & 19.93 & 22.08 & 28.42 & 35.52 & 70.75 \\
        Critical Value & 20.71 & 23.16 & 26.75 & 35.50 & 45.74 & 102.97 \\
    \hline\hline
    \end{tabular}
    \caption{Simulated critical values for $G^M$}
    \label{Tab:critval-M}
\end{table}


%Same as shown in Section \ref{Sec:Theory-null}, the limiting null distribution $G^M$ is consistent to various dimensional regimes and different temporal and cross-sectional dependence structure. 
...
% \begin{assumpt}\label{Assumpt:MultiplePower-new}
% Assume that there exists some positive normalizer $N_n$, such that $\sup\limits_{j=1,\cdots,M} \frac{n^2}{N_n} \lrabs{\delta^{\top} \delta_{j}} \rightarrow \infty$ as $n\rightarrow\infty$, $\|\delta\|_2^2 = O(\sup\limits_{j=1,\cdots,M} \lrabs{\delta^{\top} \delta_{j}})$ and $\max\limits_{t=1,\cdots,M} \|\delta_t\|_2^2 = o(\sup\limits_{j=1,\cdots,M} \lrabs{\delta^{\top} \delta_{j}})$.
% \end{assumpt}

% \begin{assumpt}\label{Assumpt:MultiplePower}
% Assume that there exists some positive normalizer $N_n$, such that $\sup\limits_{j=1,\cdots,M} \frac{n^2}{N_n} \lrabs{\delta^{\top} \delta_{j}} \rightarrow \infty$ as $n\rightarrow\infty$.

% Additionally, assume that either of the following conditions is satisfied:
% \begin{enumerate}[label=(\roman*)]
% \setcounter{enumi}{0}
%     \item \label{Assumpt:MultiplePower-1}
%     let $i$ denote the smallest integer such that $\frac{n^2}{N_n} \lrabs{\delta^{\top} \delta_{i}} \rightarrow \infty$, it holds that $\|\delta\|_2^2 = O(\lrabs{\delta^{\top} \delta_{i}})$ and $\max\limits_{j\le i}\{\|\delta_{j}\|_2^2\} = O(\lrabs{\delta^{\top} \delta_{i}})$.
%     \item \label{Assumpt:MultiplePower-2}
%     let $i$ denote the largest integer such that $\frac{n^2}{N_n} \lrabs{\delta^{\top} \delta_{i}} \rightarrow \infty$, it holds that $\|\delta\|_2^2 = O(\lrabs{\delta^{\top} \delta_{i}})$ and $\max\limits_{j\geq i}\{\|\delta_{j}\|_2^2\} = O(\lrabs{\delta^{\top} \delta_{i}})$.
% \end{enumerate}
% \end{assumpt}}
...
% \begin{assumpt}\label{Assumpt:MultiplePower}
% Assume that there exists some positive normalizer $N_n$ and $1\le i\le N$, such that $\frac{n^2}{N_n} \lrabs{\delta^{\top} \delta_{i}} \rightarrow \infty$ as $n\rightarrow\infty$ and $\frac{N_n^{1/2}}{n} \|\delta\|_2 = o(\lrabs{\delta^{\top}\delta_i})$. Additionally, either of the following conditions is satisfied:
% \begin{enumerate}[label=(\arabic*)]
%     \item \label{Assumpt:MultiplePower-1}
%     $\max\limits_{j\le i} \frac{N_n^{1/2}}{n} \|\delta_j\|_2 = o(\lrabs{\delta^{\top}\delta_i})$ and $\max\limits_{j<i}\{|\delta^{\top} \delta_j|\} = o(\delta^{\top} \delta_{i})$.
%     \item \label{Assumpt:MultiplePower-2}
%     $\max\limits_{j\geq i} \frac{N_n^{1/2}}{n} \|\delta_j\|_2 = o(\lrabs{\delta^{\top}\delta_i})$ and $\max\limits_{j>i}\{|\delta^{\top} \delta_j|\} = o(\delta^{\top} \delta_{i})$.
% \end{enumerate}
% \end{assumpt}
...
\begin{thm}\label{Thm:MultiplePower}
...
%We shall leave this for future investigation.
%In this article, we do not intend to pursue more in this topic, 

So far, we introduce two test statistics for single change point alternative, which target at the dense change point and the sparse change point respectively. In practice, we do not have prior knowledge about the  sparsity of the change, which naturally motivates the aggregation of these two tests to achieve the adaptive power. A common method to aggregate the two tests is to construct a Bonferroni test combining both the dense test and the sparse test. Specifically, given the data, we can compute the dense test statistic $G_n$ as well as the sparse test statistic $G_n^{\ast}$. At the significance level $\alpha$, we reject the null hypothesis if the $p$-value of either test drops below the $\alpha/2$ threshold. As will be shown by the simulation studies, the Bonferroni test achieves encouraging performance across multiple scenarios, regardless of the sparsity of the mean shift.

%{\color{BrickRed}
%A random thought, maybe another way to combine the dense test statistic and the sparse test statistic is via truncation. With $\hat\mu_1 = \frac{1}{m_1}\sum\limits_{i=1}^{m_1} X_i$ and $\hat\mu_n = \frac{1}{m_1} \sum\limits_{i=1}^{m_1} X_{n+1-i}$, we determine the projectional direction $\hat\nu = (\hat\nu_1,\cdots,\hat\nu_p)$ as $\hat\nu_j = (\hat\mu_{1,j}-\hat\mu_{n,j})\bone\{ |\hat\mu_{1,j}-\hat\mu_{n,j}| \geq c\}$ where $c$ is a pre-selected threshold. 

%The dense and sparse test can each be viewed as a special case of the truncated test. For example, the direction used for the dense test $\hat\mu_1-\hat\mu_n$ coincides with $\hat\nu$ with $c=0$ (deterministic), and the direction for the sparse test matches $\hat\nu$ with $c=\max\limits_{j=1,\cdots,p} |\hat\mu_{1,j}-\hat\mu_{n,j}|$ (data-driven). However, we need to specify another parameter $c\geq0$. 

%The intuition is that, the truncated test can gain some power if the mean shift is neither too dense nor too sparse, which is the most common case in reality. But the challenge seems to be the theoretical analysis, which is expected to be more involved than that of the dense test but it should be simpler than that of the sparse test, since the sparse test uses a data-dependent threshold but the truncated version uses a deterministic one. Suppose the theoretical justification of the sparse test is doable, that for the truncated test seems to be doable as well.
%}

%\xs{Yes, thresholding is common to make test adaptive. The theoretical analysis is challenging though. we may just not mention it in the paper, a good thought!}




%----------------------------------------------------------------
%\subsection{Generalization to Multiple Change Points (Estimation)}\label{Sec:Methodology-multiCP}

%Now that we have established the testing procedure for the scenario of a single change point, it is straightforward to generalize our work to the scenario of multiple change points using the wild binary segmentation proposed in \cite{fryzlewicz2014wild}. The core idea is to generate a number of random segments from the original multivariate data, where the segmentation is sufficiently fine so that each segment is expected to contain at most one change point. Then we can apply the aforementioned method to locate the potential change point in each segment and follow a binary-styled procedure to detect all the change points of the original sequence. 

%Specifically, let $\cm = \{\ell_m:=(s_m,e_m): |e_m-s_m|> \lrceil{2/\varepsilon}, 1\le m \le M\}$ denote a set of randomly drawn intervals from $[1,n]$ that satisfies the length required by the trimming method. For each $\ell_m\in\cm$, we apply the aforementioned method to the sub-sequence $\{X_t\}_{t=s_m}^{e_m}$ and calculate the test statistic based on this segment, denoted by $T_{\ell_m}(\fX)$ and $V_{\ell_m}(\fX)$ with $\fX = \{X_t\}_{t=1}^{n}$. Let $G_{\ell_m}(\fX) = \max T_{\ell_m}(\fX) V_{\ell_m}^{-1/2}(\fX)$ denote the proposed test statistic based on segment $\ell_m=(s_m,e_m)$ and let $k_{\ell_m}(\fX)$ denote the point at which the maximum is attained. A threshold is needed to determine when to stop the binary search. To this end, we randomly generate $R$ samples of data, denoted by $\cR = \{\fZ^{1},\cdots,\fZ^{R}\}$, where $\fZ^i = \{Z_t^{i}\}_{t=1}^{n} \stsim{iid} \cn_p(0,I_p) \in \br^{p}$. Using the pre-define segment set $\cm$, we calculate the maximum segment-wise test statistic for each $\fZ^i$, i.e.
%\begin{equation*}
%    \xi_n^{i} = \max\limits_{1\le m \le M} G_{\ell_m}(\fZ^{i}),\qquad i=1,\cdots,R.
%\end{equation*}
%Define $\xi_n$ as the 95\% quantile of $\xi_n^{1},\cdots,\xi_n^{R}$, and we use this threshold to determine whether a segment contains a change point. 

%Now we are ready to introduce the overall testing procedure. We start with all the segments and suppose that $m_0 = \argmax\limits_{1\le m\le M} G_{\ell_m}(\fX)$. When $G_{\ell_{m_0}}(\fX) > \xi_n$, we conclude that $k_{\ell_{m_0}}(\fX)$ forms a change point and remove all the segments that contain $k_{ell_{m_0}}$ from $\cm$. The location $k_{\ell_{m_0}}(\fX)$ is used to break the original data into two pieces, i.e. $\{X_t\}_{t=1}^{k_{\ell_{m_0}}}$ and $\{X_t\}_{t=k_{\ell_{m_0}}+1}^{n}$, and we repeat the procedures to these two pieces respectively. The loop ends until no additional change points can be detected, i.e. all the test statistics do not exceed the threshold $\xi$.
...
%\xs{Give a brief summary of what to come next}

%\hg{(1) $T(\eta)$, $T^M(\eta)$; (2) replot the curves to correct the legends; (3) explain why only consider SN (fairness); (4) distinguish the symbol $\rho$ for temporal AR(1) and cross-sectional AR(1); (5) change $\eta$ to $\eta_0$ for \cite{wang2022inference}}
...
Section \ref{Sec:NumResults-simul-singleCP} and Section \ref{Sec:NumResults-simul-multipleCP} include the simulation results for single change point alternative and multiple change points alternative respectively, and each section reports the empirical size accuracy and the power under various dimensional settings and for several data generating processes. Some additional simulation results are presented in Appendix \ref{Appdix:Additional_simul}.


%----------------------------------------------------------------
\subsection{Single Change Point Testing}\label{Sec:NumResults-simul-singleCP}

We first consider the alternative of a single change point. The simulated data throughout Section \ref{Sec:NumResults-simul-singleCP} is generated from a $p$-dimensional AR(1) process, that is,
\begin{equation*}
    X_t - \mu_t = \kappa (X_{t-1} - \mu_{t-1}) + \epsilon_{t},
    \qquad 1 \le t \le n,
\end{equation*}
where $\{\epsilon_t\}_{t=1}^{n}$ are iid $p$-dimensional multivariate normal random vectors with mean zero and variance $\Sigma$. Three structures for $\Sigma$ are considered, namely, (1) AR ($\Sigma_{i,j} = \rho^{|i-j|}$); (2) CS ($\Sigma_{i,j} = 0.5 + 0.5\1{i=j}$); and (3) ID ($\Sigma_{i,j} = \1{i=j}$). These three models correspond to weak cross-sectional dependence, strong cross-sectional dependence and independence across components, respectively. 

We denote the dense test statistic proposed in Section \ref{Sec:Methodology-dense} as $G_{n,2}$ and its counterpart in Section \ref{Sec:Extension-sparse} as $G_{n,\infty}$. Additionally, we use \texttt{Bonf} to denote the Bonferroni test based on both $G_{n,2}$ and $G_{n,\infty}$. For the proposed method, we set the splitting parameter $\varepsilon=0.1$ and the trimming parameter $\eta=0.04$ throughout Section \ref{Sec:NumResults-simul-singleCP}. More simulation results with $\eta=0.02$ can be found in Appendix \ref{Appdix:Additional_simul} of the supplement. As for the comparison, we consider the SN-based trimming test statistic proposed in Section 4 of \cite{wang2022inference}, which is denoted by $T(\eta_0)$ with $\eta_0$ being the trimming parameter. We consider the trimming parameter $\eta_0\in\{0,0.01,0.02,0.05,0.1\}$. Note that the theory in \cite{wang2022inference} requires $p$ to grow to infinity thus it is tailored to high-dimensional data, and its performance in the low-dimensional setting is unknown. By contrast, our proposed test is supposed to be  dimension agnostic. 

%\xs{I wonder if we can also present the size results in plots/figures? For example, we can plot the size versus dimension, say $5*j, j=1,\cdots,200$?}
%\hg{Added in Section: Empirical Size}



%----------------------------------------------------------------
\subsubsection{Empirical Size for Single Change Point Tesing}\label{Sec:NumResults-simul-size-singleCP}

To examine the stability of size accuracy with respect to $p$, we plot the empirical size against the logarithm of $p$. We consider $n\in\{200,800\}$, $p\in\{5,10,20,25,50,100,150,200,250,500,750,1000,2500,5000\}$ and $\kappa\in\{0.4,0.7\}$. When $\Sigma$ takes the AR(1) form, we set $\rho=0.5$. 
%For the proposed method, we fix the splitting parameter $\varepsilon=0.1$ and the trimming parameter $\eta=0.04$ and for the trimming method $T(\eta_0)$ of \cite{wang2022inference}, we consider the trimming parameter $\eta_0\in\{0,0.01,0.02,0.05,0.1\}$. 
Under each setting, we conduct 5000 Monte Carlo replicates and plot the empirical size against the $\log(p)$, see Figure \ref{Fig:SizePlot-1}.

\begin{figure}[h!]
    \centering
    \includegraphics[height=175mm,width=140mm]{SizePlot-1.pdf}
    \caption{Empirical size curves versus the logarithm of $p$ against the single change point alternative}
    \label{Fig:SizePlot-1}
\end{figure}

In each plot, the solid curves in red, orange and green correspond to $G_{n,2}$, $G_{n,\infty}$ and \texttt{Bonf}, whereas the dotted lines in other colors represent $T(\eta_0)$ with different $\eta_0$. It is apparent that the SS-SN tests $G_{n,2}$, $G_{n,\infty}$ and \texttt{Bonf} have stable empirical size close to the significance level $\alpha=0.05$ regardless of $p$, and the accuracy significantly improves as $n$ increases from $200$ to $800$. By contrast, the SN test $T(\eta_0)$ in \cite{wang2022inference} exhibits quite a bit size distortion, especially in the compound symmetric case. This is not surprising as the theory in \cite{wang2022inference} suggests that their test only works for time series with weak cross-sectional dependence. Also the trimming parameter $\eta_0$ in the test of \cite{wang2022inference} plays an important role in the size accuracy. Overall our SS-SN tests have great advantage in size. 

It is worth mentioning that when the trimming parameter is set as $\eta=0.02$, we see no obvious difference in the size accuracy from that when $\eta=0.04$. Related additional numerical results for the empirical sizes are reported in Appendix \ref{Appdix:Additional_simul}.
...
\begin{equation*}
  ...
  = \left\{
    ...
    \right.
\end{equation*}
and for the sparse change point, the mean vector $\mu_t$ is generated by
\begin{equation*}
    \mu_t
  = \left\{
    \begin{array}{ll}
...
%    \hline
%        \diagbox{$\Sigma$}{$p$} & 3 & 10 & 100 & 500 \\ \hline
%        AR & $\{0, 0.5, \cdots, 5\}$ & $\{0, 0.7, \cdots, 7\}$ & $\{0, 1.2, \cdots, 12\}$ & $\{0, 1.5, \cdots, 15\}$ \\ \hline
%        CS & $\{0, 0.4, \cdots, 4\}$ & $\{0, 0.7, \cdots, 7\}$ & $\{0, 2, \cdots, 20\}$ & $\{0, 5, \cdots, 50\}$ \\ \hline
%        ID & $\{0, 0.4, \cdots, 4\}$ & $\{0, 0.5, \cdots, 5\}$ & $\{0, 0.8, \cdots, 8\}$ & $\{0, 1, \cdots, 10\}$ \\ \hline
%    \end{tabular}}
%    \caption{The range of $\delta$ for each pair of $(p,\Sigma)$ for the single dense alternative}
%    \label{Tab:DenseDelta}
%\end{table}



%Again, the selections of $\delta$ for each pair of $(p,\Sigma)$ are shown in Table \ref{Tab:SparseDelta}:
...
%    \hline
%        \diagbox{$\Sigma$}{$p$} & 3 & 10 & 100 & 500 \\ \hline
%        AR & $\{0, 0.4, \cdots, 4\}$ & $\{0, 0.6, \cdots, 6\}$ & $\{0, 0.9, \cdots, 9\}$ & $\{0, 1.4, \cdots, 14\}$ \\ \hline
%        CS & $\{0, 0.4, \cdots, 4\}$ & $\{0, 0.6, \cdots, 6\}$ & $\{0, 1.2 \cdots, 12\}$ & $\{0, 2, \cdots, 20\}$ \\ \hline
%        ID & $\{0, 0.4, \cdots, 4\}$ & $\{0, 0.6, \cdots, 6\}$ & $\{0, 0.8, \cdots, 8\}$ & $\{0, 1.2, \cdots, 12\}$ \\ \hline
%    \end{tabular}}
%    \caption{The range of $\delta$ for each pair of $(p,\Sigma)$ for the single sparse alternative}
%    \label{Tab:SparseDelta}
%\end{table}

Apart from the trimming method $T(\eta_0)$ proposed by \cite{wang2022inference}, we also compare the results of our method with those of $\texttt{SN}$ introduced in \cite{shao2010testing} when $p$ is no larger than 10. Under each parameter setting, we plot the size-adjusted power of each method against $c$, see Figure \ref{Fig:DensePowerCurve} and Figure \ref{Fig:SparsePowerCurve}. In each figure, the three rows correspond to three structures of $\Sigma$ (i.e., AR, CS and ID) and the four columns stand for different values of $p$. All the simulation results are averaged over 5000 Monte-Carlo replicates.

%\hg{remove $\eta=0.02$ and only keep $\eta=0.04$, correct legends to $T(\eta)$}

\begin{figure}[h!]
    \centering
    \includegraphics[height=110mm,width=145mm]{TSCP-DensePower-2.pdf}
    \caption{Power curves (size-adjusted) against a single dense change point when $n=200$}
    \label{Fig:DensePowerCurve}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[height=110mm,width=145mm]{TSCP-SparsePower-2.pdf}
    \caption{Power curves (size-adjusted) against a single sparse change point when $n=200$}
    \label{Fig:SparsePowerCurve}
\end{figure}

%\xs{rewrite the paragraph below}

As shown in Figure \ref{Fig:DensePowerCurve}, for dense alternative,  $G_{n,2}$ has similar performance with \texttt{Bonf} under all dimensional settings and both appear superior to $G_{n,\infty}$,  which is expected. The power loss of $G_{n,\infty}$
seems less for small $p$ or in the compound symmetric case and grows as the dimension $p$ increases. In comparison, 
%is consistent to the dependence structure of the data. Under low dimensional settings, $G_{n,\infty}$ only has minimal power loss when compared to $G_{n,2}$ and \texttt{Bonf}, but the power loss becomes severer as $p$ grows. However, a surprising observation is that $G_{n,\infty}$ appears to gain some power when there is stronger cross-sectional dependence within the data. As for the comparison methods, 
high-dimensional SN test $T(\eta_0)$ in \cite{wang2022inference} attains the most power under all settings, even when $p$ is small. In most cases, $T(\eta_0)$ is more powerful with smaller $\eta_0$, but the impact of the trimming parameter $\eta_0$ on the power seems not much. In the ID case, the fixed-dimensional SN method of \cite{shao2010testing} has better power behavior than the proposed SS-SN methods, but it suffers quite a bit power loss when the data exhibits some componentwise dependence (i.e., AR or CS), which matches our discussion in Section \ref{Sec:Theory-power}; also see Figure \ref{Fig:PowerPlot-DGP1}.

For sparse alternative, the ordering of these methods in power very much depends on  the dependence structure $\Sigma$ and the dimension. Specifically, \texttt{SN} in \cite{shao2010testing} achieves the best power under the existence of cross-sectional dependence (AR or CS) when $p$ is small ($p=3,10$), which is contrary to our observation in the dense alternative.
 We observe that $G_{n,\infty}$ has comparable empirical rejection rate with $\texttt{Bonf}$, and both outperform $G_{n,2}$ as expected. Although our methods do not show much advantage in the ID case, it is interesting to see that the power of $G_{n,\infty}$ and $\texttt{Bonf}$ significantly outperform all others in both AR and CS cases when $p=100, 500$. The performance of $T(\eta_0)$ with multiple $\eta_0$ seems similar across all the settings. Overall, the $\texttt{Bonf}$ exhibits good all round power in both sparse and dense alternatives. 
 
 %though in this case, the smaller trimming parameter $\eta_0$ surprisingly leads to better power result under the low dimensional setting and the increase $p$ does not improve the power. 
 
 %Note that based on our generation of $\delta$, the change points has a single nonzero component regardless of $p$, which implies that the sparsity of the mean shift increases as $p$ grows and can explain why the power of $T(\eta_0)$ even drops as $p$ increase.
%According to the power curves, we notice that under the dense alternative, the performance of the Bonferroni test is competitive against both dense and sparse alternatives, which validates its effectiveness in practice. When compared to $T(\eta_0)$ and $\texttt{SN}$, we observe some power loss of our proposed method against the dense alternative, whereas our method gains some advantage when it comes to the sparse change point. As shown in Figure \ref{Fig:SparsePowerCurve}, $G_{n,\infty}$ and $\texttt{Bonf}$ beat all the other methods in some cases, especially when the cross-sectional dependence within the data is relatively strong.


%----------------------------------------------------------------
\subsection{Multiple Change Points Tesing}\label{Sec:NumResults-simul-multipleCP}

%{\color{PineGreen}

In this section, we look into the finite sample performance against the multiple change points alternative. We generate an independent sample $\{X_t\}_{t=1}^{n}$ from a $p$-dimensional normal distribution with mean $\mu_t$ and covariance matrix $\Sigma$. The same three types of $\Sigma$ are considered, that is, (1) AR ($\Sigma_{i,j} = \rho^{|i-j|}$); (2) CS ($\Sigma_{i,j} = 0.5 + 0.5\1{i=j}$); and (3) ID ($\Sigma_{i,j} = \1{i=j}$). Similar to Section \ref{Sec:NumResults-simul-singleCP}, we denote the test statistics targeting at dense and sparse alternatives by $G_{n,2}^M$ and $G_{n,\infty}^M$ respectively, and denote the Bonferroni test as $\texttt{Bonf}^M$. For the proposed method, we set the splitting ratio $\varepsilon=0.1$ and consider the trimming ratio $\eta=0.02$ in Section \ref{Sec:NumResults-simul-multipleCP}. 
...
%----------------------------------------------------------------
\subsubsection{Empirical Size for Multiple Change Points Testing}\label{Sec:NumResults-simul-size-multiCP}

To demonstrate the dimension-agnostic property against the multiple change points alternative, we plot the empirical size against the logarithm of the $p$. We fix $\mu_t=(0,\cdots,0)^{\top}$ for all $t$'s and consider $n\in\{100,300\}$, $p\in\{5,10,20,25,50,100,150,200,250,500,750,1000,2500,5000\}$. When $\Sigma$ takes the form of \texttt{AR}, we consider $\rho\in\{0.4,0.7\}$. All the empirical sizes are averaged over $T=5000$ Monte Carlo replicates, and we plot the empirical size against the logarithm of $p$ in Figure \ref{Fig:SizePlot-M-2+3+4}.  More numerical results can be found in Appendix \ref{Appdix:Additional_simul}.

%For the proposed method, we set the splitting parameter $\varepsilon=0.1$ and the trimming parameter $\eta=0.02$. 

%\hg{add another panel with $n=300$}

\begin{figure}[h!]
% TSCP-SizePlot-M-2 for ID and TSCP-SizePlot-M-3 for AR,CS
    \centering
    \includegraphics[height=85mm,width=145mm]{SizePlot-N-1+2.pdf}
...
%\xs{rewrite the paragraph after we get the new results for $n=300$}
...
In general, when the componentwise dependence is weak, $T^{M}(0.1)$ achieves more accurate  empirical size either as the sample size $n$ grows or $p$ increases. Overall, the size accuracy and stability of our SS-SN tests across all dimensional settings is impressive and they are far superior to those for $T^M(0.1)$.
%is still a major advantage of the proposed SS-SN methods, though at a price of power loss; see discussion in the next section.

%The pattern of size accuracy against the multiple change points matches the single alternative counterpart. Specifically, our proposed methods achieve stable size accuracy across all dimensional settings, even when the sample size is relatively small. By contrast, the test in \cite{wang2022inference} has noticeable size distortion under all settings when $n=100$. When the sample size increases to $n=300$ and $p$ is sufficiently large, we notice a significant improvement of size accuracy. Additionally, we observe that the cross-sectional dependence within the data has strong impact on the test in \cite{wang2022inference}, whereas SS-SN test has consistent size accuracy to various dependence structures.
%\xs{we might also add the same plot but for different type of covariance matrix for independent data, such as AR(1), compound symmetric}
...
%----------------------------------------------------------------
\subsubsection{Power Analysis for Multiple Change Points Testing}\label{Sec:NumResults-simul-power-multiCP}

Lastly, we examine the power behavior against the multiple change points alternative. We still generate a sample of iid vectors $\{\widetilde{X}_t\}_{t=1}^{n} \in \br^p$ from $\cn_p(0,\Sigma)$ with three types of $\Sigma$. For the AR(1) structure, we set $\rho=0.5$. In this section, we only present the power comparison against two dense change points or two sparse change points, denoted by \texttt{D-D} and \texttt{S-S} respectively. More settings are investigated in the supplementary material; see Appendix \ref{Appdix:Additional_simul}.

In particular, we generate the mean sequence $\{\mu_t\}_{t=1}^{n}$ for \texttt{D-D} as follows:
\BEqn
...
\EEqn
and generate the mean vectors for \texttt{S-S} as below:
\BEqn
& & \mu_1 = \cdots = \mu_{k_1} = (0,\cdots,0)^{\top} \quad\mbox{with } k_1=\lrfl{0.3n},\\
...
Under both scenarios, we fix $n=100$ and consider $p\in\{10,50,100,500\}$ as well as a respective grid of signal-noise-ratio $c$'s. Figure \ref{Fig:PowerCurve-M-DD} and Figure \ref{Fig:PowerCurve-M-SS} present the size-adjusted power curves based on $T=1000$ MC replicates.

%\hg{add another setting with both sparse \texttt{S-S}}

\begin{figure}[h!]
    \centering
    \includegraphics[height=110mm,width=145mm]{PowerPlot-N-2-DD.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=110mm,width=145mm]{PowerPlot-N-2-SS.pdf}
    \caption{Power curves (size-adjusted) against multiple change points of type \texttt{S-S} when $n=100$}
    \label{Fig:PowerCurve-M-SS}
\end{figure}


In each plot, the four columns correspond to different dimensional settings, and the three rows refer to different structures of $\Sigma$. As seen from Figure \ref{Fig:PowerCurve-M-DD} and Figure \ref{Fig:PowerCurve-M-SS}, the pattern generally matches those in Section \ref{Sec:NumResults-simul-power-singleCP}. In the \texttt{D-D} case, the sparse test $G_{n,\infty}^{M}$ does not show much power against multiple dense change points, and  its power loss is relatively much less  in the CS case, which is also observed in the case of single dense alternative. Also we see a slight advantage of $G_{n,2}^M$ over $\texttt{Bonf}^{M}$, mostly under the non-ID case, which is expected  and both tests are consistent and their powers go to $1$ when the signal-to-noise ratio is sufficiently large. The most powerful test is $T^M(0.1)$ and the tradeoff between size distortion and power loss is apparent from these plots.


%to the varying dimensionality and cross-sectional dependence. As observed and discussed in Section \ref{Sec:NumResults-simul-power-singleCP}, our methods all have power loss when compared to $T^M(0.1)$ in \cite{wang2022inference}. However, the power loss seems to improve as $p$ increases.
%A potential reason is that the performance of the Bonferroni test is heavily held back by the sparse test. 

%The power loss against the multiple alternative seems to be severer than that against the single alternative. Intuitively, for the single change point alternative, testing for $\mu_1=\mu_n$ in the original data is equivalent to testing for $\|\mu_1-\mu_n\|_2$ in the projected data. By contrast, these two testings differ when it comes to the multiple change point alternative, and the effectiveness of the test $G_{n,2}^{M}$ has a strong dependence on the relation between the overall mean shift $\delta$ and all consecutive mean shifts $\delta_j$'s, and this might explain why we observe more power loss under this setting.\xs{do we need this paragraph?}
...
%{\color{BrickRed}
%Power curves when $n=100$ are inserted above. Originally, I planned to consider multiple $n$'s, but the plots with $n=100$ already seem to be too enough and takes up too much space. Does it still improve this paper if we consider more $n$'s? Would it be better if we only present the power curves when another $n$, say 200 or 300?}




%----------------------------------------------------------------
%\subsubsection{To be removed}

%Consider the case when the two change points are extremely close. All the settings are exactly the same as in Section \ref{Sec:NumResults-simul-power-multiCP}. The mean shifts are generated as follows:
%\BEqn
%& & \mu_1 = \cdots = \mu_{k_1} = (0,\cdots,0)^{\top} \quad\mbox{with } k_1=\lrfl{0.5n},\\
%& & \mu_{k_1+1} = \cdots = \mu_{k_2} = c (1,\cdots,1)^{\top} / \sqrt{p} \quad\mbox{with } k_2=\lrfl{0.55n},\\
%& & \mu_{k_2+1} = \cdots = \mu_{n} = 2c (1,\cdots,1)^{\top} / \sqrt{p}.
%\EEqn
%That is, the second block only contains 5 observations when $n=100$.
    
%The power curves against $c$ is shown in Figure \ref{Fig:PowerCurve-M-DD-2}. The power patterns in this case have no much difference from the case when the two change points are well separated, and we don't see significant power loss of $T^M(0.1)$ in \cite{wang2022inference}. In some sense, the DGP in this case reduces to the case with a single change point at $k_1=\lrfl{0.5n}$, and the two blocks has mean zero and mean $2c (1,\cdots,1)^{\top} / \sqrt{p}$ respectively, Therefore, even though the second change point is not detected, the entire sequence still has a detectable change point in the middle. Hence, the trimming parameter used by $T^M(0.1)$ wouldn't cause a failure when testing for the multiple change points. However, when it comes to the estimation and localization of multiple change points, we might expect $T^M(0.1)$ to detect only a single change point, whereas our method might be able to detect both. But the estimation is beyond the scope of this article.

%\begin{figure}[h!]
%    \centering
%    \includegraphics[height=100mm,width=140mm]{PowerPlot-M-6-DD.pdf}
%    \caption{Power curves (size-adjusted) against two dense change points with minimal distance when $n=100$}
%    \label{Fig:PowerCurve-M-DD-2}
%\end{figure}
...
\section{Discussion}\label{Sec:Discussion}
 ...
 There is a vast literature on change point testing for both low-dimensional and high-dimensional data with or without temporal dependence. Almost all  existing test statistics  only work in a pre-determined dimensional regimes and tests developed for low-dimensional setting may not work for high-dimensional data and vice versa. 
In this paper, we advance the dimension-agnostic inference first proposed in \cite{kim2020dimensionagnostic} for iid data to the mean change point problem in the multivariate time series setting. Specifically, We adopt the sample splitting with trimming, projection and self-normalization idea to develop new test statistics for both single change point and multiple change point alternatives. On the theory front, we derive the limiting null distributions for the proposed test statistics under three data generating processes, which encompass a broad range of dimensionality  and arbitrary cross-sectional dependence. For both single and multiple change point testing, the limiting null distribution is pivotal and stays the same across both fixed and growing dimensional regimes, hence the selection of dimension-dependent calibration threshold can be avoided. Additionally, we provide a rigorous analysis of  asymptotic power behavior of the proposed tests. 
Monte Carlo simulation results strongly corroborate the theoretical phenomenon we discovered, and suggest that these dimension-agnostic test statistics maintain very accurate size across  a wide range of dimensions, albeit with a moderate amount of power loss in certain settings. 


To conclude, we mention a few topics that are worth further investigation. Firstly, as we mentioned early, the asymptotic theory for the dimension-agnostic test that targets sparse alternative is yet to be developed. Preliminary investigation seems to suggest that it is highly nontrivial. Secondly, we focus on the mean change point testing in this paper, and it would be of interest to develop new dimension-agnostic test statistics that target changes in other parameters, such as covariance matrix and marginal quantiles. Thirdly, estimation or localization of change points is an important topic. A natural next step is to combine our test statistic that targets single change point with a generic segmentation algorithm, such as Wild Binary Segmentation \cite{fryzlewicz2014wild}, or  Seeded Binary Segmentation \cite{kovacs2020seeded}. Alternatively, it might be possible to extend the nested window based SN-segmentation approach in \cite{zhaojiangshao2022}. Further research along these directions are well
underway.
...
\begin{appendices}
\appendixpage
The supplementary material contains all the proofs for the theoretical results established in this article and some additional simulation results. In particular, Appendix \ref{Appdix:MainThm} includes the proofs for all the propositions and theorems stated from Section \ref{Sec:Theory-null} to Section \ref{Sec:Extension}. The lemmas directly used by these proofs are presented in Appendix \ref{Appdix:AuxLemma-1}, and some other auxiliary lemmas are included in Appendix \ref{Appdix:AuxLemma-2}. Appendix \ref{Appdix:Additional_simul} contains some additional simulation results as a complement to those in Section \ref{Sec:NumResults-simul}.
...
It follows from Definition \ref{Def:Model_Fixedp} that $\frac{1}{\sqrt{n}} \sum\limits_{t=1}^{\lfloor{nr}\rfloor} X_t \leadsto (\Omega^{(1)})^{1/2} B_p(r)$ in $D[0,1]$ as $n\rightarrow\infty$, where $\{B_p(r)\}_{r\in[0,1]}$ denotes the standard $p$-dimensional Brownian motion. For any $r\in[0,1]$, we have that
\BEqn
    S_{1,\lrfl{Nr}} 
...
\begin{enumerate}[label=(\roman*)]
    ...
    \quad\mbox{ jointly in }D[\varepsilon,1-\varepsilon]
    \end{equation*}
...
\EEqn
where $\Xi_n(\varepsilon) = \{(\ell_1,\ell_2): 1<\ell_1<\ell_2<N=\lrfl{(1-2\varepsilon)n}, \ell_2-\ell_1>1\}$ and $\Xi(\varepsilon) = \{(r_1,r_2): \varepsilon \le r_1 \le r_2 \le 1-\varepsilon\}$ as defined in Section \ref{Sec:Extension-multipleCP} and we use $\sup^{\ast}\limits_{x\in \ca} g(x) = \sup\limits_{x\in\ca} |g(x)|$ to denote the supremum of the absolute values. Similarly,
\BEqn
...
%----------------------------------------------------------------
%	APPENDIX B: LEMMAS FOR THE ASYMPTOTIC THEORIES
%----------------------------------------------------------------
...
In this section, we present some additional simulation results as a complement to those in Section \ref{Sec:NumResults-simul}. Specifically, Appendix \ref{Appendix:AddNumResults-size-singleCP} and Appendix \ref{Appendix:AddNumResults-size-multiCP} include some empirical size results for single change point testing and multiple change points testing respectively. Appendix \ref{Appendix:AddNumResults-power-multiCP} contains some additional power comparison for multiple change points testing.

%----------------------------------------------------------------
\subsection{Empirical Size for Single Change Point Testing}\label{Appendix:AddNumResults-size-singleCP}

We have plotted the empirical sizes against the logarithm of $p$ in Section \ref{Sec:NumResults-simul-size-singleCP}. Under the same data generating process as in Section \ref{Sec:NumResults-simul-singleCP}, we report some additional results on size.  

We set $\mu_t=(0,\cdots,0)^{\top}$ to be the zero vector for $t=1,\cdots,n$, set $\rho=0.8$ for AR(1) type $\Sigma$, and consider $n\in\{200,800\}$, $p\in\{3,10,100,500\}$ and $\kappa\in\{0,0.4,0.7\}$. The empirical sizes when $n=200$ and $n=800$ averaged over 5000 Monte-Carlo replicates are reported in Table \ref{Tab:AR1-Size-1} and Table $\ref{Tab:AR1-Size-2}$ respectively.
...
%{\color{PineGreen}
In this section, we present  the empirical sizes  as a supplement to the plots in Section \ref{Sec:NumResults-simul-size-multiCP}.

We generate a sample of $n$ iid random vectors from $\cn_p(0,\Sigma)$ and consider four types of $\Sigma$ same as those in Section \ref{Sec:NumResults-simul-size-multiCP}. We consider $n\in\{100,300\}$, $p\in\{5,10,100,500\}$. For the proposed method, we fix the splitting parameter $\epsilon=0.1$ and consider the trimming parameter $\eta\in\{0.02,0.04\}$. We still adopt $T^{M}(0.1)$ of \cite{wang2022inference} as comparison. The empirical sizes averaged over 5000 Monte-Carlo replicates are reported in Table \ref{Tab:Multi-Size}.
	As shown in Table \ref{Tab:Multi-Size}, the proposed multiple change points testing method achieves stable size accuracy regardless of $p$ and the overall empirical size is already accurate when $n=100$. Overall the finding is quantitatively similar to those reported in the main paper. 
 %In general, $G_{n,2}^{M}$ and $G_{n,\infty}^{M}$ have more accurate size than the Bonferroni test, though we haven't investigated this observation theoretically.
...
In this section, we generate the iid sequence $\{X_t\}_{t=1}^{n}$ in the same way as in Section \ref{Sec:NumResults-simul-multipleCP}, and consider the following two settings of change points.
...
Again, we fix $n=100$ and consider $p\in\{10,50,100,500\}$. For each pair of $(n,p)$ and change point scenario, we choose a corresponding grid of signal-noise-ratio $c$'s. In the case where $\Sigma$ is of AR(1) type, we set $\rho=0.5$. The size-adjusted power averaged over $T=1000$ MC replicates are plotted against $c$; see Figure \ref{Fig:PowerCurve-M-DDD} for \texttt{D-D-D} and Figure \ref{Fig:PowerCurve-M-DS} for \texttt{D-S}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{PowerPlot-N-2-DDD.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{PowerPlot-N-2-DS.pdf}
    \caption{Power curves (size-adjusted) against multiple change points of type \texttt{D-S} when $n=100$}
    \label{Fig:PowerCurve-M-DS}
\end{figure}


The four columns in each plot of Figure \ref{Fig:PowerCurve-M-DDD} and \ref{Fig:PowerCurve-M-DS} correspond to the cases when $p=10,50,100$ and $500$ whereas the three rows refer to the three types of $\Sigma$. The overall patterns shown in the plot generally match those in Section \ref{Sec:NumResults-simul-power-singleCP} and Section \ref{Sec:NumResults-simul-power-multiCP}. It is not surprising that $G_{n,\infty}^{M}$ still has severe power loss in the case of \texttt{D-D-D}, especially when $\Sigma$ takes an identity matrix or an $AR(1)$ matrix. As expected, the power of $G_{n,\infty}^{M}$ improves under the existence of sparse mean shift, i.e., in the $\texttt{D-S}$ case. The performance of $G_{n,2}^{M}$ and \texttt{Bonf} are similar in most scenarios, though both of them still have a noticeable power loss when compared to $T^M(0.1)$ in \cite{wang2022inference}, except when $p$ is large and the cross-sectional dependence is strong.
