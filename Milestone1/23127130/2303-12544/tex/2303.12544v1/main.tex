...
\begin{figure*}%[tb]
   ...
   \includegraphics[width=0.9\textwidth]{Fig1.pdf}
       ...
       \textbf{c)} Times for calculation of central frequencies from a dual ODMR experiment (set of $12\times2$ arrays) as a function of array size. The times are given for a 2.5 GHz 4-core macbook pro with 16 MGb operative memory. }
  \label{results}
\end{figure*}
...
The computation times required by both methods versus imaging array size is plotted on Fig.~\ref{results}c. Both line plots are obtained by processing resampled datasets in Wolfram Mathematica on a 2.5 GHz 4-core macbook pro with 16 MGb operative memory.  It takes seven minutes to process a data set of $400\times400\times24$, while the weighted-mean method fits in half a second. Regardless of the size of the array, the fitting method is $>800$ slower than the weighted-mean method. Interestingly that about a tenfold speedup in Mathematica is due to the use of matrix calculations instead of parallel lopping.
...
\section{Summary} 
We have proposed and demonstrated a weighted-mean method for calculating center frequencies from ODMR spectra as an alternative to the redundant, slow and unstable fitting method. Our method is $10\%$ less accurate, but several hundred times faster, which can increase data collection and, in turn, sensitivity by $20\%$. The method is almost instantaneous and paves the way for further developments in magnetic imaging with a large field of view.
