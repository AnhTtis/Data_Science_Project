...
These tubes are usually polished to a average roughness of 25--$150\,\mathrm{nm}$.
...
Because of this unique property, UCN can be transported like a gas through guide tubes or stored in containers made of or coated by such materials.
...
Consequently, the surface roughness of the guide tube is one of the factors that diminishes the statistical accuracy of nEDM experiments.
...
However, as they are metal tubes, their surfaces exhibit a high roughness with a root mean square (RMS) amplitude much greater than $5 \,\mathrm{nm}$ and random surface waviness on a micrometer scale.
...
In this model, the luminous intensity to a given direction from a microarea is distributed in proportion to the cosine of the emission angle from the surface normals.
...
Therefore, the calculations cannot deal with large surface waviness of guide tubes such as those used in the TUCAN experiments.
...
\begin{figure*}[ht]
  ...
  The right panel shows the TOF of the incident UCNs until they reaches the downstream of the guide tube.  
  \label{fig_ex03}}
\end{figure*}
...
This model treats the surface roughness as an assembly of small specular surfaces (microfacets) like crumpled aluminum foil, and scatters the surface normal directions by modeling the statistical distribution of the slopes of the microfacets.
...
This model is typically used when the size of the surface undulations is sufficiently larger than the wavelength of the rays (in such case, the undulations are referred to as surface waviness rather than roughness).
...
In the ``ping-pong'' experiment performed at this UCN source~\cite{Bison2022}, where UCNs contained in a storage cell at beamport West-1 were transported to beamport South, the peak of the measured time spectrum was little wider and delayed compared to a simulation by the Lambert model.
...
An experiment to compare the Lambert model with the MR model was performed by Atchison {\it et al.}~\cite{Atchison2010} by transporting a continuous stream of UCN with limited angle of incidence through sample plate facing each other, and comparing the attenuation of the transport efficiency to a simulation.
However, the samples measured in this experiment were flat plates with small surface roughness of $b = 1$--$3\,\mathrm{nm}$.
...
Noting the similarity between optics of light and neutrons, we used these models to describe the scattering of UCN on the rough surface, assuming that geometrical optics holds in structures larger than the wavelength of a neutron.
...
In the experiment, pulsed UCNs with velocities of $6$--$10\,\mathrm{m/s}$ were incident on a sample guide tube and the TOF of the transmitted UCNs were measured while changing the angle of incidence on the guide tube between $0^{\circ}$, $10^{\circ}$, $15^{\circ}$, and $30^{\circ}$.
...
The UCN beam was deflected with an NiC-sputtered 2-mm-thick silicon wafer~\cite{Hino2015} before entering the sample guide because of the limitation of the experimental space.
...
The optical distance along the central axis from the entrance of collimator (a) to the exit of collimator (c) is $413\,\mathrm{mm}$, and the maximum divergence angle of UCN calculated geometrically from the collimator size is $\pm 6.6^{\circ}$.
...
Each of these angled flanges were made of aluminum 5056 alloy with the Fermi potential of $55\,\mathrm{neV}$.
...
The measured TOF and the velocity distribution obtained from the TOF and the flight distance along the optical axis is shown in Fig.~\ref{fig3}.
...
\begin{figure}[htbp]
  ...
  \caption{Measured rate of transmitted neutrons with no guide (a red square) and with sample guide installed at different angles (black dots), compared to simulations with no diffuse reflection (blue triangles), pure Lambert scattering with probability $p_{L} = 0.081$ (orange circles), and mf-BRDF scattering combined with Lambert scattering with probability $p_{L} = 0.039$ (pink squares). Due to gravity and incidence angle UCN have to undergo between 1 and 7 specular reflections to reach the detector, reducing the transmission compared to the no-diffusion simulation. \label{fig4}}
\end{figure}
...
The simulations are three-dimensional Monte Carlo particle transport calculations based on ray tracing methods including gravity.
\begin{figure*}[htbp]
  \centering
...
The surface consists of microfacets having randomly oriented normal vectors $\bm{m}$.
In this figure, an average height plane of microfacets, having a normal vector $\bm{n}$, is describes as ``Macrosurface'' and a dashed line.
We assume that the microarea $dA$ on the macrosurface is the area that is being irradiated.
...
To estimate the distribution of scattering directions of a single particle incident on the microarea, we used the following equation based on Ref.~\cite{Walter2007},
\begin{eqnarray}
...
\end{eqnarray}
where $\theta_{i}$, $\theta_{m}$, $\theta_{o}$ are the polar angle of the incoming direction, microfacet normal, and outgoing direction of the particle, $\theta_{om}$ is the angle between the vectors $\bm{o}$ and $\bm{m}$, $\Phi_{o}$ is the total reflected flux from the microarea, $1/(|\cos \theta_{i}|dA)$ is the incident flux of the particle, and $F$ is the Fresnel reflectivity of the microfacet.
...
\end{eqnarray}
where $\theta_{im}$ is the angle between the vectors $\bm{i}$ and $\bm{m}$, and $\mathrm{erf}(x)$ is the error function.
...
Because the NDF $D$ is expressed more simply as a function of $\theta_{m}$ than $\theta_{o}$, we produced a random distribution of $(\theta_{m},\phi_{m})$ for the simulation and obtained corresponding $(\theta_{o},\phi_{o})$ distribution by taking into account the following Jacobian,
\begin{equation}
    d\Omega_{m}=\left\| \frac{d\Omega_{m}}{d\Omega_{o}} \right\|d\Omega_{o} = \frac{1}{4 \cos \theta_{mo}}d\Omega_{o}.       \label{equ09}
\end{equation}

The Beckmann NDF is equivalent to the probability distribution of the two-dimensional slope of a microfacet described by a two-dimensional normal distribution with mean zero and isotropic.
...
\end{equation}
From this relationship, this distribution can be regarded as calculating the percentage of the projection area of numerous microfacets with slope $\tan \theta_{m}$ in the microarea shown in Fig.~\ref{fig_ex1}.
...
Although the Beckmann NDF is known to show a greater reduction in probability in regions of large $\theta_{m}$ than the more technical statistical models created later~\cite{Walter2007,Trowbridge1975}, we chose this model because it provides a better physical understanding of surface roughness.
\begin{figure}[htbp]
  \centering
...
The measurement ranges were $2\,\mathrm{\mu m} \times 2\,\mathrm{\mu m}$ and $10\,\mathrm{\mu m} \times 10\,\mathrm{\mu m}$, with a resolution of $7.8\,\mathrm{nm}$ and $39\,\mathrm{nm}$, and the number of measured locations was 8 and 2, respectively.
...
These structures shorter than the effective neutron wavelength, corresponding to a neutron momentum perpendicular to the surface, $\lambda / \cos \theta_{i}$, are not used in the mf-BRDF model, and are noise.
Therefore, high-frequency roughness was removed by Fourier filter processing.
The result of a filtering with a cutoff wavelength of $\lambda_{c} = 128\,\mathrm{nm}$ is shown in Fig.~\ref{fig5}(b). After processing the AFM image by a cutoff, normal vectors of each data point on the processed image were calculated and a distribution of polar angles $\theta_{m}$ was created. The $\theta_{m}$ distribution is shown in Fig.~\ref{fig6}. The value of $\alpha$ was obtained by fitting it using Eq.~\ref{equ03}.
\begin{figure}[htbp]
  \centering
...
We performed the process for different cutoffs, allowing us to derive the value of $\alpha$ corresponding to each value of $\lambda_{c}$. 
...
\begin{figure}[htbp]
  ...
  \caption{Variation of $\alpha$ with respect to $\lambda_{c}$ and the fitted power function.
...
The mf-BRDF model was implemented in the simulation by loading a pre-computed 2D histograms of reflection angle distributions $(\theta_{o}, \phi_{o})$.
A total of 18,000 histograms were created by changing the incident angle $\theta_{i}$ from $0^{\circ}$ to $89^{\circ}$ in $1^{\circ}$ step and the width $\alpha$ from 0 to 0.198 with 0.002 step.
The angular resolution of the histograms is $1^{\circ}$ for both $\theta_{o}$ and $\phi_{o}$.
At a reflection, the histogram corresponding to $\theta_{i}$ and $\alpha$ was selected, and the reflection angles $(\theta_{o}, \phi_{o})$ were determined using random numbers based on the histogram.
...
The transport efficiency in the no-roughness simulation is reduced compared to the no-guide measurement because of the reduction in diameter between sample guide tube and detector window.
On the other hand, even at an angle of $0^{\circ}$, this simulated efficiency is still larger than that of the measurement.
%Due to gravity, UCN undergo one reflection on average even with the sample guide installed at a $0^{\circ}$ angle, further reducing measured transmittance compared to the no-roughness simulation.
...
A least squares fit with $p_{L}$ as a free parameter was performed, and the result is shown in Fig.~\ref{fig4} as orange circles with dotted line.
...
A least-squares fit to the measured transport efficiency using a simulation with the mf-BRDF model combined with the Lambert model is shown by the pink squares with dashed line in Fig.~\ref{fig4}, where $p_{L} = 0.039 \pm 0.003$ was given as the best fit.
...
The increase in lobe-like reflections increases the number of UCN incident on the wall at speeds above the critical velocity of total reflection of $6.3\,\mathrm{m/s}$ for the NiP, resulting in a significant loss increase.
...
The main cause of UCN loss was the back scattering in the former case and the loss per bounce effect in the latter case.
...
\begin{figure}[htbp]
  ...
  \caption{Reduction of the transport efficiency of UCN at a velocity of $5\,\mathrm{m/s}$ with different diffuse scattering models when the length of guide tube is extended. In (a) the UCN returned to the inlet is assumed to be completely lost (open inlet). The statistical errors are smaller than that of the markers. In (b) the calculation is performed under the condition that the UCN returned to the inlet is reflected by a NiP mirror (closed inlet). See text for calculation settings.
\label{fig9}}
\end{figure}
...
In the case where the UCN inlet is closed by an NiP mirror, UCNs remain in the guide tube for a longer period compared to the open inlet because backscattered UCNs being reflected at the inlet.
Consequently, in this calculatons, the direction of UCN motion would be more randomized, and the contribution of the mf-BRDF model would be weakened.
...
In the future, the description of UCN scattering by structures smaller than the incident wavelength, which was ignored in the present modeling, could be achieved by analytical calculations using the Distorted Wave Born Approximation (DWBA)~\cite{Sinha1988} or by a direct numerical calculation of wave equations relying on the processing power of a computer.
...
The decrease in transport efficiency and the deformation of the TOF spectrum with increasing angle were measured.
A least-squares fit to the measured UCN transport efficiencies was performed using simulations implementing the Lambertian diffusion with the diffusion probability $p_{L}$ as a free parameter, and the results was $p_{L} = 0.081\pm0.003$ at $\chi^{2}/\mathrm{ndf} = 11$.
...
In our model, surface waviness larger than the UCN wavelength is extracted by Fourier analysis, and the distribution of the surface normals is incorporated into an mf-BRDF model based on geometrical optics.
...
Therefore, polishing of the smaller structures rather than the surface waviness was found to be the priority issue for this guide tube.
