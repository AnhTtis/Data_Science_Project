...
\begin{enumerate}[leftmargin=*]
    \item $(\mathbf{p}_k)_1^n$ is not a trivial perturbation (it is a trivial perturbation if there exist $\boldsymbol{\Omega} \in \Skew(d)$ and $\mathbf{t} \in \mathbb{R}^d$ such that $\mathbf{p}_k = \boldsymbol{\Omega}\mathbf{x}_k + \mathbf{t}$),
