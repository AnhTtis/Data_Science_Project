...
\begin{align}
    \mathcal{A}_0 \coloneqq \min_{\substack{(\mathbf{S}_i)_1^m \subseteq \mathbb{O}(d)\\(\mathbf{t}_i)_1^m \subseteq \mathbb{R}^d}}\sum_{\substack{(k,i)\in E(\Gamma)\\(k,j)\in E(\Gamma)}}\left\|(\mathbf{S}_i^T\mathbf{x}_{k,i}+\mathbf{t}_i)-(\mathbf{S}_j^T\mathbf{x}_{k,j}+\mathbf{t}_j)\right\|_2^2. \label{eq:A_0}
\end{align}
An equivalent problem is to find $(\mathbf{x}_k)_1^n$, $(\mathbf{S}_i)_{1}^{m}$ and $(\mathbf{t}_i)_{1}^{m}$ that minimize
\begin{align}
    \mathcal{A}_1 \coloneqq \min_{\substack{\substack{(\mathbf{S}_i)_1^m \subseteq \mathbb{O}(d)\\ (\mathbf{t}_i)_1^m \subseteq \mathbb{R}^d} \\ (\mathbf{x}_k)_1^n \subseteq \mathbb{R}^d}}\sum_{\substack{(k,i)\in E(\Gamma)}}\left\|\mathbf{x}_k-(\mathbf{S}_i^T\mathbf{x}_{k,i}+\mathbf{t}_i)\right\|_2^2. \label{eq:A_1}
\end{align}
Here, $\mathcal{A}_0$ and $\mathcal{A}_1$ are the optimal alignment errors. The proof of the following lemma derives that the optimal $\mathbf{x}_k$ in Eq.~(\ref{eq:A_1}) from which it follows that the minimizers $(\mathbf{S}_i)_{1}^{m}$ and $(\mathbf{t}_i)_{1}^{m}$ of the two problems coincide.
...
Note that translating the optimal $\mathbf{x}_k$ and $\mathbf{t}_i$ by the same amount does not change the objective and still leads to an optimal solution. To prevent that, we add a centering constraint $\sum_1^n \mathbf{x}_k = 0$ so that the mean of $\mathbf{x}_k$ is at the origin. A more concise representation of $\mathcal{A}_1$ with the centering constraint is as follows. Define
\begin{align}
...
\end{align}
Define $\mathbf{e}_{ki} \coloneqq \mathbf{e}^{n+m}_k - \mathbf{e}^{n+m}_{n+i}$, then $\mathbf{x}_k - \mathbf{t}_i = \mathbf{H}\mathbf{e}_{ki}$. Let $\mathbf{S} = [\mathbf{S}_i]_1^m \in \mathbb{O}(d)^m \subseteq \mathbb{R}^{md \times d}$. Then
\begin{align}
    \mathcal{A}_1 &= \min_{\substack{\mathbf{S} \in \mathbb{O}(d)^m\\\mathbf{H} \in \mathbb{R}^{d \times (n+m)}\\\mathbf{H}\mathbf{1}^{n+m}_n = 0}}\sum_{(k,i) \in E(\Gamma)}\left\|\mathbf{H}\mathbf{e}_{ki}-\mathbf{S}^T(\mathbf{e}^m_i \otimes \mathbf{I}_d)\mathbf{x}_{k,i}\right\|_2^2\\
    &= \min_{\substack{\mathbf{S} \in \mathbb{O}(d)^m\\\mathbf{H} \in \mathbb{R}^{d \times (n+m)}\\\mathbf{H}\mathbf{1}^{n+m}_n = 0}}\text{Tr}\left(\begin{bmatrix}\mathbf{H} & \mathbf{S}^T\end{bmatrix}\begin{bmatrix}\boldsymbol{\mathcal{L}}_{\Gamma} & -\mathbf{B}^T\\-\mathbf{B}^T & \mathbf{D}\end{bmatrix}\begin{bmatrix}\mathbf{H}^T\\\mathbf{S}\end{bmatrix}\right) \label{eq:A_1_}
\end{align}
where 
\begin{align}
    \boldsymbol{\mathcal{L}}_{\Gamma} &= \sum_{(k,i) \in E(\Gamma)}\mathbf{e}_{ki}\mathbf{e}_{ki}^T\label{eq:L_Gamma}\\
    \mathbf{B} &= \sum_{(k,i) \in E(\Gamma)} (\mathbf{e}^m_i \otimes \mathbf{I}_d)\mathbf{x}_{k,i}\mathbf{e}_{ki}^T \label{eq:B}\\
    \mathbf{D} &= \sum_{(k,i) \in E(\Gamma)} \mathbf{x}_{k,i}\mathbf{x}_{k,i}^T(\mathbf{e}^m_i \otimes \mathbf{I}_d)^T. \label{eq:D}
\end{align}
\begin{rmk}
\label{rmk:L0DB}
Note that $\boldsymbol{\mathcal{L}}_{\Gamma}$ is the combinatorial Laplacian of the graph $\Gamma$. Due to the bipartite structure of $\Gamma$, $\boldsymbol{\mathcal{L}}_{\Gamma}^\dagger$ can be computed in $O(nm^2)$ \citea{HO2005917}, against the complexity of $O((m+n)^3)$ for the general case.  The matrix $\mathbf{D}$ is a block diagonal matrix where the $i$th block is $\sum_{(k,i) \in E(\Gamma)}\mathbf{x}_{k,i}\mathbf{x}_{k,i}^T$. The matrix $\mathbf{B} = [\mathbf{B}_i]_1^m$ is a vertical stack of $m$ matrices, one for each view, where each $\mathbf{B}_i \in \mathbb{R}^{d \times (n+m)}$. For a fixed $i$, $\mathbf{B}_i(:,n+i) = -\sum_{(k,i)\in E}\mathbf{x}_{k,i}$, if $(k,i) \in E(\Gamma)$ then $\mathbf{B}_i(:,k) = \mathbf{x}_{k,i}$ otherwise zero. Thus, $\mathbf{1}_{n+m} \in \text{ker}(\mathbf{B}_i)$ and $\mathbf{B}\mathbf{1}_{n+m} = 0$. Finally, the $j$th row of $\mathbf{B}_i$ contains information about the $j$th coordinates of the points in the $i$th view.
...
\begin{align}
    \mathbf{H}^* = \mathbf{S}^T\mathbf{B}\boldsymbol{\mathcal{L}}_{\Gamma}^\dagger \left(\mathbf{I}_{n+m} - \frac{\mathbf{1}^{n+m}_n\mathbf{1}_{n+m}^T}{n}\right). \label{eq:opt_Z}
\end{align}
...
\begin{align}
    \mathcal{A}_1 = \min_{\mathbf{S} \in \mathbb{O}(d)^m} F(\mathbf{S}) = \min_{\mathbf{S} \in \mathbb{O}(d)^m} \text{Tr}(\mathbf{C}\mathbf{S}\mathbf{S}^T) \text{ where } \mathbf{C} = \mathbf{D} - \mathbf{B}\boldsymbol{\mathcal{L}}_{\Gamma}^{\dagger}\mathbf{B}^T. \label{eq:GPOP}
\end{align}
Here $\mathbf{C}$ is named the \textit{patch-stress matrix} in \citea{chaudhury2015global} and clearly $\mathbf{C} \succeq 0$.
\begin{dfn}
...
Since $\mathbf{C} \succeq 0$, every perfect alignment is trivially optimal, while the converse may not be true.

\begin{dfn}
\label{def:realization}
The consensus representation of the framework $\Theta$ due to an alignment $\mathbf{S}$ is given by
\begin{align}
    \Theta(\mathbf{S}) \coloneqq \mathbf{H}^*(:,1:n) = \mathbf{S}^T\mathbf{B}\boldsymbol{\mathcal{L}}_{\Gamma}^\dagger(:,1:n)\left(\mathbf{I}_n - n^{-1}\mathbf{1}_{n}\mathbf{1}_{n}^T\right). \label{eq:theta_S}
\end{align}
The representation $\Theta(\mathbf{S})$ due to a perfect alignment $\mathbf{S}$ is called a realization.
\end{dfn}