...
\icmltitlerunning{Matryoshka Policy Gradient}

\begin{document}

\twocolumn[
\icmltitle{Matryoshka Policy Gradient for Entropy-Regularized RL: Convergence and Global Optimality}
...
\icmlauthor{Fran\c{c}ois Ged}{yyy}
\icmlauthor{Maria Han Veiga}{comp}
...
\icmlaffiliation{yyy}{Chair of Statistical Field Theory, \'{E}cole Polytechnique F\'{e}d\'{e}rale de Lausanne, Lausanne, Switzerland}
\icmlaffiliation{comp}{Department of Mathematics and Michigan Institute for Data Science, University of Michigan, Ann Arbor, USA}
\icmlaffiliation{sch}{School of ZZZ, Institute of WWW, Location, Country}

\icmlcorrespondingauthor{Fran\c{c}ois Ged}{francois.ged@epfl.ch}
\icmlcorrespondingauthor{Firstname2 Lastname2}{first2.last2@www.uk}

% You may provide any keywords that you
% find helpful for describing your paper; these are used to populate
% the "keywords" metadata in the PDF but will not be shown in the document
\icmlkeywords{Reinforcement Learning, Policy Gradient, Global Convergence}
...
\begin{align}\label{eq: gen case recursive definition value function}
    V^{(n)}_{\pi}(s)
    = \EE_{\pi^{(n)}}[R_0] - &\tau\DKL{\pi^{(n)}}{\overline{\pi}}(s)\nonumber\\
    &\hspace{0.3cm}+\EE_{\pi^{(n)}}[V^{(n-1)}_{\pi'}(S_1)],
\end{align}
...
\begin{align*}
    J(\pi):= &\int_{\mathcal{S}}\EE_{\pi_t}\bigg[\sum_{k=0}^T\gamma^k\left(R_k-\DKL{\pi_t}{\overline{\pi}}(S_k)\right)\\
    &\hspace{4cm}\Big|S_0=s\bigg]\nu_0(\mathrm{d}s),
\end{align*}
...
\begin{align}\label{eq: gradient policy pi}
    ...
    &\hspace{0.1cm}=\pi_{\theta^{(i)}}^{(i)}(a|s)\sum_{a'\in\mathcal{A}}\left(\delta_{a,a'}-\pi_{\theta^{(i)}}^{(i)}(a'|s)\right)\nabla h^{(i)}_{\theta^{(i)}}(a',s)/\tau.
\end{align}
...
\begin{thm}\label{thm: general case global optimality}
    ...
    \begin{align*}
        &d(a,s):=\mathbf{m}_{\pi_\infty}^{(m)}(s)\pi^{(m)}_\infty(a|s)\\
        &\hspace{1.7cm}\times\left(\log\frac{\pi^{(m)}_\infty}{\pi^{(m)}_*}(a|s)-\DKL{\pi^{(m)}_\infty}{\pi^{(m)}_*}(s)\right).
    \end{align*}
...
\begin{lem}\label{lem: bandit case expected update log pi_t}
    ...
    \begin{align*}
        ...
        &\hspace{0.7cm}= - \learningrate \tau\sum_{a'\in\mathcal{A}}\pi_t(a')\left(\log\frac{\pi_t}{\pi_*}(a')-\DKL{\pi_t}{\pi_*}\right)\\
        &\hspace{1.5cm}\times\Big(\Theta(a,a')-\EE_{\pi_t}[\Theta(A,a')]\Big)
...
\begin{align*}
	...
	&=\frac{ \learningrate }{\tau^2} \EE_{\pi_t}\bigg[B_t\sum_{a',a''\in\mathcal{A}}(\delta_{A_t,a'}-\pi_t(a'))(\delta_{a,a''}-\pi_t(a''))\\
	&\hspace{4.5cm}\times\nabla_\theta h_t(a')\cdot\nabla_\theta h_t(a'')\bigg]\nonumber\\
	&=\frac{ \learningrate }{\tau^2} \EE_{\pi_t}\bigg[B_t\sum_{a',a''\in\mathcal{A}}(\delta_{A_t,a'}-\pi_t(a'))(\delta_{a,a''}-\pi_t(a''))\\
	&\hspace{6.2cm}\times\Theta(a',a'')\bigg]\nonumber\\
	&=\frac{ \learningrate }{\tau^2}\EE_{\pi_t}\bigg[B_t\left(\Theta(A_t,a)+\EE_{\pi_t\otimes\pi_t}[\Theta(A,A')]\\
	&\hspace{3cm}-\EE_{\pi_t}[\Theta(A,a)]-\EE_{\pi_t}[\Theta(A_t,A)]\right)\bigg],
\end{align*}
...
Using that $\EE[X(Y-\EE[Y])]=\EE[(X-\EE[X])Y]$ for any two integrable real random variables, we then get that
    \begin{align*}
        &\log\pi_{t+1}(a)-\log\pi_t(a)\\
        &\hspace{0.2cm}=\frac{ \learningrate }{\tau^2}\EE_{\pi_t}\Big[\left(B_t-\EE_{\pi_t}[B_t]\right)\left(\Theta(A_t,a)-\mathbb{E}_{\pi_t}[\Theta(A,a)]\right)\Big]\\
        &\hspace{6cm}+ o\left(\learningrate C(\theta_t)\right).
    \end{align*}
    To conclude the proof, it suffices to note that by Equation \eqref{eq: bandit case optimal policy},
    \begin{align*}
        &B_t-\EE_{\pi_t}[B_t]\\
        &\hspace{0.2cm}= R_t-\tau\log\frac{\pi_t}{\overline{\pi}}(A_t)-\EE_{\pi_t}[r(A_t)]+\DKL{\pi_t}{\overline{\pi}}\\
        &\hspace{0.2cm}=\log\frac{\pi_*}{\overline{\pi}}(A_t)-\tau\log\frac{\pi_t}{\overline{\pi}}(A_t)-\EE_{\pi_t}\left[\tau\log\frac{\pi_*}{\overline{\pi}}(A)\right]\\
        &\hspace{5.5cm}+\DKL{\pi_t}{\overline{\pi}}\\
        &\hspace{0.2cm}=-\tau\log\frac{\pi_t}{\pi_*}(A_t)+\tau\DKL{\pi_t}{\pi_*},
    \end{align*}
...
\begin{align*}
    ...
    &\hspace{0.6cm}\times\Big(\EE_p[\Theta(A,a')]-\EE_{\pi_t}[\Theta(A,a')]\Big)+o\left(\learningrate C(\theta_t)\right).
\end{align*}
...
\begin{align*}
    ...
    &\hspace{2cm}\times\left(\log\frac{\pi_t}{\pi_*}(a')-\DKL{\pi_t}{\pi_*}\right)e_i(a')\\
...
\begin{proof}[\textbf{Proof of Theorem \ref{thm: policy gradient theorem}}]
    ...
    \begin{align*}
        ...
        &\hspace{0.5cm}+\EE_{S_{n-i}\sim\mathbf{m}_{\pi_t}^{(i)}}\bigg[\EE_{\pi_t}\bigg[\sum_{\ell=n-i}^{n}R_\ell-\tau\log\frac{\pi_t^{(n-\ell)}}{\overline{\pi}}(A_{\ell}|S_\ell)\\
        &\hspace{6.5cm}\Big|S_{n-i}\bigg]\bigg]\bigg)\mathrm{d}s\\
...
\begin{proof}[\textbf{Proof of Lemma \ref{lem: general case V_* = log expectation}}]
    ...
    \begin{align*}
        ...
        &\hspace{2cm}\times\sum_{a\in\mathcal{A}}\frac{\overline{\pi}(a|s)\exp\left(Q_*^{(n)}(a,s)/\tau\right)}{\EE_{\overline{\pi}}\left[\exp\left(Q_*^{(n)}(A,s)/\tau\right)\right]}\\
        &=\tau\log\EE_{\overline{\pi}}\left[\exp\left(Q_*^{(n)}(A,s)/\tau\right)\right],
    \end{align*}
...
\begin{lem}\label{lem: general case value function is optimal value function minus DKL}
    ...
    \begin{align*}
        &V_{\pi}^{(n)}(s)
        -V_*^{(n)}(s)\\
        &\hspace{0.5cm}=-\EE_{\pi}\left[\left.\sum_{i=0}^{n-1}\DKL{\pi^{(n-i)}}{\pi_*^{(n-i)}}(S_i)\right|S_0=s\right].
    \end{align*}
\end{lem}
...
\begin{proof}
    ...
    \begin{align*}
        V_{\pi}^{(n)}(s)
        &=\sum_{a\in\mathcal{A}}\pi^{(n)}(a|s)\bigg(r(a,s) - \tau\log\frac{\pi^{(n)}}{\overline{\pi}}(a|s)\\
        &\hspace{2.8cm}+ \sum_{s'\in\mathcal{S}}p(s,a,s')V^{(n-1)}_{\pi}(s')\bigg)\\
        &=\sum_{a\in\mathcal{A}}\pi^{(n)}(a|s)\bigg(V_*^{(n)}(s) - \tau\log\frac{\pi^{(n)}}{\pi_*}(a|s)\\
        &\hspace{0.3cm}+ \sum_{s'\in\mathcal{S}}p(s,a,s')(V^{(n-1)}_{\pi}(s')-V_*^{(n-1)}(s'))\bigg),
    \end{align*}
    ...
    \begin{align*}
        &V_{\pi}^{(n)}(s)-V_*^{(n)}(s)
        =-\DKL{\pi^{(n)}}{\pi_*^{(n)}}\\
        &\hspace{2cm}+\EE\left[V^{(n-1)}_{\pi}(S_1)-V_*^{(n-1)}(S_1)\Big|S_0=s\right].
    \end{align*}
...
\begin{lem}\label{lem: general case expected update log pi_t}
    ...
    \begin{align*}
        ...
        &\hspace{0.1cm}= - \learningrate \tau\int_{\mathcal{S}}\mathbf{m}_{\pi_t}^{(m)}(\mathrm{d}s')\sum_{a'\in\mathcal{A}}\pi^{(m)}_t(a'|s')\\
        &\hspace{0.3cm}\times\left(\log\frac{\pi^{(m)}_t}{\pi^{(m)}_*}(a'|s')-\DKL{\pi^{(m)}_t}{\pi^{(m)}_*}(s')\right)\\
        &\hspace{0.3cm}\times\left(\Theta^{(m)}((a,s),(a',s'))-\EE_{\pi^{(m)}_t}[\Theta^{(m)}((A,s),(a',s'))]\right)\\
        &\hspace{6cm}+o\left(\learningrate C(\theta_t)\right),
    \end{align*}
...
\begin{proof}
    ...
    \begin{align}\label{eq: gen case log increment to compute}
        ...
        &\hspace{1.5cm}= (\theta_{t+1}^{(m)}-\theta_t^{(m)})\cdot\frac{\nabla_\theta \pi_t^{(m)}(a|s)}{\pi_t^{(m)}(a|s)}+o\left(\learningrate C(\theta_t)\right)\nonumber\\
        &\hspace{1.5cm}=\frac{\learningrate}{\tau^2}\EE_{\pi_t}\bigg[C_m\sum_{a',a''\in\mathcal{A}}\left(\delta_{a,a'}-\pi_t^{(m)}(a'|s)\right)\nonumber\\
        &\hspace{3cm}\times\left(\delta_{A_{n-m},a''}-\pi_t^{(m)}(a'|S_{n-m})\right)\nonumber\\
        &\hspace{3cm}\times\Theta^{(m)}((a',s),(a'',S_{n-m}))\bigg]\nonumber\\
        &\hspace{5cm}+o\left(\learningrate C(\theta_t)\right).
    \end{align}
    We focus on the expectation.
    It is equal to
    \begin{align*}
        &\EE_{\pi_t}\bigg[C_m\bigg(\Theta^{(m)}((a,s),(A_{n-m},S_{n-m}))\\
        &\hspace{1.5cm}-\EE_A\left[\Theta^{(m)}((A,s),(A_{n-m},S_{n-m}))\right]\\
        &\hspace{1.5cm}-\EE_A\left[\Theta^{(m)}((a,s),(A',S_{n-m}))\right]\\
        &\hspace{1.5cm}+\EE_{A,A'}\left[\Theta^{(m)}((A,s),(A',S_{n-m}))\right]\bigg)\bigg],
    \end{align*}
    ...
    \begin{align}\label{eq: gen case after expectation trick}
        ...
        &\hspace{1cm}-\EE_A\left[\Theta^{(m)}((A,s),(A_{n-m},S_{n-m}))\right]\Big)\Big].
    \end{align}
    We write
    \begin{align*}
        &\EE\left[C_m|S_{n-m}\right]\\
        &\hspace{0.2cm}=\EE\left[\sum_{\ell=n-m}^{n}\left(R_{\ell}-\tau\log\frac{\pi_t^{(n-\ell)}}{\overline{\pi}}(A_{\ell}|S_{\ell})\right)\bigg|S_{n-m}\right]\\
        &\hspace{0.2cm}=V_{\pi_t}^{(m)}(S_{n-m})\\
        &\hspace{0.2cm}=V_*^{(m)}(S_{n-m})-\DKL{\pi_t^{(m)}}{\pi_*^{(m)}}(S_{n-m}),
    \end{align*}
    ...
    \begin{align*}
        ...
        &\hspace{0.2cm}=R_{n-m}-\tau\log\frac{\pi_t^{(m)}}{\overline{\pi}}(A_{n-m}|S_{n-m})\\
        &\hspace{2.5cm}+\EE\left[V_{\pi_t}^{(m-1)}(S_{n-m})\Big|S_{n-m},A_{n-m}\right]\\
        &\hspace{0.2cm}= \EE\Big[V_{\pi_t}^{(m-1)}(S_{n-m+1})-V_*^{(m-1)}(S_{n-m+1})\\
        &\hspace{5cm}\Big|S_{n-m},A_{n-m}\Big]\\
        &\hspace{1.5cm}-\tau\log\frac{\pi_t^{(m)}}{\pi_*^{(m)}}(A_{n-m}|S_{n-m})+V_*^{(m)}(S_{n-m})\\
        &\hspace{0.2cm}= -\tau\log\frac{\pi_t^{(m)}}{\pi_*^{(m)}}(A_{n-m}|S_{n-m})+V_*^{(m)}(S_{n-m}).
    \end{align*}
    ...
    \begin{align*}
        ...
        &\hspace{4cm}-\log\frac{\pi_t^{(m)}}{\pi_*^{(m)}}(A_{n-m}|S_{n-m})\bigg)\\
        &\hspace{0.2cm}\times\Big(\Theta^{(m)}((a,s),(A_{n-m},S_{n-m}))\\
        &\hspace{2cm}-\EE_A\left[\Theta^{(m)}((A,s),(A_{n-m},S_{n-m}))\right]\Big)\bigg],
    \end{align*}
...
\begin{align*}
    ...
    &\hspace{1cm}-\EE_{\pi^{(m)}_t}[\Theta^{(m)}((A,s),(a',s'))]\bigg)+o\left(\learningrate C(\theta_t)\right).
\end{align*}
...
\begin{align}\label{eq: gen case lower bound log increment with d}
    ...
    &\hspace{1cm}-\EE_{\pi^{(m)}_t}[\Theta^{(m)}((A,s),(a',s'))]\bigg)=0.
\end{align}
Define
\begin{align*}
    f:
...
\begin{align*}
    ...
    &\hspace{3cm}\times\int_{\mathcal{S}}\mathrm{d}s'\sum_{a'\in\mathcal{A}}d_{a',s'} e_j^{(m)}(a',s')\\
...
\begin{align}\label{eq: gen case irreducibility}
    ...
    &\hspace{2.5cm}\times\prod_{m=0}^{k-1}p(s_m,a_m,s_{m+1})>0,
\end{align}
