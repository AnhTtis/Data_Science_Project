...
\subsection{i.i.d.\ sets are dense independent}
...
By the corollary above almost surely $V:=\{x_n : n\in \NN \}$ is an independent dense set. 
...
\end{align*}
For the last triangle inequality, note that if $d(y,y_2)\geq d(y,y_1)$ then it is obviously true. If not, then $k=n$ and we may switch $y_1$ and $y_2$ and the same proof goes through.
\item
...
By the property of Urysohn space, there is $y\in \mathfrak{U}$ such that $d(y,y')$ has the desired value for all $y'\in Y$. In particular $n-1<d(y,y')<n$ for all $y'\in D_n$. Now we want to find $y_0\in U_2$ that also satisfy the same property. Note that $n-1+\epsilon<d(y,y')<n-\epsilon$ for $y'\in D_n$. By density, there is a point $y_0\in U_2$ such that $d(y_0,y)<\frac{1}{2}\epsilon$. Then $d(y_0,y')<d(y,y')+d(y,y_0)<n-\frac{1}{2}\epsilon$ and $d(y_0,y')>d(y,y')-d(y,y_0)>n-1+\frac{1}{2}\epsilon$ for all $y'\in D_n$. Now we may extend $f$ by sending $x_0$ to $y_0$ and we are done.
\end{proof}
...
\end{coro}
\begin{proof}
We build a graph isomorphism by building a back-and-forth system that preserves both the edge relation and $\{C_n:n\in\mathbb{N}^{>0}\}$. Suppose $f:X\to Y$ is such a finite map and $x_0\in U_1$. We want to extend $f$ to $X\cup\{x_0\}$. We will modify proof of \cref{thm:Ury}. We know there is $\epsilon>0$ and $y'\in \mathcal{U}$ such that for all $y_0\in U_1\cap B_{\epsilon/2}(y')$, for all $n>0$, for all $y\in Y$ with $n-1<d(x_0,f^{-1}(y))<n$, we have $n-1<d(y_0,y)<n$. Let $Q:=\{y\in Y:x_0\mathrel{E_1}f^{-1}(y)\}$. Note that $Q\subseteq B_1(y')$ as $f^{-1}(Q)\subseteq B_1(x_0)$ and $B_1$ is interdefinable with $C_1$. By the g.e.c.\ property of $G_2$, there is $y_0\in U_1\cap B_{\epsilon/2}(y')$ such that $y_0\mathrel{E_2}y$ iff $y\in Q$ and we are done.
\end{proof}
