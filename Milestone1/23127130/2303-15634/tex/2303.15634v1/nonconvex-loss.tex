...
there is a line of work showing that SGD can efficiently escape saddle points if the step size is large enough~\cite{lee2016gradient,jin2017escape}.
...
\E[\Reg(T)]
    \le 
    \E\left[\frac{2\ellbar_1(\theta_1)}{a_1^2}\right]
    + \sum_{t=2}^T \E\left[2\ellbar_t(\theta_t) \left(\frac{1}{a_t} - \frac{1}{a_{t-1}^2}\right)\right] 
