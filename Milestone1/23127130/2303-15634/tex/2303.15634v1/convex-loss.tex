...
\E[\Reg(T)] &\le
 ...
 \hspace{-0.10cm}    + \frac{\|\theta^*_t - \theta^*_{t+1}\|^2}{a_t} + \frac{2}{a_t}\<\theta^*_t - \theta^*_{t+1}, \theta_{t+1}-\theta^*_t\>\Bigg]\,.
\end{align}
Here, the expectation is with respect to the randomness in data points observed in the $T$ steps.
\end{thm}
We next discuss how the regret bound~\eqref{eq:Reg-UB0} can be used to derive optimal learning rate schedules. We would like to derive optimal rates $\eta^*_t$ be minimizing the bound~\eqref{eq:Reg-UB0} in a sequential manner. However, the bound depends on $D_t$ and $\theta_{t+1}$,
...
\begin{align}\label{eq:Reg-UB}
    \E[\Reg(T)] &\le
    D_{\max}^2 \left( \E\left[\frac{1}{a_1}\right] +
    \sum_{t=2}^T \E\left[ \left(\frac{1}{a_t} - \frac{1}{a_{t-1}}\right)_+\right] \right)
...
Quantities $b_{1,t}$ and $b_{2,t}$ in~\eqref{eq:tau1} and \eqref{eq:tau2} are increasing in distribution shift $\gamma_t$ and it is easy to see that the thresholds $\tau_{1,t},\tau_{2,t}$ are so increasing in $\gamma_t$. As a result for every value of $\eta_{t-1}$, higher distribution shift $\gamma_t$ increases the optimal learning rate $\eta^*_t$.
\end{remark}
