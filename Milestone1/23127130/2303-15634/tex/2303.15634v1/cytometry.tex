...
% Since this setting uses non-convex loss, we do not have a tight (upper and lower bound) characterization of the shape of the optimal learning rate schedule to fall back on.
% It is also a more involved exercise to show that our experimental data satisfies all our  assumptions.
% Moreover, for the cytometry application below, a good learning rate schedule can be found by simply trying different learning rate schedules on different (but similar) sets of data samples, and seeing which one works the best.
% We also know the points of large distribution shifts for our examples in advance, so we can adjust the schedule to have larger learning rates at those points. Such a learning rate schedule should be close to optimal and it is worthwhile to compare the change in regret with schedules having a constant learning rate for a dataset arising from a specific instance.

% First we provide some background on flow cytometry.
% It is a medical application that can benefit from optimized learning rate schedules in the presence of distribution shift.
% Flow cytometry consists of analyzing a input stream of cells---classifying and sorting them---as they arrive, based on the RNA expression of each cell
% (see, e.g.,~\citet{LMC19} and the references therein for details).
...
\Cref{fig:cytometry_visualization} is a two-dimensional embedding of these ten
...
\begin{figure*}
    ...
    \raisebox{0.3cm}{
        \includegraphics[width=\textwidth]{figures/velocity_graph_v1.png}
    }
    \caption{}\label{fig:cytometry_visualization}
    \end{subfigure}
    \hspace{0.2cm}
    \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{figures/n100_b64_ce_regret_train.png}
    \caption{}\label{fig:cytometry_regret}
    \end{subfigure}
    \caption{
...
\begin{itemize}
    ...
    We plot the cumulative regret in \Cref{fig:cytometry_regret},
...
%     \end{subfigure}
%     \caption{Empirical regrets for different learning rate choices in a simulated flow cytometry setting. A flow cytometer classifies and sorts a stream of cells fed based on RNA expressions measured via fluorescence levels. We simulate such a stream cells using data from~\citet{Bergen20}. The left figure shows that when the distribution of the cells in the input stream changes over time, higher learning rate constants can be beneficial. Moreover, if we fix the average learning rate, then a schedule that uses higher learning rates only when the input distribution changes can outperform an equivalent constant learning rate schedule. The right figure compares the effect of batch size for the adjusted learning rate schedule. Large batch sizes can help smooth gradients but can also be counter-productive as the neural net sees fewer batches between successive distribution shifts.}
%     \label{fig:flow-cyt-summary}
% \end{figure*}

We can draw several conclusions from the results of this experiment.
...
cumulative regret, as these models can more quickly adjust to the new data distributions.
Finally, the models get stuck in local minima without
adaptive and increasing learning rate schedules, as evident by the
$\eta_0 = 0.03$ plots in \Cref{fig:cytometry_regret}, which have different
slopes in the final two phases.
