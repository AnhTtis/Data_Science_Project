...
\author{Lijun Sheng$^{1,2}$, Jian Liang$^{2,3}$, Ran He$^{2,3}$, Zilei Wang$^{1}$, Tieniu Tan$^{2,4}$ \\
...
Since the data distribution during deployment often differs from the training set, unsupervised domain adaptation \cite{ben2010theory, ganin2016domain, long2018conditional, liang2021domain} is widely studied to improve the model performance across different domains and save the cost of extra annotations.
Growing attention is devoted to a new situation, model adaptation \cite{liang2020we, li2020model}, which restricts algorithms to only access the unlabeled target data and the pretrained model from the source domain.
This new paradigm \cite{yang2021exploiting, liang2021source, kurmi2021domain, ahmed2021unsupervised, wang2020tent, ding2022proxymix, qiu2021source} quickly becomes popular among the transfer learning field thanks to the appealing privacy-preserving property and the competitive performance to source-data-dependent methods.
Nowadays, the model adaptation scheme has been widely applied to various tasks, \eg, image classification \cite{liang2020we, yang2021exploiting, liang2021source, huang2021model}, semantic segmentation \cite{liu2021source, kundu2021generalize, you2021domain}, and object detection \cite{xiong2021source, li2022source}.
...
In fact, providers would use inherent properties of models and the transferability of loopholes to attack client's models which have been re-trained by adaptation algorithms.
...
We empirically test the transferability of the selected universal attacks \cite{poursaeed2018generative, barni2019new} from the source domain towards existing model adaptation methods \cite{liang2020we, ding2022proxymix, yang2021exploiting, yang2022local}, and results are shown in Fig.~\ref{fig:intro}.
It is obvious that the adaptation performance degrade significantly, indicating the vulnerability to these perturbations embedded by the source provider still survive in model adaptation methods.
...
In particular, we conjecture the source model parameters to be risky and avoid using them as initialization directly.
...
Further, we develop a radius-adjusting strategy which constrains adversarial examples to gradually become stronger, mitigating the negative impact on student network training at an early stage.
...
Model adaptation \cite{liang2020we, huang2021model}, also known as source-free unsupervised domain adaptation \cite{yang2021exploiting, qiu2021source, qiu2021source, ding2022proxymix, yang2022local}, aims to transfer knowledge from the well-trained source model to the unlabeled target domain. 
...
Some methods \cite{mopuri2018nag, poursaeed2018generative} use the generative ability of GANs \cite{goodfellow2020generative} to avoid directly optimizing the perturbation.
...
Corresponding to attack methods, various defense strategies \cite{moosavi2017universal, mummadi2019defending, shafahi2020universal} against UAP have also been explored.
...
Backdoor attacks \cite{gu2017badnets, wu2022backdoorbench, li2022backdoor} poison the training dataset with a well-designed trigger to embed a backdoor into the model.
...
We focus on backdoor defense in the post-training stage \cite{wu2022backdoorbench, wu2021adversarial, wang2019neural} which mitigates the impact of the backdoor on a well-trained model.
...
These defense methods always require the part of clean training data, so they can not perform well in the model adaptation scenarios.
...
\begin{figure}[t]
		...
		\begin{tabular}{ccc}
            ...
            \\[1mm]
            (a) Perturbation. & (b) Clean image. & (c) UAP 
            ...
            \\[1mm]
		\end{tabular}
		\caption{Visualization of perturbations and examples of an artistic image from \textbf{OfficeHome} \cite{venkateswara2017deep} after being under four attacks. The red border is only used to distinguish the attack image here.}
		\label{fig:attack}
		\vspace{-10pt}
\end{figure} 
...
Different from image-specific attacks, universal attacks \textit{need no queries to the victim model per image}, which are more convenient to calculate and implement.
...
Due to UAP has great stealthiness and not changing the training process, it is a strong attack on existing model adaptation methods.
UAP is a powerful attack on current model adaption techniques thanks to its great stealthiness and natural generation process.


\begin{figure*}[t]
    \centering
    \includegraphics[width=0.7\linewidth]{Framework_AdaptGuard.pdf}
...
In the domain adaptation task, model providers randomly select a portion of the source data to poison and specify a targeted category label $y^b$.
Blended \cite{chen2017targeted} uses a fixed image as a trigger whose size is the same as the training data $x_i^{s}$.
...
The providers replace the selected set with its poison version $\{(x_i^{b,s}, y^b)\}_{i=1}^{N_b}$ and train the backdoor source model $f_s^b$ which is finally embedded with the backdoor.
...
Even if users re-train the victim model, the backdoor can still exist and result in a successful attack sometimes.

\section{Defending Against Universal Attacks}
As common image-agnostic attack methods, universal adversarial perturbations and backdoor attacks have an effect on the target model after model adaptation.
...
\begin{equation}
	...
	\label{eq:method_kd}
\end{equation}
where $\mathcal{KL}(\cdot\|\cdot)$ denotes the Kullback–Leibler divergence of two probability distributions and $\sigma$ is the softmax operation.
...
For the sake of safety, it is recommended to employ AdaptGuard to extract effective information within the source model and discard its risky parameters.
...
\textbf{DomainNet126} \cite{saito2019semi}, a subset version of DomainNet \cite{peng2019moment}, is a large-size benchmark which consists of 126 categories and 4 domains (\ie, Clipart (C), Painting (P), Real (R), and Sketch(S)).
Different from the above datasets, it has the separate test set, and we only choose four tasks in a loop (\ie, CP, PR, RS, and SC) due to the large number of experiments.
...
Considering backdoor defense, we use ANP \cite{wu2021adversarial} to cut the sensitive BatchNorm layers from the source model and then employ model adaptation algorithms.
As ANP fails under unlabeled data, we use half of the clean source dataset in its optimize process, which is not allowed in our framework.
...
These misclassified samples make ASR value better but contribute nothing to adaptation task, so it is necessary to consider more on accuracy metric.
...
We use a PGD-7 \cite{madry2017towards} with $L_{inf}$ norm 4/255 from the source domain to generate adversarial sample for each test instance, and report the attack accuracy of the target model in Table~\ref{tab: pgd-attack}.
From the results, the performance of model adaptation algorithms drops a lot under image-specific attack from the source side, and AdaptGuard can also provide a strong defense in various benchmarks.
This implies that our framework has an attractive flexibility.
...
\section{Conclusion}
In this paper, we study the vulnerability of model adaptation methods towards universal attacks across domains. 
...
And we plan to investigate various model adaptation tasks (e.g., segmentation) under more complex image-specific attacks (e.g., transfer-based attack) in the future work to extend our framework in broader application scenarios.
...
\setlength{\tabcolsep}{3.0pt}
    ...
    \caption{Accuracies (\%) of defense methods against \textbf{Blended} \cite{chen2017targeted} backdoor attack on \textbf{OfficeHome} \cite{venkateswara2017deep} dataset for model adaptation (ResNet-50).}
...
\setlength{\tabcolsep}{3.0pt}
    ...
    \caption{Accuracies (\%) of defense methods against \textbf{SIG} \cite{barni2019new} backdoor attack on \textbf{OfficeHome} \cite{venkateswara2017deep} dataset for model adaptation (ResNet-50).}
