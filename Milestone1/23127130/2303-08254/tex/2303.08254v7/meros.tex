...
\label{sec:intro}	
	...
	Divers robotics frameworks have been developed so far \cite{inigo2012robotics,tsardoulias2017robotic,hentout2016survey}. Some steps towards standardisation have been made in recent years, and ROS (Robot Operating System) has come to the fore. Stand-alone ROS~1 (ROS version~1)~\cite{quigley2009ros} is unsuitable for hard RT (Real Time) systems, so one of the solutions in practical applications (e.g., \cite{lages2014architecture,buys2011haptic,pages2016tiago,Seredynski-fabric-romoco-2019,kornuta-bpan-2020,cholewinski2015software}), is to integrate ROS~1 with Orocos \cite{bruyninckx2001open,bruyninckx2002orocos}. Over time, ROS~1 has evolved to, among other things, improve its performance. Known and crucial problems in the face of some contemporary applications (e.g. cybersecurity, RT performance) led to the development of a~new version of the framework, ROS~2 \cite{maruyama2016exploring,park2020real}.   

	According to analyses of ROS metrics from July 2022\footnote{\url{https://discourse.ros.org/t/2022-ros-metrics-report/29594}}, total interest in ROS was growing, and though the ROS~2 was gaining popularity, ROS~1 was still more popular than ROS~2, even in terms of new package downloads, not to mention existing applications.
	ROS~1 has evolved considerably from the initial distributions. Its metamodels created so far are now incomplete and outdated (sec.~\ref{sec:related-work}). According to ROS metrics, new ROS~1 distributions have practically replaced older ones in terms of distro downloads stats. These indications point to the need to formulate an up-to-date, recent metamodel for the latest versions of ROS~1 and ROS~2, which this article undertakes by presenting the new metamodel for ROS -- MeROS.
 ...
 MeROS is founded on SysML (Systems Modeling Language) \cite{omg-sysml16,Friedenthal:2015}, a profile of UML (Unified Modeling Language). Modelling in languages from the UML family addresses a number of important aspects of systems engineering \cite{chaudron2012effective}. These include the use cases [UCX]:
	\begin{itemize}
		...
		\item $[$UC5] Resuming work after the break,
		\item $[$UC6] Extension and modification of existing systems,
		\item $[$UC7] Support the implementation of new systems,
		\item $[$UC8] Migration of systems, in the case of MeROS -- in particular from ROS~1 to ROS~2.   
	\end{itemize}
...
% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7	
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.71]{img/requirement_pkg/general_req.png}}
		\end{center}
	...
	A~vital addition to the original ROS concepts is the grouping of communicating methods [R2.1] and grouping of communicating components [R2.2]. The motivation for the introduction of these aggregates is presented further on.
...
% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.66]{img/requirement_pkg/original_ros_concepts_req.png}}
		\end{center}
		\caption{ROS concepts requirements.} 
		\label{fig:ros_concepts_req}
	\end{figure}
	
	Communicating components (Fig.~\ref{fig:communicating_components_req}) are ROS Node [R3.1.1], ROS Nodelet [R3.1.2], ROS plugin [R3.1.3], and ROS library [R3.1.4]. Both plugin and library let to share the same code between various Nodes or ROS~1 specific Nodelets. Two ROS node are mandatory to execute the ROS~1 system: ROS Master [R3.1.1.1] and rosout [R3.1.1.2].
...
% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7
	...
	Communication methods are depicted in Fig.~\ref{fig:communication_concepts_req}.
	 Three methods of communication are considered with their inter-component connections and data structures: (i) ROS Topic [R3.2.1] and its Message [R3.2.1.1] and connection [R3.2.1.2], (ii) ROS Service [R3.2.2] comprising data structure [R3.2.2.1] and connection [R3.2.2.2], and finally (iii) ROS Action [R3.2.3] including data structure [R3.2.3.1] and connection [R3.2.3.2].
	...
	Workspace concept (Fig.~\ref{fig:workspace_concepts_req}) comprises ROS Package [R3.3.1] and Metapackage [R3.3.2] introduced in the latest releases of ROS~1.
	...
	Other concepts [R3.4] depicted in Fig.~\ref{fig:other_concepts_req} include four elements.
	...
	\end{figure}
	 ...
	 ROS Parameter Server [R3.4.1] manages ROS Parameters [3.4.2], roscore [R3.4.3] forms a collection of programs and nodes that are pre-requisites of a ROS~1-based system. Finally, ROS Namespace [R3.4.4] reflects the ROS concept to organise nodes and communication connections.
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.7]{img/requirement_pkg/compactness_and_simplicity_req.png}}
		\end{center}
	...
	There are three elements in the requirements set that satisfy the evolution of the ROS~1  finalised with its ultimate release -- Noeitc Ninjemys -- (Fig.~\ref{fig:final_ros1_release_req}): (i) ROS Nodelet [R3.1.2] (introduced primarily to increase the efficiency of ROS components switching), (ii) ROS Action [R3.2.3], and (iii) ROS Metapackage [R3.3.2].
...
\label{sec:metamodel}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=0.67]{img/meros_project_packages_pkg.png}}
		\end{center}
...
\label{sec:metamodel-composition}
	...
	The SysML blocks reflect ROS concepts [R3], and their composition is depicted in bdd (block definition) diagrams. The metamodel is formulated in a~single SysML package. Hence, Workspaces and Running Systems are composed into ROS System (Fig.~\ref{fig:ros_system_bdd}). 
	
		% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.67]{img/meros_pkg/ros_system_bdd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.67]{img/meros_pkg/communicating_components_inherit_bdd.png}}
		\end{center}
	...
	For clarity, relations of Communicating Components are depicted in several diagrams. Fig.~\ref{fig:communicating_component_topics_bdd} considers Topics and their Data Structures.
	 
	% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.8]{img/meros_pkg/communicating_component_topics_bdd.png}}
		\end{center}
		\caption{Communicating Component relations -- topics -- bdd.} 
		\label{fig:communicating_component_topics_bdd}
	\end{figure}
	
	Fig.~\ref{fig:communication_blocks_services_bdd} depicts Services and their Data Structures. 	
	
	% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.8]{img/meros_pkg/communicating_component_services_bdd.png}}
		\end{center}
		\caption{Communicating Component relations -- services -- bdd.} 
		\label{fig:communication_blocks_services_bdd}
	\end{figure}
	
	The Actions are depicted in two diagrams -- Fig.~\ref{fig:communicating_component_actions_bdd} and Fig.~\ref{fig:action_bdd}. It should be noted that in ROS~2, an Action bases on Topics and Services while in ROS~1 only on Topics. From functional point of view, ROS~1 specific Topics included in Action have their equivalents in ROS~2 specific Services. 
	
	% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[width=\columnwidth]{img/meros_pkg/communicating_component_actions_bdd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.7]{img/meros_pkg/action_bdd.png}}
		\end{center}
		\caption{Action -- bdd.} 
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.65]{img/meros_pkg/communicating_component_other_bdd.png}}
		\end{center}
	...
	Besides standard ROS communication methods, the Non-ROS are also included (e.g., http request) to achieve interfaces with Non-ROS parts of the general system. An Action Data Structure comprises data used by three of five Topics composed in Action, i.e., goal, feedback and result. Two remaining Topics, i.e., cancel and status are standardised.
 	...
 	\begin{figure}[htb]
	 	\centering
	 	\begin{center}
	 		{\includegraphics[scale=0.65]{img/meros_pkg/communication_channel_bdd.png}}
	 	\end{center}
	...
	The Node (Fig.~\ref{fig:node_bdd}) composes Parameters and Nodelets. Two specific Nodes are considered in the metamodel: ROS Master and rosout.
 	...
 	\begin{figure}[htb]
	 	\centering
	 	\begin{center}
	 		{\includegraphics[scale=0.65]{img/meros_pkg/node_bdd.png}}
	 	\end{center}
	 	\caption{Node -- bdd.} 
		 	\label{fig:node_bdd}
	 \end{figure}
  
	The Running System (Fig.~\ref{fig:running_system_bdd}) composes ROS and Non-ROS Communicating Component specializations as well as Connections between them.
	
		% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7	
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.7]{img/meros_pkg/running_system_bdd.png}}
		\end{center}
		\caption{Running System compositions -- bdd.} 
		\label{fig:running_system_bdd}
	\end{figure}
	
	 Two specific Nodes are highlighted for ROS~1: rosout and ROS master.	Additionally, a Parameter block is introduced also for ROS~1. In ROS~2, due to safety reasons Parameter is composed only into Nodes.
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.73]{img/meros_pkg/intrasystem_bdd.png}}
		\end{center}
		\caption{Intrasystem compositions -- bdd.} 
		\label{fig:intrasystem_bdd}
	\end{figure}
	
	Like the Intrasystem, the Namespace (Fig.~\ref{fig:namespace_bdd}) aggregates elements of the Running System, but only ROS related. In opposition to the Intrasystem, the Namespace does not specialise Communicating Component. Hence, it can not act as Communicating Component.
	
		% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7	
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.73]{img/meros_pkg/namespace_bdd.png}}
		\end{center}
...
\label{sec:metamodel-communication}
	...
	Thanks to a dedicated component to represent communication, the diagram in Fig.~\ref{fig:topic_communication_with_dedicated_component_ibd} can be split into two considering publisher (Fig.~\ref{fig:topic_split_publisher_ibd}) and subscriber (Fig.~\ref{fig:topic_split_subscriber_ibd}) separately, without losing information. It is useful, when fragments of the system are presented.
	
	% DLA SKLADU ZMIENIONO WIELKOSC Z 0.7	
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.8]{img/meros_pkg/topic_split_publisher_ibd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.8]{img/meros_pkg/topic_split_subscriber_ibd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[width=\columnwidth]{img/meros_pkg/topic_communication_with_dedicated_component_sd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.75]{img/meros_pkg/topic_communication_without_dedicated_component_ibd.png}}
		\end{center}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=0.75]{img/meros_pkg/topic_communication_without_dedicated_component_sd.png}}
		\end{center}
...
\label{sec:metamodel-service}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.75]{img/meros_pkg/service_communication_sd.png}}
		\end{center}
...
\label{sec:metamodel-action}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.66]{img/meros_pkg/action_communication_compact_ibd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.66]{img/meros_pkg/action_communication_detailed_ibd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.66]{img/meros_pkg/action_communication_compact_sd.png}}
		\end{center}
	...
	The detailed behaviour of the Action server and Action client in ROS~1 is specified by state machines \footnote{\url{http://wiki.ros.org/actionlib/DetailedDescription}}. ROS~2 Action server and Action client behaviour is analogous. Here, these two state machines are depicted in stm diagrams. In the description, in addition to the original ROS wiki presentation, the Topics are directly mentioned both in transitions and states actions.
	 ...
	 Fig.~\ref{fig:ros1_action_server_stm} depicts the Action server state machine.
	  Its transitions depend on the new messages sent by the Action client or internal predicates. 
	  
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.66]{img/meros_pkg/ros1_action_server_stm.png}}
		\end{center}
		\caption{ROS~1 Action server -- stm.} 
		\label{fig:ros1_action_server_stm}
	\end{figure}
	
	The Action client state machine (Fig.~\ref{fig:ros1_action_client_stm}) depends on the server state provided by the Action server in /status Topic and internal predicates. 
		
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.66]{img/meros_pkg/ros1_action_client_stm.png}}
		\end{center}
...
\label{sec:application-hints}
	...
	MeROS metamodel can be utilised in various ways in broad context of SE. It is, therefore, difficult to speak of an indication of the best procedure for its application. Nevertheless, it is possible to formulate some practical guidelines for building a particular system model based on MeROS. 
...
\label{sec:application-example}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.67]{img/rico_pkg/rico_running_system_bdd.png}}
		\end{center}
	...
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.67]{img/rico_pkg/rico_running_system_ibd.png}}
		\end{center}
	...
	Fig.~\ref{fig:robot_core_ibd} illustrates how various instances of the same block are depicted in the model.
	
	\begin{figure}[htb]
		\centering
		\begin{center}
			{\includegraphics[scale=0.67]{img/rico_pkg/robot_core_ibd.png}}
		\end{center}
		\caption{Selected elements of <<Intrasystem>> \texttt{:Robot Core} -- ibd.} 
		\label{fig:robot_core_ibd}
	\end{figure}
	
	 Two <<Parameter>> Objects of the same classifier \texttt{:Double} are composed into <<Node>> \texttt{:MoveBase}.
...
\label{sec:related-work}
	...
	Papers in the scope of the literature review are chosen based on an intensive study of the previous scientific work in robotic systems modelling. In particular, the survey \cite{de2021survey} is deeply analysed. As the qualification criterion of current work, the ROS metamodel is required. ROS must be described using UML language or SysML language. Six papers describing five metamodels met this criterion, and all used UML to describe ROS~1. The metamodels are contrasted with the representative requirements to which MeROS is subjected (Tab.~\ref{tab:ros-spec-req-sota}).
...
\label{sec:conslusions}
	...
	System development involves the use of a~number of tools organised in toolchains. The degree of tools interaction varies. In software engineering, the aim is to create clear procedures for system development, indicating the dependencies between the successive stages of the development process. In robotics, there have been many works dedicated to toolchains (SmartMDSD \cite{dennis2016smartmdsd}, RobotML \cite{robotml2}, \cite{dal2022formal}), nowadays the ROS is common middleware (e.g. \cite{wienke2012meta}, BRIDE \cite{bubeck2014bride}, HyperFlex \cite{brugali2016hyperflex}). MeROS is part of the toolchain used in the Robot Programming and Machine Perception Team at Warsaw University of Technology (WUT). 
	 The works are centred around several robotic platforms:
	 \begin{itemize}
	 	...
	 	\item Dobot Magician -- small, 4-DOF robotic manipulator \footnote{\url{https://www.robotyka.ia.pw.edu.pl/robots/magician}} (Custom controller based on ROS~2 for hardware control).
	 \end{itemize}
