...
At the critical isotropic point we instead infer a weakly non-Gaussian distribution, corroborating the view that
superdiffusive spin transport in integrable spin chains does not belong to any known dynamical universality class.
\end{abstract}
...
Regarding (ii), which is our main result, the data from our simulations does not comply with a `quasi-Gaussian' distribution recently predicted in Ref.~\cite{DeNardisKPZ} within the domain of quantum spin chains. There remains a possibility that quantum fluctuations could play a pivotal role, as already alluded to in \cite{DeNardisKPZ}, possibly spoiling universality. However, anticipating that the `quantum-classical correspondence' \cite{Gamayun_domain,10.21468/SciPostPhys.7.2.025,Misguich19,PhysRevLett.125.070601,superdiffusion_review} persists at the level of fluctuations, a plausible consequence of our findings is that superdiffusion in integrable isotropic chains does not admit a universal description in terms of an effective theory of nonlinear hydrodynamics.
...
that the asymptotic structure factors yield the Pr\"{a}hofer-Spohn scaling function \cite{Prahofer_2004} attributed to the KPZ universality class. However, as  originally pointed out earlier in Ref.~\cite{Krajnik2022a}, the KPZ equation initialized in the stationary ensemble \cite{Ferrari06,Imamura12,Corwin2012} generates inherently asymmetric fluctuations \cite{Baik2000,Baik_2001} due to broken detailed balance, contrasting with the situation in integrable spin chains.
%The stochastic Burgers equation therefore cannot correctly describe the coarse-grained dynamics of local magnetization, as initially advocated in Ref.~\cite{PhysRevB.101.041411}. 
 While hydrodynamic equations involving two coupled KPZ modes \cite{DeNardisKPZ} resolve this
particular shortcoming, our numerical simulations however reveal long-lived systematic deviations from both the Gaussian values
and the quasi-Gaussian distribution inferred in \cite{DeNardisKPZ}, with no noticeable dependence on the discretization time-step.

Our work raises several important questions. It remains to be carefully examined whether quantum corrections may play a key role
and affect the decay of higher-point correlations. It likewise remains unclear whether integrable systems featuring infinitely many local conserved quantities permit a well-posed reduction to effective mode-coupling equations with finitely many modes, particularly in absence of ergodic and strictly hyperbolic dynamics. Another important question to be explored concerns the structure of large-deviation rate functions in both dynamically critical regimes and whether dynamical phase transitions of first and second order found in charged single-file systems \cite{Krajnik2022SF} manifest themselves in the easy-axis or isotropic regimes away from equilibrium.
...
We thank S. Gopalakrishnan, V. Popkov and G. Sch\"{u}tz for insightful discussions, V. Pasquier for collaboration on related projects and J. Schmidt for collaboration on related projects and the numerical implementation.
