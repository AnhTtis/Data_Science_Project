...
\title{
%Dynamical universality classes of integrable spin chains
%On universal distributions of magnetization transfer in integrable spin chains
...
Here we revisit the problem of anomalous counting statistics in the Landau--Lifshitz field theory by specializing to two distinct
anomalous regimes featuring dynamical criticality. By performing optimized numerical simulations using an integrable space-time discretization we extract the algebraic growth exponents of time-dependent cumulants. The distinctly non-Gaussian statistics of magnetization transfer
in the easy-axis regime is found to converge towards the universal distribution of charged single-file systems.
At the critical isotropic point we instead infer a weakly non-Gaussian distribution, indicating that superdiffusive spin transport found in integrable spin chains does not belong to any known dynamical universality class.
\end{abstract}
...
statistical physics \cite{Huang_book,Livi_book}. On the other hand, the rich diversity of dynamical and non-equilibrium phenomena is much harder to describe within a common general mathematical framework. The central notion is that of \emph{dynamical universality},
most commonly encountered in the domain of noisy classical systems describing interface growth \cite{EW1982,KPZ,Quastel_2015,Takeuchi18} or, more generally, in the context of mode-coupling theory of nonlinear fluctuating hydrodynamics (NLFHD) \cite{Spohn_1991,herbert,Kulkarni2015,Popkov15}.
A general and precise mathematical foundation however appears to be still lacking.

The study of exotic dynamical properties has been at the forefront of theoretical \cite{RevModPhys.93.025003,superdiffusion_review,GV_review} and experiment \cite{Scherg2021,Scheie21,Jepsen22,Wei2022} research in recent years. Anomalous dynamical behavior is commonly associated with a lack of ergodicity. Most prominent examples include singular diffusion constants \cite{Ljubotina19,KP20,MatrixModels,DupontMoore19,Weiner20,superdiffusion_review},
...
For instance, classical fragmentation occurs in so-called charged single-file systems \cite{Krajnik2022SF} (including the solvable {charged hardcore lattice gas} \cite{Medenjak17} and the semi-classical low-energy theory of the sine-Gordon models \cite{Altshuler_2006,Kormos}), displaying a host of unorthodox dynamical properties such as, most prominently, a universal non-Gaussian {typical distribution of net charge transfer}.
...
In this work, we present a large-scale numerical study of an anisotropic classical integrable spin chain by computing the full counting statistics of cumulative spin current in thermal \emph{equilibrium}. Building upon our previous work \cite{Krajnik2022a}, we employ
an optimized numerical implementation that enables us to reach longer simulation times, improving upon Refs.~\cite{Krajnik2022a,SarangKhemani}
by three orders of magnitude. Specializing to both critical regimes, we numerically compute the time-dependent probability distribution of the cumulative spin current and extract the scaling exponents quantifying the temporal growth of cumulants.
The two main findings of our study are: (i) in the easy-axis regime, the time-dependent typical distribution slowly converges towards the universal non-Gaussian distribution characteristic of charged single-file systems \cite{Krajnik2022SF};
(ii) at the isotropic point, we infer a weakly non-Gaussian distribution and quantify systematic deviations from Gaussianity.

Invoking the `quantum-classical correspondence', see Refs.~\cite{Gamayun_domain,10.21468/SciPostPhys.7.2.025,Misguich19,PhysRevLett.125.070601,superdiffusion_review}, our findings quite plausibly also apply to quantum integrable spin chains.
We wish stress however that (ii), which is the main result of our work, conflicts with the recently predicted `quasi-Gaussian' distribution based on an effective hydrodynamic description involving two KPZ modes proposed recently in Ref.~\cite{DeNardisKPZ}. Our simulations arguably provide strong evidence that isotropic integrable spin chains exhibiting superdiffusive spin transport may not admit an effective description based on NLFHD.

{\bf Model}---We shall consider the anisotropic Landau-Lifshitz magnet in thermal equilibrium at high temperature.
...
Equation \eqref{eqn:LL} is a prime example of a completely integrable PDE~\cite{Takhtajan77,Faddeev_book}. It possesses infinitely many local conserved quantities, including the third component of
\begin{widetext}
...
the average $\langle \bullet \rangle$ is computed in a maximum entropy ensemble,
we characterize $\mathcal{P}(J|t)$ by its cumulants, $c_n(t) \equiv \frac{\dd^n}{\dd \lambda^n} \log G(\lambda|t) \big|_{\lambda=0}$.
We shall assume that $c_{n}(t)$ grow asymptotically with time as
\begin{equation}\label{c_scaling}
c_n(t) \asymp c_{n} t^{\nu_n},
\end{equation}
with algebraic growth exponents $\nu_{n}$. Moreover, time-reversal symmetry in equilibrium ensembles implies detailed balance, reflected in the symmetry $\mathcal{P}(J|t)=\mathcal{P}(-J|t)$. Accordingly, all odd cumulants identically vanish.

The growth of variance (second cumulant) $c_{2}(t)$ determines the \emph{typical} timescale of magnetization transfer, with
exponent $\nu_2 = 1/z$ given by the \emph{dynamical exponent} $z$ governs hydrodynamic relaxation of
...
The second and higher cumulants of the finite-time typical distribution $\kappa_n(t) \equiv \langle \mathcal{J}^n(t) \rangle^c$ are directly related to $c_{n}(t)$ by a simple rescaling, $\kappa_{n}(t) = t^{-n/2z} c_n(t)$.
...
improve the sampling statistics. The estimated cumulants $\hat c_n$ are computed directly from $\hat m_n$ using Fa\`{a} di Bruno's formula.
...
{\bf Easy-axis regime}--- In the easy-axis regime $\delta>0$, we confirm the anticipated critical behavior of $c_{n}(t)$ across four order of magnitude in time.  Temporal growth of the few lowest even cumulants $c_{n}(t)$ is shown in Fig.~\ref{fig:figure1}a, from where we deduce the growth exponents
...
is displayed in Fig.~\ref{fig:figure2} (left inset). The KL divergence $K_{\rm ea}(t) \equiv D_{\rm KL}(\hat{\mathcal{P}}_{1/4}|| \mathcal{M}_{\hat \sigma_{\rm ea}})$ decays approximately as $K_{\rm ea}(t)\simeq t^{-0.55}$ with $K_{\rm ea}(t_{\rm max})\approx 2.0\times 10^{-4}$,
...
\begin{figure}[t]
	...
	(right inset) Relaxation of KL divergence $K_{\rm iso}(t)$ (blue), with approximate algebraic decay $t^{-0.74}$ (black dashed line),
...
This time however, the numerically extracted first few growth exponents match the threshold values
...
We next attempt to quantify how much $\mathcal{P}^{\rm iso}_{\rm typ}$ deviates from Gaussianity.
...
Yet, it is not clear if such behavior persists for times beyond $t_{\rm max}$.
...
In addition, we compare the results of our simulations with the recent prediction of Ref.~\cite{DeNardisKPZ} which reports
(approximate) asymptotic values $\tilde \gamma \approx 0.17$ and $\tilde \mu_{|1|} \approx 0.788$.
In our simulations, see Fig.~\ref{fig:figure4}, we instead obtain $\hat \gamma(t_{\rm max}) \approx 0.02$ and
$\hat \mu_{|1|}({\rm max}) \approx 0.7972$, see inset of Fig.~\ref{fig:figure4}. Most glaringly, we find no decay or other trend
of relaxation towards the Gaussian values $\gamma^{\mathcal{N}} = 0$ and $\mu_{|1|}^{\mathcal{N}} = \sqrt{2/\pi}\approx 0.7979$.
%We note that such barely discernible deviations from Gaussianity are not incompatible with the observed decay of the KL distance for accessible finite times $t\leq t_{\rm max}$.
...
\begin{figure}[t]
	...
	with three standard deviation neighborhoods (shaded regions), shown for different $\tau$ (colored points). Solid black lines indicate Gaussian values $\gamma^{\mathcal{N}} = 0$ and $\mu_{|1|}^{\mathcal{N}} = \sqrt{2/\pi}$ (inset), while
	dashed black line marks $\tilde \gamma \approx 0.17$ of Ref.~\cite{DeNardisKPZ}.
...
of ballistic macroscopic fluctuation theory \cite{MBHD20,DoyonMyers20,BMFT} is expected to be valid in this regime.
Unfortunately, however, a reliable extraction of scaled cumulants $s_n = \lim_{t \to \infty} t^{-1}c_n(t)$ is difficult in practice.
Perhaps counterintuitively, an accurate numerical estimation of cumulants requires more computational resources in the regular scenario. To this end, we expand the finite-time moment estimates as $\hat m_n(t) \simeq \sum_{k=0}^n \hat m_{n, k}  t^{k/2z}$.
...
we numerically estimated the lowest few cumulants and extracted the algebraic exponents quantifying their temporal growth.
In the easy-axis regime and at the isotropic point, the onset of dynamical criticality causes a super-linear growth of higher cumulants.
In both cases, the estimated growth exponents coincide with threshold values, signifying a violation of the central limit property.
...
At the isotropic point the lowest standardized moments of the time-dependent typical distribution are found to converge close to Gaussian values, but we still detect systematic deviations that seemingly persist at late times.
...
a unified coarse-grained (i.e. macroscopic) evolution of the fluctuating height field in a particular scaling regime \cite{Corwin2012,Quastel_2015,baik2022kpz}, yielding an Airy process \cite{TracyWidom94,PrahoferSpohn02}.
Such a limit is unfortunately notoriously difficult to perform, and thus one instead most often focuses on simple dynamical observables such as local fluctuations of charge or current densities. In particular, the classification commonly used in the framework of NLFHD \cite{Popkov15,Popkov16,Schutz18,PhysRevE.100.052111} is based on the asymptotic form of dynamical two-point functions (dynamical structure factors), characterized by an algebraic decay exponent and stationary scaling profiles.
Such a classification is however not exhaustive, as it overlooks the fact that different processes may sometimes only be distinguished at the level of higher-order dynamical correlation functions such as e.g. the full counting statistics of charge transfer studied in this work. A subclass of \emph{ballistic} charged single-file systems provides an illustrative example \cite{Krajnik2022}: while magnetization transport in unbiased equilibrium states yields diffusive (i.e. Gaussian) scaling profiles \cite{Klobas2018}, statistics of magnetization transfer is anomalous and
described by the universal distribution \eqref{sf_dist}, thus ruling out normal diffusion.

A similar pitfall arises when classifying superdiffusive transport of non-abelian charges in integrable systems \cite{MatrixModels,superuniversality,superdiffusion_review}. There is by now ample numerical evidence \cite{Ljubotina19,DupontMoore19,KP20,MatrixModels,Das19_KPZ,Weiner20,superuniversality,PhysRevLett.129.230602} 
that the asymptotic structure factors yield the Pr\"{a}hofer-Spohn scaling function \cite{Prahofer_2004} attributed to the KPZ universality class. However, as already highlighted earlier in Ref.~\cite{Krajnik2022a}, the KPZ equation initialized in the stationary ensemble \cite{Ferrari06,Imamura12,Corwin2012} generates inherently asymmetric fluctuations \cite{Baik2000,Baik_2001} due to broken detailed balance, contrasting the situation in integrable spin chains. The stochastic Burgers equation therefore cannot correctly describe the coarse-grained dynamics of local magnetization, as initially advocated in Ref.~\cite{PhysRevB.101.041411}. Although the extended model of two coupled KPZ modes proposed in Ref.~\cite{DeNardisKPZ} resolves this particular issue, the predicted values of the associated quasi-Gaussian distribution systematically differ from the estimated values inferred from our simulations. Whether quantum fluctuations could be pivotal
and eventually affect the higher-point correlations remains to be carefully examined. Taking into account the fact that integrable systems feature infinitely many local conserved quantities, it remains unclear whether a consistent reduction to an effective mode-coupling theory with
only finitely many modes is even possible, particularly in absence of ergodic and hyperbolic dynamics.

Another important question to be explored concerns the structure of large-deviation rate functions in both dynamically critical regimes.
A related question is whether dynamical phase transitions of first and second order found in charge single-file systems \cite{Krajnik2022SF}
manifest themselves in the easy-axis regime away from equilibrium.
...
systems are now finally within reach of contemporary experimental techniques. We are thus hopeful that quantum simulators can
provide valuable insights.
 
\paragraph*{\bf Acknowledgments}
We thank V. Popkov and G. Sch\"{u}tz for illuminating discussions, V. Pasquier for collaboration on related projects and J. Schmidt for collaboration on related projects and the numerical implementation.
This work has been supported by the Slovenian Research Agency (ARRS) under the Program P1-0402 and grants N1-0233 and N1-0219. \v{Z}K gratefully acknowledges support of the Milan Lenar\v{c}i\v{c} foundation. EI is supported by grant N1-0243 of ARRS.
...
%\onecolumngrid
%\begin{center}
%	\textbf{{\large Supplemental Material for \\ ``Dynamical universality classes of integrable spin chains''}}
%\end{center}
