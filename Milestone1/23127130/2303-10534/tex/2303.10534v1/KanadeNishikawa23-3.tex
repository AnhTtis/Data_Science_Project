...
The conformal bootstrap method was applied by the paper \cite{Belavin:1984vu} to solve an infinite class of two-dimensional conformal field theories (CFTs). Now, it is known as one of the most influential and fascinating tools to analyze CFT from the point of nonperturbative aspects of the theory. It carves out the space of consistent CFTs by imposing physical conditions such as symmetry, causality, and unitarity. It leads to nontrivial conditions that cannot be obtained from the algebraic structure of conformal algebra and perturbative analysis. About thirty years after their work  \cite{Belavin:1984vu}, the paper \cite{Rattazzi:2008pe} introduced the numerical method for conformal bootstrap equations to get constraints for higher-dimensional CFTs. Using crossing symmetry for the four-point function, they showed the upper bound on the conformal dimension of the first scalar appearing in the OPE of two identical scalars. After their work, much progress has been made, such as fixing the critical exponents of the Ising model in three-dimensional CFT \cite{El-Showk:2012cjh}-\cite{Kos:2016ysd} and getting bounds on the scaling dimensions of the theory with global $O(N)$ symmetry \cite{Kos:2013tga}-\cite{Nakayama:2014yia}. Please see the reviews \cite{Poland:2016chs}-\cite{Chester:2019wfx} to check the recent works in conformal bootstrap.\par
As numerical bootstrap methods developed, there was a growing body of research investigating the analytical properties of bootstrap. One of the current research directions on conformal bootstrap is Large N expansion. This analysis method is robust in AdS/CFT correspondence \cite{Maldacena:1997re}-\cite{Heemskerk:2009pn}, in which we have a good correspondence between weakly coupled gravity in AdS and its dual CFTs. “Weakly coupled” translates to the CFT with large N, and in addition, we demand that CFT on the boundary is strongly coupled to make the gravity theories on the bulk without light particles of spin greater than two. In the analysis, we perturbative CFT by $1/N$ and $1/\Delta$, where $\Delta$ is the conformal dimension of the lightest operator in the CFT. Many researchers have studied the relationship between weakly coupled bulk theory and CFT data on its boundary \cite{Aharony:2016dwx}-\cite{Caron-Huot:2018kta}. The conformal bootstrap helps obtain consistent theories of gravity in AdS from the effective field theories, the swampland program \cite{Vafa:2005ui}-\cite{Palti:2019pca}. With these physical motivations, formulating conformal bootstrap without taking large N is naturally needed to understand the theory.\par
Recently, CFT in momentum space has been developed formally in \cite{Gillioz:2019lgs} \cite{Coriano:2013jba}-\cite{Bautista:2019qxj}. Those papers define correlation functions in momentum space as a Fourier transform of that in position space. CFT in momentum space has physical applications such as the study of anomalies \cite{Giannotti:2008cv}-\cite{Coriano:2011zk}, the determination of the form of conformal invariance in the non-Gaussian features of the Cosmic Microwave Background \cite{Antoniadis:1996dj}\cite{Antoniadis:2011ib}, and an investigation for inflation \cite{Maldacena:2011nz}\cite{Kehagias:2012pd}.\par
One of the difficulties of analysis in momentum space is that we cannot expect the time-ordered correlation function to behave well because the integral calculation in the Fourier transform involves the position where the operators are not time-ordered. Instead, the Wightman function is a good function for the analysis in momentum space. The Wightman function has an operator algebras, but we have yet to study its structure so much. The appendix summarizes the future direction that surveys algebraic construction in momentum space.\par
...
For two-dimensional CFT, the factorization method is potent as we can independently calculate a holomorphic and antiholomorphic part. After that, we can construct complete correlation functions for general operators.\par
...
\ee
We define two “Spatially-integrated mode” types for primary operator. The first one is
\be
...
\ee
where $y\eq\frac{r_1}{r_2}$ is a ratio of radius.
...
In the two-point function, $\Delta+s=h$ and $\Delta-s=\tilde{h}$ appear independently. In other words, we can factorize it into a holomorphic and antiholomorphic part. It is natural, but we summarize it to clarify the situation. Let us perform the Fourier transform of the two-point function of scalar operators.
\be
...
\ee
Energy for the state $(P_z)^{n_1+l_1}(P_{\bar{z}})^{l_1}\ket{\Phi}$ is $E_1=\frac{h+\tilde{h}+n_1+l_1+l_1}{R}=\frac{\Delta+n_1+2l_1}{R}$ and momentum for the state is $k_1=\frac{n_1}{R}$. So, the Gamma function dependent part is
\be
...
As we can calculate the holomorphic and the antiholomorphic parts independently, we only focus on the holomorphic part. First, the ket $\ket{\phi_{[J]}}$ is defined as
\be
...
\ee
The ket for descendat field $(L_{-m_k}\cdots L_{-m_2}L_{-m_1}\phi)(z)$ is
\be
...
\end{align}
The phases are chosen so that $|\ket{\phi_{[J]}}|^2\geq0$ and
$|\ket{L_{-m_k}\cdots L_{-m_1}\phi_{[J]}}|^2\geq0$ are satisfied. With those definitions, we can recover the previous result, for example,
\be
...
\ee
It is the same as a result (\ref{Gillioz's result}) in infinite volume calculated in the paper \cite{Gillioz:2021sce}.
\subsection{Three-point function}
Solving WIs for the three-point function is complex. So first, we Fourier transform the three-point function of holomorphic operators directly and check that the solution satisfies the ODEs obtained from WIs. The three-point function of holomorphic operators is
\be
...
\bml
  ...
  \times\left[\frac{\Theta(p_0^+)}{\Gamma(\Delta_i)\Gamma\left(\frac{\Delta_f+\Delta_0-\Delta_i}{2}\right)}{}_2F_1\left(\frac{\Delta_f-\Delta_0+\Delta_i}{2},\frac{\Delta_f-\tilde{\Delta}_0+\Delta_i}{2};\Delta_i;-\frac{p^+_i}{p^+_0}\right)\right.\\
...
\end{cases}
\ee
We want to get this result by taking a large volume limit for our result at a finite volume. We use a time-reversal symmetric form for three-point functions. $\KK_{red}$ is defined as
\be
  \KK(n_1,n_2,n_3)\eq\langle\OO_{n_3}(z_3)\OO_{n_2}(z_2)\OO_{n_1}(z_1)\rangle\eq\delta(n_1+n_2+n_3+h_1+h_2+h_3)\KK_{red}(n_1,n_3)
\ee
And remember that this is equal to
\be
  \KK_{red}(n_1,n_3)=\tfrac{\lambda_{321}}{\Gamma(b_1)\Gamma(b_2)\Gamma(b_3)}\sum_{q=0}^{\mathrm{Min}\{n_1,m_3\}}\tfrac{\Gamma[b_3+q+\mathrm{Max}\{{0,n_1-m_3}\}]\Gamma[b_2-q+\mathrm{Min}\{{n_1,m_3}\}]\Gamma[b_1+q+\mathrm{Max}\{{m_3-n_1,0}\}]}{\Gamma[1+q+\mathrm{Max}\{{0,n_1-m_3}\}]\Gamma[1-q+\mathrm{Min}\{{n_1,m_3}\}]\Gamma[1+q+\mathrm{Max}\{{m_3-n_1,0}\}]}
\ee
To take a large volume limit, define $\NN[x,y]\eq\frac{\Gamma(1+x+y)}{\Gamma(1+x)\Gamma(1+y)}$. Under the $y\rightarrow\infty$ limit, $\NN[x,y]\simeq\frac{y^x}{\Gamma(1+x)}$. With this function, we can write a three-point function as
\bml
\KK_{red}(n_1,n_3)=\lambda_{321}\sum_{q=0}^{\mathrm{Min}\{n_1,m_3\}}\NN[b_3-1,q+\mathrm{Max}\{{0,n_1-m_3}\}]\\
\NN[b_2-1,-q+\mathrm{Min}\{n_1,m_3\}]\NN[b_1-1,q+\mathrm{Max}\{m_3-n_1,0\}]
\end{multline}
Under a large volume limit, $n_1$ and $m_3$ also go to infinity. We define $\hat{q}$, $\hat{n}_1$ and $\hat{m}_3$ as $\hat{q}\eq\frac{q}{R}$ $\hat{n}_1\eq\frac{n_1}{R}$ and $\hat{m}_3\eq\frac{m_3}{R}$. Then,
\begin{align}
...
&=\frac{1}{\Gamma(b_1)\Gamma(b_2)\Gamma(b_3)}(q+\mathrm{Max}\{m_3-n_1,0\})^{b_1-1}(-q+\mathrm{Min}\{n_1,m_3\})^{b_2-1}(q+\mathrm{Max}\{{0,n_1-m_3}\})^{b_3-1}\notag\\
&=\frac{R^{b_1+b_2+b_3-3}}{\Gamma(b_1)\Gamma(b_2)\Gamma(b_3)}(\hat{q}+\mathrm{Max}\{\hat{m}_3-\hat{n}_1,0\})^{b_1-1}(-\hat{q}+\mathrm{Min}\{\hat{n}_1,\hat{m}_3\})^{b_2-1}(\hat{q}+\mathrm{Max}\{{0,\hat{n}_1-\hat{m}_3}\})^{b_3-1}
\end{align}
Using $\sum_q\simeq\int \dd q=R\int \dd\hat{q}$,
\begin{align}
&\KK_{red}(n_1,n_3)\notag\\
...
&\simeq A\int_0^{\mathrm{Min}\{\hat{n},\hat{n}'\}}d\hat{q}(\hat{q}+\mathrm{Max}\{\hat{m}_3-\hat{n}_1,0\})^{b_1-1}(-\hat{q}+\mathrm{Min}\{\hat{n}_1,\hat{m}_3\})^{b_2-1}(\hat{q}+\mathrm{Max}\{{0,\hat{n}_1-\hat{m}_3}\})^{b_3-1}\notag\\
\end{align}
where $A\eq\frac{\lambda_{321}R^{b_1+b_2+b_3-2}}{\Gamma(b_1)\Gamma(b_2)\Gamma(b_3)}$. Now define $x\eq\frac{\hat{q}}{\mathrm{Min}\{\hat{n}_1,\hat{m}'_3\}}$ to change the range of integration from $[0,\mathrm{Min}\{\hat{n}_1,\hat{m}_3\}]$ to $[0,1]$.
\begin{align}
  &\KK_{red}(n_1,n_3)\notag\\
  &\simeq A[\mathrm{Min}\{\hat{n}_1,\hat{m}_3\}]^{b_1+b_2+b_3-2}\int_0^1dx\left(x+\tfrac{\mathrm{Max}\{\hat{m}_3-\hat{n}_1,0\}}{\mathrm{Min}\{\hat{n}_1,\hat{m}_3\}}\right)^{b_1-1}\left(1-x\right)^{b_2-1}\left(x+\tfrac{\mathrm{Max}\{0,\hat{n}_1-\hat{m}_3\}}{\mathrm{Min}\{\hat{n}_1,\hat{m}_3\}}\right)^{b_3-1}\notag\\
  &=A\times
  \begin{cases}
...
\ee
Let us consider the case of $\hat{n}_1\geq\hat{m}_3$. In the above expression, this case corresponds to $\alpha=b_1-1$, $\beta=b_2-1$, $\gamma=b_3-1$, $t=\frac{\hat{n}_1-\hat{m}_3}{\hat{m}_3}$.
\begin{multline}
  \int_0^1dx\ x^{b_1-1}(1-x)^{b_2-1}\left(x+\tfrac{\hat{n}_1-\hat{m}_3}{\hat{m}_3}\right)^{b_3-1}=\left(\tfrac{\hat{n}_1-\hat{m}_3}{\hat{m}_3}\right)^{b_3-1}\tfrac{\Gamma(b_1)\Gamma(b_2)}{\Gamma(b_1+b_2)}{}_2F_1\left[b_1,1-b_3;b_1+b_2;-\tfrac{\hat{m}_3}{\hat{n}_1-\hat{m}_3}\right]
\end{multline}
So, the three-point function is
\begin{align}
  \left.\KK_{red}(n_1,m_3)\right|_{n_1\geq m_3}\simeq A\hat{m}_3^{2h_3-1}(\hat{n}_1-\hat{m}_3)^{h_1+h_2+h_3-1}{}_2F_1\left[b_1,1-b_3;2h_3;-\tfrac{\hat{m}_3}{\hat{n}_1-\hat{m}_3}\right]
\end{align}
In chiral theory, $P^0=E=P^1$ and $E=P^1=\frac{J}{R}$, so under large volume limit,
\begin{align}
...
\end{align}
In this case, $P_i\geq0$, $P_o\leq0$, $P_f\leq0$. And $h_i=\tilde{h}_i=\frac{\Delta_i}{2}$ for $i=1,2,3$. We can write the three-point function in terms of $P_i$, $P_o$, $P_f$ as
\bml
  \left.\KK_{red}(n_1,m_3)\right|_{P^+_i\geq -P^+_f}\\
...
\bml
  \left.\KK_{red}(n_1,m_3)\right|_{P^+_o\leq0}\\
  \simeq\tfrac{\lambda_{321}(R/2)^{h_1+h_2+h_3-2}}{\Gamma(\Delta_f)\Gamma\left(\frac{\Delta_i+\Delta_o-\Delta_f}{2}\right)}(|P^+_f|)^{\Delta_f-1}(|P^+_i|)^{\Delta_i-1}(|P^+_0|)^{\frac{\Delta_o-\Delta_i-\Delta_f}{2}}{}_2F_1\left[\tfrac{\Delta_f-\Delta_o+\Delta_i}{2},\tfrac{\Delta_f-\tilde{\Delta}_o+\Delta_i}{2};\Delta_f;-\tfrac{P^+_f}{P^+_o}\right]
\end{multline}
...
\section{Main results in D=3}
\setcounter{equation}{0}
Calculations in three-dimensional CFT proceed the same way as in two-dimensional CFT, but some difficulties were absent in two-dimensional CFT. In two-dimensional CFT, factorization made the analysis easier. In higher dimensional CFT, on the other hand, we must perform a highly complex calculation. In particular, the form of the three-point function is complicated, and there is no closed description. We must calculate the three-point function with recursion relations obtained from WI term by term.
...
\ee
We now turn our attention to scalar operators to make the discussion simple. We can make the same argument for operators with general spin. The actions of the conformal generators on the scalar operators are as follows.
\begin{align}
...
$Y_{l,m}$ is a spherical harmonics, and $Y^{*}_{l,m}=(-1)^{m}Y_{l,-m}$ is its complex conjugate. The action of conformal generators on the spatially integrated mode is as follows.
\begin{flalign}
...
\sqrt{(l_2+m_1+1)(l_2-m_1)}F_{m_1}^{l_1,l_2}+\sqrt{(l_1-m_1)(l_1+m_1+1)}F_{m_1+1}^{l_1,l_2}=0\label{J- Ward Identity}
\ee
Shift (\ref{J+ Ward Identity}) by $m_1\rightarrow m_1+1$.
\be
...
\end{align}
The normalization of operators determines the value of $K_0$. We can always set it one.
\subsection{Three-point function}
Finding the explicit closed formula for the three-point function is complex. Of course, we can perform direct integral calculations honestly, but the number of terms for that expression is too large to handle. \par
...
\end{align}
where $y_1\eq\frac{r_1}{r_2}$ and $y_3\eq\frac{r_3}{r_2}$. Next, consider the $J_x\pm iJ_y$ WIs.
\bml
...
\begin{flalign}
  &\int\left(\prod_{j=1}^4\dd^2x_j\right)e^{-i\prod_{j=1}^4P_j\cdot x_j}\bra{0}\OO^{(4)}[\OO^{(3)},\OO^{(2)}]\OO^{(1)}\ket{0}\delta(\sigma_3-\sigma_2-a)\delta(\tau_3-\tau_2)&\notag\\
  &=\int\dd^2x_2\dd^2x_3e^{-i(P_2\cdot x_2+P_3\cdot x_3)}\int\frac{\dd^2k}{(2\pi)^2} e^{ik\cdot(x_3-x_2)}e^{-iak_1}\bra{0}\tilde{\OO}^{(4)}(P_4)[\OO^{(3)},\OO^{(2)}]\tilde{\OO}^{(1)}(P_1)\ket{0}&\notag\\
  &\propto\int\dd^2ke^{-iak_1}\int\dd^2x_2\dd^2x_3e^{-i(P_2+k)\cdot x_2-i(P_3-k)\cdot x_3}\bra{0}\tilde{\OO}^{(4)}(P_4)[\OO^{(3)}(x_3),\OO^{(2)}(x_2)]\tilde{\OO}^{(1)}(P_1)\ket{0}&\notag\\
...
\end{flalign}
We defined $Q\equiv\frac{P_2-P_3+2k}{2}$.\par
...
\ee
The second term is $-1$ when $x<0$, $-\frac{1}{2}$ when $x=0$, and $0$ when $x>0$. We can check it by complex analysis.
...
In total, the second term above is $(-1)\times\frac{\pi i}{2\pi i}=-\frac{1}{2}$. In the latter, the contour encloses a pole at $k=-i\epsilon$. In total, the second term above is $(-1)\times(\frac{2\pi i}{2\pi i}-\frac{\pi i}{2\pi i})=-\frac{1}{2}$. Both calculations yield the same result.\par
...
=&\left[1+\frac{1}{2\pi i}\lim_{\epsilon_1\rightarrow0}\int_{-\infty}^\infty\dd k_1\frac{e^{i(k_1+i\epsilon_1)(\sigma_3-\sigma_2-\tau_3+\tau_2-1)}}{k_1+i\epsilon_1}\right]\left[1+\frac{1}{2\pi i}\lim_{\epsilon_2\rightarrow0}\int_{-\infty}^\infty\dd k_2\frac{e^{i(k_2+i\epsilon_2)(\sigma_3-\sigma_2+\tau_3-\tau_2-1)}}{k_2+i\epsilon_2}\right]&
\end{flalign}
...
With this test function, we get
\begin{flalign}
  &\int\left(\prod_{j=1}^4\dd^2 x_j\right)e^{-i\sum_{j=1}^4P_j\cdot x_j}\bra{0}\OO^{(4)}[\OO^{(3)},\OO^{(2)}]\OO^{(1)}\ket{0}f(x_3-x_2)&\notag\\
  ...
  &\hspace{3cm}\times\left[1+\frac{1}{2\pi i}\lim_{\epsilon_2\rightarrow0}\int_{-\infty}^\infty\dd k_2\frac{e^{i(k_2+i\epsilon_2)(\sigma_3-\sigma_2+\tau_3-\tau_2-1)}}{k_2+i\epsilon_2}\right]\bra{0}\OO^{(4)}[\OO^{(3)},\OO^{(2)}]\OO^{(1)}\ket{0}&\notag\\
  ...
  &\times\llangle\tilde{\OO}^{(4)}(P_4)[\tilde{\OO}^{(3)}(E_3-k_1+k_2,p_3-k_1-k_2),\tilde{\OO}^{(2)}(E_2+k_1-k_2,p_2+k_1+k_2)]\tilde{\OO}^{(1)}(P_1)\rrangle]&\label{long}
\end{flalign}
...
\begin{align}
  &(\ref{long})=\delta(E_2+E_3)\delta(p_2+p_3)\left[(\prod_{j=1}^4P_j^2)^{\frac{\Delta_\phi-1}{2}}[\Theta(P_{2+})\Theta(P_{2-})-\Theta(P_{3+})\Theta(P_{3-})]\right.\notag\\
  ...
  &\hspace{5cm}\times[\Theta(E_2+p_2+2k_1)\Theta(E_2-p_2)-\Theta(E_3+p_3-2k_1)\Theta(E_3-p_3)]\notag\\
  ...
  &\hspace{5cm}\times[\Theta(E_2+p_2)\Theta(E_2-p_2-2k_2)-\Theta(E_3+p_3)\Theta(E_3-p_3+2k_2)]\notag\\
  ...
  &\{(P_3^2+2k_1(E_3-p_3)-2k_2(E_3+p_3)+4k_1k_2)(P_2^2-2k_1(E_2-p_2)+2k_2(E_2+p_2)+4k_1k_2)\}^{\frac{\Delta_\phi-1}{2}}\notag\\
  &\left.\hspace{1cm}\times[\Theta(E_2+p_2+2k_1)\Theta(E_2-p_2-2k_2)-\Theta(E_3+p_3-2k_1)\Theta(E_3-p_3+2k_2)]\right]
\end{align}
...
\begin{align}
  ...
  &\hspace{4cm}\times[\Theta(E_2+p_2+2k_1)\Theta(E_2-p_2)-\Theta(-E_2-p_2-2k_1)\Theta(-E_2+p_2)]\notag\\
  ...
  &\hspace{4cm}\times[\Theta(E_2+p_2)\Theta(E_2-p_2-2k_2)-\Theta(-E_2-p_2)\Theta(-E_2+p_2+2k_2)]\notag\\
  &+\left(\frac{1}{2\pi i}\right)^2\lim_{\epsilon_1\rightarrow0}\lim_{\epsilon_2\rightarrow0}\int_{-\infty}^\infty\dd k_1\frac{e^{-i(k_1+i\epsilon_1)}}{k_1+i\epsilon_1}\int_{-\infty}^\infty\dd k_2\frac{e^{-i(k_2+i\epsilon_2)}}{k_2+i\epsilon_2}\notag\\
  &\hspace{1cm}\times(P_2^2-2k_1(E_2-p_2)+2k_2(E_2+p_2)+4k_1k_2)^{\Delta_\phi-1}\notag\\
  &\hspace{1cm}\times[\Theta(E_2+p_2+2k_1)\Theta(E_2-p_2-2k_2)-\Theta(-E_2-p_2-2k_1)\Theta(-E_2+p_2+2k_2)]
\end{align}
...
\ee
As in the case of finite volume, we adopt the following improved microcausality condition.
\be
\bra{0}\OO^{(4)}(\sigma_4,\tau_4)[\OO^{(3)}(\sigma_3,\tau_3),\OO^{(2)}(\sigma_2,\tau_2)]\OO^{(1)}(\sigma_1,\tau_1)\ket{0}f(x_3-x_2)=0
\ee
...
\begin{align}
  &\int\left(\prod_{j=1}^4\dd x_j\right) e^{-i\sum_{j=1}^4P_j\cdot x_j}\bra{0}\OO^{(4)}[\OO^{(3)},\OO^{(2)}]\OO^{(1)}\ket{0}\delta(\sigma_3-\sigma_2-a)\delta(\tau_3-\tau_2)\notag\\
  &\propto\int\dd^2x_2\dd^2x_3e^{-i(P_2\cdot x_2+P_3\cdot x_3)}\int\dd k_0e^{-ik_0(\tau_3-\tau_2)}\sum_{k_1=\frac{\mathbb{Z}}{R}}e^{ik_1(\sigma_3-\sigma_2-a)}\bra{0}\tilde{\OO}^{(4)}[\OO^{(3)},\OO^{(2)}]\tilde{\OO}^{(1)}\ket{0}\notag\\
  &\propto\int\dd k_0\prod_{j=2,3}\dd x_j^2\sum_{k_1=\frac{\mathbb{Z}}{R}}e^{-i[ak_1+(p_2+k_1)\sigma_2+(p_3-k_1)\sigma_3-(E_2+k_0)\tau_2-(E_3-k_0)\tau_3]}\bra{0}\tilde{\OO}^{(4)}[\OO^{(3)},\OO^{(2)}]\tilde{\OO}^{(1)}\ket{0}\notag\\
...
\end{align}
We defined $Q\equiv\frac{P_2-P_3+2k}{2}$. The value of $Q_0$ and $Q_1$ depends on the theory and the boundary conditions of “in-state” and “out-state.” Let us see the situation next.
\subsubsection{Completeness relation}
We need to insert a completeness relation between the second and the third operator to calculate the four-point function. In previous papers, it is constructed in momentum space in an infinite volume \cite{Gillioz:2019iye}\cite{Gillioz:2016jnn}-\cite{Karateev:2018oml}
\ba
...
\begin{multline}
  ...
  =C^{[0][0]}_{h}\dyad{\phi_{[h]}}+C^{[0][0]}_{h+1}\dyad{\phi_{[h+1]}}+C^{[0][0]}_{h+2}\dyad{\phi_{[h+2]}}+C^{[2][2]}_{h+2}\dyad{L_{-2}\phi_{[h+2]}}\\
  +C^{[2][0]}_{h+2}\dyad{L_{-2}\phi_{[h+2]}}{\phi_{[h+2]}}+C^{[0][2]}_{h+2}\dyad{\phi_{[h+2]}}{L_{-2}\phi_{[h+2]}}+\cdots
\end{multline}
Here, $\{k\}=k_i,\cdots,k_1\ (k_i\geq\cdots\geq k_1)$ means that the Virasoro operators act in the form $L_{-k_i}\cdots L_{-k_1}\ket{h}$. Please notice that $L_{-1}$ does not appear in the above expression. They are absorbed in the basis $\dyad{\phi_{J}}$ as $L_{-1}$ corresponds to a momentum operator.\par
...
\end{align}
Now we have everything, let us start from the level $J=h$. The demand that the above expression should be the completeness relation means that the expression above is identity. So, multiplying $\bra{\phi_{[h]}}$ from the left and $\ket{\phi_{[h]}}$ from the right,
\ba
...
Nontriviality appears from the next level $J=h+2$. We have four parameters $C_{h+2}^{[0][0]}$, $C_{h+2}^{[2][2]}$, $C_{h+2}^{[2][0]}$, $C_{h+2}^{[0][2]}$ and four equations obtained by sandiching it between $\bra{\phi_{[h+2]}}$ or $\bra{L_{-2}\phi_{[h+2]}}$ and $\ket{\phi_{[h+2]}}$ or $\ket{L_{-2}\phi_{[h+2]}}$. We get the following four equations.
\begin{align}
  &h(2h+1)=h^2(2h+1)^2C_{h+2}^{[0][0]}+(-3h)h(2h+1)C_{h+2}^{[2][0]}+(-3h)h(2h+1)C_{h+2}^{[0][2]}+(3h)^2C_{h+2}^{[2][2]}\\
  &4h+\tfrac{c}{2}=(3h)^2C_{h+2}^{[0][0]}+(-3h)(4h+\tfrac{c}{2})C_{h+2}^{[2][0]}+(-3h)(4h+\tfrac{c}{2})C_{h+2}^{[0][2]}+(4h+\tfrac{c}{2})^2C_{h+2}^{[2][2]}\\
  &-3h=(-3h)h(2h+1)C_{h+2}^{[0][0]}+(4h+\tfrac{c}{2})h(2h+1)C_{h+2}^{[2][0]}+(3h)^2C_{h+2}^{[0][2]}+(-3h)(4h+\tfrac{c}{2})C_{h+2}^{[2][2]}\\
  &-3h=(-3h)h(2h+1)C_{h+2}^{[0][0]}+(3h)^2C_{h+2}^{[2][0]}+(4h+\tfrac{c}{2})h(2h+1)C_{h+2}^{[0][2]}+(-3h)(4h+\tfrac{c}{2})C_{h+2}^{[2][2]}
\end{align}
They only sometimes have a solution. For example, when $h=\frac{1}{2}, c=\frac{1}{2}$, they become the same equation.
\begin{align}
...
This corresponds to a energy operator $\epsilon$ in Ising model ($\mathcal{M}_3,c=\frac{1}{2}$). The fact that the above four equations are not linearly independent corresponds to the fact that the Kac determinant \cite{KacLecturenote} vanishes at level 2 for $h=\frac{1}{2},c=\frac{1}{2}$.\par
Next, consider the case $h=\frac{3}{2},c=\frac{7}{10}$. It is one of the operators in the Tricritical Ising model \cite{Zamolodchikov:1986db}. We get $C_{h+2}^{[0][0]}=\frac{127}{357}$, $C_{h+2}^{[2][0]}=\frac{30}{119}$, $C_{h+2}^{[0][2]}=\frac{30}{119}$, $C_{h+2}^{[2][2]}=\frac{40}{119}$. Of course, we can get eigenvectors, but it is not so useful for calculating the four-point function.
\begin{multline}
...
Next, consider contributions from intermediate states other than the $\OO$ primary conformal block. First, the three-point function of two chiral operators $\phi(z_3),\phi(z_2)$ with conformal weight $h_2=h_3=h_\phi$ and $L_{-n}\OO(z_1)$ with conformal weight $h_1=h_\OO+n$.
\be
...
\end{align}
The differential operator $-\frac{1}{(z_3-z_1)^{n-1}}\pdv{z_3}$ acts on $\frac{1}{(z_2-z_3)^{h_2+h_3-h_1}}$ and $\frac{1}{(z_3-z_1)^{h_3+h_1-h_2}}$. In the former,
\ba
...
\begin{multline}
  ...
  =\frac{\KK^{(h_L,h_L,H_L)}_{red}(N_L,n'_L)\KK^{(H_L,h_L,h_L)}_{red}(n_L,N_L)}{\frac{\Gamma(N_L+2H_L)}{\Gamma(N_L+1)\Gamma(2H_L)}}\times(L\rightarrow R)
\end{multline}
...
\begin{align}
  \KK^{(h_3,h_2,h_1)}_{red}(n,n')\equiv\tfrac{1}{\Gamma(b_1)\Gamma(b_2)\Gamma(b_3)}\sum_{q=0}^{\text{Min}\{n,n'\}}\tfrac{\Gamma(b_3+q+\text{Max}\{0,n-n'\})\Gamma(b_2-q+\text{Min}\{n,n'\})\Gamma(b_1+q+\text{Max}\{n'-n,0\})}{\Gamma(1+q+\text{Max}\{0,n-n'\})\Gamma(1-q+\text{Min}\{n,n'\})\Gamma(1+q+\text{Max}\{n'-n,0\})}
\end{align}
where $b_i\equiv h_1+h_2+h_3-2h_i$.\par
By determing the boundary condition $(n_L,n_R,n'_L,n'_R)$, we get bootstrap equation. \\
...
\begin{multline}
  ...
  \times\frac{\Gamma(2H_R)(\Gamma(H_R+N_R))^2}{(\Gamma(H_R))^2\Gamma(N_R+2H_R)\Gamma(N_R+1)}
\end{multline}
...
\centerline{\textit{Summary}}
In this paper, we have done the following:
\begin{itemize}
  \item We formulated one of the conformal bootstrap equations in momentum space representation at finite volume. We dealt with two-dimensional and three-dimensional CFT, but this approach applies to conformal field theory with $d>3$.
  \item In two-dimensional CFT, factorization helps calculate momentum space's two- and three-point functions. We explained three methods, direct integral calculation, algebraic calculation, and WI method, to calculate them, though they are equivalent in principle. The most valuable and fascinating approach is the WI method since this applies to the Wightman function of operators with general conformal weight and the higher-dimensional CFTs.
  ...
  \item Taking Large volume limit, we proved that the two-point function and the three-point function we got at finite volume are consistent with the previous result \cite{Gillioz:2018mto}-\cite{Gillioz:2019iye}\cite{Gillioz:2021sce} at infinite volume.
...
\item This paper dealt with conformal bootstrap obtained from the microcausality condition. In the construction, we ignored the explicit form of the commutator by multiplying the test function that has support only at spacelike region. On the other hand, finding a concrete expression for the commutator is needed to find a valid, closed bootstrap equation for the Wightman function by comparing (43)(21), (42)(31), and (32)(41) channels. However, the commutator of the operators is not well-defined for a non-free CFT, so we need to consider the commutator of the smeared operators for time and space directions. This formulation has been constructed \cite{Nagano:2021tbu}\cite{Besken:2020snx} but has yet to apply it to the conformal bootstrap equations fully. It is a desirable research direction.
...
\end{align}
Adding equation (\ref{Ward Id2-5}) and (\ref{Ward Id2-7}) gives
\be
0=2(h_2-h_1)[G_{n+1}(y)-yG_n(y)]
\ee
And adding equation (\ref{Ward Id2-6}) and (\ref{Ward Id2-8}) gives
\be
...
\end{align}
where K is an undetermined constant depending on the normalization of operators.\par
...
\end{align}
$c_{n+s|l}$ is factorized into a holomorphic ($h=\frac{\Delta+s}{2}$) and antiholomorphic ($\tilde{h}=\frac{\Delta-s}{2}$) parts.\\
...
\end{align}
From this, we obtain recurtion relation for $K_{n+s}$, $b_{n+s|l}$ and $c_{n+s|l}$.
\begin{align}
...
\begin{align}
  ...
  &\hspace{5mm}\times\left[\frac{\Gamma(1+n)\Gamma(\Delta+s+1+n)\Gamma(\Delta-s+1)}{\Gamma(\Delta+s+n)\Gamma(\Delta-s)\Gamma(n+2)\Gamma(2)}-(\Delta+s+n)\right]^{-1}\notag\\
...
\ee
Next, define $\omega_3=\frac{1}{z_3}$ and integrate it with respect to $\omega_3$. Under this transformation, the contour integral around $z=\infty$ becomes contour integral around $\omega=0$.
\begin{flalign}
&\frac{1}{2\pi i}\oint\frac{dz_3}{z_3^{1+n_3}}\text{(\ref{integral1})}=\frac{1}{2\pi i}\oint\frac{d\omega_3}{\omega_3^{1+m_3}}\frac{\lambda_{321}}{\Gamma(b_3)\Gamma(b_2)}\sum_{k=0}^{n_1}\frac{\Gamma(b_3+k)\Gamma(b_2+n_1-k)}{\Gamma(1+k)\Gamma(1+n_1-k)}z_2^{-b_3-k}\frac{\omega_3^{n_1-k}}{(1-z_2\omega_3)^{b_1}}&\label{integral2}
\end{flalign}
...
So, the three-point function is
\begin{flalign}
  &\text{(\ref{integral0})}=\frac{\lambda_{123}}{\Gamma(b_3)\Gamma(b_2)\Gamma(b_1)}\sum_{k=n_1-m_3}^{n_1}\frac{\Gamma(b_3+k)\Gamma(b_2+n_1-k)}{\Gamma(1+k)\Gamma(1+n_1-k)}\frac{\Gamma(b_1+m_3-n_1+k)}{\Gamma(1+m_3-n_1+k)}&
\end{flalign}
(2)$m_3\geq n_1$
\begin{flalign}
...
So, the three-point function is
\begin{align}
  &\text{(\ref{integral0})}=\frac{\lambda_{123}}{\Gamma(b_3)\Gamma(b_2)\Gamma(b_1)}\sum_{k=0}^{n_1}\frac{\Gamma(b_3+k)\Gamma(b_2+n_1-k)}{\Gamma(1+k)\Gamma(1+n_1-k)}\frac{\Gamma(b_1+m_3-n_1+k)}{\Gamma(1+m_3-n_1+k)}&
\end{align}
In the end, we get
\begin{flalign}
...
The relation between $\KK_{red}(n_1,m_3)$ and $F(n_1,n_3,y_1,y_3)$ is
\be
  \KK_{red}(n_1,m_3)r_1^{n_1}r_2^{n_2}r_3^{n_3}=r_1^{-\Delta_1}r_2^{-\Delta_2}r_3^{-\Delta_3}F(n_1,n_3,y_1,y_3)
\ee
Remember that $m_3$ is defined as $m_3=-(n_3+2h_3)$. Then,
\be
  F(n_1,n_3,y_1,y_3)=y_1^{n_1+h_1}y_3^{n_3+h_3}\KK_{red}(n_1,m_3)\label{reduced form}
\ee
...
\end{align}
Substituting (\ref{reduced form}) for them gives
\begin{align}
  P_z&:0=(n_3+1)\KK_{red}(n_1,n_3+1)-(h_1+h_2+h_3+n_1+n_3)\KK_{red}(n_1,n_3)\notag\\
  &\hspace{1cm}+(n_1+1)\KK_{red}(n_1+1,n_3)\\
  P_{\bar{z}}&:\mathrm{satidfied\  trivially}\notag\\
  K^z&:0=(n_3+2h_3-1)\KK_{red}(n_1,n_3-1)-(n_1+n_3+h_1+h_3-h_2)\KK_{red}(n_1,n_3)\notag\\
  &\hspace{1cm}+(n_1+2h_1-1)\KK_{red}(n_1-1,n_3)\\
...
We have to prove the following equations. For reduced $P_z$ WI,
\ba
...
\end{align}
We call the first term (A-1) and the second (A-2). Then,
\begin{align}
&\mathrm{(RHS)}=\mathrm{((A-1)}+\mathrm{(B))}+\mathrm{((A-2)}+\mathrm{(C))}\notag\\
&=\frac{\Gamma(b_3+n_1+1)\Gamma(b_2)\Gamma(b_1+m_3)}{\Gamma(1+n_1)\Gamma(1)\Gamma(1+m_3)}+\sum_{k=0}^{n_1}k\frac{\Gamma(b_3+k)\Gamma(b_2+n_1-k+1)\Gamma(b_1+m_3-n_1+k-1)}{\Gamma(1+k)\Gamma(2+n_1-k)\Gamma(m_3-n_1+k)}\notag\\
...
\end{align}
We call the first term (D-1) and the second (D-2). Then,
\begin{align}
...
\end{align}
We call the first term (I-1) and the second (I-2). Then,
\begin{align}
...
\end{align}
We call the first term of this (L-1) and the second term (L-2). Then,
\begin{align}
...
\ee
Define local $\omega$-frame operators. That is
\be
...
modes of a holomorphic or antiholomorphic
operator.
...
\ba
  ...
  &=\delta(\PP_1+\PP_2+\PP_3)\delta(\EE_1+\EE_2+\EE_3)\KK_{red}(n_L,n'_L)\KK_{red}(n_R,n'_R)
\end{align}
where
\ba
...
All the $n_L,n_R,n'_L,n'_R$ are nonnegative. So the minimum energies of the in-state and out-state are when $n_L=n_R=n'_L=n'_R=0$. Now define
 ...
 in-state) or $\PP'_3=s_3$ (for the out-state),
 ...
 excitation energy (in units of $\frac{1}{R}$
...
$\Delta_1+|\PP_1-s_1|$ (for the in-state), or $\Delta_3+|\PP'_3-s_3|$ (for the out-state).
 Then, for a given momentum-sector within a conformal family, the minimum energy is
$\Delta_1+|\PP_1-s_1|$ (for the in-state), or $\Delta_3+|\PP'_3-s_3|$ (for the out-state).
...
\end{align}
where $\hat{E}$ is the differential operator $\hat{E}\eq r\dv{r}$.
Next, define
\be
...
\begin{align}
  0&=[\hat{\varepsilon}^+_1-h_1]\YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|\hat{\varepsilon}^+_3,\hat{\varepsilon}^+_1,\hat{\varepsilon}^-_3,\hat{\varepsilon}^-_1]+[\hat{\varepsilon}^+_3-\hat{\varepsilon}^+_1+1-h_2]\YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|\hat{\varepsilon}^+_3,\hat{\varepsilon}^+_1-1,\hat{\varepsilon}^-_3,\hat{\varepsilon}^-_1]\notag\\
  &\hspace{3cm}+[-\hat{\varepsilon}_3+1-h_3]\YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|\hat{\varepsilon}^+_3-1,\hat{\varepsilon}^+_1-1,\hat{\varepsilon}^-_3,\hat{\varepsilon}^-_1]
\end{align}
...
First, fix $n_{3,R}=n_{1,R}=0$ and derive all $\YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|n_{3,L},n_{1,L},0,0]$ values using recursion relations (\ref{recursion1}) and (\ref{recursion2}). From (\ref{recursion1}) with $n_{1,R}=n_{3,R}=0$ and $n_{3,L}=-1$,
\be
...
And from (\ref{recursion2}) with $n_{1,R}=n_{3,R}=0$ and $n_{1,L}=-1$,
\be
...
Next, from (\ref{recursion1}) with $n_{1,R}=n_{3,R}=0$ and $n_{3,L}=0$,
\begin{align}
...
\end{align}
When $n_L=0$, the first term on the RHS is a quadratic polynomial in $h_{1,2,3}$, and the second term on the RHS is a linear polynomial in $h_{1,2,3}$, so the LHS is a quadratic polynomial in $h_{1,2,3}$.\par
When $n_L=1$, the first term on the RHS is a cubic polynomial in $h_{1,2,3}$, and the second term on the RHS is a quadratic polynomial in $h_{1,2,3}$, so the LHS is a cubic polynomial in $h_{1,2,3}$.\par
In this way, we can determine all of $ \YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|1,n_{1,L},0,0]$ for $n_{1,L}\geq0$. And $ \YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|1,n_{1,L},0,0]$ is a $n_{1,L}+1$-th polynomial in $h_{1,2,3}$. Similarly, we can determine all of $ \YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|n_{3,L},1,0,0]$ for $n_{3,L}\geq0$. And $ \YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|n_{3,L},1,0,0]$ is a polynomial in $h_{1,2,3}$.\par
...
The point is that all of  $\YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|n_{3,L},n_{1,L},n_{3,R},n_{1,R}]$ are determined by \par
\begin{itemize}
...
First, prepare the arbitrarily normalized initial value $\YY 3[h_{3,2,1},\tilde{h}_{3,2,1}|0,0,0,0]$, which is green triangle in the Fig.\ref{analytic_conti1}. From the unitarity condition, three-point functions at blue points vanish. At blue points, $n_{1,L}<0$ or $n_{3_L}<0$ are satisfied.\par
...
Recursion ralation (\ref{recursion1}) with $n_{1,R}=n_{3,R}=0$ and $n_{3,L}=0$ gives three-point functions at $n_{3,L}=1$ (Fig.\ref{analytic_conti4}). And three-point functions at $n_{1,L}=1$ can be gotten in the same way (Fig. \ref{analytic_conti5}).\par
...
\ba
  ...
  &=K\YY 3^{(chiral)}[h_{3,2,1}|n_{3,L},n_{1,L}]\YY 3^{(antichiral)}[\tilde{h}_{3,2,1}|n_{3,R},n_{1,R}]\notag
\end{align}
where $K$ is an unfixed constant related to the OPE coefficient. The chiral part for $n_{1,L},n_{3,L}\geq0$ is
\begin{align}
  &\YY 3^{(chiral)}[h_{3,2,1}|n_{3,L},n_{1,L}]=\frac{1}{\Gamma(b_1)\Gamma(b_2)\Gamma(b_3)}\notag\\
  &\times\sum_{q=0}^{\mathrm{Min}\{n_{1,L},n_{3,L}\}}\tfrac{\Gamma[b_3+q+\mathrm{Max}\{{0,n_{1,L}-n_{3,L}}\}]\Gamma[b_2-q+\mathrm{Min}\{{n_{1,L},n_{3,L}}\}]\Gamma[b_1+q+\mathrm{Max}\{{n_{3,L}-n_{1,L},0}\}]}{\Gamma[1+q+\mathrm{Max}\{{0,n_{1,L}-n_{3,L}}\}]\Gamma[1-q+\mathrm{Min}\{{n_{1,L},n_{3,L}}\}]\Gamma[1+q+\mathrm{Max}\{{n_{3,L}-n_{1,L},0}\}]}\label{Direct Integral for holo}
\end{align}
and $0$ for otherwise. And the antichiral part for $n_{1,R},n_{3,R}\geq0$ is
\begin{align}
  &\YY 3^{(antichiral)}[\tilde{h}_{3,2,1}|n_{3,R},n_{1,R}]=\frac{1}{\Gamma(\tilde{b}_1)\Gamma(\tilde{b}_2)\Gamma(\tilde{b}_3)}\notag\\
  &\times\sum_{q=0}^{\mathrm{Min}\{n_{1,R},n_{3,R}\}}\tfrac{\Gamma[\tilde{b}_3+q+\mathrm{Max}\{{0,n_{1,R}-n_{3,R}}\}]\Gamma[\tilde{b}_2-q+\mathrm{Min}\{{n_{1,R},n_{3,R}}\}]\Gamma[\tilde{b}_1+q+\mathrm{Max}\{{n_{3,R}-n_{1,R},0}\}]}{\Gamma[1+q+\mathrm{Max}\{{0,n_{1,R}-n_{3,R}}\}]\Gamma[1-q+\mathrm{Min}\{{n_{1,R},n_{3,R}}\}]\Gamma[1+q+\mathrm{Max}\{{n_{3,R}-n_{1,R},0}\}]}\label{Direct Integral for antiholo}
\end{align}
...
&\text{(B)}l_1+l_2+l_3 \text{\ \ is integers.} \notag\\
&\ \ \ \ \text{And moreover, an even integers if  } m_1=m_2=m_3=0\label{selection rule B}\\
...
&\hspace{5mm}\times\sqrt{(l_1+m_1)!(l_1-m_1)!(l_2+m_2)!(l_2-m_2)!(l_3+m_3)!(l_3-m_3)!}\notag\\
&\hspace{5mm}\times\sum_k\frac{(-1)^k}{k!(l_3-l_2+k+m_1)!(l_3-l_1+k-m_2)!(l_1+l_2-l_3-k)!(l_1-k-m_1)!(l_2-k+m_2)!}\notag
\end{align}
...
&\hspace{6cm}+\frac{2\sqrt{(l+m)(l-m+1)}}{r}\sqrt{\frac{\pi}{3}}\int\dd\Omega Y^*_{l,m-1}Y_{1,0}\OO&
\end{flalign}
\begin{flalign}
...
\end{align}
From (\ref{selection rule B}) and (\ref{selection rule D}), only $c=l\pm1$ suvive when $l\geq1$, and only $c=1$ survives when $l=0$. And from (\ref{selection rule A}), only $\gamma=-m$ survives.
\begin{multline}
...
\end{multline}
From (\ref{Pz Ward Identity}) and (\ref{Kz Ward Identity}),
\be
...
\ee
The last condition comes from $m_2=-m_1-m_3$ and $-l_2\leq m_2\leq l_2$. The area which satisies the above conditions is diamond-shaped as Fig. \ref{reduction1}.
\begin{figure}[tb]
  \begin{center}
...
Now that we know the strategy, all we have to do is solve the recursion relations. Applying the $J_x-iJ_y$ WI to the right boundary gives\par
\be
...
\ba
&F_{l_1-1,-l_3+k}\notag\\
&=\frac{(-1)^k}{\sqrt{2l_1}}\sqrt{\frac{(l_2-l_1+l_3)!(l_2+l_1-l_3+k-1)!(2l_3-k)!}{(l_2-l_1+l_3-k+1)!(l_2+l_1-l_3)!(2l_3)!k!}}[2l_1k-(l_2+l_1-l_3)(l_2-l_1+l_3+1)]F_{l_1,-l_3}^{l_1,l_2,l_3}
\end{align}
...
\end{align}
We prove it by mathematical induction. First, the above configuration is consistent with the result for $(n,k)=(n,0)$ shown in (\ref{f_{n,0}}). The recursion relations of $P_{n,k,l}$ for $k$ is
\begin{align}
...
\end{align}
Assume that the above configuration (\ref{assumption f_{n,k}}) and (\ref{assumption P_{n,k,l}}) is correct for $k\geq0$. Then,
from the above recursion relations, we get
\begin{align}
&P_{n,k+1,0}=1\\
...
\begin{cases}
0 & (p < k)\\
\sum_{n=0}^{p-k}\frac{2^{2n}(2p)!}{(p-k-n)!(2n)!}\frac{g_{12}!}{(g_{12}-p+k-n)!}\Phi_{12}^{2n} & (p \geq k)
\end{cases}
\ee
\be
...
\begin{cases}
0 & (p \leq k)\\
-\sum_{n=0}^{p-k-1}\frac{2^{2n+1}(2p)!}{(p-k-n-1)!(2n+1)!}\frac{g_{12}!}{(g_{12}-p+k-n)!}\Phi_{12}^{2n+1} & (p > k)
\end{cases}
\ee
With them, we get
\begin{align}
...
JHEP \textbf{10}, 125 (2018)
doi:10.1007/JHEP10(2018)125
\href{https://arxiv.org/abs/1807.07003}{[arXiv:1807.07003 [hep-th]]}.

\bibitem{Gillioz:2019lgs}
M.~Gillioz,
``Conformal 3-point functions and the Lorentzian OPE in momentum space,''
Commun. Math. Phys. \textbf{379}, no.1, 227-259 (2020)
doi:10.1007/s00220-020-03836-8
\href{https://arxiv.org/abs/1909.00878}{[arXiv:1909.00878 [hep-th]]}.
...
JHEP \textbf{03}, 102 (2020)
doi:10.1007/JHEP03(2020)102
\href{https://arxiv.org/abs/1912.05550}{[arXiv:1912.05550 [hep-th]]}.
...
\href{https://arxiv.org/abs/0807.0004}{[arXiv:0807.0004 [hep-th]]}.
...
\href{https://arxiv.org/abs/1203.6064}{[arXiv:1203.6064 [hep-th]]}.

\bibitem{El-Showk:2014dwa}
S.~El-Showk, M.~F.~Paulos, D.~Poland, S.~Rychkov, D.~Simmons-Duffin and A.~Vichi,
J. Stat. Phys. \textbf{157}, 869 (2014)
doi:10.1007/s10955-014-1042-7
\href{https://arxiv.org/abs/1403.4545}{[arXiv:1403.4545 [hep-th]]}.

\bibitem{Kos:2014bka}
F.~Kos, D.~Poland and D.~Simmons-Duffin,
JHEP \textbf{11}, 109 (2014) doi:10.1007/JHEP\\11(2014)109
\href{https://arxiv.org/abs/1406.4858}{[arXiv:1406.4858 [hep-th]]}.

\bibitem{Simmons-Duffin:2015qma}
D.~Simmons-Duffin,
JHEP \textbf{06}, 174 (2015)
doi:10.1007/JHEP06(2015)174
\href{https://arxiv.org/abs/1502.02033}{[arXiv:1502.02033 [hep-th]]}.

\bibitem{Kos:2013tga}
F.~Kos, D.~Poland and D.~Simmons-Duffin,
JHEP \textbf{06}, 091 (2014)
doi:10.1007/JHEP06\\(2014)091
\href{https://arxiv.org/abs/1307.6856}{[arXiv:1307.6856 [hep-th]]}.

\bibitem{Kos:2015mba}
F.~Kos, D.~Poland, D.~Simmons-Duffin and A.~Vichi,
JHEP \textbf{11}, 106 (2015)
doi:10.\\1007/JHEP11(2015)106
\href{https://arxiv.org/abs/1504.07997}{[arXiv:1504.07997 [hep-th]]}.

\bibitem{Kos:2016ysd}
F.~Kos, D.~Poland, D.~Simmons-Duffin and A.~Vichi,
JHEP \textbf{08}, 036 (2016)
doi:10.\\1007/JHEP08(2016)036
\href{https://arxiv.org/abs/1603.04436}{[arXiv:1603.04436 [hep-th]]}.

\bibitem{Nakayama:2014yia}
Y.~Nakayama and T.~Ohtsuki,
Phys. Lett. B \textbf{734}, 193-197 (2014)
doi:10.1016/j.phys\\letb.2014.05.058
\href{https://arxiv.org/abs/1404.5201}{[arXiv:1404.5201 [hep-th]]}.

\bibitem{Poland:2016chs}
D.~Poland and D.~Simmons-Duffin,
Nature Phys. \textbf{12}, no.6, 535-539 (2016)
doi:10.1038/\\nphys3761
...
\href{https://arxiv.org/abs/1805.04405}{[arXiv:1805.04405 [hep-th]]}.

\bibitem{Chester:2019wfx}
S.~M.~Chester,
\href{https://arxiv.org/abs/1907.05147}{[arXiv:1907.05147 [hep-th]]}.

\bibitem{Maldacena:1997re}
J.~M.~Maldacena,
Adv. Theor. Math. Phys. \textbf{2}, 231-252 (1998)
doi:10.1023/A:1026654\\312961
\href{https://arxiv.org/abs/hep-th/9711200}{[arXiv:hep-th/9711200 [hep-th]]}.

\bibitem{Gubser:1998bc}
S.~S.~Gubser, I.~R.~Klebanov and A.~M.~Polyakov,
Phys. Lett. B \textbf{428}, 105-114 (1998)
doi:10.1016/S0370-2693(98)00377-3
\href{https://arxiv.org/abs/hep-th/9802109}{[arXiv:hep-th/9802109 [hep-th]]}.

\bibitem{Witten:1998qj}
E.~Witten,
Adv. Theor. Math. Phys. \textbf{2}, 253-291 (1998)
doi:10.4310/ATMP.1998.v2.n2.\\a2
\href{https://arxiv.org/abs/hep-th/9802150}{[arXiv:hep-th/9802150 [hep-th]]}.

\bibitem{Heemskerk:2009pn}
I.~Heemskerk, J.~Penedones, J.~Polchinski and J.~Sully,
JHEP \textbf{10}, 079 (2009)
doi:10.1088/1126-6708/2009/10/079
\href{https://arxiv.org/abs/0907.0151}{[arXiv:0907.0151 [hep-th]]}.

\bibitem{Aharony:2016dwx}
O.~Aharony, L.~F.~Alday, A.~Bissi and E.~Perlmutter,
JHEP \textbf{07}, 036 (2017)
doi:10.1007/JHEP07(2017)036
\href{https://arxiv.org/abs/1612.03891}{[arXiv:1612.03891 [hep-th]]}.

\bibitem{Aprile:2017bgs}
F.~Aprile, J.~M.~Drummond, P.~Heslop and H.~Paul,
JHEP \textbf{01}, 035 (2018)
doi:10.1007/JHEP01(2018)035
\href{https://arxiv.org/abs/1706.02822}{[arXiv:1706.02822 [hep-th]]}.

\bibitem{Alday:2017vkk}
L.~F.~Alday and S.~Caron-Huot,
JHEP \textbf{12}, 017 (2018)
doi:10.1007/JHEP12(2018)017
\href{https://arxiv.org/abs/1711.02031}{[arXiv:1711.02031 [hep-th]]}.

\bibitem{Alday:2017xua}
L.~F.~Alday and A.~Bissi,
Phys. Rev. Lett. \textbf{119}, no.17, 171601 (2017)
doi:10.1103/\\PhysRevLett.119.171601
\href{https://arxiv.org/abs/1706.02388}{[arXiv:1706.02388 [hep-th]]}.

\bibitem{Rastelli:2016nze}
L.~Rastelli and X.~Zhou,
Phys. Rev. Lett. \textbf{118}, no.9, 091602 (2017)
doi:10.1103/Phys\\RevLett.118.091602
\href{https://arxiv.org/abs/1608.06624}{[arXiv:1608.06624 [hep-th]]}.

\bibitem{Rastelli:2017udc}
L.~Rastelli and X.~Zhou,
JHEP \textbf{04}, 014 (2018)
doi:10.1007/JHEP04(2018)014
\href{https://arxiv.org/abs/1710.05923}{[arXiv:1710.05923 [hep-th]]}.

\bibitem{Caron-Huot:2018kta}
S.~Caron-Huot and A.~K.~Trinh,
JHEP \textbf{01}, 196 (2019)
doi:10.1007/JHEP01(2019)196
\href{https://arxiv.org/abs/1809.09173}{[arXiv:1809.09173 [hep-th]]}.

\bibitem{Vafa:2005ui}
C.~Vafa,
\href{https://arxiv.org/abs/hep-th/0509212}{[arXiv:hep-th/0509212 [hep-th]]}.

\bibitem{Arkani-Hamed:2006emk}
N.~Arkani-Hamed, L.~Motl, A.~Nicolis and C.~Vafa,
JHEP \textbf{06}, 060 (2007)
doi:10.1088\\/1126-6708/2007/06/060
\href{https://arxiv.org/abs/hep-th/0601001}{[arXiv:hep-th/0601001 [hep-th]]}.

\bibitem{Palti:2019pca}
E.~Palti,
Fortsch. Phys. \textbf{67}, no.6, 1900037 (2019)
doi:10.1002/prop.201900037
\href{https://arxiv.org/abs/1903.06239}{[arXiv:1903.06239 [hep-th]]}.

\bibitem{Coriano:2013jba}
C.~Coriano, L.~Delle Rose, E.~Mottola and M.~Serino,
JHEP \textbf{07}, 011 (2013)
doi:10.1007/JHEP07(2013)011
\href{https://arxiv.org/abs/1304.6944}{[arXiv:1304.6944 [hep-th]]}.

\bibitem{Bzowski:2013sza}
A.~Bzowski, P.~McFadden and K.~Skenderis,
JHEP \textbf{03}, 111 (2014)
doi:10.1007/JHEP\\03(2014)111
\href{https://arxiv.org/abs/1304.7760}{[arXiv:1304.7760 [hep-th]]}.

\bibitem{Isono:2019ihz}
H.~Isono, T.~Noumi and T.~Takeuchi,
JHEP \textbf{05}, 057 (2019)
doi:10.1007/JHEP05(2019)\\057
\href{https://arxiv.org/abs/1903.01110}{[arXiv:1903.01110 [hep-th]]}.

\bibitem{Bautista:2019qxj}
T.~Bautista and H.~Godazgar,
JHEP \textbf{01}, 142 (2020)
doi:10.1007/JHEP01(2020)142
\href{https://arxiv.org/abs/1908.04733}{[arXiv:1908.04733 [hep-th]]}.

\bibitem{Giannotti:2008cv}
M.~Giannotti and E.~Mottola,
Phys. Rev. D \textbf{79}, 045014 (2009)
doi:10.1103/PhysRevD.\\79.045014
\href{https://arxiv.org/abs/0812.0351}{[arXiv:0812.0351 [hep-th]]}.

\bibitem{Mottola:2010gp}
E.~Mottola,
Acta Phys. Polon. B \textbf{41}, 2031-2162 (2010)
\href{https://arxiv.org/abs/1008.5006}{[arXiv:1008.5006 [gr-qc]]}.

\bibitem{Armillis:2009pq}
R.~Armillis, C.~Coriano and L.~Delle Rose,
Phys. Rev. D \textbf{81}, 085001 (2010)
doi:10.1103/PhysRevD.81.085001
\href{https://arxiv.org/abs/0910.3381}{[arXiv:0910.3381 [hep-ph]]}.

\bibitem{Armillis:2009im}
R.~Armillis, C.~Coriano and L.~Delle Rose,
Phys. Lett. B \textbf{682}, 322-327 (2009)
doi:10.1016/j.physletb.2009.11.013
\href{https://arxiv.org/abs/0909.4522}{[arXiv:0909.4522 [hep-ph]]}.

\bibitem{Armillis:2010qk}
R.~Armillis, C.~Coriano and L.~Delle Rose,
Phys. Rev. D \textbf{82}, 064023 (2010)
doi:10.1103/PhysRevD.82.064023
\href{https://arxiv.org/abs/1005.4173}{[arXiv:1005.4173 [hep-ph]]}.

\bibitem{Coriano:2011ti}
C.~Coriano, L.~Delle Rose, A.~Quintavalle and M.~Serino,
Phys. Lett. B \textbf{700}, 29-38 (2011)
doi:10.1016/j.physletb.2011.04.053
\href{https://arxiv.org/abs/1101.1624}{[arXiv:1101.1624 [hep-ph]]}.

\bibitem{Coriano:2011zk}
C.~Coriano, L.~Delle Rose and M.~Serino,
Phys. Rev. D \textbf{83}, 125028 (2011)
doi:10.1103/PhysRevD.83.125028
\href{https://arxiv.org/abs/1102.4558}{[arXiv:1102.4558 [hep-ph]]}.

\bibitem{Antoniadis:1996dj}
I.~Antoniadis, P.~O.~Mazur and E.~Mottola,
Phys. Rev. Lett. \textbf{79}, 14-17 (1997)
doi:10.1103/PhysRevLett.79.14
\href{https://arxiv.org/abs/astro-ph/9611208}{[arXiv:astro-ph/9611208 [astro-ph]]}.

\bibitem{Antoniadis:2011ib}
I.~Antoniadis, P.~O.~Mazur and E.~Mottola,
JCAP \textbf{09}, 024 (2012)
doi:10.1088/1475-7516/2012/09/024
\href{https://arxiv.org/abs/1103.4164}{[arXiv:1103.4164 [gr-qc]]}.

\bibitem{Maldacena:2011nz}
J.~M.~Maldacena and G.~L.~Pimentel,
JHEP \textbf{09}, 045 (2011)
doi:10.1007/JHEP09\\(2011)045
\href{https://arxiv.org/abs/1104.2846}{[arXiv:1104.2846 [hep-th]]}.

\bibitem{Kehagias:2012pd}
A.~Kehagias and A.~Riotto,
Nucl. Phys. B \textbf{864}, 492-529 (2012)
doi:10.1016/j.nuclphys\\b.2012.07.004
\href{https://arxiv.org/abs/1205.1523}{[arXiv:1205.1523 [hep-th]]}.

\bibitem{Gillioz:2021sce}
M.~Gillioz,
``From Schwinger to Wightman: all conformal 3-point functions in momentum space,''
\href{https://arxiv.org/abs/2109.15140}{[arXiv:2109.15140 [hep-th]]}.

\bibitem{Gillioz:2016jnn}
M.~Gillioz, X.~Lu and M.~A.~Luty,
JHEP \textbf{04}, 171 (2017)
doi:10.1007/JHEP04(2017)171
\href{https://arxiv.org/abs/1612.07800}{[arXiv:1612.07800 [hep-th]]}.

\bibitem{Gillioz:2018kwh}
M.~Gillioz, X.~Lu and M.~A.~Luty,
JHEP \textbf{09}, 025 (2018)
doi:10.1007/JHEP09(2018)025
\href{https://arxiv.org/abs/1801.05807}{[arXiv:1801.05807 [hep-th]]}.

\bibitem{Karateev:2018oml}
D.~Karateev, P.~Kravchuk and D.~Simmons-Duffin,
JHEP \textbf{10}, 217 (2019)
doi:10.1007/\\JHEP10(2019)217
\href{https://arxiv.org/abs/1809.05111}{[arXiv:1809.05111 [hep-th]]}.

\bibitem{Zamolodchikov:1986db}
A.~B.~Zamolodchikov,
\href{https://www.osti.gov/biblio/6647980}{Sov. J. Nucl. Phys. \textbf{44}, 529-533 (1986)}

\bibitem{KacLecturenote}
Kac V G, Raina A K and Rozhkovskaya N 2013 Bombay lectures on highest weight representations of infinite dimensional Lie algebras 2nd ed (Advanced Series in Mathematical Physics vol 29) (World Scientific Publishing Co. Pte. Ltd., Hackensack, NJ) ISBN 978-981-4522-19-9

\bibitem{Nagano:2021tbu}
L.~Nagano and S.~Terashima,
JHEP \textbf{09}, 187 (2021)
doi:10.1007/JHEP09(2021)187
\href{https://arxiv.org/abs/2101.04090}{[arXiv:2101.04090 [hep-th]]}.

\bibitem{Besken:2020snx}
M.~Be\c{s}ken, J.~De Boer and G.~Mathys,
JHEP \textbf{21}, 148 (2020)
doi:10.1007/JHEP07\\(2021)148
\href{https://arxiv.org/abs/2012.15724}{[arXiv:2012.15724 [hep-th]]}.
