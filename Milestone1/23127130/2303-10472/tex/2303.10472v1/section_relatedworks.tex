...
However, most guarantees so far have relied on strong assumptions such as that the log-likelihood is Lipschitz~\citep{cherief-abdellatif_generalization_2019,alquier_nonexponentially_2021}, that the gradient variance is bounded by constant~\citep{liu_quasimonte_2021,buchholz_quasimonte_2018,domke_provable_2020}, and that the support of \(q_{\vlambda}\) is bounded~\citep{fujisawa_multilevel_2021}.
Our result shows that similar results could be obtained under a realistic, relaxed gradient assumption.
Meanwhile,~\citet{bhatia_statistical_2022} recently provided a full complexity guarantee for a BBVI algorithm under milder assumptions, but their algorithm only optimizes the scale matrix \(\mC\).
...
\citeauthor{domke_provable_2019}'s result can also be seen as a direct generalization of the results of~\citet{xu_variance_2019}, which are restricted to quadratic log-likelihoods and the mean-field variational family.
