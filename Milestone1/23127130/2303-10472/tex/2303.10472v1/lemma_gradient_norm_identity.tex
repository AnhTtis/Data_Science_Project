...
\begin{theoremEnd}[\lemmaproofoption,category=upperboundkeylemmagradientnormidentity]{lemma}\label{thm:general_variational_gradient_norm_identity}
...
\begin{proofEnd}
  ...
  \begin{alignat}{2}
    \frac{\partial \vt_{\vlambda}\left(\vu\right) }{ \partial m_i }   &= \ve_i \quad
    \frac{\partial \vt_{\vlambda}\left(\vu\right) }{ \partial C_{ij} } &= \ve_i u_j,\label{eq:covariance_derivative}
  \end{alignat}
  where \(\ve_i\) is the unit basis of the \(i\)th component.
  ...
  \begin{alignat*}{2}
    ...
    \right).
  \end{alignat*}
  The major difference with the proof of \citet[Lemma 8]{domke_provable_2019} for the matrix square root case is that we only sum the \(u_j^2 \ve_i \ve_i^{\top}\) terms over the \textit{lower diagonal elements}.
