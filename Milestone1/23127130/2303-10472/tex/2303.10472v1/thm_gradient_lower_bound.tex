...
Let \(\vg_{M}\) be an \(M\)-sample estimator of the gradient of the ELBO in either the entropy- or KL-regularized form.
Also, let ~\cref{assumption:q} hold where the matrix square root parameterization is used.
Then, for all \(L\)-smooth and \(\mu\)-strongly convex functions \(f\) such that $\nicefrac{L}{\mu} < \sqrt{d + 1}$, the variance of \(\vg_{M}\) is bounded below by some strictly positive constant as
\begin{align*}
  \mathbb{E}\norm{\rvvg}_2^2
  &\geq
...
\begin{proofsketch}
  ...
  \begin{align*}
    ...
    \right),
  \end{align*}
  where \(\bar{\vlambda} = \left(\bar{\vzeta}, \mO\right)\).
