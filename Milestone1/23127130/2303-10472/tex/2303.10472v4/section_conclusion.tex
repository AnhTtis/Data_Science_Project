...
Also, our bounds corresponds to the ABC condition (\cref{section:abc}) and the \textit{expected residual} (ER) condition, where the latter is a special case of the former with \(B=1\).
The ER condition has been used by \citet{gower_sgd_2021} for proving convergence of SGD on quasar convex functions, which generalize convex functions.
The results of this paper are used by~\citet{kim_blackbox_2023} to establish convergence of BBVI through the results of~\citet{khaled_better_2023}.
