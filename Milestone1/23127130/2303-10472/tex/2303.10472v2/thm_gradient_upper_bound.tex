...
\begin{proofsketch}
  ...
  The proof completes by \ding{185} applying the quadratic growth assumption to relate the parameter distance with the function suboptimality gap, and \ding{186} upper bounding the KL regularizer term.
\end{proofsketch}
\vspace{-2ex}
\begin{proofEnd}
...
\shortintertext{where by \cref{thm:reparam_quadratic},}
  ...
  As mentioned in the proof sketch, to preserve generality, it is necessary to convert the entropy-regularized form into the KL-regularized form through the following inequality:
  {%
  \setlength{\belowdisplayskip}{1ex} \setlength{\belowdisplayshortskip}{1ex}%
  \setlength{\abovedisplayskip}{1ex} \setlength{\abovedisplayshortskip}{1ex}%
  \begin{alignat*}{2}
