...
% causal discovery for MTS  (Gongchang)
MTS data, describing the calibrated states of multiple variables changing over time, is a general kind of temporal data in many domains. Discovering causal relations from MTS could be beneficial to the explainability and robustness of data analysis models. However, the definitions of causal relations are not unique, leading to different solutions.
% Inferring such relations to build a causal graph, \ie causal discovery in MTS, would provide more explainability and robustness for model design. 
Accordingly, existing works can be grouped into four categories, \ie, constraint-based methods, score-based methods, functional causal model (FCM)-based methods and Granger causality-based methods. Besides, there also exist some perspectives such as Takens' causality and differential equations. In this paper, we will specify the main idea and recent advances for each category.



% causal discovery in event streams (Chuzhe)
...
% 是否需要一段讲new perspectives呢 ?



% paragraph 5: related surveys (table)
% Although many surveys on causal discovery\cite{} have been published, most of them focus on the image\cite{} or textual data\cite{}, and do not discuss the temporal data. In addition, there exist many surveys on different tasks for temporal data analysis, such as time series forecasting and classification, but only a few works\cite{} including temporal data causal discovery. Among them, the recent survey \cite{intro/ts_surveys/AssaadDG22} discussing the existing solutions of time series causal discovery, is the most related work to this paper. Nevertheless, causal discovery methods for event stream are ignored in \cite{intro/ts_surveys/AssaadDG22}. In this paper, we not only provide a thoughtful overview of causal discovery methods of the two kinds of temporal data but also give an analysis of the connections and differences between them.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Related surveys 施工现场
...
Recently, many surveys \cite{intro/nonts_surveys/glymour2019review, intro/surveys_guoruocheng/csur/GuoCLH020, intro/nonts_surveys/Dya21, intro/nonts_surveys/XJTU22, intro/ts_surveys/Moraffah21, intro/ts_surveys/AliGranger21, intro/ts_surveys/AssaadDG22, intro/nonts_surveys/BN21, intro/DL_surveys/corr/abs-2211-03374, intro/surveys/CSL/heinze2018causal} have been published to summarize the progress of causal discovery. 
We compared the representative reviews and their highlight points in Table \ref{tab:surveys_overview}. As shown, these surveys fall into two lines. Research works in the first line \cite{intro/nonts_surveys/glymour2019review, intro/nonts_surveys/Dya21, intro/surveys_guoruocheng/csur/GuoCLH020, intro/nonts_surveys/XJTU22} discuss the general causal discovery problem in different perspectives. For example,
