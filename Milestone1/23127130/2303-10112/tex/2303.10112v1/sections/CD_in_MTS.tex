...
\section{Causal Discovery in Multivariate Time Series}\label{sec:mts}
...
Here, MCI is a measurement, which conditions on the parents of $x^t_j$ and $x^{t-k}_i$ while testing $x^{t-k}_i \to x^t_j$, i.e., $\mathrm{MCI} (x^{t-k}_i ; x^t_j) = m(x^{t-k}_i ; x^t_j | Pa(x^t_j) \textbackslash \{ x^{t-k}_i\}, x^{t-k}_j  )   )  $, where $m$ is a measure for dependence. It also provides an interpretable notion of causal strength from $x^{t-k}_i$ to $x^t_j$.
...
Then considering every component of the transformed vector as a separate random variable, the original FCI algorithm is directly applicable. As the amount of information derived from standard FCI is quite restricted, the temporal priority is further incorporated as background knowledge to make more inferences about the existence or non-existence of causal connections in the orientation phase.
...
Similar to tsFCI, it also uses stationary to infer additional edge removals.
...
It inputs time-series data and outputs either a summary time graph or remains undecided. In particular, it outputs "I do not know" if no model with independent residuals is found. This feature, to some extent, avoids leading to wrong causal conclusions when the data is insufficient or the model is mis-specified.
...
\label{subsection:Granger_early} 


Earlier methods for identifying Granger causality were limited to bivariate settings. 
Specifically, a well-documented \cite{lutkepohl1982non} issues for Granger causal analysis in bivariate settings is that the causal findings may be misleading without adjusting for all relevant covariates.   
...
\begin{equation}
   \mathrm{CE}(X_t^{\mathbf{I}} \to X_{t+1}^{\mathbf{J}} | X_t^{\mathbf{C}} )  = h(X_{t+1}^{\mathbf{J}} | X_t^{\mathbf{C}}  ) - h(  X_{t+1}^{\mathbf{J}} | X_t^{\mathbf{C}}, X_t^{\mathbf{I}} ), \nonumber
\end{equation}
...
There is abundant literature \cite{intro/ts_surveys/peters2022causal, bongers2018causal} \cite{DBLP:journals/corr/abs-1911-10500, DBLP:conf/uai/MooijJS13,DBLP:conf/uai/RubensteinBMS18} discussing the relationship between differential equations and structural causal models.  
...
This methodology has also applications in fields \cite{MTS/KleinbergS/kleinberg2013causality}. 
