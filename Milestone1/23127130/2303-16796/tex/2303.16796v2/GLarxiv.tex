...
\begin{equation}
    M^* = \argmin_M L(M),
\end{equation}
and $M^*$ is in this sense the best partition of the network. This approach is widely used and recognized as one of the best methods for detecting network communities \cite{Lancichinetti}. To connect Infomap with the GLASSO and thereby integrating modular structure with the GLASSO regularization we suggest using a cross-validation procedure to find the best regularization parameter $\lambda^*$ that maximizes the signal of modular structure present in both the training and test sets. We measure this signal using the code length savings in the test data given the optimal partition of the training data, such that
\begin{equation}
