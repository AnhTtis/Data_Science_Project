...
\leftline{\small{\it Date.\/} March 22, 2023. Final version, accepted in 
Annales de l'Institut Henri Poincar\'e, January 15, 2024.}
...
To lighten the notation, we will from now on simply write $K$ instead of 
...
\label{eq:approx_MC} 
 ...
 + \varrho^{nm}}
\end{equation} 
for all $n\in\N$ and all $j\in\set{1,\dots N}$. Here $\Hhat_{\min}$ is a constant satisfying  $\Hhat_{\min} \geqs H_0 - (N-1)\theta$, and $(Y_n)_{n\geqs0}$ is 
a Markov chain with transition matrix $P$, whose matrix elements satisfy 
\begin{equation}
\label{eq:def_Pij} 
 P_{ij} = 
...
Furthermore, these matrix elements satisfy
\begin{equation} 
\label{eq:ldp_Pij} 
\e^{-[H(i,j)-p(i,j)\theta +\eta]/\sigma^2}
 \leqs P_{ij}
 \leqs \e^{-[H(i,j)-p(i,j)\theta-\eta]/\sigma^2}\;,
\end{equation}
where the $p(i,j)$ are integers in $\set{0,\dots,N-1}$.  
\end{theorem}

We give the proof in Section~\ref{sec:proof_reduction}, which also contains more precise information on the constants $\theta_0$ and $\Hhat_{\min}$. 

Note that the result applies to time-diluted versions of the trace process on $\MN$, by a factor $m$ scaling like $\e^{\theta/\sigma^2}$. The reason for the time dilution is that it gives the process a chance to approach the local QSD once it has reached one of the $B_i$, before jumping to another $B_j$.

Relation~\eqref{eq:ldp_Pij} is a direct consequence of the large-deviation 
principle, where $p(i,j)$ represents the length of the optimal path from $i$ to $j$, see 
Lemma~\ref{lem:ldp_msigma} below. 
The main idea of the proof is to construct bases $(\mu_i)_{1\leqs i\leqs N}$ and $(\psi_j)_{1\leqs j\leqs N}$ of the $N$-dimensional vector spaces that are respectively left-invariant and 
right-invariant under the truncated kernel. The basis functions satisfy the orthonormality 
relation $\pscal{\mu_i}{\psi_j} = \delta_{ij}$. Most importantly, Theorem~\ref{thm:approx} below provides the relation
\begin{equation}
\label{eq:Pi_Ynj} 
 \bigprobin{i}{Y_n = j}
 = \int_{\MN} \int_{\MN} 
 \mu_i(x) \Bigprobin{x}{X_{\tau^{+,nm}_{\MN}} \in \6y} \psi_j(y) \6x 
 =: \Bigexpecin{\mu_i}{\psi_j \pth{X_{\tau^{+,nm}_{\MN}}}}\;,
\end{equation} 
where $\tau^{+,n}_{\MN}$ is the $n$th return time to the metastable set $\MN$. 
We emphasize that the measure $\mu_i$ in~\eqref{eq:Pi_Ynj} is in general a signed measure, because it is a linear combination of eigenfunctions of the truncated kernel. Therefore, the expectation in~\eqref{eq:Pi_Ynj} should be interpreted as a linear combination of expectations with 
respect to the positive and negative parts of $\mu_i$.
The functions $\psi_j$ can also have negative values, though they are close to indicator functions of $B_j$. See Proposition~\ref{prop:norms_mupsi} for details. However, the $P_{ij}$ are indeed positive for $\sigma$ small enough, and $P$ is a stochastic matrix, as proved in Lemma~\ref{lem:P_stoch}.

Relation~\eqref{eq:Pi_Ynj} shows in which way the original and reduced process are coupled. In fact, there exists a linear map $\cL$ from the space of measures on $\X$ to those on $\set{1,\dots,N}$ such that 
\begin{equation}
...
\begin{equation}
 X_{n+1} = \Pi(X_n) + \sigma \xi_{n+1}\;,
\end{equation} 
where $\Pi:\R^d\to\R^d$ satisfies \ref{ass:DET}, and the $\xi_n$ are i.i.d.\ 
...
of attraction, and their stable and unstable manifolds. However they are known 
...
&= \norm{\Pi(x)}^2 + 2 \sigma\pscal{\Pi(x)}{\expec{\xi_1}} 
+ \sigma^2 \expec{\norm{\xi_1}^2}\;.
\end{align}
...
the coupling argument in Proposition~\ref{prop:coupling} shows that the positivity 
...
%
\begin{proof}
See Appendix~\ref{ssec:proof_pos_gaussian_map}. 
\end{proof}

Recall that Proposition~\ref{prop:ldp_EMN} shows that $E_\X(\sigma)$ is 
...
%
\begin{proof}
See Appendix~\ref{ssec:proof_EMN_gaussian}. 
\end{proof}
...
argument of Proposition~\ref{prop:coupling}. Instead of using Harnack inequalities for the 
...
We also have an analogue of Proposition~\ref{prop:EMN_gaussian} on the expected hitting 
...
arguments as in the proof of Proposition~\ref{prop:EMN_gaussian} show that the absence of 
...
which is possible by \Cref{ass:POS} and Proposition~\ref{prop:ldp_EMN}.
...
\begin{remark}
Corollary~\ref{cor:epsij} shows in particular that the assumption that 
...
Corollary~\ref{cor:epsij}, writing for $x\in B_i$ 
\begin{align}
  \psi_j(x)-\delta_{ij} 
...
on densities, than in Corollary~\ref{cor:eps_ij}. Indeed, we have 
\begin{align}
\bra{\mu_i} - \bra{\QSD{i}}
...
\end{equation} 
instead of an integral estimate as in Corollary~\ref{cor:eps_ij}.
\end{remark}
...
\end{equation} 
Corollary~\ref{cor:matrix_element_K0m} shows that 
\begin{equation}
\label{eq:P_ij} 
 P_{ij} = 
 \bigprobin{\QSD{i}}{X_{\tau^{+,m}_{\MN}}\in B_j}
...
= 1\;. 
\end{equation}
Furthermore, \eqref{eq:P_ij} implies that the $P_{ij}$ are positive 
if $\sigma$ is small enough. Note that, as shown in Corollary~\ref{cor:matrix_element_K0m}, 
this is a consequence of the fact that $m$ diverges as $\sigma$ goes to $0$, owing to~\eqref{eq:m_ldp}.
\end{proof}
...
Proposition~\ref{prop:norms_mupsi} yields 
\begin{equation}
...
while the other terms are exponentially small by Proposition~\ref{prop:norms_mupsi}.
\end{proof}
...
\subsection{Proof of Proposition~\ref{prop:pos_gaussian_map}} 
...
and observe that Proposition~\ref{prop:ldp_committor} yields 
\begin{equation}
...
\subsection{Proof of Proposition~\ref{prop:EMN_gaussian}} 
...
Proposition~\ref{lem:stable} with $h=\delta/2$. 
\end{proof}
...
\end{align} 
Applying Proposition~\ref{lem:three_set_prob} with $A=\cS$ and $B=\cK$ yields the claimed 
result with $\cK\cup\cS$ instead of $\cU$. The result can be extended to $\cU$ 
by showing that Proposition~\ref{lem:exit_S} also applies to the larger set 
...
with $\delta$ small enough for Proposition~\ref{lem:exit_U} to apply. We denote the union 
...
$\cA_i$ contains no fixed points of $\Pi$, showing, by Proposition~\ref{lem:stable}, that 
$\smash{\bigprobin{\cA_i}{\tau^+_{\cA_i^c} \geqs n}}$ is exponentially small 
for some bounded $n$. Furthermore, the proof of 
Proposition~\ref{lem:exit_U}, in particular~\eqref{eq:proof_bound_committor}, shows that 
\begin{equation}
...
cycles). Thus Proposition~\ref{lem:three_set_prob} shows that there exist
...
Proposition~\ref{lem:stable} implies that whenever $i\prec j$ or $i$ and $j$ are not 
...
Repeated application of Proposition~\ref{lem:three_set_prob} shows that, if 
...
In $\X\setminus\hat\cU$, we can apply Corollary~\ref{cor:stable} with a uniformly 
bounded $n(x)$, which finishes the proof, applying one last time 
Proposition~\ref{lem:three_set_prob}.
\qed
