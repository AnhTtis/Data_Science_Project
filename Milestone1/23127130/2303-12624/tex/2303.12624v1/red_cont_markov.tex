...
% \newcommand{\endexample}{~$\blacklozenge$}
% \newcommand{\endremark}{~$\lozenge$}
% \newcommand{\endassumption}{~$\clubsuit$}
...
% \newcommand{\Ku}{K^{u}}
% \newcommand{\meta}[1]{\mathcal{M}_{#1}}
% 
...
\leftline{\small{\it Date.\/} March 22, 2023.}
...
Because of their importance in simulation algorithms in molecular dynamics, these results prompted a series of works aiming at obtaining precise descriptions of the exit location of solutions of SDEs from metastable sets, see in particular the works by Di Ges\`u, Leli\`evre, Le Peutrec and Nectoux \cite{GLPN19,GLPN20a,LPN22}. These authors also emphasized the importance of quasistationary distributions (QSDs) in metastable states for the reduction problem~\cite{GLPN_2016}. 
...
% For $k\in\set{1,\dots,N}$, we will sometimes also use the set 
% \begin{equation}
%  \cM_k = \bigcup_{j=1}^k B_j\;,
% \end{equation} 
% where the $B_j$ will be ordered in a convenient way, defined below. 
...
markovian kernel. Since $A$ is recurrent, it is also a stochastic kernel on 
...
then the kernel $K$ has exactly $N$ eigenvalues outside the disc $\setsuch{\lambda\in\C}{\abs{\lambda} > \varrho}$, where 
\begin{equation}
...
One consequence of Proposition~\ref{prop:spectral_gap} is that the variables of the sequence 
$(X_n)_{n\geqs0}$ will be at distance decreasing like $\varrho^n$ from a sequence 
$((\trunc{X})_n)_{n\geqs0}$, generated by the truncated kernel $\trunc{K}$, obtained by projecting $K$ on its $N$ largest eigenvalues. This truncated kernel is given, in the basis of eigenfunctions of $K$, by a matrix of size $N$. However, it is not immediatly clear how this approximate sequence relates to the sequence of visited $B_i$. The following approximation result clarifies that point. 
...
In this section, we give the proof of the spectral gap result given in 
...
\end{equation} 
Note that Proposition~\ref{prop:ldp_EMN} implies that for any $\eta > 0$, there exists a 
...
\begin{align}
 ...
 - \bigprobin{x}{\tau_{\MN\setminus B_i} \leqs n_0(\sigma)}}
...
Since $\delta = 1 - \frac12 L > 0$, we may define 
\begin{equation}
 m 
...
$\bigpar{\frac12}^{1/m_0}$. It follows that $K^m$ and $(K^u)^m$ have the same number of eigenvalues outside a disc of radius $\bigpar{\frac12}^{m/m_0}$. The choice~\eqref{eq:choice_m} of $m$ implies that this disc does not intersect that disc of radius $r$ centred in $1$, which concludes the proof. 
\qed
...
\end{equation} 
We write $\gamma: i\to j$ to if $\gamma_0 = i$ and $\gamma_p = j$. 
...
%
\begin{proof}
To show~\eqref{eq:ldp_taum}, it suffices to recall that all optimal paths have a length bounded by $N$. Therefore, the binomial coefficient $\smash{\binom{n}{p}}$ is logarithmically equivalent to $m^p$, so that the sum over optimal paths yields a prefactor equivalent to $\e^{p\theta/\sigma^2}$. 
...
\end{equation} 
for the spectral gap of the trace process killed upon leaving $B_i$, where $\nextev{i}$ is the next-to-leading eigenvalue of the trace process killed upon leaving $B_i$.
...
&\leqs \sum_{k=n_1+1}^m Q_k(x) + \sum_{k=1}^{n_1} Q_k(x) \\
&\leqs \sum_{k=n_1+1}^m Q_k(x) \biggbrak{1+ \frac{m_1}{m-m1} 
\frac{\e^{2\eta/\sigma^2}}{\bigbrak{1-\e^{-(H_0-\eta)/\sigma^2}}^{n-1}}}\;.
\end{align}
...
\end{equation} 
Collecting terms and bounding $R$ as in the proof of 
...
%
\begin{proof}
Let us consider again the case $p=1$ first. 
...
\label{eq:bound_sgap} 
\specgap{i}^{m_1} \leqs
 \bigabs{\nextev{i}}^{m_1} \leqs \e^{-H_0/\sigma^2}\;.
\end{equation} 
...
proof only requires $B\cap C=\emptyset$). Taking 
$A=\X$, $B=\MN$ and $C=\X\setminus\MN$ and rearranging, we obtain 
\begin{equation}
...
\label{eq:proof_EXMN_n0} 
 ...
 \leqs \frac{n_0\bigexpecin{\X}{\tau^+_{\X}}}
...
% \subsection{A coupling argument}
% \label{ssec:proof_coupling} 
...
\sup_{x\in B_i}\ptrace{k_{\sigma,B_i}}{\cM_k}(x,y) 
&\leqs \e^{C/\sigma^2} 
\inf_{x\in B_i}\ptrace{k_{\sigma,B_i}}{\cM_k}(x,y) \\
\sup_{x\in B_i \colon \norm{x-x_0} \leqs r\sigma^2}
\ptrace{k_{\sigma,B_i}}{\cM_k}(x,y) 
&\leqs (1 + \eta) 
\inf_{x\in B_i \colon \norm{x-x_0} \leqs r\sigma^2}
\ptrace{k_{\sigma,B_i}}{\cM_k}(x,y)
\label{eq:proof_pos_02} 
%  \sup_{y\in B_i} 
%  \Biggpar{\dfrac{\sup_{x\in B_i}\ptrace{k_{\sigma,B_i}}{\cM_k}(x,y)}
%  {\inf_{x\in B_i}\ptrace{k_{\sigma,B_i}}{\cM_k}(x,y)}}
%  \leqs \e^{C/\sigma^2}\;.
\end{align}
...
In order to extend this to $\ptrace{k_\sigma}{\cM_k}$, we use the fact that 
...
this shows that $\ptrace{k_\sigma}{\cM_k}$ also 
satisfies~\eqref{eq:proof_pos_01} and~\eqref{eq:proof_pos_02}. 

Regarding the effect of the killing, denote by 
$\smash{\ptrace{\tau^+_{B_i^c}}{\cM_k}}$ the killing time of the trace process 
and observe that \Cref{prop:ldp_committor} yields 
\begin{equation}
 \bigprobin{B_i}{\ptrace{\tau^+_{B_i^c}}{\cM_k} = 1} 
 = \bigprobin{B_i}{\tau^+_{\cM_k\setminus B_i} < \tau^+_{B_i}} 
 \leqs \e^{-H/\sigma^2}
\end{equation}
for an $H>0$. By the Markov property, we get 
$\smash{\bigprobin{B_i}{\ptrace{\tau^+_{B_i^c}}{\cM_k} \leqs n}} \leqs n 
