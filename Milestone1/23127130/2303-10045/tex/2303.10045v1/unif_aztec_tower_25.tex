...
\thanks{\textsc{${}^\mathrm{A}$ Massachusetts Institute of Technology, Department of Mathematics, 77 Massachusetts Avenue, Cambridge, Massachusetts, 02139–4307}}
\thanks{\textsc{${}^\mathrm{B}$  California Institute of Technology, Division of Physics, Mathematics and Astronomy, 1200 E California Blvd, Pasadena, CA 91125}}
...
%We build upon the work~\cite{Ch-R}; we solve the derived recurrence relation and use the solution to give a proof of the convergence of the origami maps to the Lorentz-minimal surface predicted in the same paper. 
 We solve the recurrence relation derived in~\cite{Ch-R} by obtaining a simple expression for the perfect t-embeddings in terms of certain edge inclusion probabilities. Using the solution we give a proof of the convergence of the origami maps to the Lorentz-minimal surface predicted in~\cite{Ch-R}.
...
Furthermore, we construct and study perfect t-embeddings of uniformly weighted \emph{tower graphs}, see Figures~\ref{fig:rocket} and~\ref{fig:AdT}, and we find that the graphs of origami maps converge to the \emph{same} Lorentz-minimal surface as in the Aztec diamond case.
...
\begin{enumerate}[(I)]
\item The pair $(\T_n,\Or_n) \to (z,\vartheta(z))$, as $n\to \infty$, where $(z,\vartheta(z)) \in\mathbb{R}^2\times\mathbb{R}$ is the graph of a Lorentz-minimal surface. \label{ass:informal_lorentz}
...
\end{enumerate}
 It was shown in~\cite{CLR2}  that under these assumptions the gradients of $n$-point correlation functions converge to those of the standard Gaussian free field in the intrinsic metric of the Lorentz-minimal surface. See Theorem~\ref{thm:CLR2thm} for the precise statement.
...
\begin{figure}
 ...
 \includegraphics[width=0.3\textwidth]{zz_unif_aztec_to_with_label2.png}
  ...
  {\bf Middle:} Its reduced Aztec diamond $A'_{4+1}$.  {\bf Left and middle:} Weights on black edges are~$1$ and weights on grey ones are~$2$.  {\bf Right:} perfect t-embedding~$\T_{ 4}((A'_{4+1})^*)$ shown in black and it's origami map~$\Or_4((A'_{4+1})^*)$ is shown in blue. The face $\T_4(w_0)$ is the reference face for the origami map.
  }\label{fig_period}
 \end{center}
\end{figure}
...
It was shown in~\cite{Ch-R} that the surface~$\{(z(x,y),\vartheta(x,y)) : (x,y) \in \mathcal{A}\}$ is indeed a Lorentz-minimal surface in~$\mathbb{R}^{2,1}$.
...
\begin{figure}
 ...
 \end{center}
\caption{{\bf Top:} A tower graph~$\TG_4$ of size~$4$ and its reduced one~$\TG'_4$. To get a reduced graph contract degree two vertices emphasised in grey.\\ {\bf Bottom: } Augmented dual of ~$\TG'_4$ (vertices of the augmented dual corresponding to hexagonal faces of~$\TG_4$ are emphasized in turquoise).
%blue). 
...
satisfies assumption~\eqref{ass:informal_lorentz}. Moreover, the graphs~$(\widetilde\T_n, \widetilde\Or_{n}')$ converge to the same Lorentz-minimal surface which arises as the limit of Aztec diamond t-embeddings and origami maps.
\end{corollary}
...
\begin{remark}
The fluctuations of the dimer model height function on certain finite subgraphs of lattices made up of alternating layers of squares and hexagons was studied in~\cite{BoutillierLiSHL}, and their class of graphs includes tower graphs. In Section~\ref{sec:T_n_conv}, we compare the conformal structure of the Lorentz-minimal surface obtained via perfect t-embeddings with the conformal structure for the Gaussian free field derived in \cite{BoutillierLiSHL}, and indeed we find that they match (and furthermore, both match with the conformal structure of Kenyon-Okounkov). In particular, the conformal structures of the Gaussian free field on the Aztec diamond and tower graph are identified under this change of coordinates, and thus so are the limiting height fluctuations.
\end{remark}
...
\subsection{Perfect t-embeddings and origami maps} In this section we remind of basic properties and definitions of \emph{t-embeddings} also known as \emph{Coulomb gauges}. We refer an interested reader to \cite{KLRR, CLR1, CLR2} for more details.
...
\end{itemize}
In general, if the boundary conditions are not specified and there are~$2k$ outer boundary vertices it is expected to have many (the number is a function only of the graph~$\G$) t-embeddings, though properness of at least one of them is still an open question. The existence of (maybe non-proper) t-embedding in this setup is shown in~\cite[Section 3.4]{KLRR}.
...
and~{$K_{\T}(b,w)=0$} otherwise, is a Kasteleyn matrix. Then the origami square root function $\eta: B\cup W\to \mathbb{T}$  is defined by %satisfies 
...
The origami map $\Or$ is a primitive of a piece-wise constant differential form~$d\Or$ defined by 
...
Informally speaking, to get an origami map~$\Or(\G^*)$ from~$\T(\G^*)$ one can choose a white root
...
There are~$3$ types of elementary transformations of the planar bipartite graph that, in a certain sense, preserve
%\textcolor{red}{[}doesn't change\textcolor{red}{]} 
partition and correlation functions of the dimer model, see Figure~\ref{fig:elem}.
...
\begin{remark}
Equation \eqref{eqn:peformula} first appeared as Equation (1.5) in \cite{cohn-elki-prop-96}, where the authors use this representation to prove the convergence of $p_E, p_S, p_N, p_S$ in the $n \rightarrow \infty$ limit. 
\end{remark}
...
% \item[$\bullet$] merge pairwise all the $4n$ obtained pairs of parallel edges.
\end{itemize}
Note that,  in contrast to the Aztec setup, the reduced  tower graph has the same set of inner faces as the tower graph itself, %and does not have vertices degree two, 
...
In this section we prove Proposition~\ref{prop:eqn:to_conv}. The proposition implies that the sequence~$(\T_n,\Or_n')$ of perfect t-embeddings and origami maps of uniformly weighted Aztec diamonds gives the Lorentz-minimal surface in the scaling limit. We also show that the corresponding sequence of maps of tower graphs~$(\widetilde\T_n,\widetilde\Or_n')$ converges to the same Lorentz-minimal surface, so Corollary~\ref{cor:convergence_tower} holds. In the process we also prove Corollary~\ref{Cor_main_frozen}, i.e. we show that under perfect t-embeddings, frozen regions  map to points in the scaling limit for the uniformly weighted Aztec diamond. %In other words, we verify conditions $(1)$ and $(2)$ of Theorem~\ref{thm:CLR}.
...
In Section~\ref{sec:aztec_prob} we described a connection between perfect t-embeddings~$\T_n$  of reduced Aztec diamond~$A'_{n+1}$  of size~$n+1$ and edge probabilities of the Aztec diamond~$A_n$ of size~$n$. More prcisely, recall that 
\begin{equation}\label{eq:T_f_e_reminder}
\T_n(j,k)=p_E(j,k,n)+\i p_N(j,k,n)-p_W(j,k,n)-\i  p_S(j,k,n),
\end{equation}
\begin{equation}\label{eq:O_f_e_reminder}
\Or'_n(j,k)=\tfrac{1}{\sqrt{2}}\big(p_E(j,k,n)-p_N(j,k,n)+p_W(j,k,n)- p_S(j,k,n)\big),
\end{equation}
...
It was shown in~\cite[Proposition 3.1]{Ch-R}, that the graph $(z, \vartheta(z))$ is the Lorentz-minimal surface~$S_{\diamondsuit}$ with boundary contour~$C_{\diamondsuit}$, where~$z$ and~$\vartheta$ are defined by~\eqref{eq:limit_t}--\eqref{eq:limit_o}.
\end{remark}
...
p_S(j,k,n)=\Psi_S(j/n,k/n)+o(1) \text{ as } n\to\infty, \label{eq:p_psi_S}
\end{align}
where the convergence is uniform for~$(j/n, k/n)$ on compact subsets of~$\mathcal A$. The statement now follows by comparing~$\T_n$ and~$\mathcal O'_n$ with~$z$ and~$\vartheta$, respectively, as expressed in~\eqref{eq:T_f_e_reminder},~\eqref{eq:O_f_e_reminder},~\eqref{eq:limit_t} and~\eqref{eq:limit_o}.
\end{proof}
...
$$\Or_n'(z) \rightarrow \vartheta(z)$$
 where the graph of~$\vartheta$ is the Lorentz-minimal surface~$S_{\diamondsuit} \subset \mathbb{R}^{2,1}$ with boundary contour~$C_{\diamondsuit}$.
\end{corollary}

\begin{proof}
The convergence of graphs~$(\T_n, \Or_n')$ to a Lorentz-minimal surface, essentially follows from  Proposition~\ref{prop:main_theorem_2} together with Remark~\ref{rmk:lor_min}. We give details below for completeness. %Proposition~\ref{prop:eqn:to_conv}. 
...
The convergence of the perfect t-embedding and the origami map of the Aztec diamond together with Theorem~\ref{thm:T_aztec_tower} implies that the pairs~$(\widetilde\T_n, \widetilde\Or_n')$ converges to the same Lorentz-minimal surface as~$(\T_n,\Or_n')$.


\begin{corollary}\label{cor:tower_coord} 
We have the convergence of tower graph origami maps, uniform on compact subsets of~$\diamondsuit$,$$\widetilde\Or_n'(z) \rightarrow \vartheta(z),$$ where the graph of~$\vartheta$ is the Lorentz-minimal surface~$S_{\diamondsuit}$ with boundary contour~$C_{\diamondsuit}$.
\end{corollary}
...
We have observed above that the limiting Lorentz-minimal surface for perfect t-embeddings of tower graphs is \emph{the same} as the one for the Aztec diamond,  see Figure~\ref{fig:surface_both}.
...
In addition, we know from \cite{Ch-R} that the conformal parameterization of the liquid region obtained from the limiting Lorentz-minimal surface for the Aztec diamond agrees with the Kenyon--Okounkov conformal structure. We now check the following:
...
Suppose the above assumption holds for a compact set~$\mathcal{K}\subset\Omega$. Then for all~$n>N_\mathcal{K}$ there exist constants~$\kappa=\kappa(\mathcal{K}) \in (0,1)$ and~$C=C(\mathcal{K}) > 0$ such that for~$\delta = C \mu_n$ the condition~$\LipKd$ holds for the perfect t-embedding~$\T_n$, i.e. 
...
\end{equation}
where the sign depends on the orientation of the edge. The sign will not play any role, so will not specify it more explicitly. 
...
Recall that Corollory~\ref{cor:p_E_and_f_E} expresses the perfect t-embedding in terms of edge probabilities. So from the integral formula of the inverse Kasteleyn matrix, we obtain an integral formula for~$\mathcal T_n(j,k)$. 
...
Summing up theses integrals according to Corollary~\ref{cor:p_E_and_f_E}, proves the statement.
\end{proof}
