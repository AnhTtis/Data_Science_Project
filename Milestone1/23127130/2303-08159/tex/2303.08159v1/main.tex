...
A local section $s$ of a complex line bundle $L \overset{\pi}{\rightarrow} M$ whose fiber at $x \in U \subset M$, i.e, $F_x$, is isomorphic to a trivial product of a singleton $\{x\} \subset U$ and one-dimenisional complex field $\mathbb{C}$, i.e. $F_x \approx \{x\} \times \mathbb{C}$:  
\begin{eqnarray}
s(x) = z(x) e(x) \in L,
\end{eqnarray}
...
\begin{eqnarray}
    i \longleftrightarrow \hat{i}_{-f(x)}.
\end{eqnarray}
...
\noindent {\bf Principal G-bundle.} First of all, we explore a proper principal G-bundle that can be associated with the fiber bundle of hypercomplex albegras. Let $P$ and $\pi_P$ be a total space and a projection map, respectively. Then, we define a principal G-bundle
\begin{eqnarray}
...
\end{eqnarray}
which sends a hypercomplex number in $\mathbb{C}_{p(x)}$ to a hypercomplexified one-form,
