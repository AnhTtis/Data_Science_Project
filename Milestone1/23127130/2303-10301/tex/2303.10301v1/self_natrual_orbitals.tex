...
we first carry out one step of CP-AFQMC calculation with an initial trial wave-function (can be chosen arbitrarily). Then by diagonalizing the 1-RDM from QMC calculation, we can obtain the natural orbitals. A
new trial wave-function in the form of single Slater determinant (with fixed particle number) can be constructed from the natural orbitals with large occupancy. We then continue the CP-AFQMC calculation with this new trial wave-function and this
procedure is repeated after convergence.

We compare two schemes to calculate the 1-RDM in CP-AFQMC, i.e., the mixed and real estimator. The mixed estimator is biased because the 1-RDM don't commute with the Hamiltonian, but it is cheap computationally. The real estimator gives more accurate result for 1-RDM, but it is more demanding computationally \cite{PhysRevE.70.056702}.  
