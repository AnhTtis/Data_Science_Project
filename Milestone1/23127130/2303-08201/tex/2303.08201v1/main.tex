...
\newcommand{\WG}[1]{\textcolor{red}{WG:#1}} 
\newcommand{\YZ}[1]{\textcolor{green}{YZ:#1}}

\newcommand{\deltadmde}{\ensuremath{\delta{}_{DMDE}}}
\newcommand{\Omegafld}{\ensuremath{\Omega_{\rm 0,fld}}}
\newcommand{\wfld}{\ensuremath{w_{\rm 0,fld}}}
%TABLES
\newcommand{\nq}[1]{%
...
This IDE model introduces energy-momentum transfer from dark energy to dark matter by modifying their individual energy conservation equations as follow:
\begin{align}
    \nabla_\mu {T_c^\mu}_\nu&=+\frac{Q(v_c)_{\nu}}{a}\\
    \nabla_\mu {T_x^\mu}_\nu&=-\frac{Q(v_c)_{\nu}}{a}~.
\end{align}
...
The energy density perturbation is $\delta=\delta\rho/\rho$ and the divergence of the fluid proper velocity is $\theta=ik\cdot v$. At linear order,  the perturbations in the dark fluids $\delta_{x,c}$ and $\theta_{x,c}$ evolve as~\cite{Valiviita:2008iv,Gavela:2010tm,LopezHonorez:2010esq}:
\begin{align}
...
\end{equation}
where the index $i$ runs from corresponding species of the fluid, here dark matter and dark energy.
...
\end{center}
\caption{
\small Constraints (upper limits) at 68\% (95\%) CL on the parameters of the IDE model obtained from different combinations of CMB data. The Bayes factors $\ln \mathcal{B}_{ij} = \ln \mathcal{Z}_{\rm LCDM} - \ln \mathcal{Z}_{\rm IDE}$ calculated as the difference between the evidence for $\Lambda$CDM and IDE model in such a way that a negative value indicates a preference for the IDE model over the $\Lambda$CDM scenario.
}
\label{tab.results}
\end{table*}
