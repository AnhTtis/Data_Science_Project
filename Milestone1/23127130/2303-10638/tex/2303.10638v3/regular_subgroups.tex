...
In our setting of $G$, we have the inclusion $\Aut_c(G)\subseteq \Aut_z(G)$, and so the condition (\ref{Delta1}) is vacuous when $\beta\in \Aut_c(G)$. This means that we only need to consider the action of $\Aut^c(G)$ here.
...
Under Assumption \ref{assumption}, the following data are equivalent.
\begin{enumerate}[label = $(\arabic*)$]
...
In the case that $\Aut^c(G) =1$, the condition (\ref{Delta2}) is vacuous, and $B$ is the set of all $G'$-valued bilinear forms on $G/G'$. Then $T(G)$ is likely to be large, and this is how we found examples of $G$ with large $T(G)$ in \cite{LMH}. To get examples of $G$ with a small $T(G)$, we shall consider the case when $\Aut^c(G)$ is large so that (\ref{Delta2}) yields many relations. In fact, for the groups $G$ in Theorem \ref{thm1}, there are enough relations from (\ref{Delta2}) that $S=1$ is forced to be trivial, while $\mathcal{S}'\simeq \res(\mathcal{S}')$ is forced to be the subgroup (\ref{theta})  in (a) -- (d) and is only slightly bigger in (e).
