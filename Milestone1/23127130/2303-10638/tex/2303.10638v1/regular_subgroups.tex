...
which is an abelian group under pointwise multiplication in $G'$. Let $S$ and $S'$, respectively, be the subgroups of $B$ consisting of the bilinear forms which are symmetric and anti-symmetric. Every $\Delta\in B$ may be decomposed as
\[ \Delta(u,v) = (\Delta(u,v)\Delta(v,u))^{1/2} \cdot (\Delta(u,v)\Delta(v,u)^{-1})^{1/2}.\]
We then deduce that $B = S\times S'$ is a direct product of $S$ and $S'$.
...
Each bilinear form $\Delta\in B$ gives rise to an $\Aut(G)$-equivariant anti-homomorphism $\gamma : G\rightarrow \Aut(G)$ via (\ref{gamma-Delta}). Let us write
 $N_\Delta = N_\gamma$ for the associated normal regular subgroup of $\Hol(G)$, which corresponds to an element of $T(G)$ if and only if $N_\Delta$ is isomorphic to $G$. We know by \cite[Proposition 3.1]{LMH} that the isomorphism class of $N_\Delta$ depends only on the anti-symmetric component of $\Delta$. Since $N_{\Delta_0}$ is simply $\rho(G)$ for the trivial bilinear form $\Delta_0$, for symmetric forms $\Delta$, we have that $N_\Delta$ is always isomorphic to $G$. For anti-symmetric forms $\Delta$, things are much more complicated and it seems to us that not much can be said in general, except the few facts proven in \cite[Proposition 3.3]{LMH}.
...
In the case that $\Aut^c(G) =1$, the condition (\ref{Delta2}) is vacuous, and $B$ is the set of all $G'$-valued bilinear forms on $G/G'$. Then $T(G)$ is likely to be large, and this is how we found examples of $G$ with large $T(G)$ in \cite{LMH}. To get examples of $G$ with small $T(G)$, we shall consider $G$ for which $\Aut^c(G)$ is large so that (\ref{Delta2}) yields many relations.
...
We end this section with the following remark concerning the structure of the anti-symmetric part $\mathcal{S}'\simeq \res(\mathcal{S}')$ of $T(G)$.
...
\[ \{ \theta_\kappa : \kappa\in \mathbb{Z}\mbox{ coprime to }p\} \simeq (\mathbb{Z}/p^r\mathbb{Z})^\times\]
of $T(G)$, and this is exactly the content of \cite[Proposition 3.1]{class2}. For an arbitrary $\sigma\in \End(G')$, however, it appears that there is no simple way to determine whether $N_{\Delta_{[\sigma]}} \simeq G$ in general, except the necessary condition $1+2\sigma\in \Aut(G')$ as shown in \cite[Example 3.4]{LMH}. But see Proposition \ref{criterion} for a criterion which is valid for a special family (\ref{Gpi}) of groups $G$.
\end{remark}
