...
%Multi-view learning refers to an emerging machine learning paradigm that aims to integrate data from multiple sources to improve performance~\cite{8715409,2019Mvsurvey,tian2020contrastive}. In this work, we are interested in the unsupervised scenario with un-labeled data sets from all sources. We adopt the perspective that the patterns shared by the different views, corresponds to {\em good} information, and hence, it is important to develop a principled approach for extracting this {\em common} information. This goal is closely related to the Wyner's common information where an information theoretic formulation is provided for two correlated random variables~\cite{wyner1975common}.  
Multi-view representation learning aims to efficiently extract meaningful information from data collected from different sources. In this growing body of works, it is typically assumed that there exists a shared target across the multiple views. In supervised multi-view learning settings, the common information corresponds to the available labels and can be extracted efficiently through well-established information-theoretic principles~\cite{tishby2000information,9965818,8986754,9154315,alemi2016deep}. On the other hand, the unsupervised settings, where no labels are available for extracting common information, is closely related to Wyner common information framework where an information-theoretic approach is utilized to characterize the common randomness of two (or more) correlated random variables~\cite{wyner1975common}. 

Here, we adopt the Wyner multi-view unsupervised learning paradigm and build upon the recent progress in understanding common information~\cite{kumar2014exact,relaxWynerInfo22,wynerCont16,e21121181,e22020213}. These recent works have made significant contributions in characterizing Wyner common information in several special but practical cases. For example,~\cite{kumar2014exact} introduces the notion of ``exact common information'' which can be viewed as a deterministic variant of the Wyner common information. Moreover, in~\cite{5766249,wynerCont16}, the connection to lossy source coding and an explicit expression for the case of two correlated Gaussian sources are found. Most relevant to our work, a relaxed variant of Wyner common information formulation is introduced in~\cite{relaxWynerInfo22,9834546} along with a characterization of the generalized vector Gaussian sources case. 

To the best of our knowledge, only few works have considered the development of computationally efficient learning algorithms for solving Wyner common information optimization problem. This comes in contrast to the supervised learning counterpart where several efficient off-the-shelf solvers are available~\cite{alemi2016deep,9518141,9154315,8986754}. More specifically, in the discrete settings, a heuristic gradient-descent-based algorithm was proposed in~\cite{disGrad21} for computing the relaxed Wyner common information, focusing on the comparison to canonical correlation analysis but no convergence guarantee was reported. The focus of our work here is the development of computationally efficient learning algorithms (solvers) for the Wyner multi-view learning in unsupervised setting. Towards this end, we identify two new formulations. The first is referred to as the \textit{Variational} form and is shown to enjoy a linear complexity growth rate with the the number of views. This provides a significant advantage over commonly used methods for multi-view learning whose complexity grows exponentially. For this form, we use tools from variational inference to devise an alternating minimization solver targeting a tight surrogate upper bound of our objective function. The second form is referred to as the \textit{Representation} form which includes the relaxed Wyner common information as a special case. We further devise a new alternating direction method of multipliers (ADMM)-based solver for the resulting non-convex optimization problem. We provide convergence guarantees for the proposed solvers in certain relevant regimes and, empirically, we show that our solvers can outperform the state-of-the-art in terms of clustering accuracy of a synthetic dataset. Finally, we would like to remark that variational inference was applied to the problem of unsupervised learning iin~\cite{e22020213} but the focus of this work was limited to single-view learning context with no straightforward generalization to the multi-view case. 
...
\begin{IEEEeqnarray}{rCl}
    Z^*=&\underset{P(Z,X^V)}{\arg\min} &\,I(X^V;Z),\nonumber\\
    &\text{subject to}&\,P(X^V)=\sum_{z\in\mathcal{Z}}P(Z)\prod_{i=1}^VP(X_i|Z),\IEEEeqnarraynumspace\IEEEyesnumber\label{eq:prob_gen_form}
\end{IEEEeqnarray}
where $X^V:=(X_1,\cdots,X_V)$ represents the $V$ available views; $I(X;Y|Z)$ denotes the mutual information of two random variables $X,Y$ conditioned on $Z$ \cite{cover1999elements}. It is important to note that in unsupervised learning no labels are available and one can only have access to $P(X^V)$ through the available observations. The problem \eqref{eq:prob_gen_form} is difficult to solve mainly due to the non-convex nature of the feasible solution set of $P(Z|X_1,...,X_V)$. Here we address this complexity problem by borrowing tools from the theory of variational inference~\cite{8588399,blei2017variational}. First, we define a parameterized set of distributions $P_\theta (Z)$, $P_\theta (X_1|Z)$,...,$P_\theta(X_V|Z)$, and recast the optimization problem in \eqref{eq:prob_gen_form} as:

\begin{IEEEeqnarray}{rCl}
    &\underset{\substack{\prod_{i=1}^VP_\theta(X_i|Z),\\
    P_\theta(Z)} }{\text{minimize}} &\,I(X^V;Z),\nonumber\\
    &\text{subject to}&\,P(X^V)=\sum_{z\in\mathcal{Z}}P_\theta(Z)\prod_{i=1}^VP_\theta(X_i|Z),\IEEEeqnarraynumspace\IEEEyesnumber\label{eq:prob_variational_form_1}
\end{IEEEeqnarray}
The second step is to relax the feasibility condition by allowing $P(X^V)$ and $P_\theta(X^V):=\sum_{z\in\mathcal{Z}}P(z)\prod_{i=1}^VP(X_i|Z)$ to be distinct joint distributions which fall within a bounded divergence:
\begin{equation}\label{eq:gen_dkl_feat_relax}
    D_{KL}\left[P(X^V)\parallel P_\theta(X^V)\right]\leq \xi,
\end{equation}
where $D_{KL}[\mu\parallel\nu]$ denotes the Kullback-Leibler (KL) divergence between two measures $\mu,\nu$ and $\xi>0$ is a threshold. These two steps now result in the following optimization problem: 
\begin{IEEEeqnarray}{rCl}
    &\underset{\substack{\prod_{i=1}^VP_\theta(X_i|Z),\\
    P_\theta(Z)} }{\text{minimize}} &\,I_\theta (X^V;Z),\nonumber\\
...
In the third step, we develop the following surrogate upper bound on $I_\theta(X^V;Z)$:
\begin{lemma}\label{lemma:gen_vi_bound}
    Let $P_\theta(X^V):=\sum_{z\in\mathcal{Z}}P(z)\prod_{i=1}^VP_\theta(X_i|Z)$ with $V\in\mathbb{N}\backslash\{1\}$ and $P(X^V)$ is known, then:
    \begin{IEEEeqnarray*}{rCl}
...
The bound is tight when $P_\theta(X^V)=P(X^V)$. In the final step, we combine the upper bound in Lemma \ref{lemma:gen_vi_bound} over $P_\theta(X^V)$ with the relaxed feasible set \eqref{eq:gen_dkl_feat_relax} through a Lagrange multiplier, giving the overall Lagrangian:
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
where the scalar $\beta>0$ is a multiplier. The key advantage of this~\textit{variational} form is that it always provides solutions that satisfy the factorization condition $P_\theta(X^V|Z)=\prod_{i=1}^VP_\theta(X_i|Z)$ relaxing the representation condition $P(X^V)=\sum_{z\in\mathcal{Z}}P_\theta(z)\prod_{i=1}^VP_\theta(X_i|Z)$. This results in linear complexity growth $\mathcal{O}(V)$, defined as the number of parameters to optimize with w.r.t. the number of views $V$. This can be seen as the parameter set is $\Theta:=\{P_\theta(Z),\prod_{i=1}^VP_\theta(X_i|Z)\}$. Compared to existing methods in literature~\cite{disGrad21} the complexity is reduced from $\mathcal{O}(|Z||X|^V)$ to $\mathcal{O}(V|Z||X|)$, where $|X_i|=|X|,\forall i\in[V]$.


For simplicity of illustration, we further simplify the relaxed problem by considering uniformly distributed $P(Z)=1/|Z|$, reducing the parameters to optimize to $\{P_\theta(X_i|Z)\}_{i=1}^V$ (this assumption can be easily relaxed without affecting the results). Additionally, we treat each fixed $\beta$ as a different optimization problem and sweep through a range of $\beta$ with random initialization.
...
Our proposed alternating minimization solver, denoted as \textit{Solver} \RomanNum{1}, now follows:
 ...
 \end{IEEEeqnarray}
 where $k$ denotes the iteration counter; $\Omega_i$ is the compound simplex for $i^{th}$ view; We define $\{\cdot\}_{<1}=\{\cdot\}_{>V}=\emptyset$. \textit{Solver} \RomanNum{1} updates the vectorized conditional probability $P(X_i|Z)$ (stack along each realization of $z\in\mathcal{Z}$) through \eqref{eq:vi_alg} iteratively until convergence. {\bf Note that each step corresponds to a convex optimization update and therefore convergence is assured~\cite{nesterov2003introductory,NesterovYurii2018Loco,BertsekasDimitriP.1999Np,boyd2004convex}}. We refer to Appendix \ref{appendix:conv_solver1} for details.


\section{The Representation Form}
In this approach, we relax the factorization condition resulting from the conditional independence, while fixing the representation condition $P(X^V)=\sum_{z\in\mathcal{Z}}P(z)P_\theta(z|X^V)$, which results in the following optimization problem:
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
where $S$ is a partition of the indices of $V$ views, i.e., $S\subset{[V]},S\cap S^c=\emptyset,S\cup S^c=[V]$; $[V]=\{1,\cdots,V\}$ and $R_S\geq 0$ are the control thresholds.
For illustration, let's start with two views; namely $X_1$ and $X_2$ with a known joint distribution $P(X_1,X_2)$. The problem \eqref{eq:rep_form_gen_problem} reduces to the following:
...
\end{IEEEeqnarray}
where $\gamma>0$ is the Lagrange multiplier. The expansion for $V$ views is included in Appendix \ref{appendix:multiview_rep_form} for completeness. We then define the following primal and augmented variables pair:
\begin{equation}\label{eq:var_pq}
        p:=-\log{p_{z|x_1,x_2}},\quad q:=-\log{p_{z'|x_1,x_2}},
\end{equation}
where $p_{z|x_1,x_2}$ is the vector form of the conditional probability $P(Z|X_1,X_2)$, obtained by stacking the conditional probability mass along each view-specific observation $x_i,i\in[V]$. This assignment facilitates the implementation of the solver (e.g., establishing smoothness conditions and simplex projection), the details are included in Appendix \ref{appendix:conv_prelim}.
...
\begin{figure*}[t]
    ...
    \subfloat[Noise-Free Dataset,$|Z|=8,|X_1|=|X_2|=16$]{%
      ...
      \includegraphics[width=2.9in]{figures/solver12_vs_corr0_new.eps}
    }
    \hfil
    \subfloat[Noisy, $|Z|=8,|X_1|=|X_2|=16$]{%
        \label{subfig:sol12_corr_mi}
        \includegraphics[width=2.9in]{figures/solver12_vs_corr2e_new.eps}
    }
...
\end{figure*}
 ...
 Existing convergence analysis for ADMM solvers requires a convex-weakly convex (for the two objective functions) which is not satisfied by our formulation~\cite{themelis_patrinos_2020,huang2022admm}. To see this, one can express the negative entropy $-H(X)$ with $p:=-\log{p(x)}$, which gives $-\sum_{x\in\mathcal{X}}e^{-p}p$. Then $-H(X)$ is convex w.r.t. the negative log-likelihood when $0\leq p\leq2 $ but concave when $p>2$. We address this issue by developing new techniques that generalizes the convergence analysis to objectives with only weakly convex structures, assuming that the following conditions are satisfied:
 ...
 \begin{assumption}\label{assump:main_condi}
     \begin{itemize}
         ...
         \item The penalty coefficient $c$ satisfies:
         \begin{equation*}
             c>\max\left\{\sigma_G,\frac{\sigma_F+\sqrt{\sigma_F^2+8L_p^2}}{2}\right\}.
         \end{equation*}
     \end{itemize}
 \end{assumption}
 ...
 \begin{theorem}\label{thm:main_conv}
     Let the augmented Lagrangian $\mathcal{L}_c(p,\nu,q)$ defined as in \eqref{eq:aug_lag}. Suppose Assumption \ref{assump:main_condi} holds, then the sequence $\{w^k\}_{k\in\mathbb{N}}$, where $w^k:=(p^k,\nu^k,q^k)$, obtained from the solver \eqref{eq:alg_admm_sol} converges linearly fast locally around a neighborhood of a stationary point $w^*:=(p^*,\nu^*,q^*)$.
 \end{theorem}
 \begin{IEEEproof}
     See Appendix \ref{appexdix:conv_proof}. The proof follows the outline provided by \cite[Sec. \RomanNum{3}]{huang2022admm}, but in contrast, our assumptions are more relaxed since we only require weakly-convex objectives.
 \end{IEEEproof}
 The main tool for our convergence proof is the Kurdyka-{\L}ojasiewicz inequality~\cite{attouch2009convergence,li_pong_2018}, which inspired efficient solvers for Information Bottleneck~\cite{8919752,9518141,9965818,huang2022admm}. Therefore, one can view the results of this section as an extension of this earlier line of works to the Wyner multi-view unsupervised learning paradigm. 
...
\begin{enumerate}
\item By setting $I(X_1;X_2|Z)=0$ in \eqref{eq:condmi_expand}, it reduces to a linear equation of the two objectives:
...
\begin{figure*}[t]
   ...
   \subfloat[Noisy Dataset.]{%
     ...
     \includegraphics[width=2.9in]{figures/test_acc_cr02_reverse.eps}}  
...
In this section, we evaluate the performance of \textit{Solver \RomanNum{1}} and \textit{Solver \RomanNum{2}} and compare their performance to the baseline gradient descent-based solver~\cite{disGrad21} (denoted as \textit{Baseline}). We use synthetic two-view datasets where the joint distribution $P(X_1,X_2)$ is available but not the ground-truth, i.e., $P(Y),P(X_1|Y),P(X_2|Y)$ in the factorized distribution $\sum_{y\in\mathcal{Y}}P(Y)P(X_1|Y)P(X_2|Y)$ are hidden. The synthetic datasets have $P(y)=1/|Y|,\forall y\in\mathcal{Y}$. 
...
The dataset has the conditional probability for each view as:
%\begin{equation}
%    \begin{split}
%    {}&P^{(2)}(X_i|Z)=\\
%    &\begin{bmatrix}
...
P(X_i|Y)=
    &\begin{bmatrix}
        \boldsymbol{\frac{1}{2}-\delta} &\boldsymbol{0}&\cdots&\boldsymbol{0}&\delta\\
...
\end{split}
\end{equation}
where each bold-face symbol $\boldsymbol{v}$ means a vector $v\cdot\boldsymbol{1},v\in\mathbb{R}$ with $\boldsymbol{1}$ denotes the all-one vector. We set $\boldsymbol{1}\in\mathbb{R}^2$, $|Y|=8$ and $|X_1|=|X_2|=16$. We consider two cases of \eqref{eq:eval_dataset}: 1) \textit{Noise-Free} $\delta=0$ and 2) \textit{Noisy} $\delta=0.1$. For simplicity, the cardinality is assumed to be known $|\mathcal{Z}|=|\mathcal{Y}|$. 
...
We first compare \textit{Solver \RomanNum{2}} and \textit{Baseline} as both solvers minimize the Lagrangian \eqref{eq:mv_sto_lag}. The \textit{Baseline} is as implemented in \cite[Algorithm 1]{disGrad21}. We set the range of the trade-off parameter $\gamma\in[1,50]$ with $20$ geometrically-spaced grid points. $10$ trials are performed for each grid point. The termination criterion is $\lVert p^k-q^k\rVert^2\leq 2\times 10^{-6},\forall k\leq K_{max}=3\times 10^{5}$ for \textit{convergence}. Otherwise \textit{divergence} as a result. We report the converged solutions and plot them on the information plane to demonstrate the trade-off. The plane is formed by the two competing objectives: $I(Z;X_1,X_2)$ as the $y$-axis and $I(Z;X_1)+I(Z;X_2)$ as the $x$-axis. In Fig. \ref{fig:y8b2_compare_baseline_all}, the solutions from the proposed solver better characterizes the trade-off and clearly outperforms the \textit{Baseline} in both the \textit{Noise-Free} and \textit{Noisy} datasets, where the solutions from our solver  obtain the Wyner common information (the intersection with the auxiliary dotted line corresponding to \eqref{eq:sto_line}) in contrast to the \textit{Baseline} which fails to attain this point.

%Then we compare the \textit{Stochastic} and \textit{Representation} forms using the proposed solvers. In \eqref{eq:sto_line}, we show that both forms approximate the Wyner common information in two different sets of competing goals. In Fig \ref{subfig:y8b2_corr_stoloss}, the solutions of the \textit{Representation} form find better solutions since they achieve lower $I(X_1,X_2;Z)$ at a fixed $x$-axis than that the \textit{Representation}.
%Conversely, as shown in Fig \ref{subfig:y8b2_corr_detloss}, when the objective is to minimize $H(Z)$ and maximize $H(Z|X_1)+H(Z|X_2)-H(Z|X_1,X_2)$, \textit{Deterministic} form are better than \textit{Stochastic} since lower $H(Z)$ is achieved for a fixed $x$-axis. Note that in both forms, the solutions obtained from the proposed solver attain the Wyner common information (intersections with the auxiliary lines).
Next we compare \textit{Solver \RomanNum{1}} and \textit{Solver \RomanNum{2}} on the information plans. \textit{Solver \RomanNum{1}} minimizes \eqref{eq:nv_vi_lag} with $V=2,\xi\leq0.1$ and vary the cardinalities $|Z|\in\{8,\cdots,16\}$. For \textit{Solver \RomanNum{1}}, the convergence criterion is set to $\lVert q^{k+1}_{x_i|z}-q^{k}_{x_i|z}\rVert\leq 2\times 10^{-6},\forall i\in\{1,2\}$. \textit{Solver \RomanNum{2}} follows the same configuration mentioned previously. In Fig. \ref{fig:sol12_compare_mi}, we report the minimum $I(X_1,X_2;Z)$ achieved at a fixed $I(X_1;Z)+I(X_2;Z)$. The desired solution is located at $I(X_1,X_2;Z)=3$ bits such that $I(X_1;X_2|Z)=0$. As shown in Fig. \ref{subfig:sol12_noncorr_mi} and Fig. \ref{subfig:sol12_corr_mi}, in both \textit{Noise-Free} and \textit{Noisy} datasets, the two solvers attain the desired solutions with low approximation error. Note that for both solvers, the cardinality $|Z^*|$ that achieves the desired solution coincides with that of the unavailable label, i.e., $|Z^*|=|Y|=8$.


Finally, we apply the proposed solver for unsupervised clustering with the \textit{Noise-Free} and \textit{Noise} datasets. We generate training and testing data as follows. $N$ samples of the labels $Y$ are generated uniformly with $P(Y)=1/|Y|$. Given the labels, the observations of each view are generated from inverse transform sampling~\cite{DevroyeLuc2013NRVG} with $P(X_i|Y),\forall i\in\{1,2\}$ defined in \eqref{eq:eval_dataset}. We follow this procedure to generate $N_{\text{train}}=10^{5}$ training and $N_{\text{test}}=10^4$ data. At training phase, all methods use the estimated empirical joint distribution $P(X_1,X_2)$, obtained through counting the pairs of $(x_1,x_2)\in D_{train}$ and obtain a common information encoder $P(Z|X_1,X_2)$. Then the encoder is tested with testing data. For each test sample $(x_1,x_2)\in D_{test}$, a hypothesis $\hat{z}\in\mathcal{Z}$ is obtained through inverse transform sampling of the encoder at $P(Z|x_1,x_2)$. After collecting all $\{\hat{z}\}$, exhaustive search is adopted to find the best labeling over all combinations (accomplished through one shot training)~\cite{xie2016unsupervised,Federici2020Learning,tian2020contrastive}. In Fig. \ref{fig:test_accuracy_toy} we report the testing accuracy achieved by the two new solvers and the baseline approach for both \textit{Noise-Free} and \textit{Noisy} datasets. Clearly, the proposed solvers outperform the \textit{Baseline} over a wide range of $I(X_1;X_2|Z)$. Notably, our solvers attain the Wyner common information $I(X_1;X_2|Z)\approx 0$ with $>98\%$ accuracy in \textit{Noise-Free} and $>78\%$ accuracy in \textit{Noisy} datasets in contrast to the \textit{Baseline}.


\section{Conclusion}
\label{sec:conclusion}

In this work, we proposed two forms for the Discrete Wyner multi-view unsupervised learning leading to two computationally efficient solvers with guaranteed convergence. The first borrows tools from variational inference and is shown to enjoy a linear complexity scaling with the number of views. The second solver is obtained by adapting the ADMM to a representational learning form of Wyner common information. The convergence analysis of the ADMM-based solver is shown to generalize earlier works on the Information Bottleneck solvers to our context~\cite{9518141,huang2022admm}. Numerically, both solvers are shown to outperform the existing gradient descent-based approach on a synthetic dataset. While our focus here was devoted to the discrete case, we have generalized our variational approach to continuous distributions and the corresponding excellent results are reported elsewhere. Overall, both theoretical and numerical results demonstrate the efficacy of the proposed Wyner solvers for multi-view unsupervised learning.  
...
\begin{IEEEeqnarray}{rCl}\label{eq:gennv_vi_bound}
    &{}&I_\theta(X^V;Z)\nonumber\\
    &=&\mathbb{E}_{Z}\left[D_{KL}[P_\theta(X^V|Z)\parallel \sum_{z}P(Z)P_\theta(X^V|Z)]\right]\nonumber\\
    &=&\mathbb{E}_{Z}\left[\sum_{x^v}P_\theta(X^V|Z)\log{\frac{P_\theta(X^V|Z)}{\sum_{z}P(Z)P_\theta(X^V|Z)}}\right]\nonumber\\
    &=&-H_\theta(X^V|Z)-\mathbb{E}_{X^V;\theta}\left[\log{\sum_{z'}P(Z')P_\theta(X^V|Z')}\right]\nonumber\\
    &=&-H_\theta(X^V|Z)\nonumber\\
    &&-\mathbb{E}_{X^V;\theta}\left[\log{\sum_{z'}P(Z')P_\theta(X^V|Z')}\frac{P(X^V)}{P(X^V)}\right]\nonumber\\
    &=&-\sum_{i=1}^VH_\theta(X_i|Z)-D_{KL}\left[P_\theta(X^V)\parallel P(X^V)\right]\nonumber\\
    &&-\sum_{x^V}\left[\sum_{z}P(Z)\prod_{i=1}^VP_\theta(X_i|Z)\right]\log{P(X^V)}\nonumber\\
    &\leq&-\sum_{i=1}^VH_\theta(X_i|Z)-\mathbb{E}_{X^V;\theta}\left[\log{P(X^V)}\right],\IEEEeqnarraynumspace\IEEEyesnumber
\end{IEEEeqnarray}
where the last equality is due to the Markov chain $X_S\rightarrow Z\rightarrow X_{S^c}$ and the last inequality follows by the non-negativity of KL divergence. 
...
\begin{lemma}\label{lemma:llr_weak_cvx}
    Let $q:=\log{p_x}\in\mathbb{R}^{|\mathcal{X}|}_{-}, \boldsymbol{1}^Tp_x=1$. Suppose $q_x<-\epsilon_0$, with $0<\epsilon_0\ll 1$, then the entropy $H(X)$ is $\sigma$-weakly convex, where $\sigma=2e^{-\epsilon_0}(2-\epsilon_0)$.
\end{lemma}
\begin{IEEEproof}
...
The log-likelihood optimization technique facilitates the establishment of smoothness condition. It is known that the smoothness condition depends on the minimum non-zero element of a probability mass~\cite{han2020optimal,6203416,huang2022admm}. Then consider a range bounded measure $\mu,\sum_{x\in\mathcal{X}}\mu(x)=1$ such that $\epsilon<\mu(x)<1-\epsilon,\forall x\in\mathcal{X},0<\epsilon\ll 1$. In contrast, $-\infty<-M_{\epsilon}<\log{\mu(x)}<-m_{\epsilon}$, where $M_\epsilon$ is the largest machine number, $m_\epsilon$, the machine precision. Clearly, $M_\epsilon$ can be significantly larger than $1-\epsilon$ in terms of machine precision. In practice, the log-likelihood optimization technique enables more relaxed simplex projection as the range of simplex expands from $(\epsilon,1-\epsilon)$ to $(-M_\epsilon,-m_\epsilon)$. 

\subsection{Convergence Proof for Solver \RomanNum{1}}\label{appendix:conv_solver1}
The overall Lagrangian \eqref{eq:nv_vi_lag} is minimized alternatingly with \eqref{eq:vi_alg}. Therefore, it suffices to consider a single update. Without loss of generality, we focus on the update for the first view $q_{x_1|z}$:
\begin{IEEEeqnarray}{rCl}
    \mathcal{L}^{(1)}_{\beta}:&=&-H(X_1|Z)+\beta\sum_{x^v}P(x^V)\left[\log{P(x^v)}\right.\nonumber\\
    &&\left.\vphantom{\log{P(x^v)}}-\log{\sum_{z'}\frac{1}{|\mathcal{Z}|}P(X_1|Z)\prod_{j\neq 1}P(X^k_j|Z)}\right].\IEEEeqnarraynumspace\IEEEyesnumber\label{eq:appendix_conv1_single1}
\end{IEEEeqnarray}

The first term in the above is a negative (conditional) entropy, which is convex w.r.t. $P(X_1|Z)$ since $P(Z)=1/|\mathcal{Z}|$ \cite{cover1999elements}. The second term is a constant that is independent to $P(X_1|Z)$ and the last term is convex w.r.t. $P_\theta(X^V)$, and $P_\theta(X^V)$ is an affine transform of $P(X_1|Z)$ (since $\prod_{j\neq 1}P(X_j^k|Z)$ is fixed). Therefore the last term in \eqref{eq:appendix_conv1_single1} is convex w.r.t. $P(X_1|Z)$. Since \eqref{eq:appendix_conv1_single1} is convex w.r.t. $P(X_1|Z)$, and similarly for all $i\in[V]$, the convergence is guaranteed from elementary convex optimization analysis~\cite{nesterov2003introductory,NesterovYurii2018Loco,BertsekasDimitriP.1999Np,boyd2004convex}.
