...
We here consider the contact between an altermagnet and a superconductor and determine how the altermagnetism affects the fundamental process of Andreev reflection. We show that the resulting charge conductance depends strongly on the interfacial orientation of the altermagnet relative to the superconductor, displaying features similar to normal metals or ferromagnets. The zero-bias conductance peaks present at the interface in the $d$-wave case are robust toward the presence of an altermagnetic interaction. Moreover, the spin conductance can be increased by more than an order of magnitude compared to its normal-state value. It also changes its sign via the applied bias voltage. These results show how the spin-anisotropic altermagnetic state can be probed by conductance spectroscopy and how it offers voltage control over
...
We find that the altermagnetism strongly influences both charge and spin currents flowing into the SC for high-transparency contacts. Depending on the crystallographic orientation of the interface relative to the spin-polarized lobes of the altermagnetic Fermi surface, the zero-bias charge conductance peak in $d$-wave superconductors can be either enhanced or suppressed relative to the normal-state with increasing altermagnetic strength. Moreover, the spin conductance is highly tunable via the applied bias voltage. This includes a sign reversal of the spin current when the SC features a $d$-wave symmetry. Our findings demonstrate how the unique momentum-dependent spin polarization of the altermagnetic state is revealed in conductance spectroscopy and how it offers voltage control over directionally dependent spin currents.
...
\end{align}
On the other hand, when $\alpha > \alpha_c$, the energy dispersion corresponds to a hyperbola, which can not define a closed integral path. Therefore, we confine $\alpha < \alpha_c$ in this work. 
...
We can compute the total charge current flowing in the AM using Eq. (\ref{eq:j_Q1,eup}) in the $e\uparrow$ channel. 
...
\begin{align}
J_{Q,e\uparrow} =-eN_{h\downarrow}^{\text{AM}}(E,\theta)|r_{A}|^2f(E+eV)(\frac{\hbar k_{e\uparrow,-}}{m_e}+\frac{\alpha k_y}{\hbar}).
\end{align}
Here $N_{h\downarrow}^{\text{AM}}(E,\theta)$ is the DOS in the AM for $h\downarrow$ incident with $E=-E_-$, which can be obtained by following the same procedure as described above. 

The total charge current $I$ is finally obtained by first computing the total electric current flowing in the $e$ and $h$ channels for both spins on the AM side, each contribution determined by $j_{Q,i} f_i(E) N_i(E)$ where $j_{Q,i}$ is the charge current $I(V)$ produced by an incoming particle channel $i$, $f_i(E)$ is the distribution function for channel $i$, and $N_i(E)$ is the DOS for channel $i$, and then integrating over all possible angles of incidence $\theta$ through $\int dk_y = \int(dk_y/d\theta)d\theta$. We find that the charge current carried in the $e$ and $h$ channels are equal, and it thus suffices to consider only transport in the $e$ channel for spin-$\uparrow$ and $\downarrow$. Specifically, the total charge current $I_\sigma$ flowing in the electron channel for spin $\sigma$ is obtained as:
\begin{align}\label{eq:current}
I_\sigma \propto -|e| \int d k_y j_{Q,i}^{e,\sigma}(E,k_y)f_i(E) N_i(E,k_y)
\end{align}
...
For case (b) in Fig. \ref{fig:model}, the transmitted current is spin-polarized and shows similar trends as normal FM \cite{FMSC}, e.g., the spin conductance for the $k_x^2-k_y^2$ AM/$d$-wave $\beta=0$ increases with altermagnetism strength and shows a maximum (peak) when $eV/\Delta_0$ approaching 1. Moreover, the magnitude of the spin conductance $G_S$ vanishes for $\alpha=0$ and becomes very large compared to its normal-state value close to $\alpha_c$. This is seen in the upper row of Fig. \ref{fig:Spin} with $Z=0$: the spin conductance is enhanced by more than an order of magnitude for subgap voltages $eV<\Delta_0$ compared to the normal-state value $G_S (eV\gg\Delta_0)$.  Moreover, the sign of the spin-polarization is tunable via the bias voltage $eV$ in the case where surface states exists ($d$-wave $\beta=\pi/4$), which cannot be achieved when replacing the AM with a FM. This specific sign change of spin conductance is closely related to the Andreev reflection and normal reflection probabilities and can be understood as follows. In the $d$-wave $\beta=\pi/4$ case, it is found that the Andreev reflection probability of one spin species starts off larger than the other spin for small energies but then becomes smaller than the Andreev reflection probability for the other spin. This all occurs at subgap energies $E<\Delta_0$. This "sign change" feature in Andreev reflection corresponds with the sign change of the spin conductance since Andreev reflection enhances the conductance. As for the normal reflection probability, no such sign change is observed. 
%Note that if the $k_x^2-k_y^2$ AM is replaced by a normal ferromagnet, the sign change can be obtained for both Andreev and normal reflection coefficients.
Note that if a normal ferromagnet replaces the $k_x^2-k_y^2$ AM, the sign change can be obtained for both Andreev and normal reflection coefficients.
We have numerically verified that the sign change seen in both the Andreev- and normal-reflection probabilities have a canceling effect in the sense that the total spin conductance does not have a sign change in the FM case, regardless of the value of $J_\text{ex}$. A more detailed analysis regarding this sign change can be found in the appendix.

The properties discussed in this paper make AM/SC attractive in terms of spin current control since both the magnitude and the sign can be tuned \textit{in situ} by the bias voltage. The low-transparency case is considered in the lower row of Fig. \ref{fig:Spin}. Similarly to the charge conductance, the altermagnetic interaction has very little impact on the results in this case. Therefore, high-transparency contacts between altermagnets and superconductors will offer the clearest transport signature of the altermagnetic interaction.
...
. At the same time,  the spin current can be enhanced by more than an order of magnitude relative to the normal state and also change its sign depending on the applied bias voltage. Our predicted effects can be tested experimentally using 
%an 
...
\begin{align}
    J_{Q1} &=N_{e\uparrow}^{\text{AM}}(E,\theta)f(E-eV)j_{Q1}\notag\\
    &=-eN_{e\uparrow}^{\text{AM}}(E,\theta)f(E-eV)(\frac{\hbar k_{e\uparrow,+}}{m_e}+\frac{\alpha k_y}{\hbar}),
\end{align}
...
This contributes to the total charge current density in the $e\uparrow$ channel on the AM side with 
\begin{align}
    J_{Q2} &=N_{e\uparrow}^{\text{AM}}(E,\theta)f(E-eV)j_{Q2}\notag\\
    &=-eN_{e\uparrow}^{\text{AM}}(E,\theta)|r_1|^2f(E-eV)(\frac{\hbar k_{e\uparrow,-}}{m_e}+\frac{\alpha k_y}{\hbar}).
\end{align}
...
This contributes to the total charge current density in the $e\uparrow$ channel on the AM side with 
\begin{align}
    J_{Q3} &=N_{e\uparrow}^{\text{SC}}(E,\theta)f(E)j_{Q3}\notag\\
    &=-eN_{e\uparrow}^{\text{SC}}(E,\theta)|t_2|^2f(E)(\frac{\hbar k_{e\uparrow,-}}{m_e}+\frac{\alpha k_y}{\hbar}),
\end{align}
...
\begin{align}
    J_{Q4} &=N_{h\downarrow}^{\text{AM}}(E,\theta)[1-f(-E-eV)]j_{Q4}\notag\\
    &=-eN_{h\downarrow}^{\text{AM}}(E,\theta)|r_{A3}|^2[1-f(-E-eV)](\frac{\hbar k_{e\uparrow,-}}{m_e}+\frac{\alpha k_y}{\hbar}).
\end{align}
...
This contributes to the total charge current density in the $e\uparrow$ channel on the AM side with 
\begin{align}
    J_{Q5} &=N_{h\downarrow}^{\text{SC}}(E,\theta)f(E)j_{Q5}\notag\\
    &=-eN_{h\downarrow}^{\text{SC}}(E,\theta)|t_{A4}|^2f(E)(\frac{\hbar k_{e\uparrow,-}}{m_e}+\frac{\alpha k_y}{\hbar}),
\end{align}
...
\section{Probability-related sign change in spin conductance}
To look into the sign change of the spin conductance in the $k_x^2-k_y^2$ AM/SC, we investigated the reflection probabilities for different incident angles.
...
Except for the AR probability, the normal reflection (NR) probability should also be considered since NR suppresses the conductance. Based on the above insights obtained from the AR probability, we focus on the $d$-wave $\beta=\pi/4$ SC at a small incident angle, e.g., $\theta = \pi/8$. Unlike the AR, in order to get conductance in the $e\uparrow(\downarrow)$ channel through NR, the NR probability is derived based on the wavefunctions induced by the $e\uparrow(\downarrow)$ incident. In addition, we compare the probability behaviors between AM/SC and FM/SC, as shown in the Fig. \ref{A_and_B}. 
