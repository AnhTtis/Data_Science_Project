...
%
Lastly, we discuss experiment design for learning counterfactuals over rankings. 
...
{\color{red}
\noindent  \textbf{Asymptotic Normality.} We do not provide a formal statement for asymptotic normality of Synthetic Permutations, but informally discuss how the arguments to establish normality for combinations discussed in Section \ref{sec:asymptotic_normality} can easily be adapted for rankings.
%
Specifically, if we assume that the horizontal regression predictions are asymptotically normal, and the appropriate notational changes are made to the assumptions of Theorem \ref{thm:normality}, then it is easy to verify that the predictions of Synthetic Permutations for a non-donor unit $n$, and permutation $\tau$ are asymptotically normal. 



\subsection{Experiment Design for Permutations}
\label{subsec:experiment_design_permutations}

In this section, we attempt to adapt our experimental design mechanism for combinations to permutations. 
%
However, we show our proof strategy for guaranteeing that we can recover all $N \times p!$ potential outcomes breaks down for permutations.
%
Then, we discuss possible proof techniques that can overcome the issues in our proof strategy as future work. 


\noindent \textbf{Experiment Design for Permutations.} To adapt our experimental design mechanism to permutations, we follow the same three-step procedure. 
%
That is, we first choose the donor set $\mathcal{I}$ by sampling a set of units independently and uniformly at random. 
%
Secondly, sample permutations $G_{\mathcal{I}} \subset \mathbb{S}_p$ independently and uniformly at random, and assign all donor units this set of permutations. 
%
Lastly, sample $G_N \subset \mathbb{S}_p$ permutations independently and uniformly at random, and assign all non-donor units this set of permutations.


\noindent \textbf{Proof Breakdown.} Next, we discuss why our previous proof strategy for guaranteeing that we can recover all target causal parameters via our experimental design mechanism breaks down for permutations. 
%
Our proof technique requires that the matrix of Fourier characteristics $\bchi(G_{\mathcal{I}}) \in \mathbb{R}^{|G_{\mathcal{I}}| \times 2^{\binom{p}{2}}}$ corresponding to the permutations sampled for the donor set are isotropic. 
%
That is, we require $\E[\chi_{S}^{\tau}\chi_{S'}^{\tau}] = \mathbbm{1}\{S=S'\}$ where the expectation is taken over the randomness in uniformly sampling permutations $\tau$ from $\mathbb{S}_p$. 
%
However, for a permutation $\tau$, the binary encoding $\tau \rightarrow \mathbf{v}(\tau)$ is injective, and non-surjective, i.e., there are many binary sequences of length $\binom{p}{2}$ that do not correspond to any permutation.
%
Therefore, sampling permutation $\tau$ uniformly is not equivalent to sampling uniformly from the hypercube $\{-1,1\}^{\binom{p}{2}}$.
%
As a result, the matrix of sampled Fourier characteristics $\bchi(G_{\mathcal{I}})$ is not isotropic, causing our proof strategy to fail. 

\noindent \textbf{Alternative Proof Strategies.} As discussed above, our proof breaks down because the binary encoding $\tau \rightarrow \mathbf{v}(\tau)$ is injective, and non-surjective.
%
In order to overcome this barrier, we can utilize work \cite{kondor2010ranking} that introduces a Fourier transform of any $f: \mathbb{S}_p \rightarrow \mathbb{R}$ without requiring a binary representation of permutations.
%
The representation of functions over permutations employed in \cite{kondor2010ranking} ensures that each permutation corresponds to a unique vector (i.e., the representation is bijective). 
%
We leave it as future work to investigate whether this alternative Fourier representation can guarantee that our experimental design mechanism can recover all causal parameters. }
