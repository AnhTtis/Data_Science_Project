...
%\noindent \textbf{Step 1: Feature Identification.} For each donor unit $u \in \mathcal{I}$, divide the observed interventions $\Pi_u$ equally into two sets $\Pi^{a}_u$ and $\Pi^{b}_u$. Fit a CART model $\mathcal{\hat{T}} = \{\cell_1, \ldots \cell_m \}$ on the dataset $\mathcal{D}^a_u = [\{\bv(\pi),Y_{u\pi}\}: \pi \in \Pi^a_u]$. Let $\hat{K} = \{\bv(\pi)_j : \bv(\pi)_j$ $\in \mathcal{\hat{T}}\}$ denote every feature that the CART splits on. 
...
\noindent CART consistently estimates the target causal parameter for a donor unit $u$ provided that the structural assumptions on the potential outcomes are satisfied and the number of observations $|\Pi_u|$ grows as $\omega\left(s^2\max\{\log(p),
\log(s)\} \right)$. 
%
...
%
It remains as interesting future work to analyze the use of other decision tree methods such as those proposed in \cite{tan2023fast,agarwal2022hierarchical,agarwal2023mdi,mazumder2022quant}.
...
\label{subsec:CART_horizontal_regression_proof}
 ...
 Let $\mathcal{E}_{1}$ denote the event that $\hat{K} = K$. Further, define the event $\mathcal{E}_{S}$ for any subset $S \subset K$ as  
\begin{equation*}
