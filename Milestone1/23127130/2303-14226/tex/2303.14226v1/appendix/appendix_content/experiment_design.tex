...
Let $\mathcal{E}_{1}, \ \mathcal{E}_{2},\  \mathcal{E}_{3},\  \mathcal{E}_{4}$ denote the event that horizontal span inclusion (Assumptions \ref{ass:donor_set_identification} (a)), incoherence of donor units (Assumption \ref{ass:incoherence}), linear span inclusion (\ref{ass:donor_set_identification}) (b) and subspace inclusion (Assumption \ref{ass:rowspace_inclusion}) hold, respectively. Let $\mathcal{E}_{H} = \mathcal{E}_1 \cap \mathcal{E}_2$ and $\mathcal{E}_{V} = \mathcal{E}_3 \cap \mathcal{E}_4$ denote the events that the conditions required for horizontal and vertical regression are satisfied respectively. We proceed by showing that both events $\mathcal{E}_{H}$ and $\mathcal{E}_{V}$ hold with high probability. 
\vspace{2mm}
...
s_r \left(\mathcal{A}_{\mathcal{I}}^T \mathcal{A}_{\mathcal{I}} \right) \geq \frac{C'|\mathcal{I}|}{r}
\end{equation}
for a universal constant $C' > 0$ with probability at least $ 1 - \gamma/4$.
\end{lemma}
\noindent 

\noindent Since $\mathcal{A}_{\mathcal{I}}$ has rank $r$, we have that $\text{rank}(\mathcal{A}_{\mathcal{I}}) = \text{rank}(\mathcal{A})$. Further, since  $\mathcal{A}_{\mathcal{I}}$ is a sub-matrix of $\mathcal{A}$, linear span inclusion (i.e., $\mathcal{E}_3$) holds with probability at least $ 1 - \gamma/4$. Next, we show that subspace inclusion holds. 

\begin{lemma} [Subspace Inclusion] 
\label{lem:experiment_design_subspace_inclusion}
Let the set-up of Theorem \ref{thm:experiment_design_assumptions_hold} and \eqref{eq:experimental_design_donor_set_balanced_spectrum} hold. Then, $\E[\bY_{\mathcal{I}}^{(\pi)}]$ lies in the row-space of $\E[\bY_{\mathcal{I}}^{(\Pi_N)}]$ holds with probability at least $1 - \gamma/4$. That is, subspace inclusion (Assumption \ref{ass:rowspace_inclusion}) holds with high probability.  
\end{lemma}

\noindent Let $\mathcal{E}_5$ denote the event that \eqref{eq:experimental_design_donor_set_balanced_spectrum} holds. As a result of Lemma \ref{lem:experiment_design_subspace_inclusion}, we have that $\P(\mathcal{E}_4 ~ | ~ \mathcal{E}_5) \geq 1 - \gamma/4$. Additionally, note that $\mathcal{E}_3 \subset \mathcal{E}_5$. Hence, we have
\begin{align}
    \P(\mathcal{E}_V) & = \P(\mathcal{E}_3 \cap \mathcal{E}_4)  \geq \P(\mathcal{E}_4 \cap \mathcal{E}_5) \nonumber \\
    & = \P(\mathcal{E}_4 ~ | ~ \mathcal{E}_5) \P( \mathcal{E}_5) \nonumber \\
    & \geq (1 - \gamma/4)^2 \geq 1 - \gamma/2 \label{eq:experiment_design_vertical_regression_hold}
\end{align}
...
We begin by defining some necessary notation. For a given  unit $u \in \mathcal{I}$, and let $\mathcal{S}_u = \{S \subset [p] ~ | ~ \alpha_{u,S} \neq 0 \}$. That is, $\mathcal{S}_u$ denotes the subset of coordinates where $\balpha_u \in \mathbb{R}^{2^p}$ is non-zero. Note that by Assumption \ref{ass:observation_model},  $|S_u| \leq s$. For any combination $\pi$, let $\bchi_{\mathcal{S}_u}^{\pi} = [\chi^{\pi}_{S} : S \in \mathcal{S}_u] \in \{-1,1\}^{|\mathcal{S}_u|}$ denote the projection of $\bchi^\pi$ to the non-zero coordinates of $\balpha_u$.  For example, if $\balpha_u = (1,1, \ldots ,0)$ and $\bchi^{\pi} = (1,1,\ldots ,1)$, then $\bchi_{\mathcal{S}_u}^{\pi} = (1,1)$. Finally, let $\bchi_{\mathcal{S}_u}(\Pi_{\mathcal{I}}) = [\bchi_{\mathcal{S}_u}^{\pi}: \pi \in \Pi_{\mathcal{I}}] \in \{-1,1\}^{|\Pi_{\mathcal{I}}| \times |\mathcal{S}_u|}$. \\
...
Let $A$ be an $N \times s$ matrix whose rows $A_i$ are independent isotropic random vectors in $\mathbb{R}^s$. Let $m$ be such that $\norm{A_i}_2 \le \sqrt{m}$ almost surely for all $i \in [N]$. Then, for every $t \ge 0$, one has
...
&= \left\langle \chi_{S}, \chi_{S'} \right\rangle_B = \mathbbm{1}\{S=S'\}.
\end{align*}
...
\label{eq:2_norm_fourier_coefficient}
    \norm{\balpha_n}_2  =  2^{-p/2}\norm{\bchi(\Pi)\balpha_n}_2 =  2^{-p/2}\norm{\E[\bY_n^{(\Pi)}]}_2 \leq 1 
\end{equation}
...
\[\left\|\frac{1}{q}X^{\top}X - \Sigma \right\| \le t\sqrt{\frac{m\norm{\Sigma}}{q}} \vee \frac{t^2 m}{q}\]
Here $c > 0$ is an absolute constant.
\end{theorem}


\noindent Then, we can apply Theorem \ref{thm:matrix_bernstein} with $X = \mathcal{\tilde A_{\mathcal{I}}}$, $q = |\mathcal{I}|$, $m = 1$ (obtained from \eqref{eq:2_norm_fourier_coefficient}), $\Sigma = \E[\tilde\balpha_u\tilde\balpha_u^\top]$, $\norm{\Sigma} = C/r$ (obtained from \eqref{eq:fourier_matrix_singular_value_bound}), and $t = \sqrt{\log(rs/\gamma)/c'}$ to give us that 
\begin{equation}
...
\end{equation}
holds with probability at least $1-\gamma/8$. Then using Weyl's inequality (Theorem \ref{thm:weyl_inequality}), \eqref{eq:fourier_matrix_singular_value_bound}, and \eqref{eq:matrix_bernstein_fourier_matrix_bound}, we have 
\begin{align*}
...
\end{align*}
Choosing $|\mathcal{I}| \geq Cr\log(rs/\gamma)$ for an appropriate constant $C > 0$ gives us that 
\begin{equation}
...
\end{equation}
for some universal constant $c''' > 0$ with probability at least $1 - \gamma/8$. To establish the result, observe that $ s_r(\mathcal{\tilde A_{\mathcal{I}}^T}\mathcal{\tilde A_{\mathcal{I}}}/|\mathcal{I}|) =  s_r(\mathcal{ A_{\mathcal{I}}^T}\mathcal{ A_{\mathcal{I}}}/|\mathcal{I}|)$. This completes the proof. 

\subsubsection{Proof of Lemma \ref{lem:experiment_design_subspace_inclusion}}
It suffices to show that $\E[\bY_{\mathcal{I}}^{(\Pi_N)}] = [\E[\bY_{\mathcal{I}}^{(\pi)}]: \pi \in \Pi_N] \in \mathbb{R}^{|\Pi_N| \times |\mathcal{I}|}$ has rank $r$ with high probability. We begin by establishing that $\E[\bY_{\mathcal{I}}^{(\Pi)}]$ has a balanced spectrum. To proceed, note the since $\bchi(\Pi)/2^{p/2}$ is an orthogonal matrix,  $ \E[\bY_{\mathcal{I}}^{(\Pi)}] = \bchi(\Pi)\mathcal{A}_{\mathcal{I}}^T$, and that multiplication by an orthogonal matrix preserves singular values, we have that
\begin{equation*}
...
\end{equation}
for a universal constant $C > 0$. Next, we have that 
\begin{equation}
\label{eq:top_singular_value_donor_set}
 s_1(\E[\bY_{\mathcal{I}}^{(\Pi)}]^T\E[\bY_{\mathcal{I}}^{(\Pi)}]) \leq \lVert \E[\bY_{\mathcal{I}}^{(\Pi)}] \rVert^2_F \leq \frac{C|\mathcal{I
 }|2^p}{r}
\end{equation}
...
\end{equation}
holds with probability at least $1 - \gamma/8$.  Next, using Theorem \ref{thm:weyl_inequality}, \eqref{eq:subspace_inclusion_singular_value_bound}, and \eqref{eq:matrix_bernstein_subspace_inclusion_bound}, we have 
\begin{align*}
...
\end{align*}
Choosing $|\Pi_N|\geq Cr\log(|\mathcal{I}|/\gamma)$ for appropriate constant $C > 0$ implies $ s_{r}(\E[\bY_{\mathcal{I}}^{(\Pi_N)}]^T\E[\bY_{\mathcal{I}}^{(\Pi_N)}]/|\Pi_N|) > 0$ with probability at least $1 - \gamma/8$. This completes the proof. 
