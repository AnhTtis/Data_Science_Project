...
Let $\hat{\E}[\bY_{\mathcal{I}}^{(\Pi_n),{r_n}}] = \sum^{r_n}_{l=1}\hat{s}_l\hat{\mu}_l\hat{v}'_l$ denote the $r_n$ decomposition of $\hat{\E}[\bY_{\mathcal{I}}^{(\Pi_n)}]$. Equivalently, $\hat{\E}[\bY_{\mathcal{I}}^{(\Pi_n),{r_n}}] = \hat{\bU}_{\mathcal{I}}^{(\Pi_n)}\hat{\bS}_{\mathcal{I}}^{(\Pi_n)}(\hat{\bV}_{\mathcal{I}}^{(\Pi_n)})^T$. Let $\bepsilon^{\Pi_n} = [\epsilon_n^{\pi}: \pi \in \Pi_n] \in \R^{|\Pi_n|}$. \\
