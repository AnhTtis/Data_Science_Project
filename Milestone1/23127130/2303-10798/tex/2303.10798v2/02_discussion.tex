...
\caption{\label{fig:perfect}A substitution system based on converged
	...
	tile have unit length, all tile edges except the two adjacent to 
	triskelion centres
...
\caption{\label{fig:alt_subst}An alternative substitution system that
	...
	rules that replace a single hat by a cluster labelled $H_7$ and a
	compound by a cluster labelled $H_8$.  Two additional iterations of
...
hat is merged with a specific neighbour in its shell, as shown in
the upper left, to form a larger two-hat compound.  We can then
...
The two-hat compound's cluster is always equivalent to the union of an
$H$ metatile, a $T$ metatile, and an $F$ metatile, but the hat's cluster
corresponds
...
\caption{\label{fig:gargantuan}An excerpt from a very large patch generated
	...
	to highlight the interlocking tree structures formed by the triskelions
