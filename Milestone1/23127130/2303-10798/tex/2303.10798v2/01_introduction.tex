...
tiling~\cite{penrose_epsilon, baakegahlergrimm2012,taylornotes}. Like the Trilobite and Crab tiles~\cite{ChaimTC}, these can be adjusted so that an arbitrarily high fraction of the area lies in copies of just one kind of tile, but no matter how thin or small, the other tiles remain  necessary.
...
Watson and Whittaker recently produced a hexagon marked with elegant and simple edge-to-edge ``orientational'' rules, that nonetheless cannot be encoded as a single unadorned shape~\cite{WW21}. 
...
$\mathbb{R}^3$, with tilings that have a screw motion but not
...
these concepts coincide~\cite[Theorem~3.7.1]{GS} and we do not need to make this distinction.
 ...
 Recently, Greenfeld and Tao~\cite{GT2} showed that in sufficiently 
