...
%This is the first survey on the relationship between properties of data and the resulting models' robustness against adversarial attacks,
Papers for this survey were collected in June 2022. We used the search query schematically described below,
...
top 10 journals according to the Journal Citation Reports (JCR)~\cite{JournalRanking:website:2022}
in the area of Artificial Intelligence (AI),
which includes Machine Learning, Computer Vision, and Computation Linguistics,
and top five journals in the area of Information System (IS), which includes Security.
...
The first three columns in Table~\ref{tbl:venues} shows the final list of publication venues that we selected.
...
As some venues are only partially indexed by the digital libraries, % or bibliography databases,
...
as described in Section~\ref{sec:constraints} of the Appendix.
We also used Google Scholar to search the ArXiv repository and filtered the results using \emph{publication venue} information provided by Semantic Scholar API~\cite{SemanticScholar:api:2022}.

The second to last column in Table~\ref{tbl:venues} shows the number of hits for each publication venue identified
by this search. In total, using this procedure, we identified 4,429 papers;
after removing duplicated, we obtained a set of 3,089 potentially relevant papers.

%
% \emph{ACM Digital Library}, \jr{all these footnotes need to be turned into citations, not removed}
%  %\footnote{\url{http://dl.acm.org.}},
% \emph{IEEE Xplorer}, %\footnote{\url{http://ieeexplore.ieee.org.}},
% \emph{Springer}; %\footnote{\url{https://link.springer.com.}},
% the bibliography database \emph{Scopus};
% %\footnote{\url{https://www.scopus.com.}},
% and the search engine \emph{Google Scholar}. %\footnote{\url{https://scholar.google.com.}}.
%We intentionally excluded the digital library  \emph{ScienceDirect}
%%\footnote{\url{https://www.sciencedirect.com.}}
%as it automatically expands search queries through stemming and lemmatization\footnote{\url{https://service.elsevier.com/app/answers/detail/a_id/35970/supporthub/sciencedirect/}}.
...
Four authors of this survey, again, independently read the categorized each of these papers, and all authors met to discuss the results.
...
%Based on our discussion, we further refined the selection standards.
...
%\vspace{-0.05in}
\begin{enumerate}
\item[(a)] discuss adversarial evasion attacks, but focus on features~\cite{Tong:Li:Hajaj:Chen:Xiao:Zhang:Vorobeychik:USENIX:2019}, models~\cite{Ghosh:Losalka:Black:AAAI:2019,  Xu:Chen:Liu:Chen:Weng:Hong:Lin:IJCAI:2019}, and training algorithms~\cite{Guo:Chen:Chen:Zhang:TPAMI:2021, Jeong:Shin:NeurIPS:2020} rather than data (47.2\%);
\item[(b)] discuss aspects of ML that are not related to adversarial robustness but rather related to
    ...
    %e.g., natural corruptions~\cite{Mintun:Kirillov:Xie:NeurIPS:2021} and domain adaptation~\cite{Xu:Zhang:Zhang:Wang:Tian:CVPR:2021}.
	privacy~\cite{He:Yang:CCS:2021, Hu:Salcic:Sun:Dobbie:Yu:Zhang:CSUR:2022}, and
	interpretability~\cite{Ghorbani:Abid:Zou:AAAI:2019, Guo:Mu:Xu:Su:Wang:Xing:CCS:2018} (39.7\%); %\gx{in this case, we actually excluded such work regardless if it's related to ``data'' or not.}
\item[(c)] propose new robustness evaluation metrics~\cite{Cheng:Deng:Zhao:Cai:Zhang:Feng:ICML:2021}
	or assessment frameworks~\cite{Ling:Ji:Zou:Wang:Wu:Li:Wang:SP:2019} (2.7\%);
\item[(d)] focus on poisoning~\cite{Zhang:Zheng:Gao:Miao:Su:Li:Ren:IJCAI:2019, Liu:Si:Zhu:Li:Hsieh:NeurIPS;2019} rather than evasion attacks (2.6\%); or
\item[(e)] got matched accidentally and discuss unrelated topics, e.g.,
	...
	e.g., blockchain systems need to be robust against adversarial selfish miners or Denial-of-Service attacks (6\%).
\end{enumerate}
...
Following this process, we identified 199 papers as either \emph{relevant} or \emph{unsure}.
...
Our analysis resulted in 54 \emph{relevant} papers. The distribution of these papers by publication venues is shown in the last column of Table~\ref{tbl:venues}.

We further assigned a second reader to each identified relevant paper, to extract and summarize its main findings.
To make sure that we included most of the relevant works on the topic, we also performed backward snowballing using the related work sections of the selected papers, which resulted in 3 additional papers, bringing our selection to 57 papers in total.
Fig.~\ref{fig:collection} summarizes our paper selection process.

\begin{figure*}[b]
    \vspace{-0.1in}
	\centering
	\includegraphics[scale=0.4]{images/paperCollectionStats.pdf}
	\vspace{-0.1in}
...
\centering
    \begin{minipage}{0.46\textwidth}
        \centering
        \includegraphics[width=0.75\textwidth]{images/Selected_paper_by_venue.pdf} % first figure itself
        \vspace{-0.1in}
    ...
    \begin{minipage}{0.46\textwidth}
        \centering
        \includegraphics[width=0.75\textwidth]{images/Selected_paper_by_year.pdf} % second figure itself
        \vspace{-0.1in}
        \caption{Breakdown by publication time.}
        \label{fig:selection_by_year}
    \end{minipage}
    \vspace{-0.15in}
\end{figure*}

Fig.~\ref{fig:selection_by_venue} shows another view on the distribution of publication venues for all 57 papers included in our survey.
The majority of the papers (50, 85\%) are published in Machine Learning venues.
In fact, only the \emph{Advances in Neural Information Processing Systems (NeurIPS)} conference published 19 (around 36\%)
of all papers.
Seven papers are from the Computer Vision venues and only two are from the Security venues.
We found no relevant papers in the \emph{Computational Linguistics} venues. % that discuss the direct influence of properties of training data on adversarial robustness.

Fig.~\ref{fig:selection_by_year} shows the distribution of selected papers by their publication year.
The figure suggests that mainstream research communities started to investigate the impact of training data on adversarial robustness as recently as 2018. We did not find many relevant papers from 2022 as our search was conducted in June 2022.

%\jr{if we need space, this two can safely go.}
...
%, where the attributes identified by each of the authors are unified and refined through discussion, then followed by validating on different set of papers, until the scheme can fully characterize the collected papers.
...
The detailed categorization of each papers along the attributes of this schema can be found in Section~\ref{sec:appendix-categorization} of the Appendix.
...
\begin{itemize}
	...
	e.g., error-rate-based % or (the expected error rate under attacks with a fixed perturbation budget) and
	radius-based. %(the distance to the closest adversarial example).
%	\st{and rank-based (the expected change in ranks under attacks for models that make predictions based top ranked instances).}
...
\vspace{0.02in}
\noindent \textbf{3. Practicality} specifies how to apply the approach or technique introduced in each paper: %Specifically,
\begin{itemize}
	...
	%If a paper presents an empirical validation, we collect the detailed information about the
\end{itemize}
