...
\vspace{-0.1in}
  ...
  \begin{minipage}{0.44\textwidth}
	\centering
	\vspace{-0.1in}
	\includegraphics[width =\textwidth]{images/distribution_vertical_legend.pdf}
	\vspace{-0.25in}
...
\vspace{0.05in}
\noindent
\roundrect{1} {\bf Type of Distributions}.
...
a unit-cube-like to a unit-sphere-like distribution and increases for CIFAR-10 when going the opposite way;
...
In this setting, low feature variance implies that the feature has a strong correlation with the class and
perturbing this feature unlikely result in vulnerability (i.e., will likely result in a semantically-meaningful change).
...
They further prove that even the optimal classifier is non-robust when the underlying dataset has strong asymmetry,
as in the
...
Specifically, Pang et al.~\cite{Pang:Du:Zhu:ICML:2018} show that,
for Linear Discriminant Analysis (LDA) classifiers trained on Gaussian mixtures, the robustness radius of LDA
is proportional to the distance between Gaussian centers.
...
Wan et al.~\cite{Wan:Chen:Yu:Wu:Zhong:Yang:TPAMI:2022} modify the DNN loss function to compute the centers of the Gaussians directly while generating symmetric \mbox{Gaussian feature distributions}.
