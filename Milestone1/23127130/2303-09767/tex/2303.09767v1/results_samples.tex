...
\roundrect{3} papers proposing techniques to deal with data imbalance, i.e., unequal number of samples in different classes.

\begin{figure*}[t]
	\centering
	\includegraphics[width=0.9\linewidth]{images/number_of_samples_vertical_legend_wide_updated.pdf} %samples
	\vspace{-0.1in}
...
Because AdvSNR is always no greater than SNR for a given dataset, it follows that
...
The authors show that, when the number of samples is greater than data dimensionality,
there exists a trade-off between adversarial and standard risks.
Moreover, this trade-off improves as the number of samples per dimension increases.
%d > n: no tradeoff
...
%\jr{Shubhraneel-P34}
Similarly, Gourdeau et al.~\cite{Gourdeau:Kanade:Kwiatkowska:Worrell:JMLR:2021} show that,
...
the sample complexity is proportional to the data dimensionality $d$.
...
\vspace{0.05in}
\noindent
\roundrect{2} {\bf Techniques for Resolving the Sample Complexity Gap}.
...
\vspace{0.05in}
\noindent
\roundrect{3} {\bf Data Imbalance}.
Wu et al.~\cite{Wu:Liu:Huang:Wang:Lin:CVPR:2021} analyze adversarial robustness of DNNs on long-tail distributions: setups where the training data contains a large number of classes with few samples.
