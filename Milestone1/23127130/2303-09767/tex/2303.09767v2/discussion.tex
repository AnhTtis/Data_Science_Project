...
We then outline the gaps in research and possible directions for future work 
%investigating theoretical foundations and practical implications of the influence of data on adversarial robustness. 
...
%The use of fine labels is particularly important during adversarial training, 
%as this allows the model to calibrate the label distribution 
%and mitigate the impact of adversarial perturbations~\cite{Dong:Liu:Shang:NeurIPS:2022}.
...
%\noindent
%\emph{3. .} 
%Apart from transforming the original data representation, 
%enforcing the desired data properties can be performed on latent data representations. 
%by designing the appropriate loss functions.
...
%\replace{Constraining model weights using $L_1$ norm instead of the commonly used $L_2$ or $L_\infty$ norms can reduce the effects of high dimensionality~\cite{Yin:Kannan:Bartlett:ICML:2019}.}{} 
%\jr{is that a separate though or a continuation of the previous one?
%In the former case, connect. In the latter -- how do these differ?}

\vspace{0.02in}
\emph{2. Training and inference procedures.}
%Adversarial vulnerability arising from undesired data properties can also be mitigated by altering the training and inference procedures.
%During training, assigning higher weight to under-represented classes can decrease the robustness bias in imbalanced datasets~\cite{Wang:Xu:Han:Xiaorui:Yaxin:Thuraisingham:Tang:ArrXiv:2021}.
...
%The use of fine labels is particularly important during adversarial training, to calibrate the label distribution and ensure the perturbations do not change the semantic meaning of samples~\cite{Dong:Liu:Shang:NeurIPS:2022}.
...
\vspace{0.02in}
\noindent
{\bf Influence of Data Properties on Common Model Types.}
While some works discuss data properties under a classifier-agnostic setting, e.g.,~\cite{Schmidt:Santurkar:Tsipras:Talwar:Madry:NeurIPS:2018}, 
others focus on the impact of data properties for specific types of models, e.g.,~\cite{Amsaleg:Bailey:Barbe:Erfani:Furon:Houle:Radovanovic:Nguyen:TIFS:2021}.
A classifier-agnostic setting is useful for predicting best-case robustness performance.
Yet, making specific assumptions about the types and properties of models can lead to tighter and more pragmatic bounds.
Additional future work conducting targeted analysis for popular ML algorithms can have notable \mbox{practical significance}.
...
This implies that achieving robust generalization may come at the cost of standard generalization.
...
%\vspace{0.05in}
%\noindent
%\ad{{\bf Data Quality Monitoring for Model Robustness.} \mt{Removing this, as I think it is a bit wishy washi}
%Our survey identified key data properties that influence the adversarial robustness of the resulting classifier.
%There is a rising recognition to the quality of training data for building ML models more efficiently; a number of frameworks~\cite{Wang:Zeng:Jin:Jia:ArXiv:2021} and theoretical studies~\cite{Ilyas:Park:Engstrom:Leclerc:Madry:ICML:2022} were presented on data quality assessment.
%The current data quality monitoring systems mainly focus on business-value-related aspects. % I think this is vague.
%Integrating data valuation from the robustness perspective into existing data quality monitoring systems can further enhance the security and reliability ML pipelines.}
...
%		- Improving robustness by Manipulating data
%					- Increasing dataset size and diversity
%							- unlabeled, generated, frequency diversity, fine labels
%					- cleaning and transforming data (feature selection, loss functions)
%							- feature selection, dimensionality reduction (pca), graph theory, loss functions increase inter-class distance, simulate symmetric gaussians,
%		- Improving robustness by Manipulating learning procedure
%			 	- Model selection and configuration
%			 			- probabilistic models, scale invariant classifiers, dimensionality for parameters and hyper-parameters, using L1 norm
%			 	- Training and inference
%			 			- different tasks concurrently, purify samples
%		- Estimating robustness
%		
%	- Knowledge gaps and future directions
%		x - Empirical evaluation (image dataset, simple setups)
%		x - Interdependence of properties
%		x - Additional data properties (ones for standard generalization)
%		x - Standard vs robust generalization
%		x - Simplified problem setup
%		x - Inconsistent measurement techniques
%		x - Data properties (density, separation) to configure classifiers
%		x - Observed vs "real" data properties
%		- Prioritizing unlabeled samples
%		x - Pragmatic studies on commonly used types of models
...
increasing dataset size and diversity through selection of natural or generated samples. 
...
%\subsection {Summary \ad{of Implications and Future Work}}
%\revadd{An improvement in adversarial robustness can be achieved by manipulating data with techniques for 
%increasing \replace{the number of samples} (using real or generated data),
%feature selection and dimensionality reduction, and learning a latent data representation with desirable data properties. 
%\jr{what about estimations, i.e., what they are and what are they good for.}
%\ad{Moreover, techniques to estimate achievable robustness can be used to gauge how much the robustness observed for a particular dataset can be improved.}
%For future work, there is a lack of empirical evaluation on how data properties affect robustness beyond image datasets. 
%In work that theoretically evaluates the relationship of data on robustness, most papers draw conclusions on synthetic datasets which have unrealistic assumptions that are not applicable to datasets used in practice.
%\ad{However, pragmatic studies on commonly used types of models can have larger practical significance.}
%There is also a lack of work on the inter-play of different data properties. In practice, it may be infeasible to modify a data-property without affecting others.
%\ad{Moreover, the influence of additional data properties that influence standard generalization, such as overlapping and outlier samples, are not well-studied for robust generalization.
%	Additionally, there are gaps in research on existence of accuracy-robustness trade-off, the significance of data compared to other sources of vulnerability, and the impact of data quality on robustness.
%	%Data vs other sources of adversarial vulnerability.
%	%Quality of cheaper alternative samples.
%	%Training data vs data distribution.
%}
%Lastly, some properties such as separation do not have a standard metric. \jr{why this is lastly? seems to omit several subsections. please make it complete.} \mt{added.}
%We hope the findings of this survey will serve