...
and~\roundrect{2}~papers explaining adversarial vulnerabilities using perceptual differences between humans and models. % frequency information.
...
\begin{figure*}[b]
  ...
  \begin{minipage}{0.62\textwidth}
		\centering
		  \vspace{-0.2in}
		\includegraphics[valign=t, width =\textwidth]{images/domain_specific_citation_graph.pdf} %0.56
		\vspace{-0.07in} 
...
models can utilize both %low-frequency and high-frequency 
...
\revadd{Deng and Karam~\cite{Deng:Karam:ECCVWorkshops:2020, Deng:Karam:TIP:2022} also acknowledge and 
...
%adversarial training succrifies accuracy for high-frequency images
\revadd{Zhang et al.~\cite{Zhang:Benz:Karjauv:Kweon:AAAI:2021} 
...
%  smaller margins along low-frequency vs. high-frequency ranges
% adversarial training results in significantly larger margins for high-frequency ranges --> model is vurnable to low-frequency purtrubation 
...
standard models utilize both. % low- and high-frequency components, 
...
% we know so far that adversarial training process is focusing on high-frequency components and, as a result

%While all previous works show that adversarial training process is focusing on high-frequency components,
%and, as a result
%adversarially trained models rely more heavily on low-frequency components,
%a number of works show that models are also vulnerable to perturbations in low-frequency components.}
%\ad{A number of works focus on the adverse consequences of adversarially trained models which focus more low-frequency components.} 
%Specifically,


%adversarial training allows low-frequency attacks
\revadd{Yin et al.~\cite{Yin:Lopes:Shlens:Cubuk:Gilmer:NeurIPS:2019} outline another disadvantage 
...
%, who analyze CNN robustness. % through the classifier margins along particular frequencies, which the authors define
...
%Like previous works, the authors also suggest training more robust models that enlarge margins along low-frequency ranges by augmenting training datasets with perturbations concentrated in those ranges., 


%\revreplace{To mitigate this issue, the authors propose increasing the diversity of frequency distribution in augmentations.}{}
%randomized smoothing allows low-frequency attacks
\revadd{Sun et al.~\cite{Sun:Mehra:Kailkhura:Chen:Hendrycks:Hamm:Mao:ECCV:2022} also observe the issue 
...
\revadd{Focusing on medical image classification, Ma et al.~\cite{Ma:Niu:Gu:Wang:Zhao:Bailey:Lu:PR:2021} show that image classifiers pre-trained on natural image datasets, such as ImageNet, have lower adversarial robustness when applied to medical images. 
