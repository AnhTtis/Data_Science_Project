...
Papers for this survey were collected \revreplace{in June 2022}{in May 2023}.
...
the top \revreplace{10}{15} journals according to the Journal Citation Reports (JCR)~\cite{JournalRanking:website:2022}
...
We also used Google Scholar to search the ArXiv repository and filtered the results using \emph{publication venue} information provided by the Semantic Scholar API~\cite{SemanticScholar:api:2022}.

The second to last column in Table~\ref{tbl:venues} shows the number of hits for each publication venue identified
by this search. In total, using this procedure, we identified \revreplace{4,429}{6,390} papers;
after removing duplicates, we obtained a set of \revreplace{3,089}{4,359} potentially relevant papers.
...
rather than data (\revreplace{47.2}{44.2}\%);
\item[(b)] discuss aspects of ML that are not related to adversarial robustness but rather related to
	...
	(\revreplace{39.7}{41.3}\%); %\gx{in this case, we actually excluded such work regardless if it's related to ``data'' or not.}
\item[(c)] propose new robustness evaluation metrics~\cite{Cheng:Deng:Zhao:Cai:Zhang:Feng:ICML:2021}
	or assessment frameworks~\cite{Ling:Ji:Zou:Wang:Wu:Li:Wang:SP:2019} (\revreplace{2.7}{3.1}\%);
\item[(d)] focus on poisoning~\cite{Zhang:Zheng:Gao:Miao:Su:Li:Ren:IJCAI:2019, Liu:Si:Zhu:Li:Hsieh:NeurIPS;2019} rather than evasion attacks (\revreplace{2.6}{2.7}\%); or
...
As the inclusion/exclusion labels were consistent among the raters, we decided to proceed to the next phase: distribute the remaining \revreplace{3,009}{4,279} papers among the four authors and categorize them using these inclusion and exclusion criteria. In this phase, we instructed each rater to conservatively mark as \emph{unsure} papers with even a slight doubt in categorization.

Following this process, we identified \revreplace{199}{277} papers as either \emph{relevant} or \emph{unsure}.
...
Our analysis resulted in \revreplace{54}{71} \emph{relevant} papers. The distribution of these papers by publication venues is shown in the last column of Table~\ref{tbl:venues}.
...
using the related work sections of the selected papers, which resulted in \revreplace{3}{6} additional papers, 
bringing our selection to \revreplace{57}{77} papers in total.
...
\centering
    \begin{minipage}{0.46\textwidth}
        \centering
        \includegraphics[width=0.75\textwidth]{images/Selected_paper_by_venue_update.pdf} % first figure itself
        \vspace{-0.1in}
    ...
    \begin{minipage}{0.46\textwidth}
        \centering
        \includegraphics[width=0.75\textwidth]{images/Selected_paper_by_year_updated.pdf} % second figure itself
        \vspace{-0.1in}
...
Fig.~\ref{fig:selection_by_venue} shows another view on the distribution of publication venues for all 
\revreplace{57}{77} papers included in our survey.
The majority of the papers \revreplace{(50, 85\%)}{(64, 83\%)} are published in Machine Learning venues.
In fact, only the \emph{Advances in Neural Information Processing Systems (NeurIPS)} conference published 
\revreplace{19 (around 36\%)}{25 (around 32\%)}
of all papers.
\revreplace{Seven}{Eleven} papers are from the Computer Vision venues and only two are from the Security venues.
...
The figure suggests that mainstream research communities started to investigate the impact of training data on adversarial robustness as recently as 2018. We did not find many relevant papers from \revreplace{2022}{2023} as our search was conducted in early \revreplace{June 2022}{May 2023}.
