...
The $c$-clustered independence number $\alpha_c(G)$ has been considered under the name \emph{$t$-component stability number} by Broutin and Kang~\cite{BK18} in 2018 as a tool to give a lower bound on $\chi_c(G)$ (specifically for the Erd\H{o}s-R\'enyi random graph $G_{n,p}$) by using inequality~\eqref{eq:trivial-LB}.
Apart from that, $\alpha_c(G)$ has (to the best of our knowledge) not been further considered or investigated.
But there is a number of equivalent or closely related concepts in the literature.
