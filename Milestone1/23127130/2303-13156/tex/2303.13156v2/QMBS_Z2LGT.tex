...
\begin{document}
	...
	One typical class of QMBS states are constructed by spectrum-generated algebras~\cite{buvca2019non,PhysRevB.102.085140,PhysRevB.102.075132,Moudgalya_2022},
	...
	Thus, as a typical instance, lattice gauge theories (LGTs)~\cite{PhysRevD.10.2445,RevModPhys.51.659,PhysRevB.65.024504,PhysRevD.17.2637,PhysRevD.19.3682,Fradkin2013,PhysRevA.73.022328,
		PhysRevLett.109.175302,di2019resolution,Magnifico2020realtimedynamics,banuls2020simulating,PhysRevResearch.3.023079,PhysRevA.103.053703,PRXQuantum.3.010324,Halimeh2023robustquantummany,PhysRevB.107.L201105,PhysRevB.107.205112}
	...
	which is closely related to non-thermal states~\cite{PhysRevLett.122.130603, PhysRevB.99.195108, PhysRevX.10.021041,
	...
	where a particle and an antiparticle are always bounded into a stable meson.
	...
	We identify the instability of mesons in $\ket {\Psi_{n,l}}$ by calculating the  gauge-invariant spin exchange correlation function of the matter field.
	Our results show that mesons are stable for small $l$, while these become unstable for large $l$.
     Moreover, we propose a feasible approach to detect the crossover between stable and unstable mesons in quantum simulators,
...
the numerical results are obtained from the Hamiltonian (\ref{Hdual}) by exact diagonalization.
	...
	\begin{align} \label{omega}
		\ket{\Omega} :=\ket{\downarrow\downarrow...\downarrow\downarrow}\otimes\ket{++...++},
	\end{align}
   ...
   An arbitrary gauge invariant basis of the Hamiltonian (\ref{H}) can be written as
    ...
    Here, each string excitation contains two spin charges, so the string number $n$ equals half of  total spin charges.
...
Then, we introduce another operator~\cite{PhysRevB.101.195131} $\hat L^\dagger_m =\sum_{j}  \big(\sum_{k\leq m}\prod_{\ell \leq k}\hat {\mathcal{P}}^-_{j+\frac{1}{2}-\ell} \big)\hat \sigma^-_{j}\hat \tau^{\text{M}}_{j+\frac{1}{2}}\hat \sigma^+_{j+1}$,
   ...
   Thus, intuitively, as the total string length $l$ increases, the stability of mesons for the scar state $\ket {\Psi_{n,l}}$ is expected to be significantly broken.
    ...
    \begin{figure}[t] \includegraphics[width=0.48\textwidth]{Fig2.pdf}
    	\caption{ Stability of mesons. (a) Expectation values of the electric field for all eigenstates at half filling with $L=16$ and $h=0.5$.
    	The red scatters correspond to scar states. 
    	...
    	The black dashed line is for the fit: $|C_{\sigma}(r)|\sim 1/r^\alpha$, where $\alpha\approx0.35$.
         (d) The half-chain correlation function $C_{\sigma}(L/2)$ versus the ratio $l/L$ for different system sizes with $n=L/4$.}
    	\label{fig_2}
    \end{figure}
    ...
    Previous works have shown that almost all of the nonthermal dynamics in LGTs originate from stable meson excitations~\cite{PhysRevLett.122.130603, PhysRevB.99.195108, PhysRevX.10.021041,PhysRevB.101.024306,PhysRevLett.124.207602, ISI:000395814000014,birnkammer2022prethermalization},
    including the revival dynamics in the \textit{PXP} model~\cite{PhysRevX.10.021041}.
    ...
    Hence, intuitively, isolated $\sigma$-spin charges are expected to exist in this case, leading to the instability of mesons.
    ...
    This suggests that mesons in these scar states would be stable.
    However, for large-$l$ scar states, $\braket{\hat M_x}$ is located in the main spectrum,
    implying that mesons may be unstable.
    ...
    For $\ket{\Psi_{L/4,L/4}}$, we can find  that $C_{\sigma}(r)$ approximately exhibits an exponential decay with increasing $r$, see Fig.~\ref{fig_2}(b).
    It indicates that $\sigma$-spin charges  tend to form stable bound states (i.e., mesons) for small-$l$ scar states~\cite{PhysRevLett.127.167203}.
    However, $C_{\sigma}(r)$ can exhibit a power-law decay for the state $\ket{\Psi_{L/4,L/2}}$,
    showing that the isolated $\sigma$-spin charges emerge for large $l$, see Fig.~\ref{fig_2}(c).
    Thus, the instability of mesons in QMBSs $\ket{\Psi_{n,l}}$ arises, when increasing $l$.
    Therefore, there exists a mesonic-nonmesonic crossover for these QMBSs, see Fig.~\ref{fig_2}(d).
	Figure \ref{fig_2} also reveals that the stable meson excitation is not a necessary condition to violate the ETH in LGTs.
	Note that the meson instability is a collective effect, which only emerges in many-particle systems, 
  ...
  It is obvious that both initial states host low entanglement entropies. Specifically, with the dual transformation in Eq.~(\ref{Hdual}), $\ket{\psi_{1}}$ is related to the ground state of the \textit{PXP} model~\cite{PhysRevB.101.024306}, and $\ket{\psi_{2}} =\bigotimes_j \ket{V_{2j+\frac{1}{2},2j+\frac{3}{2}}}$, with $\ket{V_{2j+\frac{1}{2},2j+\frac{3}{2}}}=(\ket{++}-\ket{+-}+\ket{-+}+\ket{--})/2$.
   ...
   Moreover, the revival dynamics signals the ETH violation for QMBSs  $\ket{\Psi_{n,l}}$.
    ...
    to identify whether meson excitations are stable during the quench dynamics.
    For $\ket{\psi_{1} }$, we can find that $\sigma$-spins  are always short-range correlated,
    and $C_{\sigma}(r)$ exhibits an exponential decay as $r$ increases during the quench dynamics, see Figs.~\ref{fig_4}(a,b).
    ...
    where $\sigma$-spins can be long-range correlated
    [$C_{\sigma}(r)\sim \text{const} $ for $r\rightarrow \infty$] at the specific time, see Figs.~\ref{fig_4}(c,d).
    Mesons in this case are unstable and can be decomposed into isolated spin charges.
   ...
   The preparation of the initial states in Eq.~(\ref{psi0}) in dual systems  is convenient in quantum simulators.
    In addition, the dual Hamiltonian (\ref{Hdual}), with three-body interaction terms, has also been realized by quantum gates, e.g, in superconducting circuits~\cite{arXiv:2203.08905,zhang2022digital}.
    Therefore, the mesonic-nonmesonic crossover for QMBSs in this $\mathbb{Z}_2$ LGT can be experimentally detected with digital quantum simulations.
     ...
     \textit{Summary.}---%
     We have investigated the instability of mesons in QMBSs of a $\mathbb{Z}_2$ LGT, minimally coupled to a dynamical spin-$\frac{1}{2}$ chain as a matter field.
     ...
     We demonstrate that scar states with a small total string length is described by stable mesons, like conventional nonthermal states in LGTs,
     ...
     Thus, the meson instability in large-$l$ QMBSs is in analogy with the asymptotic freedom of quarks in the high-energy regime of quantum chromodynamics.
      This is an interesting correspondence that deserves further study.
     The conventional QMBSs are generated by local operators, which correspond to non-fractionalized excitations~\cite{doi:10.1146/annurev-conmatphys-031620-101617},
     ...
     However, for nonmesonic scar states in this work, isolated spin charges emerge due to nonlocal generating operators. 
     The corresponding excitations have some analogies with spinons~\cite{Fradkin2013}, which are fractionalized excitations and different from conventional QMBSs.
     ...
     Another interesting issue is whether the above physics can be generalized to other gauge groups or high-dimensional LGTs~\cite{PhysRevB.38.2926,banuls2020simulating}.
    	...
    	We thank M. Dalmonte, Z. Gong, and R.-Z Huang for insightful discussions and useful suggestions.
...
In Fig.~\ref{fig_1}, we show the result of $\mathcal{N}_{n,l}^{-2}$ versus $l$ for $L=32$ and $n=8$ (half filling).
