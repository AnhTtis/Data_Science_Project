...
\icmltitlerunning{Deep Augmentation: Self-Supervised Learning with Transformations in Activation Space}
...
\icmltitle{Deep Augmentation: \\ Self-Supervised Learning with Transformations in Activation Space}
% \icmltitle{Deep Augmentation: Enhancing Self-Supervised Learning through Transformations in Higher Activation Space}
...
\icmlauthor{Rickard Br\"uel-Gabrielsson}{yyy}
\icmlauthor{Tongzhou Wang}{yyy}
\icmlauthor{Manel Baradad}{yyy}
\icmlauthor{Justin Solomon}{yyy}
...
\icmlcorrespondingauthor{Rickard Br\"uel-Gabrielsson}{brg@mit.edu}
...
\begin{itemize}
    ...
    \item Conversely, we observe an inverse effect in supervised learning contexts, potentially due to a ``collapse" phenomenon, akin to an information bottleneck, being beneficial for direct training on downstream tasks.
    ...
    \item Our techniques can ameliorate co-adaptation or ``collapse" among network layers and their resultant latent features, a long-standing challenge in contrastive learning. Based on this observation, we also propose a procedure for selecting the target layer for Deep Augmentation. % and implies that the effectiveness of these methods may in part be attributed to their addressing the ``collapse" issue, a long-standing problem in the field of contrastive learning.
...
\vspace{-.1in}
\caption{SimCSE vs.\ Deep Augmentation with (a) Dropout or (b) PCA, with and without stop-gradient. ``Stop": stop-gradient. Deep Augmentation outperforms SimCSE.\vspace{-.15in}}
%\label{fig:simcsetop}
%\vspace{-0.5cm}
\end{figure}
...
\vspace{-.1in}
\caption{SimCSE vs.\ Deep Augmentation with and without stop-gradient, both with MLM. ``Stop": stop-gradient. *: includes hyperparameter search over dropout rates.\vspace{-.1in}}
...
\vspace{-.1in}
%\caption{Continue Cifar100-Cifar100}
\caption{Comparing dropout rates at all layers versus 50\% dropout rate targeted at a specific layer.\vspace{-.1in}}
...
\end{subfigure}
\vspace{-.05in}
\caption{Contrastive learning. Deep Augmentation with (a) dropout or (b) PCA, with and without stop-gradient. *: initialized with pre-trained SimCLR model. ``Stop" is short for stop-gradient. Note: Layer 5 is an average pooling. \vspace{-.2in}}
...
\end{subfigure}
\vspace{-.2in}
\caption{Supervised only. Deep Augmentation with (a) dropout or (b) PCA, with and without stop-gradient. *: initialized with pre-trained SimCLR model. ``Stop" is short for stop-gradient. \vspace{-.2in}}
...
\begin{figure*}[ht]
\centering
\includegraphics[width=0.7\linewidth]{images/simcse_barsf.pdf}\vspace{-.05in}
...
We compute the CKA similarity index for the Transformer trained under various settings (Figure \ref{fig:simcse_bars}). ``BERT" is the starting point for both SimCSE and Deep Augmentation.
%(``Layer 10 without Stop" and ``Layer 8 with Stop" correspond to Layer 8 with stop-gradient and Layer 10 without stop-gradient, resp.). 
``BERT" has a stretch of co-adaptation between layers 8 through 11 as indicated by the two black crosses. Mirroring ResNet18 on Cifar, it is around these two crosses that Deep Augmentation is most effective, with stop-gradient at the earlier cross and without stop-gradient at the later cross. ``SimCSE" reduces the co-adaptation across layers, especially between layers 11-10-12. In ``Layer 10 without Stop" and ``Layer 8 with Stop" the red cross indicates where Deep Augmentation was applied. ``Layer 10 without Stop" and ``Layer 8 with Stop" supersede SimCSE in reducing co-adaptation among the layers following their application. Thus, CKA similarity index highlights a ``collapse" issue and determines at what layer Deep Augmentation should be applied. Future work may investigate if simultaneously targeting multiple layers further improves performance.
...
\begin{figure}[ht]
\includegraphics[width=\linewidth]{images/Bars1f.pdf}\vspace{-.05in}
...
Our findings address the pervasive issue of co-adaptation or ``collapse" among network layers and their latent features, a long-standing challenge in contrastive learning. Our targeting of specific layers underscores the potential of informed, strategic interventions in network training.
...
\begin{figure}[ht]
    ...
    \begin{minipage}[b]{0.48\linewidth}
        ...
        %\label{fig:image1}
    \end{minipage}
    \hspace{0.1cm} % Space between the images
    % Second image
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        ...
        %\label{fig:image2}
    \end{minipage}
\caption{The six largest principal value from the layers of a random initialized ResNet18 and one trained with SimCLR on Cifar100.\vspace{-.2in}}
\label{fig:PCA-image}
\end{figure}
...
\vspace{-.1in}
%\caption{Continue Cifar100-Cifar100}
\caption{PCA: Comparing sampling 50\% of batch and subtracting the largest principal component from that sample, with and without stop-gradient.\vspace{-.2in}}
...
\vspace{-.1in}
%\caption{Continue Cifar100-Cifar100}
\caption{Supervised on Cifar100: Comparing dropout rates at all layers versus 50\% dropout rate targeted at a specific layer.\vspace{-.2in}}
...
\vspace{-.2in}
%\caption{Continue Cifar100-Cifar100}
\caption{Cifar100: Comparing sampling 50\% of batch and applying dropout to that sample, with and without stop-gradient, for different dropout-rates. ``Stop" is short for stop-gradient.\vspace{-.2in}}
...
\caption{PCA Cifar10: Comparing sampling 50\% of batch and subtracting the largest principal component from that sample, with and without stop-gradient.\vspace{-.2in}}
...
\vspace{-.1in}
%\caption{Continuear100-Cifar100}
\caption{PCA Cifar10: Comparing sampling 50\% of batch and subtracting the sixth principal component from that sample, with and without stop-gradient.\vspace{-.2in}}
...
\vspace{-.1in}
%\caption{Continue Cifar100-Cifar100}
\caption{Supervised on Cifar10: Comparing dropout rates at all layers versus 50\% dropout rate targeted at a specific layer.\vspace{-.2in}}
...
In Figure \ref{fig:simcse-mlm-only-no-contrastive}, we include results with Deep Augmentation and MLM only, without contrastive learning. Deep Augmentation boosts performance substantially.
...
Figure \ref{fig:simcse_all_bars} displays results for various configurations on sentence embeddings and the STS-B development set. Deep Augmentation achieves optimal performance in the later co-adaptation region, with stop-gradient at its onset and without stop-gradient towards its conclusion.
