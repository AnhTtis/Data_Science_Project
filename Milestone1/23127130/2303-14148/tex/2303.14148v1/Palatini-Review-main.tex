...
\label{eq:Ricci_tensor}
    \mathcal{R}_{\mu\nu} = \tns{\mathcal{R}}{^\rho_\mu_\rho_\nu}\,, \quad \tns{\widehat{\mathcal{R}}}{^\mu_\nu}= g^{\rho\sigma}\tns{\mathcal{R}}{^\mu_\rho_\sigma_\nu}\,, \quad \mathcal{R}'_{\mu\nu} = \tns{\mathcal{R}}{^\rho_\rho_\mu_\nu}\,,
\end{equation}
...
\end{equation}
Furthermore, as the homothetic curvature is an antisymmetric $2-$rank tensor, its contraction with the symmetric metric tensor vanishes, so no additional scalar arises\footnote{In Palatini gravity there is also an additional scalar which is linear in the Riemann tensor. It is called Holst invariant~\cite{Holst:1995pc} and is defined as $\widetilde{\mathcal{R}} = \sqrt{-g} \epsilon^{\mu\nu\rho\sigma}\mathcal{R}_{\mu\nu\rho\sigma}$, with $g$ being the determinant of the metric tensor and $\epsilon^{\mu\nu\rho\sigma}$ the totally antisymmetric Levi-Civita symbol, with $\epsilon^{0123}=1$. In the metric formulation it vanishes identically due to the symmetry properties of the Riemann tensor. Recently the interest on the study of the Holst invariant and its influence in inflationary dynamics has been rekindled~\cite{Langvik:2020nrs, Shaposhnikov:2020gts, Shaposhnikov:2020frq, Pradisi:2022nmh, Salvio:2022suk, Gialamas:2022xtt}. }.
...
In many ways, the Palatini formalism is better-behaved and more natural than its metric counterpart. For one, it contains fewer assumptions about the relationships between the gravitational degrees of freedom. In addition, the Riemann tensor \eqref{eq:Riemann}---and thus typical gravity actions---only contain first derivatives of $\tns{\Gamma}{^\rho_\mu_\nu}$ in the Palatini formulation, contrary to its metric counterpart with second derivatives of $g_{\mu\nu}$ through $\partial_\mu \tns{\widetilde{\Gamma}}{^\rho_\nu_\sigma}$ terms. Due to this, careful computations in the metric formulation require adding the York--Gibbons--Hawking boundary term \cite{York:1972sj, Gibbons:1976ue} to produce the correct equations of motion. This term is unnecessary in the Palatini case. In addition, the Palatini formulation of Higgs inflation preserves tree-level unitarity better than its metric counterpart \cite{Barbon2009,Bauer:2010jg,Shaposhnikov:2020geh, McDonald:2020lpz, Mikura:2021clt, Ito:2021ssc, Antoniadis:2021axu, Panda:2022esd}, making the model better behaved from a quantum field theoretic point of view. All these features make the Palatini formulation theoretically appealing.

In this review, we will showcase models of Palatini inflation with non-minimal gravity sectors. They demonstrate the rich model-building potential of Palatini inflation and highlight the CMB predictions of this scenario, such as the tendency for very small values of the tensor-to-scalar ratio $r$. Such models make inflation a viable early-Universe framework even if tensor perturbations are not detected by next-generation CMB experiments. On the other hand, by distinguishing the predictions of the Palatini models from their metric counterparts, CMB observations can, in principle, probe the fundamental degrees of freedom for gravity. The paper does not aim to discuss every Palatini model of inflation found in the literature in detail; instead, we introduce some central concepts and results through examples and point out further references to the interested reader.
...
\epsilon_H = - \frac{\dot{H}}{H^2} = \frac{3 \dot{\phi}^2}{\dot{\phi}^2 + 2 U} \,, \quad \eta_H = - \frac{\ddot{\phi}}{H \dot{\phi}} \, .
\end{equation}
...
\begin{equation} 
\epsilon_U = \frac{1}{2} \left( \frac{U'}{U} \right)^2 \, , \qquad  \eta_U = \frac{U''}{U} \, .
\end{equation}
...
\mathcal{P}_{\zeta}(k) = A_s \left(\frac{k}{k_\star}\right)^{n_s-1}\,, \quad
A_s \simeq 2.1\times 10^{-9} \,, \quad
n_s = 0.9649 \pm 0.0042 \, ,
\end{equation}
where $n_s-1= \dd \ln \mathcal{P}_{\zeta}(k) /\dd \ln k$.
The perturbations have their origin in inflation. Slow-roll inflation, in particular, produces a nearly scale-invariant spectrum ($n_s \approx 1$) compatible with observations. The slow-roll predictions for the spectral strength and tilt are (see e.g. \cite{Lyth:2009zz})
\begin{equation} \label{eq:power_spectrum_sr}
A_s = \frac{1}{24 \pi^2 } \frac{U(\phi_\star)}{\epsilon_U(\phi_\star)} \, , \qquad
...
\mathcal{P}_{T} =  8 \left( \frac{H}{2\pi}\right)^2 \simeq \frac{2 U}{3 \pi^2} \, , \qquad
r \equiv \frac{\mathcal{P}_{T}}{\mathcal{P}_{\zeta}} \simeq 16\epsilon_U < 0.036 \, ,
\end{equation}
where the last limit again comes from CMB observations~\cite{Planck:2018jri,BICEP:2021xfz}.  The tensor-to-scalar ratio is usually calculated at the pivot scale $k_\star = 0.002\, \mbox{Mpc}^{-1}$ in order to compare with the Planck contours shown in figure~\ref{fig:nsr_cons}, while the bound of eq.~\eqref{eq:r} is referred to the scale $k_\star = 0.05\, \mbox{Mpc}^{-1}$. 

To find the pivot scale field value $\phi_\star$, we relate it to the number of e-folds $N_\star$ (defined as $N=\ln a$) between the Hubble exit of $k_\star$ and the end of inflation. To first order in slow roll,
\begin{equation} \label{eq:N_SR}
    N_\star \simeq \int_{\phi_{\rm end}}^{\phi_\star} \frac{{\rm d} \phi}{\sqrt{2\epsilon_U}} \, ,
\end{equation}
where $\phi_{\rm end}$ is the field value at the end of inflation, near the potential minimum, determined from $\epsilon_H \simeq \epsilon_V = 1$. Typically,  $N_\star=50$--$60$, depending on the energy scale of inflation and the post-inflationary expansion history. Below, when we need a more accurate result, we use \cite{Liddle:2003as}
\begin{eqnarray}
...
The energy density degrees of freedom are also  $g_{\star}(T_{\rm reh}) =  \mathcal{O}(10^2)$. The inflaton's energy density $(\rho)$ is given by $\rho = \frac{1}{2}\dot{\phi}^2 + U(\phi)$. Furthermore, $w$ is the equation-of-state parameter during reheating which in this review is taken to be $w=1/3$, \textit{i.e.} the Universe's thermalization  is instantaneous. Having in mind that $U(\phi_\star) \simeq \frac{3\pi^2}{2}A_s r$ and $\rho_{\rm end} = \frac{3}{2} U(\phi_{\rm end})$, Eq.~\eqref{eq:efolds} obtains the simplified form
\begin{equation}
...
\eta_V = \frac{U''}{U} = n \left( n - 1 \right) \frac{1}{\phi^2} \, .
\end{equation}
We then have 
\begin{equation} 
...
\end{equation}
For the $\left( R + \alpha R^2 \right)$ model, \eqref{eq:chi_to_phi} can be integrated to give
...
S = \int \dd^4x \sqrt{-g}\left(\frac{1}{2} A(\varphi) g^{\mu\nu}\Ri_{\mu\nu} - \frac{1}{2} B(\varphi) g^{\mu\nu}\nabla_{\mu}\varphi\nabla_{\nu}\varphi - V(\varphi) \right) \, .
\end{equation}
...
\end{equation}
The non-canonical factors $A$ and $B$ in \eqref{eq:scalar_tensor_SJ} have now been fully absorbed into the new potential $U$.
...
\caption{The normalized Einstein-frame potential $U(\phi)/U_0$ with $U_0 \equiv \lam/(4 \xi^2)$, in the metric (solid curve) and Palatini (dashed curve) formulations of gravity. For illustration purposes, we have set $\xi=1$.}
\label{fig:Higgs_MvsP}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%% END FIGURE %%%%%%%%%%%%%%%%%%%%%%%
In the Palatini case, the result is exact; in the metric case, it is the leading contribution for large $\phi$ when $\xi\varphi^2 \gg 1$ and the potential becomes a plateau, see Fig.~\ref{fig:Higgs_MvsP}. For inflation on the plateau, the observables become
\begin{alignat}{4}
...
\end{equation}
with $A(\vphi)\,, m(\vphi)$ and $V(\vphi)$ model functions to be specified, and $G^{\mu\nu} \equiv \Ri^{\mu\nu} -\frac{1}{2}g^{\mu\nu} \Ri\,.$ A Weyl rescalling of the metric which is usually applied in order to recast the action from the JF to the EF, turns out to be insufficient for~\eqref{eq:NMDC_JF_action} due to the presence of the NMDC term. To this end, one has to resort to the more general class of \emph{disformal} transformations~\cite{Bekenstein:1992pj,Zumalacarregui:2013pma,Minamitsuji:2014waa,Tsujikawa:2014uza,Domenech:2015hka,Sakstein:2015jca,vandeBruck:2015tna,Achour:2016rkg,Gumjudpai:2016ioy,Escriva:2016cwl,Lobo:2017bfh,Sato:2017qau,Galtsov:2018xuc, Karwan:2018eln,Delhom:2019yeo,Galtsov:2020jnu,Qiu:2020csx}, under which, the metric changes according to
\begin{equation}
g _{\mu\nu}= \Omega ^2(\vphi,X) \left[\bar{g}_{\mu\nu} +\beta ^2(\vphi,X) \bar{\nabla} _\mu \vphi\, \bar{\nabla} _\nu \vphi \right] \,, \quad X\equiv-\frac{1}{2} \bar{\nabla} _\mu \vphi \bar{\nabla} ^\mu\vphi\,.
\label{eq:NMDC_disformal_def}
\end{equation}
Once the transformation is applied, one has to determine the functional dependence of $\Omega^2$ and $\beta^2$ on the field $\vphi$ and its velocity $\eps u^2=u_{\mu}u^{\mu}=\bet^2 \left(\bar{\nabla}\vphi\right)^2$ such that the EF action is obtained. For~\eqref{eq:NMDC_JF_action}, the appropriate disformal factors are given by~\cite{Gialamas:2020vto}
\beq 
\Omega^2= \frac{2+ \varepsilon u^2}{2 A(\vphi) \sqrt{1+\varepsilon u^2}}\quad \text{and} \quad \beta ^2 =\frac{1}{m^2(\vphi) \sqrt{1+\varepsilon u^2}}\,.
\label{omega_beta_m}
\eeq
The kinetic and potential terms in the EF action obtained after the disformal transformation, contain prefactors that depend on both $\vphi$ and $X$. In order to separate the two, and be able to compute the inflationary observables, one has to work under the SR approximation and perform an expansion around small values of $X$ (see~\cite{Gialamas:2020vto} for more details). Eventually, we find
\begin{equation}
...
\end{equation}
Equation~\eqref{eq:NMDC_dphi/dvphi} however, cannot always be integrated analytically to yield $\phi\left(\vphi\right)$, and when this is the case, one can utilize the chain rule and compute the inflationary observables directly in terms of the non-canonical field. The first two potential slow-roll parameters and the number of e-folds, in this case, are computed via
\begin{equation}
\epsilon_U
= 
\frac{1}{2 K} 
...
The model functions for a quartic Higgs-like model with a quadratic coupling functional for the NMDC term\footnote{In~\cite{Gialamas:2020vto}, we also considered the case of a constant $m(\vphi)$ and found similar results to the ones presented here.} read as
\beq
...
\eeq
For the model functions~\eqref{eq:NMDC_model_func}, Eq.~\eqref{eq:NMDC_dphi/dvphi} cannot be solved analytically and so we work with the non-canonical field by means of~\eqref{eq:NMDC_PSRPs}. The integration for the number of e-folds in~\eqref{eq:NMDC_PSRPs} can nevertheless be performed analytically and yields the result
\beq
...
\end{equation}
By substituting the measured value for the power spectrum $A_s \simeq 2.1 \times 10^{-9}$ in the above expression, we obtain the following constraint between the model parameters ($\xi,\lambda,m_0$) at any given large $N_\star$:
\beq
\xi \approx 4 \times 10^6 N_\star^2 \lambda - \frac{m_0^2 \lambda}{4} \,.
\label{eq:NMDC_As_constraint}
\eeq
In the limit $m_0 \to 0$, Eqs.~\eqref{eq:NMDC_observables} and~\eqref{eq:NMDC_As_constraint} reduce to those of the standard Palatini-Higgs inflation. The most important consequence of the constraint~\eqref{eq:NMDC_As_constraint} has to do with the fact that for fixed $N_\star$ and $\lambda$, an increase in the value of $m_0$ reduces the value of $\xi$, and this corresponds to an increase in the predicted value for $r$ according to~\eqref{eq:NMDC_observables}. This property, is further illustrated in Fig.~\ref{fig:Case1-density-r}, for which the more precise analytic expression for $A_s$ for which no large-$N_\star$ expansion has been performed\footnote{This expression for $A_s$, along with the more accurate expressions for $r$ and $n_s$, can be found in Appx.~A.2 of~\cite{Gialamas:2020vto}.}, has been used.

From the expressions~\eqref{eq:NMDC_observables} it follows that for a typical value for the number of e-folds $N_\star=50$, the prediction for the tensor-to-scalar ratio in the theory~\eqref{eq:NMDC_JF_action} compared to the standard result for Palatini-Higgs inflation ($r_0\equiv2/(\xi N_\star^2)$) is given by~\cite{Gialamas:2020vto}
\beq
r \sim r_0 \times \left(10^{10} \frac{\lambda}{\xi} \right)\,.
\eeq
For each $\xi$ and $\lambda$, the value of $m_0$ is adjusted accordingly in such a way that $A_s$ complies with its measured value. This feature of the NMDC-extended model provides a means for the Palatini-Higgs inflation scenario (which typically predicts~$r \sim \mathcal{O}(10^{-12})$~\cite{Bauer:2008zj, Rasanen2017, Markkanen:2017tun, Takahashi2019}) to be in contact with observations in the near-future experiments that will probe the $\mathcal{O}\left(10^{-4}\right)$ regime for the tensor-to-scalar ratio~\cite{Matsumura2016,Kogut_2011,Sutin:2018onu}.
...
\end{equation}
which for $\lambda=0.1$ and $\xi=10^5$, turns out to be ${n_s}^{(2)}\sim 10^{-4}$. This means that the spectral index remains essentially unaffected in this case, assuming values around $n_s\sim1-2/N_\star\simeq0.96$.
...
This section will provide an overview of the key features of a family of theories containing an inflaton field $\varphi$ with non-minimal coupling $A(\varphi)$, along with an $\Ri^2$ term in the Palatini formulation. We also consider more general theories quadratic in the curvature tensors and finally models higher order in $\Ri$.
...
and 
\begin{equation}
K(\varphi) \equiv \frac{1}{ A (1+8\alpha \bar{U}) }\,, \qquad L(\varphi)=\frac{2\alpha}{A^2 (1+8\alpha \bar{U})}\,. 
\end{equation}
It is worth noting that the potential $\bar{U}$ has the same form as the Einstein frame potential when the $\alpha \Ri^2$ term is absent. Furthermore, aside from altering the potential, the conformal transformation and the elimination of the $\alpha \Ri^2$ term have generated a quartic kinetic term for $\varphi$ which could potentially become dominant for large values of $\alpha$. However, it has been demonstrated~\cite{Gialamas:2019nly, Karam:2021sno, Dimopoulos:2022rdp} that this term does not play a significant role either during the slow-roll phase or after the end of inflation.
...
\begin{enumerate}
    ...
    \item For large values of the field $\phi$, the potential always becomes a plateau.
\end{enumerate}
We will next see how these features modify the inflationary predictions of a given model when an $\alpha \Ri^2$ term is added and is studied in the Palatini formalism.
...
\end{equation}
The first two potential slow-roll parameters are defined as
\begin{equation} 
...
\end{equation}
In the last expression, we utilized Eq.~\eqref{eq:PalatiniR2:As-ns}, while the expression $\bar{r}=16\epsilon_{\bar{U}}$ corresponds to the tensor-to-scalar ratio of the same model but without the $\alpha \Ri^2$ term. It becomes evident that by increasing $\alpha$ significantly, we can reduce the value of $r$ in a specific model, without influencing the prediction for $n_s$.
...
Finally, in the large $\alpha$ limit and using Eq.~{\eqref{eq:e-folds:simplified}}, one can show~\cite{Gialamas:2020snr} that the number of $e$-folds is given by
\begin{equation}
...
The potential is depicted in Fig.~\ref{fig:quad_pot}. One can see that for $\alpha = 0$ the potential reduces to a simple quadratic one, while for large values of $\alpha$ the potential develops plateaus which asymptote to $U = 1/(8\alpha)$. Even higher values of $\alpha$ would reduce the height of the potential and thus the scale of inflation. For inflation to take place above the $\rm TeV$ scale, we would need $U^{1/4} \gtrsim 10^{-15}$ in Planck units, or $\alpha \lesssim 10^{60}$.
...
\begin{equation} 
    ...
    \,, \qquad
    r 
   = \frac{8}{N_\star + 16 \alpha m^2 N^2_\star} 
...
\end{equation}
We see explicitly that $A_s$ and $n_s$ (which have the standard form) do not depend on $\alpha$, but $r$ is suppressed by it (see Fig.~\ref{fig:nsrQuad}). The measured value of the amplitude $A_s = 2.1 \times 10^9$ fixes
\begin{equation}
\label{eq:m_quad}
    m \simeq 7 \times 10^{-6} \,,
\end{equation}
...
\caption{Inflationary predictions for the minimal quadratic model in the $n_s$ vs. $r$ plane. For $\alpha=0$ we recover the prediction $r = 0.13$ of the model without the $\alpha \Ri^2$, while for growing $\alpha$ (as indicated by the black arrow) the value of $r$ reduces significantly. The mass parameter $m$ is fixed by the amplitude $A_s$.}
\label{fig:nsrQuad}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%% END FIGURE %%%%%%%%%%%%%%%%%%%%%%%
...
Another simple model, that of minimal natural inflation with $U(\varphi) = \Lambda^4 \left[ 1 + \cos \left( \varphi / f \right) \right]$, was studied in~\cite{Antoniadis:2018ywb} and it was shown that the addition of the $\alpha \Ri^2$ in the Palatini formalism can render the (previously excluded) model as viable again.
...
\end{equation}
Since the kinetic term cannot be canonicalized analytically we will proceed using the initial field in what follows.
...
\end{equation}
In order to estimate the inflationary observables, we need to express the field value at horizon crossing as a function of the number of $e$-folds. Integrating~\eqref{eq:N_SR} we obtain that $\varphi_\star = \sqrt{8N_\star +\varphi_{\rm end}^2} \simeq \sqrt{8N_\star}$, where we have dropped the contribution from the field value at the end of inflation, since\footnote{Solving the equation $\epsilon_V(\varphi_{\rm end}) =1$ we obtain that $2\alpha\lambda  \varphi_{\rm end}^6 +(1+\xi  \varphi_{\rm end}^2)( \varphi_{\rm end}^2(1+\xi  \varphi_{\rm end}^2)-8)= 0$ which is a cubic equation in $ \varphi_{\rm end}^2$ so analytic solutions can be obtained. It is easily proved~\cite{Gialamas:2019nly} that there is only one real positive solution for $ \varphi_{\rm end}^2$ that is lower that 8.} $ \varphi_{\rm end} < \sqrt{8}$. The inflationary observables are given by
\begin{equation}
...
\caption{The inflationary predictions of the model given by Eq.~\eqref{eq:final_action}  compared with the observational $68\%$ and $95\%$ C.L. constraints in the $n_s-r$ plane. Dashed (solid) curves  correspond to inflation that starts from field values $s_c>v_s$ ($s_c<v_s$). The parameters $\tilde{\alpha}$ and $\xi_s$ increase as indicated by the black arrows.}
\label{fig:ns_r_agra}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%% END FIGURE %%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
...
\end{equation}
where $v^2_s= M^2_{\rm Pl}/\xi_s$ is the VEV of the scalon and $\xi_s =\xi_\varphi \cos^2 \omega + \xi_h \sin^2 \omega$ is an effective nonminimal coupling that is constructed from the initial nonminimal couplings and a mixing angle $\omega$.
...
\end{equation}
where $k$ is the Fourier wave number. When $U''(\phi)<0$, the solutions for $\delta\phi$ grow exponentially. Typically, the background inflaton field $\phi$ passes over the tachyonic region quickly, and its energy is diluted by Hubble friction so that it only oscillates near the bottom of the potential, so the tachyonic effect is weak. However, if $\phi_0 \lesssim 0.01$, corresponding to $r\lesssim 10^{-7}$, Hubble friction is inefficient, and the oscillating inflaton returns repeatedly to the tachyonic edge of the plateau~\cite{Tomberg:2021bll}. This leads to an instantaneous fragmentation of the condensate, completing preheating in much less than one e-fold of expansion. Metric plateau models, such as Starobinsky and Higgs inflation, tend to have a fixed $\phi_0 = \sqrt{6}$, see \eqref{eq:starobinsky_U}, \eqref{eq:higgs_U_metric}, but Palatini models can easily verge into the tachyonic territory, e.g. for $\xi \gtrsim 10^4$ in the Higgs model \eqref{eq:higgs_U_Palatini} with $\phi_0 = \xi^{-1/2}$, or for $\alpha \gtrsim 2500 m^{-2}$ in the $\Ri^2$ quadratic model with \eqref{eq:R2_phi_transformation}, \eqref{eq:R2_quadratic_U}.
...
Beyond linear analysis, similar models have been studied using lattice techniques e.g. in the context of $\alpha$-attractor $T$-models in \cite{Lozanov:2016hid, Lozanov:2017hjm} and explicitly for Palatini Higgs inflation in \cite{Dux:2022kuk}.
...
\begin{equation}
\label{eq:N_reh}
\Delta N_{\rm reh} = \ln \left[\frac{a_{\rm reh}}{a_{\rm end}}\right] = -\frac{1}{3(1+w)}\ln \left[\frac{\rho_{\rm reh}}{\rho_{\rm end}} \right]\,,
\end{equation}
...
\begin{equation}
\label{T_reheat}
T_{\rm reh} = T_{\rm ins} \exp\left(- \frac{3(1+w) \Delta N_{\rm reh}}{4} \right)\,, \quad \text{with} \quad T_{\rm ins} = \left( \frac{30}{\pi^2} \frac{\rho_{\rm end}}{g_\star(T_{\rm reh})}\right)^{1/4}\,.
\end{equation}
...
Also, the authors of~\cite{Lloyd-Stubbs:2020pvx} studied specific reheating mechanisms, which contain inflaton annihilation to Higgs bosons and reheating via inflaton decay to right-handed neutrinos.  In this review we focus on the computation of the instantaneous reheating temperature in the context of Palatini inflationary models discussed in subsections~\ref{Example:_Higgs_Inflation} and~\ref{sec:minimal_quadratic}.

In the Palatini-Higgs inflationary model of subsection~\ref{Example:_Higgs_Inflation}, the  field value at the end of inflation can be found approximately using the condition $\epsilon_V (\phi_{\rm end})=1$, which gives
...
\end{equation}
which for $\xi\gg 1$ can be approximated by $\rho_{\rm end}\simeq 3\lambda/(8\xi^2)$. Substituting the latter in Eq.~\eqref{T_reheat} we obtain that the instantaneous reheating temperature is
\begin{equation}
\label{eq:tins_Higgs}
...
\right.
\end{equation}
Having in mind that the mass parameter is constrained to be $m\simeq 7\times 10^{-6}$ (see Eq.~\eqref{eq:m_quad}), the large $\phi_0$ limit, $\phi_0^2 \gg 1$, means that $\alpha\ll 10^{10}$, while the other (tachyonic) limit is valid once $\alpha\gg 10^{10}$. In these limits the instantaneous reheating temperature is
\begin{eqnarray}
\label{Tins:q1}
...
\label{Tins:q2}
\end{eqnarray}
so using also that $\alpha \gtrsim 10^8$ (see Eq.~\eqref{eq:al_quad}) we conclude that $T_{\rm ins}\sim 10^{15} - 10^{16} \,\mathrm{GeV} $ in the whole range of the observationally accepted parameter space. 

The above model, along with other Palatini-$\Ri^2$ models, have been discussed in more detail in~\cite{Gialamas:2019nly, Lahanas:2022mng}. In~\cite{Lahanas:2022mng}, the higher-order kinetic terms that appear in the EF action~\eqref{eq:action:PalatiniR2:final} have been taken into account, which leads to slightly modified numerical factors in Eqs.~\eqref{Tins:q1} and~\eqref{Tins:q2}.
...
\begin{figure}
    ...
    \caption{The different phases of quintessential inflation, together with the scalar potential. In the notation of section~\ref{sec:quintessece}, $U_\star = 1/(8\alpha)$. $\phi_{\rm kin}$ denotes the value of the field after the end of inflation and during the onset of kination, $\phi_{\rm rad}$ is the field value at the beginning of the radiation era, $\phi_{\rm eq}$ denotes the radiation-matter equality, and $\phi_{\Lambda}$ is the value of the field for which the potential behaves as the cosmological constant.}
...
\begin{enumerate}
    ...
    \item Transition to \emph{kination} around $\phi \approx 0$. In kination, energy density is dominated by the kinetic energy of the field, diluting as $\rho\propto a^{-6}$, corresponding to a barotropic parameter $w=1$. Around this time, a small amount of radiation is created, e.g. through Ricci reheating \cite{Dimopoulos:2018wfg}.
    \item Since the scalar kinetic energy density dilutes faster than radiation, the latter eventually takes over, leading to the standard radiation domination, where $\rho \propto a^{-4}$, $w=1/3$. The scalar continues rolling. It can follow a tracking solution, mimicking the scaling of the fluid, or freeze to an approximately constant energy density, depending on the potential \cite{Copeland:1997et}.
    \item Eventually, the radiation becomes non-relativistic, and matter domination starts, with $\rho \propto a^{-3}$, $w=0$. In our setup, the field freezes in a minimum of the Einstein frame potential \eqref{eq:quintessence_U}, induced by the running of $\xi$.
    \item A frozen scalar condensate eventually retakes matter, leading to the final dark energy phase with an approximately constant energy density and $w\approx-1$.
\end{enumerate}
