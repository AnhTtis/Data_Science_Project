...
\begin{equation}\label{esempio_payoff}
    \J(\Lambda,\mu)=\attesa{cX_T \bar{\mu}_T},
\end{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{sections/esempio_prova1.jpg}
    \caption{Existence of mean field CCEs as the probability measure $(p_{i,j})_{i,j=1,2}$ varies.
    Yellow points correspond to the values of $(p_{i,j})$ that correspond to coarse correlated solutions, blue points to the other ones.
...
Observe that there exist infinitely many coarse correlated solutions of the mean field game so that $\Lambda$ is not a deterministic function of $\mu$, i.e., they are not a randomization of the solutions $(u^+,\mu^+)$ and $(u^-,\mu^-)$. 
They correspond to yellow points so that one between $p_{1,2}$ and $p_{2,1}$ is non null.
...
By \eqref{esempio:hp_filtrazioni}, we have $\mu_t=\prob(X_t \in \cdot \; \; \vert \; \Tilde{\mu}) = \prob(X_t \in \cdot \; \; \vert \; \mu)$, i.e. consistency condition \eqref{def_mean_field_sol:cons} is satisfied.
