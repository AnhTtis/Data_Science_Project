...
% poses a significant challenge in industrial production as it involves detecting and localizing unknown anomalies in industrial images. 
% Industrial anomaly detection, aiming to detect and localize unknown anomalies in industrial images, is a widespread issue in the field of industrial vision.
...
\begin{algorithm2e}[!t]
    ...
    \label{alg:PTS}
    \KwIn{Original template set pixels $\mathcal{T}^{*}_{x,y}$, sheets $K$, OPTICS $\psi(\cdot)$}
    \KwOut{Tiny template set pixels $\mathcal{T}^{K}_{x,y}$.}
    ...
    \eIf{$len(\mathcal{R}) > 0$} {
        ...
        \For{$r_{i} \in \mathcal{R}$}{
            Get easy-nominal prototypes $t_{i}$ by Eq. \ref{eq:optics}\;
            Update tiny template set: $\mathcal{T}^{K}_{x,y}\leftarrow \mathcal{T}^{K}_{x,y}\cup \{t_{i}\}$\;
        }
    }   {
    ...
    \While{$len(\mathcal{T}^{K}_{x,y})<K$}{
        Get hard-nominal prototypes $t_{h}$ by Eq. \ref{eq:hard}\;
        Update tiny template set: $\mathcal{T}^{K}_{x,y}\leftarrow\mathcal{T}^{K}_{x,y}\bigcup\{t_{h}\}$\;
    }
\end{algorithm2e}
...
\begin{table*}[!t]
    ...
    % \multicolumn{2}{c|}{$\downarrow$Cat.$\setminus$FPS$\to$}
    % & & & & & & & & & &3.7 &42.7\\\hline
...
% More detailed results and visual examples are reported in \ref{A:RSL} and \ref{A:VSL}.
...
% % In this section, we show the additional visual examples on the MTD and MVTec LOCO AD datasets.
% % \subsection{Visual Examples on MTD}\label{A:MTD}
% % \subsection{Visual Examples on MVTec LOCO}\label{A:VSL}
% \section{Detailed Results}
% In this section, we supplement some detailed results on MVTEC AD, MVTEC SL, MVTec LOCO and few-shot anomaly detection in the following subsections.
% \subsection{Details on MVTec AD}
% \subsection{Details on MVTec SL}\label{A:RSL}
% \subsection{Details on MVTec LOCO}
% \subsection{Details of Few-shot Anomaly Detection}\label{A:fewshot}
% \begin{table*}[!t]
%     \caption{
%         Comparisons between the proposed \textit{HTBS} and state-of-the-art approaches on MVTec AD dataset for anomaly detection and localization under the few-shot settings.
%         \textbf{Bold} text indicates the best performance.
%     }
%     \centering
%     \footnotesize
...
%     \multicolumn{8}{c}{Image-level AUROC$\uparrow$}\\\hline
...
%     \textbf{HTBS}                                       &\textbf{87.1$\pm$1.0}  &\textbf{89.5$\pm$0.7}  &\textbf{92.3$\pm$1.2}  &\textbf{94.5$\pm$0.6}  &\textbf{96.0$\pm$0.5}  &\textbf{96.8$\pm$0.6}  &\textbf{98.2$\pm$0.2} \\\hline
%     \multicolumn{8}{c}{}\vspace{-0.85em}\\
%     \multicolumn{8}{c}{Pixel-level AUROC$\uparrow$}\\\hline
...
%     \textbf{HTBS}                                       &\textbf{94.4$\pm$0.3}  &\textbf{95.2$\pm$0.5}  &\textbf{96.0$\pm$0.2}  &\textbf{96.8$\pm$0.1}  &\textbf{97.2$\pm$0.1}  &\textbf{97.3$\pm$0.1}  &\textbf{97.9$\pm$0.0} \\\hline
%     \multicolumn{8}{c}{}\vspace{-0.85em}\\
%     \multicolumn{8}{c}{PRO$\uparrow$}\\\hline
...
%     \textbf{HTBS}                                       &\textbf{87.7$\pm$0.5}  &\textbf{89.1$\pm$0.5}  &\textbf{92.2$\pm$0.2}  &\textbf{93.2$\pm$0.1}  &\textbf{93.5$\pm$0.1}  &\textbf{93.6$\pm$0.1}  &\textbf{94.0$\pm$0.2} \\\hline
%     \end{tabular}
% \end{table*}
% \begin{figure*}[!t]
%     \centering
...
%     \caption{
%     Detailed result of curves between the proposed \textit{HTBS} and other advances on the MVTec AD dataset.
%     }
%     \label{fig:details}
% \end{figure*}
