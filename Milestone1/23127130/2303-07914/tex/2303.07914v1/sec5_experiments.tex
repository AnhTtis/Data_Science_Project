...
For the acoustic encoder, we use Wav2vec2.0\footnote{\url{https://dl.fbaipublicfiles.com/fairseq/wav2vec/wav2vec\_small.pt}} \citep{NEURIPS2020_92d1e1eb} following the base configurations, which use a self-supervised learning framework to pre-train on large-scale audio data from the LibriSpeech \citep{panayotov2015librispeech} corpus. 
...
\\
\textbf{Training} 
The detailed training schedule of the offline ST model can refer to Appendix \ref{apd:details_of_training}. 
...
$k$ is counted by the detected acoustic units of the CIF acoustic boundary detector. 
...
\\
\textbf{Evaluation Metrics} We use SacreBLEU\footnote{\url{https://github.com/mjpost/sacrebleu}} to measure the translation quality. 
The latency is evaluated with Average Latency (AL) \citep{ma-etal-2019-stacl}, Average Proportion (AP) \citep{cho2016can}, and Differentiable Average Lagging (DAL) \citep{cherry2019thinking} in the SimulEval toolkit\footnote{\url{https://github.com/facebookresearch/SimulEval}} \citep{ma-etal-2020-simuleval}. 
\\
...
We presents the results on the MuST-C EnDe and EnEs tst-COMMON set in Figure \ref{fig:main_results} \footnote{The extended results for other latency metrics (AP and DAL) are described in Appendix \ref{apd:appendix_more_results}.}. 
...
In addition, our method achieves comparable translation quality with full-speech translation at middle latency (at AL around 2000\emph{ms}), especially for EnEs.
...
(1) \textit{w/o $\mathcal{L}_{KD}^{\text{W2V2}}$}: if removing the $\mathcal{L}_{KD}^{\text{W2V2}}$ in Eq.(\ref{eq:total_loss}), the translation quality drops by 1-2 BLEU in all latency regions, including high latency region.
This demonstrates optimizing $\mathcal{L}_{KD}^{\text{W2V2}}$ can guarantee the full speech translation.

(2) \textit{w/o $\mathcal{L}_{KD}^{\text{CIF}}$}: If removing the $\mathcal{L}_{KD}^{\text{CIF}}$ in Eq.(\ref{eq:total_loss}), the translation quality will be slightly degraded. 
...
Thus, optimizing $\mathcal{L}_{KD}^{\text{CIF}}$ can ensure the correct acoustic boundaries for streaming inference.
...
Figure \ref{fig:length_bleu} shows the overall results. 
...
After the FAD training, we investigate the representation of the last position (before mask tokens) by cosine similarity in Eq.~(\ref{eq:audio_cos2}) w.r.t. different $m$. 
The results are shown in Figure \ref{fig:length_cos}.
We observe that 1) as $m$ increases, the representation of the last position in the streaming speech becomes better; 
2) the curves of the cosine similarity becomes flattened when $m > 10$ significantly. 
This is consistent with the BLEU-AL curves in Figure \ref{fig:length_bleu}.
