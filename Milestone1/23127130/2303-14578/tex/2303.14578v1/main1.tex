...
\begin{enumerate}
    ...
    $C^{-1}={\displaystyle \int_{-\infty}^\infty} \exp{\bigg(\dfrac{-x^4}{12} \bigg)}dx $.
\end{enumerate}
\end{theorem}
...
where $g(m,K):=\mathrm{arctanh}(m)-Km^2$ and set $J_c=1$. Then:
    \begin{itemize}
        \item[a.] for $J<\Psi(K)$, there exist a unique solution, $m_0 = 0$, and it is the maximum point of $\phi(m)$,
        ...
        \item[d.] If $J\geq J_c$, there exist a unique positive solution $m_2$ which is the only maximum point of $\phi(m)$ in equation \eqref{varpressure}.
    \end{itemize}
\end{proposition}
...
\end{equation}
which implies \ref{asymptotic_uniq}.
%%%%%%%%%%%%%%%%%%%%%
