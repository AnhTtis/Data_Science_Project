...
(\textbf{2}) \textit{Time Incremental Stream}: In this setting, the stream data is ordered by the upload timestamp to a server, reflecting a natural distribution change $\mathcal{D}_j$ across the stream as it would in real scenarios. There is a recent shift toward studying this ordering as apparent in recent CL benchmarks, \eg, CLEAR \cite{lin2021clear}, Yearbook \cite{yao2022wild} and FMoW \cite{yao2022wild}, NEVIS22 \cite{bornschein2022nevis}, Continual YFCC100M \cite{cai2021online} and Continual Google Landmarks V2 \cite{prabhu2023online}.
