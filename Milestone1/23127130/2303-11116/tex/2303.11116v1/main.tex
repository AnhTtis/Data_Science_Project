...
We consider the excitation of the resonant medium Eq.~\eqref{Bloch} by a pair of subcycle pulses:
\begin{eqnarray}
...
Let us now consider the response of an extended layer of a multi-level resonant medium. Namely, we assume a layer thickness to largely exceed the wavelengths of the resonant transitions in the medium. In this case we have to solve the equations Eq.~\eqref{Bloch} coupled to the wave equation Eq.~\eqref{eq_wave} through the expression Eq.~\eqref{polariz}. The wave equation Eq.~\eqref{eq_wave} was solved using the finite-difference time-domain method (FDTD), while for the equations Eq.~\eqref{Bloch} we applied the high-order Runge-Kutta scheme.
