\documentclass[12pt,preprint,longbibliography,superscriptaddress,tightenlines,nofootinbib,titlepage]{revtex4-1}
...
\title{Universal chaotic dynamics from Krylov space}

\author{Johanna~Erdmenger}
\affiliation{Institute for Theoretical Physics and Astrophysics and W{\"u}rzburg-Dresden Cluster of Excellence ct.qmat,\\
	Julius-Maximilians-Universit{\"a}t W{\"u}rzburg, D-97074 W{\"u}rzburg, Germany}

\author{Shao-Kai~Jian}
\affiliation{Department of Physics and Engineering Physics, Tulane University, New Orleans, Louisiana, 70118, USA}

\author{Zhuo-Yu~Xian}
\email{zhuo-yu.xian@physik.uni-wuerzburg.de}
\affiliation{Institute for Theoretical Physics and Astrophysics and W{\"u}rzburg-Dresden Cluster of Excellence ct.qmat,\\
	Julius-Maximilians-Universit{\"a}t W{\"u}rzburg, D-97074 W{\"u}rzburg, Germany}
...
\begin{abstract}
\noindent Krylov state complexity measures the spread of the wavefunction in the Krylov basis, a particular basis that is uniquely constructed using the Hamiltonian of a given physical system. 
...
The linear growth is not a characteristic of chaos, since it may also appear in non-chaotic systems. 
...
In terms of the Krylov basis, the evolution of state $\ket{\psi_\tau}=e^{-\tau\L}\ket0$ can be effectively mapped to the propagation of a quantum particle along a one-dimensional chain, which we call Krylov chain \cite{Parker:2018a}. 
The Krylov complexity is then defined as the expectation number of time of applying $\L$ on $\ket0$ to generate $\ket{\psi_\tau}$, {\it i.e.}, the location of the particle in the Krylov chain. The Krylov state complexity is used to distinguish topological phases of matter \cite{Caputa:2022yju,Caputa:2022eye}.

Krylov operator complexity measures how far an operator in the Heisenberg picture spreads in the space of operators. 
...
Once the operator $O$ has a nonzero commutator with the Hamiltonian, it will grow under the evolution with $\L$. Methods for studying the time evolution of Krylov complexity were recently obtained based on decomposing  Liouvillian $\L$ into annihilation and creation operators \cite{Caputa:2021sib,Haque:2022ncl}.

Krylov operator complexity allows to read out the Lyapunov exponent and the scrambling time \cite{Parker:2018a,Jian:2020qpp,Barbon:2019wsy,Bhattacharjee:2022ave,Avdoshkin:2022xuw,Camargo:2022rnt,Kundu:2023hbk}  which characterize the exponential growth of the operator size \cite{Roberts:2018operator,Qi:2018quantum} given by the out-of-time-ordered correlator (OTOC) \cite{Shenker:2013black,Roberts:2014localized,Mertens:2017solving,Shenker:2013yza}. 
Krylov operator complexity also reflects late-time chaotic behavior \cite{Rabinovici:2020operator,Espanol:2022cqr}. 
It is further argued in \cite{Kar:2021nbm,Rabinovici:2021qqt,Rabinovici:2022beu,Alishahiha:2022anw} that the descent in the Lanczos coefficients as well as the late-time behavior of the Krylov operator complexity given by the evolution with a chaotic Hamiltonian $H$ is expected to be governed by the RMT.
Thus, to study the late-time chaos from Krylov complexity, it appears to be more convenient to study Krylov state complexity directly.
...
\begin{figure}
    ...
    \caption{The SFF (left) and Krylov complexity of maximally entangled state (right) as functions of time for the Gaussian unitary ensemble with dimension $L=1024$ and $128$ realizations . The blue curves denotes the numerical results and the black curves denote the values of $1/L$ (left) and $1/2$ (right).}
...
The Krylov space for the maximally entangled state has overlap $\abs{\avg{E_0|0}}^2=1/L$. At $\beta=0$, due to the orthogonality relation \eqref{Orthogonal}, the long time averages \eqref{probabilityAverage} and \eqref{ComplexityAverage} simplify to \cite{Rabinovici:2020operator,Rabinovici:2022beu}
\begin{align}\label{AverageTFD}
    &(\abs{\phi_n}^2)_\infty=\frac1L,\quad K_\infty=\frac {L-1}2.
\end{align}
We will further estimate their fluctuations for chaotic spectra in Sec.~\ref{sec:firstlook}.
...
\subsection{Lanczos coefficients in the large $L$ limit}\label{sec:AverageLanczos}
...
\begin{align}\label{sine}
	\avg{\rho(E_1)\rho(E_2)} =\avg{\rho(E)}\delta(x)+\avg{\rho(E_1)}\avg{\rho(E_2)} \kd{1-\frac{\sin^2\kc{\pi\avg{\rho(E)}s}}{\kc{\pi\avg{\rho(E)}s}^2}},
\end{align}
...
\begin{align}\label{KGUETFD}
	C(t;0)
	=&\Re\left[\frac{\left(26 L^2 t+t^3\right)\sqrt{4 L^2-t^2}}{12 \pi  L^3}
	-\frac{6 t^2 \csc ^{-1}\left(\frac{2 L}{\sqrt{4 L^2-t^2}}\right)-6 L^2 \csc ^{-1}\left(\frac{2 L}{t}\right)+16 L t+3 \pi  L}{3 \pi  L} \right. \nn\\
	&\left. +\, _1F_2\left(-\frac{1}{2};1,2;-4 t^2\right)\right],
\end{align}
which asymptotes to a constant $L-1$ at $t\to\infty$. 
...
\subsubsection{$\beta\gg1$}

In the $\beta\gg1$ limit, we may approximate the spectral density in the ``double scaled'' limit \cite{Cotler:2016fpe}, or equivalently at the lower edge of the spectrum,
\begin{align}
	\rho(E)=\frac{L}{\pi}\sqrt{E},
\end{align}
where we have shifted the energy as $E\to E-2$. 
...
\begin{align}\label{KCBeta}
	\Delta C(t;\beta)\to &
	\begin{cases}
		\frac{2 t^2 \left(\frac{L}{\sqrt{2 \pi } \beta ^{3/2}}-\frac{1}{2}\right)}{L},& t\ll\beta \\
		2\sqrt{\frac{2}{\pi \beta}}t,& \beta\ll t\ll L/\sqrt\beta \\
		\frac{L-2 \sqrt{\frac{2}{\pi }} \beta ^{3/2}}{\beta }+\frac{\sqrt{\frac{2}{\pi }} \beta ^{3/2}}{t}, & L/\sqrt\beta\ll t
	\end{cases}
\end{align}
...
\begin{align}\label{TwoPoints}
    \avg{\rho(E_1)\rho(E_2)}=\avg{\rho(E)}\delta(x)+\avg{\rho(E_1)}\avg{\rho(E_2)}-\frac12\Tr(k(E_1,E_2)k(E_2,E_1)),
\end{align}
with
\begin{align}
    &\text{GOE:}\quad 
...
\begin{align}\label{SpectralIntegral}
    \frac{2}{LZ(2\beta)}\int dE dx \avg{\rho(E_1)}\avg{\rho(E_2)} e^{-2\beta E} \kc{\frac{1-e^{itx}}{x^2}}
    \kc{1-\hat k\kc{\pi\avg{\rho(E)}x}},
\end{align}
...
The integrand is therefore finite at $x=0$. 
If $\hat k(r)$ is holomorphic, when $t>2\pi d\avg{\rho(E)}$,  we may close the contour of the $x$ integral for those terms with the factor $e^{itx}$. For this we use the arc at complex infinity in either the upper-half plane or the lower-half plane, such that the integral of those terms vanishes and the spectral complexity saturates. 
...
\begin{align}\label{GeneratingFunction}
    ...
    =&~\int_{-2}^2 dE\int_{-\infty}^{+\infty} ds \avg{\rho(E)}\kd{\delta(x)-\frac{\sin^2\kc{Ls}}{\pi Ls^2}}e^{-i(2t+t_1-t_2)s/2-(2\beta+it_1+it_2)E}\\
...
\begin{align}
    ...
    \approx&~
    \abs{\frac{1}{L}\int dE \avg{\psi_n(E)}\avg{\rho^{(L-n)}(E)}e^{-(\beta+it)E}}^2+\frac1{L^2}\int dE\avg{\psi_n(E)}^2 \avg{\rho^{(L-n)}(E)} e^{-2\beta E}  \label{ProbabilitySplit}\\
...
\begin{align}
    ...
    \int_{-2\sqrt{1-n/L}}^{2\sqrt{1-n/L}} dE\int_{-\infty}^{\infty} ds~\sqrt{4\kc{1-\frac nL}-E^2}\frac{\sin^2\kc{Ls}}{(\pi s)^2}e^{-its}\nn\\
    &~~~~
    \times \cos\kc{2L(E+s/2)(1-\sqrt{1-n/L})} \cos\kc{2L(E-s/2)(1-\sqrt{1-n/L})} \label{RampPlateauContinuum}
    \\
...
More precisely, we consider the saturated case where the wavefront reaches the $n$-th site at time $t$, and the transition probability before the $n$-th site is in its ramp region, i.e., $\avg{\abs{\phi_m(it)}^2}\gtrsim t/(2L^2)$ with $0\leq m < n$. Due to probability conservation, which is equal to $1$ at $\beta=0$, we have $\avg{\abs{\phi_n(it)}^2}\lesssim 1-tn/(2L^2)$. The wavefront is described by \eqref{shock}. Then, the Krylov complexity is bounded above by
\begin{align}
    K\leq n(t)\kc{1-\frac{tn(t)}{2L^2}}+\frac{n(t)(n(t)-1)t}{4L^2}
    =\frac{t (2 L-t) \left(2 L^3-L t (2 t+1)+t^3\right)}{2 L^4},\quad t\leq L \, , 
\end{align}
...
\begin{align}
    \avg{\rho(E_1)\rho(E_2)}=\rho(E)\delta(x)+\frac{L-1}{L}\avg{\rho(E_1)}\avg{\rho(E_2)},
\end{align}
...
The absence of the peak in the Krylov complexity is due to the absence of the ramp in  the transition probability. Recall that $\avg{\abs{\phi_n(it)}^2}<1-n/L$ due to the early plateau time and probability conservation, which bounds the complexity contributions from probability at large $n$. To describe this in detail, let us consider  the saturated case, in which the wavefront reaches the $n$-th site at time $t$, i.e.$\avg{\abs{\phi_n(it)}^2}=1-n/L$ and $\avg{\abs{\phi_m(it)}^2}=1/L$ with $0\leq m< n$. With  wavefront \eqref{shock}, the Krylov complexity is bounded from above by
\begin{align}
...
First, the saturation value predicted by our analytical expression is not accurate, and the error stems from the approximation that we neglect the fluctuations of the Lanczos coefficients in Eqs.~\eqref{LanczosLimit} and \eqref{EhrenfestTFD}. 
...
We are grateful to Vijay Balasubramanian and to Souvik Banerjee for discussions. The research of J.~E. and Z.-Y.~X.~is funded by DFG through the Collaborative Research Center SFB 1170 ToCoTronics, Project-ID 258499086—SFB 1170, as well as by Germany's Excellence Strategy through the W\"urzburg‐Dresden Cluster of Excellence on Complexity and Topology in Quantum Matter ‐ ct.qmat (EXC 2147, project‐id 390858490). Z.~Y.~X. also acknowledges support from the National Natural Science Foundation of China under Grants No.~11875053 and No.~12075298.
...
\begin{align}
    ...
    k=1,2,...,\tilde L
\end{align}
where $k$ is originally bounded by the size of the Krylov space $L$, but we have released this constraint by consider the bound $\tilde L$ instead and will sent $\tilde L\to\infty$ finally for simplicity. 
...
\begin{align}
    ...
    =&\frac L2-\frac{\epsilon^2}{L^2S(2\beta)}\sum_{kl;\, \text{odd}~k_+}
...
\begin{align}
    S(2\beta)
    =\avg{\tilde\varphi(\beta)|\tilde\varphi(\beta)}
