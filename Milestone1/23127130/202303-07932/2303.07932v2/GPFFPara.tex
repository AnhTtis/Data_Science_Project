...
Given a system $G_{LPV}:\:u\mapsto y$, a model mapping $y$ to $w=\iint u \: dt^2$ is to be identified. A cost function is defined using input-output data as \citep{Pillonetto2014,Blanken2020}
\begin{equation}
...
\begin{rem}
	Note that the calculation of $\bar{\phi}_i$ \eqref{eq:defineThetaPhibar} might require taking the derivative of the output. In the presence of noise, this can be done by, e.g., using Kalman estimation or low-pass filtering. A framework where no derivatives of the output are used is part of ongoing research.
\end{rem}
The solution to the cost function in \eqref{eq:minimization} is given by \citep{Pillonetto2014}
\begin{equation}
	\label{eq:solTheta}
...
The kernel incorporates prior knowledge on the feedforward parameters, hence is important to design carefully. The optimal kernel for solving \eqref{eq:minimization} and minimizing the mean-squared error \Citep{Pillonetto2014}, when treating feedforward parameters as random variables, is equal to
\begin{equation}
	\label{eq:optKernel}
...
%		\item Construct kernel matrix $K$ by evaluating \eqref{eq:covker} for each feedforward parameter $\theta_i$. 
		\item Construct kernel matrix $K$, e.g., based on prior expectations on parameters $\theta_i$.
...
\end{proced}
To conclude, kernel regularized identification is capable of identifying LPV feedforward parameters with input-output data of a system, without specifying a structure. In the following section, an example is shown that validates the developed framework.