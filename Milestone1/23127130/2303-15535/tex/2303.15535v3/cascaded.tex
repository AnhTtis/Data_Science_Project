...
The main result also extends to upper triangular systems.

\begin{corollary}[Upper Triangular System]
	Consider the upper triangular system on ${X_1 \times X_2 \times \cdots \times X_n}$ given by
	\begin{subequations}
	\begin{align}
		\dot{x}_1 &= f_1(x_1,x_2,\cdots,x_n), \label{outermost} \\
		\dot{x}_2 &= f_2(x_2,x_3,\cdots,x_n), \label{second_system} \\[-3pt]
		& \hspace{5.5pt} \vdots \nonumber \\[-3pt]
		\dot{x}_i &= f_i(x_i,x_{i+1},\cdots,x_n), \\[-3pt]
		&\hspace{5.5pt}\vdots \nonumber \\[-3pt]
		\dot{x}_n &= f_n(x_n). \label{innermost}
	\end{align}
	\end{subequations}
Suppose that all trajectories of \eqref{outermost}-\eqref{innermost} are precompact and that for all ${i = 1, 2, \ldots, n}$, the unforced system
\begin{equation}
			\dot{x}_i = f_i(x_i,0_{i+1},0_{i+2},\ldots,0_n)
			\label{diagonal_system}
\end{equation}
is almost globally asymptotically stable with respect to ${0_i \in X_i}$ and all chain recurrent points of \eqref{diagonal_system} are hyperbolic equilibria. Then, ${(0_1,0_2,\ldots,0_n) \in X_1 \times X_2 \times \cdots \times X_n}$ is almost globally asymptotically stable and locally exponentially stable for the upper triangular system \eqref{outermost}-\eqref{innermost}.
\end{corollary}
\begin{proof}
	The claim is tautological for ${n=1}$. Assume for the purposes of induction that the claim holds for ${n = k-1}$. Observe that the system given by \eqref{second_system}-\eqref{innermost} is an upper triangular system with ${n=k-1}$ subsystems satisfying all hypotheses of the claim, and therefore by the induction hypothesis it is almost globally asymptotically stable and locally exponentially stable. It then follows immediately by Theorem \ref{almost_global_stability} that the upper triangular system \eqref{outermost}-\eqref{innermost} with ${n=k}$ subsystems is almost globally asymptotically stable, where we take \eqref{outermost} as the outer loop \eqref{outer_theorem} and \eqref{second_system}-\eqref{innermost} as the inner loop \eqref{inner_theorem}, i.e. $x = x_1$ and $y = (x_2,x_3,\ldots,x_n)$. Thus by induction, the claim holds for all $n \in \mathbb{N}$.
\end{proof}
...
\begin{equation}
	2\dot{\theta}^2 \geq \dot{\theta} \sin \theta + \dot{\theta}^2 \textrm{ whenever } W(
	 \theta,\dot{\theta}
	) \geq 4.
