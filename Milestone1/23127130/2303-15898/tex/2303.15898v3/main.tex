...
\cite{butkovsky2014ergodic} provides conditions for the ergodicity of nonlinear Markov chains. However, these conditions are significantly stronger than those required for the ergodicity of standard linear Markov chains and are not applicable to many settings of interest including the three applications we provide in this paper. Additionally, in Example \ref{Example:convergence}, we demonstrate that even for one of the most basic nonlinear Markov chains with two states, which satisfies our uniqueness conditions, the chain is not ergodic and 
 ...
 In the framework of mean field games, \cite{lasry2007mean}, \cite{light2022mean}, and \cite{anahtarci2023learning} provide conditions on the primitives of the game to prove the uniqueness of mean field equilibrium. 
...
To prove that $T$ has a unique fixed point it is  necessary  to assume that the linear Markov kernel  $Q(x,H(\mu),\cdot)$ has a unique invariant distribution when the aggregator $H(\mu)$ is fixed. That is, the operator $M_{H(\mu)} :\mathcal{P} (S) \rightarrow \mathcal{P} (S)$ has a unique fixed point where $M_{H(\mu)}$ is the Markov chain given by
\begin{equation*}M_{H(\mu)} \theta  (B) =\int _{S}Q (x ,H(\mu) ,B) \theta  (d x)
\end{equation*}
that is parameterized by a fixed aggregator $H(\mu)$. 
...
A stronger version of Property (U) says that the Markov kernel $M_{H(\mu)}^{n} \theta $ converges weakly to $\mu _{H(\mu)}$ for any probability measure $\theta  \in \mathcal{P} (S)$ where $M_{H(\mu)}^{n}$ means applying the operator $M_{H(\mu)}$, $n$ times. 
...
(ii) Consider the $n$-dimensional nonlinear Markov chain on $\mathbb{R}^{n}$ 
with $$X_{i,t+1} =  a_{i} X_{i,t} - \beta_{i} H(\mu_{t}) + \epsilon_{i,t+1}$$ 
for $i=1,\ldots,n$ where $a_{i} > 0$, $\epsilon _{i,t} $ are I.I.D random variables with finite expectations,  and 
$H(\mu_{t}) : = \int \sum 
 ...
 consider the set of functions $D$ such that $f: \mathbb{R}^{n} \rightarrow \mathbb{R}$ is in $D$ if $f(x) = \sum _{i}^{n} y_{i}x_{i} + c$ for some $y \in O$ and $c \in \mathbb{R}$. Under this set of functions $D$, we show in the Appendix that $Q$ is $D$-preserving and $D$-decreasing. It is immediate that $H$ is increasing with respect to $\succeq _{D}$. Hence, we can use Theorem \ref{Theorem: unique} to prove that the nonlinear Markov chain has at most one invariant distribution. This example together with part (i) demonstrate the importance of the flexibility of choosing the set of functions $D$ in order to apply Theorem \ref{Theorem: unique}. 
...
\end{example}
 ...
 Example \ref{Example:convergence} illustrates that the sequence of measures $\{ \mu _{t} \}$ does not converge to the unique stationary distribution even when the conditions for uniqueness we described in Section \ref{sec:unique} hold. In addition, the nonlinear Markov chain $Q$ provided in Example \ref{Example:convergence} is very simple with only two states showcasing that we can't expect the sequence of measures $\{ \mu _{t} \}$ to converge in typical applications. In that example, the average sequence $ \sum _{t=1}^{T} \mu_{t} /T$ converges to the unique stationary distribution. However, Example \ref{Example: Average Convergence} shows that this is not always the case even when the conditions for uniqueness provided in Theorem \ref{Theorem: unique} hold. 
...
with 
\begin{align*}
    f(\mu (\{0\}) ) & =  \mu (\{0\})1_{ \{ \mu (\{0\}) \leq 0.3 
\} } + (1.2\mu (\{0\})  -0.06) 1_{ \{ 0.3 < \mu (\{0\}) \leq  0.5  \} } \\
& +  (0.8\mu (\{0\})  + 0.14) 1_{ \{ 0.5 < \mu (\{0\}) \leq  0.7 \}  } +  \mu (\{0\})1_{ \{ \mu (\{0\}) > 0.7 
\} }.  
\end{align*} 

Note that $f (\mu (\{0\}) ) \geq \mu (\{0\})$ and $f$ is increasing, and hence, the conditions of Theorem \ref{Theorem: unique} hold and there exists at most two invariant distributions. In addition, $f$ is continuous so from Proposition \ref{Prop:existence} the nonlinear Markov kernel $Q$ has a unique invariant distribution. 
...
But $\pi ( \{ 0 \} ) = \pi (\{1\}) = 1/2 $ is not an invariant distribution so $\sum _{t=1}^{T} \mu_{t} /T$ does not converge to the invariant stationary distribution even when it converges. 
...
the nonlinear Markov kernel $Q$ remains an interesting open question.
  Despite the examples of non-convergence we presented, Section \ref{sec:applications} provides some important applications where the uniqueness of the invariant measure is of interest. For example, the invariant measure can correspond to the solution of nonlinear equations in $\mathbb{R}^{n}$ or the equilibrium wealth distribution of large dynamic economies (see Section \ref{sec:applications}).  

 \subsection{Local Results} \label{sec:localresults}


In this section, we present a localized version of Theorem \ref{Theorem: unique}. Instead of applying to all probability measures as dictated by Theorem \ref{Theorem: unique}, we introduce local versions of the monotonicity conditions and Properties (U) and (C). These local versions pertain only to a particular subset of probability measures that holds specific interest. These conditions ensure that, within this subset, there are no more than two invariant distributions. This subset may encompass probability measures that naturally emerge as candidates for invariant distributions or probability measures that are relevant for an application of interest. For a subset $\mathcal{P}'(S)$ of $\mathcal{P}(S)$ let $\mathcal{H}_{\mathcal{P} ' (S)} = \{ H(\mu) : \mu \in \mathcal{P}'(S) \}$ . 



\begin{definition} Let $\mathcal{P} ' (S)$ be a subset of $\mathcal{P} (S)$  

(i) We say that $Q$ satisfies Property (U) on $\mathcal{P} ' (S)$  if for any $H(\mu) \in \mathcal{H}_{\mathcal{P} ' (S)}$, the operator $M_{H(\mu)}$ has a unique fixed point $\mu _{H(\mu)}$. 

(ii) We say that $Q$ satisfies Property (C) on $\mathcal{P} ' (S)$
if $Q$ satisfies Property (U) on $\mathcal{P} ' (S)$ and  $M_{H(\mu)}^{n} \theta $ converges weakly to $\mu _{H(\mu)}$ for any probability measure $\theta  \in \mathcal{P}  (S)$ and any $H(\mu) \in \mathcal{H}_{\mathcal{P} ' (S)}$ where $\mu _{H(\mu)}$ is the unique fixed point of $M_{H(\mu)}$. 
\end{definition}
...
We say that $Q$ is $D$-decreasing on $\mathcal{P}'(S)$ if $Q$  is decreasing in $\mu$  with respect to $\succeq _{D}$ on $\mathcal{P}'(S)$. 

We say that $Q$ is $D$-preserving on $\mathcal{P}'(S)$ if for all  $H(\mu) \in \mathcal{H}_{\mathcal{P}'(S)}$ the function 
\begin{equation*}
    v(x):=\int f (y) Q(x ,H(\mu) ,dy) 
\end{equation*}
is in $D$ whenever $f \in D$. 
...
\begin{proposition} \label{Prop:local}
    ...
    Let $D \subseteq \mathbb{R}^{S}$ be a non-empty set such that $H$ is increasing with respect to $\succeq  _{D}$ on $\mathcal{P}'(S)$. Let $\mathcal{P}'(S)$ be a subset of  $\mathcal{P}(S)$. 
    
    Assume that $Q$ is $D$-preserving on $\mathcal{P}'(S)$ and $D$-decreasing on $\mathcal{P}'(S)$.

Assume that either of the following conditions hold: 

(i) $Q$ satisfies Property (C) on $\mathcal{P}'(S)$ and $\succeq_{D}$ is closed with respect to weak convergence. 

(ii) $Q$ satisfies Property (U)  on $\mathcal{P}'(S)$ and $(\mathcal{P}'(S),\succeq_{D})$ is a complete lattice. 


Then $Q$ has at most one invariant distribution on $\mathcal{P}'(S)$. 
\end{proposition}

The proof of Proposition \ref{Prop:local} is similar to the proof of Theorem \ref{Theorem: unique} and is therefore omitted. We now provide a simple example of a random walk to illustrate the usefulness of Proposition \ref{Prop:local}. 

\begin{example} (Local Result).  \label{Example:local}
Consider the following nonlinear Markov chain $$X_{t+1} = a X_{t} + H(\mu_{t}) - cH(\mu_{t})^{2} + \epsilon_{t+1}$$ on $\mathbb{R}$ where $0<a<1$, $c>0$, $\epsilon _{t}$ are I.I.D random variables with finite expectations, and $H(\mu_{t}) = \int m(x)  \mu_{t}(dx)$ for some increasing function $m$. Let $\mathcal{P}'(S)$ be the set probability measures such that $\mu \in \mathcal{P}'(S)$ if $H(\mu) \geq 1/(2c)$.  It is easy to see that while $Q$ is not $D$-decreasing on $\mathcal{P}(S)$ it is $D$-decreasing on $\mathcal{P}'(S)$ when $D$ is the set of all increasing functions. It is easy to see that $Q$ is $D$-preserving and $H$ is increasing with respect to $\succeq_{D}$. Hence, assuming that Property (C) holds on $\mathcal{P}'(S)$ we can apply Proposition \ref{Prop:local} to conclude that $Q$ has at most two invariant distributions on $\mathcal{P}'(S)$.
...
$$ X_{t+1} = \sum _{i=1}^{n} g_{i}(R_{1}(H(\mu _{t} )),\ldots , R_{n} (H(\mu_{t}) ) ,X_{t} )R_{i}(H(\mu _{t}) ) + Y _{t+1} $$
 ...
 Under standard assumptions on the policy function, Property (C) holds, the policy function is increasing in the current wealth, i.e., savings increase when the agent's wealth is higher, and the returns are decreasing in $H(\mu)$ with respect to first order stochastic dominance, i.e., the returns are (stochastically) lower when the total savings are higher (see \cite{acemoglu2012}, and \cite{acemoglu2018equilibrium}). Hence, from Theorem \ref{Theorem: unique} with the standard first order stochastic dominance, there is at most one stationary wealth distribution equilibrium if the total amount of savings $\sum g_{i}$ is decreasing in $H(\mu)$ which is equivalent to the property that total savings are  increasing in the rate of returns (in the economics literature, this property means that the substitution effect dominates the income effect). Hence, the key condition that implies the uniqueness of a stationary wealth distribution  equilibrium is that  savings increase with the rate of returns (a special case of this result with one financial asset and rational agents is studied in \cite{light2020uniqueness}). 
...
\begin{proposition}
\label{Topkis Fixed point}Suppose that $Z$ is a nonempty complete lattice, $E$ is a partially ordered set, and $f$ is an increasing function from $Z \times E$ into $Z$. Then, for each $e \in E$, the greatest and least fixed points 
...
\begin{proof}
    [Example \ref{example:flexible} part (ii)]
   Let $f \in D$ so $f(x) = \sum _{i=1}^{n} y_{i}x_{i} + b$ for some $y \in O$. We will show that $Q$ is $D$-preserving and $D$-decreasing. 
   
   
   We have 
 \begin{align*}
     v(x):=\int f(x')Q(x,H(\mu),dx') & = \int f(a_{1}x_{1} - \beta_{1}H(\mu)+\epsilon_{1},\ldots,a_{n}x_{n}-\beta_{n}H(\mu) + \epsilon_{n}) \phi (d \epsilon ) \\
     & = \int \sum _{i=1}^{n} y_{i}(a_{i}x_{i} - \beta_{i}H(\mu) + \epsilon_{i})\phi (d \epsilon ) + b \\
     & = \sum _{i=1}^{n} y_{i}'x_{i} + b'
      \end{align*}
with $y_{i}' = a_{i}y_{i}$ and $b' = \int \sum _{i=1}^{n} y_{i} ( -\beta_{i} H(\mu) + \epsilon_{i})\phi (d \epsilon ) + b$. Note that $y' $ is in $O$ as $y \in O$ and $a_{i} \geq 0$ for all $i$. Hence, $v$ is in $D$ which means that $Q$ is $D$-preserving. 


To show that $Q$ is $D$-decreasing  let $H(\mu_{2}) \geq H(\mu_{1})$ and note that 
\begin{align*}
\int f(x')Q(x,H(\mu_{2}),dx') & =  \int \sum _{i=1}^{n} y_{i}(a_{i}x_{i} - \beta_{i}H(\mu_{2}) + \epsilon_{i})\phi (d \epsilon ) + b \\
     & \leq  \int \sum _{i=1}^{n} y_{i}(a_{i}x_{i} - \beta_{i}H(\mu_{1}) + \epsilon_{i})\phi (d \epsilon ) + b  \\
     & = \int f(x')Q(x,H(\mu_{1}),dx')
\end{align*}
