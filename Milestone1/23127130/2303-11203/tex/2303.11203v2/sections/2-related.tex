...
\bdtitle{Depthwise separable convolution}\cite{sifre2014rigidmotiona} is a depthwise convolution followed by a pointwise convolution, to reduce both model size and complexity. Being a more computationally efficient alternative than standard convolution, it is used for mobile applications~\cite{howard2017mobilenets,sandler2018mobilenetv2,howard2019searching} and %
hardware accelerators~\cite{masters2021making}. Furthermore, it is a building block of Xception~\cite{chollet2017xception}, a deep convolutional neural network architecture that achieves state-of-the-art performance on the ImageNet~\cite{deng2009imagenet} classification task, via more efficient use of model parameterization. In this work, we propose a novel sparse variant of depthwise separable convolution, which has both the efficiency advantages of depthwise separable convolution and those of sparse convolution for processing spatially-sparse data~\cite{graham20183d}.
