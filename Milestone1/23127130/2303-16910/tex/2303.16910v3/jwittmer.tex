\documentclass[pre,twocolumn,amsmath,amssymb,floatfix,superscriptaddress,nopacs,nofootinbib]{revtex4}
...
%\title{Strain correlation functions and response to localized stress perturbation in isotropic elastic bodies:
%Large wavelength limit for two-dimensional systems}
\title{Strain correlation functions in isotropic elastic bodies:\\
Large wavelength limit for two-dimensional systems}
...
the octupolar pattern\footnote{See, e.g., the wikipedia entries on quadrupoles and
            general multipolar expansions as used, say, in electrostatics.
            For the planar harmonic basis functions $\cos(p \theta)$ or $\sin(p \theta)$
            a monopole corresponds to $p=0$,
            a dipole to $p=1$, a quadrupole to $p=2$ and an octupole to $p=4$.}
%\cite{foot_multipolar} 
observed in Fig.~\ref{fig_intro} (cf. Sec.~\ref{res_Cabcd_r} and Appendix~\ref{app_Cabcd})
...
with $\qhat_1=\cos(\theta)$ and $\qhat_2=\sin(\theta)$ and $p=0$, $2$ and $4$. 
% \cite{foot_multipolar}.
...
(The failure to sum properly over {\em all} tensorial contributions
to $R_{\alpha\beta}(\qvec)$ leads to incorrect angular dependences.)
...
two orthogonal twin force dipoles of {\em opposite} signs may be imposed at the 
origin.\footnote{Let 
us impose in a rotated coordinate system at $\alpha=-\pi/4$
a symmetric source tensor with a finite ``shear" $s^{\prime}_{12}=s$ and vanishing
diagonal components $s_{11}^{\prime} = s_{22}^{\prime} =0$.
Using Eq.~(\ref{eq_ten_intro_orthtrans}) this implies
$s_{11}=-s_{22}=s$ and $s_{12}=0$ in the original coordinate system at $\alpha=0$.}
...
\end{eqnarray}
for $q \to 0$. 
The dots mark irrelevant constant contributions.\footnote{The omitted constant terms correspond 
to localized $\delta(\rvec)$ contributions to strain correlation functions in real space. 
For example, such a contribution to $c_{1212}(\rvec)$ is $\frac{\Jone+\Jtwo}{8\beta} \delta(\rvec)$.}
%
%\cite{foot_dots}.
See Appendix~\ref{app_Cabcd} for more details. 
...
\subsection{Time-dependent strain correlations}
...
As already emphasized in Sec.~\ref{theo_response} and Sec.~\ref{theo_source}, 
the response fields contains information of both the system and the imposed source 
and the source term may either be isotropic or anisotropic.
...
\item
The present work has focused on the large-wavelength limit ($q \to 0$).
More generally,
...
(with $L(q)\to \lambda+2\mu$ and $G(q) \to \mu$ for small $q$) 
\cite{lyuda18,lyuda21b,spmP5c}.\footnote{The elastic modulus tensor $\Eabcd(\qvec)$ for isotropic bodies
               at finite wavevector $\qvec$ is not only characterized by the
               longitudinal modulus $L(q)$ and the shear modulus $G(q)$
               but also by a third modulus $M(q)$ called the ``mixed modulus" \cite{lyuda18}.
               Note that $E_{1111}^{\circ}(q) = E_{2222}^{\circ}(q) = L(q)$,
               $E_{1212}^{\circ}(q) = G(q)$ and $E_{1122}^{\circ}(q) = M(q)$
               for isotropic bodies in NRC.
               It is not possible to determine $M(q)$ solely using strain fluctuations.
               This requires the additional measurement of stress fields in NRC.
               $M(q)$ may then be obtained using either the appropriate stress-strain or stress-stress
               correlation functions in reciprocal space \cite{lyuda18,spmP5c}.}
...
%
As may be seen from Eq.~(11.4.16) of Ref.~\cite{abramowitz} the latter integral 
becomes\footnote{The infinite integral Eq.~(11.4.16) of Ref.~\cite{abramowitz}
            is expressed in terms of two coefficients $\mu$ and $\nu$
            which in our case take the (real) values $\mu=1$ and $\nu=p$.
            It is stated that Eq.~(11.4.16) holds for $\Re(\mu+\nu) > -1$,
            ...
            However, the integral divergence for $t \to \infty$ for $\mu>1/2$ is fictitious as,
            e.g., discussed in the Wikipedia entry on ``Oscillatory integrals"
            where it is noted that {\em "Oscillatory integrals make rigorous many arguments that,
            on a naive level, appear to use divergent integrals."}
            Note that Eq.~(\ref{eq_q2r_fr_p}) also holds for $p=0$ and
            finite $r>0$ which follows from the fact that the FT of
            a constant is zero everywhere except at the origin.
            See Ref.~\cite{spmP5a} for an alternative more straight-forward but also
            more lengthy derivation of Eq.~(\ref{eq_q2r_fr_p}) using the asymptotic behavior of
...
%\cite{foot_uq_grid}.
(In principle one could directly without approximation compute the displacement field $\uvec(\qvec)$ 
using Eq.~(\ref{eq_uq_uq_def}) in reciprocal space.
Unfortunately, this leads to an additional loop over all $n$ particles for each wavevector $\qvec$.)
...
%\bibliography{../bibl,foot,../../../bibl/hero,../../../bibl/BOOK,../../../bibl/JPW,../../../bibl/Alexander,../../../bibl/ANS,../../../bibl/Barrat,../../../bibl/Baschnag,../../../bibl/Berthier,../../../bibl/Bocquet,../../../bibl/Debregeas,../../../bibl/EM,../../../bibl/Fielding,../../../bibl/Fuchs,../../../bibl/Harrowell,../../../bibl/Langer,../../../bibl/Lemaitre,../../../bibl/Maret,../../../bibl/MMuller,../../../bibl/Reichman,../../../bibl/Szamel,../../../bibl/Tanguy,../../../bibl/Tarjus,../../../bibl/Provencher,../../../bibl/Voigtmann,../../../bibl/Weeks}

\begin{thebibliography}{10}

\bibitem{McConnell}
McConnell AJ.
\newblock Applications of Tensor Analysis.
\newblock Hassell Street Press; 2021.

\bibitem{Schouten}
Schouten JA.
\newblock Tensor Analysis for Physicists.
\newblock Oxford: Dover Publications; 2015.

\bibitem{Schultz_Piszachich}
Schultz-Piszachich W.
\newblock Mathematik 11: Tensoralgebra und -analysis.
\newblock Thun und Frankfurt/Main: Verlag Harri Deutsch; 1977.

\bibitem{spmP5a}
Wittmer JP, Semenov AN, Baschnagel J.
\newblock Correlations of tensor field components in isotropic systems with an
  application to stress correlations in elastic bodies.
\newblock preprint http://arxivorg/abs/230316571. 2023.

\bibitem{Lambourne}
Lambourne RJA.
\newblock Relativity, Gravitation, and Cosmology.
\newblock Cambridge: Cambridge University Press; 2010.

\bibitem{Frankel_T}
Frankel T.
\newblock The Geometry of Physics. An Introduction.
\newblock 3rd ed. Cambridge: Cambridge University Press; 2012.

\bibitem{ChaikinBook}
Chaikin PM, Lubensky TC.
\newblock Principles of condensed matter physics.
\newblock Cambridge University Press; 1995.

\bibitem{LandauElasticity}
Landau LD, Lifshitz EM.
\newblock Theory of Elasticity.
\newblock New York: Pergamon Press; 1959.

\bibitem{TadmorCMTBook}
Tadmor EB, Miller RE, Elliot RS.
\newblock Continuum Mechanics and Thermodynamics.
\newblock Cambridge: Cambridge University Press; 2012.

\bibitem{Klix12}
Klix C, Ebert F, Weysser F, Fuchs M, Maret G, Keim P.
\newblock Glass elasticity from particle trajectories.
\newblock Phys Rev Lett. 2012;109:178301.

\bibitem{Klix15}
Klix CL, Maret G, Keim P.
\newblock Discontinuous shear modulus determines the glass transition
  temperature.
\newblock Phys Rev X. 2015;5:041033.

\bibitem{Fuchs17}
Maier M, Zippelius A, Fuchs M.
\newblock Emergence of long-ranged stress correlations at the liquid to glass
  transition.
\newblock Phys Rev Lett. 2017;119:265701.

\bibitem{Fuchs18}
Maier M, Zippelius A, Fuchs M.
\newblock Stress auto-correlation tensor in glass-forming isothermal fluids:
  From viscous to elastic response.
\newblock J Chem Phys. 2018;149:084502.

\bibitem{Fuchs19}
Vogel F, Zippelius A, Fuchs M.
\newblock Emergence of Goldstone excitations in stress correlations of
  glass-forming colloidal dispersions.
\newblock Europhys Lett. 2019;125:68003.

\bibitem{Lemaitre15}
Lema{\^\i}tre A.
\newblock Tensorial analysis of Eshelby stresses in 3d supercooled liquids.
\newblock J Chem Phys. 2015;143:164515.

\bibitem{Lemaitre18}
Lema{\^\i}tre A.
\newblock Stress correlations in glasses.
\newblock J Chem Phys. 2018;149:104107.

\bibitem{lyuda18}
Klochko L, Baschnagel J, Wittmer JP, Semenov AN.
\newblock Long-range stress correlations in viscoelastic and glass-forming
  fluids.
\newblock Soft Matter. 2018;14:6835.

\bibitem{lyuda22a}
Klochko L, Baschnagel J, Wittmer J, Meyer H, Benzerara O, Semenov AN.
\newblock Theory of length-scale dependent relaxation moduli and stress
  fluctuations in glass-forming and viscoelastic liquids.
\newblock J Chem Phys. 2022;156:164505.

\bibitem{MM22}
Steffen D, Schneider L, M\"uller M, Rottler J.
\newblock Molecular simulations and hydrodynamic theory of nonlocal shear
  stress correlations in supercooled fluids.
\newblock J Chem Phys. 2022;157:064501.

\bibitem{Debregeas03}
Kabla A, Debr\'egeas G.
\newblock Local Stress Relaxation and Shear Banding in a Dry Foam under Shear.
\newblock Phys Rev Lett. 2003;90:258303.

\bibitem{WXBB15}
Wittmer JP, Xu H, Benzerara O, Baschnagel J.
\newblock Fluctuation-dissipation relation between shear stress relaxation
  modulus and shear stress autocorrelation function revisited.
\newblock Mol Phys. 2015;113:2881.

\bibitem{Weeks15}
Desmond KW, Weeks ER.
\newblock Measurement of Stress Redistribution in Flowing Emulsions.
\newblock Phys Rev Lett. 2015;115:098302.

\bibitem{Bocquet04}
Picard G, Ajdari A, Lequeux F, Bocquet L.
\newblock Elastic consequences of a single plastic event: A step towards the
  microscopic modeling of the flow of yield stress fluids.
\newblock Eur Phys J E. 2004;15:371.

\bibitem{Bocquet09}
Bocquet L, Colin A, Ajdari A.
\newblock Kinetic theory of plastic flow in soft glassy materials.
\newblock Phys Rev Lett. 2009;103:036001.

\bibitem{Lemaitre13}
Chattoraj J, Lema{\^\i}tre A.
\newblock Elastic Signature of Flow Events in Supercooled Liquids Under Shear.
\newblock Phys Rev Lett. 2013;111:066001.

\bibitem{Barrat13}
Mizuno H, Mossa S, Barrat JL.
\newblock Measuring spatial distribution of the local elastic modulus in
  glasses.
\newblock Phys Rev E. 2013;87:042306.

\bibitem{Barrat14c}
Nicolas A, Rottler J, Barrat JL.
\newblock Spatiotemporal correlations between plastic events in the shear flow
  of athermal amorphous solids.
\newblock EPJE. 2014;37:50.

\bibitem{FlennerSzamel15b}
Flenner E, Szamel G.
\newblock Long-range spatial correlations of particle displacements and the
  emergence of elasticity.
\newblock Phys Rev Lett. 2015;114:025501.

\bibitem{Fuchs16}
Illing B, Fritschi S, Hajnal D, Klix C, Keim P, Fuchs M.
\newblock Strain pattern in supercooled liquids.
\newblock Phys Rev Lett. 2016;117:208002.

\bibitem{Fuchs18b}
Hassani M, Zirdehi EM, Kok K, Schall P, Fuchs M, Varnik F.
\newblock Long-range strain correlations in 3d quiescent glass forming liquids.
\newblock Europhysics Letters. 2018;124:18003.

\bibitem{Reichman21}
Liu C, Biroli G, Reichman DR, Szamel G.
\newblock Dynamics of liquids in the large-dimensional limit.
\newblock Phys Rev E. 2021;104:054606.

\bibitem{Reichman21b}
Chacko RN, Landes FP, Biroli G, Liu ODAJ, Reichman DR.
\newblock Elastoplasticity Mediates Dynamical Heterogeneity Below the Mode
  Coupling Temperature.
\newblock Phys Rev Lett. 2021;127:048002.

\bibitem{AllenTildesleyBook}
Allen MP, Tildesley DJ.
\newblock Computer Simulation of Liquids, 2nd Edition.
\newblock Oxford: Oxford University Press; 2017.

\bibitem{HansenBook}
Hansen JP, McDonald IR.
\newblock Theory of simple liquids.
\newblock New York: Academic Press; 2006.
\newblock 3nd edition.

\bibitem{ForsterBook}
Forster D.
\newblock Hydrodynamic Fluctuations, Broken Symmetry, and Correlation
  Functions.
\newblock New York: Perseus Books; 1995.

\bibitem{WXP13}
Wittmer JP, Xu H, Poli\'nska P, Weysser F, Baschnagel J.
\newblock Shear modulus of simulated glass-forming model systems: Effects of
  boundary condition, temperature and sampling time.
\newblock J Chem Phys. 2013;138:12A533.

\bibitem{lyuda19a}
Klochko L, Baschnagel J, Wittmer JP, Semenov AN.
\newblock Relaxation dynamics in supercooled oligomer liquids: From
  shear-stress fluctuations to shear modulus and structural correlations.
\newblock J Chem Phys. 2019;151:054504.

\bibitem{spmP1}
George G, Klochko L, Semenov AN, Baschnagel J, Wittmer JP.
\newblock Ensemble fluctuations matter for variances of macroscopic variables.
\newblock EPJE. 2021;44:13.

\bibitem{spmP2}
George G, Klochko L, Semenov AN, Baschnagel J, Wittmer JP.
\newblock Fluctuations of non-ergodic stochastic processes.
\newblock EPJE. 2021;44:54.

\bibitem{FerryBook}
Ferry JD.
\newblock Viscoelastic properties of polymers.
\newblock New York: John Wiley \& Sons; 1980.

\bibitem{RubinsteinBook}
Rubinstein M, Colby RH.
\newblock Polymer Physics.
\newblock Oxford: Oxford University Press; 2003.

\bibitem{DoiEdwardsBook}
Doi M, Edwards SF.
\newblock The Theory of Polymer Dynamics.
\newblock Oxford: Clarendon Press; 1986.

\bibitem{DonthGlasses}
Donth JKG.
\newblock The Glass Transition: Relaxation dynamics in liquids and disordered
  materials.
\newblock Berlin-Heidelberg: Springer; 2001.

\bibitem{Argon79}
Argon A, Kuo H.
\newblock Plastic flow in a disordered bubble raft (an analog of a metallic
  glass).
\newblock Materials Science and Engineering. 1979;39:101.

\bibitem{Tanguy11}
Rodney D, Tanguy A, Vandembroucq D.
\newblock Modeling the mechanics of amorphous solids at different length scale
  and time scale.
\newblock Modelling Simul Mater Sci Eng. 2011;19:083001.

\bibitem{Barrat18}
Nicolas A, Ferrero E, Martens K, Barrat JL.
\newblock Deformation and flow of amorphous solids: Insights from elastoplastic
  models.
\newblock Rev Mod Phys. 2018;90:045006.

\bibitem{Langer98}
Falk ML, Langer JS.
\newblock Dynamics of viscoplastic deformation in amorphous solids.
\newblock Phys Rev E. 1998;57:7192.

\bibitem{lyuda21b}
Klochko L, Baschnagel J, Wittmer JP, Semenov AN.
\newblock Relaxation moduli of glass-forming systems: temperature effects and
  fluctuations.
\newblock Soft Matter. 2021;17:7867.

\bibitem{Berthier17}
Ninarello A, Berthier L, Coslovich D.
\newblock Models and Algorithms for the Next Generation of Glass Transition
  Studies.
\newblock Phys Rev X. 2017;7:021039.

\bibitem{spmP5c}
Wittmer JP, Semenov AN, Baschnagel J.
\newblock Strain fluctuations at finite wavevectors for isotropic colloidal
  glasses using natural rotated coordinates.
\newblock in preparation. 2023.

\bibitem{Tarjus15}
Vest JP, Tarjus G, Viot P.
\newblock Mode-coupling approach for the slow dynamics of a liquid on a
  spherical substrate.
\newblock J Chem Phys. 2015;143:084505.

\bibitem{Tarjus17}
Turci F, Tarjus G, Royall CP.
\newblock From Glass Formation to Icosahedral Ordering by Curving
  Three-Dimensional Space.
\newblock Phys Rev Lett. 2017;118:215501.

\bibitem{Mossa19}
Mizuno H, Mossa S.
\newblock Mat Phys. 2019;22:43604.

\bibitem{spmP4}
Wittmer JP, Semenov AN, Baschnagel J.
\newblock Different spatial correlation functions for non-ergodic stochastic
  processes of macroscopic systems.
\newblock EPJE. 2022;45:65.
...
\newblock New York: Dover; 1964.

\bibitem{WXP13c}
Wittmer JP, Xu H, Poli\'nska P, Gillig C, Helfferich J, Weysser F, et~al.
\newblock Compressibility and pressure correlations in isotropic solids and
  fluids.
\newblock Eur Phys J E. 2013;36:131.

\bibitem{WXB15}
Wittmer JP, Xu H, Baschnagel J.
\newblock Shear-stress relaxation and ensemble transformation of shear-stress
  auto-correlation functions revisited.
\newblock Phys Rev E. 2015;91:022107.

\bibitem{Lutsko88}
Lutsko JF.
\newblock Stress and elastic constants in anisotropic solids: Molecular
  dynamics techniques.
\newblock J Appl Phys. 1988;64:1152.

\bibitem{Lutsko89}
Lutsko JF.
\newblock Generalized expressions for the calculation of elastic constants by
  computer simulation.
\newblock J Appl Phys. 1989;65:2991.
