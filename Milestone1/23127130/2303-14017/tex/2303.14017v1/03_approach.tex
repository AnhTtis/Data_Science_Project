...
\begin{aligned}
    ...
    \mathcal{L}_{pc-kl}(\boldsymbol{Y}, \hat{\boldsymbol{Y}}) &= \frac{1}{P}\sum^P_{p=1} \mathbf{KL}( \frac{\phi_p(\boldsymbol{Y})}{\sum \phi_p(\boldsymbol{Y})}-\frac{\phi_p(\hat{\boldsymbol{Y}})}{\sum \phi_p(\hat{\boldsymbol{Y}})}),
\end{aligned}
\end{equation}
...
\end{aligned}
\end{equation}
where $\lambda_{adv}$, $\lambda_{img}$, $\lambda_{pcl}$, and $\lambda_{offset}$ are hyperparameters to adjust the weight of each loss function.
