...
\renewcommand\clap[1]{\mathpalette\doclap{#1}}
\newcommand\doclap[2]{\hbox to0pt{\hss$#1#2$\hss}}
\newcommand\bret[1]{\land^{\clap{#1}}_{\vphantom l}}
...
\[\Bigl(\ET_{i<n}\LOR_{l<k}q_{il}\to\LOR_{\substack{\clap{i<j<n}\\l<k}}(q_{il}\land q_{jl}\land p_{ij})\Bigr)
  \lor\Bigl(\ET_{i<n}\LOR_{l<k}q_{il}\to\LOR_{\substack{\clap{i<j<n}\\l<k}}(r_{il}\land r_{jl}\land\neg p_{ij})\Bigr),\]
which can be made negation-free using the $\vec p\,'$ variables:
\[\ET_{i<j<n}(p_{ij}\lor p'_{ij})\to
  \Bigl(\ET_{i<n}\LOR_{l<k}q_{il}\to\LOR_{\substack{\clap{i<j<n}\\l<k}}(q_{il}\land q_{jl}\land p_{ij})\Bigr)
  \lor\Bigl(\ET_{i<n}\LOR_{l<k}r_{il}\to\LOR_{\substack{\clap{i<j<n}\\l<k}}(r_{il}\land r_{jl}\land p'_{ij})\Bigr).\]
...
\begin{align*}
P&=\{(p_i\to u)\to(p'_i\to u)\to u,\alpha_j\to u:i<n,j<2\},\\
...
In the first version of this paper, we used tautologies based on the original Clique--Colouring disjoint pair rather
than Colouring-Cocolouring. They have larger size, viz.\ $O(n^2k^2)$, but the monotone circuit lower bound increases
even more to $2^{\Omega(k^{1/2})}$ by Alon--Boppana~\cite{alonbop}. For $k\approx\sqrt n$, this improves the bound in
Theorem~\ref{thm:main} to $2^{\Omega(\lh\fii^{1/12})}$; if we raise $k$ to $\approx(n/\log n)^{2/3}$ (the largest value to
which the Alon--Boppana result applies), it improves further to $2^{\Omega(\lh\fii^{1/10}/(\log\lh\fii)^{1/5})}$.

Another possibility to entertain is that the bounds in Theorem~\ref{thm:hru-pud} might be improvable using an ad hoc analysis
(perhaps not applicable to real circuits) instead of the general method from Jukna~\cite{juk:boolf} (e.g., note that
Jukna's machinery only gives a $2^{\Omega(k^{1/4})}$ lower bound for Clique--Colouring, worse than Alon--Boppana). Say,
if we could improve the bound to $2^{\Omega(k^{1/2})}=2^{\Omega(n^{1/4})}$, the bound in Theorem~\ref{thm:main} would
improve to $2^{\Omega(\lh\fii^{1/10})}$. But this is only a speculation.
...
and the anonymous referee for improvements in the presentation.
