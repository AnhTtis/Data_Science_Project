...
Denote local features extracted from an image as $({\bf p},{\bf d})$, where $\mathbf p_i := (x, y, c)_i$ corresponds to the 2D position $x,y$ and confidence score $c$ for the $i$-th local feature, and $\mathbf d_i \in \mathbb R^D$ is the $i$-th local descriptor.
A keypoint encoder combines $\mathbf p_i$ and $\mathbf d_i$ to produce a new 
representation $\mathbf x_i$ which will be used for matching as follows:
...
between the two feature sets. Ground truth correspondences are recovered for pairs of images 
using either known homographies or complete camera and depth information.
...
Each pixel takes on a value proportional to the disparity $1/d$, where $d$ is the estimated depth of the pixel. Among various other reasons, this formulation better handles depth values at large distances. Compared to depth obtained from metric-based depth sensors (stereo cameras, LIDAR, etc.), MDE suffers from scale ambiguity, which makes it hard to be used directly for pose estimation. However, for our purpose, we found that MDE provides sufficient 3D signals to boost correspondence estimation, and we use the publicly available DPT~\cite{ranftl2021vision} model for MDE. %
...
3D-infused representation $\mathbf x'_i$ can be directly used in the GNN with the same architecture and loss functions to improve feature matching. Note that summing up the features instead of concatenating allows for pre-training on datasets where no 3D information is available, as described below.

\noindent\textbf{Improved positional encoding.}
However, MLPs are know to have a spectral bias to learn overly smooth functions~\cite{tancik2020fourier}. This is particularly prevalent in models that learn function over a low dimensional space, such as from NOCS or inverse depth. Positional encoding~\cite{tancik2020fourier, vaswani2017attention} lifts the input representation to a higher dimension space using periodic functions of increasing frequency. In our model, we use them to encode $\mathbf{n_i}$ before passing them onto the MLP. 
...
Limitations in object-centric data with the necessary match-level annotations may cause the model 
...
can be used to initialize some layers of LFM-3D. We bootstrap the LFM-3D model by training the 
$\text{MLP}_{\text{2D}}$ and $\text{GNN}$ layers of the matcher model from scratch for many 
iterations on synthetic homography and outdoor scene multi-view datasets (similar 
to~\cite{sarlin2020superglue}). Once the model has a strong understanding of the 2D 
