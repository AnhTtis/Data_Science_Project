...
%\usepackage{epstopdf} % pdflatex: '--enable-write18', texify: '--tex-option=--enable-write18'

\usepackage{epsfig}
%\usepackage[round,authoryear,semicolon]{natbib} % style of bib.~entries (see http://merkel.zoneo.net/Latex/natbib.php)
\usepackage{natbib}
...
\section{Change Point Detection with  \texorpdfstring{$\ell_1$}{l1}-Regularization in High-Frequency Data}
...
We now work in the standard formulation of LSTV instead of its LASSO counter part. In particular, we write
...
%\vspace{-2mm}
In this section, we aim at estimating the change point locations. The change point estimates proposed here are obtained from the $\widehat{\beta}_i(\lambda)$'s. Let us define the sets of active variables by
...
\begin{figure}
\begin{center}
\includegraphics[width=1\textwidth]{images_final/LSTVvstGARCHAAE.jpg}
\vspace*{-4mm}
\caption{Percentage improvement of one-step-ahead volatility forecasts in terms of predicted Average Squared Error relative to the QV model.  LSTV$^*$(BV) vs $t$-GARCH(1,1) model. LSTV$^*$(BV) consistently performs better across all the data frequencies.}
\vspace*{-4mm}
\label{fig:AAEtGARCH}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[width=1\textwidth]{images_final/LSTVFIGARCH.jpg}
\vspace*{-4mm}
\vspace{-4mm}
\caption{Percentage improvement of one-step-ahead volatility forecasts in terms of predicted Average Squared Error relative to the FIGARCH model.  LSTV$^*$(BV) vs  LSTV$^*$(FIGARCH) model. LSTV$^*$(BV) consistently performs better across all the data frequencies.}
\label{fig:FIGARCH}
\end{center}
\end{figure}
\end{comment}
