...
This will need likely the development of new techniques for speech separation. One of the major issues of having more than two speakers is that the speaker tracking for arbitrarily long inputs is significantly more difficult, potentially leading to confusion errors. Another issue is that the channel leakage problem also becomes more challenging as the number of output streams grows (e.g. $4$ channels for a max of $4$ local speakers). This latter can be addressed by adopting separation methods which can handle a variable number of output channels~\cite{nachmani2020voice}. 
At the same time, novel strategies should be investigated to mitigate the separation/ASR degradation when using end-to-end fine-tuned SSGD models. This includes distortion-less constraints and/or continual learning approaches. 
