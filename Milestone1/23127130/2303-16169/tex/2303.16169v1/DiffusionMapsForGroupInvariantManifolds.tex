...
\usepackage{bm, amsmath, mathrsfs, amssymb, amsthm, amscd, amsfonts, graphicx, color,epsfig,latexsym,eucal,layout,fancyhdr, colonequals, enumerate, hyperref, fontenc, hyperref, graphics, verbatim, psfrag}
\allowdisplaybreaks
...
\date{\today\footnote{Just prior to posting this preprint, the authors became aware of similar and simultaneous results achieved by Rosen, Cheng and Shkolnisky.  Both teams conducted their research independently.}}
...
Our approach consists in augmenting the data-induced graph Laplacian by integrating over orbits under the action of $K$ of the existing data points. 
We prove that this $K$-invariant Laplacian operator $L$ can be diagonalized by using the unitary irreducible representation matrices of $K$, and we provide an explicit formula for computing the eigenvalues and eigenvectors of $L$. Moreover, we show that the normalized Laplacian operator $L_N$ converges to the Laplace-Beltrami operator of the data manifold with an improved convergence rate, where the improvement grows with the dimension of the symmetry group $K$. 
...
In manifold learning, a given data set $\mathcal{X} = \{x_1, \dots, x_N\} \subseteq \mathbb{R}^\mathcal{D}$ is assumed to lie or near on a low-dimensional manifold $\mathcal{M}$ of dimension $d \ll \mathcal{D}$. 
...
\section{Setting}
Let $\mathcal{M}$ be a compact 
$d$-dimensional smooth manifold without boundary embedded in $\mathbb{C}^\mathcal{D}$.  Consider data points $\{ x_1, x_2, \dots, x_N \} \subseteq \mathbb{C}^\mathcal{D}$ sampled independently from the uniform probability distribution $p(x)$ on $\mathcal{M}$ \cite{lee2012smooth}. \\


\noindent Let $K$ be a compact group acting linearly and by isometries on $\mathbb{C}^\mathcal{D}$ via
\begin{align*}
  \varphi  \colon \mathbb{C}^{\mathcal{D}} \times K &\to \mathbb{C}^{\mathcal{D}} \\
   (x, \kappa) &\mapsto \varphi(x, \kappa) =: \kappa \cdot x. 
   \end{align*}
Moreover, we assume $\mathcal{M}$ is $K$-invariant: for every $x \in \mathcal{M}$ and $\kappa \in K$, it holds that $\kappa \cdot x \in \mathcal{M}$.\\ 


\section{The $K$-invariant graph Laplacian and its spectrum}\label{sec: graph Laplacian}

\noindent Take $\Gamma \colonequals \{1, 2, \dots, N\} \times K$, and consider the space $\mathcal{H} = \mathcal{L}^2(\Gamma)$ of square integrable functions $f \colon \Gamma \to \mathbb{C}$ equipped with the inner product 
\[ \langle g, f \rangle \colonequals \sum_{i=1}^N \int_{K} g^\ast(i, \kappa) f(i, \kappa) d\kappa. \]
This is an infinite-dimensional complex Hilbert space. Note that, for any $f \in \mathcal{H}$, we can write $f(i, \kappa) = f_i(\kappa)$ with $\{f_i\}_{i=1}^N \in \mathcal{L}^2(K)$. Hence, we can view a function in $\mathcal{H}$ as an $N$-dimensional vector where each entry is a square-integrable function on $K$. 
\\

\noindent For any pair of data points $x_i, x_j$ and any pair of elements $\kappa, \lambda$ in $K$, we define the affinity between the elements $\kappa \cdot x_i$ and $\lambda \cdot x_j$ in $\mathbb{C}^\mathcal{D}$ as
...
\end{lemma}
\begin{proof}
This follows from the fact that $K$ acts by isometries on $\mathbb{C}^\mathcal{D},$ since acting by $k^{-1}$ gives
\begin{align*}
    \vert\vert \kappa \cdot x_i - \lambda \cdot x_j\vert\vert^2 = \vert\vert x_i - \kappa^{-1} \lambda\cdot x_j\vert\vert^2.
\end{align*}
\end{proof}

\noindent Let $D$ be the diagonal matrix with diagonal entries given by 
\[ D_{ii} = \sum_{j=1}^N \int_{K} W_{ij}(\text{Id}, \lambda) d\lambda.  \]
Note that the entries $D_{ii}$ are constant. Moreover, as $d\lambda$ is a Haar measure and thus invariant under left translation, we have that
\[ D_{ii} = \sum_{j=1}^N \int_{K} W_{ij}(\text{Id}, \lambda) d\lambda = \sum_{j=1}^N \int_{K} W_{ij}(\kappa, \lambda) d\lambda. \]
That is, $D_{ii}$ is independent of the group element $\kappa$ in the first argument of $W_{ij}$. \\


\noindent Define the graph Laplacian $L \colon \mathcal{H} \to \mathcal{H}$ by $Lf = Df - Wf$. More explicitly, it acts on a function $f \in \mathcal{H}$ as
...
\noindent From here we conclude that $L$ is positive semidefinite, since $\langle f, Lf \rangle \geq 0$ for all $f \in \mathcal{H}$. \\

\noindent We are interested in finding the eigenvalues and eigenvectors of the graph Laplacian $L$. In doing so, we need to take the Fourier transform of each  $W_{ij}(\kappa, \lambda)$, which involves the representation theory of $K$ \cite{sternberg1995group, chirikjian2016harmonic}. \\

\noindent For each $i,j \in \{1, \dots, N\}$, consider the Fourier transform
\begin{align*}
...
\begin{align*}
    ...
    = \frac{1}{\mathrm{Vol}(K)}\sum_{\ell \in \mathcal{I}} \text{dim} E_\ell \sum_{m = 1}^{\mathrm{dim}E_\ell} \sum_{n = 1}^{\mathrm{dim}E_\ell} \left[\widehat{W}_{ij}^{(\ell)} \right]_{mn} {U_{\ell}((\kappa^{-1}\lambda)^{-1})}_{mn}.  \\
\end{align*}

\noindent Next, form the block matrix $\widehat{W}^{(\ell)}$ by putting $\widehat{W}_{ij}^{(\ell)}$ in the $(i,j)$-th block. This is a matrix of size $N(\mathrm{dim}E_{\ell}) \times N(\text{dim}E_{\ell})$. 
...
\begin{align*}
  \left[\widehat{W}^{(\ell)}_{ij} \right]^\ast_{mn}  &= \int_K W_{ij}(Id, \kappa)\left[U_\ell(\kappa)\right]^\ast_{mn}d\kappa \\
  &= \int_K W_{ji}(Id, \kappa^{-1})\left[U_\ell(\kappa^{-1})\right]_{nm}d\kappa \\
   &= \int_K W_{ji}(Id, \kappa)\left[U_\ell(\kappa)\right]_{nm}d\kappa \\
   &= \left[ \widehat{W}^{(\ell)}_{ji} \right]_{nm}.
\end{align*}
\end{proof}

\noindent Form the matrix $S_\ell = D \otimes I_{\text{dim}E_{\ell}} - \widehat{W}^{(\ell)}$, where $D \otimes I_{\text{dim}E_{\ell}}$ denotes the Kronecker product of the matrices $D$ and $I_{\text{dim}E_{\ell}}$. As the following theorem proves, we can compute the eigenvalues and eigenvectors of the graph Laplacian $L$ from those of  $S_\ell$.
...
\noindent Now let us prove the completeness of the eigenfunctions $\{ \Phi_{m}^{(\ell,1)}, \dots, \Phi_{m}^{(\ell,N\mathrm{dim}E_{\ell})}\}_{\ell, 1\leq m \leq \mathrm{dim}E_\ell}$. Note that
\[\mathcal{L}^2(\Gamma) = \mathcal{L}^2(K)\otimes \mathbb{C}^N = \bigoplus_{\ell \in \mathcal{I}} \left( E_\ell^{\oplus \mathrm{dim}E_\ell} \otimes \mathbb{C}^N\right),\]
where the second equality comes from the Peter-Weyl Theorem. For each $\ell$, let us see that the set $\{   \Phi_{m}^{(\ell,1)}, \dots, \Phi_{m}^{(\ell,N\text{dim}E_{\ell})}\}_{1\leq m \leq \mathrm{dim}E_\ell}$ is an orthogonal basis for the subspace $E_\ell^{\oplus \mathrm{dim}E_\ell} \otimes \mathbb{C}^N$ of $\mathcal{L}^2(\Gamma)$. We have $N(\mathrm{dim}E_\ell)^2 = \mathrm{dim}(E_\ell^{\oplus \mathrm{dim}E_\ell} \otimes \mathbb{C}^N)$ eigenfunctions, and the following computation proves they are orthogonal.
...
\begin{align*}
    ...
    &= \sum_{i=1}^N \int_K \sum_{n = 1}^{\mathrm{dim}E_\ell} {\left[U_\ell(\kappa)\right]^\ast_{mn}} ({v^{(\ell,s)}_{(i-1)\text{dim}E_\ell+ n}})^\ast \sum_{n^\prime = 1}^{\mathrm{dim}E_\ell} U_\ell(\kappa)_{m^\prime n^\prime}v^{(\ell,s^\prime)}_{(i-1)\text{dim}E_\ell+ n^\prime} d\kappa \\
    ...
    &= \sum_{i=1}^N \sum_{n = 1}^{\mathrm{dim}E_\ell} \sum_{n^\prime = 1}^{\mathrm{dim}E_\ell}({v^{(\ell,s)}_{(i-1)\text{dim}E_\ell+ n}})^\ast v^{(\ell,s^\prime)}_{(i-1)\text{dim}E_\ell+ n^\prime} \frac{\mathrm{\mathrm{Vol}}(K)}{\mathrm{dim}E_\ell} \delta_{m m^\prime} \delta_{n n^\prime}\\
    &= \frac{\mathrm{Vol}(K)}{\mathrm{dim}E_\ell}\delta_{m m^\prime} \sum_{i=1}^N \sum_{n = 1}^{\mathrm{dim}E_\ell} ({v^{(\ell,s)}_{(i-1)\text{dim}E_\ell+ n}} )^\ast v^{(\ell,s^\prime)}_{(i-1)\text{dim}E_\ell+ n^\prime}  \\
    &= \frac{\mathrm{Vol}(K)}{\mathrm{dim}E_\ell}\delta_{m m^\prime} \langle v^{(\ell,s)}, v^{(\ell,s^\prime)} \rangle_{\mathbb{C}^{N\text{dim}E_\ell}} \\
    &= \frac{\mathrm{Vol}(K)}{\mathrm{dim}E_\ell}\delta_{m m^\prime} \delta_{s s^\prime}.
\end{align*}
\end{proof}
...
&= \sum_{j=1}^N \int_K \frac{1}{\mathrm{Vol}(K)}\sum_{\ell \in \mathcal{I}^\prime} \text{dim} E_\ell \sum_{m^\prime = 1}^{\mathrm{dim}E_\ell} \sum_{n^\prime = 1}^{\mathrm{dim}E_\ell} \left[\widehat{W}_{ij}^{(\ell)} \right]_{m^\prime n^\prime} {U_{\ell^\prime}((\kappa^{-1}\lambda)^{-1})}_{m^\prime n^\prime}  \sum_{n = 1}^{\mathrm{dim}E_\ell} U_\ell(\lambda)_{mn}v_{(j-1)\text{dim}E_\ell+ n} d\lambda \\
...
\noindent The operator $L_N$ is generally not hermitian, but it is similar to the symmetric Laplacian $L_{\mathrm{sym}} = D^{-1/2} L D^{-1/2}$, a hermitian matrix, and hence $L_N$ is diagonalizable. More precisely, 
...
Here $d\lambda$ is the Haar measure on the compact group $K$. Thus, it is invariant under left translations, meaning that $d(\kappa \lambda) = d\lambda$ for any $\kappa \in K$. \\

\noindent Now let us take the limit as $N \to \infty$ of the numerator of the second term in Equation \eqref{eq: F/G}. By the law of large numbers, we get
\begin{align*}
    \lim_{N \to \infty} \frac{1}{N}\sum_{j=1}^N F_{i,\kappa}(x_j) &= \mathbb{E}\left[ F_{i,\kappa}(x_j)\right] \\
    &= \int_{\mathcal{M}} F_{i,\kappa}(x)p(x)dx \\
    &= \int_{\mathcal{M}} \int_K \exp\left(-|| \kappa \cdot x_i - \lambda\cdot x ||^2/\varepsilon\right)f(\lambda\cdot x)  p(x) d\lambda dx \\
     &= \int_{\mathcal{M}} \int_K \exp\left(-|| \kappa \cdot x_i - x^\prime ||^2/\varepsilon\right)f(x^\prime)  p(\lambda^{-1}x^\prime) d\lambda dx^\prime \\
     ...
     &= \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\int_{\mathcal{M}}  \exp\left(-|| \kappa \cdot x_i - x ||^2/\varepsilon\right)f(x)  dx. \numberthis \label{eq: limit F as N goes to infinity}
\end{align*}
Here we made the change of variables $x^\prime = \lambda \cdot x$. Moreover, note that $dx^\prime = dx$ since the Jacobian determinant of the change of coordinates induced by the map $\varphi_\lambda \colon \mathcal{M} \to \mathcal{M}$ with $\varphi_\lambda(x) = \lambda \cdot x$ is equal to $1$, as $K$ acts by isometries on $\mathbb{C}^{\mathcal{D}}$, and thus on $\mathcal{M}$. \\

\noindent The calculation for $G_{i,\kappa}$ is the same as above, taking $f = 1$. Hence, we have
\begin{align*}
    \lim_{N \to \infty} \frac{1}{N}\sum_{j=1}^N G_{i,\kappa}(x_j) = \mathbb{E}\left[ G_{i,\kappa}(x_j)\right] = \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\int_{\mathcal{M}}  \exp\left(-|| \kappa \cdot x_i - x ||^2/\varepsilon\right)  dx. \numberthis \label{eq: limit G as N goes to infinity}
\end{align*}
...
\begin{align*}
    ...
    &= \frac{4}{\varepsilon}\left[ f(\kappa\cdot x_i) - \frac{\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\int_{\mathcal{M}}  \exp\left(-|| \kappa \cdot x_i - x ||^2/\varepsilon\right)f(x)  dx}{\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\int_{\mathcal{M}}  \exp\left(-|| \kappa \cdot x_i - x ||^2/\varepsilon\right)  dx}\right]\\
...
\begin{align}\label{eq: p_-N, alpha}
    p_{-} (N, \alpha) &= \mathbb{P}\left[  \frac{ \frac{1}{N}\sum_{\substack{j=1 \\ j \neq i }}^N F_{i,\kappa}(x_j) }{\frac{1}{N}\sum_{\substack{j=1 \\ j \neq i }}^N G_{i,\kappa}(x_j) } - \frac{\mathbb{E}[F_{i,\kappa}]}{\mathbb{E}[G_{i,\kappa}]} < -\alpha \right].
\end{align}
We note that removing the diagonal terms (the case $j = i$) in the sums does not affect the convergence rate. 
...
\end{align}
We are interested in finding an approximation to $\mathbb{E}[Y_j^2]$ in terms of $\varepsilon$ besides the current $O(\alpha)$ term. To do this, we need to approximate the different first and second moments involving $F_{i, \kappa}$ and $G_{i, \kappa}$. The first moments are given by
\begin{align}
    &\mathbb{E}[F_{i,\kappa}(x_j)] = \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} \int_\mathcal{M}  \exp\left(-|| \kappa \cdot x_i - x ||^2/\varepsilon\right)f(x)  dx = \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} (\pi\varepsilon)^{d/2}[f(\kappa \cdot x_i) + O(\varepsilon) ], \label{eq: first moment F}  \\
   &\mathbb{E}[G_{i,\kappa}(x_j)] = \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M}} \int_\mathcal{M}  \exp\left(-|| \kappa \cdot x_i - x ||^2/\varepsilon\right)  dx = \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} (\pi\varepsilon)^{d/2}[1 + O(\varepsilon) ]. \label{eq: first moment G} 
\end{align}
And from Lemma \ref{lemma: second moments of F and G}, the second moments are
\begin{align}
    \mathbb{E}\left[F^2_{i,\kappa}\right] &= \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}}  \left[ \frac{ V(\kappa \cdot x_i)f^2(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right], \\
    \mathbb{E}\left[G^2_{i,\kappa}\right] &= \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}} \left[ \frac{ V(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right],\\    \mathbb{E}\left[F_{i,\kappa}G_{i,\kappa}\right] &= \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}}  \left[ \frac{ V(\kappa \cdot x_i)f(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right]. \label{eq: second moment FG}
\end{align}
...
\begin{align*}
    \mathbb{E}[Y_j^2] = &\left(\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\right)^3 (\pi \varepsilon)^d \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}}  \left[ \frac{V(\kappa \cdot x_i)f(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right] \\
    &-2 \left(\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\right)^3 (\pi \varepsilon)^d \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}}  \left[ \frac{V(\kappa \cdot x_i)f(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right] \\
    &+  \left(\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\right)^3 (\pi \varepsilon)^d \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}}  \left[ \frac{V(\kappa \cdot x_i)f(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right] + O(\alpha) \\
    = &\left(\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})}\right)^3 (\pi \varepsilon)^d \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}} O(\varepsilon) + O(\alpha) \\
...
\end{align*}
Finally, to obtain the variance term we multiply the error above by $4/\varepsilon$. More precisely, we have
\begin{align*}
...
\noindent We conclude this section by providing a proof of Lemma \ref{lemma: second moments of F and G}, stated below.
...
\end{split}    
 \end{align}
Hence, we need to find asymptotic expansions for each of the two terms in Equation \eqref{eq: expansion of F_i, kappa in two terms}. Let us denote by $\mathcal{O}(x) = \{ \lambda\cdot x \mid \lambda \in K \}$ the orbit of $x \in \mathcal{M}$ under the action of $K$. This is a smooth manifold of dimension equal to $ \mathrm{dim}(K)$ in $\mathbb{C}^\mathcal{D}$. Using this, we can rewrite the integrals over $K$ in Equation \eqref{eq: expansion of F_i, kappa in two terms} as integrals over $\mathcal{O}(x)$. For the first integral, we obtain
...
\end{split}
\end{align}
where $\mu(x) =$ is associated with the change of integration variable. Next, applying Proposition 9 in \cite{SteerablePaper} allows us to approximate the left-hand-side integral in Equation \eqref{eq: integral exp f} as
...
\end{align}
The last step consists of estimating the expected value of $ (F_{i,\kappa}(x))^2$ using Formula \eqref{eq: approx F squared}. To integrate over the manifold $\mathcal{M}$, we can choose appropriate coordinates that allow us to split the integral into the orbits of the action by $K$ and a complementary-dimensional submanifold $\mathcal{N} \subseteq \mathcal{M}$ of dimension equal to $d -\mathrm{dim}(K)$. The function $V(x) \colonequals \sqrt{|\mathrm{det}(g_\mathcal{M})(x)|}$, where $g_\mathcal{M}$ is the metric on $\mathcal{M}$, comes from the volume form of the manifold $\mathcal{M}$. This and Formula \eqref{eq: approx F squared} yield
\begin{align*}
    \mathbb{E}\left[ (F_{i,\kappa}(x))^2\right] &= \int_\mathcal{M} (F_{i,\kappa}(x))^2 p(x) dx \\
    &= \int_{\mathcal{N}}\int_K (F_{i,\kappa}(\lambda\cdot x))^2 p(\lambda\cdot x) V(x) dx d\lambda  \\
     &= \frac{1}{\mathrm{Vol}(\mathcal{M})} \int_{\mathcal{N}}\int_K (F_{i,\kappa}( x))^2 V(x) dx d\lambda  \\
     &=\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} \int_{\mathcal{N}} (F_{i,\kappa}( x))^2 V(x) dx \\
     &=\frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} (\pi \varepsilon)^ {\mathrm{dim}(K)} \int_{\mathcal{N}} \exp\left(-2|| \kappa \cdot x_i -x ||^2/\varepsilon\right) \frac{V(x)}{\mu^2(x)}[f^2(x) + O(q(x) + \varepsilon)] dx. \numberthis \label{eq: internal over N}
\end{align*}
...
\begin{align}\label{eq: first term int over N}
    \int_{\mathcal{N}} \exp\left(-2|| \kappa \cdot x_i -x ||^2/\varepsilon\right) \frac{V(x) f^2(x)}{\mu^2(x)} dx = {(\pi \varepsilon/2)^{(d - \mathrm{dim}(K))/2}} \left[ \frac{ V(\kappa \cdot x_i)f^2(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right],
\end{align}
and
\begin{align}\label{eq: second term int over N}
    O\left(\int_{\mathcal{N}} \exp\left(-2|| \kappa \cdot x_i -x ||^2/\varepsilon\right) \frac{V(x)q(x) }{\mu^2(x)} dx\right) &= O\left({(\pi \varepsilon)^{(d - \mathrm{dim}(K))/2}}\left[ \frac{ V(\kappa \cdot x_i) q(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + \varepsilon \right] \right)\\
...
\begin{split}
   \mathbb{E}\left[ (F_{i,\kappa}(x))^2\right] 
    &= \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} (\pi \varepsilon)^ {\mathrm{dim}(K)} {(\pi \varepsilon/2)^{(d - \mathrm{dim}(K))/2}} \left[ \frac{ V(\kappa \cdot x_i)f^2(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right] \\
    &= \frac{\mathrm{Vol}(K)}{\mathrm{Vol}(\mathcal{M})} \frac{(\pi \varepsilon)^{(d + \mathrm{dim}(K))/2}}{2^{(d - \mathrm{dim}(K))/2}} \left[ \frac{ V(\kappa \cdot x_i)f^2(\kappa \cdot x_i)}{\mu^2(\kappa \cdot x_i)} + O(\varepsilon) \right], 
\end{split}
\end{align}
...
Our work extends a method of Landa and Shkolnisky from the 2D rotation group to the case of arbitrary compact Lie groups $K$.  
