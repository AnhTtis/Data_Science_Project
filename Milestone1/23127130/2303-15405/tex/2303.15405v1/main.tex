...
\author{Marco Radaelli$^{1\dagger}$, Gabriel T. Landi$^2$ and Felix C. Binder$^1$}
\date{%
    \footnotesize
    $^1$School of Physics, Trinity College Dublin, Dublin 2, Ireland\\%
    $^2$Department of Physics and Astronomy, University of Rochester, Rochester, New York 14627, USA\\[2ex]%
    $^\dagger$radaellm@tcd.ie
}
...
\begin{figure}[h!]
    \includegraphics[width=0.99\columnwidth]{figures/FIG_both_double_qubit.pdf}
...
\begin{algorithmic}
    ...
    \For{all times $t<t_f$}
        \State{$V[t] = \exp(\tilde{\mathcal{L}}_0 t)$}
        \State{$Q_s[t] = \unvectorize(\exp(\tilde{\mathcal{L}}_0^\dagger t)\tilde{J})$}
