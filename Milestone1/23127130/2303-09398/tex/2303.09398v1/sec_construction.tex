...
We start this section by fixing notations. This is crucial for the understanding of construction of new solutions. For $i=1,2$ consider solutions $(X_i,\cdot_i)$ on the set $X_i=\{1,2,\ldots,k_i\}$. Then we have two cycle matrix $M_1$ and $M_2$. For $\sigma_1,\sigma_2,\ldots,\sigma_{k_2}\in\Sym_{k_1}$ and $\mu_1,\mu_2,\ldots,\mu_{k_1}\in\Sym_{k_2}$, define the matrix $\mathbb{M}$ to be
\begin{align*}
...
\end{theorem}
\begin{proof}
Define a binary operation $\star$ on $X=X_1\cup X_2$ in the following way;
\begin{align*}
...
\end{corollary}
\begin{proof}
    If $H=\langle \sigma_1,\sigma_2,\cdots,\sigma_r\rangle$ is an finite abelian group. Then take $X_1$ is of size $1$, $X_2$ of size $r$ and $^i\alpha_2=\sigma_i$.
\end{proof}
...
\end{proposition}
\begin{proof}
    Define the following binary operation `$\star$' on $X=X_1\cup X_2$ in the following way:
    \begin{align*}
...
\textbf{Case 4: ($y,~z\in X_2$)}
	\begin{equation*}
		...
		\end{split}
	\end{equation*}
 Similarly, we can prove the other equalities. Therefore $(X,\star)$ is a cycle set. And the matrix of the cycle set with respect to the order set $X=\{1,2,\cdots,k_1,k_1+1,\cdots,k_1+k_2\}$ is $\mathbb{M}_{(X_i),(\alpha_i),(12)}.$
 \end{proof}
...
\end{proposition}
\begin{proof}
	Define $\star$ on $X=X_1\cup X_2\cup X_3$ as follows
 \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \end{array}\right],
    \end{align*}
    is a cycle matrix for $\Delta_{n+1}\left\{id_{X_{n+1}},\alpha_{n+1}\right\}$, $\Delta^{\prime}_{1}\left\{id_{X_1},\alpha_{n+1}\right\}$ (using \cref{prop:2-times-2}). Hence it follows that $\mathbb{M}_{(X_i),(\alpha_i),\Theta,[~]}$ is a cycle matrix.
\end{proof}