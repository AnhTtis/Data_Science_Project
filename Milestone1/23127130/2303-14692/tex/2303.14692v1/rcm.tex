...
\end{align}
where $f: \Re^{n} \to \Re$ and $c: \Re^{m} \to \Re^{n} \; (m \le n)$.
...
It is not difficult to verify $P^{2}(x) = P(x)$. That is to say, the projection matrix
...
\begin{align}
     \|P(x)\| = 1. \label{MATNP}
\end{align}
We denote $P(x)^{+}$ as the Moore-Penrose generalized inverse of the projection
...
\begin{align}
      P(x)^{+} = P(x).     \label{GINVP}
\end{align}
Actually, from equation \eqref{GINVP}, we have $P(x)(P(x))^{+}P(x) = P(x)$,
$P(x)^{+}P(x)P(x)^{+} = P(x)^{+}$, $\left(P(x)^{+}P(x)\right)^{T} = P(x)^{+}P(x)$
and $\left(P(x)P(x)^{+}\right)^{T} = P(x)P(x)^{+}$.
...
\begin{remark}
From $P^{2}(x) = P(x)$, we know that the solution $x(t)$ of the ODE
...
we may not have $c\left(x_{k+1}^{p}\right) = 0$ when $c(x_{k}) = 0$, where $x_{k+1}^{p}$
is the solution of equations \eqref{TRCM}-\eqref{XK1P}. Therefore, we
need to compute the correction step $s_{k}$ such that $x_{k+1} = x_{k} + s_{k}$
is pulled back to the constraint $c(x) = 0$. In order to save the computational
time, we use the following generalized Newton iteration with the shortest
increment \cite{LX2021} to achieve this aim:
\begin{align}
    & s_{k}^{c} = -(A_{k+1}^{p})^{+} c(x_{k+1}^{p}), \;
    (A_{k+1}^{p})^{+} = (A_{k+1}^{p})^{T}\left(A_{k+1}^{p}(A_{k+1}^{p})^{T}\right)^{-1},
    \nonumber \\
    & s_{k} = s_{k}^{p} + s_{k}^{c}, \; x_{k+1} = x_{k+1}^{p} + s_{k}^{c} = x_{k} + s_{k},
      \label{GNEWTONM}
\end{align}
...
such as $\epsilon_{0} = 10^{-7}$ and $\epsilon = 10^{-6}$.
...
the correction step $s_{k}^{c}$ need to satisfy the following conditions:
\begin{align}
...
We enlarge or reduce the time step $\Delta t_{k+1}$ at every iteration according
to the following ratio:
\begin{align}
    \rho_k = \frac{f(x_k)-f(x_{k}+s_{k})}{q_k(0)-q_k(s_{k})}.
    \label{MRHOK}
\end{align}
...
\begin{align}
     \Delta t_{k+1} =
        \begin{cases}
          \gamma_1 \Delta t_k, &{\text{if} \; \left(\|c(x_{k}+s_k)\|_{\infty} \le \epsilon_{0} \;
                 \text{and} \; \rho_k  \ge \eta_2 \right),}\\
          \Delta t_k, &{\text{else if} \; \left(\|c(x_{k}+s_k)\|_{\infty} \le \epsilon_{0} \;
          \    \text{and} \; \eta_1 < \rho_k  < \eta_2 \right),}\\
          \gamma_2 \Delta t_k, &{\text{others},}
        \end{cases} \label{ADTK1}
\end{align}
where the constants are selected as
$\eta_1 = 0.25, \; \gamma_1 = 2, \; \eta_2 = 0.75, \; \gamma_2 = 0.5$ and
$\epsilon < 10^{-6}$  according to our numerical experiments. We accept the trial
step $s_{k}$ and let $x_{k+1} = x_{k}+s_{k}$, when $\rho_{k} \ge \eta_{a}$,
$\|c(x_{k}+s_{k})\|_{\infty} \le \epsilon_{0}$ and the approximation model
$q_{k}(0) - q_{k}(s_{k})$ satisfies the Armijo sufficient descent condition:
\begin{align}
     q_{k}(0) - q_{k}(s_{k}) \ge \eta_{q} \|s_{k}^{p}\| \|p_{g_k}\|,
     \label{AMGESKPGK}
\end{align}
where $p_{g_k} = P(x_k)g(x_{k}) = P(x_k)\nabla f(x_k)$ and $s_{k}^{p}$ are
computed by equation \eqref{TRCM}-\eqref{XK1P}, and  $\eta_{a}, \; \eta_{q}$ are the
small positive constants such as $\eta_{a} = \eta_{q} = 10^{-6}$.
Otherwise, we discard it and let $x_{k+1} = x_{k}$.
...
times at every iteration. Since the linear model $f(x_{k}) + \nabla f(x_{k})^{T}s_{k}$ may
...
of $(L^{-1}S)(V^{T}L^{-T})$ equal that of $(V^{T}L^{-T})(L^{-1}S)$ except for
...
\begin{align}
    \frac{dz(\tau)}{d \tau} = - A(z)^{+}c(z),  \; z(\tau_{0}) = z_{0},
    \label{GNFLOW}
\end{align}
where $A(z)^{+}$ is the Moore-Penrose generalized inverse of the Jacobian
...
\begin{align}
    ...
    {1 + \Delta \tau_{j}} A_{j}^{+}c(z_j), \;  z_{j+1} = z_{j} + \Delta z_{j},
    \label{GCNM}
\end{align}
	...
	\begin{algorithmic}[1]
        ...
        \WHILE{$\left(\left(\|p_{g_k}\|_{\infty}> \epsilon\right) \; \text{and} \;
         (\text{itc} < \text{max\_itc})\right)$}
          ...
          \IF{($\rho_k \ge \eta_{a}, \; \|c_{k+1}\|_{\infty} \le \epsilon$
...
\begin{align}
    ...
    \|A(x)\| \le M_{A}, \; \|A(x)^{+}\| \le M_{A}, \forall x \in \mathrm{S},  \nonumber \\
...
$\mu\left(P_{k}\nabla^{2}f(x_{k})P_{k}\right)$ as the eigenvalue of $P_{k}\nabla^{2}f(x_k)P_{k}$, we
know that the eigenvalue of $\left(\frac{\sigma_{0}}{\Delta t_{k}}I + P_{k}\nabla^{2}f(x_k)P_{k}\right)$
is $\frac{\sigma_{0}}{\Delta t_{k}} + \mu\left(P_{k}\nabla^{2}(f_{k})P_{k}\right)$. Consequently, from
equation \eqref{BOUNDPHESS}, we known that
\begin{align}
   \frac{\sigma_{0}}{\Delta t_{k}}I + P_{k}\nabla^{2}f(x_{k})P_{k} \succ 0, \;   \text{when} \;
   \Delta t_{k} < \frac{\sigma_{0}}{M_{B}}.    \label{SPDCON}
\end{align}
...
\end{align}
Furthermore, we suppose $s_{k}$ is solved by equation \eqref{TRCM} and
...
assumption $\|A(x)^{+}\| \le M_{A}$ into equation \eqref{TEXPCXK1}, we obtain
\begin{align}
    & \|c(x_{k}+s_{k})\| \le
    \int_{0}^{1} \|A(x_{k+1}^{p}+ ts_{k}^{c}) - A(x_{k+1}^{p})\| \|s_{k}^{c}\|dt
    ...
    \nonumber \\
    & \hskip 2mm
    = \frac{1}{2} L_{A}  \left\|A(x_{k+1}^{p})^{+}c(x_{k+1}^{p})\right\|^{2}
    \le \frac{1}{2} L_{A}  \left\|A(x_{k+1}^{p})^{+}\right\|^{2}
...
In order to prove that $p_{g_k}$ converges to zero when $k$ tends to infinity,
we need to estimate the lower bound of time step sizes
$\Delta t_{k} \, (k = 1, \, 2, \, \ldots)$. By using Lemma \ref{LEMBSOAM} and Lemma
\ref{LEMCONC}, we can obtain the lower bound of time step sizes
...
implementation codes of the SQP method. One is the built-in subroutine fmincon.m of
the MATLAB2020a environment \cite{MATLAB,Schittkowski1986}. The other is the
subroutine SNOPT \cite{GMS2005,GMS2006} executed in GAMS v28.2 (2019) environment \cite{GAMS}).
fmincon and SNOPT are two robust and efficient solvers for nonlinear
equality-constrained optimization problems. Therefore, we select these two typical
solvers as the basis for comparison.


\vskip 2mm

We select $40$ nonlinear equality-constrained optimization problems from CUTEst
\cite{GOT2015} and construct $60$ test problems. Therefore, we use these $100$
...
\ref{TABCOMRSA1}-\ref{TABCOMRSA5}. Numerical results of 40 constrained
...
From Tables \ref{TABCOMRSA1}-\ref{TABCOMRSA5}, we find that Rcm can solve most
of test problems and only one problem can not be solved. However, fmincon fails
to solve CUTEst problems about 22/100 and SNOPT fails to solve problems about 18/100.
From Tables \ref{TABCOMRSA1}-\ref{TABCOMRSA5} and Figure \ref{FIGCPURCM}, we
find that the computational time of Rcm is significantly less than that of fmincon
...
\\ \hline
  ...
  & \tabincell{c}{0+0 \\ (0.00)} &\tabincell{c}{0.00e+0 \\ (0)} \\ \hline
...
\begin{table}[!http]
  ...
  & \tabincell{c}{0+0 \\ (0.00)} &\tabincell{c}{0\\ (0)} \\ \hline
  ...
  & \tabincell{c}{3+3 \\ (0.00)} &\tabincell{c}{2.27e-13 \\ ()} \\ \hline
...
\begin{table}[!http]
  ...
  \tabincell{c}{41. Trid Function \cite{SB2013} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{42. Grewank Function \cite{SB2013} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{43. Dixon Price Function \cite{SB2013} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{44. Rosenbrock Function \cite{MGH1981} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{45. Trigonometric Function  \cite{MGH1981} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{46. Singular Broyden Function \cite{Luksan1994} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{47. Extended Powell Singular Function \cite{MGH1981} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{48. Tridiagonal System Function \cite{Luksan1994} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{49. Discrete Boundary-Value Function \cite{Luksan1994} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{50. Broyden Tridiagonal Function \cite{Luksan1994} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{51. Extended Wood Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{52. Extended Cliff Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{53. Extended Hiebert Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{54. Extended Maratos Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{55. Extended Psc1 Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{56. Extended Quadratic Penalty QP 1 \\ Function \cite{Andrei2008}  (n = 2000, m = 10)}
  ...
  \tabincell{c}{57. Extended Quadratic Penalty QP 2  \\ Function \cite{Andrei2008} (n = 2000, m = 10)}
  ...
  \tabincell{c}{58. Extended TET Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{59. EG2 Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
  ...
  \tabincell{c}{60. Extended BD1 Function \cite{Andrei2008} \\ (n = 2000, m = 10)}
...
\begin{table}[!http]
  ...
  \tabincell{c}{61. Trid Function \cite{SB2013} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{62. Grewank Function \cite{SB2013} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{63. Dixon Price Function \cite{SB2013} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{64. Rosenbrock Function \cite{MGH1981} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{65. Trigonometric Function  \cite{MGH1981} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{66. Singular Broyden Function \cite{Luksan1994} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{67. Extended Powell Singular Function \cite{MGH1981} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{68. Tridiagonal System Function \cite{Luksan1994} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{69. Discrete Boundary-Value Function \cite{Luksan1994} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{70. Broyden Tridiagonal Function \cite{Luksan1994} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{71. Extended Wood Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  & \tabincell{c}{11+6 \\ (6.58)} &\tabincell{c}{1.29e-06  \\ (0)}
  & \tabincell{c}{58 \\ (38.54)} &\tabincell{c}{6.27e-08 \\ (3.48e-09)}
  & \tabincell{c}{13+1013 \\ (4.77)} &\tabincell{c}{1.40e-06 \\ (8.53e-05)} \\ \hline

  \tabincell{c}{72. Extended Cliff Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{73. Extended Hiebert Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{74. Extended Maratos Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{75. Extended Psc1 Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{76. Extended Quadratic Penalty QP 1 \\ Function \cite{Andrei2008}  (n = 2000, m = 1000)}
  ...
  \tabincell{c}{77. Extended Quadratic Penalty QP 2  \\ Function \cite{Andrei2008} (n = 2000, m = 1000)}
  ...
  \tabincell{c}{78. Extended TET Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{79. EG2 Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
  ...
  \tabincell{c}{80. Extended BD1 Function \cite{Andrei2008} \\ (n = 2000, m = 1000)}
...
\begin{table}[!http]
  ...
  \tabincell{c}{81. Trid Function \cite{SB2013} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{82. Grewank Function \cite{SB2013} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{83. Dixon Price Function \cite{SB2013} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{84. Rosenbrock Function \cite{MGH1981} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{85. Trigonometric Function  \cite{MGH1981} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{86. Singular Broyden Function \cite{Luksan1994} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{87. Extended Powell Singular Function \cite{MGH1981} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{88. Tridiagonal System Function \cite{Luksan1994} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{89. Discrete Boundary-Value Function \cite{Luksan1994} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{90. Broyden Tridiagonal Function \cite{Luksan1994} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{91. Extended Wood Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{92. Extended Cliff Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{93. Extended Hiebert Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{94. Extended Maratos Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{95. Extended Psc1 Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{96. Extended Quadratic Penalty QP 1 \\ Function \cite{Andrei2008}  (n = 2000, m = 1999)}
  ...
  \tabincell{c}{97. Extended Quadratic Penalty QP 2  \\ Function \cite{Andrei2008} (n = 2000, m = 1999)}
  ...
  \tabincell{c}{98. Extended TET Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{99. EG2 Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
  ...
  \tabincell{c}{100. Extended BD1 Function \cite{Andrei2008} \\ (n = 2000, m = 1999)}
...
\section*{Acknowledgments} This work was supported in part by Grant 61876199 from National
Natural Science Foundation of China, Grant YBWL2011085 from Huawei Technologies
Co., Ltd., and Grant YJCB2011003HI from the Innovation Research Program of Huawei
Technologies Co., Ltd.

%The authors are grateful to the anonymous referee for
%his comments and suggestions which greatly improve presentation of this paper.
