...
For any instance of the 2-state model,
...
Fix a patient $i$, and let $M_i = \frac{\tau_i (1-p_i-q_i)}{(p_i + q_i)(1-p_i)} > 0$.
...
\begin{enumerate}[label=(\roman*)]
	...
	Let $I_t' = I_t \cup \{i : Q_{it} = 1, i \in I_t(1)\}$ be the new set of patients in state 0.
...
Since $S_{it}^{\NULL} = 1$, $Q_{is} = 0$ for every $s \in \{t'+1, \dots, t-1\}$.
Since $P_{it'} = 1$, \cref{prop:coupling_property0} implies $S^{\ALG}_{i, t'+1} = 1$.
Since $Q_{is} = 0$ for every $s \in \{t'+1, \dots, t-1\}$, $S_{it}^{\ALG} = 1$.
\end{myproof}

Lastly, using this sample path coupling, we can write an expression for the intervention values $z^{\lambda}_{it}(0)$ and $\nz_{it}(0)$.
...
z^{\lambda}_{it}(0) = \tau_i \cdot \bE[\min\{\text{Geometric}(p_i + q_i + \gamma \tau_i (1-p_i-q_i)/(1-p_i)), T-t+1\}].
\end{align}
Moreover, the null intervention value can be written as
\begin{align} \label{eq:nzformula}
\nz_{it}(0) = \tau_i \cdot \bE[\min\{\text{Geometric}(p_i + q_i, T-t+1\}].
\end{align}
\end{lemma}


\begin{myproof}[Proof of \cref{lemma:zgamma}]
Recall that $z^{\gamma}_{it}(0) = q_{it}^{\RAND(\gamma)}(0, 1) - q_{it}^{\RAND(\gamma)}(0, 0)$.
...
Conditioned on either $E_0$ or $E_1$, the distribution of the variables $W_{it'}$ for $t' > t$ and  $P_{it'}, Q_{it'}, K_{it'}$ for $t' \geq t$ are the same, due to independence.
...
Let $\tau = \min\{T+1, \min\{t' > t: S^0_{it'} = S^1_{it'}\}\}$ be the first time after time $t$ that the two states converge, where $\tau = T+1$ if they never converge. 
The states will always be the same after time $\tau$ due to the sample path coupling.
Then, $z^{\gamma}_{it}(0) = \bE[\tau] - t - 1$.

We now write an expression for $z^{\gamma}_{it}(0)$.
For the states to differ at time $t+1$, it must be that $P_{it} = 0$ and $K_{it} = 1$.
After that, the states will converge at time $t'+1$ if (i) $P_{it'} = 1$,  (ii) $Q_{it'} = 1$, or (iii) $W_{it'} = 1$ and $K_{it'} = 1$.
Let $\Gamma = \min\{t' > t : P_{it'} = 1 \} - t$ be the length of time from $t$ until $P_{it'} = 1$.
Let $\Gamma' = \min\{t' > t : Q_{it'} = 1 \} - t$ be length of time from $t$ until $Q_{it'} = 1$.
...
\end{align*}
$\min\{\Gamma, \Gamma', \Gamma''\}$ is a geometric random variable with parameter 
\begin{align*}
&1-(1-\Pr(P_{it} = 1))(1-\Pr(Q_{it} = 1))(1-\Pr(W_{it} = 1, K_{it} = 1)) \\
=& 1-(1-p)(1-q/(1-p))(1-\gamma \tau / (1-p)) \\
=& p + q + \gamma \tau (1-p-q)/(1-p)
\end{align*}
Therefore,
\begin{align*} 
z^{\gamma}_{it}(0) = \tau \cdot \bE[\min\{\text{Geometric}(p + q + \gamma \tau (1-p-q)/(1-p)), T-t+1\}].
\end{align*}
...
$\bE\big[Z_{it} \;|\; S^{\NULL}_{it} = 0 \big] = \tau_i \cdot \bE[\min\{\text{Geometric}(p_i + q_i, T-t+1\}] = \nz_{it}(0)$.
\end{lemma}
...
Let $\Gamma'_{it} = \min\{t' > t : Q_{it'} = 1 \} - t$ be length of time from $t$ until $Q_{it'} = 1$.
Then, 
...
Note that $\min\{\Gamma_{it}, \Gamma'_{it}\}$ is a geometric random variable with parameter $1-(1-\Pr(P_{it} = 1))(1-\Pr(Q_{it} = 1)) = p_i+q_i$.
Therefore,
\begin{align*}
\bE[Z_{it} \;|\; S_{it}^\NULL = 0] 
&= \Pr(P_{it} = 0) \cdot \Pr(K_{it} = 1) \; \bE[\min\{\text{Geometric}(p_i+q_i), T-t+1\}] \\
&= \tau_i \cdot \bE[\min\{\text{Geometric}(p_i+q_i), T-t+1\}].
\end{align*}
...
Fix a patient $i$ and time $t$. In this proof, we remove the subscript $i$ on $p$, $q$ and $\tau$ for convenience.
\cref{lemma:zgamma} gives us an expression for both $z^{\lambda}_{it}(0)$ and $\nz_{it}(0)$:
\begin{align} 
z^{\lambda}_{it}(0) &= \tau \cdot \bE[\min\{\text{Geometric}(p + q + \gamma \tau (1-p-q)/(1-p)), T-t+1\}] \label{eq:z1}\\
\nz_{it}(0) &= \tau \cdot \bE[\min\{\text{Geometric}(p + q, T-t+1\}]. \label{eq:z2}
\end{align}
Since $p+q < 1$, $p + q + \gamma \tau (1-p-q)/(1-p)) > p+q$, hence $z^{\pi}_{it}(0)/\nz_{it}(0) \leq 1$.
Next, to lower bound the ratio $z^{\pi}_{it}(0)/\nz_{it}(0)$, we use the following lemma:
\begin{lemma} \label{lemma:trunc_geometric}
Suppose $X = \text{Geometric}(P)$, $Y = \text{Geometric}(Q)$ with $P > Q$, and let $T > 0$ be a positive integer.
Then, 
\begin{align*}
...
That is, we have
\begin{align*}
\frac{\bE[\text{Geometric}(p + q + \gamma \tau(1-p-q)/(1-p) )]}{\bE[\text{Geometric}(p + q)]}
&= \frac{1}{1 + \gamma \frac{\tau (1-p-q)}{(p+q)(1-p)}}.
\end{align*}
Using \cref{lemma:trunc_geometric} implies
\begin{align*}
z^{\pi}_{it}/\nz_{it} 
&\geq \frac{1}{1 + \gamma \frac{\tau (1-p-q)}{(p+q)(1-p)}}.
\end{align*}

\begin{myproof}[Proof of \cref{lemma:trunc_geometric}]
Let $X = \text{Geometric}(P)$, $Y = \text{Geometric}(Q)$ with $P > Q$.
...
\begin{align*}
	...
	&= \frac{\frac{1}{P} -  \frac{  (1-P)^T }{P}}{\frac{1}{Q} -  \frac{  (1-Q)^T }{Q}} \\
	&= \frac{\frac{1}{P}(1-(1-P)^T)}{ \frac{1}{Q}(1-(1-Q)^T)} \\
	&\geq \frac{1/P}{1/Q} \\
	&=\frac{\bE[X]}{ \bE[Y] },
\end{align*}
where the inequality follows since $P > Q$.
\end{myproof}
...
\begin{align}
\frac{1}{1 + \gamma M_i} \beta_1 \nz_{it}(0) \leq z^{\ALG}_{it}(0)\leq \beta_2 \nz_{it}(0).
\end{align}
Then, we apply \cref{prop:approx_index} with $\alpha_1 = \frac{1}{1 + \gamma M_i} \beta_1$ and $\alpha_2 = \beta_2$, and the result follows.
