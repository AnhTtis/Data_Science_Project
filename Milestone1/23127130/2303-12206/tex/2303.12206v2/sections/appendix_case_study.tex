...
In our experiments, we used $\sigma^2 = 1/4$.
...
We define a patient's state at a point in time to be a 3-tuple $(u_1, u_2, u_3)$, where $u_1$ represents the number of times the patient verified in the last week, $u_2$ is the patient's historical total verification rate, and $u_3$ is the number of times that the patient received an intervention in the last week. The values of each of these terms are bucketed into a small number of bins (3 bins for $u_1$ and $u_3$, 5 bins for $u_2$), resulting in 45 states in total. 
Specifically, the bins for both $u_1$ and $u_2$ are $0, 1$ and $2-7$.
For $u_2$, the bins are $0-1\%$, $1-5\%$, $5-20\%$, $20-45\%$, and $45-100\%$.
The bin values were chosen to balance the number of samples in each bin.
Let $\cU$ be the set of all states.
...
We define $Q(u, a; \lambda)$ as the Q-value from state $u$ and action $a$ under this process.
Then, the $Q$-values satisfy:
\begin{align}
	Q(u, a; \lambda) = r(u, a) + (1-a) \lambda - \beta + \bE_{u' \sim P(u, \cdot, a)}[\max_{a'} Q(u', a'; \lambda)],
\end{align}
where $\beta \in \bR$ is the optimal reward.
Then, the Whittle's index for state $u$ is the value $\lambda(u)$ that satisfies
\begin{align}
	Q(u, 0; \lambda(u)) = Q(u, 1; \lambda(u)).
\end{align}


Recall that we have offline samples $\{(u_i, a_i, v_i, \ell_i, u_i')\}$, where $v_i \in \{0, 1\}$ is the outcome of whether the patient verified, $\ell_i \in \bN$ is the number of days the patient has left, and $u_i'$ is the next state of the patient.
Fix a number $\lambda$, and we will aim to learn $Q(u, a; \lambda)$, which represents the future verification rate.
Initialize $Q_0(\cdot, \cdot; \lambda) = 0$.
Let $\cI(u, a) = \{i : u_i = u, a_i = a\}$ be the data points with state $u$ and action $a$.
For $t = 1, 2, \dots$, update the $Q$-values as:
\begin{align} \label{eq:q_iteration}
Q_t(u, a; \lambda) = \frac{1}{|\cI(u, a)|} \sum_{i \in \cI(u, a)} \frac{1}{\ell_i} \big(v_i  + (1-a_i) \lambda + (\ell_i-1) \max_{a'}Q_{t-1}(u_i', a')\big).
\end{align}	
Then, for every state $u$, to find the $\lambda(u)$ such that $Q(u, 0; \lambda(u)) = Q(u, 1; \lambda(u))$, we did a grid search across $\lambda$. For each $\lambda$, we ran the above method \eqref{eq:q_iteration} over 100 iterations, and found the index that gave the smallest absolute difference between $Q_{100}(u, 0; \lambda(u))$ and $Q_{100}(u, 1; \lambda(u))$.
