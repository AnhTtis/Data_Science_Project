...
\]
where the minimum is taken over all subsets $S\subseteq \{0,1\}^n$, such that $x\in S$, $U(p)=S$, $l(p)\le k$. This definition was introduced by Kolmogorov in a talk at the Information Theory Symposium, Tallin[n], Estonia, in 1974. Thus $H_k(x)$ is the log of the size of the smallest set containing $x$ over all sets specifiable by a program of $k$ or fewer bits. Of special interest is the value
\[
...
Anyway, a natural question arises. Kolmogorov suggested two criteria saying when finite set of strings $S$ is a good model (statistical hypothesis) for a string $x$:
\begin{itemize}
...
Since then the randomness theory for infinite sequences developed a lot, mostly due to recursion theorists who came to this field and found many surprising connections between general recursion theory and algorithmic information theory (e.g., a very interesting notion of K-trivial Turing degrees). A lot of nice (and often difficult) results of this type can be found in~\cite{2010downeyhirschfeldt,2012nies}. However, they are interesting mostly from the mathematical point of view and rarely give something to the philosophical foundations of probability theory. (However, one should note that Schnorr's ideas about martingale randomness and algorithmic information theory in general were one of the sources of inspiration of the game approach to probability theory~\cite{2001vovkshafer}.
}%\endnote
