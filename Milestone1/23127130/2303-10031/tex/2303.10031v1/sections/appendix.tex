...
In model-based dissipativity analysis, \TR{definition and testing of} %the classical way of defining 
classical dissipativity, see \cite{Willems1972, HillMoylan1980}, is more straight-forward. 
\TR{In this section, we give a model-based equivalent of Theorem~\ref{thm:LPV_diss} in terms of an $L$-dissipativity test}.

\TR{Note that LPV systems represented by \eqref{eq:sys} have a direct minimal state-space realization in the form of}
\begin{align*}
    x_{k+1} & = A(p_k) x_k + B(p_k)u_k, \\
    y_k & = C(p_k) x_k + D(p_k)u_k.
\end{align*}
\TR{as discussed in \cite{Toth2011_SSrealizationTCST}. This form is easily computable in the SISO case, while computation in the \emph{multiple-input multiple-output} (MIMO) case requires the choice of algebraically independent state basis which can be accomplished via \cite{Toth15CDCb}.} 
...
\end{equation}
Hence, model-based verification of $L$-dissipativity  \TR{corresponds to} %results to verifying that 
...
\end{equation}
for all scheduling trajectories in $\ms{P}_{[1,L]}$. Based on the form of $\ms{P}_{[1,L]}$, e.g., polytopic or quadratic, one can select an existing method to verify \eqref{eq:LMImodelbased} in a computationally attractive manner. %\todo{\TRcom{Is this straightforward?} \chris{yes!}}