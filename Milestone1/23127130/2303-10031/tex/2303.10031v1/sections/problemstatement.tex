As mentioned in Section~\ref{sec:introduction}, an attractive approach to address identification and potentially control design \TR{in the LPV framework is to represent the system}
%for a nonlinear system is to represent it 
with a \emph{shifted-affine} LPV-IO representation \cite{Toth2011_SSrealizationTCST}.
Hence, we consider that the underlying system is described by the discrete-time LPV-IO representation \vspace{-3mm}
%
...
a_i(p_{k-i})=\sum_{j=0}^{\dnp}a_i^{[j]}p_{k-i}^{[j]}, \quad b_i(p_{k-i})=\sum_{j=0}^{\dnp}b_i^{[j]}p_{k-i}^{[j]},
\end{align}
\end{subequations}
%
where $p_k^{[0]}=1$ for all $k$. It is assumed that $q^{n_\mathrm{r}}(1+\sum_{i=1}^{\dna}a_i(p_{k-i})q^{-i})$ and $q^{n_\mathrm{r}}\sum_{i=0}^{\dnb}b_i(p_{k-i})q^{-i}$  are coprime \TR{and compose a full row rank matrix polynomial}, implying that the LPV-IO representation \eqref{eq:sys} is minimal and structurally controllable and observable \cite{Toth2010}. 
...
Similarly, we can define the projected IO behavior $\mf{B}_{\mr{IO}}$, as the set of admissible IO trajectories, i.e., $\mf{B}_{\mr{IO}}=\pi_{u,y}\mf{B}$, \TR{where $\pi_{u,y}$ stands for  $\proj_{(\R^{\dnu}\times \R^{\dny})^\N} (\mf{B})$}. Also note that $\pi_{p}\mf{B}=\ms{P}$. Moreover, we denote \TR{by} $\dnx$ the minimal state dimension among all possible LPV state-space realizations that can represent $\mf{B}$, which we will refer to as the \emph{system order}. \TR{The} \emph{lag} associated with $\mf{B}$ \TR{is the minimal order $n_\mathrm{r}=\max(\dna,\dnb)$ required to represent $\mf{B}$ in terms of \eqref{eq:lpv_shi_aff}.}
\chris{Finally, we introduce $\mf{B}_p$, which denotes the set that contains all IO trajectories compatible with some scheduling signal $p\in\ms{P}$,}
%
\begin{equation}\label{eq:behav:p}
    \mf{B}_p=\{(u,y)\in\left(\R^{\dnu}\times \R^{\dny}\right)^\N \mid  (u,p,y)\in\mf{B} \}.
\end{equation}

Analyzing the \emph{behavior} allows us to study the system trajectories independent of the underlying representation. Therefore, the behavioral framework is a helpful tool in assessing stability and performance properties in the analysis and control of systems. As highlighted in Section~\ref{sec:introduction}, an \TR{attractive approach} for simultaneously evaluating \emph{stability} and \emph{performance} is the concept of dissipativity, which is defined in \cite{HillMoylan1980} from a IO perspective in discrete-time as follows.
\begin{definition}[Dissipativity,  \cite{HillMoylan1980}]\label{def:dissipativity-hillmoylan}
    A system with input $u:\N\to\R^{\dnu}$, output $y:\N\to\R^{\dny}$ and behavior $\mf{B}$ is dissipative with respect to the supply rate $\Pi\in\mb{R}^{(\dnu+\dny)\times(\dnu+\dny)}$ if %we have
...
There are multiple \TR{model-based} techniques to verify stability and performance via \eqref{eq:dissipationinequality} for LTI \cite{Willems1972, HillMoylan1980}, LPV~{\cite{Wu1995, KoelewijnToth2021}~and nonlinear \cite{Schaft2017_L2book, VerhoekKoelewijnToth2021_incremental}} systems. % using mathematical models of the system. 
%We have 
\TR{However, when we have} only measured data available from %a data generating 
\TR{the system,} dissipativity analysis via Definition~\ref{def:dissipativity-hillmoylan} becomes a {difficult} problem, as we can only consider \TR{a few} \emph{finite-time} trajectories. {For this reason, we introduce a finite-time notion of dissipativity, $L$-dissipativity, which allows for analysis with system trajectories of finite length.} Hence, given a signal $w\in\left(\R^{\dnw}\right)^\N$ and a compact set $\mb{T}\subseteq\N$, then $w_\mb{T}$ corresponds to the truncation of $w$ to the time interval $\mb{T}$. Similarly, the restricted behavioral set $\Bfint{}{\mb{T}}$  contains the truncation of all the trajectories in $\mf{B}$ to the time interval $\mb{T}$. In this paper, we consider the following definition of $L$-dissipativity, originally proposed in \cite{maupong2017lyapunov}.
%
...
\subsubsection*{Problem statement} Consider a data-generating system that can be represented with \eqref{eq:sys}. Given a length-$N$ data set $\dataset:= \{u_k,p_k,y_k\}_{k=1}^N$ (data-dictionary) sampled from the data-generating system, i.e., $(u_{[1,N]},p_{[1,N]},y_{[1,N]})\in\Bfint{}{[1,N]}$. For an $1\leq L<N$, how \TR{can we} determine that \eqref{eq:sys} is $L$-dissipative w.r.t. the supply rate \eqref{eq:supplyrate} \TR{using only $\dataset$}?


\TR{If we can solve this problem, then} we can analyze stability and performance properties of  \TR{LPV} systems in a data-driven setting, \emph{without} the need of knowing their \TR{dynamic} model \eqref{eq:sys}. 
