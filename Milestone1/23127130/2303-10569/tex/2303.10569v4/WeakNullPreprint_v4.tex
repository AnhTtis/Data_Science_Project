...
\end{equation}
In fact, solutions to the Cauchy problem for \eqref{eq:wave:F} with  sources satisfying \eqref{eq:F:slow} display homogeneous asymptotics in the interior of the light cone,
\begin{equation}
...
and we will derive the compatibility conditions imposed by the source on the prescription of the radiation field at null infinty.
...
\begin{equation}\label{eq:asymptotics:log:div}
    \Phi[n_\omega](t,r\omega)\sim \frac{1}{r}\ln\Bigl(\frac{2r}{\tminusr}\Bigr) \mathcal{F}_{01}(r-t,\omega)\,,\qquad \mathcal{F}_{01}(r-t,\omega)= \int_{r-t}^\infty n(q,\omega)\ud q\,, \qquad (r\sim t,r\to\infty)\,.
\end{equation}
...
\begin{equation}\label{eq:interior:leading:hom}
  \Phi[n](t,r\omega) \sim \frac{1}{4\pi }\int_{\mathbb{S}^2}\frac{N(\sigma)}{t-r\langle\omega,\sigma\rangle}\ud S(\sigma),\quad (r/t<1,t\to\infty)\,,\qquad \text{ where}\quad N(\sigma)= \int_{-\infty}^\infty\!\!\! n(q,\sigma)\ud q\,.
\end{equation}
...
\begin{equation}
  ...
  \mathcal{F}_{01}(q,\omega)= \int_{q}^\infty n(q,\omega)\ud q \longrightarrow N(\omega), \qquad (q\to-\infty)\,.
\end{equation}
...
\begin{theorem}[Scattering solutions with quadratic source terms] \label{thm:n}
 ...
 \end{equation}
 Here $N_0$, $M_0$, and $\mathcal{P}$ are functions on the sphere determined from the source function $n$ alone, see Section~\ref{sec:integrabilitycondition}.
...
\end{equation}
where $\psi_{asym}$ is an approximate solution, and constructed, guided by the forward solution, by interpolating between a second order expansion $\psi_{rad}$, given in \eqref{eq:secondorderapproximatesource2},  in place of \eqref{eq:139}, in the wave zone, and a homogeneous solution $\psi_{hom}$ in the interior:
\begin{equation}
  %\label{eq:143}
...
The scattering solutions discussed in Section~\ref{sec:scattering:intro}, while all  endowed with homogeneous asymptotics in the interior, decay fast in the exterior of the light cone, and in particular we made the choice
\begin{equation}
...
\eq
For $\mathcal{F}_0$ to be well defined we need $g(y)$ and $\nabla f(y)$ to decay like $\langle y\rangle^{-2-\kappa}$ for $\kappa\!>\!0$.  Then it follows in that case that $\mathcal{F}_0(q,\omega)\to 0$ as $q\!\to\!\infty$.
However, this excludes data that are relevant in many important physical situations that involve the description of mass, charge and angular moment, which correspond to terms that decay like $r^{-1}$ or $r^{-2}$ and are therefore dealt with separately by  subtracting off the these terms from initial data.
...
\frac{\mathcal{F}_{\!01\!}(r\!-\!t,\omega) \! \! }{r}+\frac{\!\mathcal{F}_{\!0}(r\!-\!t,\omega)\!\!}{r} \qquad (r\sim t,r\to\infty)
  \end{equation}
   where $\mathcal{F}_{01}(q,\omega)=N_{01}(\omega)$ is a function of $\omega$ only, and the radiation field $\mathcal{F}_0$ given by
  \begin{equation}
...
\begin{remark}
  In \cite{LS20} we have shown that for any smooth function $\mathcal{F}_0$ satisfying $|(\langle q\rangle\partial_q)^k\partial_\omega^\alpha \mathcal{F}_0|\les \langle q\rangle^{-\gamma}$, $\gamma>1/2$ there exists a solution $\phi$ to \eqref{eq:wave:intro} with the asymptotics $\phi\sim\mathcal{F}_0(r-t,\omega)/r$  in the wave zone.
  This does not cover the case of slowly decaying data. Indeed,  initial data that decays like $r^{-1}$ corresponds to $\gamma=0$.
  Theorem~\ref{thm:ext:scattering}  covers all cases related to slowly  decaying data and gives a complete characterisation of the asymptotics of the solution towards time-like and space-like infinity. 
\end{remark}


\begin{remark}
  Friedlander showed in \cite{F80} that for all finite energy solutions $\phi$ to \eqref{eq:wave:intro:data},
  with $\|\phi\|_{H_E}^2=\int |\nabla f|^2+g^2 \ud x<\infty$, the time-derivative of $\phi$ has a radiation field, 
  \begin{equation}
    \label{eq:41}
    \partial_t\phi\sim\frac{\mathcal{G}_0(r-t,\omega)}{r}\,,\text{ and }\qquad \int_{-\infty}^{\infty} \int_{\mathbb{S}^2} \mathcal{G}_0(q,\omega)^2\ud S(\omega) \ud q \les \|\phi\|_{H_E}^2\,.
  \end{equation}
...
\begin{equation}
  ...
  \int_{-\infty}^{\infty} \mathcal{H}_0(q,\omega)\ud q=\mathcal{P}(\omega)
\end{equation}
where $\mathcal{P}(\omega)$ is the solution of an elliptic problem on $\mathbb{S}^2$ determined by the four functions on the sphere above. Moreover in the case of fast decaying radiation fields, namely when $\mathcal{F}_0=\mathcal{H}_0$, the compatibility condition \eqref{eq:H0:compatibility} reduces to the statement that $\mathcal{P}(\omega)$ is independent of $\omega$.
...
More precisely, for any given $N_{01}^{ext}$, $N_0^{ext}$, and $M_0^{ext}$, and any smooth function $\mathcal{H}_0$ satisfying \eqref{eq:H0:integrable}, we can define $\mathcal{F}_0$  by \eqref{eq:H0:F0}, where $N_0^{int}$ is determined by $N_{01}^{ext}$, and $M_0^{int}$ is given by \eqref{eq:M0:C} for any given constant $C_0$.
...
\newblock {\em Math. Proc. Cambridge Philos. Soc.}, 88(3):483--515, 1980.
