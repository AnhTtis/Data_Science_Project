...
Therefore, we use pre-specified observation $\boldsymbol{r}$ as a ``prompt'' to guide the visual reasoning process. 
Our Regional Prompt Generator (RPG) creates three detailed prompts focusing on specific regions. 
These prompts harness local features ($\boldsymbol{r}$), surrounding context, and existing visual prompts, as shown in Figure \ref{fig:prompt1}-\ref{fig:prompt3}. 
