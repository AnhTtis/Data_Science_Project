...
\begin{lemma}
\label{rootnh}
Assuming equation \ref{eq:1} holds and regularity condition \ref{regularityq} in the appendix, $\phi_o(W,X) \in \mathcal{R}(T_o')$ is necessary for $\beta_o$ to be $\sqrt{n}$ estimable.
\end{lemma}
...
\end{equation}
To facilitate analysis of the asymptotic properties of the estimator, let $\overline{\hh}$ be the closure of the linear span of $\hh$ under $\Norm{\cdot}_w$, which is a Hilbert space with the inner product:
\begin{equation*}
    \la h_1,h_2 \ra_w=\E\sqb{I(A=a)\E\{h_1(W,A,X)|A,Z,X\}\E\{h_2(W,A,X)|A,Z,X\}}.
\end{equation*}
This is indeed an inner product as long as Assumption \ref{comp1}(2) holds. Next, we require the following:
\begin{assumption}
\label{rep}
