...
%
The main factor distinguishing FePt films from other materials is it possesses one of the highest coercivity values~\cite{snarski-adamski_magnetic_2022}.
%
...
%
Such a high coercive field value is comparable to the values obtained for extremely strong samarium-cobalt permanent magnets.
...
%
The aforementioned studies of several nanometer-thick ultrathin FePt films reveal details of their granular microstructure~\cite{perumal_particulate_2008, kurth_finite-size_2010, kaidatzis_magnetic_2017, zhang_microstructure_2018}.
%
...
%
Based on the tetragonal unit cell, two sets of films ranging from 4 to 16 atomic monolayers were generated, with the [010] and [111] directions of the prepared films corresponding to the [001] direction of the bulk \loz{} phase, see Fig.~\ref{fig-111-structure}.
%
...
%
The vacuum space in the [001] direction of the film is added so that films of different thicknesses have a constant value of the unit cell lattice parameter $c$.
%
...
%
Such a simplification can be justified by results from Hammar~\textit{et al.}~\cite{hammar_theoretical_2022}, where optimization of the geometry of FePt film showed that mainly the positions of only three outer atomic monolayers change, and the change in the distance between these layers is only about 1~--~2\% and does not significantly affect the magnetic properties.
%
...
fully-relativistic iteration~\cite{eschrig_chapter_2004} was performed for each of several selected quantization directions of the considered films.
...
%
The energy minimum occurs at an angle of about 50.0$^{\circ}$ relative to the [001] direction.
%
...
%
The presented calculations also represent the first attempt to model \lozfept{} films with an arrangement of atomic monolayers other than in the plane of the films and are one of the first to study the thickness dependence of the magnetic properties of FePt films.
...
Among the magnetic compounds with the \loz{} layered structure, FePt, FePd, FeNi, and CoPt are of special interest.
%
...
%
The thickness of the 16-monolayer film is 3.5~nm, corresponding to a distance of 0.22~nm between atomic monolayers in the film direction [001].
%
...
%
Calculations were made for magnetization directions changing every 10$^{\circ}$ along the path [001]~$\to$~[010]~$\to$~[100]~$\to$~[001].
%
The selected 10-monolayer film shows an energy minimum for the in-plane direction (100) at an angle of about 50$^{\circ}$ to the [001] direction. 
%
This means that the determined direction of magnetization is not exactly parallel to the direction of alignment of the Fe and Pt monolayers, which is equal to 53.96$^{\circ}$ (counting from the [001] direction, not from the plane of the film, as done in Ref.~\cite{zha_pseudo_2009}).
%
...
Energy differences $E_{001}-E_{010}$ and $E_{100}-E_{010}$ as a function of thickness of \lozfept{} films with (010) surface.
%
...
%
Since the [100] and [010] tetragonal axis orientations are equivalent when FePt islands form on the substrate, the effective magnetization and in-plane anisotropy can average out for larger sample areas. 
%
...
%
The direction was changed according to the path of [001]~$\to$~[010]~$\to$~[100]~$\to$~[001] every 10$^{\circ}$, and the calculated results are presented in Fig.~\ref{fig-en-dir-010}.
%
The easy magnetization direction lies in the plane of the film and follows the direction of alternating Fe and Pt monolayers [010].
%
...
%
The energy difference between the two orthogonal in-plane magnetization directions ([010] and [100]) is similarly very high, confirming the intuitive expectation of a strong fixed magnetic anisotropy in the plane of the film.
...
%
Figure~\ref{fig-en-diff-dependence} shows the calculated energy differences 
 $E_{001} - E_{010}$ and $E_{100} - E_{010}$ for the full range of film thicknesses taken into account.
%
All the energy differences are positive and relatively high, which implies a high in-plane fixed magnetic anisotropy.
%
Except for the 10-monolayer film, the energy difference of $E_{001}-E_{010}$ is larger than $E_{100}-E_{010}$ by about 0.2~meV\,atom$^{-1}$, indicating that the magnetization direction perpendicular to the film plane has the highest energy and is, therefore, the least preferred.
%
In addition, the general trend shows a decrease in magnetic anisotropy with decreasing film thickness. 
%
However, the dependence on film thickness is not monotonic over the entire range considered, which is not surprising since previous calculations for ultrathin CoPt films also showed oscillations, especially in the range up to about 10 monolayers~\cite{cinal_magnetic_2022}.
%
Nevertheless, the magnetic anisotropy still shows very high values, even for the thinnest films.
...
%
We want to emphasize that the in-plane anisotropy observed for the (010) films is very unusual, as it strongly distinguishes a specific in-plane direction.
%
...
%
Moreover, calculations have shown that reducing the thickness of the (111) films to about a few atomic monolayers leads to a further tilt of the easy direction toward out of film plane.
%
...
%
We would like to thank Daniel Kiphart for reading the manuscript and for his comments on the language.
