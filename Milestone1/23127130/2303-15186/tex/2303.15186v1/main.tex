...
\title{Real spectra in 1D single-band non-Hermitian Hamiltonians}
\author{Haoyan Chen}
...
\begin{equation}
    H=\sum\limits_{m,n}a_{m-n}c_m^\dagger c_n=\sum\limits_k\sum\limits_{l}a_{l}e^{-ikl}c_k^\dagger c_k\equiv \sum\limits_k H(k)c_k^\dagger c_k, \label{Hamiltonian}
\end{equation}
where the Hamiltonian in the momentum space is:
\begin{equation}
    H(k)=\sum\limits_{n=-p}^{q}a_ne^{ikn},
\end{equation}
...
\begin{equation}
    a(\beta):=H(e^{ik}\rightarrow \beta)=\sum\limits_{n=-p}^qa_n\beta^n, \label{characteristic polynomial}
\end{equation}
...
\begin{equation}
    \det(H(\beta)-E)=a(\beta)-E=a(\beta)-a(\gamma(t))=0. \label{E=a(gamma(t))}
\end{equation}
Therefore:
\begin{equation}
...
\begin{eqnarray}
    ...
    |\beta_{p+1}(E)|,|\beta_{p+2}(E)|,\cdots,|\beta_{p+q}(E)|&\rightarrow& \infty,
\end{eqnarray}
...
These results imply that if the energy spectrum of a single-band non-Hermitian Hamiltonian is real under OBC, the preimage of $\mathbb{R}$ under its characteristic polynomial $a$, denoted as $a^{-1}(\mathbb{R})$ and contains the GBZ, must have the loop property (contain a loop that encircle $0$). Thus, we have obtained a necessary condition for real spectra in 1D non-Hermitian systems under OBCs. 

Then, one may wonder whether such a condition is sufficient. In the following, we will consider the complex-hopping Hatano-Nelson model and the Hatano-Nelson model with next-nearest-neighbor hopping terms and demonstrate the condition's sufficiency in these two models analytically. Further, we numerically verify the condition's sufficiency for models with general, higher-order polynomials, where our derivation no longer applies, in Appendix \ref{example}. 
...
First, we revisit the Hatano-Nelson model in Eq. (\ref{Hatano_Nelson}) with a similarity transformation \cite{Hatano1996, Hatano1997, Kohei2020, Okuma2022, Rivero2021}:
\begin{equation}
...
\begin{figure*}[!ht]
    \centering
    \includegraphics[width=0.98\textwidth]{a=1_b=0.16.pdf}
...
\subsection{1D single-band models with further neighbor hopping}

In the above subsections, we have analytically shown the consistency of our criteria for the Hatano-Nelson model with either complex hopping or additional NNN term. Since higher-order equations are generally too complicated to solve analytically, we resort to numerical evidence for more complex models with longer-range hopping and higher-order characteristic polynomials. In particular, we have studied general 1D non-Hermitian tight-binding models in Eq. (\ref{Hamiltonian}) with $p$ up to 2 and $q$ up to 4, and our criteria remain fully valid: the appearance (disappearance) of purely real OBC spectrum coincides with the (lack of) loop property the preimage $a^{-1}(\mathbb{R})$. We include several examples in Appendix \ref{example}. 

\iffalse
Take $A=1, B=0.16$ as an example, when $E$ goes from $-\infty$ to $\infty$, $\beta_{1,2,3}(E)$ will flow as shown in Fig. \ref{a=1_b=0.16} and Eq. (\ref{flow}), where three intersection points are $\beta_A=-2.695, \beta_B=-1.313, \beta_C=0.883$.
\begin{equation}
\left\{
    \begin{array}{lcr}
        \beta_1<0, \beta_2=\beta_3^* & &E<-1.904 \\
        \beta_1<0, \beta_2=\beta_3=\beta_A & &E=-1.904 \\
        \beta_3<\beta_2<\beta_1<0 & &-1.904<E<-1.799 \\
        \beta_1=\beta_2=\beta_B, \beta_3<0 & & E=-1.799 \\
        \beta_1=\beta_2^*, \beta_3<0 & & -1.799<E<2.140 \\
        \beta_1=\beta_2=\beta_C, \beta_3<0 & & E=2.140 \\
        \beta_3<0<\beta_1<\beta_2 & & E>2.140
    \end{array}
\right. \label{flow}
\end{equation}
\fi
