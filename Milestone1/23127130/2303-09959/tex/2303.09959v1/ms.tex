...
The expected noise performance of the WFI drives the magnitude and distance limits that we apply in our simulations. We modeled the standard deviation of normalized photometry in the time domain survey's primary \Fmag filter using the properties listed in Table~\ref{tab:photometry}, the values of which were sourced from the WFIRST Cycle 7 design described in \citet{Penny2019}. The model assumes a sky background count rate that was determined using a time-dependent zodiacal light model evaluated at the midpoint of the survey seasons. It also assumes a 9-pixel aperture \citep[which was found to produce the best photometry in the crowded microlensing fields in ][]{Penny2019} and a constant systematic noise floor of 1~mmag. 
...
%We ran a \texttt{Besançon} simulation in the direction of the center of the Roman time domain survey fields ($l = 1.0^\circ$, $b = -0.667^\circ$) with a solid angle of 1 deg$^2$ out to a distance of 2.4 kpc. The model currently does not support magnitudes in the Roman passbands, so we applied magnitude limits based on the 99.9\% quantiles of the \textit{r}, \textit{i}, \textit{z}, and \textit{J} magnitudes of our simulated sources (see Section \ref{subsec:hosts} and Figure \ref{fig:source_histograms}). These limits should guarantee that the resulting sources have $F146$ magnitudes less than 21. 
...
We injected planets into our 1,000 samples of mid-M dwarfs and UCD targets. An average of 1347$^{+208}_{-124}$ planets were detected in each simulation under the assumption of DC15 planet occurrence rates. Hosts were frequently found to host 2+ transiting planets, with with an average of 274$^{+41}_{-30}$ multiple systems identified per simulation.

We break down the average number of planets as a function of T$_{eff}$ in Table \ref{tab:host_teffs}. These results are compared against the confirmed and candidate planets with $R_p$~$<$~4~R$_\oplus$ around hosts with T$_{eff}$~$<$~3300~K detected by the Kepler (including K2) and TESS missions, which were tabulated using data from the NASA Exoplanet Archive\footnote{\url{https://exoplanetarchive.ipac.caltech.edu/}} on 2022 October 27. We visualize these results in Figure \ref{fig:kepler_tess_comp}, which shows the planet detections from a representative iteration of our simulation compared to the detections from Kepler and TESS. 
...
%         \tablehead{
%              \colhead{SpT} & 
%              \colhead{\hspace{0.5cm}DC15 Injection} & 
%              \colhead{\hspace{0.5cm}Kepler} &
%              \colhead{\hspace{0.5cm}TESS} 
%              }
%         \startdata
%             M3 & 782$^{+158}_{-133}$ & 16 & 44\\
%             M4 & 974$^{+155}_{-139}$ & 10 & 21\\
%             M5 & 230$^{+47}_{-41}$ & 5 & 11\\
%             M6 & 71$^{+16}_{-13}$  & 0 & 4\\
%             M7 & 28$^{+9}_{-8}$ & 0 & 0\\
%             M8 & 10$^{+6}_{-4}$ & 0 & 0\\
%             M9 & 4$^{+3}_{-3}$ & 0 & 0\\
%             L0$\textendash$T9.5 & 1$^{+2}_{-1}$ & 0 & 0\\
%         \enddata

%         \tablecomments{The reported Kepler detections include confirmed and candidate planets from both the Kepler and K2 missions, excluding candidates that were flagged as false positives. The reported TESS detections include confirmed planets and TESS objects of interest that have not been classified as false positives. The tables detailing these detections were accessed from the NASA Exoplanet Archive on 2022 October 27. \red{Should we have a table of the Kepler and TESS detections?}}
