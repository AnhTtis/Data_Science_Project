...
\begin{proof}
  ...
  \begin{displaymath}
    ...
    \end{displaymath}
  In addition, Lemma \ref{emb norm} shows that $0 \preceq T_\nu^{-\frac{1}{2}} T_{x_{i}} T_\nu^{-\frac{1}{2}} \preceq M_{\alpha}^{2}\lambda^{-\alpha}, \mu \text {-a.e. } x \in \mathcal{X}$. So we have
  \begin{displaymath}
...
Denote $ \xi_{i} = \xi(x_{i},y_{i}) = T_{\nu}^{- \frac{1}{2}} (K_{x_{i}} y_{i} - T_{x_i}f_{\nu} )$, (\ref{goal of theorem 4.9}) is equivalent to
   ...
   \end{equation}
Consider the subset $\Omega_{1} = \{x \in \Omega: |f_{\rho}^{*}(x)| \le t \}$ and $\Omega_{2} = \mathcal{X} \backslash \Omega_{1}$, where $t$ will be chosen appropriately later. Assume that for some $ q \ge 2$, 
\begin{displaymath}
