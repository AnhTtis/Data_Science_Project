...
\begin{subequations}
	...
	& u_\op = \begin{cases} (\Php, \delta_{\hp}, \slackVar) & \delta_\op = 1 \hspace{0.1cm} \\ (\dotQhp, \delta_{\hp}, \slackVar) & \delta_\op = 0 \end{cases} \label{subeq_concept_input_var}\\
	 &u_\op \in \realv{\Nhor} \times \{0,1\}^\Nhor \times \realv{\Nslack}_+\\
		...
		& \xv_{\iterOp+1} = \Am\xv_{\iterOp} + \Bm \dotQhpi{\iterOp} + \Em \dv_\iterOp, \hspace{0.5cm} \Tri{\iterOp} = \Cm \xv_{\iterOp} \label{subeq_conccept_dynamics}\\
...
\begin{subequations}
    ...
    \begin{align}
        ...
        &\min_{\uv \in \realv{m \times N}, } J(\xv_0, \uv)\\
        &\text{s.t.}\\
        &\SSdx{k}, \hspace{0.2cm} \xv_{k} \in  \mathcal{X},\hspace{.2cm}\xv_0 = \xv(t)\\
        &\yv_{1,k} = \Cm\xv_k + \Dm\uv_k, \hspace{0.2cm} \uv_{k} \in  \mathcal{U} \\
        &\yv_{2,k} \geq \textbf{f}_\text{convex}(\xv_k, \uv_k) \hspace{0.6cm} \yv_{3,k} \leq \textbf{f}_\text{concave}(\xv_k, \uv_k)
    \end{align}
\end{subequations}
