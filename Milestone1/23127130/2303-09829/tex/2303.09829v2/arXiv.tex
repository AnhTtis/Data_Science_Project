...
that do not correspond to  melonic diagrams of ordinary tensor field theories. They therefore describe pertinent candidates to escape the 
so-called branched polymer phase, the universal geometry found for  tensor models. For generic rank $d$ of the tensor field,  we compute the perturbative $\beta$-functions at one-loop of two just-renormalizable quartic eTFT coined by $+$ or $\times$,  depending on  their vertex weights.
The models $+$ has 
...
The RG flow of the  model $+$ exhibits neither asymptotic
...
In the UV, the mass behaves likewise namely is linear
in $t$, whereas $Z_a$ decreases exponentially towards a constant value. 
For the  model $\times$, both $\lambda$ and $\lambda_{\times}$ do not flow,  all remaining 2-point coupling constants are linear functions of the time scale in the UV. 
\end{abstract}
...
this behavior is neither comparable to asymptotic freedom, to 
 asymptotic safety,  nor to Landau ghost. 
The model $+$ has a mass and another 2-point coupling $Z_a$
for a quadratic term $p^{2a}(\phi_{p})^2$ (which is not the wave function 
renormalization and was introduced to ensure the  renormalizability of the model). 
...
however, at the contrary of relevant operators of ordinary QFT, the mass 
 grows linearly in the UV.  $Z_a$ on the other hand, is suppressed in the UV and reaches a constant, so behaves just like an ordinary relevant operator in IR.   
...
This is again a peculiar behavior different from usual QFT and TFT models. 
...
decreases exponentially in the UV ($t \rightarrow \infty$) and suppressed up to reach a constant $ - 2 \beta_{Z_a}  \, \lambda_{+} $. Meanwhile, in the IR ($t \rightarrow -\infty$),  $ Z_a(t)$ blows up as any relevant coupling. 
...
Following \cite{BenGeloun:2016rqa}, 
...
scalar field theory as well as of TFT  where relevant operators in the UV are suppressed. 
...
Understanding the flow of $\widetilde Z_a(t) $ rests upon the 
  understanding of the polynomials $ Q_{1;n}$ and  $ Q_{2;n}$. 
Focusing on the asymptotic behavior of the Erf function, 
Erf'$(z) \sim e^{-z^2}/z^2$, 
we see that 
$\widetilde Z_a(t) 
\sim 
 e^{\frac{Q_{2;n}  t^2}{2}-\frac{t}{2}}$. Thus, 
...
The system of RG flow equations can be explicitly computed. 
...
For the model $+$, the enhanced $4$-pt coupling $\lambda_+$ does not run and therefore is constant, whereas the ordinary $4$-pt coupling $\lambda$ grows linearly in $t= \log k/k_0$, where $k$ is momentum scale.
This statement is true for all orders in perturbation theory. 
The $2$-pt coupling $Z_a$ has  exponential behavior in $t$:  
it therefore is suppressed in the UV and reaches a constant.  
At first order of perturbation, we found that the mass behaves differently in the UV: it grows linearly in $t$.
In the IR, the mass behaves like a relevant operator and blows up exponentially. Thus in both direction, the mass grows. This is something specific about the present eTFT model. 
 

Concerning the model $\times$, the $4$-pt couplings $\lambda$ and $\lambda_\times$ do not run and therefore are constant at all orders of perturbation. On the other hand, all the $2$-pt couplings (mass, $Z_a$  and $Z_{2a}$) grow in their magnitude linearly in the scale $t$. Once again, this behavior is not common of ordinary QFT and TFT, and thus is brought by the particular form of the vertex weight and the combinatorics of the model. 

Furthermore, with the explicit forms of the running of the couplings at first order, together with the close observations of the power counting theorems for each model, $+$ or $\times$, in \cite{BenGeloun:2017xbd}, we have deduced the behavior of the $\beta$-functions of the couplings at all orders of perturbation. 
The computation of the polynomial coefficients at all-loops will be left for future investigations.  
We conjecture that the systems could be explicitly integrated at arbitrary and given order. 
More generally, the  amplitudes are simple enough 
to be re-summed at all orders. 
This property to be proved will require more work. 


 
Concerning the quantum gravity side, the absence of 
asymptotic freedom or non trivial fixed points in the UV  at first order of perturbation for generic $(\lambda,\lambda_+)$ 
makes unlikely the UV completion of the quartic eTFTs. However, much less is known about their IR behavior.
  The computation of higher order perturbations of the models 
$+$ and $\times$  may have IR fixed points. Flowing backwards in the IR direction, the system of $\beta$-function collects polynomial coefficients in the coupling $\lambda_+$. The root of these polynomials may lead to vanishing $\beta$-functions and hence produces non trivial IR fixed points.   The IR study of eTFTs will require the use of different tools like the Functional Renormalization Group Equation which could be also a following-up study from our work.
