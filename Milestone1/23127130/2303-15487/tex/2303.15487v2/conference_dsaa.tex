...
\begin{abstract}
  ...
  We instantiate KeGNN in conjunction with two state-of-the-art graph neural networks, Graph Convolutional Networks and Graph Attention Networks, and evaluate KeGNN on multiple benchmark datasets for node classification.
\end{abstract}
...
In KeGNN, knowledge enhancement layers are stacked on top of a GNN and adjust its predictions in order to increase the satisfaction of some prior knowledge.
...
\begin{figure*}[h] 
  ...
  \caption{\begin{footnotesize} Example extract of the Citeseer citation graph. \end{footnotesize}
...
\begin{example}
    ...
    Fig.~\ref{fig:graph_structure} shows an extract of the Citeseer citation graph that is used as example to guide through this paper. 
...
The graph $\mathbf{G}_{\mathrm{Cit}}$ in Fig.~\ref{fig:graph_structure} can be expressed in $\mathcal{L}$.
...
\begin{example}
In case of the beforementioned citation graph of Fig.~\ref{fig:graph_structure}, $\mathbf{U}$ and $\mathbf{B}$ are defined as: 
\begin{equation*}
...
Within this category, KeGNN belongs to the model-based approaches, where prior knowledge in the form of knowledge enhancement layers is an integral part of the model.
...
First, logically informed embedding approaches \cite{sole} \cite{10.1007/978-3-030-88361-4_24} use predefined logical rules that provide knowledge to a neural system, while both components are mostly distinct. 
Second, approaches for knowledge graph embedding with logical constraints \cite{simple} \cite{kale} use prior knowledge as constraints on the neural knowledge graph embedding method in order to modify predictions or embeddings. 
Thirdly, neuro-symbolic methods are used for learning rules for graph reasoning tasks \cite{expressgnn} \cite{qu2021rnnlogic}. 
...
The implementation of KeGNN and the experiments will be made publicly available after the revision period.
% \footnote{The implementation will be made public after the review period.}.
%\footnote{https://gitlab.inria.fr/tyrex/kegnn}.
\subsection{Results}
% \begin{figure}[h]
%   \centering
%   \begin{subfigure}{4cm}
...
\begin{table*}
  ...
  \hline
  \end{tabular}
  \caption{\begin{footnotesize}Average test accuracy of 50 runs (10 for Flickr). The standard deviations are reported in brackets. \end{footnotesize}}
  \label{tab:results}
  \end{table*}
...
The results are given in Tab.~\ref{tab:results}.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
     \includegraphics[width=7cm]{KeMLP_citeseer_accuracy_nodedegree.pdf}
  \end{subfigure}  
  \begin{subfigure}[b]{0.5\textwidth}
     \centering
     \includegraphics[width=7cm]{keGCN_citeseer_accuracy_nodedegree.pdf}
  \end{subfigure}
%   \begin{subfigure}[b]{0.5\textwidth}
...
\begin{subfigure}[b]{0.5\textwidth}
  \centering
  \includegraphics[width=7cm]{KeGAT_citeseer_accuracy_nodedegree.pdf} 
  \end{subfigure}
  \caption{\begin{footnotesize}The accuracy grouped by the node degree for MLP vs. KeMLP (above) and GCN vs. KeGCN (middle) and GAT vs. KeGAT(below) on Citeseer.\end{footnotesize} \label{fig:acc_nodedegree}} 
  \end{figure}
It turns out that the performance gap between MLP and KeMLP is larger than for KeGNN in comparison to the standalone GNN.
To explain this observation, we examine how the graph structure affects the prediction performance. 
Therefore, in Fig.~\ref{fig:acc_nodedegree} we analyze the accuracy grouped by the node degree for the entire graph for MLP vs. KeMLP and GCN vs. KeGCN. 
...
To analyze this question, we categorize the accuracy by the proportion of misleading nodes in the neighborhood, see Fig.~\ref{fig:acc_wrong_neighbors}.
...
When comparing GCN to KeMLP, see plot below in Fig.~\ref{fig:acc_wrong_neighbors}, KeMLP is more robust to misleading neighbors. 
...
\begin{figure}[t]
  \centering
  \begin{subfigure}{4cm}
  \centering\includegraphics[width=4cm]{kegcn_citeseer_acc_wrongneighbors.pdf}
  %\caption{Cora}
  \end{subfigure}%
  \begin{subfigure}{4cm}
  \centering\includegraphics[width=4cm]{KeMLP_citeseer_acc_wrongneighbors.pdf}
  %\caption{Citeseer}
  \end{subfigure}\vspace{10pt}
  \begin{subfigure}{4cm}
    \centering\includegraphics[width=4cm]{KeMLP_vs_GCN_citeseer_acc_wrongneighbors.pdf}
    %\caption{Citeseer}
    \end{subfigure}\vspace{10pt}
  \caption{\begin{footnotesize}The accuracy grouped by the ratio of misleading first-order neighbors for GCN vs. KeGCN (left), MLP vs. KeMLP (right), GCN vs. KeMLP (below) on Citeseer. \end{footnotesize} \label{fig:acc_wrong_neighbors}} 
  \end{figure}
...
\begin{figure}[h]
  \centering
  \begin{subfigure}{4cm}
  \centering\includegraphics[width=4cm]{CiteSeer_KeMLP_clauseweights.pdf}
  %\caption{Cora}
  \end{subfigure}%
  \begin{subfigure}{4cm}
  \centering\includegraphics[width=4cm]{CiteSeer_KeGCN_clauseweights.pdf}
  %\caption{Citeseer}
  \end{subfigure}\vspace{10pt} 
  \caption{\begin{footnotesize}Learned clause weights vs. clause compliance for KeMLP (left) and KeGCN (right) on Citeseer.\end{footnotesize} \label{fig:clause_weights}} 
  \end{figure}
...
Fig.~\ref{fig:clause_weights} displays the learned clause weights for KeGCN and KeMLP versus the clause compliance on the ground truth labels of the training set.
...
\begin{figure}[]
  \centering
  \begin{subfigure}{4cm}
  \centering\includegraphics[width=4cm]{kegcn_citeseer_complianceevolution_allnodes.pdf}
  %\caption{Cora}
  \end{subfigure}%
  \begin{subfigure}{4cm}
  \centering\includegraphics[width=4cm]{KeMLP_citeseer_complianceevolution_allnodes.pdf}
  %\caption{Citeseer}
  \end{subfigure}\vspace{10pt}
  \caption{\begin{footnotesize}Clause compliance during training for GCN vs. KeGCN (left) and MLP vs. KeMLP (right) on Citeseer. 
...
Fig.~\ref{fig:compliance_evolution} plots the evolution of the clause compliance for the six clauses for GCN vs. KeGCN and MLP vs. KeMLP.
...
This allows the use of prior knowledge to improve node classification while exploiting th strength of a GNN to learn expressive representations.
...
The implementation of KeGNN and the described experiments will be made publicly available after the revision period. 
%\footnote{\url{https://gitlab.inria.fr/tyrex/kegnn}}. 
...
%       \hline
%       \end{tabular}
%       \caption{\begin{footnotesize}Mean epoch times for all models on the Citeseer dataset.\end{footnotesize}}
%       \label{tab:runtimes}
%       \end{table}
...
%    The runtimes were calculated for models with three hidden layers and three knowledge enhancement layers in full-batch training.
...
%         \label{fig:results}
%         \end{figure}
% Fig.~\ref{fig:acc_nodedegree_gat} shows the accuracy grouped by node degree for GAT vs. KeGAT. 
% \begin{figure}[]
%   \centering
% \includegraphics[width=7cm]{figures/KeGAT_citeseer_accuracy_nodedegree.pdf}
% \caption{\begin{footnotesize}The accuracy grouped by the node degree for GAT vs. KeGAT on Citeseer.\end{footnotesize} \label{fig:acc_nodedegree_gat}} 
% \end{figure}

\subsection{Model Parameters and Hyperparameter Tuning}
...
in Tab.~\ref{tab:pubmed_flickr_hp}.
