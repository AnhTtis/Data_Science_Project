...
$
%of a weighted $L^2$-product\footnote{The corresponding term on the inflow boundary will later appear in the right-hand side.}.
...
The following proposition gives a characterization of the test space $\ycal$:
\begin{proposition}
  The test space $\ycal$ is isomorphic to the Sobolev-space
  \begin{equation}
...
\begin{equation*}
  \ycal = A^{-*}\xcal', \quad \xcal = A^{-1}\ycal'\vspace*{-1em}
\end{equation*}
and\vspace*{-1em}
\begin{equation*}
...
\end{equation}
\end{definition}
This is essentially a LL*-method (see \eg~\cite{caiFOSLL}) applied to the minimization of the residual energy $\norm{A[u]-f}_{\ycal'}$.
