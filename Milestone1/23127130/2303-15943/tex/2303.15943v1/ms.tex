...
In this contribution we are considered with stationary reactive transport equations of the form 
...
A common approach for stabilization is the enrichment of the test space by supremizers leading to a non-symmetric Petrov-Galerkin scheme. 
These supremizers are, however, expensive to compute as this requires the (approximate) solution of a PDE for each trial basis function. 
In this contribution, we present an alternative approach based on the construction introduced by Brunken et al.\ in \cite{BrunkenSmetanaUrban} 
and recently also employed by Henning et al.\ \cite{HenningPalitta} for the wave equation. 
The main idea
% relies on choosing the test space first and %the
is to first choose the test space and
subsequently determine a corresponding trial space.
...
using a related normal equation that is defined solely on the test space.
...
optimal test space is sufficient to solve the normal equations and
perform classic functional evaluations of the reconstructed solution
without the need to discretize the trial space explicitly.
% Further
Future
work will concern the robust solving of the discrete normal equations,
as well as using model order reduction techniques to efficiently solve
parameter-dependent reactive transport problems.  \vspace*{-.2em}
...
Geometry -- Structure.\vspace*{-.2em}

%\printbibliography

%cite DUNE in the numerics section
