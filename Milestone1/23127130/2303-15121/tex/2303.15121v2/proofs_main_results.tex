...
\end{align}
%
Hence using \eqref{eq:no_tancone_tmp21}, \eqref{eq:no_tancone_tmp22},   \eqref{eq:no_tancone_tmp23} and \eqref{eq:no_tancone_tmp3}, it holds w.p at least $1 - 2\exp(-c_2 \min\set{\frac{t^2}{\Vbar^2}, \frac{t}{\Ubar}}) - \exp(-u)$ (for $u \geq 1$)  that 
%
%
%
\begin{align}
...
\end{align}
%
It remains to simplify the expressions of the terms involved in our bounds. To this end, we obtain analogous to Claim \ref{claim:gamma_func_bds} that for any $\alpha > 0$, 
%
\begin{align*}
\gamma_{\alpha}(\calS_x,\norm{\cdot}_2) &\leq \gamma_{\alpha}(\calKtil \cap x\frobsphere,\norm{\cdot}_2) J(A^*), \\
%
\gamma_{\alpha}(\calS_x,\norm{\cdot}_F) &\leq \sqrt{T} J(A^*) \gamma_{\alpha}(\calKtil \cap x\frobsphere,\norm{\cdot}_F) J(A^*), \\
%
\gamma_{\alpha}(\calStil_x,\norm{\cdot}_2) &\leq \gamma_{\alpha}(\calKtil \cap x\frobball,\norm{\cdot}_2) J(A^*), \\
%
\gamma_{\alpha}(\calStil_x,\norm{\cdot}_F) &\leq \sqrt{T} J(A^*) \gamma_{\alpha}(\calKtil \cap x\frobball,\norm{\cdot}_F) J(A^*) .
\end{align*}
%
Furthermore, we can bound $\Pbar, \Vbar$ as
%
\begin{align*}
\Pbar &\leq \gamma_2(\calKtil \cap x\frobsphere,\norm{\cdot}_2) J(A^*) \left(\gamma_2(\calKtil\cap x\frobsphere,\norm{\cdot}_2) J(A^*) + x J(A^*) \sqrt{T} \right) + x^2 J^2(A^*) \sqrt{T} = : \Pbar_1, \\
%
\Vbar &\leq x J^2(A^*) \left(\gamma_2(\calKtil\cap x\frobsphere,\norm{\cdot}_2) + x\sqrt{T} \right) =: \Vbar_1,
\end{align*}
%
