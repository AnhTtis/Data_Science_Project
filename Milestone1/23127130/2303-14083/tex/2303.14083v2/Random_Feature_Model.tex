...
\begin{figure}
\centering
\includegraphics[width=1\linewidth]{Figures/Hilfskonstruktionen_LM2.pdf}
...
Eqs. (\ref{final error reduced erf}) and (\ref{final error reduced relu}) can be subsumed in the vector-matrix-form 
\begin{align}
...
\begin{figure}[t]
\centering
\subfigure{\includegraphics[width=0.75\linewidth]{Figures/odes_vs_simulation_N100_differentN.pdf}}\label{fig:1a}\\
\subfigure {\includegraphics[width=0.75\linewidth]{Figures/odes_vs_simulation_N100_differentK_relu2.pdf}}\label{fig:1b}
...
\label{mean path}
\end{align}
depending on the choice of the activation function. Therefore, we obtain the set of deterministic differential equations  Eq.~(\ref{mean path}) characterizing the dynamical behavior of the learning process that is valid for arbitrary ratios $\frac{K}{N}$. The fixed point of Eq.~(\ref{mean path}) reveals the asymptotic solutions for the weights
\begin{equation}
...
\begin{figure}[t]
\centering
\subfigure{\includegraphics[width=0.75\linewidth]{Figures/eps_von_K_herleitung.pdf}}\label{fig:2a}\\
\subfigure {\includegraphics[width=0.75\linewidth]{Figures/eps_von_K_herleitung_relu.pdf}}\label{fig:2b}
...
\begin{figure}[t]
\centering
\subfigure{\includegraphics[width=0.75\linewidth]{Figures/eps_von_K_N_M1_matrix_ini_10_differentN_loglog4.pdf}}\label{fig:3a}\\
\subfigure {\includegraphics[width=0.75\linewidth]{Figures/eps_von_K_N_M1_matrix_ini_10_differentN_loglog_relu2.pdf}}\label{fig:3b}
\caption{Asymptotic generalization error as a function of $\frac{K}{N}$ for an error activation $g\hspace{-2pt}\left(x\right)=\mathrm{erf}\hspace{-2pt}\left(\frac{x}{\sqrt{2}}\right)$ (top) and ReLU (bottom). Here, we evaluate Eq.~(\ref{asym eg}) numericall and compute the expectation value of the generalization error for a given $\frac{K}{N}$. The errorbars show the standard deviation of the average over $100$ initializations for $N=10$ and $10$ initializations for $N>10$. The asymptotic generalization error increases with $N$, even though the hidden layer size increases proportional to that of the input layer. For large enough $N$ and finite $\frac{K}{N}$, the generalization error obtains the limit value for the linearized scenario given in Eq. (\ref{plateau}) for the error function and Eq. (\ref{plateau relu}) for the ReLU activation function.} \label{fig: eg von K/N}
\end{figure}
\endgroup
...
\begin{figure}[t]
\centering
\includegraphics[width=1\linewidth]{Figures/eps_von_R_K7_M1_N5_3.pdf}
...
\begin{equation}
K > \sqrt{2N-4} \hspace{5pt} e^{\frac{N}{2} \ln\left(\frac{1}{1-{R^*}^2}\right)  } \hspace{3pt} \lvert  \ln\left(1-P^*\right)\rvert \hspace{2pt}.
\label{K_depen}
\end{equation}
