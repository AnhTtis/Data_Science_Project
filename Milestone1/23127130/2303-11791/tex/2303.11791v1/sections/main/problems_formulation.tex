...
% Now consider recording the faint 
...
Furthermore, the temporally local nature of difference frames enables them to provide ``clean" motion information. In comparison, although background frame estimation and subtraction~\cite{bouman2017turning, PrafullSharma2021WhatYC, he2022non} can increase SNR, this practice inevitably introduces information of other time to every single frame, thus making static information ``dirty".


%Due to the continuity of trajectory and stability of the imaging mapping $\mathcal{F}$, there is only a slight change between two adjoined frames. Therefore, difference frames enjoy a sparse nature since tons of pixel values become zero after subtraction.This sparse nature is observed in both real-shot data and synthetic data, which respectively have a zero-padding rate of 36.17\% and 80.25\% on average over the entire dataset.The numerous zero-padding, as black areas shown in \cref{fig:raw_diff}, makes difference frames both space- and time-saving when comparing to temporal mean-subtraction technique. Additionally, a greater zero-padding rate is observed in synthetic data because their are more ``clean" due to the rendering simulation. To maintain the sparse nature of difference frames, we don't apply normalization to difference frames in data pre-processing.
