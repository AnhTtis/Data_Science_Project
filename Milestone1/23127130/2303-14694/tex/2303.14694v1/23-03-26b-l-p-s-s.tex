...
%\address{School of Mathematics, KIAS, Seoul 02455, Republic of Korea}
...
However, the approach to define bigraded persistence via the homology of moment-angle complexes has two fundamental drawbacks. First, calculating simplicial homology groups of all full subcomplexes (or samples) in $R(X,t)$ is much resource-consuming from the computational viewpoint. Second, bigraded persistent homology lacks the stability property, which is important for applications of persistent homology in data science~\cite{c-e-h07}. 
...
In Section~\ref{sec_stability_HZK} we prove the Stability Theorem for bigraded persistent double homology (Theorem~\ref{thm_BB_HH_stable}). This is done in two stages. First, we prove that bigraded persistent homology satisfies the stability property with respect to a modified Gromov--Hausdorff distance, in which the infimum is taken over bijective correspondences only (Theorem~\ref{thm_BB_stable}). Second, the stability for bigraded persistent double homology is established using the invariance of double homology with respect to the doubling operation on simplicial complexes.
...
% for an example of a barcode. We notice that $H_i(\mathcal{C})=\bigoplus_{r\geq 1}H_i(C_\ast^r)$. 
% \begin{figure}
% \begin{tikzpicture}
% \draw[<->] (0,4)--(0,0)--(5,0);
% \draw[dotted] (0,1)--(5,1); \node[left] at (0,0.5) {$H_0(\mathcal{C})$};
% \draw[dotted] (0,2)--(5,2); \node[left] at (0,1.5) {$H_1(\mathcal{C})$};
% \draw[dotted] (0,3)--(5,3); \node[left] at (0,2.5) {$H_2(\mathcal{C})$};
% \node[left] at (-0.5,3.5) {$\vdots$};
% \node[right] at (5,0) {persistence};
% \node[above] at (0,4) {$H_\ast(\mathcal{C})$};

% \draw[thick, blue]  (0,0.1)--(1,0.1);
% \draw[thick, blue]  (0,0.2)--(1,0.2);
% \draw[thick, blue]  (0,0.3)--(1.5,0.3);
% \draw[thick, blue]  (0,0.4)--(1.5,0.4);
% \draw[thick, blue]  (0,0.5)--(2,0.5);
% \draw[thick, blue]  (0,0.6)--(3,0.6);
% \draw[thick, blue]  (0,0.7)--(3.5,0.7);
% \draw[thick, blue]  (0,0.8)--(4,0.8);
% \draw[thick, blue]  (0,0.9)--(4.5,0.9);

% \draw[thick, blue]  (1,1.2)--(3,1.2);
% \draw[thick, blue]  (2,1.4)--(3,1.4);
% \draw[thick, blue]  (1.5,1.6)--(3.5,1.6);
% \draw[thick, blue]  (3,1.8)--(4,1.8);

% \draw[thick, blue]  (1,2.2)--(3,2.2);
% \draw[thick, blue]  (2,2.5)--(3.5,2.5);
% \draw[thick, blue]  (2.5,2.8)--(4,2.8);
...
% \end{figure}
%
%We notice that intervals $[a, \infty)$ in $B(\mathcal{M})$ corresponds to the free part of \eqref{eq_alg_persistence} and the intervals $[a,b)$ with $b< \infty$ correspond to the torsion part.
...
We define the \emph{bigraded persistent homology} module of bidegree $(-i,2j)$ as 
\[
...
Let $(X, d_X)$ be a finite metric space. A point $x\in X$ is an \emph{outlier} if for any $y\ne x$ holds the inequality
\[
  d_X(x,y)\geq \max_{y'\neq x} d_X(y, y'). 
\]
\end{definition}

% \red{Taras: Original Don's definition was as follows. A point $x\in X$ is an \emph{outlier} if for any $y\ne x$ holds the inequality
% \[
%   d_X(x,y)\geq \max_{y,y'\neq x} d_X(y, y'). 
% \]
% Equivalently, $d(x,y)\ge\mathop{\mathrm{diam}(X\setminus x)}$. This condition is stronger than the one in Definition~~\ref{def_outlier}. However, Proposition below holds with Defintion~\ref{def_outlier}.}


\begin{proposition}\label{HHout}
Suppose a finite metric space $(X, d_X)$ has an outlier. Let $D$ be the diameter of $(X, d_X)$, and let $R(X,t)$ be the Vietoris--Rips complex associated with $(X, d_X)$. Then, 
\[
...
The \emph{bigraded persistent double homology} module of bidegree $(-i,2j)$ is 
\[
...
\begin{example}
Suppose $(X,d_X)$ has an outlier. Then $\BDB(X)$ consists of an infinite bar $[0,\infty)$ at bidegree $(0,0)$ and a bar $[0,D)$ at bidegree $(-1,4)$, by Proposition~\ref{HHout}.
\end{example}
...
The following construction plays a key role in proving the stability property of persistent double homology.
...
When using field coefficients, we also get the stability result for the bigraded barcodes using Theorem~\ref{prop_isometry}:
...
% \blue{(Jongbaek: I have not changed anything written below.)}
% \section{Stability for $\PHH^\ast(\mathcal{Z}_K)$}
% % \subsection{Doubling}

% \begin{definition}\label{def_doubling_orig}
% Let $K$ be a simplicial complex on $S$ and $x\in S$. Then the double of $K$ at $x$ is the complex $K^x$ on $S^x=S\cup\{x'\}$
% whose simplices are $\sigma$ where $\sigma \in K$ and for $x\in\sigma \in K$, $\sigma\cup \{x'\}$ and 
% $\sigma\cup \{x'\} \setminus \{x\}$. 
% \end{definition}

% \begin{lemma}\label{lem_HHZK_HHZKprime}
% $HHZK\cong HHZK^x$
% \end{lemma}

% \begin{proof}
% Consider the chain map $\phi\colon CHZK^x\cong \oplus_{L\subset S} \tilde H(K^x_L)\rightarrow
% CHZK\cong \oplus_{L\subset S} \tilde H(K_L)$, determined by the property that for 
% $\alpha \in\tilde H(K^x_L)$
...
% First observe that $\phi$ is a chain map. We will show that it is a weak equivalence. It is easy to see that $\phi$ is surjective, so it is enough to show that $\ker \phi$ is acyclic. 

% Consider the following subsets of the power set ${\mathcal P}(S^x)$, 
% the subsets that contain $x'$ and not $x$, 
% ${\mathcal S'}=\{ L\subset S^x| x'\in L, x\not\in L\}$ and the subsets that contain both $x$ and $x'$,
% ${\mathcal S''}=\{ L\subset S^x| x, x'\in L\}$.
% Observe that 
% $\ker \phi\cong \oplus_{L\in{\mathcal S'}\cup{\mathcal S''}} \tilde H(K^x_L)$. 

% $T_n=\{L\in {\mathcal S'}\cup{\mathcal S''}| |L\setminus \{x\}|\geq n\}$

% Put a filtration on $\ker \phi$.

% $$
% F_n=\oplus_{L\in T_n} \tilde H(K^x_L).
% $$

% Observe that the chain complex $F_n/F_{n-1}$ is isomorphic to a direct sum over all 
% $L\in {\mathcal S'}$, $|L|=n$, $\tilde H(K^x_L)\rightarrow \tilde H(K^x_{L\cup\{x\}})$. 

% Note all of these maps are isomorphisms since the inclusions  $K^x_L\rightarrow K^x_{L\cup\{x\}}$ are homotopy equivalences. Hence the $\ker \phi$ is acyclic and so $\phi$ is an equivalence and the lemma follows. 
% \end{proof}


% \begin{itemize}
%     \item Add the stability theorem of $\HH^\ast(\mathcal{Z}_K)$.
% \end{itemize}


% \section{Sampling}
% (``Maybe this is more rigorous definition of a bigraded barcode...? Notation seems bad...')
% \begin{definition}
% Let $K^0$ be a point-cloud in $\RR^n$ and $\mathcal{K}$ the corresponding Vietoris--Rips complex. The bigraded barcode of $K^0$ is set consisting of pairs of half-open interval and a position of Figure \ref{fig_bi_Betti}:
% \begin{equation}\label{eq_bigraded_barcodes_rigorous}
% BB(K^0)\colonequals \left\{\left( [b_k, b_k+c_k), (-i, 2j)\right) \mid 0 \leq k \leq \beta^{-i, 2j}(K) \text{ and } i,j \geq 0\right\},
% \end{equation}
% where the collection $\{[b_k, b_k+c_k) \mid 0 \leq k \leq \beta^{-i, 2j}(K) \}$ of half-open intervals  for fixed $(-i, 2j)$ represents the persistences of the generators in $H^{-i, 2j}(\mathcal{Z}_{K^{b_k}})$ which are born at $\mathcal{Z}_{K^{b_k}}$. 
% \end{definition}

% Suppose we take a sample $A^0$ from the given point-cloud $K^0$, and consider the bigraded persistent homology. Then, we have the following proposition. 
% \begin{proposition}
% The bigraded barcode $BB(A^0)$ of $A^0$ is a subset of $BB(K^0)$. 
% \end{proposition}
% \begin{proof}
% Let $\mathcal{K}=(K^r)_{r\geq 0}$ be the Vietoris--Rips complex of $K^0$. Since $A$ is a subset of $K^0$, for each index $s\geq 0$ of the Vietoris--Rips complex $\mathcal{A}=(A^s)_{s\geq 0}$ of $A$, there exist $r_s\geq 0$ such that the following diagram commutes
% \[
% \begin{tikzcd}
% K^0 \arrow[hook]{r} & \cdots \arrow[hook]{r} & K^{r_1}  \arrow[hook]{r} & \cdots  \arrow[hook]{r} & K^{r_2}  \arrow[hook]{r} & \cdots \\
% A^0  \arrow[hook]{u} \arrow[hook]{rr} &&A^1  \arrow[hook]{u}  \arrow[hook]{rr} && A^2  \arrow[hook]{u} \arrow[hook]{r} & \cdots .
% \end{tikzcd}
% \]

% Assume $K^0$ consists of $m$ points. Then, $A$ is a subset of $[m]$. Let $a$ be the cardinality of $A$. Since Vietoris--Rips complex consists of flag complexes, $A^{s}$ is a full subcomplex of $K^{r_s}$. Hence, for each $s$, $H^{-i, 2j}(\mathcal{Z}_{A^{s}})$ is a direct summand of $H^{-i, 2j}(\mathcal{Z}_{K^{r_s}})$. Hence, the result follows. 
...
% \section{questions}
% \begin{enumerate}
% \item Are there any two data sets which are more easily distinguished by bigraded barcodes than ordinary barcodes? 
% \item What do we mean by \emph{(Massey) product} in bigraded persistent (co)homology?
% \item How to apply Baskakov's product in persistent (co)homology?
% \item Is there anything we can say about Data set by product structure? 
% \item Any possibility to apply the result of \cite{CZ} to \eqref{eq_simplicial_inclusions}? 
% \item[] $\vdots$
% \end{enumerate}
...
Limonchenko, Ivan; Panov, Taras; Song, Jongbaek; Stanley, Donald. \emph{Double cohomology of moment-angle complexes}. Preprint (2021);
arXiv:2112.07004.

\bibitem[LFLX]{l-f-l-x23}
Liu, Xiang; Feng, Feng; L\"u, Zhi; Xia, Kelin. \emph{Persistent Tor-algebra for protein-protein interaction analysis}. Briefings in Bioinformatics, 2023; bbad046, https://doi.org/10.1093/bib/bbad046
