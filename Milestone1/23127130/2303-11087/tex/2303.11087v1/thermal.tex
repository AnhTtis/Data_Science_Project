...
\end{figure}
To validate the accuracy of the hybrid coupling method developed, we consider a two-dimensional domain with $N_{x}^{\left(c\right)}=20$ and $N_{y}^{\left(c\right)}=1$ (Figure~\ref{fig:setup-diag}). Table~\ref{tab:unit-cell-geom-params} summarizes the parameters used to define the geometry of the unit cell. Periodic boundary conditions were applied top and bottom, whereas zero gradient boundary conditions were applied to the left and right boundaries of the domain. To model a battery thermal runaway problem, we modify the power flux source term in fine-scale equations as 
\begin{align}
...
\end{cases}
\end{align}
where $\mathbf{x}_{\text{burn}}$ is the location that separates the burning and unburned regions, $\Pi_{\text{NB}}\left(T_{\epsilon}^{\left(c\right)}\right)$ and $\Pi_{\text{FB}}\left(T_{\epsilon}^{\left(c\right)}\right)$ are the dimensionless fine-scale power flux source term for unburned and burning battery cell such that
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
For upscaled equations, we use a sigmoid function to calculate the power flux source term to avoid discontinuity such that
\begin{align}
\overline{\Pi} \left(\langle T^{\left(c\right)} \rangle_{Y}, \mathbf{x}\right) &= \ddfrac{ \overline{\Pi}_{\text{FB}} \left(\langle T^{\left(c\right)} \rangle_{Y}, \mathbf{x}\right) -  \overline{\Pi}_{\text{NB}} \left(\langle T^{\left(c\right)} \rangle_{Y}, \mathbf{x}\right)}{1+\exp\left(180(\mathbf{x} - 
...
\end{align}
where $\overline{\Pi}_{\text{NB}} \left(\langle T^{\left(c\right)} \rangle_{Y}, \mathbf{x}\right)$ and $\overline{\Pi}_{\text{FB}} \left(\langle T^{\left(c\right)} \rangle_{Y}, \mathbf{x}\right)$ are the dimensionless upscaled power flux source term for unburned and burning battery cells that are defined as
\begin{subequations}
\begin{align}
...
\begin{align}
    \hat{\Pi}_{\text{burn}} = 
    \begin{cases}
    \hat{\Pi}_{\text{burn},\text{low}}=\hat{\Pi}_{\text{burn},0}, \text{ for } \mathbf{x} > \mathbf{x}_{FT}, \\
    \hat{\Pi}_{\text{burn},\text{high}}=10\hat{\Pi}_{\text{burn},0}, \text{ for } \mathbf{x} \le \mathbf{x}_{FT},
    \end{cases}
\end{align}
such that
\begin{align}
...
\end{align}
Incorporating the effect of $\hat{\Pi}_{\text{burn},\text{low}}$ and $\hat{\Pi}_{\text{burn},\text{high}}$, the dimensionless number $FT$ is defined as
\begin{align}
&\mathcal{R}(\mathbf{x}) = \left(\ddfrac{FT_{\text{high}} + FT_{\text{low}} }{2}\right) - \left(\ddfrac{FT_{\text{high}} - FT_{\text{low}} }{2}\right)\tanh\left(100(x-\mathbf{x}_{\mathcal{R}}) \right),
\end{align}
...
Temperature ranges over with $\hat{\Pi}_{\text{base}}(\hat{\mathbf{x}})$, $\hat{T}_a$ [\si{\Theta}] & 0\\
Temperature ranges over with $\hat{\Pi}_{\text{burn}}$,  $\hat{T}_b$ [\si{\Theta}] & 0\\
...
\caption{The centerline ($y=0$) average cell temperature computed at (a) $t=0.02$ and (b) $t=0.20$ with a time step size $\Delta{t}=3.15 \times 10^{-5}$ for the pore scale, hybrid with Taylor and Series expasnion, respectively, and upscaled simulations.}
...
\caption{Average cell temperature computed at $t=0.02$ with a time step size $\Delta{t}=3.15 \times 10^{-5}$ for hybrid simulations with Taylor expansion with $x_{dist}=$ (a) $0.0\epsilon$, (b) $1.0\epsilon$, (c) $1.5\epsilon$, (d) $3.0\epsilon$ and (e) $4.5\epsilon$. }
...
\caption{Average cell temperature computed at $t=0.20$ with a time step size $\Delta{t}=3.15 \times 10^{-5}$ for hybrid simulations with Taylor expansion with $x_{dist}=$ (a) $0.0\epsilon$, (b) $1.0\epsilon$, (c) $1.5\epsilon$, (d) $3.0\epsilon$ and (e) $4.5\epsilon$.}
...
\caption{Average cell temperature computed at $t=0.20$ with a time step size $\Delta{t}=3.15 \times 10^{-5}$ for hybrid simulations with Series expansion with $x_{dist}=$ (a) $0.0\epsilon$, (b) $1.0\epsilon$, (c) $1.5\epsilon$, (d) $3.0\epsilon$ and (e) $4.5\epsilon$.}
...
\caption{The centerline ($y=0$) average cell temperature computed at (a) $t=0.02$ and (b) $t=0.20$ with a time step size $\Delta{t}=3.15 \times 10^{-5}$ for hybrid simulations with Series expansion with $x_{dist}=0.0\epsilon$, $1.0\epsilon$, $1.5\epsilon$,  $3.0\epsilon$ and $4.5\epsilon$.}
...
Upscale subdomain minimum grid resolution $h_{up,min}$  &  $1.00 \times 10^{-2}$\\
fine-scale subdomain minimum grid resolution $h_{pore,min}$  &  $2.50\times10^{-4}$\\
