...
We present the holography of information in terms of  a precise mathematical result.  However this does  {\em not} imply that a physical observer with access only to a small patch of the late-time slice can glean all information about the state using local measurements.  Cosmological correlators are gauge-fixed observables that are merely labelled by a set of points. Since there are no local gauge-invariant observables in the theory, cosmological correlators also secretly correspond to nonlocal operators that cannot be measured through any strictly local process. 

Moreover, in dS, it is not fruitful to think in terms of external observers and so the question of what is physically observable requires us to construct a model of an observer who is part of the system.  Although, we do not seek to construct such a model in this paper, it is reasonable to envisage a model in which a physical observer can access low-point gauge-fixed observables of the kind we describe. But, as in AdS and in flat space, the identification of a sufficiently complicated state, $\reg$ from a small region requires very high-point cosmological correlators and presumably, in any reasonable physical model, such high-point correlators are effectively inaccessible.
...
(\Psi_1,A \Psi_2) = \sum_{n,m,n',m'}  \kappa^{n+n'} \big \langleQ \overline{\dcoeff[n,m]^* A[g, \chi] \dcoeff[n',m']} \big \rangleQ~,
\ee
...
This leads us to a remarkable result: if one is given the cosmological correlators \eqref{cosmcorrsumm} in an arbitrarily small region, then this is sufficient to determine the correlators everywhere. This means that knowledge of cosmological correlators in an arbitrarily small region is sufficient to completely specify any pure state of the theory. For any region $\reg$
\be
 \llangle \Psi_1 | \cprod^{p,q}_{\vI\vJ}(\vec{x}) | \Psi_1 \rrangle_{\text{CC}} =  \llangle \Psi_2 | \cprod^{p,q}_{\vI\vJ}(\vec{x}) | \Psi_2 \rrangle_{\text{CC}}, ~~\forall \vec{x} \in \reg~\text{and}~\forall p,q \implies |\Psi_1 \rangle = |\Psi_2\rangle~.
\ee
...
In the literature, cosmological correlators are usually computed as QFT-expectation values of the form $\langle \chi(x_1) \ldots \chi(x_n) \rangle_{\text{QFT}}$, where $x_i$ are points on the late-time boundary of de  Sitter.   In a quantum field theory, the meaning of such correlators is clear.  However, in a theory of quantum gravity, the product of local operators on the late-time slice does not commute with the constraints and so is not gauge invariant.
...
\label{actionsct}
\delta_{\xi} \chi = \xi^{i} \partial_i \chi; \qquad \delta_{\xi} g_{i j} = (P \xi)_{i j}~,
\ee
...
\begin{result}
The set of all cosmological correlators in any open region $\reg$ in a state $\Psi$  is sufficient to uniquely identify the state. 
\end{result}
If the theory is in a mixed state, the set of cosmological correlators in $\reg$ is sufficient to determine the density matrix of the theory.
...
%\paragraph{\bf Generic need for high-point correlators.}
...
So the natural question is whether there is some way of understanding bulk dynamics in all of de Sitter space from a lower-dimensional subregion on the late-time slice. Similar ideas were recently explored in \cite{Moitra:2022glw}.
...
\be
v^j (x) = \int{d^dp\/(2\pi)^d} \,{1\/p^2}\le( -\d_{ij} +{2(d-2)\/d-1}{ p_ip_j\/k^2} \ri) e^{i px}\hat{s}^i(p)~,
\ee
