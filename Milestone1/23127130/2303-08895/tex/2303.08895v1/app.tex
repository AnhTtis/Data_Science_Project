\appendix
In this appendix, we give a brief introduction to the constrained optimal control problem. It is within this context that the rollout scheme, as well as Newton's step interpretation of the method, is developed. Then we show how the mixed integer program considered here can be transformed as an equivalent optimal control problem involving only discrete variables, thus proving Prop.~\ref{prop:rollout_mixed_int}. We also provide additional variants of the proposed scheme, and connect it to a classical method within the optimal control context. The majority of the proof arguments and transformation are adopted from \cite{bertsekas2005rollout} and \cite[Section~3.4]{bertsekas2020rollout}. We also provide further details on the optimal control problem, including the sufficient conditions for its feasibility, and demonstrate procedure through which the linear constraints can be linearized.
...
where $x_k$ and $u_k$ are the state and control at time $k$, and belong to some sets $X_k$ and $U_k$ that contain finite elements, respectively, and $f_k$ is some function. Each control must be chosen from a finite constraint set $U_k(x_k)$ that depends on the current state. We are interested in policies $\pi=\{\mu_0,\mu_1,\dots,\mu_N\}$, which is a sequence of functions $\mu_k:X_k\mapsto U_k$ such that $\mu_k(x_k)\in U_k(x_k)$ for all $x_k$. The set of all the policies is denoted as $\Pi$. 

When at $x_k$ and applying $u_k\in U_k(x_k)$, there is a stage cost $g_k(x_k,u_k)\in \Re\cup\{\infty\}$. In addition, there is an additional terminal cost $g_N(x_N)\in \Re\cup\{\infty\}$ for being $x_N$ at $N$th stage. For a policy $\pi$, starting from $x_k$, the total cost accumulated over $N-k$ stages plus the terminal cost are denoted as $J_{k,\pi}(x_0)$, i.e.,
$$J_{k,\pi}(x_k)=g_N(x_N)+\sum_{i=k}^{N-1}g\big(x_i,\mu_i(x_i)\big),$$
where $x_{i+1}=f\big(x_i,\mu_i(x_i)\big)$, $i=k,\dots,N-1$. For brevity, we denote $J_{0,\pi}$ as $J_\pi$. Within this context, one hopes to find the optimal cost $J^*$ such that
...
\begin{equation}
    \label{eq:rollout_op}
    \Tilde{\mu}_k(x_k)\in\arg\min_{u_k\in U_k(x_k)}\Big\{g(x_k,u_k)+J_{\Bar{\pi},k+1}\big(f_k(x_k,u_k)\big)\Big\},
\end{equation}
where $x_{k+1}=f_{k}\big(x_k,\Tilde{\mu}_k(x_k)\big)$, $k=0,\dots,N-1$. The rollout policy is no worse than the base policy, as is given in the following proposition, which is adopted from \cite[Prop.~3.3.1]{bertsekas2020rollout}.
...
\end{proposition}
In fact, rollout can be interpreted as one step of Newton's method for computing $J^*(x_0)$. Extensive discussions on rollout that goes well beyond the context considered here can be found in \cite{bertsekas2020rollout,bertsekas2022lessons}. 

\subsection{Mixed Integer Problem and Rollout}\label{app:b}
We now show that the mixed integer problem \eqref{eq:mixed_int} can be transformed as an equivalent optimal control problem discussed above, and the algorithm described in Section~\ref{Section IV} is the rollout method applied to this equivalent problem. Based upon this transformation and Prop.~\ref{prop:rollout_original}, we are ready to prove Prop.~\ref{prop:rollout_mixed_int}.

\begin{proof}[Proof of Prop.~\ref{prop:rollout_mixed_int}]
We define a fictitious state `null' as the initial state $x_0$ so that $X_0$ is the singleton $\{\text{`null'}\}$. Moreover, the set $X_1$ is defined as $U_0$, and for $k=1,\dots,N-1$, the sets $X_{k+1}$ are defined recursively as $X_{k+1}=X_k\times U_k$. As a result, the set $X_N$ containing all terminal states equals $U$. The control constraint sets are independent of states, i.e., $U_k(x_k)=U_k$ for all $x_k\in X_k$ and $k$.
...
namely concatenating the control $u_k$ to the current state $x_k$. The stage costs associated with all the state-control pair $(x_k,u_k)$ are $g(x_k,u_k)\equiv 0$, while the terminal cost is
...
Therefore, our scheme has the character of an anytime algorithm. However, in the case where $k\!<\!N$, Newton's step interpretation described in Remark~\ref{rmk:newton} is not valid anymore.

\subsection{Sufficient Conditions for the Feasibility of Charging Problem}\label{app:e}
We provide a set of conditions under which the timid solution is feasible. Some of the conditions discussed here are restrictive. Still, they may be taken as a starting point for the construction of the base solution.

For the battery parameters, it is natural to assume that 
\begin{align*}
e_o\geq& e_s+\Bar{P}(\tau_0+d_0),\\
...
d_{N-1}+\tau_N\leq& T_d.
\end{align*}
To ensure that the timid solution is feasible, the total driving time involving all detours needs to be no more than $\Bar{T}_d$, i.e.,
\begin{equation}
    \label{eq:sufficient_Td_constraint}
    \sum_{k=0}^{N}\tau_k+\sum_{k=0}^{N-1}2d_k\leq{\bar{T}_d}.
\end{equation}
Regarding the deadline, we may require that
\begin{equation}
...
With the above transformation, the mixed integer optimal charging problem becomes one with linear constraints and costs. However, if the linear approximation \eqref{eq:charging} for charging process $\Delta e_k$ is replaced by more accurate functions that are nonlinear in $t_k$, then such transformation does not lead to any simplifications. Moreover, even for the mixed integer linear program, the iterations needed for exact solution may still be exponential.