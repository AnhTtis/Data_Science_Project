...
%\autoref{table:overal_evaluation} summarizes our evaluation results on gradient-based attacks $\ell_\infty (\epsilon = 8/255)$ on CIFAR-10. For all experiments, we report the lowest robust accuracy among three different attacks. The result includes the originally reported robust accuracy and our evaluation results. Our results indicate that all the robustness of the defenses is overestimated. Moreover, some techniques in their original evaluations significantly affect the attack success rate. Through the following three research questions, we discuss how to improve the evaluation of diffusion-based purifications.


\textbf{Surrogate process and its gradient.}
...
%Assume that the original diffusion process recovers the input data from the noisy version via $T$ steps of denoising processes.
...
\input{tables/RQ1}
\textbf{RQ1: Is the adjoint method the best way to generate adversarial examples with full gradients?} 
...
\autoref{table:RQ1} shows that the robust accuracy of DiffPure with the direct back-propagation is 51.25\% on PGD+EOT attack, which is 26.55\% lower than the reported accuracy with the adjoint method \citep{Nie2022DiffusionMF}. The results show that direct back-propagation is more effective than the adjoint method. Furthermore, we use a surrogate process for GDMP~\citep{Wang2022GuidedDM}, and the robust accuracy is 30.82\%, which is 45.9\% lower than the reported accuracy against the BPDA attack. It can be concluded that, in cases where the gradients of the defense process are unavailable to calculate, the surrogate process can be an alternative to generate adversarial examples.

SODEF, a non-diffusion-based purification, originally uses the adjoint method to generate adversarial examples. We evaluate SODEF with direct back-propagation for the attack and observe 49.28\% robust accuracy against AutoAttack, which is lower than 53.69\% of the underlying model without defense \citep{kang2021stable}. This result suggests that the use of a numerical solver would not be effective for the adversarial attack.% that the proposed ODE block may not help improve the robustness. % The results from DiffPure and SODEF allow us to conclude that the adjoint method might be one of the reasons for the gradient obfuscation.

\input{tables/RQ2}
\textbf{RQ2: Is AutoAttack still better than the other gradient-based attacks, such as PGD, when the alternative is available?}
AutoAttack has recently been used as a standard method to evaluate defenses due to its robustness against defenses. However, as shown in \autoref{table:RQ2}, AutoAttack has a lower success rate than PGD+EOT against diffusion-based purification methods. For the $\ell_\infty$ threat model $(\epsilon = 8/255)$, PGD+EOT shows 12.35\% and 22.26\% more attack success rate than AutoAttack against DiffPure and ADP, respectively. We observe a similar result with the $\ell_2$ threat model $(\epsilon = 0.5)$ with ADP, and the difference between PGD+EOT and AutoAttack is insignificant with DiffPure. Therefore, evaluation with PGD+EOT for diffusion-based purification can be useful to evaluate their robustness. Further results of the difference between PGD+EOT against DiffPure with additional settings can be found in \autoref{app:evaluation}.

\input{tables/RQ3}
\textbf{RQ3: Is BPDA still more effective than the best combination of full-gradient attacks?}
BPDA~\citep{obfuscated-gradients} has been widely used to evaluate defenses that can cause gradient obfuscation. Because multiple function calls can cause gradient obfuscation, ADP, GDMP, and DISCO have been evaluated on BPDA as the strongest adaptive white-box attack. However, our evaluation shows that BPDA has a lower attack success rate than the attacks using direct gradients of the defense process, as shown in \autoref{table:RQ3}. Against PGD+EOT using direct gradients of defense process, ADP and GDMP show robust accuracy of 37.27\% and 30.82\%, respectively, significantly lower than the reported accuracy with BPDA~\citep{Yoon2021AdversarialPW, Wang2022GuidedDM}. DISCO even has 0\% robust accuracy. From the results, we suggest that the direct gradients of the defense process need to be tested to check the robustness. %attack performance, except for some defenses that explicitly cause gradient obfuscation.
