...
To do so, we first add noise to the adversarial example via the forward process with $t^*$ steps. Then, the reverse process recovers the clean example via the denoising process. The choice of the number of forward steps $t^*$ is essential since too much noise can remove the semantic information of the original example, or too little noise cannot remove adversarial perturbation. 
In theory, as we add more noise to the adversarial example, the distributions over the noisy adversarial example and the true example becomes close to each other~\citep{Nie2022DiffusionMF}. Therefore, the denoised examples are likely to be similar.
%Although one can purify the images without having the forward process, in practice, adding the forward process results in a more robust defense since the structure of adversarial noise is not trained via the denoising process~\citep{Yoon2021AdversarialPW}.
