...
\author{Franquiz Caraballo Alba}
...
\section{Background} \label{sec:background}
    ...
    Let $A$ be an $n$ dimensional vector space over $\field$. Since $\Aut(\p A)$ is $\PGL(A)$ and $\GL(A)$ embeds into $\End (A)$ as the complement of the zero locus of the determinant, $\Aut(\p A)$ embeds into $\p \End(A)$. For the rest of the paper, we abuse notation in the following sense. Given $\phi \in \p \End (A)$, let $\hat \phi$ be a representative of $\phi$. We denote by $\p \ker \phi$ the subspace $\p \ker \hat \phi$. Every $\phi \in \p \End(A)$ determines a rational map $\p A \dashrightarrow \p A$ that is regular away from $\p \ker \phi$, we call this map $\phi$. We call $\overline{\phi(X)}$ the \textit{image} of $X$ by $\phi$ for a subvariety $X \not\subset \p \ker \phi$. Note that $\phi \in \PGL(A)$ if and only if $\p \ker \phi = \emptyset$ if and only if $\im \phi = \p A$.

    \subsection{Point Conditions}\label{sec:point_conditions}
        Let $X$ be a degree $d$ hypersurface in $\p A$ and let $f \in \Sym^d(A^\vee)$ be a generator of its homogeneous ideal. Note that this choice is unique up to scalar, so choosing this hypersurface is equivalent to choosing a point in $\p \Sym^d(A^\vee)$. Furthermore, given $\phi \in \PGL (A)$, the ideal generated by $f(\hat \phi(\_))$ in $\Sym^d(A^\vee)$ is independent of the choice of representative $\hat \phi$ of $\phi$. Let $X \circ \phi$ denote the hypersurface defined by $f(\hat \phi(\_))$.
        
        A straightforward check shows that for arbitrary elements $\phi,\psi$ of $\PGL(A)$ $(X \circ \phi) \circ \psi = X \circ (\phi \circ \psi)$, making this a right action of $\PGL(A)$ on $\p \Sym^d(A^\vee)$. Now, fix the hypersurface $X$. The action of $\PGL(A)$ on $\p \Sym^d(A^\vee)$ defines a regular map $\PGL(A) \to \p \Sym^d(A^\vee)$ by $\phi \mapsto X \circ \phi$. This regular map extends to a rational map on each compactification of $\PGL(A)$. In particular, it defines a rational map $\alpha$ from $\p \End(A)$ to $\p \Sym^d(A^\vee)$. Concretely, given $\phi \in \p \End(A)$, if $f(\phi(\_))$ is not identically zero, then $\alpha(\phi)$ is the hypersurface defined by $f(\phi(\_))$ and undefined otherwise. This map and its (partial) resolutions are central to our approach.

       Every $q\in \p A$ determines a hyperplane $H_q$ in $\p \Sym^d(A^\vee)$, consisting of the hypersurfaces $X$ containing $q$. Then, $\alpha(\varphi)$ belongs to this hyperplane if and only if $f(\phi(q))=0$. As an equation in $\phi$, $f(\phi(q))$ defines a degree $d$ hypersurface $P_q$ in $\p \End(A)$, which we call the \textit{point condition corresponding to $q$}. Since the set of hyperplanes $H_q$ spans the dual of $\p \Sym^d(A^\vee)$, the set of point conditions $P_q$ spans the linear system defining the rational map $\alpha$.
            
        Since the collection of the point conditions spans the linear system defining $\alpha$, the base scheme of $\alpha$ is the (scheme-theoretic) intersection of the point conditions $P_q$. Therefore, $\phi \in \p \End(A)$ is in the base locus of $\alpha$ if and only if $f(\phi(q)) = 0$ for all $q \in \p A$. In other words, $\phi$ is in the base locus of $\alpha$ if and only if $\im \phi \subset X$. We record this and the fact that the closure of the image of alpha is the closure of the orbit of $X$ in the following proposition, stated without proof.
        
        \begin{proposition}\label{prop:base_scheme}
            The base locus of $\alpha$ is
            $B_0 := \{\phi \in \p \End(A) | \im \phi \subset X\}$. Furthermore, the closure of the image 
            of $\alpha$ is the closure of the orbit of $X$ in $\Sym^d(A^\vee)$ under the action of $\PGL(A)$.
        \end{proposition}

        \begin{proposition}\label{prop:tangent_point_condition}
            Let $q \in \p A$ and $\phi \in \p \End(A)$. If $q \notin \p \ker \phi$ and $X$ is smooth at $\phi (q)$, then for an arbitrary $\psi \in \p \End(A)$, $\psi$ is in the embedded tangent space to $P_q$ at $\phi$ if and only if $q \in \p \ker \psi$ or $\psi(q)$ is in the embedded tangent space to $X$ at $\phi(q)$.
    	\end{proposition}
        \begin{proof}
             Let $\psi \in \p \End(A)$, $q \in \p A$ and $\phi \in P_q$ be arbitrary. Let $\hat \psi $ be a representative of $\psi$, $\hat \phi$ be a representative of $\phi$ and $v$ a representative of $q$.
            
            With this notation, $\psi$ is in the embedded tangent space to $P_q$ at $\phi$ if and only if $\nabla s_q(\hat \phi) \cdot \hat \psi  = 0$, where $s_q = f(\_(v))$. Then, using the chain rule, we see that $\nabla s_q(\hat \phi) \cdot \hat \psi = \nabla f(\hat \phi v) \cdot \hat \psi v$. Thus, expanding the expression, $\nabla s_q(\hat \phi) \cdot \hat \psi  = 0$ if and only if:
    ...
    \subsection{The Predegree Polynomial}\label{sec:predegree_polynomial}
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        Note that $\im p_2$ is the closure of the orbit of $X$ under the action of $\PGL(A)$, which we denote by $\overline{\orb(X)}$. Also note that $\dim \overline{\Gamma} = \dim \p \End(A) = n^2+2n$.
       ...
       In \cite{aluffiFaberPoints}, Aluffi and Faber defined the \textit{predegree} of the orbit closure of a hypersurface $X$ to be the degree of the orbit closure of $X$ times the degree of the stabilizer of $X$ because it was the natural output of the intersection-theoretic computation preliminary to the computation of the degree of the orbit closure. The predegree polynomial is called so because it is a direct generalization of the intersection-theoretic information. For an interpretation of the coefficients of the predegree polynomial, see Corollary \ref{cor:interpretation}.
        ...
        The definition of $c_i$ reflects the informal question posed at the beginning of this section. This comes from the fact that, given $q \in \p A$, $H_q$ is the hyperplane of $\p \Sym^d(A^\vee)$ consisting of the hypersurfaces $X$ such that $q \in X$. Then, $\widetilde{P_q} = \widetilde{\alpha}^{-1}(H_q)$ consists of the matrices sending $X$ to hypersurfaces containing $q$, so $Y_i$ consists of the matrices which send $X$ to a hypersurface containing the points $q_1, \dots, q_i$. Then, the degree of $Y_i$ is the number of matrices $\phi$ subject to $n^2+2n-i$ general linear conditions such that $\phi(X)$ contains $i$ general points.
         ...
         \begin{proposition}\label{prop:multiplicities}
            Let $L$ be a general dimension $i$ linear subspace of $\p \End(A)$ and let $X'$ be a translate of $X$ by an element of $\PGL(A) \cap L$. If $i < \dim \orb(X)$, there exists a unique $\phi \in L \cap \PGL(A)$ such that $X' = X \circ \phi$. If $i = \dim \orb(X)$, then there are $\deg \overline{\stab(X)}$ elements $\phi \in L$ such that $X'= X \circ \phi$.
        \end{proposition}
        \begin{proof}
            Assume that $i < \dim \orb(X)$ and, WLOG, assume that $X' = X$. Since $X$ is a translate of $X$ by an element of $L \cap \PGL(A)$, there exists some $\phi \in L$ such that $X \circ \phi = X$. Therefore, $\phi \in \overline{\stab(X)} \cap L$. By the orbit stabilizer theorem, $\overline{\stab(X)}$ has codimension $\dim \orb(X)$ and thus cannot intersect $L$ at more than one point. Thus $\phi$ is unique.
        ...
        \begin{corollary}\label{cor:interpretation}
            ...
            \begin{itemize}
                \item If $i < \dim \orb(X)$, then $a_i$ is the number of $L$-translates of $X$ containing $i$ general points, where $L$ is the intersection of $\PGL(A)$ and a general $i$ dimensional linear subspace of $\p \End(A)$.
    ...
    \subsection{Segre Classes}\label{sec:segre_classes}
        ...
        A central object in the study of intersection theory is the Segre class of a proper subvariety of an ambient variety. It is defined in \S~4.2 of \cite{fulton} and plays an important role in the definition of the intersection product of varieties.
        
        Let $s(B,\p\End(A))$ be the Segre class in $\p\End(A)$ of the base scheme $B$ of the rational map $\alpha$ determined by $X$, see \S\ref{sec:point_conditions}. The coefficients of the predegree polynomial of $X$ can be expressed directly in terms of the degrees of the components of this Segre class.
         Define:
        ...
        \begin{remark}\label{rem:independent_codim}
            Since $a_i$ and $a'_i$ can be defined at the generality of an arbitrary rational map $\rho:\p^n \dashrightarrow \p^m$, these results about the multidegrees of the graph of a rational map hold at that level of generality as well. In particular, the $i$th multidegree of the closure $\overline{\Gamma}$ of the graph of a rational map $\rho:\p^n \dashrightarrow \p^m$ is independent of the components of codimension greater than $i$ of the Segre class of its base scheme.
        \end{remark}
        
        \begin{remark}\label{rem:small_modifications}
            Knowing components of codimension $\leq i$ of the Segre class is enough to compute the coefficient of $t^i$ in the predegree polynomial. This allows for small enough modifications to the base scheme, which can make the computation of the coefficients of the predegree polynomial significantly simpler. We make this rigorous in the remainder of this section.
        \end{remark}
        ...
        \end{remark}
        \begin{proof}
            The base scheme $B$ is closed, thus, consider Lemma \ref{lemma:partial_resolution} in the case $C = B$. Then, $U = \p \End(A) \setminus B$ and $g'^*s(B,\p \End(A)) = 0$. Then, $a_i = e_{i,0}$.
                    \end{proof}
...
\section{The Predegree Polynomial of a Quadric in $\p^3$} \label{sec:pp_quadric_general}
    ...
    \subsection{The $n$ Dimensional Case} \label{sec:pp_quadric_n}
        In this section, we compute the leading coefficient of the predegree polynomial of a smooth quadric hypersurface in $\p^n$. Then, we compute the other coefficients for a smooth quadric surface in $\p^3$. This means that $\dim \p A = n$, $d = 2$ and $\dim \p \Sym^d(A^\vee) = \frac{(n+2)(n+1)}{2} - 1$. We choose homogeneous coordinates $x_0,\dots,x_n$ on $\p A$.
        
        Up to a choice of coordinates, a smooth quadric is given by the equation $x_0^2+\cdots+x_n^2=0$, the zero locus of the standard quadratic form on $A$. It follows that the linear orbit of a smooth quadric $Q$ is the set of smooth quadrics, and therefore the closure of its orbit is $\p \Sym^2(A^\vee)$; that is, of degree 1. Furthermore, this implies that the stabilizer of a smooth quadric is (up to this choice of coordinates) $\PO(A)$, the image in $\PGL(A)$ of the group of orthogonal transformations of $A$ naturally embeded in $\GL(A)$. There is a natural embedding of $\PGL(A)$ in $\p \End(A)$ and a natural embedding of $\GL(A)$ in $\End(A) \subset \p(\End(A) \oplus \field)$. For the rest of this discussion, the \textit{degree} of a subgroup of $\PGL(A)$ refers to the degree of its closure under this natural embedding in $\p \End(A)$ and the degree of a subgroup of $\GL(A)$ refers to the degree of its closure in $\p(\End(A) \oplus \field)$. The convention for subgroups of $\GL(A)$ matches the one used in \cite{degreeSO}.
        ...
        \begin{proposition}\label{prop:maximal_components}
            The maximal dimension of a component of the base scheme of a general smooth quadric $Q$ in $\p^n$ is 
        ...
        \end{proposition}
        \begin{proof}
            The dimension of the Fano scheme of $k$ dimensional linear subspaces of a smooth quadric $F_{k}(Q)$ is 
            \begin{equation*}
                \left(n - 1 - \frac{3}{2}k\right)\left(k + 1\right).
            \end{equation*}
            Furthermore, if $k < \frac{n}{2}$, then $F_k(Q)$ is irreducible and if $k = \frac{n}{2}$, then it consists of two irreducible connected components (cf.~Theorems 22.13 and 22.14 in \cite{harris}). Note that if $k > k'$ and $n > 1$, then $\dim F_k(Q) > \dim F_{k'}(Q)$.
    
            A dimension count shows that the space of matrices in $\p\End(A)$ mapping to a given $k$ dimensional linear subspace of $\p^n$ is $(n+1)(k+1) - 1$ dimensional. Note that if $k > k' $, then $(n+1)(k+1) > (n+1)(k'+1)$.
    
            Let $G_k$ be a maximal component of $F_k(Q)$. Consider the subbundle $\mathcal{F}_{k}$ of $G_{k} \times \p \End(A) \to G_{k}$ whose fiber over $L \in G_{k}$ is the space of $\phi \in \p \End(A)$ whose image is contained in $L$. 
            \begin{center}
                \begin{tikzcd}
                    & & G_k \times \p \End(A) \arrow[ddll] \arrow[ddrr] & & \\
                    & & \mathcal{F}_{k} \arrow[u,hook] \arrow[dll] \arrow[drr] & & \\
                    G_k & & & & \p \End(A) \\
                \end{tikzcd}
            \end{center}
            
            By Proposition \ref{prop:base_scheme}, the base scheme $B$ of $\alpha$ is supported on the space of $\phi \in \p\End(A)$ such that $\im \phi \subset Q$, so the components of $B_0$, the support of $B$, correspond to the image of the $\mathcal{F}_{k}$ under the projection $\mathcal{F}_{k} \to \p \End(A)$.
    
            The bundle $\mathcal{F}_{k}$ contains an open set $U_{k}$ consisting of maximal rank matrices and that over $U_{k}$ the projection $\mathcal{F}_{k} \to \p \End(A)$ is injective and $\mathcal{F}_{k} \to G_k$ is surjective with fibers of constant dimension. Then, the dimension of the component of $B_0$ corresponding to $\mathcal{F}_{k}$ is the dimension of $U_{k}$. 
            ...
            The dimension of $U_{k}$ is the dimension of $G_k$ plus the dimension of the space of maximal rank matrices whose image is a fixed $L \in G_k$. Thus:
            \begin{align*}
                \dim U_{k,i} = & \dim G_k + \dim \{\phi \in \p \End(A) | \im \phi = L\} \\
        ...
        Using Remark \ref{rem:bezout}, Proposition \ref{prop:degreePO}, Proposition \ref{prop:maximal_components} and \eqref{eq:degreeO}, we get Table \ref{table1} of predegree polynomials. The missing coefficients for the case $n=3$ are computed in Theorem \ref{thm:main_text}. In the case $n = 4$, the dimension of the base scheme is $12$ so the coefficients for $t^{12}$ and $t^{13}$ are unknown and beyond the scope of this paper.
    ...
    \subsection{The $3$ Dimensional Case} \label{sec:pp_quadric}
        ...
        \end{proposition}
        \begin{proof}
            A simple computation shows that the image of an arbitrary $\phi \in \im \sigma_1 $ is a line contained in $Q$, specifically, the line $a$-line determined by $(s_0:s_1)$, so $\im \sigma_1 \subset Z_1$.
        ...
        \begin{equation}\label{eqn:segre_segre}
            c(N)^{-1} = \frac{\Pi^{r}_{i=1} (1+h_i)^{n_1+1}}{(1 + \sum_{i=1}^r h_i)^{m+1}},
        \end{equation}
        ...
        \begin{proposition} \label{prop:base_scheme_open_reduced}
            The base scheme of $\alpha$ is reduced away from $Z_1 \cap Z_2$.
        \end{proposition}
        ...
        \begin{proof}
            ...
            \end{equation*}
            where the $a_{i,j}$ are the homogeneous coordinates on $\p^{15}$. Recall that $B$, the base scheme of $\alpha$ is the (scheme-theoretic) intersection of all point conditions. Then, 
            \begin{equation*}
                \TEmb_{\phi} B = \bigcap_{q \in \p^3} \TEmb_\phi P_q,
            \end{equation*}
            so $\TEmb_\phi B = V(\{\tau_q| q \in \p^3\})$. The ideal generated by $\{\tau_q| q \in \p^3\}$ is also generated by $\{a_{2,0} - a_{3,1}, a_{2,1},a_{2,2},a_{2,3},a_{3,0},a_{3,2},a_{3,3}\}$. Therefore, $\TEmb_\phi B$ is $8$ dimensional and it contains $\TEmb_\phi Z_1$ which is also $8$ dimensional. Thus, $\TEmb_\phi B = \TEmb_\phi Z_1$ and $B$ is reduced over $Z_1 \setminus Z_2$. Analogously, $B$ is reduced over $Z_2 \setminus Z_1$.
        \end{proof}
        ...
        Theorem \ref{thm:main_text} completes the entry for $n=3$ in Table \ref{table1}. Given a general $n$ dimensional linear subspace $L \subset \p^{15}$, denote the set of quadrics $Q'$ such that for some invertible $\phi \in L$, $Q'= Q \circ \phi$. Using this notation, Corollary \ref{cor:interpretation} interprets Theorem \ref{thm:main_text}  as statements of the form `given a general $7$ dimensional linear subspace $L$ of $\p^{15}$ and $7$ points in general position in $\p^3$, there are 112 quadrics $Q'$ in $Q \circ L$ such that $Q'$ contains all $7$ points.' Note that the case for $\dim L = 9$ is special since the translates here are counted with multiplicity $\deg \PO(4)$. These statements are summarized in Table \ref{table:interpretation}.
        \begin{center}
            \begin{table}
                \centering
                \small{
    ...
    \end{proposition}
    \begin{proof}
        Since $C_1 = Z_1 \cap Z_2$, $\widetilde{Z_1} \cap \widetilde{Z_2} \subset E_1$. Note that $\widetilde{Z_i} \cap E_1$ is the image of exceptional divisor in the blow-up of $Z_i$ along $C_1$ through the embedding $\widetilde{Z_i} \xhookrightarrow{} V_1$. This allows for the identification of points in $\widetilde{Z_i} \cap E_1$ with normal directions to $C_1$ that are tangent to $Z_i$; that is, we identify $\widetilde{Z_i} \cap E_1$ with the projectivization of the normal bundle to $C_1$ in $Z_i$, namely, $\N_{C_1} Z_i$. 
    ...
    \begin{proposition}\label{lemma:supp_alpha_1}
         The base locus of $\alpha_1$ is the union of
    ...
    \end{proposition}
    \begin{proof}
        It is clear that $\widetilde{Z_1}$ and $\widetilde{Z_2}$ are contained in the base locus of $\alpha_1$. Since $\alpha_1$ is defined by the linear system consisting of the proper transforms of the point conditions, all that is left to show is that over $C_1$, the intersection of all point conditions is indeed $C_2$.
        ...
        Since the base locus is the (set-theoretic) intersection of all point conditions in $V_1$, a point $\psi$ in the fiber of $E_1$ over $\phi \in C_1$ is in the base locus if it is a direction tangent to all point conditions and normal to $C_1$ at $\phi$; that is, if its representatives are in the fiber of $\mathcal{F}$ over $\phi$ and thus $\psi$ is a point in $C_2$. Therefore the base locus of $\alpha_1$ is $\widetilde{Z_1} \cup \widetilde{Z_2} \cup C_2$.

        Since $C_2 \subset E_1$, $\widetilde{Z_i} \cap C_2 \subset \widetilde{Z_i} \cap E_1$.
        
        Recall from Proposition \ref{prop:first_blow-up} that $\widetilde{Z_1} \cap E_1 = \p \N_{C_1} Z_1$; that is, the projectivization of the normal bundle to $C_1$ in $Z_1$. Thus, $\widetilde{Z_1} \cap E_1 \subset \widetilde{Z_1} \cap C_2$ is equivalent to the statement that, over $C_1$, $\T Z_1$ is a subbundle of $\mathcal{G}$.
        Since $Z_1$ is contained in the base locus of $\alpha$, $\T Z_1$ is a subbundle of $\mathcal{G}$, thus $\widetilde{Z_1} \cap E_1 \subset \widetilde{Z_1} \cap C_2$.
    ...
    \begin{proposition}\label{prop:second_blow-up}
        The base scheme of $\alpha_2$ is the union of $\widehat{{Z_1}}$ and $\widehat{{Z_2}}$, the proper transform of $\widetilde{Z_1}$ (resp. $\widetilde{Z_2}$).
    \end{proposition}
    \begin{proof}
        By Proposition \ref{prop:base_scheme_open_reduced}, away from $E_2$ and $\overline{\pi_2^{-1}(E_1 \setminus C_2)}$ the base scheme is indeed $\widehat{Z_1} \cup \widehat{Z_2}$. Consider the affine patch in $\p^{15}$ where $a_{0,0}\neq 0$. We choose coordinates $(a_{0,1}, \cdots, a_{3,3})$ for $\A^{15}$, induced by the coordinates in $\p^{15}$, and $(b_0: \cdots : b_{9}))$ for $\p^9$, which give coordinates to $\pi_1^{-1}(D(a_{0,0})) \subset \A^{15} \times \p^{9}$. Consider the affine patch where $b_{0} \neq 0$. In this affine patch, the change of variables formulae are:
        \begin{align*}
        ...
        \end{align*}
        and the equation of $E_1$ is $a_{1,1} - a_{0,1}a_{1,0} =0$. Using this information and the definition of $\alpha$, the base scheme of $\alpha_1$ is determined by the ideal
        \begin{multline*}
        ...
        \end{multline*}
        which has two components, one being the ideal of $\widetilde{Z_2}$ and the other the ideal of $C_2$. This is the case for the patches where $b_0,b_1$ or $b_2$ are non-zero. Over the patches where $b_3,b_4$ or $b_5$ are non-zero, the base scheme has two components as well, but they are $\widetilde{Z_1}$ and $C_2$ instead. Over the patch where $b_6 \neq 0$, the base scheme is empty and over the patches where $b_7,b_8$ or $b_9$ are non-zero, the base scheme has three components, $\widetilde{Z_1}, \widetilde{Z_2}$ and $C_2$. This means that the base scheme of $\alpha_2$ over $\overline{\pi_2^{-1}(E_1 \setminus C_2)}$ is $\widehat{Z_1} \cup \widehat{Z_2}$. 
        ...
        \end{align*}
        and the equation of $E_2$ is $a_{1,1} - a_{0,1}a_{1,0}$. Using this information and the definition of $\alpha$, the base scheme of $\alpha_2$ is determined by the ideal
        \begin{equation*}
            (c_0,c_1,c_2,c_3,b_3,b_4,b_5),
        \end{equation*}
        which is the ideal of $\widehat{Z_2}$. Going through each patch and doing the relevant computations we obtain that the base scheme of $\alpha_2$ is precisely $\widehat{Z_1} \cup \widehat{Z_2}$.
    \end{proof}
    ...
    \end{theorem}
    \begin{proof}
        Since the union of $\widehat{{Z_1}}$ and $\widehat{{Z_2}}$ is the base scheme of $\alpha_2$ and the center of the blow-up $\pi_3:V_3 \to V_2$, $\alpha_3$ resolves the indeterminacies of $\alpha_2$. Since $\widehat{{Z_1}}$ and $\widehat{{Z_2}}$ are blow-ups of smooth varieties at smooth centers, they are smooth. Similarly, $V_2$ is the blow up of a smooth variety along a smooth center, so $V_3$ is smooth.
    \end{proof}
