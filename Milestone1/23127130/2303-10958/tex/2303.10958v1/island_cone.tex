...
I=\int_C dx^{d+1}\sqrt{-g}\Big(R_C+d(d-1)\Big)-T_E \int_E dx^{d-1}\sqrt{-h_E}+2\int_{Q} dx^d\sqrt{-h_Q}(K-T+\lambda R_{Q}),
\end{eqnarray}
...
I_{\text{eff}}
= \frac{1}{16\pi G^{(d-1)}_{\text{eff}}}\int dy^{d-1} \sqrt{-h} \Big{(} R_{h} +(d-2)(d-3), \Big{)}\end{eqnarray}
with effective Newton's constant
\begin{eqnarray}\label{Newton's constant1}
\frac{1}{16\pi G^{(d-1)}_{\text{eff}}}=2\pi\Big( \int_0^{\rho} \sinh(r)\cosh^{d-3}(r) dr+2\lambda \sinh(\rho) \cosh^{d-3}(\rho)\Big).
\end{eqnarray}
...
\frac{1}{16\pi G^{(d-1)}_{\text{eff}}}=2\pi q\Big(\frac{\bar{r}_0^{d-2}-\bar{r}_h^{d-2}}{d-2} +2\lambda \sqrt{F(\bar{r}_0)} \bar{r}_0^{d-3} \Big).
\end{eqnarray}

Let us make some comments. First, from the effective action (\ref{AdSCaction1}) and EOM (\ref{EOMh}), it is clear that there is massless gravity on the branes. Second, we require that the effective Newton's constant (\ref{Tlambdatensive}) is positive, which yields a lower bound on the DGP parameter
...
S_{\text{HEE}}={\text{min}} \Big\{ \text{ext} \Big( 4\pi \int_{\Gamma} dx^{d-1} \sqrt{\gamma}+8\pi \int_{\partial \Gamma} dx^{d-2} \sqrt{\sigma} \lambda \Big)\Big\},
\end{eqnarray}
...
\end{eqnarray}
where $z(\rho)$ denotes the endpoint on the codim-1 brane $Q$. For simplicity, we set the horizontal volume $V=\int dy^{d-3}=1$ in this paper. From (\ref{sect2.3.1:area}), we derive the Euler-Lagrange equation
...
To start, let us explain why entanglement islands can exit in DGP cone holography. For simplicity, we focus on the black brane metric
% v12
\begin{eqnarray}\label{bhmetric}
ds^2=dr^2+\sinh^2(r)d\theta^2+\cosh^2(r)\frac{f(z)dz^2-\frac{dt^2}{f(z)}+\sum_{\hat{i}=1}^{d-3} dy^2_{\hat{i}}}{z^2}, \qquad 0\le r\le \rho,
\end{eqnarray}
...
% v12 replace z_{\rho}\ge z_\text{cri2} by z_{\rho}= z_\text{cri2} 
% v12 replace z_{\rho}\ge z_\text{cri2} by z_{\rho}> z_\text{cri2} 
...
% v12 replace -0.1624 by -0.1625, replace < by \le
...
\caption{ Various bounds of the DGP parameter $\lambda(\rho)$ for tensionless case with $d=5$. Island Constraint 1 yields $\lambda_\text{cri2} < \lambda <\lambda_\text{max}$, HM Constraint 2 gives $\lambda_\text{cri3} < \lambda <\lambda_\text{max}$, Page-curve Constraint 3 imposes $\lambda_\text{cri4} < \lambda < \lambda_\text{max}$ and Positive-Entropy Constraint 4 results in $\lambda_\text{cri4} < \lambda < \lambda_\text{cri5}$. In general we have $\lambda_\text{cri2}<\lambda_\text{cri3}<\lambda_\text{cri4} < \lambda < \lambda_\text{cri5}< \lambda_\text{max}$.}
...
ds^2=\frac{d\bar{r}^2}{F(\bar{r})}+ F(\bar{r}) d\theta^2+\bar{r}^2 \frac{f(z)dz^2-\frac{dt^2}{f(z)}+\sum_{\hat{i}=1}^{d-3} dy^2_{\hat{i}}}{z^2},\qquad \bar{r}_h\le \bar{r}\le \bar{r}_0,
\end{eqnarray}
...
%  $q=1$    & -0.1696 & -0.1646! & -0.1631& -0.1629& -0.1628 &-0.1624! & -0.1623 \\  \hline
...
%\section{Limited time for Hartman-Maldacena (HM) surface }
...
\caption{ Various bounds of the DGP parameter $\lambda(\rho)$ for $d=4$ in wedge holography. Island Constraint 1 yields $\lambda_\text{cri2} < \lambda <\lambda_\text{max}$, Page-Curve Constraint 3 imposes $\lambda_\text{cri4} < \lambda < \lambda_\text{max}$ and Positive-Entropy Constraint 4 results in $\lambda_\text{cri4} < \lambda < \lambda_\text{cri5}$. In general we have $\lambda_\text{cri2}<\lambda_\text{cri3}< \lambda < \lambda_\text{cri5}< \lambda_\text{max}$. Note that there is not HM Constraint 2 in wedge holography. }
\label{Fig:wedge}
\end{figure}
