...
\begin{align}
    ...
    \exp \left[-\frac{1}{2}\left(\bm{X}_0-\bm{\mu}_i\left(\bm{X}_0\right)\right)^\mathrm{T}\,\bm{\mathsf{C}}_i^{-1}\left(\bm{X}_0\right)\,\left(\bm{X}_0-\bm{\mu}_i\left(\bm{X}_0\right)\right)\right].
\end{align}
...
For the GTGP model the grid cell size was optimised to maximise validation scores using grid search cross validation: a common procedure which amounts to trying a range of values of a model hyperparameter (in this case the grid cell size) and choosing the value which optimises the validation score. The optimal grid cell size found ranged from $2.5^{\circ}$ in region B to $7^{\circ}$ globally. The scores are presented in table~\ref{tab:scores}. In all regions the MDN models outperform the alternatives, with the $32$-component model achieving slightly higher scores than the single-component model. Note that the scores reported happen to be negative --- this is not by convention, but instead reflects that log probability densities are often negative. A higher score is a better score.

\begin{figure}
    \centering
    \includegraphics{figures/regions.png}
    \caption{Regions considered for model comparison.}
    \label{fig:regions}
\end{figure}
...
\begin{tabular}{ll|lllll}
    ...
    Global: & Training & $-1.61$ & $-1.36$ & $-1.07$ & $-1.02$ \\
                & Validation & $-1.74$& $-1.41$ & $-1.14$ & $-1.11$ \\
    $A$: & Training & $-1.78$ & $-1.41$ & $-1.35$ & $-1.30$ \\
                & Validation & $-1.89$ & $-1.50$ & $-1.40$ & $-1.35$ \\
    $B$: & Training & $-1.95$ & $-1.94$ & $-1.91$ & $-1.85$ \\
                & Validation & $-2.16$ & $-2.05$ & $-2.01$ & $-1.96$ \\
    $C$: & Training & $-1.93$ & $-2.05$ & $-1.63$ & $-1.56$ \\
                & Validation & $-2.00$ & $-2.03$ & $-1.61$ & $-1.57$ \\
    
\end{tabular}
\caption{Training and validation scores for the transition matrix and GTGP models, as well as the single-component MDN and full $32$-component MDN models, in each of the regions considered (see the map in figure~\ref{fig:regions}).}
\label{tab:scores}
\end{table}
...
Non-Gaussianity of displacements is likely due primarily to inhomogeneity of ocean velocities --- drifters can explore a range of flow statistics as they move, and the convolved effects of these are reflected in observed displacements.
...
\small
\vspace{0.5em} \noindent \textbf{Acknowledgements.} I am grateful to Jacques Vanneste, James Maddison and Aretha Teckentrup for their advice, input and overall support of this work. I also thank Dhruv Balwada for helpful discussions.
