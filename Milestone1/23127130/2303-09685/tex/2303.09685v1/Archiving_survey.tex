...
\thanks{X. Yao is with the Shenzhen Key Laboratory of Computational Intelligence (SKyLoCI), Department
        of Computer Science and Engineering, Southern University of Science and Technology, Shenzhen, P. R. China,
...
\begin{IEEEkeywords}
Multi-objective optimisation, evolutionary computation, archive, archiving methods, population maintenance, environmental selection
\end{IEEEkeywords}
...
The population maintenance (i.e., environmental selection)
...
Although there are a number of works dedicated to the topic of archiving, e.g., \cite{Hanne1999ejor,KnoCor2003tec,LopKnoLau2011emo,Li2021telo}, most archiving methods have been proposed and studied as part of complete MOEAs and not as independent algorithmic components; which is at odds with the fact
that the key distinctive characteristic of many MOEAs is their population maintenance (i.e., archiving) rule.
...
For example,
the main innovation of NSGA-III~\cite{DebJain2014:nsga3-part1} is the method for selecting the solutions that will form the population in the next generation,
...
Moreover, most studies of archiving from a theoretical perspective have focused on specific quality indicators~\cite{KnoCor2003tec,LauZen2011ejor,AugBadBroZit2012tcs,BriFri2014convergence}. Except for few exceptions~\citep{LopKnoLau2011emo}, it remains largely unclear,  what are the theoretical properties held by the archiving algorithms used within the state-of-the-art MOEAs; for example,  whether their population may suffer from deterioration in terms of Pareto-optimality,
...
Many unwelcome phenomena caused by the lack of such properties have been reported in the literature,
 on synthetic point sequences~\cite{KnoCor2004lnems,LopKnoLau2011emo,Li2021telo},
...
We begin by providing background knowledge of multi-objective optimisation (Section~\ref{sec:preliminaries}).
We then define the archiving problem and summarise its history (Section~\ref{sec:archiving_problem}).
...
Afterwards,
we discuss important issues in multi-objective archiving (Section~\ref{sec:issues})
and suggest several future research lines which deserve more attention (Section~\ref{sec:future}).
Lastly,
...
if the indicator used does not hold this property~\cite{LiCheYao2020ieeese}.
Fortunately,
...
is non-trivial.
The main difficult is that, in contrast to the single-objective case where there is a total order relation between solutions, Pareto dominance is a partial order, which leads to solutions (and solution sets) being incomparable.
...
An MOEA using a fixed-size population needs to decide which nondominated solutions are removed from the current population to allow the entry of newly generated ones. This decision is taken by the environmental selection mechanism, which can be seen as an archiving method that receives new solutions as input, compares them with the ones in the population (archive), and decides which solutions are kept and which ones are thrown away.
...
% This is down to the population maintenance (i.e., environmental selection) operation,
...
% They did this through dropping the second principle in population maintenance.
...
The idea is to ensure that every point in the Pareto front can be represented
...
Moreover,
they proved that in general no archiving algorithm is able to maintain an ``optimal approximation'' of the Pareto front (see Def.~\ref{Def:Optimal_approximation} on page~\pageref{Def:Optimal_approximation}) of the sequence points at every timestep~\cite{KnoCor2004lnems}.
...
developing practically effective population maintenance (i.e., archiving) methods, regardless of their theoretical properties,
...
the density estimators more frequently used are crowding distance~\cite{TiwKochFad2008amga,Moubayed2014,CaiLiFan2015archive}, niching~\cite{TiwFadDeb2011amga2,LiYao2020ec} and grid techniques~\cite{KnoCor00paes,KnoCor2000mpaes,LuoBos2012elitist}.
...
%
Experimental work has shown that adding an external archive is often beneficial~\cite{TanIshOya2017}, specially if the parameters of the MOEAs are configured after adding the archive~\cite{BezLopStu2019gecco}.
%
Recently,
...
For example,
one may wish its archive to consist of only Pareto-optimal points with respect to the input sequence;
...
Anytime properties must hold at any timestep $t$, whereas limit properties must hold after a finite number of timesteps under that assumption that any solution  may appear an infinite number of times in the input sequence~\cite{RudAga2000cec}.
...
Any anytime property that holds for one pass over a finite sequence should also hold in the limit, that is, unlimited passes over a finite sequence or an infinite sequence drawn from a finite set $Y$. We denote the union of all points seen up to time $t$ by $Y^{(t)} = \bigcup_{i=1}^t S^{(i)}$.
...
\subsection{Convergence-Guaranteed Archivers with Weakly-Pareto-Compliant Indicators}
...
\subsection{Class II: Archivers Holding Some Theoretical Properties but not Useful in Practice}
...
whose archive size cannot be controlled if the parameter $\epsilon$ is pre-defined, and adapting $\epsilon$ to bound the maximum size tends to archive too few points~\cite{KnoCor2004lnems}, thus they are not limit-optimal.
...
the archiver $\epsilon$-approx~\cite{LTDZ2002b} was said not to hold the \textit{point-monotone} property, which is not the case.},
...
In any case, all of them hold the \emph{limit-stable} property since they always maximise/minimise the indicator value, as long as they remove the newest solution when they have the same indicator contribution.
...
\subsection{Class IV: Archivers Holding the Limit-Optimal Property and also Being of Practical Use}
...
its time complexity is $O(mN^2L)$,
...
Finally,
it is worth mentioning that one may not be able to say that
archivers in the Class III are certainly better in practice (e.g., have better values on an indicator) than those in Class IV,
though most of the archivers in the former are popular in the area whereas some in the latter are not (e.g., \AHV and MGA~\cite{LauZen2011ejor}).
...
under any condition are surely preferable
provided that they do not sacrifice much practical performance (e.g., convergence and diversity),
%\MANUEL{What does it mean practical performance?}
such as MGA.



\subsection{What Is An Ideal Archiver?}
...
as a result of making nondominated solutions comparable by an indicator or using the weight vectors,
respectively.
However,
such a mechanism may make them sensitive to the shape of the sequence's Pareto front.
...
may struggle to maintain a well-distributed archive when facing sequences of ``interesting'' orders of solutions
even on low-dimensional sequences with regular Pareto front shapes (i.e., simplex shapes)~\cite{Li2021telo}.
...
there are efficient algorithms available for both hypervolume and
...
Furthermore,
most archivers will deliver a final archive that consists of many solutions which are dominated by solutions removed previously (\emph{point-deteriorate}),
as shown in~\cite{LiYao2019emo}.
...
directly using it to store high-quality solutions generated by an MOEA~\cite{Ish1998,RudTraSen2013evenly,RudSchGri2016coa,Brockhoff2019b,PanIshSha2020,BezLopStu2019gecco},
...
Several subset selection methods~\cite{BriFriKli2014generic,SinBahRay2019distance},
...
to similarity-based metrics such as distance-based~\cite{SinBahRay2019distance,ShaIshNan2021subset} and clustering-based ones~\cite{CheIshSha2021clustering}.
...
On the other hand,
costly solution evaluations often imply a simulation process that generates large amounts of data on top of the decision and objective vectors,
...
Despite their practical shortcomings, archivers in Class II are the only ones respecting the property \textit{point-monotone}. Within Class II,  $\epsilon$-Pareto and $\epsilon$-approx tend to result in an archive size far smaller than the capacity available, thus preventing them from being limit-optimal.
It remains an open question whether there exist archivers with similar theoretical properties and whose archive size is always close to the given capacity or, if possible, limit-optimal.
Some recent studies are interesting attempts to address this question~\cite{HerSch2022archive,HerSch2022hausdorff}.
...
However, they may not hold \emph{set-monotonicity} fully, 
...
\subsection{Order of Solutions Arriving}
...
possible sequences.  Here, the optimal quality can be defined by using the
unbounded archive (like in \cite{LopKnoLau2011emo}) or using the best possible
...
Further theoretical development would be welcome. For example, the limit properties (Properties~\ref{def:limit-stable}, \ref{def:limit-pareto} and~\ref{def:limit-optimal}) are not very useful in practice unless the time to converge to the limit is tractable. Thus, bounds on the number of steps or input points required to reach the limit would be of practical interest.
...
while the external archive is focused on storing the best solutions found. In this setup, it does not matter
...
including
\begin{itemize}
    \item We formalised the archiving problem and introduced six theoretical properties desirable for bounded archivers to hold.
    \item We showed analytically that archivers based on a weakly Pareto compliant indicator (e.g., $\epsilon$-indicator)
    ...
    \item We suggested future research lines and pointed out several open questions that require further theoretical and empirical research.
\end{itemize}
...
\begin{thebibliography}{179}
	...
	\bibitem[L{\'o}pez-Ib{\'a}{\~n}ez et~al.(2011)L{\'o}pez-Ib{\'a}{\~n}ez,
	Knowles, and Laumanns]{LopKnoLau2011emo}
	...
	\bibitem[Li et~al.(2020)Li, Chen, and Yao]{LiCheYao2020ieeese}
	...
	guidance,'' \emph{IEEE Transactions on Software Engineering}, 2020.
	...
	Eiben, T.~B{\"a}ck, M.~Schoenauer, and H.-P. Schwefel, Eds., vol. 1498.\hskip
	1em plus 0.5em minus 0.4em\relax Heidelberg: Springer, 1998, pp. 292--301.
	...
	J.~Kolodziej, and G.~Rudolph, Eds., vol. 6238.\hskip 1em plus 0.5em minus
	0.4em\relax Heidelberg: Springer, 2010, pp. 607--616.
	...
	J.~Lang, Eds., vol. 325.\hskip 1em plus 0.5em minus 0.4em\relax IOS Press,
	2020.
	...
	\bibitem[Brockhoff and Tu{\v{s}}ar(2019)]{Brockhoff2019b}
	D.~Brockhoff and T.~Tu{\v{s}}ar, ``Benchmarking algorithms from the platypus
	framework on the biobjective bbob-biobj testbed,'' in \emph{Proceedings of
		the Genetic and Evolutionary Computation Conference Companion}, 2019, pp.
	1905--1911.
