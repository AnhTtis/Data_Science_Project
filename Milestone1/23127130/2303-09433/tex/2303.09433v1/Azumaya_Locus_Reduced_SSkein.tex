...
\keywords{Stated skein algebras, quantum cluster algebras, quantum groups,  TQFTs}
...
where $S,P$ are some formal parameter and write $\Delta \sqcup \overline{\Delta}:= \Delta \times \{0,1\}$ (two copies of $\Delta$ where we denote an element $(\alpha, 0)$ simply by $\alpha$ and an element $(\alpha, 1)$ by $\overline{\alpha}$). Let $\widehat{x}=(x,h_p,h_{\partial})_{p, \partial} \in \widehat{X}(\mathbf{\Sigma})$ and decompose the set of inner punctures as $\mathring{\mathcal{P}}= \mathring{\mathcal{P}}_0 \bigsqcup \mathring{\mathcal{P}}_1 \sqcup \mathring{\mathcal{P}}_2$ where 
\begin{align*}
...
\begin{theorem}\label{main_theorem_intro} Let $\mathbf{\Sigma}=(\Sigma, \mathcal{A})$ be a connected essential marked surface  which either has a boundary component with at least two boundary arcs or which do not have any inner puncture.
...
\vspace{2mm}\par 
\textit{Acknowledgments.} The authors thank S.Baseilhac, D.Calaque,  F.Costantino, T.L\^e and P.Roche  for valuable conversations. J.K. acknowledges support from  the European Research Council (ERC DerSympApp) under the European Union’s Horizon 2020 research and innovation program (Grant Agreement No. 768679).
H.K. was supported by JSPS KAKENHI Grant Number JP22K20342. 
...
$Z^1_{\mathbf{\Sigma}_1}\otimes Z^1_{\mathbf{\Sigma}_2}$.
 ...
 \par $\bullet$ $\underline{ \alpha(p_1)_{++}^N \otimes 1  \in R:}$ By Lemma \ref{lemma_heightexch}, there exists $n\in \mathbb{Z}$ such that $(\alpha(p_1)_{++})^N = A^{n/2} \alpha(p_1)_{++}^{(N)}$. Since $\alpha(p_1)_{++}^{(N)}$ is in the image of the Frobenius by Theorem \ref{theorem_Frobenius}, one has $(\alpha(p_1)_{++})^N \otimes 1 \in R$. 
...
with shadow $\widehat{x}$ are in $1:1$ correspondance with the set of colorings admissible at $\widehat{x}$ and we have proved $(3)$. Such an indecomposable representation $V_0 \otimes V_1 \otimes \ldots \otimes V_n$ is weight/ simple/projective in $\mathcal{C}$/ projective in $\overline{\mathcal{C}}$ if and only if each of the $V_i$ has the same property so $(4)$, $(5)$, $(6)$ follow from  Corollary \ref{coro_QG}.
 ...
 \section{Classification of weight $U_q \mathfrak{gl}_2$ modules}\label{appendix}
