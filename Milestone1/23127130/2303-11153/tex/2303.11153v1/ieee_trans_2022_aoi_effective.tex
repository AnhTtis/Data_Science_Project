...
\thanks{\ls{.5}This work is supported in part by the National key Research Program of China under Grant 2020YFB1807700. Correspondence author: Qinghe Du (e-mail:duqinghe@mail.xjtu.edu.cn).}
\vspace{-30pt}
}
...
Above unsolved issues motivate the study on shaping the distribution rather than simply control a single average or peak statistic of AoI. The recently-proposed statistical AoI metric can effectively evaluate the achievable minimum peak AoI in a statistical yet rational way, i.e., imposing a certain age violation probability constraint \cite{zhang_statistical_2021}. Specifically, the statistical AoI is defined as the normalized log-moment generating function (LMGF) of peak AoI with a required AoI exponent, which indicates the exponentially decaying rate of the violation probability of exceeding the age threshold. In the meanwhile, the statistical AoI functions as an unified metric integrating the average statistic. In particular, when the AoI exponent decreases to zero, the statistical AoI reduces to the average peak AoI \cite{costa_age_2016}. When the AoI exponent goes up to infinity, the statistical AoI converges to the maximum peak AoI \cite{cao_2020_peak}.
...
As shown in Fig. \ref{fig:aoi_wireless_fading}(a), we consider a wireless status update system over the multi-state fading channels. Time is divided into equal-length slots. We denote by $T$ and $B$ the duration of one slot and the channel bandwidth, respectively. The wireless fading channel has $K$ states, which can be characterized by the received signal-to-noise-ratio (SNR) given the unit transmit power. We denote by $\gamma_k$ the SNR of the $k$th state, where $k\in\mathbb{K}$ and $\mathbb{K}=\{1, 2, ..., K\}$. Without loss of generality, we assume that $\gamma_1 < \gamma_2 < ... < \gamma_K$. The probability that the channel stays at the $k$th state is $p_k$. We assume that the channel coherence time, denoted by $T_c$, is much longer than the duration of one slot. Also, the channel state is assumed to independently vary from one channel coherence time to the next channel coherence time. Multiple status-sampling packets might be transmitted during the one channel coherence time. Our destination is to control \emph{how many samples should be scheduled under the different channel states,} i.e., to optimize the sampling interval under the different channel states to make the information received at the receiver as fresh as much.
\vspace{-10pt}
\subsection{AoI Processes at the Transmitter and Receiver}
\begin{figure}
  \centering
  \includegraphics[scale = 0.65]{Visio-system_model.eps}
  \caption{A wireless status update system over the multi-state fading channels and the AoI processes at transmitter and receiver.\vspace{-30pt}}
  \label{fig:aoi_wireless_fading}
\end{figure}
After the latest status packet is just transmitted, we will immediately sample the current status and then transmit the generated status packet. Thus, the sampling interval is equal to the transmission interval. Suppose that during the $i$th sampling and transmission, the channel is staying at the $k$th state. We denote the number of slots used for the $i$th transmission by $n_k^{(i)}$. Then, following the standard AoI definition \cite{kaul_real-time_2012}, the AoI process can be shown in Fig. \ref{fig:aoi_wireless_fading}(b), where $A_g(t)$ and $A(t)$ denote the AoI processes at transmitter and receiver, respectively. We do not consider the propagation delay over the air because it is much smaller than the transmission delay. As shown in Fig. \ref{fig:aoi_wireless_fading}(b), the $i$th status packet is generated from the transmitter at time instant $t_i$ and delivered to the receiver at time instant $t_{i+1}$. Once the $i$th status packet is generated, the age at the transmitter is reset to $0$. Once the $i$th status packet arrives at the receiver, the age at the receiver is updated to $t_{i+1} - t_i$. During the time for status packet delivery, $A_g(t)$ and $A(t)$ increase linearly as time elapses. We denote by $A^{(i)}$ the $i$th peak age at the receiver. Based on the relation shown in Fig. \ref{fig:aoi_wireless_fading}(b), $A^{(i)}$ can be given by
\begin{align}
A^{(i)} = (n_{k}^{(i - 1)} + n_k^{(i)})T.
\end{align}
Since the transmission interval in the same channel state is identical, we have $n_{k}^{(i - 1)} = n_k^{(i)}$. Note that the notation ``$(i)$" for $n_k^{(i)}$ is omitted later in this paper for presentation convenience. Similarly, unless otherwise stated we use $A$ to represent $A^{(i)}$. Thus, the peak age can be further given by
\begin{align}
A = 2n_k T.
\end{align}
Also, we require
...
The average power constraint is imposed as most status update tasks are carried out by wireless sensors with low-power transmissions. We denote by $w_k$ the transmit power at the $k$th channel state. Then, the average power constraint can be given as follows:
\begin{align}\label{eq:average_power_constraint}
\sum_{k = 1}^{K}w_k p_k \leq \bar{w},
\end{align}
where $\bar{w}$ represents the average transmit power of the wireless sensors.

\vspace{-10pt}
\subsection{Finite Blocklength Transmission}
The status packet is generally short and corresponds to the finite blocklength transmission. According to the finite blocklength capacity \cite{polyanskiy_channel_2010}, the achievable channel coding rate, denoted by $r(n_k)$, at the $k$th channel state, is given as follows:
\begin{align}
r(n_k) = \log_2(1 + w_k\gamma_k) - \sqrt{\frac{V}{n_kTB}}f^{-1}_Q(\varepsilon)\log_2 e,
\end{align}
where $V$ represents the channel dispersion and $\varepsilon$ is error transmission probability. We denote by $N_{\rm bit}$ the number of bits for each status packet. Thus, the number of bits which can be transmitted at the $k$th channel state with $n_k$ slots should be larger than $N_{\rm bit}$, i.e.,
...
\vspace{-5pt}
To evaluate the freshness of the information in the status update systems, the average peak AoI, as an effective metric to capture the core characteristics of age process, was proposed in \cite{costa_age_2016}. Specifically, the average peak AoI, denoted by $\overline{\Delta}_{\rm p}$, for $A(t)$ is defined as follows:
\begin{align}\label{eq:average_peak_aoi}
\overline{\Delta}_{\rm p} = \lim_{N\rightarrow \infty}\frac{1}{N}\sum_{i = 1}^{N} A^{(i)}.
\end{align}
However, the average statistics cannot describe the tail characteristics of AoI's distribution. Even when the average AoI or average peak AoI is minimized, there might still exist some arrival packets that would be out of date and cause damaging effect for status monitoring, management and control systems. Moment generating function (MGF) can be used to analyze the uncertainty of age process and describe the tail characteristics of AoI. To achieve the efficient management and control over the tail probability of peak AoI, the concept of statistical AoI was developed~\cite{zhang_statistical_2021}, the mathematical definition of which is as follows.
...
\end{align}
where $M_A(\theta)$ is the MGF of peak AoI of $A(t)$, i.e., the expectation of $e^{\theta A}$. Here, $\theta$ is called \emph{AoI exponent}. Then, the violation probability of $A$ exceeding a threshold at the tail is expected to be upper-bounded by
\begin{align}\label{eq:Chernoff_crameir_AoI}
\mathbb{P}\left\{A \geq \Delta_{\rm sta}(\theta) + \sigma\right\} \leq e^{-\theta\sigma},
\end{align}
where $\sigma$ is an arbitrary positive value. It is worth noting that the upper-bound specified in Eq.~ (\ref{eq:Chernoff_crameir_AoI}) can be obtained via Chernoff-Cram\'{e}r bound. From Eq. (\ref{eq:Chernoff_crameir_AoI}), we find that as $\theta$ changes from 0 to $\infty$, the violation probability exponentially varies from 1 to 0. In particular, when there are not any constraints on the violation probability, i.e., $\theta = 0$, the statistical AoI reduces to the average peak AoI \cite{costa_age_2016}, that is
\begin{align}\label{eq:degrade_average_peak_aoi}
\lim_{\theta \rightarrow 0}\Delta_{\rm sta}(\theta) = \lim_{N \rightarrow \infty} \frac{1}{N} \sum_{i = 1}^{N} A^{(i)}.
\end{align}
On the contrary, if the most stringent constraint is imposed on the tail violation probability, i.e., $\theta = \infty$, the statistical AoI converges to the maximum peak AoI \cite{cao_2020_peak}, that is
\begin{align}\label{eq:degrade_max_peak_aoi}
\lim_{\theta \rightarrow \infty}\Delta_{\rm sta}(\theta) = \max\left\{A^{(i)}, i = 1, 2, ...\right\}.
\end{align}
...
\Delta_{\rm sta}(\theta) = \frac{1}{\theta}\log \sum_{k = 1}^{K}\mathrm{Pr}(p_k,n_k)e^{2\theta n_kT},
\end{align}
where $\mathrm{Pr}(p_k,n_k)$ is the probability distribution function (PDF) of peak age and is specified by
\begin{align}\label{eq:pdf_aoi}
\mathrm{Pr}(p_k,n_k) = \frac{p_k}{n_k}\left(\sum_{i = 1}^{K}\frac{p_i}{n_i}\right)^{-1}.
\end{align}
Again, what we concern more is how to optimize the sampling interval under the different channel states as well as the freshness requirement. Eq. (\ref{eq:bit_constraint}) implies that by setting a larger sampling interval, i.e., larger $n_k$, we can lower the transmit power. On the other hand, setting a smaller sampling interval, i.e., smaller $n_k$, we can keep information fresher. Thus, there exists the optimal sampling interval scheme for status update. To seek this scheme, the optimization problem, denoted by \textbf{\emph{P}0}, is formulated as follows:
\begin{subequations}
\begin{align}
\textbf{\emph{P}0}:
\displaystyle \min_{n_k,w_k} \,\,\,& \Delta_{\rm sta}(\theta),\\
{\rm{s.t.}}\quad
...
\vspace{-15pt}
\section{Near-Optimal Sampling Interval Scheme for Status Update}
...
\begin{align}
\Delta_{\rm sta}(\theta)
&\approx \frac{1}{\theta}\log \sum_{k = 1}^{K}\mathrm{Pr}(p_k,n_k)(1 + 2\theta n_kT) \\
&\approx \frac{1}{\theta}\log\left[1 + \sum_{k = 1}^{K}2\theta n_kT\mathrm{Pr}(p_k,n_k)\right] \\
&\approx \sum_{k = 1}^{K}2n_kT\mathrm{Pr}(p_k,n_k). \label{eq:lemma_0_proof1}
\end{align}
...
\begin{align}\label{eq:pdf_aoi_bounded}
 \frac{p_k}{n_{\rm max}}\leq \mathrm{Pr}(p_k,n_k)\leq p_k n_{\rm max}.
\end{align}
Substituting Eq. (\ref{eq:pdf_aoi_bounded}) into (\ref{eq:effective-aoi_fading}), we have Eq. (\ref{eq:error}). When $\theta$ is large, the value of $\frac{1}{\theta}\log n_{\rm max}$ is very small. Thus, the approximation form specified in Eq. (\ref{eq:effective-aoi_fading_large_theta}) is derived.
...
We find that the optimal solution for \textbf{\emph{P}0} inevitably keeps ``=" in Eq. (\ref{eq:bit_constraint}) hold. This result can be proven via contradiction method. Once ``=" holds, we have
\begin{align}\label{eq:transformer}
w_k = \frac{1}{\gamma_k}\left(\displaystyle 2^{g(n_k)} - 1\right),
\end{align}
...
{\rm{s.t.}}\quad
& \sum_{k = 1}^{K}\frac{p_k}{\gamma_k}\left(2^{g(n_k)} - 1\right) \leq \bar{w},
...
&\!+\! \alpha \left(\sum_{k = 1}^{K}\frac{p_k}{\gamma_k}\left( 2^{g(n_k)} \!-\! 1\right) \!-\! \bar{w}\right) \nonumber \\
...
\alpha\left(\sum_{k = 1}^{K}\frac{p_k}{\gamma_k}\left(2^{g(n_k)} - 1\right) - \bar{w}\right) = 0, \\
...
\vspace{-30pt}
\centering
\subfigure[]{\includegraphics[height=2in]{converage_performance.eps}\label{fig:converage_performance}}
\subfigure[]{\includegraphics[height=1.9in]{scheme.eps}\label{fig:simulation_1}}
\subfigure[]{\includegraphics[height=2in]{performance_evaluation_final.eps}\label{fig:performance_eAoI}}
\label{fig:simulation_2}
\vspace{-10pt}
\caption{(a) the convergence rate of using quadratic transform method to solve \textbf{\emph{P}1}; (b) our proposed scheme under different AoI exponents; and (c) the performance of our proposed scheme (Simulation value of each parameter: $N_{\rm bit}=500$ bits, $T=1$ ms, $B=100$ kHz, $n_{\rm max}=5$, $\gamma_k\in\{1,5,10,15,20\}$, $p_k\in\{0.1,0.2,0.4,0.2,0.1\}$, $P=1$ W, and $\varepsilon=10^{-3}$).\vspace{-20pt}}
\end{figure*}
...
For a specified AoI exponent, we select the solution that can achieve the smaller statistical AoI between the solutions for the small and large AoI exponents as the final sampling internal scheme.
...
In this section, we evaluate the performance of our proposed sampling interval scheme for status update transmission over fading channels.
Figure \ref{fig:converage_performance} shows the performance of using the quadratic transform method to solve the approximated statistical AoI minimization problem for the case of small AoI exponent. The concept of normalized statistical AoI is illustrated, which is defined as the statistical AoI normalized by $T$. We can observe that about 5-round iterations, the minimal normalized statistical AoI can be achieved.
...
Figure \ref{fig:simulation_1} shows the optimal sampling interval scheme under different AoI exponents. It can be seen from the figure that when AoI exponent is small, the optimal sampling interval is decreasing as the CSI increases, and when the AoI exponent is infinite, the optimal sampling interval is independent of the CSI. This verifies Lemma \ref{lem:lemma_1}.

Figure \ref{fig:performance_eAoI} shows the performance of our proposed scheme under different AoI exponents. We also show the performance of optimal scheme derived by using the time-inefficient exhaustive search method. It can be seen that the performance of our proposed scheme is quite close to the optimal solution, and thus can guarantee nearly best freshness of information under the different age violation probability constraints. Moreover, the performance of the schemes, which can minimize the average peak AoI~\cite{costa_age_2016} and the maximum peak AoI~\cite{cao_2020_peak}, i.e., the statistical AoI with zero and extremely large AoI exponents, is depicted, respectively. Obviously, the performance of our proposed scheme is superior to both of them.

\vspace{-7pt}
\section{Conclusions}
In this paper, we investigated the statistical AoI minimization problem for status update over multi-state fading channels. We tackled this nonconvex problem by converting it to a fractional programming problem and a convex problem for small and large AoI exponents, respectively, and derived an efficient and near-optimal sampling interval scheme. How sampling interval should be tuned catering for different AoI exponents was shown. In particular, for the extremely stringent AoI requirement, the sampling interval converged to a constant regardless of CSI's variation. The numerical results verified that our proposed scheme can significantly reduce the AoI with the given age violation probability and thus guarantee the excellent performance in terms of freshness of information.

\vspace{-10pt}
\bibliographystyle{IEEEtran}
\bibliography{References}

%\begin{align}\label{eq:peak_AoI}
%  A_n = T_{n - 1} + D_n = G_n + T_n.
%\end{align}
%\begin{equation}
%D_n=\left\{ {\begin{array}{*{20}{l}}
%S_n,&\mbox{if}~~G_n \leq T_{n - 1},\\
%G_n + S_n - T_{n - 1},&\mbox{if}~~G_n > T_{n - 1},
%\end{array}} \right.
%\end{equation}
