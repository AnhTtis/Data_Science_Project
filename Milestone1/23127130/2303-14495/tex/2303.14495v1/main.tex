...
Xinhua Shen\thanks{Institute for Mathematical Sciences, 
		...
		Email: \href{mailto:shenxinhuayouxiang@163.com}{shenxinhuayouxiang@163.com}.} 
			\quad 
Hongpeng Sun\thanks{Institute for Mathematical Sciences,
Renmin University of China, China.
Email: \href{mailto:hpsun@amss.ac.cn}{hpsun@amss.ac.cn}.} \quad 
 Xuecheng Tai\thanks{Department of Mathematics, Hong Kong Baptist University,
			Kowloon Tong, Hong Kong. 		Email: \href{mailto:xuechengtai@hkbu.edu.hk}{xuechengtai@hkbu.edu.hk}.} 
}
...
\end{equation}
It is widely employed for multiscale modeling including the phase-field simulations \cite{Ey}
...
u=\argmin_{u} F(u), \quad    F(u): = GL(u) + \frac12 \eta ||u-y||_{\Lambda}^2.
\end{equation}
Here, $||u-y||_{\Lambda}^2 :=\langle u-y, \Lambda (u-y)\rangle = \sum_{i} \Lambda_{i}(u(i)-y(i))^2$ and  $\Lambda \geq 0$ is a diagonal matrix and the values $y(i)$ are assumed to be known. In applications, $\Lambda_{i} = 1$ if $i \in Z'$ and 0 otherwise with $Z'$ being the set of pixels or data points with known prior labels. The balancing parameter $\eta>0$.  Let's  denote
\[
...
\end{cases}
\end{equation*}
Here $N(i,j)=N_s(i,j)$ for images with `$\text{dist}$' determined by the window. For clustering, $N(i,j)=N_c(i,j)$ with predetermined $n_l$. Moreover,
as in Figure \ref{tab:my_label:undirected:graph} (the left one), we employ the KNN for searching nearest neighbors, i.e., the $k$-nearest neighbors algorithm based on the Euclidean distance $\|\cdot\|_2$. We refer to \cite{MPL} for the implementations of KNN with the source code. 
...
$L$ and $L_s$ are based on an assumption: each node is connected to any other node in the window and the corresponding weight is not 0.
...
With these preparations, let us turn to the proposed preconditioned DCA for the graph Ginzburg-Landau model. 
...
\end{equation}
It turns the nonlinear and nonconvex minimization problem \eqref{eq:functional:GL} to  a sequence of convex minimization problems \eqref{eq:orig:dca} through keeping linearization of  $P_2$. With $\xi^t = cu^t - \grad \bb W(u)|_{u=u^t}$, we need to solve the following linear equation during each DCA iteration as in \eqref{eq:orig:dca}
...
Flexible step sizes can bring out some convenience including monotone decreasing of the energy $F(u)$ under certain step size constraints \cite{LB}.
...
\begin{proof}
	...
	u^{t+1}:=\argmin_{u} P_1(u) - \langle\xi^{t} , u\rangle+ \frac{1}{2}\|u-u^k\|_{\frac{I}{k}}^2 + \frac{1}{2}\|u-u^k\|_{\frac{\kappa}{k}M}^2,  \quad \xi^t  \in \partial P_2(u)|_{u=u^t}.   \tag{pDCA$_k$}
	\end{equation}
...
\begin{proposition}\label{prop:nopre}
	...
	\end{equation}
\end{proposition}
Actually, if letting the step size $k\rightarrow +\infty$, we have the following connections between the finite step size cases and the infinite step size cases.
...
\begin{remark}
	...
	\end{equation}
	Here, the constant $c>0$ is chosen such that $P_2$ is a convex functional on $u$. The difference of convex functional in \eqref{eq:dc:functional:svd} is very convenient for the SVD-Nystr\"{o}m based methods. One only needs to do Nystr\"{o}m  sampled SVD for the linear equation associated with $\epsilon L + cI$ which has constant coefficients. It does not depend on the prior $\eta \Lambda$ as the linear operator $T = \epsilon L + cI + \eta \Lambda $ as in \eqref{eq:dca:linear:system:ori} with varying coefficient $\Lambda$. However, the proposed various preconditioners in our framework can handle the preconditioning with varying coefficient $\Lambda$ efficiently. 
...
\begin{proof}
	...
	\]
	Actually, the positive semidefiniteness of $D+W$ can be deduced from that $D+W$ is diagonal dominant by the definition of the graph Laplacian $L$. 
\end{proof}
...
\begin{cases}
G_{Jd} = 2\text{diag}(T)=2(\varepsilon D+\eta \Lambda +cI), \quad &\text{damped Jacobi preconditioner}, \\
G_{Jp} = 2\varepsilon D+\eta \Lambda +cI + \delta_0 I, \quad &\text{perturbed Jacobi preconditioner},
\end{cases}
\end{equation}
...
\[
G_{Jd}-T = 2(\varepsilon D+\eta \Lambda +cI) - (\varepsilon L+\eta \Lambda +cI)=\varepsilon(2D-L) +\eta \Lambda + cI \geq cI >0, 
\]
and
\[
G_{Jp}-T = (2\varepsilon D+\eta \Lambda +cI+\delta_0I) - (\varepsilon L+\eta \Lambda +cI)=\varepsilon(2D-L) +\delta_0 I \geq \delta_0I >0.
\]

We will also consider the following generalized Richardson preconditioner
\begin{equation}\label{eq:pre:unnorm:richard}
G_{R} = \varepsilon \lambda_{\max} I + \eta \Lambda + cI + \delta_0I.
\end{equation}
Here, $\delta_0>0$ is as before. $\lambda_{\max}$ is the maximum eigenvalue of $L$ and  can be estimated by the power method. It can be checked that 
\[
G_{R} -T = \varepsilon \lambda_{\max} I + \eta \Lambda + cI + \delta_0I -(\varepsilon L+\eta \Lambda +cI) = \varepsilon (\lambda_{\max} I-L) + \delta_0I \geq \delta_0I >0,
\]
...
Similarly, for $T = \varepsilon L_s + \eta \Lambda +cI $ with  the normalized graph Laplacian $L_s$, we prefer the following \emph{feasible} preconditioners
\begin{equation}\label{eq:pre:norm:jacob:richard}
\begin{cases}
G_{Jd,s} = 2\text{diag}(T)=2(\varepsilon I+\eta \Lambda +cI), \quad &\text{damped Jacobi preconditioner}, \\
G_{Jp,s} = 2\varepsilon I+\eta \Lambda +cI + \delta_{0} I, \quad &\text{perturbed Jacobi preconditioner}, \\
G_{R,s} = \varepsilon \lambda_{s,\max} I + \eta \Lambda + cI + \delta_{0}I, \quad &\text{generalized Richardson  preconditioner}.
\end{cases}
\end{equation}
...
\begin{cases}
G_{Jd,k} = 2(I + k(\varepsilon D+\eta \Lambda +cI)), \quad &\text{damped Jacobi preconditioner}, \\
G_{Jp,k} = I+ k(2\varepsilon D +\eta \Lambda +cI) + \delta_0 I, \quad &\text{perturbed Jacobi preconditioner}, \\
G_{R,k} = I +k( \varepsilon \lambda_{\max} I + \eta \Lambda + cI) + \delta_0I, \quad &\text{generalized Richardson  preconditioner}.
\end{cases}
\end{equation}
...
\begin{cases}
G_{Jd,ks} = 2(1+ k(\varepsilon +c)I+k\eta \Lambda), \quad &\text{damped Jacobi preconditioner}, \\
G_{Jp,ks} = (2k\varepsilon  +1) I+ k(\eta \Lambda +cI) + \delta_{0} I, \quad &\text{perturbed Jacobi preconditioner}, \\
G_{R,ks} = I +k( \varepsilon \lambda_{s,\max}  I + \eta \Lambda + cI) + \delta_{0} I, \quad &\text{generalized Richardson  preconditioner}.
\end{cases}
\end{equation}
...
Before the discussion of the global convergence of the proposed preconditioned DCA \eqref{eq:pdca:framework}, let us turn to its energy decreasing. Inspired by \cite{DS} (the proof of Theorem 2) and \cite{LPH} (Theorem 3), we have the following theorem.
...
\begin{proof}
	...
	Moreover, since $F(u)$ is a polynomial with order $4$ and $u \in \mathbb{R}^{MN}$ with $MN$ variables, we conclude that the KL exponent of $F$ is $1-R(n,d)$ by \cite{LMP} (Lemma 2). While $\bar u$ is the isolated zeros of $F$,  the KL exponent of $F$ in the neighborhood of $\bar{u}$ is $1-\chi(n,d)$ still by \cite{LMP} (Lemma 2).  
...
\end{remark}
\section{Numerical result}\label{sec:num}
...
Here we use the `square seed' prior for the image `two cows', which is the same as in \cite{LB}. For all other images,  we use `seed' prior such as subfigure (d) of Figures \ref{fig:Segmentationbygeneralwindow_cow} to \ref{fig:Segmentationbygeneralwindow_stone}    due to their large sizes. 
...
Now, let us give more discussions on the sizes of search windows. A search window that is large enough not only can help obtain more accurate segmentation but also can reduce the dependence on the prior information, while they can bring out huge storage and computations simultaneously. 
...
\begin{remark}
	We employ the svd1 (or svd2) as in the publicly available implementation of \cite{BF,ME},   which is implemented by Matlab  with extremely efficient SVD, while the essential steps including computing weights and preconditioned iterations of pDCA are implemented by CUDA. 
\end{remark}
...
\begin{figure*}[!htbp]
	...
	\subfloat[pDCA final result\label{fig:subfig:Clover_pDCA_0.996per_943_general}]{
		\includegraphics[width=1.3in, origin=br]{figure/clover_pDCA_0996per_943_general.png}}
...
Now let's turn to the discussions of specially designed sparse search windows for pDCA. Note that svd1 and svd2 only depend on the sampling rate. For nonlocal models on large-size images, it is usually challenging for solving the graph Laplacian matrix $L$.  $L$ is usually too large for storing in computer memory but also costs too much time to compute the action on $u$, i.e., $Lu$. In this section, we propose some specially designed search windows to make $L$ more compact as in Figure \ref{fig:search_window}. These search windows are complementary to the usual box search window as in Figure \ref{patch_and_window}b. They can capture global information for images with disconnected components while still keeping sparse structures that are convenient for storage and computations. 
...
\begin{figure*}[!htbp]
	...
	\\
	\subfloat[pDCA final result (general method) \label{fig:subfig:Yellowflower_pDCA_0.914per_1294}]{
		\includegraphics[width=1.8in, origin=br]{figure/yellowflower_pDCA_0914per_1294.png}}
	...
	\subfloat[pDCA final result (sparse method 1)\label{fig:subfig:Cow_pDCA_97per_404}]{
		\includegraphics[width=1.8in, origin=br]{figure/yellowflower_pDCA_099per_593.png}}
...
\begin{figure*}[!htbp]
	...
	\\
	\subfloat[pDCA final result (general method) \label{fig:subfig:starfish_pDCA_0.914per_1294}]{
	...
	\subfloat[svd1 final result\label{fig:subfig:starfish_SVD_96per_135} ]{
		\includegraphics[width=1.8in, origin=br]{figure/starfish_SVD_80per_162.png}}
	% 	\subfloat[pDCA final result\label{fig:subfig:Cow_pDCA_97per_404}]{
	% 		\includegraphics[width=2in, origin=br]{figure/yellowflower_pDCA_90per_121.png}}
	\subfloat[pDCA final result (sparse method 2)\label{fig:subfig:starfish_pDCA_97per_404}]{
		\includegraphics[width=1.8in, origin=br]{figure/starfish_pDCA_0963per_792_sparse2.png}}
...
\begin{table}[h]
	...
	\end{tabular}
	\caption{Comparisons of different preconditioners using the normalized graph Laplacian.  }
...
In this section, we will present some compact comparisons between the proposed pDCA algorithm and svd1 for data clustering  \cite{GMBFP}. The main tool, KNN \cite{MPL} for producing the weights for data clustering is implemented based on cluster tree data structure. It is not implemented in parallel although highly efficient. The proposed pDCA with KNN does not have as many advantages as image segmentation. However, it is still very competitive compared to svd1.
...
The half-circle data set chooses angles and two disjoint ranges of radius randomly to generate a new data set. For the randomness of the algorithm, we calculate the mean of results in 10 experiments. The coefficient `Accuracy' indicates the proportion of points that are labeled as the correct classes. 
% \begin{equation*}
