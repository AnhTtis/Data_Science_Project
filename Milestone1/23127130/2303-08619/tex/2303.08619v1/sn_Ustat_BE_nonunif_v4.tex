...
\DeclareMathOperator{\barPhi}{\bar{\Phi}}
...
\newcommand{\corsref}[1]{Corollaries~\ref{cor:#1}}
\newcommand{\corssref}[1]{\ref{cor:#1}}
...
\keywords{U-statistics, Stein's method, self-normalization, Studentization, nonlinear statistics, variable censoring, nonuniform Berry-Esseen bound, randomized concentration inequality, sub-Gaussian lower tail of non-negative U-statistics, Bennett's inequality}

\subjclass[2000]{62H05}
...
\end{equation}
 Throughout, we will assume, without loss of generality,
\begin{equation} \label{zero_mean_kernel_assumption}
 \bE[h(X_1, \dots, X_m)] = 0.
\end{equation}
It is well-known that under the finite second moment assumption $\bE[h^2(X_1, \dots, X_m)]<\infty$
...
Nevertheless, with a few exceptions such as the Kendall's tau statistic \citep{kendall1938new} for which $\sigma = 1/3$ under \eqref{zero_mean_kernel_assumption}, $\sigma$ is typically unknown and cannot be directly used to standardize $U_n$. Practically, it is hence more relevant  to develop a B-E bound for U-statistics that are \emph{Studentized}  with the  Jackknife estimator of   $\sigma$ proposed by \citet{arvesen1969jackknifing}; in particular, for the special degree-one kernel in \eqref{sample_mean_as_ustat}, the resulting Studentized U-statistic is precisely the  t-statistic of Gosset \citep{student1908probable}. This 
...
To our best knowledge, a  nonuniform bound for Studentized U-statistics similar to \eqref{standardized_ustat_nonunif_BE} is  still eluding the literature,  even in the simplest case of the t-statistic: It should be noted that, although a   delicate \emph{\cramer-type} moderate deviation result  for the \emph{self-normalized sum}  $S_n / V_n$, where
...
\end{equation}
which is closely connected to the  t-statistic (see \eqref{efron_relationship} below), has been established in  \citet{jing2003self}, it does not directly imply a nonuniform B-E bound that applies to all  $x \in \bR$ in the spirit of \eqref{standardized_ustat_nonunif_BE}. Moreover, an earlier nonuniform B-E bound for the self-normalized sum stated in   \citet[Corollary 2.3]{jing1999exponential} has also been latter disproved by an interesting binary data example  raised by  \citet[p.342-343]{novak2005self}, which demonstrates it is in fact  \emph{impossible} to have a nonuniform B-E bound of the ``usual form", 
...
\end{equation}
that holds for an absolute constant $C$ and any non-increasing function $d:\bR_{\geq 0} \rightarrow \bR_{\geq 0}$ with the property  $\lim_{x \rightarrow 0} d(x) = 0$, assuming $\bE[X_1] = 0$.
...
{\bf Organization.} \secref{U_stat_review}  covers the basics of  Studentized U-statistics, and   revisits \citet{novak2005self}'s data example to deduce that the nonuniform bound in the usual form of \eqref{usual_form} cannot be valid. \secref{main} states our new nonuniform B-E bounds, and  \secref{mainpf} outlines the major steps of the proof for our main theorem, with the Appendices covering additional technical materials. 
...
\end{equation}
for some  $p \in (0,1)$, with  expectation and second and third absolute moments 
 \begin{equation*}
...
p = p_n = n^{-1} \text{ and }x = x_n = \sqrt{n} - \epsilon \text{ for any small fixed constant } \epsilon > 0,
\end{equation}
the right hand of \eqref{wrong_sn_sum_nonunif_bdd} is seen to be equal to
\begin{multline} \label{rhs_sn_presumed_bdd}
...
\]
on which the self-normalized sum $S_n/V_n$ can be easily seen to take upon the value $\sqrt{n}$, which is greater than $x_n$; one can then consequently derive the lower bound $e^{-1} > 0$  of the  ``liminf" of the left hand side in \eqref{wrong_sn_sum_nonunif_bdd} as:\begin{align}
...
&=  \liminf_{n \rightarrow \infty} [(1 -p_n)^n - \barPhi(x_n)] = 1/e  \label{lhs_prp_sn}.
\end{align}
...
In fact, more generally, \citet{novak2005self}'s example  readily implies a non-asymptotic bound like  \eqref{usual_form} can't hold for Studentized U-statistics either. First, recall the classical relationship between the self-normalized sum and Student's t-statistics \citep{efron1969}:
\begin{equation} \label{efron_relationship}
T_{student}  = \frac{S_n}{V_n} \Bigg\{ \frac{n-1}{n - (S_n/V_n)^2} \Bigg\}^{1/2}.
\end{equation}
Hence, on the event $\cE_n$, $T_{student}$ takes  on the value $\infty$ and one can similarly derive as in \eqref{lhs_prp_sn} that 
\begin{equation} \label{liminf_lower_bdd_student_t}
\liminf_{x \rightarrow \infty} [ P(T_{student} \leq x_n) - \Phi(x_n)] \geq 1/e.
\end{equation}
On the other hand, for  the real-valued,  symmetric kernel of degree $m$  defined by 
...
so, from \eqref{real_studentizer}, one can conclude that 
\begin{equation} \label{student_ustat_is_student_t}
T_n = T_{student} \text{ for any } m \geq 1 \text{ and the kernel  in \eqref{sum_kernel}}.
\end{equation}
Lastly, by Rosenthal's inequality \citep[Theorem 3]{rosenthal1970subspaces}, it is true that
\begin{multline} \label{rosenthal} 
g(x)  = x, \quad \sigma^2 = 1\quad \text{ and }\\
 ...
 \end{multline}
Combining \eqref{liminf_lower_bdd_student_t}, \eqref{student_ustat_is_student_t}, \eqref{rosenthal} and taking values for $(p, x)$ as in  \eqref{novak_param_choice}, we similarly refute the bound in  \eqref{usual_form}, because it shows that for some $n$, one can accordingly find a value $x$ for which the right hand side is less than the left hand side in \eqref{usual_form}. 


%\begin{convention} [Convention for $T_n^*$ when $\sigmahatstar = 0$] \label{con:convention2}
%If $\sigmahatstar = 0$, it must be that $q_i =0$ for all $i$, which implies $U_n= \sum_{i=1}^n q_i /n = 0$; $T_n^*$ is then assigned the value $0$.
%\end{convention}
%When $U_n = 0$, there is indeed no need for normalization, so the values of $T_n$ and $T_n^*$ are naturally taken to be $0$ as in the  conventions above. Lastly, when both $T_n$ and $T_n^*$ are real-valued, they are related by the algebraic equality 
%\begin{equation} \label{algebraic_relationship}
%T_n^* = \frac{T_n}{\Big( 1 +  \frac{m^2 (n-1)}{(n-m)^2} {T_n}^2  \Big)^{1/2}};
%\end{equation}
%see \citet{lai2011cramer, shao2016cramer}.  Since the function $b_n(x) \equiv x \big(1 + \frac{m^2 (n-1)}{(n-m)^2}x^2\big)^{-1/2}$ has the properties that
%\[
%\frac{d}{d x} b_n(x)= \Bigg( 1 + \frac{m^2 (n-1)}{(n-m)^2}x^2\Bigg)^{-3/2} > 0 \text{ and }
% \lim_{x \rightarrow \pm \infty} b_n(x) = \pm \frac{n-m}{m \sqrt{n-1}},
%\]
%as $T_n$ tends to $\pm \infty$, $T_n^*$ increases/decreases to the limit $\pm \frac{n-m}{m \sqrt{n-1}}$. 
% From this, we can  conclude that
%\begin{equation} \label{Tnstar_relevant_range}
%|P(T_n^* \leq x) - \Phi(x) | = \barPhi(|x|) \text{ for any } x \in \bR \text{ with  } |x| \in  \Big(\frac{n-m}{m \sqrt{n-1} }, \infty\Big), 
%\end{equation}
% by considering two separate cases:
%\begin{enumerate}
%\item Case 1, $x \in ( \frac{n-m}{m \sqrt{n-1} }, \infty)$:  If $\sigmahatstar \neq 0$,  $x$ exceeds the largest positive value $\frac{n-m}{m \sqrt{n-1}}$  that $T_n^*$ can possibly attain so  the event $\{T_n^* \leq x\}$ is true; if $\sigmahatstar = 0$,  $T_n^* = 0$ by \conref{convention2} so the event $\{T_n^* \leq x\}$ is also true. As such,  $P(T_n^* \leq x) = 1$ and  \eqref{Tnstar_relevant_range} is true.
%\item Case 2, $x \in (-\infty, -\frac{n-m}{m \sqrt{n-1} })$: If $\sigmahatstar \neq 0$,  $x$ is below the smallest negative value $-\frac{n-m}{m \sqrt{n-1}}$  that $T_n^*$ can possibly take so  the event $\{T_n^* \leq x\}$ is false; if $\sigmahatstar = 0$,  $T_n^* = 0$ by \conref{convention2} so the event $\{T_n^* \leq x\}$ is also false. As such,  $P(T_n^* \leq x) = 0$ and  \eqref{Tnstar_relevant_range} is true.
%\end{enumerate}
...
%
%\subsection{Extension of \citet{novak2005self}'s example  for U-statistics} \label{sec:Novak_extended}
%For the sample mean, which corresponds to a U-statistic with the  kernel in \eqref{sample_mean_as_ustat}, in  parallel to the nonuniform bound for standardized U-statistics in \eqref{standardized_ustat_nonunif_BE}, one would naturally expect   the bound 
%\begin{equation}\label{prospective_sn_sum_nonunif_bdd}
%\left|P\Bigg(\frac{\sum_{i=1}^n X_i}{\sqrt{\sum_{i=1}^n X_i^2}} \leq x\Bigg)  - \Phi(x) \right|\leq  \frac{ C \|X_1\|_3^3}{\sqrt{n}\|X_1\|_2^3} d(|x|)
%\end{equation}
%to hold for the self-normalized sum  under the conditions $\bE[X_1] =0$ and $0 < \bE[X_1^2]< \infty$, where 
%$C$ is a positive absolute constant and $d(x)$ is a non-increasing function  such that $d(x) \downarrow 0$ as $x \rightarrow \infty$. 
%Quite surprisingly, \citet{novak2005self} was the first to point out that such a nonuniform bound is not possible  with an interesting example, where the i.i.d. data  are assumed to  be  binary variables such that
%\begin{equation} \label{bin_data_def}
%P\Bigl(X_i = p^{1/2} (1 - p)^{-1/2}\Bigr) = 1- p \text{ and } P\Bigl(X_i = - (1 - p)^{1/2} p^{-1/2}\Bigr) = p
%\end{equation}
%for some parameter $p \in (0, 1)$. We shall first extend  his example to illustrate that an analogous nonuniform bound cannot be possibly valid for self-normalized and Studentized U-statistics of any degree either.
%
%
%Suppose, towards a contradiction, in analogy to \eqref{standardized_ustat_nonunif_BE},  the intuitive B-E bounds of rate $1/\sqrt{n}$,
%\begin{multline}\label{sn_student_u_stat_nonunif_bdd_plausible}
%\left|P(T^*_n \leq x)  - \Phi(x) \right|\leq C_1(m) \frac{\bE[|h(X_1, \dots, X_m)|^3]}{\sqrt{n} \sigma^3} d_1(|x|) \text{ for } x\in \bR \\
%\text{and}\\
%\left|P(T_n \leq x)  - \Phi(x) \right|\leq C_2(m) \frac{\bE[|h(X_1, \dots, X_m)|^3]}{\sqrt{n} \sigma^3} d_2(|x|) \text{ for } x\in \bR,
%\end{multline}
% do hold for self-normalized and Studentized U-statistics of any degree $m$ for which \eqref{zero_mean_kernel_assumption} and \eqref{non_degeneracy} are true, where  $d_1$ and $d_2$ are some non-negative and non-increasing functions with the properties that $\lim_{x \rightarrow \infty} d^*(x) = \lim_{x \rightarrow \infty} d(x) = 0$. 
%In particular, \eqref{sn_student_u_stat_nonunif_bdd_plausible} should  hold for   $T_n^*$ and $T_n$ constructed with  the binary data defined in \eqref{bin_data_def} and the symmetric kernel
%\begin{equation} \label{special_kern}
%h(x_1, \dots, x_m) = x_1 + \dots + x_m,
% \end{equation}
% whose expectation can be computed as 
%  \[
% \bE[h(X_1, \dots, X_m)] = m \bE[X_1] = 0;
% \]
%  Since the other absolute moments of the data can be computed as
% \begin{equation*}
%\sigma^2 = \bE[X_1^2] = 1 \text{ and }
%\bE[|X_1|^{3}] = p^{3/2} (1 - p)^{-1/2} + (1 - p)^{3/2} p^{-1/2},
%\end{equation*} 
%by Rosenthal's inequality \citep[Theorem 3]{rosenthal1970subspaces}, which implies
%\begin{equation*} \label{rosenthal}
% \bE[|h|^{3}] \leq  C(m)\bE[|X_1|^3],
% \end{equation*}
%   one can further obtain the bounds  
%\begin{multline} \label{sn_student_u_stat_nonunif_bdd_plausible2}
%\left|P(T^*_n \leq x)  - \Phi(x) \right|\leq
% C_1(m) \frac{ p^{\frac{3}{2}} (1 - p)^{-\frac{1}{2}} + (1 - p)^{\frac{3}{2}} p^{-\frac{1}{2}}}{\sqrt{n} } d_1(|x|) 
%\\
%\text{ and }
% \\
%\left|P(T_n \leq x)  - \Phi(x) \right|\leq
% C_2(m) \frac{ p^{\frac{3}{2}} (1 - p)^{-\frac{1}{2}} + (1 - p)^{\frac{3}{2}} p^{-\frac{1}{2}}}{\sqrt{n} } d_2(|x|), \\
% \text{ for  the binary data in \eqref{bin_data_def}  and the kernel in \eqref{special_kern}}.
%\end{multline}
%
%
%To arrive at a contradiction, consider the same binary data in \eqref{bin_data_def} and the specific event
%\[
%\cE \equiv \{X_1 = \dots = X_n = p^{1/2} (1 - p)^{-1/2}\} \text{ with } P(\cE) = (1-p)^n.
%\]
%By direct computations, with the special kernel in \eqref{special_kern}, one can easily check that 
%\[
%U_n = p^{1/2}(1-p)^{-1/2}, \quad {\sigmahatstar}^2 = \frac{n(n-1)}{(n-m)^2} p(1-p)^{-1}  \text{ and } \sigmahat^2 = 0 \text{ on the event } \cE,
%\]
%and hence
% \begin{equation} \label{sp_event}
%T^*_n = \frac{(n-m)}{m\sqrt{n-1}} \text{ and } T_n = \infty \text{ on  } \cE;
%\end{equation}
%recall that $\frac{(n-m)}{m\sqrt{n-1}}$ is the largest value $T^*_n$ can possibly take as discussed in \secref{U_stat_review}.
%By letting 
%\begin{equation} \label{novak_param_choice}
%p = p_n = n^{-1} \text{ and }x = x_n = \frac{(n-m)}{m\sqrt{n-1}} - \epsilon \text{ for any small fixed constant } \epsilon > 0,
%\end{equation}
%we can observe the following properties for the two bounds in \eqref{sn_student_u_stat_nonunif_bdd_plausible2}: On one hand, 
%\begin{align}
%&\liminf_{n \rightarrow \infty} \Bigl[P(T^*_n \leq x_n) - \Phi(x_n) \Bigr] \notag\\
% &= \liminf_{n \rightarrow \infty} \Bigl[P(T^*_n > x_n) - \barPhi(x_n) \Bigr] \notag\\
%&\geq \liminf_{n \rightarrow \infty} \Bigl[P(X_1 = \dots = X_n = p_n^{1/2} (1 - p_n)^{-1/2}) - \barPhi(x_n) \Bigr]\notag\\
%&=  \liminf_{n \rightarrow \infty} [(1 -p_n)^n - \barPhi(x_n)] = 1/e >0 \label{lhs_prp_sn},
%\end{align}
%where  we have used \eqref{sp_event} 
%and  $\lim_{n \rightarrow \infty} (1 - n^{-1})^n = 1/e$. Similarly, one can show 
%\begin{equation}\label{lhs_prp_student}
%\liminf_{n \rightarrow \infty} \Bigl[P(T_n \leq x_n) - \Phi(x_n) \Bigr]  \geq 1/e >0,
%\end{equation}
% On the other hand, substituting $p_n$ for $p$, the right hand sides of the two bounds in \eqref{sn_student_u_stat_nonunif_bdd_plausible2} can be respectively expressed as
%\begin{multline} \label{rhs_prp}
%C_1(m)\left\{ n^{- 2 } (1 - n^{-1})^{- 1/2} + (1 - n^{-1})^{3/2}  \right\} d_1(|x_n|) \text{ and } \\
%  C_2(m)\left\{ n^{- 2 } (1 - n^{-1})^{- 1/2} + (1 - n^{-1})^{3/2}  \right\} d_2(|x_n|)
%\end{multline}
%Since  both expressions in  \eqref{rhs_prp} converge to zero as $n$ tends to infinity, which, when combined with the positive lower bound $1/e$ for the ``liminf"'s  in  \eqref{lhs_prp_sn} and \eqref{lhs_prp_student}, indicates that for some large $n$, one can find a  combination of values for $p$ and $x$ so that the left hand sides are actually greater than the right hand sides for the two non-asymptotic bounds in \eqref{sn_student_u_stat_nonunif_bdd_plausible2}.
%This breaks the presumed bounds in  \eqref{sn_student_u_stat_nonunif_bdd_plausible}, from  which  the bounds in \eqref{sn_student_u_stat_nonunif_bdd_plausible2}  originate. 




The moral of  \citet{novak2005self}'s example  in \secref{U_stat_review} is that, due to the way that the Jackknife Studentizer  $\hat{\sigma}$ in  \eqref{real_studentizer} is constructed, when the distribution of the data in question is such that the moment ratio $\bE[|h|^3]/\sigma^3$ is large compared to $\sqrt{n}$ and $T_n$ can take its largest possible value (i.e. $\infty$) with a non-negligible probability, the bounds  in \eqref{usual_form} may fail to hold. 
...
Note that \eqref{BE_Tn_one_sample_simpler} is a simple consequence of \eqref{BE_Tn_one_sample} because $\|g\|_3 \leq \|h\|_3$; see the basic U-statistic property in \eqref{Jensen}.  
These bounds can now accommodate  more ``extreme" data distributions; consider \citet{novak2005self}'s data in \eqref{bin_data_def} again. By the other side of Rosenthal's inequality \citep[Theorem 3]{rosenthal1970subspaces}, for the kernel $h$ in \eqref{sum_kernel} we have 
\[
 \bE[|h|^{3}] \geq 8^{-1} m \bE[|X_1|^3] = 8^{-1} m (p^{3/2} (1 - p)^{-1/2} + (1 - p)^{3/2} p^{-1/2}).
\]
Hence for the particular  choices of $p$ and $x$ in \eqref{novak_param_choice}, we have
\begin{align*}
\exp \Bigg( - \frac{c(m) n \sigma^6 }{(\bE[|h|^3])^2}\Bigg) &\geq  \exp \Bigg( - \frac{c(m) n  }{(8^{-1} m (p_n^{3/2} (1 - p_n)^{-1/2} + (1 - p_n)^{3/2} p_n^{-1/2}))^2}\Bigg) \\
&\geq \exp \Bigg( - \frac{64 \cdot c(m)  }{m^2 (1 - n^{-1})^{3} }\Bigg).
\end{align*}
 Since   $  \exp ( - \frac{64 \cdot c(m)  }{m^2 (1 - n^{-1})^{3} })$ is  larger than the lower bound $e^{-1}$ in \eqref{liminf_lower_bdd_student_t}  for a sufficiently small $c(m) > 0$, given $n >2$, the bounds in \thmref{main} are not contradicted by \citet{novak2005self}'s example. We also remark that,  the condition 
$\max(2, m^2) < n$ is somewhat stronger than the condition $2m < n$ for the uniform B-E bound in \thmref{BE_unif}; as will be seen in \secref{mainpf}, having such a condition facilitates our analysis of the lower tail probability of the Studentizer $\sigmahat$, which ultimately leads to the augmenting term that decays exponentially in $n$.
 
 
 Our next result, whose proof is in \appref{sn_sum_pf}, is specific to  Student's t-statistic, the special case of $T_n$ with the kernel in  \eqref{sample_mean_as_ustat}. It says that the nonuniform term in $x$ can be further strengthened to be decaying exponentially in  $|x|$. 
...
\end{equation*}
The same bound can be stated with $T_n$ replaced by  $S_n/V_n$, where $S_n$ and $V_n$ are defined in \eqref{Sn_Vn_def}, 
for possibly different  constants $C_1, C_2, c_1, c_2 > 0$.
\end{theorem}



 The proof of \thmref{nonunif_BE_sn_sum}  makes use of the fundamental \emph{\cramer-type} moderate deviation result  for self-normalized sums established in  \citet{jing2003self}, and their sub-Gaussian-like tail properties (\lemref{subGauss_sn_sum}). Currently, it is unclear to us if it is possible to similarly strengthen the rate of decay of the nonuniform term in $|x|$  for a general U-statistic of degree $m \geq 2$ in \thmref{main}, due to two major barriers. One is that the current state-of-the-art in \cramer-type moderate deviations for Studentized U-statistics  \citep{shao2016cramer} is only applicable to a fairly restricted class of kernels; refer to \citet[Eqn. $(3.3)$]{shao2016cramer}. The other is that sub-Gaussian tail properties are currently not available for what we call the \emph{self-normalized} U-statistic \citep{lai2011cramer, shao2016cramer}, 
...
\footnote{Specifically, it was showed that ${\sigmahatstar}^2 = d_n^2 (V_n^2 + \delta_1^* + \delta_2)$; recall the self-normalized U-statistic in \eqref{sn_ustat}. One can then  deduce from \eqref{real_studentizer} that  $\sigmahat^2 = d_n^2 (V_n^2 + \delta_1^*  -   \frac{(n-1)^2}{(n-m)^2}U_n^2 + \delta_2)$.} 
...
\Big| P(T_n > x) - \barPhi(x)\Big| \leq  R_x 
...
\end{lemma}
 The proof of \lemref{bdd_for_remnants} (\appref{remnant_pf}) follows fairly standard arguments, and we note that 
the proofs for $(iii)$-$(v)$ repeatedly use the Chernoff-type bound
$
I\Big(W_b \geq \frac{ \frakcm x}{2}\Big) \leq e^{W_b - \frac{\frakcm x}{2}}
$
to result in the exponentially nonuniform terms in $x$. Next, we bound the other $x$-dependent term $| P(W_b + \barD_{1, x} > x (1 +   \barD_{2, V_b, \bardelta_1, \bardelta_{2,b}}   )^{1/2} )- \barPhi(x)|$ from  \eqref{distill_ineq} in  \lemref{intermediate} below. Its proof (\appref{Stein_pf}) involves Stein's method, and is  largely similar to the proof of the uniform B-E bounds in   \citet{leungshaounif}, except that the properties of the solution to the Stein equation is more thoroughly exploited,  to ensure the nonuniformity in $x$ of the bound  throughout.
...
\frakh(x_1, \dots, x_{2m})\geq 0 \text{ for all values of $x_1, \dots, x_{2m -1}$};
\end{equation}
when $n = 2m$, from the original definition of $\sigmahat^2$ in \eqref{real_studentizer}, it is seen that,  irrespective of the values of  $X_1, \dots, X_{2m-1}$,
\[
 A(m, n) \frakh(X_1, \dots, X_{2m -1}) = {\sigmahat}^2 =  \frac{n-1}{(n-m)^2} \sum_{i=1}^n (q_i- U_n)^2  \geq 0,
\]
so $\frakh$ can only take on non-negative value since $A(m, n)  > 0$.



With the insights above, we are primed to leverage the following exponential lower tail bound for non-negative kernel U-statistics to develop the exponential bound in \eqref{exponential_lower_bdd_D2}. This result is of independent interest, and it naturally extends a known   exponential lower tail bound for a sum of independent non-negative variables in the literature \citep[Theorem 2.19]{selfnorm}; surprisingly, we could not locate a result similar to \lemref{chernoff_lower_tail_bdd_U_stat} elsewhere. Its proof is included in \appref{otherproofs}, which uses a well-known trick by  \citet{hoeffding1963probability}. 
...
% \frac{   (n - m-1)(n - m-2)\cdots (n - 2m+2)}{ (n-3)(n-4) \cdots (n-m)}
  ...
  \end{cases},
\] 
in light of the assumption that $n > \max(2, m^2)$ and the definition of  $\mathfrak{b}_m$ in \eqref{frakbm_def},  one can derive the lower bound
...
&\leq \exp\left( -\frac{[n/m] \Big\{   \frac{7(2m)!}{16\{(m-1)!\}^2}  \Big(1 - \frac{m^2}{m+1}\Big)  \Big\}^3 }{ 3(\bE[\frakh^{3/2}])^2}\right) \label{sigmahatstar_sq_lower_tail_bdd}
...
one can derive
\begin{align*}
\bE[|\frakh|^{3/2}] &\leq C n^{3/2} (\bE[|\tilde{\frakh}|^{3/2}] + \frac{m^3}{n^{3/2}}\bE[|\breve{\frakh}|^{3/2}])\\
...
where $\beta_2 \equiv \sum_{i=1}^n \bE[\xi_i^2 I(|\xi_i|>1)]$ and $\beta_3 \equiv \sum_{i=1}^n \bE[\xi_i^3 I(|\xi_i|\leq1)]$.
 ...
 This subsection supplements \secref{mainpf}, and in particular  \eqref{zero_mean_kernel_assumption} and \eqref{sigma_equal_1_assumption} are assumed to hold. 
Given 
\[
...
and
 ...
 \begin{equation} \label{delta1_1st_abs_moment_bdd}
  \bE\Big[|\delta_1| \Big] \leq C(m)    \frac{\bE[h^2]}{n}
   \end{equation}
 and
 ...
 \bE \left[ h^2\right].
 \end{equation*}
 Moreover, by \eqref{simple_u_stat_moment_bdd} of \lemref{non_integral_moment_bdd_u_stat},  $\bE[U_n^2] \leq C(m) \frac{\bE[h^2]}{n}$. Collecting these facts give  \eqref{delta1_1st_abs_moment_bdd} because $\bE[|h|^2] \geq 1$.
...
\[
P\Big(D_1>\frac{\frakcm x}{4} \Big) \leq \frac{64\bE[|D_1|^3] }{ \frakcm^3 x^3} \leq \frac{C(m)}{n^{3/2}(1 +x^3)} \bE[|h|^3].
\]
...
&\leq \frac{C(m)}{(1 + x^3)} \sum_{i=1}^n \bE[|\xi_i|^3] =  \frac{C(m)\bE[|g|^3]}{(1 + x^3)\sqrt{n}}.
\end{align*}
\end{proof}
...
\begin{align*}
&  P\left( W_b  \geq  \frac{\frakcm x}{2}, | D_{2, V_b, \bardelta_1, \bardelta_{2,b}}  | > 1 \right)  \\
&\leq   Ce^{-\frakcm x/2}  \{\bE[e^{W_b} \delta_{0, b}^2] + \|e^{W_b}\|_2  \|\bardelta_1^2 \|_2+ \|e^{W_b} \|_2 \|\bardelta_{2, b}^2\|_2 + n^{-1} + \sum_{i=1}^n \bE[\xi_i^2 I(|\xi_i| >1)]\} \text{ (by \lemref{bennett_censored})}\\
&\leq   Ce^{-\frakcm x/2}  \Big\{\bE[e^{W_b} \delta_{0, b}^2] + \|e^{W_b}\|_2  \sqrt{\bE[|\bardelta_1|]}+ \|e^{W_b} \|_2 \|\bardelta_{2, b}\|_2 +  n^{-1}+\sum_{i=1}^n \bE[|\xi_i|^3]\Big\} \text{ (by $|\bardelta_1| \vee |\bardelta_{2,b}| \leq 1$)}.
\end{align*}
...
D_{2, b, -n^{-1/2}, \bardelta_{2, b}} =  d_n^2( V_{n, b}^2 -n^{-1/2} + \bar{\delta}_{2, b}) - 1,
\]
where we respectively replaced $\bardelta_1$ with its lower and upper bounds $-n^{-1/2}$ and $n^{-1/2}$  from $D_{2, V_b, \bardelta_1, \bardelta_{2,b}}$'s definition in \eqref{DR_censored_def}. Analogously to \eqref{barDR_censored_def}, we also let
\begin{multline*}
...
\end{equation}
As will be seen later, transforming the problem into one that handles $\DRplaceholder$ instead of ${D}_{2, V_b, \bardelta_1, \bardelta_{2, b}}$ has the advantage of obviating the need to deal with the variability of $\bardelta_1$;  a similar strategy has also been employed in our relate work \citet{leungshaounif} for proving uniform B-E bounds.
...
\Delta_{i^*} \equiv \bar{D}_{1, x, i^*}  - \frac{x \bar{\frakD}_{2, i^*} }{2},
\end{equation}
...
\section{Proof for \thmref{nonunif_BE_sn_sum}} \label{app:sn_sum_pf}
...
It suffices to consider $x \geq 0$, whether we are aiming to establish the theorem for the self-normalized sum $S_n/V_n$ or the t-statistic $T_{student}$, otherwise one can  replace the $X_i$'s with $- X_i$'s instead. 
 A technical tool that we will use is the following sub-Gaussian type bound  that can be found in \citet[Theorem 2.16]{victor2009self}:
...
\subsubsection{Bounding $|P(S_n/V_n >  x a_n(x)) - \barPhi(x a_n(x))| $ }
...
\exp \Bigg( \frac{-   n ( \bE[X_1^2])^3}{16 (\bE[X_1^3])^2} \Bigg) + C \frac{(1 + x)^{2}}{e^{cx^2}} \frac{ \bE[|X_1| ^3]}{\sqrt{n} (\bE[X_1^2])^{3/2}} \text{ for } 0 \leq x \leq \sqrt{n}
\end{multline}
...
\big|P( S_n/V_n > x a_{n, x}) - \barPhi( x a_{n, x})\big| \leq  \\
C_1\exp \Bigg( \frac{-  c_1 n ( \bE[X_1^2])^3}{ (\bE[X_1^3])^2} \Bigg) +  \frac{C_2}{e^{c_2x^2}} \frac{ \bE[|X_1| ^3]}{\sqrt{n} (\bE[X_1^2])^{3/2}} \text{ for all } x \geq 0.
\end{multline}

%another subsubsection
 \subsubsection{Bounding $|\barPhi(x a_n(x)) - \barPhi(x)|$ }
