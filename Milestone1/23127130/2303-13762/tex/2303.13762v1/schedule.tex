...
\begin{document}
	...
	\author{\IEEEauthorblockN{Dongxu Chang \IEEEauthorrefmark{2},
		...
		Email: dongxuchang@mail.sdu.edu.cn, ghwang@sdu.edu.cn, yangy@amss.ac.cn, daweiyin@mail.sdu.edu.cn,
	}
	...
	\begin{IEEEkeywords}
		Low-density parity-check code, scheduling policy
	\end{IEEEkeywords}
	...
	\end{equation}
	where $m_0$ is the channel message in LLR form, $l$ is the number of iterations, $N(v)$ represents the nodes connected directly to node $v$, $i \rightarrow \alpha$ means from variable node $i$ to check node $\alpha$ and $\alpha \rightarrow i$ means from check node $\alpha$ to variable node $i$. The initial message $m_{i \rightarrow \alpha}^{(0)}$ and $m_{\alpha \rightarrow i}^{(0)}$ is 0.
	...
	NMP is a metric used to measure the complexity of the decoding process by defining the V2C or C2V message passing on a directed edge as a unit of complexity. The complexity is determined by the number of messages passed during decoding, and the incremental value of NMP after decoding on a check node with degree $d_c$ is $2d_c$. The average value of NMP can be used to compare the decoding complexity under different scheduling policies through simulation experiments.
	
	\subsection{Two metrics to measure the decoding performance}
	
	\subsubsection{Average entropy (AE)}
	The BP process can be analyzed by density evolution \cite{richardson2001capacity}. In the following, the notations used in \cite{richardson2001capacity} will be employed. Assume that $\boldsymbol{x}=\left\{x_1,\cdots,x_n\right\}\in\left\{0,1\right\}^n$ are transmitted through binary memoryless symmetric channel (BMS) $W$, and $\boldsymbol{y}=\left\{y_1,\cdots,y_n\right\}$ are received signals. Let
	\begin{equation}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\STATE $\boldsymbol{\gamma_0}\gets\boldsymbol{\beta_0}\left[1:M\right]$ (Pick out bits 1 to $M$ of the sequence $\beta_0$)
			\FOR{$i=1:h$}
			...
			\ENDFOR
			\STATE Copy the sequence $\boldsymbol{\gamma_0}$ $T$ times to get new scheduling sequence $\boldsymbol{\beta_i}$
	...
	\begin{figure*}
		...
		\includegraphics[width=18cm]{1.eps}
