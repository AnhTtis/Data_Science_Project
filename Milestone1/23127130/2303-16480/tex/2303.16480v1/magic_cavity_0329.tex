...
\begin{document}
\title{Quantum interference and controllable magic cavity QED  via giant atom in coupled resonator waveguide}
...
\end{equation}
where $\Omega$ is the transition frequency of the GA between its ground state $|g\rangle$ and excited state $|e\rangle$, $\sigma_+=(\sigma_-)^\dagger=|e\rangle\langle g|$ is the {raising} operator of the GA, and $g$ is the coupling strength between the GA and the resonator in the waveguide. Without loss of generality, we assume $g$ to be real in this paper.
\begin{figure}
  \centering
...
\end{figure}
In what follows, we are interested in a scenario where $\Omega$ lies in the center of the continuum, such that the waveguide supplies an effective structured environment for the GA. As shown in Fig.~\ref{scheme1}(b), where we illustrate the energy spectrum of the whole GA-CRW coupled system in the single excitation subspace as a function of the coupling strength $g$ by setting $N=6$. We observe two BOCs, and the upper (lower) one is denoted by $E_U$ ($E_L$). Unexpectedly, we also find a single BIC, which is denoted by $E_I$ and is just located at the center of the continuum.  The single-excitation wave function can be expressed as $|E_\alpha\rangle=(\sum_j d_{\alpha,j}a_j^\dagger+c_{\alpha}\sigma_+)|g, \rm {vac}\rangle$ for $\alpha=U,L,I$, and more discussions about the BIC is given in Appendix A. In Fig.~1(c), we sketch the atomic population $|c_{\alpha}|^2$ for these three bound states. For the BOC, the atomic population is same with each other ($|c_{U}|=|c_{L}|$) and increases with the GA-CRW coupling strength, this trend is similar to that in the small atom setup~{\cite{Peter16pra}}. However, for the BIC, the single excitation is mainly on the atom in the weak coupling regime, and continually decreases as the coupling strength increases. For the other eigen states except for these three bound states, there is nearly no atomic excitation. Fig.~\ref{scheme1} (d) demonstrates the photonic amplitudes for a moderate GA-CRW coupling {$g=0.15\xi$}, in which we also observe the exotic behavior for the BIC. The photon populate only inside the GA regime and occupied resonators are labeled by the odd numbers (for example $j=1,3,5$ for $N=6$). This BIC does not exist in the single small atom scenario and the non-zero photonic amplitudes satisfy $d_{L,1}=-d_{L,3}=d_{L,5}$. As a comparison, we also plot the amplitude for the BOC, in which the photon are bounded around the two connecting sites and decays exponentially inside and outside the GA. Meanwhile, the amplitudes satisfy the relation $d_{U,j}=(-1)^{j+1}d_{L,j}$.
...
\begin{figure}
  ...
  \caption{Evolution of the atomic excited state (a,c) and photonic population (b,d). The parameters are set as $\Omega=\omega_c,g=0.1\xi$. We set $N=4$ in (a,c) and $N=6$ in (b,d).}\label{decay}
\end{figure}

To understand how the quantum interference effect leads to these dramatically different dynamical behavior in Fig.~\ref{decay} (a) and (b), we resort to Fourier transformation $a^{\dagger}_{k}=\sum_{j}e^{-ikj}a^{\dagger}_{j}/\sqrt{N_c}$
with $N_c=\infty$ being the length of the CRW, it then yields the Hamiltonian
\begin{equation}
H_s=\Omega \sigma_+\sigma_-+\sum_{k}\omega_k a_{k}^{\dagger}a_{k}
+\sum_{k}[ g_k a_{k}^{\dagger}\sigma_{-}+\rm H.c.].
\end{equation}
Here, the dispersion relation of the waveguide satisfies $\omega_k=\omega_c-2\xi\cos k$ and $g_k=g(1+e^{ikN})/\sqrt{N_c}$. We write the single excitation wave function as
 \begin{equation}
...
$F_1(\tau)=F_3(\tau)=i[J_1(2\xi\tau)-J_3(2\xi\tau)]$ leads to the photonic occupation in the resonator with $j=2$ at the early time as shown in Fig.~\ref{decay}(c). Second, for $N=6$, the destructive interference with $G(\tau)=J_0(2\xi\tau)- J_{6}(2\xi\tau)$ leads to the dissipation suppression and destructive ({constructive}) interference between the two Bessel functions in $F_j$ for $j=1,3,5$ ({2,4}) leads to the striped photonic distribution as shown in Fig.~\ref{decay}(d).
...
Then, the state of the system at moment $t$ is obtained as
...
&&+\gamma_I \left(D_{[\tau_+,\sigma_-]}\rho+D_{[\sigma_+,\tau_-]}\rho\right),
\end{eqnarray}
where $D_{[O_1,O_2]}\rho=2O_2\rho O_1-\rho O_1O_2-O_1O_2\rho$. Here, $\gamma_g={\rm Re} A, A=g^2(1+i^N)/\xi$ and $\gamma_s=g_s^2/(2\xi)$ are the individual decay rates of the giant and small atoms, respectively. $\gamma_I={\rm Re} B, B=gg_s(i^M+i^{(N-M)})/(2\xi)$ is their collective decay rate, which is induced by the common CRW environment. Taking the classical driving to the GA into consideration, the effective Hamiltonian in the rotating frame is expressed as
\begin{eqnarray}
H_{\rm eff}&=&(\Delta+\delta_g)\sigma_+\sigma_-+\Delta\tau_+\tau_-\nonumber \\
&&+g_I(\sigma_+\tau_-+\tau_+\sigma_-)+\eta(\sigma_++\sigma_-),
\end{eqnarray}
where $\Delta$ is the detuning between the atoms and the driving field and $\eta$ is the driving strength. $\delta_g={\rm Im}A$ and $g_I={\rm Im} B$ is the CRW induced Lamb shift for the GA and the effective coupling strength between the two atoms, respectively.



\begin{figure}
  \centering
  \includegraphics[width=0.5\columnwidth]{4a}\includegraphics[width=0.5\columnwidth]{4b}\\
  \includegraphics[width=0.5\columnwidth]{4c}\includegraphics[width=0.5\columnwidth]{4d}
  \caption{(a) Sketch of the magic cavity QED setup, where an auxiliary small atom is introduced. (b) Excitation of the small atom as a function of the driving detuning $\Delta$.  (c) Atomic populations when the driving field is absent. of  giant and  small The dash lines are the numerical result and the solid lines are obtained based on  a pure Markovian approximation. (d) Population of the small atom with and without the GA. The parameters are set as $\Omega=\omega_c=0$, $N=6$, $M=1$ in (c), $N=4$, $M=2$ in (d) and $g=g_s=0.1\xi$ in (b-d).}\label{decayx}
\end{figure}

The results within Markovian approximation tell us that the GA with $N=6$ will not dissipate, that is, it forms a perfect magic cavity. In Fig.~4(b), we plot the GA population as a function of the detuning $\Delta$ for different setups. The typical Rabi splitting for $N=6,M=1$ implies that we have achieved the strong coupling $g_I=gg_s/\xi$ in the effective magic cavity QED model. Also, the fact { $\gamma_I=0$} implies that the collective dissipation disappears. So that, we reach a cavity QED model, in which only the small atom undergoes dissipation. We can also observe the Rabi oscillation as shown in Fig.~4(c), where the external driving is dismissed and the initial state is prepared as $|\psi(0)\rangle=\sigma_+|g,g,\rm{vac}\rangle$ in the numerical simulation. It further predicts the validity of the effective magic cavity QED model and the decrease of the oscillation amplitudes are only due to the dissipation of the small atom.
...
Furthermore, in this setup, we also numerically find a BIC ($|E^{m}_I\rangle$) in this setup, which is free of decoherence. In terms of this BIC, the final population of the small atom is
\begin{equation}
\langle \tau_+\tau_-\rangle(t\rightarrow\infty)=|\langle \psi(0)|E^{m}_I\rangle\langle E^{m}_I|\tau_+|G\rangle|^2.
\end{equation}
In such a way, we find that the dark state emerges into the BIC, being similar to the two small atom setup, which couples to a common CRW~\cite{An1}.  In Appendix A, we further list the possible cases in which the BIC exists in both of the single GA and magic QED model, to give a guidance for the quantum source preserving in the open system where the CRW supplies a structured environment.} Meanwhile, since $\langle \psi(0)|E^{m}_\alpha \rangle\ll \langle \psi(0)|E^{m}_I\rangle ,\,(\alpha=U,L)$, we find that the BOCs play negligible roles in the dynamics. Therefore, we observe a steady but not oscillation state in the magic cavity setup. For larger $g$, the BOC will participate, and the numerical result (not shown here) will yield the oscillation, which implies that the Markovian approximation fails. 
...
=\frac{g\pi}{\xi}\int_{-\pi}^{\pi}dk\frac{(1+e^{ikN})e^{ikj}}{\cos k}.
\end{equation}
\begin{figure}
  \centering
...
When the external driving is taken into consideration phenomenologically, we should work in the rotating frame to eliminate the time dependent in the Hamiltonian. After regrouping some individual terms, we will get the final form, which is given in Eq.~(10) of the main text.
...
\begin{figure}
  \centering
  \includegraphics[width=0.4\columnwidth]{s4}
  \caption{Evolution of the atomic populations in the magic cavity setup. The numerical and approximate results are given by solid and dashed lines, respectively.  The parameters are set as $\Omega=\omega_c,g=g_s=0.1\xi,N=6,M=1$.}
  \label{NM61}
\end{figure}
...
\label{magic}
\end{eqnarray}
which are actually Eqs.~(17-19) in the main text. In Fig.~\ref{NM61}, we demonstrate the atomic populations during the time evolution by setting the initial state as $|\psi(0)\rangle=\sigma_+|G\rangle$, the good agreement between the results based on Eq.~(\ref{magic}) and the exact numerical calculation grantees the validity of the Weisskopf-Wigner approximation.
\end{widetext}
\end{subappendices}
...
\bibitem{LG2017}L. Guo, A. Grimsmo, A. F. Kockum, M. Pletyukhov, and G.
Johansson, Giant acoustic atom: A single quantum system with a deterministic time delay, Phys. Rev. A {\bf 95}, 053821 (2017).
...
\bibitem{GA2019}G. Andersson, B. Suri, L. Guo, T. Aref, and P. Delsing, Non-exponential decay of a giant artificial atom, Nat. Phys. {\bf 15}, 1123 (2019).
...
\bibitem{xian3}X. Yin, W. Luo, and J. Liao, Non-Markovian disentanglement dynamics in double-giant-atom waveguide-QED systems, Phys. Rev. A {\bf 106}, 063703 (2022).
...
\bibitem{An1}C. Chen, C. Yang, and J. An, Exact decoherence-free state of two distant quantum systems in a non-Markovian environment, {\bf 93}, 062122 (2016).
