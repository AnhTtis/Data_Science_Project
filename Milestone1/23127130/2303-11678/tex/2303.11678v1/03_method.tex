...
Let $p_\textrm{data}(\x)$ be the distribution of training images for which we have no annotations initially. Each training image~$\x$ can be annotated with a pixel-wise segmentation labeling~{$(\x,\y)\sim{}p_\textrm{data}(\x)p_\textrm{sgm}(\y\mid\x)$} or an image-wise classification annotation~{$(\x,c)\sim{}p_\textrm{data}(\x)p_\textrm{cls}(c\mid\x)$}%\footnote{Without loss of generality, we assume that image-wise labels are a discrete class label, but that other kinds of annotations are equally valid as long as they are informative in the cross-task learning process.}. 
Sampling from the distributions $p_\textrm{cls}$ and~$p_\textrm{sgm}$ represents the task of manually annotating the image and has associated costs of~$\alpha_\textrm{c}>0$ and ~$\alpha_\textrm{s}>0$, respectively. Supported by previous work~\cite{Bearman16,Mensink,mahmood2022}, we will assume that $\alpha_\textrm{s}\gg\alpha_\textrm{c}$.
...
\begin{figure*}[h]
\centering
\includegraphics[width=.95\textwidth]{method.pdf}
...
% important challenges. First, the utility function marginalizes over all possible training sets, which is intractable to compute in practice. Instead, we approximate the Eq.~\eqref{eq:utility} with a collection of samples from~$(p_\textrm{cls}^C, p_\textrm{sgm}^S)$ for multiple strategies~$(C, S)$, and then fit a Gaussian process~(GP) to generalize to the entire set of strategies. Second, approximating the utility in this way requires annotated data, which in turn relies on an annotation strategy that we are trying to find, whereby implying a circular dependency that we break solving Eq.~\eqref{eq:the_optimization} in an iterative manner. At each iteration~$t$, new data is annotated according to the current strategy~$(C_t, S_t)$ so that the sets of annotated data~$(\T_c, \T_s)$ contain $C_t$~classification and $S_t$~segmentation annotations, respectively. The available annotated data~$(\T_c, \T_s)$ is used to build new samples and fit the GP, which serves to find the next best strategy~$(C_{t+1}, S_{t+1})$. The entire process is repeated a number of iterations~$T$, which is an hyperparameter of our method. 

\subsection{Utility model}

As defined Eq.~\eqref{eq:utility}, the utility function,~$u$, marginalizes over all possible training sets, which is intractable to compute in practice. To overcome this computational challenge, we approximate~$u$ with a collection~$\M$ of discrete samples, where each sample $m\in\M$ is a tuple containing an allocation strategy~$(C, S)$ and the estimated score $m(\T_c, \T_s)$ obtained for a dataset sampled with that allocation strategy. To build~$\M$, one could simply sample a random strategy~$(C', S')$, annotate a dataset~$(\T'_c, \T'_s)\sim{}(p_\textrm{cls}^{C'}, p_\textrm{sgm}^{S'})$, and measure its performance. However, this would imply annotating for different potential budgets and is thus infeasible in practice. Instead, a practical alternative is to leverage previously annotated data~$(\T_c, \T_s)$. For each sampled strategy~$(C', S')$, we build the corresponding dataset~$(\T'_c, \T'_s)$ by taking random samples from the already annotated data according to the strategy. While this procedure, formalized in Alg.~\ref{alg:build_gp_samples}, leads to biased samples, we empirically found this bias to have a minor impact on the final strategies compared to estimations with unbiased sampling.
% Also, this alternative limits the sampled strategies to the cardinals of set of annotated data~$(\T_c, \T_s)$ \PMN{Maybe we don't need to explain this?}. \PMN{Note to me: TODO check}
...
% from~$(p_\textrm{cls}^C, p_\textrm{sgm}^S)$ for multiple strategies~$(C, S)$, and then fit a Gaussian process~(GP) to generalize to the entire set of strategies.

% To generalize to estimation of~$u$ from the discrete samples of~$\M$ to the entire set of strategies, we use the contents of~$\M$ to fit a GP,
...
% We generalize the estimation of~$u$ from the discrete samples of~$\M$ to the entire set of strategies by fitting a Gaussian process to the samples of~$\M$.
% \begin{equation}
%     \mathcal{GP}(\mu, k),
% \end{equation}
% for suitable chosen mean function~$\mu$ and covariance function~$k$ defined on the space of strategies.
In our case, we use the mean function,
\begin{equation}
    \mu(C, S) = \gamma_c\log(\beta_cC+1) + \gamma_s\log(\beta_sS+1),
    \label{eq:gp_mean}
\end{equation}
which accounts for the fact that the segmentation performance increases logarithmically with the volume of the training data~\cite{sun2017} and that each annotation type has a different rate of performance growth. Similarly, the covariance~$k$ is a combination of two RBF~kernels with different scales~$\ell_c$, $\ell_s$ for each annotation type,
\begin{equation}
    k\left((C, S), (C', S')\right) = \sigma^2e^{-\frac{(C-C')^2}{2\ell_c^2}} e^{-\frac{(S-S')^2}{2\ell_s^2}}.
\end{equation}
...
% To generalize from the discrete samples in~$\M$ to the entire set of strategies, we use the contents of~$\M$ to fit a GP, which models a distribution over plausible utility functions,
% \begin{equation}
%     u \sim{} \mathcal{GP}(\mu, k)
% \end{equation}
% for suitable mean function~$\mu$ and covariance function~$k$. We use the mean function
% \begin{equation}
%     \mu(C, S) = \gamma_c\log(\beta_cC+1) + \gamma_s\log(\beta_sS+1),
% \end{equation}
% which accounts for the fact that the segmentation performance increases logarithmically with the volume of the training data~\cite{sun2017} and that each annotation type has a different rate of performance growth. Similarly, the covariance~$k$ is a combination of two RBF~kernels with different scales~$\ell_c$, $\ell_s$ for each annotation type,
...
% Our method starts with an initial annotation strategy~$(C_0, S_0)$ with a cost~$B_0\ll B$. Following this strategy, a first annotation campaign builds the initial annotated dataset~$\T = (\T_c, \T_s)$ that contains $C_0$~classification and $S_0$~segmentation annotations. Evaluating the performance of~$\T$ and the performance of multiple of its subsets~$\T'\subset\T$, we obtain a collection of new samples for~$\M$ as detailed in Alg.~\ref{alg:build_gp_samples}. While the elements of~$\mathcal{M}$ should ideally be sampled from the original data and annotation distributions, this would involve additional costs. Instead, we sample them from the sets of already annotated data. While this biases the samples of~$\M$ and the estimation of the~GP, we empirically found this bias to have a minor impact on the final strategies compared to estimations with unbiased sampling.

The trained GP models a distribution over utility functions, $u \sim\mathcal{GP}(\mu, k)$, that are plausible under the samples~$\M$. This distribution represents not only the expected utility, but also its uncertainty in different areas of the strategy space. Sampling just a single~$u$ from the GP to solve Eq.~\eqref{eq:the_optimization} would thus be suboptimal. For this reason, we substitute the utility~$u$ in Eq.~\eqref{eq:the_optimization} by a surrogate function~$\hat{u}$ that trades-off exploitation and exploration, thus incorporating uncertainty information into the optimization problem. Following a Bayesian optimization approach~\cite{jones1998efficient}, we choose~$\hat{u}$ to be the expected improvement~(EI),
\begin{equation}
...
% To overcome the second challenge, we optimize the Eq.~\eqref{eq:the_optimization} sequentially by allocating the available budget~$B$ in several adaptive installments. 

% The previous discussion clarifies how to approximate~$u$ in order to optimize Eq.~\eqref{eq:the_optimization} in practice, but this approximation introduces two opposing requirements.
...
Note that solving Eq.~\eqref{eq:delta_strategy} requires finding~$\hat{u}_t^*$, which in turn requires solving Eq.~\eqref{eq:the_optimization}. While solving two optimization problems may seem unnecessary, the solutions of both problems are in the Pareto front of strategies (\ie, the set of non-dominated strategies for which no other strategy has simultaneously smaller cost and larger or equal expected improvement). Given that the space of strategies is discrete, the elements of the Pareto front can be easily found in linear time by enumerating all possible strategies, computing their costs and expected improvements with~$\hat{u}_t$, and discarding the dominated elements. Given the Pareto front, the strategy with the maximum EI~$u^*_t$ and the strategy of minimum budget with EI larger than $\frac{1}{T - t}\hat{u}^*_t$, which correspond to the solutions of Eq.\eqref{eq:the_optimization} and Eq.~\eqref{eq:delta_strategy}, respectively, can be found in linear time.

% The entire process is repeated a number of iterations~$T$, which is an hyperparameter of our method. 

% \subsection{Trajectory optimization}
% To overcome the second challenge of solving Eq.~\eqref{eq:the_optimization}, we chose to optimize the function over a trajectory starting at the current strategy~$(C_0, S_0)$ and ending at the budget line~$B$.

% Initially, we count with a dataset of annotated data~$(\T_c, \T_s)$ that contains $C_t$~classification and $S_t$~segmentation annotations respectively and accounts for a budget $B_0$. Our goal is to maximize~$u$ at the budget line. Let~$\hat{u}_t$ be a surrogate function for~$u$. While naively the strategy~$(C, S)$ that maximizes~$\hat{u}_t$ could be a solution, this is sub-optimal if $B \gg B_0$. Therefore, we opt for an iterative approach that finds the optimal data annotation trajectory at each step, defined as the one that maximizes~$\hat{u}_t$ along the line. Following a Bayesian optimization approach~\cite{jones1998efficient}, we model~$\hat{u}_t$ as the expected improvement~(EI),
% \begin{equation}
