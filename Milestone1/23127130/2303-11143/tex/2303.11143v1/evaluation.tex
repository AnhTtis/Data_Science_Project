...
In this section, we discuss the dataset we used for evaluating our attacks and the corpus we used for training the embedding model of Spatial Greedy.

\begin{table}[t!]
	\ra{1.2}%
	\caption{Binary functions extracted from each open-source project.\label{tab:totFuncs}}
	\centering
	\begin{tabular}{p{0.2\textwidth}r}\toprule
		{\textbf{Project}} & {\textbf{Functions}}  \\ \midrule
		binutils                           & 4199 		 \\
		curl                                    & 143				 \\
		gsl                                     & 2400 				 \\
		libconfig                               & 172				 \\
		libhttp                                 & 247 					 \\
		openssl                                 & 1074				 \\ \midrule
		\textbf{Total} & 8235					  \\
		\bottomrule
	\end{tabular}
\end{table}
...
We test our approaches by considering pairs of binary functions randomly extracted from 6 open-source projects written in C language: binutils~\cite{binutils}, curl~\cite{curl}, gsl~\cite{gsl}, libconfig~\cite{libconfig}, libhttp~\cite{libhttp}, and openssl~\cite{openssl}. We compile these programs for an \texttt{x86-64} architecture using the \texttt{gcc} 9.2.1 compiler with \texttt{O0} optimization level on Ubuntu 20.04. We then filter out all the functions made by less than six assembly instructions. Table~\ref{tab:totFuncs} reports the number of binary functions available from each program after the filtering.

To evaluate the robustness of the three target models against our proposed approaches, we used two datasets for the \textbf{targeted} scenario, each made of 500 pairs of binary functions sampled from the general dataset of Table~\ref{tab:totFuncs}. For both datasets, the same function cannot be considered twice as source function, but it can appear as target more than once. The first dataset, which we call \textsf{Random}, consists of pairs of random functions: function in a pair differ at most by 1345 instructions, and on average by 135.27. The second dataset, which we call \textsf{Balanced}, includes pairs of functions having similar length. In particular, they differ at most by 10 instructions, and on average by 5.47. We make no attempt to balance either dataset by considering the number of CFG nodes: for them, we measure an average difference of 17.8 nodes in \textsf{Random} and 7.5 in \textsf{Balanced}.
...
\subsection{Dataset used for Spatial Greedy}
...
\begin{mybox}
	\textbf{RQ1}: \textit{Are the three target models more robust against targeted or untargeted attacks?}%Which is the average cost of attacking such models in both scenarios?}
...
After detailing the experimental setup, in the following we first present the complete results for our attacks; then, we tackle our four research questions as separate sections.
...
An attack is successful depending on the similarity value between the adversarial sample and the target function. For a targeted attack, the similarity score has to be increased during the attack until it trespasses a success threshold \T. For an untargeted attack, this score, which is initially 1, has to decrease until it is below a success threshold \U. Operatively, the values of such thresholds are determined by the way the similarity score is used in practice. In our experimental evaluation, we choose the thresholds as follows. We compute the similarity scores that our attacked systems give over a set of similar pairs and over a set of dissimilar pairs. We obtain two distinct intervals of values: one is given by the similarity scores that are within one standard deviation from the average of the scores of dissimilar functions; the other one is the analogous for the similar functions. In these two intervals, we pick the thresholds to be used to determine successful attacks. Operatively, our threshold for a targeted attack is $\mT=0.8$ and the one for an untargeted attack is $\mU=0.5$. 

\paragraph*{\textbf{Black-box Attacks}}\label{sec:blacksetup}
To evaluate the effectiveness of Spatial Greedy against the baseline Greedy, we select a maximum perturbation size $\bar{\delta}$ and a number of dead branches $B$ in four settings: \textbf{C1} ($\bar{\delta}=15$, $B=5$), \textbf{C2} ($\bar{\delta}=30$, $B=10$), \textbf{C3} ($\bar{\delta}=45$, $B=15$), and \textbf{C4} ($\bar{\delta}=60$, $B=20$). 
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=3.5in]{BB_targeted.pdf}
	\caption{\textbf{Black-box targeted} attack with Spatial Greedy against the three target models on the \textsf{Random} \textbf{(a)} and \textsf{Balanced} \textbf{(b)} datasets, while varying the success threshold \T. We use a set of candidates of 400 instructions, $\varepsilon = 0.1$, and $r = 0.75$.} 
...
\begin{table*}[t!]
	...
	\resizebox{\linewidth}{!}{%
		\begin{tabular}{c|c|cccc|cccc|cccc|cccc} 
			\toprule
			\multirow{2}{*}{\textbf{Target}} & \multirow{2}{*}{\textbf{Attack}}                        & \multicolumn{4}{c|}{\textbf{A-rate (\%) ($\mU=0.50$)}} & \multicolumn{4}{c|}{\textbf{M-size ($\mU=0.50$)}}  & \multicolumn{4}{c|}{\textbf{A-sim ($\mU=0.50$)}}   & \multicolumn{4}{c}{\textbf{N-dec ($\mU=0.50$)}}     \\ 
			\cline{3-18}
			&                                                         & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4}     & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4}  \\ 
			\toprule\toprule
			...
			\cline{2-18}
			\multirow{-2}{*}{SAFE} & SG & 56.49       & 80.83       & 87.42       & \textcolor{red}{\textbf{90.62}}           & 6.67        & 7.74        & 7.77        & 7.64        & 0.39        & 0.41        & 0.42        & 0.42        & 0.60        & 0.59        & 0.58        & 0.59         \\
			\bottomrule
		\end{tabular}}
\end{table*}
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=3.5in]{BB_WB_untargeted.pdf}
	\caption{\textbf{(a) Black-box untargeted} attack with Spatial Greedy against the three target models while varying the success threshold \U and the settings \textbf{C1}, \textbf{C2}, \textbf{C3}, and \textbf{C4}. We use a set of candidates of 400 instructions, $\varepsilon = 0.1$, and $r = 0.75$. \newline \textbf{(b) White-box} \textbf{untargeted} attack against the three target models while varying the success threshold \U and the settings \textbf{C1}, \textbf{C2}, \textbf{C3}, and \textbf{C4}. \textit{Left}: GCAM attack with 40k iterations against GEMINI. \textit{Center}: GCAM attack with 1k iterations against GMN. \textit{Right}: GCAM attack with 1k iterations against SAFE.}
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=3.5in]{WB_targeted.pdf}
	\caption{\textbf{White-box} \textbf{targeted} attack against the three target models on the \textsf{Random} \textbf{(a)} and \textsf{Balanced} \textbf{(b)} datasets while varying the success threshold \T and the settings \textbf{C1}, \textbf{C2}, \textbf{C3}, and \textbf{C4}. \textit{Left}: GCAM attack with 20k iterations against GEMINI. \textit{Center}: GCAM attack with 1k iterations against GMN. \textit{Right}: GCAM attack with 1k iterations against SAFE.} 
...
\begin{table*}[t!]
	...
	\resizebox{\linewidth}{!}{%
		\begin{tabular}{c|cccc|cccc|cccc|cccc} 
			\toprule
			\multicolumn{1}{l|}{\multirow{2}{*}{\textbf{Target}}} & \multicolumn{4}{c|}{\textbf{A-rate (\%) ($\mU=0.50$)}}                 & \multicolumn{4}{c|}{\textbf{M-size ($\mU=0.50$)}}                     & \multicolumn{4}{c|}{\textbf{A-sim ($\mU=0.50$)}}   & \multicolumn{4}{c}{\textbf{N-dec ($\mU=0.50$)}}     \\ 
			\cline{2-17}
			\multicolumn{1}{l|}{}                                 & \textbf{C1} & \textbf{C2} & \textbf{C3}                     & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3}                    & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4}  \\ 
			\toprule\toprule
			...
			\hline
			SAFE                                                  & 68.46       & 83.43       & \textcolor{red}{\textbf{88.42}}      &  87.42       & 17.52       & 30.01       & 36.0       & 38.49       & 0.39        & 0.38        & 0.39        & 0.40        & 0.56        & 0.58        & 0.58        & 0.58         \\
			\bottomrule
	\end{tabular}}
\end{table*}
...
We now discuss the modification size \textbf{M-size} metric: fixing $\mT=0.80$ and considering the setting where \textbf{A-rate} peaks, we measure an \textbf{M-size} value of 38.90 for SAFE (\textbf{C4}), 133.84 for Gemini (\textbf{C4}), and 350.50 for GMN (\textbf{C2}):
SAFE is the model that sees the insertion of fewer instructions. A possible explanation is that SAFE internally truncates functions at a 150-instruction length: an attacker has no incentive in adding instructions once this threshold has been reached, as the similarity score would not change.
...
\begin{figure}[h!]
 	\centering
 	\includegraphics[width=3.5in]{greedySpatial_Targeted.pdf}
 	\caption{\textbf{Greedy} and \textbf{Spatial Greedy} targeted attacks against the three models while varying the success threshold \T, using the \textsf{Random} dataset and setting \textbf{C4}. For both, we consider $\varepsilon = 0.1$ and $|\texttt{CAND}| = 400$. For Spatial Greedy, we also set $r = 0.75$.}
...
When looking at attack success rate \textbf{A-rate} for all thresholds of similarities, the three target models are less robust against untargeted attacks (rather than targeted ones) regardless of the adversarial knowledge. For the best attack among black-box and white-box configurations, in the targeted scenario, the peak \textbf{A-rate} at $\mT=0.80$ is of 27.54\% for Gemini, of 59.68\% for GMN, and of 60.68\% for SAFE. For the untargeted scenario, the peak \textbf{A-rate} at $\mU=0.50$ is 53.89\% for Gemini, 91.62\% for GMN, and 90.62\% for SAFE.
...
For the untargeted scenario, our results (Tables~\ref{tab:BBUntargeted} and~\ref{tab:WBUntargeted}) for the \textbf{A-rate} metric considering $\mU=0.50$ show that Spatial Greedy has a slight advantage on GCAM. For Spatial Greedy, we have best-setting values of 53.89\% for Gemini, 91.62\% for GMN, and 90.62\% for SAFE; for GCAM, we have 39.52\% for Gemini, 84.63\% for GMN, and 88.42\% for SAFE.
 ...
 \begin{mybox2}
 {\bf Take at home:} Our tests show that the Spatial Greedy black-box strategy is on par or beats our white-box GCAM attack based on a rounding inverse strategy.  Further investigation is needed to confirm if this result will hold for more refined inverse feature mapping techniques and when attacking other models. 
 \end{mybox2}
...
\begin{mybox2}{\bf Take at home:} The presence of CFG-based features in the attacked model plays an important role when manipulating pairs of functions that are unbalanced in the number of CFG nodes, requiring a greater effort by the attacker. Untargeted attacks are unaffected. \end {mybox2}
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=3.5in]{BalUnbal.pdf}
	\caption{Resilience of the three models to \textbf{targeted} \textbf{black-box} and \textbf{white-box} attackers on the \textsf{Random} and \textsf{Balanced} datasets for a different success threshold \T. The best setting for each attack is shown.\newline 
