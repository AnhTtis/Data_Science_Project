...
GPS tracks were obtained for hikes in the UK from Hikr and OpenStreetMap. Visual inspection of the tracks showed that a large number were not walking tracks, as their speed was implausible, or contained long breaks which could impact the accuracy of a walking speed model. We developed a series of filters to identify and remove significant breaks and tracks or segments which were not hike or walking based (see Methods for details).

This gave us a dataset of 7,636 GPS tracks, consisting of over 1.4 million individual data points and almost 88,000 km of travel. Each datapoint contained:
\begin{itemize}
...
\end{itemize}
The elevation and slope values were calculated using data from the Ordnance Survey Terrain 5 dataset. Hill slope values were found using the quadratic surface method, and measure the hill slope at the starting co-ordinate. The on-road and paved road flags were determined by searching a 50 m radius around each point in the OSM Road dataset. Obstruction data was calculated using the lidar dataset, as the difference in values between the DSM and DTM. Heavy obstruction points are those with an obstruction value of greater than 10 cm.
