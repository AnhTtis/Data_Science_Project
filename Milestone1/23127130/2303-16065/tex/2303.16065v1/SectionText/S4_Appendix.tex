\section*{S4 Appendix.Exploring the impact of terrain obstruction.}
...
When modelling the data for the separate datasets (`obstruction available' vs `no obstruction available'), we see that the models are very similar when ascending or descending a slope (Fig \ref{fig11}A). This was not the case when traversing the slope however, as the `no obstruction available' model predicts that hill slope has a greater impact on reducing walking speed than the `obstruction available' data model (Fig \ref{fig11}B). 
