...
When modelling the data for the separate datasets (`obstruction available' vs `no obstruction available'), we see that the models are very similar when ascending or descending a slope (Fig \ref{Fig14}A). This was not the case when traversing the slope however, as the `no obstruction available' model predicts that hill slope has a greater impact on reducing walking speed than the `obstruction available' data model (Fig \ref{Fig14}B).

\begin{figure}[!h]
    \includegraphics{Images/Paper/Fig14.eps}
