...
\vspace{-1.3cm}
\begin{align*}
   {}^* \textit{\small Imperial College London} 
\end{align*}
\begin{abstract}
...
When studying mixing systems, a natural question is how fast convergence of \eqref{correlation} occurs. Two commonly employed techniques to address this question are the spectral gap method and the construction of Young towers for the system $f$. The first technique involves identifying a suitable Banach space $F\subset L^1(M,\mu)$ such that the Perron-Frobenius operator induced by $f$ exhibits the spectral gap property (see \cite{CarTransf, BTransf,Btranf2,CarlangeloTransf}). The second  requires finding a subset $\Delta\subset M$ and a return function $R: \Delta \to \mathbb N$ such that $f^R(x) = f^{R(x)}(x)\in \Delta$ for $\mu$-almost every $x\in \Delta$, and $f^R :\Delta \to \Delta$ is a Gibbs-Markov map (for more details, see \cite[Chapters 3 and 6]{Alves}).


The question of how fast convergence of \eqref{correlation} occurs can be extended to the random dynamical systems context. Given a random dynamical system $f_\omega^n: M\to M$ over an $\mathbb P$-invariant ergodic dynamical system $\theta:\Omega \to \Omega$ one may consider how fast
\begin{align}
...
\end{align*}
Considering the map $\theta: \Omega \to \Omega,$ $\theta  ((\omega_i)_{i\in\mathbb Z} ) = (\omega_{i+1})_{i\in\mathbb Z},$ it is readily verified that for every $n,m \in\mathbb N$ 
...
\begin{notacao}
We denote by $m$  the Lebesgue measure on $\mathbb S^1$. When integrating a function $\varphi:\mathbb S^1 \to \mathbb R$ over the measure $m,$ we simply denote
$$\int_{\mathbb{S}^1} \varphi(x) \d x := \int_{\mathbb{S}^1} \varphi(x) m(\d x). $$

Given a function $\varphi \in L^\infty(\mathbb{S}^1,m)$ we define 
$$\| \varphi \|_\infty = \inf\left\{ \sup_{M \subset \mathbb S^1} |\varphi(x)|; \ m(M) = 1\right\}.$$
Moreover, given a Lipschitz function $\psi :\mathbb S^1\to \mathbb R$ we denote 
$$\|\psi\|_{\mathrm{Lip}} = \|\psi\|_{\infty} + \sup_{x\neq y}\frac{|\psi(x)- \psi(y)|}{d(x,y)},$$
where $d$ is the arclength distance between two points in the circle. Since $d$ is translation invariant, by abuse of notation, given $x,y \in\mathbb S^1$ we denote $|x-y| = d(x-y,0)= d(x,y).$

Given a topological space $X$, we denote $\mathcal B(X)$ as the Borel $\sigma$-algebra of $X$. Consider a topological space $Y$, a family of probability measures $\{\mu_y(\d x) \}_{y \in Y}$ on $X,$ and a probability measure $\mathbb P(\d y)$ on $Y$. If the map $y \to \mu_y(A)$ is measurable for every $A\in \mathcal B(X)$, we denote   ${\mu(\d x , \d y) := \mu_y(\d x) \mathbb P(\d y)}$ as  the unique probability measure on $X\times Y$ such that
...
We mention that the above theorem is sharp in the sense that if  $0<\varepsilon \leq  1/\alpha$, then the above result may not hold since the system $f_\omega^n$ may present sinks with a basin of attraction of positive Lebesgue measure. Furthermore, it is worth noting that Theorem \ref{main} is not dependent on any particular characteristics of $\xi$ aside from its non-degeneracy, thus showing a significant difference between the established theory of deterministic dynamical systems and its stochastic counterpart.


\subsection{Random Young towers}\label{randomtowers}
The groundwork for the proof of Theorem \ref{decay} is the theory of random Young towers. In this subsection, we define the concept of random Young towers following closely the definitions provided in \cite{H, Baladi, Du}.

For simplicity, consider $(\Omega,\mathcal F,\mathbb P)$ and $\theta: \Omega \to \Omega$ as in Section \ref{Model}. Let $M$ be a compact metric space endowed with a probability measure $\rho$, and $\{g_\omega: M \to M\}_{\omega \in \Omega}$ a family of maps depending only on the $0^{\mathrm{th} }$ coordinate of $\omega.$ We say that $g_\omega$ \emph{admits a random  tower on $\Delta \subset M$} if there exists a measurable return function $R:\Omega\times \Delta\to \mathbb N$, and for $\mathbb P$-almost every $\omega \in \Omega$ there exists an $\rho$-mod $0$ countable partition $\mathcal P(\omega) =\{P_j(\omega)\}_{j\in \mathbb N}$ of $\Delta,$ such that $R_\omega= R(\omega,\cdot)$ is constant on each $P_j(\omega)$ and
...
We refer to $\Delta_{\omega,\ell} := \{(x,\ell) \in \Delta_\omega\} := \{(x,\ell); R_{\theta^{-\ell} \omega}(x) > \ell\}$ as the \emph{$\ell$-th level of the tower.} Considering $$\mathbbold{\Delta} = \{(\omega , (x,\ell)); \omega\in \Omega \ \text{and}\ (x,\ell)\in \Delta_\omega\},$$
...
\begin{enumerate}
    \item[(P1)] {\bf{Markov}}: For each $P\in \widehat{\mathcal P}(\omega)$ the map $F_\omega^{\widehat{ R}} : P \to \Delta_{\omega, 0}$ is $\rho_\omega$-mod $0$ bijection;
     ...
     $$\log \frac{|\d F_\omega^{{\widehat R}} (x) |}{|\d F_\omega^{{\widehat R}} (y) |} \leq C_0 \beta^{ s(F^{\widehat R}(\omega,x), F^{\widehat R}(\omega,y)) }. $$
     \item[(P3)]{\bf{Weak expansion}:} $\widehat{\mathcal P}(\omega)$ is a generating partition for $F_\omega$, i.e., the diameter of $\bigvee_{j=0}^{n} F_\omega^{-j}\widehat{\mathcal P}^{j}(\theta^j\omega)$ goes to $0$ as $n\to\infty.$
...
\begin{mtheorem}\label{TheoremB}
 Consider $f_{\omega}^n$ as the random dynamical system defined in Section \ref{Model}. Let $c\in (0,1)$ and $a\in\mathbb S^1$. Then, for every $\alpha$ sufficiently large and for $\varepsilon>\alpha^{c-1}$, there exists an interval $\Delta\subset \mathbb S^1$, such that $f_\omega$ admits a random Young tower structure on $\Delta.$ Moreover, there exists a function $C \in L^2(\Omega, \mathbb P)$ and $\gamma > 0,$ such that the return function $R$, satisfies $ m [R_\omega > n] \leq  C(\omega) e^{-\gamma n},$ where $m$ is the Lebesgue measure on $\mathbb S^1.$
\end{mtheorem}
Theorem \ref{TheoremB} is proven in Section \ref{proofB}.  

\section{Young times}

In \cite[Chapter 6]{Alves} using hyperbolic times, the author introduces a technique for constructing a Young tower applicable to non-uniformly expanding maps. However, this method does not readily extend to random systems. To address this issue, we propose a solution by defining the concept of Young times. In broad terms, Young times can be described as hyperbolic times such that $f_\omega^n$ present bounded distortion estimates when returning the reference set. 
...
Choose $x_0$ as any point in $\mathbb S^1$ such that $\d f(x_0) \neq 0.$  Take $\alpha_2 > 0$ such that for every $\alpha > \alpha_2$
\begin{align}
...
%   \end{align}
Before, we recall the definition of hyperbolic times. The following definition is adapted from  \cite[Section 6.1.1]{Alves}. We start fixing a real number $0 <b < 1/2.$
...
\begin{align*}
\prod_{i=k}^{n-1}\left|f'_{\theta^i\omega} (f_{\omega}^i(x))\right|^{-1} \leq \sigma^{2(n-k)},\ \text{and } \mathrm{dist}_{r}(f^{n-k}_\omega(x), \mathscr{C}_{\theta^{n-k}\omega}) \ge \sigma^{b (n-k)}, 
\end{align*}
...
The result below states a consequence of $n$ being a $(\sigma^2,r)$-hyperbolic time for $(\omega,x)$.  The following result is an adaptation of \cite[Proposition 6.7]{Alves}, where the same proof can be naturally adapted to the random context.
...
\begin{enumerate}\label{HipBalls}
    \item[(i)] There exists an open neighbourhood $V_n^\delta(\omega,x)$ of $x$, such that $f^n_\omega$ maps $V_n^{\delta}(\omega,x)$ diffeomorphically to $B_{\delta} (f_\omega^n(x)).$
...
Let $L$ be a natural number. Consider the family of random variables $\{\tau_i: \Omega\times \mathbb S^1 \to\mathbb N\cup \{\infty\}\}_{i\in\mathbb N},$ where $$ \tau_i(\omega,x) :=\begin{cases}
...
Let $c\in (0,1)$. For $\alpha>0$  sufficient large, and for $\varepsilon>\alpha^{c-1},$ there exists $0<\sigma < 1$ and $r>0$, independent of $L$,  such that 
...
Let $Y_n(\omega)$ be as in \eqref{YoungTijmen} and $c\in(0,1)$. Then for $\alpha>0$ sufficiently large and for $\varepsilon>\alpha^{c-1},$  there exists $\theta_1 >0$ such that for every every $x\in\mathbb S^1,$  
...
Let $c\in (0,1)$. For sufficiently $\alpha >0$ large  and for $\varepsilon >\alpha^{c -1}$, we obtain that:
\begin{enumerate}
    ...
    \item[$(I_2)$] There exists a constant $C>0,$ such that for every $x\in \tilde{H}_n(\omega)$, $V_n(\omega,x)$ is diffeomorphically mapped in a disk of radius $9\delta_1$ around $f^n_\omega(x)$, and for every $y,z \in V_n(\omega,x)$
    \begin{itemize}
...
For $\mathbb{P}$-almost every $\omega = (\omega_i)_{i\in\mathbb Z}$, we aim to construct a $m$-mod $0$  partition $\mathcal P(\omega)$ of $\Delta$ that can be used to build a Young Tower. To this end, we inductively define the families of sets $\{\mathcal P_n(\omega)\}_{n\in\mathbb N},$ $\{\Delta_n(\omega)\}_{n\in \mathbb N}$ and $\{S_n(\omega)\}_{n\in\mathbb N}$ depending only on $(\omega_0,\ldots, \omega_{n-1}),$ in a way that
\begin{itemize}
...
In the case that $n-k\geq N_1,$ we set
\begin{align}
...
\begin{teorema}
Given $c\in (0,1)$. For every $\alpha > \alpha_4$ (see \ref{defa4}) and for $\varepsilon > \alpha^{c-1}$, the partition $\mathcal P(\omega)$ constructed above is a $m$-$\mathrm{mod}\ 0$ partition.
\end{teorema}
...
$$\overline{V_n(\omega,x)} = \overline{f^{-n}_\omega \left(B_{9\delta_1}(f_\omega^n(x))\right)} \cap \overline{B_{9\delta_1 \sigma^n}(x)}.$$ The above statement is not true in general. However, the above equality holds since $\left.f_\omega\right|_{V_n(\omega,x)}$ is a diffeomorphism. Let $U\subset \mathbb S^1$ be an open set, and consider
...
From the above observation if follows that
...
Now, suppose that $n\geq N_0$ and $w_{n,\ell} \in \mathcal P_n(\omega),$ we have that $w_{n,\ell} \subset V_n(\omega,z)$ for some $z \in H_n(\omega)$. Moreover $f^{n+\ell}_\omega$ maps $w_{n,\ell}$ diffeomorphically to $(x_0-\delta_0,x_0-\delta_0).$ From Lemma \ref{i1i2i3}, $(I_2)$ and $(I_3)$ we obtain that
...
The last one follows from Lemma \ref{CentralLema} $(I_2)$ and $(I_3)$ with
...
\begin{step}{3} We show (P5) in Section \ref{randomtowers} holds, which completes the proof of the theorem.
\end{step}
...
\begin{theoremproofA}
    The result follows directly when combining Theorems \ref{TheoremB} and \ref{Du}. We mention that when applying Theorem \ref{Du} to the random dynamical system $f_\omega^n $, we set $g_\omega = f_\omega$ and $\nu= \mu$ (see \eqref{Thetainvmes}).
\end{theoremproofA}
...
This appendix is devoted to proving Proposition \ref{invmes}. While similar results have been established in the literature (e.g. \cite{Alves, Baladi, Du}), we provide an alternative proof of the existence of an invariant measure based on the aforementioned papers. We note that our approach details the arguments presented in the aforementioned references, and we believe our proof contributes to the existing literature on the topic.
...
\begin{proof}
Assume that $s(\omega,z_1,z_2) >0,$ otherwise, there is nothing to prove. From Lemma \ref{fr}, we obtain that the result is valid for $n= {\widehat R}(\omega,z_1) = {\widehat R}(\omega,z_2).$ 

Suppose that $n= {\widehat R}_i(\omega,z_1)$ and observe that
$$ s(\theta^{{\widehat R}_i}\omega, F^{{\widehat R}_i}_\omega(z_1),F^{{\widehat R}_i}_\omega (z_2)) = s_\omega(z_1,z_2) - i.$$
It follows that
\begin{align*}
    \log \frac{|\d F^{{\widehat R}_i}_\omega(z_1)|}{|\d F^{{\widehat R}_i}_\omega(z_2)|}&= \sum_{j=0}^{i-1} \log \frac{| \d F_{\theta^{{\widehat R}_j}\omega} (F^{{\widehat R}_j}_\omega (z_1) )| }{| \d F_{\theta^{{\widehat R}_j}\omega} (F^{R_j}_\omega (z_2) )| }\\ &
    \leq \mathrm{diam}(\Delta_0)C \sum_{j=0}^{i-1}\beta^{s(\theta^{{\widehat R}_j} \omega, F_\omega^{{\widehat R}_j} (x), F_\omega^{{\widehat R}_j}(y) )}\\
    &\leq \mathrm{diam}(\Delta_0)C \sum_{j=0}^{i-1} \beta^{n-i}\beta^{s(\theta^{{\widehat R}_i} \omega, F_\omega^{{\widehat R}_i} (x), F_\omega^{{\widehat R}_i}(y) )}\\
    &\leq \frac{\mathrm{diam}(\Delta_0)}{1-\beta} \beta^{s(\theta^{{\widehat R}_i} \omega, F^{{\widehat R}_i}_\omega z_1, F^{{\widehat R}_i}_\omega z_1) } 
    % =\frac{\mathrm{diam}(\Delta_0)}{1-\beta} \beta^{s(\omega, z_1,z_2) - i }
 \end{align*}

Now, observe that since for every ${\widehat R}_{i-1}(\omega,x) < n < {\widehat R}_{i}(\omega,x)$ with $0<i \leq s_\omega (x,y),$ we obtain that
$s(\theta^n \omega , F^n_\omega x, F^n_\omega y) = s(\theta^{{\widehat R}_i}\omega, F^{R_i}_\omega (x) ,  F^{{\widehat R}_i}_\omega (y) ),$
and $|\d F^n_\omega (x)| = |\d F^{{\widehat R}_i}_\omega(x)|$, we obtain the desired inequality.
\end{proof}

In the following, we define sets that are useful to prove Proposition \ref{invmes}. We start defining $$\Delta_0 := \Delta_{\omega,0} = \{(x,0); x\in \Delta, R(\omega,x) >0\} = \Delta\times\{0\},$$
...
Consider $P = m_\omega(\d x) \mathbb P(\d \omega).$ Define $\mu_0 = \left. P\right|_{\mathbbold{\Delta}_0}/P(\mathbbold{\Delta}_0),$ where $$\mathbbold{\Delta}_0 = \{(\omega,(x,0)); \omega\in \Omega, x\in \Delta \}.$$ Denote $\mu_n =  F^n_*\mu_0. $
...
\begin{align}
    \left| \log \frac{\phi_{\ell,j}^n (\theta^n \omega,y )}{\phi_{\ell,j}^n (\theta^n \omega,x)} \right| &= \left|\log\frac{\d F_\omega^n (x')}{\d F_\omega^n (y')}\right|\leq C_0 \beta^{s(\theta^n\omega, F_\omega^n (x'), F_\omega^n (y') )} = C_0 \beta^{s(\theta^n \omega,x,y)}\label{DA}
\end{align}
therefore
\begin{align}
