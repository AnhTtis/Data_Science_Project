...
2) if $I\subset \{1,\cdots,r\}$, then $\prod_{i\in I}P_i$ is WR if and only if $\dfrac{m}{4}\le \tron{\prod_{i\in I}p_i}^2\le m$.
...
\end{lemma}
From now on, we will use one of the integral bases as mentioned in Lemma \ref{integralbasis-3notdiv9} depending on which one is convenient for our calculation. It is easy to prove Lemma \ref{lencoeff}.
...
\begin{lemma}\label{traceP}
	$M_{p_i}=P_i$. As a consequence, $p_i| \Tr(z)$ for all $z \in P_i$.
\end{lemma}
\begin{proof}
...
\begin{proof}
	By Lemma \ref{traceP}, we have $P_i =\langle p_i, \alpha-n_i\rangle $. The statement $g\in P_i$ is implied from the equalities $\alpha-\sigma(\alpha) = (\alpha-n_i)+(\sigma(\alpha)-n_i)$ and $\sigma(P_i)=P_i,\forall i=1,\cdots r$.
...
The following result is new and has not been studied before. We remark that our WR lattice $P$ in Proposition \ref{prop:Psquare_is_WR}  is not one of the sublattices mentioned in \cite[Theorem 4.9]{DM20}.
...
\begin{proposition}\label{prop:cubic9ndividem}
	Let $m = p_1 \cdots p_r$ be the conductor of $F$ and let $P_i$ be the prime ideals above $p_i$ for all $i=1,\cdots r$. For each nonempty subset $I$ of $\{1,\cdots ,r\}$, let $P_I = \prod_{i\in I }P_i,p_I=\prod_{i\in I }p_i $ and $n_I = \dfrac{p_I-1}{3}$. Then $P_I$ is WR if and only if $\dfrac{m}{4}\le p_I^2 \le 4m$. 
\end{proposition}
\begin{proof}
	...
	Let $\delta$ be  a nonzero vector of $P_I$. There exist integers $x_1,x_2,x_3$ such that $\delta = x_1\tron{\alpha+n_I}+x_2\tron{\sigma(\alpha)+n_I}+x_3\tron{\sigma^2(\alpha)+n_I}$.
...
\end{lemma}
\begin{proof}
	It is clear that $L_i =  \ZZ p_i\oplus\ZZ \alpha \oplus\ZZ \sigma(\alpha)$ is a  sublattice of $P_i$ and $\det(L_i) =\det( P_i)$. Therefore $P_i = L_i$ by Lemma  \ref{sublatticeequal}.
\end{proof}
...
\begin{proposition}\label{prop:P0Psquare_WR}
	The ideal $P_0(P_1\cdots P_r)^2$ is WR with a minimal basis 
	$\{\kappa,\sigma(\kappa),\sigma^2(\kappa)\}$ where $\kappa$ is in Lemma \ref{lem:idealP1Pr}.
\end{proposition}
...
\begin{proof}
	...
	\end{enumerate}
	It implies that $\min_{\delta \ne 0}\|g\|^2 = \min\set{2m,27p_I^2, 3p_I^2+\dfrac{2m}{3}}.$ Since $\Tr(\rho_I)\ne 0$, the ideal $P_0P_I$  WR if and only if $\min_{\delta \ne 0} \|\delta\|^2 =  3p_I^2 +\dfrac{2m}{3}$. It is equivalent to the statement $ 3p_I^2+\dfrac{2m}{3}\le 2m$ and $ 3p_I^2\le 27p_I^2$. These inequalities occur if and only if $\dfrac{m}{36}\le p_I^2\le \dfrac{4m}{9}$.\end{proof}
