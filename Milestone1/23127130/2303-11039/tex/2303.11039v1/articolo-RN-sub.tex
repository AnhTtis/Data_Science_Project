...
are RN null coordinates. Note that at the Cauchy horizon $t_{EF}$ goes to $+\infty$. A spacetime diagram representing three characteristic curves $u=cst$ in the RN portion of the spacetime is given in Fig. (\ref{figdue}). 
  \begin{figure}[h]
\centering \includegraphics[angle=0, height=3.5in] {fig2.pdf}
...
\section{QFT in the 2D RN spacetime}
\label{s3}
...
$\hat\Box=\nabla_\mu\nabla^\mu$. The field is in a state, we call it $|in\rangle$, to be Minkowski vacuum on past infinity (see Fig. (\ref{figuno})), which is a Cauchy surface for our field equation.
...
Now consider an observer as he crosses the event horizon $r=r_+$. 
...
More delicate is the behaviour of $\rho$ as the observer approaches the inner horizon $r_-$. If the observer crosses the event horizon $u=+\infty$ (and hence the inner horizon) at early time after the formation of the BH (see trajectory $A$ in Fig. (\ref{figdue})) he leaves very rapidly the large (positive) $u$ region and the measured energy density smoothly approaches the limiting value
...
However, if the observer is approaching the inner horizon at very late time (trajectory $B$ in Fig (\ref{figdue})) things are strikingly different \cite{Jacobson:1997ge}. The outgoing null rays inside $r_+$ peel away from the event horizon and asymptotically approach the inner horizon $r_-$ (see Fig. (\ref{figdue})) in a diverging $t_{EF}$. So the partners emitted just outside $r_+$ with $u\to +\infty$ get in the above limit arbitrarily close to the inner horizon where $f\ll 1$. So at very late time our free falling observer meets all these partners in a region where $f\ll 1$ but $u$ is still very large and positive. 
...
\noindent where remember that $r_-<r'<r_+$ and $r>r_+$. The plot of both sides of eq. (\ref{quattrodieci}) is given in Fig. (\ref{figtre}): they both go to $+\infty$ for both $r\to r_-$ and $r\to +\infty$ and to $-\infty$ for 
$r\to r_+$.
 \begin{figure}[h]
\centering \includegraphics[angle=0, height=2in] {fig3.pdf}
\caption{Plot of both sides of eq. (\ref{quattrodieci}). In this figure and in those \\ that follow we consider  $r_+=\frac{1}{8}=0.125,\ r_-=\frac{3}{32}=0.09375,\ \kappa_+=1,\ \kappa_-=\frac{16}{9}$ .}
\label{figtre}
\end{figure} 

From these one sees that the condition (\ref{quattrodieci}) can always be satisfied, i.e. for every $r>r_+$ a value of $r'$ exists such that $r_-<r'<r_+$ and (\ref{quattrodieci}) is satisfied. However if we plot the correlator (\ref{quattronove}) for both points close to $r_+$ no sign of the particle-partner correlation appears. In Fig. (\ref{figquattro}) the correlator (\ref{quattronove}) is represented graphically at values of $r'$ fixed as a function of $r$, while in fig. (\ref{figcinque}) the 3D plot is shown. 
 \begin{figure}[h]
\centering \includegraphics[angle=0, height=2in] {fig4.pdf}
\caption{Plot of the correlator (\ref{quattronove}),  
%where we considered $r_+=8,r_-=2$   ($m=5$, $Q=4$) and 
up to the factor $(\frac{\hbar}{4\pi})^2$, \\ for fixed values of $r'=0.122$ (blue curve), $0.123$ (orange) and $0.124$ (green).}
\label{figquattro}
\end{figure} 
 \begin{figure}[h]
\centering \includegraphics[angle=0, height=2.5in] {fig5.pdf}
...
Looking at Fig. (\ref{figtre}) one sees that all points with $r-r_+\geq \frac{1}{\kappa_+}$ are correlated with corresponding partners located very close to $r_-$. So let us consider the inner point $r'$ in this region. We have to distinguish two regimes. If the inner observer approaches the inner horizon $r_-$ at early time with respect to the formation of the BH (trajectory $A$ in Fig. (\ref{figdue})) we have that the corresponding $u'$ approaches $-\infty$ as
...
We represent graphically this correlator fixing the inner point very close to $r_-$ in Fig. (\ref{figsei}) and the 3D plot is shown in Fig. (\ref{figsette}). 
 \begin{figure}[h]
\centering \includegraphics[angle=0, height=2in] {fig6.pdf}
\caption{Plot of the correlator (\ref{quattrododici}),  
%where we considered $r_+=8,r_-=2$\\  (i.e. $m=5$, $Q=4$) and 
up to the factor $(\frac{\hbar}{4\pi})^2$, for $t_{EF}=1$, \\ inner point fixed at $r'=0.0937501$ and  $0.125<r<0.130$.}
\label{figsei}
\end{figure} 
 \begin{figure}[h]
\centering \includegraphics[angle=0, height=2in] {fig7.pdf}
\caption{3D Plot of the correlator (\ref{quattrododici}),   
%where we considered $r_+=8,r_-=2$ (i.e. $m=5$, $Q=4$) and 
up to the factor $(\frac{\hbar}{4\pi})^2$, \\ for $t_{EF}=1$ ,\ \   $0.09375<x'<0.093755$ and  $0.125<r<0.130$.}
\label{figsette}
\end{figure} 
Again no sign of the correlations appear. The reason for this is clear from Fig. (\ref{figdue}). The partners of the Hawking particles pile up at $r_-$ and only at late time they are intercepted by our inner observer close to $r_-$ (see trajectory $B$ in Fig. (\ref{figdue})). So in order to reveal the correlation we have to consider the limit in which $u'$ is given by (\ref{quattrosette}) and $r'$ is close to $r_-$. The correlator is given by eq. (\ref{quattronove}) with $r'\to r_-$ and the corresponding plots are in Figs. (\ref{figotto}) and (\ref{fignove}). 
 \begin{figure}[h]
\centering \includegraphics[angle=0, height=2in] {fig8.pdf}
\caption{Plot of the correlator (\ref{quattronove}),  
%where we considered $r_+=8,r_-=2$\\  (i.e. $m=5$, $Q=4$) and 
up to the factor $(\frac{\hbar}{4\pi})^2$, with \\ the inner point fixed at $r'= 0.0937501$ and  $0.125<r<20$.}
\label{figotto}
\end{figure} 
 \begin{figure}[h]
...
\label{fignove}
\end{figure} 
We see neatly the appearance of the foreseen peak. The corresponding $r$ is not exactly the one given by the condition (\ref{quattrodieci}) because of the nontrivial role played by the geometric prefactor containing $\sqrt{ \frac{2m(r)}{r}} $. 
...
\acknowledgments
A.F. acknowledges partial financial support by Grant No. PID2020–116567 GB-C21 funded
