...
\title{The Geometrical Lemma for Smooth Representations in Natural
Characteristic}
...
subgroup with Levi quotient $L$ and unipotent radical $N$, then the Geometrical
...
However, if $k$ is a field of characterestic $p$, then the proofs break down
completely. Since there is no modulus character, one cannot talk about
normalization. We denote $\pInd_{P}^{G} \colon \Rep_k(M)\to \Rep_k(G)$ the
...
$\Rep_k(H)$ of smooth $k$-linear representations of $G$. The parabolic induction
extends to a derived functor $\R \pInd_P^G\colon \D(M)\to \D(G)$. By the main result
of \cite{Heyer.2022}, there exists a left adjoint $\L(U,\blank)$. To state the
...
triangulated functor, we refer to Definition~\ref{defn:filtration}.

\begin{thmintro}[Corollary~\ref{cor:geometrical}] % {{{
\label{thmintro:geometrical}
The functor $\L(N,\blank)\circ \pInd_P^G \colon \D(M)\to \D(L)$ admits a filtration
of length $\lvert\orbitrep{P}{Q}\rvert$ with graded pieces of the form
\[
\pInd_{n^{-1}Pn\cap L}^L \circ (\omega_n\otimes_k\blank) \circ n_*^{-1} \circ
\L(M\cap nNn^{-1},\blank),
\]
...
this is the point where our proof departs from the classical approach. Instead,
we show that the Geometrical Lemma holds almost entirely for purely formal
reasons.
The only non-formal input seems to be that the derived inflation functor
...
and~\ref{prop:explicit-character}. This is applied in Remark~\ref{rmk:delta_w}
...
\cite[Conjecture~3.7.2]{EmertonII}, which to my knowledge has only been resolved
...
$P$, \ie, the unique simple quotient of $\pInd_P^G(\one)$; see
Theorem~\ref{thm:Generalized_Steinberg} and
Corollary~\ref{cor:Generalized_Steinberg}. To my knowledge, this is the first
...
The Geometrical Lemma is the content of
Corollary~\ref{cor:geometrical}. The preparatory lemmas concerning compact
induction and derived coinvariants are proved
in~\S\ref{ss:induction_coinvariants}; these in turn rely on the abstract results
about functors in monoidal categories which are presented
in~\S\ref{ss:abstract}. The various characters that implicitly appear in the
Geometrical Lemma are completely determined in~\S\ref{ss:duality}.

Regarding applications, we compute $\L^{-j}(N,V)$ whenever $V$ is a principal
series representation (Example~\ref{ex:LU(PS)}) or a generalized Steinberg
representation (Theorem~\ref{thm:Generalized_Steinberg} and
Corollary~\ref{cor:Generalized_Steinberg}). 
Finally, we use the Geometrical Lemma to compute many Ext-groups between
parabolically induced representations in Theorems~\ref{thm:PS}
and~\ref{thm:Ext}.
% }}}
...
Kieu Hieu Nguyen, Konstantin Ardakov, Claus Sorensen, and Karol Kozio\l.

% }}}

\section{Preliminaries}
\subsection{Notation and conventions} % {{{
If $\alg H$ is an algebraic group defined over a field $\field$, we denote its
group of $\field$-points by the corresponding lightface letter, that is,
$H = \alg H(\field)$.

We fix a field $k$ of characteristic $p>0$.

For a $p$-adic Lie group $G$, we denote $\Rep_k(G)$ the
Grothendieck abelian category of smooth $k$-linear $G$-representations. 

The (unbounded) derived category of $\Rep_k(G)$ is denoted $\D(G)$; it is a
\emph{tensor triangulated category}, that is, a triangulated category which is
symmetric monoidal and such that the functors $\blank\otimes_kX$ are
triangulated for any $X$ in $\D(G)$. The tensor unit is $\one = k[0]$. 
We denote the right adjoint of $\blank\otimes_kX$ by $\hom_{G}(X,\blank) \colon
\D(G)\to \D(G)$.
The \emph{smooth dual} of $X$ is denoted $X^\vee \coloneqq \hom_G(X,\one)$. 

The category $\D(G)$ is naturally enriched over $\D(k)\coloneqq \D(\{1\})$, the
unbounded derived category of the category of $k$-vector spaces; we denote
$\RHom_G(X,Y) \in \D(k)$ the derived Hom-complex, for each $X,Y\in \D(G)$; it
defines a triangulated functor in each variable.

The derived category $\D(G)$ comes with a natural t-structure. For any integer
$n\in\ZZ$ we denote $\D^{\le n}(G)$ (resp.\ $\D^{\ge n}(G)$) the full
subcategory of objects $X$ in $\D(G)$ satisfying $\H^i(X) = 0$ for all $i>n$
(resp.\ $i<n$). Denote $\H^i\colon \D(G)\to \Rep_k(G)$ the $i$-th cohomology
functor. Note that $\RHom_G(X,Y) \in \D^{\ge0}(k)$ provided $X\in \D^{\le0}(G)$
and $Y\in \D^{\ge0}(G)$.
% }}}
\subsection{General abstract nonsense} % {{{
\label{ss:abstract}
\subsubsection{} % {{{
The purpose of this section is to formulate some formal statements about
adjunctions in monoidal categories. These will only be applied in
Lemmas~\ref{lem:LN_Inf_commute}, \ref{lem:LN_cInd_commute},
and~\ref{lem:omega_LN_cInd} and hence this section should be skipped on first
reading.
...
the fourth line from the bottom. By the mating correspondence, this means that
...
the third line from the top. By the mating correspondence, this means that
...
the second line from the top. By the mating correspondence, this means that
...
fourth line from the top. By the mating correspondence, this means that
...
$l(\chi)\colon L'F\To GL$. Denote
$\varepsilon\colon LR\To \id_{\cat Y}$ and $\varepsilon'\colon L'R'\To
\id_{\cat Y'}$ the counits of the respective adjunctions.
Then the right diagram above commutes, since the diagram
\[
\begin{tikzcd}[proof,sep=large]
L'FR
...
Observe that $\RRR^G_K \cong \Res^G_K$
provided $K\subgroup G$ is an open subgroup; in this case we prefer to write
$\ind_K^G$ instead of $\cInd_K^G$.
% }}}
...
\]
which is right adjoint to restriction $\Res^G_H\colon \D(G)\to \D(H)$. By a
slight abuse of notation we write $\Ind_H^G$ for $\RInd_H^G$ in case
...
\[
\hom_G\bigl(\cInd_H^GX, Y\bigr) \raiso \RInd_H^G\hom_H\bigl(X, \RRR^G_HY\bigr)
\]
...
[g\mapsto f(g)\otimes w]$ is a natural isomorphism,
...
isomorphism $\hom_G(\cInd_H^GX,\blank) \Raiso \RInd_H^G \hom_G(X,\blank)
...
$\L_{H\cap N}(\one) \cong \one$. Then one has a commutative diagram
\[
\begin{tikzcd}
...
\Raiso \Inf^1_G$ is an isomorphism. Let us denote $\psi\colon
...
l(r(\alpha)^{-1})$. Lemma~\ref{lem:abstract-b} shows that the diagram
\[
...
and passing to the left mate, we obtain a natural map
\[
...
Let $G$ be a $p$-adic Lie group of dimension $d$ and denote $\lie{g}$ its
$\QQ_p$-Lie algebra (cf.~\cite[p.~100]{Schneider.2011}). The determinant of the
adjoint action of $G$ on $\lie{g}$ provides a character $\bigwedge^d\lie{g}
\colon G\to \QQ_p^\times$. Denote $\delta_G\colon G\to \QQ_p^\times$ the modulus
character of $G$ (cf.~\cite[Definition I.2.6]{Vigneras.1996}). By
\cite[Theorem~5.1]{Kohlhaase.2017} the character
$\bigwedge^d\lie{g}\otimes_{\QQ_p}\delta_G$ takes values in $\ZZ_p^\times$.
Denote $\lie{d}_G$ the composition 
\[
\lie{d}_G\colon G\xrightarrow{\bigwedge^d\lie g\otimes_{\QQ_p}\delta_G}
\ZZ_p^\times \longtwoheadrightarrow \FF_p^\times \subseteq k^\times.
\]
% }}}
...
We compute
\begin{align*}
\hom_G(\omega_G,\one) &\cong \R\H^0\bigl(G_2, \hom_{G\times
G}(\cInd_{\Delta(G)}^{G\times G}(\one), \one)\bigr) & &
\text{(\cite[Corollary~3.3.7]{Heyer.2022})}\\
&\cong \R\H^0\bigl(G_2, \RInd_{\Delta(G)}^{G\times
G}\hom_G\bigl(\one,\RRR^{G\times G}_{\Delta(G)}(\one)\bigr)\bigr) & &
...
canonically $\hom_G(\one,X) \cong X$ for each $X\in \D(G)$.
...
Thorem~3]{Verdier.1969}) shows that $\RRR^{G\times G}_{\Delta(G)}(\one)$
...
\begin{ex}\label{ex:explicit-character} % {{{
With the notation and assumptions of Lemma~\ref{lem:omega_LN_cInd}, we consider
the character $\omega_{H\backslash G} = \L_N \cInd_H^G(\one)$ in $\D(G/N)$. The
same computation as in the proof of Proposition~\ref{prop:duality-character}
shows
\[
\omega_{H\backslash G}^\vee \cong \R\H^0\bigl(N, \RInd_H^G \RRR^G_H(\one)\bigr)
\cong \R\H^0\bigl(H\cap N, \RRR^G_H(\one)\bigr),
\]
where the second isomorphism uses Remark~\ref{rmk:RN_Ind}. Since
$\RRR^G_H(\one)$ is invertible, we deduce an isomorphism $\omega_{H\backslash G}
\cong \L_{H\cap N}\bigl(\RRR^G_H(\one)^\vee\bigr)$. In the specific cases
appearing in \S\ref{s:geometrical}, the group $H\cap N$ has no non-trivial
characters; in this case, we deduce
\begin{equation}\label{eq:explicit-character}
\Inf^{G/N}_H \omega_{H\backslash G} \cong \RRR^G_H(\one)^\vee \cong
\omega_H^\vee\otimes_k \Res^G_H\omega_G.
\end{equation}
\end{ex} % }}}
% }}}
...
Let $k$ be a field of characteristic $p>0$. We fix a finite field extension
$\field/\QQ_p$.
...
$\alg Z_{\alg G}(\alg T)$. Denote $\ol{\alg P} = \alg M\ltimes \ol{\alg U}$ the
...
we denote $\Rep_k(H)$ the abelian category of smooth $k$-linear representations
...
\R\H^0(U,\blank)\circ \RRR^G_P$. For any integer $i$ we denote $\L^i(U,\blank),
...
we denote $\cInd_P^ZV$ the $k$-vector space of locally constant functions
...
translation on $\cInd_P^ZV$. Observe that $\cInd_P^ZV \subseteq \Ind_P^GV$
...
Y\to Y$. For any $k$-vector space $W$ we denote $\CCC^\infty_c(P\backslash Y,
...
support. We then find an open compact subgroup $K\subgroup G$ such that $f$
takes a constant value, say, $c_{(Pz)K}$ on each $(Pz)K \cap P\backslash Z''$. 
We put $c_{(Pz)K} \coloneqq 0$ whenever $(Pz)K\cap P\backslash Z'' = \emptyset$.
As the cosets $(Pz)K$ are pairwise disjoint, the function $P\backslash Z'\to k$
that is constantly $c_{(Pz)K}$ on each $(Pz)K$ gives a well-defined
element of $\CCC^\infty_c(P\backslash Z',k)$, which extends $f$.
\end{proof} % }}}
...
isomorphism $\cInd_P^{Z'} \cong \cInd_P^{Z} \oplus \cInd_P^{Z'\setminus Z}$.
\end{rmk} % }}}
...
\end{lem} % }}}
\begin{proof} % {{{
Denote $\lambda\colon \cInd_P^Z\To \cInd_P^{Z'}$ and $\mu\colon \cInd_P^{Z'}\To
...
gHg^{-1}$ and denote $g_*\colon \D(H)\raiso \D(g(H))$ the ``inflation'' along
...
algebraic groups.}, the
inclusion $Q\hookrightarrow n^{-1}(P)Q$ and $n^{-1}(P)Q\xrightarrow{n\cdot} PnQ$
induce homeomorphisms $n^{-1}(P)\cap Q\backslash Q \raiso n^{-1}(P)\backslash
n^{-1}(P)Q \raiso P\backslash PnQ$. It follows that for any $V \in
\Rep_k(P)$ the natural map
\begin{align*}
...
Fix a field $k$ of characteristic $p>0$. 
Let $\field/\QQ_p$ be a finite extension. 
...
\]
where $\alg{U}_\alpha$ denotes the root $\field$-group associated with
$\alpha\in \Sigma$. We denote $\Weyl$ the finite Weyl group associated with
$(\alg{G},\alg{T})$.
We fix once and for all a set $\NNN \subseteq
\alg{N}_{\alg{G}}(\alg{T})(\field)$ of
representatives of $\Weyl$ and denote $n_w$ the element of $\NNN$ corresponding
to $w\in \Weyl$. We denote $n_\alpha \in \NNN$ the element lifting the simple
...
\]
We denote $\Sigma_M = \Sigma(\alg{M},\alg{T}) \subseteq \Sigma$ the root system
of $\alg{M}$; the positive roots corresponding to $\Delta_M$ are $\Sigma_M^+ =
\Sigma_M\cap \Sigma^+$. We denote $\Weyl_M \subgroup \Weyl$ the finite Weyl
...
\end{lem} % }}}
\begin{proof} % {{{
Note that $\dim_{\field}(\cdot) = [\field:\QQ_p]\cdot \dim(\cdot)$. 
Put $\Sigma_{N} =
...
Example~\ref{ex:explicit-character}. The adjoint action on
$\bigwedge^{[\field:\QQ_p]d_w} \Lie(n_w^{-1}(\ol{\alg{U}})\cap \alg{N})$ yields
a character $\delta'_w\colon n_w^{-1}(M)\cap L\to \QQ_p^\times$. Recall the mod
$p$ cyclotomic character $\varepsilon\colon \QQ_p^\times \to \ZZ_p^\times\to
\FF_p^\times\subseteq k^\times$, where the first map is given by $x\mapsto
xp^{-\val_p(x)}$. The character $\delta_w$ is given as the composite $\delta_w =
...
n_{w*}^{-1}\chi\bigr) \in \D^{\ge 1-j}(L)$. 
Since $\D^{\ge 1-j}(L)$ is closed
under extensions, we deduce $\Phi_{G\setminus
Z'}(\chi) \in \D^{\ge 1-j}(L)$. 
...
\end{align*}
where the last isomorphism follows from the fact that, since $M\cap n_w(N)$ acts
trivially on $\chi$, we have $\L(M\cap n_w(N),\chi)\cong \chi$ viewed as a
character of $M\cap n_w(L)$. This proves~\eqref{eq:PS}.
\end{ex} % }}}
...
% {{{ OBSOLETE ?!
\subsubsection{} % {{{
Observe that restriction of characters induces an inclusion $\X^*(Z) \subseteq
\X^*(T)$ with finite index, which is $\Weyl$-equivariant for the natural actions
of $\Weyl$ on $\X^*(Z)$ and $\X^*(T)$, respectively. (A priori, only
$\alg{N}_{\alg{G}}(\alg{T})(\field)$ acts on $\X^*(Z)$, but the inclusion shows
that the induced action of $Z$ is trivial.) For each $\alpha\in\Sigma$, the
...
w^{-1}(-\Sigma^+)}d_{\beta}\beta \in \X^*(Z)$. The character $\delta_w$ from
Notation~\ref{nota:delta_w} is then given by $\delta_w = \varepsilon_\field\circ
\alpha_w$, where $\varepsilon_{\field} = \varepsilon\circ
\Nm_{\field/\QQ_p}$ (here, $\Nm_{\field/\QQ_p}\colon \field^\times\to
\QQ_p^\times$ is the norm map) and where we view $\alpha_w$ as a character
$Z\to \field^\times$.
...
characters of $Z$.
\item\label{lem:cocycle-d} If $\rho\in\X^*(Z)$, then $\rho$ is a twisting
element.
...
$\alpha\in\Delta_G$, by \ref{lem:cocycle-d}. Hence, $\Weyl$ fixes $\theta-\rho$,
...
and write $\alg{B} = \alg{Z}\ltimes\alg{N}$. Denote $C(Z)$ the center of $Z$.
...
\end{thm} % }}}
\begin{proof} % {{{
The $P\times Q$-invariant open subsets $\bigl(G\setminus \bigcup_{\substack{w
...
Y \too \L\bigl(N,\pInd_P^GV\bigr) &\too Z \xrightarrow{+}\\
...
\L\bigl(M\cap n_\alpha(N),V\bigr) &\too Z \too \pInd_{P\cap L}^L \L(M\cap N,V)
\xrightarrow{+}
\end{align}
...
\[
\RHom_L(Z,W) \too \RHom_G\bigl(\pInd_P^QV, \pInd_Q^GW\bigr) \too \RHom_L(Y,W)
\xrightarrow{+},
\]
...
\H^i\RHom_L(Z,W)\raiso \Ext_G^i\bigl(\pInd_P^GV, \pInd_Q^GW\bigr).
\end{equation}
...
\[
\Ext_G^1\bigl(\pInd_P^GV,\pInd_Q^GW\bigr) \cong \H^1\RHom_L(Z,W) \cong
...
whose constituents occur with multiplicity one. Denote $\JH(M)$ the set of
...
we denote $\alg{P}_I = \alg{M}_I\ltimes \alg{U}_I$ the corresponding standard
...
\Chain(G)$ and any integer $r$, we denote $\sigma^{\ge r}X$ the stupid
