...
Here $u=u(x, t)$ is the velocity vector field, $\rho = \rho(x, t)$ is the scalar temperature or density of fluid, and $\mathbb{W}_t$ is a cylindrical Wiener process valued in some separable Hilbert space and the corresponding stochastic integral is in the Itô sense. The term $(b\cdot\nabla u, b\cdot\nabla\rho)d\mathbb{W}_t$ represents the transport noise and $(\sigma^{(1)}(u,\rho),\sigma^{(2)}(u,\rho))d\mathbb{W}_t$ is the multiplicative noise. The main result of the paper is the global well-posedness of the system \eqref{e.w09241} in $L^p(\Omega;L^p(\mathbb T^3))$ with $p>5$ and small initial data.
...
After applying Leray projection, we can rewrite system \eqref{e.w09241} in the following abstract form:
