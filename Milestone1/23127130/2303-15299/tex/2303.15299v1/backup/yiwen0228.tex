...
\title{\LARGE \bf Cooperative Cruise Control  of Heterogeneous Platoons against Byzantine Attacks: A Twin Layer Approach}
...
\begin{document}
  \maketitle
  \begin{abstract}
    This paper studies the problem of cooperative cruise control of heterogeneous platoons against Byzantine attacks. A vehicle under Byzantine attacks sends different wrong values to all of its neighbors, while it applies some wrong input signals for itself. Thus, it is extremely difficult to be depressed.  Inspired by the concept of Digital Twin, a novel hierarchical protocol equipped with a virtual Twin Layer (TL) is proposed, which decouples the above problems into the defense scheme against Byzantine edge attacks on the TL and the defense scheme against Byzantine node attacks on the Cyber-Physical Layer (CPL). On the TL, we propose a resilient topology reconfiguration strategy via adding a minimum number of key edges to improve the network resilience. It is strictly proven that the topology on the TL satisfies strongly $(2f+1)$-robustness is sufficient to achieve asymptotic performance. On the CPL, decentralized chattering-free controllers are proposed which guarantees the resilient output consensus in the heterogeneous platoons against unbounded Byzantine node attacks. Besides, this controller can achieve uniformly ultimately bounded convergence. The effectiveness and practicality of the theoretical results are verified by a  platoon against Byzantine attacks.
\end{abstract}
\begin{IEEEkeywords}
     Cooperative control, Heterogeneous platoons, Byzantine attacks, Resilient platoon 
...
{
\IEEEPARstart{I}{n} the last decade, there has sprung up a heated research trend the towards Intelligent Highway Vehicle Systems (IHVSs) to accommodate ever-increasing traffic pressures. A fundamental issue in relieving traffic jams, named vehicle platoon technology \cite{wang2019survey,zheng2015stability, xu2019energy,ma2020stability, li2017robustness, vegamoor2019reducing,bian2021behavioral}, is to coordinate a group of connected vehicles in the same lane, while ensuring the safety of each vehicle. More specifically, all vehicles in a  platoon try to maintain a safe distance w.r.t. each other while maintaining an aligned speed, as shown in Fig. \ref{image101}. It has been proved in \cite{wang2019survey,ma2020stability} that this platoon-based cooperative control scheme could possess higher traffic efficiency, employ less energy consumption, and achieve greater security, compared with multiple non-cooperative vehicles. 

A large amount of researches has been implemented to improve the autonomy, efficiency, and security of platoons (see \cite{wang2019survey} and references therein). To name just a few, the authors in \cite{zheng2015stability} solved the problem of internal stability of homogeneous vehicle platoon. A control framework for a homogeneous vehicle platoon, which could simultaneously minimize the energy consumption and ensure the platoon string stability under communication delay, is formulated in \cite{xu2019energy}. Furthermore,  the effect of packet losses on the stability of a platoon is solved via a cooperative adaptive cruise control scheme in \cite{vegamoor2019reducing}. A recent distinguished work \cite{bian2021behavioral} presents a method to dynamically harmonize the behaviors of cyclic vehicular platoons in a closed road network. However, most of the existing work focuses on homogeneous 
platoons \cite{zheng2015stability, xu2019energy,ma2020stability, li2017robustness, vegamoor2019reducing,bian2021behavioral}, which cannot be extended to the cases of heterogeneous platoons.

%For vehicle platoons, stability is the most important, which ensures convergence to the required formation. Another key is string stability, which can ensure that interference will not be amplified when it propagates in the platoons. 

\begin{figure}\label{image1}
  \centering
  \includegraphics[height=5cm, width=7cm]{1.jpg}
  \caption{ A Platoon of vehicles.}\label{image101}
\end{figure}

However, the resilient issue of platoons against malicious attacks has been rarely mentioned in the existing works.} As a representative of modern large-scale complex networked systems, the platoons may break down when a fraction of nodes is compromised via a malicious attacker. In order to improve the security of networked control systems, the resilient consensus has been a vital research topic in control system community in the presence of malicious attacks such as malicious attacks \cite{2020ResilientZ}, Byzantine attacks \cite{1982The}, denial of service attacks (DoS) \cite{2016ResilientY}, false-data injection (FDI) attacks \cite{2018State}, deception attacks \cite{2020Secure}, and replay attacks \cite{2020Event}. The resilient control of MASs aims to develop distributed consensus protocols which provide an admissible system performance in the hostile situation despite the network misbehavior. In the first study of the resilience of consensus under Byzantine attacks in \cite{1982The}, the problem was described abstractly in terms of a group of Byzantine generals with their troops camped in different places around an enemy city. However, some of them
...
\item   
We propose a resilient topology reconfiguration strategy via adding a minimum number of key edges to improve the network resilience of the TL. Then we strictly prove that strongly $(2f+1)$-robustness of the topology
...
\item  Different from the existing work in \cite{2021ResilientY}, we manage to ensure both Byzantine nodes and normal nodes own a decent error bound in the presence of Byzantine attacks. This implies that the network connectivity of the whole platoons is preserved thanks to our hierarchal approach. Thus, the cooperative cruise control of vehicle platoon could be stilled accomplished via our double-layer resilient control scheme, despite Byzantine attacks.
\end{enumerate}

\noindent\textbf{Notations:}
In this paper, { $\sigma_{{\rm min}}(S)$ and $\sigma(S)$ are the minimum singular value and the spectrum of matrix $S$,
respectively.} $|S|$ is the cardinality of a set $S$. $\Vert \cdot \Vert$ is the Euclidean norm of a vector. Define the sets of real numbers, positive real numbers, nonnegative integers, positive integers as $\mathbb{R}$, $\mathbb{R}_{> 0}$, $\mathbb{Z}_{\geq 0}$ and $\mathbb{Z}_{> 0}$. {The in-neighbor set and out-neighbor set denote the index sets denoted by $\mathcal{N}_i^{in}= \{i \in \textbf{I} \ | \ (v_j, v_i) \in \mathcal{E} \}$ and $\mathcal{N}_i^{out}= \{i \in \textbf{I} \ | \ (v_i, v_j) \in \mathcal{E} \}$, respectively.}
...
($v_j$, $v_i$) is $[a_{ij}]$, and $a_{ij} > 0$ when $(v_j, v_i) \in  \mathcal{E} $, otherwise $a_{ij} = 0$. $b_{i0}> 0 $ if there is an edge between the leader and the $i$th vehicle, otherwise $b_{i0} = 0$. Some useful definitions of network robustness are recalled first.
...
\end{equation*}
has the property that all nodes may be reached from the node $k$, then all components of any solution $x(t)$ of \eqref{eq1}  exponentially converge
...
{
Consider a platoon of vehicles with different dynamics. In each platoon,  the vehicles are considered  indexed by $\mathcal{F}=\{1,\dots, N\}$. 
As shown in \cite{ghasemi2013stable} and \cite{hu2020cooperative:}, the  dynamics of each vehicle can be described by the nonlinear input-affine differential
equation
\begin{align}\label{eq2}
\dot{a}_i&=g_i(v_i, a_i)+h_i(v_i)k_i,\\
...
Bring \eqref{eq5} into \eqref{eq2}, the dynamics of each vehicle in the platoon can be obtained as follows
\begin{equation*}
\begin{cases}
\dot{s}_i=v_i,\\
\dot{v}_i=a_i,\\
...
\end{cases}
\end{equation*}
where $s_i \in \mathbb{R}^2, v_i \in \mathbb{R}^2$ and $ a_i \in \mathbb{R}^2$ denote position, velocity and acceleration, respectively
}

Then the dynamics of the vehicles are described as
...
The target $x_0$, indexed by 0, represents the reference signal to be tracked. The dynamic of the target is described as
\begin{equation}\label{eq7}
\begin{cases}
\dot{x}_0(t)=S x_0(t)+ d(t),\\
y_0(t)=R x_0(t),
\end{cases}
\end{equation}
where $x_0(t) \in \mathbb{R}^q$ and $y_0(t) \in \mathbb{R}^p$ are the state and output of the  target, respectively.
...
Under Assumptions $1$-$3$, for each vehicle $i$, there exist some matrices $\Gamma_i$ and $\Pi_i$ that satisfy the following regulator equations:
...
\begin{assumption}\label{assumption_4}
The directed graph $\mathcal{G}$ contains a spanning tree rooted by the target node. 
\end{assumption}

In this paper, only a fraction of vehicles are pinned to the target. 
}
For the sake of illustration, we take the vehicles where the target can be directly observed as the pinned vehicles. The set of these pinned vehicles is described as set $\mathcal{V}_p$. On the other hand, the remaining vehicles are regarded as non-pinned vehicles and  collected in set $\mathcal{V}_{np}$.

Define the following local output consensus error:
\begin{equation}\label{eq9}
e_i(t)=y_i(t)-y_0(t)-h_i.
\end{equation}
where $h_i$ denotes a pre-specified offset w.r.t. the moving target.
...
\begin{figure}
  \centering
  \includegraphics[height=6.5cm, width=7.8cm]{a.PNG}
  \caption{Distributed attack-resilient platoons against Composite Attacks: A two-layer framework.}\label{image2}
\end{figure}
...
{
There exist at most $f$  Byzantine vehicles in the in-neighborhood of each vehicle, that is, $|\mathcal{B}\cap \mathcal{N}^{in} _i|\leq f , \ \forall i \in \mathcal{F}$.}
\end{assumption}
...
The input signals of all Byzantine nodes on the CPL could be falsified via Byzantine node attacks. Under the influence of the Byzantine node attacks, the input signal of each Byzantine agent will be subverted as
 ...
 \end{equation*}
 where $\psi_i(t)$ represents a unknown, time-varying, and potentially unbounded signals satisfying Assumption \ref{assumption_6}.

 \begin{assumption}\label{assumption_6}
   The time derivative of the magnitude of the Byzantine node attacks on each vehicle is bounded by  $\bar{\kappa}>0$, that is, $\sup_{t\in \mathbb{R}_{\geq 0}} \frac{d\|\psi_i(t)\|}{dt}\leq \bar{\kappa}$, $\forall i \in \mathcal{F}$.
 \end{assumption}
...
attacks, this work deals with unbounded node attacks under Assumption \ref{assumption_6}. When the attackers launch injections with unbounded derivative values, it is easy for the platoon to  detect and discard these signals.
$\hfill \square$
\end{remark}

Note that the $i$-index set of Assumption \ref{assumption_5} includes the Byzantine agents, apart from normal ones. This means, not only do we try to ensure that the normal agents are free from the compromised information from Byzantine edge attacks, but also we consider correcting the performance of Byzantine agents. As shown in Figure \ref{image2}, a supervising layer, named TL, provides another control signals to fight against the Byzantine node attacks. With the reference signals of TL, the real input signal of each vehicle consists of two parts:
 \begin{equation*}%\label{ucorrupt}
   \bar{u}_i(t)={u}_i(t)+\psi_i(t),\ \forall i\in  \mathcal{F},
 \end{equation*}
 where $u_i(t)$ is the input {signal (correction signal)} to be designed later, which is concerned about the virtual states on the TL.
...
\noindent \textbf{Problem RC3HPB} (Resilient Cooperative Cruise Control of Heterogeneous Platoons against Byzantine attacks):
Under Assumptions \ref{assumption_1}$\sim$\ref{assumption_5}, consider the platoons in \eqref{eq6} subject to Byzantine attacks defined in Section \ref{2B}, design distributed protocols $u_i$ such that the global error $e_i$ in (\ref{eq9}) is uniformly ultimately bounded (UUB) \cite[Definition 4.6]{khalil2002nonlinear}.
$\hfill \hfill \square $

\section{Main Results}

Motivated by the recent sprung-up digital twin technology, a double-layer resilient control scheme is investigated in this section, as shown in Fig. \ref{image2}.
Owing to the introduction of TL, the resilient control scheme against BAs can be decoupled into the defense against Byzantine edge attacks on the TL and the defense against potentially unbounded Byzantine node attacks on the CPL. More specifically, this hierarchal control scheme solves the \textbf{Problem RC3HPB} by employing a TL with a edge-adding strategy against Byzantine edge attacks and a decentralized adaptive controller on the CPL, featured by a soft-sign function, against potentially unbounded Byzantine node attacks.

\begin{figure}
  \centering
  \includegraphics[height=6.5cm, width=9.8cm]{CT.PNG}
...
As shown in Fig. \ref{image2}, there exists a TL governor to modify the network topology via adding a minimum number of key edges to improve the network resilience of the TL topology.
...
Fig. $\ref{image3}$, whose detailed process can be found in the example 1. Notice that $\mathcal{G}$ and $\mathcal{G}_T$ are not necessarily the same, since the TL has certain programmable flexibility in topology. Notice that the network topology $\mathcal{G}_T$ except the adding edges $(v_4,v_6)$ on the TL is a subset of the unweighed network topology $\bar{\mathcal{G}}=(\mathcal{V}, \mathcal{E})$ on the CPL. Denote the in-neighbor and out-neighbor sets of node $i$ on the TL as $\mathcal{N}_{T,i}^{in}$ and $\mathcal{N}_{T,i}^{out}$, respectively. Now 
...
\begin{algorithmic}[1]
   ...
   \State Add the target into $\mathcal{M}_a$. Update $\mathcal{M}_b=\mathcal{M}_b \backslash \{0\}$.
   \While {$\mathcal{M}_a \neq \varnothing$}
   \State Denote  the vehicles pinned to  $\mathcal{M}_a$ as $\mathcal{F}_u$.
   \While {$\mathcal{F}_u \ne \varnothing$}
   \For {$i \in \mathcal{F}_u$}
   \If {Node $i$ is pinned to the target or at least $(2f+1)$ nodes in $\mathcal{M}_a$}
...
{
\begin{enumerate}
\item Add the target into $\mathcal{M}_a$. Update $\mathcal{M}_a={0}$ and $\mathcal{M}_b=\mathcal{F}$.
\item Collect the vehicles pinned to  $\mathcal{M}_a$ as $\mathcal{F}_u=\{1, 2, 3\}$.
Since the nodes in $\mathcal{F}_u$ is pinned to the target, we update $\mathcal{M}_a=\{0, 1, 2, 3\}$, $\mathcal{F}_u=\{4, 5, 6\}$, $\mathcal{V}_T=\{v_0, v_1, v_2, v_3\}$ and $\mathcal{E}_T=\{(v_0, v_1), (v_0, v_2), (v_0, v_3)\}$.
\item Collect the vehicles pinned to $\mathcal{M}_a$ as $\mathcal{F}_u=\{4, 5, 6\}$.\\ As only node 5 pins to at least $(2f+1)$ nodes in $\mathcal{M}_a$, we update $\mathcal{M}_a=\{0, 1, 2, 3, 5\}$, $\mathcal{F}_u=\{4, 6\}$,\\ $\mathcal{V}_T=  \{v_0, v_1, v_2, v_3, v_5\}$ and $\mathcal{E}_T=\{(v_0, v_1), (v_0, v_2),\\ (v_0, v_3), (v_1, v_5), (v_2, v_5), (v_3, v_5)\}$.

\item Collect the vehicles pinned to  $\mathcal{M}_a$ as $\mathcal{F}_u=\{4, 6\}$.\\ Since only node 4 pins to at least $(2f+1)$ nodes  in  $\mathcal{M}_a$, we update $\mathcal{M}_a=\{0, 1, 2, 3, 4,  5\}$, $\mathcal{F}_u=\{ 6\}$, $\mathcal{V}_T=\{v_0, v_1, v_2, v_3, v_4, v_5\}$ and $\mathcal{E}_T=\{(v_0, v_1), (v_0, v_2), (v_0, v_3), (v_1, v_5), (v_2, v_5), (v_3, v_5),\\ (v_2, v_4), (v_3, v_4), (v_5, v_4)\}$.
\item Collect the vehicles pinned to $\mathcal{M}_a$ as $\mathcal{F}_u=\{6\}$. As none of node  pins to the target or at least $(2f+1)$ nodes in $\mathcal{M}_a$, the edge $(v_4, v_6)$ should be added. Then we can find that node 4 pins to at least $(2f+1)$ nodes in $\mathcal{M}_a$, we update $\mathcal{M}_a = \{0, 1, 2, 3, 4,  5, 6\}$, $\mathcal{F}_u=\{ 6\}$, $\mathcal{V}_T=\{v_0, v_1, v_2, v_3, v_4, v_5, v_6\}$ and $\mathcal{E}_T=\{(v_0, v_1), (v_0, v_2), (v_0, v_3), (v_1, v_5), (v_2, v_5), (v_3, v_5),\\ (v_2, v_4), (v_3, v_4), (v_5, v_4), (v_1, v_6), (v_2, v_6), (v_4, v_6)\}$.
\end{enumerate}
}
...
Then we use the the MSR algorithm \cite{1994Reaching} to process the virtual state $z_i(t)$ on the TL. At any time $t$, each vehicle  always makes updates as below:
\begin{enumerate}
\item Collect the status of all neighbor agents (except the target if $i \in \mathcal{V}_p$) in a list $\Lambda_i(t)$.
...
\end{aligned}
\end{equation}
where $z_i(t)$ is the local state of the virtual layer, $\bar{\alpha}> \alpha$, $m_{ij}$ denotes the number of adjacent vehicles of agent $i$, and $m_{i0}$ denotes the number of adjacent target of agent $i \in \Omega_i(t)$.
\end{enumerate}

Based on the above discussions, an improved MSR algorithm based on the minimum edge number approach is summarized as Algorithm 2.

\begin{algorithm}
\caption{Resilient MSR algorithm on the TL}
\begin{algorithmic}[1]
   \State  Acquire the topology on the TL by using the Algorithm 1. 
...
Consider the platoon satisfying the Assumption \ref{assumption_5}. Suppose each vehicle on the TL makes updates via Algorithm 2. Then, we obtain two statements as follows:
\begin{enumerate}
...
\textbf{Case 1:}  $i \in \mathcal{V}_{np}$. Since the in-neighbors of each agent $i$ are at least $2f+1$ by recalling Algorithm 2, it holds that $\Omega_i(t) \neq \varnothing$ as each agent
...
By repeating the above steps, we can obtain that \eqref{eq11} is hold. Moreover, we can find that
\begin{equation*}
\begin{aligned}
...
\textbf{Case 2:} $i \in \mathcal{V}_p$. This case is similar to Case 1 except that the information from target will be preserved.

The proof is completed.  $\hfill \blacksquare$

\begin{lemma}\label{lemma_5}
Consider the platoon satisfying Assumption \ref{assumption_5}. Suppose $\mathcal{G} =(\mathcal{V}, \mathcal{E})$  is strongly $(2f+1)$-robustness w.r.t. $\mathcal{V}_p$ and all vehicles make updates according to the MSR algorithm. At any time $t$, there always exists a secure and equivalent $\frac{1}{2}\alpha$-directed path from target to any vehicle in $\mathcal{F}$.
\end{lemma}

\textbf{Proof.}
The first statement in Lemma \ref{lemma_4} ensures that there exists a secure spanning tree rooted by the target to all vehicles in $\mathcal{F}$. Moreover, the second statement in Lemma \ref{lemma_4} ensures that the above
...
In order to cope with the multi-dimensional case, we discuss the extension of scalar MSR algorithm to vector spaces. Towards the end, for any vector $z_i(t) \in \mathbb{R}^q$, let us introduce the notation:
...
\end{equation}
where $\Theta_{f}(\{z_j(t)\}_{j \in \mathcal{N}_{T,i}^{in}})$ is defined in \eqref{eq12}. With the same settings as Lemma \ref{lemma_5}, $z_i(t)$ exponentially converges to $z_0(0)$.
\end{lemma}
...
\begin{aligned}
\dot{z}_i(t) =
\begin{bmatrix} \sum\limits_{j \in \Upsilon_i^1(t)} \bar{m}_{i j}^1(t)[z_j^1(t)-z_i^1(t)]\\+\bar{m}_{i0}^1(t)[z_0^1(t)-z_i^1(t)] \\ \vdots \\  \sum\limits_{j \in \Upsilon_i^q(t)} \bar{m}_{i j}^q(t)[z_j^q(t)-z_i^q(t)]\\+\bar{m}_{i0}^q(t)[z_0^q(t)-z_i^q(t)] \end{bmatrix},
\end{aligned}
\end{equation}
...
In (\ref{EQ15}), $\bar{\Phi}^s(t)$ is a Metzler matrix with zero row sums. According to Lemma \ref{lemma_5}, each $\bar{\Phi}^s(t)$ has the property that any vehicle can be reached from the target. Recalling Lemma \ref{lemma_1}, $z_i^s(t)$ exponentially converges to a common values as $t \rightarrow \infty$. The target can not collect states from vehicles, so it holds that $z_0^s(t)=z_0^s(0)$. Thus, $z_i^s(t)$ exponentially converges to $z_0^s(0)$ for any $i$. The proof is completed.    $\hfill \blacksquare$

{ There exists $\bar{R}\in \mathbb{R}^{n \times p}$ such that $R\bar{R}=I_p$}. 
We formulate a distributed TL to achieve the resilient control of platoons:
...
\end{split}
\end{equation}
where the virtual state ${v}_i$ denote the $i$th vehicle's estimation on the target state, $\xi_i$ denotes the update signals of the TL to be designed, and $\varpi_i$ denotes denotes the
feed-forward platoon compensation signal.  

The secure fully-distributed estimation scheme is proposed as
\begin{equation}\label{eq17}
\xi_i(t)=e^{S t} \Theta_{f}(e^{-S t} \{v_j\}_{j \in \mathcal{N}_{T, i}^{in}}),
\end{equation}
where  $z_i(t)=v_i(t)-\bar{R}h_i$. Specially, $z_0=v_0=x_0$ in (\ref{eq16}) and (\ref{eq17}). Let
\begin{equation}\label{eq18}
\delta_i(t)= v_i(t)-\bar{R}h_i-v_0(t),
\end{equation}
denotes the estimation error of the $i$th agent.
...
\begin{myTheo}
Consider a TL in (\ref{eq16}) and (\ref{eq17}) under Assumptions \ref{assumption_1}$\sim$\ref{assumption_5}. The estimation error on
the TL layer under Byzantine  attacks converges into zero asymptotically, that is,
$\lim_{t \to \infty} \delta_i(t) =0, i \in \mathcal{F}$, if $m_{ij}$ is solved via Algorithm 2.
\end{myTheo}

\textbf{Proof.}
Define the auxiliary variable $q_i(t)=e^{-S t}(v_i(t)
-\bar{R}h_i)=e^{-S t}z_i(t)$. Similarly, let $q_0(t)= e^{-S t} v_0(t)=e^{-S t} z_0(t)$. It is obvious that $q_i(0)
=v_i(0), i \in \mathcal{F} \cup \{0\}$. We compute the derivative of $q_i(t)$:
\begin{equation}
\begin{split}
\dot{q}_i(t)
=&-S e^{-S t} z_i(t)
+e^{-S t}(S v_i+\xi_i+\varpi_i)\\
=&-Se^{-S t} z_i(t)+e^{-S t}(S z_i+S \bar{R}h_i\\
&+ e^{S t}\Theta_f( e^{-S t} \{v_j\}_{j \in \mathcal{N}_{T,i}^{in}})-S \bar{R}h_i)\\
=&\Theta_f(\{q_j\}_{j \in \mathcal{N}_{T,i}^{in}})\label{eq19}.
\end{split}
\end{equation}
by recalling the fact that
$S e^{-S t}= e^{-S t} S$.
By employing (\ref{eq19}) and Lemma \ref{lemma_6}, we conclude that
...
Since $q_i(0)=z_i(0)$, one has
\begin{equation*}
\Vert q_i(t)-z_0(0) \Vert \leq e^{-\rho t} \Vert  z_i(0)- z_0(0) \Vert .
\end{equation*}
...
\mathrm{e}^{-\rho t}   \varphi(\epsilon)\mathrm{e}^{\epsilon t} \Vert (z_i(0)-z_0(0)) \Vert \\  \label{eq60}
...
\begin{align}
u_i&=K_ix_i+H_i v_i-\theta_i-\hat{\psi}_i, \label{eq21}\\
\theta_i&=\bar{B}_i \Pi_i S\bar{R}h_i,\\
\hat{\psi}_i&=\frac{B^T_i P_i\varepsilon_i}{\Vert \varepsilon^T_i P_i B_i \Vert+\omega}\hat{\rho}_i,  \label{eq22}\\
\dot{\hat{\rho}}_i&=
\begin{cases}
\Vert \varepsilon^T_i P_i B_i \Vert +2\omega, \quad  \mbox{if} \   \Vert \varepsilon^T_i P_i B_i \Vert \geq \bar{\kappa}\label{eq23}\\
\Vert \varepsilon^T_i P_i B_i \Vert +2\omega(\frac{\Vert \varepsilon^T_i P_i B_i \Vert}{\bar{\kappa}}), \quad \mbox{otherwise,}
\end{cases}
\end{align}
...
Consider the heterogeneous platoons with \eqref{eq6}.
 \textbf{Problem RC3HPB} can be solved  via the secure TL in (\ref{eq16}) and (\ref{eq17}) and the decentralized controller (\ref{eq20})-(\ref{eq23}), if the following two conditions hold simultaneously:
...
\item  $m_{ij}$ in \eqref{eq10} is tackled by using Algorithm 2.
\end{enumerate}
\end{myTheo}
...
To show that $ \mathrm{e}_i(t)$ is UUB, in the following part, we shall prove that $\Vert \varepsilon_i(t) \Vert$ is UUB and $\Vert (z_i(t)-x_0(t)) \Vert  \rightarrow 0$ as $t \rightarrow \infty$. Using $\eqref{eq60}$, we can obtain
\begin{equation*}
\begin{split}
...
Next, we prove that $\Vert \varepsilon(t) \Vert$ is UUB. From \eqref{eq6}, \eqref{eq7}, \eqref{eq8},  \eqref{eq16}, \eqref{eq17} and \eqref{eq25}, we obtain the time derivative of \eqref{eq20} as
\begin{equation*}
\begin{split}
...
Recalling the fact that $\widetilde{B}_i \Pi_i S \bar{R}h_i=0$, we can obtain that 
\begin{equation}\label{eq30}
\widetilde{B}_i \Pi_i S\bar{R}h_i-\widetilde{B}_i B_i \theta_i =0.
\end{equation}

Using $\eqref{eq29}$ and $\eqref{eq30}$, we obtain
\begin{equation}\label{eq31}
B_i \theta_i- \Pi_i S \bar{R}h_i=0.
\end{equation}

It follows from $\eqref{eq16}$ and $\eqref{eq31}$, we obtain that
...
Noting that $[({\rm d}\Vert \psi_i \Vert )/ {\rm d}t]$ is bounded by $\bar{\kappa}$.  Choose $\dot{\hat{\rho}}_i \geq  \bar{\kappa} \geq [({\rm d}\Vert \psi_i \Vert )/ {\rm d}t]$,that is, $\frac{\bar{\kappa}+\omega}{\bar{\kappa}} \frac{d \Vert \psi_i \Vert}{dt}-\dot{\hat{\rho}}_i \leq \bar{\kappa}+\omega-\dot{\hat{\rho}}_i \leq -\omega < 0$. When $t \geq t_1$, we have
...
In light of \eqref{eq60}, $\Theta_{f}(\cdot)$  exponentially tends to 0. According to Lemma 2,  it follows that $\xi_i$ also exponentially tends to 0. We conclude that there exist $\epsilon_1$
...
Hence, the cooperative control problem for platoons \eqref{eq6} has been solved, which completes the proof.  $\hfill \blacksquare$
...
\begin{enumerate}
  ...
  \item Different from the existing work \cite{2021ResilientY} only ensuring the performance of normal agents, we manage to ensure both Byzantine vehicles and normal vehicles own a decent error bound in the presence of Byzantine attacks via our twin layer approach. Thus, we first achieve resilient control \textbf{against} Byzantine attacks, rather than \textbf{under} Byzantine attacks.

  \item Compared with the recent resilient networked control schemes against unbounded attacks \cite{2020ResilientZ,2020Resilient}, this work introduces a soft-sign function, which is continuous and free from chattering phenomena. Also, an explicit bound of the UUB boundary, that is, the spherical surface $\varepsilon_i^{\mathrm{T}}(P_iB_i B_i^{\mathrm{T}}P_i)\varepsilon_i = \bar{\kappa}^2$, is given, which facilitates the pre-estimation of the controller designers. Notice that this bound is not related to the selection of $\omega$ in (\ref{eq21}). However, a larger $\omega$ could speed up convergence w.r.t. the above UUB bound. Moreover, this UUB bound can be adjusted by selecting an appropriate $P_i$.$\hfill \square$
\end{enumerate}
}
\end{remark}
...
Our goal is to achieve resilient cooperative cruise control of heterogeneous platoons. Consider a heterogeneous platoons of six vehicles. Their communication
topology is shown in Figure. \ref{image2}. The  heterogeneous dynamics of vehicles indexed by $1\sim 6$ can be described by
\begin{align*}
&A_{i}=I_2 \otimes  \left[  \begin{array}{ccc}
0 &  1 &0  \\
0 &  0  &1\\
0 & 0 & \frac{-1}{\tau_i}\\
  \end{array}\right], \\
  & B_{i}=I_2 \otimes  \left[  \begin{array}{ccc}
0   \\
0  \\
\frac{1}{\tau_i}\\
  \end{array}\right], ~C_{i}=I_2 \otimes  \left[  \begin{array}{ccc}
1 &  0 & 0  \\
  \end{array}\right],
\end{align*}
where $\tau_i$, $i\in\textbf{I}[1,6]$, are given in Table I.
\begin{table}
  \centering
 \caption{Vehicle Parameters Used in the Simulation}
 {\begin{tabular}[l]{@{}lcccccc}
 \toprule
 Index & $1\sim 3$ & $4\sim 6$  \\
 \hline
   %$m_i$ (kg) &  1752 & 1836 \\
   $\tau_i$ (s) &  0.51 & 0.55  \\
 \toprule
...
%   \end{array}\right].
% \end{align*}
Moreover, we consider a autonomous target, indexed by 0, with the following dynamics:
    \begin{align*}
      &S=I_2 \otimes  \left[  \begin{array}{ccc}
1 &  -3   \\
1 &  -1 \\
  \end{array}\right],
R=I_2 \otimes  \left[  \begin{array}{ccc}
1 & 0  \\
  \end{array}\right].
\end{align*}
The pre-specified offset of all vehicles w.r.t. the target is defined as
\begin{equation}\label{leaderformpattern}
  \begin{cases}
    h_{ix}(t)=6\cos((i-1)\frac{\pi}{3}),\\
    h_{iy}(t)=6\cos((i-1)\frac{\pi}{3}),
  \end{cases}
% h_{ix}(t)=
% \begin{bmatrix}
%   6\cos((i-1)\frac{\pi}{3})\\
...
\ i\in \textbf{I}[1,6].
\end{equation}
If the desired offset in (\ref{leaderformpattern}) can be achieved, the trajectories of six targets will form a 2D hexagon inscribed by the oval in (\ref{leaderformpattern}) round the target. Moreover, we denote the two dimensions of the above 2D trajectories as the X-direction and Y-direction, respectively.
...
We consider the following kind of Byzantine attacks, that is, vehicles 3 and 4 are Byzantine vehicles which satisfy $f$-local assumption, that is, Assumption \ref{assumption_4}. The Byzantine edge attacks on the TL  are to replace the information among agents as:
\begin{equation*}
e_{3,4}= \left[  \begin{array}{ccc}
t  \\
t  \\
2t   \\
2t   \\
  \end{array}\right], e_{3,5}=  \left[  \begin{array}{ccc}
1.5t   \\
-2.5t  \\
1.5t   \\
-2.5t  \\
  \end{array}\right],
  e_{5,6}=  \left[  \begin{array}{ccc}
-1.5t   \\
2.5t   \\
-1.5t   \\
2.5t   \\
  \end{array}\right].
\end{equation*}
where $e_{i,j}$ denote the information flow from vehicle $i$ to vehicle $j$ falsified by the Byzantine edge attacks.
...
\begin{equation*}
\psi_3=  \left[  \begin{array}{ccc}
0.5t  \\
t   \\
  \end{array}\right], \psi_5=  \left[  \begin{array}{ccc}
-t   \\
-0.5t  \\
  \end{array}\right].
\end{equation*}
...
against Byzantine edge attack is shown in Fig. \ref{image4}. It is shown that the asymptotic performance of distributed estimation
error on the TL can be achieved despite the Byzantine edge attacks.

\begin{figure}
  \centering
  \includegraphics[height=6cm, width=8cm]{a1.eps}
  \caption{Distributed resilient estimation performance on the TL}\label{image4}
\end{figure}
Then, we focus on the performance of CPL with the consideration of the potentially unbounded Byzantine node attacks.
In light of \eqref{eq47}, the Byzantine node attack signals satisfy Assumption 6. It could be computed from \eqref{eq8} that
\begin{align*}
&\Pi_{1,2,3}=I_2 \otimes  \left[  \begin{array}{ccc}
1 &  0  \\
1 &  -3   \\
-2 &  0  \\
  \end{array}\right], \\
 & \Gamma_{1,2,3}=I_2 \otimes  \left[  \begin{array}{ccc}
-3.02 & 3.06   \\
  \end{array}\right],\\
&\Pi_{4,5,6}=I_2 \otimes  \left[  \begin{array}{ccc}
1 &  0  \\
1 &  -3   \\
-2 &  0   \\
  \end{array}\right], \\
 & \Gamma_{4,5,6}=I_2 \otimes  \left[  \begin{array}{ccc}
-3.1 & 3.3   \\
  \end{array}\right].
\end{align*}
...
\begin{align*}
    &P_{1,2,3}=I_2 \otimes  \left[  \begin{array}{ccc}
8.499 &  10.539& 3.396 \\
10.539 &  26.462& 9.622   \\
3.396&  9.622 & 4.392   \\
  \end{array}\right], \\
 &P_{4,5,6}=I_2 \otimes  \left[  \begin{array}{ccc}
6.186 & 4.877 & 0.953   \\
4.877 & 9.103 & 1.964   \\
0.953 & 1.964 & 0.999 \\
  \end{array}\right],\\
&K_{1,2,3}=I_2 \otimes  \left[  \begin{array}{ccc}
-1.732 &  -4.907 & -2.240 \\
  \end{array}\right], \\
 &H_{1,2,3}=I_2 \otimes  \left[  \begin{array}{ccc}
-0.861 & -11.661  \\
  \end{array}\right],\\
&K_{4,5,6}=I_2 \otimes  \left[  \begin{array}{ccc}
-1.732 &  -3.571 & -1.816  \\
  \end{array}\right], \\
 &H_{4,5,6}=I_2 \otimes  \left[  \begin{array}{ccc}
-1.428 & -7.414  \\
  \end{array}\right].
\end{align*}
...
\begin{figure}
  ...
  \caption{Resilient trajectories of all vehicles in $0\sim 10$ s : The hollow and solid blocks denote coordinates of vehicles at $t=0$ s and $t = 10$ s, respectively.}\label{image5}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[height=6cm, width=8cm]{a3.eps}
  \caption{Resilient cooperative cruise control error versus time}\label{image6}
\end{figure}
...
{
By employing the above parameters, the trajectories of all vehicles are depicted in Fig. \ref{image5}, which will  form a  hexagon with little tolerance.}
More specifically, the cooperative control error is recorded in Fig. \ref{image6}. The exact platoon error $\varepsilon_i$ between the CPL and the TL  is depicted in Fig. \ref{image7}, which is proved to be UUB with the prescribed error bound.



\section{Conclusion}
In this paper, we investigate the problem of cooperative control of heterogeneous vehicle platoons in the presence of Byzantine attacks. Apart from CPL, a virtual TL is deployed on the cloud. The double-layer  scheme also decouples the defense strategy against Byzantine  attacks into the defense against Byzantine edge attacks on the TL and the defense against Byzantine node attacks on the CPL. On the TL, we propose a resilient topology reconfiguration strategy via adding a minimum number of key edges to ensure the $(2f+1)$-robustness of network topology on the TL. The effectiveness of the TL has been verified via strictly mathematical derivation. On the CPL, a decentralized controller is designed against unbounded Byzantine node attacks, which ensures UUB convergence. Based on the theoretical result,  a  platoon is achieved against Byzantine attacks.
