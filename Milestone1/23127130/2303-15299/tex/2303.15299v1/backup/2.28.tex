...
\title{\LARGE \bf Cooperative Control of Heterogeneous Platoons against Byzantine Attacks}
...
can achieve uniformly ultimately bounded convergence. The effectiveness and practicality  of the theoretical results are verified by a heterogeneous platoon  against Byzantine attacks.
\end{abstract}
\begin{IEEEkeywords}
...
{\color{blue}
\IEEEPARstart{I}{n} recent years, the number of  vehicles has continued to grow, and the increase rate of the number of vehicles is higher than the expansion rate of road space. The resulting traffic jam problem is becoming more and more serious. A way is to coordinate and control connected vehicles in the same lane, which is the so-called vehicle platoon technology \cite{shladover1991automated}. It has been proved that compared with driving alone, this platoon based cooperative control model can significantly improve road capacity and energy efficiency \cite{tsugawa2011automated}. 

In a platoon, all vehicles try to maintain a safe distance between vehicles while maintaining an expected speed. People have conducted extensive research on platoon control to improve its practicability and performance. Recent progress in vehicle emission control can be found in \cite{wang2019survey}. For vehicle platoons, stability is the most important, which ensures convergence to the required formation. Another key is string stability, which can ensure that interference will not be amplified when it propagates in the platoons. The article \cite{zheng2015stability} solves the problem of internal stability of homogeneous vehicle platoon. At present, most of the work is mainly focused on homogeneous 
platoons \cite{ma2020stability}-\cite{li2017robustness}, and heterogeneous platoons has not been paid attention to.
...
\item   
{\color{blue}
We propose a strategy of adding edges to strengthen the security of weak nodes on the TL.} Then we strictly prove that strongly $(2f+1)$-robustness of the topology
...
\begin{figure}\label{image2}
  \centering
  \includegraphics[height=7.3cm, width=8cm]{a.PNG}
...
{\color{blue}
As shown in Fig. $\ref{image3}$, the network topology is modified on the TL. Denote the in-neighbor and out-neighbor sets of node $i$ on the TL as $\mathcal{N}_{T,i}^{in}$ and $\mathcal{N}_{T,i}^{out}$, respectively. 
}
...
\begin{figure}
  \centering
  \includegraphics[height=6cm, width=8.8cm]{CT.PNG}
...
\begin{algorithm}
\caption{Resilient Cooperative Control with $f$}
\begin{algorithmic}[1]
...
\end{equation}
where  $z_i(t)=v_i(t)-\bar{R}h_i$. Specially, $z_0=v_0=x_0$ in (24). Let
\begin{equation}
...
\begin{myTheo}
Suppose that Assumptions \ref{assumption_1}-\ref{assumption_6} hold.
Consider the heterogeneous platoons with \eqref{eq4} and \eqref{eq5}.
...
Hence, the cooperative control problem for platoons \eqref{eq4} and \eqref{eq5}  has been solved, which completes the proof.  $\hfill \blacksquare$

\begin{remark}
{\color{blue} 
Compared with the recent work \cite{gusrialdi2018competitive}, this paper studies the more complex heterogeneous dynamics. We get a new TL topology by using algorithm 1 and then perform topology switching according to algorithm 2. We strictly proved that the topology is strongly (2$f$ + 1)-robustnes, rather than
(3$f$ + 1)-robustness in \cite{2019Byzantine}.
}
$\hfill \square$
\end{remark}
...
and one leader. Our goal is to achieve platoon against Byzantine attacks on the horizontal plane. In the simulation study, the desired spacing are given as 
\begin{equation}
    h_r=6 ,
\end{equation}
...
0 &  0  &1\\
0 & 0 & -\frac{1}{0.55}\\
  \end{array}\right], ~B_{4,5,6}=I_2 \otimes  \left[
  \begin{array}{ccc}
0   \\
0  \\
\frac{1}{0.55}\\
...
We consider the following kind of Byzantine attacks, that is, vehicle 3 and 4 are Byzantine vehicles which satisfy $f$-local assumption, that is, Assumption \ref{assumption_4}. The Byzantine edge attacks on the TL  are to replace the information among agents as:
\begin{equation}
e_{3,4}= \left[
  \begin{array}{ccc}
t  \\
...
We conduct the resilient design of the TL. The  construction process of topology on the TL is shown in the following example. {\color{blue}
...
against Byzantine edge attack is shown in Fig. \ref{image2}. It is shown that the asymptotic performance of distributed estimation
...
In light of (60), the Byzantine node attack signals satisfy Assumption 6. It could be computed from \eqref{eq6} that
\begin{align*}
...
\begin{figure}
  ...
  \caption{Resilient platoon performance}\label{image5}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[height=6cm, width=8cm]{a3.eps}
  \caption{Cooperative control error w.r.t. time}\label{image6}
\end{figure}
...
{\color{blue}
By employing the above parameters, the trajectories of all vehicles are depicted in Fig. \ref{image5}, which will  form a  hexagon with little tolerance. Resilient platoon performance: The hollow and solid blocks denote coordinates of vehicles at $t=0$ s and $t = 10$ s, respectively.}
More specifically, the cooperative control error is recorded in Fig. \ref{image6}. The exact platoon error between the CPL and the TL $\varepsilon_i$ is depicted in Fig. \ref{image7}, which is proved to be UUB with the prescribed error bound.


\section{Conclusion}
In this paper, we investigate the problem of cooperative control of heterogeneous vehicle platoons in the presence of Byzantine attacks. Apart from CPL, a virtual TL is deployed on the cloud. The double-layer  scheme also decouples the defense strategy against Byzantine  attacks into the defense against Byzantine edge attacks on the TL and the defense against Byzantine node attacks on the CPL. The effectiveness of the TL has been verified, under the condition that the network topology on the TL is strongly $(2f+1)$-robust. A decentralized controller is designed against unbounded Byzantine node attacks, which ensures UUB convergence. Based on the theoretical result,  a  platoon is achieved against Byzantine attacks.
