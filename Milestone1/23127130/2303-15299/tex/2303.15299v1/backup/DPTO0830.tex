...
\documentclass[letterpaper, 12pt, journal,twoside, twocolumn]{support/IEEEtran}
...
\title{\LARGE \bf Output Leader-following Consensus of Heterogeneous Multi-agent system against Byzantine Attacks  Theory and Applications in Resilient UAV-UGV Formation}
...
\begin{document}
  ...
  \begin{abstract}
    This paper studies the problem
     of output consensus in  heterogeneous multi-agent systems (MASs) against Byzantine attacks. The Byzantine agents can completely understand the network topology and  send arbitrary and different false data to different neighbors. Inspired by the concept of Digital Twin, a novel hierarchical protocol equipped with a virtual
Twin Layer (TL) is proposed, which decouples the above problems into the defense scheme against
Byzantine edge attacks on the TL and the defense scheme against Byzantine node attacks  on the Cyber-Physical Layer (CPL). It is strictly proven that the topology on the TL satisfies strongly $(2f+1)$-robustness is sufficient to achieve asymptotical  performance. On the CPL, decentralized
controllers are proposed which guarantees the  resilient output consensus in the heterogeneous MASs against  unbounded Byzantine node attacks on the CPL. The controller
...
The leader-following consensus problem of multi-agent systems has attracted considerable attention in the communities of robotics and control systems due to its wide practical applications in formation of unmanned aerial vehicles \cite{2017Time}, electrical power grids \cite{2017Distributed}, attitude alignment of satellites \cite{2020Robust}, aggregation behavior analysis of animals \cite{2004Consensus}, etc. It is well known that leader-following consensus can be reached via sufficient local neighboring relative interactions. As the state variables of each agent cannot always be available in
practice, various output feedback-based control protocols have been developed for second-order multi-agent systems based on event-triggering sampling \cite{2015Event},
linear time-delay multi-agent systems \cite{2016A}, high-order nonlinear multi-agent systems \cite{2019Leader}, etc. It should be pointed out that these results have an assumption commonly that the network is homogeneous with all dynamics of coupled agents being identical. However, in real engineering application, the network is usually heterogeneous. With the development of this issue, leader-following consensus of heterogeneous multi-agent systems has been studied in \cite{2018Consensus,2019LeaderS,2020A,luo2021leader}. In \cite{2018Consensus}, distributed consensus controllers were designed to guarantee the leader-following consensus with heterogeneous input saturation. The authors in \cite{2019LeaderS} investigated leader-following consensus of coupled heterogeneous harmonic oscillators that are one kind of second-order multi-agent systems based on relative position measurements. The leader-following consensus problem of heterogeneous multi-agent systems with nonlinear units and communication time-delay was studied in \cite{2020A} and \cite{luo2021leader}, respectively.

In practical applications, modern large-scale complex networked systems are vulnerable to break down when one or more nodes are compromised and become non-cooperative, which may be caused by malicious attacks to the communication network. In order to improve the safety of networks, resilience of consensus has been a vital research topic in control system community in the presence of malicious attacks such as Byzantine attacks \cite{1982The}, denial of service attacks (DoS) \cite{2016ResilientY}, false-data injection (FDI) attacks \cite{2018State}, deception attacks \cite{2020Secure}, and replay attacks \cite{2020Event}. The resilient control of multi-agent systems aims to develop distributed consensus protocols which provide the admissible system performance in hostile situation despite the network misbehaviors. In the first study of the resilience of consensus under malicious attacks in \cite{1982The}, the problem was described abstractly in terms of a group Byzantine generals with their troops camped in different places around an enemy city. However, some of them
may be traitors who will confuse the others to reach agreement, which are referred to as Byzantine agents and Byzantine attracts. Since this pioneering work of the analysis of Byzantine attracts, several important advances towards the resilient consensus have been made in the last decades. In \cite{1994Reaching}, a series of algorithms, called the Mean-Subsequence-Reduced (MSR) algorithms were developed to handle the approximate Byzantine consensus problem, in which the loyal nodes are able to achieve approximate agreement in the presence of $F$-total Byzantine faults in finite time. Then, authors in \cite{2012Low} put forward a continuous-time variation of the MSR algorithms, referred to as the Adversarial Robust Consensus Protocol (ARC-P), to achieve asymptotic consensus for Byzantine threat networks with constrained topology. In \cite{2013Resilient}, a new topological property, named network robustness, was first introduced to measure the ability of the developed local algorithms W-MSR to succeed. These concepts have been later extended and employed to analyze second-order multi-agent systems in \cite{2015Consensus}. Furthermore, \cite{2017Secure} investigated the consensus problem for nonlinear multi-agent systems suffering from attacks and communication delays. Besides the above works, researchers also investigate some other consensus based settings like synchronization of linear time-invariant systems\cite{2017Resilient}, distributed observers \cite{2019Byzantine}, distributed optimization\cite{2017DistributedS} and so on.

On one hand, besides the aforementioned works, there are few efforts available for heterogeneous networks, which complicates the design as the agents with different dynamics are coupled through information exchange. In this work, we investigate the output leader-following consensus of heterogeneous multi-agent system against Byzantine attacks. On the other hand, as most of the previous works \cite{2020ByzantineS,2021ByzantineL,2021ResilientY} dealing with Byzantine attacks could only consider attacks with bounded magnitude, which are born to be incapable of unbounded node attacks aiming at maximizing its destructive power. Recently, some attempts have been made to deal with the resilient formation control against unbounded attacks in \cite{2020ResilientZ,2020Resilient}. Inspired by the above techniques, we propose a hierarchical resilient framework to achieve the consensus of heterogeneous multi-agent system against unbounded Byzantine attacks. For addressing this issue, the main contributions can be summarized as follows:
\begin{enumerate}
\item Inspired by digital twin technology, we design a double-layer resilient control architecture, including TL and CPL. The TL can be  deployed on the  virtual space. Thus, there exists no Byzantine node attacks on the TL. Because of this feature of the TL, the resilient control strategy can be decomposed into  the defense against  Byzantine edge attacks on TL and  the defense against Byzantine node attacks on CPL.
...
\item A decentralized controller is designed on the CPL. Different from the existing work in \cite{2021ResilientY}, we manage to ensure that Byzantine agents and normal agents operate on an acceptable level, whose error is uniformly
ultimately bounded (UUB) even aganist unbounded Byzantine attacks.
\end{enumerate}
...
respectively. $|S|$ is the cardinality of a set S. $\Vert \cdot \Vert$ is the Euclidean norm of a vector. Define the sets of positive real numbers, positive integers as $\mathbb{R}_{> 0}$ and $\mathbb{Z}_{> 0}$. The Kronecker product is denoted by $\otimes$. $I_N \in \mathbb{R}^{N \times N}$ is the identity matrix.
\section{Preliminaries}
\subsection{Robust Graph Theory}
For a group of agents, define a digraph $\mathcal{G } =(\mathcal{V}, \mathcal{E}, \mathcal{A} )$ with $\mathcal{V}=\{ 1, 2, \ldots~ , N \}$, which indicates the edge set the vertex set, $\mathcal{E} \subset \mathcal{V} \times \mathcal{V}$ indicates the edge set. The associated adjacency matrix is represented as $\mathcal{A} = [a_{ij}]$. An edge rooting from node $j$ and ending at node $i$ is represented by ($v_j$, $v_i$), meaning the information flow from node
...
For a multi-agent system consisting of $N + 1$ agents, the agent assigned as leader is indexed by $0$ and the agents assigned as follows are indexed by $1,\ldots, N$. The dynamics of the followers are described as
...
With Assumptions $1$-$3$, for each follower $i$, there exist some matrices $\Gamma_i$ and $\Pi_i$ that satisfy the following regulator equations:
...
There exist at most $f $  Byzantine agents in the neighborhood of each agent. Namely, $|\mathcal{B}\cap \mathcal{N}^{in} _i|\leq f , \ \forall i \in (\mathcal{B} \cup \mathcal{K})$.
\end{assumption}
...
dynamics. Different from general malicious nodes, they can send arbitrary and different false data to different in-neighbors and  cooperate with other Byzantine nodes at any time.
\end{remark}

\subsubsection{Node Attack}
For Byzantine agents, attacker permeates the actuator to change the input signal of the system by adding the unbounded signal.  Hence, instead of the control input $u_i$ in (2), we may only obtain the following corrupted input signal:
\begin{equation}
\hat{u}_i=u_i+\psi_i
\end{equation}
...
\begin{remark}
In contrast to many existing works\cite{gusrialdi2018competitive}-\cite{de2014resilient}, which could only tackle with bounded node
attacks, this work deals with unbounded node attacks under \ref{assumption_5}. When the attackers launch injections with unbounded derivative values, it is easy for the MAS to  detect and discard these signals.
\end{remark}


\textbf{Problem}(Attack-resilient output consensus control of MASs against Byzantine Attacks):
Consider the MASs in \eqref{eq4} and \eqref{eq5} on graphs suffered the compound attack, design  distributed protocols $u_i$ such that the global error $e_i$ is UUB.
...
In the paper, we use the virtual state $z_i(t)$ to introduce MSR algorithm \cite{1994Reaching}. At any time $t$, each follower  always makes updates as below:
\begin{enumerate}
...
\end{aligned}
\end{equation}
where $z_i(t)$ is the local state of the virtual layer. $\bar{\alpha}> \alpha$ and $m_{ij}$ denotes the number of adjacent followers of agent i and $m_{i0}$ denotes the number of adjacent leader of agent $i \in \Omega_i(t)$.
\end{enumerate}
...
\textbf{Case 2:} $i \in \mathcal{V}_p$. This case is similar to the Case 1 except that the information from leader will be preserved. The proof is completed.  $\hfill \blacksquare$

\begin{lemma}\label{lemma_5}
Consider the MASs satisfying Assumption \ref{assumption_5}. Suppose $\mathcal{G} =(\mathcal{V}, \mathcal{E})$  is strongly $(2f+1)$-robustness w.r.t. $\mathcal{V}_p$ and all followers make updates according to MSR algorithm. At any time $t$, there always exists a secure and equivalent $\frac{1}{2}\alpha$-directed path from leader to any follower in $\mathcal{G}$.
\end{lemma}

\textbf{Proof.}
The first statement in Lemma \ref{lemma_4} ensures that there exists a secure spanning tree rooted by the leader to all followers in $\mathcal{G}$. Moreover, the second statement in Lemma \ref{lemma_4} ensures that the above
...
\end{equation}
where $\Theta_{f}(\{z_j(t)\}_{j \in \mathcal{N}_i^{in}})$ is defined in \eqref{eq13}. Since Lemma \ref{lemma_5} hold, $z_i(t)$ exponentially converges to $z_0(0)$.
\end{lemma}

\textbf{Proof.}
Since the Lemma \ref{lemma_5} hold. Hence, the conditions of Lemma \ref{lemma_4} are satisfied. By applying Lemma \ref{lemma_4} on \eqref{eq13}, \eqref{eq13} is equivalent to
\begin{equation}
\begin{aligned}
\dot{z}_i(t) =
...
$\bar{\Phi}^s(t)$ is a Metzler matrix with zero row sums. According to Lemma \ref{lemma_5}, each $\bar{\Phi}^s(t)$ has the property that any follower can be reached from leader. Recalling Lemma \ref{lemma_1}, $z_i^s(t)$ exponentially converges to a common values as $t \rightarrow \infty$. Leader can not collect states from followers, so it holds that $z_0^s(t)=z_0^s(0)$. Thus, $z_i^s(t)$ exponentially converges to $z_0^s(0)$ for any $i$. The proof is completed.    $\hfill \blacksquare$

We formulate a distributed TL to achieve the resilient control of MASs:
\begin{equation}
\begin{split}
\begin{cases}
\dot{g}_i=Sg_i+\xi_i+\varpi_i\\
\varpi_i=-S\bar{R}h_i+\bar{R}\dot{h}_i, & i \in \mathcal{G},
\end{cases}
\end{split}
\end{equation}
where $\xi_i$ denotes the update signals of the TL to be designed, $\bar{R} \in \mathbb{R}_{n \times p}$ such that $\bar{R}R=I_n$, and $\varpi_i$ denotes the feed-forward formation compensation signals. $h_i$ denotes the time-varying
formation pattern.

The secure distributed estimation scheme is proposed as
\begin{equation}
\xi_i(t)=Q e^{\Lambda t}\Theta_{f}(e^{\Lambda t} Q^{-1}\{z_j\}_{j \in \mathcal{N}_i^{in}}),
\end{equation}
where $\Lambda=Q^{-1}SQ$ denotes the Jordan triangular matrix and $z_j(t) = g_j(t)-\bar{R}h_j(t)$. Especially, $z_0=v_0$ in (19). Let
\begin{equation}
\delta_i(t)= g_i(t)-\bar{R}h_i(t)-g_0(t),
\end{equation}
denote as the estimation error of the ith agent.
...
$\lim_{t \to \infty} \delta_i(t) =0, i \in \mathcal{G}$, if $m_{ij}$ is solved via Algorithm 1.
\end{myTheo}

\textbf{Proof.}
Define the auxiliary variable $q_i=e^{-\Lambda t}Q_{-1}(g_i(t)-\bar{R}h_i(t))=e^{-\Lambda t}Q_{-1} z_i(t)$. Similarly, let $q_0(t)=e^{-\Lambda t}Q^{-1} z_0(t)$. It is obvious that $q_i(0) = Q^{-1}(v_i(0)-\bar{R}h_i(0))=Q^{-1}z_i(0), i \in \mathcal{G} \cup {0}$. We compute the derivative of $q_i(t)$:

\begin{equation}
\begin{split}
&\dot{q}_i(t)\\
=&-\Lambda e^{-\Lambda t}Q^{-1} z_i(t)\\
&+e^{-\Lambda t}Q^{-1}(S g_i+\xi_i+\varpi_i-\bar{R}\dot{h}_i(t))\\
=&-\Lambda e^{-\Lambda t}Q^{-1} z_i(t)+e^{-\Lambda t}Q_{-1}(Q \Lambda Q^{-1}g_i\\
&+Q e^{\Lambda t}\Theta_f(e^{-\Lambda t} Q^{-1}\{z_j\}_{j \in \mathbb{N}_i^{in}})-Q\Lambda Q^{-1}\bar{R}h_i)
\\
...
$z_i(t)$  converges to $z_0(0)$ exponentially. In other words, there exists a $\rho > 0$ such that
\begin{equation}
...
\leq& \Vert Q e^{\Lambda t}\Vert \Vert Q^{-1}z_i(t)-Q^{-1}z_0(0) \Vert\\
...
Therefore, we have $\lim_{t \to \infty} z_i(t)=z_0(t)$, and thus the estimation error $\delta_i(t), i \in \mathcal{G}$, converges to 0 exponentially.
...
\Vert \varepsilon^T_i P_i B_i \Vert +2\omega(\frac{\Vert \varepsilon^T_i P_i B_i \Vert}{\bar{\kappa}}), \quad {\rm otherwise}\label{eq21}
\end{cases}
\end{align}
...
Consider the heterogeneous MASs with \eqref{eq4} and \eqref{eq5}, and the communication topology satisfying Lemma \ref{lemma_5}.
The  leader-following consensus problem can be solved  by using the controller \eqref{eq19}, if the following local conditions hold for each  follower.
\begin{enumerate}
...
\begin{equation}
\Vert z_i(t)-z_0(0) \Vert \leq \mathrm{e}^{-\varrho t} \Vert z_i(0)-x_0(0) \Vert
\end{equation}

Since $z_0(0)=x_0(0)$ and $z_i(0)= v_i(0)$, one has
\begin{equation}\label{eq28}
\Vert z_i(t)-x_0(0) \Vert \leq \mathrm{e}^{-\varrho t} \Vert z_i(0)-x_0(0) \Vert.
\end{equation}

Using \eqref{eq5} and lemma \ref{lemma_6}, we can obtain
\begin{equation}\label{eq29}
\begin{split}
\Vert v_i(t)-x_0(t) \Vert &= \Vert \mathrm{e}^{St}(z_i(t)-x_0(0)) \Vert \\
&\leq \Vert \mathrm{e}^{St} \Vert  \Vert (z_i(t)-x_0(0)) \Vert \\
...
Next, we prove that $\Vert \varepsilon(t) \Vert$ is UUB. From \eqref{eq4}, \eqref{eq6}, \eqref{eq14}, \eqref{eq23}, we obtain the time derivative of \eqref{eq11} as
\begin{equation}
\begin{split}
...
ground vehicles (UGVs) and unmanned aerial vehicles (UAVs). Their communication
topology is shown in Figure 2.  There are six followers
and one leader. The six followers are set as three UGVs indexed by 1, 2, 3  and three UAVs indexed by 4, 5, 6 .  The UGV can receive the desired signal and share it with the UAV according to the topology on the TL. Our goal is to achieve formation tracking of UAVs and UGVs against Byzantine attacks.

\begin{figure}
  \centering
  \includegraphics[height=6cm, width=8cm]{b.PNG}
  \caption{UAV-UGV coordination}\label{image2}
\end{figure}
...
We consider the following kind of Byzantine attacks, that is, followers 3 and 4 are Byzantine agents which satisfy $f-local$ Byzantine edge attacks. The Byzantine edge attacks on the TL  are to replace the information among agents as:
\begin{equation}
e_{3,4}= \left[
  \begin{array}{ccc}
t  \\
...
\begin{figure}
  ...
  \caption{Estimation performance}\label{image3}
\end{figure}
...
\begin{figure}
  ...
  \caption{Formation error}\label{image5}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[height=6cm, width=8cm]{a4.eps}
  \caption{Formation-tracking error: The light red zone denotes the UUB bound $\Vert \varepsilon_i^T P_i B_i\Vert \leq \bar{d}$}.\label{image6}
\end{figure}


By employing the above parameters, the trajectories of all agents are depicted in Fig. \ref{image4}, which shows
that the whole MAS can achieve the formation tracking with little tolerance. Also, the exact tracking error between the CPL and the TL is shown in Fig. \ref{image5}. $\Vert \varepsilon_i^T P_i B_i\Vert$ is proved to be UUB in Fig. \ref{image6}.


\section{Conclusion}
In this paper, we investigate the problem of output consensus of heterogeneous MASs in the presence of TL against Byzantine attacks. Apart from CPL, a virtual TL is deployed on the cloud. The double-layer  scheme also decouples the defense strategy against Byzantine  attacks into the defense against Byzantine edge attacks on the TL and the defense against Byzantine node attacks on the CPL. The effectiveness of the TL has been verified, under the condition that the
network topology on the TL is strongly $(2f+1)$-robustness. A decentralized controller is designed against unbounded Byzantine node attacks, which ensures UUB convergence. Based on the theoretical result,  a  hybrid formation is achieved against Byzantine attacks.
