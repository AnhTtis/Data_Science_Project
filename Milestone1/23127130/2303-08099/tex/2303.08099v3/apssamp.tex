...
\title[On adaptive low-depth quantum algorithms for robust multiple-phase estimation]{On adaptive low-depth quantum algorithms for robust multiple-phase estimation}
...
The difficulty level of multiple eigenvalue estimation depends on the gap between the dominant eigenvalues. The problem becomes more challenging when the gap gets smaller. 
...
\begin{proof}
    ...
    \begin{equation}
    \begin{aligned}
        R_{ij} := [2,4]\cap&\bigg\{m: \exists\ q\in\ZZ\backslash\{0\}\text{ such that }\\
        &\abs{\theta_i-\theta_j-\frac{q}{Mm}}\le \frac{1}{M}(\zeta_i+\zeta_j)\bigg\},\label{eq:uij}
    \end{aligned}
    \end{equation}
    ...
    \begin{aligned}
        &R_{ij} = [2,4]\cap\\
        &\left(\bigcup_{q=1}^{\infty}\left[\frac{q}{M\xiij+\deij},\frac{q}{M\xiij-\deij}\right]\right).
    \end{aligned}
    \end{equation}
...
\[
\begin{aligned}
&\left(B_\TT(M_{\ell-1}E_{\ell-1}, {\eta})+\frac{q}{m_\ell }\right)\\
&\cap B_\TT(M_{\ell-1}E_{\ell-1}, {\eta}) = \varnothing \mod 1,\quad \text{if }m_\ell \nmid q,
\end{aligned}
\]
...
\begin{aligned}
&\left(B_\TT(E_{\ell-1}, \frac{\eta}{M_{\ell-1}})+\frac{q}{m_\ell M_{\ell-1}}\right)\\
&\cap B_\TT(E_{\ell-1}, \frac{\eta}{M_{\ell-1}}) = \varnothing \mod 1,\quad \text{if }m_\ell \nmid q.
\end{aligned}
\end{equation}
...
\begin{aligned}
&\left(B_\TT(E_{\ell-1}, \frac{\eta}{M_{\ell-1}})+\frac{q'}{M_{\ell-1}}\right)\\
&\cap B_\TT(E_{\ell-1}, \frac{\eta}{M_{\ell-1}}) = \varnothing\mod 1,\quad \text{if }M_{\ell-1}\nmid q',
\end{aligned}
\end{equation}
...
When the left inequality of \eqref{eq:cond eta} is tight, i.e. $\eta = \Theta(\frac{\omega}{\beta}S^2)$, we will have the following corollary that gives a smaller prefactor of $T_{max}$ compared with \Cref{cor: real complexity}.
...
\begin{proof}
    ...
    \[
    \begin{aligned}
    &\min_{\substack{1\le s\le s'\le S\\q\in \ZZ, m_\ell\nmid q}} \left|M_{\ell-1}(\lambda_s-\lambda_{s'})-\frac{q}{m_\ell}\right| \\ 
    &\ge \min_{\substack{1\le s\le s'\le S\\q\in \ZZ, m_\ell\nmid q}} \left|\theta_s-\theta_{s'}-\frac{q}{m_\ell}\right| -S\eta\ge\frac{1}{4p_{\half{S(S-1)}}p_{\half{S(S-1)}+1}}.
    \end{aligned}
    \]
    Thus
    \begin{equation}\label{eq:gapstep}
    \begin{aligned}
    \min_{\substack{1\le s\le s'\le S\\q\in \ZZ, m_\ell\nmid q}} &\left|M_{\ell}(\lambda_s-\lambda_{s'})-{q}\right| \ge \frac{m_\ell}{4p_{\half{S(S-1)}}p_{\half{S(S-1)}+1}} \\ 
    &\ge \frac{1}{2p_{\half{S(S-1)}}p_{\half{S(S-1)}+1}}\ge\tilde{\Delta}.
    \end{aligned}
    \end{equation}
...
The following theorem gives the detailed description and complexity bound of the hybrid algorithm when we have access to the real powers of the given unitary $U$.
...
\begin{aligned}
    &\left(B\left(E_{\ell-1},\frac{\max\{\tilde{\Delta}, \eta\}}{2M_{\ell-1}}\right) + \frac{q}{M_{\ell-1}m_\ell}\right)\\ 
    &\cap B\left(E_{\ell-1},\frac{\max\{\tilde{\Delta}, \eta\}}{2M_{\ell-1}}\right) = \varnothing, \quad \forall q\in\ZZ\backslash\{0\},
\end{aligned}
\end{equation} 
...
Now we give the analysis of the hybrid algorithm for the integer-power model.
...
\begin{proof}
  ...
  We first show that the spectral gap can be sufficiently enlarged by applying a burn-in process using the method in \Cref{sec:ns}. The proof is similar to that of \Cref{thm:complexityint 2}. We enhance the induction hypothesis in \Cref{thm:intmk} by an additional condition
    ...
    \[
    \begin{aligned}
    &\min_{q\in \ZZ, m_\ell\nmid q} \left|M_{\ell-1}(\lambda-\lambda')-\frac{q}{m_\ell}\right| \\ 
    &\ge \min_{\substack{1\le s\le s'\le t\\q\in \ZZ, m_\ell\nmid q}} \left|\theta_s-\theta_{s'}-\frac{q}{m_\ell}\right| -S\eta\\ 
    &\ge\frac{1}{4p_{\half{S(S-1)}}p_{\half{S(S-1)}+1}},
    \end{aligned}
    \]
    ...
    \begin{aligned}
    \min_{q\in \ZZ, m_\ell\nmid q}& \left|M_{\ell}(\lambda-\lambda')-{q}\right| \ge \frac{m_\ell}{4p_{\half{S(S-1)}}p_{\half{S(S-1)}+1}} \\ 
    &\ge\min\left\{M_\ell\Delta, \frac{1}{2p_{\frac{S(S-1)}{2}}p_{\frac{S(S-1)}{2}+1}}\right\}.
    \end{aligned}
    \end{equation}
    ...
    \begin{aligned}
    \min_{q\in \ZZ, m_\ell\nmid q} &\left|M_{\ell}(\lambda-\lambda')-{q}\right| = |M_\ell(\lambda-\lambda')| \ge M_\ell \Delta \\ 
    &\ge \min\left\{M_\ell\Delta, \frac{1}{2p_{\frac{S(S-1)}{2}}p_{\frac{S(S-1)}{2}+1}}\right\}.
    \end{aligned}
    \end{equation}
    By the induction hypothesis, one also has
