    ...
    model enumeration, described in~\sref{sec:problem}. We show that a
    simple preprocessing can avoid this situation. \RSCHANGE{We transform first
...
\poslab{B_i}\vee\neg\neglab{B_i})$ \RSCHANGE{when both $\poslab{B_i}$ and
...
\begin{example}%
    ...
    \begin{alignat}{2}
        ...
        &(\pos B_1\vee\pos B_{7})&\wedge&\label{eq:ex3:vicnf:line11}\\
        & (\neg \poslab{B_2}\vee \neg \neglab{B_2}) &\wedge&\\
        & (\neg \poslab{B_3}\vee \neg \neglab{B_3}) &\wedge&\\
        & (\neg \poslab{B_4}\vee \neg \neglab{B_4}) & 
    \end{alignat}
...
$\etaB(\poslab{B_3})=\etaB(\poslab{B_3})=\bot$, the clauses
...
{\em every partial model $\muA$ for $\vi$ is also a model for
$\exists\allB.\vinnfcnfpg$}, that is, if $\muA\models\vi$, then there exists $\etaB$ s.t.\
$\muA\cup\etaB\models\vinnfcnfpg$.
...
\ignoreinshort{
  This is illustrated by the following theorem.
    \begin{theorem}%
    \label{th:existsetaB}
    For every partial truth assignment $\muA$ s.t.\ $\muA\models\vi(\allA)$, there exists a total truth assignment \etaB s.t.\ $\muA\cup\etaB\models\vinnfcnfpg$.
    \end{theorem}
    The proof of this theorem is shown in~\cref{sec:proofexistsetaB}.
  }
  
  % \ignoreinshort{
  %\ignore{%%%% tolto perche' secondo me misleading
    We stress the fact that this does not guarantee that the
    enumeration procedure always finds this $\etaB$, but only that
    such $\etaB$ exists. 
    \ignoreinshort{E.g., the enumeration procedure of \sref{sec:background:allsat} finds a total truth assignment $\etaA\cup\etaB$ that satisfies the formula, and then finds $\muA\subseteq\etaA$ that is minimal w.r.t.\ that specific $\etaB$ so that $\muA\cup\etaB\models\vinnfcnfpg$, thus the $\etaB$ found is not guaranteed to be the one that allows for the most effective minimization of $\muA$.}
    Ad-hoc enumeration heuristics should be investigated. %\\
    % }
    
\ignoreinshort{Notice that the above fact is agnostic w.r.t.\ the disjoint-AllSAT algorithm adopted, provided its output assignments match conditions (\ref{item:mumodelsvi})-(\ref{item:muAminimal}) in \sref{sec:background:allsat}.
}
