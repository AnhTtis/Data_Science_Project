...
used for enumeration: \RSCHANGE{\em if a minimal partial assignment \muA suffices
  to satisfy
$\vi$, this does not imply that \muA suffices to satisfy $\exists
\allB.\vicnfts$,} i.e., that some $\etaB$ exists 
such that $\muA\cup\etaB$satisfies $\vicnfts$ \cite{sebastiani_are_2020}.
...
which may not occur in $\muA$\RSCHANGE{, so that $\muAprime\supset\muA$}.
%\end{rschange}
%
...
minimal partial model $\muA$ for \vi, the solver may {be forced to} enumerate
many partial models $\muAprime$ that are minimal for
${\exists \allB.}\TseitinCNF(\vi)$ but not for 
$\vi$, so that their number can be up to exponentially larger in the
number of unassigned atoms in \muA.
In fact, each such model \muAprime conjoins to \muA one of the (up to $2^{|\allA|-|\muA|}$) partial
assignments which are needed to evaluate  to either $\top$ or $\bot$ all unassigned $\vi_i$'s.
\GMCHANGE{(E.g., in~\eqref{eq:ex1:muA:all}, the solver enumerates nine
...
instance, because $\vi_i$ occurs into some positive subformula $\vi_i\vee\vi_j$ and $\muA$ satisfies $\vi_j$. (In~\cref{ex1}, $\muA\defas\set{\neg A_3, \neg A_4, \neg A_7}$, $\vi_i\defas(A_1\wedge A_2)$ %and $\vi_j\defas(((A_3\vee A_4)\wedge(A_5\vee A_6))\iff A_7)$
or $\vi_i\defas(A_5\vee A_6)$ respectively.)
\RSCHANGE{\em Then, there is no guarantee that $\muA$ suffices to
  satisfy $\exists \allB.\vicnfts$, i.e., that there exists some $\etaB$
...
partial models that, although minimal for $\RSCHANGE{\exists \allB.}\TseitinCNF(\vi)$, are not minimal for $\vi$, so that their number can be up to exponentially bigger in the number of unassigned atoms, thus affecting the effectiveness of the enumeration.
}
\subsection{The impact of Plaisted and Greenbaum CNF transformation}
\label{sec:problem:polarity}

We point out how the \PlaistedCNF{}~\cite{plaisted1986structure} can be used to solve these issues, but only in part. We first illustrate it with an example.
\begin{example}%
    \label{ex2}
    ...
    clause representing $(B_1\imp(A_1\wedge A_2))$ is satisfied by
...
its effectiveness issues only in part, \RSCHANGE{{\em because a minimal assignment
