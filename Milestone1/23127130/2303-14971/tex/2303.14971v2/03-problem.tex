In this section we analyse the impact of different CNF-izations on the AllSAT task. In particular, we focus on \TseitinCNF~\cite{tseitin68} and \PlaistedCNF~\cite{plaisted1986structure}. We point out how CNF-izing AllSAT problems using these transformations can introduce unexpected drawbacks in the enumeration process. % when they are used to preprocess non-CNF formulas. 
In fact, we show that the resulting encodings can prevent the solver from effectively minimizing the models, and thus from enumerating a small set of short partial models.

\subsection{Tseitin CNF-ization}%
...
\begin{example}%
    ...
    \begin{alignat}{2}
        ...
        &\qquad(\pos B_5\vee\pos B_4\vee\pos A_7)\wedge(\pos B_5\vee\neg B_4\vee\neg A_7)&\wedge\nonumber%\tag{...}
        \\
        &\qquad(\pos B_1\vee\pos B_5)&\wedge\label{eq:ex1:vicnf:line6}
    \end{alignat}
    \end{subequations}
    The fresh atoms $\allB\defas\set{B_1, B_2, B_3, B_4, B_5}$ label sub-formulas as in~\eqref{eq:ex1:vi}. The solver proceeds to compute $\TA{\exists\allB.\vicnfts}$ by enumerating the models of $\vicnfts$ projected over \allA{}. 
    % As described in~\cref{sec:background}, the solver first enumerates a total truth assignment $\mu\defas\muA\cup\etaB$ such that $\mu\models\vicnf$. Then, the minimization step finds a minimal $\muAprime\subseteq\muA$ s.t.\ $\muAprime\cup\etaB\models\vicnf$.
    ...
    % $\muAprime\defas\set{\neg A_1, A_3, A_4, A_5}$ s.t.\ $\muAprime\cup\etaB\models\vicnf$. Notice that $\muAprime$ is minimal, even though it assigns a truth value also to $A_1$. In fact, whereas first two clauses in~\eqref{eq:ex1:vicnf:line1} are satisfied by $\neg B_1$, the last one must be satisfied by assigning either $\neg A_1$ or $\neg A_2$.
    ...
    $\muAprime\cup\etaB{}\models\vicnfts$.
    One possible output of this procedure is the minimal assignment:
    \begin{equation}%
        \label{eq:ex1:muAprime}
        \muAprime\defas\set{\neg A_1,\neg A_3,\neg A_4,\neg A_5,\neg A_6,\neg A_7}.
    \end{equation}
    %Notice that $\muAprime$ is minimal for $\vicnf$, meaning that any subset $\muAsecond\subset\muAprime$ is s.t.\ $\muAsecond\cup\etaB\not\models\vicnf$. %We remark that this is not a coincidence, in fact, there does not exist any $\etaBprime$ s.t.\ $\muAprime\cup\etaBprime\models\vicnf$.
    %$\residual{\vicnf}{\muAprime\cup\etaB}\neq\top$. 
    We notice that the partial truth assignment $\muA$~\eqref{eq:ex1:muA} satisfies $\vi$ and it is s.t.\ $\muA\subset\muAprime$, but it does not satisfy $\exists\allB.\vicnfts$.
    ...
    (\neg A_1\vee\neg A_2)\wedge(\neg A_5)\wedge(\neg A_6)$. 
    We remark that this is not a coincidence, since there is no $\etaBprime$ such that $\muA\cup\etaBprime\models\vicnfts$, because~\eqref{eq:ex1:vicnf:line1} and~\eqref{eq:ex1:vicnf:line3} cannot be satisfied without assigning any atom in $\set{A_1,A_2}$ and $\set{A_5,A_6}$ respectively.

    Finding $\muAprime$~\eqref{eq:ex1:muAprime} instead of $\muA$~\eqref{eq:ex1:muA} clearly causes an efficiency problem, since finding longer partial models implies that the total number of enumerated models could be up to exponentially larger.
    For instance, instead of the single partial assignment $\muA$~\eqref{eq:ex1:muA}, the solver may return the following list of 9 partial assignments satisfying $\exists\allB.\vicnfts$:\\ 
    $
        ...
        \end{array}
    $\\
    where $\muAprime$~\eqref{eq:ex1:muAprime} is the first in the list.\exdone{}
...
instance, because $\vi_i$ occurs into some positive subformula $\vi_i\vee\vi_j$ and $\muA$ satisfies $\vi_j$. (In~\cref{ex1}, $\muA\defas\set{\neg A_3, \neg A_4, \neg A_7}$, $\vi_i\defas(A_1\wedge A_2)$ %and $\vi_j\defas(((A_3\vee A_4)\wedge(A_5\vee A_6))\iff A_7)$
or $\vi_i\defas(A_5\vee A_6)$ respectively.) \TseitinCNF{}
...
As a consequence, by using \TseitinCNF{}, the solver may enumerate partial models that, although minimal for $\TseitinCNF(\vi)$, are not minimal for $\vi$, so that their number can be up to exponentially bigger in the number of unassigned atoms, thus affecting the effectiveness of the enumeration.


\subsection{Plaisted\&Greenbaum CNF-ization}
\label{sec:problem:polarity}

We point out how the \PlaistedCNF{}~\cite{plaisted1986structure} can be used to solve these issues, but only in part. We first illustrate this issue with an example.
\begin{example}%
    \label{ex2}
    ...
    \begin{alignat}{2}
        ...
        &\qquad(\pos B_1\vee\pos B_5)&\wedge\label{eq:ex2:vicnf:line6}
    \end{alignat}
    \end{subequations}
    ...
    \begin{equation}
        \muAsecond\defas\set{\neg A_3,\neg A_4,\neg A_5,\neg A_6,\neg A_7}.
    \end{equation}
    %s.t.\ $\muA$ is minimal and $\muA\cup\etaB\models\vi$.
    %$\residual{\vi}{\muA\cup\etaB}=\top$.
    This assignment is a strict sub-assignment of $\muAprime$ in~\eqref{eq:ex1:muAprime}, since the atom $A_1$ is not assigned. This is possible because the sub-formula $(A_1\wedge A_2)$ is labelled by $B_1$ using a single implication, and the clause representing $(B_1\imp(A_1\wedge A_2))$ is satisfied by $\etaB(B_1)=\bot$ even without assigning $A_1$ and $A_2$. Nevertheless, the assignment $\muA$~in~\eqref{eq:ex1:muA} that satisfies $\vi$ still does not satisfy $\exists\allB.\vicnfpg$.
    Indeed, sub-formulas occurring with double polarity are labelled using double implications as for \TseitinCNF{}, raising the same problems as the latter. For instance, the sub-formula $(A_5\vee A_6)$ occurs with double polarity, since it is under the scope of a $\iff$. Hence, the clauses in~\eqref{eq:ex2:vicnf:line3} must be satisfied by assigning a truth value also to $A_5$ or $A_6$, and so the partial truth assignment $\muA$ in~\eqref{eq:ex1:muAprime} does not satisfy $\vicnfpg$ because $\residual{\vicnfpg}{\muA\cup\etaB}=(\neg A_5)\wedge(\neg A_6)$.\exdone{}
\end{example}

The example above shows that \PlaistedCNF{} has some advantage over \TseitinCNF{} when enumerating partial assignments, but it overcomes its effectiveness issues only in part.
    Consider, as in~\sref{sec:problem:label}, a generic non-CNF formula $\vi(\allA)$ and a partial truth assignment $\muA$ that satisfies $\vi$ without assigning a truth value to some sub-formula $\vi_i$. Suppose that $\vi_i$ occurs only positively in $\vi$ ---whereas for the negative case the reasoning is dual. (In~\cref{ex2}, $\muA\defas\set{\neg A_3, \neg A_4, \neg A_7}$, $\vi_i\defas(A_1\wedge A_2)$.)\@
    ...
    Notice that the sub-formulas that should be treated as ``don't care'' must have their label assigned to false. This can be achieved by instructing the solver to split on negative value in decision branches\footnote{To exploit this heuristic also for sub-formulas occurring only negatively, the latter can be labelled with a negative label $\neg B_i$ as $(\neg B_i\limp\vi_i)$.}. 
