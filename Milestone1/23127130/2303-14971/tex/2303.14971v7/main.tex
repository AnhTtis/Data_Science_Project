...
\addbibresource{jair-allsat-cnfs.bib}
...
\begin{abstract}
  ...
  effectiveness of CNF conversions for
  %\ignoreinlong{disjoint} 
  SAT and SMT enumeration. On the negative side, we show that: (i)~Tseitin
  transformation prevents the solver from producing short partial assignments,
  thus seriously affecting the effectiveness of enumeration; (ii)~Plaisted and
  Greenbaum transformation overcomes this problem only in part. On the positive
  side, we prove theoretically and we show empirically that combining Plaisted
  and Greenbaum transformation with NNF preprocessing upfront \mbox{---which} is
  typically not used in solving--- can fully overcome the problem and can
  drastically reduce both the number of partial assignments and the execution
  time. \ignore{This analysis is validated by an experimental evaluation on
    non-CNF problems originating from both synthetic and real-world-inspired
    applications.}%\ignoreinshort{%} % mia aggiunta%significantly
\end{abstract}
