...
\end{equation}
where, the \(\sup\) is taken over distributions \(\distY\in\modelDist\) parameterized by \(\challenge\), and the \(\inf\) is over all estimators with \(n\) samples drawn according to \(\distY\). 
...
From Markov's inequality, we have:
    ...
    \begin{split}
        ...
        &\geq \frac{\Delta^{2}}{M}\inf_{\hat{z}}\sum_{i\in[M]}\prob[\|\challenge_{i} - \zhat\|_{2}\geq\Delta]
    \end{split}
    \end{equation}
For any estimator \(\zhat\), we define a minimum distance test \(\Psi\) as
    \begin{equation}
        \Psi(\zhat) = \underset{j\in M}{\operatorname{argmin}}\|z_{j}-\zhat\|_{2}
    \end{equation}
    ...
    \begin{split}\label{eq:hyp-reduction}
        \minimax &\geq \frac{\Delta^{2}}{M}\inf_{\hat{z}}\max_{i\in[M]}\prob[|\challenge_{i} - \zhat|_{2}\geq\Delta]\\
        &\geq \frac{\Delta^{2}}{M}\inf_{\Psi}\sum_{i\in[M]}\prob_{z_{i}}[\Psi\neq i]
    \end{split}
    \end{equation}
For \(M=2\), by Lemma \ref{lemma:le-cam}, we have
\begin{equation}
\label{eq:reduction-lecam}
    \minimax \geq \frac{\Delta^{2}}{2}(1 - \|P_{1}^{n}-P_{2}^{n}\|_{TV})
\end{equation}
...
1. The choice of \(\Delta\) is instrumental in determining the tightness of the lower bound. As \(\Delta\) grows, \(\Delta^{2}\) is larger, but it becomes increasingly easy to distinguish between the distributions \(P_{1}\) and \(P_{2}\), thereby reducing the probability of error for the test \(\Psi\)

2. Total variation distance is not very well attuned to the i.i.d. sampling of \(n\) samples from the product distributions \(P^{n}_{1}, P^{n}_{2}\). The KL Divergence is more suited for this analysis, owing to the tensorization property \(D_{KL}(P^{n}||Q^{n}) = nD_{KL}(P||Q)\). 
To this end, we first represent the total variation distance between the distributions \(P_{1}\) and \(P_{2}\) as a function of the probability density functions \(p_{1}(x)\) and \(p_{2}(x)\) for \(x\in\mathcal{X}\) 
    \begin{equation}
        \begin{split}
            1 - \|P_{1}-P_{2}\|_{TV} & =  \int min\{p_{1}(x), p_{2}(x)\}dx\\
                            & \geq \frac{1}{2}\int min\{dP_{1}, dP_{2}\} \int max \{dP_{1}, dP_{2}\}\\
                            & \geq \frac{1}{2} \left( \int \sqrt{min\{dP_{1}, dP_{2}\}, max\{dP_{1}, dP_{2}\}} \right)^{2}\\
                            & = \frac{1}{2}\left(\int\sqrt{dP_{1} dP_{2}}\right)^{2}\\
                            & = \frac{1}{2}\left(\int dP_{1}\sqrt{\frac{dP_{2}}{dP_{1}}}\right)^{2}\\
                            & = \frac{1}{2}\left(\mathbb{E}_{P_{1}}\left[\sqrt{\frac{dP_{2}}{dP_{1}}}\right]\right)^{2}\\
    ...
    \begin{equation}
        \begin{split}
            ...
            & = \frac{\Delta^{2}}{4}\exp\left(-\mathbb{E}_{P_{1}}\left[\log\left(\frac{dP_{1}}{dP_{2}}\right)\right]\right)\\
...
\textbf{Claim 1. Theorem 1 of Guo et.~al.\cite{Guo2022} gives vacuous Lower Bounds for \(\epsilon < \left(1 + \frac{d}{4}\right)\).}
%\begin{remark}
...
\begin{proof}
    ...
    \end{equation}
    \paragraph{Case 2: $\epsilon\in[0,1]$.}
    %This gives us 
    \begin{equation}
...
Recall that the reconstruction process detailed in Section \ref{sec:problem-formulation} admits the following privacy game: 
...
2. The adversary draws \(n\) samples \(\{h_{1}, ..., h_{n}\}\) from the output distribution \(P_{z}\) induced by \(\learner(\dataset\cup\{\challenge\})\) and outputs a reconstruction \(\zhat\)

This can be interpreted as the Markov chain: \(\challenge\rightarrow\{h_{1}, ..., h_{n}\}\rightarrow\zhat\). We have from Lemma \ref{lemma:fanos}
\begin{equation}
\label{eq:minimax}
...
\end{equation}
where \((1)\) follows from Data Processing Inequality and (2) follows as a result of the convexity of \(\log\), and tensorization property of KL divergence. Note that the last inequality is an upper bound for all pairs \(z,z'\in Z_{\Delta}\). We construct a local packing of \(\dataspace\), such that:\\
1. \(|z-z'|\geq 2\Delta\)\\
