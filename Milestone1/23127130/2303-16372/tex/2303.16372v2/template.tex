...
\label{eq:reduction-lecam}
    \minimax \geq \Delta^{2}(1 - \|P_{1}^{n}-P_{2}^{n}\|_{TV})
\end{equation}
...
2. Total variation distance is not very well attuned to the i.i.d. sampling of \(n\) samples from the product distributions \(P^{n}_{1}, P^{n}_{2}\). The KL Divergence is more suited for this analysis, due to the tensorization property \(D_{KL}(P^{n}||Q^{n}) = nD_{KL}(P||Q)\). 
...
This can be interpreted as a Markov chain: \(\challenge\rightarrow\{h_{1}, ..., h_{n}\}\rightarrow\zhat\). We have from Lemma \ref{lemma:fanos}
\begin{equation}
\label{eq:minimax}
