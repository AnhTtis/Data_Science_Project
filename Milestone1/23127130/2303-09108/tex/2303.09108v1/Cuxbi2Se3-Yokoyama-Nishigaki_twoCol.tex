...
%	
\begin{figure}[htbp]
	\includegraphics[clip,width=80mm]{Fig3.pdf}
...
%
\begin{figure}[htbp]
	\includegraphics[clip,width=80mm]{Fig4.pdf}
...
\begin{figure}[htbp]
	\includegraphics[clip,width=80mm]{Fig5.pdf}
...
agree well with the simulation of a single ellipse (a single component), the data for $x$=0.20 and 0.27 do not. There is a small second component, as can be seen in the lower panels of  Fig.\ref{x=0.2}(b) and Fig.\ref{x=0.27}(b). For $x$=0.20, the $\theta$ for the small, second component is  -32$^{\circ}$, while it is 79$^{\circ}$ for the main component. As the difference in $\theta$ is 111$^{\circ}$ which is close to 120$^{\circ}$, the two components can be regarded as belonging to the same gap symmetry in view of the trigonal crystal symmetry. For $x$=0.27, however, the difference in $\theta$ is about 82$^{\circ}$, which is closer to 90$^{\circ}$ and the two components correspond to %gapscome from two crystal domians with 
different  symmetries that are orthogonal to each other. 
...
with the   $\textbf{{d}}$-vector rotated by an angle of -$\delta$. This is exactly we have found for the $x$=0.20 and 0.27 crystals.
...
At low fields, the two-fold nematic behavior in the physical quantities is well understood as due to the \dv\ pinning to a certain direction by, {\it e.g.},  disorder or defects,
...
with $\theta$=80$^{\circ}$ for $x$=0.15 and $\theta$=79$^{\circ}$ for $x$=0.20, 
 it is restored to the high symmetry axis directions at high fields $H\geq$1.5 T. 
 This means that the phonon-induced interaction  $E_{\rm em-ph}$ is also smaller than the Zeeman interaction. % which, in terms of magnetic field, is $H_{\rm pin}$ =  1.0$\sim$1.2 T. 
%More importantly, s
...
In contrast, the ellipse for $x$=0.27 at low fields has an elliptical direction along  $\theta$=-10$^{\circ}$ and thus the gap
...
This work was supported in part by the JSPS Grants (19H00657, 20K03862 and 22H04482).
%\end{acknowledgments}
%
