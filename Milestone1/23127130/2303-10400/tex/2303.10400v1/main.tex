...
Supported by IBS-R029-C4.
 Emails: hongliu@ibs.re.kr, salianika@gmail.com. }
\and
Nika Salia\footnotemark[2]
}
...
By Erd\H{o}s-Gallai theorem and Kopylov's result~\eqref{Eq:Erd_gallai_connected}, we see that the asymptotic of the maximum number of edges in an $n$-vertex $P_k$-free graph does not change by an additional connectivity constraint as $n\rightarrow \infty$ and $k\rightarrow \infty$. Caro, Patk\'{o}s, and Tuza~\cite{caro2022connected} studied how much smaller $\ex_c(n,T)$ can be for a $k$-vertex tree $T$, compared to $\frac{n(k-2)}{2}$. Formally, they defined
...
\begin{proposition}\label{claim:barycenter}
    Every tree has at least one barycenter and at most two joined by an edge.
\end{proposition}
\begin{proof}
...
It is easy to see that $e(\textsf{S}_{n,x})=(\frac{1}{4}-o_k(1))kn$ and $e(\textsf{P}_{n,x})=(\frac{1}{4}-o_k(1))kn$. Now we provide constructions for the lower bound in Theorem~\ref{thm:brooms}.

\medskip

\noindent\textbf{The family $\textsf{F}_{n,\cdot,\cdot}$.} 
For $n>d\geq 2$, let $\textsf{F}_{n,d,1}$ be the $n$-vertex connected graph such that  every maximal $2$-connected block is $K_{d-1}$ except at most one clique of size $n-\floor{\frac{n-1}{d-2}}(d-2)$, all sharing a common vertex.
...
Let $\textsf{F}_{n,d,2}$ be the $n$-vertex connected graph such that  every maximal $2$-connected block is a clique with one of size $d-1$, the rest of size $d-2$ except at most one clique of size $n-1-\floor{\frac{n-2}{d-3}}(d-3)$, all sharing a common vertex.
...
If $p_3\geq q_3$, let $\textsf{F}_{n,d,3}$ be the $n$-vertex connected graph such that it contains $p_3$ maximal $2$-connected blocks $G_1,\ldots, G_{p_3}$ all sharing a common vertex $v$ with $p_3-q_3$ of them being $K_{d-2}$. The rest of the maximal $2$-connected blocks $G_i$ are the cliques of size $d-1$ without a perfect matching of $G_i-v$. 
We have
\[
e(\textsf{F}_{n,d,3})=\frac{(d-2)(n-1)}{2}.
\]
If $p_3<q_3$, let $\textsf{F}_{n,d,3}$ be the $n$-vertex connected graph such that it contains $p_3+1$ maximal $2$-connected blocks $G_1, \ldots, G_{p_3+1}$ all sharing a common vertex $v$ with $p_3$ of them being the cliques of size $d-1$ without a perfect matching of $G_i-v$ and the remaining one is a clique of size $q_3-p_3+1$.
We have
\[
...
Woodall~\cite{woodall1976maximal} and independently Kopylov~\cite{kopylov1977maximal} improved Erdős-Gallai theorem~\cite{erdHos1959maximal} for long cycles and obtained the following exact result for every $n$, see also~\cite{faudree1975path}. 
...
\begin{claim}\label{claim:Balanced}
    Let $c$ be a constant and let $S_1$ and $S_2$ be vertex disjoint sub-trees of $T_k$ such that $v(S_1)+v(S_2)\geq \frac{k-c}{2}$. If each of $S_1$ and $S_2$ is a spider\footnote{Spider is a tree with all vertices of degree at most two, except possibly one vertex of any degree, referred to as the central vertex of the spider.} with  the central vertex of degree at most three.
...
Let $x$ be a number such that $a_x-x+2\geq \frac{k-c}{4}$, for some constant $c$. Consider an embedding of $T_k$ in $\textsf{S}_{n,x}$. Note first that $v$ is  not  mapped to $w$, otherwise, as at least two of the trees $T_1,T_2,T_3$ are not a path,  two of them contain a sub-path of length at least $\frac{k-c}{4}-2$ from the vertex $w$ to a vertex of a clique, 
and we are done by Claim~\ref{claim:Balanced}.
The same argument shows that, every embedding  of $T_k$ in $\textsf{P}_{n,x}$ maps $v$ to a vertex of a clique for each $x$ such that $a_x-x+2\geq \frac{k-c}{4}$. 
...
\draw (15.3,19.5) node[anchor=center] {$P_{\geq a_{x_1}-x_1+2}$};
...
By the previous paragraph, every embedding  of $T_k$ in $\textsf{S}_{n,x_1}$ maps  $v$  to a vertex of a clique.
...
Indeed, $T_k$ does not contain a pair of vertices from different cliques in $\textsf{S}_{n,x_1}$, otherwise there is a sub-path of $T_k$ of length $2(a_{x_1}-x_1+1)\geq \frac{k}{2}-10$ and we are done by Claim~\ref{claim:Balanced}.
...
Thus $T_k$ contains an edge such that after removing it from $T_k$ we get a component where degrees of the vertices are at most three and it contains a sub-path of length  $a_{x_1}-x_1+1$, each vertex of this path has degree at most two in~$T_k$. 
 By the previous two observations and Claim~\ref{claim:Balanced} there exists an   edge $v_1v_2$ of $T_k$ such that $T_k- v_1v_2$  contains a component denoted by $S_y$ (containing the vertex $v_2$) isomorphic to a spider with $y$ vertices and  at most three legs, with one leg of length at least $a_{x_1}-x_1+1$, see Figure~\ref{fig:ConstructionsII}.  
...
If $v$ is mapped to $w$, then there are two vertex disjoint paths of length $a_{x_2}-x_2+1$ from $v$ since at least two of the trees $T_1,T_2,T_3$ are not isomorphic to a path. 
Furthermore, one of them is vertex disjoint from $S_y$ and  we are done by Claim~\ref{claim:Balanced}, since $a_{x_2}-x_2+2+y\geq \frac{2(k-y-1-k/2)^2}{k}+\frac{k}{2}-2\geq\frac{k}{2}-2$ for $x_2=k-y-1$.
...
Indeed, If  $v_1$ is mapped in the same clique as $v$ and part of $S_y$ is mapped out of that clique, then the number of vertices outside of the clique is at most $y$ while the size of the clique is $k-y-1$ and does not have enough room, a contradiction.
...
For the lower bound, let $n>k-2\geq d\geq 2$. First, we consider $\textsf{G}_{n,d,\floor{\frac{d-1}{2}}}$ and an almost $(k-d)$-regular graph $\textsf G^\prime$, which is $(k-d)$-regular graph if $(k-d)n$ is even, and if $(k-d)n$ is odd then every vertex of $\textsf G^\prime$ has degree $k-d$ except one vertex of degree $k-d-1$. Clearly, $\textsf{G}_{n,d,\floor{\frac{d-1}{2}}}$ is $P_{d+1}$-free, thus it does not contain $B(k,d)$ as a subgraph. The almost $(k-d)$-regular graph $G^\prime$ is $B(k,d)$-free as it does not contain a vertex of degree at least $k-d+1$ and for every $n>k-d$ the almost ($k-d$)-regular graph $\textsf{G}^\prime$ always exists. The number of edges in these graphs are $e(\textsf G^\prime)=\floor{\frac{(k-d)n}{2}}$ and 
\[
...
At first, we show that there exists a path of at least $dk$ vertices starting at $v$. 
...
Thus by the connectivity of $G$, we have a path of $kd$ vertices starting at  the vertex $v$.
Let $P=vu_2u_3\cdots u_{dk}$ be a path of order $kd$ starting at $v$, a vertex of maximum degree. Note that, we have $u_2\in N(v)$. 
...
By Theorem~\ref{thm:2-connected_Kopylov}, for each $i\in [s]$ and $n_i\geq d-1$ we have $e(G_i)\leq \max\{e(\textsf{G}_{n_i,d-1,2}), e(\textsf{G}_{n_i,d-1,\floor{\frac{d-2}{2}}})\}$.  It is easy to see that there exists a constant $a<2d$ such that  $e(\textsf{G}_{n_i,d-1,2})\leq e(\textsf{G}_{n_i,d-1,\floor{\frac{d-2}{2}}})$ if and only if $n_i\geq a$.\footnote{In particular if $d$ is even  $a=\frac{5d-16}{4}$ and if $d$ is odd then $a=\frac{5d-13}{4}$.} Thus we have
\begin{equation}\label{eqcase1}
\begin{displaystyle}
e(G_i) \leq 
\left.
\begin{cases}
 \floor{\frac{d-2}{2}}\left(n_i-\ceil{\frac{d}{2}}\right)+\binom{\ceil{\frac{d}{2}}}{2} \:\quad\quad\mbox{if  $n_i\geq \max\{a, d-1\}$,} \\
...
Let $\ell=\floor{\frac{d-1}{2}}\ceil{\frac{d+1}{2}}-\binom{\ceil{\frac{d+1}{2}}}{2}$. We claim that if one of the following statements holds, then $e(G)\leq e(G_{n,d,\floor{\frac{d-1}{2}}})$.
...
If (iii) holds, then by the same calculation as in (ii) we have $e(G)=\sum_{i\in I}e(G_i)+\sum_{j\in [s]\setminus I}e(G_j)\leq e(G_{n,d,\floor{\frac{d-1}{2}}})$.

If (i), (ii) and (iii) do not hold, then we have $n_1<a$ and $n_{\ell}\leq d-2$ and the number of $G_i$ with $\ell \leq i\leq s$ which is not isomorphic to $K_{d-2}$ is at most $\ell-1$, so there must exist two blocks $G_{j_1}$ and $G_{j_2}$ isomorphic to $K_{d-2}$, since $n\geq k^{dk}>(\ell-1)(a+(d-2))+2(d-2)$. 
Note that $G$ is $B(k,d)$-free and $G$ is connected, we must have $2(d-2)-1\leq k-1$ if $k-d+1\leq d-3$, thus $d\leq \frac{k+4}{2}$. 
...
Similarly, as in the previous case, we define threshold $a$.\footnote{Here $a=\frac{5d-8}{4}$ if $d$ is even and $a=\frac{5d-11}{4}$ if $d$ is odd.} Thus we have 
...
In this paper, for the Tur\'an problem, we determine the maximum effect the additional connectivity condition could have over all trees. An interesting future direction of research would be to identify appropriate parameters (if exist) of a tree that determine the asymptotic behavior of its connected extremal number. The constructions in this paper could be useful for this problem. 
...
The reader interested in the problems and extensions related to Erd\H{o}s-S\'{o}s conjecture we refer to  the following papers \cite{rozhon2019local,mclennan2005erdHos,goerlich2016erdHos,wozniak1996erdos,lidicky2012turan}, extensions for Berge hypergraphs see \cite{gerbner2020general,gyHori2019tur}, extensions for colored graphs see \cite{salia2019erdHos}. 
...
\end{proposition}
\begin{proof}
    Let $C_{\ell}$ be a cycle of $F$. Note that $ex(n, C_{\ell})=\floor{\frac{n^2}{4}}$ if $\ell$ is odd and $ex(n, C_{\ell})\geq cn^{1+4/(3\ell-4)}$ for some constant $c$ if $\ell$ is even (see \cite{Lazebnik1995new}).
