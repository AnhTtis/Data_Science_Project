...
\author{Hao Shi$^{1,4,}$\thanks{The first two authors contribute equally to this work.}\, ,
~~Yu Li$^{2,*}$,
...
To achieve this goal, we must overcome several key challenges: First, how to effectively take advantage of the strong coupling characteristics of these two sub-tasks to realize a win-win situation? Second, since existing fisheye semantic segmentation datasets cannot provide images and semantic ground truth outside the FoV, how can we obtain fisheye beyond-FoV training data with complete annotations?
...
As for the data challenge, we leverage the popular Cityscapes~\cite{cordts2016cityscapes} and KITTI360~\cite{liao2022kitti} semantic segmentation datasets via pinhole-to-fisheye projection and derive the new CityScapes-BF and KITTI360-BF benchmark variants, therefore enabling training and evaluation of fisheye semantic completion.
...
On the derived CityScapes-BF benchmark, FishDreamer achieves state-of-the-art performance with a mIoU of $54.54\%$ and a peak-SNR of $25.05dB$, showing a promising result.
...
%   \vspace{-2em}
   \caption{\emph{FishDreamer: Fisheye semantic completion transformer.} Taking a single fisheye image as input, the model predicts pixel patterns and object labels beyond fisheye vision. The two sub-task heads interact each other via the proposed Polar-aware Cross Attention module bidirectionally.}
   \label{fig:overview}
%   \vspace{-2.0em}
\end{figure}
...
\begin{figure*}[!t]
   \centering
   \includegraphics[width=1.0\linewidth]{Figures/compare-v2.pdf}
  % \vspace{-2em}
...
Without using the polar mask, the three ways of S2P, P2S, and Bi-direction achieve respective $53.73$, $54.17$, and $53.89$ in mIoU of semantic completion task, and $24.93$, $24.56$, and $24.82$ in image completion PSNR.
