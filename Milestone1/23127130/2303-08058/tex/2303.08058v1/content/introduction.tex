...
underlying API,
allowing us to obtain an HPX future for asynchronous API calls (for example for
\lstinline[language=c++]{cudaLaunchKernel}).
...
operations. This allows a user to define an entire graph of operations with
SYCL, but in the end to get one HPX future for the final result, which can be
used to schedule communication to other nodes (or run arbitrary other CPU
...
graph using \lstinline[language=c++]{host_task}s depending on SYCL events. One
way to integrate HPX and SYCL is to directly use a
\lstinline[language=c++]{host_task} to set an associated future to the ready
...
does not rely on the \lstinline[language=c++]{host_task}s and instead uses
event polling implemented within the HPX scheduler, as we found that this
vastly outperforms the alternative.
...
\item We collected runtime data using Octo-Tiger on an AMD MI100 and an
  NVIDIA A100, using not only the SYCL
    execution space, but also the same compute kernels running on the
    respective native execution spaces (CUDA,
...
fully incorporate SYCL into their own applications (either by using the Kokkos
...
it work asynchronously with the Kokkos SYCL execution space and, thus, with
