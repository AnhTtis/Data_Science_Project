...
In this section, we describe how we integrated HPX with SYCL, HPX-Kokkos, and
...
developed at the University of Heidelberg~\cite{alpay2020sycl} and support all
three CPU architectures using OpenMP and Intel
...
DPC\texttt{++} are the only implementation supporting all three GPU
...
for USM and \lstinline{in_order} queues. Support for SYCL
\lstinline{host_task}s is actually optional, as only one of our integration
...
\lstinputlisting[language=C,float=*,caption={Using the basic HPX-SYCL integration.},label={lst:code1},deletekeywords={for}]{./code.tex}

%\begin{lstlisting}[float,floatplacement=*t,caption={Using the basic HPX-SYCL integration.},label={lst:code1},deletekeywords={for}]
...
There are two distinct ways to implement this: We can either use SYCL
\lstinline{host_task}s directly or implement an event polling scheme within the
HPX scheduler.

\subsubsection{Integration using SYCL Host Tasks}
\label{sec:integration:hosttasks}
SYCL allows creating host tasks that depend on SYCL events. Once the required
events enter the status \texttt{info::event\_command\_status::complete}, the
host task is triggered and executed on the CPU by a thread managed by SYCL
runtime. This host task may include arbitrary C\texttt{++} code, hence we can
...
potential continuations defined by the user. Hence, we use the host task as a
...
already). Nevertheless, we include this method here, as it seems to the most
...
Instead of using host tasks, we can make use of HPX runtime itself here. As we
...
(again, turning the future ready and triggering potential continuations). We add this
\lstinline[language=c++]{event_callback} struct to the scheduler. To keep the
...
executed. If not, the event\_callback is moved to a vector to be polled again
...
ready eventually, without ever having to call any blocking methods such as the event
\lstinline[language=c++]{wait()} method. Instead, we merely have to poll
...
In case we need a future for an in\_order SYCL queue without having any SYCL
event available, we provide an overload for \lstinline{get_future} that inserts
a dummy SYCL single\_task and uses its SYCL event to get a future. It is
checked that the queue is in\_order as only in this case the dummy kernel will
...
we do not have their events anymore (as those commands might be submitted
...
\texttt{-fsycl} flag for the SYCL-related source files (such as tests and the
source file that implements polling) and \texttt{-fno-sycl} to all others. Our
...
this point, as the poll function creates a temporary \lstinline{event_callback}
...
make sure that the hipSYCL runtime remains alive with
\lstinline{HIPSYCL_PERSISTENT_RUNTIME=1}. However, to be certain that the user
...
a SYCL HPX executor. This executor wraps an in\_order SYCL command queue and
allows the dispatch of function calls by wrapping them in either
\lstinline[language=c++]{hpx::async} (two-way execution, which means that it
returns an \lstinline[language=c++]{hpx::future}) or
\lstinline[language=c++]{hpx::apply} (one-way execution without a future).
...
that also support \lstinline[language=c++]{hpx::async} and
\lstinline[language=c++]{hpx::apply}. In fact, we use an in-order command
...
functionality to make use of our own events in calls (which allows us to reuse
them, avoiding potentially expensive construction and destruction). Here, we
...
space to this list of supported spaces. This requires adding the correct
overloads, calling the correct \lstinline[language=c++]{get_future} method from
HPX instead of returning a dummy future, which is the default behavior for
...
executor instance. Not to be confused with the HPX-SYCL executor mentioned
above, this executor is merely a wrapper around a Kokkos SYCL execution space.
To keep the behavior the same as for other Kokkos execution spaces, we use
in-order command queues to construct this SYCL execution space. Lastly, we add
an additional overload for \lstinline{deep_copy_async} which directly uses the
...
std ones (\emph{i.e.}\ using \lstinline[language=c++]{sycl::sqrt} instead of
\lstinline[language=c++]{std::sqrt}) fixed this issue.
...
experimented with some changes to Kokkos. First, since we are using in\_order queues,
we noticed that we can remove some barriers within the SYCL execution space in
Kokkos. Furthermore, to use the Kokkos SYCL execution space on AMD GPUs, we
needed to make some modifications to the Kokkos CMake configuration to
correctly pass the arguments. These changes are not yet upstreamed as we would
like to refine and test them further first. However, both the patch removing
