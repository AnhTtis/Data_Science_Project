...
regarding adaptivity, data-distribution, and FMM performance as it uses the
...
HPX is an Asynchronous Many-Task Runtime system~\cite{kaiser2020hpx}. It is not
only itself implemented in C\texttt{++}, but also implements all C\texttt{++}
...
implements a programming model for developing portable kernel~\cite{9485033}.
...
implementation also works using
\lstinline[language=c++]{std::experimental::simd} on various CPUs. In previous
...
fact that the compute kernels only work on one sub-grid at time, the workload
...
simulation and old ones may be deleted and migrated (all depending on the
...
\end{figure}
During solver iterations, such as the gravity solver, Octo-Tiger traverses the
...
HPX-Kokkos, giving us yet another future. Each kernel itself can be executed in
...
Of course, we could already use the Kokkos SYCL execution space, albeit
...
able to return a future for the underlying SYCL kernel. Otherwise, the
...
launches of the same kernel on different sub-grids. As shown in detail in
