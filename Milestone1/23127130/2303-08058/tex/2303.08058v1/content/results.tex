...
time-step we call 15 GPU kernels per sub-grid, giving us 7680 GPU kernel calls
...
EPYC\textsuperscript{\texttrademark} 7H12 CPU. We use 32 cores on both CPUs to
keep the runs more comparable to each other.
...
get rid of some of those barriers when using in\_order queues, as this queue
...
The third change we apply is switching between the host task based HPX-SYCL
...
\texttt{get\_future} functionality. Depending on the configuration here, we
either call the event polling version or the host tasks version.
...
% When we have fewer overall kernel launches
% for the same work, we need fewer API calls.

The dynamic work aggregation also influences how often the
\lstinline[language=c++]{get_future} functionality is called. For each leaf
...
neighbors. With the work aggregation turned on (\emph{i.e.}\ a maximum larger
...
aggregated kernel. The other \lstinline[language=c++]{get_future} call is to
communicate the results. This means that we effectively decrease the number
...
For our first test, we take a look at the host task version of our integration
...
underlying in-order SYCL command queue).
...
previous two parameters that we found ($128$ GPU executors with up to eight
kernels aggregated on the A100). We then run this combination not only with
$32$ HPX worker threads but also with fewer cores. This is usually the most interesting
...
waiting on SYCL results (with the integration turned off) blocks the remaining
cores.
...
\end{figure*}
Given the disappointing results of the host\_task integration, it is clear that we
...
so on the lower core counts, however, even when using $32$ cores with the best
...
space is using an in\_order queue. When using it with Octo-Tiger we did not
...
\caption{Sedov Blast Scenario with Octo-Tiger using the available backends on both the A100 node (\ref{fig:a100-backend}) A100 and the MI100 node (\ref{fig:mi100-backend}). We used the best combinations for each backend and added CPU-only runs for comparison.}
