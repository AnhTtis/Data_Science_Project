...
mT5-Base & 75.4 & 86.4 & - & - & 67.0 / 49.0 & 57.2 / 41.2 & 64.6 / 45.0 & - & 55.7 & \underline{\textbf{28.1}}$^{\dagger}$ \\
XLM-R Large & 79.2 & 86.4 & 69.2 & - & 76.6 / 60.8 &	65.1 / 45.0	& 71.6 / 53.2 & 76.2 & 65.2 & -\\
TuLRv6 - XXL & \underline{\textbf{88.8}}$^{\dagger}$ & \underline{\textbf{93.2}}$^{\dagger}$ & \textbf{82.2}$^{\dagger}$ & - & \underline{\textbf{86 / 72.9}}$^{\dagger}$ & \underline{\textbf{84.6 / 73.8}}$^{\dagger}$ & \underline{\textbf{81 / 63.9}}$^{\dagger}$ &  \underline{\textbf{83.0}}$^{\dagger}$ &  \underline{\textbf{84.7}}$^{\dagger}$ & - \\
\midrule
...
\texttt{gpt-3.5-turbo} & 62.1 & 70.0 & 81.0 & 87.7 & 60.4 / 38.2 & 60.1 / 38.4 & \textbf{56.1 / 32.8}$^{\ddagger}$ & \textbf{60.2}$^{\ddagger}$ & \textbf{40.3}$^{\ddagger}$ & 17.9  \\
\texttt{gpt-3.5-turbo} (TT) & 64.3 & 67.2 & 83.5 & 93.8 & - & - & 46.3 / 27.0 & - & - & -  \\
\texttt{text-davinci-003} & 59.27 & 67.08 & 82.5 & 74.7 & 40.5 / 28.0 & 49.7 / 38.3 & 44.0 / 28.8 & - & - & -\\
\texttt{text-davinci-003} (TT) & 67.0 & 68.5 & \underline{\textbf{83.8}}$^{\ddagger}$ & \underline{\textbf{94.8}}$^{\ddagger}$ & - & - & 54.9 / 34.6 & - & - & - \\
\texttt{gpt-4-32k} & \textbf{75.4}$^{\ddagger}$ & - & - & - & 68.3 / 46.6 & 71.5 / 50.9 & - & - & - & \textbf{19.3}$^{\ddagger}$ \\
\bottomrule
\end{tabular}}
\caption{Average performance across languages in each of the different tasks that we study for our benchmarking exercise. TT suffix refers to the translate-test prompting strategy discussed in Section \ref{sec:prompt_strategies}, without any suffix we refer to the monolingual strategy by default (except for XQuAD and IndicQA where it refers to cross-lingual setup). Numbers in \textbf{bold} with $\dagger$ symbol indicate best performing Fine-tuned model and the ones with $\ddagger$ refer to the best prompt-based generative model. The best overall numbers are \underline{underlined}. For BLOOMZ the values in parenthesis indicate that the model was fine-tuned on the task during multi-task training \cite{muennighoff2022crosslingual}}
...
% Accuracy & F1-Score \\
\midrule
MuRIL & \textbf{72.4} & \textbf{47.7} \\ 

\texttt{text-davinci-003} & 49.6 & 8.45\\

\texttt{text-davinci-003} (TT) & 62.4 & - \\

\texttt{gpt-3.5-turbo} & 50.7  & 38.6 \\
\texttt{gpt-3.5-turbo} (TT) & 59.7 & - \\
\midrule
\end{tabular}}
\caption{Comparing performance of \texttt{text-davinci-003} and \texttt{gpt-3.5-turbo} with fine-tuned baseline MuRIL \cite{khanuja2021muril} on Indic datasets \cite{doddapaneni2022indicxtreme}. For IndicXNLI we report Accuracy and F1-score for IndicQA.}
\label{tab:indic_results}
...
In this section, we discuss the results across different tasks and languages included in our benchmarking exercise. We start by providing an aggregate overview of the results and discussing the general trends that we observe. We then move to a more detailed discussion, going over each dataset and comparing the performance of different models, and prompting strategies for languages included in that dataset.
...
\noindent \textbf{Comparing different prompting strategies}: As discussed in \textsection \ref{sec:prompt_strategies}, we evaluate OpenAI models (only DV003 and GPT-3.5-Turbo) with monolingual, translate-test, and zero-shot cross-lingual prompting strategies. In figure \ref{fig:prompting_comp}, we compare the performance of the three strategies. We find that translate-test often improves the performance over the monolingual strategy, especially so in the case of DV003. We also find that datasets including many low resource and non-latin script languages like IndicXNLI (Table \ref{tab:indic_results}) and XStoryCloze (Table \ref{tab:agg_results_summary}), the gains with translate-test are even more substantial for both the models. Zero-shot cross-lingual for DV003 often performs on par with monolingual but for GPT-3.5-Turbo, there is a drop in performance, especially so for tasks like XCOPA which have some extremely low resource languages: Quechua and Haitian Creole. For these languages, we observed that when provided few-shot examples in English, GPT-3.5-Turbo would often resort to predicting outputs like \textit{"I'm sorry, but the premise is not in a language that I understand."}. However, by providing examples in the language (monolingual prompting), we are able to ground the model to understand these languages and we almost never observe such predictions in that case.
...
\begin{figure*}
\includegraphics[width=0.95\textwidth]{figures/prompting_strategies.pdf}
\caption{Comparing different prompting strategies discussed in \textsection \ref{sec:prompt_strategies} for evaluating DV003 and GPT-3.5-Turbo}
...
\begin{figure*}
    ...
    \caption{Comparing performance of different models on the classification datasets.}
\end{figure*}
...
\begin{figure*}
    ...
    \label{fig:indicqav2}
    \end{subfigure}\\
    \caption{Comparing performance of different models on the Question Answering datasets.}
    % \caption{Language distribution of pre-training data for different LLMs. For GPT-3 we report the percentage of the total words of a language in the pre-training corpus\footnote{\url{https://github.com/openai/gpt-3/blob/master/dataset_statistics/languages_by_word_count.csv}}, disk-size for BLOOM, and number of tokens for PALM. We label top-6 represented languages for each model.}
\end{figure*}
...
\begin{figure*}
    ...
    \label{fig:winomtv2}
    \end{subfigure}\\
    \caption{Comparing performance of different models on the RAI datasets.}
    % \caption{Language distribution of pre-training data for different LLMs. For GPT-3 we report the percentage of the total words of a language in the pre-training corpus\footnote{\url{https://github.com/openai/gpt-3/blob/master/dataset_statistics/languages_by_word_count.csv}}, disk-size for BLOOM, and number of tokens for PALM. We label top-6 represented languages for each model.}
\end{figure*}
