...
\begin{document}
	...
	In [12], Li considered a binary quadratic Titchmarsh divisor function. He proved that
	...
	Let $s$ be a $y$-smooth number, $T(x,y)=\sum_{s\le x}\tau(s-1)$.
	 In [6], Fouvry and Tenenbaum proved that whenever $x^{\eta\log_{3}x/\log_{2}x}\le y\le x$, $\eta$, $A>0$,
	$$T(x,y)=T_{0}(x,y)\log x+T_{1}(x)+O\left(\frac{x}{\left(\log x\right)^{A}}\right),$$
	where $T_{0}(x,y)$ and $T_{1}(x,y)$ are defined in [6, Th$\mathrm{\acute{e}}$or$\mathrm{\grave{e}}$m 1]. And when $x\ge3$, $(\log x)^{2}\le y\le x$, $u\in\mathcal{C}_{k}(\frac{\log y}{2})$, one has
	$$T_{0}(x,y)\log x+T_{1}(x)=x\log x\left\{\sum_{j=0}^{k}\frac{\sigma_{j}(u)}{(\log y)^{j}}+O\left(\frac{\rho^{j+1}(u)}{(\log y)^{j+1}}\right)\right\},$$
	where $\rho(u)$ is the Dickman function, and $\mathcal{C}_{k}(\frac{\log y}{2})$ and $\sigma_{j}(u)$ are defined in [6, Th$\mathrm{\acute{e}}$or$\mathrm{\grave{e}}$m 2].
...
\begin{proof}
	See [1, Lemma 2.8].
\end{proof}
By Hardy and Wright[7, Theorem 67 and 272], one has
...
\end{proof}
\begin{remark}
	In fact, [8, Lemma 2] implies, in particular, that when $(\log x)^{K}\le y< x^{\frac{1}{K}}$ one has
	\begin{eqnarray*}
...
\begin{proof}
	...
	\end{eqnarray*}
	Hence by Mertens' three estimates[11, Theorem 3.4] we can show that
	\begin{eqnarray*}
...
\end{eqnarray}
By [16, Part II, Theorem 5.7] and [16, Part II, Corollary 5.14], we have $|\rho'\left(\frac{\log x}{2\log y}\right)|\ll\rho\left(\frac{\log x}{2\log y}\right)\log\log x$ and $|\rho^{(k)}(u)|\ll\rho(u)(\log u)^{k}$, then $f'(t)\ll\rho\left(\frac{\log x}{2\log y}\right)$. Hence the contribution of the second part to (4.15) does not exceed
...
\begin{thebibliography}{99}
	...
	\bibitem{F-G}E. Fouvry, G. Tenenbaum. Diviseurs de Titchmarsh des entiers sans grand facteur premier. Analytic number theory (Tokyo, 1988), 86–102, Lecture Notes in Math., 1434, Springer, Berlin, 1990. DOI 10.1007/BFb0097126
