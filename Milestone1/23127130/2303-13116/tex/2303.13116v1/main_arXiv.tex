...
\newcommand\zr{{z^{\mathrm{R}}}}
\newcommand\zi{{z^{\mathrm{I}}}}
...
\newcommand{\Bvert}[1]{\Biggl\vert #1 \Biggr\vert}
...
\begin{figure}[b]
    \centering
    \includegraphics[width=\hsize,clip]{fig_illustration3_edit.pdf}
    ...
    (c) Our main result is a thermodynamic relation between $\asymm$ and cyclic affinity, Eq.~\eqref{eq:main}. 
    (d) The result is derived in part by considering $\asymm$ as the ratio between the area and perimeter of closed loops swept out by $a$ and $b$ over time, and then using the isoperimetric inequality.  
    }
\label{fig_illustration}
\end{figure}


One of the most common ways to characterize a physical system is by studying its spatiotemporal correlations.  Imagine measuring a pair of observables $a(t)$ and $b(t)$ in a stochastic system in steady state, e.g., two interacting degrees of freedom, counts of two chemical species, fluorescence intensity of two colors, voltages of two points, etc.~(Fig.~\ref{fig_illustration}a). The two-time correlation between $a$ and $b$ at time lag $\tau$ is 
\begin{align}
...
In particular, in systems without odd degrees of freedom,  cross-correlations in equilibrium  obey the symmetry $ C_{ba}^{\tau}= C_{ab}^{\tau}$ for all $a$, $b$, and $\tau$. This is called the principle of \textit{microscopic reversibility}, and it serves as the basis of the celebrated Onsager reciprocity~\cite{OnsagerReciprocalI1931,CasimirOnOnsagers1945}.
Violation of this symmetry is a fundamental and often-used statistical signature of nonequilibrium steady states~\cite{steinberg1986time,rothberg2001testing,QianFluorescenceCorrelation2004,JachensAsymmetryOfTemporalCrossCorrelations2006,PaneniTemporalAsymmetry2008,choi2008testing,crisanti2012nonequilibrium,battleBrokenDetailedBalance2016,ghantaFluctuationLoops2017,BrogioliAsymmetricTimeCrossCorrelation2019,muraNonequilibriumScaling2018}.


To maintain a nonequilibrium steady state, a system must be subjected to thermodynamic driving. In discrete-state stochastic systems as considered here, the driving strength can be quantified by the \textit{cycle affinity}~\cite{schnakenberg1976network,seifert2012stochastic}. Given a cyclic sequence of states $c$, the cycle affinity $\aff_c$ is defined so that each turn of the cycle increases the thermodynamic entropy of the environment by $\kB\aff_c$ ($\kB$ is the Boltzmann constant). Cycle affinity is determined by macroscopic thermodynamic driving (e.g., temperature gradient, chemical potential gradient, or an external force) and, in principle, it is interpretable and tunable at the macroscopic level. 
It vanishes in equilibrium, and it is a fundamental thermodynamic signature of nonequilibrium steady states.  Until now, however, these thermodynamic and statistical signatures of nonequilibrium steady states have not been directly related.
...
\begin{align}
    ...
    \label{eq:asymm_def}
\end{align}
$\asymm$ is dimensionless and invariant under shifting/scaling of observables and time. 
The normalization terms $ \Delta_\tau C_{aa} \coloneqq C_{aa}^0 - C_{aa}^\tau$ and $ \Delta_\tau C_{bb} \coloneqq C_{bb}^0 - C_{bb}^\tau$  quantify the decay of autocorrelations of $a$ and $b$ (Fig.~\ref{fig_illustration}b). They may also be interpreted as a measure of diffusion (mean-squared displacements), since $\Delta_\tau C_{aa} = \frac{1}{2}\ave{ [a(t+\tau) - a(t)]^2 }\ge 0$.
...
\begin{align}
    ...
    \label{eq:main}
\end{align}
where the maximum is over all cycles, and $n_c$ is the number of states in cycle $c$ (see Fig.~\ref{fig_illustration}c). 
The second bound, which refers to cycle affinity per radian, is approached in the limit $n_c\to \infty$ at a fixed cycle affinity.  
Some extensions and tighter versions of Eq.~\eqref{eq:main} are provided below. 


Our result is physically meaningful and experimentally accessible. It provides a fundamental bound on the asymmetry of cross-correlations achievable at a given level of affinity. This suggests the existence of thermodynamic tradeoffs for various functions that can be quantified by such asymmetry,
including directed interactions and information flow~\cite{NolteIdentifyingInteractions2006,SisanEventOrdering2010,BorysovCrossCorrelationAsymmetries2014,kullmann2002time,HaufeCriticalAssessmentOfConnectivity2013,QinBiochemicalNetworkControlling2018}, nonequilibrium oscillations and  circulation~\cite{tomitaIrreversibleCirculation1974,QianConcentrationFluctuations2002,ghantaFluctuationLoops2017,gonzalezExperimentalMetrics2019,yasudaNonreciprocality2021,teitsworthStochasticLineIntegrals2022}, nonreciprocal motion~\cite{golestanian2008analytic,golestanian2009stochastic,tarama2018mechanics}, and anomalous response (such as odd viscosity)~\cite{evansSymmetryAnalysis1989,EpsteinTimeReversal2020,HargusTimeReversal2020,YasudaTimeCorrelation2022}.


The normalized asymmetry $\asymm$ is also experimentally accessible, since it depends only on the short-time two-time correlation functions. In fact, $\asymm$ can be expressed in terms of the slopes of auto- and cross-correlation functions, since $\asymm= \partial_\tau (C_{ba}^{\tau}-C_{ab}^{\tau})/2 \!\sqrt{ (\partial_\tau C_{aa}^\tau )( \partial_\tau C_{bb}^\tau)}$, where the derivative $\partial_\tau \equiv\partial/\partial \tau$ is evaluated at $\tau = 0$ (Fig.~\ref{fig_illustration}b). 
...
\begin{align}
    \label{eq:CbaDef}
    C_{ba}^\tau = \sum_{i,j}(e^{\tau\R })_{ij} \st_j b_i a_j \approx  \sum_{i}\st_i a_i b_i + \tau \sum_{i,j} \flux_{ij} b_i a_j ,
\end{align}
...
Plugging into \eqref{eq:asymm_def} and simplifying allows us to express $\asymm$ as 
\begin{align}
...
A cycle $c=(i_{1}\to i_{2}\to\dots\to i_{n_{c}}\to i_{1})$ is a closed path of  $n_{c}\ge 3$ distinct states which are connected, 
$R_{i_{k+1}i_k} > 0$ for $k \in \{ 1, \dots, n_c\}$ (we use the convention $n_c+1 \equiv 1$). 
...
With these definitions, the main result~\eqref{eq:main} holds. This bound can be saturated, even arbitrarily far from equilibrium. For a unicyclic system consisting of a single cycle $c = (1 \to 2 \to \cdots \to n \to 1)$, the tighter bound holds with equality 
if the one-way fluxes are translation invariant, i.e., $\flux_{12}=\flux_{23} =\dots = \flux_{n1}$ and $\flux_{21}=\flux_{32} = \dots = \flux_{1n}$,
and there exists a scaling constant $\gamma $ such that the points $(\gamma a_1, \gamma^{-1} b_1), \dots, (\gamma a_n,\gamma^{-1} b_n)$ form a regular $n$-polygon on the $a$--$b$ plane in this order.



\paragraph{Tighter bounds}
We can obtain tighter versions of our main result in two ways. 
First, we may write
\begin{equation}
    \vert \asymm\vert \leq \max_{c} \frac{n_c\tanh(\aff_{c}/2n_{c})}{n'_c \tan(\pi/n'_{c})} \leq \max_{c}  \frac{\aff_{c}}{2\pi}  ,
    \label{eq:main_tighter}
\end{equation}
where the choice of $n'_c$ depends on the situation. The bound~\eqref{eq:main_tighter} always holds with  $n'_c = n_c$, in which case it reduces to~\eqref{eq:main}, but it is tighter whenever $n_c'<n_c$. 
In particular, we can set $n'_c$ to be the number of times the joint value $(a_i, b_i)$ changes over course of cycle $c$, which is sometimes smaller than $n_c$. Furthermore, if the two observables are bipartite, i.e.,  $a$ and $b$ do not change simultaneously in any transition, we can always set $n'_c = 4$ regardless of $n_c$.

Second, we can further tighten the bound~\eqref{eq:main_tighter} by narrowing the set of cycles over which we maximize the affinity.
In fact, it always suffices to consider the cycles generated by the so-called ``uniform cycle decomposition''~\cite{PietzonkaAffinity2016}. This restriction not only makes the inequality tighter, it  also provides a fast algorithm for solving the maximization over cycles. In addition, we can also exclude any cycle $c = (i_1 \to \cdots \to i_{n_c} \to i_1)$ with $\sum_k (b_{i_{k+1}} a_{i_k} - b_{i_k} a_{i_{k+1}}) = 0$. Such a cycle does not have a net asymmetry and cannot contribute to asymmetry of cross-correlations, no matter how large $\aff_c$ is.

The second bound in Eqs.~\eqref{eq:main} and \eqref{eq:main_tighter} is derived by combining $\tanh(\aff_c/2n_c)\le \aff_c/2n_c$, which is tight in the local equilibrium limit $\aff_c/n_c\to 0$, and $\tan(2\pi/2{n_c'})\ge 2\pi/2{n_c'}$, which is tight in the large cycle limit $n_c\to \infty$. Separately, these inequalities gives two intermediate bounds,
\begin{align}
     \vert \asymm \vert  \leq  \max_c \frac{\aff_c/2n_c'}{\tan(\pi/n_{c}')},\quad
     \vert \asymm \vert  \leq \max_c \frac{\tanh(\aff_{c}/2n_{c})}{\pi/n_c }.
    \label{eq:main_simpler}
\end{align}
...
but the oscillation should be coherent in time for reliable biological functionality~\cite{BarkaiCircadianClocks2000,GaspardCorrelationTime2002}.
Coherence is supported at the cost of dissipation, thus the relation between thermodynamic cost and the coherence of noisy oscillations is an active area of  research~\cite{CaoFreeEnergy2015,baratoCoherence2017,NguyenPhaseTransition2018,OberreiterSubharmonic2019,delJunco2020High,delJunco2020Robust,OberreiterUniversalMinimalCost2022}.



The coherence of oscillation is quantified in terms of the number of oscillations that occur before the steady-state auto-correlations die down. 
To introduce this, let  $\lambda_{1},\dots,\lambda_{n}$ be the eigenvalues of the rate matrix $\R$ with real and imaginary parts $\lambda_{\alpha}=-\lr_{\alpha}+\imag\li_{\alpha}$. Suppose for simplicity that the eigenvalues are non-degenerate, in which case the matrix exponential can be expressed as $e^{\tau\R } = \sum_a \exp(-\lr_\alpha \tau) \exp(\imag \li_\alpha \tau ) \revecA  {\levecA}{}^{\mathsf T}$, where $\levecA$ and $\revecA$ indicate the left and right eigenvectors of $\R$ normalized so that $ \levecA {}^{\mathsf T} \revecA = 1$. Plugging this expression into Eq.~\eqref{eq:CbaDef} shows that any two-time correlations can be written as a sum over the eigenmodes of $\R$, where the contribution from mode $\alpha$ decays with timescale $(\lr_\alpha)^{-1}$ and oscillates with period $2\pi\vert \li_\alpha\vert^{-1}$. 
The number of coherent oscillations for mode $\alpha$ is the ratio between the decay time and the period of the oscillations,  $(\lr_\alpha)^{-1}/2\pi|\li_\alpha|^{-1}=|\li_\alpha|/2\pi\lr_\alpha$~\cite{QianPumped2000,baratoCoherence2017}.


Barato and Seifert~\cite{baratoCoherence2017} conjectured that the slowest decay mode (with the smallest nonzero $\lr_\alpha$) obeys a thermodynamic bound on the number of coherent oscillations:
\begin{equation}
    \frac{\left| \li_\alpha \right|}{2\pi\lr_\alpha}
    \leq\max_{c}\frac{\tanh(\aff_{c}/2n_{c})}{2\pi \tan(\pi/n_{c})} .
    \label{eq:Barato_Seifert}
\end{equation}
This bound implies that long-lasting and coherent oscillations require strong thermodynamic driving. Despite its fundamental and profound nature, this inequality was only supported by numerical evidence. 
In general, there have been few analytical results concerning the relationship between thermodynamics and eigenvalues~\cite{HodasQualityOfOscillations2010,delJunco2020High,delJunco2020Robust}, although eigenvalues provide a fundamental quantification of a system's timescales. 
...
$a_i = \operatorname{Re} u^{(\alpha)}_i/\st_i$ and  $b_i = \operatorname{Im} u^{(\alpha)}_i/\st_i$. 
...
\label{eq:lrli}
\end{align}
To our knowledge, this expression of eigenvalues in terms of two-point correlations is new to the literature. 
Combining Eqs.~\eqref{eq:lrli} and \eqref{eq:main}, together with the  inequality $(x+y)/2 \geq \sqrt{xy}$, 
proves the conjecture~\eqref{eq:Barato_Seifert}. 
The oscillation bound is saturated in unicyclic systems with uniform transition rates. 


The relation \eqref{eq:Barato_Seifert} may also be used to infer cyclic affinity from empirical observations, assuming some two-time correlation function exhibits a clear damped oscillation corresponding to a particular mode. In this case, one would not directly measure the ``observables'' $a$ and $b$, but rather estimate $\li_\alpha$ and $\lr_\alpha$  by fitting the two-time correlation function with a damped oscillation.



\paragraph{Application 2:~Signal transduction in a biochemical  system} 
One of the goals of stochastic thermodynamics is to understand the costs of information processing in biochemical systems~\cite{MehtaEnergeticCosts2012,barato2013information,ItoInformationThermodynamics2013,govern2014energy,govern2014optimal,barato2014efficiency,sartori2014thermodynamic,ten2016fundamental,ito2015maxwell,mehta2016landauer}. To illustrate a practical application of our result, we derive a thermodynamic bound on directed information flow in a standard model of biochemical signal transduction (Fig.~\ref{fig_application}a)~\cite{MehtaEnergeticCosts2012}. In the model, there is an upstream receptor and a downstream protein. The upstream receptor stochastically switches between ``OFF'' and ``ON'' states due to ligand binding, corresponding to the observable $a=0,1$. The downstream stochastically switches between ``0'' (inactive) and ``1'' (active) states, corresponding the observable $b=0,1$. When the upstream is ``ON'', the activation of the downstream ($0\to 1$) is driven by a chemical force $\Delta\mu > 0$. For example, if the driving is provided by the hydrolysis of a molecule of ATP, $\Delta\mu = \mu_\mathrm{ATP} - \mu_\mathrm{ADP} - \mu_\mathrm{Pi}$ with $\mu_\mathrm{X}$ being the environmental chemical potential of $\mathrm{X}$. The bipartite dynamics are modeled as a four-state master equation depicted in Fig.~\ref{fig_application}(b). The unique cycle in this system has cycle affinity $\aff = \Delta \mu /\kB T$, where $T$ is the environmental temperature. 

In this model, the cross-correlation $C_{ba}^\tau$ is the joint probability of the receptor being ``ON'' and the protein being ``1'' time $\tau$ later, and vice versa for $C_{ab}^\tau$.
The asymmetry $C_{ba}^\tau- C_{ab}^\tau$ provides a simple and natural measure of directed information flow from $a$ to  $b$~\cite{NolteIdentifyingInteractions2006,SisanEventOrdering2010,BorysovCrossCorrelationAsymmetries2014}. 
As for the normalization factor $\!\sqrt{(\Delta_\tau C_{aa})(\Delta_\tau C_{bb})}$, Eq.~\eqref{eq:dCaaDef} implies that $\Delta_\tau C_{aa}$ is the expected number of switching events of $a$ during a short period $\tau$, and similarly for  $\Delta_\tau C_{bb}$. 
Therefore, $\asymm$ essentially quantifies the asymmetry per switch of $a$ or $b$. 
...
\begin{figure}[t]
    ...
    (b) Formulation as a stochastic master equation with a nonequilibrium cycle. 
    ...
    (d) The ratio between $\chi_{ba}$ and the tightest bound $\tanh(\Delta\mu/8\kB T)$ in Eq.~\eqref{eq:receptor}. }
    \label{fig_application}
\end{figure}

Our general results, Eqs.~\eqref{eq:main_tighter} and \eqref{eq:main_simpler},  specialize to
\begin{equation}
    \vert \asymm\vert  \leq \tanh \frac{\Delta\mu}{8 \kB T} \le \frac{\Delta\mu}{ 8 \kB T} \le \frac{\Delta\mu}{ 2\pi \kB T}.
    \label{eq:receptor}
\end{equation}
Thus, the chemical force $\Delta\mu$  bounds directed information flow, irrespective on the details of the transition rates. Even if the transition rates are perturbed, the bound is not affected as long as  $\Delta\mu$ is unchanged. 


Although Eq.~\eqref{eq:receptor} was motivated by a specific model of signal transduction~\cite{PietzonkaAffinity2016}, the result is much more general. The bound $\vert \asymm\vert  \leq {\Delta\mu}/{ 2\pi \kB T}$ applies to any signal transduction system with a binary upstream and downstream, including multi-cyclic systems and systems with non-observed transition and states, 
as long as there is only one driven state transition with chemical force $\Delta \mu$.
The tighter bound $\vert \asymm\vert \leq {\Delta\mu}/{ 8 \kB T}$ holds when, in addition, the upstream and downstream observables are bipartite (do not change at the same time), as follows from Eq.~\eqref{eq:main_simpler}.
...
Our result is similar in spirit to TURs, which also relate the statistical and thermodynamic properties of nonequilibrium steady states~\cite{Barato2015thermodynamic,GingrichInferringDissipation2017,SeifertFromStochasticThermodynamics2019,horowitz2020thermodynamic}. However, the two approaches differ  both in statistical and thermodynamic aspects. First, our bound is defined in terms of
two-time correlations of 
state observables (e.g., counts of chemical species, voltages, etc.), while TURs are usually defined in terms of
the mean and variance of 
anti-symmetric current observables (e.g., chemical reaction fluxes, electric currents, etc.).  
Although the asymmetry $C_{ba}^\tau-C_{ab}^\tau$ can be interpreted as the mean of a specific  anti-symmetric current observable, the variance of this current observable lacks an intuitive physical or statistical interpretation.    
Second, our bound uses the cycle affinity as the measure of thermodynamic cost, while TURs use the entropy production rate. The affinity is determined from the macroscopic forces and is robust against microscopic perturbations, while the entropy production rate captures the resulting dissipation rate and is sensitive to microscopic details. Therefore, these two measures provide different and complementary characterizations of the thermodynamic cost.
To our knowledge, there is no way to use TURs to bound cycle affinities in general (multi-cyclic) networks
~\footnote{Inequalities on cycle affinities in terms of cycle-based statistics~\cite{PolettiniTightUncertaintyRelation2022} and waiting-time statistics~\cite{VanDerMeerThermodynamicInference2022} have recently been proposed. However, these inequalities cannot be used to study the asymmetry of cross-correlations.}. 
\nocite{PolettiniTightUncertaintyRelation2022,VanDerMeerThermodynamicInference2022}

In future work, it would be interesting to generalize our approach to cases when cross-correlations between three or more observables are available at once. It is also interesting to investigate whether similar bounds hold in the large-$\tau$ regime. Finally, future work may extend our analysis to continuous-state systems and quantum systems.



\paragraph{Derivation}
We sketch the derivation of Eq.~\eqref{eq:main}. We  restrict our attention to unicyclic systems for simplicity. Further details, including derivation of our tighter bound~\eqref{eq:main_tighter}  and consideration of multi-cyclic networks, are in the Supplemental Material (SM)~\cite{SM}.
...
For each transition $j\to i$, we define the net probability current $\cur_{ij}= \flux_{ij} -  \flux_{ji}$ and the dynamical activity (frequency of back-and-forth transitions) $\act_{ij} =  \flux_{ij} + \flux_{ji}$. We also define $\Omega_{ij} \coloneqq \frac 12 (b_i a_j  - b_j a_i ) $  and  $L_{ij} \coloneqq \sqrt{ (a_i-a_j)^2 + (b_i - b_j)^2 }$. 


We recast $\chi_{ba}$ in a convenient form. By assumption, the system consists of a single cycle, $c=(1\to 2\to \dots \to n \to 1)$ with $n=n_c$, and its reversal. The steady state condition $\R\bm q = 0$ implies that the current is uniform, $\cur_{21} = \cur_{32} = \dots = \cur_{1n} \eqqcolon \cur $. We assume without loss of generality that $\cur \ge 0$  (otherwise we may consider the cycle in reverse). 
Next, since $\vert \asymm \vert$ is invariant under multiplication of $a$ and $b$ by any pair of real numbers,  we may assume without loss of generality that $a$ and $b$ are scaled to satisfy
$ \sum_{i,j} \flux_{ij} a_ia_j = \sum_{i,j} \flux_{ij} b_ib_j $. Using this assumption combined with Eq.~\eqref{eq:dCaaDef}, we may rewrite the denominator of Eq.~\eqref{eq:asymmExplicit} as $- \sum_{i,j} \flux_{ij} ( a_i a_j + b_i b_j ) = \frac 12 \sum_{i>j} \act_{ij} L_{ij}^2 $. Combining leads to  
\begin{align}
...
\label{eq:genTUR}
\end{equation}
To prove Eq.~\eqref{eq:genTUR}, we first use the Cauchy--Schwarz inequality to show that the left-hand side is less than $\cur^2 \sum_i \act_{i+1,i}^{\,-1}$. Next, we rewrite the affinity as $\aff = 2n \sum_i n^{\,-1} \artanh (\cur/\act_{i+1,i})$ and apply the Jensen's inequality to $\artanh$ to show that $ \cur \sum_i (\cur/\act_{i+1,i})$ is less than the right-hand side.
...
$\sum_i L_{i+1,i} $. We  relate the area and length using the isoperimetric inequality~\cite{OssermanIsoperimetricInequality1978},
\begin{align}
    \qty( 4n\tan\frac{\pi}{n} ) \, \Bvert {\sum_i  \Omega_{i+1,i} } \leq \Bparen{ \sum_i L_{i+1,i} } ^{2}. \label{eq:isoperimetric}
\end{align}
As we show in the SM, this inequality holds even if the curve has self-intersections. Combining Eqs.~\eqref{eq:gg}--\eqref{eq:isoperimetric} leads to our main result. 
...
We thank Kohei Yoshimura for discussions. S.~I.~is supported by JSPS KAKENHI Grants No.~19H05796, No.~21H01560, and No.~22H01141, and UTEC-UTokyo FSI Research Grant Program.
...
\bibitem [{\citenamefont {Qian}\ and\ \citenamefont
  ...
  }\bibfield  {title} {\bibinfo {title} {Pumped biochemical reactions,
  nonequilibrium circulation, and stochastic resonance},\ }\href
  {https://doi.org/10.1103/PhysRevLett.84.2271} {\bibfield  {journal} {\bibinfo
   {journal} {Phys. Rev. Lett.}\ }\textbf {\bibinfo {volume} {84}},\ \bibinfo
  {pages} {2271} (\bibinfo {year} {2000})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Hodas}()}]{HodasQualityOfOscillations2010}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {N.~O.}\ \bibnamefont
  {Hodas}},\ }\bibfield  {title} {\bibinfo {title} {The quality of oscillations
  in overdamped networks},\ }\href@noop {} {\bibinfo  {journal}
  {\href{https://arxiv.org/abs/1006.0271}{arXiv preprint arXiv:1006.0271
...
\bibfield  {journal} {  }\bibfield  {author} {\bibinfo {author} {\bibfnamefont
  {P.}~\bibnamefont {Mehta}}\ and\ \bibinfo {author} {\bibfnamefont {D.~J.}\
  \bibnamefont {Schwab}},\ }\bibfield  {title} {\bibinfo {title} {Energetic
  costs of cellular computation},\ }\href
  {https://doi.org/10.1073/pnas.1207814109} {\bibfield  {journal} {\bibinfo
  {journal} {Proc. Natl. Acad. Sci. USA}\ }\textbf {\bibinfo {volume} {109}},\
  \bibinfo {pages} {17978} (\bibinfo {year} {2012})}\BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Ito}\ and\ \citenamefont
  ...
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {S.}~\bibnamefont
  {Ito}}\ and\ \bibinfo {author} {\bibfnamefont {T.}~\bibnamefont {Sagawa}},\
  }\bibfield  {title} {\bibinfo {title} {Maxwell’s demon in biochemical
  signal transduction with feedback loop},\ }\href
...
\section{Derivation of our result}

We prove our main result for general multicyclic systems with incorporating all generalizations discussed below Eq.~\eqref{eq:main_tighter}. We use the symbols $\cur_{ij}$, $\act_{ij}$, $\Omega_{ij}$, and $L_{ij}$ defined in the \textit{Derivation} section in the main text. 


\subsection{Rescaling of the observables}
\label{sec_rescaling}

The first step of the proof is to show that it suffices to prove a slightly different inequality for any observables $a$ and $b$:
\begin{equation}
    \left| \phi_{ba} \right|
    \leq \max_{c} \frac{n_c\tanh(\aff_{c}/2n_{c})}{n'_c \tan(\pi/n'_{c})},
    \label{sm:target}
\end{equation}
with
\begin{equation}
    \phi_{ba} = \frac{\sum_{i,j} \flux_{ij} (b_i a_j - b_j a_i)}{- \sum_{i,j } \flux_{ij} ( a_i a_j  + b_i b_j) }.
\end{equation}
The right-hand side of Eq.~\eqref{sm:target} is interpreted with incorporating all extensions discussed below Eq.~\eqref{eq:main_tighter}. Namely, $n'_c$ may be smaller than $n_c$, and the range of maximization is restricted as discussed there.

\paragraph{Proposition}
Assuming that Eq.~\eqref{sm:target} holds for any $a$ and $b$, Eq.~\eqref{eq:main_tighter} holds for any $a$ and $b$.

\paragraph{Proof}
For a given pair of observables $a$ and $b$, we introduce a coefficient
\begin{equation}
    \delta = \sqrt[{\scriptstyle 4}]{\frac{ \sum_{i,j} \flux_{ij} b_i b_j}{ \sum_{i,j} \flux_{ij} a_i a_j}},
    \label{sm:coefficient}
\end{equation}
and new observables $a' = \delta a$ and $b' = \delta^{-1} b$. 
Due to these definitions, $a'$ and $b'$ satisfy
\begin{align}
    &- \sum_{i,j} \flux_{ij} a'_i a'_j = - \sum_{i,j} \flux_{ij} b'_i b'_j
    \notag \\
    & \qquad = \!\! \sqrt{- \sum_{i,j} \flux_{ij} a_i a_j } \sqrt{- \sum_{i,j} \flux_{ij} b_i b_j }.
\end{align}
Using this relation, we can recast the numerator and denominator of $\chi_{ba}$ in Eq.~\eqref{eq:asymmExplicit} in terms of $a'$ and $b'$ as
\begin{equation}
    \chi_{ba} = \frac{\sum_{i,j} \flux_{ij} (b'_i a'_j - b'_j a'_i )}{ - \sum_{i,j} \flux_{ij}  (a'_i  a'_j  + b'_i b'_j)   }
    = \phi_{b'a'}.
\end{equation}
Since we assume that Eq.~\eqref{sm:target} holds for any pair of observables, it holds for $a'$ and $b'$. Therefore, $|\chi_{ba}| = |\phi_{b'a'}|$ is upper-bounded by the right-hand side of Eq.~\eqref{sm:target}. This proves Eq.~\eqref{eq:main_tighter} for the given $a$ and $b$. 

Since above argument holds for any pair of observables $a$ and $b$, Eq.~\eqref{eq:main_tighter} holds for any $a$ and $b$. \qed 

Note that we have implicitly excluded the case of $\sum_{i,j} \flux_{ij} a_i a_j = 0 $ or $\sum_{i,j} \flux_{ij} b_i b_j = 0$. When $\sum_{i,j} \flux_{ij} a_i a_j = 0 $ holds,  Eq.~\eqref{eq:dCaaDef} implies $a_i = a_j$ for any pairs of $i, j$ with nonzero flux $\flux_{ij}\neq 0$. This is equivalent to that $a_i$ is constant over all the states with a nonzero steady-state probability. Such cases produce no asymmetry, $C^\tau_{ba} - C^\tau_{ab} = 0 $, and is not of interest. 

Below, we prove Eq.~\eqref{sm:target} for any observables $a$, $b$ and for general multicyclic systems.
We can rewrite $\phi_{ba}$ using Eq.~\eqref{eq:dCaaDef} as
\begin{align}
    \phi_{ba} 
    ...
    \label{sm:phi_convenient}
\end{align}
which is the most convenient for the proof below.

\subsection{Notation}

We introduce notation suitable for analyzing multicyclic systems. We use symbol $e=(i\to j)$ to denote the transition from $i$ to $j$, which we call a (directed) edge after the terminology in graph theory. For an edge $e$, we use the symbol $-e$ to denote the transition in the opposite direction $-e=(j \to i)$. We regard a cycle $c=(i_{1}\to i_{2}\to\dots\to i_{n_{c}}\to i_{1})$ as a sequence of edges $(e_1, e_2,\dots, e_{n_c})$ with $e_k = (i_{k} \to i_{k+1})$. For a cycle $c$ and an edge $e$, let $S_{ec}$ be $1$ if $e$ is in $c$, $-1$ if $-e$ is in $c$, and 0 otherwise.

For an edge $e=(j \to i)$, we use $\flux_e \equiv \flux_{ij} = R_{ij} q_j $ and $\flux_{-e} \equiv \flux_{ji} = R_{ji} q_i$ for the one-way fluxes, and we use the symbols $\cur_e \equiv \cur_{ij} = \flux_e - \flux_{-e}$, $\act_e \equiv \act_{ij} = \flux_e + \flux_{-e}$,
$\Omega_e \equiv \Omega_{ij}$, and $L_e \equiv L_{ij}$.
We define the set of edges with a positive current, $\mathcal{E}^{+} \coloneqq \Set{e|\cur_e>0}$. Below, $\sum_{e}$ always denotes the sum over $e\in\mathcal{E}^{+}$.



\subsection{Uniform cycle decomposition}
\label{sec_uniform_decomp}
...
\end{equation}
where $\Cuni$ is a subset of cycles, with the conditions (i) all the edges of every $c\in\Cuni$ belong to $\mathcal{E}^{+}$, and (ii) the cycle currents are positive, $\cur_{c}>0$. The condition (i) implies that $S_{ec}$ is either 0 or 1 for $e\in\mathcal{E}^{+}$ and $c\in\Cuni$. It also implies $\aff_c > 0$ for $c\in\Cuni$ because we can rewrite the cycle affinity as $\aff_c = \sum_e S_{ec} \ln(\flux_e / \flux_{-e})$, and the sign of $\ln(\flux_e / \flux_{-e})$ is the same as the sign of $\flux_e - \flux_{-e} = \cur_e > 0$. Although the choice of $\Cuni$ and $\cur_c$ is not unique in Eq.~\eqref{sm:uniform_decomp}, below we fix one out of them.


Reference \cite{PietzonkaAffinity2016} discusses an algorithm to obtain such a special cycle decomposition. The algorithm basically repeats (i) finding a edge with the minimum remaining edge current and pick a cycle that passes through the edge and is aligned with the edge currents and (ii) assigning the maximum possible cycle current to the cycle, until all the edge currents are assigned to cycle currents. 
 
Using a uniform cycle decomposition, we can rewrite the numerator of Eq.~\eqref{sm:phi_convenient} as
\begin{align}
    \sum_{i>j} \cur_{ij} \Omega_{ij} 
    &= \sum_{c\in \Cuni} \cur_c \sum_{e} S_{ec} \Omega_e 
    \notag \\
    &= \sum_{c\in \CS} \cur_{c} \sum_{e} S_{ec} \Omega_e
    \label{sm:numerator} 
\end{align}
where 
\begin{equation}
    \CS \coloneqq \Set{ c \in \Cuni | \sum_e S_{ce} \Omega_e \neq 0 } 
    \label{eq:cs-def}
\end{equation}
is the subset of $\Cuni$ with the cycles with nonzero net asymmetry. Similarly, the denominator of Eq.~\eqref{sm:phi_convenient} is rewritten as
\begin{align}
    \sum_{i>j}\act_{ij}L_{ij}^2
    & \geq\sum_{e}\act_{e} L^2_{e}
    \notag \\ 
    & = \sum_{e}\cur_{e}\frac{\act_{e}}{\cur_{e}} L^2_{e} 
    \notag \\
    & = \sum_{c\in \Cuni} \cur_{c}\sum_{e}S_{ec}\frac{\act_{e}}{\cur_{e}} L^2_{e},
    \notag \\
    & \geq \sum_{c\in \CS} \cur_{c} \sum_{e}S_{ec}\frac{\act_{e}}{\cur_{e}} L^2_{e},
    \label{sm:denomenator}
\end{align}
where we have dropped the edges with $ \cur_{ij} = 0 $ in the first inequality (recall that $\sum_e $ indicates the sum over $e \in \mathcal E^+$), and further narrowed the range of the sum in the last inequality. In the following, we develop two tools to evaluate these cyclewise quantities.


\subsection{Generalized TUR}


The first tool is a generalized TUR for multicyclic case. This generalized TUR itself does not admit clear physical interpretation, but it reduces to Eq.~\eqref{eq:genTUR} for unicyclic systems.

\paragraph{Lemma}
Let $d_{e}$ be any anti-symmetric current observable, i.e., quantity associated with the edges that satisfies $d_e = - d_{-e}$.  Then, for any cycle $c \in \Cuni$ from a uniform cycle decomposition, 
\begin{equation}
    \frac{\left(\sum_{e}S_{ec}d_e\right)^{2} }{\sum_{e}S_{ec} d_e^2 {\act_{e}}/{\cur_{e}}}
    \leq
    n_c \tanh \qty( \frac{\aff_c}{2n_c} ).
    \label{sm:genTUR_multicyclic}
\end{equation}
Equality holds if and only if $\cur_e / \act_e$ and $d_e$ are each uniform across the cycle. 

\paragraph{Proof}
We use the Cauchy--Schwarz inequality between
vectors $\qty( S_{ec} d_{e} \!\sqrt{ {\act_{e}} / {\cur_{e}} } ) {}_{e \in \mathcal E^+}$ and $\qty( S_{ec}\!\sqrt{{\cur_{e}}/{\act_{e}} } ) {}_{e \in \mathcal E^+}  $ to obtain
\begin{equation}
    \frac{\left(\sum_{e}S_{ec}d_e\right)^{2} }{\sum_{e}S_{ec} d_e^2 {\act_{e}}/{\cur_{e}}}
    \leq
    \sum_{e}S_{ec}\frac{\cur_{e}}{\act_{e}},
    \label{sm:CauchySchwarz}
\end{equation}
where we have used that $(S_{ec})^2 = S_{ec}$ because $S_{ec}$ is 0 or 1 and $\cur_e > 0$ for $e \in \mathcal E^+$.
To relate the right-hand side with the cycle affinity, we rewrite the cycle affinity~\eqref{eq:cycle_affinity} as
\begin{align}
    \aff_{c} 
    & = \sum_{e}S_{ec}\ln\frac{\flux_e}{\flux_{-e}} 
    \notag \\
    & = \sum_{e}S_{ec}\ln\frac{\act_{e}+\cur_{e}}{\act_{e}-\cur_{e}}
    \notag \\
    & =2n_{c}\sum_{e}\frac{1}{n_{c}} S_{ec} \artanh \qty( \frac{\cur_{e}}{\act_{e}} ).
\end{align}
Since $\cur_{e} / \act_e \geq 0$ and $\sum_e S_{ec} / n_c = 1$, we can apply Jensen's inequality to the $\artanh$ function in the last line to give
\begin{equation}
  \aff_c 
  \geq 2n_{c} \artanh \qty(\sum_{e}\frac{1}{n_{c}}S_{ec}\frac{\cur_{e}}{\act_{e}}).
\end{equation}
Inverting this inequality gives 
\begin{equation}
  \frac{1}{n_{c}} \sum_e S_{ec}\frac{\cur_{e}}{\act_{e}}
  \leq  \tanh \qty( \frac{\aff_c}{2n_{c}} ).
\end{equation}
Combining with Eq.~\eqref{sm:CauchySchwarz} gives the desired result. 

The equality condition for the Cauchy--Schwarz inequality is that the vector $\qty(S_{ec} d_e \sqrt{\act_e / \cur_e}) {}_{e\in\mathcal E^+}$ is proportional to $\qty( S_{ec} \sqrt{\cur_e / \act_e} ) {}_{e\in\mathcal E^+}$, or equivalently, $d_e \act_e / \cur_e$ is uniform along the cycle. The equality condition for the Jensen's inequality is that $\cur_e / \act_e$ is uniform across the cycle. Rearranging these two conditions gives the equality condition in the statement. \qed


Note that, when the cycle contain a edge with absolute irreversibility, i.e., $R_{ij} > 0 $ and $R_{ji}=0$, the affinity is $\aff_c = \infty$. Eq.~\eqref{sm:genTUR_multicyclic} still holds formally for such cases. 
...
Next, we prove the other element of the proof, the isoperimetric inequality for polygons. This is a purely mathematical theorem that does not rely on the context. Although isoperimetric inequality is a known result, here we need to generalize the inequality for possibly self-intersecting polygons, which has not been explicitly stated in literature.
...
\paragraph{Proof}
Let $s_i $ indicate the line segment connecting $(a_i , b_i )$ and $(a_{i+1}, b_{i+1})$ in the $a$--$b$ plane. Consider the polygon $P$ formed by the sequence of edges $(s_{1},\dots,s_{n})$, which may be non-simple, i.e., self-intersecting. The length of the perimeter of $P$ is given by $L(P) \coloneqq \sum_i L_{i+1,i}  $, and the signed area of a polygon $P$ is defined as $\Omega (P) \coloneqq \sum_i \Omega_{i+1,i} $ \citep{radoIsoperimetricInequalityLebesgue1947}.
The signed area has been shown to be equivalent to \citep[section 6.4,][]{radoLemmaTopologicalIndex1936}
\begin{equation}
    \Omega(P)=\iint_{\mathbb R^2} w_{P}(a,b)\,dadb,
    \label{sm:winding_number}
\end{equation}
where $w_{P}(a,b)$ is the winding number of the curve $P$ at $(a,b)$, i.e., the number of times the curve $P$ circulates around the point $(a,b)$ in the clockwise direction subtracted by that in the counterclockwise circulation. 

We now define another polygon $Q$, which is called the ``convexification'' of $P$ \citep{scottFamilyPolygons1982,faryIsoperimetryVariableMetric1982}. $Q$ is constructed from the same set of line segments as edges, $(s_{1},\dots,s_{n})$, but the edges are translated and permuted so that their angle monotonically increases in a counter-clockwise fashion. Formally, $Q$ is formed by translating and connecting the sequence of edges $(s_{\sigma(1)},\dots,s_{\sigma(n)})$, where $\sigma$ is a permutation that guarantees that the angle of the line segment $(a_{\sigma(i)}, b_{\sigma(i)})$--$(a_{\sigma(i+1)}, b_{\sigma(i+1)})$ relative to the $a$ axis is monotonically increasing in $i$.
...
\begin{equation}
    ...
    \label{sm:iso_Q}
\end{equation}
Since $Q$ and $P$ have the same number of edges of the same length, $L(Q)=L(P)$. As for the signed area, it has been proved that $\Omega(P) \leq \Omega(Q)$, using the expression for the signed area in Eq.~\eqref{sm:winding_number} \citep[Lemma 1,][]{faryIsoperimetryVariableMetric1982}. Plugging these relations into Eq.~\eqref{sm:iso_Q} gives 
\begin{equation}
    4n\tan(\pi/n) \, \Omega (P)
    \leq L(P)^{2}.
    \label{sm:iso_proof1}
...
We note that the statement is purely algebraic, although the proof relies on geometric concepts. 

We use the similar technique to prove another Lemma, which will be used for bipartite cases:
...
\paragraph{Proof}
We define the edges $s_i$ as in the previous proof, and let $P$ be the polygon formed by the edges $(s_{1},\dots,s_{n})$. Due to the assumption, the direction of the edges are either $0$, $\pi/2$, $\pi$, or $3\pi/2$ relative to the $a$ axis.
...
Note that Eq.~\eqref{sm:isoperimetric_bipartite} is formally obtained by inserting $n=4$ in Eq.~\eqref{sm:isoperimetric}. 


\subsection{Proof of Eq.~\eqref{sm:target}}
\label{sec_final}

Finally, we combine these tools to finish the proof. Here we prove the fully generalized version Eq.~\eqref{eq:main_tighter}, where $n'_c$ may be different from $n_c$, and the range of maximization in the right hand side is narrowed down to cycles with nonzero net asymmetry.


\paragraph{Proof of Eq.~\eqref{sm:target}}
First, we combine Eq.~\eqref{sm:numerator} with the isoperimetric inequality~\eqref{sm:isoperimetric} applied to the sequence of points $(a_{i_1}, b_{i_1}), \dots, (a_{i_{n_c}}, b_{i_{n_c}})$ for each cycle $c=(i_1 \to \dots \to i_{n_c} \to i_1 ) $:
\begin{align}
    \Bvert{\sum_{i>j} \cur_{ij}\Omega_{ij}}
    &= \sum_{c\in \CS} \cur_{c} \, \Bvert{\sum_{e}S_{ec}\Omega_{e} }
    \notag \\
    &\leq \sum_{c\in \CS} \cur_c \, \Bparen{ \sum_e S_{ec} L_e }^2 \qty( 4n'_c \tan\frac{\pi}{n'_c} ) ^{-1},
    \label{sm:final_numerator}
\end{align}
where we used that $\cur_{c} \ge 0$. 
This bound always holds with $n'_c = n_c$, but in some situations we can choose a smaller $n'_c$. In particular, if some neighboring states on the cycle has the same value of $(a,b)$, we can choose $n'_c$ to be the number of times the joint value $(a,b)$ changes along the cycle by using the isoperimetric inequality for the sequence of $n'_c$ points. If $a$ and $b$ are bipartite, i.e., do not jump simultaneously, we can choose $n'_c=4$ in view of Eq.~\eqref{sm:isoperimetric_bipartite}. 


Next, we combine Eq.~\eqref{sm:denomenator} with the generalized TUR~\eqref{sm:genTUR_multicyclic} applied to $d_e = -d_{-e} = L_e$ for $e\in\mathcal E^+$ to rewrite
\begin{align}
    \sum_{i>j}\act_{ij}L^2_{ij}
    &\geq
    \sum_{c\in \CS} \cur_{c}\sum_{e}S_{ec} L^2_{e} \frac{\act_{e}} { \cur_{e} }
    \notag \\
    &\geq
    \sum_{c\in \CS} \cur_c \, \Bparen{ \sum_e S_{ec} L_e }^2 \qty ( n_c \tanh \frac{\aff_c}{2n_c} )^{-1}.
    \label{sm:final_denominator}
\end{align}

Finally, we plug Eqs.~\eqref{sm:final_numerator} and \eqref{sm:final_denominator} into the expression of $\phi_{ba}$ in Eq.~\eqref{sm:phi_convenient} and use the inequality
\begin{equation}
...
\begin{align}
    | \phi_{ba} |
    &\leq \max_{c\in \CS} \frac{4  \cur_c (\sum_e S_{ec} L_e)^2 [4n'_c \tan(\pi / n'_c)]^{-1}}
    { \cur_c (\sum_e S_{ec} L_e)^2 [n_c \tanh(\aff_c/2n_c)]^{-1}} 
    \notag \\
    &= \max_{c\in \CS} \frac{n_c\tanh(\aff_c/2n_c)}{n'_c\tan(\pi/n'_c)},
    \label{sm:final_proof}
\end{align}
which proves the desired inequality in Eq.~\eqref{sm:target}.
\qed


In view of Eq.~\eqref{sm:final_proof}, as discussed in the section \emph{Tighter bounds} in the main text, our main result holds when the range of maximization of $c$ is restricted to $\CS$, as defined in Equation~\eqref{eq:cs-def}.  
...
Here we discuss the equality condition of our main result, Eq.~\eqref{eq:main}, for unicyclic systems. We omit the discussion for multicyclic systems.

\paragraph{Proposition}
For unicyclic systems, the first (tighter) inequality in our main result~\eqref{eq:main} is saturated if and only if (i) $\act_e$ is uniform across the cycle, and (ii) there exist a constant $\gamma$ such that the points $(\gamma a_1, \gamma^{-1} b_1), \allowbreak \dots, (\gamma a_n, \gamma^{-1} b_n)$ form a regular $n$-polygon in this order. 

\paragraph{Proof} 
We first find the equality condition of the bound on $\phi_{ba}$ in Eq.~\eqref{sm:target}. Equation~\eqref{sm:target} for unicyclic systems is simply the combination of the generalized TUR and the isoperimetric inequality. By combining their equality conditions and noting that the current $\cur_e$ is always uniform for unicyclic systems, one can easily find that the equality of Eq.~\eqref{sm:target} holds if and only if $\act_e$ is uniform across the cycle, and $\{(a_i, b_i)\}$ form a regular $n$-polygon in this order. 

As discussed in Sec.~\ref{sec_rescaling}, the main result~\eqref{eq:main} is obtained by applying Eq.~\eqref{sm:target} to the rescaled variables $a' = \delta a$ and $b' = \delta^{-1} b$. Therefore, the equality of the main result~\eqref{eq:main} for unicyclic system is that (i) $\act_e$ is uniform along the cycle, and (ii') $\{ (\delta a_i , \delta^{-1} b_i)\}$ form a regular $n$-polygon in this order. We are left with showing that the combination of (i) and (ii') is equivalent to the combination of (i) and (ii) in the statement. When (i) and (ii') holds, (ii) immediately holds with the choice $\gamma = \delta$. Conversely, suppose that (i) and (ii) are satisfied. Then, the coefficient $\delta$ in Eq.~\eqref{sm:coefficient} is calculated as
\begin{align}
    \delta &= \sqrt[{\scriptstyle 4}]{ \frac{ \sum_{i} \act_{i+1,i} (b_i - b_{i+1})^2 }{\sum_{i} \act_{i+1,i} (a_i - a_{i+1})^2 }  }
    \notag \\
    &= \gamma \sqrt[{\scriptstyle 4}]{ \frac{\sum_{i} (\gamma^{-1} b_i - \gamma^{-1}  b_{i+1})^2 }{ \sum_{i} (\gamma a_i - \gamma a_{i+1})^2 }  } 
    \notag \\
    &= \gamma,
    \label{sm:delta_gamma}
\end{align}
where the first equality is due to Eq.~\eqref{eq:dCaaDef}, the second equality is due to the condition (i), and the last equality is easily confirmed by expressing $\gamma a_i = r \cos( i\cdot 2\pi/n + \theta_0 )$ and $\gamma^{-1} b_i = r \sin( i\cdot 2\pi/n + \theta_0)$ for any $r$ and $\theta_0$. Equation~\eqref{sm:delta_gamma} combined with (ii) implies (ii').
\qed
