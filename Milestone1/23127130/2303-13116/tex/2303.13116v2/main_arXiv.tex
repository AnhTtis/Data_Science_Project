...
\begin{figure}[b]
    ...
    (d) The result is derived in part by considering $\asymm$ as the ratio between the area and perimeter of
    the polygon formed by the values $(a,b)$ over the cycle
    and then using the isoperimetric inequality.  
    }
...
To maintain a nonequilibrium steady state, a system must be subjected to thermodynamic driving, such as temperature gradient, chemical potential gradient, and an external force. In discrete-state stochastic systems as considered here, the strength of different kinds of driving can be quantified in a unified way by the \textit{cycle affinity}~\cite{schnakenberg1976network,seifert2012stochastic}. 
...
but the oscillation should be coherent in time for reliable biological functionality~\cite{BarkaiCircadianClocks2000,GaspardCorrelationTime2002}. Coherence is supported at the cost of dissipation, thus the relation between thermodynamic cost and the coherence of noisy oscillations is an active area of  research~\cite{CaoFreeEnergy2015,baratoCoherence2017,NguyenPhaseTransition2018,OberreiterSubharmonic2019,delJunco2020High,delJunco2020Robust,RemleinCoherenceOfOscillations2022,UhlAffinity2019,OberreiterUniversalMinimalCost2022,kolchinsky2023thermodynamicSpectral}.
...
% In general, there have been relatively few analytical results concerning the relationship between thermodynamics and eigenvalues~\cite{delJunco2020High,delJunco2020Robust,RemleinCoherenceOfOscillations2022,kolchinsky2023thermodynamicSpectral}, although eigenvalues provide a fundamental quantification of a system's oscillations timescales. 
...
To our knowledge, our expression of eigenvalues in terms of two-point correlations is new to the literature. More generally, our analytical approach may prove useful for further study of the relationship between thermodynamics and  eigenvalues of the rate matrix.
%~\cite{UhlAffinity2019,baratoCoherence2017,OberreiterUniversalMinimalCost2022,OberreiterSubharmonic2019,delJunco2020High,delJunco2020Robust,RemleinCoherenceOfOscillations2022,kolchinsky2023thermodynamicSpectral}. 

The result~\eqref{eq:Barato_Seifert} also has practical implications, as it may be 
%The relation \eqref{eq:Barato_Seifert}  may also be 
used to infer cyclic affinity from empirical observations, assuming some two-time correlation function exhibits a clear damped oscillation corresponding to a particular mode. In this case, one would not directly measure the ``observables'' $a$ and $b$, but rather estimate $\li_\alpha$ and $\lr_\alpha$  by fitting the two-time correlation function with a damped oscillation.
...
Our general result~\eqref{eq:main_tighter} specialize to
\begin{equation}
...
%It is interesting to consider whether our bound~\eqref{eq:main} holds for finite time lags, not only the short-time limit $\lim_{\tau\to0}$ in Eq.~\eqref{eq:asymm_def}. Given numerical evidence presented in the SM, we conjecture that our bound holds for all $\tau$. A proof of this conjecture is an interesting direction for future work.
% Finally, numerics shown in SM suggest that our bound holds for any finite $\tau$ without taking the limit $\lim_{\tau\to0}$ in Eq.~\eqref{eq:asymm_def}. We leave exploration of this finite-$\tau$ bound for future work. 
...
We recast $\chi_{ba}$ in a convenient form. By assumption, the system consists of a single cycle, $c=(1\,{\to}\, 2 \,{\to}\, \cdots \,{\to}\, n \,{\to}\, 1)$, and its reversal. The steady state condition $\R\bm q = 0$ implies that currents are uniform, $\cur_{21} = \cur_{32} = \dots = \cur_{1n} \eqqcolon \cur $. We assume without loss of generality that $\cur \ge 0$ (otherwise we may consider the cycle in reverse). 
...
In Sec.~\ref{sec:additional}, we make supplemental discussions, such as implicit assumptions made in the main text and the equality conditions. 
...
\end{equation}
where $\Cuni$ is a subset of cycles, with the conditions that (i) all the edges of every $c\in\Cuni$ belong to $\Eplus$, and (ii) the cycle currents are positive, $\cur_{c}>0$. 
...
\paragraph{Corollary}
For a sequence of $n$ points $(a_1, b_1), \allowbreak \dots,\allowbreak (a_n, b_n)$ on $\mathbb R^2$, let $n'$ be the number of times the joint value $(a,b)$ changes over the sequence. Formally, $n'$ is the number of labels $i\in\{1,\dots, n\}$ such that $(a_i, b_i) \neq (a_{i+1}, b_{i+1})$, where we use the convention $n+1 \equiv 1$. Then,
\begin{equation}
    \qty(4n' \tan \frac{\pi}{n'} )\,
    \Bvert{ \sum_{i=1}^n \Omega_{i+1,i} }
    \leq \Bparen{ \sum_{i=1}^n  L_{i+1,i} } ^2 \,.
