...
\begin{abstract}
We prove Michael-Simon type Sobolev inequalities for $n$-dimensional submanifolds in $(n+m)$-dimensional Riemannian manifolds with nonnegative $n$-th intermediate Ricci curvature by using the Alexandrov-Bakelman-Pucci method.
These inequalities extends Theorem 1.4 in Brendle's work \cite{Brendle22} and Theorem 1.5 in Dong-Lin-Lu's work \cite{DLL22} to the $n$-Ricci curvature setting. In particular, a simple application of these inequalities gives rise to some isoperimetric inequalities for minimal submanifolds in Riemannian manifolds.
\end{abstract}
...
Our main result is the following theorem which extends Theorem 1.4 in \cite{Brendle22} to the $n$-Ricci setting.
\begin{thm}\label{thm1}
Let $M$ be a complete noncompact $(n+m)$-dimensional Riemannian manifold with nonnegative $n$-Ricci curvature. Let $\Sigma$ be a compact $n$-dimensional  submanifold of $M$ (possibly with boundary $\partial\Sigma$), and let $f$ be a positive smooth function on $\Sigma$. If $m\ge 2$, then
\begin{equation}
...
\begin{cor}
Let $M$ be a complete noncompact $(n+m)$-dimensional Riemannian manifold with nonnegative $n$-Ricci curvature. Let $\Sigma$ be a compact minimal $n$-dimensional submanifold of $M$ with boundary $\partial\Sigma$. If $m\ge 2$, then
\begin{equation}
...
This paper is organized as follows. Section \ref{sec2} contains some basic concepts. In Section \ref{sec3}, we deal with the $n$-Ricci curvature and prove Theorem \ref{thm1}. Using similar argument, we prove a Michael-Simon type Sobolev inequality (Theorem \ref{thm2})
for manifolds with asymptotic nonnegative $n$-Ricci curvature in Section \ref{sec4}.
...
A manifold $M$ is said to have nonnegative $k$-Ricci curvature if $Ric_k(X,V)\ge 0$ for any unit tangent vector $X\in T M$ and $k$-dimensional subspace $V$ such that $X\perp V$. It is worth noting that $Ric_1$ is just sectional curvature and $Ric_{n-1}$ is just Ricci curvature.
...
\begin{equation}\label{keyestimate}
    ...
    \end{aligned}
\end{equation}
By assumption, $M$ has nonnegative $n$-Ricci curvature, so both $\sum_i S_{ii}(t)$ and $\sum_A S_{AA}(t)$ are nonnegative for all $t\in(0,r)$. By Lemma \ref{lemma1}, we have
\begin{equation}
...
\section{Manifolds with asymptotically nonnegative n-Ricci curvature}\label{sec4}

As a generalization of Theorem \ref{thm1}, we now extend Theorem 1.5 in \cite{DLL22} to the case that the ambient manifold has asymptotically nonnegative $n$-Ricci curvature.
\begin{thm}\label{thm2}
Let $M$ be a complete noncompact $n+m$ dimensional Riemannian manifold of asymptotically nonnegative $n$-Ricci curvature with respect to a base point $o$ in $M$. Let $\Sigma$ be a compact $n$-dimensional submanifold of $M$ (possibly with boundary $\partial\Sigma$), and let $f$ be a positive smooth function on $\Sigma$. If $m\ge 2$, then
\begin{equation}
\begin{aligned}
...
By assumption, $M$ has asymptotically nonnegative $n$-Ricci curvature with respect to a base point $o$ in $M$, that is, $\sum_i S_{ii}(t)\ge -n \lambda(d(o,\bar \gamma(t)))$. Furthermore, we have
...
They would like to thank Professor Chao Qian for his helpful suggestions.
\end{ack}
%----------------------------------------------
