...
\begin{document}
	...
	\begin{theorem}\label{thm:QH}[B\'ar\'any, Katchalski and Pach \cite{BKP82}]
	...
	\begin{theorem}\label{thm:QCH}[Damásdi, Földvári and Naszódi \cite{damasdi2021colorful}]
	...
	\begin{theorem}\label{thm:QFH}[Jung and Naszódi \cite{jung2022quantitative}]
	...
	%This Lemma alone is enough to prove Theorem~\ref{thm:stabHelly}.
	...
	\section{Radon-type Hypergraph Chains}\label{sec:radon}
	
	Radon used the following lemma to prove Helly's Theorem.
	
	\begin{lemma}
		If $X \subset \Red$ with $|X| \geq d+2$, then there exists a partition $X = X^+ \cup X^-$ with $\conv(X^+) \cap \conv(X^-) \neq \emptyset$.
	\end{lemma}
	
	To translate this property into a property of the hypergraph $\Kd$, we need to identify each point $x \in \Red$ with the family $\C_x$ of all the convex sets $C \in \cvx$ containing $x$. Note that $x = \cap_{C \in \C_x} C$ for all $x \in \Red$. We can identify $\conv(X)$ with $\C_X = \cap_{x \in X} \C_x$ for a subset $X \subset \Red$, since $\conv(X) = \cap_{C \in \C_X}C$. Finally, we have $\conv(X) \cap \conv(Y) = \cap_{C \in \C_X \cup \C_Y}C$ for two subsets $X, Y \subset \Red$, thus intersections of convex hulls can be described as unions of families of convex sets. We arrive at the following definition.
	
	\begin{definition}
		A hypergraph $\Hc$ is Radon-type with Radon number $r$ if for every $H_1, \ldots, H_r \in \Hc$ there exists a partition $I \cup J = [r]$ with $\cap_{i \in I} H_i \cup \cap_{j \in J}H_j \in \Hc$.
	\end{definition}
	
	Radon's Lemma states that $\Kd$ has Radon number $d+2$. A hypergraph chain analog can be the following.
	
	\begin{definition}
		Radon-type hypergraph chains
	\end{definition}
	
	A result of Holmsen and Lee implies a bound of the Colorful Helly Number of hypergraphs as a function of their Radon Numbers. We show that their result extends to hypergraph chains as follows.
