...
\author{Li Luo}
\affiliation{Interdisciplinary Center of Quantum Information, State Key Laboratory of Modern Optical Instrumentation, and Zhejiang Province Key Laboratory of Quantum Technology and Device, Department of Physics, Zhejiang University, Hangzhou 310027, China}
...
We encode the gauge-transformed spin configurations $\sigma_{j}^{'k}$ on a single phase-only SLM with wavelength-division multiplexing. For the $k$-th Mattis model, we assume a uniform spectrum intensity illuminating on SLM and apply a phase modulation on the $y$-directional pixels for each spin as
\begin{equation}
