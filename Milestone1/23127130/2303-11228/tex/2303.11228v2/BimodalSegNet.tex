...
%object segmentation is relevant to robotic grasping, as they aim to extract the object  boundaries at pixel level. Since visual information collected by cameras in the industrial environment may be affected by low light conditions, motion blur, object size variance, occlusion, etc \cite{Bader2018ChallengesManufacturing}, object segmentation methods should address such challenges. 
...
\begin{enumerate}
    % \item We propose a novel encoder-decoder-based architecture named Bimodal SegNet. This model integrates the cross-attention mechanism from the transformer architecture into the conventional U-Net structure, accommodating signals from multiple modalities. This helps to improve segmentation results and directly addresses the identified limitations of conventional vision systems.

   % \item In our Bimodal SegNet, we employ two separate encoders to manage multimodal input. These encoders fuse features at multiple resolutions using a cross-attention mechanism. Additionally, they demonstrate the utilization of weighted features with Atrous spatial pyramidal pooling within the context of a Bimodal encoder-decoder-based architecture. This approach facilitates more robust and adaptable processing of visual information under diverse conditions.

   % \item On the ESD dataset, we have created a rigorous experimental method, where our Bimodal SegNet is validated. This dataset, providing a comprehensive set of scenarios encountered in real-world industrial settings, stands as an important resource for future research in the field.
...
%The Dynamic and Active-pixel Vision Sensor (DAVIS) camera generates both asynchronous events and RGB frame-based signals with consistent spatial dimensions. %This ensures that when merging the RGB frames with the event frames, the resultant composite feature map maintain the same perspective. 
...
%The event stream generated from a dynamic vision sensor (DVS) is characterized by a position $x_e$ and $y_e$, timestamp $t_e$ and polarity $p_e$ where e is the counter of events. The polarity is binary and thus either 0 for negative or 1 for positive polarity. 
...
%\begin{equation}
%    U_{RGB} = F_{RGB}^{inter} \times softmax\left(\frac{G_{event}}{\sqrt{d_k}}\right)
%\end{equation}

%\begin{equation}
%    U_{event} = F_{event}^{inter} \times softmax\left(\frac{G_{RGB}}{\sqrt{d_k}}\right)
%\end{equation}
...
%, where the head count aligns with the transformer backbone. 
...
%To capture image-level context, an additional branch employing global average pooling (denoted as $\text{GlobalAvgPool}$) is incorporated into the ASPP module. This branch processes the weighted feature map $X_{\text{combined}}$:

%\begin{equation}
%X_{global} = {GlobalAvgPool}(X_{combined})
%\end{equation}

%\begin{equation}
%F_{new_{combined}} = {Concat}(F_1, F_2, ..., F_N, F_{global})
%\end{equation}

%\begin{equation}
%F_{out} = {Conv}(F_{new_{combined}})
%\end{equation}
...
%where we consider the task of instance segmentation of the objects in the scene by utilizing the inputs in form of event frames and RGB frames using. 
...
%The RGB and event frame resolution is 346$\times$300, which differs from the expected input of the network (296$\times$296). Therefore, the popular approach of patchifying the input images is employed and the final  segmentation map is reconstructed by unpatching the outputs.
...
%Evaluation metrics, ablation study, and results are discussed below. and models in the ablation study in subsection \ref{subsection : Ablation_Study}. 
...
\begin{table}[t]
  ...
  \specialrule{.1em}{.1em}{.1em} 
        Method & 2 Obj & 4 Obj & 6 Obj & 8 Obj & 10 Obj   \\  %& \begin{tabular}{c} Overlapped \\ Events \\ (\% of FP)\end{tabular}
  \specialrule{.1em}{.1em}{.1em}   
  ...
  \specialrule{.1em}{.1em}{.1em} 
        Method & Bright Light & Low light &  &  &    \\  %& \begin{tabular}{c} Overlapped \\ Events \\ (\% of FP)\end{tabular}
    \specialrule{.1em}{.1em}{.1em}
  ...
  \specialrule{.1em}{.1em}{.1em} 
        Method & Linear & Rotational & Partial Rotational &  &    \\  %& \begin{tabular}{c} Overlapped \\ Events \\ (\% of FP)\end{tabular}
\specialrule{.1em}{.1em}{.1em}
  ...
  \specialrule{.1em}{.1em}{.1em} 
        Method & 0.1 m/s & 0.15 m/s &   0.3 m/s &  &    \\  %& \begin{tabular}{c} Overlapped \\ Events \\ (\% of FP)\end{tabular}
  \specialrule{.1em}{.1em}{.1em}   
  ...
  \specialrule{.1em}{.1em}{.1em} 
        Method & 62 cm & 82 cm &    &   &    \\  %& \begin{tabular}{c} Overlapped \\ Events \\ (\% of FP)\end{tabular}
  \specialrule{.1em}{.1em}{.1em}   
...
\begin{table}[t]
  ...
  \specialrule{.15em}{.1em}{.1em}  
        \multicolumn{6}{c}{Exp 1: \textbf{varying clutter objects}, Bright light, 62cm height,  Rotational motion, 0.15 m/s speed } \\
  \specialrule{.1em}{.1em}{.1em} 
        Method & 2 Obj & 5 Obj &  &  &    \\  %& \begin{tabular}{c} Overlapped \\ Events \\ (\% of FP)\end{tabular}
  \specialrule{.1em}{.1em}{.1em}   
