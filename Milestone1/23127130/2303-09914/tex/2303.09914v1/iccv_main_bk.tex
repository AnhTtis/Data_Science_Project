...
%%%%%%%%% ABSTRACT
\begin{abstract}
Face Anti-Spoofing (FAS) is recently formulated as a continual learning task as the FAS models are expected to evolve after encountering the data from new domains. However, existing methods require using extra replay buffers to store previous data for rehearsal, which becomes infeasible when previous data is unavailable because of privacy issues. In this paper, we propose the first rehearsal-free method for Domain Continual Learning of FAS, which deals with catastrophic forgetting and unseen domain generalization problems simultaneously. The model architecture is based on Vision Transformer (ViT). For better generalization to unseen domains, we design the Dynamic Central Difference Convolutional Adapter (DCDCA) to adapt ViT during the continual learning of the FAS model. To alleviate the forgetting of previous domains without using rehearsal data, we propose the Proxy Prototype Contrastive Regularization (PPCR) to constrain the continual learning with previous domain knowledge provided by proxy prototypes. To simulate practical continual learning scenarios, we devise two new protocols which evaluate the performance of FAS models regarding both generalization and anti-forgetting. Extensive experimental results show that our proposed method helps in improving the generalization performance in unseen domains and alleviates the catastrophic forgetting of the previous domains during continual learning. The codes and protocols will be released soon.
\end{abstract}
...
\vspace{-0.3em}
Face recognition has been widely used in identity authentication because of its convenience. However, face recognition-based authentication systems are threatened by face spoofing attacks \cite{ramachandra2017presentation, kong2022digital, yu2022deep}. To protect face recognition systems from spoofing attacks, Face Anti-Spoofing (FAS) techniques are deployed to detect spoofing faces and reject malicious attempts. Although recent FAS methods based on deep learning and neural networks achieve exquisite accuracy in intra-domain testing, the performance of existing methods heavily relies on the diversity of the training data and degrades severely if there are domain shifts between the training and testing data domains \cite{li2018unsupervised}. The cross-domain problem hence becomes the most pressing open issue of state-of-the-art FAS research.

%Attackers could spoof the systems by presenting forgery faces to the camera sensors. 

\begin{figure}[t]
    \centering
    %\vspace{-0.8em}
    \includegraphics[width=0.47\textwidth]{config.pdf}
    %\vspace{-0.8em}
    \caption{The rehearsal-free DCL consists of a sequence of learning sessions. The FAS model is initially trained on a large-scale base domain and then continuously adapts to new domains in the following continual sessions. For each continual session, only a few data of new domain is available for training and previous data is NOT available. After the DCL, the model is tested on all previous domains and extra unseen domains.}
    \label{fig:my_label}
    %\vspace{-1em}
\end{figure}

To tackle the cross-domain problem, domain generalization~\cite{MADDG-CVPR-2019,SSDG-CVPR-2020} and adaption~\cite{li2018unsupervised,huang2022adaptive} techniques for FAS have been extensively studied in recent years. Domain generalization-based methods aim to develop a generalized FAS model with training data from multiple source domains. Despite improving the generalization to some extent, they are still far from satisfaction in unseen domains. Besides, domain adaptation-based methods utilize a few target domain data for model adaptation. Although the target domain performance can be boosted with the help of more effective training data, the benefit is at the cost of expensive data collection. Moreover, it is even impractical to collect sufficient data at a static point of time since the domain shifts are caused by complex and constantly changing factors like illumination and attack types.

In real-world scenarios, the deployed FAS systems constantly encounter novel data from unseen domains. The novel data will be collected and become available for model training gradually. Completely retraining a model from scratch with old and new data faces both efficiency and privacy issues. Although fine-tuning the base model with the new data only is more efficient, the past knowledge will be overwritten after fine-tuning, and the performance on previous data decreases dramatically, \textit{i.e.} catastrophic forgetting \cite{perez2020learning}. To adapt models efficiently, continual learning methods for FAS have been proposed in recent works \cite{perez2020learning, iccv2021_dcl_fas}. To alleviate the catastrophic forgetting, both methods \cite{perez2020learning, iccv2021_dcl_fas} utilize replay buffers to store previous data for rehearsal while fine-tuning with new data. However, the use of replay buffers causes extra storage burdens. Even worse, the previous data is not always available for storage and transfer since face data contains identity information.

In this work, we formulate the FAS problem as a rehearsal-free Domain Continual Learning (DCL) task. Unlike existing work \cite{iccv2021_dcl_fas} where the FAS model is expected to learn sequentially from data of novel attacks, the aim of our work is enabling the FAS model continuously evolve with the data from constantly varying domains. Due to efficiency and privacy issues, previous data is not allowed to be stored and accessed for rehearsal, which is different from previous works \cite{perez2020learning, iccv2021_dcl_fas}. We first evaluate the baseline method under the DCL setting and obtain some interesting observations from experiments: 1) catastrophic forgetting usually occurs when the new coming data domain deviates significantly from previous ones; 2) \textcolor{red}{a model generalizes better to unseen domains usually forgets less previous domain knowledge}.
...
\par
\textcolor{red}{Motivated by the observations, we start to devise the model architecture for the DCL-FAS. The architecture is based on Transformer (ViT) \cite{dosovitskiy2020image} and Adapter \cite{houlsby2019parameter,huang2022adaptive} for its powerful feature representation ability.} To learn fine-grained features for FAS tasks, we propose a Dynamic Central Difference Convolutional Adapter (DCDCA) to replace the Linear Adapter. The DCDCA introduces image-specific inductive bias to ViT with convolution and adaptively extracts features with central difference information \cite{CDCN-CVPR-2020}. Unlike \cite{CDCN-CVPR-2020} where the ratio of central difference information is fixed for all layers, the ratio in our design is self-adaptive to new data domains, which is more suitable in the continual learning setting. To alleviate the forgetting, we propose to contrastively regularize the optimization of DCDCA with the class centroids of previous tasks. We use the model weights of the fully-connected layers as proxy prototypes to approximate the class centroids of previous tasks without accessing previous data.
%A contrastive regularization is introduced to optimize the DCDCA. 
%
Moreover, we establish two protocols to evaluate the FAS models under the DCL settings. Our contributions include:
\begin{itemize}
    \item We formulate and tackle the FAS problem in a more practical scenario: low-shot and rehearsal-free Domain Continual Learning (DCL). For each continual learning task, only a few new data is available for training and no previous data is accessible. 
    \item We design the Dynamic Central Difference Convolutional Adapter (DCDCA) to efficiently tune ViT-based models in continual domains and capture intrinsic live/spoof cues.
    \item We propose the Proxy Prototype Contrastive Regularization (PPCR) to further alleviate the forgetting of FAS models during rehearsal-free DCL.
    \item We design two practical protocols to evaluate both anti-forgetting and generalization capacities of FAS models under DCL settings, with up to 15 public datasets covering both 2D and 3D attacks. We find that the proposed DCDCA and PPCR can significantly improve generalization while forgetting less over baselines on these two DCL protocols.  
    %, which has better generalization performance and less forgetting than baseline FAS models
    % based on ResNet18 and vanilla ViT with linear adapters.
    %We empirically find that more generalized models have less forgetting problem. 
\end{itemize}
...
We observe from experiments that more generalized architectures usually have less catastrophic forgetting (see Sec.~\ref{sec:Analysis1} and ~\ref{sec:Analysis2}). %
Therefore, to achieve the goals of DCL-FAS: generalize more and forget less, we propose Dynamic Central Difference Convolutional Adapter (DCDCA), which adapts ViT with dynamic central difference information during the continual learning.
...
Specifically, 2D convolutional layers inside the DCDCA are utilized to provide the locality inductive bias. To fit the convolution operation, the 1D flattened image token from the ViT backbone is reshaped back to a 2D structure for processing. Then, the reshaped 2D token is forwarded to a stack of convolutional layers for feature extraction. To learn features for subtle live/spoof discrimination, we use CDC to extract fine-grained contextual info from neighbor visual tokens. The output $y(p_0)$ is defined as 
%\vspace{-0.5em}
...
%\vspace{-0.2em}
To learn more generalized features, supervised contrastive loss \cite{SupCon} is adapted for network optimization. Considering that the distributions of real face samples are relatively similar than spoofing ones \cite{SSDG-CVPR-2020}, all real face samples are regarded as one cluster while spoofing face samples are divided into 2D attacks and 3D mask attacks. Therefore, the loss for optimization is expressed as
%\vspace{-0.2em}
...
% When the privacy issue is strictly concerned, and even previous features are not stored, how to utilize the previous knowledge to  is important.
\par
...
\begin{algorithm}[t]  \caption{Domain Continual Face Anti-Spoofing with PPCR} \label{algo}
        \SetAlgoLined
...
\resizebox{0.50\textwidth}{!}{
    \begin{aligned}
     mAA \triangleq\frac{1}{T+1}\sum_{t=0}^T (\frac{1}{T-t+1} \sum_{i=t}^T R_{t,i})&,  mABT \triangleq\frac{1}{T}\sum_{t=0}^{T-1} (\frac{1}{T-t}\sum_{i=t+1}^{T}R_{i,t}- R_{t,t}),\\
    AGA_u \triangleq \frac{1}{T+1} \sum_{t=0}^T R^u_{t}&, mAGA=\frac{1}{2}(AGA_{1}+AGA_{2}),
    \end{aligned}
    }
%\vspace{0.3em}
\end{equation}
...
%\vspace{-0.3em}
  ...
  \vspace{-0.8em}\label{tab:comparebackbone}%
...
%\vspace{0.3em}
\noindent\textbf{Observation 2:} Generalized architectures tend to forget less. From Fig.~\ref{fig:P1}(l) and Fig.~\ref{fig:P2}(l), we can see the ResNet18 has much lower generalization performance on the ROSE-YOUTU dataset than our ViT-CDCA and ViT-DCDCA. Also, Fig.~\ref{fig:P1}(a)-(k) and Fig.~\ref{fig:P2}(a)-(k) show that ResNet18's performance fluctuates more than our ViT-CDCA and ViT-DCDCA, indicating that ResNet18 often forgets previous knowledge in continual learning. By contrast, the performance curve of our ViT-CDCA and ViT-DCDCA are more stable, meaning our ViT-CDCA and ViT-DCDCA forget less than ResNet18. Therefore, we observe that a more generalized backbone tends to forget less in continual learning. The insight behind this observation is intuitive. Although different FAS datasets have domain gaps between each other, there could be some shared knowledge and information. If a model can extract generalized features across different datasets, when learning on a new dataset, it may learn related cues about previous datasets, and thus it suffers from less catastrophic forgetting. Therefore, designing a generalized architecture is essential for DCL-FAS.
...
\section{Conclusion}
%\vspace{-0.4em}
In this paper, we raise the concern of privacy in continual FAS and formulate the FAS in rehearsal-free Domain Continual Learning settings. We devise more practical protocols for FAS evaluation and experimentally find that more generalized architectures can benefit unseen domain generalization and also alleviate forgetting during continual learning. For better generalization performance, we develop a Dynamic Central Difference Convolutional Adapter to adapt ViT continually. Besides, we propose Proxy Prototype Contrastive Regularization to regularize learning with previous knowledge from previous model weights. Extensive experiments on two protocols demonstrate that the proposed method generalizes better while forgetting less compared with baseline backbones and fine-tuning methods.
