...
\begin{align} \label{MRI_eq1}
    \underset{u_1, \dots, u_E}{\text{minimize}} \,\frac{1}{2}\sum_{j=1}^{C}\sum_{k=1}^{E} ||DFS_j u_k - y_{j,k}||_F^2 + \lambda \cdot {\cal R}(u_1, \dots, u_E),
\end{align}
where $S_j: \mathbb{C}^{N \times T} \to \mathbb{C}^{N \times T}$ is the $j$-th coil sensitivity map, $F: \mathbb{C}^{N \times T} \to \mathbb{C}^{\lfloor M/T \rfloor \times T}$ is the nonuniform Fourier transform, and $D: \mathbb{C}^{\lfloor M/T \rfloor \times T} \to \mathbb{C}^{\lfloor M/T \rfloor \times T}$ is the density compensation factor. $||\cdot||_F$ is the Frobenius norm in complex space and ${\cal R}(u_1, \dots, u_E)$ is the regularization term that can be defined via $\ell_2$ coupling along the echo as follows.
...
\begin{align} \label{MRI_eq3}
    \min_{u_1, \dots, u_E} \max_{\substack{(\xi_1, \dots, \xi_E) \in K \\ \zeta_{1,1}, \dots, \zeta_{C,E}}}  \left( \sum_{j=1}^{C} \sum_{k=1}^E \langle DFS_ju_k - y_{j,k}, \zeta_{j,k} \rangle - \frac{1}{2}||\zeta_{j,k}||_2^2 \right)
...
\begin{align}
    ...
    \zeta_{j,k}^{n+1} &\gets \operatorname{prox}(\zeta_{j,k}^n + \sigma (DFS_j\bar{u}_k^n - y_{j,k})), \label{PDHG_2}\\
    u_k^{n+1} &\gets u_k^n - \tau \left( \sum_{j=1}^C (DFS_j)^H\zeta_{j,k}^n + \partial_t^H\xi_k^n \right), \label{PDHG_3}\\
...
\paragraph{\underline{Remark I.}} The calculation of $(DFS_j)^H\zeta_{j,k}^n$ for $j=1, \dots, C$, which is the most computationally intensive step, can be distributed across multiple GPUs using the Message Passing Interface (MPI) to achieve speedup.
...
The R, G, and B channels can be considered as multi-echo images, where each channel represents the images at TE1, TE2, and TE3, respectively. The intensity profiles of each color channel, or equivalently each echo, can be viewed as representing different motion states. We formulate the following reconstruction problem: Let $m_j: (\Omega_{{\cal F}} \subset \mathbb{R}^2) \to \mathbb{C}$ be the $j$-th channel of the vector-valued function in Fourier space (k-space) $m: (\Omega_{{\cal F}} \subset \mathbb{R}^2) \to \mathbb{C}^3, (k_x, k_y) \mapsto (m_1(k_x,k_y), m_2(k_x,k_y),$ $m_3(k_x,k_y))$. The goal is to reconstruct $u: (\Omega \subset \mathbb{R}^2) \to \mathbb{C}^3, (x, y) \mapsto (u_1(x,y), u_2(x,y), u_3(x,y))$ from the undersampled input Fourier samples $m$ by solving
\begin{align} \label{reconstruction_eq1}
    \underset{u_1, u_2, u_3}{\text{minimize}} \, \frac{1}{2} \sum_{k=1}^{3} \int_{\Omega_{\cal F}} \|Eu_k - m_k\|_2^2 + \lambda \int_\Omega ||\partial_y u||_1,
\end{align}
where $E:\mathbb{C} \to \mathbb{C}$ is the encoding operator (e.g., composition of density compensation and nonuniform Fourier transform). The channels (or echoes) can be coupled with the $\ell_2$ norm as follows.
\begin{align} \label{reconstruction_eq2}
    \int_\Omega ||\partial_y u||_2 = \int_\Omega \left( |\partial_y u_1|^2 + |\partial_y u_2|^2 + |\partial_y u_3|^2 \right)^{1/2}.
\end{align}
