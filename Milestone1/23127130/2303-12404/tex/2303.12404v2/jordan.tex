...
Led by a group of approximately 25 people,
...
\emph{
I thank Javier Fresán for having inviting me to write this paper,
a Spanish version of which appeared in
 \emph{La Gaceta de la Real Sociedad Matemática Española},
Vol. 26 (2023), Núm. 3, Págs. 535–553.
I also thank him, together with Riccardo Brasca, Patrick Massot and Filippo Nuccio,
for their comments, as well as Martin Liebeck and Raphaël Rouquier for their help.
I am grateful to the \emph{mathlib} community for their 
...
for every $x,x'\in X$, there exists $h\in H$ such that $h\cdot x=x'$.
...
Another \texttt{mathlib} idiosyncracy that appears in the definitions
...
containing a point and subgroups containing its stabilizer},float=hbt,label=lst:ordereq,captionpos=t]
...
there are exactly two permutations~$g,g'$ such that $g\cdot x_i=g'\cdot x_i=y_i$ for all~$i$, and $g'g^{-1}$ is the permutation that exchanges
the two elements of~$\{1,\dots,n\}$
not in $\{y_1,\dots,y_{n-2}\}$; in particular, one of them is even
...
the path that led to their proof,
