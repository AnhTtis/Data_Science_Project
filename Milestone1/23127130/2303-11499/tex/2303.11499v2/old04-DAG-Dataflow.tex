\section{Inter-Operation Reuse Patterns}
%\vspace{-1mm}
\label{sec:dataflows}

This section formulates inter-operation reuse opportunities in the dependency graph of tensor operations using a systematic methodology.
%It formulates a generalized set of inter-operation reuse patterns in an arbitrary dependency graph of tensor operations. 
...
%\TK{As I am readinf through this I realize this sectin is still using the old terminology of ``traditional" rather than ``adjacent-op". @Raveesh can u fix?}
...
\textbf{Sequential} - The sequential pattern executes that particular edge one by one, although it maybe possible to reuse the data on-chip with Sequential pattern depending on the space in the buffer (Note that the term 'sequential' applies to one edge connecting two nodes while we previously defined 'op-by-op' as a configuration computes one op at a time for the entire execution). There are generally no constraints on this type of pattern and this is used when no other pattern is possible. Also, this pattern does not give any benefit, and the output of the operation is both written and re-read from on-chip or off-chip depending on the capacity.

\insertFigure{buffer}{Inter-operation reuse patterns in steady state and their data orchestration.\vspace{-2mm}}
...
\textbf{{Adjacent pipeline}} - An adjacent-pipeline reuse pattern refers to pipelining used between the two operations in prior works~\cite{flat,garg2021understanding,tangram,yan2020hygcn}. More than two operations could also be pipelined if more than two consecutive nodes are connected by edges with {adjacent pipeline} patterns.\reviewme{~\autoref{fig:buffer} shows how pipelining reuses the data within SRAM. It discards the tile once its consumed.} For an example pair of Einsums ${Z_{m,n}=\sum_{k}A_{m,k}*B_{k,n}}$ and ${W_{o,n}=\sum_{j}C_{o,j}*Z_{j,n}}$, if the contracted rank in the producer ie. $K$ and the unshared rank in the consumer ie. $O$ are dominant ranks, then pipelining is not beneficial as the producer/consumer needs to wait for several iterations along that dimension. %However, the {pipelineable} pattern does not guarantee pipelining unless additional conditions related to intra-operation dataflow are satisfied which we discuss in~\autoref{sec:gogeta}. 
 Pipelining completely removes the need to write and read the intermediate tensor. However, if the intermediate tensor is needed in the future, there is an additional need to perform {pipeline\_with\_writeback} or {pipeline\_with\_hold}. 


\textbf{{Pipeline\_with\_writeback}} -  Adjacent pipelining completely consumes the intermediate tensor, with the new produced tile replacing the one that has already been consumed. However, the intermediate tensor might be needed in a future computation, a pattern that has not been observed in previously popular applications like Attention, Graph Neural Networks etc. This is represented by the transitive edge (discussed in \autoref{sec:graph}) on the graph. \reviewme{Therefore, as~\autoref{fig:buffer} shows, this tensor needs to be stored in an on-chip SRAM buffer or DRAM depending on the capacity, even though we still avoid reading the tensor for the adjacent operation. }{Pipeline\_with\_writeback} pattern can be seen in CG as~\autoref{fig:inter-op-reuse} shows in brick red color.

\textbf{{Pipeline\_with\_hold}} - As discussed above, we may need a pipelined tensor in the future. But there is a possibility that the complete chain until the future destination of this tensor is pipelineable. \reviewme{In that case, we hold the tile in the SRAM until the destination consumes that tile as~\autoref{fig:buffer} shows.} The number of held tiles essentially depends on the reuse distance of the transitive edge. This pattern provides the full benefit of pipelining but requires slightly more occupancy in the on-chip SRAM. This pattern can be used in ResNet~\cite{resnet} block with skip connections as~\autoref{fig:inter-op-reuse} shows in cyan color.
...
\textbf{{Parallel Multicast}} - It is also possible to reuse the tensor into multiple parallel tensor operations, which we call {Parallel\_multicast}. These parallel operations are non-transitive edges from the multicasting nodes to the directly connected nodes. This can be seen in CG as~\autoref{fig:inter-op-reuse} shows in green color.

\subsection{Node and Edge Attributes of the Graph}
\label{sec:graph}
\vspace{-1mm}
...
\textbf{edge.inter\_datflow}: It represents one of the inter-operation patterns in~\autoref{sec:patterns} except {Parallel\_multicast} which is an attribute of the multicasting node and the non-transitive edges are marked automatically if the node is multicasting.

\textbf{node.dominance}: We define dominance in both relative terms and absolute terms since relative terms affect the length of the stage while absolute terms affect the occupancy of the intermediate data. If a rank is greater than 1000 AND the ratio of the rank to the other ranks is more skewed than 100:1, the rank is considered as dominant. This attribute can take four values 'U' (uncontracted rank is dominant), 'C' (contracted rank is dominant), 'bal' (All ranks are not too small and none of them is dominant, minimum value of the rank is 50) and 'small' (all ranks are non-dominant and one of them is less than 50).

\textbf{Critical Path/Longest path}: It is the longest path between first and the last node. Intuitively, its the path that moves forward rather than branching out. $longestpath(edge)$ represents the portion of the critical path between source and destination nodes of that edge.

\textbf{node.pathnext/node.pathprevious}: Next/previous node to/from the current node on the longest path.

\textbf{edge.isTransitive}: This checks whether the edge is transitive or not. A transitive edge is an edge which is not on the critical path but the nodes that it connects are on the critical path. For example, in the CG iteration shown in~\autoref{fig:inter-op-reuse}, both the red edges are transitive edges, since they connect nodes on critical path but are not themselves on it. A transitive edge indicates the output of an operation that will be used later. For a {pipelineable} pattern, transitive edge represents the additional {writeback} or {hold} pattern.  
...
\reviewmehpca{\textbf{Shared rank}: Rank of the intermediate tensor that is the output of the source node and input of the destination node}.

\textbf{edge.tensor.swizzled}: A tensor is swizzled when its read in the opposite order to the order in which it was previously written. For e.g., a tensor $P$ written in M-major order but read in the N-major order is considered swizzled.
%A tensor is also said to be swizzled when the transpose is read in the same order as the tensor. For example, if the tensor $P$ with ranks M,N is written in an M-major order and $P^T$ is also read in the M-major order. 
Swizzling is determined from the relative order of the ranks in the loop order of the source and the destination nodes of the edge. Swizzling of the shared ranks is unacceptable if the inter\_dataflow of the edge is {pipelineable} or {pipeline\_with\_hold}. It incurs a penalty in case of {pipeline\_with\_writeback}. This is further discussed in~\autoref{sec:gogeta}.

\subsection{Inter-Operation Reuse Opportunities}
%\vspace{-1mm}
\input{algorithm/interop.tex}
~\autoref{alg:inter-op} shows the methodology to mark the tensor dependency graph with reuse patterns. A node which has more than one non-transitive edges can multicast its output into more than one parallel operations and thus all the non-transitive edges from that node have the~{parllel\_multicast} pattern. A non-transitive edge where the source node is uncontracted dominant and the destination is unshared dominant has a~{pipelineable} pattern. Note that~{pipelineable} inter-operation pattern does not guarantee pipelining since it also depends on the loop order of the individual operations as we discuss in \autoref{sec:gogeta}. Any edge where the source node is contracted dominant or the destination is unshared dominant, has a sequential inter-operation pattern. A transitive edge with uncontracted dominant source node could have~{Pipeline\_with\_hold} pattern, if all the edges on the corresponding critical path are pipelienable, otherwise it has the~{Pipeline\_with\_writeback} pattern.
