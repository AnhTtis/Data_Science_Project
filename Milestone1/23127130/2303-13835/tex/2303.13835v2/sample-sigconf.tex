...
% \renewcommand{\shortauthors}{Zheng Yuan and Fajie Yuan, et al.}
...
% We release our code and other materials at \textcolor{blue}{https://github.com/westlake-repl/IDvs.MoRec}.
...
\keywords{Recommender Systems, ID-based Recommendation,
Modality-based Recommendation, End-to-end training}
...
the community  can not get the real progress of MoRec and the expressive power of ME for recommendation.
...
\textit{Novelty clarification}: Though several recent literature has explored E2E learning~\citep{wu2021empowering,wu2021newsbert,bi2022mtrec,yang2022gram,xiao2022training} for recommendation, none of them explicitly discussed the substantial accuracy  and efficiency gap (100x-1000x) between TS and E2E paradigms. Besides, most of them only discussed the DSSM architecture (or other two-tower variants)  without considering the more powerful sequence-to-sequence (seq2seq) training approach (e.g., used in SASRec and NextItNet~\citep{yuan2019simple}). 
...
assume that the user has clicked on the item before making a purchase or leaving a comment.  However, we cannot assume the opposite holds, which is a common challenge in most recommendation datasets, i.e.  unobserved items can be either positive or negative for the user.
...
To ensure a fair comparison between IDRec and MoRec,
 ...
 Intuitively, the cover of an image or video and the title of a news article, play a crucial role in providing users with a very first impression of an item. This impression greatly influences  their decision on whether or not to click on the item.
 ...
 % This allows us to study the effectiveness of pure textual or visual recommendation.
  ...
  Despite this, it is still possible that these datasets may not \textit{fully } meet the requirement. Especially in the e-commerce scene of the HM dataset, factors such as the cover image, item price, and  sales volume may  jointly  determine whether or not a user clicks on an item (see Figure~\ref{apx:fig:Item cases}).   
 ...
 % In Figure ~\ref{apx:fig:Item cases of HM} and~\ref{apx:fig:Item cases of Bili}, we present items from HM and Bili, respectively, which show that the actual recommendation scene contains other information.  Images, item price, sales, brand, and other factors work together to entice user click interactions.
  Clearly, the cover image alone  cannot provide sufficient information for MoRec to capture such non-visual features, as it is the only input to the item encoder.  Conversely, IDRec is known to be capable of  implicitly learning such features from the latent embedding space~\citep{koren2009matrix}. In other words,  MoRec's performance may have room for improvement if a more ideal   dataset or additional content features were taken into account.
  % \footnote{Unfortunately, we have not yet found a publicly available dataset that fully satisfies this strict assumption.}
...
\subsection{Hyper-parameters}\label{apx:sec:hyper parameters and training cost}
   ...
   For both text and vision encoders, we 
     ...
     Regarding the hyper-parameter setting, our principle is to  ensure that IDRec are thoroughly tuned in terms of learning rate $\gamma$,
...
\begin{table}[htbp]
  ...
  % \footnote{This is simply because the original testing set has too few cold items for evaluation.}
    We report the results in Table~\ref{tb:cold_new_item_recommendation}. As expected, MoRec consistently and substantially improve IDRec on all three datasets for both text and vision modalities in both cold and new settings. In particular, the HR@10 on MIND rises from 0.0036\% to 3.0637\% for cold item recommendation and from 0.0125\% to 0.5899\% for new item recommendation.
