...
We consider finite element approximations to the incompressible Navier-Stokes equations. Our aim is to propose a method that is able to get both pressure-robust error bounds and convection robust error bounds. Pressure-robust error bounds are those for which the error bounds of the velocity are independent on Sobolev norms of the true pressure. Convection robust error bounds are those in which the constants in the error bounds are independent of the Reynolds number. This means that for a solution regular enough such that
the corresponding norms appearing in the bounds do not depend on the inverse of the diffusion parameter $\nu$ then the
errors do not deteriorate while increasing the Reynolds numbers. For the first property, we need to use divergence free pairs
...
%In the numerical experiments of
%this paper, we observe that for the three added stabilization terms (one that appears both in the methods in \cite{barren_cip}
%and \cite{ahmed_etal} and the two other that replace the bulk stabilization term in \cite{ahmed_etal}) only the first one seems
%to be needed. Although we have some ideas that seem to indicate that the two last stabilization terms could be bounded by the first %stabilization term, or a generalization of it including the normal jumps of the
%gradients and not only the tangential jumps, we do not have a rigorous proof of this fact. We leave this question as an open %problem for future research.
...
we define the tangetial jumps across $F=K_1\cup K_2$, $K_1,K_2\in \mathcal{T}_{h}$ as
$$
...
\end{equation}
\end{enumerate}
As shown in \cite{barren_cip} the Scott-Vogelious finite element spaces on Clough-Tocher triangulations verify the above
...
$$
 \displaylines{
 (\nabla \bs_h,\nabla \bvar_h)  = (\nabla \bu,\nabla \bvar_h),\qquad \bvar_h \in {\cal V}_{h},
 }
 $$
...
\|\bz-\bpsi_h\|_*^2.
\end{eqnarray}
To finish the error bound we need to bound the second and third terms above. To this end we will apply some lemmas.
...
\begin{eqnarray}
\label{I11+I12}
I_{11}+I_{12} \le Cc_{\rm inv}\left\| \nabla\bw_h\right\|_\infty \sum_{\left|\balpha\right|=0}^1 h^{\left|\balpha\right|+1/2}\left\| \partial^{\balpha} \be\right\|_h.
\end{eqnarray}
Consequently, applying~\eqref{028} it follows that
\begin{equation}
\label{aaver02}
\left\|h^3  \llbracket \nabla {\cal B}_{\bw}\bv \rrbracket\right\|_{{\cal F}_i} \le 
CC_{\rm tr} \left\| \nabla\bw_h\right\|_\infty
...
\end{equation}
To bound~$S_{\bu}(\bw_h,\bw_h)$ we estimate the different terms separately. Since we are assuming that $\bu\in H^3(\Omega)$ we have that the $(\bu\cdot\nabla)\bu \in H^2(\Omega)$ and, hence, is continuous, so that
that
$$
...
\end{align}
sot that from~\eqref{baver00b},  it follows that
\begin{align}
\label{baver00}
\left\|h \llbracket (\bu\cdot\nabla)\bw_h\times \bn\rrbracket \right\|_{{\cal F}_i}
...
Also, since we are assuming that $\bu\in H^3$, we have that ${\cal B}_{\bu}\bu\in H^1(\Omega)$, which implies (see e.g., \cite[p.~37]{libro_dis})
$$
...
\end{align*}
so that applying the trace inquality~\eqref{trace}
we get
\begin{align}
\label{aaver03c}
\left\|h^3  \llbracket \partial^{\balpha}\bbeta\cdot \nabla \bbeta_h \rrbracket\right\|_{{\cal F}_i}&\le CC_{\rm tr}
h\left(\left\| \nabla \bu\right\|_\infty+\left\| \nabla\bw_h\right\|_\infty\right)\left(h^{3/2} \left\|D^{2}\bbeta\right\|_h
+h^{5/2} \left\|D^{3}\bbeta\right\|_h\right)
\nonumber\\
&{}\le  CC_{\rm tr}
...
\left(h^{1/2} \left\|D^{1}\bbeta\right\|_h
+h^{3/2} \left\|D^{2}\bbeta\right\|_h\right)
\nonumber\\
...
\frac{1}{v_0} \left\| \bz-\bpsi_h\right\|_{*}^2,
\end{equation}
which, together with \eqref{aver00} and~\eqref{aaver03}, and enlarging the constant $C_2$ in~\eqref{aaver03} if necessary, finishes the proof.
 \end{proof}
% \begin{remark}\label{Class2-3} \rm Notice that the hypothesis $\bu \in C^{\min(k,3)}(\Omega)$ can be guaranteed if
%%\begin{equation}
%%\label{strong_condk}
%$\bu \in H^{\min(k,3)+d/2+\alpha}$
%%\end{equation}
%for some $\alpha>0$. Notice also that we have applied such regularity requirement for assuming that a continuous function has zero jumps. However, we
%can reduce the regularity requirements of the true solution assuming only that the jumps are zero (or that they
%are zero except for a set of zero measure respect to the face) which is a weaker assumption than
%assuming that the function is continuous. Then, in practice. it is enough to assume $\bu\in H^{\min{(k+1,4)}}$, see \cite[Theorem 1.1]{libro_dis}.
%\end{remark}
...
$\max(u_0,\left\|\bu_h\right\|_{\infty})^{-1}$.
\end{remark}
\begin{remark} 
...
+ C \| \bz -\bpsi_h\|_{*}^2.
 \end{align}
 Observe that the constants~$C$ in~\eqref{paso_231} and~\eqref{paso_232}, depend on the number of terms of~${\cal B}$ (hence, on the dimension~$d$), the constant $C_{\rm tr}$ in the trace inequality~\eqref{trace}, and the constant $c_{\rm inv}$ in the inverse inequatlity \eqref{inv}. Since, as mentioned, the constant~$C_{\cal B}$ multiplying  $\| \bz -\bpsi_h\|_*^2$ in~\eqref{tercer_paso_22} depends on the number of terms in~${\cal B}$, we conclude that the constant~$C$ in~\eqref{tercer_paso_4} depends on the dimension, $C_{\rm tr}$ $c_{\rm inv}$ and~$C_{4.1}$.
\end{proof}
In next theorem we prove the main result of the paper.
\begin{Theorem}\label{th:main} Let $k\ge 2$ and assume that %$\bu \in L^{\infty}(0,T,H^{k+1})\bigcap C(0,2,C^{\min(3,k)})$ 
...
\begin{eqnarray*}
S(\bw_h,\bw_h)\le \left(C\frac{\delta_{\rm max}}{\bu_0}\|\nabla \bw_h\|_\infty^2 h\right)\|\be_h\|_0^2
+\frac{C_{\bu,\bw_h}}{v_0}\left\| \bz-\bpsi_h\right\|_{*}^2,
\end{eqnarray*}
where $C_{\bu,\bw_h}$ is the constant in \ref{Cuw}.
...
\nonumber\\
&\quad{} +\left(\left(1+C_{\ref{lema_41}}^2\right)\frac{2C_{\bu_h}}{\delta_{\min} v_0}+2C_1\right)
\left\| \bz-\bpsi_h\right\|_{*}^2.
\end{align*}
...
+\left(\left(1+C_{\ref{lema_41}}^2\right)\frac{2C_{\bu_h}}{\delta_{\min} v_0}+2C_1\right).
\end{eqnarray*}
...
%bx=8*(sin(pi*x).^2).*(2*y.*(1-y).*(1-2*y)).*(0.5*(1.2+0.8*cos(4*t)));
%by=-16*pi*((y.*(1-y)).^2).*sin(pi*x).*cos(pi*x).*(0.5*(1.2+0.8*cos(4*t)));
%    p=sin(pi*x).*cos(pi*y).*(0.5*(1.2+0.8*cos(4*t)));
\begin{eqnarray}
\label{eq:exactau}
...
with $N=6$, $12$, $24$ and~$48$ subdivisions in each coordinate direction. In Fig.~\ref{red} we can see the coarsest mesh used. For the CIP stabilized method the parameters were set to $\gamma_1=10^{-2}$, $\gamma_2=10^{-4}$ and~$\gamma_3=10^{-3}$. These values were found to produced the best results after a simple and quick search starting from the values used in~\cite{barren_cip}
\begin{figure}
\begin{center}
