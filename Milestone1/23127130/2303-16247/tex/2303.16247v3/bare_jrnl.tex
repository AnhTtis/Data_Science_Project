...
% !!! IMPORTANT !!! how is this different to semi-supervised learning --> semi uses the derived pseudo labels to retrain the model --> why aren't you doing it rather then this? --> ON THE MARGINAL BENEFIT OF ACTIVE LEARNING:
% DOES SELF-SUPERVISION EAT ITS CAKE? Focusing on S4L, we show that pre-training with contrastive learning has two key benefits. First, it improves the classification performance over plain semisupervised learning. Secondly, it also stabilizes semisupervised learning and reduces the variability when there are very few labels and the quality of the labeled examples significantly affects the outcome. Integrating S4L and active learning and using extensive experiments with contemporary methods, we demonstrate that much of the benefit of active learning is subsumed by S4L techniques. Via ablation experiments, we identify the source of this phenomena as the remarkable benefit of data augmentation in improving label efficiency. 
% !!!IMPORTANT!!! The above discussion leads to another research question, Are we regaining AL usefulness in CL using our method.

% \reasat{(plug in some similar work in supervised domain, get it from joshi)}
% !!! IMPORTANT !!! Can Semantic Labels Assist Self-Supervised Visual Representation Learning?
...
The benchmark achieved its highest score at  100th epoch of SimCLR training and at the 150th epoch of the proxy model training.  The average runtime for 100 epochs of SimCLR training is 538.70 min.
...
Due to uncertainty sampling there is a time reduction of 62\% while coreset sampling requires similar amount of time. Further hyperparameter (such as SimCLR learning epochs, proxy model learning epochs, etc.) tuning might decrease these time requirements which is left for future research. The runtime comparisons are summarized in Table \ref{tab:runtime}. 
...
\section{Conclusion}
In this paper, we have demonstrated that active learning strategies can be used in CL to speed up training using less data. Also, using the uncertainty sampling we have proposed a feedback system for the contrastive learning framework to select the subset with the most amount of information related to the downstream task. We showed for histopathological cancer classification, we can achieve a speedup of 62\% while using 93\% less data. This method requires only a small labeled set. Future work could explore whether this number can be reduced and how that would affect contrastive training. This research focused on classification as the downstream task. Future research can extend this to other tasks such as regression or segmentation. 
