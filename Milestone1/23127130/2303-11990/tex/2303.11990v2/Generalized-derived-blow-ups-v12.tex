...
The key difference with the previous approach is that we are now able to pass through nonconnective algebras, even though connective algebras are our main interest. Since simplicial commutative $k$-algebras are generally different from connective $\Einfty$-$k$-algebras, the latter are not suitable for this. In contrast, connective derived $k$-algebras do coincide with  simplicial $k$-algebras.
 ...
 In ordinary derived algebraic geometry the derived  Rees algebra only makes sense for closed immersions of derived schemes. For example, it can be shown that the deformation space $D_{\A^1/\{0\}}$ is equivalent to $B\G_a$, which is not connectively affine. The $\infty$-category of $A$-algebras $B$ with $A \to B$ surjective on $\pi_0$ is not generated under sifted colimits of compact projective objects. This means it is generally hard to define a functor out of this $\infty$-category, since it has no good theory of left derived functors.
...
Note that $\A^1_\CCC$ has the $\G_{m,\CCC}$-action of weight $-1$, i.e., the one corresponding to the $\Z$-grading on $\LSym_\CCC(\mathbbm{1}_\CCC(1))$. This is the same action as the one induced by the $\G_m$-action on $\A^1$ with weight $-1$ via the functor $F$.
...
\begin{Ass}
	\label{Ass:Bcover}
	For any $\N$-graded, connective $\CCC_{\geq 0}$-algebra $R$ which is generated by weight $1$ over $R_0$, the map
...
\begin{Thm}
	...
	\begin{enumerate}
		...
		\item\label{PI:3} Suppose that $\pi_*\OO_{\DDD_{Z/Z}}$ is perfect, for $\pi: \DDD_{Z/Z} \to \DDD_{Z/X}$ the natural map. Then $\Bl_ZX \to X \times [\A^1_{\CCC}/\G_{m,\CCC}]$ is nonconnectively affine.
	\end{enumerate}
\end{Thm}                       
...
\begin{proof}[Proof of Theorem \ref{Thm:BlFOPaff}]
	...
	By \itemref{PI:1} and Proposition \ref{Prop:VanLocAff}, it holds that $\Bl_ZX$ is nonconnectively affine over $\DDD_{Z/X}$. Hence \itemref{PI:3} follows, since $\DDD_{Z/X}$ is nonconnectively affine over $X \times [\A^1_\CCC/\G_{m,\CCC}]$ by Theorem \ref{Thm:Rees-affine}.
\end{proof}
\begin{Rem}
