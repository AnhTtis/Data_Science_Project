...
\label{exp3r}
% This section discusses the experiment results described in \sref{exp3}.
...
% In order of procedure, we will begin with the results from the model running out to 1,000 epochs.
% Then, we discuss the results in-depth on the experiment, where we reduced the batch size while simultaneously increasing the test size and ran the models for the epochs determined in the first half of this experiment.
...
% For this portion of the experiment, we ran four models with four different batch sizes out to 1,000 epochs.
% This experiment aimed to discover the minimum number of epochs required for the models to train before their respective accuracies begin to plateau.
% Why would we want to find the minimum number of epochs?
% To increase efficiency.
% The purpose of experiment 3 is to increase efficiency while maintaining accuracy. The first step to increasing efficiency is to reduce the number of epochs, but not too much; otherwise, we will lose the necessary training time for the model to achieve the required weights to make accurate predictions.

% If we look at \fref{1000e}, there is a clear distinction around 600 epochs.
% All four runs increase in accuracy, and all four runs begin to plateau around the 600 mark.



%
% So, now that we know where the minimum number of epochs is to help keep the training time down, we run all remaining experiments to this benchmark.
...
% We have the minimum number of epochs; now, the next portion of the experiment is to reduce the batch sizes (the training and validation set sizes) while at the same time increasing test set sizes.
% This experiment aims to see what minimum amount of data is required for the models to retain a high level of accuracy.

%
% \begin{figure}[t]
% \centering
% \includegraphics[scale=0.95]{input/figures/exp3_accuracy.pdf}
% \caption{Accuracy per epoch during training on $\mathcal{G}_{training}$ for different known sizes $N_{known}$ between 0.3125\% and 80\% of the total set size.}
% \label{acc_red}
% \end{figure}
%
...
% We will look at all the scores for each run using their respective confusion matrix followed by their actual score calculations shown in .
% We also have in \fref{acc_red} the accuracy with respect to $\mathcal{G}_{training}$ at each epoch.
As we might expect, the larger $N_{known}$ is, the higher the model's accuracy. %  on $\mathcal{G}_{known}$.
% Comparing this figure and \tref{exp3graphs},
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.95]{input/figures/iterations-stats.pdf}
\caption{Median values of the ``Known 1'' and ``Predicted 1'' sets averaged over six runs using the iterative GDL classification approach with different performance percentiles for $J(\mathcal{G}_{all})$ shown.} 
\label{fig:iterations-stats}
\end{figure}
...
% \begin{figure}
%     \subCM{2,158}{408}{435}{2,190}{CM for the $80\%$ batch.}{80cm}
%     % \hfill
%     \subCM{9,564}{1,259}{2,617}{10,781}{CM for the $40\%$ batch.}{40cm}
%     % \vskip\baselineskip
%     \subCM{14,680}{3,604}{2,223}{13,229}{CM for the $20\%$ batch.}{20cm}
%     % \hfill
%     \subCM{16,632}{4,392}{2,641}{14,828}{CM for the $10\%$ batch.}{10cm}
%     \vskip\baselineskip
%     % \subCM{16,606}{4,911}{3,803}{15,551}{CM for the $5\%$ batch.}{5cm}
%     % \hfill
%     \subCM{15,876}{4,875}{5,155}{16,154}{CM for the $2.5\%$ batch.}{2.5cm}
%     % \vskip\baselineskip
%     \subCM{16,085}{6,578}{5,244}{14,748}{CM for the $1.25\%$ batch.}{1.25cm}
%     % \hfill
%     % \subCM{12,326}{5,148}{9,151}{16,327}{CM for the $0.625\%$ batch.}{0.625cm}
%     \caption{CMs for 8 out of the 9 runs conducted in this experiment.}
%     \label{exp3cms}
% \end{figure}

% \CM{8,817}{4,207}{12,738}{17,339}{CM for the $0.3125\%$ batch.}{0.3125cm}



% \begin{figure}
%     \centering
%     \includegraphics[scale=0.75]{input/figures/exp3_batch_reduce.pdf}
%     \caption{Score results of the batch reduction experiment with respect to training time in seconds.}
%     \label{exp3timeplot}
% \end{figure}

% \begin{figure}
% \centering
% \includegraphics[width=\columnwidth]{input/figures/exp3_batch_reduce_train_size.pdf}
% \caption{Score results of the batch reduction experiment with respect to training set size (\%).}
% \label{exp3sizeplot}
% \end{figure}
