...
\label{model}
% In this section, we dive extensively into the model itself.
% We will go over each layer's job to understand what the model is actually doing.
% To help visualize how the model works, you need to use the information learned in \sref{gt} and picture how a graph is passed through the GNN.
...
% As we discussed in \sref{gclass}, the purpose of this model is to perform graph classifications.
...
% So what are node embeddings?
% If one was to create a graph as seen in \sref{gt}, node embeddings would be the mapping of those nodes in $N$-dimensional space.

\begin{figure}[t]
\centering
% \begin{subfigure}[b]{0.45\textwidth}
%     \includegraphics[width=\columnwidth]{input/figures/circuit.pdf}
%     \caption{\fref{circgraph} represented as a graph.}
%     \label{nwxcirc}
% \end{subfigure}%
% \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[width=0.85\columnwidth]{input/figures/node_embed_mod.pdf}
    \caption{Calculated node embeddings for the graph in \fref{circgraph}.}
    \label{embed}
% \end{subfigure}
% \caption{An example of node embedding.}
% \label{embed2}
\end{figure}

Node embeddings are a mapping of a graph's nodes or vertices into an $N$-dimensional numerical space.
As an example, using the graph from \fref{circgraph}, we can use Node2Vec \cite{b79} to compute two node embeddings, visualized in \fref{embed}.
...
% , we can create a graph in python using Networkx as shown in \fref{nwxcirc}.
% Then, using a module called 
% , we can compute the node embeddings for this graph and plot them, shown in \fref{embed}.
...
% In the next section, we look at the metrics we can "fine-tune" so that the model trains well on the given data.

% Now that we have constructed our model, there are a few things that we can work with to help us achieve an accurate working model.
% These are known as hyperparameters.
% Here, we discuss a few of the hyperparameters we can easily adjust before training the model.
...
% Once the data has been split into the three sets, we can now decide on batch size or the number of data points to work through before updating the model weights. 



% DRH: moved to an ealier section
% For batch sizes, we created three subsets of the data from the primary dataset: the training dataset, the validation dataset, and the test dataset. 
% The model fits the training set, and the fitted model is used to predict the responses for the observations in the validation set \cite{b46}. 
% For example, after each epoch, the model will adjust its weights accordingly and test them on the validation set. 
% It helps the user better understand the model performance and adjust the other hyperparameters as necessary. 
% The test set is used for when the model has completed its training, and you want to now test it on data that the model has not seen.


% These hyperparameters are the easier ones to change without diving into the model itself, and in the next section, we will look at a case study using GDL and graph classification to show their efficiency and effectiveness.
