...
\ee
Nonetheless, the extra terms cannot be absorbed into the $d$-dimensional action, hence they are interpreted as a 't Hooft anomaly polynomial.
...
\be
S[\psi]=\int_{M_d}-\bar{\psi}_{(p)}\wedge\gamma_{(d-2p-1)}\wedge\nabla\psi_{(p)}\,,
\ee
...
On a curved manifold with metric $g_{\mu\nu}$, $\varepsilon$ generalises to a tensor according to the normalisation
\be
