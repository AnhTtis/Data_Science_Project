\begin{figure}
\centering
  \includegraphics[scale=0.6]{./figs/result_analysis.pdf}
  \vspace{-0.5em}
  \caption{
...
%   \includegraphics[scale=0.37]{./figs/cat_teen_plant.pdf}
  ...
  \vspace{-0.5em}
  \caption{Examples of Pascal VOC (PAS-20) segmentation dataset}%. As there are only 20 classes, many objects in the scene are left unlabeled.  which may not be suitable for zero-guidance segmentation.
...
%   \includegraphics[scale=0.512]{./figs/failure_case_long.pdf}
  ...
  1) Classes like `person' that has many visually distinct parts are difficult to reassign labels correctly. 
  2) Background regions that share boundaries with salient objects are still prone to global leak. 
...
\myparagraph{Global context leakage.} Some background segments that share significant boundaries with primary objects can be mislabeled due to the influence of global contexts as shown in Figure \ref{fig:failure_case}.
%which encode the primary objects.
%Despite global subtraction, some of the results still exhibits global leak. 
%These segments are mostly background regions that share boundary with the salient object.
Another problem that can cause context leakage is from the low-resolution 24x24 image grid of CLIP visual tokens. As we downsample our segment masks to fit this grid, we lose masking precision and information can leak between neighboring segments.
