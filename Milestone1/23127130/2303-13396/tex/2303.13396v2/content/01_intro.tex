...
it has been used in the literature for setups that \emph{require} a text query but allow it to be unseen during training, whereas our problem does not require any text query.}.   %This query text can be specified in 
%one needs to gather a dataset with segmentation ground truth.  
%however, with recent advances in joint vision-language representation learning, recent methods allow segmentation
%Conventional image segmentation approaches recognize and locate objects seen during training, ignoring any other objects in the screen. Recently, with the rapid advancements in the joint vision-language domain, new methods use text to guide the segmentation of objects unseen during training; this task is called open-vocabulary segmentation. However, in order to draw out a segmentation map of a certain object, a predefined input text representing the object is needed. 

%In this work, we introduce a novel problem setup: open-vocabulary segmentation with text labeling generation. This problem setup finds segments in an image and output texts by which these segments are identified.
...
%to solve [] segmentation. %In particular, we rely on 
%Surprisingly, this is possible with a pipeline that use a self-supervised visual model DINO-Vit and a visual-language model (CLIP).

%typically required to train a segmentation network. 

%a recent research direction that leverages CLIP, a model trained with image-caption pairs, to solve segmentation \cite{}. These methods  

%To our knowledge, state-of-the-art segmentation algorithms still require semantic supervision or a dataset with ground-truth segments. 

%Recent segmentation techniques that 
%Generally, segmentation algorithms require a dataset with ground-truth segments.

%To train a segmentation network, costly per-pixel annotation is generally required; hence, segmentation systems often lack scalability. To tackle this problem, we propose a framework to utilize existing pretrained generalist visual and visual-language modules to perform semantic segmentation without the need for fine-tuning. 
%Our key idea is to cluster deep per-pixel features of DINO-Vit into segments and translate all segments into natural language texts using a visual-language model (CLIP) and a generative language model.
...
To evaluate our algorithm that can output arbitrary text labels, we also propose new evaluation metrics. Evaluating an algorithm under this setup is not straightforward as the predicted labels may not necessarily match predefined labels in a test set but can still be correct.
