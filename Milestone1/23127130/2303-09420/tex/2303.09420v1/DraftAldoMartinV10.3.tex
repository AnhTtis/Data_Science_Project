...
\begin{document}
    ...
    \begin{abstract}
Interesting phenomena happen when an eccentric rigid body is
rolling on an inclined or horizontal plane. For example, a variety
of motions between rolling and sliding is exhibited until suddenly
a jump occurs. We provide a detailed theoretical description of
the last effect for a general eccentric cylinder. Before the jump,
when the cylinder moves along the ramp, we could assume a pure
rolling motion, however it turns out that when the cylinder is
reaching its jump position, both the normal and static frictional
forces approach zero. Thus, it seems that there will no longer be
...
The physical system consisting of a rigid body, such as hoops,
wheels, disks, and spheres whose center of mass is located at a
distance $d\neq 0$ from the geometric center, that rolls on an
horizontal or inclined plane with friction, has an interesting and
unexpected dynamic behavior, and so the study of this system has
attracted the attention of the community. In the Fig.
\ref{fsystem1}, a general configuration of the physical system is
exhibited, where we can identify: an inclined ramp making an angle
$\alpha$ with the horizontal, and a cylinder of radius $R$ with
its center of mass located at a distance $d$ from the geometric
center. The cylinder rolls over the ramp and its motion is tracked
by the angle $\theta$.
...
In the case of an horizontal plane (i. e. $\alpha=0^{\circ}$),
there is a literature where the dynamics of a particular cylinder,
which consists of a very thin massless cylindrical shell with a
point mass stuck to its surface, is studied and debated
\cite{littlewood,Tokieda}. When the point mass attached to the
cylinder is initially in the highest part of the cylinder and the
system is released from rest (i.e. the initial conditions are such
that $\theta_0=0$ and $\dot{\theta}_0=0$), Tokieda \cite{Tokieda}
showed that the cylinder must jump at $\theta=90^{\circ}$. The
author assumed that the jump happens immediately after the pure
rolling motion, but in subsequent articles, this assumption was
shown to be wrong, for instance, it was shown that before the jump
there must be sliding \cite{Butler,Pritchett,yanzhu2}. Despite
this fact, Pritchett \cite{Pritchett} obtained numerical and
experimental results (for a hula hoop with a stuck point mass)
showing that the jump happens around $\theta = 90^{\circ}$, even
considering slippage. Moreover, to test the jump at this angle of
$90^{\circ}$, Theron \cite{Theron2} claimed that the elasticity of
the hula hoop must be considered. As observed, despite the
physical system being simple, the cylinder motion turns out to be,
in general, complex, and indeed, further analysis showed that a
variety of these motions can include self-induced jumping motions
\cite{Bronars,Batista,Ivanov,Shimomura}, and multiple transitions
back and forth from rolling to slipping
\cite{Kessler,Leine,Theron,Onorato}.

In the case of an inclined plane (i. e. $\alpha\neq 0$) the
cylinder rolls down due to the force of gravity, and then at some
point it loses contact with the ramp and jumps. In the literature,
we have found a few models that consider this case, one of them
studied an eccentric wheel \cite{Theron2}, and the other an
eccentric disk \cite{Moore2021}, these works hypothesize that the
wheel (or disk) must slip before jumping. In other references
\cite{Carnevalia,Turner,Jensen,Hu}, by means of the Lagrangian
formulation or a cumbersome torque analysis, the equations of
motion for this cylinder have been studied. However, these works
did not determine the position where the cylinder jumps.

In all references we have found (with exception of \cite{gomes}),
it was assumed that the necessary condition for the jump is when
the normal force $F_y$ (acting perpendicularly from the ramp over
the cylinder) vanishes ($F_y=0$) at the instant of the hop. Let us
remark that a different jump condition will be used in this work,
and this condition turns out to be equivalent to that of
\cite{gomes} but derived from a different approach. As argued in
reference \cite{gomes}, the equation that the angle $\theta$ and
the angular velocity $\dot{\theta}$ must satisfy at the position
where the cylinder jumps is given by
\begin{eqnarray}
    \label{jumpconditionintro}
    -g \cos\alpha + d \, \dot{\theta}^2
    \cos\theta = 0.
\end{eqnarray}
Assuming pure rolling motion which implies conservation of energy,
and using the initial conditions $\theta_0 =0$ and $\dot\theta_0
=0$, together with Eq. (\ref{jumpconditionintro}), it was possible
to determine an angle $\theta_J$, and consequently the position
$l_J = R \theta_J$ along the inclined plane where the cylinder
jumps. However, there is a subtlety in the pure rolling
assumption. For a given value of the coefficient of static
friction $\mu_s$ between the cylinder and inclined plane, before
the cylinder reaches the position determined by the angle
$\theta_J$, it may happen that the static friction exceeds its
maximum value which would imply that the cylinder slips before
jumping.
...
jumps just after a pure rolling motion? As we are going to see,
the answer to this question is not trivial and depends on the
values of $\mu_s$, the angle $\alpha$, and the inertia moment of
the rigid body considered. A general body that encompasses all the
types of eccentric bodies having a cylindrical shape will be
referred as a general eccentric cylinder. It turns out that the
dynamic characteristic of this cylinder can be parameterized using
two parameters, $\chi$ and $k_m$. Exploring this generality that
we are considering in our work, we prove that there are (quite
common) situations in which the jump occurs immediately after the
sliding roll, and there are (less common) situations where the
jump occurs immediately after the pure rolling. We also prove that
when the jump occurs immediately after the sliding roll, we have
that $F_y=0$; and in the cases where the jump occurs immediately
after the pure rolling, we have that $F_y>0$. Then it would be
correct to consider $F_y=0$ as a necessary condition for the jump
whenever there is sliding rolling immediately before the jump, as
occurs in many studies in the literature, but it would be
incorrect to state the same when the jumping occurs immediately
after a pure rolling, or in general cases.

This paper is organized as follows. In section 2, we describe the
physical system to be analyzed and derive the corresponding
equations of motion. Then we propose a model for the general
eccentric cylinder. In section 3, in order to have a jump without
slip, for any value of $\alpha$ and $\mu_s$ we determine the
region where the parameters that characterized the dynamical
behavior of the general eccentric cylinder must belong. In section
4, a summary and further directions of exploration are given.


\section{Description of the system and derivation of the equations of motion}
\label{system-equations}

In this section, we define the physical system to be studied and
derive the corresponding equations of motion. The mechanical
system consists of a general eccentric cylinder rolling down a
inclined ramp as shown in Fig. \ref{fsystem1}. The term eccentric
means that the center of mass is located at a distance $d$ from
the geometric center. And by general we mean that the mass
distribution within the cylinder is arbitrary with the only
restriction that this distribution is invariant under translations
along the $z$-axis. Throughout the motion, we will assume that the
principal axis of the cylinder passing through point $C$ always
remains parallel to the $z$-axis. Note that when the cylinder's
height is very small compared to the radius, the cylinder
represents a disk, and if the mass distribution is concentrated on
the edge, the cylinder becomes a hoop or wheel. In this sense, we
say that this general eccentric cylinder can encompass all types
of eccentric bodies that have cylindrical shape.

In Fig. \ref{fsystem1}, we also show the coordinate system $xy$,
where its origin is set at the point $O$ which is located at the
top of the ramp. The $x$-axis and $y$-axis are parallel and
perpendicular to the ramp, respectively. The position of the
center of mass, with respect to the geometric center of the
cylinder, is given by the vector
\begin{eqnarray}
...
The equations (\ref{ffs}) and (\ref{theta-eq}) will be useful to
analyze three possible types of motion for the cylinder: pure
rolling, rolling plus slipping and flight motion; where for each
type of motion, it should be set additional relations between the
forces and positions. In the next two subsections, we study the
equations in the case of pure rolling and flight motion.

\subsection{Equations in the case of pure rolling motion}
Since the cylinder is in contact with the ramp, we have that
$b=R$, i.e., $b$ is constant, therefore $\ddot{b}=0$. Moreover,
due to the pure rolling condition, we also have $l=l_0+R
(\theta-\theta_0)$, and consequently $\ddot{l}=R\,\ddot{\theta}$.
The moments of inertia $I_C$ and $I_{CM}$ with respect to the
geometric center $C$ and the center of mass $CM$ of the cylinder
are related by the equation $I_C = I_{CM} + M d^2$. Substituting
these equations into Eqs. (\ref{ffs}) and (\ref{theta-eq}), it is
not difficult to derive the following nonlinear second order
...
Before the jump, the cylinder stays in contact with the ramp, then
$b(t)=R$ and $\dot{b}(t)=\ddot{b}(t)=0$, and the values of
...
change continuously; where the validity of the continuity of
$\dot{b}(t)$ and $\dot{\theta}(t)$ is supported by the fact that
there is not additional external forces acting on the cylinder at
the jump's instant, that would change the cylinder's momenta.
...
initial conditions for $b(t)$ in the flight motion, therefore when
the value of $\ddot{b}$, given by the Eq. (\ref{lb-eq2}), starts
to be greater than zero, it implies that the values of $\dot{b}$
and $b$ start to grow. It is interesting to note that this
increase in the values of $\dot{b}$ and $b$ happens because we assumed that the cylinder is not attached to the ramp. Consequently, at the point where the transition from rolling
to flight motion occurs, we must have that $\ddot{b}=0$ in Eq.
(\ref{lb-eq2}), this condition means \footnote{Using another
alternative approach, this jump condition was obtained in Ref.
\cite{gomes}.}:
\begin{eqnarray}
    \label{jumpcondition}
...
Before the jump, in order to know the behavior of $\ddot{b}$ as
defined in Eq. (\ref{lb-eq2}), we need to track the value of
$\dot{\theta}$. The cylinder's motion starts with
$\dot{\theta}=0$, and after that, because the action of gravity,
$\dot{\theta}$ grows. Therefore, at the beginning
$\ddot{b}=-g\cos\alpha<0$, and after that, $\ddot{b}$ also grows
because the contribution $\dot{\theta}^2$ (with some oscillation
due the factor $\cos\theta$). This means that at some future
instant, when $\ddot{b}$ goes to zero, i.e. when Eq.
(\ref{jumpcondition}) is satisfied, we reach the instant where the
cylinder jumps.

Let us comment that, in other works
\cite{Theron2,yanzhu2,Moore2021}, the jump condition has been
given by $F_y=0$, namely the jump happens at the point where the
normal force vanishes. However, these works consider the
hypothesis that the jump is not possible from pure rolling, which
means that the cylinder must slide before jumping. Although the
main study of our work is not focused on the slip case, using
equations (\ref{ffs}) and (\ref{theta-eq}), together with the
relation $F_x = \sigma \, \mu_k \, F_y$, where $u_k$ is the
coefficient of kinetic friction, and $\sigma = -1$, in case $
\dot{l} > R \dot{\theta}$ (i.e., skidding motion), and $\sigma =
+1$, in case $ \dot{l} < R \dot{\theta}$ (i.e., spinning motion),
we can show that the normal force $F_y$ is given by
\begin{eqnarray}
    \label{normalslip1}
...
We note from Eq. (\ref{normalslip1}) that the jump condition given
by Eq. (\ref{jumpcondition}) clearly implies that the normal force
$F_y$ vanishes. We would like to stress that this is not true in
the case where the jump happens from pure rolling, and in fact as
we will see, at the jump point the normal force does not vanish,
moreover $F_y > 0$.
...
the only restriction that this distribution is invariant under
translations along the cylinder's principal axis, so that the
center of mass does not coincide with the cylinder's geometric
...
of motion have a scale invariance, using this invariance we can
find common characteristics between two different cylinders in
such a way that the dynamics of both cylinders are equivalent,
where those common characteristic between two different cylinders
can be parameterized using two independent parameters. We also
propose a simplified model of the eccentric cylinder, where the
two independent parameters have a simple geometric and mass
distribution interpretation. It is proved that this simplified
model is dynamically equivalent to any general cylinder considered
in our study.

Let us consider a cartesian coordinate system fixed to the
cylinder, such that the $z$-axis coincides with the principal axis
and the coordinate origin is located at the cylinder's geometric
center $C$. The center of mass $CM$ and cylinder's moment of
inertia respect to the $z$-axis are computed by
$\vec{r}_{CM}=\frac{1}{M}\int dxdydz\,\vec{r}\rho(x,y)$ and
$I_C=\int dxdydz\,(x^2+y^2)\rho(x,y)$ respectively, where
$\rho(x,y)$ is the mass density which is bounded by the
cylindrical surface. Since the mass distribution is invariant
under translations along the $z$-axis, the density $\rho(x,y)$
does not depend on $z$. These specifications provide two general
implications:
\begin{enumerate}
    \item The $CM$ is located at any
 distance $d$ from the cylinder's geometric
center $C$, and this distance is restricted to the interval
$d\in[0,R]$. For example, the particular case where $d=R$ happens
when the whole cylinder's mass is distributed along a line on the
cylindrical lateral surface which is parallel to the $z$-axis.

    \item For fixed values of $M$, $R$
        and $d$, the moment of inertia $I_{CM}$ (or
        equivalently $I_{C}$) has minimum and maximum values, where these bounding values are:
    \begin{enumerate}
...
the equations of motion, more explicitly these relations are given
by $\theta(t)=\tilde{\theta}(\sqrt{\frac{g}{R}}t)$, $b(t)=R \,
\tilde{b}(\sqrt{\frac{g}{R}}t)$ and $l(t)=R \,
\tilde{l}(\sqrt{\frac{g}{R}}t)$. At this point, we are ready to
analyze the dependence of the cylinder's dynamics with respect to
$M$ and $R$. Consider two cylinders, one of them with mass and
radius given by the set $(M,R)$ and the other by $(M',R')$. If
$\chi$ and $\tilde{I}_{CM}$ are the same for both cylinders, then
due to Eq. (\ref{eq-scale}) they share the same solutions for
...
events that happens throughout the motion (like the time where the
jump happens) scales by a factor proportional to the square root
of $R$. From these observations, and defining the scale factor
...
$$\int dxdydz\,\left(\left(\frac{x}{R}\right)^2+\left(\frac{y}{R}\right)^2\right)\frac{\rho(x,y)}{M}=\int dx'dy'dz'\,
\left(\left(\frac{x'}{R'}\right)^2+\left(\frac{y'}{R'}\right)^2\right)\frac{\rho'(x',y')}{M'}\,,$$
which means that under the transformation $(x',y',z')=(\lambda
x,\lambda y,\lambda z)$, and the change of mass from $M$ to $M'$,
we have that
...
equivalent. Now, since the cylinder's dynamics is basically ruled by
...
In this section, we study the following sequence of motions for
the cylinder and the conditions to have such a sequence:
\begin{align}
...
use the following shortcut JARM to mean: jump after a pure rolling
motion.
...
values of the normal force ($F_y\leq 0$) are not physical allowed
in our study.

Before the normal force becomes negative (when it is positive and
approaching to zero), the inequality given by Eq.
(\ref{ineqcondition2}) is fulfilled, nevertheless at some point
this inequality will not be true, which physically means that the
cylinder starts to slip. Numerical inspection \cite{progresswork}
evidenced that the point, where inequality (\ref{ineqcondition2})
is not fulfilled, is close to the point where the normal force is
null ($F_y=0$), and also is close to the point where the jump
condition (given by  Eq. (\ref{jumpcondition})) is satisfied. Due
to these observations, and in order to have a JARM, in the case of
pure rolling motion, we are motivated to analyze the following
condition for the normal force
\begin{align}
    \label{ineqcondition1}
    F_{y} > 0\,.
\end{align}
Since this condition is less restrictive than the condition given
by Eq. (\ref{ineqcondition2}), clearly we will need to complement
Eq. (\ref{ineqcondition1}) with Eq. (\ref{ineqcondition2}).
...
Regarding this result of the normal force $F_{y,J}$, from equation
(\ref{Njumpfn}), we note that $F_{y,J}$ is not necessarily equal
to zero. In the case where $F_{y,J} > 0$, and since for $\theta>
\theta_J$ we have that $F_y = 0$, it is clear that the normal
force changes discontinuously from a non-vanishing to zero value
at the point where the cylinder jumps.
...
A subtle issue that can be observed from equation
(\ref{signs0}) is that the normal force at the jump point could
even be negative, this physically would mean that before the
cylinder jumps, the normal force could have been zero and
therefore the cylinder may have slipped before jumping, and this
would imply that the pure rolling assumption is no longer valid.
In what follows, this issue will be treated in more detail.
...
Eq. (\ref{ineqcondition2}). Therefore, through Eq. (\ref{signs0})
let us start our analysis by searching possible allowed values of
$\theta_J$, so that the condition given by Eq.
...
As a particular application of the above general results, let us
comment about the case $\alpha = 0$, namely the case of the
horizontal plane. Note that according to Eq. (\ref{inethetaj}),
when $\alpha=0$ the only allowed possibility for the value of the
angle $\theta_J$ should be $\theta_J = 2 \pi n$. Nevertheless, if
we replace this value $\theta_J = 2 \pi n$ into the left hand side
of Eq. (\ref{ineq2}), the result blows up, which means that to
maintain roll without slip, we would need $\mu_s \rightarrow
\infty$, since it is not physically possible to have an infinity
value for the coefficient of static friction, the conclusion is
that in the case where $\alpha = 0$, regardless the values of
$\chi$ and $k_m$, the no-slip condition must be violated before
jumping. In what follows, we are going to discuss the case $\alpha
\neq 0$.
...
It turns out that to guarantee the existence of a root $\theta_J
\in [2\pi n-\alpha, 2\pi n]$ of the function $ J(\theta)$, we must
have that
\begin{align}
...
0, \;\;\; \text{and}, \\
 ...
 0.
\end{align}
This result means that for given values of $n$ and the parameters
$\alpha$, $\chi$, $k_m$ that satisfy the inequalities
(\ref{JtaC1}) and (\ref{JtaC2}), there is an angle $\theta_J \in
[2\pi n-\alpha, 2\pi n]$ that is a solution of the equation
$J(\theta_J) = 0$. Clearly, through this equation, it should be
...
Since the extra conditions will come from the analysis of
inequalities of the type given in (\ref{ineqcondition1}) and
...
$\alpha$, and the parameters $\chi$ and $k_m$ that characterized
the cylinder, belong to a restricted region. If these parameters
do not belong to the aforementioned region, before the jump the
cylinder have to do another type of motion that goes beyond pure
rolling, for instance slipping with rolling. Using our general
cylinder, these other varieties of motions will be analyzed in a
future paper \cite{progresswork}.
...
needed, so we have used the somewhat standard initial conditions,
$\theta_0=0$ and $\dot \theta_0 =0$. It will be an interesting and
non-trivial problem, to analyze how the regions shown in Fig.
\ref{resultado1} change when other initial conditions are used.
For example, we can see if these regions increase or decrease,
which would physically imply a greater or lesser chance of having
JARM. What would be the optimal initial conditions that allow a
greater chance of having JARM?

Finally, since only the slipping motion before the jump was
observed in experiments and theoretical studies carried out to
date, it was a common conclusion that the no-slip conditions must
be violated before the jump, and indeed when $\alpha=0$, on
general grounds, we have definitely shown that this last
conclusion is true. While in the case where $\alpha \neq 0$, there
is chance to have a JARM, so to empirically test the occurrence of
JARM, it would be important to set up an experiment that takes
into account appropriate values for $\mu_s$ and the parameters
$\chi$ and $k_m$.
...
%We would like to thank... This work is supported by ...
