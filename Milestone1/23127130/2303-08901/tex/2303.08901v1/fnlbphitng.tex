...
We train a machine learning model on observable properties of simulated Illustris-TNG galaxies to predict $\bphi$ for samples constructed to approximate DESI emission line galaxies (ELGs) and luminous red galaxies (LRGs). We find $\sigma(\fnlloc) = 2.3$, and $\sigma(\fnlloc) = 3.7$, respectively.
...
However, all of these analyses (save Ref.~\cite{Cabass22BOSS}) make a simplifying assumption that the relationship between the LPNG bias $\bphi$ and the linear tracer bias $b_1$ is perfectly known. 

A slew of recent papers has called this knowledge, which was always approximate, into significant question for simulated galaxies and other tracers \cite{2020JCAP...12..013B,2020JCAP...12..031B,2022JCAP...01..033B,2022JCAP...11..013B}.
...
First, we obtain $b_{1}(M,c), b_{\phi}(M,c)$ from the gravity-only Separate Universe (SU) results of \cite{Lazeyras22}, and assign $b_{1}(M,c), b_{\phi}(M,c)$ to each TNG galaxy's host halo of mass $M$ and concentration $c$.
...
To ensure that we correctly model the local environment surrounding the halos, we drop the galaxies near the edges of the simulation (near $1\ h^{-1}~\mathrm{Mpc}$)\footnote{But take them into account when calculating neighborhood information of other halos/galaxies}. We do this to eliminate boundary bias and edge effects and choose the value of $1\ h^{-1}~\mathrm{Mpc}$ based on the tradeoff between the loss of data and the potential for edge effects. We observe no noticeable variation in results upon increasing this distance further.

\subsubsection{Observables}\label{sec:observables}
The halos in the IllustrisTNG simulation are determined with the friend-of-friends (FoF) algorithm \cite{davis_fof}, with linking length $b=0.2$.

We use the results of Ref. \cite{Lazeyras22} to link $\bphi$ and $b_1$ to halo mass and concentration. We take halo concentration from the IllustrisTNG supplementary data catalog \cite{Anbajagane2021}. It is defined as $c=R_{200c}/R_s$, where $R_s$ is determined by fitting a Navarro-Frenk-White (NFW) \cite{nfw} profile to the dark matter density profile. 
...
\hline
Type & $\log_{10}{sSFR}$ & $n$ $[h^{-1}~\mathrm{Mpc}]^{-3}$& $\min{\log_{10}{M^*}}$ & $\bar{b}_{1}$\\ \hline
...
\begin{align}
C_{XY} &= \left(b^{X}_{1} + \mu^{2} f + \bphi^{X}\fnlloc \mathcal{M}^{-1}(k) \right)\left(b^{Y}_{1} + \mu^{2} f + \bphi^{Y}\fnlloc \mathcal{M}^{-1}(k) \right) P_{L}(k) + \frac{\delta^{D}_{XY} }{\bar{n}_{X} } 
,\\
...
Here we report results for both the simulated LRG and ELG galaxy samples, for the ideal values of $\bphi(b_{1})$ in each tertile (i.e. concentration information is recovered perfectly) and for the predicted (``pred.'') values using the machine learned relationship $\bphi(b_{1})$ from observable galaxy properties.
...
Including $P_{2}$ as a parameter generally reduces the forecasted error, however, in the sample-variance limit $P_{2}$ adds no information (see discussion around eqn.~\ref{eqn:fish_info_23_rat}).
...
In more detail, while the $\bphi$ values drop uniformly across all tertiles with the mean, the $b_1$ value in the highest tertile hardly changes with increasing number density, but the lower tertile drops significantly, and this leads to a lower change in linear bias between the upper and lower tertiles at higher number density, which in turn leads to a smaller relative amplitude Jacobian factor in the Fisher matrix (see eqn.~\ref{eqn:jac_alpha}).
...
\begin{table}[h!]
    ...
    The entries listed ``($\overline{\mathrm{LRG}}$, $\overline{\mathrm{ELG}}$)'' use the mean $\bphi(b_1)$ prediction with the full number density of each sample
    Here ``+'' and ``-'' denote the top and bottom tertile values of $\bphi(b_1)$ of the respective samples, respectively. 
...
This forecast does not make use of the learned $\bphi(b_1)$ relations presented in this paper.
We next consider a similar case that uses the mean of the $\bphi(b_{1})$ values for the simulated LRG sample and the simulated ELG sample (with the fiducial choice of sample parameters: $\bar{n}_{ELG} = 5 \times 10^{-4}$, $\bar{n}_{LRG} = 2 \times 10^{-4}$, and $\log_{10}(\mathrm{sSFR}) = -9.09$).
...
We now make the even further simplification, for illustrative purposes, that each $\bphi$ of the two samples in question is described by a UMF relation for some choice of the parameter $p$, and so $b_{\phi,1} = \delta_{c}(b_{1}-p_{1})$, $b_{\phi,2} = \delta_{c}(b_{2}-p_{2})$, and for $\alpha\to1$ we have $\Delta\bphi \to \delta_{c}\Delta p$, and $\frac{\partial \alpha}{\partial \fnlloc} \to -\delta_{c} \Delta p \mathcal{M}^{-1}(k)$.
...
To explore the potential of using samples with large differences in $\bphi$ further, we apply our 2-way forecasting framework to two future surveys that will target LPNG, MegaMapper \cite{simone_highz_spectro}, and SPHEREx \cite{spherex14} using the setup described in Section~\ref{subsec:fisher_setup}. 
...
For the following multi-tracer forecasts, we will set $b_{\phi,i} = \delta_{c}(b_{i} -[p \pm \Delta p])$ where $p=1$ unless otherwise indicated\footnote{This is in a sense a somewhat pessimistic choice, see Appendix~\ref{app:fisher_triple} for more discussion}.
...
The solid blue line shows the two-tracer forecast for the scenario in which the number density of the MegaMapper sample is split into thirds, and the top and bottom tertiles of the sample are assigned the \textit{same} linear bias $b$ and an LPNG bias of either $\bphi = b -(1 \pm \Delta p)$.
...
These expressions illustrate that these contributions both approach $\frac{\Delta b_{\phi,23}}{\Delta b_{\phi,12}}$ as $X_{2}\to 0$, but can be significantly different otherwise.
...
We note that even in the sample variance limit, it is clearly possible to have three tracers be less informative than two tracers - as this can occur when $(b_{\phi,2}-b_{\phi,3})^2 < (b_{\phi,1}-b_{\phi,2})^2$.
To be consistent with the notation of Ref.~\cite{UrosMulti} we have taken the two-tracer case to have LPNG biases $b_{\phi,1}$ and $b_{\phi,2}$, so care should be taken to interperet these expressions appropriately (and not to simply associate $b_{\phi,1}$ with $b_{\phi}^{-}$ or $b_{\phi,3}$ with $b_{\phi}^{+}$).
...
Eqn.~\ref{eqn:fish_info_23_rat} also indicates that the exact departure from this symmetry may affect whether a 2-tracer of 3-tracer forecast is desirable.
