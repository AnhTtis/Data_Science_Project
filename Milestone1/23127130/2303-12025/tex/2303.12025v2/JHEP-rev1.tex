...
\ba
\label{S3}
S_{\pi^3} = - M_P^2 \int dt d^3 x   a^3 \Big[ \big( H H^{(3)} + 3 \dot H  H^{(2)}  + \frac{1}{3} H^{(4)}\big) \pi^3  + H^{(3)} \pi^2 \dot \pi + H^{(2)} \pi \dot \pi^2 
\Big] \, ,
\ea
and
\ba
\label{S4}
S_{\pi^4} = - M_P^2 \int dt d^3 x  a^3 \Big[ \big( \dot H H^{(3)} + \frac{3}{4}   {\ddot H}^2  + \frac{1}{4} H H^{(4)}+ \frac{1}{12}  H^{(5)}  \big) \pi^4  + \frac{H^{(4)}}{3} \pi^3 \dot \pi + \frac{H^{(3)}}{2} \pi^2 \dot \pi^2  
\Big] \, .
\ea
...
\ba
\label{H4}
{\bf H}_4 =  \frac{M_P^2}{2} \Big( H^4 \eta^2 \epsilon a^2 - \eta'  H^3  \epsilon a \Big)  \pi^2 \pi'^2 \, .
\ea
...
\rangle = - 4 M_P^2   \mathrm{Im}  \int d \tau \eta \epsilon a^2
 ...
 \big \rangle  %\nonumber\\
\ea
in which $\mathrm{C. P.}$ stands for cyclic permutations. 
...
in our analysis can be compared with the {\it total} one-loop corrections in \cite{Kristiano:2022maq}. %However, since the contribution from the quartic Hamiltonian is not calculated in \cite{Kristiano:2022maq}, the comparison between our result and the result in \cite{Kristiano:2022maq} can not proceed further. 
...
\ba
\label{fraction}
\frac{\Delta \calP}{\calP_{\mathrm{CMB}}} = \frac{2 \Delta N}{h} 
   ( 3 h^2 - 9 h + 378)  e^{6 \Delta N} \calP_{\mathrm{CMB}} \, .
\ea
The above ratio depends on the two independent parameters $h$ and $\Delta N$.
If one takes the duration of the USR period to be long enough, or for a given value of $\Delta N$, takes $|h|$ to be arbitrarily large, then the above ratio becomes  unity, invalidating the perturbative analysis. For example,  for the standard sharp transition with $h=-6$   the ratio becomes order unity 
...
\frac{( 6- h)^2}{2 |h| \Delta N ( 3 h^2 - 9 h + 378)} \, .
\ea 
...
$\frac{\Delta \calP}{\calP_{\mathrm{CMB}}}  \ll 1$, we require  ${\cal C}$ no to be very small. For example, one may require ${\cal C} > 10^{-1}$ or so. 
 Now let us estimate the numerical factor $ {\cal C}(\Delta N, h)$ for various values of $h$. For $h=-6$,  as used in  \cite{Kristiano:2022maq}, we obtain $ {\cal  C}(\Delta N, h) \simeq  0.02  $ while for sharper transitions $h=-50$ and $h=-150$ we obtain $ {\cal C} \simeq  0.004  $ and  $ {\cal C} \simeq  0.001$ respectively. It is clear that in the models with sharp transitions there is no practical chance of  PBHs formation as the naumerical values of ${\cal C}$ are typically very small. 
 
% the criteria  $\calP_{\mathrm{USR}} \sim 10^{-2}$ with the additional physical requirement that  $\frac{\Delta \calP}{\calP_{\mathrm{CMB}}}  \ll1$ becomes difficult to meet.  
...
\vspace{0.5cm}
 ...
 and for pointing out the typos.  
