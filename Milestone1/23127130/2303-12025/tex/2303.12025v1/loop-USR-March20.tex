...
attracted interests. More specifically, it was argued in   \cite{Kristiano:2022maq} 
...
in \cite{Riotto:2023gpm, Riotto:2023hoz}. In particular, it was argued in 
\cite{Riotto:2023hoz} that if one considers a physical situation where the transition from the USR phase to the final slow-roll phase takes place mildly, then the dangerous one-loop corrections are washed out and the mechanism of PBHs formation in USR setup is still working. We also comment that one-loop correction in power spectrum was also studied in \cite{Inomata:2022yte} in a model of inflation where perturbations are resonantly amplified because of the oscillatory feature 
in potential.  It was shown that the one-loop correction can become large to invalidate the perturbative assumption. 
...
while $\eta\simeq -6$. This is the hallmark of the USR inflation \cite{Kinney:2005vj}.
 ...
 \ea
in which $\epsilon_i$ is the value of $\epsilon$ at the start of inflation and prior to USP phase. The USR phase is during the period $\tau_i < \tau <\tau_e$ so the value of $\epsilon$ at the end of USR phase is $\epsilon_e = \epsilon_i \big( \frac{\tau_e}{\tau_i} \big)^6 $. Defining the number of e-fold as $d N= H dt$, the duration of the USR phase is denoted by $\Delta N \equiv N(\tau_e) - N(\tau_i)$ and
...
\ba
M_P^{-1}\phi(N)= \frac{C1}{3} e^{-3 N} + \frac{h}{6}  \sqrt{2 \epsilon_V} N + C_2 \, ,
\ea
...
\ba
\label{h-def}
h\equiv \frac{6 \sqrt{2 \epsilon_V} }{\dot \phi(t_e)}  = -6 \sqrt{\frac{\epsilon_V}{\epsilon_e}} \, .
\ea
...
\ea
From the above formulas we see that towards the final stage of inflation, $\tau \rightarrow 0$, $\epsilon \rightarrow \epsilon_e (\frac{h}{6})^2$ while $\eta $ vanishes like $\tau^3$. While $\epsilon$ is by construction  smooth  at the transition point but $\eta$ has a radical change at $\tau=\tau_e$. Right before the transition (i.e.  during the USR phase) $\eta=-6$
...
The other case is ``extreme" sharp transition in which $|h| \gg 1$. In this case, $\epsilon$ after the transition evolves to a larger value and at the end of inflation (or when the evolution in final stage has reached its attractor phase) we have the final value  $\epsilon_0  = \epsilon_e (\frac{h}{6})^2$. 
...
Our goal is to expend the EFT action to fourth order in perturbations of $\pi$. For this purpose,  we build upon the analysis of  \cite{Akhshik:2015nfa} which calculated the action to cubic order in order to  calculate $f_{NL}$. 
...
- \epsilon \frac{d \eta}{d t} H^3
+O(\epsilon ^2) \, , \\
{H^{(5)}} &=& -\eta ^4 \epsilon H^6 - 3 \epsilon  \eta^2  \frac{d \eta}{d t} H^5  
- 3  ( \frac{d \eta}{d t})^2 H^4 - 4 \epsilon  \eta  \frac{d^2 \eta}{d t^2} H^4
-   \epsilon    \frac{d^3 \eta}{d t^3} H^2
+O(\epsilon ^2) \, .
\ea
...
\ba
\label{S2}
S_2= M_P^3 \int dt d^3 x   a^3 \epsilon H^2 \big( \dot \pi^2 - (\partial_i \pi)^2 \big) \, ,
\ea 
while the cubic and quartic actions on the superhorizon limit where we neglect the 
spatial derivative terms are given by 
\ba
\label{S3}
S_{\pi^3} = - M_P^3 \int dt d^3 x   a^3 \Big[ \big( H H^{(3)} + 3 \dot H  H^{(2)}  + \frac{1}{3} H^{(4)}\big) \pi^3  + H^{(3)} \pi^2 \dot \pi + H^{(2)} \pi \dot \pi^2 
\Big] \, ,
\ea
and
\ba
\label{S4}
S_{\pi^4} = - M_P^3 \int dt d^3 x  a^3 \Big[ \big( \dot H H^{(3)} + \frac{3}{4}   {\ddot H}^2  + \frac{1}{4} H H^{(4)}+ \frac{1}{12}  H^{(5)}  \big) \pi^4  + \frac{H^{(4)}}{3} \pi^3 \dot \pi + \frac{H^{(3)}}{2} \pi^2 \dot \pi^2  
\Big] \, .
\ea
...
\ba
\label{action3}
S_{\pi^3} =  M_P^2 H^3 \int d\tau d^3 x\,  \eta \epsilon a^2\,    \pi \pi'^2  \, .
\ea
Correspondingly, the cubic interaction Hamiltonian is obtained to be
\ba
\label{H3}
{\bf H}_3 = - M_P^2 H^3 \eta \epsilon a^2\,    \pi \pi'^2 \, .
\ea
Here and below, a prime denotes the derivative with respect to the conformal time. 
...
\ba
\label{action4}
S_{\pi^4} =  \frac{M_P^2}{2} \int d\tau d^3 x\,   \epsilon \Big( \eta^2  a^2 H^4 
+ a \frac{d \eta}{d \tau} H^3\Big)\,    \pi^2 \pi'^2  \, .
\ea
...
\ba
\label{H4}
{\bf H}_4 =  \frac{M_P^2}{2} \Big( H^4 \eta^2 \epsilon a^2 - \eta'  M_P^2 H^3 \eta \epsilon a^2 \Big)  \pi^2 \pi'^2 \, .
\ea
Equipped with the above interaction Hamiltonians, we can proceed to calculate the one-loop corrections in curvature perturbation power spectrum. 
 ...
 $\tau=\tau_0 \rightarrow 0$ where it is assume that the system is in the slow-roll regime and the perturbations are frozen on superhorizon scales, $\dot \pi =\ddot \pi=0$. Fortunately, in this limit all the non-linear corrections in $\calR$ in Eq. (\ref{pi-R1}) are suppressed  and we end up with the simple linear relation between $\calR$ and $\pi$:
 \ba
 \label{pi-R}
...
\label{bispectrum}
  ...
  setup. This is a two-step model in which the first phase is USR inflation where the large CMB scales are generated followed by a second phase of slow-roll inflation. 
  So this setup should not be confused with our main three-phases setup which was reviewed in section \ref{setup} which will be employed for the loop correction in the next section. While the current analysis is new and interesting by itself, the reader who is only interested in one-loop corrections  can skip directly to the next section.
    
 The analysis of bispectrum for the simple USR setup was performed originally in \cite{Namjoo:2012aa}, see also \cite{Chen:2013aj, Chen:2013eea}.  This analysis was extended to a general non-attractor setup with $c_s \neq 1$ in \cite{Akhshik:2015nfa} using the EFT approach as in current analysis. However, the analysis in \cite{Akhshik:2015nfa} was for the case of extreme sharp transition $h \rightarrow -\infty$. Here, we extend the analysis of \cite{Akhshik:2015nfa} to the general case where the transition from USR phase to SR phase is controlled by the parameter $h \leq -6$. We confirm that our result for $f_{NL}$ is in agreement with the results of  \cite{Cai:2018dkf} for a general sharp transition. 
...
\Big)^2  \Big(\frac{H^2}{4 k^3M_P^2 \epsilon_e} \Big) \, .
\ea
...
\big \rangle|_{\tau_0} = - 4 M_P^2\,   \mathrm{Im}  \int d \tau \eta(\tau) \epsilon a^2
 \big \langle 
 \calR^*_{ p_1}(\tau_0) \calR^*_{ p_2}(\tau_0)  \calR^*_{ p_3}(\tau_0)  
 \calR_{p_1}(\tau)  \calR'_{p_2}(\tau) \calR'_{p_3}(\tau) \big \rangle \, .
\ea

%\ba \big \langle \calR_{\bf p_1}(\tau_0)  \calR_{\bf p_2}(\tau_0)  \calR_{\bf p_3}(\tau_0) \big \rangle& =& - 2  \mathrm{Im}  \int d \tau \big \langle {\bf H}_3 (\tau) \calR_{\bf p_1}(\tau_0) \calR_{\bf p_2}(\tau_0)  \calR_{\bf p_3}(\tau_0) \nonumber\\&=& - 4 M_P^2\,   \mathrm{Im}  \int d \tau \eta(\tau) \big \langle  \calR^*_{ p_1}(\tau_0) \calR^*_{ p_2}(\tau_0)  \calR^*_{ p_3}(\tau_0)  \calR_{p_1}(\tau)  \calR'_{p_2}(\tau) \calR'_{p_3}(\tau) \big \rangle  \nonumber\\ \ea
 ...
 \label{first-region}
 \big \langle \calR_{\bf p_1}(\tau_0)  \calR_{\bf p_2}(\tau_0)  \calR_{\bf p_3}(\tau_0) 
\big \rangle_{(1)} = - \frac{3(h-6)^2 (h+12)}{16 h^3}  \Big(\frac{H^2}{M_P^2 \epsilon_e} \Big) \Big(\frac{1}{p_1^3 p_2^3} +  \frac{1}{p_1^3 p_3^3} + \frac{1}{p_2^3 p_3^3} \Big) \, .
 \ea
On the other hand, from the second contribution for the period  $\tau_e < \tau < \tau_0$  we obtain
 \ba
 \label{second-region}
 \big \langle \calR_{\bf p_1}(\tau_0)  \calR_{\bf p_2}(\tau_0)  \calR_{\bf p_3}(\tau_0) 
\big \rangle_{(2)} =  \frac{3(h-6)^2 (h+6)}{8 h^3}  \Big(\frac{H^2}{M_P^2 \epsilon_e} \Big) \Big(\frac{1}{p_1^3 p_2^3} +  \frac{1}{p_1^3 p_3^3} + \frac{1}{p_2^3 p_3^3} \Big) \, .
 \ea
The non-Gaussianity parameter $f_{NL}$ is defined via 
\ba
 \big \langle \calR_{\bf p_1}(\tau_0)  \calR_{\bf p_2}(\tau_0)  \calR_{\bf p_3}(\tau_0) 
\big \rangle \equiv  \frac{5}{6} f_{NL} \Big( P_\calR (p_1, \tau_0) P_\calR (p_2, \tau_0) +
...
Since we are interested only to the modes which leave the horizon during the USR phase, we cut the loop integrals in the intervals $ q_i \leq q < q_e $ in which $q_i = -\frac{1}{\tau_i}$ and $q_e= - \frac{1}{\tau_e}$. The number of e-folds of the USR phase  $\Delta N= N(\tau_e) - N(\tau_i)$ is related to $q_i$ and $q_e$ via  
\ba
\label{delta-N-def}
...
$\bfq$ running in the loop yielding to two different contributions, which we denote by terms A and B as follows 
 \ba
 \langle \calR_{\bfp_{1}}(\tau_0) \calR_{\bfp_{2}}(\tau_0) \rangle|_{\mathrm{bulk}}  
 \equiv   (2 \pi)^3 \delta^3 (\bfp_1 + \bfp_2)  (A + B) \, ,
 \ea
 with
 \ba
 \label{A-term}
...
Note that as $p \ll q$, we have approximated $\bfp-\bfq \simeq -\bfq$ in the above integrals  to leading orders in $p$. 
 ...
 There are two different contributions in the bulk for both A and B terms: during the USR phase with  $\tau_i < \tau < \tau_e$ and after the USR phase $\tau_e < \tau < \tau_0$.
...
On the other hand,  during the second bulk phase, we approximately have  
 \ba
   \eta^2 - \frac{\eta'}{a H} \simeq \frac{18 (6+h) \tau^3}{h \tau_e^3} + O(\tau^5)\, .
 \ea
...
form and we will have additional  suppressions from the time derivatives of the mode function. Correspondingly, one expects the contribution from the final slow-roll region   
 ...
 \frac{9}{\tau^2} \big(\frac{\tau_e}{\tau} \big)^2  |\calR_q(\tau_e)|^2 \, .
 \ea
On the other hand, 
\ba
\label{Im1}
\mathrm{Im} \Big(  \calR_\bfp^*(\tau_0)^2  \calR_\bfp(\tau)^2 )   \Big) 
=\frac{1}{24} \frac{H^2}{M_P^4 \epsilon_i^2 p^3} \frac{\tau_e^3}{r^6 \tau^3}
...
After calculating the bulk contributions, now we calculate the localized terms
 induced by $\eta'$ at $\tau= \tau_e$. Again, we have two contributions from the source terms denoted by terms C and D. The term C is similar to term A and the term D is similar to term B  but 
 with the understanding that $\eta' = - h \delta (\tau -\tau_e)$. More specifically,
  \ba
 \label{C-term}
...
Summing the terms  $A+ B+ C+D$, the total contribution from the quartic Hamiltonian corresponding to the right panel of Fig. \ref{Feynman-fig} is obtained to be 
\ba
\label{quartic-power}
  \langle \calR_{\bfp_{1}}(\tau_0) \calR_{\bfp_{2}}(\tau_0) \rangle_{\Hb}  
 =  \frac{3}{8h} \big(  h^2 + 6h + 36
...
\ba
   ...
   - 8 M_P^4  \int \frac{d^3 \bfq}{(2 \pi)^3}  \int_{\tau_i}^{\tau_e} d \tau_1 
    ...
    \big( \delta (\tau_2) + 2 \beta (\delta \tau_2)\,  \big)  \Big] \, ,
\ea
 in which
 \ba
 ...
 \ea
Calculating the above integrals yields  the final result as follows
\ba
\label{cubic-power}
   \langle \calR_{\bfp_{1}}(\tau_0) \calR_{\bfp_{2}}(\tau_0) \rangle_{\Ha}  =  -\frac{27 (h-10)}{8h}  \big( \Delta N e^{6 \Delta N} \big)
 \frac{H^4  }{  2 \pi^2 M_P^4 \epsilon_i^2 p^3 }  \,   ( 2 \pi)^3 \delta^3 (\bfp_1 + \bfp_2) \, .
\ea 
As in the quartic case, we see a non-linear dependence on the parameter $h$. 
...
\ba 
\label{loop-power}
\Delta \calP(\bfp) \equiv    \frac{p^3}{2 \pi^2}   \langle \calR_{\bfp_{1}}(\tau_0) \calR_{\bfp_{2}}(\tau_0) \rangle_{\mathrm{1-loop}} =\frac{2}{ h} \big( 3 h^2 - 9 h + 378) (\Delta N e^{6 \Delta N}) \calP_{\mathrm{CMB}}^2 \, ,
\ea
...
$|h| \gg 1 $, the one-loop correction in power spectrum becomes arbitrarily large. Indeed, in this limit we would have $\frac{\Delta \calP}{\calP_{\mathrm{CMB}}} \simeq  6h (\Delta N e^{6 \Delta N}) \calP_{\mathrm{CMB}}$ so the amplitude of this ratio will easily become larger than unity. It is more useful to eliminate the parameter $h$ in this ratio in favour of the final slow-roll parameter $\epsilon_0= \epsilon(\tau_0)$.
Using Eq. (\ref{ep-N}) (or directly from the definition of $h$ in Eq. (\ref{h-def}) )
we have $| h| = 6 \sqrt\frac{\epsilon_0}{\epsilon_e}$  while $\epsilon_e = e^{-6 \Delta N} \epsilon_i$ 
so we obtain
\ba
\big| \frac{\Delta \calP}{\calP_{\mathrm{CMB}}}  \big| = 
36   \sqrt\frac{\epsilon_0}{\epsilon_i} 
(\Delta N e^{3 \Delta N}) \calP_{\mathrm{CMB}} \, .
\ea
Assuming that there is no large hierarchy between the CMB scale $\epsilon_i$ and the final value $\epsilon_0$, we see that the above ratio scales like $36 \, e^{3 \Delta N} \calP_{\mathrm{CMB}}$. For example, with $\Delta N \simeq 6$ the above  ratio 
becomes larger than unity and the perturbative approximation breaks down. 
%For example, for $\Delta N=1$, we need  $h\simeq -450$ while for $\Delta N=2$  we only need $h \simeq -16$ for this ratio to become  unity. 
Alternatively, Eq. (\ref{loop-power}) can be used to put an upper bound on the duration of the USR period, the number of e-fold  $\Delta N$. For example, for the standard sharp transition with $h=-6$ and $\epsilon_0= \epsilon_e$,  the ratio becomes order unity 
for $\Delta N \simeq 2.3$. A longer period of USR phase will amplify  the amplitude  of curvature perturbations such that eventually the one-loop corrections become comparable to the tree level. 
In either way, i.e. either by taking $|h|$ to be arbitrarily large or for a fixed value of 
$|h|$  considering a long enough period of USR phase, one would invalidate the perturbative theory as the one-loop contribution surpasses the tree level contribution.
 
The second comment is the issue concerning PBHs formation as debated in 
\cite{Kristiano:2022maq, Riotto:2023gpm, Riotto:2023hoz}. As argued in    \cite{Kristiano:2022maq} in order  to generate PBHs as candidate for dark matter from USR quantum fluctuations, one typically requires that  
$\calP(\tau_e) \simeq e^{6 \Delta N} \calP_{\mathrm{CMB}} \sim 10^{-2}$ or so. However, in order to trust the perturbative approximation, we need $\frac{\Delta \calP}{\calP_{\mathrm{CMB}}}  \ll1$. Combining this requirement with Eq. (\ref{loop-power}) we conclude that for sharp transition  $\calP(\tau_e) \ll 10^{-2} $ so the advocated mechanism to generate enough PBHs as the candidate for dark matter does not work in USR setup with a sharp transition.  

The third comment is that Eq.  (\ref{loop-power}) is obtained in the limit of sharp transition, corresponding to $h \leq -6$. A more realistic setup is when the transition from a USR phase to the final slow-roll phase happens mildly. As we discussed  in section \ref{bispectrum} for the case of bispectrum,  the effects of the transition on the final value of $f_{NL}$ were studied in details in \cite{Cai:2018dkf}.
It was demonstrated that the final value of $f_{NL}$ can be washed out if the transition is mild. The maximum value $f_{NL}=\frac{5}{2}$ happens only for the case of extreme sharp transition $|h| \gg 1 $ while for the standard  sharp transition with $h=-6$, one obtains a smaller value $f_{NL}=\frac{5}{8}$ as we demonstrated it explicitly  in section \ref{bispectrum}. However, for a mild transition with  small value of $|h|$, the  $f_{NL}$ parameter falls off to the order of the slow-roll parameters (while the  Maldacena consistency condition is still violated). We believe that a similar effects can happen for the one-loop corrections as well. If the transition takes place in a mild manner, then the leading order corrections proportional to $e^{6 \Delta N}$ will be washed out by the subsequent evolution of the mode function during the transition from the USR phase to the final slow-roll phase.  
...
of $20$ compared to that of \cite{Kristiano:2022maq, Kristiano:2023scm} in the limit where these two calculations have overlap (i.e. in the special case $h=-6$). 
...
\vspace{1cm}
 ...
 Xingang Chen, Junichi Yokoyama, Mahdiyar Noorbala, Jason Kristiano and Sina Hooshangi for useful  discussions and comments on the draft.   This research is  partially supported by the ``Saramadan" Federation of Iran.  
...
As explained in the main  text, the dominant contributions for the time integrals above  come from the USR phase as the mode functions are growing and the time derivatives are not slow-roll suppressed. During the USR phase $\eta=-6$ so it simply can be taken out of the time integrals. With this discussion in mind, let us start with Eq. (\ref{11-int-b}). After performing all the contractions and considering the limit that $| \bfp| \ll |\bfq|$, we obtain four different contributions in $\langle \calR_{\bfp_{1}}(\tau_0) \calR_{\bfp_{2}}(\tau_0) \rangle_{(1,1)}$ denoted by  
$A_i$ for $i=1...4$. For example, $A_1$ is given by  
 \ba
  \label{A1-a}
A_1 =  4 \eta^2  M_P^4 \int \frac{d^3 \bfq}{(2 \pi)^3}
 \int_{\tau_i}^{\tau_e} d \tau_1 \epsilon a^2 \int_{\tau_i}^{\tau_1} d \tau_2 \epsilon a^2
...
A_1=  4   M_P^4 \int \frac{d^3 \bfq}{(2 \pi)^3}
 ...
 $X_2 \propto p^{-2}$ with the subleading correction of order $p^{0}$. Since we have the large hierarchy $p \ll q$, we can safely neglect the contribution of $A_4$ in the following analysis. Intuitively speaking, $X_2$ has a time derivative $\calR'_p(\tau)$
...
Now we calculate   $\langle \calR_{\bfp_{1}}(\tau_0) \calR_{\bfp_{2}}(\tau_0) \rangle_{(2,0)} $. Performing all the contractions, we have four different contributions 
denoted by $B_i, i=1,...4$. For example, $B_1$ has the following form
 \ba
  \label{A1-a}
B_1 =  8 \eta^2  M_P^4 \int \frac{d^3 \bfq}{(2 \pi)^3}
 \int_{\tau_i}^{\tau_e} d \tau_1 \epsilon a^2 \int_{\tau_i}^{\tau_1} d \tau_2 \epsilon a^2
\calR_p(\tau_e)  \calR^*_p(\tau_e)  \calR_p(\tau_1)  \calR^*_p(\tau_2) 
\calR'_q(\tau_1)^2 \calR^{'*}_q(\tau_2)   \calR^{*}_q(\tau_2) ,
\ea  
...
\label{H3-int}
   ...
   - 8 M_P^4  \int \frac{d^3 \bfq}{(2 \pi)^3}  \int_{\tau_i}^{\tau_e} d \tau_1 
    ...
    \big( \delta (\tau_2) + 2 \beta (\delta \tau_2)\,  \big)  \Big] \, .
\ea
Note that in obtaining the above results we have neglected the contributions from the 
terms $A_4$ and $B_4$ which are subleading as mentioned previously. 
...
\ba
 \mathrm{Im}  \big[  \calR_p^*(\tau_0)  \calR_p(\tau)   \big]  = \frac{H^3 \tau_i^6}{12 M_P^2 \epsilon_i  h \tau_e^3 \tau^3 } \big( h \tau_e^3 + (6- h) \tau^3
 \big) \, ,
\ea
and
\ba
...
\ea
Using these expressions in $\delta$ and $\beta$ and plugging them in Eq. (\ref{H3-int}) we obtain  the correction from the cubic Hamiltonian as given in Eq. (\ref{cubic-power}). To perform the integrals we employ the Maple computational software. 
