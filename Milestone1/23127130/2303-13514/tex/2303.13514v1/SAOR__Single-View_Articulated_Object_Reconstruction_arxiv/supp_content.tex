...
\noindent{\bf Qualitative Results.} In Fig.~\ref{fig:qual1} we present additional qualitative results on quadruped categories from COCO~\cite{coco} and for birds from CUB~\cite{CUB}. 
We provide additional results showing full 360 degree predictions for multiple different categories on project website: \url{mehmetaygun.github.io/saor}
...
\input{figs/sup_qual1/sup_qual1}
...
When constructing our training datasets, we run a general-purpose animal object detector~\cite{beery2019efficient} and eliminate objects that if any of the following criteria hold: i) the confidence of the detection is less than 0.8, ii) the minimum side of the bounding box is less than 32 pixels, iii) the maximum side of the bounding box is less than 128 pixels, and iv) there is no margin greater than 10 pixels on all sides of the bounding box. 

We then automatically extract segmentation masks using PointRend~\cite{kirillov2020pointrend} pre-trained on COCO~\cite{coco}. As PointRend estimates multiple segmentation masks for each instance present in the image, we pick the segmentation mask that overlaps the most with the detected bounding box from the general-purpose detector. 
...
Visualization of cluster centers of various animals can be found in Figure~\ref{fig:clusters}.
...
\begin{table}
    ...
    \toprule
          ...
          Linear (512,512) & $\phi_{im}$  & $l_{z}$&  512 \\
          2 x Linear (512,128) &  $l_{x} + l_{z}$  & $L$ &  $N\times128$\\
...
\begin{table}
    ...
    \toprule
          ...
          Linear (3,512) & $S^{\circ}$  & $l_{x}$ &  $N\times512$\\
          Linear (512,512) & $\phi_{im}$  & $l_{z}$&  512 \\
          Linear (512,128) &  $l_{x} + l_{z}$  &  &  $N\times128$ \\
          Linear (128,128) &  $l_{x} + l_{z}$  & $L$ &  $N\times128$ \\
          Linear (128,K) &  $L$  & $W$ &  $N \times K$ \\
          K x Linear (512, 9) &  $\phi_{enc}$  & $\pi$ &  $K \times 9$ \\
          \bottomrule
    \end{tabular}
    \caption{Architecture details of our Articulation Net $f_{a}$. $K$ is the number of parts and $N$ is the number of vertices.}       \label{tab:arch_art}
\end{table}
...
\begin{table}
    ...
    \toprule
          ...
          Upsample + Conv2D & $L_{up}$  &  &  $256\times8\times8$ \\
          Upsample + Conv2D &  &  &  $128\times16\times16$ \\
          Upsample + Conv2D &  &  &  $64\times32\times32$ \\
          Upsample + Conv2D &  &  &  $32\times64\times64$ \\
          Upsample + Conv2D &  &  &  $16\times128\times128$ \\
          Conv2D &  & T &  $3\times128\times128$ \\
...
\begin{table}
    ...
    \toprule
          Layer & Input & Output & Dim  \\ \midrule
          1 x Linear (512,128) & $\phi_{im}$  & $L$ &  128\\
          C x Linear (128,6) & $L$  & $\bm{r}_{p},\bm{t}_{p}$ &  128\\
          Linear (128,C) & $L$  & $\alpha$ &  128\\
          \bottomrule
    \end{tabular}
    \caption{Architecture details of our Pose Net $f_{p}$. C is the number of cameras, and $\alpha$ is the associated scores for each camera as in~\cite{wu2022magicpony}.} 
    \label{tab:arch_pose}
\end{table}
...
\begin{equation}
    \mathcal{L}_{swap} = \lambda_{swap} \mathcal{L}_{mask}(I, I^{sw}) + \mathcal{L}_{rgb}(I, I^{sw}).
\end{equation} 

Finally, we also employ part regularization on the part assignment matrix $W$ to encourage equal-sized parts,
\begin{equation}
    \mathcal{L}_{part} = \lambda_{part}\sum_{k}^{K} \left((\sum_{i}^{N} W_{i,k}) - N/K\right)
\end{equation}where $N$ is the number of vertices in the mesh and $K$ is the number of parts. We also apply 3D regularization on the 3D shape, $\mathcal{L}_{smooth} = \lambda_{smooth}\sum_{}LS $, where $L$ is the laplacian of shape $S$ and $\mathcal{L}_{normal}$ which is defined below,

\begin{equation}
    \mathcal{L}_{normal} = \lambda_{normal} \sum_{fi,fj \in \Omega} 
    1 - \frac{\bf{n_{i}}.\bf{n_{j}}}{||\bf{n_{i}}|| . ||\bf{n_{j}}||} 
\end{equation} here $\Omega$ is the set that includes all the pairs of faces that are neighbours, and $n_{i}$ is the normal vector of the face $f_{i}$.
...
\end{align}
We present the weights used in our experiments for each loss in Table~\ref{tab:hyperparams}.


\subsection{Training}
We trained the Bird and Horse models from scratch. 
Each was trained for 500 epochs. 
In the first 100 epochs we only learn deformation, and then enable articulation afterward. In the case of other non-horse quadrupeds, we finetune models for about 200 epochs starting from the horse model.
We utilize Adam~\cite{kingma2014adam} with a fixed learning rate for optimizing our networks. 
We present all the hyperparameters in Table~\ref{tab:hyperparams}.
