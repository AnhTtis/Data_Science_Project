...
\begin{table}[h!]
    ...
    \caption{The number of rows in the $M$ and $N$ matrices are given as a function of $|X|$. Notably, in the case of $|X|=15$, the number of rows of $M$ given in this table is an underapproximation.}
...
\end{equation}
Since our test already imposes $q\geq 0$, Equation \ref{partial} is equivalent to asking that $p(x,A)=0$ when $A$ contains some element $y \in U(x)$. While encoding an underlying order obviously increases the computation time of our test, we note that we only need to add one row to our $N$ matrix for each $x$ with nonempty $U(x)$.
