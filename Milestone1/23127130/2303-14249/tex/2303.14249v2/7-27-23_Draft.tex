...
\title{On Graphical Methods in Stochastic Choice\footnote{I am thankful to Christopher Chambers, Yusufcan Masatlioglu, and Alexandre Poirier for helpful discussions. I am particularly thankful to Peter Caradonna for fruitful conversations during which the idea behind this paper was developed.\\
...
We offer two applications of our main result. Our first application is a new characterization of random utility for incomplete data sets. The classic characterization of random utility for incomplete data sets is from \citet{mcfadden1990stochastic}. Their method asks that there is some probability distribution over preferences which induce the observed choice probabilities. One way to get their axiom is to look at the alternative linear program of this existence question. One problem with this method is that it produces an axiom that references the underlying representation. An alternative way of posing the stochastic rationality question is by asking if there is an extension of the observed choice probabilities to a complete domain which satisfy the conditions of \citet{falmagne1978representation}. Initially, this method may appear to give messy conditions as asking for an extension of choice probabilities is a statement about probabilities and asking that they satisfy the conditions of \citet{falmagne1978representation} is a statement about the M\"{o}bius inverse of probabilities. Our main insight is that you can write the extension problem using only the M\"{o}bius inverse of choice probabilities using the flow conditions from our main result. Similar to \citet{mcfadden1990stochastic}, we then get our axiom by looking at the alternative linear program of the restatement of the stochastically rational extension linear program.

Our second application consider the hypothesis test for random utility developed in \citet{kitamura2018nonparametric}. \citet{smeulders2021nonparametric} shows that this testing problem is NP-hard and much of this computational difficulty arises due to the computation of a given matrix. We use our main result to develop an equivalent hypothesis test for random utility. Both our test and the test of \citet{kitamura2018nonparametric} require us to minimize a quadratic program subject to non-negative constraints. Further, both of these tests require the construction of a large matrix before minimization of the quadratic program. However, while the matrix in our test and the matrix in the test of \citet{kitamura2018nonparametric} have the same number of rows, the number of columns in the test of \citet{kitamura2018nonparametric} scales at a rate of $|X|!$ while the number of columns in our test scales at a rate of $|X|2^{|X|-1}$. This means that the matrix in our test is much smaller than the matrix in the test of \citet{kitamura2018nonparametric} once there are five or more alternatives.

A common theme between our two applications is that we bring graph theoretic tools to incomplete domain problems. Up until now, these tools have been almost solely used to study complete domain problems. Part of our contribution is showing how these tools can be extended to incomplete domains and can actually provide improvements over current techniques on these incomplete domains.
...
While the value of a cut rewards an edge for going from $S$ to $T$ and for going from $T$ to $S$, the net flow through a cut rewards an edge for going from $S$ to $T$ but punishes an edge for going from $T$ to $S$. Consider a cut and a path. This path could in theory pass between $S$ and $T$ multiple times and thus be counted multiple times in the value of the cut. By considering the net flow through a cut, we are able to deal with this double counting of paths.
...
In the next part of the proof we show that inflow equals outflow is equivalent to constant cuts. To show that inflow equals outflow implies constant cuts, we show that, when inflow equals outflow holds, we can always completely decompose the flow diagram into a flow assignment. By this, we mean that you can always find a flow assignment which assigns flow at an edge equal to the edge weight of that edge for all edges. We call such a flow assignment a flow decomposition. This means that we can calculate the edge weights of any edge by calculating the flow through that edge from a flow decomposition. We then consider any cut of the flow diagram. Since the net flow through a cut counts every path at least once and avoids double counting of paths and using the fact that we can calculate an edge weight from flows through that edge, the net flow through any cut is equal to the value of a flow decomposition. Since the total flow of a flow decomposition is constant, this gives us constant cuts.

To show that constant cuts implies inflow equals outflow, we consider two specific cuts. The first cut we consider is given by $S=\{A \subseteq X|n\leq |A| \}$ and the second cut we consider is $S' = \{A \subseteq X|n \leq |A|\} \setminus B$ where $|B|=n$. When calculating the net flows through these two cuts, each edge leaving $B$ is counted for $S$ but not for $S'$ and each edge going into $B$ is counted for $S'$ but not for $S$. This means that the difference between the net flow through these two cuts is given by $\sum_{x \in A}g(x,B) - \sum_{y \in X\setminus A}g(y,B \cup \{y\})$. Since $f$ satisfies constant cuts, this difference is zero and thus $f$ satisfies inflow equals outflow.
...
\section{Two Applications to Random Utility}

\subsection{Axiomatics on an Incomplete Domain}
...
\begin{align}
    \sum_{A \subseteq A'} q(x,A) = p(x,A) \text{ } \forall A \in \mathcal{X}, x\in A \label{linconsist}\\
    ...
    q \geq 0 \label{linnonneg}
\end{align}

In the above linear program, we use $q$ to represent the M\"{o}bius inverse of a potential full domain random choice rule. Equation \ref{linconsist} encodes that this potential full domain random choice rule must agree with our observed random choice rule on observed choice sets. We use Equation \ref{lininflow} to encode the inflow equals outflow axiom and use Equation \ref{lininitial} to encode the condition from Corollary \ref{srcr}. From Theorem \ref{falmagne}, we know that a full domain random choice rule is stochastically rational if and only if the M\"{o}bius inverse of choice probabilities is non-negative. This condition implies $\sum_{A\subseteq B}q(x,B)\geq 0$, the condition from Corollary \ref{rcr}. This means that the condition $q \geq 0$ encodes both stochastic rationality as well as the last condition necessary to ensure that the potential $q$ is in fact the M\"{o}bius inverse of some full domain random choice rule. We can now obtain the alternative linear program through Farkas's Lemma and some minor manipulation leaves us with our condition in Theorem \ref{rumme}.

\subsection{A Faster Hypothesis Test}

In this section, we consider the hypothesis test developed by \citet{kitamura2018nonparametric} (henceforth KS) to test the random utility model with real data. \citet{smeulders2021nonparametric} shows that this hypothesis is NP-hard and uses a column generation approach to develop computational improvements over the base test developed by KS. Similar to \citet{smeulders2021nonparametric}, we offer a computational improvement over KS. However, instead of improving the computation time by using an inner approximation of the quadratic program developed by KS (i.e. the column generation approach), we improve the test of KS by developing an alternative but equivalent quadratic program which has vastly fewer constraints than the KS quadratic program. While KS works in the price-wealth domain, they show that their problem is equivalent to working in a specific incomplete domain in the abstract setup. As such, similar to \citet{smeulders2021nonparametric}, we consider the test of KS in the abstract domain without loss.

We begin by reviewing the hypothesis test developed by KS. In the last section, we noted that Theorem \ref{mcfadden} follows from the alternative linear program of \begin{align}
    rM=p \label{KSlin}\\
    r\geq 0 \label{KSpos}\\
    \sum_{i}r_i=1 \label{KSdist}
\end{align} where this $M$ matrix encodes the choices induced by every linear order over $X$, $p$ represents observed choice probabilities, and $r$ corresponds to some probability distribution over over linear orders. The basis for the test of KS is given by Equations \ref{KSlin}-\ref{KSdist}. KS shows that Equation \ref{KSdist} can be dropped from the above linear program and the existence of a solution does not change. KS then notes that there exists a solution to Equations \ref{KSlin} and \ref{KSpos} if and only if \begin{align}\label{KSquad}
    \min_{r\geq 0} (rM-p)^T \Omega (rM-p)
\end{align}
is equal to zero where $\Omega$ is some positive definite matrix and the superscript $T$ denotes the transpose of a matrix. KS then develops techniques for testing this quadratic program and, more generally, for conic shape constraints. As mentioned prior, the computation of this test statistic is computationally burdensome, and \citet{smeulders2021nonparametric} shows that most of this burden arises from computing the $M$ matrix. Our goal in this section is to develop an alternative quadratic program which is both amenable to the general tools developed in KS and is much less computationally burdensome.

Our approach builds on the linear program described in Equations \ref{linconsist}-\ref{linnonneg}. Note that this linear program is already in a form that is amenable to the tools developed in KS. Specifically, we have a set of equality constraints based off of the variable $q$ and our sole inequality constraint asks that $q$ be non-negative. However, we can further reduce this linear program in order to improve computation times. Consider the following linear program.
\begin{align}
    \sum_{A \subseteq A'} q(x,A) = p(x,A) \text{ } \forall A \in \mathcal{X}, x\in A \label{impconsist}\\
    \sum_{x \in A}q(x,A) = \sum_{z \not \in A} q(z,A\cup\{z\}) \text{ }\forall A \in (2^X \setminus\{X,\emptyset\})\setminus \mathcal{X} \label{impinflow}\\
    \sum_{x \in X} q(x,X)=1 \text{ if } X \not \in \mathcal{X}\label{impinitial}\\
    q \geq 0 \label{impnonneg}
\end{align}

\begin{thm}\label{hypothesistest}
    There exists a solution to Equations \ref{linconsist}-\ref{linnonneg} if and only if there exists a solution to Equations \ref{impconsist}-\ref{impnonneg}.
\end{thm}

While Equations \ref{linconsist}-\ref{linnonneg} and Equations \ref{impconsist}-\ref{impnonneg} appear quite similar, there is one main difference between these linear programs. This difference comes when comparing Equation \ref{lininflow} with Equation \ref{impinflow} and Equation \ref{lininitial} with Equation \ref{impinitial}. Equation \ref{lininflow} is asked to hold for every subset of $X$ whether or not data is observed at that set. On the other hand, Equation \ref{impinflow} is only asked to hold at every subset of $X$ which is not observed. Similarly, Equation \ref{lininitial} is always asked to hold, while Equation \ref{impinitial} is only asked to hold when $X$ is not observed in the data. The intuition for the equivalence is as follows. Equations \ref{lininflow} and \ref{lininitial} enter into our linear program in order to guarantee that our extension problem induces a full domain random choice rule. In other words, the role of these equality constraints is to ensure that probabilities sum to one. However, on choice sets that we observe in the data, we already know that probabilities sum to one. Equations \ref{impinflow} and \ref{impinitial} take advantage of this observation by dropping the equalities in Equations \ref{lininflow} and \ref{lininitial} for choice sets that are observed in the data.

Given a realized data set, we can encode the linear program of Equations \ref{impconsist}-\ref{impnonneg} via a matrix $N$ and a vector $l$.
\begin{align}
    Nq = l \label{impmat} \\
    q \geq 0 \label{impmatnonneg}
\end{align}
Following the same steps as KS, it is easy to show that there exists a solution to Equations \ref{impmat} and \ref{impmatnonneg} if and only if 
\begin{equation}\label{impquad}
    \min_{q\geq 0}(Nq-l)^T \Omega (Nq-l)
\end{equation}
is equal to zero. Equation \ref{impquad} obviously takes the same form as Equation \ref{KSquad} and thus the tools developed in KS can be used to test Equation \ref{impquad}.



Our goal in this section is not just to develop an alternative approach to the testing problem of KS, but rather it is to develop a faster testing approach. Much of the computational burden in the test of KS comes from forming the $M$ matrix in Equation \ref{KSlin}. The analogue of the $M$ matrix in our setup is the $N$ matrix in Equation \ref{impmat}. Given that the size of our $N$ matrix is largest when $\mathcal{X}=2^X\setminus\{\emptyset\}$, we now compare the sizes of $M$ and $N$ when $\mathcal{X}=2^X\setminus\{\emptyset\}$. We first note that both $M$ and $N$ have the same number of rows, one for each pair $(x,A)$ with $x\in A \subseteq X$. As such, it is sufficient to compare the number of columns in each of these matrices. The matrix $M$ has one column for each linear order of $X$. This is given by $|X|!$. The matrix $N$ has one column for each pair $(x,A)$ with $x\in A \subseteq X$. This is given by $\sum_{i=1}^{|X|} i \binom{|X|}{i}=|X|2^{|X|-1}$. When $|X| \leq 4$, $M$ is smaller than $N$, but when $|X|\geq 5$, $N$ is always smaller than $M$. In Table \ref{table:matrixsize} we calculate the number of columns in $M$ and $N$ for a few sizes of $X$.

\begin{table}[h!]
    \centering
    \begin{tabular}{|c|c|c|}
        \cline{1-3}
        $|X| $& $M$ columns & $N$ columns \\
        \cline{1-3}
        3 & $6$ & $12$ \\
        \cline{1-3}
        4 & $24$ & $32$\\
    ...
    \caption{The number of columns in the $M$ and $N$ matrices are given as a function of $|X|$. Notably, in the case of $|X|=15$, the number of columns of $M$ given in this table is an underapproximation.}
    \label{table:matrixsize}
\end{table}

While we develop our test for the general abstract domain, we note that our test can be customized to suit specialized domains. First, using the results of \citet{mcfadden2005revealed}, we can further reduce the computational burden of our test in a specific limited domain. \citet{mcfadden2005revealed} notes that the conditions of Theorem \ref{falmagne} are sufficient on a limited domain so long as for every set $A$ in our data, our data contains every set $B$ such that $A \subseteq B \subseteq X$. For our test, this means that for every set $A$ for which there is no set $B$ in our data set with $A \subseteq B$, we can drop every constraint referring to set $A$ in Equations \ref{impconsist}-\ref{impnonneg}. Second, suppose we are considering a domain that is naturally endowed with a partial order. Take as a motivating example the greater than or equal to relation that endows the price-wealth domain studied in KS. In this domain, not only do we want consistency with random utility, but we also want to encode that dominated alternatives are never chosen. For each $x \in X$, let $U(x)$ denote the set of alternatives dominating $x$ according to the underlying partial order. Then for each $x$ with nonempty $U(x)$, we can encode the partial order using the following equality constraint.
\begin{equation}\label{partial}
    \sum_{A:x \in A, A \cap U(x) \neq \emptyset}q(x,A)=0
\end{equation}
Since our test already imposes $q\geq 0$, Equation \ref{partial} is equivalent to asking that $p(x,A)=0$ when $A$ contains some element $y \in U(x)$. While encoding an underlying order obviously increases the computation time of our test, we note that we only need to add one column to our $N$ matrix for each $x$ with nonempty $U(x)$.
...
\subsection{Proof of Theorem \ref{hypothesistest}}
As the constraints in Equations \ref{impconsist}-\ref{impnonneg} are a subset of the constraints in Equations \ref{linconsist}-\ref{linnonneg}, it follows immediately that if Equations \ref{linconsist}-\ref{linnonneg} have a solution then Equations \ref{impconsist}-\ref{impnonneg} have a solution. We now show the other direction. Let $q$ be a solution to Equations \ref{impconsist}-\ref{impnonneg}. We proceed by induction on $A$.

The base case of our induction is when $A =X$. Note that either $X \in \mathcal{X}$ or $x \not \in \mathcal{X}$. If $X \not \in \mathcal{X}$, then Equations \ref{impconsist}-\ref{impnonneg} coincide with Equations \ref{linconsist}-\ref{linnonneg} at $X$. If $X \in \mathcal{X}$, then we observe choice probabilities at $X$, and so $q(x,X) \geq 0$ for all $x \in X$ as $p(x,X)=q(x,X)$ by Equation \ref{impconsist}. Since probabilities sum to one, this gives us $\sum_{x \in X} p(x,X)=1$ which is exactly Equation \ref{lininitial}. Thus Equations \ref{lininitial}-\ref{linnonneg} hold at $X$. Now fix a set $A \subsetneq X$ and suppose that Equations \ref{linconsist}-\ref{linnonneg} hold for every set $B$ such that $A \subsetneq B$. This is our induction hypothesis. There are two cases; either $A \in \mathcal{X}$ or $A \not \in \mathcal{X}$. If $A \in \mathcal{X}$, then we know that $\sum_{x\in A}p(x,A)=1$ as choices are observed at $A$. Further, by our induction hypothesis $\sum_{B \in B'}q(x,B')=p(x,B)$. It then follows that $q$ is the M\"{o}bius inverse of $p$ on the domain $\{B|A\subseteq B\}$. This gives us that $p(x,A)=\sum_{A \subseteq B}q(x,B)$. As $q(x,A)\geq 0$ is assumed in Equation \ref{impnonneg}, all we need to show is that Equation \ref{lininflow} holds at $A$. Since $p(\cdot)$ is set constant on $\{B|A\subseteq B\}$, it follows from (the construction used in) Theorem \ref{maintheorem} that Equation \ref{lininflow} holds at $A$. Thus, if $A \in \mathcal{X}$, then Equations \ref{linconsist}-\ref{linnonneg} hold at set $A$. Now suppose that $A \not \in \mathcal{X}$. When $A \not \in \mathcal{A}$, Equations \ref{impconsist}-\ref{impnonneg} coincide with Equations \ref{linconsist}-\ref{linnonneg} at set $A$. Thus by induction, if Equations \ref{impconsist}-\ref{impnonneg} hold, then Equations \ref{linconsist}-\ref{linnonneg} hold and so we are done.
