\documentclass[aps,amsmath,preprint,superscriptaddress,nofootinbib]{revtex4-1}
\usepackage{graphicx}
\usepackage{bm}
\usepackage{color}
...
\title{More on Dark Topological Defects}
...
One possibility is to identify the dark photon model with the St\"ukelberg model (see Ref.~\cite{Ruegg:2003ps} for a review).
...
\footnote{Although the St\"ukelberg
vector boson does not cause the unitarity when it couples to only conserved current, other interactions such as self-couplings the unitarity violation.}
...
-\frac{1}{2}\sum_{i=1,2}D_{\mu}\suphi_i^a D^{\mu}\suphi_i^a
-V(\suphi_1,\suphi_2) + \mathcal{L}_\theta + \mathcal{L}_{\mathrm{mix}}, \\
\label{eq:Ltheta}
\mathcal{L}_\theta &= -\frac{e^2\theta}{32\pi^2}F_{\mu\nu}\tilde{F}^{\mu\nu} - \frac{e_\mD^2\theta_\mD}{32\pi^2}F_{\mD\mu\nu}^a\tilde{F}_\mD^{a\mu\nu} \\
\label{eq:Lmix}
\mathcal{L}_{\mathrm{mix}} &= -\frac{c_1\suphi_1^a}{2\Lambda} {F_\mD}^{a}_{\mu\nu}F^{\mu\nu}-
  \frac{c_2\suphi_1^a}{16\pi^2\Lambda} {F_\mD}^{a}_{\mu\nu}\tilde{F}^{\mu\nu}\ .
\end{align}
Here, $F_{\mu\nu}$ and $F^a_{\mD\mu\nu}$
($a = 1,2,3$)
...
We introduced two SU$(2)_\mD$ adjoint scalar fields $\suphi_{1,2}^{a}$ ($a = 1,2,3$).
We call the $\uone$ and $\suD$ gauge coupling constants $e$ and $e_\mD$.
The covariant derivatives of $\suphi$'s are given by,
\begin{equation}
    D_\mu \bm{\suphi}_{1,2} = \partial_\mu \bm{\suphi}_{1,2} - i e_\mD [\bm{A}_{\mD\mu},\bm{\phi}_{1,2}]\ ,
\end{equation}
where $\bm{\phi}_{1,2} = \phi_{1,2}^a t^a$ 
and $\bm{A}_{\mD\mu} = A_{\mD\mu}^a t^a$
...
The scalar potential of $\suphi_{1}^a$ and $\suphi_{2}^a$ is assumed to be
\begin{equation}
\label{eq:potential}
	V(\phi_1,\phi_2) = \frac{\lambda_1}{4}
	(\suphi_1\cdot\suphi_1-v_1^2)^2 + \frac{\lambda_2}{4}(\suphi_2\cdot\suphi_2-v_2^2)^2 + \frac{\kappa}{2}(\suphi_1\cdot \suphi_2)^2 \ ,
\end{equation}
where $\suphi_i\cdot\suphi_j= \suphi_i^a\suphi_j^a$.
For simplicity, we omit terms such as $(\suphi_1\cdot\suphi_1)(\suphi_2\cdot\suphi_2)$.
...
At the vacuum, $\suphi_1^a$ takes the trivial configuration
\begin{equation}
\label{eq:VEV1}
   \langle{\suphi_1^a}\rangle = v_1 \delta^{a3}\ ,
\end{equation}
with which $\mathrm{SU}(2)_\mD$ is broken down to $\mathrm{U}(1)_\mD$.
The remaining $\mathrm{U}(1)_\mD$ symmetry corresponds to the SO(2) symmetry around the $a=3$ axis of $\mathrm{SO}(3)\simeq\mathrm{SU}(2)_\mD$ vectors $\suphi_{1,2}^a$.

Below the $\suD$ breaking scale,
a U(1)$_\mD$ charged field $\uphi$ can be formed out of $\suphi_2^a$ as
\begin{equation}\label{eq:u1phi}
    \uphi = \frac{1}{\sqrt{2}}(\suphi_2^1-i\suphi_2^2)\ .
\end{equation}
For $\kappa>0$, 
the last term of the potential \eqref{eq:potential} 
lifts the $a=3$ component of $\suphi_2$ and the VEV of $\suphi_2^a$ is required to be orthogonal to 
$\langle\suphi_1^a\rangle$. 
As a result, $\langle\suphi_2^a\rangle$ takes a value in the $(\suphi_2^1,\suphi_2^2)$ plane, i.e.,
\begin{equation}\label{eq:VEV2}
    \langle\suphi_2^a\rangle = v_2 \delta^{a1}\ , 
\end{equation}
or 
\begin{equation}
    \label{eq:U1VEV}
...
\begin{align}
\label{eq:heg}
\suphi_1^a=v_1 H(r)\displaystyle{\frac{x^a}{r}}\ , \quad
A_{\mD 0}^a = 0 \ ,\quad
A^{a}_{\mD i}=
...
\begin{equation}
\label{eq:effectiveF}
F_{\mD\mu\nu} := \frac{1}{v_1}\suphi_1^a F^{a}_{\mD\mu\nu}
\end{equation}
...
Table \ref{tab:U1symmetric} summarizes the results in this section.

\newcommand{\chargesize}{0.15\textwidth}
\newcommand{\echargefig}{\includegraphics[height=\chargesize]{images/electric-charge.jpg}}
\newcommand{\mchargefig}{\includegraphics[height=\chargesize]{images/monopole.jpg}}
\newcommand{\eechargefig}{\includegraphics[height=\chargesize]{images/double-electric-charge.jpg}}
\newcommand{\dyonfig}{\includegraphics[height=\chargesize]{images/dyon.jpg}}
\begin{table}
    \centering
    \begin{ruledtabular}
    \begin{tabular}{c|ccc}
        \multirow{2}*{Dark Sector} & \multicolumn{3}{c}{QED} \\
        & Kinetic Mixing & Magnetic Mixing & Both \\
        \colrule
        \echargefig & \echargefig & & \echargefig \\
        \mchargefig  & & \echargefig & \echargefig \\
        \dyonfig  & \echargefig & \rotatebox[origin=c]{30}{\echargefig} & \eechargefig
    \end{tabular}
    \end{ruledtabular}
...
the residual global $\uone[\mD]$ symmetry around $\suphi_1$,
\begin{align}
\label{eq:U1D trans}
    \delta A^a_{\mD\mu} = -\frac{1}{e_\mD v_1}D_\mu \suphi_1^a\ , \quad
    \delta A_\mu = 0 \ ,\quad
    \delta \suphi_1^a = 0\ .
\end{align}
...
Before closing this section, let us comment on the effects of the $\theta$-terms $\mathcal{L}_{\theta}$ to the equations of motion.
...
(see also Ref.~\cite{Coleman:1982cx}).%
\footnote{
Strictly speaking, 
...
\begin{align}
    J_\mathrm{\uphi}^i= i \uphi D^i\uphi^\dagger - i \uphi^\dagger D_i \uphi = 2v^2 n \frac{\epsilon^{ij}x^j}{\rho^2}h^2(f-1)\ ,
\end{align}
through the kinetic mixing.
...
$\suphi_2$ prefers to be orthogonal to $\suphi_1$ because of the $\kappa$ term in the potential \eqref{eq:potential}.
However, such a configuration of $\phi_2$ with 
a constant amplitude, $|\phi_2| = v_2$,
is impossible due to the
Poincar\'e--Hopf (hairy ball) theorem
around the monopole solution \eqref{eq:heg}.
Rather, $|\phi_2|$ should vanish at some points at $r\to \infty$ and strings must extend in those directions.
...
that is,
\begin{align}
    \bm{\suphi} &\to  \bm{\suphi}_{N,S} = g_{N,S} \bm{\suphi} g_{N,S}^\dagger\ , \\
\bm{A}_{\mD i} &\to  \bm{A}_{\mD N,S\,i} = g_{N,S} \bm{A}_{\mD i}g_{N,S}^\dagger +
...
This configuration is consistent with the Poincar\'e--Hopf theorem since $\phi^a_2 = 0$ at the center of the string.%
\footnote{This configuration is not static, and the dark monopole is pulled in the negative $z$ direction.}

\begin{figure}[tbp]
 \begin{minipage}{0.45\linewidth}
\includegraphics[width=0.3\linewidth]{images/string2Flux.pdf}
 \end{minipage}
 \begin{minipage}{0.45\linewidth}
\includegraphics[width=0.23\linewidth]{images/beadFlux.pdf}
\end{minipage}
...
\begin{figure}[tbp]
\includegraphics[width=0.23\linewidth]{images/pseudomonopole.pdf}
...
We show the summary of the QED field strengths that the QED charged particle feel in Table \ref{tab:U1broken}.

\newcommand{\yukawafig}{\includegraphics[height=\chargesize]{images/yukawa.jpg}}
\newcommand{\beadheight}{0.2\textwidth}
\newcommand{\raiseheight}{0.025\textwidth}
\begin{table}[tbp]
    \centering
    \begin{ruledtabular}
    \begin{tabular}{c|ccc}
        ...
        & Kinetic Mixing & Magnetic Mixing & Both \\
        \colrule
        \yukawafig & \yukawafig & & \yukawafig \\
        \includegraphics[height=\beadheight]{images/beads.png} & \includegraphics[height=\beadheight]{images/pseudo_monopole.png} & \raisebox{\raiseheight}{\echargefig} & \includegraphics[height=\beadheight]{images/pseudo_dyon.png} \\
        \includegraphics[height=\beadheight]{images/dyonic_beads.png}  & \includegraphics[height=\beadheight]{images/pseudo_yukawa_dyon.png} & \raisebox{\raiseheight}{\echargefig}
        & \includegraphics[height=\beadheight]{images/pseudo_full_dyon.png}
    \end{tabular}
    \end{ruledtabular}
...
\begin{align}
    ...
    &= \int \dd[3]{x} \pqty{
        -F_{\mD}^{a0i}
        + \epsilon \frac{\suphi_1^a}{v_1} F^{0i}
        - \frac{e_\mD^2 \theta_\mD}{8\pi^2} \tilde{F}_\mD^{a0i}
        - \frac{\tM}{8\pi^2}\frac{\suphi_1^a}{v_1}\tilde{F}^{0i}
    } \pqty{-\frac{1}{e_\mD v_1}D_i \suphi_1^a}\ .
\end{align}

The contribution from the kinetic term is
\begin{equation}
    \frac{1}{e_\mD v_1}\int \dd[3]{x} F_{\mD}^{a0i} D_i \suphi_1^a
    = \frac{1}{e_\mD}\int \dd[2]S_i \pqty{\frac{\suphi_1^a}{v_1} F_{\mD}^{a0i}}
       - \frac{1}{e_\mD v_1}\int \dd[3]{x} \suphi_1^a D_i F_{\mD}^{a0i}\ .
\end{equation}
The surface integral reduces to $\qcharge[\mD]/e_\mD$.
The integrand of the other term can be written as
\begin{align}
    \suphi_1^a D_i F_{\mD}^{a0i} &= \suphi_1^a D_\mu F_{\mD}^{a0\mu} \\
    \label{eq:noether1}
    &= \suphi_1^a \bqty{\epsilon D_\mu \pqty{\frac{\suphi^a_1}{v_1}F^{0\mu}} - \frac{\tM}{8\pi^2}D_\mu \pqty{\frac{\suphi^a_1}{v_1}\tilde{F}^{0\mu}}}\ ,
\end{align}
where we used the equation of motion for $A_{\mD\mu}^a$
\begin{equation}
    \label{eq:SU2EoM}
    D_\mu F_\mD^{a\mu\nu} - \epsilon D_\mu \pqty{\frac{\suphi^a_1}{v_1}F^{\mu\nu}} + \frac{\tM}{8\pi^2}D_\mu \pqty{\frac{\suphi^a_1}{v_1}\tilde{F}^{\mu\nu}} = e_\mD\epsilon^{abc}\suphi_1^b D^\nu \suphi_1^c\ .
\end{equation}

The contribution from the kinetic mixing term is
\begin{align}
     -\frac{\epsilon}{e_\mD v_1}\int \dd[3]{x} \frac{\suphi_1^a}{v_1} F^{0i}D_i \suphi_1^a
     &= -\frac{\epsilon}{e_\mD}\int \dd[2]{S}_i \pqty{\frac{\suphi_1^a\suphi_1^a}{v_1^2}F^{0i}}
     +\frac{\epsilon}{e_\mD v_1}\int \dd[3]{x} D_i \pqty{\frac{\suphi_1^a}{v_1} F^{0i}}\suphi_1^a\ .
\end{align}
...
\begin{align}
        \frac{e_\mD \theta_\mD}{8\pi^2 v_1}\int \dd[3]{x}  \tilde{F}_\mD^{a0i}D_i\suphi_1^a
        &= \frac{e_\mD \theta_\mD}{8\pi^2 }\int \dd[2]{S}_i \pqty{\frac{\suphi_1^a}{v_1}\tilde{F}_\mD^{a0i}} - \frac{e_\mD \theta_\mD}{8\pi^2 v_1}\int \dd[3]{x}  \suphi_1^aD_i\tilde{F}_\mD^{a0i} \\
...
\begin{equation}
    \frac{1}{e_\mD v_1}\frac{\tM}{8\pi^2}\int \dd[3]{x} \frac{\suphi_1^a}{v_1}\tilde{F}^{0i} D_i\suphi_1^a 
    = \frac{1}{e_\mD}\frac{\tM}{8\pi^2}\int \dd[2]{S}_i \pqty{\frac{\suphi_1^a\suphi_1^a}{v_1^2}\tilde{F}^{0i}}- \frac{1}{e_\mD v_1}\frac{\tM}{8\pi^2}\int \dd[3]{x} \suphi_1^a D_i \pqty{\frac{\suphi_1^a}{v_1} \tilde{F}^{0i}}\ .
\end{equation}
