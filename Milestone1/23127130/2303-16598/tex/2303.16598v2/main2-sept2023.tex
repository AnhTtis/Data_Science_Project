...
Graph limit theory provides an effective approach to capturing the common large-scale features of networks.
...
We refer to \cite{lovaszbook} for a comprehensive account of (dense) graph limit theory.  To see some applications of graphons in network analysis, see \cite{uncertainty,cgao2015,cgao2021,sgao2020,PDE1,Medvedev2014}.
...
%
The approach in the current paper offers a two-fold advantage. Firstly, our newly proposed function $\Lambda$ possesses the desired properties of recognition, continuity, and recovery for all $L^p$-graphons, extending beyond the restricted case of $L^\infty$-graphons. 
It is worth noting that the proof of continuity (and recovery) of $\Gamma$ in \cite{Chuangpishit_2015,ghandehari2020graph}  cannot be adjusted to work for the setting of $L^p$-graphons (see Remark \ref{remark:teddy}). 
Thus, to  handle $L^p$-graphons, the new approach presented here seemed to be necessary. 
%
Secondly, the formula defining the function $\Lambda$ is less intricate.
Leveraging the simplicity of $\Lambda$ and applying meticulous estimates, we not only extend the results achieved by $\Gamma$ to encompass $L^p$-graphons (as detailed in Theorem~\ref{thm:main-result}), but also notably enhance the outcomes within the domain of $L^{\infty}$-graphons (as demonstrated in Corollary~\ref{cor:bnddstability}).
...
Informally speaking, these are graphs whose parameter value is small, and we think of them as `almost Robinson graphons'. 
...
${\mathcal W}_0$, which cannot be generalized to $\W^p$ \cite{MISHURA202226}. Thus, the cut-norm continuity of $\Gamma$ cannot be easily extended to the setting of $L^p$-graphons. In fact, whether $\Gamma$ is cut-norm continuous on $\W^p$ remains an open question.
\end{remark}
%
%-------------------------------------------------------------------
%-------------------------------------------------------------------
...
\end{definition}
%
Clearly, for every $w\in \W^p$, the  approximation  $R_w^{\alpha}$ is a Robinson graphon. 
%
\begin{remark}
...
Grey regions provide no information pertinent to our method of proof.

We observe that the $\mathcal{R}_k$ regions partition $\Delta\setminus \mathcal{G}$, skipping the proof of the following lemma as it is identical to that of \cite[Lemma 4.8]{ghandehari2020graph}. 
%
\begin{figure}
    \centering
...
Let $w \in {\mathcal W}^\infty$ be such that $w \geq 0$ and $\lceil \|w\|_{\infty} \rceil = M$, and let $\mathcal{B}_k$, $\mathcal{V}_k$, and $\mathcal{G}_k$ be as defined above. Then, 
%
...
\end{theorem}
%
The idea of the proof is as follows: We use an $\epsilon/3$ argument on $\|w-R_w^{\alpha}\|_{\Box}$ using $w_M$ and $R_{w_M}^{\alpha}$, then bound each term above using $\Lambda$. If $w_M$ is Robinson, then the upper bound is proved without use of Corollary \ref{cor:bnddstability}. If $w_M$ is not Robinson, then $\|w_M-R_{w_M}^{\alpha}\|_{\Box}$ must be handled using Corollary \ref{cor:bnddstability}. Handling these two cases finishes the proof.
...
\begin{description}
\item[\textbf{Case 1:}] Suppose that $\Lambda(w_M) >0$. 
...
\item[\textbf{Case 2:}] Suppose that $\Lambda(w_M) = 0$. 
...
Therefore, if $\Lambda(w_M)=0$, we have that $\|w-R_w^{\alpha}\|_{\Box} \leq 5\Lambda(w)^{\frac{p-5}{5p-5}}$, proving the theorem in Case 2. 
\end{description} 
...
%
\begin{description}
\item[{\bf Step 1:}] There must exist a set $Q \subset P$ such that $|Q| = |P|-\beta\left(\left\lceil \dfrac{|P|}{\delta}\right\rceil-1\right)$ and $\int_Q u~dx > 0$. 

To show this statement, note that
%
...
where $P^+ := \{x \in P : u(x) > 0\}$ and $P^- :=\{y \in P : u(y) < 0\}$. If $|P^+|\geq \delta$, then any subset $Q$ of measure $\delta$ from $P^+$ suffices. If $|P^+| < \delta$, then we let $Q=P^+\cup Q'$ for a subset  $Q'\subseteq P^-$ with $|Q'|=\delta-|P^+|$, and note that 
 $\int_{Q} u~dx\geq \int_{P} u~dx>0$ by \eqref{eq:Qpositivity}.

\item[{\bf Step 2:}] We now prove the existence of a subset $P_N\subseteq P$ satisfying $|P_N|=\delta$ and condition (iii). 

Towards a contradiction, assume that for any set $S \subseteq P$ such that $|S| = \delta$, we have 
%
...
\end{equation}
%
for all $p \in P$. Consider $\{p_i\}_{i=1}^M \subset R$, where $p_1=0$, $p_{i+1}=r_{p_i}$ for $1 < i < M$, and $M= \left\lceil \frac{|P|}{\delta}\right\rceil-1$. Then due to the positivity of $w$ and \eqref{eq:piecesbnd}, that 
%
...
\newblock To appear in \emph{European Journal of Combinatorics}, 2023.
