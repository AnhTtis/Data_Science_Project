...
halo is developed by using a new asymmetric equation, 
which is complementary to the Schr\"odinger equation.  
The theory predicts that each dark matter halo has its 
core and envelope with very distinct physical properties.  
The core is free of any quantum structure and its dark 
matter particles are in random motion and frequently 
collide with each other.  However, the envelope has a 
global quantum structure that contains quantized orbits 
populated by the particles.  The predicted quantum 
structure of the halo resembles an atom, hence, it 
is named the atomic model of dark matter halo.  
Applications of the theory to a dark matter halo 
with a given density profile are described, and 
predictions of the theory are discussed. 
...
Recent results show [19] the existence of the Schr\"odinger 
...
In this paper a new quantum theory of a DM halo is developed 
based exclusively on the NAE, which may describe DM because 
both the SE and NAE originate from the irreps of the extended 
Galilean group, and they are complementary.  This idea of using
the NAE to develop a DM model is justified by the fact that the 
SE describes well OM but fails to represent DM, and that the 
NAE may not be applicable to OM, at least at its microscopic 
level [36], but it may describe DM, specifically its halos as this 
paper demonstrates.

The developed theory predicts a quantum structure of the halo 
that resembles an atom, hence, it is called the {\it atomic model 
of dark matter halo}.  According to this theory, each halo has a 
...
equations are derived in Section II; previous theories of DM 
and their validity are discussed in Section III; a quantum theory 
of DM particles confined to a spherical DM halo is formulated in 
Section IV;  physical implications of the theory and its predictions 
are discussed in Section V; and conclusions are given in Section VI.   
%
...
halo's density profile (see Section IV.B); the orbital frequency 
...
\end{equation}  
%  
Let $\mu^2 = C_{\rho} \Omega_n^2$, with $\Omega_n$being 
the quantized orbital frequencies.  Then, Eq. (\ref{eq16}) can be 
written as 
%
\begin{equation}
...
The lowest frequency orbit inside the halo is 
...
halo will be considered.  Multiplying Eq. (\ref{eq17}) by 
$\hbar^2$, the spectrum of quantum energies corresponding 
to the orbits $\Omega_n^2$ inside the halo is obtained 
%
\begin{equation}
