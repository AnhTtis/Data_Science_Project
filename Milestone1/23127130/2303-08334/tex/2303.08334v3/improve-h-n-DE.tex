...
In the Sinc approximation, the mesh size and truncation numbers should be
...
can be written as $\ee^{-\pi d n/\log(2 d n/\mu)}$,
and the right-hand side in~\eqref{eq:double-expo-case}
can be written as $\ee^{-\pi d n}$.
...
Thus far, $n$ has denoted the maximum number of truncation numbers $M$ and $N$,
i.e., $n=\max\{M, N\}$.
This condition has been a barrier to the construction
of the optimal selection formula that
eliminates the gap between the discretization and truncation errors.
...
we state two new theorems, which are the primary results of this study.
In Section~\ref{sec3},
...
The above theorem states not only the convergence rate
...
on the following 20001 points:
\[
 x = \frac{4i}{10000}\quad
 (i = -10000,\,-9999,\,\ldots,\,-1,\,0,\,1,\,\ldots,\,9999,\,10000),
\]
...
%Then, to specify the relation between $h$ and $n$,
%we equate the exponential rates of the two errors:
%$\ee^{-\pi d/h}$ and $\ee^{-\pi\mu q(dn/\mu)}$.
We then specify the relation between $h$ and $n$,
considering the exponential rates of the two errors:
$\ee^{-\pi d/h}$ and $\ee^{-\pi\mu q(dn/\mu)}$.
...
$\ee^{-\pi d/h}=\ee^{-\pi\mu q(dn/\mu)}$ hold,
i.e., we have
\begin{align*}
...
the formula~\eqref{eq:improve-M-N-2} is designed so that
$\ee^{-\pi\alpha\sinh(Mh)}\leq \ee^{-\pi\mu q(dn/\mu)}$
and
$\ee^{-\pi\beta\sinh(Nh)}\leq \ee^{-\pi\mu q(dn/\mu)}$
hold.
We note that the explicit form of the function $q(x)$ is not used
in this proof;
this point is discussed in Section~\ref{sec:remark-q}.
\end{remark}
...
\cdot\frac{\ee^{-\pi d/h}}{(1 - \ee^{-2\pi d/h})}\nonumber\\
&\quad + \frac{2 R}{\pi \mu h\sqrt{1 + \{q(dn/\mu)\}^2}}
\ee^{-\pi \mu q(dn/\mu)}\nonumber\\
...
For the proof of Theorem~\ref{thm:new2},
we use the following proposition.

\begin{proposition}[Okayama and Kawai~{\cite[Proposition 7]{OkaKawai}}]
\label{prop:q-x}
Let $q(x)$ be defined by~\eqref{eq:q-x}.
Then, $q(x)$
is a monotonically increasing function for $x\geq 0$.
\end{proposition}

Using this result, we prove Theorem~\ref{thm:new2} as follows.
\begin{proof}
From Theorem~\ref{thm:discrete} and Lemma~\ref{lem:new},
...
&\quad + \frac{2 R q(dn/\mu)}{\pi d\sqrt{1 + \{q(dn/\mu)\}^2}}
\ee^{-\pi \mu q(dn/\mu)}.
%& \sup_{x\in\mathbb{R}}\left\lvert
%F(x) - \sum_{k=-M}^N F(kh)\sinc\left(\frac{x - kh}{h}\right)
%\right\rvert\nonumber\\
...
is bounded as
\begin{align*}
\frac{2 R q(dn/\mu)}{\pi d\sqrt{1 + \{q(dn/\mu)\}^2}}
\ee^{-\pi \mu q(dn/\mu)}
...
However, notice that
in the proof of Theorem~\ref{thm:discrete} and Lemma~\ref{lem:new},
we do not use the explicit form of $q(x)$.
Therefore, even if we choose another function for $q(x)$,
...
\label{thm:new3}
Assume that $F\in\LC_{L,R,\alpha,\beta}(\domD_d)$.
Here, let $q(x)$ be a function that satisfies
$q(x)\geq 0$ for $x\geq 0$ and $q'(x)\geq 0$
for $x\geq 0$.
Let $\mu=\min\{\alpha,\beta\}$,
let $h$ be defined as
\[
h = \frac{d}{\mu q(dn/\mu)},
\]
and let $M$ and $N$ be defined as~\eqref{eq:improve-M-N-2}.
Then, it holds that
\[
 \sup_{x\in\mathbb{R}}
\left\lvert
F(x) - \sum_{k=-M}^N F(kh)\sinc\left(\frac{x - kh}{h}\right)
\right\rvert
\leq C \ee^{-\pi \mu q(dn/\mu)},
\]
where $C$ is a constant independent of $n$,
expressed as~\eqref{eq:constant-C-thm:new2}.
\end{theorem}

According to this theorem,
if we define $q(x)$ as $q(x)=x$,
we can derive the following result instead of Theorem~\ref{thm:new2}.
...
Although Corollary~\ref{cor:new3} is meaningful
