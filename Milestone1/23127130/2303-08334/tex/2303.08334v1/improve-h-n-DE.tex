...
This approximation has notably been utilized for many applications
because of its high efficiency.
The Sinc approximation's mesh size and truncation numbers should be
optimally selected to avail its full performance. However,
the usual formula has only been ``near-optimally'' selected
because the optimal formula between the two cannot be expressed
in terms of elementary functions.
In this study, we propose two improved formulas.
The first one is based on the concept by an earlier research
that produced an improved selection formula
for the double-exponential formula.
%104
The formula performed better than the usual one, but was still not optimal.
As a second formula,
we introduce a new parameter to propose a truly optimal formula between the two.
We give explicit error bounds for both formulas.
Numerical comparisons show that the first formula gives
a better error bound than the standard formula,
and the second formula gives a far better error bound
than both the standard and first formulas.
%176
}
...
\end{equation}
where $\sinc x=\sin(\pi x)/(\pi x)$, and $h$ is a mesh size.
...
\label{eq:domDd}
\end{equation}
this approximation achieves exponential convergence:
$\OO(\ee^{-\pi d/h})$.
...
(called the discretization error) and
that in~\eqref{eq:sinc-approx-finite} (called the truncation error).
Thus,
by equating the exponential rates of the two errors, the optimal formula of
...
which is far smaller than that in Stenger's result.
The Sinc approximation for
...
\end{equation}
but the solution $h$ cannot be expressed
in terms of elementary functions of $n$.
Instead, several authors have employed a near-optimal formula
\begin{equation}
...
becomes $\ee^{-\pi d n}$.
Although those two rates are approximately equal,
the former rate is slightly lower than the latter rate,
resulting in $\OO(\ee^{-\pi d n/\log(2 d n/\mu)})$
as the final error rate of the DE-Sinc approximation.
Furthermore, aiming for a computation with guaranteed accuracy,
computable error bounds for both the SE- and DE-Sinc approximations
have been given~\cite{OkaMatsuSugi,Oka1}.

Considering the above background, this study improves the formula
relating $h$ and $n$ in the case of the DE-Sinc approximation.
...
we give a sharper error bound for the DE-Sinc approximation
than the existing one~\cite{OkaMatsuSugi,Oka1}.

Our second formula is based on a completely different approach.
Thus far, $n$ denotes a maximum number of truncation numbers $M$ and $N$,
i.e., $n=\max\{M, N\}$.
In the new formula, we regard $n$ as a free parameter
that is not required to be equal to $M$ or $N$.
Then, we can derive formulas for $h$, $M$, and $N$
with respect to $n$, such that
the discretization and truncation errors
become exactly the same rate.
...
In Section~\ref{sec2},
we state existing and new theoretical results (main results of this paper).
In Section~\ref{sec3},
...
$n\geq (\nu\ee)/(2d)$,
which is removed in Theorem~\ref{thm:new1}.
...
For simplicity, let us consider the case $\alpha=\beta$.
...
\right\rceil.
\]
Because $q(x)< x$ holds for $x> \sinh 1$,
this equation implies $M=N < n$ for sufficiently large $n$.
...
we employ the double-exponential transformation
\[
...
\]
and investigated the maximum error among those points.
...
Then, we consider equating exponential rates in those two errors:
...
\end{align*}
and the latter order is slightly worse than the former order.
...
$\OO(\ee^{-\pi d n})$, which seems better than
that by Theorem~\ref{thm:new2}: $\OO(\ee^{-\pi d n/\arsinh(dn/\mu)})$.
However, we should note that those rates are given
with respect to $n$,
...
as an error bound with simpler formulas of $h$, $M$, $N$,
