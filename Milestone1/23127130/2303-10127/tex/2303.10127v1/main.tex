...
\begin{abstract}
  This paper studies the celebrated Kuramoto-Sakaguchi model of coupled
  oscillators adopting two recent concepts.  First, we consider
  appropriately-defined subsets of the $n$-torus called winding cells.
  Second, we analyze the semicontractivity of the model, i.e., the property
  that the distance between trajectories decreases when measured according
  to a seminorm.

  This paper establishes the local semicontractivity of the
  Kuramoto-Sakaguchi model, which is equivalent to the local contractivity
  for the reduced model.  The reduced model is defined modulo the
  rotational symmetry.  The domains where the system is semicontracting are
  convex phase-cohesive subsets of winding cells.  Our sufficient
  conditions and estimates of the semicontracting domains are less
  conservative and more explicit than in previous works.  Based on
  semicontraction on phase-cohesive subsets, we establish the "at most
  uniqueness" of synchronous states within these domains, thereby
  characterizing the multistability of this model.
\end{abstract}
...
In an attempt to tackle wider classes of systems, variation of contraction
...
systems.  Our first contribution (Theorem~\ref{thm:max_cohes}) is to
...
light on the fundamental reasons leading to "at most uniqueness", namely
...
In the following, we first review the Kuramoto-Sakaguchi model
(Sec.~\ref{sec:ks-all}) as well as semicontraction theory
(Sec.~\ref{sec:semic-recap}).  Our main semicontractivity and "at most
uniqueness"results are presented in Secs.~\ref{sec:semicontraction} and
\ref{sec:amu} respectively.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Kuramoto-Sakaguchi model on complex graphs}\label{sec:ks-all}
Let $\G$ denote a weighted, undirected, connected graph with $n$ vertices
and $m$ edges.  The matrices $B\in\mathbb{R}^{n\times m}$,
$\A\in\mathbb{R}^{m\times m}$, and $L_{\G}=B\A B^\top$ are the
\emph{incidence}, \emph{weight}, and \emph{Laplacian} matrices of $\G$
respectively.  The Laplacian's eigenvalues are $0 = \lambda_1(L_{\G}) <
\lambda_2(L_{\G}) \leq \ldots \leq \lambda_n(L_{\G})$, and $\lambda_2(L)$
is known as the \emph{algebraic connectivity} of $\G$~\cite{Fie73}.
$\bm{1}_n\in\mathbb{R}^n$ is the vector with all components equal to one
and $I_n\in\mathbb{R}^{n\times n}$ is the identity matrix.

A \emph{simple path} in $\G$ is a sequence of vertices
$p=(i_1,...,i_\ell)$, such that each consecutive pair is connected in $\G$
and each vertex appears at most once in $\sigma$.  A \emph{cycle} of $\G$
...
We consider the \emph{Kuramoto-Sakaguchi model}~\cite{Sak88} over a weighted undirected graph $\G$ with frustration parameter $\varphi\in[0,\pi/2]$,
...
\end{align}
A collection of cycles $\Sigma=(\sigma_1,...,\sigma_c)$ of $\G$ induces the associated \emph{winding vector}, 
...
\begin{figure}
 \centering
 \includegraphics[width=\columnwidth]{flow5red.pdf}
...
{\bf Cohesive sets.}
For $\gamma\in[0,\pi]$, a state $\bm{x}\in\mathbb{R}^n$ is \emph{$\gamma$-cohesive} if for each edge $(i,j)$ of $\G$, $|\dcc(x_i,x_j)| \leq \gamma$. 
...
\begin{align}\label{eq:semin-def}
 ...
 \|x+y\| &\leq \|x\| + \|y\|\, .
\end{align}
Each seminorm has a \emph{kernel}, i.e., a vector subspace where the
seminorm vanishes.  Each seminorm is a norm when restricted to the subspace
perpendicular to the kernel.

The Kuramoto-Sakaguchi model being invariant under homogeneous angle
shifts, we consider seminorms whose kernel is the consensus space
$\operatorname{span}(\bm{1}_n)$. Let $R\in\mathbb{R}^{(n-1)\times n}$ be a
full rank matrix whose rows form an orthonormal basis of the subspace
$\bm{1}_n^\perp$, and define the orthogonal projection
\begin{align}
 \Pin &= R^\top R = I_n - n^{-1}\bm{1}_n\bm{1}_n^\top\, .
\end{align}
In what follows we consider the \emph{$(\ell_2,\Pi_n)$ consensus seminorm}
defined by $\|\bm{x}\|_\cons = \|\Pi_n\bm{x}\|_2=\|R\bm{x}\|_2$.
...
A remarkable consequence of Eq.~\eqref{eq:l2-log-seminorm2} is that, when $L_{\G}$ is a weighted Laplacian matrix,
\begin{align}\label{eq:muL}
 \mu_\cons(-L_{\G}) &= -\lambda_2(L_{\G})\, .
\end{align}
...
\begin{align}
 \left[\Jac{\hod}(\bm{x})\right]_{ij} &= \ds \left\{
 \begin{array}{ll}
   \ds c_{ij}\cos(x_i - x_j)\, , & \text{if } i\neq j\, , \\
   \ds -\sum_{k=1}^n c_{ik}\cos(x_i - x_k)\, , & \text{if } i = j\, ,
 \end{array}
 \right. \label{eq:jaco}
 \\[.2ex]
  \left[\Jac{\hev}(\bm{x})\right]_{ij} &= \ds \left\{
 \begin{array}{ll}
   \ds -s_{ij}\sin(x_i - x_j)\, , & \text{if } i\neq j\, , \\
   \ds \sum_{k=1}^n s_{ik}\sin(x_i - x_k)\, , & \text{if } i = j\, ,
 \end{array}
 \right. \label{eq:jace}
\end{align}
...
\end{align} 
Note that all three Jacobians above have zero row sum, the same sparsity pattern as $L_{\G}$, and that $\Jac{\hod}$ is symmetric. 
...
\begin{lemma}[Log-seminorm of the odd Jacobian]\label{lem:jaco}
 ...
 \begin{align}\label{eq:bound_muf}
  \mu_\cons(\Jac{\hod}(\bm{x})) &\leq -\cos(\varphi)\cos(\gamma)\lambda_2(L_{\G})\, ,
 \end{align}  
 where $\lambda_2(L_{\G})$ is the algebraic connectivity of the graph $\G$. 
\end{lemma}
...
\begin{align}
 L(\bm{x}) &= \Jac{\hod}(\bm{x}) + \cos(\varphi)\cos(\gamma) L_{\G}\, .
\end{align}
...
\begin{align}
 -\lambda_2(\Jac{\hod}(\bm{x})) &= \lambda_2\left(\cos(\varphi)\cos(\gamma) L_{\G} - L(\bm{x})\right) \notag\\
 &\geq \cos(\varphi)\cos(\gamma)\lambda_2(L_{\G}) 
\end{align}
...
\begin{lemma}[Log-seminorm of the even Jacobian]\label{lem:jace}
 ...
 \end{align}
 where $\dmax$ denotes the maximal weighted degree of $\G$. 
\end{lemma}
...
\begin{theorem}[Local strong semicontraction]\label{thm:max_cohes}
 ...
 graph $\G$, with frustration parameter $\varphi\in[0,\pi/2]$.  Let
 $\lambda_2$ and $\dmax$ denote the algebraic connectivity and maximal
 degree of $\G$ respectively, and define the angle
...
\begin{figure}
 \centering
 \includegraphics[width=.9\columnwidth]{heatmap-13-0.01.pdf}
 \caption{Two-dimensional slice of $\mathbb{R}^{13}$, showing the Jacobian's logarithmic seminorm for the Kuramoto-Sakaguchi model on $\G$ (see inset) and frustration $\varphi=0.01$. 
...
\begin{figure}
 \centering
 \includegraphics[width=.37\textwidth]{bounds.pdf}
...
The proof of Theorem~\ref{thm:amu} (see Appendix~\ref{sec:thm4}) leverages a natural polytopic representation (see Appendix~\ref{sec:polytope}) of each connected component of the $\gamma$-cohesive winding cell, presented in Ref.~\cite{Jaf22}. 
...
In summary, infinitesimal semicontractivity implies strong infinitesimal contractivity of a reduced system and the polytopic representation of the state space guarantees convexity of the contractive subdomain, implying "at most uniqueness" of the equilibrium therein. 

Theorem~\ref{thm:amu} improves previous "at most uniqueness" conditions \cite[Theorem 4.1]{Jaf22} and \cite[Corollary 6]{Del22} in that it applies to a wider class of models, has a less conservative condition on cohesiveness [Eq.~\eqref{eq:max_cohes}], and the condition for semicontractivity [Eq.~\eqref{eq:max_cohes}] is much more explicit than its counterpart in \cite[Eq. (45)]{Del22}. 
Furthermore, the proof is much more streamlined and elegant, which provides a clear insight on the underlying mechanisms leading to "at most uniqueness", namely the local semicontractivity of the Kuramoto-Sakaguchi model. 
...
\end{align}
where $B$ is the incidence matrix of the graph $\G$ and $C_\Sigma$ is the cycle-edge incidence matrix, defined, e.g., in \cite[Eq. (2.1)]{Jaf22}.
...
\subsection{Proof the "at most uniqueness" (Theorem~\ref{thm:amu})} \label{sec:thm4}
...
\end{align}
where we used Eq.~\eqref{eq:n2sn}. 
...
The graph $\G$ being connected, we can recursively reconstruct the vector of differences $\xs-\xt$ up to a constant shift. 
We conclude that for each node $i$, the difference $(\xs-\xt)_i$ is of the form 
\begin{align}
 (\xs-\xt)_i &= \rho + 2\pi\ell_i\, , & \ell_i\in\mathbb{Z}\, ,
\end{align}
where we do not know $\rho\in\mathbb{R}$, but it does not matter. 
