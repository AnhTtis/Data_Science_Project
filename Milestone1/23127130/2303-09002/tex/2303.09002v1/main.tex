...
\author{Taosha Guo, Abed AlRahman Al Makdah, Vishaal Krishnan, and
  Fabio Pasqualetti \thanks{This material is based upon work supported
    in part by awards ONR-N00014-19-1-2264 and AFOSR-FA9550-20-1-0140.
    T. Guo and F. Pasqualetti are with the Department of Mechanical Engineering, and A. A. Al Makdah is with the Department of Electrical and Computer Engineering at the University of California, Riverside. \href{mailto:tguo023@ucr.edu}{\{\texttt{tguo}},\href{mailto:aalmakdah@engr.ucr.edu}{\texttt{aalmakdah}},\href{mailto:fabiopas@engr.ucr.edu}{\texttt{fabiopas\}@engr.ucr.edu}}.
    V. Krishnan is with the School of Engineering and Applied Sciences, Harvard University, \href{mailto:vkrishnan@seas.harvard.edu}{\texttt{vkrishnan@seas.harvard.edu}}.}}
%    
...
behavior and seeks to learn a policy that mimics this behavior. In
transfer learning, a model trained on one task is used as the starting
point for a model on a second related task. The key idea is that
certain features learned by the model on the first task can be used as
a general-purpose set of features for the second task, allowing the
model to learn the second task efficiently. While these
techniques have proven useful in multiple learning scenarios,
including image classification and natural language processing, their use and~utility in
control settings have mostly escaped~scrutiny.

In this paper we investigate the use of imitation and transfer
learning for Linear Quadratic Gaussian (LQG) control, which seeks a
control policy for a stochastic linear system that minimizes the
expected value of a quadratic function of the state and input
trajectories \cite{KZ-JCD-KG:96}. We consider multiple control tasks,
where the system dynamics are fixed but the quadratic cost function
varies. We assume that the system dynamics, noise statistics, and cost
functions are unknown, and that datasets are available containing
optimal input and output trajectories for the different cost functions
(source tasks). The questions that we will answer include whether it
is possible to learn the LQG controllers from expert data, the
required size of the datasets, and whether the source datasets can be
leveraged to learn the controller for a target task. We will show that
our data-based controller enjoys a separation property similar to the
well-known separation principle \cite{KZ-JCD-KG:96}, and that the
lower-dimensional data-based estimation module can be transferred upon
changes of the cost function to reduce the amount of expert data
required for control design.
...
cost, e.g., see \cite{BR:18,KZ-BH-TB:20}. Differently %LF-YZ-AP-MK:2019
...
less attention, with
\cite{LX-LY-GC-SS:22,YC-AMO-FP-EDA:22,HW-LFT-JA:22} and
...
\cite{SL-KA-BH-AA:20,YZ-LF-MK-NL:21}, which primarily
...
gains, respectively. Although different choices of the matrices in
\eqref{eq: compensator matrices} are possible, in what follows we
...
with $M_y$ and $F_y$ being constructed in the same way as $M_u$ and
...
for different choices of the weight matrices $Q$ and $R$.  The
question that we will address is to quantify the sample complexity of
learning the LQG controllers, that is, the amount of expert data
needed to learn the LQG controller. We will show that, after
sufficient (finite) training with different weight matrices, as little
as ${2n+m-1}$ expert samples are sufficient to learn new
LQG~controllers.

To formalize the considered problem, let
$D_i = \{u_i^*, y_i^*\}_{t}^{t+T}$ be the sequence of expert inputs
solving the LQG problem from time $t $ up to time $t+T$ for the
matrices $Q_i$ and $R_i$, and the corresponding outputs of the system
\eqref{eq: system}.
...
\bigskip Theorem \ref{thm: data-driven separation} shows how the LQG
...
\emph{internal representation} that can be used to generate optimal
LQG inputs via the LQR gain in a model-based setting, the estimation
matrix $\subscr{L}{est}$ provides an $(n+m)$-dimensional internal
representation, $z(t)$, that can be used to generate optimal LQG
inputs in a data-driven setting. Yet, while the Kalman filter uses the
whole history of inputs and outputs to generate an optimal internal
representation, the estimation $\subscr{L}{est}$ only uses a finite
window of past inputs and outputs. Finally, as we will show in the
...
inputs from input-output data as in \eqref{eq: static LQG}.
...
controller. Next, since the focus of this paper is in solving
imitation and transfer learning problems, we provide an expression of
the estimation matrix $\subscr{L}{est}$ given data from a set of source
LQG tasks. We start with the case of single-input systems and discuss
the general case afterwards. We make the following assumption
regarding the source tasks.
...
\begin{assumption}{\bf \emph{(Number and diversity of source
  ...
  Let $D_1, \cdots, D_N$ be the expert trajectories from the source
  LQG tasks with weight matrices
...
% column spaces?}
% \margin{Possibly, but not sure how that will look like. Also, this
...
system:%{\aammargin{why do we have a bar on $U$?}}
\begin{align*} {\bar{U}}^{\text{target}}_{c_t} =
...
$T \ge n(l+2)-1$ for the single-input case (when $T \ge n(l+2)-1$ the
data matrix in \eqref{eq: reconstruct KLQG} is of full row rank). This
bounds reflects the complexity of learning the LQG controller in an
imitation learning framework. On the other hand, leveraging the
separation principle in Theorem~\ref{thm: data-driven separation}, the
matrix $\subscr{\hat L}{est}$ can be learned from $N \ge n+1$ expert
datasets and used to learn the LQG controller of any target task. By
doing so, Theorem \ref{thm: sample complexity} states that the expert
trajectory of the target task need only to be of length $2n$ for the
single-input case. This reduced bound reflects the benefits of the
considered imitation and transfer learning setting, where data from
earlier tasks are used to solve future LQG tasks. For instance, when
$m = 1$ and $l=2$, the imitation and transfer approach requires about
$50\%$ less expert data compared to the imitation approach~alone.
...
\begin{remark}{\bf \emph{(Learning $\subscr{L}{est}$ when
  ...
  of ${H^i_{n,c_s}}$. By stacking these expressions together we obtain  \begin{align}\label{eq: kernel matrix}
    ...
    \subscr{L}{est} =
    \begin{bmatrix}
      \bar{U}^1_{c_s} H^{i^\dag}_{n,c_s} + X_1 Z_1 \\ \vdots \\ \bar{U}^i_{c_s} H^{i^\dag}_{n,c_s} + X_N Z_N .
    \end{bmatrix}
    .
  \end{align}
...
$V = 0.6$. The weight matrices of the target task~are {
  \begin{align*}
   ...
   % \end{smallarray}.
  \end{align*}
}We will compare the model-based approach to compute $\subscr{K}{LQG}^{\text{target}}$ in Theorem  \ref{thm: data-driven separation}
 with the data-driven approach in Theorem \ref{thm: Lest m1}. Assume that the LQG compensator for \eqref{eq: reactor} is solved by the model-based approach in 
\cite{KZ-JCD-KG:96} given the system model, noise statistics $\{W,V\}$
and the weighing matrices $\{\subscr{Q}{target},
\subscr{R}{target}\}$. Using \eqref{eq: Klqg model} we obtain
$\subscr{K}{LQG}^{\text{target}}$ as
...
{\footnotesize
  \begin{align*}
    ...
    \end{bmatrix}
                                                                ...
                                                                % \right].
  \end{align*}
} For our data-based approach, we assume that we have collected the
expert trajectories $D_1, \dots, D_N$ of length $ T = 11 $ from
$N = 5$ source tasks, with weighting matrices $Q_i = i I_4$ and
$R_i = I_2$ respectively. Using Theorem \ref{thm: Lest m1}, we compute
the estimation matrix $\subscr{\hat L}{est}$ as
...
\eqref{remark: multi input}, we compute $\subscr{\hat L}{est}$. In
Fig.~\ref{fig: multi input} we plot the error
$\|\subscr{K}{LQG}^\text{target}(I - \subscr{\hat{L}}{est}^\dagger
\subscr{\hat{L}}{est}) \|$ as the number of source tasks increases.
The convergence of the error implies that $\subscr{\hat L}{est}$
obtained using Remark \eqref{remark: multi input} becomes the
correct estimation matrix for the target LQG~controller.
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.8\columnwidth,trim={0cm 0cm 0cm
    0cm},clip]{tutorial.pdf}
  \caption{This figure shows the error
    $\|\subscr{K}{LQG}^\text{target}(I - \subscr{\hat{L}}{est}^\dagger
    \subscr{\hat{L}}{est}) \|$ as a function of the number of source
    tasks. It can be seen that the procedure in Remark~\eqref{remark:
      multi input} reconstructs the estimation matrix with a finite
...
needed to learn the controller, and show how the controller can
...
dynamics also change, and the extension to more general optimal
control problems.
