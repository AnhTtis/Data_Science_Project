...
\end{align*}
To see that the above assignment is correct, we note that the right-hand-side of \eqref{eq:dominance:u} is a fully-factorized function of $\atn{t}{n}$'s, as follows.
\begin{align*}
&p\l( \mu, t, h, a \r) = \int_{U} \mu\l( du \r) \prup{u}{P_1} \l( \Hst{t} = \hst{h}{t}, \At{t} = \at{t} \r)\\
&= \int_{\uspace} P_1\l( \sspace, \hst{h}{1} \r) \prod_{t'=2}^{t} \pr_{P_1} \l( \ot{t'} | \hst{h}{t'-1}, \at{t'-1} \r)\\
&\hspace{10pt} \times \prod_{n=1}^{N} \prod_{t'=1}^{t} \utn{u}{t'}{n}\l( \atn{t'}{n} |  \hstn{h}{t'}{0}, \hstn{h}{t'}{n} \r) \l( \l( \mymathop{\times}_{n=1}^{N} \mun{n}\r)(du) \r)\\
...
\end{align*}
where the last equality follows from Tonneli's Theorem (see Proposition \ref{prop:tonneli}). We will now prove, by forward induction, that $\udl{u}$ and $\mu$ induce the same distribution on $\l( \Hst{t}, \At{t} \r)$ for all $t\in\mbb{N}$.
\begin{enumerate}
...
&\hspace{-60pt} =\int_{\uspace} \mu\l( du \r) \prup{u}{P_1} \l( \Hst{t+1} = \hst{h}{t+1} \r)\\
...
\end{cor}
\begin{proof}
One notes that $\fullccosts{u}$ and $\fulldcosts{u}$ can be written as:
\begin{align*}
\fullccosts{u} &= \sum_{t=1}^{\infty} \alpha^{t-1} \E{\mu}{P_1} \l[ \mbb{E}_{P_1} \l[ c\l( \Stt{t}, \At{t} \r)  \r] | \Hst{t}, \At{t} \r],\\
\fulldcosts{u} &= \sum_{t=1}^{\infty} \alpha^{t-1} \E{\mu}{P_1} \l[ \mbb{E}_{P_1} \l[ d\l( \Stt{t}, \At{t} \r)  \r] | \Hst{t}, \At{t} \r],
\end{align*}
...
\begin{dfn}[Lower Semi-continuous and Upper Semi-continuous Functions]\label{dfn:lsc}
...
\begin{enumerate}
\item The topological space $ \l(\m{\mcl{X}} , \tau\r)$ is completely-metrizable. That is, there exists a complete metric $\metric{M}$ on $ \m{\mcl{X}}$ that induces the same topology as $ \tau $.
