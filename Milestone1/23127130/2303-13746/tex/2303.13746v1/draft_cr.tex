\documentclass[aps,prx,reprint,groupedaddress,superscriptaddress,nofootinbib,onecolumn]{revtex4-1}
...
\title{FixFit: using parameter-compression to solve\\ the inverse problem in overdetermined models}


\author{Botond B Antal}
\affiliation{Department of Biomedical Engineering, State University of New York at Stony Brook}
\affiliation{Athinoula A. Martinos Center for Biomedical Imaging, Massachusetts General Hospital and Harvard Medical School}
\author{Anthony G Chesebro}
\affiliation{Department of Biomedical Engineering, State University of New York at Stony Brook}
\affiliation{Renaissance School of Medicine, State University of New York at Stony Brook}

\author{Helmut H Strey}
\affiliation{Department of Biomedical Engineering, State University of New York at Stony Brook}
\affiliation{Laufer Center for Physical and Quantitative Biology, State University of New York at Stony Brook}
...
Mathematical models are commonly used to describe the dynamical behavior of physical systems. As model parameters are often associated with real-world properties, methods for fitting parameters to experimental data can be used to characterize phenomena such as particle properties \cite{gfitter2012updated}, interactions among genetic networks \cite{d1999linear}, and the mechanism of neuronal signals \cite{hodgkin1952currents}. One fundamental challenge in parameter fitting, however, stems from parameter redundancies. These can come from intrinsic relationships between parameters but are also observed in highly complex and hierarchical dynamical systems where the measured outputs are many layers above where the microscopic parameters act. This results in a conundrum in which many different combinations of parameters fit the observed data equally well \cite{gutenkunst2007universally, chis2011structural}. Finding these non-unique solutions is also difficult, as parameter interactions can introduce numerous local minima that hinder algorithms for data fitting \cite{weise2009global}. Consequently, there is often a trade-off between using an easier-to-interpret model with fewer parameters that fails to describe the system accurately and using a highly-detailed model that risks redundancy of its parameters. 

A solution to this trade-off is to identify and account for parameter dependencies. One of the most widely-used algorithms to quantify interactions among parameters is the nonlinear least-square Levenberg-Marquardt algorithm, which, for given data, finds the locally best-fitting parameter values and their covariance matrix \cite{more2006levenberg}. In overdetermined models, these covariances are strong for pairs of parameters that are not separable. While the Levenberg-Marquardt algorithm only characterizes linear interactions, other methods can also uncover nonlinear interactions. For example, \cite{pohjanpalo1978system} determines parameter interactions by approximating the model's behavior as a function of its parameters using the Taylor series expansion. Nevertheless, a common limitation of these methods is that they do not provide a functional form for the redundancies and, therefore, cannot guide the parameter fitting process. A sensible strategy is to combine the redundant parameters into composite parameters with a unique best fit. Although suitable methods exist, they rely on studying the model analytically or through its local behaviors and derivatives \cite{cole2020parameter}, a strategy that would not be feasible for highly complex models. As such, a more general approach is needed.

To address this problem, we developed a general framework for identifying and estimating the largest set of lower dimensional latent parameters uniquely resolved by the data (\autoref{fig:scheme}). Our framework, \textit{FixFit}, consists of three steps. First, we find these latent parameters using a neural network with a bottleneck. Then, we optimize the same architecture at variable bottleneck widths to identify the optimal latent dimension \cite{tishby2000information, tishby2015deep, kramer1991nonlinear, achille2017emergence}. The ability of neural networks to approximate any function allows our method to be applied to models of arbitrary complexity \cite{hornik1989multilayer, hornik1991approximation, csaji2001approximation}. In the second step, FixFit establishes the relationship between latent and original parameters using global sensitivity analysis \cite{li2010global}. Lastly, we fit latent parameters to data using global optimization techniques \cite{virtanen2020scipy}.

To demonstrate the utility of FixFit, we showcase its ability to recover known nonlinear parameter combinations in the Kepler orbit model \cite{bate2020fundamentals} and to identify previously undiscovered parameter redundancies in a higher-dimensional multi-scale Larter-Breakspear neural mass model \cite{larter1999coupled, breakspear2003modulation}.
...
\begin{figure}[!htb]
    ...
    \end{center}
	\caption{
 \textbf{Recovering the known parameter redundancies of the Kepler orbit model.}
 \textbf{A}: Diagram of an example Keplerian orbit, the corresponding input parameters (red), and the model outputs ($\theta$, $r$ pairs). \textbf{B}: Validation error of the compressed representation found by FixFit as a function of the bottleneck dimension ($k$). Multiple replicates were performed at each $k$ using a stochastic optimizer. FixFit correctly identified the underlying redundancy of the Kepler orbit model by indicating saturating error at $k=2$ (see Methods). In subsequent panels, we utilized one of the fitted neural network replicates at $k=2$.
...
Our framework uses SCSA to quantify the impact of each model parameter on the latent parameters \cite{li2010global}.  However, as with other sensitivity-based approaches, SCSA does not provide explicit formulas for the nonlinear parameter relationships identified by the neural network.  Besides revealing more information about relationships between input parameters, such formulas can allow latent parameters to be understood as composite parameters that may be easier to interpret (i.e., eccentricity in the Kepler orbit model). This, in turn, could allow FixFit to develop intuitive, simplified models. Symbolic regression offers a possible avenue to achieve this by approximating the encoder \cite{schmidt2009distilling, la2021contemporary} or by itself being incorporated into the neural network \cite{petersen2019deep, kim2020integration}.
...
\begin{equation}
\begin{split}
    \frac{dV_i}{dt} = -(g_\text{Ca} + r_{NMDA} \cdot a_{ee} \cdot [(1 - c) \cdot Q_V + c \cdot Q_i^{network}]) \cdot m_\text{Ca} \cdot (V_i - V_\text{Ca}) \\
        -(g_\text{Na} \cdot m_\text{Na} + a_{ee} \cdot  [(1 - c) \cdot Q_V + c \cdot Q_i^{network}]) \cdot (V_i - V_\text{Na}) \\
        -g_\text{K} \cdot W_i \cdot (V_i - V_\text{K}) - g_L \cdot (V_i - V_L) - a_{ie} \cdot Z_i \cdot Q_z + a_{ne} \cdot I_0
\end{split}
\end{equation}

\begin{equation}
    \frac{dZ_i}{dt} = b \cdot (a_{ni} \cdot I_0 + a_{ei} \cdot V_i \cdot Q_V)
\end{equation}
...
\begin{equation}
    Q_V = 0.5 \cdot Q_{V_{max}} \cdot (1 + tanh(\frac{V - V_T}{\delta}))
\end{equation}

\begin{equation}
    Q_Z = 0.5 \cdot Q_{Z_{max}} \cdot (1 + tanh(\frac{V - V_Z}{\delta}))
\end{equation}

\begin{equation}
    m_{ion} = 0.5 \cdot (1 + tanh(\frac{V - T_{ion}}{\delta_{ion}}))
\end{equation}

Brain regions are connected through the coupling term $Q_i^{network}$, which is scaled with a global coupling constant $c$, to produce whole-brain-scale dynamics. $Q_i^{network}$ is given by:

\begin{equation}
    Q_i^{network} = \frac{\sum_j u_{i,j} \cdot Q_{V_j}}{\sum u_{i,j}}
\end{equation}
...
\begin{table}[h]
\center
\caption{Parameters of the Larter-Breakspear multi-scale brain model}
\label{tab:lb_pars}
...
\begin{figure}[htb]
    \begin{center}
	    ...
	    figs_cr/FigS4.pdf}
    \end{center}
	\caption*{SI FIG. 4: \textbf{Brain network model:  distribution of latent parameters following training.}
