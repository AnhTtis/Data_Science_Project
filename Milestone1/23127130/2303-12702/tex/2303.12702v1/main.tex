...
% \newcommand{\CL}[1]{\textcolor{green}{#1}}
...
%\author[4]{Donghun Kim}
%\author[1,5]{Matthias Scheffler}
\author[3]{Christian H. Liebscher\thanks{Corresponding author}}
\author[1, 4]{Luca M. Ghiringhelli\protect\CoAuthorMark}
...
%\affil[4]{Computational Science Research Center, Korea Institute of Science and Technology, Seoul 02792, Republic of Korea}
\affil[4]{Physics Department and IRIS Adlershof, Humboldt-Universität zu Berlin, Berlin, Germany}

\affil[$\dag$]{leitherer@fhi-berlin.mpg.de, ghiringhelli@fhi-berlin.mpg.de,liebscher@mpie.de}
...
Excellent performance is achieved for automatically identifying crystal structure, lattice orientation, and location of interface regions in synthetic and experimental images. 
The model yields classifications and uncertainty estimates through which both bulk and interface regions are identified.
Notably, no explicit information on structural patterns at the interfaces is provided during training. 
...
To engineer novel materials for such applications, it is necessary to characterize their crystalline structure down to the atomic level, including defects or interfaces, local lattice orientations, and distortions \cite{Lu2018AdvMater,Liebscher2018PhysRevLett,Meiners2020Nature}. 
Currently, the ultimate tool to probe hidden imperfections in crystalline materials is electron microscopy.
...
Such methods are introduced to uncover overlooked characteristics and this way drive a paradigm shift in image analysis and design of novel descriptors of atomic-resolution data. 
...
Here, we propose AI-STEM, which stands for Artificial-Intelligence Scanning Transmission Electron Microscopy. AI-STEM automatically identifies crystal symmetry and lattice orientation as well as the location of defects such as grain boundaries in STEM images. 
...
%, a quantity that has no meaningful interpretation at the interface. 
...
%One may then perform augmenting analysis (cf. Fig. \ref{fig:fig1}g) 
%that provides information beyond AI-STEM's predictions, which, however are required 
%for determining where and how to calculate these additional properties. 
...
In addition, translational invariance is introduced, which otherwise would have to be learned from the data. 
...
%contain a defect (since the model is trained only on crystalline images).  
...
Notably, this process is in principle trivial to parallelize. We discuss more details on computation time and choice of $T$ in the Supplementary Information (cf. Fig. \ref{fig:figS4}). 
...
In this work, we include lattice shear, blurring, as well as Gaussian and Poisson noise \textendash\ resulting in $31\,470$ data points. 
...
% The stride is $\sim 0.14\,\text{nm}$ (12 pixels). 
\textbf{b} The Bayesian CNN employed in the AI-STEM workflow (cf. Fig. \ref{fig:fig1}) provides a distribution and not only point estimates in the final output layer. The averaged classification probabilities can be used to identify the most likely class (\textbf{c}). An uncertainty estimate (a scalar value, cf. \textbf{b} and Eq. \ref{equation:mutual_information}) can be obtained via the via mutual information (\textbf{d}), revealing the grain boundaries as well as the amorphous region.}
 \label{fig:fig3}
 \end{figure*}
...
%The classification results are depicted in the central panel and the mutual information is depicted in the right panel of \textbf{a}. 
...
Specifically, for each local window, we reconstruct\cite{nord2017atomap} the real-space lattice from the atomic columns and determine\cite{myronenko2010point, gatti2022pycpd}  the angle of misalignment with respect to a reference training image or rather its reconstructed atomic columns (cf. Supplementary Note \ref{section:suppl_note_1} for more details).  
...
%It should be mentioned that these parameters can now be used to determine the type of grain boundary by for example considering the in plane crystallographic orientation of the training data (not performed here).
...
Smaller strides can suffice to reveal the main characteristics, cf. Fig. \ref{fig:figS3} (in particular, it is possible to separate an image into bulk and interface regions). 
...
%Using one GPU, an image can be decoded in about 30 seconds, while we also tested without use of GPUs, using a DELL Laptop and 4 of its CPUs (Intel Core i7-7820HQ CPU, 2.90GHz × 8), where the processing time can increase to minutes, depending on the stride.  
...
This provides visible enhancement of peak patterns around the central peak, which is visualized for all classes in this work in  Supplementary Figure \ref{fig:figS1}.
...
% shown in the main text in Eq. \ref{eq:1}.
\begin{equation}
\begin{aligned}
...
We apply additional noise sources for a subselection of data points (only every second rotated and sheared image, keeping the dataset size below 100k), including Gaussian blurring (scanning a Gaussian filter of certain width over the image), and finally, addition of random noise sources (Gaussian or Poisson). Visual examples are provided in Fig. \ref{fig:figS2}. 
...
%(insert zenodo link later).
%(NOMAD doi to be inserted)



\section*{Code availability}
%Python codes for neural networks and Jupyter notebook for regression models are available at . 
...
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:2212.02586}}}
  (\bibinfo{year}{2022}).
...
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:2207.10173}}}
  (\bibinfo{year}{2022}).
...
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:2208.13286}}}
  (\bibinfo{year}{2022}).
...
\newblock \bibinfo{journal}{\bibinfo{title}{{Dropout as a Bayesian
  Approximation: Representing Model Uncertainty in Deep Learning}}}.
\newblock {\emph{\JournalTitle{{ICML-16}}}}  (\bibinfo{year}{2016}).
...
\newblock In \emph{\bibinfo{booktitle}{Deep Learning and Data Labeling for
  Medical Applications: First International Workshop, LABELS 2016, and Second
  International Workshop, DLMIA 2016, Held in Conjunction with MICCAI 2016,
  Athens, Greece, October 21, 2016, Proceedings 1}}, \bibinfo{pages}{48--57}
...
\newblock \bibinfo{journal}{\bibinfo{title}{{Improving neural networks by
  preventing co-adaptation of feature detectors}}}.
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:1207.0580}}}
  (\bibinfo{year}{2012}).
...
\bibinfo{author}{Michelmore, R.}, \bibinfo{author}{Kwiatkowska, M.} \&
  \bibinfo{author}{Gal, Y.}
\newblock \bibinfo{journal}{\bibinfo{title}{{Evaluating Uncertainty
  Quantification in End-to-End Autonomous Driving Control}}}.
\newblock {\emph{\JournalTitle{{ArXiv e-prints}}}}  (\bibinfo{year}{2018}).
\newblock \eprint{1811.06817}.
...
\newblock {\emph{\JournalTitle{Open Research Europe}}}
  \textbf{\bibinfo{volume}{1}}, \bibinfo{pages}{13015} (\bibinfo{year}{2021}).
\newblock \doiprefix 10.12688/openreseurope.13015.1.
...
\newblock In \emph{\bibinfo{booktitle}{Proceedings of the 30th International
  ...
  ICML’13, \bibinfo{pages}{I–115–I–123} (\bibinfo{publisher}{JMLR.org},
  \bibinfo{year}{2013}).
...
\newblock \bibinfo{journal}{\bibinfo{title}{{UMAP: Uniform manifold
  approximation and projection for dimension reduction}}}.
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:1802.03426}}}
  (\bibinfo{year}{2018}).
...
\newblock \bibinfo{journal}{\bibinfo{title}{{Adam: A method for stochastic
  optimization}}}.
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:1412.6980}}}
  (\bibinfo{year}{2014}).
...
\newblock \bibinfo{journal}{\bibinfo{title}{{Bayesian active learning for
  classification and preference learning}}}.
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:1112.5745}}}
  (\bibinfo{year}{2011}).

\bibitem{smith2018understanding}
\bibinfo{author}{Smith, L.} \& \bibinfo{author}{Gal, Y.}
\newblock \bibinfo{journal}{\bibinfo{title}{Understanding measures of
  uncertainty for adversarial example detection}}.
\newblock {\emph{\JournalTitle{arXiv preprint arXiv:1803.08533}}}
  (\bibinfo{year}{2018}).
...
\newblock \doiprefix 10.1038/s41592-019-0686-2.
...
%We acknowledge funding from BiGmax, the Max Planck Society’s Research Network on Big-Data-Driven Materials Science. We acknowledge the Max Planck Computing and Data facility (MPCDF) for computational resources. 
...
%The authors thank Matthias Scheffler for initiating this research direction. 
%The authors thank Angelo Ziletti for preparing the initial proposal for this research project. 
B.C.Y. acknowledges funding from the National Research Foundation (NRF) of Korea under Project Number 2021M3A7C2090586.

\section*{Author contributions}
% All authors designed the idea and the project. B.C.Y. performed all the calculations. C.H.L. performed all the STEM experiments. B.C.Y., A.L., C.H.L., N. C., D. K., and L.M.G. discussed the final results. C.H.L., M.S., and L.M.G. supervised the project. B.C.Y. drafted the manuscript. All the authors approved the final version of the manuscript.
...
% A.L. and B.C.Y. drafted the manuscript. 
% A.L., B.C.Y., C.H.L., and L.M.G. provided comments on the manuscript.
% All the authors approved the final version of the manuscript.
...
\subsection{Supplementary Note 1 \label{section:suppl_note_1}}
...
In particular, a value has to be identified, for which the above quantities show convergent behavior with respect to $T$. 
 ...
 We investigate for each of the in total 576 local regions a range of Monte Carlo samples (10, 25, 50, 75, 100, 200, 300, 400). 
...
For instance, for $T=100$, one calculation (classification probabilities and mutual information for 576 local windows) takes around $\sim29.92$ seconds and doubling the number of MC samples doubles also the computation time to $59.52$ seconds. 
...
% \caption{
%\textbf{Gaussian-blurred HAADF and FFT-HAADF images at %atom peaks. }
%(a), (b), (c) Simulated and Gaussian-blurred HAADF %images (above) of a) fcc100, b) bcc100, and c) hcp0001 %with different standard deviation parameters ($\sigma$ %= 0.01, 0.02, 0.03) in a Gaussian distribution and %corresponding FFT-HAADF images (below).
% }
...
%  \caption{
% \textbf{The error matrices of the CNN. }
% (a) The result of the model performance when it used 90\% simulated FFT-HAADF image of total data points. (b) The result of the model performance when it used 10\% simulated FFT-HAADF image of total data points. The red boxes highlight the elements whose values are higher than zero. Otherwise, all the elements’ values except diagonal elements are equal to zero.
%  }
%  \label{fig:fig_caption}
%  \end{figure*}
 ...
 %\begin{figure*}[ht]
 %\centering
 %\includegraphics[width=0.7\textwidth]{figS4.png}
% \caption{
%\textbf{Application to an experimental STEM images with grain boundary.  }
%(a), (d), (g) The experimental HAADF images of a) Cu, d) Ti, and g) Fe crystals. The total size of each image is 6.0 $\times$ 6.0 nm$^2$. (b), (e), (h) The classification map of crystalline surfaces by the CNN of FFT-HAADF images of b) Cu, e) Ti, and h) Fe crystals for local windows of 1.2 $\times$ 1.2 nm$^2$ and their stride of 0.6 nm in x and y directions. (c), (f), (i) The uncertainty map of crystalline surfaces by the BNN of FFT-HAADF images of c) Cu, f) Ti, and i) Fe crystals for local windows of 1.2 $\times$ 1.2 nm$^2$ and their stride of 0.6 nm in x and y directions.  In the maps, the color intensity correlates with the value of mutual information.
% }
% \label{fig:fig_caption}
% \end{figure*}
...
% \begin{figure*}[ht]
% \centering
% \includegraphics[width=0.5\textwidth]{figS5.png}
% \caption{
%\textbf{Additional uncertainty map to an experimental STEM image of Cu crystal.  }
%(a) An experimental HAADF image. The white vertical and horizontal lines divide to 10-by-10 square windows. (b) The uncertainty map of crystalline surfaces by the BNN of FFT-HAADF images for local windows of 1.2 $\times$ 1.2 nm$^2$ and their stride of 0.6 nm in x and y directions. In the maps, the color intensity correlates with the value of mutual information.
% }
% \label{fig:fig_caption}
% \end{figure*}
