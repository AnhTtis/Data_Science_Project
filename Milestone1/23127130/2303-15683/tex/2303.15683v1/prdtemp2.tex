...
\documentclass[aps,prd,twocolumn,superscriptaddress,amsmath,amssymb,floatfix]{revtex4-2}
...
\author{Qi-Ling Chen}
% \email{chenql@ihep.ac.cn}
 \affiliation{College of Physics, Sichuan University, Chengdu 610064, China}
\author{Pei-Jin Hu}
% \email{hupeijin@stu.scu.edu.cn}
 \affiliation{College of Physics, Sichuan University, Chengdu 610064, China}
\author{Jing-Jing Su}
 \affiliation{College of Physics, Sichuan University, Chengdu 610064, China}

\author{Ming-Ming Kang}
 %\email[Authors to whom correspondence should be addressed:]{kangmm@ihep.ac.cn}
...
\author{Quan-Bu Gou}
 \affiliation{Key Laboratory of Particle Astrophysics, Institute of High Energy Physics, Chinese Academy of Sciences, Beijing 100049, China}
  %\author{Yi-Qing Guo}
 %\email{guoyq@ihep.ac.cn}
 %\affiliation{Key Laboratory of Particle Astrophysics, Institute of High Energy Physics, Chinese Academy of Sciences, Beijing 100049, China}\affiliation{University of Chinese Academy of Sciences, 19 A Yuquan Rd, Shijingshan District, Beijing 100049, P.R.China}
\author{Hong-Bo Hu}
 \affiliation{Key Laboratory of Particle Astrophysics, Institute of High Energy Physics, Chinese Academy of Sciences, Beijing 100049, China}
...
%\author{Ming-Ming Kang}
 %\email[Authors to whom correspondence should be addressed:]{kangmm@ihep.ac.cn}
 % \email{kangmm@scu.edu.cn}
%\affiliation{College of Physics, Sichuan University, Chengdu 610064, P.R. China}
...
\author{Bing-Qiang Qiao}
 \affiliation{Key Laboratory of Particle Astrophysics, Institute of High Energy Physics, Chinese Academy of Sciences, Beijing 100049, China}
\author{Hui-Ying Sun}
 \affiliation{School of Intelligent Enginerring, Shandong Management University, Jinan 250357, China}
\author{Xu Wang}
 \affiliation{School of Intelligent Enginerring, Shandong Management University, Jinan 250357, China}
\author{Zhen Wang}
...
\begin{abstract}
    %%%1. short GRB 高能辐射研究重要意义：本身辐射机制，LIV。
    The observation of short gamma ray bursts (SGRBs) in the TeV energy range plays an important role in understanding the radiation mechanism and probing new areas of physics such as Lorentz invariance violation. However, no SGRB has been observed in this energy range due to the short duration of SGRBs and the weakness of current experiments. New experiments with new technology are required to detect sub-TeV SGRBs.
    %%工作方法
    In this work, we  observe the very high energy (VHE) $\gamma$-ray emissions from SGRBs and calculate the annual detection rate with the High Altitude Detection of Astronomical Radiation HADAR (HADAR) experiment. First, a set of pseudo-SGRB samples is generated and checked using the observations of Fermi-GBM, Fermi-LAT, and SWIFT measurements. The annual detection rate is calculated from these SGRB samples based on the performance of the HADAR instrument. 
    %%%结果与结论
    As a result, the HADAR experiment can detect 0.5 SGRB per year if the spectral break-off of $\gamma$-rays caused by the internal absorption is larger than 100 GeV.
    For a GRB09010-like GRB in HADAR's view, it should be possible to detect approximately 2000 photons considering the internal absorption. With a time delay assumption due to the Lorentz invariance violation effects, a simulated light curve of GRB090510 has evident energy dependence. We hope that the HADAR experiment can perform the SGRB observations and test our calculations in the future. 
    %As a simple assumption as GRB090510 in HADAR view, we will detect more than 8000 photons. With a time delay assumption due to Lorentz invariance violation effects, a simulated light curve of GRB090510 have obvious energy dependence. We hope that HADAR experiment can perform the SGRB observations and test our calculations in future. 
\end{abstract}
...
\section{Introduction \label{Introduction}}
    %%%%
       %%%%
    GRBs are some of the most powerful explosions in the universe and can last from 10 ms to several hours as prompt emissions, releasing most of  the energy in the form of photons from 30 keV to a few MeV. The properties of GRBs mainly include two components of temporal and spectral information. First, from the point view of the temporal property, its prompt phase has a bimodal duration distribution separated at $t\sim 2$ s, indicating that there are two different groups as the long GRBs (LGRBs) with $t_{LGRB} > 2\ s$ and SGRBs with $t_{SGRB} <2\ s$ \cite{kouveliotou1993identification,meszaros2006gamma}. LGRBs are likely formed by the collapses of massive star cores \cite{woosley1993gamma,macfadyen1999collapsars}, while SGRBs are thought to come from the coalescence of compact binary systems, such as binary neutron stars (BNS) and neutron star-black hole (NS-BH) systems \cite{blinnikov1984exploding,paczynski1986gamma,eichler1989nucleosynthesis}. The associated events GW170817/GRB170817A \cite{abbott2017gravitational} from a BNS merger make the grounded assumption that BNS mergers are sources (or parts of the sources) of SGRBs.  For a spectrum, a two-component structure has been revealed above tens of MeV. At low energy, an experiential Band-function can describe observations well \citep{band1993batse,band2003comparison}, which is possibly induced by the synchrotron radiation process \citep{oganesyan2017detection,oganesyan2018characterization,oganesyan2019prompt,ravasio2018consistency}. At high energy, the model predicts that another component generated from the inverse-Compton (IC) process should exist \citep{meszaros1994delayed,dermer2000beaming,sari2001synchrotron,zhang2001high}. In the past few decades, great achievements have been made in experimental measurements to study temporal and spectral characteristics. It is evident that the measurements are relatively rich at low energy, but observations in the TeV band are more desirable. 
 ...
 %   The optimal choice is the IACT with wide FOV like HADAR project \citep{}. Therefore， it is necessary to know the detection capability of SGRBs for HADAR experiments. In this work, we present the annual detection rate based on the a set of pseudo SGRB samples is generated and checked by the observations of space-borne experiments.


    
    %$\left| \Delta t/ \Delta E \right|  <$ $0.03\ \text{ms GeV}^{-1}$ 
    %Furthermore,
    % the TeV $\gamma$-rays emissions from GRBs play very important roles in understanding their radiation mechanism and probing the new physics of Lorentz Invariance Violation (LIV).  The observations of LGRB at this energy have made great progress and 
    %although some inner sights of SGRBs have been revealed by past studies, a lot of fundamental questions stand in the way. On the one hand, observations of more VHE emissions will be a great step further into physics of SGRBs, such as the nature of their central engine \cite{king2005gamma,fryer2019understanding}, the mechanisms of jet formation \cite{meszaros1997poynting,kumar2015physics,ruiz2016binary}, the acceleration mechanisms of particles \cite{waxman1995cosmological,waxman1997high}. Also, it's necessary to verify SGRBs as electromagnetic counterparts of gravitational waves (GW) and constrain on their BNS sources. On the other hand, such observations can be probes for cosmological study because of SGRBs' high redshift, engaging in non-GRB physics like extragalactic background light (EBL; \cite{hartmann2003gamma}), intergalactic magnetic fields \cite{ichiki2008probing,takahashi2008detectability}, and the fundamental nature of spacetime \cite{wei2016limits}. And most importantly, the short-hard GRBs are ideal for the study of LIV, benefiting from their short duration and hard spectra \cite{amelino1998tests,martinez2006grb,scargle2008algorithm}. 
    %For observations of LIV effects, photons of higher energy lead to more distinguishable difference in speed from photons of low energy, thus causing an observational delay on the arrival time \cite{acciari2020bounds}. GRB090510 is a typical bright SGRB among high-energy observations, with a photon observed to 31 GeV arriving at 0.829 s after the trigger of Fermi-GBM. This SGRB provides conditions to constrain LIV effect and gives an upper limit of $\left| \Delta t/ \Delta E \right|  <$ $0.03\ \text{ms GeV}^{-1}$. 

    %Observations of SGRBs with Harder spectrum will help to better study the VHE or even ultra-high-enery physics in the universe. However, the very short durations have caused difficulties for detection. Up to now, only a few SGRBs are detected in the VHE range. With too small effective areas space-borne instruments could not reach enough sensitivity for VHE observations. Because of that, the large-area ground-based detection was proposed. Of the two ground-based observation techniques, the traditional extensive-air-shower (EAS) array method has a wide field of view (FOV) to better suit for full-sky searches, but is rather insensitive due to its limited ability to distinguish gamma/hadron and its poor resolution. The other one, the Cherenkov telescope method, of which the majority are the imaging atmospheric Cherenkov telescopes (IACTs), is inherently more sensitive in both energy and flux, such as the Cherenkov Telescope Array (CTA), MAGIC and H.E.S.S. GRB160821B was a nearby energetic short GRB at z=0.162, detected by space-borne Swift BAT \cite{siegel2016grb} , Fermi-GBM \cite{stanbro2016grb} in the prompt phase, and ground-based instrument MAGIC in the early aftefglow phase \cite{miceli2022gamma, acciari2021magic} , with a 24 s observation delay after $T_{0}$. With a limited energy band, detections of Swift BAT and Fermi-GBM did not reach TeV, while the MAGIC observations give a 0.5 $\sim$ 5 TeV range of photon energies. However, the analysis of MAGIC observations only give a ~ $3\sigma$ excess. With a rather narrow FOV, it's hard for IACTs to detect the energetic prompt phase, especially for emissions within seconds, like SGRBs.    Limited by these constraints of temporal methods and instruments, it is hard to detect the prompt VHE emissions from SGRBs despite the importance, where a ground-based instrument with excellent sensitivity and a low-energy threshold over a wide FOV is needed. In order to achieve this, the High Altitude Detection of Astronomical Radiation (HADAR) experiment has been carried out, which is sited at 4300 m above sea level in the Yangbajing Observatory in Tibet, China.

    Our work in this paper is organized as follows. First, we provide a brief introduction to the HADAR experiment and its performance parameters in Section~\ref{sec:HADAR}. Then, to predict the detection rate of SGRBs with HADAR as reliably as possible, we model the pseudo-SGRBs and the simulations of the detecting process with consideration of the performance of detectors for both Fermi-GBM (for consistency check) and HADAR. The simulation methods and results are shown in Sections~\ref{sec:SGRB} and ~\ref{sec:result}, respectively. In Section~\ref{sec:result}, we also describe the simulation of the observational results of GRB090510 in HADAR's case, given as a light curve with consideration of LIV effects. In Section \ref{sec:conclusion}, we describe the conclusion of our findings.  


\section{\label{sec:HADAR}HADAR Experiment}
    \begin{figure*}
...
\section{\label{sec:SGRB}Modeling SGRB Samples}
 ...
 %Based on theoretical framework, a set of pseusdo SGRB sample is produced and then tested by various experimental observation results. The 
 To estimate the detection rate of the prompt emissions from SGRBs for HADAR, we adopt a simulation method based on Monte Carlo. A set of SGRB samples is generated according to the phenomenological models of their intrinsic parameters, such as space density, luminosity, energy spectrum, temporal duration, and EBL attenuation. The expected detection rate is calculated using the spectra for each GRB and the HADAR sensitivity. %The resultant spectra for each GRB event are then calculated together with the HADAR sensitivity to determine the detection rate of the modelled samples.  
 
\subsection{Redshift Distribution}
    The redshift distribution describes the number of SGRBs per unit redshift bin $\mathrm{d}z$ per unit (observed) time $\mathrm{d}t$, which is written as \cite{zhang2018physics}:
    \begin{equation}
	...
	\end{equation}
	In our work, we adopt $H_0 = 67.4\ \mathrm{km}\ \mathrm{s}^{-1}\ \mathrm{Mpc}$, $\Omega_m = 0.315$, and $\Omega_{\Lambda} = 0.685$ \cite{pdg2020}. In addition, $\dot{\rho}(z)$ is the event rate density at z (in units of $\ \mathrm{Gpc}^{-3} \ \text{yr}^{-1}$), which is \cite{howell2019joint,regimbau2009gravitational}:
	\begin{equation}
		\dot{\rho}(z)\propto(1+z)\int_{t_{min}}^{t_{max}}R_F(z_f)P(t_d)dt_d \ ,
	\end{equation}
	where $P(t_d)\propto1/t_d$,\ $t_{min}=20\ \mathrm{Myr}$ is the minimum delay time for a BNS system to evolve to merge, and $t_{max}$ is the age of the universe at the time of merging $t(z)$. We normalize the event rate density $\dot{\rho}(z)$ with $1540^{+3200}_{-1220}\ \mathrm{Gpc}^{-3} \ \text{yr}^{-1}$ \cite{abbott2017gravitational}, which corresponds to the local event rate density at $z = 0$. 
...
\subsection{Prompt Emission Spectrum}
    ...
    The Band function is usually used to fit the spectrum of a GRB when the detector's energy band is wide enough. The Band function is \cite{band1993batse, band2003comparison}: 
    \begin{eqnarray}
        N(E)=A_0 \begin{cases}
        \left( \frac{E}{E_0} \right)^{\alpha} e^{\left(-E/E_p\right)}, & E \le E_c \\
        \left(\frac{E_c}{E_0}\right)^{\alpha-\beta}
        e^{(\beta-\alpha)}\left(\frac{E}{E_0}\right)^{\beta}, & E > E_c
        \end{cases}
    \end{eqnarray}
    where $E_0=100 \ \mathrm{keV}$ and $E_c=(\alpha-\beta)E_p$. $A_0$ is the normalization constant in units of $\mathrm{photons} \ \mathrm{s}^{-1} \ \mathrm{cm}^{-1} \ \mathrm{keV}^{-1}$. $\alpha$ and $\beta$ are the low and high energy photon indices, respectively, and $E_p$ is the peak energy of the spectrum. In this work, $\alpha$ and $\beta$ are adopted from the observations of Fermi-GBM \cite{poolakkil2021fermi}. $E_p$ is determined by the $E_p-L_p$ relationships examined by Ref.~\cite{tsutsui2013possible}.

    Beyond the Band function, a high-energy and power law spectral component is necessary to fit the spectrum in some GRBs \cite{abdo2009limit,ackermann2010fermi,ackermann2011detection}. The power law component is the dominant contribution to the high-energy prompt emission beyond tens of GeV. In this work,  we introduce the extra component for GRB spectra. The luminosity ratio $R_{ext} = L_{ext} / L_{ave}$ and spectral index $\beta_{ext}$ are used to describe the extra component. $L_{ext}$ is the luminosity of the extra component and $L_{ave}$ is the luminosity of the Band component. We take $R_{ext} = 0.1$, $\beta_{ext} = 1.5$. 


\subsection{$\gamma \gamma $ Absorption}

    The photons with the highest energies within the emission region may be strongly attenuated by the low-energy photons through the $\gamma \gamma \rightarrow e^{\pm}$ interaction. The specific spectral breaks occur because of the two-photon pair production, which is related to different bulk Lorentz factors and the fireball radius \cite{chen2018constraints,asano2007prompt}. In this work, we introduce an exponential cutoff on the spectrum to describe the internal absorption when calculating the detection rate. For typical burst parameters, the internal shock spectrum cuts off above a threshold energy of $E_{\gamma,th}  \sim 10 - 100\ \text{GeV}$ and has no internal cutoff with a large enough bulk Lorentz factor \cite{razzaque2004gev}, so the cases of 30 GeV, 50 GeV, 100 GeV, and 1 TeV with and without an energy cutoff are considered. 

    High-energy photons from distant astrophysical sources are subject to attenuation because of the two-photon pair production with the EBL. This absorption of $\gamma$-rays can be described as $e^{-\tau(E,z)}$, where $\tau(E,z)$ is the optical depth for the $\gamma$-rays at energy E. The EBL attenuation introduced in the work of Ref.~\cite{finke2010modeling} is used by default. Figure~\ref{fig:spectrum} shows the energy spectrum of the SGRBs. The EBL and internal absorption are considered. 



\subsection{Temporal Properties}
    The duration of the GRBs is approximately estimated using $T_{90}$, which corresponds to the time in which $90\%$ of the counts arrive. $T_{90}$ is described as \cite{ghisellini2010gev,kakuwa2012prospects}:
	\begin{equation}\label{eqnT90}
		T_{90}=(1+z)\frac{E_{iso}}{L_{ave}} \ ,
	\end{equation}
	where isotropic energy $E_{iso}$ is calculated with \cite{kakuwa2012prospects}:
	\begin{equation}
...
\subsection{Significance}
    ...
    \end{equation}	
    where $T_{90}$ is described in Equation.~\eqref{eqnT90}. N(E) is the spectrum of the SGRB. $A_{eff}$ is the effective area of the detector. For HADAR, the effective area is adopted from Ref.~\cite{xin2021prospects}. The factor $e^{-\tau(E,z)}$ is from the EBL attenuation. The integral limit $(E_{min},E_{max})$ is the detector's energy band, which is $(10\ \text{GeV},10\ \text{TeV})$ for HADAR. 
    
    For space-bone detectors such as GBM, $N_{bkg}$ comes from cosmic $\gamma$ background rates in the corresponding energy bands. %by direct multiplication of background photon rates and $T_{90}$. 
    For HADAR, %the background photon number is calculated by: 
    $N_{bkg}$ results from the cascades of cosmic rays:
	\begin{equation}
        N_{bkg}=\int_{E_{min}}^{E_{max}}S_p(E)T_{90}A_{eff}^{p}(E,\theta)\Omega(E)\mathrm{d}E \ ,
	\end{equation}
	where $S_p(E)$ represents the spectrum of cosmic rays. $A_{eff}^p$ is the effective area of HADAR corresponding to cosmic rays, and $\Omega(E)$ is the angular resolution of HADAR.  These are given in Ref.~\cite{xin2021prospects}.
...
\begin{figure*}
	...
	\caption{\label{fig:rate2}Top left: The SGRB detection rate with the extra + Band spectra considered for different energy breaks, in the case of $\beta_{ext}$ = -1.5, $R_{ext}=0.1$, and the EBL model \cite{finke2010modeling}. Top right: The SGRB detection rate with the extra + Band spectra considered for different EBL models, $\beta_{ext}$ = -1.5, $R_{ext}=0.1$. The solid lines are cases without internal absorption and the dashed line are the results when the energy spectrum is considered to cut off at 100 GeV. }
\end{figure*}
...
%    \begin{figure}
%        \includegraphics[width=0.45\textwidth]{./Rate_HADAR_0.1.pdf}
%        \caption{\label{fig:rate1}The SGRB dection rate of HADAR for different combinations or parameter for the extra component, in which the black line cyan line represents the Band spectrum, the yellow, blue, red lines represent the combination of the exra + Band components. Here the model of \cite{finke2010modeling} \textcolor{magenta}{Finke et al.(2010)} is considered, and no internal absorption is taken into account.}
%    \end{figure}

    Considering two-photon pair production within the emission region and during the propagation, the results are presented in Figure~\ref{fig:rate2}. It can be seen that the internal absorption prevents HADAR from observing the VHE photons to some extent. If the energy cutoff due to the two-photon pair production within the emission region is higher than 30 GeV, HADAR can detect at least 0.15 SGRB per year with the EBL model of Ref.~\cite{finke2010modeling}. The result in the right-hand panel of Figure~\ref{fig:rate2} shows that the different EBL models have a relatively small impact on the detection rate compared with that of the internal absorption. Adopting different EBL absorption models from Refs.~\cite{finke2010modeling,gilmore2012semi,stecker2012determination,inoue2013extragalactic}, the expected detection rate of HADAR is approximately 0.49--1.1 SGRBs per year with or without a 100 GeV energy cutoff. 
    

\subsection{\label{sec:lightcurve}Pseudo Light Curve of GRB090510}

    Based on our HADAR model and the parameters of GRB090510 extracted from observational results \cite{ackermann2010fermi}, including the redshift and spectrum, taking into account the model of the LIV effects for this GRB from Ref. ~\cite{abdo2009limit}, a pseudo light curve of HADAR observation is generated, as shown in the lower panels (\texttt{c}) and  (\texttt{d}) of Figure~\ref{fig:LC}, as well as each photon with the observed time and energy. 

    To display this light curve, we consider the photon energy distribution to follow the spectrum of 090510-like GRB combined with EBL attenuation. Then, these photons are detected by HADAR with effective areas and a $20\%$ energy resolution. An assumption is made that all photons from the source burst at exactly the same time, which means that the duration for the emission at the source is $\Delta t = 0$. For the LIV effects, a positive time delay $\Delta t = (\Delta E/E_{QG})D/c$ is included in the simulated arrival time of photons, where $E_{QG}=M_{QG}c^2$ is the Planck energy, D is the co-moving distance from source to detector, and $\Delta E = E_{\gamma, high} - E_{\gamma,low}$. The energy dispersion of the time delay $\Delta t/\Delta E = +0.03\ \text{s}/\text{GeV}$ is adopted \cite{abdo2009limit}. From Figure~\ref{fig:LC}, it is clear that HADAR works better in higher-energy photon detection than LAT, which is helpful for providing more constraints on the parameters of LIV effects. 
...
of China (12263004 and 12263005).
