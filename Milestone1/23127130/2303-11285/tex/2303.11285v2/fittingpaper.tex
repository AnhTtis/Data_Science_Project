...
In this paper we propose a method to determine the parameters of one such parameterisation, the one by Boyd-Grinstein-Lebed (BGL)~\cite{Boyd:1994tt}, without truncation errors. In order to make BGL and therefore our approach applicable to a wider range of decay channels, we also adopt a generalisation~\cite{Gubernari:2020eft,Gubernari:2022hxn,Blake:2022vfl} of the BGL unitarity constraint.
...
provided in Refs.~\cite{Szego:1939,Simon:2004,Gubernari:2022hxn,Blake:2022vfl}. Here we propose to modify just the unitarity constraint Eq.~\eqnref{eq:vanilla unitarity constraint}
...
\end{center}
\caption{Form factor data from FNAL/MILC 19~\cite{FermilabLattice:2019ikx}. The table shows form-factor reference values and errors
...
A simple modification of existing fit-codes should allow the 
modified unitarity constraint presented in Eq.~\eqnref{eq:modified unitarity constraint} to be imposed.
%
...
% \end{equation}
% For $B_s \to K\ell\nu$  decays the $t$-channel exchange has quantum numbers of a non-strange $B$ meson and the cut starts
...
expansion~\cite{Boyd:1994tt} with the modified unitarity constraint Eq.~(\ref{eq:modified unitarity constraint app}) agree exactly. While the implementation in Refs.~\cite{Gubernari:2020eft,Gubernari:2022hxn,Blake:2022vfl}
...
\end{center}
\caption{Results for the frequentist BGL fit to HPQCD 14~\cite{Bouchard:2014ypa}, FNAL/MILC 19~\cite{Bazavov:2019aom} and RBC/UKQCD 23~\cite{RBCUKQCD:2023}.
