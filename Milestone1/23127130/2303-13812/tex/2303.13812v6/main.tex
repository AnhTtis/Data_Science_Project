...
\noindent{\textbf{Keyword:}} Rectangular matrices $\cdot$ Beta ensembles $\cdot$ Type BC Bessel functions $\cdot$ Law of Large Numbers \\

\noindent{\textbf{Mathematical Subject Classification}} Primary 60B20; Secondary 60C05$\cdot$60F05 
\end{abstract}
...
The function $\B(\Vec{a};z_{1},...,z_{M};\theta,N)$ is known as multivariate type BC Bessel function in a general theoretical framework of special functions. Instead of introducing this framework, we give a more concrete definition of $\B(\Vec{a};z_{1},...,z_{M};\theta,N)$, and state its properties needed later in Section \ref{sec:typebc}, \ref{sec:dunkl}.
...
\end{equation}
See Appendix A for more details. Such limit expression has a clear probabilistic interpretation. Given deterministic M-tuples $\Vec{a}$ and $\Vec{b}$ as before, let $\Vec{c}=(c_{1},...,c_{M}\ge 0)$ be obtained by choosing uniformly random an element $\sigma$ in $S_{M}$, and taking 
...
\begin{remark}
    The same intermediate regime was considered in \cite{ABMV}, where the authors study the limiting behavior of Laguerre ensemble, and proved that its empirical measure converges in this regime to some deterministic probability measure $\mu_{q,\gamma}$, which interpolates between Marcenko-Pastur law (when $\gamma\rightarrow\infty$) and some version of Gamma distribution (when $\gamma\rightarrow 0$). As an application of the theory we develop in this text, we reobtain this result in Section \ref{sec:laguerre}. There are also similar results for Gaussian ensemble, see \cite{ABG}.
\end{remark}
...
\begin{remark}
    There exists an analog of Theorem \ref{thm:hightemperaturemainthm} in fixed temperature regime (for $\beta=1,2$), given in \cite{B2} using a different approach that relies on the concrete matrix structure.
\end{remark}
...
Take $\theta>0$, $M\le N$, the type BC multivariate Bessel function labeled by $\Vec{a}=(a_{1}\ge a_{2}\ge...\ge a_{M})$ is an multivariate analytic function in both $\Vec{a}$ and $(z_{1},...,z_{M})$, defined by 
...
\begin{thm}{\cite[ Section 13.4.3]{Forrester}}\label{thm:fouriertransform}
...
\end{split}
\end{align*}
where the matrix integral on the left is defined in the same way as in Definition \ref{def:matrixintegral}.
\end{thm}

\begin{remark}
 There are more than one way to show the equivalence of these two expressions. See the Appendix B for more information and yet another short proof of this result.

 Let us mention that the integral expression on the left can be identified (up to a factor $2i$ in the exponent) with the spherical function of the Euclidean symmetric spaces corresponding to $O(M+N)/O(M)\times O(N),U(M+N)/U(M)\times U(N),Sp(M+N)/Sp(M)\times Sp(N)$ respectively for $\theta=\frac{1}{2},1,2$. This is related to the fact that Bessel functions are eigenfunctions of Dunkl operators (presented in next section), since spherical function can also be defined as the (unique up to constant) eigenfunction of certain differential operators acting on its corresponding symmetric space. See e.g \cite{Hel2} for more details.
\end{remark}

\subsection{Type BC Dunkl operators}\label{sec:dunkl}
As a special class of differential operators, Dunkl operators were introduced in \cite{D}, and can be thought as a generalization of the usual partial derivatives on multivariate analytic functions, that take Fourier kernels as eigenfunction. For the purpose of this text, we specify to a special class of rational Dunkl operators under root system of type BC, which is parametrized by a single variable $\theta>0$ and plays a central role in Section \ref{sec:lowtemp}. For more detailed exposition of Dunkl theory, see \cite{Ro} and \cite{A}.
...
\begin{remark}
    $D_{i}'s$ specify \cite[Definition 2.7]{Ro} by setting the root multiplicity function to be $k_{e_{i}}=\theta(N-M+1)-\frac{1}{2}$, $k_{e_{i}+e_{j}}=\theta$.
\end{remark}
...
\end{prop}
The following result provides connection of type BC multivariate Bessel functions and Dunkl operators, namely, the former are eigenfunctions of the latter. We refer to \cite{Ro3} for a rigorous proof, but the statement itself was stated earlier in \cite[Section 6]{BF}.

\begin{definition}
    Fix $M\ge 1$. For $k=1,2,...$, denote
    $$\mathrm{P}_{k}=D_{1}^{k}+...+D_{M}^{k}.$$
\end{definition}
\begin{thm}\cite[Proposition 4.5]{Ro3}\label{thm:dunklonbessel}
Given $\Vec{a}=(a_{1}\ge ...\ge a_{M})$ for each $k=1,2,...$,
    \begin{equation}\label{eq_eigenfunction}
        \mathrm{P}_{2k}\B(\Vec{a}, z_{1},..,z_{M};\theta, N)=\left(\sum_{i=1}^{M}(a_{i})^{2k}\right)\cdot \B(\Vec{a},z_{1},...,z_{M};\theta, N).
    \end{equation}
\end{thm}

\begin{remark}
     From Definition \ref{def:bessel}, one can see that $\B(\Vec{a},z_{1},...,z_{M};\theta)$ is symmetric under actions of Weyl group of root system $BC_{M}$, namely, invariant by interchanging $z_{i}$ with $z_{j}$ and replacing $z_{i}$ by $-z_{i}$. Similarly, it's necessary to take symmetric power sum of $D_{i}'s$ with even power, which satisfies the same symmetry. 

\end{remark}

\subsection{Matrix addition and moments}\label{sec:addition}

For $\Vec{c}=\Vec{a}\boxplus_{M,N}^{\theta}\Vec{b}$, we assume in this section that $\Vec{a},\Vec{b}$ are deterministic, and recall from Definition \ref{def:deterministicaddition} that the distribution $\mathfrak{m}$ of $\Vec{c}$ is given by testing on type BC Bessel functions.
Note that polynomials are bounded and smooth on compact sets, and therefore are legitimate test functions of $\mathfrak{m}$. Moreover, since \ref{prop:characteristic2} Bessel function is analytic and symmetric on $\C^{M}$, we can view it as a generating function of symmetric polynomials of M variables $c_{1},...,c_{M}$. More precisely, by expanding Bessel functions on both sides of (\ref{eq_additioindef}) using (\ref{eq_expansion}), we have the following:
...
This coincides with the moments of $Z\sim \mathcal{N}(0,\frac{2}{N}a_{1}^{2}b_{1}^{2})$. By Example \ref{ex:usualbessel} and  
\cite[Section 1]{Ro3} or more explicitly \cite[Theorem 2]{MP}, which states that products of two usual Bessel functions can be written as a convex combination of Bessel functions, we have that for each $\theta>0$, $c_{1}^{2}$ is supported by a legitimate probability measure $\mu_{\theta}$. The convergence of second moment when $\theta\rightarrow\infty$ implies that $\{\mu_{\theta}\}_{\theta>0}$ are tight, hence (\ref{eq_fluctuation2}) follows from the moment convergence.
\end{proof}
...
\begin{proof}
    ...
    Writing $\exp(F(z_{1},...,z_{M}))$ as power series of $F(z_{1},...,z_{m})$. Since each term in $D_{i}$ reduces the total power of a monomial by 1, and $F(z_{1},...,z_{M})=\sum_{\mu:|\mu|\le 2k}c^{\mu}_{F}\cdot m_{\mu}(\Vec{z})$ where each $m_{\mu}(\Vec{z})$ is homogeneous of degree $|\mu|$,  we see that $b^{\lambda}_{\mu}$ is obtained from the action of $\prod_{i=1}^{l(\lambda)}D_{i}^{\lambda_{i}}$ on the single symmetric monomial $m_{\mu}(\Vec{z})$.
    
    Again let $l=l(\lambda)$. By Lemma \ref{lem:generic} and \ref{lem:generic2}, we first consider the generic part of $\prod_{i=1}^{l(\lambda)}D_{i}^{\lambda_{i}}m_{\mu}(\Vec{z})$. When $l(\mu)<l$, each monomial of $m_{\mu}(\Vec{z})$ is missing some variable among $z_{1},...,z_{M}$, say $z_{m}$. Then when acting the $\partial_{m}$ in (\ref{eq_symmetricdunklaction}), we get 0 since $\Big[D_{m-1}^{\lambda_{m-1}-1}\partial_{m-1}\Big]...\Big[D_{1}^{\lambda_{1}-1}\partial_{1}\Big]$ does not produce any power of $z_{m}$ to $m_{\mu}(\Vec{z})$. And the remaining part of the action gives $O(\frac{1}{M})$.
...
\begin{remark}
     It is well known (see e.g \cite[Proposition 9.8]{NS}) that non-crossing partitions are in bijection with the so-called Lukasiewicz paths, i.e, lattice paths whose steps take value in $\Z_{\ge -1}$ with starting and ending point at height 0. Therefore, the cumulant expression in (\ref{eq_cumulanttomomentcomb}) for $m_{2k}$ can be rewritten as a weighted sum over Lukasiewicz paths of length 2k. Such interpretation is used in the subsequent work \cite{KX} and \cite{CDM}, which studies random partitions under certain Jack measures. In particular, the moment expression in \cite[Theorem 3.7]{CDM} is similar to (\ref{eq_cumulanttomomentcomb}) and \cite[Theorem 3.10]{BCG}. But notice that in \cite{CDM} Lukasiewicz paths are not allowed to have horizontal steps at height 0.
\end{remark}
...
\end{lemma}
\begin{proof}
   (a) is a well known result that can be found in \cite[p378 and p380]{M}. (b) follows from Definition \ref{def:bessel} and (\ref{eq_jack2}), after specifying all but one variables to 0. 
\end{proof}
...
and $m_{2k-1}=0$ for $k=1,2,...$ Denote the map sending even moments to rectangular free cumulants by $\mathrm{T}^{\infty}_{m\rightarrow k}$, i.e, $\mathrm{T}^{\infty}_{m\rightarrow k}(\{m_{2k}\}_{k=1}^{\infty})=\{c_{l}\}_{l=1}^{\infty}$.
    \begin{thm}\label{thm:connectiontofreecumulant}
        ...
        $$\{k_{l}\}_{l=1}^{\infty}=\mathrm{T}_{m\rightarrow k}^{q,\gamma}(\{m_{2k}\}_{k=1}^{\infty}),\quad r_{l}^{\gamma}=(2\gamma)^{l-1}q^{\frac{l}{2}}\cdot k_{l}.$$
...
\begin{thm}
    ...
    weakly in moment, where $\mu_{q,\gamma}$ is a probability measure on $\R_{\ge 0}$, which is uniquely determined by its moments:
...
\end{thm}
\begin{remark}
    The weak convergence of $\mu_{M,N,\theta}$ to $\mu_{q,\gamma}$ in probability was already proved in \cite{ABMV}, in which the authors give an explicit density of $\mu_{q,\gamma}$ in terms of Whitakker function. 
\end{remark}
\begin{proof}
    ...
    Since the existence of probability measure $\mu_{q,\gamma}$ is known by \cite{ABMV}, it just remains to check that the moments in (\ref{eq_cumulanttomomentlaguerre}) does correspond to a unique probability measure. This is the so-called Stieltjes moment problem, since the (potential) corresponding measure lies on $[0,\infty)$, see e.g \cite{Ak}.
    We need to check
...
\section{Appendix A: Limit transition of type BC Bessel functions}
...
\begin{proof}
    This follows from a straightforward calculation. Indeed, by Definition \ref{def:bessel}, 
    \begin{equation}
        \begin{split}
...
\section{Appendix B: Power series expression of Harish-Chandra integrals}
...
\end{align*}
where the third equality follows from Lemma \ref{lem:hooklength} and (see \cite[(10,20)]{M}) 
...
\end{proof}
\subsection*{Acknowledgements} The author is grateful to Vadim Gorin for a lot of stimulating discussions, and all his useful suggestions on the presentation of this text. We thank Grigori Olshanski for pointing out one useful reference, Simon Marshall for explaining some basics of symmetric spaces, and Margit Roesler for clarification of a technical issue in her lecture notes. We are also indebted to Florent Benaych-Georges, Pierre Mergny, Michael Voit and Cesar Cuenca for their valuable comments. The project was partially supported by NSF grants DMS-1949820 and DMS - 2152588.
\section{Financial interests}
The project was partially supported by NSF grants DMS-1949820 and DMS - 2152588.
...
\bibitem[ABG]{ABG} R. Allez, J.-P. Bouchaud,  A. Guionnet, Invariant Beta ensembles and the Gauss-Wigner crossover, \emph{Phys. Rev. Lett.} 109, 094102 (2012).

\bibitem[ABMV]{ABMV} R. Allez, J.P. Bouchaud2, S. N. Majumdar, P. Vivo, Invariant $\beta$-Wishart ensembles, crossover densities and asymptotic corrections to the Marchenko-Pastur law \emph{Journal of Physics A: Mathematical and Theoretical}, Vol 46, 015001, (2013).

\bibitem[Ak]{Ak} N. I. Akhiezer, The Classical Moment Problem and Some Related Questions in Analysis, \emph{SIAM}, (2020).

\bibitem[AP]{AP}O. Arizmendi, D. Perales, Cumulants for finite free convolution. \emph{Journal of Combinatorial
Theory, Series A} 155, (2018), 244-266. arXiv:1611.06598

\bibitem[B1]{B1} F. Benaych-Georges, Rectangular random matrices, related convolution, \emph{Probability Theory and Related Fields},
...
\bibitem[BF]{BF} T. H. Baker and P. J. Forrester, The Calogero–Sutherland model and generalized classical polynomials, \emph{Commnications in  Mathematical Physics}, 188, 175–216, (1997).
...
\bibitem[BG2]{BG2} A. Bufetov, V. Gorin, Fourier transform on high-dimensional unitary groups with applications to random tilings, 	\emph{Duke Math. J.}, 168, no. 13 (2019), 2559-2649

\bibitem[CDM]{CDM} C. Cuenca, M. Dolega, A. Moll, Universality of global asymptotics of Jack-deformed random Young diagrams at varying temperatures. arXiv:2304.04089
...
matics Research Notices}, no. 3 (2020), 883-913.
...
\bibitem[GW]{GW} T. Guhr, T. Wettig, An Itzykson–Zuber‐like integral and diffusion for complex ordinary and supermatrices,  \emph{Journal of Mathematical Physics} 37, 6395-6413, (1996).
...
\bibitem{KX}{KX} D. Keating, J. Xu, Edge universality of $\beta$-additions through Dunkl operators. 	arXiv:2411.12149

\bibitem[M]{M} I. G. Macdonald, Symmetric functions and Hall polynomials, second edition, \emph{Oxford University
Press}, (1995)

\bibitem[Me]{Me} P. Mergny, Spherical integrals and their applications to random
matrix theory, \emph{Université Paris-Saclay}, (2022).

\bibitem[Me2]{Me2} P. Mergny, An explicit example for the high temperature convolution: crossover between the binomial law B(2,1/2) and the arcsine law. 	arXiv:2204.07744

\bibitem[MeP]{MeP} P. Mergny, M. Potters, Rank one HCIZ at high temperature: interpolating between classical and free convolutions, \emph{SciPost Physics}, 12, 022 (2022). 
...
\bibitem[NS]{NS} A. Nica, R. Speicher, Lectures on the Combinatorics of Free Probability, \emph{London Mathematical
...
\bibitem[Ro3]{Ro3} M. Rösler, Bessel convolutions on matrix cones. \emph{Compositio Mathematica}, 143(3) (2007), 749-779. 	arXiv:math/0512474.
