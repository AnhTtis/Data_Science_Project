...
\begin{abstract}
A nonnegative integer matrix is said to have width 1 if its nonzero entries lie along a path consisting of steps to the south and to the east. 
...
We prove two strikingly different formulas.  The first, a $\leftidx{_4}{F}{_3}$ hypergeometric series with unit argument, is obtained by applying the Robinson--Schensted--Knuth correspondence to the width-one matrices; the second is obtained via Stanley--Reisner theory.
Computationally, our two formulas are in a sense dual to each other: the first formula outperforms the second if $d$ is fixed and $n$ increases, while the second outperforms the first if $n$ is fixed and $d$ increases.
...
\keywords{RSK correspondence, generalized hypergeometric series, order complex, Stanley--Reisner rings, Stanley decompositions, optimal transport, earth mover's distance}
...
\begin{rem}
    The only reason we restrict our attention to \emph{square} width-one matrices is to keep notation uncluttered; as the reader will observe, it is trivial to adapt our final formulas for matrices of any dimensions $n_1 \times n_2$.
\end{rem}
...
\begin{equation}
    ...
    \T(d,n) \coloneqq \{ T \in {\rm M}_n(\N) : \text{$T$ has width 1 and $\sum_{i,j} T_{ij} = d$}\}.
\end{equation}
...
%\textcolor{red}{Should we be more specific? I.e., mention CPU in a footnote, or even mention complexity. If implemented "efficiently", computing $\binom{n}{k}$ has complexity $O(nk)$, so the total $\SS$ with this formula has $O(d (2d^2 + 2d(n+d)))$, which asymptotically should be $O(4d^3+2d^2n)$?}
%Note that $\SS(10000,30)$ is, by definition, the sum of approximately $1.4 \times 10^{170}$ width-one matrices.  
...
% \end{align*}
% obtained by viewing each matrix in $\T(d,n)$ as the exponent matrix of a monomial in $K[\Delta_n]_d$.
% It follows that 
%     \begin{equation}
