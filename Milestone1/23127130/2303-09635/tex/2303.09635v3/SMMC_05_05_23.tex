...
\partial_{x_i}\left(w_i({\bm x})+m_{ij} x_j\right)\\ \nonumber  & + \kappa_{ij}\partial_{x_i}\partial_{x_j}+D_{ik;j\ell}\partial_{x_i}x_k  \partial_{x_j} x_{\ell},
\end{align}
...
\end{gather}
is normalizable if $c(\phi)>0$. Then expectation of the cost evaluated according to Eqs.~(\ref{eq:steady-control})  is finite and convex at $c(\phi)=c_0+c_1\phi >D(\max(q,2)-1)$, i.e. the system is M$q$P-stable at $\phi>\phi^{(s)}= (D(\max(q,2)-1)-c_0)/c_1$. The optimal value is achieved at $\phi^* >\phi^{(s)}$ which returns the following explicit expression at $q=2$:
\begin{align}
...
\end{align}
Here  ${\bm \theta}=(\theta_i:i\in\V)$; and $\hat{l}_i,\hat{l}_{ij}$ are the first order differential operators in ${\bm \theta}$: $\hat{l}_i= \partial_{\theta_i} \theta_i$, 
$\hat{l}_{ij}=(\partial_{\theta_i}\!-\!\partial_{\theta_j})(\theta_i\!-\!\theta_j)$.
...
\end{align}
Notice that the stationary version of the last line in Eq.~(\ref{eq:swim-general-statistics}) settles if $\phi>\bar{\lambda}_1+1/(2S''(\bar{\lambda}_1))$ and that it is fully consistent with Eq.~(\ref{eq:P-sw}) derived for the short ($\delta$)-correlated velocity gradient,  where $S''_1(\bar{\lambda}_1)=1/((d-1)D)$ and $\bar{\lambda}_1=(d-1)^2 D/2$. 
