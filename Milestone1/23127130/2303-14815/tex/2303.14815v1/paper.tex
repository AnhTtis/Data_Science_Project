...
%\usepackage{wasysym}           %\astrosun \earth \jupiter
%\usepackage{url}
%\usepackage{eurosym}
%\usepackage{authblk}
...
It is shown that an exact mapping onto a set of $N+1$
ordinary differential equations exists when the
respective delay distribution is given in terms 
of a gamma distribution with discrete exponents. 
The number of auxiliary variables one needs to
introduce, $N$, is inversely proportional to 
the variance of the delay distribution. The case of
a single delay is therefore recovered when 
$N\to\infty$. Using this approach, denoted the `kernel 
series framework', we examine systematically how
the bifurcation phase diagram of the Mackey-Glass 
system changes under the influence of distributed
delays. We find that local properties, f.i.\ the
locus of a Hopf bifurcation, are robust against
the introduction of broadened memory kernels.
Period-doubling transitions and the onset of 
chaos, which involve non-local properties of the 
flow, are found in contrast to be more sensible to 
distributed delays. Our results indicate that modeling
approaches of real-world processes should take 
the effects of distributed delay times into account.
\end{abstract}
\maketitle
...
Here we show that dynamical systems with 
distributed time delays can be mapped onto sets 
of ordinary differential equations (ODEs), with
the resulting ODE system taking a particular simple
form when the distribution of the delays is described
by a gamma distribution with integer exponents.
This method, the kernel series framework, allows
for the systematic study of delay differential equations
(DDEs) with distributed delays.
...
The method is presented in Section\,\ref{sec:theory},
...
topology of the resulting attractors differ
...
is be given by a superposition of past states 
$x(t-\tau)$,
%
\begin{equation}
h(t) = \int_0^\infty x(t-\tau) K(\tau)d\tau,
\qquad\quad
\int_0^\infty K(\tau)d\tau=1\,,
...
\begin{figure}[t]
\centering
\includegraphics[]{kernel_convergence.png}
...
% %
% \begin{equation}
% K^{(N, T)}_{m}(\tau) = \frac{\tau^{m-1}}{(m-1)!\,T_N^m}\,
% \erm^{-\tau / T_N},
% \qquad\quad T_N = \frac{T}{N}\,.
% \label{eq:kernel_def}
% \end{equation}
% %
...
There are many representations of the Dirac 
$\delta$-functions commonly used. What makes 
(\ref{eq:kernel_def}) especially interesting
is that the individual kernels can be evaluated
by a simple additional ordinary differential
equation. For this purpose auxiliary 
variables $x_m$ are introduced via
%
\begin{equation}
x_m(t) = \int_0^\infty x(t-\tau) K_m^{(N,T)}(\tau)\,d\tau, 
\qquad\quad
m = 1, \dots, N\,.
\label{eq_def_x_m}
\end{equation}
%
At face value, it may seem that one needs to
store the full history of $x(t)$ for the evaluation
of the convolutions defining the $x_m$. This is 
however not the case. The reason is 
that the $x_m(t)$ form a closed set of recursive
differential equations \cite{gedeon1999upper},
%
\begin{equation}
...
\dot{x}_m(t) &=&
\int_0^\infty K_m^{(N,T)}(\tau) \dv{t} x_0(t-\tau) d\tau
\\
&=& -\int_0^\infty K_m^{(N,T)}(\tau) \dv{\tau} x_0(t-\tau)d\tau\,
\nonumber
\end{eqnarray}
%
with an integration by parts of the last expression 
leading directly to (\ref{eq_dot_x_m}). 
...
(\ref{eq:general_dde}) as a proxy for
...
%---------------------------
\subsection{Generalized state history}
%---------------------------
...
framework formulated by (\ref{eq:kernel_approx}),
...
\begin{figure}[t]
\centering
\includegraphics[]{hopf_bifurcation_new.png}
\caption{{\bf Hopf bifurcation.}
\textbf{(a)} As a function of the average time delay $T$, 
the numerically obtained eigenvalues of the Jacobian (\ref{eq:linear_Jacobian}) 
$\lambda = p+\irm q$, for $a = 0.4$ and $b = 0.1$
and the linear delay system (\ref{eq:characteristic_eq}).
...
for the kernel series framework. Shown are results for
$N=3,\,10,\,100$.
}
...
It is amendable to an analytic solution \cite{gros2015complex}.
...
\begin{figure}[t]
\centering
\includegraphics[]{period_doublings_new.png}
\caption{
...
point, as definded by (\ref{eq:mu_hopf}).
...
\begin{figure}[t]
\centering
\includegraphics[]{attractors.png}
\caption{{\bf Mackey-Glass system.}
The stroboscopic projection $(x_0(t),x_N(t-T))$ of attractors 
of the Mackey-Glass system (\ref{eq:mg}), where
$x_0(t)=x(t)$. Shown are numerical solutions of 
...
bifurcations occuring in (\ref{eq:mg})
changes when the relative width 
$\sigma/\mu=1/\sqrt{N}$,
see (\ref{eq_mean_variance}), of the delay 
distribution becomes finite. For an overview
...
projection, plotting $x_N(t-T)$ as a function
...
distributions of time delays with finite widths 
...
\begin{figure}[t]
\centering
\includegraphics[]{lyapunov_exponents_new.png}
\caption{
...
within the time scale of the initial exponential 
...
\begin{figure}[t]
\centering
\includegraphics[]{wernecke_test_chaos.png}
...
%
\begin{equation}
h(t) = \int_0^\infty  \kappa(\tau) x(t-\tau)d\tau,
\qquad\quad
\int_0^\infty  \kappa(\tau) d\tau = 1\,,
...
\qquad\quad 
c_i = \int_0^\infty \psi_i(t; \beta) \kappa(t) dt\,,
...
time delays are important. On a fundamental
...
In this paper, we studied numerically the influence of
time delay distributions for the Mackey-Glass 
system. Good agreement is observed in the local 
regime for the stability of fixed points, for which
we prove analytically that corrections scale as $1/N$.
We also find that higher-order phenomena, such as 
period doubling transitions or the occurrence of 
chaotic dynamics, are substantially more sensitive
to the introduction of distributed delays. It may 
hence be difficult to compare the predictions of 
dynamical systems with precisely defined time 
delays with observations, in particular when
distributed time delays play an important
role in the respective real-world applications.
...
occuring within a kernel series framework of order $N$,
...
The authors thank Bulcs\'u S\'andor
