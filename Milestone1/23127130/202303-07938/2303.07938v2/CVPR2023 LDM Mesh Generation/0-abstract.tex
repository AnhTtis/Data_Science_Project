% The diffusion model was shown to achieve good results on the task of point cloud generation. Further, we want to achieve good results on the mesh generation task because the mesh will be used more often in practice.Our model is more interpretable than LION because we use the key points as the latent variables to represent the shape, which will also accelerate the diffusion model's inference time.
...
To boost the efficiency and controllability of our mesh generation method,
...
Extensive experiments are conducted on the ShapeNet dataset, where our proposed sparse latent point diffusion model achieves superior performance in terms of generation quality and controllability when compared to existing methods.
Project page, code and appendix: \url{https://slide-3d.github.io}.
