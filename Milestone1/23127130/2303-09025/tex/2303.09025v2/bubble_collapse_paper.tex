...
]{revtex4-2} % revtex4-2 is used instead of revtex4-1, as the latter is outdated and does not exist anymore
...
% \preprint{AIP/123-QED} % what does this do?
...
\par
Even for ideal hydrodynamics the compressibility of the fluid has a strong influence on the extent to which energy density is concentrated. Motivated by this perspective we present a calculation for $n(\gamma)$ for a wide range of $\gamma$. Key results are displayed in Fig.~\ref{figure_results}. These calculations might motivate a search for candidate liquids to achieve greater levels of energy focusing. These calculations also provide an asymptotic limit that can be used to evaluate accuracy of more general numerical solutions, for example, simulations of all-Mach number bubble dynamics \cite{FUSTER2018752}. 

\begin{figure}
\includegraphics[width=4.8in, height=2.0in]{bubble_collapse_paper_fig_1_v2.png} %for two-column figure, 6.69 inches x 8.25 inches. 
\caption{Predicted values of $n$ as a function of $\gamma$ are denoted as dots, and the dashed line corresponding to $n = 0.4$ is the value for the incompressible fluid. In (a), values of $\gamma$ are those close to the water that has $\gamma = 7$, and the solid line is the proposed fit of the form $n = 0.4 + a \gamma^{-b}$. In (b), a larger range of $\gamma$ is considered.}
\label{figure_results}
\end{figure}

\section{Theory}
In this section, we give a summary of the methods used to perform the computations \cite{hunter_1960}.
...
\bibliography{bubble_collapse_paper}% Produces the bibliography via BibTeX.
