...
By employing the optimal test norm, the `ideal' method yields the best approximation to the pair of the Helmholtz solution and its scaled gradient 
w.r.t.~the norm on $L_2(\Omega)\times L_2(\Omega)^d$ from the selected finite element trial space.
On convex polygons, the `practical', implementable method is shown to be pollution-free when the polynomial degree of the finite element test space grows proportionally with $\log \kappa$.
...
We write the Helmholtz equation as an ultra-weak first order variational system $\langle \mathbbm{u}, B_\kappa' \mathbbm{v}\rangle_U=q(\mathbbm{v})$ ($ \mathbbm{v} \in V_\mp$). Here,  $\mathbbm{u}=(\phi,\frac{1}{\kappa} \nabla \phi)$, where $\phi$ is the Helmholtz solution, and $B_\kappa'$ is a partial differential operator of first order.
We show that for any $\kappa>0$ this formulation is well-posed in the sense that $B_\kappa$ is  a boundedly invertible operator
 from $U:=L_2(\Omega)\times L_2(\Omega)^d$ to the dual space $V_\mp'$, where $V_\mp$ is a closed subspace of $H^1(\Omega)\times H(\divv;\Omega)$ defined by the incorporation of homogeneous boundary conditions (the symbol $\pm$ refers to either of the possible signs in the Robin boundary condition).
...
The use of the optimal test norm has been advocated in \cite{64.14,45.44}, see also \cite{35.8565}.
Presented in a somewhat different way, it can also be found in \cite{19.83}.

\subsection{`Practical' method}
The residual minimizer w.r.t.~the norm $\sup_{0 \neq \mathbbm{v}\in V_\mp} \frac{|\cdot(\mathbbm{v})|}{\|B_\kappa' \mathbbm{v}\|_{U}}$ on $V_\mp'$ is however not computable.
...
we obtain an implementable `practical' method. Its solution, that we again denote with $\mathbbm{u}^\delta$, is the second component of the pair $(\mathbbm{v}^\delta,\mathbbm{u}^\delta) \in V_\pm^\delta \times U^\delta$ that solves 
\be \label{zadel}
\left\{
\begin{array}{lcll}
...
These trace variables are measured in intrinsically stronger norms, which has the consequence that in order to guarantee a certain rate for the field variables stronger regularity conditions are needed. The analysis in \cite{64.155,75.63} was restricted to the `ideal' method in which the residual is minimized in the non-computable dual norm.
\end{remark}
...
\ee
In other words, the error in $\mathbbm{u}^\delta$ is at most a factor $1/\gamma^\delta_\kappa$ larger than the error in the best approximation from $U^\delta$, and for some $\mathbbm{u}$, it will be a factor $1/\gamma^\delta_\kappa$ larger. We will therefore call $1/\gamma^\delta_\kappa$ the pollution factor of the method.
The constant $\gamma^\delta_\kappa$ can be computed as the square root of the smallest eigenvalue of a generalized eigenvalue problem in terms of $\kappa$, $U^\delta$ and $V^\delta$.
...
The least squares approximation $\mathbbm{u}^{\delta}$ can be called to be quasi-best when $\delta$ is from a collection $\Delta$ for which 
 ...
 Numerical results for uniform triangulations of the unit square confirm that the Galerkin method is only pollution-free under the conditions of $\frac{h \kappa}{p}$ and $\frac{\log \kappa}{p}$ being sufficiently small, whereas the ultra-weak FOSLS discretization is pollution free whenever the polynomial degree $\tilde{p}$ at the test side is sufficiently large dependent on $p$,
  and $\frac{\log \kappa}{\tilde{p}}$  is sufficiently small.
 ...
 An iterative solution of this problem, however,  requires only a preconditioner for the Hermitian positive definite left upper block, which might be easier than the iterative solution of the indefinite matrix-vector system resulting from the standard Galerkin discretization. This will be study of future work.
...
By $C \lesssim D$ we will mean that $C$ can be bounded by a multiple of $D$, \emph{independently} of parameters which $C$ and $D$ may depend on, as the wave number $\kappa$ or the discretisation index $\delta$.
...
Let $\Omega \subset \R^d$ be a bounded Lipschitz domain with boundary $\partial\Omega$ decomposed into $\Gamma_D \,\dot{\cup}\, \Gamma_N \,\dot{\cup}\,\Gamma_R$, where $|\Gamma_R|>0$, and let $\kappa\geq \kappa_0>0$. 
...
and $g \in H^{-\frac12}(\Gamma_N \cup \Gamma_R)$ (=$H_{00}^{\frac12}(\Gamma_N \cup \Gamma_R)'$), we consider the Helmholtz equation with (mixed) Dirichlet, Neumann and/or Robin boundary conditions of finding $\phi\in H^1(\Omega)$ that satisfies
...
$$
\phi\in H^1_{\kappa g_D,\Gamma_D}(\Omega):=\{\breve{\phi} \in H^1(\Omega)\colon \breve{\phi}|_{\Gamma_D}=\kappa g_D \text{ on } \Gamma_D\}
$$
such that 
\be \label{2nd}
\begin{split}
...
\be \label{1storder}
\begin{aligned}
-\tfrac{1}{\kappa}\nabla \cdot \vec{u}- \phi&= f_1 & &\text{on } \Omega,\\
...
\int_\Omega  \tfrac{1}{\kappa} \vec{u}\cdot\nabla \overline{\eta} -  \phi \overline{\eta}- \tfrac{1}{\kappa} \phi \,\nabla \cdot \overline{\vec{v}}- \vec{u}\cdot \overline{\vec{v}}\,dx\\
...
$$
\nabla\cdot \vec{u}+\kappa \phi=0,\quad
...
Taking $(\eta,\vec{v}) \in \{0\}\times H_{0,\Gamma_N \cup \Gamma_R}(\divv;\Omega)$ in \eqref{8} we infer that
\be \label{e0}
\phi \in H^1_{0,\Gamma_D}(\Omega).
\ee
...
$$
From $\nabla\phi=\kappa \vec{u}$ and $\nabla\cdot \vec{u}+\kappa \phi=0$, we infer that for 
...
\\ & = - \tfrac{1}{\kappa} \int_\Omega (\nabla \cdot \vec{u}+ \kappa \phi) \overline{\eta}\,dx=0.
\end{align*}
...
$$
q(\eta,\vec{v})=\int_\Omega \vec{w}\cdot \overline{\vec{v}}+\nabla\cdot \vec{w} \,\nabla\cdot \overline{\vec{v}}+\zeta \overline{\eta}+\nabla \zeta \cdot \nabla \overline{\eta} \,dx
\quad( (\eta,\vec{v}) \in V_{\mp}).
$$
Setting $\undertilde{\vec{u}}=\vec{u}+\vec{w}$, $\undertilde{\phi}=\phi +\kappa \nabla \cdot \vec{w}$,
the equation $B_\kappa (\phi,\vec{u})=q$ is equivalent to 
\be \label{3}
\begin{split}
\int_\Omega \tfrac{1}{\kappa} \undertilde{\vec{u}} \cdot \nabla \overline{\eta}&- \undertilde{\phi} \overline{\eta}-  \tfrac{1}{\kappa} \undertilde{\phi} \,\nabla\cdot \overline{\vec{v}}-  \undertilde{\vec{u}}  \cdot 
\overline{\vec{v}}\,dx=\\
& \int_\Omega 
(\zeta-\kappa \nabla \cdot \vec{w})\overline{\eta}+(\nabla \zeta+\tfrac{\vec{w}}{\kappa})\cdot \nabla \overline{\eta}\,dx  =:\undertilde{q}(\eta)\quad((\eta,\vec{v}) \in V_{\mp}),
\end{split}
\ee
...
\begin{remark}(Alternative choice for domain and codomain of $B_\kappa$)  The boundary condition $\vec{v}\cdot\vec{n} \mp i \eta =0$ on $\Gamma_R$, which is incorporated in the definition of $V_{\mp}$, will pose some challenges for both analysis and implementation. Considering $\partial\Omega=\Gamma_R$, alternatively we expect that well-posedness similarly to Theorem~\ref{thm:main} can also be shown for $V_{\mp}$ and $U$ replaced by $H^1(\Omega) \times H(\divv;\Omega)$ and $L_2(\Omega) \times L_2(\Omega)^d \times H^{\frac12}(\partial\Omega)$, respectively, the latter factor being the space for an additional variable being the trace $\phi|_{\partial\Omega}$. In this case, however, for non-smooth solutions best approximation errors in $L_2(\Omega) \times L_2(\Omega)^d \times H^{\frac12}(\partial\Omega)$ will generally be dominated by the error in this additional trace variable. We therefore do not investigate this option.
\end{remark}

\subsection{Adjoint problem} 
From $U \simeq U'$, one has that $B_{\kappa} \in \Lis(U,V_{\mp}')$ is equivalent to 
$$
B'_{\kappa}\colon (\eta,\vec{v}) \mapsto (- \tfrac{1}{\kappa} \nabla \cdot \vec{v}- \eta, \tfrac{1}{\kappa} \nabla \eta- \vec{v}) \in \Lis(V_{\mp},U).
$$
...
(L'_\kappa \eta)(\zeta):=\int_\Omega \tfrac{1}{\kappa^2} \nabla \eta  \cdot \nabla \overline{\zeta}-\eta \overline{\zeta}\,dx \mp \tfrac{i}{\kappa}
 \int_{\Gamma_R} \eta \overline{\zeta}\,ds=\int_\Omega ( h_1 -\tfrac{1}{\kappa} \nabla\cdot \vec{h}_2)  \overline{\zeta} \,dx.
$$
...
=\int_\Omega (-\tfrac{1}{\kappa} \nabla \cdot (\vec{h}_2+\vec{v}) -\eta)  \overline{\zeta}\,dx=\int_\Omega (h_1 -\tfrac{1}{\kappa} \nabla\cdot \vec{h}_2)  \overline{\zeta} \,dx.
\end{align*}
...
$$
It shows that $\vec{v}  \in H(\divv;\Omega)$, and from a subsequent integration-by-parts one infers $- \tfrac{1}{\kappa} \nabla \cdot \vec{v}- \eta=h_1$, and
...
Since furthermore  $B'_{\kappa}\in \cL(H^1(\Omega) \times H(\divv;\Omega),U)$, and
\begin{align*}
...
means error minimization in the norm on $U=L_2(\Omega)\times L_2(\Omega)^d$.
...
Assuming a finite dimensional $U^\delta$, now consider a finite dimensional subspace $V_{\mp}^\delta \subset V_{\mp}$ for which
$$
\gamma^\delta_\kappa:=\inf_{0 \neq \undertilde{\mathbbm{u}}^\delta \in U^\delta} \sup_{0 \neq \undertilde{\mathbbm{v}}^\delta \in V_{\mp}^\delta} \frac{|
(B_\kappa \undertilde{\mathbbm{u}}^\delta)(\undertilde{\mathbbm{v}}^\delta)|}{\|\undertilde{\mathbbm{u}}^\delta\|_U\|\undertilde{\mathbbm{v}}^\delta\|_{V_{\pm,\kappa}}}>0,
$$
...
\right.\hspace*{-1.2em}
\ee
has a unique solution, which is computable. For notational convenience from here on we will denote $\hat{\mathbbm{u}}^{\delta}$ again by $\mathbbm{u}^\delta=(\phi^\delta,\vec{u}^\delta)$.
...
\begin{proof} With $B_\kappa^\delta\colon U^\delta \rightarrow V_{\mp}^\delta$, ${B_\kappa^\delta}'\colon V_{\mp}^\delta \rightarrow U^\delta$ defined by
$(B^\delta \undertilde{\mathbbm{u}}^\delta)(\undertilde{\mathbbm{v}}^\delta)=(B \undertilde{\mathbbm{u}}^\delta)(\undertilde{\mathbbm{v}}^\delta)=\langle {B_\kappa^\delta}'\undertilde{\mathbbm{v}}^\delta,\undertilde{\mathbbm{u}}^\delta\rangle_U$, \eqref{saddle-discrete} reads as the system 
$B_\kappa^\delta {B_\kappa^\delta}' \mathbbm{v}^\delta+B_\kappa^\delta \mathbbm{u}^\delta=q|_{V_{\mp}^\delta}$, ${B_\kappa^\delta}' \mathbbm{v}^\delta=0$.
So $\mathbbm{u}^\delta$ is the unique solution of the Schur complement ${B_\kappa^\delta}' (B_\kappa^\delta {B_\kappa^\delta}')^{-1} (B_\kappa^\delta \mathbbm{u}^\delta-q|_{V_{\mp}^\delta})=0$, or equivalently, of the Petrov-Galerkin discretisation
$(B_\kappa u^\delta)(\undertilde{\mathbbm{v}}^\delta )=q(\undertilde{\mathbbm{v}}^\delta )$ for $\undertilde{\mathbbm{v}}^\delta \in \ran (B_\kappa^\delta {B_\kappa^\delta}')^{-1}B_\kappa^\delta$.
 
 Let $\undertilde{\mathbbm{z}}^\delta \in (\ran (B_\kappa^\delta {B_\kappa^\delta}')^{-1}B_\kappa^\delta)^{\perp_{\langle \cdot,\cdot \rangle_{V_{\pm,\kappa}}}}$ and $\undertilde{\mathbbm{u}}^\delta \in U^\delta$. Setting $\undertilde{\mathbbm{v}}^\delta:=(B_\kappa^\delta {B_\kappa^\delta}')^{-1}B_\kappa^\delta\undertilde{\mathbbm{u}}^\delta$, it holds that
 $$
 0=\langle {B_\kappa^\delta}' \undertilde{\mathbbm{z}}^\delta, {B_\kappa^\delta}' \undertilde{\mathbbm{v}}^\delta\rangle_U=
 (B_\kappa^\delta {B_\kappa^\delta}' \undertilde{\mathbbm{v}}^\delta)( \undertilde{\mathbbm{z}}^\delta)=
 (B_\kappa \undertilde{\mathbbm{u}}^\delta)( \undertilde{\mathbbm{z}}^\delta).
 $$
 One infers that consequently
 the value of inf-sup constant $\gamma^\delta_\kappa$ remains unchanged when in its definition the space $V_{\mp}^\delta$ is replaced by its subspace $\ran (B_\kappa^\delta {B_\kappa^\delta}')^{-1}B_\kappa^\delta$. 
...
\begin{remark} One easily infers that $\mathbbm{u} \mapsto  \mathbbm{u}^\delta$ is a projector onto $U^\delta$ with $\|\mathbbm{u} \mapsto  \mathbbm{u}^\delta\|_{\cL(U,U)} \leq \frac{1}{\gamma^\delta_\kappa}$. Even $\|\mathbbm{u} \mapsto  \mathbbm{u}^\delta\|_{\cL(U,U)} = \frac{1}{\gamma^\delta_\kappa}$ is valid (see \cite[Thm.~3.1]{249.99}).
\end{remark}
...
\begin{remark} \label{remmie}
In view of the equivalent definition of $\mathbbm{u}^\delta$ as the solution of a Petrov-Galerkin discretisation, we refer to $U^\delta$ as the \emph{trial space}, and $V_{\mp}^\delta$ as the \emph{test space} (actually its subspace $\ran T^\delta|_{U^\delta}$ is the test space in the Petrov-Galerkin discretisation, and therefore in the literature $V_{\mp}^\delta$ is  sometimes called the test search space).
\end{remark}
...
We will refer to the `practical' method discussed in this subsection as the \emph{ultra-weak FOSLS}, or shortly, \emph{FOSLS} method or discretisation, with the note that its solution not only depends on $U^\delta$ but also on $V^\delta_{\mp}$. \medskip
...
To show uniform inf-sup stability, one can use the characterization of $\gamma_\kappa^\delta$ being the reciprocal of the $\cL\big((V_{\mp},\|\cdot\|_{V_{\mp},\kappa}), (V_{\mp},\|\cdot\|_{V_{\mp},\kappa})\big)$-norm of the Fortin interpolator\footnote{A linear mapping $\Pi^\delta\colon V_{\mp} \rightarrow V_{\mp}^\delta$ with $(B_\kappa U^\delta)(\ran (\identity-\Pi^\delta))=0$.} with smallest norm.
We will rely on an alternative expression for $\gamma_\kappa^\delta$ in terms of approximability from $V_{\mp}^\delta$ of the solution of an adjoint problem with a forcing function from $U^\delta$ which we recall next.

\begin{proposition}[{\cite[Proposition 2.5]{35.8565}}] \label{proppie}
With $P^\delta\colon V_\mp \rightarrow V_\mp^\delta$ being the $\langle\cdot,\cdot\rangle_{V_\mp,\kappa}$-orthogonal projector onto $V_{\mp}^\delta$,
it holds that
$$
\|(\identity-P^\delta){B_\kappa'}^{-1}\|_{\cL\big(U^\delta,(V_\mp,\|\cdot\|_{V_{\mp,\kappa}})\big)}^2 =1-(\gamma_k^\delta)^2.
$$
\end{proposition}
...
}{\|\mathbbm{w}^\delta\|_U^2} = 1-(\gamma^\delta_\kappa)^2. \qedhere
\end{align*}
\end{proof}
...
Notice, however, that generally the solution of the Helmholtz problem can be expected to exhibit oscillations with wavelength $\sim \frac{2\pi}{\kappa}$, so that from an approximation point of view, the case of $\kappa h_{\delta,\min}$ being sufficiently large is not the most relevant one.

Restricting ourselves to quasi-uniform meshes, in Sect.~\ref{sec:practical} we will study the most relevant case of $\kappa h_{\delta,\min}$ being uniformly bounded.
...
$$
For $p \in \N_0$, with $\cS_p^{-1}(\tria^\delta):=\{v \in L_2(\Omega)\colon v|_K \in P_p(K)\,\,(K \in \tria)\}$, and
$\cS_p^{0}(\tria^\delta):=\cS^{-1}(\tria) \cap C(\overline{\Omega})$, let
$$
...
$\gamma_\kappa^\delta \gtrsim 1$, only dependent on $d$, $p$, and the shape regularity of $\tria^\delta$.
\end{theorem}
...
For $q \in P_p(K)$, it holds that $\int_K b_K |q|^2\,dx \gtrsim \int_K |q|^2\,dx \gtrsim \int_K b^2_K |q|^2\,dx$, which shows that
$$
...
Let $\hat{K}$ some fixed reference $d$-simplex. Since an $\vec{f} \in \RT_{p+1}(\hat{K}) \cap H_0(\divv;\hat{K})$ is uniquely determined by the DoFs 
$\vec{q} \mapsto \int_{\hat{K}} \vec{f} \cdot \vec{q}\,dx$ ($\vec{q} \in P_k(\hat{K})$), it holds that
$$
 \inf_{0 \neq \vec{q} \in P_{p}(\hat{K})^d} \sup_{0\neq \vec{f} \in \RT_{p+1}(\hat{K}) \cap H_0(\divv;\hat{K})} \frac{\Re \langle \vec{q},\vec{f}\rangle_{L_2(\hat{K})^d}}{\|\vec{q}\|_{L_2(\hat{K})^d}\|\vec{f}\|_{L_2(\hat{K})^d}}\gtrsim 1.
$$
By employing a contravariant Piola transformation this results carries over to each uniformly shape regular $d$-simplex $K$.


We conclude that with $\hat{V}^\delta:=\{\mathbbm{v} \in L_2(\Omega)^{d+1}\colon \mathbbm{v}|_K \in (P_{p+d+1}(K) \cap H^1_0(K)) \times (\RT_{p+1}(K) \cap H_0(\divv;K))\}$,
\be \label{r1}
...
Using an inverse inequality on $\hat{V}^\delta$, for $(\eta,\vec{v}) \in \hat{V}^\delta$ it holds that
\be \label{r2}
\|B_\kappa'(\eta,\vec{v})+(\eta,\vec{v})\|_{U}=\tfrac{1}{\kappa} \|(-\divv \vec{v},\nabla \eta)\|_{U} \lesssim \tfrac{1}{\kappa h_{\delta,\min}} \|(\eta,\vec{v})\|_{U},
\ee
and so obviously $\|B_\kappa'(\eta,\vec{v})\|_U-\|(\eta,\vec{v})\|_{U} \lesssim \tfrac{1}{\kappa h_{\delta,\min}} \|(\eta,\vec{v})\|_{U}$.

From \eqref{r1}, \eqref{r2}, and
\begin{align*}
...
\end{align*}
we infer inf-sup stability $\gamma_\kappa^\delta \geq 1$ for the the pair $(U^\delta,\hat{V}^\delta)$ when $\kappa h_{\delta,\min}$ is sufficiently large. Obviously this inf-sup stability remains valid when we replace $\hat{V}^\delta$ by the larger space
$V^\delta_{\mp}$.
\end{proof}

\begin{remark} The results in this and the next subsection about inf-sup stability will be shown for  for pairs $(U^\delta,V^\delta_\mp)$ where $U^\delta$ is a space of discontinuous piecewise polynomials. Obviously, these results carry over to the case when such a space of discontinuous piecewise polynomials is replaced by the smaller space 
 of continuous piecewise polynomials.
\end{remark}

\

\subsection{The case of $\kappa h_\delta$ uniformly bounded} \label{sec:practical}
In this subsection, we assume that the conforming subdivision $\tria^\delta$ of $\Omega$ into $d$-simplices is \emph{quasi-uniform}, i.e., that
$$
h_\delta:=h_{\delta,\min} \eqsim h_{\delta,\max}:=\max_{K \in \tria^\delta} |K|^{1/d}.
$$
For some $p \in \N_0$, again we take  $U^\delta:=\cS_p^{-1}(\tria^\delta)^{d+1}$.
We consider the most relevant case that $\kappa h_\delta \lesssim 1$. Notice that this case together with the case of $\kappa h_\delta$ being sufficiently large, dealt with in the previous subsection, covers \emph{all} possible combinations of quasi-uniform partitions and $\kappa \geq \kappa_0>0$.

For our analysis we equip  $H^1(\Omega)$ and $H(\divv;\Omega)$ with the $\kappa$-dependent norms
\begin{align*}
\|\cdot\|_{1,\kappa}& :=\sqrt{\|\cdot\|_{L_2(\Omega)}^2+\tfrac{1}{\kappa^{2}} |\cdot|_{H^1(\Omega)}^2}\,\,,\\
\|\cdot\|_{\divv,\kappa}& :=\sqrt{\|\cdot\|_{L_2(\Omega)}^2+\tfrac{1}{\kappa^{2}} \|\divv \cdot\|_{L_2(\Omega)}^2}\,\,,
...
and
$$
\|\divv \vec{v}\|_{L_2(\Omega)} \leq \sum_i \|\divv \vec{v}_i\|_{L_2(\Omega)} \lesssim \sum_i  \sum_k |\vec{r}\cdot\vec{e}_k|\|\partial_k (\chi_i \phi)\|_{L_2(\Omega)}
...
\ee
Recall that $h_\delta$ is the mesh-size of $\tria^\delta$, and that the adjoint $L_\kappa'$ equals $L_\kappa$ with $\pm$ replaced by $\mp$.
...
\begin{theorem} \label{thm:fine} W.r.t.~a quasi-uniform shape regular simplicial partition $\tria^\delta$ of $\Omega$ with mesh-size $h_\delta$, let $U^\delta=\cS_p^{-1}(\tria^\delta)^{d+1}$, and let $V_\mp^\delta$ be as in \eqref{301}. 
Besides the conditions needed to guarantee \eqref{bdrnorms} and \eqref{MS}, assume that $\max_{K \in \tilde{\tria}^\delta} \diam(K) \lesssim h_\delta$, $\kappa h_\delta \lesssim 1$, and let $\tilde{p}$ be sufficiently large, dependent on $p$ and the shape regularity of $\tria^\delta$. Then $\gamma_\kappa^\delta \gtrsim 1$.
\end{theorem}
...
\begin{proof} The proof consists of five steps.  {\bf (I)}  In view of Proposition~\ref{proppie} we have to show that
\be \label{200}
\sqrt{1-(\gamma_\kappa^\delta)^2} =\|(\identity-P^\delta) {B_\kappa'}^{-1}\|_{\cL\big(U^\delta,(V_\mp,\|\cdot\|_{V_\pm,\kappa})\big)} \leq \rho<1.
\ee
It means that we have to show approximability from $V_{\mp}^\delta$ of the solution of the adjoint (first order ultra-weak) Helmholtz problem with data from $U^\delta$.
We are going to use that such data has some smoothness beyond being in $U$.

We set $U_1:=H^1(\Omega)\times H_0^1(\Omega)^d$, equipped with squared norm
$$
\|(\phi,\vec{u})\|^2_{U_1}:=
\|\phi\|^2_{L_2(\Omega)}+h_\delta^2|\phi|^2_{H^1(\Omega)}+ \|\vec{u}\|^2_{L_2(\Omega)^d}+h_\delta^2|\vec{u}|^2_{H^1(\Omega)^d}.
$$
From $\|(\identity-P^\delta) (B_\kappa')^{-1}\|_{\cL(U,(V_{\mp},\|\cdot\|_{V_{\mp},\kappa}))} = 1$, we infer that for $s \in [0,1]$,
$$
\|(\identity-P^\delta) (B_\kappa')^{-1}\|_{\cL([U,U_1]_{s,2},(V_{\mp},\|\cdot\|_{V_{\mp},\kappa}))} \leq \|(\identity-P^\delta) (B_\kappa')^{-1}\|^s_{\cL(U_1,(V_{\mp},\|\cdot\|_{V_{\mp},\kappa}))}.
$$
For $s \in [0,\frac12)$, $[U,U_1]_{s,2} \simeq H^s(\Omega)\times H^s(\Omega)^d$ with a squared norm that is equivalent, uniformly in $h_\delta$, to
$
\|\phi\|^2_{L_2(\Omega)}+h_\delta^{2s}\|\phi\|^2_{H^s(\Omega)}+ \|\vec{u}\|^2_{L_2(\Omega)^d}+h_\delta^{2s}\|\vec{u}\|^2_{H^s(\Omega)^d}$.
For $s$ in this range, it holds that $U^\delta \subset [U,U_1]_{s,2}$, and there exists a constant $C_{\text{inv}}$, only dependent on $p$, the shape regularity of $\tria^\delta$, and on $s$ when it tends to $\frac12$,
such that
$$
\|(\phi^\delta,\vec{u}^\delta)\|_{[U,U_1]_{s,2}} \leq C_{\text{inv}} \|(\phi^\delta,\vec{u}^\delta)\|_{U} \quad ((\phi^\delta,\vec{u}^\delta) \in U^\delta).
$$
We conclude that \eqref{200} holds whenever for some $s\in [0,\frac12)$,
\be \label{p2}
C_{\text{inv}} \|(\identity-P^\delta) (B_\kappa')^{-1}\|^s_{\cL(U_1,(V_{\mp},\|\cdot\|_{V_{\mp},\kappa}))} \leq \varrho<1.
\ee

{\bf (II)} Given $(\phi,\vec{u}) \in U_1$, let $(\eta,\vec{v}) \in V_{\mp}$ denote the solution of
$$
B'_\kappa(\eta,\vec{v})=(-\kappa^{-1} \divv \vec{v}-\eta,\kappa^{-1} \nabla \eta-\vec{v})=(\phi,\vec{u}).
$$
Verifying \eqref{p2} amounts to showing that
$$
\sup_{0 \neq (\phi,\vec{u}) \in U_1}\inf_{(\eta^\delta,\vec{v}^\delta) \in V^\delta_{\mp}} \frac{\|(\eta-\eta^\delta,\vec{v}-\vec{v}^\delta)\|_{V_{\mp},\kappa}}{\|(\phi,\vec{u})\|_{U_1}} \leq (C_{\text{inv}}^{-1} \varrho)^{1/s}.
$$
An application of \eqref{triangle}\footnote{Knowing that this inequality can be arbitrarily crude, see Remark~\ref{crude}, if we could avoid its use, then it might be possible to relax on the assumption~\eqref{MS}.} shows that it suffices to show that
\be \label{p5}
\sup_{0 \neq (\phi,\vec{u}) \in U_1}\inf_{(\eta^\delta,\vec{v}^\delta) \in V^\delta_{\mp}} \frac{\|\eta-\eta^\delta\|_{1,\kappa}+\|\vec{v}-\vec{v}^\delta\|_{\divv,\kappa}}{\|(\phi,\vec{u})\|_{U_1}} \leq \tfrac12 \sqrt{2}\, (C_{\text{inv}}^{-1} \varrho)^{1/s}.
\ee
As shown in Lemma~\ref{lem:easy}, $\eta$ is the solution in $H^1(\Omega)$ of $L_\kappa' \eta= \phi-\kappa^{-1} \divv \vec{u}$.
...
{\bf (III)} To construct a similarly accurate approximation $\vec{v}^\delta \in RT_{\tilde{p}}(\tilde{\tria}^\delta)$ to $\vec{v}=\kappa \nabla \eta -\vec{u}$ in $\|\cdot\|_{\divv,\kappa}$-norm, a complication is the boundary condition incorporated in the definition of $V^\delta_{\mp}$. The pair $(\eta^\delta,\vec{v}^\delta)$ 
has to satisfy $\vec{v}^\delta\cdot \vec{n} \mp i \eta^\delta=0$ on $\partial\Omega$.

Lemma~\ref{lem3} shows that
\be \label{p6}
\begin{split}
\|(\vec{v}\cdot\vec{n} \mp i \eta^\delta)|_{\partial\Omega}\|_{-\frac12,\kappa}&=
\|(\eta -\eta^\delta)|_{\partial\Omega}\|_{-\frac12,\kappa} \\
& \lesssim \| (\eta -\eta^\delta)|_{\partial\Omega}\|_{\frac12,\kappa} \lesssim 
\|\eta -\eta^\delta\|_{1,\kappa}.
\end{split}
\ee
The definition of $\|\cdot\|_{-\frac12,\kappa}$ shows the existence of a $\vec{w} \in H(\divv;\Omega)$ with 
\be \label{p7}
\vec{w}\cdot\vec{n}|_{\partial\Omega}=(\vec{v}\cdot\vec{n} \mp i \eta^\delta)|_{\partial\Omega},\quad
\|\vec{w}\|_{\divv,\kappa} \lesssim \|(\vec{v}\cdot\vec{n} \mp i \eta^\delta)|_{\partial\Omega}\|_{-\frac12,\kappa}.
\ee

Knowing that the range of the normal trace operator restricted to $\RT_{\tilde{p}}(\tilde{\tria}^\delta)$ contains the range of the trace operator restricted to $\cS^0_{\tilde{p}}(\tilde{\tria}^\delta)$, there exists a $\vec{z}^\delta\in \RT_{\tilde{p}}(\tilde{\tria}^\delta)$ with $(\vec{z}^\delta\cdot\vec{n} \mp i \eta^\delta)|_{\partial\Omega}=0$.
Noticing that $(\vec{v}-\vec{w}-\vec{z}^\delta) \in H_0(\divv;\Omega)$, with $Q^\delta\colon H_0(\divv;\Omega) \rightarrow \RT_{\tilde{p}}(\tilde{\tria}^\delta) \cap H_0(\divv;\Omega)$ being the projector constructed in \cite{70.991},
we select
$$
\vec{v}^\delta :=Q^\delta (\vec{v}-\vec{w}-\vec{z}^\delta)+\vec{z}^\delta.
$$
and conclude that $(\eta^\delta, \vec{v}^\delta) \in V_{\mp}^\delta$.
What remains is to bound $\|\vec{v}-\vec{v}^\delta\|_{\divv,\kappa}$.

{\bf (IV)}  The projector $Q^\delta$ satisfies the commuting diagram property $\divv Q^\delta=\Pi^\delta \divv$, where $\Pi^\delta$ is the $L_2(\Omega)$-orthogonal projector onto $\cS^{-1}_{\tilde{p}}(\tilde{\tria}^\delta)$, and for $K \in \tilde{\tria}^\delta$,
\be \label{Voh}
\begin{split}
...
\kappa^{-1}\|\divv(\vec{v}-Q^\delta (\vec{v}-\vec{w}-\vec{z}^\delta)-\vec{z}^\delta)\|_{L_2(\Omega)}\\
&=
...
&\lesssim \|\eta -\eta^\delta\|_{L_2(\Omega)}+\tfrac{h_\delta}{\tilde{p}}|\phi|_{H^1(\Omega)}\\
&\lesssim \|\eta -\eta^\delta\|_{1,\kappa}+\tfrac{1}{\tilde{p}} \|(\phi,\vec{u})\|_{U_1}.
\end{split}
\ee
...
{\bf (V)} What remains is to show such an upper bound for  $\|\vec{v}-\vec{v}^\delta\|_{L_2(\Omega)}$. 
We write $\vec{v}-\vec{v}^\delta=(\identity-Q^\delta)(\vec{v}-\vec{w}-\vec{z}^\delta)+\vec{w}$.
It holds that
\be \label{ee2}
...
\begin{align} \nonumber
\|(&\identity-Q^\delta)(\vec{v}-\vec{w}-\vec{z}^\delta)\|_{L_2(\Omega)}
\lesssim \tfrac{h_\delta}{\tilde{p}} \|(\identity-\Pi^\delta)\divv(\vec{v}-\vec{w}-\vec{z}^\delta)\|_{L_2(\Omega)}\\  \nonumber
&\hspace*{4cm}+
\sqrt{\sum_{K \in \tilde{\tria}^\delta} \min_{\vec{q}_K \in \RT_{\tilde{p}}(K)}\| \vec{v}-\vec{w}-\vec{z}^\delta-\vec{q}_K\|^2_{L_2(K)}}\\  \nonumber
...
\\ \nonumber
&\hspace*{5cm} \min_{\vec{q}^\delta \in \cS^{-1,\tilde{p}}(\tilde{\tria}^\delta)^d} \|\kappa^{-1} \nabla \eta -\vec{u}-\vec{q}^\delta\|_{L_2(\Omega)^d}\\ \label{ee4}
&\lesssim
...
for  \eqref{ee4} that $\tfrac{h_\delta}{\tilde{p}} \leq h_\delta \lesssim \kappa^{-1}$ and $\nabla \cS^{0,\tilde{p}}(\tilde{\tria}^\delta) \subset \cS^{-1,\tilde{p}}(\tilde{\tria}^\delta)^d$,
for  \eqref{ee5} equations \eqref{ee2} and \eqref{ee1}, and
for  \eqref{ee6} equation \eqref{p4}.
We conclude that $\|\vec{v}-\vec{v}^\delta\|_{L_2(\Omega)} \lesssim \tfrac{1}{\tilde{p}} \|(\phi,\vec{u})\|_{U_1}$, which together with \eqref{p10} and \eqref{p4} shows \eqref{p5} when $\tilde{p}$ is sufficiently large, and thus completes the proof.
%%%%%%%%%
\end{proof}
%%%%%%%%%
...
As in the proof of Theorem~\ref{thm:coarse}, for $\tilde{Y}^\delta := \prod_{K \in \tria^\delta} P_{p+d+1}(K) \cap H^1_0(K) \subset H^1_{0,\Gamma_D}(\Omega)$, it holds that
$$
...
$$
(L_\kappa' \phi^\delta)(\eta)=\sum_{K \in \tria^\delta} \big\{\int_K (-\kappa^{-2} \Delta\phi^\delta-\phi^\delta) \overline{\eta}\,dx+\int_{\partial K} \tfrac{\partial \phi^\delta}{\partial\vec{n}} \overline{\eta}\big\} \mp \tfrac{i}{\kappa} \int_{\Gamma_R} \phi^\delta \overline{\eta}\,ds,
$$
...
$$
Thanks to $\kappa h_{\max}^\delta \lesssim 1$ it satisfies the first condition in \eqref{q5}. In two steps we will construct a modified interpolator that also satisfies \eqref{q4}.
...
$$
Thanks to the condition $\kappa h_{\delta,\max} \lesssim 1$, this latter estimate implies the first condition in \eqref{q5} and so completes the proof.
\end{proof}
...
The next theorem shows that $\mathbbm{u}^\delta+B_\kappa'\mathbbm{v}^\delta$ is always more accurate than $\mathbbm{u}^\delta$ (more precisely, never less accurate), and, assuming $\gamma_\kappa^\delta \gtrsim 1$, that it is a quasi-best approximation from $U^\delta+ (B_\kappa' V^\delta \cap (U^\delta)^{\perp})$.
...
and
$$
\|\mathbbm{u}-(\mathbbm{u}^\delta+B_\kappa'\mathbbm{v}^\delta)\|_U \leq \frac{1}{\gamma_\kappa^\delta} \,\,\inf_{\mathbbm{w}^\delta \in U^\delta+ (B_\kappa' V^\delta \cap (U^\delta)^{\perp})} \|\mathbbm{u}-\mathbbm{w}^\delta\|_U.
$$
\end{theorem}
...
From \eqref{300} one infers that $P_2^\delta:=\mathbbm{u} \mapsto \mathbbm{u}^\delta+B_\kappa'\mathbbm{v}^\delta$ is a projector onto $U^\delta+ (B_\kappa' V^\delta \cap (U^\delta)^{\perp})$.
Equation \eqref{201} shows that $\|\identity - P_2^\delta\|_{\cL(U,U)}\leq \|\identity - P_1^\delta\|_{\cL(U,U)}=\frac{1}{\gamma_\kappa^\delta}$.
From
$$
\|(\identity - P_2^\delta)\mathbbm{u}\|_U = \inf_{\mathbbm{w}^\delta \in U^\delta+ (B_\kappa' V^\delta \cap (U^\delta)^{\perp})}\|(\identity - P_2^\delta)(\mathbbm{u}-\mathbbm{w}^\delta)\|_U,
$$
...
$$
is an increasingly more accurate approximation to $\mathbbm{u}$ than $\mathbbm{u}^\delta$. Moreover, having solved the saddle point system \eqref{201}, one gets this improved approximation for free. On the other hand, the representation of $\mathbbm{u}^\delta+B_\kappa'\mathbbm{v}^\delta$ requires more storage than that of $\mathbbm{u}^\delta$. Furthermore, because of the unusual `trial space' $U^\delta+ (B_\kappa' V^\delta \cap (U^\delta)^{\perp})$, it is not so clear how to compare this boosted approximation with a Galerkin approximation using a standard finite element space.

\begin{remark}[$L L^*$-method] Keeping $V^\delta$, but replacing $U^\delta$ by $\{0\}$, the boosted FOSLS approximation reduces to finding $\mathbbm{v}^\delta \in V^\delta$ that solves
...
\min_{\tilde{\mathbbm{v}}^\delta \in V^\delta} \|B_\kappa'^{-1} \mathbbm{u}-\tilde{\mathbbm{v}}^\delta\|_{V_{\mp,\kappa}}$.
...
$V_\mp^\delta=(\cS_{p+2}^{0}(\tria^\delta) \times \RT_{p+2}(\tria^\delta)) \cap V_\mp$, and for the Galerkin method,
...
Given any fixed degree $p$, the pollution factor for the FOSLS method can be uniformly controlled when the polynomial degree $\tilde{p}$ at the test size grows proportionally with $\log \kappa$.
...
Because of $\|\cdot\|_{1,\kappa}=\|(\cdot,\frac{1}{\kappa}\nabla\cdot)\|_U$, these four errors can also be mutually compared.
...
When number of degrees of freedom in $V_\mp^\delta$ per wavelength is, however, less than $2$, the boosted solution cannot provide a meaningful approximation, and so the error estimator is of no use.
...
$$
see left picture in Figure~\ref{fig:domainplusmesh}.
We set $f=g_D=0$ and 
...
and for the FOSLS case, the effectivity index of the a posteriori error estimator. To compute these quantities, we have replaced the unknown exact solution $(\phi,\vec{u})$ by the boosted FOSLS solution with trial- and test-spaces $\cS_4^{0}(\tilde{\tria})\times \cS_4^{0}(\tilde{\tria})^2$ and
$(\cS_{6}^{0}(\tilde{\tria}) \times \RT_{6}(\tilde{\tria})) \cap V_\mp$ w.r.t.~a sufficiently fine adaptively refined partition $\tilde{\tria}$.
\begin{figure}[h!]
\begin{subfigure}{.5\textwidth}
\centering
...
The performance of the practical, implementable method, which uses a discretized dual norm, is governed by the inf-sup constant $\gamma_\kappa^\delta$ determined by $\kappa$ and trial and test spaces.
Its reciprocal is the pollution factor of the method, which is computable.
...
Using results from \cite{202.8}, we showed that on convex polygons the FOSLS method is pollution-free when the polynomial degree at the test side grows proportionally with $\log \kappa$.

The (approximate) Riesz' lift of the residual, which is a variable in the system, can be used for a posteriori error estimation, as a local error indicator for adaptive refinement, and to construct an improved approximation. In experiments, this `boosted' approximation converges at an improved rate, so that the estimator is asymptotically exact.
...
So far, we have solved the linear system that arises from the FOSLS method with a direct solver. We are, however, optimistic that its Hermitian saddle-point form provides advantages when it concerns the development of preconditioners. This will be topic of future study.
