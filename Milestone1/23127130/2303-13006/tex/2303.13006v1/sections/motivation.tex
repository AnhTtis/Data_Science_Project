...
A standard way to handle the model-inversion problem when $f$ is not bijective is to treat it pointwise, defining a loss, such as 
\begin{equation}
	\mathcal{L} = \| \mb{y} - f(\mb{x}) \|^2, \label{eq:loss}
\end{equation}
and minimizing it via gradient descent on $\mb{x}$ from some starting point $\mb{x}_0$. 
One would then iteratively update an initial guess $\mb{x}_0$ according to 
\begin{equation}
...
\begin{equation}
	p(\mb{y} |\mb{x}) = \mathcal{N}(\mb{y}; f(\mb{x}), \sigma^2 \mb{I}) \propto \exp \left( -\frac{\mathcal{L}}{2 \sigma^2} \right), \label{eq:observation}
\end{equation}
