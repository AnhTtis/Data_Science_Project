...
\author{Letizia Angeli$^{(1)}$, Dan Crisan$^{(2)}$, Michela Ottobre$^{(3)}$} 

\address{(1) Maxwell Institute for Mathematical Sciences and Mathematics Department, Heriot-Watt University, Edinburgh EH14 4AS, UK. l.angeli@hw.ac.uk}
	
\address{(2) Department of Mathematics, Imperial College London, London, SW7 2AZ, UK. d.crisan@imperial.ac.uk}


\address{(3) Maxwell Institute for Mathematical Sciences and Mathematics Department, Heriot-Watt University, Edinburgh EH14 4AS, UK. m.ottobre@hw.ac.uk}
...
We prove a general criterion providing sufficient  conditions under which a time-discretiziation of a given Stochastic Differential Equation (SDE) is a {\em uniform in time} approximation of the SDE.  The criterion is also, to a certain extent, discussed in the paper, necessary. 
...
Euler methods. 
In particular, we show that, under mild conditions on the coefficients of the SDE (locally Lipschitz and  strictly monotonic), these methods produce approximations of the law of the solution of the SDE that converge {uniformly in time}. 
The theoretical results are verified by  numerical examples.             
\vspace{5pt}
\\
{\sc Keywords.} Stochastic Differential Equations, numerical methods for SDEs, Explicit and Implicit Euler schemes, Split Step, Tamed Euler Schemes,  Derivative estimates,  Markov Semigroups, Strong Exponential Stability.  
\vspace{5pt}
\\
{\sc AMS Classification (MSC 2020).} 65C20, 65C30, 60H10 , 65G99, 47D07, 60J60. 
...
Explicitly solvable SDEs are rare in practical applications, so  one often  needs to rely on numerical approximations in order to be able to study the underlying physical phenomenon. There are two sets of numerical approximations for solutions of SDEs: pathwise (or strong) approximations and approximations of the law of the solution of the SDE, also known as weak approximations. 
  Let  $\{x_t, t\ge 0 \}$ denote the solution of the SDE at hand,  let $\{\mathcal{X}^{\delta}_{t_n}\}_{n \in \mathbb N}$ be some time-discretisation (with step $\delta$) of $\{x_t, t\ge 0 \}$ and let $\{\mathcal{X}^{\delta}_{t}, t\geq 0\}$ be a continuous-time interpolant of  $\{\mathcal{X}^{\delta}_{t_n}\}_{n \in \mathbb N}$.
...
In this paper, we consider various approximation schemes for SDEs and we show that, under mild conditions on the coefficients of the SDE (locally-Lipschitz and  strictly monotonic drift, we will be more precise below), the bound \eqref{eqn:boundintro} holds with a constant $C$ independent of time; that is, we show that the following holds
...
\end{equation}
where now $K>0$ is a constant independent of time (but dependent on $g$), and $\beta$ will depend on the scheme at hand.\footnote{For the sake of clarity,  $\beta$ does not depend $t$ nor on $g$. }  When the bound \eqref{eqn:boundintrouniform} holds for some constant $K>0$ we say that the approximating scheme  $\mathcal{X}^{\delta}_t$ is a {\em Uniform in time} (UiT) weak approximation of the given SDE. 

Clearly, a bound of the form \eqref{eqn:boundintrouniform} cannot hold in general; whether it holds or not will depend on both the dynamics to approximate and on the chosen approximation. In this paper we state sufficient conditions, on both the SDE and the approximating scheme,  to guarantee the validity of the UiT bound \eqref{eqn:boundintrouniform}. We will phrase such criteria both in terms of abstract conditions and then in terms of easily verifiable, explicit conditions on the coefficients of the SDE. Note that if \eqref{eqn:boundintrouniform} holds,
...
\subsection{Main results and relation to literature}For a clearer description of the results in this paper, let us introduce some essential notation:  let $x_t $ be the solution of the SDE
\begin{equation}\label{SDE_ito}
d x^{(x)}_{t} =U_{0}\left(x^{(x)}_{t} \right) d t+\sqrt{2} \sum_{i=1}^{d} V_{i}\left(x^{(x)}_{t} \right) d B_{t}^{i}, \quad x_{0} =x \in \mathbb{R}^{N}\ ,
\end{equation}
...
\begin{equation}\label{MarkovSemigroup}
    (\mathcal{P}_{ {t }} f)(x):=\mathbb{E}\left[f\left(x^{(x)}_{t}\right) \right], \quad x \in \mathbb{R}^{N}\, .
\end{equation}
...
\begin{itemize}
    ...
    \item In Section \ref{section_split}, we consider split-step (see \eqref{SS2}-\eqref{SS1}) and the Implicit Euler scheme (see \eqref{general_implicit}) for SDEs of the form \eqref{SDE_ito}, when the drift is just locally Lipschitz and strictly monotone, see Assumption \ref{ass_onesidedLip}, and the diffusion coefficient is bounded. Under these assumptions we provide UiT  estimates for the weak error  associated to such schemes, see Theorem \ref{thm_split} for  split-step and Theorem \ref{thm_implicit} for Implicit Euler.  These are the main results of Section \ref{section_split}. The proofs in this section do not leverage directly the abstract criterion of Section \ref{section_general}; rather,  they use the fact that split-step and implicit Euler (for SDEs with locally Lipschitz drift) can be interpreted  as explicit Euler-Maruyama schemes for a modified SDE, see \cite[Section 3.3]{higham2002strong}. The modified SDE has globally Lipschitz coefficients (however with Lipschitz constants depending on $\delta$, which leads to technical subtleties). Using this fact, the proof is based on  the result of Section \ref{section_EM} for the Euler-Maruyama scheme for SDEs with globally Lipschitz coefficients.  
    \item In Section \ref{section_tamed}, we prove  UiT results for a truncated version of the tamed Euler scheme, see \eqref{tamed}, assuming again that the drift $U_0$ of the SDE \eqref{SDE_ito} is only locally Lipschitz and strictly monotone, see Theorem \ref{thm_weak_tamed}, which is the main result of Section \ref{section_tamed}.  The version of the  tamed Euler scheme that we consider, see \eqref{tamed},  has been introduced (to the best of our knowledge) in \cite{sabanis2016euler}. We refer to this version of tamed Euler as {\em truncated tamed Euler} or {\em Newton-tamed Euler}, we explain the reason for this name in Section \ref{section_tamed}. The UiT convergence for this scheme is proved by applying directly  the abstract criterion of Section \ref{section_general}, Proposition \ref{prop_weak}. The variant of the tamed scheme that we consider appears to be `more stable' than standard tamed, we make more comments on this below and, more in detail, in Section \ref{section_tamed}.
\end{itemize}
Examples of the drift coefficients  satisfying our assumptions can be found in Example \ref{toy_eg}. Throughout the paper, various remarks are devoted to showing that these running examples satisfy the assumptions of our main results.  

Let us clarify that in this paper we consider  time-discretisation schemes with constant time step $\Delta t = \delta$ (for which the $n$-th step is $t_n=n\delta$), which can be described as a family of discrete-time Markov processes $\{\mathcal{X}^\delta_{t_n}\}_{n\in\N}$, $\delta>0$. The analysis of schemes with varying time-step has been considered e.g.~in \cite{pages2023unadjusted} for Langevin algorithms, but the focus there is on approximating the invariant measure of the SDE, not the law at time $t$,  for each positive time, which is what we are concerned with here. 

Let us now comment on the results of this paper, section by section, relating them to existing literature. 

The abstract criterion we state in Section \ref{section_general} is rather general, and it can be in principle applied to any approximation of SDEs of type \eqref{SDE_ito}, whether produced numerically or not. Such a criterion shows that, besides a local consistency condition on the approximating scheme, see (\ref{cond_Euler}), one needs two main conditions to hold in order for the UiT bound \eqref{eqn:boundintrouniform} to be satisfied,  namely:  
...
\end{equation}
    for some constants $C, \lambda>0$ and for any $x,y \in \R^N$.  
...
Finally, on a general note,  we observe that results of the form \eqref{eqn:boundintrouniform} substantially imply that the scheme will also approximate the invariant measure of the SDE correctly (assuming the SDE does admit one, see \cite[Corollary 3.6]{crisan2021uniform} for a more careful statement of this fact).  
...
\begin{subequations}
    ...
    %\end{equation}
    \item[(i)] {\em Strong Exponential Stability}: there exist constants $K_0,\,\l>0$ such that
    \begin{equation}\label{cond_SDE}
        \|\PP_t f\|_{\CC^2_b}\,\leq\,K_0\, \|f\|_{\CC^2_b}\cdot e^{-\l t}\ ,
    \end{equation}
...
\begin{equation}\label{weak_conv}
    \sup_{n\in\N} \,\left| \E_xf(\mathcal{X}^{\delta}_{t_n})-\E_x f(x_{t_n})\right|\,\leq\, \frac{K\|f\|_{\CC^2_b}\cdot \Phi(x,\delta)}{1-e^{-\l\,\delta}}\  ,
\end{equation}
with $K:=K_1( K_0\vee 1 )$, where $K_0$ and $K_1$ are as in Assumption \ref{ass_weak}.
\end{prop}

In the note below we make comments to help the reader interpret the conditions in Assumption \ref{ass_weak};  in Lemma \ref{standard_eg} we state a criterion to check \eqref{cond_bound_Phi} in practice. We will discuss conditions under which SES (the bound \eqref{cond_SDE}) holds in Section \ref{s:s}.  

\begin{note}\label{note_on_main_ass}
Some observations:
\begin{itemize}
\item The positive function $\phi$ from condition \eqref{cond_Euler} provides a bound for the local weak error of the time approximation (local in the sense that it is the weak error after one time-step); clearly, such an error depends both on the initial state $x\in\R^N$ and on the time-step $\delta>0$. The schemes we consider in this paper satisfy   \eqref{cond_Euler}   with functions $\phi$ of the form $\phi(x,\delta)=\delta^\a |x|^q +\delta^\beta$, for some constants $q,\a,\beta>0$. In this case, condition \eqref{cond_bound_Phi}  boils down to requiring time-uniform $q$-moment bounds. A practical way of checking that \eqref{cond_bound_Phi} holds is by using  Lemma \ref{standard_eg} below. Using such a lemma  one obtains  that, if $\phi$ is as in the above (and if the assumption of the lemma is satisfied), then $\Phi$ (appearing in \eqref{cond_bound_Phi}) is substantially  of the form $\Phi(x,\delta) = \delta^\a |x|^q +\delta^{\gamma} $, for some $\gamma>0$. 
Since the term $1-e^{-\lambda \delta}$ at the denominator of \eqref{weak_conv} is 
$o(\delta)$, if $\a, \gamma>1$ then Proposition \ref{prop_weak} yields the desired UiT result. Note that the term $1-e^{-\lambda \delta} \approx o(\delta)$  makes one lose one power of $\delta$, which is in fact expected in going from local to global error. 
...
\end{equation}
for some $\e\in(0,1)$ and $c>0$ (both constants possibly depending on $\delta$). Then, condition \eqref{cond_bound_Phi} is satisfied with
\begin{equation*}
...
\begin{align*}
     \MM^\delta_n f(x)-\PP_{n\delta} f(x)=\underbrace{ \MM^\delta_n f(x)-\PP_{(n-m)\delta}\left(\MM^\delta_m f\right)(x)}_{=:A_{m,n}}+\underbrace{ \PP_{(n-m)\delta}\left(\MM^\delta_m f\right)(x) -\PP_{n\delta}f(x) }_{=:B_{m,n}}\ ,
\end{align*}
...
\begin{align}
    ...
    &\leq\,K_1\,K_0\,\|f\|_{\CC^2_b}\cdot\phi(x,\delta) \,e^{-\l (n-1)\delta}\ .\label{B1n_bound}
\end{align}
...
\begin{align}
    ...
    &\leq K\,\|f\|_{\CC^2_b}\cdot\left(\MM^\delta_1\phi\left(x,\,\delta\right)\,+\, \phi(x,\delta) \,e^{-\l \delta}\right)\nonumber\\
    &= K\,\|f\|_{\CC^2_b}\sum_{k=0}^1\MM^\delta_{k}\phi(x,\delta)\,e^{-\l(1-k)\delta}  \ ,\label{basecase}
\end{align}
...
\begin{equation}
    ...
    \leq\,K\,\|f\|_{\CC^2_b}\,\sum_{k=0}^{n-2}\MM^\delta_{k}\phi(x,\delta)\,e^{-\l(n-2-k)\delta} \ ,\label{inductive_hp}
\end{equation}
...
\begin{equation}
    ...
    \leq\,K\,\|f\|_{\CC^2_b}\,\sum_{k=0}^{n-1}\MM^\delta_{k}\phi(x,\delta)\,e^{-\l(n-1-k)\delta} \ ,\label{induction}
\end{equation}
...
\begin{align*}
    ...
    &\leq\, K\|f\|_{\CC^2_b}\,\E\left[ \sum_{k=0}^{n-2}\MM^\delta_{k}\phi(\XX^\delta_1,\delta)\,e^{-\l(n-2-k)\delta} \,\bigg|\,\XX^\delta_0=x\right]\\
    &=\, K\|f\|_{\CC^2_b}\sum_{k=1}^{n-1} \MM^\delta_{k}\phi(x,\delta)\,e^{-\l(n-1-k)\delta} \ ,
\end{align*}
...
\begin{align*}
    \sum_{k=0}^{n-1}\MM^\delta_{k}\phi(x,\delta)\,e^{-\l(n-1-k)\delta} \,\leq\, \Phi(x,\delta)\, \sum_{k=0}^{n-1} e^{-\l\,k\delta}\,\leq\,  \,\frac{ \Phi(x,\delta)}{1-e^{-\l\,\delta}}\ .
\end{align*}
...
Under Assumption \ref{ass_expdecay2}, there exist constants $\sigma,\, K_0>0$ such that
    \begin{equation*}
         \|\nabla^2 \PP_t f\|^2_{\infty}\,\leq\,K_0\,e^{-\sigma t}\,\left(\|\nabla f\|^2_{\infty}+\|\nabla^2 f\|^2_{\infty}\right)\ ,
    \end{equation*}
...
More specifically, we show that, under Assumption \ref{ass_globalLip}, the hypotheses in Lemma \ref{standard_eg} hold, i.e.~we can show that condition \eqref{cond_Euler} is satisfied with
...
\begin{equation}\label{interpolant_overline}
    d\overline{X}^\delta_t\,=\, U_0\left(\overline{X}^\delta_{t_{n(t)}}\right)\,dt\,+\, \sum_{k=1}^d V_k\left(\overline{X}^\delta_{t_{n(t)}}\right)\, dB^k_t\ ,
\end{equation}
with $t_{n(t)}=t_i$ for $t\in[t_i,\,t_{i+1})$;
...
Under conditions \eqref{U0_onesidedLip}-\eqref{cond_poly} and \eqref{Vk_globalLip}-\eqref{Vk_bounded}, $U^\delta_0$ converges to $U_0$ and $V^\delta_k$ converges to $V_k$ as $\delta\to 0$ uniformly on compact sets, for $k=1,\dots,d$; moreover for $\delta\in(0,\,\frac{1}{2\,c_0})$ the vector fields $U^\delta_0$, $V^\delta_k$, $k=1,\dots,d$, are globally Lipschitz (with Lipschitz constant dependent on $\delta$). More precisely, the following bounds hold,
\begin{subequations}
\begin{align}
...
Under Assumption \ref{ass_for_modified_expdecay}, the bound \eqref{cond_SDE} is satisfied for the modified SDE \eqref{modifiedSDE} uniformly in $\delta$. Namely, there exist constants $\sigma,\, K_0>0$ (independent of $\delta$) such that
    \begin{equation*}
         \|\PP^\delta_t f\|_{\CC^2_b}\,\leq\,K_0\,e^{-\sigma t}\, \| f\|_{\CC^2_b}\ ,
    \end{equation*}
...
\begin{align*}
    ...
    &\qquad\leq\, K_0\,\|f\|_{\CC^2_b}\,\int_0^t  e^{-\sigma(t-s)}\,\E_x\left[ \left|U^\delta_0(x_s)-U_0(x_s)\right|+\sum_{k=1}^d\left|V^\delta_k(x_s)-V_k(x_s)\right|^2 \right]\, ds\ .
\end{align*}
...
\end{align*}
Hence, recalling that $F^\delta(x)=x+\delta\,U^\delta(x)$ by definition, using Young's inequality and the above bound one gets
\begin{align*}
    \left|F^\delta(x)\right|^2\,&=\, \langle x,\,F^\delta(x) \rangle\,+\, \delta\cdot \langle U^\delta(x),\,F^\delta(x) \rangle\\
...
\begin{equation}
    \label{2moment}
    \E_x\left[ \left| \overline{X}^\delta_{\delta}\right|^{2} \right]\,\leq\, \frac{1}{(1+b_0\delta)^{2}}\,|x|^{2}\,+\, (2b_1+c)\,\delta\ .
\end{equation}
...
\begin{align*}
     ...
     \leq\, &\begin{multlined}[t]
        C\,\|f\|_{\CC^2_b}\int_0^\delta\bigg( \E_x\left[ 1+|x|^{2q}+ \left|\overline{X}^\delta_r\right|^{2q}\right]^{1/2}\cdot\E_x\left[\left|\overline{X}^\delta_r-x\right|^2\right]^{1/2}\\
...
\begin{align*}
    \E_x\left[ 1+|x|^{2q}+ \left|\overline{X}^\delta_r\right|^{2q}\right]^{1/2}\,&=\,  \E_x\left[ 1+|x|^{2q}+ \left|x+r\,U^\delta_0(x)+\sum_{k=1}^d V^\delta_k(x)\,B^k_r\right|^{2q}\right]^{1/2}\\
    &\leq\, C_q\,\E_x\left[1+|x|^{2q}+ r^{2q}\,\frac{|x|^{2q}}{\delta^{2q}}\,+\, \sum_{k=1}^d \left|B^k_r\right|^{2q} \right]^{1/2}\\&\leq\,C_q\,\left(1+|x|^q\right)\ ,
\end{align*}
...
\begin{align*}
    ...
    &\leq\, \left\|\nabla \overline\PP^\delta_{t_n} f \right\|\cdot \left|x-x^\star\right|\\
...
\begin{align*}
   \left|\E_{x^\star}\left[ f(\overline x^{\delta}_{t_n})\right]-\E_{x^\star}\left[ f^\star(\overline x^{\delta}_{t_n})\right]\right|
    &\leq \, \left\|\nabla f \right\| \cdot \E_{x^\star}\left[ \left|\overline x^{\delta}_{t_n}-F^\delta(\overline x^{\delta}_{t_n}) \right| \right] \\
    &\leq\, C\, \left\|\nabla f \right\|\,\delta\, \E_{x^\star}\left[1+\left|\overline x^{\delta}_{t_n}\right|^{1+q}\right]\\
...
\begin{align*}
   \langle U^\delta_0(x),\,x\rangle\,+\,\delta\,|U^\delta_0(x)|^2 \,&=\,\langle U^\delta_0(x),\,F(x)\rangle\,=\,\langle U_0(F(x)),\,F(x)\rangle\\
...
We assume that there exist constants $q\geq 0$ and $b_0,b_1,c_2>0$ such that the polynomial
condition \eqref{cond_poly} and the following inequality are satisfied (for the same constant $q\geq 0$):
\begin{align}
...
It is fair to point out that our proof of the moment bounds for TTE does not use directly the above mentioned Lyapunov property, but an explicit recursive argument. We explain in Note \ref{note_recursion} why also that scheme of proof cannot be adapted to the standard tamed scheme to produce UiT moment bounds for that scheme.   
\end{itemize}
\end{note}
...
\begin{proof}
    First consider the case $k=1$. From the definition of truncated tamed Euler schemes \eqref{tamed}, we can write
    \begin{align*}
        \E_x\left[\left|J^\delta_{\delta}\right|^2 \right]\,&=\,\left|x\,+\, \frac{\delta\,U_0(x)}{1\,+\, \delta\alpha\,|x|^q}\right|^2\,+\, 2\sum_{k=1}^d \left|V_k(x)\right|^2\,\E_x\left[\left|\Delta B^k_\delta \right|^2\right]\\
        &\leq\,\left|x\,+\, \frac{\delta\,U_0(x)}{1\,+\, \delta\alpha\,|x|^q}\right|^2\,+\,K\delta\ ,
    \end{align*}
    by condition \eqref{Vk_bounded}. We consider the case $|x|\leq 1$ and $|x|>1$ separately. If $|x|\leq 1$, we simply have
    \begin{align*}
        \E_x\left[\left|J^\delta_{\delta}\right|^2 \right]\,\leq\, \tilde{C}_0\ ,
    \end{align*}    
    for some constant $\tilde{C}_0>0$. Now, consider $|x|>1$. By Lyapunov condition \eqref{tamed_Lyap}, we have
    \begin{align*}
        \left|x\,+\, \frac{\delta\,U_0(x)}{1\,+\, \delta\alpha\,|x|^q}\right|^2\,&=\, |x|^2\,+\, \frac{\delta^2 |U_0(x)|^2}{(1+\delta\alpha |x|^q)^2}\,+\, \frac{2\delta \langle U_0(x),x\rangle}{1+\delta\alpha |x|^q} \\
        &\leq\, |x|^2\,+\,  \frac{\delta^2 |U_0(x)|^2}{(1+\delta \alpha|x|^q)^2}\,-\,\frac{2b_0 \delta |x|^{q+2}}{1+\delta |x|^q}\,+\, \frac{2b_1\delta}{1+\delta \alpha|x|^q}\ .
    \end{align*}
    
    By condition \eqref{cond_poly}, we know that there exist constants $c_0,c_1>0$ such that
    \begin{equation} \label{tamed_bound}
        \left| U_0(x)\right|^2\,\leq\,c_0 |x|^{2q+2}\,+\, c_1\,(1+|x|^{2q}) \ ,
    \end{equation}
    thus we obtain
    \begin{align}\label{tamed_basic_bound}
        \left|x\,+\, \frac{\delta\,U_0(x)}{1\,+\, \delta\alpha\,|x|^q}\right|^2\,\leq\, \left(1\,+\,  \frac{c_0\delta^2|x|^{2q} \,-\, 2b_0\alpha \delta^2 |x|^{2q}}{(1+\delta \alpha|x|^q)^2}\right)\,|x|^2\,+\, C\delta\ ,
    \end{align}
    for some constant $C>0$ independent of $x$ and $\delta$.
    Let
    \begin{equation*}
        g_\delta(x)\, =\,   \frac{(2b_0\alpha - c_0)\,\delta^2\, |x|^{2q}}{(1+\delta \alpha|x|^q)^2} \ .
    \end{equation*}

    Note that for $|x|>1$, $g_\delta(x)$ is strictly increasing with respect to $|x|$, with $\inf_{|x|>1}g_\delta(x)= \frac{(2b_0 \a-c_0)\delta^2}{(1+\delta\a)^2}$ and $\sup_{|x|>1}g_\delta(x)= \frac{2b_0\a-c_0}{\a^2}$. Thus, choosing the constant $\a$ such that $\a> \max\{ \frac{c_0}{2 b_0},\, b_0+\sqrt{b_0^2-c_0} \}$ (in case $b_0^2<c_0$, simply choose $\a>\frac{c_0}{2 b_0}$),  we ensure
    \begin{equation}\label{tamed_edelta}
        0< 1-g_\delta(x)\, <\, 1\,-\, \frac{(2b_0 \a-c_0)\delta^2}{(1+\delta\a)^2} \,=:\,\e_\delta < 1\ ,
    \end{equation}
    for every $|x|>1$.

    Hence,
    \begin{equation*}
        \E_x\left[\left|J^\delta_{\delta}\right|^2 \right]\,\leq\, \max\left\{\tilde{C}_0,\, \e_\delta |x|^2\,+\,(C+K)\delta \right\}\ .
    \end{equation*}
    By a standard recursion argument we obtain 
    \begin{equation*}
        \sup_{n\in\N}\E_x\left[\left|J^\delta_{n\delta}\right|^2 \right]\,\leq\,\max\left\{\tilde{C}_0,\, |x|^2 \right\} \,+\, \frac{(C+K)\delta}{1-\e_\delta} \ , 
    \end{equation*}
    and this proves the statement for $k=1$.
    
    Now, let $k>1$. First, consider the case $|x|>1$. From the explicit expression of $J^\delta_\delta$ \eqref{tamed}, we can write
\begin{align*}
    \E_x\left[ \left| J^\delta_\delta\right|^{2k} \right]\,&=\, \E\left[ \left|x\,+\,\frac{\delta U_0(x)}{1+\delta |x|^q}\,+\,\sum_{i=1}^d V_i^\delta (x)B_\delta^i  \right|^{2k} \right]\\
    &=\,\sum_{\ell=0}^k \binom{2k}{2\ell} \left|x\,+\,\frac{\delta U_0(x)}{1+\delta|x|^q}\right|^{2\ell}\, \E\left[  \left|\sum_{i=1}^d V_i^\delta (x)B_\delta^i  \right|^{2k-2\ell}  \right]\\
    &\leq \sum_{\ell=0}^k \binom{2k}{2\ell}\,\left(\e_\delta\,|x|^2\, +\, C\delta\right)^{\ell}\cdot  K^{2k-2\ell}\,\sum_{i=1}^d \E\left[|B^i_\delta|^{2k-2\ell}\right]\ ,
\end{align*}
by \eqref{tamed_basic_bound}, where $\e_\delta\in(0,1)$ is given by \eqref{tamed_edelta}. Proceeding in the same fashion as the proof of Lemma \ref{lemma_alternative} replacing $\frac{1}{h^2}$ with $\e_\delta$, we obtain

\begin{equation*}
    \E_x\left[ \left| J^\delta_{\delta}\right|^{2k} \right]\,\leq\, \e_\delta^{k-1}\,|x|^{2k}\,+\, {C}_k\,\delta^{k-1}\ ,
\end{equation*}
provided $|x|>1$. Whereas, for the case $|x|\leq 1$, we simply have 
\begin{equation*}
    \E_x\left[ \left| J^\delta_{\delta}\right|^{2k} \right]\,\leq\, 2^{2k}\left|x\,+\,\frac{\delta U_0(x)}{1+\delta |x|^q}\right|^{2k}\,+\, 2^{2k}\,\E_x\left[ \left|\sum_{i=1}^d V_i^\delta (x)B_\delta^i  \right|^{2k} \right]\,\leq\, C_k\ ,
\end{equation*}
for some constant $C_k$ independent of $\delta$. Thus, we can conclude by recursion (similarly to Proposition \ref{prop_moments_tamed}) and we obtain the statement. 
\end{proof}
...
\begin{align*}
    \big|\E_x\big[f\big(J^\delta_\delta\big)\,&-\,f(x^\delta_\delta)\big]\big|\\
     ...
     \leq\, &\begin{multlined}[t]
    C\,\|f\|_{\CC^2_b}\int_0^\delta\bigg( \delta\,|x|^{2q+1}\,+\,\E_x\left[ 1+|x|^{2q}+ \left|J^\delta_r\right|^{2q}\right]^{1/2}\cdot\E_x\left[\left|J^\delta_r-x\right|^2\right]^{1/2}\\
...
with
\begin{align*}
    \partial_i \LL \PP_s f(x)\,=\, \langle \partial_i U_0(x),\,\nabla \PP_t f(x)\rangle+\langle  U_0,\,\nabla \partial_i \PP_t f(x)\rangle
...
\end{equation*} 
this proves the statement, with $K_0 = \frac{\max\{1,c\}}{c}$.
...
\begin{proof}
It is a straightforward consequence of generalised Young's inequality. Indeed, we have 
\begin{align*}
...
\begin{align*}
    \sum_{\ell=1}^{k-1} \binom{k}{\ell}\,\left( \frac{\eta_\ell^{p_\ell}}{p_\ell}\, a^k + \frac{1}{\eta_\ell^{q_\ell}\cdot q_\ell}\, b^k \right)\,&=\, \sum_{\ell=1}^{k-1} \binom{k}{\ell}\, \frac{\eta_\ell^{p_\ell}}{p_\ell}\, a^k \,+\, \sum_{\ell=1}^{k-1} \binom{k}{k-\ell}\, \frac{\eta_\ell^{q_{k-\ell}}}{q_{k-\ell}}\, b^k \\
    &=\, \sum_{\ell=1}^{k-1} \binom{k}{\ell}\, \frac{\eta_\ell^{p_\ell}}{p_\ell}\, a^k \,+\, \sum_{\ell=1}^{k-1} \binom{k}{\ell}\, \frac{\eta_\ell^{p_\ell}}{p_\ell}\, b^k\ .
\end{align*}
Thus, 
\begin{equation*}
...
\begin{lemma}\label{lemma_derivativesF}
    ...
    Moreover, if we further assume that there exists a constant $C>0$ independent of $y\in\R^N$ and $\delta>0$ (but possibly dependent on $N$) such that
...
\end{equation*}
    ...
    \begin{equation*}
        |Aw|\,\geq\, \frac{|w^T A(y) w|}{|w|}\,=\,\frac{|w|^2 -\delta w^T \nabla U_0(y) w}{|w|}\,\geq\, (1+\delta \lambda(y))|w|\ .
    \end{equation*}
    Therefore, we obtain
    \begin{equation*}
...
\end{equation*}
where we denoted $A(y):= \1 -\delta\,\nabla U_0(y)$, $y=F^\delta(x)$, to simplify the presentation. By \eqref{dF}, we have
\begin{equation*}
