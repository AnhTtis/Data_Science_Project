...
%\documentclass{IEEEtran}[onecolumn]
\usepackage{arxiv}
...
% The distinctive features of MagicEye's solution compared to existing ones are:

% \begin{itemize}
% \item MagicEye utilizes a proximity sensor to detect and recognize obstacles in real-time, without the need for manual activation by the user.

% \item The system's neural engine has been trained on 35 different objects, providing robust performance both indoors and outdoors.

% \item MagicEye incorporates facial recognition and currency classification models to help individuals identify faces and banknotes.

% \item The facial recognition model goes beyond basic identification by also providing information on the individual's gender, race, age, and facial expression.
% \end{itemize}
...
% 	\caption{Comparison of MagicEye with similar assistive systems}
% 	\label{TBL:Product_Comparison}
% 	%\begin{tabular}{ |p{4cm}||p{.51cm}|p{1.5cm}|p{1.5cm}|  }
% 	\begin{tabular}{p{3.5cm} p{1cm} p{1.9cm} p{1.5cm} p{1.5cm} p{2.5cm} p{1.6cm}}
% 		\hline \hline \\
% 	Assistive System &Year &Technology Used   &Precision   &Indoor/
%  Outdoor &Facial \& Currency identification modules	&GPS Navigation 
% 		\\
% 		\hline
%   \\
%   Garrido et al. \cite{garrido_support_2012} &2012 &NFC &Low &Indoors &No &No \\
%   Nagarajan et al. \cite{nagarajan_localization_2020} &2020  &BLE &Medium &Indoors &No &No\\
%   Alnafessah et al. \cite{alnafessah_developing_2016} &2016  &UWB &Medium &Indoors &No &No\\
%   SUGAR \cite{martinez-sala_design_2015} &2015  &UWB &Medium &Indoors &No &No\\
%   ARIANNA \cite{croce_enhancing_2016} &2016  &Smartphone &Good &Outdoors &No &No\\
%   NavCog3 \cite{sato_navcog3_2019} &2019  &Smartphone &Good &Outdoors &No &Yes\\
%   M. Affif et al. \cite{afif_evaluation_2020} &2020  &RetinaNet &Good &Limited outdoors &No &No\\
%   M. A. Khan et al. \cite{khan_ai-based_2020} &2020  &MobileNet &Good &Both &No &No\\
%   Chang et al. \cite{9253595} &2021  &InceptionNet &Excellent  &Very limited outdoors &No &No\\
%   R. C. Joshi et al. \cite{joshi_yolo-v3_2020} &2020 &YoloV3 &Excellent  &Limited outdoors &No/Yes &No\\
%   \textbf{MagicEye (Current Paper)} &2023 &YoloV5 &Excellent &Both &Yes &Yes \\
% 		\hline
% 	\end{tabular}
% \end{table*}
% \usepackage{rotating}
...
% Additionally, the critical component of MagicEye is the infused Intelligent Data Engine. This allows the model to improve progressively over time. It is evident that as time passes, the performance of a conventional model becomes stale, causing the model to deteriorate eventually. However, in the case of MagicEye, we were able to develop a system that would gradually adapt to changes and improve its performance over time. Besides these, MagicEye is also integrated with a GPS sensor, allowing the device to provide the user with turn-by-turn directions guided by audio feedback.
...
\begin{eqnarray}
w & = & w - \eta \triangledown Q_{i}(w) \\
\label{EQN:SGD}
%\end{equation}
%
%\begin{eqnarray}
w & = & w -\eta \triangledown Q_{i}(w)+\alpha\triangle w 
\label{EQN:SGD-model}
\end{eqnarray}
In the above expressions, $w$ denotes the weight calculated and $\eta$ represents the learning rate (also called step size), and $\alpha$ represents the exponential decay factor ranging from 0 to 1, which determines the relative contribution of the current gradient to the weight change. $Q_{i}$  denotes empirical risk and is calculated using Equation \ref{EQN:grad}, where it represents the loss function value of the $i$-th image sample:
\begin{equation}
...
\begin{equation}
mAP\ =\ \frac{1}{n} \ \sum _{i=0}^{n} AP_{i}\\
\label{EQN:mAP}
\end{equation}
...
% In addition, Fig. \ref{FIG:Comparison-MagicEye_Predicted-labels_ground-truth-labels} illustrates the predictions and detections generated by MagicEye. As demonstrated in the top-right image of Fig. \ref{FIG:Comparison-MagicEye_Predicted-labels_ground-truth-labels}.


%\begin{figure*} [htbp]
%\centering
%	\includegraphics[width=0.8\textwidth]{Comparison-MagicEye_Predicted-labels_ground-truth-labels}
%\caption{Comparison of MagicEye predicted labels v/s ground-truth labels}
%\label{FIG:Comparison-MagicEye_Predicted-labels_ground-truth-labels}
%\end{figure*}
