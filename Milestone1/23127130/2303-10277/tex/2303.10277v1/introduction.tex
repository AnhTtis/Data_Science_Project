...
However, existing system abstraction methods~\cite{smith2019continuous, yin2020optimization} can not be used for ensuring persistent feasibility. Because they usually assume the abstract model has uniform control constraints, which may lead to an unrealizable abstract control that breaks the feasibility. For instance, in \cref{fig:idea}, although both situations correspond to the same abstract state $z$, the feasible abstract control (Cartesian acceleration of the drill) in these two situations is different. In particular, at $x_2$, the horizontal acceleration of the drill is unrealizable due to singularity. 
