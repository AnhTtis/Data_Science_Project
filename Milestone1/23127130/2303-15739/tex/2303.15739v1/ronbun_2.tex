...
and Bayesian stochastic complexity, where $n$ is a sample with size. 
...
then it should be 
\[
\EE[G_n]=\frac{\lambda_{ReLU}}{n}+o(1/n). 
\]
...
&= D_{\mathrm{KL}}(q(x^n,y^n)|p(y^n|x^n)q(x^n))
\end{align}
...
\begin{align}
G_n = D_{\mathrm{KL}}(q(y|x)q(x)|p(y|x,X^n,Y^n)q(x))\mathrm{d}x\mathrm{d}y
\end{align}
The generalization error is also used for model selection\cite{Akaike1974}\cite{S_Watanabe2010a}. 
For $p(y|x,X^n,Y_n) = p(Y_{n+1}|X_{n+1},X^n,Y_n)$, the following equation holds
\begin{align}
p(y|x,X^n,Y_n) &= \frac{1}{Z_n}\int p(Y_{n+1}|X_{n+1},\theta)\varphi(\theta)\prod_{i=1}^{n}p(Y_i|X_i,\theta)\mathrm{d}\theta\\
...
\end{array}\right),
\end{eqnarray}
where $-{\cal M}^{*(1)}_{B0}$is arbitrary vector which components are negative. 
...
\end{array}\right),
\end{eqnarray}
where ${\cal M}^{*(k)}_{AB}, {\cal M}^{*(k)}_{BB}$ are arbitrary matrices, $-{\cal M}^{*(k)}_{BA}$ are arbitrary matrices which components are negative and $-{\cal M}^{*(k)}_{B0}$is arbitrary vector which components are negative. In each layer$(k \geq 2)$ the output $f^{(k)}(\hat{w},\hat{b},x)$ are positive. From this positivity, the following equation holds in $2 \leq k \leq N^*-1$  
\begin{align}
f_A^{(k)}(\hat{w},\hat{b},x) &= f^{(k)}(w^*,w^*,x), \\
f_B^{(k)}(\hat{w},\hat{b},x) &= 0.
\end{align}
Figure\ref{fig_op1} shows the relationships between units in this optimal parameter in $2 \leq k \leq N^*-1$.
\begin{figure}[htbp]
...
\left(H^*_{N^*}(H^*_{N-1}+1)
+ H^*_3(H_2-H^*_2)+ \sum_{k=2}^{N^*-1} H^*_{k} (H_{k-1}^*+1)
\right).
\end{align}
...
\lambda_{ReLU}=\frac{1}{2}
\left(H^*_{N^*}(H^*_{N-1}+1)+ \sum_{k=2}^{N^*-1} H^*_{k} (H_{k-1}^*+1)
\right).
\end{align}
...
\[
K(w,b)=\int q(x)q(y|x)\log\frac{q(y|x)}{p(y|x,w,b)}dxdy. 
\]
...
\[
\EE[F_n]\leq nS -\log \int_{W} \exp(-n K(w,b))\varphi(w,b) dw db.
\]
\end{lemma}
...
\begin{align}
\EE[F_n] & \leq -\log \int \varphi(w,b) \exp(-\EE[nK_n(w,b)])dwdb +nS
\\
&\leq -\log  \int \varphi(w,b) \exp(-nK(w,b))dwdb +nS
\\
&\leq -\log \int_{W} \varphi(w,b) \exp(-nK(w,b))dwdb +nS,
\end{align}
...
all entries are in the inverval $[1,2]$, 
\begin{align}
{\cal M}^{(k)}=
\left(\begin{array}{cc}
...
\\
b^{(N)}&=(b^*)^{(N^*)}-
\sum_{k=N^*}^{N-1}w^{(N-1)}w^{(N-2)}\cdots w^{(k)}b^{(k-1)}
+{\cal E}^{(N)}_{B0},
\end{align}
...
\\
\|f_B^{(N^*-1)}(w,b,x)\|&\leq  c_2(\|x\|+1). \label{eq:lemma5552}
\end{align}
\end{lemma}
\begin{proof} 
...
the output of ReLU function $f_B^{(k-2)}(w,b,x)$ is nonnegative. On the other hand, 
\begin{align}
f^{(k)}(w^*,b^*,x)
&=\sigma((w^*)^{(k)}f^{(k-1)}(w^*,b^*,x)+(b^*)^{(k)}).
\end{align}
...
\begin{align}
& \|f_1^{(k)}(w,b,x)-f^{(k)}(w^*,b^*,x)\|
\\
& \leq \|{\cal E}_{AA}^{(k-1)}f_A^{(k-1)}(w,b,x)+{\cal E}_{01}^{(k)}\|
...
\\&
+b_{A0}^{(N)}+\sum_{k=N^*}^{N-1}w_{AA}^{(N-1)}\cdots w_{AA}^{(k)}b_{A0}^{(k-1)}.
\end{align}
On the other hand,
\begin{align}
...
& w_{AA}^{(N)}w_{AA}^{(N-1)}\cdots w_{AA}^{(N^*)} 
 = ((w^*)^{(N^*)}B^{-1}+ {\cal E}^{(N)}_{AA})w_{AA}^{(N-1)}\cdots w_{AA}^{(N^*)} 
\\
...
\begin{align}
\|b^{(N)}+\sum_{k=N^*}^{N-1}w^{(N-1)}\cdots w^{(k)}b^{(k-1)}
...
\[
\int_{W_E}\exp(-nK(w,b))\varphi(w,b)dwdb 
\geq \frac{C}{n^{\lambda}}
\]
where
\[
