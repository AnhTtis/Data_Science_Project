\noindent\textbf{Denoising Diffusion Models.}
 ...
 % and a convolution layer after linear projection. 
% DiT~\cite{peebles2022dit} achieved SoTA results on class conditional image generation on ImageNet $256\times 256$~\cite{deng2009imagenet}.

% DiT~\cite{peebles2022dit} adopts the AdaLN design from ADM~\cite{dhariwal2021adm} and finds zero initialization strategy is important for the performance.



% \noindent\textbf{Efficient Generation (training/sampling)}
% % Even though DMs could generate high resolution images from complex domains, the sampling efficiency is worse than GAN models.
% DMs' sampling efficiency is worse than GAN models, despite their ability to generate high resolution images from complex domains.
% It takes one model forward pass to generate one sample for GANs, while it requires more iterative steps for DMs.
% Several works are proposed to improve the sampling speed~\cite{ddim,Liu2022PNMD,Lu2022DPMSolverAF,karras2022edm}, and can synthesize one sample in about 10 steps.
% However, there are few research works on the acceleration of training diffusion process.
% It takes 7M training iterations for DiT~\cite{peebles2022dit} to achieve the SoTA FID score 2.27.
% In this paper, we adopt a simple design to accelerate the training of diffusion models.
...
% leverage  Other works focus on how to speed up the generation process.


% Noise schedule, model architecture~\cite{dhariwal2021adm,bao2022uvit,peebles2022dit}, and predicting target have been studied to improve diffusion training.
% To better capture the distinctly different modes of generation process,~\cite{balaji2022eDiff-I,feng2022ernie2} adopt several denoising experts instead of sharing model parameters.
% Several works directly design fast samplers to improve the sampling speed~\cite{ddim,Liu2022PNMD,Lu2022DPMSolverAF,karras2022edm}, and can synthesize one sample in about 10 steps. 
% {\color{magenta} Add sharing weight here?}
% Some other works~\cite{salimans2022distillprogressive,meng2022ondistillation} rely on extra training to distill diffusion models for faster inference.
% However, there are few research works on the acceleration of training diffusion process.
% In this paper, we adopt a simple design to accelerate the training of diffusion models.
