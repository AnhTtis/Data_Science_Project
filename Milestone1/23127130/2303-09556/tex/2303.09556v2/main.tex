...
% \title{On the Importance of Step-wise Weighting Strategy for Diffusion Models}

% \title{Diffusion Models as Multi-Task Learners}

% \title{Min-SNR Weighting Strategy for Efficient Diffusion Training}

%Tuning step-wise loss weight matters for diffusion models
...
Shuyang Gu$^2$\thanks{ Corresponding authors: Shuyang Gu and Baining Guo.},
Chen Li$^3$,
Jianmin Bao$^2$,
Dong Chen$^2$,\\
Han Hu$^2$,
...
$^1$Southeast University, $^2$Microsoft Research Asia, $^3$Xi'an Jiaotong University \\
...
%On the Importance of Weighting Strategy for Diffusion Models

%Efficient Diffusion Training via Min-SNR-$\gamma$ Weighting Strategy
...
%Diffusion models have seen significant advancement in recent years, however, one of the major problems is that these models converge slowly during training. We carefully analyze the correlation between different timesteps and find that each of them has different optimization directions, therefore they may hinder each other, and cause slow convergence. To address it, in this paper, we first regard the diffusion training process as a multi-task learning problem, then we introduce a novel weighting strategy, called Min-SNR-$\gamma$, which adopts different loss weights according to the signal-to-noise ratio. It can effectively balance the conflicts among different timesteps. Experimentally, we find that our method converges 3.4 times faster than previous methods and achieves stronger performance.
   
   % Loss weight design + Vision Transformer + extensive experiments ...
\end{abstract}
