...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \State $\quad \sigma_{t+1, i}^2:= \frac{1}{\sigma_0^{-2} + N_{t+1}(i) \sigma_i^{-2}}$
  			\State Update posterior mean: 
     \State $~~\hmu_{t+1, i}
  := \sigma_{t+1,i}^2\Big( \frac{\mu_{0,i}}{\sigma_{0,i}^2} + \frac{N_{t+1}(i)\bar x_{t+1,i}}{\sigma_{i}^2} \Big)$, where %\todob{$\sigma_{t,i}^2$ should be $\sigma_{t + 1,i}^2$.} \todob{It is strange that we use $\bar x_{t+1,i}$ (above) before we define it (below).}
