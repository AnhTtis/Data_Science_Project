...
\begin{align*}
  & R_n
  \leq \sum_{i = 1}^K\sqrt{\frac{2 \sigma_{0,i}^2}{\pi}} n \delta + \sqrt{2n} \sqrt{\sum_{i =1}^K\sigma_i^2\Big(\log(1 + n\sigma_{0,i}^{2}\sigma_i^{-2}) + \sigma_{0,i}^{2}\sigma_i^{-2}\Big)\log(1/\delta)}\,.
\end{align*}
\end{restatable}

\textbf{Discussion.} For $\delta = 1 / n$, the bound in \cref{thm:bayes_known} scales roughly as:
\begin{align*}
  {O}\left(\sqrt{n \log n}\Bigg(\sqrt{\sum_{i = 1}^K\sigma_i^2 \log\Bign{1 + n\sigma_{0,i}^{2}\sigma_i^{-2} } + \sum_{i = 1}^K \sigma_{0,i}^2}\Bigg)\right)\,.
