...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \For{$t = 1, \dots, n$} %\todob{Up to $n$. Not up to $T$.}
      ...
      \lambda_{t,i}\sim\text{Gam}(\alpha_{t,i},\beta_{t,i}), \ \tilde \mu_{t,i} \sim \cN(\hmu_{t,i},\frac{1}{\kappa_{t,i}\lambda_{t,i}})$
		...
		%%%    
			...
			\State $N_{t+1}(i):= N_{t}(i) + \I{A_t = i}$    		
    			\State $\beta_{t+1, i}:= \beta_{0,i} + \frac{1}{2}\sum_{s =1}^{t}\I{A_s = i}(x_{s,i} - \bar{x}_{t+1,i})^2 + 
    \frac{\kappa_{0,i} N_{t+1}(i) (\bar{x}_{t+1,i} - \mu_{0,i})^2 }{2(\kappa_{0,i} + N_{t+1}(i))}$ 
  			\State $ \alpha_{t+1, i}
  := \alpha_{0,i} + N_{t+1}(i)/2$
        \State $\kappa_{t+1, i}:= \kappa_{0,i} + N_{t+1}(i)$
  	\State Posterior mean:		
        \State ~~$\hmu_{t+1,i}:= \frac{\kappa_{0,i}\mu_{0,i} + N_{t+1}(i)\bar{x}_{t+1,i}}{\kappa_{0,i}+N_{t+1}(i)}$, where ~$\bar x_{t+1,i}:= \frac{1}{N_{t+1}(i)}\sum_{s=1}^{t} \I{A_{s}=i} x_{s,i}$
    		\EndFor
