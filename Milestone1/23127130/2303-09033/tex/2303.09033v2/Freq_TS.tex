...
\subsection{Analysis for General Randomized Exploration \citep{giro}}
\label{sec:gre_analysis}
...
\begin{align}
  Q_{i, s}(\tau)
  = \condprob{\tilde{\mu}_{i,s} \geq \tau}{\tilde{\mu}_{i,s} \sim p(\cH_{i, s}), \, \cH_{i, s}}
  \label{eq:optimism}
\end{align} \todob{The above is due to my bad notation. $\tilde{\mu}_{i,s} \sim p(\cH_{i, s})$ should be in the subindex.}
...
We now would analyze the bounds for Variance-sensitive Gaussian rewards, where we assume that reward of arm $i$ $\sim \cN(\mu_i,\sigma_i)$.
\begin{cor}[Variance-Aware Regret bounds for Gaussian Rewards]
\label{thm:freq_reg_gauss} 
The expected $n$-round regret of $\giro$ (\red{rename}) is bounded above as:
\[
R(n) \leq \sum_{i = 2}^K \Bign{\frac{62 \sigma_i^2}{\Delta_i} (2\log (n) + 1) + 4\Delta_i}.
\]
\iffalse %%%%%%%%%%%%%
\red{To be updated}...
...
\begin{align}
\label{eq:1}
Q_{i,s}(\tau_i)& = \condprob{\tilde{\mu}_{i,s} - \mu_i \geq \Delta_i/2 \cap \hmu_{i,s} - \mu_i < \Delta_i/4}{\tilde{\mu}_{i,s}  \sim p(\cH_{i, s}), \, \cH_{i, s}}  \nonumber\\
& = \P\Bign{(\tmu_{i,s} - \mu_i + \hmu_{i,s} -\hmu_{i,s}) \geq \Delta_i/2 \cap \hmu_{i,s} - \mu_i < \Delta_i/4 \mid \tilde{\mu}_{i,s}  \sim p(\cH_{i, s}), \, \cH_{i, s}} \nonumber
	\\
...
\begin{align}
\label{eq:2}
1 - Q_{1,s}(\tau_i)  & = \condprob{\tilde{\mu}_{1,s} - \mu_1 < -\Delta_i/2 \cap \mu_1 -\hmu_{1,s} < \Delta_i/4}{\tilde{\mu}_{1,s}  \sim p(\cH_{1, s}), \, \cH_{1, s}}  \nonumber
\\
& = 
\P\Bign{(\tmu_{1,s} - \mu_1 + \hmu_{1,s} -\hmu_{1,s}) < -\Delta_i/2 \cap \mu_1 -\hmu_{1,s} < \Delta_i/4 \mid \tilde{\mu}_{1,s}  \sim p(\cH_{1, s}), \, \cH_{1, s}} \nonumber
\\
& = 
\P\Bign{(\mu_1 - \tmu_{1,s} + \hmu_{1,s} -\hmu_{1,s}) > \Delta_i/2 \cap \mu_1 -\hmu_{1,s} < \Delta_i/4 \mid \tilde{\mu}_{1,s}  \sim p(\cH_{1, s}), \, \cH_{1, s}} \nonumber\\
& \leq 
...
\red{some problem} - $\tau_i$s are different for $a_i, b_i$ + how to bound $a_i$ for small $s$? \todob{I do not understand the issue.}
