...
Thus following same analysis as in \eqref{eq:bayes regret scale}, we get: 
...
\label{eq:long1}
  ...
  \\	
  & \leq   \nonumber
    \frac{\delta \sqrt{nK}}{\sqrt{2\pi}}\times
    \\
    &\biggsn{ \biggsn {\sqrt{ \mathbb{E}_{H_n, \bsigma, \bmu} \bigsn{ \underbrace{\sum_{i = 1}^K \frac{2 \beta_{0,i} + \kappa_{0,i}(\mu_i-\mu_{0,i})^2}{2\kappa_{0,i}(\alpha_{0,1}-1)}}_{(A)} + \underbrace{ \sum_{i = 1}^K \sum_{s = 1}^{n}  \frac{ 2\beta_{0,i} 
    +
    \sigma_i^2 (s-1)
    +
