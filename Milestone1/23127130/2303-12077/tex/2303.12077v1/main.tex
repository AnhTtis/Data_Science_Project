% updated Mar 2023 by Bo Jiang and Shaoyu Chen
...
\author{Bo Jiang$^{1,\star,\diamond}$,\quad Shaoyu Chen$^{1,\star,\diamond}$,\quad Qing Xu$^2$,\quad Bencheng Liao$^{1,\diamond}$,  Jiajie Chen$^2$,\\
...
Without fancy tricks or hand-designed post-processing steps, VAD achieves state-of-the-art (SOTA) end-to-end planning performance and the best efficiency on  challenging nuScenes~\cite{caesar2020nuscenes} dataset. Compared with the previous SOTA method UniAD~\cite{hu2022uniad}, 
our base model, VAD-Base, greatly reduces the average planning displacement error by  
64.1\% (1.03m \vs\ 0.37m) and the average collision rate by 54.8\% (0.31\% \vs\ 0.14\%), while running 2.5$\times$ faster (1.8 FPS \vs\ 4.5 FPS); the lightweight variant, VAD-Tiny, reduces the average planning displacement error by  60.2\% (1.03m \vs\ 0.41m) and the average collision rate by 48.4\% (0.31\% \vs\ 0.16\%), while running 9.3$\times$ faster (1.8 FPS \vs\ 16.8 FPS). We also demonstrate the effectiveness of our design choices through thorough ablations.

Our key contributions are summarized as follows:
\begin{itemize}
\item We propose VAD, an end-to-end vectorized paradigm for autonomous driving. VAD models the driving scene as fully vectorized representation, getting rid of computationally intensive dense rasterized representation and hand-designed post-processing steps.
...
VAD first adopts a group of agent queries $Q_{a}$ to learn agent-level features from the shared BEV feature map via deformable attention~\cite{zhu2020deformable}. And agent's attributes (location, class score, orientation, \etc) are decoded from the agent queries by an MLP-based decoder head. 
...
VAD regularizes the planning trajectory $\hat{V}_{\rm ego}$  with instance-level vectorized constraints during the training phase.
\vspace{-1mm}
...
\midrule
NMP$^{\text{\textdagger}}$~\cite{zeng2019nmp} & - & - & 2.31 & \cellcolor{gray!30}- & - & - & 1.92 & \cellcolor{gray!30}- & - & \cellcolor{gray!30}- \\
SA-NMP$^{\text{\textdagger}}$~\cite{zeng2019nmp} & - & - & 2.05 & \cellcolor{gray!30}- & - & - & 1.59 & \cellcolor{gray!30}- & - & \cellcolor{gray!30}- \\
FF$^{\text{\textdagger}}$~\cite{hu2021ff} & 0.55 & 1.20 & 2.54 & \cellcolor{gray!30}1.43 & 0.06 & 0.17 & 1.07 & \cellcolor{gray!30}0.43 & - & \cellcolor{gray!30}- \\
EO$^{\text{\textdagger}}$~\cite{khurana2022eo} & 0.67 & 1.36 & 2.78 & \cellcolor{gray!30}1.60 & 0.04 & 0.09 & 0.88 & \cellcolor{gray!30}0.33 & - & \cellcolor{gray!30}- \\
\midrule
...
UniAD~\cite{hu2022uniad} & 0.48 & 0.96 & 1.65 & \cellcolor{gray!30}1.03 & \textbf{0.05} & 0.17 & 0.71 & \cellcolor{gray!30}0.31 & 555.6 & \cellcolor{gray!30}1.8 \\
VAD-Tiny & 0.20 & 0.38 & 0.65 & \cellcolor{gray!30}0.41 & 0.10 & 0.12 & 0.27 & \cellcolor{gray!30}0.16 & \textbf{59.5} & \cellcolor{gray!30}\textbf{16.8} \\
VAD-Base & \textbf{0.17} & \textbf{0.34} & \textbf{0.60} & \cellcolor{gray!30}\textbf{0.37} & 0.07 & \textbf{0.10} & \textbf{0.24} & \cellcolor{gray!30}\textbf{0.14} & 224.3 & \cellcolor{gray!30}4.5 \\
\bottomrule
\end{tabular}% }
\end{center}
\vspace{2pt}
\caption{\textbf{Planning Performance.} VAD achieves the best end-to-end planning performance and the fastest inference speed on the nuScenes~\cite{caesar2020nuscenes} val dataset. LiDAR-based methods are denoted with ${\text{\textdagger}}$. FPS of ST-P3 and VAD are measured on one NVIDIA Geforce RTX 3090 GPU. FPS of UniAD is measured on one NVIDIA Tesla A100 GPU.
}
\label{tab:sota-plan}
\end{table*}
...
We use End-to-end Prediction Accuracy (EPA)~\cite{gu2022vip3d, jiang2022pip} to evaluate the performance of vectorized motion prediction, and use mean Average Precision (mAP) to evaluate the vectorized map learning performance, following previous works~\cite{liu2022vectormapnet, liao2022maptr}. As for planning, Displacement Error (DE) and Collision Rate (CR) are adopted to comprehensively evaluate the planning performance.
...
\midrule
1 & - & - & \cmark & \cmark & \cmark & 0.23 & 0.42 & 0.72 & \cellcolor{gray!30}0.46 & 0.34 & 0.26 & 0.36 & \cellcolor{gray!30}0.32 \\
2 & \cmark & - & \cmark & \cmark & \cmark & 0.17 & 0.34 & 0.84 & \cellcolor{gray!30}0.45 & 0.04 & 0.11 & 0.30 & \cellcolor{gray!30}0.15 \\
3 & \cmark & \cmark & - & - & - & 0.16 & 0.33 & 0.60 & \cellcolor{gray!30}0.36 & 0.21 & 0.24 & 0.53 & \cellcolor{gray!30}0.33 \\
4 & \cmark & \cmark & \cmark & - & - & 0.17 & 0.35 & 0.63 & \cellcolor{gray!30}0.38 & 0.02 & 0.10 & 0.35 & \cellcolor{gray!30}0.16 \\
5 & \cmark & \cmark & - & \cmark & - & 0.16 & 0.33 & 0.61 & \cellcolor{gray!30}0.37 & 0.17 & 0.18 & 0.35 & \cellcolor{gray!30}0.23 \\
6 & \cmark & \cmark & - & - & \cmark & 0.15 & 0.32 & 0.58 & \cellcolor{gray!30}0.35 & 0.18 & 0.21 & 0.41 & \cellcolor{gray!30}0.27 \\
7 & \cmark & \cmark & \cmark & \cmark & \cmark & 0.17 & 0.34 & 0.60 & \cellcolor{gray!30}0.37 & 0.07 & 0.10 & 0.24 & \cellcolor{gray!30}0.14 \\
\bottomrule
\end{tabular}
% }
\end{center}
...
\toprule
\multirow{2}{*}{Motion} & \multirow{2}{*}{Map} & \multicolumn{4}{c|}{L2 (m) $\downarrow$} &\multicolumn{4}{c}{Collsion (\%) $\downarrow$}  \\
& & 1s & 2s & 3s & \cellcolor{gray!30}Avg. & 1s & 2s & 3s & \cellcolor{gray!30}Avg. \\
\midrule
- & \cmark & 0.16 & 0.33 & 0.60 & \cellcolor{gray!30}0.36 & 0.55 & 0.65 & 0.84 & \cellcolor{gray!30}0.68 \\
\cmark & - & 0.15 & 0.32 & 0.58 & \cellcolor{gray!30}0.35 & 0.17 & 0.17 & 0.48 & \cellcolor{gray!30}0.27 \\
\cmark & \cmark & 0.17 & 0.34 & 0.60 & \cellcolor{gray!30}0.37 & 0.07 & 0.10 & 0.24 & \cellcolor{gray!30}0.14 \\
\bottomrule
\end{tabular}}
\end{center}
\vspace{2pt}
\caption{\textbf{Ablation for module necessity.} "Motion" and "Map" indicate whether VAD adopts the motion and map modules to perform the tasks of motion prediction and online mapping, as well as utilizing the module outputs for planning.}
% \vspace{1mm}
\label{tab:task_necess}
\end{table}
...
\end{center}
\vspace{2pt}
\caption{\textbf{Ablation for module runtime.} The inference speed is measured for VAD-Tiny on one NVIDIA GeForce RTX 3090 GPU.}
\label{tab:com_speed}
% \vspace{1mm}
\end{table}
...
Compared with the previous SOTA method~\cite{hu2022uniad}, VAD shows great advantages in both performance and speed, as shown in Tab.~\ref{tab:sota-plan}. On one hand, VAD-Tiny and VAD-Base greatly reduce the average planning displacement error by 0.62m and 0.66m,  Meanwhile, the average collision rates of VAD-Tiny and VAD-Base are only 51.6\% and 45.2\%. On the other hand, because VAD does not need many auxiliary tasks (\eg, tracking and occupancy prediction) and tedious post-processing steps, it achieves the fastest inference speed based on the vectorized scene representation. VAD-Tiny runs 9.3$\times$ faster while keeping a better planning performance. VAD-Base achieves the best planning performance and still runs 2.5$\times$ faster.
...
\vspace{2mm}
\caption{\textbf{Visualization results of VAD.} VAD outputs planning results based on the vectorized scene representation in an end-to-end manner, not requiring any dense maps or hand-designed post-processing steps.}
\label{fig:vis}
\end{figure*}
...
Tab.~\ref{tab:design} shows the effectiveness of our design choices. First, the ego-agent interaction provides implicit agent motion features for the ego query so that the ego car can realize others' driving intentions and plan safely. The collision rate becomes higher without ego-agent interaction (ID 1). Second, because map can provide critical guidance for planning, the planning distance error is much larger without ego-map interaction (ID 1-2). Finally, the collision rate can be reduced with any of our proposed vectorized planning constraints (ID 4-6). When utilizing the three constraints together, VAD achieves the lowest collision rate while having great planning accuracy (ID 7).

\vspace{1mm}

\paragraph{Module Necessity.}
Tab.~\ref{tab:task_necess} shows the necessity of the preceding motion module and map module. With only map module, the average collision rate is 0.68\%.  With only motion module, the average collision rate is 0.27\%. When adopting both motion and map modules, VAD achieves the lowest average collision rate (0.14\%). Both motion and map modules contribute to improving planning safety.
