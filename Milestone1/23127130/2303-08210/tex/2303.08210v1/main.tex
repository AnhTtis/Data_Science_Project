...
\end{align}
where $\Delta_{p} \equiv 2p+1$ denotes the conformal dimension of the chiral operator $O_{2p+1}$ and the numerical coefficient $R_3^{(1)}=-2.69(4)$ has been evaluated in Section \ref{sec:example}. In the future we would like to clarify if the analytical method developed in \cite{Belitsky:2020qrm,Belitsky:2020qir} can be applied also for the computation of this coefficient, in order to provide a non-trivial check of our numerical analysis.
