...
A particular variant are signed networks, in which links are either positive or negative and thus can be used to model valenced relations such as liking and disliking, or alliances and 
...
\caption{
    Schematic explanation of the meaning of high and low polarization in social systems and
    ...
    \textbf{(Bottom)} All possible (strongly) balanced and unbalanced triads (3-cycles) 
    and the relationship between their relative frequencies and polarization.
...
algebra~\cite{estradaWalkbasedMeasureBalance2014,singhMeasuringBalanceSigned2017,kirkleyBalanceSignedNetworks2019}. 
Moreover, it can produce both local and global measures as well as
capture strong and weak balance properties. However, walk-based approximations can be potentially
...
counterpart of $\mathbf{A}$ such that $|\mathbf{A}|_{ij} = |\omega(i, j)|$. 
...
\begin{equation}\label{eq:general-balance}
    B = \sum_{k}\omega_kB_k
\end{equation}
where $\omega_k$'s are normalized weights ($w_k \geq 0$ and $\sum_k\omega_k = 1$) 
...
can be measured with frustration index, which can be defined, 
...
Frustration index is a very natural and straightforward measure of the extent to which a given
network partition is close to the perfectly balanced state. It ranges from 0 for perfectly balanced
systems to 1 for maximally unbalanced ones (Fig.~\ref{fig:findex}), 
and since SBT is at its core a theory of clusterability of signed networks, 
we argue that SBT operationalizations should be judged primarily according to their ability 
to identify network partitions with low frustration.

\begin{figure}[ht!]
\centering
\includegraphics[width=\textwidth]{frustration}
\caption{
    Relationship between frustration index and structural balance in signed networks.
...
\rev{
It is important to note that frustration index, while closely related to DoB,
...
On the other hand, frustration is a property of a network and a specific partition, and is directly
related to how close a given partition is to be perfectly balanced. That is why we argue that
frustration index is an appropriate measure of the quality of a partition vis-à-vis the tenets of SBT.
Thus, DoB and frustration index are closely related but not
...
However, in the case of truly directed networks a correction factor should be used to account for 
...
Generalizations to directed signed graphs and weak balance are presented in Methods, 
Secs. \ref{sec:methods:directed} and \ref{sec:methods:weak-balance}.
}
...
\begin{equation}\label{eq:W}
    ...
    = \sum_k\frac{\beta^k}{k!}\mathbf{A}^k
...
(see Methods, Sec.~\ref{sec:methods:directed}, for the generalization to directed measures
based on semiwalks).
}
...
\begin{equation}\label{eq:weighting-scheme}
    w'_{ij} = \frac{|E|w_{ij}}{\sum_{kl} |w_{kl}|}
\end{equation}
...
\rev{
In Sec.~\ref{sec:results:monks} we show that this approach to incorporating edge weights
is indeed effective and leads to better, more meaningful results.
}
...
\textbf{(D)}~Time series of frustration indices for $t = 1, \ldots, 5$
...
This can be measured objectively in terms of frustration index, $F(\mathbf{B})$.
...
\rev{
On the other hand, frustration values obtained when ignoring edge weights ($F_{\text{MSB}}^U$)
...
\rev{
Moreover, through a combination of global and local measures applied to a time series of network
snapshots insights into the impact of microscopic changes (e.g.~edge sign switching) 
on the meso- and macroscopic structure can be gained. 
}
...
(Methods, Sec.~\ref{app:sec:numerical}).
On the other hand, being based on (semi)walks that can be counted easily using standard linear algebra,
MSB computations can be remarkably fast (SI, Sec.~\ref{app:fig:efficiency}).
...
and finally macrolevel is equated with the line index (an analogue of the frustration index), 
but computed only for partitions into two groups. 
Furthermore, our approach follows the structure theorems of SBT
as closely as possible, meaning that analyses for directed networks are based on semiwalks and thus
edge directions are mostly ignored (with some caveats, see Methods, Sec.~\ref{sec:methods:directed}),
while triad-level methods from Ref.~\cite{arefMultilevelStructuralEvaluation2020}
use edge direction information in a more complicated fashion, which is not necessary from the vantage
point of SBT theorems and the problem of network partitioning.
}
...
(both physical and social) for social phenomena and notions such as social impact 
...
weighted sum of walks of different lengths between pairs of
