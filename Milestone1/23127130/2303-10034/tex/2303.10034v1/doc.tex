...
\begin{document}
	...
	\begin{algorithm}[tb!]
		...
		\caption{Simple Statement of Dijkstra's Algorithm}
	...
	\begin{table}[tb!]
		\begin{center}
			\begin{tabular}{lll}
				...
				Basic form                 & $O(n^2)$      & Minimum label $L(u)$ found using linear search. Optimal   \\
	...
	In this section, we consider three different C++ implementations of Dijkstra's algorithm. These use a self-balancing binary tree (\py{std::set}), a binary heap (\py{std:priority_queue}), and a Fibonacci heap, as described in the previous section. In this case, Fibonacci heaps are defined using our bespoke \py{FibonacciHeap} class.
	
	A complete listing of our code is shown in Appendix~\ref{app:code} and can be downloaded at~\cite{SourceCode}. The \py{FibonacciHeap} class is defined on Lines~79 to 296. Lines 299 to 405 then give our three implementations of Dijkstra's algorithm: (\py{dijkstraFibonacci(...)}), (\py{dijkstraTree(...)}), and (\py{dijkstraHeap(...)}). The \py{main()} function on Lines 421 onwards applies these methods to a small toy graph. The output is shown at the end of Appendix~\ref{app:code}.
	...
	\begin{itemize}
		...
		\item Lines 407 to 419 of the code also give a function for extracting a path from $P$. This corresponds to Algorithm~\ref{alg:GetPath} above.
	\end{itemize}
	...
	\begin{figure}[tb!]
		\centering
		%clip l b r t
		\includegraphics[scale=0.4, clip= true, trim=60 100 80 100]{planar.pdf}
		\caption{Execution times of the three algorithm variants with dense planar graphs. Each point on the graph is the mean taken from 100 runs. Error bars show one standard deviation either side of the mean.}
		\label{fig:planar}
	\end{figure}
	
	Execution times of the three algorithm variants with our dense planar graphs are summarised in Figure~\ref{fig:planar} using a range of values for $n$. Across this range, we see that the use of a self-balancing binary tree gives faster run times than Fibonacci heaps, though these differences are very marginal. On the other hand, the use of a binary heap gives a noticeable improvement, with run times of approximately half of the other two algorithms. That said, each of these algorithms can compute shortest path trees of up to a million vertices in well under a second with these graphs. As noted, the maximum number of arcs in a directed planar graph is $6n - 12$ meaning that, in these cases, $m=O(n)$. Consequently, we can consider our three variants of Dijkstra's algorithm to have a complexity of $O(n \lg n)$ here. Given that binary heaps involve fewer computational overheads than self-balancing binary trees and Fibonacci heaps, this helps to explain the superior performance of the binary heap variant in these cases. 
	
	\begin{figure}[tb!]
		\centering
		%clip l b r t
		\includegraphics[scale=0.48, clip= true, trim=60 250 60 290]{rand01.pdf}
		\includegraphics[scale=0.48, clip= true, trim=60 250 60 290]{rand05.pdf}
		\includegraphics[scale=0.48, clip= true, trim=60 250 60 290]{rand09.pdf}
		\caption{Execution times of the three algorithm variants with random graphs using $p= 0.1$, $0.5$ and $0.9$ respectively. Each point on the graph is the mean taken from 100 runs. Error bars show one standard deviation either side of the mean.}
		\label{fig:rand}
	\end{figure}
	
	The charts in Figure~\ref{fig:rand} show the results of the same experiments with random graphs, using $p= 0.1$, $0.5$ and $0.9$ respectively. Note that, in these cases, the number of arcs is much higher than the number of vertices; consequently, the value of $m$ is the dominant factor in the algorithms' time requirements. Since $m\approx pn(n-1)$, increases to $p$ and/or $n$ result in longer run times. In addition, the relative performance of the Fibonacci heap variant is also improved here, with its results being almost indistinguishable from those of the binary heap. The reasons for this are that, with these denser graphs, the number of neighbours per vertex is larger. This brings a higher number of Decrease-Key and Insert operations during execution, which are more efficient with Fibonacci heaps. Despite this, however, the additional overheads required by Fibonacci heaps seem to prevent the algorithm from improving on the binary heap's run times. 
	
	Finally, note that the largest instances considered here involve $n=10,000$ vertices, density $p=0.9$, and therefore approximately $90$ million arcs. In our runs, such graphs were seen to occupy around $900$ MB of memory, but run times were still well below half a second in all cases. The times taken to load these graphs into RAM are not included in the above timings, however.
	
	\section{Conclusions}
	\label{sec:conclusion}
	
	This paper has described the shortest path problem and shown how the performance of Dijkstra's algorithm can be affected by the choice of data structure used for its priority queue. Using a C++ implementation tested over a large range of problem instances, we have seen that the best-performing algorithm does not always have the lowest complexity. Indeed, on the whole, the best performance has been seen when using Dijkstra's algorithm with a binary heap, even though its complexity of $O(m \lg m)$ is higher than the other two variants. For dense graphs, however, the variant using a Fibonacci heap features very similar run times to the binary heap version. Interestingly, this binary-heap variant is also the chosen method of implementation in several open-source libraries including NetworkX~\cite{NetworkX} and GraphHopper~\cite{GraphHopper}. 	
