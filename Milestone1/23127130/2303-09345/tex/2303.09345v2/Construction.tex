...
This paper will be working closely with section 4 in \cite{franchi20211} and section 3 in \cite{rehren2017generalised}, which will be cited accordingly. We will state some of the results in those papers however we will omit the proofs.
...
If $\al = 2\bt$, by Lemma 3.5 in \cite{franchi20212}, $s_{0,1}^2$ is computed. As $s_{0,3}=s_{0,1}$, $s_{0,1}^2\in \Span{\mcal{B}}$.  \qed
\end{prop}
...
Let $\al,\bt\notin \{0,1\}$ be distinct values. Suppose that $V=\GenA{p,q}$ be a axial algebra such that $p$ is a $\mcal{J}(\al)$-axis and $q$ is a $\mcal{J}(\bt)$-axis. Then $V\cong 2B$ or $V\cong 3C(\al,1-\al)$.
\end{prop}

\begin{rem}
In Rehren's proof, he only shows that $V\cong 3C(\al)$ or $V\cong 3C(2)$ if $\al=-1$. However, for $V\cong 3C(\al)$, it is clear that $V\cong 3C(\al,1-\al)$ due $(V,\{p,q\})$ satisfying a fusion law of $\mon{\al,1-\al}$. Further, for $V\cong 3C(2)$, it is easy to show that $3C(2)\cong 3C(-1,2)$. Again, this is clear since $(V,\{p,q\})$ satisfies a fusion law of $\mon{-1,2}$.  
\end{rem}
