...
\end{itemize}
When axes are orthogonal, this gives rise to double axes, which has been studied in multiple papers, for example \cite{joshi2020axial}. Further when $\al=\frac{1}{2}$, we need to change our approach due to $\jor{\frac{1}{2}}$-axial algebras being an infinite family by the classification of primitive axial algebras of Jordan type in \cite{hall2015primitive}. Looking at those three parts, our work proves the following:
\begin{thm*}
Let $\F$ be a field of characteristic not equal to $2$. Suppose $(A,X)$ is a $2$-generated primitive $\mon{\al,\bt}$-axial algebra over $\F$ and $\GenG{X}$ is isomorphic to $X'(1+2)$. Then we have either
\begin{enumerate}
\item[$1.$] $A \cong 3C(\al,1-\al)$ for $\al\neq \frac{1}{2}$ and $\al+\bt=1$, or
\item[$2.$] $(\al,\bt)=(\frac{1}{3},\frac{2}{3})$ and either
\begin{enumerate}
    \item[$i)$] $A\cong Q_2(\frac{1}{3},\frac{2}{3}$) if the characteristic is not equal to $5$, or
    \item[$ii)$] $A\cong Q_2(\frac{1}{3})^\times\oplus \GenG{\id}$ if the characteristic is equal to $5$.
\end{enumerate}
\end{enumerate}
\end{thm*}
\begin{rem}
Adopting the notation in \cite{franchi20212}, we will denote the algebra of $3C(\al)$, $\al\neq -1$, with Monster fusion law $\mon{\al,1-\al}$ as $3C(\al,1-\al)$ to avoid any confusion in which fusion law we have on this algebra. We denote $3C(-1,2)$ to be $3C(2)$ with $\mon{2,-1}$ fusion law.  Further, $Q_2(\frac{1}{3},\frac{2}{3})\cong Q_2(\frac{1}{3})$ has a different fusion law to its definition in \cite{galt2021double}. The final algebra is defined in Table \ref{mult Qx}.
\end{rem}
\begin{note}
In all cases, $\al+\bt=1$. This may be coincidence however this is due to one of the axes in the axet being twisted by the identity element. 
\end{note}
...
\begin{enumerate}
    ...
    \item[$2.$] an axial algebra listed in the Theorem.
\end{enumerate}
\end{cor*}

The final section will start the discussion on larger skew axets, $X'(k+2k)$, and how our result could help with such task. When $k$ is odd and choosing the right axes, there will be a subcase of $X'(1+2)$ inside the larger skew axet. We will conclude proposing a plan to tackle the odd $k$ case, which we hope to present in a secondary paper with work on even $k$.
