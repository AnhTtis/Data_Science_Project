...
It is not known why the use of hidden representations is preferable, but having a theory of final embeddings may aid efforts to understand how they differ from hidden representations.
% While we expect the representations themselves to differ from our predictions in realistic settings, however, we nonetheless see
% Surmounting this will probably require a theory of feature learning in neural networks, but in the mean time, we find empirically that the hidden representations exhibit the same stepwise behavior as the output representations.
...
% As discussed in Section \ref{sec:kernel_th}, our toy model can be seen as mapping a naive initial kernel to an improved final kernel.
% One interesting direction to explore might be running this algorithm again on the final kernel.
% This does not yield the same final kernel, and the result might actually be better.

% discussion of what else we might hope to understand about CL without a full theory of DL
% While a full theory is a big ask (being necessarily at least as complex as a hitherto-elusive theory of deep learning itself), ...

% Limitation: our theory only describes embeddings, while in practice, CL uses representations
% [CITATION: guillotine regularization]
