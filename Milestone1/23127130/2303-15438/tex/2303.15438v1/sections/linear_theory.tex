...
\subsection[The cross-covariance matrix]{The cross-correlation matrix $\mGamma$}
...
We will be particularly interested in the set of such terminal solutions.
...
% are positive and (for simplicity) unique.
...
\L(\mW) = 0.
\end{equation}
% Let $\mathcal{M}$ be the set of local minima of the modified loss $\tilde{\L}(\mW) \equiv \L(\mW) + \epsilon \norm{\mW}_F$. Then $\lim_{\epsilon \rightarrow 0} \mathcal{M}$ is precisely the set of top spectral $\mW$.
\end{proposition}
...
% is this worth saying here? kinda breaks the flow
Note that $\tau_j$ is principally set by $\gamma_j$
% \footnote{The fact that $\tau_j \sim \gamma_j^{-1}$ is sensible because, when training a linear model, eigenvalues typically have units of inverse time.}
...
% However, it is a common feature of matrix factorization problems that the trajectory from generic \textit{small} initialization closely follows the trajectory from a particular aligned initialization [CITATIONS].
...
% Let $\mW(t)$ be the solution to Equation \ref{eqn:dW_dt_expanded} with initial condition $\mW(0) = \alpha \tilde\mW_0$, and let $\mW^*(t)$ be the aligned solution with initial condition $\mW^*(0) = \alpha \mathcal{A}(\tilde\mW_0 {\mGamma^{(\le m)}}\T) \mGamma^{(\le m)}$, whose dynamics are given exactly by Proposition \ref{prop:exact_dynamics}.
% Then as $\alpha \rightarrow 0$, $\norm{\mW(t) - \mW^*(t)}_F \rightarrow 0$ at all $t$.
% \end{result}
...
% \For{$j \in 1...d$}
% \For{$k \in 1...(j-1)$}
% \State $\vu_j \gets \vu_j - \frac{\vu_k \vu_k\T}{\norm{\vu_k}^2} \vu_j$
% \EndFor
% \State $s_j(0) \gets \norm{\vu_j}$
% \EndFor \\
% \Return $[s_1(0), ..., s_d(0)]$
% \end{algorithmic}
% \end{algorithm}
% We can now write down our conjecture:
% \begin{conjecture}[Trajectory from small initialization]
% \label{conj:small_init}
% Let $\mW_0 \in \R^{d \times m}$ with $\tilde{\mW_0} \mGamma^{(\le d)}$ full rank.
% Let $\mW(t)$ be the solution to Equation \ref{eqn:dW_dt_expanded} with initial condition $\mW_0$.
% As $\norm{\mW_0}_F \rightarrow 0$, $\mW(t)$ approaches the trajectory of Proposition \ref{prop:exact_dynamics} with $s_j(0)$ given by Equation \ref{eqn:s_j(0)_approx}.
% \end{conjecture}
...
During training, we track both the loss and the eigenvalues of the cross-covariance matrix $\mC$.
...
% \begin{figure}
%   \centering
%   \includegraphics[width=8cm]{img/cifar10_linear_cl.pdf}
%   \vspace{-3mm}
%   \caption{
%   % \textbf{Our predicted trajectories closely match true trajectories for a linear contrastive model on CIFAR-10 with random initialization.}
%   \textbf{(A)} Loss of a linear contrastive model on CIFAR-10 with random small initialization as a function of training time $t = \eta \times \text{[step]}$.
%   Red dashed lines show theoretical predictions for learning timescales $\tau_j$.
%   \textbf{(B)} Eigenvalues of the contrastive matrix $\mC$ over time compared to theoretical predictions.
%   }
%   \label{fig:linear_model_traces}
% \end{figure}