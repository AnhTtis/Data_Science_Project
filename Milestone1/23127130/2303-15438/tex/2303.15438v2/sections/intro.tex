...
\begin{itemize}
    ...
    \item We extend our model to arbitrary kernel machines including infinite-width neural networks and give closed-form equations for the final embeddings in terms of the training kernel.
...
However, we find that networks in the kernel limit \textit{do} learn nontrivial data embeddings when trained with SSL, and, what is more, we can obtain closed-form expressions for these final embeddings in terms of the NTK (Section \ref{sec:kernel_th}).
