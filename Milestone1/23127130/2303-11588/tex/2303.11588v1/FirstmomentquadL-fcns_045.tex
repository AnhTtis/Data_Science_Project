...
\begin{split}	
\sum_{\substack{(n,2)=1}}L^{(2)}(\tfrac{1}{2}+\alpha, \chi_{n}) w \bfrac {n}X =&  X\M w(1)\frac {\zeta(1+2\alpha)}{\zeta(2+2\alpha)}\frac {1-2^{-1-2\alpha}}{2(1-2^{-2-2\alpha})}+X^{1-\alpha}\M w(1-\alpha)\frac{\pi^{\alpha}\Gamma (1/2-\alpha)\Gamma (\frac {\alpha}2)}{\Gamma(\frac{1-\alpha}2)\Gamma (\alpha)}\cdot\frac{\zeta(1-2\alpha)}{\zeta(2)}\cdot \frac{2^{2\alpha}}{6} \\
& \hspace*{3cm} +O\lz(1+|\alpha|)^{2+\varepsilon}X^{1/4+\varepsilon}\pz.
\end{split}
\end{align}
\end{theorem}
...
\ll& \sum_{\substack{(h,2)=1}}\frac {h^{\max (0,-\Re(w))+\varepsilon} }{h^{2s}}\sumstar_{\substack{(n,2)=1}}\Big | \frac{L^{(2)}(w, \chi_n)}{n^s} \Big |,
\end{split}
\end{align}
...
\label{Aboundinitial}
		A(s,w)
\ll& \sum_{\substack{(h,2)=1}}\frac {h^{\max (0,-\Re(w))+\varepsilon} }{h^{2s}}\sumstar_{\substack{(n,2)=1}}\frac{|L(w, \widetilde\chi_n)|}{|n^{s}|}.
\end{split}
\end{align}
...
\label{D1pexp}	
\begin{split}
   D_{1,p}(s, w; 1,  \psi_1,\psi_0)= D_{1,p}(s, w; 1,  \psi_1,\psi_{-1})=& 1+\frac{1}{p^{w-1/2}}+\Big( 1+\frac 1p \Big)p^{-2s}(1-p^{-2s})^{-1}
