...
\begin{split}
%%\label{Abound}
	\big|L^{(2)}(w, \chi_n)\big|=&	\big|(1-\widetilde\chi_n(2)2^{-w})L(w, \widetilde\chi_n)\big| \ll  \big|(1+100\cdot 2^{-w})L(w, \widetilde\chi_n)\big |.
\end{split}
\end{align*}
We note here that the significance of the number $100$ above that it ensures the function in the upper bound above is free of zeros for $\Re (w)$ small and hence makes possible the use of Proposition~\ref{Extending inequalities}(See the paragraph leading to \eqref{AboundS2}.) \newline
...
\label{Aboundinitial}
		A(s,w)
\ll& |\zeta^{(2)}(2s)|\sumstar_{\substack{(n,2)=1}}\frac{|(1+100\cdot 2^{-w})L(w, \widetilde\chi_n)|}{|n^{s}L^{(2)}(2s+w,  \chi_{n})|}.
\end{split}
\end{align}
...
For $\Re(w) \leq 1/2$, we apply \eqref{fneqnquad} to estimate $A(s,w)$ from above by way of \eqref{Aboundinitial} to revert to the case $\Re(w) \geq 1/2$.  This gives that the bound in \eqref{AboundS0} continues to hold $\widetilde S_0\cap\{(s,w): \Re(w) \leq 1/2 \}$.  Thus \eqref{AboundS0} is valid in the entire region $\widetilde S_0$. \newline
