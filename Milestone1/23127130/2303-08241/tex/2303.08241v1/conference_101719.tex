...
\begin{gather}
    \textbf{(D)} \in \{\text{N,\ NW,\ W,\ SW,\ S,\ SE,\ E,\ NE}\} \\
...
Recalling the chordal distance metric for measuring scenario similarity (see Section \ref{Sec3.3}), Table \ref{chordal distance} summarizes the chordal distances, $\mathbf{\hat{d}}_{\text{chordal}}$, between the original platform location instance and each displaced platform location instance. The gain afforded by our CNN (over the (more) classical approach) is also provided for each displaced platform location instance at $(\overline{\text{SCNR}}_{\text{Output}})_{\hat{\rho}} = 20 \ \text{dB}$. The chordal distance informs us that our CNN should achieve the highest gain for the \textbf{(N)}, \textbf{(NW)}, and \textbf{(W)} cases, followed by the \textbf{(SW)}, and \textbf{(NE)} cases. The \textbf{(S)}, \textbf{(E)}, and \textbf{(SE)} cases are expected to yield the lowest gain. We observe that this ordering yields a pairwise match with the gain afforded by the CNN (above the breakdown threshold for the NAMF test statistic \cite{shyam_TAES}) at $(\overline{\text{SCNR}}_{\text{Output}})_{\hat{\rho}} = 20 \ \text{dB}$.
\begin{table}[h!]
