...
\author{Markus B.\ Fr\"ob and William C.\ C.\ Lima}
\address{Institut f\"ur Theoretische Physik, Universit\"at Leipzig,\\ Br\"uderstra\ss e 16, 04103 Leipzig, Germany}
\ead{\href{mailto:mfroeb@itp.uni-leipzig.de}{mfroeb@itp.uni-leipzig.de}, \href{mailto:lima@itp.uni-leipzig.de}{lima@itp.uni-leipzig.de}}
...
To make these ideas more concrete, let us consider a gravitational system formed by a spacetime with metric $g_{\mu\nu}$ (covered by coordinates $x^\mu$) and matter fields that we will collectively denote by $\psi$. Let us assume that we can find $n$ scalar fields $X^{(\mu)} = X^{(\mu)}[g,\psi]$ that are functionals of the spacetime metric and matter fields.\footnote{We keep the index $\mu$ in $X^{(\mu)}$ within parentheses to stress that these are a collection of scalar fields.} We further assume that the the map $x^\mu \to X^{(\mu)}(x)$ defines a diffeomorphism in our spacetime, and then can view the dynamical fields $X^{(\mu)}$ as a field-dependent reference frame or dynamical coordinate system. We can then measure any tensor field with components $T^{\mu_1 \cdots \mu_m}_{\nu_1 \cdots \nu_p}(x)$ in this dynamical coordinate system by performing the coordinate transformation
\begin{equation}
...
\end{equation}
where $x^\mu = x^\mu(X)$ denotes the inverse of $X^{(\mu)} = X^{(\mu)}(x)$.
...
\mathscr{S}(X) = S[x(X)] \eqend{.}
\end{equation}
Now, consider an arbitrary diffeomorphism $f \colon x^\mu \to x'^\mu = f^\mu(x)$. Since $X^{(\mu)}$ and $S$ are scalar fields, they transform under $f$ as
\begin{equations}
X^{(\mu)}(x) &\to X'^{(\mu)}(x') = X^{(\mu)}(x) = X^{(\mu)}[f^{-1}(x')] \eqend{,} \\
S(x) &\to S'(x') = S(x) = S[f^{-1}(x')] \eqend{,} \label{eq:scalar_transform}
\end{equations}
such that
\begin{equations}
...
\end{equations}
\ie, the scalar field $S$ and the field-dependent frame $X^{(\mu)}$ both change under diffeomorphisms when evaluated at a point of the background manifold. Now let us hold instead the value of the amplitude of the scalar fields $X^{(\mu)}$ fixed, \ie, let us take $x^\mu$ and $x'^\mu$ such that $X^{(\mu)}(x') = X^{(\mu)}(x)$. Then it follows straightforwardly that
\begin{equation}
\mathscr{S}'(X) = S'[x'(X)] = S[x(X)] = \mathscr{S}(X)
\end{equation}
where we have used Eq.~\eqref{eq:scalar_transform} in the second equality. Hence, the quantity $\mathscr{S}$~\eqref{eq:invariant_scalar} is defined in such a way that the changes of the field-dependent frame and the scalar field compensate each other, leaving the final expression invariant.
...
&\quad- \partial_\mu \left[ X^{(\rho)}_{(2)} - X^{(\sigma)}_{(1)} \partial_\sigma X^{(\rho)}_{(1)} \right] g_{\rho\nu} - \partial_\nu \left[ X^{(\rho)}_{(2)} - X^{(\sigma)}_{(1)} \partial_\sigma X^{(\rho)}_{(1)} \right] g_{\rho\mu} \\
&= - \lie_{X_{(2)}} g_{\mu\nu} - \lie_{X_{(1)}} h_{\mu\nu} + \frac{1}{2} \lie_{X_{(1)}}^2 g_{\mu\nu} + \frac{1}{2} \lie_{X^{(\rho)}_{(1)} \partial_\rho X_{(1)}} g_{\mu\nu} \eqend{,} \raisetag{1.6em} \label{eq:invariant_metric_corrections_2}
\end{split}
\end{equations}
where $\lie_X$ is the Lie derivative with respect to $X^\mu$. We can also define a invariant metric perturbation by taking
\begin{equation}
...
\begin{equation}
\mathscr{W}_\mu(X) \equiv \frac{\partial x^\alpha(X)}{\partial X^{(\mu)}} \tilde{W}_\mu[x(X)] \eqend{.}
\end{equation}
...
\begin{split}
\mathscr{W}^{(2)}_\mu &\equiv W^{(2)}_\mu - X_{(1)}^{(\rho)} \partial_\rho W^{(1)}_\mu - \left( X_{(2)}^{(\sigma)} - X_{(1)}^{(\rho)} \partial_\rho X_{(1)}^{(\sigma)} \right) \partial_\sigma W_\mu \\
&\quad+ \frac{1}{2} X_{(1)}^{(\rho)} X_{(1)}^{(\sigma)} \partial_\rho \partial_\sigma W_\mu - \partial_\mu X_{(1)}^{(\sigma)} \left( W^{(1)}_\sigma - X_{(1)}^{(\rho)} \partial_\rho W_\sigma \right) \\
&\quad- \partial_\mu \left( X_{(2)}^{(\sigma)} - X_{(1)}^{(\rho)} \partial_\rho X_{(1)}^{(\sigma)} \right) W_\sigma \\
&= W^{(2)}_\mu - \lie_{X_{(2)}} W_\mu - \lie_{X_{(1)}} W^{(1)}_\mu + \frac{1}{2} \lie_{X_{(1)}}^2 W_\mu + \frac{1}{2} \lie_{X^{(\rho)}_{(1)} \partial_\rho X_{(1)}} W_\mu \eqend{.}
\end{split}
\end{equations}
...
\end{equation}
where $a(t)$ is the scale factor depending on cosmic time $t$, and the $x^i$ are Cartesian coordinates. The background coordinates are synchronous, \ie, the four-velocity $u_\mu \equiv - \partial_\mu t$ is co-moving observers is orthogonal to the spatial coordinate basis:
\begin{equation}
...
\end{splitequation}
As for the temporal second-order correction~\eqref{eq:coord_correction_second_order_integral_t}, it changes by
\begin{splitequation}
...
&= \xi^\mu \partial_\mu t_{(1)}(t,\vec{x}) + \frac{1}{2} \int_{-\infty}^t \left[ \xi^\mu \partial_\mu h_{tt} + 2 \partial_t \xi^\mu h_{t\mu} \right](s,\vec{x}) \total s + \bigo{\kappa} \eqend{,}
\end{splitequation}
where again we have use the assumption that the diffeomorphism is localised, and we see that the integral term exactly cancels the one from~\eqref{eq:coord_correction_deltaxi_t1}. The computation of the change in the spatial coordinates again leads to similar results. In conclusion, we have shown that the perturbed synchronous coordinates change by
\begin{equations}
...
In Eqs.~\eqref{eq:invariant_metric}--\eqref{eq:invariant_metric_perturbation} we have already defined the gauge-invariant metric $\mathscr{g}_{\mu\nu}$, its perturbative expansion and the gauge-invariant metric perturbation $\mathscr{h}_{\mu\nu}$, respectively. Our aim now is to check that the gauge-invariant metric perturbation in the dynamical synchronous coordinate system $X^{(\mu)} = (\tilde{t}, \tilde{\vec{x}})$ satisifes the synchronous condition $\mathscr{h}_{t\nu} = 0$, compare the discussion at the end of Sec.~\ref{sec:relational_observables}. We first show this explicitly, up to second order in perturbation theory. The correction to the invariant metric at first order~\eqref{eq:invariant_metric_corrections_1} can be cast in the form
\begin{splitequation}
\mathscr{g}^{(1)}_{t\nu} &= h_{t\nu} + \partial_\nu t_{(1)} - g_{t\nu} \partial_t t_{(1)} - g_{i\nu} \partial_t x^i_{(1)} \\
&= h_{t\nu} + \partial_\nu t_{(1)} - g_{t\nu} \partial_t t_{(1)} - g_{i\nu} \left( \partial^i t_{(1)} + h_t{}^i \right) \\
&= h_{t\nu} + \partial_\nu t_{(1)} - g_{t\nu} \partial_t t_{(1)} - g_{\nu\mu} \left( \partial^\mu t_{(1)} + h_t{}^\mu \right) + g_{t\nu} \left( \partial^t t_{(1)} + h_t{}^t \right) \\
&= - g_{t\nu} \left( 2 \partial_t t_{(1)} + h_{tt} \right) = 0 \eqend{,}
\end{splitequation}
where we have used Eqs.~\eqref{eq:coord_correction_first_order}. For the second-order correction, Eq.~\eqref{eq:invariant_metric_corrections_2}, we find
\begin{splitequation}
...
\end{splitequation}
where we have used Eqs.~\eqref{eq:coord_correction_first_order} and~\eqref{eq:coord_correction_second_order}. Hence, we have shown that
\begin{equation}
...
\tilde{T}_{\mu\nu} &= T_{\mu\nu} + \kappa T_{\mu\nu}^{(1)} + \kappa^2 T_{\mu\nu}^{(2)} + \bigo{\kappa^3} \eqend{.}
\end{equations}
Assuming the FLRW metric~\eqref{eq:FLRW_line_element} for the background, we obtain for the perturbed Einstein tensor the following:
\begin{equations}
...
\end{split}
\end{equations}
where $\nabla^2 \equiv \nabla^\rho \nabla_\rho$, $h \equiv g^{\mu\nu} h_{\mu\nu}$ and $\epsilon \equiv - H^{-2} \dot{H}$ is the deceleration parameter. We will leave the matter stress tensor unspecified for the time being. The perturbative expansion of Einstein's equations~\eqref{eq:perturbed_Einstein_eq} then reads
\begin{equation}
\tilde{E}_{\mu\nu} = E_{\mu\nu} + \kappa E^{(1)}_{\mu\nu} + \kappa^2 E^{(2)}_{\mu\nu} + \bigo{\kappa^3} \eqend{,}
\end{equation}
with $E_{\mu\nu} = 0$ being Einstein's equation for the background and
\begin{equations}[eq:expansion_einstein_eq]
E^{(1)}_{\mu\nu} &= 2 G^{(1)}_{\mu\nu} - \kappa^2 T^{(1)}_{\mu\nu} \eqend{,} \\
E^{(2)}_{\mu\nu} &= 2 G^{(2)}_{\mu\nu} - \kappa^2 T^{(2)}_{\mu\nu} \eqend{.}
\end{equations}

To obtain the gauge-invariant part of Einstein's equations, we perform the transformation~\eqref{eq:invariant_tensor} for the tensor $\tilde{E}_{\mu\nu}$. This results in
\begin{splitequation}
...
\end{splitequation}
where again we have used Eq.~\eqref{eq:X_inverse_power_series} to obtain the perturbative expansion and that the equation of motion is satisfied on the background, $F_\mu = 0$. We see again that as for Einstein's equations~\eqref{eq:invariant_Einstein_eq} the fluid equation of motion is already gauge-invariant, in the sense that the relational version just corresponds to a rearrangement of the terms in the perturbative series, such that $\mathscr{F}_\mu = 0$ and $\tilde{F}_\mu = 0$ are equivalent.
...
At the background level, the trace and the time-time and space-space components of Einstein's equations give Friedmann's equations
...
\end{equation}
Then, by taking the time derivative of Eq.~\eqref{eq:speed_sound} and Friedmann's equation for $\rho$~\eqref{eq:rho_eq}, we obtain
\begin{equation}
...
\end{equation}
We can then use Friedmann's equations~\eqref{eq:friedmann_fluid} and Eq.~\eqref{eq:T_eq} to simplify the temporal component~\eqref{eq:constraint_eq_time_fluid_1st_order_svt} of the constraint equations. We obtain
\begin{equation}
\label{eq:T_S_eq}
...
&\quad + \bar{\partial}_{\alpha }\mathscr{h}^{\alpha \beta } \bar{\partial}_{\gamma }\mathscr{h}_{\beta }{}^{\gamma } + \frac{1}{2} \bar{\partial}_{\beta }\mathscr{h}_{\alpha \gamma } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } -  \frac{3}{4} \bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } -  \mathscr{h}^{\alpha \beta } \bar{\partial}_\alpha\bar{\partial}_\beta\mathscr{h} + 2 \mathscr{h}^{\alpha \beta } \bar{\partial}_\beta\bar{\partial}_\gamma\mathscr{h}_{\alpha }{}^{\gamma }\\
&\quad -  \mathscr{h}^{\alpha \beta } \bar{\partial}^\gamma\bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta } + (n - 8)H \mathscr{h}^{\alpha \beta } \partial_t\mathscr{h}_{\alpha \beta } - (2n - 9 - 2\epsilon)H^2 \mathscr{h}_{\alpha \beta } \mathscr{h}^{\alpha \beta }\\
&\quad + (n-2)\left(1-\frac{\dot{\delta}}{2\epsilon\delta H}\right) \delta H^2\mathscr{d}^2\bigg] \eqend{,} \raisetag{2em}
\end{splitequation}
and we recall that $\bar{g}_{\mu\nu}$ is the induced spatial metric and $\bar{\mathscr{v}}_\mu$ is the projection of the four-velocity perturbation on the background spatial section, see Eqs.~\eqref{eq:induced_spatial_metric} and~\eqref{eq:spatial_projection}.
...
&\qquad+ 2 \epsilon \bar{\partial}_{\alpha }\bar{\mathscr{v}}^{\alpha } \bigg] + \frac{(n-2) H^2}{2}u_{\mu } \bigg[(1 + 2\epsilon - 2\delta) \partial_t\mathscr{h} \mathscr{d} - 2 \epsilon \mathscr{h}^{\alpha \beta } (\partial_t - 2 H)\mathscr{h}_{\alpha \beta } \\
&\qquad+ 4 (\epsilon - \delta) \mathscr{d}\bar{\partial}_{\alpha }\bar{\mathscr{v}}^{\alpha } - 4\epsilon \mathscr{h}_{\alpha \beta } \bar{\partial}^{\beta }\bar{\mathscr{v}}^{\alpha } - 2(n-1)\left(1 - \frac{\dot{\delta}}{2\epsilon\delta H}\right)\delta H\mathscr{d}^2 \\
&\qquad+ 4 \bar{\mathscr{v}}^{\alpha } \left(2\epsilon \partial_t\bar{\mathscr{v}}_{\alpha } + (n - 2 + 2 \delta - 2 \epsilon) \epsilon H\bar{\mathscr{v}}_{\alpha } + \frac{1}{2}\epsilon \bar{\partial}_{\alpha }\mathscr{h} - (\delta - \epsilon) \bar{\partial}_{\alpha }\mathscr{d} - \epsilon \bar{\partial}_{\beta }\mathscr{h}_{\alpha }{}^{\beta }\right) \bigg] \\
...
Now, let us consider the perturbed Einstein's equations sourced by a perturbed scalar field 
\begin{equation}
...
\end{equation}
with
\begin{equations}[eq:expansion_field_equation]
F^{(1)} &= - \nabla^2 \phi^{(1)} + h^{\mu\nu} \nabla_\mu \nabla_\nu\phi + \left( \nabla_\rho h^{\rho\sigma} + \frac{1}{2} \nabla^\sigma h \right) \nabla_\sigma \phi + \frac{1}{2} V''(\phi) \phi^{(1)} \eqend{,} \\
\begin{split}
F^{(2)} &= \frac{1}{2} h^{\beta \gamma } \nabla_{\alpha }h_{\beta \gamma } \nabla^{\alpha }\phi -  \frac{1}{2} \nabla_{\alpha }h \nabla^{\alpha }\phi^{(1)} + \nabla^{\alpha }\phi^{(1)} \nabla_{\beta }h_{\alpha }{}^{\beta } + \frac{1}{2}h^{\alpha\beta }\nabla_{\alpha }\phi \nabla_{\beta }h\\
&\quad + h^{\alpha \beta } \nabla_{\alpha }\nabla_{\beta }\phi^{(1)} -  h^{\beta \gamma } \nabla^{\alpha }\phi \nabla_{\gamma }h_{\alpha \beta } -  h^{\alpha\beta } \nabla_{\alpha }\phi \nabla_{\gamma }h_{\beta }{}^{\gamma } - h_{\alpha }{}^{\gamma } h^{\alpha \beta } \nabla_{\gamma }\nabla_{\beta }\phi\\
&\quad + \frac{1}{4}V'''(\phi) \phi^{(1)2} \eqend{.}
\end{split}
\end{equations}
...
Friedmann's equations for the scalar fields can be written as
...
As for the fluid case, to obtain the invariant Einstein's equation at first order it is enough to make the substitutions
\begin{equation}
...
&\quad+ \left[ (n-1-\epsilon+\delta) (\epsilon-\delta) + 2 \epsilon \delta - \frac{\dot\delta}{H} \right] H^2 \upphi + \frac{\dot{\phi}}{2 H a^4} \laplace \mathcal{T} = 0 \eqend{,}
\end{splitequation}
...
&\quad - \mathscr{h}_{\mu \nu } \bar{\partial}_\alpha \bar{\partial}_\beta \mathscr{h}^{\alpha \beta } + \mathscr{h}^{\alpha \beta } \bar{\partial}_\alpha \bar{\partial}_\beta \mathscr{h}_{\mu \nu } + \mathscr{h}_{\mu \nu } \bar{\partial}^\alpha\bar{\partial}_\alpha\mathscr{h} + 2u_{(\mu } \bigg[\partial_t\mathscr{h}_{\nu) }{}^{\alpha } \bar{\partial}_{\beta }\mathscr{h}_{\alpha }{}^{\beta } - \frac{1}{2} \partial_t\mathscr{h}_{\nu) \alpha } \bar{\partial}^{\alpha }\mathscr{h}\\
&\quad + H\left(\mathscr{h}_{\nu) \alpha } \bar{\partial}^{\alpha }\mathscr{h} - 2\mathscr{h}_{\nu) }{}^{\alpha } \bar{\partial}_{\beta }\mathscr{h}_{\alpha }{}^{\beta } - 2\mathscr{h}^{\alpha \beta } \bar{\partial}_{\beta }\mathscr{h}_{\nu) \alpha } + 3\mathscr{h}^{\alpha \beta } \bar{\partial}_{\nu) }\mathscr{h}_{\alpha \beta }\right) + \mathscr{h}^{\alpha \beta } \bar{\partial}_{\beta }\partial_t\mathscr{h}_{\nu) \alpha }\\
&\quad -  \frac{1}{2} \partial_t\mathscr{h}^{\alpha \beta } \bar{\partial}_{\nu) }\mathscr{h}_{\alpha \beta } -  \mathscr{h}^{\alpha \beta } \bar{\partial}_{\nu) }\partial_t\mathscr{h}_{\alpha \beta }\bigg] + \bar{g}_{\mu \nu } \bigg[\frac{3}{4} \partial_t\mathscr{h}_{\alpha \beta } \partial_t\mathscr{h}^{\alpha \beta } -  \frac{1}{4} \left(\partial_t\mathscr{h}\right)^2 + \mathscr{h}^{\alpha \beta } \partial_t^2\mathscr{h}_{\alpha \beta }\\
&\quad + (n - 8) H\mathscr{h}^{\alpha \beta } \partial_t\mathscr{h}_{\alpha \beta } - (2n - 9 - 2\epsilon)H^2 \mathscr{h}_{\alpha \beta } \mathscr{h}^{\alpha \beta } + \frac{1}{4} \bar{\partial}_{\alpha }\mathscr{h} \bar{\partial}^{\alpha }\mathscr{h} -  \bar{\partial}^{\alpha }\mathscr{h} \bar{\partial}_{\beta }\mathscr{h}_{\alpha }{}^{\beta }\\
&\quad  + \bar{\partial}_{\alpha }\mathscr{h}^{\alpha \beta } \bar{\partial}_{\gamma }\mathscr{h}_{\beta }{}^{\gamma } + \frac{1}{2} \bar{\partial}_{\beta }\mathscr{h}_{\alpha \gamma } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } -  \frac{3}{4} \bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } -  \mathscr{h}^{\alpha \beta } \bar{\partial}_\alpha \bar{\partial}_\beta \mathscr{h} + 2 \mathscr{h}^{\alpha \beta } \bar{\partial}_\beta \bar{\partial}_\gamma \mathscr{h}_{\alpha }{}^{\gamma }\\
&\quad  -  \mathscr{h}^{\alpha \beta } \bar{\partial}^\gamma \bar{\partial}_\gamma\mathscr{h}_{\alpha \beta }\bigg] + u_{\mu } u_{\nu } \bigg[- \frac{1}{4} \partial_t\mathscr{h}_{\alpha \beta } \partial_t\mathscr{h}^{\alpha \beta } + \frac{1}{4} \left(\partial_t\mathscr{h}\right)^2 - (n - 3)H \mathscr{h}^{\alpha \beta } \partial_t\mathscr{h}_{\alpha \beta }\\
&\quad + (2n - 5)H^2 \mathscr{h}_{\alpha \beta } \mathscr{h}^{\alpha \beta } -  \frac{1}{4} \bar{\partial}_{\alpha }\mathscr{h} \bar{\partial}^{\alpha }\mathscr{h} + \bar{\partial}^{\alpha }\mathscr{h} \bar{\partial}_{\beta }\mathscr{h}_{\alpha }{}^{\beta } -  \bar{\partial}_{\alpha }\mathscr{h}^{\alpha \beta } \bar{\partial}_{\gamma }\mathscr{h}_{\beta }{}^{\gamma }\\
&\quad - \frac{1}{2} \bar{\partial}_{\beta }\mathscr{h}_{\alpha \gamma } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } + \frac{3}{4} \bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } + \mathscr{h}^{\alpha \beta } \bar{\partial}_{\alpha}\bar{\partial}_{\beta }\mathscr{h} - 2 \mathscr{h}^{\alpha \beta } \bar{\partial}_{\beta}\bar{\partial}_{\gamma }\mathscr{h}_{\alpha }{}^{\gamma } + \mathscr{h}^{\alpha \beta } \bar{\partial}^{\gamma }\bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta }\bigg]\\
&\quad - \kappa^2 \bigg\{\bar{\partial}_{\mu }\upphi \bar{\partial}_{\nu }\upphi - 2u_{(\mu}\partial_t\upphi \bar{\partial}_{\nu) }\upphi + \frac{1}{2}\bar{g}_{\mu \nu } \bigg[\left(\partial_t\upphi\right)^2 - \bar{\partial}_{\alpha }\upphi \bar{\partial}^{\alpha }\upphi + \bigg((n - 1 - \epsilon + \delta) \delta\\
&\quad - 2(n - 1 - \epsilon + \delta) \epsilon - 2\epsilon\delta\left(1 - \frac{\dot{\delta}}{2\epsilon\delta H}\right)\bigg)H^2 \upphi^2 \bigg] + \frac{1}{2}u_{\mu } u_{\nu } \bigg[\left(\partial_t\upphi\right)^2 + \bar{\partial}_{\alpha }\upphi \bar{\partial}^{\alpha }\upphi\\
&\quad - \bigg((n - 1 - \epsilon + \delta) \delta - 2(n - 1 - \epsilon + \delta) \epsilon - 2\epsilon\delta\left(1 - \frac{\dot{\delta}}{2\epsilon\delta H}\right)\bigg) H^2 \upphi^2\bigg]\\
&\quad +  \dot{\phi}\mathscr{h}_{\mu \nu }\Big[\partial_t\upphi + (n - 1 - \epsilon + \delta) H \upphi\Big] \bigg\} \eqend{.} \raisetag{2em}
\end{splitequation}

We can now obtain the second-order correction to the invariant constraint equation. It reads
\begin{splitequation}
...
&\quad + \frac{1}{4} \bar{\partial}_{\alpha }\mathscr{h} \bar{\partial}^{\alpha }\mathscr{h} - \bar{\partial}^{\alpha }\mathscr{h} \bar{\partial}_{\beta }\mathscr{h}_{\alpha }{}^{\beta } + \bar{\partial}_{\alpha }\mathscr{h}^{\alpha \beta } \bar{\partial}_{\gamma }\mathscr{h}_{\beta }{}^{\gamma } + \frac{1}{2} \bar{\partial}_{\beta }\mathscr{h}_{\alpha \gamma } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta } - \frac{3}{4} \bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta } \bar{\partial}^{\gamma }\mathscr{h}^{\alpha \beta }\\
&\quad - \mathscr{h}^{\alpha \beta } \bar{\partial}_\alpha \bar{\partial}_\beta\mathscr{h} + 2 \mathscr{h}^{\alpha \beta } \bar{\partial}_\beta \bar{\partial}_\gamma\mathscr{h}_{\alpha }{}^{\gamma } - \mathscr{h}^{\alpha \beta } \bar{\partial}^\gamma \bar{\partial}_{\gamma }\mathscr{h}_{\alpha \beta }\bigg] - \kappa^2 \bigg\{\partial_t\upphi \bar{\partial}_{\mu }\upphi - \frac{1}{2}u_{\mu } \bigg[ \bar{\partial}_{\alpha }\upphi \bar{\partial}^{\alpha }\upphi \\
&\quad+ \left(\partial_t\upphi\right)^2 - \bigg((n - 1 - \epsilon + \delta) \delta - 2(n - 1 - \epsilon + \delta) \epsilon - 2\epsilon\delta\left(1 - \frac{\dot{\delta}}{2\epsilon\delta H}\right)\bigg) H^2 \upphi^2\bigg]\bigg\} \eqend{.}
\end{splitequation}
In terms of its components, the second-order corrections to the invariant constraint equation read
\begin{splitequation}
...
&\quad+ \mathscr{h}^{ij } \partial_i\partial_j \mathscr{h} - 2 \mathscr{h}^{ij } \partial_j \partial_k\mathscr{h}_{i }{}^{k } + \mathscr{h}^{ij } \partial^{k }\partial_{k }\mathscr{h}_{ij } - \frac{1}{2}\kappa^2 \bigg[\left(\partial_t\upphi\right)^2 + \partial_{i }\upphi \partial^{i }\upphi \\
&\quad+ \left( (n - 1 - \epsilon + \delta) (2\epsilon-\delta) + 2\epsilon\delta - \frac{\dot{\delta}}{H}\right) H^2 \upphi^2\bigg]
\end{splitequation} 
and
\begin{splitequation}
...
\end{equation}
So far, to obtain an invariant observable from a gauge-dependent expression, we have been using the coordinate transformation from the background to the field-dependent synchronous coordinates, which in the case of Eq.~\eqref{eq:H_u_tilde} gives the following invariant local Hubble rate:
\begin{equation}
...
\end{splitequation}
which in general differs from $\mathscr{H}_u$. However, they agree if the fluid is non-expanding as seen by the observer, which at lowest order is the condition that $\partial_i \mathscr{v}^i = 0$.
...
%\begin{splitequation}
%\mathscr{H}_\phi &= \mathscr{H}_u - \frac{\kappa}{(n-1)a^2 \dot{\phi}} \laplace\Phi^{(1)} - \frac{\kappa^2}{(n-1) \dot{\phi}^2} \bigg[ \frac{\dot{\phi}}{a^2} \laplace \Phi^{(2)} + \dot{\phi} \partial^i \mathscr{h} \partial_i \Phi^{(1)} - \dot{\phi} \partial_i \left( \mathscr{h}^{ij} \partial_j \Phi^{(1)} \right) \\
%&\quad- \left[ \partial_t + \left( \frac{n-1}{2} + \epsilon - \delta \right) H \right] \left( \partial^i \Phi^{(1)} \partial_i \Phi^{(1)} \right) - \frac{1}{a^2} \laplace \Phi^{(1)} \partial_t \Phi^{(1)} \bigg] + \bigo{\kappa^3} \eqend{,}
%\end{splitequation}
\begin{splitequation}
...
We now consider the linearised theory in the absence of matter and quantise the metric perturbation $h_{\mu\nu}$ in some suitable gauge. Our aim is to construct the two-point function of the gauge-invariant metric perturbation $\mathscr{h}_{\mu\nu}$, starting from the gauge-fixed one. At linear order, the invariant two-point function
\begin{equation}
\label{eq:inv_2_point_function_def}
\mathscr{G}_{\mu\nu\rho\sigma}(x,x') \equiv - \mathi \expect{ \mathscr{h}_{\mu\nu}(x) \mathscr{h}_{\rho\sigma}(x') }
\end{equation}
is given in terms of the gauge-dependent metric perturbation as
\begin{splitequation}
...
\end{splitequation}
where $\eta_{\mu\nu}$ is the Minkowski metric, $\partial^{-2}$ denotes the Green's function of the d'Alembertian and
\begin{equation}
...
We have already shown in Sec.~\ref{subsec:gauge_invariant_metric} that our gauge-invariant metric perturbation satisfies $\mathscr{h}_{t\nu} = 0$, which is expected from the synchronous condition. For the same reason, any temporal component of the invariant two-point function~\eqref{eq:inv_2_point_function_def} must vanish. Indeed, by using the equations of motion~\eqref{eq:coord_correction_first_order} for the coordinate corrections $X_{(1)}^{(\mu)}$ in Eq.~\eqref{eq:inv_2_point_function}, it is easy to check that
\begin{equation}
...
\end{splitequation}
where primed derivatives act on $\vec{x}'$. It remains to insert the gauge-fixed graviton two-point function~\eqref{eq:gauge_fixed_2_point_function} and perform the integrals. We first notice that the derivative terms in Eq.~\eqref{eq:gauge_fixed_2_point_function} are symmetrised in the indices $\mu\nu$ and $\rho\sigma$, and thus have the form of a two-point function of operator-valued diffeomorphisms. Since we have already shown that $\mathscr{h}_{\mu\nu}$ is gauge invariant, those terms in the gauge-fixed graviton propagator do not contribute to Eq.~\eqref{eq:inv_2_point_function_spatial}. To check this, let us consider explicitly the term $\partial_{(\mu} \eta_{\nu)(\rho} \partial_{\sigma)} \partial^{-2} G_0$ of Eq.~\eqref{eq:gauge_fixed_2_point_function}. Its contribution to Eq.~\eqref{eq:inv_2_point_function_spatial} reduces to
\begin{splitequation}
...
We thus only need to care about the derivative-free part of the graviton two-point function~\eqref{eq:gauge_fixed_2_point_function} involving $2 \eta_{\mu(\rho} \eta_{\sigma)\nu} - \eta_{\mu\nu} \eta_{\rho\sigma}$. Its contribution to the invariant two-point function~\eqref{eq:inv_2_point_function_spatial} is
\begin{splitequation}
...
As a second example, let us consider quantum fluctuations of the metric for a de~Sitter background $g_{\mu\nu} = a^2(t) \eta_{\mu\nu}$ with the scale factor $a(t) = \mathe^{Ht}$ in cosmological time. We now follow Ref.~\cite{tsamis_woodard_cmp_1994} and use the gauge-fixing Lagrangian density
\begin{equation}
\label{eq:gauge_Lagrangian_dS}
\mathcal{L}_\text{gf} = - \frac{1}{2} G_\mu G^\mu \quad\text{with}\quad G_\mu \equiv \partial_\nu h^\nu{}_\mu - \frac{1}{2} \partial_\mu h + 2 H h^t{}_\mu \eqend{,}
\end{equation}
where we have expressed their gauge condition using the cosmological time. The gauge condition~\eqref{eq:gauge_Lagrangian_dS} corresponds to the Feynman-gauge version of the harmonic gauge in four dimensions, see for example Ref.~\cite{froeb_lima_cqg_2018}. With this choice of gauge, the graviton two-point function in the Euclidean or Bunch--Davies vacuum assumes a particularly simple form:
\begin{splitequation}
...
As before, all the temporal components of the invariant metric correlator vanish, and we only need to be concerned with its purely spatial components. These are given by
\begin{splitequation}
...
\end{splitequation}
where we have expressed the functional time-dependence of the two-point function in terms of the conformal time $\eta$, and the tensor $K_{ijkl}(\vec{p})$ was defined in Eq.~\eqref{eq:K_ijkl}. It is thus clear that the invariant graviton two-point function also in the de~Sitter case only contains the propagating transverse and traceless tensor modes. Moreover, as in the flat-space case it is positive definite (apart from the usual infrared issues of massless fields in de~Sitter space) and thus has a spectral representation. The flat-space result~\eqref{eq:inv_2point_function_final_flat} can in fact be easily obtained from Eq.~\eqref{eq:inv_2point_function_final_dS}, expressing the conformal time $\eta$ in terms of cosmological time $t$ and then taking the limit $H \to 0$.
...
With a proper definition of a gauge-invariant Hubble rate, the issue of back-reaction can now be tackled anew. In previous work~\cite{froeb_lima_cqg_2018,froeb_cqg_2019,lima_cqg_2021}, we have already computed one-loop quantum corrections to the invariant Hubble rate in generalised harmonic coordinates. While these coordinates do not have such a clear physical interpretation as the synchronous one that we studied in this work (or the geodesic lightcone coordinates~\cite{gasperini_et_al_jcap_2011,fleury_nugier_fanizza_jcap_2016,froeb_lima_jcap_2022,fanizza_marozzi_medeiros_arxiv_2023}), the computation done there shows that back-reaction exists for a fully gauge-invariant observable and, thus, is not merely a gauge effect. Moreover, the results obtained there agree with the physical intuition: the accelerated expansion of the background spacetime creates particles and in particular gravitons, whose mutual attraction then slows down the expansion. In the future, we would like to compute the back-reaction also for the invariant Hubble rate in synchronous coordinates, and verify that it persists also for measurements done in this coordinate system.
...
The authors thank Kevin Falls, Renata Ferrero and Manuel Reichert for discussions on relational observables at the early stages of this work. M.B.F.\ acknowledges the support by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation), project No.~396692871 within the Emmy Noether grant~CA1850/1-1 and project No.~406116891 within the Research Training Group RTG~2522/1.
...
\end{equation}
which corresponds to the equation of motion for the background scalar field $\phi$. Friedmann's equations can also be easily checked using that identifications.
...
\mathscr{d} = \frac{\epsilon}{(n-1) H} \left[ \partial_t Q - (n-1) H \left( Q + a^{-2} \mathcal{T} \right) \right] \eqend{,}
\end{equation}
where we have used Eq.~\eqref{eq:sasaki_mukhanov_variable} to eliminate $\upphi$ and the constraint equation~\eqref{eq:T_eq} for $\mathcal{T}$, together with the identification~\eqref{eq:N_V_identification_SVT}. We can now check the equation of motion for $\mathscr{d}$~\eqref{eq:eom_d_SVT}. Indeed, if we substitute Eq.~\eqref{eq:d_phi_comparison} into the left-hand side of Eq.~\eqref{eq:eom_d_SVT}, then the equation of motion for $\mathscr{d}$ is satisfied if the equation of motion for $Q$~\eqref{eq:eom_sasaki_mukhanov_variable} is fulfilled (or, alternatively, if the equation of motion for $\upphi$~\eqref{eq:eom_upphi} holds).
