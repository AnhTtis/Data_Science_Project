The methods of \Cref{sec:proofs} allow us to determine the topological $4$-genus up to an error of $1$ for knots which are closures of positive $3$-braids under an additional assumption on their Xu normal form (see \Cref{prop:braidposbound}). In certain cases, we can determine $\gtop$ exactly (see \Cref{ex:exactex1,ex:exactex2} and \Cref{rem:exactfamilies}). 
For these results, we use $\frac12 |\sigma| \leq \gtop$.
We end with a discussion of examples where this bound is not sharp (\cref{rem:sign}) and give examples where instead of untwisting, we determine $\gtop$ via their algebraic genus (\cref{ex:galg}).
We are led by the following questions.
\begin{question}
Does the equality $\frac12 \hatsigma(K) = \gtop(K)$ hold for all braid positive 3-braid knots?
For all strongly quasipositive 3-braid knots? For all braid positive knots?
\end{question}
...
= \delta a^4 xabxa^2 
= x^5 bxabxa^2\\
&\sim b^5 abxabx x \to b^5 x,
\end{align*}
which turns into $bx\sim \delta$ using two crossing changes. Recall that the two twists needed to untwist $abxabx \to \varnothing$ are shown in \cref{fig:abxabx} of \Cref{sec:proofs}.
Now, for $\ell \geq 2$, we have
\begin{align*}
\delta^{3\ell+2}a^2 &= \delta^{3\ell-3}x^5 bxabxa^2\sim \delta^{3\ell-3}b^5 abxabx x \to \delta^{3\ell-3}b^5 x \\&\sim \delta^{3\ell-3} \delta b^4
= \delta^{3\ell-3} a \delta b^3 \sim \delta^{3\ell-1} b^2 \sim 
\delta^{3(\ell-1)+2} a^2,
\end{align*}
where we again used two twists for `$\to$'. Inductively this shows that $\delta^{3\ell+2}a^2$ can be untwisted with $2\ell+2$ twists as claimed. 
\end{example}

\Cref{ex:exactex1} combined with the results from \cite{BBL} for $3$-strand torus knots shows that
the equalities $\gtop = \tu =\frac{\hatsigma}{2}$ hold for all strongly quasipositive $3$-braid knots in Xu normal form (a) or (b) from \Cref{lem:xuform}, where $\hatsigma = \left|\sigma \right|$ except for certain torus knots of braid index $3$; see also \Cref{rem:sign}. We next consider a sub-case of case (c). 
...
= xa^4 x a b x ab^2 \\&\sim ab^4abxabxx \to  ab^4x,
\end{align*}
which can be untwisted using two crossing changes. 
For $\ell \geq 2$, we have
\begin{align*}
\delta^{3\ell +1}a^2 b^2 &=\delta^{3\ell-3} xa^4 x a b x ab^2 \sim \delta^{3\ell-3} ab^4abxabxx \to\delta^{3\ell-3} ab^4x \\
&= \delta^{3\ell-4} x a^3 ba bx  
= \delta^{3\ell-5} b x^3 a^2 x a bx 
= \delta^{3\ell-6} a b^3 x^3 b x a bx 
 \\&\sim \delta^{3\ell-6} a^3 b^3 abxabx \to \delta^{3\ell-6} a^3 b^3 \sim \delta^{3\ell-5} a^2 b^2 = \delta^{3(\ell-2)+1} a^2 b^2,
\end{align*}
...
\to \tau_{0}\delta^{3\ell + 2}  \tau_1\tau_2 \tau_6 \sim \delta^{3(\ell+1)+1}
\end{align*}
...
\\&\to \tau_{(1-r)-r}\delta^{3\ell + r} \prod_{i=1}^{r-2} \tau_{i-r-1}^2 \tau_{1}\tau_{2}\tau_{3}\prod_{i={r+4}}^{2r+1} \tau_{i-r}^2
...
Let us try to determine the topological $4$-genera of the knots arising as closures of the following positive 3-braids:
\begin{align*}
...
which is practical for individual knots with low genus. 
...
an upper bound $g_{\text{alg}}(K) \geq \gtop(K)$~\cite{FL}, satisfies $g_{\text{alg}}(K) \leq g(K) - 2$ for all our knots $K$.
\end{example}
