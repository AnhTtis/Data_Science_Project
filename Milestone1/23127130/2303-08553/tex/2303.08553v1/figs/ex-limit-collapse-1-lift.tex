\begin{tikzpicture}
  ...
  %
  \path (acd) ++ (-1.5cm,1.25cm) node {\LARGE $\aonechartacci{1}$};
  
  \draw[<-,very thick,color=chocolate,>=latex](abc) -- ++ (90:0.5cm););
  %
  \path (abc) ++ (0.1cm,0.35cm) node[right]{$\tightfbox{$abc$} = \avert$};
  \draw[->,very thick,royalblue] (abc) to node[right,pos=0.65]{$\hspace*{-0.2em}\black{a}$} (a1);
  ...
  \draw[->,thick,densely dotted,out=135,in=0] (abc) to node[pos=0.5,above,xshift=0.045cm]{$\sone$} (ac);
  % 
  %
  %
  \path (ac) ++ (0cm,0.4cm) node{$\tightfbox{$ac$}$};
...
%   \end{scope}
  ...
  %
  \path (acd) ++ (0cm,0.4cm) node[left]{$\tightfbox{$acd$}$};
