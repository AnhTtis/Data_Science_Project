\begin{tikzpicture}
  ...
  % 
  \path (abc) ++ (-1.5cm,0.5cm) node{\LARGE$\aonechart\:/\:\aonecharthat$};
  %
%   % Mark the angle XAY
%   \begin{scope}
%     \path[clip] (A) -- (X) -- (Y);
...
%   \end{scope}
  ...
  \draw[->,bend right,relative=false,out=-45,in=167.5,looseness=1] (acd) to node[above,pos=0.84]{$d$} (e);
  %
  ...
  %
  \path (abcd-2) ++ (0.05cm,0cm) node[right]{$\tightfbox{$(abcd)_2$} \parbox[t]{\widthof{${} = \bvertbari{2}$}}
                                                                             {${} = \bvertbari{2}$
                                                                              \\
                                                                              ${} = \bverti{2}$}$};
  ...
  %
  \path (abcd-1) ++ (0cm,-0.3cm) node[left]{$\bverti{1} = \tightfbox{$(abcd)_1$}$};
  \draw[->] (abcd-1) to node[left]{$b\hspace*{-0.1em}$} (f);
  \draw[->,densely dotted,thick,bend left,relative=false,out=180,in=180,looseness=2] (abcd-1) to node[left,pos=0.45,xshift=0.05cm]{$\sone$} (acd);
  ...
  % bisimulation
  \draw[densely dashed,very thick,magenta,bend right,distance=1.25cm] (abcd-1) to (abcd-2);
  %
\end{tikzpicture}
