\begin{tikzpicture}
  ...
  % 
  \path (abc) ++ (-2.5cm,0.5cm) node{\LARGE $\aonecharti{1/2}$};
  %
%   % Mark the angle XAY
%   \begin{scope}
%     \path[clip] (A) -- (X) -- (Y);
...
%   \end{scope}
  ...
  %
  \path (acd) ++ (0cm,0.4cm) node[xshift=-0.25cm]{$\tightfbox{$acd$}$};
...
%   \end{scope}
  ...
  \draw[->,densely dotted,red,thick,bend right,distance=1.25cm,relative=false,out=90,in=0] (abcd-2) to node[right,pos=0.45,xshift=0.05cm]{$\sone$} (abc);
  \draw[->,red,thick,densely dotted,out=95,in=40,distance=4.15cm,shorten >= 3pt] (abcd-2) to node[above,pos=0.375]{$\sone$} (acd);
  %
  ...
  \draw[<-,very thick,color=chocolate,>=latex](abc--coll) -- ++ (90:0.5cm););
  %
  \path (abc--coll) ++ (0.1cm,0.35cm) node[right]{$\tightfbox{$abc$} = \avert$};
  \draw[->] (abc--coll) to node[right]{$\hspace*{-0.2em}\black{a}$} (a1);
  \draw[->] (abc--coll) to node[right]{$\hspace*{-0.2em}\black{a}$} (a2);
  \draw[->] (abc--coll) to node[left,pos=0.25]{$\hspace*{0.2em}\black{c}$} (c1); 
  \draw[->] (abc--coll) to node[right,pos=0.35,xshift=-0.15em]{$\black{c}$} (c2);
  \draw[->] (abc--coll) to node[right,pos=0.85]{$\black{b}$} (f);
  % 
  \path (abc--coll) ++ (-2.5cm,0.5cm) node{\LARGE $\acharti{10}$};
  %
%   % Mark the angle XAY
%   \begin{scope}
%     \path[clip] (A) -- (X) -- (Y);
...
%   \end{scope}
  ...
  %\draw[->,bend left,distance=0.75cm,out=110] (a1) to (abc--coll);
  \draw[->,bend left,distance=2.9cm,relative=false,out=0,in=-20,shorten >=4pt] (a1) to node[right]{$a_1\hspace*{-0.18em}$} (abc--coll);
  %
  \path (a2) ++ (0.32cm,0.4cm) node{\tightfbox{$a_2$}};
  \draw[->] (a2) to node[above,near end]{$a_2$} (abcd-2);
  \draw[->,bend left,distance=1.65cm,relative=false,out=-5,in=-20,shorten >=4pt] (a2) to node[left,pos=0.65]{$a_2$} (abc--coll);
  %\draw[->,bend left,distance=2cm,out=80,in=120] (a2) to (abc--coll);
  %
  \path (c1) ++ (-0.35cm,0.25cm) node{\tightfbox{$c_1$}};
  \draw[->,bend left,relative=false,out=200,in=180,looseness=1.9] (c1) to node[pos=0.55,right]{$c_1\hspace*{1.5em}$} (acd);
  \draw[->,out=280,in=210,distance=2.5cm] (c1) to node[below]{$c_1$} (abcd-2);
  ...
  %
  \path (e) ++ (0cm,-0.4cm) node{\tightfbox{$e$}};
  % \draw[->] (e) .. controls ($(abcd-2) + (-2cm,0.5cm)$) and ($(abc--coll) + (-1.75cm,-0.25cm)$) .. (abc--coll);
  \draw[->,relative=false,out=12.5,in=0,distance=5cm] (e) to node[right]{$e$} (abc--coll);
  %
  \path (f) ++ (0cm,-0.45cm) node{\tightfbox{$f$}};
  %\path (f) ++ (0.35cm,0.4cm) node{\tightfbox{$f$}};
  \draw[->,relative=false,out=180,in=180,distance=3.5cm] (f) to node[left]{$f$} (acd);
  ...
  %
  %
  \path ($(abc)!0.5!(abc--coll)$) ++ (0cm,0cm) node{\Huge $\sotelimredtc$};
  %
\end{tikzpicture}