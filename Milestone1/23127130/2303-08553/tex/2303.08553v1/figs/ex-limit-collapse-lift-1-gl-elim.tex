\begin{tikzpicture}
  ...
  % 
  \path (abc--C10) ++ (-2.5cm,0.5cm) node{\LARGE $\aonechartacci{10}$};
  %
%   % Mark the angle XAY
%   \begin{scope}
%     \path[clip] (A) -- (X) -- (Y);
...
%   \end{scope}
  ...
  \draw[->,out=105,in=60,distance=5.4cm,shorten >= 3pt] (abcd-2--C10) to node[left,pos=0.78]{\black{$\cact$}} (c1--C10);
  \draw[->,out=110,in=65,distance=5cm,shorten >= 3pt] (abcd-2--C10) to node[left,pos=0.78]{\black{$\cact$}} (c2--C10);
  %
  ...
  % 
  \path (abc--C1) ++ (-1.5cm,0.5cm) node{\LARGE $\aonechartacci{1}$};
  %
%   % Mark the angle XAY
%   \begin{scope}
%     \path[clip] (A) -- (X) -- (Y);
