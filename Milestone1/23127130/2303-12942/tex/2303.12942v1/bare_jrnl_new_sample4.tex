...
\lhead{This work has been submitted to the IEEE Transactions on Network and Service Management for possible publication} % except the center
...
\title{A Survey on Explainable Artificial Intelligence for  Network Cybersecurity
}
...
\begin{IEEEkeywords}
Explainable Artificial Intelligence (XAI), Network Cybersecurity, Interpretability, Trustworthiness.
\end{IEEEkeywords}
...
In the domain of network cybersecurity, the application of AI has become mainstream in both research and industry. This interest is largely driven by the fact that AI-based tools and methods are claimed to be able to detect and prevent sophisticated attacks that could not be detected and prevented by conventional approaches. However, such tools and methods also raise concerns regarding their potential misuse, especially in critical domains such as cybersecurity. This opens up a debate on the need for AI tools that are able to explain their behavior. With the aim of providing a better understanding of the importance and need for XAI in the domain of cybersecurity, we survey in this paper state-of-the-art XAI, with a particular focus on XAI approaches applicable to network cybersecurity. We provide an overview on the general principles and methodologies for XAI literature and analyze their potential benefits when applied to network cybersecurity. We then provide a comprehensive classification of the cybersecurity domain, which we use to discuss the literature and promising applications of XAI in each cybersecurity sub-domain. Finally, we discuss promising directions for future research and some ethical challenges associated with XAI for cybersecurity.

%To the best of our knowledge, our work is the first to provide a survey on XAI in the domain of cybersecurity. 
The future of network cybersecurity will be defined by how we approach AI models and how we can make them safer, transparent and efficient through the use of XAI solutions. For example, ML models that can explain their decisions can reduce the number of false alarms, which can lead to better security for users. They can also reduce the need for human oversight, which can save money for organizations. As we move toward a world where we can program our machines to make decisions for us, we will need to build systems that can explain their actions to humans when appropriate. It is essential, despite the fact that this is one of the very few surveys on XAI for cybersecurity, %the first and possibly only mention of XAI-based cybersecurity in the body of published survey research, 
that we begin this discussion with the caveat that there is a long way to go before we see widespread use of XAI in the real world for the purpose of ensuring network cybersecurity.  To analyze the impact of this fact on the field and discuss how it can be addressed in the future, it is necessary to identify the various ways in which XAI can be used to improve cybersecurity. This can be done by examining existing products and services that are based on XAI, as well as the proposed use cases. It is also important to discuss the challenges that have been faced in the field of network cybersecurity, and how XAI can be used to address them. More specific contributions of this article are reported in Section \ref{related} when we compare our survey with the related ones.

The remainder of the paper is organized as follows. Preliminary concepts important for the rest of the paper are explained in Section \ref{Preliminaries}. A detailed description of the systematic methodology used to search for relevant research articles is provided in Section \ref{Methodology}.  Section \ref{related} presents related review papers and discusses similarities and differences with the current survey paper, which helps us identify the unique characteristics and the value added of our survey. A Classification of the selected scientific articles on explainability of AI models for network cybersecurity based on the proposed solutions is provided in Section \ref{Papers_Classification}.
Section \ref{Math_Models} discusses the use of mathematical models for explainability in AI models for network cybersecurity.
Section \ref{xai-methods} provides a classification of XAI methods and techniques for network cybersecurity.
Section \ref{xai-cyb} explores the use of XAI-based solutions for network cybersecurity.
Section \ref{criteria} introduces a set of desirable criteria for future solutions for explainability in AI models for network cybersecurity. Section \ref{future} outlines potential directions for future work in the field of explainability in AI models for network cybersecurity.
Finally, Section \ref{conc} concludes the paper and summarizes the key findings and contributions of the study.

\section{Preliminaries} \label{Preliminaries}
Before delving into the literature of XAI-based network cybersecurity, let us describe key terms that will be used throughout this article. It is important to distinguish between the terms of explainability, interpretability, trustworthiness, interactivity, stability, and robustness as they are very related and often used interchangeably when in fact, they have different meanings. %In particular, the following is a list of key terms that are necessary to understand the rest of the paper, which we relate to each other in the knowledge:
...
Finally, there is a lack of standardization in the metrics and evaluation criteria used in XAI cybersecurity surveys, particularly network cybersecurity, which makes it difficult to compare results across studies and to draw definitive conclusions about the effectiveness of different XAI techniques.
...
The survey we present in this article aims to provide a comprehensive review of the current literature on XAI methods for network cybersecurity applications, including a review of existing challenges and problems, identification of available frameworks and datasets, and analysis of successful XAI-based systems and applications. Its ultimate goal is to identify research gaps and future directions for applying XAI in the network cybersecurity domain. Compared to the other surveys discussed in this section, our survey offers several unique features and advantages. \textbf{The first aspect} that sets this survey apart from related surveys that have a broader focus on different applications of XAI is its specific focus on the application of XAI methods in the field of network cybersecurity. This specific focus on the intersection of XAI and network cybersecurity allows us to offer a more targeted and comprehensive analysis of this particular domain, which will help researchers, network security engineers and practitioners identify the existing solutions and gaps. \textbf{Second}, the survey introduces two unique and systematic mappings linking eight XAI model classes that we discuss in this survey to nine desirable criteria that we identify in this article for XAI network cybersecurity solutions. The first mapping is linking the eight comprehensive XAI models to our taxonomy of six network cybersecurity classes. The second mapping is about linking the identified six classes of network cybersecurity to the nine assessment criteria of network cybersecurity XAI solutions. The two mappings allows us to introduce a more organized and structured overview of the different approaches and techniques used in the field of network cybersecurity. Combining the two mappings provides a clear understanding of which XAI models are applicable to which network cybersecurity classes, and how they can be used to address specific network cybersecurity challenges and threats. This is a unique contribution compared to the existing surveys as it helps researchers and practitioners navigate the complex landscape of XAI and network cybersecurity and make informed decisions about which techniques to use in specific scenarios. Moreover, identifying and mapping the different network cybersecurity classes to desirable criteria of explainable AI for network cybersecurity provide a clear and concise framework for evaluating and comparing different XAI models and their suitability for different network cybersecurity applications. 
This mapping allows for a more thorough evaluation of the XAI models in terms of their ability to meet specific criteria and address the unique challenges of cybersecurity. This approach can also aid researchers and practitioners in selecting the most appropriate XAI model for a particular cybersecurity task based on the desired outcomes and criteria. \textbf{Third}, our survey provides a detailed discussion of the limitations and challenges of current XAI approaches in network cybersecurity, as well as potential solutions and future research directions.
...
%\textbf{First}, our survey focuses specifically on the intersection of XAI and network cybersecurity, which allows for a more targeted and comprehensive analysis of this particular domain.
%\textbf{Second}, our survey provides a systematic mapping of XAI models to network cybersecurity classes, which allows for a more organized and structured overview of the different approaches and techniques used in the field. This mapping provides a clear understanding of which XAI models are applicable to which network cybersecurity classes, and how they can be used to address specific cybersecurity challenges. This is a unique contribution to the existing surveys, as it helps researchers and practitioners to navigate the complex landscape of XAI and cybersecurity and make informed decisions about which techniques to use in specific scenarios.
%\textbf{Third}, our survey identifies and maps the different classes to desirable criteria of explainable AI for network cybersecurity, which provides a clear and concise framework for evaluating and comparing different XAI models and their suitability for different cybersecurity applications. 
...
%\textbf{Fourth}, our survey provides a detailed discussion of the limitations and challenges of current XAI approaches in network cybersecurity, as well as potential solutions and future research directions.
...
\end{tabular}  & \begin{tabular}{l}
\begin{minipage}[t]{0.8\columnwidth} \begin{itemize}
\item This survey not target a specifice audience, but it is aim including policymakers, ethicists, and members of the general public who are interested in understanding the implications of XAI for cybersecurity. Our work primarily aim at researchers and practitioners in the field of cybersecurity who are interested in understanding the current state of XAI models for network security. 
\item Our survey focus heavily on the technical details of various XAI models, including their strengths and weaknesses, their performance on different types of cybersecurity tasks, and the specific features of network data that they are able to analyze effectively. \cite{srivastava2022xai}, in contrast, places more emphasis on the broader societal and ethical issues related to XAI in cybersecurity, such as the potential impact of these technologies on privacy, fairness, and accountability.
\end{itemize}\end{minipage}\tabularnewline
\end{tabular}\\\\\\

Capuano al. \cite{capuano2022explainable} & \begin{tabular}{l}
\begin{minipage}[t]{0.8\columnwidth}
Similar to our survey, this survey provide a comprehensive list of XAI techniques and algorithms that can be used in cybersecurity applications.
\end{minipage}\tabularnewline
\end{tabular}  & \begin{tabular}{l}
\begin{minipage}[t]{0.8\columnwidth} \begin{itemize}
\item This survey classify XAI models into four categories based on their interpretability: black box, glass box, gray box, and white box models. They then provide an overview of various XAI models within each of these categories and discuss their strengths and limitations.

In contrast, our survey included a wider range of classification methods and techniques used in XAI for cybersecurity models. Additionally, our survey includes a detailed discussion of the strengths and limitations of each method, which provides readers with a comprehensive understanding of the different approaches.
...
\end{tabular}  & \begin{tabular}{l}
\begin{minipage}[t]{0.8\columnwidth} \begin{itemize}
\item Our survey uses a classification approach to organize the XAI models and techniques for network cybersecurity, while the work in \cite{charmet2022explainable}  uses a narrative approach to summarize the existing literature on XAI in cybersecurity in general. 
\item Our survey places a greater emphasis on recent developments and emerging trends in XAI models for network cybersecurity, while the work in \cite{charmet2022explainable}  has a historical perspective on the evolution of XAI in cybersecurity.
...
\end{tabular}  & \begin{tabular}{l}
\begin{minipage}[t]{0.8\columnwidth} \begin{itemize}
\item Our survey places greater emphasis on the challenges and limitations of XAI models for network cybersecurity and provides future directions for research in this area, while the work in \cite{zhang2022explainable} is more descriptive and less prescriptive.

\item Our survey aims to provide a comprehensive classification of XAI models for network cybersecurity, focusing on the types of models, techniques, and evaluation metrics used. It also covers a wide range of XAI models and their applications in network security. In contrast, the work in \cite{zhang2022explainable} primarily focuses on summarizing and analyzing the results of individual studies and experiments that have used XAI models for cybersecurity.
...
\section{Classification of Scientific Articles on Explainability of AI Models for Network Cybersecurity} \label{Papers_Classification}

As a result of conducting a thorough analysis of all selected articles following the methodology explained in Section \ref{Methodology}, the following network cybersecurity categories were identified:
...
\end{enumerate}
    ...
    Authentication in network cybersecurity refers to the process of verifying the identity of a user, device, or system before granting access to a network system. The goal of authentication is to ensure that only authorized individuals and devices have access to sensitive information and resources.
...
\end{enumerate}
Building trust and maintaining a positive reputation are important for organizations and individuals as they help to build trust and confidence among users and stakeholders, and to prevent network security breaches and other malicious activities. To enhance trust and reputation in network cybersecurity, organizations should focus on implementing robust network security and privacy measures, maintaining transparency and accountability, and responding promptly to network security incidents and online data breaches.
\end{itemize}

We will discuss the-state-of-the-art for each one of the six network cybersecurity classes in Section \ref{xai-cyb} and identify a set of criteria that we believe important to consider when designing future XAI solutions for network cybersecurity.
\begin{figure*}[!ht]
...
In order to gain a full picture of the research area of XAI in network cybersecurity, we used bibliometric analysis. Our study shows the state of the field by presenting a map of the main problem-specific notions based upon author keywords, as well as an annual scientific production report.  We utilized the tool VOSViewer \cite{van2013vosviewer} to generate the author keyword maps. 
\begin{figure*}[!htbp]
 \includegraphics[width=\textwidth,height=.7\textwidth]{XAI.png}
  \caption{The map of author keywords for XAI with network cybersecurity publications}
  \label{fig6}
\end{figure*}
Fig. \ref{fig5} shows a graph containing the most popular author keywords for XAI. It can be concluded that XAI is often investigated in the context of AI (i.e.  ‘‘NLP’’, ‘‘CNN’’, "deep reinforcement learning", etc) as well as medical imaging (i.e. ‘‘COVID-19’’, ‘‘clinical support system’’, etc.) and network cybersecurity (i.e. ‘‘trust’’, ‘‘anomaly detection’’, ‘privacy’’, etc). 
Similar notions are observed to be essential for network cybersecurity  explanation as shown in Fig. \ref{fig6}. However, a distinction between different clusters in the latter case is visible more clearly. This is hypothesized to be due to a more diverse usage of the term XAI across various cybersecurity areas.

XAI has emerged as a key area of research in recent years, attracting attention across a wide range of subject areas. As shown in Fig. \ref{fig3}, there has been a significant increase in the number of XAI publications over the past three years, indicating the growing interest in this field. In particular, we focus on the intersection of XAI and network cybersecurity, as shown in Fig. \ref{fig4}. It should be noted that the number of publications in 2023 is limited by the search date, and the actual number of publications is likely to be higher.
...
\begin{figure*}[!ht]
 \includegraphics[width=\textwidth,height=.4\textwidth]{publ2.png}
  ...
  \caption{Annual scientific production for XAI with network cybersecurity publications}
  \label{fig4}
\end{figure*}
...
\section{XAI-based Network Cybersecurity}
\label{xai-cyb}

%In the literature surveys, nothing has been said about cybersecurity linked to XAI and not much attention has been payed to this concept. It is important, though, as this is the first (and possibly only) mention of  cybersecurity in the literature, that XAI-based cybersecurity is a concept with multiple definitions. 
As discussed earlier in Section \ref{related}, very few surveys have addressed the application of XAI to cybersecurity, but no paper has focused on the network cybersecurity field. It is important, though, to draw attention to this topic, particularly since the use of technologies such as big data, cloud computing, and IoT is continually growing, along with the need to employ technologies (i.e. AI and ML) that prevent attacks on data and networks.
To this end, network security can benefit greatly from XAI and the advancements in ML to avoid the mass exploitation of vulnerabilities that can be used by attackers for personal gain and to implement measures that can detect and remediate advanced network cyberattacks. Considering the above, network cybersecurity linked to XAI, particularly the use of AI/ML for its implementation, can help users avoid attacks that are easy to launch but difficult to detect and can block access to critical systems as a means of carrying them out. Moreover, AI and ML techniques applied to network cybersecurity can offer a better approach for detecting network cyberattacks, such as botnets, brute force attacks, or attempts to penetrate a network system with weak authentication.

AI, in particular the field of ML, has also gained attraction recently in the network cybersecurity field, with large number of works on automated diagnosis, prognosis, and risk assessment. Recent research has also focused on using AI and ML for incident response, in order to quickly detect and mitigate new threats \cite{kuppa2021adversarial}. This is especially critical as the cyber threat landscape changes rapidly, with zero-day exploits becoming a rarity, and more traditional attack vectors like web applications and email becoming the preferred method of an attacker. The AI techniques used in these areas are also finding applications in other industries, such as finance, advertising, and healthcare. For instance, AI is being used to analyse large amounts of data in order to discover trends and correlations that human analysts would otherwise miss. This enables the discovery of fraudulent or anomalous transactions that would have gone unnoticed by human analysts.   
To pay more attention to this topic and consider how it fits into XAI, we will assess the results of the proposals that have been launched on these topics. We will perform this assessment in light of our network cybersecutity classification we present in the following.

\subsection{Privacy} 
XAI can play a significant role in ensuring privacy by providing a deeper understanding of how AI models handle and process sensitive information related to network cybersecurity threats and vulnerabilities. For example, XAI can help us identify any potential privacy issues that may arise in the data processing and analysis stages, such as the unauthorized access or sharing of sensitive information. Additionally, XAI can provide insights into the model's decision-making process and help us ensure that the data is being used for its intended purpose, such as detecting and preventing cyber attacks.
In order to increase privacy in XAI for network cybersecurity, researchers  explored various techniques and approaches, such as differential privacy \cite{saifullah2022privacy,wang2022consensus}, federated learning \cite{wahab2021federated,rjoub2022trust22,huong2022federated,barcena2022approach}, and homomorphic encryption \cite{muftuouglu2022privacy}. These techniques aim to protect sensitive data while still allowing the model to learn and make accurate predictions about cyber threats and vulnerabilities.
...
{\textwidth}
\begin{center}
\caption{Comparative summary of the main XAI techniques for network cybersecurity}
\label{table777}
\hspace*{-77.0pt}
...
Table \ref{table777} presents a comparative summary of XAI techniques used in network cybersecurity, along with the approach, XAI method level, and cybersecurity class. The table includes various studies that have used different XAI techniques, such as local explanation, global explanation, post-hoc explanation, pre-hoc explanation, and model-specific explainers to analyze network cybersecurity issues according to our classification introduced in Section \ref{Papers_Classification}, which includes  privacy, trust, intrusion detection and prevention, access control, and authentication.

\section{Desirable Criteria for Future Solutions}
\label{criteria}
Our classification and discussion above have led us to identify key crucial criteria that, in our opinion, ought to be taken into account when developing future solutions for XAI and network cybersecurity. We will present and debate how these criteria may be practically applied to design effective solutions in the areas of XAI and network cybersecurity, using these criteria as our guide. %We propose to take into consideration these criteria for potential future solutions in the fields of XAI and network cybersecurity:
...
{\begin{minipage}
{\textwidth}
\caption{ Summary of the main XAI techniques and criteria for network cybersecurity}
\hspace{-2.5cm}
\small
{\large %
...
XAI in the domain of network cybersecurity applications.
...
\subsection{Network Cybersecurity-Specific Evaluation Metrics}
...
We have also clarified specific challenges that arise for XAI when the AI systems in question are being used for network cybersecurity. In order to ground our survey of various models that are in use, we then distinguished a core collection of key terms that are of concern. Following this, we made clear how our particular survey differs from other efforts to date that summarize XAI approaches in the literature. We then situated ourselves squarely in our area of focus, XAI for network cybersecurity, outlining some well-founded decisions for what to study, how to characterize the field and and how best to present our results.
...
We reflected at great length on the most important considerations for the network cybersecurity application, leading to a comparative summary of the main XAI techniques for systems that are designed for this domain.
A key contribution is our specific analysis that proposes the primary
criteria to resolve, in order to achieve successful XAI network cybersecurity
solutions. This is followed by a deep discussion of key
...
to engender trust of AI systems through XAI, but with a new awareness of the landscape of methods and challenges, when network cybersecurity is the central concern of these AI systems.
...
%\begin{IEEEbiographynophoto}{Gaith Rjoub}
