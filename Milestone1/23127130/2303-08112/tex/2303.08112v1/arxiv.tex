...
One limitation of our method is that it involves training a translator layer for each layer of the network, while the logit lens can be used on any pretrained model out-of-the-box. This training process, however, is quite fast: our code can train a full set of probes in under an hour on a single 8$\times$A40 node, and further speedups are likely possible. We also plan to release tuned lens checkpoints for the most commonly used pretrained models, which should eliminate this problem for most applications.
