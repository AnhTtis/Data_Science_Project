We consider the problem of image classification where our dataset is denoted by $(x_i, y_i)_{i=1}^N$, with $x_i \in \mathbb{R}^{H \times W \times 3}$ and $y_i \in \{1, \ldots, C\}$. Here, $x_i$ denotes the pixel values of the $i$-th input image with height $H$ and width $W$, while $y_i$ denotes the corresponding label. Our goal is to train a network $y = \phi(x)$ that maps an input image $x$ to a label $y$. Specifically, we strive to formulate the celebrated ResNet \cite{he2015b} architecture in the Poincar\'e ball model.
...
We start by formalizing 2D convolutional operations for images in the Poincar\'e ball model using the approach of Shimizu \etal~\cite{shimizu2021}. Suppose we have an input image $x$ with pixel values
\begin{equation}
    x_{ij} \in \mathbb{B}_{c}^{C_{in}}, \quad i = 1, \ldots, H_{in}, \enspace j = 1, \ldots, W_{in},
\end{equation}
...
\begin{equation}
    h_{ij} \in \mathbb{B}_c^{C_{out}}, \quad i = 1, \ldots, H_{out}, \enspace j = 1, \ldots, W_{out},
\end{equation}
where $h_{kl}$ is computed from the pixels $x_{ij}$ in the receptive field at that position, so where
\begin{align}
\begin{split}
...
\end{split}
\end{align}
We will denote this receptive field at position $(k, l)$ by $X_{kl}$. Note that $H_{out}$ and $W_{out}$ depend on the input dimensions, the receptive field size $K$ and, optionally, on stride and padding.

Similar to the Euclidean convolutional layer, for each $h_{kl}$, we want to apply a fully connected layer to the concatenation of the vectors within the receptive field, so we want to compute the output as
\begin{equation}
    h_{kl} = \mathcal{F}^c ({\parallel} X_{kl}; Z, r),
\end{equation}
where ${\parallel} \cdot$ denotes some concatenation operation and $\mathcal{F}^c$ is the Poincar\'e fully-connected layer defined in equation (\ref{eq:FC_layer}) with parameters $Z$ and $r$. Note that the usual concatenation is inappropriate for vectors on the Poincar\'e ball as this can result in vectors outside the manifold. We therefore employ $\beta$-concatenation as an alternative, which is a concatenation operation that preserves the expectation of the Poincar\'e norm of the output vector \cite{shimizu2021}. This operation, applied to $M$ Poincar\'e vectors $\{b_i \in \mathbb{B}_c^{n_i}\}_{i=1}^M$ with $n = \sum_i n_i$, is defined in three steps:
\begin{enumerate}
    \item Map each of the vectors to the tangent space at the origin of their respective Poincar\'e balls: $v_i = \log_0^c (b_i)$;
    \item Let $\beta_n = B(\frac{n}{2}, \frac{1}{2})$, with $B$ the beta function, scale each of the vectors $v_i$ by $\beta_n \beta_{n_i}^{-1}$ and let $v$ be the concatenation of these scaled vectors, so $v = (\beta_n \beta_{n_1}^{-1} v_1^T, \ldots, \beta_n \beta_{n_N}^{-1} v_N^T)^T$;
    \item Project the resulting vector back onto the $n$-dimensional Poincar\'e ball: $\exp_0^c (v)$.
\end{enumerate}
We denote this operation by $\prescript{}{\beta}{\parallel} \cdot$. Now, we can write the 2D Poincar\'e convolution operation as
\begin{equation}
    h_{kl} = \mathcal{F}^c (\prescript{}{\beta}{\parallel} X_{kl}; Z, r),
\end{equation}
...
We can furthermore replace the skip connection by $x \oplus_c \mathcal{G} (x)$, where $\mathcal{G}$ denotes the transformation given by the two Poincar\'e convolutional layers and Poincar\'e batch normalizations. The overall Poincar\'e residual block is visualized in \ref{fig:residual_block}. 
...
\begin{algorithm}[tb]
    ...
    \begin{algorithmic}
        \STATE {\bfseries Training Input:} Data batches $\{x_1^{(t)}, \ldots, x_m^{(t)}\} \subseteq \mathbb{B}_c^n$ for $t \in [1, \ldots, T]$, testing momentum $\eta \in [0, 1]$
        \STATE {\bfseries Learned Parameters:} $\beta \in \mathbb{B}_c^n$, $\gamma \in \mathbb{R}$
        \STATE {\bfseries Normalization Algorithm:} 
        \FOR{$t = 1, \ldots, T$}
            \STATE $\mu \gets \text{Poincar\'eMidpoint}(\{x_1^{(t)}, \ldots, x_m^{(t)}\})$
            \STATE $\sigma^2 \gets \frac{1}{m} \sum_{i=1}^m d(x_i^{(t)}, \mu)^2$
            \FOR{$i = 1, \ldots, m$}
                \STATE $\tilde{x}_i^{(t)} \gets \exp_{\beta}^c \Big( \sqrt{\frac{\gamma}{\sigma^2}} P_{\mu \rightarrow \beta}^c (\log_{\mu}^c x_i^{(t)}) \Big)$
            \ENDFOR
            \RETURN normalized batch $\tilde{x}_1^{(t)}, \ldots, \tilde{x}_m^{(t)}$
        \ENDFOR
    \end{algorithmic}
\end{algorithm}
...
We suggest to take an alternative aggregation of Poincar\'e vectors, namely the Poincar\'e midpoint \cite{ungar2009}. The midpoint of the Poincar\'e vectors $\{x_i \in \mathbb{B}_c^n\}_{i=1}^N$ is computed as
\begin{equation}
    \mu = \frac{1}{2} \otimes_c \frac{\sum_{i=1}^N \lambda_{x_i}^c x_i}{\sum_{i=1}^N (\lambda_{x_i}^c - 1)}.
\end{equation}
...
Using this initialization, for the case $m = n$, we can see that
\begin{equation}
    v_k(x) = \frac{1}{\sqrt{c}} \sinh^{-1} \Big(\sqrt{c} \lambda_x^c x_k\Big),
\end{equation}
and, therefore,
\begin{align}
    w = \lambda_x^c x_k,
\end{align}
from which it follows that $y = x$. When $m < n$, we get $y = (x^T | 0_{n-m}^T)^T$ instead, where $0_{n-m}$ is an $(n-m)$-dimensional vector of zeros. Thus, for the cases $m \leq n$, this initialization keeps the norms of the vectors constant throughout the network.

For residual networks, $m \leq n$ for each layer except for the linear layer at the end of the network. Therefore, we initialize each of the convolutional layers using our identity initialization. The final linear layer will be initialized using the initialization by \cite{shimizu2021}. We find the vanishing effect of this single layer to be harmless to the performance of the networks.


\subsection{Optimization and backward propagation}
For neural networks on Riemannian manifolds, one generally has to consider the manifold on which the parameters live to properly apply optimization \cite{bonnabel2013riemanniansgd}. For Poincar\'e residual networks, we need to consider the weights of three different layers, namely, the fully-connected layer, the convolutional layer, and the batch normalization. The parameters of the fully-connected layer and the convolutional layer as proposed by Shimizu \etal~ \cite{shimizu2021} live in Euclidean space, so we can use Euclidean optimizers for these layers. However, the batch normalization algorithm shown in Algorithm \ref{alg:batch_norm} makes use of a parameter vector living on $\mathbb{B}_c^n$. To avoid difficulties with optimizers, we instead supply the algorithm with a parameter vector in $\mathbb{R}^n$ that is mapped to the Poincar\'e ball using the exponential map around the origin. This approach is used by Lou \etal \cite{lou2020} as well. As a result, we can optimize Poincar\'e residual networks using traditional Euclidean optimizers.

A direct consequence of applying hyperbolic operations in a neural network is the large computational cost incurred by the many applications of nonlinear operations. This leads to a significant increase in memory requirements as all these intermediate steps become part of the computation graph during training. To maintain compact computation graphs, we have manually derived the backward pass of several core hyperbolic operations, namely M\"obius addition, the exponential and logarithmic maps and the conformal factor $\lambda_x^c$. The use of these manually defined derivatives also reduces the size of the computation graph of many other operations defined on the Poincar\'e ball, as these generally build upon the more basic operations. We find that using manually defined derivatives reduces memory usage by approximately 30\%, but increases computation time. Due to the length of the derivations, we provide a full breakdown in the supplementary materials.
