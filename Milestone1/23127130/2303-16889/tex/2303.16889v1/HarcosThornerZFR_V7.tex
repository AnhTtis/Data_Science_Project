...
\Re(s)\geq 1-\Cr{ZFR_Brumley}/((C(\pi)C(\pi'))^{n+n'-1+\epsilon}(|\im(s)|+1)^{n'n[F:\Q](1-\frac{1}{n+n'})+\epsilon}).
\end{equation}
...
\begin{theorem}
\label{thm:Siegel}
Let $\pi\in\mathfrak{F}_n$ and $\pi'\in\mathfrak{F}_{n'}$. For all $\epsilon>0$, there exists an ineffective constant $\Cl[abcon]{ZFR2}=\Cr{ZFR2}(\pi,\pi',\epsilon)>0$ such that for any $\chi\in\mathfrak{F}_1$ with analytic conductor $C(\chi)$, we have
\begin{equation}
...
\end{theorem}
\begin{remark}
By replacing $\chi$ with $\chi|\det|^{it}$, the bound \eqref{eqn:finalbound2} becomes
\begin{equation}
\label{eqn:ZFR}
...
\begin{equation}
\label{eqn:Edef}
\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C}):=\sum_{\substack{\N\ka\leq x \\ \ka\in\mathcal{C}}}\Lambda_{\pi\times\pi'}(\ka)-\frac{1}{|\mathrm{Cl}(\kq)|}\sum_{\chi\in\mathscr{P}(\kq)}\mathcal{M}_{\pi\times(\pi'\otimes\chi)}(x)=o(x)
\end{equation}
is equivalent to the fact that for all $\chi\in\mathscr{P}(\kq)$, the $L$-functions $L(s,\pi\times(\pi'\otimes\chi))$ do not vanish on the line $\Re(s)\geq 1$. 


Unconditionally, Brumley's narrow zero-free region \eqref{eqn:Brumley_ZFR} suffices to prove that there exist effectively computable constants $\Cl[abcon]{Brumley_PNTAP1},\Cl[abcon]{Brumley_PNTAP2}\in(0,1)$ depending at most on $n$, $n'$, and $[F:\Q]$ such that $\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C})\ll_{\pi,\pi'}x(\log x)^{-\Cr{Brumley_PNTAP1}}$ as long as $\N\kq\leq(\log x)^{\Cr{Brumley_PNTAP2}}$.  The bound $\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C})\ll_{\pi,\pi',A}x(\log x)^{-A}$ for all $A>0$ holds in very limited cases, depending on the zero-free regions proved in \cite{Humphries,HumphriesThorner2,HumphriesThorner,JLTW}.  \cref{thm:Siegel} permits a significant improvement.
\begin{theorem}
\label{thm:PNTAP}
...
\begin{equation}
\label{eqn:pidecomp}
\pi=\pi^*\otimes|\det|^{it_{\pi}},\qquad \pi^*\in\mathfrak{F}_n^*,\quad t_{\pi}\in\R.
\end{equation}
...
\]
The absolute convergence for $\re(s)>1$ ensures that
\begin{equation}
\label{eqn:Jacquet-Shalika}
|\alpha_{j,\pi}(\kp)\alpha_{j',\pi'}(\kp)|\leq \N\kp,\qquad|\alpha_{j,j',\pi\times\pi'}(\kp)|\leq \N\kp.
\end{equation}
...
$F=\Q$, but a generalization to an arbitrary number field $F$ is straightforward (with extra constants depending on $[F:\Q]$ in the exponents). Second, \cite{SoundararajanThorner} tacitly assumes that each central character is trivial on the diagonally embedded positive reals, but this assumption is not used in the proof of \cite[Lemma~2.1]{SoundararajanThorner}. Third, we rely on the fact that $C(it,\pi\times\pi')$ in \eqref{eqn:bound_on_1-line} equals $C(\pi\times(\pi'\otimes|\det|^{it}))$.
...
\]
We similarly bound the second factor on the right-hand side of \eqref{eqn:BTCS}, and \eqref{eqn:BTbound} follows.
\end{proof}
...
We begin with a finiteness statement on $\GL_1$-twists.
...
$\omega_{\pi'}\chi^n=\bar{\omega_\pi}$. Since $F_v^\times/{F_v^\times}^n$ is finite for every place $v$ of $F$, the result follows.
\end{proof}
...
\begin{lemma}
\label{lem:residue}
Let $f(s)$, $g(s)$, $h(s)$ be holomorphic in an open neighborhood of $s_0\in\CC$. Assume that $|f(s_0)|=|g(s_0)|$. Then, for any integer $k\geq 1$, the residue
\[
\mathop{\mathrm{Res}}_{s=s_0}\frac{(f(s)g(s))^k}{(s-s_0)^{2k}}h(s)
\]
equals $|f(s_0)|=|g(s_0)|$ times a $\CC$-linear combination of monomials of the derivative values $f^{(\ell)}(s_0)$, $g^{(\ell)}(s_0)$, $h^{(\ell)}(s_0)$ for $\ell\in\{0,\dotsc,2k-1\}$. The monomials in the linear combination, and the modulus of each coefficient in the linear combination depend at most on $k$.
\end{lemma}
\begin{proof}
Expanding $f(s)$, $g(s)$, $h(s)$ into Taylor series around $s_0$, we obtain
\[
\mathop{\mathrm{Res}}_{s=s_0}\frac{(f(s)g(s))^k}{(s-s_0)^{2k}}h(s) = 
\sum_{\substack{\ell_1,\dotsc,\ell_{2k+1}\geq 0\\ \ell_1+\dotsb+\ell_{2k+1}=2k-1}}
\frac{h^{(\ell_{2k+1})}(s_0)}{\ell_1!\dotsb\ell_{2k+1}!}\prod_{j=1}^{k}f^{(\ell_j)}(s_0)g^{(\ell_{j+k})}(s_0).
\]
Since $\ell_{1}+\dotsb+\ell_{2k+1}=2k-1$, at least one of $\ell_1,\ldots,\ell_{2k}$ equals zero. Consequently, the $j$-product above has a factor of $f(s_0)$ or $g(s_0)$, and the result follows.
\end{proof}
...
\end{equation}
If $\chi^*$ is trivial, then assume also that $|t_\chi|>1$. We have that
\[
...
\begin{proof}
First, assume that $\chi^*$ is trivial, in which case $\chi=|\det|^{it_\chi}$ with $|t_\chi|>1$. The initial assumptions imply that $\mathcal{S}=\{1,1-it_\chi,1+it_\chi\}$. For each choice of $s_0\in\mathcal{S}$, the order of the pole of $D(s)$ at $s=s_0$ is even, say $2k$. Specifically, $k=2$ for $s_0=1$, and $k=1$ for $s_0=1\pm it_\chi$.
We consider the decomposition
\[
(s-s_0)^{2k}D(s)x^{s-\beta}\Gamma(s-\beta)=(f(s)g(s))^k h(s),
\]
where
\begin{itemize}
\item $f(s)=L(s+it_\chi,\pi\times\pi')$ and $g(s)=L(s-it_\chi,\tilde{\pi}\times\tilde{\pi}')$ for $s_0=1$;
\item $f(s)=L(s,\tilde\pi\times\tilde\pi')$ and $g(s)=L(s+2it_\chi,\pi\times\pi')$ for $s_0=1-it_\chi$;
\item $f(s)=L(s,\pi\times\pi')$ and $g(s)=L(s-2it_\chi,\tilde{\pi}\times\tilde{\pi}')$ for $s_0=1+it_\chi$.
\end{itemize}
These three cases correspond to the three lines in \eqref{eqn:D_def}, with $f(s)^k$ and $g(s)^k$ occurring on the relevant line.

Now we apply \cref{lem:residue} in conjunction with \cref{lem:Li1} and \eqref{eqn:BH}. The functions $f(s)$, $g(s)$, $h(s)$ defined above are holomorphic in the open disk $|s-s_0|<1-\beta$. Moreover,
\[
|f(s_0)|=|g(s_0)|=|L(1+it_\chi,\pi\times\pi')|.
\]
We are finished upon noting that, at the point $s_0$, the $\ell$-th derivative of $s\mapsto x^{s-\beta}$ is bounded by $x^{\epsilon/2}(\log x)^\ell$, while
\[
\left|\Gamma^{(\ell)}(s_0-\beta)\right|\leq\int_0^\infty r^{-\beta}|\log r|^\ell e^{-r}\,dr.
\]

Second, assume that $\chi^*$ is nontrivial. The initial assumptions imply that $\mathcal{S}=\{1\}$, and the order of the pole at $s=1$ is 4. We consider the decomposition
\[
(s-1)^{4}D(s)x^{s-\beta}\Gamma(s-\beta)=(f(s)g(s))^2 h(s),
\]
where
\[
f(s)=L(s,\pi\times(\pi'\otimes\chi))\quad\textup{and}\quad g(s)=L(s,\tilde{\pi}\times(\tilde{\pi}'\otimes\bar{\chi})).
\]
Now we apply \cref{lem:residue} in conjunction with \cref{lem:Li1} and \eqref{eqn:BH}, as before.
\end{proof}
...
Assume that the $L$-functions under \eqref{eqn:Lfns} are entire.
...
\end{proposition}
\begin{proof}
Recall the notations \eqref{eqn:pidecomp} and \eqref{eqn:Qdef}. If $\chi^*$ is trivial and $|t_{\chi}|\leq 1$, then \eqref{eqn:main_lower} holds because both sides are positive continuous functions of $t_\chi\in\R$. Henceforth, we may assume that $\chi^*$ is nontrivial or $|t_{\chi}|>1$. Subject to these constraints, we let $D(s)$ be as in \eqref{eqn:D_def} and $\mathcal{S}$ be the set of its poles. Let $x>1$ be a parameter to be chosen later. By the initial assumptions, $D(\beta)=0$. Hence $\mathcal{S}$ is also the set of poles of $D(s)x^{s-\beta}\Gamma(s-\beta)$ in the half-plane $\Re(s)>0$, and we shall use this below. If $\lambda_{D}(\ka)$ is the $\ka$-th Dirichlet coefficient of $D(s)$, then $\lambda_{D}(\ka)\geq 0$ by \cref{lem:nonneg}. Since $\lambda_{D}(\cO_F)=1$, we have by the residue theorem
\begin{align*}
...
At this point we choose $x = \max\left(1,Q^{2}|L(1,\pi\times(\pi'\otimes\chi))|^{-2}\right)$. If $x=1$, then \eqref{eqn:main_lower} is trivial. Otherwise, $x=Q^{2}|L(1,\pi\times(\pi'\otimes\chi))|^{-2}>1$, and the previous bound yields
...
In the first step, we prove \eqref{eqn:finalbound3} for $\chi\in\mathfrak{F}_1^{(1)}$. In the second step, we extend the validity of \eqref{eqn:finalbound3} to $\chi\in\mathfrak{F}_1^{(2)}$. In the third step, we establish \eqref{eqn:finalbound3} in full generality.
...
However, we cannot use the pair $(\beta,\eta)$ directly because the implied constants are not allowed to depend on $\chi$. Instead, we use that each step operates with a given subgroup $G\in\{\mathfrak{F}_1^{(1)},\mathfrak{F}_1^{(2)},\mathfrak{F}_1\}$, and by the above hypothesis there exists $(\beta,\psi)\in(1-\epsilon',1)\times G$ such that 
\begin{equation}
...
$G=\mathfrak{F}_1^{(1)}$. Let us write $\chi=|\det|^{it}\in G$ with $t\in\R$. If $L(s,\pi\times\pi')$ has a pole, then there exists $u\in\R$ such that
\[
\pi=\tilde\pi\otimes|\det|^{iu}\qquad\textup{or}
\qquad\pi'=\tilde\pi'\otimes|\det|^{iu}\qquad\textup{or}
\qquad\pi'=\tilde\pi\otimes|\det|^{iu},
\]
hence \eqref{eqn:finalbound3} holds in the stronger form
\[
...
We have shown that \eqref{eqn:finalbound3} holds for all $\chi\in\mathfrak{F}_1^{(1)}$. Consequently, \eqref{eqn:finalbound3} also holds for the elements of any \emph{fixed coset} of $\mathfrak{F}_1^{(1)}$. We shall use this principle in the next subsection.
...
As before, we also assume that \eqref{eqn:siegelhypothesis} holds for some $(\beta,\psi)\in(1-\epsilon',1)\times G$ depending only on $(\pi,\pi',\epsilon)$, and we need to prove \eqref{eqn:finalbound4} with the notation \eqref{eqn:changeofvariable}. But \eqref{eqn:finalbound4} follows readily from \cref{prop:P1}, upon noting that $L(\beta,\pi\times\pi'')=0$ and the $L$-functions under \eqref{eqn:Lfns2} are entire. Indeed, the first two $L$-functions under \eqref{eqn:Lfns2} are entire by assumption, while the third $L$-function is a shift of the first one due to $\chi\in G$.

We have shown that \eqref{eqn:finalbound3} holds for all $\chi\in\mathfrak{F}_1^{(2)}$. Consequently, \eqref{eqn:finalbound3} also holds for the elements of any \emph{fixed coset} of $\mathfrak{F}_1^{(2)}$. We shall use this principle in the next subsection.
...
If $L(s,\pi\times(\pi''\otimes\chi'^2))$ has a pole, then by \cref{lem:twists}, $\chi'$ lies in finitely many cosets of $\mathfrak{F}_1^{(2)}$ depending only on $(\pi,\pi'')$, hence \eqref{eqn:finalbound4} holds by the concluding remark of the previous subsection. Therefore, we shall assume that $L(s,\pi\times(\pi''\otimes\chi'^2))$ is entire. But then \eqref{eqn:finalbound4} follows readily from \cref{prop:P1}, because $L(\beta,\pi\times\pi'')=0$ and the $L$-functions under \eqref{eqn:Lfns2} are entire by assumption.
...
We compare $L(\sigma,\pi\times(\pi'\otimes\chi))$ with $L(1,\pi\times(\pi'\otimes\chi))$ via the bound
\[
...
It follows from \eqref{eqn:Jacquet-Shalika} that
\begin{equation}
\label{eqn:edge2}
...
\[
\sum_{\substack{\ka\in\mathcal{C}}}\Lambda_{\pi\times\pi'}(\ka)\phi(\N\ka)=\frac{1}{|\mathrm{Cl}(\kq)|}\sum_{\psi\in\widehat{\mathrm{Cl}(\kq)}}\sum_{\ka}\Lambda_{\pi\times\pi'}(\ka)\psi(\ka)\phi(\N\ka).
\]
...
\[
\sum_{\substack{\N\ka\leq x \\ \ka\in\mathcal{C}}}\Lambda_{\pi\times\pi'}(\ka)=\frac{1}{|\mathrm{Cl}(\kq)|}\sum_{\chi\in\mathscr{P}(\kq)}\sum_{\ka}\Lambda_{\pi\times(\pi'\otimes\chi)}(\ka)\phi(\N\ka)+O_{\pi,\pi',A}(y).
\]

Recall the definition of $\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C})$ from \eqref{eqn:Edef}. Applying Mellin inversion, we obtain
\begin{multline*}
\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C})=\frac{1}{|\mathrm{Cl}(\kq)|}\sum_{\chi\in\mathscr{P}(\kq)}\frac{1}{2\pi i}\int_{3-i\infty}^{3+i\infty}-\frac{L'}{L}(s,\pi\times(\pi'\otimes\chi))\,\hat\phi(s)\,ds\\
-\frac{1}{|\mathrm{Cl}(\kq)|}\sum_{\chi\in\mathscr{P}(\kq)}\mathcal{M}_{\pi\times(\pi'\otimes\chi)}(x)+O_{\pi,\pi',A}(y).
\end{multline*}
...
If there exists $u\in\R$ such that $\pi'\otimes\chi=\tilde{\pi}\otimes|\det|^{iu}$, then the integrand has a pole at $s=1-iu$ with residue
\[
...
\begin{align*}
\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C})&=\frac{1}{|\mathrm{Cl}(\kq)|}\sum_{\chi\in\mathscr{P}(\kq)}\frac{1}{2\pi i}\int_{\mathscr{C}}-\frac{L'}{L}(s,\pi\times(\pi'\otimes\chi))\,\hat\phi(s)\,ds+O_{\pi,\pi',A}(y)\\
...
\]
Our choices of $y$ and $\epsilon$ along with our range of $\N\kq$ ensures that $\mathcal{E}_{\pi\times\pi'}(x;\kq,\mathcal{C})
