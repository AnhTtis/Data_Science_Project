...
\begin{abstract}
  ...
  sedimentation, however, we find that some other sets of WD evolution
  models may overestimate the cooling delay associated with
  sedimentation, and this may therefore require a re-evaluation of
...
These two sets of models are motivated by the fact that significant
uncertainty remains as to the precise interior composition and
...
Some recent studies have attempted to use seismology to constrain WD
interior compositions \citep{Giammichele2018,Charpinet2019,Giammichele2022},
yielding central O mass fractions as high as 0.86, which is difficult
...
much larger uncertainties than currently appreciated.

Overall, the best constraints for C/O ratios in WD
...
\footnote{Most WD evolutionary codes implement an approximate latent
  ...
  on the averaged calculations of \cite{Salaris2000}. In principle, the
  MESA latent heat implementation based on Skye is significantly better
  than this due to being calculated self-consistently based on the
  local thermodynamics; in practice, Skye often returns a latent heat
  result that is similar to the \cite{Salaris2000} approximation
  \citep{Jermyn2021}.}
...
use similar composition input and assumptions. Our models that descend
from full evolutionary calculations
...
rescaled overall mass with fixed composition profiles. For a typical
$\approx 0.6\,M_\odot$ WD, the phase separation cooling delay is
...
with mass for the 50/50 C/O models. For the models descended from
prior stellar evolution calculations, the cooling delays from our MESA models are
fairly comparable to other codes that use more modern phase diagrams,
such as the LPCODE models of \cite{Althaus2012}.
...
\vspace{1em}
  ...
  \noindent EB gratefully acknowledges helpful conversations with Simon Blouin,
  Mar\'ia Camisassa, Bart Dunlap, Mike Montgomery, and Alejandra
  Romero during the development of this work.
...
because of the overall faster cooling for these models. As the
focus of the current work is simply to describe our implementation of
