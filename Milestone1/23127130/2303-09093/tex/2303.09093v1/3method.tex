...
We then compute the probability of span $[s_i\cdots s_j]$ to be an event trigger as:
\begin{equation*}
\begin{aligned}
    &p([s_i\cdots s_j])  \\
...
\end{equation}
Since this similarity computation does not involve any parameters, we can pre-compute all of the event embeddings $h_e$ during inference.
...
As mentioned in Section \ref{sec:data_cleaning}, our data contains significant label noise due to the many-to-one mapping from Qnodes to PropBank rolesets. 
Since the same event types always co-occur due to the fixed mapping, learning directly from partial labels~\cite{Cour2011LearningPartialLabels} cannot resolve the ambiguity. 
We adopt an incremental self-labeling procedure to handle the partial labels. We start by training a base classifier on clean data labeled with PropBank rolesets that map to only one candidate event type. Despite being trained on only a subset of event types, the base model exhibits good generalization to rolesets with multiple candidate event types. Figure \ref{fig:self_labeling} indicates that with a threshold\footnote{The threshold is the margin between the probability of the top 1 event type and the other types in a candidate set. A higher threshold means a higher level of confidence.} of 0.9, the accuracy of selecting the correct label from a candidate set reaches 57.8\% on the dev set. We then use the base classifier to predict pseudo-labels for the noisy portion of the training data, selecting data with high confidence to train another classifier in conjunction with the clean data. This process can be iterated multiple times to make use of more training data.
