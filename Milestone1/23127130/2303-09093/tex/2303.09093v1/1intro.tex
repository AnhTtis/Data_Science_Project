...
The development and application of event extraction systems has long been stagnated by the limited availability of datasets. 
...
\input{figures/data}
\input{figures/model}
To develop a general-purpose event extraction system, we first seek to build an open-domain event extraction dataset.
...
Thus, instead of aiming for perfect annotation from scratch, we start with the curated DWD Overlay~\footnote{\url{https://github.com/e-spaulding/xpo}} which defines a mapping between Wikidata\footnote{\url{wikidata.org}} and PropBank  rolesets\cite{Palmer2005Propbank}\footnote{A roleset is a set of roles that correspond to the distinct usage of a predicate.}. While Wikidata Qnodes can serve as our general-purpose event ontology, the PropBank roleset information allows us to build a large-scale distantly supervised dataset by linking our event types to existing expert-annotated resources. 
In this process, we reuse the span-level annotations of experts, while dramatically expanding the size of the target ontology. 
...
Our dataset also contains partial labels due to the distant supervision procedure. Specifically, in the DWD Overlay, multiple WikiData nodes are often mapped to the same PropBank roleset, resulting in multiple candidate labels for each data instance as shown in Figure \ref{fig:dataset}.
