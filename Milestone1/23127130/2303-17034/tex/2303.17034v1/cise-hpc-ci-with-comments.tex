...
\begin{abstract}
%Abstract text goes here. This single paragraph ($\le$200 words) summarizes the
  %significant aspects of the manuscript.
...
%
% From:
%   https://www.computer.org/csdl/magazine/cs/write-for-us/14597?title=Author%20Information&periodical=Computing%20in%20Science%20&%20Engineering=
%   https://www.authorea.com/doi/full/10.22541/au.163398435.53883148/v1

% Department articles are shorter, typically up to 3,000 words (including abstract,
% references, author biographies, and tables/figures, which count as 250 words each),
% and are only reviewed by the department editors. To pitch or submit a department
% article, please contact the editor(s) directly using the email address(es) listed on
% the Editorial Board page.

% * Brief intro to HPC and S&E and software
% * Include increasing reliance on open source dependencies
% * Much HPC work has evolved to be done in the open – not entirely internal

\chapterinitial{High performance computing} is a key enabler for developing scientific understanding and knowledge. %modeling and simulation.
...
% Key challenges in build & test for HPC
% * Traditional CI tries to provide a nice clean isolated environment for building software

% * Traditional CI also provides no guarantees about system arch you’ll get (e.g.,
%   GCP/AWS instances have varied uarch)
%   * No free GPU CI
%   * No Cray PE in CI (yet)

% * When working at the system level, we need a way to test on various real HPC/cloud
%   systems, with proprietary software, different schedulers, access, environments,
%   configurations, etc.

% We need to see how the system is set up in practice, and not be cleanly isolated from
% it in our CI

%   * Need interfaces and/or abstractions that let you build something portable across
%     schedulers/configurations (this may conflict with above point)

%   * Need vendors to be more open with SDKs (e.g. NVHPC, OneAPI, Cray PE, etc.)
%     * There is at least a shift towards more openness (e.g., you can just run OneAPI in CI)

%   * Need representative environment where build and test are useful for the production machine
%     * Should sites provide container images to do this?
%     * Should sites provide representative test clusters?
%     * Should vendors provide cloud cycles on representative hardware?
...
%HPC sites tend to have bleeding-edge hardware, like GPUs and recent-generation CPUs.
...
%Beyond hardware, key components of the local software stack on HPC systems are often
%bespoke. For example, vendor-proprietary MPI implementations like Cray MPI can only run
%on Cray systems. In this case, Cray MPI's, license prevents it from being included in
%containers or other software distributions that run in the cloud, and the same is true
%for math libraries and compilers in the Cray environment.
...
%   * HPC administrators: "Any random user can provide code and you want to
%     automatically run it on my system? No!"

% * Sites do not prioritize build or test cycles
%   * “Takes away from production cycles”
%     * Does it really, if production codes break all the time?
...
% * Potential solutions
%   * Solutions exist for internal code developed onsite:
%     * Jacamar developed under ECP, deployed at many ECP sites
%       * https://ecp-ci.gitlab.io/docs/releasenotes/jacamar/jacamar_0.1.0.html

%   * Can extend trust to GitHub *if* you know who the users are

%     * E.g., projects at LLNL allow *specific* github users to comment on PRs to
%       trigger on-site CI (we require membership in LLNL GitHub org and 2FA for GitHub)

%     * These approaches are good for small projects but are too manual and don’t
%       support testing untrusted contributions on large projects (e.g. Spack)

%   * Isolated sandbox environments (either in hardware or software) for production HPC systems that is identical to the production system for software testing

%     * How to scale this?
%     * How to predict balance between build/test and production cycles?

%       * Most centers have no idea how to do this.

%   * Better isolation on HPC systems themselves (e.g., SDN, storage isolation, VMs,
%     etc.) would enable more flexible division of workload

%     * One system, not two
%       * Less admin burden

%       * More likely that envs are not out of sync (extremely likely w/two systems that
%         HPC admins will not upgrade test and prod at same time)

%     * Current HPC security posture precludes this (e.g., shared file system across all
%       nodes is a huge security risk)

%       * HPC networks need improvement (IB doesn’t support SDN well, nothing like
%         SmartNICs in HPC domain yet)

%       * Sites need to ask for good multi-tenant security (but frequently don’t
%         understand the need)

% Other:

%   * When users have problems that are related to the environment, debugging by software
%     creators can be difficult (and very iterative) if they don't have access to the same
%     exact system and its unique configuration

%   * Containers might help?
%     * They can – need to socialize these skills
%   * More cloud-like features on HPC systems will help
%     * You can stand up a representative env with IaaS
%     * Clouds actually offer ready-to-go configurations with SLURM and other schedulers
%     * Leveraging clouds may make sense here.
