...
The results illustrated in \autoref{fig:stability-plots-real-data} thus primarily indicate that low pAUC limits lead to rankings that deviate the most from the mean ranking.
The choice of starting error appears to be less important to maintain ranking stability,
except for the very low 0.01 starting error in combination with low pAUC limits.
Based on these results it appears that higher pAUC limits should be preferred, and that very low starting errors should be avoided.
...
Restricting the EDC pAUC limit/starting error parameters to more stable configurations means that such placement optimisation becomes more restricted,
...
low pAUC limits led to rankings that deviated strongly from the expected rankings,
and for higher pAUC limits the starting error choice had only a minor effect.
Higher starting errors in combination with low pAUC limits however increased the deviation,
which is roughly converse to the real data analysis,
for which the starting error choice had less of an effect even at low pAUC limits,
except for the lowest starting error that increased the instability.
