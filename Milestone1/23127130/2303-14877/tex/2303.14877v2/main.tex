...
\begin{document}
\title{Quantum Approximate Optimization via Learning-based Adaptive Optimization}
...
In addition to the efficiency and accuracy, the DARBO performance is also more stable in terms of  different problem graph instances. We also report the results for fidelity in the Supplemental Material, which essentially gives the probability of obtaining the exact solution state from measuring the QAOA circuit.
...
The DARBO algorithm is inspired by one of the most efficient BO algorithms, trust region Bayesian optimization algorithm (TuRBO) \cite{eriksson2019scalable}, which performs global optimization by conducting BO locally to avoid exploring highly uncertain regions in the search space. TuRBO was developed to mainly resolve the issues of high-dimensionality and heterogeneity of the problem and has been demonstrated to obtain remarkable accuracy on a range of datasets \cite{eriksson2019scalable}. We have applied TuRBO to QAOA problem and identified its performance advantages. DARBO inherits the advantages of TuRBO with an additional abstraction of the adaptive search region; therefore, it further enhances the optimization efficiency of QAOA problems.
...
{\bf Na\"ive BO} \cite{frazier2018tutorial}: We use the default BO implementations in the BoTorch and ODBO packages with a sequential search strategy (batch size = 1) and a UCB acquisition function with $\beta$=0.2. The kernel used is Matern 5/2 kernel, the same as the ones used in TuRBO and DARBO (Eq.\ref{eq:matern52}) as suggested by BoTorch and GPyTorch.
...
\begin{figure*}[bhtp]
    \centering
    \includegraphics[width=1.0\textwidth]{SI_figures/s2_weighted_graphs_diff_N.pdf}
    \caption{The four w3R graphs with different number of graph sizes (N= 10, 12, 14, 18) used in the MAX-CUT problems. The graph with N=16 is the same one as Fig.~\ref{fig:si-fig1} (a). The node ID and the weights are labeled in the corresponding subfigures (a) to (d). }
    \label{fig:si-fig2}
\end{figure*}

\section{MAX-CUT problem instances}

We test the performance of different optimizers on QAOA of the MAX-CUT problem. All the instances are generated using the random\_regular\_graph functions in NetworkX package \cite{hagberg2008exploring}. The codes are also provided in the github repository. The five 16-node weighted graph instances we utilized in the numerical simulations are shown in Fig.~\ref{fig:si-fig1}. The additional four weighted graph instances with different number of nodes are shown in Fig.~\ref{fig:si-fig2}.

\section{Results for more optimizers}

We test more choices of different optimizers on Graph 0, to further demonstrate the advantage of DARBO. See Fig.~\ref{fig:si-fig3} for results with the analytical exact simulation for N = 16 and Fig.~\ref{fig:si-fig5} for results with finite shot noise.
Fig.~\ref{fig:si-fig4} shows the differences of $r$ and fidelity $f$ between DARBO and other optimizers with different sizes of the graphs. The fidelity of optimized QAOA is defined as the square of the overlap between the exact ground state subspace for MAX-CUT problem (a pair of product state hosting MAX-CUT solution configuration) and the output quantum state of the QAOA circuit. In other words, the fidelity is the probability for measuring QAOA output state to obtain the solution bitstrings with the maximal cut value.

\begin{figure*}[bhtp]
    \centering
    \includegraphics[width=1.0\textwidth]{SI_figures/s3_graph0_nonoise_avgr.pdf}
...
\begin{figure*}[bhtp]
    \centering
    \includegraphics[width=0.4\textwidth]{SI_figures/s4_diff_N_nonnoise_p10.pdf}
    \caption{Differences of DARBO, Adam and COBYLA for optimizing the MAX-CUT problem in the analytical exact case on different sizes of the graphs. The average $\Delta r$ and $f$ ratio are computed across 20 different trials from different initialization parameters and plotted as a function of number of nodes ($N$) on the graphs. The error bar is the accumulated standard deviations of two compared optimizers. For each trial, all the optimizers share the same parameter initialization. }
    \label{fig:si-fig4}
\end{figure*}

\begin{figure*}[bhtp]
    \centering
    \includegraphics[width=1.0\textwidth]{SI_figures/s5_graph0_noise_avgr_200.pdf}
    \caption{
...
The optimization results in the finite shot noise case are summarized in Fig.~\ref{fig:si-fig6} for different combinations of shot numbers and the circuit depths.

\begin{figure*}[bhtp]
    \centering
    \includegraphics[width=1.0\textwidth]{SI_figures/s6_allgraph_noise_bestr_p2-8.pdf}
...
The superconducting processor we utilized has a $10\times 2$ grid geometry hosting $20$ transmon qubits in total.
Typical mean error rates are
are $2.4 \times 10^{-2}$ for two-qubit gates, $0.2 \times
10^{-2}$ for single-qubit gates and $7\times 10^{-2}$ for
readout errors. Mean $T_1$ and $T_2$ time  are $23\mu s$ and $5\mu s$, respectively. 
...
\begin{figure*}[bhtp]
    \centering
    \includegraphics[width=0.8\textwidth]{SI_figures/s7_additional_hardware_opt_loss_curve.pdf}
