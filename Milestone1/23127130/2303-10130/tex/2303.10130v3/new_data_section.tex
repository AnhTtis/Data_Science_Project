...
\subsection{Data on LLM Exposure Scores}
...
To evaluate the exposure of DWAs and Tasks to LLM capabilities, we worked with a set of external reviewers who assessed exposure levels to a theoretical LLM based on the following guidelines:
...
\1 Exposure by LLM-powered applications (E2): Label activities/tasks E2 if having access to the LLM alone may not reduce the time it takes to complete the activity/task by at least half, but it is easy to imagine additional software that could be developed on top of the LLM that could reduce the time it takes to complete the specific activity/task with equivalent quality by at least half.
\1 Exposure given image capabilities (E3): Label activities/tasks E3 if there would be a significant reduction in the time it takes to complete the activity/task with equivalent quality given access to a LLM and the following image capabilities:
\2 Reading text from PDFs
\2 Scanning images
\2 Creating or editing digital assets according to instructions
\end{outline}

The threshold for exposure was set at a perceived 50\%\ possible reduction in time taken to complete a given task/activity while holding quality constant. While this threshold is somewhat arbitrary, its use was largely motivated by its ease of interpretation for external reviewers as opposed to assigning a continuous value as an exposure score or thinking about more specific classes of potential impacts than a binary designation for either above or below a 50\%\ reduction. Reviewers included the authors, external reviewers who have significant experience reviewing LLM outputs as part of OpenAI's alignment work \cite{ouyang2022training}, and additional OpenAI researchers.
...
First we present agreement levels between the task-level exposure scores generated by external reviewers and those generated by the LLM. 
