...
\toprule
{} & Med(Med) Inc & Emp (000s) & H $\pmb{\alpha}$ &  M $\pmb{\alpha}$ &  H $\pmb{\beta}$ &  M $\pmb{\beta}$ &  H $\pmb{\zeta}$ &  M $\pmb{\zeta}$ \\
 &      &      &      &      &      &      \\
\midrule
...
\bottomrule
\end{tabular}
    \caption{Mean exposure scores for occupations, grouped by level of on-the-job training required to attain competency in the job. Alongside exposure scores, we display the median of median annual income for each occupation, as well as the total number of workers in each group, in thousands.}
