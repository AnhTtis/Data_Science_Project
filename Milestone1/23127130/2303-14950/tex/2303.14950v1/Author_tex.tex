...
\title{Parameter estimation for many-particle models from aggregate observations: A Wasserstein distance based sequential Monte Carlo sampler}
...
A main difficulty is that, in order to conduct the standard Bayesian inference for a microscopic model, one needs to track of each particle, 
...
that are collected at the ensemble level and do not characterise the state of each individual particle.
In this case the standard posterior computation methods such as Markov Chain Monte Carlo (MCMC) can not be used as the likelihood function is not available. 
...
To address the issue, we propose to use the Wasserstein distance~\cite{rubner1998metric} as a distance measure between the simulated and the observed distributions. The Wasserstein distance~(WD) is a commonly used measure for similarity between two distributions, which loosely speaking, is the minimal cost for transforming one distribution into the other. WD has been widely used in many applications in statistics and machine learning, such as~\cite{frogner2015learning,lee2018minimax},
...
\caption{An illustrative example of the TV distance where the histograms of three probability measures, $U$, $V$ and $W$, are shown. 
The TV distance between $U$ and $V$ and that between $U$ and $W$ are the same. }
\label{fig.hist}
\end{figure*}
...
We start with a generic setup of the sequential inference problem considered here.
Suppose that we have measurement data $y_1, ..., y_T$ (with $y_t \in \mathbb{R}^{n_y}$ for $t=1,...,T$) that are collected sequentially in time, and 
from the data we want to estimate the parameter $\theta \in \mathbb{R}^{n_{\theta}}$. 
In such problem, after collecting $t$ data points $y_1,...,y_t$, the posterior distribution of $\theta$ is in the following form:
...
In principle, the posterior $\pi_T(\theta)$ can be computed with the standard sampling methods such as MCMC. 
...
To this end, the SMCS method is 
 particularly designed to exploit the sequential structure of such problems. 


We here describe SMCS in a recursive formulation. Given an arbitrary conditional distribution $L_{t-1}(\theta_{t-1}|\theta_t)$, 
one can construct the following joint distribution of $\theta_{t-1}$ and $\theta_t$:
\begin{equation}
p_t(\theta_{t-1},\theta_t)=\pi_{t}(\theta_t)L_{t-1}(\theta_{t-1}|\theta_t), 
\end{equation}
whose marginal distribution over $\theta_{t-1}$ is $\pi_t(\theta_t)$. Importance sampling is applied to draw samples for $p_t(\theta_{t-1},\theta_t)$
where the proposal distribution $q_t(\theta_{t-1},\theta_t)$ is constructed in the form of 
\begin{equation}
q_t(\theta_{t-1},\theta_t)=q_{t-1}(\theta_{t-1})K_t(\theta_t|\theta_{t-1}),
\end{equation}
from which we can directly draw samples $(\theta_{t-1}, \theta_t)$. Here $q_{t-1}(\theta_{t-1})$ is a marginal distribution and $K_t(\theta_t|\theta_{t-1})$ a conditional distribution. The weights of samples are computed according to 
\begin{subequations}\label{eq:weight_all}
\begin{equation}
\begin{aligned}
\omega_t(\theta_{t-1}, \theta_{t}) &=\frac{p_t(\theta_{t-1},\theta_t)}{q_t(\theta_{t-1}, \theta_t)}=\frac{\pi_{t}(\theta_t)L_{t-1}(\theta_{t-1}|\theta_t)}{q_{t-1}(\theta_{t-1})K_t(\theta_t|\theta_{t-1})}\\
...
Now that we have weighted ensembles $\{\theta_{t-1:t}^j, \omega_t(\theta_{t-1}^j, \theta_{t}^j)\}_{j=1}^N$ from the joint distribution $q_t(\theta_{t-1},\theta_t)$, and $\{\theta_t^j, \omega_t(\theta_{t-1}^j, \theta_{t}^j)\}_{j=1}^N$ follows the marginal distribution $\pi_t(\theta_t)$. The complete SMCS algorithm proceeds as the following: 
\begin{enumerate}
\item  let $t=0$, draw an ensemble $\{\theta^m_{0}\}_{m=1}^M$ from $q_0(\cdot)$,
and compute $w^m_0=\pi_0(\theta^m_0)/q_0(\theta_0^m)$ for $m=1...M$;
\item let $t=t+1$; \label{st:t+1}
\item draw $\theta^m_{t}$ from $K_t(\cdot|\theta^m_{t-1})$ for each $m=1...M$;
\item compute $w^{m}_{t}$ using Eq.~\eqref{eq:weight_all} for each $m=1...M$;
\item  return to step~\ref{st:t+1} if $t<T$.
\end{enumerate}
...
Here we adopt the MCMC moves developed in~\cite{del2006sequential}. 
The forward kernel is constructed as follows. 
First we choose a proposal distribution $k(\theta_t|\theta_{t-1})$ and draw a sample $\theta^*_t$ from it with a sample $\theta_{t-1}^j$ from the previous iteration. The sample $\theta^*_t$ is accepted or rejected according to the following acceptance probability:
...
with a kernel function $K(\cdot,\cdot)$ that characterizes the similarity between $\hat{y}$ and the actual data $y$:
 \begin{equation}\label{eq.kernel}
\pi(y|\theta) \approx K(\hat{y}, y),
\end{equation}
where $\hat{y}$ is the synthetic observation data generated from $\pi(\hat{y}|\theta)$.
We reinstate here that the kernel function $K$ is chosen 
...
at each time $t$, from which the parameter $\theta$ is inferred. 
...
in this framework, the key issue is to choose the kernel function $K(\cdot,\cdot)$ that measures the similarity between two data points that are essentially two distributions in the present problem. 
To this end, we choose to define the kernel function $K$ via the Wasserstein distance of two distributions. 
...
$\rho_U=\{(u_1, \omega_1^u), ..., (u_m, \omega_m^u)\}, \rho_V=\{(v_1, \omega_1^v), ..., (v_n, \omega_n^v)\}$ be two discrete-valued distributions, and $c(u_i,v_j)$ be the cost for transporting a unit from $u_i\in U$ to $v_j \in V$, 
which is taken to be  the Euclidean distance between $u_i$ and $v_j$ in this work. 
...
\begin{equation}
\left\{\begin{array}{l}
\sum\limits_{j=1}^n \gamma(u_i, v_j)=\omega_i^u, \\
\sum\limits_{i=1}^m \gamma(u_i, v_j)=\omega_j^v, \\
\gamma \geq 0.
\end{array}\right.
\end{equation}
...
K(\rho_U,\rho_V)= \frac{1}{\sqrt{2\pi}h} \exp{(-D_W(\rho_U,\rho_V)^2/2h^2)},
\end{equation}
...
\label{alg:2}
{\bf Input:} 
Number of ensembles $N$, Prior of parameters $\pi_0(\theta)$, observed distributions $y_{1:T}$
\smallskip
\begin{algorithmic}[1]
\State draw $N$ samples $\{\theta_0^j\}_{j=1}^N$ from prior $\pi_0(\theta)$ and initialize weights $\omega_0^j = \frac{1}{N}$;
\For{$t = 1, ..., T$}
\State draw a candidate sample $\theta_t^{j*}\sim k(\cdot \mid \theta_{t-1}^j)$;
\State generate simulated distributions $\hat{y}_{1:t}^j$ and $\hat{y}_{1:t}^{j*}$ with model parameter $\theta_{t-1}^j$ and $\theta_t^{j*}$;
\State compute pair-wise Wasserstein distance $\operatorname{WD}(\hat{y}_{1:t}^{j}, y_{1:t})$ and $\operatorname{WD}(\hat{y}_{1:t}^{j*}, y_{1:t})$;
\State compute approximated likelihood $\pi(y_{1:t} \mid \theta_t^{j*})$ and $\pi(y_{1:t} \mid \theta_{t-1}^j)$ using Eq.~\eqref{eq.kernel};
...
\State update the weights $$\omega_t^j=\omega_{t-1}^j \pi(y_t \mid \theta_{t}^j);$$
\State resample if needed.
\EndFor
\end{algorithmic}
\smallskip
\end{algorithm}
...
The second is the intelligent driver model (IDM) for 
the collective behavior of traffic flow. 
...
The position vector $\boldsymbol{r}_i(t)$ is updated by the velocity $\boldsymbol{v}_i(t)
 ...
 \begin{split}
\boldsymbol{f}_{ij} =& \{A_i{\rm exp}[(r_{ij} - d_{ij})/B_i] + kg(r_{ij} - d_{ij})\}\boldsymbol{n}_{ij}\\ 
&+ \kappa g(r_{ij} - d_{ij})\Delta v_{ji}^t\boldsymbol{t}_{ij},
\end{split}
\end{equation}
where the indicator function $g(r_{ij} - d_{ij})$ is zero for
...
With the SFM described above, we simulate the collective escape of pedestrians towards a single exit in a room. 
The parameter values used to simulate the data are given in Table~\ref{tab.1}, largely following~\cite{Helbing2000VICSEK}.
Among these parameters we assume that $v^p$, $A$ and $B$ are unknown and need to be inferred from the observation data. 
Namely, suppose we can observe the positions of pedestrians $y_t$ at different times $t^{obs} = t \times\Delta_t$ for $t = 1, \ldots, T$, and we aim to estimate the aforementioned parameters in the SFM with these sequential observations. The observation noise is assumed to be a zero-mean Guassian with standard deviation $\sigma$.
We emphasize that we are able to observe the particle locations, but we are unable to track each particle. 
In this experiment, we take $\Delta_t = 0.1\rm{s}$ and $T = 30$ and the simulation time step in SFM is $dt = 0.001\rm{s}$. The prior distribution for $\theta = (A, B, v^p)^{\intercal}$ is taken to be uniform: $A\sim U[1200, 2200]$, $B\sim U[0.02, 0.2]$ and $v^p\sim U[0, 1.5]$. We simulate the observation data with two different noise levels: $\sigma = 0.04$ and $\sigma = 0.1$, and use 500 particles in WD-SMCS to estimate parameters at each time step. 

\begin{figure*}[!htb]
\centering
\includegraphics[width=\textwidth]{sfm.png}
\caption{($\sigma = 0.04$) Inference results of the SFM. Blue lines represent the average values of parameters estimated by 500 weighted particles and the standard deviations are shown as error bars. The horizontal lines represent the true parameter values.}
\label{fig.sfm_small}
\end{figure*}
...
\caption{($\sigma = 0.04$) Comparison among the observation (first row), the prior prediction (second row) and the posterior prediction (third row) in terms of crowd density map at specific time points. The prior prediction is based on the mean values of parameter priors, while the posterior prediction is generated with the posterior means. WD1 is the WD between the observed density and the prior-simulated one, and WD2 is that between the observed density and 
the posterior-generated one.}
...
and the converged values are rather close to the ground truth,
indicating good inference accuracy in the case when the observation noise is small. 
Recall that, a key element in the proposed method is that the simulated distribution density should be close 
to the observed one. 
...
$t=1,\,11,\,25$;
 we also show  the density from the  prior means (referred to as the prior-simulated density) as a reference. 
...
\caption{($\sigma = 0.1$) Inference results of the SFM. Blue lines represent the average values of parameters estimated by 500 weighted particles and the standard deviations are shown as error bars. The horizontal lines represent the true parameter values.}
\label{fig.sfm_large}
\end{figure*}
...
\caption{($\sigma = 0.1$) Comparison among the observation (first row), the prior prediction (second row) and the posterior prediction (third row) in terms of crowd density map at specific time points. The prior prediction is based on the mean values of parameter priors, while the posterior prediction is generated with the posterior means. WD1 is the WD between the observed density and the prior-simulated one, and WD2 is that between the observed density and 
the posterior-generated one.}
...
IDM is a widely adopted car-following model in microscopic traffic simulation. In this type of models, as its name suggests, the $\alpha$-th vehicle follows the $(\alpha-1)$-th vehicle in front of it. The IDM assumes the acceleration of a follower $\alpha$ is a continuous function of a set of inputs - the follower's current velocity $v_\alpha$; the gap distance $s_\alpha$ to the leader; and the relative velocity (approach rate) $\Delta v_\alpha$ with respect to the preceding vehicle. The dynamics equation is defined as:
\begin{equation}
\dot{v}_\alpha=a^{(\alpha)}\left[1-\left(\frac{v_\alpha}{v_0^{(\alpha)}}\right)^\delta-\left(\frac{s^*\left(v_\alpha, \Delta v_\alpha\right)}{s_\alpha}\right)^2\right] .
\end{equation}
in which
\begin{equation}
...
\end{equation}
\begin{equation}
s^*(v, \Delta v)=s_0^{(\alpha)}+T_s^{(\alpha)} v+\frac{v \Delta v}{2 \sqrt{a^{(\alpha)} b^{(\alpha)}}}.
\end{equation}
This expression can be understood as an interpolation of a free road acceleration $a_f = a^{(\alpha)}\left[1-(v_{\alpha}/v_0^{(\alpha)})^{\delta}\right]$ and an interaction deceleration $a_{int} = - a^{(\alpha)}(s^*/s_{\alpha})^2$.
Other constant parameters include the maximum desired velocity $v_0^{(\alpha)}$, jam distance $s_0^{(\alpha)}$, maximum desired acceleration $a^{(\alpha)}$ and deceleration $b^{(\alpha)}$, safe time headway $T_s^{(\alpha)}$ and configurable exponent parameter $\delta$, which controls the smoothness of the acceleration/deceleration. In this example, we study the case of identical vehicles whose model parameters $v_0^{(\alpha)} = v_0$, $s_0^{(\alpha)} = s_0$, $T_s^{(\alpha)} = T_s$, $a^{(\alpha)} = a$, $b^{(\alpha)} = b$ are given in Table~\ref{tab.2}.
...
Suppose that we are able to observe the vehicle locations $y_t$ at a sequence of discrete time points: $t^{obs} = t\times\Delta_t$ for $t = 1, \ldots, T$, and we want to estimate the parameters $\theta = (v_0, a, T_s)^{\intercal}$ in the IDM with these observed data with all other parameters known. The measurement noise is assumed to be a zero-mean Guassian with standard deviation $\sigma$. In this experiment, we take $\Delta_t = 1.0\rm{s}$ and $T = 30$. The prior distribution  is taken to be uniform: $v_0\sim U[5.56, 22.22]$, $a\sim U[0.5, 5]$ and $T_s\sim U[0.5, 4]$. We simulate the observation data with two different noise levels: $\sigma = 0.1$ and $\sigma = 1.0$, and use 500 particles in WD-SMCS to estimate parameters at each time step. 
...
\caption{( $\sigma=0.1$ ) The observed vehicle distribution (blue) compared against
the posterior-simulated (green) and the prior-simulated (red) distributions. 
WD1 is the WD between the observed distribution and the prior-simulated one,
and WD2 is that between the observed distribution and 
the posterior-simulated one.}
...
\caption{( $\sigma=1.0$ ) The observed vehicle distribution (blue) compared against
the posterior-simulated (green) and the prior-simulated (red) distributions. 
WD1 is the WD between the observed distribution and the prior-simulated one,
and WD2 is that between the observed distribution and 
the posterior-simulated one.}
...
Krauss S, Wagner P, Gawron C. 1997  Metastable states in a microscopic model of
  traffic flow. {\em Phys. Rev. E} \textbf{55}, 5597--5602.
...
Gödel M, Bode N, Köster G, Bungartz HJ. 2022  Bayesian inference methods to
  calibrate crowd dynamics models for safety applications. {\em Safety Science}
  \textbf{147}, 105586.
...
Frogner C, Zhang C, Mobahi H, Araya-Polo M, Poggio T. 2015  Learning with a
  ...
  Neural Information Processing Systems - Volume 2} NIPS'15 p. 2053–2061
...
Lee J, Raginsky M. 2018  Minimax Statistical Learning with Wasserstein
  ...
  Information Processing Systems} NIPS'18 p. 2692–2701 Red Hook, NY, USA.
