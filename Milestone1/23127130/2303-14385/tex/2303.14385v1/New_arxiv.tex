...
\begin{abstract}
{Josephson tunnel junctions exhibit a supercurrent typically proportional to the sine of the superconducting phase difference $\phi$. In general, a term proportional to $\cos(\phi)$ is also present, alongside microscopic electronic retardation effects. We show that voltage pulses sharply varying in time prompt a significant impact of the $\cos(\phi)$ term. Its interplay with the $\sin(\phi)$ term results in a non-equilibrium fractional Josephson effect $\sim\sin(\phi/2)$ in the presence of bound states close to zero frequency. Our microscopic analysis reveals that the interference of non-equilibrium virtual quasiparticle excitations is responsible for this phenomenon. We also analyse this phenomenon for topological Josephson junctions with Majorana bound states. Remarkably, the non-equilibrium fractional Josephson effect is independent of the ground state fermion parity unlike its equilibrium counterpart.}
\end{abstract}
\maketitle

The Josephson effect~\cite{Josephson1962,Josephson1964,Josephson1965,Anderson1963,Rowell1963,Yanson1965}, characterised by coherent tunneling of Cooper pairs across a superconducting junction, is the quintessential experimental manifestation of superconducting phase coherence. In its simplest version, it leads to an equilibrium supercurrent with the current-phase relation (C$\phi$R) $I_S(\phi)\sim\sin(\phi)$, where $\phi(t)$ is the superconducting phase difference satisfying $d\phi(t)/dt=2eV(t)/\hbar$, which equals the Josephson frequency $\omega_J=2eV/\hbar$ for constant bias. Typically, time-reversal symmetry implies $I_S(\phi)=-I_S(-\phi)$ which, combined with the requirement of periodicity $I_S(\phi+2\pi)=I_S(\phi)$, prompts a sinusoid as the basic model for Josephson \emph{tunnel} junctions, where higher harmonics may be neglected~\cite{Golubov2004}. However, the C$\phi$R is altered by the retardation imparted by the electronic excitations mediating the current in a microscopic description~\cite{Josephson1962,Ambegaokar1963,Werthamer1966,Larkin1967,Choi2022,Barone1982}. Specifically, the current is given by $I[\phi(t)]=I_N[\phi(t)]+I_S[\phi(t)]$, with $I_N[\phi(t)]=\Re J_{n}[\phi(t)]$ and $I_S[\phi(t)]=\Re J_p[\phi(t)]\sin(\phi(t))-\Im J_p[\phi(t)]\cos(\phi(t))$, where $J_{n,p}[\phi(t)]$ are complex-valued functionals of the phase characterising the electronic retardation~\cite{Josephson1962,Ambegaokar1963,Werthamer1966,Larkin1967,Choi2022,Harris1974}. $I_N$ describes the resistive quasi-particle tunneling current~\cite{Giaever1960a,Giaever1960b,Cohen1962,Bardeen1962}. The first term in $I_S$ represents the standard Josephson effect (SJE), proportional to sine of $\phi$, henceforth dubbed as the $\sin(\phi)$ term. It arises as the tunnel Hamiltonian connects the ground states of the two superconductors via virtual excitations. The second term, referred to as the $\cos(\phi)$ term, is associated with the resistive component of the pair current as it includes a response in-phase with an alternating bias~\cite{Harris1974,Harris1975,Zorin1979,Pop2014}. 

In this work, we extend these concepts beyond the steady state, studying the dynamics of Josephson junctions subjected to sharply varying voltages. This relatively unexplored regime~\cite{Perfetto2009,Stefanucci2010,Suoto2016} is relevant for their applications in high-speed electronics and superconducting qubits~\cite{Devoret2013,Makhlin2001,Clarke2008,Zazunov2003}. Moreover, it reveals the intrinsic energy scales of the junction materials, such as the superconducting gap for $s-$wave superconductors, and the quasiparticle lifetime. We discover that in Josephson tunnel junctions with sub-gap bound states (BSs) or Majorana zero modes (MZMs)~\cite{Kitaev2001,Kitaev2003,Nayak2008,Alicea2012,Lutchyn2010,Oreg2010,Lutchyn2018} subjected to sharply varying bias voltages, both the $\sin(\phi)$ and $\cos(\phi)$ terms are modified, and their interplay effectively yields a rich C$\phi$R. Specifically, for near-zero-frequency BSs, we obtain a non-equilibrium fractional Josephson effect (NFJE) with the C$\phi$R $I_S(\phi)\sim\sin(\phi/2)$, oscillating at the \emph{fractional} Josephson frequency $\omega_J/2$. It initially dominates the SJE but eventually decays over the time-scale $\sim\hbar/\Gamma$, associated with the quasiparticle lifetime. For a topological Josephson junction hosting MZMs, we develop the perturbative formalism for handling the topological degeneracy. The resulting current, which may be noticeably distinct from that of low-energy BSs, is independent of the ground-state fermion parity, establishing resilience against quasiparticle poisoning~\cite{Goldstein2011,Rainis2012,Budich2012,Karzig2021}. We propose using a square wave bias to sustain the NFJE.

\textit{Phenomenology of the NFJE.--} We elucidate the key concepts underpinning the NFJE by considering the response of a Josephson junction to a voltage step rising to a height $V_0$ over time $\tau$. A microscopic calculation shows that the dominant contribution to the steady state pair current is given by Cooper pairs breaking into two Bogoliubov intermediate quasiparticles (IQPs), which tunnel across the junction and subsequently pair up (see Fig. \ref{Fig1} for a schematic, and the Supplemental Material (SM)~\cite{sm} for details). The non-equilibrium pair current carries an imprint of this picture, with the dynamical response of IQPs dictating its temporal behaviour. Indeed, a sharply varying voltage excites a non-equilibrium distribution of IQPs, characterised by time and bias dependent phases, whose mutual interference~\cite{Gaury2014,Gaury2015,Wingreen1993,Wingreen1994} manifests as a non-equilibrium modulation of the amplitudes of the $\sin(\phi)$ and $\cos(\phi)$ terms at a fractional Josephson frequency $\omega_J'\approx\omega_J/2$.

We now describe the underlying physics of this phenomenon. In a heuristic description, at leading order in the tunneling amplitude, pair tunneling generates states with a single pair transferred across the junction in both directions. The corresponding pair current is given by the rate of change of their coefficients~\cite{sm}, with the resulting current oscillations determined by $J_p(t)= \int_{-\infty}^t dt' \exp[i(\phi(t)-\phi(t'))/2]\Im \iint_{-\infty}^\infty d\epsilon_L d\epsilon_R \nu(\epsilon_L)\nu(\epsilon_R)\exp[-i(\epsilon_R-\epsilon_L)(t-t')]  ( f(\epsilon_R) - f(\epsilon_L))$ where $\nu$ is the anomalous density of states~\cite{Bzdusek2013}, $\phi(t)/2=\int_{-\infty}^t dt' (e/\hbar)V(t')$, and $\Re J_P$ and $\Im J_P$ are the amplitudes of the $\sin(\phi)$ and $\cos(\phi)$ terms, respectively. This is the time-domain analogue of the Fermi's golden rule. Its oscillatory response is determined by three predominant factors operating concurrently: (i) non-equilibrium excitations generated by the voltage step, (ii) interplay of $\cos(\phi)$ and $\sin(\phi)$ terms, and (iii) electronic retardation. We now provide a concise account of these factors. First, the IQPs at the junction ($x=0$) are characterised by the dynamic phase factor $e^{-i\chi(\epsilon,t)}$ with $\chi(\epsilon,t)=\epsilon t$. The voltage step alters it to $\chi'(t)=\chi(t)+\phi(t)/2$. Note that $\phi(t\gg\tau)/2\sim (\omega_J/2)t$ as $V(t\gg\tau)\to V_0$, while $\phi(t<0)=0$. Consequently, the IQPs tunneling at different times $t'$ acquire different phases $\phi$, commensurate with the time spent in the biased lead since the rise of the voltage step~\cite{sm}. The resulting mutual interference is captured by the factor $\exp[i(\phi(t)-\phi(t'))/2]$ in $J_p$, generating oscillations with frequency $\omega_J'\approx\omega_J/2$. Second, the $\cos(\phi)$ term typically requires a driving energy greater than the pair-breaking energy $E_{pb}$, which entails $V_0>E_{pb}$ for a constant bias $V(t)=V_0$~\cite{Barone1982}, or $\omega_d>E_{pb}$ for an alternating bias $V(t)=V_0\cos(\omega_d t)$~\cite{Harris1975}. However, in our dynamic scenario, the sharply varying voltage can supply $E_{pb}$. In particular, in the presence of low-energy BSs, apart from exhibiting the modulation described above, \emph{both} the $\sin(\phi)$ and $\cos(\phi)$ terms are of similar magnitudes. As such, they work concomitantly to suppress the underlying SJE and reinforce the NFJE. Third, the time-evolution of IQPs governed by their dynamic phases $\sim \exp(i(\epsilon_L+\epsilon_R)(t-t'))$ defines the microscopic retardation which, together with the first factor, determines $\omega_{J}'$. Consequently, the Josephson effect is enriched by exploring the electronic spectrum of the constituent superconductors. Additionally, the time-evolution is characterised by the inverse quasiparticle lifetime $\Gamma$, which dictates the longevity of the NFJE $\sim \hbar/\Gamma$. It naturally arises, e.g., from the inevitable relaxation to the high-energy quasiparticle continuum. It also accounts for the coupling to the leads~\cite{Budich2012} which, however, is relatively small in the tunnel limit.

Considering a BS at frequency $\omega_0$ in both leads for simplicity, which serve as the IQP states, the bias-dependent and dynamic phases operate collectively to generate oscillations at frequency $\omega_{J,1/2}'=\omega_J/2\pm 2\omega_0$ in $J_p(t)$ (see Fig. \ref{FigS5} in~\cite{sm}), as well as the current. Notably, for $\omega_0\to 0$ when the dynamic phase vanishes, we obtain the fractional Josephson frequency $\omega_J'=\omega_J/2$. For IQPs belonging to a band characterised by the band-width $\Lambda\gg \omega_0$, which is the largest energy scale, their mutual interference eventually washes out the coherent $\omega_J/2-$oscillations for $t\gtrsim \hbar/\Lambda$. Consequently, the oscillations produced by the band states decay rapidly, rendering them imperceptible. Therefore, NFJE requires BSs with sharp spectral support in \emph{both} leads, although not necessarily at the same frequency. The interference is dominated by IQPs tunneling presently at $t$ and those having tunneled in the recent past $t-\hbar/\Gamma\lesssim\tau<t$, in particular the ones which originated prior to the voltage step. Not only does this necessitate a voltage step varying faster than the quasiparticle lifetime, i.e., $\tau<\hbar/\Gamma$ (for smooth steps see~\cite{sm}), it also causes the NFJE to vanish for $t\gtrsim t^*\coloneqq\hbar/\Gamma$ when the waves whose origin precede the bias step have decayed. As such, $\Gamma$ plays a vital role in determining the relevant time scales for the NFJE. It may be estimated from microwave spectroscopy studies of Andreev bound states~\cite{elfeky2023}, which revealed $t^*\sim 1\mu$s. 
\begin{figure}
...
\begin{overpic}[height=6.0cm,width=.72\linewidth]{Fig_1a.pdf}
 \put (74,68) {\includegraphics[scale=0.1]{Fig_1b.pdf}}
 \put (4,70) {\large	$eV_0$}
 \put (44,66) {\large	$e^{i\omega_J't}$}
 \put (69.5,54) {\large  	$\omega_0$}
 \put (22,57) {\large		$\omega_0$}
\end{overpic} 
\caption{Schematic of the dominant microscopic process for the Josephson pair current. A Cooper pair breaks up into two excitations (green and red circles), here involving the BSs at frequency $\omega_0$, combining after tunneling. The un-biased density of states (DOS) $(t\ll 0)$ is shown in red, while its biased counterpart $(t\gg 0)$ is shown in green. The interference (shaded blue) of the IQPs tunneling before and after the voltage step causes the $\omega_J'=\omega_J/2\pm 2\omega_0$ oscillations.}
\label{Fig1}
\end{figure}

\textit{Model.--} For a microscopic analysis, we follow the seminal work by Werthamer~\cite{Werthamer1966}, constituting a perturbative non-equilibrium formalism capable of handling an arbitrary time-dependent bias. Considering a single-channel Josephson junction, with two s-wave superconducting leads of equal superconducting gaps coupled by the tunneling amplitude $\mathcal{T}$, we obtain the current
\begin{equation}
\begin{split}I(t)=  \frac{e\mathcal{T}^2}{\hbar}\int_{-\infty}^t dt' \bigg[  &-\sin\bigg(\frac{\phi(t)-\phi(t')}{2}\bigg)K_N(t-t')\\
 &+\sin\bigg(\frac{\phi(t)+\phi(t')}{2}\bigg)K_S(t-t')\bigg].
\end{split}
\label{iwerttime2}
\end{equation}
The first and second lines represent the quasiparticle and pair currents, respectively. $K_{N/S}$ are the response kernels describing the retardation, given by $K_{N/S}(t)=\Im \int d\epsilon_L d\epsilon_R e^{-i(\epsilon_L-\epsilon_R)t}A^{N/S}_L(\epsilon_L)A^{N/S}_R(\epsilon_R)[f(\epsilon_R)-f(\epsilon_L)]$, where $f$ is the Fermi function, and $A^N_j=-[\textbf{Im}G^r_j]_{1,1}$ and $A^S_j=-[\textbf{Im}G^r_j]_{1,2}$ are normal and anomalous components of the surface spectral function of the lead $j=L(\text{left})/R(\text{right})$. They contain the dynamic phases representing the time-evolution of IQPs, weighted by the corresponding spectral functions. Eq.\eqref{iwerttime2}, in turn, represents the interference between the tunneled state at $t'$ and its time-evolved version at the present time $t$, considering both the retardation from $K_{N/S}$ and the bias-dependent phases, thereby accounting for the mutual interference of IQPs~\cite{sm}. In the presence of sub-gap BS, noting that $A^S_j(\omega)$ is odd in $\omega$ for s-wave superconductors~\cite{Bzdusek2013}, $A^{N/S}_j(\omega)=A^{N/S}_{j,\text{band}}(\omega)+h_{j,n/s}(4\Delta/\zeta)[\delta(\omega-\omega_0)\pm\delta(\omega+\omega_0)]$, where $\omega_0$ and $h_{j,n/s}$ are frequency and spectral weight of the BS, respectively, $\zeta\gg\Delta$ is the band-width of the system in the normal state, and $A^{N/S}_{j,\text{band}}$ correspond to band states. The BS peak is regularised by the inverse quasiparticle lifetime $0<\Gamma\ll \Delta$. We relegate the expressions for $K_{N/S}$ to the SM~\cite{sm} for brevity. For the bands, $K_{N/S}$ oscillates at frequency, $\omega=2\Delta/\hbar$~\cite{Harris1975,Choi2022}, corresponding to the singular band-edge spectral density. Additionally, being associated with the energy scale $\Delta$, it follows from the uncertainty principle that it is significant only for a typically short time $\sim \hbar/\Delta$. However, when both leads have BSs with $\Gamma\ll\Delta$, $K_{N/S}$ is significant for a longer time $\sim\hbar/\Gamma\gg \hbar/\Delta$. Moreover, corresponding to the intrinsic dynamical evolution of the BS, it contains oscillations associated with the BS frequency $\omega_0$.

In the case of topological leads with MZMs, we need to carefully treat the four-fold ground state degeneracy associated with the (even/odd) parity states of each lead. Specifically, the initial ground state of the junction must be properly chosen for a convergent perturbative result~\cite{Brouder2009,sm}. Consequently, at leading order in $\mathcal{T}$, we obtain an additional term in the current $I^{(0)}=(e\mathcal{T}/\hbar)\mathpzc{p}_I\sin(\phi(t)/2)\psi_{L,1}\psi_{R,N}/2$. This is the standard parity-dependent fractional Josephson current~\cite{Kitaev2001,Lutchyn2010}, where $\psi_{L,1}(\psi_{R,N})$ represents the left(right)-localised MZM wavefunctions evaluated at the left(right) end of the wire, and $\mathpzc{p}_I$ is the fermion-parity defined by the two MZMs located near the junction. At order $\mathcal{T}^2$, we obtain Eq. \eqref{iwerttime2} as before. Remarkably, unlike $I^{(0)}$, Eq. \eqref{iwerttime2} is independent of the choice of the ground state, and hence, the parity. The reason is that it involves the mutual interference of IQPs having tunneled at different times as described earlier. While the tunneled IQPs indeed depend on parity, they do so in the same way, rendering the relative amplitude defining the pair current parity-independent. As such, even though parity-flipping processes may alter the state, the current remains untouched. Furthermore, for a statistically mixed ensemble of ground states with different parities, which may be established by incoherent transitions to the environment, the NFJE current survives while $I^{(0)}$ averages out to zero. We discuss these points further in the SM~\cite{sm}.

\begin{figure}
\begin{overpic}[clip,width=0.99\columnwidth]{Fig_2.pdf}
 \put(72.9,13){\tikz \draw[line width=0.5mm,dashed,gray](60,4)--(60,6.85);}
 \put(73,20){\tikz \draw [-{Stealth[scale=2,gray]}] (0,0) -- (1,0);} 
 \put (88.5,20) {\large SJE}
 \put(62,20){\tikz \draw [-{Stealth[scale=2,gray]}] (0.0,0) -- (-1,0);} 
 \put (45,20) {\large NFJE}
\end{overpic}
%\includegraphics[clip,width=0.99\columnwidth]{Fig_2.pdf}
\caption{Time-resolved normalised current response to the step bias $V(t)=V_0\Theta(t)$, with $eV_0=(\pi/2)\Delta$, $\zeta=150\Delta$, $\Gamma=0.1\Delta$, and $I_0=(e\mathcal{T}^2/\hbar)(2\Delta^2/\hbar^2\zeta^2)(\hbar/\Delta)$. We consider BSs at $\omega_0=0.1\Delta$ with $h_n=h_s=1$ in both leads~\cite{bandneg}. Strong $\omega_J/2-$oscillations are obtained initially with period $T_{\omega_J/2}=2\pi/(\omega_J/2)$. Eventually, they decay to SJE oscillations with period $T_{\omega_J}=2\pi/(\omega_J)$.}
\label{Fig2}
\end{figure}

\textit{Heaviside step pulse.--} In this section, we elaborate on the response to a Heaviside step voltage pulse, $V(t)=V_0\Theta(t)$, focusing on the cases with BS and MZMs. For $t>0$, the pair current is obtained from Eq. \eqref{iwerttime2} as
\begin{equation}
...
\end{equation}
$\Re J_p$ and $\Im J_p$ define the amplitude of the $\cos(\phi)$ and $\sin(\phi)$ terms, respectively. The oscillations in $J_p$ arise from a combination of the bias-dependent excitation of the IQP states and the electronic retardation. An analytical approximation~\cite{sm} valid in the limit $eV_0\gg\omega_0\gg\Gamma$, shows that $I_S(t)$ is initially dominated by NFJE oscillating at $\omega_{J,1/2}'=\omega_J/2\pm 2\omega_0$ with an amplitude $\sim (e\mathcal{T}^2/\hbar)(2\Delta^2/\hbar^2\zeta^2)(\hbar/\Gamma)=I_0(\Delta/\Gamma)$, which subsides over a time $\sim\hbar/\Gamma$, transitioning into the SJE. Here, $I_0=(e\mathcal{T}^2/\hbar)(2\Delta^2/\hbar^2\zeta^2)(\hbar/\Delta)$ is the critical current of the SJE arising from the band states~\cite{sm}. These predictions follow from the temporal behaviour of $K_{S}$, as described earlier. Importantly, the NFJE is parametrically larger than the SJE by the factor $\Delta/\Gamma$. For a given voltage $V_0$, since $K_{N/S}$ contains oscillations associated with $\omega_0$, the NFJE response improves with decreasing $\omega_0$~\cite{sm}. Hence, a high voltage $eV_0>>\hbar\omega_0$ alongside $eV_0>>\Gamma$ yields a pronounced NFJE at frequency $\omega_J/2$. These features are shown in Fig. \ref{Fig2} for a single BS in each lead~\cite{bandneg}. Note that BSs with $\omega_0\lesssim\Gamma$, in particlular MZMs, naturally satisfy this criterion. We remark that the $\sin(\phi)-$term, on its own, is insufficient to generate the NFJE as it results in beatings containing both $\omega_J$ and $\omega_J/2-$oscillations. Hence, a coherent interference between the $\cos(\phi)$ and $\sin(\phi)$ channels is required for the emergence of the NFJE composed solely of $\omega_J/2-$oscillations.

\begin{figure}
\includegraphics[clip,width=0.99\columnwidth]{Fig_3.pdf}
\caption{Time-resolved normalised current response to the step bias $V(t)=V_0\Theta(t)$, with $eV_0=0.4\Delta$, $\zeta=150\Delta$, $\Gamma=0.1\Delta$. In (a), we consider topologically superconducting (TS) leads based on Kitaev chains, and in (b) we consider two BSs in each lead at $\omega_0=0.1\Delta=\Gamma$ with $h_n=h_s=1$ in both leads~\cite{bandneg} ($eV_0=4\hbar\omega_0$). Unlike the TS junction in (a), there is no noticeable NFJE for the BS in (b).}
\label{Fig3}
\end{figure}
...
\end{equation} 
The sub-leading term, which is parametrically smaller than the NFJE current by the factor $\Gamma/\Delta$, is the SJE current arising from the band contributions. 

The non-equilibrium interference of IQPs in the NFJE permits a distinction between MZMs and non-topological BSs, even for $\omega_0\ll\Delta$ and a large spectral broadening $\Gamma\sim\hbar\omega_0$. This is because the intrinsic energy scales are embodied in the NFJE frequencies as $\omega'_{J,\text{MZM}}=\omega_J/2$ for MZMs and $\omega'_{J,\text{BS}}=\omega_J/2-2\omega_0$ for the BSs (see Fig. \ref{FigS5} in~\cite{sm}), which can be significantly different at low voltages comparable to $\omega_0$. Specifically, for $eV_0=\alpha\hbar\omega_0$, we provide two conditions maximizing the distinction: (a) $\omega_{J,\text{MZM}}'=\omega_J/2$ and $\omega_{J,\text{BS}}'=(\alpha-2)\omega_0=[(\alpha-2)/\alpha]\omega_J/2$ are noticeably different to ensure distinguishability; (b) Since the NFJE lifetime is $\sim\hbar/\Gamma$, impose $2\pi/\omega_{J,\text{BS}}=2\pi/((\alpha-2)\omega_0)\gtrsim \hbar/\Gamma$ to render the NFJE from BS inconspicuous as it decays rapidly within a period, whereas $2\pi/\omega_{J,\text{MZM}}=2\pi/(\alpha\omega_0)\lesssim \hbar/\Gamma$, leading to noticeable NFJE oscillations from MZMs. Practically, for $\omega_0\approx\Gamma$, we find $\alpha\approx 3-5$, as shown in Fig. \ref{Fig3}.

\begin{figure}
%\includegraphics[clip,width=0.99\columnwidth]{Fig_3a.pdf}\\
\includegraphics[height=4.7cm,width=0.99\columnwidth]{Fig_4.pdf}
\caption{Normalised current $(I/I_0)$ response to a square wave bias with $eV_0=0.8\Delta$ with varying $\omega_d$, and resolved in Fourier domain. In (a) we assume BCS superconducting leads, and in (b) BS at $\omega_0=0.1\Delta$ in both leads~\cite{bandneg} with $\Gamma=0.1\Delta$. We have considered a duty-cycle $D=0.75$. For $\omega_d\gtrsim \Gamma/\hbar$, the NFJE oscillations dominate each pulse cycle in the presence of BSs, whereas for $\omega_d\lesssim \Gamma/\hbar$, they decay to reveal the static normal current through the BS. The stripes arise due to the non-linear response to the periodic voltage drive, creating resonances. Some of the prominent ones are marked in green(red), corresponding to positive bias(both) section(s) of each square pulse being commensurate with the NFJE oscillation period.}
\label{Fig4}
\end{figure}

\textit{Square wave bias.--}  For experimental detection, the NFJE can be sustained by periodically driving it with a square wave bias varying between zero and a positive voltage. For this, we require that the frequency of the bias $\omega_d\gtrsim \Gamma/\hbar$, such that new NFJE pulses are generated before the preceding ones decay. As shown in Fig. \ref{Fig4}, the response is dominated by $\omega_J/2$ oscillations in the presence of low-energy BS, reflecting the underlying NFJE. Additionally, the commensurability of the NFJE oscillations with the voltage drive, along with the non-linearities of the system, generate several resonances, presenting the avenue to choose $\omega_d$ and the fraction $D$ of the square pulse having positive voltage (duty cycle) to strengthen the NFJE signal. Specifically, the strongest resonances are obtained when the durations of both sections of each square pulse are commensurate with the NFJE oscillation period $2\pi \hbar/eV$. This translates to the requirement $(2\pi D/\omega_d)/(2\pi\hbar/eV),(2\pi (1-D)/\omega_d)/(2\pi\hbar/eV)\in \mathbb{Z}$, which ensures that the pair current presents a repeatable form in each pulse, and is in-phase with the normal current. Further examples are presented in the SM~\cite{sm}, including the cases of MZMs and near-zero-frequency BS as discussed in the previous section. 


\textit{Discussion.--} 
We propose a novel non-equilibrium fractional Josephson effect in Josephson junctions subjected to sharply varying voltage pulses across the junction. It requires bound states in both leads near the gap center. Such bound states may arise from disorder~\cite{Pan2020,Pan2021a,Pan2021}, inhomogeneities in the chemical potential or the superconducting gap~\cite{Kells2012,Liu2017,Reeg2018,Moore2018,Moore2018a}, as Andreev bound states in SNS junctions, Shiba states in presence of magnetic adatoms~\cite{Yu1965,Shiba1968,Rusinov1969}, or Majorana zero modes in topological superconductors. The broadening of those bound states is crucial for the lifetime of the non-equilibrium fractional Josephson effect. The sharper the spectral support of the states, the easier it is to observe this phenomenon experimentally.
...
\bibitem{sm} See Supplemental Material, which
...
%\bibitem{Jose2012} P. San-Jose, E. Prada and R. Aguado, Phys. Rev. Lett., {\bf108}, 25, 257001, (2012).
...
\label{FigS1}
\end{figure}
For a physical interpretation of the terms entering the expression for the current, we refer back to Eq.\eqref{Isstemp}. We look at the pair current, given by its second line, for a simple discussion. The first term, $e^{-i\frac{(\phi(t)+\phi(t_1))}{2}} \tilde{f}_{R,1,R,1,\sigma\sigma'}^{-+}(t,t_1)f_{L,1,L,1,\sigma'\sigma}^{+-}(t_1,t)$, represents the tunneling from the right to the left lead. Defining $|i\rangle=|g_L\rangle |g_R\rangle$ as the initial ground state, which is a product of the BCS ground states of each lead, a single tunneling event at time $t_1$ yields $|i'\rangle= e^{-i\frac{\phi(t_1)}{2}}c^\dagger_{L,\sigma'}(t_1)c_{R,\sigma'}(t_1)|g_L\rangle |g_R\rangle$. Its time evolved version at the present time $t$ is $|f\rangle= e^{-i\frac{\phi(t)}{2}}c_{L,\sigma}(t)c_{R,\sigma}^\dagger(t)|g_L+2\rangle |g_R-2\rangle$, where $\pm 2e$ denotes the addition/removal of a Cooper pair. The Cooper instability allows the exchange of a pair without any energy cost. Note that we have the opposite spin $\sigma'\neq \sigma$ as $c_{R,\sigma}^\dagger|g_R-2\rangle=c_{R,\sigma'}|g_R\rangle$ as the pair contains an electron with $\sigma$ and another with $\sigma'\neq \sigma$. Finally, the corresponding current is obtained as the overlap of these two states,
\begin{align}
...
\label{FigS3}
\end{figure}
In this section we consider an inhomogeneous s-wave superconducting nanowire with low-energy Andreev bound states developing due to inhomogeneity in the chemical potential $\mu$ and the pairing potential $\Delta$ near the junction, which in turn typically arise from gate-induced confinement potentials. We employ the Hamiltonian Eq. \eqref{Hsw}, but with the position-dependent chemical potential and pairing amplitude, as shown in Fig. \ref{FigS1pa}. We assume that both the leads are identical, apart from having the non-superconducting potential well on opposite ends (see Fig. \ref{FigS3} for a schematic).
\begin{figure}[htb!]
...
\end{figure}
\newpage
\subsection{Majorana zero modes vs near-zero-frequency bound state}
Here we extend the comparison between non-topological near-zero-frequency BS with $\omega_0\sim\Gamma$ and the MZMs, by using a square-wave bias. Choosing the voltage $eV_0=4\omega_0$ as motivated in the main text, we obtain significantly distinct responses from the MZMs and BS. While the former responds at the fractional Josephson frequency $\omega_{J,\text{MZM}}'=\omega_J/2$, the latter largely exhibits SJE at frequency $\omega_J$ as its NFJE is extremely short-lived and more importantly, inconspicuous, due to our choice of voltage $V_0$.
\begin{figure}[!ht]
\includegraphics[height=8.4cm,width=.8\linewidth]{Fig_S10.pdf}
\caption{Following the discussion associated with Fig. \ref{Fig3} in the main text, using the same parameters, we extend it to a square-wave bias. We show the normalised current $(I/I_0)$ resolved in Fourier domain, with varying drive frequency $\omega_d$. The fractional $\omega_J/2$ response dominates in the TS case, while the BS largely shows SJE. }
\label{FigS10}
\end{figure}
