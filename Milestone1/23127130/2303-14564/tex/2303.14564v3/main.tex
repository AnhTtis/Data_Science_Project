...
\subsubsection{Discussion on the GridVoltage8 Environment}

For GridVoltage8, PPO, MAPPO, and LYPPO have almost identically bad rewards and tracking errors. This is because we use a professional solver PandaPower \citep{pandapower} to simulate the underlying distribution grid. All three RL methods return controllers that quickly drive the system to an unsafe state, making the solver fail to solve the underlying model. In such cases, we simply set the reward and the tracking error assuming a 10\% voltage deviation, which is a typical safety limit for real-world distribution grids \citep{shi2022stability}.
