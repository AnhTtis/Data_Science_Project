...
\begin{figure}
    ...
    \begin{figure}
        ...
        \caption{The power spectrum $P(k)$ corresponding to the example showcased in Fig. \ref{fig:ex1}. The black line is the power spectrum of the mock signal, while the orange lines correspond to the posterior samples, providing the variance for the posterior mean power spectrum estimate. The wavevector $k$ is given in units of $2 \pi / N$. The large scatter is due to the fact that we have few and noisy datapoints, in a signal that is dominated by its uniform component, and thus the modes with $k > 0$ are poorly constrained.}
        \label{fig:pspec_ex1}
    \end{figure}
...
\subsection{Correcting the arrival directions of UHECRs}
...
An example can be seen in Fig. \ref{fig:backtrack}. For the situation presented in Fig. \ref{fig:ex1}, a particle of charge $e$ with final position $\mathbf{r}_i = (L/2,L/2,0)^T$ (Earth's location inside $\mathcal{V}$), velocity direction $\mathbf{\hat{v}}^{\text{obs}} = (0,0,-1)^T$, and energy in the lab frame $E = 5 \times 10^{19} \text{ }eV$ is backtracked through the true GMF (red line), as well as samples of the posterior distribution of the GMF given the local and sparse data shown in Fig. \ref{fig:10b} (magenta lines). A kernel density estimation (KDE) of the posterior distribution for the true arrival directions of the UHECR, based on $100$ posterior samples, is also shown in Fig. \ref{fig:Galactic_coords_Posterior_aniso}. It can be seen, at least in this case,  that we are able to substantially correct for the effect of the GMF. In order to quantify this improvement, we use the Mahalanobis distance (\citealt{mahalanobis}) between a given arrival direction $\hat{\mathbf{v}}$, and the posterior distribution $P(\hat{\mathbf{v}} | d)$ acquired from the geoVI algorithm,

\begin{equation} \label{Mahalanobis}
d_M[\hat{\mathbf{v}} ; P] = \sqrt{(\hat{\mathbf{v}} - \hat{\mathbf{\mu}})^{T} S^{-1}(\hat{\mathbf{v}} - \hat{\mathbf{\mu}})},
\end{equation}
where $S^{-1}$ and $\hat{\mathbf{\mu}}$ are the inverse covariance matrix and mean of $P(\hat{\mathbf{v}} | d)$, respectively. Essentially, $d_M$ measures how many standard deviations $\hat{\mathbf{v}}$ is located away from $\hat{\mathbf{\mu}}$. If  $\mathbf{\hat{v}}^{\text{true}}$ and $\mathbf{\hat{v}}^{\text{obs}}$ are the true and observed arrival directions respectively, then for the specific example shown in Figs \ref{fig:ex1}, \ref{fig:backtrack}, \ref{fig:Galactic_coords_Posterior_aniso}, we calculate $d_M[\hat{\mathbf{v}}^{\text{true}} ; P] = 0.2 \sigma$ and $d_M[\hat{\mathbf{v}}^{\text{obs}} ; P] =35.0 \sigma$.

\begin{figure}
    \centering
        \includegraphics[scale = 0.37]{figures/sky_PDF.png}
        \caption{Kernel density estimation of the posterior PDF for the UHECR arrival directions on the PoS given local and sparse GMF data based on $100$ posterior samples (magenta colormap). The GMF reconstruction problem is the one shown in \ref{fig:ex1}. The red star denotes the true arrival direction, while the black star denotes the observed arrival direction, and they are found $0.2 \sigma$ and $35.0 \sigma$ from the posterior's mean, respectively. This is the same data shown in Fig. \ref{fig:backtrack}, projected on the PoS. The black ellipses signify the $1 \sigma$, $2 \sigma$, and $3 \sigma$ confidence levels. The inset plot zooms in at the location of the posterior's peak. The colormap is in arbitrary units.}
...
As was done with the weakly turbulent example, in Fig. \ref{fig:backtrack_iso} we show the actual path of a UHECR with charge $e$, observed arrival direction $\mathbf{\hat{v}}^{\text{obs}} = (0,0,-1)^T$, and energy in the lab frame $E = 5 \times 10^{19} \text{ }eV$ through this strongly turbulent field (red path). The magenta paths correspond to those that the same UHECR would have taken if it travelled through samples of the posterior distribution of the GMF. The black path is the zeroth order approximation - unaltered by the presence of the GMF.  Especially in Fig. \ref{fig:backtrack_iso_b}, it can clearly be seen that in the extreme case of completely isotropic turbulence, there is still a substantial deviation due to the presence of the GMF, and the zeroth order approximation (that is, assuming $\hat{\mathbf{v}}^{\text{true}} \simeq \hat{\mathbf{v}}^{\text{obs}}$) is much further away from the posterior's mean than the  true arrival direction. In Fig. \ref{fig:Galactic_coords_Posterior_iso} we plot a KDE of the initial velocity's posterior on the PoS, based on $100$ posterior samples  As with Fig. \ref{fig:Galactic_coords_Posterior_aniso}, the red star denotes the true arrival direction, while the black star denotes the observed arrival direction. The Mahalanobis distance between these directions and the posterior (magenta contours) is computed at $2.0 \sigma$ and $7.6 \sigma$ respectively.
...
\begin{figure}
    ...
    \caption{The power spectrum corresponding to the example showcased in Fig. \ref{fig:ex2}. The black line is the power spectrum of the mock signal, while the orange lines correspond to the posterior samples, providing the variance for the posterior mean power spectrum estimate. The wavevector $k$ is given in units of $2 \pi / N$. In this case, all modes are relevant and well constrained, as opposed to the case of Fig. \ref{fig:pspec_ex1}. Further, note that as $k$ becomes larger the posterior of the power spectrum starts deviating from that of the signal, implying a loss of information at smaller lengthscales, as observed in Fig. \ref{fig:ex2}.}
    \label{fig:pspec_ex2}
\end{figure}

\begin{figure}
    \centering
        \includegraphics[scale = 0.37]{figures/sky_PDF_iso.png}
        \caption{Kernel density estimation of the posterior PDF for the UHECR arrival directions on the PoS given local and sparse GMF data based on $100$ posterior samples (magenta colormap). The GMF reconstruction problem is the one shown in Fig. \ref{fig:ex2}. The red star denotes the true arrival direction, while the black star denotes the observed arrival direction, and they are found $2.0 \sigma$ and $7.6 \sigma$ from the posterior's mean, respectively. This is the same data shown in Fig. \ref{fig:backtrack_iso}, projected on the PoS. The black ellipses signify the $1 \sigma$, $2 \sigma$, and $3 \sigma$ confidence levels.}
