...
These models with boundary, which we will call half space models, are expected to exhibit universal behavior with a rich phase diagram depending on the boundary strength. Some progress towards this has been made in zero-temperature models \cite{BR01b, SI04,BBCS18}, hinting that similar phase transitions should hold for all half space models in the KPZ universality class, but progress in positive temperature models has been more difficult.

This problem was first considered by Kardar \cite{K85}, and has been studied in the physics literature using a replica Bethe Ansatz approach \cite{K87, GP12, DNKLDT20, KLD20}, culminating in work of De Nardis, Krajenbrink, Le Doussal, and Thiery \cite{DNKLDT20}, where a phase transition for the one point distribution at the boundary of the half space KPZ equation was found. In the physics work of Borodin, Bufetov, and Corwin \cite{BBC16}, nested contour integral formulas were found for certain $q$-moments, some of which were then proven rigorously in work of Barraquand, Borodin, and Corwin \cite{BBC20}, but these still did not lead to mathematically rigorous asymptotics. For a long time, the only mathematically rigorous result on positive temperature models was work of Barraquand, Borodin, Corwin, and Wheeler \cite{BBCW18} on the half space asymmetric simple exclusion process (ASEP) and KPZ equation, and this was restricted to the critical boundary strength.

Recently, the groundbreaking work of Imamura, Mucciconi, and Sasamoto \cite{IMS22} established a phase transition for the one point distribution at the boundary for the free energy of the half space log-gamma polymer and the solution to the half space KPZ equation, the first such mathematically rigorous results for positive temperature models. They found a connection to a free fermionic model via a bijective approach, resulting in formulas amenable to asymptotics. However, this left the problem of studying a variety of other discrete models including the ASEP, which contains an additional boundary parameter.

In this paper, we study the ASEP on the non-negative integers, a system of particles which can jump to unoccupied sites, and with particles allowed to enter and exit at $0$. Initially, the system starts with no particles. From known results for zero temperature models (see e.g. \cite{BR01b,SI04,BBCS18}), it is expected that the fluctuations for the total number of particles in the system, which we call the current, has a phase transition depending on the asymptotic density $\rho$ of particles at the boundary, which depends on the rates at which particles enter and exit. In particular, Tracy--Widom GSE and GOE fluctuations are expected when $\rho>\frac{1}{2}$ and $\rho=\frac{1}{2}$ respectively, with $\tau^{1/3}$ scaling, and Gaussian fluctuations with $\tau^{1/2}$ scaling are expected when $\rho<\frac{1}{2}$. However, for the ASEP, results are currently only known at the critical point $\rho=\frac{1}{2}$, and only for a specific choice of parameters, by work of Barraquand, Borodin, Corwin, and Wheeler \cite{BBCW18}. 

Our main result is to establish these asymptotics as well as the appearance of the Baik--Rains crossover distribution introduced in \cite{BR01b} if the boundary strength is tuned appropriately, subject only to a condition that the direction of the asymmetry at the boundary matches that of the bulk. For the Tracy--Widom regimes, all possible values for the boundary parameters are allowed, and moreover this allows for all possible densities at the boundary. We expect that this condition can be removed, but the methods in this paper do not seem to be able to handle this. We are also able to study the half space stochastic six vertex model, and we establish the same phase transition for the fluctuations of the height function at the boundary.

While one point fluctuations for a wide variety of models without a boundary have been established, see e.g. \cite{TW09, BCG16, A18}, and indeed there are now classes of models for which we even have multipoint and process convergence to the limiting universal objects \cite{QS23, W23, V20, DOV22, MQR21}, the analysis of systems with boundary is usually much more complicated. Mathematical progress has been made on zero-temperature models, starting with the work of Baik and Rains on longest increasing subsequences of random involutions \cite{BR01a,BR01b}, with subsequent work on the polynuclear growth model due to Imamura--Sasamoto \cite{SI04}, and work on the Pfaffian Schur process and related models due to Borodin--Rains \cite{BR05}, and Baik--Barraquand--Corwin--Suidan \cite{BBCS18}. There is also work of Barraquand--Borodin--Corwin--Wheeler \cite{BBCW18} for the ASEP, although this is restricted to the critical boundary density.

Only recently has the Baik--Rains phase transition for the one point fluctuations at the boundary been established for any positive-temperature model \cite{IMS22}. This is in spite of tremendous progress in understanding half space models at positive temperature and their algebraic structure, see e.g. \cite{TW13,TW13b,OSZ14,BR22,H22, BC22, BW22}. Our work thus presents important additional evidence for universality in the half space setting. For some additional recent works on half space models, see \cite{BFO20, CS18,P22, P19, W20, BBS23}, and also relevant work in the physics literature \cite{BD21, BKD22, BBC16}.
...
A key quantity associated to the system is the asymptotic density of particles at $0$,
\begin{equation*}
    \rho=\frac{1}{1+\nu^{-1}},
\end{equation*}
where
...
\begin{theorem}
\label{thm: main}
Let $N(\tau)$ denote the number of particles within the system at time $\tau$ in the half space ASEP started from the empty configuration, and let $\rho$ denote the asymptotic density of particles at $0$. Assume that $t<1$. Then depending on $\rho$, as $\tau\to\infty$,
...
\begin{remark}
Our exact formulas apply to the general inhomogeneous half space stochastic six vertex model, and so in principle more general asymptotic results could be obtained, say for periodic parameters. We do not pursue the details of this in this paper.
\end{remark}
...
% Analogous questions without a boundary have been well-studied, starting with the work of Tracy and Widom for ASEP on the full line with step initial data \cite{TW09}. Borodin, Corwin, and Gorin studied the analogous question for the stochastic six vertex model \cite{BCG16}. Aggarwal studied the same problem with stationary initial data for both the ASEP and six vertex model without boundary \cite{A18}. More recently, progress has been made towards multipoint observables, including the groundbreaking works of Quastel and Sarkar \cite{QS23} for the ASEP (progress has also been made for other models, see works of Virag \cite{V20}, and Wu \cite{W23}).

% The half space ASEP and six vertex model have been extensively studied, see for example \cite{H22, BBCW18, BBC20, CS18, P19}. The study of half space models began in the work of Baik and Rains on longest increasing subsequences of random involutions \cite{BR01a,BR01b}, with subsequent work on the Pfaffian Schur process and related models due to Borodin--Rains \cite{BR05}, Imamura--Sasamoto \cite{SI04}, and Baik--Barraquand--Corwin--Suidan \cite{BBCS18}. These models are all free-fermionic, leading to Pfaffian formulas which can be asymptotically analyzed. The ASEP is not free-fermionic, which makes it very surprising that Fredholm Pfaffian formulas can be found, albeit only after a random shift.

% Tracy and Widom found exact formulas for the half space ASEP in \cite{TW13} using coordinate Betha Ansatz. More recently, Barraquand and Corwin found exact formulas for certain $q$-moments of the current in half space ASEP via Markov duality \cite{BC22}. Formulas for the $q$-moments of the height function in the half space six vertex model were found in work of Barraquand--Borodin--Corwin \cite{BBC20}. Unfortunately, these formulas do not seem to be amenable to asymptotic analysis

% It is natural to wonder about convergence of multipoint observables in models with boundary, but this seems to be a hard problem. In fact, beyond zero temperature models such the TASEP, even single point observables away from the boundary are currently out of reach. Moreover, unlike in the full space setting, where objects such as the directed landscape, Airy line ensemble, or the KPZ fixed point, are expected to be the universal limiting objects, it is still unclear what these should be in the half space setting.

% The asymptotics obtained in Theorem \ref{thm: main} in the $q=\beta=0$ case were established in \cite{BBCS18}. Asymptotics for $N(\tau)$ when $\rho=\frac{1}{2}$ under the restriction that $\alpha=1/2$ and $\beta=q/2$ were already computed in \cite{BBCW18}, and the extension to the half space six vertex model (with similar restrictions on the parameters) seems approachable using similar methods. More recently, the entire phase transition was established for certain positive temperature half space models including the log-gamma polymer and the KPZ equation \cite{IMS22}.

% The relationship between the half space six vertex model and half space Hall--Littlewood measures has already been noticed in a special case in \cite{BBCW18}, building on the ideas developed in \cite{WZJ16}. The relationship between half space Hall--Littlewood measures and free boundary Schur processes with one fewer parameter than considered in this work was established in the recent work \cite{IMS21} via a bijective approach, and we use the same bijection to prove our more general result.
...
Let $q,t\in [0,1)$ and $\nu>0$ such that $\nu^2 t<1$ be parameters, and let $a$ denote a specialization of the Hall--Littlewood polynomials of parameter $q$ such that $P_\lambda(a;q)\geq 0$ for all $\lambda$. We define the \emph{half space Hall--Littlewood measure} to be
\begin{equation*}
    \mathbb{HL}_a^{(q,t,\nu)}(\lambda)=\frac{1}{\Pi(a;q,t,\nu)} h_\lambda(q,t,\nu)P_\lambda(a;q),
\end{equation*}
where $\Pi$ normalizes this to be a probability measure. If $a=(a_1,a_2,\dotsc)$, we have
\begin{equation}
...
\begin{equation*}
    \mathcal{E}_\lambda(b)=\sum_{\mu'\text{ even}}b^{el}_\mu(q)Q_{\lambda/\mu}(\rho)=h_\lambda(q,t,\nu),
\end{equation*}
...
\begin{equation*}
    p_k(b)=\frac{1+(-1)^k-(\nu t)^k-(-\nu)^{-k}}{1-q^k}.
\end{equation*}
In particular, the half space Hall--Littlewood process considered in this paper can be obtained via the specialization $\rho$ from the one defined in \cite{BBC20}. 
\end{lemma}
\begin{proof}
...
Both models are models for arrows on a lattice, where the horizontal edges again can have at most one arrow, but where the vertical edges can have any number of arrows. The model is again Markovian, like the six vertex model, with parameters (called \emph{rapidities}) associated to each row and a parameter $q$. The vertex weights given by
...
\begin{equation*}
\frac{1}{\Pi(a;q,t,\nu)}\sum_{\lambda}h_\lambda(q,t,\nu)\begin{tikzpicture}[scale=0.8,baseline=(current bounding box.center),>=stealth]
%lattice
\foreach\x in {0,...,6}{
...
\begin{remark}
Note that due to the sign, the free boundary Schur process we consider is not really a probability measure on partitions. However, the marginal $\lambda_1$ is a genuine random variable, at least when $t\leq q$. Moreover, as long as $t<1$ and $\nu^{-1}<1$, the definition is absolutely convergence and decays in $\lambda_1$, and the expression $\PP(\lambda_1\leq s)=\sum_{x\leq s}\mathbb{FBS}_a^{(q,t,\nu)}(x)$ is still analytic in $t$ and $\nu$. Thus, the formulas attained, which are also analytic functions of $t$ and $\nu$, continue to apply by analytic continuation.
\end{remark}
...
\end{definition}
\begin{remark}
The extra summands in the definition of $G_{GSE}$ come from poles which cross the contour as $\xi\to 0$, see \cite{IMS22} for details. We will see the same phenomenon for the pre-limiting kernels when we study asymptotics in the $\rho=\frac{1}{2}$ regime.
\end{remark}
...
Let $\sigma=\frac{(a(1-a)n)^{1/3}}{(1+a)}$, $x=\frac{2a}{1+a}n+\sigma u$ and $y=\frac{2a}{1+a}n+\sigma v$. For $i,j\in \N$, we have
\begin{equation*}
    \left(\sigma D_x^{(\gamma_2)}\right)^i\left(\sigma D_y^{(\gamma_2)}\right)^jk(x,y)\to \partial_u^i\partial_v^j k_{GSE}
\end{equation*}
if $\nu\geq 1$ is fixed, and
\begin{equation*}
    \left(\sigma D_x^{(\gamma_2)}\right)^i\left(\sigma D_y^{(\gamma_2)}\right)^jk(x,y)\to  \partial_u^i\partial_v^j k_{cross}^{\xi}(u,v)
\end{equation*}
if $\frac{1}{\nu}=1-\xi/\sigma$. Moreover, $k(u,v)$ satisfies Assumption \ref{asmp: kernels}.
\end{lemma}
\begin{proof}
...
\begin{equation*}
\begin{split}
    S_k(x,y)=&\frac{(1-\gamma_2)^{-2}}{2\pi i}\oint _{|w|=r'}(q^kw)^{x+3/2} w^{-y-5/2}F(w)F(q^{-k}w^{-1})
...
\begin{equation*}
    (w)^{-2k}\frac{(w^2;q)_\infty(q/w^2;q)_\infty}{(q^{2k}w^2;q)_\infty(1/w^2;q)_\infty}\frac{\theta_3(\zeta^2q^{-2k};q^2)}{\theta(\zeta^2;q^2)}g(w)g(q^{-k}w^{-1}).
\end{equation*}
...
\begin{split}
    ...
    \\&\qquad \times\nu^{-\sigma\tau^{1/2}u}\exp(\tau G(\nu))\nu^{-3/2}\frac{(q;q)_\infty(-t;q)_\infty}{(-qt\nu^2;q)_\infty\theta_3(\zeta^2;q^2)},
...
\end{equation*}
where $C_k$ is not a function of $k$ or $u$. For $k>0$, since we have already seen that $G(\nu)=-G(\nu^{-1})>G(q^{-k}\nu)$ for $k\leq 2m+\varepsilon-2$, this is bounded by
\begin{equation*}
