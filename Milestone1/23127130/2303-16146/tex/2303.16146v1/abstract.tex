...
diverse, including custom functions which can span libraries or be
written in pure Python. The majority of systems available to accelerate EDA workloads
focus on bulk-parallel workloads, which contain vastly different computational
...
implemented our techniques in \system{}, which rewrites notebook cells to be
more efficient for ad-hoc EDA workloads. We develop techniques for efficient
rewrites in \system{}, including dynamic checking of preconditions under which
rewrites are correct and just-in-time rewrites for notebook environments. We
show that \system{} can rewrite individual cells to be 57$\times$ faster
compared to \code{pandas} and 1909$\times$ faster compared to optimized systems
such as \code{modin}. Furthermore, \system{} can accelerate whole notebooks by
up to 3.6$\times$ compared to \code{pandas} and 26.4$\times$ compared to
\code{modin}.
...
%compared to \code{pandas} and \emph{26.4$\times$} compared to \code{modin}.
...
% such as \code{modin}. Furthermore, \system{} can accelerate whole notebooks by
% up to 3.6$\times$ compared to \code{pandas} and \emph{26.4$\times$} compared to
% \code{modin}.
