...
which typically do not fit in main memory on a single server. To accelerate such
workloads, both industry and academia have invested in accelerating
\emph{bulk-parallel} workloads.
...
the peak memory usage being up to 772$\times$ higher than native
\texttt{pandas}. The peak memory usage increased 53$\times$ on average
(geometric mean).
...
Six of these notebooks (30\%) were unable to run when \texttt{pandas} was
