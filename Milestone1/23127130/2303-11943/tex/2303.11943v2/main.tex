...
$$
\\
Case 2 occurs when $ a \neq s{0}_{n\times1}\,\text{and}\, a^T (\Tilde{L} - (\lambda_{\alpha} + \phi_{\alpha}^T E \phi_{\alpha})I) a = 0.$ One possibility is that  $M_{\alpha} = (\Tilde{L} - (\lambda_{\alpha} + \phi_{\alpha}^T E \phi_{\alpha})I)$ needs to be a rotation matrix, but this case cannot happen if we impose a minimum degree constraint as mentioned in (\ref{Theo2}).
