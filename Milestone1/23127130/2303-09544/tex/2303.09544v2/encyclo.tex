...
\documentclass[aps,prd,reprint,showpacs,showkeys,superscriptaddress,nofootinbib]{revtex4-1}
...
\usepackage{natbib} 
\setcitestyle{authoryear,square,comma,compress}
\setlength{\bibsep}{2pt plus 0.3ex} %separation between references
\renewcommand{\cite}{\citep}
%\setcitestyle{authoryear,square,comma,open={([},close={])}}
...
Chronologically, these are (cf.\ Fig.~\ref{fig:perspectives}):
...
%	"Bose and Fermi N-particle systems were recovered as unitarily inequivalent induced representations of the group [...] In two-dimensional space, however, the coordinate space is multiply connected, implying induced representations other than those describing the usual Bose or Fermi statistics; these are explored in the present paper." [GMS81]
	...
	\cite{Froehlich-88,FroMar-88,FroMar-89,Froehlich-90,FroGab-90,FroKer-93}, 
...
%	"Since interchange of two of these particles can give any phase, I will call them generically anyons." [W82b]
	...
	\cite{AroSchWilZee-85} (see also \cite{Dowker-85}).
\end{enumerate}
...
has been covered in the books and reviews
...
already in 1940-'42 \cite{Gentile-40,Gentile-42},
...
parastatistics (nonabelian representations of the permutation group) 
...
and Fr\"ohlich \cite{Froehlich-76}. 
...
see e.g.\ \cite{Dowker-85,Froehlich-90,MunSch-95,DelFigTet-97,Myrheim-99,Lundholm-17,LunQva-20}.
...
\begin{equation} \label{eq:exch-phase}
	\rho(\sigma_j) = e^{i\alpha\pi}, \qquad \text{for all $j=1,2,\ldots,N-1$};
\end{equation}
cf.\ Figs.~\ref{fig:circ}-\ref{fig:loops}.
...
see however \cite{Jackiw-90,Froehlich-09,Mund-09}.} 
free kinetic energy for $N$ particles with mass $m$:
...
\cite{Dowker-85,MunSch-95,LunQva-20}.}
...
\begin{equation} \label{eq:U-transmute}
...
always gives a rigorous lower bound:
...
and then to again shoot upwards in energy with a slope $\gtrsim O(N^2)$.
...
The few-particle spectra led to some initial results on the thermodynamics
of the anyon gas, by considering the high-temperature, low-density virial
expansions \cite{AroSchWilZee-85,Dowker-85} 
...
i.e. (taking some suitable scale $a>0$ for normalization):
\begin{eqnarray*}
...
The kinetic energy density is here given by the energy per unit area 
...
\end{equation}
thus suggesting \eqref{eq:cf-func}.
...
%	\includegraphics{figz-popcorn.pdf}
	...
	\eqref{eq:fractionality} \cite{LunSol-13a,Lundholm-16}.}
	\label{fig:popcorn}
\end{figure}
	...
	Therefore the minimum of $\hat{\ell}^2$ is $\min_{q \in \Z}|\alpha-2q|^2 = \alpha_2^2$.
...
%	\includegraphics{figz-IdealEnergy.pdf}
	...
	$F_2(\alpha_2) = \frac{1}{4}\min\{ E_2(\alpha_2),0.147 \}$, correspondingly.
	}
	\label{fig:ideal}
\end{figure}
...
\item {\bf Local exclusion principle.} 
	...
	$\tilde{E}_2(\alpha) \approx 2\pi (j_{\alpha_2}')^2 \ge 4\pi\alpha_2$, 
	where $j_\alpha'$ denotes the 
	first zero of the derivative of the Bessel function $J_\alpha$ 
	\cite{LarLun-16}.}
	$\tilde{E}_2$ to it: 
	for $N \ge 2$,
	...
	\begin{equation} \label{eq:E-N-4}
		E_N / N \ge \frac{1}{4}\min \{E_2,E_3,E_4\} \ge 
...
\item {\bf Degeneracy pressure.} 
	...
	Indeed, the ideal Fermi gas satisfies the following kinetic energy inequality
	(known in mathematics as a \keyword{Lieb--Thirring inequality} \cite{LieThi-75})
	which captures this type of degeneracy pressure:
	for any number $N$ of particles and any 
	...
	their thermodynamic stability w.r.t.\ Coulomb interaction with a mixture of charges
...
%	\cite{LunSol-13b,LunSol-13a,LunSol-13b,LunSol-14,LarLun-16,LunSei-17,GirRou-22}:
	...
	for any number $N$ of particles and any $N$-anyon state $\Psi \in L^2_\alpha$,
...
\begin{figure}
	\centering
	\scalebox{0.96}{\begin{tikzpicture}[>=stealth']
		%COLOR GROUPS
		\def\x{0}
		\def\y{0}
		\def\c{red}
...
\item Some of the states \eqref{eq:trial-even} are exact but 
	...
	low-energy eigenfunctions of the Hamiltonian with singular boundary conditions
...
In the \keyword{extended anyon gas}, we work in the magnetic perspective 
...
gas with fixed 
extension parameter $R>0$ will at low temperatures prefer to adjust its density to 
$\bar\gamma \gtrsim 1$,
...
as the coupling $\beta$ (total number of flux units) grows
...
the cancellation is not complete, but both terms in \eqref{eq:mod-phase-decomp}
contribute an energy of order $\beta$.}
...
confirm the theoretical prediction of 
...
functional, plus added a hard-core delta 
...
it is equivalent to study a semiclassical mean-field limit
$$
...
More precisely, one obtains a semiclassical {\bf Vlasov functional}:
$$
...
\begin{equation} \label{eq:mu-Vla}
...
such as translation and scale covariance.
...
\item
	...
	\begin{equation} \label{eq:rank-growth}
...
instead we refer to 
\cite{Beer-etal-18,LunQva-20,Stern-08,Nayak-etal-08,LahPac-17,MasMizNit-23} 
...
Then, the statistical repulsion as given both by the Hardy inequality \eqref{eq:Hardy} 
(and its variants, depending on $\alpha_{N,n}$), 
the local exclusion principle \eqref{eq:E-2-approx}-\eqref{eq:E-N-4},
as well as the degeneracy pressure \eqref{eq:LT-anyons}, 
generalize to the ideal nonabelion gas:
...
(in the rep $\rho_N$)
that sets the main behavior of the gas w.r.t.\ fractional exclusion statistics.
...
The lowest Landau level (LLL) is the kernel of $\ha$
...
$$
For electrons, $\mu$ is an odd integer, so that $\PsiLau_\mu \in L^2_\asym$.
...
\begin{equation} \label{eq:Berry-holonomy}
	\rho(\sigma) = e^{i\Phi_\sigma}, \qquad
	\Phi_\sigma = \oint_\sigma \sum_{j=1}^n \tilde{\bA}_j \cdot d\by_j,
\end{equation}
...
$+1/\mu$ that \emph{reduces} the field),
...
sufficiently strong intra-species repulsion.
...
(cf.\ Fig.~\ref{fig:experiments}, upper):
\begin{multline*}
...
of $n$ identical linear molecules (cf.\ Fig.~\ref{fig:experiments}, lower).
...
Emergent anyons can also be expected on spin networks and lattices 
\cite{Bachmann-17,Bachmann-etal-20,Bachmann-etal-23}.
...
Even in the quantum gravity context might we expect nonabelions, 
as they can capture
...
(cf. \cite{Forte-91,Myrheim-99,Jain-07,LunRou-16,LamLunRou-22}).
...
\begin{ack}
	...
	Further, I thank the editor Tapash Chakraborty for the timely invitation to
...
%%\bibliographystyle{amsplainhyperdupernd}
%%\bibliographystyle{amsplainnatduper}
%\bibliographystyle{amsplainnathyperduper}
%\bibliography{biblio}

\begin{thebibliography}{203}
\providecommand{\natexlab}[1]{#1}

\bibitem[Abramsky and Brandenburger(2011)]{AbrBra-11}
S.~Abramsky and A.~Brandenburger, \emph{The sheaf-theoretic structure of
  ...
  no.~11, 113036,
  \href{http://dx.doi.org/10.1088/1367-2630/13/11/113036}{\path{doi}}.

\bibitem[Aftalion et~al.(2006{\natexlab{a}})Aftalion, Blanc, and
  Nier]{AftBlaNie-06a}
A.~Aftalion, X.~Blanc, and F.~Nier, \emph{{Vortex distribution in the lowest
  Landau level}}, Phys. Rev. A \textbf{73} (2006), 011601(R),
  \href{http://dx.doi.org/10.1103/PhysRevA.73.011601}{\path{doi}}.

\bibitem[Aftalion et~al.(2006{\natexlab{b}})Aftalion, Blanc, and
  Nier]{AftBlaNie-06b}
A.~Aftalion, X.~Blanc, and F.~Nier, \emph{Lowest {L}andau level functional and
  ...
  \textbf{241} (2006), no.~2, 661--702,
  \href{http://dx.doi.org/10.1016/j.jfa.2006.04.027}{\path{doi}}. \MR{MR2271933
  (2008c:82052)}

\bibitem[Aharonov and Bohm(1959)]{AhaBoh-59}
...
\bibitem[Albeverio et~al.(2005)Albeverio, Gesztesy, H{\o}egh-Krohn, and
  Holden]{Albeverio-etal-05}
...
\bibitem[Alonso and Simon(1980)]{AloSim-80}
...
\bibitem[Arovas et~al.(1984)Arovas, Schrieffer, and Wilczek]{AroSchWil-84}
...
\bibitem[Arovas et~al.(1985)Arovas, Schrieffer, Wilczek, and
  Zee]{AroSchWilZee-85}
...
\bibitem[Bachmann(2017)]{Bachmann-17}
...
\bibitem[Bachmann et~al.(2020)Bachmann, Bols, De~Roeck, and
  Fraas]{Bachmann-etal-20}
...
\bibitem[Bachmann et~al.(2023)Bachmann, Nachtergaele, and
  Vadnerkar]{Bachmann-etal-23}
S.~Bachmann, B.~Nachtergaele, and S.~Vadnerkar, \emph{Dynamical abelian anyons
  with bound states and scattering states}, arXiv e-prints, 2023,
  \href{http://arxiv.org/abs/2303.07379}{\path{arXiv:2303.07379}}.

\bibitem[Baldelli et~al.(2021)Baldelli, Juli{\'a}-D{\'{\i}}az, Bhattacharya,
  Lewenstein, and Gra{\ss{}}]{Baldelli-etal-21}
N.~Baldelli, B.~Juli{\'a}-D{\'{\i}}az, U.~Bhattacharya, M.~Lewenstein, and
  T.~Gra{\ss{}}, \emph{Tracing non-abelian anyons via impurity particles},
  Phys. Rev. B \textbf{104} (2021), 035133,
  \href{http://dx.doi.org/10.1103/PhysRevB.104.035133}{\path{doi}}.

\bibitem[Bartolomei et~al.(2020)Bartolomei, Kumar, Bisognin, Marguerite,
  Berroir, Bocquillon, Pla{\c c}ais, Cavanna, Dong, Gennser, Jin, and
  F{\`e}ve]{Bartolomei-etal-20}
H.~Bartolomei, M.~Kumar, R.~Bisognin, A.~Marguerite, J.-M. Berroir,
  E.~Bocquillon, B.~Pla{\c c}ais, A.~Cavanna, Q.~Dong, U.~Gennser, Y.~Jin, and
  G.~F{\`e}ve, \emph{Fractional statistics in anyon collisions}, Science
  \textbf{368} (2020), no.~6487, 173--177,
  \href{http://dx.doi.org/10.1126/science.aaz5601}{\path{doi}}.

\bibitem[Beer et~al.(2018)Beer, Bondarenko, Hahn, Kalabakov, Knust, Niermann,
  Osborne, Schridde, Seckmeyer, Stiegemann, et~al.]{Beer-etal-18}
K.~Beer, D.~Bondarenko, A.~Hahn, M.~Kalabakov, N.~Knust, L.~Niermann, T.~J.
  Osborne, C.~Schridde, S.~Seckmeyer, D.~E. Stiegemann, et~al., \emph{From
  categories to anyons: a travelogue}, arXiv e-prints, 2018,
  \href{http://arxiv.org/abs/1811.06670}{\path{arXiv:1811.06670}}.

\bibitem[Berry(1984)]{Berry-84}
...
\bibitem[Biedenharn et~al.(1990)Biedenharn, Lieb, Simon, and
  Wilczek]{BieLieSimWil-90}
...
\bibitem[Bonderson et~al.(2011)Bonderson, Gurarie, and Nayak]{BonGurNay-11}
...
\bibitem[Bourdeau and Sorkin(1992)]{BorSor-92}
...
\bibitem[Brooks et~al.(2021{\natexlab{a}})Brooks, Lemeshko, Lundholm, and
  Yakaboylu]{Yakaboylu-etal-20}
...
\bibitem[Brooks et~al.(2021{\natexlab{b}})Brooks, Lemeshko, Lundholm, and
  Yakaboylu]{Brooks-etal-21}
...
\bibitem[Caenepeel and MacKenzie(1994)]{CaeMac-94}
...
\bibitem[Calogero(1969{\natexlab{a}})]{Calogero-69b}
...
\bibitem[Camino et~al.(2005)Camino, Zhou, and Goldman]{CamZhoGol-05}
...
\bibitem[Canright and Johnson(1994)]{CanJoh-94}
...
\bibitem[Cappelli et~al.(2001)Cappelli, Georgiev, and Todorov]{CapGeoTod-01}
...
\bibitem[Chen et~al.(1989)Chen, Wilczek, Witten, and Halperin]{CheWilWitHal-89}
...
\bibitem[Chitra and Sen(1992)]{ChiSen-92}
...
\bibitem[Cooper and Simon(2015)]{CooSim-15}
...
\bibitem[Correggi and Fermi(2021)]{CorFer-21}
...
\bibitem[Correggi and Oddis(2018)]{CorOdd-18}
...
\bibitem[Correggi et~al.(2017)Correggi, Lundholm, and Rougerie]{CorLunRou-16}
...
\bibitem[Correggi et~al.(2018)Correggi, Lundholm, and
  Rougerie]{CorLunRou-proc-17}
M.~Correggi, D.~Lundholm, and N.~Rougerie, \emph{Local density approximation
  for almost-bosonic anyons}, Proceedings of QMath13, Atlanta, October 8--11,
  2016, Mathematical problems in quantum physics (F.~Bonetto, D.~Borthwick,
  E.~Harrell, and M.~Loss, eds.), Contemp. Math., vol. 717, 2018, pp.~77--92,
  \href{http://dx.doi.org/10.1090/conm/717}{\path{doi}}.

\bibitem[Correggi et~al.(2019)Correggi, Duboscq, Lundholm, and
  Rougerie]{CorDubLunRou-19}
...
\bibitem[Coutinho et~al.(1992)Coutinho, Nogami, and
  Fernando~Perez]{CouNogPer-92}
...
\bibitem[Date and Murthy(1993)]{DatMur-93}
...
\bibitem[Date et~al.(2003)Date, Murthy, and Vathsan]{DatMurVat-03}
...
\bibitem[Delaney et~al.(2016)Delaney, Rowell, and Wang]{DelRowWan-16}
...
\bibitem[Dell'Antonio et~al.(1997)Dell'Antonio, Figari, and Teta]{DelFigTet-97}
...
\bibitem[Dowker(1985)]{Dowker-85}
J.~S. Dowker, \emph{Remarks on nonstandard statistics}, J. Phys. A \textbf{18}
  (1985), no.~18, 3521--3530,
  \href{http://dx.doi.org/10.1088/0305-4470/18/18/015}{\path{doi}}. \MR{822912}

\bibitem[Dub{\v{c}}ek et~al.(2018)Dub{\v{c}}ek, Klajn, Pezer, Buljan, and
  Juki{\'{c}}]{Dubcek-etal-18}
...
\bibitem[Dyson and Lenard(1967)]{DysLen-67}
...
\bibitem[Ehrenberg and Siday(1949)]{EhrSid-49}
...
\bibitem[Einarsson(1992)]{Einarsson-92}
...
\bibitem[Fan et~al.(2022)Fan, Li, Hu, Li, Long, Liu, Yang, Nie, Li, Xin,
  et~al.]{Fan-etal-22}
Y.-a. Fan, Y.~Li, Y.~Hu, Y.~Li, X.~Long, H.~Liu, X.~Yang, X.~Nie, J.~Li,
  T.~Xin, et~al., \emph{Experimental realization of a topologically protected
...
\bibitem[Fetter et~al.(1989)Fetter, Hanna, and Laughlin]{FetHanLau-89}
A.~L. Fetter, C.~B. Hanna, and R.~B. Laughlin, \emph{Random-phase approximation
  ...
  9679--9681, \href{http://dx.doi.org/10.1103/PhysRevB.39.9679}{\path{doi}}.

\bibitem[Formanek(1996)]{Formanek-96}
...
\bibitem[Fredenhagen et~al.(1989)Fredenhagen, Rehren, and
  Schroer]{FreRehSch-89}
...
\bibitem[Freedman et~al.(2003)Freedman, Kitaev, Larsen, and
  Wang]{FreKitLarWan-03}
...
\bibitem[Fr{\"o}hlich(1954)]{Frohlich-54}
...
\bibitem[Fr\"ohlich and Gabbiani(1990)]{FroGab-90}
...
\bibitem[Fr\"{o}hlich and Marchetti(1988)]{FroMar-88}
...
\bibitem[Fr\"{o}hlich and Marchetti(1989)]{FroMar-89}
...
\bibitem[Fr{\"o}hlich(1976)]{Froehlich-76}
J.~Fr{\"o}hlich, \emph{New super-selection sectors {(``soliton-states'')} in
  two dimensional {B}ose quantum field models}, Commun. Math. Phys. \textbf{47}
  (1976), no.~3, 269--310.

\bibitem[Fr{\"o}hlich(1988)]{Froehlich-88}
...
\bibitem[Fr{\"o}hlich(1990)]{Froehlich-90}
...
\bibitem[Fr{\"o}hlich(2009)]{Froehlich-09}
J.~Fr{\"o}hlich, \emph{Spin, or actually: Spin and quantum statistics}, The
  Spin: Poincar{\'e} Seminar 2007 (B.~Duplantier, J.-M. Raimond, and
  V.~Rivasseau, eds.), Birkh{\"a}user Basel, Basel, 2009, pp.~1--60,
  \href{http://dx.doi.org/10.1007/978-3-7643-8799-0_1}{\path{doi}}.

\bibitem[Fr{\"o}hlich and Kerler(1993)]{FroKer-93}
J.~Fr{\"o}hlich and T.~Kerler, \emph{Quantum groups, quantum categories and
  quantum field theory}, Berlin-Heidelberg-New York: Springer-Verlag, 1993,
  \href{http://dx.doi.org/10.1007/BFb0084244}{\path{doi}}.

\bibitem[Gentile(1940)]{Gentile-40}
...
\bibitem[Gentile(1942)]{Gentile-42}
...
\bibitem[Girardeau(1960)]{Girardeau-60}
M.~Girardeau, \emph{Relationship between systems of impenetrable bosons and
  ...
  \href{http://dx.doi.org/10.1063/1.1703687}{\path{doi}}. \MR{0128913 (23
  {\#}B1950)}

\bibitem[Girardeau(1965)]{Girardeau-65}
...
\bibitem[Girardot(2020)]{Girardot-20}
...
\bibitem[Girardot(2021)]{Girardot-21}
...
\bibitem[Girardot and Rougerie(2021)]{GirRou-21}
...
\bibitem[Girardot and Rougerie(2022)]{GirRou-22}
...
\bibitem[Goldin et~al.(1981)Goldin, Menikoff, and Sharp]{GolMenSha-81}
...
\bibitem[Goldin et~al.(1980)Goldin, Menikoff, and Sharp]{GolMenSha-80}
...
\bibitem[Goldin and Majid(2004)]{GolMaj-04}
...
\bibitem[Goldin and Sharp(1996)]{GolSha-96}
...
\bibitem[Goldin et~al.(1983)Goldin, Menikoff, and Sharp]{GolMenSha-83}
...
\bibitem[Goldin et~al.(1985)Goldin, Menikoff, and Sharp]{GolMenSha-85}
...
\bibitem[{Google Quantum AI and Collaborators}(2022)]{Google-22}
...
\bibitem[Gra{\ss{}} et~al.(2020)Gra{\ss{}}, Juli{\'a}-D{\'{\i}}az, Baldelli,
  Bhattacharya, and Lewenstein]{Grass-etal-20}
T.~Gra{\ss{}}, B.~Juli{\'a}-D{\'{\i}}az, N.~Baldelli, U.~Bhattacharya, and
  M.~Lewenstein, \emph{Fractional angular momentum and anyon statistics of
...
\bibitem[Gross(1961)]{Gross-61}
E.~Gross, \emph{Structure of a quantized vortex in boson systems}, Nuovo
  Cimento \textbf{20} (1961), no.~3, 454--477.

\bibitem[Grundberg et~al.(1991)Grundberg, Hansson, Karlhede, and
  Leinaas]{Grundberg-etal-91}
...
\bibitem[Haldane(1991)]{Haldane-91}
...
\bibitem[Halperin(1984)]{Halperin-84}
...
\bibitem[Hoffmann-Ostenhof et~al.(2008)Hoffmann-Ostenhof, Hoffmann-Ostenhof,
  Laptev, and Tidblom]{HofLapTid-08}
...
\bibitem[Hu et~al.(2021)Hu, Murthy, Rao, and Jain]{Hu-etal-21}
...
\bibitem[Iengo and Lechner(1992)]{IenLec-92}
...
\bibitem[Klaiber(1968)]{Klaiber-68}
...
\bibitem[Korff et~al.(1999)Korff, Lang, and Schrader]{KorLanSch-99}
...
\bibitem[Lahtinen and Pachos(2017)]{LahPac-17}
...
\bibitem[Laidlaw and DeWitt(1971)]{LaiDeW-71}
...
\bibitem[Lambert et~al.(2023)Lambert, Lundholm, and Rougerie]{LamLunRou-22}
G.~Lambert, D.~Lundholm, and N.~Rougerie, \emph{Quantum statistics
  transmutation via magnetic flux attachment}, to appear in Prob. Math. Phys.,
  2023, \href{http://arxiv.org/abs/2201.03518}{\path{arXiv:2201.03518}}.

\bibitem[Landau(1933)]{Landau-33}
L.~Landau, \emph{{\"U}ber die {B}ewegung der {E}lektronen in {K}ristallgitter},
  Physik Z. Sowjetunion \textbf{3} (1933), 644--645.

\bibitem[Lankhorst et~al.(2018)Lankhorst, Brinkman, Hilgenkamp, Poccia, and
  Golubov]{Lankhorst-etal-18}
...
\bibitem[Larson and Lundholm(2018)]{LarLun-16}
...
\bibitem[Larson et~al.(2021)Larson, Lundholm, and Nam]{LarLunNam-19}
...
\bibitem[Laughlin(1983)]{Laughlin-83}
...
\bibitem[Laughlin(1999)]{Laughlin-99}
...
\bibitem[Lee and Oh(1994)]{LeeOh-94}
...
\bibitem[{Leinaas} and {Myrheim}(1977)]{LeiMyr-77}
...
\bibitem[Leinaas and Myrheim(1993)]{LeiMyr-93}
...
\bibitem[Leinaas and Myrheim(2022)]{LeiMyr-22}
...
\bibitem[Lerda(1992)]{Lerda-92}
A.~Lerda, \emph{{Anyons}}, Springer-Verlag, Berlin--Heidelberg, 1992.

\bibitem[Li et~al.(1992)Li, Bhaduri, and Murthy]{LiBhaMur-92}
...
\bibitem[Lieb and Liniger(1963)]{LieLin-63}
...
\bibitem[Lieb and Thirring(1976)]{LieThi-76}
...
\bibitem[Lieb and Yngvason(2001)]{LieYng-01}
...
\bibitem[Lieb et~al.(1995)Lieb, Solovej, and Yngvason]{LieSolYng-95}
...
\bibitem[Lieb et~al.(2005)Lieb, Seiringer, Solovej, and
  Yngvason]{LieSeiSolYng-05}
...
\bibitem[Lundholm(2017)]{Lundholm-16}
...
\bibitem[Lundholm(2019)]{Lundholm-17}
...
\bibitem[Lundholm and Qvarfordt(2020)]{LunQva-20}
...
\bibitem[Lundholm and Rougerie(2015)]{LunRou-15}
...
\bibitem[Lundholm and Rougerie(2016)]{LunRou-16}
...
\bibitem[Lundholm and Seiringer(2018)]{LunSei-17}
...
\bibitem[Lundholm and Solovej(2013{\natexlab{a}})]{LunSol-13a}
...
\bibitem[Lundholm and Solovej(2014)]{LunSol-14}
...
\bibitem[Lundholm et~al.(2015)Lundholm, Portmann, and Solovej]{LunPorSol-15}
...
\bibitem[Lundholm et~al.(2016)Lundholm, Nam, and Portmann]{LunNamPor-16}
...
\bibitem[Maciazek and Sawicki(2019)]{MacSaw-19}
...
\bibitem[Mancarella et~al.(2013{\natexlab{a}})Mancarella, Trombettoni, and
  Mussardo]{ManTroMus-13a}
...
\bibitem[Mancarella et~al.(2013{\natexlab{b}})Mancarella, Trombettoni, and
  Mussardo]{ManTroMus-13b}
...
\bibitem[Manuel and Tarrach(1991)]{ManTar-91}
...
\bibitem[Masaki et~al.(2023)Masaki, Mizushima, and Nitta]{MasMizNit-23}
...
\bibitem[McEuen et~al.(1992)McEuen, Foxman, Kinaret, Meirav, Kastner, Wingreen,
  and Wind]{McEuen-etal-92}
...
\bibitem[Messiah and Greenberg(1964)]{MesGre-64}
...
\bibitem[Moore and Read(1991)]{MooRea-91}
...
\bibitem[Morampudi et~al.(2017)Morampudi, Turner, Pollmann, and
  Wilczek]{MorTurPolWil-17}
...
\bibitem[Mouchet(2021)]{Mouchet-21}
...
\bibitem[Mund and Schrader(1995)]{MunSch-95}
...
\bibitem[Murthy et~al.(1991)Murthy, Law, Brack, and Bhaduri]{MurLawBraBha-91}
...
\bibitem[Murthy et~al.(1992)Murthy, Law, Bhaduri, and Date]{MurLawBhaDat-92}
...
\bibitem[Myrheim(1999)]{Myrheim-99}
J.~Myrheim, \emph{Anyons}, Topological aspects of low dimensional systems
  (A.~Comtet, T.~Jolic{\oe}ur, S.~Ouvry, and F.~David, eds.), Les Houches -
  Ecole d'Ete de Physique Theorique, vol.~69, (Springer-Verlag, Berlin,
  Germany), 1999, pp.~265--413,
  \href{http://dx.doi.org/10.1007/3-540-46637-1_4}{\path{doi}}.

\bibitem[Nakamura et~al.(2020)Nakamura, Liang, Gardner, and
  Manfra]{Nakamura-etal-20}
...
\bibitem[Nayak and Wilczek(1996)]{NayWil-96}
...
\bibitem[Nayak et~al.(2008)Nayak, Simon, Stern, Freedman, and
  Das~Sarma]{Nayak-etal-08}
...
\bibitem[Noh et~al.(2020)Noh, Schuster, Iadecola, Huang, Wang, Chen, Chamon,
  and Rechtsman]{Noh-etal-20}
...
\bibitem[Ouvry and Polychronakos(2019)]{OuvPol-19}
...
\bibitem[Pancharatnam(1956)]{Pancharatnam-56}
...
\bibitem[Polychronakos(1989)]{Polychronakos-89}
...
\bibitem[Polychronakos and Ouvry(2020)]{PolOuv-20}
...
\bibitem[Qvarfordt(2017)]{Qvarfordt-17}
...
\bibitem[Read and Rezayi(1999)]{ReaRez-99}
...
\bibitem[Regnault et~al.(2008)Regnault, Goerbig, and Jolicoeur]{RegGoeJol-08}
...
\bibitem[Rougerie and Yang(2023)]{RouYan-23b}
N.~Rougerie and Q.~Yang, \emph{Dimensional reduction for a system of {2D}
  anyons}, arXiv e-prints, 2023,
  \href{http://arxiv.org/abs/2305.06670}{\path{arXiv:2305.06670}}.

\bibitem[Rowell and Wang(2012)]{RowWan-12}
...
\bibitem[Schmidt and Lemeshko(2015)]{SchLem-15}
...
\bibitem[Schulman(1968)]{Schulman-68}
...
\bibitem[Seiringer and Solovej(2023)]{SeiSol-23}
...
\bibitem[Souriau(1970)]{Souriau-70}
...
\bibitem[Sporre et~al.(1991)Sporre, Verbaarschot, and Zahed]{SpoVerZah-91}
...
\bibitem[Sporre et~al.(1992)Sporre, Verbaarschot, and Zahed]{SpoVerZah-92}
...
\bibitem[Stormer(1999)]{Stormer-99}
...
\bibitem[Streater and Wilde(1970)]{StrWil-70}
...
\bibitem[{Sutherland}(1971)]{Sutherland-71}
...
\bibitem[Trugenberger(1992{\natexlab{a}})]{Trugenberger-92}
...
\bibitem[Tsuchiya and Kanie(1987)]{TsuKan-87}
...
\bibitem[Tsuchiya and Kanie(1988)]{TsuKan-88}
...
\bibitem[Tsui et~al.(1982)Tsui, Stormer, and Gossard]{TsuStoGos-82}
...
\bibitem[Umucal{\i}lar et~al.(2018)Umucal{\i}lar, Macaluso, Comparin, and
  Carusotto]{UmuMacComCar-18}
...
\bibitem[Verlinde(1991)]{Verlinde-91}
...
\bibitem[Weinberger(2015)]{Weinberger-15}
...
\bibitem[Wilczek(1982{\natexlab{a}})]{Wilczek-82a}
...
\bibitem[Yakaboylu and Lemeshko(2018)]{YakLem-18}
...
\bibitem[Yakaboylu et~al.(2020)Yakaboylu, Ghazaryan, Lundholm, Rougerie,
  Lemeshko, and Seiringer]{Yakaboylu-etal-19}
E.~Yakaboylu, A.~Ghazaryan, D.~Lundholm, N.~Rougerie, M.~Lemeshko, and
  R.~Seiringer, \emph{Quantum impurity model for anyons}, Phys. Rev. B
  \textbf{102} (2020), 144109,
  \href{http://dx.doi.org/10.1103/PhysRevB.102.144109}{\path{doi}}.

\bibitem[Zhang et~al.(2014)Zhang, Sreejith, Gemelke, and Jain]{ZhaSreGemJai-14}
...
\bibitem[Zhang et~al.(2015)Zhang, Sreejith, and Jain]{ZhaSreJai-15}
