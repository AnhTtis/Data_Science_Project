...
Its gauge symmetry is spontaneously broken at a high mass scale,
leading to parity violation in the weak-interaction sector of the SM\@.
...
and thus the analysis exploits their muon-like signature, making the muon trigger a natural choice
...
The addition of the missing-transverse-momentum trigger and of the `late-muon' trigger partially mitigates this issue.
...
The background expected from SM processes (largely high-\pT{} muons) is estimated by using a data-driven technique.
...
between \SI{81}{\GeV{}} and \SI{101}{\GeV{}}, corresponding to the width of the $Z$~boson mass peak.
...
\end{center}
\end{figure}
The fraction of simulated signal events satisfying the cumulative selection requirements is given
in \Tab{\ref{tab:EfficiencyCutflow}} for several benchmark points.
...
\hline
\multicolumn{2}{|c|}{Signal benchmark point} 	& Trigger 			& Candidate-track 		& Tight 			& Final 			\\
\cline{1-2}
$z$	& Mass [\si{\GeV{}}]	 		& selection [\si{\percent}]	& preselection [\si{\percent}]	& selection [\si{\percent}]	& selection [\si{\percent}]	\\
\hline
\hline
& \num{500} 	& \num{61.7+-0.2} & \num{42.7+-0.2}	& \num{42.5+-0.2} 	& \num{40.1+-0.2} \\
$2$ 	& \num{1100} 	& \num{49.9+-0.2} & \num{30.8+-0.2}	& \num{30.6+-0.2} 	& \num{29.1+-0.2} \\
...
The main reason for the search to be limited by the ${z=7}$ is that MCPs with higher charges will fail to be reconstructed as muons
in at least \SI{95}{\percent} of cases.
...
\item the signal inefficiency should be kept minimal ($\leq\SI{5}{\percent}$);
...
provided the entries inside the masked regions do not contribute to the background estimate.
...
This method provides an insight into any possible correlations between
the two variables used to construct the ABCD plane.
...
Finally, any other possible contributions from background processes are expected to be negligible and
hence not introduce correlations in the ABCD planes.
...
When considering the $\Zmm{}$ $\dEdx{}$ distributions together with those of the signal, the lower parts of
the $\dEdx{}$ ranges are the most important for determining the signal efficiency.
These correspond to the cores of the $\Zmm{}$ distributions and are the most relevant because
the signal efficiency is most sensitive to the quality of their modelling.
...
To improve the description of the trigger simulation, parameterized corrections
based on the time interval needed for MCPs to reach the RPC planes were applied to the probability
for MCPs to fire the RPC trigger.
To assess the uncertainty, the parameters of these corrections were varied.
...
(as well as signal leakages -- fractions of the signal distributions outside the D region of the ABCD plane),
