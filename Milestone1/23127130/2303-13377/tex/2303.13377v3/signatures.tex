...
\begin{equation}\label{eq:dimension}
 - 1 + \left(\sum_{l=1}^h\sum_{k|l} \frac{\mu\left(k\right)}{k}  \sum_{\alpha\in C(l/k)} \frac{1}{\ell (\alpha)} \prod_{i=1}^{\ell(\alpha)} \binom{\alpha_i + d - 1}{d - 1}\right)\,,
\end{equation}
where $\mu $ is the M\"obius function, $C(k)$ is the set of composition of the integer $k$ and $\ell(\alpha)$ is the length of each composition.
\end{thm}
...
For $V_{2,3, 4}$ it is not computationally feasible to present a  Gr\"obner basis, but we can perform the Buchberger algorithm and present some low degree terms generating the varieties that are presented in \cite{M2_MATHREPO}.
...
This allows us to create the following values of $\lambda_{d, h} $ in \cref{tbl:dimension}.  Code created to generate this table can be found in \cite{M2_MATHREPO}.
...
In what follows, we illustrate a strategy to prove Conjecture \ref{thm_chen_chow}, through which we will prove the case $h=2$. Using the results in section \ref{sec_Lie}  we can show that \cref{thm_chen_chow} is equivalent to a simpler property on the finite-dimensional vector space $\polyd^{\leq h} $, that we call \textbf{reachability}.
...
Due to the properties of applications $\Phi^{*}$ and $\log_{\otimes_h}$, the notion of reachability is equivalent to an explicit system of polynomial equations describing the image of $\Dsign^{\leq h}$.
...
\end{cases}
\end{equation}
for all  $I \in \MS_d$ of weight at most  $h$ and all Lyndon words  $J $ of weight at most  $h$ such that $|J|\geq 2$ with $b(J)$ the Lyndon bracket associated to it. We call the equations in \eqref{eq_reach_eq} the \textbf{reachability equations}. 
\end{lm}
\begin{proof}
Looking at the diagram  \ref{cd:level_h_lie}, we observe that $\log_{\otimes_h}\circ \,\Phi^{*}_H \Dsign^{\leq h}(x) = \vv$ if and only if $\langle \Phi^{*}_H \log_{\otimes_h}\Dsign(x), \bv\rangle =\langle \vv, \bv \rangle $ for all $\bv$ running over a basis of $\mathcal L^{\leq h}(\polyd)$. Thanks to Lemma \ref{dim_lie} one has
\[ \langle \Phi^{*}_H \log_{\otimes_h}\Dsign(x), b(K)\rangle =\langle \vv, b(K) \rangle\]  
for all Lyndon words  $K $ of weight at most  $h$. By using \cref{lm:adjoint_log} the left-hand side in the equation above becomes
\begin{align*}
...
Then, we show that for any time series $x$, the time series $ g(x) = x |\overleftarrow{x} $  satisfies the equations of weight two, while having an amenable level one result, that is:
\[
\begin{cases}
...
\begin{rem}
We note that the dimension-bound argument presented in the proof above works for any fixed weight, and displays the intuition that given a large enough time series size, a simple dimension argument allows us to construct a solution. However, to solve the equations \eqref{level_2eq} we could not follow from a dimension argument due to the presence of the symmetric constraint $\langle \Dsign (x), [i][j]-[j] [i]\rangle=0$. When $h>2$ the complexity of the rechability equation \eqref{eq_reach_eq} becomes even more challenging without even a clear expression in any order. For example in case $h=3$ writing any generic element $\vv\in \polyd^{\leq 3}$ as the sum
...
\end{cases}
\]
For which we are not able to provide the existence of a solution.
...
\begin{itemize}
\item  What is the role of the length  between the parameters $N$ and $h$  in $\mathcal{V}_{d,h, N}$ when we consider the identifiability of a time series?  Can we expect results similar to \cite[Section 6]{amendola2019varieties} or \cite{seigal19}?
...
CB is supported by the ERC starting grant ``LoRDeT"; he also acknowledges the support of the TU Berlin, where he was employed when this project was started. RP was supported by the Max Planck Institute for the mathematics in the sciences for part of this project. Both authors would like to thank fruitful conversations with Sylvie Paycha and Bernd Sturmfels. We would also like to thank  \'Angel R\'ios Ortiz, Pierpaola Santarsiero and Nikolas Tapia for all the suggestions and comments on discrete signatures, tensor algebras and algebraic varieties. The authors are thankful for the helpful comments of the reviewers.
