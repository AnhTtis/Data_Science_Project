...
\end{equation}
and can be interpreted as a $\fZ$-graded version of  Quillen supeconnection in $\cN$, seen as a bundle over $X$. Superconnections were introduced in \cite{Quillen:1985vya} and turned out to be useful in the QFT context, see e.g.~\cite{Neeman:1990tfh,Roepstorff:1998vh}.   The linear in $\theta^\mu$ piece of $l_1$ reads as
\begin{equation}
    \nabla  \alpha =(\dx \alpha^A) e_A-(-1)^{|e_A|}\theta^{\mu}q^{C}_{\mu A}\alpha^A e_C
\end{equation}
and can be interpreted as a usual linear connection in $\cN$, seen as a vector bundle over $X$. 
...
\begin{equation}\label{hb1}
    P[v,w]=P[Pv,w]+P[v,Pw],
\end{equation}
for all $v,w\in V$. The identity \eqref{hb1} means that the kernel of $P$ is also a subalgebra in $V$. Let $\Delta \in V$  be an element satisfying $\frac{1}{2}[\Delta,\Delta]\equiv \Delta^2=0$ and $\Delta\in \ker{P}$. Then $U$ is naturally an $L_{\infty}$-algebra, whose $n$-ary map is given by the \textit{$n$-th derived bracket formula} \footnote{The formula \eqref{hb2} differs from the one in \cite{Voronov:2005} by prefactor $(-1)^{n+1}$ because we want to be in agreement with the notations of $L_{\infty}$-algebra, introduced above.}:
\begin{equation}\label{hb2}    l_n(v_1,\dots,v_n)=(-1)^{n+1}P[\dots[[\Delta,v_1],v_2],\dots,v_n],
\end{equation}
...
\begin{equation}
    l_2( \alpha,\beta)=(-1)^{|\alpha|+|\beta|+|e_A||\beta|+1} \alpha^A\beta^Bq^C_{AB}(x,\theta)e_C  \,,\quad \alpha=\alpha^A(x,\theta)e_A\quad \beta=\beta^A(x,\theta)e_A \, ,
\end{equation}
...
A clear disadvantage of the above construction is that $L_\infty$ structure generally depends on the choice of trivialization and hence is in general defined only locally. It should be possible to develop a globally well-defined version by allowing $\nabla$ to be curved. This should extend the discussion of Appendix~\bref{app:Fedosov} to the case of $Q$-bundles. In this context it is also worth mentioning the related developments of \cite{Mehta_2015,Seol:2021tol, Arvanitakis:2022npu,Chatzistavrakidis:2023otk}.
...
\begin{equation}\label{l31}		Q=\theta^{\mu}\left(\frac{\partial}{\partial x^{\mu}}-q^{a}_{\mu b}(x)c^{b}\frac{\partial}{\partial c^{a}}\right)-\frac{1}{2}U^{a}_{bc}c^{b}c^{c}\frac{\partial}{\partial c^{a}},
	\end{equation}
...
\end{equation}
where for simplicity we assumed $\p{\psi^A}=0$. In particular, solutions of the linearized equation are the degree $0$ section $v=v^A(x)e_A$ that are covariantly constant:
\begin{equation}
...
\begin{gathered}
    ...
    Q(\textbf{F})=-\frac{1}{\hbar}[\textbf{A},\textbf{F}]_*-p^a\frac{\partial}{\partial y^a}\textbf{A}-\theta^{\mu}\left(e^a_{\mu}\frac{\partial}{\partial y^a}+\omega^a_{\mu b}y^b\frac{\partial}{\partial y^a}-\omega^a_{\mu b}p_a\frac{\partial}{\partial p_b}\right)\textbf{F}\,, \qquad Q(x^{\mu})=\theta^{\mu}\,.
\end{gathered}
\end{equation}
...
\begin{equation}\label{hs11}
 ...
 \qquad l_2(\Phi,\Phi)=\frac{(-1)^{\p{\Phi}}}{2\hbar}\qcommut{\Phi}{\Phi}\,,
\end{equation}
...
l_2(\alpha e_1,\gamma e_1)=-\commut{\alpha}{\gamma} e_1\,,\qquad
l_2(\beta_{\mu\nu}e^{\mu\nu}_0,\alpha e_1)= 2\commut{\beta_{\mu\nu}}{\alpha} e_0^{\mu\nu}\,,\qquad
...
The standard fact is that there exists a flat nonlinear connection $D$ in $W$ of the following form
\begin{equation}
D=\nabla+\pb{r}{\cdot}\,, \qquad D(Df)=0\,\quad \forall f\in\Gamma(W)
\end{equation}
where $\nabla$ is an extension of $\Gamma$ to $W$ and $r$ is a $W$-valued  $1$-form on $M$ which is linear in $p_A$ (in fact it is enough to consider a bundle whose fiber is a Lie algebra of formal vector fields on the tangent space). Any section $f_0$ of $S(TM)$ has a unique extension to $f\in\Gamma(W)$ such that $DF=0$ and $f|_{Y=0}=f_0$ and moreover
\begin{equation}
\pb{f}{g}|_{Y=0}=\pb{f_0}{g_0}_{T^*M},
\end{equation}
where $\pb{-}{-}_{T^*M}$ is a standard Poisson bracket on $S(TM)$. Moreover, $f$ has the same homogeneity in $p_A$ as $f_0$. Details can be found in~\cite{}. Note that $D\pb{f}{g}=0$ because both $\nabla$ and $\pb{r}{-}$ differentiate the fiberwise Poisson bracket.

Let $V,W$ are two  vector fields on $M$  such that $\commut{V}{W}=0$. Seen as sections
of $S(TM)$ they are simply functions $v_0,w_0$ linear in $P_A$. Their unique covariantly constant lifts $v,w$ satisfy $\pb{v}{w}=0$ because $D\pb{v}{w}=0$ and $\pb{v_0}{w_0}_{T^*M}$ by the assumption. If $M$ is a $Q$-manifold, we apply this to $V=W=Q$ 
and find that $\pb{q}{q}=0$. At the same time $q$This is equivalent to having a formal homological vector field on each fiber of $TM$. It gives a curved $L_\infty$ structure on $\Gamma(TM)$.

One can equivalently equip $W(M)$ with a fiberwise Weyl star-product determined by the above Poisson bracket. In this case the construction is version of Fedosov quantization for cotangent bundles~\cite{} and gives a deformation quantisation of $S(TM)$.
\fi
...
\bibitem{Krotov:2006th}
D.~Krotov and A.~Losev, ``{Quantum field theory as effective BV theory from
  Chern-Simons},''
  \href{http://dx.doi.org/10.1016/j.nuclphysb.2008.07.021}{{\em Nucl. Phys. B}
  {\bfseries 806} (2009) 529--566},
  \href{http://arxiv.org/abs/hep-th/0603201}{{\ttfamily arXiv:hep-th/0603201}}.

\bibitem{Losev2007FROMBI}
A.~S. Losev, ``From berezin integral to batalin-vilkovisky formalism: A
  mathematical physicist's point of view,''
\newblock 2007.
\newblock In Felix Berezin: Life and Death of the Mastermind of
  Supermathematics, pp. 3-30. 2007. Available at
  http://homepages.spa.umn.edu/~shifman/Berezin/Losev/Losev.pdf.
...
P.~Mnev, ``{Discrete BF theory},''
  \href{http://arxiv.org/abs/0809.1160}{{\ttfamily arXiv:0809.1160 [hep-th]}}.
...
\bibitem{Mnev:2006ch}
P.~Mnev, ``{Notes on simplicial BF theory},''
  \href{http://arxiv.org/abs/hep-th/0610326}{{\ttfamily arXiv:hep-th/0610326}}.

\bibitem{Cattaneo:2009deo}
A.~S. Cattaneo and P.~Mnev, ``{Remarks on Chern-Simons invariants},''
  \href{http://dx.doi.org/10.1007/s00220-009-0959-1}{{\em Commun. Math. Phys.}
  {\bfseries 293} (2010) 803--836},
  \href{http://arxiv.org/abs/0811.2045}{{\ttfamily arXiv:0811.2045 [math.QA]}}.
...
\bibitem{Quillen:1985vya}
D.~Quillen, ``{Superconnections and the Chern character},''
  \href{http://dx.doi.org/10.1016/0040-9383(85)90047-3}{{\em Topology}
  {\bfseries 24} no.~1, (1985) 89--95}.

\bibitem{Neeman:1990tfh}
Y.~Ne'eman and S.~Sternberg, ``{Superconnections and internal supersymmetry
  dynamics},'' \href{http://dx.doi.org/10.1073/pnas.87.20.7875}{{\em Proc. Nat.
  Acad. Sci.} {\bfseries 87} (1990) 7875--7877}.

\bibitem{Roepstorff:1998vh}
G.~Roepstorff, ``{Superconnections and the Higgs field},''
  \href{http://dx.doi.org/10.1063/1.532724}{{\em J. Math. Phys.} {\bfseries 40}
  (1999) 2698--2715}, \href{http://arxiv.org/abs/hep-th/9801040}{{\ttfamily
...
\bibitem{Arvanitakis:2022npu}
A.~S. Arvanitakis, ``{Formal exponentials and linearisations of
  QP-manifolds},'' \href{http://arxiv.org/abs/2204.12613}{{\ttfamily
  arXiv:2204.12613 [math-ph]}}.
