...
Additionally, badges are generally poorly correlated with developers' qualities and dispositions such as timeliness and desire to collaborate.
...
\textit{\textbf{Conclusions}.} We provide recommendations to the designers of the \gh\ platform on how to improve the current implementation of personal badges as both a gamification mechanism and as sources of reliable cues for the assessment of developers' abilities.
...
Previous research has also found evidence that gamification mechanisms are in part responsible for the success of websites such as \textsc{Stack Overflow}~(\so)~\citep{vasilescu13}.
...
\gh\ not only has earned the reputation of being the one-stop-shop for software development~\citep{metz2015}, but has also quickly become a useful platform for job seekers~\citep{superhire-blog,hacker-news-post,anti-pattern-github-resume}.
Employers access the \gh\ profile pages of job candidates to supplement resumes and further assess their knowledge, skills, and abilities~\citep{marlow2013activity}.
...
We characterize the distribution of badges in \gh\ (RQ$_1$), investigate the qualities of developers they are intended to signal and how well they correlate with these qualities (RQ$_2$), and examine the effects, if any, that the introduction of this feature has had on developers' activities (RQ$_3$).
...
\gh\ has recently implemented personal badges to reward developers for  achievements unlocked through their activities on the coding platform.
...
On \gh, neither the list of achievements nor the rules for obtaining them are publicly disclosed.
...
\textit{RQ$_3$ -- Has the introduction of personal badges affected developers' activities?}\\
...
Data were collected from a sample of organizations used in the preparation of the follow-up study on core developers who abandon \gh\ projects~\citep{Calefato2022}.
...
For each them, we used the \gh\ API to retrieve the list of organization members.
The API returned an error when retrieving the members of seven organizations; hence, we ended up with 6,022 developers from 51 organizations. 
Then, for each organization, we selected the `reference project' (e.g., \GHicon{rails/rails}); %, \GHicon{nodejs/node}); 
when unclear (e.g., \GHicon{laravell/framework}), we chose the largest in terms of contributors and, if more projects had a similar number of contributors, we chose the one with most stars. 
...
A copy of the survey is available as supplementary material in the replication package.
...
Also, we plot the monthly progression of the number of unlocked badges.
...
\begin{table*}[h!]
%\vspace{-0.1cm}
\caption{Categories of personal achievement badges unlocked as of December 2022 and their distribution in our dataset. The gray-hued badges are excluded from the empirical analysis.}
...
This result is `inflated' because it is among the badges that already existed before the launch of the personal badges in June 2022; besides, this badge is not earnable anymore as it was awarded to anyone contributing code to \gh\ in 2020. 
...
We also discuss the bad coding practice promoted by the \texttt{YOLO} badge during the analysis of survey responses in Section~\ref{sec:res-rq4:survey}.
...
However, for question Q$_{14}$, we received 31 responses.
...
Regarding demographics (Q$_{1}$-Q$_{4}$), participants in the survey reported having on average 9 years of experience (min. 1, max. 31). 
...
Finally, a couple of additional questions with this code asked how to suggest new types of badge, showing that some users are willing to participate in the definition of future personal achievements.
...
Then (RQ$_2$), we investigated whether the ownership of badges is associated with signaling certain hypothesized developers' qualities and dispositions. 
...
However, previous research found that, although  conscious, only few \gh\ developers engaged in efforts to clean up their profile page because deemed too costly~\citep{marlow2013activity}. 
Our findings show that \gh\ developers may have grown  more aware over time and that the decision to allow them to easily opt out of displaying personal badges is a step in the right direction. 
Nevertheless, \gh\ platform designers should also consider implementing the ability to selectively hide/show badges on user profile pages -- a feature requested by several survey participants; this might alleviate the discontent caused by badges like \texttt{YOLO} and \texttt{Quickdraw}, which mark and publicly expose undesirable and unprofessional user behaviors. 
