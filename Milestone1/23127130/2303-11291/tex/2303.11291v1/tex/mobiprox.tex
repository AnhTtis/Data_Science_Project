...
An overview of Mobiprox building blocks is illustrated in Figure~\ref{fig:amc-pipeline}.  Android app compiled through Mobiprox is empowered to use an arbitrary runtime approximation adaptation strategy (e.g. ``run low quality network when the battery is low'', ``execute high-quality inference when a user is at a particular location'', etc.) for its deep learning models. To achieve this, for a given pre-trained neural network, Mobiprox, relying on ApproxTuner~\cite{sharif2021approxtuner}, first investigates the effect of different per NN-operation approximations on the inference accuracy and identifies speedup-inference accuracy trade-off points. Each of these points is \blue{realized through} a different set of approximations (and is termed: configuration) applied to layers of a neural network. The most promising candidate configurations are then funneled to a novel profiler built within Mobiprox that assesses the true performance of approximated NNs on the mobile. Mobiprox Android runtime then enables the execution of, and dynamic switching among, approximated operations on a mobile device. Through the framework-generated JNI interface library, the execution control can be done via the mobile app itself. Finally, within Mobiprox we also devise new approximation adaptation strategies that are designed to leverage the generated trade-off curves to match the required and the delivered quality of computation, in order to enable energy-efficient DL on mobile devices. 
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/system-diagram.png}
    \caption{Mobiprox overview. The original modules built in this work are marked with green.}
...
Each of the approximation techniques described in Section~\ref{sec:preliminaries-acts} exposes one or more \textbf{approximation knobs} that can change the degree of approximation and thus adjust the accuracy and the execution time (consequently the energy efficiency) of a tensor operation. These knobs are \ttit{offset} and \ttit{stride} for convolution perforation and filter sampling as well as an indicator \texttt{\_fp16} of whether an operation is executed using half-precision quantization. An \textbf{approximation configuration} is a set of pairs $\langle \mathrm{Op.}, \mathrm{KnobValue} \rangle$ for all operations in a given NN. Each of the configurations leads to a single \textbf{trade-off point} on an speedup-accuracy trade-off curve. 
...
However, the above method of finding the optimal approximation configurations does not readily translate to mobile devices. %First, 
The mobile platform is substantially different than the server environment for which ApproxTuner was initially developed. The intrinsics of GPU-based execution (e.g. CUDA vs OpenCL), heterogeneous CPUs with fewer cores, and other factors render the results of the approximation configuration search, which is conduced using ApproxTuner on a server, a rather poor representation of the actual approximated NN performance on a mobile. In Figure~\ref{fig:combined-pareto-android}   we show the actual on-device speedup and QoS loss achieved by the configurations that ApproxTuner identified as the most promising (Figure~\ref{fig:combined-pareto}) for a MobileNetV2 model used for HAR from smartphone accelerometer samples (detailed in Section~\ref{sec:methodology}). While in general the Pareto points identified by ApproxTuner remain relevant, the achieved on-device speedup is about 50\% lower than that inferred by ApproxTuner.
...
Mobiprox therefore introduces a novel configuration identification approach. First, %observing that it fails to predict the performance of approximations applied to neural networks that do not deal with computer vision tasks, we use only empirical tuning 
we run the tuning on a computing cluster to identify candidate approximation configurations. Then, we develop an Android-based profiler (described in Section~\ref{sec:hpvm-profiler-android}), that runs each candidate configuration on a mobile device and obtains the realistic picture of the approximated neural network performance. The resulting picture of the speedup -- QoS loss space charted by these configurations is then used to guide the dynamic approximation adaptation. As an end result, the profiler produces a file listing configurations that will be switched during runtime, yet \textit{only a single original network model is deployed on a mobile}.
...
Mobiprox introduces significant extensions to the ApproxHPVM compiler infrastructure to support compilation and tuning of Android applications. The development of mobile apps using Mobiprox involves the compilation of the tuning binary and the inference binary (Figure~\ref{fig:amc-pipeline}). While the tuning binary remains confined to the server environment and is handled by the ApproxHPVM compilation pipeline, the inference binary is cross-compiled from a server to a mobile (Android). 

We implement a mechanism for turn-taking interplay of the ApproxHPVM and Android NDK LLVM compiler toolchains (Algorithm~\ref{alg:llvm-compile}). We enabled this by clearly partitioning the compilation steps, harnessing the fact that LLVM-based compilers apply transformations to an intermediate representation termed LLVM-IR. Note that HPVM and ApproxHPVM extend LLVM-IR by defining HPVM-IR upon which approximation-related transformations are applied. This clear partitioning allows us to use Android NDK for generating the initial LLVM-IR suitable for Android applications and for generating the machine code containing approximate NN operations suitable for mobile GPUs at the final compilation step, while using ApproxHPVM's HPVM-IR transformations for the internal part of the compilation pipeline in order to insert the description of the desired approximate tensor operations.
...
\subsubsection{HPVM Tensor Runtime for Android}
\label{sec:android-rt}

A core component of Mobiprox Android implementation is HPVM Tensor Runtime, which implements tuneable approximable tensor operations for neural network inference. The existing support for approximate neural network operations for Nvidia CUDA GPUs is not suitable for mobiles, which seldom host such hardware. Instead, Mobiprox implements an own Android-compatible tensor runtime using OpenCL, an open standard for GPU-accelerated computation which is available on a wide variety of hardware, including mobile platforms.
...
To enable an enhanced control over low-level concepts (such as memory allocation), we implemented the HPVM Tensor Runtime for Android using CLBlast~\cite{clblast}, an OpenCL implementation of basic linear algebra subprograms (BLAS). However, this library is not intended for deep learning: it does not implement operations commonly used in neural networks. Therefore we extended CLBlast with the following operators: \textit{i)} Point-wise tensor addition, \textit{ii)} Bias addition, \textit{iii)} Activation functions (ReLU, clipped ReLU, $tanh$), \textit{iv)} FP-16 -- FP-32 tensor conversion, \textit{v)} Batch normalisation, \textit{vi)} Pooling ($min$, $max$, $average$), \textit{vii)} Convolution approximations operators optimized with tiling and vectorisation: \textit{Image-to-Column} ($im2col$) transformations with row perforation, column perforation, and filter sampling, \textit{Kernel-to-Row} ($kn2row$) transformation with filter sampling, and \textit{Interpolation} of missing values in convolution perforation. % \textit{viii}, \textit{ix}, \textit{x} 
...
\blue{Finally,} during the mobile app compilation Java Native Interface (JNI) is exposed, enabling HPVM Tensor Runtime initialization and destruction, NN inference invocation, and dynamic approximation configuration loading.

\subsubsection{HPVM Profiler for Android}
\label{sec:hpvm-profiler-android}

To measure the speedups and consequently the energy efficiency of approximated neural networks we implement a profiler tool. The profiler is implemented as a Python library that enables selecting an inference binary (i.e. the compiled network) for profiling and recording the profiling information (accuracy, softmax confidence, execution time) while executing NN inference on a test data set. Due to a high discrepancy between the speedup observed on a mobile device and on a server for the same approximated network (Figure~\ref{fig:pareto}), the profiler uses the Android Debug Bridge (ADB)~\cite{adb} to run measurements on an actual Android device and to transfer the profiling information files back to the host machine for analysis.
