...
Euclidean inner product on $\R^{d}$ and $\|\cdot\|$ represents $\ell_{2}$
norm. $\mathrm{int}(A)$ stands for the interior points of any set
$A\subseteq\R^{d}$. Given a closed and convex set $C$, $\Pi_{C}(\cdot)$
is the projection operator onto $C$, i.e., $\Pi_{C}(x)=\argmin_{z\in C}\|z-x\|$.
...
mention that the reason for using $\ell_{2}$ norm is only for convenience.
When considering a general norm, similar results to our theorems still
hold after changing the algorithmic framework into mirror descent.
A more detailed discussion will be given in Section \ref{sec:general-norm}
in the appendix.
