...
time. Besides, the dependence on the failure probability $\delta$
is $O(\log^{2}(\log T/\delta))$ when $T$ is unknown. It will be
further improved to $O(\sqrt{\log(\log(T/\delta))\log(1/\delta)})$
for the case of known $T$. It is worth emphasizing that our analysis
neither makes any compact assumption on the constrained set $\dom$
nor requires the knowledge of the distance between $x_{1}$ and $x_{*}$
where $x_{1}$ and $x_{*}$ are the initial point and a local minimizer
on $\dom$ respectively.
...
\begin{itemize}
\item We give the first high-probability convergence analysis and show a
nearly optimal convergence rate $O(\log^{2}(T/\delta)T^{\frac{2(1-p)}{p}})$
with probability at least $1-\delta$, which matches the lower bound
$\Omega(T^{\frac{2(1-p)}{p}})$ up to logarithmic factors. The same
as the convex case, the choices of $M_{t}$ and $\eta_{t}$ don't
require any prior knowledge of the time horizon $T$ and the noise
level $\sigma$, either.
\item With the same $M_{t}$ and $\eta_{t}$ used for high-probability bounds,
our algorithm also guarantees a rate of $O(G^{2}T^{-1}+(\sigma^{p}G^{2-p}+\sigma^{2p}G^{2-2p})T^{\frac{2(1-p)}{p}})$
in expectation. It is worth mentioning that, unlike the convex case,
such an $\sigma$-adaptive rate doesn't need $\sigma$ to set $M_{t}$
and $\eta_{t}$.
\end{itemize}
\end{itemize}
...
estimate when $\dom$ is unbounded.
...
\cite{nemirovskij1983problem,raginsky2009information,vural2022mirror}
show that the convergence rate of any first-order algorithm can be
not faster than $\Omega(T^{\frac{1-p}{p}})$. If the strong convexity
