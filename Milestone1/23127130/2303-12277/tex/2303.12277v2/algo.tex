...
%
\begin{rem}
It is possible to choose $\eta_{t}=\frac{\alpha_{1}}{G\sqrt{t}}\land\frac{\alpha_{2}}{M_{t}}$
...
Additionally, the choice of $\alpha=\frac{\beta}{\log(4/\delta)}$
is only for optimizing the dependence on $\log(1/\delta)$. Our theoretical
analysis works for any $\alpha>0.$
\end{rem}
%
...
\ref{subsec:Two-lemmas}, two of the most fundamental lemmas used in the
proof are presented. In Sections \ref{subsec:lip-prob} and \ref{subsec:lip-exp},
...
Now we provide the desired bound described above in Lemma \ref{lem:lip-prob-loglog},
...
\ref{lem:lip-prob-loglog}.
\begin{lem}
\label{lem:lip-prob-loglog}When $\mu=0$, under the choices of $M_{t}=2G\lor Mt^{\frac{1}{p}}$
and $\eta_{t}=\frac{\alpha}{G\sqrt{t}}\land\frac{\alpha}{M_{t}}$,
...
the term $\log(eT)$ in Lemma \ref{lem:lip-prob-loglog} will be removed
...
By Lemma \ref{lem:lip-prob-loglog}. we have with probability at least
