...
\begin{itemize}
    \item We propose Q-GAT for the task of GRNs, which is the pioneer in addressing the stability issue of constructing GRNs. Instead of a modification for network structures, our innovation is at the neuronal level from the perspective of neuronal diversity.
...
\underline{Graph neural networks.} Deep graph convolutional neural network (DGCNN) uses a SortPooling layer to sort the graph's vertices. The SortPooling layer serves as a link between the graph convolution layer and the fully-connected layer. By recording the sorting order of the input and returning the gradient to the previous layer, the SortPooling layer not only enables the training of the previous layer but also helps learning from the topology of the global graph \cite{zhang_end--end_nodate}. Differentiable pooling (DiffPool) employs a differentiable graph pooling module to provide hierarchical graph representations, which can be integrated with diverse GNN structures \cite{ying_hierarchical_nodate}. Graph multiset transformer (GMT) utilizes a global pooling layer based on multi-headed attention to capture interactions between nodes and their structural dependencies \cite{velickovic_graph_2018,baek_accurate_2021}. PiNet uses a generalized differentiable attention-based pooling technique combined with graph convolution operations for graph-level classification \cite{meltzer_pinet_2020}. Autoregressive moving average filters (ARMAs) use a graph convolution layer that not only is more flexible and noise-resistant but also can retain the global graph structure better than polynomials \cite{bianchi_graph_2022}. Deep graph convolution network (DeepGCN) uses the residual/dense connectivity and dilated convolution to train very deep GNNs \cite{li_deepgcns_2019}. Self-attention graph pooling (SAGPool) uses a self-attention graph pooling technique that considers both node attributes and graph topology \cite{lee_self-attention_2019}. Top-k pooling (TopKPool) employs a new graph pooling method suggested by graph U-Nets, which picks nodes to construct smaller graphs based on scalar projection values \cite{gao_graph_2021}. Graph sampling and aggregation (GraphSAGE) are a generalized induction framework that employs node feature information to construct the embeddings for previously-unknown data in an efficient manner \cite{hamilton_inductive_nodate}. 

Like other kinds of deep learning models, GNNs are often easily fooled by small perturbations. Thus, when applying GNNs to constructing GRNs, it is hard to tell the correctness of the inferred relationship due to the noise that emerged in biological measurements. Given the criticality of GRNs, it is highly necessary to solve the instability issue in constructing GRNs.
...
Denote a collection of transcriptional factors (TFs) as $T$, a collection of target genes as $G$, and their gene expression data as $R_{i,j}$, where $i\in \{T, G \}$ and $j\in [n]$ for every $T$ and $G$ with $n$ arrays. Our task is to obtain a GRN that is essentially an $\mathbb{R}^{T\times(T+G)}$ matrix $A_{T,T+G}$, where $T$ and $G$ are both nodes in the graph, and $E$ denotes the edges representing regulatory relationships between transcriptional factors and genes and between transcriptional factors themselves. Notably, edges between a pair of genes are prohibited because no such mechanisms are discovered so far. If $\left (i,j \right) \in E$, $A_{i,j}=1$; otherwise, $A_{i,j}=0$. 
...
% 		%	\addtocounter{figure}{+1}
%     \centering
%     \includegraphics[width=3in]{figs/graph_v2.pdf} 
%     \caption{Extraction of subgraphs and a flow chart for the training test.\textcolor[rgb]{0,0,1}{Diagram of Pearson correlation network's subgraph extraction. The graph neural network is trained using these subgraphs to produce the classifier.The classifier was used for link prediction by graph classification.}} \label{fig1}
%     \end{figure}
...
Wang \textsl{et al.} \cite{wang_inductive_2020} showed that adding more hops does not produce significantly better results in a GRN.
...
We exclude the bias terms $\boldsymbol{w}_2 b_1+\boldsymbol{w}_1b_2$ because they are always the same for different $\x$; therefore, they cannot work as importance scores. 
...
\begin{table}[htbp]
	\centering
	\caption{The statistics of the used datasets.}\label{tbl1}
 \scalebox{0.85}{
	\begin{tabular}{lcccc}
		\toprule
		Species   & TFs & Target Genes &  Links & Samples\\
		\midrule
		\emph{E. coli}         & 344 & 4177    & 2066  & 805     \\ 
		\emph{S. cerevisiae}  & 333 & 5617    & 3940  & 536     \\		
		\bottomrule
	\end{tabular}}
\end{table}
...
Although the focus of this draft is the stability, the predictive performance should be also cared for the usefulness of the model. To evaluate both the stability and predicative performance of different models in constructing GRNs, we propose a new and a more balanced evaluation metric as follows:
	\begin{equation}
		Cscore=\frac{Mean}{Bias} \label{eq3}
	\end{equation}
    in which 		
	\begin{equation}
		Bias=\sqrt{\frac{ {\textstyle \sum_{i=1}^{N}} {{(I_{i}-\overline{I}})}^{2}}{N-1}}, \label{eq2}
	\end{equation}
	where $N$ is the number of repeated experiments, $\overline{I}$ is the accuracy for the perturbation-free dataset, and $I_{i}$ denotes the accuracy for the $i$-th perturbed dataset. A larger mean and a smaller bias lead to a larger Cscore. Note that one can use the variability (mean divided by standard deviation).
...
% To comprehensively assess the advantages of Q-GAT, we compare the prediction results of Q-GAT and GAT in \emph{E. coli} datasets at different perturbation levels. \Cref{figc} shows that Q-GAT achieves better prediction results under different disturbance levels, and the prediction results of GAT show a significant downward trend with the increase of noise level. 

% \begin{figure}[htbp]  %bias
% 		%	\addtocounter{figure}{+1}
%     \centering
%     \includegraphics[width=0.3\textwidth]{pics/GAT_Q-GAT_comparison1.pdf} 
%     \caption{the prediction results of GAT and Q-GAT in the \emph{E. coli} datasets at different perturbation levels} \label{figc}
%     \end{figure}
