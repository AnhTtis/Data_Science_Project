...
However, it was noted in \cite{darbon2016algorithms,chow2017algorithm} that any linear time-varying system can be mapped to a state-independent system 
\beqn
\begin{aligned}
...
\begin{figure*}[t]
    ...
    \caption{The four controlled evolutions of the 10D glycolysis model with the same random disturbance trajectory and initial condition. `Auto' signifies the controller which is disturbed but always chooses the trivial input. It is possible to see from the example trajectory the behavior which allows Hopf to succeed; it amplifies the cycle of the phosphate, glucose and ATP trajectories to achieve the ATP target.}
...
Note, in the non-convex case we lose guaranteed convergence to the global optimum, which may explain why the Jacard index is similar between the Koopman and Taylor series results.% the equal $JI$ with the Taylor series and worse performance compared with the convex case, but overall, the similarity is decent.
...
We thank Steven Brunton, Stanley Bak and Masih Haseli for discussions about Koopman spaces and control in them, Gary Hewer and Matthew Kirchner for discussions about applying Hopf Reachability to Naval applications, and Somil Bansal for thought-provoking discussions that bettered the work. Finally, we thank Zheng Gong and Sander Tonkens for valuable feedback on the paper.
