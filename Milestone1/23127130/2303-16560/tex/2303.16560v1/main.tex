...
%--------------------------------------------------------------------
\title{The Mott transition in the 5d$^1$ compound Ba$_2$NaOsO$_6:$ a DFT+DMFT study with PAW non-collinear projectors}
%--------------------------------------------------------------------
...
Our calculations on larger supercell structures with different magnetic configurations and local structural environments show that the canted antiferromagnetic (cAFM) ground state provides both metallic and insulating solutions depending on whether distortions of the octahedra are considered, regardless on the inclusion of SOC (see Fig.~\ref{fig:0}). This result indicates that a gap in BNOO can be open without invoking  SOC effects (more details are reported in the Supplementary information). 
However, the onset of the magnetic ordering and local symmetry breaking  occurs at T$_N \approx 6.8$\,K and T$~\approx 10$\,K~\cite{Erickson2007, lu2017magnetism, PhysRevB.100.041108}, respectively, well below the experimental observation of the insulating phase to justify their impact on the gapped spectra. 
...
\caption{The density of states of BNOO in an 8 f.u. supercell without SOC is shown for the cases of fully ferromagnetic solution with magnetic moments along [110] (top panel), the canted antiferromagnetic phase of Ref.~\cite{mosca2021interplay} without distortions (middle panel), and the JT-distorted antiferromagnetic structure (bottom panel). For the computational details see the supplementary materials.
}
\label{fig:0} 
\end{figure}
%-------------------------------------------------------------------------
In this work, we are interested in understanding the insulating nature of BNOO and in deciphering the role of SOC on its MIT. To do so, we employ DFT plus dynamical mean-field theory (DFT+DMFT) calculations at room temperature in the paramagnetic regime, well above the magnetic phase transition. To account for SOC effects from VASP bandstructure calculations~\cite{Kresse_1994,PhysRevB.54.11169, PhysRevB.47.558}, we implemented an extension of the plovasp converter included in the TRIQS/DFTTools~\cite{Parcollet_2015_PAPER,Aichhorn2016,triqs_wien2k_interface} package that encompasses non-collinear PAW projectors. We tested its accuracy by comparing our results with the equivalent linearized augumented plane wave (LAPW) projectors of Wien2k, showing that VASP non-collinear projectors allow treating, fully ab initio, SOC effect in DFT+DMFT calculations. Furthermore, this implementation also provides the opportunity to explore magnetic non-collinear phases within DFT+DMFT.

Moving back to BNOO, we show that SOC is not essential for opening the gap, as its insulating phase is equivalently reproduced without relativistic corrections for both the experimental value of interaction $U$ and the one computed within constrained Random Phase Approximation (cRPA)~\cite{PhysRevB.70.195104}, making this material a Mott insulator. We find that SOC mainly affects the nature of its atomic states, described by $J_\text{eff}$ states, which determines the observation of the exotic magnetic ground state~\cite{mosca2021interplay},  as well as the appearance of an additional structure in the upper Hubbard band (UHB). 



The paper is structured as follows: Section \href{Sec:2a}{II A} summarizes the non-collinear implementation of SOC in VASP, the non-collinear extension of the PAW projectors, and DMFT equations are in Sec. \href{Sec:2b}{II B} and \href{Sec:2c}{II C} respectively. Section \href{Sec:3}{III} describes the computational procedure for the DFT+DMFT calculations. In Sec. \href{Sec:4a}{IV A} we compare the treatment of the correlated subspace with Wien2k~\cite{Blaha_2020_PAPER}; in Sec. \href{Sec:4b}{IV B} we present the electronic properties of BNOO and discuss its MIT. Section \href{Sec:5}{V} offers some final remarks. 
...
\begin{equation}
    | \Tilde{\Psi}^{\alpha}_n \rangle = \sum_{\alpha \beta} \Tilde{H}^{\alpha\beta}_{SOC} | \Tilde{\Psi}^{\beta}_{n} \rangle \ ,
\end{equation}
 and 
\begin{equation}
...
\subsection{B. Non-collinear projected localized orbitals}
\label{Sec:2b}
...
In this section, we present our results for the correlated subspace properties and illustrate how the non-collinear PAW projectors compare with the corresponding projectors calculated from Wien2k. The comparison was achieved via computation of the local Green's function $G_\text{loc}(\omega)$, the density of states (DOS) and local impurity Bloch Hamiltonian after convergence of the DFT self-consistent cycle.
...
In order to investigate the nature of BNOO and the influence of SOC, we employed a series of calculations at different values of $U$ and $J$, while keeping the $J/U$ ratio fixed to 0.16.  Our results show, for both non-relativistic and relativistic calculations, a transition between $U=1$\,eV and $U=2$\,eV, see Fig.~\ref{fig:4}. This can be seen in the imaginary part of the interacting local Green's function $\text{Im}G_\text{loc}$, when taking the limit of $i\omega_n \to 0$. Refined analysis done at several values of $U$ in the proximity of the transition region reveal that the critical interaction without SOC is at about 1.6\,eV, whereas the transition is at roughly $U=1.2$\,eV when SOC is taken into account. This is shown in Fig.~\ref{fig:4} c).
...
%-------------------------------------------------------------------------
\begin{table}[b]
\caption{\label{Tab:2} Values of the screened Coulomb interactions $\tilde{U}_{ij}$ and $\tilde{J}_{ij}$ for BNOO from cRPA calculation in the "$t_{2g}$/$t_{2g}$" scheme, with an overall value of $\tilde U=2.9$\,eV and $\tilde J=0.2$\,eV. The calculations were done using the PBE functional. }
	\begin{center}
			\renewcommand{\arraystretch}{1.2}
			\begin{ruledtabular}
			\begin{tabular}{| c | c c c | c c c |}
				 \multicolumn{1}{c}{} & \multicolumn{3}{c}{$\tilde{U}_{ij}$ (eV)} & \multicolumn{3}{c}{$\tilde{J}_{ij}$ (eV)} \\ 
				 \cline{2-7}
...
Given that the properties of BNOO are closely linked to the strength of its onsite Coulomb interaction, we decided to compute the screened interaction parameters, $\tilde U$ and $\tilde J$, via cRPA. The orbitally-averaged Kanamori interaction values are denoted by the symbol $\tilde U$ and $\tilde J$. The calculation was conducted using the "$t_{2g}/t_{2g}$" scheme~\cite{PhysRevB.86.165105} based on a non-magnetic and non-relativistic band structures, with the correlated subspace subjected to wannierization, performed within a window $\mathcal{W} = [-1,1]$\,eV centered around the Fermi level. The results are shown in Tab.\ref{Tab:2}. If non-relativistic calculations are employed, the $\tilde U$ and $\tilde J$ constants can be converted to the Slater convention used in our DMFT calculations, as described in Ref.\cite{dajone}.
Our cRPA~results lead to a on-site averaged intra-orbital Coulomb interaction $\tilde U=2.9$\,eV and an averaged Hund's exchange $\tilde J=0.20$\,eV. These values correspond to $U=2.6$~eV and $J=0.25$\,eV  in the Slater~convention, 
slightly lower than the experimentally calculated values~\cite{Erickson2007} or from other theoretical works~\cite{mosca2021interplay}.

These results indicate that electron correlation plays a crucial role, exerting a greater influence than expected on 5d transition metals~\cite{Liu2015}. We suggest that the strong electronic correlation results from hybridization between Os-d and O-p orbitals, which is substantial in this compound~\cite{Gangopadhyay2015, mosca2021interplay}. The scenario provided by the 5d Sr$_2$IrO$_4$ is similar in this respect since the cRPA calculations reveal a higher value of $U$ than in the 4d isovalent and isostructural Sr$_2$RhO$_4$~\cite{Martins_2017}. In that case, the stronger hybridization with the ligand ions leads to significant overlap between Ir-d and O-p orbitals, which reduces screening effects, despite the more diffuse nature of the orbitals~\cite{Martins_2017}.
Although the cRPA results serve as only an estimate of the actual interaction values, it is evident that the genuine interaction values for BNOO are much higher than the range where SOC significantly influences the MIT (Fig.~\ref{fig:4}). As BNOO is an insulator regardless of its relativistic properties, it should be classified as a more standard Mott insulator.

Our results shed light on the insulating nature of BNOO and highlight possible future pathways. Understanding the role played by magnetic interactions in combination with correlation effects, possible local distortions and SOC effect is an ongoing research topic, and further studies are needed. The present implementation of the non-collinear DMFT will allow to further clarify the complex interplay of these phenomena and address the phase transition of the canted AFM phase. 
