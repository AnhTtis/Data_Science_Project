...
\author{Soyeon~Jung~and~Mykel~J.~Kochenderder
...
(e-mail: {soyeonj, mykel}@stanford.edu).}}
...
\begin{IEEEkeywords}
Air traffic control, trajectory modeling, Gaussian mixture model, conditional distribution, generative model, probabilistic model, machine learning
\end{IEEEkeywords}
...
We fit GMMs for pairs of trajectories and generate multiple trajectories by combining the mean vectors and covariance matrices of the pairwise GMM.
This approach allows us to efficiently generate an arbitrary number of trajectories from a single model.
In addition, we further develop the single trajectory model by performing a low-rank approximation for the covariance matrices of the GMMs.
This technique helps reduce feature redundancy and overfitting.
...
Fig. \ref{fig:jfk_arrivals_hist} shows a lateral log-histogram of approximately 48,000 processed arrival tracks to KJFK.
...
\caption{Actual arrival traffic of KJFK.}
...
\caption{Actual arrival tracks to KJFK 04R and associated flight procedures.}
...
This method preserves monotonicity and avoids oscillation on the intervals where the data is monotonic.
...
\label{subsec:GMM}
For each flight stage, we construct a Gaussian mixture model (GMM) with a set of training input vectors defined as (\ref{eq:single_model_training_data}).
A GMM is a probabilistic model that assumes the observed data points are generated from a mixture of a finite number of Gaussian densities.
...
To remove feature redundancy and overfitting, we perform a low-rank approximation for each covariance matrix of our GMM in Section \ref{subsec:GMM} using eigenvalue decomposition. 
...
\end{align*}
are the sequences of deviations for the first and the second aircraft, each of which are in the form of (\ref{eq:single_model_training_data}).
...
We evaluate the proposed model on the arrival trajectories to KJFK.
Trajectories and procedures for all the runways except for 13L are used for training. The performance of the models are validated with 13L data.
...
The silhouette method also provides a better quantification of the clustering performance, as it evaluates both intra-cluster and inter-cluster distances. A good clustering can be characterized by high inter-cluser distance and low intra-cluster distance.
...
In this paper, we propose a method for modeling aircraft trajectories in terminal airspace based on radar flight tracks and procedure data. 
We partition the arrival trajectories into the radar vector segment and the final approach segment, and then trained a Gaussian mixture model (GMM) for each segment using the deviations of aircraft from their procedures as the input features.
Given a new set of procedures, we can generate synthetic trajectories by sampling data from the radar vector segment and then from the conditional distribution of the final approach segment given the last few points of the sampled radar vector segment. 
To select the number of GMM components, we use the silhouette score. 
We evaluated the performance of the proposed model by computing the JS divergence between the original and generated data.
The results show that the model can learn the major patterns of aircraft behavior as well as capture uncertainties in the aircraft trajectories relative to their intended procedures.
...
Thus, the trained model is not generalizable to test procedures that have distinctively different characteristics than the training data, as discussed in Section \ref{subsec:experiment_single_trajectory_model}.
...
The authors thank Evan Maki, Randal Guendel, and Mikhail Krichman from MIT Lincoln Laboratory for their support and assistance.
