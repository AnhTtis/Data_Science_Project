...
Understanding the missions, operational tempo, and conditions for the first four days (Sept. 29-Oct. 2) of the Ian effort is important in establishing the wireless data demands and gaps as it represents the immediate life saving response and mitigation phase. During that phase, FL-UAS1 teams conducted 34 missions and collected more than 636GB of data. Although the teams brought with them six different wireless hotspot devices (Starlink: RV Package, Business Package; FirstNet: Sonim, custom PSEC router, and personal phones: AT\&T, Verizon), the large amount of data exceeded the capability and the team had to resort to ``war driving" to Florida Gulf Coast University 40 minutes away. That wireless network did not have sufficient capacity and eventually the data managers made nightly trips to Collier County Emergency Operations Center, the nearest intact agency with wired infrastructure. 
...
Understanding the demands on the wireless infrastructure after a disaster and how it differs from normal conditions is expected to enable both roboticists to design more effective systems and the wireless communications community to better support higher throughput and Cloud computing capabilities. The analysis is limited, as observing the condition of the wireless networks at Ian was opportunistic and secondary to primary goals of providing the data products. It should also be noted that the  observations focused on the volume, types, rates, and spatial distribution of data in the data-to-decision process, not network 
 metrics such as received signal strength indicator \cite{channelMeasurements}. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=9cm]{informationCurve.png}
    \caption{Impact (green dashed) versus availability of information (blue dashed) to emergency responders during the first 72 hours of a disaster, adapted from \cite{ccc:EmergencyInformatics}. Scaled availability of drone data products as \% update rate over wireless networks at Hurricane Ian in gold.}
...
%This resulted in the collection of 636GB of data and 48730 videos and images. This course of this collection is shown in Fig.~\ref{fig:data_collection}.
 ...
 \begin{figure}
     \centering
     \includegraphics[width=9cm]{images/DataOverTimeWPercentiles.PNG}
     \caption{Amount of data that was generated each day by FL-UAS1 during the response to Hurricane Ian. Regions of shading represent periods between sunset and sunrise.}
...
As a result opportunistic collection of network speeds began on this date.

\textit{October 1, 2022}.
%The third day represented a significant change from the prior two days.
On this day four more squads joined the response resulting in substantially more capacity for data collection. 
This day also marked a change in the type of mission to which squads were being dispatched. 
Unlike the previous two days which focused heavily on the opportunistic collection of video imagery, this day transitioned to missions focused on mapping and damage assessment in relative equal proportion.
On this day nine squads collected 33000 files totalling 369GB. 
Opportunistic collection of network speeds continued on this date.
...
Nine squads collected 14715 files totalling 174GB.
As with the previous days, the opportunistic observations of network speeds continued.

\section{FINDINGS}

 In this section, several findings are discussed that are of particular relevance to the research community and those developing wireless infrastructure for use in large scale disasters. 

\begin{figure}
    \centering
    \includegraphics[width=9cm]{images/DistanceFromFOBwPercentiles.PNG}
    \caption{A histogram of the amount of data collected in GB versus the distance from the FOB in miles (Bins of 1mi). The cumulative distribution function is shown in yellow and plotted along y-axis on the right.}
    \label{fig:data2fob}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=8.5cm]{images/ExtensionswPercentile.PNG}
    \caption{Volume of data collected per day segmented by file types. Images are shown in blue and videos are shown in orange. A clear shift can be seen in the distribution along the 72 hour mark.}
...
\begin{figure}
    \centering
    \includegraphics[width=9cm]{images/max_upload_rates_by_date.png}
    \caption{Maximum observed upload rate at the FOB at each date in the response in GB/hr. On 9/29 no observations of wireless upload speeds were made from which a maximum could be computed. The upload rate on that date has been approximated as 0 GB/hr and plotted it here as a dashed line for awareness.}
    \label{fig:upload_rates}
\end{figure}

\begin{figure*}
    \centering
    \includegraphics[width=15cm]{images/volume_of_data_by_creation_datetime_bandwidth_labels_bin_1.png}
    \caption{Amount of data collected by FL-UAS1 as a part of the response to Hurricane Ian compared to the upload speeds observed in the field. Any time a blue histogram bin surpasses a colored horizontal line, there was more data collected in that hour than could have been uploaded. Horizontal bars represent upload speeds in GB/hr. This plot highlights how, every day, FL-UAS1 experienced at least one hour where the volume of data collected exceeded the best case capacity of the wireless networks available.}
...
This change in data was also marked by the increased capacity of FL-UAS1 as additional personnel joined the response as it progressed.
On the first day, the response started with four squads, progressed to five on the second day, and reaching nine on the third and fourth days.
This increase in personnel as the response progresses demonstrates how the distribution of data collection changes as the response progressed, but also highlights the need for a wireless network that can handle an increasing number of endpoints as the response progresses.

Another finding is that network speeds appear to slowly recover in the field as each day of the response continues.
...
As can be seen in this figure, FL-UAS1 frequently, and at least once per day, collected data at a rate that exceeded even the best case upload speed that was observed in the field.
This presented a scenario where the network was saturated and there was more imagery to upload than the network could handle. 
...
Burstiness stems from sUAS storing high resolution data onboard and recovered only when the drone has landed (20-40 minutes) and from teams in the field conducting  multiple missions over 6-12 hours before returning to the FOB. 

\item The teams currently do not have sufficient connectivity in the field to a) live stream, b) upload bursts of tactical data directly to agencies after each mission, or c) upload those mission bursts for cloud computing.
They teams instead must transfer data when they return to the FOB at the end of the day, leading to bursts of data, which is exacerbated by multiple teams returning at one time.
...
The data generated exceeded the observed data rates of cell providers (ATT, Verizon), FirstNet, PSEC, and satellite providers (commercial and personal). A wireless connectivity “bubble” around the FOB would have to extend on the order of 10-40 miles (90\% of data is collected $>$10mi away), consistent with Harvey, Michael, and Irma.

\item The greatest need for large-scale wireless network for the FOB is for the first 3 days (also corresponding to the nominal 72 hour default response phase).
...
The missions tasking may increase unpredictably and more teams may join the response  as it progresses. 
This would increase the need for team-owned connections in field and burstiness for the FOB. The average squad generated 1874 files totalling 24GB per day.
\end{enumerate}

\section{CONCLUSIONS}
%RRM

The analysis of Hurricane Ian provides a quantitative analysis of wireless demands and current gaps for supporting the drone data-to-decision workflow during a disaster.  The results document the temporal aspects of wireless availability, and that the first 72 hours are the most critical in terms of impact of information on the decision-making process, yet there is not sufficient  bandwidth with either FirstNet, satellite, or consumer cellular service. An effective wireless network would need to provide  communications for mobile drone squads working from 10 to 40 miles from Forward Operating Base (spatial distribution). 
The gap in bandwidth is the upload rate, as the primary data transfers are uploading from drone to either data manager, agency, or the cloud; and from data manager to either the cloud or agencies. Downloading occurs rarely. 
The lack of high upload rates is exacerbated by burstiness, where drone squads attempt to upload mission data after the drone lands or the data from all missions at the end of the day. 
