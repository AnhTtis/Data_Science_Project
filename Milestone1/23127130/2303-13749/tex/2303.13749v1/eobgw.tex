...
\title{The influence of mass-ratio in extreme-mass-ratio inspirals for testing general relativity} 
...
\begin{abstract} 
The popular waveform templates of extreme-mass-ratio inspirals usually omit the mass-ratio in the conservative dynamics, and employ adiabatic approximation to include the radiation reaction. With the help of effective-one-body dynamics, we investigate the influence of mass-ratio in the conservative part on the gravitational waves. We find that for the extra-relativistic orbits, the mass-ratio can produce obvious orbital dephasing even for very small mass-ratio and then affect the waveforms. More importantly, it is noted that omitting the mass-ratio in waveform templates may induce a fake signal of deviation from general relativity.  
\end{abstract} 
...
black hole (BH) )  inspiraling into a supermassive black hole (SMBH), are the prime sources of the space-borne gravitational wave (GW)  detectors, such as the  Laser Interferometer Space Antenna (LISA)   \cite{Danzmann2017LISALI}, Taiji  \cite{Hu2017TheTP}  and TianQin  \cite{Luo_2016}. 
...
Extensive work has been done to quantify the ability of space-based GW detectors (LISA, TianQin, etc)  to measure the deviation of multipole moments from the Kerr BH and then test GR.
...
\end{equation} 
where $M_l$ and $S_l$ are the mass and mass-current multipole moments.
...
With the accumulation of observation time from several months to a few years, the signal-to-noise ratio (SNR)  of EMRIs could become large enough to be detected by matched filtering   \cite{2007Intermediate}. However, the accuracy of the waveform template is also expected to be very high to detect such long-duration signals. In general, it is required that the dephasing of $ 10^5$ cycles should be less than a few radians  \cite{PhysRevD.95.103012,PhysRevD.78.064028}.

Currently, there is a relatively accurate method based on the black hole perturbation theory, i.e. the waveform can be obtained by solving the Teukolsky equation  \cite{teukolsky1973perturbations}. This method is based on frequency domain decomposition and treats the SCO as a test particle. However, the waveform template based on the Teukolsky method is still computationally expensive and is not suitable for parameter estimation. Some approximation methods, such as the kludge model  \cite{barack2004lisa,babak2007kludge}  and the effective-one-body (EOB)  formalism  \cite{PhysRevLett.104.091102,PhysRevD.84.044014,Han_2017,2014IJMPD..2350064H,PhysRevD.83.044044}, make it possible to efficiently calculate a large number of relatively accurate waveforms.
...
The AK   \cite{barack2004lisa}  model is fast in generating waveforms, but it is not accurate in the latter stages of inspiral, in which a Keplerian orbit is not a good approximation  \cite{2007Intermediate}. 
The waveform generated by the NK model \cite{babak2007kludge}  can match well in some respects to the one obtained by solving the Teukolsky equation and can be used to introduce additional physical effects.
However, this model consumes more computational time than the AK model and is less accurate for eccentric orbits. The AAK  \cite{chua2017augmented}  model maps the parameters of the AK model to the NK model, and uses the NK model to improve the accuracy of the AK model, but retains the high computational efficiency of the AK model. 

These kludge models can generate waveforms more quickly than using the Teukolsky equation, but they regard SCOs as test particles in the orbital evolution, ignoring the mass-ratio correction in the conservative dynamics.  This  test-particle approximation  simplifies the problem a lot, however, it will also lead to inaccurate waveform templates,  which may give rise to some errors in  GW parameter estimation \cite{arun2009massive,huerta2009influence}. Especially, using a waveform template that is not accurate enough can lead to the detection of a spurious signal deviating from the Kerr black hole case.
...
In our work, we are interested in evaluating the impact of inaccurate waveforms due to the ignoring of mass-ratio, which may mistakenly lead to fake deviations from GR in parameter estimation. In other words, even if GR is a correct description of nature, there is a risk of making a wrong judgment on the test of gravity theory if mass-ratio correction is ignored in waveform templates.
...
In the present work, we make a further and detailed investigation of mass-ratio correction on the orbital motion (orbit, frequency, and phase)  and gravitational radiation. We also show the discrepancy of the Teukolsky-based energy fluxes by using the EOB orbit and the test particle one. More importantly, based on the Bayesian analysis, we evaluate the possibility of inducing a fake signal of deviation from GR due to omitting the mass-ratio corrections in waveform templates. We find that for the case of mass-ratio $\nu \lesssim 10^{-5} $, the mismatch  between the EOB and test particle waveforms is so small  that can be ignored. However, for the case of $\nu \gtrsim 10^{-5} $,  the mismatch is much larger, and there is a risk of making the incorrect judgment that we have detected a deviation from GR. 
...
\end{equation} 
where $H_{\rm S} $ and $H_{\rm NS} $ represent the Hamiltonian with and without spin,  $S_*=aM (m_2/m_1) $ is the effective spin, and $a=\mid S_{\rm Kerr}\mid/M$. $H_{\rm S} $ and $S_*$ are small enough to be ignored within the range of error precision  \cite{barausse2011extending}. The deformed Kerr metric  is given by  \cite{barausse2010improved} 
...
\begin{subequations} 
    ...
    \end{gather} 
\end{subequations} 
where $\omega^{\rm fd} _1=-10$, $\omega^{\rm fd} _1=20$  \cite{2014IJMPD..2350064H,PhysRevD.78.044002,Barausse_2009}, $A$ and $D$  are metric potentials for the EOB formalism mentioned in Ref.  \cite{steinhoff2016dynamical}, which are given in Appendix A.
...
\end{gather} 
\end{subequations} 
where $L_z$ is the angular momentum in the z-direction, $P_\theta$, $P_r$, and $P_\phi$ are the polar, radial, and  azimuthal angular momentum, and $E$ is the total system energy. The expressions of these parameters are given in Appendix A.
...
\begin{figure} 
	...
	\includegraphics[scale=0.6]{orbit1.pdf}         
} 
	\end{subfigure} 
    \begin{subfigure}[$a=0.95,e=0.9,\iota=30^{\circ},p=3.15 M$]{
	\includegraphics[scale=0.6]{orbit2.pdf}  
} 
        \end{subfigure} 
        
\begin{subfigure}[$a=0.9,e=0.9,\iota=10^{\circ},p=3.39 M$ ]{
	\includegraphics[scale=0.6]{orbit3.pdf} 
} 
	\end{subfigure}  	
 \begin{subfigure}[$a=0.98,e=0.8,\iota=0^{\circ},p=2.41 M$]{
	\includegraphics[scale=0.6]{orbit4.pdf}  
} 
	\end{subfigure} 
    \caption{\label{fig:orbit}  Four orbits  in the x-y plane of different orbital parameters. The green, blue, red, yellow, and black lines  represent $\nu=10^{-4},10^{-5},10^{-6},10^{-7} $, and   $\nu=0$, respectively. Note that $\nu=0$ denotes the case of the test particle. } 
\end{figure} 


In  Fig. \ref{fig:orbit}, We select four sets of parameters and compare the orbital evolution in the x-y plane for different mass-ratios in each group. We can see that the  difference between the EOB orbit  and  the test particle one  is obvious, especially when the mass-ratio takes $10^{-4} $ and $10^{-5} $. Note that the four orbits  we choose here are all zoom-whirl  \cite{PhysRevLett.103.131101}  whose orbital deviations are much more glaring than the generic orbits.
...
\begin{figure} 
    \centering
    \begin{subfigure}[$a=0.99,e=0.9,\iota=0^{\circ},p=2.35 M$]{
	\includegraphics[scale=0.6]{deltaphi1.pdf}         
} 
	\end{subfigure}  
    \begin{subfigure}[$a=0.95,e=0.9,\iota=30^{\circ},p=3.15 M$]{
	\includegraphics[scale=0.6]{deltaphi2.pdf}  
} 
        \end{subfigure}  
        
\begin{subfigure}[$a=0.9,e=0.9,\iota=10^{\circ},p=3.39 M$ ]{
	\includegraphics[scale=0.6]{deltaphi3.pdf} 
} 
	\end{subfigure}  	
 \begin{subfigure}[$a=0.98,e=0.8,\iota=0^{\circ},p=2.41 M$]{
	\includegraphics[scale=0.6]{deltaphi4.pdf}  
} 
	\end{subfigure}  
...
\end{figure}  
Fig. \ref{fig:delta phi}  represents the dephasing, $\Delta \phi$, for the four orbits presented in Fig. \ref{fig:orbit}. We notice that when $\nu$ is very low  ($\nu = 10^{-7} $), this dephasing is negligible. But for higher values of $\nu$, the phase shift $\Delta \phi$ becomes significant. Furthermore, we observe that as time increases, the dephasing $\Delta \phi$ derived for different mass-ratios becomes progressively larger.
...
\begin{subequations} \label{eq:eob-omega} 
    ...
    \end{gather} 
\end{subequations} 
The expressions of $K (k) $, $E (k) $, $\Pi (z^2_- ,k) $, $z_+$, $z_-$, and $k$ are given in Appendix A.


\begin{figure} 
    \centering
    \begin{subfigure}[$a=0.99,e=0.9,\iota=0^{\circ},p=2.35 M$]{
	\includegraphics[scale=0.6]{omega1.pdf}         
} 
	\end{subfigure}  
    \begin{subfigure}[$a=0.95,e=0.9,\iota=30^{\circ},p=3.15 M$]{
	\includegraphics[scale=0.6]{omega2.pdf}  
} 
        \end{subfigure}  
        
\begin{subfigure}[$a=0.9,e=0.9,\iota=10^{\circ},p=3.39 M$ ]{
	\includegraphics[scale=0.6]{omega3.pdf} 
} 
	\end{subfigure}  	
 \begin{subfigure}[$a=0.98,e=0.8,\iota=0^{\circ},p=2.41 M$]{
	\includegraphics[scale=0.6]{omega4.pdf}  
} 
	\end{subfigure}  
...
\end{equation} 
where $\rho=-1/ (r-i a \cos{\theta} ) $, $\omega$ is the  discrete frequency spectrum, and $\phi=\int \omega dt $.
...
\end{equation} 
where the $R^{\infty} _{lm\omega}  (r) $ and $R^{\rm H} _{lm\omega}  (r) $ are the two independent solutions of the homogeneous 
Teukolsky equation, which are given by
\begin{equation} 
...
\begin{figure} 
    \centering
	\includegraphics[scale=0.55]{dedtnu.pdf}         
    \caption{\label{fig:dedtnu}  The spectrum of energy fluxes versus the harmonic number $k$ with parameters $a = 0.99$, $p = 2.11$ and $e = 0.7$. The yellow bar represents $\nu = 0$ and the blue one $\nu  =10^{-4} $ respectively.} 
\end{figure} 
...
Once we have the GW  fluxes $\Dot{E} $, $\Dot{L} _z$, and $\Dot{Q} $, substituting  expressions of these  fluxes  into Eqs.  \eqref{eq:p-dot}--\eqref{eq:iota-dot}, we can derive the orbital evolution. Combing Eqs.  \eqref{eq:eob-omega-r}--\eqref{eq:eob-omega-phi}  and Eq. \eqref{eq:strain}, we can generate the gravitational waveforms. However, fully using the numerical Teukolsky-based fluxes is computationally expensive. Without impacting on our main target, we here employ a hybrid scheme of fluxes \cite{gair2006improved}  (hereafter GG) which is calibrated by the numerical Teukolsky data.

Compared  to the numerical fluxes, the GG formalism performs well for orbits of low  eccentricity, especially  for circular, inclined orbits.  We also include mass-ratio corrections to the original 2PN fluxes based on \cite{PhysRevD.96.104048,Zhang_2021}, though this is almost no influence on the data analysis. The final expressions for $\Dot{E} $, $\Dot{L} _z$, and $\Dot{Q} $ (Eqs. \eqref{eq:Lz mod}--\eqref{eq:E mod}, \eqref{eq:newEdot2pn}--\eqref{eq:newQdot} )   are given in Appendix B. 

\begin{figure} 
    \centering
	\includegraphics[scale=0.6]{waveform3.pdf}         
    \caption{\label{fig:strain}  Two adiabatic waveforms of  EMRI systems with     
    $ (M, \mu, q )  = (5\times10^6 M_{\odot}, 50 M_{\odot},0.43) $, starting at $ (p_0, e_0, \iota_0)  = (6.84, 0.166, 45^\circ) $.  We  plot the one-year  evolving waveform at the viewing angle $ (\Theta, \Phi)  = (45^\circ, 0^\circ) $. The orange line represents the waveform excluding mass-ratio corrections, and the blue line denotes the waveform including mass-ratio corrections.   The adiabatic waveforms are generated using the BHPC’s numerical Teukolsky code  \cite{10.1143/PTP.121.843}. } 
\end{figure} 

As an example, Fig. \ref{fig:strain}  shows two  adiabatic waveforms  including or excluding mass-ratio corrections  starting at $ (p_0, e_0, \iota_0)  = (6.84, 0.166, 45^\circ) $, and the mass-ratio is $10^{-5} $. We plot the one-year evolving waveforms at the viewing angle $ (\Theta, \Phi)  = (45^\circ, 0^\circ) $.  In our work, we generate the gravitational waveforms only considering $ (l,m) = (2,2) $ mode, which is the dominant strain mode. 
...
However, waveform errors due to the mass-ratio can accumulate as time goes by. And then an obvious waveform difference can be found in the waveforms at the latter stage of evolution (middle and right columns).
...
The matched filtering  \cite{PhysRevD.46.5236}  technique is widely used in LIGO and Virgo data processing, and it will also be applicable to future space-borne GW detectors. We also apply this technique to analyze the influence of mass-ratio correction on EMRIs waveforms quantitatively.
...
\begin{figure} 
    \centering
	\includegraphics[scale=0.7]{mismatcheob.pdf}         
    \caption{\label{fig:mismatch} The mismatch  between the EOB orbit and the test particle one. 
    The blue, red, yellow, and green lines  represent systems with different mass-ratios of $10^{-4} $, $10^{-5} $, $10^{-6} $, and  $10^{-7} $,  respectively.
    For each $\nu$, we calculate the mismatch considering different evolution times from 1 month to 12 months. We fix the EMRI parameters to $a=0.9$, $ (\mu, M )  = ( 50 M_{\odot}, 5\mu/\nu ) $ at the distance of $1\rm{Gpc} $. Our waveforms end at $ ( p, e, \iota )  = (5.1, 0.1, 80^\circ) $. } 
\end{figure} 
...
\begin{figure} 
\includegraphics[scale=0.8]{mismatchlarge.pdf} % Here is how to import EPS art
...
\begin{figure} 
\includegraphics[scale=0.7]{logBeob1.pdf} % Here is how to import EPS art
...
\hline
1  & 0.30     &  0.12      &  45    & 5.7    &   50    &  $ 1\times 10^{-4} $   &   2.5    & $ 8.4\times 10^{-1} $  &   150.1   & 88.99 \\
2  &  0.43    &   0.10     &  72    &  7.2   &   20   &  $ 6\times 10^{-5} $   &   1.5   & $ 4.7\times 10^{-1} $    &  78.4    & 34.41  \\

3  &  0.73   &   0.08     &  81    &  5.8   &   25   &  $ 6\times 10^{-5} $   &   7.0   & $ 7.4\times 10^{-1} $    &  16.2   & 7.96 \\

4  &   0.14   &  0.16      &  70    & 6.9   & 35  &   $ 5 \times 10^{-5} $     &  2.0  &  $ 3.0\times 10^{-1} $   & 81.8   & 28.32     \\ 
5  &  0.80    &   0.15     &   60   &  6.5  & 40  &    $ 4\times 10^{-5} $    & 6.0  &   $ 4.0\times 10^{-1} $   &  35.8 &  13.43   \\
6 &  0.35    &   0.13     &   35   &  6.4  & 30  &    $ 4\times 10^{-5} $    & 5.0  &   $ 1.9\times 10^{-1} $   &  51.8 &  13.0   \\
7  &  0.60   &   0.10     & 45    & 5.2    &  25    &   $ 3\times 10^{-5} $   & 3.0  &   $ 5.6 \times 10^{-1} $    & 62.6  & 29.95  \\

8  &  0.70    &  0.20       &  10     &  6.0    &  20  &  $ 3\times 10^{-5} $   &  2.0 &  $ 4.4\times 10^{-1} $   &  80.7 &  34.48  \\ 

9  &  0.80    & 0.10       &  70    &   5.2  &   40    &   $ 2\times 10^{-5} $    &     1.0  &   $ 3.4\times 10^{-1} $ & 134.4     & 51.13  \\ 

10  &   0.90   &0.20        & 85     &  6.3   &  45      &  $ 1\times 10^{-5} $     &    8.0  &   $ 2.4\times 10^{-2} $  &   9.0   &-0.30  \\

11  &   0.70   &0.12        & 50     &  6.0   &  35      &  $ 1\times 10^{-5} $     &    1.5  &   $ 1.6 \times 10^{-2} $  &   78.5    &   3.63 \\

12  &   0.10   &  0.30      &  40    &  7.1   & 30  &   $ 8\times 10^{-6} $     &  3.0 &  $ 2.4\times 10^{-3} $   &  38.5 &  -1.4   \\ 
13  &  0.60    &  0.22      &  45    &  7.2   & 40   &  $ 6\times 10^{-6} $  & 4.5   & $2.4 \times 10^{-4} $ &  24.6  & -2.0  \\

14  &  0.55    &  0.40      &  38    &  6.8   & 30   &  $ 5\times 10^{-6} $  &1.0   & $ 1.2\times 10^{-3} $ &  87.9 &   -1.37\\ 
15  & 0.85     &  0.15      &   30    & 5.5     &  50   &  $ 4\times 10^{-6} $    & 2.0  &   $ 1.1\times 10^{-3} $   &   48.7     & -1.82   \\

16  & 0.20     &  0.35      &   55    & 7.0     &  30   &  $ 3\times 10^{-6} $    & 8.0  &   $ 2.5\times 10^{-4} $   &    7.1 &  -0.96\\

17  &   0.40    & 0.30        &  50    &  6.5   &  25    &  $ 2\times 10^{-6} $    & 5.0     &   $ 1.3\times 10^{-4} $   & 8.8    &    -1.18            \\

18  &  0.50   &  0.22      &  20    &  6.0   & 10   &  $ 2\times 10^{-6} $  & 4.0   & $5.0\times 10^{-4} $ &  8.9 &   -1.13\\

19   &  0.30     &   0.40      & 35      &  6.8    &    25  &   $ 1\times 10^{-6} $ & 1.0  &  $ 1.9\times 10^{-5} $  & 58.8   & -2.97  \\

20  &  0.62    &   0.30     &  60    & 5.8    &  10    &   $ 1\times 10^{-6} $   &  3.5  &   $  6.2\times 10^{-4} $    & 4.9  &  -0.59 \\
...
\begin{subequations} \label{eq:angular momentum} 
    \begin{gather} 
        \hat{P_\theta} ^2=\Hat{Q} - \cos^2{\theta_{\rm min} } [a^2 (1-\hat{H} ^2_{\rm eff} ) +\frac{\hat{L} ^2_z} {\sin^2{\theta_{\rm min} } } ]  \\
    ...
    \end{gather} 
\end{subequations} 
where $P_\theta$, $P_r$, and $P_\phi$ are the polar, azimuthal radial and canonical radial angular momentum.

From Eqs.  \eqref{eq:metric}, \eqref{eq:Q}--\eqref{eq:E}  and \eqref{eq:angular momentum}, the orbital evolution equations for $r$, $\phi$, $\chi$ can be obtained by
\begin{subequations} 
    \begin{gather} 
...
The  parameters in the expressions of the coordinate-time frequencies $\omega_{r} $, $\omega_{\theta} $, $\omega_{\phi} $ (Eqs.  \eqref{eq:eob-omega-r}--\eqref{eq:eob-omega-theta} )  are expressed by
\begin{subequations} 
    \begin{gather} 
         K (k) =\int_{0} ^{\pi/2}  \frac{d \chi} {\sqrt{1-k \sin^2{\chi} } }  \\
...
\begin{split} 
    (\Dot{L_z} ) _{\rm mod}  = (1-e^2) ^{3/2}  [ (1-e^2) ^{-3/2}  (\Dot{L_z} ) _{\rm 2PN}  (p,\iota, e, a)  - (\Dot{L_z} ) _{\rm 2PN}  (p,\iota, 0, a) + (\Dot{L_z} ) _{\rm fit} ]
\end{split} 
\end{equation} 
...
\begin{split} 
    ...
    & +2\tan \iota ( (\Dot{L_z} ) _{\rm fit} +\frac{ \sqrt{Q (p,\iota, 0, a) } } {\sin^2{\iota} }  (\Dot{\iota} ) _{\rm fit} ) ]
\end{split} 
\end{equation} 
...
\begin{split} 
     ...
     -\frac{N_4 (p,\iota) } {N_1 (p,\iota) }  (\Dot{L_z} ) _{\rm mod}  (p,\iota, 0, a)  \\
...
\begin{split} 
     (\Dot{L_z} ) _{\rm 2PN}  = &-\frac{32} {5}  \frac{\mu^2} {M}  (\frac{M} {p} ) ^{7/2}  (1-e^2) ^{3/2}  [
...
\begin{equation} 
\begin{split} 
     (\Dot{L_z} ) _{\rm fit}  =&-\frac{32} {5}  \frac{\mu^2} {M}  (\frac{M} {p} ) ^{7/2} [
...
\end{equation} 
 ...
 But  the mass-ratio correction is not involved in the expressions of their 2PN fluxes. In this work, we make some corresponding improvements  by adding mass-ratio correction terms in  Eqs.  \eqref{eq:Edot2pn}--\eqref{eq:Qdot}  to get the evolution expressions of orbit parameters $ (p,e,\iota) $. Our improved 2PN fluxes are given by
