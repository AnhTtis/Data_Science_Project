...
Suppose $R$ is an excellent, reduced ring of prime characteristic $p>0$ which is locally equidimensional. Then, if $R$ is $F$-nilpotent, $R_\mfm$ is geometrically unibranched for each $\mfm \in \Max R$. In particular, reduced, excellent $F$-nilpotent local rings are domains. Furthermore, if $\dim R = 1$, $R$ is $F$-nilpotent if and only if $R_\mfm$ is geometrically unibranched for each $\mfm \in \Max R$.
\end{mthm}
...
The arguments involved in the proof of \Cref{thm: counting branches main thm} immediately give:
...
Little has been written about the condition that $I^*=I^F$ for all ideals $I$. An additional class of rings with this property are \textit{$F$-coherent} rings, whose definition we avoid here. The class of $F$-coherent rings were studied by Shimomoto in \cite{Shi11}. We note that there are regular rings (in which $I^*=I^F=I$ for all ideals $I$) which are not $F$-coherent. \end{srmk}
