...
\keywords{F-nilpotent rings, geometric branches}
...
In \cite{SW08}, Singh-Walther give a formula to count the number of connected components of the punctured spectrum of the strict henselization of a complete local ring $(R,\mfm)$ with algebraically closed coefficient field using the \textit{semi-stable part} of the Frobenius action on $H^1_\mfm(R)$; see \Cref{rmk: SW reconciliation}. Our work extends theirs in dimension one by removing the hypotheses that $R$ be complete and that the residue field be algebraically closed. There are also computational advantages to our results since an $R/\mfm$-vector space basis of $0^*_{H^1_\mfm(R)}/0^F_{H^1_\mfm(R)}$ is readily available in many situations, including when $R$ is standard graded over the field $R/\mfm$. We discuss this in greater length in \Cref{sec: Frobenius and geometric branches}.

A major aim of this paper is to understand rings with a single geometric branch. In the light of the above theorem, this naturally leads us to study $F$-nilpotent rings---a recently introduced singularity type in prime characteristic. Defined by Blickle-Bondu in \cite{BB} under the name ``close to $F$-rational", a local ring $(R,\mfm)$ of dimension $d$ is \textit{$F$-nilpotent} if, for each $j<d$, the canonical Frobenius action on $H^j_\mfm(R)$ is nilpotent, and the tight closure of the zero submodule in $H^d_\mfm(R)$ is also nilpotent, that is, $0^*_{H^d_\mfm(R)} = 0^F_{H^d_\mfm(R)}$.  

In \cite{ST17}, Srinivas and Takagi define a ring of characteristic zero to be of \textit{$F$-nilpotent type} if almost all of its mod $p$ reductions are $F$-nilpotent. Crucially, they give a characterization of two-dimensional normal rings of $F$-nilpotent type over the complex numbers in terms of their divisor class groups. They also give a characterization of three-dimensional graded normal rings of $F$-nilpotent type over the complex numbers in terms of the divisor class groups and Brauer groups (cf. \cite[Theorems 4.1,4.2]{ST17}). In this paper, we show that an $F$-nilpotent local ring over a field is geometrically unibranched, and that the property of being geometrically unibranched at each closed point characterizes $F$-nilpotent curves.

\begin{mthm}[\Cref{thm: F-nilpotent implies geo unibranched}, \Cref{col:main corollary}]
Suppose $R$ is an excellent, reduced ring of prime characteristic $p>0$ which is locally equidimensional. Then, if $R$ is $F$-nilpotent, $R_\mfm$ is geometrically unibranched for each $\mfm \in \Max R$. In particular, reduced, excellent $F$-nilpotent local rings are domains. 

Furthermore, if $\dim R = 1$, $R$ is $F$-nilpotent if and only if $R_\mfm$ is geometrically unibranched for each $\mfm \in \Max R$.
\end{mthm}

A key insight of this paper is that the number of branches of a local ring in positive characteristic can be counted by studying the natural Frobenius action on its weak normalization. The weak normalization of a reduced ring encapsulates the purely inseparable part of its normalization. In \cite{Sch09}, Schwede showed that an $F$-injective ring which admits a dualizing complex (a very mild requirement) must be weakly normal. In the course of proving the above theorem, we show that $F$-nilpotent rings exhibit a ``dual" property to $F$-injective rings, in that the weak normalization of an $F$-nilpotent ring must itself be normal. Since an $F$-rational local ring is precisely one which is both $F$-injective and $F$-nilpotent, our result, together with that of Schwede, provides, perhaps amusingly, a novel proof of the well-known fact that an $F$-rational local ring is a normal domain; see Remark \ref{rmk: dual to schwede}.
...
The following theorem demonstrates that the usually intractable calculations involved in computing the tight closure are much easier in $F$-nilpotent affine semigroup rings, and furthermore, that the pure inseparability of the normalization map characterizes $F$-nilpotent affine semigroup rings in any dimension.
...
\begin{sdef}
Let $R$ be a ring and let $\mfm \in \Max R$. The \textbf{number of (geometric) branches of $R$ at $\mfm$} is the number of minimal primes of the (strict) henselization of the local ring $R_\mfm$. If $(R,\mfm)$ is local, we denote the number of geometric branches of $R$ by $b(R)$, that is, $b(R)=|\Min R^{sh}|$. If $R$ has a single (geometric) branch, then we say $R$ is \textbf{(geometrically) unibranched}.
\end{sdef}
...
Let $(R,\mfm)$ be a local ring with $k=R/\mfm$ and let $\overline{R}$ be its normalization, a semi-local ring. By \cite[Tag~0C24]{stacks}, the maximal ideals of $\overline{R}$ correspond bijectively with the minimal primes of $R^{h}$ and the minimal primes of the completion  $\widehat{R}$. Thus, $R$ is unibranched if and only if its normalization $\overline{R}$ is a local ring. 

Next, write $\Max \overline{R} = \{\mathfrak{M}_1,\cdots,\mathfrak{M}_b\}$, and $K_i = \overline{R}/\mathfrak{M}_i$. Then, $b(R) = \sum_i [K_i:k]_{\text{sep}}$, where $[K_i:k]_{\text{sep}}$ is the separable degree of the extension $k\subset K_i$. Thus, $R$ is geometrically unibranched, that is, $b(R) = 1$ if and only if $\overline{R} = (\overline{R}, \mathfrak{M}, K)$ is local and the field extension $k \rightarrow K$ is purely inseparable, that is, for each $x$ in $K$, there exists some $e > 0$ such that $x^{p^e}$ lies in $k$ where $p>0$ is the characteristic of $k$. 
\end{srmk}

We now provide an example to illustrate the difference between the number of branches and geometric branches of a ring. 
 
\begin{sex} 
Let $R$ the ring $\mathbb{F}_3[x,y]/(x^2+y^2)$ localized at the ideal $\mathbf{m} = (x,y)$ where $\mathbb{F}_3$ is the field with three elements. Note that the completion of $R$ at $\mathbf{m}$ is $\mathbb{F}_3[|x,y|]/(x^2+y^2)$, which is a domain. Therefore $R$ has a single branch. Alternatively, notice that $t = y/x$ lies in the normalization $\overline{R}$ of $R$. Further, $R[t]$ is the ring \[ \left( \frac{\mathbb{F}_3[t]}{(t^2+1)}[x]\right)_{\mathbf{m}} = (\mathbb{F}_9[x])_{(x)}.\] Since $R[t]$ is a normal domain, it must be equal to $\overline{R}$. As $\overline{R}$ is local, we again see that $R$ has a single branch.

Notice however that $R$ has two geometric branches, that is, $b(R) = 2$. This is because $R^{sh}$ is the ring $\mathbb{F}_3^{\text{sep}}[x,y]/(x+iy)(x-iy)$ which has two minimal primes; here $i$ is a root of the separable polynomial $t^2+1$ over $\mathbb{F}_3[t]$. Alternatively, $\overline{R}$ is the ring $(\mathbb{F}_9[x])_{(x)}$ and $[\mathbb{F}_9 : \mathbb{F}_3]_{\text{sep}} = [\mathbb{F}_9 : \mathbb{F}_3] =2$ also confirms that $R$ has two geometric branches by \Cref{rmk: geometric branches counted by separable degree}.
\end{sex}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item $(x^n)^* = \mfm^n$.
\end{enumerate}
Finally, the above equalities also hold in the localized ring $R_\mfm$.
\end{slem}
...
Since the ideal $(x^n)$ is principal, its tight closure $(x^n)^*$ equals its integral closure $\overline{(x^n)}$. Since $(x^n)$ is a reduction of $\mfm^n$, we must have $\mfm^n \subset \overline{(x^n)}$. For the reverse containment, suppose $y \in (x^n)^*$, and again since $(x^n)^*$ is homogeneous, it suffices to assume $y$ is homogeneous. We now need to show that $y$ lies in $\mfm ^n$. Let $c \in R^\circ$ be such that $cy^{p^e} \in (x^{np^e})$ for all $e \gg 0$. Since $(x^{np^e})$ is homogeneous for all $e$, we can assume that $c$ is homogeneous. Then, we have $\deg (cy^{p^e}) \ge np^e$ for all $e\gg 0$, so that $$\deg y \ge n - \frac{\deg c}{p^e} \text{ for all } e \gg 0.$$ Thus, $\deg y \ge n$ so that $y$ lies in $\mfm ^n$ as required. (See \cite[Proposition~2.1]{Smi2} for a more general statement concerning lower bounds of the degree of an element contained in the tight closure of a homogeneous ideal.)
...
We note in the proof of (a) above, we may relax the condition that $k$ be perfect to only assume that $k$ has all $p^e$th roots of its elements for $e = \fte R$, as $x^n$ is a parameter for all $n$.

\subsection{Weak normalization and purely inseparable extensions}

In this subsection, all rings considered will be of prime characteristic $p>0$. Recall that a field extension $k\rightarrow \ell$ is \textbf{purely inseparable} if $k$ is of characteristic $0$ or if $k$ is characteristic $p>0$ and every element $\lambda$ of $\ell$ satisfies an equation of the form $\lambda^{p^e}=x$ for some $x \in k$ and $e \in \mbn$. One can similarly define the notion of purely inseparable ring extensions:

\begin{sdef}
A ring extension (that is, an injective homomorphism) $\phi: R\rightarrow S$ is \textbf{purely inseparable} if for all $s \in S$ there is an $e\in \mbn$ such that $s^{p^e}$ lies in $\phi(R)$. If $\phi$ is a finite map, there must be an $e$ so that $F^e(S)\subset R$, we call the smallest such $e$ the \textbf{pure inseparability index} of $\phi$.
\end{sdef}

It is clear that a purely inseparable ring extension induces purely inseparable extensions on the residue fields of local rings and the total quotient rings of reduced rings.
...
For any ideal $I\subset R$ and $j\in \mbn$, the ring homomorphism $F:R\rightarrow R$ induces an additive map $F:H^j_I(R)\rightarrow H^j_I(R)$ called a \textbf{Frobenius action}. Further, $F$ is \textbf{$p$-linear}, i.e. for all $\xi \in H^j_I(R)$ and $r \in R$, $F(r\xi)=r^pF(\xi)$.
\end{sdef}

For a discussion on how $F:H^j_I(R)\rightarrow H^j_I(R)$ is induced from $F:R\rightarrow R$, we direct the reader to \cite[Remark~2.1]{Sha06}. One way to measure the singularity of a local ring $(R,\mfm)$ using $F:H^j_\mfm(R)\rightarrow H^j_\mfm(R)$ is by understanding how much of the local cohomology vanishes under high iterates of $F$. Studied by Srinivas-Takagi, Polstra-Quy, Quy, Kenkel-Maddox-Polstra-Simpson, and more, the following singularity types are defined in terms of when as much of the local cohomology of $R$ is as nilpotent as possible.

\begin{sdef}\label{def: weak and F-nilp}
Let $(R,\mfm)$ be a local ring. We say that $R$ is weakly $F$-nilpotent if for each $0 \le j < \dim R$, $H^j_\mfm(R)$ is nilpotent under $F$. Further, $R$ is \textbf{$F$-nilpotent} if, in addition, $0^*_{H^d_\mfm(R)}=0^F_{H^d_\mfm(R)}$. A non-local ring $R$ is (weakly) \textbf{$F$-nilpotent} if $R_\mfm$ is (weakly) $F$-nilpotent for all $\mfm \in \Max R$. 

If $R$ is a non-negatively graded ring over a field and $\mfm$ is its unique homogeneous maximal ideal, we say that $R$ is (weakly) $F$-nilpotent in the same way as for local rings, replacing the local cohomology modules with those supported at $\mfm$.
\end{sdef}

The class of $F$-nilpotent rings were introduced Blickle-Bondu in \cite{BB} (under the name \textit{close to $F$-rational}) and studied further by Srinivas-Takagi in \cite{ST17}. They can be viewed as a weakening of $F$-rational rings---a classical $F$-singularity type. We remind the reader of this definition below.

\begin{sdef}\label{def: F-inj and F-rational}
Let $(R,\mfm)$ be a local ring of dimension $d$. We say $R$ is \textbf{$F$-injective} if $F:H^j_\mfm(R)\rightarrow H^j_\mfm(R)$ is injective for all $j$, and $R$ is \textbf{$F$-rational} if it is Cohen-Macaulay and $0^*_{H^d_\mfm(R)}=0$.  
\end{sdef}
...
Finally, we will need to utilize one more singularity type defined in terms of triviality of the ideal closures of all ideals, not only parameter ideals.

\begin{sdef}\label{def: weakly F-regular and F-regular}
The ring $R$ is \textbf{weakly $F$-regular} if $I^*=I$ for all ideals $I\subset R$, and is \textbf{$F$-regular} if $W^{-1}R$ is weakly $F$-regular for all multiplicative sets $W\subset R$. Trivially, the $F$-regular condition localizes. 
\end{sdef}

We will demonstrate in \Cref{sec: Fte and pi exts} that rings which are ``close" to having the singularity types given in this subsection (that is, upto a finite, purely inseparable extension) have similar uniformity properties with respect to ideal closures. We conclude this subsection by demonstrating the ascent and descent of (weakly) $F$-nilpotent singularities along purely inseparable extensions (compare with \cite[Theorem~4.5]{KMPS} and \cite[Lemma~2.14]{MP22}).

\begin{sthm} \label{thm: ascent/descent of F-nilpotence}
Let $R\rightarrow S$ be a finite, purely inseparable extension. Then $R$ is (weakly) $F$-nilpotent if and only if $S$ is (weakly) $F$-nilpotent.
\end{sthm}

\begin{proof}
A map being finite and purely inseparable localizes, and the (weakly) $F$-nilpotent condition is local. Further, since $R\rightarrow S$ is purely inseparable, for each maximal ideal $\mfm$ of $R$, there is a unique maximal ideal $\mfn$ of $S$, and $\mfm S$ is $\mfn$-primary. So, after localizing, we may assume $(R,\mfm)\rightarrow (S,\mfn)$ is a finite, purely inseparable extension of local rings. Write $d = \dim R = \dim S$. 
...
To describe the connection between geometric branches and the Frobenius map, we first consider the case of a single geometric branch. We will give a characterization of this property for curves. 
...
Let $R$ be a excellent, reduced $F$-nilpotent ring. Then $R_\mfm$ is geometrically unibranched for all $\mfm \in \Max R$. In particular, excellent, reduced $F$-nilpotent local rings are domains.
\end{sthm}

\begin{proof}
Assume that $(R,\mfm)$ is local. We first show that $R \rightarrow \overline{R}$ is a purely inseparable map of rings. Let $Q(R)$ be the total quotient ring of $R$, and let $x/y \in Q(R)$ be integral over $R$. Then, $y$ is either a unit of $R$ (in which case the remainder of the argument is trivial) or a regular element of $R$, and $x \in \overline{(y)}$, the integral closure of the ideal $(y)$ in $R$. However, by \cite[Corollary~5.8]{HH}, $\overline{(y)}=(y)^*$ as $(y)$ is principal. But since $S$ is an excellent $F$-nilpotent local ring and $y$ is a parameter element, we have $(y)^*=(y)^F$ by \cite[Corollary~5.15]{PQ}.

Thus, there is an $e \in \mbn$ so that $x^{p^e} \in (y)^{\fbp{p^e}}=(y^{p^e})$, that is, $x^{p^e}=ry^{p^e}$ for some $r \in R$. Note that this means $(x/y)^{p^e} \in S$, the weak normalization of $R$. Since $x/y$ was chosen to be an arbitrary element of $\overline{R}$, we get that $S = \overline{R}$, that is, the weak normalization of $R$ is normal. In particular, $R \rightarrow \overline{R}$ is a purely inseparable map of rings. 
...
\begin{proof}
If $R$ is normal, then it is regular, so the result is trivial. Otherwise, the conductor ideal $\mfc$ of $R\rightarrow \overline{R}$ is an $\mfm$-primary ideal of $R$. Let $e$ be the smallest natural number so that $\mfm^{\fbp{p^e}}\subset \mfc$. Further, recall the tight closure of a principal ideal agrees with its integral closure.

let $\xi$ be an element of the $R$-module $0^*_{H^1_\mfm(R)}$. By \cite[Proposition~2.5]{Smi97}, for a regular parameter $x \in R$, we have $\xi=[a+(x^n)]$ for some $a$ lying in $(x^n)^*=\overline{(x^n)}$. Thus, the element $a/x^n \in Q(R)$ is inside $\overline{R}$, and for any element $b \in \mfm$, we have $b^{p^e}(a/x^n)^{p^e} \in R$ since $\mfm^{\fbp{p^e}}\subset \mfc$. Hence, $(ba)^{p^e} \in (x^{np^e})=(x^n)^{\fbp{p^e}}$, thus $ba \in (x^n)^F$. This implies $b[a+(x^n)] \in 0^F_{H^1_\mfm(R)}$ so that the $R$-module $\mfm 0^*_{H^1_\mfm(R)}$ is contained in $0^F_{H^1_\mfm(R)}$, as required.
\end{proof}

We are now prepared to prove our main result. 

\begin{sthm}\label{thm: counting branches main thm}
Let $(R,\mfm,k)$ be an excellent, reduced local ring of dimension one and let $(S,\mfn,\ell)$ be the weak normalization of $R$. Then \[\dim_k 0^*_{H^1_\mfm(R)}/0^F_{H^1_\mfm(R)} = [\ell : k](b(R)-1).\] In particular, if $k$ is perfect, \[\dim_k 0^*_{H^1_\mfm(R)}/0^F_{H^1_\mfm(R)} = b(R)-1.\] 
\end{sthm}
...
Since $R$ is one dimensional, the conductor ideal $\mfc$ of $R\rightarrow \overline{R}$ is $\mfm$-primary, so there is an $N$ such that $\mfm ^N$ lies in $\mfc$. We may replace $x$ by $x^n$ for any $n \ge N$ and thus assume that $x \in \mfc$.

By \cite[Proposition~1.6.1]{HS} and \cite[Corollary~5.8]{HH}, we have $(x)^*=\overline{(x)} = x\overline{R} \cap R$, but $x\overline{R}\subset R$ since $x \in \mfc$, i.e. $\overline{(x)} = x\overline{R}$ as $R$-submodules of $\overline{R}$. Similarly, $(x)^F = xS \cap R = xS$ since $S\subset\overline{R}$. Further, $x$ is a regular element which implies $x\overline{R}/xS \simeq \overline{R}/S$, so we must compute the length of the $R$-module $\overline{R}/S$. 

Let $J\subset \overline{R}$ be the Jacobson radical of $\overline{R}$, which is the intersection of finitely many maximal ideals $\mathfrak{M}_1,\cdots,\mathfrak{M}_t$ since $\overline{R}$ is semi-local. Write $K_i = \overline{R}/\mathfrak{M}_i$, and by \Cref{rmk: geometric branches counted by separable degree}, we have $\sum_i [K_i:k]_{\text{sep}} = b(R)$. For convenience, write $[\ell:k]=p^w$. Now, we have a sequence of finite field extensions $k\rightarrow \ell \rightarrow K_i$ for each $i$, and $k\rightarrow \ell$ is the perfect closure of $k$ in $K_i$. This implies $\ell \rightarrow K_i$ is a separable extension, and thus $[K_i:k]=p^w\cdot [K_i:k]_{\text{sep}}$ for each $i$. 

Note that since the conductor ideal of $R\rightarrow \overline{R}$ has height $1$, some power of $J$ must be inside $\mfc$. Thus, some Frobenius image of $J$ must be inside $R$, and so $J$ lies in $S$. Furthermore, $J = J\cap S=\mfn$ is the unique maximal ideal of the local ring $S$ since $J$ is the Jacobson radical of $\overline{R}$ and $S\rightarrow \overline{R}$ is an integral extension.  

The map $S\rightarrow \overline{R}$ induces an injective map $\ell=S/\mfn \rightarrow \overline{R}/J$. Further, by \cite[Corollary~1.5]{LV81} we must have the conductor $S\rightarrow \overline{R}$ is radical, and is $\mfn$-primary since $\dim S = 1$ and $S$ is reduced, so the conductor must be $\mfn$. This implies $J\overline{R} = \mfn S$, so we may apply the isomorphism theorems to see $(\overline{R}/J)/(S/\mfn) \simeq \overline{R}/S$. Consequently, we get a short exact sequence of $k$-vector spaces:\begin{center}
    \begin{tikzcd}
        0\arrow{r} & \ell \arrow{r} & K_1\times \cdots \times K_t \arrow{r} & \overline{R}/S \arrow{r} & 0.  
    \end{tikzcd}
\end{center} This gives the dimension equality \[\dim_k \overline{R}/S = \dim_k K_1\times \cdots \times K_b - \dim_k \ell =  \sum_i [K_i:k] - p^w, 
\] from which we get $\dim_k \overline{R}/S = p^w \sum_i [K_i:k]_{\text{sep}} - p^w = p^w(b(R)-1)$, as required.
\end{proof}
...
Let $R$ be an excellent, reduced ring of dimension one. Then, $R_\mfm$ is geometrically unibranched for each $\mfm \in \Max R$ if and only if $R$ is $F$-nilpotent.
\end{scor}

The arguments involved in the proof of \Cref{thm: counting branches main thm} immediately give the following corollary. 
...
The formula of Singh-Walther uses the \textit{semi-stable part} $[H]_{ss}$ of $H=H^1_\mfm(R)$ whose definition we will not reproduce here. They show that $\dim_k [H]_{ss} = b(R)-1$ (assuming $(R,\mfm)$ is a one dimensional complete local ring with an algebraically closed coefficient field), just as we show $\dim_k 0^*_{H}/0^F_{H} = b(R)-1$. Thus, the $k$-vector spaces $[H]_{ss}$ and $0^*_H/0^F_H$ are isomorphic in their setting. In general, computing the space $[H]_{ss}$ can be quite difficult. On the other hand, we give an explicit formula to compute the $k$-vector space generators of $0^*_H/0^F_H$ when $R$ is standard graded over a perfect field $k$; see \Cref{lem: tight and frob closure of reduction of m}. 
\end{srmk}

The following result is likely well-known and is independent of the characteristic of the field; it follows from the projective Nullstellensatz. In contrast, it does not seem to be easily recoverable from the main result of Singh-Walther (\cite{SW08}) in dimension one. 

\begin{scor} 
Let $k$ be an infinite perfect field of positive characteristic. A reduced, standard graded algebra over $k$ with Hilbert-Samuel multiplicity $e$ must have $e$ geometric branches.
\end{scor}

\begin{proof}
Let $R$ be a reduced, standard graded $k$-algebra with homogeneous maximal ideal $\mfm$. For all $n\gg 0$, we have $e=\dim_k \mfm^n /\mfm^{n+1}$. Let $x\in [R]_1$ be a linear form which reduces $\mfm$, then for all $n$, $x^n$ is a part of a minimal generating set of the ideal $\mfm^n$. Let $J=(x^n)+\mfm^{n+1}$; we have a short exact sequence of $k$-vector spaces: \begin{center}
    \begin{tikzcd}
        0\arrow{r} & J/\mfm^{n+1} \arrow{r} & \mfm^n/\mfm^{n+1} \arrow{r} & \mfm^n/J \arrow{r} & 0.
    \end{tikzcd}
\end{center} 

Hence, $e-1 = \dim_k \mfm^n/\mfm^{n+1} - 1  = \dim_k \mfm^n/J$. But then by \Cref{lem: tight and frob closure of reduction of m} and \Cref{thm: counting branches main thm}, we know $\mfm^n/J =(x^n)^*/(x^n)^F$, so we have $b(R)=e$. 
\end{proof}

We conclude this section by using \Cref{thm: counting branches main thm} and \Cref{cor: counting dimension for tight mod frob} to count the number of geometric branches of some reduced, one dimensional local rings. 

\begin{sex} \label{ex1: dimension count example 1}
Let $k$ be a perfect field of characteristic $p>0$ and let $S=k[x_1,\cdots,x_d]$ and $I=(x_ix_j \mid i<j)$ with $R=S/I$ localized at the maximal ideal $\mfm = (x_1, \ldots, x_d)$. 

Notice that $x=x_1+\cdots+x_d$ is a parameter element of $R$, and for any $n \ge 1$ we have $x^n = x_1^n + \cdots + x_d^n$ and $\mfm^n = (x_1^n,\cdots,x_d^n)$.
...
We immediately see that $x \in R^\circ$, and $x \cdot x_i^{np^e} = x_i^{np^e+1} = x_i \cdot x^{np^e}$, so that $x_i^n \in (x^n)^*$ for each $i$. Consequently, $\mfm^n \subset (x^n)^*$ and by degree considerations, we must have $(x^n)^* = \mfm^n$. Then, $0^*_{H^1_\mfm(R)} = \varinjlim \mfm^n/(x^n) = \varinjlim (x^n,x_2^n,\cdots,x_d^n)/(x^n)$. We claim that $B:=\{[x_i+(x)]\mid 2 \le i \le d\}$ is a $k$-basis of $0^*_{H^1_\mfm(R)}$. 

To see this, note that for any $[\sum_i \alpha_i x_i^n + (x^n)] \in 0^*_{H^1_\mfm(R)}$, we can write $[\sum_i\alpha_i x_i^n+(x^n)] = [\sum_i \alpha_i x_i + (x)]$ as $x_i^n = x_i\cdot x^{n-1}$ and multiplication by $x$ is injective on the direct limit system defining $H^1_\mfm(R)$ as $R$ is Cohen-Macaulay. Thus, $B$ is a spanning set for $0^*_{H^1_\mfm(R)}$. Since $x,x_2,\cdots,x_d$ are minimal generators of $\mfm$, $B$ is also linearly independent. Consequently, $\dim_k\, 0^*_{H^1_\mfm(R)}/0^F_{H^1_\mfm(R)} = d-1$. It follows that $R$ has $d$ geometric branches by \Cref{thm: counting branches main thm}.
\end{sex}

\begin{sex}
Let $k$ be a perfect field of characteristic $p>0$ and fix $d>1$ such that $p$ does not divide $d$. Let $R=k[x,y]/(x^d+y^d)$ localized at $\mfm=(x,y)$. Then, $y$ is a reduction of $\mfm$ and $y^d$ is a parameter element in the conductor ideal of $R$. We have $(y^d)^F=(y^d)+\mfm^{d+1}$ and $(y^d)^* = \mfm^d$ by \Cref{lem: tight and frob closure of reduction of m}. Since $\mfm^d$ is minimally generated by the elements $y^d,y^{d-1}x,\cdots,yx^{d-1}$, we have \[\dim_k (y^d)^*/(y^d)^F = \dim_k \mfm^d/((y^d)+\mfm^{d+1}) = d-1.\] Consequently, $R$ has $d$ geometric branches by \Cref{cor: counting dimension for tight mod frob}.
\end{sex}

\begin{srmk}
Geometric unibranchedness does not characterize $F$-nilpotence in general. Any normal local domain with a separably closed residue field must be geometrically unibranched, but need not be $F$-nilpotent. For a particular example, let $k$ be a separably closed field of prime characteristic $p$ and let $R=k[[x,y,z]]/(x^2+y^3+z^7+xyz)$. Then $R$ is an $F$-injective normal local domain but is not $F$-nilpotent (as it is not $F$-rational). 

Further note that since $R$ is not $F$-nilpotent, clearly $0^*_{H^1_\mfm(R)}\neq 0^F_{H^1_\mfm(R)}$ while $R$ is geometrically unibranched. So, the formula to count the number of geometric branches does not immediately extend to higher dimensions. It would be interesting to find an extension of our formula for local rings of higher dimensions in positive characteristic.
\end{srmk}


\section{Frobenius test exponents and purely inseparable extensions}\label{sec: Fte and pi exts}

In this section, we work towards using purely inseparable extensions to compute tight and Frobenius closure. First, we record a theorem regarding the Frobenius test exponent of an ideal extended or contracted along a purely inseparable extension.

\begin{sthm}\label{thm: computing fte along pi exts}
Let $R\rightarrow S$ be a purely inseparable ring extension with finite pure inseparability index $e_0$, and let $I\subset R$ and $J\subset S$ be ideals. Then, $\fte I\le \fte IS +e_0$ and $\fte J\le \fte F^{e_0}(J)R +e_0$. In particular, if $(R,\mfm)$ and $(S,\mfn)$ are local, then $$\fte R \le \fte S+e_0 \text{ and } \fte S \le \fte R + e_0.$$
\end{sthm}

\begin{proof}
Write $I=(x_1,\cdots,x_n)R$. Then, for any $x \in I^F$, we have $x \in I^FS \subset (IS)^F$, so for $e = \fte IS$, we have an equation $x^{p^e} = \sum s_i x_i^{p^e}$ for some $s_i \in S$. Further, since $F^{e_0}(s_i)\in R$, we get $x^{p^{e+e_0}} = \sum s_i^{p^{e_0}} x_i^{p^{e+e_0}}$ is a Frobenius closure equation in $R$, thus $\fte I \le e+e_0 = \fte IS + e_0$.

Similarly, write $J=(y_1,\cdots,y_m)S$. Then, for any $y \in J^F$ we have an $e \in \mbn$ and $s_1,\cdots,s_m$ in $S$ with $y^{p^e} = \sum s_i y_i^{p^e}$. By applying $F^{e_0}$ we get $y^{p^{e+e_0}} = \sum s_i^{p^{e_0}} y_i^{p^{e+e_0}}$ is a Frobenius closure equation demonstrating $y^{e_0} \in ((y_1^{p^{e_0}},\cdots,y_m^{p^{e_0}}) R)^F$. Consequently, we can choose $e=\fte (y_1^{p^{e_0}},\cdots,y_m^{p^{e_0}}) R + e_0$ in our initial equation showing $y\in J^F$ in $S$ independent of $y$.

In the local case, if $\mfq \subset R$ is a parameter ideal, $\mfq S$ is also a parameter ideal of $S$, so $\fte R \le \fte S + e_0$ is shown. Similarly, if $\mathfrak{r}$ is a parameter ideal of $S$, then $F^{e_0}(\mathfrak{r})R$ is a parameter ideal of $R$, so $\fte S \le \fte R + e_0$. 
\end{proof}

This theorem demonstrates that when determining whether all  local rings have finite Frobenius test exponent, it suffices to consider weakly normal rings.
...
\begin{srmk}\label{rmk: pi exts and R circ} Let $\phi:R\rightarrow S$ be a purely inseparable extension. Notice for all $\mfp \in \Min S$, $\phi^{-1}(\mfp) \in \Min R$ since $\phi$ induces a homeomorphism on spectra. Consequently, $\phi(R^\circ)\subset S^\circ$ and for each $x \in S^\circ$ there is an $e \in \mbn$ such that $x^{p^e} \in R^\circ$. \end{srmk}

In general, if $R\rightarrow S$ is purely inseparable, we can expect similarities between the singularities of $R$ and $S$. Heuristically, we should expect a nilpotent version of the singularity type of $S$ to descend to $R$. In the context of ideal closures, closure properties which hold in $S$ should also hold in $R$ up to Frobenius closure. The following theorem demonstrates one example of this principle, with an application to affine semigroup rings.

\begin{sthm}\label{thm: sf-nilp if pi ext is freg}
Let $\phi:R\rightarrow S$ be a finite, purely inseparable extension with pure inseparability index $e_0$, and suppose $S$ is weakly $F$-regular. Then, for all ideals $I\subset R$, $I^*=I^F$ and $\fte I \le e_0$.
\end{sthm}

\begin{proof}
Let $I\subset R$ be an ideal and suppose $I=(x_1,\cdots,x_n)R$. Then, $I^*S\subset (IS)^*=IS$ since $\phi(R^\circ)\subset S^\circ$, so for any $x \in I^*$, we have $x=\sum s_i x_i$ for some $s_1,\cdots,s_n \in S$. Then, $x^{p^{e_0}} = \sum s_i^{p^{e_0}} x_i^{p^{e_0}}$ is a Frobenius closure equation in $R$, so $x \in I^F$. Further, $e_0$ is independent of $x$, so $\fte I\le e_0$.
\end{proof}

\begin{srmk}
A variety of modifications to \Cref{thm: sf-nilp if pi ext is freg} can be made by replacing the requirement that $S$ be weakly $F$-regular with another condition defined in terms of ideal closures. For instance, if $R\rightarrow S$ is finite and purely inseparable with pure inseparability index $e_0$, and $S$ is $F$-pure so that $\fte IS = 0$ then $\fte I \le e_0$.
\end{srmk}
...
Finally, if $A$ is an affine semigroup and $R=k[A]$ is the associated affine semigroup ring, then $\overline{R}$ is a direct summand of a polynomial ring and is thus $F$-regular. Then, we may apply the proof above to $R$, as affine semigroup rings are excellent domains.
\end{proof}

Note that in the proof above, we only need the weaker assumption that $\overline{R}$ is $F$-nilpotent (and not necessarily $F$-regular) to conclude that $R$ is $F$-nilpotent if and only if $R\rightarrow \overline{R}$ is purely inseparable. Furthermore, we note the rings described above avoid the problem raised by Brenner in \cite[Theorem~2.4]{Bre06}, that is, they have a uniform trivializing exponent for Frobenius closure over all ideals simultaneously. 

\begin{srmk}\label{rmk: f-coherent}
An additional class of rings $R$ with the property that $I^*=I^F$ for all ideals $I\subset R$ are \textit{$F$-coherent} rings, whose definition we avoid here. The class of $F$-coherent rings were studied by Shimomoto in \cite{Shi11}. We note the property that $I^*=I^F$ for all ideals $I$ does not characterize $F$-coherent rings, since there are (strongly) $F$-regular rings (in which $I^*=I^F=I$ for all ideals $I$) which are not $F$-coherent, see \cite[Example~3.14]{Shi11}.
\end{srmk}

In \cite{MP22}, the latter two authors of this paper studied a family of examples of $F$-nilpotent affine semigroup rings called \textit{pinched Veronese rings}, formed by removing a single algebra generator from a Veronese subring of a polynomial ring. All but one small family of examples of pinched Veronese rings are $F$-nilpotent, and with combinatorial techniques we showed that the number $e_0$ in the corollary above is $1$ (see \cite[Theorem B, Corollary 4.9]{MP22}). This vastly improves the previously known bounds for the Frobenius test exponents of these rings $R$, which were roughly of the order $\fte R \le 2^{\dim R}$ by \cite[Theorem~4.2]{Quy}. 
...
In \cite[Theorem B]{MP22}, the latter two authors of this paper showed that the pure insperability of the normalization map $\iota: R\rightarrow \overline{R}$ depends on the characteristic $p$ of $k$. In particular, $\iota$ is purely inseparable if and only if $p=2$. Further, if $p=2$, then the pure inseparability index of $\iota$ is $1$. Thus, by \Cref{cor: f-nilp affine semigroup ring}, $R$ is $F$-nilpotent if and only if $p=2$, and in this case, for all ideals $I$ of $R$, $x \in I^*$ if and only if $x^p \in I^{\fbp{p}}$.
...
The authors would like to thank Linquan Ma, Anurag Singh, and Uli Walther for several helpful discussions. We are also grateful to Austyn Simpson for discussions related to \Cref{rmk: f-coherent}.
