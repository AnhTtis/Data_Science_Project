...
Keywords: Gas Electron Multiplier~(GEM), High Energy Physics~(HEP), Micro Pattern Gas Detector~(MPGD), Gain, Divider current, Long-term stability
...
In order to nullify the effect of temperature~(T = t + 273 K) and pressure~(p in mbar) variations on the gain, it is normalized using the following relation~\cite{pisa};
...
S. Chatterjee would like to thank Mr. Pranjal Barik and Mr. Ayan Dandapat for their help in setting up the measurement setup. The authors would like to thank Mr. Subrata Das for designing the collimator used in this study. 
