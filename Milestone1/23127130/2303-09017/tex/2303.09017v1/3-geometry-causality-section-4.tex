...
As a concrete example, we look at space $\Theta_{33}$ from Section \ref{section:spaces}.
...
As an example not fitting this mold, consider space $\Theta_{101}$ from the Section \ref{section:spaces}:
\begin{center}
    \begin{tabular}{cc}
...
\begin{itemize}
    ...
    \end{equation*}
\end{itemize}
Compatible families are families of functions which are compatible in the sense of Subsection \ref{subsection:spaces-pfuns}: their gluing is given by their compatible join whenever the compatible join is causal, and no gluing exists otherwise (cf. Theorem 4.30 p.56 \cite{gogioso2022topology}).
\end{definition}
...
\]
Because the space $\Theta$ is both tight and causally complete, the causal functions on $\downset{\hist{A/i_A, B/i_B, C/i_C}}$ take the following form:
\[
...
An \emph{open cover}, or simply a \emph{cover}, for $X$ is an antichain in the partial order $\mathcal{T}(X)$, i.e. a collection $\mathcal{C} \subseteq \mathcal{T}(X)$ of open sets which are incomparable:
\[
