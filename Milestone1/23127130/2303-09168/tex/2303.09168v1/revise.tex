...
Here $C_s$ is the split para-octonion algebra over $k\llp t\rlp$ obtained from $C$, and $\star$ is the multiplication in $C_s$. The element $e\in C_s$ is the para-unit. We refer \S 2 for more details about composition algebras. This theorem is shown in \S 5. 	

For the other two types of parahoric subgroups, we have similar results. By fixing the parahoric subgroups $\scrG_2$ (resp. $\scrG_3$) in $G=G_2$ (see Definition \ref{def 63}), our second main theorem is the following:
\quash{
...
\section{Octonion Algebras and Para-Octonion Algebras}	
...
\begin{rk}\label{rk 32}{\rm
	...
	\]
	(See \cite[Remark 3.1]{AG}). It is also easy to get the above isomorphism from Theorem \ref{thm 31} by regarding $x\star y=\bar{x}\cdot \bar{y}$. Since it is more clear to view the spin group as the subgroup of $\SSO_8^{\times 3}$ preserving multiplication, we prefer to use para-octonion algebras in the following sections. 
}\end{rk}

From Theorem \ref{thm 31}, it is easy to see that there is an action $\rho$ on $\SSpin(C_s,\star)$ given by 
\[
...
\]
The involution $\theta$ and the $\ZZ/3\ZZ$-action $\rho$ generate an action $S_3$ on $\SSpin(C_s,\star)$, whose group of fixed points, is precisely the exceptional group $G_2$:
...
\]
for any $F$-algebra $R$, $x,y\in C_s\otimes_{F} R$. The group $G$ is of type $G_2$.
\end{prop}
...
\begin{rk}\label{rk 34}{\rm
	In \cite{GY1}, Gan-Yu define that an order $L$ in the split octonion algebra $(C_s,\cdot)$ is an $\calO_F$-lattice which is a unital ring. We transfer this definition to the para-octonion algebra $(C_s,\star)$ here. Notice that any order is closed under conjugation, so that the condition $x\cdot y\in L$ is equivalent to 
	\[
...
\begin{itemize}
	\item type $1$: $L$ is a maximal order in $C_s$, i.e., $L$ is self-dual, and closed under multiplication. The corresponding graded lattice chain is a pair $(\{\pi^i L\},c)$:
	\[
	\cdots\subsetneq\pi L\subsetneq L\subsetneq \pi^{-1}L\subsetneq\cdots, ~\text{where}~c(L)=0.
	\]
	...
	\[
	\cdots\subsetneq \pi L^\vee \subsetneq L\subsetneq L^{\vee}\subsetneq \pi^{-1}L \subsetneq\cdots, ~\text{where}~c(L)=0, ~c(L^\vee)=-\frac12.
	\]
	...
	\[
	\cdots\subsetneq \pi L^\vee\subsetneq L\subsetneq M \subsetneq L^{\vee}\subsetneq \pi^{-1}L \subsetneq\cdots, ~\text{where}~c(L)=0, ~c(M)=-\frac13,~c(L^\vee)=-\frac23.
	\]
 \end{itemize}
\end{thm}
...
Let $(C_s,\star)=\Lambda\otimes_{\ZZ}F$ be the split para-octonion algebra, with the basis $e_1,e_2, \{u_i, v_i\}_{i=1,2,3}$. The multiplication table with respect to this basis is in Table \ref{tab 1}.

(1). The trivial example is the lattice 
\[
\LL_1=\calO_F\bb e_1, e_2, u_1, u_2, u_3, v_1, v_2, v_3\pp.
\] 
It is easy to see that $\LL_1$ is self-dual, and closed under multiplication table, so $\LL_1$ is the vertex in $\calB(G)$ of type 1. We call $\LL_1$ the standard lattice of type 1.
...
\section{Affine Grassmannians for $\scrG$}

In this section, we will define affine Grassmannians by loop groups.  The affine Grassmannians is represented by an ind-scheme. We refer \cite{BL} \cite{BD} for important results on the structure of loop groups and associated affine Grassmannians.
...
In \cite{PR3}, Pappas-Rapoport develop this definition for ``twisted" loop groups as follows: Consider $G$ an algebraic group over $\Spec(k\llp t\rlp)$. The algbraic loop group $LG$ is the functor form the category of $k$-algebras to the sets given by
\[
LG(R)=G(R\llp t\rlp).
\]
...
\]
Notice that when $G =G_0\otimes_k k\llp t\rlp$, we recover the previous definition in the untwisted case. Since the generic fiber $\scrG_\eta$ of $\scrG$ is always equal to $G$, our affine Grassmannian $\Gr_\scrG$ associated to $\scrG$ is the fpqc-sheaf $GL\scrG_\eta/L^+\scrG=LG/L^+\scrG$. We refer to \cite{PR3} for more details. 
...
\[
\{L \mid \text{$L$ is a finitely generated projective  $R\lp t\rp$-modules in $R\llp t\rlp^n$, such that $L[t^{-1}]\simeq R\llp t\rlp^n$}\}.
\]

In the rest of this section, let $k$ be a field with char$(k)\neq 2$. The field $F=k\llp t\rlp$ is a discrete valuation field with the ring of integers $\calO_F=k\lp t\rp$, and uniformizer $t\in k\lp t\rp$. Let $(C_s,\star)=\Lambda\otimes_{\ZZ}F$ be the split para-octonion algebra over $F$, and $G=\AAut(C_s,\star)$ be the automorphism group scheme of split para-octonion algebra, which is a group of type $G_2$. Consider the parahoric subgroup $\scrG$ corresponding to hyperspecial points in $\calB(G)$ as defined in Definition \ref{def 37}. Then the quotient fpqc-sheaf 
\[
\Gr_{\scrG}=L\scrG_\eta/L^+\scrG
\] 
is by definition the affine Grassmannian for  $G_2$.

Our goal in this section is to give an explicit description of the affine Grassmannian $\Gr_{\scrG}$. Inspired by the results from \cite{GY1}, we already know the $k$-points in $\Gr_{\scrG}$: since $L^+\scrG(k)=\scrG(k\lp t\rp)$ is the stabilizer of the standard lattice $\LL$, and $L\scrG_\eta(k)=G(k\llp t\rlp)$ acts transitively on vertices of type 1, then there is a bijection between $k$-points of $\Gr_{\scrG}$ and the set of maximal orders in $C_s$. Our first main theorem is the following:
...
This theorem is proven in the next section. It gives a bijection between $R$-points in the affine Grassmannian for  $\scrG$ and a certain set of $R\lp t\rp$-lattices in $C_s\otimes_{k\llp t\rlp} R\llp t\rlp$ that are self-dual and closed under multiplication.
...
In \cite{PR3}, Pappas-Rapoport show that there is a bijection between the affine Grassmannians for unitary groups and the space of self-dual lattice chains for a hermitian form. Recall that $\LL_1=k\lp t\rp \bb e_1, e_2, u_i, v_i\pp_{i=1,2,3}$ is the standard lattice of type 1 in the split para-octonion algebra $C_s=\Lambda\otimes_{\ZZ}k\llp t\rlp$. Similar to the proof of \cite[Theorem 4.1]{PR3} in the unitary case, it suffices to check the following two statements:
	\begin{itemize}
		\item [(i)] For any $R$, the subgroup of $L\scrG_\eta(R)$ consisting of elements that stabilize the standard lattice $\LL_1\otimes_{k\lp t\rp}R\lp t\rp$ agrees with $L^+\scrG(R)$. 
...
It is easy to see part (i). By definition, the $R\lp t\rp$-points of $\scrG$ is the stabilizer of the base change of the standard lattice $\LL_1\otimes_{k\lp t\rp}R\lp t\rp$ (we also call $\LL_1\otimes_{k\lp t\rp}R\lp t\rp$ the standard lattice in this section if there is no confusion). We will focus to prove part (ii). The key is to find a basis in $L\in\scrF(R)$, such that its multiplication table is the same as Table \ref{tab 1} (the multiplication table of the standard lattice $\LL_1\otimes_{k\lp t\rp}R\lp t\rp$). If this basis exists, then there is an element $g\in \SL_8$ such that $g(\LL_1\otimes_{k\lp t\rp}R\lp t\rp)=L$, with $g(x\star y)=g(x)\star g(y)$. By Lemma \ref{lm 28}, the quadratic form $q$ is determined by its multiplication, so that $g$ also preserves the form $q$. Thus, $g\in\SSO(C_s,\star)(R\llp t\rlp)$, and so that $g\in G(R\llp t\rlp)=L\scrG_{\eta}(R)$.

For any $L\in\scrF(R)$, by Proposition \ref{prop 33}, $L$ is a maximal order in $C_s\otimes_{k\llp t\rlp}R\llp t\rlp$, which contains a para-unit element $e\in L$. Since $L$ is a self-dual lattice, we have $L\simeq \Hom_{R\lp t\rp}(L, R\lp t\rp)$. So there exists a basis $\{x_i\}_{i=1}^8$ of $L$ such that $\bb x_i,x_{9-j}\pp=\delta_{ij}$. Consider $e=\sum a_i x_i$ for all $a_i\in R\lp t\rp$. We set:
\[
...
\begin{flalign*}
			...
			&=0.
\end{flalign*} 
Also $e_2\star e_1=0$ by similar calculation. Therefore, the sublattice $R\lp t\rp e_1\oplus R\lp t\rp e_2$ is a hyperbolic space satisfying:
\[
\begin{array}{l}
...
\end{array}
\]  
Readers may notice that here $e_1, e_2$ play the same roles as for $e_1, e_2$ in the Table \ref{tab 1} (that is the reason we keep the same symbol). We claim that $e_1, e_2$ are primitive elements in $L$ (elements that extends to a basis of $L$): Since $(R,\calM)$ is a local ring, the canonical quotient map:
\[
R\lp t\rp\rightarrow R\rightarrow \kappa=R/\calM
\]
gives a base change of $L\rightarrow L\otimes_{R\lp t\rp}\kappa$. The image of $e_1, e_2$ in $L\otimes_{R\lp t\rp}\kappa$ are non-zero elements by $\bb e_1,e_2\pp=1$. By Nakayama's lemma, we can extend $\{e_1, e_2\}$ as a basis of $L$. So they are primitive elements. Choose $y_3,...,y_8\in L$ such that $\{e_1, e_2, y_3, ... , y_8\}$ is a basis of $L$. Consider the sublattice $L_0\subset L$:
\[
...
\begin{proof}
	(1). For any $x\in L_1$, we can write $x$ as $x_1\star e_1$ for $x_1\in L_0$. By the definition of symmetric composition algebra,
	\[\begin{array}{l}
	...
	(2). Again, we only consider $L_1$ case. For any $x=x_1\star e_1\in L_1$, we have $e_1\star(x_1\star e_1)=q(e_1)x_1=0$ by Lemma \ref{lm 28}. Hence $e_i\star L_i=0$. For $(x_1\star e_1)\star e_2$, consider
	\[
	(x_1\star e_1)\star e_2+(e_2\star e_1)\star x_1=\bb x_1,e_2\pp e_1.
	\]
	Since $e_2\star e_1=0$, and $\bb x_1,e_2\pp=0$, we get $x\star e_2=(x_1\star e_1)\star e_2=0$ for all $x\in L_1$.
\end{proof} 
...
\end{flalign*} 
by Lemma \ref{lm 28}. Since $e_1\star x, e_2\star x\in L_0$, we get $L_0=L_1+L_2$. Each $L_i$ is totally isotropic with rank $\leq 3$, so that $L_0=L_1\oplus L_2$. Since $L_1, L_2$ are totally isotropic, and $\bb \ ,\ \pp$ restricted to $L_0$ is nondegenerate, we see that $L_1, L_2$ are in duality: Consider the isomorphism $L_1\rightarrow L_2^\vee$ given by $x\mapsto \bb x,-\pp$, we have $L_1\simeq \Hom_{R\lp t\rp}(L_2,R\lp t\rp)$. Set the basis for $L_i$:
\[
...
\begin{proof}
We can directly get these equations from Lemma \ref{lm 51} (2): Since for any $x\in L_0$, we have $e\star x=x\star e=-x$. Thus $u_i\star e=u_i\star (e_1+e_2)=-u_i$. By $u_i\star e_2=0$, we obtain $u_i\star e_1=-u_i$. Similarly, we get the rest equations.
\end{proof} 

So far, we find the basis $e_1,e_2,\{u_i,v_i\}_{i=1,2,3}$ of $L$. To show that the multiplication table of this basis is the same as Table \ref{tab 1}. It remains to check $u_i\star v_j$ (resp. $v_i\star u_j$), and $u_i\star u_j$ (resp. $v_i\star v_j$) for $i=1,2,3$. Let us check $u_i\star v_j$ first. Consider
\begin{flalign*}
u_i\star v_j &=-(u_i\star e_1)\star v_j\\
             &=	(v_j\star e_1)\star u_i-\bb u_i,v_j\pp e_1\\
             &= -\delta_{ij}e_1.
\end{flalign*}
The first row in the above equation comes from Lemma \ref{lm 52}, and the second row comes from Lemma \ref{lm 28}. Similarly, we also have 
\[
v_i\star u_j=-\delta_{ij}e_2.
\]
for $i=1,2,3$. Finally, for $u_i\star u_j$ (resp. $v_i\star v_j$), we claim that $u_i\star u_j\in L_0$. It is easy to see that $\bb u_i\star u_j,e_1\pp=\bb u_j\star e_1, u_i\pp=-\bb u_j,u_i\pp$, where $\bb u_j,u_i\pp=0$ since $L_i$ is totally isotropic. Also, $\bb u_i\star u_j,e_2\pp=\bb u_j\star e_2, u_i\pp=0$ by $u_j\star e_2=0$. Thus, $u_i\star u_j\in L_0$. We can also get $v_i\star v_j\in L_0$ by the same way. Moreover, consider
\begin{flalign*}
	(u_j\star u_i)\star e_2+(e_2\star u_i)\star u_j=\bb u_j,e_2\pp u_i=0
\end{flalign*}
by Lemma \ref{lm 28}. We have
\[
...
\end{array}
\] 
by Lemma \ref{lm 27}. We get $\bb u_1\star u_2, u_1\pp=\bb u_1\star u_2, u_2\pp=0$ by $L_1$ totally isotropic, which implies 
\[
u_1\star u_2=\lambda v_3
\] 
for some $\lambda\in R\lp t\rp$. Multiplying $v_1$ on the right side, we get $(u_1\star u_2)\star v_1=\lambda(v_3\star v_1)$. Since $(u_1\star u_2)\star v_1+(v_1\star u_2)\star u_1=\bb u_1,v_1\pp u_2$,, and $v_1\star u_2=0$, $\bb u_1,v_1\pp=1$, we have $(u_1\star u_2)\star v_1=u_2$. Hence $v_3\star v_1= \lambda^{-1} u_2$. Therefore, $\lambda, \lambda^{-1}\in R\lp t\rp$ by $L\star L\subset L$, which implies $\lambda$ is a unit in $R\lp t\rp$. We can assume $\lambda=1$, so that 
\[
u_1\star u_2=v_3. 
\]
...
Therefore, we complete the multiplication table of $L$, which it is the same as 	Table \ref{tab 1}. So there exist $g\in L\scrG_\eta(R)$, such that $L=g(\LL_1\otimes_{k\lp t\rp} R\lp t\rp)$. That proves Theorem \ref{thm 41}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	

\section{Affine Grassmannians for Other Parahoric Subgroups}
	
So far, we described the affine Grassmannians for $\scrG$, where $\scrG$ is the maximal parahoric subgroups corresponding to hyperspecial points in the building $\calB(G)$. We can view $\scrG$ as the stabilizer subgroups of the standard lattice $\LL_1$ (type 1). According to Theorem 3.6, there are two other maximal parahoric subgroups up to conjugation, corresponding to vertices of type 2 (resp. type 3) in the building $\calB(G)$. It is natural to ask if we can describe the affine Grassmannians for these parahoric subgroups.

To answer this question, we first need to fix the parahoric subgroup that we want. The notation here is the same as that in \S 5. That is: $F=k\llp t\rlp$ is a discrete valuation field with the ring of integers $\calO_F = k\lp t\rp$. Let $(C_s, \star)=\Lambda\otimes_\ZZ F$ be the split para-octonion algebra over $F$. Consider the lattice $\LL_2\subset C_s$ in Example 3.7:
\[
...
\]
It is easy to see that $\LL_2$ satisfies $e\in \LL_2$, $\LL_2\star \LL_2\subset \LL_2$, $\LL_2\subsetneq \LL_2^{\vee}\subsetneq t^{-1}\LL_2$, and $\LL_2^{\vee}\star \LL_2^{\vee}\subset t^{-1}\LL_2$. The corresponding graded lattice chain 
\begin{equation}\label{eq 61}
	\cdots\subsetneq t\LL_2^{\vee}\subsetneq \LL_2\subsetneq \LL_2^{\vee}\subsetneq t^{-1}\LL_2\subsetneq \cdots
\end{equation}
is the vertex of type 2 in $\calB(G)$. Similarly, the lattice $\LL_3$ satisfies $e\in \LL_3$, $\LL_3\star \LL_3\subset \LL_3$, $\LL_3\subsetneq \LL_3^{\vee}\subsetneq t^{-1}\LL_3$, and $M=t\LL_3^{\vee}\star \LL_3^{\vee}+\LL_3$ is self-dual. The corresponding graded lattice chain
...
{\begin{rk}\label{rk 64}\rm
	...
	For $g\in\scrG_3(k\lp t\rp)$, we get $g(\LL_3^{\vee})=\LL_3^{\vee}$ similarly as above. and $g(M)=tg(\LL_3^{\vee}\star \LL_3^{\vee})+g(\LL_3)$. Since $g$ is a change of basis of $\LL_3^{\vee}$, we have $g(\LL_3^{\vee}\star \LL_3^{\vee})=\LL_3^{\vee}\star \LL_3^{\vee}$. Thus $g(M)=M$, and $g$ preserves the graded lattice chain (\ref{eq 62}).
\end{rk}
}
...
Notice that for any $k$-algebra $R$, $\calF_2(R)\cap\calF_3(R)=\emptyset$. If $L\in\calF_2(R)$, we have $L^{\vee}\star L^{\vee}\subset t^{-1}L$, and $M=tL^{\vee}\star L^{\vee}+L=L$ is not self-dual. So that $L\notin\calF_3(R)$.

The proof of Theorem \ref{thm 65} is similar to what we did in \S 5, i.e., we want to check to following two statements:
\begin{itemize}
	\item For any $k$-algebra $R$, the subgroup of $LG(R)$ consisting of elements that stabilize the lattice $\LL_2 \otimes_{k\lp t\rp} R\lp t\rp$ (resp. $\LL_3 \otimes_{k\lp t\rp} R\lp t\rp$) agrees with $L^+\scrG_2 (R)$ (resp. $L^+\scrG_3 (R)$).
	\item For any $L\in\calF_2(R)$ (resp. $L\in\calF_3(R)$) with $(R,\calM)$ a local henselian ring with maximal ideal $\calM$, there exists $g\in LG(R)$ such that $L = g(\LL_2 \otimes_{k\lp t\rp} R\lp t\rp)$ (resp. $L = g(\LL_3 \otimes_{k\lp t\rp} R\lp t\rp)$).
\end{itemize}

The first statement comes directly from Definition \ref{def 63}. To prove the second statement, we use the same idea that we did in \S 5: Find a basis in $L\in\calF_2(R)$ (resp. $L\in\calF_3(R)$), such that its multiplication table is the same as the multiplication table of $\LL_2 \otimes_{k\lp t\rp} R\lp t\rp$ (resp. $\LL_3 \otimes_{k\lp t\rp} R\lp t\rp$). The hard part is that we cannot find a basis $\{x_i\}_{i=1}^8$ in $L$ such that $\bb e_i,e_{9-j}\pp=\delta_{ij}$, since $L$ is not a self-dual lattice anymore. We now turn to find ``standard forms" of these lattice chains.

By \cite[Appendix, Lemma A.21]{RZ}, after a finite  unramified base change $F'/F$, there exists a ``standard form" of lattice $L\otimes_{\calO_F}\calO_{F'}$. Generally, let $V$ be an $F$-vector space with dimension $d=2n$ or $d=2n+1$ (we assume $d\geq 5)$, equipped with  a non-degenerate symmetric form $\bb  ~,~\pp$. Consider a lattice $L\subset V$ satisfying $L\subset L^{\vee}\subset t^{-1}L$ with $l+l'=d$. Here $l$ (resp. $l'$) is the length $l=lg(L^{\vee}/L)$ (resp. $l'=lg(t^{-1}L/L^{\vee}))$. We assume $l\geq 1$ and $l'\geq 1$, i.e., $L\neq L^{\vee}$ and $L^{\vee}\neq t^{-1}L$. Since octonion algebras have dimension 8, we only list results for even dimension case.

For $d=2n$,  after a finite unramified extension, we can find a basis $\{x_i\}$ of $L_{\calO_{F'}}=L\otimes_{\calO_F}\calO_{F'}$ as in one of the following cases:
\begin{itemize}
	\item [(1)] Split form: when $l=2r$, we have $L_{\calO_{F'}}=\oplus_{i=1}^d\calO_{F'}\cdot x_i$ with 
	\begin{flalign*}
		\bb x_i,x_{d+1-j}\pp &=\delta_{ij}, ~\text{for}~ i\notin [n-r+1,n+r];\\
		\bb x_i,x_{d+1-j}\pp &=t\delta_{ij}, ~\text{for}~ i\in [n-r+1,n+r].
	\end{flalign*}
	\item [(2)] 	Quasi-split form: when $l=2r+1$, we have $L_{\calO_{F'}}=\oplus_{i=1}^d \calO_{F'}\cdot  x_i$ with 
	\begin{flalign*}
		&\bb x_i,x_{d+1-j}\pp =\delta_{ij}, ~\text{for}~ i\notin [n-r,n+r+1];\\
...
In our case, $L$ is a lattice in  $C_s$ with $d=8$. No matter $L\in \calF_2(R)$ or $L\in \calF_3(R)$, we always have $L\subsetneq L^{\vee}\subsetneq t^{-1}L$. Hence $l\geq 1$ and $l'\geq 1$. We claim that $L\subset C_s$ is of split form.
...
\begin{itemize}
	...
	\item when $l=4$, we claim that there exists a basis of $L$, such that its multiplication table is the same as multiplication table in $\LL_2 \otimes_{k\lp t\rp} R\lp t\rp$. Hence there exists $g\in LG(R)$, such that $L=g(\LL_2 \otimes_{k\lp t\rp} R\lp t\rp)$. Then we get:
	\begin{flalign*}
	...
	\end{flalign*}
	since $L^\vee=g(\LL_2^\vee \otimes_{k\lp t\rp} R\lp t\rp)$. So that $L\in\calF_2(R)$.
	\item when $l=6$, there exists a basis of $L$ such that its multiplication table is the same as  table in $\LL_3 \otimes_{k\lp t\rp} R\lp t\rp$. Similarly, we have $L=g(\LL_3 \otimes_{k\lp t\rp} R\lp t\rp)$ for some $g\in LG(R)$, and we get
	\begin{flalign*}
		&L\star L\subset L;\\
		&M=tL^\vee\star L^\vee+L ~\text{is self-dual},
	\end{flalign*}
since $M=g(t(\LL_3^\vee \otimes_{k\lp t\rp} R\lp t\rp)\star(\LL_3^\vee \otimes_{k\lp t\rp} R\lp t\rp)+(\LL_3 \otimes_{k\lp t\rp} R\lp t\rp))$. So that $L\in\calF_3(R)$.
\end{itemize}

Our claims as above will finish the proof of Theorem \ref{thm 65}, and the proof of our claims is similar to \S 5. 

Let $L$ be a lattice with $e\in L$, $L\subsetneq L^{\vee}\subsetneq t^{-1}L$. No matter $l=lg(L^\vee/L)=2,4$ or $6$, there exists a basis $\{x_i\}$ of $L$, such that the symmetric form is an anti-diagonal matrix with respect to $\{x_i\}$. Consider $e=\sum_{i=1}^8 a_ix_i\in L$. We set:
\[
e_1=\sum_{i=1}^4 a_ix_i,\quad e_2=\sum_{i=5}^8 a_ix_i.
\]
Then $q(e_1)=q(e_2)=0$. We get $\bb e_1,e_2\pp=1$ by $q(e)=1$, and we also have $e_1\star e_1=e_2$, $ e_2\star e_2=e_1$, $e_1\star e_2=e_2\star e_1=0$ by similar calculation that we did in \S 5. Thus, $R\lp t\rp e_1\oplus R\lp t\rp$ is a hyperbolic subspace. By Nakayama's lemma, $\{e_1, e_2\}$ are primitive elements in $L$. Choose elements $y_3,\cdots,y_8\in L$ such that $\{e_1, e_2,  y_3,..., y_8\}$ is a basis of $L$, By replacing $y_i$ to 
\[
y_i'=y_i-\bb y_i,e_2\pp e_1-\bb y_i,e_1\pp e_2,
\]
...
\]
by Lemma 2.8. Hence $L_0=L_1+L_2$. We again get $L_0=L_1\oplus L_2$	since $L_1, L_2$ are totally isotropic with rank $\leq 3$. When we restrict the symmetric form to $L_0$, there exists a basis $\{u_i\}_{i=1}^3$ of $L_1$ (resp. $\{v_i\}_{i=1}^3$ of $L_2$) such that 
\[
\bb u_i,v_j\pp=0,
\]
...
\begin{proof}
The proof of Lemma \ref{lm 66} (1) is similar to Lemma 5.2. For (2), we have
\begin{flalign*}
	u_i\star v_j&=-(u_i\star e_1)\star v_j\\
	            &=(v_j\star e_1)\star u_i-\bb u_i,v_j\pp e_1\\
	            &=-\bb u_i,v_j\pp e_1.
...
\end{flalign*}
for $i\neq j$. By Lemma \ref{lm 66}, Lemma \ref{lm 67}, we almost complete the multiplication table of $L$. The only left part is to understand $u_i\star u_j$ (resp. $v_i\star v_j$).
...
\]
Next we consider $u_2\star u_3\in L_2$, we can assume $u_2\star u_3=\mu v_1$ for some $\mu\in R\lp t\rp$, since $\bb u_2\star u_3, v_2\pp=\bb u_2\star u_3, v_3\pp=0$. Since
\[
(u_2\star u_3)\star v_3+(v_3\star u_3)\star u_2=\bb u_2,v_3\pp u_3,
\]
and $(v_3\star u_3)\star u_2=u_2, \bb u_2,v_3\pp=0$ by Lemma \ref{lm 67} (1), (2), we get $\mu(v_1\star v_3)=-u_2$. But $v_1\star v_3=-v_3\star v_1=-tu_2$ by our discussion above. Therefore
\[
-t\mu u_2=-u_2.
\]
...
It is easy to see that $L$ has the same multiplication table as $\LL_2\otimes_{k\lp t\rp} R\lp t\rp$. Thus, there exists $g\in LG(R)$, such that $L=g(\LL_2 \otimes_{k\lp t\rp} R\lp t\rp)$, and we have:
\[
...
Thus $L$ has the same multiplication table as  $\LL_3 \otimes_{k\lp t\rp} R\lp t\rp$. Similarly, there exists $L=g(\LL_3 \otimes_{k\lp t\rp} R\lp t\rp)$ for some $g\in LG(R)$, and we have
\[
M=tL^\vee\star L^\vee+L ~\text{is self-dual}.
\]
So that $L\in\calF_3(R)$.

Therefore, for $L\in\calF_2(R)$, we show that $l=lg(L^\vee/L)=4$ (if $l=2$, $L\star L\nsubseteq L$; if $l=6$, $L\in\calF_3(R)$), and there exists $g\in LG(R)$, such that $L=g(\LL_2 \otimes_{k\lp t\rp} R\lp t\rp)$. For $L\in\calF_3(R)$, we show that $l=lg(L^\vee/L)=6$, and there exists $g\in LG(R)$, such that $L=g(\LL_3 \otimes_{k\lp t\rp} R\lp t\rp)$.
