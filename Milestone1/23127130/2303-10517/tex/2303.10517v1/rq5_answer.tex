\subsection{Overlap Analysis} \label{ssec:rq5}
In this section, we investigate to which extent tools agree in their judgments.
We use the SWC registry as a common frame of reference and map all findings to an appropriate SWC class, if any.
Clearly, this excludes weaknesses that do not fit any SWC class and properties other than weaknesses.
Most notably, the comparison excludes Honeybadger, as it detects properties characteristic of honeypots.
...
\begin{table}\caption{Overlap of Tool Findings [\%]}  \label{tab:agreement}
\centering
...
\makebox[0em]{%
  \pgfplotstabletypeset[
    ...
    columns/Tool/.style={string type, column type=l},
]{csv/overlaptable.csv}}
\end{table}

\autoref{tab:agreement} shows the overlap between any two tools.
Since eThor detects reentrancy only, its row and column in the table give us an impression of how differently a weakness may be assessed by the tools.
Regarding Vandal, we find the highest values in its column and the lowest in its row. 
This means that most weaknesses it reports are not backed by other tools, a sign of overreporting.
...
Osiris detects not only additional weaknesses, which are not considered in the comparison, but flags additional contracts with weaknesses the tools have in common.
