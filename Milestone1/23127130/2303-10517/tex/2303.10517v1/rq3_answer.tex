\subsection{Tool Quality} \label{sec:rq3}
...
The early tools, released in 2017 and 2018, are tailored to the Ethereum fork at block 4.37\,M and are no longer maintained.
Judging by the last code commits, Mythril is the only tool that is actively maintained: it has 74 contributors, \np{4785} commits, and 709 issues resolved. 
...
The last column, \emph{Prg.issues}, averages over runs with programming issues, like exceptions caused by typing errors.
Clearly, the average time for runs stopped by a timeout is close to \np[s]{1800}.
...
\begin{figure}
\centering
\includegraphics[width=.9\columnwidth]{pics/tool_failure_br.png}
\caption{Tool failures over time. Percentage of failures encountered by the tools, in bins of 100k blocks. Note the stretched verticale scale of the lower plots.
Ethainter and MadMax had no failures and therefore do not show up.}
\label{fig:failures}
\end{figure}
...
A run terminates properly if it is stops under control of the tool, either successfully or with an error condition detected by the tool.
...
Conkas, eThor, Pakala, and teEhter fail most often and are shown at the top, while the other tools appear below at an enlarged scale.
...
\begin{figure}
\centering
\includegraphics[width=.9\columnwidth]{pics/tool_error_br.png}
\caption{Tool errors over time. Percentage of errors reported by the tools, in bins of 100k blocks. Note the stretched y scale of the lower two plots.
Mythril, Oyente and Vandal had no errors and thus are not depicted.}
\label{fig:errors}
\end{figure}

Honeybadger, Maian, and Osiris report a large number of errors (marked red), with the majority related to unknown instructions (mainly the shift operations \OP{SHR} and \OP{SHL}).
This is also reflected in \autoref{fig:errors}, where virtually all errors occur after the Constantinople fork (block 7.28\,M), when the shift instructions were added (EIP 145).
Interestingly, even though Osiris and HoneyBadger are based on Oyente, the latter tool does not report any such errors.
...
Conkas fails for \np{63111} runs, with the most common causes being \emph{maximum recursion depth exceeded} \percent[b]{55652}{63111}; assertion failures \percent[b]{2499}{63111}; and type errors \percent[b]{2038}{63111}.
The \np{17407} fails of eThor result from the instruction EXTCODEHASH not being processed properly \percent[b]{11366}{17407}, arithmetic exceptions \percent[b]{5930}{17407}, and runtime exceptions \percent[b]{106}{17407}.
Securify fails for \np{9586} runs, mainly because of \emph{null pointer} exceptions \percent[b]{9496}{9586}.
Mythril, as the only tool actively maintained, fails for only \np{1022} bytecodes, the most frequent cause being type errors \percent[b]{952}{1022}, predominantly due to undefined terms in integer expressions.
At the lower end, we find Ethainter, Honeybadger and MadMax with no program issues at all, and Maian, Osiris and Oyente with just a few.
