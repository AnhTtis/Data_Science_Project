...
To establish that $\mc{L}_g$ determines the complex structure, we show that it encodes the period matrix of the underlying Riemann surface (Proposition \ref{proposition:key}) and then use mapping class group theory to recover the structure in Teichmüller space. This relies on the resolution of McMullen's conjecture for the virtual spectral radius of infinite-order elements of the mapping class group by Liu \cite{Liu-20} (Lemma \ref{lemma:cover}).
...
The proof of Theorem \ref{theorem:main} relies on several tools, which we introduce in Section \S\ref{section:tools}. Specifically, Section  \S\ref{ssection:complex} provides a brief review of the geometry of complex curves, while \S\ref{ssection:unit} is devoted to the geometry and harmonic analysis of the unit tangent bundle of a surface.
 In  \S\ref{ssection:hyperbolic}, we delve into hyperbolic dynamics and tensor tomography. Finally, we present the proof of Theorem \ref{theorem:main} in \S\ref{section:proof}. \\
 
 
\noindent \textbf{Acknowledgement.} We warmly thank J. Marché, B. Petri and I. Smith for helpful discussions related to the proof of Lemma \ref{lemma:cover}.
...
Let $g$ be a smooth Riemannian metric on $\Sigma$. The conformal class $[g]$ of $g$ (and the orientation of $\Sigma$) induces a complex structure $J \in C^\infty(\Sigma,\mathrm{End}(T\Sigma))$ on $\Sigma$, turning it into a Riemann surface which we shall denote by $(\Sigma,J)$. 
...
\end{equation}
whose kernel $\mathcal{I}(\Sigma) := \ker \rho$ is called the \emph{Torelli group} of $\Sigma$, see \cite[Section 6.2]{Farb-Margalit-11} for instance. We shall also need $\mc{I}^{\pm}(\Sigma)$ the degree $2$ extension of the Torelli group obtained by considering elements of $\mathrm{MCG}(\Sigma)$ acting as $\pm \mathbf{1}$ on $H_1(\Sigma,\Z)$.




\subsubsection{Period matrix. Jacobian.} Let $\left\{a_i,b_j\right\}$ be a canonical homology basis of $H_1(\Sigma,\Z)$ on the surface $\Sigma$. Let $(\Sigma,J)$ be a Riemann surface structure on $\Sigma$ and denote by $H_J^0(\Sigma,K^m)$ the space of holomorphic sections of the $m$-th power of the canonical bundle $K := T^*_{\C}\Sigma^{1,0}$ of $(\Sigma,J)$, for $m\geq 1$.
It is well-known that there exists a unique basis $\left\{\zeta_i\right\}$ of holomorphic Abelian differentials in $H_J^0(\Sigma,K)$ such that $(a_j,\zeta_k) = \int_{a_j} \zeta_k = \delta_{jk}$, see \cite[Proposition, page 63]{Farkas-Kra-92} for instance. The \emph{period matrix} of $(\Sigma,J)$ is then defined as the matrix $\Pi(J)$ whose $jk$-entry is $(b_j, \zeta_k)$.
...
If $\Sigma$ has genus $\geq 2$, the map
\[
\Pi : \mc{T}(\Sigma)/\mc{I}^{\pm}(\Sigma) \longrightarrow \mc{H}(\Sigma)
\]
is injective. In other words, if $\Pi(J_1) = \Pi(J_2)$, then there exists a diffeomorphism $\psi : \Sigma \to \Sigma$ acting as $\pm \mathbf{1}$ on $H_1(\Sigma,\Z)$ such that $\psi^*J_1 =  J_2$.
\end{theorem}

We refer to \cite[Theorem III.12.3]{Farkas-Kra-92} for a proof (note that, although not explicitly stated in \cite[Theorem III.12.3]{Farkas-Kra-92}, it follows from the construction that the diffeomorphism $\psi$ acts as $\pm \mathbf{1}$ on homology; the $-\mathbf{1}$ on homology is obtained when $V_1$ is a translate of $-W_1$ in the notation of \cite{Farkas-Kra-92}).
...
\]
where $\kappa$ denotes the sectional curvature on $(\Sigma,g)$, such that
\[
...
In this section, we first prove that $\mc{L}_g$ determines the class $[J_g] \in \mc{T}(\Sigma)/\mc{I}^{\pm}(\Sigma)$ in the quotient of the Teichmüller space by the degree $2$ extension of the Torelli group. Note that all the objects defined above (complex structures, stable and unstable bundles, etc.) now depend on the metric $g_1$ or $g_2$ and we shall add a subscript to distinguish them. The following is key to Theorem \ref{theorem:main}:

\begin{proposition}
\label{proposition:key}
Let $g_1,g_2$ be two smooth Anosov metrics on $\Sigma$. If $\mc{L}_{g_1} = \mc{L}_{g_2}$ then $[J_1]=  [J_2]$ in $\mc{T}(\Sigma)/\mc{I}^{\pm}(\Sigma)$. Equivalently, there exists a diffeomorphism $\psi : \Sigma \to \Sigma$ acting as $\pm \mathbf{1}$ on $H_1(\Sigma,\Z)$ such that $\psi^*J_1 = J_2$ and $\psi^* g_2 = e^{2 f} g_1$ for some $f \in C^\infty(\Sigma)$.
\end{proposition}
...
We introduce $\mc{C} \subset \{(v,\xi)\in T^*(S\Sigma) \,|\, \xi(X(v))=0\}$, the closed cone enclosed by $(E^s)^*$ and $(E^u)^*$ in the half-space $\left\{ \xi(V) \geq 0\right\}$, see Figure \ref{figure} for a description.
...
\]
while by standard propagation of singularities (see \cite[Theorem 26.1.1]{Hormander-4}) $\WF(f)$ is invariant by the symplectic lift of $(\varphi_t)_{t \in \R}$. But the maximal flow-invariant subset of $T^*(S\Sigma)$ contained in $\left\{ \xi(V) \geq 0, \xi(X(v))=0 \right\}$ is $\mc{C}$, so this proves the claim. Finally, ${\pi_n}_* f \in C^\infty(\Sigma,K^{\otimes n})$ as the pushforward operator ${\pi_n}_*$ only selects the wavefront set of $f$ in $(E^0)^* \oplus \HH^*$ (see \cite[Proposition 11.3.3]{FriedlanderJoshi} for instance)  which is empty as $\mc{C} \cap (E^0)^* \oplus \HH^* = \left\{0\right\}$.
\end{proof}

We now prove Proposition \ref{proposition:algebra-iso}.


\begin{proof}[Proof of Proposition \ref{proposition:algebra-iso}]
...
As $\phi$ preserves the orientation, $\Sigma$ is connected, and $d \phi^{\top}(\lambda_1) = \lambda_2$, we deduce that $\mc{C}' := d\phi^{\top}(\mc{C}_{1}) = \pm \mc{C}_{2}$. Indeed, since $d\phi^\top$ maps connected set to connected sets, $(E_1^u)^*$ to $(E^u_2)^*$ and $(E^s_1)^*$ to $(E^s_2)^*$, $d\phi^\top(\mc{C}_1)$ must be one of the 4 cones in the right Figure \ref{figure} (inside the subspace $\{\xi(X_2(v))=0\}$). Now, if $\mc{C}_{1}$ were mapped by $\phi$ to one of the other two cones on Figure \ref{figure}, $\phi$ would reverse the orientation, which is absurd.
...
F : H^0_{J_1}(\Sigma,K) \longrightarrow H^0_{\pm J_2}(\Sigma,K)
\]
is a period-preserving $\C$-linear complex isomorphism. Hence, by the Torelli Theorem \ref{theorem:torelli}, we get that $(\Sigma,J_1)$ and $(\Sigma,\pm J_2)$ have same period matrix, so they are biholomorphic and differ in the Teichm\"uller space by an element $[\psi] \in \mc{I}^\pm(\Sigma)$ in the degree $2$ extension of the Torelli group. Note that in the $(-)$ case, $[\psi]$ would reverse the orientation which is absurd as $[\psi] \in \mathrm{MCG}(\Sigma)$, hence only the $(+)$ case survives which is the content of Proposition \ref{proposition:key}.
\end{proof}
...
\begin{lemma}
\label{lemma:cover}
Let $J_1$ and $J_2$ be two complex structures on $\Sigma$. Assume that for any finite cover $\Sigma' \longrightarrow \Sigma$, the lifts $J_1'$ and $J_2'$ differ by an element of the degree $2$ extension of the Torelli group $\mc{I}^{\pm}(\Sigma')$ that is, there exists $\psi' \in \mc{I}^{\pm}(\Sigma')$ such that $(\psi')^* J_1' =  J_2'$. Then, $[J_1]=[J_2]$ in $\mc{T}(\Sigma)$, that is, there exists a diffeomorphism $\psi : \Sigma \longrightarrow \Sigma$, isotopic to the identity, such that $\psi^*J_1 = J_2$.
\end{lemma}

Let us first derive Proposition \ref{proposition:key2} from that.

\begin{proof}[Proof of Proposition \ref{proposition:key2}]
If $\mc{L}_{g_1} = \mc{L}_{g_2}$ on $\Sigma$, then the same equality $\mc{L}_{g'_1} = \mc{L}_{g'_2}$ holds on any finite cover $\Sigma' \longrightarrow \Sigma$. We can then conclude by combining Proposition \ref{proposition:key} and Lemma \ref{lemma:cover}.
\end{proof}

The proof of Lemma \ref{lemma:cover} was kindly explained to us independently by J. Marché, B. Petri on the one hand and I. Smith on the other hand. We start with the following:

\begin{claim}
\label{claim:order}
Let $[\psi] \in \mc{I}^{\pm}(\Sigma)$. Then $[\psi]$ has infinite order in the mapping class group $\mathrm{MCG}(\Sigma)$, unless it is trivial.
\end{claim}

\begin{proof}This is an immediate consequence of the fact that the Torelli group $\mc{I}(\Sigma)$ is torsion-free \cite[Theorem 6.12]{Farb-Margalit-11} and $(\mc{I}^{\pm}(\Sigma))^2 = \mc{I}(\Sigma)$.
\end{proof}

%{\color{red}The following is for the sake of our own curiosity. It should be removed from the final version of the paper.} {\color{blue} Proof that $\mc{I}(\Sigma)$ is torsion-free: assume that $[\psi]^k = 1$. By the Nielsen realization Theorem \cite[Theorem 7.1]{Farb-Margalit-11}, there exists a hyperbolic metric $g$ and and an isometry $\psi$ of $g$ realizing $[\psi]$ such that $\psi^k = 1$. (The proof of this fact is not difficult: one looks at the action of $[\psi]$ on $\mathrm{T}(\Sigma)$ which is a contractible space. Since $[\psi]^k=1$, one can easily see that $[\psi]^\ell$ (for some $1 \leq \ell < k$) admits a fixed point $[J]$ on $\mathcal{T}(\Sigma)$ (otherwise $\langle [\psi] \rangle = \Z/k\Z$ would act freely on $\mc{T}(\Sigma)$ but then $\mc{T}(\Sigma)/(\Z/k\Z)$ would be a $K(\Z/k\Z,1)$ which is absurd as $\mc{T}(\Sigma)$ is contractible). If $\ell \wedge k = 1$, we must get that $[\psi][J]=[J]$, otherwise a slight adaptation of the argument allows to retrieve this case. But $[\psi][J]=[J]$ means precisely that there exists a representative $\psi$ such $\psi^*J=J$ and $[\psi]^k =1$ so $\psi^k=1$. Hence $\psi$ is a $J$-biholomorphism (for the complex structure induced by the conformal class of $g$ and the orientation of $\Sigma$) so its Lefschetz number (fixed points are isolated by biholomorphicity) is $> 0$. On the other hand, its Lefschetz number computed by the action on homology (which is trivial by assumption) is $\chi(\Sigma)=2-2g < 0$. By the Hopf-Lefschetz formula, this is a contradiction.}

The following lemma is based on a deep result by Liu \cite{Liu-20} proving a conjecture of McMullen.

\begin{lemma}
\label{lemma:infinite}
Let $[\psi] \in \mathrm{MCG}(\Sigma)$ be an element of infinite order. Then it is virtually of infinite order on homology that is, there exists a finite cover $\Sigma' \to \Sigma$ on which $\psi$ lifts to $\psi' : \Sigma' \to \Sigma'$ and such that $\psi'_* : H_1(\Sigma',\Z) \to H_1(\Sigma',\Z)$ has infinite order.
\end{lemma}

\begin{proof}
We argue on the mapping class type of $[\psi]$, following the Nielsen-Thurston classification (see \cite[Theorem 13.2 and Corollary 13.2]{Farb-Margalit-11}). If $[\psi]$ contains a pseudo-Anosov part in its canonical form, then its mapping class entropy is $> 0$, see \cite[Corollary 10]{Kojima-12}. As a consequence, by Liu's Theorem \cite{Liu-20}, it has virtual spectral radius $> 1$ that is, there exists a finite cover $\Sigma' \to \Sigma$ on which $\psi$ lifts to $\psi' : \Sigma' \to \Sigma'$ and such that $\psi'_* : H_1(\Sigma',\Z) \to H_1(\Sigma',\Z)$ has spectral radius $> 1$, hence infinite order. If $[\psi]$ does not contain any pseudo-Anosov part, there is an integer $k \geq 1$ such that $[\psi]^k = \prod_{i=1}^n T_{\gamma_1}^{\ell_1}...T_{\gamma_n}^{\ell_n}$ is a finite product of powers of Dehn twists along disjoint simple closed curves $\gamma_i$, see \cite[Corollary 13.2]{Farb-Margalit-11}, with $\ell_1,...,\ell_n \geq 0$. We can then conclude by \cite[Corollary 3.5]{Koberda-12} which shows virtual infinite order in homology of such a mapping class.

%Consider a double cover $\Sigma' \longrightarrow \Sigma$ such that the $\gamma_i$'s can be lifted to non-separating disjoint simple closed curves $\gamma_i'$ and a lift $T'_{\gamma_i}$ of the Dehn twists $\Sigma'$. Then, each $T'_{\gamma_i}$ acts as a transvection matrix by a factor $\sum_{g \in \Gamma} \widehat{i}(\cdot,g\cdot\gamma_i')g \cdot\gamma_i'$ (see \cite[Proposition 6.3]{Farb-Margalit-11}), where $\widehat{i}$ denotes the algebraic intersection number of curves on the (oriented) surface $\Sigma'$ and $\Sigma = \Sigma'/\Gamma$. Hence, using the disjointness of the $\gamma'_i$'s for all $[\gamma] \in H_1(\Sigma',\Z)$, we have:
%\begin{equation}
%\label{equation:dehn}
%\begin{split}
%((T'_{\gamma_1})^{\ell_1}_* ... (T'_{\gamma_n})^{\ell_n}_*)^k ([\gamma]) & = ((T'_{\gamma_1})^{k \ell_1}_* ... (T'_{\gamma_n})^{k \ell_n}_*) ([\gamma]) \\
%& = [\gamma] + k \sum_{g \in \Gamma} \sum_{j=1}^n \ell_j \widehat{i}(\gamma,g\gamma'_j) [g\gamma'_j].
% \end{split}
%\end{equation}
%The last term defines an endomorphism
%\[
%\alpha : H_1(\Sigma',\Z) \ni [\gamma] \mapsto \sum_{g \in \Gamma} \sum_{j=1}^n \ell_j \widehat{i}(\gamma,g\gamma'_j) [g\gamma'_j] \in H_1(\Sigma',\Z),
%\]
%which we claim is non-trivial. Indeed, pairing again with the algebraic intersection number $\widehat{i}$, one finds:
%\[
%\widehat{i}(\gamma,\alpha(\gamma)) = \sum_{g \in \Gamma} \sum_{j=1}^n \ell_j \widehat{i}(\gamma,g\gamma'_j)^2 \geq 0,
%\]
%since $\ell_1,...,\ell_n \geq 0$. Now, observe that for every closed non-separating simple curve $\gamma'$, there exists a closed curve $\gamma$ such that $\widehat{i}(\gamma,\gamma')\neq 0$. Hence, we can find $\gamma$ such that $\widehat{i}(\gamma,\alpha(\gamma)) \neq 0$. As $\widehat{i}$ is a symplectic $2$-form on $H_1(\Sigma',\Z)$, this implies that $\alpha(\gamma) \neq 0$, and by \eqref{equation:dehn}, we obtain that the product of Dehn twists has virtual infinite order in homology. This concludes the proof.
\end{proof}

We now prove Lemma \ref{lemma:cover}.

\begin{proof}[Proof of Lemma \ref{lemma:cover}]
By assumption, there exists a diffeomorphism $\psi : \Sigma \longrightarrow \Sigma$ acting as $\pm \mathbf{1}$ on $H_1(\Sigma,\Z)$ such that $\psi^* J_1 = J_2$. We want to show that $\psi$ is isotopic to the identity. By Claim \ref{claim:order} above, it suffices to show that $[\psi] \in \mathrm{MCG}(\Sigma)$ has finite order.

Let $\Sigma' \longrightarrow \Sigma$ be any finite cover such that $\psi$ can be lifted to a diffeomorphism $\psi'$ of $\Sigma'$.

\begin{claim} The endomorphism  $\psi'_* : H_1(\Sigma',\Z) \righttoleftarrow$ has finite order.
\end{claim}

\begin{proof}
Let $J_1', J_2'$ be the lifts of the complex structures $J_1, J_2$ to $\Sigma'$. Since $\psi^*J_1 = J_2$, we have $\psi'^*J_1' = J_2'$. By assumption, there exists $\theta : \Sigma' \to \Sigma'$ acting as $\pm \mathbf{1}$ on $H_1(\Sigma',\Z)$ such that $\theta^* J_1' = J_2'$. Hence, $(\theta \circ (\psi')^{-1})^*J_1' = J_1'$ that is, $\theta \circ (\psi')^{-1}$ is a $J_1$-biholomorphism. By the Hurwitz automorphism Theorem, $\mathrm{Aut}(\Sigma',J'_1)$ is finite so $\theta \circ (\psi')^{-1}$ has finite order. The induced endomorphism on $H_1(\Sigma',\Z)$ thus has finite order as well, but since $\theta_*$ acts as $\pm\mathbf{1}$ on $H_1(\Sigma',\Z)$, this implies that $\psi'_* : H_1(\Sigma',\Z) \righttoleftarrow$ has finite order.
\end{proof}

By Lemma \ref{lemma:infinite}, we conclude that $[\psi] \in \mathrm{MCG}(\Sigma)$ has finite order, and by Claim \ref{claim:order}, it is thus trivial, i.e. $\psi$ is isotopic to the identity.
\end{proof}
...
If $\mc{L}_{g_1}=\mc{L}_{g_2}$, we get by Proposition \ref{proposition:key2} that there exists $\psi : \Sigma \to \Sigma$ isotopic to the identity such that $\psi^*g_1 = e^{2f} g_2$. But then, applying Katok's argument \cite{Katok-88} for conformal metrics with same marked length spectrum, we get that $f=0$. This concludes the proof.
\end{proof}
