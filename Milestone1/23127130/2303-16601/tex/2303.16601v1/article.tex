...
Cloud environments provide on-demand access over a shared pool of computing resources for users and host a vast number of applications with various resource demands and optimization objectives\cite{song2018host}. The dynamic workload of applications and the ever-changing need of users make it hard to estimate resource utilization accurately, which may result in failure or a reduction in Quality of service (QoS) due to inaccurate resource allocation decisions.

Optimal allocation of resources ensures satisfying users' needs and minimizing cost. Keeping all the computational resources active results in over-provisioning and is not efficient. On the other hand, provisioning insufficient resources causes violations of the SLA~\cite{gupta2017online},~\cite{kaur2015energy}. The approach towards resource provisioning could be either reactive or proactive.
