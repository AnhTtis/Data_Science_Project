...
~\cite{kupyn2018deblurgan,kupyn2019deblurgan,asim2020blind}. GANs are in general hard to train and also hard to control image hallucinations since the two terms play an antagonic role (\cite{lugmayr2021ntire}). 
...
The goal of image restoration is to generate a high-quality image from its degraded low-quality measurement (e.g., low-resolution, compressed, noisy, blurry). Since the seminal super-resolution work of \citep{dong2015image} many recent image restoration methods adopt an end-to-end supervised formulation where a deep neural network is trained to directly produce a point estimate~\cite{zhao2016loss,lim2017enhanced,tao2018scale,chen2018learning}
These methods rely on low-quality high-quality image pairs to train a regression model. Most of the work has been focused on developing better and more powerful network architectures~\cite{zamir2022restormer,chen2022simple,tu2022maxim,zamir2021multi} so we can achieve better pixel-level reconstruction. While this formulation leads to state-of-the-art PSNR, the image generated is at best an average of all plausible solutions (regression to the mean). In the limit case where the low-quality image is completely obfuscated  the best prediction in terms of PSNR is the average of the distribution.

Generative adversarial networks~\cite{goodfellow2014generative,arjovsky2017wasserstein}, and adversarial formulations~\cite{ledig2017photo,isola2017image,kupyn2018deblurgan,kupyn2019deblurgan} have been introduced to push the generated image towards the manifold of natural images. GANs suffer from unstable training~\cite{arora2017generalization,salimans2016improved,arjovsky2017wasserstein}, while being prone to introduce significant image hallucinations. This is a direct consequence of a non-reference formulation that directly tries to minimize the distance of the range of the generator to the manifold of natural images~\cite{cohen2018distribution}.

\cite{blau2018perception} proved that there is a trade-off between image perceptual quality and distortion. It is not possible to minimize both distortion and perceptual quality simultaneously~\cite{blau2018perception, freirich2021theory}. In fact, minimizing the average point distortion (e.g., PSNR) can be only done in detriment of the perceptual quality. 

A powerful way of avoiding the regression to the mean is to formulate the problem as one of sampling from the posterior distribution~\cite{kawar2021stochastic,kawar2021snips,kawar2022denoising,ohayon2021high,kadkhodaie2021stochastic,whang2022deblurring}. An additional benefit of this formulation is to be able to generate multiple different plausible solutiones that can be used for uncertainty quantification~\cite{whang2021composing} or improving fairness~\cite{jalal2021fairness}.

Variational auto-encoders~\citet{prakash2020fully}, normalizing flows~\cite{lugmayr2020srflow,lugmayr2021ntire}, and Diffusion probabilistic models (DPMs)~\cite{saharia2021image,li2021srdiff,whang2022deblurring} have been successfully applied to different image restoration tasks, where a diverse set of candidates can be generated from the learned posterior~\cite{prakash2020fully}.

Denoising Diffusion Probabilistic Models (DDPMs)~\citep{sohl2015deep, ho2020denoising, song2021denoising}, Score-based models~\citep{ncsn, ncsnv2, ncsnv3} and their recent generalizations~\cite{bansal2022cold, hoogeboom2022blurring, daras2022soft, deasy2021heavy, hoogeboom2022autoregressive, hoogeboom2022equivariant, nachmani2021denoising, johnson2021beyond, lee2022progressive, ye2022hitting} generate high-quality samples by inverting a known degradation process. The main strategy is to analytically define a known degradation process that is reversed to generate new samples starting from a fully degraded image (e.g., pure noise). 
\cite{bansal2022cold}~introduced Cold Diffusion a generative framework to generate images by reverting arbitrary (known) degradations. They show promising results even with non-stochastic degradations such as blur, masking or pixelization. The strategy is to define intermediate analytical degradations (diffusion) and then revert them step by step. \cite{daras2022soft} presented Soft Diffusion a generalization of diffusion models to linear degradations. The authors argue that noise is a fundamental component that is needed to provably learn the score.
...
Our recovery method starts with the input degraded image (time $t=1$), and then at a given time-step $t$ generates the best possible reconstruction at time $t-\delta$.  This can be done, for example, by the short-time conditional mean $\hat{\vx}_{t-\delta} = \condExpec{\vx_{t-\delta}}{\vx_t}$. As we will show, by repeating this process we can thus invert the full degradation little by little. The following proposition provides the cornerstone of our approach.

\begin{proposition}
\label{prop:conditional_mean}
Let $\vx_s, \vx_t$  be given from \eqref{eq:model}, where $s \le t$. Then,
$$
\condExpec{\vx_s}{\vx_t} = \left(1 - \frac{s}{t}\right)  \condExpec{\vx}{\vx_t} + \frac{s}{t} \vx_t.
$$
\end{proposition}
...
$$
\hat{\vx}_{t-\delta} = \frac{\delta}{t} \frac{\sum_{i=1}^d \vc_i w_i G\left(\frac{\vx_t - \mH_t\vc_i}{\sigma_t}\right)}{\sum_{i=1}^d w_i G\left(\frac{\vx_t - \mH_t\vc_i}{\sigma_t}\right)} + \left(1-\frac{\delta}{t}\right)\hat{\vx}_t.
$$
...
\noindent \textbf{Training and Inference:} We train a family of regressors $F_\theta(\cdot; t)$, each of them specialized in reconstructing $\vx$ (i.e., $\vx_0$) from $\vx_t$ at a given $t$. That is,
\begin{align}
...
\begin{align}
\vx_{t-\delta} &= \frac{\delta}{t} F(\vx_t, t) +  \left(1-\frac{\delta}{t} \right)\vx_t, 
...
Note that in the limit $\delta \to 0$, \eqref{eq:baseline_sampler} leads to an ordinary differential equation (ODE),
\begin{align}
\frac{ d\vx_t}{d t} = \frac{F(\vx_t, t)-\vx_t}{t}.
\label{eq:ode_formulation}
\end{align}
The ODE can be interpreted as a sort of ``residual flow''. We are interested in the solution of this equation at $t=0$, starting from the initial condition $\vx_1=\vy$ at $t=1$.
...
\begin{align}
\vx_{t-\delta} &= \frac{\delta}{t} F(\vx_t, t) + \left(1-\frac{\delta}{t} \right) \vx_t + (t-\delta)\sqrt{\epsilon_{t-\delta}^2 - \epsilon_t^2}\vzeta 
\label{eq:baseline_sampler_noise},
\end{align}
where the reconstruction process starts from $t=1$, $\vx_{1} = \vy + \epsilon \vn$ and $\vn \sim \mathcal{N}(0, Id)$. At each step, a new $\vzeta \sim \mathcal{N}(0, Id)$ is sampled and noise is added to the current state.
...
Our full iterative restoration inference scheme is given in Algorithm~\ref{alg:inference}.
  ...
  \label{alg:inference}
  \begin{algorithmic}
    \Require $\vy, F(\cdot, t), \delta, \epsilon_t$
     \State $\vn \sim \mathcal N(\vzero, \mI)$
     \State $\vx_1 = \vy + \epsilon_1 \vn$
    \For{$t=1 \ \mathrm{\mathbf{to}} \ 0 \,\, \mathrm{\mathbf{with \,\, step}} \ \mathrm{-\delta}$}
      \State $\vzeta \sim \mathcal N(\vzero, \mI)$
      \State $\vx_{t - \delta} \gets \frac{\delta}{t} \hat F(\vx_t, t) + \left(1 - \frac{\delta}{t} \right) \vx_t +(t-\delta)\sqrt{\epsilon_{t-\delta}^2 - \epsilon_t^2}\vzeta$  \Comment{Update rule from~\eqref{eq:baseline_sampler_noise}.}
    \EndFor
    \State \textbf{return} $\vx_0$
  \end{algorithmic}
  \end{algorithm}
...
\noindent \textbf{Perception -- Distortion tradeoff~\citep{blau2018perception}.} To illustrate the potential of the method, we present results when using different number of steps for the reconstruction. This has a direct impact on the distortion -- perception tradeoff. In general, a single step reconstruction with our model, will lead to an estimate that minimizes the average point distortion (e.g., PSNR) but this can be only done to the detriment of the perceptual quality.  


\noindent \textbf{Model Architecture and Training.}
We adopt a U-Net-like architecture similar to the ones in diffusion strategies~\cite{saharia2021image,whang2022deblurring}. Following~\cite{whang2022deblurring} we removed attention layers and group normalization to have a fully-convolutional architecture. %
...
\input{figures/fig_sisr_x4_comparison}
\input{tables/table_sisr_div2k}

Table~\ref{tab:sisr_4x}(b) summarizes the quantitative results on $4\times$ SR div2k validation dataset. Figure~\ref{fig:sisr_4x_comparison} shows a selection of results. Our proposed framework leads to upscaled images with more defined structure than regression based formulations producing larger PSNR (RRDB~\cite{wang2018esrgan}). The recently introduced adversarial formulation LDL~\citep{liang2022details} produces slightly better fine grain details. This could indicate that in the situation where there is limited training data, careful adversarial formulation may be more data efficient.

\textbf{The importance of adding noise in deterministic super-resolution.} In our formulation of super-resolution, the degradation is a deterministic linear (blurring plus subsampling) operator. Table~\ref{tab:sisr_4x}(a) shows the importance of adding a small amount of noise to the input image. Directly applying the original iterative procedure (without adding noise to the input) leads to a blurry reconstruction (high PSNR but low FID score, $\epsilon=0.0$ in Table~\ref{tab:sisr_4x} (a)). Adding a small amount of noise  ($\epsilon > 0$ in Table~\ref{tab:sisr_4x}(a)) leads to significant better results in terms of perceptual quality (e.g., FID score).
...
\input{figures/fig_dejpeg_q15_samples}
...
\input{figures/fig_generative_celeba_samples}
...
\noindent \textbf{Naive Procedure and Relevance to Cold Diffusion:} Given~\eqref{eq:model}, one may be tempted to directly replace the clean image by the current estimate. This would lead to $\hat{\vx}_{t} = (1-t) F(\vx_s, t) + t \vy$, and the inference iterative rule would become
\begin{align}
\vx_{t-\delta} &= (1-t+\delta) F(\vx_t, t) + (t-\delta) \vy.
...
\begin{align}
\vx_{t-\delta} &= D(F(\vx_t,t),t-\delta)  = (1-t+\delta)F(\vx_t,t) + (t - \delta)\vy.
\end{align}
...
\begin{align}
\vx_{t-\delta} &= \vx_t - D(F(\vx_t, t),t) + D(F(\vx_t, t),t-\delta) \\
&= \vx_t - (1-t)F(\vx_t, t) - t\vy + (1-t + \delta)F(\vx_t, t) + (t - \delta )\vy \\
&= \vx_t + \delta (F(\vx_t, t)-\vy).
...
\input{figures/fig_sampler_comparison} Figure~\ref{fig:impact_sampler}. shows the FID score of CelebA 64x64 generated images when using the three different variants of the inference algorithm (sampler).
 
\subsection{Impact of distribution p(t)} 

\label{sec:impact_pt}
\input{figures/fig_dejpeg_q15_distribution_t}
...
\input{figures/fig_dejpeg_q15_noise}
...
In all our restoration experiments we use a U-Net-like architecture~\cite{ronneberger2015u} similar to the one in SR3~\cite{saharia2021image} and DvSR~\cite{whang2022deblurring}.  We followed the same adaptations as the ones introduced in~\cite{whang2022deblurring} to make it fully-convolutional (removed self-attention layers and group normalization). Our U-Net has an adaptive number of resolutions each of them having an arbitrary number of channels (given by a multiplication factor from a base set of channels).

Table~\ref{table:model_params} summarizes the model definition for each of the tested applications.

\input{tables/table_params}
...
\input{figures/fig_appendix_gopro}
\input{figures/fig_appendix_sisr_x4}
\input{figures/fig_appendix_dejpeg_q15}
\input{tables/table_deblurring_ddpd}
