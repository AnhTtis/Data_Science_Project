...
Causal graphs are often used to study the underlying data-generating mechanism in real-world problems. For a dataset $D$ $=$ ($D_{1}, D_{2}, ..., D_{n}$) where each data point $D$ is a vector of values over variables $X$, causal graphs encode the cause-effect relationships among the variables using directed edges ($\rightarrow$) from cause to the effect. Most of the time causal graphs take the form of a DAG. In  Figure \ref{DAGs&Skeleton} (a), $X$ is the cause that effects both $Y$ and $Z$ (i.e. $Y$ $\leftarrow$ $X$ $\rightarrow$ $Z$). Also, $Z$ is a cause of $Y$ (i.e. $Z$ $\rightarrow$ $Y$). The mechanism that enables the estimation of a causal graph from a dataset $D$ is called \emph{\textbf{causal discovery} (CD)} (Figure \ref{CG}). The outcome of any causal discovery algorithm is a causal graph $G$ where the directed edges ($\rightarrow$) represent the cause-and-effect relationship between the variables $X$ in $D$. However, some causal discovery approaches have different forms of graphs (PDAGs, CPDAGs, ancestral graphs, etc.) as the output causal graph. Table \ref{table-3} presents the output causal graphs of some common CD approaches which are discussed in section \ref{Section-3}.
...
More specifically, in a CPDAG, an undirected edge between any two nodes $X$ and $Y$ indicate that some DAG in the equivalence class contains the edge $X$→$Y$ and some DAG may contain $Y$→$X$. Figure \ref{CPDAG} shows a CPDAG %$C_{g}$ 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.35\textwidth]{CMC_assumption.png} 
    ...
    \centering
        ...
        for tree={draw,align=left,l=1.7cm,edge={-latex}}
          [Causal Discovery Algorithms
            [Contraint-based
                [PC (\ref{PC})\\FCI (\ref{fci})\\Anytime FCI (\ref{anytimefci})\\RFCI (\ref{rfci})\\PC-stable (\ref{pcstable})\\RRCD (\ref{rrcd})]
            ]
            [Score-based
                [GES (\ref{ges})\\FGS (\ref{fgs})\\SGES (\ref{sges})\\RL-BIC (\ref{rlbic})\\Triplet A\textsuperscript{*} (\ref{tripleta*})]
            ]
            [Functional Causal\\Model-based
                [LiNGAM (\ref{lingam-section})\\ANM (\ref{anm_section})\\PNL (\ref{pnl})\\Direct \\ LiNGAM (\ref{directlingam})\\SAM (\ref{sam})\\CGNN (\ref{cgnn})]
            ]
            [Continuous \\Optimization-based
                [NOTEARS (\ref{notears})\\GraN-DAG (\ref{gran-dag})\\GAE (\ref{gae})\\DAG-GNN (\ref{daggnn})\\GOLEM (\ref{golem})\\CAREFL (\ref{carefl})\\DAG-NoCurl (\ref{dag-nocurl})\\ENCO (\ref{enco})\\CORL (\ref{corl})\\MCSL (\ref{mcsl})]
            ]
            [Prior \\Knowledge-based
                [C-MCMC (\ref{c-mcmc})\\JCI (\ref{jci})\\FCI with TBK  (\ref{fci-tiered})\\PKCL (\ref{pkcl})\\Kg2Causal (\ref{kg2causal})\\KCRL (\ref{kcrl})]
            ]
          ]
        \end{forest}
        }
    \caption{Taxonomy of some causal discovery approaches for i.i.d. data.}
    \label{Assumption}
\end{figure}
...
Constraint-based approaches conduct \emph{conditional independence (CI)} tests between the variables to check for the presence or absence of edges. These approaches infer the conditional independencies within the data using the \emph{d-separation criterion} to search for a DAG that entails these independencies (\cite{paper7}). Let us consider the graph in Figure \ref{cons} (a). After testing the conditional independencies, it is found that $X$ $\indep$ $W$ $|$ $Z$ and $Y$ $\indep$ $W$ $|$ $Z$. Hence,  the edges $X$-$W$ and $Y$-$W$ are removed (Figure \ref{cons} (b)). Such CI tests are commonly used by constraint-based approaches to detect which variables are d-separated and which are d-connected. Different types of CI tests used by constraint-based causal discovery approaches are listed in Table \ref{tab:2}. 

\begin{figure}[h]
\centering
\includegraphics[width=0.37\textwidth]{Cons_based.PNG} 
...
\subsection{Score Function-based}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{RL-BIC.PNG}
...
To discover the causal structure, it imposes the proposed acyclicity function as a constraint (Equation \ref{acyc}) combined with a weighted adjacency matrix $W$ with least squares loss.  %To ensure that a matrix W is a DAG, it must follow the proposed acyclicity constraint (cite equation 1) where 0 is the Hadamard product and $e^{W0W}$ is the matrix exponential of WoW. 
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.77\textwidth] {ENCO.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{JCI.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{KCRL.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.65\textwidth]{HCM.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{LFCM.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.47\textwidth]{Meta-RL.png}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.43\textwidth]{Temporal_Causal_edges.PNG} 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{Full-time-causal-graph.jpeg} \hspace{1cm}\includegraphics[width=0.21\textwidth]{Summary_Causal_Graph.jpeg} 
    ...
    \centering
        ...
        for tree={draw,align=left,l=1.7cm,edge={-latex}}
            [Causal Discovery Algorithms\\(for time series data)
                [Contraint-based
                    [tsFCI (\ref{tsfci})\\PCMCI (\ref{pcmci})\\PCMCI+ (\ref{pcmci+})\\LPCMCI (\ref{lpcmci})\\CD-NOD (\ref{cd-nod})]
                ]
                [Functional Causal\\Model-based
                    [VarLiNGAM (\ref{varlingam})\\TiMINo (\ref{timino})]
                ]
                [Continuous\\Optimization-based
                    [DYNOTEARS (\ref{dynotears})\\NTS-NOTEARS (\ref{nts-notears})]
                ]
                [Granger\\Causality-based
                    [GVAR (\ref{gvar})\\NAVAR (\ref{navar})]
                ]
                [Miscellaneous\\approaches
                    [oCSE (\ref{ocse})\\TCDF (\ref{tcdf})\\NBCB (\ref{nbcb})\\PCTMI (\ref{pctmi})\\ACD (\ref{acd})]
                ]
          ]
        \end{forest}
        }
...
\begin{figure}[h]
\centering
\includegraphics[width=0.93\textwidth]{CDNOD.PNG}
...
We show the calculation of SHD for the graphs in Figure \ref{SHD} using the formula in Equation \ref{eq1} where $A$ = total number of edge additions, $D$ = total number of edge deletions, and $R$ = total number of edge reversals. In the Figure \ref{SHD}, we need to \textit{add} the edge $D$ → $C$, \textit{delete} the edges $D$ → $B$ and $D$ → $A$, and \textit{reverse} the edges $C$ → $B$ and $C$ → $A$ in the generated graph (graph b) to convert it into the true graph (graph a). Therefore, the $SHD = 1 + 2 + 2 = 5$ means a total of 5 actions are required to reach the true graph (graph a). 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{GT_SHD.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{ASIA.png} \includegraphics[width=0.5\textwidth]{LUCAS.png}
\caption{Ground-truth network of the ASIA (left), and (b) LUCAS (right) datasets.}
\label{ASIA}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{SACHS.png} %\includegraphics[width=0.52\textwidth]{LUCAS.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{CHILD.png} %\includegraphics[width=0.52\textwidth]{LUCAS.png}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.45\textwidth]{SHD_IID.png} \includegraphics[width=0.46\textwidth]{TPR_IID.png}
\includegraphics[width=0.46\textwidth]{FDR_IID.png}
...
%\scriptsize
%\tiny
\textbf{Table 7: A brief overview of the tools/packages for Causal Discovery.}
    \begin{longtable}{ | c | p{1.7cm} | p{1.5cm} | p{2.3cm} | c | p{13cm} |}
    \hline
...
\begin{figure}[]
\centering
\includegraphics[width=0.63\textwidth]{Climate_application_causal_graph.png} %\includegraphics[width=0.52\textwidth]{LUCAS.png}
