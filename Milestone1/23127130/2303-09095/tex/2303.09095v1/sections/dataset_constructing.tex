...
We use Detectron~\cite{wu2019detectron2} to detect and Deepsort~\cite{wojke2017simple} to track humans in videos. However, the tracking often fails due to the IMUs wearer entering/exiting the field of view or occlusions. To solve this problem, we manually assign the same ID for the tracked person in a video sequence. As for 3D point cloud reference, we project them on images according to the $K_{ex}$. However, due to the jitter brought by dynamic motions, the camera and the LiDAR are not perfectly rigidly connected. Thus, $K_{ex}$ will be further optimized in \cref{subsec:optimization}. 

\PAR{LiDAR-inertial localization and mapping.}
The LiDAR-only method often fails on scene mapping because of the dynamic head rotation and crowded urban environments. Incorporating an IMU will solve this problem by compensating for the motion distortion in a LiDAR scan and providing a good initial pose. Using a LiDAR with an integrated IMU, and by combining Kalman filter-based lidar-inertial odometry\cite{Xu2022FASTLIO2FD} with factor graph-based loop closure optimization\cite{gtsam}\cite{Kim2018ScanCE}, we successfully estimate the ego-motion of LiDAR and build the global consistency 3D scene map with $P_k^L, k\in~Z^+$ in \{$L$\}. To provide accurate scene constrain in \cref{subsec:optimization}, we utilize the VDB-Fusion~\cite{vizzo2022sensors} to generate a clean scene mesh $\bm{S}$ that excludes moving objects.

\PAR{IMUs pose estimation.}
We use SMPL~\cite{smpl2015loper} to represent the human body motion ${M^I} = \varPhi (T^I, \theta^I, \beta)$ in IMU coordinate space\{$I$\}, 
where pose parameter $\theta^I$, $\theta \in \mathbb{R}^{24 \times 3}$, is composed of the pelvis joint's orientation $R^I$ relative to the start frame and the other 23 joints' rotation relative to their parent joint. 
The $T$, $T\in\mathbb{R}^{3}$, is the pelvis joint's global translation and $\beta$, $\beta \in \mathbb{R}^{10}$,  is a constant value representing a person's body shape. 
$T$ and $\theta$ are are estimated by the commercial MoCap product, while $\beta$ is obtained by using IPNet~\cite{bhatnagar2020ipnet} to fit the scanned model captured by iPhone13 Promax.
$T_k^I$ indicates the $k$-th frame translation in global coordinates.
Since the relative value of $T^I$ and $R^I$ in the IMU data are accurate locally but drift globally, the IMU data is only used for raw calibration of the \{$I$\} to \{$W$\} and to provide an initial position of the human points in LiDAR.
