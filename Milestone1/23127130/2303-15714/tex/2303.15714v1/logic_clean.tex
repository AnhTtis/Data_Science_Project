...
In our experiments, our full system significantly outperforms other competing systems. 
On a multiple-choice question answering task, our system performs competitively compared to GPT-3-davinci despite having only around 1.5B parameters. 
We conduct several ablation studies to demonstrate that explicit planning plays a crucial role in the system's performance.
\end{abstract}
...
The selection-inference system of~\citet{creswell2022selection} is also similar to our base system but their selection and deduction models are few-shot-prompted GPT-3, whose size is a hundred times larger than ours. 
...
Another kind of way to use LMs for reasoning is to prompt GPT-3 with spelled-out reasoning procedure~\citep{wei2022chain,talmor2020leap}: it is orthogonal to the above approaches and we compared with it in our experiments. 
...
Base System & 0.56 (0.51, 0.60) & 0.42 (0.38, 0.47) & 0.78 (0.76, 0.81) & 0.66 (0.66, 0.66)\\
...
We trained and evaluated our methods on the standard benchmark Entailment Bank~\citep{dalvi-etal-2021-explaining} dataset. 
...
\subsection{Evaluation on Binary Classification}\label{sec:binary}

We first evaluated the abilities of the systems to classify provable and non-provable goals. 
...
\paragraph{Robustness to size of training data.}
...
\paragraph{About the regularization in \cref{eqn:reg}.} 
...
\paragraph{Robustness to distractors.} 
...
\paragraph{Formal logic approach.}
...
\subsection{Multiple-Choice Question Answering}\label{sec:mcqat5}
We further evaluated the systems in a multiple-choice setting. 
...
\paragraph{Results.} As shown in \cref{tab:mainresultqa}, the systems behave similarly as they do on the binary classification: in both Version-I and Version-II settings, System-A and System-B perform significantly better than the baselines, and System-B significantly outperforms System-A. 
...
Our experiments have shown that explicit planning significantly helps language models in logical reasoning. 
Our full system only has around 1.5B parameters but performs competitive to GPT-3-davinci: although our system requires supervised training, it yields a strong performance by only consuming a thousand training examples. 
