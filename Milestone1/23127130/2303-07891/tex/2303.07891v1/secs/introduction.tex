...
% Our proposal
\cstart This paper presents \cend the \ac{ourmethod} method, which is a data-driven approach for deriving \acp{ssm} that are not limited to certain types of scenarios.
Because \cstart the \cend method is not bound to certain predetermined assumptions about driver behavior, the derived \acp{ssm} can be adapted to the situations in which they are applied. 
...
The \ac{ourmethod} method \cstart has the following characteristics\cend:
\begin{itemize}
	...
	Therefore, \cstart the \ac{ourmethod} method \cend can be seen as a generalization for deriving such existing \acp{ssm}.
	
	\item \cstart A \cend driver behavior model can be used.
	...
	\item Because a data-driven approach \cstart is adopted, the derived \cend \ac{ssm} adapts to the recorded data. 
	...
	\item \cstart The \ac{ourmethod} method \cend is not limited to one type of scenario.
\end{itemize}

% Explain case study.
We illustrate the \ac{ourmethod} method and its benefits by means of a case study. 
The case study demonstrates that when using \cstart the \ac{ourmethod} \cend method with the assumptions of the \ac{ssm} of \textcite{wang2014evaluation}, both the \ac{ssm} derived by the \ac{ourmethod} method and the latter yield the same result.
...
Moreover, since a comparison between these measures is not directly possible in general scenarios, \cstart a method to benchmark \acp{ssm} using expected risk trends is introduced in the case study\cend.
