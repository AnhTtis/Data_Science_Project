...
\textbf{\textit{Introduction.}}--The wave-particle duality (WPD) expresses the coexistence of particle- and wave-like behavior in a single quantum system. This fundamental principle, confirmed in both photon \cite{Compton1923, Weinberg2015Historical} and matter interferometers \cite{Davisson1927, Olivier1999, Weinberg2015Historical}, is a pillar of our understanding of quantum mechanics. The WPD has been expressed in quantitative terms~\cite{Wootters1979, Englert1996}, extended to many-body interference~\cite{Dittel2021}, and even tested by interferometric scenarios in which neither particle nor wave models can describe the measurement outcomes~\cite{Wang2019}.

Identifying genuine quantum features requires a comparison with classical models. This practice is central to understanding the potential of quantum technologies~\cite{Mitchison2015, Nimmrichter2017, Kalaee2021, Korzekwa2021, Prech2022} as well as fundamental aspects of quantum theory~\cite{Unruh1989, Zurek1993, Zurek2003, Spekkens2007, Spekkens2016}. Such a comparison is not always straightforward as there is no unique way of comparing classical and quantum models:
One could for instance start from a classical theory and quantize it (e.g. electrodynamics~\cite{Weinberg2015Electrodynamics, Yurke1984}), or one could derive an effective classical model from a quantum model exploiting, e.g., the effect of decoherence~\cite{Unruh1989, Zurek1993, Zurek2003, Mitchison2015}. 
In these approaches, a quantum model is compared to a single classical counterpart. However, the WPD implies that at least two classical models are admissible; one based on waves, and one based on particles. 


% In this work, we exploit this insight from the WPD and contrast two classical counterparts with bosonic transport.
...
We focus on a setup where quantum coherence is relevant: a pair of harmonic oscillators coupled coherently to each other and to thermal reservoirs at different temperatures, see Fig.~\ref{fig:setup}. This system implements a quantum heat engine~\cite{Ghosh2018}; heat flowing from the hot bath to the cold bath gives rise to power output. Comparison to two classical models, where bosons are modeled as waves and particles respectively, allows us to investigate the role of the WPD in the quantum heat engine.
...
These limits correspond to classical heat engines, but, crucially, with respect to \textit{two} distinct classical models. And, away from these limits, noise contains signatures of the WPD as neither waves nor particles can capture it. Our results thus provide fundamental insight into the role of the WPD in non-equilibrium transport scenarios.

\begin{figure}
\centering
	\begin{tikzpicture}
            \node (a) [label={[label distance=-.5 cm]145:\textbf{(a)}}]   at (0,1) {\includegraphics[width=.9\columnwidth]{figures/fig1a.pdf}};
            \node (a) [label={[label distance=-.5 cm]145:\textbf{(b)}}]  at (-2.5,-1) {\includegraphics[width =.35\columnwidth]{figures/fig1b.pdf}};
            \node (a) [label={[label distance=-.5 cm]145:\textbf{(c)}}]  at (1.75,-1.5) {\includegraphics[width = .5\columnwidth]{ figures/fig1c.pdf}};
	\end{tikzpicture}
...
\end{equation}
We work with units $\hbar = k_B = 1$ and  $\Delta = \Omega_h - \Omega_c$. Each system mode is connected to a bath with different temperatures, and the heat flow leads to power output, $P(t) = -\partial_t H(t)$, our quantity of interest. The setup is depicted in Fig.~\ref{fig:setup}\,(a). 
...
The LME \eqref{qme} is equivalent to a set of quantum Langevin equations (QLEs), in the input-output formalism~\cite{Gardiner1985}
\begin{subequations}\label{qle}
   \begin{align}
    ...
    \end{align}
\end{subequations}
where the thermal baths are captured by input fields, $\bin{\alpha}$, with \textit{quantum} white noise auto-correlation function,
...
\textbf{\textit{Wave heat engine.}}-- Our wave model, sketched in Fig.~\ref{fig:setup}\,(b), consists of pair of classical field amplitudes, externally driven by two thermal white noise sources. Formally, the model is based on the canonical association between the ladder operators and complex field amplitudes, $a_\alpha \leftrightarrow A_\alpha$. The classical dynamics is given by classical Langevin equations,
...
\begin{figure*}[thb]
	\begin{tikzpicture}
        \node (a) [label={[label distance=-.65 cm]145:\textbf{(a)}}]  at (0,0) {\includegraphics[width=0.33\textwidth]{figures/plotnoise.png}};
        \node (a) [label={[label distance=-.65 cm]145:\textbf{(b)}}]  at (6,0) {\includegraphics[width=0.33\textwidth]{figures/plotfano.png}};
        \node (a) [label={[label distance=-.65 cm]145:\textbf{(c)}}]  at (12,0) {\includegraphics[width=0.33\textwidth]{figures/plotnh.png}};
	\end{tikzpicture}   
...
\textbf{\textit{Particle heat engine.}}--Our particle model consists of a classical rate equation for the fluctuating particle numbers, as sketched in Fig~\ref{fig:setup}\,(c), 
\begin{align}
...
% \mathcal{S}_p&= \mathcal{S}  + \frac{ 48g^6\kappa^3 \Delta^2 }{(6g^2 + \kappa^2)(4g^2 + \kappa^2)^3}. \label{sprime}
\end{align}
...
Our work can be extended in multiple directions. By considering the full Josephson interaction in a circuit QED implementation~\cite{Hofer2016}, the WPD can be explored in a richer model, which contains squeezing and non-Gaussian effects in the power statistics. Another possibility is to extend the classical comparison to few-level systems and qubits, which can be contrasted to particle-like models or wave-like models provided by classical magnets with oscillating magnetization. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\emph{Acknowledgements.} We acknowledge fruitful discussions with G. Landi and A. Tettamanti for carefully reading the manuscript. This work was supported by the Swiss National Science Foundation (Eccellenza Professorial Fellowship PCEFP2 194268).
