...
\item
  \label{a:subgaussian}
  There is an impulse  response sequence $h[k]\in\bbR^{n\times m}$ and independent $\sigma$-sub-Gaussian random variables $\bzeta_i[k]$ with $\bbE\left[\bzeta_i[k]^2\right]=1$  for $i=1,\ldots,m$ and $k\in\bbZ$ 
  such that $\by[k] = \sum_{\ell=-\infty}^{\infty} h[k-\ell]\bzeta[\ell]$, where $\bzeta[\ell]=\begin{bmatrix}\bzeta_1[\ell] & \cdots \bzeta_m[\ell]\end{bmatrix}^\top$. 
\end{enumerate}
...
Now we describe some specialized notation used to present our general results on the convergence of spectral estimators of the form (\ref{eq:quadraticPeriodogram}).
...
\label{eq:dataLowerBound}
  \xi &= \frac{\log\left(\delta^{-1} 10^{2n} c_{\ref{ConcentrationMult}}\right)}{c_{\ref{ConcentrationExp}}}\max\left\{\frac{c_{\ref{ConcentrationSubGauss}}^4\|\Phi\|_{\infty}^2}{\epsilon^2},\frac{c_{\ref{ConcentrationSubGauss}}^2\|\Phi\|_\infty}{\epsilon}\right\} \\
    \label{eq:minWindow}
  \hat M &= \inf\left\{\tilde  M\in \bbN \middle| \sum_{|k|\ge \tilde M} \|R[k]\|_2 \le \frac{\epsilon}{2}\right\}.
\end{align}
\end{subequations}
...
\begin{theorem}
  ...
  For all $\epsilon >0$ and all $\delta \in (0,1)$, define $\xi$ and $\hat M$ as in (\ref{eq:collectedConstants}).
  \begin{enumerate}
  \item \label{it:concentration}
    If $\frac{1}{\max\{\|A\|_2,\|A\|_F^2\}}\ge \xi$, then for all $s\in \left[-\frac{1}{2},\frac{1}{2} \right] $ we have
    $$ \bbP\left(\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right] \right\|_2 >\epsilon \right) \le \delta.$$
  \item \label{it:worst}
    Let $g\ge \|B[k]\|_2$ and $g\ge \|B[k]\|_F^2$ for all $k\in\bbZ$, and assume that there is a number $\hat N\le N$ such that  $B[k]=0$ for $|k|\ge \hat N$. If $\frac{1}{g(2\hat N-1)^2\log(2\hat N-1)}\ge \xi$ then 
    $$
     ...
     $$
   \item \label{it:bias}
     Assume that $b[k]\in [0,1]$ for all $k\in \bbZ$. If $b[k]\ge 1-\frac{\epsilon}{2\|R\|_1}$ for $|k|< \hat M$, then
     $$
...
\section{Convergence of Specific Classical Spectrum Estimators}
\label{sec:classical}

This section shows how to analyze periodograms, Blackman-Tukey estimators, Bartlett estimators, and Welch estimators in terms of the general result from \ref{thm:genConvergence}. In particular, high probability convergence bounds are obtained in the case of Blackman-Tukey, Bartlett, and Welch estimators. For periodograms, the bias is bounded, but high-probability bounds cannot be obtained, consistent with classical calculations on variance of periodograms. (See \cite{stoica2005spectral}.)

The definitions of the various estimators follows the presentation from \cite{stoica2005spectral}, and it is shown how each estimator can be expressed in the form of (\ref{eq:quadraticPeriodogram}). This leads to a unified approach to convergence analysis. All of the propositions and theorems of this section are proved in Appendix~\ref{app:specific}.
...
\begin{proposition}
  ...
  If $N\ge \frac{2 \hat M \|R\|_1}{\epsilon}$, then
  $$
  \sup_{s\in \left[-\frac{1}{2},\frac{1}{2}\right]} \left\|\Phi(s)-\bbE\left[\hat\bPhi(s) \right]\right\|_2 \le \epsilon.
  $$
\end{proposition}
...
\left(\frac{1}{\sqrt{N}}1_{N\times 1}\right) \le \|A\|_2\le \|A\|_F,
  $$
  for all values of $N$.
As a result, the conditions of Theorem~\ref{thm:genConvergence} Part \ref{it:concentration}) on pointwise convergence cannot be met for $\xi\ge 1$.  Similarly, the conditions of Part~\ref{it:worst}) cannot be met.
...
For symmetry of $A$, we must have $w[k]=w[-k]$.
  ...
  \item \label{it:BTconcentration}
    If $\frac{N}{2M-1}\ge \xi$, then for all $s\in \left[-\frac{1}{2},\frac{1}{2} \right] $ we have
    $$ \bbP\left(\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right] \right\|_2 >\epsilon \right) \le \delta.$$
  \item \label{it:BTworst}
    If $\frac{N}{(2M-1)^2\log(2M-1)}\ge \xi$ then 
    $$
     \bbP\left(\sup_{s\in\left[-\frac{1}{2},\frac{1}{2}\right]}\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right] \right\|_2 >\epsilon \right) \le \delta.
     $$
...
where there are $L$ blocks of size $M\times M$.
  ...
  \item \label{it:BartlettConcentration}
    If $\frac{N}{M}\ge \xi$, then for all $s\in \left[-\frac{1}{2},\frac{1}{2} \right] $ we have
    $$ \bbP\left(\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right] \right\|_2 >\epsilon \right) \le \delta.$$
  \item \label{it:BartlettWorst}
   ...
   \item \label{it:BartlettBias}
     If  $M\ge \frac{2 \hat M \|R\|_1}{\epsilon}$, then
     $$
     \sup_{s\in \left[-\frac{1}{2},\frac{1}{2}\right]} \left\|\Phi(s)-\bbE\left[\hat\bPhi(s) \right]\right\|_2 \le \epsilon.
     $$
   % \item \label{it:BTfullPointwise} If the conditions of both \ref{it:concentration}) and \ref{it:bias}) are satisfied, then for all $s\in \left[-\frac{1}{2},\frac{1}{2}\right]$ we have
...
\end{equation}
  ...
  \item \label{it:WelchConcentration}
    If $\frac{S}{1+2\frac{M}{K}}\ge \xi$, then for all $s\in \left[-\frac{1}{2},\frac{1}{2} \right] $ we have
    $$ \bbP\left(\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right] \right\|_2 >\epsilon \right) \le \delta.$$
  \item \label{it:WelchWorst}
    If $\frac{S}{(2M-1)^2\log(2M-1)}\ge \xi$ then 
    $$
     \bbP\left(\sup_{s\in\left[-\frac{1}{2},\frac{1}{2}\right]}\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right] \right\|_2 >\epsilon \right) \le \delta.
     $$
   \item \label{it:WelchBias}
     If  $M\ge\hat M$ and for all $|k|<\hat M$ we have $ \sum_{i=|k|}^{M-1}\frac{v[i-|k|]v[i]}{\|v\|_2^2}\ge 1-\frac{\epsilon}{2\|R\|_1}$, then
     $$
     \sup_{s\in \left[-\frac{1}{2},\frac{1}{2}\right]} \left\|\Phi(s)-\bbE\left[\hat\bPhi(s) \right]\right\|_2 \le \epsilon.
     $$
   % \item \label{it:BTfullPointwise} If the conditions of both \ref{it:concentration}) and \ref{it:bias}) are satisfied, then for all $s\in \left[-\frac{1}{2},\frac{1}{2}\right]$ we have
...
This paper gives a method for deriving non-asymptotic error bounds for a class of spectrum estimators. This method is used to derive error bounds for a variety of classical estimators. Future work includes more fine-grained bounds on the estimators, extensions to continuous time, applications to network identification, and applications to system identification.  
...
\begin{align*}
  z^\top \underline{R}z &= \sum_{k,\ell=-\infty}^{\infty}z[k]^\top R[k-\ell]z[\ell] \\
...
\begin{lemma}
  Let Assumption~\ref{a:gaussian} hold, so that $\by$ is a zero-mean Gaussian process. Let $J\in \bbC^{N\times N}$, $u\in\bbC^{n}$, $v\in \bbC^n$ be such that one of the following conditions holds:
  \begin{enumerate}
...
\begin{lemma}
  Let Assumption~\ref{a:subgaussian} hold. Let $J\in \bbC^{N\times N}$, $u\in\bbC^{n}$, $v\in \bbC^n$ be such that one of the following conditions holds:
  \begin{enumerate}
  \item $J\in \bbR^{N\times N}$, $u\in \bbR^n$, and $v\in\bbR^n$ or
  \item $J$ is  Hermitian and $u=v$. 
  \end{enumerate}
...
\subsection{Proof of Lemma~\ref{lem:covFromSpec}}
...
We complete the proof of Lemma~\ref{lem:covFromSpec} by a covering argument, similar to the proof of Theorem 6.5 of \cite{wainwright2019high}. For any $\delta >0$, the Euclidean ball of dimension $n$ can be covered by a collection of at most $\left(1+\frac{2}{\delta}\right)^n$ balls with radius $\delta$. (See Example 5.8 of \cite{wainwright2019high}.) Let $\cC_n = \{w_1,\ldots,w_{Q_n}\}$ be the centers of such a covering with $\|w_i\|_2\le 1$ and $\delta = \frac{2}{9}$ so that $Q_n \le 10^n$. 
...
\begin{align*}
  \MoveEqLeft[0]
  \left|u^\top \bS u\right| = \left|\left(\hat u + (u-\hat u)\right)^\star \bS \left(\hat u + (u-\hat u)\right))\right| \\
...
\end{align*}
After maximizing both sides and re-arranging, we get $\|\bS\|_2 \le 2\max_{\hat u\in\hat \cC_n}\left|\hat u^\star \bS u \right| $.
...
Using (\ref{eq:expandedEstimator}), followed by the triangle inequality gives:
\begin{align*}
  \MoveEqLeft[0]
  \sup_{s\in\left[-\frac{1}{2},\frac{1}{2} \right]}\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s) \right]\right\|_2\\
  &=
    \hspace{-6pt}\sup_{s\in\left[-\frac{1}{2},\frac{1}{2} \right]}
    \hspace{-2pt}
    \left\|
\sum_{k=-\hat N+1}^{\hat N-1} \hspace{-3pt}e^{-j2\pi sk} \left(\bY B[k] \bY^\top  - \bbE\left[\bY B[k]\bY^\top\right]\right)
    \right\|_2 \\
  &\le \sum_{k=-\hat N+1}^{\hat N-1}\left\| \bY B[k] \bY^\top  - \bbE\left[\bY B[k]\bY^\top\right] \right\|_2.
\end{align*}
Here we also used that $B[k]=0$ for $|k|\ge \hat N$. 

So, if $ \sup_{s\in\left[-\frac{1}{2},\frac{1}{2} \right]}\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right]\right\|_2>\epsilon$, at least one term in the sum must have $\left\| \bY B[k] \bY^\top  - \bbE\left[\bY B[k]\bY^\top\right] \right\|_2 > \frac{\epsilon}{2\hat N-1}$.

For compact notation, set $\hat \epsilon:=\frac{\epsilon}{2\hat N-1}$. Using a union bound, followed by Lemma~\ref{lem:matrixConcentration}, along with $\|B[k]\|_2\le g$ and $\|B[k]\|_F^2 \le g$ gives:
\begin{align*}
  \MoveEqLeft[0]
  \bbP\left(
   \sup_{s\in\left[-\frac{1}{2},\frac{1}{2} \right]}\left\|\hat\bPhi(s)-\bbE\left[\hat\bPhi(s)\right]\right\|_2>\epsilon
  \right) \\
  &\le \sum_{k=-\hat N +1}^{\hat N-1}  \bbP\left(
    \left\| \bY B[k] \bY^\top  - \bbE\left[\bY B[k]\bY^\top\right] \right\|_2 > \hat \epsilon 
    \right) \\
  &\le \sum_{k=-\hat N +1}^{\hat N-1}
    10^{2n} c_{\ref{ConcentrationMult}}e^{-c_{\ref{ConcentrationExp}} \min\left\{\frac{\hat\epsilon^2}{c_{\ref{ConcentrationSubGauss}}^4\|B[k]\|_F^2\|\Phi\|_{\infty}^2},\frac{\hat\epsilon}{c_{\ref{ConcentrationSubGauss}}^2\|B[k]\|_2\|\Phi\|_\infty}\right\}} \\
  &\le (2\hat N-1) 10^{2n} c_{\ref{ConcentrationMult}}e^{-\frac{c_{\ref{ConcentrationExp}}}{g(2\hat N-1)^2} \min\left\{\frac{ \epsilon^2}{c_{\ref{ConcentrationSubGauss}}^4 \|\Phi\|_{\infty}^2},\frac{ \epsilon}{c_{\ref{ConcentrationSubGauss}}^2 \|\Phi\|_\infty}\right\}}.
\end{align*}
The right side is at most $\delta$ if and only if $\frac{1}{g(2\hat N-1)^2\log(2\hat N-1)}\ge \xi$. 
...
\section{Convergence Proofs for Specific Estimators}
\label{app:specific}
...
For all $|k|<N$, we have $b[k] = 1-\frac{|k|}{N}\in [0,1]$. Then $b[k]\ge 1-\frac{\epsilon}{2\|R\|_1}$ if and only if $|k|\le  \frac{\epsilon N}{2\|R\|_1}$. So, to have $b[k]\ge 1-\frac{\epsilon}{2\|R\|_1}$ for all $|k|<\hat M$, it suffices to have $\hat M \le \frac{N\epsilon}{2\|R\|_1}$.  
\hfill\IEEEQED

\subsection{Proof of Proposition~\ref{prop:unbiasedPeriodogram} on Unbiased Periodograms}
For all $|k|<N$, we have $b[k]=1$. So to have $b[k]\ge 1-\frac{\epsilon}{2\|R\|_1}$ for all $|k|<\hat M$ it suffices that $N\ge \hat M$.
\hfill\IEEEQED
...
To prove \ref{it:BTconcentration}) it suffices to show $\|A\|_2 \le \frac{(2M-1)}{N}$ and $ \|A\|_F^2\le \frac{(2M-1)}{N}$. 
  ...
  It suffices to show that $\|B[k]\|_2\le \frac{1}{N}$ and $\|B[k]\|_F^2\le \frac{1}{N}$ for $|k| < M$. Direct calculation gives: 
    \begin{align*}
...
%
if and only if $|k|\le \frac{N\epsilon}{2\|R\|_1}$. Thus, if $\hat M\le \frac{N\epsilon}{2\|R\|_1}$, the bias bound from \ref{it:BTbias})
will be achieved as long as (\ref{eq:BTWeightBound}) holds for $|k|<\hat M$ and $w[k] \in [0,1]$ for $|k|\ge \hat M$. 
\hfill\IEEEQED
...
Now we prove \ref{it:BartlettWorst}). We have $\hat N=M$. So the result follows since for $|k|<M$ direct calculation gives
\begin{align*}
  \|B[k]\|_2 &= \frac{1}{N} \\
...
$$
Let $\hat \epsilon = \frac{\epsilon}{2\|R\|_1}$. We see that $b[k] \ge 1-\hat\epsilon$ if and only if $|k| \le M\hat\epsilon$. So, to ensure that $b[k]\ge 1-\hat \epsilon$ for all $|k|< \hat M$, it suffices to have $\hat M \le M\hat \epsilon$.
\hfill\IEEEQED
...
\setlength\arraycolsep{2pt}
  ...
  SA= \sum_{i=0}^{S-1}\begin{bmatrix}
    ...
    Now we prove \ref{it:WelchWorst}). First note  that $\hat N=M$. We will show that $\|B[k]\|_2\le \frac{1}{S}$ and $\|B[k]\|_F^2 \le \frac{1}{S}$ for $|k|<M$. 
...
b[k]=\begin{cases}
  ...
  \end{cases}
  $$
  So, it suffices to have  $M\ge \hat M$ and for $|k|<\hat M$ to have
  $$
