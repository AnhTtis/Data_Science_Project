...
The ELBO optimization process for semantic segmentation involves two main steps, as described in \cite{van2017neural}.
%
...
The final training objective of this stage cane be then revised as:
\begin{equation}
\label{equ:opt_unlabel}
    \min_\psi D_{KL}\left(q_{\phi}(z|\tilde{c}), p_{\psi}(z|x)\right) + p_\xi(\bar{c}|z).
\end{equation}
