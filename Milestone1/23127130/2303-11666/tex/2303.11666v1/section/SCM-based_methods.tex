...
As represented in Figure~\ref{fig:junction}(c), a collider node occurs when it receives effects from two or more other factors. Collider exists in recommender systems. For instance, item positions in the ranking list are influenced by user preference and item popularity. 
...
\begin{table*}[pt]
  ...
  \setlength{\tabcolsep}{2.2mm}{
    ...
    \multirow{6}[2]{*}{\makecell{Causal\\ recommendation \\with mediator \\structure}} & ~\cite{choi2011influence} & Mediation analysis & -     & Effect of social presence & 2011 \\
          ...
          & CCF~\cite{xu2021causal} & \makecell{Intervention on\\ the mediator, \\counterfactuals} & \makecell{NCF~\cite{he2017neural}, \\GRU4Rec~\cite{hidasi2015session}, etc.} & Historical bias & 2021 \\
          & MACR~\cite{wei2021model} & TIE   & \makecell{MF, LightGCN \\ (multi-task)} & Popularity bias & 2021 \\
...
\end{definition}
If a set of variables $X$ satisfies the back-door criterion for $T$ and $Y$, the causal effect of $T$ on $Y$ is given by the formula:
\begin{equation}
\textrm{Pr}(Y=y \mid do(T=t)) =\sum_{x} \textrm{Pr}(Y=y \mid T=t, X=x) \textrm{Pr}(X=x),
\end{equation}
which adjusts fewer variables compared to the Causal Effect Rule (Definition \ref{def:ce_rule}).
...
In addition to leveraging back-door adjustment, some works block the back-door path by the direct intervention on the treatment variable~\cite{wang2021deconfounded, zhang2021causal, he2022addressing, wang2022causal, zhan2022deconfounding, rajanala2022descover}. For example, Wang et al.~\cite{wang2021deconfounded} propose the framework named DecRS (Deconfounded Recommender System) to eliminate bias amplification through intervention on the user representation $U$, which removes the effect of the historical user distribution over item groups $D$ on $U$, as Fig. \ref{fig:backpath} (a) shown. Zhang et al.~\cite{zhang2021causal} propose PDA (Popularity-bias Deconfounding and Adjusting) to eliminate the effect of item popularity $P$ through intervention on the item $I$ (see Fig. \ref{fig:backpath} (b)), denoted as:
\begin{equation}
\textrm{Pr}(Y=y \mid do(U=u, I=i))=\sum_{p} \textrm{Pr}(y \mid u, i, p) \textrm{Pr}(p \mid u,i)
=\textrm{Pr}(y \mid u, i, p) \textrm{Pr}(p),
\end{equation}
where $U$ denotes the user representation and $Y$ represents interactions. $\textrm{Pr}(y \mid u, i, p)$ and $\textrm{Pr}(p)$ are learned separately. It is worth mentioning that PDA can leverage popularity bias to enhance the recommendation performance by adjusting $\textrm{Pr}(p)$ in the inference stage, which can be regarded as counterfactual inference.
...
\centering
\vspace{-1mm}
\includegraphics[height=0.45\textheight,trim=0 10 0 0 ,clip]{pic/he_type.pdf} %l b r t
\vspace{-1mm}
...
\centering
\vspace{-1mm}
\includegraphics[height=0.09\textheight,trim=0 0 0 0 ,clip]{pic/iv_ex.pdf} %l b r t
\vspace{-1mm}
\caption{The causal graph of a general setup for instrumental variables, where $Z$ is an instrumental variable.}
\vspace{-1mm}
\label{fig:iv_ex}
\end{figure}
...
\begin{table*}[htbp]
  ...
  \setlength{\tabcolsep}{2.0mm}{
    ...
    CPR~\cite{yang2021top} & Intervention on the treatment & MF, LightGCN, etc. & Data insufficiency & 2021 \\
    CauSeR~\cite{gupta2021causer}. & Intervention on the treatment & SR-GNN~\cite{wu2019session} & Popularity bias in SBRSs & 2021 \\
    MCT~\cite{tran2021recommending} & Back-door criterion, CATE & (self-designed) & Disability employment & 2021 \\
    DecRS~\cite{wang2021deconfounded} & Intervention on the treatment & FM, NFM~\cite{he2017neuralfactorization} & Bias amplification & 2021 \\
    PDA~\cite{zhang2021causal} & Intervention on the treatment & MF    & Popularity bias & 2021 \\
    CR~\cite{wang2021clicks} & Back-door criterion, TIE & \makecell{MMGCN~\cite{wei2019mmgcn}\\(multi-task)} & Clickbait & 2021 \\
    DCR~\cite{he2022addressing} & Intervention on the treatment & NFM   & \multicolumn{1}{c|}{Observed confounding bias} & 2022 \\
    CaDSI~\cite{wang2022causal} & Intervention on the treatment & (self-designed) & Observed confounding bias & 2022 \\
    D2Q~\cite{zhan2022deconfounding} & Intervention on the treatment & (self-designed) & Duration bias & 2022 \\
    DeSCoVeR~\cite{rajanala2022descover} & Intervention on the treatment & (self-designed) & Venue recommendation & 2022 \\
    ...
    COCO-SBRS~\cite{song2023counterfactual} & Intervention on the treatment &  (self-designed) & Unobserved confounding bias in SBRSs & 2023 \\
    \bottomrule
    \end{tabular}}%
    \label{tab:confounder}%
    \begin{tablenotes}
    \small
    \item Here, RL means reinforcement learning and WOM means word-of-mouth.
    \end{tablenotes}
    \vspace{-4mm}
\end{table*}%
