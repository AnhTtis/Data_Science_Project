...
\begin{equation}
    \frac{d^2-1}{d^2} \lessapprox g \lessapprox q \cdot \frac{d^2-1}{d^2}.
\end{equation}
...
Due to our previous arguments, we can expect the approximation to not hold anymore by the time $w_D$ is of order $n$ since then the case of \eqref{eq:growth_saturated} will dominate. In this case we say that the weight is \emph{saturated}, and we can estimate the order of magnitude of the saturation depth $D_{\textrm{sat}}$ by requiring that $g^{D_{\textrm{sat}}} \cdot w_0 \lessapprox n$, leading to:
\begin{equation}
\label{eq:D_max}
    D_{\textrm{sat}} \approx \log_g \left( \frac{n}{w_0} \right).
\end{equation}
