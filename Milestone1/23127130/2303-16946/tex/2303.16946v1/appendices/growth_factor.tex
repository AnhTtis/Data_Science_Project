...
Given a generic string of $n$ generalized Pauli operators with local dimension $d$ and total weight $w \ll n$, we can estimate the relative weight growth $g$ the string experiences from one layer of $n/q$ random Cliffords being applied to random disjoint substrings of length $q$. The weight $w_k$ at the $k$th layer can therefore be estimated as
\begin{equation}
...
\end{equation}
However, for our purposes we will always have $w \ll n$, which makes it more likely for $g$ to be closer to the upper bound. Hence we can assume that
\begin{equation}
...
Due to our previous arguments, we can expect the approximation to not hold anymore by the time $w_D$ is of order $n$ since then the case of \eqref{eq:growth_saturated} will dominate. We can therefore estimate the order of magnitude of $D$ by requiring that $g^{D_{\max}} \cdot w_0 \lessapprox n$ and hence
\begin{equation}
\label{eq:D_max}
    D_{\max} \approx \log_g \left( \frac{n}{w_0} \right).
\end{equation}
...
\begin{figure}[htb]
    ...
    \caption{Averaged weight growth factors $w_D / w_{D-1}$ of a single Pauli string ($d=3, n=128$) with increasing circuit depth $D$, together with our estimate of the effect growth factor $g$ \eqref{eq:growth_factor} and the maximum Depth(s) $D_{\max}$ \eqref{eq:D_max} for which it can be considered to hold. While \eqref{eq:D_max} indeed provides a good maximum circuit depth for which the data and our estimate for $g$ approximately coincide, a tighter bound can be achieved by using $q$ as the base of the logarithm instead of $g$.}
