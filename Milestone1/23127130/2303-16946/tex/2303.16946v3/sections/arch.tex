...
\end{equation}
The total number of qudits is therfore
\begin{equation}
...
We next discuss the entanglement and complexity of the RG-inspired network. It is straightforward to establish via bond counting that the network has the potential to encode volume law entanglement for sub-regions of the UV qudits. We also explicitly demonstrate that volume law entanglement is achievable within the Clifford model discussed below in Sections~\ref{sec:clifford}, \ref{sec:numerical}, and \ref{sec:code_structure}.
...
\end{equation}
where $J_i$ are a set of free parameters that determine the energy scale associated with each thermal qudit. This setup is described in more detail in appendix \ref{sec:entropy_scaling}. 
...
\label{eq:stab_entropy_approx}
    S \lessapprox k \log(d) + (d-1) (N-k) \cdot \frac{\alpha}{\gamma} \cdot \Gamma\left(\frac{\alpha}{\gamma} + 1\right) \cdot (\beta \Lambda)^{-\alpha/\gamma} \propto (T/\Lambda)^{\alpha/\gamma},
\end{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.9\textwidth]{figures/entropy_scaling.png}
    \caption{Logarithmic scaling of the exact Gibbs entropy $S_{\text{stab}}$ associated to $H$, and the low-temperature approximation $S_{\text{approx}}$ for $L=20$, $r=2$, $k=1$, $d=2$, $\Lambda=1$ and $\gamma = 0.4$. Both match almost exactly for our choice of parameters and small $T/\Lambda$, confirming the existence of a scaling law. The same is also the case for other choices of $\gamma$ (the only significant free parameter), as seen in appendix figure \ref{fig:entropy_scaling_appendix}}.
