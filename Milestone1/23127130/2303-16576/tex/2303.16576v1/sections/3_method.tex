...
\begin{equation}
    q(x_{1:T}) = \prod_{t=1}^{T} q(x_t|x_{t-1}),\quad
...
The training loss is calculated as the reconstruction error between the actual noise, $\epsilon$, and the estimated noise, $\epsilon_\theta$.
...
\subsubsection{Latent Diffusion Models (LDM).} Diffusion Models have demonstrated remarkable performance in image generation and transformation tasks~\cite{ho2020denoising,kingma2021variational,mittal2021symbolic,kong2020diffwave}.
However, their computational cost is high due to the size of the input data and the use of cross-attention in images.
To address this issue, Latent Diffusion Models were introduced in~\cite{rombach2022high} to model the data distribution in a a lower-dimensional latent representation space.
...
The figure shows that below 500 timesteps the quality of the images is really affected, thus for to make sure the generated samples are not affected dramatically we proceed with a value of 600.
...
Finally, to create the text input condition $c_\tau$ a dot-product attention layer is used, defined as $Att(Q,K,V) = softmax(QK)V$, to create a weighted sum of the character representations.
...
We perform extensive experiments using the IAM offline handwriting database on word-level.
Similar to \cite{mattick2021smartpatch} and \cite{kang2020ganwriting}, we use the Aachen split train set to train the Diffusion model.
Thus, during training the model sees 339 writer styles and approximately 45K words.
For training, all images are resized to a fixed height of $64$ pixels, retaining the aspect ratio.
Images having width smaller than $256$ pixels are center-padded to $256$ width while larger ones are resized to the max width.
AdamW~\cite{loshchilov2017decoupled} is used as the optimizer during training with a learning rate of 0.0001.
