...
In terms of neural network architecture, they are comprised of two network parts, named the Encoder and the Decoder.
...
prompted many researchers to experiment with their use in a wide spectrum of different applications. %other applications as well.
Diffusion models \cite{SohlDickstein2015DeepUL} are based on a probabilistic framework like VAEs or NFs, but propose a different approach to the problem of image syntehsis, cast in its standard form as density estimation followed by sampling.
...
In this work, instead of using text only as description of the image contents, 
...
In Section \ref{sec:bg_related_work}, we briefly review the theoretical background for the used methods and present an overview of the related work. 
Our proposed method is introduced in Section \ref{sec:method}, while Section \ref{sec:experiments} includes the experimental setup, evaluation and results.
Finally, we discuss future directions and conclusions in Section \ref{sec:conclusion}. 
