...
\newcommand{\RL}[1]{{\color{red!80!black} [Russ: #1]}}
\newcommand{\rl}[1]{{\color{green!50!black} #1}}
\newcommand{\newrl}[1]{{\color{blue!50!green} #1}}
\newcommand{\mm}[1]{{\color{magenta}#1}}
\newcommand\mob{\textnormal{\textsf{M\"ob}}}  % M\"obius group
\newcommand{\mmn}[1]{{\color{yellow!80!black} #1}}
...
\begin{theorem}[\textsc{Convergence of low-intensity tessellations}]\label{thm.convhyp} Let $ \mathbf{X}_{d}^{{(\lambda)}}$ be a Poisson point process with intensity $\lambda>0$ on the $d$-dimensional hyperbolic space $ \mathbb{H}_{d}$ with $d \geq 2$. Then we have the following convergence in law
...
\end{itemize}
In particular,  in dimension {$2$}, the union $ \partial \mathcal{V}_{2}$ of the boundaries of the tiles  is a random embedding of the 3-regular tree in $ \mathbb{H}_{2}$ with geodesic edges whose law is invariant under isometries of the hyperbolic plane.
\end{thm}
...
\begin{overpic}[width=\textwidth]{figs_final/C2_median_correct_nolabels_ee0b0a.pdf}
   \put (45,45) {$\origin$}
\end{overpic}   
     \end{minipage}
     \hspace{12pt}
     \begin{minipage}{.47\textwidth}
\begin{overpic}[width=\textwidth]{figs_final/C1_BM_jewel_median_11_nolabels_transp_rotated_5_arrow_2_ee0b0a}
   \put (26,18) {$\origin$}
\end{overpic}   
     \end{minipage}
...
\rlabel e.defc_d
{c_d:=2^{2-d}\frac{\pi^{\frac{d}{2}}}{\Gamma\big(\frac{d}{2}\big)}
}
...
\begin{figure}[!h]
 \begin{center}
\includegraphics[height=0.3\linewidth]{figs_final/foam_Seed12345_PPP_20x20_stat_median_z1_origin-new.pdf}

\includegraphics[width=.7\linewidth]{figs_final/UHPbdry_seed_12345_8000pts_width_200_for_10_median}
...
\begin{figure}[!h]
 \begin{center}
\includegraphics[width=.77\linewidth]{figs_final/foam-cake_Seed12348_Nuclei1000000_PPP0_yzPlane_colorSeed59808LAB_16x9x12_origin-crop.pdf}
...
Let us denote a point on the Gromov boundary by $\theta$ and fix the associated representative function $ \mathrm{d}_{\theta}(\cdot)$ so that $ \mathrm{d}_{\theta}(\origin)=0$ for all $\theta \in \partial E$. In particular, if $\theta, \theta' \in \partial E$ and $x \in E$, one can make sense of the ``difference of distances''
 \begin{equation} \mathrm{d}_{\theta}(x) - \mathrm{d}_{\theta'}(x) \in \mathbb{R}. \label{eq:difference}  \end{equation}  We will then enhance the Gromov boundary by the addition of a second coordinate, real numbers that we call the \dfn{delays}, yielding the \dfn{extended ideal boundary}, $ \widehat{\partial E} = \partial E \times \mathbb{R}$. Extending the preceding display, if $x \in E$ and $(\theta,\delta), (\theta',\delta') \in \widehat{\partial E}$ are two extended ideal points, we say that $x$ is \dfn{closer} to $(\theta,\delta)$ than to $(\theta',\delta')$ if 
...
Hence, if we write 
\rlabel e.horoaction
{\phi(\theta, \delta) := \bigl(\phi(\theta), \delta + \ud_\theta(\phi^{-1}(\origin))\bigr),}
...
We will use the \dfn{Fell topology} on the collection of closed subsets of $E$, which is generated by the sets $\{F \sut F \cap C = \varnothing\}$ for compact $C \subset E$ and the sets $\{F \sut F \cap G \ne \varnothing\}$ for open $G \subseteq E$. This topology makes the collection of closed subsets compact, while if the empty set is omitted, then it becomes locally compact (see \cite[Section 12.2]{SchneiderWeil}).
...
\begin{thm}\label{lem.convvor} Suppose that for each $k$, we have a sequence of points $(x_i^{ (k)} \ST i \geq 1)$ ranked by increasing distances to $\origin$ on a space $(E,d)$ that together satisfy the two conditions:
...
\end{enumerate}
Then the Voronoi {diagrams} $ \mathrm{Vor}(  x_{i} ^{(k)} \ST \allowbreak i \geq 1)$ converge as $ k \to \infty$ to the ideal {diagram} $ \mathrm{Vor}\bigl( (\theta_{i}, \delta_{i}) \ST {i \geq 1}\bigr)$.
\end{thm}
...
An isometric mapping from $\mathbb{B}_d$ onto $\mathbb{U}_d$ is given by the generalized Cayley transform, which is the diffeomorphism $\kappa\colon \mathbb{B}_d \rightarrow \mathbb{U}_d$
\begin{align*}
\kappa(z) &= x := \frac{1}{z_1 ^2 + \dots + z_{d-1} ^2 + (z_d-1)^2}\big( 2z_1, \dots, 2z_{d-1}, 1-|z|^2 \big) , \\
\kappa^{-1} (x) &= z := \frac{1}{x_1 ^2 + \dots + x_{d-1} ^2 + (x_d+1)^2}\big( 2x_1, \dots, 2x_{d-1}, |x|^2 -1 \big) 
\end{align*}
...
%
 ...
 Passing to the upper half-space model $ \mathbb{U}_{d}$, we find that the image of the uniform measure over $ \partial \mathbb{B}_{d}$ onto $\partial{U}_{d} = \mathbb{R}^{d-1}$ is given by:
...
We call $\mathrm{Vor}( \boldsymbol{\Theta},  \mathbf{D})$ the \dfn{ideal Poisson--Voronoi tessellation} of $ \mathbb{H}_d$ and denote it by $ \mathcal{V}_{d}${; we still need to prove that it is a tessellation}.
...
\paragraph{Convergence of proto-delays.} By the mapping theorem for Poisson processes, it is simple to see that as $\lambda \to 0$, we have 
...
These can be proved via the following simple calculation. Recall that the volume growth function for hyperbolic space is given by $$f_{d}({u}) := \mathrm{Vol}_{\Hyp{d}}\big(B_\Hyp{d}(\origin,{u})\big)=\Omega_d \int_0^{{u}} \left(\sinh{\rho}\right)^{d-1} \;  \mathrm{d}\rho, $$ where $\Omega_d:= 2\frac{\pi^{d/2}}{\Gamma(\frac{d}{2})}$ is the Euclidean volume of $\mathbb{S}_{d-1}$. A straightforward calculation shows that for all $x,y \in \mathbb{R}$, we have
$$
...
\]
We call the first coordinate of a point in the corona its \dfn{angle} and the second coordinate its \dfn{radius}. We will also use the letter ``r" for radii $r, r_i, R, R_i$ and the letter ``$\delta$'' or ``D" for delays. 
It follows from the first part of the proof of \cref{cor.thetaD} that $(\theta _i, \frac{c_d}{d-1} e^{(d-1)D_i})_{i \geq 1}$ is a Poisson point process in the corona with intensity
\begin{equation}  \label{eq:defmud}
\mu_{d} :=  \mathrm{Unif} \otimes \mathrm{Leb};
\end{equation}
the points of this PPP will be called \dfn{ideal nuclei}. In these new coordinates where $r = \frac{c_d}{d-1} \ue^{(d-1)\delta}$, we introduce the \dfn{(exponential) separation} between $z$ and a point $(\theta,r)$ in the corona in terms of the linear separation between $z$ and an ideal point $(\theta,\delta)$ as follows:
...
Here, one associates to every Voronoi vertex $v$ the ideal simplex formed from the $d+1$ angles of the ideal nuclei whose separation from $v$ is smallest{. The resulting collection of ideal simplices forms the Delaunay tessellation}; complete details are given in \cref{s.delaunay}.
...
\rproof
Since $e^{\arcsinh t} -e^{-\arcsinh t}=2t$, we have  $e^{\arcsinh t} \sim 2t$ as $t \to \infty$. Therefore, if $\uball_d \ni x_n \to \theta \in \sph$ as $n \to\infty$, we have for all $y \in \uball_d$ that 
\rlabel e.asympdist
{\exp\bigl\{\ud_{\HH_d}(x_n, y)\bigr\}
\sim
\frac{4|\theta - y|^2}{\bigl(1 -|x_n|^2\bigr)\bigl(1-|y|^2\bigr)}.}
Applying this to $y = z$ and to $y = \origin$ yields
\rlabel e.evallim
{\ue^{\ud_\theta(z)}
=
\frac{|z-\theta|^2}{1-|z|^2}.}
...
The convergence in the above theorem is a convergence in law, but actually there are explicit ways to realize the coupling towards the end of the proof for different $\lambda $  via dilation as follows:
...
$$ \mathrm{Angle}(\tilde{X}_{i}^{(\lambda)}) \coloneqq {\Theta}_{i} \quad \mbox{ and }\quad \mathrm{d}_{ \mathrm{Euc}}( \mathbf{0}, \tilde{X}_{i}^{(\lambda)}) \coloneqq v_d^{-1}\bigl(v_d({R}_{i})/\lambda^{d-1}\bigr).$$
...
\]
where $\theta := (x_1, \dots, x_{d-1})$, showing that the proto-delays converge a.s. Write $p(x) := \bigl(\theta, \frac{1}{(d-1)} x_d^{1-d}\bigr)$. Since $\XX^{(\lambda)}$ converges to $\cells_d$ by \cref{cor.thetaD}, it remains to show that $\bigl\{p(x) \ST x \in \XX\bigr\}$ is a Poisson process of intensity $\Leb \otimes \Leb$. This follows from the Poisson mapping theorem.
\Qed

We can now conclude the proof of \cref{t.weighted}:

 \begin{proofof}{\cref{t.weighted}}
Knowing that the limit $\cells_d$ exists and described by \cref{cor.thetaD}, the proof follows by appeal to \eqref{eq:closernuclei} in the case of the ball model.
...
If not, then there exist two ideal nuclei, $X$ and $Y$, whose cells, $C(X)$ and $C(Y)$ respectively, intersect in a nonempty set $S$ that is not a face of $C(X)$. That means a.s.\ there is some $w \in C(X) \setminus S$ that lies in the geodesic hyperplane spanned by $S$, in other words, $\bud(w, X) = \bud(w, Y)$. Since $w \notin C(Y)$, there is some ideal nucleus $Z$ with $\bud(w, Z) < \bud(w, Y)$. However, $\bud(w, X) \le \bud(w, Z)$, and these two inequalities contradict $\bud(w, X) = \bud(w, Y)$.

To show normality, we first claim that a.s., at most $d+1$ tiles can share a common point.
Given $d+1$ ideal nuclei, {\eqref{eq:closernuclei} implies that} there is a.s.\ at most one point in $\HH_d$ at equal separation from these ideal nuclei. If we label the ideal nuclei in order of increasing radius as $X_1, X_2, \ldots$, it follows that for each {$(d+2)$-tuple $1 \le k_1 < k_2< \cdots < k_{d+2}$}, the probability is $0$ that $X_{k_1}, \ldots, X_{k_{d+1}}, X_{n+1}$ have the same separation from any point in $\HH_d$. Therefore, a.s., no set of $d+2$ ideal nuclei is at equal separation from any point in $\HH_d$, which proves our claim. 

Next, for points $x_1, x_2, \dots, x_m$ in the corona, write $S(x_1, \ldots, x_m)$ for the set of points at equal separation from $x_i$ with $1 \le i \le m$. An argument very similar to the preceding one shows more generally that a.s., the ideal nuclei are in general separation position, meaning that if $Y_1, Y_2, \ldots, Y_m$ are distinct ideal nuclei with $m \le d+1$, then there is no additional ideal nucleus $Y_{m+1}$ such that $\varnothing \ne S(Y_1, \ldots, Y_m) = S(Y_1, \ldots, Y_m, Y_{m+1})$. Therefore, the totally geodesic plane $S(Y_1, \ldots, Y_m)$ has dimension $d-m+1$ or is empty.

Finally, suppose that $S$ is a $k$-face of $\cells_d$ with $0 \le k \le d-1$. Because $\cells_d$ is face-to-face a.s., there exist distinct ideal nuclei $Y_1, Y_2, \ldots, Y_m$ the intersection of whose cells is $S$. {By what we proved in the paragraph before the last one, $m \le d+1$. Thus, $S$ affinely spans $S(Y_1, \ldots, Y_m)$ and so by the previous paragraph, $d-m+1 = k$.}
\end{proof}
...
Let $B_u = B_u(\origin)$ be the ball centered at $\origin$ with hyperbolic radius $u$. Write $\alpha := 1/(2\ue^{2(d-1)u} - 1)$.
...
Since this bound tends to 0 as $u \to\infty$, it follows that $\ocell_d$ a.s. contains no limit point in $A$. Since $\alpha$ can be chosen as close to $2\pi$ as we wish, we conclude that $\ocell_d$ has only the end ${\Theta}_1$. There are only countably many cells, whence the same holds for all cells.
\Qed
...
\subsubsection{M\"obius action on the corona via the Poisson kernel}
...
In \cref{s.zerocell}, we consider the zero cell in the upper half-space model with its ideal nucleus at $\infty$. In \cref{s.asymp0cell}, we explain how the zero cell is closely related to the Laguerre diagrams studied in \cite{gusakova2022delaunay} for certain choices of parameters there. In particular, a.s.\ each set of $d$ ideal nuclei in $\R^{d-1}$ that form an ideal Delaunay simplex when joined with the ideal nucleus at $\infty$ gives a Euclidean simplex in $\R^{d-1}$ that is one of the so-called $\beta'$-Delaunay simplices, and conversely. These $\beta'$-Delaunay simplices tessellate $\R^{d-1}$ \cite[p.~1263]{gusakova2022delaunay}.  It follows that the set of ideal Delaunay simplices containing $\infty$ is locally finite a.s.\ and covers a neighborhood of $\infty$, whence each ideal Delaunay simplex containing $\infty$ has the property that each of its $(d-1)$-faces containing $\infty$ is a face of another such Delaunay simplex. The same then holds a.s.\ for every ideal nucleus, whence $\dcells_d$ is locally finite and covers $\HH_d$ a.s.
\Qed

There is an equivariant, measurable bijection between the $k$-faces of $\cells_d$ and the $(d+1-k)$-faces of $\dcells_d$ (and similarly for the positive-intensity tessellations), known as duality: the $k$-face of $\cells_d$ given by the intersection of the cells corresponding to ideal nuclei $Y_1, \ldots, Y_{d+1-k}$ is mapped to the $(d+1-k)$-face of $\dcells_d$ given by the convex hull of $Y_1, \ldots, Y_{d+1-k}$.
...
Since $ \mathcal{V}_{d}$ is invariant under isometries of $ \mathbb{H}_{d}$, we can define the intensities of various geometric quantities such as $k$-faces. Using existing results in stochastic geometry, those quantities can be explicitly computed for the ideal Poisson--Voronoi tessellations. Let us first recall the classical construction of the Palm distribution for general stationary point processes.


\subsection{Palm measures and typical points}\label{s.palm}
\subsubsection{Euclidean space}
Let $\XX$ be a point process, or, more generally, a random measure on $\R^d$ whose law is invariant under translations. Then $A \mapsto \EE\bigl[\XX(A)\bigr]$ is a translation-invariant map on Borel subsets of $\R^d$, whence is a multiple of Lebesgue measure; that constant multiplier is called the \dfn{intensity} $\mathrm I_{\XX}$ of $\XX$. Define $\mu(B) := \EE\int_{\R^d} \II{B}(x, \XX - x) \,\ud\XX(x)$ for $B$ in the product $\sigma$-field generated by Borel sets in $\R^d$ and the evaluation functionals on Borel sets. Because the law of $\XX$ is invariant, $\mu$ is invariant under translations in the first coordinate, that is, $\mu(B_1 \times B_2) = \mu\bigl((B_1 - x) \times B_2\bigr)$ for Borel $B_1 \subseteq \R^d$, measurable $B_2$, and $x \in \R^d$. Hence, if $\Intn_{\XX}$ is positive and finite, then $\mu = \mathrm I_{\XX} \cdot \Leb \otimes \PP^\origin$ for a unique probability measure $\PP^\origin$ on {measures} on $\R^d$ \cite[Theorem 3.3.1]{SchneiderWeil}. {For example, $\PP^\origin(B_2) = \Intn_{\XX}^{-1} \cdot \EE \int_{x \in B_1} \II{B_2}(\XX - x) \,\ud\XX(x)$ for every $B_1$ with $\Leb(B_1) = 1$.} The measure $\PP^\origin$ is called the \dfn{Palm distribution} of $\XX$. 
...
\[
g(x, \XX) := \int_{\mob_d^{x}} \II{B_1}(x) \II{B_2}(\XX \circ \phi) \,\ud\kappa_x(\phi).
\]
...
\subsubsection{Marked random measures}
...
Similar {considerations apply} for the faces of the (possibly ideal) Delaunay tessellations $\dcells_d^{(\lambda)}$ or $\dcells_d$ and for the Delaunay simplices themselves. Here, it may be necessary to use \dfn{(generalized) centers} of the $k$-simplices, where we no longer require that the generalized center of a $k$-simplex lie in that simplex or we use extra randomness. In our case, we may use the randomness of the PPP of (ideal) nuclei. In fact, for the generalized center of a Delaunay $k$-simplex, we will use the center of the corresponding $(d+1-k)$-face of the Voronoi tessellation. Here, $1 \le k \le d+1$, although in the case of positive-intensity nuclei, rather than ideal nuclei, we may allow $k = 0$. The only ones we will use for the ideal Delaunay tessellation are the generalized centers of the ideal Delaunay simplices, which are, then, the corresponding Voronoi vertices.



\subsection{Intensities and ergodic properties}  {Let $\cells_{d, k}$ denote the collection of $k$-faces of cells of $\cells_d$; in particular, $\cells_{d, d} = \cells_d$.} For a $k$-face $\mathfrak s \in \cells_{d, k}$, write $\nu_{ \mathfrak{s}}$ for the $k$-dimensional hyperbolic measure on $ \mathfrak{s}$. Consider the random measures
$$ \mu_{d,k} \coloneqq \sum_{ \begin{subarray}{c} \mathfrak{s} \in {\cells_{d, k}} \end{subarray}} \delta_{ \mathrm{Cen}( \mathfrak{s})} \quad \mbox{and} \quad \tilde{\mu}_{d,k}:= \sum_{ \begin{subarray}{c} \mathfrak{s} \in {\cells_{d, k}} \end{subarray}}\nu_{ \mathfrak{s}}.$$
...
(resp.,~\dfn{integral  $k$-face intensity}) of $ \mathcal{V}_{d}$. The latter intensity is called by \cite[p.~141]{SchneiderWeil} the \dfn{$k$-volume
density} or \dfn{specific $k$-volume}. %pub%; see Theorem 4.5.1 on p.~142.

{
The counting intensities do not depend on the choice of center function. To see this, let $\Cen$ and $\Cen'$ be two center functions. Denote by $\mu_{d, k}$ and $\mu'_{d, k}$ the corresponding random measures.
Define the measure
\[
\widehat\mu_{d,k} := \sum_{ \mathfrak{s} \in \cells_{d, k} } \delta_{ \mathrm{Cen}( \mathfrak{s})} \otimes \delta_{\Cen'( \mathfrak{s})}
\]
...
Define the measure
\[
\widehat\mu_{d,k} := \sum_{ \begin{subarray}{c} \mathfrak{s} \in {\cells_{d, k}}\end{subarray}} \delta_{ \mathrm{Cen}( \mathfrak{s})} \otimes \nu_{ \mathfrak{s}}
\]
...
\[
\widehat\mu_{d,k}^{(\lambda)} := \sum_{ \mathfrak{s} \in \cells_{d, k}^{(\lambda)},\ \mathfrak{t} \in \cells_d^{(\lambda)} } \II{\mathfrak{s} \subset \mathfrak{t}} \cdot \delta_{ \mathrm{Cen}( \mathfrak{s})} \otimes \delta_{ \mathrm{Cen}( \mathfrak{t})}
\]
on $\HH_d \times \HH_d$. For Borel $A \subseteq \HH_d$, we have 
\[
...
the expected number of $k$-faces of the typical cell times $\Intn_{d,d}^{(\lambda)} \cdot \Vol_{\HH_d}(A)$. Hence the displayed equality gives our claim.
...
\rproof 
\textbf{(i)}
Let $f_\epsilon({\cells_d}) := \mu_{d,k}\bigl({B_\epsilon}\bigr)/b_\epsilon$ be the number of $k$-faces within distance $\epsilon$ of {$\origin$} divided by
$b_\epsilon$.  Note that $\EE[f_\epsilon({\cells_d})] = \mathrm{I}_{d,k}$. Write
\[
A(u, \epsilon) := b_u^{-1} \int_{F_u} f_\epsilon(g \cells_d) \,\ud\mu(g).
\]
...
\textbf{(iii)}
Let $f_{\epsilon, t}({\cells_d})$ be the number of vertices within distance $\epsilon$ of {$\origin$} whose separations from their nuclei are at most $t$, divided by ${\mathrm{I}_{d,0}} \cdot b_\epsilon$.  Then $\EE[f_{\epsilon, t}(\cells_d)] = \frac{1}{d!}\int_{0}^{t} x^{d} e^{-x} \,\ud x $ for all $\epsilon, t > 0$ by \cref{p.typ-sep} and the refined Campbell theorem.
{Write
\[
A(u, \epsilon, t) := b_u^{-1} \int_{F_u} f_{\epsilon, t}(g {\cells_d}) \,\ud\mu(g).
\]
According to \cref{t.nevo}, $\lim_{u \to\infty} A(u, \epsilon, t) = \frac{1}{d!}\int_{0}^{t} x^{d} e^{-x} \,\ud x $ a.s.\ for each $\epsilon, t > 0$.
We have the inequalities
\[
   \Intn_{d, 0} b_{u-\epsilon} A(u-\epsilon, \epsilon, t) \le \mu_{d,0}(B_u) D_u(t) \le \Intn_{d, 0} b_{u+\epsilon} A(u + \epsilon, \epsilon, t).
\]
Divide by $\Intn_{d,0} b_u$, take $u \to\infty$, and then take $\epsilon$ to 0 along a sequence. The result of part (i) for $k = 0$ then gives $\lim_{u \to\infty} D_u(t) = \frac{1}{d!}\int_{0}^{t} x^{d} e^{-x} \,\ud x $ a.s. Use this for all rational $t$ together with monotonicity in $t$ to get the result for all $t$.}
\Qed
...
\end{itemize}
with $\alpha := 2^d \lambda/\tilde c_{d, d}$, $\tilde c_{a, b} := \Gamma(b)/\bigl(\pi^{a/2} \Gamma(b-a/2)\bigr)$, and 
\[
I_{\alpha, d}^*(d)
=
...
Taking the limit $\lambda \to 0$, we can approximate in the integrand $\sinh \theta$ by $\ue^\theta/2$ and likewise $\sinh \varphi$ by $\ue^\varphi/2$. Approximate the resulting inner integral by $\ue^{(d-1)\varphi}/\bigl((d-1)2^{d-1}\bigr)$ and change variables to $\psi := \ue^{(d-1) \varphi}$. Then observe that the resulting integral is asymptotic to $1/\bigl(2^{d(d-1)}\alpha \tilde c_{1, (d+1)/2}\bigr)$ times the $d$th moment of an exponential random variable with parameter $(d-1)2^{d-1}/(\alpha \tilde c_{1, (d+1)/2})$. 
\Qed
...
We can combine \cref{p.typ-face} with results of Isokawa \cite{isokawa2000H2,isokawa2000H3} to compute integral intensities in dimensions $2$ and $3$.


\begin{prop}[{\textsc{Integral face intensities through Isokawa}}] We have 
$$ \tilde{\mathrm{I}}_{2,1} = \frac{2}{\pi}, \quad   \tilde{\mathrm{I}}_{3,2} = \frac{4}{3} ,   \quad  \tilde{\mathrm{I}}_{3,1} = \frac{8\pi}{15}  \, .$$
\end{prop}
...
\cref{tab:isokawa} gathers these integral face intensities with the corresponding counting face intensities.
...
Recall the construction of $ \mathcal{V}_{d} = \mathrm{Vor}\bigl( ({\Theta}_{i}, R_{i})_{i\geq 1}\bigr)$ given in \cref{t.weighted}. We will focus here on the cell $ \mathcal{C}_{d}$ containing the origin $ \origin \in \mathbb{H}_{d}$. Recall that by our change of variable, $$R_i = \frac{c_{d}}{d-1} \mathrm{exp}\bigl\{(d-1)D_i\bigr\}$$ are the radii of the nuclei on the corona. Clearly, the origin is closer to the nucleus $({\Theta}_{1}, R_{1})$ than to any other ideal nuclei. We will consider the tile $ \mathcal{C}_{d}$ in the upper half-space model and where the ideal nucleus angle ${\Theta}_{1}$ has been sent to $\infty$ in $ \mathbb{U}_{d}$. 

%


\begin{proofof}{\cref{thm.superposition}}
By \cref{prop:bissec}, inside the upper half-space model~$\mathbb{U}_{d}$ and where ${\Theta}_{1}$ is sent to $\infty$, for all $i \geq 2$, the bisector between $({\Theta}_1, R_1)$ and $({\Theta}_i, R_i)$ is a Euclidean hemisphere centered at $ \mathrm{Ste}({\Theta}_{i})$ with radius $\sqrt{1+ |\mathrm{Ste}({\Theta}_{i})|^{2}} (R_1/R_i)^{ \frac{1}{2(d-1)}}$, where $\mathrm{Ste}({\Theta}_{i}) \in \mathbb{R}^{d-1}$ denotes the stereographic projection of ${\Theta}_{i}$.

By Lemma~\ref{lem.stereo}, {conditional on $R_1$,} the point process $\left(\mathrm{Ste}({\Theta}_{i}),R_i-R_1\right)_{ i \geq 2 }$ is a PPP with intensity 
$$
...
In a first step, we express the point process of centers and radii in terms of this latter PPP, as follows:
 \[
 \Bigl(\mathrm{Ste}({\Theta}_{i}),\sqrt{1+ |\mathrm{Ste}({\Theta}_{i})|^{2}}  (R_1/R_i)^{ \frac{1}{2(d-1)}}\Bigr) 
  = \Bigl(\mathrm{Ste}({\Theta}_{i}),\sqrt{1+ |\mathrm{Ste}({\Theta}_{i})|^{2}} \; \Bigl(\frac{R_1}{R_1+\left(R_i-R_1\right)}\Bigr)^{\frac{1}{2(d-1)}}\Bigr) .
\]

In a second step, conditionally on $ R_1=s$, we apply the Poisson mapping theorem to compute the intensity of the point process of the centers and radii
by the change of variable $\rho := \sqrt{1+|x|^{2}} \bigl(\frac{s}{s+t}\bigr)^{\frac{1}{2(d-1)}}$, which gives, for a test function $u$,
\begin{equation*}
\begin{split}
...
\begin{split}
 I_{d}(u) &:= \frac{2(d-1)}{c_{d}} \int_{\mathbb{R}^{d-1}}\mathrm{d}x \int_{\mathbb{R}_{+}} \frac{\mathrm{d}\rho}{\rho^{2d-1}} \mathbf{1}_{(\rho + R_\origin)^{2}\geq |x|^{2}+C_\origin^{2}}\times \mathbf{1}_{\rho^2\leq 1+|x|^{2}}\\
...
The second integral is equal to 1 by \cref{lem.stereo}. For the first integral, change to polar coordinates, then to $\sinh \eta := |x|/|C_\origin|$, and finally to $t := 1/\cosh \eta$. {This gives (i), which immediately implies (ii).}
%
%
%
%
...
where we used the change of variables $y := \ue^{u} x$.
...
It is easily seen, using the change of variables $v := 1 - t \tanh u$,
%
%
...
\begin{figure}[htp] 
\centering
\includegraphics[height=0.95\textheight]{figs_final/2Dpotrait_red_ee0b0a.png}
...
\begin{prop}[\textsc{Integral face intensities $ \tilde{ \mathrm{I}}_{d,d-1}$}]
\label{p.isokawa}
For all $d \ge 2$, we have
\[
\tilde \Intn_{d,d-1} = \frac{2\Gamma (\frac{d}{2})\Gamma(d)}{\Gamma(\frac{d-1}2)\Gamma (d-\frac{1}{2})}.
\]
\end{prop}

{The first few values are $4/\pi$, $8/3$, $64/(5 \pi)$, and $192/35$.
For large $d$, it is asymptotically $  \sqrt{2}d  -\frac{9}{4\sqrt{2}}+O(\frac{1}{d})$.}

\rproof
Fix $d \geq 2$, and for  $\lambda, \varepsilon>0$,  consider the  probability that the origin is within distance less than $\varepsilon$ from the boundary {$\partial \cellsl_d := \bigcup \cellsl_{d, d-1}$} of the Poisson--Voronoi tessellation {$\cells_d^{(\lambda)}$} with intensity $\lambda$ on $ \mathbb{H}_d$:
$$ h(\varepsilon,d,\lambda) \coloneqq  \mathbb{P}\bigl[ B_\varepsilon( \origin) \cap \partial{\cells_d^{(\lambda)}} \ne \varnothing \bigr].$$  By isometry invariance, $ h(\varepsilon,d,\lambda)$ can be interpreted as the mean volume (per unit of volume of $ \mathbb{H}_d$) of the region within distance $\varepsilon$ from  $\partial{\cells_d^{(\lambda)}}$. We then  have the following limit
  \begin{equation} \label{eq:limitisokawa} \frac{h(\varepsilon,d,\lambda)}{\varepsilon} \xrightarrow[ \varepsilon\to 0]{}  \tilde{ \mathrm{I}}_{d,d-1}^{{( \lambda)}} \xrightarrow[ \lambda \to 0]{ \mathrm{Lem.\ \ref{lem:convintensities}}} \tilde{ \mathrm{I}}_{d,d-1}    \end{equation} 
and the one in which we interchange the order of limits, which can be evaluated explicitly thanks to our results,  
 \begin{eqnarray*}
 \lim_{\varepsilon \to 0} \lim_{\lambda \to 0} \frac{h(\varepsilon,d,\lambda)}{\varepsilon} &\underset{  \mathrm{Thm.}\ \ref{thm:decomposition}}{=} & \lim_{\varepsilon\to 0} \frac{ \mathbb{P}\bigl[ B_\varepsilon(\origin) \cap \partial \mathcal{V}_d \ne \varnothing\bigr]}{\varepsilon}\\
  & {=} &  - \left. \frac{ \mathrm{d}}{ \mathrm{d} \varepsilon} \mathbb{P}\bigl[B_\varepsilon(\origin) \subset \mathcal{C}_d\bigr] \right|_{\varepsilon=0}\\ &\underset{ \mathrm{Prop.}\ \ref{prop.holeprob}}{=}&  \frac{2\Gamma (\frac{d}{2})\Gamma(d)}{\Gamma(\frac{d-1}2)\Gamma (d-\frac{1}{2})}.
  \end{eqnarray*}
  We claim that exchanging the order of limits gives the same result, whence the above display computes $ \tilde{\mathrm{I}}_{d,d-1}$.  
...
\begin{equation}\label{eq.intmes}
 ...
 \end{equation}
 {In their natural coupling, the sets} ${\partial}\scell_d{(R_1)}$ and ${\partial}\ocell_d$ differ by at most $2(d-1)R_1$ boundary faces in expectation given $R_1$ (and hence at most $2(d-1)$ faces without conditioning on $R_1$), as we saw in evaluating the last integral of \eqref{eq.idr2}.
{In particular, the asymptotic properties of $\ocell_d$ and of $\scell_d(R_1)$ are the same a.s. Clearly, the law of $\scell_d$ is $\R^{d-1}$-invariant and ergodic. Generally, we are interested only in geometric properties of $\scell_d$ that are $\mob_d$-invariant. For example, given $t > 0$, the homothety $(x, \rho) \mapsto (tx, t\rho)$ of $\uhs_d = \R^{d-1} \times \R_+$ is an isometry that preserves $\infty$. It corresponds to changing $s$ in \eqref{eq.intmes} to $t^{d-1}s$. Thus, $\scell_d(s)$ for all $s > 0$ can be coupled to be isometric to each other. Hence, in studying $\scell_d(s)$, the value of $s$ will mainly be important for the natural coupling of $\ocell_d$ with $\scell_d$.}
...
The {typical cell} $ \scell_{d}$ can be seen as the epigraph (see Figure \ref{fig.cell_A} for the graph) of a stationary random function $ \mathcal{H} \colon \mathbb{R}^{{d-1}} \to \mathbb{R}_{+}$ whose law is computed below. Specifically, for $x_{0} \in \mathbb{R}^{d-1}$, let us denote by $\mathcal{H}(x_{0})$ the height of $ \partial \scell_{d}$ at $x_{0}$ and by $\Theta(x_{0})$ the angle the hypersurface $ \partial \scell_{d}$ makes with the  vertical direction at that point  (in other words, the complement of the angle between the vertical direction and the direction orthogonal to the hyper{surface}). It should be clear that this angle is defined for almost all $x_{0}  \in \mathbb{R}^{{d-1}}$, and since the law of $\bigl(\mathcal{H}(x_{0}),\Theta(x_{0})\bigr)$ is independent of $x_{0}$, we denote it for short by $(\mathcal{H}, \Theta)$.
...
\end{enumerate}
\end{prop}
\begin{proof} \textbf{(i)} \enspace Without loss of generality, we consider the variables $\bigl(\mathcal{H}(\bfz),\Theta(\bfz)\bigr)$.
Condition on $R_{1}=s$. In a preliminary step, we study the first marginal of this pair, namely, the law of $\mathcal{H}(\bfz)$. Recall that the intensity measure of the Poisson point process $\Pi_d$ is given by $(x,\rho) \mapsto 2s \frac{d-1}{c_{d}} \mathrm{d}x \otimes \frac{ \mathrm{d}\rho}{\rho^{2d-1}}$.  The event $\mathcal{H}(\bfz)\leq h$ corresponds to the event where this point process has no point inside the region $\bigl\lbrace (x,\rho) \ST \rho^{2} > |x|^{2} + h^{2}\bigr\rbrace$. Hence, changing variables to $y := x/h$ and $\sigma := \rho/h$ gives
\begin{equation}\label{eq.phq}
\begin{split}
\mathbb{P}[\mathcal{H}(\bfz) \leq h ]
&=
...
\begin{figure}[!h]
 \begin{center}
 \includegraphics[width=.32\linewidth]{foam20/foam_Seed12345_PPP_20x20_stat_median_z1new.png}
  \includegraphics[width=.32\linewidth]{foam20/foam_Seed12345_PPP_20x20_stat_median_z3new.png}
   \includegraphics[width=.32\linewidth]{foam20/foam_Seed12345_PPP_20x20_stat_median_z5new.png}
...
\begin{figure}[!h]
 \begin{center}
 \includegraphics[width=.5\linewidth]{foam20/foam_Seed12345_PPP_20x20_stat_median_top-new.png}
...
\label{f.foam-top}
 ...
 This tessellation $\GKT_{{d-1}}$ is actually a special case of point processes studied in \cite{gusakova2022delaunay}: the relevant process for us is what they call the $\beta'$-model with certain choices for their parameters $\beta$ and $\gamma$. They use the PPP on $\R^{d-1} \times \R_-$ of nuclei and heights with intensity given in \cite[(3.2)]{gusakova2022delaunay}, where their $-h$  corresponds to our $\rho^2$: see the last line of \cite[p.~1258]{gusakova2022delaunay}. The point is that the Laguerre diagram in $\R^{d-1}$ corresponding to their power function is precisely the tessellation {$\GKT_{{d-1}}$} for an appropriate choice of their parameters, $\beta$ and $\gamma$.  We use $\beta := d$. Their other parameter  $\gamma$ is a scaling factor. To be able to make the correspondence, we need to match our intensity in \eqref{eq.intmes} with their intensity.  We will condition that $R_1 = s$. Writing \eqref{eq.intmes} as 
\vadjust{\kern2pt}%
$s\frac{d-1}{c_d} \,\ud x \,\ud(\rho^2)/(\rho^2)^d$, 
we see that for the two models to match, we need to choose $\gamma := s \sqrt\pi\,\Gamma(d/2)/\Gamma\bigl(\frac{d-1}2\bigr)$. %

In \cite[Theorem 2]{gusakova2022delaunay}, they compute moments of volumes of weighted typical cells of the Delaunay tessellation corresponding to their Laguerre diagram. Of most interest to us is the case where $\beta := d$ as we said, their $\nu := 0$, and their $s := 1$. This yields after some simplification the following:
...
{Comparing \cref{t.f-vector} with \cref{c.f-vector} {with $\ell := d-1$ there} shows agreement {when we note that each $k$-face of $\partial \scell_d$ belongs to $d-k$ cells of $\partial \scell_d$, but we cannot explain this coincidence}.}
\end{remark}
...
\rproof
By \cref{lem.EuclFaceInt} and the ergodic theorem (e.g., \cite[Theorem 9.3.1]{SchneiderWeil}), the number of $(d-1)$-faces of $\GKT_{{d-1}}$ in a $(d-1)$-dimensional Euclidean ball of radius $\rho$ about $\bfz$ is a.s.\ asymptotic to $b_\rho\JJ_{d, d-1}/(s \cdot \DelVol_d)$, where $b_\rho$ is the Euclidean volume of that ball. By \cref{prop.hscpl}, the hyperbolic $(d-1)$-volume of the union of those faces is a.s.\ asymptotic to 
\eqaln
...
For $\sigma := (\harp{x_1}, \dots, \harp{x_d}, \rho_1, \dots, \rho_d) \in (\mathbb{R}^{d-1})^{d}\times (\mathbb{R}_{+})^{d}$, write $v(\sigma) := \bigcap_{i=1}^{d} \partial B(\harp{x_{i}},\rho_{i})$. 
...
Let $\Sigma := \{ \sigma \in A \ST z(\sigma) = 1\}$.
...
\rproof
Let $V$ be a tessellation.  Define $f$ to be 0 if some cell of $V$ does not have a unique end. If each cell has a unique end, then let $\theta_1$ be the end of the cell of $\origin$. Set $g(\theta_1) := 0$. There is a unique extension of $g$ to the set $E$ of ends of all cells such that when two cells with ends $\theta$ and $\theta'$ share a boundary point $x$, we have $g(\theta) + \ud_\theta(x) = g(\theta') + \ud_{\theta'}(x)$. When $V$ is the tessellation corresponding to ideal nuclei $({\Theta}_i, \DD_i)$, we have that $g({\Theta}_i) = \DD_i - \DD_1$ a.s. Define $G(t) := |\{\theta \in E \ST g(\theta) \le t\}|$ for $t \ge 0$. Finally, let
\[
f(V)
:=
...
What are the values for the integral face intensities, $\tilde\Intn_{d, k}$ for $1 \le k \le d-2$? Related to this: what are the hole probabilities for $k$-faces with $0 \le k \le d-2$?
\end{question}
...
%pub%I think it is not hard to prove that the tail probability is asymptotic to $u \mapsto a_d \ue^{-2(d-1)u}$ as $u \to\infty$, where $a_d := 2^{2-d} \sqrt{\pi}\, \Gamma(2d-2)/\bigl(\Gamma(d/2)\Gamma\bigl(3(d-1)/2\bigr)\bigr)$, and $a_d \sim \bigl(\frac8{3\sqrt3}\bigr)^{d-1}$ as $d \to\infty$.
\end{conj}
