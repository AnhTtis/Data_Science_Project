...
\title{\LARGE \bf  Zero-Sum Games between Mean-Field Teams: Reachability-based Analysis
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
    This work studies the behaviors of two competing teams in a discrete environment, where the team-level interactions are captured as a zero-sum game while the dynamics within each team is formulated as a mean-field team problem.
    Following the ideas from the mean-field game literature, we first approximate the large-population team game with its infinite-population limit.
    We then introduce two fictitious coordinators and transform the infinite-population game into an equivalent zero-sum game between the two coordinators.
...
Under the assumption that all agents apply the same strategy, a dynamic programming decomposition is developed leveraging the common-information approach~\cite{nayyar2013decentralized} so that all agents within the team deploy the same strategy prescribed by a fictitious coordinator. 
The identical strategy assumption significantly simplifies the problem, but the optimality of such approach is only guaranteed for the linear-quadratic models~\cite{arabneydi2015team}.
In the competitive team setting, although one may restrict strategies used by agents within its team to be identical, making the same assumption for the opponent team may lead to a significant underestimation of the opponent's capabilities and thus requires further justification. 

Motivated by the results in the MFG literature, we investigate team-optimal solutions in zero-sum mean-field team games (ZS-MFTG). 
...
Furthermore, we assume a \textit{mean-field sharing} information structure~\citep{arabneydi:2014}; 
that is, each agent observes its local state and the mean-fields of both its own team and the opponent team.%
\footnote{
...
In this work, we show that the solution obtained under the assumption that both teams deploy identical team strategies is still $\mathcal{O}(1 / \sqrt{\min\{N_1, N_2\}})$-suboptimal, where $N_1$ and $N_2$ are the number of agents within each team.
The key intuition behind this result leverages the fact that one can always construct an identical team strategy that induces a controlled dynamics similar to that of a non-identical team strategy.
Through a reachability-based analysis, we conclude that the class of teams using identical team strategies is rich enough to approximate any team using non-identical strategies.
Thus, we can restrict our attention to identical team strategies and still obtain satisfactory team performance.
%

Based on the above result, we extend the common-information approach~\citep{arabneydi:2014} to the zero-sum team game setting and treat the mean-fields of the two teams as the common information.
We first consider the limiting case where both teams have infinite population, and we introduce two fictitious coordinators, one for each team. 
The fictitious coordinator of a team observes the mean-fields of both teams and prescribes a local policy (decision rule at a time step) for all agents within its team; 
each agent then \emph{independently} selects its action according to the prescribed policy and its observed individual state (see~\figref{fig:schematic}).
The infinite-population team game can then be cast into an equivalent zero-sum game between the two coordinators, where the joint state is the  joint mean-fields and the actions are the local policies.
...
Finally, the theoretical claims are verified by numerical examples.

In summary, the contributions of this work are three-fold: 
(i) a novel formulation of zero-sum mean-field team games in discrete environments;
(ii) simplification of large-population games to equivalent coordinator games with identical team strategies;
and
(iii) derivation of the sub-optimality bounds for the proposed approach.

\begin{figure}[b]
    \centering
    \includegraphics[width = 0.6\linewidth]{Figures/MFTG_Schematic.png}
...
% \begin{enumerate}
%     \item 
%     In Section~\ref{sec:formulation}, we present the formulation of the zero-sum mean-field team games. 
%     \item 
%     In Section~\ref{sec:mean-field-approx},
%     we make the identical team strategy assumption on both teams and consider the infinite-population limit employing the mean-field approximation.  
%     We demonstrate that the finite-population empirical distribution induced by a non-identical team policy can be closely matched by a infinite-population mean-field induced by some identical team policy \emph{in expectation}, which justifies restricting our attention to identical team strategy at the infinite-population limit. 
%     \item In Section~\ref{sec:coordinator-game}, we use the common information approach~\cite{nayyar2013decentralized} and reformulate the problem as a zero-sum game between two coordinators. This allows us to efficiently find the max-min and min-max optimal strategies through dynamic programming.
%     \item In Section~\ref{sec:optimal-value}, we show that both the optimal lower value function and the optimal upper coordinator game value are Lipschitz continuous.
%     \item In Section~\ref{sec:performance}, we establish that the optimal strategies for the coordinator game (employing the common information approach, mean-field approximation, and identical team  strategies) remain $\epsilon$-optimal for the original finite-population game. 
%     Specifically, if one team applies the proposed strategy, the opponent team can only increase its performance by $\epsilon$, even if the opponent team uses a non-identical team strategy to exploit.
% \end{enumerate}

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.45\linewidth]{Figures/road_map.png}
...
Additional important notation used in this paper is presented in Table~\ref{table:notation}.
...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{l|l|l}
      ...
      $\phi^{N_1}_t$ / $\psi^{N_2}_t$ & Arbitrary Blue/Red team policy &  Finite\\
      $\phi$ / $\psi$ & Identical Blue/Red team strategy & Finite \& Infinite\\  
      $\phi^{N_1}$ / $\psi^{N_2}$ & Arbitrary Blue/Red team strategy &  Finite\\
      ...
      $\R^\rho_{\mu}$ / $\R^\rho_{\nu}$ & Reachablity correspondence for Blue/Red mean-field  \qquad  & Infinite \\
...
to denote the joint state of the Blue team excluding Blue agent $i$.
...
\begin{definition}  [Empirical Distribution]
    ...
    The \textit{empirical distribution} for the Blue and Red teams are defined as
    ...
    \end{subequations}
\end{definition}
Note that $\M^{N_1}_t \in \P(\X)$ and $\N^{N_2}_t \in \P(\Y)$. 
...
% The empirical distribution flows of the two teams are denoted as the time sequences $\M^{N_1}=\{\M^{N_1}_t\}_{t=0}^T$ and $\N^{N_2}=\{\N^{N_2}_t\}_{t=0}^T$.

We introduce two operators $\mathrm{Emp}_\mu: \X^{N_1} \to \P(\X)$ and $\mathrm{Emp}_\nu : \Y^{N_2} \to \P(\X)$ to relate the joint states and their corresponding empirical distributions:
\begin{align}
...
We consider a weakly-coupled dynamics, where the dynamics of each individual agent is coupled with other agents through the empirical distributions. 
...
\begin{assumption}
    ...
    Formally, for all $\mu, \mu' \in \P(\X)$, $\nu, \nu' \in \P(\Y)$, and for all $t \in [T]$, there exists a positive constant $L_r$ such that 
    \begin{equation}
...
Hence, it is necessary to include $\rho = N_1/N$ as a parameter.
It can be further verified that the above weakly-coupled transition kernels $f_t$ and $g_t$ are both $2$-Lipschitz continuous with respect to the empirical distributions. Additionally, the weakly-coupled reward function has a Lipschitz constant of $L_{r} = 2 \max \{r_{1,\max}, r_{2,\max}\}$, 
...
At each time step $t$, Blue agent $i$ observes its own state $X_{i,t}^{N_1}$ and the empirical distributions $\M_t^{N_1}$ and $\N_t^{N_2}$.
Similarly, Red agent $j$ observes $Y_{j,t}^{N_2}$, $\M_t^{N_1}$ and $\N_t^{N_2}$.
%
Viewing the empirical distributions as aggregated states of the teams, 
...
%
An individual strategy for the Blue agent $i$ is defined as a time sequence $\phi_{i} = \{\phi_{i, t}\}_{t=0}^T$.
Since Blue agents have the same state and action spaces, they share the same policy space. Hence, we denote $\Phi_t$ and $\Phi$ to be the set of individual policies and strategies available to each Blue agent, respectively.
The Blue team strategy $\phi^{N_1} = \{\phi_i\}_{i=1}^{N_1}$ is the collection of individual strategies used by each Blue agent. 
We use $\Phi^{N_1} = \times_{i=1}^{N_1} \Phi$ to denote the set of Blue team strategies.
All the notations extend naturally to the Red team.
...
The objective of the Blue team is to maximize the cumulative rewards, while the Red team strives to minimize it.

\begin{remark}
    The work of~\cite{arabneydi2015team} assumes that all agents within the same team apply the same strategy, and hence the empirical distribution can be regarded as the information state (sufficient statistics) that characterizes the value function of the finite-population game. 
    In our formulation, however, agents in both teams are allowed to deploy strategies that are different from their teammates. 
    Since the empirical distribution does not differentiate agents within the same team, it can no longer serve as the information state.
    As a result, the value function in~\eqref{eqn:expected-cum-reward} takes the joint states as arguments.
    A counter-example that shows the empirical distribution is not an information state is presented in Appendix~\ref{appdx-sec:infomration-example}.
\end{remark}

We let the Blue team be the \textit{maximizing} team and the Red team be the \textit{minimizing} team.
...
We will present a special case of the mean-field team games where the game value exists in Section~\ref{sec:existence-game-value}.
In the general case, we consider the following optimality condition for the Blue team strategy.

\begin{definition}
    \label{def:team-optimal}
    A Blue team strategy $\phi^{N_1 *} \in \Phi^{N_1}$ is optimal if
    \begin{equation}
        \underline{J}^{N*}  = \min_{\mathbf{\psi}^{N_2} \in \Psi^{N_2}} J^{N, \phi^{N_1 *}, \psi^{N_2}}.
        % \geq \min_{\mathbf{\psi}^{N_2} \in \Psi^{N_2}} J^{N, \phi, \psi^{N_2}}
        % \qquad \forall \; \phi \in \Phi.
    \end{equation} 
\end{definition}

In other words, the optimal Blue team strategy $\phi^{N_1 *}$ guarantees the highest worst-case performance against any Red team strategy.
%
A relaxed version of the above optimality condition is the following 
\begin{definition}
...
To address this scalability challenge, we first consider the limiting case of the large-population team games, when the number of agents of both teams goes to infinity.
...
%
As we will show later, due to the law of large numbers, the empirical distribution of an infinite-population team becomes deterministic and converges to the state distribution of its corresponding typical agent. 
This limiting distribution is known as the \emph{mean-field}~\citep{huang2006large}.
Using the deterministic mean-field to approximate the stochastic dynamics of a finite-population system is known as the mean-field approximation.
...
\paragraph{Mean-field dynamics.}

At the infinite population limit, we specifically focus on the Blue and Red teams that employ identical team strategies. These strategies are defined as follows.
\begin{definition} [Identical Blue Team Strategy]
    \label{def:identical-policy}
    The Blue team strategy $\phi^{N_1} = \{\phi_{1},\ldots, \phi_{N_1}\}$ is an identical team strategy, if $\phi_{i_1,t} = \phi_{i_2,t}$ for all $i_1,i_2 \in [N_1]$ and all $t \in [T]$. 
\end{definition}

When all Blue agents apply the same individual strategy $\phi$, we slightly abuse the notation and use $\phi$ to again denote the corresponding identical Blue \textit{team} strategy.
Furthermore, we use $\Phi$ to denote both the set of Blue individual strategies and the set of identical Blue team strategies.
\footnote{When $\Phi$ denotes the set of identical team strategies, it is a subset of the original set of Blue team strategies $\Phi^{N_1}$, i.e., $\Phi \subseteq \Phi^{N_1}$.}
Whether the symbols refer to the individual agent strategy or the team strategy should be clear from the context.
% With a slight abuse of notation, we denote the identical Blue team strategy as $\phi \in \Phi^{N_1}$, when all Blue agents apply the same individual strategy $\phi \in \Phi$.
% We further overload the notation and use  $\Phi$ to denote the set of identical Blue team strategies.
% Note that when $\Phi$ denotes the set of identical team strategies, it satisfies $\Phi \subseteq \Phi^{N_1}$, which will be clear from the context.
% In later sections, it will be clear from the context whether $\Phi$ refers to the set of individual strategies or the set of identical team strategies.
The definition and notations extend to the identical Red team strategies.

With the notions of identical team strategies, we define the mean-field flow as a deterministic sequence of state distributions that coincides with the state distribution of a typical agent. 
\begin{definition}
    Given identical team strategies $\phi \in \Phi$ and $\psi \in \Psi$, the mean-fields propagate according to the following coupled deterministic dynamics:
    \begin{subequations}
...
The following lemma justifies the use of the deterministic mean-fields to approximate the empirical distributions of a finite-population system.

\begin{restatable}{lemma}{mfp}
    \label{lmm:mf-aprx}
    Let $\bfX^{N_1}_t$, $\bfY^{N_2}_t$, $\M^{N_1}_t$ and $\N^{N_2}_t$ be the joint states and the corresponding empirical distributions of a finite-population game.
    Denote the next empirical distributions induced by an identical policy pair $(\phi_t, \psi_t) \in \Phi_t \times \Psi_t$ as $(\M_{t+1}^{N_1}, \N_{t+1}^{N_2})$.
...
designing the policies $\phi_t$ and $\psi_t$ at time $t$ is equivalent to selecting the desirable next mean-fields. 
Consequently, we want to examine the set of mean-fields that can be achieved at the next time step, starting from the current mean-fields.
We use $\pi_t : \U \times \X \to [0,1]$ to denote the identical local policies of the Blue agents, which are \emph{open-loop} with respect to the mean-fields. 
...
Similarly, $\sigma_t : \V \times \Y \to [0,1]$ and $\Sigma_t$ denote the open-loop Red local policy and the set of open-loop Red local policies.

Given open-loop local policies $\pi_t$ and $\sigma_t$, the mean-fields of the two teams propagate in a deterministic manner just as in~\eqref{eqn:mf-dynamics}, but with the identical team policies replaced by the identical open-loop local policies. 
Formally, the propagation rule under these open-loop local policies is governed by
\begin{subequations}
\label{eqn:mf-dynamics-local-policy}
\begin{align}
    \mu^\rho_{t+1} (x') &= \sum_{x \in \X} \left[ \sum_{u \in \U}f^\rho_t(x'|x, u, \mu^\rho_t, \nu^\rho_t ) \pi_t(u|x)\right]\mu^\rho_t(x), \label{eqn:mf-dynamics-local-policy-mu}\\
    \nu^\rho_{t+1} (y') &= \sum_{y \in \Y} \left[ \sum_{v \in \V}g^\rho_t(y'|y, v, \mu^\rho_t, \nu^\rho_t ) \sigma_t(v|y)\right] \nu^\rho_t(y), \label{eqn:mf-dynamics-local-policy-nu}
\end{align}
\end{subequations}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where the entries of the matrices are given by $\left[F_t^{\rho}(\mu_t, \nu_t,\pi_t)\right]_{pq} = \sum_{u \in \U}f^\rho_t(q|p, u, \mu_t, \nu_t ) \pi_t(u|p)$, and similarly for the $G_t^\rho$ matrix.
It is important to note that both of these matrices represent transition matrices. Specifically, they correspond to the transition probabilities of a typical agent under the open-loop local policies.

\vspace{+5pt}
To facilitate the analysis later on, we provide the following definition of the reachable sets for the mean-fields of the Blue and Red team populations.
...
\begin{proof}
    ...
    It can be easily verified that the next mean-field propagated using $\phi_t$ and the dynamics in~\eqref{eqn:mf-dynamics} is the same as the mean-field propagated using the constructed $\pi_t$ and~\eqref{eqn:mf-dynamics-local-policy}.
...
Under the mean-field settings, we restrict both the Blue and Red teams to identical team policies.
In this subsection, we show that the reachable set constructed from the mean-field dynamics under the identical policy assumption for both teams is actually rich enough to approximate the empirical distributions induced by non-identical team policies. 
% In greater detail, for each admissible finite-population Blue team policy (including non-identical ones), its resulting empirical distribution can be properly approximated by a Blue mean-field in the reachable set propagated from the current empirical distributions. 
...
\begin{restatable}{lemma}{mfa}
    ...
    \end{equation}
    Then, the expected distance between the next Blue empirical distribution $\M^{N_1}_{t+1}$ induced by the team policy $\phi^{N_1}_t$ and the mean-field $\M_{\apprx, t+1}$ satisfies
    \begin{equation}
...
The mean-field induced by $\pi_{\apprx,t}$ is within the reachable set and is close to the next empirical distribution induced by the non-identical team policy $\phi^{N_1}_t$ in expectation.
...
Lemma~\ref{lmm:mf-apprx-team-policy} directly leads to the following theorem regarding the richness of the reachable sets.

\begin{theorem}
    \label{thm:R-set-rich}
    Let $\bfX^{N_1}_t$, $\bfY^{N_2}_t$, $\M^{N_1}_t$, and $\N^{N_2}_t$ be the joint states and the corresponding empirical distributions at time~$t$.
    Denote the next Blue empirical distribution induced by some Blue team policy $\phi^{N_1}_t \in \Phi^{N_1}_t$ as $\M^{N_1}_{t+1}$. 
...
\begin{remark}
    ...
    each Blue agent's state, which may seem to violate the mean-field sharing information structure in~\eqref{eqn:policy}. 
...
\begin{remark}
    All results in this subsection extend to the Red team's side.
\end{remark}

\section{Equivalent Zero-Sum Game Between Coordinators}
\label{sec:coordinator-game}
In this section, we leverage the common information approach~\citep{nayyar2013decentralized} to transform the infinite-population game in the previous section to an equivalent zero-sum game between two fictitious coordinators.

We construct a fictitious centralized coordinated system by introducing the Blue and Red coordinators for the two teams respectively. 
...
\end{equation}
where  $\alpha_t: \P(\X) \times \P(\Y) \to \Pi_t$ is a deterministic Blue \textit{coordination policy}, and $\pi_t(u_t|x_t) \triangleq \alpha_t(\mu^\rho_t,\nu^\rho_t)(u_t|x_t)$ gives the probability that a Blue agent selects action $u_t$ given its current state $x_t$.

Similarly, the Red coordinator observes the mean-fields of both teams and selects a local policy $\sigma_t: \V \times \Y \to [0,1]$ for the Red agents according to 
\begin{equation}
    \sigma_t = \beta_t \big(\mu^\rho_t, \nu^\rho_t\big).
\end{equation}

We refer to the time sequence $\alpha =\big(\alpha_1, \ldots, \alpha_T\big)$ as the \textit{coordination strategy}
for the Blue team and similarly $\beta=\big(\beta_1, \ldots, \beta_T\big)$ as the Red coordination strategy.
...
\begin{remark}
    ...
    There is a one-to-one correspondence between the Blue coordination strategies and the identical Blue team strategies.
    Given an identical team strategy $\phi \in \Phi$, one can define the coordination strategy 
    \begin{equation*}
...
Plugging in the \textit{deterministic} coordination strategies, the mean-fields dynamics in~\eqref{eqn:mf-dynamics-local-policy} becomes
\begin{subequations}
...
\begin{equation}
    ...
    \texttt{ZS-CG} = \langle \P(\X), \P(Y), \Pi_t, \Sigma_t, F^\rho_t, G^\rho_t, r^\rho_t, T\rangle.
\end{equation}
...
Before analyzing the value functions, we first present an equivalent form of the optimization problem in~\eqref{eqn:j-ab-t-blue} and~\eqref{eqn:j-ab-t-red}
by leveraging the reachable set introduced in Definition~\ref{def:reachable-set}.
...
Exploiting the deterministic mean-field dynamics, we can change the optimization domains in~\eqref{eqn:J-ab-T} and~\eqref{eqn:j-ab-t-blue} from the policy spaces to the corresponding reachable sets. 
...
In later analysis, we mainly work with the above reachability-based optimization.
There are two major advantages for such approach:
(i) the reachable sets have lower dimension than the action spaces\footnote{The Blue reachable set is a subset of $\P(\X)$, while the Blue coordinator action space is given by $(\P(\U))^{|\X|}$.},
which is desirable for numerical algorithms,
and (ii) the reachability-based optimization allows us to apply Theorem~\ref{thm:R-set-rich} and study the performance loss due to the identical-strategy assumption introduced by the mean-field approximation. 
...
In this section, we study the continuity of the optimal value function in~\eqref{eqn:lower-j-ab-t-rset} with respect to the mean-field arguments.
...
In this section, we show that the coordinator game value exists for a special class of mean-field team games, where the dynamics of each individual agent is independent of other agents' states (both its teammates and its opponents).
...
\begin{restatable}{definition}{IDD}
    \label{def:independent-dynamics}
        We say that the weakly-coupled dynamics are independent if the following holds for all $\mu_t \in \P(\X)$ and $\nu_t \in \P(\Y)$
        \begin{equation}
...
In other words, the dynamics of an agent only depends on that agent's current state and action and is fully decoupled from all other agents.
...
\begin{restatable}{theorem}{EGV}
    ...
    Suppose the reward function $r^\rho_t$ is concave-convex for all $t \in [T]$ and the system dynamics is independent.
    Then, 
...
In this section, we analyze the performance guarantees for $\alpha^*$ in the \emph{finite-population} game and compare the worst-case performance of this coordinator-induced Blue team strategy to the original max-min optimization in~\eqref{eqn:origin-optimization} where agents have the flexibility to apply non-identical strategies within each team.


To this end, let the Red team apply strategies $\psi^{N_2} = (\psi^{N_2}_1, \ldots \psi^{N_2}_{N_2}) \in \Psi^{N_2}$. 
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
where $\mu_t^{N_1} = \empMu{\bfx^{N_1}_t}$ and $\nu_t^{N_2} = \empNu{\bfy^{N_2}_t}$ are the empirical distributions corresponding to the joint states $\bfx^{N_1}_t$ and $\bfy^{N_2}_t$, respectively.
...
\begin{theorem}
    ...
    The optimal Blue coordination strategy $\alpha^*$ obtained from~\eqref{eqn:optimal-blue-policy} is an $\epsilon$-optimal Blue team strategy. 
%
    Formally, 
    \begin{equation}
        \label{eqn:performance-bounds}
        \underline{J}^{N*}(\bfx^{N_1}, \bfy^{N_2}) \geq \min_{\psi^{N_2} \in \Psi^{N_2}} J^{N,\alpha^*,\psi^{N_2}} (\bfx^{N_1}, \bfy^{N_2}) \geq \underline{J}^{N*}(\bfx^{N_1}, \bfy^{N_2}) - \residue \quad \forall \bfx^{N_1} \in \X^{N_1}, \bfy^{N_2} \in \Y^{N_2},
    \end{equation}
...
\begin{lemma}
    ...
    \end{equation}
    where $\mu^{N_1}=\empMu{\bfx^{N_1}}$ and $\nu^{N_2} = \empNu{\bfy^{N_2}}$ are the corresponding empirical distributions.
\end{lemma}
...
\begin{proof}
 ...
 %   \pn{why?}\sg{an explanation added.}
    ...
    For simplicity, we do not emphasize the correspondence between the joint states and the empirical distributions for the rest of the proof, as it is clear from the context.
    ...
    \textit{Induction: }
    At timestep $t$, consider an arbitrary pair of joint states $\bfx^{N_1}_{t} \in \X^{N_1}$ and $\bfy^{N_2}_{t} \in \Y^{N_2}$, and their corresponding empirical distributions $\mu^{N_1}_t$ and $\nu^{N_2}_t$. 
    Define $\mu_{t+1}^{*}$ as
    \begin{equation*}
...
\begin{remark}
    The continuity of the reward and dynamics in Assumptions~\ref{assmpt:lipschitiz-dynamics} and~\ref{assmpt:lipschitiz-rewards} are necessary for translating the performance of the infinite-population coordinator game back to the finite population. 
    A counter-example with discontinuous rewards, where the coordinator game value is significantly better than any finite-population game, is provided in Appendix~\ref{appdx-sec:discontinuity}
\end{remark}
...
The work~\citep{arabneydi:2014} directly assumes that all agents within the team apply the same strategy and the optimality for the finite-population game is \emph{only} guaranteed in the LQG setting~\citep{mahajan2015linear}.
...
(iii) The work in~\citep{sanjari2022nash} only provides asymptotic performance guarantees when the population size approaches infinity. 
The proposed reachability-based approach and the additional Lipschitz assumptions allow us to provide the  convergence rate of the finite-population team performance to its infinite-population limit.
...
&f^\rho(x^1|x^2, u^3, \mu, \nu) = 0.5 \big(1 + \big(\rho \mu(x^1) - (1-\rho) \nu(y^1)\big)\big), ~~
&&f^\rho(x^2|x^2, u^3, \mu, \nu) = 0.5 \big(1 - \big(\rho \mu(x^1) - (1-\rho) \nu(y^1)\big)\big), \\
&f^\rho(x^1|x^2, u^4, \mu, \nu) = 0.5 \big(1 + 0.3\big(\rho \mu(x^1) - (1-\rho) \nu(y^1)\big)\big), ~~
&&f^\rho(x^2|x^2, u^4, \mu, \nu) = 0.5 \big(1 -0.3\big(\rho \mu(x^1) - (1-\rho) \nu(y^1)\big)\big).
\end{alignat*}
\end{small}
...
%
Since the value function $J^{\rho*}_{\cor,1}$ at $t=1$ is not convex-concave, the best-response correspondence in the coordinator game is not convex-valued. Consequently, a Nash equilibrium does not exist in this example and the upper and lower game values differ at $t=0$ as shown in~\figref{fig:simple-example-values}(a). 
Specifically, at $\mu^\rho_0=[0.96, 0.04]$ and $\nu^\rho_0=[0.04, 0.96]$, we have the lower value $\underline{J}^{\rho*}_{\cor,0} = 0.5298$ and the upper value $\bar{J}^{\rho*}_{\cor,0} = 0.5384$, which are visualized as the green and yellow points respectively.
The reachable sets from  $\mu^\rho_0$ and $\nu^\rho_0$ are plotted in~\figref{fig:simple-example-values} and also visualized as the box in~\figref{fig:simple-example-values}(b), which serves as the optimization domains to compute $\bar{J}^{\rho*}_{\cor,0}(\mu^\rho_0, \nu^\rho_0)$ and $\underline{J}^{\rho*}_{\cor,0}(\mu^\rho_0, \nu^\rho_0)$.
\figref{fig:simple-example-values}(c) presents a zoom-in for the local optimization and the marginalized functions with respect to $\mu$ and $\nu$.
The max-min value (green point) and min-max value (yellow point) follow directly from the marginalized functions.

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\linewidth]{Figures/simple-example-value.pdf}
    \vspace{-0.25in}
...
The following figure presents the max-min and min-max trajectory of the coordinator game. 
\begin{figure}[!htb]
    \centering
...
Consider a ZS-MFTG instance with a terminal time at $t=2$ and population ratio $\rho=0.375$.
The Blue transition kernels are time-invariant and are defined as 
\begin{equation}
    ...
    \end{alignedat}
\end{equation}
In words, the transitions of a Blue agent are deterministic and independent from the mean-fields. 
Action $u^1$ instructs a Blue agent to remain at its current state, while action $u^2$ directs it to transition to the other state.
One can verify that the above Blue transition kernel ensures that the Blue reachable set covers the whole simplex regardless of the initial team distributions, i.e., $\mathcal{R}_{\mu, t}(\mu, \nu) = \P(\X)$ for all $\mu \in \P(\X)$, $\nu \in \P(\Y)$ and $t\in \{1,2\}$.
...
\begin{align*}
    ...
    &r^\rho_2(\mu, \nu) = - \nu(y^1)
\end{align*}
which incentivizes the (minimizing) Red team to maximize its distribution on $y^1$ at the terminal time $t=2$, and hence, the Blue team has the incentive to achieve the target distribution and block Red agents' transition from $y^2$ to $y^1$.
...
Regardless of the Blue team's distribution, all Red agents at $y^2$ should use action~$v^2$ at $t=1$ and try to transition to state $y^1$ for the sake of maximizing the team's presence at $y^1$.
This is the (dominant) optimal strategy against all Blue team strategies in both finite and infinite-population scenarios. 
Specifically, 
\begin{equation}
...
The following figure presents the coordinator game value, which is solved via discretization. 
In this example, the upper and lower game coincides, and thus the game value exists.
The black line in the middle figure marks the game value with $\mu^\rho_1(x^1) = 1/\sqrt{2}$, when the Blue team perfectly achieves the target distribution. 
The values on the black line dominate and give the highest performance the Blue team can achieve given any $\nu_1^\rho(y^1)$, which aligns with the expectation that the Blue team should match the target distribution. 

\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.95\linewidth]{Figures/SimpleExample2_values.png}
...
and thus the game value is simply given by $J^{\rho*}_0(\mu_0, \nu_0) = - \nu^\rho_0(y^1)$, which matches the game value plotted in~\figref{fig:example-2-value}.

However, in the finite-population case, the Blue team cannot match the given \emph{irrational} target distribution. 
Furthermore, if the finite-population Blue team applies the identical team policy induced by the coordination strategy in~\eqref{eqn:example2-coord-strate} at $t=0$, the Blue empirical distribution at $t=1$ is a random variable due to the stochasticity in the mixed policy.
To deterministically achieve a distribution that is closest to the target distribution, the Blue team has the incentive to deviate from the identical mixed team policy and apply a \emph{non-identical} but deterministic policy.

For example, consider a Blue team with three Blue agents ($N_1 = 3$) 
all starting on $x^1$, i.e., $\mu^{3}_0 = [1,0]$.
Consequently, the Red team consists of five agents ($N_2 = 5$) so that $\rho = 3/8 = 0.375$.
%
The Blue coordination strategy prescribes that all these three Blue agents pick $u^1$ with probability $1/\sqrt{2}$ and $u^2$ with probability $(1-1/\sqrt{2})$ in order to match the target distribution \emph{in expectation}.
%
Such action selection leads to the following four possible outcomes of the Blue empirical distributions at $t=1$:
$\mathbb{P}(\mu_1^{3} = [1, 0]) = 0.354$,
$\mathbb{P}(\mu_1^{3} = [2/3, 1/3]) = 0.439$,
$\mathbb{P}(\mu_1^{3} = [1/3, 2/3]) = 0.182$,
and $\mathbb{P}(\mu_1^{3} = [0, 1]) = 0.025$.
% 
In expectation, these empirical distributions lead to a transition probability of 0.518 for a Red agent to move from $y^2$ to $y^1$ at $t=1$ (see~\eqref{eqn:example2-red-dynamics-t1-3}).
%
Next, consider the non-identical but deterministic Blue team strategy, such that the Blue agents 1 and 2 apply action $u^1$  and Blue agent 3 applies $u^2$.
Specifically, consider a Blue team policy $\phi^{3}_0 \in \Phi^{3}_0$ such that
\begin{align*}
    &\phi^{3}_{1, 0} (u^1|x^1, [1,0], \nu) = 1.0, \quad \forall \nu \in \P(\Y), \\
    &\phi^{3}_{2, 0} (u^1|x^1, [1,0], \nu) = 1.0, \quad \forall \nu \in \P(\Y), \\
    &\phi^{3}_{3, 0} (u^2|x^1, [1,0], \nu) = 1.0, \quad \forall \nu \in \P(\Y), \\
\end{align*}
The above non-identical Blue team policy deterministically leads to the empirical distribution $\mu_1^3=[2/3, 1/3]$ at $t=1$, and the resultant Red transition probability from $y^2$ to $y^1$ is 0.016.
Clearly, the non-identical Blue team policy significantly outperforms the identical mixed team policy in this \emph{three-agent} case. 

\figref{fig:deviation} presents the difference in values in the finite-population scenario with initial distributions $\mu_0 = [1,0]$ and $\nu_0 = [0.4, 0.6]$.
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.55\linewidth]{Figures/SimpleExample2_deviation_supp.pdf}
    \caption{Difference between game values with initial distributions $\mu_0 = [1,0]$ and $\nu_0 = [0.6, 0.4]$.
    }
    \label{fig:deviation}
\end{figure}
%
The red line in~\figref{fig:deviation} plots the difference between the finite-population optimal value and the worst-case performance induced by the optimal Blue coordination strategy (identical across all Blue agents).
It verifies the theoretical claim in Theorem~\ref{thm:performance-guarantees}.
Specifically, the optimal value for the finite-population game is computed based on the following:
\begin{itemize}
    \item At $t=0$, the Blue team uses a non-identical deterministic team strategy and deterministically achieves the optimal empirical distribution $\mu^{N_1}_1$, which minimize the two-norm distance to the target distribution. 
    Such optimal Red empirical distribution is constructed as $[\frac{n}{N_1}, \frac{N_1 - n}{N1}]$ where 
        $n = \argmin_{0 \leq n\leq N_1 } (\frac{n}{N_1} - \frac{1}{\sqrt{2}})^2$.
    \item 
    At $t=1$, all Red agents apply $v^2$, 
    and the distribution of the next Red empirical distributions $\nu^{N_2}_2$ can be computed based on the optimal $\mu^{N_1}_1$ achieved at $t=1$.
    \item 
    The expected game value is then computed based on the distribution of the Red empirical distribution $\nu_2^{N_2}$.
\end{itemize}



The black line shows the difference between the finite-population optimal value and the value of the coordinator game, which is the subject studied in Lemma~\ref{lmm:cor-apprx-err}.
The optimal coordinator game value is 0.4 with the given initial distribution.
The trend is not monotonically decreasing. For example, the three points marked blue corresponds to 39, 48 and 54 Blue agents, and the best Blue empirical distribution these agents can achieve to minimize the difference from the target distribution are $\mu_1^{39}=[0.7179, 0.2821]$, $\mu_1^{48}=[0.7083, 0.2917]$ and $\mu_1^{54}=[0.7037, 0.2963]$.
The corresponding two-norm from the target distributions are 0.0153, 0.0017, and 0.0048, respectively. 
As a result, the difference between the finite-population value and the coordinator game value also oscillates. 
However, the trend is still bounded by the $\mathcal{O}(1/\sqrt{\underline{N}})$ rate ($\underline{N} = N_1$ in this example).
...
We first approximated the large-population game with its infinite-population limit. 
Leveraging the common information approach, we transformed the infinite-population game into an equivalent zero-sum game between two fictitious coordinators. 
We should that even though the optimal strategies are solved assuming that both teams follow the same fictitious-coordinator method and apply identical team strategies, the strategies constructed are still $\epsilon$-optimal for the original finite-population game with the general class of non-identical team strategies. 
The derived performance guarantees are verified through numerical examples.
Future work will investigate the LQG setup of this problem and deploy machine learning techniques to solve more complex zero-sum mean-field team problems. 
It would also be interesting to generalize the results to infinite horizon discounted problems, the general-sum case, and heterogeneous subteams. 
...
&= \frac{N_1}{N}\frac{ \sum_{k=1}^{N_1}f_{1,t}(x^{N_1}_{i,t+1}\big \vert x^{N_1}_{i,t}, u^{N_1}_{i,t}, x^{N_1}_{k,t} )}{N_1} +
    ...
    &\triangleq f^\rho_t\big(x^{N_1}_{i,t+1}\big \vert x^{N_1}_{i,t}, u^{N_1}_{i,t},  \mu^{N_1}_t, \nu^{N_2}_t).   \nonumber
\end{align*}
%where we use the equality $\indicator{x}{x^{N_1}_{k,t}}f_{1,t}(x^{N_1}_{i,t+1}\big \vert x^{N_1}_{i,t}, u^{N_1}_{i,t}, x^{N_1}_{k,t} ) = \indicator{x}{x^{N_1}_{k,t}}f_{1,t}(x^{N_1}_{i,t+1}\big \vert x^{N_1}_{i,t}, u^{N_1}_{i,t}, x)$ and the definition of the empirical distribution.
...
\begin{proposition}
    ...
    For all $\mu, \mu' \in \P(\X)$, $\nu, \nu' \in \P(\Y)$ and $t \in [T]$, the reward structure in~\eqref{eqn:team-rewards-eg} satisfies 
    \begin{equation*}
...
One can verify that the above coordination strategy will achieve the mean-field $\mu_1 = (\sqrt{3}, 1-1 / \sqrt{3})$ from all $\mu_0 \in \P(\X)$ under the mean-field dynamics~\eqref{eqn:mf-dynamics-local-policy}.
Consequently, we have
\begin{equation*}
    J^{\rho*}_{\cor} (\mu_0, \nu_0) = 1, \quad \forall \mu_0\in \P(\X), \nu_0 \in \P(\Y).
\end{equation*}

However, we know that for all $N_1$, the empirical distribution must be rational, and thus $\M^{N_1} \neq (\sqrt{3}, 1-1 / \sqrt{3})$ for all $N_1$ almost surely, which leads to 
\begin{equation*}
...
For simplicity, we use a single-team example to illustrate why the empirical distribution cannot be an information state when different agents are allowed to apply different strategies, even under the weakly-coupled dynamics~\eqref{eqn:dynamics-blue} and rewards~\eqref{eqn:reward}.
%
...
\begin{figure}
    ...
    \caption{A counter-example showing empirical distribution cannot serve as an information state.}
...
To illustrate that the empirical distribution cannot serve as an information state, consider a two-agent team and the following non-identical team strategy:
\begin{alignat*}{2}
...
We now consider two initial configurations with the same empirical distribution.
\begin{itemize}
    \item 
    $\X^2_{1,0} = x^1$ and $\X^2_{2,0} = x^2$, i.e., agent 1 is initially at~$x^1$ and agent~2 at~$x^2$.
    The empirical distribution is thus $\M^2_{0}=[0.5, 0.5]$.
    ...
    The empirical distribution is again $\M^2_{0}=[0.5, 0.5]$.
...
From this example, we have shown that the values can be different under the same team strategy given different initial conditions that correspond to the same empirical distribution since the empirical distribution does not differentiate the ordering of the agents.
Clearly, the empirical distribution alone is not enough to characterize the future evolution of the game, nor the value function. 
Consequently, the empirical distribution is not an information state and we need the joint state information in the finite-population game to properly construct the value functions when different agents apply different strategies.
...
The following lemma is a modified version of the $\ell^2$ weak law of large numbers~\citep{chung2001course} adapted to accommodate the notion of empirical distributions used in this work.
...
\begin{proof}
    ...
    Note that we can compute the empirical distribution of the whole Blue team  based on the empirical distribution of sub-populations of Blue agents on different states via
    \begin{equation}
        \label{appdx-eqn:001}
        \begin{aligned}
...
\end{equation*}
Define the empirical distribution of these $N^x_{1,t}$ Blue agents at time $t+1$ as 
\begin{equation}
...
%     Suppose that the reward function $r^\rho_t$ is concave-convex for all $t \in [T]$ and the system dynamics is independent, then both the lower and upper game values are concave-convex at each timestep. 
% \end{theorem}
% \begin{proof}
