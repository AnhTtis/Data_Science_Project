...
\subjclass[2000]{57S15, 57R15, 57R20, 57R85, 55N91}
...
\begin{question}
  ...
  Given a class \(\mathfrak{M}\) of closed manifolds and positive integers \(n\) and \(k\), does there exist a manifold \(M\) in \(\mathfrak{M}\) of dimension \(n\) admitting a \(S^1\)-action with exactly  \(k\) fixed points?
\end{question}
...
This conjecture and the above question have been studied before for different classes of manifolds and actions including actions on symplectic manifolds and almost complex manifolds (see for example \cite{MR776689}, \cite{MR2818694}, \cite{MR2802578},  \cite{MR2916364}, \cite{MR3272779}, \cite{MR3230015}, \cite{MR3506787},  \cite{zbMATH06498086}, \cite{MR3649458}, \cite{MR4161918}, \cite{MR4050069}).
...
For the above classes the following restriction on \(n\) are known, when \(k\leq 3\).
 ...
 \item Since the fixed point set \(M^{S^1}\) of any \(S^1\)-manifold \(M\) is a submanifold of even codimension, \(S^1\)-manifolds with at least one but at most finitely many fixed points can only exist in even dimensions.
   \item There is no closed orientable \(S^1\)-manifold with exactly one fixed point (see for example \cite[Remark 3.2]{zbMATH06498086}).
...
This corollary generalizes a result of Jang \cite{zbMATH06738992} on almost complex \(S^1\)-manifolds which was based on his work \cite{MR3272779} dealing with the symplectic case.
...
Hence it does not come as a surprise that there are some similarities between those dimensions which support a rational projective plane and those dimensions which support an orientable \(S^1\)-manifold with three fixed points.
...
This was combined with the fact that a rational projective plane has signature one and can have at most two non-zero Pontrjagin numbers.
...
Moreover, the signature of \(M\) is one.
To get from there to Theorem \ref{sec:ll} is then similar to the study of rational projective planes.
...
These are lifts of the ordinary characteristic classes of \(M\).
...
\subsection{Hirzebruch signature theorem}
...
Here the sum runs over all partition of \(k\) and the \(s_I\) are certain polynomials with rational coefficients in the Bernoulli numbers \(B_i\in \mathbb{Q}\).
...
\begin{proof}
  ...
  for all \(n=1,\dots,m\) (see \cite[p. 149]{zbMATH05031717}). In particular, \(|A_n|\geq 2\) for all \(n\), because \(\frac{t^{2k}}{e(q)}\epsilon(q)\neq 0\) for all \(q\in M^{S^1}\).
...
The claim about spin manifolds in Theorem~\ref{sec:ll} follows from Lemma~\ref{sec:proof-theorem} in combination with
...
The Pontrjagin classes of a unitary manifold \(M\) can be computed from the Chern classes of the manifold as follows:

\[p_k(M)=\sum_{i=0}^{2k}(-1)^{i+k}c_i(M)c_{2k-i}(M).\]

So if \(\dim M=8k\) and \(M\) satisfies the conclusion of Lemma~\ref{sec:proof-theorem} then we have
\begin{align*}
  p_{k,k}[M]&=4c_{2k,2k}[M],&p_{2k}[M]&=2c_{4k}[M]+c_{2k,2k}[M].
\end{align*}

Hence using the Hirzebruch signature theorem as in the case of oriented manifolds it follows that \(k\) must be a power of two.

Moreover, in these dimensions we have
\begin{equation}
  \label{eq:3}\sign(M)=\left(2s_k^2-s_{2k}\right)c_{2k,2k}[M]+ 2 s_{2k} c_{4k}[M],\end{equation}
and
\begin{equation}\label{eq:4}\td(M)=\frac{1}{2}\left(t_{2k}^2-t_{4k}\right)c_{2k,2k}[M] + t_{4k}c_{4k}[M].\end{equation}


We first show that for \(k\geq 2\) there are no solutions to these equations satisfying the constraints from Lemma~\ref{sec:proof-theorem}.

To do so first note that
\begin{equation}
  \label{eq:5}
  t_{2k}=\frac{B_{2k}}{2k!}=(-1)^{k+1}a_k\zeta(2k),
\end{equation}
where \(a_k=\frac{2}{(2\pi)^{2k}}\) and \(\zeta(s)=\sum_{n=1}^\infty n^{-s}\), for \(s\in \mathbb{C}\), \(\Re s >1\), is the Riemann zeta function (see \cite[p. 286]{MR0440554}).
Moreover,
\begin{equation}
  \label{eq:6}
  s_k=2^{2k}\left(2^{2k-1}-1\right)\frac{|B_{2k}|}{(2k)!}=a_kb_k\zeta(2k)
\end{equation}
with \(b_k=2^{2k}(2^{2k-1}-1)\).
Note that
\begin{equation}
  \label{eq:7}
  a_k^2=2a_{2k}
...
\begin{equation}
  \label{eq:8}
  \frac{3}{8} \leq \frac{b_k^2}{b_{2k}}\leq \frac{1}{2}
\end{equation}
for \(k\geq 2\).
In particular,
\[2s_k^2-s_{2k}=\left(4 \frac{b_k^2}{b_{2k}} \zeta(2k)^2-\zeta(4k)\right)b_{2k}a_{2k} >0,\]
since
\begin{equation}
  \label{eq:9}
  \zeta(2k)^2>\zeta(2k)>\zeta(2k+2)>\zeta(4k)>1.
\end{equation}

Assume that there is a closed unitary manifold \(M\) satisfying the constraints of Lemma~\ref{sec:proof-theorem} such that \(\dim M=8k\) with \(k\geq 2\).
We first show that we must have \(\td(M)=0\).
Indeed, by Equations (\ref{eq:3}) and (\ref{eq:4}), we have
\begin{align*}
  |\td(M)|&=|\left(-\frac{t^2_{2k}-t_{4k}}{2s_k^2-s_{2k}}s_{2k}+t_{4k}\right)c_{4k}[M]+\frac{1}{2}\frac{t^2_{2k}-t_{4k}}{2s_k^2-s_{2k}}\sign(M)|\\
          &\leq 3\cdot \left(\frac{t^2_{2k}-t_{4k}}{2s_k^2-s_{2k}}s_{2k}-t_{4k}+\frac{1}{2}\frac{t^2_{2k}-t_{4k}}{2s_k^2-s_{2k}}\right)\\\intertext{by Lemma~\ref{sec:proof-theorem}}
          &=3\cdot\left( \frac{2\zeta(2k)^2+\zeta(4k)}{4 \frac{b_k^2}{b_{2k}} \zeta(2k)^2-\zeta(4k)}\left(a_{2k}\zeta(4k)+\frac{1}{2b_{2k}}\right)+ a_{2k}\zeta(4k)\right)\\
  \intertext{by (\ref{eq:5}), (\ref{eq:6}), and (\ref{eq:7})}
          &\leq 3\cdot\left(2 \cdot \frac{2\zeta(2k)^2+\zeta(4k)}{\zeta(2k)^2}\left(a_{2k}\zeta(4k)+\frac{1}{2b_{2k}}\right)+ a_{2k}\zeta(4k)\right)\\
  \intertext{by (\ref{eq:8}) and (\ref{eq:9})}
          &\leq 3\cdot\left( 7a_{2k}\zeta(4k)+\frac{3}{b_{2k}}\right)\\\intertext{by (\ref{eq:9})}
          &\leq 3\cdot\left( 7\frac{2}{(2\pi)^8}\zeta(8)+\frac{3}{2^8(2^7-1)}\right)\\\intertext{by (\ref{eq:9}) and  $k\geq 2$}
  &=\frac{269}{914400}<1.
\end{align*}
Since \(\td(M)\) is an integer it follows that \(\td(M)=0\).

Hence, we have, again by Equations (\ref{eq:3}) and (\ref{eq:4}),
\begin{align*}
  1&= |\sign(M)|\\
   &=|\left(-\left(2s_k^2-s_{2k}\right)\frac{2t_{4k}}{t_{2k}^2-t_{4k}} + 2 s_{2k}\right) c_{4k}[M]|\\
   &\leq 3\cdot \left(\left(4 \frac{b_k^2}{b_{2k}} \zeta(2k)^2-\zeta(4k)\right)\frac{2\zeta(4k)}{2\zeta(2k)^2+\zeta(4k)}+2\zeta(4k)\right)b_{2k}a_{2k}\\
  \intertext{by Lemma \ref{sec:proof-theorem} and Equations (\ref{eq:5}), (\ref{eq:6}), (\ref{eq:7})}
   &\leq 3\cdot\left(\left( 2 \zeta(2k)^2-\zeta(4k)\right)\cdot \frac{2}{3} + 2\zeta(4k)\right)b_{2k}a_{2k}\\\intertext{by (\ref{eq:8}) and (\ref{eq:9})}
   &< 4\cdot\left(\zeta(4)^2+\zeta(8)\right)\left(\frac{2}{\pi}\right)^8\\\intertext{by (\ref{eq:9}) and  $k\geq 2$}
     &= \frac{3328}{14175}< 1.
\end{align*}
This is a contradiction.
Therefore there are no solutions to Equations~(\ref{eq:3}) and~(\ref{eq:4}) satisfying the constraints of Lemma~\ref{sec:proof-theorem} when \(k\geq 2\).

Next let \(k=1\).
Assume there is a \(8\)-dimensional unitary \(S^1\)-manifold \(M\) with exactly three fixed points.
After changing the orientation of \(M\) if necessary we can assume \(\sign(M)>0\).
In this case we have
\begin{align*}
  s_1&=\frac{1}{3}&s_2&=\frac{7}{45}&t_2&=\frac{1}{12}&t_4&=-\frac{1}{720}.
\end{align*}
Hence Equations~(\ref{eq:3}) and (\ref{eq:4}) have only one solution satisfying the constraints from Lemma~\ref{sec:proof-theorem}, namely
\begin{align*}
