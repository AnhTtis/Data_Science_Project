...
\affil{Department of Astrophysics/IMAPP, Radboud University Nĳmegen, P.O. Box 9010, 6500 GL Nĳmegen, The Netherlands}
...
\citep{Perna2021,Wang2022}. In some cases they would also 
produce luminous neutrino bursts \citep{Zhu2021nu}. Time-dependent photoiozation of
...
%
\begin{equation}
    T_0=\left( \frac{L_{\rm{iso}}}{4\pi r_0^2 a c}\right)^\frac14,
    \label{eq:t0}
\end{equation}
%
...
\caption{Best fit model for the prompt light curve of GRB~191019A. \textit{Neil Gehrels Swift Observatory} data (blue symbols) are adapted from \cite{Levan2023}. The thick orange line shows the best fit model for the overall prompt light curve. The thin green line marks the 
...
The overall fit is acceptable, even if some deviations are noticeable (Figure~\ref{fig:bestfit}). These are due to the fact that we are not attempting to carry out a formal fit to the data. The unknown properties of the GRB engine are too many. Investigating, e.g., whether the engine duration is different from 1.1~s, or whether the number of shells ejected is truly 6 is beyond the scope of this paper. What we focus here is showing that a vanilla short GRB in terms of energy, engine duration and Lorentz factor, can reproduce the overall shape of the burst, and we can set limits on the properties of the external medium. In addition we
note that we have binned the prompt light curve from \cite{Levan2023} and may have smoothed out some short time-scale variability in the process. Some variability on time scales shorter than the overall burst duration is expected in our model, due to the fact that each shell from the engine produces a reverse shock with its individual peak time and duration. As discussed above, a detailed fit with individual shell ejection times, Lorentz factors, thicknesses, and energetics is beyond the scope of this manuscript. In addition, such a fit would not be supported by the low signal to noise ratio of the prompt light curve. We stress, however, that if well-separated peaks were present in the light curve, it would make it difficult to reconcile the data with the model.
...
emerging from a circumburst medium of density $n_{\rm ext}\sim 10^7-10^8$~cm$^{-3}$. This value is higher than even the densest known  molecular clouds, but fully consistent with the outer parts of an AGN disk.
 ...
 The host galaxy	of GRB~191019A	has a smooth morphology with a very
compact, almost point-like core. A scaling relation with the galaxy
mass would imply a central SMBH of a few $\times 10^7
M_\odot$. \citet{Levan2023} explored the possibility that the central
light concentration could arise from an AGN.  The observation of a
weak [N$_{\rm II}$] line combined with the absence of hydrogen or oxygen
lines was found to be compatible with an AGN-like set of line ratios,
although this is not conclusive. Late observations with the {\em Swift} X-ray
Telescope set a limit on the X-ray luminosity of $L_X<6\times
10^{42}$~erg~s$^{-1}$, which rules out a high-luminosity AGN, while
still leaving the possibility of a low-luminosity
one\footnote{Measurements of the X-ray luminosity function of AGNs in
the low-redshift Universe \citep{Ueda2003} show that the low
luminosity AGNs ($\sim 10^{41}-10^{42}$~erg~s$^{-1}$) are largely
outnumbering the high luminosity ones.}.
The localization of GRB~191019A	within the innermost 100~pc of its
host galaxy makes the AGN disk interpretation especially tantalizing.
However we note	that, for this scenario	to hold, either	the SMBH mass
has to be somewhat lower than the relatively high value implied	by the
galaxy mass, or	accretion has to be somewhat lower than the close-to-Eddington rate typically assumed, to allow for both
a low-luminosity AGN as	well as	a not too massive disk that would diffuse the GRB radiation (see discussion
in \citealt{Perna2021})\footnote{The latter condition can however be somewhat relaxed considering the
fact that GRB jets can excavate a funnel in the disk and their cocoon emerge
\citep{Tagawa2022}.}. In the following,	to make	our discussion
more quantitative using specific	examples from the literature, we will
consider an AGN	disk around a SMBH of $4\times 10^6 M_\odot$.


In order to relate the inferred circumburst density to that of an AGN disk, we can refer to specific disk models which have been developed in the literature, and in particular the ones of \citet{Sirko2003} and of \citet{Thompson2005}. A central density of $\sim 10^7-10^8$~cm$^{-3}$ is realized between $\sim 5\times 10^5-10^6$ gravitational radii ($R_g$) for the former model, and  $\sim (10^6-3\times 10^6)~R_g$ for the latter. 
For a $4\times 10^6 M_\odot$ SMBH, the range of density inferred for the ambient medium of GRB~101019A would hence correspond to radial distances between a tenth of pc to a pc.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\acknowledgements We would like to acknowledge insightful comments from Daniele Bj{\o}rn Malesani and by the anonymous referee that helped improving the manuscript. DL acknowledges support from NSF grant AST-1907955.
RP acknowledges support by NSF grant AST-2006839.
