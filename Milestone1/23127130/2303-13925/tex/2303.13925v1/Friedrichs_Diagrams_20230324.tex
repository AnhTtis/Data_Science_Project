...
\author{
Morris Brooks\footnote{Institute of Science and Technology Austria, Am Campus 1, 3400 Klosterneuburg, Austria}\ \footnote{E-Mail: \url{morris.brooks@ist.ac.at }}~and
...
\begin{equation*}
	A = \int f_A(\bX_A, \bY_A) a^*_{x_{A, n_A}} \ldots a^*_{x_{A, 1}} a_{y_{A, 1}} \ldots a_{y_{A. m_A}} \; d\bX_A d\bY_A
\end{equation*}
...
\begin{itemize}
    \item Friedrichs diagrams generally represent operator products, while Feynman diagrams represent complex numbers or distributions that describe the vacuum expectation value of time ordered operator products.
...
\end{itemize}
More generally, Feynman diagrams often appear in the context of perturbative QFT, while Friedrichs diagrams are a non--perturbative tool.\\

The rest of this article is structured as follows: In Section \ref{sec:diagrams}, we introduce the notation and give the rules for translating diagrams into mathematical expressions. In Section \ref{sec:mainresults}, we state our main results, including formulas for $ [A, B], \{A, B\} $ and $ AB $ in terms of normal ordered products, and provide algorithms for their practical employment in commutator computations. Proofs of the main results are given in Section \ref{sec:proofs}. Section \ref{sec:applications} concerns an application of the graphical framework presented here, where we demonstrate that the various contributions arising in the non--linear Hartree dynamics can naturally be identified as a subset of all the diagrams arising in the quantum time evolution.\\  
%applications
...
%\end{equation}
%Using these spaces,
We now define the operator--valued distributions $ a^*_{\bx} $ and $ a_{\bx} $, such that for $ n_A, m_A \in \NNN_0 $ and $ f_A \in \fh^{\otimes n_A} \otimes \fh^{\otimes m_A} $, the normal ordered distribution pairing
\begin{equation}
	A = \int f_A(\bX_A, \bY_A) a^*_{x_{A, n_A}} \ldots a^*_{x_{A, 1}} a_{y_{A, 1}} \ldots a_{y_{A. m_A}} \; d\bX_A d\bY_A
\label{eq:A}
\end{equation}
...
\begin{equation}
	...
	d\bX := \prod_{v = 1}^V d\bX_v, \quad
	d\bY := \prod_{v = 1}^V d\bY_v.
\label{eq:bXvbYv}
\end{equation}
...
\begin{equation}
	...
	\left( \prod_{\ell' = 1}^{|\cI'| - C} a_{\sigma'(\ell')} \right) \; d\bX d\bY.
\label{eq:G}
\end{equation}
...
\begin{equation}
	...
	\left( \prod_{c = 1}^C \delta(\bx_{\pi(c)} - \by_{\pi'(c)}) \right) \; d\bX_{\pi} d\bY_{\pi'}.
\end{equation}
...
\begin{aligned}
	...
	\prod_{u' \in \cU'} a_{u'}
	\prod_{\ell' = 1}^{m_B} a_{B, \ell'} \; d\bX d\bY.
\end{aligned}
\label{eq:cont}
\end{equation}
...
\begin{aligned}
	...
	\left( \prod_{\ell' = 1}^{m_B} a_{B, \ell'} \right) \; d\bX d\bY.
\end{aligned}
...
\end{equation}
The proof of \eqref{eq:contproduct2} is done by an induction over $ n_B, m_A \in \NNN_0 $. We start with the case where $ n_B = 0 $ or $ m_A = 0 $ and establish two induction steps:
\begin{itemize}
    \item To $ (m_A, n_B) $, assuming the statement holds for $ (m_A, n_B - 1) $ and $ (m_A - 1, n_B - 1) $.
    \item To $ (m_A, n_B) $, assuming the statement holds for $ (m_A - 1, n_B) $ and $ (m_A - 1, n_B - 1) $.
\end{itemize}
These two steps can then be used to gradually ``fill up the $ m_A $--$ n_B $--quadrant'' of indices, for which \eqref{eq:contproduct2} holds true, starting from the $ m_A = 0 $-- and $ n_B = 0 $--axes.\\
...
\noindent \underline{Induction step $ (m_A - 1, n_B - 1) \land (m_A, n_B - 1) \mapsto (m_A, n_B) $:} We consider the left--hand side of \eqref{eq:contproduct2} and commute $ a^*_{B, n_B} $ to the very left, before using the induction assumption:
\begin{equation}
\begin{aligned}
	&a_{A, 1} \ldots a_{A, m_A} a^*_{B, n_B} \ldots a^*_{B, 1}\\
...
Finally, \circled{3} is the normal ordered first term on the r.h.s of \eqref{eq:contproduct2}. This establishes \eqref{eq:contproduct2} for $ (m_A, n_B) $ and thus the first induction step.\\

\noindent \underline{Induction step $ (m_A - 1, n_B - 1) \land (m_A - 1, n_B) \mapsto (m_A, n_B) $:} This step can be inferred from the first one. Applying the bijective involution $ ^*: \cA_+ \to \cA_+ $, the l.h.s. of \eqref{eq:contproduct2} becomes
\begin{equation}
\begin{aligned}
	&\left( a_{A, 1} \ldots a_{A, m_A} a^*_{B, n_B} \ldots a^*_{B, 1} \right)^*\\
	= &a_{B, 1} \ldots a_{B, n_B} a^*_{A, m_A} \ldots a^*_{A, 1}\\
	= &a^*_{A, m_A} \ldots a^*_{A, 1} a_{B, 1} \ldots a_{B, n_B}
	+ \sum_{(\pi, \pi') \in \cC} \prod_{c = 1}^C \delta(\by_{\pi'(c)} - \bx_{\pi(c)})
	\left( \prod_{u' \in \cU'} a_{u'} \right)^*
	\prod_{u \in \cU} a_u\\
	= &\left( a^*_{B, n_B} \ldots a^*_{B, 1} a_{A, 1} \ldots a_{A, m_A}
	+ \sum_{(\pi, \pi') \in \cC} \prod_{c = 1}^C \delta(\bx_{\pi(c)} - \by_{\pi'(c)})
	\left( \prod_{u \in \cU} a_u \right)^*
	\prod_{u' \in \cU'} a_{u'} \right)^*,\\
\end{aligned}
\label{eq:adjointtrick}
\end{equation}
where we used in the third line that by the first induction step, \eqref{eq:contproduct2} for $ (n_B - 1, m_A - 1) $ and $ (n_B, m_A - 1) $ implies \eqref{eq:contproduct2} for $ (n_B, m_A) $. Now, applying $ ^* $ to \eqref{eq:adjointtrick} immediately establishes the second induction step.\\
\end{proof}
...
As in the previous proof, let us assume w.l.o.g. that $A$ and $B$ are of the form $A=a_{A, 1} \ldots a_{A, m_A} $ and $B=a^*_{B, n_B} \ldots a^*_{B, 1}$. Since $A$ and $B$ only involve finitely many modes, we can further assume that the one particle Hilbert space is finite dimensional, i.e., given by $\mathfrak{h}=\mathbb{C}^N$. In the following, let $|z\rangle $ denote the coherent state corresponding to $z\in \mathbb{C}^N$ and let us define the lower symbol of an operator $X$ as the function $z\mapsto \langle{z|X|z\rangle}$. Clearly the lower symbol of the operator $\normalordered{AB}$ is given by $\langle z|\normalordered{AB}|z\rangle=\prod_{i=1}^{n_B}\overline{z}_{B,i}\prod_{j=1}^{m_A}z_{A,j}$, and therefore the lower symbol of the operator $AB$ is given by
\begin{equation}
\label{eq: symbol}
\langle z|AB|z\rangle\! =\! e^{\nabla_{\overline{z}}\nabla_z}\! \! \left(\prod_{i=1}^{n_B}\overline{z}_{B,i}\prod_{j=1}^{m_A}z_{A,j}\! \right)\! \! =\!  \langle z| \normalordered{AB} |z\rangle\, +\! \! \! \! \! \sum_{C=1}^{\min\{n_B,m_A\}} \! \frac{\left(\nabla_{\overline{z}}\nabla_z\right)^C}{C!} \! \left(\prod_{i=1}^{n_B}\overline{z}_{B,i}\prod_{j=1}^{m_A}z_{A,j}\! \right),
\end{equation}
see e.g., \cite{perel}. We are going to prove by induction in $ C $ that 
\begin{equation}
\label{eq: coherent}
    \left(\nabla_{\overline{z}}\nabla_z\right)^C \left(\prod_{i=1}^{n_B}\overline{z}_{B,i}\prod_{j=1}^{m_A}z_{A,j}\! \right)=\sum_{\pi\in \cC^B_C,\pi'\in \cC^A_C}f_{\pi,\pi'}(z),
\end{equation}
where $\cC^A_C := \big\{ \pi' \; \big\vert \;
	\pi': \{1, \ldots, C \} \to \{1,\ldots , m_A\},\, |\mathrm{imag}(\pi)|=C
	 \big\}$, $\cC^B_C := \big\{ \pi \; \big\vert \;
	\pi: \{1, \ldots, C \} \to \{1,\ldots , n_B\},\, |\mathrm{imag}(\pi)|=C \big\}$ and $f_{\pi,\pi'}$ is defined as
 \begin{equation}
     f_{\pi,\pi'}(z):=\prod_{p=1}^C \delta\left(x_{B,\pi(p)}-y_{A,\pi'(p)} \right)\prod_{i\notin \mathrm{imag}(\pi) }\overline{z}_{B,i}\prod_{j\notin \mathrm{imag}(\pi')} z_{A,j}.
 \end{equation}
 The case $C=0$ is trivial. The induction step $C\mapsto C+1$ follows from the observation that we can express the term $\left(\nabla_{\overline{z}}\nabla_z\right)^{C+1} \left(\prod_{i=1}^{n_B}\overline{z}_{B,i}\prod_{j=1}^{m_A}z_{A,j}\! \right)= \sum_{\pi\in \cC^B_C,\pi'\in \cC^A_C}\nabla_{\overline{z}}\nabla_z f_{\pi,\pi'}(z)$ as
 \begin{equation}
 \begin{aligned}
    \sum_{\pi\in \cC^B_C,\pi'\in \cC^A_C}& \prod_{p=1}^C \delta\left(x_{B,\pi(p)}-y_{A,\pi'(p)} \right)\! \! \! \sum_{\ell \notin \mathrm{imag}(\pi),k \notin \mathrm{imag}(\pi')}\! \! \! \delta\left(x_{B,\ell}-y_{A,k} \right)\! \! \! \prod_{i\notin \mathrm{imag}(\pi) \cup \{\ell\}}\! \! \! \overline{z}_{B,i}\! \! \! \prod_{j\notin \mathrm{imag}(\pi')\cup \{k\}}\! \! \!  z_{A,j}\\
 ...
 \begin{equation}
     \frac{\left(\nabla_{\overline{z}}\nabla_z\right)^C}{C!} \! \left(\prod_{i=1}^{n_B}\overline{z}_{B,i}\prod_{j=1}^{m_A}z_{A,j}\! \right)=\sum_{(\tilde{\pi},\tilde{\pi}')\in \cC_C}\sum_{\sigma\in S_C}\frac{1}{C!}f_{\tilde{\pi}\circ \sigma ,\tilde{\pi}'\circ \sigma }(z)=\sum_{(\tilde{\pi},\tilde{\pi}')\in \cC_C}f_{\tilde{\pi} ,\tilde{\pi}' }(z),
 \end{equation}
 where we have used $f_{\tilde{\pi}\circ \sigma ,\tilde{\pi}'\circ \sigma }=f_{\tilde{\pi} ,\tilde{\pi}' }$. Since $\sum_{(\tilde{\pi},\tilde{\pi}')\in \cC}f_{\tilde{\pi} ,\tilde{\pi}' }(z)$ is the lower symbol of the operator $ A \cont B =\sum_{(\tilde{\pi},\tilde{\pi}')\in \cC}\prod_{p=1}^C \delta\left(x_{B,\tilde{\pi}(p)}-y_{A,\tilde{\pi}'(p)} \right) \left( \prod_{i\notin \mathrm{imag}(\tilde{\pi}) }a_{B,i} \right)^* \prod_{j\notin \mathrm{imag}(\tilde{\pi}')} a_{A,j}$, we obtain by Eq.~(\ref{eq: symbol}) that $\langle z|AB|z\rangle=\langle z|\normalordered{AB}|z\rangle+\langle z|A \cont B|z\rangle$ for all $z\in \mathbb{C}^N$, and therefore $AB=\normalordered{AB}+A \cont B$.
\end{proof} 
...
\end{equation}
with $ \sgn(\pi, \pi') = \sgn(\pi, \pi', m_A, n_B) $ given by \eqref{eq:sgnpipi}. As in the proof of Lemma \ref{lem:cont}, we perform an induction over $ m_A, n_B \in \NNN_0 $, which amounts to verifying \eqref{eq:contferproduct2} for $ m_A = 0 $ or $ n_B = 0 $ and then establishing the two induction steps $ (m_A - 1, n_B - 1) \land (m_A, n_B - 1) \mapsto (m_A, n_B) $ and $ (m_A - 1, n_B - 1) \land (m_A - 1, n_B) \mapsto (m_A, n_B) $.\\
The cases $ m_A = 0 $ and $ n_B = 0 $ are trivial, since the sum over $ (\pi, \pi') \in \cC $ is empty.\\

\noindent \underline{Induction step $ (m_A - 1, n_B - 1) \land (m_A, n_B - 1) \mapsto (m_A, n_B) $:} This step is performed in similarity to \eqref{eq:evaluatecont}: We pull $ a^*_{B, n_B} $ to the left, which generates several anticommutators
\begin{equation}
\begin{aligned}
	&a_{A, 1} \ldots a_{A, m_A} a^*_{B, n_B} \ldots a^*_{B, 1}\\
...
Finally, \circled{3} is just the normal ordered first term on the r.h.s. of \eqref{eq:contferproduct2}, which establishes the equality and thus the first induction step.\\

\noindent \underline{Induction step $ (m_A - 1, n_B - 1) \land (m_A - 1, n_B) \mapsto (m_A, n_B) $:} This step can be inferred from the first one by the same arguments as in the proof of Lemma \ref{lem:cont}. \eqref{eq:adjointtrick} can readily be re--used, while inserting factors of $ \sgn(\pi, \pi') $ and $ (-1)^{m_A n_B} $ in the appropriate positions.\\
\end{proof}
...
\label{eq:nested}
\end{equation}
Here, $A_{k,\ell}^{t_1,\dots , t_k}$ is defined as containing all contributions in $\frac{1}{i^k}\left[\dots \left[A,\frac{1}{2}\widehat{V^{(t_1)}}\right],\dots , \frac{1}{2}\widehat{V^{(t_k)}}\right]$ whose diagrams have $\ell$ incoming and outgoing legs: We write the nested commutator according to Theorem \ref{thm:cont} as a sum of connected diagrams, see also the definition of the bosonic attached product in Eq.~(\ref{eq:cont}) and the comment below, and sort the diagrams according to the number of incoming and outgoing legs. In the subsequent Lemma \ref{lem:classic}, we are going to verify that the sum over all acyclic graphs (i.e., those with $ l = k+1 $ legs on both sides) appearing in Eq.~(\ref{eq:nested}) amounts to the time evolution given by the non--linear Hartree dynamics (see also Figure \ref{fig:Friedrichs_evolution}). In order to formulate Lemma \ref{lem:classic}, let us define the evaluation $\langle f \rangle_u$ of a graph $f\in \fh^{\otimes n}\otimes  \fh^{\otimes m}$ by a one particle state $u$ as the expression obtained by connecting each incoming vertex with $\overline{u}$ and each outgoing vertex with $u$, i.e., we define the expectation value as $\langle f \rangle_u:=\int f(X,Y)\overline{u(x_1)}\dots \overline{u(x_n)}u(y_1)\dots u(y_{m})\mathrm{d}X\mathrm{d}Y$. Furthermore, we define the mean field potential $V_u$ corresponding to a one particle state $u$ via its integral kernel as $V_u(x,y):=\int V(x,x',y,y')\overline{u}(x')u(y')\mathrm{d}x'\mathrm{d}y'$.
\begin{figure}
	\centering
...
\begin{equation}
    \langle f_A \rangle_{u_t}=\sum_{k=0}^\infty \int\limits_{0\leq t_1\leq \dots \leq t_k\leq t} \langle f_{k,k+1}^{t_1,\dots , t_k} \rangle_{e^{-i t T}\! u_0}\, \mathrm{d}t_1\dots \mathrm{d}t_k,
    \label{eq: classicEvo}
\end{equation}
where $f_{k,k+1}^{t_1,\dots , t_k}$ is the integral kernel of $A_{k,k+1}^{t_1,\dots , t_k}$ and $\|V\|$ denotes the operator norm.
\end{lemma}
\begin{proof}
...
\begin{equation}
    \sum_{k=0}^\infty \int\limits_{0\leq t_1\leq \dots \leq t_k\leq t-s} \langle f_{k,k+1}^{t_1,\dots , t_k} \rangle_{v_s}\, \mathrm{d}t_1\dots \mathrm{d}t_k
    \label{eq:interpolation}
\end{equation}
is constant with respect to $s$, which immediately concludes the proof by plugging in the values $s=0$ and $s=t$ (note that for $k=0$, $f_{0,1}=f_A$). In order to do this, we define $\{f_A,f_B\}_*$ as the sum over all graphs appearing in the expression $ A \cont B - B \cont A$ that have only a single connection between the connectors of $f_A$ and $f_B$, where we add the subscript $*$ in order to distinguish $\{f_A,f_B\}_*$ from the anti--commutator and we purposefully use curly brackets in analogy to the Poisson bracket from classical mechanics. Using this notation, we obtain $f_{k,k+1}^{t_1,\dots , t_k}=\frac{1}{2i}\{f_{k-1,k}^{t_1,\dots , t_{k-1}},V^{(t_k)}\}_*$. Since there are $4k$ different graphs appearing in $\{f_{k-1,k}^{t_1,\dots , t_{k-1}},V^{(t_k)}\}_*$, we have the estimate $\|f_{k,k+1}^{t_1,\dots , t_k}\|\leq 4k\frac{1}{2}\|V\|\|f_{k-1,k}^{t_1,\dots , t_{k-1}}\|$, where we have used that the operator norm of a contracted operator, defined in Eq.~(\ref{eq:G}), is bounded from above by the product of the operator norms of the individual operators. Consequently, $\|f_{k,k+1}^{t_1,\dots , t_k}\|\leq k!\|f_A\|(2\|V\|)^k$. Using the control of the norm and $\Vert v_s \Vert=\Vert u_0 \Vert$, we immediately obtain 
\begin{equation}
\begin{aligned}
  \bigg|\int\limits_{0\leq t_1\leq \dots \leq t_k\leq t-s}\! \! \! \!\!  \!  \!  \langle f_{k,k+1}^{t_1,\dots , t_k}  & \rangle_{v_s}\, \mathrm{d}t_1\dots \mathrm{d}t_k\bigg| \leq  \! \! \! \! \! \int\limits_{0\leq t_1\leq \dots \leq t_k\leq t-s}\! \! \! \! \! \! \! \! \! k!\|f_A\|(2\|V\|)^k\|u_0\|^{2(k+1)}\, \mathrm{d}t_1\dots \mathrm{d}t_k\\
   &\leq\!  \|f_A\|\|u_0\|^2 \left(2(t-s)\|V\|\|u_0\|^2\right)^k.
   \end{aligned}
\end{equation}
Therefore the sum in Eq.~(\ref{eq:interpolation}) converges absolutely for $|t-s|<\frac{1}{2\|V\|\Vert u_0 \Vert}$, and we can express the derivative $\frac{\mathrm{d}}{\mathrm{d}s}\left(\sum_{k=0}^\infty \int\limits_{0\leq t_1\leq \dots \leq t_k\leq t-s} \langle f_{k,k+1}^{t_1,\dots , t_k} \rangle_{v_s}\, \mathrm{d}t_1\dots \mathrm{d}t_k\right)$ as 
\begin{equation}
    \begin{aligned}
\sum_{k=0}^\infty & \int\limits_{0\leq t_1\leq \dots \leq t_k\leq t-s}\! \! \! \!   \! \!  \! \! \! \! \! \frac{\mathrm{d}}{\mathrm{d}s}\langle f_{k,k+1}^{t_1,\dots , t_k} \rangle_{v_s} \mathrm{d}t_1\! \dots\!  \mathrm{d}t_k \! -\! \sum_{k=1}^\infty \int\limits_{0\leq t_1\leq \dots \leq t_{k-1}\leq t-s}\! \! \! \! \! \! \! \!  \! \! \! \! \! \! \frac{1}{2i}\langle \{f_{k-1,k}^{t_1,\dots , t_{k-1}},V^{(t-s)}\}_* \rangle_{v_s} \mathrm{d}t_1\! \dots \! \mathrm{d}t_{k-1}\\
       &=\sum_{k=0}^\infty  \int\limits_{0\leq t_1\leq \dots \leq t_k\leq t-s}\! \! \! \!   \! \!  \! \! \! \! \left(\frac{\mathrm{d}}{\mathrm{d}s}\langle f_{k,k+1}^{t_1,\dots , t_k} \rangle_{v_s}-\frac{1}{2i}\langle \{f_{k,k+1}^{t_1,\dots , t_{k}},V^{(t-s)}\}_* \rangle_{v_s}\right)\mathrm{d}t_1\! \dots \! \mathrm{d}t_{k}=0,
    \end{aligned}
\end{equation}
where we use that $i\frac{\mathrm{d}}{\mathrm{d}s}v_s=V^{(t-s)}_{v_s}v_s$, with $V^{(t)}_u(x,y):=\int V^{(t)}(x,x',y,y')\overline{u}(x')u(y')\mathrm{d}x'\mathrm{d}y'$, which allows us to express $\frac{\mathrm{d}}{\mathrm{d}s}\langle f \rangle_{v_s}$ for any $f\in \fh^{\otimes n} \otimes \fh^{\otimes n}$ as
\begin{equation}
    \begin{aligned}
        & \frac{1}{i}\sum_{j=1}^n\int f(X,Y) \prod_{k=1}^n \overline{v_s(x_k)} v_s(y_1)\dots v_s(y_{j-1})(V^{(t-s)}_{v_s}v_s)(y_j)v_s(y_{j+1})\dots v_s(y_n)\mathrm{d}X\mathrm{d}Y\\
        &\ \ \ -\frac{1}{i}\sum_{j=1}^n\int f(X,Y)\overline{v_s(x_1)}\dots \overline{v_s(x_{j-1})}\overline{(V^{(t-s)}_{v_s}v_s)(x_j)}\overline{v_s(x_{j+1})}\dots \overline{v_s(x_n)}\prod_{k=1}^n v_s(y_k)\mathrm{d}X\mathrm{d}Y\\
        &=\frac{1}{i}\! \sum_{j=1}^n\! \int\!  f(X,Y)V^{(t-s)}(x_{n+1},x_{n+2},y_{n+1},y_{n+2})\delta(y_j-x_{n+2})\! \! \prod_{k=1}^{n+1}\overline{v_s(x_k)}\! \! \! \! \! \! \! \prod_{\ell\in \{1,\dots ,n+2\}\setminus \{j\}}\! \! \! \! \! \! \! v_s(y_\ell) \mathrm{d}X'\mathrm{d}Y'\\
        &-\frac{1}{i}\! \sum_{j=1}^n\! \int\!  f(X,Y)V^{(t-s)}(x_{n+1},x_{n+2},y_{n+1},y_{n+2})\delta(x_j-y_{n+2})\! \! \! \! \! \! \!  \! \! \prod_{k\in \{1,\dots ,n+2\}\setminus \{j\}}\! \! \! \! \! \! \!\overline{v_s(x_k)} \prod_{\ell=1}^{n+1}v_s(y_\ell) \mathrm{d}X'\mathrm{d}Y'\\
        &=\frac{1}{2i}\langle \{f,V^{(t-s)}\}_* \rangle_{v_s},
    \end{aligned}
\end{equation}
...
\noindent Now, consider some generic $ A $ and $ B $, involving operator products $ a_{A, 1} \ldots a_{A, n_A} $ and $ a^*_{B, n_B} \ldots a^*_{B, 1} $. The product $ AB $ then takes the form (compare \eqref{eq:contfer}):
\begin{equation}
    AB = \sum_{(\pi, \pi') \in \cC} \sgn(\pi, \pi') G_{\pi, \pi'}.
\end{equation}
Our goal is to determine $ \sgn(\pi, \pi') $. We first consider the easier case where $ (\pi, \pi') $ corresponds to a maximally crossed diagram and then reduce to the generic case to the maximally crossed one. A ``smart zero insertion'' as in \eqref{eq:stepcomm} will produce many anticommutators, where the first term appearing is
\begin{equation}
...
SL was supported by the European Research Council (ERC) through the Starting Grant \textsc{FermiMath}, Grant Agreement No. 101040991.\\
