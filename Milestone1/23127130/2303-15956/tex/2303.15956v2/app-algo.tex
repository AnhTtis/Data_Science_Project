\section{DL Advocate}\label{app:algorithm}

The formal definition of the optimization algorithm is presented in Algorithm~\ref{alg:training}.
...
\While{not converged}
    ...
    \For{$(x,y) \in D$}
    \State{$\partial\ell_{sd} \gets \frac{1}{N_y}\sum_{i=1}^k(E_{y,i}-H^+_{y,i}) \frac{\partial h_i(x;\theta)}{\partial\theta}$}
...
The final linear layer has three output nodes, which correspond to the number of considered channels.
...
to keep the resulting function smooth in the domain.
