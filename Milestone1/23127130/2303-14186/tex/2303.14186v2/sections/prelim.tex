...
\begin{example}[\em Influence functions as an attribution method]
  ...
  \[
    \tau_{\text{IF}}(z)_i = \nabla_\theta \modeleval{z}{\thetastar}^\top\cdot H_{\thetastar}^{-1}\cdot \nabla_\theta \lossstar{z_i},
  \]
  where, again, $\thetastar$ are the parameters that minimize the empirical risk, $\lossstar{z_i}$ is the training loss of example $z_i$, and $H_{\thetastar}$ is the Hessian
