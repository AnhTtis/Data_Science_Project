...
\begin{itemize}
    \item The dimension $k$ of the random projection. %
...
the gradient features. Then, beyond a certain point, increasing projection
...
have a regularizing effect, so \trak may also benefit from that effect.} Last,
...
\textbf{Right:}
  ...
  performance of \trak  as a function with the number of models used in the
  ensembling step. \trak scores are computed with
  projection dimension of size $4000$.
}
\label{fig:jl_dim}
\end{figure}
...
better data attribution performance when we ensemble scores across multiple
models (Step 4 in \cref{sec:estimator_algo}). In our experiments, we average
...
\Cref{fig:jl_dim} (Right) verifies that increasing the number $M$ of
models used in the ensemble significantly increases \trak's efficacy. That said,
...
train the several independent models needed to ensemble.
In this subsection,
we study whether there is a cheaper alternative to
...
using a {\em fixed} number of checkpoints, but vary the number of
...
\cref{tab:num_independent_runs}) that \trak achieves comparable LDS when we use
multiple checkpoints along the trajectory of the same models as a proxy for
...
epochs on LDS. While \trak benefits from using later-epoch gradient features, it
maintains its efficacy even when we use gradient features from training runs
long before reaching convergence (see \Cref{tab:ablation_epoch_used}). We expect
this to further improve the computational efficiency of \trak.
...
\[
    \tau(z)_i = \frac{\phi(z)^\top \overbrace{(\Phi^\top R \Phi)^{-1}}^\text{reweighting (w/ or w/o $R$)} \phi(z_i) \cdot \overbrace{\frac{1}{1+e^{f(z_i)}}}^\text{loss gradient}}{1 - \underbrace{h_i}_\text{leverage score}}
\]

\noindent We ablate each of the terms above and re-evaluated the resulting variant of \trak on \cifartwo.
Our results in \Cref{tab:terms_ablate} indicate the following:
\begin{itemize}
\item {\bf Reweighting:} Experiment 6 shows that this matrix is the most crucial component of our estimator, and what distinguishes it from prior gradient based similarity metrics such as \tracin.
\item {\bf Loss gradient}: This term corresponds to the probability of the incorrect class, $1-p_i$; the name is based on the derivation in \Cref{app:theory_newton}, where this term corresponds to a multiplier associated with the gradient of the loss. Experiment 5 shows that this term contributes to the performance. Intuitively, this term helps reweight training examples depending on models' expected confidence on them.
\item {\bf Diagonal term $R$:} Although it is theoretically motivated by \Cref{lem:formal}, including this term results in lower LDS, so we do not include it in general (Experiments 2,4).
\item {\bf Leverage score:} This term does not impact LDS meaningfully, so we do not include it in general (Experiments 1,2).
\item {\bf Averaging ``out'' vs ``in'':} Averaging the estimator without the loss gradient term first, then re-scaling by the average loss gradient results in higher LDS (Experiment 3).
\end{itemize}
...
\begin{table}[!bht]
  \begin{tabular}{llllllr}
      ...
      \end{tabular}
  \centering
  \caption{The contribution of each term in \trak. For these experiments, we use JL dimension of 2000.}
...
Moreover, the larger gap on \cifarten and \qnli compared to \cifartwo hints that the gap will only widen on larger and more complex datasets or models.
...
There are different ways to ensemble a kernel method given multiple kernels $\{K_i\}_i$: (i) we can average the Gram matrices corresponding to each kernel first and predict using the averaged kernel (i.e., work with $\overline{K} = \frac{1}{n} \sum K_i$),  (ii) we can average their induced features (with respect to some fixed basis of functions) and use the corresponding kernel, or (iii) we can average the predictions derived from each kernel \citep{atanasov2023onset}.
\trak's algorithm follows the third approach.

Here we look at ensembling instead with the first approach using the averaged eNTK. We do this by first averaging the Gram matrices corresponding to each models' eNTK, using the Cholesky decomposition to extract features from the averaged Gram matrix ($G=LL^\top$), then plugging in the resulting features $L$ into \trak.
...
This gap suggests that the underlying model is better approximated as an ensemble over kernel predictors rather than evolving according to a single kernel.
...
the gradient features from the full model gradient are also both critical to \trak's
