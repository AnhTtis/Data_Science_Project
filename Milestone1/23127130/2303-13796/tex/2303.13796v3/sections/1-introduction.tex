...
(1) We analyze the state-of-the-art (SOTA) 3DHMR methods and propose a novel approach tailored to the perspective-distorted 3DHMR task.
...
(3) We build the first large-scale synthetic dataset PDHuman for the perspective-distorted 3DHMR task, with high-quality SMPL ground truth and camera parameters. To evaluate the performance on real images, we prepare two real-world benchmark datasets, SPEC-MTP~\cite{spec} and HuMMan~\cite{humman}, which contain perspective-distorted images with well-fitted SMPL parameters and camera parameters.
