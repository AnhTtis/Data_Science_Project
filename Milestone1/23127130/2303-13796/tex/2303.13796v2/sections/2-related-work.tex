...
Inspired by 3D mesh reconstruction tasks\cite{c-axis, topdown, close, rfeps, global}, Model-based methods works~\cite{hmr,spec,pare,hybrik,smoothnet,deciwatch} utilize SMPL parameters to recover the human pose and shape. The milestone method HMR~\cite{hmr} takes it as a direct regression task. Some other works try to utilize the help from intermediate representations, \eg 2D/3D joint heatmaps~\cite{skeleton2mesh,Pavlakos}, silhouettes~\cite{pare} and IUV map~\cite{pymaf}. Model-free works~\cite{graphcmr, meshgraphormer, fastmetro, tore} directly reconstructing 3D meshes from single view images. 
