...
3D human pose estimation from a single RGB image is essentially an ill-posed problem.  Some works~\cite{simplebaseline3d,moreno20173d,mehta2018single,poseaug} tackle this problem by estimating the 3D joint locations. To obtain more realistic and manipulable human bodies, a parametric body model SMPL~\cite{smpl} was proposed, which uses 3D rotation representation to model human joint motions with defined LBS weights. To reconstruct human mesh from RGB images, there exist two mainstream pipelines: optimization-based methods and learning-based methods.
...
Learning-based approaches~\cite{hmr,Pavlakos,decomr,pymaf,romp,bev,pare, eft} leverage a deep neural network to regress the human body model parameters or 3D coordinates of the human mesh, which can be further divided into model-based and model-free methods.
Model-based methods works~\cite{hmr,spec,pare,pymaf,romp,bev,hybrik,tuch,shapy} utilize SMPL parameters to recover the human pose and shape. The milestone method HMR~\cite{hmr} takes it as a direct regression task. Some other works try to utilize the help from intermediate representations, \eg 2D/3D joint heatmaps~\cite{skeleton2mesh,Pavlakos,romp,bev}, silhouettes~\cite{pare,nbf} and IUV map~\cite{pymaf,pymaf-x}. Model-free works~\cite{graphcmr, meshgraphormer, fastmetro, tore} directly reconstructing 3D meshes from single view images. 
GraphCMR~\cite{graphcmr} uses a graph convolutional neural network, and FastMETRO~\cite{fastmetro} uses transformer encoder architectures, to learn the relations among the vertices for better reconstruction.

\begin{figure}[t]
\begin{center}
    \includegraphics[width=0.9\linewidth]{pictures/camera_systems.pdf}
\end{center}
\vspace{-10pt}
\caption{
