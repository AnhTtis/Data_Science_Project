...
Through extensive experiments on the two tasks and on all the language pairs of the MuST-C v1.0 corpus \cite{CATTONI2021101155}, we demonstrate that the presence of bugs can be hidden by good -- but incorrect -- results (\S\ref{subsec:impact_bug}) that consequently lead to wrong findings (\S\ref{subsec:impact_code}).
...
Since with correct implementations the results are independent of the presence of padding (and\sara{,} therefore\sara{,} from the IBS),
...
%will
demonstrate in this section, the
%presence of bugs
...
\mg{an open source codebase named \enquote{Conformer}\footnote{\url{https://github.com/sooftware/conformer}}, and \mg{\torchimpl{}}~\citep{yang2021torchaudio}.}
...
In Figure~\ref{fig:relativePEs}, for the sake of clarity of the example, we pretend that existing implementations set to 0 the PE in the padding area. While this is not what happens in practice (as the padding area contains other sinusoidal PEs), it shows that the correct values are discarded and the final matrix significantly differs from that obtained without padding, which is
...
%
Also, 
all are affected by \bugtwo{} and \bugthree, except for the 
%implementation by Pytorch,
%\mn{PyTorch one,}
\torchimpl{} implementation,
which does not introduce relative PEs in the attention and 
\sara{does not include}
%replaces 
the initial sub-sampling convolutional layers 
\sara{in its implementation.}
...
It is worth remarking that, by default, on the Ampere GPUs the PyTorch backend computes convolutions and matrix multiplications with TensorFloat-32\footnote{\url{https://pytorch.org/docs/stable/notes/cuda.html\#tensorfloat-32-tf32-on-ampere-devices}.} (\textbf{TF32}) cores. TF32 speeds up the computation but introduces numeric errors that can cause small 
%but 
...
\setlength{\tabcolsep}{12pt}
    ...
    \specialrule{.1em}{.05em}{.05em} 
    \end{tabular}
    \caption{WER ($\downarrow$) scores \sara{for ASR} obtained with \sara{TF32 and bugs}
    %the different
    %types of bug \bugall{} 
    %\mg{bugs}
    %and TF32 for \mg{ASR}
...
\setlength{\tabcolsep}{2.5pt}
    ...
    % %(computed with bootstrap resampling with 95\% confidence interval).}
    % }
    \caption{\mn{BLEU ($\uparrow$) \mg{scores} for \sara{ST obtained with TF32 and bugs as IBS varies (1, 10, and 100 sentences).}
    %the st task. 
...
\setlength{\tabcolsep}{5.7pt}
    ...
    \specialrule{.1em}{.05em}{.05em} 
    \end{tabular}
    \caption{BLEU ($\uparrow$) \sara{scores for ST}
    ...
    %given by 
    \sara{of}
    the CTC compression is statistically significant, computed with bootstrap resampling (95\% CI).
    %(computed with bootstrap resampling with 95\% confidence interval).}
    }
    \label{tab:ST}
\end{table*}
...
% To showcase how the development of new solutions on top of incorrect code can result in
% %\sara{the use of incorrect code to develop new software can lead to}
% %building new works on incorrect code can cause 
% misleading findings, we evaluate the introduction of the CTC compression (or shrinking) mechanism \citep{liu2020bridging,gaido-etal-2021-ctc}
% %to 
% into the Conformer encoder. 
% The CTC \citep{Graves2006ConnectionistTC} enables producing an output sequence of variable length that is shorter than the input one, as in the case in which the input is audio and the target is the 
% %sequence of 
% %%uttered symbols -- characters, sub-words --
% %subwords
% %corresponding to the 
% \sara{transcript.}
% %\sara{while preserving its original content.}
...
% corresponding to the same prediction by averaging them and is generally applied 
% %in 
% to
% an intermediate encoder layer 
% %(in our case, the 8th out of 12). 
% (8\textsuperscript{th} in our case).
...
% \citet{liu2020bridging} and \citet{gaido-etal-2021-ctc} demonstrated
% that this method greatly reduces 
% %the 
% training and inference costs, while it brings limited gains in terms of output quality (BLEU) in the Transformer architecture.
% %that this method brings limited gains in terms of %translation 
% %output quality in the Transformer architecture, while it greatly reduces the training and inference costs.
% In this section, we analyze the effect of 
% %adopting 
% applying this technique 
% %in 
% to the Conformer architecture both with all bugs (\bugall) and without them (\smallcorrect).
...
% \mn{[PENSO CHE QUESTA PARTE ANDREBBE RISTRUTTURATA.\\\\1) NON SI CAPISCE PERCHE' CI SI CONCENTRA SULLA CTC E QUESTO DOVREBBE ESSERE CHIARITO SUBITO, MAGARI SPOSTANDO E RIELABORANDO IL RIFERIMENTO A LIU E GAIDO (SU TRANSFORMER), FACENDO PASSARE L'IDEA CHE INTERROGARSI SU USO ED EFFICACIA DELLA CTC E' LEGITTIMO E MOLTO SENSATO IN QUANTO TECNICA NOTA, POTENZIALMENTE EFFICACE, E PER QUESTO UTILIZZATA.\\\\2) L'ULTIMA FRASE LA VEDREI MEGLIO ALL'INIZIO, QUANDO SI DICE COSA VOGLIAMO FARE IN QUESTA SEZIONE.\\\\STRUTTURA SUGGERITA: \textit{i)} TO SHOWCASE...BLA...CONSIDERIAMO L'INTEGRAZIONE DI UNA TECNICA NOTA IN VERSIONI CON E SENZA BUG DEL CONFORMER, CHIEDENDOCI SE E COME I BACHI ESISTENTI INFLUISCONO SULLE CONCLUSIONI A VALLE CIRCA L'EFFICACIA DI TALE INTEGRAZIONE,  \textit{ii)} TALE TECNICA E' LA CTC, CHE LAVORI PRECEDENTI HANNO DIMOSTRATO ESSERE...BLA, \textit{iii)} LA CTC ENABLES...BLA BLA SU COS'E' LA CTC.]}
...
\sara{To showcase how the development of new solutions on top of incorrect code can result in misleading conclusions, we conducted ad-hoc experiments on both the correct (\smallcorrect) and incorrect (\bugall) codebases.
These experiments involve the use of CTC compression, a popular technique adopted in speech processing that, given its intrinsic characteristics, is potentially sensitive to the bugs identified in \S\ref{subsec:analysis}.
%and whose impact has been analyzed in \S\ref{subsec:impact_bug}. 
Specifically, CTC compression aims at reducing the sequence length of the input while maintaining the original content. It is based on the CTC mechanism \citep{Graves2006ConnectionistTC}, which enables the prediction of output sequences of variable length shorter than the input one\mg{s}, as in the case in which the \mg{transcript sub-words are predicted from audio frames.}
%source audio frames are used to predict the transcript sub-words.
For each input time step, the CTC produces a probability distribution over the possible target labels augmented with a \texttt{<blank>} symbol.
This probability distribution is leveraged by the CTC compression (here applied to the 8\textsuperscript{th} layer) to collapse contiguous vectors corresponding to the same label, padding included, by averaging them.
As we will see, this characteristic leads experiments with the flawed code to yield very positive results, contradicting those of other recent works on Transformers \citep{liu2020bridging,gaido-etal-2021-ctc} indicating that CTC compression greatly reduces training and inference costs but brings limited gains in terms of output quality. In the following, we will examine whether these improvements are also valid for the correct Conformer implementation.
}

% \mg{To showcase how the development of new solutions on top of incorrect code can result in misleading conclusions, we evaluate the \sara{effects on both correct~(\smallcorrect) and flawed~(\bugall) code of a popular technique, named CTC compression, applied to the Conformer encoder.
% CTC compression aims at reducing the sequence length of the input
%%, padding included, 
%while maintaining the original content and has been shown to greatly reduce training and inference costs while bringing limited gains in terms of output quality 
%%(BLEU) 
% in 
%%the 
% Transformer architecture\sara{s} \citep{liu2020bridging,gaido-etal-2021-ctc}. It is based on the}
% \mg{%The CTC \sara{mechanism} \citep{Graves2006ConnectionistTC}\sara{, which} enables the prediction of an output sequence of variable length 
%%that is 
% shorter than the input one, as in the case in which the source audio frames are used to predict the transcript sub-words. For each input time step, the CTC produces a probability distribution over the possible target labels augmented with a \texttt{<blank>} symbol\sara{.} %,
%%and the CTC compression (which we apply on top of the 8th of 12 encoder layers) 
% \sara{This probability distribution is leveraged by the CTC compression (here applied to the 8\textsuperscript{th} layer)}
%%leverages these predictions 
% to collapse contiguous vectors corresponding to the same label\sara{, padding included,} by averaging them.}
%%introduction of a \mg{technique that effectively reduces the sequence length (and consequently the amount of padding)}
%%%simple technique --  CTC compression (or shrinking) mechanism \citep{liu2020bridging,gaido-etal-2021-ctc} -- 
%%into the Conformer encoder both with correct and flawed code. 
%%By
% \sara{In} doing this, \sara{the impact of padding on the output is inevitably reduced and, for this reason, we hypothesize that the presence of bugs can unfairly reward its integration.}
%%\mg{we hypothesize that the presence of bugs may unfairly reward its integration, because of the reduced impact of padding on the outputs.}
% }
%%aim at investigating whether the presence of bugs can influence the conclusions about the effectiveness of such integration.}
%%\mg{Specifically, we investigate the effect of the CTC compression (or shrinking) mechanism, which has been shown to greatly reduce training and inference costs while bringing limited gains in terms of output quality (BLEU) in the Transformer architecture \citep{liu2020bridging,gaido-etal-2021-ctc}.}
...
On average 
%over 
on all the language directions, the improvements
%brought 
achieved
...
in the ST task,
as the introduction of CTC compression appears to significantly improve the translation quality %,
