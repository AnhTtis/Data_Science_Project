...
Kumar et al.~\cite{lpft} showcase how the linear probing followed by fine-tuning strategy effectively balances performance across out-of-distribution and in-distribution tasks. We employ a similar technique to calibrate the $\boldsymbol{\theta}_{pet}$'s adaptation speed relative to $\boldsymbol{\phi}_{new}$.
Specifically, at the beginning of the training, we only learn $\boldsymbol{\phi}_{new}$ with $\boldsymbol{\theta}_{pet}$ frozen; then after $\boldsymbol{\phi}_{new}$ has sufficiently learned and the loss has significantly decreased, we jointly learn both $\boldsymbol{\phi}_{new}$ and $\boldsymbol{\theta}_{pet}$.
