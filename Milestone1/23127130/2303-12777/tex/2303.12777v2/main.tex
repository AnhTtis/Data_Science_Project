...
\title{Viscous heat backflow and temperature resonances in graphite}
...
Microscopic theoretical analyses suggested that these phenomena emerge when 
heat behaves analogously to a viscous fluid, and raised fundamental questions on how to control and technologically exploit them.
Here, we rely on the viscous heat equations [Phys. Rev. X 10, 011019 (2020)] parametrized from first principles to 
shed light on the fundamental physics governing signatures of viscous heat flow such as steady-state heat vortices and damped temperature waves, also rationalizing recent transient experiments. We discuss strategies to detect and control viscous heat hydrodynamics, suggesting that heat vortices can be detected in thermal-imaging experiments, and viscous temperature waves can be driven to resonance and amplified. 
\end{abstract}

%Graphite's extreme ability to conduct heat is technologically relevant for a variety of applications in electronics and thermal management, and scientifically intriguing for violating Fourier's law. Striking non-diffusive phenomena such as temperature waves---where heat transiently backflows from cooler to warmer regions---have recently been observed in experiments. Microscopic theoretical analyses suggested that these phenomena emerge when momentum-conserving phonon collisions dominate over momentum-relaxing ones,  hinting that under these circumstances heat can behave hydrodynamic-like, and raising fundamental questions on the existence, possible practical detection, and technological exploitation of macroscopic hallmarks of viscous heat flow. Here we rely on the viscous heat equations [Phys. Rev. X 10, 011019 (2020)] parametrized from first principles to show that viscous heat backflow can emerge in graphite. We predict the appearance of heat vortices in the steady-state regime, and we rationalize recent measurements of temperature waves. We discuss analogies and differences between the temperature waves emerging from the viscous heat equations and from the inviscid dual-phase-lag equation, showing that it is necessary to consider viscous effects to quantitatively rationalize experiments. Finally, we show that viscous temperature waves can be driven to resonance, proposing setups for their amplification and detection.  

%In this work, we shed light on the fundamental physics determining the emergence of both steady-state and time-dependent viscous heat hydrodynamics, quantitatively rationalizing the appearance of hydrodynamic deviations from Fourier’s diffusive law in transient experiments performed in natural-isotopic-concentration graphite [Huberman et al., Science 364 (2019); Jeong et al., Phys. Rev. Lett. 127 (2021)] and in isotopically purified graphite [Ding et al., Nat. Commun. 13 (2022)]. Importantly, we show that the non-diffusive, transient hydrodynamic temperature waves observed in the above experiments can be driven to resonance and amplified; thus, we propose a strategy to amplify and control these hydrodynamic excitations. Moreover, we predict the emergence of hydrodynamic deviations from Fourier’s law also in the steady-state regime, suggesting the possibility to detect non-diffusive, vortical heat flow via a steady-state temperature measurement in a tunnel-chamber graphitic device.
...
In particular, we show that in the limit of vanishing viscosity the VHE reduce to the dual-phase-lag equation\cite{joseph_heat_1989,tzou_unified_1995} for temperature waves (DPLE), which also encompasses Cattaneo's second-sound\cite{cattaneo1958form} equation as a special case.
However, we show that transient heat backflow is significantly affected by the thermal viscosity, and that it is necessary to account for the thermal viscosity to rationalize the hydrodynamic relaxation timescales\cite{Jeong2021} and lengthscales\cite{Huberman2019,Ding2022} measured in recent experiments.
Finally, we discuss how temperature waves can be driven to resonance, and the consequent amplification can facilitate their measurement.
...
\end{align}
\end{widetext}
In these equations, $T({\bm r}, t)$ is the local temperature, $\bm{u}({\bm r}, t)$ the local drift-velocity, and $\dot{q}({\bm r}, t)$ accounts for the space- and time-dependent energy exchange with an external heat source. $T({\bm r}, t)$ and $\bm{u}({\bm r}, t)$ emerge from the conservation of energy and quasi-conservation of crystal momentum in microscopic phonon collisions in the hydrodynamic regime, respectively \cite{Simoncelli2020}. The term ``quasi-conservation'' is used for crystal momentum because momentum-dissipating Umklapp collisions are always present in real materials at finite temperature---in practice the magnitude of hydrodynamic effects depends on the relative strength between normal and Umklapp collision---and the presence of Umklapp collisions is taken into account by the dissipative term $-\gamma^{ij}u^j({\bm r}, t)$.
The thermal conductivity $\kappa^{ij}$ and viscosity $\mu^{ijkl}$ quantify the response of the crystal to a perturbation of temperature and drift-velocity, respectively~\cite{Simoncelli2020}. The coupling coefficients\footnote{In order to simplify the notation, here we defined $\alpha^{ij}=W_{0j}^i\sqrt{\overline T A^jC}$, $\beta^{ij}=\sqrt{\frac{CA^i}{\overline T}} W_{i0}^j$, $\gamma^{ij}=\sqrt{A^i A^j} D_U^{ij}$, where $\overline{T}$ is the temperature at which thermal conductivity and viscosity are determined, $W_{0j}^i$ is the velocity tensor arising from the non-diagonal form of the diffusion operator in the basis of the eigenvectors of the normal part of the scattering matrix discussed in Ref.~\cite{Simoncelli2020}, and $D_U^{ij}$ the Umklapp-dissipation timescale discussed in Ref.~\cite{Simoncelli2020}.} $\alpha^{ij}$ and $\beta^{ij}$ originate from the relation between energy and crystal momentum for phonons. All these parameters are determined exactly from the LBTE (\textit{i.e.} accounting for the actual phonon band structure and full collision matrix) and with first-principles accuracy, details are reported in the Supplementary Material (SM) and in Ref.~\cite{Simoncelli2020}. Finally, we recall that in the VHE framework the total heat flux ($\bm{Q}^{TOT}$) is determined by temperature gradient and drift velocity\cite{Simoncelli2020}, \textit{i.e.}, $\bm{Q}^{TOT}{=}\bm{Q}^{\delta}{+}\bm{Q}^{D}$ where ${Q}^{\delta,i}{=}{-}\sum_j \kappa^{ij}\nabla^j T$ and ${Q}^{D,i}{=}\sum_j\alpha^{ij}u^j$.

The VHE encompass Fourier’s law and temperature waves \cite{joseph_heat_1989,tzou_unified_1995} as special limiting cases.
Specifically, it can be shown that in the limit of strong crystal-momentum dissipation and negligible viscous effects ($\mu_\mathrm{max}{\to}0$ and $[\gamma_\mathrm{max}]^{-1}{\to}0$, where $\mu_\mathrm{max}$ and $\gamma_\mathrm{max}$ are the maximum component of the viscosity and Umklapp dissipation tensors) the VHE yield Fourier's diffusive and trivially irrotational behavior \cite{Simoncelli2020}.
In contrast, we show in Sec.~\ref{sec:DPLE_derivation} of the SM that in the time-dependent regime and inviscid limit ($\mu{=}0$) the VHE reduce to the DPLE \cite{joseph_heat_1989,tzou_unified_1995} for temperature waves (while in the steady-state the DPLE trivially yields Fourier's diffusive behavior\cite{tzou_unified_1995}).
Both the known Fourier and DPLE special limiting cases are obtained when viscous effects are negligible. In the following we use first-principles calculations to parametrize the VHE for natural and isotopically purified graphite, thus to explore how viscosity affects the emergence of non-diffusive behavior both in the steady-state and transient domain.

\begin{figure*}
\vspace*{-7mm}
	\centering
	\includegraphics[width=0.75\textwidth]{fig2.png}\\[-3mm]
	\caption{\textbf{Transient hydrodynamic heat backflow and lattice cooling.}
	We show the VHE predictions for the relaxation in time of a temperature perturbation (obtained applying a localized heater to a device in the time interval from 0.0 to 0.4 ns) in a graphitic device thermalised at 80 K the boundaries (thermalisation occurs in shaded regions, see SM~\ref{sec:boundary_conditions_for_time_dependent_simulations}).
...
Starting from the steady-state regime, we investigate in Fig.~\ref{fig:1_vortex} how viscosity affects thermal transport by comparing the numerical solution of Fourier's (inviscid) equation (panel \textbf{a}) with that of the VHE (panel \textbf{b}). We consider a graphitic device having tunnel-chamber geometry, \textit{i.e.} a geometry that promotes vortical hydrodynamic behavior\cite{e-vortexes}. 
We highlight that the VHE temperature profile in the circular chamber is reversed compared to the temperature profile in the tunnel, a behavior completely opposite to that predicted by Fourier's law. Panel \textbf{c} shows that this temperature inversion---which in principle can be detected in thermal-imaging experiments\cite{menges_nanoscale_2016,cheng_battery_2022,cahill_nanoscale_2014,braun_spatially_2022,ziabari_full-field_2018}---occurs in the presence of viscous vortical flow, specifically as a consequence of heat backflowing against the temperature gradient.
...
Then, if we consider the inviscid limit ($\mu^{ijkl}{=}0\;\forall \; i,j,k,l$), Eq.~(\ref{viscous_heat_U}) for an isotropic material such as graphite in the in-plane direction (hereafter tensor indexes will be omitted for tensors that are proportional to the identity in the in-plane directions of graphite, see SM.~\ref{sec:parameters_entering_in_the_viscous_heat_equations}) reduces to 
...
Such an oscillatory behavior is in sharp contrast with that predicted by Fourier's diffusive equation (see Fig.~\ref{fig:figFourier} in SM~\ref{sec:effects_of_viscosity_on_lattice_cooling}), whose smoothing property \cite{skinner_university_nodate} implies that the evolution of a smooth, positive temperature perturbation relaxes to equilibrium 
...
This also shows that while steady-state hydrodynamic heat backflow can emerge exclusively as a consequence of a finite thermal viscosity, time-dependent hydrodynamic backflow (\textit{i.e.}, temperature oscillations) do not necessarily require viscosity to appear. Nevertheless, we show in SM~\ref{sec:comparison_between_theory_and_experiments_for_lattice_cooling} that accounting for a finite thermal viscosity is necessary to obtain quantitative agreement between the relaxation timescales predicted by the VHE and those measured in recent experiments~\cite{Jeong2021}.

\begin{figure}[b]
\includegraphics[width=\columnwidth]{fig3a.png}
\includegraphics[width=0.9\columnwidth]{fig3b.pdf}\\[-3mm]
...
We highlight how the analysis in Fig.~\ref{fig:rect_freq} improves upon previous studies based on the inviscid DPLE\cite{barletta_hyperbolic_1996,xu_thermal_2002,xu_thermal_2021} by
providing insights into how the resonant behavior of temperature waves is affected by thermal viscosity.
Finally, the inset shows that reducing isotopic-mass disorder in graphite yields a stronger VHE resonant response. %We reiterate that the theoretical predictions in Fig.~\ref{fig:rect_freq} relied on realistic parameters for natural or isotopically purified graphite, and therefore can be checked experimentally.

Next, we systematically investigate how the maximum VHE and DPLE resonant amplification vary as a function of device size, average temperature, and isotopic disorder. 
...
This value was estimated considering the largest grains in Fig.~S3 of Ref.~\cite{Huberman2019}, Fig.~S1 of Ref.\cite{Jeong2021}, and Fig.~S11 of Ref.~\cite{Ding2022}, which are all in broad agreement.} and equilibrium temperature, computing for every simulation the maximum resonant amplification as ${\rm max}_f[a(f)]/a_0$ (where $a_0{=}\lim_{f{\to} 0}a(f)$, see Fig.~\ref{fig:rect_freq}).
Fig.~\ref{fig:freq_VHE_vs_DPLE}\textbf{a} shows that the temperatures and lengthscales at which the VHE predict the emergence of hydrodynamic resonant behavior in natural samples are in broad agreement with the temperatures and lengthscales for the emergence of heat hydrodynamics discussed by \citet{Huberman2019}. In contrast, Fig.~\ref{fig:freq_VHE_vs_DPLE}\textbf{b} shows that the DPLE fails to capture the reduction of hydrodynamic behavior observed in experiments in natural samples as temperature is decreased below 100 K.
Turning our attention to isotopically pure samples, Fig.~\ref{fig:freq_VHE_vs_DPLE}\textbf{c,d} show that the resonant response is stronger compared to natural samples, and it also persists up to larger lengthscales and slightly higher temperatures. Experiments\cite{Ding2022} for isotopically pure samples are available only at temperatures higher than 100 K, preventing us to compare VHE and DPLE with experiments in the small-size-and-low-temperature region, where VHE and DPLE display the most significant differences. In the high-temperature-and-small-size limit, where both the VHE and DPLE show a qualitatively similar reduction of the hydrodynamic signatures, experiments show a qualitative behavior similar to that obtained from the VHE and DPLE. 

In summary, we have shown analytically that the VHE\cite{Simoncelli2020} generalize the DPLE\cite{joseph_heat_1989,tzou_unified_1995} by accounting for heat-viscosity effects. We focused on graphite with natural and reduced isotopic-mass disorder, evaluating from first principles all the parameters entering in the VHE and DPLE and showing that it is crucial to account for heat-viscosity effects to rationalize the hydrodynamic relaxation timescales\cite{Jeong2021} and lengthscales\cite{Huberman2019,Ding2022} measured in recent experiments.
We have provided novel, fundamental insights on how the thermal viscosity affects the resonant behavior of temperature waves, an effect which was not considered in past works based on the inviscid DPLE\cite{barletta_hyperbolic_1996,xu_thermal_2002,xu_thermal_2021}.
Additionally, we have shown that the heat viscosity can give rise to striking hydrodynamic deviations from Fourier's law also in the steady-state regime, predicting the emergence of heat vortices and backflow in a tunnel-chamber graphitic device and suggesting their detection via a temperature measurement. 
Overall, this work sheds light on the fundamental physics determining the emergence of viscous heat hydrodynamics both in the steady-state and time-dependent domain, also proposing experimental setups for the detection and control (amplification) of heat hydrodynamic phenomena. These findings will potentially be useful for the technological exploitation of heat hydrodynamics in next-generation electronic and phononic devices.
...
\begin{figure*}[!htb]
\includegraphics[width=0.9\textwidth]{sf1.pdf}\\[-3mm]
...
\begin{figure}[htbp!]
\includegraphics[width=\columnwidth]{sf2.pdf}
...
\begin{figure}[b]
\includegraphics[width=\columnwidth]{sf3.pdf}\\[-4mm]
...
\begin{figure}[b]
\includegraphics[width=\columnwidth]{sf4a.pdf}
\includegraphics[width=\columnwidth]{sf4b.pdf}\\[-4mm]
...
We show in \ref{fig:tunnel_cont} that the temperature inversion due to viscous heat backflow is maximized at average temperatures around 50-70 K and at sizes around 2$\mu m$, and  in isotopically pure samples (99.9 \% $^{12}$C, 0.1 \% $^{13}$C) persists up to temperatures slightly higher than those observed for natural samples (98.9 \% $^{12}$C, 1.1 \% $^{13}$C). We note finite-size effects on the parameters entering in the VHE have been taken into account following the approach detailed in Ref.~\cite{Simoncelli2020} and considering a grain size equal to the diameter of the radius of the inner chamber.\\
...
\newpage
\begin{figure}[t!]
\includegraphics[width=0.92\columnwidth]{sf5.pdf}
...
The expression for the smoothed rectangular domain employed in Figs.~\ref{fig:fig2},\ref{fig:rect_freq},\ref{fig:freq_VHE_vs_DPLE} is:  
\begin{equation*}
...
\begin{equation}
	\Tilde{r}(x,y)=\left[\left(\frac{|x|}{a_{\rm out}}\right)^{\frac{2a}{\lambda }}+\left(\frac{|y|}{b_{\rm out}}\right)^{\frac{2b}{\lambda }}\right]^{1/p},
	\label{eq:dist}
\end{equation}
...
The value of $p$ in Eq.~(\ref{eq:dist}) affects how the sigmoid function goes from zero to one, and the aforementioned value $p=20$ was chosen after checking that the overall transition was sufficiently smooth for our purposes (Fig.~\ref{fig:sig_3d}). In Sec.~\ref{ssec:dependence_of_lattice_cooling_from_boundary_conditions} we investigate how the thermalisation lengthscale, $a_{\rm out}{-}a_{\rm in}$, affects lattice cooling (this investigation will be done varying only $a_{\rm out}$, $b_{\rm out}$ in the above equations and keeping all the other parameters fixed).

\begin{figure}[t!]
\includegraphics[width=0.92\columnwidth]{sf6.pdf}
\caption{\label{fig:sig_3d}
\textbf{Sigmoid function to model realistic thermalisation in rectangular geometry.} Green (outside the dahsed-lime line) is  the perfectly thermalised region, where the sigmoid function $f\big(d(\Tilde{r})\big)=1$, and the thermal reservoir ensures that temperature is perfectly constant and the drift velocity is zero. Color red (inside the dashed-orange line) represents the inner "free" region where $f\big[d(\Tilde{r})\big]=0$ and the evolution of temperature and drift velocity is determined by the VHE.}
\end{figure}
...
\begin{figure}[htbp!]
\includegraphics[width=\columnwidth]{sf7.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.47\textwidth]{sf8a.pdf}
\includegraphics[width=\columnwidth]{sf8b.pdf}
...
When we used a simulation setup similar to the experiments of Ref.~\cite{Jeong2021}, \textit{i.e.} $\sigma_x=6 \mu m$ in Eq.~(\ref{eq:pert_LC}) and thermalised boundaries separated by a large distance (80 $\mu m$), we did not find temperature oscillations (LCS=0 in the point highlighted with the white cross in the upper panel of Fig.~\ref{fig:iso_pure}), in agreement with the experiments of Ref.\cite{Jeong2021}.
...
\subsection{Inviscid (DPLE) relaxation: transient heat backflow}
\begin{figure}[b]
	\centering
	\includegraphics[width=0.49\textwidth]{sf9.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\columnwidth]{sf10.png}
	\caption{\textbf{Temporal evolution of a localized temperature perturbation according to Fourier's law.} The left column presents the temperature profile, while the right column shows the heat flux. Each row is a snapshot of the sample taken at the time indicated on the left.}
	\label{fig:figFourier}
\end{figure}


\subsection{Comparison between VHE, DPLE and Fourier's relaxation}
\label{sub:inviscid}
...
In this section we simulate the ring-shaped perturbation representing the experimental setup by \citet{Jeong2021}.
...
\begin{figure}[b]
\includegraphics[width=\columnwidth]{sf12.pdf}
...
\bibitem [{\citenamefont {Zhang}\ \emph {et~al.}(2022)\citenamefont {Zhang},
  ...
  }\textbf {\bibinfo {volume} {132}},\ \bibinfo {pages} {085103} (\bibinfo
...
\bibitem [{\citenamefont {Zhang}\ and\ \citenamefont
  ...
  }\href {https://doi.org/10.1016/j.ijheatmasstransfer.2021.121847} {\bibfield
  {journal} {\bibinfo  {journal} {International Journal of Heat and Mass
  Transfer}\ }\textbf {\bibinfo {volume} {181}},\ \bibinfo {pages} {121847}
...
\bibitem [{Note5()}]{Note5}%
  ...
  {Huberman2019}, Fig.~S1 of Ref.\cite {Jeong2021}, and Fig.~S11 of Ref.~\cite
  {Ding2022}, which are all in broad agreement.}\BibitemShut {Stop}%
