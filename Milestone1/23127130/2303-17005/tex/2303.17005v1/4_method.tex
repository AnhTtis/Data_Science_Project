...
\subsection{Keyframe Selection Strategy}
...
When we reach the maximum number of keyframes (e.g., 15 keyframes), the algorithm will perform a marginalization similar to the one in S-MSCKF~\cite{S-MSCKF_Sun_2018}. 
While the standard MSCKF \cite{MSCKF_Mourikis_2007} uses the feature measurements in several poses (one-third of slide window evenly spaced), we will only use the feature measurements in two keyframes (the oldest one and the second latest) for the EKF update to reduce the computational cost.
We also implemented another change during marginalization.
Instead of removing the poses that were used in marginalization from the state matrix, our method will keep the second latest pose even though it was used for marginalization.
The second latest pose is saved because of two reasons.
First, any new features detected in the image attached to this pose will have minimum impact on the update because these features only have one measurement when doing the marginalization.
However, keeping these features and the pose will allow us to use them in future time steps for feature updates.  
