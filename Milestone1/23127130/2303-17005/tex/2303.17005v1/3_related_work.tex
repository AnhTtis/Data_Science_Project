...
%%% related 1: DVL-fused(e.g. velocity, pointcloud) 
In underwater applications where the vehicle is close to the target (e.g. seafloor or sea-ice), DVL could obtain bottom lock to provide 3-axis velocity measurements, which is commonly fused with inertial navigation system (INS) for underwater navigation. 
To increase localization accuracy, INS/DVL integrated with pressure measurements in a tightly-coupled EKF framework for under-ice navigation was presented in ~\cite{INS-DVL-Pressure_Zhao_2022}. 
In addition, acoustic beacon aided INS/DVL solutions using LBL~\cite{INS-DVL-LBL_Miller_2010} and USBL~\cite{INS-DVL-USBL_Li_2023} have existed for years to bound the odometry drift.
...
Even though there is a significant SLAM research done in the marine robotics community, the gap in visual-based method still exists due to the challenges such as  poor image quality and limited on-board processing capability.
...
Yet, these methods are computationally expensive that are not realistic to run online on an AUV. 
On the other hand, VO is capable of processing images at high frame rates (e.g. 10-20Hz), and the uses of monocular camera and stereo cameras have been investigated for underwater scenarios~\cite{Mono-VO-Mosaic_Gracias_2003,Mono-VO-Turbid_Ferrera_2019,Stereo-VO_OpticalFlow_Corke_2007,Stereo-VO-Selective_Bellavia_2017}.
...
For example, the pressure sensor can be used to aide the VIO algorithms using filter-based and optimization-based methods~\cite{VIPO-Filter_Shkurti_2011} and~\cite{VIPO-Optimization_Hu_2022}.  
...
Similar to our method presented in this paper, DVL velocity fused VIO are developed in ~\cite{DVIO-HullInspection_Kim_2013, DVIO-Habor_Vargas_2021,DVIO-DeepSea_Chavez_2019} for hull inspection, harbor exploration and deep-sea operation. 
...
To our best knowledge, there is limited research in using sparse point cloud for Visual SLAM.
