...
We use the same layout classification model as LayoutGan++~\cite{layoutganpp} and UniLayout~\cite{unilayout}. Specifically, the model includes an encoder and a decoder, both of which are based on the Transformer architecture.
...
Our evaluation results using this model are consistent with those reported in the UniLayout.
...
SelfSim$\downarrow$ & & LayoutTransformer & UniLayout & Diffusion-LM & D3PM (absorbing) & D3PM (uniform) & LayoutDiffusion\\
\midrule
...
\noindent \textbf{Quantitative results.} As shown in~\cref{Tab:refine_table}, for the two baselines (RUITE~\cite{rahman2021ruite} and UniLayout~\cite{unilayout}), the models exhibit favorable performances when dealing with noise levels less than or equal to the training level (std.=0.005 and std.=0.01).
...
We compare to the baseline with the best quality performance (i.e., UniLayout~\cite{unilayout}), which can generate multiple layouts with top-k sampling~\cite{fan-etal-2018-hierarchical}. LayoutDiffusion can generate multiple layouts by simply running the inference process multiple times.
...
\noindent \textbf{Quantitative results.} The quantitative results is given in~\cref{Tab:type_table}. Compared to UniLayout, LayoutDiffusion performs significantly better in diversity (as suggested by SelfSim), while achieving comparable quality performance (as suggested by Overlap and Align.).
...
\noindent \textbf{Qualitative results.} As show in~\cref{type suppl}, despite equipped with top-k sampling, UniLayout still suffers severe diversity problem (duplication occurs in the first three row and the last row. Besides, the generated layouts in the fourth row share similar patterns). 
...
\begin{figure}[th]
  ...
  \vspace{-10px}
   ...
   \caption{Qualitative comparison of diversity on RICO. Each type set corresponds to five samples from UniLayout (left) and five samples from LayoutDiffusion (right). For more qualitative results of LayoutDiffusion on Gen-Type, please refer to~\cref{more gen-type}.}
   \label{type suppl} 
\end{figure}
...
In addition to the ablation studies on unconditional layout generation discussed in the main paper (see~\cref{Tab:ablation_table}), we also conducted ablation studies with the variations on two conditional generation tasks, i.e., Gen-Type and Refinement, to further investigate the effectiveness of our design.
