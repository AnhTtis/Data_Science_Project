...
Similarly, the expansion factor is constant on astrophysical scales~\cite{He:2017alg}.
...
It is perhaps noteworthy that in GR, the components of the energy-momentum tensor are closely related with the referred basis $\{\mathrm{d} x^{\mu}\}$.
...
&=&-\int\frac{\rho_\mathrm{m}(\vec{r^\prime})}{\mid \vec{r}-\vec{r^\prime}\mid}\mathrm{d} \vec{r^\prime}-\left(\frac{r_{\mbox{o}}}{r}\right)^{3w+1},
