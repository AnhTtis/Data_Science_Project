...
\ee
It follows that the achievable accuracy over the population is determined by the extent to which  $\frac{ \tntensor }{ \norm{\tntensor} }$ can fit the normalized population data tensor \smash{$\frac{ \popdatatensor }{ \norm{\popdatatensor} }$}.
...
Fortunately, as~\cref{prop:data_tensor_concentration} below shows, the entanglements of $\popdatatensor$ under all partitions are with high probability well-approximated by the entanglements of the empirical data tensor $\datatensor$.
...
\begin{proposition}
	...
	If the training set size $M$ satisfies $M \geq \frac{128 \ln ( \frac{2}{\delta} )( \ln ( \max_{\K' \subseteq [N]} \D_{\K'} ))^{4}}{\norm{\popdatatensor}^{2} \gamma^{4}}$, where $D_{\K'} := \min \brk[c]{ \prod_{n \in \K'} D_n , \prod_{n \in \K'^c} D_n }$ for $\K' \subseteq [N]$, then with probability at least $1 - \delta$:
	\[
	\abs*{ \qe{\datatensor}{\K} - \qe{\popdatatensor}{\K} } \leq \gamma ~~ \text{for all $\K \subseteq [N]$}
	\text{\,.}
	\]
\end{proposition}

\begin{proof}[Proof sketch (proof in~\cref{app:proofs:data_tensor_concentration})]
	A standard generalization of the Hoeffding inequality to random vectors in a Hilbert space allows bounding $\norm{\datatensor - \popdatatensor}$ with high probability.
	Bounding differences between entanglements via Euclidean distance concludes the proof.
\end{proof}

\begin{corollary}
	\label{cor:eff_acc_pred_nec_and_suf}
	Consider the setting and notation of~\cref{cor:acc_pred_nec_and_suf}, with $\epsilon \neq 0$.
	For $\delta \in (0, 1)$, suppose that the training set size $M$ satisfies $M \geq \frac{128 \ln ( \frac{2}{\delta} )( (16 N - 24) \ln ( \max_{\K' \subseteq [N]} D_{\K'} ))^{4}}{\norm{\popdatatensor}^{2}\brk{ \ln (R) \cdot \epsilon^2}^4}$.
	Then, with probability at least $1 - \delta$ the following hold.
	First, if there exists a canonical partition $\brk{ \K, \K^c} \in \can$ (\cref{def:canonical_partitions}) under which $\qe{\datatensor}{\K} > (1 + \frac{\epsilon^{2}}{16 N - 24} ) \cdot \ln (R) + 2 \epsilon \cdot \ln (D_\K) + 2 \sqrt{ 2 \epsilon }$, then:
	\[
	\subopt > \epsilon
	\text{\,.}
	\]
	Second, if for all $\brk{ \K, \K^c} \in \can$ it holds that $\qe{\datatensor}{\K} \leq \frac{\epsilon^{2}}{16 N - 24} \cdot \ln (R)$, then:
	\[
	\subopt \leq \epsilon
	\text{\,.}
	\]
	Moreover, the conditions above on the entanglements of $\datatensor$ can be evaluated efficiently (in $\OO (D N^2 M^2 + N M^3)$ time $\OO (D N M + M^2)$ and memory, where $D := \max_{n \in [N]} D_n$).
\end{corollary}

\begin{proof} 
Implied by~\cref{cor:acc_pred_nec_and_suf},~\cref{prop:data_tensor_concentration} with $\gamma = \frac{\epsilon^2}{16N - 24} \cdot \ln (R)$ and~\cref{alg:ent_comp} in~\cref{app:ent_comp}.
\end{proof}
...
Namely, applying convolutional neural networks, S4 (a popular recurrent neural network; see~\citep{gu2022efficiently}), and a local self-attention model~\citep{tay2021long} to different datasets, we show that the achieved test accuracy is inversely correlated with the entanglements of the empirical data tensor under canonical partitions.
...
First, select one a dataset which locally connected neural networks perform well, in the hopes that it admits low entanglement under canonical partitions; natural candidates are datasets comprising images, text or audio.
...
\begin{figure}[t]
	...
	\begin{center}
		\hspace{0mm}
		\includegraphics[width=1\textwidth]{figs/entanglement_inv_corr_acc_flipped.pdf}
	\end{center}
	\vspace{-2.5mm}
	\caption{
		...
		\textbf{Right:} Test accuracies achieved by a convolutional neural network (CNN)~\citep{dai2017very}, S4 (a popular class of recurrent neural networks; see~\citep{gu2022efficiently}), and a local self-attention model~\citep{tay2021long}, against the number of random feature swaps performed to create the dataset.
