...
The complement of $\K \subseteq [N]$ is denoted by $\K^c := [N] \setminus \K$.
...
For our purposes, a \emph{tensor} is a multi-dimensional array $\A \in \R^{D_1 \times \cdots \times D_N}$, where $N \in \N$ is its \emph{dimension} and $D_1, \ldots, D_N \in \N$ are its \emph{axes lengths}.
...
More specifically, a tensor network is a weighted graph that describes formation of a (typically high-dimensional) tensor via contractions between (typically low-dimensional) tensors.
...
\begin{figure*}[t]
	...
	\caption{
		...
		\textbf{Tensor network definition:} Every node in a tensor network is associated with a tensor, whose dimension is equal to the number of edges emanating from the node.
		...
		The number of such open edges is equal to the dimension of the tensor produced by contracting the tensor network.
		\textbf{Illustrations:} Presented are exemplar tensor network diagrams of: \textbf{(a)} an $N$-dimensional tensor $\A \in \R^{D_1 \times \cdots \times D_N}$; \textbf{(b)} a vector-matrix multiplication between $\Mbf \in \R^{D_1 \times D_2}$ and $\vbf \in \R^{D_2}$, which results in the vector $\Mbf \vbf \in \R^{D_1}$; and \textbf{(c)} a more elaborate tensor network generating $\W \in \R^{D_1 \times D_2 \times D_3}$.
	}
