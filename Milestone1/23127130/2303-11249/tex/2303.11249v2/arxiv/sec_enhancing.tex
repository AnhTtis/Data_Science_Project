...
We empirically evaluate~\cref{alg:ent_struct_search} using common locally connected neural networks~---~a convolutional neural network, an S4 (popular recurrent neural network; see~\citep{gu2022efficiently}, and a local self-attention model~\citep{rae-razavi-2020-transformers}~---~over randomly permuted audio datasets (\cref{sec:enhancing:exp:perm_audio}) and several tabular datasets (\cref{sec:enhancing:exp:tab}).
...
We demonstrate the scalability of~\cref{alg:ent_struct_search} by including in~\cref{app:experiments:further} an experiment over audio data with 50,000 features.  
