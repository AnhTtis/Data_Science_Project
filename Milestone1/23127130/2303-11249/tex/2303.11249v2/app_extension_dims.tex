...
Then, given a compatible $\axismap : [N]^P \to [N^P]$ (\cref{def:compat_map}),~\cref{thm:fit_necessary_pdim} establishes that the entanglement of $\A$ with respect to $\axismap (\K)$, where $\K$ is a canonical partition, cannot be much larger than $\ln (R)$, whereas the entanglement attainable by an arbitrary tensor can be linear in~$N^P$.
...
\begin{theorem}
	...
	In contrast, there exists $\A' \in \R^{D_1 \times \cdots \times D_{N^P}}$ such that for all canonical partitions $\brk{\K, \K^c} \in \can^P$:
	\be
	\qe{\A'}{\K} \geq \min \brk[c]*{ \abs{\K} , \abs{\K^c} } \cdot  \ln \brk{ \min\nolimits_{n \in [N^P]} D_n }
	\text{\,.}
...
\begin{proof}
The proof follows a line analogous to that of~\cref{thm:fit_necessary}.
\end{proof}

%\begin{proof}[Proof sketch (proof in~\cref{app:proofs:fit_necssary_pdim})]
%The proof follows a line identical to that of~\cref{thm:fit_necessary}.
%\end{proof}
...
\begin{proof}
	The proof follows a line analogous to that of~\cref{thm:fit_sufficient}.
\end{proof}
...
Specifically, we establish~\cref{cor:acc_pred_nec_and_suf_pdim},~\cref{prop:data_tensor_concentration_pdim} and~\cref{cor:eff_acc_pred_nec_and_suf_pdim}, which generalize~\cref{cor:acc_pred_nec_and_suf},~\cref{prop:data_tensor_concentration_pdim} and~\cref{cor:eff_acc_pred_nec_and_suf} from~\cref{sec:accurate_predict:nec_and_suf}, respectively, to $P$-dimensional model and data.
...
\begin{proposition}
	...
	If the training set size $M$ satisfies $M \geq \frac{128 \ln ( \frac{2}{\delta} )( \ln ( \max_{\J \subseteq [N^P]} \D_{\J} ))^{4}}{\norm{\popdatatensorpdim{P}}^{2} \gamma^{4}}$, where $D_{\J} := \min \brk[c]{ \prod_{n \in \J} D_n , \prod_{n \in \J^c} D_n }$ for $\J \subseteq [N^P]$, then with probability at least $1 - \delta$:
	\[
	\abs*{ \qe{ \datatensorpdim{P} }{\J} - \qe{ \popdatatensorpdim{P} }{\J} } \leq \gamma ~~ \text{for all $\J \subseteq [N^P]$}
	\text{\,.}
	\]
\end{proposition}
...
\begin{corollary}
	...
	Consider the setting and notation of~\cref{cor:acc_pred_nec_and_suf_pdim}, with $\epsilon \neq 0$.
	For $\delta \in (0, 1)$, suppose that the training set size $M$ satisfies $M \geq \frac{128 \ln ( \frac{2}{\delta} )( (16 N^P - 24) \ln ( \max_{\J \subseteq [N^P]} \D_{\J} ))^{4}}{\norm{\popdatatensor}^{2}\brk{ \ln (R) \cdot \epsilon^2}^4}$.
	Then, with probability at least $1 - \delta$ the following hold.
	First, if there exists a canonical partition $\brk{ \K, \K^c} \in \can^P$ (\cref{def:canonical_partitions_pdim}) under which $\qe{\datatensorpdim{P}}{ \axismap (\K) } > (1 + \frac{\epsilon^{2}}{16 N^P - 24} ) \cdot \ln (R) + 2 \epsilon \cdot \ln (D_{ \axismap (\K) })  + 2 \sqrt{ 2 \epsilon }$, then:
	\[
	\suboptpdim{P} > \epsilon
	\text{\,.}
	\]
	Second, if for all $\brk{ \K, \K^c} \in \can^P$ it holds that $\qe{ \datatensorpdim{P} }{\axismap (\K)} \leq \frac{\epsilon^{2}}{16 N^P - 24} \cdot \ln (R)$, then:
	\[
	\suboptpdim{P} \leq \epsilon
	\text{\,.}
	\]
	Moreover, the conditions above on the entanglements of $\datatensorpdim{P}$ can be evaluated efficiently (in $\OO (D N^{2P} M^2 + N^P M^3)$ time $\OO (D N^P M + M^2)$ and memory, where $D := \max_{n \in [N^P]} D_n$).
\end{corollary}

\begin{proof} 
	Implied by~\cref{cor:acc_pred_nec_and_suf_pdim},~\cref{prop:data_tensor_concentration_pdim} with $\gamma = \frac{\epsilon^2}{16N^P - 24} \cdot \ln (R)$ and~\cref{alg:ent_comp} in~\cref{app:ent_comp}.
\end{proof}
...
\begin{figure}[t]
	...
	\begin{center}
		\hspace{0mm}
		\includegraphics[width=1\textwidth]{figs/entanglement_inv_corr_acc_image_flipped.pdf}
	\end{center}
	\vspace{-2mm}
	\caption{
		The prediction accuracies of common locally connected neural networks are inversely correlated with the entanglements of image data under canonical partitions of features, in compliance with our theory (\cref{app:extension_dims:fit_tensor,app:extension_dims:accurate_predict}).
		This figure is identical to~\cref{fig:entanglement_inv_corr_acc}, except that the measurements were carried over a binary classification version of the CIFAR10 image dataset, as opposed to a one-dimensional (sequential) audio dataset.
		% the average entanglement includes only entanglement under canonical partitions whose sides contain at least $25\%$ of the features.
...
%Empirical evaluations demonstrate that it significantly improves prediction accuracy of wide-spread architectures on various datasets (\cref{app:extension_dims:enhancing:exp}).
...
%
%We supplement the experiments from~\cref{sec:enhancing:exp} for one-dimensional models and data, by empirically evaluating~\cref{alg:ent_struct_search_pdim} using \red{two-dimensional convolutional neural networks} over randomly permuted image data and several tabular datasets.
%For brevity, we defer some implementation details to~\cref{app:experiments:details}.
%
%\todo{Write according to experiments that we have.}
%
%\paragraph*{Randomly Permuted Image Datasets} 
%
%\cref{tab:image} presents experiments analogous to those of~\cref{tab:audio_moderate_dim} from~\cref{sec:enhancing:exp:perm_audio} over the CIFAR10~\citep{krizhevsky2009learning} image dataset.
%
%\begin{table}[t]
%	\caption{
%		\red{Arranging features of randomly permuted image data via~\cref{alg:ent_struct_search_pdim} significantly improves the prediction accuracies of locally connected neural networks.
...
%		}
%%		Reported are test accuracies (mean and standard deviation) of a convolutional neural network (CNN), an S4 model~\citep{gu2022efficiently}, and a local self-attention model~\citep{tay2021long},over the CIFAR10~\citep{krizhevsky2009learning} subject to different arrangements of features: \emph{(i)} a random arrangement; \emph{(ii)} an arrangement provided by applying~\cref{alg:ent_struct_search_pdim} to the random arrangement; and \emph{(iii)} an arrangement provided by applying an adaptation of IGTD~\citep{zhu2021converting}~---~a heuristic method designed for convolutional neural networks~---~to the random arrangement.
...
%%		As can be seen,~\cref{alg:ent_struct_search_pdim} leads to significant improvements in test accuracies, surpassing the improvements brought forth by IGTD.
...
%			\midrule
%			CNN & $\red{XX}~\pm$ \scriptsize{$\red{XX}$} & $\red{\mathbf{XX}}~\pm$ \scriptsize{$\red{XX}$} & $\red{XX}~\pm$ \scriptsize{$\red{XX}$} \\
%			S4 & $\red{XX}~\pm$ \scriptsize{$\red{XX}$} &  $\red{XX}~\pm$ \scriptsize{$\red{XX}$} & $\red{XX}~\pm$ \scriptsize{$\red{XX}$} \\
%			Local-Attention & $\red{XX}~\pm$ \scriptsize{$\red{XX}$} &  $\red{XX}~\pm$ \scriptsize{$\red{XX}$} & $\red{XX}~\pm$ \scriptsize{$\red{XX}$}  \\
%			\bottomrule
%		\end{tabular}	
%	\end{center}
%	\label{tab:image}
...
%
%\cref{tab:tabular_datasets_two_dim} presents experiments analogous to those of~\cref{tab:tabular_datasets} from~\cref{sec:enhancing:exp:tab}, evaluating~\cref{alg:ent_struct_search_pdim} via locally connected neural networks that operate over two-dimensional data (as opposed to one-dimensional data).
%
%\begin{table}[t]
%	\caption{
%		\red{Arranging features of tabular datasets via~\cref{alg:ent_struct_search_pdim} significantly improves the prediction accuracies of locally connected neural networks whose input data is two-dimensional.
%		Reported are the results of experiments identical to those of~\cref{tab:tabular_datasets}, except that...
%		For further details see caption of~\cref{tab:tabular_datasets} as well as~\cref{app:experiments:details}.
%		}
%	}
%	\begin{center}
%		\small
...
%	\end{center}
%	\vspace{-4mm}
%	\label{tab:tabular_datasets_two_dim}
%\end{table}