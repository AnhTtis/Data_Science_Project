...
Specifically, the algorithm runs in $\OO (D N M^2 + M^3)$ time and requires $\OO (D N M + M^2)$ memory, where $D := \max_{n \in [N]} D_n$ is the maximal feature dimension (\ie~axis length of $\datatensor$), $N$ is the number of features in the data (\ie~number of axis that $\datatensor$ has), and $M$ is the number of training instances.
