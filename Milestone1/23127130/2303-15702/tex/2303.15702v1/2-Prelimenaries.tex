...
$r$, the difference $D_r(q \rVert p)$ gradually converges, which means that the probability distribution of nodes' occurrences in the generated corpus has stabilized.
%
\begin{small}
\begin{equation}
\Delta D_r(q \rVert p)= |D_r(q\rVert p)-D_{r-1}(q\rVert p)|
\end{equation}
\end{small}
%
{\sf HuGE} leverages $\Delta D_r(q \rVert p) \leq \delta$ as the termination condition. Based on our experimental results, $\delta=0.001$ usually produces
