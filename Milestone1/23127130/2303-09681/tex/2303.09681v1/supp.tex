...
\title{Supplementary Material \\ Human Pose Tracking from Only Events using Spiking Spatiotemporal Transformer}
%
%
...
\section{Backpropagation in SNN}\label{sec:bp}
\begin{figure*}[ptb]
    \centering
...
\end{figure*}
\begin{strip}
\textbf{Backpropagation through time in SNNs.} Given the backpropagate gradients from the last layer $\frac{\partial \mathcal{L}}{\mathbf{s}^{(l)}_{[t]}}$, we can unfold the iterative update of membrane potential for $T$ time steps and calculate the gradients $\frac{\partial \mathcal{L}}{\partial \mathbf{s}^{(l-1)}_{[t]}}$ and $\frac{\partial \mathcal{L}}{\partial \mathbf{W}^{(l)}}$ respectively. As is analysed in~\cite{xiao2022online}, since the derivative of the Heaviside step function is 0 almost everywhere, we can detach the neuron reset operation from the computational graph and do not backpropagating gradients in this path. So we have the BPTT in SNN shown in Fig.~\ref{fig:neuron_model}~(c).
...
\section{Johnson–Lindenstrauss Lemma on Binary Embeddin}
\begin{lemma}[Johnson–Lindenstrauss Lemma on Binary Embedding~\cite{jacques2013robust,yi2015binary}]
\label{lemma:JL-Lemma}
Let $\{\mathbf{x}_i\}_{i=1}^{N}$ be set of $N$ real-valued points, define its one bit quantization of the projections,
\begin{equation}\small
    \mathbf{b}(\mathbf{x}) = \text{sign}(\mathbf{Ax}), \nonumber
\end{equation}
where $\mathbf{b}(\mathbf{x})\in \{0, 1\}^{m}$ is the binary embedding of $\mathbf{x}\in\mathbb{R}^{n}$ and $\mathbf{A}\in \mathbb{R}^{m\times n}$ is a projection matrix with each entry generated independently from the normal distribution $\mathcal{N}(0, 1)$. Given $\scriptstyle m > \frac{\log N}{\delta^2}$, for any two points among $N$, 
\begin{equation}\small
    |d_{\mathcal{H}}(\mathbf{b}_i, \mathbf{b}_j) - d(\mathbf{x}_i, \mathbf{x}_j)| \leq \delta
\end{equation}
holds true with probability at least $1-2e^{-\delta^2m}$. Here $d_{\mathcal{H}}$ is the normalized Hamming similarity
\begin{equation}\small
    d_{\mathcal{H}}(\mathbf{b}_i, \mathbf{b}_j) = \frac{1}{m}\sum_{k=1}^{m} \mathds{1}(\mathbf{b}_{ik} \neq \mathbf{b}_{jk}),
    % = \frac{1}{2}(1-\frac{\mathbf{s}_i^{\top}\mathbf{s}_j}{d_v}),
\end{equation}
and $d$ is cosine similarity
\begin{equation}\small
    d(\mathbf{x}_i, \mathbf{x}_j) =  \frac{\mathbf{x}_i^{\top}\mathbf{x}_j}{\|\mathbf{x}_i\|\|\mathbf{x}_j\|}.
\end{equation}
\end{lemma}

\section{Large-Scale Dataset for Event-based Pose Tracking}
\begin{figure*}[ptb]
    \centering
    \includegraphics[width=\textwidth]{figures/amass_all_avatars.jpg}
    \caption{\textbf{Front and Back Views of Avatars in EventAMASS dataset.} There are 13 avatars used for the synthesis of EventAMASS dataset.}
...
\begin{figure*}[ptb]
    ...
    \caption{\textbf{Example Sequence from Each Dataset.}}
    \label{fig:amass_avatars}
\end{figure*}


\textcolor{red}{multi-head attention}
\textcolor{red}{add architecture details in supp}
% references section
