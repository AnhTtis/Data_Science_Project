...
\par Let $(X,d_X)$ be a geodesic metric spaces.
...
\begin{equation*}
d_X(\gamma(ct),\eta(cs)) \leq cEd_X(\gamma(t),\gamma(s)) + C.
\end{equation*}
...
By slightly modifying their construction, we define \textit{free products of metric spaces with lattices}, 
and we obtain the following result.
\begin{theorem}\label{IFM}
Let $X$ and $Y$ be metric spaces with lattices.
...
Let $X$ and $Y$ be proper metric spaces with lattices.
If $X$ and $Y$ are symmetric geodesic coarsely convex spaces,
...
Let $e_X\in i_X(X_0)$.
We choose the \textit{base point} $e_X\in i_X(X_0)$.
We call $(X_0, i_X,e_X)$ a \textit{lattice} of $X$.
...
Then $(G, o(e_X),e_X)$ is a lattice of $X$. 
We call $(G,o(e_X),e_X)$ the \textit{G-lattice}.
\end{example}

\begin{remark}
 In the definition of the lattice in Definition~\ref{def:lattice}, we do not require that
 the index map is injective. Indeed, let $G$ be a group acting on a metric space $X$ 
 with base point $e_X$ as in Example~\ref{eg:lattice}.
 Then the orbit map $o(e_X)\colon G\to X$ is injective if and only if the stabilizer of 
 $e_X$ is trivial.
\end{remark}

Let $(X,d_X)$ and $(Y,d_Y)$ be metric spaces with lattices and
let $(X_0, i_X,e_X)$ and $(Y_0, i_Y,e_Y)$ be lattices of $X$ and $Y$, respectively.
A \textit{normal word} on $X_0\sqcup Y_0$ is a finite sequence
\begin{align*}
 w_0w_1\cdots w_n \quad (n\geq 0, w_i\in X_0\cup Y_0)
\end{align*}
...
%\begin{align*}
% W \coloneqq &\{ x_0y_0x_1y_1x_2y_2 \cdots z \mid x_i \in X_0, y_j \in Y_0, z \in X_0 \cup Y_0\} \\
%                   &\bigcup \{ y_0x_1y_1x_2y_2\cdots  z \mid x_i \in X_0, y_j \in Y_0, z \in X_0 \cup Y_0\} \\
%                   &\bigcup \{ \epsilon \}.
%\end{align*} 
...
$\omega$ is a proper subword of $\tau$.
       ...
       Let $z$ be the initial letter of $\tau'$. First, we assume that $\omega$ is not the empty word (see Figure \ref{fig 3}).
       Then we define 
       \begin{align*}
 ...
 \item Neither \ref{item:w=t} nor \ref{item:w-sub-t}. \label{III}
       ...
       Let $z_0$ and $w_0$ be the initial letter of $\omega'$ and $\tau'$, respectively (see Figure \ref{fig 4}).
       Then we define
       \begin{align*}
...
\end{enumerate}
      ...
      \fill(4,-2.5)circle(0.06);
       ...
       Let $p=(\omega,u)$ and $q=(\omega  x_0^{\prime}y_0^{\prime}x_1^{\prime},v)$, where $\omega \in W_X$, $x_i^{\prime} \in X_0$, $y_0^{\prime} \in Y_0$, $u \in X$, and $v \in Y$.}
       \label{fig 3}
       \end{figure}
       %%%
       \begin{figure}[h]
       ...
       \draw(5.5,-3)--++(0,-1);
       \draw(5,-2.5)node[left]{$\overline{x_0}$};
...
%Next, we assume that $p$ belongs to a sheet and $q$ belongs to an edge.
%Set $p=(\omega,u)$ and $q=(\tau,z,t)$ for some $\omega,\tau \in W$, $u\in X\sqcup Y$, 
%$z\in X_0\sqcup Y_0$, and $t\in [0,1]$. Then we define
%\begin{align*}
% d_*(p,q) \coloneqq d_{X\sqcup Y}(z_0,w_0) + D_*(\omega',\tau')+ \norm{u} + \norm{v} +2.
%\end{align*}
...
%Moreover, Let $L$ and $L^{\prime}$ be distinct lattices of $X$ and let $L_Y$ be a lattice of $Y$.
%The free product $X*Y$ with respect to $L$ and $L_Y$ is not quasi-isomorphic to the free product $X*Y$ with respect to $L^{\prime}$ and $L_Y$. 
\end{remark}

%\begin{remark}
%We identify $y \in Y$ with $(\epsilon, y) \in X*Y$. 
%Then we can consider $X$ and $Y$ as the subsets in $X*Y$. 
%\end{remark}
...
Let $\lambda \geq 1, k \geq 0, E \geq 1$, and $C \geq 0$ be constants.
Let $\mathcal{L}$ be a family of $(\lambda,k)$-quasi-geodesic segments.
We suppose that $\mathcal{L}$ is symmetric and satisfies $(\mathrm{CC1})$. 
Then the following condition $\mathrm{(CC2)}^{\prime}$
implies the condition $\mathrm{(CC2)}$.
\begin{enumerate}
%\item[$\mathrm{(CC1)}$] For any $v,w \in X$, there exists a quasi-geodesic segment $\gamma \in \mathcal{L}$ with $\gamma : [0,a] \to X$, $\gamma(0)=v$ and $\gamma(a)=w$.
\item[$\mathrm{(CC2)}^{\prime}$] Let $\gamma, \eta \in \mathcal{L}$ be quasi-geodesic segments with $\gamma : [0,a] \to X$ and $\eta : [0,b] \to X$.
                              Let $\gamma(0)=\eta(0)$.
                              Then for all $t \in [0,a], s \in [0,b]$, and $c \in [0,1]$, we have that 
                              \[ d_X(\gamma(ct), \eta(cs)) \leq cEd_X(\gamma(t), \eta(s))+C/2. \]
\end{enumerate}
\end{lemma}
\begin{proof}
Let $\gamma, \eta \in \mathcal{L}$ be $(\lambda,k)$-quasi-geodesic segments with $\gamma:[0,a] \to X$ and $\eta:[0,b] \to X$.
We suppose that $\gamma(0) \neq \eta(0)$.
Set $t \in [0,a], s \in [0,b]$, and $c \in [0,1]$.
By (CC1), there exists a $\omega \in \mathcal{L}$ with  $\omega :[0,u] \to X$ such that $\omega(0)=\gamma(0)$ and $\omega(u)=\eta(s)$.
Since the family $\mathcal{L}$ is symmetric, quasi-geodesic segments $\eta^{-1}$ and $\omega^{-1}$ is in $\mathcal{L}$.
We remark that $\gamma(0)=\omega(0)$ and $\omega^{-1}(0)=\eta^{-1}(0)$. %消して
Then, by \textrm{(CC2)}$^{\prime}$, we have 
\begin{align*}
&d_X(\gamma(ct), \eta(cs)) \\
&\leq d_X(\gamma(ct), \omega(cu)) + d_X(\omega(cu), \eta(cs)) \\
&\leq cEd_X(\gamma(t),\omega(u))+C/2 + d_X(\omega^{-1}((1-c)u), \eta^{-1}((1-c)s)) \\
&\leq cEd_X(\gamma(t),\omega(u))+C/2 + (1-c)Ed_X(\omega^{-1}(u),\eta^{-1}(s)) + C/2 \\
&= cEd_X(\gamma(t),\eta(s))+ (1-c)Ed_X(\gamma(0),\eta(0)) + C .
\end{align*}
This completes the proof.
\end{proof}
Combining Lemma \ref{geodesic} and Lemma \ref{symmetric}, we obtain the following.
...
$X$ is $(1, 0, E, 2C, \mathrm{id}_{\mathbb{R}_{\geq 0}}, \mathcal{L})$-coarsely convex.
\begin{itemize}
...
\par For $a,b \in X*Y$, we define a geodesic segment from $a$ to $b$
by connecting edges and $\gamma \in \mathcal{L}$ in each sheet at the identified points, denoted by $\Gamma(a,b)$.
...
$\omega$ is a proper subword of $\tau$.
       ...
       (\omega, z, t-t_1)                   & \text{for}\ t_1 \leq t \leq t_1+1,\\
       \gamma_{S_2}(e_{S_2}, v)(t-t_2) &\text{for}\ t_2 \leq t \leq t_2+ \norm{v},
        \end{cases}
       \end{align*}
      ...
      \Gamma(a,b)(t) \coloneqq \begin{cases}
             \gamma_{S_1}(u, \overline{z})(t) & \text{for}\ 0 \leq t \leq d_*(u,\overline{z}),\\ 
       (\epsilon, z, t-t_1)                   & \text{for}\ t_1 \leq t \leq t_1+1,\\
       \gamma_{S_2}(e_{S_2}, v)(t-t_2) &\text{for}\ t_2 \leq t \leq t_2+ \norm{v},
        \end{cases}
       \end{align*}
       where $t_1=d_*(u,\overline{z})$ and $t_2=t_1+1$.
       ...
       (\epsilon, z, t-t_3) &\text{for}\ t_3 \leq t \leq t_3+1, \\
       ...
       Moreover, since $\gamma_S(y,x)=\gamma_S(x,y)^{-1}$ for any $S \in \mathcal{S}$ and any $x,y \in S$, then we have $\Gamma(b,a)=\Gamma(a,b)^{-1}$. %\Gamma(a,b)(d_*(a,b)-t)
       %%%%%%%%%%%
       % extend to edges
\begin{comment}
       Moreover, 
...
\end{comment}
       ...
       Set $a^{\prime}=( \omega l_0l_1 \dots l_{n-1}, \overline{l_n})$.
       ...
       (\omega l_0 \dots l_{n-1}, l_n, t-t_1) &\text{for}\ t_1 \leq t \leq t_1+1, \\
       \gamma_{S_2}(e_{S_2}, v)(t-t_2) &\text{for}\ t_2 \leq t \leq t_2 + \norm{v}, \\
       \end{cases}
       \end{align*}
...
% write inverse as a map \Gamma(a,b)^{-1} and \Gamma(b,a) 
% geodesic
% symmetric
% the path to the geodesic
% coordinates or edge?

\begin{example}
If $a=(ux_0y_0,x)$ and $b=(ux_0^{\prime},y)$, where $u \in W_X$ is the maximal common prefix, $x_0,  x^{\prime}_0 \in X_0$, $y_0 \in Y_0$, $x \in X$, and $y \in Y$, 
...
%\begin{equation*}
%\Gamma(a,b)(ct)=\Gamma(a,b)^{-1}((1-c)t)
%\end{equation*}
%, where $c \in [0,1]$ and $t=d_*(a,b)$.
%%%
%%%
\vspace{5cm}
%%% 
...
\fill(3,-2.5)circle(0.06);
%%%
\draw(1,0.5)node[above]{$\Gamma_1(ct)$};
\fill(1,0.5)circle(0.06);
...
\end{align*}
\par We consider case II).
In case II), we supposed that $\Gamma_1(ct) \in [p,a]$ and $\Gamma_2(cs) \in [b, \Gamma_2(s)]$,
...
&= c(2+E) d_*(\Gamma_1(t),\Gamma_2(s)) + C. 
\end{align*}
\par We consider case III).
In case III), we supposed that $\Gamma_1(ct) \in [a, \Gamma_1(t)]$ and $\Gamma_2(cs) \in [b, \Gamma_2(s)]$,
...
&=c\left(1-\dfrac{c_1}{c} \right)t + c^{\prime}_1d_*(\Gamma_1(t),\Gamma_2(s)) + c\left( 1-\dfrac{c^{\prime}_1}{c} \right)s \\
...
\end{align*}
\par We consider case IV).
In case IV), we supposed that $\Gamma_1(0)=\Gamma_2(0)$, $\Gamma_1(ct)$, and $\Gamma_2(cs)$ are on the same geodesic segment.
Then we have that
\begin{align*}
...
\end{equation*}
We remark that in general, $\Gamma_1|_{[0,t^{\prime}]}$ is not in $\mathcal{L}_{*}$.
Set $a=\Gamma_1(0)$ and $a^{\prime}=\Gamma_1(t^{\prime})$.
Let $\Gamma(a,a^{\prime}) \in \mathcal{L}_{*}$ be the geodesic segment from $a$ to $a^{\prime}$.
For simplicity, we write $\Gamma(a,a^{\prime})$ as $\Gamma^{\prime}_1$.
Since $\Gamma_1$ and $\Gamma^{\prime}_1$ are geodesic segments, we define $\Gamma^{\prime}_1$ to be $\Gamma^{\prime}_1 : [0,t^{\prime}] \to X*Y$.
There exist $S' \in \mathcal{S}$ and $p, p^{\prime} \in S'$ satisfying the following conditions:
\begin{enumerate}
\item $a^{\prime} \in S'$.
\item There exists $c_0 \in [0,1]$ such that $p=\Gamma_1(c_0t^{\prime})=\Gamma_1^{\prime}(c_0t^{\prime})$.
\item Let $\gamma_{S'}(p,p^{\prime}) \in \mathcal{L}$ be the geodesic segment from $p$ to $p^{\prime}$ with $\gamma_{S'}(p,p^{\prime}) : [0,u] \to S' $
         and let $\gamma_{S'}(p,a^{\prime}) \in \mathcal{L}$ be the geodesic segment from $p$ to $a^{\prime}$ with $\gamma_{S'}(p,a^{\prime}) :[0,v] \to S'$. 
         Then we have that
\begin{align*}
...
\end{align*}
\end{enumerate}
We remark that $\Gamma_1([0,c_0t^{\prime}]) = \Gamma^{\prime}_1([0,c_0t^{\prime}])$, $u \geq v$, and $a^{\prime}=\Gamma_1(t^{\prime})=\gamma_{S'}(p,p^{\prime})(v)$.
...
Therefore, by Proposition~\ref{Propo L} and~\ref{sgCC}, the free product
$X*Y$ is a coarsely convex space, in particular, $X*Y$ is a symmetric geodesic $(2+E,3C,\mathcal{L}_{*})$-coarsely convex
space.
\end{proof}
...
%Let $L$ and $L^{\prime}$ be distinct lattices of $X$ and let $L_Y$ be a lattice of $Y$.
%In general, by Remark \ref{wp}, the free product $X$ and $Y$ with respect to $L$ and $L_Y$ is not quasi-isomorphic to the free product $X$ and $Y$ with respect to $L^{\prime}$ and $L_Y$.
%For any lattices of $X$ and $Y$, free products $X*Y$ are symmetric geodesic coarsely convex spaces.
For any lattices of $X$ and $Y$, the free product $X*Y$ is a symmetric geodesic coarsely convex space.
\end{remark}
\end{comment}
...
Let $G$ and $H$ be groups acting properly and cocompactly on $X$ and $Y$, respectively.
Bridson and Haefliger \cite[Theorem II.11.18]{BH} construct a metric space on which the free product $G*H$ acts properly and cocompactly.
We briefly review their construction.
Let $\Gamma = G*H$.
Define $Z$ by 
\begin{equation*}
...
We remark that $\overline{X}$ is isomorphic to $(W_X \cup \{\epsilon\}) \times X$, where $W_X$ is the set of words of $G*H$ such that the last letter of each word of $W_X$ is in $H$,
and $\overline{Y}$ is isomorphic to $(W_Y \cup \{\epsilon\}) \times Y$, where $W_Y$ is the set of words of $G*H$ such that the last letter of each word of $W_Y$ is in $G$.
...
We remark that $\overline{Z}$ is isomorphic to the free product $X*Y$ with respect to the $G$-lattice $(G, o(e_X),e_X)$ and the $H$-lattice $(H, o(e_Y),e_Y)$, where $o(e_X)$ and $o(e_Y)$ are the orbit maps.
Therefore, we have
\begin{proposition}[{\cite[Theorem II.11.18]{BH}}]
\label{prop:GHactsonXY}
Let $X$ and $Y$ be metric spaces.
Let $G$ and $H$ be groups acting properly and cocompactly on $X$ and $Y$, respectively.
We associate $X$ and $Y$ with the $G$-lattice and the $H$-lattice, respectively.
...
\]
Here, the left-hand side is the \textit{coarse K-homology} of $X$, and
the right-hand side is the $K$-theory of 
the $C^*$-algebra $C^*(X)$, called the \textit{Roe algebra} of $X$.
...
\end{equation*}
By the definition of lattices, 
the height of the sheets that intersect $B_*(a,R)$ is in the interval $\left[n- [R]-1 , n+ [R]+1\right]$.
Since $X$ and $Y$ are proper metric spaces, by the definition of lattices,  for any bounded closed subset $K$ in each sheet, the preimage of $K$ by the index maps is a finite set.
...
As mentioned in Section \ref{S4}, the free product $G*H$ acts properly and cocompactly on $X*Y$ with respect to the $G$-lattice and the $H$-lattice.
...
this space is not coarsely embeddable into a Hibert space.
