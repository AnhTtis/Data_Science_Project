...
\begin{abstract}
   Human-Object Interaction(HOI) detection aims to localize human-object pairs and understand their interactions. Recently, Contrastive Language-Image Pre-training (CLIP) has shown great potential in providing interaction concepts for HOI detectors via knowledge distillation. However, such approaches often rely on large-scale training data and suffer from inferior performance under few/zero-shot scenarios. In this paper, we propose a novel {HOI} detection framework with {CLIP}, which efficiently extracts prior knowledge from CLIP and achieves better generalization performance. In detail, we first design a novel interaction decoder module where interaction queries are directly guided to extract informative regions in the visual feature map of CLIP via a cross-attention mechanism. This is fused with the dense feature map from detection backbone by a knowledge integration block for more accurate human-object pair detection. In addition, prior knowledge in CLIP text encoder is leveraged to generate a classifier by embedding HOI descriptions. To distinguish fine-grained interactions, we build a verb classifier from training data via visual semantic arithmetic and design a lightweight verb representation adapter. Furthermore, we propose a training-free enhancement where global HOI predictions from CLIP are exploited to further boost performance. Extensive experiments demonstrate that our method outperformed the state of the art by a large margin on various settings, e.g. +4.04 mAP on HICO-Det. The source code will be released soon.
...
Human-Object Interaction (HOI) detection, which aims to localize human-object pairs and identify their interactions, is a core task towards a comprehensive understanding of visual scenes. It has attracted increasing interests in recent years for its key role in a wide range of applications, such as assistive robots, visual surveillance and video analysis \cite{bemelmans2012socially,dee2008close,feichtenhofer2017spatiotemporal,bolme2010visual}. 
...
Thanks to the development of end-to-end object detectors\cite{detr}, recent research\cite{qpic, gen, CDN,hotr,XinpengLiu2022InteractivenessFI} has made remarkable progress on localizing and associating human-object instances in interaction. 
Nonetheless, the problem of identifying interaction concepts between human and object remains particularly challenging. Conventional strategies \cite{as-net,hotr,qpic,CDN} for interaction understanding, which simply learn a multi-label classifier, often suffer from long-tailed distributions and lacking generalization ability to unseen interaction. As such, it typically requires large-scale fully annotated data to achieve satisfactory performance, which is excessively costly.
...
\begin{figure}
     ...
     \begin{subfigure}[t]{0.23\textwidth}
         \centering
         \includegraphics[width=\textwidth]{data_efficiency.jpg}
     ...
     \begin{subfigure}[t]{0.23\textwidth}
         ...
         \includegraphics[width=\textwidth]{verb_analysis.jpg}
         \caption{Verb Long-tail Problem}
         \label{fig:frac_rare}
     \end{subfigure}
     \vspace{-0.3cm}
        \caption{\textbf{Data efficiency comparison and verb distribution Analysis.} In figure (a), we increase training data percentage from $5\%$ to $100\%$ and show the result of HOICLIP and GEN-VLKT. In figure (b), the nodes indicate the mean mAP and length of vertical line indicate the variance of mAP for a group of verb categorized by sample number.}
\label{fig:frac}
\vspace{-0.3cm}
\end{figure}
...
In particular, recent work on open-vocabulary detection utilizes knowledge distillation to transfer CLIP's instant-level object representation to object detectors\cite{vild, prompt_vild,zhao2022exploiting,feng2022promptdet,ma2022open,DBLP:journals/corr/abs-2109-12066}. Such a strategy has been adopted in the literature of HOI detection, including  GEN-VLKT\cite{gen} and EoID\cite{eoid}, which leverage CLIP's knowledge to tackle the long-tail and zero-shot learning in the HOI tasks.
...
Despite their promising performance, it remains an open question how we can transfer CLIP knowledge to the HOI recognition task in an effective manner as it involves compositional concepts composed of visual objects and potentially abstract interactions. First, as pointed out in\cite{cho2019efficacy,SamuelStanton2021DoesKD}, improving the predictive distributions between a student and teacher model is not aligned with improving generalization of the student. In addition, as shown in Figure \ref{fig:frac}, we empirically observe that the knowledge distillation in learning HOIs (e.g., GEN-VLKT) typically requires a substantial amount of training data, which indicates its \textit{low data efficiency}. Furthermore, knowledge distillation often suffer from \textit{performance degradation in zero-shot generalization} as it lacks training signal for unseen classes which is critical to inherit knowledge from the teacher model.  
...
As the feature localization is key to the detection task, we aim to fully exploit the visual representation in CLIP and extract features only from informative image regions. 
To this end, we utilize CLIP's feature map with 2D spatial dimensions, and develop a transformer-based interaction decoder module that learns a localized interaction feature with cross-modal attention.     
...
We empirically find that previous CLIP-based HOI methods still suffer from the long-tail problem in terms of verb classes  Figure \ref{fig:frac}. To address this, we develop a verb classifier focusing on learning a better representation for the verbs. Our verb classifier consists of a verb feature adapter\cite{adapter,vladapter,clipadapter,tipadapter} and a set of class weights computed via visual semantic arithmetic \cite{zero_cap}. We then enhance the HOI prediction by fusing the outputs of the verb classifier and the common interaction classifier.  
...
\setlength{\parskip}{0pt}%
    ...
    \item We develop a fine-grained transfer strategy, leveraging regional visual feature of HOIs via cross attention and specific verb representation via visual semantic arithmetic for deeper interaction understanding. 
...
Recently, several HOI methods inspired \cite{hotr,qpic,CDN,XinpengLiu2022InteractivenessFI,gen,eoid,zhong2022towards,wu2022mining,yuanrlip} by Transformer-based Detectors\cite{detr} have achieved promising performance. In particular,  
...
CDN\cite{CDN} proposes a framework with cascade decoder architectures, disentangling human-object localization and interaction classification. GEN-VLKT\cite{gen} further designs a two-branch pipeline to provide simultaneous forward process, and uses separated query for human and object instead of the unified query used in CDN. Our method builds on the top of the transformer-based HOI detection strategy and focuses on improving interaction understanding.
...
\begin{figure*}[t!]
  \centering
    \includegraphics[width=0.925\textwidth]{pipeline.jpg}
   \caption{\textbf{Architecture of HOICLIP.} Given an image, HOICLIP encodes it with a detection encoder and CLIP encoder. The instance decoder localizes human and object pair from image feature from detection encoder. The interaction decoder leverage image feature from both encoder and extract interaction representation. The verb adapter extract verb representation at the top of interaction representation.}
...
\textbf{Downstream Adaption of Visual Linguistic Model.}
Recent breakthrough in visual Linguistic Models\cite{clip, align} demonstrates its promising transfer ability to downstream tasks. The visual representations learned from natural language supervision pave the way for zero-shot and open vocabulary tasks\cite{OVDDETR, vild, prompt_vild, must, zero_cap,zhao2022exploiting,feng2022promptdet,ma2022open,DBLP:journals/corr/abs-2109-12066}. Pioneer works\cite{vild} transfer the VLM to open vocabulary object detection
 through knowledge distillation. Inspired by this idea, recent research\cite{gen, eoid} adopts the same strategy to HOI detection. Previous efforts to transfer VLM to detection tasks can be summarized into two aspects: (1) Prior knowledge integration through texts. Classifier are initialized with labels' text embedding from CLIP; (2) Feature or logtis level knowledge distillation. The learned features or logit predictions are guided to align with image feature embedded by CLIP or logtis predicted by zero-shot CLIP. In this work, we propose a novel way for transferring VLM knowledge to HOI detection tasks. Different from the above methods, we directly retrieve related information from CLIP which leads to superior performance and higher data efficiency.
 
\textbf{Zero-shot HOI Detection.} The target of zero-shot HOI detection is to detect and recognize HOI categories absent from training data. Due to the compositionality of HOIs, annotations of all possible HOI combinations are impractical. Therefore, the zero-shot HOI detection setting is important for application in real-world scenario. Previous work\cite{AnkanBansal2019DetectingHI,gupta2019no,hou2021affordance,VCL,FCL, ConsNet,peyre2019detecting} tackle such a challenge in a compositional manner, which disentangle reasoning on actions and objects during training. This makes it possible to recognize unseen $<\text{human, object, verb}>$ combination during inference. As breakthrough in visual-linguistic models\cite{clip}, recent research\cite{gen, eoid} focus on transfer knowledge from VLM to recognize unseen HOI concepts and achieve a promising performance gain on the zero-shot setting. Our work aims to explore a multi-facet strategy for knowledge transfer from VLM, which better exploits the learned VL knowledge for the zero-shot HOI task.
 
%------------------------------------------------------------------------
\section{Method}
In this section, we introduce the proposed HOICLIP for efficient CLIP knowledge transfer to HOI detection while preserving the generalization ability. We illustrate the overall architecture in Section \ref{sec:arch}, which consists of three key components. In Section \ref{sec:query}, we introduce the query-based knowledge retrieval strategy for transferring CLIP knowledge to HOI detection task. In Section \ref{sec:verb}, we present our verb representation adapter and verb classifier extraction. In Section \ref{sec:free}, we propose a training-free enhancement for HOI classification. Finally in Section \ref{sec:train}, we describe the training and inference pipeline. 
...
\label{sec:arch}
The overall architecture of our HOICLIP is illustrated in Figure~\ref{figure:pipeline}. We first adopt the transformer-based end-to-end object detector\cite{detr} to localize the humans and objects. Specifically, we use a transformer encoder to obtain a spatial image feature map $V_{d}$, %$ \in R^{H' \times W' \times C_{b}}$
followed by two transformer decoder modules to accomplish instance detection and interaction recognition, i.e. instance decoder and interaction decoder. Inspired by GEN-VLKT\cite{gen}, the instance decoder take two groups of separated query as input for human and object respectively, namely human query $Q_h$,
%$\in R^{N_q \times C_{e}}$ 
and object query $Q_o$. %$\in R^{N_q \times C_{e}}$. 
We use a novel interaction decoder module to perform interaction recognition, in which we utilize the information from the previous extracted feature map $V_d$ and from a spatial feature map $V_s$ generated by CLIP and perform a knowledge integration via a cross-attention module. Subsequently, a verb adapter extracts the action information to augment the interaction representation and recognition. A linear classifier takes the output of interaction decoder to predict the HOI category, which is further enhanced by a training-free classifier using CLIP's linguistic features.
...
\label{sec:query}
In this part, we explain the design of query based interaction knowledge retrieval in detail. 
\vspace{-2mm}
\paragraph{Zero-shot CLIP} We first revisit the zero-shot CLIP inference process for image classification with notations. CLIP extract dual-modality features by a visual encoder and text encoder. The visual encoder consist of a backbone $VisEnc(\cdot)$ and a projection layer $Proj(\cdot)$. The visual backbone extracts visual spatial feature $V_{s} \in R^{H_s \times W_s \times C_{s}}$, which is fed into the projection layer to obtain a global visual feature $V_{g}\in R^{1 \times D}$. The text encoder $TextEnc(\cdot)$ extracts global text representation $T_{g} \in R^{K \times D}$ for each category where K is the number of classes. The classification $S \in R^{1 \times K}$ is computed as follow:
\begin{align}
    &T_g = \mathrm{TextEnc}(\mathrm{T_K}),\\
    &V_g = \mathrm{Proj}(V_{s}),\ \ V_{s} = \mathrm{VisEnc}(\mathrm{I}),\\
    &S = V_g T_g^T,
\end{align}
where $T_g$ and $V_g$ are L2 normalized features and $T_K$ is the sentences describing the K categories. The matrix multiplication is equivalent to cosine similarity.

\begin{figure}[t!]
  \centering
    \includegraphics[width=0.35\textwidth]{inter_decoder2.png}
    \vspace{-0.3cm}
    \caption{\textbf{Structure of Knowledge Integration Cross Attention.} Interaction queries first go through a self attention layer. Then, it's fed into two shared cross attention layer with $V_s$ and $V_d$. The outputs are summed up and feed into a feed-forward network.}
    \label{figure:fusion}
    \vspace{-0.3cm}
\end{figure}

\vspace{-4mm}
\paragraph{Interaction Decoder with Knowledge Integration} To predict HOI category for a pair of human and object query, we generate a set of interaction queries $Q_{inter} \in R^{N_q \times C_{s}}$ by feeding the average of $O_h$ and $O_o$ to a projection layer. To fully exploit CLIP knowledge, we propose to retrieve interaction feature from CLIP which is better align with prior knowledge in classifier. In detail, we preserve the CLIP spatial feature $V_{s}$ and project detection visual feature $V_{d}$ to the same dimension as $V_{s}$: 
\begin{align}
    &Q_{inter} = (\frac{O_o + O_h}{2}) W_i + b_i \\
    &V_d' = V_dW_p + b_p
\end{align}
where $W_i$, $b_i$, $W_p$, $b_p$ are projection parameters. 

To guide interaction queries $Q_{inter} \in R^{N_q \times C_{s}}$ to explore informative region in both $V_{s}$ and $V_{d}$, we design a knowledge integration cross attention module and the architecture is showed in Figure \ref{figure:fusion}. The $Q_{inter}$ are first updated by self-attention, and then fed into a cross-attention module with $V_{s}$ and $V_{d}$ respectively and obtain two output feature. Finally, we sum up the outputs and feed it into a feed-forward network. Formally,
\begin{align}
...
\end{align}
where the $V_s$, $V_d'$ are the key and value, and $Q_{inter}$ is the query in the shared cross attention. To extract final interaction representation $O_{inter} R^{N_q \times D}$, we adopt the same projection operation as CLIP to convert the output of cross attention into the CLIP feature space as follows,
\begin{align}
...
In this way, we leverage the object and human information from instance decoder to retrieve interaction representation from the spatial feature map of CLIP and visual features from detector. This query-based knowledge retrieval design leads to efficient representation learning and strong generalization capabilities.

\subsection{Verb Representation}
\label{sec:verb}
In this subsection, we introduce a novel verb representation and a verb classifier built from CLIP features. 
...
\vspace{-4mm}
\paragraph{Visual Semantic Arithmetic} We propose to build a verb classifier through visual semantic arithmetic from CLIP. In detail, we adopt a hypothesis that the verb representation can be derived from the difference between the global visual feature of an HOI and the global visual feature of its object. The concept is illustrate in Figure \ref{figure:arithmetic}. 

Specifically, we use the smallest region covering object and human bounding boxes to indicate a HOI triplet. Then we define $\mathbb{OBJ}_j$ as a set containing all region of object with index $i$. Additionally, we use the tuple $(i,j)$ to indicate a HOI category, where $i$ and $j$ stand for index of verb and object respectively. Similarly, we define $\mathbb{HOI}_{(i,j)}$ as a set containing all regions of HOI category $(i, j)$.
For both HOI region and object region, we use the CLIP image encoder to obtain the features of image region, then use a projector to map the features into the global feature space. Formally, we define a transform function as follow:
\begin{align}
    f(R) = \mathrm{Proj}(\mathrm{VisEnc}(R))
\end{align}
The representation of $k^{th}$ verb can be computed by taking the average of HOI and object region features and then their difference: 
\begin{align}
...
\end{align}  
where $\mathrm{L2Norm}$ stands for L2 normalization operation and $E^{k,j}_{h}$, $E^{j}_{o}$ are the computed HOI and object representation.
...
\begin{figure}[t!]
  \centering
    \includegraphics[width=0.4\textwidth]{arithmetic.jpg}
    \caption{\textbf{Illustration of Visual Semantic Arithmetic} The object and HOI representations are extracted by encoding cropped regions of object and HOI. Then, verb representation is obtained by HOI representation minus object representation.}
    \label{figure:arithmetic}
    \vspace{-0.6cm}
\end{figure}

\vspace{-4mm}
\paragraph{Verb Adapter} To use the extracted verb representation for classification, we propose a light-weight adapter \cite{adapter,vladapter} module to extract verb feature $O_{verb} \in R^{N_q \times D}$ based on the interaction representation $O_{inter}$. Specifically, we adopt a three-layer MLP with weight and bias parameters $\{(W_i, b_i)\}_{i=1}^3$ as our adapter. The verb feature $O_{verb} \in R^{N_q \times D}$ is obtained as follows,
\begin{align}
&O_{verb} = \sigma(\sigma(Q_{inter}W_1 + b_1)W_2 + b_2)W_3 + b_3, \\
&S_v = O_{verb} E_v^T
\end{align}
where $\sigma$ denotes RELU operation in MLP. The verb logits $S_v$ is computed as the cosine similarity between verb feature $O_{verb}$ and verb representation $E_v$.

In this way, we leverage the prior knowledge in the visual encoder of CLIP to extract a verb classifier from training data and design a verb adapter for better verb representation. These design benefits HOI prediction through fine-grained verb information.

\subsection{Zero-shot HOI Enhancement}
\label{sec:free}
In this section, we introduce an HOI classifier generated by the prior knowledge in CLIP text encoder, which provides a training-free Enhancement for HOI classification.
%\vspace{-4mm}
%\paragraph{Prior Knowledge for HOI Classifier}

Specifically, we build a zero-shot HOI classifier by exploiting the alignment between image and natural language learned by CLIP, in which the label descriptions embedded by CLIP text encoder $TextEnc$ is used as the classifier weights. Similar to\cite{eoid, gen}, we convert each HOI category to a sentence with a hand-crafted template, ``A photo of a person [Verb-ing] a [Object]". The templates are fed into the CLIP text encoder $TextEnc$ to obtain an HOI classifier $E_{inter} \in R^{K_h \times D}$ where $K_h$ is the number of HOI categories. 
...
\begin{align}
&S_{zs} = \mathrm{TopK}(V_{g}E_{inter}^T)
\end{align}
% \begin{equation}
% logits_{zs}(i) = \left\{
% \begin{aligned}
...
The updated $S_{zs}$ is a training-free HOI prediction with high confidence, which leverages the zero-shot CLIP knowledge to benefits tail classes prediction.
...
\label{sec:train}
In this section, we present the details of the training and inference pipeline.
\vspace{-2mm}
\paragraph{Training}
During training, we obtain the training HOI logits $S_{t}$ by combining HOI prediction $S_{inter}$ and verb prediction $S_v$,
\begin{align}
&S_t = S_{inter} + \alpha * S_v
\end{align}
where $\alpha \in R$ is a hyperparameter. For bipartite matching process, we follow previous HOI detector\cite{gen,hotr,CDN,qpic} based on DETR framework and use Hungarian algorithm to assign ground truth to prediction. The matching cost consist of human and object bounding box regression loss, object classification loss, interaction-over-union loss and HOI classification loss. Auxiliary losses are used on intermediate outputs of decoder layers.

\vspace{-2mm}
\paragraph{Inference}
The zero-shot HOI prediction $S_{zs}$ is used in inference time. The inference HOI logits $S_{i}$ is obtained by,
\begin{align}
&S_i = S_{inter} + \alpha * S_v + S_{zs}
\end{align}
Follow previous methods\cite{gen}, We use the object scores $C_o$ from instance decoder to compute the HOI triplet score, which is
\begin{align}
&score^n = S_i^n + C_o^m * C_o^m
\end{align}
where $n$ is the HOI category index and $m$ is the object category index corresponding with $n^{th}$ HOI category. Finally, triplet NMS are applied to top K HOI triplet according to confidence score.

\section{Experiments}
\label{sec:exp}
In this section, we introduce a series of experimental analysis and comprehensive ablation study to demonstrate the effectiveness of our method. 
...
\textbf{Datasets.} We conduct our experiments on two public benchmarks HICO-DET\cite{hico_det} and V-COCO\cite{vcoco}. HICO-Det contains 47,776 images (38,118 for training and 9,658 for testing). The annotations of HICO-Det conclude 600 categories of HOI triplets, which formed from 80 object categories and 117 action categories. Among the 600 HOI categories, there are 138 categories with less than 10 training instances, defined as Rare, and the other 462 categoies are defined as Non-Rare.
...
\textbf{Evaluation Metric.} We follow the settings of previous work\cite{hico_det, gen,qpic,CDN,XinpengLiu2022InteractivenessFI} to use the mean Average Presicion (mAP) as evaluation metric. We define a HOI triplet prediction as a true positive example if it the following criteria are met: 1) The IoU of human bounding box and object bounding box are lager than 0.5 w.r.t, the GT bounding box; 2) the interaction category predicted is accurate.


\textbf{Zero-shot Construction.} Following previous work\cite{AnkanBansal2019DetectingHI}, we construct our zero-shot setting experiments in four manners: Rare First Unseen Combination (RF-UC), Non-rare First Unseen Combination (NF-UC), Unseen Verb (UV), Unseen Object (UO) and Unseen Combination(UC). Specifically, the UC indicates that all action categories and object categories are included during training, but some HOI triplets (i.e. combinations) are missing. Specially, under RF-UC setting, the tail HOI categories are selected as unseen classes, while the NF-UC uses head HOI categories as unseen classes. UV setting and UO setting indicate that some action categories and object categories are not concluded in training set respectively. For RF-UC and NF-UC, we select 120 HOI categories as unseen classes. For UV, HOI categories involving 12 randomly selected objects among 80 objects categories are defined as unseen classes. For UV, HOI categories involving 20 randomly selected verb categories are not given during training.
...
\end{tabular}
\vspace{-0.3cm}
\caption{\textbf{Zero-shot performance comparison with state-of-the-art methods on HICO-DET.} We use RF-UC and NF-UC to represent rare first and non-rare first unseen combination setting respectively. UO is short for unseen object setting, and UV indicates unseen verb setting. $\ast$ means only the detected boxes are used without object identity information from the detector. $\dagger$ indicate HOICLIP without training-free enhancement.} 
...
For fair comparison with previous methods \cite{gen, qpic,CDN,XinpengLiu2022InteractivenessFI}, we use ResNet-50 as our backbone feature extractor and ViT-32/B CLIP variant. The number of layers of the transformer encoder and transformer decoder are set to be 3. The numbers of queries are set to be 64. We train HOICLIP with a batch size of 16 with optimizer AdamW and a weight decay of $10^{-4}$. The total number of training epoch is set to be 90. For the first 60 epoch, we set the initial learning rate to be $10^{-4}$ and use learning rate drop for the last 30 epoch. The parameters are initialized with MS-COCO trained DETR. The CLIP model and classifiers are fixed during training. We conduct our experiments with a batch size of 8 on 2 NVIDIA A40 GPUs.

\subsection{Experiments on Partial Data}
To verify the data efficiency of HOICLIP, we reduce the training data to 50\%, 25\%, 15\% and 5\% respectively and compare our performance with state-of-the-art method\cite{gen} trained with those partial data settings. We show the performance on all HOI categories and rare HOI categories respectively in Table~\ref{tab:frac}.
% The expression rare HOI categories refers to the categories with the number of training examples less than 10. 
On each partial training data scenarios, HOICLIP significantly outperform its opponent, especially on the rare HOI categories. HOICLIP maintains a satisfactory and stable performance as the percentage of available data decreases while other method suffers a dramatic degradation due to insufficient training data. We observe HOICLIP achieves a significant performance gain more than 81.41\% on rare categories under $25\%$ fractional data scenario. The performance demonstrate efficiency and robustness of our methods for knowledge transfer.
...
\subsection{Analysis under Regular Setting} For a comprehensive demonstration of HOICLIP's characteristic, we conduct experiments on various zero-shot settings, RF-UC, NF-UC, UV and UO. We compare HOICLIP with state-of-the-art methods. The results is showed in Table~\ref{tab:zs_results}, which emphasize the superior performance of HOICLIP. Compared with GEN-VLKT \cite{gen}, we achieve a impressive +4.04 mAP gain under NF-UC settings for all categories and a +5.69 mAP improvement on rare categories under UO setting. 

To further demonstrate the effectiveness of HOICLIP, we compare performance of HOICLIP with state-of-the-art on default HOI detection setting. We show the performance on dataset HICO-DET in Table~\ref{tab:default_hico}. HOICLIP achieve competitive performance with previous methods especially a +1.87 mAP improvement in rare categories compared with GEN-VLKT. For V-COCO, the result are presented in Table \ref{tab:default_vcoco}. We outperform previous methods in Scenario 1 for a new state-of-the-art performance of 63.5 AP. The improvement are not significant as HICO-DET considering V-COCO's relatively small scale.
...
To demonstrate the characteristic of our model design, we first introduce a baseline model by following the design of GEN-VLKT but remove the knowledge distillation part, denoted as \textit{Base} in Table \ref{tab:ab_fusion}. The first modification is changing the image input feature for interaction decoder from detection visual feature $V_d$ to visual spatial feature $V_s$ from CLIP image encoder, donated as \textit{+CLIP}. We observe a improvement of 2.06 mAP improvement in rare categories. Next, we replace the conventional cross attention layer in interaction decoder to knowledge integration cross attention layer and change the input to  both $V_d$ and $V_s$, donated as \textit{+integration}. The results show \textit{+integration} outperform all existing HOI detectors and gain +1.41 mAP compared with \textit{+CLIP}. Towards the goal of deeper interaction understanding, we add a verb adapter and verb classifier from visual semantic arithmetic, denoted as \textit{+verb}. The performance is further improved to 34.54 mAP on all categories. Finally, we introduce training-free enhancement for \textit{+verb}, denoted as \textit{+free}. A gain of +0.15 mAP is acquired without additional training.
...
\begin{figure*}[!t]
    ...
    \begin{subfigure}[t]{0.2\textwidth}
         \centering
         \includegraphics[width=\textwidth]{vis-1.jpg}
         \caption{}
     \end{subfigure}
     \begin{subfigure}[t]{0.2\textwidth}
         \centering
         \includegraphics[width=\textwidth]{vis-2.jpg}
         \caption{}
     \end{subfigure}
     \begin{subfigure}[t]{0.2\textwidth}
         \centering
         \includegraphics[width=\textwidth]{vis-3.jpg}
         \caption{}
     \end{subfigure}
     \begin{subfigure}[t]{0.2\textwidth}
         \centering
         \includegraphics[width=\textwidth]{vis-4.jpg}
         \caption{}
     \end{subfigure}
     \vspace{-3mm}
...
We explore different methods to extract verb representation in the following experiments. A naive alternative is to describe human action using sentences similar to the way used in CLIP. For example, we can describe the action \textit{ride} by a sentence: ``a photo of a person riding". Having generated representation for each HOI category using CLIP, we also can obtain the representation of a verb by taking the average of all HOI representation involving the verb. For all the three approaches, we use the representation of all verbs to initialize the weight of verb adapter. We show our experiments results in Table~\ref{tab:ab_verb_weight}. We see that our visual semantic arithmetic strategy obtain a performance gain of 1.16 mAP compared with the sentence describing method and 1.45 mAP compared with the HOI average method on full classes setting.
...
In this part we discuss the choice of hyperparameters for each module. Different from the previous work, %where the hyperparameters are directly decided then evaluate on the testing set, 
we split a separated validation set, which does not appear during training, thus ensure fair and reliable results. The validation set is randomly selected from the training set and is guaranteed no unseen classes. We evaluate the choice of all hyperparameters on the validation set. In addition, due to the smaller number of training examples used in the following experiments, the performance shown in this part is different from the final results where the models are trained on full training set. 

Firstly, we ablate the choice of $S_v$ weight $\alpha$. We train our model with different $S_v$ weights and find the weight of $0.5$ obtain the best performance. The results are shown in Table~\ref{tab:hyper_choice}. We also decide the value of $k$ used in training-free enhancement in the same way. 
% We try the different $k$ values in $\{0, 5, 10, 15, 20, 25\}$, where $k=0$ indicates that the training-free enhancement is not used. 
We find that $k=10$ achieve the best performance. All results are shown in Table~\ref{tab:hyper_choice}.
...
\end{tabular}
\vspace{-0.3cm}
\caption{\textbf{Hyperparameter Choice.} Experiments are conducted in validation set to search for best hyperparameters.}
...
\section{Conclusion}
We propose HOICLIP, a novel framework for transfer visual linguistic model knowledge to HOI detection. HOICLIP retrieve CLIP knowledge in a query-based manner and leverage the spatial visual feature for efficient and robust interaction representation learning. We also extract a verb classifier from the prior knowledge in CLIP through visual semantic arithmetic and introduce a verb adapter for deeper interaction understanding. To further improve the model's generalization ability, we adopt a training-free enhancement for HOI classification via the text feature of CLIP. Extensive experiments demonstrate the superior performance in both fully-supervised and zero-shot scenario with high data efficiency. In future, we will focus on exploring HOI detection in low data regime and zero-shot scenario with natural language supervision.
