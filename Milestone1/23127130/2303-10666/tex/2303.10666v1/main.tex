...
The DEOM \cite{Yan14054105} is not only an exact and nonperturbative approach to the  real--time evolution of open quantum system plus hybrid bath mode, but also serves as a
...
scenarios.
%
Without loss of generality, we only consider the single dissipative--mode cases, where the system--bath coupling assumes a linear form of
$H_{\SB}=\hat Q\hat{F}$.
%
...
or  using the he time--domain Prony fitting decomposition
...
closely related to that in the  Fokker--Planck theory, Zusman theory and the pseudomode method. \cite{Tan06082001,Dal01053813,Tam18030402,Tam19090402,Che19123035,Lam193721}
%
...
\begin{align}\label{rhoB}
  \rho_{\B}(\bm x,t)\equiv {\rm tr}_{\tS}\hat{\rho}({\bm x},t),
\end{align}
and obtain 
\begin{align}\label{distri}
\frac{\partial }{\partial t}\rho_{\B}(\bm x,t)=\sum_k\hat{\Gamma}_k \rho_{\B}(\bm x,t) 
...
\be \label{Mean}
\la  {\bm x}^{(n)}_{\bf n}(t)\ra={\rm tr}_{\tS}\hat X^{(n)}_{\bf n}(t) = \int {\bm x}^{(n)}_{\bf n}\,\rho_{\B}({\bm x},t)\,{\rm d}{\bm x}.
\ee
...
\quad\la \hat{F}^n(t) \ra&=\!\int\! \Big(\sum_k \zeta_k x_k \Big)^n \rho_{\B}(\bm x, t) \, \d {\bm x}
\nl
...
Figure \ref{Fig.2} depicts the results of calculating the statistical characteristics  of transient hybrid mode $\hat{F}(t)$ at different coupling strength, i.e.  the reorganization energy $\lambda$.
 ...
 \begin{figure}[h]
\includegraphics[width=\columnwidth]{lamb.pdf}
...
\end{figure}
 ...
 Figure \ref{Fig.3} shows the transient evolutions of statistical characteristics  of  the hybrid mode at different  temperatures.
Similar dynamical oscillation behaviors as that in \Fig{Fig.2} are displayed. On the other hand, when temperature increases,  $\la \hat{F} \ra$ at the steady state  increases (cf.\,\Fig{Fig.3}a),  while $\sigma_F(t)/\sigma_F(0)$ and the skewness reduce (cf.\,\Fig{Fig.3}b and 3c).  However,   the kurtosis does not varies monotunously in this regime, which suggests the complicated behaviors of the tailedness of relevant distributions.
...
\begin{align} 
\int {\bm x}_{\bf n}^{(n)} \,{\hat \Gamma}\rho_{\B}^{\rm st}\,\d {\bm x} =\int {\bm x}_{\bf n}^{(n)} {\bm \nabla} \cdot {\bm J}^{\rm st}\,\d {\bm x} .
\end{align}
...
To close the input--output relation, it requires further $\la Q{\bm x}^{(n>2)} \ra$ be expressed in terms of the system properties only. 
%
This could be achieved for some special systems, such as spin--boson case.
%
The recurrence relation can be closed when the involved system operators form a Lie algebra.\cite{Gu851310}
%
These input--output relations reflect the  system--bath entanglement that depicts the information exchange between them in realistic experimental measurements.





%To close above recurrence relation,  consider $\la \hat{Q}^m {\bm x}^{(n)}_{\bf n} \ra$:
%\begin{align}
%&\quad -i\la [H_{\tS},%\hat{Q}^m] {\bm x}^{(n)}_{\bf n} \ra+2\sum_k n_k \xi_k \la \hat{Q}^{m+1} {\bm x}_{{\bf n}^-_{k}}^{(n-1)} \ra
%\nl
%&=\sum_k \gamma_k\big[ n_k \la \hat{Q}^m {\bm x}_{\bf n}^{(n)}\ra- n_k (n_k-1)\la \hat{Q}^{m}{\bm x}_{{\bf n}^{--}_{kk}}^{(n-2)}\ra\big].
%\end{align}
%We can see that the recurrence relation can be closed when $[H_{\tS},[H_{\tS},\cdots [H_{\tS},Q]]]$ form a closed Lie algebra
