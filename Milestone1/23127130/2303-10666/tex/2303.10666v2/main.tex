...
\begin{align}
&\quad -i\la [H_{\tS},\hat A] {\bm x}^{(n)}_{\bf n} \ra+\sum_k \xi_k n_k \la \{\hat A,\hat{Q}\} {\bm x}_{{\bf n}^-_{k}}^{(n-1)} \ra
\nl
&=\sum_k \gamma_k\Big[ n_k \la \hat A {\bm x}_{\bf n}^{(n)}\ra- n_k (n_k-1)\la \hat A{\bm x}_{{\bf n}^{--}_{kk}}^{(n-2)}\ra\Big].
\end{align}
...
It is evident that the recurrence relation  is closed when $\hat{A}$ belongs to a linear space $\mathbb{L}$ of operators.  Here, the linearly independent  basis set of $\mathbb{L}$ has two parts:   (\emph{i})  basis vectors $\hat V$ satisfying $[H_{\tS},\hat{V}]=0$ and $\{\hat{Q},\hat{V}\}=0$;  (\emph{ii})   finite basis vectors  generated from the identical operator $\hat I$  via the operation $\{\hat Q,\cdot \, \}$ and $[H_{\tS},\cdot\, ]$. As a result, the basis set is  $\{\hat I, \hat{Q},[H_{\tS},\hat{Q}],\hat{Q}^2,[H_{\tS}, [H_{\tS},\hat{Q}]],\{\hat{Q},[H_{\tS},\hat{Q}]\},  \cdots\}$.\cite{Gu851310}
...
\begin{align}
0=\sum_k \gamma_k\Big[ n_k \la \hat\sigma_{x} {\bm x}_{\bf n}^{(n)}\ra- n_k (n_k-1)\la \hat\sigma_{x}{\bm x}_{{\bf n}^{--}_{kk}}^{(n-2)}\ra\Big],
\end{align}
\begin{align}
2\la \hat\sigma_{z} {\bm x}^{(n)}_{\bf n} \ra
=\sum_k \gamma_k\Big[ n_k \la \hat\sigma_{y} {\bm x}_{\bf n}^{(n)}\ra- n_k (n_k-1)\la \hat\sigma_{y} {\bm x}_{{\bf n}^{--}_{kk}}^{(n-2)}\ra\Big],
\end{align}
and
\begin{align}
&\quad 2\la \hat \sigma_{y}{\bm x}^{(n)}_{\bf n} \ra-2\sum_k \xi_k n_k \la \hat \sigma_{z}{\bm x}_{{\bf n}^-_{k}}^{(n-1)} \ra
\nl
