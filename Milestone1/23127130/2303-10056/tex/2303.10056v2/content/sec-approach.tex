...
\red{Given sets of parallel corpus $\mathcal{X}_{src} = \{x_{ij}^s|i=1,...,L^s, j=1,...,N^s\}$  and $\mathcal{X}_{tar} = \{x_{ij}^t|i=1,...,L^t, j=1,...,N^t\}$ with $L$ as length of tokens and $N$ as total quantity,  the source model ${F}^{s}$ and a target encoder ${F}^{t}$ have mapped the raw data into embeddings denoted as $\mathcal{S} = \{s_{ij}|i=1,...,L^s, j=1,...,N\}$ where $s_{ij} = F^{s}(x_{ij}^s) \in \mathbb{R}^{d_{s}}$ and  $\mathcal{T} = \{t_{ij}|i=1,..,L^t, j=1,...,N\}$ where $t_{ij} = F^{t}(x_{ij}^t) \in \mathbb{R}^{d_{t}}$. Due to the different models, there is severe distribution mismatch between the source and target features $p(s) \neq p(t)$.}
