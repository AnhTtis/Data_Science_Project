...
\rev{Finally note that, although Problem~\eqref{equation5} is an MIQP,
  it is a feasibility problem, which is often easier to solve than an
  optimization problem in practice.
  Besides that, if the point obtained from Algorithm~\ref{Second
    version} is close to the optimum of Problem~\eqref{equation2},
  many unlabeled points will be fixed and Problem~\eqref{equation2}
