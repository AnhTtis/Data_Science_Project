...
SVM. For a survey, see, e.g., \textcite{reducing}. However,
...
\rev{Although Theorem~\ref{algo2end} shows that, in the worst case,
  Algorithm~\ref{Second version} can take more iterations than
  Algorithm~\ref{First version} to terminate, Algorithm~\ref{Second
    version} solves problems with less binary variable in every
  iteration, which means that the time per iteration will be lower
  compared to Algorithm~\ref{First version}.}
...
\begin{thm}
  ...
  point returned by Algorithm~\ref{Second version}.
  Then, \rev{$(\bar{\omega}, \bar{b}, \bar{\xi}, \bar{\eta}, \bar{z})$ is feasible}
  for Problem~\eqref{equation2} with
  \begin{equation*}
...
\rev{As before, we can use the point obtained from
  Algorithm~\ref{Second version} to warm start
