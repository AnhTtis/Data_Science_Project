...
Election 1, the sorted positive majorities are ($A$,$B$), ($B$,$C$), and ($A$,
$C$).  We first add $A \succ B$, and then $B \succ C$, to $\mathcal{G}$. At
...
winner is established after the first two commits. Thus, $\mathcal{M}$ $=$ \{$A
\succ B$, $B \succ C$\}. For Election 2 of \autoref{tab:Example}, $\mathcal{M}
