...
We design a novel framework for uncertainty propagation of collaborative object detection (COD) to MOT, named Multiple Object Tracking with Conformal Uncertainty Propagation (MOT-CUP). Fig.~\ref{fig:overview} presents the methodology overview. The major novelties are: (1). MOT-CUP framework first rigorously quantifies the uncertainty (UQ) in the COD stage based on direct modeling and conformal prediction. (2). The uncertainty information is leveraged in the motion prediction stage of MOT, where a Standard Deviation-based Kalman Filter (SDKF) takes the COD uncertainty information as its input to improve the predicted precision of location. (3). We define a new Negative Log Likelihood similarity (SNLL) metric for the association step utilizing the UQ of COD, called NLLAI, to improve the MOT accuracy and reduce uncertainty.

In this section, we first introduce the conformal prediction in Subsecection~\ref{subsec:pre} as preliminary literature uncertainty quantification  and a useful method to construct predicted uncertainty. We describe our proposed MOP-CUP (Multiply Object Tracking with Conformal Uncertainty Propagation) method as shown in Algorithm~\ref{alg:whole} and Subsection~\ref{subsec:our_app}, followed by the detailed process of uncertainty quantification of COD based on direct modeling and conformal prediction in Subsection~\ref{subsec:uq_detection}, and uncertainty propogation to MOT in Subsection~\ref{subsec:uq_track}. %The major novelty of MOP-UP includes: we introduce the process of estimating uncertainty on the object detection stage with direct modeling and conformal prediction. Then, in subsection~\ref{subsec:uq_track}, we propose SDKF and NLLAI methods to leverage the uncertainty to improve the performance of MOT.
\subsection{Preliminary} 
\label{subsec:pre}

Conformal prediction (CP)~\cite{angelopoulos2021gentle} is a statistical method to generate prediction sets for any model. Conformal prediction is a method to convert any heuristic notion of uncertainty (like an estimate of the standard deviation) to rigorous uncertainty quantification. The beauty of conformal prediction lies in its explicit, non-asymptotic guarantees that are not contingent upon distributional or model assumptions. This makes it a valuable tool for statistical inference and prediction in a variety of applications.  For example, we assume an uncertain scalar follows Gaussian distribution and train a model to output the mean and standard deviation. To be precise, we choose to model $Y_{test} \sim \mathcal{N}(\mu(x), \sigma(x)) \mid_{X_{test} = x}$ where $X_{test}$ is a testing data, and $Y_{test}$ is the corresponding label. We have models $\hat{\mu}(x)$ and $\hat{\sigma}(x)$ trained to maximize the likelihood of the data. We can use conformal prediction to turn this heuristic uncertainty notion into rigorous prediction intervals of the form $\hat{\mu}(x) \pm \hat{q}\hat{\sigma}(x)$, where $\hat{q}$ is a quantile found by conformal prediction. % \hsh{$Y_{test} = f(X_{test} = x) \sim \mathcal{N}(\mu(x), \sigma(x))$}

Consider the validation data $(X_1, Y_1), ..., (X_N, Y_N)$ with $N$ datapoints that are never seen during training, the conformal prediction for a general input $x$ and output $y$ includes the following steps:
\textit{(1)} Define the score function $s(x, y) \in \mathbb{R}$. (Smaller scores encode better agreement between $x$ and $y$). \textit{(2)} Compute $\hat{q}$ as the $\frac{\lceil (N+ 1) (1-\alpha) \rceil}{N}$ quantile of the validation scores $s_1 = s(X_1, Y_1), ..., s_N = s(X_N, Y_N)$, where $\alpha \in [0, 1]$ is a user-chosen error rate. \textit{(3)} Use this quantile to form the prediction sets for new examples:
\begin{equation}
...
\end{itemize}
\fi
where $\mathcal{C}(X_{test})$ is the prediction set for $X_{test}$. Note that $(X_{test}, Y_{test})$ is a fresh test point from the same distributions of the validation data. The conformal prediction provides a coverage guarantee, as stated in the following lemma.
...
In other words, the probability that the prediction set contains the correct label is almost exactly $1-\alpha$. Note that this is only a special case of conformal prediction, called split conformal prediction. This is the most widely used version of conformal prediction. %Specifically, when we select the score function as $s(x, y) = \frac{\left| y - \hat{\mu}(x) \right|}{\hat{\sigma}(x)}$, the prediction set is $\mathcal{C}(x) = \left[\hat{\mu}(x) - \hat{\sigma}(x)\hat{q}, \hat{\mu}(x) + \hat{\sigma}(x)\hat{q} \right]$.
...
\For{point cloud frame $C$ in $S$ in time sequence}{
    ...
    \tcc{\textcolor{blue}{Adjust std by \textbf{CP}}}
...
We use direct modeling~\cite{he2019bounding,Su2022uncertainty} to estimate the standard deviation of each variable for the detected object during the collaborative object detection stage. We assume all variables are independent and the distribution of each variable is a single-variate Gaussian distribution. For the distribution of each variable of the ground-truth, we assume it as a Dirac delta function~\cite{he2019bounding}. Then we define the regression loss function for the $i$-th variable as the Kullback-Leibler (KL) divergence between the single-variate Gaussian distribution and the Dirac delta function~\cite{murphy2012machine}:
...
After we have the trained object detection model, we compute the quantile for the standard deviation of each variable by conformal prediction (CP)~\cite{angelopoulos2021gentle} based on the Validation dataset, as introduced in Subsection~\ref{subsec:pre}.
...
\begin{prop}
When we assume the uncertain scalar for $i$-th variable follows Gaussian distribution with mean $\hat{y}_i(x_i)$ and standard deviation $\hat{\sigma}_i(x_i)$ and select the score function as $s(x_i, y_i) = \frac{\left| y_i - \hat{y}_i(x_i) \right|}{\hat{\sigma_i}(x_i)}$ in conformal prediction, the prediction set for $i$-th variable is $\mathcal{C}_i(x_i) = \left[\hat{y}_i(x_i) - \hat{\sigma}_i(x_i)\hat{q}_i, \hat{y}_i(x_i) + \hat{\sigma}_i(x_i)\hat{q}_i \right]$.
\end{prop}
\begin{proof}
...
\subsection{Uncertainty Propogation to MOT}
...
\textbf{Standard Deviation-based Kalman Filter (SDKF):} As shown in Section~\ref{sec:relatedwork}, Kalman Filter~\cite{kalman1960new} is one important step for motion prediction. The inputs of the Kalman Filter encompass the observed state and measurement uncertainty. Compared to the existing MOT algorithm, we leverage our rectified  standard deviation as the measurement uncertainty in place of the pre-established values. By taking into account both the mean and standard deviation of the detections, we are able to better account for the uncertainty of objects and provide more robust tracklets over time. SDKF does not significantly impact the time complexity of algorithms, as it only modifies the measurement uncertainty input from fixed values to rigorously estimated ones.

 

\textbf{Algorithm~\ref{alg:nllai} Negative Log Likelihood-based Association Improvement (NLLAI):} Using Intersection over Union (IoU)-based similarity score cannot match low-quality detection results as shown in Section~\ref{sec:intro}. The inability to match such detections to the corresponding tracklets during the association stage poses a significant challenge. To address this issue, we propose the NLLAI technique as shown in Algorithm~\ref{alg:nllai}. We first define Negative Log Likelihood (NLL) between the predicted locations of tracklets and the detected locations as a novel similarity score:
\begin{equation}
    snll = - \frac{1}{I} \sum_{i=1}^I \log P(\dot{y}_i|\hat{y}_i, \hat{\sigma}_i)
\label{eq:nll_sim}
\end{equation}
where $\dot{y}_i$ is the predicted value for the $i$-th variable of the tracklet from the motion prediction model such as Kalman Filter. As Subsection~\ref{subsec:uq_detection}, the distribution of each $i$-th variable for detected objects is a single-variate Gaussian distribution where $\hat{y}_i$ is the mean and $\hat{\sigma}_i$ is the standard deviation. Given the set of unmatched detections and unmatched tracklets after the original association method, we compute the NLL similarity matrix $SNLL$ with Equation~\ref{eq:nll_sim} (see Line~\ref{alg:line_snll}). 
...
\KwResult{New association results $\mathcal{A}'_{matched}$, $\mathcal{D}'_{matched}$, $\mathcal{T}'_{matched}$}
