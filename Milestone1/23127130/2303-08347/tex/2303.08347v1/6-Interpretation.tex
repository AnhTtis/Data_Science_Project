...
A lot of interest in GFFs comes from the fact that 
they contain information on the spatial distributions 
of energy, angular momentum, and internal forces via 
...
The 3D interpretation of the information encoded by 
GFFs provides analogies to intuitive concepts such 
as pressure. The interpretation is carried out in the 
...
that in this case the transverse center of momentum of 
the nucleon is well defined \cite{Burkardt:2000za}. 
In other frames or in 3D, an exact probabilistic parton 
density interpretations does not hold in general.
...
In~(\ref{Eq:static-EMT}) the total static EMT is considered, 
but one can also define separate quark and gluon static EMTs 
\cite{Polyakov:2002yz,Lorce:2018egm}.
...
$\bar{C}_q(t)$ and $\bar{C}_G(t)$ is required. 
The latter are responsible for ``reshuffling'' of forces
between the gluon and quark subsystems inside the proton
\cite{Polyakov:2018exb} and are difficult to access 
experimentally. The instanton vacuum model predicts
$\bar{C}_q(t)$ to be very small \cite{Polyakov:2018exb} 
but other estimates indicate that it may be significant~\cite{Hatta:2018sqd,Tanaka:2022wzy}.
...
\begin{figure}[bhp!]
\includegraphics[width=0.95\columnwidth]{Figures/pressure.png} 
\includegraphics[width=0.95\columnwidth]{Figures/shear.png}
...
$\langle p'|T_a^{\mu\nu}|p\rangle$ is 
well-behaving in the forward limit.
