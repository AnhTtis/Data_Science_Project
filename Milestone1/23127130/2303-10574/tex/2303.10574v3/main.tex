...
decay to a right-handed muon: $Q^\mu_{R}=(1-\xip)/2$. This paper is accompanied by a Letter in Physical Review Letters~\cite{Belle:2023udc}.
...
\caption{Momentum $p_{p_1p_2}$ and angular $\cos{\theta_{p_1p_2}}$ distributions for the daughter particle (the mass hypothesis $p_1$) in the mother particle (the mass hypothesis $p_2$) rest frame. (a) $p_{e\mu}$, (b) $\cos{\theta_{e\mu}}$, (c) $p_{\pi K}$, (d) $\cos{\theta_{\pi K}}$, (e) $p_{\mu\pi}$, and (f) $\cos{\theta_{\mu\pi}}$.}
\label{fig:4.1}
\end{figure*}
