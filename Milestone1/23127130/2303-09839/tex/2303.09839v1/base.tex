...
\author{
Bruno Chiarellotto\footnote{\Letter \, chiarbru@math.unipd.it}, Nicola Mazzari\footnote{\Letter \, nicola.mazzari@unipd.it}, Yukihide Nakada\footnote{\Letter \, yukihide.nakada@unipd.it \newline \indent $\ast, \dag, \ddag$ Dipartimento di Matematica ``Tullio Levi-Civita", Universit\'a degli Studi di Padova, Via Trieste 63, 35121 Padova, Italy
\newline
\newline 
Acknowledgments: All the authors would like to thank Veronika Ertl for stimulating conversations. Chiarellotto and Nakada were supported by grant MIUR-PRIN2017 ``Geometric, Algebraic, and Analytic Methods in Arithmetic". Nakada was also supported by INdAM grant INdAM-DP-COFUND-2015.}
}
...
\begin{abstract}
    ...
    We prove the conjecture in the case the special fiber, of the given arithmetic scheme, is also a fiber of a geometric family over a curve in positive characteristic.
\end{abstract}

\textbf{Keywords}\, Log-crystalline cohomology $\cdot$ Monodromy $\cdot$ Rigid cohomology $\cdot$ $p$-adic cohomology

\vspace{12pt}

\noindent \textbf{Mathematics Subject Classification}\, 14F30; Secondary 14G22, 11G25
\section{Introduction}
    % \subsection{The Flach-Morin Conjecture}
    ...
    We write $S = \op{Spec}(W)$, and in the context of log structures we write $S^{\times}$ (resp.\,$S^\varnothing$) for $S$ equipped with the canonical log structure $1 \mapsto 0$ (resp.\,the trivial log structure).
    ...
    The geometric cohomology theory of the generic fiber are the cohomology groups 
    \[
        X \mapsto H^{B,i}_{HK}(X_{\overline{\eta},h}) \cong D_{pst}(H^i(X_{\overline{\eta}}, \Q_p)),
    \]
    called \emph{Beilinson-Hyodo-Kato} cohomology following \cite{NekovarNiziol2016}, taking values in the category of $(\phi, N, G_K)$-modules (for more on this structure see, for example, \cite{Berger2004}).
    This cohomology theory was defined for $X_\eta$ neither smooth nor proper by Beilinson using $h$-descent \cite{Beilinson2013}.
    However, when the morphism $f$ is smooth or log-smooth, there are canonical isomorphisms between Beilinson-Hyodo-Kato cohomology and the cohomology of the geometric special fiber: namely, if $f$ is smooth then it coincides with crystalline cohomology of the geometric special fiber $X_{\overline{s}}$ and if $f$ is log-smooth then it coincides with the log-crystalline (Hyodo-Kato) cohomology of $X_{\overline{s}}$ (see \cite[\S 7.2]{FlachMorin2018}).
    ...
    \begin{manualconjecture}{(\cite[Conjecture 7.15]{FlachMorin2018})}
    For regular $X$ there is an exact triangle in the category of $\vp$-modules 
\begin{align*}
...
\end{align*}
    ...
    \end{manualconjecture}

    We can reformulate this conjecture in the case where $f$ is log-smooth  using existing comparisons between the cohomology of the special and generic fibers. 
    ...
    \begin{align*}
    H^{B,i}_{HK}(X_{\overline{\eta}, h})^{G_K} &\cong D_{pst}(H^i(X_{\overline{\eta}}, \Q_p))^{G_K}\\
    &\cong D_{st}(H^i(X_{\overline{\eta}}, \Q_p))\\
    &\cong H^i_{\op{log-crys}}(X_k/S^{\times}).
    \end{align*}
    ...
    Since triangle (\ref{flach_morin_crystalline_realization}) no longer involves the generic fiber $X_\eta$ or the valuation ring $\O_K$, one can untwine the triangle from its original context and ask under what conditions on a $k$-scheme $X_s$ such an exact triangle exists. 
    The main result of this paper is Theorem \ref{flach-morin_main_theorem}, which states that such an exact triangle exists when $X_s$ is the special fiber, not of an  arithmetic family $f: X \to\op{Spec}( \O_K)$, but a geometric family $f: X \to C$ where $C$ is a curve over $k$:
    \begin{manualtheorem}{\ref{flach-morin_main_theorem}}
    ...
    \end{align*}
    together with the theory of log-convergent and log-rigid cohomology. 
    ...
    In our argument we work with log-rigid cohomology in place of log-crystalline cohomology because of the flexibility of the former.
    This is possible because log-crystalline and log-rigid cohomology, and their respective monodromy operators, agree in the proper and log-smooth case (Lemma \ref{log-rig_instead_of_log-crys}).
    ...
    We begin by reviewing the basic concepts of rigid cohomology that we'll need in the sequel.
    ...
    \]
    where $X \hookrightarrow Y$ is an open immersion of $k$-varieties and $Y \hookrightarrow \mathfrak{P}$ is a closed immersion into a formal $W$-scheme $\mathfrak{P}$. 
    ...
    The \emph{rigid cohomology of the pair $(X,X')$} is defined to be
    \[
        R\Gamma_{\op{rig}}((X,X')) := R\Gamma(\tube{X'}{\fP}, \jdag_{\tube{X}{\fP}}\Omega^\bullet_{\tube{X'}{\fP}})
    \]
    \emph{A priori} this definition depends also on the formal scheme $\mathfrak{P}$, but it can be shown (see \cite[7.4.2]{LeStum2007}, for example) that it depends (up to quasi-isomorphism) only on the immersion $X \hookrightarrow X'$.

    Two important cases of this construction that we use are the following.
    Firstly, the \emph{convergent cohomology} of $X$ is defined to be 
    \[
    R\Gamma_{\op{conv}}(X) := R\Gamma_{\op{rig}}((X,X)).
    \]
    Secondly, if $X \hookrightarrow \overline{X}$ is an open immersion with $\overline{X}$ proper, the \emph{rigid cohomology} of $X$ is defined to be 
    \[
        R\Gamma_{\op{rig}}(X) := R\Gamma_{\op{rig}}((X,\overline{X})).
    \]
    It is a foundational result in the theory of rigid cohomology, established by Berthelot, that this definition is not only independent of the formal scheme $\mathfrak{P}$ but also on the compactification $\overline{X}$ (see \cite[Proposition 8.2.1]{LeStum2007}).
    Note that if $X$ is already proper then convergent cohomology coincides with rigid cohomology.

    To understand the dual $R\Gamma^*_{\op{rig}}(X_s)$, we will also need the notion of rigid cohomology with support in a closed subset (see \cite{Berthelot1997} or \cite[Definition 6.3.1]{LeStum2007}).
    \begin{definition}\label{rigid_coh_with_support_def}
        ...
        \]
        to be the \emph{rigid cohomology of $X$ with support in $Z$}.
    \end{definition}   

    Its relation to standard rigid cohomology is the following.
    ...
    \]
    But by \cite[Proposition 5.1.7]{LeStum2007}, we have
    \begin{align*}
        \jdag_{\tube{\overline{X}}{\fP} \setminus \tube{Z}{\fP}} \circ \jdag_X &:= \jdag_{\tube{\overline{X}}{P} \setminus \tube{Z}{\fP}} \circ \jdag_{\tube{\overline{X}}{\fP} \setminus \tube{\overline{X} \setminus X}{\fP}}\\
        &= \jdag_{\tube{\overline{X}}{\fP} \setminus (\tube{Z}{\fP} \cup \tube{\overline{X} \setminus X}{\fP})}\\
        &= \jdag_{\tube{\overline{X}}{\fP} \setminus \tube{\overline{X} \setminus U}{\fP}}\\
        &=: \jdag_U
    \end{align*}
    ...
    \[
    R\Gamma_{Z,\op{rig}}(X) \to R\Gamma_{\op{rig}}(X) \to R\Gamma_{\op{rig}}(U) \to.  
    \]
    ...
    There is a theory of log-rigid cohomology, described by Gro{\ss}e-Kl\"onne (see, for example, \cite{GrosseKlonne2005}) which generalizes the log-crystalline cohomology of Hyodo and Kato in the same way that rigid cohomology generalizes crystalline cohomology.
    We give a brief overview here and refer the reader to \cite[\S 1.3]{GrosseKlonne2005} for precise definitions.
    ...
    \[
    V_H := \bigcup_{i \in H}V_i \to \mathfrak{V}_H \to  \varprojlim_{i\in H}  \mathfrak{V}_i \ ,%\times_{W} (\mathfrak{V}_i)_{i \in H}. 
    \]
    ...
    \]
    is defined to be the \emph{log-rigid cohomology of $X$} with respect to $\fS^{\times}$.
    It can be shown to be independent of the covering $X = \bigcup_i V_i$.

    One can also define a logarithmic equivalent of convergent cohomology for a fine log scheme $X/k$, called log-convergent cohomology, which we denote by $R\Gamma_{\op{log-conv}}(X/\fS^\varnothing)$.
    It is formally defined in general to be the cohomology of isocrystals on the log-convergent site (\cite[\S 2.1]{Shiho2002}), but it also admits an interpretation via the cohomology of a suitable logarithmic de Rham complex \cite[\S2.1, Corollary 2.3.9]{Shiho2002}. 
    As a matter of fact, replacing weak formal schemes and dagger spaces with formal schemes and rigid spaces in the definition of log-rigid cohomology, one recovers Shiho's log-convergent cohomology \cite[\S 1.5]{GrosseKlonne2005}.
    ...
    Let $f: X \to C$ be a proper, flat, generically smooth morphism over $k$ of relative dimension $n$, where $C$ is a smooth curve.
    Assume that for some $k$-rational point $s \in C$ the fiber $X_s$ is a normal crossing divisor in $X$.
    Endow $X$ with the log structure given by the divisor $X_s$ and to $X_s$ itself the pullback log structure.
    Then there is an exact triangle 
    \begin{center}
        \begin{tikzcd}[row sep=4em, column sep=-3.2em]
           & R\Gamma^*_{\op{rig}}(X_s)(-n-1)[-2n-1] \ar[dl]\\
    ...
    \begin{remark}
        Our setting is the derived category of $\vp$-modules, and it will be implicit that all of the quasi-isomorphisms below are compatible with the Frobenius when the objects have a nontrivial $\vp$-module structure.
    \end{remark}
    
  

    \begin{lemma}\label{log-rig_instead_of_log-crys}
        To prove Theorem \ref{flach-morin_main_theorem}, it suffices to construct an exact triangle as in the statement where $R\Gamma_{\op{log-crys}}(X_s/S^{\times})$ and its monodromy operator $N$ is replaced by $R\Gamma_{\op{log-rig}}(X_s/\fS^{\times})$ and its monodromy operator. 
    \end{lemma}
    \begin{proof}
        The coincidence of the cohomology theories is described in \cite[Theorem 3.1.1]{Shiho2002} and \cite[\S 1.5]{GrosseKlonne2005}, and the monodromy operators agree because they are both defined as the connecting morphism of an appropriate short exact sequence (see \cite[\S 5.4]{GrosseKlonne2005} and \cite[\S 2.6]{GrosseKlonne2007}).
    \end{proof}

As a first step, we reformulate the results of \cite{ChiarellottoTsuzuki2014} entirely in the language of derived categories.
First we prove 
\begin{lemma}\label{flach_morin_support_no_compactification}
    There is a canonical isomorphism 
    \[
    R\Gamma_{X_s, \op{rig}}(X) \cong [R\Gamma_{\op{conv}}(X) \to R\Gamma_{\op{rig}}((X \setminus X_s, X))]
    \]
...
\begin{proof}
    We temporarily forget the log structures on $X$ and $X_s$.
    ...
    \begin{center}
        ...
        \end{tikzcd}
    \end{center}
    Here the simplicial map $\overline{X}_\bullet \to \overline{X}$ is a Zariski affine hypercovering, $\mathscr{P}_\bullet$ is a simplicial formal scheme separated and of finite type over $\O_K$ which is smooth around $X_\bullet$, and which admits a Frobenius $\sigma_\bullet$ lifting that on $\O_K$
    (unlike in \cite[\S 4]{ChiarellottoTsuzuki2014}, we will not need a compactification $\overline{X}$ over a smooth compactification of $C$).
    ...
    \end{equation}
    where $(-)_s$ denotes the total complex of the morphism of complexes, interpreted as the rows of a double complex with $\jdag_{\tube{X_\bullet}{\mathscr{P}_\bullet}}\O_{\tube{\overline{X}_\bullet}{\mathscr{P}_\bullet}}$ in degree $(0,0)$.
    ...
    Furthermore, it was proven in \cite[Proposition 4.1]{ChiarellottoTsuzuki2014} that the direct image $a_{\bullet, *}$ is exact on the sheaves $\Omega^\bullet_{\tube{X_\bullet}{\mathscr{P}_\bullet}}$ and $\jdag_{\tube{X_\bullet \setminus X_{s,\bullet }}{\mathscr{P}_\bullet}}\Omega^\bullet_{\tube{X_\bullet}{\mathscr{P}_\bullet}}$, stemming from the fact that $X_s$ is a divisor.
    Hence
    \begin{align*}
        R\Gamma_{X_s,\op{rig}}(X) &= R\Gamma(\tube{\overline{X}_\bullet}{\mathscr{P_\bullet}},(\jdag_{\tube{X_\bullet}{\mathscr{P}_\bullet}}\Omega^\bullet_{\tube{\overline{X}_\bullet}{\mathscr{P}_\bullet}} \to \jdag_{\tube{X_\bullet \setminus X_{s,\bullet }}{\mathscr{P}_\bullet}}\Omega^\bullet_{\tube{\overline{X}_\bullet}{\mathscr{P}_\bullet}})_s) \\
        &\cong R\Gamma(\tube{\overline{X}_\bullet}{\mathscr{P_\bullet}},Ra_{\bullet, *}((\jdag_{\tube{X_\bullet}{\mathscr{P}_\bullet}}\Omega^\bullet_{\tube{\overline{X}_\bullet}{\mathscr{P}_\bullet}})|_{\tube{X_\bullet}{\mathscr{P}_\bullet}}\\ 
...
\begin{proof}
    ...
    In addition the log structure $\widetilde{\mathscr{M}}_\bullet$ comes from a log structure on the Zariski topology on $\widetilde{\mathcal{Q}}^{\op{ex}}_\bullet$ and is such that $(\widetilde{\mathcal{Q}}^{\op{ex}}_\bullet, \widetilde{\mathscr{M}}_\bullet) \to (\mathscr{C},\mathscr{N})$ is formally log-smooth and admits a lift of Frobenius.

    \begin{remark}
    The virtue of such a hypercovering is that, as we will see, it can be used both to compute rigid cohomology \emph{and} logarithmic cohomology. 
    \end{remark}

    If we let $X_{s,\bullet}$ denote the induced \'etale hypercovering of $X_s$, we can now apply a pair of results of Shiho \cite[Corollary 2.3.9, Proposition 2.4.4]{Shiho2002} which in conjunction say that, since each $(X_m,M_m)$ is a smooth log scheme with trivial locus $X_m \setminus X_{s,m}$ whose log structure comes from the Zariski topology on $X_m$, we have an identification $R\Gamma_{\op{rig}}((X_m \setminus X_{s,m}, X_m)) \cong R\Gamma_{\op{log-conv}}((X_m,M_m)/\fS^\varnothing)$.
    ...
    \]
    It is clear by looking at the level of their representative de Rham complexes that this isomorphism fits into the commutative diagram
    \begin{center}
        \begin{tikzcd}
    ...
    It thus follows by the same argument as in Lemma \ref{flach_morin_support_no_compactification}, using Kiehl's result that $R\iota_{\bullet, \eta *} = \iota_{\bullet, \eta *}$ on coherent sheaves, that (\ref{flach_morin_conv_logconv_X}) is isomorphic to
    \[
    ...
    \[
        [R\Gamma_{\op{conv}}(X_s) \to R\Gamma_{\op{log-conv}}((X_s,M_s)/\fS^\varnothing)]    
    \]
    as desired.
\end{proof}

We're now in a position to prove the main claim:

\begin{proof}[Proof of Theorem \ref{flach-morin_main_theorem}]
    The monodromy operator on $R\Gamma_{\op{log-rig}}(X_s/\fS^{\times})$ is defined in \cite[Definition 3.28]{ErtlYamadaRigAnalRecon} and \cite[\S5.4]{GrosseKlonne2005} as the connecting homomorphism of an appropriate exact sequence (for their coincidence, see \cite[Remark 2.4]{ErtlYamadaRigAnalRecon}).
    The associated exact triangle, by virtue of \cite[Proposition 3.29]{ErtlYamadaRigAnalRecon}, is
    \[
...
\]
The bounded Robba ring has the additional virtue that it is a Henselian discrete valued field with residue field $k((t))$.
