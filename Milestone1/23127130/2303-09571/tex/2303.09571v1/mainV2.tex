...
\begin{figure}
    ...
    \caption{The diameter of the black string is determined by the order parameter in the theory, $u_h$. The bulk theory is dual to a thermal BCFT state on the surface of the sphere. The black string is susceptible to the Gregory-Laflamme instability when $u_h$ becomes larger than $u_{\text{HP}} = \frac{1}{2^{1/3}}$ -- beyond this point, which corresponds to the Hawking-Page transition on the brane, both the black hole on the brane and the black string are unstable. The black string reaches minimum temperature at the spinoidal point $u_s = \frac{\sqrt{3}}{2^{2/3}}$, where the specific heat changes sign from positive to negative. Also see Figure \ref{fig:bhtemperature}.} 
    \label{fig:BS}
\end{figure}
...
\indent It can be seen in Figure \ref{fig:BS} that the black string has the topology of a cylinder. Such black string solutions are known to have an instability, corresponding to the rippling of the horizon, which is known as the Gregory Laflamme instability \cite{Gregory:1993vy}. This instability is first encountered for the global AdS black string when the black holes on each slice dip below a certain size, and it is dual to the Hawking-Page phase transition for the black hole in the intermediate picture \emph{if} there is dynamical gravity at the location of the brane \cite{Chamblin:2004vr,Gregory:2008br}. While this apparently does not affect our calculation, it should be noted that, since the bath is non-gravitating, the black hole on the CFT boundary presumably does not transition. While this could be avoided by studying this model in the context of wedge holography, the absence of gravity on the heat bath allows us to employ Dirichlet boundary conditions on the boundary. Doing so allows us to partition the gravitational degrees of freedom because we have a well-defined local Hilbert space of observables at low energies. This would not be the case if we included dynamical gravity on the boundary; we could not define a starting point for the radiation region because of the redundancy due to the absence of  diffeomorphism invariance. Note that, while we will limit ourselves by defining the radiation region as the full boundary -- meaning our RT surfaces will anchor to the defect -- we could have picked another point on the boundary. 
...
The Hawking-Page transition occurs when $I$ changes sign -- in other words, an AdS black hole is unstable when its radius, measured from the origin \emph{on} the KR braneworld in $r$ coordinates, is less than the bulk AdS radius:
...
\subsubsection{The Gregory Laflamme Instability}
 ...
 Beyond a certain value of $u_h$, the cylindrical topology of the black string is unstable to small perturbations which lead to the formation of a series of black holes along the extra dimension \cite{Gregory:1993vy}. This is the well-known Gregory LaFlamme instability for the black string. Fortunately for the self-consistency of this model, this instability is dual to the Hawking-Page phase transition on the brane \cite{Chamblin:2004vr}. These two instabilities, which occur simultaneously, are controlled by a dimensionless combination of the horizon temperature and the radius of the sphere containing the thermal boundary state.

The black hole in the non-gravitating bath is presumably stable. But if one studies the system with a gravitating bath, which one can establish by introducing another brane at a \emph{smaller} angle, the field theory on that brane will be susceptible to the Hawking-Page phase transition as well. It is in this setting that these instabilities are truly dual. But so long as one studies the entanglement entropy of surfaces anchored to the defect, as we do here, the computation will essentially proceed unchanged after the second brane is added -- the only difference is that, since the areas of the island RT surfaces decrease monotonically with angle, as noted later in Figure \ref{fig:Areas}, the minimal RT surface would be associated with the brane at a larger angle. 
