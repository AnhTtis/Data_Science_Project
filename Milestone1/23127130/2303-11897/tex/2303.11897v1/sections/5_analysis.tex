...
One limitation of \NAME is its runtime. Answering multiple visual questions is slower than one CLIP inference. In the scenario described in Section~\ref{sec:exp:comparisonVQA}, mPLUG takes 1.6s to evaluate one image (without batching). Also, our question generation pipeline needs one inference on a modern language model for each text input.
...
Another limitation is that \NAME requires VQA models to work reasonably well, which is true given the current models and \NAME v1.0, as shown in Section~\ref{sec:exp}.
However, the assumption might not hold in domains like anime and abstract art.
One possible solution is to ensemble multiple VQA models, which we leave for future work.