...
poses a real thread to the applicability of machine learning models.
...
we empirically evaluate the approach and we conclude in Section~\ref{sec:conclusion}.
...
\begin{figure}
    ...
    \caption{ The original (the top row), binarized (the middle row), and perturbed with $\epsilon = 1$
...
setup is the same for all experiments described in this section.
...
As the MNIST dataset contains $ 10 $ different classes, we conduct 
...
resulted in greater resistance to adversarial inputs. Due to the fact
...
would always give the same classification results for any input. The equivalence test showed
them to not  be fully equivalent in this strong sense.

We then considered similarity, which is easier to achieve as it only requires that the classification matches in most classes (where the threshold is given as a parameter), for a particular dataset. % (Table~\ref{tab:sim}).
Table~\ref{tab:sim} presents the result of the similarity experiment on a test set with $100$ instances. Columns show the number of instances
solved by the SAT solver and the number of $\epsilon$-similar instances. ``Solved' in this context
means that the SAT solver determines within 300 seconds whether two models return the same classification
...
the SAT solver time out on several instances. Even then, Models 2 and 3 are  $\epsilon$-similar on more instances.
%\todo{check}
\begin{table}[t]
\centering
...
timeout value of 300 seconds for each test instance. Table 4.5 presents results for both
...
$\epsilon$-robust instances in these two models are similar. This indicates that both models
have approximate robustness performance 
on  MNIST-back-image perturbed inputs.
