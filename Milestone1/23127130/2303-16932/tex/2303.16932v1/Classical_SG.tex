...
This is reflected into the scale invariant solution of Eq. \eqref{eq_ghdfill} when applied to the partitioning protocol: after a short time transient which depends on the details of the interface between the two initial halves, the evolving hydrodynamic state is not an independent function of $t$ and $x$, but only depends on the ratio $\zeta=x/t$. This allows one to derive an exact implicit solution to the partitioning protocol \cite{Doyon2016,Bertini2016}
\be\label{eq_part}
...
\end{align}
From here, one can show how the classical dressing operation naturally emerges in the  semiclassical limit. We  first replace the quantum expressions with the classical ones with the correct scaling of $\hbar$ (see Eq. \eqref{eq_limit_phaseshift}, \eqref{eq_rootdensity_scaling}, \eqref{eq_totalroot_scaling}), pass over from the sum over breathers to an integral according to Eq. \eqref{eq_sigmamap}, and see that $\hbar$ drops out everywhere. \\
...
\be
\langle O(\phi(0))=\frac{\int \mathcal{D} O(\phi(0)) e^{\beta \int_{-L/2}^{L/2} \frac{1}{2}(\partial_x\phi)^2+\frac{m^2c^2}{g^2}(1-\cos(g\phi)}}{\int \mathcal{D} e^{\beta \int_{-L/2}^{L/2} \frac{1}{2}(\partial_x\phi)^2+\frac{m^2c^2}{g^2}(1-\cos(g\phi)}}=\frac{\text{Tr}[ e^{-\frac{L}{2} \hat{H}_\text{eff}}O(\phi) e^{-\frac{L}{2} \hat{H}_\text{eff}}]}{\text{Tr}[ e^{-L \hat{H}_\text{eff}}]}\, ,
\ee
...
For each parameter choice, we run $20$ independent Monte Carlo, each of them collecting approximately $500$ samples. Then, we consider the total average as the representative value and the errorbars are estimated as the variance over the independent runs.
