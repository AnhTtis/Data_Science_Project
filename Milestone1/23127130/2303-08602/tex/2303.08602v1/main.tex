...
Exact formulas to calculate the necessary corrections for the de- and encoding of the full LHZ triangle are given in Appendix~\ref{sec:formula_corr}.
...
\appendices
\section{Formula for corrections in LHZ}\label{sec:formula_corr}
For the LHZ-layout and constraints as depicted in Fig.~\ref{fig:encoding_decoding_corr}, the formulas for the corrections after measurements can be derived as follows. Let ${m_{ij}^z\in \{\pm 1\}}$ denote the result of the $Z$-measurement of the constraint used to encode the qubit labelled ${(i, j)}$, with ${i, j \in\{0, 1, \dots, n-1\}}$.  We further define the spin variable
\begin{equation}
