...
\fe
and the ``source term" $\cC_{N}(\tau, \bar \tau)$ is the integrated correlator that appeared in \eqref{eq:2222}, whose expression is known exactly, and  obeys its own Laplace-difference equation as given in \eqref{eq:N-LG} \cite{Dorigoni:2021bvj,Dorigoni:2021guq}. The Laplace-difference equation \eqref{eq:gen-LDint} is a recursion relation that completely determines ${\widehat \cC_{N,p}}^{(M, M'|i, i')}  (\tau, \bar \tau)$ for any $p$ in terms of the initial condition ${\widehat \cC_{N,0}}^{(M, M'|i, i')}  (\tau, \bar \tau)$. 
...
\fe
It is clear that for a given dimension $\sum_{i=1}^n p_i = p \geq 4$, there are in general more than one distinguishable operator. For example, $\mathcal{O}_2$, which appears in the definition of the integrated correlators, is simply $T_2$, and at dimension four, one may choose  $\mathcal{O}^{(1)}_4=T_4$ and  $\mathcal{O}^{(2)}_4=T_{2,2}$. 
...
\mathcal{O}_{0|6}^{(1)} &= T_{3,3} -{9\over N^2+7} T_{4,2} +\frac{3 \left(5 N^2+1\right)}{N
   \left(N^2+3\right) \left(N^2+7\right)} T_{222}\, , \cr
...
The set of equations \eqref{eq:vMp} uniquely determine $v^{\mu}_{p|M}$. We find the mixing coefficients $v_{p|M}^{\mu}$  for the operators $\mathcal{O}_{p|M}^{(i)}$ are in fact independent of the index $i$. Explicitly, they take the following form, 
...
\ie
 ...
 = \frac{(-1)^q  \,  \Gamma
   \left(\frac{N^2+2M-1}{2}+\mu+q\right
   )}{ (-2i)^{\mu+q} \, \Gamma
   \left(\frac{N^2+2M-1}{2} \right
   )} {\tau_2}^{-\frac{2 M+N^2-1}{2} -\mu-q} \, .
\fe
...
\frac{(-1)^q  \,  \Gamma
   \left(\frac{N^2+2M-1}{2}+q\right
   ) \Gamma\left(p-q\right)}{ 2^{p+q} \, \Gamma
   \left(\frac{N^2+2M-1}{2} \right
   ) \Gamma
   \left(-q \right
   )} {\tau_2}^{-\frac{2 M+N^2-1}{2} -p-q} \, , 
\fe
...
\fe
Note the operators $\mathcal{O}^{(i)}_{p|M}$ and $\mathcal{O}^{(i')}_{p'|M'}$ need to have the same dimension, i.e. $2p+M=2p'+M'$, therefore one may remove the dependence on $p'$. We will denote these integrated correlators as $\cC_{N,p}^{(M, M'|i,i')}(\tau, \bar \tau)$.  However, the definition of $\widehat{\cC}_{N,p}^{(M, M'|i,i')}(\tau, \bar \tau)$ following  \eqref{eq:C-hat} in general is problematic now. Following \eqref{eq:CRCh}, we would naively write, 
\ie
...
\fe
Using the fact $\mathcal{O}_{p'|M'}^{(i')} =\mathcal{O}_d +  \mathcal{O}_{p|M}^{(i)}$, the numerators can be combined, which leads to
\ie \label{eq:C=C+C3}
...
\fe
Using the fact $2p+M=2p'+M'$, this relation implies that in obtaining the expression for ${\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ one could also eliminate $\mathcal{O}_{p|M}^{(i)}$ in favour of $\mathcal{O}_{p'|M'}^{(i')}$, and lead to the same result for ${\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$. This gives a consistency check of the argument. 

Now, we could simply use \eqref{eq:C=C+C3} and define\footnote{Another possibility is to factor out $\sqrt{R^{(M', M'|i',i')}_{N,p'} R^{(M, M|i,i)}_{N,p}}$ and use the following normalisation factor $$\sqrt{ v_{p'|M'}^{ \mu}\, \bar{v}_{p'|M'}^{ \nu}\partial_{\tau^{(i')}_{\mu|M'}}\partial_{{\bar{\tau}}^{{}_\prime (i')}_{\nu|M'}}  \log \mathcal{Z}_N(\tau, \tau_A'; 0) \big{|}_{\tau_A'=0} \,  v_{p|M}^{ \mu}\, \bar{v}_{p|M}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime(i)}_{\nu|M}}  \log \mathcal{Z}_N(\tau, \tau_A'; 0) \big{|}_{\tau_A'=0}}$$ in the definition of $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$. Once again, we find that $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ defined in this way does not obey a simple universal Laplace-difference equation.}
\ie \label{eq:Chat-naive}
\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)  &=   \frac{v_{p|M}^{ \mu}\, \bar{v}_{p'|M'}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime (i')}_{\nu|M'}} \partial_m^2  \log \mathcal{Z}_N(\tau, \tau_A'; m) \big{|}_{m=\tau_A'=0}  }{ v_{p|M}^{ \mu}\, \bar{v}_{p|M}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime(i)}_{\nu|M}}  \log \mathcal{Z}_N(\tau, \tau_A'; 0) \big{|}_{\tau_A'=0} } \, .
\fe
However, with this normalisation, $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ does not obey a simple Laplace-difference relation.  The denominator in this definition is not very natural; in particular, it does not treat $p,p'$ on an equal footing. Instead, we define $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ as, 
\ie \label{eq:newCh}
\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau) = \frac{v_{p|M}^{ \mu}\, \bar{v}_{p'|M'}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime (i')}_{\nu|M'}} \partial_m^2  \log \mathcal{Z}_N(\tau, \tau_A'; m) \big{|}_{m=\tau_A'=0}  }{ \mathcal{N}_M^{(i)} \,  D^{(M, M')}_{N,p} }\, ,
\fe
where the factor $D^{(M, M')}_{N,p}$ in the denominator -- the crucial part in the definition of $\widehat{\cC}^{(M, M'|i,i')}_{N,p}$ -- is defined as follows: when $M'\neq 0$,  it is given by,
\ie \label{eq:DDnew}
...
\fe
The expression of $D^{(M, M')}_{N,p}$ now treats $p, p'$ on an equal footing, and as we will see it naturally generalises the definition \eqref{eq:CMMiip} that is valid only for $M=M', i=i'$.  The right choice of $D^{(M, M')}_{N,p}$  is crucial for $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ to satisfy a universal and simple Laplace-difference equation, which will be presented in the next section. 

Finally, the factor $\mathcal{N}_M^{(i)}$ is introduced so that when $M'=M$ and $i'=i$, $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ reduces to the original definition \eqref{eq:C-hat}. This factor is given by,\footnote{One could choose a different definition for $\mathcal{N}_M^{(i)}$, for example $$ 
...
\fe 
Using $2p+M=2p'+M'$, it is easy to see that $D_{N,p}^{(M,M')}$ is symmetric under $M \leftrightarrow M', p \leftrightarrow p'$. Without loss of generality, one may assume $M \geq M'$ so that $\Theta(\delta)=0$.  When $M'=0$, we have, 
\ie \label{eq:DNp}
...
With $\widehat{\cC}^{(M, M'|i,i')}_{N,p}(\tau, \bar \tau)$ defined in  \eqref{eq:newCh}, the integrated correlators are then given by
\ie \label{eq:newC}
...
\fe
The prefactor $\widetilde{R}^{(M, M'|i,i')}_{N,p}$ is determined by simply comparing the above expression with \eqref{eq:C=C+C3}. 
Using the relation \eqref{eq:relationRZ} and the expressions for $\mathcal{N}_N^{(i)}$  and $D_{N,p}^{(M,M')}$ as given in \eqref{eq:Ni} and \eqref{eq:DD}, we can simplify $\widetilde{R}^{(M, M'|i,i')}_{N,p}$ and find
\ie \label{eq:Rt1}
...
and $\Delta_\tau = 4 \tau_2^2 \partial_{\tau}\partial_{\bar \tau}$.  The ``source term" $\cC_{N}(\tau, \bar \tau)$, which only appears when $M=M'$ and $i=i'$, is given in \eqref{eq:2222}, which also obeys a Laplace-difference equation \eqref{eq:N-LG} and  is known exactly for any $N$ and $\tau$ \cite{Dorigoni:2021bvj, Dorigoni:2021guq}.\footnote{Note $\cC_{N}(\tau, \bar \tau)$ is the integrated correlator associated with four-point function $\langle \mathcal{O}_2 \mathcal{O}_2 \mathcal{O}_2 \mathcal{O}_2 \rangle$, so in our notation it is equivalent to $\cC_{N,1}^{(0,0|1,1)}(\tau, \bar \tau)$. Namely, this corresponds to $M=M'=0$ and $p=1$, for which there is no degeneracy and we may set $i=1$.}  It is important to note that because ${\widehat \cC_{N,p}}^{(M, M'|i, i')}  (\tau, \bar \tau)=0$ when $p<0$, \eqref{eq:gen-LDu} is a recursion relation (rather than a differential equation) that determines ${\widehat \cC_{N,p}}^{(M, M'|i, i')} (\tau, \bar \tau)$ for any $p$ in terms of the initial data ${\widehat \cC_{N,0}}^{(M, M'|i, i')}  (\tau, \bar \tau)$. In the special case $M=M'=0$ (the operators were called maximal-trace operators with even dimensions), our Laplace-difference equation \eqref{eq:gen-LDu} reproduces what has been found in \cite{Paul:2022piq}. We will therefore mainly focus on $M>0$ in the cases with $M=M', i=i'$.  It is intriguing to note that the $N$-dependence of the recursion only appears in the parameter $a$ (given in \eqref{eq:para}), which has the symmetry of $N^2 \leftrightarrow M+M'$.
...
\widehat{\cC}^{(M, M|i,i)}_{N,p}(\tau, \bar \tau) = \frac{v_{p|M}^{ \mu}\, \bar{v}_{p|M}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime(i)}_{\nu|M}} \partial_m^2  \log \mathcal{Z}_N(\tau, \tau_A'; m) \big{|}_{m=\tau_A'=0}  }{ v_{p|M}^{ \mu}\, \bar{v}_{p|M}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime(i)}_{\nu|M}}  \log \mathcal{Z}_N(\tau, \tau_A'; 0) \big{|}_{\tau_A'=0} }\, . 
\fe
Using the relation (due to the fact that $\mathcal{Z}_N(\tau, \tau_A'; m)$ is an even function of $m$)
\ie
...
\fe
which follows from \eqref{eq:orthog}, 
we find, 
\ie  \label{eq:i=i}
...
\ie
\Delta_{\tau} {\partial_m^2  \mathcal{Z}_N(\tau, 0; m) \big{|}_{m=0} \over \mathcal{Z}_N(\tau, 0; 0) } = 4 \, \cC_{N}(\tau, \bar \tau)\, ,
\fe
where we have used the definition of $\cC_{N}(\tau, \bar \tau)$ as given in \eqref{eq:2222}.  Using this fact, it is easy to see that this term alone obeys the Laplace-difference equation \eqref{eq:gen-LDu}.  Therefore, from now on we will only focus on the first term of \eqref{eq:i=i}, which we will denote as $\widehat{\cC'}^{(M, M|i,i)}_{N,p}(\tau, \bar \tau)$ and quote as below, 
\ie \label{eq:Cpp}
\widehat{\cC'}^{(M, M|i,i)}_{N,p}(\tau, \bar \tau)  = \frac{v_{p|M}^{ \mu}\, \bar{v}_{p|M}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime(i)}_{\nu|M}} \partial_m^2  \mathcal{Z}_N(\tau, \tau_A'; m) \big{|}_{m=\tau_A'=0}  }{ v_{p|M}^{ \mu}\, \bar{v}_{p|M}^{ \nu}\partial_{{\tau'}^{(i)}_{\mu|M}}\partial_{{\bar{\tau}}^{{}_\prime(i)}_{\nu|M}}  \mathcal{Z}_N(\tau, \tau_A'; 0) \big{|}_{\tau_A'=0} } \, .
\fe
Since the source term $-4 \, \cC_{N}(\tau, \bar \tau)$ arises from the second term of \eqref{eq:i=i} as we just showed, we will prove that $\widehat{\cC'}^{(M, M|i,i)}_{N,p}(\tau, \bar \tau)$ obeys  \eqref{eq:gen-LDu} without the source term,  namely
...
\fe
 Therefore in these cases, there is no source term $-4 \, \cC_{N}(\tau, \bar \tau)$, consistent with the Laplace-difference equation \eqref{eq:gen-LDu}.  
...
\fe
Here and in the following, because when $M'=0$ there is no degeneracy, we set the degeneracy index $i$ as $1$. Equation \eqref{eq:dZ0}  implies that there is an additional term in $\widehat{\cC}^{(M, 0|i,1)}_{N,p}(\tau, \bar \tau) $, 
\ie \label{eq:Mp=0}
...
the results for $\widehat{\cC}^{(4, 0)}_{N,0}(\tau, \bar \tau)$ in terms of only $\mathcal{\cC}_{N}(\tau, \bar \tau)$. Once again, $\mathcal{\cC}_{N}(\tau, \bar \tau)$ is known exactly \cite{Dorigoni:2021bvj, Dorigoni:2021guq}, which may be determined by the recursion relation \eqref{eq:N-LG}.  Explicitly, we find
\ie
\widehat{\cC}^{(4, 0)}_{N,0}(\tau, \bar \tau)=\frac{\left(N-3\right)_7}{32N(N^2+1)} \left[{F}_N^{(4)}(\tau, \bar \tau) -\frac{2N^2-3}{N(N^2+1)}\left(\Delta_{\tau}+2N^2 \right)\mathcal{\cC}_{N}(\tau, \bar \tau) \right] \, ,
\fe
where $\left(N-3\right)_7$ is the Pochhammer symbol.
Using the Laplace-difference equation \eqref{eq:N-LG}, one could replace $\Delta_{\tau}\mathcal{\cC}_{N}(\tau, \bar \tau)$ by linear combinations of $\mathcal{\cC}_{N-1}(\tau, \bar \tau)$, $\mathcal{\cC}_{N}(\tau, \bar \tau)$ and $\mathcal{\cC}_{N+1}(\tau, \bar \tau)$.  Furthermore, the function ${F}_N^{(4)}(\tau, \bar \tau)$ is also expressed in terms of $\mathcal{\cC}_{N}(\tau, \bar \tau)$, 
\ie \label{eq:FN4}
{F}_N^{(4)}(\tau, \bar \tau)=\sum_{l=1}^{N-1} \frac{1}{2l(l+1)} &\left[l^2(l+1)^2 \left(\mathcal{\cC}_{l+2}(\tau, \bar \tau)+\mathcal{\cC}_{l-1}(\tau, \bar \tau)\right)-l (l-2) \left( l^2+4l -1\right) \mathcal{\cC}_{l+1}(\tau, \bar \tau) \right.\cr 
&\left.- (l+1) (l+3) \left(l^2-2 l-4\right) \mathcal{\cC}_{l}(\tau, \bar \tau)\right] \, .
\fe
...
&\left.-6N(N+1)(N^2+2N-21)\mathcal{\cC}_{N-1}(\tau, \bar \tau)+2(N-1)(N-2)\left((N^3+5N^2-13N+25)\mathcal{\cC}_{N}(\tau, \bar \tau) \right. \right. \cr
& \left. \left. +6N(N-1)\mathcal{\cC}_{N+1}(\tau, \bar \tau)\right)\right] \, ,
\fe
...
\ie \label{eq:solp3}
{\cC}^{(3,3)}_{N,0}(\tau, \bar \tau) = 2 \sum_{l=1}^{N-1}  \left( \cC_{l} (\tau, \bar \tau)+ \cC_{l+1}(\tau, \bar \tau)\right) - {4\over N} \cC_{N}(\tau, \bar \tau) \, ,
\fe
and ${F}_N^{(5)}(\tau, \bar \tau)$ taking the following form,  
\ie
\!\!\!\!\!\!\! {F}_N^{(5)}(\tau, \bar \tau)=\sum_{l=1}^{N-1} &\frac{6}{5l^2 (l+1)^2}\! \left[4l(l+2)(l+1)^2 {F}_N^{(4)}(\tau, \bar \tau) + l(l^2-1)\left((9l-6) \cC^{(3,3)}_{N,0}(\tau, \bar \tau)+2l^2(l+1) \mathcal{\cC}_{N+2}(\tau, \bar \tau) \right)\right.\cr 
&\left.-2l^2(l-1)(3l^2-4l+3)\mathcal{\cC}_{N+1}(\tau, \bar \tau) -2(l+1)(l+2)(l^4+l^3-11l^2+11l-8)\mathcal{\cC}_{N}(\tau, \bar \tau)\right] \, ,
\fe
