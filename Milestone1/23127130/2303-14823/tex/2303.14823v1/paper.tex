...
\begin{document}
	...
	\title{Microscopic study of the Josephson supercurrent diode effect in 2DEG-based Josephson junctions}
    ...
    \begin{abstract}
            ...
            Their ability to carry supercurrents with magnitudes depending on the polarity~(current direction)---termed supercurrent diode effect---might be practically exploited to design dissipationless counterparts of contemporary semiconductor-based diodes. 
    ...
    Since their initial experimental discovery in synthesized (Nb/Ta/V)$ _n $-based superlattices~\cite{Ando2020}, nonreciprocal supercurrent phenomena have attracted enormous interest and have been thoroughly investigated in various systems covering, e.g., twisted magic-angle bi- and trilayer graphene~\cite{diezmerida2021magnetic,lin2022zerofield,scammell2022theory}, proximitized topological insulators~\cite{BoLu2022,Pal2022,Fu2022TopoDiode} and van-der-Waals heterostructures~\cite{Wu2022,Bauriedl2022}, as well as Josephson junctions~\cite{Baumgartner2021,Baumgartner2022a,Baumgartner2022,Jeon2022,Pal2022,Turini2022,FrolovQW2022,Costa2023,Banerjee2023}. 
    Their ability to carry supercurrents of different magnitude depending on the polarity, which motivated to term them \emph{supercurrent diodes}~\cite{Hu2007}, makes these systems promising candidates for future applications in dissipation-free electronics and information technology. 
    ...
    The first approach~\cite{Daido2022,Yuan2022,He2022,Ilic2022,Davydova2022,Banerjee2023}, elaborated to understand the SDE in quasi-2D superconductors, relies on a shift of the SOC-split Fermi surfaces through the Zeeman coupling, which results in a finite Cooper-pair momentum and thus in a helical phase that reveals a non-reciprocal current response. The second one~\cite{Buzdin2008,Grein2009,Baumgartner2022a,Baumgartner2022,Costa2023,Lotfizadeh2023} links the SDE in Josephson junctions to a strong phase-asymmetry of the Andreev-bound-state spectra~\cite{Andreev1966,*Andreev1966alt}, which likewise originates from the competition of SOC and Zeeman coupling. 
    Since the coherent Cooper-pair transport through Josephson junctions is mediated by electrons tunneling via these Andreev bound states, the phase-asymmetry will be imprinted on the Josephson current--phase~relation~(CPR) and endow the latter with an anomalous $ \varphi_0 $-phase shift~\cite{Bezuglyi2002,Krive2004,Buzdin2008,Reynoso2008,Zazunov2009,Liu2010a,Liu2010,Liu2011,Reynoso2012,YokoyamaJPSJ2013,Brunetti2013,Yokoyama2013,Shen2014,Yokoyama2014Anomalous,Konschelle2015,Szombati2016,Assouline2019,Mayer2020b,Strambini2020}, the SDE~\cite{Grein2009,Lotfizadeh2023}, and even current-reversing     $0$--$\pi$-like phase transitions~\cite{Bulaevskii1977a,*Bulaevskii1977b,Ryazanov2001,Andersen2006,Kawabata2010,Kawabata2012}. 
    ...
    Analytical calculations~\cite{Buzdin2008} suggested that $ |\varphi_0| $ scales, in the simplest case, linearly with the product of the SOC and Zeeman-coupling strengths, and inversely with the square of the velocity components of the incident electrons~(holes) that is parallel to the current direction. 
    Since the latter are different for different transverse channels of two-dimensional junctions, the CPRs of the individual channels acquire distinct $ \varphi_0 $-shifts. 
    In multi-channel Josephson junctions, the total CPR emerges as the sum of all these channels' CPRs and gets therefore strongly distorted. 
    ...
    \begin{align}
        ...
        &\hspace{70 pt} + \alpha \left( k_y \hat{\sigma}_x - k_x \hat{\sigma}_y \right) \nonumber \\
    ...
    The deltalike model enormously simplifies the analytical treatment, and still gives valid results for the experimentally relevant short-junction~scenarios~\cite{Scharf2019,Baumgartner2021,Lotfizadeh2023}, in which the length of the F link is substantially smaller than the underlying superconducting coherence length $\xi$. 
    
    
    Inside the superconducting regions, we assume an $ s $-wave coherence governed by the pairing potential 
    \begin{equation}
        \hat{\Delta}_\mathrm{S} (x) = \Delta_0 \left[ \Theta(-x) + \mathrm{e}^{\mathrm{i} \varphi} \Theta(x) \right] ,
    \end{equation}
    ...
    \end{align*}
    The electron-hole content in $\psi(x)$ is encoded in the energy-dependent (and generally complex-valued) BCS coherence factors 
    \begin{align} 
        u &= \sqrt{\frac{1}{2} \left( 1 + \sqrt{1 - \frac{\Delta_0^2}{E^2}} \right)}\,, &
        v &= \sqrt{1 - u^2} \,,
        \label{Eq_BCSCoh_Fact}
    \end{align}
    while the electronlike and holelike quasiparticle wave vectors in \emph{Andreev approximation}---i.e.,~for $ \mu \gg E , \Delta_0 $---are given by 
    \begin{align}
    ...
    \end{align}
    For a more compact notation, we introduced the Fermi wave vector~$ k_\mathrm{F} = \sqrt{2m\mu}/\hbar $ and the dimensionless effective SOC strength~$ \lambda_\mathrm{SOC} = (m \alpha) / (\hbar^2 k_\mathrm{F}) $. 
    ...
    \begin{align}
        1 &= \int_{-\infty}^\infty \mathrm{d} x \, \left[ \big| \psi(x) \big|^2 \right]  \\
    ...
    \begin{equation}
        ...
        \right] 
    \end{equation} 
    refers to the corresponding current-density operator along the transport direction~($\hat{x}$) that we resolved within the particle-hole space in terms of the tensor product with the $\tau_z$-Pauli matrix. 
    ...
    \begin{widetext} 
    \begin{multline}
        I_\mathrm{J}(\varphi) = -e \sum_{E > 0} \frac{W}{2\pi} \int_{-k_\mathrm{F}}^{k_\mathrm{F}} \mathrm{d} k_y \, \frac{\hbar}{m} \Bigg\{ \mathfrak{R} \bigg[ q_{x,\mathrm{e}}^\Uparrow + \frac{\big(\alpha_\Uparrow\big)^* + \alpha_\Uparrow}{2} \lambda_\mathrm{SOC} k_\mathrm{F} \bigg] \big| \mathcal{E} \big|^2 - \mathfrak{R} \bigg[ q_{x,\mathrm{h}}^\Uparrow - \frac{\big(\tilde{\alpha}_\Uparrow\big)^* + \tilde{\alpha}_\Uparrow}{2} \lambda_\mathrm{SOC} k_\mathrm{F} \bigg] \big| \mathcal{F} \big|^2 %%\bigg[ \big| u^\Uparrow(E_\mathrm{B}) \big|^2 + \big| v^\Uparrow(E_\mathrm{B}) \big|^2 \bigg] 
        \\
        %% 
    ...
    When presenting our numerical results in~Sec.~\ref{Sec_Results}, we always normalize the Josephson current with respect to~$ (\pi \Delta_0)/(e R_\mathrm{S}) $, where $ R_\mathrm{S} = ( \pi^2 \hbar ) / (e^2 k_\mathrm{F} W) $ stands for the Sharvin resistance of a perfectly transparent two-dimensional point contact; the normalized quantity $ I_{\mathrm{J}} e R_\mathrm{S} / (\pi \Delta_0) $ is thus independent of the transverse width~$ W $~(number of channels). Furthermore, the Josephson current is always evaluated at zero temperature. 
    ...
    The dimensionless Rashba~parameter~$ \lambda_\mathrm{SOC} = 0.4 $ corresponds to the realistic Rashba SOC strength~\cite{Fabian2004,Fabian2007} of~$ \alpha \approx 20 \, \mathrm{meV} \, \mathrm{nm} $~(assuming a typical mass of~$ m = 0.1 m_0 $, where $ m_0 $ is the free-electron mass, and Fermi energy of~$ \mu = 1.5 \, \mathrm{meV} $~\cite{Dartiailh2021,Lotfizadeh2023}). 
    ...
    At $ k_y = 0 $, SOC can indeed be neglected and the Andreev branches~$ E_1^\pm $ and $ E_2^\pm $ merge into a single doubly spin-degenerate 
    branch, whose energies are---in good approximation---given by the well-known result valid for an effectively one-dimensional short-junction~\cite{Golubov2004,Beenakker1991,Costa2018}
    \begin{equation}
    ...
    The Andreev states with energy~$ E_1^+ $---more closer to the center of the superconducting gap---have dominant 
    spin-down~(spin-up) projections along the $x$-axis for $ k_y>0 $~($k_y<0$), while the states with 
    ...
    for current-carrying in-gap $ E_1^\pm $-states at any~$ |k_y|<k_\mathrm{F} $, establishing the $ E_1^\pm $-Andreev states as the main channels to transport Cooper pairs. 
    ...
    \begin{figure}
        ...
        The insets illustrate the $ k_y $-resolved individual contributions of the states to the total Josephson CPR. In the magnetic case, the Andreev bound states and CPRs develop an asymmetry with respect to $ \varphi = 0 $ at larger $|k_y|$~(i.e., at~$ |k_y| > |k_y^\mathrm{crit.}| $). When superimposed, the different asymmetries of different $ k_y $-channels imprint a global $ \varphi_0 $-distortion on the total Josephson CPR and result in the Josephson SDE. The junction is overall in the $0$-like state. }
    ...
    For the exchange parameter, we use~$ \lambda_\mathrm{XC} = 1.5 $, which becomes equivalent to $ E_\mathrm{XC} d \approx 36 \, \mathrm{meV} \, \mathrm{nm} $~(assuming the same mass and Fermi energy as stated in the nonmagnetic part); for a tunnel barrier in the $ \text{\AA} $-regime, this gives experimentally feasible values 
    of the exchange splitting in the $ \mathrm{meV} $-range. 
    ...
    (i)~the $ E_1^\pm $-bands manifestly lack the symmetry with respect to a reversal of $\varphi$ and 
    (ii)~the CPRs, associated with the $ E_1^\pm $-bound states, acquire clearly observable $\varphi_0$-shifts, which---after averaging over all channels' CPRs---results in the distorted total CPR with different magnitudes of the positive and negative critical Josephson currents, and thereby in the Josephson SDE; see Figs.~\ref{Fig_BoundStatesCurrent_MAGNETIC} and \ref{Fig_BoundStatesCurrent_COLORMAP}(c).  
    All these observations are the consequence of the broken time-reversal symmetry together with different spectral characteristics that the bound-state wave functions $\Psi(x,y)$ possess with respect to the $ y $-coordinate at $ |k_y| < |k_y^\mathrm{crit.}| $ and $ |k_y| > |k_y^\mathrm{crit.}| $---i.e., spatially extended in the first case, while localized in the second one. 
    According to Eq.~\eqref{Eq_Critky}, the value of $|k_y^\mathrm{crit.}| $ is determined by the Rashba SOC, which coincides with earlier findings~\cite{Buzdin2008} that $ \varphi_0 $-Josephson effects require at the same time magnetic exchange and SOC. 
    

    Why do we not observe $ \varphi_0 $-shifts at $ |k_y| < |k_y^\mathrm{crit.}| $? 
    In~fact, we do when calculating more ``sensitive'' quantities like the wave-function squares. However, these asymmetries are numerically small and not relevant to the Josephson current. For this reason, we still call the CPRs of these channels with $ |k_y| < |k_y^\mathrm{crit.}| $ ``quasi-sinusoidal''. As a rule of thumb, the asymmetric and anharmonic $ \varphi_0 $-features are expected to become more pronounced and relevant for larger $\lambda_\mathrm{SOC}$ when $|k_y^\mathrm{crit.}|$ shifts to lower values following~Eq.~\eqref{Eq_Critky}. The transverse channels with $|k_y| > |k_y^\mathrm{crit.}| $, which cause the dominant $ \varphi_0 $-shifts in the current, gain then more weight and clearly distort the total CPR.     
    
    
    So far, we have displayed the Andreev bound states and their contributions to the Josephson current for a discrete set of transverse momenta $|k_y|$---namely the~0.05, 0.20, 0.40, 0.60, 0.80, and 1 multiples of the Fermi wave vector~$k_\mathrm{F}$. 
    ...
    For the sake of completeness, and to better visualize the aforementioned anharmonicity, we present the full CPRs of a junction close to the $ 0 $--$ \pi $~transition, varying~$ \lambda_\mathrm{XC} $ in steps of 0.1 from 1.5 to 2.2, in~Fig.~\ref{Fig_BoundStatesCurrent_COLORMAP}(c). For $\lambda_\mathrm{XC} \lesssim 1.7 $, the junction is in a $0$-like state, while it is already in a $ \pi $-like state at $ \lambda_\mathrm{XC} \approx 2.0 $; the interim exchange range between 1.7 and 2.0 spans the transition region in which the state of the junction is usually a mixture of both and hard to uniquely discern. The reason to term the states $ 0 $- or $ \pi $-\emph{like} is as follows. Given the CPR~$I_\mathrm{J}(\varphi)$, we compute the Josephson energy~$ E_\mathrm{J}(\varphi)\propto\int_0^\varphi \mathrm{d} \phi \, I_\mathrm{J}(\phi) $ as a function of the superconducting phase difference and look for the position of its global minimum to determine the energetically favored junction ground state. The minima are neither strictly at $0$ nor at $\pi$ phase difference, but close to these values motivating to call the corresponding junction states $ 0 $-\emph{like} in the first and $ \pi $-\emph{like} in the second case. For the same reason, the phase jump of the ground-state wave function when the junction undergoes the $ 0 $--$ \pi $~transition is also not strictly $ \pi $, but instead rather a generic $ |\Delta \varphi_\mathrm{GS}| \neq \pi $, and we likewise term the transition $ 0 $--$ \pi $-\emph{like}. 
    The $ \Delta \varphi_\mathrm{GS} $-phase jumps during the $ 0 $--$ \pi $-like transitions are illustrated for two different SOCs in the plot of the global $\varphi_0$-shift of the total Josephson CPRs versus $\lambda_\mathrm{XC}$ that we provide in~Fig.~\ref{Fig_Phi0}. 
    ...
    \begin{figure}
        ...
        \caption{Polarity-dependence of the critical Josephson current, $ I_\mathrm{c}^+ $ and $ |I_\mathrm{c}^-| $, displayed as functions of the magnetic exchange~$ \lambda_\mathrm{XC} $; the other junction parameters are (a)~$ Z = 0.5 $ and $ \lambda_\mathrm{SOC} = 0.4 $, as well as (b)~$ Z = 0.5 $ and $ \lambda_\mathrm{SOC} = 1.6 $. 
    ...
    Contrary, the behavior of the $\varphi_0$-shifted CPRs of the  $|k_y|>|k_y^\mathrm{crit.}| \approx 0.68 k_\mathrm{F}$-channels is
    ...
    and hence favor positive critical Josephson currents at positive phase differences. In total, the latter dominate over the ``quasi-sinusoidal'' modes and the junction is overall still in the $0$-like state.
    ...
    \begin{figure}
        ...
        (b)~Same as in~(a), but as color map with a denser range of $ \lambda_\mathrm{SOC} $ and $ \lambda_\mathrm{XC} $. 
    ...
    It is remarkable that---despite the minimality of our model---the calculated diode efficiencies convincingly reproduce all experimentally determined characteristics reported for Al-gated InAs 2DEG-based Josephson junctions in Refs.~\cite{Baumgartner2022a,Baumgartner2022,Costa2023}: (i)~a weak ``kink'' (soft steepness) at small~$ \lambda_\mathrm{XC} $, (ii)~followed by a nearly-linear increase that reaches (iii)~a maximum~(in terms of a sharp peak at realistic SOC), and (iv)~a down-turn~(that can even be accompanied by a sign reversal at large enough $\lambda_\mathrm{SOC}$), with a final fading at large $ \lambda_\mathrm{XC} $. 
    

    Based on our critical-current analyses, we can describe these features in terms of their physical origin. 
    The sharp peaks---feature~(iii)---stem from the cusps experienced by~$ |I_\mathrm{c}^-| $, which play the crucial role in the ``first act'' of the $ 0 $--$ \pi $-like transition; since $ |I_\mathrm{c}^-| $ drops into a sharp local minimum, $\eta_\mathrm{c}$~[see Eq.~\eqref{Eq_DiodeEff}] maximizes. 
    ...
    Apart from the good qualitative agreement, also the order of magnitude of the Josephson SDE extracted from our theoretical model compares reasonably well with the experimental values~(about~$ 20 \, \% $) measured in Al-gated InAs 2DEG-based junctions~\cite{Baumgartner2022a,Baumgartner2022,Costa2023}. 
    
    
    \begin{figure}
        \centering
        \includegraphics[width=0.49\textwidth]{Fig_CritCurrent_DifferentBarriers_ReducedSize.pdf}
        \caption{Polarity-dependence of the critical Josephson current, $ I_\mathrm{c}^+ $ and $ |I_\mathrm{c}^-| $, displayed as functions of the magnetic exchange~$ \lambda_\mathrm{XC} $, and for~$ \lambda_\mathrm{SOC} = 0.4 $; the barrier $ Z $-parameters are (a)~$ Z = 1 $ and (b)~$ Z = 2 $. The shift~$ \lambda_\mathrm{XC}^* $ of the critical-current maxima strongly depends on the junction transparency through~$ Z $. 
    ...
    Another knob to tune the Josephson SDE---except for the SOC---is the $ Z $-parameter of the deltalike tunnel barrier, which is experimentally determined by the transparency of the nonsuperconducting weak link. 
    In all calculations discussed so far, we have assumed a rather small value of~$ Z = 0.5 $ to model the highly transparent junctions~(transparency~$ \overline{\tau} = 1 / [1 + (Z/2)^2] \approx 0.94 $) corresponding to recent experiments~\cite{Baumgartner2022a,Baumgartner2022,Costa2023}. 
    ...
    %A possible reason to explain the sharpened SDE~peaks might be that strong tunnel barriers mostly suppress the ``quasi-sinusoidal'' CPRs of the $ |k_y| < |k_y^\mathrm{crit.}| $-channels such that the $ \varphi_0 $-shifted CPRs of the $ |k_y| > |k_y^\mathrm{crit.}| $-channels, and the therefrom resulting SDE, gain more weight. 
    ...
    Larger~$ Z $~(reduced interfacial transparency) results in a greatly enhanced critical-current asymmetry and more clearly apparent sign changes of~$ \eta_\mathrm{c} $ are more clearly apparent. 
    From that viewpoint, one reason that these sign changes were hardly discernible in the experiment~\cite{Baumgartner2022a,Baumgartner2022,Costa2023} might be that the junction transparency is even larger than the theoretically assumed~$ \overline{\tau} \approx 0.94 $. 
    ...
    Investigating a wide range of realistic junction parameters, we analyzed the impact of magnetic exchange, SOC, and junction transparency on the underlying Josephson CPRs---scrutinizing their distortion, anomalous $ \varphi_0 $-pase shift, Josephson SDE, and $ 0 $--$ \pi $-like transitions. 
    We quantified the diode effect in terms of the SDE efficiency~$ \eta_\mathrm{c} $ that can reach values up to~$ 30 \, \% $, and demonstrated that $ \eta_\mathrm{c} $ depends on the magnetic exchange~$ \lambda_\mathrm{XC} $ in a unique way, which agrees reasonably well with recent experiments. 
    ...
    Finally, we showed that the SDE can even be further enhanced through reducing the transparency of the weak link. This might open a promising path for future experimental works provided that mostly junctions with high transparency have been investigated so far. 
