...
\end{align*}
Since the gradients' magnitude tends to near-zero values close to convergence, statistical errors are more likely to make the VMC optimization more challenging. We focus on this regime for this derivation to show the importance of the baseline in reducing noise. Thus, we assume that $E_{\text{loc}} (\bm{\sigma}) = E_{\bm{\theta}} + \xi(\bm{\sigma})$, where the supremum of the local energies fluctuations is much smaller compared to the variational energy, i.e., $(\sup_{\bm{\sigma}} |\xi(\bm{\sigma})|) \ll E_{\bm{\theta}}$. From this assumption, we can deduce that:
\begin{align}
