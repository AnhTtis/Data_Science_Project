...
\newcommand{\SY}[1]{\textcolor{teal}{#1}} % SungWoo
\newcommand{\JN}[1]{\textcolor{blue}{#1}} % Junu
...
In this report, we simulate axion electrodynamics using \texttt{COMSOL Multiphyics}, a commercially available simulation software, for various experimental schemes, including the dish antenna haloscope, cavity haloscope, dielectric haloscope, and axion-photon regeneration.
\end{abstract}
...
They include the cavity haloscope~\cite{PRD1985Sikivie}, dielectric haloscope~\cite{PRL2017MADMAX}, and dish antenna haloscope~\cite{JCAP2013Horns}.
...
In particular, the {\it RF module} included in the package provides the ability to design and optimize RF devices and components by solving Maxwell's equations for arbitrary geometries.
...
The angular frequency determines the axion mass ($m_a$) by $\omega_{a} \approx m_{a} c^{2} / \hbar$, where $c$ is the speed of light and $\hbar$ is the reduced Plank constant.
...
%Since $g_{a\gamma\gamma} a_{0} \ll 1$ for invisible axions, the ordinary field and the reacted field can be separated through a linear approximation of modified Maxwell's equations~\cite{PDU2019Kim}.
...
%In other words, dark matter axions drive electromagnetic fields by the additional oscillating current density, proportional to the ordinary magnetic field.
...
In this case, the frequency of the reacted fields become that of the axion.
In other words, there is no frequency mixing in the equations of motion.
...
\subsection{Dish antenna haloscope}
According to Equation~\ref{eq:axion_haloscope_maxwell}, dark matter axions on a metal plate under a magnetic field are converted into photons.
...
The simplest dish antenna haloscope is conversion in an infinite metal plate.
...
At the bottom of the rectangle, the external current density induced by the axion, corresponding to Equation~\ref{eq:axion_haloscope_maxwell}, was driven with a boundary condition of a perfect electric conductor.
...
Fig.~\ref{fig:haloscope_simul}~(a) shows the resulting conversion power obtained by the simulation and also by Equation~\ref{eq:DAH_power}.
The conversion power is exactly given as expected and follows the sin to the power of 4 function with respect to the frequency.
...
Since the axion frequency is {\it a prioir} unknown, the resonant frequency of the cavity must be tuned and the possible axion frequencies are scanned.
...
Here, a monochromatic axion ($Q_{a}\to {\rm inf.}$) is assumed for simulation convenience.
The conversion power was calculated under a uniform magnetic field of 10~T in the height direction in a cylinder with a radius of 50~mm and a height of 100~mm.
The boundaries of the cylinder were set to copper.
The same values were used for the axion-photon coupling and the local dark matter density in the previous subsection.
The simulation results were compared with the conversion power of the analytic solution, as shown in Figure~\ref{fig:haloscope_simul}~(b).
...
The phase of the photons converted at each dielectric surface is the same within the axion de Broglie wavelength.
Given a dielectric surface at the wavelength interval of the converted photon, it coherently accumulates similar to a cavity haloscope, and the conversion power is boosted.
This enables an effective axion search for higher frequencies~\cite{PRL2017MADMAX}.

In this scheme, an analytic solution for a 1-dimensional array of dielectric plates has been studied~\cite{JCAP2017Millar}.
...
The conversion power obtained from the analytic solution and the power obtained from \texttt{COMSOL} are compared in Figure~\ref{fig:haloscope_simul}~(c).
...
The axion-to-photon coupling goes in twice, making it less sensitive than haloscope experiments.
...
One of the representative experiments is Axion-Photon Regeneration (APR)~\cite{PRL1987Bibber,PLB2010Ehret,PRD2015Ballou,PRL2007Sikivie}.
Photons propagating under a strong magnetic field induce an axion field by Equation~\ref{eq:axion_eom}.
...
In the simulation, the cavity was not long enough compared to the wavelength due to memory limitations, so a factor was additionally considered.
...
\bibitem{NPB1980SVZ} M. A. Shifman, A. I. Vainshtein and V. I. Zakharov, Nucl. Phys. B {\bf 166}, 4933 (1980).
\bibitem{YF1980Zhitnitsky} A. P. Zhitnitsky, Yad. Fiz. {\bf 31}, 497 (1980); Nucl. Phys. B {\bf 31}, 260 (1980).
\bibitem{PLB1981DFS} M. Dine, W. Fischler and M. Srednicki, Phys. Lett. B {\bf 104}, 199 (1981).
\bibitem{PRL1987Wilczek} F. Wilczek, Phys. Rev. Lett. {\bf 58}, 18 (1987).
\bibitem{PRD1985Sikivie} P. Sikivie, Phys. Rev. D {\bf 32}, 2988 (1985).
\bibitem{PRL2017MADMAX} MADMAX Working Group, Phys. Rev. Lett. {\bf 118}, 091801 (2017).
\bibitem{JCAP2013Horns} D. Horns {\it et al.}, J. Cosmol. Astropart. Phys. {\bf 2013.04}, 016 (2013).
...
\bibitem{JPG2014Read} J. I. Read, J. Phys. G {\bf 41}, 063101 (2014).
...
\bibitem{JCAP2020Kim} D. Kim {\it et al.}, J. Cosmol. Astropart. Phys. {\bf 2020.03}, 066 (2020).
\bibitem{JCAP2017Millar} A. J. Millar {\it et al.}, J. Cosmol. Astropart. Phys. {\bf 2017.01}, 061 (2017).
\bibitem{PLB2010Ehret} K. Ehret {\it et al.}, Phys. Let. B {\bf 689}, 149 (2010).
% \bibitem{PRL2019Lawson} M. Lawson {\it et al.}, Phys. Rev. Lett. {\bf 123}, 141802 (2019).
\bibitem{PRD2015Ballou} R. Ballou {\it et al.}, Phys. Rev. D {\bf 92}, 9 (2015).
