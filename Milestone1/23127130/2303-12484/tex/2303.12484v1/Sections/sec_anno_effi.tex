...
Several works propose to use CAM to generate object localization proposals or even to perform whole-object pixel-wise segmentation.
...
It integrates domain-specific anatomical information of breast ultrasound to reduce the search space for breast tumor segmentation.
Meanwhile, Chen \textit{et al.}~\cite{chen2022c} propose a causal CAM method for organ segmentation, which is based on the idea of causal inference with a category-causality chain and a anatomy-causality chain.
...
First, existing methods typically derived pseudo labels from the point annotation, which are based on strong constraints or assumption (e.g., Voronoi) from the data, restricting the generalization of these methods to other datasets~\cite{lin2022label}.
...
In a recent work, Zhang \textit{et al.}~\cite{zhang2022cyclemix} adopt mix augmentation and cycle consistency for the Scribble-Pixel method, demonstrating the improvement of both weakly and fully supervised segmentation methods.
