...
Based on the assumptions, MIL methods can perform both bag-level and instance-level tasks (illustrated in Fig. \ref{fig_mil_schematic}), with the latter often used in weakly-supervised learning. Concretely, MIL algorithms leverage the instances to identify positive or negative bags, which contributes not only to the image-level diagnosis, but also precise abnormal region detection and localization. This great interpretability of the MIL algorithm fits well in MIA, as both the global structure and local details are crucial components for solving such problems.

In this survey, we categorize MIL methods that aim at detecting all the particular target patterns in the data, such as every patch with a special disease manifestation in a large histopathology image, as \textbf{local detection}; and methods that aim at simply detecting whether or not the particular target patterns exist in the given sample as \textbf{global detection}. Tab. \ref{tab:mil} presents an overview of the representative publications of each method.

\subsection{Active Learning}
\input{Figures/fig_al_schematic.tex}
\textbf{Active learning (AL)} is a relatively understudied area in the MIA field. It attempts to maintain the performance of a deep learning model while annotating the fewest data with the help of an oracle, which resonates with the philosophy of label-efficient learning, \textit{i.e.}, how to effectively use noisy, limited and unannotated labels throughout the deep learning process. More specifically, its goal is to select the most valuable samples and forward them to the oracle (\textit{e.g.}, human annotator) for labeling to improve the generalization capability of the model. In AL practice, the annotation uncertainty is often considered as the sample valueness metric. Meanwhile, in order to maintain the generalization capability of the network, various mechanisms are developed to ensure the sampled images are distributed diversely. % since the annotation uncertainty tends to occur in ambiguous boundaries and the region with partial volume effect\cite{karimi2023learning}, etc. 

As Fig. \ref{fig_al_schematic} illustrates, before the start of the data selection process, a deep learning model is initialized or pre-trained from a labeled dataset $L$ with its corresponding parameter $\theta$. After that, AL sampling algorithms construct an uncertainty metric $\mathcal{U}$ for each item of unlabeled data from a large set $U$. This metric determines whether an oracle is required for annotation, forming a new labeled dataset $X_{L^{\prime}}, L^{\prime}\subset U$. Then the network model will either use $X_{\hat{L}}=X_{L_0}\cup X_{L^{\prime}}$ to train with the labeled data from scratch or only use $X_{\hat{L}}=X_{L^{\prime}}$ to fine-tune the model. Denoting the fully labeled version of $U$ as $L$, the goal of AL is to build a model $f(\theta\mid X_L^{*})$ with $X_L^{*}
\ll L$ to perform equivalently or better than $f(\theta\mid L)$. 
...
% The use of deep learning methods with partially labeled data for dense predictions to improve labeling efficiency is here dubbed annotation-efficient learning. 
...
Fig.~\ref{fig_anno_effi} shows different forms of annotation, %including pixel-wise annotation, box, scribble, and point annotation. 
