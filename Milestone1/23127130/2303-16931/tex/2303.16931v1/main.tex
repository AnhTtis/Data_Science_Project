...
\documentclass[prl,aps,showpacs,twocolumn,unsortedaddress,longbibliography,superscriptaddress]{revtex4-2}
...
Recently there has been a great deal of activity investigating non-reciprocal effects and supercurrent rectification in superconducting thin films~\cite{Silaev2014, Wakatsuki2017, Yasuda2019, Ando2020, Shin2021, Lyu2021, Daido2022, Yuan2022, Illic2022, He2022, Karabassov2022, Bauriedl2022} and Josephson junctions~\cite{Hu2007, Shi2015, Bocquillon2017, Misaki2021, Baumgartner2022_JPhys, Baumgartner2022_NatNano, Jeon2022, Halterman2022, Pal2022, Wu2022, Kokkeler2022, Zhang2022, Davydova2022, Trahms2023}. 
...
In this Letter we study the DC and AC response of asymmetric SQUIDs~\cite{Fulton1972}.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=1.99\columnwidth]{letter_fig1_v4.pdf} 
    \caption{ 
...
We can describe the fluxoid quantization condition with %topologically trivial superconducting electrodes 
...
$\hat{\Phi} = \Phi/\Phi_0$. 
    ...
    \end{align}
    We assume $a_1 + b_1 + c_1 = 1$ and $a_2 + b_2 + c_2 = \Delta_{21}$ throughout the paper for simplicity. Furthermore, we assume $\Delta_{21} = 1$ throughout the Letter which implies the gaps of the junctions in the SQUID are the same. Following previous work \cite{Romeo2005}, we can reduce the SQUID dynamical equations to a single equation of motion by considering $\beta_L, ~\vert 1 - R_{21} \vert \ll 1$. Retaining terms linear in $\beta_L$, the SQUID dynamics are determined by the average phase $\phi_A$
%
\begin{equation}
...
$\beta_L/(1+R_{21})$~\footnote{$x_m$ and $y_m$ are defined explicitly in the Supplementary Information.}. 
The diode efficiency $\eta_c \equiv  (I_+ - I_-)/(I_+ + I_-)$ is often used to characterize superconducting diodes where the critical current $I_{+}$ ($-I_-$) corresponds to positive (negative) current bias. We extract $I_{\pm}$ from Eq.~(\ref{eq:dPhiA}) where the last two terms describe an effective CPR. First, it is worth noting that the effect of screening enters the dynamics via the term $\beta_L/(1+R_{21})$ suggesting an increase of $R_{21}$ is similar to a decrease of $\beta_L$. Second, the last term in Eq.~(\ref{eq:dPhiA}) is independent of $\phi_A$ but odd in $\hat{\Phi}$. This term applies an overall shift in the CPR which suggests a bipartite form of the diode effect 
...
to anomalous scenarios such as $\vert \eta_c \vert >1$. %or current-bias offsets in Shapiro step maps~\cite{Souto2022}.

    We start by considering two types of SQUIDs. The first is a 2$\pi$-4$\pi$ SQUID with trivial supercurrent in the topological arm characterized by the parameter $W_{4\pi} = b_2 = 1 - a_2$. The second is a trivial asymmetric SQUID (skewed SQUID) with $a_1 = 1$ and $a_2 = 0.9 = 1 - c_2$ ($b_1 = 0 = b_2$).
...
Using $\vert \Delta I_c \vert$ as an additional quality-factor we find that the 2$\pi$-4$\pi$ SQUID diode outperforms the trivial asymmetric SQUID (see SI).
...
\begin{figure}[htb!]
 ...
 %\noteER{The units of $V_{ac}$ on x-axes of bottom panels don't seem to match y-axes of top panels.}
 %AC power dependence of diode efficiency for an asymmetric SQUID with $a_1 = 1$, $a_2 = 0.5 = b_2$, $R_{21}=1$, (c) $\beta_L=0$ and (d) $\beta_L = 0.25$.
 }
\end{figure}
...
In this Letter, we studied the JDE in the DC and AC response of asymmetric SQUIDs, including the effects of inductance and asymmetries in $I_c$ and $R_n$. 
We showed that the inductance $\beta_L$ and the ratio $R_{21} = R_{n,2}/R_{n,1}$ can tune the diode efficiency of an asymmetric DC SQUID. For SQUIDs with a 4$\pi$ junction, tuning $\beta_L$ and $R_{21}$ can cause a switching on the diode polarity.
...
\section{I. 2$\pi$-4$\pi$ SQUID dynamics}
    ...
    \begin{subequations}
    \begin{equation}
        \frac{d^2\phi_1}{d\tau^{\prime~2}} + \sigma \frac{d\phi_1}{d\tau^{\prime}} = \frac{i_B}{2} - i_{s,~1}(\phi_1) + \frac{1}{4 \pi \beta_L} \left( \phi_2 - \phi_1 - 2\pi \hat{\Phi} \right)
    \end{equation}
    \begin{equation}
        C_{21} \frac{d^2\phi_2}{d\tau^{\prime~2}} + \frac{\sigma}{R_{21}}\frac{d\phi_2}{d\tau^{\prime}} = \frac{i_B}{2} - I_{21} i_{s,~2}(\phi_2) - \frac{1}{4 \pi \beta_L} \left( \phi_2 - \phi_1 - 2\pi \hat{\Phi} \right)
    \end{equation}
    \end{subequations}
    ...
    \begin{align}
        \tilde{i}_s(\phi_A, \hat{\Phi}) =~& \frac{1}{2} \sin\left( \frac{\phi_A+\pi \hat{\Phi}}{2} \right) + \frac{1}{2} \sin\left( \phi_A - \pi \hat{\Phi} \right) - \frac{\pi \beta_L}{8} \left[ 2 \sin\left( 2(\phi_A -\pi\hat{\Phi})\right) \right] \\ \nonumber
        -~& \frac{\pi \beta_L}{8} \left[ \sin\left(\frac{\phi_A-3\pi \hat{\Phi}}{2} \right) - 3\sin\left(\frac{3\phi_A - \pi \hat{\Phi}}{2} \right) + \sin\left( \phi_A + \pi\hat{\Phi} \right) \right].
    \end{align}   
...
\section{II. Symmetric SQUID with $\pi$-, $2\pi$-, \& $4\pi$-periodic Channels} 
...
\begin{align}
    \Psi_1(\tau) = -2 \left( \alpha \sin(\pi \hat{\Phi}/2) \cos(\phi_A/2) + \tilde{\beta} \sin(2 \pi \hat{\Phi}) \cos(2\phi_A) + \sin(\pi \hat{\Phi}) \cos(\phi_A) \right).
    \label{PsiSoln}
\end{align}
...
\begin{align}
    ...
    &+ d e^{4i(\phi_0 + \omega_0 \tau)} J_n(4z) + f e^{\frac{1}{2}i(\phi_0 + \omega_0 \tau)} J_n(z/2) \nonumber \\
    &+ g e^{\frac{3}{2}i(\phi_0 + \omega_0 \tau)} J_n(3z/2) + h e^{\frac{5}{2}i(\phi_0 + \omega_0 \tau)} J_n(5z/2)] \}
\end{align}

\section{III. Asymmetric SQUID dynamics}
    Now we assume a general CPR with $\pi$-, $2\pi$-, and $4\pi$-periodic channels,
    \begin{align}
        i_{s,~1}(\phi_1) & = a_1 \sin(\phi_1) + b_1 \sin(\phi_1/2) + c_1 \sin(2 \phi_1) \\
    ...
    \begin{align}
        ...
        & + x_3 \sin(3\phi_A/2) + x_5 \sin(5\phi_A/2) + y_2 \cos(\phi_A) \nonumber \\
        &+ y_4 \cos(2 \phi_A) + y_6 \cos(3 \phi_A) + y_8 \cos(4 \phi_A) \nonumber \\  
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.8\columnwidth]{letter_figS1_v1.pdf} 
\caption{ 
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.8\columnwidth]{letter_figS2_v1.pdf} 
\caption{ 
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.8\columnwidth]{letter_figS3_v1.pdf} 
\caption{ 
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.8\columnwidth]{letter_figS4_v1.pdf} 
\caption{ 
