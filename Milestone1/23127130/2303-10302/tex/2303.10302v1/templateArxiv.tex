...
An algorithm for solving cost-constrained POMDPs has been proposed in \cite{CC-POMCP}. However, this algorithm becomes computationally infeasible for multi-component POMDPs with very large state spaces. A POMDP-based solution for optimal maintenance and inspection of structures using Dynamic Bayesian Networks is presented in \cite{optmaint}. However, in addition to computational infeasibility, this algorithm does not account for budget constraints.  A method for solving budget-constrained MDPs has been presented in \cite{cmdps} where they introduce a budgeted-MDP model and then calculate the optimal budget split by posing it as a multi-choice knapsack problem. The budgeted-MDP model includes the budget as an implicit part of the state. This is because they use a cost function, similar to Constrained MDPs (CMDPs) \cite{cmdp1}, to keep a track of the cost incurred by the policy. Hence, this algorithm cannot be directly extended to POMDPs because the partial observability of the state would cause a violation of the budget constraint in some cases. Also, finding the optimal budget split by solving the multi-choice knapsack problem is not suitable for applications like optimal maintenance and inspection scheduling for a group of infrastructure components.

In this work, we propose an algorithm for optimal policy synthesis of a multi-component budget-constrained POMDP by splitting it into individual component POMDPs through a priori allocation of the total budget. Our contribution here is two-fold. First we introduce the b-POMDP model which includes the total cost incurred upto a time instant $k$ explicitly as a part of the state vector and hence facilitates strict adherence to the budget constraint. Next, we show that the optimal value function for budget-constrained POMDPs is a concave function of the budget for a unit maintenance cost. 
We then compute the optimal policy for the individual component POMDPs by modelling them as b-POMDPs and using an online solver like POMCP \cite{NIPS2010_edfbe1af}. Doing so gives us the approximate maximal total reward collected by the policy in terms of the budget allocated to the b-POMDP. Our second contribution involves finding the approximately optimal policy of the multi-component POMDP by calculating optimum distribution of the total budget among the individual component b-POMDPs. The budget splitting is posed as a welfare maximization problem where we maximize the sum of the optimal value functions of these b-POMDPs with respect to the budget while adhering to the total budget constraint of the multi-component POMDP. The concave nature of the value function renders this as a convex minimization problem with linear constraints thus guaranteeing a well-defined optimum. The utility of the proposed algorithm is demonstrated by performing simulations for finding optimal maintenance and inspection policies for multiple components of a general administrative building, subject to a budget. Performing simulations on real data for the components, we show that our algorithm vastly outperforms the policy currently used in practice.
...
Given a finite set $\actions$, $|\actions|$ denotes its cardinality. $2^{\actions}$ denotes the power set of $\actions$, and $\Delta(\actions)$ denotes the set of all probability distributions over the set $\actions$. Notation $\mathbb{N}_0$ denotes the set of natural numbers including 0 i.e. $\mathbb{N}_0 = \{0,1,2, \ldots\}$. The symbol $\lfloor . \rfloor$ denotes the floor function and $\lceil . \rceil$ denotes the ceiling function.

\subsection{Partially Observable Markov Decision Process}
A discrete-time finite-horizon POMDP \cite{pomdpcassandra, Braziunas-POMDPsurvey} $M$ is specified by the 8-tuple $(\states, A, \pi, T, \Omega, O, R, H)$, where:

\begin{itemize}
    \item $\states$ denotes a finite set of states.
    \item $A$ denotes a finite set of actions
    \item $\pi : \states \rightarrow A$ denotes the policy which specifies the action to take in a given state $s$
    \item $T :  \states\times A \rightarrow \Delta(\states)$ denotes the transition probability function where $\Delta(\states)$ is the space of probability distributions over $\states$. 
    \item $\Omega$ denotes a finite set of observations.
    \item $O : \Omega\times\states\times A \rightarrow \Delta(\Omega)$ denotes the observation probability function where $\Delta(\Omega)$ is analogous to $\Delta(\states)$ as defined for $T$.
    \item $R : \states \times A \rightarrow [-R_\tss{max}, R_\tss{max}]$ denotes the bounded reward function. 
    \item $H \in \mathbb{N}_0$ denotes the finite planning horizon for the problem.
\end{itemize}

For the above POMDP, at each time step, the environment is in some state $s \in \states$ and the agent interacts with the environment by taking an action $a \in A$. Doing so results in the environment transitioning to a new state $s^{\prime} \in \states$ in the next time step with probability $T(s^{\prime}|s,a)$. Simultaneously, the agent receives an observation $o \in \Omega$ regarding the state of the environment with probability $O(o|s^{\prime},a)$ which depends on the new state of the environment and the action taken by the agent. The agent also receives a reward $R(s,a)$. 

The problem of optimal policy synthesis for a finite-horizon POMDP is that of choosing a sequence of actions which maximizes the expected future reward $\EE[\sum_{t=0}^{H}r_t]$ where $r_t$ is the reward earned at time instant $t$. In a POMDP the agent doesn't have access to the true state of the environment. However, the agent can update it's belief about the true state of the environment by interacting with the environment. Hence the optimal behavior may often include actions which are taken simply because they improve the agent's belief about the true state. After reaching $s^{\prime}$, the agent receives observation $o \in \Omega$ with probability $O(o|s^{\prime},a)$. Let the belief $x$ be a probability distribution over $\states$. Then, $x(s)$ denotes the belief state and the agent updates the belief state \cite{pruning} according to Bayes' rule. The updated belief state $x^\prime(s^\prime)$ is given by:
\begin{equation*}
    x^{\prime}(s^{\prime}) = \eta O(o|s^{\prime},a)\sum_{s \in \states}T(s^{\prime}|s,a)x(s)
\end{equation*}
where $\eta$ is a normalizing constant and is defined as:
% and is defined as:
\begin{equation*}
    \eta = \frac{1}{\sum_{s^\prime \in \states}O(o|s^\prime,a)\sum_{s \in \states}T(s^\prime|s,a)x(s)}.
\end{equation*}


\section{Problem Formulation}
\label{sec:problem}
We consider a multi-component POMDP, which is a collection of $n$ component POMDPs. The component POMDPs are weakly-coupled in the sense that they have independent transition probabilities and are connected only by the shared total budget. In this paper, we consider optimal policy synthesis for POMDPs with budget, i.e., each action incurs a cost and the total cost incurred by the optimal policy is limited by the budget. We first formally define the multi-component POMDP with a budget and then define the problem of finding the optimal policy for the multi-component POMDP within such a budget.

\subsection{Multi-Component Decoupled POMDP with Shared Budget}
\label{subsec:multicomponent}
For a multi-component POMDP, the state space $\states \subseteq \mathbb{N}_0^n$ is given by $\states = \states^1 \times \states^2 \times \ldots \times \states^n$ where $\states^i \in \mathbb{N}_0$ denotes the state space for component $i$ for all $i \in \{1,2,\ldots,n\}$. The state space for each component $i$ is $\states^i = \{0,1,2, \ldots, s_{max}\}$, where $s_{max} \in \mathbb{N}_0$.The state $s_k \in \states$, at time step $k$ is given by $s_k = \{s^1_k,s^2_k, \ldots, s^n_k\}$ where $s^i_k \in \states^i$ represents the state of component $i$ at time step $k$. 
...
\begin{equation*}
    A = \prod_{i = 1}^n A^i,
\end{equation*}
where the action space $A^i$ for component $i$ is given by $A^i = \{d^i,i^i,m^i\}$. $d^i$ is the do-nothing action and it lets the component move to a new state according to the transition probabilities. The action $i^i$ provides an observation which is equal to the next state $s^{\prime,i}$ of the component and action $m^i$ drives the component state transition to state $s_{max}$. The transition probability function for the multi-component POMDP for $s,s^\prime \in \states$ and $a \in A$ is given by
\begin{equation*}
    T(s,a,s^\prime) = \prod_{i=1}^n T^i(s^i,a^i,s^{\prime,i}).
\end{equation*}
$T^i$ denotes the transition probability function for component $i$ and is defined as
\begin{equation}
    T^i(s^{\prime,i}|s,a) = \begin{cases}
    1,& \text{if } s^{\prime,i} = s_{max} \text{ and } a^i = m, \\
    p^i(s^i,a^i,s^{\prime,i}),& \text{if } s^{\prime,i} \leq s^i \text{ and } a^i \in \{d, i\},\\
    1,& \text{if } s^{\prime,i} = 0 = s^i \text{ and } a^i \in A,\\
    0,& \text{otherwise}.
    \end{cases}, \label{transition}
\end{equation}
The probability $p^i(s^i,a^i,s^{\prime,i})$ is chosen according to some probability distribution specific to component $i$. Clearly, from \eqref{transition} it can be observed that $0$ is an absorbing state of component $i$. 
...
\begin{equation*}
    O(s^\prime,a,o) = \prod_{i=0}^n O^i(s^{\prime,i},a^i,o^i).
\end{equation*}
Here, $O^i$ is the observation probability function for component $i$ and is defined as
\begin{equation*}
    O^i(o^i|s^{\prime,i},a^i) = \begin{cases}
        1,& \text{if } o^i = s^{\prime,i} \text{ and } a^i \in \{i, m\} \\
        1,& \text{if } o^i = e \text{ and } a = d \\
        0,& \text{otherwise}.
    \end{cases}
\end{equation*}
...
For each component $i$, each action $d^i,i^i$ and $m^i$ incurs a cost $c_d^i, c_i^i$ and $c_m^i$ respectively, against a total budget $B$. 
...
For a multi-component POMDP, given by the formulation in the previous section, we consider the problem of finding an optimal policy $\pi^*$ which maximizes the time before reaching the absorbing state. Mathematically, $\pi^*$ maximizes $t$ such that $s_t > 0$. Furthermore, for a horizon of length $H$, $\pi^*$ should be such that the total cost incurred for the multi-component POMDP, $C_H$, does not exceed the total budget. Assume that, for a horizon $H$, the number of $d^i,i^i$ and $m^i$ actions taken, according to the optimal policy, for component $i$  are $n^i_d,n^i_i$ and $n^i_m$ respectively. Then
\begin{equation*}
    C_H = \sum_{i=1}^n (n^i_dc^i_d + n^i_ic^i_i + n^i_mc^i_m) \leq B,
\end{equation*}
We propose an approximately optimal solution for the above problem through a two-step approach. In the first step, we will solve a single-component POMDP for any given budget. In the second step, we will partition the total budget $B$ into budgets for individual components of an $n$-component POMDP.
...
\label{sec:solution}
In this section, we detail our methodology for solving the problem of optimal policy synthesis of a multi-component POMDP. First, we introduce the budgeted-POMDP model and discuss how to solve a single budgeted-POMDP with the transition, observation and reward functions similar to those mentioned in the previous section. Next, we discuss why the value function for such a POMDP is a concave function of the budget. Finally, we present our proposed method for solving $n$ such POMDPs, constrained by a total budget, by finding the optimal budget split among the POMDPs. 

\subsection{Budgeted-POMDP Model (b-POMDP)}
\label{subsec:budgeted-POMDP}

The main goal of our agent is to find an optimal policy for the POMDP while adhering to a total budget for its actions. A budgeted-MDP model is presented in \cite{cmdps}. Here, the budget is treated as a part of the state implicitly because they use a cost function, similar to Constrained MDPs (CMDPs) \cite{cmdp1}, to keep a track of the cost incurred by the policy. However, this model can't be extended directly to a POMDP because the partial observability of the state would lead to budget violation in some cases. Hence, we introduce a new b-POMDP model. In a b-POMDP, the budget constraint is incorporated by introducing an extra component, in each state of the state space, representing the total cost incurred upto time instant k. So, if we consider a single component POMDP with a total budget $B$, the modified state at time instant $k$ according to the b-POMDP formulation is given by $(s_k,c_k)$ where $s_k$ is as defined in the previous section, for $i = 1$. We assume that unlike $s_k$, $c_k$ is completely observable at all time instants. The transition function for the cost component of the state is given by:
\begin{equation*}
    T_c(c^{\prime}|c,a) = \begin{cases}
    1,& \text{if } c^{\prime} = c+c_m \text{ and } a = m \\
    1,& \text{if } c^{\prime} = c+c_i \text{ and } a = i \\
    1,& \text{if } c^{\prime} = c+c_d \text{ and } a = d \\
    0,& \text{ otherwise }.
    \end{cases}
\end{equation*}
The transition function for the overall budgeted-POMDP is given by:
\begin{equation}
    T^\prime((s^\prime,c^\prime)|(s,c),a) = T(s^\prime|s,a)T_c(c^\prime|c,a),
\end{equation}
where $T(s^\prime|s,a)$ is given by \eqref{transition}. Furthermore, since we now have the cost incurred explicitly as part of the state vector, we can prevent the policy from taking actions which would result in $c_k > B$ at any time instant $k$. Such a modification can be easily performed by making $c_k > min\{B-c_m,B-c_i\}$ an absorbing state, similar to $s=0$. The reward function for the b-POMDP is given by:
\begin{equation*}
...
To find an optimal policy for a b-POMDP, we use the method of Monte-Carlo Planning in POMDPs (POMCP \cite{NIPS2010_edfbe1af}). POMCP is an online planning algorithm for large POMDPs, which combines a Monte-Carlo update of the agent’s belief with a Monte-Carlo tree search for the best action from the current belief state. Each node of the tree represents a belief state and action pair and at each node, POMCP simulates a rollout by selecting actions according to a policy and updating the belief state based on the observations. Furthermore, POMCP uses an Upper Confidence Bound (UCB) heuristic to balance exploration and exploitation during this tree search. For a b-POMDP, the maximal collected reward (value function) obtained using POMCP will be a function of the budget $B$ associated with it. We will now prove that this value function is concave in the budget by considering a simple MDP which is a special subclass of our overall problem.

\subsection{Proof for Concavity of Optimal Value Function}
\label{subsec:proof}
Consider an MDP with state space $\states_{MDP} = \{0,1,2 \ldots s_{max}\}$, where $s_{max} \in \mathbb{N}_0$ and action space is $A_{MDP} = \{m,o\}$. The state of the system decreases by a value $d \in \mathbb{N}_0$ unless we perform action $m$. The transition probability function is defined as:
\begin{equation*}
    T(s^{\prime}|s,a) = \begin{cases}
    1,& \text{if } s^{\prime} = s_{max} \text{ and } a = m \\
    1,& \text{if } s^{\prime} = s-d \text{ and } a = o\\
...
Also, $d$ is a decrease in the state value. The cost for the $o$ action is $c_o = 0$ and the cost for the $m$ action is $c_m > 0$. For simplicity, assume that $c_m = 1$. Let the available budget be denoted by $b \in \mathbb{N}_0$. This means that we can perform the action $m$ at most $b$ times. 
...
\begin{lemma}\label{lemma:3}
    For a given initial state $s_0 > d$ and horizon $H$, if $b > 0$, the optimal action to take at time $t=0$ is $o$.
\end{lemma}
\begin{proof}
    We will compare four cases:
\begin{enumerate}
    \item We first take an action m and then action o. Doing so yields the maximal collected reward $V_{H}^{mo}$ given by 
    \begin{equation}
    V_{H}^{mo}(s_0,b) = 2r + V_{H-2}(s_{max}-d,b-1). \label{mo}
    \end{equation}
    \item We first take an action o and then action m. Doing so yields the maximal collected reward $V_{H+1}^{om}$ given by
    \begin{equation}
    V_{H}^{om}(s_0,b) = 2r + V_{H-2}(s_{max},b-1).\label{om}
    \end{equation}
    ...
    \end{equation}
    \item We first take an action o and then action o. Doing so yields the maximal collected reward $V_{H+1}^{oo}$ given by
    \begin{equation}
    V_{H}^{oo}(s_0,b) = 2r + V_{H-2}(s_0-2d,b).\label{oo}
    \end{equation}
\end{enumerate}
...
\begin{equation}
    V_{H}^{om} = \max\{V_{H}^{mo}, V_{H}^{om}, V_{H}^{mm}\}. \label{o_optimal}
\end{equation}
We don't need to compare $V_{H}^{oo}(s_0,b)$ because \eqref{o_optimal} is sufficient to prove that for $s_0 > d$, action $o$ is the optimal initial action.
\end{proof}

\begin{lemma}\label{lemma:4}
    For a given initial state $0 < s_0 \leq d$ and horizon $H$, if $b > 0$, the optimal action to take at time $t=0$ is $m$.
\end{lemma}
\begin{proof}
    We will consider two cases:
    \begin{enumerate}
        \item We take action $o$ in the first step. Doing so yields the maximal collected reward $V_H^o$ given by
        \begin{equation}
            V^o_{H}(s_0,b) = r + V_{H-1}(0,b), \label{ofirst}\\
        \end{equation}
        \item We take action $o$ in the first step. Doing so yields the maximal collected reward $V_H^m$ given by
        \begin{equation}
            V^m_{H}(s_0,b) = r + V_{H-1}(s_{max},b-1), \label{mfirst}
        \end{equation}
    \end{enumerate}
Clearly from \eqref{ofirst} and \eqref{mfirst}, taking action $o$ in the first step leads to the absorbing state and hence we get a total reward of $r$. Taking action $m$ gives the same reward $r$ and drives the state to $s_{max}$. This implies that the system will not reach the absorbing state for atleast one more step and hence leads to higher total reward. Thus, we have
\begin{equation*}
    V^m_{H}(s_0,b) > V^o_{H}(s_0,b).
\end{equation*}
Hence, we can say that if $b > 0$, action $m$ is the optimal action to perform at $t=0$ when $0 < s_0 \leq d$.
\end{proof}
...
\end{itemize}
As can be clearly seen, the above assumptions hold true for $H=0$ as we proved previously. Now, assume that they hold for some $H > 0$ and consider a horizon of length $H+1$. We will prove that the assumptions hold true for horizon $H+1$ when $s_0>d$ and when $s_0 \leq d$.
...
First we will consider $s_0 > d$. Using Lemma \eqref{lemma:3} we know that, if $b > 0$, action $o$ is the optimal action to take at time $t=0$ when $s_0 > d$. Hence for horizon $H+1$, if $s_0 > d$, the value function is given by:
\begin{equation}
    V_{H+1}(s_0,b) = r + V_H(s_0-d,b), \quad \forall b \geq 0 \label{V:s>d}
\end{equation}
We will now consider two cases:
\begin{enumerate}
    \item Case 1 : $s_0-d > d$. In this case, using the results for horizon $H$, we get that $V_{H+1}(s_0,b)$ becomes constant in budget for all $b \geq \lfloor H/2 \rfloor$. Using the properties of the floor function, we have:
    \begin{equation*}
        \lfloor (H+1)/2 \rfloor \geq \lfloor H/2 \rfloor.
    \end{equation*}
    \item Case 2 : $s_0-d \leq d$. In this case, using the results for horizon $H$, we get that $V_{H+1}(s_0,b)$ becomes constant in budget for all $b \geq \lceil H/2 \rceil$. Using the properties of the ceiling and floor functions, we have:
    \begin{equation*}
...
\begin{equation*}
    % \begin{aligned}
    V_{H+1}(s_0,b^{\prime\prime}) - V_{H+1}(s_0,b^\prime) = V_H(s_0-d,b^{\prime\prime}) - V_H(s_0-d,b^\prime)) \leq V_H(s_0-d,b^\prime) - V_H(s_0-d,b),
    % \end{aligned}
\end{equation*}
...
\begin{equation*}
    V_{H+1}(s_0,b^\prime) - V_{H+1}(s_0,b) = V_H(s_0-d,b^\prime) - V_H(s_0-d,b)
\end{equation*}
Hence, from this we can say that:
\begin{equation*}
    V_{H+1}(s_0,b^{\prime\prime}) - V_{H+1}(s_0,b^\prime) \leq V_{H+1}(s_0,b^\prime) - V_{H+1}(s_0,b),
\end{equation*}
and thus, relation \eqref{relation} holds true for horizon $H+1$ when $s_0 > d$.
...
Using Lemma \eqref{lemma:4} we know that, if $b > 0$, action $m$ is the optimal action to take at time $t=0$ when $0 < s_0 \leq d$. Hence for horizon $H+1$, if $0 < s_0 \leq d$, the value function is given by:
\begin{equation}
    V_{H+1}(s_0,b) = \begin{cases}
        r + V_H(s_0-d,b),& \text{if } b = 0\\
...
Note that if $s_0 = 0$, then we are in the absorbing state and we get $V(0,b) = 0$ for all $b \geq 0$. Thus, we can say that $V(0,b)$ is constant with respect to budget for all $b \geq \lceil (H+1)/2 \rceil$.
 ...
 % \end{equation*}
 Hence, we can say that for a horizon $H+1$ and $s_0 \leq d$, the value function $V_{H+1}(s_0,b)$ becomes constant in b for all $b \geq \lceil (H+1)/2 \rceil$. 
 
Now, to prove that relation \eqref{relation} holds for $s_0 \leq d$, we will consider two cases:
\begin{enumerate}
    ...
    \begin{equation*}
        V_{H+1}(s_0,b^{\prime\prime}) - V_{H+1}(s_0,b^\prime) \leq V_{H+1}(s_0,b^\prime) - V_{H+1}(s_0,b)
    \end{equation*}
    \item Consider three budget values $b^{\prime\prime} = b^\prime + 1 = b + 2$ such that $b=0$. If we are at $s = s_{max}$, we will reach $s=0$ in $\lceil \frac{s_{max}}{d} \rceil$ steps if we take only action $o$ repeatedly. We will now consider two cases:
    \begin{enumerate}
        \item Case 1 :  $\lceil \frac{s_{max}}{d} \rceil < H$.
        Using \eqref{V:s<d} we have:
    \begin{gather*}
        V_{H+1}(s_0,b) = r + V_H(0,b) = r\\
        V_{H+1}(s_0,b^\prime) = r + V_H(s_{max},b^\prime-1) = (\lceil \frac{s_{max}}{d} \rceil + 1)r\\
        V_{H+1}(s_0,b^{\prime\prime}) = r + V_H(s_{max},b^{\prime\prime}-1) = (2\lceil \frac{s_{max}}{d} \rceil)r.
    \end{gather*}
    Using the above equations, we have:
    \begin{gather*}
        V_{H+1}(s_0,b^{\prime\prime}) - V_{H+1}(s_0,b^\prime) = (\lceil \frac{s_{max}}{d} \rceil-1)r\\
        V_{H+1}(s_0,b^\prime) - V_{H+1}(s_0,b) = (\lceil \frac{s_{max}}{d} \rceil)r,
    \end{gather*}
    ...
    \end{equation*}
        ...
        \item Case 2 : $\lceil \frac{s_{max}}{d} \rceil \geq H$. Using \eqref{V:s<d} we have:
        \begin{gather*}
        V_{H+1}(s_0,b) = r + V_H(0,b) = r\\
...
\end{enumerate}
 ...
 Hence, we can say that relation \eqref{relation} holds for horizon $H$ when $s_0 \leq d$. 

 

 Thus, using induction, we have shown that for any horizon of length $H \geq 0$ the following holds:
 \begin{itemize}
     \item The value function becomes constant with respect to the budget for $b \geq \lfloor H/2 \rfloor$ if $s_0 > d$ and $b \geq \lceil H/2 \rceil$ if $s_0 < d$.
...
\subsection{Proposed Approach for Optimal Policy Synthesis for Multi-Component POMDP with a Budget}
Consider a multi-component POMDP with $n$ components and budget $B$ as described in Section ~\ref{subsec:multicomponent}. If we consider an $n$-component POMDP, the size of the state space would be $(|\states|)^n$ where $|\states|$ is the size of the state space of each component POMDP. Also, the size of the total action space will be $3^n$. Directly applying a POMDP solver to such a large state and action space may not be computationally feasible. Hence, we propose an algorithm which decouples the $n$ component POMDPs by allocating a portion of the total budget to each of them prior to the beginning of the system run. We then compute the  approximate value function for each component POMDP as a function of the budget and then using that, obtain the optimal split of the total budget among them.
...
\end{equation}
We now have $n$ independent POMDPs, where each POMDP has its own total budget. Hence, we formulate each of them as a b-POMDP and solve each b-POMDP using the POMCP algorithm as discussed in Section ~\ref{subsec:budgeted-POMDP}. The optimal policy and hence the maximal collected reward for the $i^{th}$ component b-POMDP will depend on the budget $b_i$ allocated to it. Let the maximal collected reward, for component $i$, obtained using the POMCP algorithm, for a given initial state $s_0$ and horizon $H$, be denoted by $V^i_H(s_0,b_i)$. We can then find the optimal budget split among the $n$ b-POMDPs by posing it as a welfare maximization problem. Welfare maximization is the concept of maximizing the overall well-being or welfare of a society, and is achieved by maximizing some measure of social welfare (e.g. maximal collected reward). We thus maximize the total maximal collected reward, for all components, with respect to $b_i$ while adhering to the constraint in \eqref{eq1}. Hence, mathematically
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Using the results we proved in Section \ref{subsec:proof}, we know that $V_H(s_0,b_i)$ is a concave function of $b_i$. The welfare maximization problem then becomes a constrained concave maximization or equivalently a constrained convex minimization problem. Hence, it can be solved easily and is guaranteed to have a global optimum. The solution to \eqref{maximize} provides the optimal budget allocation for each b-POMDP which in-turn gives us the optimal policy for all $n$ component POMDPs. Let $\pi_i : \states^i \rightarrow A^i$ be a policy for component $i$ with budget $b_i$ obtained using the POMCP algorithm. Then, we define the overall policy, for the $n$-component POMDP, $\pi : \states \rightarrow A$ by $$\pi = \prod_{i=1}^n \pi_i.$$
While such a policy is naturally not guaranteed to be generally optimal on the multi-component POMDP, it provably satisfies the budgetary constraints and performs well in practice. To illustrate its performance, we now move to the implementation and evaluation section. We compare the perfomance of the proposed algorithm with a realistic baseline for a multi-component infrastructure management scenario.  
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{ttfr_vs_budget.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{component_history.png}
  \caption{Sample condition index (CI) histories illustrating the performance of the proposed policy when compared to the baseline policy for the Boiler component with a replacement cost of 45 units, an inspection cost of 1 unit, and a total budget of 500 units. (a) CI history using proposed approach showing failure at 80 time steps. (b) Baseline approach failing at 39 time steps.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{allocation.png}
...
%     \begin{tabular}{c cc}
%         \toprule
%         Component Name & \multicolumn{2}{c}{Time-to-failure} \\
%         \cmidrule(lr){2-3}
%         & Baseline & Proposed \\
%         \midrule
%         Overall Building & $242 \pm 3$ & $515 \pm 8$ \\
%         \midrule
%         Roofing & $24 \pm 1$ & $25 \pm 2$ \\
%         Lighting Equipment & $10 \pm 1$ & $85 \pm 3$ \\
%         Carpeting &  $21 \pm 2$ & $18 \pm 1$ \\
%         \bottomrule
%     \end{tabular}
%     \vspace{0.3cm}
%     \caption{Comparison of baseline and proposed budget allocation approaches for the overall building and some representative components for an overall budget of 10,000 units.}
%     \label{tab:param}
% \end{table}
Figure \ref{fig:allocation} summarizes the TTF results from the baseline and the proposed approaches for all components in the building. The proposed allocation approach achieves an overall TTF of 1510, outperforming the baseline that achieves an overall TTF of 1355. Analyzing individual component data we observe that instances where the proposed approach underperforms the baseline exhibit only slight differences in TTF values. In contrast, when the proposed approach outperforms the baseline, we observe a significant improvement. Note that the proposed approach performs better in cases where we are tightly constrained. For components with low replacement cost, both approaches achieve a TTF of 100. 
...
There are two possible directions of future work. While the proposed approach makes it possible to compute an approximately optimal policy in a feasible amount of time, the computational cost of using the POMCP algorithm for each component is still high. Our first direction of work is to reduce this cost by incorporating a learning framework so as eliminate repeated runs of POMCP. Finally, the budget allocation scheme is fixed in the sense that the budget-split is done before the start of the planning horizon. The second direction of work is to derive an efficient and optimal dynamic budget allocation scheme to account for change in transition probabilities of the component states during the planning horizon and also account for a rolling or cyclical budget.
