...
\newif\ifAnonymousAuthors
%\AnonymousAuthorstrue
\AnonymousAuthorsfalse
...
\begin{abstract}
	...
	We further propose another class of explicit MSR code constructions (not optimal-access) with even smaller sub-packetization $s^{\left\lceil n/(s+1)\right\rceil }$ for all admissible values of $d$, making significant progress on another open problem in the survey.
...
For $d<n-1$, optimal-access MSR codes with the smallest sub-packetization $\ell=s^{\lceil\frac{n}{s}\rceil}$ and linear field size are only known for $d\in\{k+1,k+2,k+3\}$, which come from a recent paper~\cite{Vajha21}. Obtaining such code constructions for general values of $d$ was listed as an open problem in the survey paper \cite{CIT-115}:
...
In~\cite{Liu22, Zhang23, Li23}, explicit constructions of MSR codes with $\ell=s^{\left\lceil\frac{n}{2}\right\rceil}$ for any $d$ were presented. Another open problem in \cite{CIT-115} concerns MSR codes without the optimal-access requirement:
\begin{openproblem}\cite[Open Problem 4]{CIT-115}\label{op-general}
	Construct MSR codes with least-possible sub-packetization level for all $(n, k, d)$. (There is no optimal-access requirement here).
\end{openproblem}
...
\begin{table}[ht]
	...
	\begin{tabular}{|cl|c|c|l|}
		\hline
		\multicolumn{2}{|c|}{Sub-packetization $\ell$} & Optimal access      & Restrictions & References                                   \\
		\hline
		\hline
		$s^{n}$                                        &                     & yes          &                 & \cite[Section VIII]{Ye16}  \\
		\hline
		$s^{\left\lceil \frac{n}{2} \right\rceil}$     &                     & yes          &                 & \cite{Liu22}               \\
		\hline
		$s^{\left\lceil \frac{n}{s} \right\rceil}$     & (smallest possible) & yes          & $s\in\{2,3,4\}$ & \cite{Vajha21}             \\
		\hline
		$s^{\left\lceil \frac{n}{s} \right\rceil}$     & (smallest possible) & yes          &                 & Section~\ref{sect:consA}   \\
		\hline
		$s^{n}$                                        &                     & no           &                 & \cite[Section IV]{Ye16}    \\
		\hline
		$s^{n}$                                        &                     & no           &                 & \cite{CB}                  \\
		\hline
		$s^{\left\lceil \frac{n}{3} \right\rceil}$     & (best-known)        & no           & $d=k+1$         & \cite{LWHY22}              \\
		\hline
		$s^{\left\lceil \frac{n}{2} \right\rceil}$     &                     & no           &                 & \cite{Zhang23} \cite{Li23} \\
		\hline
		$s^{\left\lceil \frac{n}{s+1} \right\rceil}$   & (best-known)        & no           &                 & Section~\ref{sect:consB}   \\
		\hline
	\end{tabular}
...
In this paper, we propose two classes of explicit MSR code constructions with linear field size $q=O_s(n)$ for any repair degree $d$ between $k+1$ and $n-1$, where $O_s$ means that we treat $s$ as a constant\footnote{We can do so because $s$ is upper bounded by $r$, which is assumed to be a constant.}.
...
\subsection{Construction of \texorpdfstring{$(n,k,\ell = s^{n/s})$}{}  MSR code with optimal access property}\label{sect:consA}
...
The $s$ kernel matrices in group $a$ can be written as
\begin{align*}
	K_{a,0}^{(t)}   & =
...
\begin{lemma}\label{lemma:fieldsizeA}
	If $q \ge ns + \sum_{t = 1}^{s}\binom{s}{t}\frac{st(t-1)}{2}$, then we can find $ns$ distinct elements $\lambda_0,\dots, \lambda_{ns-1}$ from $\Fq$ satisfying \eqref{eq:localA} in $O_s(n)$ time.
\end{lemma}
...
\begin{equation}\label{eq:pcmA}
	H_{as+b} = A_{a,b}^{(r)}.
\end{equation}
The MDS property of our MSR codes defined by \eqref{eq:parity_check} and \eqref{eq:pcmA} will follow directly from
...
\subsection{Construction of \texorpdfstring{$(n,k,\ell = s^{n/(s+1)})$}{} MSR code}\label{sect:consB}
...
for each $a\in [n/(s+1)]$, the $s+1$ nodes $(C_{a(s+1)+b}: b\in [s+1])$ are in group $a$.
...
\begin{equation}\label{eq:kerB}
	...
	\right.
\end{equation}
Note that the first $s$ kernel matrices in group $a$, $K_{a,b}^{(t)}, b\in [s]$,
...
\begin{equation}\label{eq:matB_use_kerB}
	A_{a,b}^{(t)} = \bI_{\ell/s^{a+1}}\otimes K_{a,b}^{(t)}.
\end{equation}
We can see that $A_{a,b}^{(t)}$ is an $ \ell t\times \ell$ matrix over $\Fq$.
...
\begin{lemma}\label{lemma:fieldsizeB}
	If a prime power $q \ge ns + \sum_{t = 1}^{s+1}\binom{s+1}{t}\frac{st(t-1)}{2}$, then we can find $ns$ distinct elements $\lambda_0, \dots, \lambda_{ns-1}$ from $\Fq$ satisfying \eqref{eq:localB} in $O_s(n)$ time.
\end{lemma}
...
\begin{equation}\label{eq:pcmB}
	H_{a(s+1)+b} = A_{a,b}^{(r)}.
\end{equation}
The MDS property of our MSR codes defined by \eqref{eq:parity_check} and \eqref{eq:pcmB} will follow directly from
...
More precisely, for each $t \le 3$, if we choose $t$ distinct matrices from $H_0, H_1, H_2$ (or $H_3, H_4, H_5$), and only save the first $t$ rows of every block entry,
then the $9t\times 9t$ submatrix of $H_0, H_1, H_2$ (or $H_3, H_4, H_5$) is invertible.
...
Then we define a $3\times3$ square matrix
...
Furthermore, we have
$$\det(M^{(4)}_{0,\{0,1\}}\cdot[A_{0,0}^{(4)} \smat A_{0,1}^{(4)} \smat  A_{1,0}^{(4)} \smat  A_{1,1}^{(4)}]) = \det([A_{0,0}^{(2)} \smat A_{0,1}^{(2)}])\cdot \det([\tilde A_{1,0}^{(2)} \smat \tilde A_{1,1}^{(2)}])\not=0.$$
...
for $a\in [n/s]$, a positive integer $t$ and subset $\cB = \{b_1, b_2, \dots,\linebreak  b_{v}\}\subseteq [s]$ of size $v = |\cB|$, we define
...
\begin{proof}
	\begin{itemize}
		\item[(i)]
			...
			in which
			\begin{equation*}
				e_{i,j}(z) = \left\{ \begin{array}{ll}
					1 & \text{if~} i=j\text{~and~} z=v        \\
					0 & \text{if~} i=j\text{~and~}z>v         \\
					0 & \text{if~} i\neq j\text{~and~} z\ge v
				\end{array}\right.
			\end{equation*}
			such that
			...
			\begin{align*}
				\left[\begin{array}{l}
					      ...
					      \end{array}\right]
			\end{align*}
			to move the last $t-v$ rows of each block row of $E \cdot K_{0, \cB}^{(t)}$, in this case they are all-zero rows, to the bottom.
		\item[(ii)]
			...
			This implies that
			\begin{align*}
				(\be_{i,0}^{(c)}, \be_{i,1}^{(c)}, \dots, \be_{i,s-1}^{(c)})\cdot(\bI_s\otimes L_m^{(t)}) =\lambda_m^{c} \cdot (\be_{i,0}, \be_{i,1}, \dots, \be_{i,s-1})\cdot (\bI_s\otimes L_m^{(t)}).
			\end{align*}
		...
		\item [(ii)] and for  any $0< a < n/s$, $b\in [s]$,
		      \begin{align*}
			      ...
			      \left[\begin{array}{c}
				            ...
				            \end{array} \hspace*{-0.05in}\right],
		      \end{align*}
		      where $\widehat{A}_{a,b}^{(t-v)}$ is column equivalent to $A_{a,b}^{(t-v)}$.
	\end{itemize}
\end{lemma}
\begin{proof}
	\begin{itemize}
	...
	\end{itemize}
\end{proof}
According to the permutation equivalence property of our MSR codes (see Appendix~\ref{sect:permu}), we can generalize Lemma~\ref{lemma:A0} as follows.
...
\begin{proof}[Proof of Lemma~\ref{lemma:globalA}]
	...
	If $c = 1$, since all the $\lambda_i$s satisfying local constraints~\eqref{eq:localA}, we have $\det(A_{a_0, \cB_0})\neq0$.
	...
	Therefore the matrix $[\widehat A_{a_1,\cB_1}^{(t-v)}\smat  \widehat A_{a_2,\cB_2}^{(t-v)} \smat  \cdots\smat  \widehat A_{a_c,\cB_c}^{(t-v)}]$ is also invertible since each $\widehat A_{a_i,\cB_i}^{(t-v)}$ is column equivalent to $A_{a_i,\cB_i}^{(t-v)}$.
	From~\eqref{eq:det}, we can see that $\det([ A_{a_0, \cB_0}^{(t)}\smat A_{a_1, \cB_1}^{(t)}\smat \dots \smat A_{a_{c}, \cB_{c}}^{(t)} ])\neq 0$.
\end{proof}
...
\subsection{Repair single node failure for \texorpdfstring{$(n,k,\ell = s^{n/s})$}{} MDS array code}\label{sect:repairA}
...
According to Appendix~\ref{sect:permu}, no matter which node fails, we can always convert it into group $0$.
Therefore, we only need to prove that every node in group $0$ can be repaired.
...
The other matrices $\bar{H}_{h}^{\left\langle c\right\rangle }, c\in [s]$ and $\bar{H}_{z}, z\in [s]\setminus\{h\}$ are block-diagonal matrices,
...
\subsection{Repair single node failure of \texorpdfstring{$(n,k,\ell = s^{n/(s+1)})$}{} MDS array code}\label{sect:repairB}
...
Next, we also use the nodes in group $0$ to illustrate the repair procedure.
...
\end{equation}
where all these $n+s-1$ matrices $\bar{H}_z, z\in[n]\setminus\{s\}$, $\bar{H}_s^{\left\langle c\right\rangle }, c\in [s]$ are $\ell/s \times \ell/s$ matrices, and
$\bar{C}_z, z\in[n]\setminus\{s\}$, $\bar{C}_s^{\left\langle c\right\rangle }, c\in [s]$ are column vectors of length $\ell/s$.
To be specific, we have
\begin{itemize}
...
The other matrices $\bar{H}_{z}, z\in [s]$ and $\bar{H}_{s}^{\left\langle c\right\rangle }, c\in [s]$ are block-diagonal matrices,
and the diagonal entries are the same within each matrix.
...
We choose any two distinct groups from these $n/s$ groups, say group $g$ and group $h$.
...
\begin{proof}
	...
	\begin{equation*}
		\bar{A}^{(t)}_{g,b}(i, j) = A_{g,b}^{(t)}(\bar{i}, \bar{j})= \left\{
		\begin{aligned}
...
\begin{equation*}
	\begin{aligned}
		  & (\bP_{g,h}\otimes \bI_{r})\cdot \sum_{a\in[n/s], b\in[s]}  A_{a,b}^{(r)}\cdot (\bP_{g,h}\cdot \bP_{g,h}) \cdot C_{as+b}   \\
		= & \sum_{a\in[n/s], b\in[s]} ((\bP_{g,h}\otimes \bI_{r})\cdot A_{a,b}^{(r)} \cdot \bP_{g,h})\cdot (\bP_{g,h} \cdot C_{as+b}) \\
...
We further construct a new code by exchanging the nodes in group $g$ with the nodes in group $h$ of $\bar{\cC}$.
...
then
$(\widehat{C}_0,\widehat{C}_1, \dots, \widehat{C}_{n-1})\in\widehat{\cC}$,
...
To be specific, for $i\in [s], j\in [t]$,  all the nonzero entries in the $it+j$th row of $K_{0,\cB}$ have the same degree $j$.
Therefore, $\det(K_{0,\cB})$ is a homogeneous polynomial.
Note that the term
$$
	\prod_{j\in [s]}1\cdot \prod_{j\in [s]} \lambda_{b_2s+j}^1 \cdot \prod_{j\in [s]} \lambda_{b_3s+j}^2  \cdots \prod_{j\in [s]} \lambda_{b_ts+j}^{t-1}
$$
appears in the expansion of $\det(K_{0,\cB})$, and can only be obtained by
picking constant entries from $K_{0,b_1}^{(|\cB|)}$, degree-$1$ entries from $K_{0, b_2}^{(|\cB|)}$, and so on.
...
$$
One can check that $D$ is a homogeneous polynomial %with $s^2$ indeterminates
of degree $\sum_{t=1}^{s}{\tbinom{s}{t}}\frac{st(t-1)}{2} = \alpha(s)$.
...
$D(\lambda_0, \lambda_1, \dots, \lambda_{s^2-1})\neq0$, and also the local constraints for group $0$.
More precisely, suppose that the coefficient of the term $\lambda_0^{i_0}\lambda_{1}^{i_1}\cdots \lambda_{s^2-1}^{i_{s^2-1}}$ is nonzero in the expansion of determinate $D$, where
$i_0 + i_1 + \cdots + i_{s^2-1} = \alpha(s)$, and let $S_0, S_1, \dots, S_{s^2-1}$ be $s^2$ disjoint subsets of $\Fq$  such that $|S_0|=i_0+1, |S_1|=i_1+1, \dots, |S_{s^2-1}|= i_{s^2-1}+1$. Then by brute force we can find $(\lambda_0, \lambda_1, \dots, \lambda_{s^2-1})\in S_0\times S_1 \times \cdots \times S_{s^2-1}$ such that $D(\lambda_0, \dots, \lambda_{s^2-1})\neq 0$ in $O_s(1)$ time.
