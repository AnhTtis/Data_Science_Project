...
\title{Pricing of Electricity Swaps with Geometric Averaging\thanks{We would like to thank Viviana Fanelli, Carme Frau, and Christa Cuchiero for their fruitful comments and suggestions. Financial support from the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) – SFB 1283/2 2021 – 317210226 is gratefully acknowledged.}}
...
\maketitle
\begin{abstract}
	\textit{In this paper, we provide empirical evidence on the \textit{market price of risk for delivery periods} (MPDP) of electricity swap contracts.
	...
	In preparation for empirical investigations, we adjust the work by \cite{Kemper2022} in two directions:
	...
	%Beyond jumps, the model is feasible for further characteristics of the market like mean-reversion, seasonalities in the delivery period, and the Samuelson effect.
	...
	In our empirical study, we analyze two types of models, characterized either by seasonality and or by term-structure effects, and identify the resulting MPDP in both cases.}
...
%	In this paper, we tackle a common approximation procedure for electricity swaps appearing in geometric settings to overcome pricing errors.
...
With the turn of the millennium, pricing derivatives on electricity has become important through the liberalization of the energy markets.
...
In electricity markets, futures contracts are the most important derivatives. They deliver the underlying over a period of time since electricity is not storable on a large scale. We therefore call them electricity \textit{swaps}.
...
In this paper, we provide empirical evidence for the MPDP. To do so, we extend their pricing approach  to the physical measure allowing for mean-reversion and jumps.
...
This goes back to the famous model by \cite{HJM}.
This approach was firstly connected to energy-related derivatives by \cite{ClewlowStrickland1999} and to electricity derivatives by \cite{Bjerksund} followed by a row of works (see, e.g., \Cref{tab:ClassificationSwapPriceModels} for geometric settings, \cite{Hinderks2020} for a structural model and \cite{CuchieroPersioGuidaSvalute2022} for measure-valued processes).\\
...
we stick to a geometric setting and compare the ladder averaging procedures adjusting the MPDP to a \citeauthor{Merton1976} type model.\\
...
Renewable energy, like wind and solar energy, intensify especially the seasonal effects mentioned before. Hence, the higher the market share of renewables, the more pronounced the MPDP will be. This applies especially for Germany, having ambitious plans for future investments in renewable energy.
...
We contribute to the literature by investigating the MPDP empirically, affected by seasonalities and the Samuelson effect.\\
...
While extending the paper by \cite{Kemper2022} to jumps, we establish the MPDP of jump risk whenever the jump coefficient relies on delivery-dependent effects. \\
...
First, we extend the paper by \cite{Kemper2022} to the jump case under the artificial risk-neutral measure leading to an extended characterization of the MPDP regarding diffusion \textit{and} jump risk.
...
as well as their risk-neutral measures revealing the decomposition of the market price of risk into the classical one and the MPDP.
...
We focus on an electricity swap contract delivering  1 MWh of electricity during the agreed delivery period $(\tau_1,\tau_2]$. 
...
We first model the solution of a futures contract and then derive the corresponding dynamics to avoid lacks of existence in the presence of jumps (see \cite{Papapantoleon}). At time $t\leq\tau$, let the logarithmic price process of the futures contract be defined as
	...
	\end{align}
	where $W^{\mathbb{Q}}$ is a standard Brownian motion under $\mathbb{Q}$ independent of the jump process $\widetilde{J}^{\mathbb{Q}}$. 
	...
	The last term of the logarithmic rate $Y$ in \Eqref{eq:YunderQ} defines its compensator under the current measure $\mathbb{Q}$:
	\begin{align}
	...
	In addition, $\psi^{\mathbb{Q}}(ir)$ is the integrand of the Lévy-Khintchine exponential  and the moment generating function is defined by
	\begin{align}
	...
	\end{align}
As $\sigma(t,\tau)$ depends on both, trading time $t$ and delivery time $\tau$, we allow for volatility structures as the Samuelson effect or seasonalities in the delivery time, which are addressed in \Cref{sec:DiscretizedMultiModel}.\\
...
To overcome this issue, \cite{Bjerksund} suggest an approximation in the continuous setting, which we call \textit{approximated averaging} since it is the arithmetic average of approximated logarithmic returns. Approximated averaging maintains the martingale property
...
\begin{Lemma} \label{lemma:dyn_Swap_Q}
	Let  \Cref{ass:TechnicalRequirementsQ2} in \Cref{app:tech_requ3} be satisfied.
	Under the artificial pricing measure $\mathbb{Q}$, the dynamics of the swap price, defined in \Eqref{eq:GeomAverage}, are given by
...
\end{Lemma}
\begin{proof}
	Plugging \Eqref{eq:futuresQsol1} into \Eqref{eq:GeomAverage} gives us
	\begin{align*}
	F(t,\tau_1,\tau_2)
	=F(0, \tau_1, \tau_{2})
	e^{\bar{X}(t,\tau_1, \tau_{2})}\;, 
	\end{align*}
	where $\bar{X}(t,\tau_1, \tau_{2}) := \int_{\tau_1}^{\tau_2}w(u, \tau_1, \tau_{2})\ln f(t,u)du$ is the swap price rate.
	Using the integral representation of the futures rate process from \Eqref{eq:futuresQsol1} and applying stochastic Fubini Theorem (see \cite{Protter2005}, cf.\ Theorem~65, Chapter IV.6) leads to
	\begin{align*}
	\bar{X}(t,\tau_1, \tau_{2})=&\int_{0}^{t}\mathbb{E} \Big[\sigma(s,U)\Big]dW^{\mathbb{Q}}_s+\int_{0}^{t}\mathbb{E} [\eta(s,U)]d\widetilde{J}^{\mathbb{Q}}_s-\frac12\int_{0}^{t}\mathbb{E} \Big[\sigma^2(s,U)\Big]ds
	-\int_{0}^{t}
	\mathbb{E} [\psi^{\mathbb{Q}}( \eta(s,U))]ds\;.
	\end{align*}
...
Our goal throughout this paper is to investigate the pricing spread between geometric and approximated averaging theoretically and empirically.\\
...
\begin{Definition} \label{Def_MPDP}
At time $t\in[0,\tau_1]$, the \textit{market price of jump and diffusion risk for delivery periods} associated to the delivery period $(\tau_1,\tau_2]$ is defined by
...
&\Pi_2^{\mathbb{Q}\widetilde{\mathbb{Q}}}(t, \tau_1,\tau_2):= -\frac{\mathbb{E} [\psi^{{\mathbb{Q}}}(\eta(t,U))]- \psi^{{\mathbb{Q}}}(\mathbb{E}[\eta(t,U)])}{\int_{\mathbb{R}}\left(e^{\mathbb{E} [\eta(t,U)]z}-1\right)\ell^{\mathbb{Q}}(dz) } = - \frac{\int_{\mathbb{R}}\mathbb{E} [e^{\eta(t,U)z}]-e^{\mathbb{E} [\eta(t,U)]z}\ell^{\mathbb{Q}}(dz)}{\int_{\mathbb{R}}\left(e^{\mathbb{E} [\eta(t,U)]z}-1\right)\ell^{\mathbb{Q}}(dz)}\;.\label{eq:AddMP2}
\end{align}
\end{Definition}
...
\begin{Remark} \label{re:1}
	...
	\item[$(ii)$] The MPDP of diffusion and jump risk is strengthened by delivery-dependent effects within the volatility and jump coefficients. For example, pronounced term-structure effects or seasonalities in the delivery period within these coefficients capture a distinct dependence on the delivery period and, consequently, lead to a high MPDP.
...
\begin{Proposition} \label{prop:FunderPtilde}
...
\begin{Corollary} \label{lem:differenceAveraging}
	...
	\item[$(ii)$]  The swap price $F$ is smaller or equal than $F^a$.
	The pricing spread is attained by
	\begin{align}
...
\begin{proof}
	...
	\end{align*}
	Hence, the difference between the approximated and exact solution is given by
	\begin{align*}
	...
	\end{align*}
	Since $\mathbb{V} [\sigma(\cdot,U)]\geq 0$ and $\ln \mathbb{E} [e^{\eta(\cdot,U)z}]\geq\mathbb{E} [\eta(\cdot,U)]z$ by Jensen, it follows that $D(t,\tau_1,\tau_2)\in (0,1]$ and thus $F\leq F^a$.	\qed
\end{itemize}
\end{proof}

We conclude that arithmetic and approximated averaging lead to higher swap prices than the geometric average. 
...
Vice versa, consider
	...
	We can thus use it to price options and other derivatives on the swap. In order to investigate the model empirically, we introduce the model under its physical measure $\mathbb{P}$ in the subsequent section.
...
In order to implement the drift feature and to investigate the model empirically, we derive the futures under the physical measure $\mathbb{P}$. Note that we will include mean-reversion at the futures and thus the swap's \textit{rate} level.
...
\begin{align}
\ln f(t,\tau) = \ln f(0,\tau) + Y(t,\tau)\;, \label{eq:LogFuturesP}
\end{align}
is now characterized by
the futures logarithmic rate component $Y$ under the physical measure given by
\begin{align}
dY(t,\tau)=\left(\mu(t,\tau)-\kappa(t)Y(t,\tau)\right)dt+ \sigma(t,\tau)dW^{\mathbb{P}}_t + \eta(t,\tau) d\widetilde{J}^{\mathbb{P}}_t\;,
\end{align}
for $Y(0,\tau)=0$, where $W^{\mathbb{P}}$ is a Brownian motion under the physical measure $\mathbb{P}$ independent of the compound compensated jump process $\widetilde{J}^{\mathbb{P}}$. In particular, $\widetilde{J}^{\mathbb{P}}$ is defined through the $\mathbb{P}$-compensated Poisson random measure $\widetilde{N}^{\mathbb{P}}(dt,dz)=N(dt,dz)-\ell^{\mathbb{P}}(dz)dt$ with Lévy measure $\ell^{\mathbb{P}}(dz) = \lambda^{\mathbb{P}}G(dz)$ that is independent of delivery time. Note that $\lambda^{\mathbb{P}}>0$ indicates the jump intensity under the physical measure and $G(dz)$ is the jump size distribution.\\

In order to characterize the futures logarithmic rate  in more detail, we introduce the following lemma.
\begin{Lemma}
	...
	\begin{align}
	d\ln f(t,\tau) = dY(t,\tau)\;. \label{eq:AdjOU2}
	\end{align}
	%	\begin{align}
...
Equation \eqref{eq:LogFuturesP} is the unique strong solution to the dynamics
\begin{align}
\frac{df(t,\tau)}{f(t-,\tau)}=  \sigma(t,\tau)dW^{\mathbb{P}}_t + \int_{\mathbb{R}}\left(e^{\eta(t,\tau)z}-1\right)\widetilde{N}^{\mathbb{P}}(dt,dz)+c^{\mathbb{P}}(t,\tau,Y(t,\tau))dt\;, \label{eq:dynFuturesP}
\end{align}
where the drift-term is characterized by
\begin{align}
c^{\mathbb{P}}(t,\tau,Y) =\mu(t,\tau)  - \kappa(t)Y+ \frac12 s^2(t,\tau)\sigma^2(t,\tau) + \psi^{\mathbb{P}}(\eta(t,\tau))\;.
\end{align}
\end{Lemma}
\begin{proof}
Applying Ito's formula  to $e^{Y(t,\tau)}$ leads to the desired dynamics (see \cite{OksendalSulem}, cf.\ Theorem 1.16).\qed\\
\end{proof}

Note that the assumptions behind the model induce a finite second moment as well as a finite moment generating function of the jump size distribution. We will consider later some suitable distributions for these jump sizes.\\

As in the previous section, we now derive the swap prices  resulting from geometric and approximated averaging.

\begin{Lemma} \label{lem:SwapPGeom}
%Assume  \discuss{$\widetilde{\kappa}(t,\tau):=\widetilde{\kappa}(t)\independent \tau$} and 
Let \Cref{ass:TechnicalRequirementsP,ass:TechnicalRequirementsP2,ass:StochasticFubini} in  \Cref{app:tech_requ3} be satisfied.
Then, the swap price resulting from \textit{geometric averaging} is defined by
\begin{align}
...
\begin{equation}
\begin{split}
d\bar{Y}(t,\tau_1,\tau_2) =&~ \left(\mathbb{E}[\mu(t,U)]-\kappa(t)\bar{Y}(t,\tau_1,\tau_2)\right)dt  + \mathbb{E}[\sigma(t,U)]dW^{\mathbb{P}}_t+\mathbb{E}[\eta(t,U)]d\widetilde{J}^{\mathbb{P}}_t\;.\label{eq:Y2tilde}
\end{split}
\end{equation}
Moreover, the swap price based on geometric averaging evolves as
\begin{equation}
\begin{split}
\frac{dF(t,\tau_1,\tau_2)}{F(t-,\tau_1,\tau_2)} =&~
\mathbb{E}[\sigma(t,U)]dW^{\mathbb{P}}_t + \int_{\mathbb{R}}\left(e^{\mathbb{E}[\eta(t,U)]z}-1\right)  \widetilde{N}^{\mathbb{P}}(dt,dz) + \widetilde{c}^{\mathbb{P}}(t,\tau_1,\tau_2,\bar{Y}(t,\tau_1,\tau_2))dt\;, \label{eq:SwapEvolP}
\end{split}
\end{equation}
...
\end{Lemma}
\begin{proof}
Following the considerations in the previous section, the swap price is defined by the geometric average in \Eqref{eq:GeomAverage}. Using the notation from \Cref{lemma:dyn_Swap_Q}, \Eqref{eq:GeomSwapP} follows. Using the stochastic Fubini theorem (see \cite{Protter2005}, cf.\ Theorem 65), we can introduce the dynamics of $\bar{Y}$. An application of Ito's formula (see \cite{OksendalSulem}, cf.\ Theorem 1.16) yields the desired swap dynamics. \qed\\
\end{proof}
...
\mathbb{E}[\sigma(t,U)]dW^{\mathbb{P}}_t + \int_{\mathbb{R}}\left(\mathbb{E}[e^{\eta(t,U)z}]-1\right)  \widetilde{N}^{\mathbb{P}}(dt,dz) +\mathbb{E}_U[c^{\mathbb{P}}(t,U,Y(t,U))]dt\;, \label{eq:DynArithmSwapP}
\end{split}
\end{equation}
where
$
...
The dynamics are solved by
\begin{equation}
\begin{split}
F^a(t,\tau_1,\tau_2)= F^a(0,\tau_1,\tau_2) e^{ \bar{Y}(t,\tau_1,\tau_2) + \frac12\int_{0}^{t}\mathbb{V}[\sigma(t,U)]dv + \int_{0}^{t}\int_{\mathbb{R}}  \left(\ln \mathbb{E}[e^{\eta(v,U)z}]-\mathbb{E}[\eta(v,U)]z\right)N(dv,dz)}
\;, \label{eq:ArithmSwapP}
\end{split}
\end{equation}
%\begin{equation}
%\begin{split}
%\ln F^a(t,\tau_1,\tau_2)=& \ln F^a(0,\tau_1,\tau_2) + \bar{Y}(t,\tau_1,\tau_2) + \frac12\int_{0}^{t}\mathbb{V}[\sigma(t,U)]dv\\& + \int_{0}^{t}\int_{\mathbb{R}}  \left(\ln \mathbb{E}[e^{\eta(v,U)z}]-\mathbb{E}[\eta(v,U)]z\right)N(dv,dz)
%\;, \label{eq:ArithmSwapP}
%\end{split}
%\end{equation}
...
\end{Lemma}
\begin{proof}
We use the approximated averaging methodology (see \Eqref{eq:arithAverage_drift}) in order to derive the swap price evolution and apply the stochastic Fubini theorem (see \cite{Protter2005}, cf.\ Theorem 65) leading to \Eqref{eq:DynArithmSwapP}. By Ito's formula (see \cite{OksendalSulem}, cf.\ Theorem 1.16), we find that \Eqref{eq:ArithmSwapP} solves the dynamics. \qed\\
\end{proof}

Note that the speed of mean-reversion $\kappa(t)$ has to be independent of the delivery time. This assumption ensures that $\bar{Y}$, in \Eqref{eq:Y2tilde}, is again an Ornstein-Uhlenbeck process and that the swaps' logarithmic rates in Equations \eqref{eq:GeomSwapP} and \eqref{eq:ArithmSwapP} stay tractable. This is also in line with the findings in \cite{Benth2019} (cf.~Proposition 2.2) and \cite{Latini2019}.\\

Similar to \Cref{lem:differenceAveraging}, we now evaluate the error between the exact geometric averaged swap and the approximated version under the physical measure in the next corollary.
\begin{Corollary}
The spread between the swap prices $F$ and $F^a$ under $\mathbb{P}$ resulting from \Cref{lem:SwapPGeom} and \Cref{lem:SwapPArithm} coincides with the pricing spread from \Cref{lem:differenceAveraging}.
\end{Corollary}
As the numéraire in \Eqref{eq:Numeraire} is not affected by a change of measure, the error between the exact and approximated swap price stays the same independent of the measure.
...
%		\item[$(i)$] 
		...
		\frac{\mathbb{E}[\mu(t,U)]-\kappa(t)\bar{Y}(t,\tau_1,\tau_2)+\frac12\mathbb{E}[\sigma(t,U)]^2 }{\mathbb{E}[\sigma(t,U)]} \;, \label{eq:MPR_F_1}\\
...
We follow the methodology of \cite{Benth2019} to change the measure from $\mathbb{P}$ to $\widetilde{\mathbb{Q}}$. Therefore, let $\pi=(\pi_1,\pi_2)$ be a predictable process satisfying 
\begin{align}
\mathbb{E}\left[\int_{0}^{\tau_1}\norm{\pi(s)}^2ds\right]<\infty\;. \label{eq:piSquareIntegrable}
\end{align}
...
\end{align}
such that $Z^{\mathbb{P}\widetilde{\mathbb{Q}}}(0)=1$, where 
\begin{align}
...
\begin{align}
Z^{\mathbb{P}\widetilde{\mathbb{Q}}}(t) = e^{H(t)-\frac12\int_{0}^{t}\pi_1(s)^2 ds} \prod_{0<s\leq t}\left(1+\Delta H(s)\right)e^{-\Delta H(s)}\;. \label{eq:DoleanDadeExponential}
\end{align}
...
\begin{align}
\frac{d\widetilde{\mathbb{Q}}}{d \mathbb{P}} = Z^{\mathbb{P}\widetilde{\mathbb{Q}}}(\tau_1)\;,
\end{align} 
where $Z^{\mathbb{P}\widetilde{\mathbb{Q}}}$ functions as the Radon-Nikodym derivative. If we further assume that $\mathbb{E}_{\mathbb{P}}[Z^{\mathbb{P}\widetilde{\mathbb{Q}}}(\tau_1)]=1$, then Girsanov's theorem (see \cite{OksendalSulem}, cf.\ Theorem 1.35) states for $\pi:=-\Pi^{\mathbb{P}\widetilde{\mathbb{Q}}}$ that 
\begin{align}
...
\item[$(ii)$]
If $Y$ is driven by a compensated Poisson process only,
...
\Pi_2^{\mathbb{P}\widetilde{\mathbb{Q}}}(t,\tau_1,\tau_2):= 
1-\frac{\mathbb{E}[\eta(t,U)]\int_{\mathbb{R}}z\ell^{\mathbb{P}}(dz)}{\int_{\mathbb{R}}\left(e^{\mathbb{E}[\eta(t,U)]z}-1\right)\ell^{\mathbb{P}}(dz)} + 
\frac{\mathbb{E}[\mu(t,U)]-\kappa(t)\bar{Y}(t,\tau_1,\tau_2)}{\int_{\mathbb{R}}\left(e^{\mathbb{E}[\eta(t,U)]z}-1\right)\ell^{\mathbb{P}}(dz)}\;.
\end{align}
...
Hence, in order to prove that the Radon-Nikodym density $Z^{\mathbb{P}\widetilde{\mathbb{Q}}}$ is a true martingale, it is sufficient to verify that $\mathbb{E}_{\mathbb{P}}[Z^{\mathbb{P}\widetilde{\mathbb{Q}}}(\tau_1)]=1$ is satisfied, which is proven in the next proposition. 
...
\frac{\mathbb{E}[\mu(t,U)]-\kappa(t)\bar{Y}(t,\tau_1,\tau_2)+\frac12\mathbb{E}[\sigma^2(t,U)] }{\mathbb{E}[\sigma(t,U)]} \;,\\
...
\begin{align}
\frac{d\mathbb{Q}}{d \mathbb{P}} = Z^{\mathbb{P}\mathbb{Q}}(\tau_1)\;,
\end{align} 
where $Z^{\mathbb{P}\mathbb{Q}}$ functions as the Radon-Nikodym derivative characterized by $\pi_j:=-\Pi_j^{\mathbb{P}\mathbb{Q}}$. If we further assume that $\mathbb{E}_{\mathbb{P}}[Z^{\mathbb{P}\mathbb{Q}}(\tau_1)]=1$, then Girsanov's theorem (see \cite{OksendalSulem}, cf.\ Theorem 1.35) states that
\begin{align}
...
%	\item[$(i)$] 
	...
	\begin{align}
	\bar{\Pi}_1^{\mathbb{Q}\widetilde{\mathbb{Q}}} &= -\frac12 \frac{\mathbb{V}[\sigma(t,U)]}{\mathbb{E}[\sigma(t,U)]}\;, \label{eq:Spread1} \\
	\bar{\Pi}_2^{\mathbb{Q}\widetilde{\mathbb{Q}}} &= 
...
\end{Proposition}
\begin{proof}
The result is attained by subtracting the true market price of risk defined in \Cref{def:MarketPricesGeom} from the classical market price of risk defined in \Cref{def:MarketPricesAppr}. \qed \\
\end{proof}

Hence, we found a representation of the true market price of risk of the swap price $F$, characterized by the classical market price of risk of the approximated swap $F^a$ and the spread $\bar{\Pi}_j^{\mathbb{Q}\widetilde{\mathbb{Q}}}$.
...
\begin{Lemma}
	\begin{itemize}
		...
		\item[$(ii)$] If the average jump size is positive, i.e., if $\mathbb{E}[Z]>0$, then  the spread of jump risk, $\bar{\Pi}_2^{\mathbb{Q}\widetilde{\mathbb{Q}}}$, is always negative.
...
In the following examples, we characterize the MPDP of jump risk and the spread based on three suitable jump size distributions: Normal and exponential.
...
\begin{Example}
 ...
 such that the MPDP and the spread in Equations~\eqref{eq:AddMP2} and \eqref{eq:Spread2} are given by
	\begin{align}
	{\Pi}^{\mathbb{Q}\widetilde{\mathbb{Q}}}_2  &= - \frac{\mathbb{E}[e^{\frac12 \eta^2(U)\sigma_J^2+\eta(U)\mu_J}]-e^{\frac12 \mathbb{E}[\eta(U)]^2\sigma_J^2+\mathbb{E}[\eta(U)]\mu_J}}{e^{\frac12 \mathbb{E}[\eta(U)]^2\sigma_J^2+\mathbb{E}[\eta(U)]\mu_J}-1}\;,\\
	\bar{\Pi}^{\mathbb{Q}\widetilde{\mathbb{Q}}}_2  &= - \mu_J\mathbb{E}[\eta(U)]  \frac{\mathbb{E}[e^{\frac12 \eta^2(U)\sigma_J^2+\eta(U)\mu_J}]-e^{\frac12 \mathbb{E}[\eta(U)]^2\sigma_J^2+\mathbb{E}[\eta(U)]\mu_J}}{\left(\mathbb{E}[e^{\frac12 \eta^2(U)\sigma_J^2+\eta(U)\mu_J}]-1\right)\left(e^{\frac12 \mathbb{E}[\eta(U)]^2\sigma_J^2+\mathbb{E}[\eta(U)]\mu_J}-1\right)}\;.
	\end{align}
...
\begin{Example}
	...
	\begin{align}
	{\Pi}^{\mathbb{Q}\widetilde{\mathbb{Q}}}_2  &=- \frac{\lambda_J}{\mathbb{E}[\eta(U)]}\left(1-\mathbb{E}\left[\frac{\lambda_J-\mathbb{E}[\eta(U)]}{\lambda_J-\eta(U)}\right]\right) \;,\\
	\bar{\Pi}^{\mathbb{Q}\widetilde{\mathbb{Q}}}_2  &=
...
$\bar{\mu}_{t,m}=\mathbb{E}[\mu(t,U_m)]-\mathbb{E}[\eta(t,U_m)]\lambda^{\mathbb{P}}\mathbb{E}[Z]$, $\bar{\phi}=\bm{1-\kappa}\in\mathbb{R}^M$, and the volatility matrix
$\bar{\sigma}_t\in \mathcal{S}^M$, which is a diagonal matrix with diagonal entries
...
\begin{Remark}
	By construction, the contracts are independent of each other since the volatility and jump coefficient matrices are diagonal.
...
\begin{Type} \label{Type2}
	...
	\begin{align*}
	\bar{\phi}&=\bm{1-(\kappa-\Lambda)\Delta t}\;,\\ \bar{\mu}_{t}&=\mu\bar{\Lambda}\left(e^{-\Lambda(\tau_1-t)\Delta t},\ldots,e^{-\Lambda(\tau_M-t)\Delta t}\right)^\top-\lambda^{\mathbb{P}}\mathbb{E}[Z]\bm{1}\;,\\
	\bar{\sigma}_{t}&=\bar{\Lambda} ~diag \left(e^{-\Lambda(\tau_1-t)\Delta t},\ldots,e^{-\Lambda(\tau_M-t)\Delta t} \right)^\top
	\;.
	\end{align*}
	%where
	%\begin{align}
...
We employ the first window of the thresholding procedure as a training period, which should be excluded from the data used to estimate the parameters in the second step. Otherwise, the thresholding procedure might not exclude all jumps, especially within the first half of the considered time window.\\ 
...
The standard deviations computed over all monthly contracts range from 2.863 to 6.396, appearing highest for winter and summer month like January, February, and June. \\
...
\begin{table} [H]
	...
	\begin{tabular}{|l|ccccccc|}
		\hline
		\rule{0pt}{15pt}
		Contracts &   Starting Date    &  Maturity &    Data Points  &    Min   &    Max  &    Average Price  &  Standard Deviation \\
		\hline
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Identified jumps by thresholding using a training period of 10 days (exemplified for January and July).}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Histogram and QQ-Plot of all identified jumps for all contracts fitted with a normal distribution. (Normal -- p-value of $\mu$: 0.8965 and p-value of $\sigma$: 8.4341e-66.)}
...
%	\end{subfigure}
	\caption{QQ-Plots of positive and negative jumps over all contracts. (Positive jumps -- p-value of $\mu$: 3.11536e-18.
		Negative jumps -- p-value of $\mu$: 4.91286e-18.)
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Volatility function for seasonality type (Type 1)  and Samuelson type (Type 2).}
...
The simulations in \Cref{app:SimulationLogReturns_SepFit_completeTH} confirm that even the model with the lower likelihood fit the data comparably well.
...
\begin{figure}[t]
	...
	\end{subfigure}
	\caption{MPDP of diffusion risk $\Pi_1$ for all contracts for seasonality (Type 1) and Samuelson type (Type 2).}
...
In this paper, we provide empirical evidence as well as analytical characteristics of the MPDP for electricity swaps embedded in a Merton type model.\\
...
In addition, we transfer the model to the physical measure under which we allow for mean-reversion, seasonalities, and term-structure effects.
...
We finally investigate the model under the physical measure as well as the resulting MPDP empirically.
...
In contrast, term-structure dependence induces a decreasing behavior of the MPDP over trading time.
...
To conclude, we expand the MPDP to the jump setting and provide empirical evidence for the MPDP influenced by typical characteristics of the electricity market.
We even expect a growing MPDP caused by a rising market share of renewable energy:
Varying weather conditions over the seasons of a year will cause stronger seasonal behavior when the share of renewables is growing.  Consequently, this increases the importance of the MPDP of diffusion risk, which has to be taken into account to ensure an accurate pricing procedure.
