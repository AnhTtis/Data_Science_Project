...
For the scope of the proof, we consider the swap price $F$ in  \Eqref{eq:GeomSwapP} characterized by  stochastic volatility of the form
\begin{align}
\sigma (t,\tau) \sqrt{\nu (t)}\;,
\end{align} 
...
\end{align}
for $\nu(0)=\nu_0>0$, where $B^\mathbb{P}$ and $\widetilde{N}^{\mathbb{P}}$ are independent of each other and $B^\mathbb{P}$ and $W^{\mathbb{P}}$ are correlated.
...
\begin{enumerate}
\item Derivation of a new risk-neutral measure $\widetilde{\mathbb{Q}}^n$ through the stopping time $\hat{\tau}_n$.
...
1-\frac1n \mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_s\widetilde{Y} (s,\tau_1,\tau_2)\right]-\frac1n \mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_s\nu ^{-1}(s)\right]-\frac1n \mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_s\nu (s)\right]\;.$$
\item Proof that there exist upper boundaries for $\mathbb{E}_{\widetilde{\mathbb{Q}}^n}[\sup_s\widetilde{Y} (s,\tau_1,\tau_2)]$, $\mathbb{E}_{\widetilde{\mathbb{Q}}^n}[\sup_s\nu ^{-1}(s)]$, and $\mathbb{E}_{\widetilde{\mathbb{Q}}^n}[\sup_s\nu (s)]$,  that are independent of $n$.\\
\end{enumerate}
...
\begin{align}
\hat{\tau}_n:=\inf \big\{ t\in[0,\tau_1]: \vert \widetilde{Y}(t,\tau_1,\tau_2)\vert\geq n, \text{ or } \vert \nu^{-1}(t)\vert \geq n,  \text{ or } \vert \nu(t)\vert \geq n \big\}\;, \label{eq:StoppingTime}
\end{align}
...
\begin{align*}
\mathbb{E}_{\mathbb{P}}[Z^{\mathbb{P}\widetilde{\mathbb{Q}}}(\tau_1,\tau_1,\tau_2)] \geq & 1- \widetilde{\mathbb{Q}}^n[\hat{\tau}_n \leq \tau_1]\\ \geq & 1- \left(\widetilde{\mathbb{Q}}^n\left[\sup_{s\in[0,\tau_1]}\widetilde{Y} (s,\tau_1,\tau_2)\geq n\right]+\widetilde{\mathbb{Q}}^n\left[\sup_{s\in[0,\tau_1]}\nu ^{-1}(s)\geq n\right] +\widetilde{\mathbb{Q}}^n\left[\sup_{s\in[0,\tau_1]}\nu (s)\geq n\right] \right)\\
\geq & 1- \frac1n  \left(\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]}\widetilde{Y} (s,\tau_1,\tau_2)\right]+\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]}\nu ^{-1}(s)\right]+\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]}\nu (s)\right] \right)\;,
\end{align*}
...
In order to identify upper boundaries under the measure $\widetilde{\mathbb{Q}}^n$ defined in \Eqref{eq:MeasureChangeQn}, we need to derive the dynamics of $\widetilde{Y}$, $\nu ^{-1}$, and $\nu$  under $\widetilde{\mathbb{Q}}^n$. 
...
Next, we show that $\vert \widetilde{Y} \vert^2$ is uniformly integrable:
%\begingroup
%\allowdisplaybreaks
%\begin{align*}
%&\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]} \vert \widetilde{Y}_1(s,\tau_1,\tau_2) \vert^2\right]\\
%=& \mathbb{E}_{\widetilde{\mathbb{Q}}^n}\Bigg[\sup_{s\in[0,\tau_1]} \Big(\widetilde{Y}_1(0,\tau_1,\tau_2)+ \int_{0}^{s}  \left(1-\mathbbm{1}_{[0,\hat{\tau}_n]}(t)\right)\left(\mathbb{E} [s_1(t,U)\mu_1(t,U)]- \kappa_1(t)\widetilde{Y}_1(t,\tau_1,\tau_2)\right)dt\\&
...
%%
%\overset{(\star)}{\leq}&
%5\Big(\widetilde{Y}_1(0,\tau_1,\tau_2)^2 + \mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[
...
%\\&+
%\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]}\left(\int_{0}^{s}\left(1-\mathbbm{1}_{[0,\hat{\tau}_n]}(t)\right) \kappa_1(t)\widetilde{Y}_1(t,\tau_1,\tau_2)dt\right)^2\right]+
...
%\\
%\overset{(\star\star)}{\leq}&
%5\Big(\widetilde{Y}_1(0,\tau_1,\tau_2)^2 + 4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[
...
% \frac12\mathbb{E} [\sigma_1(t,U)s_1(t,U)]^2\mathbbm{1}_{[0,\hat{\tau}_n]}(t)dt\right)^2\right]
%\\&+
%4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\left(\int_{0}^{\tau_1}\left(1-\mathbbm{1}_{[0,\hat{\tau}_n]}(t)\right) \kappa_1(t)\widetilde{Y}_1(t,\tau_1,\tau_2)dt\right)^2\right]+
...
%\Big)\\
%\overset{(\star\star\star)}{\leq}&
%5\Big(\widetilde{Y}_1(0,\tau_1,\tau_2)^2 + 
...
%\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\int_{0}^{\tau_1}\widetilde{Y}_1(t,\tau_1,\tau_2)^2dt\right]+
%4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\left(\int_{0}^{\tau_1}\mathbb{E} [\sigma_1(t,U)s_1(t,U)]dW_1^{\widetilde{\mathbb{Q}}^n}(t) \right)^2\right]
%\Big)\\
%\overset{}{\leq}&
%5\Big(\widetilde{Y}_1(0,\tau_1,\tau_2)^2 + 
...
%+\tau_1\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\int_{0}^{\tau_1}\mathbb{E} [\sigma_1(t,U)s_1(t,U)]^4dt\right]\\&
%+4\int_{0}^{\tau_1} \kappa_1(t)^2dt
%\int_{0}^{\tau_1}\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,t]}\widetilde{Y}_1(s,\tau_1,\tau_2)^2\right]dt+
...
%\end{align*}
%\endgroup
%The first equality represents the integral version of $\widetilde{Y}_1$. Inequality $(\star)$ results from the Cauchy-Schwartz inequality to the sum
%and an application of the triangle inequality. 
%We apply Doob's inequality to all expectations in Inequality $(\star\star)$. 
%In Inequality $(\star\star\star)$, we apply the Cauchy-Schwartz inequality to the first three integrals.  
%We finish with Ito's isometry to the last summand and applying stochastic Fubini to the $\widetilde{Y}_1$-term while making the integrand even bigger. 
...
%\end{align*}
%
%Setting $c_{Y_1}:=5\widetilde{Y}_1(0,\tau_1,\tau_2)^2 + 
%20\tau_1\int_{0}^{\tau_1}\mathbb{E} [s_1(t,U)\mu_1(t,U)]^2dt
%+\sigma \bar{s}_1 c_{\nu_1} \tau_1^2
%+   20 \sigma_1^2\bar{s}_1^2\tilde{c}_{\nu}$, then, by Gronwall
%\begin{align*}
%\mathbb{E}_{\mathbb{P}^n}\left[\sup_{s\in[0,\tau_1]} \vert \widetilde{Y}_1(s,\tau_1,\tau_2) \vert^2\right] \leq c_{Y_1} e^{16\int_{0}^{\tau_1} \kappa_1(t)^2dt } =:c_3\independent n\;.
%\end{align*}
%
%For $\vert \widetilde{Y}_2 \vert^2$ we proceed analogously:
{\small
\begingroup
\allowdisplaybreaks
\begin{align*}
&~\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]} \vert \widetilde{Y}(s,\tau_1,\tau_2) \vert^2\right]\\
=&~ 
\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\Bigg[\sup_{s\in[0,\tau_1]} \Big(\widetilde{Y}(0,\tau_1,\tau_2)+ \int_{0}^{s}  \left(1-\mathbbm{1}_{[0,\hat{\tau}_n]}(t)\right)\left(\mathbb{E} [ \mu(t,U)]- \kappa(t)\widetilde{Y}(t,\tau_1,\tau_2)\right)dt
...
%
\overset{(\star)}{\leq}&~
7\Bigg(\widetilde{Y}^2(0,\tau_1,\tau_2) + \mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[
...
+
\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sup_{s\in[0,\tau_1]}\left(\int_{0}^{s}\left(1-\mathbbm{1}_{[0,\hat{\tau}_n]}(t)\right) \kappa(t)\widetilde{Y}(t,\tau_1,\tau_2)dt\right)^2\right]\\&~
+
...
\\
\overset{(\star\star)}{\leq}&~
7\Bigg(\widetilde{Y}^2(0,\tau_1,\tau_2) + 4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[
...
\frac12\mathbb{E} [\sigma (t,U) ]^2\nu(t)\mathbbm{1}_{[0,\hat{\tau}_n]}(t)dt\right)^2\right]
+
4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\left(\int_{0}^{\tau_1}\left(1-\mathbbm{1}_{[0,\hat{\tau}_n]}(t)\right) \kappa(t)\widetilde{Y}(t,\tau_1,\tau_2)dt\right)^2\right]\\&~
+
...
\\
\overset{(\star\star\star)}{\leq}&~
7\Bigg(\widetilde{Y}^2(0,\tau_1,\tau_2) + 4\tau_1\int_{0}^{\tau_1}\mathbb{E} [ \mu(t,U)]^2dt
\\&~
+
4\int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]^4dt~\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[ \int_{0}^{\tau_1}\nu^2(t)dt\right]
+
4\int_{0}^{\tau_1} \kappa^2(t)dt~\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\int_{0}^{\tau_1}\widetilde{Y}^2(t,\tau_1,\tau_2)dt\right]\\&~
+
4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\left(\int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]\sqrt{\nu(t)}dW^{\widetilde{\mathbb{Q}}^n}_t \right)^2\right]
+4\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\left(\int_{0}^{\tau_1}\mathbb{E} [ \eta(t,U)]d\widetilde{J}^{\widetilde{\mathbb{Q}}^n}_t
\right)^2\right]\\&~
...
+
4\int_{0}^{\tau_1} \kappa^2(t)dt~\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\int_{0}^{\tau_1}\sup_{s\in[0,t]}\widetilde{Y}^2(s,\tau_1,\tau_2)dt\right]
+
...
\Bigg)\;,\\
=:&~c_Y +28\int_{0}^{\tau_1} \kappa^2(t)dt~\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\int_{0}^{\tau_1}\sup_{s\in[0,t]}\widetilde{Y}^2(s,\tau_1,\tau_2)dt\right]\;.
\end{align*}
\endgroup
}
The first equality represents the integral version of $\widetilde{Y}$. Inequality~$(\star)$ results from the Cauchy-Schwartz inequality to the sum
and an application of the triangle inequality. 
We apply Doob's inequality to all expectations in Inequality~$(\star\star)$. 
In Inequality~$(\star\star\star)$, we apply the Cauchy-Schwartz inequality to the first three integrals.  
We finish with Itô-Lévy Isometry (see \cite{OksendalSulem}, cf.\ Theorem 1.17) to the last summand and an application of the stochastic Fubini theorem  to the fourth summand (including $\widetilde{Y}$) while making the integrand even bigger. 
By the previous considerations, we know that  $\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]^2\nu(t)dt \right]\leq\sqrt{\int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]^4dt}\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[\sqrt{\int_{0}^{\tau_1}\nu^2(t)dt}\right]\leq \sqrt{\int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]^4dt} \sqrt{\tau_1 c_2}$ is bounded independently of  $n$ and that
$\int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]^4dt~\mathbb{E}_{\widetilde{\mathbb{Q}}^n}\left[ \int_{0}^{\tau_1}\nu^2(t)dt\right] \leq c_2 \tau_1 \int_{0}^{\tau_1}\mathbb{E} [\sigma (t,U) ]^4dt$ is independent of $n$.
By the choice of $c_Y$, an application of Gronwall's inequality yields
$
\mathbb{E}_{\mathbb{P}^n}\left[\sup_{s\in[0,\tau_1]} \vert \widetilde{Y}(s,\tau_1,\tau_2) \vert^2\right] \leq c_{Y} e^{28\int_{0}^{\tau_1} \kappa^2(t)dt } =:c_3\independent n\;,
$
