...
\title{MadMax electrodynamics: Solutions  and basic properties} 
...
\begin{abstract}
The MadMax theory is a recently discovered theory of non-linear electrodynamics that admits many important gravitating solutions within the Einstein theory. Namely, it 
...
In this paper, after discussing the   basic properties of the MadMax theory, 
...
Among other notable solutions, we construct  a novel C-metric describing accelerated AdS black holes in the MadMax theory. We also 
present a generalization of the MadMax Lagrangian applicable to magnetic solutions, and find the corresponding spherical, slowly rotating, and weakly NUT charged solutions. 
\end{abstract}
...
The NLE model studied in this paper, which we call the {\em MadMax theory},  
is exceptional regarding this latter criterion. 


The MadMax model was first derived in \cite{Tahamtan:2020lvq}. It admits the Maxwell limit and, as we shall see, provides in some sense the simplest regularization of point charge field and its self-energy. However, its main distinguishing feature centers around the fact that it provides important gravitating solutions. 
Namely, 
...
MadMax theory also provides slowly rotating black holes that can be found  in a form  naturally generalizing the corresponding Maxwell solution \cite{Kubiznak:2022vft}, and, as we shall see shorty, one can also find a very natural generalization of the C-metric -- describing charged accelerated black holes in the MadMax theory.




Our paper is organized as follows. In Sec.~\ref{sec2} we review the basic framework for theories of nonlinear electrodynamics. In Sec.~\ref{sec3} the MadMax theory is introduced and its basic properties are discussed. Sec.~\ref{sec4} is devoted to (electrically charged) spherical solutions and their thermodynamic properties in both canonical and grandcanonical ensembles.
It is shown that such thermodynamics is quite rich, and in particular leads to an  unprecedented grandcanonical phase diagram with multiple phase transitions of various kinds. Other notable electric solutions, including the novel AdS C-metric, are studied in Sec.~\ref{sec5}. Sec.~\ref{sec6} is devoted to a generalization of the MadMax theory that is also applicable to purely magnetic solutions, allowing us to construct magnetically charged black holes, and their slowly rotating and weakly NUT charged cousins.  We summarize our findings in Sec.~\ref{sec7}. App.~\ref{appA} contains formulae for the Maxwell charged and the MadMax charged C-metrics written in the ``standard" C-metric $x-y$ coordinates, drawing the parallel between the two cases.
...
a condition known as the {\em principle of correspondence}. This condition is satisfied by the MadMax theory studied in this paper, c.f. \eqref{POCMadMax} below. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{MadMax theory: basic properties}
\label{sec3}

In what follows, we focus on yet another type of NLE, defined by the following {\em `MadMax' Lagrangian}:
\ba
...
Since the MadMax theory contains a dimensionfull parameter, it cannot be, similar to the Born--Infeld theory \eqref{BI}, a {\em conformal} field theory. That this is indeed the case can easily explicitly be shown by calculating the trace of the energy momentum tensor \eqref{Tmunu}, employing that   
\be
...
%\begin{figure}[h]
	...
	\caption{\textbf{Electric field strength.} $E$ of a point charge is displayed for the MadMax theory with $
...
Let us first study the field of a test point-like charge in the MadMax theory \eqref{Tay}, neglecting the backreaction of the electromagnetic field on the geometry. Writing the flat space in spherical coordinates, 
\be
ds^2=-dt^2+{dr^2}+r^2d\Omega^2\,,
\ee
where $d\Omega^2=d\theta^2+\sin^2\!\theta d\varphi^2$, the corresponding solution of MadMax equations  reads
\be\label{phi}
...
Moreover, since $E_0$ is finite, the MadMax model leads to finite self-energy, similar to what happens in the Born--Infeld case.
...
\subsection{MadMax AdS black holes}
...
%\begin{figure}[h]
	...
	\caption{\textbf{Two types of MadMax AdS black holes.} Based on the bahavior of the metric function $f_0$ near the origin, we distinguish two types of MadMax black holes: the RN-type (blue) and the S-type (red). The marginal case $m=M_m$ is highlighted by a thick black line. In particular, for the RN-type ($m<M_m$) we have from top to bottom: naked singularity (top two lines with the top one corresponding to $m\to0$), extremal black hole with one horizon (solid blue), and the case with two horizons (lower thin blue). The figure is displayed for $Q=1$, $\ell=1$, and $\alpha=1$.   
}\label{fig2}
\end{center}
\end{figure}
...
The thermodynamic behavior of MadMax black holes  is also interesting in the grandcanonical (fixed $\phi$) ensemble. In this case, we are interested in 
...
thermodynamics of MadMax black holes.  
...
in MadMax theory. We also review slowly rotating black holes, following \cite{Kubiznak:2022vft}, and 
construct  weakly NUT charged Taub-NUT solutions.
It is a remarkable property of the MadMax theory, that all these solutions can analytically be found and take a simple form that is in many ways very similar to what happens in the linear Maxwell case.   
...
However, a generalization to theories of NLE seems quite challenging, e.g., \cite{Breton:2023bwf}. In fact, the only known C-metric solution in NLE is for the ModMax theory \cite{Barrientos:2022bzm}, which, however, is in many aspects very similar to what happens in the Maxwell case. Here, we present highly non-trivial generalization of the standard charged C-metric in MadMax theory, see also App.~\ref{appA} for the presentation of this solution in a different coordinate system.  


The charged AdS C-metric in the MadMax theory takes the form \eqref{Cansatz}, \eqref{OmegaHl},   where the metric functions now read
\ba
...
\ea
Here, $f_0$ is the static MadMax function \eqref{f-SSS}, $f_0^M$ is given by \eqref{f0M}, and the metric is accompanied by the following vector potential:
\be
...
\includegraphics[scale=0.67]
{Fig18}
	\caption{{\bf Horizons of MadMax C-metric.}
...
both (slowly accelerating and fast accelerating) regimes remain present even in the deep MadMax regime. We plan to return to the discussion of the admissible parameter space of this newly found solution and the discussion of its thermodynamics in future work \cite{TDsprep}.  


To conclude this section, let us stress that 
it is a remarkable feature of the MadMax theory, that the charged AdS C-metric can be written in the standard form \eqref{Cansatz}, with the conformal pre-factor $\Omega$ given by \eqref{OmegaHl}. While we were not able to check this explicitly, it is reasonable to conjecture that apart from the ModMax and Maxwell theories, MadMax is  
...
In this respect, MadMax has a privileged position. Namely, as shown in \cite{Kubiznak:2022vft}, MadMax is the only NLE apart from Maxwell, that is fully characterized by invariant ${\cal S}$ and whose slowly rotating solutions can be written in the above form with 
\be
\omega=1\,. 
\ee  
In other words, MadMax and Maxwell are the only restricted NLE's whose slowly rotating vector potential is fully characterized by the electrostatic potential $\psi_0$. Interestingly, the proof of this statement 
...
It would be very interesting to see, whether this approximate solution can be extended to a fully rotating black hole in the MadMax theory -- giving thus a first example of a rotating solution in theories of NLE.  
...
The MadMax equations together with the Einstein equations then yield ordinary differential equations for $f(r)$ and $\psi(r)$. Unfortunately, it turns out that for the MadMax theory these are difficult to solve and we were unable to find the full analytic solution.
...
\label{sec6}
In order to construct magnetically charged solutions in the MadMax theory we need to generalize the Lagrangian \eqref{Tay}, to accommodate for the possibility of ${\cal S}>0$ while keeping $\alpha$ positive. To this purpose, we start with the 
Lagrangian ${\cal L(S)}$, \eqref{Tay}, which is valid only for ${\cal S}<0$, and define new  Lagrangian by:
\be\label{LMag}
\tilde {\cal L}({\cal S})=-\sgn({\cal S})\, {\cal L}\big(-\sgn({\cal S})\,{\cal S}\big)\,.
\ee
This has the following derivatives:
\ba
\tilde {\cal L}_{{\cal S}}&=&-2\delta({\cal S})\big({\cal L}-\sgn({\cal S}){\cal S}{\cal L}_{{\cal S}}\big)+{\cal L}_{{\cal S}}={\cal L}_{{\cal S}}\,,\nonumber\\
\tilde {\cal L}_{{\cal SS}}&=&-{\cal L}_{{\cal SS}}\big(2{\cal S}\delta({\cal S})+\sgn({\cal S})\big)=-\sgn({\cal S}) {\cal L}_{{\cal SS}}\,,\quad
\ea
which are well-defined when we assume ${\cal L}|_{{\cal S}=0}=0$ and the regularity of derivatives of ${\cal L}$. Furthermore, we see that the sign of the first derivative does not flip when ${\cal S}$ changes sign but for the second derivative it seemingly does. However, considering the dependence on invariant ${\cal S}$ we observe that $\tilde{\cal L}_{{\cal S}}<0$ and  $\tilde{\cal L}_{{\cal SS}}>0$ irrespective of the sign of ${\cal S}$. This means that the discussion of birefringence in section \ref{sec3} (see mainly Eq.~\eqref{null-norm} and the text thereafter) is valid for $\tilde{\cal L}$ and the propagation of modes is causal as desired. 
...
\ea 
This, as we shall see, leads to a well-defined solution with correct Maxwell limit. Namely, we assume the following 
local potential 
\be
...
The MadMax equations, \eqref{FE}, are then automatically satisfied, and from the Einstein equations \eqref{Hmunu}, the metric ansatz \eqref{SSS} yields 
\ba\label{Magnetic-NEW-SSS}
f_0&=&1-2|Q_m|\alpha^{2}-\frac {4\alpha|Q_m|^{3/2}+6M}{3r}+\frac{r^2}{\ell^2}\nonumber\\
...
\phi_m&=&\frac{\alpha Q_m}{r_+\alpha-\sqrt{|Q_m|}}\,, \quad 
...
\ee
The phase structure of magnetic solutions is quite interesting and will be discussed elsewhere \cite{magPrep}.  
...
\ea
where $\omega(r)$ and $h(r)$ are new vector potential and metric functions, respectively. Here we construct the slowly rotating solution in the MadMax theory explicitly. 
...
Following the above discussion, we seek the magnetized weakly NUT charged solution in the form \eqref{TNvector} for the metric, together with the vector potential ansatz \eqref{vectorTNsmall}. For the MadMax theory this yields $f=f_0$, \eqref{Magnetic-NEW-SSS}, together with:
\be 
\nu=\frac{3r\alpha-2\sqrt{|Q_m|}}{3\alpha r^3}\,,
\ee
...
the basic properties and solutions of a particular model of non-linear electrodynamics, which we named the MadMax theory. Such a theory is characterized by a dimensionfull parameter $\alpha$ and is neither conformal nor it possesses an electromagnetic duality. However, while the 
MadMax Lagrangian \eqref{Tay} seems rather complicated at first sight, it
leads to arguably the most straightforward regularization of the electric field of a point charge \eqref{E}. Even more importantly, the MadMax theory becomes truly 
...
\cite{Kubiznak:2022vft}, the MadMax model provides further important gravitating solutions. 
...
It is tempting to conjecture that the MadMax theory is the only restricted NLE for which the solution for accelerated AdS black holes
...
we have `naturally' extended the MadMax theory in 
...
Or perhaps even more interestingly, can the (possibly generalized) MadMax theory, similar to the Born--Infeld case, be derived from some more fundamental (possibly higher-dimensional) theory? 
...
\label{appA}
The C-metric is most easily written in the standard ``$x-y$ coordinate system". In this appendix, we present the corresponding ansatz for the Maxwell theory and show that it can also be applied in the MadMax case. 
...
\subsection{MadMax theory}
The calculations in the MadMax electrodynamics proceed in the same way. In particular, we also find three interesting solutions upon choosing $H\in\{1,y, x+y\}$. 
...
\ea
together with \eqref{Aapp}, we recover the MadMax AdS C-metric. Note that when the cosmological constant vanishes, the functions $F(y)$ and $G(x)$ have the following property: $F(w)=-G(-w)$. As discussed in the main text,  to maintain a Lorentzian signature of the metric \eqref{XYansatz}, it is necessary
...
Finally, as we mentioned above, Eq.~\ref{psiprime} imposes a very strong restriction on NLE Lagragians, which seems almost impossible to satisfy. Surprisingly, for the MadMax theory and its vector potential \eqref{Aapp}, we find 
\begin{equation}
	{\cal L}_S=-\frac{(\alpha+y\sqrt{|Q|})^{2}}{(\alpha-x\sqrt{|Q|})^{2}}\,,
\end{equation}
which indeed is in the multiplicative separated form. Although we were not able to prove this rigorously,  we conjecture that MadMax is the only special NLE for which this happens.    
...
T.~Tahamtan, O.~Sv{\'i}tek and D.~Kubiznak, \emph{{Well–posed electromagnetic
...
\bibitem{TDsprep}
D.~Kubiznak, T.~Tahamtan and O.~Svitek, \emph{{C-metric in MadMax
...
T.~Hale and J.~Men{\v s}{\'i}kov{\'a}, \emph{{On thermodynamics of magnetically
  chared MadMax black holes}}, {\emph{in preparation} (2023) }.
