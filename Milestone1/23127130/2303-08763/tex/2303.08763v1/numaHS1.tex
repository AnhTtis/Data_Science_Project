...
\title[Numerical algorithm for the HS equation]{ A numerical algorithm for \fontsize{15}{17}{$\alpha$}-dissipative solutions of the Hunter--Saxton equation}
...
Weak solutions can be extended past wave breaking in various ways, see for instance \cite{BressanDissipative, UniquenessConservative, alphaHS, HunterHyperbolicEquation}. The two most prominent notions in the literature are that of a dissipative solution, where one removes the concentrated energy from the system, and that of a conservative solution, where one reinserts the concentrated energy. In this work, we consider the concept of $\alpha$-dissipative solutions, first introduced in \cite{alphaCH} for the related Camassa--Holm equation, and in \cite{alphaHS} for \eqref{eq:Hunter-Saxton}. Instead of removing all the concentrated energy or none of it, an $\alpha$-fraction, where $\alpha \in [0, 1]$, is removed. This way, the notion of $\alpha$-dissipative solutions acts as a continuous interpolant between the two extreme cases, $\alpha=0$ corresponding to conservative solutions, and $\alpha=1$ corresponding to dissipative solutions. Thus, the notion of $\alpha$-dissipative solutions allows for a uniform treatment of weak solutions with nonincreasing energy. The existence of $\alpha$-dissipative solutions was established in \cite{alphaHS} for the two-component Hunter--Saxton system which generalizes \eqref{eq:Hunter-Saxton}, and in the more general setting where $\alpha \in W^{1, \infty}(\R, [0, 1)) \cup \{1 \}$. 

One approach to solve the Cauchy problem \eqref{eq:Hunter-Saxton} is to use a generalized method of characteristics, see for instance \cite{UniquenessConservative, alphaHS}. We follow this approach here, and in a similar spirit to \cite{alphaCH, alphaHS}, we augment the solution of \eqref{eq:Hunter-Saxton} with a dummy measure $\nu$. This measure is added exclusively for technical reasons as the same pair $(u, \mu)$ is obtained for any choice of $\nu$, see \cite[Lem. 2.13]{NewstLipschitzMetrichAlpha}.  

 Given an $\alpha$-dissipative solution $(u,\mu,\nu)$, the corresponding Lagrangian coordinates are given by the tuple $(y,U,V,H)$, whose time evolution is governed by the following system of differential equations, cf. \cite{alphaHS},
\begin{align*}
    y_t(t, \xi) &= U(t, \xi),  \\
    U_t(t, \xi) &= \frac{1}{2}V(t, \xi) - \frac{1}{4}V_{\infty}(t),  \\
    V(t, \xi) &= \int_{-\infty}^{\xi}V_{0, \xi}(\eta) \big(1 - \alpha \chi_{\{t \geq \tau(\eta) > 0\}}(\eta) \big) d\eta, \\
    H_t(t, \xi) &= 0. 
\end{align*}
Here $V_{\infty}(t) = \lim_{\xi \rightarrow \infty}V(t, \xi)$ is the total current energy in the system at time $t$, and it is in general a time-dependent quantity, except in the case where  $\alpha=0$. Moreover, $\tau: \R \rightarrow [0, \infty]$ is the wave breaking function given by
\begin{align*}
    ...
    \end{cases}
\end{align*}
That means, identifying the variable $\xi$ with a particle, $y(t,\xi)$ represents the particle trajectory for a particle starting from $y(0,\xi)$ and $\tau(\xi)$ is the time at which the solution experiences wave breaking along this characteristic. In particular, $\tau(\xi) = \infty$ indicates that no wave breaking happens along this characteristic. 
Moreover, $U(t, \xi) = u(t, y(t, \xi))$ represents the velocity of the particle, while $V_{\xi}(t, \xi)$ is its energy. $H(t, \xi)$ is a conserved quantity stemming from the measure $\nu$ when transforming to Lagrangian coordinates.

Despite receiving a considerable amount of attention from a theoretical perspective, only a few numerical schemes have been proposed for the HS equation. In \cite{NumericalFDSchemes} several finite difference schemes for dissipative solutions were proved to converge. Also for dissipative solutions, \cite{NumericalDGDissipative} introduces a convergent, discontinuous Galerkin method. Furthermore, in \cite{SatoNumerical} a finite difference scheme on a periodic domain for a modified HS equation was derived and proven to converge towards conservative solutions. More recently, a Godunov-inspired scheme \cite[Chp. 12.1]{FiniteVolumeMethodsBook} for conservative solutions based on tracking the solution along characteristics, but set in Eulerian coordinates, was introduced in \cite{NumericalConservative}. The scheme was proved to converge and a convergence rate prior to wave breaking was derived. 
...
\begin{align*}
    ...
    \end{align*}
\end{subequations}
 Here $y_{\#}(V_{\xi}d\xi)$ is the pushforward of $V_{\xi}d\xi$ by $y$. 
\end{definition}
...
Based on \eqref{eq:integrated_ODE} we can now define the solution operator $S_t$ in Lagrangian coordinates.
...
For any $t\geq 0$ and $X_0\in \F$ define $S_t(X_0)=X(t)$, where $X(t)$ denotes the unique $\alpha$-dissipative solution to \eqref{eq:integrated_ODE} with initial data $X(0)=X_0$.
 ...
 \end{equation*}
 As consequence of this result, we will restrict ourselves from now on to consider only initial data $(u_0,\mu_0,\nu_0)\in \D_0$, which means in particular that $\mu_0=\nu_0$.
...
As the projection operator $P_{\Delta x}$ is the only part that actually introduces any error, its construction is the crucial step. In particular, $P_{\Delta x}$ must preserve key properties of $\alpha$-dissipative solutions such as the energy $\mu(\mathbb{R})$ and Definition \ref{def:EulerianSet}~\ref{def:EulerianSetlist:condition3}. Beside the construction of $P_{\Delta x}$, we will, at the end of this section, discuss the implementation of our algorithm.
...
To avoid any ambiguity between breakpoints of a function and points of wave breaking, we will denote the first as nodes in the following. 

By construction, $x+G_{\Delta x}(0, x)$ is an increasing, piecewise linear function with nodes situated at the points $\{x_j\}_{j\in \mathbb{Z}}$, and hence $y_{\Delta x}(0,\xi)$ is again an increasing and piecewise linear function due to Definition~\ref{def:MappingL}. Furthermore, $y_{\Delta x}(0,\xi)$ is continuous and its nodes can be identified by finding all $\xi$ which satisfy $y_{\Delta x}(0,\xi)=x_j$ for some $j\in \mathbb{Z}$ as follows. By definition  
\begin{align*}
...
\begin{figure}
    ...
    \caption{An illustration of how jumps in $G_{\Delta x}$ are mapped into constant intervals of $y_{\Delta x}$.}
...
The auxiliary function $f(Z)$ will be an essential part in the upcoming convergence analysis. 
It is defined as
\begin{align}
        f(Z)(x) &= \begin{cases}
...
\end{align*}
 and  
\begin{equation*}
        Z := (\id, u, 1, u_{x}, u_x^2, u_x^2),
\end{equation*} 
and satisfies the following $L^1(\R)$-estimate.
...
\begin{equation*}
    ...
    \begin{align*}
        ...
        &= \left(1 + 2\sqrt{2} \right)\sqrt{F_{\mathrm{ac}, \infty}}\Delta x^{\frac{1}{2}}.
    \end{align*}
\end{proof}
...
\begin{align*}
    ...
    & = I_1+I_2.
    \end{align*} 
  For $I_1$ note that using the same argument as the one leading to \eqref{hilfsabsch}, we obtain  
  \begin{equation*}
...
\end{corollary}
\begin{proof}
By \eqref{eq:convergence_V_xi_later} and \cite[Cor. 2.32]{RealAnalysisFolland}, there exists a subsequence $\{V_{\Delta x_m, \xi }\}_{m \in \mathbb{N}}$ such that $V_{\Delta x_m, \xi}(t,\cdot) \rightarrow V_{\xi}(t,\cdot)$ in $L^1(\R)$ for a.e. $t \in [0, \infty)$. Let $N \subset [0, \infty)$ be the null set of times for which the convergence does not hold. Then by \eqref{eq:seq_converges_V_xi} we have 
\begin{align*}
    V_{\Delta x_m, \infty}(t) = \|V_{\Delta x_m, \xi}(t) \|_1 \rightarrow \|V_{\xi}(t) \|_1 = V_{\infty}(t), 
\end{align*}
for all $t \in N^c$. 
\end{proof}

Observe that in the above argument the times for which the convergence in Corollary~\ref{corr:PointwiseConv} fails depend on the particular chosen subsequence. Therefore, there is no natural way to extend this convergence to the whole sequence.
...
\begin{proof}
Following the same line of reasoning as in the proof of \eqref{eq:initial_approx_U}, we obtain  \eqref{eq:convergence_u}. 
...
\begin{align}\nonumber
 ...
 & =  2e^{\frac12 t} \sum_{j=-\infty}^\infty (G_0(x_{2j+2})-G_0(x_{2j}))\Delta x+2e^{\frac12 t} G_\infty(0) \Delta x\\ \label{eq:L1:conv:conv}
 & = 4e^{\frac12 t} G_\infty(0)\Delta x,
 \end{align}
...
\end{align} 
To estimate \eqref{eq:L1_conv_G_part_1_3}, we exploit that $G_{\Delta x}(t)\in \text{BV}(\R)$, since it is monotonically increasing and bounded. Introducing $\Theta = \|y(t) - y_{\Delta x}(t) \|_{\infty}$, we therefore end up with
\begin{align}
...
\begin{theorem}
Given $(u_0, \mu_0, \nu_0) \in \D_0$, let $(u, \mu, \nu)(t)= T_t \left((u_0, \mu_0, \nu_0)\right)$ and $(u_{\Delta x}, \mu_{\Delta x}, \nu_{\Delta x})(t) = T_t \circ P_{\Delta x} \left((u_0, \mu_0, \nu_0)\right)$ for $t \in [0, \infty)$. Then\footnote{We  say that $\eta_{\Delta x} \Longrightarrow \eta$ if $\int_{\R}\phi d\eta_{\Delta x} \rightarrow \int_{\R}\phi d \eta$ for all  $\phi \in C_b(\R)= C(\R) \cap L^{\infty}(\R)$.},  
\begin{align}
...
G_{\Delta x}(t)& \to G(t) \text{ in } L^1(\R), \label{eq:convergence_G_11}\\
 ...
 as $\Delta x\to 0$.
  ...
  Furthermore, there exists a subsequence $\{\Delta x_m \}_{m \in \mathbb{N}}$ with $\Delta x_m \rightarrow 0$ as $m \rightarrow \infty$, and a null set $N \subset [0, \infty)$ such that for all $t \in N^c$ we have 
    \begin{align}
...
For \eqref{eq:weak_convergence_nu_2} on the other hand, we observe that $\phi (y_{\Delta x}(t))\in L^{\infty}(\R)$ and \phantom{aa} $H_{\Delta x, \xi}(t)\rightarrow H_{\xi}(t)$ in $L^1(\R)$, see \eqref{eq:convergence_H_xi_later}, imply
\begin{align*}
...
In the first example, wave breaking happens twice; initially, and also at a later time, and at each occurrence a finite amount of energy concentrates on a set of Lebesgue measure zero. For the second example on the other hand, the wave breaking times accumulate in the interval $[0, 3]$, but at every time only an infinitesimal amount of energy concentrates. The behavior of this latter example is thoroughly studied in \cite[Sec. 5.3]{myMaster}. In both cases, expressions for the exact solutions in Eulerian coordinates can be found, see Appendix \ref{sec:Appendix}.   

In the third example, we consider a cosine wave profile as initial data. Here, an expression for the Lagrangian solution is known, but the exact expression for $u$ in Eulerian coordinates is not available for comparison, see Appendix \ref{sec:Appendix}. The exact solution possesses accumulating wave breaking times, and, similarly to the cusp initial data, experiences wave breaking continuously in time, but now over the unbounded interval $[\frac{2}{\pi}, \infty)$. Furthermore, in contrast to the cusp example, the rate at which the total energy dissipates is nonlinear, making this an interesting challenge for the numerical algorithm.
...
\end{align*}
For the exact solution, which can be found in Appendix \ref{subsec:multipeakon}, the measure $\mu_0$ has a singular part. In addition, a finite amount of energy will concentrate in a single point at time $t=2$. 
...
Figure \ref{fig:ex1_convergence_order} displays the numerically computed errors. In this case the numerically computed convergence order, which is $\geq 0.99$ for both $u$ and $F$, is greater than the general order of the initial approximation error, see Proposition \ref{prop:prop_projection}. We also computed the errors using \eqref{eq:quantity_rhs}, which led to an order of $0.56$. This is significantly lower than what we observe in Figure \ref{fig:ex1_convergence_order}. Thus, the upper bound in \eqref{eq:convergence_u} is not optimal.
...
\begin{subfigure}{.99\textwidth}
  \centering
\includegraphics[width=0.8\textwidth]{Figures/conv_rate_peakon_example.pdf}
\end{subfigure}
...
Let $\alpha=\frac25$ and consider the initial data
 \begin{align*}
        ...
        d\mu_0 &= d\nu_0 = u_{0, x}^2dx. 
    \end{align*}
The exact solution for $t \in [0, 3]$ is computed in Appendix \ref{subsec:cusp}. Here the measure is purely absolutely continuous, but the wave breaking times accumulate. In particular, for each $t \in [0, 3]$, wave breaking happens at a single point. Furthermore, $u_{0,x}$ is quite irregular: $u_{0, x} \in L^1(\R) \cap L^2(\R)$, but $u_{0, x} \notin L^{\infty}(\R)$. 

In Figure \ref{fig:ex_2_cusp_solution} $(u_{\Delta x}, F_{\Delta x})$ with $\Delta x = 1.76 \cdot 10^{-1}$ and $\Delta x = 6.01 \cdot 10^{-3}$, and the exact solution, are displayed at $t=0$, $t=\frac{3}{2}$, and $t=3$. Figure \ref{fig:ex2_convergence_order} shows the errors as we refine the mesh. 
\begin{figure}
\centering
\begin{subfigure}{.99\textwidth}
  \centering
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/conv_rate_cusp_example.pdf}
...
As can be observed in Figure \ref{fig:ex2_convergence_order} of Example \ref{ex:numerical_cusp}, the convergence order is lower than in Example \ref{ex:numerical_multipeakon}, which suggests that accumulating wave breaking times affect the rate of convergence of the numerical method negatively. However, the upcoming example indicates that accumulation is of less significance than the regularity of the initial data.  
...
Moreover, for each $t \in \left(\tfrac{2}{\pi}, \infty\right)$, wave breaking occurs at four distinct isolated points both in Eulerian and Lagrangian coordinates, and happens continuously in the time interval $[\frac{2}{\pi}, \infty)$.

As mentioned, a closed form expression for $u$ in Eulerian coordinates is not available,  but the exact total energy $F_{\infty}(t)$ for all $t \geq 0$ and the exact solution in Lagrangian coordinates is given in Appendix \ref{subsec:cosinus}. A mapping of this solution into Eulerian coordinates is then used for comparison. Figure \ref{fig:ex_3_sinus_simulations} displays the time evolution of this solution and of two different numerical approximations at the times $t=0$, $\frac{2}{\pi}$, and $\frac{4}{\pi}$. 

Figure \ref{fig:ex3_convergence_order} shows the approximation errors. As can be seen from this plot, the convergence order is higher (from approximately $0.24$ to $1$ for $u$), even though the wave breaking times accumulate as in Example \ref{ex:numerical_cusp}. 
...
\begin{figure}
    \centering
\includegraphics[width=0.8\textwidth]{Figures/conv_rate_cosinus_example.pdf}
...
\newblock Pure and Applied Mathematics (New York). John Wiley \& Sons, Inc.,
  New York, second edition edition, 1999.
...
\newblock In {\em Current research in nonlinear analysis}, volume 135 of {\em
  Springer Optim. Appl.}, pages 157--201. Springer, Cham, 2018.
...
\newblock Academic Press Inc. San Diego CA, second edition edition, 2013.
...
\begin{align*}
   ...
   -\frac12 \alpha - \frac{1}{4} \left(2+\beta-\alpha \right)t , & x \leq x_1(t), \\
    ...
    F(t, x) &= \begin{cases}
    0, & x \leq x_1(t), \\
    \frac{8x-4\alpha+4\alpha t+(2+\beta-\alpha) t^2}{2^2}, & x_1(t) < x \leq x_2(t), \\
...
\begin{align*}
    B(t) &:= \int_{\frac{2}{\pi}}^t b(s, \alpha)ds 
    \\ 
    &=  \frac{\pi^2}{8}(2-\alpha) \left(t - \frac{2}{\pi} \right)^2 + \frac{1}{2}\alpha(1 - \pi t) + \alpha t \ln \left(\frac{2}{\pi} \right) + 2 \alpha  \sqrt{t^2 - \left(\frac{2}{\pi}\right)^2}
    \\ 
    & \quad + \frac{\alpha }{4} \left(\pi t^2\arcsin \left( \frac{2}{\pi t} \right) - 2 \sqrt{t^2 - \left( \frac{2}{\pi}\right)^2} + 4 t \arcosh \left(\frac{\pi}{2}t \right) \right) 
    \\
    & \quad  - \alpha \bigg(\frac{2}{\pi} \arctan \left(\frac{\pi}{2}\sqrt{t^2 - \left(\frac{2}{\pi}\right)^2} \right) + t\ln \left(\sqrt{t^2 - \left(\frac{2}{\pi}\right)^2} + t \right). 
\end{align*}
