...
\begin{abstract}
    ...
    Our code is publicly available.\footnote{Project page: \url{https://time-diffusion.github.io/}.}
\end{abstract}
...
When editing such assumptions, we generally try to avoid expensive data recollection and filtering, as well as model retraining or finetuning. These would consume considerable time and energy, thus significantly increasing the carbon footprint of deep learning research~\cite{strubell2020energy}.
...
TIME is highly efficient: It does not require optimization or finetuning, it can be applied in parallel for all cross-attention layers, and it modifies only a small portion of the diffusion model weights while leaving the language model unchanged.
...
We experiment with different versions of the loss function in \autoref{eq:loss} (\textit{e.g.}, only editing ${\W'}_V$, varying $\lambda$) %
and show this ablation study in \autoref{app:ablation_study}.
...
The dataset and more details about it are provided in \autoref{app:gender}.
...
Note that as the baseline model is more biased, the editing should be stronger. %
 Consequently, we adjust a different $\lambda_p$ for each profession $p$, aiming for an equal gender representation in generations for the prompt ``A photo of a/an [profession]'', which is not included in the test prompts.
...
To do so, we generate $24$ images for each test prompt, and
 use CLIP~\cite{clip} to classify each image as either ``A female [profession]'' or ``A male [profession]''.
...
We present $\Delta$, along with the percentage of females $F_p$ in the test prompt generations for a representative subset of professions.
We report these metrics for the unedited baseline model, as well as the model debiased using TIME.
...
We expand upon this discussion and present additional analyses of our results in \autoref{app:gender}.
