...
Over the last three decades, many countries have experienced appreciation in housing prices, with upward trends in the price-rent ratio.\footnote{\label{fn:OECD}For OECD countries, see \url{https://data.oecd.org/price/housing-prices.htm}.} The situation is often referred to in the popular press as a housing bubble. Many academics and policymakers want to understand why and how housing bubbles emerge in the first place because housing bubbles and their collapse are often associated with macroeconomic problems \citep{JordaSchularickTaylor2015}. However, the mechanism of the emergence of housing bubbles is not well understood. In addition, theoretically, it is well known since the \citet{SantosWoodford1997} Bubble Impossibility Theorem that there is a fundamental difficulty in generating asset price bubbles in dividend-paying assets such as housing, land, and stocks.\footnote{The Bubble Impossibility Theorem can be summarized by their Theorem 3.3, which states that if the present value of the aggregate endowment is finite, then the price of an asset in positive net supply or with finite maturity equals its fundamental value defined by the present value of the dividend stream of the asset.} The theory of rational bubbles attached to assets with positive dividends remains largely underdeveloped. 

This paper presents a theory of rational housing bubbles. In particular, we are interested in the following questions.
\begin{enumerate*}
...
We emphasize that we obtain these highly nontrivial results and draw new insights from what could be called the simplest possible model of housing. We thus see our paper as a fundamental theoretical contribution that could be used as a stepping stone for constructing more complicated models aimed for empirical or quantitative analysis.
...
Although our paper is theoretical, there are three basic testable implications to be drawn from our analysis. First, from the analysis on the long run behavior, housing bubbles are more likely to emerge if the incomes (or available funds through credit) of home buyers are higher or expected to be higher. If the incomes of home buyers rise as economic development progresses, housing bubbles may naturally arise first by optimistic expectations, and then inevitably emerge as the optimistic fundamentals materialize. There is some empirical evidence consistent with this narrative. \citet{GyourkoMayerSinai2013} document that an increase in the high-income population in a metropolitan area is associated with high housing appreciation. \citet{BarlevyFisher2021} document that the share of interest-only mortgages is correlated with the housing price growth rates across regions. 

Second, if there is a housing bubble on the long run trend, rents grow at rate $G^\gamma$, whereas housing prices grow at rate $G$, implying that the price-rent ratio will rise. Hence, an upward trend in the price-rent ratio could be an indicator for housing bubbles. Here we present some anecdotal evidence of this implication. According to OECD data cited in Footnote \ref{fn:OECD}, we can observe that in many OECD countries including OECD total, the price-rent ratio has shown an upward trend over the last three decades. Figure \ref{fig:ts} shows the time series of U.S. real housing price and rent indexes. We can see that the housing price index has a steeper trend than the rent index, suggesting an upward trend in the price-rent ratio. Figure \ref{fig:rent_price} plots the housing price against rent in log-log scale across U.S. counties for 2019 (other years are similar). If we interpret housing prices as reflecting the incomes of the home buyers in the model, the slope $1/\gamma=1.34>1$ is consistent with our theoretical analysis.

\begin{figure}[htb!]
\centering
\begin{subfigure}{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/fig_ts.pdf}
    \caption{Housing price and rent index.}\label{fig:ts}
\end{subfigure}
\begin{subfigure}{0.48\linewidth}
...
\end{subfigure}
\caption{Time series and cross-section of U.S. housing prices and rents.}\label{fig:stylizedfact}
\caption*{\footnotesize See Appendix \ref{sec:data} for detailed explanation.}
\end{figure}

Third, if the incomes of home buyers are expected to increase in the future with economic development, this expectation generates a simultaneous rise in the price-rent ratio and the price-income ratio, even before incomes actually start to rise. During this transition, the housing price dynamics may appear unsustainable because prices grow faster than incomes. The OECD data show that in countries including Ireland, Spain, U.K, and U.S. that experienced housing booms and busts, both ratios continued to rise during housing booms and then fell with their busts. Moreover, together with the first testable implication, when mortgages are expected to become more available in the future, similar expectation-driven housing booms and busts would occur.

\subsection{Related literature}

Our paper belongs to the large theoretical literature of rational bubbles. Since \citet{Samuelson1958} and \citet{Tirole1985}, the existing literature has almost exclusively focused on a special case of pure bubbles, namely assets that pay no dividend, which are intrinsically useless like fiat money.\footnote{It is well known since \citet{Samuelson1958} and \citet{Tirole1985} that in overlapping generations (OLG) models, a pure bubble asset like fiat money may have a positive price. \citet{Bewley1980} constructed an example with infinitely-lived agents when they are subject to shortsales constraints. \citet{Kocherlakota1992} showed that, in such bubbly equilibria, the present value of endowments must be infinite whenever the shortsales constraint binds. See \citet{Gali2021}, \citet{Plantin2021}, \citet{Kocherlakota2022}, and \citet{Guerron-QuintanaHiranoJinnai2023} for the recent development of pure bubble models.} Note that pure bubble models are subject to several criticisms as discussed in Introduction of \citet*{HiranoJinnaiTodaBubble}. As noted in Introduction, it is well known that there are fundamental difficulties in generating bubbles in dividend-paying assets summarized by the \citet{SantosWoodford1997} Bubble Impossibility Theorem. 

To the best of our knowledge, there are only two papers that study rational bubbles attached to dividend-paying assets.\footnote{ \citet*{AllenBarlevyGale2022} studied a risk-shifting model in which borrowers know the risks of their investments better than lenders. This asymmetric information encourages the borrowers to gamble on risky assets, allowing asset prices to exceed fundamentals. Although our approach is different because we focus on rational bubbles and abstract from asymmetric information, the two different approach should be viewed as 
complementary, not as a substitute. } \citet[Proposition 1(c)]{Tirole1985} shows that in an OLG model with population growth and a long-lived asset generating constant dividends, if the interest rate without the asset is negative, bubbles necessarily occur. There are substantial differences between his model and ours. First, unlike \citet{Tirole1985}, our main focus is to study housing bubbles in a model where housing provides housing services. As far as we are aware, our paper is the first to prove the existence of rational bubbles attached to housing yielding positive rents in a dynamic general equilibrium model. Second, although \citet{Tirole1985} recognized the possibility of the nonexistence of fundamental equilibria, he did not necessarily provide a formal proof nor investigate the local determinacy of the bubbly equilibrium. In contrast, we provide a full analysis of the (non)existence and the inevitability of housing bubbles, including local determinacy. Third, housing rents are endogenously determined in our model, while in his model, the dividends are assumed to be exogenous. The other paper on rational bubbles attached to dividend-paying assets is a recent paper by \citet*{HiranoJinnaiTodaBubble}. They provide the Bubble Characterization Theorem for dividend-paying assets and show that there exist plausible economic models in which a bubbly equilibrium exists but fundamental equilibria do not. They study an endogenous growth model with two sectors in which in the production sector, heterogeneous entrepreneurs invest capital using leverage and the land sector generates positive dividends. They find that when leverage gets sufficiently high, the production sector grows faster than the land sector, which inevitably makes land prices higher and disconnected from dividends, generating a land price bubble. 
...
\section{Details on Figure \ref{fig:stylizedfact}}\label{sec:data}

This appendix explains how we construct Figure \ref{fig:stylizedfact}.
