...
%opening
\title{Housing Bubbles with Phase Transitions}
...
\begin{enumerate*}
    \item How can equilibrium housing prices be disconnected from fundamentals in the long term, exhibiting a speculative bubble in a dynamic general equilibrium setting in which housing rents and prices are both endogenously determined?
...
% rational bubble
Our paper is related to the literature on the valuation of housing. Unlike quantitative models reviewed in \citet{PiazzesiSchneider2016}, our primary interest is to study conditions under which housing \emph{can} or \emph{must} be overvalued, in the sense that equilibrium housing prices contain a speculative aspect. Our paper belongs to the so-called ``rational bubble literature'' that studies bubbles as speculation, which was pioneered by \citet{Samuelson1958}, \citet{Bewley1980}, \citet{Tirole1985}, \citet{ScheinkmanWeiss1986}, \citet{Kocherlakota1992}, and \citet{SantosWoodford1997}. Theoretical foundations in rational bubble models include \citet{HuangWerner2000}, \citet{HellwigLorenzoni2009}, and \citet{BloiseCitanna2019}, among others.\footnote{See \citet{HiranoToda2024JME} for a recent review of this literature. There are other approaches including heterogeneous beliefs \citep{ScheinkmanXiong2003,FostelGeanakoplos2012Tranching}, asymmetric information \citep{Barlevy2014,AllenBarlevyGale2022}, among others. See \citet{HiranoTodaClarification} for a discussion of other approaches as well as the confusion in the literature.}
...
Due to the aforementioned fundamental difficulty of attaching bubbles to dividend-paying assets, there are only a handful papers that treat this topic. \citet[\S7]{Wilson1981} provides the first example of bubbles attached to dividend-paying assets (see \citet{HiranoTodaNecessity} for more discussion of this example). \citet[Proposition 1(c)]{Tirole1985} recognizes that, with dividend-paying assets, bubbles could be necessary for equilibrium existence if the bubbleless interest rate is less than the dividend growth rate. There are important differences from \citet{Tirole1985} and our results.
\begin{enumerate*}
    ...
    \item Second, Tirole assumes exogenous and constant dividends. In contrast, in our model housing prices and rents are both endogenous. Under this circumstance, it is not obvious whether housing prices \emph{can} or \emph{must} grow faster than rents, \ie, whether housing bubbles \emph{can} or \emph{must} arise.
\end{enumerate*} 
The recent forthcoming paper of \citet{HiranoTodaNecessity} establishes the concept of the necessity of bubbles in modern macro-finance models including OLG models and Bewley-type infinite-horizon models. Our results build on this earlier paper (see, for instance, Lemma \ref{lem:neccesity}) but there are two main differences.
\begin{enumerate*}
    \item First, in our model rents are endogenous. Nevertheless, we characterize the long run rent growth rate in \emph{all} equilibria (Theorem \ref{thm:Gr}), and we identify the importance of the income ratio between the young and old and the elasticity of substitution between consumption and housing for endogenously satisfying the bubble necessity condition.
    \item Second, we provide a complete analysis of the long run behavior of equilibria.
\end{enumerate*}
...
\begin{proof}
The existence of equilibrium is standard \citep{BalaskoShell1980} and follows from the same argument as the proof of \citet[Theorem 1]{HiranoTodaNecessity}. The equilibrium quantities \eqref{eq:eqobj} follow from the preceding argument.
\end{proof}
...
\subsection{Assumptions}\label{subsec:model_asmp}
...
Assumption \ref{asmp:U}\ref{item:U_c} implies that agents (apart from the initial old) care about consumption $(c^y,c^o)$ only through the homothetic composite consumption $c(c^y,c^o)$, which (together with Assumption \ref{asmp:G}) allows us to study asymptotically balanced growth paths. Assumption \ref{asmp:U}\ref{item:U_u} implies that agents have constant elasticity of substitution $1/\gamma>0$ between consumption and housing service.\footnote{To see this, consider an agent who maximizes utility $u(c)+v(h)$ subject to the budget constraint $c+rh\le w$, where $r>0$ is the rent (in units of consumption). Letting $\lambda$ be the Lagrange multiplier, the first-order conditions are $c^{-\gamma}=\lambda$ and $v'(h)=\lambda r$. But in equilibrium we have $h=1$, so $r=v'(1)c^\gamma$. Log-differentiating both sides, we obtain $-\frac{\partial \log(h/c)}{\partial \log r}=\frac{\partial \log c}{\partial \log r}=\frac{1}{\gamma}$, so the elasticity of substitution between consumption and housing is $1/\gamma$.}
...
The second result is the Bubble Necessity Theorem due to \citet{HiranoTodaNecessity}. To make the paper self-contained but to avoid technicalities, here we specialize the setting of \citet{HiranoTodaNecessity}. As in \S\ref{sec:model}, consider a two-period OLG model with a long-lived asset but assume that there is a single perishable good and the date-$t$ dividend $D_t\ge 0$ is exogenous (unlike our setting with endogenous rents). Define the long run dividend growth rate by
\begin{equation}
...
\begin{lem}[\citealp{HiranoTodaNecessity}, Theorem 2]\label{lem:neccesity}
Suppose that
\begin{enumerate*}
...
\end{equation}
where $m\coloneqq v'(1)$ is the marginal utility of housing service and $c,c_y,c_z$ are evaluated at $(y,z)=(e_t^y-S_t,e_{t+1}^o+S_{t+1})$. To study asymptotically balanced growth paths, let $s_t\coloneqq S_t/e_t^y=S_t/(e_1G^t)$ be the housing expenditure normalized by the income of the young. Since $c$ is homogeneous of degree 1, its partial derivatives $c_y,c_z$ are homogeneous of degree 0. Therefore dividing both sides of \eqref{eq:s_dynamics2} by $e_1G^t$, we obtain
\begin{equation}
    Gs_{t+1}c_z=s_tc_y-me_1^{\gamma-1}G^{(\gamma-1)t}c^\gamma, \label{eq:s_dynamics3}
\end{equation}
where $c,c_y,c_z$ are evaluated at $(y,z)=(1-s_t,G(w+s_{t+1}))$ for the old to young income ratio $w\coloneqq e^o/e^y$.
...
\begin{subequations}\label{eq:H}
    ...
    \end{align}
\end{subequations}
and $c,c_y,c_z$ are evaluated at $(y,z)=(1-\xi_1,G(w+\eta_1))$ with $w\coloneqq e^o/e^y$. We can now define a long run equilibrium.
...
If $\xi_t\to \xi$, since $G>1$ and $\gamma\in (0,1)$, we have $\Phi(\xi,\xi)=0$ if and only if $\xi_2=0$ and $\xi_1(Gc_z-c_y)=0$, where $c_y,c_z$ are evaluated at $(y,z)=(1-\xi_1,G(w+\xi_1))$. Clearly $\xi_f^*\coloneqq (0,0)$ is a steady state of $\Phi$, which we refer to as the \emph{fundamental} steady state.\footnote{Obviously, the steady state corresponds to the detrended system $\Phi(\xi_t,\xi_{t+1})=0$. The original economy grows at rate $G$, so there is an asymptotically balanced growth path instead of a steady state.} In order for $\Phi$ to have a nontrivial ($\xi_1=s>0$) steady state, which we refer to as the \emph{bubbly} steady state, it is necessary and sufficient that $Gc_z-c_y=0$.
...
Suppose Assumptions \ref{asmp:G}--\ref{asmp:c} hold and $\gamma<1$. Let $w=e^o/e^y$ and $w_f^*,w_b^*$ be as in \eqref{eq:w*f} and \eqref{eq:w*b}. Then the following statements are true.
\begin{enumerate}
...
\draw[->] (0,-0.1) -- (0,2.5) node[above] {$1/w=e^y/e^o$};
...
% plot the axes
\draw[->] (-0.1,0) -- (4.03,0) node[below] {$1/w=e^y/e^o$};
...
Suppose Assumptions \ref{asmp:G}--\ref{asmp:c} hold, $\gamma<1$, and let $w=e^o/e^y$. Then the following statements are true.
\begin{enumerate}
...
\begin{equation*}
    q_tP_t=\left(q_T\prod_{s=T}^{t-1}\frac{1}{R_s}\right)P_t\le q_T R^{T-t}e_1G^t=e_1q_TR^T(G/R)^t\to 0
\end{equation*}
...
\begin{equation}
    r_t=\frac{v'(1)}{u'(c)c_y}=m\frac{c(e_t^y-S_t,e_{t+1}^o+s_{t+1})^\gamma}{c_y(e_t^y-S_t,e_{t+1}^o+s_{t+1})}.\label{eq:r_gamma}
\end{equation}
...
Since $P_t\le S_t\sim e_1G^t$ grows at rate at most $G$ and the risk-free rate diverges to infinity (hence eventually exceeds the housing price growth rate), the transversality condition holds and there is no housing bubble. Using \eqref{eq:Pr>1}, we obtain $P_t/r_t\to 0$, so the price-rent ratio converges to 0. The Pareto efficiency of equilibrium follows from \eqref{eq:R>1} and Lemma \ref{lem:efficient}\ref{item:efficient1}.
\end{proof}
...
\end{equation*}
so the transversality condition holds and there is no housing bubble. The Pareto efficiency of equilibrium follows from \eqref{eq:R=1} and Lemma \ref{lem:efficient}\ref{item:efficient1}.
