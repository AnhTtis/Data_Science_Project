...
We will show that given that gradient flow is run starting from a small initialization, at convergence, the column norms $\| U e_i \|_2$'s approximately distribute themselves proportional to the inner product of the corresponding column of $U$ with $U_\star$ initially at $t=0$. Namely,
...
\subsection{Understanding the gradient flow: Proof of \Cref{lemma:ratiopreserved}}
...
\end{align}
where the last inequality uses the fact that when $t \le 4 T_0$, $(1 - \| E(0) \|_F^2) (t - T_0) \ge t - T_0 - 4$ (see the analysis in \cref{eq:62-1-1,eq:E0r0_bound}). This inequality also uses \Cref{lemma:T0bound} to bound $\| r(0) \|_2^2$.
From \Cref{lemma:Er-bound}, therefore, denoting $x = \| r \|_2^2$, for some absolute constant $C_{\ref{eq:r2-rate-1}} > 0$,
\begin{align}
...
Below we state and prove a stronger version of \Cref{theorem:lb-implicit} which follows by combining \Cref{lemma:ratiopreserved,lemma:unidist}. The strengthening is in the sense of holding for any constant lower bound on the ratio of the norm of a column to that of the maximum (and not just $0.99$ as in \Cref{theorem:lb-implicit}).

\begin{theorem}
\label{theorem:lb-implicit-restate}
Consider the population loss defined in \cref{eq:Lpop}, for the case that $r=1$ and $k\gg 1$. Moreover, assume that the entries of the initial model $U_0$ are i.i.d. samples from $\mathcal{N} (0,\alpha^2)$, where $\alpha \le \eta^2/k^3 d \log (kd)$. Then, for any constant $\eta \in (0,1)$, the iterates generated by gradient descent converge to a model $U_{gd}$ where $\widetilde{\Omega} (k^{\eta})$ columns of which are active and satisfy, 
\begin{align}
...
\end{theorem}
\begin{proof}
Consider any $i \in [k]$, such that,
\begin{align} \label{eq:initr}
...
As introduced earlier in \cref{eq:apxorth}, the main result we prove in this section is that the ``bounded'' (in operator norm) $(\epsilon,\gamma)$-approximate second-order stationary points of $f_{\text{pop}}$ satisfies the following condition,
\begin{equation} \nonumber
...
\end{lemma}
Regardless of whether $U$ is approximately stationary or not, for any $i \ne j \in [k]$, if $\min \{ \| U e_i\|_2, \| U e_j \|_2 \} \le 2\sqrt{\beta}$, then,
\begin{align}
...
\end{align}
This proves the first part of the theorem statement. Henceforth, we will assume that $\min \{ \| U e_i\|_2, \| U e_j \|_2 \} \ge 2\sqrt{\beta}$. This condition will turn out to lower bound the Frobenius norm of a matrix $Z$ which we will require later for the proof of the second part.
...
\begin{align}
    &\vect (Z)^T [ \nabla^2 (\cL + \lambda \cR_\beta) (U)] \vect(Z) \\
    &\le \epsilon \| Z \|_F + \lambda \tr ( (W L - L W ) \cdot Z^T U ) - \lambda (G(U)_{11} + G(U)_{22} ) \langle Z e_i, U e_i \rangle^2 \label{eq:middleterm-2}
\end{align}
...
\end{align}
which is precisely the bound in \cref{eq:corrlb}. On the other hand, under the large-$\Delta$ condition, from \cref{eq:initbound}, we have that,
\begin{align}
...
Consider an $\epsilon$-approximate first-order stationary point $U$. If the generalization loss is large, i.e. $\| UU^T - U_\star U_\star^T \|_F \ge 8 \max \left\{ \epsilon^{2/3} d^{1/6}, \lambda \sqrt{\frac{2k}{\beta}} \right\}$, then the Hessian of $\cL + \lambda \cR_\beta$ at $U$ has a large negative eigenvalue,
\begin{align} \label{eq:181}
    \lambda_{\min} [\nabla^2 (\cL + \lambda \cR_\beta) (U)] \vect(Z) \le - \frac{1}{\sqrt{2k}} \| UU^T - U_\star U_\star^T \|_F.
\end{align}
In other words, at an $(\epsilon,\gamma)$-approximate second order stationary point,
\begin{align}
    \| UU^T - U_\star U_\star^T \|_F \lesssim \max \left\{ \epsilon^{2/3} d^{1/6}, \lambda \sqrt{\frac{2k}{\beta}}, \sqrt{k} \gamma \right\} \label{eq:182}
\end{align}
\end{lemma}
\begin{proof}
...
\end{align}
\end{lemma}
\noindent The proof of this result is deferred to \Cref{app:B2}.\\

\noindent Finally, combining \Cref{lemma:26} with \Cref{eq:21112},
\begin{align}
    \vect(Z)^T [\nabla^2 (\cL + \lambda \cR_\beta) (U)] \vect(Z) &\le - 2 \| UU^T - U_\star U_\star^T \|_F^2 + 4 \epsilon \| Z \|_F \nonumber\\
...
Next we show that each column of $U$ is almost entirely contained in the span of $V_r$ in that the angle between $U e_i$ and its projection onto $V_r^\perp$ is almost 90$^\circ$.

\begin{lemma} \label[lemma]{lemma:ZFbound}
Consider an $\epsilon$-approximate first order stationary point $U$. Let $V_r$ denote the matrix with columns as the non-zero eigenvectors of $U_\star U_\star^T$. Then,
\begin{align}
    \| V_r^\perp (V_r^\perp)^T U \|_F \le \epsilon k.
\end{align}
\end{lemma}
\begin{proof}
By \cref{eq:plass1} for $Z = V_r^\perp (V_r^\perp)^T U$,
...
\begin{align}
    ...
    &\ge 4\| V_r^\perp (V_r^\perp)^T U U^T  \|_F^2 \\
    &\ge 4\| V_r^\perp (V_r^\perp)^T U \cdot (V_r^\perp (V_r^\perp)^T U)^T \|_F^2 \\
    &= 4\| Z Z^T \|_F^2 \\
    &\ge \frac{4}{\textsf{rank} (Z)} \| Z \|_F^4 \\
    &= \frac{4}{k} \| Z \|_F^2. \label{eq:cp2}
\end{align}
Combining with \cref{eq:plass2} completes the proof (we drop the constant $4$ for succinctness).
\end{proof}

\begin{lemma} \label[lemma]{lemma:proj-similar}
Consider an $\epsilon$-approximate first order stationary point $U$.  Let $V_r$ be as defined in \Cref{lemma:ZFbound}. Then, for any column $i \in [k]$ such that $\| U e_i \|_2 \ge 2\sqrt{\beta}$,
    \begin{align}
        \frac{\| V_r^\perp (V_r^\perp)^T U e_i \|_2}{\| U e_i \|_2} \le \epsilon \sqrt{\frac{k}{\beta^{1/2} \lambda}}.
    \end{align}
    Note that the LHS is also the cosine of the angle between $U e_i$ and its projection onto $V_r^\perp$, or the sine of the angle between $U e_i$ and its projection onto $V_r$.
\end{lemma}
\begin{proof}
At an $\epsilon$-approximate first order stationary point the gradient is upper bounded in $L_2$-norm by $\epsilon$. From the gradient and Hessian computations in \Cref{lemma:L-grad-hessian} and \Cref{lemma:R-grad-hessian}, with $Z = V_r^\perp (V_r^\perp)^T U$, this implies that,
\begin{align} \label{eq:plass1}
    2\langle UU^T - U_\star U_\star^T, U Z^T + ZU^T \rangle + \lambda \tr ( D(U) Z^T U) \le \epsilon \| Z \|_F
\end{align}
Noting that $U_\star$ has rank $r$ and is orthogonal to $V_r^\perp$, the LHS simplifies as,
\begin{align}
    &4\tr (UU^T V_r^\perp (V_r^\perp)^T U U^T ) + \lambda \tr (U D(U) U^T V_r^\perp (V_r^\perp)^T ) \label{eq:00000}\\
    &\ge \lambda \sum_{j=1}^{k} D(U)_{jj} \| V_r (V_r^\perp)^T U e_j \|_2^2 \\
    &\ge \frac{\lambda}{2} \frac{\| V_r (V_r^\perp)^T U e_i \|_2^2}{\| U e_i \|_2},
\end{align}
where the last inequality is for any column $i$ such that $\| U e_i \|_2 \ge 2\sqrt{\beta}$, since we have that $D(U)_{ii} = \frac{2 \beta + \| U e_i \|_2^2}{(\| U e_i \|_2^2 + \beta)^{3/2}} \ge \frac{1}{2 \| U e_i \|_2}$. Putting everything together, we get that
\begin{align}
    \frac{\lambda}{2} \| U e_i \|_2 \cdot \frac{\| V_r (V_r^\perp)^T U e_i \|_2^2}{\| U e_i \|_2^2} \le \epsilon \| Z \|_F \label{eq:00001}
\end{align}
Since $\| U e_i \|_2 \ge 2\sqrt{\beta}$, rearranging the terms around and finally upper bounding $\| Z \|_F$ using \Cref{lemma:ZFbound} completes the proof.
\end{proof}
...
\begin{align} \label{eq:pruning_condition}
    \| U e_i \|_2 \le \max \{ 2\sqrt{\beta}, \epsilon k \} \implies U_{\text{prune}} e_i \gets 0
\end{align}
Suppose $\epsilon^2 < c \beta^{1/2} \lambda/ k r^2$ and $\epsilon + \gamma \le c\lambda /r^2$. Then,
\begin{enumerate}
    \item $U_{\text{prune}}$ has at most $r$ non-zero columns.
    \item Furthermore,
\begin{align}
    \| U_{\text{prune}} U_{\text{prune}}^T - U_\star U_\star^T \|_F \lesssim \max \left\{ \sqrt{k}\gamma, \epsilon^{2/3} k^{1/6}, \lambda \sqrt{\frac{k}{\beta}}, k \beta, \epsilon^2 k^3 \right\}.
    \end{align}
\end{enumerate}
\end{theorem}
\begin{proof}
...
In \Cref{lemma:apx-orthogonal}, this means that the condition \cref{eq:corrlb} is therefore violated, and for every pair of column indices $i \ne j \in S$ where $S = \{ i \in [k] : \| U e_i \|_2 > 0 \}$,
\begin{align}
...
At any $\epsilon$-approximate first order stationary point $U$, by \Cref{lemma:proj-similar}, for any $i$ such that $\| U e_i \|_2 \ge 2\sqrt{\beta}$ the angle between $U e_i$ and its projection onto $V_r$ is intuitively small. By \cref{eq:thetaij}, we expect $U e_i$ and $U e_j$ to have an angle close to 90$^\circ$ between them. Therefore, we expect the projections of $U e_i$ and $U e_j$ onto $V_r$ to also have an angle close to 90$^\circ$ between them. Given vectors $v_1,v_2,v_3$ and $v_4$ such that $|\angle v_1, v_2| \le \varepsilon_1$, $|\angle v_2, v_3 - 90^\circ| \le \varepsilon_2$ and $|\angle v_3, v_4 | \le \varepsilon_3$, then $|\angle v_1, v_4 - 90^\circ| \le \varepsilon_1 + \varepsilon_2 + \varepsilon_3$. This means that for any columns $Ue_i$ and $U e_j$ such that $\| U e_i \|_2$ and $\| U e_j \|_2 \ge \sqrt{\beta}$,
\begin{align}
    \left| \theta^{\text{proj}}_{ij} - \frac{\pi}{2} \right| \lesssim \sqrt{\frac{\epsilon + \gamma}{\lambda}} + \epsilon \sqrt{\frac{k}{\beta^{1/2} \lambda}} < \frac{c_1}{r} \label{eq:orthproj}
\end{align}
for some small absolute constant $0 \le c_1 \le 1/3$ and $\theta_{ij}^{\text{proj}}$ is the angle between $V_r V_r^T U e_i$ and $V_r V_r^T U e_j$. This assumes that $\epsilon^2 < c \beta^{1/2} \lambda/ k r^2$ and $\epsilon + \gamma \le c \lambda /r^2$ for a sufficiently small constant $c > 0$. This can be written differently as,
\begin{align} \label{eq:orthproj:vectors}
    \forall i \ne j \in S,\ \frac{|\langle V_r V_r^T U e_i, V_r V_r^T U e_j \rangle|}{\| V_r V_r^T U e_i \|_2 \| V_r V_r^T U e_j \|_2} < \frac{c_1}{r}.
\end{align}
\cref{eq:orthproj:vectors} implies that the projections of the columns of $U_{\text{prune}}$ onto $V_r$ are approximately at 90$^\circ$ to each other. However, intuitively, since $V_r$ is an $r$ dimensional space, this means that at most $r$ of these projections can be large in norm.

\begin{lemma} \label[lemma]{lemma:atmostr}
Let $S_{\max} \subseteq [k]$ denote the subset of column indices of $U$ such that the corresponding column has a non-zero projection onto $V_r$. Then, $|S_{\max}| \le r$.
\end{lemma}
\noindent The proof of this result is deferred to \Cref{app:B2}.\\

\noindent What is the implication of \Cref{lemma:atmostr}? At most $r$ columns of $U$ and therefore $U_{\text{prune}}$ have non-zero projections onto $V_r$. Moreover by implication of \Cref{lemma:ZFbound}, for any column $i \in [k]$, its projection onto $V_r^\perp$, $\| V_r^\perp (V_r^\perp)^T U e_i \|_F$, is small at an approximate stationary point. By decomposing the $\ell_2$ norm of $U e_i$ onto its component on $V_r$ and its component on $V_r^\perp$, this implies that at most $r$ columns of $U$ can be large in norm. Formally, consider any of the $\ge k - r$ columns of $U$ having $0$ projections onto $V_r$, i.e. $i \in [k] \setminus S_{\max}$. Then,
\begin{align}
    \| U e_i \|_2 &= \| V_r^\perp (V_r^\perp)^T U e_i \|_2 \\
    &\le \| V_r^\perp (V_r^\perp)^T U \|_F \\
    &\le \epsilon k,
\end{align}
which follows from \Cref{lemma:ZFbound}. Note however, by the pruning condition in \Cref{eq:pruning_condition}, all columns of $U$ with norm smaller than $\epsilon k$ are pruned out. Therefore for all $i \in [k] \setminus S_{\max}$, this means that,
\begin{align}
    \| U_{\text{prune}} e_i \|_2 = 0.
\end{align}
And by extension, $U_{\text{prune}}$ has at most $r$ non-zero columns.

For the analysis of $\| U_{\text{prune}} U_{\text{prune}}^T - U_\star U_\star^T \|_F$, note that, the columns of $U$ with $L_2$ norm smaller than $\omega \triangleq \max \{ 2\sqrt{\beta}, \epsilon k \}$ are set to $0$ in $U_{\text{prune}}$. This implies that,
\begin{align}
    \| U_{\text{prune}} U_{\text{prune}}^T - U_\star U_\star^T \|_F - \| UU^T - U_\star U_\star^T \|_F &\le \| U_{\text{prune}} U_{\text{prune}}^T - U U^T \|_F \label{eq:komega1}\\
    &\le \left\| \sum_{i \in [k]} U e_i (U e_i)^T \mathbb{I} ( \| U e_i \|_2 \le \omega) \right\|_F \\
    &\le \sum_{i \in [k]} \left\| U e_i (U e_i)^T \mathbb{I} ( \| U e_i \|_2 \le \omega) \right\|_F \\
    &\le \sum_{i \in [k]} \| U e_i \|_2^2 \mathbb{I} ( \| U e_i \|_2 \le \omega) \\
    &\le k \omega^2 \label{eq:komega2}
\end{align}
Plugging in the value of $\omega$ and combining with the guarantee on $\| UU^T - U_\star U_\star^T \|_F$ from \Cref{lemma:negativeeig} completes the proof.
\end{proof}
...
\begin{align} \label{eq:91103332}
    \| U_{\text{prune}} U_{\text{prune}}^T - U_\star U_\star^T \|_F \lesssim \max \left\{ \sqrt{k} \gamma, \epsilon^{2/3} k^{1/6}, \lambda \sqrt{\frac{k}{\beta}}, k \beta, \epsilon^2 k^3 \right\}.
\end{align}
For the sufficiently small choice of regularization and smoothing parameters, and the bounds on $\epsilon$ and $\gamma$, the bound in \cref{eq:91103332} results in,
...
\begin{proof}
By definition of $W$ and $L$, $w\tr ((W L - L W ) \cdot Z^T U)$, can be simplified to
\begin{align}
...
\begin{align}
    ...
    &\le \frac{2}{\sqrt{\beta}} \| Z \|_F^2 + 4 \| D(U) \|_F \| Z U^T \|_F \\
    &\overset{(i)}{\le} \frac{2}{\sqrt{\beta}} \| Z \|_F^2 + 8 \sqrt{\frac{k}{\beta}} \| Z U^T \|_F \\
...
\end{align}
where $(i)$ follows from the fact that for any $i \in [k]$, $| D(U))_{ii} | \le \max_{x \ge 0} \frac{x+2\beta}{(x + \beta)^{3/2}} = \frac{2}{\sqrt{\beta}}$, $(ii)$ uses \cref{eq:L41} and
the last inequality uses the fact that $\| Z \|_F^4 = \| Z^T \|_F^4 \le \textsf{rank} (Z^T) \| Z Z^T \|_F^2\le 2 k \| UU^T - U_\star U_\star^T \|_F^2$ by \cref{eq:L40}.
\end{proof}
...
\begin{align}
\mathbb{P}\left(\left\|\frac{1}{n} \sum_i^n A_i \epsilon_i\right\|_{\text{op}} \ge 4  \sqrt{\frac{d \log(d/\eta)}{n}}\right) \leq \eta.
\end{align}
\end{lemma}
\begin{proof}
...
\noindent By virtue of this result, we expect that for all $U$, with probability $\ge 1 - \eta$, the loss $\cL$ in \cref{eq:lossexpanded} is $\approx \| UU^T - U_\star U_\star^T \|_F^2$ when $n$ is large. Indeed, by an application of Holder's inequality, $\tr (AB) \le \| A \|_{\text{op}} \| B \|_* \le \| A \|_{\text{op}} \sqrt{\textsf{rank} (B)} \| B \|_F$ and,
\begin{align}
...
\noindent Prior to proving our results, we define the choice of parameters of the algorithm which we will use throughout.

\begin{definition}[Choice of parameters] \label[definition]{def:parameter}
Suppose,
\begin{align}
    &\lambda \beta^{1/2} \le c \min \left\{ kr (\sigma_r^\star)^4, \frac{r ( \sigma_r^\star )^2}{k^2} \right\}, \nonumber\\
    &n \ge \frac{\sigma^2  \cdot k^2 r^2 d \log (d/\eta)}{c_{\ref{def:parameter}} \lambda \beta^{1/2}}, \label{eq:lowerbound-n}\\
    &\epsilon \le c_{\ref{def:parameter}} \min \left\{ \frac{\lambda^{3/4} \beta^{3/8}}{(\delta r)^{3/2} k}, \frac{\lambda}{r^2} \right\}, \nonumber\\ 
    &\lambda \le c_{\ref{def:parameter}} \frac{\beta^{3/2}}{k^2 (\delta r)^2}, \text{ and,} \nonumber\\
    &\gamma \le c_{\ref{def:parameter}} \min \left\{ \frac{\sqrt{\beta^{1/2} \lambda}}{\delta r k}, \frac{\lambda}{r^2} \right\}, \nonumber \\
    &\beta \le c_{\ref{def:parameter}} \frac{(\sigma_r^\star)^2}{k}, \nonumber\\
    &\delta \le \frac{1}{10}, \label{eq:parameter}
\end{align}
for some sufficiently small universal constant $c_{\ref{def:parameter}} > 0$.
\end{definition}

\noindent Note that in order for the measurements to satisfy $(k,\delta)$-RIP for $\delta = \frac{1}{10}$, it suffices to have $n \gtrsim k d \log(d)$ Gaussian measurements which is satisfied by the lower bound on $n$ in \cref{eq:lowerbound-n}.

\begin{remark} \label[remark]{remark:parameter}
In case $\sigma_r^\star$ is known, the parameter choices in \Cref{def:parameter} are satisfied by choosing,
\begin{align}
    &\delta = \frac{1}{10},\\
    &\beta \asymp \frac{(\sigma_r^\star)^2}{k}, \\
    &\lambda \asymp \frac{(\sigma_r^\star)^3}{r^2 k^{7/2}}, \\
    &n \asymp \frac{\sigma^2  k^6 r^3 d \log(d/\eta)}{(\sigma_r^\star)^4}, \\
    &\epsilon \lesssim \frac{(\sigma_r^\star)^3}{r^4 k^4}, \\
    &\gamma \lesssim \frac{(\sigma^\star_r)^3}{r^4 k^{7/2}}.
\end{align}
Note that these choices are all polynomial in the problem parameters.
\end{remark}
...
\end{lemma}
\begin{proof}
From \cite[Lemma 7]{rong} and \cref{eq:hessianerror} we have the following bound,
\begin{align}
\vect(Z)^T [\nabla^2 (\cL + \lambda \cR_\beta) (U)] \vect(Z) &\le 2\| ZZ^T \|_{\mathcal{H}}^2 - 6\| UU^T - U_\star U_\star^T \|_{\mathcal{H}}^2 + 4 \langle \nabla (\cL + \lambda \cR_\beta) (U), Z \rangle \nonumber\\
&\qquad +\lambda \left[\vect(Z)^T [\nabla^2 \cR_\beta (U)] \vect (Z) - 4 \lambda \langle \nabla \cR_\beta (U), Z \rangle \right] \nonumber \\
&\qquad + 4 \sigma  \sqrt{\frac{d \log(kd/\eta)}{n}} \| Z Z^T \|_F \label{eq:presimple-finite}
\end{align}
Plugging \cref{eq:L40} into \cref{eq:presimple-finite} and using \Cref{lemma:26} and the fact that $\| \nabla (\cL + \cR_\beta) (U) \|_F \le \epsilon$,
\begin{align}
    \vect(Z)^T [\nabla^2 (\cL + \lambda \cR_\beta) (U)] \vect(Z) &\le 2 \| ZZ^T \|_{\mathcal{H}}^2 - 6 \| UU^T - U_\star U_\star^T \|_{\mathcal{H}}^2 + 4 \epsilon \| Z \|_F \nonumber\\
    &\quad + \left( 8\lambda \sqrt{\frac{2k}{\beta}} + 8 \sigma  \sqrt{\frac{kd \log(d/\eta)}{n}} \right) \| UU^T - U_\star U_\star^T \|_F. \label{eq:21112-finite}
\end{align}
By the RIP condition on the measurements, $\| ZZ^T \|_{\mathcal{H}}^2 \ge (1 - \delta) \| ZZ^T \|_F^2$ and likewise, $\| UU^T - U_\star U_\star^T \|_{\mathcal{H}}^2 \le (1 + \delta) \| UU^T - U_\star U_\star^T \|_F^2$. Therefore, assuming $\delta \le 1/10$ and simplifying as done in \cref{eq:1821,eq:1822}, we get,
\begin{align}
    \vect(Z)^T [\nabla^2 (\cL + \lambda \cR_\beta) (U)] \vect(Z) &\le - \| UU^T - U_\star U_\star^T \|_F^2 + 8 \epsilon k^{1/4} \| UU^T - U_\star U_\star^T \|_F^{1/2} \nonumber\\
    &\quad + \left( 8\lambda \sqrt{\frac{2k}{\beta}} + 8 \sigma  \sqrt{\frac{kd \log(d/\eta)}{n}} \right) \| UU^T - U_\star U_\star^T \|_F. \nonumber
\end{align}
The rest of the analysis resembles the calculations from \cref{eq:1822} to \cref{eq:1823}.
\end{proof}

\paragraph{\Cref{lemma:ZFbound,lemma:proj-similar}:} These lemmas are slightly changed in the finite sample setting. The new result is presented below.

\begin{lemma} \label[lemma]{lemma:proj-similar-finite}
Consider an $\epsilon$-approximate first order stationary point $U$ satisfying $\| U \|_{\text{op}} \le 2$. Let $V_r$ denote the top-$r$ eigenspace of $U_\star U_\star^T$. Then,
\begin{align}
    \| V_r^\perp (V_r^\perp)^T U \|_F \lesssim \epsilon k + \delta k \| UU^T - U_\star U_\star^T \|_F + \sigma  \sqrt{\frac{k^3 d \log(d/\eta)}{n}}.
\end{align}
Furthermore, for any column $i \in [k]$ such that $\| U e_i \|_2 \ge 2\sqrt{\beta}$,
    \begin{align}
        &\frac{\| V_r^\perp (V_r^\perp)^T U e_i \|_2}{\| U e_i \|_2} \le \left( \epsilon + 2 \delta \| UU^T - U_\star U_\star^T \|_F + \sigma  \sqrt{\frac{k d \log(d/\eta)}{n}} \right) \sqrt{\frac{k}{\beta^{1/2} \lambda}}. \label{eq:projangle90-finite}
    \end{align}
\end{lemma}
\begin{proof}
From the gradient computations in \Cref{lemma:L-grad-hessian-finite} and \Cref{lemma:R-grad-hessian}, with $Z = V_r^\perp (V_r^\perp)^T U$, approximate first-order stationarity of $U$ implies that,
\begin{align}
...
\begin{align}
    ...
    &\ge \langle UU^T - U_\star U_\star^T, U Z^T + ZU^T \rangle - 2 \delta \| Z \|_F \| UU^T - U_\star U_\star^T \|_F
\end{align}
where the last inequality uses the assumption that $\| U \|_{\text{op}} \le 2$. On the other hand,
\begin{align}
    \| ZU^T + UZ^T \|_F \le 2 \| U \|_{\text{op}} \| Z \|_F \le 4 \| Z \|_F.
\end{align}
...
\end{align}
Following the analysis from \cref{eq:cp1} to \cref{eq:cp2} results in the upper bound,
\begin{align} \label{eq:ZFub}
    \| Z \|_F \lesssim \epsilon k + \delta k \| UU^T - U_\star U_\star^T \|_F + \sigma  \sqrt{\frac{k^3 d \log(d/\eta)}{n}}
\end{align}
Likewise, using an analysis resembling \cref{eq:00000} to \cref{eq:00001} and plugging in the upper bound on $\| Z \|_F$ from \cref{eq:ZFub}, we get that, for any column $i \in [k]$ such that $\| U e_i \|_2 \ge 2\sqrt{\beta}$,
\begin{align}
    \frac{\| V_r^\perp (V_r^\perp)^T U e_i \|_2}{\| U e_i \|_2} \le \left( \epsilon + 2\delta \| UU^T - U_\star U_\star^T \|_F + \sigma  \sqrt{\frac{kd \log(d/\eta)}{n}} \right) \sqrt{\frac{k}{\lambda \sqrt{\beta}}}.
\end{align}
Since $\| U e_i \|_2 \ge 2\sqrt{\beta}$, rearranging the terms around and finally upper bounding $\| Z \|_F$ using \Cref{lemma:ZFbound} completes the proof.
\end{proof}

\begin{theorem}[Modified \Cref{theorem:prune-prebound}] \label{theorem:prune-prebound-finite}
Consider any $(\epsilon,\gamma)$-approximate second-order stationary point of $\cL + \lambda \cR_\beta$ denoted $U$. Suppose $\| U \|_{\text{op}} \le 2$. Construct $U_{\text{prune}}$ by following the pruning condition:
\begin{align} \label{eq:pruning_condition-finite}
    \| U e_i \|_2 \le \max \left\{ 2\sqrt{\beta}, \frac{\sqrt{\lambda \beta^{1/2} k}}{r} \right\} \implies U_{\text{prune}} e_i \gets 0
\end{align}
For a small absolute constant $c > 0$, suppose the parameter conditions in \Cref{def:parameter} are satisfied. Then,
\begin{enumerate}
    \item $U_{\text{prune}}$ has at most $r$ non-zero columns.
    \item Furthermore,
\begin{align}
    \| U_{\text{prune}} U_{\text{prune}}^T - U_\star U_\star^T \|_F \lesssim \frac{\sqrt{\lambda \beta^{1/2}}}{k r} + k \beta + \frac{k^2 \lambda \beta^{1/2}}{r}.
    \end{align}
\end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{align} \label{eq:247}
    \left| \theta_{ij}^{\text{proj}} - \frac{\pi}{2} \right| \lesssim \sqrt{\frac{\epsilon+\gamma}{\lambda}} + \left( \epsilon + 2 \delta \| UU^T - U_\star U_\star^T \|_F + \sigma  \sqrt{\frac{kd \log(d/\eta)}{n}} \right)\sqrt{\frac{k}{\beta^{1/2} \lambda}}
\end{align}
Assume for a sufficiently small constant $c_2 > 0$, $\epsilon + \gamma \le \lambda/r^2$. Then the first term on the RHS is upper bounded by $\frac{c_1}{2r}$ for a sufficiently small $c_1 \le 1/3$. On the other hand, by the assumptions on the parameters in \cref{eq:parameter}, it is easily verified that,
\begin{align}
    \epsilon + 2 \delta \| UU^T - U_\star U_\star^T \|_F + \sigma \sqrt{\frac{kd \log (d/\eta)}{n}} \le \frac{c_1}{2r} \sqrt{\frac{\lambda \beta^{1/2}}{k}} \label{eq:009111}
\end{align}
And overall, the RHS of \cref{eq:247} is upper bounded by $c_1/r$ for a sufficiently small absolute constant $0 \le c_1 \le 1/3$. The proof until \cref{eq:210} carries over unchanged, and we have that at most $k$ columns of $U$ can have a non-zero projection onto $V_r$. For any remaining column $i \in [d_1]$ of $U$,
\begin{align}
    \| U e_i \|_2 &= \| V_r^\perp (V_r^\perp)^T U e_i \|_2 \\
    &\le \| V_r^\perp (V_r^\perp)^T U \|_F \\
    &\lesssim \epsilon k + \delta k \| UU^T - U_\star U_\star^T \|_F + \sigma  \sqrt{\frac{k^3d \log(d/\eta)}{n}},\\
    &\lesssim \frac{\sqrt{\lambda \beta^{1/2} k}}{r},
\end{align}
where $(i)$ is by \cref{lemma:proj-similar-finite} and $(ii)$ is by \cref{eq:009111}. Since these columns satisfy the pruning criterion in \cref{eq:pruning_condition-finite}, this in fact implies that $U_{\text{prune}}$ has at most $r$ non-zero columns. Finally, by following the analysis from \cref{eq:komega1} to \cref{eq:komega2}, and with the pruning threshold $\omega = \max \left\{ 2 \sqrt{\beta}, \frac{\sqrt{\lambda \beta^{1/2} k}}{r} \right\}$, we have that,
\begin{align}
    \| U_{\text{prune}} U_{\text{prune}}^T - U_\star U_\star^T \|_F &\le \| UU^T - U_\star U_\star^T \|_F + k \omega^2 \\
    &\lesssim \frac{\sqrt{\lambda \beta^{1/2}}}{k r} + k \beta + \frac{k^2 \lambda \beta^{1/2}}{r}
    \end{align}
\end{proof}
Note that in order for the measurements to satisfy $(k,\delta)$-RIP for $\delta = \frac{1}{10}$, it suffices to have $n \gtrsim k d \log(d)$ Gaussian measurements \citep{RECHT}. This is satisfied by the choice of $n$ in our theorem. 

\begin{theorem}[Main result in the finite sample setting] \label[theorem]{theorem:main-finite}
Suppose the conditions on the parameters $\epsilon,\gamma,\lambda$ and $\beta$ in \cref{def:parameter} are satisfied. Construct $U_{\text{prune}}$ using \Cref{alg:main}. Then,
\begin{enumerate}
...
\end{enumerate}
In other words, given $n \gtrsim \sigma^2  \frac{d \log(d/\eta) \cdot \poly{r,k}}{(\sigma^\star_r)^4}$ samples, \Cref{alg:main} results in a solution having exactly $r$ non-zero columns, and also serves as a spectral initialization.
\end{theorem}
...
In this section we prove \Cref{theorem:lipgradhess}. We first prove the gradient Lipschitzness condition by showing that the Hessian of $f_{\text{pop}}$ is bounded on the domain $\{ U : \| U \|_{\text{op}} \le 3 \}$. Below we provide an analysis for the population setting, starting with showing gradient Lipschitzness. Observe that,
\begin{align}
...
\end{align}
where $(i)$ repeatedly uses the bound $\| AB \|_F \le \| A \|_{\text{op}} \| B \|_F$ and the last inequality follows from the fact that $\| U \|_{\text{pop}} \le 3$ and $\| U_\star \|_{\text{pop}} = 1$. On the other hand, by the gradient computations in \Cref{lemma:R-grad-hessian},
\begin{align}
...
Next we prove Hessian Lipschitzness of $f_{\text{pop}}$. By \cite[Theorem 23]{hessianbound}, the Hessian of $\cL_{\text{pop}}$ satisfies the condition,
\begin{align}
    \| \nabla^2 \cL_{\text{pop}} (U) - \nabla^2 \cL_{\text{pop}} (V) \|_{\text{op}} \lesssim \| U - V \|_F. \label{eq:Lpophess}
\end{align}
Although the analysis in \cite{hessianbound} is provided for the exactly specified case ($k=r$), it carries over unchanged to the overparameterized case as well. On the other hand, for any $Z, Z_U \in \mathbb{R}^{d \times k}$ such that $\| Z \|_F, \| Z_U \|_F \le 1$,
\begin{align}
...
\begin{align}
    ...
    &\le \sum_{i=1}^k H(U)_{ii} \langle Z e_i, U e_i \rangle^2 \label{eq:004294729}
\end{align}
where $H(U)_{ii} = \frac{3(\| U e_i \|_2^2 + 6 \beta) \| Ue_i \|_2}{(\| U e_i \|_2^2 + \beta)^{7/2}}$. Note that for any $c \ge 0$, the optimization problem which takes the form $\max_{Z e_i : \| Z e_i \|_2^2 \le c} \langle Ze_i, U e_i \rangle^2$ is maximized when $Z e_i \propto U e_i$. Therefore, one may replace $Z e_i$ by $x_i \cdot U e_i / \| U e_i \|_2$ for some $x_i \in \mathbb{R}$ and bound \cref{eq:004294729} as,
\begin{align}
    \lim_{t \to 0} \frac{\left( G(U+tZ_U)_{ii} - G(U)_{ii} \right) \langle Z e_i, U e_i \rangle^2}{t}
    &\le \max_{\sum_{i=1}^k x_i^2 \le 1} \sum_{i=1}^k H(U)_{ii} x_i^2 \| U e_i \|_2^2\\
    &= \max_{i \in [k]} H(U)_{ii} \| U e_i \|_2^2\\
    &\lesssim \frac{1}{\beta}, \label{eq:last2}
\end{align}
where the last inequality uses the fact that $\min_{x \ge 0} \frac{3\sqrt{x^3}(x+6\beta)}{(x + \beta)^{7/2}} \lesssim \frac{1}{\beta}$. The second term on the RHS of \cref{eq:limt} is,
\begin{align}
...
Consider $\cL_{\text{emp}} (\cdot)$ as defined in \cref{eq:empirical_loss}. Then,
\begin{enumerate}
    \item The gradient of $\cL_{\text{emp}}$ satisfies, $\langle \nabla \cL_{\text{emp}} (U), Z \rangle = 2 \langle UU^T - U_\star U_\star^T, U Z^T + Z U^T \rangle_{\mathcal{H}}$.
    \item For any $Z \in \mathbb{R}^{d \times d}$,
    \begin{align}
        \vect (Z)^T [ \nabla^2 \cL_{\text{emp}} (U)] \vect (Z) = 4 \langle UU^T - U_\star U_\star^T, ZZ^T \rangle_{\mathcal{H}} + 2\| U Z^T + Z U^T \|_{\mathcal{H}}^2.
    \end{align}
\end{enumerate}
\end{lemma}
\begin{proof}
...
\subsection{Regularization $\cR_\beta$}
...
\begin{align} \label{eq:DU-def}
    D (U) &= \textsf{diag} \left( \left\{ \frac{(\| U e_i \|_2^2 + 2 \beta)}{(\| U e_i\|_2^2 + \beta)^{3/2}} : i \in [m] \right\} \right), \text{ and}, \\
    G (U) &= \textsf{diag} \left( \left\{ \frac{(\| U e_i \|_2^2 + 4 \beta)}{(\| U e_i\|_2^2 + \beta)^{5/2}} : i \in [m] \right\} \right).
\end{align}
