...
Second, the per-period price the principal charges in any stationary equilibrium causes the agent's participation to bind given the (optimal) signal structure. In general, this per-period price is a fraction of the expected total profit the principal derives. Consequently, the principal extracts the entire surplus in equilibrium through dribs and drabs.
...
We extend our analysis to the case in which the agent observes a public signal of the quality of each sampled good, inducing potentially heterogeneous interim beliefs. We show that the agent again receives his autarky payoff in equilibrium, which now may be higher due to the presence of public information. In contrast, the principal may not be able to generate and extract the socially efficient surplus. Nonetheless, when public signals do not render the agent too optimistic, full extraction continues to hold.
...
Because $\pi_t$ impacts the agent's payoffs only through a good's expected quality, it suffices for our purposes to only consider the distribution over posterior means that is induced by the signal structure. From \cite{bla53} and \cite{rot70}, each signal structure $\pi$ induces a distribution over posterior means $G^\pi:\Theta\to[0,1]$ that is a mean-preserving contraction of $F$, i.e., %\bph{We could index $G$ in this sentence and in the below display equation with $\pi$, $G^\pi$, to make clearer the dependence. I do not think we need to do it elsewhere}
\[
\int^{\bar \theta}_x G^\pi(m)dm\geq \int^{\bar \theta}_x F(m)dm
\]
for all $x\in\Theta$ with equality at $x=\underline\theta$. For example, a fully informative signal structure induces the distribution $F$ while an uninformative signal structure induces the degenerate distribution $G^\varnothing$ given by
\[
G^\varnothing(m)=\left\{\begin{array}{ccc}
     ...
     \end{array}\right..
\]
Conversely, each distribution $G$ that is a  mean-preserving contraction of $F$ can be induced by some signal structure $\pi$ where $G^\pi=G$. Thus, without loss of generality, we assume that for each period $t$, the principal can propose a contract of the form $\langle p_t, G_t \rangle \in \mathbb{R}_+\times \mathcal{G}(F)$, where $\mathcal{G}(F)$ is the set of mean-preserving contractions of $F$. 
\subsection{Signal structures as convex functions}

Given some posterior-mean distribution $G\in\mathcal{G}(F)$, let $c_G:\Theta\to \mathbb{R}_+$ be the function given by
\begin{align*}
 c_G(x)&\triangleq\int^{\bar\theta}_{x}(1-G(m))dm.
\end{align*}
To understand what $c_G(x)$ represents, suppose the agent has a guaranteed payoff of $x\in\Theta$ from some ``outside option." The agent's expected surplus over the outside option from a random draw of distribution $G$ is then given by
\begin{align*}
...
Intuitively, if the payoff from the outside option is $\underline \theta$, then a random draw from $G$ yields a higher payoff than the outside option with probability one. Thus, $c_G(\underline\theta)=m_\varnothing-\underline\theta$. In contrast, if the payoff from the outside option is $\bar \theta$, then there is zero probability that a random draw from $G$ yields a higher payoff than the outside option. Thus, $c_G(\bar\theta)=0$. Additionally, as the payoff of the outside option increases, the probability that $G$ yields a higher payoff than the outside option decreases. Even when $G$ yields a higher payoff than the outside option, the surplus over the outside option decreases. These two properties combined imply that $c_G(x)$ is a decreasing and convex function of $x$. Convexity further implies that $c_G(x)$ is left- and right-differentiable for all $x\in\Theta$. We focus on the right derivative for our purposes.\footnote{Both the left and right derivatives of $c_G$ are well-defined as $G$ is c\`adl\`ag. The left derivative is given by $\partial_-c_G(x)=G(x_-)-1$. $c_G$ is differentiable everywhere but at the mass points of $G$.}

Notice that if $G'$ is mean-preserving contraction of $G''$, then $c_{G'}\leq c_{G''}$ pointwise. Hence, for all $G\in\mathcal{G}(F)$,  $c_{G^\varnothing}\leq c_G\leq c_F$ pointwise. Let $\mathcal{C}(F)$ be the set of continuous, weakly decreasing, and convex functions $c:\Theta\to \mathbb{R}_+$ such that $c_{G^\varnothing}\leq c\leq c_F$ pointwise. Then for all $G\in\mathcal{G}(F)$, $c_G\in\mathcal{C}(F)$. Additionally, for each $c\in\mathcal{C}(F)$, there exists a distribution $G\in \mathcal{G}(F)$ such that $c=c_G$  (Proposition 1, \cite{gen16}). Therefore, we can represent a signal structure not only as a distribution in $\mathcal{G}(F)$ but also as a continuous, weakly decreasing, and convex function in $\mathcal{C}(F)$.

\subsection{Agent's search problem}\label{autarky_section}
Before we analyze the principal-agent game, we first consider a hypothetical setting in which $(a)$ there is no principal, and $(b)$ the agent  observes the realization of some distribution $G\in \mathcal{G}(F)$ for free in each period. In this section, we study this stationary setting, which is a single-agent search problem akin to \cite{wol86}. This setting is instructive in characterizing when there is scope in our main model for contracting between the principal and the agent.  
...
\noindent where the final equality follows because \hyperref[ass:1]{\Cref{ass:1}} implies $\delta u(G)\in(\underline \theta, \bar \theta)$ for any $G\in \mathcal{G}(F)$, i.e. that $c_G(\delta u)$ is well defined.\footnote{Recall that we have defined the domain of the function $c_G$ to be $\Theta$.}
...
In this section, we characterize stationary equilibrium outcomes. To that end, we first make two observations that simplify our task.

First, since all players are risk neutral and the space of contracts is convex, the principal does not gain from randomizing over contracts. To guarantee equilibrium existence, we also assume that whenever the agent is indifferent between any two actions on the equilibrium path, he chooses the one that maximizes the principal's payoff. Therefore, it suffices for our purposes to only consider pure strategies. Second, since the agent rejecting a contract is payoff-equivalent to the principal proposing a contract  $\langle 0, G^\varnothing\rangle$ and the agent accepting, we can restrict attention to contracts that induce the agent to accept.
...
G^*(m)=\left\{\begin{array}{ccl}
     ...
     \end{array}\right..
\]
We call this distribution binary as it has mass at only two values, namely $\mathbb{E}_F[\theta|\theta\leq  \bar r]$ and $\mathbb{E}_F[\theta|\theta>  \bar r]$. As this distribution is useful in characterizing stationary equilibria, we state some of its properties in the following lemma. 
...
\end{enumerate}
\end{lemma}\
Graphically, the findings of \autoref{lemma:2} are summarized in \autoref{fig:1.optimal_pass_fail}.
...
Our main result, \autoref{prop:1}, establishes not only the existence of a stationary equilibrium but also that all stationary equilibria are payoff equivalent. In any stationary equilibrium, the socially efficient surplus $\bar u$ is generated but the agent's payoff is the same as his autarky payoff $\underline u$. The remaining surplus $\bar u-\underline u$ is extracted by the principal. 

Given the agent earns his autarky payoff, he would be willing to stop searching whenever the expected quality of a sampled good exceeds the autarky reservation value $\underline r$. However, in equilibrium, the agent does not stop searching until he samples a good whose quality exceeds the efficient threshold $\bar r$.\footnote{Under \hyperref[ass:1]{Assumption 1}, $\underline \theta<\underline r<\bar r<\bar\theta$.} In other words, the agent searches as if he observes a fully informative signal structure for free, even though his payoff is as if he observes no information.

In order to achieve this seemingly contradictory behavior in the agent, the principal proposes a contract in which the signal structure pools sufficiently high quality goods\textemdash for which the agent would be willing to stop his search\textemdash with sufficiently low quality goods\textemdash for which the agent would rather continue searching. One example is the pass/fail signal structure that generates the distribution  $G^*$ in \eqref{eq:3}. This signal structure pools and labels all goods of quality less than $\bar r$ as a ``fail" and all other goods as a ``pass."  Whenever, the agent sees a ``fail," he is persuaded to continue searching because he is unable to distinguish between a good whose quality lies in the interval $[\underline r, \bar r]$ from a good whose quality is lower than $\underline r$. 

The principal then sets the price $p$ of such a pooling signal structure to equal the value of information generated in a single period. Interestingly, $p$ is a (possibly small) fraction of the maximal net surplus $\bar u-\underline u$ that the principal ultimately extracts.\footnote{As we show in point $(iii)$ of \autoref{prop:1}, the per-period price is strictly below $\bar u-\underline u$ for any $\delta>0$. Additionally, as $\delta\to 1$, the per-period price converges to zero while the duration of search converges to infinity. }

However, the principal is able to persuade the agent to search for longer than he otherwise would like, allowing the principal to extract the maximal net surplus through dribs and drabs.
...
Since $\langle p, G\rangle$ is a solution to \eqref{eq:4}, $G$ is a solution to \eqref{eq:44}. Additionally, the agent must  be both stopping his search and continuing his search in each period with a positive probability, i.e., $G(\underline r)\in (0,1)$. Otherwise, the distribution $G$ would induce the agent to either always stop or to never stop his search regardless of the realized posterior mean. There would be no value of information associated with such a distribution and the agent would not be willing to buy it from the principal for a positive price. Thus, the principal's profit from inducing a distribution with $G(\underline r)=0$ or $G(\underline r)=1$ would be zero, which contradicts our earlier statement that $V>0$.\footnote{Formally, if $G(\underline r)=0$, then 
\begin{align*}
    c_G(\underline r)&=c_G(\underline \theta)-\int_{\underline \theta}^{\underline r}1-G(m)dm \leq c_{G^\varnothing}(\underline \theta)-\int_{\underline \theta}^{\underline r}1-G_{\varnothing}(m)dm =c_{G^\varnothing}(\underline r),
\end{align*}
...
\[
c_G(\underline r)=\int^{\bar\theta}_{\underline r}1-G(m)dm\leq \int^{\bar\theta}_{\underline r}1-G^\varnothing(m)dm=c_{G^\varnothing}(\underline r), 
\]
where the inequality follows from the fact that $0=1-G(m)\leq 1-G_{\varnothing}(m)$ for all $m\geq \underline r$. 

However, since $G^\varnothing$ is a mean-preserving contraction of $G$, we have $c_G\geq c_{G^\varnothing}$ pointwise, which implies that $c_G(\underline r)=c_{G^\varnothing}(\underline r)$. Consequently, the solution to \eqref{eq:44} would be  $V=0$, which establishes a contradiction. } 
...
\label{remark:2}
    \begin{enumerate}[$(a)$]
        ...
        \item $G^\dagger$ is a mean-preserving contraction of $G$. Hence,  $c_{G^\dagger}(x)\leq c_{G}(x)$ for all $x\in\Theta$.\footnote{Note that $G^\dagger$ is derived as only a function of $G$ and thus cannot be more informative. It is also possible to directly verify $c_{G^\dagger}\leq c_G$ pointwise.} Additionally, 
        \begin{align*}
    c_G(\underline r)&=\int_{\bar r}^{\bar \theta}(m-\underline r)dG(m)\\[6pt]
...
\]
and $G^\dagger$ is also a solution to \eqref{eq:44}. In fact, for any binary distribution $\widehat G\in\mathcal{G}(F)$ with $\widehat G(\underline r)\in (0,1)$, we have 
\begin{align*}
    ...
    &= \frac{c_{ \widehat G}(r(\widehat G))-c_{G^\varnothing}(\underline r)+\displaystyle\int_{ \underline r}^{r(\widehat G)}(1-\widehat G(m))dm}{1-\delta  \widehat G( \underline r)}\\[9pt]
    &=\frac{\left(r(\widehat G)-\underline r\right)\left(\frac{1-\delta}{\delta}\right)+\Big(r(\widehat G)-\underline r\Big)\big(1-\widehat G(\underline r)\big)}{1-\delta  \widehat G(\underline r)}\\[9pt]
    &=\frac{r(\widehat G)-\underline r}{\delta},
\end{align*}
where the first inequality follows because $G^\dagger$ solves \eqref{eq:44} with equality, the second equality follows from breaking up the limits of the integral in $c_{\widehat G}(\underline r)$, the first term in the numerator of the third equality follows from $r(G')$ being the unique reservation value that solves \eqref{eq:2} computed for $G'=\widehat G$ and $G'=G^\varnothing$, and the second term in the numerator of the third equality follows by the fact that a binary distribution $\widehat G$ with $\widehat G(\underline r)\in (0,1)$ must be constant over the interval $[\underline r, r(\widehat G)]$.
...
V=\frac{r(G^*)-\underline r}{\delta}=\frac{\bar  r-\underline r}{\delta}=\bar u-\underline u,
 ...
 \]
 where the first equality follows from \autoref{remark:2} and the last equality follows from point $(iv)$ of \autoref{lemma:2}. This establishes point $(iv)$ of the theorem. Similarly, by \autoref{remark:2}, $c_{G^*}(x)=c_{G^\dagger}(x)\leq c_G(x)$ for all $x\in\Theta$ with equality at $x=\underline r$, which establishes point $(v)$ of the theorem. Finally, as \eqref{eq:pc} binds in a stationary equilibrium, we can compute the price of a contract as 
 \begin{align*}
     p&=c_{G^*}(\underline r)-c_{G^\varnothing}(\underline r)\\[6pt]
     &= c_{G^*}(\bar r)-c_{G^\varnothing}(\underline r)+\int^{\bar r}_{\underline r}1-G^*(m)dm\\[6pt]
     &=\left(\frac{1-\delta}{\delta}\right)(\bar r-\underline r)+(\bar r-\underline r)(1-F(\bar r))\\[6pt]
     &=(\bar u-\underline u)(1-\delta F(\bar r)),
     \end{align*}
     where the first equality follows from $(v)$ of \autoref{prop:1}, the second equality follows by breaking up the integral in $c_{G^*}(\underline r)$, the first term in the third equality follows from $r(G')$ being the unique reservation value that solves \eqref{eq:2} computed for $G'=G^*$ and $G'=G^\varnothing$, and the second term in the third equality follows by point $(iv)$ of \autoref{lemma:2}. This establishes point $(iii)$ of the theorem. \\

 \noindent ($\Leftarrow$) Suppose the principal proposes a contract $\langle p, G\rangle$ that satisfies $(iii)$-$(v)$  of \autoref{prop:1} in every period following any history of events. Additionally, suppose the agent and the principal anticipate to get continuation values $U=\underline u$ and $V=\bar u-\underline u$ respectively following any history.
...
\begin{align*}
     ...
     &= \underline r+c_{G^*}(\bar r)-(\bar u-\underline u)\big(1-\delta F(\bar r)\big)+\int^{\bar r}_{\underline r}1-G^*(m)dm\\[6pt]
    &= \underline r+\left(\frac{1-\delta}{\delta}\right)\bar r-(\bar u-\underline u)\big(1-\delta F(\bar r)\big)+(\bar r-\underline r)(1-F(\bar r))\\[6pt]
...
From the statement of \autoref{prop:1}, it is clear that the price and the continuation values in a stationary equilibrium are uniquely pinned down. However, there are  an uncountable number of posterior-mean distributions that can sustain a stationary equilibrium. For example, consider a lower-censorship signal structure that  reveals a good's quality if it exceeds $\bar r$ and pools all goods whose quality lies below $\bar r$ by labeling them as ``fail." This signal structure induces the distribution $G^{LC}$ given by 
\[
G^{LC}(m)=\left\{\begin{array}{ccl}
   0 &\mbox{if} &~~~ m<\mathbb{E}_F[\theta|\theta\leq \bar   r]  \\
     ...
     \end{array}\right. .
\]
Note that this lower-censorship signal structure is Blackwell more informative than the pass/fail signal structure that induces $G^*$. Hence, $G^*$ is a mean-preserving contraction of $G^{LC}$ as can be seen in \autoref{fig:optdist}, and $G^{LC}$ is also an equilibrium distribution as it satisfies $(iv)$ and $(v)$ of \autoref{prop:1}.
...
Yet, despite the multitude of equilibrium distributions, all of them have the same distribution on the relevant interval of stopping thresholds $[\underline r,  \bar r]$. Thus, all equilibrium signal structures induce the same search behavior in the agent. 
...
Consider first the case of autarky in which the agent could only observe the public signal $\xi$. His induced posterior-mean distribution would be $G^\xi\in\mathcal{G}(F)$ and his expected payoff in the search market would be $u(G^\xi)$, which is the solution to \eqref{eq:1'} when $G=G^\xi$. Note that $u(G^\xi)\geq \underline u$, i.e., the agent's autarky payoff improves in the current setting since he has access to a free signal.

Next, consider the case in which agent could observe the public signal followed by a second signal. Given a public signal realization $z\in Z$, the agent updates his belief from his prior $F$ to an interim belief $F_{z}^{\xi}$. When the agent further updates his belief based on the second signal, $F^\xi_z$ serves as his new ``prior."  Thus, the set of posterior-mean distributions that can be induced through a second signal is the set of mean-preserving contraction of $F^\xi_z$, i.e., $\mathcal{G}(F^\xi_z)$. For example, a fully informative second signal would induce $F^{\xi}_z$ itself, whereas an uninformative  second signal would induce the degenerate distribution
\[
...
Therefore, following a public signal realization $z\in Z$, the principal can propose any contract of the form $\langle p, G\rangle\in\mathbb{R}_+\times \mathcal{G}(F^\xi_z)$. Our goal is to characterize stationary equilibrium outcomes. However, we must first update the conditions  for a stationary equilibrium given in \autoref{def:1} so as to account for the agent's ability to learn from the public signal.  

\begin{definition}
    \label{def:2}
    \normalfont Given a public signal structure $\xi$, a stationary equilibrium  is defined by a pair of agent-principal continuation values $(U, V)\in \mathbb{R}^2$ and a family of  contracts $\{\langle p_z,G_z\rangle\}_{z\in Z}$ with $\langle p_z, G_z\rangle\in\mathbb{R}_+\times \mathcal{G}(F^\xi_z)$ for each $z\in Z$ such that in each period and after every history:
    \begin{enumerate}
...
In order to state our main result of this section, we must first introduce some notation. Given a public signal realization $z\in Z$, we denote the lower and upper bounds on the support of $F^\xi_z$ by $\underline \theta_z=\inf \hspace*{.2em} \supp(F^\xi_z)$ and $\bar \theta_z=\sup \hspace*{.3em}\supp(F^\xi_z)$ respectively, and let
\[
Z_A=\{z\in Z:r^\xi \hspace*{.1em} < \hspace*{.1em}\underline \theta_z\}.
\]
In the case that $r^\xi\in [\underline\theta_z, \bar\theta_z]$, let 
$\bar x_z=\sup\{x\in [\underline\theta_z, \bar\theta_z]:\mathbb{E}_{F_z^\xi}[\theta|\theta\leq x]\leq r^\xi\}$, and for some $k\geq 0$, let 
\[
Z_B(k)=\{z\in Z:r^\xi\in [\underline \theta_z ,\bar\theta_z]\hspace*{.1em} \wedge \hspace*{.1em}\bar x_z\hspace*{.1em} < \hspace*{.1em}\bar r-k\}.
\]
Note that $Z_A$ and $Z_B(k)$ could be empty. Finally, we define  the function $\Phi:\mathbb{R}_+\to \mathbb{R}$ by
\small{\begin{align*}
        \label{eq:5}
        \tag{5}
\Phi(k)=-\bigintsss_{\bar r-k}^{\bar r}\hspace*{-.75em} 1-F(m)dm+\bigintsss\limits_{Z_A}\bigintsss_{r^\xi}^{\bar r-k}\hspace*{-
        1.5em}F^\xi_z(m)dm \hspace*{.2em}\xi(dz)+\bigintsss\limits_{Z_B(k)}\bigintsss_{\bar x_z}^{\bar r -k}\hspace*{-1.5em}F^\xi_z(m)-F^\xi_z(\bar x_z)dm \hspace*{.2em}\xi(dz).
        \end{align*}} %%
       \normalsize 
We provide a detailed discussion of  $Z_A$, $Z_B(\cdot)$, and $\Phi(\cdot)$ following our main result of this section.

\begin{theorem}
\label{prop:2}
    For any public signal structure $\xi:\Theta\to \Delta(Z)$, a stationary equilibrium exists. Furthermore, in any stationary equilibrium, 
    \begin{enumerate}[$(i)$]
        \item $U=u(G^\xi)$ and
...
To see why, suppose the principal seeks to induce a  stopping threshold $\bar r - k$ for some $k\geq 0$ in a stationary equilibrium. The change in total surplus that can be extracted by the principal from inducing $\bar r-k$ instead $\bar r $ is captured by the first term in \eqref{eq:5}. However, given the agent's actual reservation value in a stationary equilibrium is $\delta u(G^\xi)=r^\xi$, the principal may not be able to induce the  desired stopping threshold $\bar r-k$. In particular, there are two cases when the principal fails to induce $\bar r-k$ after some realization $z\in Z$ of the public signal. First, if $r^\xi<\underline \theta_z$, then the lowest quality the agent expects to receive when he observes $z$ is above his autarky reservation value. Hence, the agent is too optimistic and he  terminates his search regardless of the contract offered by the principal. This occurs when $z\in Z_A$ and the efficiency loss in this case is captured by 
 \[
 \int_{Z_A}\int_{r^\xi}^{\bar r-k}F^\xi_z(m)-F^\xi_z(r^\xi)dm,
 \]
...
Consider a binary distribution $\widehat G\in\mathcal{G}(F)$ with $r(\widehat G)=\bar r$. Let $\supp(\widehat G)=\{m_1, m_2\}$ be the support of $\widehat G$ with $m_1<m_\varnothing<m_2$. Our goal is to show that $\widehat G=G^*$.
...
\end{array} \right..
\]
Notice that $c_{\widehat G}(x)=c_{G^\varnothing}(x)$ for all $x\in [\underline\theta, m_1]\cup [m_2, \bar \theta]$ by construction. Suppose to the contrary that $\bar r\in[\underline\theta, m_1]\cup [m_2, \bar \theta]$. We then would have 
\[
...
Suppose $r^\xi\in [\underline \theta_z, \bar \theta_z]$. Then the principal's value function can be attained by a pass/fail signal structure such that for some $x_z\in [r^\xi, \bar\theta_z]$,  all goods of quality $\theta>x_z$ ``pass" while goods of quality $\theta\leq x_z$ ``fail."
\end{lemma}
\begin{proof}[Proof of  \autoref{lemma_theorem_2_proof}]
As  $F_z^\xi$ does not have any atoms by assumption, there exists an  $x^*\in [r^\xi, \bar\theta_z]$ such that $G_z(r^\xi)=F_z^\xi(x^*)$. We proceed by showing that there exists a pass/fail signal structure with $x_z=x^*$ that attains the principal's value function $\psi_z(V).$
...
\end{align*}
Thus, $G^\varnothing_z$ is also a solution to \eqref{eq:6}, which can be induced by a pass/fail signal structure that fails all goods, i.e., $x_z=x^*=\bar\theta_z$.
...
\begin{align*}
    ...
    \label{eq:7}
        \tag{7}
    &=\big(1-F_z^\xi(x^*)\big)\left(\mathbb{E}_{F_z^\xi}[\theta|\theta>x^*]-r^\xi\right),
\end{align*}
...
Consider a pass/fail signal structure with $x_z=x^*$. This signal structure would induce the distribution $G_{x^*}$ given by 
\[
...
Thus far, we have shown that following any realization $z\in Z$ such that $r^\xi\in [\underline \theta_z, \bar \theta_z]$, there exists a pass/fail signal structure such that the agent will continue his search if and only if the good ``fails." In other words, 
...
\small{\begin{align*}
 V=&\int_Z \psi_z(V)\xi(dz)\\[6pt]
    =& \delta V \xi(Z_2)+\int\limits_{Z_3(V)}\left[c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)+\delta V\right] \xi(dz) + \int\limits_{Z_4(V)}\delta VF_z^\xi(\bar x_z) \xi(dz)\\[6pt]
    =&\int\limits_{Z}\left[c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)+\delta V \right]\xi(dz) -\int\limits_{Z_1}\left[c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)+\delta V \right]\xi(dz)\\[6pt]
    -&\int\limits_{Z_2}\left[c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)\right] \xi(dz)- \hspace*{-.75em}\int\limits_{Z_4(V)}\hspace*{-.5em}\left[c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)+\big(1-F_z^\xi(\bar x_z)\big)\delta V \right]\xi(dz).
       \label{eq:9}
    \tag{9}
\end{align*}}
\normalsize
...
\begin{align*}
    ...
    &=c_{F}(\bar r)-c_{G^\xi}(r^\xi)+\delta V+\int\limits_{\delta V+r^\xi}^{\bar r}\hspace*{-.5em}1-F(m)dm\\[6pt]
    &=(1-\delta)\big(\bar u-u(G^\xi)\big)+\delta V +\int\limits_{\delta V+r^\xi}^{\bar r}\hspace*{-.5em}1-F(m)dm
\end{align*}
...
\begin{align*}
    c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)+\delta V &=c_{F^\xi_z}(r^\xi)-c_{G^\varnothing_z}(r^\xi)+\delta V-\hspace*{-.5em}\int\limits^{\delta V+r^\xi}_{r^\xi}\hspace*{-.5em}1-F_z^\xi(m)dm\\[6pt]
    &=\int\limits^{\delta V+r^\xi}_{r^\xi}\hspace*{-.5em}F_z^\xi(m)dm.
\end{align*}
...
\small{\begin{align*}
    c_{F^\xi_z}(\delta V+r^\xi)-c_{G^\varnothing_z}(r^\xi)+\big(1-F_z^\xi(\bar x_z)\big)\delta V &=c_{F^\xi_z}(\bar x_z)-c_{G^\varnothing_z}(r^\xi)-\hspace*{-.5em}\int\limits^{\delta V+r^\xi}_{\bar x_z}\hspace*{-.5em}1-F_z^\xi(m)dm+\big(1-F_z^\xi(\bar x_z)\big)\delta V\\[6pt]
    &=\int\limits^{\delta V+r^\xi}_{\bar x_z}\left[F_z^\xi(m)-F_z^\xi(\bar x_z)\right]dm,
\end{align*}}%
\normalsize
...
\small{\begin{align*}
    ...
    &-\frac{1}{1-\delta}\left(\int\limits_{Z_1}\int^{\delta V+r^\xi}_{r^\xi}F_z^\xi(m)dm \xi(dz)+\int\limits_{Z_4(V)}\int^{\delta V+r^\xi}_{\bar x_z}\left[F_z^\xi(m)-F_z^\xi(\bar x_z)\right]dm \xi(dz)\right)\\[6pt]
...
\[
\Phi(0)=\bigintsss\limits_{Z_A}\bigintsss_{r^\xi}^{\bar r}F^\xi_z(m)dm \hspace*{.2em}\xi(dz)+\bigintsss\limits_{Z_B(0)}\bigintsss_{\bar x_z}^{\bar r}\left[F^\xi_z(m)-F^\xi_z(\bar x_z)\right]dm \hspace*{.2em}\xi(dz)\geq 0.
\]
...
\[
\Phi(\bar r-r^\xi)=-\bigintsss_{r^\xi}^{\bar r}\hspace*{-.75em} 1-F(m)dm\leq 0
\] 
...
We have found a unique pair $(U,V)$ and constructed a family of pass/fail signal structures that attains these values for the agent and principal, respectively, such that \autoref{def:2} is satisfied. Therefore, an equilibrium exists, completing the proof.
\end{proof}
