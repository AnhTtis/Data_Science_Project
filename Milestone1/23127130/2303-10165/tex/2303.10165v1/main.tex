...
\title{\huge Optimal Horizon-Free Reward-Free Exploration for Linear Mixture MDPs}
\date{}
\author
{
    Junkai Zhang\thanks{Department of Computer Science, University of California, Los Angeles, CA 90095, USA; e-mail: {\tt zhang@cs.ucla.edu}} 
    ~~and~~
...
\hline
\rowcolor{white}
Setting & Algorithm & Rewards Scale & \makecell[c]{Time \\ Homo.} & Sample Complexity \\
\hline
\rowcolor{white}
...
\begin{align*}
& Q_h^\pi(s,a) = \EE \bigg[ \sum_{h'=h}^H r(s_{h'},a_{h'}) | s_h = s, a_h = a,
  s_{h'}\sim\PP(\cdot|s_{h'-1},a_{h'-1}),a_{h'} = \pi_{h'}(s_{h'})\bigg], \\
...
\begin{align*}
\left[\PP V\right](s,a) = \EE_{s'\sim\PP(\cdot|s,a)}V\left(s'\right),\quad
...
\begin{align*}
Q_h^\pi (s,a) = r(s,a) + \left[\PP V_{h+1}^\pi\right](s,a), \quad Q_h^* (s,a) = r(s,a) + \left[\PP V_{h+1}^*\right](s,a)
\end{align*}
...
\STATE Set $\big\{\tilde \sigma_{k, h, m}\big\} \leftarrow \text{HOME}_{\text{Alg.~\ref{alg:home}}}\Big(\big\{\tilde \bphi_{k, h, m}, \tilde \btheta_{k, m}, \tilde\bSigma_{k, h, m}, \dot{\tilde\bSigma}_{k, m}\big\}, \beta_k, \alpha, \gamma\Big)$
...
\begin{align}
\arg\min_{\btheta}\bigg( \lambda \|\btheta\|_2^2 + \sum_{j=1}^{k-1}\sum_{h=1}^{H}\big( \bphi_{j,h,0}^\top \btheta - V_{j,h}(s^j_{h+1})\big)^2/\bar{\sigma}_{j,h,0}^2\bigg),\label{eq:reg}
\end{align}
...
\begin{align}
& V_1^*(s_1;r) - V_1(s_1;\btheta^*, \hat\pi_r, r)   \le 4 \hat V_{K,1}\left(s_1\right)   
\end{align}
