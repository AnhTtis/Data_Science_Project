...
\subsection{Global Position Alignment}
\label{subsec:gpr}
...
\subsection{Local Pose Augmentation}
\label{subsec:ra}
...
\begin{figure*}
    ...
    \caption{The pose transformations applied on 3D pose from source dataset~(column 1). Bone angle~(column 2), bone length~(column 3), and rotation~(column 4) are root-relative transformations given by local pose augmentation (LPA). Translation~(column 5) is given by global position alignment~(GPA). Target~(column 6) is the sampled 2D pose from target dataset used for GPA.}
    \label{fig:pipeline}
\end{figure*}
