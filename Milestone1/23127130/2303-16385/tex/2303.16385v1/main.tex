...
%\textit{Corresponding Author}: Duong Thuy Anh Nguyen.} no need this for conferences
}
...
Nash equilibrium (NE) computation is essential for examining decision-making and strategic behavior in multi-agent systems, especially in non-cooperative games. These games have applications in numerous engineering domains, including communication networks, electricity markets, power systems, flow control, and crowdsourcing \cite{dyan16, BasharSG}.
In non-cooperative games, each agent has their own goals and seeks to maximize their payoff, without coordinating with other agents. Classical non-cooperative complete information game theory typically 
%a large body of works 
employs best-response or gradient-based methods to find NE, but these approaches %require each agent to possess 
necessitate each agent having complete information regarding the actions of its competitors \cite{Belgioioso2018,Yi2019}, which can be unrealistic in many practical engineering systems.

Extensive research has %focused on developing 
been conducted to develop efficient distributed techniques for seeking NE with partial information. These algorithms are primarily built upon projected gradient and consensus dynamics approaches and exist in both continuous-time \cite{Ye2017,Gadjov2019} and discrete-time \cite{Koshal2012,Koshal2016,Tatiana2020} forms. 
Early works only consider time-invariant undirected networks. For example,  \cite{SALEHISADAGHIANI201927} develops an algorithm within the framework of inexact-ADMM and establishes its convergence rate under the co-coercivity assumption on the game mapping. %Reference \cite{Tatiana2018} provides an
The accelerated gradient play algorithm (Acc-GRANE) presented in \cite{Tatiana2018}  is based on the strong monotonicity of an augmented mapping but is only applicable to a subclass of games. 
...
Reference \cite{Tatiana2020} develops a distributed gradient scheme based on the contractivity properties of doubly stochastic matrices, whose convergence properties do not depend on an augmented mapping.

Recently, there is a growing interest in studying NE computation in communication networks with switching topologies. Early works \cite{Koshal2012,Koshal2016} focus on aggregative games over time-varying, jointly connected, and undirected graphs, while \cite{Grammatico2021} extends the study to games with coupling constraints. In \cite{Farzad2019}, the authors propose An asynchronous gossip algorithm is developed for a directed graph, assuming each agent can update all estimates of the agents who interfere with its cost function. In \cite{Bianchi_2021}, a projected pseudo-gradient-based algorithm is proposed for time-varying,  directed graphs that are weight-balanced, while  \cite{Bianchi2020NashES} relaxes the weight-balancedness assumption for a static directed graph, assuming complete knowledge of the communication graph. Reference \cite{nguyen2022distributed} proposes a gradient algorithm that only requires connected communication graphs and row-stochastic weight matrices, and establishes a condition to deal with the frequently encountered loss of monotonicity with weighted norms.

Most of the works mentioned above rely on gradient-based methods to search for NE. The heavy-ball method, introduced in \cite{POLYAK19641}, has been widely used as an acceleration technique for gradient-based methods to achieve faster convergence \cite{Nguyen2023AccAB}. In the context of aggregative games, the heavy-ball method has been employed in algorithms for both undirected \cite{Song2020} and directed \cite{Fang2022} graphs, achieving convergence for diminishing step-sizes. Despite its effectiveness, there is a dearth of research on the use of heavy-ball-based methods within non-cooperative games.

\textbf{Contributions.} Motivated by the potential benefits of the heavy-ball method in accelerating convergence, this paper presents a novel distributed, discrete-time algorithm for NE seeking by integrating the heavy-ball momentum and consensus-based gradient method. The proposed algorithm is designed to work on a general sequence of time-varying directed graphs and allows for non-identical step-sizes and momentum parameters. Unlike many existing works, it does not require coordination among agents for the weights. 
%and is rigorously analyzed to converge geometrically to NE under 
The convergence of the algorithm to NE is rigorously analyzed under strong convexity and Lipschitz continuity assumptions, and explicit bounds for the constant step-sizes and momentum parameters are provided based on the properties of cost functions, mixing matrices, and graph connectivity structures.
...
It is worth noting that the proposed method has an optimal linear convergence rate for gradient-based methods with a constant step-size. In contrast, heavy-ball algorithms in \cite{Song2020,Fang2022} algorithms for aggregative games utilize diminishing step-size, which leads to slower convergence rates (at best, sublinear). The effectiveness of the proposed algorithm is demonstrated through numerical simulations on a Nash-Cournot game, showing accelerated convergence compared to existing methods.


This paper is structured as follows. Section~\ref{sec:formu} presents the problem formulation. In Section~\ref{sec:algo}, the distributed algorithm is introduced. Section~\ref{sec:basicre} and Section~\ref{sec:conv} provide auxiliary results and convergence analysis, respectively. Section~\ref{sec:simulation} numerically evaluates the performance of the proposed algorithm. Section~\ref{sec:conc} concludes with key points.

\textbf{Notations.} 
%Time index is denoted by $k$.
All vectors are viewed as column vectors unless otherwise stated. We write $u^\T$ for the transpose of a vector $u\in \re^n$. We write $\zero$ and $\one$ to denote the vector with all entries equal to $0$ and $1$, respectively.  The $i$-th entry of a vector $u$ is denoted by $u_i$, while it is denoted by $[u_k]_i$ for a time-varying vector $u_k$. We denote $\min(u)=\min_i u_i$ and $\max(u)=\max_i u_i$. We  use $\Diag(u)$ to denote the diagonal matrix with the vector $u$ on its diagonal. A nonnegative vector is called stochastic if its entries sum to $1$. 
...
\end{center}
where $\bu\!:=\![u_1^\T,\ldots,u_m^\T]^\T, \bv \!:=\![v_1^\T,\ldots,v_m^\T]^\T \!\!\in\!\! \re^{m\times n}$, and $u_i,v_i\!\in\!\re^n$. When $\pi = \one$, we write $\la \bu,\bv\ra$ and $\|\bu\|$. We have
...
\beqn
J_i(x_i^*,x_{-i}^*)\le J_i(x_i,x_{-i}^*), \quad \qquad\hbox{for all } x_i\in X_i.
\eeqn
...
Assumption~\ref{asum-graphs} can be relaxed by considering $B$-strongly-connected graph sequence.
, i.e., there exists an integer {$B\ge 1$} such that the graph with edge set 
...
\centering \normalsize
    ...
    \hline
    Every agent $i\in[m]$ selects a local stepsize $\a_i>0$, a\\ local momentum parameter $\beta_i>0$ and initializes with\\ arbitrary initial vectors $z_i^0, x_i^{-1}\in\re^{n}$.\\
    ...
    \emph{~~~~}$\qquad ~~~~ +\beta_i(x_i^k-x_i^{k-1})$;\\
...
% In particular, in the forthcoming lemma, we establish a relationship for the squared norm of a linear combination of vectors, which we will employ in our analysis with various identifications.
    ...
    \begin{align*}
    \Big\|\!\sum_{i=1}^n \!\g_i u_i - u \Big\|^2 \!\!\!= \!\sum_{i=1}^n \!\g_i \|u_i-u\|^2 \!-\!\!\sum_{i=1}^n \g_i \Big\|u_i \!-\! \Big(\sum_{\ell=1}^n \g_\ell u_\ell\Big)\!\Big \|^2\!\!\!.
    \end{align*}
    \end{lemma}
...
Assuming Assumption~\ref{asum-graphs} and that ${W_k}$ satisfies Assumption~\ref{asum-row-stochastic}, we have for all $k\ge 0$:
\begin{enumerate}
...
$$[\pi_k]_i\ge\frac{w^m}{m} \quad \hbox{for all $i\in[m]$ and all $k\ge0$},$$
...
Consider the vectors $r_i=\sum_{j=1}^m W_{ij}z_j$, for all $i\in[m]$, and let $\hat{z}_{\pi}=\sum_{i=1}^m \pi_{i}z_i$, for all $u\!\in\! \re^n$, we have
...
\begin{lemma}\label{lemma-LipschitzbF}
Let Assumptions~\ref{assum:lip} hold and 
% Consider the mapping $\bF_\a(\cdot)$ defined by~\eqref{eq-def-bfa} and  
$\bL_\a=\sqrt{\max\limits_{i\in[m]}\{\a_i^2(L_{-i}^2+ L_i^2)\}}$.
...
\begin{lemma}\label{lem-contraction} For a $\mu$-strongly convex function $f$ with $L$-Lipschitz continuous gradients, at the point $x^*=\argmin_{x} f(x)$, for all $\a$ with $0<\a<2L^{-1}$, we have
...
Consider the sequence of time-varying directed graphs $\G_k=([m],\E_k)$ that are are strongly connected.
% and the row-stochastic weight matrices $W_k\in \re^{m\times m}$ that are compatible with the graph $\G_k$. 
Under assumptions \ref{assum:map_monotone}-\ref{asum-row-stochastic}, we provide a proof demonstrating that the iterate sequence ${x^k}$ generated by Algorithm \textbf{DNE-HB} exhibit geometric convergence towards the NE.
...
\end{equation}
where $\beta=(\one_{n_1}^\T\beta_1,\ldots,\one_{n_m}^\T\beta_m)^\T$ 
...
For all $k\ge0$, we have
\begin{align*}
\|\bz^{k+1} &- \hat \bz^{k+1}\|_{\pi_{k+1}} \le  (1+\bL_\a) c_k \|\bz^k-\hat{\bz}^k\|_{\pi_k}\\
&+\bL_\a \|\hat \bz^k- \bx^* \|_{\pi_k}+ \bar{\b} \|\bz^k-\bz^{k-1} \|.\nonumber
\end{align*}
...
\end{equation} 
where $c_k=\sqrt{1- \tfrac{\min(\pi_{k+1})w^2}{\max^2(\pi_k)\mathsf{D}(\G_k)\mathsf{K}(\G_k)}} \in (0,1)$.
...
\begin{align*}
&\|(\mbi - \boldsymbol{\Pi}_{k})\bF_\a(W_k\bz^k)\|^2_{\pi_{k+1}} \nonumber\\ 
...
\!=& \|\bF_\a(W_k\bz^k)\!-\!\bF_\a(\bx^*) \|^2_{\pi_{k+1}} \!\le \bL_\a\|W_k\bz^k\!-\!\bx^*\|_{\pi_{k+1}},
\end{align} 
...
Applying Lemma~\ref{lem-contraction}, we obtain the following relation, for all $\a_i$ such that $0<[\pi_{k+1}]_i\a_i<2L_i^{-1}$:
\begin{align*}
...
\end{align}
where $q_k(\a)\!=\!\max\limits_{i\in [m]}\{|1\!-[\pi_{k+1}]_i\a_i\mu_i|,|1\!-[\pi_{k+1}]_i\a_i L_i|\}\!<\!1$.
...
For the last term on the RHS of \eqref{eq-zhatxstar}, from Assumption~\ref{assum:lip} we obtain
\begin{align*}
...
\end{align*}
where $L_1=\max\limits_{i\in[m]}{L_i}$ and $L_{2}=\max\limits_{i\in[m]}{L_{-i}}$.

Combining the relations in \eqref{eq-piNormEstFromWeightedAvg} and \eqref{eq-piNormEstFromNash1} with the preceding relation and using $\sqrt{a+b}\le \sqrt{a}+\sqrt{b}, \forall a,b\ge0$, yields
...
% Let Assumption~\ref{assum:map_monotone}-\ref{asum-row-stochastic} hold. Consider Algorithm~\textbf{DNE-HB} and the notations in~\eqref{eq-notat} and \eqref{eq-notat2}.
Let $\varphi_{k+1}=\frac{1}{\sqrt{\min\pi_{k+1}}}$. We then have
\begin{align*}
...
Finally, for the last term on the RHS of \eqref{eq-statediffrel}, we have
...
\end{bmatrix}\!\!,$\\
where $\bL_\a$ defined as in Lemma~\ref{lemma-LipschitzbF}, $q_k(\a)$ defined as in Proposition~\ref{prop-NEdist}, and $\varphi_{k+1}$ defined as in Proposition~\ref{prop-statediff}. 
\end{proposition}
\begin{proof}
...
for some matrix $M(\a,\b)$. Then, we can select appropriate step-size and the acceleration parameter such that the spectral radius $\rho_{M}$ of $M(\a,\b)$ is less than $1$, as follows.
We have $\bL_\a =\bar{\a}L$, where $L= \sqrt{L_1^2+L_2^2}$. Additionally, for $\bar{\a}\in(0,2(L_1+\mu)^{-1})$, we obtain $q_k(\a)=1-\munderbar{\a} \min(\pi_k)\mu <1$. We denote $\sigma$ as a lower bound for $\min(\pi_k)$, for $k\ge0$, i.e., $\sigma \le \min_{k\ge0}{\min(\pi_k)}$ (see Lemma~\ref{lemma-TimeVaryPiVectors}).
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.24\textwidth]{betaEffect.png}\vspace{-0.2cm}
...
\label{sec:conc}
This paper introduced a novel approach for finding NE for games over time-varying directed networks. The proposed algorithm incorporates the heavy-ball acceleration to improve the performance of the gradient-based distributed NE-seeking algorithm. Explicit bounds for the non-identical step-sizes and momentum parameters are obtained based on the properties of the cost functions and network structure, and a geometric convergence rate of the algorithm is shown. Our numerical results illustrate the benefits of the proposed accelerated distributed method. A main future research direction is to investigate the convergence for games with constrained action sets.
