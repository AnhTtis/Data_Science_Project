...
\subsection*{Acknowledgement} The first author is supported by the UC Davis Chancellor’s Fellowship and the Collaboration Grants for Mathematicians by the Simons Foundation.
...
\begin{lemma}\label{lemma:all nonzero nonempty}
    Suppose $H_0(x)$ is nonempty for all $x\in E\setminus\{0\}$. Then, $H_k(x)=H_0(x)$ for all $x\in E\setminus\{0\}$ and $k\ge0$ and $H_k(0)=H_1(0)$ for ll $k\ge1$.
\end{lemma}
...
Viewing $K$ as a four-dimensional Hilbert space, we claim $K_0$ is not a polytope. To see this, restrict further to the affine subspace where $f_3(0)=-1$ and $f_1(0)=2$. Thus,
\begin{equation*}
    R_1=\{(y_1,y_2):y_1\le 2,y_2\le 2\}
\end{equation*}
and
\begin{equation*}
...
\end{equation*}
and $f_2(0),f_4(0)\ge0$, the question becomes whether the upper right corner $(f_2(0),f_4(0))$ meets $R_1\cap R_3$. In the range of $1\le f_2(0),f_4(0)\le 2$, this is a nonlinear problem since $R_1\cap R_3$ has a curved boundary given by $y_2\ge 1+\frac{1}{y_1-1}$. Thus, $K_0$ may not be defined by a finite number of linear inequalities.
