...
\author{Yunfeng~Hou, Ching-Yen~Weng, and  Qingdu~Li
\thanks{Yunfeng Hou (yunfenghou@usst.edu.cn) and Qingdu Li (liqd@usst.edu.cn) are with the Institute of Machine Intelligence, University of Shanghai for Science and Technology, Shanghai 200093, China.
...
Based on the communication automaton, we can define dynamic observations of a communicated string.
  To guarantee that a sensor activation policy is physically implementable and insensitive to random control delays and observation delays, we further introduce the definition of delay feasibility.
...
A practical example is provided to show the application of the developed sensor activation methods.
Finally, we briefly discuss how to extend the proposed framework to a decentralized sensing architecture.
\end{abstract}
...
It is of interest to turn on and off sensors to efficiently make observations for fulfilling certain tasks, such as fault diagnosis and control.
The problem of sensor optimization in discrete-event systems (DESs) has been extensively investigated for more than twenty years.
...
After that, many approaches have been proposed for minimizing the sensor activations. Generally, these approaches can be divided into language-based and transition-based according to the solution domain of the sensor activation policy (SAP) to be synthesized.
...
How to minimize sensor activations for diagnosis in a language-based solution domain was also investigated in
\cite{wang19tac}, where the version of diagnosability does not require how soon diagnosis decisions should be announced.
Additionally, the authors in \cite{shu13tase,wang2016online} developed online approaches for the minimization of sensor activations.
Sensor activations for the purpose of control or diagnosis were considered in a decentralized observation setting in \cite{lafortune10tac,wang10ac,yin18tac} where the system is monitored by several local agents, and each of them makes its own observation and sensor activation decision.
...
We call such a system as a networked DES, which has drawn considerable attention in the past few years in the literature \cite{lin2014control,liu21tac,wang16tase,shu17tac1,zhu19cdc,tai22ac,yunfeng23tcns,hou20tac,xu20tcns,lin20tcns,zhao17tcns,alves20tac,weilin22ac}. 
Compared with non-networked control structure, the networked control structure provides a more flexible way to drive a system. 
For example, the agent can be an edge computing node, which can share the computing resource with other edge computing nodes.
However, communicating data over networks is also challenging since nondeterministic observation delays and control delays are unavoidable.
From another perspective, even for the non-networked DESs, implementing sensor activation commands could involve preparations after commands are received, which incurs control delays, and detecting whether an event has occurred or not takes time, which incurs observation delays.
Therefore, a key issue for sensor optimization in DESs is how to overcome possible delays existing between the agent and plant.
To the best of our knowledge, this is the first paper to consider delays when activating sensors, and all the previous works  assume that communications between the plant and the agent are instaneous.}
...
These delays are random and upper bounded but do not change the order of observations and controls.

To track possible observation delays and control delays, we first model the observation channel by a sequence of events that have occurred but not been communicated to the agent (observation channel configuration).
We also model the control channel by a sequence of events that have been communicated to the agent but the sensor activation decisions made following these communications have not been executed (control channel configuration).
Then, a communication automaton is constructed to dynamically track the plant state, the observation channel configuration, and the control channel configuration.
Using the communication automaton, we determine which sensor activation decision is taking effect in the presence of observation delays and control delays.
To guarantee that an SAP is insensitive to random observation delays and control delays, it is required that (i) if the SAP intends to activate or deactivate a sensor after the communication of a string, this sensor can be activated or deactivated after this string for all possible control delays and observation delays.
...
We also prove that the anti-monotonicity property holds for a delay feasible SAP, i.e., the more sensors a delay feasible SAP activates, the fewer string pairs an agent confuses under observation delays.
Finally, we present a set of algorithms to compute a minimal delay feasible transition-based  SAP while ensuring that a given specification condition is satisfied.

The rest of this paper is organized as follows. 
Section II presents some preliminary concepts. 
Section III discusses how to activate sensors under observation delays and control delays and introduces the definition of delay feasibility.  
Section IV provides a set of algorithms to calculate a minimal delay feasible transition-based SAP.
We use a practical example to illustrate the application of the proposed algorithms in Section V.
Section VI extends the proposed framework to a decentralized sensing architecture.
Section VII concludes this paper.
%Due to the limitation of the space, all the proofs of this paper are omitted and can be found in Appendix.
...
\textcolor{blue}{The occurrences of transitions of $G$ take time. To describe this, we define the minimum occurring time function $t_{\downarrow}: Q \times \Sigma \rightarrow \mathbb{N}$. Formally, for each transition $(q,\sigma)\in TR(G)$, $t_{\downarrow}(q,\sigma)$ means that event $\sigma$ cannot complete its occurrence at state $q$ unless $t_{\downarrow}(q,\sigma)$ units of time have elapsed since it was fired.
Only a finite number of events can occur in one unit of time.
Note that, we say that an event occurs if this event completes its occurrence in this paper.}
%We extend function $t_{\downarrow}$ to a sequence of event transitions in the followig manner.
%Given a sequence of event transitions $s=(q_1,\sigma_1)\cdots (q_k,\sigma_k) \in (TR(G))^*$, the minimum activation time of $s$ is defined as: $t_{\downarrow}(s)=t_{\downarrow}(q_1,\sigma_1)+\cdots + t_{\downarrow}(q_k,\sigma_k).$ }
...
\begin{enumerate}
\item \textcolor{blue}{The observation delay is upper bounded by $N_o$ units of time, i.e., an event must have been communicated to the agent before no more than $N_o$ units of time since it occurred;}
\item \textcolor{blue}{The control delay is upper bounded by $N_c$ units of time, i.e., a sensor activation command must have been executed by the actuator of the plant before no more than $N_c$ units of time since it was issued;}
\item Both the control channel and the observation channel satisfy the first-in-first-out (FIFO) property, i.e., the sensor activation commands are executed in the same order as they were issued, and the observable event occurrences are received in the same order as they occurred;
\item The actuator always uses the most recently received sensor activation command, and the initial sensor activation command can be executed without any delays since the initial command can be executed beforehand.
\end{enumerate}
...
\textcolor{blue}{To determine whether an event occurrence is observable or not, we need the information of the sensor activation command being in use before this event occurrence.
When there exist no delays, the sensor activation command taking effect can be uniquely determined by the current observation.
However, when there are observation delays and control delays, the sensor activation commands taking effect between two successive event occurrences are nondeterministic.
An event occurrence after a string can be observed for some delays, but cannot be observed for the other delays.
%That is, the observation to an occurred string is nondeterministic.
It is challenging to turn on and off sensors as requested in networked DESs.
We use the following example to further illustrate this.}
  

\begin{figure}
	\begin{center}
		\includegraphics[width=7.6cm]{Fig2.pdf}   
...
We want to infer the occurrence of $f$ when $bfa$ occurs.
...
And we will know that event $f$ must have occurred when we see $ba$.
We can write such a SAP as: $\Omega(\varepsilon)=\{b\}$, $\Omega(b)=\Omega(bf)=\{a\}$, and  $\Omega(a)=\Omega(bfa)=\emptyset$.}

\textcolor{blue}{However, if there are observation delays and control delays, the SAP $\Omega$ may do not work.
For example, suppose that $N_o=1$ and $N_c=2$.
Since $t_{\downarrow}(2,f)=1$, we can observe the occurrence of $b$ when event $a$ occurs in state 3 (after $bf$).
Nevertheless, due to control delays, the sensor activation command taking effect when event $a$ starts to occur in state $3$ may not be the one issued after observing $b$, but the one initially issued.
If this is the case,  we cannot detect the occurrence of $a$ after $bf$ since $a \notin \Omega(\varepsilon)$.}
...
In this section, we develop a framework for sensor activations under observation and control delays.
...
To be more specific, let $s=\sigma_1\cdots\sigma_n \in \Sigma^*$ be a string that is defined at some $q\in Q$ in $G$ and  with a minimum occurring time that is no larger than $N_o$. 
Without loss of generality, we suppose that it is the longest such string.
Then, we have $N=|s|$ since $\sigma_1$ must have been communicated to the agent before the next event occurrence (after $s$).
%Therefore, we have $N=|s|$ in the worst case.
Since only a finite number of events can occur in $N_o$ units of time, $N$ is a finite constant.}
\end{remark}
...
When a new event occurs or a new event is communicated to the agent, the observation channel configuration is updated as follows.
...
\textcolor{blue}{Since the observation delay is upper bounded by $N_o$ units of time, the minimum occurring time of the string occurring when $\sigma_1$ is delayed at the observation channel should never exceed $N_o$.
...
%We have $N=N_o+1=2$.
Let ${\theta}_o=(b,0)\in \Theta_o$ be the current observation channel configuration.
Since $\mathbf{MAX}^{obs}(\theta_o)=0 \le N_o$, when event $f$ occurs after $b$, by (\ref{Eq2}), $\theta'_o=\mathbf{IN}^{obs}({\theta}_o,2,f)=(b,1)(f,0)$.
Since $\mathbf{MAX}^{obs}(\theta'_o)=1=N_o$, $a$ cannot occur at state 3 unless $b$ is communicated to the agent.
If the occurrence of $b$ is communicated, by (\ref{Eq3}),  $\mathbf{OUT}^{obs}({\theta}'_o,b)=(f,0)$.}
\end{example}
...
As discussed in Remark \ref{Rem1}, $M$ is determined by the maximum length of a string whose minimum occurring time is no larger than $N_o+N_c$.
Since only a finite number of events can occur in $N_o+N_c$ units of time, $M$ is a finite constant.}
\end{remark}
...
\textcolor{blue}{When a new event $\sigma$ (observable or not) is communicated to the agent, by Definition \ref{Def2}, we  add $(\sigma,0)$ to the end of $\theta_c$ to show that the sensor activation command made following the communication of $\sigma$ has been issued and been pushed into the control channel.
...
Thus, $\textbf{OUT}^{ctr}(\theta_c,\sigma)$ is defined iff $\theta_c=(\sigma_1,m_1)\cdots(\sigma_k,m_k)\neq \varepsilon \wedge  \sigma=\sigma_1$.
When the sensor activation command issued after the communication of $\sigma$ is executed, $\textbf{OUT}^{ctr}(\theta_c,\sigma)$ removes the first component from the head of $\theta_c$, as shown in (\ref{Eq6}).


\begin{example}\label{Ex3}
Let us continue with Example \ref{Exa1}.
Without loss of generality, let ${\theta}_c=(b,0)\in \Theta_c$ be the current control channel configuration.
Since $\mathbf{MAX}^{ctr}(\theta_c)=0\le N_c$, when event $f$ occurs after $b$, by (\ref{Eq4}), $\theta'_c=\mathbf{PLUS}(\theta_c,2,f)=(b,1)$.
Then, if the occurrence of $f$ is communicated to the agent, by (\ref{Eq5}), $\theta''_c=\mathbf{IN}^{ctr}(\theta'_c,f)=(b,1)(f,0)$.
Since $\mathbf{MAX}^{ctr}(\theta''_c)=1=N_c$, $a$ cannot occur at state 3 unless the sensor activation command made after the communication of $b$ is executed.
When the sensor activation command made after the communication of $b$ is executed, by (\ref{Eq6}), $\mathbf{OUT}^{ctr}(\theta''_c,b)=(f,0)$.
\end{example}
...
For all $\sigma \in \Sigma$,  we use $h(\sigma)$ to denote that the event occurrence of $\sigma$ has been communicated to the agent.
...
For all $\sigma \in \Sigma$,  we use $g(\sigma)$ to denote that the sensor activation command made immediately after the communication of $\sigma$ has been executed.
\end{enumerate}
...
\textcolor{blue}{Since we distingusih ``the occurrence of an event", ``the communication of an event", and ``the execution of a sensor activation command'' in this paper, we assume that $\Sigma$, $\Sigma_h$, and $\Sigma_g$ are mutually disjoint.}
...
When $\sigma$ occurs, we set $q \leftarrow \delta(q,\sigma)$  to track the plant state.
Meanwhile,  by (\ref{Eq2}) and (\ref{Eq4}), we have $\theta_o \leftarrow \textbf{IN}^{obs}(\theta_o,q,\sigma)$ and $\theta_c \leftarrow \textbf{PLUS}(\theta_c,q,\sigma)$.
...
Now, we use an example to illustrate the construction of $\tilde{G}$.

\begin{figure}
	\begin{center}
		\includegraphics[width=8.8cm]{Fig3}   
...
Let us consider again Example \ref{Exa1}.
The system $G$ is given by Fig. \ref{Fig2}. 
%Suppose that the control delays and observation delays are both upper bounded by 1,  i.e., $N_c=N_o=1$. 
...
After a new event occurrence, by (\ref{Eq2}), we have $\theta_{o}=\mathbf{IN}^{obs}(\varepsilon,0,a)=(a,0)$ and $\theta'_{o}=\mathbf{IN}^{obs}(\varepsilon,0,b)=(b,0)$.
%Moreover, since $\mathbf{MAX}(\varepsilon^+)=0\le N_c$, by (\ref{Eq4}), $\mathbf{PLUS}(\varepsilon^+)=\varepsilon$.
Thus, when $a$ or $b$ occurs in state $\tilde{q}_0$, by (\ref{Eq7}), $\tilde{\delta}(\tilde{q}_0,a)=\tilde{q}_1=(1,\theta_{o},\varepsilon)$ or $\tilde{\delta}(\tilde{q}_0,b)=\tilde{q}_2=(2,\theta'_{o},\varepsilon)$.
...
When the occurrence of $a$ is communicated to the agent,  $\theta_o \leftarrow \mathbf{OUT}^{obs}(\theta_{o},a)=\varepsilon$.
...
Therefore,  the commands taking effect right before the occurrences of $b$ and $f$ are both the initial sensor activation command.
\end{remark}
...
For example, let us consider the string $\mu=bh(b)f \in \mathcal{L}(\tilde{G})$ in Fig. \ref{Fig3}. 
...
Not all arbitrary SAPs $\Omega$ can be used to activate sensors in the context of networked DESs.
Specifically, due to observation delays and control delays, the command being in use after the occurrence of a string $s$ may not be $\Omega(s)$.
If $\sigma \in \Omega(s)$ (or $\sigma \notin \Omega(s)$), to guarantee that the event occurrence of $\sigma$ can (or cannot) be observed by the agent after the communication of $s$, it is required that all the sensor activation commands that may be taking effect when $\sigma$ occurs after $s$ must activate (or deactivate) the sensor for $\sigma$.
That is, the SAP should be insensitive to random observation delays and control delays.
Additionally, to guarantee that $\Omega$ is practically feasible, it is required that any two communicated strings that can confuse the agent must be followed by the same sensor activation mappings.
Formally, we have the following definition of delay feasibility.



\begin{definition}\label{Def3}
(Delay Feasibility) A given SAP $\Omega$ is said to be delay feasible w.r.t. $\tilde{G}$ if the following two conditions are true. 
\begin{align*}
...
Let us first show that delay feasibility is closed under union.
To prove this, we need the following lemma.

\begin{lemma}\label{Lem1}
Given two SAPs $\Omega_1$ and $\Omega_2$, if $\Omega_1 \supseteq \Omega_2$ and $\Omega_2$ is delay feasible, then 
\begin{align}(\forall \mu,\mu' \in \mathcal{L}(\tilde{G}))P_{\Omega_1}(\mu)=P_{\Omega_1}(\mu') \Rightarrow P_{\Omega_2}(\mu)=P_{\Omega_2}(\mu').\end{align}
\end{lemma}
%\begin{proof}
...
In this section, we first formulate the sensor
...
We would like to find an SAP $\Delta^*\subseteq \Delta^{all}$ such that:
\begin{enumerate}
\item  $\Delta^*$ is delay feasible, i.e., \textup{C}3 and \textup{C}4 holds under $\Delta^*$; 
\item $\mathcal{T}_{spec}$ is satisfied under $\Delta^*$, i.e., $\mathcal{T}_{conf,G}(\Theta_{\Delta^*}^{N_o}) \cap \mathcal{T}_{spec}=\emptyset$; 
\item $\Delta^*$ is minimal, i.e., there is no other $\Delta'\subset \Delta^*$ satisfying 1) and 2).
\end{enumerate}
\end{problem}
...
{To determine which command is taking effect when checking delay feasibility, we first refine the state space of the communication automaton $\tilde{G}$ as follows:}
\begin{enumerate}
...
We next discuss how to verify delay feasibility of a given  $\Delta$.
\begin{proposition}\label{Proposition5}
\textcolor{blue}{Given system $G$, we construct $\tilde{G}$, $G^g$, and $W$ as described above. 
...
Thus, we let $\mathcal{T}_{spec}\leftarrow\{(0,5),(1,5),(2,5)\}.$
...
Next, we use a practical example to illustrate the applicatioin.
...
\begin{definition}
(Delay $K$-diagnosability) A prefix-closed and live language $\mathcal{L}(G)$ is said to be delay $K$-diagnosable w.r.t. $\Theta_{\Omega}^{N_o}$, $E_f$, and $\mathcal{L}(G)$ if the following condition holds:
...
When $N_o=0$, delay $K$-diagnosability reduces to $K$-diagnosability in nonnetworked DESs \cite{dallal14tac,yin18tac,yin19ac}.
...
Furthermore, due to the generality of $\mathcal{T}_{spec}$, the proposed framework is also applicable for solving the dynamic sensor activation problems for other state pairs disambiguation properties (not confined to just the fault diagnosis problem), such as, fault prognosis problem, supervisory control problem, and detectability problem, in the context of networked DESs.}
\end{remark}

\subsection{\textcolor{blue}{Illustrative Example}}


We consider a production line that are serviced by a robot.
...
To diagnose the conveyor belt failure, we  dynamically activate  sensors  to observe the system under observation delays and control delays.
The event set for this example is $\Sigma=\{a,b,f,c,d\}$, where
\begin{itemize}
\item{
$a$: A part arrives at Machine $A$, and the robot takes it to Machine $A$, and Machine $A$ starts to work on this part.}
\item{
$b$: Machine $A$ completes its process,  and the robot puts the processed part on a working conveyor belt.}
\item{
$f$:  Machine $A$ completes its process,  and the robot puts the processed part on the conveyor belt that has been broken down.}
\item{$c$: Robot 2 takes a part from the conveyor belt to Machine $B$, and Machine $B$ starts to process it.}
\item{$d$: Machine $B$ completes the process, and the robot takes the processed part away from  Machine $B$.
}
\end{itemize}

The state of the production line can be defined as a vector $\bar{x}_k=[q_1,q_2,q_3,q_4]$, $k=0,1,\ldots,5$, where $q_{1}$ takes 0 or 1, corresponding to Machine $A$ being idle or busy; $q_{2}$ takes 0 or 1, corresponding to Machine $B$ being idle or busy; $q_{3}$ also takes 0 or 1, corresponding to the conveyor belt being working well or broken down; $q_{4}$ is number of parts waiting to be processed at Machine $B$, which is constrained to the values $\{0,1\}$.

\begin{figure}
	\begin{center}
		\includegraphics[width=7.4cm]{Fig5.pdf}   
...
\begin{table}
\begin{center}
\caption{States of system $G$.}
{\begin{tabular}{cccccc} \toprule
State & Description & State & Description & State & Description \\ \midrule
$\bar{x}_0$ & [0,0,0,0] & $\bar{x}_2$ & [0,0,0,1] & $\bar{x}_4$ & [0,0,1,0]\\
$\bar{x}_1$ & [1,0,0,0]& $\bar{x}_3$ & [0,1,0,0] & $\bar{x}_5$ & [1,0,1,0]\\
 \bottomrule
\end{tabular}}
\end{center}
\label{Tab1}
...
Only event $a$ could occur at $\bar{x}_0$, which corresponds to an part arriving at Machine $A$, and Machine $A$ starting to process this part.
Upon the occurrence of $a$, the system moves to $\bar{x}_1=[1,0,0,0]$. 
In state $\bar{x}_1$, $b$ could occur, which corresponds to Machine $A$ completing its process and the robot placing the part on the conveyor belt.
In addition, the conveyor belt may get stuck when we try to deliver a processed part from Machine $A$ to $B$.
Thus,  $f$ could also occur in state $\bar{x}_1$.

If $b$ occurs in state $\bar{x}_1$, the system moves to state $\bar{x}_2=[0,0,0,1]$. 
Since the robot serves in a sequential order of Machines $A$ and $B$, and can only serve a machine at a time, only $c$ could occur in state $\bar{x}_2$, which corresponds to the robot taking the part from the conveyor belt to Machine $B$, and Machine $B$ starting to process it. 
Upon the occurrence of $c$, the system moves to state $\bar{x}_3=[0,1,0,0]$.
When Machine $B$ completes its process, upon the occurrence of $d$, the system moves from state $\bar{x}_3$ to the initial state $\bar{x}_0$ and makes state transitions as mentioned above.

On the other hand, if $f$ occurs in state $\bar{x}_1$, the system moves to state $\bar{x}_4=[0,0,1,0]$. 
In state $\bar{x}_4$, since the conveyor belt does not work, $a$ is the only event could occur, which corresponds to Machine $A$ starting to process another new part.
Upon the occurrence of $a$, the system moves to state $\bar{x}_5=[1,0,1,0]$. 
When Machine $A$ completes its process, upon the occurrence of $f$, the system moves back to $\bar{x}_4$, and repeats the state transitions again and again.
...
In this example, we assume that events $a,b,c,d$ are observable, and $f$ is unobservable, that is, $\Sigma_o=\{a,b,c,d\}$ and $\Sigma_{uo}=\{f\}$.
Since it takes time to  complete a process at Machines $A$ and $B$, we let $t_{\downarrow}(\bar{x}_0,a)=t_{\downarrow}(\bar{x}_2,c)=t_{\downarrow}(\bar{x}_4,a)=3$ and $t_{\downarrow}(\bar{x}_1,b)=t_{\downarrow}(\bar{x}_1,f)=t_{\downarrow}(\bar{x}_5,f)=t_{\downarrow}(\bar{x}_3,d)=1$.
We also assume that both the observation delays and the control delays are upper bounded by 2 units of time, i.e., $N_o=N_c=2$.

Suppose we want to diagnose if the the conveyor belt has been broken down ($f$ occurs or not).
Cleary, if the sensors for $a$ and $c$ keep working since the system starts running, $\mathcal{L}(G)$ is delay $2$-diagnosable. 
...
Then, by using Algorithms 1 and 2, one can calculate a minimal SAP $\Delta^*$ such that delay $2$-diagnosability holds under it:
\begin{align*}
\Delta^*=&\{(\bar{x}_0,a),(\bar{x}_1,a), (\bar{x}_2,a), (\bar{x}_3, a), (\bar{x}_4,a), (\bar{x}_5,a)\} \\
&\cup \{(\bar{x}_0,b),(\bar{x}_1,b), (\bar{x}_2,b), (\bar{x}_4,b), (\bar{x}_5,b)\}.
\end{align*}
By $\Delta^*$, we need to first activate the sensor for $a$ at the time it occurs.
Because otherwise, we cannot distinguish $afaf$ and $\varepsilon$.
Meanwhile, to distinguish $afaf$ and $abcda$, $\Delta^*$ actives the sensor for $b$ when it occurs (but deactives the sensors of $c$ and $d$ for the minimization of sensor activations.).
To ensure delay feasibility of $\Delta^*$, by $\textup{C}3$ and $\textup{C}4$, we have $(\bar{x}_i,a)\in \Delta^*$ for $i=0,1,\ldots,5$, and $(\bar{x}_j,b)\in \Delta^*$ for $j=0,1,2,4,5$.
%That is, we may active the sensors $a$ or $b$ at some states in which $a$ or $b$ are not defined in $G$.


\section{\textcolor{blue}{Extension to a decentralized sensing architecture}}
...
In this section, we briefly discuss how to extend the proposed framework to a decentralized sensing and communication architecture.
...
Decentralized fault diagnosis, where several diagnosing agents jointly diagnose
the plant based on their own observations, is an efficient way
of fulfilling fault diagnosis for these large-scale networked
systems.
As shown in Fig. \ref{Fig6}, there is a set of $n$ partial-observation diagnosing agents, and all the local diagnosing agents work independently, i.e., there are no communications among them. 
Events occurring in the plant are communicated to local diagnosing agent $i$ over observation channel $i$, $i=1,\ldots,n$, and the local diagnosing agent $i$ sends sensor activation decisions to the actuator of the plant based on its SAP over control channel $i$.
...
Since we focus on the centralized sensor activation problem in this paper, the formal algorithms for the computation of  $\mathcal{T}_{conf,G}(\Theta^{\bar{\Delta}})$ and the verification of (\ref{Eq20}) are beyond the scope of this paper, yet a fruitful area for the futher exploration.
...
(Decentralized Sensor Optimization) Suppose we are given a system $G$, a specification $\mathcal{T}'_{spec}$, and a set of agents.
...
\begin{enumerate}
\item  $\bar{\Delta}^*$ is delay feasible, i.e., ${\Delta}^*_i$ satisfies \textup{C}3 and \textup{C}4 for $i=1,\ldots, n$; 
\item $\mathcal{T}'_{spec}$ is satisfied under $\bar{\Delta}^*$, i.e., $\mathcal{T}_{conf,G}(\Theta^{\bar{\Delta}^*}) \cap \mathcal{T}'_{spec}=\emptyset$; 
\item $\bar{\Delta}^*$ is minimal, i.e., there is no other $\bar{\Delta}'\subset \bar{\Delta}^*$ satisfying 1) and 2).
\end{enumerate}
\end{problem}
...
Building on the results of Section IV and exploiting Proposition \ref{Proposition9}, we propose the following algorithm for solving Problem \ref{Prob2}.
...
\eIf{$\mathcal{T}_{conf,G}(\bar{\Delta}^{\uparrow}) \cap \mathcal{T}'_{spec}=\emptyset$}
...
In Algorithm 3, we initialize $\Omega_i\leftarrow \Sigma_{o,i}$, i.e., the set of all transitions that can be activated by agent $i$. 
$D_i$ is the set of transitions that cannot be removed; it is initially empty.
The correctness of the above algorithm is established in the
following theorem
...
In all, the worst-case complexity of using the preceding algorithms is of
the order of $n\times |Q|\times |\Sigma|$ times the maximum of $|Q|^3 \times |\Sigma|^{2\times(N+M)}\times N_c^N \times N_o^M$ and
the complexity for verifying $\mathcal{T}_{conf,G}(\bar{\Delta}^{\uparrow}) \cap \mathcal{T}'_{spec}$.
...
In this paper, we have considered the sensor activation problem for networked DESs, where the sensor systems communicated with the agent over a shared network are subject to random observation delays and control delays.
...
We have shown that a delay feasible SAP can be used to dynamically activate sensors, even if there exist nondeterministic control delays and observation delays.
...
Finally, we have extended the proposed framework to a decentralized sensing architecture.
...
($\Rightarrow$) 
The proof is by contradiction.
If $(\ref{Equation14})$ does not hold, $\exists(q,\theta_o,\theta_c,x)\in Q_W$ such that (i) $\tilde{\delta}((q,\theta_o,\theta_c),\sigma)!$ and $(q,\sigma) \in \Delta$ and $(x,\sigma) \notin \Delta$, or (ii) $\tilde{\delta}((q,\theta_o,\theta_c),\sigma)!$ and $(q,\sigma) \notin \Delta$ and $(x,\sigma) \in \Delta$.
Without loss of generality,  we write $\delta_W(q_{0,W},\mu)=(q,\theta_o,\theta_c,x)$.
Since $W=\tilde{G}||G^g$,  we have that $\tilde{\delta}(\tilde{q}_{0},\mu)=(q,\theta_o,\theta_c)$ and $\delta(q_{0},g^{-1}(\psi^g(\mu)))=x$.
If (i), since $\tilde{\delta}((q,\theta_o,\theta_c),\sigma)!$, $\mu\sigma \in \mathcal{L}(\tilde{G})$.
Since $(x,\sigma) \notin {\Delta}$,  $(\delta(q_0,g^{-1}(\psi^g(\mu))),\sigma) \notin  {\Delta}$.
...
Therefore, $\exists \mu,\mu'\in \mathcal{L}(\tilde{G})$ such that $\delta(q_0,\psi(\mu))=q$, $\delta(q_0,\psi(\mu'))=q'$, $P_{\Delta}(\mu)=P_{\Delta}(\mu')$, $(q,\sigma) \notin \Delta$, and $(q',\sigma) \notin \Delta$.
