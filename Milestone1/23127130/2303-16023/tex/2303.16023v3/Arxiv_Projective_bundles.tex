...
\subsection*{Notation}
We work on a complex number field $\mathbb{C}$. We use standard notation for abelian varieties. If $X$ is an abelian variety, we denote by $\hat{X}$ its dual abelian variety. Given an element $\alpha\in\hat{X}$, we let $P_\alpha$ be the corresponding line bundle in $\operatorname{Pic}^0(X)$, that is 
...
Let $Z$ be an algebraic variety over an algebraically closed field $\mathbb{K}$ and let $L$ be an ample
