...
We have implemented an automatic element-wise operations for vector-valued fields in order to facilitate their use.
...
Due to the preliminary nature of the DaCe prototype, we decided not to include it in the performance evaluation. The \texttt{gt:cpu\_ifirst} backend was used at the baseline, since this is the best performing CPU backend.
...
%We use the code generated by the \texttt{dace:cpu} backend -- which is currently only a prototype -- as the baseline, in the hope that DaCe will produce more performant code with time. The first observation is that all other backends perform significantly better. In addition, its GPU equivalent (\texttt{dace:gpu}) is not listed, because it was not yet functionally complete.
    ...
    The two GPU backends (namely the \texttt{cuda} and \texttt{gt:gpu}), perform worse than the CPU backends for small problem sizes.
    This is due to the under-utilization of the GPU resources for small-scale problems which do not fully saturate the device.
    The delayed quadratic scaling of 
    However, their scaling with increased problem size is more favorable and hence they both end up outperforming the CPU code for larger problems.
...
%             Speedup Factor & 1.00 & 5.14 & 5.40 & 6.22 & 17.2 \\\bottomrule
        ...
        \label{table:speedup}
    \end{table}
One surprising observation from Table \ref{table:speedup} is that the fastest GPU backend only produces a $\sim 3.2$x speedup versus the fastest CPU backend.
