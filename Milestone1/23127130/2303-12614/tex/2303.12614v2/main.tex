...
\begin{document}
	...
	\begin{abstract}
		We establish several non-existence results of positive scalar curvature (PSC) on fiber bundles. As one of our main results, we show up to dimension 7, for $X^m$ a Cartan-Hadamard manifold or an aspherical manifold when $m=3$, the fiber bundle over $X^m\#M^m$ ($m\ge 3$) with $K(\pi,1)$ fiber carries no PSC metric under an incompressible condition. Under certain spin compatible condition, we will prove the similar result for fibers in a class $NPSC^+$ (including enlargeable and \textit{SYS} ones) or with non-vanishing Rosenberg index. Furthermore, we also give a systematic study on PSC obstruction on $S^1$ bundles over large class of spaces. These results would provide new examples for Gromov's question of PSC quadratic decay in twisted setting, and extend some previous results of Hanke, Schick and Zeidler concerning PSC obstruction on fiber bundles.
	\end{abstract}
\tableofcontents	
	\section{Introduction}
	
	$S^1$ factor plays important roles in the study of geometry and topology of positive scalar curvature.  The famous Georch conjecture, resolved in \cite{ref24} by Gromov-Lawson and in \cite{ref21} by Schoen-Yau, states $T^n$, completely constructed by $S^1$ factor, is such a typical manifold which admits no positive scalar curvature metric. One of the major advantage of the $S^1$ factor is that it can be used in the descending arguement combining with the minimal hypersurface technique. The descending arguement was first proposed by Schoen-Yau in \cite{ref21}. They observed a stable minimal hypersurface in the codimension 1 homology class of a positive scalar curvature manifold must have positive Yamabe quotient. This codimension 1 homology class is always assigned to be the Poincare Dual of 1-dimensional cohomology class, which can be regarded as $S^1$ factor in an abstract sense. Later in \cite{ref25}, Schick noted by taking cup product of such $S^1$ factor $(n-2)$ times would yield complete topological condition to obstruct the existence of a positive scalar curvature (PSC) metric. By this observation, he gave counterexample of the unstable Gromov-Lawson-Rosenberg conjecture. The construction in \cite{ref25} was later generalized to the notion of \textit{SYS} manifold.
	
	The $S^1$ factors are also widely used in geometric problems related to positive scalar curvature. It was first observed in \cite{ref4} by Fischer-Colbrie and Schoen that an appropriate warped product of the stable minimal hypersurface with $S^1$ carries positive scalar curvature metric, it was then developed by Gromov and Lawson to a torical symmetrization technique to estimate the  Lipshitz constant of non-zero degree map from  a PSC manifold to a fixed model model in \cite{ref6}. Recently, Gromov observed such symmetrization method can be used to prove width estimate of Riemannian band in his notable paper\cite{ref7}, and many breakthroughs have been made following this strategy recently (For example, see \cite{ref3}\cite{ref9}\cite{ref26}\cite{ref27}).
	
	In \cite{ref18}, Rosenberg made the following stability conjecture for $S^1$ multiplication: {\it a closed manifold $M$ admits no metric of positive scalar curvature if and only if $M\times S^1$ admits no metric of positive scalar curvature, provided the dimension of $M$  not equal to 4,} where the requirement of dimension not being $4$ is due to a counterexample coming from the Seiberg-Witten invariant (c.f.\cite{ref19}). In \cite{ref16}, by using the $\mu$-bubble arguement, R{\"{a}}de completely solved this conjecture in dimension no greater than 7 by establishing a width inequality for Riemannian bands obtained from the product of general closed manifold and the closed interval. In fact, the following result is proved in \cite{ref16} (Corollary 2.25): {\it Let $n\in\lbrace 2,3,4,6,7\rbrace$ and $Y^{n-1}$ be a closed connected oriented manifold which does not admit a metric of positive scalar curvature. If $X = Y\times\lbrack -1,1\rbrack$ and $g$ is a Riemannian metric on $X$ with $Sc(X,g)\ge n(n-1)$, then $width(X,g)<\frac{2\pi}{n}$.} By using this, one can  verify Rosenberg's stability conjecture in dimension no greater than $7$.
	
	However, all of the above construction only make sense in product case, that is, the trivial $S^1$ bundle case. In 1983,  B{\'{e}}rard-Bergery studied the geometry of manifolds with $S^1$-equivariant PSC metrics in \cite{ref1}. Among other things, he got a necessary and sufficient condition for the existence of a $S^1$-invariant PSC metric on a compact manifold with free $S^1$-action (See Theorem C of \cite{ref1}).  As a direct corollary, $S^1$ bundles over a closed manifold with a PSC metric must admit a metric of positive scalar curvature. With those facts in mind, it is natural to ask the problems:
		...
		\end{prob}
		Clearly, Problem \ref{mainprob} generalizes the Rosenberg stability conjecture, and its answer may not be always affirmative. To go further, let $B$ be as in  Problem \ref{mainprob},  we call $B$ has the \textsl{twisted stabilized property} if all  $S^1$- bundle over $B$ carries no PSC metric. Therefore, we have the following more specific problems:
		
		\begin{prob}\label{proba}
			Let $B$ be a closed manifold, when does it have twisted stabilized property?
		\end{prob}
  
		In \cite{ref10} and \cite{ref22}, by developing a series of equivariant surgery theorems and equivariant bordism theorems, several existence results of positive scalar curvature on $S^1$ manifolds are established. However, to our knowledge, there are not so many results concerning non-existence result of positive scalar curvature on non-trivial bundle. The following example presents some progress on Problem \ref{mainprob}
  
 \begin{example}\label{eg1}
 $\quad$
 
     (a) (\cite{Zei}, Theorem 1.5) Let $E$ be a $F$-bundle over $B$, where $E$ is spin, $F$ has non-vanishing Rosenberg index and $B$ is a closed aspherical manifold whose fundamental group has finite asymptotic dimension. Then $E$ has non-vanishing Rosenberg index and hence carries no PSC metric.
     
     (b) (\cite{HS06}, Proposition 6.1) Let $E$ be a $F$-bundle over $B$, where $E$ is spin, $F$ and $B$ are enlargeable spin manifold of even dimension. If the short exact sequence
     \begin{align*}
     ...
     (c) Let $E$ be a fiber bundle over $B$ with fiber $F$ and $dimE\le 5$, where both $B$ and $F$ are $K(\pi,1)$ manifolds. Then $E$ admits no $PSC$ metric. In particular, the conclusion is  true for the case that $F = S^1$.

     (d) Let $E$ be a fiber bundle over $B$ with fiber $F$, where both $B$ and $F$ are Cartan-Hadamard manifolds. Then $E$ is enlargeable and hence admit no PSC metric. In particular, the conclusion is  true for the case that $F = S^1$.
     
 \end{example} 

  Among these, (a) and (b) are obtained from the index theory, (c) is a consequence of recent progress of the aspherical conjecture in dimension no greater than 5 in \cite{ref3} and \cite{ref9}, and (d) is mentioned by Gromov in \cite{ref7} (p.658, Example(c)). Indeed, the total spaces in both (c) and (d) are aspherical. Therefore, the answer to Problem \ref{proba} should be positive if the base manifold is Cartan-Hadamard manifold or aspherical manifold of dimension no greater than $4$. However, the following example in (see Example 3.3 in \cite{ref20}) shows that the answer to Problem \ref{mainprob} may not be positive in general cases. Namely, we have
	
	
	\begin{example}\label{eg2}
		Let $E$ be a $S^1$ bundle over the $K_3$ surface with non-divisible Euler class. It's not hard to show $E$ is simply connected, hence carrying positive scalar curvature metric due to \cite{ref5}. However, it's well known that the $K_3$ surface does not carry positive scalar curvature metric by the $\hat{A}$-genus obstruction.
	\end{example}
	
	Example \ref{eg2} illustrates that neither of the property of having non-vanishing Rosenberg index or the property of carrying no PSC metric is stable under constructing $S^1$ bundle over certain manifold. This makes the setting of twisted bundle absolutely different from that of the trivial ones and Problem \ref{mainprob} interesting to study. To state our main result, we introduce a $NPSC^+$ class of manifold, which consists of closed manifold with the property that any manifold with degree 1 map to such manifold admits no PSC metric. Clearly, enlargeable manifold and \textit{SYS} manifold of dimension no greater than 7 and aspherical manifold of dimension no greater than 5 belongs to the class $NPSC^+$ as a result of  \cite{CLL}\cite{ref6}\cite{ref21}. We will show:
	
	
	\begin{theorem}\label{thm2}
		Let $E^n$ be a closed manifold which is a fiber bundle over $B^m$ with fiber $F$ ($m\ge 3$), $B = X\#M$, where $X$ is Cartan-Hadamard or an aspherical manifold when $m=3$, satisfying one of the following properties:
  
  (1) $F$ is $K(\pi,1)$, $n\le 7$.
...
Recall a closed 3-manifold admits no PSC metric if and only if it contains a $K(\pi,1)$ factor in its prime decomposition. Therefore, Theorem \ref{thm2} actually extends the result Example \ref{eg1} (a) (\cite{Zei}) of Zeidler to bundles over all 3-manifold of interest. In addition, Theorem \ref{thm2} implies under certain spin compactible condition, the fiber bundle over enlargeable 3-manifold whose fiber is also enlargeable admits no PSC metric under the incompressible condition, this provides further investigation in the direction as Example\ref{eg1} (b) (\cite{HS06}) by Hanke and Schick. Theorem \ref{thm2} also gives new class of manifolds which does not admit PSC metric obtained by taking \textsl{generalized connected sum} along $K(\pi,1)$ submanifold. As is shown in \cite{ref13}, this does not hold true in general even one takes generalized connected sum along closed curves.\\
	

	
	We next focus on the case of $S^1$ bundle. We say a manifold is 1-enlargeable, if the non-zero degree map in the definition of the enlargeable manifold is required to be of degree 1. We first prove the following general result under a homological nontrivial condition.
	
	\begin{theorem}\label{thm3}
		Let $B$ be a $n$-dimensional closed 1-enlargeable manifold and $E$ a $S^1$ bundle over $B$ ($n+1\le 7$). If the fiber of $E$ is homologically non-trivial, then $E$ carries no PSC metric.
	\end{theorem}

The following Theorem is motivated by \cite{ref6} (Remark 7.49) of Gromov and Lawson and the question in \cite{ref7} (p.661) of Gromov.
...
The condition that the universal covering being hyperspherical may not always be guaranteed by the enlargeability of $B$ as is shown in \cite{BH09}. The incompressible condition may be necessary here, since it seems that compressible fiber would lead to the decrease of the macroscopic dimension (for definition see \cite{Gr96}), which makes the total space hard to be enlargeable. However, for some more concret base manifolds, we can establish the following result under weaker condition of the fiber.
	
	\begin{theorem}\label{thm4}
		Let $B=X\#M$ be a $n$ dimensional closed manifold and $E^{n+1}$ a principle $S^1$ bundle over $B$ ($n+1\le 7$), where $X$ is Cartan-Hadamard or an aspherical manifold when $n=3$. If the $S^1$ fiber of $E$ is homotopically non-trivial, then $E$ admits no PSC metric. Moreover, the conclusion holds true for manifolds admitting a degree 1 map to $E$, and any metric on $E$ with nonnegative scalar curvature is flat.
	\end{theorem}
	
	As a corollary, we obtain the following class of manifolds with \textit{twisted stability property}
	\begin{corollary}\label{cor2}
		Let $B$ be the closed manifold whose universal covering is hyperspherical, and the image of the Hurwitz homomorphism of $\pi_2(B)$ vanishes. Then $B$ has the \textit{twisted stabilized property}. The conclusion also holds true for 3-manifold admiting no positive scalar curvature metric and contains no $S^2\times S^1$ factor in its prime decomposition.
	\end{corollary}
	
	In \cite{ref7}, by developing a series of metric inequality for Riemannian band with positive scalar curvature, Gromov showed for an iso-enlargeable manifold $B$, there exists no uniformly positive scalar curvature metric on  $B\times\mathbb{R}^2$ and noncompact manifolds which admit non-zero degree map to it. It was asked by Gromov in \cite{ref7} whether this can be generalized to non-trivial bundle cases. Our result actually provides examples in this direction and we will discuss it in Sec.5.
	
	Finally, let us say some words on the difficulties and the main idea in the proof of the main theorems. For the case of $S^1$ bundle, a natural idea is to apply the Schoen-Yau descent arguement \cite{ref21} for the homology class with non-zero intersection with the fiber. However, this requires the fiber to be homologically free and which could not be satisfied in most situations (See Sec.4.3 for more detailed discussion). Another difficulty comes from the base space. The base space is always harder to deal with compared with the fibers, since it cannot always be embedded into the total space. As one could see, in Example \ref{eg1}, (a)(c)(d) all focus on the case of aspherical base space. In the case of (d) (The original discussion on this can be found in Sec.4 in \cite{ref24}), the advantage of aspherical condition forces the pullback bundle on the universal covering to be the trivial one and one could argue by directly constructing the hyperspherical map. However, in non-aspherical cases like the connected sum $X\#M$, this arguement would fail since there is no way to guarantee the pullback bundle on any covering space is topologically simple. Inspired by \cite{ref7}, we will overcome this difficulty by showing the manifold is \textit{deep in some sense} or \textit{wide in some direction}. The techniques are based on the depth estimate developed in Sec.2 and band width estimate in \cite{ref7}. By virtue of these, on one hand, we can still show the covering space is geometrically very large; On the other hand, the complicated topological twist of the bundle would not impact any more.

    The rest of the paper is arranged as follows. In Sec.2 we develop a conception called \textit{incompressible depth}. Sec.3 is devoted to the Proof of Theorem \ref{thm2} and Sec.4 is mainly about results concerning $S^1$ bundles. In Sec.5 we study the codimension 2 obstruction of PSC in twisted settings.

    \textbf{Acknowledgement} I am deeply indebted to my advisor Prof. Yuguang Shi for suggesting this problem, constant support and many inspiring conversations. I am grateful to Dr. Jintian Zhu for many helpful discussions and suggestions, to Prof. Yi Xie for sharing his view on the problem from gauge theory. I would like to thank Prof. Yi Liu for kindly answering my questions on the geometrization conjecture. Moreover, I would like to thank Siwei Liang, Xingpei Liu, Yi Sha, Runzhang Wu and Yaoping Xie for helpful discussion.
	
	\section{$\mu$-bubble and incompressible depth in manifold with boundary}
	In this section, we introduce the conception and basic estimate of incompressible depth in manifold with boundary of uniformly positive scalar curvature. The classical incompressible hypersurface obstruction of PSC was first developed in \cite{ref6}\cite{SY1}. Recent years, this has been generalized in various directions by many authors, see \cite{ref2}\cite{CRZ}\cite{Zei}. Our goal is to carry out these result to a depth version by using $\mu$-bubble. First, let us recall several basic conception and results of $\mu$-bubble in \cite{ref23}.
	...
	\begin{definition}(\cite{ref2})\label{def2}
		Define $\mathcal{C}_{deg}$ to be the collection of closed aspherical manifold $M$ satisfyting the following property: any manifold which admits a non-zero degree map to $M$ carries no metric of positive scalar curvature.
	\end{definition}
	...
	\begin{proof}
       ...
       \partial_+V &= d_0^{-1}(T),\quad \partial_-V = d_0^{-1}(-T)
		...
		\end{align*}
		Then following a conformal transformation arguement in \cite{ref21}, $W$ admits a metric of positive scalar curvature. On the other hand, $W$ and $\Sigma$ are homologuous as chain since they are both cobordant to $\partial_-V$ in $V$. Therefore,
		\begin{align*}
			deg(j)|_W = deg(j)|_{\Sigma} = deg(j\circ i) = 1 \ne 0
		\end{align*}
	which contradicts with the fact that $W$ admits a metric of positive scalar curvature.
	\end{proof}
...
\end{proposition}
\begin{proof}
    Let $M_0$ be the manifold obtained by pasting $M$ and $\partial M\times\lbrack 0,\infty)$ along the boundary. Suppose there is a metric $g$ on $M$ with $Sc(M)\ge 1$ and a incompressible surface $\Sigma$ with non-vanishing Rosenberg index, such that $d(\Sigma,\partial M)>\pi$. Then $g$ can be extended to a metric $g^+$ on $M_0$, which equals a product metric at infinity, hence the scalar curvature of $g^+$ is bounded from below due to the compactness of $M$. However, by \cite{Zei20} (Theorem 1.4), we have $\inf_{x\in M_0}Sc(g^+) = -\infty$, which leads to a contradiction.
\end{proof}
	
	
	\section{PSC obstructions on fiber bundles: Proof of Theorem \ref{thm2}}
	
	In this section, we focus on the proof of Theorem \ref{thm2}. Recall it was first proposed in \cite{ref31} by Gromov the philosophy that Riemannian manifolds large in some sense does not carry metric of positive scalar curvature. However, there're still quite limited ways in establishing the connection between the largeness of Riemannian manifolds and nonexistence of PSC metric, even today. Inspired by \cite{ref7}, our proof of Theorem \ref{thm2} is based on arguing that the manifold is \textit{deep} in some sense. We divide the proof into two parts, the first part dealing with the nonexistence of positive scalar curvature on $E$ and the second part dealing with the rigidity in the setting of nonnegative scalar curvature.
...
\subsection{The nonexistence part}
  ...
  Consider the following diagram:
		...
		\begin{align*}
			\mu = \tan(\frac{1}{2}d)
		\end{align*}
	We can then solve a stable $\mu$-bubble $\Omega$ as in (2.1), with $\partial\Omega = \Sigma$. By the same arguement as in the proof of Theorem \ref{thm6}, $S$ carries a positive scalar curvature metric. On the other hand, we consider the map:
		\begin{align*}
	...
	\begin{proof}[Proof of Theorem \ref{thm2} (Nonexistence part)]
		
		Throughout our proof, $m$ is always used to denote the dimension of the base space and $n$ the dimension of the total space of the fiber bundle. We first focus on the case that $X$ is a Cartan-Hadamard manifold. Fix a metric on $B = X\#M $, $E$ a fiber bundle over $B$ with an incompressible fiber $F$ satisfying one of (1)(2)(3) in the theorem. We show $E^n$ admits no metric of positive scalar curvature. Or else, we assume there exists a metric $E$ on $B$ with $Sc(E) \ge 1$ due to the compactness of $E$. 
		
		Let $k:B\longrightarrow X$ be a smooth map which collapses $M$ to a point with $Lip(k)\le 1$. Denote the universal Riemannian covering of $X$ by $\Tilde{X}$, this induces a Riemannian covering $\Tilde{B}$ of $B$, such the following diagram commutes:
...
where $\Tilde{E}$ is the pullback bundle of $E$ under the map $\pi$. Obviously, $\Tilde{E}$ is a covering space of $E$ with incompressible fiber $F$. In fact, if there is an element in $\pi_1(F)$ homotopic to zero in $\Tilde{E}$, then under the covering map $\Tilde{\pi}$, this would give an element in $\pi_1(F)$ homotopic to zero in $E$, which contradicts the fact that $F$ is incompressible in $E$.
		...
		We then start on the depth analysis of the bands. We first construct a torical band $\mathcal{V}$ in $\Tilde{X}$, $\mathcal{V}\cong T^{m-1}\times \lbrack 0,1\rbrack$, with:
		\begin{equation}\label{35}
			\begin{split}
				&\partial_-\mathcal{V} = T^{m-1}\times \lbrace 0\rbrace\\
				&\partial_0\mathcal{V} = T^{m-1}\times \lbrace \frac{1}{2}\rbrace\\
				&\partial_+\mathcal{V} = T^{m-1}\times \lbrace 1\rbrace\\
				&d(\partial_-\mathcal{V},\partial_0\mathcal{V}) > L\\
				&d(\partial_+\mathcal{V},\partial_0\mathcal{V}) > L\\
			\end{split}
		\end{equation}
		for arbitrary large $L$. In fact, since $\Tilde{X}$ is a simply connected Cartan-Hadamard manifold, there is a map $f$ which maps the geodesic ball $B_R(0)$ diffeomorphicly to the unit $D^n$ with flat metric, with $Lip(f)\le \frac{1}{R}$ (though the metric on $\Tilde{X}$ may not be of non-positive sectional curvature, it's $C^0$ equivalent to such a metric). It's easy to choose a torical band $V$ in $D^n$, such that
		\begin{align*}
			d(\partial_-V,\partial_0V) > \delta,d(\partial_+{V},\partial_0{V}) > \delta
		\end{align*}
		in $D^n$, where $\delta$ is a fixed small number only rely on dimension $n$. Therefore, the pullback band of $V$ under $f$, denoted by $\mathcal{V}$, satisfies the desired property as long as we choose $R$ arbitrarily large.
...
We next make the construction in $\Tilde{B}$. The construction in (\ref{31}) indicates that $\Tilde{B}$ is the connected sum of $\Tilde{X}$ and infinite copies of $M$:
		...
		\end{align*}
		where $\#_{p_i}$ means taking connected sum at a small neighborhood near $p_i\in\Tilde{X}$. The map $\Bar{k}:\Tilde{B}\longrightarrow \Tilde{X}$ in (\ref{31}) pinches these $M$'s to the points $p_1,p_2,p_3,\dots$. By applying small perturbation on the hypersurfaces $\partial_-\mathcal{V},\partial_0\mathcal{V},\partial_+\mathcal{V}$, one can assume none of $p_i$ is on $\partial_-\mathcal{V},\partial_0\mathcal{V}$, or $\partial_+\mathcal{V}$. We then construct the overtorical band $\mathcal{U}$ in $\Tilde{B}$ by simply setting $\mathcal{U} = \Bar{k}^{-1}\mathcal{V}$. Also define:
		\begin{align*}
			\partial_-\mathcal{U} = \Bar{k}^{-1}\partial_-\mathcal{V}\\
			\partial_0\mathcal{U} = \Bar{k}^{-1}\partial_0\mathcal{V}\\
			\partial_+\mathcal{U} = \Bar{k}^{-1}\partial_+\mathcal{V}\\
		\end{align*}
		Since $\partial_-\mathcal{V},\partial_0\mathcal{V},\partial_+\mathcal{V}$ has no intersection with $p_i$'s, $\partial_-\mathcal{U},\partial_0\mathcal{U},\partial_+\mathcal{U}$ are mapped diffeomorphically to $\partial_-\mathcal{V},\partial_0\mathcal{V},\partial_+\mathcal{V}$ under $\Bar{k}$.  This guarentees $\partial_-\mathcal{U},\partial_0\mathcal{U},\partial_+\mathcal{U}$ are torus. Topologically, $\mathcal{U}$ is the connected sum of a torical band $T^{m-1}\times\lbrack 0,1 \rbrack$ and finite copies of $M$.    
		...
		\begin{align*}
			d(\partial_-\mathcal{U},\partial_0\mathcal{U}) > L,d(\partial_+\mathcal{U},\partial_0\mathcal{U}) > L
		\end{align*} 
		by (\ref{35}).
		
		Now we can construct the band in $\Tilde{E}$. Define $\mathcal{W}$ is the restriction of $\Tilde{E}$ on $\mathcal{U}$. Since $Lip(q) = Lip(p) = C<+\infty$ by (\ref{34}), we have:
		\begin{align*}
			d(\partial_-\mathcal{W},\partial_0\mathcal{W}) > \frac{L}{C},d(\partial_+\mathcal{W},\partial_0\mathcal{W}) > \frac{L}{C}
		\end{align*}
		where $\partial_-\mathcal{W},\partial_0\mathcal{W},\partial_+\mathcal{W}$ are the restricted bundle of $\mathcal{W}$ over $\partial_-\mathcal{U},\partial_0\mathcal{U},\partial_+\mathcal{U}$. We next show $\partial_0\mathcal{W}$ is an incompressible hypersurface in $\mathcal{W}$. We have the following commutative diagram:
		...
		Now we have constructed an incompressible hypersurface $\partial_0\mathcal{W}$ in the band $\mathcal{W}$, the distance of which to either boundary of $\mathcal{W}$ is no less than $\frac{L}{C}$. Also notice that $\partial_0\mathcal{W}$ is a $F$ bundle over $\partial_0\mathcal{U}\cong T^{m-1}$. We then discuss the cases in the theorem one by one
  
  (1) $F$ is $K(\pi,1)$, $n\le 7$. By Lemma \ref{lem5}, $\partial_0\mathcal{W}$ belongs to the class $\mathcal{C}_{deg}$ in Definition \ref{def3}. In the sense of the terminology in Sec.2, this means $\mathcal{W}$ has $\mathcal{C}_{deg}$ incompressible depth no less than $\frac{L}{C}$. Since the scalar curvature of $\mathcal{W}$ is no less than $1$ and $L$ can be taken arbitrarily large, we obtain a contradiction due to the $\mathcal{C}_{deg}$ incompressible depth estimate of Proposition \ref{thm6}. 
  
  (2) $F$ is in the class $NPSC^+$, $E$ is spin or $F$ is totally non-spin, $n\le 7$. We need only focus on the case that $n\ge 6$, since $m\ge 3$ and the condition $n\le 5$ would imply that the fiber is $K(\pi,1)$. If $F\in NPSC^+$, by Lemma \ref{lem5}, $\partial_0\mathcal{W}$ admits no PSC metric. If $F$ is totally non-spin, then by passing to the pullback bundle of the covering map $\mathbb{R}^{m-1}\longrightarrow T^{m-1}$ one could see $\partial_0\mathcal{W}$ is also totally non-spin. A contradiction then follows from the $\mathcal{N}$ and $\mathcal{N}_{nspin}$ incompressible depth estimate Proposition \ref{pro2} and Proposition \ref{pro4}.

  (3) $F$ is spin and has non-vanishing Rosenberg index, $E$ is spin. It follows from \cite{Zei} (Theorem 1.5) that as the $F$ bundle over $T^{m-1}$ $\partial_0\mathcal{W}$ also has non-vanishing Rosenberg index, and the contradiction follows from the $\mathcal{R}^{\ne 0}$ incompressible depth estimate Proposition \ref{pro3}.
  
  Finally, we deal with the case that $X$ is a $K(\pi,1)$ 3-manifold. Let's recall the hyperbolisation conjecture, a corollary of the geometrization conjecture (see \cite{MG}), which says a 3-dimensional closed aspherical manifold is either hyperbolic or contains a $\mathbb{Z}\oplus\mathbb{Z}$ subgroup in its fundamental group(see \cite{GT} 12.9.4). By using this we can assume $\pi_1(X)$ contains a subgroup isomorphic to $\mathbb{Z}\oplus\mathbb{Z}$, since the hyperbolic case is already contained in the Cartan-Hadamard case. It follows $X$ contains a incompressible torus, so does $B=X\#M$. By the similar diagram chase as (\ref{chase}), $E$ contains a nice incompressible hypersurface. It follows from \cite{ref2} (Theorem 1.1), \cite{CRZ} (Theorem 1.5) and \cite{Zei} (Theorem 1.7) that $E$ admits no PSC metric.
  \end{proof}
 ...
 \subsection{The rigidity part}
	...
	Call two metrics $g_1$ and $g_2$ equivalent if there exists constant $C$ such that:
	\begin{align*}
	...
	The following lemma gives an estimate to the lower bound of volume rate of $\Tilde{B}$ appeared in the first part of the proof of Theorem \ref{thm2}. To some extent this lemma shows the largeness of $\Tilde{B}$ in volume sense (compared with \cite{ref31}).
	\begin{lemma}\label{lem9}
		Let $B^m=X\#M$ be the manifold in Theorem \ref{thm2} and $\Tilde{B}$ the covering of $B$ appeared in the first part of the proof of Theorem \ref{thm2}. Then $r(\Tilde{B})\ge m$
	\end{lemma}
	\begin{proof}
		...
		\begin{align*}
			\frac{d_S(x,y)}{d_3(x,y)}\le \frac{\pi}{2},\quad for \quad x,y\in S
		\end{align*}   
		...
		\end{split}
            \end{equation}
		By (\ref{15.5}), for sufficiently large $R$, it's not hard to find a hypersurface $\mathcal{H}$ in $\Tilde{X}_0$, satisfying:\\
		(i) In both $\Tilde{X}$ and $\Tilde{B}$, $\mathcal{H}$ separates the space into two connected components.\\
		(ii) In $\Tilde{X}$, the region bounded by $\mathcal{H}$ contains $B(a,R)$.\\
		(iii) In $\Tilde{B}$, the region bounded by $\mathcal{H}$ is contained in $\Tilde{B}(a,2CR)$.\\
		Therefore,
		\begin{align*}
	...
	\begin{lemma}\label{lem10}
		Let $\Tilde{E}$ be the covering of $E$ in the first part of the proof of Theorem \ref{thm2}. Then $r(\Tilde{E})\ge m$, $m$ the dimension of $B$.
	\end{lemma}
	\begin{proof}
            ...
            \textit{Claim. There exists a constant $C$ relying only on the $C^0$ geometry of $B$ and $E$, such that for any $x,y\in \Tilde{B}$ and $z\in q^{-1}(x)$, one can always pick $w\in q^{-1}(y)$ such that the following inequality holds:}
            ...
            \begin{equation}
                \begin{split}
                    Vol(B(\Tilde{a},CR+D))\ge N(\Tilde{E},\Tilde{a},CR+D)\ge N(\Tilde{B},a,R)\ge Vol(a,R)\ge cR^m
                \end{split}
            \end{equation}
	...
	\begin{proof}[Proof of Theorem \ref{thm2} (Rigidity part)]
 ...
 \section{PSC obstructions on $S^1$ bundles}
	We study the positive scalar curvature obstruction on $S^1$ bundles in this section.
 
 \subsection{$S^1$ bundle and \textit{SYS} bands}
 We focus on the proof of Theorem \ref{thm3} and Theorem \ref{thm4} in this subsection. Instead of the depth estimate used to derive Theorem \ref{thm2} in the preceding section, our tools in this subsection are based on Gromov's width estimate for \textit{SYS} bands in \cite{ref7}. By doing this we could show the manifold is \textit{wide in some direction}, which gives desired $PSC$ nonexistence.
    ...
    With these preperation, The proof of Theorem \ref{thm3} is near at hand. In fact, we will prove the following Theorem in a more general form:
    \begin{theorem}\label{thm3'}
...
\begin{proof}[Proof of Theorem \ref{thm3}]
    Let the map in Theorem \ref{thm3'} be the projection map $\pi:E\longrightarrow B$, and the conclusion is obvious due to Theorem \ref{thm3'}.
\end{proof}
	...
	\begin{lemma}(The structure lemma)\label{lem11}
		Let $M$ and $N$ be $n$-dimensional closed manifold and $E$ a $S^1$ principle bundle over $B = M\#N$, Then $E$ is the generalized connected sum of $E_M$ and $E_N$ along the fiber, where $E_M$ and $E_N$ are $S^1$ bundle over $M$ and $N$.
	\end{lemma}
	\begin{proof}
		We write $B=M\#N = M\backslash D^n \sqcup_{S^{n-1}} N\backslash D^n$, and denote the $(n-1)$-sphere connecting two sides of the connected sum by $S$. We first show the restriction of $E$ on $S$ is trivial.
		
		If $n\le 4$, by (3.1) we the triviality of the restricted bundle simply follows from the fact that $H^2(S) = H^2(S^{n-1}) = 0$. We then focus on the case $n = 3$. Recall the isomorphism class of $E$ is completed determined by the homotopy class of the classifying map (c.f.\cite{ref12},P379):
		\begin{align*}
			f:B\longrightarrow \mathbb{C}\mathbb{P}^\infty
		\end{align*}
		By the cellular approximation theorem, $f$ is homotopic to a map $g$ which maps into the 3-skeleton of $\mathbb{C}\mathbb{P}^\infty$, which is simply $\mathbb{C}\mathbb{P}^1 = S^2$. Consider the restriction of $g$ on $S$, Notice that $S$ is the boundary of $M\backslash D^n$ and $g|_S$ has an extension to $M\backslash D^n$ by $g|_{M\backslash D^n}$, we have $deg(g|_S) = 0$. By the Hopf's degree theorem, $g|_S$ is homotopic to zero, which shows the restriction of $E$ on $S$ is trivial.
		
		Lastly, Construct $E_M$ by filling the boundary of the bundle restricted on $M\backslash D^n$ by the trivial bundle over $D^n$ and construct $E_N$ in the similar fashion. This yields the structure of the generalized connected sum.
	\end{proof}
	
	We can now start the proof of Theorem \ref{thm4}, we would deal with the case that $X$ is a 3-dimensional aspherical manifold first. The method to handle Cartan-Hadamard manifold of higher dimension is similar. For $X$ a 3 dimensional aspherical manifold, we work on the covering space $\Tilde{X}$ homotopic equivalent to $S^1$. We start with a lemma concerning basic geometric property of spaces of such form.
	
	\begin{lemma}\label{lem3-1}
		Let $X$ be a 3-dimensional orientable manifold homotopic equivalent to $S^1$, $\gamma$ a curve representing the generator of the fundamental group, $V$ a small tubular neighborhood of $\gamma$. Then there is a map $j:X\backslash V\longrightarrow \partial V = \Sigma_0$, such that $j\circ i$ is of degree $1$, where $i$ is the inclusion map of $V$ in $X$.
	\end{lemma}
	\begin{proof}
		We follow the arguement of \cite{ref6} (Theorem 7.47). By the Whitehead's Theorem the inclusion map $\gamma\longrightarrow X$ induces homotopic equivalence, and there is a map $h:X\longrightarrow \gamma$ being the homotopy inverse. Without loss of generality we can assume the normal bundle of $\gamma$ is trivial for we can pass to two sheeted covering if necessary. We will lift the restriction of $h$ on $X\backslash V$ to the desired map $j$, such that the restriction of $j$ on $\partial V = \Sigma_0$ is of degree 1. First, it's not hard to lift the restriction of $h$ on $\partial V$ to a degree 1 map to $\partial V$. Next, since $\pi_1(S^1) = \mathbb{Z}$, the only obstruction of the lifting lies in $H^2(X\backslash \gamma, \partial V, \mathbb{Z})$. On the other hand, we have $H^2(X\backslash \gamma, \partial V, \mathbb{Z})=0$ by exicision lemma. Hence, we obtain the desired lifting. 
	\end{proof}
...
\end{align}
which is an overtorical band of width $R$ by the existence of $j$ in the preceding lemma. The covering $\Tilde{X}\longrightarrow X$ gives rise to a covering $\Tilde{B}\longrightarrow B$:
\begin{align}\label{31'}
			\begin{CD}
				\Tilde{B}     @>\pi>>  B\\
				@VV\Bar{k}V        @VVkV\\
...
\begin{align}\label{40}
		...
		\end{align*}
		It follows that the restriction of $\Tilde{E}$ on each $S_i$ is trivial ($i = 1,2,3,\dots$). Then one removes the bundle over the infinite copies of $M$ in $\Tilde{E}$, and fill the trivial bundle over $S_i$ by $D^n\times S^1$. This gives a $S^1$ bundle over $\Tilde{X}$, which is trivial since $H^2(\Tilde{X})=H^2(S^1) = 0$. Hence, topologically, $\Tilde{E}$ is the generalized connected sum of $\Tilde{X}\times S^1$ and infinite copies of a $S^1$ bundle over $M$ along the fiber:
		\begin{align}\label{42}
			\Tilde{E} = \Tilde{X}\times S^1\#_{S^1}Z\#_{S^1}Z\#\dots
		\end{align}
		where $Z$ is some $S^1$ bundle over $M$.

Next we construct $\mathcal{U} = \Bar{k}^{-1}(\mathcal{V})$, by taking small perturbation we assume none of $p_i$ in (\ref{40}) is in $\partial\mathcal{V}$, and hence we have
...
\end{align}
where $P$ is the $S^1$ principle over $\mathcal{V}$, which must be trivial since it is induced from the trivial $S^1$ bundle over $\Tilde{X}$. Thus we have $P = \mathcal{V}\times S^1$. The notation $\#_{p_i\times S^1}$ here means taking generalized connected sum at $p_i\times S^1\subset \mathcal{V}\times S^1 = P$.
...
\begin{proof}
    Since $U$ is a subregion of $S^2$ it must be of the shape in Figure \ref{f1}. $\partial U = \gamma_1\cup\gamma_2\cup\dots\cup\gamma_r$, each $\gamma_i$ mapped to $L$ by $f$, and represents $k$ multiple of the generator as shown before. Suppose $\gamma_i$ represent $ka_i$ times generator in $\pi_1(L) = \pi_1(S^2\times S^1)$. Denote a standard curve $\beta = \lbrace 0 \rbrace\times S^1\in L$ with base point $p$, then $\gamma_i$ can be deformed to $ka_i\beta$ without changing the homology class of the image of $U$, thus we can assume $\gamma_i$ is exactly $ka_i\beta$. By means of $U$, $\theta = \lbrack\gamma_1\rbrack+\lbrack\gamma_2\rbrack+\dots +\lbrack\gamma_r\rbrack = 0\in H_1(A_1)$. Since $A_1 = (\mathcal{V}-D^3)\times S^1$ and $\gamma_i\in L\subset\partial A_1$, $\theta$ must be in the image of
    \begin{align*}
    ...
    \end{align*}
    If $a_1 = 0$, fill it by a disk in $L$ and $r$ minus $1$, without changing the homology class of $U$. If $a_1\ne 0$, we assume $a_2$ satisfies that $a_1a_2<0$. We pick a segment in $\gamma_2$ which corresponds to $-ka_1$ times $\beta$ (this can always be done since $|a_2|\ge|a_1|$), and join two ends of this segment by a curve $\Gamma$ (as shown in figure \ref{f1} by the dotted line). Then the region enclosed by $\gamma_1$, $-ka_1\beta$ and $\Gamma$ is a cylinder. This shows $-\gamma_1$ and the composition of $-ka_1$ and $\Gamma$ are freely homotopic. That is,
    \begin{align*}
        \lbrack -ka_1\beta\rbrack*\lbrack\Gamma\rbrack = \lbrack-\gamma_1\rbrack = \lbrack -ka_1\beta\rbrack \in\lbrack S^1, A_1\rbrack
    \end{align*}
    Thus as the elements of $\pi_1(A_1)$, $\lbrack -ka_1\beta\rbrack*\lbrack\Gamma\rbrack$ and $\lbrack -ka_1\beta\rbrack$ are conjugate
...
\begin{remark}
		Recall it was defined in \cite{ref7} by Gromov the class of \textit{SYS}-Enlargeable manifolds which includes the manifolds with infinite \textit{SYS} width. In the preceding two theorems, we actually produce new classes of \textit{SYS}-Enlargeable manifolds.
	\end{remark}
\begin{remark}
...
In this subsection, we present a proof of Theorem \ref{thm3.5} and Corollary \ref{cor2}
...
\begin{proof}[Proof of Corollary \ref{cor2}]
    ...
    \end{align*}
    vanishes, and the fiber must be incompressible in $E$. Hence, the conclusion follows directly from Theorem \ref{thm3.5}. If $B$ is of dimension 3 and contains no $S^2\times S^1$ factor, then each prime factor of $B$ has trivial $\pi_2$. The incompressibility of the fiber follows from Lemma \ref{lem11} and Lemma \ref{A2}, and the conclusion that $E$ admits no PSC metric follows from Theorem \ref{thm4}.
\end{proof}


 \subsection{Examples}
 To give a better interpretation of how the topological non-trivial condition of the $S^1$ fiber affect the geometry of the bundle, we would like to include some examples here. We begin with the following topological claim:\\

 \textit{Claim. Let $B^n$ be a closed manifold with $H^2(B,\mathbb{Z})$ free, $\pi:E\longrightarrow B$ a $S^1$ principle bundle over $B$ with Euler class $e$. Let $\tau$ be the homology class represented by the fiber in $H_1(E)$, then\\
 ...
 Even for the simplest $T^2$, the $S^1$ bundle over it with non-divisible Euler class has zero-homologuous fiber. However, it's always incompressible since $\pi_2(T^2)=0$. For similar example one could also consider $T^n\#T^n$, which has the twisted stablized property by Theorem \ref{cor2}. Additionally, one could see that the Schoen-Yau descent arguement can hardly apply in these situation since it requires homologically free fiber, and this is the case only when the bundle is trivial.

There also exists the case that the fiber is compressible but homologically nontrivial. In fact, the $S^1$ bundle over $T^4\#\mathbb{C}\mathbb{P}^2$, whose restriction on $T^4$ is trivial while on $\mathbb{C}\mathbb{P}^2$ is like $S^1\longrightarrow S^5\slash\mathbb{Z}_p\longrightarrow \mathbb{C}\mathbb{P}^2$ is such an example, and it admits no PSC metric by Theorem \ref{thm3}. Moreover, if restriction of the bundle on $T^4$ has non-divisible Euler class, then the fiber is forced to be zero-homologuous, but still homotopically non-trivial provided $p>1$. Then Theorem \ref{thm4} is the only result to be applied under this situation.
 
 We conclude this section by providing an example that even the homological nontrivial condition for $S^1$ fiber is not always a sufficient condition to rule out positive scalar curvature metric on the fiber bundle.
	\begin{example}
  ...
  \section{Codim 2 PSC obstructions in twisted settings}
	As an application of our result, we give an partial extension of the quadratic decay theorems in \cite{ref7} raised by Gromov to non-trivial bundle case. We say a closed manifold $B$ have \textit{dominated twisted stabilized property} if any manifold which admits a degree 1 map to certain $S^1$ bundle over $B$ carries no positive scalar curvature metrics. It follows that manifolds in Corollary \ref{cor2} have \textit{dominated twisted stabilized property}.
	...
	\end{proposition}
\begin{proof}
    Let $V$ be a small tubular neighbourhood of the zero section of $X$, then $\partial V$ is a principle $S^1$ bundle over $B$. Therefore, any closed manifold with degree 1 map to $\partial V$ admits no metrics of positive scalar curvature. Moreover, $X\backslash V$ is diffeomorphic to $\partial V\times\lbrack 0,+\infty)$, let
    \begin{align}\label{2}
    ...
    \end{align}
    Since $\partial V$ has dominated twisted stablized property, $\Sigma$ does not admit metric of positive scalar curvature.

 
    By a standard $\mu$-bubble arguement, the width estimate holds for the band $U_R$. That is, if the band $U_R$ has scalar curvature no smaller than $\sigma$, then it has width no greater than $\frac{2\pi}{\sqrt{\sigma}}$. Since the width of $U_R$ is at least $R$, we obtain:
    \begin{align}\label{4}
...
With the benifit of positive scalar curvature obstruction on non-trivial bundle, we can also prove the following result:
...
\end{proposition}
\begin{proof}
    We follow the arguement of \cite{ref6} (Theorem 7.47). Let $r:X\longrightarrow B$ be the retracting map, $V$ the tubular neighbourhood of $B$, then $\partial V$ is diffeomorphic to the $S^1$ principle bundle over $B$. First, the restriction of $r$ on $\partial V$ is not hard to lift to a degree 1 map from $\partial V$ to $\partial V$. Since $B$ is the deformation retract of $X$ we have
    \begin{align*}
        H^2(X\backslash V, \partial V, \mathbb{Z}) = H^2(X,V,\mathbb{Z}) = H^2(X,B,\mathbb{Z}) = 0
    \end{align*}
    Therefore, there is no obstruction to lift $r$ to a map $\tilde{r}: X\backslash V\longrightarrow\partial V$, such that the restriction of $\tilde{r}$ on $\partial V$ is the lifted map stated above. This shows any hypersurface seperating $\partial V$ and the infinity admits a degree 1 map to $\partial V$, and the conclusion follows from a similar arguement as in the proof of Proposition \ref{thm9}.
\end{proof}
	...
	In the appendix, we prove several topological and algebraic facts which are used in the paper.
...
\begin{proof}
    ...
    \begin{split}
        ...
        \end{split}
    \end{equation}
    where $J = \partial(I\times I)\backslash\lbrack 0,1\rbrack\times\lbrace 1\rbrace$, $K=\lbrack 0,1\rbrack\times\lbrace 1\rbrace$. Then $\Tilde{F}|K: K\longrightarrow \pi^{-1}(b_0)=S^1$ gives an element in $\pi_1(S^1)$, this corresponds the image of $F$ under $\partial$ (\cite{ref11}, Theorem 4.41). To compute this element, we first view $F$ as the map to $\Tilde{E}$, the $\mathbb{R}^2$ vector bundle which induces $E$. $B$ can then be viewed as the zero section of $\Tilde{E}$. Since $\Tilde{E}$ and $E$ are homotopy equivalent, we can lift $F$ to a map to $\Tilde{E}$ such that the boundary of $I\times I$ is maped to $x_0$. Perturb $F$ such that it keeps stationary on $\partial(I\times I)$ and intersect transversally with $B$ at points $p_1,p_2,\dots,p_k$. One can assume $p_i$ being sufficiently close to $K$. In fact, for a point $p_i$, we can join it to a small neibourhood of $K$ by a curve $c_i$, such that $c_i$ only intersect $B$ at $p_i$. It's not hard to imagine $p_i$ can be straped to the neibourhood of $K$ along the curve $c_i$. Then we can remove the small disk around $p_i$ and construct a lifting of $F$ to $\Tilde{E}$ which does not touch the zero section. This gives a desired lifting as in (A.2). Since there is a transversally intersecting relation, each intersecting point contribute once in the process of winding the $S^1$ fiber over $b_0$. From this fact, one can conclude the element in $\pi_1(S^1)$ is given by the oriented intersection number of $F$ and the zero section, and the final conclusion follows with a use of Poincare duality.
    
\end{proof}
 

	\begin{lemma}\label{A2}
		Let $M,N$ be $n$-manifold, possibly non-compact, $\alpha,\beta$ are two curves in $M,N$, both have infinite order in $\pi_1(M)$ and $\pi_1(N)$. then $\alpha$ and $\beta$ have infinite order in the fundamental group of the generalized connected sum of $M,N$ respect to $\alpha$ and $\beta$.
	\end{lemma}
	\begin{proof}
		Let $Y$ be the total space of the generalized connected sum, $Y = A\cup B $, where $A$ is obtained by removing a tubular neighborhood of $\alpha$ in $M$ and $B$ the counterpart in $N$. Denote $S = S^{n-2}\times S^1$ to be the hypersurface connecting $A$ and $B$ in $Y$. choose a curve $\gamma$ in $S$, such that $\gamma$ is contractible in $Y$. We will show $\gamma$ is contractible in $S$.
		
		In fact, since $\gamma$ is contractible in $Y$, there is a map $f:D^2\longrightarrow Y$, such that $\partial D^2$ maps to $\gamma$. Since $S$ is two sided, we can move $\gamma$ a little bit outside $S$ such that they intersect nowhere. Then by the transversality theorem one can construct a map $g$ homotopic to $f$, such that $g$ and $f$ coincide on $\partial D^2$, and $g$ intersects transversely with $S$. Then $g^{-1}(S)$ is constituted by disjoint closed curves, that is:
		\begin{align*}
			g^{-1}(S) = \gamma_1\cup\gamma_2\cup\dots\gamma_m
		\end{align*}
		There exists a innermost curve $\gamma_k$ which bounds a disk $D$ in $D^2$, the image of $D$ under $g$ lying in either $A$ or $B$. We assume it lies in $A$ without loss of generality. Since $\alpha$ is of infinite order in $A$, it's not hard to see the map $\pi_1(S)\longrightarrow \pi_1(A)$ is injective. Therefore, $\gamma_k$ contracts to a point in $S$. Then we modify the value of $g$ on $D$ by a map from $D$ to $S$. By a small deformation near $S$, the image of a small neighbourhood of $D$ under $g$ can be moved into $B$. Therefore, we obtain a new map $g':D^2\longrightarrow Y$, the boundary of $D^2$ mapped to $\gamma$ and the number of closed curves in $g'^{-1}(S)$ minors 1. By finite step of this operation, one deduces $\gamma$ is homotopic to zero in  $S$, which finishes the proof of the lemma.
	\end{proof}
 ...
 \end{proof}
	...
	\begin{thebibliography}{100}
		...
		\bibitem{ref20}B. Botvinnik, J. Rosenberg, Positive Scalar Curvature on $Pin^{\pm}-$
		and $Spin^c-$ Manifolds
		and Manifolds with Singularities, in Perspectives in scalar curvature (2022).
  \bibitem{BH09} M. Brunnbauer, B. Hanke, Large and small group homology , J.
Topology \textbf{3} (2010), 463-486.
\bibitem{GT} B. Martelli, An Introduction to Geometric Topology, version 2 (2022).
		...
		\bibitem{CLL} O. Chodosh, C. Li, Y.Liokumovich, Classifying sufficiently connected psc manifolds in 4 and 5
		dimensions (2021). arXiv:2105.07306, to appear in Geom. Topol.
		...
		\bibitem{FW}Arthur E. Fischer and Joseph A. Wolf, The Calabi construction for compact Ricci
		...
		\bibitem{ref31}M. Gromov, Large Riemannian manifolds, Curvature and topology of Riemannian manifolds (Katata, 1985),
		Lecture Notes in Math., vol. 1201, Springer, 1986, pp. 108-121.
...
curvature, Ann. Inst. Fourier (Grenoble) \textbf{65} 
 (2015), 2681–2710.
		...
		\bibitem{ref13}P. Liu, Y. Shi, J. Zhu, Positive mass theorems of ALF and ALG manifolds, arXiv:2103.11289 (2021).
		\bibitem{ref15}J. Milnor, J. D. Stasheff. Characteristic Classes. Princeton University Press, Princeton (1974).
