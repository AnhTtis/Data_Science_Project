...
%Below, we list the classifier-free guidance weights used in the main results --- note that these vary for different requested edits (as they do in InstructPix2Pix~\cite{brooks2022instructpix2pix}).
...
Our process of optimizing for an edited NeRF uses a diffusion model as guidance, which can produce a collection of temporally varying images (i.e., varying over the course of optimization). As a result, the optimization process does not have a single convergence point, as standard NeRF optimization does, where all images are sufficiently well explained by the reconstructed model. Therefore, the edited NeRF also varies in type and strength of edit over the course of optimization, and one must select an iteration at which to terminate optimization and visualize the edited scene. The results in the paper are shown after a varying number of iterations, $3000-4000$ for smaller scenes (e.g., Fig.~1 and Fig.~4~bear, which both have under 100 images each), and $7000-8000$ for larger scenes (the remaining scenes, with over 200-300 images each).
...
Furthermore, over much longer optimization runs (i.e., when the process of rendering, editing, and propagating the edited images back into the NeRF has been repeated many times), we note a decrease in visual quality. We also largely attribute this to the effects of the autoencoder. %Figure~\ref{fig:drift} shows another experiment, in which the images of the scene are repeatedly rendered and autoencoded (effectively our proposed method, but without the diffusion process). We find that this similarly degrades in quality over longer optimization processes. 


\section{Metrics}
%More detailed description of the two metrics (CLIP direction and CLIP consistency).
...
%We demonstrate that our method can achieve alignment on par with editing each frame independently while also having the strongest consistency in our renders.


%\section{One-time dataset update}
%From the section called “One-time dataset update”: This problem is even more prominent when contextual objects are added to the portraits; please refer to the supplemental for more details.
