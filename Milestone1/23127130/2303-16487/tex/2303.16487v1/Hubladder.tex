...
\title{Reexamining the doped two-legged Hubbard ladders}
...
\begin{figure}[t]
    \includegraphics[width=0.45\textwidth]{one_eighth_doping/ElecOneeighthL192.pdf}
...
We notice that $K_\rho$ in Tab.~\ref{Summary} are consistent with the recently results obtained from infinite system matrix product states \cite{2023arXiv230300663E}.


\begin{table}[t]
    \caption{The dependence of the extracted parameters $K_\rho$ and $n_0$ (using Eq.~\eqref{charge_oscillation}) on the range of sites used ($\delta=1/8$, $L_x$ = 192).}
    \includegraphics[width=0.3\textwidth]{one_eighth_doping/ElecOscillationFit_oneeighthL192.pdf}
...
We focus on ladders with width $L_y \equiv 2$ and length $L_x$, so there are totally $N=L_x \times L_y$ lattice sites. The average hole concentration away from half-filling is defined as $\delta = N_h/N$ with $N_h=\sum_i(1-\hat{n}_i)$. We apply a one-site pinning field (an addition term $h_m\cdot \hat{S}_{0z}$  with $h_m = 0.5$ is added to the Hamiltonian in Eq.~(\ref{Ham})) at the left edge of the ladder, which allows us to detect the local magnetization $\langle \hat{S}_{i}^z \rangle$ instead of the more demanding correlation functions. We also measure the $d$-wave pair-pair correlation function defined as $D(r) = \langle \hat{\Delta}_i^{\dagger}\hat{\Delta}_{i+r}\rangle$ with $\hat{\Delta}_i^{\dagger}=\hat{c}_{(i,1),\uparrow}^{\dagger}\hat{c}_{(i,2),\downarrow}^{\dagger}-\hat{c}_{(i,1),\downarrow}^{\dagger}\hat{c}_{(i,2),\uparrow}^{\dagger}$ creating a singlet on the rungs \cite{PhysRevB.92.195139,shen2022comparative}. Other physical observables are defined wherein they are firstly mentioned.
...
\begin{figure}[t]
    \includegraphics[width=0.4\textwidth]{one_eighth_doping/PairOneeighth_ref49.pdf}
    \includegraphics[width=0.4\textwidth]{one_eighth_doping/KscOneeighth.pdf}
    \caption{Pair-pair correlation and the extracted exponents at $1/8$ doping. (a) Pair-pair correlation function $D(r)$ for different numbers of kept states $m$ and the extrapolated result. The length of the system is $L_x = 192$. {The reference bond is set at the 49th vertical bond.}  The red and blue lines denote linear and cosine fits defined in the main text, respectively. Only peaked values are used in the linear fit. (b) $K_{sc}$ as a function of the reference bond. Here we only show the results of $K_{sc1}$ from linear fits ($K_{sc2}$ has similar value). %and compare the average and minimum values fitted from different range of sites as described in Tab.~\ref{Pair-Fit-oneeighth}. %
...
\begin{figure}[t]
    ...
    %}
    %\quad    %用 \quad 来换行
    \includegraphics[width=0.4\textwidth]{one_eighth_doping/SzOneeighth.pdf}
    \includegraphics[width=0.4\textwidth]{one_eighth_doping/GFOneeighth192_ref49.pdf}
...
where $A$ is an amplitude, $\phi_1$ and $\phi_2$ are phase shifts, and $n_0=1-\delta$ is the averaged charge density (we set $n_0$ as a parameter to be determined in the fit \cite{PhysRevB.92.195139} ). %Note that half the exponent $K_\rho$ determines the decaying behavior in CDW amplitude.%
 To get rid of the boundary effect, we fit the density by gradually excluding data near the boundaries, and the results are listed in Tab.~\ref{ElecOscillationFit-oneeighthL192}. We can find an obvious boundary effect on the extracted exponent $K_\rho$. After excluding about a dozen of data near the boundary, the extracted $K_\rho$ converges fast to a value about $1.02(4)$ and the $R^2$ of the fit is very close to 1 (with a deviation in the order of 0.0001). We also study ladders with lengths 48, 64, 96, and 128 and find the extracted values of $K_\rho$ is converged. For comparison, we can alternatively obtain the parameter $K_\rho$ from the finite-size scaling, because the density at the center of the system scales as \cite{PhysRevB.92.195139}
\begin{equation}
...
\begin{figure}[t]
    \includegraphics[width=0.4\textwidth]{one_twelfth_doping/ElecOnetwelfthL120.pdf}
    \includegraphics[width=0.4\textwidth]{one_sixteenth_doping/ElecOnesixteenthL192.pdf}
...
\begin{table}[t]
    \caption{The dependence of the extracted parameters $K_\rho$ and $n_0$ (using Eq.~\eqref{charge_oscillation}) on the range of sites used ($\delta=1/4$, $L_x$ = 128).}
    \includegraphics[width=0.3\textwidth]{one_fourth_doping/ElecOscillationFit_onefourthL128.pdf}
    \label{ElecOscillationFit_onefourthL128}
\end{table}


\begin{figure*}[t]
    \includegraphics[width=0.3\textwidth]{one_twelfth_doping/KscOnetwelfth.pdf}
    \includegraphics[width=0.3\textwidth]{one_twelfth_doping/SzOnetwelfth.pdf}
    \includegraphics[width=0.3\textwidth]{one_twelfth_doping/GFOnetwelfthL120_ref31.pdf}
    \includegraphics[width=0.3\textwidth]{one_sixteenth_doping/KscOnesixteenth.pdf}
    \includegraphics[width=0.3\textwidth]{one_sixteenth_doping/SzOnesixteenth.pdf}
    \includegraphics[width=0.3\textwidth]{one_sixteenth_doping/GFOnesixteenthL192_ref49.pdf}
...
The charge density profiles for 1/12 and 1/16 dopings are shown in Figs.~\ref{ElecOnesixteenth_Onetwelfth}(a) and (b), which also display the Friedel oscillations with $\lambda_c = 1/\delta$. We find the amplitude of charge density oscillations is reduced with the decrease of doping level. The exponents increases from $K_\rho = 1.13(3)$ to $K_\rho = 1.28(3)$ from $\delta = 1/12$ to $\delta = 1/16$ (see the insets of Fig.~\ref{ElecOnesixteenth_Onetwelfth} and Tab.~\ref{Summary}). This behavior is consistent of the prediction that $K_\rho \rightarrow 2$ with $\delta \rightarrow 0$ \cite{PhysRevB.59.R2471,PhysRevB.63.195106}.
...
\begin{table}[t]
    \caption{The dependence of the extracted parameters $K_\rho$ and $n_0$ (using Eq.~\eqref{charge_oscillation} on the range of sites used ($\delta=1/3$, $L_x$ = 96)).}
    \includegraphics[width=0.3\textwidth]{one_third_doping/ElecOscillationFit_onethirdL96.pdf}
    \label{ElecOscillationFit_onethirdL96}
\end{table}

\begin{figure}[b]
    \includegraphics[width=0.4\textwidth]{one_sixth_doping/ElecOnesixthL120.pdf}
...
\begin{figure}[t]
    \includegraphics[width=0.4\textwidth]{one_sixth_doping/KscOnesixth.pdf}
    \includegraphics[width=0.4\textwidth]{one_sixth_doping/SzOnesixth.pdf}
    \includegraphics[width=0.4\textwidth]{one_sixth_doping/GFOnesixthL120_ref31.pdf}
...
Fig.~\ref{ElecOnesixthL120} shows the charge density profiles for a $L_x = 120$ ladder, which is also characterized by an algebraic quasi-long range order with period $\lambda_c=1/\delta=6$. As we can see in the inset of Fig.~\ref{ElecOnesixthL120}, $K_\rho$ extracted from two approaches are consistent with $K_\rho \approx 1.01(4)$. Following the procedure described above, we show the variance of $K_{sc}$ with the position of reference bond in Fig.~\ref{Onesixth}(a). We find $K_\rho < K_{sc}$ (also see Tab.~\ref{Summary}), suggesting the charge correlation is dominating at 1/6 doping. We find that charge and superconducting exponents are likely to violate the relationship $K_{sc} \cdot K_\rho = 1$.
...
\begin{figure}[h]
    \includegraphics[width=0.4\textwidth]{one_fourth_doping/ElecOnefourthL128.pdf}
    \includegraphics[width=0.4\textwidth]{one_third_doping/ElecOnethirdL96.pdf}
...
Fig.~\ref{ElecOnefourth_Onethird}(a) shows the charge density profile at $1/4$ doping for a $L_x=128$ ladder. We can see that the charge modulation has a persistent amplitude with period $\lambda_c = 1/\delta$ which doesn't decay in the bulk. The extracted $K_\rho$ using Eq.~\eqref{charge_oscillation} are listed in Tab.~\ref{ElecOscillationFit_onefourthL128}. The value of $K_\rho$ varies dramatically with the data range used in the fit. The fact that $K_\rho$ has very large error bars and oscillates across zero indicates the failure of the fit using Eq.~\eqref{charge_oscillation}, which suggests the charge order is likely long-ranged in the system.
...
\begin{figure*}[h]
    \includegraphics[width=0.4\textwidth]{one_fourth_doping/KscOnefourth.pdf}
    \includegraphics[width=0.4\textwidth]{one_fourth_doping/PairCorrelationOneFourth_ref65.pdf}
    \includegraphics[width=0.4\textwidth]{one_fourth_doping/SzOnefourthL128-semilog.pdf}
    \includegraphics[width=0.4\textwidth]{one_fourth_doping/SzOnefourthL128-loglog.pdf}
    \caption{Pair-pair correlation and local magnetization at $1/4$ doping. (a) $K_{sc}$ as a function of the reference bond. The length of the system is $L_x = 128$. (b) A typical fit of pair-pair correlation function. The reference bond is set at the 65th vertical bond. (c) and (d) Absolute values of the local magnetization. The results from finite kept state and the extrapolated with truncation errors results are shown. The solid line in (c) denotes exponential fitting using $\left| \mathbf{\textit{S}}_\mathbf{\textit{z}}\right| \propto \mathrm{e}^{-x_i / \xi_{\mathrm{s}}}$ with $\xi_{\mathrm{s}}=107(2)$. The plots in (c) and (d) are shown in the semi- and double-logarithmic axes, respectively. By comparing (c) and (d), we find the decay of the local magnetization is likely to be exponential, even though the extracted correlation length is comparable to the length of the system. Only peaked values are used in the fits for both pair-pair correlation and the local magnetization.}
    \label{Onefourth}
\end{figure*}

\begin{figure*}[h]
    \includegraphics[width=0.4\textwidth]{one_third_doping/KscOnethird.pdf}
    \includegraphics[width=0.4\textwidth]{one_third_doping/PairCorrelationOneThird_ref49.pdf}
    \includegraphics[width=0.4\textwidth]{one_third_doping/SzOnethirdL96-semilog.pdf}
    \includegraphics[width=0.4\textwidth]{one_third_doping/SzOnethirdL96-loglog.pdf}
    \caption{Pair-pair correlation and local magnetization at $1/3$ doping. (a) $K_{sc}$ as a function of the reference bond. The length of the system is $L_x = 96$. (b) A typical fit of pair-pair correlation function. The reference bond is set at the 49th vertical bond. (c) and (d) Absolute values of spin density. The results from finite kept state and the extrapolated with truncation errors results are shown. The solid lines in (c) and (d) denote exponential and power-law fits, respectively. We find the data can be fitted with either exponential or power law. But the correlation length extracted from the power law fit is larger than the system size which indicates the decay of the local magnetization is likely to be power law. Only peaked values are used in the fits for both pair-pair correlation and the local magnetization.}
    \label{Onethird}
\end{figure*}

\begin{acknowledgments}
We thank useful discussions with Xingjie Han.
Y. Shen and M. P. Qin thank Weidong Luo for his generosity to provide computational resources for this work. M. P. Qin acknowledges the support from the National Key Research and Development Program of MOST of China (2022YFA1405400), the National Natural Science Foundation of China (Grant No. 12274290) and the sponsorship from Yangyang Development Fund. All the DMRG calculations are carried out with iTensor library\cite{itensor-r0.3}.
\end{acknowledgments}
