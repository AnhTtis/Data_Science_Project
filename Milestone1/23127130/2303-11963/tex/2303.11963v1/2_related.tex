...
Wu \etal~\cite{wu2018full} and Lyu \etal \cite{lyu2020differentiable} use turntables in front of static structured backlights to reconstruct geometry, but our work does not require these special setups. %, assuming that light paths only refract two bounces within the media. % Wu \etal \cite{wu2018full} and Lyu \etal \cite{lyu2020differentiable} capture multi-views of transparent objects on a turntable in front of a static structured backlight and reconstruct the correspondence between camera ray and background pixel, under the assumption that light paths only refract two bounces within the media. 
Neural networks are later introduced with analytical refraction modeling to approach the problem. Li \etal~\cite{li2020through} assume known environment illumination and IOR to only optimize for geometry with one neural network on all kinds of geometry. %with synthetic renderings of transparent objects. 
...
% Our methods are evaluated on real world dataset to close up the domain gap between rendering and captured images.
