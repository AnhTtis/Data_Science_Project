...
baryon, from the surface to a given density in the crust, as a function of
mass density, for the F+18 model of the accreted crust and the GC models
based on the BSk24 and BSk25 nuclear forces (which provide more
...
A.1 of \citealt{Fantina_18}). For the GC models, we consider the
minimum and maximum values of $P_\mathrm{oi}$. The corresponding crust
structure and heat production are presented in Tables~\ref{tab:GCheat24}
and \ref{tab:GCheat25}.  Making use of the fact that, as found by
\citet{GusakovChugunov20}, the EoS of the accreted inner crust is close
to the non-accreted one, we assume that the number fraction of protons
among all nucleons $Y_\mathrm{p}=Z/A'$, where $Z$ and $A'$ are the
numbers of protons and all nucleons in a Wigner-Seitz cell, equals the
$Y_\mathrm{p}$ value in the non-accreted crust ($Y_\mathrm{p}$ affects the
number density of the nuclei and consequently the ion heat capacity and
electron-ion transport coefficients). We assume that the
residual heat release in the deep layers of the inner crust, predicted
by \citet{GusakovChugunov21}, occurs at the proton-drip point determined
by \citet{Pearson_18}.
...
corresponding to four masses of accreted material, from
...
corresponding nuclear charge number $Z$, the total number of baryons per
one nucleus $A'$ and the heat release per an accreted nucleon, $\Delta
...
case we use the $\rho$ and $P$ values and the ratio $Y_\mathrm{p}=Z/A'$
...
values of the pressure $P_\mathrm{oi}$ at the inner-outer crust
...
but with two updates: for MXB 1659$-$29 (object 7), an improved estimate
...
which corresponds to the results obtained by \citet{Degenaar_21} with
...
thermal quasi-equilibrium by 2018 \citep{Degenaar_21}. If it is the
case, then this luminosity estimate should be considered as an upper
limit. The estimates of $\langle \dot{M}_\mathrm{obs}\rangle$ are rather
...
to accretion ($\sim10^4$ yr, which would be a proper interval for the
time averaging). In Fig.~\ref{fig:qemxsf25gs} we conditionally plot
uncertainties of  $\langle \dot{M}_\mathrm{obs}\rangle$ as a factor 2
around the most likely values based on the available observations (in
the case of MXB 1659$-$29 this factor somewhat underestimates the true
uncertainty -- see \citealt{PotekhinChabrier21}).
...
MAXI J0556$-$332 (35). For MXB 1659$-$29 (object 7) and HETE
J1900.1$-$2455 (object 25), the estimates of the average accretion rate
and quiescent luminosity have been corrected as per
\citet{PotekhinChabrier21} and \citet{Degenaar_21}, respectively (see
text). The lines show theoretical predictions for neutron stars of
...
Urca processes. We see that an increase of the heating power by two
orders of magnitude is far from enough to overcome this difference
(which would be even greater if we used the same heat blanket for
both stellar models).
 Fig.~\ref{fig:fluxd} shows the
...
\newcommand{\jaa}{JA\&A}
\newcommand{\ptp}{Progress Theor.\ Phys.}
\newcommand{\ppnp}{Progress Part.\ Nucl.\ Phys.}
...
2014,
Stellar Superfluids, 
in Novel Superfluids, vol.~2,
ed.{} K.-H. Bennemann and J.~B. Ketterson
