...
%\newcommand{\remove}[1]{\textcolor{RoyalBlue}{\xout{(#1)}}}
...
\amend{Phonon properties of realistic materials are routinely calculated within the Density Functional Perturbation Theory\,(DFPT). This is a semi--classical
approach where the atoms are assumed to oscillate along classical trajectories immersed in the electronic Kohn--Sham density, treated quantistically. 
In this work I demonstrate that, in metals, non--adiabatic effects induce a gap between the DFTP phonon frequencies and the fully quantistic solution of the phonon Dyson equation.
A gap that increases with the phonon energy width reflecting the breakdown of the semi--classical DFPT description. The final message is that non--adiabatic
phonon effects can be included only by using a fully quantistic approach.}
\end{abstract}
\date{\today}
\maketitle
...
theory 
\remove{is based on} \amend{leads to} the introduction of a frequency dependent and non--hermitian dynamical matrix whose eigenvalues, the phonons frequencies, are complex. 
\remove{
Let's
...
\remove{The message of \e{eq:intro.1} is clear: non adiabatic effects induce an energy indetermination of the semi--classical DFPT phonon frequencies which result in
the decay of the atomic oscillations. A}
\amend{FD--DFPT provides, thus,} a picture conceptually equivalent to MBPT \amend{where non--adiabatic effects induce a renormalization and broadeding of the
...
\remove{In this work I propose a time--dependent extension of DFPT\,(TD--DFPT) whose master equation is showed to belong to the family of Volterra integro--differential
equations. I solve exactly the TD--DFPT master equation to define the semi--classical phonon energy beyond the adiabatic regime.  This energy will be demonstrated
to be real, meaning that}
\amend{In this work I mathematically demonstrate that, within non--adiabatic Density Functional Perturbation Theory,
phonon widths are strictly zero.  This implies that}
\remove{small} semi--classical atomic oscillation\amend{s} never decay in time, even when they are resonant with quantistic electron--hole pairs. I show
that an infinite phonon lifetime \remove{($\gc_\gl=0$)} is necessary to ensure that the dynamics conserves the total energy.  The FD--DFPT approach will be demonstrated
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{sec:H}
In order to \amend{describe non--adiabatic effects I} rewrite DFPT in the time domain\amend{.} I start from the perturbed DFT Hamiltonian written in second quantization:
\ml{
...
and annihilation operators: $\h{c}^\dag_i/\h{c}_j$ and \remove{is} $\Delta \h{\gr}_{ij}=\h{c}^\dag_i\h{c}_j-\average{ \h{c}^\dag_i\h{c}_j}$.
...
with $\h{\callD}_\gl\(t\)=\frac{1}{\go_\gl}\bpr{\frac{\di^2}{\di t^2}-\go_\gl\(C^{e-n}_\gl-\go_\gl\)}$. $t=0$ is the initial time\amend{,} \remove{and} underlined quantities are
matrices in the single--particle basis\amend{, and $\Delta\gee_{ij}=\gee_i-\gee_j$}.
...
$\ul{\Delta V}^{Hxc}$ from \e{eq:EOM.1} and replace, in the commutator, $\ul{g}^\gl$ with the {\em screened} \amend{and frequency dependent} electron--phonon interaction, 
\amend{$\ti{\ul{g}}^\gl\(\go\)=\ul{\ul{\gee}}^{-1}\(\go\)\ul{g}^\gl$}, with $\ul{\ul{\gee}}^{-1}$ the dielectric tensor. For simplicity here we ignore the time dependence of
$\ul{\ul{\gee}}^{-1}$ \amend{and assume $\ti{\ul{g}}^\gl\(\go\)\approx \ti{\ul{g}}^\gl\(\go=0\)$\cite{LK}}.
...
\eql{eq:TDDFPT.1}
{
 f\(z\)=\callL[f]\(z\)=\int_0^{\amend{\infty}} e^{-z t} f\(t\) d\,t.
}
...
\eql{eq:TDDFPT.5}
{
 \gC_\gl\(\go\)=-\pi \sum_{ij}g_{ji}^\gl \ti{g}_{ij}^\gl \Delta f_{ij}\gd\(\go\amend{+}\Delta\gee_{ij}\).
}
...
It follows that the Laplace solution leads to \remove{real} \amend{imaginary} poles \amend{corresponding to real frequencies}.

If we call the zeros of \amend{\elab{eq:TDDFPT.3}{a}} $\gO^{CM}_\gl$ we thus finally get 
\eql{eq:TDDFPT.6}
{
u_\gl\(t\)=\sum_{s=\pm}
\begin{cases}
...
we recover the FD--DFPT\fddfpt\remove{ and \e{eq:intro.1}}.
...
is not time--dependent. More importantly, as we are in the linear regime and $f_i$ is time--independent, \remove{a} decaying atomic oscillations lead\remove{s} to 
\remove{the a}\amend{a decaying} total
energy\remove{ which decays in time}. This is in contrast with the fact that within the mean--field approximation the energy is conserved. 
...
basic theorem of MBPT \amend{$\Pi_\gl\(-\im \go\)$} acquires a finite imaginary part and, consequently, provides the phonon with a finite energy indetermination. 
...
phonon energies are purely rea\amend{l} and that a semi--classical description has no access to the phonon energy indetermination.
...
\amend{As I am working within DFT the electron--electron interaction is replaced by the exact mean--field exchange--correlation potential, $\ul{\Delta
V}^{Hxc}$}.
...
\amend{The notation used in \e{eq:SM_H.1} is very general. The $i$ label can represent any one--body quantum number. If we consider, for example, the electronic momentum $\kk$ (bold symbols are vectors) \e{eq:SM_H.1}
turns into}
\amend{
\mll{eq:SM_H.4}{
...
\amend{From \e{eq:SM_H.4}, the very same procedure described in the main text leads to}
\amend{\eql{eq:SM_H.5}{
 C_{\qq\gl}\(z\)=-\im g_{\qq}^\gl \ti{g}_{-\qq}^\gl \sum_\kk \frac{f_{\kk}-f_{\kk+\qq}}{z-\im\(\gee_{\kk}-\gee_{\kk+\qq}\)}.
}}
\amend{\e{eq:SM_H.5} is equivalent to Eq.(145) of \ocite{Giustino2017}, for example. 
...
\remove{neglect, for simplicity, the time--dependence of $f^{Hxc}$ and} plug \e{eq:SM_g_dress.1} in \e{eq:SM.eom.2} and take the average of both terms we get
\amend{\mll{eq:SM_g_dress.2}
{
...
\amend{If we now use \e{eq:SM_g_dress.2} in \e{eq:SM.eom.2} we get} 
\amend{\eql{eq:SM_g_dress.3}{
 \Delta \ul{\gr}\(t\)=\int \di t^\p \ul{\ul{\chi}}\(t-t^\p\)\ul{g}^{\gl} u_\gl\(t^\p\),
}}
\amend{that corresponds to the TD--DFT Kubo equation. If we now use the K\"all\'en--Lehmann spectral representation of $\ul{\ul{\chi}}\(t-t^\p\)$~\cite{ALEXANDERL.FETTER1971} we can expand
\e{eq:SM_g_dress.3} over the poles ($E_I$) and residuals ($\ul{\ul{\chi}}^I$) of $\ul{\ul{\chi}}$:}
\amend{\eql{eq:SM_g_dress.4}{
 \gr_{ij}\(t\)=\sum_I \[\ul{\ul{\chi}}^I\ul{g}^{\gl}\]_{ij} \int \di t^\p e^{\im E_I\(t-t^\p\)} u_\gl\(t^\p\),
}}
\amend{From \e{eq:SM_g_dress.4} it follows that the effect of the time--dependent exchange--correlation DFT potential is to replace the independent particle
...
\amend{\eql{eq:SM_g_dress.5}{
 \gr_{ij}\(t\)\approx \ti{g}^{\gl}_{ij} \int \di t^\p e^{\im \Delta \gee_{ij}\(t-t^\p\)}u_\gl\(t^\p\),
}}
\amend{with $\ul{\ti{g}}^{\gl}\sim \ul{\ul{\varepsilon}}^{-1}\(\go=0\) \ul{g}^{\gl}$.}
...
\mll{eq:SM.damp.A.2}
{
 ...
 -\im\go\[ \left. e^{\im \go t} u_\gl\(t\)\right|_{0}^{\infty}-\im\go \amend{u_\gl^f\(\go\)}\],
}
\amend{with $u_\gl^f\(\go\)$ the Fourier transformation of $u\(t\)$.}
...
We now assume $\gD C_\gl\(-\im \go\) \approx \callP \[ \Pi_\gl\(-\im \go\)-\remove{\go_\gl} C^{e-n}_\gl\]$. This is a very good approximation as the phonon self--energy
