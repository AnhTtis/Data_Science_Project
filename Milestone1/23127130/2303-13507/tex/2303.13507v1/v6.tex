...
%
We find some similarities between the weakly and the strongly interacting case, 
...
Moreover, transport of carriers through heterostructures consisting of a weakly and strongly interaction part like molecular layers or quantum dots has prompted the development of a special branch of density functional theory, the so-called i-DFT for stationary currents \cite{stefanucci2015,jacob2020}. In the present work, we employ a discrete lattice model of solids that provides a mathematical description of systems with a well-defined amount of electronic correlations.
%
As a prototypical example for such a heterostructure, we study the interface 
between a semiconductor (where the interactions between the electrons play no role)
and a Mott insulator (where the interactions play a major role). The fabrication of these systems \cite{doi:10.1126/sciadv.aaz0611,noh2014direct} as well as the theoretical investigation how properties carrier over through the interfaces \cite{yonemitsu2007suppression,jiang2012density,zhao2010mott,helmes2008kondo} has advanced over the years.
...
%
In comparison to many other methods, this approach can be applied to the regions 
of weak and strong interactions on the same footing and is capable of deriving 
...
%\red{Additionally, we use numerical methods to calculate the transmission through a Mott insulating layer between two semiconductors as support for the analytical claims.}
...
In order to start with a tight-binding model which is capable of describing a 
...
only if the semiconductor band edge falls into the lower or upper Hubbard band, shown as gray-shaded areas in Fig.~\ref{fig:trans_interface},
appreciable transmission is found. The parameter $V$ on the abscissa is used to tune the semiconductor band edge through the relevant range of energies. 
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=\linewidth]{pics/trans_interface.pdf}
    \caption{Transmission probability through the Mott insulator semiconductor interface as function of the energy $E$ and the on-site potential $V$ in a two-dimensional lattice with $T_\mathbf{k}^\|=\frac{2T}{Z}$ and $T=0.4U$. The gray shaded areas depict the positions of the lower (from $-0.25E/U$ to $0E/U$) and upper ($0.85E/U-1E/U$) Hubbard band of the Mott side. The semiconductor band shifts with $V$. The transmission quickly drops with non-overlapping wave numbers $\kappa_\mathrm{Mott} \neq \kappa_\mathrm{semi}$.}
...
The layer is supposed to have a thickness of $d$ layers.  Again, we deduct boundary conditions at the interfaces and find after some algebra the transmission
\begin{equation}
...
In the true Mott state, however, each of the middle sites must be modeled by the density matrix $\frac{1}{2}(\ketbra{\uparrow}{\uparrow} + \ketbra{\downarrow}{\downarrow})$.
Then, the cancellation in the transmission probability is not perfect but becomes more efficient when we consider a larger number of rows. 
...
sites.
%
The Mott insulating layer with a random spin distribution  
generates a random distribution of minus signs of the single amplitudes which efficiently cancel out (but not exactly to zero) in the middle between the upper and lower Hubbard bands. 

This destructive interference is quite similar to the suppression of tunneling 
by a strong disorder potential. 

Since in this example the rows are not coupled with each other 
it is possible to obtain the limit of the infinite number of rows analytically, 
based on the elementary amplitudes, $T^\uparrow$ and $ T^\downarrow$. 
The transition probability for the coherent wave is proportional to 
$|T^\uparrow|^2 + |T^\downarrow|^2 + \rm{Re}(T^\uparrow T^\downarrow)$
%\begin{equation}
%    \left[ |T^\uparrow|^2 + |T^\downarrow|^2 + \rm{Re}(T^\uparrow T^\downarrow) \right]
%\end{equation}
and reduces for the above obtained values of $T^\uparrow$ and $ T^\downarrow$ to
\begin{equation}
\label{pert_theo}
    \frac{\frac{U^2}{4} + \left( \frac{U}{2} - V \right)^2}{\left[ \frac{U^2}{4} - \left( \frac{U}{2} - V \right)^2 \right]^2}\,,
\end{equation}
which for $V=U/2$ gives a strongly suppressed minimal value of $4/U^2$. 
This perturbative formula has been compared to numerical simulations of up to three rows as well as of a single row scaled up analytically to the infinite number thereof.

%
Fig.~\ref{fig:vgl_pert} shows a very good agreement between the perturbation theory  and the numerical results, especially around $V\approx U/2$.
%
It is expected that for a larger number of Mott sites per row the discussed cancellation mechanism should become even more intense.
    ...
    \centering
   	\begin{tikzpicture}
		\begin{axis}[
			...
			ylabel = {$P(V)/P(V=0.5U)$},
			legend cell align = {right},
            legend style={at={(0.8,.8)},anchor=east}
			]
		
            \addplot[smooth, blue] table[x index=0, y index=1] {pics/exakt_pert.dat};
    
			\addplot+[
			  only marks, red
			] table[x index=0, y index=1] {pics/vglunendl.dat};
             \addplot+[
			  only marks, orange
			] table[x index=0, y index=2] {pics/vglunendl.dat};

   \addplot+[
			only marks
			] table[x index=0, y index=3] {pics/vglunendl.dat};


        
			\legend{Eq.~\ref{pert_theo},Num. obt. $T_{\uparrow/\downarrow}$, Simul. two rows, Simul. three rows }
	...
	\end{tikzpicture}
    \caption{Transition probability $P$ for a coherent wave through the toy model, scaled with the individual minimum value at $V=0.5U$. The blue line gives the perturbation theory result, the red squares give the probability using numerically obtained amplitudes $T_\uparrow$ and $T_\downarrow$, orange circles give the result of a numerical simulation for two rows and the stars do the same for three rows.}
    \label{fig:vgl_pert}
\end{figure}
