...
$K\in\T^2$ the fixed quasi-momentum of the particles pair,
associated with a system of two identical fermions on the
two-dimen\-sional lattice $\mathbb{Z}^2$ with first and second
...
LAbdukhakimov:2020,LDKh:2016,LLakaev:2017,LOzdemir:2016}.
In addition, these discrete Hamiltonians may be viewed as a natural
...
may be written as the von Neumann direct integral
\begin{equation}
...
nontrivially depend on the quasi-momentum $K\in\T^d$ (see e.g.,
...
\ref{eigen-zeros} and \ref{asympt}). Therefore, the number of
eigenvalues  of
 $H_{\lambda\mu}(0)$  changes if and only if the point
$(\lambda,\mu)$ on the parameter plane $\R^2$ crosses one of the
{curves  $C^{-}(\lambda,\mu)=0$ or $C^{+}(\lambda,\mu)=0$} (see also
Lemmas \ref{simple1}-\ref{simple3}). Moreover, after each such a
single  crossing, the number of eigenvalues of  $H_{\lambda\mu}(0)$
changes exactly by one. Surely,   this crossing event is interpreted
as a moment when the essential spectrum of $H_{\lambda\mu}(0)$
either gives birth to or absorbs a bound state of
$H_{\lambda\mu}(0)$(see Theorem \ref{teo:disc_quasi0}). Furthermore,
the curves $C^{-}(\lambda,\mu)=0$ and $C^{+} (\lambda,\mu)=0$ divide
the $(\lambda,\mu)$ parameter plane into several simply connected
domains on each of which the number of eigenvalues of the operator
$H_{\lambda\mu}(0)$ remains constant (see Theorem
\ref{teo:constant}).
...
technique.
Surprisingly, the maximum number of isolated eigenvalues is achieved
only in four connected components where both $\lambda$ and $\mu$ run
through infinite intervals.
...
$K\in\T^2$ is called the \emph{two-particle quasi-momentum} and the
...
\begin{figure}[h!]
    \centering
\includegraphics[width=0.9\textwidth]{picture2.jpg}
\caption{Plot of the curves defined by equation
$C^-(\lambda,\mu)=0$}\label{fig:1}
\end{figure}
\begin{figure}[h!]
    \centering
\includegraphics[width=0.9\textwidth]{picture3.jpg}
...
$L^{2,\mathrm{os}}(\T^2)$ or $L^{2,\mathrm{oa}}(\T^2)$ may be
represented respectively as the sum or difference of elements of
$L^{2,\mathrm{o}}(\T^2)$.%
\end{proof}
...
\eqref{moment_poten} an, thus, it reads as
\begin{align*}
...
\frac{\mu}{2}({f},\alpha_{3}^\theta)\alpha_{3}^\theta
,\quad \theta\in\{\mathrm{os},\,\,\mathrm{oa}\},
\end{align}
...
\label{fc}
&c^{\pm}(z)
=\frac{1}{8\pi^2}\int\limits_ {\mathbb{T}^2}\frac{(\sin p_{1}\pm\sin
p_2)(\sin 2p_{1}\pm\sin 2p_2)
\mathrm{d}p}{\cE_{0}(p)-z},\\
\label{fd} &d^{\pm}(z)
=\frac{\sqrt{2}}{8\pi^2}\int\limits_ {\mathbb{T}^2}\frac{(\sin
p_{1}\pm\sin p_2)
...
(\sin p_{1}\cos p_2\pm\sin p_2\cos p_1) \mathrm{d}p}{\cE_{0}(p)-z},\\
\label{ff} &f^{\pm}(z)
=\frac{1}{2\pi^2}\int\limits_ {\mathbb{T}^2}\frac{(\sin p_{1}\cos
p_2\pm\sin p_2\cos p_1)^2 \mathrm{d}p}{\cE_{0}(p)-z}.
\end{align}
...
\,\,z\searrow 8,
\end{align}
where the functions $C^\pm(\lambda,mu)$ are given by \eqref{abobel}.
\end{lemma}
\begin{proof}
...
\ref{teo:discr_Kne0} --\ref{teo:disc_quasi0}.
We will see below that Theorems \ref{teo:discr_Kne0} and
\ref{teo:disc_quasiK}  are rather corollaries of Theorems
\ref{teo:constant} and \ref{teo:disc_quasi0}. Thus, we start with a
proof of the two last ones.
\smallskip
...
In view of $C^{-}(\lambda,\mu)<0$, Lemma \ref{lemm:asymp_determ}
implies
\begin{align*}
&\lim\limits_{z\nearrow 0}
...
C^{-}(\lambda,\mu)>0
\end{align*}
and hence Lemma \ref{asympt} gives
\begin{align*}
&\lim\limits_{z\nearrow 0} \Delta_{\lambda\mu}(z)>0.
\end{align*}
...
{\bf Acknowledgments}. The authors acknowledge support of this
research by Ministry of Innovative Development of the Republic of
Uzbekistan (Grant No. FZ–20200929224).
...
\bibitem{Efimov:1970}
V.N. Efimov, Yad. Fiz. \textbf{12}, 1080 (1970) [Sov. J. Nucl. Phys.
\textbf{12}, 589 (1970)].
...
periodic Hamiltonians, Journal of Mathematical Physics {\bf39}
(1998), 4369--4416.
...
$N$-dimensional lattice}. Linear and Multilinear Algebra (2020).
...
system on an optical lattice. Theoret. and Math. Phys. {\bf203}:2
(2020), 251--268
...
lattice}. Theoret. and Math. Phys. {\bf 158} (2009), 360--376.
...
\bibitem{LLakaev:2017} S.N.Lakaev, Sh.S. Lakaev:
...
lattices}. Hacettepe J. Math. Stat. {\bf45} (2016), 1693--1703.
...
Phys. Rev. Lett. \textbf{118}, 230601 (2017).
...
Rev. Lett. \textbf{110}, 235301 (2013).
...
dimensions}. Ann. Phys. {\bf97} (1976), 279--288.
...
Efimov effect of three-body Schrödinger operators}. J. Funct. Anal.
...
pairs in an optical lattice}. Nature {\bf 441}, 853--856, 2006.

\bibitem{Yafaev:1974}
D.R. Yafaev: {On the theory of the discrete spectrum of the
three-particle Schr\"odinger operator}, Mat. Sb. \textbf{94(136)},
567--593, 655--656 (1974).
