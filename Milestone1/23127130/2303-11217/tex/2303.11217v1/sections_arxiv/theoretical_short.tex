...
\begin{proposition}[Proof in Appendix~\ref{app:prop_lipschitz}]
...
\begin{proposition}[Proof in Appendix~\ref{app:fixed_point}]
...
This result, detailed in Appendix~\ref{app:fixed_points_are_critical_points}, follows from an interpretation of $\HVAE(\x,\tauvec)$ as an MMSE denoiser. As a consequence Tweedie's formula provides the link between the right-hand side of equation~\eqref{eq:fixed_point} and $\nabla g$.
...
On noisy images $\sigma>0$, the reconstruction behaves as a contraction, as the ratio $r<1$ is always  observed. Moreover, reducing the temperature of the latent regularization $\tau$ increases the strength of the contraction. This experiment suggests that with the trained PatchVDVAE architecture, the hypothesis $L_{\tauvec}<1$ in  Corollary~\ref{cor:convergence} holds for noisy images.\\

\begin{figure}
    \includegraphics[width=\columnwidth,trim={0 .5cm 0 .4cm},clip]{figures_arxiv/hist.pdf}\vspace{-0.2cm}
...
We now illustrate the effectiveness of PnP-HVAE through comparisons with the optimization of the objective $J_1(\x_k,\z_k)$ in~\eqref{eq:J1} using  the Adam algorithm~\cite{kingma2014adam} for two  learning rates $lr \in \{0.01, 0.001\}$.  The left plot in figure~\ref{fig:conv} shows that Adam is able to estimate a better minima of $J_1$. However, our alternate algorithm requires a smaller number of iterations to converge.  
On the other hand, as illustrated by the right plot in figure~\ref{fig:conv}, the use of Adam involves numerical unstabilities. Oscillations of the ratio $ L_k :=\frac{||\T\left(\x_{k+1}\right) - \T\left(\x_{k}\right)||}{||\x_{k+1} - \x_{k}||}  
$ are even increased  with larger learning rates, whereas our method provides a stable iterates sequence.
...
\begin{figure}[t]
    ...
    \centering
    \begin{tabular}{cc}
    \hspace{-.2cm}\includegraphics[height=.33\linewidth]{{figures_arxiv/butterfly_T_0.70_J1_2}.pdf}&\hspace{-.2cm}\includegraphics[height=.33\linewidth]{{figures_arxiv/butterfly_T_0.70_Lk_2}.pdf}\vspace{-0.4cm}
    \end{tabular}
...
\begin{figure}[ht]
    \center
    \includegraphics[width=0.95\columnwidth]{figures_arxiv/demo_face_restoration.pdf}\vspace{-0.1cm}
...
\begin{table}[ht]\small
    \centering
        ...
        \hline     
        \multirow{2}{*}{SR $\times 4$}&Adam & $28.56$ & $\underbar{0.75}$ & $0.38$ & $26$  \\
       \multirow{2}{*}{$\sigma=3$}& ILO & $\underbar{28.28}$ & $0.76$ & $\mathbf{0.19}$ & $\underbar{19}$\\
       & PnP-HVAE & $\mathbf{29.32}$ & $\mathbf{0.82}$ & $\underbar{0.28}$ & $\mathbf{15}$\\      
        \hline 
        Deblurring &Adam & $26.69$ & $\underbar{0.75}$ & $0.27$ &$ \underbar{11.3}$  \\
        (motion) &ILO & $\underbar{27.62}$ & $0.76$ & $\underbar{0.20}$ & $21$ \\
        $\sigma=8$&PnP-HVAE & $\mathbf{30.40}$ & $\mathbf{0.84}$ & $\mathbf{0.16}$ & $\mathbf{10.4}$\\
        \hline 
         Deblurring &Adam & $\underbar{30.17}$ & $\underbar{0.83}$ & $\underbar{0.21}$& $\underbar{11.3}$\\
         (gaussian) &ILO & $28.58$ & $0.78$ & $\mathbf{0.19}$ & $21$\\
        $\sigma=8$&PnP-HVAE& $\mathbf{30.81}$ & $\mathbf{0.86}$ & $0.24$ & $\mathbf{11.6}$
        \end{tabular}\vspace*{-0.1cm}
    \caption{\label{table:comp_ILO}Quantitative evalutation on face restoration. Best results in {\bf bold}, second best \underline{underlined}.\vspace{-0.2cm}}
\end{table}
