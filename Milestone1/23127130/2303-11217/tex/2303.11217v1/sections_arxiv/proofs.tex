...
\begin{proposition}\label{prop:J2decomp}
	...
	where
	\begin{align}
		A(\z) &:= \sum_{l=0}^{L-1} A_l(\z_l,\z_{<l}) \label{eq:A}\\
    ...
    \begin{align}
    	...
    	S_l(\zll) &:= S_{q,l}(\zll) + \lambda_l S_{p,l}(\zll)  \label{eq:gaussianprod}  %\label{eq:mlSl}
    \end{align}
\end{proposition}
...
\subsection{Proof of Proposition~\ref{eq:prop_lipschitz}}\label{app:prop_lipschitz}
...
\subsection{Proof of Proposition~\ref{prop:fixed_point}}\label{app:fixed_point}
...
$$ \ptt{\z} := \prod_{l=0}^{L-1} \frac{\pt{\z_l|\z_{<l}}^{\frac{1}{\tau_l^2}}}{\tau_l^{\frac{d_l}{2}}} $$
...
\begin{align*}
	\nabla\log\ptt{\x} & = \frac{1}{\gamma^2} \int (\mut{\z}-\x) \frac{\pt{\x|\z}\ptt{\z}}{\ptt{\z}}d\z \\
