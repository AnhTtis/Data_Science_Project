...
The latent optimization methods~\eqref{eq:map} provide high-quality solutions that are guaranteed to be in the range of a generative network. %
...
In this work, we propose an algorithm that exploits the strong prior of a deep generative model while providing realistic convergence guarantees.
...
As the HVAE  model differs significantly from the architecture of concurrent models, it is necessary to design algorithms adapted to their specific structure.
The latent space dimension of HVAE is significantly higher than the image dimension.
 Hence, constraining the solution to lie in the image of the generator is not sufficient enough to regularize inverse problems.
...
Under this perspective, we give sufficient conditions to ensure the convergence of our method, and we  provide an explicit characterization of the fixed-point
of the iterations. 
