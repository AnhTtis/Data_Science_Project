...
%\subsubsection{Uniform Crops vs Density-based Crops}
...
\begin{table}
    ...
    \caption{Comparison of detection performance between a baseline detector, uniform crops and density crops on VisDrone dataset (1.5K pixel resolution). %Uniform cropping creates further scale imbalance and decreases performance.
    }
...
%Then the second (2k $\times$ 2k without P2) up-scales the image crops at higher resolution while still not detecting on P2. 
%We can also conclude that with our approach the gap between with and without high resolution features is minimized, signifying its boost in small object detection performance. Thus we can avoid using expensive high-resolution features without compromising the detection accuracy.
\begin{table}
    \centering
    \resizebox{.48\textwidth}{!}{% <------ Don't forget this %
      \begin{tabular}{l||rrr|rrr|r}
    ...
    \hline
    %\emph{2k$\times$2k without P2} & & & & & &\\
    %Baseline & 32.73 & 52.94 & 34.96 & 16.71 & 34.92 & 43.74 & 0.54 \\ 
    %CZ Det. (Ours) & 34.52 & 56.57 & 36.41 & 15.85 & 37.26 & 45.32 & 0.35 \\
    %\hline
    \emph{With P2} &&&&&& \\
...
%In this section, we present the observations from several ablation studies conducted to validate the effectiveness of our approach. We used the VisDrone dataset for all studies in this section. Additional studies about the detector backbone, the analysis of the detection errors, and the effect of the maximum crop size $\pi$ are presented in the supplementary material.
...
%\paragraph{Density Crops effect at Training and Inference.}
...
%\paragraph{Impact of the Quality of Crops.}
...
%\paragraph{Why Iterative Merging for Crop Discovery?}
...
\begin{table}
    ...
    \hline 
    %Ours(without P2) & 33.02 & 57.87 & 33.09 & 25.74 & \textbf{42.93} & 41.44 \\
...
%\subsection{With Other Backbones}
