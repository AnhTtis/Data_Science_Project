...
\title[Principal eigenvalue of a linear elliptic operator with large advection]{Asymptotic behavior of the principal eigenvalue of a linear second order elliptic operator with large advection and general boundary conditions}

\author[Rui Peng]{Rui Peng${}^{\dag}$}
\author[Guanghui Zhang]{Guanghui Zhang${}^{\S,*}$}
\author[Maolin Zhou]{Maolin Zhou${}^{\ddag}$}
...
\thanks{${}^\ddag$Chern Institute of Mathematics and LPMC, Nankai University, Tianjin, 300071, China.  (Email: {\tt zhouml123@nankai.edu.cn})}


\thanks{M. Zhou was supported by the National Key Research and
  Development Program of China (2021YFA1002400),  Nankai Zhide Foundation and NSF
  of China (No. 12271437, 11971498).  R. Peng was supported by NSF of China (No. 12271486, 12171176). G. Zhang was supported by NSF of China (No. 12171176, 11971187).}

\subjclass[2010]{35P15, 35J20, 35P20.}
 \keywords{Second order elliptic operator; principal eigenvalue;
   large advection;  asymptotics.}
...
In this paper, we continue our work on the following eigenvalue problem of a linear second order elliptic operator with Robin boundary condition:
\begin{equation}
  \label{eq:1.1}
  \left\{
    \begin{aligned}
...
\item {\rm (ii)} A point $x\in\partial\Omega$ is called a \textbf{boundary critical
  ...
  \text{det}\left(D^{2}m(x)\right)\neq 0$ or $|\nabla m(x)|\neq 0, \text{det}\left(D^{2}_{\partial\Omega}m(x)\right)\neq 0$.
...
\item[\rm (ii)] In dimension $N=2$, if $v$ is a $C^1(\overline\Omega)$ satisfying \eqref{div-f} and $v\cdot n=0$ on $\partial\Omega$, then it is not hard to see that $v$ has a first integral, and therefore $\lim_{\alpha\to\infty}{\lambda}(\alpha)$ is finite and is determined by Theorem \ref{theorem-ND}(ii).
  ...
  In dimension $N=1$, that is, $\Omega$ is an open interval, say $\Omega=(0,1)$, then $\Delta m=0$ on $\overline\Omega$ implies $m(x)=ax+b,\, \forall x\in[0,1]$ for some constants $a,\,b$, and so $v(x)=a$ has a first integral in $H_0^1(\Omega)$ if and only if $a=0$. It turns out that this result is a very special case of problem \eqref{p} to be treated in Section 3 below. As mentioned before, for any dimension $N\geq1$, Theorem \ref{theorem-ND}(i) and (ii) can allow $v$ to be degenerate so that it has a first integral.
...
Let us now comment on the proofs of the main results of this paper. Though our approach is in the spirit of \cite{CL2008}, several essential difficulties arise, which require delicate techniques to overcome. One of such difficulties is to determine whether the following energy functional
...
The case that $\beta$ may be negative somewhere on $\partial\Omega$ turns out to be more challenging to handle since \eqref{energy} may be unbounded from either below or above. Among other ingredients in our argument, we shall establish a trace-type inequality (see Lemma \ref{lemma-trace}), which allows us to control the boundary integral term in \eqref{energy} by the first integral term of \eqref{energy}. As a result, under proper assumptions on the advection function $m$, we can determine whether \eqref{energy} is bounded or unbounded; based on such information, $\text{supp}(\mu)$ can be therefore clarified.

All the aforementioned results apply to any dimension case (i.e., $N\geq1$). As far as the dimension
$N=1$ is concerned, by allowing the advection function $m$ to have natural kinds of degeneracy, we can give a complete description of the limiting behavior of the principal eigenvalue of the associated eigenvalue problem (i.e., \eqref{p} in Section 3); our results exhaust all parameter ranges of the boundary conditions and thus complement those in \cite{PZ2018}. See Section 3 for the precise details.
We want to stress that the trace inequality given by Lemma \ref{lemma-trace} is also crucial in yielding the results of the one-dimensional problem \eqref{p}. We also note that the principal eigenvalue of the periodic eigenvalue problem \eqref{eq:periodic} with $N=1$ and the degenerate function $m$ was treated in \cite{PZ2018}; see Theorem 2.13 there.


The rest of our paper is organized as follows. In Section 2, we will give the proofs of Theorem \ref{nonnegative}-\ref{degenerate}. In Section 3, we will consider the one-dimensional problem and obtain a complete characterization of the asymptotic behavior of the principal eigenvalue, and some typical examples will also be used to demonstrate the impact of boundary conditions and the degeneracy of $m$.
...
\mu(\Sigma_4)=0.
 \end{equation}
Note that $\Sigma_4$ is a discrete set. It is sufficient to show that $\mu(\{x_{0}\})=0$ for each
$x_{0}\in\Sigma_{4}$.
...
With the help of Lemma \ref{trace}, we are able to obtain the following boundedness of $\{\lambda_1(\alpha)\}$ from below as $\alpha\to\infty$.
...
$$
\mbox{$[a^i,b^j]\subset[0,1]$\ \ \ with $0\leq a<b\leq 1$,\ $i,j\in\{\mathcal{N},\, \mathcal{D}\}$}
$$
...
\centering
{\includegraphics[height=1.0in]{1-1.jpg}}\ \
{\includegraphics[height=1.0in]{1-2.jpg}}\ \
{\includegraphics[height=1.0in]{2-1.jpg}}\ \
{\includegraphics[height=1.0in]{2-2.jpg}}
...
\centering
{\includegraphics[height=1.0in]{3-1.jpg}}\ \
{\includegraphics[height=1.0in]{3-2.jpg}}\ \
{\includegraphics[height=1.0in]{4-1.jpg}}\ \
{\includegraphics[height=1.0in]{4-2.jpg}}
...
-{{\ell_1}\over{\hbar_1}}w_\alpha^2(0)\leq C,\ \ \int_0^1[(w_\alpha'-\alpha w_\alpha m')^2dx\leq C,\ \ \forall \alpha\geq0.
  \end{equation}
Combined with \eqref{4.1i} and the fact that $m$ is increasing on $[0,1]$, one can use the same analysis as in the proofs of \cite[Lemma 2.6 and Lemma 2.8]{PZ2018} to conclude that $\mu([0,1))=1$.
...
\bibitem{BHN2023}X. Bai, X. He and W.-M. Ni, Dynamics of a periodic-parabolic Lotka-Volterra competition-diffusion system in heterogeneous environments, {\em J. European Math. Soc. (JEMS)}, to appear, 2022.
...
\newblock Asymptotics of the principal eigenvalue for an elliptic operator with general advection, in preparation, 2022.
