...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{\LARGE \bf Long-Term Indoor Localization \\ with Metric-Semantic Mapping using a Floor Plan Prior}
...
\author{Nicky Zimmerman$^{\star}$ \and Matteo Sodano$^{\star}$ \and Elias Marks \and Jens Behley \and Cyrill Stachniss% <-this % stops a space
  ...
  \thanks{All authors are with the University of Bonn, Germany. Cyrill Stachniss is additionally with the Department of Engineering Science at the University of Oxford, UK, and with the Lamarr Institute for Machine Learning and Artificial Intelligence, Germany.}%
  \thanks{This work has been funded 
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.99\linewidth]{pics/3dmap.png}
...
The main contribution of this paper is a global localization and object-based mapping system using 3D semantic information.
We address the difficulty of creating 2D and 3D labels by proposing an efficient method for label generation from RGB frames.
We analyze the performance of our detector, create a probabilistic detection model that benefits both, map creation and object-based global localization.  
We utilize 3D object detection to construct object-centric maps, as seen in \figref{fig:motivation}, augmenting readily-available floor plan maps with object information. 
We provide a global localization system for the pre-built object-based map with a sensor model for 3D object information, relying solely on monocular cameras. 
%%%%%%%%%%%%%%%%%%%
...
Many works were aimed at object-aware mapping~\cite{grinvald2019ral}~\cite{kostavelis2015jras} or simultaneous localization and mapping (SLAM)~\cite{hoang2020ral}~\cite{mccormac20183dv}~\cite{stuckler2012iros} with RGB-D cameras. 
...
Long-term global localization in changing environments is a challenging task. Researchers investigated different sources of information such as textual cues~\cite{cui2021iros}~\cite{zimmerman2022iros}, wifi signal strength~\cite{miyagusuku2018ral},
RF-ID signal strength~\cite{joho2009icra}, and semantic cues to support pose estimation. 
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.99\linewidth]{pics/overview.pdf}
...
\begin{figure}[t]
  ...
  \caption{We use 2D object detection to create better 3D annotations. Top: when rendering the GT objects
  based on the camera parameters, we have no information about dynamic objects like closed doors, which would result in wrong annotations. The 2D object detection detects a door, and none of the objects in the 3D map. Therefore, no 3D annotations 
  are generated for this image (faded colors). Bottom: the 2D object detection detects the tables and boards, but does not
detect the drawers due to occlusion, therefore 3D annotations are generated only for the boards and the tables. }
  \label{fig:rendering}
\end{figure}
...
Given a posed RGB image, we project the 3D bounding boxes onto the camera frame using
\begin{align}
...
For every object detected using the 2D object detector, we match a previously-annotated 3D bounding box based on the semantic class and the IoU~\cite{everingham2015ijcv} between the projected and the detected 2D bounding box.
...
Using these labels, we fine-tune a 3D object detector which is used in the following sections. We choose Cube R-CNN~\cite{brazil2022arxiv}, but our implementation allows integration of any other 3D object detector with the same output structure. Further details about the architecture and training procedure can be found in \secref{sec:exp}.
...
For every class, we take the matched predictions and project their center on the 2D ground plane. Additionally, we discretize the 2D plane into cells of $0.05 \, m$ creating a histogram of center occurrences. Then, we fit a Gaussian distribution
\begin{align}
...
\begin{table*}[t] 
  \caption{Computed metrics for the two constructed maps compared to the GT map obtained with a FARO 3D scan. KP was constructed based on known poses from 
...
wheel encoders, 4 cameras with a joint coverage of $360^{\circ}$ field-of-view. 
...
To extract precise ground truth information about the robot's pose, we use an external localization infrastructure based on densely placed (approx. $1 \, \mathrm{tag/m}^2$) AprilTags, covering the ceiling of each room and corridor of our lab.
...
For Cube R-CNN~\cite{brazil2022arxiv}, we fine-tuned the ResNet34-based model~\cite{he2016cvpr} the authors provide for indoor perception on the classes of our interest. We optimize with stochastic gradient descent for 100 epochs, with an initial learning rate of 0.0015 and a batch size of 12. For training, we recorded sequences T1-T7 on the second floor of our building.
...
We define convergence as the time when the estimate pose is within $0.3 \, m$ radius of the GT pose, and the orientation is within 
...
\begin{figure}[t]
  ...
  \caption{2D projection for the 3 maps. Left: GT map. Middle: KP map. Right: map built using scan matching (ICP).}
  \label{fig:maps}
\end{figure}
\subsection{Mapping} 
...
We compare our constructed maps to a ground truth (GT) map, extracted with a highly accurate 3D scanner, and annotated manually.
...
We also generated a map using sensor-based poses, estimated via ICP on 2D LiDAR scans. We aligned the estimated poses to an existing grid map, such as floor plan, by initializing the ICP with a known pose. We believe the poses could also be extracted with low-drift visual-inertial odometry pipelines~\cite{rosinol2020icra}~\cite{seiskari2022wacv}.
We evaluated the accuracy of the generated maps directly by comparing it to the GT map, considering the IoU between objects in the object-based map, and precision and recall. 
As can be seen in \tabref{tab:mapping_eval} and \figref{fig:maps}, the constructed maps are well-matched to the GT map:
the IoU scores between the GT map and our generated map are high, while precision and recall are close to $1$, indicating a low number of false positives and false negatives. 
We also evaluate the performance of robot localization based on the constructed maps. The localization accuracy for 
the KP map and the ICP map is on par with the accuracy for the GT map, see \tabref{tab:success_eval}, suggesting that our maps are well-suited for localization. 
...
and quasi-static changes such as the closing and opening of doors. 
Accuracy and success rate are reported in~\tabref{tab:success_eval}. Convergence time averaged over sequences R1-R10 is $10.9 \, s$ for GT map and $12.1 \, s$ for KP map. Qualitative results can be seen in~\figref{fig:traj}.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{pics/trajectory.pdf}
...
On our robots, we use an Intel NUC10i7FNK and a NVidia Jetson Xavier AGX, see the supplementary video. The 3D object detection runs at $\sim$$9$ FPS on the NVidia Jetson, and the sensor model executes at $60$ FPS on the NUC10i7FNK. This performance
is sufficient to construct a 3D metric semantic map and globally localize on our mobile platform. A longer video, including live demoes for both mapping and localization can be found on our GitHub repository\footnote{https://github.com/PRBonn/SIMP}.
