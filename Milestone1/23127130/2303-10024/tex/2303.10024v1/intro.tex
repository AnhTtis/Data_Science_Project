...
Among the avenues to do so, Lyapunov methods~\cite{blanchini2015set} have traditionally attracted interest in the systems-and-control community in view of their flexibility and power. Indeed, several approaches have been developed to synthesize different forms of Lyapunov functions (e.g., quadratic/quartic, piecewise affine/quadratic) for various type of systems, a task that is often accomplished through convex optimization~\cite{biswas2005survey, kothare1996robust, parrilo2000structured}, %\abnote{[convex optimization anche nel caso di funzioni basati su reti neurali??]} %\DM{~}{mi riferivo anche a quello con "other approaches"}
%,
 although other approaches have been explored, based, e.g., on neural networks~\cite{gabyetal2022} or formal verification~\cite{munser2021synthesis}. 
...
recent years~\cite{solar2006combinatorial,abate2017automated,berger2022learning,chang2019neural,dai2020counter,Abate2023Neural}.  The main idea behind CEGIS consists of designing a function $f: \mathbb{R}^n \to \mathbb{R}$ by exploiting the information contained in a growing dataset 
of \mbox{(counter-)}examples (hereinafter called $\mc C_i$ at step $i$), while making sure that $f$ %also
belongs to some prescribed set of functions $\Theta$. 
To this end, CEGIS relies on an iterative adversarial procedure that involves two actors, a \textit{learner} and a \textit{verifier}.
...
While CEGIS schemes have been successfully employed to design disparate type of (control) Lyapunov functions for different classes of systems \cite{solar2006combinatorial,abate2017automated,berger2022learning,chang2019neural,dai2020counter,Abate2023Neural},  little attention has been given to how such methods can be used to extend the capabilities of traditional robust control approaches, both in terms of enlarging their applicability and, more prominently, of reducing their computational~requirements.
...
This is achieved without introducing any kind of conservativeness due %to set overapproximation, due
either to the outer approximation of a convex %non-polytopic 
uncertainty set or the convexification in the non-convex case. % like often done by other authors~\cite{alessio2007squaring,bernardini2011stabilizing}.
