...
\newcommand{\MaD}[1]{\textcolor{orange}{Marco: #1}}
\newcommand{\mad}[1]{\textcolor{blue}{Lei: #1}}
\newcommand{\IO}[1]{\textcolor{red}{Isabel: #1}}
...
\author{
%\vspace{-0.3cm}
Marco Drewes$^{a}$\footnote{marco.drewes@uclouvain.be}, Lei Ming$^{b,c}$\footnote{minglei@mail.sysu.edu.cn}, Isabel Oldengott$^{a}$\footnote{isabel.oldengott@uclouvain.be} \\
%\vspace{-0.3cm}
\\
...
However, very little is known about the mechanism that may have driven the accelerated expansion. A wide range of theoretical models of inflation exist, but the observational data is not sufficient to clearly single out any of them. Moreover, even less is known about how a given model of inflation should be embedded into a more fundamental microphysical theory of nature. Understanding this connection would be highly desirable from the viewpoints of both cosmology and particle physics. 
 ...
 In the next decade the observational situation will change drastically. Upgrades at the South Pole Observatory \cite{Moncelsi:2020ppj}
and the Simmons Observatory \cite{SimonsObservatory:2018koc} aim at pushing the uncertainty in the scalar-to-tensor ratio $r$ down to $\sigma_r \sim 3\times 10^{-3}$. 
In the 2030s JAXA's LiteBIRD satellite \cite{Sugai:2020pjw} and the ground-based  CMB Stage 4 (CMB-S4) program \cite{CMB-S4:2020lpa} can further reduce this to $\sigma_r \sim  1 \times 10^{-3}$ and $\sigma_r \sim  0.5 \times 10^{-3}$.
These missions will be sensitive to the impact of reheating on CMB observables \cite{Martin:2014rqa}, caused by the impact of the reheating process on the redshifting of metric  perturbations  \cite{Martin:2010kz,Adshead:2010mc,Easther:2011yq}. 
It has been estimated in \cite{Drewes:2019rxn} that this accuracy should be sufficient to measure\footnote{Here and in the following we use the term \emph{measurement} if the one-$\sigma$ interval of the posterior is smaller than the range of values that a given parameter can take, i.e., data imposes both an upper and a lower limit on that parameter.} the order of magnitude of individual coupling constants in models with a plateau potential, a class of models that is currently favoured by observations.
In \cite{Drewes:2022nhu} a simple semi-analytic Bayesian approach was used to confirm this.  %in RGI and $\alpha$-T models.
This calls for a more detailed evaluation of the perspectives to determine the inflaton coupling from CMB data in different models of inflation.
 ...
 In the present work we investigate the possibility to obtain information on the reheating epoch from future CMB observations through the impact that cosmic reheating has on the redshifting of cosmological perturbations. 
...
couplings that connect $\Phi$ to other fields in a model-independent way. Here model-independent means that one can obtain a constraint on the inflaton coupling constants 
...
We perform  Monte Carlo Markov Chain (MCMC) based forecasts using %\texttt{Monte Python}
{\sc MontePython}
\cite{Audren:2012wb,Brinckmann:2018cvx} and compare it to the semi-analytic approach pursued in \cite{Drewes:2022nhu}. 
In doing so we also include the observational uncertainty in the amplitude of spectral perturbations $A_s$, which had been fixed to its best fit value in \cite{Drewes:2022nhu}.
...
In the appendices we comment on the choice of the pivot scale (appendix \ref{Sec:PivotScale}), review the conditions under which the inflaton coupling can be constrained from CMB data without having to specify all details of the underlying particle physics models (appendix \ref{Sec:FeedbaclEffects}), comment on the difficulty to fulfil those conditions in $E$-type $\alpha$-attractor models (appendix \ref{JinURevisited}), and present a large set of plots to illustrate the relations between various parameters and observables in the models we consider (appendix \ref{Sec:AppendixWithAllPlots}).


\begin{figure}
\centering
\subfloat[]{
\includegraphics[width=0.49\linewidth]{mutated hilltop inflation/VVV1.pdf}}
\subfloat[]{
\includegraphics[width=0.49\linewidth]{mutated hilltop inflation/VVV2.pdf}}	
\caption{The shape of the potential for the mutated hilltop inflation (MHI) model, the radion gauge inflation (RGI) model and the $\alpha$-attractor T ($\alpha$-T) model 
...
\section{Review of the formalism}\label{Sec:OlleKamellen}
...
axion inflation \cite{Cai:2015soa,Takahashi:2019qmh},
...
Firstly, we for the first time use fully fleshed MCMC forecasts to estimate the sensitivity of LiteBIRD and CMB-S4 to $\Treh$ in all three classes of models under consideration.
...
Ignoring the running couplings during inflation and reheating, specifying $\V(\varphi)$ fixes the set of coefficients $\{\vv_i\}$ of all operators in the  action that can be constructed from $\Phi$ alone. In the models considered here all coefficients $\{\vv_i\}$ are fixed by two parameters $M$ and $\alpha$. 
...
In the models we consider here there are two free parameters in each potential. One of them indicates the height of the plateaus in Fig.~\ref{VVV}.
...
The third relevant scale is energy density at the beginning of the radiation dominated epoch, conventionally parameterised in terms of $\Treh$, which necessarily depends on the inflaton couplings $\{\g_i\}$, as they govern the efficiency of the reheating process.
We assume that one interaction with coupling constant $\g\in\{\g_i\}$ dominates the reheating process. 
Our goal is then to investigate the constrain the microphysical parameters $\{M,\alpha,\g\}$ that can be obtained with  CMB-S4 \cite{CMB-S4:2020lpa} and LiteBIRD \cite{LiteBIRD:2022cnt}.
We restrict ourselves to the observables $\{n_s,A_s,r\}$, the relation of which to $\Treh$ is well-described by the above equations, and which can readily be implemented in forecasts.\footnote{In the future, further observables may help improving the situation \cite{Achucarro:2022qrl,Komatsu:2022nvu}.
...
and can provide information on reheating \cite{Martin:2016oyk}, especially if $r$ is small \cite{Easther:2021eje}.  Also non-Gaussianities which will be probed with CMB observations, galaxy surveys and 21cm observations  \cite{Renaux-Petel:2015bja,Sekiguchi:2018kqe,Achucarro:2022qrl}.}
...
\label{slowrollpara}
\end{equation}
 by the field value $\varphi_{\rm end}$ with $\epsilon|_{\varphi_{\rm end}}=1$. 
...
For a given model of inflation the power spectrum of cosmological perturbations at the end of inflation is fixed in terms of the parameters $\{\vv_i\}$ in the potential $\V(\varphi)$. 
...
We follow the approach taken in section 2.2 of ref.~\cite{Drewes:2017fmn}, which is based on the derivation given in \cite{Ueno:2016dim}.  
...
\end{equation}
where $M_{pl}=2.435\times 10^{18}~{\rm GeV}$ is the reduced Planck mass, and $g_*$ and $g_{s*}$ are the effective number of degrees of freedom contributing to the energy- and entropy-densities of the plasma, respectively.  
...
We introduce the subscript notation $H_k, \varphi_k, \epsilon_k, \eta_k$ to indicate the value of the respective quantities  $H, \varphi, \epsilon, \eta$ in the moment when the mode $k$ crosses the horizon.
...
$\varphi_k$ can be expressed in terms of the CMB parameters, namely the spectral index $n_s$ and the tensor-to-scalar ratio $r$, through the relation 
...
\end{equation}
In the present work we consider three observables $\{n_s,A_s,r\}$.  
...
For the models considered in this work the solution for the set of equations relating $\{M,\alpha,\g\}$ is always invertible within the observationally allowed range for $\{n_s,A_s,r\}$. However, the current error bar on $n_s$ is too large to fit all three parameters, cf.~e.g.~Fig.~\ref{MHI ghy 1}.
...
\subsection{Constraining the reheating temperature}\label{sec:TRconstraints} 



Knowledge of $\Nreh$ and $\wreh$ permits to compute 
the energy density $\rhoreh$ at the end of reheating
\begin{equation}\label{ThisIsTheEndMyOnlyFriendTheEnd}
    \rhoreh = \rhoend\exp(- 3\Nreh(1 + \wrehbar)),
\end{equation}
by using the redshifting relation $\rho(N)=\rhoend\exp[-3\int_0^N(1+w(N'))dN']$ to write the Friedmann equation as
\begin{equation}\label{FriedmannNre}
    H^2=\frac{\rho_{\rm end}}{3M^2_{pl}}e^{-3N_{\rm re}(1+\bar{w}_{\rm re})}. 
\end{equation}
The energy density at the end of inflation $\rhoend$ is also determined by the parameters of the inflationary model 
...
\begin{eqnarray}\label{TR}
    ...
    \rhoreh.
\end{eqnarray}
$\Treh$ can be identified with a physical temperature in the thermodynamic sense if the decay products reach local thermal equilibrium quickly, cf. refs.~\cite{Mazumdar:2013gya,Harigaya:2013vwa,Harigaya:2014waa,Mukaida:2015ria} for a discussion, otherwise it should be interpreted as an effective parameter that parameterises the energy density $\rho_{\rm re}$.
Assuming that reheating happens instantaneously when $\GG = H$, one obtains the standard relation
\begin{eqnarray}
...
by plugging \eqref{Nre} with \eqref{Nk} into \eqref{Tre}, 
...
\subsection{Constraining the inflaton coupling}\label{Sec:InflatonCoupling}
...
is already of great interest from the viewpoint of particle physics because the thermal history of the universe determines the abundances of thermal relics  and is crucial for the viability of baryogenesis scenarios. 
...
%rescaling factor & $1$ & $2$ & $1$ & $768\pi^2$ \\
rescaling factor & $1$ & 1 & $\frac{1}{\sqrt{2}}$ & $8\sqrt{6}\pi$
\end{tabular}
\caption{\label{ConversionTable}
Conversion of the bounds on the Yukawa coupling $y$ into bounds on the other couplings in \eqref{WecheselWirkungen}.
Note that the simple conversion by multiplying with a number is only possible when $\GG$ is expressed in the form \eqref{GammaPerturbativeGeneric}. In particular, it does not apply to the dimensionful couplings $g_\phi$, $g$ and $\upalphatosigma$, but only to the dimensionless ratios \eqref{DimensionlessCouplings}.}
\end{table}
 

We can use the Friedmann equation \eqref{FriedmannNre} and the knowledge that reheating ends when $\GG=H$ to obtain the identity
 \begin{eqnarray}
...
\GG = \g^2 m_\phi /{\#}, 
\end{equation}
with $\g\in \{\g_i\}$ a coupling constant,  $m_\phi \in \{\vv_i\}$ the inflaton mass, and $\#$ a numerical factor. 
A set of explicit examples for interactions that lead to decay rates of this form are given in table \ref{ConversionTable} 
They include interactions of $\Phi$ with other scalars $\chi$, fermions $\psi$ with gauge interactions as well as U(1) gauge bosons with field strength tensor $F_{\mu\nu}$. 
...
\end{eqnarray}
The interactions \eqref{WecheselWirkungen}  yield $\GG$ of the form \eqref{GammaPerturbativeGeneric}, cf.~table \ref{ConversionTable}.
If the effective dissipation rate $\GG$ at the moment when it equals the Hubble rate $H$ is dominated by elementary decays, one can plug \eqref{GammaPerturbativeGeneric} into \eqref{GammaConstraint} and solve for $\g$ to translate a constraint on $\Nreh$ into one on $\g$.
...
The conditions on the $\{\vv_i\}$ and $\g$ under which these effects can be avoided have been studied in \cite{Drewes:2019rxn} and are briefly sketched in appendix \ref{Sec:FeedbaclEffects}.

When it comes to the self-interactions
we restrict ourselves to values of the parameters $\{\vv_i\}$ in $\V(\varphi)$ for which these conditions are fulfilled, as otherwise no model-independent constraint on any microphysical parameter can be derived, irrespective of the value of $\g$.
To identify the parameter range where this is guaranteed we expand 
...
\end{eqnarray}
with $\vv_3 = g_\phi/3!$ and $\vv_4 = \lambda_\phi/4!$. 
...
The conditions under which \eqref{GammaPerturbativeGeneric} can be used on the LHS of \eqref{GammaConstraint} 
read \cite{Drewes:2019rxn}\footnote{Note that there are two numerical errors in the constraints on $g_\phi$ in equation (4.7) in \cite{Drewes:2019rxn}, one forgotten factor $1/3!$ and a typo in the decimal point.}
\begin{eqnarray}
\label{avoid self PR}
\begin{tabular}{c c c}
...
%\label{avoid self PR}
\end{eqnarray}
The conditions in \eqref{avoid self PR} practically restrict us to symmetric potentials and field oscillations that are approximately harmonic with a frequency given by the inflaton mass $m_\phi$,
so that
\begin{equation}\label{EquationOfState}
\wrehbar = 0. 
\end{equation}
A very simple criterion on the potential can be imposed by demanding that 
the cubic and quartic terms are sub-dominant at the end of inflation, i.e., 
...
\end{eqnarray}
Note that the simple criterion \eqref{phi cr} represents a necessary, but not a sufficient condition, and that \eqref{avoid self PR} in general is more constraining.
...
When it comes to the inflaton coupling to other fields $\g$, we ignore the restrictions arising from the requirement to justify usage of \eqref{GammaPerturbativeGeneric} on the LHS of \eqref{GammaConstraint} in the forecasts, but indicate the region where it is not fulfilled. 
In plateau-type inflation models these restrictions can parametrically be estimated by
\begin{eqnarray}\label{YippieYaYaySchweinebacke}
|\vv_\n| \ll \left(3\pi^2  r A_s\right)^{(\n-2)/2} \ , \ 
|\g| \ll \left(3\pi^2  r A_s\right)^{\n/2},
\end{eqnarray}
with $\n$ the power at which $\Phi$ appears in a given interaction term, and we have ignored various numerical factors. 
While \eqref{YippieYaYaySchweinebacke} provides a simple criterion that can easily be compared with observational constraints, it relies on additional assumptions compared to the criteria  introduced in \cite{Drewes:2019rxn}, cf.~ appendix \ref{Sec:FeedbaclEffects}. In our analysis we use the mode general condition \eqref{CrocodileBatidaSpecial} given in that appendix.
These conditions impose a very strong constraint on couplings with $\n>1$. Luckily they are too conservative in many realistic scenarios. We briefly comment on this in Appendix \ref{Sec:FeedbaclEffects}.
Note also that 
\eqref{CrocodileBatidaSpecial} only restricts the translation of a bound on $\Nreh$ into a constraint on $\g$ 
...
\begin{figure}
    ...
    \caption{Impact of the reheating phase on the predictions of the model 
 ...
 The green disc indicates the point where $N_k=50$. 
...
%in \eqref{DimensionlessCouplings}
The model is quite strongly disfavoured by observations for any choice of the inflaton coupling.
Fig.~\ref{STRAIGHT} shows the dependence of $\g$ on $r$, indicating that a measurement with an uncertainty $\sigma_r \sim 10^{-2}$ would be sufficient to pin down the order of magnitude of $\g$. However, in most realistic models the dependence of $r$ on $\g$ is weaker for smaller values of $r$, meaning that a more accurate measurement will be needed if the true value of $r$ lies within the currently favoured region. In \cite{Drewes:2019rxn} it was estimated that an uncertainty of $\sigma_r \sim 10^{-3}$ will be needed to impose a constraint on $r$, which can be achieved with LiteBIRD or CMB-S4. In the remainder of the present article we apply different methods to quantify the knowledge gain on $\g$ and $\Treh$ that can be achieved with those missions. 
...
A_s=10^{-10}e^{3.043}. 
\end{eqnarray}
Note that this is not necessarily required for the method used in Sec.~\ref{Sec:SemiAnalyticMethod}, but a simplification which we justify a posteriori in Sec.~\ref{ChorPasseMole}.
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{mutated hilltop inflation/MHI_yru.pdf}
    \caption{The relation between the inflaton coupling $\g$ and $\alpha$ in the MHI model for 
different choices of $n_s$ marking 
the $1\sigma$ and $2\sigma$ confidence interval of $n_s$ ~\cite{Planck:2018vyg}. 
...
The potential of the mutated hilltop inflation (MHI) model~\cite{Pal:2009sd,Pal:2017bmd} has the form
\begin{equation}
\label{MHI V}
 \V(\varphi)=M^4\left[1-\frac{1}{{\rm cosh}(\varphi/\mu)}\right]~
 =M^4\left[1-\frac{1}{{\rm cosh}(\alpha\varphi/M_{pl})}\right]~.   
\end{equation}
Here $M$ represents the typical energy scale for hilltop inflation and $\mu$ is a parameter which has the dimension of mass. It is often convenient to use the dimensionless parameter $\upmutobealpha\equiv\mu/M_{pl}$ instead.
...
\begin{equation}\label{mutatedlamda}
    m_\phi=\frac{M^2}{\mu}=\frac{M^2}{\alpha M_{pl}}~,\quad g_\phi = 0~, \quad \lambda_\phi=-\frac{5M^4}{\mu^4}=-\frac{5M^4}{(\alpha M_{pl})^4}~.
\end{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=0.69\linewidth]{mutated hilltop inflation/MHI_ratios.pdf}
    \caption{Graphic representation of the conditions on the self-coupling $\lambda_\phi$ in the MHI model, which is a function of $\alpha$. 
    We show $\varphi_{\rm end}/\varphi_{\rm cr}$, $\lambda_\phi/{\rm RHS}_1$ and $\lambda_\phi/{\rm RHS}_2$, where ${\rm RHS}_{1,2}$ represent the RHS of the two inequalities in \eqref{avoid self PR} as well as  \eqref{phi cr}. These conditions roughly require $\mu > M_{pl}$, i.e., $\alpha>1$.}
    \label{MHI ratios}
\end{figure}
...
\begin{equation}
    M=M_{pl}\sqrt[4]{\frac{3\pi^2r A_s}{2[1-1/{\rm cosh}(\varphi_k/\mu)]}},	
    \label{M1}
\end{equation}
consistent with \eqref{InflatonMass}.
We do not find an analytic solution for the set of two equations \eqref{TakaTukaUltras} to express $(\varphi_k,\alpha)$ in terms of $\{r,n_s\}$, but we find numerically that such solution exists and is unique within the observationally allowed range of $\{r,n_s\}$ and for the values of $\alpha$ that can be made consistent with the conditions
...
Using  and \eqref{Nk}, \eqref{EquationOfState} and   \eqref{Nre} one can in addition determine $N_k$ and $\Nreh$. Knowledge of $\Nreh$ already permits to determine the reheating temperature \eqref{Tre}.
...
\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.47\linewidth]{mutated hilltop inflation/MHI_Nre.pdf}
		%\caption{The e-folding number of reheating $N_{\rm re}$ in mutated hilltop inflation}
		\label{MHINre}
	}
\quad
\subfloat[]{
	\includegraphics[width=0.47\linewidth]{mutated hilltop inflation/MHI_Nk.pdf}
	%\caption{The tensor-to-scalar ratio $r$ in mutated hilltop inflation, with $\mu$ being super-Planckian}
	\label{MHI Nk}
}
	\quad
	\subfloat[]{
		\includegraphics[width=0.47\linewidth]{mutated hilltop inflation/MHI_r.pdf}
		%\caption{The tensor-to-scalar ratio $r$ in mutated hilltop inflation, with $\mu$ being sub-Planckian}
		\label{MHIr}
	}
\quad
\subfloat[]{
	\includegraphics[width=0.47\linewidth]{mutated hilltop inflation/MHI_Tre.pdf}
	%\caption{The reheating temperature $T_{{\rm re}}$ in mutated hilltop inflation}
	\label{MHITre}
}
 ...
 The colour coding is used to denote different values of $\alpha$.}
	\label{MHI 4}
\end{figure}
...
However, in practice this is difficult even when $r$ is determined by future missions like LiteBIRD and CMB-S4.
 ...
 $\alpha$ for different values of $n_s$.
The different values of $n_s$ correspond to the range of $1\sigma$ and $2\sigma$ confidence interval of $n_s$~\cite{Planck:2018vyg}. 
...
We choose this parameter to be $\alpha=\mu/M_{pl}$.
This is possible because fixing $\alpha$ does not uniquely fix $n_s$ and $r$, but only restricts predictions a line in the $n_s$-$r$ plane, the position along which is determined by the value of the inflaton coupling $\g$.
...
$\alpha=1$, ~$5$, ~$10$ and $50$.
Combining equations~(\ref{Nre})-(\ref{nANDr}) as well as~(\ref{Tre}) we obtain the $n_s$-dependence of four variables in the MHI model: the e-folding number of reheating $\Nreh$, the e-folding number after horizon crossing $N_k$, the tensor-to-scalar ratio $r$ and the reheating temperature $\Treh$, as shown in Fig.~\ref{MHI 4}. These variables have an analytic but complicated dependence on $\alpha$ and $n_s$, thus in our work we present the numerical results for different relations in the form of plots.
...
As an example, Fig.~\ref{MHI R d} shows the impact of the reheating era on the observables $n_s$ and $r$ for different values of $\alpha$, assuming an Yukawa coupling. 
...
The relation between the inflaton coupling and $n_s$ is shown in more detail in
Fig.~\ref{MHI g 1 a}, where we have fixed $\alpha = 5$ and again assumed an Yukawa coupling.
Results for other choices of $\alpha$ and other inflaton interactions are shown in 
Figs.~\ref{MHI g}, \ref{MHI h},  \ref{MHI y} and \ref{MHI a} in the appendix.
...
This relation is approximately but not exactly linear because  the inflaton mass $m_\phi$ changes slightly for different choice of parameters.
...
In the following sections we use two methods to quantify the information gain on $\Treh$ and $\g$ that can be achieved with CMB-S4 and LiteBIRD.
...
\begin{figure}
    \centering
 	\includegraphics[width=0.7\linewidth]{mutated hilltop inflation/MHI_Ry.pdf}
 	\caption{Impact of the reheating phase on the predictions of MHI model 
 	for $n_s$ and $r$ for a set of sample points. Here we assume an Yukawa inflaton coupling $y$;
 	we show results for all couplings in table~\ref{ConversionTable} in Fig.~\ref{MHI R} in the appendix.
 	The red and blue areas display  marginalized joint confidence regions for $(n_s, r)$ at the $1\sigma$ $(68\%)$ CL (dark) and $2\sigma$ $(95\%)$ CL (light) from Planck+BICEP/Keck 2015 and 2018 data, respectively (Fig.~28 in~\cite{Planck:2018vyg} and Fig.~5 in~\cite{BICEP:2021xfz}, respectively). The predictions of the MHI model with  different values of $\alpha$ and $y$ are indicated by the discs. The colour of the disc represents the value of $\alpha$, and the values of ${\rm log}_{10}(y)$ are given by the numbers above the discs. These values have been chosen consistent with condition \eqref{CrocodileBatidaSpecial}; predictions for other values of $y$ lie on the curve of the same colour, as the choice of $\alpha$ defines a line in the $n_s$-$r$ plane.
	The grey region on the left side corresponds to the lower bound  $T_{\rm re} < 10$ MeV from BBN in \eqref{TreTBBN}, and the grey region on the right side corresponds to $N_{\rm re} < 0$. The dotted line corresponds to $N_k=50$, which guarantees a sufficient duration of inflation, as specified later in Sec.~\ref{Sec:CurrentKnowledge}.  }
		\label{MHI R d}
\end{figure}
...
\begin{figure}
    \centering
 	\includegraphics[width=0.71\linewidth]{mutated hilltop inflation/MHI_y_5.pdf}
 	\caption{
	...
	The grey region on the left side corresponds to  $T_{\rm re}<10~{\rm MeV}$ and is excluded by BBN, and the grey region on the right side corresponds to $N_{\rm re}<0$. The dotted line indicates the electroweak sphaleron freeze-out temperature $T_{\rm re}=131~{\rm GeV}$. The region to the right of the dashed line satisfies the requirement  $N_k>50$.
	The yellow region, inside which the parametric resonance occurs, gives the upper bound on the inflaton coupling $\g$ from the condition to avoid $\{\SM_i\}$-dependencies due to feedback caused by resonant particle production. To identify this region we used the more general conditions \eqref{CrocodileBatidaSpecial} instead of \eqref{YippieYaYaySchweinebacke} and neglected a mild dependence on $r$, cf.~footnote \ref{SimpleResonanceFootnote}.
...
\begin{figure}
    \centering
 	\includegraphics[width=0.7\linewidth]{mutated hilltop inflation/newplot.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{radion gauge inflation/RGI_yru.pdf}
    \caption{The relation between the inflaton coupling $\g$ and $\alpha$ in the RGI model for 
different choices of $n_s$ marking 
the $1\sigma$ and $2\sigma$ confidence interval of $n_s$. 
We here chose the Yukawa coupling $\g=y$, 
with conventions as in Fig.~\ref{MHI ghy 1}. Plot for other couplings are shown in Fig.~\ref{RGI ghy} in the appendix.}
\label{RGI ghy 1}
\end{figure}

The potential of the radion gauge inflation (RGI) model has the form~\cite{Fairbairn:2003yx,Martin:2013nzq}
\begin{equation}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.47\linewidth]{radion gauge inflation/RGI_Nre.pdf}
		%\caption{The e-folding number of reheating $N_{\rm re}$ in mutated hilltop inflation}
		\label{RGINre}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.47\linewidth]{radion gauge inflation/RGI_Nk.pdf}
		\label{RGINk}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.47\linewidth]{radion gauge inflation/RGI_r.pdf}
	%\caption{The tensor-to-scalar ratio $r$ in mutated hilltop inflation, with $\mu$ being sub-Planckian}
	%\caption{$r$ in RGI..}
	\label{RGIr}
    }
    \quad
    	\subfloat[]{
    	\includegraphics[width=0.47\linewidth]{radion gauge inflation/RGI_Tre.pdf}
    	%\caption{The reheating temperature $T_{{\rm re}}$ in mutated hilltop inflation}
    	\label{RGITre}
    }
...
\begin{figure}
    \centering
 	\includegraphics[width=0.7\linewidth]{radion gauge inflation/RGI_Ry.pdf}
 	\caption{
  Impact of the reheating phase on the predictions of RGI model for $n_s$ and $r$ for a set of sample points and an Yukawa coupling $y$, with conventions as in Fig.~\ref{MHI R d}. Results for other couplings in table~\ref{ConversionTable} in Fig.~\ref{RGI R} in the appendix.}
		\label{RGI R d}
\end{figure}


The range of allowed values for $\alpha$ is restricted by the conditions to avoid feedback during reheating.
Near the minimum $\varphi=0$ of the potential the inflaton mass $m_\phi$ and self-couplings are, 
...
For fixed $\alpha$ we can get the $n_s$-dependence of $N_{\rm re}$, $N_k$, $r$ and $T_{\rm re}$ in RGI model, as shown in Fig.~\ref{RGI 4}.
...
The corresponding values of ${\rm log}_{10}\g$ are indicated by the numbers above the discs, and different values of $\alpha$ are distinguished by color coding (same as in  Fig.~\ref{MHI R d}).
...
\begin{figure}
    \centering
 	\includegraphics[width=0.71\linewidth]{radion gauge inflation/RGI_y_100.pdf}
 	\caption{
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{alpha T model/alpha_T_yru.pdf}
    \caption{The relation between the inflaton coupling $\g$ and $\alpha$ in the $\alpha$-T model for 
different choices of $n_s$ marking 
the $1\sigma$ and $2\sigma$ confidence interval of $n_s$. 
We again chose the Yukawa coupling $\g=y$, 
with conventions as in Fig.~\ref{MHI ghy 1}. Plot for other couplings are shown in Fig.~\ref{RGI ghy} in the appendix.}
\label{alpha T ghy 1}
\end{figure}
...
\centering
\subfloat[]{
\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Nre}
\label{alpha T Nre}
}
\quad
\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Nk}}
\quad
\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_r}
	\label{alpha T r}
}
\quad
\subfloat[]{
	\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Tre}
	\label{alpha T Tre}
}
...
\right)
\end{eqnarray}
To ensure a parabolic potential near the minimum and fulfil condition \eqref{phi cr} we have to choose $n=1$.  
...
\begin{figure}
    \centering
 	\includegraphics[width=0.7\linewidth]{alpha T model/alpha_T_Ry.pdf}
 	\caption{
...
The corresponding values of ${\rm log}_{10}\g$ are indicated by the numbers over the discs, and different choices of $\alpha$ distinguished by color of the discs, following the notation of Fig.~\ref{MHI R d}.
...
\begin{figure}
    \centering
 	\includegraphics[width=0.71\linewidth]{alpha T model/alpha_T_y_10.pdf}
 	\caption{
...
Our task is straightforwardly phrased in the language of Bayesian statistics. We have some knowledge on a set of fundamental parameters $X$ from observation and theoretical consistency specified in Sec.~\ref{Sec:CurrentKnowledge} that we can use to define a prior $P(X)$. 
...
This permits to identify the physical scales via a mapping
\begin{equation}
...
The inflaton coupling $\g$ can in general only be extracted from data if many parameters $\{\SM_i\}$ in the underlying particle physics model are specified.
However, if conditions in appendix \ref{Sec:FeedbaclEffects} are fulfilled, then the formalism summarised in Sec.~\ref{Sec:InflatonCoupling} further permits to determine $\g$ unambiguously, i.e., there is a mapping
\begin{equation}
...
One result that is common to all classes of models in Sec.~\ref{sec:Benchmark models and analytic estimates} is that current and near-future observational information on the quantities $\{A_s,n_s,r\}$ will not be sufficient to simultaneously fit all microphysical parameters $\{M,\alpha,\g\}$ because the error bar on $n_s$ is too large.  This can e.g.~be seen by comparing Figs.~\ref{MHI ghy 1}, \ref{RGI ghy 1} and \ref{alpha T ghy 1} to Fig.~\ref{fig:my_label}. In terms of energy scales this means that one cannot observationally determine $\{M,m_\phi,\Treh\}$  within a class of models without making at least some additional assumptions. It may be that the situation improves if further properties of the CMB spectra are taken into account (such as the running of $n_s$ or non-Gaussianities). However, since the relation between those properties and the microphysical parameters $\{M,\alpha,\g\}$ has not been studied in detail yet, we take the approach to consider families of inflationary models with fixed $\alpha$. This establishes a relation between the inflaton mass $m_\phi$ and the scale of inflation $M$, with the latter being the sole free parameter in the potential $\V(\varphi)$ in each family of models.
Since fixing $\alpha$  fixes the inflaton mass as a function of $M$ (cf.~\eqref{mutatedlamda}, \eqref{InflatonMassRGI}, \eqref{alphaTlambda}), we practically determine $\{M,\g\}$ from data, and $m_\phi$ is then fixed by $M$ and the choice of $\alpha$. Further, fixing $\alpha$ establishes a relation between $n_s$ and $r$, 
...
$M$ and $\g$ vary along this line, and the accuracy by which they can be constrained depends on the sensitivity of future experiments to $n_s$ and $r$. In practice the  information gain that can be achieved with CMB-S4 and LiteBIRD will primarily be driven by their sensitivity to $r$.\footnote{If one fixes $A_s$ to \eqref{AsBestFit} as done in \cite{Drewes:2022nhu}, then $M$ and $\g$ are uniquely fixed for given $r$. In practice the error bar on $A_s$ will not introduce a considerable uncertainty, as we will see in Fig.~\ref{cp_RGI}.} 
...
\end{itemize}
Our choice of values is summarised in table \ref{tab:priors} together with other parameters relevant for the method described in Sec.~\ref{Sec:ForecastMethod}. 

\begin{table}
\centering
\begin{tabular}{c||c|c|c|c|c|c}
model & $\alpha$ & ${\rm log}_{10}(\bar{y})$ & prior range ${\rm log}_{10}(y)$ & $\bar{M} [M_{\text{pl}} ]$ & prior range $M [M_{\text{pl}}]$ &  $\bar{n}_s$\\
\hline
\hline
...
\end{tabular}
\caption{Fiducial values of ${\rm log}_{10}(y)$ and $M$ (${\rm log}_{10}(\bar{y})$ and $\bar{M}$) and their prior ranges, for the different models under consideration. }
\label{tab:priors}
\end{table}
...
\begin{itemize}
\item \textbf{Model choice and parametrisation}. Even if no explicit prior $P(X)$ is imposed, the parametrisation of $\V$ in terms of the $\{\vv_i\}\subset X$ induces a metric in the mapping between $\mathcal{D}$ and $X$, and $P(\mathcal{D}|X)$ in general is not invariant under changes in the parametrisation. While this is not represent a Bayesian prior in the strict sense, it does reflect prior assumptions about what the fundamental parameters in nature are, and this has an impact on the posteriors.
    \item \textbf{Overall geometry of the universe.} 
    One of the main motivations for cosmic inflation comes from the observation that the observable universe is homogeneous and isotropic on large scales.  Explaining this observation imposes a lower bound on the duration of inflation in terms of $e$-folds, and hence also a lower bound on $N_k$.  The precise value of this bound depends a bit on the specific problem that one wants to solve (cf.~e.g.~\cite{Kolb:1990vq,Liddle:2000cg,Mukhanov:2005sc}). 
  For the sake of definiteness we impose
\begin{eqnarray}\label{NkCondition}
    N_k > 50,
\end{eqnarray}
which is consistent with the lower bound has been used by the Planck collaboration, cf.~Fig.~8 in \cite{Planck:2018jri}.\footnote{We do not impose the upper bound on $N_k$ quoted by Planck (which is based on \cite{Liddle:2003as}) because the formalism we employ automatically guarantees consistency in this regard.}
Note that, while the precise value used in \eqref{NkCondition} can be a matter of debate,
conceptually this is a hard observational bound (based on the observation that the universe is homogeneous, isotropic, and spatially flat), and not a theoretical criterion.\footnote{Of course, one may argue that values of $N_k$ much smaller than \eqref{NkCondition} are strictly speaking not observationally excluded. In such scenarios inflation simply fails to do its job, and the homogeneity and isotropy of the CMB remain unexplained. We adapt the viewpoint that inflation as a paradigm draws is motivation from the requirement to solve these puzzles, making \eqref{NkCondition} an observational bound on the theory.} 
It should also be noted that this piece of information does not come from any of the study of the spectrum of CMB perturbations, but can be inferred from the high level of isotropy of the CMB, which was known \emph{prior} to the missions that studies the spectrum of perturbations. 
\item \textbf{Previous measurements of the CMB power spectrum.}
The power spectrum of CMB anisotropies has been studied by a number of probes, with the most recent one being the combined analysis of Planck, WMAP, and BICEP/Keck observations in \cite{BICEP:2021xfz}. This data was obtained from the same CMB that will be studied by CMB-S4 and LiteBIRD, hence it strictly speaking does not represent independent information. To avoid complications related to combining correlated data in a Bayesian analysis we refrain from including this information into our priors. Practically this has little consequences, as it has been estimated in \cite{Drewes:2022nhu} that the bounds on $r$ and $n_s$ reported in \cite{BICEP:2021xfz} do not impose a significant constraint on $\g$ or $\Treh$.
\item \textbf{Requirement to actually reheat the universe.}
...
However, given our ignorance about baryogenesis and Dark Matter, we impose the conservative bound $\Treh > T_{\rm BBN}$. A lower bound $\g$ from any other lower bound on $\Treh$ can be obtained trivially from \eqref{BBNsimple} by replacing $T_{\rm BBN}$ with the respective temperature.
\item \textbf{Physical consistency}. Physical consistency obviously requires $\Nreh\geq 0$.
In contrast to \eqref{NkCondition} this constraint has no uncertainty or tolerance (cf.~footnote \ref{NkUncertaintyComment}), but must be modelled by a prior $P(\Nreh)\propto \theta(\Nreh)$. Conceptually it may be more appropriate to view this as a condition that defines the boundaries of the range of  values for $\{M,\alpha,\g\}$ (and not as a prior, as it is not derived from any prior observation, but from physical consistency alone).
\end{itemize}
...
In this method the set of observables $\mathcal{D}$ in principle comprises $A_s$, $n_s$ and $r$. 
...
This is not strictly necessary for this method to be applied, but conveniently reduces \eqref{BayesFormula} to a one-dimensional integral with  $X={\rm log}_{10}\g$ and $\mathcal{D}=\{n_s,r\}$
(as $A_s$, $n_s$, $r$, $N_k$ and $\Nreh$
are functions of $\g$ alone, leaving aside the small disclaimers mentioned in footnote \ref{WhatWeNeglect}).
We a posteriori find that the validity of the method is not significantly reduced by this simplification, cf.~Fig.~\ref{cp_RGI}.
...
P(\x)  
= c_1 P\big(N_k(\x)\big) \ \gamma(\x) \ \theta\big(\Nreh(\x)\big) \ \theta\big(\Treh(\x) - T_{\rm BBN} \big),
\end{eqnarray}
with 
$P(N_k)\propto \theta(N_k-50)$,\footnote{\label{NkUncertaintyComment}In \cite{Drewes:2022nhu} the uncertainty in the bound \eqref{NkCondition} was accounted for by using a prior $P(N_k)\propto \frac{1}{2}\big(\tanh(N_k - 50) + 1\big)$ instead of a hard cut.
}  
...
\begin{figure}
    ...
    The diagonal line is given by \eqref{珍珠奶茶} with $\alpha=19$. The colourful part of this line corresponds to the region where $N_k>50$ and $\Nreh>0$. In the green part in addition \eqref{CrocodileBatidaSpecial} is fulfilled. All other conventions as as in Fig.~\ref{MHI R d}. }
    \label{fig:my_label}
\end{figure}
...
As explained in Sec. \ref{sec:Benchmark models and analytic estimates}, the fundamental parameters ${\rm log}_{10}(y)$, $M$ and $\alpha$ can be mapped into the phenomenological parameters $A_s$, $n_s$ and $r$ which are conventionally used in the context of the CMB. For the reasons discussed in Sec.~\ref{sec:choosing_alpha}, a simultaneous measurement of
all three parameters $\{M,\alpha,\g\}$ from $\{A_s,n_s,r\}$ 
will not be possible neither with LiteBIRD nor with CMB-S4. Therefore, we fix the value of $\alpha$ according to the criteria described at the end of Sec.~\ref{sec:choosing_alpha} while ${\rm log}_{10}(y)$ and $M$ are allowed to vary freely in our analysis. The corresponding choices for $\alpha$ for the different models considered in this work are summarised in Tab.~\ref{tab:priors}. 
...
\begin{equation}
X=   \lbrace \omega_{\mathrm{b}}, \omega_{\mathrm{cdm}}, 100 \theta_{\mathrm{s}}, \tau_{\mathrm{reio}} \rbrace  + {\rm log}_{10}(y) + M \, ,
...
We modified the linear Boltzmann-Einstein solver {\sc class} \cite{Blas:2011rf} (which calculates the CMB anisotropy spectrum) such that it takes ${\rm log}_{10}(y)$ and $M$ as free input parameters while the values of $A_s$, $n_s$ and $r$ are inferred from them. This requires to create numerical tables for $A_s$, $n_s$ and $r$ on a grid of ${\rm log}_{10}(y)$ and $M$ values. The step size both of $M$ and ${\rm log}_{10}(y)$ was chosen carefully such that it corresponds to less than $0.2$ of the expected standard deviations in $A_s$, $n_s$ and $r$. The range of the covered ${\rm log}_{10}(y)$ and $M$ values was chosen such that it covers around $10$ of the expected standard deviations in $A_s$, $n_s$ and $r$. We implemented a routine in {\sc class} that interpolates between the ${\rm log}_{10}(y)$ and $M$ values in this table such that the code can interpret any arbitrary value of ${\rm log}_{10}(y)$ and $M$ (inside the covered range)\footnote{We therefore modified and re-used the already existing 2-dim interpolation routine of CLASS that is usually used to derive the helium fraction $Y_p$ as a function of the baryon density $\omega_b$ and the effective number of relativistic degrees of freedom $N_{\mathrm{eff}}$.}. 
...
\begin{itemize}
    \item[i)] Mock data for the fiducial model are created. The fiducial values for the 4 base parameters can be found in Tab.~\ref{tab:fiducial_model} and the fiducial values for ${\rm log}_{10}(y)$ and $M$ in Tab.~\ref{tab:priors}. The 
    values for ${\rm log}_{10}(\bar{y})$ and $\bar{M}$ are chosen in a way that one can expect the resulting value of $A_s$ near \eqref{AsBestFit} and the values of $n_s$ and $r$ inside the region allowed by current observations, based on the formulae in Sec.~\ref{sec:Benchmark models and analytic estimates}.     It has been shown in \cite{Perotto:2006rj} that scattering of the mock data (due to cosmic variance) can be neglected.
    \item[ii)] The Fisher matrix and its inverse are derived. In case of Gaussian likelihoods, the standard deviations for the parameters in Eq.~\eqref{eq:free_parameters} are directly given by the square roots of the diagonal elements of the inverse Fisher matrix. However, as shown in \cite{Perotto:2006rj}, in general this does not hold for non-Gaussian likelihoods and in particular in presence of parameter degeneracies (as is expected for ${\rm log}_{10}(y)$ and $M$). We therefore proceed with iii).
    ...
    $N_{\mathrm{re}} \gtrsim 0$ imposes a hard cut on ${\rm log}_{10}(y)$ which can be found in table \ref{tab:priors}. Even though just as well physically motivated, the precise definition of the the other priors discussed in Sec.~\ref{Sec:CurrentKnowledge} is somewhat more flexible. We therefore decided to not impose these priors at the level of creating the MCMCs but only take them into account afterwards. The prior on $M$ in Tab.~\ref{tab:priors} is of practical nature and has no physical significance (and also does not affect our results), it simply reflects the range of $M$ covered by our numerical table. Note that the translation of the conditions $\Nreh >0$ and $N_k>50$ into a boundary for ${\rm log}_{10}(y)$ in general depends on the value of $M$ (which is varied freely in our analysis). We have however checked that the prior boundary of ${\rm log}_{10}(y)$ only varies mildly over the prior range of $M$, such that we can simply ignore this dependency in our analysis. 
\end{itemize}
...
\begin{figure}
    ...
    \caption{Marginalized 1d- and 2d-posterior distributions for ${\rm log}_{10}(y)$, $M$ and the derived parameters $\Treh$, $\ln(10^{10}A_s)$, $n_s$ and $r$ for the MHI model. }
...
The results of our forecast can be found in Fig.~\ref{fig:triangle_MHI} (MHI model), Fig.~\ref{fig:triangle_RGI} (RGI model) and Fig.~\ref{fig:triangle_alphaT} ($\alpha$-T) in form of 2d- and 1d- marginalized posterior distributions for ${\rm log}_{10}(y)$ and $M$. We also show the resulting posterior distributions for the \textit{derived} parameters ${\rm ln}(10^{10}A_s)$, $n_s$ and $r$, along with the posterior distribution for the reheating temperature $T_{\mathrm{re}}$. The reheating temperature can be calculated from inserting Eq.~\eqref{GammaPerturbativeGeneric} into Eq.~\eqref{TR},
...
In general, the figures show that CMB-S4 is expected to have higher sensitivity on ${\rm log}_{10}(y)$ and $M$ than LiteBIRD -- to which extent however strongly depends on the success of delensing. The only parameter in Figs.~\ref{fig:triangle_MHI}, \ref{fig:triangle_RGI} and \ref{fig:triangle_alphaT} that is more constrained by LiteBIRD than by CMB-S4 is the scalar amplitude $A_s$. At scales below the reionization bump, $\ell \gtrsim 10$, there is a strong degeneracy between $A_s$ and the optical depth to reionization $\tau_{\mathrm{reio}}$.  
...
both will lead to an improved determination of $\Treh$ and ${\rm log}_{10}(y)$. Due to complications related to partially overlapping data sets, we however do not perform such a combined analysis.
...
\centering
\begin{tabular}{c||c|c|c}
model & ${\rm log}_{10}(y)$ & $ M [M_{\text{pl}} ]$ & ${\rm log}_{10}(T_{\text{re}}[\text{GeV}])$\\
\hline
\hline
MHI & $-5.39\pm1.52$ & $0.00516\pm0.00005$ & $9.49\pm1.51$ \\
\hline
  RGI   & $-5.56\pm1.44$ & $0.00526\pm0.00005$ & $9.34\pm1.44$ \\
 \hline 
  $\alpha$-T & $-1.70\pm1.43$ & $0.00521\pm0.00005$ & $13.13\pm1.43$ \\
\end{tabular} 
\caption{Expected 1 $\sigma$ sensitivities on the inflaton coupling $y$, $M$ and the reheating temperature $\Treh$
for LiteBIRD. The values in brackets are without delensing.
...
\hline
\hline
MHI & \makecell{$-6.28\pm0.63$ \\ \;  $\left( -5.80\pm1.19 \right)$}& \makecell{$0.00519\pm0.00002$ \\ \; $\left( 0.00518\pm0.00004 \right)$ }& \makecell{$8.59\pm0.63$ \\ \; $\left( 9.08\pm1.19 \right)$ }\\
\hline
RGI  & \makecell{$-6.67\pm0.57$ \\ \; $\left( -5.97\pm1.14 \right)$ } & \makecell{$0.00529\pm0.00002$ \\ \; $\left( 0.00527\pm0.00004 \right)$ }&\makecell{ $8.23\pm0.57$ \\ \, $\left( 8.93\pm1.14 \right)$ }\\
\hline
$\alpha$-T & \makecell{$-1.03\pm0.62$ \\ \; $\left( -1.62\pm1.32 \right)$} & \makecell{$0.00518\pm0.00002$ \\ \; $\left( 0.00520\pm0.00005 \right)$ }& \makecell{$13.80\pm0.62$ \\ \; $\left( 13.20\pm1.32 \right)$ }\\
\end{tabular}
\caption{Expected 1 $\sigma$ sensitivities on the inflaton coupling $y$, $M$ and the reheating temperature $\Treh$
for CMB-S4. The values in brackets are without delensing.
Here we considered a Yukawa coupling $y$, corresponding ranges for other types of interactions can be obtained with table \ref{ConversionTable}. 
}
...
Except for the prior $\Nreh>0$ we have not imposed any other of the priors discussed in Sec.~\ref{Sec:CurrentKnowledge} in order to produce Figs. \ref{fig:triangle_MHI}, \ref{fig:triangle_RGI} and \ref{fig:triangle_alphaT}. These figures  therefore mainly reflect the experiments' pure sensitivity towards ${\rm log}_{10}(y)$ and $M$. 
We also added enlarged figures for the 1-d posterior distribution of ${\rm log}_{10}(y)$ in Figs. \ref{cp_MHI}, \ref{cp_RGI} and \ref{cp_alphaT}, along with the different priors discussed in Sec.~\ref{Sec:CurrentKnowledge}. The top  x-axis displays the reheating temperature $\Treh$.
...
In all cases the prior $N_k>50$ turns out to be more stringent than the $\Treh >10$ MeV prior. Tab.~\ref{tab:sensitivities_litebird} displays $1\sigma$ sensitivities for ${\rm log}_{10}(y)$, $M$ and $\Treh$ \emph{after the $N_k>50$ prior has been imposed.} 
...
In the $\alpha$-T model condition \eqref{YippieYaYaySchweinebacke} is violated for the fiducial value of the inflaton coupling $\bar{y}$ that we picked in order to bring the predictions for $n_s$ and $r$ into a regime that is allowed by current observations. Hence, the values for $\g$ given in tables~ \ref{tab:sensitivities_litebird} and \ref{tab:sensitivities_cmbs4} for the $\alpha$-T model cannot be interpreted as measurements of an actual microphysical parameter, but should rather be seen as proxies for $\Treh$.


The blue discs in Fig.~\ref{alpha T R d} indicate that there are parameter choices in $\alpha$-T models for which \ref{tab:sensitivities_litebird} and \ref{tab:sensitivities_cmbs4} can be fulfilled while keeping $r$ large enough to enable a measurement of  ${\rm log}_{10}\g$ with LiteBIRD or CMB-S4 and staying within the $2\sigma$ region preferred by current constraints. 
However, since the most popular particle physics of the potential \eqref{alpha V} feature multifield effects that trigger resonant particle production anyway, we decided to chose a fiducial value of $\bar{y}$ that leads to $n_s$ and $r$ inside the current $1\sigma$ region.
...
\begin{figure}
    ...
    \caption{
    Here we present the posteriors for the MHI model: the green line is the result of the analytic method from Sec.~\ref{Sec:SemiAnalyticMethod} for LiteBIRD with fixed $A_s$. The red line is the forecast for LiteBIRD, the blue and blue-dashed lines correspond to the forecast for CMB-S4 with or without delensing, respectively (both without fixing $A_s$). We have not imposed the prior \eqref{NkCondition} while running the forecast, but instead indicate the region where $N_k<50$ in gray. When computing the sensitivities in table \ref{tab:sensitivities_cmbs4} and \ref{tab:sensitivities_litebird} the prior \eqref{NkCondition} was included. In the region labelled as \emph{feedback effects} the conditions outlined in appendix \ref{Sec:FeedbaclEffects} are not fulfilled, implying that the interpretation of the curves as posteriors for the microphysical coupling constant $y$ is questionable in this regime (while they can still be read as posteriors for $\Treh$). Here we assumed a Yukawa coupling $y$, posteriors  for other types of interactions can be obtained with table \ref{ConversionTable}.
    } 
    \label{cp_MHI}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{comparison/RGI_comparison.pdf}
...
For the analytic method we therefore set $\nsbar=0.967$, $\sigmans=0.02$, $\rbar=$ and $\sigmar$ 
in \eqref{GaussianLikelihood} (as in Fig.~\ref{fig:my_label})
 while fixing $A_s$ to \eqref{AsBestFit}. As for the MCMC forecast, we choose the values of ${\rm log}_{10}{\Bar{y}}$ and $\alpha$ as in Tab.~\ref{tab:priors}. The resulting posteriors are the green curves in Figs.~\ref{cp_MHI}, \ref{cp_RGI} and \ref{cp_alphaT}. Comparison to the results of Sec.~\ref{Sec:ForecastMethod} shows that the two methods are overall in satisfactory agreement, proving that the simple analytic method gives reliable results when being fed with realistic estimates of $\sigmans$ and $\sigmar$
for given $\nsbar$ and $\rbar$. For the RGI model and for the example of LiteBIRD we have also checked that fixing $A_s$ (as in the analytic method) would not have a significant impact on the results of the MCMC forecast: The red dashed curve in Fig.~\ref{cp_RGI} shows the ${\rm log}_{10}(y)$ posterior for a forecast with $M$ (or respectively $A_s$) and looks almost identical to the one with $M$ being varied freely. However, as can be seen in Fig.~\ref{cp_MHI}, \ref{cp_RGI} and \ref{cp_alphaT} the simple analytic method overestimates the confidence region of ${\rm log}_{10}(y)$ somewhat. This can be understood in the following way: In the analytic method the $n_s$-$r$ 2d-posterior is translated into a 1d-posterior for ${\rm log}_{10}(y)$ basically by inserting parametric expression for $n_s(x)$ and $r(x)$. In practice this method is therefore equivalent to treating $n_s$ and $r$ as free MCMC parameters while ${\rm log}_{10}(y)$ is a derived parameter. The forecasts described in Sec.~\ref{Sec:ForecastMethod} works the other way around, it assumes ${\rm log}_{10}(y)$ (and $M$ respectively) as free MCMC parameters (being the fundamental parameters of the theory) while $n_s$ and $r$ are derived parameters. Assuming ${\rm log}_{10}(y)$ to be the free MCMC parameter however only admits certain combinations of $n_s$ and $r$ values (for given $\alpha$) such that the accessible parameter space is reduced compared to the case of $n_s$ and $r$ as free MCMC parameters. This is ultimately reflected by a wider ${\rm log}_{10}(y)$ posterior for the analytic method. 
...
We studied the sensitivity of LiteBIRD and CMB-S4 to cosmic reheating in three classes of inflationary models, 
namely RGI, MHI and $\alpha$-T models. 
This sensitivity is a result of the impact that the modified expansion history during the reheating epoch has on the redshifting of cosmological perturbations. 
In each of these classes of models we considered three fundamental parameters: The scale of inflation $M$, a parameter $\alpha$ that basically determines the relation between $M$ and the inflaton mass $m_\phi$, and a coupling constant $\g$ which controls the interaction of the inflaton with other fields. 
While $M$ and $\alpha$ are parameters in the effective potential $\V(\varphi)$ that define a model of inflation, $\g$ determines the efficiency of the transfer from $\varphi$ to other degrees of freedom during reheating.
Gaining information on $\g$ probes the connection between a given model of inflation and its embedding into a more fundamental theory of particle physics. 
This is possible 
without specifying further details of the underlying particle physics model 
if feedback effects  do not considerably modify the duration of the reheating period in a way that depends  parameters other than those in the potential $\V(\varphi)$ and the inflaton coupling $\g$. 
We considered the four types of interactions summarised in table \ref{ConversionTable}, including scalar interactions, a Yukawa interaction and and axion-like coupling.
...
\item[2)] Fixing $\alpha$ defines families of inflationary models with one free parameter $M$ in the potential. In each of these families $M$ and the order of magnitude of $\Treh$ can, as a rule of thumb, be measured from a determination of $r$ at the level $\sigma_r\sim 10^{-3}$ if the true value of $r$ exceeds $10^{-2}$. 
This requires $\alpha>1$ in all models.
\item[3)] This can be translated into a measurement of the order of magnitude of the inflaton coupling $\g$ if the values of all involved coupling constants are smaller than a critical value that can be estimated in terms of half-integer powers of $r A_s$ in \eqref{YippieYaYaySchweinebacke}. 
\end{itemize} A slightly more general discussion of point 3) is presented in appendix \ref{Sec:FeedbaclEffects}. In MHI and RHI type models there is a sizeable parameter region in which these conditions can be fulfilled while being consistent with current observational data, cf.~Figs.~\ref{MHI R d} and \ref{RGI R d}. In $\alpha$-T models it is more challenging, though the blue discs in Fig.~\ref{alpha T R d} show that it is possible if multi-field effects can be neglected at the end of reheating. 

In the second part of this work we investigated whether observations with LiteBIRD or CMB-S4 (both of which are expected to have a sensitivity around $\sigma_r\sim 10^{-3}$) will be able to impose meaningful constraints on the orders of magnitude of the reheating temperature and the inflaton coupling. 
The main result of this part are presented in Figs.~\ref{fig:triangle_MHI}-\ref{fig:triangle_alphaT} and Figs.~\ref{cp_MHI}-\ref{cp_alphaT}, with the most important quantities summarised in tables \ref{tab:sensitivities_litebird} and \ref{tab:sensitivities_cmbs4}.
They were obtained by performing MCMC forecasts in which $M$ and ${\rm log}_{10}\g$ were treated as free MCMC parameters (along with the other parameters of the cosmological concordance model), as outlined in Sec.~\ref{Sec:ForecastMethod}.
The main conclusions are:
\begin{itemize}
\item[4)] Figs.~\ref{fig:triangle_MHI}-\ref{fig:triangle_alphaT} show that both, CMB-S4 and LiteBIRD can measure the orders of magnitudes of $M$ and $\Treh$. 
\item[5)] In the RGI and MHI type models this will permit to measure the order of magnitude of the inflaton coupling $\g$ independently of parameters other than those in the inflaton potential, as the peak of the posterior falls into the white regions in Figs.~Figs.~\ref{cp_MHI} and \ref{cp_RGI}. 
\item[6)] CMB-S4 tends to perform better than LiteBIRD in all scenarios we considered. In addition to the moderately higher sensitivity to $r$, this is also related to the considerably better sensitivity to $n_s$.
...
\end{itemize}
Further improvement would be possible if additional quantities are included (e.g.~the running of $n_s$ or non-Gaussianities), and if non-CMB data is added (e.g.~from galaxy surveys and 21cm observations).  Quantifying this improvement goes beyond the scope of this work and should be addressed with future studies.
...
We find that the analytic method gives rather accurate results.
 This is true even if $A_s$ is fixed, so that one practically performs an very simple one-dimensional application of Bayes formula.
...
This would be the first ever measurement of ${\rm log}_{10}\Treh$ (in the sense that both an upper and lower bound can be obtained). 
For a sizeable fraction of the parameter space in the models considered here this can be translated into a measurement of the order of magnitude of the inflaton coupling. 
 ...
 This perspective clearly adds to the science cases for LiteBIRD and CMB-S4. It also motivates further theoretical work to investigate which observables in addition to $\{ A_s, n_s, r\}$ can be used to constrain the inflaton coupling. 

\subsection*{Acknowledgments}
We thank J. Hamann, Y. Y. Y. Wong and  for helpful discussions. We furthermore thank T. Brinckmann for his help with {\sc MontePython}. 
...
\begin{appendix}
%\section{Review of basic ingrediants}
\section{Pivot scale for $r$}\label{Sec:PivotScale}
In this brief appendix we explicitly write out the relation between the tensor-to-scalar ratio $r$ and the also often quoted tensor-to-scalar ratio $r_{0.002}$. While the here presented derivation is essentially trivial, we believe that the relation between $r$ and $r_{0.002}$ is nevertheless sometimes a source of confusion.
...
Preheating leads to very large occupation numbers for the produced particles, and the interactions of $\varphi$ with the produced particles affect the effective $\GG$. This introduces a dependence of $\GG$ on the properties of the produced particles, and hence the $\{\SM_i\}$. 
...
Contributions to $\GG$ from different elementary processes 
mediated by some of the interactions in table \ref{ConversionTable}
in a thermal plasma at temperature $T$ that were considered in \cite{Drewes:2019rxn}, cf.~\eqref{WecheselWirkungen}. 
...
The interaction with $\kappa$ (which leads to non-linearites that can limit the validity of \eqref{EOM} \cite{Ai:2021gtg})  cannot effectively reheat the universe in the regime where the condition \eqref{CrocodileBatidaSpecial} is fulfilled.
...
\paragraph{Conditions on the inflaton potential.} The first type concerns the inflaton self-interactions $\{\vv_i\}$ and thereby restricts the choice of the model of inflation (specified by a choice of $\{\vv_i\}$). 
...
\V(\varphi) = \sum_\n
    \frac{\vv_{\n}}{\n !} \frac{\varphi^\n}{\Lambda^{\n-4}} 
\end{eqnarray}
with $\vv_3 = g_\phi/3!$, $\vv_4 = \lambda_\phi/4!$ in \eqref{TaylorV} etc.
...
The inflaton self-interactions can in general lead to parametric or tachyonic resonances that reheat the universe well before elementary decays become relevant, invalidating the use of \eqref{GammaPerturbativeGeneric} in \eqref{GammaConstraint}, and introducing a dependence on the $\{\SM_i\}$.
This can be avoided if \cite{Drewes:2019rxn}
\begin{eqnarray}
 |\vv_\n| \ll \left(\frac{\omega}{\varphi}\right)^{\n-\frac{5
}{2}}
{\rm min}\left(
\sqrt{\frac{\omega}{M_{pl}}}
,
...
This practically restricts us to approximately parabolic potentials in which the oscillating inflaton has a matter-like equation of state \eqref{EquationOfState}.
...
In principle $\M_\phi$ depends on not only on the couplings to other particles $\{\g_i\}$, but also on their interactions  with each other, and hence on the $\{\SM_i\}$. However, \eqref{GeneralConstraintSelf} imposes the strongest constraint when $\varphi$ is maximal, i.e., at the beginning of the reheating phase, and we can replace $\varphi\to\varphi_{\rm end}$ in \eqref{GeneralConstraintSelf} to obtain a conservative bound. 
At that moment the universe is empty. Hence, the frequency of the inflaton oscillations is in good approximation given by its vacuum mass $\omega\simeq m_\phi$.
...
For $\DD<4$ we set $\Lambda=m_\phi$ for convenience, in this case $\Lambda$ is simply introduced to make $\g$ dimensionless and has no physical meaning.
...
For terms with $\DD>4$ the smallness of $(m_\phi/\varphi_{\rm end})^{\n-\frac{1}{2}}$ can be alleviated by the fact that $m_\phi /\Lambda \ll 1$, but e.g.~represents a strong restriction on interactions of the type $\frac{\hh}{4}\Phi^2\chi^2$.
This is the reason why we have focused on operators with $\n=1$ in table \ref{ConversionTable}.

Keeping in mind that the requirement \eqref{avoid self PR} of avoiding self-resonances  during reheating restricts us to potentials that are roughly parabolic at the end of inflation, 
...
\end{eqnarray}
Focusing on renormalisable couplings and assuming $\phi_{\rm end}\simeq M_{pl}$ one finds a simple estimate \eqref{YippieYaYaySchweinebacke} for \eqref{CrocodileBatidaSpecial},
\begin{eqnarray}
|\vv_\n| \ll \left(3\pi^2  r A_s\right)^{(\n-2)/2} \ , \ 
|\g| \ll \left(3\pi^2  r A_s\right)^{\n/2}   \nonumber
\end{eqnarray}
Plugging in the upper bound $r=0.06$ from \cite{Planck:2018vyg} yields an upper bound of $4\times 10^{-5}$ on $\g$ for interactions that are linear in $\Phi$, which is larger than the electron Yukawa coupling in the Standard Model of particle physics.
In the derivation of  \eqref{YippieYaYaySchweinebacke} we neglected various numerical prefactors that depend on the parameters $\{\vv_i\}$ in the potential, which limits its applicability to rough estimates. It is nevertheless a very useful criterion due to its remarkable simplicity, and it provides an explanation for the otherwise surprisingly similar numerical values that the upper bound \eqref{CrocodileBatidaSpecial} yields when applied to the different models. 
Finally, we can use the estimate of $M$ in \eqref{InflatonMass} to rewrite \eqref{YippieYaYaySchweinebacke} as 
\begin{eqnarray}
|\vv_\n| \ll \left(M/M_{pl}\right)^{2(\n-2)} \ , \ 
|\g| \ll \left(M/M_{pl}\right)^{2\n}
\end{eqnarray}
implying that models with a comparably low energy scale of inflation can avoid a parametric resonance only for very small values of the inflaton coupling.

\paragraph{Effects that can relax the conditions.} The conditions \eqref{GeneralConstraintSelf} and \eqref{CrocodileBatidaSpecial} 
impose very strong constraints on interactions that are not linear in $\Phi$, i.e., with $\n>1$.
Luckily it turns out that the relations \eqref{CrocodileBatidaSpecial} and \eqref{GeneralConstraintSelf} from which it is derived 
...
elementary decays may still account for most of ther energy transfer from $\varphi$ into radiation. 
...
However, condition \eqref{avoid self PR} is violated for practically all values of $\alpha$ used in \cite{Ueno:2016dim,Drewes:2017fmn,Ellis:2021kad} (and likely all values of $\alpha$ that can be made consistent with the current observational bound on $r$), invalidating the extraction of the inflaton coupling $\g$ from CMB data (while the determination of the effective $\Treh$ remains valid). 
Note, however, that this conclusion may be too conservative, as already stated in Sec.~\ref{Sec:FeedbaclEffects}. 
The more detailed analysis in \cite{Lozanov:2017hjm} suggests that a self-resonance may still be avoided for values of $\alpha$ or order unity.
...
\begin{figure}
	\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha E model/alpha_E_Nre}}
	\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha E model/alpha_E_Tre}}
	\caption{Corrected version of Fig.~3 in~\cite{Drewes:2017fmn}, describing the $n_s$-dependence of $N_{\rm re}$ and $T_{\rm re}$ in the $\alpha$-attractor E-model with $n=1$ for various values of $\alpha$.}
	\label{alpha NT}
\end{figure}
...
\centering
\subfloat[]{
\includegraphics[width=0.45\linewidth,height=0.4\textheight]{mutated hilltop inflation/MHI_gru.pdf}
\label{MHI ghy g}
}
\quad
\subfloat[]{
\includegraphics[width=0.45\linewidth,height=0.4\textheight]{
mutated hilltop inflation/MHI_hru.pdf}}
\quad
\subfloat[]{
	\includegraphics[width=0.45\linewidth,height=0.4\textheight]{
		mutated hilltop inflation/MHI_yru.pdf}}
\quad
\subfloat[]{
	\includegraphics[width=0.45\linewidth,height=0.4\textheight]{
		mutated hilltop inflation/MHI_aru.pdf}}
...
\centering
\subfloat[]{\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_Rg.pdf}}
\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_Rh.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_Ry.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_Ra.pdf}}
	\caption{Impact of the reheating phase on the predictions of MHI model for $n_s$ and $r$ for a set of sample points.
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_g_1.pdf}
		\label{MHI g 1}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_g_5.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_g_10.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_g_50.pdf}
		%\caption{11}
	}
	\caption{
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_h_1.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_h_5.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_h_10.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_h_50.pdf}
		%\caption{11}
	}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_y_1.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_y_5.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_y_10.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_y_50.pdf}
		%\caption{11}
	}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_a_1.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_a_5.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_a_10.pdf}
		%\caption{11}
	}
	\quad
	\subfloat[]{
		\includegraphics[width=0.45\linewidth]{mutated hilltop inflation/MHI_a_50.pdf}
		%\caption{11}
	}
...
\clearpage\begin{figure}
\centering
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_gru.pdf}}	
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_hru.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_yru.pdf}}	
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_aru.pdf}}
...
\clearpage\begin{figure}
	\centering
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_Rg.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_Rh.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_Ry.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{radion gauge inflation/RGI_Ra.pdf}}
	\caption{Impact of the reheating phase on the predictions of RGI model for $n_s$ and $r$ for a set of sample points.  The notations are the same with Fig~\ref{MHI R d}.} 
	\label{RGI R}
\clearpage\end{figure}
...
\centering
\subfloat[]{
\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_g_5.pdf}}
\quad
\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_g_10.pdf}}
\quad
\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_g_50.pdf}}
\quad
\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_g_100.pdf}}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_h_5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_h_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_h_50.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_h_100.pdf}}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_y_5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_y_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_y_50.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_y_100.pdf}}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_a_5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_a_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_a_50.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{radion gauge inflation/RGI_a_100.pdf}}
...
\clearpage\begin{figure}
\centering
\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_gru.pdf}
\label{alpha gru}}
\quad
\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_hru.pdf}
\label{alpha hru}}
\quad
\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_yru.pdf}
	\label{alpha yru}}
\quad
\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_aru.pdf}
	\label{alpha xru}}
...
\clearpage\begin{figure}
	\centering
	\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Rg.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Rh.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Ry.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.45\linewidth]{alpha T model/alpha_T_Ra.pdf}}
	\caption{Impact of the reheating phase on the predictions of $\alpha$-T model for $n_s$ and $r$ for a set of sample points.  The notations are the same with Fig.~\ref{MHI R d}.} 
	\label{alpha T R}
\clearpage\end{figure}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_g_0.5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_g_1.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_g_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_g_100.pdf}}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_h_0.5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_h_1.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_h_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_h_100.pdf}}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_y_0.5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_y_1.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_y_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_y_100.pdf}}
...
\clearpage\begin{figure}[!h]%[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_a_0.5.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_a_1.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_a_10.pdf}}
	\quad
	\subfloat[]{\includegraphics[width=0.46\linewidth]{alpha T model/alpha_T_a_100.pdf}}
