...
\noindent
$^{12}$ Weizmann Institute of Science, Rehovot, Israel
}
...
of $\sim0.4$. At these energies, the nucleon density in the fireball 
...
efficiency. Section \ref{sect5} addresses the evaluation of the cross sections,
 ...
 interactions measured at lower energies. Finally, the results are summarized in Section .
...
BT = BC1$\otimes\overline{\rm{VC}}\otimes$BC2 was used.
The following logic conditions were applied to generate the trigger signal: 1) BT$\otimes$(BD$\ge\rm{3,4}$);
2) BT$\otimes$(SiMD$\ge\rm{3,4}$); 3) BT$\otimes$(BD$\ge\rm{2}$)$\otimes$(SiMD$\ge\rm{3}$).
...
per spill and a spill duration of 2–2.5 sec.  A set of solid targets of various 
...
$\epsilon_{trig}$ (BD $\ge$ m) = N(BD $\ge$ m $\otimes$ SiMD $\ge$ n)/N(SiMD $\ge$ n), 
...
all data collected with the trigger conditions 1) BT$\otimes$(BD $\ge\rm{3,4}$);
2) BT$\otimes$(SiMD $\ge\rm{3,4}$);
3) BT$\otimes$(BD $\ge\rm{2}$)$\otimes$(SiMD $\ge\rm{3}$) (see Section \ref{sect2}) 
is shown in Fig.~\ref{TrigEff} as a function of the event centrality estimated from 
simulation. It is clearly seen that the trigger efficiency decreases with a decrease 
...
combined trigger BT$\otimes$(BD $\ge\rm{2}$)$\otimes$(SiMD $\ge\rm{3}$).
...
$A_{\mathrm{part}}$. The values of $A_{\mathrm{part}}$, based on the DCM-SMM model, 
