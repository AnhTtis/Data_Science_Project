...
\begin{figure}
  \centering
  \includegraphics[scale=0.65]{figures/arch.pdf}
...
persistent to save or guard against.
...
vulnerabilities in a centralized the software component of the \vtpm{} manager.
%
...
\begin{figure}
  \centering
  \includegraphics[scale=0.6]{figures/attestation.pdf}
...
%
However, in a bare-metal environment such as \svsm{}, one do no have
...
\subsection{Secure communication between VM and \vtpm{}}
...
execution of the VM at a corresponding privilege level, i.e., \vmpl{0}, if the
guest system triggers a transition to the TPM.
%
Hence, the hypervisor is unable to suppress messages unless the whole VM is
halted.

To implement the communication protocol, we developed a modified version of the
Qemu emulator and the guest Linux kernel to interact with our \svtpm{} operating
at \vmpl{0}.
%
Qemu exports the TPM space as a memory-mapped IO~(MMIO) region and traps
all guest accesses to this region to forward the TPM commands to the
appropriate backend implementation - i.e., a passthrough device or an
emulator (\lstinline{swtpm}).
%
We cannot utilize the existing Qemu TPM interfaces for communicating with
the \svtpm{} as MMIO region of the TPM remains unencrypted.
%
A malicious hypervisor could alter the commands and perform a
man-in-the-middle (MITM) attack~\cite{tpm-genie}.
%
One could possibly implement a secure communication channel using secure
protocol and data model (SPDM) protocol to prevent such attacks.
%
However, it is much simpler to implement a new TPM interface for Qemu.

We created a new Qemu TPM interface based on the command response buffer
(CRB) specification~\cite{spec:tpm-crb} in which the exported MMIO region
is part of the guest memory and is encrypted and accessible only to the
\cvm{}.
%
However, with this mechanism the accesses to this MMIO region are not
trapped anymore and the hypervisor is no longer aware on when to invoke the
TPM backend interface.
%
Moreover, with \snp{}, the TPM region is marked as private to the guest
\cvm{} in the reverse mapping table and a hypervisor cannot perform a page
remapping attack.

We modify the Linux CRB driver in the guest kernel to trigger an exit into
the hypervisor, after every write to the TPM MMIO region.
%
...
%
First the user has to verify the pre-attestation report of the load-time
...
%
First, we create a new endorsement key (EK) pair $EK_{pub}$ and
$EK_{priv}$ from random seeds.
%
...
\emph{pre-attestation} measurements, vmpl level and the user-data (i.e.,
...
We can easily verify \textbf{S1} because the attestation report is generated
by the AMD-SP processor and signed using AMD's private keys. Verifying that
the attestation report is genuine implicitly guarantee that we obtained it from
a genuine AMD processor, within a \cvm{}.

\paragraph*{Ensuring \textbf{S2}}
Before launching the \cvm{}, the AMD-SP hardware measures all the load-time
binaries as part of the \emph{pre-attestation}. This includes the \svsm{}
and our \svtpm{} code.
%
