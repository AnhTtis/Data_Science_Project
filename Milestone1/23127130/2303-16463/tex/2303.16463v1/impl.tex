...
We add 1500 lines of code\footnote{measured using
\lstinline{scc}~\cite{sloc:scc}} to the existing SVSM implementation in
Rust.
%
To implement \vtpm{}, we utilize third party libraries: a minimal C library
(4100 LOC), WolfSSL crypto library (970 kLOC) and Microsoft's reference TPM
implementation (32 kLOC)~\footnote{measured using David Wheeler's
\lstinline{sloccount}}~\cite{gh:ms-tpm-ref}.
%

We trust the \svsm{} implementation and all the third-party crates that are
part of the dependency chain (i.e., recursive dependencies).
%
We also assume that WolfSSL and Microsoft's TPM implementations are bug-free.

\begin{figure*}
  \centering
  \includegraphics[scale=0.75]{figures/remote-attestation.pdf}
