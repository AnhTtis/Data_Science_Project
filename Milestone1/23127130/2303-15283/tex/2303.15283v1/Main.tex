...
%Most interesting for applications is the fact that there exists a range of current densities for which nucleation is maximally efficient in the device with ion irradiation while no skyrmions are nucleated in the non-irradiated device (with the exception of the device with $d = 5 \times 10^{12}$ ions \SI{}{cm^{-2}}). This means that local {\Ga} ion irradiation can be used to define regions in devices where skyrmions can be  nucleated electrically (as has been successfully achieved with {\He} ion irradiation \cite{Kern2022}).


%This trend, a steep reduction followed by a saturation, is similar to the change in the domain wall energy density we reported for this exact material system in an earlier publication \cite{DeJong2022}. This leads us to conclude that the drop in threshold current is indicative of a reduction in the energy barrier preventing skyrmion nucleation caused by the modification of the anisotropy and DMI as a result of the {\Ga} ion irradiation.
