...
\subsection{Proof of \texorpdfstring{\cref{theorem:power_laws}}{theorem:powerlaws}}\label{app:proof:theorem:power_laws}
    ...
    Let $G \sim \mathsf{rdag}(K, p)$, with nodes $i_1, \dots, i_K$ (in this order). Let $P_{k, f}$ be the probability of having $f$ distinct failures in the random DAG with $k$ nodes conditioned on a failure on node 1. We have that $P_{1, 1} = 1$ and $P_{k, f} = 0$ for $f > i$ and $f < 1$. To devise a recurrence formula for $P_{i, f}$ note that for the $i$-th node we have the following: 

    \begin{compactenum}
        \item $i_k$ is affected by the cascade. That happens if at least one connection to $f - 1$ infected nodes upto node $i_{k - 1}$, or if $i_k$ fails due to percolation. This happens with probability $\big \{ [ 1 - (1 - p)^{f - 1} ] + x^n - [ 1 - (1 - p)^{f - 1} ] x^n \big \} P_{k - 1, f - 1} = \big [ 1 - (1 - p)^{f - 1}(1 - x^n) \big ]P_{k - 1, f - 1}$.
   
        \item $i_{k}$ is not affected by the cascade. That means that $i_k$ has $\ge 1$ functional supplier, and no connection exists from the $f$ infected nodes. That happens with probability $(1 - p)^f (1 - x^n) P_{k - 1, f}$.
