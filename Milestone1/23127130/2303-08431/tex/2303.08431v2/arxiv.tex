...
\begin{Proposition}\label{prop: inv of design matrix}
    {Assume Assumptions \ref{ass:feature} and \ref{ass:initial_distr} hold. }
...
Assume Assumptions \ref{ass:feature} and \ref{ass: cost function}--\ref{ass:initial_distr} hold.
...
\begin{Theorem}\label{thm: conv of algo}
    ...
    \begin{align*}
    J \geq \frac{\widehat{D}^{2}}{e_{\text{grad}}^{2}r^{2}}\log \frac{4\widehat{D}M}{\nu}\max\curly{36\parenthesis{\mcal{C}(K^{*}) + 2h\delta^{2}}^{2}, 144C_{\max}^{2}}, \; \text{and} \; T \geq \frac{1}{1 - \rho_{1}}\log\frac{6\widehat{D}C_{\text{max}}}{e_{\text{grad}}r},
    \end{align*}
...
\begin{Lemma}\label{lemma: inv of sed mom matrix}
    Assume Assumption \ref{ass:initial_distr} holds. If the random vector $ u_{0}^{(i)} \in \R^{p} $ is such that $ 0 < \P\parenthesis{\abs{s^{\top}u_{0}^{(i)}} > 0} < 1 $ for all $ s \neq 0 $, then the matrix $ \Sigma $ is invertible.
\end{Lemma}
...
\begin{Lemma}\label{lemma: bound P_K}
    Assume Assumption \ref{ass: cost function} holds. When $ \norm{K - K^{\rm lin}}_F \leq \delta \leq 1$, we have
