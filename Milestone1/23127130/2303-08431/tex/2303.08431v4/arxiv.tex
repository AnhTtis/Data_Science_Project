...
Let $ N_{i} \geq 0, i = 1, \dots, d $, be non-negative integers. For fixed $ w_{j}^{i} \in \curly{-1, 1}^{n}, j = 1, \dots, N_{i} $, define $ \phi_{i}(x) = \alpha_{i}\prod_{j = 1}^{N_{i}}x^{\top}w_{j}^{i} $ with $ \alpha_{i} \geq 0 $. Then if $ \norm{x} \leq M_{0} $, the kernel basis $ \phi(x) = (\phi_{1}, \dots, \phi_{d})^{\top}(x) $ satisfies Assumption \ref{ass:feature} with $ \ell = \parenthesis{\sum_{i = 1}^{d}n^{2}\alpha_{i}^{2}N_{i}^{2}M_{0}^{2(N_{i}-1)}}^{1/2} $ and $ \ell' = \parenthesis{\sum_{i = 1}^{d}n^{3}\alpha_{i}^{2}N_{i}^{2}(N_{i} - 1)^{2}M_{0}^{2(N_{i}-2)}}^{1/2} $.
\end{Example}
