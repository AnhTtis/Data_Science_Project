...
Furthermore, key quantities of the nodal ring such as the band overlap energy, average ring radius, ring ellipticity, and the SOC-gap are determined from this comparative study.  
...
Optically, NLSMs are predicted to exhibit universal power-law behavior as they absorb light: When the two crossing bands forming the nodal line disperse as $E(k) \sim k^{z}$ in the effective dimensions $d$, the interband transition taking place between them leads to optical conductivity $\sigma_{1}(\omega) \sim \omega^{(d-2)/z}$ \cite{PhysRevB.87.125425}.
For $d$ = 2 cases, $\sigma_{1}(\omega)$ becomes a frequency-independent constant irrespective of $z$. 
This flat optical conductivity, $\sigma^{\mathrm{flat}}$, has been demonstrated experimentally, among others, in nodal-line materials such as ZrSiS, ZrSiSe, BaNiS$_2$, YbMnSb$_2$, TbMn$_6$Sn$_6$ and NbAs$_2$ \cite{PhysRevLett.119.187401, Shao2020, PhysRevB.104.L201115, PhysRevB.100.125136, PhysRevB.107.045115, doi:10.1073/pnas.1809631115}.
These works further showed that $\sigma_{1}(\omega)$ provides some key information about the nodal line  including the nodal-line length \cite{PhysRevLett.119.187401} or energy dispersion along the nodal line \cite{doi:10.1073/pnas.1809631115}.

Nodal rings are a particular type of nodal-line structure that arises when two conical bands cross at a finite energy overlap, leading to a closed nodal line.
For a circular nodal ring, theory predicts that a $\sigma^{\mathrm{flat}}$ emerges consistently with the power law. Furthermore, 
characteristic properties of a nodal ring such as the radius and tilting are manifested through optical conductivity \cite{PhysRevLett.119.147402, PhysRevB.96.155150}.
However, on the experimental side, an optical study of a nodal ring is still lacking.
Most of the previous optical works were performed on either open nodal lines unclosed in the Brillouin zone (BZ) or closed nodal lines in which, instead of a single nodal ring, multiples of rectangular-shaped closed nodal lines are linked together into a complicated structure.
Quantitative analysis of optical data for such materials presents, in general, a significant challenge, though not impossible.
On the contrary, a nodal ring with a circular or elliptical shape offers a much simpler structure for optical studies, especially when there is only a single nodal ring in the BZ.

One critical issue in the optical experiment is that, in most NLSM materials, topologically trivial bands coexist at the Fermi energy $E_{\rm F}$ with the non-trivial bands.
The trivial bands create their own intra- and interband optical transitions that seriously obscure the optical signal from the non-trivial band \cite{PhysRevB.104.L201115, doi:10.1073/pnas.1809631115, C9TC03464A, PhysRevB.93.121113, Feng2017, PhysRevB.103.125131}.
To overcome this problem, a material possessing only the nodal ring without any trivial band crossing $E_{\mathrm{F}}$ is strongly called for.
An optical study of a single nodal ring with no interruption from trivial bands has not been reported up to date.

In this work, we conduct an optical measurement on $\rm{SrAs_3}$.
Recent studies have revealed that $\rm{SrAs_3}$ is a rare example that features only a single (doubly degenerate) nodal ring in the BZ.
\cite{PhysRevLett.124.056402, Hosen2020, Kim2022}.
Furthermore, no trivial bands intersect the Fermi level \cite{Hosen2020, https://doi.org/10.48550/arxiv.2211.05978}, making $\rm{SrAs_3}$ an excellent candidate for optical study.
...
In $\rm{SrAs_3}$, the nodal ring has a slightly elliptical shape, which may cause $\sigma_{1}(\omega)$ to become different from the predicted constant, where the actual $\sigma_{1}(\omega)$ is an interesting open question. 
Here we measure $\sigma_{1}(\omega)$ of $\rm{SrAs_3}$ over a wide range of frequencies.
We further perform theoretical calculations of $\sigma_{1}(\omega)$ and compare the result with the data, which yields excellent agreement. 

Single crystals of $\rm{SrAs_3}$ were grown using the Bridgman method and their structural and electrical properties were thoroughly characterized \cite{Kim2022}.
The optical reflectivity was measured on the $ab$-plane of $\rm{SrAs_3}$ in the infrared range using polarized light for temperatures ranging from 5 K to 300 K in combination with the in-situ gold evaporation technique.
The dielectric function  from 0.7 eV to 4.5 eV was obtained at $T$ = 300 K using a Spectroscopic Ellipsometer (J.A. Woollam VASE).


\begin{figure}
\includegraphics[width=1\columnwidth]{fig1}
\caption{\label{fig:fig1}
Schematic drawing of (a) crystal structure and (b) band structure of $\rm{SrAs_3}$. (c) A simplified view of (b) where $2\varepsilon_0$ and $E_{\mathrm{F}}$ are the band overlap energy and Fermi energy, respectively.
(d) The nodal ring is elliptically elongated where $k_{l}$, $k_{s}$, and $\phi_{0}$ are the semi-major and semi-minor axis lengths and the rotation angle, respectively. The blue and red pockets represent the hole and electron carriers, respectively.
 The yellow curves in (b), (c), and (d) highlight the nodal ring. The SOC gap is omitted in the figures for clarity. In the optical measurement, incident light is polarized as specified in (a) and (d).}
\end{figure}
...
In $\rm{SrAs_3}$, the spin-orbit coupling (SOC) creates an energy gap along the nodal ring.
...
Inset shows the wide-range reflectivity up to 4.8 eV measured at $T$ = 300 K.}
\end{figure}
...
Figure 2 shows that for the $E$ $\parallel$ $k_x$ polarization, the rise of reflectivity for $\hbar\omega$ $<$ 50 meV ($T$ = 300 K) represents a Drude response. 
...
Optical conductivity for $E$ $\parallel$ $k_x$ and $E$ $\parallel$ $k_z$ obtained from (a) optical  reflectivity measured at $T$ = 5 K and (b) theoretical calculation for interband transitions at zero temperature in the clean limit.
For numerical calculations, we set the broadening term as $\gamma$ = 1 meV.
The SOC-induced optical peak ($\hbar\omega$ = 30 meV), the flat conductivity region (70 meV $<$ $\hbar\omega$ $<$ 129 meV), and the interband (IB) transition above the band overlap energy ($\hbar\omega$ $>$ 129 meV) are labeled as A, B and C, respectively, in panel (a).
The levels of flat conductivities are marked by red dashed lines in panel (b).
(c) Schematic band structure of $\rm{SrAs_3}$ near the Fermi energy $E_{\mathrm{F}}$ along the radial direction, where SOC opens a gap of 2$\Delta_{\mathrm{SOC}}$ along the nodal ring. The black solid line indicates the band overlap energy 2$\varepsilon_0$.
Arrows labeled A, B and C represent the corresponding interband transitions in the optical conductivity shown in panel (a).
(d) A schematic diagram illustrating the nodal ring as a collection of gapped anisotropic graphene sheets.
}
\end{figure}
...
We present the full $T$-dependent $\sigma_{1}(\omega)$ data in the Supplemental Material (SM) Fig. S1. 
For $E \parallel k_x$, $\sigma_{1}(\omega)$ displays an absorption peak (A) at 40 meV and a flat conductivity (B) over the 70 - 130 meV range.
At higher energies, $\sigma_{1}(\omega)$ gradually increases (C).
For $E \parallel k_z$, Peak-A becomes more pronounced and the flat optical conductivity $\sigma_{zz}^{\mathrm{flat}} \approx 239 \ \mathrm{cm^{-1} \thinspace \mathrm{\Omega^{-1}}}$ is significantly higher than $\sigma_{xx}^{\mathrm{flat}} \approx 83 \ \mathrm{cm^{-1} \thinspace \mathrm{\Omega^{-1}}}$ for $E \parallel k_x$. 
%(Note the different $y$-scales for the $E \parallel k_x$ and $E \parallel k_z$ plots.)
...
The A, B, and C represent, as we will show, the interband transitions of $\rm{SrAs_3}$.
Along with the interband transition, $\sigma_1(\omega)$ exhibits an intraband transition (Drude) peak at low energies. 
The two sharp peaks between the Drude peak and Peak-A are optical phonons of $A_u$ mode (for $E \parallel k_z$) and $B_u$ mode (for $E \parallel k_x$), respectively \cite{B_Oles_1981}.
...
Each component is obtained separately by fitting the data (see SM Sec. I for details) \cite{SM}. In the following discussion, we will focus on the Drude Peak and interband transitions.


The Drude peak in the optical conductivity is described by $\sigma^{\mathrm{D}}(\omega) = \frac{\omega^2_{p}}{4\pi}\frac{\gamma}{(\gamma^2+\omega^2)}$, where $\omega_{p}$ and $\gamma$ = 1/$\tau$ are the plasma frequency and the carrier scattering rate, respectively.
$\sigma^{\mathrm{D}}(\omega)$ is mostly contributed by the hole carrier, which dominates over the electron.
Using $\omega_{p,x}^{2}$ ($E \parallel k_x$) and $\omega_{p,z}^{2}$ ($E \parallel k_z$), summarized in Table S1, we obtain $\omega_{p,z}^{2}$/$\omega_{p,x}^{2}$ = 9.5 which, from the definition of $\omega_{p}^{2}$ = $\frac{4\pi ne^{2}}{m^*}$ ($n$ = hole density,  $m^{*}$ = effective mass), is equivalent to  $m^*_{x}$/$m^*_{z}$.
It shows that the effective mass of the hole along the Sr-chain ($b$-axis) is substantially lighter than that along the Sr-zigzag direction ($a$-axis).
A similar result was previously reported for the chain- and zigzag-directions of black phosphorus \cite{19833544}.
We can calculate $m^*$ by inserting the hole density $n$ = 4.24$\times 10^{17}$ cm$^{-3}$ into $\omega_{p,z}^{2}$, which yields $m^*_z$ = 0.016$m_0$.
We further calculate the carrier mobility $\mu = \frac{e\tau}{{m^*}}$ using $\tau$ = 1/$\gamma$ and $m^{*}$ for $E$ $\parallel$ $k_x$ and $E$ $\parallel$ $k_z$.
This leads to $\mu_{x}$ = 3.8$\times 10^{3}$ $\rm{cm}^2 \thinspace V^{-1} \thinspace s^{-1}$ and $\mu_{z}$ = 2.3$\times 10^{4}$ $\rm{cm}^2 \thinspace V^{-1} \thinspace s^{-1}$, respectively, which is similar to the transport results \cite{PhysRevB.50.5180, PhysRevB.32.1183}.  
...
\end{align}
where $f_0(\bm{k}) = a_0 + a_x k_x^2 + a_{xy} k_x k_y + a_y k_y^2 + a_z k_z^2$, $f_1(\bm{k}) = b_0 + b_x k_x^2 + b_{xy} k_x k_y + b_y k_y^2 + b_z k_z^2$, $f_2(\bm{k}) = \hbar v_z k_z$, and $\bm{k}$ is the wave vector measured from the Y point in the BZ, and $\bm{\sigma}$ and $\bm{s}$ are Pauli matrices acting on the pseudospin and spin degrees of freedom, respectively, and $\Delta_{\rm SOC}$ is the strength of the spin-orbit coupling, which, as a leading approximation, we take as constant. Here the coefficients $a_0$, $a_i$, $b_i$ ($i=x$, $xy$, $y$, $z$), and $v_z$ are obtained from first-principles calculations. For the band overlap energy 2$\varepsilon_0 = 2b_0$ and the spin-orbit coupling $\Delta_{\mathrm{SOC}}$, which are difficult to estimate correctly from first-principles calculations, we determine them by comparing with the optical data (see SM Sec. II \cite{SM}). Then we calculate the interband part of the optical conductivity $\sigma_1(\omega)$ using the Kubo formula. The full numerical results of $\sigma_1(\omega)$ are presented in Fig. 3(b).
...
\end{equation}
where $i = x$, $y$, $z$ are spatial directions, $\sigma_{ii}^{\mathrm{gr}}(\omega, \phi)$ is the optical conductivity of the gapped anisotropic graphene, $k_0$ is the characteristic scale of the nodal ring size which will be defined later, and $\mathcal{F}_{ii}(\phi)$ are geometric factors related to the shape of the nodal ring. For the details, see SM Sec. III \cite{SM}.
...
In obtaining Eq. (\ref{eq:opcd}), we use the fact that the Fermi energy of $\mathrm{SrAs_3}$ lies within the SOC gap for at least a certain range of angle, as shown in Fig. 3(c), giving a peak corresponding to the gap size. We note that Eq. (\ref{eq:opcd}) is an approximate form neglecting the effect of the energy tilt term. For details, see SM Sec. III \cite{PhysRevLett.119.147402}.


Based on the experimental data in Fig. 3(a), we estimate $\Delta_{\mathrm{SOC}} \approx 30$ meV. The SOC gap is one of the key factors that determines the band structure of NLSMs but has seldom been measured precisely due to its small magnitude. We emphasize that the high energy resolution of IR spectroscopy, in combination with the successful theoretical analysis, allows for the accurate determination of $\Delta_{\mathrm{SOC}}$.
...
Having investigated the SOC-induced peak (A) and the flat conductivity (B), we move to the region C. For $\hbar \omega$ $>$ $2\varepsilon_0 \approx$ 129 meV, the flat conductivity is no longer maintained and $\sigma_{xx}$ increases gradually while $\sigma_{zz}$ shows a sudden drop, indicating that the picture we employed in Eq. (2) does not hold any more at high frequencies \cite{PhysRevLett.119.147402, PhysRevB.96.155150}. At $\hbar \omega = 2\varepsilon_0$, the joint density of states for interband transitions shows an abrupt change, and above this frequency, the optical conductivity exhibits a nontrivial frequency-dependence depending on the direction due to the difference in the velocity matrix elements along the $k_x$ and $k_z$ directions.
...
For comparison, we numerically calculate $\sigma^{\mathrm{IB}}(\omega)$ at finite-$T$ by taking the thermal effect into account in the Kubo formula, i.e., by incorporating the Fermi-Dirac distribution and $T$-driven chemical potential $\mu(T)$ shift (see SM Sec. IV \cite{SM}).
...
However, unlike this prediction, the $\sigma^{\mathrm{IB}}(\omega)$ data exhibits substantial changes in B and C: $\sigma_{zz}^{\mathrm{flat}}$ is reduced more than expected by the calculation and 2$\varepsilon_0$ redshifts by $\sim$ 40 meV. 
...
However, at this point, we can not provide any further supporting evidence or underlying cause for the movement.
Nevertheless, it is clear that the thermal effect alone cannot account for the $T$-dependent changes in regions B and C.
We think further studies are necessary to elucidate the origin of this intriguing $T$-driven change in $\sigma^{\mathrm{IB}}(\omega)$.
...
For a comparative study, we theoretically calculated the optical conductivity $\sigma_{1}(\omega)$, which successfully reproduced the characteristic optical transitions of the nodal ring such as the SOC-induced Peak-A and the flat conductivity $\sigma^{\mathrm{flat}}$.
Our comparative study demonstrates that $\sigma_{1}(\omega)$ originates exclusively from the single nodal ring, establishing $\rm{SrAs_3}$ as an ideal platform for investigating nodal rings in topological materials.
...
J.J. and H.M. were supported by the NRF grant funded by the Korea government (NRF-2018R1A2B6007837) and the Creative-Pioneering Researchers Program through Seoul National University (SNU). 
