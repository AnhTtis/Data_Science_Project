...
\title{Fast Convergence Federated Learning with Aggregated Gradients\\
...
\item \emph{Algorithms for finalizing the optimal learning rate:} Through rigorous analysis, we prove that there exist fixed points for the mean-field terms, and an algorithm is introduced to calculate the fixed points, which finalizes the optimal learning rate. The experimental results show that our FL method has a faster convergent rate and higher accuracy compared to other FL algorithms.
\end{itemize}
...
The rest of this paper is organized as follows. Problem formulation is given in Section \uppercase\expandafter{\romannumeral2}. In Section \uppercase\expandafter{\romannumeral3}, the approach for adaptive learning rate and mean field analysis are presented. Experimental results are shown in Section \uppercase\expandafter{\romannumeral4}. Section \uppercase\expandafter{\romannumeral5} presents the conclusion.

\section{Problem Formulation}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Standard Federated Learning Model}%%%%%%%%%%%%%%%%%%%%%%

In general, FL method\cite{mcmahan2017communication,lim2020federated} is a decentralized machine learning mechanism, which is designed to deal with the consensus learning task. In FL, a global model is collaboratively trained by a large number of clients with locally collected data. Each client performs one epoch of mini-batch stochastic gradient descent and transfers the updated local parameter to the central server. After updating global parameter by aggregating the local parameters, the central server sends the updated global parameter to all clients to trigger a new round of local training until the global model converges or reach the maximum iteration\cite{tu2022adaptive}.  
...
In particular, we assume that a standard federated leaning framework has $N$ clients and each participating client $i \in \{0, \ldots, N\}$ uses its local data $\mathcal{D}_{i}$ with datasize $D_{i}$, where $D_{i} \triangleq\left|\mathcal{D}_{i}\right|$ and $\left|\ \cdot\ \right|$ denotes the cardinality of sets. Define $\{\boldsymbol{x_{i}^{j}},y_{i}^{j} \}_{j=1}^{D_{i}}$ as the data samples in $\mathcal{D}_{i}$, where $\boldsymbol{x}_{\boldsymbol{i}}^{j} \in \mathbb{R}^{d \times 1}$ represents the input sample vector and $y_{i}^{j} \in \mathbb{R}$  denotes the labeled output value. As the object of the federated learning model is to access the optimal parameter $\boldsymbol{w} \in \mathbb{R}^{d \times 1}$ based on the loss function $f_{i}^{j}\left(\boldsymbol{w},\boldsymbol{x}\right)$, which measures the difference between real output $y_{i}^{j}$ and predicted output $\hat{y}_{i}^{j}$. The local loss function of client $i$ on dataset $\mathcal{D}_{i}$ is as follow:
\[\begin{split}
F_{i}\left(\boldsymbol{w}, \mathcal{D}_{i}\right)=\frac{1}{D_{i}} \sum_{j \in \mathcal{D}_{i}} f_{i}^{j}(\boldsymbol{w}, \boldsymbol{x}_{\boldsymbol{i}}^{\boldsymbol{j}})
\end{split} \tag{1}\]

In each local training epoch $t \in\{0,1, \ldots, T\}$, each client $i$ performs SGD training to update its local parameter as follows:
\[\begin{split}
\boldsymbol{w_{i}(t+1)}=\boldsymbol{w(t)}-\eta_{i} \nabla F_{i}(\boldsymbol{w(t)})
\end{split} \tag{2}\]
where $\eta_{i} \in \left[0,1 \right]$ is the learning rate of client $i$. In classic federated algorithms, such as FedAvg, $\eta_{i}$ is a constant. $\boldsymbol{w(t)}$ is the global parameter from central sever in last round, and $\nabla F_{i}(\boldsymbol{w}(\boldsymbol{t}))$ denotes the gradient of client $i$ at iteration $t$. The central server averages the parameters sent back by $N$ training clients to generate the updated global parameters:
\[\begin{split}
\boldsymbol{w(t+1)}=\sum_{i=1}^{N} \frac{D_{i}}{\sum_{j=1}^{N} D_{j}} \boldsymbol{w_{i}(t+1)}
\end{split} \tag{3}\]

After updating global model, the central sever sends updated global parameter back to all clients for next round's training.
...
To deal with the client drifting effect for Non-IID dataset, we first modify the client local parameter updating rules by introducing the aggregated gradients of all participating clients to replace the gradient term of conventional SGD formula in Eq (2) as follows:
\[\begin{split}
\boldsymbol{w_{i,l+1}^{t}}=\boldsymbol{w_{i,l}^{t}}-\eta_{i,l}^{t} \cdot \left[\frac{1}{N} \sum_{i=1}^{N} \nabla{F_{i}(\boldsymbol{w_{i,l}^{t})}} \right]
\end{split} \tag{5}\]
where $\eta_{i,l}^{t}$ is the adaptive learning rate of client $i$ at global iteration $t$ and at local update epoch $l$, and $\boldsymbol{w_{i,l}^{t}}$ is the local parameter of client $i$ at the $l$-th local epoch of global iteration $t$, where $l \in \{0,1,  \ldots, L\}$. At the onset of every round $t$, we have $\boldsymbol{w_{i,0}^{t}} = \boldsymbol{\bar w^{t}} = \sum_{i=1}^{N} \dfrac{D_{i}}{\sum_{j=1}^{N} D_{j}}\boldsymbol{w_{i,L}^{t-1}}$.

Then, in order to achieve fast convergence for each client $i \in\{0, 1, \ldots, N\}$, a penalty term that measures the deviation between the local model parameter of client $i$ and the average local model parameter at each local training iteration $l$ is introduced in the objective function $U_{i}(T)$ of each client $i$:
\begin{align}
U_{i}(l) &= \min_{\substack{\eta_{i,l}^{t}, t \in\{0,1, \ldots, T\} \\  l \in\{0,1, \ldots, L\}}}\sum_{t=0}^{T} \sum_{l=0}^{L} \alpha (\eta_{i,l}^{t})^{2}+(1-\alpha) \cdot \nonumber \\
& \qquad\quad (\boldsymbol{w_{i,l}^{t}}-\frac{1}{N} \sum_{j=1}^{N} \boldsymbol{w_{j,l}^{t}})^{T}(\boldsymbol{w_{i,l}^{t}}-\frac{1}{N}\sum_{j=1}^{N} \boldsymbol{w_{j,l}^{t}}) \tag{6} 
\end{align} 
\begin{align}
...
\end{align} 
where $\alpha \in [0, 1]$ is a hyperparameter that adjusts the weight of each part in objective function $U_{i}(T)$.

As there is no communication among clients during each local training epoch, client $i$ can not access $\boldsymbol{w_{j,l}^{t}}$ and $\nabla{F_{j}(\boldsymbol{w_{j,l}^{t})}}$, where $j \in S \setminus \{i\}$. The learning rate $\eta_{i,l}^{t}$ of each client $i$ is affected by other clients' local parameters $\boldsymbol{w_{j,l}^{t}}$ and $\nabla{F_{j}(\boldsymbol{w_{j,l}^{t})}}$, where $j \in S \setminus \{i\}$, which makes this multi-clients joint adaptive learning rate design over time challenging. To tackle the above challenges, we first introduce two mean fields term to estimate the average of local parameter and gradient. Then, the decentralized adaptive learning rate of each client can be derived based on the two mean field terms, without requiring the clients to frequently communicate with each other.

\section{Approach For Adaptive Learning Rate}%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we develop our methodology for improving the convergence rate without communication at the end of each local training epoch. We first find the optimal adaptive learning rate $\eta_{i,l}^{t}$ for each client $i$ at each local update epoch $l$ through introducing two estimators $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ and then prove that $\eta_{i,l}^{t}$ can be present by linear combination of $\boldsymbol{w_{i,l}^{t}}$, $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ (Section \uppercase\expandafter{\romannumeral4}-A). Subsequently, we utilize Brouwer’s fixed point theorem to prove the existence of mean field term $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$, then $\eta_{i,l}^{t}$ is solvable can be proved(Section \uppercase\expandafter{\romannumeral4}-B).

\subsection{Adaptive Learning Rate For Each Client}%%%%%%%%%%%%%%%%%%

To figure out the adaptive learning rate for each client, we introduce two mean field terms $\boldsymbol{\phi_{1,l}^{t}}$, $t \in\{0,1, \ldots, T\}$, $l \in\{0,1, \ldots, L\}$ to estimate the average of all clients' gradients at local epoch $l$ and $\boldsymbol{\phi_{2,l}^{t}}$, $t \in\{0,1, \ldots, T\}$ , $l \in\{0,1, \ldots, L\}$ to estimate the average of all clients' local parameter, i.e.,
\begin{align}
...
From the mathematical point of view, $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ are given functions. The deviation between local parameter and average local parameter could be evaluate as $\boldsymbol{w_{i,l}^{t}}-\boldsymbol{\phi_{2,l}^{t}}$. Accordingly, the objective function of each client $i$ can be rewritten as follow:
\begin{align}
...
& \qquad\qquad\qquad\qquad (\boldsymbol{w_{i,l}^{t}}-\boldsymbol{\phi_{2,l}^{t}})^{T}(\boldsymbol{w_{i,l}^{t}}-\boldsymbol{\phi_{2,l}^{t}}) \tag{10}
\end{align} 
\[\begin{split}
...
Given the mean field term $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$, the optimal adaptive learning rate of client $i$, $i \in\{1, \ldots, N\}$ at iteration $t$, $t \in\{0,1, \ldots, T\}$ and local update epoch $l$, $l \in\{0,1, \ldots, L\}$ is:
\begin{align}
\eta_{i,l}^{t}&=\dfrac{1-\alpha}{\alpha}(\boldsymbol{\phi_{1,l}^{t}})^{T}((L-l)\boldsymbol{w_{i,l}^{t}} \nonumber \\
& \qquad\qquad -\sum_{r=l}^{L} (L-r)\eta_{i,r}^{t}\boldsymbol{\phi_{1,r}^{t}}- \sum_{k=l+1}^{L} \boldsymbol{\phi_{2,k}^{t}}) \tag{12}
\end{align}
with $\eta_{i,L}^{t}=0$. 
\end{proposition}
...
+\alpha (\eta_{i,l}^{t})^{2}-\boldsymbol{\lambda(l+1)}^{T}\eta_{i,l}^{t}\boldsymbol{\phi_{1,l}^{t}}
\end{split} \tag{13}\]
...
& \qquad\qquad -\eta_{i,L-1}^{t} \boldsymbol{\phi_{1,L-1}^{t}} -\sum_{k=L-1}^{L}\boldsymbol{\phi_{2,k}^{t}}) \nonumber \\
...
& \quad -\sum_{r=L-2}^{L-1} (L-r) \eta_{i,r}^{t} \boldsymbol{\phi_{1,r}^{t}}-\sum_{k=L-2}^{L} \boldsymbol{\phi_{2,k}^{t}}) \nonumber \\
&\quad \vdots  \nonumber \\
\boldsymbol{\lambda(l+1)} &= 2(1-\alpha)((L-l) \boldsymbol{w_{i,l+1}^{t}} \nonumber 
\\ & \qquad -\sum_{r=l+1}^{L}(L-r)\eta_{i,r}^{t} \boldsymbol{\phi_{1,r}^{t}}  -\sum_{k=l+1}^{L} \boldsymbol{\phi_{2,k}^{t}}) \tag{21}
\end{align}

Then, based on the expression of $\eta_{i,l}^{t}$ in Eq(16) and $\boldsymbol{\lambda(l+1)}$ in Eq(21), we can get the new form of $\eta_{i,l}^{t}$:
\begin{align}
...
& \qquad\qquad -\sum_{r=l+1}^{L}(L-r)\eta_{i,r}^{t}\boldsymbol{\phi_{1,r}^{t}}-\sum_{k=l+1}^{L} \boldsymbol{\phi_{2,k}^{t}}) \nonumber \\
&=\dfrac{1-\alpha}{\alpha}(\boldsymbol{\phi_{1,l}^{t}})^{T}((L-l) \boldsymbol{w_{i,l}^{t}} \nonumber \\
& \qquad\qquad\quad -\sum_{r=l}^{L} (L-r)\eta_{i,r}^{t}\boldsymbol{\phi_{1,r}^{t}}-\sum_{k=l+1}^{L}\boldsymbol{\phi_{2,k}^{t}}) \nonumber
\end{align} $\hfill\blacksquare$ 

\begin{proposition} \label{proposition 2}
$\eta_{i,l}^{t}$ can be represented by linear combination of $\boldsymbol{w_{i,l}^{t}}$, $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$, $l \in\{0,1, \ldots, L\}$ at global iteration $t$. That is, $\eta_{i,l}^{t} = \Lambda \left(\boldsymbol{w_{i,l}^{t}},\boldsymbol{\phi_{1,0}^{t}},\ldots,\boldsymbol{\phi_{1,L}^{t}}, \boldsymbol{\phi_{2,0}^{t}}, \ldots, \boldsymbol{\phi_{2,L}^{t}}\right)$.
\end{proposition}
\emph{Proof}: According to the process of deducing $\eta_{i,l}^{t}$, we can find the potential rules among $\eta_{i,l}^{t}$ , $\boldsymbol{w_{i,l}^{t}}$, $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$, $l \in\{0,1, \ldots, L\}$. Accordingly, based on the expression of $\boldsymbol{\lambda(L\!-\!1)}$ and $\boldsymbol{\lambda(L\!-\!2)}$ above, we can derive as follows:
...
& \qquad\qquad\qquad\quad -\boldsymbol{\phi_{2,L}^{t}}-\eta_{i,L-1}^{t}\boldsymbol{\phi_{1,L-1}^{t}}) \nonumber \\
...
Given the optimal adaptive learning rate $\eta_{i,l}^{t}$ in Eq (12), we are ready to find the estimator $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ in this section. Noted that estimators $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ are affected by the local parameter $\boldsymbol{w_{i,l+1}^{t}}$ and local gradient $\nabla{F_{i}(\boldsymbol{w_{i,l}^{t})}}$ of all clients which inversely affect $\boldsymbol{w_{i,l+1}^{t}}$ and $\nabla{F_{i}(\boldsymbol{w_{i,l}^{t})}}$. Subsequently, we will acquire precise solutions for estimator $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ by finding the fixed point.
...
Local training epoch $L$, initiate $\epsilon_{1}=1$, $\epsilon_{2}=1$, $\epsilon=0.001$, $j=1$, arbitrary initial $\boldsymbol{\phi_{1,l}^{t}}^{est}(0) \ge 0$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}$, $\boldsymbol{\phi_{2,l}^{t}}^{est}(0) \ge 0$, $ t \in\{0,1, \ldots, T\}, l \in\{0,1, \ldots, L\}$ and $\boldsymbol{\phi_{1,l}^{t}}=\boldsymbol{\phi_{1,l}^{t}}^{est}(0)$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}$, $\boldsymbol{\phi_{2,l}^{t}}=\boldsymbol{\phi_{2,l}^{t}}^{est}(0)$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}$, $\boldsymbol{w_{i,0}^{t}}$, $ t \in\{0,1, \ldots, T\}, i \in\{0,1, \ldots, N\}$, arbitrary initial $\eta_{i,l}^{t} \in (0, 1)$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}, i \in\{0,1, \ldots, N\}$
\begin{algorithmic}[1]
\For{$t=0$ to $T$}
...
\[\begin{split}
\mid \nabla{F_{i} (\boldsymbol{w_{i,l}^{t}})}\mid \le A
\end{split} \tag{31} \]
...
Then, according to Eq (27), we noted that both $\boldsymbol{w_{i,0}^{t}}$ and $\eta_{i,0}^{t}$, $t \in\{0,1, \ldots, T\}$ are bounded. Besides, on account of $\nabla{F_{i} (\boldsymbol{w_{i,l}^{t}})}$, $t \in\{0,1, \ldots, T\}, l \in\{0,1, \ldots, L\}$ is bounded as we proved above and \emph{Proposition 3.2}, we have each term in the Eq (27) is bounded. Thus, we also have the following upperbound for $\boldsymbol{w_{i,l}^{t}}$:
\[\begin{split}
\mid \boldsymbol{w_{i,l}^{t}}\mid \le B
\end{split} \tag{33} \]

Define continuous space $\Omega=[-A, A]_{0}^{0} \times \ldots \times[-A, A]_{L}^{0} \times \ldots \times[-A, A]_{0}^{T}\times \ldots \times[-A, A]_{L}^{T}\times \ldots \times[-B, B]_{0}^{0} \times \ldots \times[-B, B]_{L}^{0} \times \ldots \times[-B, B]_{0}^{T}\times \ldots \times[-B, B]_{L}^{T}$ for $\boldsymbol{w_{i,0}^{t}}$ and $\nabla{F_{i} (\boldsymbol{w_{i,l}^{t}})}$. Since $\Gamma_{l}^{t}$ is continuous in $\Omega$, $\Gamma$ is a continuous mapping from $\Omega$ to $\Omega$. According to the Brouwer's fixed-point theorem, we have the following proposition.
...
\hspace*{0.02in} {\bf Input: } %算法的输入， \hspace*{0.02in}用来控制位置，同时利用 \\ 进行换行
The client size $N$, hyperparameter $\alpha$, local training epoch $L$, $\boldsymbol{w_{i,0}^{t}}$, $ t \in\{0,1, \ldots, T\}$, $ i \in\{0,1, \ldots, N\}$, fixed point estimators $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}$, $\eta_{i,l}^{t}$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}, i \in\{0,1, \ldots, N\}$
\begin{algorithmic}[1]
\For{$t=0$ to $T$}
    \For{each client $i \in\{0,1, \ldots, N\}$}
        \For{$l=0$ to $L-1$}
...
In accordance with the deduction discussed before, we can figure out the fixed points of $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ by technological process in \textbf{Algorithm 1}. Given $\boldsymbol{w_{i,0}^{t}}$, $ t \in\{0,1, \ldots, T\}, i \in\{0,1, \ldots, N\}$, arbitrary initial $\boldsymbol{\phi_{1,l}^{t}}(0)$, $\boldsymbol{\phi_{2,l}^{t}}(0)$ and $\eta_{i,l}^{t}$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}, i \in\{0,1, \ldots, N\}$, we can easily solve $\eta_{i,l}^{t}$, $l \in\{0,1, \ldots, L\}, i \in\{0,1, \ldots, N\}$ according to Eq(12). Then, based on $\eta_{i,l}^{t}$ and $\boldsymbol{w_{i,0}^{t}}$, we can solve $\boldsymbol{w_{i,l}^{t}}$ and $\nabla{F_{i}(\boldsymbol{w_{i,l}^{t})}}$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}, i \in\{0,1, \ldots, N\}$ by Eq(11) and Eq(32), respectively. Accordingly, mean field terms $\boldsymbol{\phi_{1,l}^{t}}$ and $\boldsymbol{\phi_{2,l}^{t}}$ can be updated by using $\boldsymbol{w_{i,l}^{t}}$ and $\nabla{F_{i}(\boldsymbol{w_{i,l}^{t})}}$. Repeating the process above until $\boldsymbol{\phi_{1,l}^{t}}^{est}(j+1) \rightarrow \boldsymbol{\phi_{1,l}^{t}}^{est}(j)$ and $\boldsymbol{\phi_{2,l}^{t}}^{est}(j+1) \rightarrow \boldsymbol{\phi_{2,l}^{t}}^{est}(j)$ within arbitrarily small error $\epsilon$ simultaneously, the fixed point is found. In the meanwhile, $\eta_{i,l}^{t}$, $ t \in\{0,1, \ldots, T\},l \in\{0,1, \ldots, L\}, i \in\{0,1, \ldots, N\}$ is also obtained. Then, our adaptive federated learning process can be summarized in \textbf{Algorithm 2}.  
...
For the convergent rate, it is shown in Fig.1 that our algorithm not only keeps high accuracy in all iterations, but also converges much faster than than other methods on the IID client data distribution. With regard to Non-IID dataset, our method also performs well. As shown in Fig.2, our method effectively speeds up the convergence of the global model training under the premise of ensuring the accuracy.
...
In this paper, we have studied the fast convergence problem of global model in federated learning by designing dynamic learning rate for Non-IID dataset. We have formulated a discrete time linear quadratic optimal control problem under the incomplete information about clients' local parameters and gradients in each local training epochs. To deal with the client drifting issue, we first ameliorate the gradient descend approach by substituting the gradient term in initial formula with the aggregated gradients of all participating clients. Moreover, we introduce a penalty term which measures the deviation between the local parameter and average parameters of all clients into each client's objective function. To deal with the communication burden, two mean field terms are introduced to evaluate the average local parameter and gradient, respectively. Accordingly, the optimal decentralized adaptive learning rate is obtained for each client in each local and global epoch to make global model training more efficient and converge faster, especially for Non-IID data cases. Finally, the experimental results show that our adaptive learning rate algorithm achieves higher accuracy, as well as faster convergent rate on MNIST dataset.
