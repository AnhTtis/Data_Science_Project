...
% General introduction to continual learning
% Humans are born to be continual learners, accumulating knowledge learned throughout their entire life. 
% Learning new knowledge and skills does not easily erase a human's memory of old ones, but sometimes reinforces them and improves together~\cite{flesch2018comparing}. 
...
% Brief descriptions of major categories of CL methods
The continual learning problem has received significant attention and multiple solution themes have emerged.
% including methods based on experience replay~\cite{buzzega2020dark, lopez2017gradient}, parameter regularization~\cite{lee2017overcoming, zenke2017continual}, architectural modification\cite{rusu2016progressive, yan2021dynamically} or knowledge distillation~\cite{li2017learning, dhar2019learning, douillard2020podnet}. 
...
% \qiao{Give a reason for not mentioning the architectural methods.}


% Propose our methods
We present a continual learning algorithm, focusing on knowledge distillation (KD) in feature space. 
% Knowledge distillation (KD) was originally designed to transfer learned knowledge from large models (teacher) to small ones (student)~\cite{hinton2015distilling}. 
% Because KD distills the intermediate network output, it provides additional supervision signals for the student model and makes it easier to learn. 
In the continual learning context, KD treats the continual learning model as the student and its old checkpoint as the teacher and regularizes the network intermediate outputs to reduce forgetting~\cite{li2017learning, buzzega2020dark, douillard2020podnet, dhar2019learning, kang2022class, cha2021co2l, barletti2022contrastive, dhawan2023efficient}.
% Recent distillation-based CL methods have drawn inspiration from this and integrated distillation of logits~\cite{li2017learning, buzzega2020dark}, intermediate feature map, and attention distillation~\cite{douillard2020podnet, dhar2019learning, kang2022class} and contrastive distillation~\cite{cha2021co2l, barletti2022contrastive} into continual learning.
...
% Specifically, we found that during continual learning, new dimensions in feature space emerge to accommodate the classification of new classes, while those for old classes may diminish and lead to forgetting. 
...
% and thus it enforces that the new features contain at least the same amount of information as the old ones. 
% A key effect is that it helps retain salient information, and maintains linear separability of the previously learned classes in the new features. 
...
% In this way, it helps retain salient information (i.e., what enables linear separability) from previously learned tasks in the new features. 
% We show that this loss preserves the linear separability in the feature space in incremental learning, while allowing expansion along new feature dimensions. 
...
% \qiao{Summary of contributions of this paper.}
% \qiao{Do we need this summary of contributions?}
...
% However, This line of work has so far ignored the structure in the feature space in continual learning when applying distillation.
% However, this line of work has so far been missing a trade-off between plasticity and stability. 
