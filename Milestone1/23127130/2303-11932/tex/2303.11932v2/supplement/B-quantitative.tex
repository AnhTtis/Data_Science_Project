...
\myparagraph{Additional \iou results.} In \cref{fig:sub:iou:voc,fig:sub:iou:coco}, we show the remaining results comparing \iou vs.~\fone scores that were not shown in the main paper for \vocs and \cocos respectively.
...
\subsection{Evaluation on DenseNet and ViT models}
\label{supp:sec:quantitative:morearchs}
\input{supplement/tex_figures/backbones}

In \cref{fig:supp:moremodels}, we evaluate the best performing configurations from our study, \ie performing guidance using B-cos attributions at input, on additional model backbones, specifically DenseNet-121 and ViT-S. We find that the trends observed with ResNet-50 models generalizes to these backbones, with the Energy loss yielding the highest gains for EPG, and the $L_1$ loss yielding the highest gains for IoU.
