...
$\overline{\alpha}: A^{op}\longrightarrow C(\mathbb{G})\otimes A^{op}$ is defined by $\overline{\alpha}:=(R\otimes id)\circ\Sigma\circ\alpha$, where $R$ denotes the unitary antipode of $\mathbb{G}$.
	...
	\begin{defi}
		Let $\mathbb{G}$ be a compact quantum group. A torsion action of $\mathbb{G}$ is a $\mathbb{G}$-C$^*$-algebra $(T, \tau)$ where $T$ is finite dimensional and $\tau$ is ergodic. We say that $\widehat{\mathbb{G}}$ is torsion-free if any torsion action of $\mathbb{G}$ is $\mathbb{G}$-equivariantly Morita equivalent to the trivial $\mathbb{G}$-C$^*$-algebra $\mathbb{C}$. The set of all equivalence classes under equivariant Morita equivalence of non-trivial torsion actions of $\mathbb{G}$ is denoted by $\text{Tor}(\widehat{\mathbb{G}})$.
	\end{defi}
	\begin{note}
		Notice that there is no risk of confusion between the notation $\text{Tor}(\widehat{\mathbb{G}})$ for torsion actions of $\mathbb{G}$ and the usual $\text{Tor}$-functor.
	\end{note}
	...
	\begin{note}
		In the original paper \cite{WangSemidirect} by S. Wang the construction $\mathbb{G}\times\mathbb{H}$ is called \emph{Cartesian product of $\mathbb{G}$ by $\mathbb{H}$}. Moreover, this construction is consistent with the construction of direct product of classical discrete groups, for instance. Hence, the terminology \emph{direct product of $\mathbb{G}$ by $\mathbb{H}$} should not cause any confusion. However, we use the terminology \emph{quantum direct product} to emphasise the use of arbitrary compact quantum groups in this construction.
	\end{note}
	...
	$$\mathscr{N}_{\widehat{\mathbb{G}}}:=\mathscr{L}^{\dashv}_{\widehat{\mathbb{G}}}=\{A\in \text{Obj}(\mathscr{KK}^{\widehat{\mathbb{G}}})\ |\ KK^{\widehat{\mathbb{G}}}(L, A)\cong 0\mbox{, $\forall$ $L\in \text{Obj}(\mathscr{L}_{\widehat{\mathbb{G}}})$}\}.$$
...
\section{\textsc{The Künneth classes}}\label{sec.KunnethFunctors}
	...
	\begin{defiSec}\label{defi.KunnethFunctor}
		...
		\begin{enumerate}[i)]
			...
			\item $\kappa_*$ is stable, i.e. $\kappa_*(\Sigma(B))\cong \kappa_{*+1}(B)$ naturally, for all $B\in \text{Obj}(\mathcal{S})$.
...
\begin{theoSec}\label{theo.QuantumGroupCalgebraKunneth}
	...
	\begin{enumerate}[i)]
		\item If $(B, \beta)$ is a $\mathbb{G}$-C$^*$-algebra such that $B\underset{r, \beta}{\rtimes}\mathbb{G}\underset{r, \overline{\delta}}{\ltimes}T^{op}\in\mathcal{N}$, for all torsion action $(T, \delta)\in\text{Tor}(\widehat{\mathbb{G}})$; then $\widehat{L}(B)\in\mathcal{N}$.
