...
However, AutoBag frequently fails when attempting to orient the bag upward since it is not a stable pose for deformable bags. In this work, we 
%use AutoBag's perception representation but 
avoid orienting the bag upward. Instead, we flatten the bag and singulate the top layer of the bag to open it, which results in much higher success rates. Moreover, while AutoBag is designed specifically for opening thin plastic bags, we show evidence that \bagging is effective on other bag materials and shapes. %since flattening is easier than orienting a bag upward, and fabric smoothing methods can be applied to flattening a fabric bag.
