...
We choose the value of $\beta=$1 and consider system sizes $L=$ 32, 64, 128, and 256 such that the system remains in the transition regime from the weak to the strong disorder where avalanche dynamics can be observed.
...
\caption{The plot of $g(q)$ is plotted against $k(q)$ for different interaction ranges $(R = 1, 100, 500, 1000, 3000, 5000, 7000)$ for three different system sizes $(L = 10000, 30000, 50000)$ in LLS fiber bundle model. It is shown from the plot that terminal values of $g$ and $k$ are independent of the system size and as the $R$ values are approaches to higher, the terminal values of $g$ and $k$ are close to the analytical results.}
\label{k_vs_g_LLS}
\end{figure*}
...
\begin{equation}
    g_f(N)-g_f(N\to\infty) \propto |\sigma_c(N)-\sigma_c(N\to\infty)|\propto N^{-1/\nu}.
\end{equation}
Hence it follows that
\begin{equation}
    |g_f(N)-g_f(N\to\infty)| \propto N^{-1/2\nu}.
\end{equation}
Given that $\nu=3/2$, we should have $|g_f(N)-g_f(N\to\infty)|\propto N^{-1/3}$, which is what we see in Fig. \ref{fss_h}.
...
\begin{figure}
    ...
    \caption{System size $L$ dependence of the terminal values of $h=h_f$ for different interaction ranges $R$ for LLS fiber bundle model in a log-log scale. It is seen that for $R=1$ when the load sharing is fully local $h_f$ shows the logarithmic dependence, and when $R= 100, 500$ $h_f$ shows the power law dependence. Here the solid blue line indicates the $log(L)$ and the solid black line indicates the $L^{1/3}$}.
    \label{L_vs_hf}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}
    ...
    \caption{The $h$-index (scaled by $L^{2/3}$) is plotted against the time (scaled by time/L) for different system sizes $(L = 32, 64, 128, 256)$ for the random fuse model with power-law distributed breaking threshold. The inset shows the variation of $h$ with time for the different system sizes.}
    \label{rf_t_h}
\end{figure}
