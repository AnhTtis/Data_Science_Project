...
The \ac{fcm} approximation of this problem without a source term reads as: find $\theta_h \in V_{h,0}^{1}$ such that  
  ...
  \label{eq:RT}
\end{equation}
One can readily check that this method is weakly enforcing the zero flux condition on $\Gamma(\phi)$ as $\alpha_{\mathrm{out}} \rightarrow 0$.  We observe that we use the same differential operator in the artificial domain for stabilization purposes (times the scaling coefficient $\alpha_{\mathrm{out}}$).
...
The \ac{fcm} approximation of this problem without a forcing term reads as: find $\boldsymbol{d}_h \in \boldsymbol{V}_{h,0}^{1} \doteq  [V_{h,0}^1 ]^D$ such that  
  ...
  \label{eq:Rd}
\end{equation}
It is easy to check that the zero-stress condition on $\Gamma(\phi)$ is recovered as $\alpha_{\mathrm{out}} \rightarrow 0$. For the linear elasticity equation, we again use the same differential operator in the artificial domain for stabilization purposes.
...
is differentiable at the admissible set of domains in $\Omega$ defined by $\phi$.
In this section, we discuss how the choice of \ac{fe} stabilization can affect this property. 
...
Regions of non-differentiability arise (typically when the boundary crosses over mesh nodes) harming the convergence of the geometry to optimized solutions \cite{Sharma2016}. To see why this is the case, we investigate shape perturbations under the \ac{cutfem} and \ac{agfem} formulations.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
for the extension operator $\mathcal{E}$ and space $V^{agg}_{h,0}$ as defined in \cite{Badia2018}. Similar to the CutFEM, regions of non-differentiability in the problem exist when the zero iso-surface of the \ac{ls} crosses over mesh nodes:
\begin{equation}
  \begin{aligned}
    \lim_{\epsilon \to 0} \ 
    [
