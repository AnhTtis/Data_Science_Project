\documentclass[twocolumn,preprintnumbers,floatfix,aps,prd,amssymb,floatfix,nofootinbib,balancelastpage,superscriptaddress,amsmath]{revtex4-1}
...
%\deleted{Our results show that both calculations support that the decay of $X(4500)$ $\to$ $J/\psi\phi$ will be preponderant if $X$(4500) is a D-wave tetraquark.} 
...
%\deleted{In this paper, only the hidden-charm decay channel is considered. 
%In order to determine whether D-wave tetraquark assignment is appropriate, 
%we recommend calculating open-charm decays when assigning $X(4500)$ as a D-wave tetraquark state.
%The results are instructive for future experiments to further determine the structure of $X(4500)$.}
In this paper, only the hidden-charm decay channel is considered. 
With these results and those from the open-charm decay channel, we are able to give a more rational conclusion when comparing
with the total width of $X(4500)$. In the future, experiments will be more helpful in determining whether or not this structure of $X(4500)$ is appropriate.
...
First, it is interesting to ascertain whether the re-scattering effects may contribute to the resonance peaks in the $J/\psi \phi$ mass spectrum or not.
...
% In the constituent quark model~\cite{Fernandez:2016bqr,Ortega:2016hde}, 
% $X(4500)$ lies in the predicted mass and width ranges of \commented{$4^3P_0$}. 
% The theoretical calculation shows that the most important decay channels of $4^3P_0$ are $DD$, \commented{$DD_1^{(\prime)}$}, $D^*D^\prime$, and $D^*D^*_2$. 
% Their \commented{decay branching ratio} \deleted{branching fractions} \deleted{ranging} \added{is} around $10\%$ to $20\%$. 
% \added{However, no relevant signal of $X(4500)$ appears in the $B\to$ $D^{(*)}_{(s)}\bar{D}^*_{(s)}K$ decays}\commented{[????]}.
% \deleted{But the $B\to$ $D^{(*)}_{(s)}\bar{D}^*_{(s)}K$ decays do not appears no relevant signal about $X(4500)$.}
% \added{Similarly,} \deleted{In addition,} the mass \added{of $\chi_{c0}(4P)$} predicted in the linear potential \deleted{(LP)} model 
% \deleted{of $\chi_{c0}(4P)$} is $4506\pm 11^{+12}_{-15}$ MeV \cite{Gui:2018rvv}, which is very close to the experimental value of $X(4500)$. 
% \added{It's total decay width is about 50 MeV, which is close to the observed width of $X(4500)$ within the uncertainties.}
% \deleted{And the predicted total width of $\chi_{c0}(4P)$ is about 50 MeV, is close to $92\pm21^{+21}_{-20}$ MeV within the uncertainties, the observed width of $X(4500)$.} 
% \added{But its dominant decays into charm mesons, like $DD_1$, $DD_1^\prime$, $D^*D_0$, $D^*D_2$, do not appear in $B\to$ $D^{(*)}_{(s)}\bar{D}^*_{(s)}K$ decays}~\commented{[????]}.
% \deleted{It will dominantly decay into charm mesons like $DD_1$, $DD_1^\prime$, $D^*D_0$, $D^*D_2$ if $X$(4500) is $\chi_{c0}(4P)$.
% Likewise, $X(4500)$ do not appears in $B\to$ $D^{(*)}_{(s)}\bar{D}^*_{(s)}K$ decays.} 
% \added{In addition, since $\chi_{cJ}(nP)$ $\to$ $J/\psi\omega$ is similar to $X(4500)\to J/\psi\phi$, 
...
% But this was rejected by Babar experiment~\cite{LHCb:2016axx,LHCb:2016nsl}, which contradicts a charmonium interpretation for the $X(4500)$.}
% \deleted{Besides, since $\chi_{cJ}(nP)$ to $J/\psi\omega$ is very similar with that to $J/\psi\phi$, we can surmise the $J/\psi\omega$ mass spectrum in $B^{+}$ $\to J/\psi\omega K^+$ decay should appear some structures resembling the $J/\psi\phi$ mass peaks. But the BaBar collaboration's measurement did not find those structures, which contradicts a charmonium interpretation for the $X(4500)$.}
% \deleted{Besides,} \added{I}n the screened potential \deleted{(SP)} model \cite{Gui:2018rvv}, 
% $\chi_{c0}(5P)$ is a good candidate for $X$(4500), since its predicted mass is $4537$ \added{MeV}. 
% \deleted{Unfortunately,} \added{However,} the total width of $\chi_{c0}(5P)$ is about $15$ MeV, 4 times smaller than \added{that} \deleted{the width} of $X$(4500) observed in the experiment.
...
Although $X(4500)$ can't tally with pure charmonium, 
it could be $J/\psi-\phi$ or $\Psi(2S)-\phi$ bound state since its observation in the $J/\psi\phi$ spectrum. 
...
Similarly, in Ref.\cite{Lu:2016cwr}, it was regarded as a tetraquark that incorporate one 2S scalar diquark and one scalar antidiquark within the relativized quark model.
...
$X(4500)$ has been assigned assigned as a $IJ^P = 00^+$ compact tetraquark resonance state,
...
Because of the deficiency of hidden-charm decay width, we consider the hidden-charm decay channel in this article.
%\deleted{In Ref.\cite{Chen:2016oma}, $X$(4500) has been studied and its mass is predicted through the method of SVZ sum rules.
...
We calculate the mass of $X(4500)$ by conducting the dimension regulation in SVZ sum rules, and obtain the results comparable with those in PDG \cite{ParticleDataGroup:2020ssz}.
...
We are going to consider $X$(4500) as a D-wave tetraquark state and predict the decay width of $X(4500)$ $\to$ $J/\psi\phi$.
But we need to calculate the strong coupling $g_{X J/\psi \phi}$ in the first place.
...
&&\Pi_{\mu \nu}^{\text{TP}}\left(p^{\prime}, p, q\right) \\ \nonumber
&=&\int d^{4} x d^{4} y e^{i p \cdot x} e^{i q \cdot y}\left\langle 0\left|T\left[J^{J/\psi}_\mu(x) J^{\phi}_\nu(y) J^{X \dagger}(0)\right]\right| 0\right\rangle
\end{eqnarray}
where $p$, $q$ represent the four-momentum of $J/\psi$ and $\phi$ respectively.
Momentum conservation dictates that $X(4500)$ has the four-momentum of $p^\prime=p+q$.
Meanwhile the interpolating currents for $J/\psi$, $\phi$ are given by:
\begin{eqnarray}\label{currents}\nonumber
		&J_{\mu}^{J/\psi}(x)&=\bar{c}_m(x)\gamma_\mu c_m(x),\\
		&J_{\nu}^{\phi}(x)&=\bar{s}_n(x)\gamma_\nu s_n(x).
\end{eqnarray}
%\deleted{where $\overrightarrow{D}_\mu=\overrightarrow{\partial}_\mu +igT^jA_\mu^j$.
%The subscripts $s, c$ refer to the strange and charm quarks, and the subscripts $m$, $n$ $j$, $k$, $l$ to the color indices,and $C$ for the charge conjugation matrix} 
where the subscripts $m$, $n$ refer to the color indices.

Next, in an attempt to evaluate the phenomenological side of the correlation function,
we insert intermediate states for $X(4500)$, $J/\psi$, $\phi$ into Eq.\eqref{three-point} and write down the correlation function as
\begin{eqnarray}\label{Three-pheno} \nonumber
&& \Pi_{\mu \nu}^{\text{TP,phen}}\left(p^{\prime}, p, q\right) \\\nonumber
&=& g_{X J/\psi \phi}^{TP} g_{\mu^{\prime} \nu^{\prime}}\left(g_{\mu \mu^{\prime}}-\frac{p_{\mu} p_{\mu^{\prime}}}{m_{J/\psi}^{2}}\right)\left(g_{\nu \nu^{\prime}}-\frac{p_{\nu} p_{\nu^{\prime}}}{m_{X}^{2}}\right) \times \\ \nonumber
&& \frac{f_{J/\psi}f_{\phi} f_{X}}{\left(p^{\prime 2}-m_{X}^{2}+i \epsilon\right)\left(p^{2}-m_{J/\psi}^{2}+i \epsilon\right)\left(q^{2}-m_{\phi}^{2}+i \epsilon\right)} \\ \nonumber
&&+\ldots \\ \nonumber
...
In order to establish Eq.\eqref{Three-pheno}, we have used the relationships:
\begin{eqnarray}\label{decay-cons}\nonumber
        &&\braket{0|J^X|X(p^\prime)}=m_Xf_X\\ 
...
$f_{\phi (X, J/\psi)}$ is the decay constant of $\phi (X(4500), J/\psi)$.
...
After entering the currents into Eq.\eqref{three-point} and applying the Wick Theorem, the OPE side of the sum rules is:
\begin{eqnarray} \nonumber
		&& \Pi_{\mu \nu}^{\text{TP,OPE}}\left(p^{\prime}, p, q\right) =\int d^{4} x d^{4} y e^{i p \cdot x} e^{i q \cdot y} \\\nonumber
...
\end{eqnarray}
%\begin{eqnarray} \nonumber
%		&&\hat{\Pi}_{\mu \nu}\left(p^{\prime}, p, q\right) \\\nonumber
%				&&=\int d^{4} x d^{4} y e^{i p \cdot x} e^{i q \cdot y}\left\langle 0\left|T\left[J^{J/\psi}_\mu(x) J^{\phi}_\nu(y) J^{X \dagger}(0)\right]\right| 0\right\rangle\\\nonumber
%		&&=\int d^{4} x d^{4} y e^{i p \cdot x} e^{i q \cdot y}\{\\\nonumber
%		&&\quad\quad \text{Tr}[S^{nl}_s(-y)\gamma_\nu S^{nl}_s(y-z)\overleftarrow{D}_{\mu 3}\overleftarrow{D}_{\mu 4}\gamma_{\nu1} \\\nonumber
%		&&\quad\quad\quad \tilde{S}^{km}_c(x)\gamma_\mu\tilde{S}^{km}(-x)\gamma_{\mu2}]\\\nonumber
%		&&\quad\quad -\text{Tr}[S^{nk}_s(-y)\gamma_\nu S^{nl}_s(y-z)\overleftarrow{D}_{\mu 3}\overleftarrow{D}_{\mu 4}\gamma_{\nu1} \\
%		&&\quad\quad\quad \tilde{S}^{km}_c(x)\gamma_\mu\tilde{S}^{lm}(-x)\gamma_{\mu2}]\} .
%\end{eqnarray}
...
As we see, in the phenomenological side expressed in Eq.\eqref{Three-pheno}, there emerge several structures like $g_{\mu\nu}$, $p_\mu p_\nu$ and so on. 
Considering structures with more momenta should give better results, we consider the $p^\prime_\mu p^\prime_\nu$ structure in this works.
Besides, there is an assumption that those terms proportional to $\frac{1}{q^2}$ will contribute
to the OPE side ~\cite{Reinders:1984sr}. Hence, we derive the spectral density as:
\begin{eqnarray} 
		\rho^{TP}(\hat{s},Q^2)&=& \frac{9 \braket{\frac{\alpha_s}{\pi}GG} m_c^2 \sqrt{\hat{s} \left(\hat{s}-4 m_c^2\right)}}{16 \pi^2 Q^2 \hat{s} \left(\hat{s}-4 m_c^2\right)^2}(1-\frac{2m_c^2}{\hat{s}}) ~~~~~~
\end{eqnarray}
where $Q^2=-q^2$ and
\begin{eqnarray} 
        \rho^{\text{TP}}(\hat{s},Q^2)=\frac{\text{Im}\Pi^{\text{TP,OPE}}(p^\prime,p,q)}{\pi}.
\end{eqnarray}
Finally, we assume $p^{\prime 2}=p^2$ and conduct the Borel transformation of the variable $p^2$ on both the phenomenological
and the OPE sides to get the coupling constant: 
%\deleted{$\hat{g}_{XJ/\psi\phi}^{TP}\left(\hat{s}_{0}, M^{2}, Q^{2}\right)$. Therefore we obtian}
\begin{eqnarray} \label{Three-Point} \nonumber
...
Finally, the decay width of $X(4500)$ $\to$ $J/\psi \phi$ can be calculated by equation \cite{Dias:2013xfa}
\begin{eqnarray} \nonumber
 		&&\Gamma^{\text{TP}}(X(4500)\to J/\psi \phi)= \frac{(\hat{g}_{XJ/\psi \phi}^{TP})^2}{24\pi m_{X}^2}\\
 		&\times& \lambda(m_X,m_{J/\psi},m_{\phi})\left(3+\frac{\lambda(m_X,m_{J/\psi},m_{\phi})}{m^2_{J/\psi}}\right), \quad
\end{eqnarray}
% \begin{equation}
% 	\begin{aligned}
...
Base on the Belle Collaboration measurements~\cite{BESIII:2016bnd}, $X(4500)$ has mass of $4506\pm 11^{+12}_{-15}$ MeV.
...
A good Borel mass working region can be determined by the fact that $\hat{g}_{XJ/\psi \phi}^{TP}$ should be as much independent of $M^2$ as possible. 
In FIG.\ref{Fig:fig4a}, we show the coupling constant as functions of $M^2$ and $Q^2$ in the contour map. 
As shown in the picture, the contour lines have a minimum dependence of $M^2$ while 
\begin{equation}
    \begin{aligned}
...
from $Q^2=-m_\phi^2$.
The exponential form of $\hat{g}_{XJ/\psi\phi}^{TP}$ is pictured in Fig.\ref{Fig:fig4b}. Fig.\ref{Fig:fig4b} shows the $Q^2$ dependence of $\hat{g}_{XJ/\psi \phi}^{TP}$ while we set $M^2=4.0$ $\rm GeV^2$.
For other values of $M^2$, the results are similar in the range of $3$ $\rm GeV^2$ $\leq M^2\leq 5$ $\rm GeV^2$. The purple dots are the data we tried to fit and the
...
\begin{equation}
	\begin{aligned}
	\Gamma^{\text{TP}}(X(4500) \rightarrow J/\psi \phi)=124.32_{-12}^{+13}\ \text{MeV}
	\end{aligned}
\end{equation}
and 
\begin{equation}
	\begin{aligned}
	\Gamma^{\text{LC}}(X(4500) \rightarrow J/\psi \phi)= 116^{+38}_{-17} \ \text{MeV}
	\end{aligned}
\end{equation}
for the three-point sum rules and the light-cone sum rules respectively. Base on the experiment~\cite{BESIII:2016bnd},
$X(4500)$ has total width of $92\pm 21^{+21}_{-20}$ $\rm MeV$.
...
%\deleted{Our results indicate that, $X$(4500) $\to$ $J/\psi \phi$ will be its most significant decay channel if $X(4500)$ is a D-wave $cs\bar{c}\bar{s}$ tetraquark state.}
Therefore, we strongly recommend calculating open-charm decays when we assign $X(4500)$ as a D-wave $cs\bar{c}\bar{s}$ tetraquark state.
The D-wave interpretation of state $X(4500)$ cannot be established if the results of the open-charm decay channels, combined with the result of the hidden decay channel, exceed the total width of the state.
If all those widths of open and hidden decays are also consistent with the total width of $X(4500)$, then the D-wave assignment for $X(4500)$ may be appropriate, and $X$(4500) $\to$ $J/\psi \phi$ will be its most significant decay channel. Then we need fulture experiments to further determine the structure of $X(4500)$.
