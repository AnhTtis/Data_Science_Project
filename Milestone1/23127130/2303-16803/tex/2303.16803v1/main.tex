...
\begin{itemize}
    ...
    \item[(C4)] $\dfrac{m'}{m}$ and $\dfrac{m''}{m'}$ are decreasing functions on $(0,1)$.
\end{itemize}
\end{definition}

\begin{remark}\label{rem:Corey}
Any power function $m(s) = As^a$ with $A>0$ and power $a > 1$ is in $\mathcal{M}$, since $\dfrac{m'}{m} = \dfrac{a}{s}$ and $\dfrac{m''}{m'} = \dfrac{a-1}{s}$ are decreasing. Thus the following theorem covers the result of \cite[Theorem 409]{Castaneda}.
\end{remark}
...
\end{theorem}
\begin{proof}
In this proof, we will use the notation $i = 1, 2$. Note that since $\dfrac{m_i'}{m_i}$ and $\dfrac{m_i''}{m_i'}$, their product $\dfrac{m_i''}{m_i}$ is also decreasing. Consider the derivatives of these fractions and we obtain
\[
...
\end{proof}
Therefore, $m_b \in\mathcal{M}$ for all $\alpha, \eta \geq 2$ (or $0<\lambda \leq 2$).
...
\bibitem{Oleinik}
Oleinik, O.~A., 1957. Discontinuous solutions of non-linear differential equations (in Russian). Uspehi Mat. Nauk, 12: 3 (1957), 3-73. English transl. in Russian Mathematical Surveys, 3.

\bibitem{Gelfand}
Gelfand, I.~M., 1959. Some problems in the theory of quasilinear equations (in Russian). Usp. mat. nauk, 14(2), pp.87-158.
...
\bibitem{Castaneda}
Casta\~{n}eda, P., 2016. Dogma: S-shaped. press in The Mathematical Inteligencer.
