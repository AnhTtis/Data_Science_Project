...
In this section, we aim to give an upperbound, dependent on $\eta$, for the number of Algorithm \ref{alg:sslo}'s iterates. To this purpose, we consider the following assumption, which allows us to show the convergence of $\{x_k\}_{k\geq 1}$ in Lemma~\ref{lmm: convergence}.
\begin{assumption}
...
\noindent\textbf{Discussion}:\vspace{0.2cm}\\
...
\includegraphics[width=0.6\linewidth]{LM.pdf}
    \caption{Objective value as a function of computation time: $M_0=3$ fixed and $L_0$ varied}
    \label{fig:objective_decrease:M fixed}
\end{figure}
\begin{figure}[htbp!]
    \centering
\includegraphics[width=0.6\linewidth]{LM2.pdf}
    \caption{Objective value as a function of computation time: $L_0=5$ fixed and $M_0$ varied}
