...
\title{\huge Image Label based Semantic Segmentation Framework using Object Perimeters\\
...
%\author{\IEEEauthorblockN{ Erik Ostrowski} %1\textsuperscript{st}
%\IEEEauthorblockA{\textit{Institute of Computer Engineering} \\
%\textit{TU Wien}\\
%Vienna, Austria \\
%erik.ostrowski@tuwien.ac.at}
%\and
%\IEEEauthorblockN{ Bharath Srinivas Prabakaran} % 2\textsuperscript{nd}
%\IEEEauthorblockA{\textit{Institute of Computer Engineering} \\
%\textit{TU Wien}\\
%Vienna, Austria \\
%bharath.prabakaran@tuwien.ac.at}
%\and
%\IEEEauthorblockN{ Muhammad Shafique} % 3\textsuperscript{rd}
%\IEEEauthorblockA{\textit{Division of Engineering} \\
%\textit{New York University (NYU)}\\
%Abu Dhabi, UAE \\
...
We conduct an exhaustive analysis to illustrate that our framework enhances existing state-of-the-art frameworks for image-level-based semantic segmentation.
The framework is open-source and accessible online at \url{https://github.com/ErikOstrowski/Perimeter-based-Semantic-Segmentation}. 
\end{abstract}
...
\begin{figure*}[ht]
\centering
\includegraphics[width=150mm,scale=0.50]{Figures/flow.png} %width=80mm,scale=0.80 width=\linewidth
\caption{High-level overview of training of the proposed framework, starting with the input and a CAM to create the first predictions, then our PerimeterFit module pre-refines the input with the help of the perimeter map, then AffinityNet performs the final refinement and a fully supervised network uses the refined masks as labels.\label{flow}}
\end{figure*}


%\begin{table}[ht]
%\caption{Comparison of pseudo labels of our framework over the baseline. B is the baseline CAM, PF our PerimeterFit module, Aff. the AffinityNet refinement and CRF is the Conditional Random Field.\label{check}}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.65\columnwidth]{Figures/CAM_comp.pdf} %width=80mm,scale=0.80 width=\linewidth
...
On the other side, pixel-similarity-based methods can generate sharp borders.
...
Therefore, we propose a framework that introduces a pre-refinement step between CAM and pixel-similarity-based refinement.
...
Fig.~\ref{flow} presents a high-level overview of our framework.
...
\begin{enumerate}
    \item Our framework improves the prediction quality of the segmentation mask by introducing an additional step between CAM and pixel similarity-based refinement.
   \item  Our PerimeterFit module can be incorporated into any conventional CAM framework and already working WSSS pipelines. 
   Since the PerimeterFit module is only used for the generation of labels, it won't add more computations for inference predictions.
   \item  We have presented detailed ablation studies and analysis of the results comparing our framework to state-of-the-art methods on the VOC2012 dataset to evaluate our method's efficacy and the improvements achieved. 
   \item  The complete framework, including the novel PerimeterFit module, is open-source and accessible online at \url{https://github.com/ErikOstrowski/Perimeter-based-Semantic-Segmentation}.
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Additionally, Lee et al. introduced a regularization to punish activations outside the adversarially highlighted areas.
...
% \caption{Comparison of state-of-the-art and our framework on their major source of improvement. Aff. stands for AffinityNet and O.P. of object perimeters.\label{check}}
...
Our framework             & \cmark  &   \cmark &   \cmark  \\
\end{tabular}}
\end{table}
...
%our framework (Ours)            & \cmark     & \cmark  &   \cmark   \\
%\end{tabular}}
%\end{table}
...
Moreover, we note that Wang et al.~\cite{ MCOF} did utilize a kind of perimeter-based approach, but without many of the additional steps our framework uses to maximise the utility.
...
%\begin{multicols}{3}
%\end{multicols}
Fig.~\ref{fig2} presents an overview of our framework.
...
Our framework starts with the generation of an initial response map using a CAM model, which given an input image $I$, will return $n$ masks.
...
Instead, we propose to apply an intermediate step to improve the CAM's quality.
...
For this purpose, we choose a clustering method that groups image pixels together meaningfully, so we do not lose the target object's shape.
...
A low $q$ value results in a very coarse simplification with next to no objects left, while a high $q$ value results in very little simplification of the original image.
...
Thresholds are essential hyperparameters for our framework because they are a major factor in balancing the overall quality of the CAM and its FP rate.
...
\begin{figure}[t]
\centering
\includegraphics[width=60mm,scale=0.60]{Figures/Edges.pdf} % width=50mm,scale=0.50 width=\linewidth
\caption{Analysing the effect of USS on images. (a) and (c) illustrate the use of edge detection on the original images, whereas (b) and (d) illustrate the decrease in edges when USS-generated perimeter maps are used as input.\label{edge}}
...
This methodology works great in cases where the initial response map contains only one part of the object while the rest differs very little from the recognized part.
...
We utilize the DeepLab-V3+ model with the ResNeSt-101 architecture~\cite{RESNEST} as the backbone model, similar to PuzzleCAM~\cite{PUZZLE}.
...
\end{equation}}
Generally, based on experimental results, we have observed that negatively predicted pixels are determined with pretty high confidence, whereas the confidence of positive pixel prediction is comparatively low.
...
Since AffinityNet uses a set of positive pixels and negative pixels as anchors to determine if the pixels in between are more similar to the positive or negative pixel set, it is of utmost importance to ensure that the anchors are correctly classified, i.e., true-positive or true-negative.
...
On the one side, we observed that this method excels when using images with very few objects of different colors, e.g., a green plane in the blue sky.
...
Before discussing the results and illustrating our framework's efficacy, we first discuss the experimental setup used to generate the results discussed in this section. 
...
\begin{table}[ht]
\caption{Performance in mIoU (\%) for PuzzleCAM and our framework on the VOC2012 training dataset.\label{tab1}}
...
Our framework & 61.1 & 70.0   & \textbf{70.2}        \\ \hline
\end{tabular}}
\end{table}


Table~\ref{tab1} compares the mIoU score of our framework and the baseline, PuzzleCAM, when deployed with a ResNeSt101 backbone.
We observe that our framework achieves better prediction quality than the baseline, across all stages, even through the AffinityNet stage.
Note that applying our framework after the AffinityNet, does not lead to any improvements in the quality of the masks. 
...
\centering 
{%\fontsize{9pt}{9pt}\selectfont
\caption{Comparison of our framework with state-of-the-art techniques on the VOC2012 val and test datasets.\label{tab2}}
\begin{tabular}{lccc}
\hline
...
Our framework        & ResNeSt-101    & 68.4 &  68.0 \\
...
Our framework (w/o CRF)              &\textbf{89.0}& 78.3        &\textbf{44.4}& 88.1        &\textbf{66.0}& 73.8        & 84.0        & 73.8        &\textbf{92.2}& 30.6        & 83.1\\ \hline
...
Our framework (w/ CRF)               & 89.1        & 78.9        &\textbf{44.9}& 88.5        &\textbf{66.7}& 73.7        & 84.0        & 73.8        &\textbf{92.4}& 30.8        & 84.0
\end{tabular}}%}
...
Our framework (w/o CRF)               & 44.4           & 90.8          & 82.0        & 79.5         & 46.4        & 66.7        &\textbf{92.5}&\textbf{53.2}&\textbf{68.5}& 42.3        &\textbf{70.0}\\ \hline
...
Our framework (w/ CRF)                & 44.6           & 91.2          & 82.3        & 79.5         & 46.5        & 66.9        &\textbf{93.2}& 53.3        & 68.3        & 42.4        &\textbf{70.2}
\end{tabular}}%}
...
Our framework performs better than most state-of-the-art approaches on both validation and testing sets.
Only the CLIMS approach reaches a higher mIoU score than our framework.
Note that since our framework is the only approach utilizing edge detection, its use is orthogonal to state-of-the-art \cite{AdvCAM} or \cite{CLIMS}, and deploying them together may further increase the output quality.
Additionally, our framework reaches a higher score than the more recent AdvCAM, despite using an outdated baseline. %, despite it being better than PuzzleCAM.
...
As seen in Table~\ref{tab2}, although our framework achieves the highest quality predictions, on average, we observe in Table~\ref{tab3} and Table~\ref{tab4} that our framework might perform worse than the baseline for specific classes.
This hints that for some classes, our framework removes a few of the true positives and thereby reduces the performance of the class.
Our framework loses around $2\%$ mIoU to PuzzleCAM on the classes \texttt{car} and \texttt{cow}.
...
In contrast, our framework gains $4.9\%$, $3.7\%$, and $3.5\%$ over PuzzleCAM for the classes \texttt{boat}, \texttt{person}, and \texttt{train}, respectively.
...
%For example, we observe that our framework performs better compared to the baseline on geometrically differentiable classes like \texttt{tv} while performing worse on classes that tend to overlap with other surrounding objects like \texttt{table}.
Our framework is better than PuzzleCAM for $11$ out of $10$ classes.
However, on average, our framework gains $2.2\%$  if it is better, whereas PuzzleCAN gains, on average, $0.9\%$ over our framework when it is better.
Therefore, we conclude that if our framework reduces the mask too much compared to the baseline, then we tend to lose not many true positives. At the same time, our framework gains more mIoU points when it is successful in removing false positives and performs best when there is mostly a singular big object in the given image.
...
Our framework & 0.27/0.39 & 0.26/0.19   & \textbf{0.25}/0.18        \\
\hline
\end{tabular}}

\end{table}

We will now analyze the prediction results of our framework compared to the baseline, PuzzleCAM, at different stages of the framework to see where the improvements are derived.
...
\caption{Example results of our framework and PuzzleCAM for comparison. The first row shows the original VOC2012 images. The first image contains two objects of the class bus, the second a monitor and bottle, the third a ship and a car, and the fourth two couches. The second row shows the ground truths, the third row the results of the PuzzleCAM framework, and the last row the results of our framework.
\label{examples}}
\end{figure*}

When analyzing our framework's and PuzzleCAM's predictions with these two metrics ($m_{FP}$ and $m_{FN}$), we have observed a reduction in the number of false-positive values but an increase in false-negative values for the proposed approach when compared to the baseline, as seen in Table~\ref{tab5}.
...
In the third row, we show the results of the baseline PuzzleCAM, and finally, the last row shows the results of our framework. 
...
Surprisingly, the mask of our framework covers more of the target object in the third image than PuzzleCAM.
...
In this paper, we have proposed our framework that introduces the novel PerimeterFit module between CAM and the Affinitynet model to address the weaknesses of pixel-similarity-based refinement methods.
...
We showed that the predictions generated by our framework achieve state-of-the-art performance on the VOC2012 dataset, which proves its effectiveness compared to other approaches.
Our framework is open-source to ensure reproducible research and accessibility.
The source code is online at \url{https://github.com/ErikOstrowski/Perimeter-based-Semantic-Segmentation}.
