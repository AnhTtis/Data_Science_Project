\begin{algorithm}[t!]
	\caption{Expected Actor-Critic Deep Hedging with Compound Neural Networks}
	\label{algorithm:expected-deep-hedging}
	\begin{algorithmic}
		...
		\STATE Compute action $a^i_t:=\text{\normalfont Tr}[O^{\mathbf{a}}_t\rho^{\boldsymbol{\phi}_{t}}_t(\mathbf{a}|s_t^i) ]$. 
		...
		$$\widetilde{\mathcal{L}}(\boldsymbol{\omega}) =  \frac{1}{N}\sum_{i=1}^{N}\sum_{t=0}^{T} \big( \text{\normalfont Tr}[O^{\mathbf{z}}_t \rho^{\boldsymbol{\omega}_t}_t(\mathbf{z}|s^i_t))] - \exp(-\lambda\widetilde{R}_t^i) \big)^2.$$
