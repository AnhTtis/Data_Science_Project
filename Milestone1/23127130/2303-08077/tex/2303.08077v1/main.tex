...
Furthermore, the recent detection of f modes in Saturn \citep{Hedman2013} has sparked kronoseismic investigations into Saturn's core which required taking into account the effects of rotation on Saturn's structure and pulsations \citep{Fuller2014}.
...
baroclinic.  Currently, neither code is capable of carrying out stellar
...
\citet{Zahn1992} and \citet{Maeder1998}.  \citet{Manchon2021} is developing a method which
...
The hydrostatic equilibrium of a rotating star with a conservative rotation
...
from a potential and leads to a barotropic structure for the star, as can be
seen by taking the curl of Eq.~(\ref{eq:hydrostatic}) divided by the density: $\grad \rho \times \grad P = \mathbf{0}$.

Let us introduce a coordinate system $(\zeta,\theta,\varphi)$ such that $\zeta$ is constant along isopotential lines.
We note that at this stage, the values that $\zeta$ takes on are arbitrary apart from the need for a monotonic variation from the centre of the star or planet to the surface. 
Accordingly, given the barotropic structure of the star, quantities such as $P$ and $\rho$ only depend on $\zeta$. 
...
the above equation at the star's centre, \ie\ $\rsph=\zeta=0$ (assuming the
...
which is a simplified version of the Self-Consistent Field algorithm
\citep{Jackson2005, MacGregor2007}:
\begin{enumerate}
\item Find the gravitational potential based on the matter distribution.  
\item Add the centrifugal potential to the gravitational one.
\item Find the level surfaces, \ie\ the lines of constant total potential, and redistribute the matter on these surfaces.
\item Return to step 1 and iterate till convergence.      
\end{enumerate}
...
\subsection{Finding the gravitational potential}

There are two different ways of calculating the gravitational potential:
\begin{itemize}
\item Interpolating the matter distribution onto the spherical coordinate system
      and solve Poisson's equation after having projected it onto the spherical
      harmonic basis.
\item Solving Poisson's equation using directly the spheroidal coordinate system.
\end{itemize}

\subsubsection{Using spherical coordinates \label{ss:Poisson_spherical}}
...
The interpolation onto the spherical coordinate system is carried out using
...
\begin{equation}
\rho(r,\theta,\varphi) = \sum_{\l=0}^{\infty} \sum_{m=-\l}^{\ell} \rholm(r) \Ylm(\theta,\varphi),
\label{eq:harmonic_decomposition}
\end{equation}
where
\begin{equation}
\rholm(r) = \iint_{4\pi} \rho(r,\theta,\varphi) \left[ \Ylm(\theta,\varphi) \right]^*
...
Given that the star is axisymmetric, only the $m=0$ components of $\rho$ and
...
\begin{equation}
\PhiGl = -\frac{4\pi\mathcal{G}}{2\l+1} \left[\int_0^r \rhol(s) \frac{s^{\l+2}}{r^{\l+1}} ds
       + \int_r^R \rhol(s) \frac{r^{\l}}{s^{\l-1}} ds\right],
\end{equation}
...
system using an efficient band matrix factorisation using the appropriate
\href{https://netlib.org/lapack/}{Lapack} wrapper available in \href{https://docs.scipy.org/doc/scipy/reference/linalg.lapack.html}{\texttt{scipy.linalg.lapack}}. This requires using
the following boundary condition on $\PhiGl$ to ensure it matches a vacuum
potential, \ie\ a non-divergent solution to Laplace's equation outside the star:
\begin{equation}
\dfrac{\PhiGl}{r}(R) + \frac{\l+1}{R} \PhiGl(R) = 0
\label{eq:Poisson_boundary_condition}
\end{equation}

Equations~\eqref{eq:Poisson_harmonic} together with the boundary conditions \eqref{eq:Poisson_boundary_condition} are solved up to a certain degree, $L$, which is fixed by the user at the beginning of the procedure. Once the $\PhiGl(r)$ functions have been obtained, the potential is then deduced in
terms of the $(r,\theta)$ coordinates using an equation analogous to
Eq.~(\ref{eq:harmonic_decomposition}).

\subsubsection{Using spheroidal coordinates \label{ss:Poisson_spheroidal}}
...
level surface due to the star's barotropic structure.  Hence, it will intersect
...
\begin{equation}
\rho(\zeta,\theta,\varphi) = \sum_{\l=0}^{\infty} \sum_{m=-\l}^{\ell} \rholm(\zeta) \Ylm(\theta,\varphi),
\label{eq:harmonic_decomposition_spheroidal}
\end{equation}
If we treat the radial distance, $r$, as a function of $\zeta$ and $\theta$,
then thanks to tensor analysis, we obtain the following explicit expression for
Poisson's equation:
\begin{equation}
...
star, a second domain is added with the star's surface as an inner boundary and
...
the gravitational potential and its gradient which, in terms of the spheroidal coordinates, results in preserving $\rz^{-1}\dz \PhiG$ in addition to $\PhiG$ through the surface. In terms of harmonics, the boundary conditions on the surface are written as:
\begin{align}
    \label{eq:interface_phi}
    \forall \ell, \qquad\qquad\quad \PhiG^\ell &= \PhiG^\ell, \\
    \label{eq:interface_dphi}
    \sum_{\ell' = 0}^L \mathcal{P}^{\ell \ell'}_{\mathrm{BC}} \dz \PhiG^{\ell'} &= \sum_{\ell' = 0}^L \mathcal{P}^{\ell \ell'}_{\mathrm{BC}} \dz \PhiG^{\ell'},
\end{align} where left-hand side (respectively right-hand side) terms denote quantities  below (resp. above) the surface. In the case of internal density discontinuities, boundary conditions~\eqref{eq:interface_phi} \& \eqref{eq:interface_dphi} must be added at each of the domain interfaces. These two conditions are derived in Appendix~\ref{app:Poisson}. Finally, conditions analogous to Eq.~(\ref{eq:Poisson_boundary_condition}) are found on the outer spherical boundary.

Once more, $\PhiG(\zeta,\theta)$ can subsequently be deduced from the
$\PhiGl(\zeta)$ using Eq.~(\ref{eq:harmonic_decomposition_spheroidal}).  It should be noted that the latter equation implicitly also gives us the gravitational potential as a function of $r$ and $\theta$ by using the relation $r(\zeta, \theta)$.

\subsection{Adding the centrifugal potential}


Whether solving Poisson's equation in spherical (cf. paragraph \ref{ss:Poisson_spherical}) or spheroidal (cf. paragraph \ref{ss:Poisson_spheroidal}) coordinates, we derived the gravitational potential $\PhiG(r,\theta)$ at this point. The total potential at any point in the structure is therefore determined by simply adding the centrifugal potential $\PhiC(r,\theta)$. As mentioned above, the latter must satisfy a cylindrical symmetry in order to preserve the barotropic relation. As a consequence, this approach cannot be used on potentials derived from shellular rotation profiles, \ie ~$\Omega = \Omega(r)$, for instance. It is worth noting, however, the large number of profiles that can be used, since any 1D rotation profile integrated using Eq.~\eqref{eq:def_PhiC} could lead to an eligible $\PhiC(s)$ in theory. In practice, it may be advisable to check that at least the Rayleigh criterion for stability is verified: $\partial_r(r^4\Omega^2) > 0$.
...
Once the total potential has been calculated, level surfaces may be obtained by
finding isopotential lines.  As a first step, a set of target values at which to
calculate these level surfaces are deduced from the initial (spherical)
potential to which a suitable constant has been added based (see
Eq.~(\ref{eq:potential_1D})).  Indeed, as explained at the start of the section,
...
Then, for each latitude, the set of $r$ values such that $\Phieff(r,\theta) =
\Phi_{\mathrm{sph}}(\rsph)+\mathrm{cnst}$ are obtained. The value of this constant is immediately found by applying the same relation at the model centre:
\begin{equation}
    \mathrm{cnst} = \PhiG(0, 0) + \PhiC(0, 0) - \Phi_{\mathrm{sph}}(0).
\end{equation}
Finding the level surfaces can be achieved in
many different ways.  In our algorithm, we decompose $f(r)=\Phieff(r,\theta)$ as a
sum of Hermite's splines (since both $\Phieff$ and $\partial_r\Phieff$ are available), store the relevant spline
coefficients, and subsequently apply Newton's root finding method to each of
the target values for the potential, using as an initial guess the relevant $r$
value from the level surfaces of the previous iteration.  This approach can
reach a high degree of accuracy fairly quickly given the efficiency of Newton's
method and the fact that interpolated function values and their associated
derivatives are readily available for a spline representation of the function.
...
\begin{equation}
    \frac{df}{dr} =  \left.\frac{\partial\phi_g}{\partial r}\right|_{\theta=0} + \mathcal{R}\,{\left(\frac{\omega_\mathrm{eq}}{r}\right)}^2
...
\end{center}
\caption{Number of iterations needed to reach the convergence criterion (in this case $\left|(\Rpol/\Req)_{i+1} - (\Rpol/\Req)_{i}\right| < 10^{-11}$) for both the fixed rotation rate approach (red tone curves) and ``adaptive'' method (blue tone curves) presented in paragraph~\ref{par:adaptive_rotation_rate}. 
...
%Figure~\ref{fig:polytrope_surface_diff} shows the differences between their surfaces.  As can be seen, the relative differences are of the order of $10^{-9}$.
...
%In order to do a more in-depth verification, we also compared level surfaces within the models at $0.8\,\OmegaK$.  
...
Figure~\ref{fig:polytrope_levels_diff} shows the result of this comparison.  The maximum differences are of the order of $10^{-9}$ -- and, in most of the model, well below this value -- thus confirming the excellent agreement between the two methods. The most central (and pronounced) differences here are the result of the solving method used for Poisson's equation. In RUBIS, this equation is solved on $r^2$ rather than $r$ for regularity purposes, which may explain the $10^{-9}$ variations on $\PhiG$ in this region although the deformation is very small in practice.
...
In the following, we will account for the impact of those differences on the models' oscillation frequencies.  Using TOP \citep{Reese2006,Reese2009a}, we computed and identified the oscillation modes corresponding to the $\Tilde{\ell} = 0, 1$, $\tilde{n}=9,23$ acoustic island modes (see \citealt{Lignieres2008}, \citealt{Reese2008b}, and \citealt{Pasek2012} for a definition of $\Tilde{\ell}$ and $\tilde{n}$ and their link with usual spherical quantum numbers). 
...
\end{equation}
and therefore to account for the departures from a spherically symmetric matter distribution caused by the rotation.
...
However, a considerable difficulty in comparing these observational values with those of Jovian models is the accuracy that deformation codes can achieve for these moments. A classic benchmark for testing this is the calculation of the gravitational moments of the $N=1$ polytrope with deformation parameter $q = \omega^2 = 0.089195487$, for the reason that they can be calculated analytically. We provide, in Table~\ref{tab:moments_poly}, a comparison between these analytical values and the numerical estimates from several methods: the Theory of Figure (ToF) to order $q^3$, the Concentric MacLaurin Spheroids (CMS) method using $512$ spheroids and the method presented here. The studies from which these values are taken can be found under the table. We facilitate the comparison between columns by indicating in red the digits that do not match the analytical values.
...
Now looking at the boundary conditions to impose on the domain interfaces, the two quantities that need to be continuous are $\PhiG$ and its gradient.  The continuity of $\PhiG$ leads to the first condition on $\PhiGl$, \ie\ its continuity as expressed in Eq.~\ref{eq:interface_phi}. In order to find a second boundary condition, the gradient of $\PhiG$ must first be re-expressed from the natural basis to an orthogonal basis (such as the spherical one):
\begin{equation}
    \begin{split}
        \bm{\nabla}\PhiG &= \dz\PhiG \, \bm{\upzeta} + \dt\PhiG \, \bm{\uptheta} \\
...
\end{align}
where `-` and `+` denote quantities below and above the interface. Therefore, both $\rt$ and $\dt\PhiG$ are continuous, and preserving $\dt\PhiG - (\rt/\rz)\dz\PhiG$ is equivalent to preserving $\dz\PhiG/\rz$. Let us now express this condition on the $\PhiGl$. We have:
\begin{equation}
    \frac{\dz\PhiG}{\rz} = \sum_{\ell' = 0}^\infty \frac{\dz\PhiG^{\ell'}}{\rz} P_{\ell'}
\end{equation} and projecting this decomposition on the $\ell$-th polynomial leads to:
\begin{equation}
...
We recognise here condition~\eqref{eq:interface_dphi}.
