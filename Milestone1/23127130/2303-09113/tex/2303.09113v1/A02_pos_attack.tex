\section{Attack On PoS LC Without Equivocation Removal}
\label{sec:pos-attack}

In this section, we present an attack to establish a bound (as a function of the security level) on the block production rate (and hence, throughput, or bandwidth requirement) of a single chain PoS LC protocol without an equivocation removal policy.
For concreteness, we demonstrate this attack on PoS LC using any one of three download rules: `download the longest header chain', `download towards the freshest block', and `equivocation avoidance'.
%
...
%
On the other hand, \cite{bwlimitedposlc} proved PoS LC secure under the other two download rules by setting the duration of a \timeslot proportional to the security parameter $\kappa$, to achieve security with probability $1-\negl(\kappa)$.
...
Furthermore, while the attack in \cite[Figure~3]{bwlimitedposlc} required that the PoS LC protocol rejects blocks with invalid transactions after downloading them, this attack does not require that. Therefore, this attack works even if the PoS LC protocol accepts blocks with invalid transactions into the output ledger (\eg, to subsequently clean them up deterministically across honest nodes).
This is because as noted in \secref{throughputloss}, even if the protocol accepts blocks with invalid transactions, honest nodes must download the block content (to ensure data availability). 
...
The `download longest header chain' rule is as described in \algref{longest-header-chain-rule}.
...
%
The attack works in two phases. See Figure~\ref{fig:tp-log-kappa-attack} for reference.
...
The analysis below reuses notation defined in Section~\ref{sec:analysis-definitions}.
...
This attack breaks safety of the protocol because the downloaded longest chain of honest nodes switches to a different chain every time the condition in Section~\ref{sec:tp-attack-strategy}~(\ref{item:tp-attack-exec-end}) occurs.
%
...
Building up on the definitions from Section~\ref{sec:analysis-definitions}, define a time slot $t$ to be \emph{honest} if $H_t>0$, and \emph{attacking} if $A_t>0$. 
%
%
...
\begin{IEEEeqnarray}{C}
    %
    F_t := \left\{ \exists r < t \colon (\Hat{r} > 0) \land (\Aint{r,t} \geq \Hint{r,t}) \land (\Aint{r,t} \geq L) \right\}. \IEEEeqnarraynumspace
\end{IEEEeqnarray}
\end{definition}
...
\end{lemma}
\begin{proof}
For all blocks $b$ that are produced in slot $t$, the attack strategy in Section~\ref{sec:tp-attack-strategy}
...
\begin{IEEEeqnarray}{rCl}
    \Prob{ \Bint{s,t} < p(t-s)(1-\delta) } &\leq& \exp\left( -\frac{p(t-s)\delta^2}{2} \right), \\
    \Prob{ \Bint{s,t} > p(t-s)(1+\delta) } &\leq& \exp\left( -\frac{p(t-s)\delta^2}{3} \right).
\end{IEEEeqnarray}
%
%
%
...
\begin{IEEEeqnarray}{rCl}
    ...
    \implies \Prob{ 2L \leq \Bint{s,t} \leq 2L(1+2\epsilon) } &\geq& 1 - 2\exp\left( -\frac{2L\epsilon^2}{3(1+\epsilon)} \right). \IEEEeqnarraynumspace
\end{IEEEeqnarray}
%
%
%
...
\begin{IEEEeqnarray}{rCl}
    \Prob{ F_t } \geq \pu \left(1 - 2e^{-2L\epsilon^2/3(1+\epsilon)} \right) \frac{1}{\sqrt{4L(1+2\epsilon)}} e^{-2\constAttackSetup L(1+2\epsilon)}.   \IEEEeqnarraynumspace
\end{IEEEeqnarray}
%
%
%
...
\end{lemma}
\begin{proof}
We begin by calculating the probability there is at least one attacking slot before there is an $T$-loner. This ensures that the final step of the attack in Section~\ref{sec:tp-attack-strategy} is successful and that the adversary can updates it state and continue the attack.
...
\end{lemma}
\begin{proof}
From Corollary~\ref{cor:attack_setup_prob}, for $L=\log(\kappa)$ and large enough $\blkrateslot$, the attack setup occurs with non-negligible probability.
...
The condition $\frac{\blkrateslot}{\slotduration} > \frac{\bwtime}{L}\log\frac{1-\beta}{\beta}$ is derived from the condition in Lemma~\ref{lem:tp-attack-t-loner-prob} with $T = \frac{L}{\bwtime\slotduration}$.
\end{proof}
...
\end{corollary}
This is seen by noting that the throughput is $\frac{1-e^{-\blkrateslot}}{\slotduration} \leq \frac{\blkrateslot}{\slotduration} \leq \frac{\bwtime}{\log\kappa}\log\frac{1-\beta}{\beta}$. If this is not true, then the attacker never terminates, hence there is a safety violation as per Lemma~\ref{lem:tp-attack-safety-viol}. The maximum block production rate $\lambda = \frac{\blkrateslot}{\slotduration}$ calculated from \lemref{tp-attack-never-terminates} is plotted in \figref{comparison-bddelay-bdbandwidth}(b).