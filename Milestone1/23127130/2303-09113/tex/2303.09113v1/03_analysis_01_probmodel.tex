...
%
of both PoW and PoS as per \algref{hdrtree-pow, hdrtree-pos}.
This enables us to prove properties of the block production process and block tree structure that are common to both variants (\secref{analysis-details}).
We then use these properties to prove security of PoW LC (\secref{pow}) and PoS LC (\secref{pos}).
%

Recall that the protocol runs in discrete units of time called \timeslots, and that we consider $\slotduration \to 0$ to model PoW.
...
As noted in \secref{modelprotocol-protocolfeatures}, each \BPO corresponds to a different \timeslot, and in both PoW and PoS we may assume that blocks in one chain must come from increasing \timeslots.
