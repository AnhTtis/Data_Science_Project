...
%
%
In \lemref{one-cp-induction-full}, we showed that under this download rule, \sltcps occur in every $\Kcp$-interval. 
We will use this to prove safety and liveness 
%
and identify the protocol parameters for which this holds \wop in \thmref{safety-and-liveness-pow}.
Proofs are in \secref{appendix-pow-proofs}.
...
%
%
As noted in \secref{modelprotocol-protocolfeatures},
it is most appropriate for 
PoW to set $\slotduration \to 0$, and
%
to state its security properties in terms of real time.
%
In order to use the results from \secref{analysis}, we must bridge between \iindices and real time.
...
Proof details in \secref{appendix-pow-proofs}. Result with $\DeltaHeader \approx 0$ (reasonable approximation for large block sizes) plotted in \figref{comparison-bddelay-bdbandwidth}(c).
...
\end{IEEEeqnarray}
the PoW longest chain protocol $\protocol$ 
with 
