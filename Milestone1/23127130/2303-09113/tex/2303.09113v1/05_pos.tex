...
Pseudocodes \algref{posequivblank-lc} and \algref{posequivblank-hdrtree} are in \secref{appendix-pos-eqremoval-pseudocodes}.

\paragraph{The Download Rule in \sapos}
%
On top of any existing download rule (such as $\dlrulelong$), we add another rule that an honest node does not download content for a header $\Chain$ if it has seen 
...
%
(\myalgref{posequivblank-lc}{posequivblank-lc-downloadrule}).
...
In order to output a consistent ledger that all honest nodes have downloaded, honest nodes must agree on which blocks had an equivocation, and must unilaterally blank their contents.
...
The node then creates an equivocation proof which consists of the two block headers $\Chain$ and $\Chain'$ and adds the equivocation proof to the header of the block that it creates (\myalgref{posequivblank-lc}{posequivblank-lc-construct-eq-proof}).
...
%
Therefore, a block header $\Chain$ is considered invalid if it contains an equivocation proof against a block not in the prefix of $\Chain$, a block more than $\keqproof$ blocks above $\Chain$, or contains an equivocation proof that has already been proven in the prefix of $\Chain$ (\myalgref{posequivblank-hdrtree}{posequivblank-hdrtree-valid-eq-proof}).
...
(\myalgref{posequivblank-lc}{posequivblank-lc-confirmation-blanking}).
...
Recall that the analysis in \secref{analysis-details-many-pps-one-cps} uses four properties of the download rule. It is easy to see that with the addition of \equivocationremoval, the `download longest header chain' rule satisfies these properties in PoS.
...
Finally, \sapos does not spend bandwidth on any more blocks than the base download rule does, and since the base download rule $\dlrulelong$ does not download blocks before the most recent \sltcp (\propref{download-or-spend-budget}), the rule with \equivocationremoval also does not \ref{item:good-download-rule-cutoff}.
This means that the analysis of \secref{analysis-details-many-pps-one-cps} works for \sapos.
%
...
%
in \sapos
retains consistency. That is, if one honest node blanks the content of a block in its ledger, then all honest nodes do. Conversely, if one honest node does not blank the content for a block in its ledger, no honest node does. Proof details in \secref{appendix-pos-proofs}.
Result with $\DeltaHeader \approx 0$ (large block sizes), and $\slotduration \to 0$ (small slot approximation) plotted in \figref{comparison-bddelay-bdbandwidth}(c).
...
\begin{IEEEeqnarray}{C}
    ...
    \quad \probGood = (1-\beta)e^{-\frac{\blkrateslot}{\slotduration} \left(\DeltaHeader + \goodsepbw/\bwtime\right)}, \IEEEeqnarraynumspace
\end{IEEEeqnarray}
