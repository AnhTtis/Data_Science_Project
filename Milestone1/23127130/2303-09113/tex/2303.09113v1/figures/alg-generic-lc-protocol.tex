\begin{algorithm}[tb]%
    \caption{%
        Idealized LC consensus protocol $\protocol$ with download logic
        (helper functions: \secref{algos-reference-helperfunctions},
        environment $\Env$: \secref{algos-reference-environment},
        functionality $\Ftree$: \algref{hdrtree-pow} for PoW,
        \algref{hdrtree-pos} for PoS)%
    }%
    \label{alg:generic-lc-protocol}%
    \begin{algorithmic}[1]%
        \footnotesize%
        
        \LineComment{Global counter of \timeslots $t \gets 1,2,...$ of duration $\tau$ (for PoW: $\tau \to 0$, \cf~\secref{pow})}
        ...
        \LineComment{Throughout, download content for some $\Chain$ chosen by download rule (\eg \algref{longest-header-chain-rule})
        %
        }
