\begin{algorithm}[tb]%
    ...
    \begin{algorithmic}[1]%
        ...
        %
        %
        \LineComment{$\Call{init}{\genesisHeaderChain, \mathsf{numNodes}}$ and $\Call{verify}{\Chain}$ same as in \algref{hdrtree-pow}}
        
        \LineComment{$\Call{isLeader}{P,t}$ same as in \algref{hdrtree-pos}}
        %
        %
        %
        %
        ...
        \On{$\Call{extend}{t', \Chain, \txs \textcolor{jnSUDigitalGreen}{, \eqProofs}}$ \textbf{from} $\mathcal A$ (from adversarial node $P$) or $\FtreePoS$}
