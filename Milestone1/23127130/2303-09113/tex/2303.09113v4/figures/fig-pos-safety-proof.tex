\begin{figure}[tb]
    ...
    \begin{tikzpicture}[blockchainold,x=0.8cm,y=0.4cm]
        ...
        \draw [<-,dashed] (blj.north) -- ([yshift=1.2cm]blj.north) node [anchor=south west,align=left,xshift=-3.5em] {All honest nodes have processed $b$ or seen an equivocation};
    ...
    \caption{%
        ...
        %
        Consider a block $b$ that is $\confDepth$-deep in the longest processed chain of a node.
        %
        %
        \Iindices $j,k,l$ are \sltcps.
        Since \sltcps stabilize (\cref{lem:cps-stabilize-informal}), the corresponding blocks $b_j, b_k, b_l$ are in all honest nodes' processed longest chains.
        At \sltcp $j$, we know for sure that all honest nodes 
        %
        either processed $b$'s content or saw an equivocation for it, because they have processed $b_j$'s content.
        At \sltcp $k$, we know for sure that if $b$ had an equivocation, preventing processing of its content, then an equivocation proof against $b$ must have entered the chain.
