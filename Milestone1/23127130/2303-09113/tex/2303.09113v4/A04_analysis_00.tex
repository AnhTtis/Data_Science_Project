...
%
%
header propagation delay $\DeltaHeader$ and capacity $\bwtime$, as well as the mining rate $\blkratetime$, are specified in units of \emph{real time}.
%

We denote by $\dC_p(t)$ the longest fully processed chain of an honest node $p$ at the end of \timeslot $t$, and let $\len{b}$ denote the height of a block $b$. We use the same notation $\len{\Chain}$ to denote the length of a chain $\Chain$, define $L_p(t)=\len{\dC_p(t)}$ and $L_{\min}(t) = \min_p L_p(t)$
