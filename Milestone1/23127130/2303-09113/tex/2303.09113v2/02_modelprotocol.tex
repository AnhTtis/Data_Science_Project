...
%
Pseudocode of an idealized Nakamoto consensus protocol $\protocol$ is provided in \cref{alg:generic-lc-protocol}.
...
Pseudocode for instantiations $\FtreePoW$ and $\FtreePoS$ modeling proof-of-work (PoW) and proof-of-stake (PoS) are provided in \cref{alg:hdrtree-pow} and \cref{alg:hdrtree-pos}, respectively.
Helper functions used in the pseudocode are detailed in \fullVersionRef{\cref{sec:algos-reference-helperfunctions}}.
We study these protocols in a unified model for a network $\Env$ with finite bandwidth (\cref{fig:model}), and for the powers and limits of an adversary $\Adv$.



\subsection{Nakamoto's Longest Chain Consensus Protocols}
...
Nodes interact with each other and with the adversary $\Adv$ through an environment $\Env$ that models the network and is detailed in \cref{sec:modelprotocol-modelfeatures,sec:algos-reference-environment}.
The protocol proceeds in \emph{\timeslots} of duration $\slotduration$ (\myalgref{alg:generic-lc-protocol}{loc:generic-lc-protocol-mainloop}).
...
When a node $p$ receives a new valid block header $\Chain$ (\myalgref{alg:generic-lc-protocol}{loc:generic-lc-protocol-receiveheader}), $p$ adds $\Chain$ to its header tree $\hT$, records $\Chain$ as first seen at the current \timeslot, and relays $\Chain$ to all other nodes via $\Env$.
Throughout the execution, the protocol requests from $\Env$ the content 
for
%
block headers 
%
decided by a download priority rule (also called scheduling policy) (\myalgref{alg:generic-lc-protocol}{loc:generic-lc-protocol-downloadrule}).
As a concrete example, we use the \rulelc rule (\cref{alg:longest-header-chain-rule})
in which
a node downloads content for the first block header with unknown content on the longest header chain it has seen.
%
%
Once a valid block's content is received (\myalgref{alg:generic-lc-protocol}{loc:generic-lc-protocol-receivecontent}), the node makes it available to other nodes via $\Env$, and updates its 
%
$\dC$.
...
%
The characteristics of PoW-based block production, \eg, in Bitcoin~\cite{nakamoto_paper,backbone}, are captured by the idealized functionality $\FtreePoW$ (\cref{alg:hdrtree-pow}).
Each block production attempt is committed to a parent block and block content (\myalgref{alg:hdrtree-pow}{loc:hdrtree-pow-binding}), and only a single block is produced when the attempt is successful.
Per \timeslot, each node can make one block production attempt that will be successful with probability $\blkrateslot/N$, independently of other nodes and \timeslots (\myalgref{alg:hdrtree-pow}{loc:hdrtree-pow-blockproductionlottery}).
...
%
PoS NC protocols such as from the Ouroboros~\cite{kiayias2017ouroboros,david2018ouroboros,badertscher2018ouroboros} or Sleepy Consensus~\cite{sleepy,snowwhite} families can be modeled using $\FtreePoS$ (\cref{alg:hdrtree-pos}).
As in PoW, each node can make one block production attempt per \timeslot that will be successful with probability $\blkrateslot/N$, independently of other nodes and \timeslots (\myalgref{alg:hdrtree-pos}{loc:hdrtree-pos-blockproductionlottery})%
...
In PoS, however, (even past) block production opportunities can be `reused' to produce multiple blocks with different parents and/or content, \ie, to equivocate (\myalgref{alg:hdrtree-pos}{loc:hdrtree-pos-leader,loc:hdrtree-pos-binding}).
...
We borrow the bandwidth constrained network model of~\cite{bwlimitedposlc} (\cref{fig:model}).
...
%
\cf~\myalgref{alg:generic-lc-protocol}{loc:generic-lc-protocol-receiveheader}.
...
%
\cf~\myalgref{alg:generic-lc-protocol}{loc:generic-lc-protocol-receivecontent}.
Content download is subject to a per-node bandwidth constraint of~$\bwtime$.
%
See \fullVersionRef{\cref{sec:algos-reference-environment}} for a more formal description of $\Env$.
...
\end{itemize}
A consensus protocol is \emph{secure over time horizon $\Thorizon$ \timeslots with parameter $\Tlive$} iff it satisfies safety, and liveness with parameter $\Tlive$, with overwhelming probability over executions of time horizon $\Thorizon$ \timeslots.
...
\Timeslots in which one or more blocks are produced form a sub-sequence $\{t_k\}$, defined in \cref{sec:analysis-definitions}.
