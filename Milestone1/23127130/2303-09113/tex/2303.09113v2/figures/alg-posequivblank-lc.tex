\begin{algorithm}[tb]%
    \caption{%
        \textcolor{jnSUDigitalGreen}{PoS NC protocol $\protocolPoSEquivBlank$} with download logic
        ...
        \textcolor{jnSUDigitalGreen}{functionality $\FtreePoSEquivBlank$: \cref{alg:posequivblank-hdrtree}})%
    }%
    ...
    \begin{algorithmic}[1]%
        ...
        %
        \LineComment{Global counter of time slots $t \gets 1,2,...$ of duration $\tau$}%
        ...
        \LineComment{Same as in \cref{alg:generic-lc-protocol}: $\Call{init}{\genesisHeaderChain, \mathsf{genesisTxs}}$, $\Call{receivedHeaderChain}{\Chain}$, $\Call{receivedContent}{\Chain,\txs}$}
        ...
        \Throughout
            \LineComment{Choose $\Chain$ from download rule (\eg \cref{alg:longest-header-chain-rule})}
            \If{\textcolor{jnSUDigitalGreen}{$\exists \Chain' \in \hT \colon \Chain' \eqEquivocation \Chain$}}
                ...
                \Comment{\textcolor{jnSUDigitalGreen}{Do not download $\Chain$, query download rule again}}
            \Else
