\begin{algorithm}[tb]%
    \caption{%
        Idealized NC protocol $\protocol$ with download logic
        (helper functions: \fullVersionRef{\cref{sec:algos-reference-helperfunctions}},
        environment $\Env$: \fullVersionRef{\cref{sec:algos-reference-environment}},
        functionality $\Ftree$: \cref{alg:hdrtree-pow} for PoW,
        \cref{alg:hdrtree-pos} for PoS)%
    }%
    ...
    \begin{algorithmic}[1]%
        ...
        \LineComment{Global counter of \timeslots $t \gets 1,2,...$ of duration $\tau$ (for PoW: $\tau \to 0$, \cf~\cref{sec:pow})}
        
        \On{$\Call{init}{\genesisHeaderChain, \mathsf{genesisTxs}}$}
                \label{loc:generic-lc-protocol-init}
            \LineComment{Initialize header tree $\hT$, longest downloaded chain $\dC$, and mappings from block header to content $\TxsMap$}
            %
        ...
        \On{$\Call{receivedHeaderChain}{\Chain}$}
            ...
            \State \Assert{$\Ftree.\Call{verify}{\Chain}$}
        ...
        \On{$\Call{receivedContent}{\Chain, \txs}$}
                ...
                \Comment{Called by $\Env$ or $\Adv$}
            \LineComment{Defer processing the content until all prefixes' contents are downloaded}
            ...
            \LineComment{Update the longest downloaded chain among downloaded chains}
        ...
        \At{\timeslot $t \gets 1,2,...$}
            ...
            \If{$\Chain' \neq \bot$ \textbf{with} $\Chain' \gets \Ftree.\Call{extend}{\dC, \txs}$}
                \State $\Env.\Call{broadcastHeaderChain}{\Chain'}$
                \State $\Env.\Call{uploadContent}{\Chain', \txs}$
            \EndIf
            %
        ...
        \Throughout
            \State{Download content for some $\Chain$ chosen by download rule (\eg \cref{alg:longest-header-chain-rule})}
        \EndThroughout
