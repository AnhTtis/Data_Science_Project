...
%
and the bounded-band\-width model of~\cite{bwlimitedposlc}.
Detailed pseudocode of the protocol is provided in \cref{sec:algos-reference-pseudocode}.
Technical details about the model are provided in \cref{sec:algos-reference-environment}.
%
%
%
%
...
(pseudocode in \cref{alg:generic-lc-protocol})
...
(idealized functionality in \cref{alg:hdrtree-pow})
%
%
in an attempt to extend the \emph{longest downloaded chain} $\dC$ in the node's view with a new block 
of
%
...
block headers 
%
decided by a \emph{download priority rule} (also called \emph{scheduling policy}).
%
As a concrete example, we use the \rulelc rule
(pseudocode in \cref{alg:longest-header-chain-rule})
in which
a node downloads content for the first block header with unknown content on the longest header chain it sees.
%
%
Once a valid block's content is received, 
%
...
\myparagraph{Bandwidth Constrained Network}
%
%
%
%
...
See \cref{sec:algos-reference-environment} for a more formal description of $\Env$.
...
%
This models non-uniform bandwidth (higher than the lower bound $C$)
...
push headers and contents to nodes while bypassing the delay and bandwidth constraints;
break ties in the chain selection and content download policy.
%
The adversary's power to bypass delay and bandwidth constraints allows our model to capture non-uniform constraints across nodes.
%
%
