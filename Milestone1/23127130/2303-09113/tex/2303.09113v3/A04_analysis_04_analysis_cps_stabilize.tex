...
then the length of the longest downloaded chain of honest nodes increases, \ie, a \emph{chain growth event} (made precise in \cref{prop:chain-growth}).
Due to this, since, by \cref{def:cp}, all intervals around a \sltcp contain more \iindices with $\Dat{k}=1$ than those with $\Dat{k}=0$,
there can never be some honest node with a longest downloaded chain that does not contain the block corresponding to the \sltcp (\cref{lem:cps-stabilize}).
This is because there are not enough blocks for any other chain to outnumber the \emph{chain growth events} that contributed to the growth of the downloaded chain containing the \sltcp's block.
Thus, the block corresponding to the \sltcp remains in all honest nodes' longest downloaded chains forever.
%
\Cref{lem:cps-stabilize} is proven
analogously to the combinatorial argument of~\cite{sleepy}.


Recall that $\dC_p(t)$ is the longest downloaded chain of node $p$ at the end of \timeslot $t$, $\len{\Chain}$ denotes the length of chain $\Chain$, $L_p(t) = \len{\dC_p(t)}$ and the length of the ``shortest (across honest nodes) longest downloaded chain'' is $L_{\min}(t) = \min_p L_p(t)$ (where ``$\min_p$'' ranges only over honest nodes).
%
...
The following proposition is helpful for proving \cref{lem:cps-stabilize}.
\begin{proposition}
