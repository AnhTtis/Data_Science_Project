...
a node's longest header chain cannot fork off too much from its longest downloaded chain.
%
...
Thus, if the adversary wants to prevent honest nodes from downloading the block produced at a \sltgood \iindex $k$,
...
and thus apply to several other simple download rules, including the freshest-block rule 
...
%
    ...
    using the \rulelc download rule 
    %
...
Under the \rulelc download rule ,
if $\Dat{k} = 0$,
...
Since honest nodes produce blocks extending their longest chain, $b$ extends the longest downloaded chain of some honest node at \timeslot $t_k - 1$.
...
Due to \cref{lem:cps-stabilize-informal}, any valid header chain longer than $b$ (which is some node's longest downloaded chain) at time slot $t_k$ must contain $b^*$.
...
(\cref{prop:lower-tailbound-X}, see top row in \cref{fig:ppivot-cpivot-intuitive}).
...
%
In \cref{lem:one-cp-induction-base}, through a combinatorial argument, we show that
...
But, given that one \sltcp occurred in $\intvl{(m-1)\Kcp}{m\Kcp}$ (by induction hypothesis), \cref{prop:download-or-spend-budget} ensures that honest nodes will only download blocks that are produced after $(m-1)\Kcp$.
%
...
%
%
\begin{lemma}[\FormalVersion{\cref{lem:many-pps-one-cps}}]
\label{lem:many-pps-one-cps-informal}
    If honest nodes use the \rulelc download rule,
