...
We begin by exploring a strategy that the attacker can adopt which forces honest nodes to waste bandwidth on blocks that do not contribute to chain growth. This strategy demonstrates that the well-studied \emph{private attack} \cite{nakamoto_paper,dem20} is not the worst case behavior of the attacker, and that the previously established security bounds of the bounded-delay model do not hold in the bounded-bandwidth setting. 
...
%
Only after an honest node has downloaded the first adversary block and realizes that the content for the remaining blocks in the announced adversary chain segment are unavailable, does the \rulelc rule switch back to downloading the newly created honest block.
...
We further elaborate on the setup and other simulation details in \Cref{sec:attacks-details}.
...
mainly because 
%
miners have over-provisioned bandwidth. 
%
%
%
%
...
%
the chain growth rate under the \PoSteaserattack is nearly zero (details in \cref{sec:pos-teaser-attack}).
...
%
%
Since SPV miners are immune to congestion (as they do not download block content), how does their presence affect the \teaserattack?
