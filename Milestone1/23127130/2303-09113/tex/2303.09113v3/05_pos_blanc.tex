...
The protocol is identical to what was described in \cref{sec:modelprotocol} and formalized in \cref{alg:generic-lc-protocol}, except for a few key differences.
The block production oracle for proof-of-stake (idealized in \cref{alg:hdrtree-pos}) behaves differently.
...
%
%
Under bounded-bandwidth, however, as observed in~\cite{bwlimitedposlc}, honest nodes may waste their limited bandwidth downloading 
equivocations
%
...
%
%
The key idea in \ProtShort is to modify the download rule of PoS NC 
such that per \BPO at most one block is downloaded.
This restores the one assumption of the bounded-bandwidth PoW NC analysis (\cref{sec:proof-analysis-many-pps-one-cps}~(P1))
...
alternative:
%
defer content downloads until after consensus has been reached on a header chain.
...
\myparagraph{The Download Rule in \ProtShort}
%
\ProtShort uses
%
any download rule that is secure for PoW NC (such as \rulelc),
modified as follows:
%
a node does not download content for a header
...
Instead,
%
the node \emph{pretends} that content was ``downloaded'' and sets it to be empty.
%
%
%
The node can then continue downloading content for headers that extend $\Chain$, and these blocks will be candidates for the node's longest downloaded chain.
%
%
With only the above download rule,
one honest node may download
the real
...
Whenever a node produces a new block header extending its longest downloaded chain,
%
%
it 
...
%
%
all blocks on its longest downloaded chain that 
%
are $\confDepth$-deep,
except it blanks
%
...
The download rule of \ProtShort ensures that,
just like in PoW NC,
honest nodes download at most one block per \BPO.
%
This eliminates additional block download delays caused by equivocations, allowing the honest chain growth rate
...
\cref{sec:proof} (details in \cref{sec:fullproof}) applies to \ProtShort as well, \cref{lem:cps-stabilize-informal} (\sltcps stabilize the longest downloaded chains of all nodes) and \cref{lem:many-pps-one-cps-informal} (\sltcps recur) hold for \ProtShort.
%
...
%
(2) If $b \in \dC_p(t)\trunc{\confDepth}$ and there is no equivocation proof in a block header following it, then node $p$ must have downloaded the content of $b$ before \timeslot $t$.
%
(3) 
%
...
To prove (2), suppose that node $p$ did not download the content of block $b_i$.
Since block $b_j$, and hence also $b_i$, is in every honest node's longest downloaded chain at \timeslot $t_{k+1} - 1$ (\cref{lem:cps-stabilize-informal}), it must have been that $p$ saw an equivocation for $b_i$ before \timeslot $t_{k+1} - 1$ (otherwise it must have actually downloaded the content of $b_i$).
