...
Secondly, to justify only the non-salient branch need to use spectral matching(SM)~\cite{leordeanu2005spectral} in dual-classes patch matching, we compare the following manners of using SM block: 1) Applying SM block in both salient and non-salient branches. 2) Applying SM block in either salient branch or non-salient branch. 3) Applying SM block in neither the salient nor non-salient branches. As shown in Table \ref{tabb3}, using SM only in the non-salient branch provides the most significant performance boost for the model. The following analysis is introduced to explain why SM should be only used in non-salient branch.
