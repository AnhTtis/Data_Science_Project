...
% \author{Talip Uçar$^{\dag}$,  Ehsan Hajiramezanali$^{\ddag}$,  Lindsay Edwards$^{\dag}$ \\
\author{Talip Uçar\\
Centre for AI, BioPharmaceuticals R\&D, AstraZeneca\\
% $\dag$ AstraZeneca, Cambridge, UK \\
% $\ddag$ AstraZeneca, Gaithersburg, US \\
...
%Each node embedding learned in this way captures the shared information across its different views while distinguishing itself from the rest of nodes in the graph
...
%In Section~\ref{ness_principles}, we discuss various properties of subgraphs. 
...
% Referring back to our discussion in Section~\ref{ness_principles}, we investigate various properties of subgraphs. We refer the reader to Figure~\ref{fig:subgraph_framework} as reference for the purpose of our discussion. For the following experiments, we use Cora dataset and five different GNN encoders for three cases, in which we divide the graph into $K=2, 4$ and $8$ subgraphs. \textbf{i)} Figure~\ref{fig:analysis_subgraphs}a shows that the average pearson correlation between the latent variables ($\bm{z_1}, \bm{z_2}, ..., \bm{z_K}$ of subgraphs increases as we divide the graph into $K=2, 4$ and $8$ subgraphs. Please note that each latent variable $\bm{z_K}$ contain all the nodes in the graph. We compute the correlation between every pair of latent variables, and then average them all. This observation is expected since as we divide the graph into more subgraphs, number of connected nodes in each subgraph shrinks, resulting in many unconnected nodes common across all of them and cintributing to the correlation between subgraphs. \textbf{ii)} In the next experiment, we first compute the joint embedding of subgraphs in each of three cases ($K=2, 4$ and $8$). Then, we extract the embeddings of connected nodes in each subgraph (e.g., all the connected nodes in $\bm{g_1}, \bm{g_2}, ...$ etc. in Figure~\ref{fig:subgraph_framework}) and compute the representation for each subgraph by averaging the embedding of all connected nodes in each e.g., for $k^{th}$ subgraph: ${\bm{g}_k}_{ave} = \frac{1}{n_k}\sum_{i=1}^{n_k} \bm{n_i} : \bm{n_i} \in \bm{g}_k,  n_k=|\bm{g}_k| \}$. Finally, we compute the pearson correlation between representation of subgraphs as shown in Figure~\ref{fig:analysis_subgraphs}b. We observe that the correlation between subgraphs goes down as we divide the graph into more subgraphs. This is also expected since as we divide the graph into more sparsed subgraphs with non-overlapping edges, the mixing and correlation between the nodes will reduce. This is a desired property since as the subgraphs become more uncorrelated, the ensemble of computed similarity scores will result in better performance as discussed in Section~\ref{ness_principles}. \textbf{iii)} To further investigate the mutual independence of subgraphs, we measure the consensus of link predictions obtained using the latent variables of each subgraph. For example, for $K=2$, we get two separate predictions on the test set, one from $\bm{z_1}$ and another from $\bm{z_2}$, each of which is latent variable of a different subgraph. And then we look at the ratio of \textit{common} samples predicted correctly by \textit{both} subgraphs to compute their consensus. We repeat it for $K=4$ and $K=8$. As shown in Figure~\ref{fig:analysis_subgraphs}c, the consensus goes down as $K$ increases, indicating that the subgraphs become more uncorrelated. \textbf{iv)} Finally, we compute AUC on the test set by using the aggregated embeddings for all three cases in Figure~\ref{fig:analysis_subgraphs}d. It shows that as the subgraphs become more uncorrelated (see figures in (b), and (c)), the performance obtained from the joint embedding increases. 
...
% \midrule
% \textbf{NESS2$_{CL}$}&   & 96.55±1.1  &   &  &  &  &  &  &  &  &  &  &  &  \\
% \textbf{NESS4$_{CL}$}&   & 97.56±0.2  &   &  &  &  &  &  &  &  &  &  &  &  \\
% \textbf{NESS8$_{CL}$}&   & 97.53±0.3 &   &  &  &  &  &  &  &  &  &  &  &  \\
\bottomrule
...
% * indicates that the node attributes are ignored in the learning process.
\label{Tab:ablation}}
...
%we begin by including only the first latent variable, \{$\pmb{z}_1$\}, from the first subgraph and assign it as the embedding of the entire graph, $\pmb{Z}=\pmb{z}_1$. We then compute the AUC score for link prediction on the test set of Cora. Subsequently, we add $\pmb{z}_2$ to the set and aggregate \{$\pmb{z}_1$, $\pmb{z}_2$\} to compute $\pmb{Z}$ for evaluation. This process is repeated until we obtain $\bm Z$ as the aggregate of all eight latent variables \{$\pmb{z}_1$, $\pmb{z}_2$, ..., $\pmb{z}_8$\}. Our experiments demonstrate that as we aggregate more subgraphs, $\pmb{Z}$ becomes more expressive for the link prediction task, as depicted in Figure~\ref{fig:gradual_aggregation_performance}b, resulting in a significant performance boost. 
%Furthermore, we observe that $\pmb{Z}$ obtained by aggregating only the first few subgraphs already attains a good performance at test time, suggesting that our approach would be effective in large graphs with many missing links. 
...
% \textbf{Evaluating Predictive Quality of Node Embeddings from Individual Subgraphs and Their Gradual Aggregation}

% In the NESS8 framework, we assess the effectiveness of node embeddings derived from each of the eight subgraphs by employing them for link prediction, as displayed in Figure~\ref{fig:gradual_aggregation_performance}a. We repeat the experiment ten times with different random seeds, finding that some subgraphs are more informative than others, but the differences are not substantial. Additionally, we incrementally evaluate the quality of the joint embeddings in Figure~\ref{fig:gradual_aggregation_performance}b. As we successively aggregate subgraph latent variables to compute the joint embeddings, $\bm Z$, we anticipate that $\bm Z$ will become increasingly informative. We begin by including only the first latent variable, {$\pmb{z}_1$}, and then progressively add and aggregate variables up to {$\pmb{z}_1$, ..., $\pmb{z}_8$}. Our experiments demonstrate that as more subgraphs are aggregated, $\pmb{Z}$ becomes more expressive for the link prediction task, resulting in a significant performance boost. We also observe that $\pmb{Z}$, obtained by aggregating only the first few subgraphs, already achieves good performance, suggesting that our approach could be effective in large graphs with many missing links. We conduct the same experiment for the DRES2 model, which uses randomly sampled subgraphs during training. Aggregating these embeddings does not lead to substantial performance improvement. Finally, we repeat the experiments for the Texas dataset, observing similar trends in heterophilous graphs, although the impact of aggregating latent vectors is less pronounced. This is consistent with the observation that ego-features are more important in heterophily settings, while higher-order neighborhoods contribute most in homophily.
...
%Although we focus on feature-rich graphs in the experiments, NESS can also be applied to graphs without node features. 
