...
\author{Paige Randall {North}}{Department of Mathematics and Department of Information and Computing Sciences, Utrecht University, Netherlands}{}{}{}%TODO mandatory, please use full name; only 1 author per \author macro; first two parameters are mandatory, other parameters can be empty. Please provide at least the name of the affiliation and the country. The full address is optional. Use additional curly braces to indicate the correct name splitting when the last name consists of multiple name parts.
...
We are then able to generalize the notion of \emph{initial algebra}, taking inspiration from the theory of enriched limits, which is more expressive, and thus we are able to specify more objects than the usual notion of initial algebra.
Because of our passage to the enriched setting, we have more precise control than in the unenriched setting, and we are able to specify more data types than just those which are captured by the theory of W-types. This general theory is presented in \cref{sec:umeas}.

But first, in \cref{section: guided examples}, we begin our paper with an enlightening example which serves as an illustration of the relevance of our enriched theory and as a motivation for the more general setting. Therein, we provide explicit calculations for the case of algebras over the endofunctor $X\mapsto X+1$ on $\Set$.
...
\section{Illustrative example: $\id +1$}\label{section: guided examples}
...
Dually, because $\eN$ is terminal in $\bcoalg$, there is a function $\ind{-} : C \to \eN$ for any coalgebra $(C, \chi_A)$, and we will say that the \emph{index} of a $c \in C$ is $\ind{c}$. Then the elements of $C$ that have index $0$ are those $c$ such that $\chi_C(c) = t$, those that have index $1$ are all those other $c$ such that $\chi_C^2(c)  = t$, etc. For $c \in C$ where $\ind{c} > 0$, we will also often write $c -1$ to denote $\chi_C (c)$ (especially when the coalgebra structure morphism does not have an explicit name).

\begin{notation}
    Besides $\N$, we often consider algebras of the form
$\standalg{n} := (\{0,1,...,n\}, \alpha_\standalg{n})$ for any $n \in \N$. Here, $\alpha_\standalg{n}$ is the algebra structure that $\{0,1,...,n\}$ inherits as the quotient of $\N$ in $\Set$ that identifies all $m \geq n$. In fact, these are quotient algebras of $\N$, in the sense that the algebra homomorphism $\N \to \standalg{n}$ is an epimorphism (see \cref{ex: quotient algebras for id + 1} and preceding \cref{lem: quotient algebras} in the appendix).

Dually, besides $\eN$, we will also consider subcoalgebras of $\eN$, the \emph{subterminal coalgebras}: $\standcoalg{n}$ with underlying subset $\{0,1,...,n\}$ of $\eN$, $\coaN$ with underlying subset $\N$, and $\I$ with underlying subset $\{\infty\}$. These all inherit coalgebra structures from $\eN$ (see \cref{ex: subcoalgebras for id + 1} and the preceding \cref{lem:char-sub} in the appendix).
\end{notation}
...
If $f_c$ is defined for all $c \in \N$, then there is a total homomorphism $f_\infty: A \to B$ (the `diagonal' $f_\infty(x) : = f_x (x)$). Otherwise, we will have defined $f_c$ only for all $c \in \{0, ..., n\}$, and in this case we will say that we have defined an \emph{$n$-partial homomorphism}. We then also sometimes call a total homomorphism $f: A \to B$ an \emph{$\infty$-partial homomorphism}.
\end{construction}

Notice that in the term ``$n$-partial homomorphism'' in the above \cref{con: partial induction}, $n$ takes values in $\eN$, the terminal coalgebra of our endofunctor. So now we make the following definition in which we encode the coalgebra directly. This allows us to be much more general. 
...
\begin{example}
    Suppose that $!_A: \N \to A$ is surjective, so that in particular, every element of $A$ is of the form $0_A$ or $a + 1$.
    ...
    There is a measuring by $\coaN$ if the induction never fails, and again the functions $f_c$ from \cref{con: partial induction} and \cref{definition: partial homomorphism} coincide. Now, note that exhibiting a measuring by $\eN$ amounts to exhibiting a measuring by $\coaN$ together with a total algebra homomorphism $f_\infty$. For such an $A$, then, exhibiting a measuring by $\N$ is equivalent to exhibiting one by $\eN$.
\end{example}

The reader might wonder why \cref{definition: partial homomorphism} does not require $\mathsf{(M2')}$ $f_c(x) = 0_B$ for any $\ind{c} = 0$ and any $x$, but rather only requires $f_c(x) = 0_B$ when $\ind{c} = 0$ and $x$ is either the zero or a successor. In the previous example, when $!_A: \N \to A$ is surjective, every $x \in A$ is either the zero or a successor, so there is no difference between these two requirements. In the following example, we consider an algebra $A$ where this is not the case, and illustrate why we only stipulate $\mathsf{(M2)}$ and not $\mathsf{(M2')}$.
...
Again, suppose that $!_A: \N \to A$ is surjective.
We show in the appendix (\cref{lem:subobject}) that in this case, the universal measuring is a subcoalgebra of $\eN$. The subcoalgebras of $\eN$ are of the form $\emptyset, \standcoalg{n}, \coaN, \I ,$ and $ \eN$. We have shown that if the induction of \cref{con: partial induction} creates an $n$-partial homomorphism, then the maximum subcoalgebra of $\eN$ that measures $A$ to $B$ is $\standcoalg{n}$, so this is the universal measuring. And if the induction of \cref{con: partial induction} creates an total homomorphism, then the maximum subcoalgebra of $\eN$ that measures $A$ to $B$ is $\eN$ itself, so this is the universal measuring.
We will also show this fact more directly in \cref{ssec: The convolution algebra} below.
\end{example}
...
By \cref{theorem: equalizer formula for universal measuring}, we will see that $\ubalg(A,B)$ always exists (for this and other endofunctors of interest). The coalgebra $\ubalg(A,B)$ will constitute the hom-coalgebra from $A$ to $B$ of our enriched category of algebras (\cref{thm:enriched}).
...
Indeed $D\times C$ has a coalgebra structure where $\ind{(d,c)} = \min(\ind{d},\ind{c})$ for $(d,c)\in D\times C$ and
\[
(d,c)-1= \begin{cases}
    (d-1, c-1) & \text{if }\ind{(d,c)}>0;\\
    t &  \text{if }\ind{(d,c)}=0.
\end{cases}
\]
...
We will show in full generality in \cref{thm:enriched} that this constitutes an enrichment of $\balg$ in $(\bcoalg, \otimes, \I)$, i.e., that all the axioms for an enriched category are satisfied by this choice of composition and identities.
 
\subsection{The convolution algebra}\label{ssec: The convolution algebra}
We now give an alternative representation of $\phom_C(A,B)$ that can be directly defined and computed. In this section, we will be able to use it to compute $\ubalg(A,B)$ without appealing to \cref{lem:subobject}.
...
Therefore, we find the following string of bijections
\begin{align}\label{eq:univ prop of conv alg}
    \phom_C(A,B) \cong \bcoalg(C, \ubalg(A,B)) \cong \balg(A ,  [C,B])
\end{align}
so that we see that $\phom_C(-,B)$ is represented by $[C,B]$. We can even find a representation for $\phom_C(-,B)$, but we will leave this for later (\cref{thm: copower}).

In practice, when we want to compute $\ubalg(A,B)$, we will compute $[C,B]$ and then apply the universal property above. We do that now, computing some of the results of \cref{ex:universal measuring for preinitial} without appealing to \cref{lem:subobject}.

\begin{example}
    \label{ex: compute partial alg hom}
We will now compute $\ubalg(\standalg{n},B)$ using the right-hand bijection in \cref{eq:univ prop of conv alg}.
...
\end{cases}
\] 
Since we are considering $Z:= [C,B]$, we need to understand when $n_{[C,B]} = (n+1)_{[C,B]}$. By definition, $0_{[C,A]}$ is the constant function at $0_A$. Then the successor of $0_{[C,A]}$, i.e. $1_{[C,A]}$, is the function that takes every $c \in C$ of index $0$ to $0_B$, and every other $c \in C$ to $1_B$, so this is $1_{[C,B]}$. Inductively, we can show that $n_{[C,B]}(c) = \min(\llbracket c \rrbracket , n)_{B}$.
...
\begin{example}
    ...
    Since $\balg(A,X) \cong \bcoalg(\I, \ubalg(A,X))$, the initial algebra $\N$ is the (only) $\I$-initial algebra. In fact, since $\ubalg(\N,X) = \eN$ for all $X$ by \cref{lem:subobject}, we find that $\N$ is a $C$-initial algebra for any subterminal coalgebra (i.e., $\emptyset, \standcoalg{n}, \coaN, \eN$).
\end{example}
...
We can immediately calculate (\cref{prop:map from initial alg to dual}) that $\lim^C\id_\ubalg$ is $C^*$,
the dual of $C$ (\cref{ex: dual}). By the bijection above, there is a unique total algebra homomorphism from each $C$-initial object to $\lim^C\id_\ubalg$. This will help us understand the possible structure that a $C$-initial object can have. But first, we must understand the structure of $C^* := [C, \N]$.
...
\begin{definition}\label{definition: lax symmetric monoidal}
 ...
 \end{description}
 such that $(F, \nabla, \eta)$ is associative, unital and commutative, as described in \cref{subsection appendix: diagrams for lax symmetric monoidal functors}.
\end{definition}

\begin{example}
    In \cref{section: guided examples}, we considered the (cartesian closed) symmetric monoidal category $(\Set, \times, 1)$. For the endofunctor $\id + 1$, we define $\nabla_{X,Y}: (X + 1) \times (Y \times 1) \to (X \times Y) + 1$ to take $(x,y) \mapsto (x,y)$, $(t, y) \mapsto t$, and $(x,t) \mapsto t$ for $x \in X, y \in Y, t \in 1$. We define $\eta : 1 \to 1 + 1$ to be the inclusion into the first summand.
\end{example}
...
If $\phi\colon C\otimes A \rightarrow B$ is a measuring, $a\colon (A', \alpha_{A'}) \rightarrow (A, \alpha_A)$ and $b\colon (B, \alpha_{B}) \rightarrow (B', \alpha_{B'})$ are algebra homomorphisms, and $c\colon (C', \chi_{C'})\rightarrow (C, \chi_{C})$ is a coalgebra homomorphism, then one can check that the composition
\[
\begin{tikzcd}
...
\]
We shall see that this functor is representable in each of its variables.
...
\[
\begin{tikzcd}
   \free(C\otimes F(A)) \ar[shift left]{r} \ar[shift right]{r} & \free(C\otimes A) \ar[dashed]{r}{\mathsf{coeq}} & C\triangleright A.
\end{tikzcd}
\]
...
\begin{proposition}
    ...
    \[
        \begin{tikzcd}[row sep=0]
          & F(C) \ar{r}{F(\widetilde{\phi})} & F(\inthom{\C}{A}{B}) \ar{r}{\widetilde{\nabla}_{A,B}} & \inthom{\C}{F(A)}{F(B)} \ar{dd}{\beta_*}\\
         C \ar{ur}{\chi} \ar{dr}[swap]{\widetilde{\phi}} \\
         & \inthom{\C}{A}{B} \ar{rr}{\alpha^*} & & \inthom{\C}{F(A)}{B}
        \end{tikzcd}
       \]

       We shall also refer to the pair $(C; \widetilde{\phi})$ as a measuring.
\end{proposition}
...
\begin{theorem}
    [{Proof in \cref{subsection appendix: equalizer formula for universal measuring}}]
...
\begin{definition}
    ...
    Suppose that $\C$ is closed.
    Given a coalgebra $(C, \chi_C)$ and an algebra $(A, \alpha_A)$ in $\C$, we define an algebra structure on $\inthom{\C}{C}{A}$, called the \emph{convolution algebra}, which is denoted $[(C, \chi_C), (A, \alpha_A)]$ or simply $[C,A]$, as follows. 
...
\[
\begin{tikzcd}
F(\inthom{\C}{C}{A}) \ar{r}{\widetilde{\nabla}_{C,A}} & {\inthom{\C}{FC}{FA}}\ar{r}{\alpha_A^*{\chi_C}_*} & [10pt] {\inthom{\C}{C}{A}},
    \end{tikzcd}
\]
where $\alpha_A^*{\chi_C}_*(f)=\alpha_A\circ f \circ \chi_C$, for any $f:FC\rightarrow FA$.
...
\]
In other words, a measuring $\phi\colon C\otimes A\rightarrow B$ corresponds to an algebra homomorphism $\phi':A\rightarrow [C,B]$ under the identification $\C(C\otimes A, B)\cong \C(A, \inthom{\C}{C}{B})$. Indeed, notice that $\phi'$ is a homomorphism if and only if the following diagram, adjunct to the one appearing in \cref{definition: measuring coalgebra}, commutes.
 \[
     \begin{tikzcd}[row sep=0]
       & F([C,B]) \ar{r}{\widetilde{\nabla}_{C,B}} & {[F(C), F(B)]} \ar{dd}{\alpha_A^*{\chi_C}_*}\\
      F(A) \ar{ur}{F(\phi')} \ar{dr}[swap]{\alpha_A} \\
      & A \ar{r}{\phi'} &  {[C,B]}
     \end{tikzcd}
    \]
...
\begin{definition}
    \label{ex: dual}
    Assuming that $\C$ is locally presentable and $F$ is accessible, $\balg$ has an initial object $N$.

    Let $(-)^*:\bcoalg^\op\rightarrow \balg$ denote the functor $[-,N]$, and call $C^*$ the \emph{dual algebra of $C$} for any coalgebra $C$.

    Let $(-)^\circ:\balg^\op\rightarrow \bcoalg$ denote the functor $\ubalg(A,N)$, and call $A^\circ$ the \emph{dual coalgebra of $A$} for any algebra $A$.
...
One can verify that $(\bcoalg, \otimes, (\I, \eta))$ is a symmetric monoidal category, see details in \cref{subsection appendix: details for symmetric monoidal structure}.
\end{proof}
...
\begin{theorem}
    [{Proof in \cref{subsection appendix: algebras enriched over coalgebras}}]
    \label{thm:enriched}
Suppose that $\C$ is locally presentable and closed and that $F$ is accessible.
...
\begin{example}
    [Details in \cref{ssec: details for big example}]
...
On some occasions, the category of coalgebras of $F$ can be interesting while its category of algebras is vacuous. 
...
\begin{theorem}[{Proof in \cref{subsection appendix: details for mixed enriched GF over F}}]\label{theorem: mixed enriched GF over F}
...
\]
regarded as a enriched composition, for any algebras $A$, $B$ and $T$.
...
\subsection{General $C$-initial objects}

Now we follow \cref{ssec: Generalizing initial objects}. We can use the extra structure in the enriched category of algebras to specify more algebras than we could in the unenriched category of algebras.
...
Given a coalgebra $C$,
    we say an algebra $A$ is a $C$-initial algebra if there exists a unique map $C\rightarrow \ubalg(C,X)$, for all algebras $X$.
...
\begin{proposition}
    [Proof in \cref{details:prop:map from initial alg to dual}]
...
Lastly in \cref{example: further list of examples}, when we considered the constant functor at an object $A$, we must choose a commutative monoid structure on $A$. What if we had two different monoidal structures on $A$? There are other such choices that are needed in \cref{example: further list of examples}. We seek to understand how these choices interact with one another.
...
\subsection{Details for \cref{section: guided examples}}
...
\begin{lemma}\label{lem:subobject}
    ...
    \end{example}
\subsection{Axioms for \cref{definition: lax symmetric monoidal}}\label{subsection appendix: diagrams for lax symmetric monoidal functors}
...
\subsection{Proof of \cref{theorem: equalizer formula for universal measuring}}\label{subsection appendix: equalizer formula for universal measuring}
...
\subsection{Proof of \cref{prop:sym-mon}}\label{subsection appendix: details for symmetric monoidal structure}
...
\subsection{Proof of \cref{thm:enriched}}\label{subsection appendix: algebras enriched over coalgebras}
...
\subsection{Details for \cref{theorem: mixed enriched GF over F}}\label{subsection appendix: details for mixed enriched GF over F}
...
\subsection{Details for \cref{example: further list of examples}}
...
\begin{example}
 ...
 \begin{description}
    ...
    \item[($W$-types)] Suppose that the monoidal structure on $\C$ is not only cartesian (so that we write it as $(\C,\term ,\times)$) but also locally cartesian closed. Consider a morphism $f: A \to C$ in $\C$ and a point $c: * \to C$. We consider the symmetric, cartesian monoidal categories $(\C / A,A ,\times_A)$ and $(\C / C,C ,\times_C)$ where $\C / A$ and $\C / C$ denote the slice categories. The universal properties of the product, pullback, and local exponentiation give $A^* := A \times -: \C \to \C / A$ and $\Pi_f: \C / A \to \C / C$ (where $\Pi_f$ denotes the local exponential along $f$) the structure of a lax symmetric monoidal functor. To see $\Sigma_C:= \dom: \C / C \to \C$ as lax symmetric monoidal, we use the point $c : * \to C$ as $\eta$ and $\nabla$ is given by the universal property of the pullback and product. Then the composition $\Sigma_C \Pi_f A^*: \C \to \C$ is lax symmetric monoidal, and it is accessible since it is the composition of left and right adjoints.
...
\subsection{Details for \cref{prop:map from initial alg to dual}}
...
% Consider a symmetric monoidal category $(\mathcal{C}, \otimes, \I)$.

% \begin{lemma}
%  Let $F:\C \rightarrow \C$ be a lax symmetric monoidal functor. Then $\coalg{\C}{F}$ is a symmetric monoidal category and the forgetful functor $\und{-} : \coalg{\C}{F} \rightarrow \C$ is strong monoidal.
% \end{lemma}

% \begin{notation}
% We denote the forgetful functor from coalgebras as $\und{-} : \coalg{\C}{F} \to \C$, and given a coalgebra $C$, we will denote the coalgebra structure map $\und{C} \to F \und{C}$ by $\chi_C$.

% Similarly, we denote the forgetful functor from algebras as $\und{-} : \alg{\C}{F} \to \C$, and given an algebra $A$, we will denote the algebra structure map $F \und{A} \to \und{A}$ by $\alpha_A$.
% \end{notation}

% \begin{proof}
% Consider $F$-coalgebras $C$ and $D$. We define $C \otimes D$ to be $\und{C} \otimes \und{D}$ together with the following $F$-coalgebra structure map:
% \[
% \begin{tikzcd}
% {\und{C} \otimes \und{D}} \xrightarrow{\chi_C \otimes \chi_D}  {F\und{C} \otimes F\und{D}} \rightarrow {F(\und{C}\otimes \und{D})},
% \end{tikzcd}
% \]
% in which the unlabeled map stems from the lax monoidal structure of $F$. This product is associative.

% The lax symmetric monoidal structure of $F$ provides a map $\I \rightarrow F(\I)$, and we can check that $\mathbb{I}$ is the monoidal unit for $\coalg{\C}{F}$ for the monoidal product described above.
% \end{proof}



% Now we show that $\alg{\C}{F}$ can be enriched in $\coalg{\C}{F}$. To do this, we closely follow the literature on the Sweedler hom, or universal measuring coalgebra. See in particular \cite[3.1]{measuring}.

% \begin{theorem}
% Let $\mathcal{C}$ be a presentably symmetric monoidal closed category. Let $F$ be lax symmetric monoidal and preserve filtered colimits.
% Then $\alg{\C}{F}$ is enriched, tensored, and powered over $\coalg{\C}{F}$.
% \end{theorem}

% \begin{proof}
% First note that $F$ is a lax closed functor (where $\inthom{\C}{-}{-}$ denotes the internal hom) -- that is, there is a map
% \[
% F\left( \inthom{\C}{X}{Y}\right) \xrightarrow{c} \inthom{\C}{FX}{FY}. 
% \]
% Indeed, this is the adjoint of: \PN{adjunct or adjunct?}
% \[
% F(\inthom{\C}{X}{Y})\otimes F(X)\rightarrow F(\inthom{\C}{X}{Y}\otimes X) \rightarrow F(Y),
% \]
% in which the first map is the lax symmetric monoidal structure and the second map is the image of the evaluation functor $\inthom{\C}{X}{Y}\otimes X\rightarrow Y$, the counit of the adjunction.


% The internal hom in $\C$ lifts to a functor
% \[
% \inthom{\C}{-}{-}: \coalg{\C}{F}^\mathsf{op}\times \alg{\C}{F} \longrightarrow \alg{\C}{F}
% \]
% where for a coalgebra $C$ and algebra $A$, we endow $\inthom{\C}{C}{A}$ with the following algebra structure:
% \[
% \begin{tikzcd}
%   F (\inthom{\C}{C}{A}) \ar[r,"c"] & {\inthom{\C}{FC}{FA}} \ar[rr, "{\alpha_A \circ - \circ \chi_C}"] && {\inthom{\C}{C}{A}}.
% \end{tikzcd}
% \]
% Now, for a fixed algebra $A$, we consider the functor
% \[
% \inthom{\C}{-}{A}^\mathsf{op}: \coalg{\C}{F}  \longrightarrow \alg{\C}{F}^\mathsf{op}
% \]

% By ???, $\inthom{\C}{-}{A}^\mathsf{op}$ admits a right adjoint $\inthom{\alg{\C}{F}}{-}{A}$ (note the subscript) -- that is, there is a functor $\inthom{\alg{\C}{F}}{-}{A}$ with the following universal property.
% \[ \hom_{\alg{\C}{F}} (C, \inthom{\C}{B}{A}) \cong \hom_{\coalg{\C}{F}} (B, \inthom{\alg{\C}{F}}{C}{A}) \]

% This assembles into a functor 
% \[ \inthom{\alg{\C}{F}}{-}{-} : \alg{\C}{F}^\mathsf{op} \times \alg{\C}{F} \to \coalg{\C}{F}\]
% which makes $\alg{\C}{F}$ enriched in $\coalg{\C}{F}$.

% The functor $\hom_{\alg{\C}{F}} (-,-)$, endowed with the algebra structure exhibited above, then provides the power.

% By \cite[3.1]{measuring}, then $\mathsf{Alg}_F(\mathcal{C})^\mathsf{op}$ is enriched over $\mathsf{CoAlg}_F(\mathcal{C})$ with tensor $[-,-]^\mathsf{op}$, thus $\mathsf{Alg}_F(\mathcal{C})$ is enriched over $\mathsf{CoAlg}_F(\mathcal{C})$ with power $[-,-]$.\PN{Shouldn't the power be a functor $\coalg{\C}{F} \times \alg{\C}{F} \to \alg{\C}{F}$?}
% \MP{No, the power should be contravariant in one entry.}
% \end{proof}



% In more details, here is how to construct the enrichment and tensoring. Fix an $F$-algebra $A$, then $[-,A]^\mathsf{op}:\mathsf{CoAlg}_F(\mathcal{C})\rightarrow\mathsf{Alg}_F(\mathcal{C})^\mathsf{op}$ preserves colimits from a presentable category (see \cite{porst}) to an essentially small category . By the adjoint functor theorem, there is a right adjoint $\{-, A\}:\mathsf{Alg}_F(\mathcal{C})^\mathsf{op}\rightarrow \mathsf{CoAlg}_F(\mathcal{C})$, this provides the enrichment:
% \[
% \{-,-\}:\mathsf{Alg}_F(\mathcal{C})^\mathsf{op}\times \mathsf{Alg}_F(\mathcal{C}) \longrightarrow \mathsf{Alg}_F(\mathcal{C}).
% \]
% This is called the Sweedler hom and $\{A, B\}$ is the universal measuring $F$-coalgebra from the $F$-algebras $A$ to $B$. In short, for an $F$-coalgebra $C$ and $F$-algebras $A$ and $B$ we obtain:
% \[
% \mathsf{CoAlg}_F(\mathcal{C})(C, \{A, B\})\cong \mathsf{Alg}_F(\mathcal{C})(A, [C,B]).
% \]

% Let $C$ be an $F$-coalgebra. Since $[C,-]:\mathcal{C}\rightarrow \mathcal{C}$ is a right adjoint, it preserves limits and filtered colimits. Since limits and filtered colimits in $\mathsf{Alg}_F(\mathcal{C})$ are computed in $\mathcal{C}$, then $[C,-]:\mathsf{Alg}_F(\mathcal{C})\rightarrow \mathsf{Alg}_F(\mathcal{C})$ preserves limits and filtered colimits. It is between presentable categories, thus it has a left adjoint $C\triangleright -: \mathsf{Alg}_F(\mathcal{C})\rightarrow \mathsf{Alg}_F(\mathcal{C})$. The construction is natural in $C$ thus it defines a functor:
% \[
% -\triangleright -: \mathsf{CoAlg}_F(\mathcal{C})\times \mathsf{Alg}_F(\mathcal{C})\longrightarrow \mathsf{Alg}_F(\mathcal{C})
% \]
% called the Sweedler tensor. In short, for an $F$-coalgebra $C$ and $F$-algebras $A$ and $B$ we obtain:
% \[
% \mathsf{Alg}_F(\mathcal{C})(C\triangleright A, B) \cong \mathsf{Alg}_F(\mathcal{C})(A, [C,B]).
% \]

% In fact, since the tensor product preserves filtered colimits in $\mathcal{C}$ and thus in $\mathsf{CoAlg}_F(\mathcal{C})$, then $\mathsf{CoAlg}_F(\mathcal{C})$ is endowed with a internal hom $\underline{\mathsf{CoAlg}_F(\mathcal{C})}(-,-)$, and our theorem says that for any $F$-coalgebra $C$, and $F$-algebras $A$ and $B$, we obtain natural $F$-coalgebra isomorphisms:
% \[
% \underline{\mathsf{CoAlg}_F(\mathcal{C})}\Big( C, \{A, B\} \Big) \cong \Big\{ A, [C, B] \Big\} \cong \Big\{ C\triangleright A, B \Big\}.
% \]

% We have a more general situation.

% \begin{theorem}
% Let $\C$ be a presentably symmetric monoidal category. Let $F\colon\C\rightarrow \C$ be a lax symmetric monoidal functor that preserves filtered colimits. 
% Let $G\colon\C\rightarrow \C$ be a $\C$-enriched functor that preserves filtered colimits. 
% Then $\alg{GF}{\C}$ is enriched, tensored and powered over $\coalg{F}{\C}$.
% \end{theorem}

% \section{Calculations}

% In this section, we consider an endofunctor $F$ on a category $\C$. We suppose that $\C$ has a terminal object $\term_\C$. 
%  We suppose that $F$ has an initial algebra $\init_{\alg{\C}{F}}$ and a terminal coalgebra $\term_{\coalg{\C}{F}}$.

% \begin{notation}
%     When the ambient category is clear, we will drop subscripts and denote initial and terminal objects as $\init$ and $\term$, respectively.
% \end{notation}

% Our goal in this section is to compute $\homalg{\C}{F}{X}{Y}$ for various categories $\C$, endofunctors $F: \C \to \C$, and algebras $X, Y$ of $F$. To do this, we use the defining universal property of $\homalg{\C}{F}{X}{Y}:$
% \[ \hom_{\coalg{\C}{F}}\left(Z,\homalg{\C}{F}{X}{Y}\right) \cong \hom_{\alg{\C}{F}}\left(X, [Z,Y] \right).\]
% Thus, we will first compute $[Z,Y]$ and then $\hom_{\alg{\C}{F}}(X, [Z,Y])$ for arbitrary coalgebras $Z$ of $F$.

% \begin{example}
%     We will be examining several endofunctors repeatedly. These are all endofunctors on $\Set$.
%     \begin{enumerate}
%         \item $\id(X) := X$
%         \item $(\id + 1)(X) : = X + 1$
%         \item $(A \times \id) (X) := A \times X$, for a given set $A$
%         \item $(A \times \id + 1) (X) := (A \times X )+ 1$, for a given set $A$
%         \item $(2 \times \id^A) (X) := 2 \times X^A$, for a given set $A$
%     \end{enumerate}
%     Here $1 := \term_{\Set}$ is a set with one element, and $2$ is a set of two elements.\PN{We want to generalize the move from $1 \to 2$ and $3 \to 4$ by thinking of $- + 1$ as another functor $G$ that we are composing with. What do we want to get out of this? What other $G$'s should we consider?}
% \end{example}

% \begin{example}[$\id$]
%     Here, $\init$ has an empty underlying set with trivial algebra structure. The terminal coalgebra $\term$ has underlying set $1$ with trivial algebra structure.
% \end{example}

% \begin{example}[$\id +1$]
%     Here, $\init$ is the natural numbers $\N$ and its algebra structure map $\alpha_\N : \N + \term \to \N$ takes $n \in \N$ to $n + 1 \in \N$ and $\term$ to $\init$ (here and in the following, we will use $\term$ to denote both the singleton set and its only inhabitant).
    
%     When we need to be clear, we will denote the elements of $\N$ as $0_\N$, $1_\N$, $2_\N$, etc. For any algebra $A$, we will denote the image of $n_\N$ under the canonical map $\N \to A$ by $n_A$.

%     The terminal coalgebra $\term$ is the extended natural numbers $\eN$ whose underlying set is $\N \cup \{\infty \}$ where $\chi_{\eN} : \eN \to \eN + 1$
%     takes $\infty \in \eN$ to $\infty$, a finite $n +1 \in \eN$ to $n \in \eN$, and $0$ to the inhabitant of $1$ in the sum $\eN + 1$.
%     Again, when we need to be clear, we will denote the elements of $\eN$ by $0^{\eN}, 1^{\eN}, ...$ and $\infty^{\eN}$.
% \end{example}

% \begin{example}[$A \times \id$]
%     Here the initial algebra is $0$, the empty set, with trivial algebra structure.

%     The terminal coalgebra is the set $A^{\N}$ of functions $\N \to A$, whose elements are called \emph{streams}. The coalgebra morphism $A^{\N} \to A \times A^{\N}$ takes an element $s \in A^{\N}$ to the pair $(s(0), s')$ where $s'(n) := s(n+1)$.
% \end{example}

% \begin{example}[$A \times \id + 1$]
%     Here the initial algebra has underlying set $\List{A} := \sum_{n \in \N} A^n$: the set of lists of finite length with entries from $A$. The algebra structure morphism $A \times \List{A} + 1 \to \List{A}$ takes a pair $(a \in A, \ell \in A^n)$ to the new list $a.\ell \in A^{n+1}$ where $a.\ell(0) := a$ and $a.\ell(m) := \ell(m+1)$, and it takes the inhabitant of $1$ to the list of length $0$.

%     The terminal coalgebra $1$ is the set $\List{A} + A^\N$.
    
%     The coalgebra morphism $\List{A} + A^\N \to A \times (\List{A} + A^\N) + 1$ takes a list $\ell \in A^0$ to the inhabitant of $1$, a list $\ell \in A^{n+1}$ to the pair $(\ell(0), \overline{\ell}) \in A \times A^{n}$ where $\overline{\ell}(m) := \ell(m+1)$, and a stream $s \in A^\N$ to the pair $(s(0), \overline{s}) \in A \times A^{\N}$ where $\overline{s}(m) := s(m+1)$.
% \end{example}

% \begin{example}[$2 \times \id^A$]
%     Here the initial algebra is $0$ with the trivial algebra structure.

%     Coalgebras are known as \emph{deterministic automata}. The terminal coalgebra has underlying set $\P(\List{A})$. The first component $\P(\List{A}) \to 2$ of the coalgebra structure sends a subset $S$ of $\List{A}$ to $1 \in 2$ if it contains the list of length $0$ and sends $S$ to $0 \in 2$ otherwise. The second component $\P(\List{A}) \to \P(\List{A})^A$, equivalently sends a subset $S$ of $\List{A}$ and an element $a \in A$ to the subset $\{ \ell \in \List{A} \ | \ a.\ell \in S \}$.
% \end{example}
...
%     \item $\chi$ is invertible, and
%     \item there is a canonical comparison map $c: \init \to 1$ such that
%     \item $\chi \circ c \circ \alpha = Fc$ .
% \end{enumerate}

% Note that we always have the following algebra morphisms into $[Z,Y]$.
% \[
%      \begin{tikzcd}
%         && {[Z,\init]} \ar[dr,"{[\id{Z},!]}"] \\
%          \init \ar[r,"!" ]& {[\term,\init]} \ar[ur,"{[!, \id{\init} ]}"] \ar[dr,"{[\id{\term}, !]}"'] & & {[Z,Y]} \\
%          && {[\term, Y]} \ar[ur,"{[!,\id{Y}]}"'] 
%      \end{tikzcd}
% \]
% To begin to understand $[Z,Y]$, we thus try to understand some components of this diagram.



% \begin{example}[$\id$]
%     Calculating $[Z,Y]$ is relatively easy: the algebra structure $\alpha_{[Z,Y]}: \hom_\Set(\und{Z},\und{Y}) \to \hom_\Set(\und{Z},\und{Y})$ takes $f$ to $\alpha_Y f  \chi_Z$.
% \end{example}

% \begin{example}[$\id +1$]
%     The algebra structure $\alpha_{[Z,Y]} : \hom_\Set(\und{Z},\und{Y}) + 1 \to \hom_\Set(\und{Z},\und{Y})$ takes $f \in \hom_\Set(\und{Z},\und{Y})$ to $\alpha_Y (f + 1)  \chi_Z$ and the inhabitant of $1$ to the constant function at $0_Y$.

%     Consider the algebra $[\eN,\N]$. The elements in the image of the canonical map $\N \to [\eN,\N]$ can be described as:
%     \[ n_{[\eN,\N]} (m^{\eN}) = \min(n,m).\]
%     Note that though a canonical map $\N \xrightarrow{!} [X,\N]$ always factors through $[\eN,\N]$, it is not the case that $\N$ is isomorphic to $[\eN,\N]$. Indeed, the underlying set of $\N$ is countable, but $[\eN,\N]$ is uncountable.

%     Now consider an algebra $[X, \N]$ for a coalgebra $(X,\chi: X \to X + *)$. The canonical map $X \to \eN$ sends an element $x \in X$ to $(\min\{ m \ | \ \chi^{m +1} x = * \})^{\eN}$ (where we let $\min(\init) := \infty$ and where we abuse notation by letting $*$ denote not just the singleton set but also its sole inhabitant). We can then describe the elements in the image of the canonical map $\N \to [X,\N]$, which factors as $\N \to [\eN, \N] \to [X, \N]$. We have that
%     \[ n_{[X, \N]} = \min(\{n\} \cup \{ m \ | \ \chi^{m +1} x = * \})\]
% \end{example}

% \begin{example}[$A \times \id$]
%     The algebra structure $\alpha_{[Z,Y]} : A \times \hom_\Set(\und{Z},\und{Y}) \to \hom_\Set(\und{Z},\und{Y})$ takes a pair $(a,f)$ to $(A \times f)\chi_Z $...
% \end{example}

% \begin{example}[$F_{A \times \id + *}$]
%     Here $\init$ has underlying set $\List{A} := \sum_{n \in \N} A^n$: the set of lists of finite length with entries from $A$. The algebra structure morphism $A \times \List{A} + * \to \List{A}$ takes a pair $(a \in A, \ell \in A^n)$ to the new list $a.\ell \in A^{n+1}$ where $a.\ell(0) := a$ and $a.\ell(m) := \ell(m+1)$ and takes $*$ to the list of length $0$.

%     The terminal coalgebra $1$ is the set $\List{A} + A^\N$.
    
%     The coalgebra morphism $\List{A} + A^\N \to A \times (\List{A} + A^\N) + *$ takes a list $\ell \in A^0$ to $*$, a list $\ell \in A^{n+1}$ to the pair $(\ell(0), \overline{\ell}) \in A \times A^{n}$ where $\overline{\ell}(m) := \ell(m+1)$, and a stream $s \in A^\N$ to the pair $(s(0), \overline{s}) \in A \times A^{\N}$ where $\overline{s}(m) := s(m+1)$.
% \end{example}
...
% \end{example}
    ...
    % Now we need to understand subobjects of $1$. To do this, we are going to make an assumption akin to well-pointedness, since whenever a category is well-pointed, a subobject of an object $X$ is just an object $S$ such that the points of $S$ form a subset of the points of $X$. We first note that any point of the underlying object of a coalgebra has an \emph{index} in the following sense.

    % \begin{definition}
    % Let $*$ denote the terminal object of $\C$. An \emph{index} is a point $* \to 1$, and given a coalgebra $(C, \chi_C)$ and point $c : * \to C$, the \emph{index of $c$}, denoted $\ind(c)$, is the composition $* \xrightarrow{c} C \xrightarrow{!} 1$.
    % \end{definition}

    % \begin{example}[$F_\init$]
    %     For every coalgebra $(C, \chi_C)$ and element $c \in C$, we have$\ind(c) = *$.
    % \end{example}

    % \begin{example}[$F_{\List{A}}$]
    %     For a coalgebra $(C, \chi_C)$ and element $c \in C$, $\ind(c)$ is a stream in $A$. Let $\tr$ denote $\pi_C \chi_C : C \to C$, and let $\o$ denote $\pi_A \chi_C: C \to A$. Then $\ind(c)$ is the stream $\N \to A$ that takes $n$ to $\o \ \tr^n c$.
    % \end{example}

    % \begin{example}[$F_\N$]
    %     For a coalgebra $(C, \chi_C)$ and element $c \in C$, $\ind(c)$ is an element of $\eN$. It is $\min \{ n \ | \ \chi_C^{n+1}(c) = * \}$.
    % \end{example}

    % We have been discussing points of the underlying object of a coalgebra, that is, morphisms of the form $* \to C$ in $\C$. We would like to be able to lift this to a morphism in $\coalg{\C}{F}$. Thus, we introduce the notion of a coalgebra \emph{freely generated by a point of index $n$}.
    ...
    %     Using the first part of \cref{lem:char-sub}, $\eN$ has as subcoalgebras all those subsets which are closed under the coalgebra morphism, which we denote as follows. For $n = -1, 0, ..., \infty$, let $\eNs{n}$ denote the subcoalgebra of $\eN$ with underlying set $\{0, 1, ..., n \}$. Let $\eN_{< \infty}$ denote the subcoalgebra of $\eN$ with underlying set $\N$.

    %     We want to characterize the subcoalgebras of $\eN$ using the second part of \cref{lem:char-sub} as exactly these, so we show that $F$ is well-equipped.
        
    %     For finite $n$, we let $I_n$ be $\eNs{n}$, and and we let $I_\infty$ be the coalgebra with underlying set $\N$ and where the coalgebra structure map $\chi_\N: \N \to \N + *$ takes $n$ to $n+1$. (Note that this has a different coalgebra structure than $\eN_{<\infty}$: the coalgebra structure on $I_\infty$ is `increasing' and the coalgebra structure on $\eN_{<\infty}$ is `decreasing'.) For finite $n$, we let $\top_n$ be $n$, and we let $\top_\infty$ be $0$. These $I_n$'s are universal.
    % \end{example}

    

    % \begin{example}
    %     When $\C := \Set$ and $FX := X + *$, as above, $\init$ is $\N$. Since the forgetful functor preserves pushouts, the quotient algebras are those quotients of $\N$ which are `compatible' with the algebra structure $\chi_\N$. That is, if $n$ and $m$ are identified, then $n + 1$ and $m+1$ are also identified. Thus, if $m = n + \ell$, we find for all $x \geq n$, that $x$ is identified with $x + \ell$. 
    % \end{example}
