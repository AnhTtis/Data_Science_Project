...
%%% abstract %%%
\begin{abstract}
Heat conduction equation on a lattice composed of nodes and bonds is formulated 
...
We show that the temperature measurement at a conner node can detect the corner state
...
non-equilibrium systems such as photonic crystals 
...
Recently, heat conduction on a one-dimensional lattice 
...
Such effect have been experimentally 
observed  in Ref. \cite{Hu:2022aa}, indeed. 
...
typical model of the HOTI \cite{Ezawa:2018aa}.
In Sec. \ref{s:single_node}, we show that the measurement of the temperature only at single corner is enough to detect 
...
In Appendix \ref{s:app}, we argue the relationship between the lattice heat equation and the continuum heat equation, using the one-dimensional model
...
$\rho_{\bm i}$ denotes some source term of heat.
In this paper, we consider effects of the thermal radiation obeying the Stefan-Boltzmann law, 
$\rho_{\bm i}=\varepsilon\sigma S_{\bm i}(T_0^4- T_{\bm i}^4)$, where $T_0$ is the temperature of the heat bath,
...
that of the {\it bonds}, whereas the heat capacity  is
$c_{\bm i}\rho_{\bm i}$
%, and the heat capacity 
that of the {\it nodes}.
...
We exemplify this fact in the Appendix \ref{s:app}, comparing the heat equation in one dimension 
...
\end{alignat}
where $\delta T+T_0$ is the initial highest temperature and $\phi_0=T_0/\delta T$.
...
$\bm\phi_{\rm i}$ which is also the same vector notation as introduced above whose components are $\phi_{\bm i}(0)$, 
%\equiv (\phi_{\bm i_1}(0),\phi_{\bm i_2}(0),\cdots)$ 
is simply obtained as
\begin{alignat}1
\phi_{\bm i}(t)&=e^{-{\cal H}t}\phi_{\bm i}(0)
%\nonumber\\&
=\sum_ne^{-\varepsilon_nt}\phi_{\bm in}\bm\phi_{n}^T\bm \phi_{\rm i },
\label{TimEvo}%---
\end{alignat}
where $\bm\phi_{n}^T\bm \phi_{\rm i }=\sum_{\bm j}\phi_{\bm j n}\phi_{\bm j}(0)$.
...
The breathing kagome lattice is one of well-known examples of the HOTI \cite{Ezawa:2018aa}. 
...
we have define the unit cell composed by three nodes specified by $a=1,2,3$.
...
denote the conventional thermal diffusivities for the continuum system, and
...
and define a typical time scale
\begin{alignat}1
...
numbers 1,2, and 3 are the bulk bands, and the band with no number is due to the edge-states.
...
let us consider the system of triangle shape surrounded by heat bath, including $N$ nodes.
For example, $N$ is given by $N=55\times3=165$ for the triangle with ten unit cells per side. 
Figure \ref{f:full} shows the spectrum for $\delta=0.3$ in the HOTI phase. 
We can see  two dispersive bulk bands and a flat band, as well as 
threefold degenerate corner states and edge states in between the gap of dispersive bands 1 and 2.
We shown in Fig. \ref{f:corner}, the spectrum  of $\cal H$ as a function of $\delta$.
Since this model belongs to HOTI phase for $0<\delta <1/2$, it shows the corner states in this range
\cite{Ezawa:2018aa}. 
...
Hamiltonians. A and B include 18 and 12 nodes, respectively, in Sec. \ref{s:effective}.
}
\label{f:ini}%-----------------------------------------------
\end{center}
\end{figure}

\subsection{Towards the detection of the corner states}
...
Next, we discuss improved way to determine the corner states by using data of temperature of several nodes around a corner
...
The results is shown in Fig. \ref{f:ces} (a), in which one can see clear distinction between the topological phase (solid-curve) 
...
is achieved if one measures the temperature at more than one nodes \cite{Yoshida:2021vt}.  
...
\label{Ort}%---
\end{alignat}
Since the local temperature $\phi_{\bm i}(t)$ depends on the initial distribution $\bm \phi_p$, let us denotes
...
fewer initial conditions and fewer nodes temperature measurements are sufficient, since each corner state is localized
around each corner. 
...
We keep the temperature of all the notes $\phi_{\bm j}=0$
...
the region A and B include only 18 and 12 nodes, each of which is given the initial temperature $1$. 
Then, the measurements of the local temperature of each nodes in region A and B at time $t$ allow us to calculate
...
In Fig. \ref{f:eff}, we plot the spectra of the reduce Hamiltonian for the restricted nodes in A and B regions in Fig. \ref{f:ini}.
...
give rise to nonlinear effect term  in Eq. (\ref{GenDifEqu}). We solve such a nonlinear equation numerically, and apply the 
...
Such a description has indeed intimate relationship with the continuum heat conduction equation, as examined 
in Appendix \ref{s:app} using one-dimensional system with the SSH-like structure.
...
\acknowledgements
This work was supported in part by Grants-in-Aid for Scientific Research Number 
22K03448, JP21K13850, and JP22H05247
...
\end{tabular}
\caption{
Schematic illustration of the 1D system. (a) Experimental implementation in Ref. \cite{Hu:2022aa}.
...
The SSH-like structure for the heat conduction \cite{Yoshida:2021vt} has been realized in experimental implementation (a) in Fig. \ref{f:1d_lat} \cite{Hu:2022aa},
...
heat conductivity.
Instead of solving the continuum diffusion equation, we discretized (b) in a fine meshes like (c) in Fig. \ref{f:1d_lat}.
...
We here assume that $d_0\ll d$, since  the heat capacity of the nodes are very large, as mentioned above.
...
Of course, if one wants to fine-tune the energies, one should regard $\tau'$ as parameter, or
