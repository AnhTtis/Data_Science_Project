...
\subsection{SIFT for Convolutional Layers \label{app:sift_conv}} In this
section, we detail the straightforward extension of SIFT families for
...
\subsubsection{SIFT for Depthwise Convolution Layers}
...
\paragraph{Other SIFT Transformations} The Sparse Wide transformation generally
changes a layer's input and output channels, subsequently scaling the following
layers in a CNN. However, the other SIFT transforms (Sparse Parallel, Sparse
Factorized, and Sparse Doped) do not modify a convolution layer's input or
output channels (as seen in Figure~\ref{fig:diff_members_of_sift}). This allows
for fine-grained control of what layers to apply the SIFT transformations. Since
depthwise convolutions are an extreme form of structured sparsity, where some
filters interact with only specific input channels, we opt not to sparsify them
when using the other SIFT transformations and leave the layer unchanged while
still maintaining FLOPs equivalent to the dense baseline. Note that the
different convolution layers surrounding the depthwise convolution are still
transformed with SIFT to increase their representational capacity.
...
training with and without non-linearity by training configurations of the SIFT
Sparse Parallel, Factorized, and Doped families at different levels of sparsity.
The results without non-linear activation functions are often worse than the
dense accuracy (77\%) across all SIFT family transformations. We omit Sparse
...
across different SIFT family transformations with ResNet18 on CIFAR100 across
different values of sparsity (columns). Non-linear activations enhance the
representational capacity of SIFT, leading to higher accuracy. All reported
results are the average over 3 random seeds.} \vskip 2pt
    \centering
    \begin{small}
    \begin{sc}
...
% \begin{table}[h] \caption{Evaluation on the importance of utilizing the
%     non-linear activation across different SIFT family transformations with
%     ResNet18 on CIFAR100 across different values of sparsity (columns).
%     Non-linear activations enhance the representational capacity of SIFT,
%     leading to higher accuracy.} \vskip 2pt \centering \begin{small}
%     \begin{sc} \begin{tabular}{c|cccc} \toprule Transformation & Non-linear
%     activation & 0.50  & 0.75 & 0.90  \\
%   \midrule \multirow{2}{*}{Sparse Factorized} & \xmark    &  75.9 & 76.6 &
%     76.5  \\
%        &  \cmark

% &  \textbf{77.8} &  \textbf{78.4} & \textbf{78.9} \\
%        \midrule \multirow{2}{*}{Sparse Parallel} & \xmark                   &
%        77.1 &  77.2 & 77.6  \\
%        &  \cmark                &  \textbf{77.9} &  \textbf{79.1} &
%        \textbf{78.2} \\
%        \midrule \multirow{2}{*}{Sparse Doped} & \xmark                      &
%        77.3 &  77.1 & 76.5  \\
%        &  \cmark                &  \textbf{78.2} &  \textbf{77.8} &
%        \textbf{76.9} \\
%         \bottomrule \end{tabular} \end{sc} \end{small}
%     \label{tab:non_linearity_importance} \end{table}
...
\paragraph{Sparsity Setup} For enabling the SIFT transformations, we use the
...
\subsubsection{SIFT on Efficient Computer Vision
...
we enable Sparse Wide and Sparse Parallel SIFT transformations at 50\% and 75\%
sparsity values (see Table~\ref{tab:mbv2-cifar-app}).

\begin{table}[!ht]
    \caption{Evaluation of Sparse Wide and Sparse Parallel SIFT families with
various compute efficient architectures on CIFAR-100 across different values of
sparsity (columns). Using SIFT, all architectures outperform the dense baseline
by a significant margin.} \vskip 2pt
    \centering
    \begin{small}
    \begin{sc}
...
\subsubsection{Evaluation of SIFT with Structured Sparsity}
...
\begin{table*}[]
    \caption{Comparison of structured sparse and unstructured sparse methods on CIFAR-100 test accuracy on ResNet-18.}
    \vskip 2pt
    \centering
    \begin{small}
    \begin{sc}
...
\citep{evci2020rigging}\end{tabular} } & 50\%    &         &    79.0       & &
\\
        &                                       & 75\%   &         &           &
        79.5     &           \\
        &                                       & 90\%    &         & &        &
...
% \begin{table*}[h] \caption{ Object detection results on COCO \texttt{minival}
%     in the RetinaNet framework. SIFT (Sparse Wide) configurations of RetinaNet
%     outperform the dense baseline by a large margin on all metrics while using
%     similar FLOPs. We report GFLOPs per sample using the training image
%     resolution (800$\times$1333). } \vskip 10pt \centering \sc
%     \begin{tabular}{l|rr|rrrrrrrr} \toprule Backbone   &  Params &    FLOPs &
%     AP   & AP$_{50}$ & AP$_{75}$ & AP$_{S}$ & AP$_{M}$ & AP$_{L}$ \\ \midrule
%     Dense           &  21.25M & 197.36G & 29.3 & 46.2 & 30.9 & 14.7 & 31.5 &
%     39.6 \\
%         SW (50\%) &  21.70M & 198.57G & 31.3 & 49.0 & 33.0 & 16.6 & 34.0 &
%         42.0
%         \\
%         SW (75\%) &  22.66M & 201.32G & 32.8 & 51.0 & 34.8 & 17.3 & 35.8 & 43.3
%         \\
%         SW (90\%) &  24.40M & 206.48G & 34.5 & 53.5 & 36.5 & 18.6 & 37.6 & 45.3
%         \\
%         \bottomrule
%     \end{tabular}
...
\begin{table*}[h]
    \caption{ Object detection results on COCO \texttt{minival} in the RetinaNet
        framework. SIFT (Sparse Wide) configurations of RetinaNet outperform the
...
% \paragraph{FLOPs and Parameters} While we ensure that our backbones are
% sparse, using the Sparse Wide (SW) transformation widens the outputs channels,
% which leads to a small increase in parameters (and equivalently, FLOPs) in the
% FPN neck. We give a breakdown of the MACs for the RetinaNet framework in
% Table~\ref{tab:app-cocoflops} for the different models shown in
% Table~\ref{tab:app-cocoretinanet}.

% \begin{table*}[h] \caption{ A detailed breakdown of GFLOPs per sample (during
%     forward pass), reported using the training image resolution of 800
%     $\times$ 1333. The additional FLOPs in the backbone come from the widened
%     BatchNorm and ReLU layers. } \centering \vskip 10pt \sc
%     \begin{tabular}{l|cccc} \toprule \multirow{2}{*}{SIFT Family} &
%     \multicolumn{4}{c}{GFLOPs} \\
%         & Total & Backbone & FPN Neck & BBox Head \\ \midrule Dense          &
%         197.36 & 38.904 & 14.286 & 144.175 \\
%         SW (50\%)      &  198.57 & 39.589 & 14.806 & 144.175 \\
%         SW (75\%)      &  201.32 & 41.577 & 15.568 & 144.175 \\
%         SW (90\%)      &  206.48 & 45.255 & 17.050 & 144.175 \\
%         \bottomrule \end{tabular} \label{tab:app-cocoflops} \end{table*}
...
\begin{table*}[h]
    \caption{ Semantic segmentation results on the Cityscapes \texttt{val} set
        using DeepLabV3+. SIFT (Sparse Wide) transformed ResNet18 backbones
...
% \begin{table*}[h] \caption{ Semantic segmentation results on the Cityscapes
%     \texttt{val} set using DeepLabV3+. SIFT (Sparse Wide) transformed ResNet18
%     backbones outperform the dense baseline on all metrics while using similar
%     FLOPs. We report GFLOPs per sample using the training image resolution
%     (512$\times$1024) } \centering \vskip 10pt \sc \begin{tabular}{l|rr|rr}
%     \toprule Backbone &  Params & FLOPs &  mIoU & mAcc \\ \midrule Dense &
%     12.43M &  105.96G & 76.72 & 84.40 \\
%         SW (50\%)      &  12.51M &  106.36G & 77.90 & 85.12 \\
%         SW (75\%)      &  12.92M &  109.55G & 78.92 & 85.68 \\
%         SW (90\%)      &  13.44M &  113.34G & 79.10 & 86.01 \\
%         \bottomrule \end{tabular} \label{tab:app-cityscapes} \end{table*}

% \paragraph{FLOPs and Parameters} While we ensure that our backbones are
% sparse, using the Sparse Wide (SW) transformation widens the outputs channels,
% which leads to a small increase in parameters (and equivalently, FLOPs) in the
% Decoder. We give a breakdown of the MACs for the DeepLab3+ framework in
% Table~\ref{tab:app-cityscapesflops} for the different models shown in
% Table~\ref{tab:cityscapes}.

% \begin{table*}[h] \caption{ A detailed breakdown of GFLOPs per sample (during
%     forward pass), reported using the training image resolution of 512
%     $\times$ 1024. The additional FLOPs in the backbone come from the widened
%     BatchNorm and ReLU layers. } \centering \vskip 10pt \sc
%     \begin{tabular}{l|cccc} \toprule \multirow{2}{*}{SIFT Family} &
%     \multicolumn{3}{c}{GFLOPs} \\
%         & Total & Encoder (Backbone) & Decoder  \\ \midrule Dense          &
%         105.96 & 96.311 & 9.649 \\
%         SW (50\%)      &  106.36 & 95.776 & 10.584 \\
%         SW (75\%)      &  109.55 & 97.598 & 11.952 \\
%         SW (90\%)      &  113.34 & 98.726 & 14.614 \\
%         \bottomrule \end{tabular} \label{tab:app-cityscapesflops} \end{table*}
...
architecture configurations for the original dense model and its Sparse Wide
50\% and 75\% SIFT variants.

\begin{table*}[!ht]
    \caption{ Sizes and architecture definitions of the dense GPT-3 Small model
        and its SIFT Sparse Wide variants. } \vskip 2pt
    \centering
    \begin{small}
    \begin{sc}
...
Table~\ref{tab:gpt3_scratch_flops}, the SIFT Sparse Wide GPT-3 Small at 50\%
...
Small model. Note, out of all the SIFT transformations, this increase only
occurs in the Sparse Wide transformation.
...
\subsection{Details for Sparse Pre-training and Dense Fine-tuning~\citep{thangarasa2023spdf}}
\label{app:spdf}

We provide an extended set of results that showcase the added benefit of using
SIFT transformations. Here, we apply the Sparse Pre-training and Dense
Fine-tuning (SPDF) framework introduced by~\citet{thangarasa2023spdf}. In this
setup, all models are pre-trained under a similar FLOP budget. However, during
the fine-tuning stage, SIFT models have extra representational capacity which
can be enabled by allowing the zeroed weights to learn (i.e., dense
fine-tuning). Even though the fine-tuning FLOPs are more than the original dense
model, we leverage SIFT method's extra capacity to obtain accuracy gains on the
downstream task. To ensure a fair baseline, we also compare dense fine-tuning to
sparse fine-tuning (i.e., pre-trained model remains as-is) similar
...
members of the SIFT family. All transformations, except Sparse Parallel, perform
comparably to the dense baseline on SQuAD. Unlike CV architectures, BERT
...
these hyperparameters on SIFT for future work and restrict our current scope to
demonstrating gains without tuning any hyperparameters. Using the Sparse
Parallel transformation with 50\% sparsity leads to a 0.7\% improvement in the
exact match (EM) accuracy over the dense baseline (see
Table~\ref{tab:bert_wikitext_ft}).

% \begin{table}[ht] \caption{ Evaluation of SIFT for pre-training BERT-Small and
%     GPT-3 Small. We report EM (higher is better) and perplexity (lower is
%     better) obtained by fine-tuning BERT and GPT models on SQuAD v1.1 and
%     Wikitext-103, respectively. } \vskip 1pt \centering
%     \resizebox{0.45\columnwidth}{!}{ \begin{small} \begin{sc}
%     \begin{tabular}{ccc|cc} \toprule & Metric & Dense & 0.50 & 0.75 \\
%   \midrule BERT Small  & EM ($\uparrow$)  &  70.6  &  \textbf{71.3} & 70.8  \\
%         \bottomrule \end{tabular} \end{sc} \end{small}
%     }
% \label{tab:bert_wikitext_ft} \vspace{-0.1in} \end{table}


\begin{table}[ht]
    \caption{ Evaluation of SIFT for pre-training BERT Small. We report EM
(higher is better) obtained by sparse fine-tuning and dense fine-tuning BERT
models on SQuADv1.1, respectively. } \vskip 1pt
    \centering
...
Sparse Wide 50\% and 75\% SIFT variants.
...
We finetune the SIFT Sparse Wide variants of GPT-3 Small on the
...
specifically, on the dense baseline and SIFT variants, we use a batch size of 32
and select the best learning rate among \{5e-3, 3e-3, 1e-3, 3e-4, 1e-4, 3e-5,
...
WikiText-103. The GPT-3 Small 75\% SIFT model reduces the perplexity (PPL) by a
noticeable 1.3 points compared to dense (refer to
Table~\ref{tab:gpt_wikitext_ft}).

\begin{table}[ht]
    \caption{ Evaluation of SIFT for pre-training GPT-3 Small. We report
perplexity (lower is better) obtained by sparse fine-tuning and dense
...
for benchmarking SIFT variants. The benchmarking is conducted on G4dn instances
available on the AWS cloud. These instances support the AVX-512 instruction set,
which is used by the DeepSparse inference runtime to accelerate unstructured
sparsity. We report runtime for batch-inference of 64 images at 224 $\times$ 224
resolution.
...
sparsity.
%Note that in smaller models of the GPT family, e.g., GPT-3 Small, FLOPs spent
%in attention represent a significant part of the overall FLOPs. However,
%attention FLOPs (i.e., spent in multi-head attention) scale quadratically with
%sequence length and are invariant to weight sparsity. To demonstrate the
%efficacy of sparse kernels for unstructured weight sparsity, 
...
%since the attention FLOPs represent a small fraction of overall FLOPs in this
%configuration. Note we did not get enough time to benchmark SIFT variants for
%this model, and hence report the projected speed-up based on the speed-up we
%observe for the dense GPT-3 20B model with unstructured sparsity.
...
\begin{itemize}
    ...
    SIFT transformations, helped with the implementation, established
    cardinality of SIFT members to explain the results, solved issues in the
    standard RigL optimizer for multi-head attention, conducted experiments for
    BERT, benchmarked SIFT for inference, and wrote majority of the manuscript.
    ...
    method. He also implemented all SIFT transformations in PyTorch, proposed
    using non-linearity in SIFT, conducted experiments for the entire study on
    CIFAR-100 and its ablations, obtained initial results on ImageNet, extended
    SIFT to efficient architectures (e.g., BotNet, MobileViT), conducted the
    entire study with GPT on Cerebras CS-2, and contributed to writing parts of
    the manuscript.
    
    \item Abhay Gupta validated sparse optimizers in PyTorch, conducted
    experiments with SIFT ResNet variants on ImageNet, obtained results with
    MobileNetV2 architecture, helped with pre-training of SIFT variants of GPT
    on Cerebras CS-2, conducted all experiments of SIFT on downstream CV tasks,
    and contributed to writing parts of the manuscript.
    
    \item Sean helped with the bring-up of sparsity support on Cerebras CS-2
    which was crucial for benchmarking and training SIFT variants of GPT models,
    and provided feedback to improve the structuring and presentation of the
    manuscript.
