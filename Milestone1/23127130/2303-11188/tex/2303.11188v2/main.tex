...
\end{equation*}
Note that $\Expect [\nabla\mathcal{L}(\tilde{\param})] = \nabla g(\param)$, where the randomness stems from the choice of the datapoint and the update rule \ref{rule:1}.
...
\begin{figure} 
    ...
    \else
    \includegraphics[width=0.45\textwidth, trim={0 1cm 0 2cm}]{data/lines4.pdf}
    \fi 
...
\begin{figure}
    \centering
    \ifpreprint
    \includegraphics[width=0.8\textwidth, trim={0 2cm 0 0 }]{data/gurobi_3.pdf}
    \else
    \includegraphics[width=0.45\textwidth, trim={0 2cm 0 0 }]{data/gurobi_3.pdf}
    \fi
