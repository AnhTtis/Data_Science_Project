...
%
\author{
Petra \v Stevuli\'akov\'a\orcidID{0000-0002-7879-1397} \and Petr Hurtik\orcidID{0000-0003-4349-9705}}
%

\authorrunning{P. \v Stevuli\'akov\'a, P. Hurtik}
...
\begin{figure}[!h]
    ...
    \caption{%A very simplified demonstration of how our loss function works. It is an analogy to a child game of throwing rings to a stick. The ring represents the predicted box and the stick represents the loss function. The goal is to place the ring in order that it is centered with respect to the stick. The standard IoU loss behaves as the simple stick; the DIoU loss puts a rubber between the stick and the ring which makes it to move (converge) faster. Our proposed IoU with smoothing is represented by a cone covering the loss space and continuously navigating the ring to the correct position.
    An illustration of the proposed loss given by a child game of throwing rings to a stick. The ring and the stick represent the predicted box and the loss function, respectively. The goal is to place the ring in order to centre it with respect to the stick. The standard IoU loss represents the simple stick, the DIoU loss uses a rubber between the ring and the stick to move (converge) it faster. The proposed IoU with smoothing is represented by a cone covering the loss space and continuously navigating the ring.} 
    \label{fig-intuition}
\end{figure}
...
%In the object detection problem, a bounding box regression (BBR) is typically used to improve the accuracy of object localization. Therefore, a well-defined loss function is important for the success of BBR. 
...
\begin{figure}[!h]
    ...
    \caption{2D visualization of how the smoothing part is constructed. %From left: ground truth; smoothing loss constructed using the ground truth; predicted box; smoothing loss. 
...
%The aim of the smoothing part is to add a non-negative element whose purpose is to navigate the gradient to converge faster, so the motivation is similar to the other variants of IoU-based losses. From the above, it is natural that $$\mathcal{L_S}(B^g, B^p) \geq \mathcal{L}_{IOU}(B^g, B^p)$$ due to non-negative $\mathcal{R_S}(B^g, B^p)$. As we will show later, this non-equality is broken when losses of the same epochs are compared, which proves the faster convergence property.
...
\end{equation}
%\begin{equation}
%    \mathbf{\omega}^R = (\omega^R_x, \omega^R_y) = (1 - (x^R_{bot} + x^R_{top})/2d^c_x, 1 - (y^R_{bot} + y^R_{top})/2d^c_y)
%\end{equation}
\begin{equation}
...
\end{equation}
%\begin{equation}
%$    \mathbf{\omega}^L = (\omega^L_x, \omega^L_y) = (1 - (x^L_{bot} + x^L_{top})/2d^c_x, 1 - (y^L_{bot} + y^L_{top})/2d^c_y).
%\end{equation}
...
\end{equation}
%\begin{equation}
%    l_x = ((1 - d^R_x)*\omega^R_{x} + (1 - d^L_x)*\omega^L_{x})/2,
%\end{equation}

%\begin{equation}
%    l_y = ((1 - d^R_y)*\omega^R_{y} + (1 - d^L_y)*\omega^L_{y})/2.
%\end{equation}
$\square$
...
\begin{enumerate}
    ...
    \item The smoothing part $\mathcal{R_S}(B^g, B^p)$ is always non-negative and therefore, $$\mathcal{L_S}(B^g, B^p) \geq \mathcal{L}_{IOU}(B^g, B^p).$$% This non-equality is broken when losses of the same epochs are compared, which proves the faster convergence property.
\end{enumerate}
