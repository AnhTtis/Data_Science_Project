...
Hengyue Zhang,$^{1,2}$\thanks{E-mail: hengyue.zhang@physics.ox.ac.uk}
Timothy D.~Brandt,$^{1}$
Rocio Kiman,$^{3}$
Alexander Venner,$^{4}$
\newauthor{
...
\citep[e.g.,][]{Bond_2017b}, or the combination of different data types to compensate for limited phase coverage. On the one hand, absolute astrometry from the {\Hipparcos} \citep{ESA_1997} and {\Gaia} \citep{Gaia_2016} missions probe the transverse acceleration of the MS star due to the tug of a WD companion. These powerful astrometric measurements have been complied and calibrated to the same reference frame by the Hipparcos-Gaia Catalog of Accelerations \citep[HGCA,][]{Brandt_2018,Brandt_2021}. On the other hand, spectrographs like AAT-UCLES \citep{Diego_1990}, Keck-HIRES \citep{Vogt_1994}, APF-Levy \citep{Vogt_2014}, and Lick-Hamilton \citep{Fischer_2014} provide high-precision radial velocities (RVs) that probe the MS star's acceleration along the line-of-sight. These measurements, together with relative astrometry of a short orbital arc, enable a direct measurement of WD mass \citep[e.g.,][]{Brandt_2019,Bowler_2021,Zeng_2022}.  

In this paper, we combine absolute astrometry from the HGCA with radial velocities and relative astrometry from various sources to measure orbits and masses of six confirmed and one candidate nearby Sirius-like systems, including the Hyades member HD 27483. For HD 118475 and HD 136138, we compare our results against {\Gaia} DR3 two-body solutions \citep{Gaia_DR3,Holl_2022}. We use the open-source code \textsc{orvara} \citep{Brandt_2021_orvara} to perform the fits and to derive constraints on the WDs' masses and orbits. Then, we infer age constraints on four WDs from their dynamical masses and effective temperatures ($\Teff$). 

We structure the paper as follows. Section \ref{sec:targets} introduces our target selection method and summarizes the properties of the selected targets. In Section \ref{sec:data+obs}, we outline the observational data available for each target and present our new observation for HD 27483 and the corresponding data reduction. We introduce our orbit-fitting method and discuss our results in Section \ref{sec:orbit_fit}. In Section \ref{sec:age}, we perform a Bayesian age analysis on the WDs and compare our results with existing age estimates by other methods. We conclude in Section \ref{sec:conclude}.


\section{Target Selection} \label{sec:targets}

We selected our sample from the list of known Sirius-like systems in \cite{Holberg_2013} and the more recently detected WD + late MS star binaries in the Montreal White Dwarf Database\footnote{https://www.montrealwhitedwarfdatabase.org/home.html} \citep{Dufour_2017}. Of more than 100 such systems, we first selected 63 targets that have astrometric accelerations in the HGCA \citep{Brandt_2018,Brandt_2021}. Then, we excluded 49 targets with neither high-precision RVs nor at least two epochs of relative astrometry. We choose not to redo the dynamical mass analysis for seven systems that already have precise dynamical mass measurements: 40 Eri \citep{Mason_2017}, Procyon \citep{Bond_2015}, Sirius \citep{Bond_2017b}, Stein 2051 \citep{Sahu_2017}, GJ 86 \cite{Brandt_2019}, 12 Psc and HD 159062 \citep{Bowler_2021}. In addition to the selection criteria, we removed HD 149499 as we found that its $\sim 0.45$ $M_{\rm \sun}$ inner companion \citep{Tokovinin_2019} alone explains the host star's astrometric acceleration and RV trend, which leaves almost no information to constrain the outer WD companion (first discovered by \citealt{Jordan_1997}). Furthermore, we added HD 118475 B, an unconfirmed WD detected by RV in \cite{Kane_2019}. We ended up with seven targets, three of which have both high-precision RVs and relative astrometry, two of which only have relative astrometry (HD 27483 and HD 27786), and two of which (HD 118475 and HD 136138) only have RVs. The host star properties are summarized in Table \ref{tab:host_prop}. We collected the spectral types from \cite{Skiff_2014_catalogue} and parallaxes from {\Gaia} DR3 \citep{Gaia_DR3}. We adopted the most recent isochronological mass for each host star except HD~27483~A, for which we used the total dynamical mass of the two F6V components of the spectroscopic binary, measured by \cite{Konacki_2004}. 

%We note that HD 118475 B was hypothesized to be a WD in \cite{Kane_2019} but has never been resolved. We include the companion in this work to test the consistency between its dynamical mass and the WD hypothesis.
...
\begin{table} %[htbp]
    ...
    \begin{tabular}{ccccc} \hline
        ...
        19019 & G0V & $31.979 \pm 0.029$ & $1.06 \pm 0.06$ & 1 \\ \hline
        27483 & F6V+F6V & $21.094 \pm 0.032$ & $2.77 \pm 0.26$ & 2 \\ \hline
        27786 & F4IV-V & $23.79 \pm 0.11$ & $1.54 \pm 0.08$ & 3 \\ \hline
        114174 & G3IV & $37.868 \pm 0.024$ & $0.97 \pm 0.04$ & 4 \\ \hline
        118475 & F9V & $29.537 \pm 0.017$ & $1.16 \pm 0.06$ & 3 \\ \hline
        136138 & G8IIIa & $9.011 \pm 0.051$ & $1.84 \pm 0.40$ & 5 \\ \hline
        169889 & G7V & $28.279 \pm 0.026$ & $0.98 \pm 0.05$ & 3 \\ \hline
    \end{tabular}
    \centering
...
Unpublished observations by HST resolved the system twice in the WFPC2 F170W filter in January 2003 (HST proposal 9334, PI Howard Bond) and December 2003 (HST proposal 9964, PI Howard Bond) and once in the WFC3 F218W filter in February 2012 (HST proposal 12606, PI Martin Barstow). We derived relative astrometry and photometry from these unpublished observations using methods similar to that for the HD 27483 HST observation. The only difference is that the WD companion is too close to the host star that we cannot use the host star PSF as a template. Instead, we adopted the observation of HIP 66578 on 2000 January 18 (HST proposal 8496, PI Stefano Casertano) as the PSF template for WFPC F170W observations and the previously analyzed image of HD 27483 A as the PSF template for the WFC3 F218W observation. We find that adopting a different image as the PSF template results in positional differences of up to 7 mas, comparable to our statistical uncertainties. We add this to our error budget. The results are listed in Table \ref{tab:new_relast}. 
...
Our MCMC analysis improves the orbital parameters and gives unprecedented dynamical mass constraints for all systems except HD 118475 and HD 136138, for which we have imprecise minimum masses due to leaving out {\Hipparcos}-{\Gaia} proper motions. Figure \ref{fig:orbits} shows the best-fitting relative astrometric orbits of the systems. Figures \ref{fig:corner_19019}, \ref{fig:corner_27483}, \ref{fig:corner_27786Ab}, \ref{fig:corner_27786B}, \ref{fig:corner_114174}, \ref{fig:corner_118475}, \ref{fig:corner_136138}, and \ref{fig:corner_169889} are the corner plots of the key orbital parameters. Figures \ref{fig:RV_19019}, \ref{fig:RV_114174}, \ref{fig:RV_118475},\ref{fig:RV_136138}, and \ref{fig:RV_169889} gives the fitted RV orbits and the corresponding residuals. Figures \ref{fig:relAst_19019}, \ref{fig:relAst_27483}, \ref{fig:relAst_27786}, \ref{fig:relAst_114174}, and \ref{fig:relAst_169889} show the fits to relative and absolute astrometry and the corresponding residuals. Tables \ref{tab:post_19019} to \ref{tab:post_169889} list the priors and posteriors of all fitted and derived parameters. We discuss the details of each system below.

\begin{figure*}
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 19019/astrometric_orbit_HD19019.pdf}}
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 27483/astrometric_orbit_HD27483.pdf}} \\
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 27786/astrometric_orbit_HD27786_inflated_err_Ab.pdf}}
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 114174/astrometric_orbit_HD114174_new.pdf}} \\
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 118475/astrometric_orbit_HD118475_RV.pdf}}
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 136138/astrometric_orbit_HD136138.pdf}} 
    \subfloat[]{
    \includegraphics[height=4.4cm,width=0.32\linewidth]{Figures/HD 169889/astrometric_orbit_HD169889_newpri.pdf}} \\
...
The WD companion, HD 27483 B, has a long history of spectroscopic observations. \cite{Boehm-Vitense_1993} derived a mass of $\sim 0.6$ $M_{\sun}$ from the WD's IUE spectrum. \cite{Burleigh_1998} re-analyzed the spectrum and got $M_{\rm WD}\approx 0.94$ $M_{\sun}$. \cite{Joyce_2018} analyzed two HST spectra of the WD and got $0.748 \pm 0.072$ $M_{\sun}$ and $0.711 \pm 0.137$ $M_{\sun}$, respectively.
...
If we assume a standard evolutionary track, such a small dynamical mass implies a $< 1$ $M_{\sun}$ progenitor, which does not have enough time to evolve off the MS within the primary star's age of ${2.11}_{-0.79}^{+1.26}$ Gyr \citep{David_2015}. One plausible explanation is that the WD had a $\sim 2$ $M_{\sun}$ progenitor but underwent faster-than-usual mass loss in its RGB phase due to interactions with the primary star, such as common envelope evolution \citep{Althaus_2001}. This scenario is not uncommon in compact binary systems with periods of days \citep{Kilic_2007,Brown_2011} but is not known to happen for wider WD companions. Future spectroscopic observations of the system may provide evidence for or against such interaction.
...
\subsubsection{HD 169889}
Our dynamical mass of ${0.526}_{-0.037}^{+0.039}$ $M_{\sun}$ is the first precise mass of the WD, consistent with the dynamical mass lower limit of $0.369 \pm 0.010$ $M_{\sun}$ from \cite{Crepp_2018}. The eccentricity of $e={0.896}_{-0.088}^{+0.064}$ is the largest in our sample. A large eccentricity is not surprising because the eccentricity distribution of wide MS binaries with separations smaller than 100 AU is nearly uniform \citep{Hwang_2022}, and the isotropic and adiabatic mass loss of a star during the AGB phase does not modify the eccentricity of its orbit \citep{Dosopoulou_2016}. Our fit to relative separation has a $\chi^2$ of 32 for only five observations, suggesting that the relative astrometry in \cite{Crepp_2018} may have underestimated uncertainties. Inflating the uncertainties to get a reduced $\chi^2$ of 1 does not affect the results significantly. The WD only has two broadband photometry measurements in the $H$-band and the $L'$-band, which are insufficient to precisely measure its $\Teff$, $\logg$, or photometric mass \citep{Crepp_2018}. Additional photometry or spectroscopy will be necessary to characterize the fundamental properties of the WD.
...
Our analysis is subject to potential systematics in our choice of IFMR, IMF, and stellar evolutionary model. We repeat the above procedures with the \cite{Cummings_2018} IFMR, the \cite{Kroupa_2001} IMF, and PARSEC evolutionary tracks \citep{Bressan_2012}, confirming that our conclusions in Section \ref{subsec:age_results} do not change significantly.
...
\begin{enumerate}
    ...
    \item Our three-body fit on HD 27786 results in an unusually low mass ($0.443 \pm 0.012$ $M_{\sun}$) for the WD HD 27786 Ab, suggesting the possibility of common envelope evolution with the primary star during its progenitor's RGB phase.
...
This research has made use of the SIMBAD database \citep{Wenger_2000}, operated at CDS, Strasbourg, France. This research has made use of the SVO Filter Profile Service \citep{SVO_2020} supported by the Spanish MINECO through grant AYA2017-84089. 
...
\section{Figures and tables of fit results}

\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 19019/Corner_HD19019.pdf}
...
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \includegraphics[height=8cm,width=0.43\linewidth]{Figures/HD 19019/RV_orbit_HD19019.pdf}
    }
    \subfloat[]{
    \includegraphics[height=8cm,width=0.53\linewidth]{Figures/HD 19019/RV_OC_HD19019_InstAll.pdf}
    }
...
\captionsetup[subfigure]{labelformat=empty}
    ...
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.44\linewidth]{Figures/HD 19019/relsep_OC_HD19019.pdf}
        }
        \hfill
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.52\linewidth]{Figures/HD 19019/PA_OC_HD19019.pdf}
    } \\
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.985\linewidth]{Figures/HD 19019/Proper_Motions_HD19019.pdf}
    }
...
\begin{table} %[htbp]
    \centering
    \begin{tabular}{c@{\hskip 2.5cm}c@{\hskip 2.5cm}c} \hline \hline
...
\begin{table} %[htbp]
    \centering
    \begin{tabular}{c@{\hskip 3.1cm}c@{\hskip 3.1cm}c} \hline \hline
         Parameter & Prior Distribution & Posteriors $\pm 1\sigma$ \\ \hline
         \multicolumn{3}{c}{Fitted parameters} \\ \hline
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 27483/Corner_HD27483.pdf}
    \caption{Best-fitting orbital parameters for the HD 27483 system. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_27483}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    ...
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.44\linewidth]{Figures/HD 27483/relsep_OC_HD27483.pdf}
        }
        \hfill
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.52\linewidth]{Figures/HD 27483/PA_OC_HD27483.pdf}
    } \\
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.985\linewidth]{Figures/HD 27483/Proper_Motions_HD27483.pdf}
    }
    \caption{Relative (top panel) and absolute (bottom panel) astrometry for the HD 27483 system. Other descriptions are identical to those for Figure \ref{fig:relAst_19019}.}
    \label{fig:relAst_27483}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 27786/Corner_HD27786_inflated_err_Ab.pdf}
    \caption{Best-fitting orbital parameters for HD 27786 Ab. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_27786Ab}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 27786/Corner_HD27786_inflated_err_B.pdf}
    \caption{Best-fitting orbital parameters for HD 27786 B. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_27786B}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \centering      \includegraphics[height=6.7cm,width=0.358\linewidth]{Figures/HD 27786/relsep_OC_HD27786_inflated_err_Ab.pdf}}
    \subfloat[]{
    \centering    \includegraphics[height=6.7cm,width=0.439\linewidth]{Figures/HD 27786/PA_OC_HD27786_inflated_err_Ab.pdf}
    } \\
    \subfloat[]{
    \centering
    \includegraphics[height=6.7cm,width=0.358\linewidth]{Figures/HD 27786/relsep_OC_HD27786_inflated_err_B.pdf}}
    \subfloat[]{
    \centering
    \includegraphics[height=6.7cm,width=0.439\linewidth]{Figures/HD 27786/PA_OC_HD27786_inflated_err_B.pdf}
    } \\
    \subfloat[]{
    \centering
    \includegraphics[height=6.7cm,width=0.797\linewidth]{Figures/HD 27786/Proper_Motions_HD27786_inflated_err_Ab.pdf}
    }
    \caption{Relative (top and middle panels) and absolute (bottom panel) astrometry for the HD 27786 system. Other descriptions are identical to those for Figure \ref{fig:relAst_19019}.}
    \label{fig:relAst_27786}
\end{figure*}
...
\begin{table} %[htbp]
    \centering
    \begin{tabular}{c@{\hskip 1.4cm}c@{\hskip 1.4cm}c@{\hskip 1.4cm}c} \hline \hline
         \multirow{2}{1.4cm}{Parameter} & Prior Distribution &  Posteriors $\pm 1\sigma$ & Posteriors $\pm 1\sigma$ \\ 
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 114174/Corner_HD114174_new.pdf}
    \caption{Best-fitting orbital parameters for the HD 114174 system. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_114174}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \includegraphics[height=8cm,width=0.45\linewidth]{Figures/HD 114174/RV_orbit_HD114174_new.pdf}
    }
    \subfloat[]{
    \includegraphics[height=8cm,width=0.51\linewidth]{Figures/HD 114174/RV_OC_HD114174_new_InstAll.pdf}
    }
    \caption{RV orbits of HD 114174. Other descriptions are identical to those for Figure \ref{fig:RV_19019}.}
    \label{fig:RV_114174}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    ...
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.44\linewidth]{Figures/HD 114174/relsep_OC_HD114174_new.pdf}
        }
        \hfill
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.52\linewidth]{Figures/HD 114174/PA_OC_HD114174_new.pdf}
    } \\
    \subfloat[]{
        \centering
        \includegraphics[height=8cm,width=0.985\linewidth]{Figures/HD 114174/Proper_Motions_HD114174_new.pdf}
    }
    \caption{Relative (top panel) and absolute (bottom panel) astrometry for the HD 114174 system. Other descriptions are identical to those for Figure \ref{fig:relAst_19019}.}
    \label{fig:relAst_114174}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 118475/Corner_HD118475_RV.pdf}
    \caption{Best-fitting orbital parameters for the HD 118475 system. Because fitting only the RVs does not break the degeneracy between secondary mass and inclination, we plot the minimum mass $M_{\rm sec}\sin{i}$ instead of $M$ and $i$ separately. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_118475}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \includegraphics[height=8cm,width=0.45\linewidth]{Figures/HD 118475/RV_orbit_HD118475_RV.pdf}
    }
    \subfloat[]{
    \includegraphics[height=8cm,width=0.51\linewidth]{Figures/HD 118475/RV_OC_HD118475_RV_InstAll.pdf}
    }
    \caption{RV orbits of HD 118475. Other descriptions are identical to those for Figure \ref{fig:RV_19019}.}
    \label{fig:RV_118475}
\end{figure*}
...
\begin{table} %[htbp]
    \centering
    \begin{tabular}{c@{\hskip 1.1cm}c@{\hskip 1.1cm}c@{\hskip 1.1cm}c} \hline \hline
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 136138/Corner_HD136138.pdf}
    \caption{Best-fitting orbital parameters for the HD 136138 system. Because fitting only the RVs does not break the degeneracy between secondary mass and inclination, we plot the minimum mass $M_{\rm sec}\sin{i}$ instead of $M$ and $i$ separately. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_136138}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \includegraphics[height=8cm,width=0.45\linewidth]{Figures/HD 136138/RV_orbit_HD136138.pdf}
    }
    \subfloat[]{
    \includegraphics[height=8cm,width=0.51\linewidth]{Figures/HD 136138/RV_OC_HD136138_InstAll.pdf}
    }
    \caption{RV orbits of HD 136138. Other descriptions are identical to those for Figure \ref{fig:RV_19019}.}
    \label{fig:RV_136138}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/HD 169889/Corner_HD169889_newpri.pdf}
    \caption{Best-fitting orbital parameters for the HD 169889 system. Other descriptions are identical to those for Figure \ref{fig:corner_19019}.}
    \label{fig:corner_169889}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    \centering
    \subfloat[]{
    \includegraphics[height=8cm,width=0.45\linewidth]{Figures/HD 169889/RV_orbit_HD169889_newpri.pdf}
    }
    \subfloat[]{
    \includegraphics[height=8cm,width=0.51\linewidth]{Figures/HD 169889/RV_OC_HD169889_newpri_InstAll.pdf}
    }
    \caption{RV orbits of HD 169889. Other descriptions are identical to those for Figure \ref{fig:RV_19019}.}
    \label{fig:RV_169889}
\end{figure*}
...
\captionsetup[subfigure]{labelformat=empty}
    ...
    \subfloat[]{
        \centering
        \includegraphics[height=7.8cm,width=0.46\linewidth]{Figures/HD 169889/relsep_OC_HD169889_newpri.pdf}
        }
        \hfill
    \subfloat[]{
        \centering
        \includegraphics[height=7.8cm,width=0.50\linewidth]{Figures/HD 169889/PA_OC_HD169889_newpri.pdf}
    } \\
    \subfloat[]{
        \centering
        \includegraphics[height=7.8cm,width=0.985\linewidth]{Figures/HD 169889/Proper_Motions_HD169889_newpri.pdf}
    }
    \caption{Relative (top panel) and absolute (bottom panel) astrometry for the HD 169889 system. Other descriptions are identical to those for Figure \ref{fig:relAst_19019}.}
    \label{fig:relAst_169889}
\end{figure*}
