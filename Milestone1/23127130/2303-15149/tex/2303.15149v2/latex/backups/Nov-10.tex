...
%%%%%%%%% ABSTRACT
\begin{abstract}
    Sketches are highly expressive with ability to describe fine-grained visual cues. This innate descriptive potential of sketch over other modalities like text motivated the development of sketch based image retrieval (SBIR). Despite significant progress, SBIR serves a very ``niche" requirement -- given a sketch, search photos from a gallery. In this paper, we set out to ask if this ``niche" SBIR can be a game changer for mainstream computer vision tasks like object detection. In particular, we show how a powerful SBIR model is all you need to train an object detector. Towards this goal, we first solve a major bottleneck in SBIR -- scale SBIR models to generalise for unseen categories with high accuracy. We achieve this by introducing prompt learning for SBIR to exploit generalisation potential of foundation models (CLIP). Next, using our new sketch and photo encoders, we device a training paradigm for object detection such that the region embeddings of detected boxes are aligned with the sketch and photo embeddings from SBIR. Apart from uplifting SBIR to mainstream vision tasks, the resulting framework also radicalise object detection landscape with: (i) training object detector without using bounding box, or image-level class labels, (ii) detect all instances of the category in query sketch -- category-level object detection, (iii) detect a specific instance given a query sketch -- fine-grained object detection, (iv) detect only part of an object (e.g., ``head" of a ``horse") from partial sketches. Evaluating our framework on standard object detection datasets like PASCAL-VOC and MS-COCO outperforms both supervised (SOD) and weakly supervised object detectors (WSOD) on zero-shot setups.
...
Sketching is a universal form of human expression \cite{hertzmann2020perception} which can be directed to describe a user's intentions. With the proliferation of touch-screen devices, the digitisation of sketch opens several applications like fine-grained image retrieval \cite{sketchformer, yelamarthi2018sketch}, 3D shape modelling \cite{chowdhury20223Dsynthesis}, image synthesis \cite{cusuh2022synthesis}, among others. Despite gaining traction, sketch has either flourished in ``niche" applications like sketch-based image retrieval (SBIR) \cite{liveSketch}, or acted as an additional \emph{support} modality for incremental learning \cite{bhunia2022incremental}, representation learning \cite{pang2019generalising}, medical image analysis \cite{wang2022medical}, or image captioning \cite{fscoco}.
...
Given a highly generalisable SBIR, we train object detector such that the region embeddings of detected boxes are aligned with the pre-trained sketch and photo embeddings from SBIR. We extend object detection from pre-defined fixed-set categories to the open-vocab prototype learning \cite{prototype-segmentation-2021} setup where each category or instance-level prototype is computed using our pre-trained sketch encoder. Next, the model is trained with the training objective of weakly supervised object detectors (WSOD) \cite{wsodsurvey2022} using a multi-category cross-entropy loss over the prototypes of all possible categories or instances. While SBIR is trained using object-level (single object) sketch/photo pairs, object detection works on image-level (multiple categories). Hence, to train object detector using SBIR, we need to bridge the gap between object and image-level feature. Towards this, we use a data augmentation technique trick that is embarrassingly simple yet highly effective for robustness to corruption and generalisation to out-of-vocab \cite{cutmix, mixup}, by randomly combining $n=\{1, \dots, 7\}$ photos in SBIR. 

Apart from uplifting SBIR from a ``niche" application to mainstream computer vision tasks, the introduction of sketches radically changes the object detection landscape by opening new avenues: (i) detect a specific instance given a query sketch -- fine-grained object detection. This unlocks a more focused object detector allowing end-users to specify fine-grained RoI via sketching. (ii) Utilising the descriptive potential of sketch to detect only part of an object (e.g., ``head" of a ``horse") from a partial sketch. Achieving such specificity is cumbersome with alternatives like text \cite{text-sketch-dilemma}. (iii) Detect all instances of a category in the query sketch -- category-level object detection. This is analogous to object detection using text as query \cite{zareian2021ODcaptions}. Unlike prior works \cite{tripathi2020object}, we do not use sketch as an \emph{additional} label, but entirely rely on SBIR for training. (iv) We introduce a new setup for training object detectors -- extremely weakly supervised object detection (EWSOD) -- without using bounding box annotations (supervised setup), image-level class labels (weakly supervised setup) in downstream dataset, or ``seeing" to downstream data distribution given train-set.
