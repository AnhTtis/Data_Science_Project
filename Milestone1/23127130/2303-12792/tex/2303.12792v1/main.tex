...
\title[Resonant neutrino self-interactions]{Resonant neutrino self-interactions and the $H_0$ tension}
   ...
   \author{Jorge Venzor}\email{jorge.venzor@cinvestav.mx}
     \affiliation{ Departamento de Física, Centro de Investigación y de Estudios Avanzados del I.P.N. 
                    Apartado Postal 14-740, 07000, Ciudad de México, México. }
       \author{Gabriela Garcia-Arroyo}\email{arroyo@icf.unam.mx}
    \affiliation{Instituto de Ciencias Físicas, Universidad Nacional Autónoma de México, Apartado Postal 48-3, 62251 Cuernavaca, Morelos, México}
    ...
    \author{Abdel Pérez-Lorenzana}\email{aplorenz@fis.cinvestav.mx}
    \affiliation{ Departamento de Física, Centro de Investigación y de Estudios Avanzados del I.P.N. 
                    Apartado Postal 14-740, 07000, Ciudad de México, México. }
    \author{Josue De-Santiago}\email{Josue.desantiago@cinvestav.mx}
    \affiliation{ Departamento de Física, Centro de Investigación y de Estudios Avanzados del I.P.N. 
                    Apartado Postal 14-740, 07000, Ciudad de México, México. }
    \affiliation{Consejo Nacional de Ciencia y Tecnología.
    			Av. Insurgentes Sur 1582, 03940, Ciudad de México, México}
...
new physics acting before the recombination era. Nonstandard neutrino self-interactions are among the most appealing candidates to solve this issue since they could be testable in the near (or middle) future.
...
The joint data set which includes Planck, BAO, and $H_0$ prefers a non-zero interaction with at least 2.3$\sigma$ significance in the range $0.5$ eV $\leq m_{\varphi}\leq 10$ eV.
...
while early Universe measurements of the cosmic microwave background (CMB) radiation and the baryonic acoustic oscillations (BAO) consistently lead to a smaller value \cite{Aghanim2018Planck,ACT020,DES2018}, leading to a discrepancy of around $5\sigma$ (see for instance  \cite{Verde2019,Riess_2021,Riess2019ApJ,H0licow2020,Riess_2022,Riess:2022mme}).
...
While there are models that have tried to solve the tension at late epochs (see for example \cite{Alestas2021,Alestas2022,Heisenberg2022,Perivolaropoulos2021}), CMB measurements of $H_0$ certainly depend on the cosmological model, thus, it seems more natural to rather introduce new physics beyond the base $\Lambda$CDM model at early epochs.
Early Universe models may include modifications on the dark components, such as early dark energy (EDE) \cite{Niedermann2021,Boylan_Kolchin_2021} among others \cite{Di_Valentino_2021,SCHONEBERG20221}.
...
That is, it is easier to test neutrino NSI models compared with the same task for dark sector models \cite{Heurtier2017,Huang2018,Brune2019,Das2021,Schoneberg2019,Brdar2020,Venzor2021,Esteban2021_probing,Cerdeno2021,Deppisch2020,Bustamante2020,Akita2022,Dutta:2022fdt,Medhi:2021wxj,Ge:2021lur,Escrihuela:2021mud,Escudero2020_CMB_search}.

The dynamics of the neutrino NSIs heavily depends on the nature of the mediator particle, particularly on its mass.
On the one hand, an interaction mediated by a very light particle (compared with the temperature of the Universe) gets stronger at late times, the information about the mediator mass is erased by the interaction's nature, and strong bounds are obtained for the interaction coupling \cite{Forastieri2015,Forastieri2019,Venzor2022}.
...
In the latter case, observables depend on the effective coupling $(g_{\nu}/m_{\varphi})^2$ which presents a bimodal posterior distribution. One of its peaks prefers a large positive coupling, that deviates from the standard neutrino behavior. 
Because the required parameters are too large, the cosmological and astrophysical relics may constrain this scenario \cite{Bell2006,Archidiacono2014,CyrRacine2014,Lancaster2017,Kreisch2020,Park2019,Choudhury2021,Blinov2019,brinckmann2020self,mazumdar2020flavour,Huang2021,kreisch2022atacama,das2023magnificent}.
...
Resonant neutrino self-interaction implications have been previously studied in other astrophysical environments \cite{Creque2021,Chang2022aas,cerdeno2023constraints}. 
...
Yet, we should stress that the analysis could be extended to interactions mediated by higher spin particles with a proper re-scaling.
...
This approach is model neutrino independent.
In above, $g_\nu$ stands for the self-interacting coupling, $s=E_{\rm CM}^2=4E_{\nu}^2$ is the center of mass Mandelstam variable, $m_{\varphi}$ is the scalar mediator mass particle, and $\Gamma_{\varphi}=g_{\nu}m_{\varphi}/4\pi$ corresponds to the neutrino width. 
This cross-section peaks at $E_{\nu}\approx m_{\varphi}/2$, which would be later translated to a peak at $T_{\nu}\approx m_{\varphi}/2$.
...
Furthermore, it is worth mentioning that we have compared the above scattering rate with the one obtained by using the non-approximated expression \eqref{eq:breit}, where, to have a good numerical result, it is necessary to numerically sample it with very high precision around the resonance. 
...
The resonant scattering rate is a smooth function that peaks around temperatures $m_{\varphi}/2$ or at a redshift
\begin{equation}\label{eq:redshift_peak}
    z_{\rm peak}\approx 2.13 \times 10^3\left(\frac{m_{\varphi}}{{\rm eV}} \right) \, .
\end{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=.47\textwidth]{Figures/gamma_overH.pdf}
...
we can check when the interaction rate surpasses the Hubble function.
In Fig. \ref{fig:gamma_mphi} we plot the ratio $\Gamma/H$ for a relatively small coupling.
There, we can observe that lighter mediator's self-interactions enter into equilibrium too late, while heavier mediator's ratio is not as large, making it harder for the interaction to get into equilibrium.
...
We employ the formalism of the Boltzmann hierarchy equations~\cite{Ma1995} in conformal Newtonian gauge  (with $\Gamma=\Gamma_{\rm  scatt}$) and the relaxation time approximation. These considerations result in the system of equations, for the multipole Legendre moments of $\Psi$, 
\begin{eqnarray} 
...
As the modification of the Boltzmann hierarchy takes place just for $\ell \geq 2$  and therefore Eq. \eqref{eq:boltzmann_l2}  creates a change in the  neutrinos anisotropic stress, which in turn modifies the  Einstein shear equation:
...
\begin{figure}
    \centering
    \includegraphics[width=.48\textwidth]{Figures/D_ell_TT_5mphi_11.pdf}
...
To address the effects induced by the mediator mass, in Fig. \ref{fig:TT_resonance_mphi} we plot the spectra in two mass regions; below and above 1 eV. There, we see that the highest amplification of the CMB TT spectrum corresponds to the mass of  $1\, \rm{eV}$. This agrees with the previously observed fact that for such a mass the peak of the interaction rate to Hubble ratio happens just right above recombination. This reduces the free-streaming more than other masses, which prevents photon fluctuations to be suppressed. 
...
Also for the same reason, in this mass regime, the spectra have a less noticeable impact at low multipoles. On the contrary, the smaller masses will contribute more at low $\ell$ multipoles because its interaction rate is negligible compared to Hubble at early times and, as a consequence, we obtain deviations that decay at high multipoles.
 ...
 \begin{figure}
    \centering
    \includegraphics[width=.48\textwidth]{Figures/MPS_5mphi.pdf}
...
Thus, we focus here on the  MPS produced by masses below or equal to $1 \rm{eV}$, for which the global behavior is the same. The largest $k$ modes enter the horizon and decay before the interaction becomes relevant so we don’t have differences with respect to $\Lambda$CDM.
As we go to smaller $k$ values we have a small suppression of power (as a consequence of a smaller metric potential $\psi$ compared to $\Lambda$CDM) that once we move to smaller $k$ grows until it exceeds the MPS of $\Lambda$CDM and reaches a maximum (associated now with a boost of $\psi$). Finally, for the smaller $k$'s, the MPS behaves again like in $\Lambda$CDM.
...
Additionally, we also incorporate the most recent local measurement of $H_0$ by the SH0ES collaboration \cite{Riess_2022}.
...
For this reason, we can not use $m_\varphi$ as a free parameter.
...
It is worth mentioning that we also ran multiple MCMCs for larger mediator masses, although, these chains did not converge because they need a larger value of $g_\nu$, mimicking the heavy mediator scenario.
...
\hline
Data: Planck + & BAO & BAO+$H_0$  & BAO & BAO+$H_0$ & BAO & BAO+$H_0$ & BAO & BAO+$H_0$ & BAO & BAO+$H_0$\\
\hline
    $m_{\varphi}[\rm{eV}]$ & \multicolumn{2}{c|}{ $10^{-2}$} &\multicolumn{2}{c|}{ $10^{-1}$} &\multicolumn{2}{c|}{ 1} &\multicolumn{2}{c|}{ 10}  &\multicolumn{2}{c|}{ 100} \\
    \hline
$H_0  [\rm{km\ s^{-1}Mpc^{-1}}]   $ & $68.1\pm 1.4        $  & $71.12\pm 0.85           $ & $68.1\pm 1.4    $ & $71.22\pm 0.85   $ & $67.9\pm 1.3                $ & $71.18\pm 0.81         $ & $67.7^{+1.3}_{-1.4}      $ & $71.09\pm 0.81     $ & $67.5\pm 1.3      $ & $71.02\pm 0.81       $\\

$N_{\rm eff}                   $ & $3.12\pm 0.23       $  & $3.58^{+0.15}_{-0.17}    $ & $3.12\pm 0.22   $ & $3.59\pm 0.16    $ & $3.05^{+0.19}_{-0.22}       $ & $3.56\pm 0.15          $ & $3.03^{+0.20}_{-0.23}    $ & $3.54\pm 0.15      $ & $3.02\pm 0.21     $ & $3.54\pm 0.14        $\\

$\sum m_{\nu}[\rm{eV}]          $ & $< 0.110               $ & $< 0.119                $ & $< 0.114                  $ & $< 0.123                  $ & $< 0.112                   $ & $< 0.110                  $ & $< 0.120                  $ & $< 0.117                    $ & $< 0.110               $ & $< 0.0915                     $\\
                                   
$g_{\nu}\times 10^{14}     $ & $< 861                 $ & $< 909                  $ & $< 91.7                   $ & $< 106                    $ & $< 15.0                    $ & $12.4^{+4.4}_{-3.5}       $ & $< 36.7                   $ & $27\pm 10          $ & $< 147                 $ & $< 207                        $\\
$\Delta$AIC & 5.66 &-6.60  & 4.98 &-10.20 &5.56 &-9.38 &6.02 & -4.82& 8.22& -6.56 \\
$H_0$ tension &$2.83\sigma$&&$2.83\sigma$&&$3.09\sigma$&&$3.21\sigma$&&$3.33\sigma$&\\
\hline
\end{tabular}}
...
as we can see in the table for the different masses and when using the Planck + BAO dataset the measurement of $H_0$ presents a larger uncertainty compared to the $\Lambda$CDM model ($H_0=67.56^{+0.96}_{-0.88}\rm{km\ s^{-1}Mpc^{-1}}$). 
...
These results show that the RNSIs do indeed help to reduce the Hubble tension, as previously observed in the heavy mediator approximation \cite{Kreisch2020,Park2019,Choudhury2021,brinckmann2020self,kreisch2022atacama}.
...
We introduced the extra radiation via the parameter $N_{\rm ur}$ which corresponds to non-interacting radiation and $N_{\rm eff}=3.046+N_{\rm ur}$.
...
\begin{figure}
    \centering
    \includegraphics[width=.48\textwidth]{Figures/H0_vs_mass.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=.44\textwidth]{Figures/gnu_linear.pdf}
...
In Fig. \ref{fig:gnu_linear} we show the posterior distribution for $g_\nu$ with both data sets.
The two data sets indicate that the coupling is more constrained for the intermediate masses of 1 and 10 eV, which is consistent with our analysis in Sec. \ref{sec:class} where we showed that these masses led to the strongest effects on the CMB.
...
\begin{figure}
    \centering
    \includegraphics[width=.49\textwidth]{Figures/gnu_vs_mass.pdf}
...
For the intermediate masses of 1 and 10 eV, the data prefer a non-zero interaction with a significance larger than $2\sigma$.
...
In Fig. \ref{fig:gnu_vs_mass} we show the estimations of $g_\nu$, the range where we have at least a 2.3$\sigma$ preference for non-zero values is $0.5$ eV $\leq m_{\varphi}\leq 10$ eV.
Specifically, the statistical preference for nonnull values are $3.90\sigma$ ($m_{\varphi}=0.5\rm{eV}$),  $3.54\sigma$ ($m_{\varphi}=1\rm{eV}$),  $2.31\sigma$ ($m_{\varphi}=5\rm{eV}$), and $2.70\sigma$ ($m_{\varphi}=10\rm{eV}$).
...
New neutrino mass models should consider our results.
...
Consequently, we argue the that resonant NSI model is a good candidate to solve the $H_0$ tension.
Furthermore, when adding local $H_0$ data, the model accepts large $H_0$ values with extra radiation and non-null interaction couplings.
Actually models with masses in the range $0.5$ eV $\leq m_{\varphi}\leq 10$ eV prefer non-zero interactions with 2.3$\sigma$ to 3.9$\sigma$ levels of significance.
...
The AIC favored our model over $\Lambda$CDM when we used Planck + BAO + $H_0$ data. This means that the model improved the likelihood enough to justify the additional parameters. When we used only Planck + BAO data the criterion prefers the $\Lambda$CDM model, which indicates that the preference in the full data set comes from the ability of our model to reduce the $H_0$ tension.
...
Secondly, we ignored the resonant production of scalars and the neutrino decay (inverse decays) into lighter neutrinos and scalars.
These processes would be important because they may change the neutrino background and perturbation evolution.
Future studies in RNSIs should pursue a joint analysis that includes both inelastic and elastic processes.
From the data analysis point of view, it would be interesting to test the model with new data sets.
...
The previously unexplored region studied in this work now brings interesting links to physics beyond the standard model of particle physics. In particular, the preference in certain masses for a non-zero interaction is intriguing and should be studied further.
...
Work partially supported by Conacyt, Mexico, under FORDECYT-PRONACES Grant No. 490769.  G.G.A. is grateful to FORDECYT PRONACES-CONACYT Grant No. CF-MG-2558591, FOSEC SEP-CONACYT Ciencia Básica A1-S-21925, FORDECYT-PRONACES-
CONACYT 304001 and UNAM-DGAPA-PAPIIT IN117723.
J.V. acknowledges Conacyt, for partial support, under the project within ‘Paradigmas y Controversias de la Ciencia 2022’, No. 319395.
\end{acknowledgments}
...
\begin{figure}
    \centering
    \includegraphics[width=.42\textwidth]{Figures/100eV.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=.42\textwidth]{Figures/10eV.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=.42\textwidth]{Figures/1eV.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=.42\textwidth]{Figures/10minus1ev.pdf}
    \caption{Posterior probability for different sets of data and for $m_\varphi=10^{-1}$ eV.}
    \label{fig:triangular10minus1ev}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=.42\textwidth]{Figures/10minus2ev.pdf}
...
Specifically, we run these extra MCMCs to observe the behaviour of the $g_\nu$ bound with all the datasets.
We can see that $0.5$ and $5$ eV are consistent with a non-zero value, and, for $50$ eV, we do not observe any preference for nonstandard physics.
