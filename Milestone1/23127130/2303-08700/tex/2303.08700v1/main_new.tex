...
\title{Anomalous weak values require coherence}
...
\begin{abstract}
The quantum mechanical weak value $A_w=\left\langle {\phi}|A|\psi \right \rangle / \left\langle \phi | \psi \right\rangle$ of an observable $A$ is a measurable quantity associated with an observable $A$ and pre- and post-selected states $\ket{\psi}, \ket{\phi}$. Much has been discussed about the meaning and metrological uses of anomalous weak values, lying outside of the range of eigenvalues of $A$. We show that anomalous weak values require that both $\ket{\psi}, \ket{\phi}$  display coherence in the eigenbasis of $A$. We also present conditions under which anomalous $A_w$ are witnesses of generalized contextuality.
...
\textit{Introduction.}-- Superposition states are a defining hallmark of quantum mechanics. For general mixed states this resource is known as quantum coherence, and is defined with respect to a specific choice of basis $\{\vert a \rangle \}_{a}$ associated with a (non-degenerate) observable $A$. In this context, \textit{coherent states} $\rho$ are defined as those which have non-null off-diagonal density matrix elements $\langle a\vert \rho \vert a'\rangle \neq 0$ for $a \neq a' $.  Coherence can be formally treated as a resource~\cite{gour2019quantumresource}, and shown to be responsible for various nonclassical phenomena, providing advantage in information processing  tasks~\cite{streltsov2017colloquium}. 
...
\textit{A quasi-probability distribution associated with weak values.}-- Consider an arbitrary observable $A$, with eigenbasis $\{ \ket{a_i}\}_{i=1}^d$ and corresponding eigenvalues $\{a_i\}_{i=1}^d$. The weak value~\cite{aharonov1988result,aharonov1990properties} $A_w$ of $A$ is defined as:
\begin{equation}
...
\textit{Anomalous weak values imply coherence.}--An anomalous weak value $A_w$ is one that is outside of the range of eigenvalues of $A$. Clearly, the existence of an anomalous $A_w$ requires at least one anomalous quasi-probability~\cite{dressel2015weak}. Our first result is: 
...
\textit{Example 2: coherent states yielding non-anomalous quasi-probabilities.}-- Consider $\{\vert 0\rangle, \vert 1 \rangle\}$ to be a reference basis. Define $\vert \phi \rangle = \cos(\theta_\phi/2)\vert 0\rangle + \sin(\theta_\phi/2)\vert 1\rangle$ with $\theta_\phi \in [0,2\pi)$; and similarly for $\vert \psi \rangle$. Generic coherent states will not necessarily lead to anomalous values of $g$. In this example, no pair of angles $(\theta_\phi,\theta_\psi) \in [0,\pi]$ leads to anomalous values of $g$, despite the fact that generic parameters will result in coherent states in the computational basis. 

The presence of coherence without an associated weak value anomaly can also happen with mixed pre- and post-selected states. 
Let $\rho_\phi,\rho_\psi \in \mathcal{D}(\mathcal{H})$ be an arbitrary pair of non-commuting density matrices: $[\rho_\phi,\rho_\psi] \neq 0$. Non-commutativity guarantees coherence with respect to any basis, hence in particular $A$'s eigenbasis. Consider the corresponding real-amplitude states, by mapping $\rho_\phi \mapsto \rho_\phi^{\mathbb{R}} = (\rho_\phi + \rho_\phi^T)/2$ and similarly for $\rho_\psi$. Random generation of state pairs will rapidly turn up examples with only non-anomalous  $g(\rho_\phi^{\mathbb{R}},\rho_\psi^{\mathbb{R}} | a_i) \in [0,1], \forall i$. Here is a qubit example:
\begin{equation*}
...
\textit{Anomaly of $A_w$ as a witness of generalized contextuality.}-- Of particular significance for the discussion of nonclassicality of weak values is their connection with 
