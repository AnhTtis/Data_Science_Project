...
%    The main idea in the proofs of \cref{thm:CocycleIntro} and \cite[Lemma 1]{KuhnCocycle} is the same.
%    A notable difference in the implementation is that our approach is more local, we do all the computations on the level of points.
%    This, on one hand, results in a longer proof, but provides, on the other hand, more intuition.
\end{remark}
...
\subsection*{Acknowledgements} 
	I would like to thank Oleg Pikhurko for many insightful discussions and constant support, and to G\'{a}bor Elek for pointing out the reference \cite{KuhnCocycle}.
...
In order to devise an efficient (randomized) local algorithm that produces a proper edge colorings with $\Delta+1$ colors on finite graphs, Bernshteyn \cite{BernshteynVizing} iterated this process in a randomized way $\log(n)$ times, where $n$ is the number of vertices of the graph.
