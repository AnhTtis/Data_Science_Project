...
\begin{keywords}
dark matter -- cosmological simulations -- halo surface density
\end{keywords}
...
The aforementioned results for the constancy of dark matter halo surface density have however been  disputed by a number of authors who have shown that the dark matter column density (Eq.~\ref{eq:SB}) as well as the halo surface density (Eq.~\ref{eq:halosurfacedensity}) is not constant and is correlated with the galaxy luminosity,  halo mass, and stellar age~\citep{Boyarsky,Napolitano,Cardone, DelPopolo12,Cardonedel,Saburova,DelPopolo17,DelPopolo20}. Some of these works have found $S \propto M_{halo}^{0.16-0.2}$~\citep{DelPopolo12} as well as  $S \propto L^{0.13}$~\citep{DelPopolo20}.
...
\begin{equation}
    ...
    \label{eq:Smass}
\end{equation}
Therefore   $S_{gNFW}$ agrees with a  constant value to within 1$\sigma$. However, we cannot rule out a power law dependence  as a function of halo mass given the current errors.
Therefore, this result is in general agreement with the results that found a constant halo surface density~\citep{Donato} as well as a power law dependence on halo mass~\citep{Boyarsky}.  Since all the previous studies of halo surface density have used either a cored or NFW profile, a direct comparison of absolute value of halo surface density with our simulation results cannot be  easily done.
...
\begin{itemize}
    ...
    \item The dark matter column-density is consistent with a constant halo surface density as a function of halo mass, which is in agreement with previous results from galaxy-halo observations. However, we cannot rule out  a power-law dependence with halo mass also.
    ...
    \item The numerical values for column density with best-fit gNFW halos agree within $1\sigma$ with observational data for the halo surface density
    obtained from observations~\citep{Salucci19}.
