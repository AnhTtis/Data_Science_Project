...
{\color{blue}
\begin{remark}
Assumptions 1,3 and 4 are standard in the classic output regulation problem, Assumption 2 is avoid the trivial case of stable S. Assumption 5 guarantees that $\Psi_k$ defined in (19) is positive definite  \cite{haghshenas2015}, which is useful to prove the exponential convergence of estimators and observers in the following. $\hfill \hfill \square $
\end{remark}
}
...
\begin{figure}[!]
  ...
  \centering
  \includegraphics[width=0.5\textwidth]{pic/CaL.jpg}
...
\end{cases}$ and $\tilde{\Upsilon}_0(t)= [\tilde{\Upsilon}_{01}(t),\tilde{\Upsilon}_{01}(t),\dots,\tilde{\Upsilon}_{0N}(t)]$, $\bar{\xi} = [\bar{\xi}_1^{\mathrm{ T}},\bar{\xi}_2^{\mathrm{T}},\dots,\bar{\xi}_n^{\mathrm{T}}]^{\mathrm{T}}$, and $\bar{y}=[\bar{y}_1^{\mathrm{T}},\bar{y}_2^{\mathrm{T}},\dots,\bar{y}_n^{\mathrm{T}}]^{\mathrm{T}}$.

{\color{red}
Define the following global output containment error:
\begin{equation}
e= \bar{y} - (\sum_{r\in \mathcal{L} }(\Psi^D_r \otimes I_p))^{-1} \sum_{k \in \mathcal{L} } (\Psi^D_k \otimes I_p) \underline{y}_k,
\end{equation}
...
In this section, a double-layer resilient control scheme is used to solve the \textbf{Problem ACMCA}. Firstly, we consider a full distributed observers to estimate the unknown matrix $S$ and $R$ for all agents under the effect of DoS attacks. Next, a distributed virtual resilient TL, which can resist most attacks, is proposed to estimate the state of followers containment under DoS attacker. Then, we  use the estimation of leaders in Theorem 1 to solve the the solve it by the  output regulation equation. Finally, a adaptive controllers are proposed to resist  AAs in CPL.
...
%{\color{blue}
\textbf{Proof.} 
Form the (\ref{EQ15}) and (\ref{EQ16}), it can seen that We only used the  relative neighborhood information to estimate leader dynamics, therefore,  the leader estimator will suffer the influence of Dos attacks.
...
\begin{equation}
\begin{aligned}
  \tilde{\Upsilon}_0(t) &\leq  \tilde{\Upsilon}_0(t_0)e^{-\sigma_{\rm max}(\bar{\Psi}_k^D)(t-t_0)} \\
  &\leq \tilde{\Upsilon}_0(t_0)e^{-\sigma_{\rm max}(\bar{\Psi}_k)\left\lvert S_N(t_0,t)\right\rvert }
 , t\geq t_0.
\end{aligned}
\end{equation}
...
\end{aligned}
\end{equation}
By $F_2^x$ exponentially converges to zero, there exist $a_f >0$ and $\lambda_f >0$ such that following holds
...
\begin{equation}
\begin{aligned}
   \tilde{z} & = e^{\int_{t_0}^{t} \hat{S}_b(\tau) \, d\tau-\bar{c}_{\Psi}|S_N|}\tilde{z}(t_0) +    a_f  \int_{t_0}^{t} e^{ \int_{\tau}^{t} \hat{S}_b(\tau^*) \, d\tau^*-\bar{c}_{\Psi}|S_N(\tau,t)| +\lambda_f I_{N\times p}(-\tau)} \,d\tau \\
\end{aligned}
\end{equation}
with
...
\begin{align}
    &\dot{\hat{\Delta}}_{0i} = -  \hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{0i}-\hat{\mathcal{R}}_i) \label{equation 310} \\
    &\dot{\hat{\Delta}}_{1i} = \left\lVert \hat{\Upsilon}_i\right\rVert(\hat{\Delta}_{0i}- \hat{\Delta}_{1i})- \hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{1i}-\hat{\mathcal{R}}_i)\\
   &\dot{\hat{\Delta}}_{i} = \left\lVert \hat{\Upsilon}_i\right\rVert(\hat{\Delta}_{1i}- \hat{\Delta}_{i})- \hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{i}-\hat{\mathcal{R} }_i) ,
\end{align}
...
&=-  \Phi_i^{\mathrm{T}} \Phi_i \hat{\Delta}_{0i} +   \Phi_i^{\mathrm{T}} \mathcal{R}_i +d_1(t) ,\\
\end{aligned}
\end{equation}
where  $d_1(t)= -  (\hat{\Phi}_i^{\mathrm{T}}\hat{\Phi}_i - \Phi_i^{\mathrm{T}}\Phi_i) \hat{\Delta}_{0i} +   \tilde{\Phi}_i^{\mathrm{T}} \hat{\mathcal{R}}_i +   \Phi_i^{\mathrm{T}} \tilde{\mathcal{R}}_i$ with $\tilde{\Phi}_i=\hat{\Phi}_i-\Phi_i=\tilde{S}^{\mathrm{T}} \otimes A_{2i}$ and $\tilde{\mathcal{R}}={\rm vec}(\left[\begin{array}{cc}
    0  \\
    \tilde{R}_i
    \end{array}
    \right])$,
...
\end{aligned}
\end{equation}
 since $ \Phi_i^{\mathrm{T}} \Phi_i$ is  Hurwitz, and $d_i(t)$ coverage to $0$ at rate of $\alpha_{\Upsilon}$, by Lemma \ref{Lemma 1}   $\lim _{t \rightarrow \infty }\tilde{\Delta}_{0i}=0$ exponentially .
...
\begin{figure}[!]
  %\begin{minipage}[t]{1\linewidth}
  \centering
  \includegraphics[width=0.5\textwidth]{pic/Upsilon3.jpg}
...
\begin{figure*}[htbp]
  ...
  \centering
  \includegraphics[width=1\textwidth]{pic/TLz12.jpg}
  \label{fig:figure2:1}
  %\caption{fig1}
  \end{minipage}%
  }%
  ...
  \begin{minipage}[t]{0.32\textwidth}
  \centering
  \includegraphics[width=1\textwidth]{pic/TLz222.jpg}
  \label{fig:figure2:2}
  %\caption{fig2}
  \end{minipage}%
  }%
...
\begin{figure}[!]
  %\begin{minipage}[t]{1\linewidth}
  \centering
  \includegraphics[width=0.5\textwidth]{pic/Delta3.jpg}
...
\begin{figure}[!]
  %\begin{minipage}[t]{1\linewidth}
  \centering
  \includegraphics[width=0.5\textwidth]{pic/2D2.jpg}
...
\begin{figure*}[htbp]
  ...
  \begin{minipage}[t]{0.32\textwidth}
  \centering
  \includegraphics[width=1\textwidth]{pic/e1.jpg}
  \label{fig:figure5:1}
  %\caption{fig1}
  \end{minipage}%
  }%
  ...
  \begin{minipage}[t]{0.32\textwidth}
  \centering
  \includegraphics[width=1\textwidth]{pic/e2.jpg}
  \label{fig:figure5:2}
  %\caption{fig2}
  \end{minipage}%
  }%
  \centering
  \caption{The output containment errors.}
  \label{fig:figure5}
  \end{figure*}
