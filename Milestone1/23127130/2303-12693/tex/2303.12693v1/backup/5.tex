...
In this work, we consider the MASs consisting of cooperative agents with potential malicious attackers. As shown in the figure.1, the attackers will use four kinds of attacks to compromise the containment performance of the MASs:
...
\begin{assumption}
  The actuator attack $\chi$ is unbounded and its derivative $\dot{d}_i$ is bounded.
\end{assumption}
...
\end{aligned}
\end{equation}
Under Assumption \ref{assumption 2} and the Lemma 6 of \cite{haghshenas2015}, all the eigenvalues of $\Psi_k$ have positive real parts. 
...
\end{equation}
Based on (\ref{EQF3}),(\ref{EQF4}), we can obtain the following inequality from (\ref{EQ32})
\begin{equation}
...
\begin{align}
    &\dot{\hat{\Delta}}_{0i} = -\mu \hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{0i}-\hat{\mathcal{R}}_i) \label{equation 310} \\
    &\dot{\hat{\Delta}}_{1i} = \left\lVert \hat{\Upsilon}_i\right\rVert_2(\hat{\Delta}_{0i}- \hat{\Delta}_{1i})-\mu\hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{1i}-\hat{\mathcal{R}}_i)\\
   &\dot{\hat{\Delta}}_{i} = \left\lVert \hat{\Upsilon}_i\right\rVert_2(\hat{\Delta}_{1i}- \hat{\Delta}_{i})-\mu\hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{i}-\hat{\mathcal{R} }_i) ,
\end{align}

where $\mu > 0$, $\hat{\Delta}_{ji}={\rm vec}(\hat{Y}_{ji})$ for $j=0,1$ , $\hat{\Delta}_i={\rm vec}(\hat{Y}_i), \hat{\Phi}_i=(I_q \otimes  A_{1i}-\hat{S}_i^{\mathrm{T}} \otimes A_{2i})$, $\hat{R}_i={\rm vec}(\mathcal{\hat{R}}^{\ast}_i)$; $\hat{Y}_{ji}=[\hat{\Pi}_{ji}^{\mathrm{T}}$, $\hat{\Gamma}_{ji}^{\mathrm{T}}]^{\mathrm{T}}$ for $j=0,1$ , $\hat{Y_{i}}=[\hat{\Pi}_{i}^{\mathrm{T}}$, $\hat{\Gamma}_{i}^{\mathrm{T}}]^{\mathrm{T}}$, $\hat{R}^{\ast}_i=[0,\hat{R}_i^{\mathrm{T}}]^{\mathrm{T}}$, $
A_{1i}=\left[
  \begin{array}{cc}
  A_i &  B_i  \\
  C_i &  0   \\
...
\begin{aligned}
\dot{\hat{\Delta}}_{0i}
&= -\mu \hat{\Phi }^{\mathrm{T}}_i(\hat{\Phi }_i \hat{\Delta}_{0i}-\hat{\mathcal{R}}_i) \\
&=-\mu \hat{\Phi}^{\mathrm{T}}_i \hat{\Phi}_i \hat{\Delta}_{0i}+ \mu \hat{\Phi}_i \hat{\mathcal{R}_i} \\
&=-\mu \Phi_i^{\mathrm{T}} \Phi_i \hat{\Delta}_{0i} +\mu \Phi_i^{\mathrm{T}} \Phi_i \hat{\Delta}_{0i} -\mu \hat{\Phi}_i^{\mathrm{T}}  \hat{\Phi}_i \hat{\Delta}_{0i} +\mu \hat{\Phi}_i^{\mathrm{T}} \hat{\mathcal{R}_i} -\mu \Phi_i^{\mathrm{T}} \hat{\mathcal{R}_i} + \mu \Phi_i^{\mathrm{T}} \hat{\mathcal{R}_i} -\mu \Phi_i^{\mathrm{T}} \mathcal{R}_i + \mu \Phi_i^{\mathrm{T}} \mathcal{R}_i \\
&=-\mu \Phi_i^{\mathrm{T}} \Phi_i \hat{\Delta}_{0i} + \mu (\Phi_i^{\mathrm{T}} \Phi_i - \hat{\Phi}_i^{\mathrm{T}} \hat{\Phi}_i) \hat{\Delta}_{0i} + \mu (\hat{\Phi}_i^{\mathrm{T}} - \Phi_i^{\mathrm{T}}) \hat{\mathcal{R}_i} + \mu \Phi_i^{\mathrm{T}} (\hat{\mathcal{R}_i}-\mathcal{R}_i) + \mu \Phi_i^{\mathrm{T}} \mathcal{R}_i \\
&=-\mu \Phi_i^{\mathrm{T}} \Phi_i \hat{\Delta}_{0i} + \mu \Phi_i^{\mathrm{T}} \mathcal{R}_i +d_1(t) ,\\
\end{aligned}
\end{equation}
where  $d_1(t)= -\mu (\hat{\Phi}_i^{\mathrm{T}}\hat{\Phi}_i - \Phi_i^{\mathrm{T}}\Phi_i) \hat{\Delta}_{0i} + \mu \tilde{\Phi}_i^{\mathrm{T}} \hat{\mathcal{R}}_i + \mu \Phi_i^{\mathrm{T}} \tilde{\mathcal{R}}_i$ . Then, since the origin of $\dot{\hat{\Delta}}_{0i}=-\mu \Phi_i^{\mathrm{T}}\Phi_i \hat{\Delta}_{0i}$ is exponentially stable , system (\ref{equation 310}) is input-to-stable with $d_1(t)+ \mu \Phi_i^{\mathrm{T}} \mathcal{R}_i$ as the input.
$\tilde{\Phi}_i=\hat{\Phi}_i-\Phi_i=\tilde{S}^{\mathrm{T}} \otimes A_{2i}$, $\tilde{\mathcal{R}}={\rm vec}(\left[\begin{array}{cc}
    0  \\
    \tilde{R}_i
    \end{array}
    \right])$,
...
\textbf{Step 2:}
To prepare the proof for the next step, we use the solution of Step 1 to calculate the convergence rates of $\mu\hat{\Phi}^{\mathrm{T}}_i(\hat{\Phi}_i \Delta_i-\hat{\mathcal{R} }_i)$.
...
\begin{aligned}
    \dot{\tilde{\Delta}}_{0i}
    =-\mu \Phi_i^{\mathrm{T}} \Phi_i \tilde{\Delta}_{0i} - \mu \Phi_i^{\mathrm{T}} \Phi_i \Delta+\mu \Phi_i^{\mathrm{T}} \mathcal{R}_i +d_1(t)
    =-\mu \Phi_i^{\mathrm{T}} \Phi_i \tilde{\Delta}_{0i} +d_1(t),
\end{aligned}
\end{equation}
 so   $\lim _{t \rightarrow \infty }\tilde{\Delta}_{0i}=0$ exponentially. In particular, if $\mu \geq \frac{\alpha_{\Upsilon}}{ \sigma_{\rm min}(\Phi_i^{\mathrm{T}}\Phi_i)}$($\mu$ is sufficiently large and $\alpha_{\Upsilon}$ is the exponential convergence rate of $\tilde{\Upsilon}$ ), $\lim _{t \rightarrow \infty }\tilde{\Delta}_{0i}=0$ exponentially at the rate of $\alpha_{\Upsilon}$.
 
 By
 \begin{equation}
 \dot{\tilde{\Delta}}_{0i}
=\dot{\Delta}-\dot{\hat{\Delta}}_{0i}
=-\mu\hat{\Phi}^{\mathrm{T}}_i\hat{\Phi}_i\tilde{\Delta}_{0i}+\mu\hat{\Phi}^{\mathrm{T}}_i(\hat{\Phi}_i \Delta_i-\hat{\mathcal{R} }_i),
 \end{equation}
coverage to $0$ exponentially at the rate of $\alpha_{\Upsilon}$ and $ \sigma_{\rm min}(\mu\hat{\Phi}^{\mathrm{T}}_i\hat{\Phi}_i)) \geq \alpha_{\Upsilon}$, we can get that
${\rm  lim}_{t\rightarrow \infty} \mu\hat{\Phi}^{\mathrm{T}}_i(\hat{\Phi}_i \Delta_i-\hat{\mathcal{R} }_i)=0$ exponentially at the rate of $\alpha_{\Upsilon}$ at least.
...
=-(\left\lVert \hat{\Upsilon}_i\right\rVert_2I_{p^2+pq}+ \mu\hat{\Phi}^{\mathrm{T}}_i\hat{\Phi}_i)\tilde{\Delta}_{1i}+\left\lVert \hat{\Upsilon}_i\right\rVert_2\tilde{\Delta}_{0i} +\mu\hat{\Phi}^{\mathrm{T}}_i(\hat{\Phi}_i \Delta_i-\hat{\mathcal{R} }_i),
\end{equation}
$ \sigma_{\rm min}(\left\lVert \hat{\Upsilon}_i\right\rVert_2I_{p^2+pq}+ \mu\hat{\Phi}^{\mathrm{T}}_i\hat{\Phi}_i) > \alpha_{\Upsilon}$, $\left\lVert \hat{\Upsilon}_i\right\rVert_2\tilde{\Delta}_{0i} +\mu\hat{\Phi}^{\mathrm{T}}_i(\hat{\Phi}_i \Delta_i-\hat{\mathcal{R} }_i)$ coverage to $0$ exponentially at the rate of $\alpha_{\Upsilon}$.
By Lemma1, we obtain ${\rm  lim}_{t\rightarrow \infty }  \tilde{\Delta}_{1i} = 0$ at the rate of $\alpha_{\Upsilon}$,


Let $\tilde{\Delta}_{i}= \Delta - \Delta_{i}$ , as the same proof of $\tilde{\Delta}_{1i}$, it easy to get that ${\rm lim}_{t\rightarrow \infty }  \tilde{\Delta}_{i} = 0$ exponentially at the rate of $\alpha_{\Upsilon}$, the proof is completed. $\hfill \hfill \blacksquare $
%
...
&\hat{\chi}_i=\frac{B_i^{\mathrm{T}} C_i^{\mathrm{T}} \bar{P}_i \tilde{y}_i}{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +\omega} \hat{\rho_i} \\ \label{EQ65}
&\dot{\hat{\rho}}_i=\begin{cases}
 \left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +2\omega , {\rm if} \left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert \geq \bar{\kappa} \\
 \left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +2\omega \frac{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert}{\bar{\kappa}}, {\rm otherwise},
\end{cases}
\end{align}
where $\tilde{y}_i=y_i - C_i \hat{x}_i$, $\bar{P}_i=C_i P_i  C_i^{\mathrm{T}}$, $\hat{\chi}_i$ is an adaptive compensational signal, $\hat{\rho}_i$ is an adaptive updating parameter and the controller gain $K_i$ is designed as 
\begin{equation}
...
Next, we show that the observer output error converges exponentially to 0.

Define the estimator state error $\tilde{x}_i= x_i-\hat{x}_i$, from (\ref{EQ2}) and (\ref{EQ58}),  we obtain that
\begin{equation}
\begin{aligned}
        \dot{\tilde{x}}_i  
        &= A_i x_i +B_i(u_i +\chi_i) - A_i \hat{x}_i -B_i(u_i+\hat{\chi_i}) -G(C_i \hat{x_i} - y_i)\\
        &=(A_i+ G_i C_i) \tilde{x}_i+ B_i \tilde{\chi}_i.
\end{aligned}
\end{equation}
...
\begin{equation}
    V_i^x =\tilde{y}_i^{\mathrm{T}}  \bar{P}_i \tilde{y}_i,
\end{equation}
and its times derivative is presented as follows:
\begin{equation}\label{EQ74}
    \dot{V}_i^x = 2 \tilde{x}_i^{\mathrm{T}} C_i^{\mathrm{T}} \bar{P}_i C_i (A_i+ G_i C_i) \tilde{x}_i + 2 \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \tilde{\chi}_i,
\end{equation}
%}
%{\color{red}
noting that
\begin{equation}
\begin{aligned}
\tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i\tilde{\chi}_i
        &= \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \chi_i -\frac{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert^2}{ \left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +\omega} \hat{\rho_i} \\
& \leq  \left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i\right\rVert \left\lVert \chi_i \right\rVert - \frac{\left\lVert \tilde{y}_i^{\mathrm{T}} P_i C_i B_i \right\rVert^2  }{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert + \omega} \hat{\rho_i} \\
& = \frac{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert ^2 (  \left\lVert \chi_i \right\rVert-  \hat{\rho}_i)+ \left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert\left\lVert \chi_i \right\rVert \omega}{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +\omega } \\
&=\frac{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert ^2 (\frac{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +\omega}{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert}||\chi_i||-\hat{\rho}_i)}{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +\omega}.
\end{aligned}
\end{equation}
Noting that $d\left\lVert \chi_i \right\rVert/dt $ is bounded,  so, if $\frac{\bar{\kappa}+\omega}{\bar{\kappa}}\geq \bar{\kappa} \geq \frac{d||\chi_i||}{d t}$, that is, $\frac{\bar{\kappa}+\omega}{\bar{\kappa}} \frac{d||\chi_i||}{dt} -\dot{\hat{\rho}} \leq \bar{\kappa}+\omega-\dot{\hat{\rho}} \leq -\omega < 0$. Then , there exists $t_2 > 0$ such that for all $t \geq t_2$ , we have 
\begin{equation}\label{EQ76}
     (\frac{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert +\omega}{\left\lVert \tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i \right\rVert}||\chi_i||-\hat{\rho}_i) \leq (\frac{\bar{\kappa}+\omega}{\kappa}||\chi_i||-\hat{\rho}_i) <0  .
\end{equation}
Thus, we can obtain that $\tilde{y}_i^{\mathrm{T}} \bar{P}_i C_i B_i\tilde{\chi}_i <0$ over $t \in[t_2,\infty)$.
from (\ref{EQ74}) ~ (\ref{EQ76}), it yields
\begin{equation}\label{EQ77}
\begin{aligned}
      \dot{V}_i^x 
      & \leq 2 \tilde{x}_i^{\mathrm{T}} C_i^{\mathrm{T}} \bar{P}_i C_i (A_i+ G_i C_i) \tilde{x}_i \\
...
\end{equation}
{\color{black}
Since ($C_i,A_i$) is detectable, let $G_i$ be such that $A_i +G_i C_i$ is Hurwitz. It is obvious that $V_i^x$ is unbounded. And it is obvious that $\tilde{y}_i$ are unbounded by $\frac{d||\chi_i||/dt}{|| \bar{P}_i C_i B_i||}$.
}
\begin{myTheo}
...
The derivative of $\epsilon_i$ is presented as follows:
  ...
  \begin{aligned}
      \dot{\epsilon}_i
      &= A_i \hat{x}_i + B_i u_i +B_i \hat{\chi}_i -\dot{\hat{\Pi}}_i  z_i - \hat{\Pi}_i(\hat{S}_i z_i - c_{\Psi} (\sum _{j \in \mathcal{F} }a_{ij}(z_i-z_j)+\sum_{k \in \mathcal{L}}g_{ik}(z_i-x_k)))
      +G_i C_i (\hat{x}_i - x_i)\\
      &=(A_i+B_i K_i)\epsilon_i  - \dot{\hat{\Pi}}_i z_i +c_{\Psi} \hat{\Pi}_i  (\sum _{j \in \mathcal{F} }a_{ij}(z_i-z_j)+\sum_{k \in \mathcal{L}}g_{ik}(z_i-x_k))
       -G_i \tilde{y}_i.
  \end{aligned}
  \end{equation}
%  
 ...
 \begin{aligned}
  \dot{\epsilon}
  &={\rm blkdiag}(A_i+B_iK_i)\epsilon -{\rm blkdiag}(\dot{\hat{\Pi}}_i)z + c_{\Psi} {\rm blkdiag}(\hat{\Pi}_i)(\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )(z-\underline{x}_k))
  -{\rm blkdiag}(G_i ) \tilde{y}\\
  &={\rm blkdiag}(A_i+B_i K_i)\epsilon -{\rm blkdiag}(\dot{\hat{\Pi}}_i)(\tilde{z}+ (\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1} \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \underline{x}_k) + c_{\Psi} {\rm blkdiag}(\hat{\Pi}_i)\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )\tilde{z} \\
  & -{\rm blkdiag}(G_i ) \tilde{y}.\\
\end{aligned}
 \end{equation}
%
...
\begin{equation} 
    V= \epsilon ^{\mathrm{T}} P \epsilon,
\end{equation}
and its time derivate is given as follows: 
\begin{equation}\label{EQ80}
\begin{aligned}
    \dot{V}
    &= 2\epsilon^{\mathrm{T}} P \dot{\epsilon}_i \\
    &=-\epsilon^{\mathrm{T}} {\rm blkdiag}(Q_i) \epsilon +2\epsilon^{\mathrm{T}} P ({\rm blkdiag}(\dot{\hat{\Pi}}_i)+  c_{\Psi} {\rm blkdiag}(\hat{\Pi}_i)\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) )\tilde{z} \\
    & + 2\epsilon^{\mathrm{T}} P {\rm blkdiag}(\dot{\hat{\Pi}}_i)(\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1} \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \underline{x}_k - 2\epsilon^{\mathrm{T}} P {\rm blkdiag}(G_i)\tilde{y}\\
    &\leq -\sigma_{min}(Q) \left\lVert \epsilon\right\rVert^2  +2\left\lVert \epsilon^{\mathrm{T}}\right\rVert \left\lVert P\right\rVert_2  \left\lVert {\rm blkdiag}(\dot{\hat{\Pi}}_i)+  c_{\Psi} {\rm blkdiag}(\hat{\Pi}_i)\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )  \right\rVert  \left\lVert \tilde{z}\right\rVert\\ 
 & +2\left\lVert \epsilon^{\mathrm{T}}\right\rVert \left\lVert P\right\rVert_2   \left\lVert  
  (\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1}\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) {\rm blkdiag}(\dot{\hat{\Pi}}_i) \underline{x}_k\right\rVert + 2\left\lVert \epsilon^{\mathrm{T}}\right\rVert \left\lVert P\right\rVert_2 \left\lVert {\rm blkdiag}(G_i )\right\rVert \left\lVert \tilde{y} \right\rVert, \\
\end{aligned}
\end{equation}
...
\begin{aligned}
    & 2\left\lVert \epsilon^{\mathrm{T}}\right\rVert \left\lVert P\right\rVert_2   \left\lVert  {\rm blkdiag}(\dot{\hat{\Pi}}_i) 
...
\begin{aligned}
   & 2\left\lVert \epsilon^{\mathrm{T}}\right\rVert \left\lVert P\right\rVert_2  \left\lVert {\rm blkdiag}(\dot{\hat{\Pi}}_i)+  c_{\Psi} {\rm blkdiag}(\hat{\Pi}_i)\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )  \right\rVert  \left\lVert \tilde{z}\right\rVert \\
...
\begin{aligned}
   & 2\left\lVert \epsilon^{\mathrm{T}}\right\rVert \left\lVert P\right\rVert_2 \left\lVert {\rm blkdiag}(G_i )\right\rVert \left\lVert \tilde{y} \right\rVert, \\
...
\begin{equation}\label{EQ70}
  \dot{V} \leq -(\beta_{V1}+\frac{1}{2} \sigma_{min}(Q) ) \epsilon^{\mathrm{T}} \epsilon +\beta_{V2}e^{\alpha_{V1}t}.
\end{equation}
Solving (\ref{EQ70}) yields the following:
\begin{equation}
\begin{aligned}
     V(t) 
    & \leq V(0) - \int_{0}^{t} (\beta_{V1}+\frac{1}{2} \sigma_{min}(Q) ) \epsilon^{\mathrm{T}}\epsilon \,d\tau + \int_{0}^{t} \beta_{V2}e^{\alpha_{V1}t} \,d\tau ,\\
\end{aligned}
\end{equation}
then
