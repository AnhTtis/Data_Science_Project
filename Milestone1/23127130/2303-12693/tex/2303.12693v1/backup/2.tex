...
\begin{lemma}[{\cite[Lemma 1]{cai2017 }}]\label{Lemma 1}
    ...
    $$\dot{x}=\epsilon F x +F_1(t)x+F_2^x(t)$$
    ...
    (ii) if $F_1(t) = 0$, $F_2^x(t)$ decays to zero exponentially at the rate of $\alpha$,
...
\begin{equation}
    \tilde{\Upsilon}(t) \leq (\tilde{\Upsilon}(t_0) + \int_{t_0} ^{t} {\rm blkdiag}(\left\lVert \hat{\Upsilon } _{0i}(t) \right\rVert_2)\otimes I_{p+q} \tilde{\Upsilon}_0(t) \,d\tau) e^{- {\rm min}(\left\lVert \hat{\Upsilon } _{0i}(t) \right\rVert_2)(t-t_0)}.
\end{equation}
...
\begin{equation}\label{equation 200}
  \dot{z}_i=\hat{S}_i z_i -\chi (\sum _{j \in \mathcal{F} }w_{ij}(z_i-z_j)+\sum_{k \in \mathcal{L}}w_{ik}(z_i-x_k)), t\geq T_i
\end{equation}
%{\color{blue}
where $z_i$ is the local state of the virtual layer and $\chi  >  0$ is the estimator gain designed in Theorem \ref{Theorem 2}. $T_i$ represents a time before the first Dos attack and guarantee $\tilde{\Upsilon}(T_i)$ small enough.
%}
The global state of virtual resilient layer   can be written as
\begin{equation}
  \dot{z}= {\rm blkdiag}(\hat{S}_i)z-\chi (\sum_{k \in \mathcal{L}}(\Psi_k^w \otimes I_p )(z-\underline{x}_k)), 
\end{equation}
where $z=[z_1,z_2,\dots,z_N]$, $\underline{x}_k=l_n \otimes x_k$ and $\chi  >  0$ is the estimator gain designed in Theorem \ref{Theorem 2}.
...
\begin{equation}\label{EQ26}
    \begin{aligned}
      \dot{\tilde{z}}
       &={\rm blkdiag}(\hat{S}_i)z-\chi (\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )(z-\underline{x}_k))-
  (\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1}\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) (I_n \otimes S) \underline{x}_k \\
      &={\rm blkdiag}(\hat{S}_i)z- (I_n \otimes S)z+ (I_n \otimes S)z 
  ...
  &={\rm blkdiag}(\tilde{S}_i)z+(I_n \otimes S) \tilde{z}-\chi\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \tilde{z} -
  \chi(\sum_{kk \in \mathcal{L}}(\Psi_{kk} \otimes I_p ) \underline{x}_{kk}- \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )\underline{x}_k) +M\\
  & =(I_n \otimes S) \tilde{z}-\chi\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \tilde{z}+ {\rm blkdiag}(\tilde{S}_i) \tilde{z} + F_2^x(t) ,
    \end{aligned}
\end{equation}
where $F_2^x(t)={\rm blkdiag}(\tilde{S}_i)(\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1} \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \underline{x}_k +M$ and
...
\begin{equation}\label{EQ27}
    \begin{aligned}
      \dot{\tilde{z}}
       &={\rm blkdiag}(\hat{S}_i)z-
  (\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1}\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) (I_n \otimes S) \underline{x}_k \\
      &={\rm blkdiag}(\hat{S}_i)z- (I_n \otimes S)z+ (I_n \otimes S)z 
  -(I_n \otimes S)(\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1} \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p )  \underline{x}_k  +M \\
  &={\rm blkdiag}(\tilde{S}_i)z+(I_n \otimes S) \tilde{z} +M\\
  & =(I_n \otimes S) \tilde{z}+ {\rm blkdiag}(\tilde{S}_i) \tilde{z} + F_2^x(t) ,
    \end{aligned}
\end{equation}
So, we can conclude (\ref{EQ26}) and (\ref{EQ27}) that
\begin{equation}\label{EQ32}
    \dot{\tilde{z}}_i = \begin{cases}
     (I_n \otimes S) \tilde{z}-\chi\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \tilde{z}+ {\rm blkdiag}(\tilde{S}_i) \tilde{z} + F_2^x(t), t \in S_N(t_0,t) \\
     (I_n \otimes S) \tilde{z}+ {\rm blkdiag}(\tilde{S}_i) \tilde{z} + F_2^x(t) , t \in S_A(t_0,t).
    \end{cases}
\end{equation}
Let 
\begin{equation}
...
Then $F_2^x(t)={\rm blkdiag}(\tilde{S}_i)(\sum_{r \in \mathcal{L}}(\Psi_r \otimes I_p ))^{-1} \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \underline{x}_k$ and $\lim_{t \rightarrow \infty}F_2^x(t)=0$ is exponentially at the rate of $\tilde{S}$.
...
\begin{equation}
    \tilde{z}(t) \leq (\tilde{z}(t_0)+ \int_{t_0}^{t} F_4(\tau) \, d\tau )e^{\int_{t_0}^{t} F_3(\tau) \, d\tau}.
\end{equation}

Based on (36)~(38), we can obtain the following inequality from (\ref{EQ32})
\begin{equation}
    \tilde{z}(t) \leq \begin{cases}
     e^{\int_{\sigma_{2i}}^{t} \hat{S}_d -\bar{\chi}\, d\tau}(\tilde{z}(\sigma_{2i}) +\int_{\sigma_{2i}}^{t} F_2^x(\tau) \, d\tau) , t \in [\sigma_{2i},\sigma_{2i+1}) \\
     e^{\int_{\sigma_{2i+1}}^{t} \hat{S}_d \, d\tau}(\tilde{z}(\sigma_{2i+1}) +\int_{\sigma_{2i+1}}^{t} F_2^x(\tau) \, d\tau) , t \in [\sigma_{2i+1},\sigma_{2i+2}).
    \end{cases}
\end{equation}
where $\hat{S}_d= {\rm blkdiag}(\hat{S}_i)$  and $\bar{\chi}= \chi\sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) $.
...
\end{cases}
\end{equation}
and letting $D_{2k}(t,r)=\int_{t_{2k}}^{t} \hat{S}_d-\bar{\chi} \, d\tau +\sum_{m=r}^{2k} a(m-1)$.
\begin{align}
    &\int_{t_{2k}}^{t} F_2^x(\tau) \,d\tau \leq (t-t_{2k})a_f e^{-\lambda \frac{t+t_{2k}}{2}} \\
    & \int_{t_{2k-1}}^{2k} F_2^x(\tau) \, d\tau \leq (t_{2k}-t_{2k-1})a_f e^{-\lambda\frac{t_{2k}+t_{2k-1}}{2}} \leq  (t_{2k}-t_{2k-1})a_f e^{-\lambda(\frac{t+t_{2k}}{2})+\lambda \frac{t-t_{2k-1}}{2}} \leq c_{2k-1}e^{\lambda(t-t_0)}e^{-\lambda \frac{t+t_{2k}}{2}} \\
    & \int_{t_{2k-2}}^{t_{2k-1}} F_2^x(\tau) \, d\tau \leq (t_{2k-1}-t_{2k-2})a_f e^{-\lambda\frac{t_{2k-1}+t_{2k-2}}{2}} \leq c_{2k-2}e^{\lambda(t-t_0)}e^{-\lambda \frac{t+t_{2k}}{2}} \\
    & \dots \\
    & \int_{t_{0}}^{t_1} F_2^x(\tau) \, d\tau \leq (t_{1}-t_{0})a_f e^{-\lambda\frac{t_{1}+t_{0}}{2}} \leq c_{0}e^{\lambda(t-t_0)}e^{-\lambda \frac{t+t_{2k}}{2}} \\
\end{align}
where $c_k=(t_{k+1}-t_{k})a_f$.

When $t\in[t_2k,t_{2k+1}]$, we obtain
...
\begin{aligned}
    \tilde{z} 
    &\leq  e^{\int_{\sigma_{2i}}^{t} \hat{S}_d(\tau) \, d\tau-\bar{\chi}|S_N|}\tilde{z}(t_0) \\
   & +e^{ D_{2k}(t,2)+\int_{t_0}^{t}\hat{S}_d-\bar{\chi}\, d\tau}\int_{t_0}^{t_1}F_2^x(\tau)\, d\tau \\
   & +e^{ D_{2k}(t,3)+\int_{t_1}^{t_2}\hat{S}_d\, d\tau}\int_{t_1}^{t_2}F_2^x(\tau)\, d\tau \\
   &+ \dots \\
   & +e^{ D_{2k}(t,2k)+\int_{t_{2k-1}}^{t_{2k}}\hat{S}_d\, d\tau}\int_{t_{2k-1}}^{t_{2k}}F_2^x(\tau)\, d\tau \\
   & +e^{ \int_{t_{2k}}^{t}\hat{S}_d-\bar{\chi}\, d\tau}\int_{t_{2k}}^{t}F_2^x(\tau)\, d\tau \\
\end{aligned}
\end{equation}

\begin{equation}
    \int_{\sigma_{2i}}^{t} F_2^x(\tau)\, d\tau =  \int_{\sigma_{2i}}^{t} a e^{-\lambda \tau}\, d\tau \leq a(t- \sigma_{2i}) e^{-\frac{\sigma_{2i}+t}{2}}
\end{equation}

\begin{equation}
\begin{aligned}
 & \int_{\sigma_{2i-1}}^{\sigma_{2i}} F_2^x(\tau)\, d\tau =  \int_{\sigma_{2i-1}}^{\sigma_{2i}} a e^{-\lambda \tau}\, d\tau = (\sigma_{2i}- \sigma_{2i-1}) F_2^x({\frac{\sigma_{2i-1}+\sigma_{2i}}{2}}) \\
 & \leq   c_1 e^{T(\frac{\sigma_{2i-1}+\sigma_{2i}}{2},\frac{\sigma_{2i}+t}{2})} \int_{\sigma_{2i}}^{t} F_2^x(\tau)\, d\tau \leq c_1 e^{\lambda(t-\sigma_{2i-1})}\int_{\sigma_{2i}}^{t} F_2^x(\tau)\, d\tau
\end{aligned}
\end{equation}

\begin{equation}
\begin{aligned}
 &\int_{\sigma_{2i-2}}^{\sigma_{2i-1}} F_2^x(\tau)\, d\tau =  \int_{\sigma_{2i-2}}^{\sigma_{2i-1}} a e^{-\lambda \tau}\, d\tau = (\sigma_{2i-1}- \sigma_{2i-2}) e ^{-\frac{\sigma_{2i-2}+\sigma_{2i-1}}{2}} \\
 &\leq   a  c_2 e^{T(\frac{\sigma_{2i-2}+\sigma_{2i-1}}{2},\frac{\sigma_{2i}+t}{2})} \int_{\sigma_{2i}}^{t} F_2^x(\tau)\, d\tau \leq a c_2 e^{\lambda(t-\sigma_{2i-2})}\int_{\sigma_{2i}}^{t} F_2^x(\tau)\, d\tau
\end{aligned}
\end{equation}
where .

where $\int_{\sigma_{2i}}^{t} F_2^x(\tau)\, d\tau =a(t-\sigma_{2i})e^{-\frac{\sigma_{2i}+t}{2}}$, $c_i \leq {\rm max}(\sigma_i -\sigma_{i-1})^i$

\begin{equation}
    \tilde{z}\leq e^{\int_{t_0}^{t} \hat{S}_d(\tau) \, d\tau-\bar{\chi}|S_N|}\tilde{z}(t_0) + \int_{t_0}^{t}  c_f e^{-\frac{\sigma_{2i}+t}{2}}e^{\int_{t_0}^{t} \hat{S}_d(\tau^*) \, d\tau^*-\bar{\chi}|S_N(t_0,t)| +\lambda(t-t_0)} \, d\tau
\end{equation}
where $c_f=a_f {\rm max}({c_k})$

}



base on the Theorem 1 of \cite{yang2020}, we can obtain that
\begin{equation}
    \tilde{z}\leq e^{\int_{t_0}^{t} \hat{S}_d(\tau) \, d\tau-\bar{\chi}|S_N|}\tilde{z}(t_0) + \int_{t_0}^{t} F_{2m}e^{\int_{t_0}^{t} \hat{S}_d(\tau) \, d\tau-\bar{\chi}|S_N|} \, d\tau
\end{equation}
where $F_{2m}= \int_{t_0}^{t} F_2^x(\tau)\, d\tau$.
...
\begin{equation}
    ...
    &\leq S_d (t-t_0)+||\int_{t_0}^{t}  \tilde{S}_d(\tau) \, d\tau|| - \tilde{\chi}(t-t_0) + \tilde{\chi}(\frac{t-t_0}{\tau_a} +S_0 +(1+n_a(t_0,t))\Delta_*)\\
    &\leq S_d (t-t_0)+||\int_{t_0}^{t}  \tilde{S}_d(\tau) \, d\tau|| -\tilde{\chi}(t-t_0)
    +\frac{\tilde{\chi}}{\tau_a}(t-t_0) +\tilde{\chi}(S_0+\Delta_*) + \tilde{\chi} n_a(t_0,t)\Delta_* \\
    & \leq (S_d-\tilde{\chi}+\frac{\tilde{\chi}}{\tau_a}+ \eta^*)(t-t_0) +||\int_{t_0}^{t}  \tilde{S}_d(\tau) \, d\tau||+ \tilde{\chi}(S_0+\Delta_*)
    \end{aligned}
\end{equation}
where $S_d=I_N \otimes S$, $\tilde{S}_d={\rm blkdiag}(\tilde{S}_i)$ and $\eta^*(t-t_0)=\tilde{\chi} n_a(t_0,t)\Delta_*$.

from (40)~(41), we can obtain that
\begin{equation}
    \tilde{z} \leq c e^f +C_{c}
\end{equation}
}


{\color{black}
Define the leader dynamic estimation error as $\tilde{S}_i(t)=D_i F_i(t) E_i$ for $t \geq T_i$, where $D \in \mathbb{R}^{q \times m}$, $E \in \mathbb{R}^{n \times q}$ are known constant matrices, and $F_i(t) \in \mathbb{R}_{m \times n}$ are unsuspected matrix function meeting the bound
\begin{equation}
    F_i(t)^T F_i(t) \leq I .
\end{equation}
}
...
satisfied and there exist scalars $\alpha >  0$, $\beta > 0$ , $\chi > 0$ and
a matrix $ P > 0$ such that the conditions 1)$ P S + S^T P - \beta P +\gamma_i P D_i^T D_i P+\frac{1}{\gamma_i} E_i^T E_i +I <  0$;
2) $P S + S^T P - (\frac{\chi}{N^2})P +\alpha P +\gamma_i P D_i^T D_i P+\frac{1}{\gamma_i} E_i^T E_i +I < 0$;
3) $\alpha +\beta < \alpha \tau_{G}$ hold, then it can be guaranteed that the estimation error
...
For clarity, we first redefine the set $S_A[T_i, \infty)$ as $S_A[T_i, \infty)= \bigcup_{i=0,1,2,\dots} [\delta_{2i+1},\delta_{2i+2})$, where $\delta_{2i+1}$ and $\delta_{2i+2}$ indicate the time instants that attacks start and end, respectively.
Then, the set $S_N[T_i,\infty)$ can be redefined as $S_N[T_i,\infty)$ as $S_N[T_i, \infty)= [T_i,\delta_1)\bigcup_{i=1,2,\dots} [\delta_{2i},\delta_{2i+1})$ with $\delta_1= t_0$.

Define the following Lyapunov function
\begin{equation}
    V_1=\tilde{z}(I_N  \otimes P) \tilde{z},
\end{equation}
...
\begin{aligned}
    \dot{V_1}
    &=2 \tilde{z}^T (I_N  \otimes PS) \tilde{z}+ 2 \tilde{z}^T (I_N  \otimes P \tilde{S}) \tilde{z} -2\chi \tilde{z}^T (I_N  \otimes P) \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \tilde{z} +2 \tilde{z}^T (I_N  \otimes PS) F_2^x(t).\\
\end{aligned}
\end{equation}
\textbf{Step 1:}Firstly, we focus on the safe period $S_A(T_i,t)$.

For any $t \in S_N(T_i,\infty)$, there exists an interval $[\delta_{2i},\delta_{2i+1})$ such that
$t\in [\delta_{2i},\delta_{2i+1})$. In this case,we have
\begin{equation}
\begin{aligned}
        \dot{V_1}
    &=\tilde{z}^T (I_N  \otimes (PS+S^T P)) \tilde{z} +2 \tilde{z}^T (I_N  \otimes P \tilde{S}) \tilde{z} -2\chi \tilde{z}^T (I_N  \otimes P) \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \tilde{z} +2 \tilde{z}^T (I_N  \otimes PS) F_2^x(t)\\
    &\leq \tilde{z}^T (I_N  \otimes (PS+S^T P) + {\rm blkdiag}(\gamma_i P D_i^T D_i P) + {\rm blkdiag}(\frac{1}{\gamma_i} E_i^T E_i)) \tilde{z}\\
    &-2\chi \tilde{z}^T (I_N  \otimes P) \sum_{k \in \mathcal{L}}(\Psi_k \otimes I_p ) \tilde{z} + \tilde{z}^T\tilde{z}+ \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2.\\
\end{aligned}
\end{equation}
Then, based on the condition 2) of Theorem 2, we have
\begin{equation}
    \dot{V_1}\leq -\alpha V_1 +2 \tilde{z}^T (I_N  \otimes PS) F_2^x(t),
\end{equation}
based on the Lemma \ref{Bellman-Gronwall Lemma 2}, we can obtain that 
\begin{equation}\label{EQ21}
    V_1(t)\leq e^{-\alpha(t-\delta_{2i})} V_{1a}(\delta_{2i}) +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau .
\end{equation}

\textbf{Step 2:}Next, we consider the time interval $S_A[\delta_{2i+1},\delta_{2i+2}$  that the communication network is denied.
For any $t \in S_A(T_i,\infty)$, there exists an interval $[\delta_{2i+1},\delta_{2i+2})$ such that
$t\in [\delta_{2i+1},\delta_{2i+2})$. In this case, we have
\begin{equation}
\begin{aligned}
    \dot{V}_{1}
   & =\tilde{z}^T (I_N  \otimes (PS+S^T P)) \tilde{z} +2 \tilde{z}^T (I_N  \otimes P \tilde{S}) \tilde{z} +2 \tilde{z}^T (I_N  \otimes PS) F_2^x(t) \\
   & \leq \tilde{z}^T (I_N  \otimes (PS+S^T P) + {\rm blkdiag}(\gamma_i P D_i^T D_i P) + {\rm blkdiag}(\frac{1}{\gamma_i} E_i^T E_i) ) \tilde{z} + \tilde{z}^T\tilde{z}+ \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2,
\end{aligned}
\end{equation}
from the condition 1) of theorem 2, one has
\begin{equation}\label{EQ23}
    V_1(t)\leq e^{\beta(t-\delta_{2i+1})} V_{1b}(\delta_{2i+1}) + e^{\beta(t-\delta_{2i+1})} \int_{\sigma_{2i+1}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau .
\end{equation}


\textbf{Step 3:} Based on the above discussions, We now conclude from (\ref{EQ21}) and (\ref{EQ23}) that  

\begin{equation}
V_1(t)\leq 
\begin{cases}
 e^{-\alpha(t-\delta_{2i})} V_{1a}(\delta_{2i}) +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau ,
 t\in [\delta_{2i},\delta_{2i+1}),\\
 e^{\beta(t-\delta_{2i+1})} V_{1b}(\delta_{2i+1}) +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i+1}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau,
~~~t\in [\delta_{2i+1},\delta_{2i+2}).
\end{cases}
\end{equation}

When $t\in [\delta_{2i},\delta_{2i+1})$, we obtain 
\begin{equation}\label{EQ48}
    \begin{aligned}
      V_1(t)  
      & \leq e^{-\alpha(t-\delta_{2i})} V_{1a}(\delta_{2i}) +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau \\
      & \leq e^{-\alpha(t-\delta_{2i})} V_{1b}(\delta_{2i}^-) +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau \\
      & \leq e^{-\alpha(t-\delta_{2i})} e^{\beta(t-\delta_{2i-1})} V_{1b}(\delta_{2i-1}) \\
     & +e^{-\alpha(t-\delta_{2i})}  e^{\beta(t-\delta_{2i-1})} \int_{\sigma_{2i+1}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau \\
     & +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau \\
     & \leq \dots \\
     & \leq e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } V(t_{0}) \\
    & +\int_{t0}^{t}  e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau .\\
    \end{aligned}
\end{equation}

Similar to the case that $t\in [\delta_{2i},\delta_{2i+1})$, when $t\in [\delta_{2i+},\delta_{2i+2})$, we obtain
\begin{equation}\label{EQ49}
    \begin{aligned}
      V_1(t)  & \leq e^{\beta(t-\delta_{2i+1})} V_{1b}(\delta_{2i+1}) +e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i+1}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau, \\
      & \leq e^{\beta(t-\delta_{2i+1})} e^{-\alpha(t-\delta_{2i})} V_{1a}(\delta_{2i}) \\
     & +e^{\beta(t-\delta_{2i+1})}  e^{-\alpha(t-\delta_{2i})} \int_{\sigma_{2i+1}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau \\
     & +e^{\beta(t-\delta_{2i+1})} \int_{\sigma_{2i}}^{t} \sigma_{\rm max}(PS)^2 ||F_2^x(t)||^2 \,d\tau \\
     & \leq \dots \\
     & \leq e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } V(t_{0}) \\
    & +\int_{t0}^{t}  e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau .\\
    \end{aligned}
...
Therefore, we can conclude from (\ref{EQ48}) and (\ref{EQ49}) that
\begin{equation}
\begin{aligned}
    V_1(t) 
    & \leq e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } V(t_0) \\
    & +\int_{t_0}^{t}  e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau  , \forall t \geq t_0.
\end{aligned}
\end{equation}


%{\color{blue}
It is not difficult to obtain that $|S_N(t_0,t)|=t-t_0-|S_A(t_0,t)|$ and $|S_A(t_0,t)|\leq |S_A(t_0,t)|+(1+N_a(t_0,t))\Delta_*$ for all $t\geq t_0$, we can obtain
\begin{align}
 -\alpha|S_N(t_0,t)|+\beta|S_A(t_0,t)|
=& -\alpha(t-t_0-|S_A(t_0,t)|)+\beta|S_A(t_0,t)|  \nonumber\\
\leq& -\alpha(t-t_0)+(\alpha+\beta)|S_A(t_0,t)| \nonumber\\
\leq& -\alpha(t-t_0)+(\alpha+\beta)(\Xi_0+\frac{t-t_0}{\tau_a}+(1+n_a(t_0,t))\Delta_*) \nonumber\\
\leq& (-\alpha +(\alpha+\beta)\frac{1}{\tau_a})(t-t_0)+(\alpha+\beta)(\Xi_0+(1+n_a(t_0,t))\Delta_*) 
.
\end{align}
%
%
...
\begin{align}
V_1(t)
\leq& e^{-\alpha|S_N(t_0,t)|}e^{\beta|S_A(t_0,t)|}V(t_0)  + \int_{t_0}^{t}  e^{-\alpha \left\lvert S_N(t_0,t) \right\rvert } e^{\beta \left\lvert S_A(t_0,t) \right\rvert } \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau  \nonumber\\
=& e^{-\alpha|t-t_0-S_A(t_0,t)|}e^{\beta|S_A(t_0,t)|}V(t_0)  + \int_{t_0}^{t} e^{-\alpha|t-t_0-S_A(t_0,t)|}e^{\beta|S_A(t_0,t)|} \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau \nonumber\\
\leq&  e^{(-\alpha+(\alpha+\beta)\frac{1}{\tau_a})(t-t_0)+(\alpha+\beta)(S_0+(1+n_a(t_0,t))\Delta_*)}V(t_0) \nonumber   \\
+& \int_{t_0}^{t}  e^{(-\alpha+(\alpha+\beta)\frac{1}{\tau_a})(t-t_0)+(\alpha+\beta)(S_0+(1+n_a(t_0,t))\Delta_*)} \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau \nonumber\\
=&e^{(\alpha+\beta)(S_0+\Delta_*)+(-\alpha+\frac{\alpha+\beta}{\tau_a})(t-t_0)+n_a(t_0,t)((\alpha+\beta)\Delta_*)}V(t_0)\nonumber\\
+& \int_{t_0}^{t}  e^{(\alpha+\beta)(S_0+\Delta_*)+(-\alpha+\frac{\alpha+\beta}{\tau_a})(t-t_0)+\eta^*(t-t_0)} \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau ,
\end{align}
where $\eta^*(t-t_0)=n_a(t_0,t)((\alpha+\beta)\Delta_* > 0$, one can obtain $\frac{n_a(t_0,t)}{t-t_0}\leq F_a(t_0,t) \leq \frac{\eta^*}{(\alpha+\beta)\Delta_*}$ and $\tau_G >\frac{\alpha+\beta}{a_1-\eta^*}$, then
\begin{align}
 V_1(t)
\leq& e^{(\alpha+\beta)(S_0+\Delta_*)+(-\alpha+\frac{\alpha+\beta}{\tau_a})(t-t_0)+\eta^*(t-t_0)}V(t_0) \nonumber \\  
+& \int_{t_0}^{t}  e^{(\alpha+\beta)(S_0+\Delta_*)+(-\alpha+\frac{\alpha+\beta}{\tau_a})(t-t_0)+\eta^*(t-t_0)} \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau \nonumber\\
=& e^{(\alpha+\beta)(S_0+\Delta_*)+(-\alpha+\frac{\alpha+\beta}{\tau_a}+\eta^*)(t-t_0)}V(t_0) \nonumber \\
+& \int_{t_0}^{t}  e^{(\alpha+\beta)(S_0+\Delta_*)+(-\alpha+\frac{\alpha+\beta}{\tau_a}+\eta^*)(t-t_0)} \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau 
\nonumber\\
\leq& ce^{-\eta(t-t_0)}V(t_0) + \int_{t_0}^{t}  ce^{-\eta(t-t_0)} \sigma_{\rm max}(PS)^2 ||F_2^x(\tau)||^2 \,d\tau  , \\
\end{align}
where $c=e^{(\alpha+\beta)(S_0+\Delta_*)}$ , $\eta = \alpha-\frac{\alpha+\beta}{\tau_a}-\eta^* >0$ and $F_2^x(t)$ converges to 0 exponentially.
...
\end{aligned}
\end{equation}
Since $\tilde{x}$, $\epsilon$, $\tilde{R}_i$, $\tilde{z}$  converge to 0  exponentially which have been proofed before, it is obviously that the global output containment error $e$ converge to 0 exponentially. 
