...
Pinaki Nath Chowdhury\textsuperscript{1} \hspace{.3cm}
Subhadeep Koley\textsuperscript{1} \hspace{.3cm}\\
...
% This is extremely difficult in zero-shot, as apart from varying drastically across categories, shape itself is \textit{unknown} during inference. Addressing such challenges therefore, in this paper we aim to adapt CLIP for everything that's \textit{SBIR}, fine-grained or not, in a zero-shot setup, and hope to inspire future research in exploring CLIP towards zero-shot fine-grained sketch based applications.
% Our novel CLIP-adapted models surpass prior arts on ZS-SBIR and FG-ZS-SBIR significantly.
% Extensive experimentation and ablation on relevant benchmarks shows our model to outperform existing state-of-the-arts.
}
\end{abstract}
...
% Although research advanced using complex strategies \cite{sain2022sketch3t} and frameworks \cite{tian2022tvt}, the zero-shot capability of such models remained restricted to a smaller scope, barring them from achieving zero-shot capabilities at the open-vocab scale, which we aim to achieve in this work.

%, hinting at adapting such foundation models for downstream tasks like retrieval
% In this regard of zero-shot generalisation potential, parallel research on large-scale vision-language pre-training \cite{CLIP-Adapter,clip-continual2022,DenseCLIP,luo2021clip4clip} has achieved remarkable performance in image recognition \cite{maple2022} using foundation models like CLIP\cite{clip} -- models pre-trained on simple tasks like text-image mapping, over enormous data. Thanks to its highly enriched latent space, holding knowledge across a myriad of cross-modal data, recent methods have attempted to adapt CLIP to task-specific models \cite{actionclip2021} to impart generalisability. Following suit we aim to adapt CLIP for ZS-SBIR in this work.

% which has been recently used for imparting generalisability to task-specific models \cite{actionclip2021}.
% Recent literature harps on the custom that using large foundation models should be task-agnostic for the problem at hand such that common hurdles of scarcity of training data, or computational resources for downstream tasks can be avoided. In other words, re-training such a large model, would defeat the core purpose of being already trained on such a large distribution -- \textit{i.e.} its generalisation potential. 
% Using CLIP for zero-shot SBIR is therefore the only logical step towards future research on sketch based applications, which we attempt here as our first contribution.

% \usetikzlibrary{patterns}
\begin{figure}[!t]
\centering
\includegraphics[width=0.35\linewidth]{figs/CLIP-teaser-left.pdf}
% \vspace{-0.4cm}
\begin{tikzpicture}
    \begin{axis}[
        ybar,
        bar width=8pt,
        % bar interval shift=0.0,
        % bar interval width=0.0,
        width = 2.2cm,
        height= 2.5cm,
        ...
        ymin=0.45, ymax=0.75, 
        xtick={ 1, 2, 3},
        xticklabels={Text, SBIR, Ours },
        xticklabel style={rotate=0,font=\scriptsize},
        ytick= {0.4, 0.5, 0.6, 0.7},
        yticklabels={0.4, 0.5, 0.6, 0.7},% {31.9, 25.9, 27.8, 14.3},
        yticklabel style={ xshift=0.1cm, font=\scriptsize},
        axis y line=center,
        axis x line=middle,
        ymajorgrids=true,
        xmajorgrids=true,
        ...
        \addplot [ draw= orange, fill = orange!40]
        coordinates { (1,0.521) (2,0.579) (3,0.723) }; 
    \end{axis}
\end{tikzpicture}
\begin{tikzpicture}
    \begin{axis}[
        ...
        xlabel near ticks,
        ylabel near ticks,
        % xlabel={\large{(c) \% Methods}},
        ylabel={\scriptsize{Acc@1 (\%)}},
        xmin=0.5, xmax=4.5,
        ymin=15, ymax=37, 
        xtick={ 1.5, 3},
        xticklabels={SOTA, Ours},
        xticklabel style={rotate=0,font=\scriptsize},
        ytick= {20, 25, 30, 35},
        yticklabels= {20, 25, 30, 35}, % {31.9, 25.9, 27.8, 14.3},
        yticklabel style={ xshift=0.1cm, font=\scriptsize},
        axis y line=center,
        axis x line=middle,
        ymajorgrids=true,
        ...
        \addplot [ draw= blue, fill = blue!20]
        coordinates { (1.5,22.6) (3,28.68)};
        % \addplot [black,
        % nodes near coords= 22.6,
        % every node near coord/.style={font=\scriptsize}]
        % coordinates {(1.5, 23.2)};
        % \addplot [black,
        % nodes near coords= 28.68,
        % every node near coord/.style={font=\scriptsize}]
        % coordinates {(3, 23.2)};
    \end{axis}
\end{tikzpicture}
...
%, thus ensuring a computationally economic FG-SBIR model. }
\label{fig:teaser}
\vspace{-0.6cm}
\end{figure}
% \footnote{FLOPS = \textit{FLO}ating \textit{P}oint operation\textit{S} }

%\footnote{Using sketch as anchor, a paired photo as positive and a random non-matching one as negative, triplet loss aims at bringing a sketch and its paired photo (positive) closer while distancing it from a random non-matching photo}
% Learning two prompts (sketch and photo) therefore preserves the knowledge of CLIP in addition to providing exposure on our very own sketch-specific datasets \cite{sangkloy2016sketchy,quickdraw}.

% As seen from \cref{fig:teaser}, with advent of CLIP, retrieval via keyword-based query, in the form of template like : \textit{photo of a `category'} has come up close to that of existing state-of-the art SBIR pipelines. Our CLIP-adapted paradigm however, beats existing performances by a high margin of $\approx$20-25\%, demonstrating ample scope of future research in adapting CLIP for sketch-based applications.
...
% \blue{The paradigm of prompt learning essentially learns a vector of size $\mathbb{R}^{K\times d}$ such that it induces large foundation models like CLIP to adapt to downstream sketch-photo distributions and simultaneously preserve the desired generalisability of CLIP. Ours takes a further step ahead of prompt learning to additionally fine-tune the LayerNorm parameters to improve performance. This is analagous to domain adaptation techniques that only train BatchNorm/LayerNorm parameters.}
% Recent literature shows optimal solution to come via learning a \textit{prompt} \cite{jia2022visual} specific to the task, while keeping the entire foundation model frozen during training, thus preserving CLIP's knowledge, while learning from task-specific (sketch \cite{sangkloy2016sketchy,quickdraw} in our case) datasets.
...
% and injected to CLIP's image encoder (frozen) over a standard triplet loss \cite{yu2016sketch} on extracted sketch-photo features (\cref{fig:teaser}). 
Motivated by the efficacy of training batch normalisation for image recognition \cite{frankle2021batchNorm}, we additionally fine-tune a small subset of trainable parameters of every Layer Normalisation (LN) layer for additional performance gain.
% Only using CLIP's Image encoder however, ignores the additional open-vocab zero-shot potential of CLIP's text encoder. 
{Furthermore, to enhance cross-category semantic transfer, we also resort to CLIP's text encoder further cultivating its zero-shot potential. In particular, in addition to the said \textit{visual} prompts onto the CLIP's image encoder, we use handcrafted \textit{text} prompts via templates like `\textit{photo of a [category]}' to its text encoder, during training. }
% We specifically focus on hand-crafted templates as learned text prompts were found to be not rooted in natural language \cite{zhou2022conditional}, thereby failing our motivation to distill from natural language for high-level generalisation to unseen (open-vocab) categories.}
% \blue{
% Lastly, to further cultivate the zero-shot potential of open-vocab pre-trained CLIP's text encoder \cite{clip}, we use it for our class-discrimination objective.
% %
% Following recent literature we take to handcrafted text prompts \cite{anonymous2023rethinking} -- the input format to CLIP's text encoder, such as `\textit{photo of a [category]}').
% % we use it to extract features from handcrafted prompts (`\textit{photo of a [category]}'), distilling its open-vocab enriched image-text semantic correspondence towards our retrieval accuracy.
% % origin of the prompt lies in the natural language space
% }

% Furthermore, refusing to ignore the extra zero-shot potential of open-vocab pre-trained CLIP's text encoder \cite{clip}, we use it to extract features from handcrafted prompts as : `\textit{photo of a \<category\>}', to distill its semantically rich open-vocab image-text correspondence towards our retrieval accuracy.
% Alternatively, although one may train via an appended MLP layer to obtain certain accuracy, it does not guarantee CLIP's generalisation potential to remain preserved as once again, the model gets adapted to smaller data distribution. 
% As recent literature dictates, the optimal solution comes via learning a prompt specific to the task, keeping the entire foundation model frozen during training. Following recent works, we design two sets of prompts shared across categories -- one for learning photo representation and the other for sketch which are used with the frozen image encoder of CLIP during training, over a standard triplet loss \cite{yu2016sketch}. In pursuit of higher performance, we further fine-tune a small subset of trainable parameters of every layer normalisation (LN) layer , motivated by prior observation on unusual efficacy of training batch normalisation for image recognition \cite{}. However using only image encoder of CLIP without its text encoder would be limiting our zero-shot performance being deprived of natural language supervision \cite{}. Accordingly we create handcrafted templates by filling up standard templates with category names as : `\textit{photo of a \<category\>}', which are passed via CLIP's text encoder to excersize a classification objective, thus distilling CLIP's generalisation potential into our learnable prompts for efficient ZS-SBIR.

% Having dealt with ZS-SBIR at category-level, an obvious query comes up: why not extend CLIP to the more realistic cross-category fine-grained setting ? Attempting this in conjunction with category-level, we advocate for a design that adapts \textit{CLIP for everything SBIR}, fine-grained (FG) or otherwise.
% While off-the-shelf CLIP itself has zero-shot image retrieval potential where someone can feed the category level query as ‘a photo of a [query]’, it raises a question – how much is a category-level query-sketch beneficial over text-keyword based query? Attending to sketch’s specialty in modelling fine-grained details hence, as our second contribution we go beyond category-level ZS-SBIR to a more practical and long-standing research problem of cross-category zero-shot FG-SBIR.
...
% Due to the diverse shape morphology existing across categories, instance-wise matching based on recognising structurally similar sketch-photo cues becomes extremely difficult, especially for \textit{unseen} classes as their shape is itself \textit{unknown}.

% The fundamental difference between category-level SBIR and category-level FG-SBIR lies in (i) sketch-photo-negative mining for triplet loss. Unlike SBIR where the paired photo and negative one belong to different category, FG-SBIR require all three (sketch, positive and negative) to come from the same category. (ii) A classification head to learn the class discriminative knowledge. Following our adaptation of CLIP to ZS-SBIR, we extend it for FG-ZS-SBIR by replacing the category-level triplet with hard triplets per category, and using CLIP's text encoder for class-discriminative objective. However we are obstructed with two major bottlenecks in this regard. Firstly the margin hyperparameter, is quite category-specific with respect to FG-SBIR -- in that it varies significantly across categories as seen from recent literature. Secondly, unlike category-level SBIR that focus on semantics alone, due to the diverse shape morphology existing across categories it becomes extremely challenging during instance-wise matching of FG-SBIR, to recognise fine-grained associations for \textit{unseen} classes whose shape is \textit{unknown}. 
It follows that for the first challenge, we propose a new regularisation term that aims at making the relative sketch-photo feature-distances uniform across categories, such that a single (global) margin parameter works across all of them. Specifically, taking the distribution of relative distances for all sketch-positive-negative hard-triplets \cite{yu2016sketch} in a category, we aim to minimise the KL-divergence \cite{matthews2016sparse} between every pair of distributions, which trains the model towards making such relative sketch-photo distances uniform across categories.
%
% Attending to the first challenge we first explore the margin hyperparameter. We realise that using a fixed margin across categories signifies that on average the relative sketch-photo distances across categories are similar, which is far from reality. However, for a \textit{truly} generalisable cross-category FG-SBIR paradigm we need to use the same margin value across categories, or else using specific margin value per category breaks the zero-shot cross-category perspective where sketch-photo distance distributions are unknown during inference. Alleviating this we propose a impose a regularisation that aims to make this relative distance uniform across categories such that the same triplet loss, with single (global) margin parameter works across categories. Specifically taking the distribution of relative distances for all sketch-positive-negative triplets in a category, we aim to minimise the KL-divergence of such distributions between every pair of categories, which effectively progresses towards making such relative sketch-photo distances uniform across categories.
%
For the latter, we propose a clever patch shuffling technique, where equally divided patches of a sketch and its \textit{corresponding} photo (\textbf{n}$\times$\textbf{n}) are first{ shuffled following a random permutation order of patches. We then advocate that a shuffled sketch should be closer to a shuffled photo having the same permutation order, but far from that of a different permutation.}
% .                                                                         bringing a shuffled sketch closer to the shuffled paired-photo following the same permutation as that of sketch, while distancing it from another shuffled photo following a different permutation, would instill permutation-invariant knowledge, thus imparting a broad notion of structural correspondences thus helping fine-grained understanding.
% randomly shuffled before feature extraction. Training thereafter decreases feature-distance of a sketch's permutation from the \textit{same permutation} of its paired photo, while increasing it from a different one. 
...
% Being directly dealt with sketches instead of a middle-ground like edgemaps, structural similarity of photos and sketches are  more consistently learned by the model which helps it perform better in associating more structurally with sketches during zero-shot inference.

% Summing up:
% (i) We for the first time adapt CLIP for the practical paradigm of ZS-SBIR -- both category-level and fine-grained, via novel-prompt design for each. (ii) We propose a novel patch-shuffling objective in learning the structural correspondences between sketch-photo pairs, and a regularisation strategy towards making relative sketch-photo feature distances across categories uniform. (iii) Most importantly, providing a thorough analysis of the challenges and opportunities via extensive experiments and ablation, we encourage future research towards using foundation models for fine-grained sketch-based applications in the wild (zero-shot). Our novel CLIP-adapted models exceed prior arts on both ZS-SBIR and FG-ZS-SBIR (\cref{fig:teaser}) significantly. 
% % 
...
% based on CNNs, RNNs~\cite{xu2018sketchmate} or Transformers~\cite{ribeiro2020sketchformer}\cut{ or their combination}, 
% content dropped : pre-training strategies like mixed modal jigsaw solving ~\cite{pang2020solving}
...
%  out of them two works --> in context of generalization  to novel classed for fg-sbir -- has been address by Pang et al cvpr 2020 and adaptive fgsbir eccv 2020. While Pang et al -- handles this by leraning a coodebook which is used to embed the sketches from novel classes for zs- fg-sbir. Bhunia et al takes an adaptive approach to generalise to new class using a fewshot sketch-photo pairs examples. Nevertheless, we here aim to addres sthe zero=shot FG-SBIR using foundation model like CLIP. 

% content cut : considering both visual and semantic information -- thus minimising the sketch photo domain gap
%without any paired-data of unseen classes
...
% thus go beyond such usual approaches leveraging

% Another work \cite{tian2022tvt} employs a three-way transformer setup with two modality-specific and one shared ViT~\cite{dosovitskiy2021image}, to use the shared ViT to predict the sketch or photo specific output feature, for ZS-SBIR.
% Further reducing the distribution gap between training and testing data, Sketch3T\cite{Sketch3T}
% employs a test-time training paradigm using self-reconstruction on test-set sketches, to adapt the trained model to the test set. 

% Add the one liner of sketch3t
% utilizing the graph convolution network, which simultaneously considers both the visual information and the semantic information to reduce the domain gap. AAAI20
% ignoring the semantic supervision -- distill the constrative relationshing (using contrastive KD) in the embedding space from pretarined imagenet teacher ACMM21
% coupled sekcth/photo encoider with shared conv layers and indepndnent batchnorm layer --TPMAI2022
% Prototypebased Selective Knowledge Distillation (PSKD) for ZS-SBIR based on learned  correlation matrix ACMMM-22
% three way visual transformer-- sk, photo , common 0sketch/photo -- where the where the common sketch/photo VIT is used to predict the sketch or photo specific output feature , thus misning the ksetch photo domain gap. 
...
% unlike those two which keep it frozen.

% linear classifier

% \textbf{(ii) Prompt-learning baselines:}
% \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ *

% \keypoint{Cross-category FG-ZS-SBIR:}  We compare against:
% \textbf{(i) State-of-the-art}: \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ *
% \textbf{(ii) Prompt Learning baselines:} \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ * \\ *
...
Our CLIP adapted paradigm easily surpasses the two existing SOTAs on cross-category FG-ZS-SBIR (\Cref{tab:main_table} right).
% Motivated by the high performance on ZS-SBIR, we emphasise on studying the realistic setup of cross-category FG-ZS-SBIR on sketches from Sketchy \cite{sangkloy2016sketchy} dataset. From \cref{tab:main_table} we make the following observations:
% Although CLIP-based methods outperform the two SOTAs (\Cref{tab:main_table} right), there is still room for improvement.
While CLIP models \cite{DenseCLIP} shows impressive performance at category-level ZS-SBIR, there is reasonable scope for improvement in FG-ZS-SBIR that additionally requires structural matching\cite{pang2020solving}.
Relatively higher improvement of \textit{B-FG-MM} over \textit{B-FG-IP} \cite{zhou2022learning} than its category-level counterpart (\textit{B-MM} over \textit{B-IP}), suggests the efficacy of multi-modal prompts over independent sketch/photo prompts, in FG-SBIR. This supports the prior observation \cite{yu2016sketch} that sharing encoder parameters is more suited to FG-SBIR \cite{sain2020cross} whereas separate sketch/photo weights work best at category-level. Lastly, learning a conditional prompt \cite{zhou2022conditional} in \textit{B-FG-Cond} offers marginal gain due to its sensitivity to training strategies \cite{maple2022, zhou2022conditional}. 
% Future works can further examine a stable adaptation of conditional prompt for sketch applications.
...
\centering
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            legend style = {font=\scriptsize, draw=none,fill=none, at={(-0.1, 1.5)}}, % ,at={(1.15, -0.02)}
            scale only axis=true,
            width = 3.5cm,%0.45\linewidth,
            height= 1.8cm,
            xmin=0, xmax=10.2,
            ymin=0, ymax=0.75,             
            % xticklabels={$|$,,,,,,, $\leftarrow \text{Seen} \rightarrow$ ,,,,,,,,$|$,,,,, $\leftarrow  \text{Unseen}  \rightarrow$,,,, $|$},                       
            xtick={ 0, 1, 3, 5, 7, 10},
            xticklabels={, 10\%, 30\%, 50\%, 70\%, 100\%},
            xticklabel style={rotate=0,font=\scriptsize},
            ytick= {0.25,0.50,0.75},
            yticklabels={0.25,0.50,0.75},%{5,15,25},
            ymajorgrids=true,
            % xmajorgrids=true,
            grid style=dashed,
            axis y line=center,
            axis x line=bottom, % middle
            ylabel={\scriptsize{mAP@200}},
            yticklabel style={ xshift=0.1cm, font=\scriptsize},
            % xlabel={$\leftarrow \;\; \quad \text{Seen} \;\; \quad \rightarrow |\leftarrow \text{Unseen} \rightarrow$},
            xlabel={ZS-SBIR}, % \makecell{Training\\Data}
            % xlabel near ticks,
            xlabel style={at={(0.50, -0.17)}, font =\scriptsize},
            extra x tick labels={Cut},
        ]                              
        \addplot[color= red, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{ (1, 0.084) (3, 0.126) (5, 0.228) (7, 0.365) (10, 0.422) };
                      \addlegendentry{B-Lin} 
        \addplot[color= orange, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{  (1, 0.184) (3, 0.381) (5, 0.512) (7, 0.649) (10, 0.702) };
                      \addlegendentry{B-Deep}                      
        \addplot[color= newcolor, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (1, 0.010) (3, 0.056) (5, 0.124) (7, 0.259) (10, 0.369) };
                      \addlegendentry{ZS-GRL}                      
        \addplot[color= deepGreen, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (1, 0.383) (3, 0.546) (5, 0.648) (7, 0.701) (10, 0.730) };
                      \addlegendentry{Ours}        
        \end{axis}
    \end{tikzpicture}    
    \hspace{-0.2cm}
    \begin{tikzpicture}
        ...
        \addplot[color= red, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{ (1, 1.0) (3, 2.1) (5, 3.8) (7, 11.12) (10, 15.75) };
                      \addlegendentry{B-FG-Lin} 
        \addplot[color= orange, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{  (1, 6) (3, 8) (5, 14) (7, 22) (10, 27.0) };
                      \addlegendentry{B-FG-Deep}                      
        \addplot[color= blue, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (1, 2) (3, 3.5) (5, 6.2) (7, 15.32) (10, 22.42) };
                      \addlegendentry{CC-DG}                      
        \addplot[color= deepGreen, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (1, 10) (3, 14) (5, 20) (7, 26) (10, 28.68) };
                      \addlegendentry{Ours}        
        \end{axis}
    \end{tikzpicture}
    \hspace{-0.2cm}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            % xticklabels={$|$,,,,,,, $\leftarrow \text{Seen} \rightarrow$ ,,,,,,,,$|$,,,,, $\leftarrow  \text{Unseen}  \rightarrow$,,,, $|$},                       
            xtick={ 0, 2,4,6,8, 10},
            xticklabels={classes$>$, 20, 40, 60, 80, 104},
            xticklabel style={rotate=0,font=\scriptsize},
            ytick= {0.25,0.50,0.75},
            yticklabels={0.25,0.50,0.75},%{5,15,25},
            ymajorgrids=true,
        ...
        \addplot[color= red, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{ (2, 0.024) (4, 0.116) (6, 0.255) (8, 0.365) (10, 0.422) };
                      \addlegendentry{B-Lin} 
        \addplot[color= orange, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{  (2, 0.150) (4, 0.360) (6, 0.542) (8, 0.640) (10, 0.700) };
                      \addlegendentry{B-Deep}                      
        \addplot[color= newcolor, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (2, 0.010) (4, 0.056) (6, 0.124) (8, 0.207) (10, 0.369) };
                      \addlegendentry{ZS-GRL}                      
        \addplot[color= deepGreen, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (2, 0.323) (4, 0.521) (6, 0.638) (8, 0.690) (10, 0.730) };
                      \addlegendentry{Ours}        
        \end{axis}
    \end{tikzpicture}  
    \hspace{-0.2cm}
    \begin{tikzpicture}
        ...
        \addplot[color= red, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{ (2, 1.5) (4, 2.1) (6, 4.8) (8, 11.12) (10, 15.75) };
                      \addlegendentry{B-FG-Lin} 
        \addplot[color= orange, mark = *, mark size=0.4mm, thick, smooth]
        coordinates{  (2, 4) (4, 10) (6, 18) (8, 24) (10, 27.0) };
                      \addlegendentry{B-FG-Deep}                      
        \addplot[color= blue, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (2, 0.7) (4, 3.5) (6, 8.2) (8, 16.32) (10, 22.42) };
                      \addlegendentry{CC-DG}                      
        \addplot[color= deepGreen, mark = *, mark size=0.4mm, thick, smooth] % mark = *, mark size=0.4mm,
        coordinates{ (2, 7) (4, 16) (6, 23) (8, 26.5) (10, 28.68) };
                      \addlegendentry{Ours}        
        \end{axis}
    \end{tikzpicture}
    \vspace{-0.35cm}
...
% Justifying our use of CLIP in FG-ZS-SBIR therefore, it paves the way for sketch applications towards open-vocab scenario.
...
% This is mainly because the \textit{learned} language prompt encode meanings that are \textit{beyond} the existing vocabulary \cite{zhou2022learning} thus failing to exploit the high generalisation ability that natural language brings \cite{mikolov2013efficient, clip}.
% An alternative to creating handcrafted prompt templates is to learn a language prompt 

% that are injected in the first ViT \cite{dosovitskiy2021image} transformer layers
...
% En-armed with our powerful sketch-photo encoder(s) for ZS-SBIR and cross-category FG-ZS-SBIR we compare SBIR with keyword-based retrieval, 
...
% We observe (i) for category-level, keyword-based  retrieval is at $0.523/0.612$ mAP/P@200 whereas SBIR \cite{dey2019doodle,sain2022sketch3t} reaches $0.723/0.725$. (ii) for fine-grained, keyword-query acquires $4.6\%$ in Acc.@1 whereas SBIR \cite{sain2020cross,yu2016sketch} achieves $18.68\%$. This shows that while query-sketches and texts are comparable at category-level, sketch provides a notable performance rise over text for fine-grained retrieval. We hope this motivates future research towards cross-category FG-ZS-SBIR to explore fine-grained cues of sketches, in open-vocab scenario.
...
% \red{In this paper we put forth a strong baseline for FG-SBIR with PVT\cite{wang2021pyramid}-backbone, and offer a novel paradigm that at its core aims at learning from unlabelled data in FG-SBIR by distilling knowledge from unlabelled photos.  While our proposed intra-modal triplet loss increases feature separation in model's latent space, an EMA paradigm stabilises its training. Importantly, we for the first time introduce a distillation token in PVT architecture that explicitly caters to knowledge distillation. Extensive experiments against existing frameworks and various baselines show our method to outperform them, thus proving its significance.}
% {This work addresses the most fundamental nature of sketches, first time from a practical perspective of compute -- sketches are quite sparse in nature. Accordingly we have modelled a \textit{fast} FG-SBIR framework that specifically exploits this trait to gain advantage in computational efficiency. As our first contribution we put forth a shallow network which is incapable of performing like existing state-of-the-arts on its own, but performs at par, when trained via our knowledge distillation strategy. More importantly, we also design a Smart Canvas selector which leverages stroke level vector information from input sketches, exploiting their sparse nature to recommend a minimal canvas size that in turn gains computational speed and is optimal in keeping the performance at par. Extensive experiments and ablation show the significance of method in pushing existing state-of-the-art FG-SBIR models towards deployment.}
