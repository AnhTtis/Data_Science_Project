...
\begin{figure}[!hbt]
    ...
    \caption{Qualitative results of ZS-SBIR on Sketchy \cite{liu2017deep} by a {baseline} (\textcolor{myBlue}{blue}) method vs {Ours} (\textcolor{deepGreen}{green}). }
    \label{fig:qual-SBIR}
    \vspace{-0.4cm}
\end{figure} 
...
\begin{figure}[!hbt]
    ...
    \caption{Qualitative results of FG-ZS-SBIR on Sketchy \cite{sangkloy2016sketchy} by a {baseline} (\textcolor{myBlue}{blue}) method vs {Ours} (\textcolor{deepGreen}{green}). The images are arranged in increasing order of the ranks beside their corresponding sketch-query, i.e the left-most image was retrieved at rank-1 for every category. The true-match for every query, if appearing in top-5 is marked in a green frame. Numbers denote the rank at which that true-match is retrieved for every corresponding sketch-query.}
