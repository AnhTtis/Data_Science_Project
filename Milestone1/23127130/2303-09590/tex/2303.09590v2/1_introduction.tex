...
Multivariate networks~\cite{kerren2014multivariate} consisting of both topological and semantic information can model complex relations between entities.
...
Existing visual analytics methods~\cite{fujiwara2020visual,fujiwara2022network,martins2012multidimensional,martins2017mvn,vandenelzen2016reducing,song2022interactive} utilize NRL methods to learn networks' \textit{general} representations---overall summaries of networks (e.g., variance of node degrees).
% without a certain analysis focus.
...
On the other hand, as seen in the machine learning field, NRL using neural networks (NNs) can learn representations specific to an analysis focus by using appropriate loss functions~\cite{hamilton2017inductive}.
...
To support effective analysis of the associations, we develop a new density scatterplot, named \textit{two-class density scatterplot}, that can clearly depict a trend of data distribution as well as inform the mixture and separation of two classes.
...
Moreover, we validate the usability of the workflow through an expert interview.
...
\begin{compactitem}
    ...
    \item a mechanism of composite variable construction that aids in attribute selection and attribute weight optimization for  interpretations with an influence from multiple attributes; and
