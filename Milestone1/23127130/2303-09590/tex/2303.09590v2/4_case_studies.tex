...
Each weight's sign agrees with the corresponding attribute's positive or negative influence on the 1D representation, which is observed in \autoref{fig:ui}-a.
...
Thus, having both openness and introversion tends to have clear negative impacts on their score levels.
...
% Even though similar information to the attribute combining \texttt{open} and \texttt{extra} could be captured by the network centralities such as node degree (e.g., the students with high \texttt{extra} could have more friends), as mentioned, the centralities do not have clear influences on the 1D representation.
We want to investigate another question raised during the above analysis: why network centralities were not listed as highly contributed attributes even though
the composite variable, --\,0.9\,\texttt{open} +\,0.4\,\texttt{extra}, shows a clear contribution to the 1D representation.
...
From the distributions of \texttt{neuro} shown in \autoref{fig:case1}-b1 and b2, the students with high (--\,0.9\,\texttt{open} +\,0.4\,\texttt{extra}) or high $\texttt{extra}$ tend to have lower \texttt{neuro} than others. 
...
This result emphasizes the importance of interpretation using a mix of multiple attributes.
...
\autoref{fig:case2}-f shows the derived top 8 contributing attributes, where red and blue colors are now used for low and high \texttt{eigenvector} groups. 
...
Meanwhile, the listed attributes in \autoref{fig:case2}-f tend to correspond with the minimum values of their friends', as indicated with the prefix, ``\texttt{min\_}''.
...
As taking the minimum of 1-hop neighbor statistics was problematic,  we should adjust the feature extraction parameters to examine these groups' differences.
...
We then perform the classification of these classes, resulting in accuracies of 1.0 and 0.82 for Steps 2 and 3.
