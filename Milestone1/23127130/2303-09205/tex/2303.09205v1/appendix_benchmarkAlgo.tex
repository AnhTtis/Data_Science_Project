\subsection{Analysis of Algorithm \ref{algo:bench-algo}}\label{appx:bench-algo}


\begin{lemma}\label{lem:tau=rho}
If we have a budget of $B$ comparisons, then the stopping time $\tau$ of Algorithm \ref{algo:bench-algo} is such that 
\begin{enumerate}
...
% \paragraph{11}
    ...
    By definition of $\rho_\ell$,  $x_{\rho_\ell} \geq \best G^2_{\rho_\ell-1} = \best G^2_{\rho_0} \geq \best G^1_{\rho_0}$. Thus, the algorithm must terminate on $\rho_\ell$ contradicting our assumption that $\rho_\ell < \rho_b = \tau$. \\
...
we will need concentration inequality for controlling the cardinals of the sets $G^1_t$ and $G^2_t$ for $t \in \{s, \ldots, N\}$.

\begin{theoremEnd}{lemma}\label{lem:conc-card}
...
\end{theoremEnd}
\begin{proofEnd}
% \nic{Up to one remaining comment the proof seems correct to me. I will polish it later.}
    We apply Lemma~\ref{lem:hoeff-max} with $\delta = 1/s$, first for the random variables $(\indic{x_t \in G^1} - \lambda)_{r\leq t \leq N}$, then for the random variables $(\lambda - \indic{x_t \in G^1} )_{s \leq t \leq N}$. A union bound then gives
    \begin{align*}
        \Prob\left(
    ...
    \end{align*} 
    where $\Phi : x>0 \mapsto \frac{(1 + \sqrt{x})^2}{4\sqrt{x}}$. A direct computation gives $\sqrt{\dfrac{\Phi(1/ \w)}{2}} =  \dfrac{1+\sqrt{\w}}{2\sqrt{2}\w^{1/4}}$.
    %We can write $\phi(x) = \frac{1}{4}\left( x^{1/4} + \frac{1}{x^{1/4}} \right)^2$, thus $\sqrt{\Phi(1/ \w)/2} = A_\w$. 
\end{proofEnd}
...
\begin{align*}
 ...
 \end{align*}
Let $i \in \{s, \ldots, N\}$. Conditionally on the event that $t^\star=i$, one can check that the equalities $\tau = \rho_1 = i$ hold if and only if none of the candidates between times $s$ and $i{-}1$ are ranked first among the observed candidates inside their groups. Moreover, the latter event depending solely on the relative ranks of the first $i{-}1$ candidates, it is independent of the event that the $i$-th candidate is the best overall. Consequently, we can write
\begin{align*}
...
Let us place ourselves on the event $\mathcal{C}_{s,N}$. Since the sequence $(|G^1_j|)_j$ is non-decreasing,
%\begin{align*}
...
The following step is to estimate the sum appearing in the previous Lemma. We do that using results from Section \ref{sec:comp}.
...
\subsection{Lower bound of the success probability of Algorithm \ref{algo:bench-algo}}\label{appx:lower-bound-benchalgo}
\begin{proof}[Proof of Corollary \ref{cor:success_proba_bound_budget}]
For any $\w \in (0,1)$, set
\[
P^B_{\infty}( \w) = \w^2 \sum_{b=0}^B \left( \frac{1}{\w} - \sum_{\ell=0}^{b} \frac{\log(1/ \w)^\ell}{\ell !}   \right)\enspace.
\]
In Lemma \ref{lem:sum-exp-remainder} we show that
\begin{align*}
\w\log(1/ \w) - P^B_{\infty}( \w)
= \w^2\left( \frac{1}{\w}\log(1/ \w) - \sum_{b=1}^{B+1} 
   \left( \frac{1}{\w} - \sum_{\ell=1}^{b-1} \frac{\log(1/ \w)^\ell}{\ell !}   \right) \right)
\leq \frac{\w \log(1/ \w)^{B+2}}{(B+1)!}\enspace.
\end{align*}
Using Theorem \ref{thm:lb-PB(s,N)} and the inequality above with $\w = 1/\e$ we deduce that
\begin{align*}
\frac{1}{\e} - \sup_{\w \in (0,1)}P^B_N(\w)
\leq \frac{1}{\e} - P_B(N/\e,N)
&\leq \frac{1}{\e} - P^B_{\infty}(1/\e) + K_{\lambda, \frac{1}{\e}, B} \sqrt{\frac{\log N}{N}}\\
&\leq \frac{1}{\e(B+1)!}  + K_{\lambda, B} \sqrt{\frac{\log N}{N}}\enspace,
\end{align*}
where $K_{\lambda, B} = K_{\lambda, \frac{1}{e}, B}$ as defined in Theorem~\ref{thm:lb-PB(s,N)}.
% \[
% K_{\lambda, \frac{1}{e}, B}
