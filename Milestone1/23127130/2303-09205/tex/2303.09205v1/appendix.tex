...
\section{Proofs}
Here we collect the proofs that were omitted from the main body of the paper.
...
Let $I,J$ are two disjoint subsets of $\{1,\ldots,N\}$. If $X=\{x_i\}_{i\in I}$ and $Y = \{x_j\}_{j\in J}$ then the event $\{\max X < \max Y\}$ is independent from the inside rankings of $X$ and $Y$: $\pi(X)$ and $\pi(Y)$ (See Definition \ref{def:inrank}). Moreover 
\[
\Prob(\max X < \max Y)
= \frac{|Y|}{|X| + |Y|}.
\]
...
\begin{proof}
The event $\{\max X < \max Y\}$ can be written as $\{\forall x \in X: \,x < \max Y\}$, which is independent of the inside-ranking of $X$, because reshuffling $X$ does not have any impact on the event.\\ 
...
\begin{lemma}\label{lem:rank_indep_unif}
    For any positive integer $t$, conditionally to $G^1_t, G^2_t$, the ranks $(r_j)_{j=1}^t$ are mutually independent. Moreover,  the conditional distribution of the rank $r_t$ knowing $\lvert G_t^{g_t}\rvert$ is uniform over the set $\{1, \dots, \lvert G_t^{g_t}\rvert\}$.
\end{lemma}
\begin{proof}
Since the candidates arrive in a uniformly random order, then the rank of each candidate is a uniform random variable in $\{1, \ldots, N\}$. Moreover, any subset of $\{x_1, \ldots, x_N)$ is also uniformly shuffled. In particular, if $g_t = g$, this is the case for $G^g_t$, $r_t$ is therefore a uniform random variable in $\{1, \ldots, |G^g_t|\}$. It is independent of the relative ranks of the past candidates in $G^g_{t-1}$, because all the possible relative ranks of $G^g_{t-1}$ are equally likely, independently of where $x_t$ stands among them.
\end{proof}

We remind the Hoeffding's maximal inequality.
