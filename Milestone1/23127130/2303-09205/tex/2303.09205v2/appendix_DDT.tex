\section{Analysis of DDT algorithms}\label{appendix:analysisDDT}

\subsection{DDT with $B=0$: Proof of Theorem \ref{thm:prob-win0}}\label{appx:DDT-0}
...
For $t \in \{\beta_0 N, \ldots, N\}$, $\A(\alpha_0, \beta_0)$ stops on $t$ if $x_t$ is the best candidate of its group so far, i.e $r_t = 1$. For $\A(\alpha_0, \beta_0)$ to reach step $t$ and not stop before it, we must have $\best G^1_{t-1} \in G^1_{\alpha_0 N - 1}$ and $\best G^2_{t-1} \in G^2_{\beta_0 N - 1}$, which means that no candidate before $x_t$ surpasses the maximum of its group from the exploration phase, and conditioning on $t = t^\star$, the previous condition guarantees that the algorithm will stop on $t$, therefore
\begin{align*}
\Prob(\tau = t \mid t = t^\star,  \mathcal{C}_{\alpha_0})
...
\end{align*}
proceeding as we did earlier in the proof, we obtain that
...
Combining Equations \eqref{eq:prob0-A20}, \eqref{eq:prob1-A20}, \eqref{eq:prob2-A20} and \eqref{eq:prob3-A20} we obtain that
\begin{align*}
\Prob(\A(\alpha_0, \beta_0)& \text{ succeeds})\\
...
\subsection{DDT with $B \geq 0$: Proof of Theorem \ref{thm:asymptoticDDT} and associated Lemmas}\label{appx:DDT-B}
We will first show in the following Proposition that the result is true for $B=0$, then with the help of other lemmas, we will prove the recursion theorem.


\begin{proposition}\label{prop:asymptoticDDT0}
For any $\alpha_0, \beta_0 \in (0,1)$, and any $w \in (0,1)$, $\varphi_0(w; (\alpha_0, \beta_0))$ defined in Eq~\eqref{eq:phi_function} exists.
% \[
...
\begin{align*}
\Prob(\tau = t \mid t = t^\star) 
&= \Prob(\tau = t \mid t = t^\star, \mathcal{C}_{\alpha_0})(1 - O(1/N))\\
&= \frac{\alpha_0 \beta_0 N^2}{t^2} + O\left( \sqrt{\frac{\log N}{N}}\right),
\end{align*}
...
\[
\Prob(\rho_1 \geq t)
= \Prob(\rho_1 \geq t \mid \mathcal{C}_{\alpha_B})(1 - O(1/N))\enspace,
\]
...
\end{align*}
since $\alpha_B N \leq t \leq N$ we have that $\alpha_B N/t \leq 1$ and thus
\begin{align*}
\Prob(\rho_1 \geq t)
...
\end{align*}
since $\alpha_B N / t^2 = O(1/N)$, we deduce that
\begin{align*}
\Prob(\rho_1 = t)
...
&= \frac{\alpha_B \beta_B N^2}{t^2} + O\left( \sqrt{\frac{\log N}{N}}\right)\enspace,
\end{align*}
and also
\begin{align*}
\Prob(\rho_1 = t \mid \mathcal{C}_{\alpha_B})
...
&= \frac{2\alpha_B \beta_B N^2}{t^3} + O\left( \sqrt{\frac{\log N}{N^3}} \right)\enspace,
\end{align*}
we deduce the non-conditional probabilities $\Prob(\rho_1 \geq t), \Prob(\rho_1 = t)$ as in the case $\alpha_B \leq t \leq \beta_B$.
\end{proof}


\subsubsection{Success probability knowing $\rho_1$}
Using Lemma \ref{lem:rho1=t}, we now estimate the success probability of the algorithm conditionally to $\rho_1$. For any  $0 \leq b \leq B$ we denote $\A^b$ the algorithm $\A((\alpha_\ell, \beta_\ell)_{\ell=0}^b)$ with the initial budget $b$, $\rho_1^b$ the first time when $\A^b$ uses a comparison, and for any $1 \leq t \leq N$ we define
\[
...
\begin{proof}
Let $t^\star$ be the position of the best candidate overall.
...
= \Prob(\A^{B-1} \text{ succeeds} \mid \rho^{B-1}_1 \geq t+1)
= \rd^{B-1}_{N,t+1}\enspace,
\]
in fact, if $\rho^B_1 = t$ and $R_t \neq 1$ then $\A^B$ uses a comparison but does not stop and moves to the next candidate, the state of $\A^B$ at step $t+1$ is therefore exactly the same as that of $\A^{B-1}$ if the latter still has not used any comparison. Finally, using the Bayes rule again gives
\[
\Prob(R_t \neq 1 \mid \rho_1 = t)
...
Let us now consider $\beta_B N \leq t \leq N$. Since $\rho_1$ can belong to any of the two groups, conditionally to $\{ t = t^\star \}$ we have that $\{ \rho_1 = t \} = \{ \rho_1 \geq t \}$, which is independent of the event $\{ t = t^\star \}$ because it it is measurable with the respect to the relative ranks of the first $t-1$ applicants. Using this argument and Lemma \ref{lem:rho1=t} gives
\begin{align*}
\frac{\Prob(\rho_1 = t \mid t = t^\star)}{\Prob(\rho_1 = t)}
&= \frac{\Prob(\rho_1 \geq t \mid t^\star = t)}{\Prob(\rho_1 = t)}
= \frac{\Prob(\rho_1 \geq t)}{\Prob(\rho_1 = t)}\\
&= \frac{\frac{\alpha_B \beta_B N^2}{t^2} +  O\left( \sqrt{\frac{\log N}{N}}\right)}{ \frac{2 \alpha_B \beta_B N^2}{t^3} +  O\left( \sqrt{\frac{\log N}{N^3}}\right)}\\
&= \frac{t}{2}\left(1 + O\left( \sqrt{\frac{\log N}{N}} \right)\right),
\end{align*}
in the same way, conditionally to $\{R_t = 1\}$, we have that $\{ \rho_1 = t \} = \{ \rho_1 \geq t \}$, and the latter is independent of $\{R_t = 1\}$, thus 
\begin{align*}
\frac{\Prob(\rho_1 = t \mid R_t = 1)}{\Prob(\rho_1 = t)}
&= \frac{\Prob(\rho_1 \geq t \mid R_t = 1)}{\Prob(\rho_1 = t)}
= \frac{\Prob(\rho_1 \geq t)}{\Prob(\rho_1 = t)}\\
&= \frac{t}{2}\left(1 + O\left( \sqrt{\frac{\log N}{N}} \right)\right),    
\end{align*}
we deduce that
...
+ O\left( \sqrt{\frac{\log N}{N}}\right),
\end{align}
consequently, using Lemma \ref{lem:rho1=t} to estimate $\Prob(\rho_1 \geq wN)$ and substituting into Equation \eqref{eq:lemU-0}
\begin{align*}
\rd^B_{N,wN}
&= \frac{1}{\Prob(\rho_1 \geq wN)}
    \sum_{t=wN}^{N} \Prob(\A^B \text{ succeeds} \mid \rho_1 = t)\Prob(\rho_1 = t)\\
...
+ O\left( \sqrt{\frac{\log N}{N}}\right),
\end{align*}
and using Equation \eqref{eq:lemU-1} for $\beta_B$ instead of $w$ we get
\begin{align*}
...
+ O\left( \sqrt{\frac{\log N}{N}}\right),
\end{align*}
finally, using Lemma \ref{lem:rho1=t} to estimate $\Prob(\rho_1 \geq wN)$ and by Equation \eqref{eq:lemU-0}
...
We remind this classical result on Riemann sums.
...
\subsection{Lower bound on the success probability: Proof of Proposition \ref{prop:lb-optDDT} }\label{appx:lb-optDDT}

The lower bound we obtain on the success probability is derived by considering the cases where all the thresholds are equal to the same value --giving a lower bound in $B$--, and the case where $B=0$ --giving a lower bound in $\lambda$.

\begin{proposition}
Let $B\geq 0$ and $\w \in (0,1)$. If we choose all the thresholds from $0$ to $B$ equal to $\w$, then the asymptotic success probability of the unique threshold algorithm $\A^B(\w) \eqdef \A^B((\w,\w)_{b=0}^B)$ is
\[
\lim_{N \to \infty}
\Prob\big( \A^B(\w) \text{ succeeds} \big)
= \w^2 \sum_{b=0}^B \left( \frac{1}{\w} - \sum_{\ell=0}^{b} \frac{\log(1/ \w)^\ell}{\ell !}   \right)\enspace.
\]
\end{proposition}


\begin{proof}
Let $\w \in (0,1)$ and let us denote for simplicity $\phi_B(w;\w) \eqdef \phi_B\big(w;(\w,\w)_{b=0}^B\big)$ for any $w \in (0,1)$. The asymptotic success probability of $\A^B(\w)$ is $\phi_B(0;\w)$, which is equal to $\phi_B(\w;\w)$, because no comparison can be made before exploring a fraction $\w$ of the candidates. By Theorem \ref{thm:asymptoticDDT} we have for any $B\geq 0$ and $w \geq \w$ that 
\begin{align}
\phi_B(w;\w) \nonumber
&= g_B(w,w)\\ \label{eq:gB-rec}
&= g_0(w,w) + w^2\int_{w}^1 \frac{\phi_{B-1}(u;\w)}{u^3}du
= w(1-w) + w^2\int_{w}^1 \frac{g_{B-1}(u,u)}{u^3}du
\enspace. 
\end{align}
We will show by induction over $B$ that for any $w \in [\w,1)$ we have
\begin{align*}
g_B(w,w)
&= w^2 \sum_{b=0}^B \left( \frac{1}{w} - \sum_{\ell=0}^{b} \frac{\log(1/w)^\ell}{\ell !}   \right)\\
&= w^2 \sum_{b=0}^B \sum_{\ell=b+1}^{\infty} \frac{\log(1/w)^\ell}{\ell !}  
\enspace.
\end{align*}
For $B=0$ we have immediately for any $w$ that $g_0(w,w) = w^2(1/w - 1)$. Let $B\geq 1$ and assume that the result is true for $B-1$, and let $w \geq \w$. We have by Equation \eqref{eq:gB-rec} and by the induction hypothesis
\begin{align*}
g_B(w,w)
&= w(1-w) + w^2\int_{w}^1 \frac{g_{B-1}(u,u)}{u^3}du\\
&= w(1-w) + w^2\sum_{b=0}^{B-1} \sum_{\ell=b+1}^{\infty} \int_w^1  \frac{\log(1/u)^\ell}{\ell ! u}du  \\
&= w^2(\exp(\log(1/w))-1) + w^2\sum_{b=0}^{B-1} \sum_{\ell=b+1}^{\infty} \frac{\log(1/w)^{\ell+1}}{(\ell+1)!}du \\
&= w^2\sum_{\ell = 1}^{\infty} \frac{\log(1/w)^{\ell}}{\ell !}
+ w^2\sum_{b=1}^{B} \sum_{\ell=b+1}^{\infty} \frac{\log(1/w)^{\ell}}{\ell!}du\\
&= w^2\sum_{b=0}^{B} \sum_{\ell=b+1}^{\infty} \frac{\log(1/w)^{\ell}}{\ell!}du \enspace.
\end{align*}
This concludes the induction. In particular, for any $B \geq 0$ and for $w = \w$ we obtain that
\begin{align*}
\lim_{N \to \infty} \Prob\big( \A^B(\w) \text{ succeeds} \big)
&= \varphi_B(0,\w)
= g_B(\w, \w)\\
&= \w^2\sum_{b=0}^{B} \sum_{\ell=b+1}^{\infty} \frac{\log(1/\w)^{\ell}}{\ell!}du \\
&=  \w^2 \sum_{b=0}^B \left( \frac{1}{\w} - \sum_{\ell=0}^{b} \frac{\log(1/\w)^\ell}{\ell !}   \right) \enspace.
\end{align*}

\end{proof}

\begin{corollary}\label{cor:lb-unique-thresh}
With the same notations as in the previous Proposition, we have for any $\w \in (0,1)$
\[
\lim_{N \to \infty}
\Prob\big( \A^B(1/e) \text{ succeeds} \big)
\geq \frac{1}{e} - \frac{1}{e(B+1)!}\enspace.
\]
\end{corollary}

\begin{proof}
Using the previous proposition and Lemma \ref{lem:sum-exp-remainder} for $\log(1/\w)$ we obtain for any $B \geq 0$ and $\w \in (0,1)$
\begin{align*}
\w\log(1/ \w) - \phi_B(0, \w)
&= \w^2\left( \frac{1}{\w}\log(1/ \w) - \sum_{b=1}^{B+1} 
   \left( \frac{1}{\w} - \sum_{\ell=1}^{b-1} \frac{\log(1/ \w)^\ell}{\ell !}   \right) \right)\\
&\leq \frac{\w \log(1/ \w)^{B+2}}{(B+1)!}
\enspace,
\end{align*}
choosing $\w = 1/e$ yields the result.
\end{proof}

Finally, using the previous corollary, we only need to demonstrate the lower bound in $\lambda$ to complete the proof of Proposition \ref{prop:lb-optDDT}. We do so by comparing the success probability to the case of zero budget.

\begin{proof}[Proof of Proposition \ref{prop:lb-optDDT}]
Since $(\alpha^\star_b, \beta^\star_b)_{b=0}^B$ are optimal thresholds we have that the limit success probability of the algorithm is higher than the success probability of any other DDT algorithm, in particular, if we choose all the thresholds equal to the same value $1/e$
\begin{align*}
\lim_{N \to \infty} \Prob( \A_2((\alpha^\star_b, \beta^\star_b)_{b=0}^B) \text{ succeeds})
&\geq \phi_B(0,1/e)\\
&\geq \frac{1}{e} - \frac{1}{e(B+1)!}\enspace,
\end{align*}
where the last lower bound is deduced from Corollary \ref{cor:lb-unique-thresh}. Assuming that $\lambda \geq 1/2$, we also have by Proposition \ref{prop:asymptoticDDT0} that
\begin{align*}
