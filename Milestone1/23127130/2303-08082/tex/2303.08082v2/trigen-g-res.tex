...
\draw[edge,-o-={0.2}{<}] (60:2.8) node[left]{$n$}
	arc[radius=1.2,start angle=-120,end angle=-60] node[right]{$\sigma_2(n)$};
\draw[edge,-o-={0.2}{<}] (60:2.2) node[left]{$\bar{k}$}
	arc[radius=1.8,start angle=-120,end angle=-60] node[right]{$\sigma_2(\bar{k})$};
...
\draw[edge,-o-={0.2}{<}] ({2.9-0.3*\x},0)
	...
	coordinate (D\x);
\path (D-1)node[right]{$\sigma_2(i+1)$} (D1)node[right]{$\sigma_2(i+j)$};
