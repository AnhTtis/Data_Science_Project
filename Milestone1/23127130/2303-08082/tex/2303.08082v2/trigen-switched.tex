\begin{tikzpicture}[baseline=0cm]
\draw[wall] (0,0) -- (4.8,0) -- (60:4.8) -- cycle;

\foreach \x in {0.8,1.6,2.4,3.2,4.0}
\draw (4.8,0)++(120:\x) edge (4.8-\x,0) -- (60:\x) -- (\x,0);

\tikzmath{\half=0.4/sqrt(3);\full=2*\half;\more=\full+0.05;}

\begin{scope}[edge]
\draw[->] (4.8,0) ++ (120:1.2) coordinate(g1s)
	-- ++(-150:\half) -- ++(150:\full) -- ++ (-150:\full)
	-- ++(150:\full) -- ++(90:\full) -- ++(150:\full)
	-- ++(90:\full) coordinate(B)
	++(150:\full) -- ++(90:\full) -- ++(150:\half)coordinate(g2);
\path (g1s)++(30:0.45)node{$p'_{l+1}$} (g2)++(150:0.4)node{$p'_{l+1}$};

\draw[blue,->] (4.8,0) ++ (120:2.8) coordinate(g2s)
	-- ++(-150:\half) -- ++(150:\full) -- ++(-150:\full)
	++(150:\full) -- ++(-150:\full) -- ++(150:\half)coordinate(g1);
\path[blue] (g2s)++(30:0.3)node{$p'_l$} (g1)++(150:0.3)node{$p'_l$};

\draw[blue] (B) ++ (-90:0.05) -- ++(150:\more);
\draw (B) ++ (30:0.05) -- ++(150:\more);
\end{scope}
\end{tikzpicture}