...
\begin{align}
    Q &= 8\pi \phi_0^2\, \omega \lambda^{3} \, \int_0^\infty \dd \rho\, \rho^2 f^2\,=\frac{4 \pi  \kappa }{d^2} \, \int_0^\infty \dd \rho\, \rho^2 f^2\,, \\
...
Note that while $E< m_\phi Q$ indeed guarantees soliton stability, solitons with $E> m_\phi Q$ can still be stable at the \emph{classical} level as long as $\dd Q/\dd \omega <0$~\cite{Friedberg:1976me}. These solitons decay only through quantum effects and can hence be metastable, indicated in Fig.~\ref{fig:PlotEtaVsKappaCritical}.
...
From the above $E(Q)$ expression it is obvious that the solitons are stable against dispersion once the charge exceeds a critical value~\cite{Friedberg:1976me},
\begin{align}
...
Even without Coleman's thin-wall limit, the FLS solitons form stable objects as long as their charge exceeds a minimal value.
...
Numerical data files can be found as ancillary files on the arXiv page of this article.
