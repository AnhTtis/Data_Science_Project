...
\begin{figure}
  ...
  \includegraphics[width=\linewidth]{figure_intro_3.pdf}
   \caption{
   BoxSnake is a box-supervised instance segmentation model that predicts the segmentation of the interested object in the form of polygons. Three losses, involving a point-based unary term and two pairwise terms, are proposed to constrain the predicted polygon to fit the object boundary. The grey dotted line indicates that the proposed losses only works during training.
   }
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=0pt,parsep=0pt]
    \item We design a novel end-to-end training technique to approach polygonal instance segmentation with only box supervision for the first time.
    \item We propose a point-based unary loss which regularizes the predicted polygon to objects using box-based IoU.
    \item We propose a distanced-based pairwise loss, involving local and global terms, to encourage the predicted polygon to fit object boundaries. More importantly, we build a method that converts the polygon regression problem into a classification problem, thus enabling the pairwise loss. 
\end{itemize}
...
\rowcolor{Gray}
BoxSnake         & R101-FPN & $\mathcal{C}$                     & \textbf{31.6}                 & \textbf{54.0}                 & \textbf{8.3} \\ \Xhline{1.2pt}
\end{tabular}
}
...
\rowcolor{Gray}
BoxSnake  & R50-FPN             & $\mathcal{C}$    & \textbf{26.3}                                        & \textbf{54.2}    \\ \Xhline{1.2pt}
\end{tabular}}
...
\begin{figure*}[t]
  ...
  \includegraphics[width=\linewidth]{vis_coco_6.pdf}
   \caption{
Comparison of visualization on COCO $\mathrm{val2017}$ set. $\mathcal{L}_{u}$, $\mathcal{L}_{lp}$ and $\mathcal{L}_{gp}$ refer to the unary loss (Eq.~\ref{eq:unary_loss}), the local pairwise loss (Eq.~\ref{eq:local_pairwise_loss}) and the global pairwise loss (Eq.\ref{eq:gloabl_pairwise_loss}), respectively.  The pairwise losses enable the predicted polygon to fit boundaries well.
}
   \label{fig:coco_vis}
   \vspace{-3mm}
\end{figure*}
...
CIoU~\cite{CIoU}       & 23.9 & 48.8   & 21.3   & 12.4  & 24.6  & 34.4 \\ \Xhline{1.2pt}
\end{tabular}
}
...
Clipping Strategy & 31.1 & 53.4   & 31.3   & 14.6  & 33.5  & 46.7 \\ \Xhline{1.2pt} 
\end{tabular}
}
...
ellipse        & 31.1 & 53.4   & 31.3   & 14.6  & 33.5  & 46.7  \\ \Xhline{1.2pt}
\end{tabular}
}
...
$5\times5$  & 1        & 30.9 & 53.2   & 30.9   & 14.4  & 33.0    & 46.3 \\ \Xhline{1.2pt}
\end{tabular}
}
...
\multicolumn{3}{c|}{ellipse mask}                                     & \multicolumn{1}{c|}{15.5} & \multicolumn{1}{c}{39.4} & \multicolumn{1}{c|}{10.1} & \multicolumn{1}{c}{9.5} & \multicolumn{1}{c}{16.3} & \multicolumn{1}{c}{21.5} \\ \hline
  ...
  $\checkmark$   & $\checkmark$       & $\checkmark$      & 31.1 & 53.4   & 31.3   & 14.6  & 33.5  & 46.7 \\\Xhline{1.2pt}
\end{tabular}}
...
\rowcolor{Gray}
BoxSnake & Swin-L   & M-RCNN~\cite{MaskRCNN} & $\mathcal{C}$  & \textbf{39.5} & \textbf{66.8} & \textbf{39.9} \\ \Xhline{1.2pt}
\end{tabular}}
...
1.0    & 30.5 & 52.2   & 30.5   & 13.1  & 23.9  & 45.5 \\ \Xhline{1.2pt}
\end{tabular}}
...
0.1   & 30.4 & 52.7   & 30.6   & 13.5  & 32.3  & 46.0  \\ \Xhline{1.2pt} 
\end{tabular}}
...
0.5    & 29.9 & 52.6   & 29.6   & 13.6  & 32.2  & 45.0  \\ \Xhline{1.2pt}
\end{tabular}}
...
\begin{tabular}{l|c|cc|ccc}
\Xhline{1.2pt}
local pairwise & $\mathrm{AP}$          & $\mathrm{AP_{50}}$      & $\mathrm{AP_{75}}$      & $\mathrm{AP_{S}}$       & $\mathrm{AP_{M}}$       & $\mathrm{AP_{L}}$ \\ \Xhline{1.2pt}
CRF Loss~\cite{on_regularization}       & 30.2 & 53.2 & 30.2 & 13.9  & 32.1 & 45.3  \\
% \rowcolor[HTML]{ECF4FF}
$\mathcal{L}_{lp}$ (Eq.~\ref{eq:local_pairwise_loss})          & 31.1 & 53.4 & 31.3 & 14.3  & 33.4 & 46.4  \\ \Xhline{1.2pt}
\end{tabular}}
