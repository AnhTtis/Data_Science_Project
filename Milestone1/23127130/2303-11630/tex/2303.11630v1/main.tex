...
$^2$Tencent AI Lab \\
{\tt\small \{r-yang20@mails, li.xiu@sz\}.tsinghua.edu.cn} \quad
{\tt\small \{ronnysong, yixiaoge\}@tencent.com}
}
...
The source code will be available at \url{https://github.com/Yangr116/BoxSnake}.
\end{abstract}
...
\begin{figure}
  ...
  \includegraphics[width=\linewidth]{figure_intro_3.pdf}
   \caption{
   BoxSnake is a box-supervised instance segmentation model that predicts the segmentation of the interested object in the form of polygons. Three losses, involving a point-based unary term and two pairwise terms, are proposed to constrain the predicted polygon to fit the object boundary.  
   }
...
Thus, we design a point-based unary loss relying on CIoU loss~\cite{CIoU} to constrain the bounding box of the predicted polygon, maximizing its Intersection-over-Union (IoU) with the annotation box.
As shown in Figure~\ref{fig:diagram_loss}~(b) and Figure~\ref{fig:coco_vis}~(b), since the point-based unary loss only optimizes the outermost vertices of the predicted polygon, it can roughly regress to the object of interest but is hard to fit the boundary well.
...
To achieve it, we introduce a hard mapping function based on the curve evolution~\cite{GAC, osher1988fronts} method to transform the 2D polygon into a 3D plane, as illustrated in Figure~\ref{fig:diagram_relaxation}~(b), which maps the pixels in the interior and exterior of the polygon to two separated level sets.
%
...
Compared with the mask-based weakly-supervised counterparts, our method can further narrow the performance gap between the predicted segmentation and the bounding box.
With ResNet-50 backbone, our method obtains $3.9\%$ absolute gains over the BoxInst~\cite{BoxInst} on Cityscapes dataset and shows significant superiority over some fully-supervised methods on COCO dataset, including DeepSnake~\cite{DeepSnake} and PolarMask~\cite{PolarMask}.
%
...
\begin{figure}
  ...
  \includegraphics[width=\linewidth]{diagram_loss.pdf}
   \caption{
   Diagram of impacts of the proposed losses. (a) indicates the initial polygon sampled from an ellipse enclosed by the bounding box. (b) denotes the predicted polygon supervised by the point-based unary loss only. (c) is the predicted polygon jointly supervised by the point-based unary loss and the distance-aware pairwise loss jointly.
   }
...
In the context of deep learning, many arts~\cite{BBTP, BoxInst, Discobox, BoxLevelSet, A2GNN} tried to perform mask-based instance segmentation with just bounding-box annotations. BBTP~\cite{BBTP} converted the box tightness prior~\cite{BoxPrior} as the latent ground truth via multiple instance learning (MIL) and employed the structural constraint to maintain the piece-wise smoothness in predicted masks. BoxInst~\cite{BoxInst} achieved stunning instance segmentation results by substituting the mask loss with two carefully designed regularization terms in CondInst~\cite{CondInst}. The first one was the projection term, also based on MIL, which minimizes the discrepancy between projections of the predicted mask and the ground-truth box. The latter was the pairwise term which encourages the spatially neighboring pixels with a similar color to have the same label. DiscoBox~\cite{Discobox} further leveraged cross-image correspondence to improve segmentation performance. The above methods can be summarized as a CRF energy model~\cite{CRF}, where the unary potential is responsible for finding the initial instance mask (seeds) and the pairwise potential for label propagation. Similar appearance models~\cite{ShapeMask, CPMask} are also applied in the partially supervised instance segmentation. Moreover, based on the Chan-Vese level set energy function~\cite{ChanVeseLevelSet}, BoxLevelSet~\cite{BoxLevelSet} evolved the instance mask through low-level image features and tree-filter~\cite{TreeFilter, TreeFilterV2} refined high-level features within the object bounding box. By contrast, we in this paper formulate a method to train the polygon-based instance segmentation frameworks with only box annotations.
...
\end{equation}
%
where $\underset{k}{\mathring{\Omega}}(i,j)$ means the adjacent pixels within a $k\times k$ window at the position $(i, j)$. $w_{[(i,j), (p, q)]}$ measures the affinity of two pixels by color distance:
%
\begin{equation}
...
\begin{figure}
  ...
  \includegraphics[width=\linewidth]{diagram_polygon_head.pdf}
   \caption{
   The diagram of the network architecture of BoxSnake. The multi-scale features are extracted from the input image by a backbone network. The box predictor is attached to these features to obtain bounding boxes. The polygon head predicts the polygon for each box, which is trained with box annotation only.
   }
...
\rowcolor{Gray}
BoxSnake         & R101-FPN & $\mathcal{C}$                     & \textbf{31.6}                 & \textbf{54.0}                 & \textbf{8.3}                
\end{tabular}
}
\caption{Comparisons with classical instance segmentation methods on COCO $\mathrm{val2017}$ set. All models are trained with the $1\times$ schedule. $\Delta\text{AP}_b$ indicates the accuracy gap between the predicted bounding box and segmentation. $\mathcal{M}$ and $\mathcal{C}$ denote the segmentation formats being mask and polygon, respectively. With just box supervision, our BoxSnake achieves better or comparable performance to mask-based methods. Our model even surpasses fully supervised methods, such as PolarMask and Deep Snake.}
...
method         & backbone              & out & $\text{AP}$ & $\text{AP}_{50}$ \\ \Xhline{1.2pt}
...
\rowcolor{Gray}
BoxSnake  & R50-FPN             & $\mathcal{C}$    & \textbf{26.3}                                        & \textbf{54.2}                                  
\end{tabular}}
\caption{Results on Cityscapes validation set. $\mathcal{M}$ and $\mathcal{C}$ denote the segmentation formats being mask and polygon, respectively. DLA-34 refers to the backbone used in \cite{CenterNet}. The reported results of BoxInst are obtained from the official repository~\cite{tian2019adelaidet}. BoxSnake outperforms other mask-based instance segmentation methods with only box annotations.}
...
\begin{figure*}[t]
  ...
  \includegraphics[width=\linewidth]{vis_coco_6.pdf}
   \caption{
   Comparison of visualization on COCO $\mathrm{val2017}$ set.
   }
   \label{fig:coco_vis}
\end{figure*}
%-------------- figure
...
\begin{figure}[t]
  ...
  \includegraphics[width=\linewidth]{vis_cityscapes_2.pdf}
   \caption{
   Comparison of visualization on Cityscapes validation set. The major difference is marked by the green rectangle, best viewed in color.
   }
...
unary loss & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
Dice on $P_3$ & 19.3 & 43.6   & 14.5   & 7.3   & 19.9  & 30.6  \\
GIoU~\cite{GIoU}       & 23.7 & 48.6   & 20.7   & 11.9  & 24.4  & 33.7  \\
CIoU~\cite{CIoU}       & 23.9 & 48.8   & 21.3   & 12.4  & 24.6  & 34.4 
\end{tabular}
}
...
\resizebox{\columnwidth}{!}{
\begin{tabular}{l|c|cc|ccc}
method         & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
...
\caption{Ablation study for clipping strategy on COCO $\mathrm{val2017}$ set. '$P_3$' and '$P_2$' denote that the predicted polygon is scaled to the size of $P_3$ and $P_2$, respectively. The listed fully-supervised methods adopt the same architecture and hyper-parameters as BoxSnake. The clipping strategy (\S~\ref{sec:pairwise_loss}) can bring a noticeable improvement in both fully-supervised and box-supervised scenarios.}
...
size & dilation & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
$3\times3$  & 1        & 30.8 & 53.3   & 30.8   & 13.4  & 33.0    & 46.5  \\
$3\times3$  & 2        & 31.1 & 53.4   & 31.3   & 14.6  & 33.5  & 46.7  \\
$5\times5$  & 1        & 30.9 & 53.2   & 30.9   & 14.4  & 33.0    & 46.3 
\end{tabular}
}
...
\resizebox{\columnwidth}{!}{
\begin{tabular}{l|c|cc|ccc}
initial method & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
...
$\mathcal{L}_u$ & $\mathcal{L}_{lp}$ & $\mathcal{L}_{gp}$ & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
...
\multicolumn{3}{c|}{ellipse mask}                                     & \multicolumn{1}{c|}{15.5} & \multicolumn{1}{c}{39.4} & \multicolumn{1}{c|}{10.1} & \multicolumn{1}{c}{9.5} & \multicolumn{1}{c}{16.3} & \multicolumn{1}{c}{21.5} \\ \hline
  ...
  $\checkmark$   & $\checkmark$       & $\checkmark$      & 31.1 & 53.4   & 31.3   & 14.6  & 33.5  & 46.7 
\end{tabular}}
...
method           & backbone & architecture   & out & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ \\ \Xhline{1.2pt}
...
\rowcolor{Gray}
BoxSnake & Swin-L   & M-RCNN~\cite{MaskRCNN} & $\mathcal{C}$  & \textbf{39.5} & \textbf{66.8} & \textbf{39.9} 
\end{tabular}}
\caption{Comparisons with state-of-the-art methods on COCO $\mathrm{test}$-$\mathrm{dev}$ set. $\mathcal{M}$ and $\mathcal{C}$ denote the output formats being mask and polygon, respectively. Our BoxSnake predicts polygon with box supervision, which achieves comparable performance to mask-based methods.}
\label{tab:comp_on_coco2017test-dev}
\end{table}
% ----------------------tab:comparision_on_coco_test end---------------------------------- 

\section{Conclusion}
This paper introduces a new end-to-end training technique for weakly-supervised instance segmentation based on polygons, utilizing only box annotations. Our method integrates a point-based unary loss and a distance-aware pairwise loss. The former maximizes the Intersection-over-Union between the circumscribed box of the predicted polygon and its ground-truth box, thereby making the predicted polygons tight with the target objects. The latter encourages that the predicted polygons are better to fit the object boundary and are robust to the local noise. BoxSnake achieves competitive performance on both COCO and Cityscapes datasets, making an effective polygon-based instance segmentation with solely box supervision for the first time.


\appendix

\section{Details about Distance Relaxation}
...
We conduct additional ablation studies for hyper-parameters on COCO $\mathrm{val2017}$ set.

% --------------------------------------------------------------------------------------
\subsection{Weights of Pairwise Terms.}
$\beta$ and $\gamma$ determine weights between local and glbal pairwise terms. As reported in Table~\ref{tab:ablation_beta} and Table~\ref{tab:ablation_gamma}, $\beta$ being $0.5$ and $\gamma$ being $0.03$ obtain the best performance.
...
$\beta$ & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$  \\ \Xhline{1.2pt}
...
1.0    & 30.5 & 52.2   & 30.5   & 13.1  & 23.9  & 45.5 
\end{tabular}}
...
\begin{tabular}{c|c|cc|ccc}
$\gamma$ & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
...
0.1   & 30.4 & 52.7   & 30.6   & 13.5  & 32.3  & 46.0   
\end{tabular}}
...
\resizebox{0.85\columnwidth}{!}{
\begin{tabular}{c|c|cc|ccc}
$\tau$ & AP   & $\text{AP}_{50}$ & $\text{AP}_{75}$ & $\text{AP}_{S}$ & $\text{AP}_{M}$ & $\text{AP}_{L}$ \\ \Xhline{1.2pt}
...
0.5    & 29.9 & 52.6   & 29.6   & 13.6  & 32.2  & 45.0 
\end{tabular}}
...
\begin{figure*}[t]
  ...
  \includegraphics[width=1.0\linewidth]{more_vis_cityscapes_2.pdf}
   \caption{
   More visualization on Cityscapes validation set. The major difference between BoxInst (left)~\cite{BoxInst} and our BoxSnake (right) is marked by the green rectangle, best viewed in color.
   }
...
We visualize the curve evolution in Figure~\ref{fig:vis_curve_evolution}. It demonstrates that the curve progresses towards greater accuracy from the first decoder to the fourth decoder. Additionally, in Figure~\ref{fig:more_vis_cityscapes}, we show more qualitative comparison with BoxInst~\cite{BoxInst} on Cityscapes validation set.
