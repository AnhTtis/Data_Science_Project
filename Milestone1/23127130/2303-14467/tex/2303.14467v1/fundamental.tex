...
Goldberg~\cite{goldberg1984finding} then gave in 1984 an improved version of the above algorithm in terms of time complexity.
...
However, Charikar's algorithm is based on a totally different idea from the
maximum-flow ones and it is of independent interest; moreover, it enables us to see the $2$-approximation algorithm, which will be described later in Section \ref{subsec:approx},
as a primal-dual algorithm for DSP.
In the rest of this section we will first review Goldberg's maximum-flow-based algorithm,
Charikar's LP-based algorithm, and an algorithm based on submodular function minimization.
...
Strictly speaking, the algorithm we describe is slightly different from Golberg's one, but the difference is not essential and it is just for the sake of simplicity of presentation.

The algorithm strategy is as follows:
it maintains upper and lower bound on the (unknown) optimal value of the problem,
and tightens the bounds step-by-step using the binary search, until the lower bound at hand is guaranteed to be the optimal value of DSP.
To update upper and lower bounds, the algorithm utilizes maximum-flow computation.
As initial upper and lower bounds, for example, $m/2$ and $0$, respectively, can be employed.

Let $\beta \geq 0$ be the midpoint of the upper and lower bounds kept in the current iteration.
For $G=(V,E)$ and $\beta \geq 0$, the algorithm constructs the following edge-weighted directed graph $(U,A,w_\beta)$:
\begin{itemize}
\item $U=V\cup \{s,t\}$;
\item $A=A_s\cup A_E\cup A_t$
\begin{itemize}
\item $A_s=\{(s,v)\mid v\in V\}$,
\item $A_E=\{(u,v), (v,u)\mid \{u,v\}\in E\}$,
\item $A_t=\{(v,t)\mid v\in V\}$;
\end{itemize}
\item $w_\beta \colon A\rightarrow \mathbb{R}_+$
\begin{align*}
w_\beta(e) =
\begin{cases}
...
\end{align*}
\end{itemize}
For reference, this graph is illustrated in Figure~\ref{fig:digraph}\footnote{This figure and part of the presentation is borrowed from Section 4.2.2 of a Japanese textbook~\cite{Soma22} co-authored by Atsushi Miyauchi, one of the authors of this survey.}.
...
\end{align*}
\end{proof}
From this lemma, we see the following:
Let $(X,Y)$ be the minimum $s$--$t$ cut of $(U,A,w_\beta)$ computed by the algorithm.
If $X= \{s\}$ does not hold,
then $S=X\setminus \{s\}$ is a vertex subset that satisfies $\beta|S|-e[S]\leq 0$, i.e., $\frac{e[S]}{|S|}\geq \beta$;
hence, the lower bound on the optimal value can be replaced by $\beta$.
On the other hand, if $X=\{s\}$, then we see that there is no $S\subseteq V$ that satisfies $\frac{e[S]}{|S|}> \beta$;
...
\begin{align*}
\left| d(S_1)-d(S_2)\right|\geq \frac{1}{n(n-1)}.
\end{align*}
\end{mylemma}
\begin{proof}
...
The following theorem guarantees the solution quality and time complexity of the algorithm.
\begin{algorithm}[t]
...
\begin{mytheorem}
Let $T_\mathrm{Flow}$ be the time complexity required to compute a minimum $s$--$t$ cut $(X^{(i)},Y^{(i)})$ using maximum-flow computation ($i=0,1,\dots$).
Then Algorithm~\ref{alg:flow} returns an optimal solution to DSP in $O(T_\mathrm{Flow}\log n)$ time.
\end{mytheorem}
\begin{proof}
...
For example, employing the maximum-flow computation algorithm by Cheriyan, Hagerup, and Mehlhorn~\cite{Cheriyan96},
we can compute a minimum $s$--$t$ cut of $(U,A,w_{\beta^{(i)}})$ in $O\left(\frac{|U|^3}{\log|U|}\right)=O\left(\frac{n^3}{\log n}\right)$ time.
Therefore, in that case, the time complexity of Algorithm~\ref{alg:flow} becomes $O(n^3)$.

In the context of maximum-flow-based exact algorithms for DSP, we would like to remark that Gallo, Grigoriadis, and Tarjan~\cite{gallo1989fast} showed how a single parametric maximum flow computation provides an optimal solution to DSP.
...
Next we review an LP-based exact algorithm for DSP, designed by Charikar~\cite{Charikar2000}.
...
\begin{mylemma}\label{lem:OPT_LP}
...
It should be noted that Balalau et al.~\cite{balalau2015topkoverlapping} proved that $\{v\in V\mid y^*_v >0\}$ is a densest subgraph, based on a more sophisticated analysis. This simplifies the above rounding procedure and reduces its time complexity from $O(m+n\log n)$ to $O(n)$, while the time complexity of Algorithm~\ref{alg:LP} is dominated by that for solving the LP.


\subsubsection{An exact algorithm based on submodular function minimization}
The objective function of DSP, i.e., the degree density of a subgraph, has a strong connection with submodularity property of set functions.
Based on this observation, an exact algorithm for DSP can be designed using submodular function minimization.
...
%Strictly speaking, the time complexity is polynomial in the number of calls of the value oracle.
Indeed, Gr\"otschel, Lov\'asz, and Schrijver~\cite{groetschel1981ellipsoid} gave the first polynomial-time algorithm based on the ellipsoid method.
Later, Iwata, Fleischer, and Fujishige~\cite{iwata2001combinatorial} designed a combinatorial, strongly-polynomial-time algorithm.
Today some faster algorithms are known, e.g., \cite{orlin2009faster,lee2015faster}.

%The submodularity appears in DSP in the following context.
Let us consider the following problem:
\begin{align*}
\text{minimize}\ \ \beta |S| - e[S] \quad \text{subject to}\ \ S\subseteq V,
\end{align*}
...
where $\beta^{(i)}=\frac{\beta_\text{ub}^{(i)}+\beta_\text{ub}^{(i)}}{2}$.
...
If $S_\text{out}\neq \emptyset$ holds,
then $S_\text{out}$ is a vertex subset that satisfies $\beta^{(i)}|S_\text{out}|-e[S_\text{out}]\leq 0$, i.e., $\frac{e[S_\text{out}]}{|S_\text{out}|}\geq \beta^{(i)}$;
hence, the lower bound $\beta_\text{lb}^{(i)}$ can be replaced by $\beta^{(i)}$.
On the other hand, if $S_\text{out}=\emptyset$, then we see that for any $S\subseteq V$,
$\frac{e[S]}{|S|}\leq \beta^{(i)}$ holds; hence, the upper bound $\beta_\text{ub}^{(i)}$ can be replaced by $\beta^{(i)}$.
...
The first approximation algorithm was identified by Kortsarz and Peleg~\cite{Kortsarz-Peleg94}.
They essentially proved that the $k$-core \cite{malliaros2020core} with the maximum $k$ is a $2$-approximate solution for DSP.
Although the details of the implementation of the algorithm was not give in the paper, they derived the time complexity of $O((n+m\log n)\log n)$.
...
It is now widely known that the algorithm can be implemented to run in linear time for unweighed graphs, i.e., $O(m+n)$ time.
...
Very recently, Chekuri, Quanrud, and Torres~\cite{Chekuri2022supermod} proved that the iterative greedy peeling algorithm converges to an optimal solution to DSP.
...
The algorithm iteratively removes a vertex with the minimum degree in the graph at hand to obtain a sequence of vertex subsets from $V$ to a singleton,
and then outputs a vertex subset with the maximum density among the sequence.
...
\Return{$S_\mathrm{max}\in \argmax\{d(S)\mid S\in \{S_1,\dots, S_{n}\}\}$}
\end{algorithm}


\begin{mytheorem}\label{thm:peeling}
...
However, Gudapati, Malaguti, and Monaci~\cite{gudapati2021greedy} proved that the approximation ratio of $2$ is tight,
...
Finally, we wish to mention the connection between the densest subgraph and the $k$-core, identified by Kortsarz and Peleg~\cite{Kortsarz-Peleg94}.
For $G=(V,E)$ and $k\in \mathbb{Z}_{>0}$, the $k$-core is the (unique) maximal subgraph in which every vertex has degree at least $k$ \cite{malliaros2020core}.
%An important fact is that the $k$-core with the maximum value of $k$ is also a $2$-approximate solution for DSP.
Let $k^*$ be the maximum value among $k$'s such that there exists a nonempty $k$-core, then $k^*$-core is a $2$-approximate solution for DSP.
...
\begin{align*}
d(S^*_\text{core})%=\frac{\frac{1}{2}\sum_{v\in S^*_\text{core}}\deg_{S^*_\text{core}}(v)}{|S^*_\text{core}|}
\geq \frac{1}{2}k^*
\geq \frac{1}{2}\deg_{S^*}(v^*)
\geq \frac{1}{2}d(S^*).
\end{align*}
...
Let $T\in \mathbb{Z}_{>0}$.
The algorithm runs Charikar's greedy peeling routine for $T$ times
...
Note that each iteration of {\sc Greedy++} still requires $\bigO(m+n\log n)$ time. %for a constant T?
...
\Return{$S_\mathrm{max}\in \argmax\{d(S)\mid S\in \{S^1_n,\dots, S^1_1,S^2_n,\dots, S^2_1,\dots, S^T_n,\dots, S^T_1\}\}$}
\end{algorithm}


Chekuri, Quanrud, and Torres~\cite{Chekuri2022supermod} proved the truthfulness of what was conjectured in Boob et al.~\cite{boob2020flowless},
...
where $\Delta$ is the maximum degree of a vertex in the graph and $\text{OPT}$ is the density of a densest subgraph.
...
This certificate is computed without altering the algorithm's computational cost and consists in comparing the value of the best solution computed by the algorithm with the value of a feasible solution of the dual LP used in the above.
Thanks to this quantitative certificate, the authors equipped {\sc Greedy++} with a practical device
...
Prior to Chekuri, Quanrud, and Torres~\cite{Chekuri2022supermod},
Boob, Sawlani, and Wang~\cite{boob2019faster} provided a $(1+\epsilon)$-approximation algorithm for DSP
...
Chekuri, Quanrud, and Torres~\cite{Chekuri2022supermod} designed another $(1+\epsilon)$-approximation algorithm running in
$\tilde\bigO\left(\frac{m}{\epsilon} \right)$ time by approximating maximum flow.
Very recently, Harb, Quanrud, and Chekuri~\cite{harb2022faster} proposed a new iterative algorithm,
