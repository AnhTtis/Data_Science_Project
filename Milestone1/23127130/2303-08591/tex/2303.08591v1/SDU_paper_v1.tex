...
\documentclass[a4paper,onecolumn,11pt, unpublished, titlepage]{quantumarticle}
\pdfoutput=1
...
%	\title{Metrological approach to extension of squeezed state interferometry to spin displacement estimation}
%	\title{Metrological approach to squeezed state interferometry extension to spin displacement estimation}
%	\title{Interferometry-inspired minimal-noise estimation of spin-rotation}
%DB I don't think we should emphasize the interferometry too much. After all, Caves wrote that the SU(1,1) should NOT be thought of an interferometer, but as a detector of displacement. See also below. How about this title:
	\title{Simultaneous minimal-noise estimation of noncommuting rotations of a spin}
	% simplified title (KZ) I like that one (DB)
%	\title{Simultaneous estimation of minimal noise
%	  for generic spin rotations}
...
%	Auf der Morgenstelle 14, 72076 T{\"u}bingen, Germany}
    ...
    direction and strength
     ...
     %
     over the classical bound is shown in terms of quadratic scaling of the quantum Fisher information. Our results provide a method for measuring magnetic fields in any direction in the $x$-$y$-plane with the same optimized initial state. 
...
%	 and some others??		
        ...
        While in the standard Mach-Zehnder interferometer the variance of %\DB
        ...
        limit, they demonstrated that %DB, by introducing non-linear squeezing elements to
        the modified interferometer  %DB and interpretting it as
        generates a $\text{SU}(1,1)$ transformation of the annihilation and creation operators of the two modes and % 
        achieves the
         scaling $\sqrt{\Delta\phi^2 }\propto O(N^{-1})$, %DB achieving what is
        now known as the Heisenberg limit. In a recent reinterpretation, Caves \cite{Caves20} %DB has shown
        argued %DB showed
...
%%%% further changes to be processed  (KZ) 
        ...
        In the micro-wave domain, noiseless amplification of small signals plays an important  role and has been boosted by the tremendous technological progreess in the context of quantum computing based on super-conducting qubits \cite{bergeal_phase-preserving_2010,PhysRevB.98.045405,PhysRevLett.106.110502,renger_beyond_2021}, as well as with nano-mechanical oscillators \cite{PhysRevLett.118.103601}.  Recently, noiseless microwave amplification has even been proposed for speeding-up the search for axions \cite{Sik83,zheng_accelerating_2016}.   
...
We demonstrate that a squeeze-rotate-unsqueeze (SRU) protocol for a single spin as well as for a two-spin system is able to achieve the Heisenberg scaling of quantum Fisher information (QFI) for rotation estimation, $\text{QFI}\propto N^2$ in the size of the displaced system, for particular equatorial axes of rotation. Moreover, for a particular value of squeezing we demonstrate % consistent
simultaneous %
scaling of the $\text{QFI}\propto N^2/2$ for {\it any} equatorial rotation
...
estimation of the rotation axis, as contrasted with the estimation of the rotation angle around a known axis.
%\rd{We show (?)} better than $N^2$ scaling of QFI and explain it by setting the problem in the context of \rd{as yet unexplored (?)} setting of superchannel estimation. 	
		% The results, despite their preliminary character, point to a possibility of a tremendous increase in estimation of spin displacement, thus contributing to the developing field of spintronics. 
...
%		\rd{This feels too extensive for the needs of the article... I'm not sure how to shorten it.}
	...
	\subsection{Spin Coherent and Squeezed States}
		...
		Spin coherent states (SCS), first introduced in 1971 by Radcliffe \cite{Rad71} by analogy to the coherent states of light and later analysed in further details by Arecchi \textit{et al.} and Holtz and Hanus \cite{ACGT72, HH74}, can be understood most easily in terms of a collection of $2S$ spin $\sfrac{1}{2}$ particles, all in an eigenstate of the operator, $\sigma_{\theta,\phi} = \sin\theta\qty(\cos\phi\, \sigma_x + \sin\phi \,\sigma_y) + \cos\theta \sigma_z$, with the positive eigenvalue, $\sigma_{\theta,\phi} \ket{\theta,\phi}= \ket{\theta,\phi}$. By projecting the state $\ket{\theta,\phi}^{\otimes 2S}$ of such collection onto the $2S+1$-dimensional subspace spanned by the symmetric states $\ket{S,M}$ one arrives at a well known expression,
		...
		\begin{equation}
			...
			\end{aligned}}
		\end{equation}  
		where the basis states are defined as eigenstates of spin operator $S_z$, namely $S_z \ket{S,M} = M \ket{S,M}$, defining the basis for states constituting a $2S+1$-dimensional representation of the SU(2) group. The group-theoretic approach to the SCS and an extensive review can be found in \cite{ZFG90}.
	
		In a seminal work \cite{KU93} Kitagawa and Ueda reimagined what it means for a spin state to be squeezed by understanding that one should consider the squeezing in directions perpendicular to the main axis of a SCS. The authors have proposed two squeezing hamiltonians,
		\begin{align}
			H_1 = \mu S_z^2, &&
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		In particular, we will be interested in squeezing the SCS along the $X$ axis, therefore we introduce notation
		...
		\end{equation}
		We note that $\ket{n,n;\pi}=\ket{n,n}_{\sfrac{\pi}{2},\pi}$ and $\ket{\frac{2n+1}{2},\frac{2n+1}{2};\pi}=\ket{\frac{2n+1}{2},\frac{2n+1}{2}}_{\sfrac{\pi}{2},0}$.  Note that squeezed spin states can be oversqueezed, unlike for the squeezed states of light, which has been emphasized in the original work of Kitagawa and Ueda. In Fig \ref{fig:wignershape} we present three examples of squeezed spin states most important in this work, ranging from SCS with $\mu = 0$ to the maximum squeezing of $\mu = \sfrac{\pi}{2}$. For the visualization we go to the spherical Wigner function, as defined in \cite{Agar81}, later analysed in \cite{DAS94} and \cite{DKMG21} in the context of negativity of the spherical Wigner function $W(\theta, \phi)$ \cite{KZ04}.
		\begin{figure}[H]
			\centering
			\includegraphics[width=.32\linewidth]{Not_a_squeezed_state_s40_3} %uncomment for color figure
			\includegraphics[width=.32\linewidth]{Pi4_squeezed_state_s40_3} %uncomment for color figure
			\includegraphics[width=.32\linewidth]{Pi2_squeezed_state_s40_3} %uncomment for color figure
			\caption{
...
%			after rotations by $\sfrac{\pi}{2}$ and $\sfrac{3\pi}{2}$ around the $Y$ axis.}
	...
	\subsection{Quantum Fisher Information and Hamiltonian Estimation}
		...
		As a proxy for the sensitivity of rotation estimation we will use the quantum Fisher information in connection to the quantum Cram\'er-Rao bound, defined in the following theorem \cite{Hels67, Fraise17}.
		...
		\begin{theorem}
			...
			\begin{equation}
				\operatorname{E}[\hat{\theta}] \geq \frac{1}{I_\theta(\rho_\theta)}
			\end{equation}
		...
		For the purpose of this work, however, we will be mostly concerned with the closed form of QFI for pure states. Given that $\rho_\theta = \op{\psi_\theta}$ one finds that the expression for QFI reduces to 
		\begin{equation}
		...
		\end{equation}
		and the optimal measurement saturating the bound on the variance given by QFI can be given in terms of projectors $M_i = \op{l_i}$ on the eigenvectors $\ket{l_i}$ of the SLD. 
...
%		from which it is easily extracted that
		...
		\end{equation}
		which shows linear scaling with the number of the systems $N = 2S$, often referred to as the classical limit, and the state $\ket{S,S}_{\sfrac{\pi}{2},\phi}$ being the most classical state of a spin system. 
		In the remainder of this paper we will demonstrate a protocol that allows quadratic scaling with $N$, thus the quantum counterpart of the classical limit. 
...
%                \rd{
		...
		\begin{equation}
			\qty(\mathbf{I}_{\boldsymbol{\theta}}(\rho_{\boldsymbol{\theta}}))_{ij} :=  \frac{1}{2}\left(\rho_{\boldsymbol{\theta}}\{L_{\theta_i},L_{\theta_j}\}\right)\,,
                      \end{equation}
                      where $\{\cdot,\cdot\}$ denotes the anti-commutator. 
In contrast to the single parameter quantum Cram\'er-Rao bound, the bound \eqref{eq:multiparam_est_bound} can usually not be saturated. A bound that can be up to a factor two tighter \cite{demkowicz-dobrzanski_multi-parameter_2020} and that can be saturated asymptotically in the limit of infinitely many samples was introduced by Holevo \cite{holevo_statistical_1973}, but it is, in general, more difficult to evaluate than the quantum Cram\'er-Rao bound. If the SLD operators for different parameters commute on average, $\tr \left(\rho_{\boldsymbol{\theta}}[L_{\theta_i},L_{\theta_j}]\right)=0$, then this Holevo-Cram\'er-Rao bound coincides with the quantum Cram\'er-Rao bound, such that also the latter can be saturated asymptotically \cite{AFD19,tsang_quantum_2020,demkowicz-dobrzanski_multi-parameter_2020}.
%			\comment{I don't like this part, but I tried at least to write a sketch... At this moment it is based on \cite{Fraise17}. I would say that we need a little more information about recent developments in this case, but I don't feel knowledgeable enough to write this...}
			% \comment{Missing the tighter bounds, when the eq (16) agrees with the actual sensitivity.}
%		}
	\section{Results}
%	\rd{
%	Here should go...
%	\begin{enumerate}
%		\item Justifying example together with the definition of SDU protocol -- application to 1-spin 1-axis squeezed state with $X$ or $Y$ rotation axis and arbitrary spin $S_M$, demonstrating extreme cases and optimal measurements (here easily interpretable)
%		\item 2-spin 1-axis SDU rotation angle estimation -- general expression for any squeezing parameter, any equatorial axis and any main spin $S_M$ and probe spin $S_P$. Demonstration of the intermediate regime of $(N^2 + N)/2$. Relation with 1-spin squeezing through case of $S_P = S_M$.
%		\item Equatorial axis estimation -- closed expressions for specific values of rotation $\gamma$; surprising expression for sum of QFI for angle and axis estimation. Maximum $\text{QFI} = N^2 + N$, potential relation to superchannel estimation (is it already in the literature?)
%		\item Hamiltonian superchannel estimation problem. Demonstration of $2N^2$ behaviour for particular choice of the Hamiltonian on which superchannel acts. Open problem -- whether it is possible to formulate general theory for strange sum identities like the one from the previous point...
%	\end{enumerate}
%	}
...
%			\right.
		\end{equation}
		where $N = 2S$ is the number of spins-$1/2$ in a composite system setting. This illustrates that for both chosen axes ($\Phi = 0$ or $\pi/2$) we find both characteristic scalings of the QFI, depending on whether the spin is integer or half-integer. 
...
%		With the single-state procedure demonstrates explicitly that it is possible to reach the scaling of the QFI with the square of the number of spins $1/2$ we 
		...
		\end{align}
		with the approximation $(a1)$ achieved by Taylor expansion around $\epsilon = 0$ in the lowest order, $(a2)$ by noting that $2^{-2 S_P}$ vanishes in the limit of large $S_P$, and $(a3)$ by noting that $\epsilon\ll 1$ justifies neglecting the term of order $\epsilon^{2S_P}$. This provides the formal demonstration that the QFI is flat around $\mu = \sfrac{\pi}{2}$ and becomes progressively more flat for large probe spins.  The final line demonstrates that we are exactly at the ``optimal'' value halfway between the Heisenberg and the classical scalings, giving a stable region within which rotation around any axis can be estimated. In particular, in the stable region, one still retains $I_\gamma\qty(\ket{\text{SRU}_2(\gamma,\Phi;\sfrac{\pi}{2})}) \simeq O(N_M^2)$ for $S_M = N_M/2$.
...
%		In Appendix \ref{} we present details of its composition together with demonstration of stellar representations for selected values of $S$.
	...
	\subsection{Different ways of coding a relative rotation}
...
%		\caption{\rd{The additional rotation $R(\gamma_2, \Phi_2)$ on the probe spin is introduced in order to provide considerations similar to the frame of reference problem known for the SU(1,1) interferometer. \comment{This description is too short, I think... The picture can be refined too, but I didn't yet come up with a proper place for showing the $\mu$ axis scaling...}}}
		\caption{Bounds on the sensitivity for estimating $\gamma_1$ in the scheme as shown in Fig. \ref{fig:scheme_Spin2_2_rots} as function of the squeezing parameter $\mu$, for both main and probe spins equal and for selected angles $\Phi_1$ and $\Phi_2$ of rotation axes. Left panel: $S=1/2$, right panel: $S=3/2$.  Combinations of the same relative rotation angle $\Phi_2-\Phi_1$ are on the lines perpendicular to the diagonal of the matrix of plots and seen to give quite different lower bounds $(I_{\gamma_1} - \frac{\qty(I_{\boldsymbol{\gamma}})_{12}^2}{I_{\gamma_2}})^{-1} $ for the error of the $\gamma_1$ estimation. Note that the optimal bounds at $\mu = \pi/2$ are achieved at $\Phi_1 = \pi/2$ and $\Phi_1 = 0$ for $S=1$ and $3/2$ respectively. Furthermore, for $S = 1$ and $\Phi_1 = 0$ we note decrease of the bound from $3$ to $2.67$ together with decreasing $\Phi_2$ from $\pi/2$ to $0$.%
                  %\qq what is a bit surprising is that for relative phase zero, the lower left corner $\Phi_1=\Phi_2=0$ is optimal for $S=3/2$, whereas for $ S=1$, it is the upper right corner.
                                    % \comment{What else to write here...  DB: discuss those thin lines presumably linked to $\mu=\pi/2$}
                }
...
%\JCz I think that the proper comparison for $S=1$ is not actually 4 - 2.67, but 3 - 2.67, as this is the comparison for $\phi_1 = 0$ between sensitivity for gamma_1 without any phase on the second arm and under presence of gamma_2. This way the difference is 0.33, whereas similar comparison for S = 3/2 should be between 6 and 5.56, which is aroudn 0.24.
        %\DB let's discuss it!
...
%	}
%\qq can we conclude now what is the optimal way of coding the same relative phase?	
%\JCz not precisely, since the "relative phase" itself is not well defined on a sphere.
%DB ok, I meant relative rotation angle. That is, $\Phi_2-\Phi_1 \text{mod} 2\pi$. it seems to me that this well defined?!
        %Here we have something like sensitivity for one rotation in presence of rotation in the other arm... I'm not sure, how we could define optimality here.
% One thing we can conclude, though, I think, is that the scaling is still of $O(S^2)$ order.
...
%	\comment{In principle these expressions are correct but useless for equal spins, since then $\sqrt{\Delta^2\gamma_1} - \sqrt{\Delta^2\gamma_2} = 0$. }
	...
	\subsection{Axis angle estimation and %\qq peculiar DB that
                                %indicates we dont understand it. 
          sum rule}
		...
		As we have seen, the SRU protocol allows for a QFI for
                ...
                case. First, it is easy to demonstrate that the QFI
		...
		\end{align}
		suggesting that the case $\gamma = \pi$ is optimal. This is not the case as $I_\Phi(\ket{\text{SRU}_2(\pi,\Phi;\mu)}) = 8S_M$, scaling linearly with the size of the spin system. Consider then the intermediate case with the state turned by a quarter-turn, $\gamma = \pi/2$, which yields
...
%			 =  e^{i \Phi \text{Ad}_{S_z}} e^{i \gamma S_y}
		\end{equation}
		to notice that the rotation of the angle $\Phi$ is imprinted twice on the state. One thus deviates from the standard protocol that leads to the HL. The double imprinting comes with a slight twist that the evolution seems to be taken once forward in time and the second time in reverse or, in other words, 
		double imprinting with time reversal. %Strange place
...
%		and from this we actually see that the auxiliary Hamiltonian takes the form of squeezed rotation operator.
	...
	\subsection{Numerical study of two-axis squeezing}
		...
		For completeness, we shortly consider a variant of SRU protocol where instead of one-axis squeezing we  utilize two-axis squeezing operator. The full protocol will be now defined as
		\begin{equation}
			\ket{\text{SRU}^2_2(\gamma,\Phi;\mu)} = e^{-\mu(S_-^2 - S_+^2)}e^{i \gamma S(\Phi)}e^{\mu(S_-^2 - S_+^2)}\ket{S,S}  \label{eq:2_axis_SRU_state},
		\end{equation}
		in analogy to the SRU protocol as defined in \eqref{eq:1_spin_SRU}. Here the squeezing Hamiltonian has been changed to $i (S_+^2 - S_-^2)$ and the input state is a spin coherent state pointing along the $Z$ axis, following the preparation presented in \cite{KU93}. Therein, the authors have demonstrated that the variance of a spin in a selected direction orthogonal to the main axis of the squeezed state for large spins approaches a constant. Later the two-axis squeezing has been studied theoretically \cite{KW15} and only recently an experimental realization has been proposed \cite{HEtAl22}.  
		
		In this case analytical treatment is not accessible, therefore we present a set of plots for selected values of $S$ which present the qualitative behaviour of two-axis squeezing. Despite clearly periodic behaviour for $S = 1$ and $S = \sfrac{5}{2}$, for larger spins QFI fails to reach the value of $4S^2$ demonstrated to be attainable for one-axis squeezing. Furthermore, despite isolated points at which Fisher information agrees for different rotation axes, no stable plateaus can be found.
...
%		As a preliminary study, many problems have been left open to explore for the future research, starting with investigation of an optimal measurement which could take consistent advantage from the $N^2/2$ scaling demonstrated for intermediate squeezing. We point out that one promising candidate is the Coherent Spin Measurement recently studied in \cite{JC21}. Further questions include noise-resilience of the introduced SRU protocol as well as possibility of extraction of information about the main spin through the measurement of the state of the probe system alone. 
	...
	In this work we have translated $\text{SU}(1,1)$ interferometry,
        ...
        simultaneous quadratic scaling for the estimation of the rotation %
        angle %
        around % an arbitrary 
        {\it all} equatorial axes, which can be compared to the
        simultaneous estimation of displacements with unlimited
        sensitivity in an arbitrary direction in the phase space for
        bosonic systems. % In both situations one might naively expect
                         % to be limited by a Heisenberg uncertainty
                         % relation, but this is not the case. %\DB  I
                         % think this is too naive, is we are talking
                         % about classical parameters to be estimated,
                         % not quantum observables.
...
%	Similar effects have been also shown for a single-spin systems. 
	Furthermore, we have shown that the introduced scheme provides the advantage of quadratic scaling for rotation-axis estimation given control over the rotation angle. %\qq, e.g.~by controling the time of the interaction. %DB what interaction? I assume you mean with an external magnetic field that rotates the spin? If so, it would need to be introduced so that people know what interaction we talk about. %\qq add some discusion of magnetic field measurements.  
	%\JCz Then I think that we need to skip it indeed
	...
	The SRU protocol for a single-spin can be seen as a special case of the delta-kick squeezing (DKS) protocol introduced in \cite{CGSP21}. One has, however, to stress that in our work the inspiration comes from the SU(1,1) displacement detector's ability to achieve an advantage for arbitrary direction of displacement. Therefore, we have focused on finding stable advantages over a wide range of different axes of rotation. The 2-spin SRU protocol goes beyond DKS, % where
        ...
        % \qq
        This allows the simultaneous estimation of different
        components of a magnetic field that lead to non-commuting rotations. 
        This could be extended in a future research in the spirit of \cite{KEtAl19} to ask for variational generation of spin states which are flexible for rotation angle estimation over a wide range of axes.
...
Several other open directions are left to pursue in the context of the 2-spin SRU protocol.
         For instance, one may consider the QFI of the probe system for estimating the rotation angle of the primary system. Finally, the problems of estimation of the rotation angle and rotation axis have been understood in this work as two separate one-parameter estimation tasks with the remaining parameter being under control --- in a future work SRU protocol should be analysed in the context of two-parameter estimation of the full rotation.
	
	\section*{Acknowledgments}
	
		JCz and KŻ would like to thank Jan Kołodyński for fruitful discussion, in particular on the topic of multiparameter estimation. KŻ and JCz acknowledge gratefully financial support by Narodowe Centrum Nauki under the grant numbers DEC-2015/18/A/ST2/00274 and 2019/35/O/ST2/01049 and by Foundation for Polish Science under the Team-Net NTQC project.
	...
	\section{QFI for rotation angle of 2-spin SRU around any equatorial axis}\label{app:2spin_equat_axes}
		...
		and its derivative,
			{\small\begin{equation}
				\begin{aligned}
					\ket{\partial_\gamma\text{SRU}_2(\gamma,\Phi;\mu)} = & \frac{1}{2^{S_M + S_P}}\sum
					 \sqrt{\binom{2S_M}{S_M - j}\binom{2S_P}{S_P - k}}\\
