...
\begin{abstract}
  ...
  degrade upon spatial and temporal refinement. For that purpose, we extend
 ...
 An example model problem, for which we also provide an open-source
...
solution of the minimization problem~\eqref{eq:if-intro}, the instanton $\eta_z$, as a
background field.
We show how to evaluate this determinant numerically for general
SDEs and SPDEs,
and demonstrate through multiple examples that it is possible to do so
even for very high-dimensional systems. Our approach is based on computing
the dominant eigenvalues of the trace-class integral operator entering
the Fredholm determinant.
\begin{figure}
  \centering
  \includegraphics[width = .425 \textwidth]{2mb-paths-sketch.pdf}
...
as reviewed e.g.\ by~\citet{rackwitz:2001}.
...
\paragraph*{Notation.}
We use the following notations throughout the paper: We denote the
...
real functions $g$ and $h$, we write $g(\eps) \overset{\eps \downarrow 0}{
\sim} h(\eps)$ if $\lim_{\eps \downarrow 0} \tfrac{g(\eps)}{h(\eps)} = 1$,
i.e.\ if the functions $g$ and $h$ are asymptotically equivalent
as $\eps \downarrow 0$.
...
and is more amenable to analysis than rareness due to $z\to\infty$.
Since
$\eta$ is normally distributed, the extreme event probability
is available as an integral,
\begin{equation}
...
Then, there is a sharp estimate for the extreme event
...
\begin{figure}
  \centering
  \includegraphics[width = .48 \textwidth]{{2mb-paths-eps-0.5}.pdf}
...
There is a small amount of added stochasticity in the
form of Brownian noise, and we ask the question: What is the
probability $P_F^\eps(z)$ that the system ends up, at time $T=1$, in
...
probability $P_F^\eps(z) = \PP \left[F[\sqrt{\eps} \eta] \geq z \right] = \PP \left[f(X_T^\eps) \geq z \right]$
with $z > F(0)$,
...
\item Without any additional work, we also obtain a sharp estimate for the
...
\begin{figure}
\centering
\includegraphics[width = .48 \textwidth]{2mb-evals-conv}
...
\begin{figure*}
\centering
\includegraphics[width = \textwidth]{checkpointing}
...
\begin{figure*}
\centering
\includegraphics[width = \textwidth]{{2mb-filter-pdfs-eps-0.5}.pdf}
...
\begin{figure*}
\centering
\includegraphics[width = \textwidth]{kdv-prefac-prob}
...
obtained from numerical instanton and prefactor computations. Right: Comparison
...
example at $\eps > 0.1$. Results use $n_x = 1024, n_t = 4000$ for
...
under changes of the numerical resolution. The numerical details for
...
methods are scalable, i.e., their cost does not increase beyond the
...
\begin{figure}
\centering
\includegraphics[width = .48 \textwidth]{kdv-evals-conv}
...
\end{ruledtabular}\\
\vspace{.3cm}
\includegraphics[width = .48 \textwidth]{kdv-evals-conv-diff-res}
...
\begin{figure*}
\centering
\includegraphics[width = \textwidth]{{kdv-inst-gammas-obs-8.39125}.pdf}
...
\begin{figure*}
\centering
\includegraphics[width = \textwidth]{{kdv-final-time-obs-8.39125}.pdf}
...
\begin{figure}
\centering
\includegraphics[width = .48\textwidth]{nse-evals-conv}
...
\begin{figure*}
\centering
\includegraphics[width = \textwidth]{nse-evecs-final}
...
from theoretical physics could be investigated. Finally, one could use
