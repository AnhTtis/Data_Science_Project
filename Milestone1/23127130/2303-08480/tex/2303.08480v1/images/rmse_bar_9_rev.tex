\begin{tikzpicture}%
	\begin{axis}[%
   	ybar=.1cm,%
    bar width=5pt,%
    width = 0.378\textwidth,%0.77\columnwidth,%
    height = \figSize,
    grid = both,%
    ymin=0,%
    ymax=7,%
    enlarge x limits=0.15,%
    major grid style = {lightgray},%
    minor grid style = {lightgray!25},%
    % ylabel = $\RMSE$,%
    % y unit= \deg,%
    xmode=log,%
    log ticks with fixed point,%
    xtick = {5,10,20,40},%
    xticklabels = {5,10,20,40},%
    x unit = \decibel,%
    xlabel={$\operatorname{SNR}$},%
    style={ font=\footnotesize},%
    %  legend style={anchor=south,legend columns=1, font=\small},
    legend style={at={(0.5,1.3)},%
    anchor=north,legend columns=3, font=\tiny},%
    ]%
    \addplot+[%
        % only marks, 
        blue, %
        %mark=*, 
        error bars/.cd, %
        y dir=both, %
        y explicit,%
        error bar style=black%
        ] %
		table[x index=0, y index=1, y error index=2] {images/data/doa_shd_lra_9.txt};%
% 	\addlegendentry{SHD-LRA};%
	\addplot+[%
        % only marks, 
        % blue, 
        %mark=*, 
        error bars/.cd, %
        y dir=both, %
        y explicit,%
        error bar style=black%
        ] %
		table[x index=0, y index=1, y error index=2] {images/data/doa_rmusic_9.txt};%
% 	\addlegendentry{RMUSIC};%
	\addplot+[%
        % only marks, 
        % blue, 
        % mark=diamond*, 
        error bars/.cd, %
        y dir=both, %
        y explicit,%
        error bar style=black%
        ] %
		table[x index=0, y index=1, y error index=2] {images/data/doa_shd_rmusic_9.txt};%
% 	\addlegendentry{SHD-RMUSIC};%
\end{axis}%
\end{tikzpicture}%