\begin{tikzpicture}
	\begin{axis}[
    width = \columnwidth,
    height=4cm,
    grid = both,
    major grid style = {lightgray},
    minor grid style = {lightgray!25},
    ylabel = $\PD$,
    y unit= \percent,
    xmode=log,%
    log ticks with fixed point,%
    xtick = {5,10,20,40},
    xticklabels = {5,10,20,40},
    x unit = \decibel,
    xlabel={$\operatorname{SNR}$},
    style={ font=\footnotesize},%
    %  legend style={anchor=south,legend columns=1, font=\small},
    legend style={at={(0.5,1.2)},
    anchor=north,legend columns=3, font=\tiny},
    ]
    \addplot+[
        only marks, 
        blue, 
        %mark=*, 
        error bars/.cd, 
        y dir=both, 
        y explicit
        ] 
		table[x index=0, y index=1, y error index=2] {images/data/pd_shd_lra_8.txt};
	\addlegendentry{SHD-LRA};
	\addplot+[
        only marks, 
        % blue, 
        %mark=*, 
        error bars/.cd, 
        y dir=both, 
        y explicit
        ] 
		table[x index=0, y index=1, y error index=2] {images/data/pd_rmusic_8.txt};
	\addlegendentry{RMUSIC};
	\addplot+[
        only marks, 
        % blue, 
        mark=diamond*, 
        error bars/.cd, 
        y dir=both, 
        y explicit
        ] 
		table[x index=0, y index=1, y error index=2] {images/data/pd_shd_rmusic_8.txt};
	\addlegendentry{SHD-RMUSIC};
%		\addlegendentry{Sinc}
% \legend{2DSinc,2DUnet\textsuperscript{50},2DUnet\textsuperscript{25},2DUnet\textsuperscript{12}}
\end{axis}
\end{tikzpicture}