...
\def\svnVersion{375da43}\def\svnDate{2023/09/05}\def\cmsCernNoTag{CERN-EP-2023-021}\def\cmsCernDate{\today}\def\cmsMessage{Published in the Journal of High Energy Physics as \href{http://dx.doi.org/10.1007/JHEP08(2023)204}{\doi{10.1007/JHEP08(2023)204}.}}
...
\newcommand{\xsecpred}{\ensuremath{924\hspace{1pt}^{+32}_{-40}\pb}\xspace}
\newcommand{\xsecmeas}{\ensuremath{881\pm 23\statsyst\pm 20\lum\pb}\xspace}
...
\hypersetup{%
    ...
    pdfsubject={CMS},%
    pdfkeywords={CMS, top quark, Hadron-Hadron Scattering, Top Physics},%
}
...
At the LHC, the \ttbar production cross section has been measured from \pp collisions by the ATLAS, CMS, and LHCb Collaborations at $\sqs=5.02$, 7, 8, and 13\TeV~\cite{CMS:TOP-11-007, ATLAS:2014nxi, CMS:TOP-14-018, CMS:TOP-12-006, CMS:TOP-13-004, ATLAS:2017jkf, ATLAS:2017wvi, LHCb:2018usb, CMS:TOP-17-001, CMS:TOP-18-005, ATLAS:2020ccu, ATLAS:2020aln, CMS:TOP-20-001, CMS:TOP-20-004, CMS:TOP-18-014, ATLAS:2022jbj, ATLAS:2022xny, ATLAS:2023gsl}.
...
At \sqrts, assuming a top quark mass of $\mtop=172.5\GeV$, and using the PDF4LHC21 parton distribution function (PDF) set~\cite{PDF4LHCWorkingGroup:2022cjn}, a value of $\sigtt=\xsecpred$ is predicted, about 10\% larger than the value at \sqrts[13].
...
Numerous upgrades and improvements to the subdetectors, readout electronics, trigger, data-acquisition, software, and offline computing systems were implemented before the start of the 2022 data taking.
Notable examples include new silicon photomultipliers and readout electronics for the HCAL that allow for a finer granularity and longitudinal segmentation~\cite{CMS:TDR-010}, the replacement of the innermost layer of the silicon pixel detector~\cite{CMS:DP-2022-047}, the new hybrid CPU/GPU farm for the HLT~\cite{CMS:DP-2023-004}, and rebuilt dedicated online luminosity monitors~\cite{CMS:NOTE-2022-007, Karunarathna:2022tyd, Wanczyk:2022avs}.
...
In situ measurements of the momentum balance in dijet, {\PGg}+jet, {\PZ}+jet, and multijet events from data collected in 2022 are used to account for any residual differences in the JES between data and simulation~\cite{CMS:JME-13-004, CMS:DP-2022-054}.
...
The dilepton HLT selection requires the presence of two isolated electrons with $\pt>23$ and 12\GeV, two isolated muons with $\pt>17$ and 8\GeV, one isolated electron with $\pt>23\GeV$ and one isolated muon with $\pt>8\GeV$, or one isolated muon with $\pt>23\GeV$ and one isolated electron with $\pt>12\GeV$.
...
The CMS detector recorded \intlumi of data during this time period that pass data quality requirements, yielding the data set used in this measurement.
...
Top quark decays are simulated with \POWHEG for the \ttbar and \tW-channel single top quark samples, and with \textsc{MadSpin}~\cite{EXT:Madspin-2013} for the $t$-channel single top quark sample.
...
For all simulated samples, the proton structure in the matrix-element (ME) calculation is described with the NNPDF3.1 PDF set~\cite{EXT:NNPDF-2017} at NNLO.
...
Top quark production cross sections are calculated assuming a top quark mass of $\mtop=172.5\GeV$ and are evaluated using the PDF4LHC21 PDF set~\cite{PDF4LHCWorkingGroup:2022cjn}.
The \ttbar signal samples are normalized to the NNLO+NNLL cross section of \xsecpred.
Single top quark production in the \tchannel is normalized to the cross section calculated at NNLO in QCD with \MCFM v10.1~\cite{Campbell:2020fhf}, and in the \tW channel to the cross section calculated up to approximate next-to-NNLO corrections matched to complete NLO results in QCD, as described in Ref.~\cite{Kidonakis:2021vob}. 
...
We use additional identification (ID) criteria to identify ``prompt'' leptons originating from top quark decays at the PV and to reduce background contributions from ``nonprompt'' leptons, defined as any reconstructed lepton that does not match a true lepton produced in a hard scattering process at the PV.
The latter category includes leptons from photon conversions, leptons from semileptonic decays of hadrons, and hadrons misidentified as leptons.
...
The \textsc{DeepJet} algorithm~\cite{CMS:BTV-16-002, CMS:DP-2018-058, EXT:DeepJet-2020}, trained on simulated events at \sqrts[13], is used for the identification of \PQb jets (``\PQb tagging'').
We apply a working point with an observed selection efficiency for \PQb quark jets of more than 80\%, and a misidentification rate for \PQc quark jets (light quark and gluon jets) of around 17 (2)\%, as determined in simulated \ttbar events.

For the \ljets channel, events are selected by requiring exactly one lepton with $\pt>35\GeV$ in order to match the single-electron trigger threshold.
...
A total of 11 event categories are obtained, which are further subdivided by the total number of jets yielding 40 bins to be considered in the maximum likelihood fit.
...
Background contributions after the event selection arise from single top quark (\tchannel and \tW, collectively referred to as ``single \PQt''), \Zjets, \Wjets, and diboson production.
They are estimated using simulated event samples.
Additional contributions arise from SM events composed uniquely of jets produced through the strong interaction, referred to as QCD multijet events, which are estimated using control regions (CRs) in data.
...
The largest background contributions are single-\PQt production in the \emu channel, \Zjets production in the \ee and \mumu channels, and QCD multijet events in the \ljets channels.
...
QCD multijet events contribute to the \ljets final state because of the presence of nonprompt leptons, whereas their contribution is negligible in the dilepton channels.
We estimate the QCD multijet background in the \ljets channel from two orthogonal CRs in data.
...
For each uncertainty source, the simulated event samples are used to construct corresponding template histograms that describe the expected signal and background distributions for a given nuisance parameter variation.
...
Lepton ID efficiencies are derived as functions of lepton \abseta, \pt, and flavor using the tag-and-probe method in a \Zjets-enriched CR.
...
Since the lepton \pt and \sigeta selection criteria are identical across all event categories, scale factors for these integrated efficiencies can be applied to each channel based solely on lepton multiplicity.
These scale factors can then be constrained by the combination of multiple decay channels, provided that the events are not further separated in a way that breaks the approximate independence of the lepton ID efficiency.
...
The misidentification rate of light and \PQc quark jets as \PQb jets is described in a similar way. 
It is allowed to vary within 10\% from that observed in simulation, and variation beyond this level is not found to affect the measurement outcome or sensitivity.
...
In order to remove effects on the cross sections of signal and background processes while still considering possible acceptance effects, each variation is normalized to preserve the respective nominal sample cross section before any event selection.
The ME scale variations are performed for the \ttbar, single-\PQt, \Zjets, and \Wjets samples, and treated as uncorrelated between the processes.
...
The PDF uncertainty is estimated by reweighting the simulated samples to match 100 different PDF replicas in the NNPDF3.1 set, following the PDF4LHC recommendations~\cite{Butterworth:2015oua}.
...
It has been shown in differential measurements of \sigtt at \sqrts[13] that the \pt distribution of the top quark is softer than predicted by the \POWHEG simulation used for our \ttbar signal sample~\cite{CMS:TOP-17-014, CMS:TOP-16-007, CMS:TOP-16-008}.
Because of this, an additional uncertainty is estimated by reweighting the top quark \pt of the simulation to a fixed-order calculation at NNLO in QCD and electroweak contributions~\cite{Czakon:2017wor}.
The resulting nuisance parameter is one-sided by definition.
...
Uncertainties of 30\% are assigned to the \Zjets, \Wjets, diboson, and QCD multijet backgrounds, all of which have little impact on the fit, and of 15\% to the \tW and $t$-channel single-\PQt backgrounds, based on the precision of experimental measurements~\cite{CMS:SMP-14-020, CMS:TOP-17-011, CMS:SMP-18-004, CMS:TOP-20-002, CMS:SMP-20-014, CMS:SMP-20-015, CMS:TOP-21-010} and consistent with the treatment in previous \ttbar measurements~\cite{CMS:TOP-15-003, CMS:TOP-16-005, CMS:TOP-17-001}.
...
In addition, bin-by-bin statistical uncertainties are applied to the signal and background processes using the method given in Ref.~\cite{Barlow:1993dm}.
...
To evaluate the approximate effect of a given source of uncertainty on the overall measurement precision, the fit (described in Section~\ref{sec:fit}) is repeated with the associated nuisance parameter(s) frozen at the best-fit value(s).
...
In the special case of statistical uncertainty, the fit is repeated with all nuisance parameters frozen at their best-fit values and the resulting uncertainty is taken to be that arising purely from statistical effects.
...
\begin{tabular}{l@{}c}
    ...
    ME scale, \ttbar & 0.5 \\
    ME scale, backgrounds & 0.2 \\
    ME/PS matching & 0.1 \\
    PS scales & 0.3 \\
    PDF and \alpS & 0.3 \\
    Top quark \pt & 0.5 \\
    \tW background & 0.7 \\
    $t$-channel single-\PQt background & 0.4 \\
    \Zjets background & 0.3 \\
    \Wjets background & \makebox[0pt][r]{$<$}0.1 \\
    Diboson background & 0.6 \\
    QCD multijet background & 0.3 \\
    Statistical uncertainty & 0.5 \\ \hline
    Combined uncertainty & 2.5 \\ \hline
...
The result of the fit is shown in the lower plot of Fig.~\ref{fig:fit}.
Considering only the statistical uncertainty in the data, the agreement between the data and prediction in the final analysis binning is greatly improved.
An important contribution to that effect is the estimation of the \PQb tagging efficiency in the fit.
The uncertainties are greatly reduced due to additional constraints of the nuisance parameters as well as correlations between them.
...
\caption{%
    ...
    The SM prediction at NNLO+NNLL precision~\cite{Czakon:2013goa} using the PDF4LHC21 PDF set~\cite{PDF4LHCWorkingGroup:2022cjn} and values of $\mtop=172.5\GeV$ and $\alpS(\mZ)=0.118$ is shown with a black line and green uncertainty bands.
...
We find that for an increase (decrease) of \mtop by its current experimental uncertainty of 0.3\GeV~\cite{EXT:PDG-2022}, the measured \ttbar cross section decreases (increases) by 0.3\%.

An independent cross section measurement is performed using an event-counting method restricted to events containing an opposite-sign \emu pair and at least two jets.
This strategy follows closely the methods of Ref.~\cite{CMS:TOP-15-003}, and can also be compared to event counting measurements found in Refs.~\cite{CMS:TOP-13-004, CMS:TOP-16-005, CMS:TOP-17-001}.
