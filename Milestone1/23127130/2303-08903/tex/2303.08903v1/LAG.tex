...
\newcommand\elena[1]{{\textcolor{purple}{#1}}}
\newcommand\xavier[1]{{\textcolor{blue}{#1}}}
...
\newcommand\pl{\mathfrak{p}} %notation pour les places sur X
\newcommand\ql{\mathfrak{q}} %notation pour les places sur Y
...
$\underline{V}=(V_1,\dots,V_s)$ and 
$\underline{W}=(W_1,\dots,W_s)$ be two $s$-uples of $k$-vector spaces. 
Write $n_i = \dim_k V_i$ and $m_i = \dim_k W_i$. Let $\Hom_k(W_i,V_i)$ 
...
The sum-rank distance between $\varphi,\psi\in \Hom_k(\underline{V},\underline{W})$ is 
...
$\Hom_k(\underline{V},\underline{W})$ endowed with the sum-rank distance.
By defintion, its \emph{length} $n$ is $\sum_{i=1}^s m_i n_i$. Its 
\emph{dimension} $\delta$ is $\dim_k \mathcal{C}$. Its \emph{minimum 
distance} is 
...
$n_\ell \coloneqq
\sum_{i=1}^s m_i$, the $\ell$--dimension of $\CC$ is $\delta_\ell \coloneqq
\dim_\ell \CC$, and the 
...
setting writes $d + \delta_\ell \leq n_\ell + 1$ \cite[Prop.~34]{MP18}. Codes
...
\begin{remark*}
To avoid heavy notations in the following lemma we assume $W_i=\ell$ for every $i$, and consider $\CC$ to be a $\ell$-subspace of $\End_k(\ell)^s$ instead of $\Hom_k(\underline{W},\ell)$. However, our proof can be easily generalised to the latter.
\end{remark*}
...
For any $i\in\{1,...,s\}$ and any $x \in \ell^\times \coloneqq \ell \setminus \{0\}$, we define the following element of $\PP^1(\ell)$:
...
Riemann--Roch theorem. In Section \ref{sec:thecode}, we construct linearized Algebraic Geometry codes, and study their parameters. When 
considering the case of curves of genus $0$, we retrieve linearized 
...
a generator of the former Galois group. We set $L \coloneqq L_0^m$ and embed
$K$ and $L_0$ diagonally into $L$. We define
...
In this subsection, we assume that $K = k\llparen t \rrparen$, where
$k$ is a field. We write $\OO_K \coloneqq k\llbracket t \rrbracket$ and let 
$v_t : K \to \Z \sqcup \{\infty\}$ be the $t$-adic valuation on $K$.
...
We write $\OO_{L_0}$ for the ring of integers of $L_0$, that is the 
...
Let $m=1$, $L_0/K$ unramified and $\gcd(v_t(x), r) = 1$.
...
\begin{proof}
Write $\delta = \dim_k \ker \bar\varepsilon(f)$, and let
$\bar e_1, \ldots, \bar e_r$ be a $k$-basis of $\OO_L/t\OO_L$ 
such that $\bar \varepsilon(\bar e_i) = 0$ for $i \in \{1,
\ldots, \delta\}$. For $1 \leq i \leq r$, we choose an element
...
$\delta$ columns of $M$ have entries divisible by~$t$ by construction.
Therefore, $\det(M)$ is divisible by $t^\delta$.
...
and $Y$ defined over $k$, together with an étale surjective map $\pi : 
Y \to X$.
We assume that $\pi$ is a Galois cover with cyclic Galois group of 
order~$r$.
...
Moreover, our assumptions on $\pi$ ensure that $L/K$ is Galois with
cyclic Galois group of order~$r$. For definitions and classical results on Galois covers of curves we refer the reader to \cite[Chapter 3]{S09}.
\medskip
...
\end{equation}
where, letting $\lfloor \cdot \rfloor$ denote the integer part function,
...
In this section we introduce codes in the sum-rank metric from algebraic curves, that we call linearized Algebraic Geometry codes.  We propose a general construction using a Galois cover $\pi : Y \to X$ between two curves (Subsection \ref{ssec:construction}). We give bounds for the dimension and the minimum distance of our codes (Theorem \ref{th:codeparameters}). Finally, in Subsection \ref{ssec:isotrivial}, we consider the case of isotrivial covers. In particular, when the curve $X$ has genus $g_X=0$, we retrieve the construction of linearized Reed--Solomon codes, as proposed in \cite{MP18, CD22}.
...
a Galois cover $\pi : Y \to X$ between 
smooth projective irreducible algebraic curves defined over $k$. We assume that $\text{Gal}
(Y|X)$ is cyclic of order~$r$.
We write $K \coloneqq X(k)$ and $L \coloneqq Y(k)$ for the function fields of $X$ and $Y$, respectively. The
...
\item its \emph{dimension} $\delta$ as its $k$-dimension, \ie 
$\delta \coloneqq \dim_k \CC$,
...
reads $r d + \delta \leq n + r$ in our setting~\cite[Thm.~3.1]{BGLR21}. The next theorem provides explicit lower bounds for  the dimension and the minimum 
...
that $\deg_Y(E) < s r$.
Then, the dimension $\delta$ and the minimum distance $d$ of
...
Therefore $\delta = \dim_k \Lambda_{L,x}(E)$, and the announced lower
bound on $\delta$ now follows from Corollary~\ref{cor:RROre}.
\end{proof}

\begin{corollary}
Under the assumptions of Theorem~\ref{th:codeparameters}, and still
writing $n$, $\delta$ and $d$ for the length, the dimension and the
minimum distance of $\CC(x; E; \pl_1, \ldots, \pl_s)$, respectively, 
we have
$$r d + \delta \geq n + r - \left(r{\cdot}g_Y
...
Given $X$ as before, the curve $Y = \text{Spec }\ell 
\times_{\text{Spec }k} X$ is a cyclic Galois cover of $X$ of
degree~$r$ for which the theory developed earlier applies.
...
if $\pl$ is a inert place of $X$ and $\ql$ is the unique place
...
\item its $\ell$-dimension $\delta_\ell$ by $\delta_\ell \coloneqq
\dim_\ell \CC$.
\end{itemize}
The Singleton bound now reads $d + \delta_\ell \leq n_\ell + 1$ where
...
\end{align}
from what we derive
$$d + \delta_\ell \geq n_\ell + 1 - \left(r{\cdot}(g_X-1)+1 
  + \frac {r} 2 \sum_{\pl \in X^\star} \frac{b_\pl{-}1}{b_\pl} \deg_X(\pl)\right).$$

\subsubsection*{Linearized Reed--Solomon codes}

To conclude this subsection, we consider the example where $X = \PP^1_k$ 
and $Y = \PP^1_\ell$, both viewed as curves over $\text{Spec }k$. We
have $g_X = 0$.
...
$\infty$ or to a irreducible monic polynomial in $k[t]$ (resp. in 
...
and~\eqref{eq:ell:dist} specialize to $\delta_\ell \geq m + 1$ and $d 
...
\subsection*{Acknowledgements}  This work was funded in part by the grants
ANR-21-CE39-0009-BARRACUDA and ANR-18-CE40-0026-01-CLap--CLap
from the French National Research Agency. The first author has also received funding from the  European Union’s Horizon 2020 research and innovation programme under the Marie Skłodowska--Curie grant agreement No 899987. 
