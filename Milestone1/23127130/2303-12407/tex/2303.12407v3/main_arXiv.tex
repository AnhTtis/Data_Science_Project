...
\begin{abstract}
    ...
    Our main result is a non-asymptotic upper bound of the 2-Wasserstein distance between the Gibbs distribution and the law of general Langevin-type algorithms based on the Liptser--Shiryaev theory and Poincar\'{e} inequalities.
...
Since the 2-Wasserstein or total variation distance between $\pi$ and the law of $X_{t}$ is convergent under mild conditions, we expect that Langevin-type algorithms inspired by $X_{t}$ should converge to $\pi$.
...
To see what difficulties we need to deal with, we review a typical analysis \citep{raginsky2017non} based on the smoothness of $U$, that is, twice continuous differentiability of $U$ and the Lipschitz continuity of $\nabla U$.
Firstly, twice continuous differentiability simplifies or plays significant roles in studies of functional inequalities such as Poincar\'{e} inequalities and logarithmic Sobolev inequalities \citep[e.g.,][]{bakry2008simple,cattiaux2010note}.
Since those functional inequalities are essential in the analysis of Langevin algorithms, the assumption that $U$ is of class $\mathcal{C}^{2}$ frequently appears in previous studies.
...
Our approaches to overcome the non-smoothness of $U$ are mollification, a classical approach to dealing with non-smoothness in differential equations, and the `misuse' of moduli of continuity for possibly discontinuous functions.
...
We can let $U\ast \rho_{r}$ be of class $\mathcal{C}^{2}$ and obtain bounds for the constant of Poincar\'{e} inequalities, which suffice to show the convergence of the mollified dynamics $\{\Bar{X}_{t}\}$ defined by the SDE
\begin{align*}
    \diff \Bar{X}_{t}^{r}=-\nabla \Bar{U}_{r}\left(\Bar{X}_{t}^{r}\right)\diff t +\sqrt{2\beta^{-1}}\diff B_{t},\ \Bar{X}_{0}^{r}=\xi
\end{align*} 
in 2-Wasserstein distance to the corresponding mollified Gibbs distribution owing to \citet{bakry2008simple}, \citet{liu2020poincare}, and \citet{lehec2021langevin}.
Since the convolution $\nabla \Bar{U}_{r}$ is Lipschitz continuous if the modulus of continuity of a representative $\nabla U$ is finite (the convergence to zero is unnecessary), a concise representation of the likelihood ratios between the mollified dynamics $\{\Bar{X}_{t}\}$ and $\{Y_{t}\}$ is available, and we can evaluate the Kullback--Leibler divergence under weak assumptions.
...
\citet{pereyra2016proximal}, \citet{chatterji2020langevin}, and \citet{lehec2021langevin} study Langevin-type algorithms under the convexity and the non-smoothness of potentials.
...
\citet{chatterji2020langevin} propose the perturbed Langevin Monte Carlo algorithm for non-smooth potential functions and show its performance to approximate the Gibbs distributions.
...
\citet{erdogdu2021convergence} show convergence guarantees of LMC under degenerate convexity at infinity and weak dissipativity of the potentials with H\"{o}lder continuous gradients, which are the assumptions for modified logarithmic Sobolev inequalities.
...
Since the assumptions (a) and (b) do not imply each other, our main contribution on the analysis of LMC is not to strengthen the previous studies but to broaden the theoretical guarantees of LMC under weak smoothness in a different direction.
...
The proposed algorithms are useful for sampling from the posterior distributions with some models in statistics and machine learning, whose corresponding potential functions are dissipative and weakly differentiable but neither convex nor continuously differentiable (e.g., some weakly differentiable non-convex losses with elastic net regularization).
...
\begin{remark}
    ...
    Note that we allow $\lim_{r\downarrow}\omega_{\nabla U}(r)\neq0$ and $\lim_{r\downarrow}\omega_{\widetilde{G}}(r)\neq0$.
\end{remark}
...
\end{align*}
where the last equality holds since (A1) and (A3) along with Lemmas \ref{lem:FMC:linearGrowth} and \ref{lem:FMC:quadGrowth} give the essential boundedness of $U$ and $\nabla U$ on any compact sets and we can approximate $\rho_{r}\in C_{0}^{1}(\R^{d})\cap W^{1,1}(\R^{d})$ with some $\{\varphi_{n}\}\subset\mathcal{C}_{0}^{\infty}(\R^{d})$.
...
\begin{theorem}[error estimate of general SG-LMC]\label{thm:sglmc}
    ...
    \end{align*}
    $C_{0},C_{1},C_{2},\kappa_{\infty}$ are the positive constants such that
    \begin{align*}
        C_{0}&:=\left(d+2\right)\left(\frac{\beta}{3}\left(\delta_{\variance,0}+\left\|\widetilde{G}\right\|_{\FMC}^{2}+\left((\omega_{\widetilde{G}}(1))^{2}+\delta_{\variance,2}\right)\kappa_{\infty}\right)+\frac{d}{2}\right)\\
        C_{1}&:=2\sqrt{4\kappa_{0}+\frac{32(b+m+d/\beta)}{m}+\frac{10}{1\wedge (\beta m/4)}}\\
        C_{2}&:=\sqrt{3^{\beta b/2}\left(\frac{3\psi_{2}}{m\beta}\right)^{d/2}\exp\left(\beta\left(\frac{25}{4}\left\|\nabla U\right\|_{\FMC}+U_{0}\right)+2\psi_{0}\right)}\\
    ...
    \end{align*}
    $c_{\rm P}(\Bar{\pi}^{r})$ is the constant of the Poincar\'{e} inequality for $\Bar{\pi}^{r}(\diff x)\propto \exp(-\Bar{U}_{r}(x))\diff x$ such that
    \begin{align*}
        c_{\rm P}(\Bar{\pi}^{r})
...
\subsection{A concise representation of Theorem \ref{thm:sglmc} and the growth of the constants of Poincar\'{e} inequalities}
...
Assuming that $f(\delta_{r},r,k,\eta)\le 1$ and $\eta\in(0,1\wedge \Tilde{m}/2((\omega_{\widetilde{G}}(1))^{2}+\delta_{\variance,2}))$, we obtain that for some $C\ge 1$ independent of $\delta_{r},r,k,\eta,d,\ucpi$,
\begin{align*}%\label{eq:asymp:sglmc}
        \Wasserstein_{2}\left(\mu_{k\eta},\pi\right)\le C\sqrt{d}\sqrt[4]{\left(d^{2}\frac{\omega_{\nabla U}(r)}{r}\eta+d\delta_{r,2}+\delta_{r,0}\right)k\eta+r\sqrt{d}}+e^{Cd}\exp\left(-\frac{k\eta}{C\ucpi}\right),
\end{align*}
with $\ucpi:=\sup_{r\in(0,1]}c_{\rm P}(\Bar{\pi}^{r})<\infty$.
Note that $\ucpi\le 1/\lambda$ with $\lambda>0$ if $U\in W_{\loc}^{1,1}$ is $\lambda$-strongly convex and $\nabla U$ is both a subgradient and a weak gradient satisfying (A3) since $\Bar{U}_{r}$ is also $\lambda$-strongly convex for all $r\in(0,1]$. 
We see the strong convexity of $\Bar{U}_{r}$ as follows:
\begin{align*}
    \Bar{U}_{r}\left(y\right)&=\int_{\R^{d}}U\left(y-z\right)\rho_{r}\left(z\right)\diff z\\
    &\ge \int_{\R^{d}}U\left(x-z\right)\rho_{r}\left(z\right)\diff z+\int_{\R^{d}}\left\langle \nabla U\left(x-z\right),(y-z)-(x-z)\right\rangle \rho_{r}\left(z\right)\diff z\\
    &\quad+\frac{\lambda}{2}\int_{\R^{d}}\left|(y-z)-(x-z)\right|^{2}\rho_{r}\left(z\right)\diff z\\
    &=\Bar{U}_{r}\left(x\right)+\left\langle \nabla\Bar{U}_{r}\left(x\right),y-x\right\rangle+\frac{\lambda}{2}\left|y-x\right|^{2}.
\end{align*}
In the worst case, however, $\ucpi$ for non-convex dissipative $U$ is of exponential growth in $d$ as shown in Theorem \ref{thm:sglmc}.
...
\begin{enumerate}
    ...
    \item[(B2)] $\nabla U(x)$ is uniformly continuous, that is, the modulus of continuity $\omega_{\nabla U}$ defined as
    \begin{align*}
...
We define a generalized inverse of $\omega_{\nabla U}$ as follows:
\begin{align*}
    \omega_{\nabla U}^{\dagger}\left(s\right):=\sup\left\{r\ge 0:\omega_{\nabla U}(r)\le s\right\}.
\end{align*}

We obtain the following result on the sampling complexity using this generalized inverse.
\begin{proposition}\label{prop:LMC:1}
    ...
    \begin{align*}
        \Bar{r}_{1}:=\omega_{\nabla U}^{\dagger}\left(\sqrt{\frac{\epsilon^{4}}{48C^{4}d^{2}\left(C\ucpi\left(\log\left(2/\epsilon\right)+Cd\right)+1\right)}}\right),\ \Bar{r}_{2}:=\frac{\epsilon^{4}}{48C^{4}d^{5/2}}.
    \end{align*}
...
\begin{align*}
    \max\left\{d^{2}\frac{\omega_{\nabla U}\left(r\right)}{r}k\eta^{2},\left(\omega_{\nabla U}\left(r\right)\right)^{2}k\eta,r\sqrt{d}\right\}\le \frac{\epsilon^{4}}{48C^{4}d^{2}},\ e^{Cd}\exp\left(-\frac{k\eta}{C\ucpi}\right)\le \frac{\epsilon}{2}.
\end{align*}
$r\sqrt{d}\le \epsilon^{4}/48C^{4}d^{2}$ is immediate.
...
We can apply Proposition \ref{prop:LMC:1} to the analysis of the sampling complexity of LMC with $\alpha$-mixture weakly smooth gradients \citep{chatterji2020langevin,nguyen2022unadjusted}.
...
Rough estimates of $r/\omega_{\nabla U}(r)$ by the inequalities $r/\omega_{\nabla U}(r)\ge r^{1-\alpha}/M$ and $\omega_{\nabla U}^{\dagger}(s)/\omega_{\nabla}(\omega_{\nabla U}^{\dagger}(s))\ge \omega_{\nabla U}^{\dagger}(s)/s\ge s^{1/\alpha-1}/M^{1/\alpha}$ for sufficiently small $r,s>0$ yield the sampling complexity
\begin{align*}
    k=\mathcal{O}\left(\frac{d^{4}\ucpi^{2}\left(\log\epsilon^{-1}+d\right)^{2}}{\epsilon^{4}}\left(\left(\frac{d^{2}\ucpi\left(\log\epsilon^{-1}+d\right)}{\epsilon^{4}}\right)^{\frac{1-\alpha}{2\alpha}}\vee \left(\frac{d^{5/2}}{\epsilon^{4}}\right)^{1-\alpha}\right)\right).
\end{align*}
This estimate and $\ucpi$ of exponential growth recover the complexity by \citet{raginsky2017non} under $U$ of class $\mathcal{C}^{2}$ and with the Lipschitz continuous gradient $\nabla U$.
Note that we only need to assume that $U$ is of class $\mathcal{C}^{1}$ and its gradient is Lipschitz continuous.

\subsection{The spherically smoothed Langevin Monte Carlo (SS-LMC) algorithm}
...
Then we see that for any $x\in\R^{d}$, $\E[{G}(x,a_{i\eta})]=\nabla \Bar{U}_{r'}(x)$,
\begin{align*}
...
\begin{enumerate}
    \item[(C1)] $U\in W_{\loc}^{1,1}(\R^{d})$.
    \item[(C2)] $|\nabla U(\zero)|<\infty$ and the modulus of continuity of $\nabla U$ is bounded, that is,
    \begin{align*}
        \omega_{\nabla U}(r):=\sup_{x,y\in\R^{d}:|x-y|\le r}\left|U(x)-U(y)\right|<\infty
    \end{align*}
    for some $r\in(0,1]$.
...
\begin{corollary}[error estimate of SS-LMC]
    Under (C1)--(C3) and (A6), there exists a constant $C\ge 1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $k\in\N$, $\eta\in(0,1\wedge m/(2(\omega_{\nabla U}(1))^{2}))$, $r\in(0,1]$, and $N_{B}\in\N$ with $(d^{2}(\omega_{\nabla U}(r)/r)\eta+(\omega_{\nabla U}(r))^{2}/N_{B})k\eta+r\sqrt{d}\le 1$, 
    \begin{align*}
        \Wasserstein_{2}\left(\mu_{k\eta},\pi\right)&\le C\sqrt{d}\sqrt[4]{\left(d^{2}\frac{\omega_{\nabla U}(r)}{r}\eta+\frac{(\omega_{\nabla U}(r))^{2}}{N_{B}}\right)k\eta+r\sqrt{d}}+e^{Cd}\exp\left(-\frac{k\eta}{C\ucpi}\right).
    \end{align*}
\end{corollary}

\subsubsection{The sampling complexity of SS-LMC}
We analyse the behaviour of SS-LMC; to see that we do not need the convergence $\omega_{\nabla U}(r)\downarrow 0$, we consider a rough version of the upper bound by replacing $\omega_{\nabla U}(r)$ with the constant $\omega_{\nabla U}(1)$.
\begin{corollary}
Under (C1)--(C3) and (A6), there exists a constant $C\ge1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $N_{B}\in\N$, $k\in\N$, $\eta\in(0,1\wedge m/(2(\omega_{\nabla U}(1))^{2}))$, and $r\in(0,1]$ with $\left(d^{2}r^{-1}\eta+N_{B}^{-1}\right)k\eta+r\sqrt{d}\le 1$,
\begin{align*}%\label{eq:asymp:sslmc}
    \Wasserstein_{2}\left(\mu_{k\eta},\pi\right)&\le C\sqrt{d}\sqrt[4]{\left(d^{2}r^{-1}\eta+N_{B}^{-1}\right)k\eta+r\sqrt{d}}+e^{Cd}\exp\left(-\frac{k\eta}{C\ucpi}\right).
\end{align*}
\end{corollary}
...
\begin{proposition}
    ...
    If $r=\epsilon^{4}/48C^{4}d^{5/2}$, $N_{B}\ge 48C^{4}d^{2}(C\ucpi(\log(2/\epsilon)+Cd)+1)/\epsilon^{4}$, and $\eta$ satisfies
    \begin{align*}
...
Since the complexities of $N_{B}$ and $k$ are given as $N_{B}=\mathcal{O}(d^{2}\ucpi(\log\epsilon^{-1}+d)/\epsilon^{4})$ and $k=\mathcal{O}(d^{6.5}\ucpi^{2}(\log\epsilon^{-1}+d)^{2}/\epsilon^{8})$, 
% \begin{align*}
...
% \end{align*}
we obtain the sampling complexity of SS-LMC as $N_{B}k=\mathcal{O}(d^{8.5}\ucpi^{3}(\log\epsilon^{-1}+d)^{3}/\epsilon^{12})$.
% \begin{align*}
...
\begin{enumerate}
    \item[(D1)] For all $\ell=1,\ldots,N$, $U_{\ell}\in W_{\loc}^{1,1}(\R^{d})$.
    ...
    \begin{align*}
        \sup_{x,y\in\R^{d}:|x-y|\le r}\left|U_{\ell}(x)-U_{\ell}(y)\right|\le \Hat{\omega}\left(r\right)<\infty.
    \end{align*}
...
SS-SG-LMC is useful for reducing computational complexity if $N\gg N_{B}$.


Similar to the discussion on SS-LMC, we observe that (D1)--(D3) yield (A1)--(A5).

\begin{corollary}[error estimate of SS-SG-LMC]
    Under (D1)--(D3) and (A6), there exists a constant $C\ge 1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $k\in\N$, $\eta\in(0,1\wedge m/(4(\Hat{\omega}(1))^{2}))$, $r\in(0,1]$, and $N_{B}\in\N$ with $(d^{2}(\Hat{\omega}(r)/r)\eta+d/N_{B})k\eta+r\sqrt{d}\le 1$, 
    \begin{align*}
        \Wasserstein_{2}\left(\mu_{k\eta},\pi\right)\le C\sqrt{d}\sqrt[4]{\left(d^{2}\frac{\Hat{\omega}(r)}{r}\eta+\frac{d}{N_{B}}\right)k\eta+r\sqrt{d}}+e^{Cd}\exp\left(-\frac{k\eta}{C\ucpi}\right).
    \end{align*}
\end{corollary}
...
\begin{corollary}
Under (D1)--(D3) and (A6), there exists a constant $C\ge1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $N_{B}\in\N$, $k\in\N$, $\eta\in(0,1\wedge m/(4(\Hat{\omega}(1))^{2}))$, and $r\in(0,1]$ with $\left(d^{2}r^{-1}\eta+dN_{B}^{-1}\right)k\eta+r\sqrt{d}\le 1$,
\begin{align*}%\label{eq:asymp:sssglmc}
    \Wasserstein_{2}\left(\mu_{k\eta},\pi\right)&\le C\sqrt{d}\sqrt[4]{\left(d^{2}r^{-1}\eta+dN_{B}^{-1}\right)k\eta+r\sqrt{d}}+e^{Cd}\exp\left(-\frac{k\eta}{C\ucpi}\right).
\end{align*}
\end{corollary}

We yield the following estimate of the sampling complexity, which is worse than that of SS-LMC by a factor of $\mathcal{O}(d)$.
\begin{proposition}
    Assume (D1)--(D3) and (A6) and fix $\epsilon\in(0,1]$.
    If $r=\epsilon^{4}/48C^{4}d^{5/2}$, $N_{B}\ge 48C^{4}d^{3}(C\ucpi(\log(2/\epsilon)+Cd)+1)/\epsilon^{4}$, and $\eta$ satisfies
    \begin{align*}
        \eta\le 1&\wedge \frac{m}{2\left(\Hat{\omega}(1)\right)^{2}}\wedge \frac{r\epsilon^{4}}{48C^{4}d^{4}(C\ucpi(\log(2/\epsilon)+Cd)+1)},
    \end{align*}
    then $\Wasserstein_{2}\left(\mu_{k\eta},\pi\right)\le \epsilon$ for $k=\lceil C\ucpi(\log(2/\epsilon)+Cd)/\eta\rceil$.
\end{proposition}

\subsection{On zeroth-order Langevin algorithms}

Let us consider a zeroth-order version of SS-LMC as an analogue to \citet{roy2022stochastic} with the following $G$ under (C1)--(C3):
\begin{align*}
...
\end{align*}
the symmetricity of $\rho$, the approximation of $\rho\in \mathcal{C}_{0}^{1}(\R^{d})\cap W^{1,1}(\R^{d})$, and the essential boundedness of $U$ and $\nabla U$ on compact sets by Lemmas \ref{lem:FMC:linearGrowth} and \ref{lem:FMC:quadGrowth} yield that for all $x\in\R^{d}$,
\begin{align*}
...
\begin{align*}
    ...
    &=\frac{32\Gamma(d/2+3)}{(d+2)\Gamma(d/2)\Gamma(3)}
...
\begin{align*}
    ...
    &\le 16\left(\frac{3}{2}\left\|\nabla U\right\|_{\FMC}+\frac{\left|x\right|}{2}\right)^{2}\int_{B_{1}(\zero)}\frac{\left|\nabla \rho\left(z\right)\right|^{2}}{\rho\left(z\right)}\diff z\\
    &= 2d(d+4)\left(\frac{3}{2}\left\|\nabla U\right\|_{\FMC}+\frac{\omega_{\nabla U}(1)\left|x\right|}{2}\right)^{2}\\
    &\le d(d+4)\left(9\left\|\nabla U\right\|_{\FMC}^{2}+\left(\omega_{\nabla U}(1)\right)^{2}\left|x\right|^{2}\right).
\end{align*}
...
\end{align*} 
yield (A5) with $\delta_{\bias,r,0}=\delta_{\bias,r,2}=0$, $\delta_{\variance,0}=9d(d+4)\left\|\nabla U\right\|_{\FMC}^{2}/2N_{B}$, and $\delta_{\variance,2}=d(d+4)(\omega_{\nabla U}(1))^{2}/2N_{B}$ if $r=r'$.
...
We can also consider a zeroth-order version of SS-SG-LMC with the potential $U$ in Eq.~\eqref{eq:emploss} and the following $G$ under (D1)--(D3):
\begin{align*}
...
\begin{align*}
    ...
    &=\frac{1}{NN_{B}}\sum_{j=1}^{N_{B}}\sum_{\ell=1}^{N}\int\left|\frac{U_{\ell}\left(x+r'z\right)-U_{\ell}\left(x\right)}{r'}\right|^{2}\frac{\left|\nabla \rho(z)\right|^{2}}{\rho(z)}\diff z\\
    &\le d(d+4)\max_{\ell=1,\ldots,N}\left(9\left\|\nabla U_{\ell}\right\|_{\FMC}^{2}+\left(\Hat{\omega}(1)\right)^{2}\left|x\right|^{2}\right).
\end{align*}
Hence (A5) for this $G$ holds with $\delta_{\bias,r',0}=\delta_{\bias,r',2}=0$, $\delta_{\variance,0}=9d(d+4)(\max_{\ell}\left|\nabla U_{\ell}(\zero)\right|+\Hat{\omega}(1))^{2}/2N_{B}$, and $\delta_{\variance,2}=d(d+4)(\Hat{\omega}(1))^{2}/2N_{B}$ if $r=r'$.
Therefore, this SG-LMC can achieve $\Wasserstein_{2}(\mu_{k\eta},\pi)\le \epsilon$ for any $\epsilon>0$, whilst the complexity is worse than that of SS-SG-LMC by a factor of $\mathcal{O}(d^{2})$.
...
Note that our non-asymptotic analysis needs the mollifiers of class $\mathcal{C}^{1}$ whose gradients have explicit $L^{1}$-bounds and whose supports are in the unit ball of $\R^{d}$, and it is nontrivial to obtain explicit $L^{1}$-bounds for the gradients of well-known $\mathcal{C}^{\infty}$ mollifiers.

\begin{remark}
    We need mollifiers of class $\mathcal{C}^{1}$ to let $U\ast\rho$ with $U\in W_{\loc}^{1,1}$ be of class $\mathcal{C}^{2}$ and give the bound for the constant of a Poincare inequality by \citet{bakry2008simple}; see Lemma \ref{lem:FMC:C2} and Proposition \ref{prop:BBCG08T14}.
\end{remark}
...
\begin{proof}
    ...
    \end{align*}
    and thus $\nabla \rho(x)$ is continuous for any $x\in\R^{d}$ by $\nabla \rho(x)=\zero$ for all $x$ with $|x|=1$.
...
\begin{proof}
    Since $p\in \mathcal{C}^{1}(\R^{d})$, the $L^{1}$-norm of the gradient equals the total variation: for arbitrary $R> 1$,
    \begin{align*}
    ...
    For all $\delta\in(0,1]\cap \Q$, by fixing $\varphi_{\delta}\in\mathcal{C}_{0}^{1}(B_{R}(\zero),\R^{d})$ such that $\varphi_{\delta}(x)=x$ for all $x\in B_{1-\delta}(\zero)$ and $\|\varphi_{\delta}\|_{\infty}\le 1$, we have
    \begin{align*}
...
\begin{proof}
    Lemma \ref{lem:FMC:localLipschitz} and $\Phi\in W_{\loc}^{1,1}$ give $\|\Phi\|_{L^{\infty}(B_{1}(\zero))}<\infty$ since $|\Phi(x)|<\infty$ for almost all $x\in\R^{d}$.
    Lemma \ref{lem:FMC:localLipschitz} also gives that for almost all $x\in\R^{d}$ and $y \in B_{1}(\zero)\cap B_{|x|}(x)$,
    \begin{align*}
...
\begin{proof}
    Since $\Phi$ and $\nabla \Phi$ are essentially bounded on any compact sets owing to Lemmas \ref{lem:FMC:linearGrowth} and \ref{lem:FMC:quadGrowth}, for some $\{\varphi_{n}\}\subset\mathcal{C}_{0}^{\infty}(\R^{d})$ approximating $\rho_{r}\in C_{0}^{1}(\R^{d})\cap W^{1,1}(\R^{d})$, $\nabla (\Phi\ast\rho_{r})=\Phi\ast\nabla \rho_{r}=\lim_{n}\Phi\ast\nabla \varphi_{n}=\lim_{n}\nabla\Phi\ast\varphi_{n}=\nabla\Phi\ast\rho_{r}$ and thus $\Phi\in\mathcal{C}^{2}(\R^{d})$ with $\nabla^{2}\Bar{\Phi}_{r}=(\nabla\Phi\ast\nabla\rho_{r})$. 
\end{proof}
...
\end{align*}
by change of the variables $z=y/r$ with $r^{d}\diff z=\diff y$ and Lemma \ref{lem:kernel:2}.
\end{proof}
...
Let us consider Poincar\'{e} inequalities of the probability measure $P_{\Phi}$ whose density is $(\int e^{-\Phi(x)}\diff x)^{-1}e^{-\Phi(x)}$ with lower bounded $\Phi\in\mathcal{C}^{2}(\R^{d})$ such that $\int e^{-\Phi(x)}\diff x<\infty$.
...
\subsection{A bound for the 2-Wasserstein distance by KL divergences}
...
\begin{lemma}[square integrability]\label{lem:RRT17L03}
    ...
    \begin{align*}
        \sup_{k\ge 0}\E\left[\left|Y_{k\eta}\right|^{2}\right]&\le \kappa_{0}+2\left(1\vee \frac{1}{\Tilde{m}}\right)\left(\Tilde{b}+\eta\left\|\widetilde{G}\right\|_{\FMC}^{2}+\delta_{\variance,0}+\frac{d}{\beta}\right)=:\kappa_{\infty}.
    \end{align*}
...
\begin{proof}%[Proof of Lemma \ref{lem:LD:exponential}]
    ...
    \begin{align*}
        ...
        \nabla^{2} V_{\alpha}(x)=2\alpha^{2} V_{\alpha}(x)xx^{\top}+2\alpha V_{\alpha}(x)I_{d}.
    \end{align*}
    ...
    It holds that
    \begin{align*}
        \Bar{\mathcal{L}}^{r}V_{\alpha}(x)&\le 2\alpha V_{\alpha}(x)\left\langle \nabla \Bar{U}_{r}(x),x\right\rangle +\alpha/\beta V_{\alpha}(x)\left(\alpha\left|x\right|^{2}+d\right)\\
        &\le 2\alpha V_{\alpha}(x)\left(\left(-\Bar{m}\left|x\right|^{2}+\Bar{b}\right) +\left(\alpha\left|x\right|^{2}+d\right)/\beta\right)\\
        &=2\alpha V_{\alpha}(x)\left(\left(\alpha/\beta-\Bar{m}\right)\left|x\right|^{2}+\Bar{b}+ d/\beta\right).
    \end{align*}
    Let $R^{2}= 2(\Bar{b}+d/\beta)/(\Bar{m}-\alpha/\beta)$ be a fixed constant and then we obtain for all $x$ with $|x|\ge R$,
    \begin{align*}
        \Bar{\mathcal{L}}^{r}V_{\alpha}(x)&\le -2\alpha \left(\Bar{m}+ d/\beta\right)V_{\alpha}(x)
    \end{align*}
    and trivially for all $x$ with $|x|<R$,
    \begin{align*}
        \Bar{\mathcal{L}}^{r}V_{\alpha}(x)&\le 2\alpha e^{2\alpha(\Bar{b}+d/\beta)/(\Bar{m}-\alpha/\beta)}\left(\Bar{b}+d/\beta\right)\\
        &\le 4\alpha e^{2\alpha(\Bar{b}+d/\beta)/(\Bar{m}-\alpha/\beta)}\left(\Bar{b}+d/\beta\right)-2\alpha \left(\Bar{b}+ d/\beta\right)V_{\alpha}(x).
    \end{align*}
    Thus we have for all $x\in\R^{d}$,
    \begin{align*}
        \Bar{\mathcal{L}}^{r}V_{\alpha}(x)\le -2\alpha \left(\Bar{b}+ d/\beta\right)V_{\alpha}(x)+4\alpha e^{2\alpha(\Bar{b}+d/\beta)/(\Bar{m}-\alpha/\beta)}\left(\Bar{b}+d/\beta\right).
    \end{align*}
    ...
    \begin{align*}
        &\E\left[e^{2\alpha \left(\Bar{b}+ d/\beta\right)(t\wedge\sigma_{n})}V_{\alpha}(X_{t\wedge\sigma_{n}})\right]\\
        &= \E\left[V_{\alpha}(X_{0})\right]\\
        &\quad+\E\left[\int_{0}^{t\wedge\sigma_{n}}\left(e^{2\alpha \left(\Bar{b}+ d/\beta\right)s}\Bar{\mathcal{L}}^{r}V_{\alpha}(X_{s})+2\alpha \left(\Bar{b}+ d/\beta\right)e^{2\alpha \left(\Bar{b}+ d/\beta\right)s}V_{\alpha}(X_{s})\right)\diff s\right].
    \end{align*}
    It holds that
    \begin{align*}
        &\E\left[\int_{0}^{t\wedge\sigma_{n}}\left(e^{2\alpha \left(\Bar{b}+ d/\beta\right)s}\Bar{\mathcal{L}}^{r}V_{\alpha}(X_{s})+2\alpha \left(\Bar{b}+ d/\beta\right)e^{2\alpha \left(\Bar{b}+ d/\beta\right)s}V_{\alpha}(X_{s})\right)\diff s\right]\\
    ...
    \begin{align*}
        &\E\left[e^{2\alpha \left(\Bar{b}+ d/\beta\right)t}V_{\alpha}(X_{t})\right]\\
        &=\E\left[\lim_{n\to\infty} e^{2\alpha \left(\Bar{b}+ d/\beta\right)(t\wedge\sigma_{n})}V_{\alpha}(X_{t\wedge\sigma_{n}})\right]\\
        &\le \liminf_{n\to\infty}\E\left[e^{2\alpha \left(\Bar{b}+ d/\beta\right)(t\wedge\sigma_{n})}V_{\alpha}(X_{t\wedge\sigma_{n}})\right]\\
        &\le \E\left[V_{\alpha}(X_{0})\right]+4\alpha e^{2\alpha(\Bar{b}+d/\beta)/(\Bar{m}-\alpha/\beta)}\left(\Bar{b}+d/\beta\right)\int_{0}^{t}e^{2\alpha \left(\Bar{b}+ d/\beta\right)s}\diff s.
    \end{align*}
    Therefore,
    \begin{align*}
        \E\left[V_{\alpha}(X_{t})\right]&\le \E\left[e^{\alpha |X_{0}|^{2}}\right]e^{-2\alpha \left(\Bar{b}+ d/\beta\right)t}+2 e^{2\alpha(\Bar{b}+d/\beta)/(\Bar{m}-\alpha/\beta)}\left(1- e^{-2\alpha \left(\Bar{b}+ d/\beta\right)t}\right)
    \end{align*}
...
\begin{proof}
    ...
    \end{align*}
    Lemmas \ref{lem:FMC:quadGrowth} and \ref{lem:RRT17L02}, $5a/2\le a^{2}+(5/4)^{2}$ for $a>0$, and $U(x)\ge 0$ give $\left\|U\right\|_{L^{\infty}(B_{1}(\zero))}<\infty$ and
    \begin{align*}
...
\begin{proof}%[Proof of Lemma \ref{lem:RRT17L07}]
    ...
    \end{align*}
    Now we see that the assumptions (LS1)--(LS4) of Propositions \ref{prop:LS01L0706} and \ref{prop:KL} are satisfied owing to (A1)--(A6), Lemma \ref{lem:RRT17L03}, and linear growths of $\widetilde{G}(w_{\lfloor \cdot /\eta\rfloor \eta})$ with respect to $\max_{i=0,\ldots,k}|w_{i\eta}|$ and $\nabla \Bar{U}_{r}(w_{t})$ with respect to $|w_{t}|$.
...
\begin{lemma}\label{lem:chi2}
    ...
    \begin{align*}
        \chi^{2}(\mu_{0}\|\Bar{\pi}^{r})\le 3^{\beta b/2}\left(\frac{3\psi_{2}}{m\beta}\right)^{d/2}\exp\left(\beta\left(\frac{25}{4}\left\|\nabla U\right\|_{\FMC}+U_{0}\right)+2\psi_{0}\right).
    \end{align*}
\end{lemma}
...
Lemma \ref{lem:RRT17L03}, the Cauchy--Schwarz inequality, and Fatou's lemma yield $\int_{\R^{d}}|x|\pi(\diff x)\le \sqrt{(b+d/\beta)/m}$ and the desired conclusion.
\end{proof}
