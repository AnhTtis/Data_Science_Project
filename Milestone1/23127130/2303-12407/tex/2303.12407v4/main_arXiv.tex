...
\end{align*} 
to the corresponding Gibbs distribution $\Bar{\pi}^{r}$ in 2-Wasserstein distance owing to \citet{bakry2008simple}, \citet{liu2020poincare}, and \citet{lehec2021langevin}.
...
Non-asymptotic analysis of Langevin-based algorithms under convex potentials has been one of the subjects of much attention and intense research \citep{Dal17,durmus2017nonasymptotic,durmus2019high}, and that without convexity has also gathered keen interest \citep{raginsky2017non,XCZ+18,erdogdu2018global}.
...
\begin{theorem}[error estimate of general SG-LMC]\label{thm:sglmc}
    ...
    \begin{align*}
        f(\delta_{r},r,k,\eta):=\left(C_{0}\frac{\omega_{\nabla U}(r)}{r}\eta+\beta\left(\delta_{r,2}\kappa_{\infty}+\delta_{r,0}\right)\right)k\eta+\beta r\left\|\nabla U\right\|_{\FMC}\left(\frac{3}{2}+\sqrt{\frac{b+d/\beta}{m}}\right),
    \end{align*}
    ...
    \begin{align*}
        ...
        C_{1}&:=2\sqrt{4\kappa_{0}+\frac{32(b+m+d/\beta)}{m}+\frac{10}{1\wedge (\beta m/8)}},\\
        C_{2}&:=\sqrt{3^{\beta b/2}\left(\frac{3\psi_{2}}{m\beta}\right)^{d/2}\exp\left(\beta\left(\frac{25}{8}\left\|\nabla U\right\|_{\FMC}+U_{0}\right)+2\psi_{0}\right)},\\
...
Note that $\ucpi\le 1/\beta\lambda$ with $\lambda>0$ if $U\in W_{\loc}^{1,1}(\R^{d})$ is $\lambda$-strongly convex with $\nabla U$ being both a subgradient and a weak gradient satisfying (A3) since it holds that
\begin{align*}
...
\end{align*}
and thus $\Bar{U}_{r}$ is also $\lambda$-strongly convex for all $r\in(0,1]$.
...
\section{The sampling complexities of Langevin-type algorithms}\label{sec:algorithms}
...
Define a generalized inverse of $\omega_{\nabla U}$ as follows: for any $s>0$,
\begin{align*}
...
\begin{corollary}
Under (C1)--(C3) and (A6), there exists a constant $C\ge1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $N_{B}\in\N$, $k\in\N$, $\eta\in(0,1\wedge (m/(4(\omega_{\nabla U}(1))^{2}))]$, and $r\in(0,1]$ with $\left(d^{2}r^{-1}\eta+N_{B}^{-1}\right)k\eta+r\sqrt{d}\le 1$,
...
% \end{align*}
we obtain the sampling complexity of SS-LMC as $N_{B}k=\mathcal{O}(d^{8.5}\ucpi^{3}(\log\epsilon^{-1}+d)^{3}/\epsilon^{12})$ or $N_{B}k=\widetilde{\mathcal{O}}(d^{11.5}\ucpi^{3}/\epsilon^{12})$, where $\widetilde{\mathcal{O}}$ ignores logarithmic factors.
% \begin{align*}
...
We first examine a squared loss for nonlinear regression (or equivalently nonlinear regression with Gaussian errors) such that $L_{\rm NLR}(x)=\sum_{\ell=1}^{N}\left(y_{\ell}-\phi_{\ell}\left(x\right)\right)^{2}/2\sigma^{2}$, where $N\in\N$, $\sigma>0$ is fixed, $y_{\ell}\in\R$, and $\phi_{\ell}\in W_{\loc}^{1,1}(\R^{d})$ with $\|\phi_{\ell}\|_{\infty}+\|\nabla \phi_{\ell}\|_{\infty}<\infty$ for some $\nabla \phi_{\ell}$
...
\begin{corollary}[error estimate of SS-SG-LMC]
    Under (D1)--(D3) and (A6), there exists a constant $C\ge 1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $k\in\N$, $\eta\in(0,1\wedge (m/(8(\Hat{\omega}(1))^{2}))]$, $r\in(0,1]$, and $N_{B}\in\N$ with $(d^{2}(\Hat{\omega}(r)/r)\eta+d/N_{B})k\eta+r\sqrt{d}\le 1$, 
    \begin{align*}
...
\begin{corollary}
Under (D1)--(D3) and (A6), there exists a constant $C\ge1$ independent of $N_{B},r,k,\eta,d,\ucpi$ such that for all $N_{B}\in\N$, $k\in\N$, $\eta\in(0,1\wedge (m/(8(\Hat{\omega}(1))^{2}))]$, and $r\in(0,1]$ with $\left(d^{2}r^{-1}\eta+dN_{B}^{-1}\right)k\eta+r\sqrt{d}\le 1$,
...
We use the following result on the fundamental theorem of calculus for functions in $W_{\loc}^{1,1}(\R^{d})$.
...
\begin{remark}
    We need mollifiers of class $\mathcal{C}^{1}$ to let $U\ast\rho$ with $U\in W_{\loc}^{1,1}(\R^{d})$ be of class $\mathcal{C}^{2}$ and give a bound for the constant of a Poincare inequality by \citet{bakry2008simple}; see Lemma \ref{lem:FMC:C2} and Proposition \ref{prop:BBCG08T14}.
\end{remark}
...
\begin{proof}%[Proof of Lemma \ref{lem:FMC:localLipschitz}]
    ...
    \begin{align*}
        ...
        &\le \left(\left|\nabla \Phi\left(\zero\right)\right|+\omega_{\nabla\Phi}(1)\left(1+\frac{r}{2}+|x|\right)\right)\int_{\R^{d}}\left|x-y\right|\rho_{r}(x-y)\diff y\\
        &\le r\left(\left|\nabla \Phi\left(\zero\right)\right|+\omega_{\nabla\Phi}(1)\left(1+\frac{r}{2}+|x|\right)\right).
    \end{align*}
...
\begin{proof}
    Lemma \ref{lem:FMC:localLipschitz} and $\Phi\in W_{\loc}^{1,1}(\R^{d})$ give $\|\Phi\|_{L^{\infty}(B_{1}(\zero))}<\infty$ since $|\Phi(x)|<\infty$ for almost all $x\in\R^{d}$.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\le \liminf_{n\to\infty}\E\left[\frac{\beta^{2}}{4}\int_{0}^{T\wedge\sigma_{n}}\left|\left(b^{P}-b^{Q}\right)\left(s,a,X^{P}\right)\right|^{2}\diff s\right]\\
...
$(X_{t},B_{t}')$ denotes the unique weak solution of the following SDE under some stochastic basis and (A3) with fixed $\nabla U$ (Lemma \ref{lem:FMC:linearGrowth} yields the existence and uniqueness):
\begin{align}\label{eq:LD}
    \diff X_{t}=-\nabla U\left(X_{t}\right)+\sqrt{2\beta^{-1}}\diff B_{t}',\ X_{0}=\xi,
\end{align}
...
\begin{align}\label{eq:sLD}
    \diff \Bar{X}_{t}^{r}=-\nabla \Bar{U}_{r}\left(\Bar{X}_{t}^{r}\right)+\sqrt{2\beta^{-1}}\diff B_{t},\ \Bar{X}_{0}^{r}=\xi,
\end{align}
and $\Bar{\nu}_{t}^{r}$ represents the probability measure of $\Bar{X}_{t}^{r}$.
% We define $\Bar{\pi}^{r}$, a probability measure on $(\R^{d},\Borel(\R^{d}))$, as
% \begin{align*}
...
\begin{align*}
    ...
    \ge \int_{\R^{d}}\left(m|x-y|^{2}-b\right) \rho_{r}(y)\diff y\\
    &\ge \int_{\R^{d}}\left(\frac{1}{2}m|x|^{2}-m|y|^{2}-b\right) \rho_{r}(y)\diff y
...
% \begin{lemma}[uniform $L^{2}$ moments]\label{lem:RRT17L03}
%     Assume that (A1)--(A6) hold.
%     (1) For all $0<\eta\le 1\wedge (\Tilde{m}/2((\omega_{\widetilde{G}}(1))^{2}+\delta_{\variance,2}))$,
%     \begin{align*}
%         \sup_{k\ge 0}\E\left[\left|Y_{k\eta}\right|^{2}\right]&\le \kappa_{0}+2\left(1\vee \frac{1}{\Tilde{m}}\right)\left(\Tilde{b}+\left\|\widetilde{G}\right\|_{\FMC}^{2}+\delta_{\variance,0}+\frac{d}{\beta}\right)=:\kappa_{\infty}.
%     \end{align*}
%     (2) It holds that for any $t\ge0$, 
...
\begin{proof}%[Proof of Lemma \ref{lem:RRT17L03}]
    ...
    and if $\gamma\in(0,1)$,
    \begin{align*}
        \E\left[\left|Y_{k\eta}\right|^{2}\right]&\le \gamma^{k}\E\left[\left|Y_{0}\right|^{2}\right]+\frac{2\eta \Tilde{b}+2\eta^{2}\left\|\widetilde{G}\right\|_{\FMC}^{2}+2\eta^{2}\delta_{\variance,0}+\frac{2\eta d}{\beta}}{2\eta \Tilde{m}-2\eta^{2}\left(\left(\omega_{\widetilde{G}}(1)\right)^{2}+\delta_{\variance,2}\right)^{2}}\\
    ...
    \begin{align*}
        ...
        &= e^{-2mt}\kappa_{0}+\frac{b+d/\beta}{m}\left(1-e^{-2mt}\right).
    \end{align*}
    We obtain the conclusion.
\end{proof}
...
\begin{proof}
    ...
    We set a Lyapunov function $V(x)=e^{m\beta|x|^{2}/8}$.
    Since $-\langle \nabla \Bar{U}_{r}(x),x\rangle \le -(m/2)|x|^{2}+(b+m)$ for all $x\in\R^{d}$, it holds that
    \begin{align*}
        ...
        &\le \left(\frac{dm\beta}{4}+\frac{\left(m\beta\right)^{2}}{16}\left|x\right|^{2}-\frac{m^{2}\beta^{2}}{8}\left|x\right|^{2}+\frac{m\beta^{2}}{4}\left(b+m\right)\right)V(x)\\
        &= \left(\frac{m\beta\left(d+\left(b+m\right)\beta\right)}{4}-\frac{m^{2}\beta^{2}}{16}|x|^{2}\right)V(x).
    \end{align*}
    ...
    \end{align*}
    Lemma \ref{lem:FMC:quadGrowth}, $5a/2\le a^{2}+(5/4)^{2}$ for $a>0$, and $U(x)\ge 0$ give $\left\|U\right\|_{L^{\infty}(B_{1}(\zero))}<\infty$ and
...
\begin{lemma}\label{lem:chi2}
    ...
    \begin{align*}
        \chi^{2}(\mu_{0}\|\Bar{\pi}^{r})\le 3^{\beta b/2}\left(\frac{3\psi_{2}}{m\beta}\right)^{d/2}\exp\left(\beta\left(\frac{25}{8}\left\|\nabla U\right\|_{\FMC}+U_{0}\right)+2\psi_{0}\right).
    \end{align*}
\end{lemma}
...
\begin{lemma}[moments of the Gibbs distribution $\pi$]\label{lem:momentGibbs}
    Under (A1), (A3), and (A4), for any non-negative measurable function $f:\R^{d}\to[0,\infty)$, it holds
    \begin{align*}
        \int_{\R^{d}}f\left(x\right)\pi\left(\diff x\right)\le \liminf_{n\to\infty}\E\left[f\left(X_{n}\right)\right].
    \end{align*}
\end{lemma}

\begin{proof}
    Theorem 3.3.4 of \citet{kulik2017ergodic} yields the existence and uniqueness of the invariant probability measure of $X_{t}$ and its exponential ergodicity in total variation distance.
    Therefore, it suffices to see that $\pi$ is an invariant measure of $X_{t}$ or equivalently $\int \mathcal{L}\phi(x)\pi(\diff x)=0$ for all $\phi\in\mathcal{C}_{b}^{2}(\R^{d})$ with the generator $\mathcal{L}:=\beta^{-1} \Delta-\langle \nabla U,\nabla f\rangle$ owing to Fatou's lemma and Skorokhod's representation theorem.
    
    Since $U\in W_{\loc}^{1,1}(\R^{d})$, $U(x)\ge0$, and the derivative of $g(a)=\exp(-a)$ on an open neighbourhood of $[0,\infty)$ is bounded, $\exp(-\beta U)$ has a weak derivative $-\beta\nabla U\exp(-\beta U)$ and for all $\phi\in \mathcal{C}_{b}^{2}(\R^{d})$, $\int \Delta \phi(x)\exp(-\beta U(x))\diff x=\int \beta\langle \nabla U(x),\nabla \phi(x)\rangle \exp(-\beta U(x))\diff x$ by approximation of $\phi$.
...
Lemmas \ref{lem:RRT17L03} and \ref{lem:momentGibbs} and the Cauchy--Schwarz inequality yield $\int_{\R^{d}}|x|\pi(\diff x)\le \sqrt{(b+d/\beta)/m}$ and the desired conclusion.
\end{proof}
...
\end{align*}
We fix $\lambda=1\wedge (\beta m/8)$ and then Lemma \ref{lem:LD:exponential} leads to
\begin{align*}
    ...
    &\le \left(4\kappa_{0}+\frac{32(b+m+d/\beta)}{m}+\frac{10}{1\wedge (\beta m/8)}\right)^{\frac{1}{2}}.
\end{align*}
...
(3)
Thirdly, we consider a bound for $\Wasserstein_{2}(\Bar{\nu}_{k\eta}^{r},\Bar{\pi}^{r})$.
Proposition \ref{prop:BV05C23} gives
\begin{align*}
...
We fix $\lambda=1\wedge (\beta m/8)$ and then Lemmas \ref{lem:LD:exponential} along with Fatou's lemma leads to
\begin{align*}
    C_{\Bar{\pi}^{r}}
    &\le \left(\frac{32(b+m+d/\beta)}{m}+\frac{10}{1\wedge (\beta m/8)}\right)^{\frac{1}{2}}.
\end{align*}
