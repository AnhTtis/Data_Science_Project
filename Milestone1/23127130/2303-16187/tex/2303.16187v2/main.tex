...
Note that, while this technique is related to text-conditional image generation, we are instead applying it to improved {\em unconditional} image generation. We call the resulting model a Visual Chain-of-Thought Diffusion Model (VCDM) and release code at \url{https://github.com/plai-group/vcdm}.
...
Sampling each CLIP embedding from our auxiliary model takes only 35ms. Sampling from our image model takes 862ms and so there is only a $4\%$ increase in inference time when using VCDM vs EDM.
