...
\title{A theoretical model for \enface optical coherence tomography imaging and its application to volumetric differential contrast imaging}
...
\begin{abstract}
	A new formulation of lateral optical coherence tomography (OCT) imaging process and a new differential contrast method designed by using this formulation are presented.
...
Several analytic models of OCT imaging have been developed. % \cite{Schmitt:93, Schmitt:97, Thrane:00}.
...
In this model, signals of single-backscattered light and multiple-backscattered light from a sample are formulated individually \cite{Schmitt:97}.
Additionally, it was clarified that speckle is caused by the interference of light backscattered by many scatterers in the sample.
Based on this model, a Monte Carlo method was developed \cite{Smithies_1998,Yao_1999}.
This Monte Carlo simulation accurately numerically simulates an OCT image with a specifically defined sample structure.
In recent years, another promising simulation method using Maxwell's equations has been introduced \cite{Munro:15, Brenner:16}.
An extension of this method, the pseudo-spectral time domain (PTSD) method, enables the realistic simulation of three-dimensional OCT imaging \cite{Munro:16}. 

Aforementioned approaches aimed to recapiture accurate and realistic OCT signals.
By contrast, we aim to establish a simplified theoretical framework that enables the intuitive understanding of the OCT imaging process and the interpretation of an \enface OCT image.
...
By actively using this theoretical framework, we design and experimentally demonstrate an OCT-based volumetric differential contrast (VDC) imaging method (Sections \ref{sec:VDC}).
...
Another example relates to the specific imaging condition required for computational multi-directional OCT \cite{Oida_2021}.
...
The slowly varying refractive index is represented by $n(x,y,z)$, in which infinitely small scatterers are dispersed.
The position of the $i$-th scatterer is $(x_i, y_i, z_i)$ and all the scatterers have an identical refractive index of $n_s$.
The green region in the figure represents the coherence gate.
In our formulations for OCT (Section \ref{sec:OctFormulation}) and VDC (Section \ref{sec:DSM}), we consider \enface images in which scattering only within this coherence gate contributes to imaging.
We assume that the probe beam is incident from right above of the sample and only the backscattered light directed to the incident direction contributes to the imaging process; that is, backscattered light in other directions is rejected by a pinhole, which is the single-mode fiber tip of the interferometer's probe arm.
...
We assume that a Gaussian beam is incident on the sample and there are no higher-order aberrations.
...
Throughout this paper, we use $\bm{x}$ and $z$ to represent the position in the object (sample) space, and $\bmxp=(x',y')$ and $z'$ to represent the position in the image space.
...
$B(\bm{x}, z_0)$ is a quantity defined by the refractive indexes of the medium $n(x,y,z)$ and the scatterer $n_s$.
For the case of sufficiently small scatterers, $B(\bm{x}, z_0)$ can be considered as the scattering potential 
$B(\bm{x}, z_0) \simeq \cbra{2\pi n(\bm{x}, z_0)/\lambda_0} \left[\left\{n_s/n(\bm{x}, z_0)\right\}^2-1\right]$, where $\lambda_0$ is the central wavelength of the probe beam. % \cite{Yasuhiko:22}.
...
\begin{equation}
	\phi_i = k_0\, n\, (z_i-z_0),
\end{equation}
where $k_0 = 2 \pi/\lambda_0$ is the wavenumber and $n$ is the refractive index of the sample around the scatterers.
...
%
In this formulation, each term in the first group (the first line of the equation, for $i = j$) is the numerical interaction between contributions from the same scatterer; hence, we refer to this group as an auto-interaction (AI) group.
By contrast, each term in the second group (the second line, for $i \neq j$) is the numerical interaction between contributions from different scatterers.
...
$\phi_{pq}$ is a phase that is originated from $\phi_i-\phi_j$ in Eq.\@ (\ref{eq:OctIntensityGrouped}) and is unpredictable and random in practice, and $\phi_{pq} = -\phi_{qp}$.
The first line of Eq.\@ (\ref{eq:OctInterContinuous}) corresponds to the AI group [the first line of Eq.\@ (\ref{eq:OctIntensityGrouped})] and the second to fourth lines of Eq.\@ (\ref{eq:OctInterContinuous}) correspond to the CI group [the second line of Eq.\@ (\ref{eq:OctIntensityGrouped})].
...
\begin{multline}
	...
	+ \left[ \iiiint D'(\bmxpp, \bmxqq) B(\bmxpp) B(\bmxqq) 
	e^{i(\phi_{pq})} \right. 
	\aPsf{\bmxpp - \bmxp} \aPsf{\bmxp - \bmxqq} 
	e^{i \left[\varphi \left( \bmxp - \bmxqq \right) 
		- \varphi \left(\bmxp - \bmxqq \right)     \right]}  \\
	\left. - \iint D(\bmxpp) B^2(\bmxpp) 
	P_a^2 \rbra{\bmxp - \bmxpp} d \bmxpp \right].
\end{multline}
The first integral (i.e., the AI part) of Eq.\@ (\ref{eq:OCTInt_integral}) is ``an imaging equation of the intensity-OCT image'' in continuous form.
...
The complex Gaussian PSF is obtained from a Gaussian probe beam.
...
\begin{multline}
	...
	\exp\abra{- \frac{(\bmxi-\bmxj)^2}{w^2}}
        ...
        \cdot \rbra{\bmxp - \frac{\bmxi + \bmxj}{2}}} .
	\label{OctInt_Discrete_PSF}
\end{multline}
We omit depth positions for simplicity.
The first line of this equation corresponds the AI group, while second and third lines correspond to the CI group.
...
\end{multline}
where the last line, which is identical to the the AI group, was subtracted from the second integral, so that the last three lines of the equation represent the CI group.
...
\begin{multline}
	...
	\exp\abra{- \frac{4}{w^2} \left(\bmxp - \bmxpp \right)^2} d \bmxpp \\
	+\frac{2 w_0^4}{w^4}
	\iiiint D'(\bmxpp,\bmxqq)  B(\bmxpp) B(\bmxqq) \exp\abra{- \frac{(\bmxpp-\bmxqq)^2}{w^2}}
	\exp\abra{- \frac{4}{w^2} \rbra{\bmxp - \frac{\bmxpp+\bmxqq}{2}}^2} \\
	\times
	\cos\abra{\phi_{pq} + \frac{2 n k_0}{R} 
        \rbra{\bmxpp - \bmxqq} \cdot \rbra{\bmxp - \frac{\bmxpp + \bmxqq}{2}}}  d \bmxpp d \bmxqq\\
	%%%%%
	-\mathrm{the\,\,\, first\,\,\, line \,\,\, of \,\,\, R.H.S.}
...
All the equations suggest that the OCT intensity consists of AI, that is, the first summation or integral of each equation, and CI, that is, the residual terms. 
...
And hence, the AI is the incoherent image of the object.
%%%%%%%%%% LOGIC MEMO %%%%%%%%%%%%%%
%- There are AI and CI.
...
To intuitively understand the image property of CI, we split the product parts inside the summation or integral into two parts: the object to be measured ($B^2(\bmxi)$ or $D(\bmx)B^2(\bmx)$) and the residual part.
...
Fourth, the frequency and phase offset, that is, the shift of the cosine wave, depend on the positions of the scatterers of the scatterer pair; hence, they are practically unpredictable and random.
...
\begin{multline}
	...
	B(\bmxqq) e^{i (\phi_{pq})} \aPsf{\bmxp - \bmxpp+\frac{\deltax}{2}}
	\aPsf{\bmxp - \bmxqq - \frac{\deltax}{2}} \\
	\qquad \times 
	\exp{i \left[ \varphi \left(\bmxp - \bmxpp+\frac{\deltax}{2} \right)
		-  \varphi \left(\bmxp - \bmxqq-\frac{\deltax}{2} \right) \right]} d\bmxpp d\bmxqq \\
	-\mathrm{the\,\,\, first\,\,\, integral \,\,\, of \,\,\, R.H.S.},
	\label{eq:s1s2_cont}
\end{multline}
...
%The detailed derivation of this equation is in the Supplementary document.
...
\begin{multline}
	...
	\iiiint D'(\bmxpp,\bmxqq) B(\bmxpp) B(\bmxqq) 
         \\
	\times	\exp\abra{- \frac{2}{w^2} (\bmxpp - \bmxqq) 
        \cdot\rbra{\frac{\bmxpp+\bmxqq}{2} -\deltax}}  
        e^{i \phi_{pq}}
        \\
        \times \exp\abra{- \frac{4}{w^2} \rbra{\bmxp - \frac{\bmxpp+\bmxqq}{2}}^2} 
        \exp i\abra{\frac{2 n k_0}{R} 
	...
	\label{eq:s1s2_Gauss}
\end{multline}
Similar to the conventional intensity-OCT image [Eq.\@ (\ref{eq:OCTInt_integral})], this \enface signal consists of the AI part [the first two lines of Eq.\@ (\ref{eq:s1s2_Gauss})] and the CI part (the next four lines).
...
%Again, similar to the conventional intensity-OCT, CI part is practically random.
...
\begin{multline}
	...
	\exp \rbra{-\frac{\deltax^2}{w^2}}
	\iiiint D'(\bmxpp,\bmxqq) B(\bmxpp) B(\bmxqq)  \\
	\times	\exp\abra{- \frac{2}{w^2} (\bmxpp-\bmxqq)\cdot \rbra{ \frac{\bmxpp+\bmxqq}{2} -\deltax}}  
	\exp\abra{- \frac{4}{w^2} \rbra{\bmxp - \frac{\bmxpp+\bmxqq}{2}}^2} \\
	\times \sin \abra{\phi_{pq} + \frac{2 n k_0}{R} 
        \rbra{\bmxpp - \bmxqq - \deltax }
        \cdot \rbra{\bmxp - \frac{\bmxpp + \bmxqq}{2}}   
	}  d \bmxpp d \bmxqq\\
	-\mathrm{the\,\,\, first\,\,\, integral \,\,\, of \,\,\, R.H.S.}
...
The last four lines of Eq.\@ (\ref{eq:VdicFull}) correspond to the CI part.
Similar to the case of the intensity-OCT formula, $\phi_{pq}$ and pseudo-PSF are practically random; hence, CI becomes a random speckle pattern.
...
The axial resolution was 3.1 \um in air and the in-focus lateral resolution was 4.9 \um.
...
\begin{figure}
	\centering\includegraphics[width=13cm]{IntensityImage.pdf}
	\caption{
...
\begin{figure}
	\centering\includegraphics[width=13cm]{volumeVDC.pdf}
	\caption{
        ...
        %\ktcom{Images for the media files is in the "Media file" folder in the repository.}
...
To summarize, the structural phase artifact appears only when the structure and defocus exist.
...
Oida \etal demonstrated computational multi-directional (CMD)-OCT for qualitatively visualizing the microscopic phase structure of the sample \cite{Oida_2021}.
...
\begin{align}
	...
	\aPsf{\bmxp -\bmxj - \frac{\deltax}{2}}  
        e^{i \left[\varphi \rbra{\bmxp -\bmxi + \frac{\deltax}{2}}
        - \varphi \rbra{\bmxp -\bmxj - \frac{\deltax}{2}} \right]}.
        \label{eq:VDC_discrete}
\end{align}	
The first summation of the final expression (the third last line of the equation) is for the case of $i=j$, i.e., for the AI group.
The second summation (the last two lines of the equation) is for the case of $i \neq j$, that is, the CI group.
...
\begin{multline}
	...
	= \iint \sum_{i=1}^{N} \delta \rbra{\bmxpp - \bmxi}
        ...
        \aPsf{\bmxp - \bmxpp - \frac{\deltax}{2}}
        \\
        \times \exp {i \left[\varphi \left(\bmxp - \bmxpp+\frac{\deltax}{2} \right)    - \varphi \left(\bmxp - \bmxpp-\frac{\deltax}{2} \right) \right]}
        d\bmxpp \\  
         + \iiiint  \sum_{i=1}^{N} \sum_{j=1}^{N} 
         \delta \rbra{\bmxpp - \bmxi, \bmxqq - \bmxj}
         B(\bmxpp) B(\bmxqq) e^{i(\phi_{pq})} \\
	\times \aPsf{\bmxp - \bmxpp+\frac{\deltax}{2}}
	\aPsf{\bmxp - \bmxqq - \frac{\deltax}{2}}
	e^{i \left[ \varphi 
