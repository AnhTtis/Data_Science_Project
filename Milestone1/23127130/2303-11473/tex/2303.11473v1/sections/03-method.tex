...
Our video codec proxy, shown in Figure~\ref{fig:diagram}, maps an input group of images (like a group of pictures, or GOP) to an output group of images, plus a bit rate for each image.  
%This contrasts to sandwiched image compression, in which the image codec proxy maps a single input image to a single output image plus a bit rate for the image \cite{guleryuz2021sandwiched,guleryuz2022sandwiched}.  
The details of our video codec proxy 
%where  
%You will see that 
%we re-use the image codec proxy of \cite{guleryuz2021sandwiched,guleryuz2022sandwiched} as one of the components 
...
\begin{figure}[t]
    ...
    Note the substantially improved definition on the crowd and the building. While not shown here the sandwich system also obtains significant improvements over neural-post-processed HEVC LR %\cite{guleryuz2022sandwiched} 
    thanks to the information embedded by the neural pre-processor. }
...
As mentioned in the Introduction, the pre- and post-processors, and the loop filter in the video codec proxy, are trained to minimize the total rate-distortion Lagrangian $\sum_{i,t}D_i(t)+\lambda R_i(t)$, where $D_i(t)$ and $R_i(t)$ are the distortion and rate of frame $t$ in clip $i$. The rate term in particular serves to encourage the pre- and post-processors to produce temporally consistent neural codes, since neural codes that move according to our ``ground truth'' motion flow field will be well predicted, leading to smaller residuals and rate terms.  We do indeed observe that the resulting neural codes seem to have some degree of temporal consistency, as shown in Figure~\ref{fig:subjective_YUV}. Note that the overall mapping from the input images through the pre-processor, video codec proxy, post-processor, and loss function is differentiable.
% Note that the overall video coding proxy is a differentiable mapping from the input group of images to the output group of images and their bit rates.

% Designing a video codec proxy is much more challenging than designing an image codec proxy due to separate encoding mechanisms for I- and P-frames and motion compensation steps. In this work, we design and implement the proxy given in Figure~\ref{fig:diagram}. Notice that the video codec proxy performs the frame compression through the image codec proxy that was implemented in the prior work \cite{guleryuz2021sandwiched}. The additional features in this proxy are motion compensation steps while encoding the P-frames, which requires access to motion flows. Our one option would be to learn how to extract motion flows in the training loop for the best compression performance. However, this would overcomplicate the already-involved end-to-end scheme. Instead of generating the motion flows in the training loop, we generate the flows prior to training using UFlow \cite{lodha1996uflow}, state-of-the-art in optical flow. However, notice that we do not have access to the bottlenecks (output of the pre-processors) prior to training. Therefore, we generate the flows on the original input frames, assuming that the motion will be preserved in the bottlenecks. We test this assumption and verify that the motion information is indeed preserved to some extent even with the sandwiched image compressor (see Figure~\ref{fig:modulation_patterns}). To summarize, we have a video codec proxy that uses an image codec proxy for each frame while performing an extra motion compensation step for the P-frames using the pre-generated motion flow vectors.


% \berivan{don't forget to mention the architectures.}
%\begin{figure}
%    \centering
%    \subfigure[\em ]{\includegraphics[width=0.45\linewidth]{ICIP/figures/sand_image_on_video_original.png}}
%    \subfigure[\em ]{\includegraphics[width=0.45\linewidth]{ICIP/figures/sand_image_on_video_bottleneck.png}}
%    \caption{Original frame and the bottleneck frame extracted from the sandwiched image compressor while compressing the frames of a video. Notice the modulation patterns in the bottleneck frame.}
%    \label{fig:modulation_patterns}
%    \vspace{-0.2in}
%\end{figure}