...
The last decade 
%we have 
has witnessed neural image compression methods \cite{balle2016end_pcs} gradually outperforming traditional image compressors such as JPEG2000 and HEIC \cite{sullivan2012overview} and replacing them as state-of-the-art image compressors. The success of neural image compressors is due to their ability to perform complex non-linear transforms, which are learned end-to-end through back-propagation \cite{balle2020nonlinear}. More recently, the first learned video compressors have been proposed \cite{rippel2019learned} and research on improving neural video compressors has accelerated \cite{agustsson2020scale, hu2021fvc, rippel2021elf, yang2020hierarchical}. The existing strategies focus mainly on learning how to predict flows, 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/new_figures/yuv_subjective_6_D=7.63_R=0.50.drawio.png}
    \vspace{-0.2in}
...
\begin{figure*}[t]
    \centering
     \vspace{-0.2in}
    \includegraphics[width=0.8\linewidth]{figures/sandwiched_video_diagram.jpg}
        \vspace{-0.25in}
...
% Throughout the manuscript, we refer to the output of the pre-processor network $L_i(\cdot)$ as the \emph{bottleneck} and input to the post-processor network $\hat{L}_i(\cdot)$ as the \emph{reconstructed bottleneck}.
 ...
 (pre and post combined), a reduction in complexity by two orders of magnitude over state-of-the-art neural video compressors.  Few works prior to ours sandwich a standard video codec between neural processors. Those that do, e.g. \cite{EusebioAP20,QiuLD21,Andreopoulos22}, do so in such a way that the pre- and post-processors may be used independently --- thus do not take full advantage of the communication available between pre- and post-processors. Our contributions can be summarized as:
 
%  (1) The video coding will be performed on the pre-processed (or transformed) frames, which should preserve the motion information to get any benefit from the video codec.
 
%  (2) There are multiple blocks in the video codec performing complicated operations such as warping, flow estimation, and residual compensation. In fact, the large number of blocks to be designed and optimized for the best rate-distortion performance resulted in many different video coding standards such as ISO/IEC MPEG series \cite{tudor1995mpeg, haskell1996digital, sikora1997mpeg, li2001overview}, ITU-T H.26x series \cite{wiegand2003overview, sullivan2012overview, sze2014high, sullivan2004h, vetro2011overview}, AVS series \cite{yu2009overview, ma2013overview, zhang2019recent}, VP9 \cite{mukherjee2013latest}, and AV1 \cite{chen2018overview, han2021technical}.
 
%  (3) The video codec needs the output of the neural pre-processors from the adjacent frames (previous and perhaps the next frame) as well to encode and decode the current frame. 
 
%  We design our sandwiched video compression framework while trying to resolve these challenges in the most computation-efficient way. Our contributions can be summarized as follows:
...
(2) We propose a differentiable video codec proxy that can be used effectively 
     %in various other frameworks as well that require a video codec 
     in the training loop.
     
(3) We demonstrate that the sandwiched HEVC 
     %in YUV 4:4:4 format in low-resolution mode and sandwiched HEVC in YUV 4:0:0 format show around 6.5 dB and 8 dB improvements over the standard HEVC in the same format and mode, respectively.
     obtains 6.5 dB improvement over the standard HEVC 4:4:4 in low-resolution transport (8 dB improvement over 4:0:0 when coding color over gray-scale.)
     Moreover, when optimized for and tested with a perceptual similarity metric, Learned Perceptual Image Patch Similarity (LPIPS), we observe $\sim 30 \%$ improvements in rate at the same quality over the standard HEVC 4:4:4.
 

 %As shown in Figure 1, the lightweight neural networks serve as the pre- and post-processors to perform non-linear transforms before and after the standard image codec, respectively. This provides more efficient training and inference due to the simplicity of the neural networks but still enjoys the flexibility of neural networks in learning non-linear transforms. Empirically, the authors demonstrated that the proposed sandwiched compressor with JPEG compressor being the standard image codec in the bottleneck outperforms JPEG alone while yielding more reasonable more reasonable time and resource cost to perform encoding and decoding.
