...
%TCIDATA{LastRevised=Saturday, March 18, 2023 17:01:58}
...
admits a sub-Gaussian heat kernel. Using these equivalence, we verify
various weak-monotonicity properties on nested fractals and their blowups.
Immediate consequences are that, many classical results on $p$-energy norms
hold for such bounded and unbounded fractals, including the BBM convergence,
Gagliardo-Nirenberg and Sobolev-type inequalities.
\end{abstract}
...
Our original motivation is generalising the `Bourgain-Brezis-Mironescu (BBM)
convergence' \eqref{BBM} of $p$-energy forms ($1<p<\infty$) in \cite%
{GaoYuZhang2022PA} to bounded and unbounded metric measure space, which also
extends the classical convergence of non-local Dirichlet forms to a local
one on unbounded metric measure spaces or fractals (see \cite%
{Pietruska-Paluba.2008} and \cite{Yang2022MZ}). This originates from
Bourgain, Brezis and Mironescu in \cite[Corollary 2]%
...
by a scaling factor $1-\sigma $, the fractional Gagliardo seminorm of a
function converges to the first-order Sobolev seminorm as $\sigma
\rightarrow 1$. For other metric spaces, a natural analogue to BBM
convergence is the convergence of Besov seminorms $(1-\sigma
)B_{2,2}^{\sigma }$ to $B_{2,\infty }^{\sigma ^{\ast }}$, where $\sigma
^{\ast }$ is a fixed number (usually termed as `walk dimension') in \cite%
{Pietruska-Paluba.2008,Yang.2018.PA,Yang2022MZ}. Gu and Lau left an open
problem in \cite[Section 7]{GuLau.2020.AASFM} to extend such convergence to
more general settings under more general `weak-monotonicity properties'. To
answer this problem, we study the BBM convergence and weak-monotonicity
properties on metric measure spaces.

Recently, there are quite a few development on the $p$-energy forms defined
on fractals and general metric measure spaces, which is first studied by
\cite{HermanPeironeStrichartz.2004.PA125}. For a smooth Euclidean area $D$,
...
see \cite{Caoqiugu2022adv,GaoYuZhang2022PA} for p.c.f. self-similar sets,
\cite{Shimuzu.2022} for the Sierpi\'{n}ski carpet, and \cite%
{Kigami2022penergy} for metric measure spaces.
%////For the resistance estimate (which is a key
%part), they use or develop ideas from \cite%
%{BourdonKleiner2010,HermanPeironeStrichartz.2004.PA125,KusuokaZhou.1992.PTRF169}.////

We consider another way to define $p$-energy norms (or forms) by equivalent
Besov-type norms, which naturally generalises the energy of Dirichlet forms
when $p=2$, and the graph-approximation does not appear in the definition.
Heat kernel-based $p$-energy norms basically appeared in \cite%
{Pietruska-Paluba.2010.}, and later heat semigroup-based norms are
...
`limsup' for BBM convergence. Once this is done, we need to verify such weak
monotonicity properties for good metric measure spaces, like nested fractals
and their unbounded blow-ups. To do this, we establish the equivalence of
properties (KE), (NE) and (VE) on certain bounded and unbounded fractals
(including nested fractals), and verify a relatively easier condition (VE)
with the help of \cite{Caoqiugu2022adv}. Such properties are not easy to
examine when $p\neq 2$ in general (property (KE) holds automatically
when $p=2$). Alonso Ruiz, Baudoin et al. examined property (KE) in the case $%
p=1$ using weak Bakry-Emery type hypothesis and $p\neq 2$ for spaces with
the same critical exponent as Euclidean smooth manifolds (see for example
\cite[Lemma 4.13]{AlonsoBaudoinchen2021CVPDE}). But for fractal-type spaces,
different ideas are required, since the critical exponents are often
different from smooth manifolds and (bi-)linearity is missing.

By the time we are about to finish our paper, we notice a lecture note \cite%
...
{Gorny2022,HanPina2021}), and the verification of properties (KE) and (NE) for
two nested fractals using a different argument (see \cite%
...
fractals is to avoid complexity in defining (VE) properly, and also property
(VE) is verified for a large class of p.c.f. fractals very recently in \cite%
{Caoqiugu2022adv}. Alonso Ruiz, Baudoin et al. established two analogues of
classical analysis results under property (KE) or (NE) for $1<p<\infty $,
that is, a full scale of Gagliardo-Nirenberg inequalities introduced by \cite%
[Theorem 4.3 and Theorem 4.4]{BaudoinLecture2022} and a Sobolev-type
inequality introduced by \cite[Theorem 1.1]{AlonsoBaudoinchen2020JFA}, which
will be introduced later. The latter inequality only holds for unbounded
spaces, and this motivates us to study unbounded fractals.

\subsection{Preliminaries}

\subsubsection{Basic definitions}

Let $(M,d,\mu )$ be a metric measure space with the same setting as in \cite%
{BaudoinLecture2022}, where $(M,d)$ has more than one point with positive
diameter, and $\mu $ is a Borel regular measure satisfying the following
positivity and doubling property: there exists a constant $C>0$ such that
for every $x \in M, r>0$,
\begin{equation*}
0<\mu(B(x, 2 r)) \leq C \mu(B(x, r))<\infty.
\end{equation*}
Fix a value (which could be infinite)
\begin{equation*}
R_{0}\in (0,\mathrm{diam}(M)]
\end{equation*}
that will be used for localization throughoutly, where
\begin{equation*}
\mathrm{diam}(M):=\sup \{d(x,y):x,y\in M\}\in (0,\infty]
\end{equation*}
is infinite if $M$ is unbounded and is finite if $M$ is bounded. Denote by $%
\alpha $ the Hausdorff dimension of $(M,d)$. We say that $\mu $ is \emph{$%
\alpha $-regular}, if for all $0<r<R_{0}$,
\begin{equation}
C^{-1}r^{\alpha }\leq \mu (B(x,r))\leq Cr^{\alpha },  \label{alpha_r}
\end{equation}%
where $C\geq 1$ is a constant and%
\begin{equation*}
B(x,r):=\{y\in M:d(x,y)<r\}
\end{equation*}
denotes the open metric ball centered at $x\in M$ with radius $r>0$
throughoutly.
...
\item[(3)] Semigroup property or Markov property:
\begin{equation*}
p_{s+t}(x,y)=\int_{M}p_{s}(x,z)p_{t}(z,y)d\mu (z).
\end{equation*}

\item[(4)] Identity approximation or strong continuity: for any $f\in
L^{2}(M,\mu )$,
\begin{equation*}
...
We say that a heat kernel $\{p_{t}\}_{t>0}$ satisfies the sub-Gaussian
estimate, if for $\mu $-almost all $x,y\in M$ and all $t\in (0,R_{0}^{\beta
^{\ast }})$:
\begin{equation}
\frac{c_{1}}{t^{\alpha /\beta ^{\ast }}}\exp \left( -c_{2}\left( \frac{d(x,y)%
}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}%
}\right) \leq p_{t}(x,y)\leq \frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}\exp
\left( -c_{4}\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta
^{\ast }}{\beta ^{\ast }-1}}\right) ,  \label{hk}
\end{equation}%
where the fixed positive number $\beta ^{\ast }$ is usually called the walk
dimension of $M$. Such estimate holds for many classical cases, and implies
that $\mu $ is $\alpha $-regular (see for example \cite[Theorem 3.2]%
{GrigoryanHuLau.2003.TAMS2065}). For example, the classical
Gauss-Weierstrass function in $\mathbb{R}^{n}$
\begin{equation}
p_{t}(x,y)=\frac{1}{(4\pi t)^{n/2}}\exp \left( -\frac{|x-y|^{2}}{4t}\right) ,
\label{Gau-Wei}
\end{equation}%
is a heat kernel for the standard Brownian motion. On certain manifolds
(with nonnegative Ricci curvature), there exists a Gaussian heat kernel
\begin{equation}
p_{t}(x,y)\asymp \frac{C}{t^{n/2}}\exp \left( -\frac{|x-y|^{2}}{ct}\right) .
\label{Gau}
\end{equation}%
Such estimate also holds on many fractal spaces, such as the Sierpi\'{n}ski
carpet and nested fractals, see examples in \cite%
{BarlowBass.1989.AIHPPS225,Kigami.1993.TAMS721}.
...
}\int_{M}\frac{1}{\mu (B(x,r))}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x),
\label{pi}
\end{equation}%
and define
\begin{equation*}
...
\int_{M}\frac{1}{\mu (B(x,r))}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu
(x)\right) \frac{dr}{r},  \label{pp}
\end{equation}%
and define
\begin{equation*}
...
\frac{1}{\mu (B(x,r ))}\int_{B(x,r )}\frac{|u(x)-u(y)|^{p}}{r ^{p\sigma }}%
d\mu (y)d\mu (x)<\infty.  \label{ks_energy1}
\end{equation}
...
Clearly, $\sigma
_{p}^{\ast }\leq \sigma _{p}^{\#}$. Whenever the critical exponents appear
in our paper, we assume that they are finite, and it is known by \cite[%
Theorem 4.2]{BaudoinLecture2022} that when the chain condition (see for
example \cite[Definition 4.4]{BaudoinLecture2022}) holds, then
\begin{equation*}
\sigma _{p}^{\#}<\infty .
\end{equation*}
...
It is well-known (see \cite[Section 1.3]{FukushimaOshimaTakeda.2011.489})
that, a heat kernel $\{p_{t}\}_{t>0}$ can deduce a Dirichlet form in $%
L^{2}(M,\mu )$ given by
\begin{align*}
\mathcal{E}(u,u)& =\lim_{t\rightarrow 0^{+}}\mathcal{E}_{t}(u,u),\ u\in L^{2}
\\
...
In view of this, we adapt the heat-kernel norms in \cite%
...
\end{equation*}%
This is a natural way to extend the Dirichlet form case for $p=2$ to $%
1<p<\infty$, and we will call them heat kernel-based $p$-energy norms. The
...
\end{equation*}
It is known that (see \cite{Pietruska-Paluba.2008}), when there admits a
sub-Gaussian heat kernel, the proper scaling of $E_{2,2}^{\sigma }$
...
defined by the subordinated heat kernel $q_{t}(x,y)$. We also consider the
convergence of heat kernel-based $p$-energy norms $E_{p,p}^{\sigma }$ to $%
E_{p,\infty }^{\sigma _{p}^{\#}}$, but unfortunately $E_{p,p}^{\sigma }$
does not seem to be the $p$-energy norm of the subordinated heat kernel $%
q_{t}(x,y)$, so we need weak-monotonicity conditions.

\subsection{Statement of weak-monotonicity conditions and main results}
...
\{p_{t}\}_{t>0}$ satisfies property (KE) with $\sigma>0$, if there exist a
constant $C>0$ such that for every $u\in \mathcal{D}(E_{p,\infty }^{\sigma
}) $,
\begin{equation*}
...
exists a constant $C>0$ such that for every $u\in \mathcal{D}(E_{p,\infty
}^{\sigma })$,
\begin{equation*}
...
\begin{equation}
\Phi _{u}^{\sigma }(r)=r^{-p\sigma }\int_{M}\frac{1}{\mu (B(x,r))}%
...
_{u}^{\sigma }(r)\frac{dr}{r},\ \Vert u\Vert^p _{KS_{p,\infty }^{\sigma
}}=\limsup_{r\rightarrow 0}\Phi _{u}^{\sigma }(r).
\end{equation*}

\begin{definition}
We say that a metric measure space $(M,d,\mu )$ satisfies property ($NE$)
with $\sigma >0$ if there exists $C>0$ such that for $u\in B_{p,\infty
}^{\sigma }$,
\begin{equation}
...
\sigma >0$, if there exists $C>0$ such that for $u\in B_{p,\infty }^{\sigma
} $,
\begin{equation*}
...
It is known in \cite[Lemma 4.7]{BaudoinLecture2022} that property (NE) is
...
\end{equation}%
Suppose that $(M,d,\mu )$ satisfies property $(\widetilde{NE})$ for $%
...
Under the sub-Gaussian heat-kernel estimate, we have the following two
equivalence.

\begin{theorem}
\label{thm1}If a metric measure space $(M,d,\mu )$ admits a heat kernel $%
\{p_{t}\}_{t>0}$ satisfying \eqref{hk}, then for $\sigma >0$, $u\in
B_{p,\infty }^{\sigma }$,
\begin{equation}
E_{p,\infty }^{\sigma }(u)\asymp \lbrack u]_{B_{p,\infty }^{\sigma }}^{p}%
\text{\ and\ }\Vert u\Vert^p _{KS_{p,\infty }^{\sigma }}\asymp
\limsup_{t\rightarrow 0}\Psi _{u}^{\sigma }(t).  \label{eq1.7}
\end{equation}%
In addition, for $u\in B_{p,p}^{\sigma }$,
\begin{equation}
E_{p,p}^{\sigma }(u)\asymp \lbrack u]_{B_{p,p}^{\sigma }}^{p}.  \label{eq1.8}
\end{equation}
So $\mathcal{D}(E_{p,\infty }^{\sigma })=B_{p,\infty }^{\sigma }$ and $%
\mathcal{D}(E_{p,p}^{\sigma })=B_{p,p}^{\sigma }$.
\end{theorem}
...
condition (VE), which also motivates us to explore the equivalence of weak
monotonicity (VE) and (KE), (NE) on fractal spaces, see Section \ref{sec5}
for related definitions. Whenever we mention a fractal glue-up, we equip it
with Euclidean metric and $\alpha$-dimensional Hausdorff measure.

\begin{theorem}
\label{thm5}Let $K^{F}$ be a fractal glue-up, where $K$ is a connected
homogeneous p.c.f. self-similar set, then we have the following equivalence:
...
\begin{corollary}
\label{corol1.8}Let $K^{F}$ be a fractal glue-up, where $K$ is a connected
homogeneous p.c.f. self-similar set, suppose that $K^{F}$ admits a heat
kernel satisfying \eqref{hk}. Then properties $(\widetilde{KE}),(\widetilde{%
NE}),(\widetilde{VE})$ are equivalent, and properties $(KE),(NE),(VE)$ are
equivalent with $R_0=1$.
\end{corollary}

When $p=2$, property (KE) automatically holds true as we have mentioned
before. So Theorem \ref{thm4} and Theorem \ref{thm5} will give corollaries
that are important to Dirichlet forms.

Finally, we turn to the embedding inequalities given by Alonso Ruiz, Baudoin
et al., using our notions.

\begin{lemma}
\label{GNTM}(\cite[Theorem 4.3 and Theorem 4.4]{BaudoinLecture2022}) Suppose
that property (NE) holds with $R_0=\mathrm{diam}(M)$ for the metric measure space $(M,d,\mu )$, and
there exists $R>0$ such that $\inf_{x\in M}\mu (B(x,R))>0$. When $p\sigma
_{p}^{\#}\neq \alpha $, let $q=\frac{p\alpha }{\alpha -p\sigma _{p}^{\#}}$.
For $r,s\in (0,\infty ]$, $\theta \in (0,1)$ satisfying
\begin{equation*}
\frac{1}{r}=\frac{\theta }{q}+\frac{1-\theta }{s},
\end{equation*}%
there exists a constant $C>0$ such that for every $f\in B_{p,\infty
}^{\sigma _{p}^{\#}}$,
\begin{equation*}
\Vert f\Vert _{r}\leq C(\Vert f\Vert _{p}+[f]_{B_{p,\infty }^{\sigma
_{p}^{\#}}})^{\theta }\Vert f\Vert _{s}^{1-\theta }.
\end{equation*}%
When $p\sigma _{p}^{\#}=\alpha $, there exist constants $c,C>0$ such that
for every $f\in B_{p,\infty }^{\sigma _{p}^{\#}}$ with $\Vert f\Vert
_{1}+[f]_{B_{1,\infty }^{\sigma _{1}^{\#}}}\leq 1$,
\begin{equation*}
\int_{M}\left( e^{c|f|^{\frac{p}{p-1}}}-1\right) d\mu \leq C\Vert f\Vert
_{1}.
\end{equation*}
\end{lemma}

\begin{lemma}
(\cite[Theorem 1.1]{AlonsoBaudoinchen2020JFA})\label{SI} Suppose that an unbounded
metric measure space admits a heat kernel satisfying (DUE) (see Definition %
\ref{due}), and property (KE) is satisfied for $\sigma _{p}$, then for $u\in
L^{p}(M,\mu )$,
\begin{equation}
\Vert u\Vert _{q}\leq C_{p,\alpha ,\sigma _{p},{\beta ^{\ast }}}\left(
E_{p,\infty }^{\sigma _{p}}(u)\right) ^{1/p},  \label{sobo}
\end{equation}%
where $q=\frac{p\alpha }{\alpha -p\sigma _{p}{\beta ^{\ast }}}$, $%
C_{p,\alpha ,\sigma _{p},{\beta ^{\ast }}}$ is a constant depending only on $%
p,\alpha ,\sigma _{p},{\beta ^{\ast }}$. \label{Sb}
\end{lemma}

The following corollary can be regarded as a summary of our paper restricted
to fractal spaces.
...
_{p}^{\#}(K^{F})}(K^{F})$ contains non-constant functions, so the BBM
convergence Theorem \ref{thm4.2} holds with $R_{0}=1$. Furthermore, if $K$
is a nested fractal and $K^{F}=K_{\infty }$ is the blow-up of $K$, where $%
K_{\infty }$ is defined in Example \ref{ex1} in Section \ref{sec5}, then
(NE) and (KE) are satisfied with $R_{0}=\infty $, so that the
Gagliardo-Nirenberg inequalities in Lemma \ref{GNTM} and Sobolev-type
inequalities in \eqref{sobo} with $p$-energy norm equivalence Theorem \ref%
{thm1} all hold true.
\end{corollary}
...
present our proof of two kinds of BBM convergence in Theorem \ref{thm4.1}
and Theorem \ref{thm4.2}. In Section \ref{sec2}, we present our proof of
Theorem \ref{thm1} and Theorem \ref{thm4}. In Section \ref{sec5}, we first
introduce `fractal glue-ups' $K^F$ using connected homogeneous p.c.f.
self-similar set $K$, which gives bounded and unbounded fractal spaces by
properly choosing the set $F$ of similitudes, including fractal blowups in
Subsection \ref{subsec5.1}. Based on our previous work \cite%
{GaoYuZhang2022PA}, we study the equivalent discrete $p$-energy norms in
Lemma \ref{lem6.2} and Lemma \ref{lem6.3} which will be used in Subsection %
\ref{subsec5.3} to prove \ref{thm5}. In Subsection \ref{subsec5.2}, we
introduce vertex-type weak-monotonicity properties and show that they hold
for nested fractal (glue-ups). Finally, we summarize our final corollaries
including Corollary \ref{corol1.8} and \ref{corol1.9} in Subsection \ref%
{subsec5.4}. We leave some conjectures and remarks in the last section.

\textbf{Notation}: The letters $C$,$C^{\prime }$, $C_{i}$,$C_{i}^{\prime }$, $C_{i}^{\prime\prime }$, $c$
are universal positive constants depending only on $M$ which may vary at
each occurrence. The sign $\asymp $ means that both $\leq $ and $\geq $ are
true with uniform values of $C$ depending only on $M$. $a\wedge b:=\min
\{a,b\}$, $a\vee b:=\max \{a,b\}$.

\section{BBM Convergence for various $p$-energy norms}

\label{sec4} In this section, we prove BBM convergence for heat-kernel based $%
p$-energy norms without further heat kernel estimates, and also the
BBM convergence for Besov-Korevaar-Schoen $p$-energy norms. Under the
settings in Section \ref{sec2}, these two BBM convergence results will coincide.
...
Note that $\Psi _{u}^{\sigma }(t)=t^{p(\tilde{\sigma}_{p}-\sigma )/{\beta
^{\ast }}}\Psi _{u}^{\tilde{\sigma}_{p}}(t).$
...
\tilde{\sigma}_{p}-\sigma )^{p(\tilde{\sigma}_{p}-\sigma )}\inf_{t\in (0,%
\tilde{\sigma}_{p}-\sigma )}\Psi _{u}^{\tilde{\sigma}_{p}}(t)  \notag \\
=& \frac{{\beta ^{\ast }}}{p}\liminf_{\sigma \uparrow \tilde{\sigma}%
_{p}}\inf_{t\in (0,\tilde{\sigma}_{p}-\sigma )}\Psi _{u}^{\tilde{\sigma}%
_{p}}(t)  \notag \\
=& \frac{{\beta ^{\ast }}}{p}\liminf_{t\rightarrow 0}\Psi _{u}^{\tilde{\sigma%
}_{p}}(t).  \notag
\end{align}%
Using property (KE), it follows that
\begin{equation*}
\liminf_{\sigma \uparrow \tilde{\sigma}_{p}}(\tilde{\sigma}_{p}-\sigma ){E}%
...
For the right-hand side, since $u\in \mathcal{D}(E_{p,\infty }^{\tilde{%
\sigma _{p}}})$ implies $u\in \mathcal{D}(E_{p,p}^{\sigma })$ by Proposition %
\ref{Prop32} for any $\sigma \in (0,\tilde{\sigma _{p}})$, we can find a
finite positive real number $R_{u}<R_{0}$ such that
\begin{equation*}
\int_{0}^{R_{u}^{{\beta ^{\ast }}}}\Psi _{u}^{\sigma }(t)\frac{dt}{t}\geq
\frac{1}{2}\int_{0}^{R_{0}^{{\beta ^{\ast }}}}\Psi _{u}^{\sigma }(t)\frac{dt%
}{t}=\frac{1}{2}E_{p,p}^{\sigma }(u).
\end{equation*}%
It follows that
\begin{align}
\limsup_{\sigma \uparrow \tilde{\sigma}_{p}}(\tilde{\sigma}_{p}-\sigma ){E}%
_{p,p}^{\sigma }(u)\leq & 2\limsup_{\sigma \uparrow \tilde{\sigma}_{p}}(%
\tilde{\sigma}_{p}-\sigma )\int_{0}^{R_{u}^{{\beta ^{\ast }}}}t^{p(\tilde{%
\sigma}_{p}-\sigma )/{\beta ^{\ast }}}\Psi _{u}^{\tilde{\sigma}_{p}}(t)\frac{%
dt}{t}  \notag \\
\leq & 2\limsup_{\sigma \uparrow \tilde{\sigma}_{p}}(\tilde{\sigma}%
_{p}-\sigma )\int_{0}^{R_{u}^{{\beta ^{\ast }}}}t^{p(\tilde{\sigma}%
_{p}-\sigma )/{\beta ^{\ast }}}\frac{dt}{t}\sup_{t\in (0,R_{u}^{{\beta
^{\ast }}})}\Psi _{u}^{\tilde{\sigma}_{p}}(t)  \notag \\
=& \frac{2{\beta ^{\ast }}}{p}\limsup_{\sigma \uparrow \tilde{\sigma}%
_{p}}R_{u}^{p(\tilde{\sigma}_{p}-\sigma )}\sup_{t\in (0,R_{u}^{{\beta ^{\ast
}}})}\Psi _{u}^{\tilde{\sigma}_{p}}(t)  \notag \\
=& \frac{2{\beta ^{\ast }}}{p}\sup_{t\in (0,R_{u}^{{\beta ^{\ast }}})}\Psi
_{u}^{\tilde{\sigma}_{p}}(t)  \notag \\
\leq & \frac{2{\beta ^{\ast }}}{p}E_{p,\infty }^{\tilde{\sigma}_{p}}(u).
\label{NE_right}
\end{align}%
...
The proof of this version is highly similar to the previous one.
...
& \int_{M}\frac{1}{\mu (B(x,r))}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)
\notag \\
\leq & 2^{p-1}\int_{M}\frac{1}{\mu (B(x,r))}%
\int_{B(x,r)}(|u(x)|^{p}+|u(y)|^{p})d\mu (y)d\mu (x)  \notag \\
=& 2^{p-1}\left( \Vert u\Vert _{p}^{p}+\int_{M}\int_{M}\frac{\mathbf{1}%
_{\{d(y,x)<r\}}}{\mu (B(x,r))}|u(y)|^{p}d\mu (y)d\mu (x)\right)  \notag \\
=& 2^{p-1}\left( \Vert u\Vert _{p}^{p}+\int_{M}\left( \int_{B(y,r)}\frac{1}{%
\mu (B(x,r))}d\mu (x)\right) |u(y)|^{p}d\mu (y)\right)  \notag \\
\leq & 2^{p-1}\left( \Vert u\Vert _{p}^{p}+\sup_{x\in M}\frac{\mu (B(x,2r))}{%
\mu (B(x,r))}\int_{M}|u(y)|^{p}d\mu (y)\right) \leq C\Vert u\Vert _{p}^{p}.
\label{300}
\end{align}
...
-p\delta +\alpha )}\int_{M}\frac{1}{\mu (B(x,r))}%
\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)\frac{dr}{r} \\
&\leq &\int_{0}^{\epsilon }r^{-(p\sigma -p\delta )}\int_{M}\frac{1}{\mu
(B(x,r))}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)\frac{dr}{r} \\
&&+\int_{\epsilon }^{\infty }r^{-(p\sigma -p\delta )}\int_{M}\frac{1}{\mu
(B(x,r))}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)\frac{dr}{r} \\
...
Firstly, we give a proof of\eqref{NE_conv}. For the left-hand side,
similarly, we have that for any $\sigma _{p}>0,$
\begin{align}
...
\end{align}%
By property (NE), we have for $\sigma _{p}=\sigma _{p}^{\#}$ that
\begin{equation}
\liminf_{\sigma \uparrow \sigma _{p}^{\#}}(\sigma _{p}^{\#}-\sigma )[u]_{{B}%
...
For the other side, since $u\in B_{p,p}^{\sigma }$, which means $%
\int_{0}^{\infty }\Phi _{u}^{\sigma }(r)\frac{dr}{r}=[u]_{B_{p,p}^{\sigma
}}^{p}<\infty $, we can find a finite positive number $R_{u}<R_{0}$ such
that
\begin{equation*}
\int_{0}^{R_{u}}\Phi _{u}^{\sigma }(r)\frac{dr}{r}\geq \frac{1}{2}%
\int_{0}^{\infty }\Phi _{u}^{\sigma }(r)\frac{dr}{r}=\frac{1}{2}%
[u]_{B_{p,p}^{\sigma }}^{p}.
\end{equation*}
...
_{p,p}^{\sigma }}^{p}\leq& 2 \limsup_{\sigma \uparrow \sigma
_{p}^{\#}}(\sigma _{p}^{\#}-\sigma )\int_{0}^{R_{u}}r^{p(\sigma
_{p}^{\#}-\sigma )}\Phi _{u}^{\sigma _{p}^{\#}}(r)\frac{dr}{r}  \notag \\
\leq & 2\limsup_{\sigma \uparrow \sigma _{p}^{\#}}(\sigma _{p}^{\#}-\sigma
)\int_{0}^{R_{u}}r^{p(\sigma _{p}^{\#}-\sigma )}\frac{dr}{r}\sup_{r\in
(0,R_{u})}\Phi _{u}^{\sigma _{p}^{\#}}(r)  \notag \\
=& \frac{2}{p}\sup_{r\in (0,R_{u})}\Phi _{u}^{\sigma _{p}^{\#}}(r)\leq \frac{%
2}{p}[u]_{B_{p,\infty }^{\sigma _{p}^{\#}}}^{p},  \label{42}
\end{align}
showing \eqref{NE_conv}.

Next, we show the proof of \eqref{KS_conv}. Combining property $(\widetilde{%
NE})$ with \eqref{408}, we have
\begin{equation}
\liminf_{\sigma \uparrow \sigma _{p}}(\sigma _{p}-\sigma )[u]_{{B}%
_{p,p}^{\sigma }}^{p}\geq C^{-1}\Vert u\Vert^p _{KS_{p,p}^{\sigma _{p}}}.
\label{KS_left}
\end{equation}%
...
We use the same notion $R_{u}<R_{0}$ defined in the first part of the proof.
It follows that,%
\begin{align}
\limsup_{\sigma \uparrow \sigma _{p}}(\sigma _{p}-\sigma )[u]_{{B}%
_{p,p}^{\sigma }}^{p} \leq& 2\limsup_{\sigma \uparrow \sigma _{p}}(\sigma
_{p}-\sigma )\int_{0}^{R_{u}}r^{p(\sigma _{p}-\sigma )}\Phi _{u}^{\sigma
_{p}}(r)\frac{dr}{r}  \notag \\
\leq & 2\limsup_{\sigma \uparrow \sigma _{p}}(\sigma _{p}-\sigma
)\int_{0}^{\epsilon }r^{p(\sigma _{p}-\sigma )}\frac{dr}{r}\sup_{r\in
(0,\epsilon )}\Phi _{u}^{\sigma _{p}}(r)  \notag \\
& +2\limsup_{\sigma \uparrow \sigma _{p}}(\sigma _{p}-\sigma )\int_{\epsilon
}^{R_{u}}r^{p(\sigma _{p}-\sigma )}\frac{dr}{r}\sup_{r\in (0,R_{u})}\Phi
_{u}^{\sigma _{p}}(r)  \notag \\
=& \frac{2}{p}\sup_{r\in (0,\epsilon )}\Phi _{u}^{\sigma
_{p}}(r)\limsup_{\sigma \uparrow \sigma _{p}}\epsilon ^{p(\sigma _{p}-\sigma
)}+0  \notag \\
\leq &\frac{4}{p}\limsup_{r\rightarrow 0}\Phi _{u}^{\sigma _{p}}(r)=\frac{4}{%
p}\Vert u\Vert^p _{{KS}_{p,\infty }^{\sigma _{p}}} .  \label{KS_right}
\end{align}%
The proof is complete.
\end{proof}

\section{Weak-monotonicity properties on metric measure space}

\label{sec2} In this section, we assume that the metric measure space admits
a heat kernel with estimate \eqref{hk}, and fix $\sigma>0$.
...
arguments). Some similar results can be found in \cite[Proposition 4.2]%
{AlonsoBaudoinchen2020JFA} or \cite{Pietruska-Paluba.2010.}. We naturally
...
\end{equation}%
where the constant $C$ depends only on ${\beta ^{\ast }}$, $c_{1}$, $c_{2}$
in \eqref{hk}.
\end{lemma}
...
& \geq c_{1}\int_{0}^{R_{0}^{\beta ^{\ast }}}\frac{1}{t^{{(p\sigma +\alpha )}%
/{\beta ^{\ast }}}}\left( \int_{M}\int_{B(x,t^{{1}/{\beta ^{\ast }}%
})}|u(x)-u(y)|^{p}\exp \left( -c_{2}\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}%
}\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) d\mu (y)d\mu
(x)\right) \frac{dt}{t}  \notag \\
& \geq c_{1}e^{-c_{2}}\int_{0}^{R_{0}^{\beta ^{\ast }}}\frac{1}{t^{{(p\sigma
+\alpha )}/{\beta ^{\ast }}}}\left( \int_{M}\int_{B(x,t^{{1}/{\beta ^{\ast }}%
})}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)\right) \frac{dt}{t}  \notag \\
& =\beta ^{\ast }c_{1}e^{-c_{2}}\int_{0}^{R_{0}}\frac{1}{r^{{(p\sigma
+\alpha )}}}\left( \int_{M}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu
(x)\right) \frac{dr}{r}=\beta ^{\ast }c_{1}e^{-c_{2}}[u]_{B_{p,p}^{\sigma
}}^{p},  \label{eq3.2}
\end{align}%
...
\begin{proof}
Firstly, we will split $M$ into union of rings $B(x,2^{n}t^{1/\beta ^{\ast
}})\setminus B(x,2^{n-1}t^{1/\beta ^{\ast }})$, where the integers $n\leq
\log _{2}(R_{0}t^{-1/\beta ^{\ast }})$, then by \eqref{hk}, we have
\begin{align}
...
\leq & \frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}\int_{M}\sum_{n\leq \log
...
(x)  \notag \\
\leq & \frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}\sum_{n\leq \log
_{2}(R_{0}t^{-1/\beta ^{\ast }})}\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast
...
(x)d\mu (y)\right) \frac{dt}{t} \\
& \leq \int_{0}^{R_{0}^{\beta ^{\ast }}}\frac{c_{3}}{t^{{(p\sigma +\alpha )}/%
{\beta ^{\ast }}}}\left( \sum_{n\leq \log _{2}(R_{0}t^{-1/\beta ^{\ast
}})}\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}\exp \left(
-c_{4}2^{\frac{(n-1)\beta ^{\ast }}{\beta ^{\ast }-1}}\right) d\mu (y)d\mu
(x)\right) \frac{dt}{t} \\
& =\int_{0}^{R_{0}^{\beta ^{\ast }}}\frac{c_{3}}{t^{{(p\sigma +\alpha )}/{%
\beta ^{\ast }}}}\left( \sum_{n=-\infty }^{\infty }1_{\{n\leq \log
_{2}(R_{0}t^{-1/\beta ^{\ast }})\}}(n)\int_{M}\int_{B(x,2^{n}t^{1/\beta
^{\ast }})}|u(x)-u(y)|^{p}\exp \left( -c_{4}2^{\frac{(n-1)\beta ^{\ast }}{%
\beta ^{\ast }-1}}\right) d\mu (y)d\mu (x)\right) \frac{dt}{t} \\
& \leq \sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{(n-1)\beta
^{\ast }}{\beta ^{\ast }-1}}\right) \int_{0}^{2^{-n\beta ^{\ast
}}R_{0}^{\beta ^{\ast }}}\frac{c_{3}}{t^{(p\sigma +\alpha )/\beta ^{\ast }}}%
\left( \int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}d\mu
(y)d\mu (x)\right) \frac{dt}{t} \\
& =c_{3}\beta ^{\ast }\sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{%
(n-1)\beta ^{\ast }}{\beta ^{\ast }-1}}\right) 2^{(p\sigma +\alpha
)n}\int_{0}^{R_{0}}r^{-(p\sigma +\alpha )}\left(
\int_{M}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)\right) \frac{dr}{r} \\
& =c_{3}\beta ^{\ast }C_{0}[u]_{B_{p,p}^{\sigma }}^{p},
\end{align*}%
where
\begin{eqnarray}
C_{0} &=&\sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{(n-1)\beta
^{\ast }}{\beta ^{\ast }-1}}\right) 2^{(p\sigma +\alpha )n}  \notag \\
&\leq &\sum_{n=-\infty }^{0}2^{(p\sigma +\alpha )n}+\sum_{n=1}^{\infty }\exp
\left( -c_{4}2^{\frac{(n-1)\beta ^{\ast }}{\beta ^{\ast }-1}}\right)
2^{(p\sigma +\alpha )n}<\infty ,  \label{c1}
\end{eqnarray}%
which completes the proof.
\end{proof}
...
\begin{align*}
E_{p,\infty }^{\sigma }(u)& =\sup_{t\in (0,R_{0}^{\beta ^{\ast }})}\frac{1}{%
t^{p\sigma /\beta ^{\ast }}}\int_{M}\int_{M}p_{t}(x,y)|u(x)-u(y)|^{p}d\mu
(y)d\mu (x) \\
...
^{\ast }}}\frac{1}{t^{\alpha /\beta ^{\ast }}}\int_{M}\int_{B(x,t^{1/\beta
^{\ast }})}|u(x)-u(y)|^{p}\exp \left( -c_{2}\left( \frac{d(x,y)}{t^{1/\beta
^{\ast }}}\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) d\mu
(y)d\mu (x) \\
& \geq c_{1}e^{-c_{2}}\sup_{t\in (0,R_{0}^{\beta ^{\ast }})}\frac{1}{%
t^{(p\sigma +\alpha )/\beta ^{\ast }}}\int_{M}\int_{B(x,t^{1/\beta ^{\ast
}})}|u(x)-u(y)|^{p}d\mu (y)d\mu (x) \\
& =c_{1}e^{-c_{2}}\sup_{r\in (0,R_{0})}\frac{1}{r^{(p\sigma +\alpha )}}%
\int_{M}\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu
...
\begin{proof}
By \eqref{eq3.3}, we have
\begin{align}
& \frac{1}{t^{p\sigma /\beta ^{\ast }}}\int_{M}%
\int_{M}|u(x)-u(y)|^{p}p_{t}(x,y)d\mu (y)d\mu (x)  \notag \\
& \leq \frac{c_{3}}{t^{(p\sigma +\alpha )/\beta ^{\ast }}}\sum_{n\leq \log
_{2}(R_{0}t^{-1/\beta ^{\ast }})}\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast
}})}|u(x)-u(y)|^{p}\exp \left( -c_{4}2^{\frac{(n-1)\beta ^{\ast }}{\beta
^{\ast }-1}}\right) d\mu (y)d\mu (x)  \notag \\
& =c_{3}\sum_{n\leq \log _{2}(R_{0}t^{-1/\beta ^{\ast }})}2^{(p\sigma
+\alpha )n}\int_{M}\frac{1}{(2^{n}t^{1/\beta ^{\ast }})^{p\sigma +\alpha }}%
\int_{B(x,2^{n}t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}\exp \left( -c_{4}2^{%
\frac{(n-1)\beta ^{\ast }}{\beta ^{\ast }-1}}\right) d\mu (y)d\mu (x)  \notag
\\
& \leq c_{3}\sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{(n-1)\beta
^{\ast }}{\beta ^{\ast }-1}}\right) 2^{(p\sigma +\alpha )n}\sup_{r\in
(0,R_{0})}\frac{1}{r^{(p\sigma +\alpha )}}\int_{M}%
\int_{B(x,r)}|u(x)-u(y)|^{p}d\mu (y)d\mu (x)  \notag \\
& =c_{3}\sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{(n-1)\beta
^{\ast }}{\beta ^{\ast }-1}}\right) 2^{(p\sigma +\alpha )n}[u]_{B_{p,\infty
}^{\sigma }}^{p}=c_{3}C_{0}[u]_{B_{p,\infty }^{\sigma }}^{p},  \label{eq3.14}
\end{align}%
...
%Finally, we prove the last inclusion in Theorem \ref{thm1}.
...
\begin{equation}
\sup_{r\in (0,R_{0})}\Phi _{u}(r)\leq C\sup_{t\in (0,R_{0}^{\beta ^{\ast
}})}\Psi _{u}^{\sigma }(t),  \label{31}
\end{equation}
...
\end{align}%
By the last four line of \eqref{eq3.14}, we know that%
\begin{align*}
& \frac{c_{3}}{t^{{(p\sigma +\alpha )}/{\beta ^{\ast }}}}\sum_{n=-\infty
}^{\infty }1_{\{n\leq \log _{2}(R_{0}t^{-1/\beta ^{\ast
}})\}}(n)\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}\exp
\left( -c_{4}2^{\frac{(n-1)\beta ^{\ast }}{\beta ^{\ast }-1}}\right) d\mu
(y)d\mu (x) \\
& =\frac{c_{3}}{t^{(p\sigma +\alpha )/\beta ^{\ast }}}\sum_{n\leq \log
_{2}(R_{0}t^{-1/\beta ^{\ast }})}\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast
}})}|u(x)-u(y)|^{p}\exp \left( -c_{4}2^{\frac{(n-1)\beta ^{\ast }}{\beta
^{\ast }-1}}\right) d\mu (y)d\mu (x)<\infty .
\end{align*}

Hence, we have by \eqref{32} and the dominated convergence theorem that
\begin{align*}
& \limsup_{t\rightarrow 0}\frac{1}{t^{p\sigma /\beta ^{\ast }}}%
\int_{M}\int_{M}|u(x)-u(y)|^{p}p_{t}(x,y)d\mu (y)d\mu (x) \\
\leq & \limsup_{t\rightarrow 0}\frac{c_{3}}{t^{{(p\sigma +\alpha )}/{\beta
^{\ast }}}}\sum_{n=-\infty }^{\infty }1_{\{n\leq \log _{2}(R_{0}t^{-1/\beta
^{\ast }})\}}(n)\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast
}})}|u(x)-u(y)|^{p}\exp \left( -c_{4}2^{\frac{(n-1)\beta ^{\ast }}{\beta
^{\ast }-1}}\right) d\mu (y)d\mu (x) \\
\leq & c_{3}\sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{(n-1)\beta
^{\ast }}{\beta ^{\ast }-1}}\right) 2^{(p\sigma +\alpha
)n}\limsup_{t\rightarrow 0}\frac{1_{\{n\leq \log _{2}(R_{0}t^{-1/\beta
^{\ast }})\}}(n)}{(2^{n}t^{1/\beta ^{\ast }})^{(p\sigma +\alpha )/\beta
^{\ast }}}\int_{M}\int_{B(x,2^{n}t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}d\mu
(y)d\mu (x) \\
\leq & c_{3}\sum_{n=-\infty }^{\infty }\exp \left( -c_{4}2^{\frac{(n-1)\beta
^{\ast }}{\beta ^{\ast }-1}}\right) 2^{(p\sigma +\alpha
)n}\limsup_{r\rightarrow 0}\Phi _{u}^{\sigma }(r)\leq
c_{3}C_{0}\limsup_{r\rightarrow 0}\Phi _{u}^{\sigma }(r),
\end{align*}%
where the constant $C_{0}$ is from \eqref{c1}, which ends the proof.
\end{proof}
...
\end{equation*}%
The properties (1)--(4) of $\{p_{t}\}_{t>0}$ imply that $P_{t}$ is a bounded
...
L^{2}(M,\mu )$. We also obtain the following tail-estimate with the same form
as in \cite[Theorem 2.1]{GrigoryanHu.2014.MMJ505}, where the metric space
must be unbounded. We still use analytic tools to obtain the tail estimates
for both bounded and unbounded metric spaces.
...
the condition ($UE$) holds. When $\mu$ is $\alpha $-regular, there is an
equivalence (see \cite{GrigoryanHu.2014.MMJ505})
\begin{align*}
(UE)\Leftrightarrow (DUE)+(T_{\text{exp}}).
\end{align*}
which is related to Corollary \ref{corol1.9}, where $(DUE)$, $(T_{\text{exp}%
...
condition ($DUE$) holds (short for `diagonal upper estimate'), if there
exists a heat kernel $p_{t}(x,y)$ satisfying the following estimate
\begin{align*}
p_t(x,y)\leq Ct^{-\alpha/\beta^*}
\end{align*}
for any $t\in(0,R_0^{\beta^*})$ and for $\mu$-almost all $x, y\in M$.
\end{definition}

Using the semigroup property, one can deduce that $(DUE)+(T_{\text{exp}%
})\Rightarrow (UE)$. It is trivial that $(DUE)$ follows from $(UE)$, then we
give an elementary argument for $(UE)\Rightarrow(T_{\text{exp}})$.

\begin{proposition}
\label{tail_hk} If a heat kernel $p_{t}(x,y)$ satisfies the upper bound in %
\eqref{hk}, then we have the following exponential tail estimate (termed ($%
T_{\text{exp}}$)),
\begin{equation}
...
\int_{B(x,r)^{c}}\frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}\exp \left(
...
c_{3}}{t^{\alpha /\beta ^{\ast }}}\exp \left( -c_{4}\left( \frac{2^{n}r}{%
t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}%
}\right) d\mu (y) \\
& \leq \sum_{n=0}^{\infty }\int_{B(x,2^{n+1}r)}\frac{c_{3}}{t^{\alpha /\beta
^{\ast }}}\exp \left( -c_{4}\left( \frac{2^{n}r}{t^{1/\beta ^{\ast }}}%
\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) d\mu (y).
\end{align*}%
Therefore, by \eqref{alpha_r},
\begin{align}
P_{t}1_{B(x,r)^{c}}(x)& \leq 2^{\alpha }\sum_{n=0}^{\infty }\frac{c_{3}}{%
t^{\alpha /\beta ^{\ast }}}C\left( 2^{n}r\right) ^{\alpha }\exp \left(
-c_{4}\left( \frac{2^{n}r}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta ^{\ast
}}{\beta ^{\ast }-1}}\right)  \notag \\
& =C_{1}\sum_{n=0}^{\infty }\left( \frac{2^{n}r}{t^{1/\beta ^{\ast }}}%
\right) ^{\alpha }\exp \left( -c_{4}\left( \frac{2^{n}r}{t^{1/\beta ^{\ast }}%
}\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right)  \notag \\
& \leq C_{1}\int_{\frac{r}{2t^{1/\beta ^{\ast }}}}^{\infty }s^{\alpha
-1}\exp \left( -c_{4}s^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) ds,
\label{eq3.4}
\end{align}%
where $C_{1}=2^{\alpha }c_{3}C$, and $C$ is from \eqref{alpha_r}. The last
line in \eqref{eq3.4} is obtained by setting $t_{n}=2^{n}r/t^{1/\beta }$
for $n\geq -1$ and noting that
\begin{equation*}
\int_{t_{n}}^{t_{n+1}}s^{\alpha -1}\exp \left( -c_{4}s^{\frac{\beta ^{\ast }%
}{\beta ^{\ast }-1}}\right) ds\geq t_{n}^{\alpha -1}\exp \left( -c_{4}t_{n}^{%
\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right)
(t_{n+1}-t_{n})=t_{n}^{\alpha }\exp \left( -c_{4}t_{n}^{\frac{\beta ^{\ast }%
}{\beta ^{\ast }-1}}\right) .
\end{equation*}%
Next, we estimate the term $\int_{\frac{r}{2t^{1/\beta ^{\ast }}}}^{\infty
}s^{\alpha -1}\exp \left( -c_{4}s^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}%
}\right) ds$. Indeed,
\begin{align}
& \int_{\frac{r}{2t^{1/\beta ^{\ast }}}}^{\infty }s^{\alpha -1}\exp \left(
-c_{4}s^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) ds  \notag \\
\leq & \int_{\frac{r}{2t^{1/\beta ^{\ast }}}}^{\infty }s^{\alpha -1}\exp
...
}s^{\alpha -1}\exp \left( -\frac{c_{4}}{2}s^{\frac{\beta ^{\ast }}{\beta
^{\ast }-1}}\right) ds  \notag \\
=& \exp \left( -\frac{c_{4}}{2}\left( \frac{r}{2t^{1/\beta ^{\ast }}}\right)
^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) \frac{\beta ^{\ast }-1}{%
\beta ^{\ast }}\left( \frac{c_{4}}{2}\right) ^{-\alpha (\beta ^{\ast
}-1)/\beta ^{\ast }}\int_{0}^{\infty }m^{\alpha (\beta ^{\ast }-1)/\beta
^{\ast }-1}\exp \left( -m\right) dm  \notag \\
=& \frac{\beta ^{\ast }-1}{\beta ^{\ast }}\left( \frac{c_{4}}{2}\right)
^{-\alpha (\beta ^{\ast }-1)/\beta ^{\ast }}\Gamma (\alpha (\beta ^{\ast
}-1)/\beta ^{\ast })\exp \left( -\frac{c_{4}}{2}\left( \frac{r}{2t^{1/\beta
...
^{\ast }\left( c_{4}/2\right) ^{-\alpha (\beta ^{\ast }-1)/\beta ^{\ast
}}\Gamma (\alpha (\beta ^{\ast }-1)/\beta ^{\ast })$.
\end{proof}

We need to use the following proposition taken from the proof of \cite[Lemma
4.13]{AlonsoBaudoinchen2021CVPDE} (without proving) and we present its proof
here for later use.

\begin{proposition}
\label{hk_ct} Suppose that $M$ admits a heat kernel satisfying \eqref{hk},
then there exist $C,c>1$ and $\delta ,c^{\prime }>0$ such that for $%
...
\begin{proof}
By the upper bound of heat kernel in \eqref{hk}, we have
\begin{align*}
p_{t}(x,y)& \leq \frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}\exp \left(
-c_{4}\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta ^{\ast
}}{\beta ^{\ast }-1}}\right) \\
& =c^{\alpha /\beta ^{\ast }}\frac{c_{3}}{(ct)^{\alpha /\beta ^{\ast }}}\exp
\left( -c_{4}\left( \frac{d(x,y)}{(ct)^{1/\beta ^{\ast }}}\right) ^{\frac{%
\beta ^{\ast }}{\beta ^{\ast }-1}}\cdot c^{\frac{1}{\beta ^{\ast }-1}}\right)
\\
& =\frac{c_{3}c^{\alpha /\beta ^{\ast }}}{c_{1}}\frac{c_{1}}{(ct)^{\alpha
/\beta ^{\ast }}}\exp \left( -c_{2}\left( \frac{d(x,y)}{(ct)^{1/\beta ^{\ast
}}}\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\cdot \frac{c_{4}}{c_{2}%
}\cdot c^{\frac{1}{\beta ^{\ast }-1}}\right) .
\end{align*}%
Taking $\tilde{c}=c_{4}c^{\frac{1}{\beta ^{\ast }-1}}-c_{2}$, then we use
lower bound of \eqref{hk} to have
\begin{equation*}
p_{t}(x,y)\leq \frac{c_{3}c^{\alpha /\beta ^{\ast }}}{c_{1}}\exp \left( -%
\tilde{c}\left( \frac{d(x,y)}{(ct)^{1/\beta ^{\ast }}}\right) ^{\frac{\beta
^{\ast }}{\beta ^{\ast }-1}}\right) p_{ct}(x,y).
\end{equation*}%
...
\begin{align*}
p_{t}(x,y)\leq & \frac{c_{3}c^{\alpha /\beta ^{\ast }}}{c_{1}}\exp \left(
-c^{\prime }\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta
^{\ast }}{\beta ^{\ast }-1}}\right) p_{ct}(x,y) \\
\leq & C\exp \left( -c^{\prime }\delta ^{\frac{\beta ^{\ast }}{\beta ^{\ast
}-1}}\right) p_{ct}(x,y)
\end{align*}%
where $C=\frac{c_{3}c^{\alpha /\beta ^{\ast }}}{c_{1}}$. The proof is
complete.
\end{proof}
...
^{\ast }})}|u(x)-u(y)|^{p}p_{t}(x,y)d\mu (y)d\mu (x) \\
& \geq c_{1}\frac{1}{t^{p\sigma /\beta ^{\ast }}}\cdot \frac{1}{t^{\alpha
/\beta ^{\ast }}}\int_{M}\int_{B(x,t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}\exp
\left( -c_{2}\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta
^{\ast }}{\beta \ast -1}}\right) d\mu (y)d\mu (x) \\
& \geq c_{1}e^{-c_{2}}\frac{1}{t^{(p\sigma +\alpha )/\beta ^{\ast }}}%
\int_{M}\int_{B(x,t^{1/\beta ^{\ast }})}|u(x)-u(y)|^{p}d\mu (y)d\mu
(x)=c_{1}e^{-c_{2}}\Phi _{u}(t^{1/\beta ^{\ast }}).
\end{align*}%
...
\begin{proof}
Without loss of generality, assume that $\liminf_{t\rightarrow 0}\Psi
_{u}^{\sigma }(t)>0$. Temporally fix $u\in L^{p}(M,\mu )$ and $%
0<t<R_{0}^{\beta ^{\ast }}$, then we decompose $\Psi _{u}^{\sigma }(t)$
according to the space-time relation
\begin{equation*}
...
\delta t^{1/\beta ^{\ast }}\}}|u(x)-u(y)|^{p}p_{t}(x,y)d\mu (x)d\mu (y), \\
\Psi _{2}(t)& =\frac{1}{t^{{p\sigma }/{\beta ^{\ast }}}}\iint_{\{d(x,y)>%
\delta t^{1/\beta ^{\ast }}\}}|u(x)-u(y)|^{p}p_{t}(x,y)d\mu (x)d\mu (y).
\end{align*}

For $\Psi _{1}(t)$, when $d(x,y)\leq \delta t^{1/\beta ^{\ast }}$, $%
p_{t}(x,y)$ is nearly constant, since
\begin{align*}
\frac{c_{1}}{t^{\alpha /\beta ^{\ast }}}\exp \left( -c_{2}\delta ^{\frac{%
\beta ^{\ast }}{\beta ^{\ast }-1}}\right) \leq & \frac{c_{1}}{t^{\alpha
/\beta ^{\ast }}}\exp \left( -c_{2}\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}}%
\right) ^{\frac{\beta ^{\ast }}{\beta ^{\ast }-1}}\right) \\
\leq & p_{t}(x,y)\leq \frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}\exp \left(
-c_{4}\left( \frac{d(x,y)}{t^{1/\beta ^{\ast }}}\right) ^{\frac{\beta ^{\ast
}}{\beta ^{\ast }-1}}\right) \leq \frac{c_{3}}{t^{\alpha /\beta ^{\ast }}}.
\end{align*}%
Therefore, we have
\begin{equation}
\Psi _{1}(t)\leq \frac{c_{3}}{t^{(p\sigma +\alpha )/\beta ^{\ast }}}%
\iint_{\{d(x,y)\leq \delta t^{1/\beta ^{\ast }}\}}|u(x)-u(y)|^{p}d\mu
(x)d\mu (y)=C_{1}\Phi _{u}^{\sigma }(\delta t^{1/\beta ^{\ast }}),
\label{psi1}
\end{equation}%
where $C_{1}=c_{3}\delta ^{p\sigma +\alpha }$.

For $\Psi _{2}(t)$, by Lemma \ref{hk_ct}, we have
\begin{equation}
\Psi _{2}(t)\leq A\Psi _{u}^{\sigma }(ct),  \label{psi2}
\end{equation}%
where $A=Cc^{p\sigma /\beta ^{\ast }}\exp \left( -c^{\prime }\delta ^{\frac{%
\beta ^{\ast }}{\beta ^{\ast }-1}}\right) $ is a constant and can be made as
small as we desire by making $\delta $ large enough. So from now on we only
consider large enough $\delta $ making $A<\frac{1}{2}$. Combining %
\eqref{psi1} and \eqref{psi2}, we have%
\begin{equation}
...
\end{equation}%
Thus, by \eqref{ke_ne1} and \eqref{ne_ke1}, we show Theorem \ref{thm4} (i).
...
\end{equation}%
Thus, by \eqref{ke_ne2} and \eqref{ne_ke2}, we show Theorem \ref{thm4} (ii).
\end{proof}

By Theorem \ref{thm4}, we have the following interesting corollary for $p=2$%
, which is important for defining a local Dirichlet form based on Besov
norms.
...
\begin{proof}
If $p=2$, we have the fact that property (KE) automatically holds with $%
\sigma =\beta ^{\ast }/2$ since $t\rightarrow \mathcal{E}_{t}(u,u)$ is
monotone decreasing. By Theorem \ref{thm4}, we obtain that (NE)
automatically holds for the same critical exponent.
\end{proof}
...
properties (VE) and $(\widetilde{VE})$. We will verify property (E) on nested
fractals, and our main goal is to show the equivalence of (KE) and (VE).
Lastly, we show that aforementioned consequences hold for certain `fractal
glue-ups', for example, nested fractals and the fractal blow-ups.
...
\end{equation*}%
The $\mathcal{H}^{\alpha }$-measure of $K$, denoted by $\mu $, is $\alpha $%
-regular with
\begin{equation*}
...
Condition(H)} holds for a connected homogeneous p.c.f. self-similar set $K$.
That is, there exists $c>0$ depending only on $K$ such that, for any two
words $w$ and $w^{\prime }$ with the same length $m\geq 1$, $K_{w}\cap
K_{w^{\prime }}=\emptyset $ implies that $\func{dist}\left(
K_{w},K_{w^{\prime }}\right) \geq c\rho ^{m}$.

Next, we introduce the construction of \emph{fractal glue-ups}. The
definition is adapted from the idea of fractafold in \cite{Strichartz.2006.}
but not the same (see \cite{StrichartzTeplyaev2012}). Given a compact set $%
K\subset \mathbb{R}^d$ and a set $F$ of similitudes on $\mathbb{R}^d$, we
can define
\begin{equation*}
...
denote all the cells that intersect it by $f_{n}(K),\ 1\leq n\leq N$ (here $%
N $ might be $\infty $). Choose an `interior' (away from boundaries) level-2
cell $\phi _{w}(K)$ ($|w|=2$) that does not intersect $V_{0}$, which exists
due to the p.c.f. condition. Condition (H) implies that all $f_{n}(\phi
_{w}K)$ are separated by distance $C_{H}\rho ^{2}$ for $1\leq n\leq N$, but
they all belong to the ball $B_{1}(f(v))$! This means that $N$ is uniformly
bounded, since there are $N$ disjoint balls $B_{C_{H}\rho ^{2}/2}(f_{n}(\phi
_{w}(v)))$ inside a unit ball $B_{1}(f(v))$.

From now on, whenever we mention a \emph{fractal glue-up} $K^{F}$, we
automatically assume that these three conditions are satisfied. We can
regard $f(K)$ as tiles of $K^{F}$, and we just simply glue these tiles up
which only overlap at boundaries by the `just-touching' property.
...
the upperscript $F$ to distinguish the underlying space $K$ and $K^{F}$ for
the energies and norms etc.

Next, we consider discrete $p$-energy norms of $K^F$. Following the
definitions for $K$ in \cite{GaoYuZhang2022PA},
\begin{align*}
E_n^{(p)}(u):=\sum_{x, y \in V_w , |w| = n} |u (x) - u(y) |^p\text{\ and\ }%
\mathcal{E}_{n}^{\sigma}(u):=\rho^{- n (p \sigma -\alpha)}E_n^{(p)}(u),
\end{align*}
then the local $p$-energy norm on $K^F$ is given by
\begin{equation}
E_n^{(p),F}(u):=\sum_{f\in F} \sum_{x, y \in V_w , |w| = n} |u \circ f(x) -
u\circ f(y) |^p=\sum_{f\in F} E_n^{(p)}(u\circ f),  \label{p_energy}
\end{equation}
and
\begin{equation*}
\mathcal{E}_{n}^{\sigma,F}(u)=\rho^{- n (p \sigma -\alpha)}E_n^{(p),F}(u).
\end{equation*}

We define
\begin{equation}
\mu _{m}^{F}=\sum_{f\in F}\frac{1}{|V_{m}|}\sum_{a\in V_{m}}\delta _{f(a)},
\label{200}
\end{equation}%
...
,F}(\rho ^{n}),\ [u]_{B_{p,p}^{\sigma ,F}}^{p}:=\sum_{n=0}^{\infty }\Phi
_{u}^{\sigma ,F}(\rho ^{n}),
\end{equation*}%
where
\begin{equation*}
...
B_{p,p}^{\sigma ,F} &:&=\{u\in L^{p}(K^{F},\mu ^{F}):[u]_{B_{p,p}^{\sigma
,F}}<\infty \}.
\end{eqnarray*}%
...
\mu ^{F},$ which is naturally defined by
\begin{equation*}
\mu ^{F}=\sum_{f\in F}\mu \circ f^{-1},
\end{equation*}%
i.e., just `glue' up the measures for each tile. So we denote
\begin{equation}
I_{\infty ,n}^{F}(u)=\lim_{m\rightarrow \infty
}I_{m,n}^{F}=\int_{K^{F}}\int_{B(x,C_{H}\rho ^{n})}|u(x)-u(y)|^{p}d\mu
^{F}(y)d\mu ^{F}(x).  \label{mu_m}
\end{equation}%
%
%
%
...
%\end{equation}
We will see the estimates of $I_{\infty,n}^{F}(u)$ in Proposition \ref%
{prop:I} and Lemma \ref{lem6.2}.

\begin{proposition}
\label{prop:I}For all $u\in B_{p,\infty }^{\sigma ,F}$, we have%
\begin{equation}
\liminf_{n\rightarrow \infty }\rho ^{-n(p\sigma +\alpha )}I_{\infty
,n}^{F}(u)\geq \rho ^{p\sigma +\alpha }C_{H}\liminf_{n\rightarrow \infty
}\Phi _{u}^{\sigma ,F}(\rho ^{n}).  \label{II-1}
\end{equation}
\end{proposition}
...
^{F}(x)  \notag \\
&>&\rho ^{p\sigma +\alpha }C_{H}\rho ^{-(n+s+1)(p\sigma +\alpha
)}\int_{K^{F}}\int_{B(x,\rho ^{n+s+1})}|u(x)-u(y)|^{p}d\mu ^{F}(y)d\mu
^{F}(x).  \notag
\end{eqnarray}
...
\begin{lemma}
\label{lem6.2} Let $K^{F}$ be a fractal glue-up, then
\begin{equation}
...
\sum_{|w|=n}\sum_{f\in F}1_{\{z\in f(V_{w})\cap g(V_{\tilde{w}%
 ...
 \leq &C_{2}N^{-(m+n)}\sum_{|w|=n}\sum_{f\in F}\sum_{z\in
f(V_{w})}\sum_{x\in f(K_{w}\cap V_{m})}|u(x)-u(z)|^{p}\notag \\
&+C_{2}N^{-(m+n)}\sum_{|%
\tilde{w}|=n}\sum_{g\in F}\sum_{z\in g(V_{\tilde{w}})}\sum_{y\in g(K_{\tilde{%
w}}\cap V_{m})}|u(z)-u(y)|^{p}  \notag \\
...
estimate
\begin{eqnarray}
|V_{m}| &\asymp &N^{m}=\rho ^{-\alpha m}\text{ and }  \label{v1} \\
...
f(V_{w})}\sum_{k=n}^{m-1}N^{k-n}|u(x_{k}(x,z))-u(x_{k+1})|^{p}  \notag \\
...
}^{\sigma }$ essentially embeds into $C(K)$ when $\sigma >\alpha /p$.

\begin{lemma}
\cite[Theorem 3.2]{BaudoinLecture2022} \label{lemmaMR} Let $(M,d,\mu )$ be a
metric measure space. When $\sigma >\alpha /p$, for any $u\in B_{p,\infty
}^{\sigma }$, there exists a continuous version $\tilde{u}\in C^{(p\sigma
-\alpha )/p}(M)$ satisfying $\tilde{u}=u$ $\mu $-almost everywhere in $M$
and
\begin{equation}
|\tilde{u}(x)-\tilde{u}(y)|\leq C|x-y|^{(p\sigma -\alpha )/p}\sup_{r\in
(0,3d(x,y)]}\Phi _{u}^{\sigma }(r)\leq C|x-y|^{(p\sigma -\alpha
)/p}[u]_{B_{p,\infty }^{\sigma }},  \label{M-S}
\end{equation}%
...
So from now on, we regard $B_{p,\infty }^{\sigma }$ and $B_{p,p}^{\sigma }$
as subspaces of $C(K)$ (recall Proposition \ref{Prop31}) when $\sigma
>\alpha/p$.

\begin{lemma}
\label{lem6.3} Let $K^{F}$ be a fractal glue-up. If $\sigma >\alpha /p$,
then for any $0<\delta <p\sigma -\alpha $ and $u\in B_{p,\infty }^{\sigma
,F} $,
\begin{equation}
...
$x_{k}\in f(K_{w_{k}})$ for $k=n,n+1,...m$. Let $0<\delta <p\sigma -\alpha $%
...
K_{w_{k}}$ and dividing by $\mu ^{F}(f(K_{w_{k}}))$, we obtain
\begin{align*}
...
\int_{f(K_{w_{m}})}|u(a)-u(x_{m})|^{p}d\mu ^{F}(x_{m})\\ \leq&
C|f(V_{n})|2^{p-1}\rho ^{(p\sigma -\alpha )m}[u]_{B_{p,\infty }^{\sigma
}(K^{F})}^{p} \\
\leq& C^{\prime }|f(V_{n})|2^{p-1}\rho ^{(p\sigma -\alpha
)m}[u]_{B_{p,\infty }^{\sigma ,F}}^{p} \\
...
)k}\right) \sup_{k\geq 0}\Phi _{u}^{\sigma }(\rho ^{n+k})
=\frac{C_{5}}{%
...
In \cite[Theorems 1.4 and 1.5]{GaoYuZhang2022PA}), for a homogeneous p.c.f.
self-similar set, we show that the local $p$-energy norm equivalence
\begin{equation}
\mathcal{E}_{p,\infty }^{\sigma }(u)\asymp \lbrack u]_{B_{p,\infty }^{\sigma
}}^{p}\text{ \ (}p\sigma >\alpha \text{),}  \label{204}
\end{equation}%
and the non-local $p$-energy norm equivalence%
\begin{equation*}
\mathcal{E}_{p,p}^{\sigma }(u)\asymp \lbrack u]_{B_{p,p}^{\sigma }}^{p}\text{
\ (}p\sigma >\alpha \text{)}.
\end{equation*}%
...
_{n}^{\sigma ,F}(u),
\end{equation*}%
and non-local $p$-energy norm for $K^{F}$ by
\begin{equation*}
\mathcal{E}_{p,p}^{\sigma ,F}(u):=\sum_{n=0}^{\infty }\rho ^{-n(p\sigma
-\alpha )}E_{n}^{(p),F}(u)=\sum_{n=0}^{\infty }\mathcal{E}_{n}^{\sigma
,F}(u).
\end{equation*}

\begin{corollary}
\label{lem6.1} Let $K^{F}$ be a fractal glue-up. If $p\sigma >\alpha $, then
for $u\in B_{p,\infty }^{\sigma ,F}$
\begin{equation}
...
\eqref{eq6.4} gives the desired.
\end{proof}

\subsection{Property (E) holds for nested fractals}

\label{subsec5.2} Let us introduce the discrete (vertex-type) weak
monotonicity we concern. For a set $V$, denote $\ell (V)=\{u:$ $u$ maps $V$
into $\mathbb{R}\}$.
...
(E)}, if there exists $\sigma>\alpha/p$ and a positive constant $C$ such that
...
_{p}^{*}}(u)\leq C\mathcal{E}_{n}^{\sigma _{p}^{*}}(u)$, so Property (E)
implies property (VE) that is defined as follows.
...
\end{equation*}%
And we say that a fractal glue-up $K^{F}$ satisfies \textrm{property $(%
...
The fact $\sigma _{p}^{\ast }(K)=\sigma _{p}^{\#}(K)$ follows from \cite[%
Proposition 3.4]{GaoYuZhang2022PA} (the proof of this part does not make use
of property (VE)). It is also clear that $\sigma _{p}^{\#}(K)\geq \sigma
_{p}^{\#}(K^{F})$, since any non-constant function $u\in l(K^{F})$ in $%
B_{p,\infty }^{\sigma ,F}$ naturally gives a non-constant function in $%
B_{p,\infty }^{\sigma }(K)$ for any $\sigma >0$: just restrict $u$ to a tile
$f(K)$ where it is non-constant (for some $f\in F$).
...
Finally, since $K$ satisfies property (VE), that is for all $n$,
\begin{equation*}
...
\begin{lemma}
\label{lemma:E}Property (E) holds for nested fractals. \label{yyqx}
\end{lemma}

\begin{proof}
In this proof we use the same terminology and notions as \cite%
{Caoqiugu2022adv}. Fix a nested fractal $K$. We fix the components $r_{i}$
of $\mathbf{r}$ in \cite[Theorem 6.3]{Caoqiugu2022adv} to be the same number
$r$, so it is `$\mathcal{Q}$-symmetric' (see \cite[Section 3]{Caoqiugu2022adv} for
definition). Then \cite[Theorem 6.3]{Caoqiugu2022adv} states that,
`condition (A)' holds for affine nested fractals (probably inhomogeneous),
which includes $K$. So by \cite[Lemma 5.4]{Caoqiugu2022adv}, we can fix $r=
r_{p}$ to further satisfy condition (A') and condition (R) on $K$ (see \cite[%
Section 5]{Caoqiugu2022adv}).

Now \cite[Theorem 4.2]{Caoqiugu2022adv} gives a fixed point $E$ for the operator $\mathcal{T}$ with $\mathcal{T}E=E$ (see \cite[%
Definition 2.8, Definition 3.1]{Caoqiugu2022adv} for related definitions).
The equivalence of $E$ and $E_{0}^{(p)}$ (for functions $u\in l(V_{0})$,
see the statement of \cite[Theorem 5.1]{Caoqiugu2022adv}) is stated in the
proof of \cite[Proposition 5.3 (b)]{Caoqiugu2022adv}, which implies that
\begin{equation}
\Lambda ^{n}E(u)\asymp \Lambda ^{n}E_{0}^{(p)}(u)=r_{p}^{-n}E_{n}^{(p)}(u)
\label{205}
\end{equation}%
for all functions $u\in l(V_{n})$ and all $n\geq 0$ by the definition of $%
\Lambda $ in \cite[Definition 3.1]{Caoqiugu2022adv} and $E_{n}^{(p)}$ in our paper.

Now fix $\sigma=\frac{\log_{\rho}r_p+\alpha}{p}$ so that $r_{p}^{-n}=\rho^{-
n (p \sigma -\alpha)}$.

For property (E)(i), just use the monotonicity property in \cite[Proposition
5.3 (a)]{Caoqiugu2022adv}, and note that (\ref{205}),
\begin{equation*}
E_{0}^{(p)}(u)\asymp E(u)\leq \Lambda E(u)\leq \cdots \leq \Lambda
^{n}E(u)\asymp r_{p}^{-n}E_{n}^{(p)}(u)=\rho ^{-n(p\sigma -\alpha
)}E_{n}^{(p)}(u)=\mathcal{E}_{n}^{\sigma }(u),
\end{equation*}%
so (VE) holds with $\sigma $.

Property (E)(ii) are called piecewise harmonic functions in \cite[Section 5]%
{Caoqiugu2022adv}, which has a standard iterative construction as in \cite%
{HermanPeironeStrichartz.2004.PA125} and Dirichlet form theory. The
iteration part is, for any boundary value of a cell $u_{w}\in
l(f_{w}(V_{0})) $, take its next-level harmonic extension $\tilde{u}_{w}\in
l(f_{w}(V_{1}))$ such that $\Lambda E(\tilde{u}_{w})=E(u_{w})$ and $\tilde{u}%
_{w}|_{f_{w}(V_{0})}=u_{w}$. The iteration starts from $V_{0}$ to $V_{1}$,
and from each level-1 cell to level-2 cells in the above way, then finally
to $V_{\ast }$. The p.c.f. property is very crucial for the piecewise
extension to be well-defined, that is, to avoid different values for the
same vertex (since the overlap of same-level cells only occurs on the
same-level scaling of $V_{0}$). Such an extension satisfies the definition
of piecewise harmonic functions in \cite[Section 5]{Caoqiugu2022adv}, so by
\cite[Section 5.2]{Caoqiugu2022adv} we know that it naturally embeds into $%
C(K)$ and this gives an extension to $K$ rather than $V_{\ast }$. Therefore $%
r_{p}=\rho ^{p\sigma -\alpha }$ and our chosen $\sigma =\sigma _{p}^{\#}$.
\end{proof}
...
It is known that $K$ admits a heat kernel with estimate \eqref{hk}, so
...
Using property (E), the $p$-energy in \cite[Theorem 5.1]{Caoqiugu2022adv}
with $r_{i}=r_{p}$ is equivalent to $\lim_{n\rightarrow \infty }\Lambda
^{n}E(u)$ (which exists by \cite[Proposition 5.3]{Caoqiugu2022adv}), and in
our proof we see that $[u]_{B_{p,\infty }^{\sigma _{p}^{\#}}}^{p}$ is
equivalent to $\lim_{n}\Lambda ^{n}E(u)$.

The equivalence of $[u]_{B_{p,\infty }^{\sigma
_{p}^{\#}}}^{p}$ and the $p$-energy defined in \cite%
{HermanPeironeStrichartz.2004.PA125} on the Sierpi\'{n}ski gasket have already been explained in \cite[%
Section 4]{GaoYuZhang2022PA}.
\end{remark}
...
double-side estimate, so a lower bound can be obtained after we find the
...
\begin{proof}
Since $C_{H}\in (0,1)$, assume that $\rho ^{s+1}\leq C_{H}<\rho ^{s}$ for $%
s\in \mathbb{N}$. For all $n>0$, we have by (\ref{II-2}) that%
\begin{equation}
...
By definition of $\limsup_{n\rightarrow \infty }\mathcal{E}_{k}^{\sigma
...
\end{align*}%
where we use \eqref{I_n} in the third line. Therefore, taking $\liminf $ in
the right-hand side above, we have
\begin{equation*}
...
\end{equation*}%
Fix a large integer $L$ such that $C_{3}>0$, it follows that
\begin{align*}
...
\end{align*}%
The second inclusion follows by definition of $K^{F}$.
\end{proof}
...
\begin{equation*}
\rho ^{p\sigma +\alpha }C_{H}\liminf_{n\rightarrow \infty }\Phi _{u}^{\sigma
,F}(\rho ^{n})\leq C\liminf_{n\rightarrow \infty }\sum_{k=0}^{L}\rho
^{k(p\sigma -\alpha )}\mathcal{E}_{k+n}^{\sigma ,F}(u)+\frac{C^{\prime }\rho
^{(p\sigma -\alpha )(L+1)}}{1-\rho ^{p\sigma -\alpha }}\liminf_{n\rightarrow
\infty }\Phi _{u}^{\sigma ,F}(\rho ^{n}).
\end{equation*}

For sufficiently large $L$ ($>n_{0}$), we have
\begin{equation*}
C^{\prime \prime }:=\frac{1}{C}\left( \rho ^{p\sigma +\alpha }C_{H}-\frac{%
C^{\prime }\rho ^{(p\sigma -\alpha )(L+1)}}{1-\rho ^{p\sigma -\alpha }}%
\right) >0.
\end{equation*}
...
Let $\sigma >\alpha /p$. Note that (\ref{ns}), (\ref{ne}). Using %
\eqref{eq6.2} and \eqref{limphi2}, $(\widetilde{VE})$ implies $(\widetilde{NE%
})$ since
\begin{equation}
...
\end{equation}%
By \eqref{eq6.2} and \eqref{limphi}, $(\widetilde{NE})$ implies $(\widetilde{%
VE})$ since
\begin{equation}
\limsup_{n\rightarrow \infty }\mathcal{E}_{n}^{\sigma ,F}(u)\leq
...
By \eqref{eq6.2} and \eqref{limphi2} adjoint with the fact $(VE)\Rightarrow (%
...
Finally, we mention that, by Theorem \ref{thm4.2}, Theorem \ref{thm5} and
Corollary \ref{lem6.1}, we can only use property (VE) instead of property
(E) in \cite[Theorem 1.6]{GaoYuZhang2022PA} to obtain the vertex-type BBM
convergence of non-local $p$-energy forms to a local $p$-energy form for $K$%
, without using the Gamma-convergence.

\subsection{Consequences of (VE): BBM convergence, Sobolev-type and
Gargliado inequalities}

\label{subsec5.4}

\begin{proof}[Proof of Corollary \protect\ref{corol1.9}]
By Proposition \ref{prop:critical} and (\ref{BC}), we have that $K^{F}$
satisfies (VE) and the critical domain $B_{p,\infty }^{\sigma
_{p}^{\#}(K^{F})}(K^{F})$ is non-trivial, so that the BBM convergence holds
and (NE) holds true for $K^{F}$ by Theorem \ref{thm5} with $R_{0}=1$.

For a nested fractal $K$, it is known by \cite[Theorem 1.1]%
{FitzsimmonsmHamblyKumagai.1994.CMP595} that $K_{\infty }$ admits a heat
kernel with estimate \eqref{hk}. Combining Lemma \ref{lemma:E}, previous
inclusions hold. Now we state how to upgrade $R_0=1$ to $R_0=\infty $ for
(NE), so Lemma \ref{GNTM} will hold true. Moreover, (KE) will hold true by Theorem \ref{thm4}, (DUE) is satisfied and %
\eqref{sobo} will hold true for $K^{F}$.

For any $n\in \mathbb{N}$, define the scaling function $g_{n}:K^{F}\rightarrow \rho
^{-n}K^{F}$ by
\begin{equation*}
g_{n}(x)=\rho ^{-n}x.
\end{equation*}
...
\end{eqnarray}
where we use (\ref{ss}) and $\mu ^{F}$ is $\alpha$-regular in the forth line.
For any $u\in B_{p,\infty }^{\sigma ,F}$, there exists $n<\infty $ such that
\begin{equation*}
...
holds with $R_{0}=\infty $.

As $\mu $ and $\mu ^{F}$ are both $\alpha $-regular, we know that Lemma \ref{GNTM}
holds.
\end{proof}
...
quite different way (sub-Gaussian heat kernel estimate) to obtain the
...
\begin{corollary}
When $p=2$, if a homogeneous p.c.f. self-similar $K$ admits heat kernel
estimate \eqref{hk}, then (VE) and (NE) automatically hold for $K$ and $K^F$.
\end{corollary}

\begin{proof}
When $p=2$, property (KE) automatically holds. By Corollary \ref{corol1.8},
we have (VE) and (NE) automatically hold for $K$. By proposition \ref%
{prop:critical}, we have the same conclusion for $K^F$.
\end{proof}
...
suggested.
\begin{conjecture}
Suppose that two metric measure spaces $(M_1,d_1,\mu_1),(M_2,d_2,\mu_2)$ admit two heat kernels satisfying \eqref{hk} respectively, with the same exponent $\alpha,\beta^*$.
...
.
\begin{conjecture}Does heat kernel estimate \eqref{hk} implies
(KE)?\end{conjecture}Beyond this, it is also interesting to consider other
infinitesimal characterization using heat kernels (2-Laplacian). Heat-kernel
based $p$-energy has a short and direct expression than other types of $p$%
-energy (which involves discretization), especially when the underlying
space do not have Ahlfors-regularity (even for the Dirichlet form). For
spaces with less regular heat kernel, one can consider the variant of %
\eqref{psi_u}:
\begin{equation}
...
(11871296). The authors thank Jiaxin Hu, Gu Qingsong and Meng Yang for many
...
{\sc P.~Alonso~Ruiz and F.~Baudoin}, {\em Gagliardo-{N}irenberg,
  ...
  Math. Anal. Appl., 497 (2021), pp.~124899, 26.
...
{\sc P.~Alonso-Ruiz, F.~Baudoin, L.~Chen, L.~Rogers, N.~Shanmugalingam, and
  ...
  {S}obolev type inequalities}, J. Funct. Anal., 278 (2020), pp.~108459, 48.
...
{\sc S.~Cao, Q.~Gu, and H.~Qiu}, {\em {$p$}-energies on p.c.f. self-similar
  sets}, Adv. Math., 405 (2022), pp.~108517, 58.
...
{\sc J.~Gao, Z.~Yu, and J.~Zhang}, {\em Convergence of p-energy forms on
  homogeneous p.c.f self-similar sets}, Potential Anal.,  (2022).

\bibitem{Gorny2022}
{\sc W.~G\'{o}rny}, {\em Bourgain-{B}rezis-{M}ironescu approach in metric
  spaces with {E}uclidean tangents}, J. Geom. Anal., 32 (2022), pp.~128, 22.
...
\bibitem{Strichartz.2006.}
{\sc R.~Strichartz}, {\em Differential equations on fractals: a tutorial},
