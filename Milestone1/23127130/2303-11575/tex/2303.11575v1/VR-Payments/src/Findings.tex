...
% \TODOS{May need to rephrase it to fit better as a factor}
...
% \item \textbf{Consent:} the authentication requires users’ consent and ensures their consent in initiating the task~\cite{missing}.
% \item \textbf{Privacy:} the authentication requires private information, e.g., a secret key or user signature, to complete the process~\cite{missing}. 
% \item \textbf{Security:} the authentication employs a mechanism to secure the task~\cite{missing}.
% \item \textbf{Being alerted:} the authentication makes users aware, cautious and pay attention on the task~\cite{missing}.
% \item \textbf{In-control:} the authentication impacts users’ perceived control and agency over the task~\cite{missing}.
% \end{itemize}
...
Usability challenges during interaction reduced realism of authentication mechanisms for participants. In addition, lack of proper feedback negatively impacts participants’ perception of consent in VR. Some participants assumed that the certain authentication mechanisms trade off security for usability %better tolerance in interaction,
...
% \begin{figure}[h]
% \centering
% \includegraphics[width=\columnwidth]{charts/consent.png}
% \caption{Users' perception that consent was given to the payment method}\label{fig:consent}
% \end{figure}
...
% Though previously we show that participants prefer PIN-based interfaces for their security, we marked that the usability of authentication directed their preference in adoption.
% The section below lists the qualities that the users prefer and would expect in authenticating in VR.
...
% Such modalities may also offer additional security in VR, e.g., by \textit{``using unique 3D movment instead of a flat 2D traditional signature''} (P5).
...
% Participants realized conflicts between usability and being informed, e.g., for \tap:
% \begin{displayquote}
% \textit{``Maybe we can try to resolve the issues and still keep its convenience.''} (P19)
% \end{displayquote}
