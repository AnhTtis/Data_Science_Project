...
%
%   This version: V9, 3.9.2023
%   Additions on p.29, in particular, Corollary 9.2.	 
%												           	
...
\keywords{Riemann surface; complex curve; Oka-1 manifold; Oka manifold; K3 surface; rationally connected manifold}
...
We provide many examples of Oka-1 manifolds among compact complex surfaces, 
including all Kummer surfaces and all elliptic K3 surfaces.
We also show that every compact rationally connected K\"ahler manifold is 
an Oka-1 manifold. The class of Oka-1 manifolds is invariant under Oka maps inducing 
...
The analogous result holds for holomorphic Legendrian immersions in an arbitrary
...
sufficient conditions in the theory of Oka manifolds,  
and it is the first known condition implying an Oka-type property of a complex
manifold which is purely local. 
%An immediate corollary is that for a Riemann surface $X$ the conditions 
%\[
%	\text{Oka} \Longrightarrow \text{densely dominable} \Longrightarrow \text{Oka-1} 
...
In Section \ref{sec:RC} we prove that every compact rationally connected 
K\"ahler manifold $X$ is an Oka-1 manifold; see Theorem \ref{th:RC}. 
We were motivated by the paper 
...
in a rationally connected manifold; in particular, such lines can be dense in $X$.
In the proof of Theorem \ref{th:RC} we use a Runge approximation theorem 
for compact complex curves in certain almost complex manifolds, 
due to Gournay \cite{Gournay2012}, combined with Proposition \ref{prop:int}
which adds interpolation to Gournay's result.  
...
We denote by $\Hscr^{k}=\Hscr^{k}_{X,g}$ the $k$-dimensional Hausdorff measure on
...
irrelevant in the condition $\Hscr^{2n-1}(E)=0$ used in the theorem. 
...
there is a closed subset $E\subset X$ with $\Hscr^{2n-1}(E)=0$ such
...
or by Euclidean spaces, is a considerably weaker condition on a complex manifold 
...
$A=X\setminus \Omega$ is a closed complex subvariety of $X$ with $\Hscr^{2n-1}(A)=0$.
...
%
Likewise, if $\Hscr^{2n-1}(E)=0$, the same argument shows that 
...
where we focus on complex surfaces, and in Section \ref{sec:RC}
where we show that every compact rationally connected K\"ahler manifold
...
order $k$ at the points in $A\subset \mathring K$,
...
there is a closed subset $E\subset \Omega$ with $\Hscr^{2n-1}(E)=0$ such that
...
with $\Hscr^{2n-1}(E)=0$ such that $\Omega$ is dominable by a spanning tube 
...
and let $f:K\to Y$ be a holomorphic map. Then $f$ lifts 
to a holomorphic map $g:K\to X$. By the assumption we can approximate 
$g$ by a holomorphic map $\tilde g: \C\to X$ which agrees with $g$ at the points
$a_1,\ldots, a_m\in K$. The holomorphic map $\tilde f=h\circ \tilde g:\C\to Y$ 
then approximates $f$ on $K$ and agrees with $f$ at the points $a_1,\ldots, a_m$. 
Applying this inductively on an increasing sequence of discs 
exhausting $\C$ gives (b).
\end{proof}
...
invariance of Oka-1 manifolds under Oka maps with connected fibres. 
We recall this notion, referring to \cite[Section 7.4]{Forstneric2017E} and
\cite[Section 3.6]{Forstneric2023Indag} for a more complete presentation.
...
%   OKA-1 MAP
%
In the proof of parts (a) and (b) of Theorem \ref{th:updown} 
we only used the following properties of the map $h:X\to Y$. 
...
\begin{proof} 
It follows from the definition of an Oka-1 map
$h:X\to Y$ that for every holomorphic disc $g:\Delta\to Y$ and point 
$x\in h^{-1}(g(0)) \in X$
...
Furthermore, since any pair of points in $Y$ lie in the image 
of a holomorphic disc $\Delta\to Y$, we see that $h$ is surjective. 
Hence, every fibre $h^{-1}(y)$ $(y\in Y)$ is smooth (a complex submanifold
of $X$), and it follows from the definition of an Oka-1 map that 
$h^{-1}(y)$ is an Oka-1 manifold for every $y\in Y$.
\end{proof}

The proof of Theorem \ref{th:updown} gives the following corollary.

\begin{corollary}
If $X$ and $Y$ are connected complex manifolds and $h:X\to Y$ is an Oka-1 map,  
then parts (a) and (b) of Theorem \ref{th:updown} hold.
\end{corollary}
...
Note that in part (c) we may use different manifolds with LSAP to dominate $X$ 
at different points. 
\end{proof}
...
Let $f:X\to C$ be an elliptic fibration over an elliptic curve $C$. Then, the surface $X$ is 
densely dominable by $\C^2$ if and only if the fibration $f$ has no multiple fibres.
Every elliptic surface with this property is an Oka-1 manifold.
\end{corollary}
...
\section{Every compact rationally connected K\"ahler manifold is Oka-1}\label{sec:RC}
...
is connected by a rational curve $\CP^1\to X$. Among many excellent 
references, we mention the monographs by Koll\'ar \cite{Kollar1995E} and 
Debarre \cite{Debarre2001}.
...
We remark that every compact rationally connected K\"ahler manifold is 
a projective manifold (see Voisin \cite[Remark 0.5]{Voisin2008}). 

Theorem \ref{th:RC} is new for manifolds of dimension $>2$. 
In dimension $1$ the only such manifold is $\CP^1$, which is Oka, 
while every compact rationally connected surface is rational
(i.e., birationally equivalent to $\CP^2$), hence an Oka manifold; 
see Subsection \ref{ss:Kodairaminusinfinity}.

Theorem \ref{th:RC} is interesting from another viewpoint. 
There is well known close relationship between negativity properties of hermitian 
metrics on a complex manifold $X$ and Kobayashi hyperbolicity of $X$,
going back to the Ahlfors lemma; see \cite[Section 11]{Forstneric2023Indag}
for a discussion of this topic and references.  
It is conjectured that a similar relationship
exists between the Oka properties of a complex manifold 
%(which are opposite to the rigidity properties encapsulated by hyperbolicity)
and positivity of complete hermitian or K\"ahler metrics on it. 
Evidence for this comes from the Frankel Conjecture, solved affirmatively by 
Mori \cite{Mori1979} and Siu and Yau \cite{SiuYau1980}, saying that a 
compact K\"ahler manifold with positive 
holomorphic bisectional curvature is biholomorphic to a complex projective space, 
which is an Oka manifold. 
(Mori's theorem holds under the weaker assumption that the manifold 
has ample tangent bundle.) This was generalized by Mok 
\cite{Mok1988} whose result  implies 
that every compact K\"ahler manifold with nonnegative holomorphic bisectional 
curvature is an Oka manifold (see \cite[Theorem 11.4]{Forstneric2023Indag}).
It was asked in \cite{Forstneric2023Indag} whether 
every compact hermitian or K\"ahler manifold with positive holomorphic
sectional curvature is Oka. (Note that the sectional curvature provides
much less information than the bisectional curvature.) This question
seems completely open. However, the following is a corollary to Theorem \ref{th:RC} 
and the solution of Yau's conjecture \cite[Problem 47]{Yau1982}
by X.\ Yang \cite{YangX2018}, saying that every compact K\"ahler manifold with 
positive holomorphic sectional curvature is projective and rationally connected.

\begin{corollary}
A compact K\"{a}hler manifold with positive holomorphic sectional curvature
is an Oka-1 manifold.
\end{corollary}
...
In particular, every such manifold contains dense entire curves $\C\to X$. 
Theorem \ref{th:RC} applies more generally to maps $R\to X$ from any
open Riemann surface, and it is more precise 
even in the special case $R=\C$ since it gives interpolation 
on a discrete set, instead of mere hitting of a given sequence in $X$. 
In their paper, Campana and Winkelmann give further examples of complex manifolds 
...
rational curves in rationally connected projective manifolds, in particular, on smoothing a  
...
Using these methods, we tried to show that a rationally connected projective manifold  
...
While it may seem unusual to obtain an approximation result for noncompact 
holomorphic curves in a compact complex manifold from an approximation result
for compact curves, one should bear in mind that the holomorphic function theory on
open Riemann surfaces, and in particular the Runge approximation theorem
on such surfaces, was developed by Behnke and Stein \cite{BehnkeStein1949}
in 1949 by applying the previously established theory of meromorphic functions
on compact Riemann surfaces.

%%%%%%%%%%%%

Before turning to the proof of Theorem \ref{th:RC}, we recall some background on
rationally connected projective manifolds, referring to the monographs by 
Debarre \cite{Debarre2001} and Koll\'ar \cite{Kollar1995E} for more information,
and we indicate a possible approach to finding dominating
...
Trees $C\to X$ as in \eqref{eq:dominatingrational} exist in every
rationally connected projective $n$-manifold $X$. To see this, recall that 
for every rational curve $f:\CP^1\to X$ we have that 
\begin{equation}\label{eq:Birkhoff}
	f^*TX=\oplus_{i=1}^n \Oscr_{\CP^1}(a_i)\ \ \  \text{with $a_i\in \Z$}
\end{equation}
by the Birkhoff factorization theorem \cite{Birkhoff1909}, and $\max a_i\ge 2$ if $f$ 
is nonconstant since in this case the bundle $f^*TX$ contains the tangent bundle 
$T\CP^1\cong \Oscr_{\CP^1}(2)$ of $\CP^1$.
...
A compact projective manifold $X$ with a very free curve $f:\CP^1\to X$ 
is rationally connected, and the associated evaluation map $ev:M\times \CP^1\to X$ 
with $ev(m_0,\cdotp)=f$, where $M$  (with a base point $m_0\in M$) is the moduli space 
of rational curves in $X$ homologous to $f$, is a smooth morphism with dense image 
(see \cite{AraujoKollar2003,Debarre2001,Kollar1995E}). 
In particular, every free rational curve $f:\CP^1\to X$ is {\em regular}, 
in the sense that the linearized $\dibar$-operator $D_f$
%\[ 
...
in the same bundle is surjective. An explicit definition and expression 
for $D_f$ is given by \eqref{eq:Dfdef} and \eqref{eq:Df} 
below in a much more general situation.
If a projective manifold $X$ is rationally connected, then for most points $x\in X$ and 
tangent vectors $v\in T_xX$ there is a rational curve $\CP^1\to X$ through $x$ 
in the direction $v$, so we obtain trees as in \eqref{eq:dominatingrational}. 
...
from some open neighbourhood $U\subset (\CP^1)^n$ of $C$?
\end{problem}
...
result from Oka theory, a holomorphic extension of $f$ 
...
However, $C$ has neighbourhoods in $Y$ which admit a nonconstant bounded 
plurisubharmonic function, so they are too small to contain 
a spanning tube of lines in $\C^2$ since the latter are Liouville.
...
great abundance even if the almost complex structure $J$ on $X$ is nonintegrable; 
...
The precise choice of the metric on $X$ is not important in Gournay's proof,
a fact which we shall explore in the proof of the interpolation 
result in Proposition \ref{prop:int}.
Given a smooth map $f:\Sigma\to X$, its differential $df$, considered 
as a $1$-form on $X$ with values in $f^* TX$, 
decomposes as $df=\di_J f + \dibar_J f$ where
\[
...
Elements $\xi\in \Cscr^\infty(\Sigma,f^* TX)$ are smooth vector fields on $X$
along the map $f$. For $z\in \Sigma$ we have the geodesic curve 
$\R \ni s\mapsto \exp_{f(z)}(s\xi(z))\in X$ passing at $s=0$ through the point 
$f(z)$ in the direction of the vector $\xi(z)\in T_{f(z)}X$. 
Let $\xi\in \Cscr^\infty(\Sigma,f^* TX)$. The parallel transport along the geodesic flow 
...
so $\nabla J=0$ and the second term on the right hand side of 
\eqref{eq:Df} vanishes. If in addition the map $f$ is holomorphic then 
the first term on the right hand side equals $\dibar_J \xi$, so 
$D_f$ is the standard $\dibar$-operator on the holomorphic 
vector bundle $f^*TX$. The same 
...
holomorphic everywhere, so this argument does not apply. 
...
Assume now that $f\in\Bscr$ is such that $\dibar_J f$ is small in a suitable norm and 
the linearized operator $D_f: T_f\Bscr \to \Escr_f$ given by \eqref{eq:Dfdef} has a bounded
right inverse $S_f$ so that $D_f \circ S_f=\Id$ on $T_f\Bscr=\Cscr^\infty(\Sigma,f^*TX)$.
The plan is to find a holomorphic map 
$\tilde f=\exp_f (\tilde \xi):\Sigma\to X$ close to $f=\exp_f(0)$, 
where $\tilde \xi$ is the fixed point of the Newton iteration given by the map
\[
...
of $\Hcal$ which solves the problem. In the context of Banach spaces, a precise 
...
\begin{equation}\label{eq:DTP}
	...
	\text{on the annulus $A_r=\Bigl\{\frac{r}{1+r^\epsilon} \le |z|\le  r(1+r^\epsilon)\Bigr\}$}.
\end{equation}
...
$J$ is not regular as long as these curves are not used in the construction.
...
The holomorphic map $F:\Sigma\to X$ in Theorem \ref{th:Gournay} can 
...
version of the comb smoothing theorem of Koll\'ar, Miyaoka, and Mori 
...
on the interpolation data. To obtain such $f$, one proceeds as follows; 
see \cite[Sect.\ 3.1]{Gournay2012}. 

In local coordinates at a point $p\in \Sigma\setminus K$ and 
$g(p)\in X$ as in the definition of the double tangent property, in which these points are 
...
by the holomorphic disc $f_p(\{|z|\le r\})$ without changing the map much 
...
Note that $f$ agrees with $g$ near $K$ but may be far from 
...
$D_{f}\xi_i =\eta_i$ on each of the grafted discs $U_i=\Delta_i\subset \Sigma$ $(i>0)$, 
%, which come from free rational curves as described in the first step of the proof, 
and for $i=0$ on a neighbourhood $U_0\subset \Sigma$ of the set 
$\Sigma \setminus \bigcup_{i=1}^m \Delta_i$.
%, a finite bordered Riemann surface. 
The sections $\xi_i\in \Cscr^\infty(f^*TX|_{U_i})$ 
...
$D_f \xi_i=\eta_i$ which ensure that $\xi$ satisfies the equation $D_f\xi=\eta$. 
...
\end{equation}
and considers the corresponding Neumann problem 
...
$\eta_i$, $h_i$ for $i>0$. This process converges to a solution 
...
If $(X,J)$ is a rationally connected projective manifold,
...
In the first part we use an idea of Kuzman \cite[proof of Corollary 7]{Kuzman2018}. 
...
neighbourhood of $K$ where the map $g$ is holomorphic and $f=g$. 
On each $\Delta'_i$ the linearized equation $D_f\xi=\eta$ admits a solution 
...
The collection of grafted discs $\Delta_i$ has to depend on the 
...
Assume now that $J$ is integrable on a neighbourhood of 
$\bigcup_{i=1}^k f(\overline\Delta'_i)$. 
We may assume that the discs $\Delta'_i$ are chosen small 
...
\[
	\exp_{f(z)}(\tilde \xi(z)) = f(z) + \tilde \xi(z) \quad
	\text{for $z\in \Delta'_i$ and $\tilde \xi(z)$ small enough.}
\]
...
Recall that every compact rationally connected K\"ahler manifold $X$ is a projective manifold
(see Voisin \cite[Remark 0.5]{Voisin2008}).
%
% https://mathoverflow.net/questions/332780/rationally-connected-k%C3%A4hler-manifolds-are-projective
%
It is simply connected \cite[Corollary 4.18]{Debarre2001}. 
Furthermore, on any compact rationally connected projective 
manifold there is a positive rational curve passing through any given 
...
positive (very free) rational curves in $X$ satisfy \cite[Proposition 1.2.1]{Gournay2012},
and hence the double tangent property holds. This gives the following.

\begin{corollary}\label{cor:RC}
Theorem \ref{th:Gournay} and Proposition \ref{prop:int} hold on every 
compact rationally connected K\"ahler manifold $(X,J)$.
\end{corollary}
...
we see that a compact rationally connected projective manifold $X$ satisfies the 
Oka-1 property with approximation (see Definition \ref{def:Oka1}).  
...
\cite[proofs of Theorems 1.6 and 1.8]{AlarconForstneric2023Embedded} 
...
The authors wish to thank Rafael Andrist, Kyle Broder, 
...
\bibitem{Mok1988}
N.~Mok.
\newblock The uniformization theorem for compact {K}\"{a}hler manifolds of
...
\newblock {\em Ann. Math. (2)}, 110:593--606, 1979.
...
\bibitem{SiuYau1980}
Y.-T. Siu and S.-T. Yau.
\newblock Compact {K{\"a}hler} manifolds of positive bisectional curvature.
\newblock {\em Invent. Math.}, 59:189--204, 1980.
...
\bibitem{Voisin2008}
C.~Voisin.
\newblock Rationally connected 3-folds and symplectic geometry.
\newblock In {\em G\'eom\'etrie diff\'erentielle, physique math\'ematique,
  math\'ematiques et soci\'et\'e (II). Volume en l'honneur de Jean Pierre
  Bourguignon}, pages 1--21. Paris: Soci{\'e}t{\'e} Math{\'e}matique de France,
  2008.
...
\bibitem{Yau1982}
S.-T.~Yau.
\newblock Problem section. In {\em Seminar on Differential Geometry}. 
\newblock {\em Ann. of Math Stud. 102 (1982)}, 669--706.
