...
\keywords{Riemann surface; complex curve; Oka-1 manifold; Oka manifold}


\begin{abstract}
We introduce a new class of complex manifolds: Oka-1 manifolds. 
They are characterized by the property that holomorphic maps from any open Riemann 
surface satisfy the Runge approximation and the Weierstrass interpolation condition.
We prove that every complex manifold which is dominable by tubes of complex lines
is an Oka-1 manifold. In particular, a manifold dominable by $\C^n$
at most points is an Oka-1 manifold. This provides many examples of Oka-1 manifolds
among compact algebraic surfaces, including all Kummer and all elliptic K3 surfaces.
We also show that every compact rationally connected manifold is an Oka-1 manifold.  
The class of Oka-1 manifolds is invariant under Oka maps inducing 
...
In this paper, we introduce and investigate a new class of complex manifolds which admit plenty of holomorphic curves parameterized by any open Riemann surface, with natural approximation and interpolation conditions. Here is the definition of this class.
%In this paper, we introduce a new class of complex manifolds characterized by the property that they admit holomorphic maps from any open Riemann surface in any given homotopy class, satisfying Runge approximation and Weierstrass interpolation conditions. Here is the definition. 

%
%  OKA-1
%
\begin{definition}\label{def:Oka1} 
A connected complex manifold $X$ with a complete distance function $\dist_X$ is an 
...
\item $F$ agrees with $f$ to order $k_i$ at the point $a_i$ for every $i$.
\end{enumerate}
If (2) holds with $k_i=1$ then $X$ is said to satisfy the Oka-1 property 
with approximation and interpolation. If only (1) holds then $X$ 
satisfies the Oka-1 property with approximation. 
A not necessarily connected manifold $X$ is Oka-1 if every component of $X$ is such.
\end{definition}
...
component. The conditions in Definition \ref{def:Oka1} 
imply that a homotopy from $f$ to $F$ can be chosen to consist of 
maps $R\to X$ which are holomorphic on a neighbourhood of $K\cup \bigcup_i\{a_i\}$
and agree with $f$ to order $k_i$ at $a_i$ for every $i$.

In this paper, we investigate the class of Oka-1 manifolds by 
combining techniques from complex analysis, complex and algebraic geometry,
and nonlinear analysis. Here are some immediate observations. % about this class.
...
Hence, the class of compact K\"ahler or projective Oka-1 manifolds 
...
%
% AA: I added the reference to Campana
%
\cite{Campana2004AIF,Campana2004AIF-2}; see the discussion 
by Campana and Winkelmann in \cite{CampanaWinkelmann2020}. 

The term Oka-1 manifold is inspired by the notion of an Oka manifold;
see \cite[Chapter 5]{Forstneric2017E} and \cite{Forstneric2023Indag,ForstnericLarusson2011}. A complex manifold $X$ is an Oka manifold 
...
Stein manifold and, more generally, from any reduced Stein space $S$.
...
%However, we are lacking examples among compact complex manifolds. 

Note that the axiom of Oka-1 manifolds does not include  
...
uses the Oka principle for maps from Stein manifolds of arbitrary dimension,
and it does not apply to Oka-1 manifolds. One can of course introduce 
and study the class of manifolds satisfying the parameteric Oka-1 property,
but most of the techniques developed here do not apply to this case.
...
Dominability by tubes of lines is a considerably weaker condition than any of the 
sufficient conditions that are used in the theory of Oka manifolds,  
and is the first known condition implying an Oka-type property of a complex
manifold which is purely local. An immediate 
corollary is that for a Riemann surface $X$ the conditions 
\[
	\text{Oka} \Longrightarrow \text{dominable} \Longrightarrow \text{Oka-1} 
	 \Longrightarrow \text{nonhyperbolic}
\]
are pairwise equivalent; see Corollary \ref{cor:RS}. It also follows that a connected 
algebraic manifold which is algebraically dominable by $\C^n$ is Oka-1. 
...
In particular, Proposition \ref{prop:weakOka1} shows that, in order to establish 
...
by a holomorphic map $L=K\cup D\to X$, where $D\subset R$ is a compact disc attached 
...
In Section \ref{sec:functorial} we study the behaviour of the Oka-1 property 
under holomorphic maps. 
The main result of the section is Theorem \ref{th:updown}, which shows that 
the class of Oka-1 manifolds is invariant under Oka maps inducing a surjective 
homomorphism of fundamental groups. In particular, if $h:X\to Y$ is a holomorphic 
fibre bundle projection with a connected Oka fibre, then $X$ is an Oka-1 manifold 
...
We also show by examples that Oka-1 manifolds are in general not open or 
closed in smooth families of manifolds.
...
all Kummer and elliptic K3 surfaces, and many elliptic surfaces of Kodaira dimension $1$.
It turns out that for many classes of compact complex surfaces,  
the conditions of being Oka, Oka-1, dominable by $\C^2$, and having a Zariski dense 
entire line $\C\to X$ are pairwise equivalent.
However, we expect that this is a low dimensional phenomenon,  
and that the gaps between these conditions increase with the dimension
of the manifold. 

In Section \ref{sec:RC} we prove that every compact rationally connected manifold
$X$ is Oka-1; see Theorem \ref{th:RC}. We were motivated by the paper 
\cite{CampanaWinkelmann2020} by Campana and Winkelmann, who 
constructed holomorphic lines $\C\to X$ through any given sequence of points; 
in particular, such lines can be dense in $X$.
We obtain Theorem \ref{th:RC} from a Runge approximation theorem 
for compact complex curves in certain almost complex manifolds, 
due to Gournay \cite{Gournay2012}, combined with Proposition \ref{prop:weakOka1}
which adds interpolation to his statement.  

Finally, we prove in Section \ref{sec:dense} that for every connected complex
manifold $X$ and open bordered Riemann surface $R$ there exist
holomorphic curves $R\to X$ passing through any given sequence
of points in $X$. The existence of proper holomorphic
maps from bordered Riemann surfaces to certain noncompact complex manifolds was studied in \cite{DrinovecForstneric2007DMJ,Forstneric2023Roumaine}.
...
%
%
\section{A manifold densely dominable by tubes of lines is Oka-1}\label{sec:tubes}

In this section, we introduce a geometric condition implying the Oka-1 property. 
It is based on the notions of a tree and a tube of complex lines, and of dominating
maps from such tubes. It holds on any complex 
manifold $X$ which is dominable by $\C^{\dim X}$ at most points. 
...
\item 
$X$ is {\em densely dominable by $\Acal$} if there is a closed subset $E$ of $X$ with $\Hscr^{2n-1}(E)=0$ such that $X$ is dominable by $\Acal$ at every point $x\in X\setminus E$. (Here, $\Hscr^{k}=\Hscr^{k}_{X,g}$ stands for the $k$-dimensional Hausdorff measure with respect to a Riemannian metric $g$ on $X$. See Federer \cite{Federer1969} or Morgan \cite{Morgan2009} for this notion. The choice of $g$ is irrelevant for our definition.)
\item
$X$ is {\em strongly dominable by $\Acal$} if it is dominable by $\Acal$ at every point $x\in X$.
\end{enumerate}
\end{definition}

If $\Acal'\subset\Acal$ is a subcollection and $X$ is dominable (at a point, densely, or strongly) by $\Acal'$, then $X$ is dominable by $\Acal$ (at the point, densely, or strongly) as well. The converse is not true in general.
In the sequel, by dominability by a single domain $W\subset\C^N$, $N\ge n$, we shall mean dominability in the sense of Definition \ref{def:dominable} by the collection $\Acal=\{W\}$ whose only element is $W$. Likewise, by dominability of a complex $n$-dimensional manifold we shall mean dominability by $\C^n$ unless stated otherwise; 
cf.\ \cite[Definition 7.1.7]{Forstneric2017E}. For an algebraic manifold we have the corresponding notions of {\em algebraic dominability}.
...
%; hence the {\em length} $k$ of $\Lambda$ satisfies $k\le n$. 
The tree $\Lambda$ is {\em spanning} if $k=n$; equivalently, 
the direction vectors $v_1,\ldots,v_n$ are a basis of $\C^n$. 
%
...
of $\Lambda$. The tube $T$ is {\em spanning} if the tree $\Lambda$ is spanning.
\end{definition}
...
We have that $\Acal^n=\bigcup_{N\ge n}\Acal_N$, where $\Acal_N$ denotes the collection of all spanning tubes of lines $T\subset\C^N$. Therefore, if $X$ is dominable (at a point, densely, or strongly) by $\Acal_N$ for some $N\ge n$, then $X$ is dominable by tubes of lines (at the point, densely, or strongly) as well. In particular, each type of dominability by tubes of lines is implied by the corresponding dominability by $\C^N$ or, more generally, by any fixed spanning tube of lines in $\C^N$, $N\ge n$; note that $\C^N$ itself is a spanning tube of lines in $\C^N$.
...
is a considerably weaker condition on a complex manifold than any of the 
sufficient conditions used in the theory of Oka manifolds.  
In fact, it is the first known condition implying an Oka-type property of a complex
manifold $X$ which is purely local, in the sense that dominability by a tube of lines
...
%In that paper, a domain having the Oka-1 property was called {\em flexible for complex curves}. The main topic of \cite{DrinovecForstneric2023JMAA} is the analogous flexibility property for domains in $\R^n$ with respect to minimal surfaces.

Theorem \ref{th:main1} is proved in Section \ref{sec:proof} in a slightly more general
...
$z\in \Omega$ is contained in a spanning tube of lines $T \subset \Omega$,
then $\Omega$ is an Oka-1 manifold. In particular, every spanning tube
of lines in $\C^n$ is an Oka-1 manifold.
\end{corollary}

A domain $\Omega$ in a complex manifold $X$ is called {\em Zariski open}
...
%Let us record several observations which follow from Theorem \ref{th:main1}.
...
which is densely dominable (by tubes of lines, or by $\C^n$) and
...
%
\begin{proof}
Part (a) follows from Theorem \ref{th:main1}, and 
(b) follows since the complement of a Zariski open domain in $X$
is a closed complex subvariety $E$ of $X$ with $\Hscr^{2n-1}(E)=0$. 
To prove part (c), assume that $E$ is a closed subset 
of $Y$ with $\Hscr^{2n-1}(E)=0$. Then, $E'=h(E\cup \br\, h)$ 
is a closed subset of $\Omega$ with $\Hscr^{2n-1}(E')=0$. 
(We denote by $\br\, h$ the branch locus of a holomorphic map $h$.)
Since $A=X\setminus \Omega$ is a proper complex subvariety
of $X$, $A\cup E'$ is a closed subset of $X$ with 
$\Hscr^{2n-1}(A\cup E')=0$. Note that $X$ is dominable at every 
...
if $X$ is algebraic and $F:\C^n\to X$ is a dominating algebraic map
...
However, we shall prove that for an Oka map $h: Y\to X$ which induces
a surjective homomorphism of fundamental groups, $X$ is Oka-1 
if and only if $Y$ is Oka-1; see Theorem \ref{th:updown}. 
Recall that a connected Oka manifold $X$ is strongly dominable 
by a single holomorphic map $F:\C^n\to X$, in the sense that 
$F(\C^n\setminus \br\, F)=X$ (see \cite{Forstneric2017Indam}). 
It is not known whether every strongly dominable manifold is 
an Oka manifold; see \cite[Section 7.1]{Forstneric2017E}.
...
$\mathbb C^2$ was made by Buzzard and Lu \cite{BuzzardLu2000}. Although  
we are unable to draw the conclusion of Corollary \ref{cor:dominable} (e) under
holomorphic dominability (indeed, dominating holomorphic maps
can have rather small range as shown by Fatou--Bieberbach domains), 
inspection of the proofs in \cite{BuzzardLu2000} shows that the complex surfaces 
for which dominability is established in that paper are actually densely dominable by $\C^n$, 
...
trees of lines is clearly equivalent to dominability by $\C$, 
which holds if and only if $X$ is one of the Riemann surfaces $\CP^1,\C,\C^*=\C\setminus\{0\}$, or a torus.
These are precisely the Riemann surfaces which are Oka manifolds;
see \cite[Corollary 5.6.4]{Forstneric2017E}. 
%Conversely, every Oka-1 Riemann surface is clearly dominable by $\C$. 
...
of $X$ with $\Hscr^{2n-2}(E)=0$, then a generic holomorphic map $f:M\to X$ 
...
the condition $\Hscr^{2n-2}(E)=0$ implies $\Hscr^{2N} (F^{-1}(E))=0$ by Fubini's theorem, 
...
of removability of a point is open, and discrete sets in $\C^n$ for $n>1$
...
manifold $X$ is an immersion if $\dim X>1$, and is an injective immersion if $\dim X>2$; 
see \cite[Section 8.8]{Forstneric2017E}. Using this inductively 
...
(and injective immersions if $n>2$) provided that the interpolation conditions allow it.
...
that every connected complex manifold $X$ admits a holomorphic disc $\Delta \to X$ hitting any given sequence in $X$, so there are
...
where we focus on complex surfaces, and Section \ref{sec:RC}
where we show that every compact rationally connected manifold
is an Oka-1 manifold. Note however that a compact complex manifold 
...
We claim that the extended tube of rational curves given by 
\[
...
In Section \ref{sec:noncritical} we use Lemma \ref{lem:bump} and methods from Oka theory 
to prove Lemma \ref{lem:noncritical}, which provides the noncritical case in 
...
The following result, which we state here for reader's convenience,
...
to any given finite order at the points $a_i\in \mathring K=K\setminus bK$.
\end{theorem}


\begin{comment}
\begin{proof}
If $X=\C$, we can correct the jets of a function at the points $a_i$ by adding 
a holomorphic function on a neighbourhood of $K$ having suitable jets at these points,
and this is well known. 

A Mergelyan approximation theorem for manifold-valued maps but 
without interpolation is given by \cite[Corollaries 6 and 7, p.\ 177]{FornaessForstnericWold2020}. 
The proof is obtained by a reduction to the case of functions;
see \cite[Lemma 3 and Theorem 32]{FornaessForstnericWold2020}.
The main point of the reduction is that the graph of a map $f:K\to X$ of class
$\Ascr(K)$ has a basis of Stein neighbourhoods in $R\times X$
(see  Poletsky \cite{Poletsky2013}). Embedding a Stein neighbourhood of the graph in a 
Euclidean space and using a holomorphic retraction from a neighbourhood of the embedded 
submanifold onto the submanifold, the correction method for functions applies also in the
general case.
\end{proof}
\end{comment}
...
%Clearly, a tree is regular if and only if its branches can be ordered so that for each $i\ge 2$, $\Lambda_i$ intersects the subtree $\Lambda^{i-1}$ in a single smooth point of $\Lambda^{i-1}$. 
...
simple examples show that a spanning tree with more than $n$ branches need not contain 
...
%
%
\section{Extending a holomorphic map to a bump taking values in a tube}\label{sec:bump} 

The following lemma will be used in the proof of Theorem \ref{th:main1}. 
The notions of a tree and a tube of complex lines were introduced in
...
$T\subset \C^n$ be a tube around a spanning tree of lines such that $f(\alpha)\subset T$.
...
we shall also interpolate the given map at the given finite set of
...
Pick a closed disc $\Delta_0\subset D$ such that 
$\Delta_0 \cap \alpha =\varnothing$ and $\Delta_0 \cap bD$ is an arc contained 
in $\beta$. (This disc will not be used in the special case of a comb, but it will come
handy in the induction given in the second part of the proof.)
...
$\overline{D\setminus \Delta_1}$ shrinks to $\alpha\cup \Delta_0$ where 
$|\tilde f_1|<r$. Note that $K\cap \Delta_1=\varnothing$, and hence the 
set $K\cup \Delta_1$ is Runge in an open neighbourhood of $L=K\cup D$. 

If $n=2$, we proceed to the final argument explaining how to choose the last component $\tilde f_n$. 
Assume now that $n>2$. Let $\wt\Delta_1$ denote the union of $\Delta_1$ and the component of 
$D\setminus \Delta_1$ containing $\Delta_0$, so 
$\wt \Delta_1\subset D$ is a closed disc disjoint from $\alpha$.
...
the affine lines in $\Lambda^j$. By renumbering the coordinates we may 
...
By the inductive hypothesis, we can approximate the component functions $f_i$ for $i=1,\ldots,n_1$ 
by functions $\tilde f_i\in\Oscr(K\cup D)$ satisfying $|\tilde f_i|<r$ on $\alpha\cup\Delta_0$ 
such that $\tilde f^1=(\tilde f_1,\ldots,\tilde f_{n_1}): K\cup D\to \C^{n_1}$
maps $D$ into the polydisc tube $T^1\subset \C^{n_1}$ of radius $r$ around the tree $\Lambda^1$. 
Choose a disc $\Delta_1\subset D$ satisfying conditions (i$_{1}$)--(ii$_{1}$)
above, and with condition (iii$_{1}$) replaced by
...
$i=n_1+1,\ldots,n_2$.	
\end{enumerate}
Clearly this process continues inductively. In the last step, we choose the last
component $\tilde f_n$ as in \eqref{eq:cond2} with $n-1$ replaced by $l$.
It can be verified just as before that the resulting holomorphic map 
$\tilde f=(\tilde f^1,\ldots,\tilde f^l,\tilde f_n):L \to \C^n$ send $D$ into $T$. 
...
and an affine change of coordinates $U_p:\C^n\to \C^n$ such that
$U_p(f(p))=0$, $U_p(T_p)$ is a tube of radius $r_p>0$ around a spanning tree
of the normal form \eqref{eq:normal}, and $U_p(f(\alpha_p)) \subset r_p\Delta^n$. 
In other words, the assumptions (a) and (b)
hold for the arc $\alpha_p$, the map $U_p\circ f$,
the tube $U_p(T_p)$, and the number $r_p$. 

This observation allows us to subdivide the arc $\alpha$ into a finite union 
...
for each $p\in\gamma_j$. Note that the compact set 
$
	S=K\cup\bigcup_{j=1}^{s-1}\gamma_j
$ 
is admissible for Mergelyan approximation. Hence, by Mergelyan theorem 
we can approximate $f$ as closely as desired on $S$ by a holomorphic map defined 
on an open neighbourhood $V$ of $S$. Choose a small compact neighbourhood 
...
and interpolation at the given finitely many points in $\mathring K$)
...
Instead of a general statement, we shall provide the details for the precise result 
that will be used at the appropriate place in the proof of Lemma \ref{lem:noncritical}.
\end{remark}
...
%
%
\section{Extending a holomorphic map to an annulus}\label{sec:noncritical} 

By using Lemma \ref{lem:bump} and methods from Oka theory,
we now prove the following lemma, which is the main ingredient in the proof of 
Theorems \ref{th:main1} and \ref{th:main2}. 
%We point out that dominability by trees of lines is only used in this lemma.

%
% Lemma: noncritical
%
\begin{lemma}\label{lem:noncritical}
Assume that $X$ is a complex manifold with a complete distance function $\dist_X$, 
and $\Omega\subset X$ is a domain such that $X$ is dominable by tubes of lines at 
every point $x\in\Omega$ (see Definitions \ref{def:dominable} and \ref{def:dominable-tubes}).
...
% AA: I added "strongly"
%
strongly dominable by tubes of lines then $\tilde f$ can be chosen such that it also satisfies 
\begin{itemize}
...
%
% AA: I edited the following paragraph to mention that dominability is with respect to tubes of lines
%
Note that the condition that $X$ be dominable by tubes of lines at every point 
of the domain $\Omega\subset X$ is weaker than the condition that $\Omega$
...
$T\subset\C^N$ $(N\ge \dim X)$ in Definition
\ref{def:dominable} (a), which is dominating at a given point $x\in\Omega$,
may have range in $X$, while in the latter case it must lie in $\Omega$.

\begin{proof}
The conditions imply that the compact set 
$D=L\setminus \mathring K$ is a union of finitely many pairwise disjoint annuli.
It clearly suffices to consider the case when $D$ is a single annulus.
...
The assumptions on the domain $\Omega\subset X$ imply that 
$X$ is dominable by tubes of lines at every point in the compact set $f(bK)$.
Hence, we can subdivide the closed Jordan curve $bK$ into a finite union of 
compact subarcs $\{\alpha_i: i\in \Z_l\}$ $(l\ge 2)$ lying back-to-back and satisfying 
...
for some $n_i \ge \dim X$
%
% AA: I added the following clarification
%
(i.e., $T_i\in \Acal_{n_i}\subset\Acal^{\dim X}$ in the notation of Definition 
\ref{def:dominable-tubes}), a holomorphic map $\sigma_i:T_i\to X$, a neighbourhood 
$U_i\subset \Omega$ of $f(\alpha_i)$, and an open subset $\omega_i\subset T_i$ such that 
...
they intersect $bD=bK\cup bL$ only at the respective endpoints $p_i$ and $q_i$,
...
\begin{equation} \label{eq:fwtK}
	f(\wt K\setminus \mathring K) \subset \Omega.
\end{equation}
...
$D_i\setminus \mathring C_i$ is a closed disc attached to $C_i$ as in the 
hypotheses of Lemma \ref{lem:bump}; this will be used later in the proof. See Figure \ref{fig:51}.
...
$\tilde g_i:(C_i\cup D_i)\times \overline \B^N \to T_i\subset \C^{n_i}$. Define the map $G_i$ by
\begin{equation}\label{eq:Gi}
	G_i=\sigma_i\circ \tilde g_i:(C_i\cup D_i)\times \overline \B^N \to X
	\ \ \text{for $i\in\Z_l$}.
\end{equation} 
By the construction, $G_i$ approximates $F$ uniformly
...
the approximations are close enough, we can glue $F$ and the sprays $G_i$ $(i\in\Z_l)$ into 
a holomorphic spray $\wt F:L\times r\B^N\to X$ which approximates $F$ on
$\wt K\times r\B^N$,
%  (depending on the rate of approximation of $F$ by $G_i$ on $C_i \times \overline \B^N$ for each $i\in\Z_l$), 
...
It remains to justify the last claim (iii). Assume that the 
open subset $\Omega\subset X$ is strongly dominable by trees
of lines and $f(bK)\subset \Omega$. There is a subdivision of $bK$
into arcs $\alpha_i$ as in (A2), open neighbourhoods $U_i\subset \Omega$
of their images $f(\alpha_i)\subset \Omega$, and holomorphic maps 
$\sigma_i:T_i\to \Omega$ from 
...
it follows that each spray $G_i$ also has range in $\Omega$. An inspection of the gluing method 
...
In this section we prove the following result, which clearly includes Theorem \ref{th:main1} 
...
Let $X$ be a complex manifold with a complete distance function 
$\dist_X$, and let $\Omega \subset X$ be a connected domain which is  
densely dominable by tubes of lines in the sense of Definition \ref{def:dominable-tubes}. 
...
$K\cup \bigcup_i\{a_i\}$ and satisfies
%
% AA: I strengthen the next assumption. I think it is required for the homotopic condition
%
%$f(bK)\subset \Omega$ and $f(a_i)\in \Omega$ if $a_i\notin K$.
$f(R\setminus\mathring K)\subset\Omega$.
...
\begin{proof}
By the argument in Remark \ref{rem:transversality}, we may assume without loss of generality 
that  $\Omega$ is strongly dominable by tubes of lines in the sense of Definition 
\ref{def:dominable-tubes}.
Call $A=\{a_i\}_{i\in\N}$, let $f:R\to X$ be as in the statement, and let $U\subset R$ be an 
open neighbourhood of $K\cup A$ such that $f$ is holomorphic on $U$.
...
\begin{enumerate}[\rm (A)] 
\item $K\subset K_1\subset U$ and $A\cap K_1\setminus K=\varnothing$.
\item Every $c_j$ for $j=1,2,\ldots$ is a regular value of $\rho$ 
and the level set $\{\rho=c_j\}=bK_j$ is disjoint from $A$.
\item For every $j\in\N$ the set $D_j=\{c_j\le \rho\le c_{j+1}\}=K_{j+1}\setminus \mathring K_j$
contains at most one critical point of $\rho$ or at most one point of $A$, but not both.
\end{enumerate}
%
The construction of such a function $\rho$ and sequence $c_j$ is standard, using the fact that
Morse critical points are isolated. Condition (A) is achieved by choosing $\rho$ and $c_1$ 
...
\]
Set $K_0=K$ and $\epsilon_1=\epsilon/2$, where $K$ and $\epsilon>0$ are 
given in the statement. We shall inductively construct a sequence of 
holomorphic maps $f_j:K_j\to X$ $(j\in \N)$, with $f_1=f|_{K_1}$,
and a sequence of numbers $\epsilon_j>0$  
satisfying the following conditions for each $j=1,2,\ldots$: 
\begin{enumerate}[\rm (a$_{j}$)] 
\item $d_{K_{j-1}}(f_{j},f_{j-1})<\epsilon_{j-1}$. 
\item $f_{j}(D_{j-1})\subset \Omega$. (Here, $D_{j-1}=K_{j}\setminus \mathring K_{j-1}$ 
is given by (C).) 
\item $f_{j}$ agrees with the given map $f$ to order $k_i$ at every point $a_i\in A\cap K_{j}$.
\item $f_{j}$ is homotopic to $f|_{K_{j}}:K_{j}\to X$.
\item $\epsilon_{j}< \frac12\min\{\epsilon_{j-1},\dist_X(f_{j}(D_{j-1}),X\setminus \Omega)\}$.
\end{enumerate}
The beginning of the induction is given by the map $f_1$ and the number $\epsilon_1$,
and the above conditions are void. 
...
satisfying conditions (a$_{j+1}$)--(c$_{j+1}$) is given by Lemma \ref{lem:noncritical}. 
Moreover, since $K_j$ is a strong deformation retract of $K_{j+1}$, we have that $f_{j+1}|_{K_j}$ 
is homotopic to $f_j$ provided the approximation in (a$_{j+1}$) is close enough, and hence to 
$f|_{K_j}$ by (d$_j$). Using again that $K_j$ is a strong deformation retract of $K_{j+1}$, 
this shows (d$_{j+1}$). We then choose a number $\epsilon_{j+1}>0$ satisfying (e$_{j+1}$), 
...
imply that such a point $p$ is unique and is contained in the interior 
$\mathring D_j=\mathring K_{j+1}\setminus K_j$; furthermore, $D_j\cap A=\varnothing$.
In this case there is a compact smoothly embedded arc 
...
only at these endpoints and the intersections are transverse, and the set $K_j\cup\lambda$
is a strong deformation retract of $K_{j+1}$. (A detailed discussion of the possible 
changes of topology depending on the Morse index of $\rho$ at $p$ can be found in 
\cite[Section 1.4]{AlarconForstnericLopez2021}.) Since the set $\Omega\subset X$ 
is open and connected, we can extend $f_j$ continuously across the arc $\lambda$ such that 
$f_j(\lambda)\subset\Omega$ 
and $f_j:K_j\cup\lambda\to X$ is homotopic to $f|_{K_j\cup\lambda}$; take into account 
{\rm (d$_j$)} and that $f(R\setminus\mathring K_j)\subset\Omega$ by assumption. 
We then apply Mergelyan theorem (see Theorem \ref{th:Mergelyan})
to approximate $f_j$ by a holomorphic map $\tilde f_j:V\to X$ on a neighbourhood 
$V\subset K_{j+1}$ of $K_j\cup\lambda$ such that 
$\tilde f_j(bK_j\cup \lambda \subset\Omega$,
$\tilde f_j$ agrees with $f_j$ to the given order $k_i$ at all points $a_i\in A\cap K_j$, and 
$\tilde f_j$ is homotopic to $f|_V$; the last condition holds whenever that the approximation 
is close enough. Then, there is a compact smoothly bounded neighbourhood $K'\subset V$ 
of $K_j\cup\lambda$ such that $\tilde f_j(K'\setminus \mathring K_j)\subset\Omega$ and 
$K_{j+1}\setminus K'$ is a finite union of annuli.
Applying Lemma \ref{lem:noncritical} as in Case 1 to the pair of sets 
$K'\subset K_{j+1}$ and the map $\tilde f_j$ gives a holomorphic 
map $f_{j+1}:K_{j+1}\to X$ satisfying conditions (a$_{j+1}$)--(d$_{j+1}$). 
...
Recall that $U\subset R$ is the open set on which the initial  
...
connecting a point $p\in bK_{j}$ with a point $q\in b\Delta$ 
...
Since $\Omega$ is connected and we have $f_j(bK_j) \subset \Omega$ and
$f(a_i)\in\Omega$ by the assumption, we can extend 
$f_j$ from $K_j$ to a continuous map $\tilde f_j:\wt K_j\to X$ which equals $f$ on $\Delta$
and $\tilde f_j$ is homotopic to $f|_{\wt K_j}$. 
We can now complete the proof as in Case 2, 
first approximating the extended map $\tilde f_j:\wt K_j\to X$ by a holomorphic
map on a neighbourhood of $\wt K_j$ which agrees with $\tilde f_j$ to 
the given order $k_m$ at every point $a_m\in \wt K_j$ (in particular, at $a_i$)
and is homotopic to $f$,
and then applying Lemma \ref{lem:noncritical} as in Case 1 to find the next holomorphic map 
$f_{j+1}:K_{j+1}\to X$. To finish, we choose any $\epsilon_{j+1}>0$ satisfying (e$_{j+1}$).
...
Oka-1 property with approximation and interpolation at the finitely many points
...
A survey of these issues for the class of Oka manifolds can be found in 
\cite[Chapter 7]{Forstneric2017E} and in 
...
is an Oka-1 manifold. The proof of the following observation is left to the reader.
...
%Here is a consequence of Proposition \ref{prop:weakOka1}.
...
\begin{enumerate}[\rm (a)]
\item If $Y$ is an Oka-1 manifold, then $X$ is also an Oka-1 manifold. 
...
$h\circ f:K\to Y$ can be approximated uniformly on $K$ by holomorphic maps $g:L\to Y$, 
...
If the approximation is close enough then $g$ is homotopic to $h\circ f$ on $K$,
...
let $K\subset\C$ be a Runge compact 
set,  hence simply connected, and let $f:K\to Y$ be holomorphic. By what has just been 
said, $f$ lifts to a holomorphic map $g:K\to X$, and by the assumption we can approximate 
$g$ by a holomorphic map $\tilde g: \C\to X$ which agrees with $g$ at the points
$a_1,\ldots, a_m\in K$. This gives a holomorphic map $\tilde f=h\circ \tilde g:\C\to Y$ 
approximating $f$ on $K$ which agrees with $f$ at the points $a_1,\ldots, a_m$. 
Applying this argument inductively on an increasing sequence of closed discs 
in $\C$ gives (b).
\end{proof}

\begin{problem}\label{prob:coverings}
If $h:X\to Y$ is a holomorphic covering projection and $X$ is an Oka-1 manifold, 
is $Y$ also an Oka-1 manifold?
\end{problem}
...
covering space of $X$ is Liouville. As remarked in the introduction, it follows from the 
definition that an Oka-1 manifold is Liouville. Proposition \ref{prop:ascent} gives the 
...
We now present a considerably more general result which gives
the invariance of Oka-1 property under certain {\em Oka maps}.
We recall this notion, referring to \cite[Section 7.4]{Forstneric2017E} and
\cite[Section 3.6]{Forstneric2023Indag} for more complete presentations.
A holomorphic map $h:X\to Y$ of complex manifolds 
% (or, more generally, of reduced complex spaces)
...
any continuous lifting $f_0:S\to X$ (if such a lifting exists) is homotopic through 
...
%   OKA MAPS
%
A holomorphic map $h:X\to Y$ of connected complex manifolds
is said to be an {\em Oka map} if it enjoys POPAI and is a Serre fibration
(see \cite{Larusson2004,Forstneric2010CR} and \cite[Definition 7.4.7]{Forstneric2017E}).
Such $h$ is a surjective submersion and the fibres of $h$ are Oka  
manifolds \cite[Proposition 3.14]{Forstneric2023Indag}. 
In particular, the constant map $X\to point$ is an Oka map if and only if $X$ 
is an Oka manifold. 
More generally, if $h:X\to Y$ is an Oka map then $X$ is an Oka manifold if and only if 
$Y$ is an Oka manifold \cite[Theorem 3.15]{Forstneric2023Indag}. 
...
\begin{proof} 
Fix compact sets $K\subset L=K\cup D$ in an open Riemann surface $R$ 
as in Proposition \ref{prop:weakOka1}, where $D$ is a disc attached to $K$ along 
a boundary arc $\alpha=bD\cap bK$. 

%\smallskip
\noindent {\em Proof of (a).} Let $f_0:K\to X$ be a holomorphic map.
Since $Y$ is an Oka-1 manifold, the holomorphic map $h\circ f_0:K\to Y$ can be approximated 
uniformly on $K$ by holomorphic maps $g:L\to Y$ with interpolation to a given order
in given finitely many points $a_1,\ldots, a_m\in K$. Assuming that the approximation 
...
with $f_0$ to a given order at the points  $a_1,\ldots, a_m\in K$, 
and it satisfies $h\circ f_1=g$ on $K$. (The proof uses holomorphic 
retractions provided by \cite[Lemma 3.16]{Forstneric2023Indag}.)
...
$K$ by holomorphic maps $f:L \to X$ satisfying $h\circ f=g$ and such that 
$f$ agrees with $f_1$ to a given finite order at  the points $a_1,\ldots, a_m\in K$. 
Hence, $X$ is an Oka-1 manifold by Proposition \ref{prop:weakOka1}.  

%\smallskip
\noindent {\em Proof of (b).} 
%Since $K$ is a domain with piecewise smooth boundary in an open Riemann surface, it 
Note that $K$ has the homotopy type of bouquets of circles,  
one for each connected component of $K$. The assumption that the homomorphism 
$h_*:\pi_1(X,x)\to \pi_1(Y,h(x))$ of fundamental groups is surjective for some 
(and hence for all) points $x\in X$ therefore implies that 
any holomorphic map $g:K\to Y$ lifts to a continuous map $f_0:K\to X$ such that 
$h\circ f_0=g$. Since $h$ is assumed to be an Oka map, we can homotopically 
deform $f_0$ to a holomorphic map $f_1:K\to X$ satisfying $h\circ f_1=g$. 
Since $X$ is an Oka-1 manifold, we can approximate $f_1$ uniformly on $K$ by a 
holomorphic map $\tilde f:L\to X$ which agrees with $f_1$ to a given order 
at the points  $a_1,\ldots, a_m\in K$. The holomorphic map $\tilde g=h\circ \tilde f:L\to Y$ 
then approximates $g$ uniformly on $K$ and it agrees with $g$ to a given order 
at the points $a_1,\ldots, a_m\in K$. Hence, $Y$ is an Oka-1 manifold 
...
%   OKA-1 MAP
%
%\begin{remark}[{\bf Oka-1 maps}] \label{rem:Oka1map}
In the proof of Theorem \ref{th:updown} we have only used the following properties of
a holomorphic map $h:X\to Y$. 

\begin{definition}\label{def:Okamap}
A holomorphic map $X\to Y$ of complex manifolds is an {\em Oka-1 map} if
\begin{enumerate}[\rm (i)] 
...
as desired on $K$ and agrees with $f_0$ to any given finite order at finitely
many given points of $K$. (See the diagram \eqref{eq:lifting}.)
\end{enumerate}
\end{definition}
...
at least for maps with noncompact fibres. It follows from the definition of an Oka-1 map
$h:X\to Y$ that for every holomorphic disc $g:\Delta\to Y$ and point $x\in h^{-1}(g(0))$
...
the differential $dh_x:T_x X\to T_{g(0)}Y$, so an Oka-1 map $h$ is a 
submersion. Furthermore, since any pair of points in $Y$ lie in the image 
of a holomorphic disc, we see that $h$ is surjective. Hence, the proof
of Theorem \ref{th:updown} gives the following corollary.

\begin{corollary}
Parts (a) and (b) in Theorem \ref{th:updown} hold if $h$ is an Oka-1 map.
\end{corollary}
...
The proof of the analogous result for Oka manifolds and maps does not apply.
%\end{remark}


One might wonder why is the conclusion in Theorem \ref{th:updown} (b) weaker for 
Oka-1 manifolds than for Oka manifolds, where the condition on the homotopy groups 
is unnecessary. The reason is that Oka manifolds can be characterized by the 
convex approximation property (CAP), which refers to holomorphic maps from bounded 
convex sets in Euclidean spaces $\C^{N}$ to the given manifold. 
We now introduce a local approximation condition on a  
complex manifold which implies that the manifold is Oka-1. This is essentially a  
localization of CAP, applied to dominating holomorphic sprays on discs. 
...
and $\overline {D'\setminus D}$ is a disc attached to $D$ along a boundary arc.
Given a holomorphic map $f:D\to X$ to a complex manifold $X$, 
a holomorphic map $F:D \times \B^N\to X$ is called a (local) dominating holomorphic 
spray with the core $f$ if $F(\cdotp,0)=f$ and the partial derivative 
$\frac{\di}{\di t}\big|_{t=0}F(z,t):\C^N\to T_{f(z)}X$ is surjective for all points $z\in D$. 
(Such sprays were used in the proof of Lemma \ref{lem:noncritical}, see \eqref{eq:F}.)  
...
LSAP, if every point $x\in X$ has an open neighbourhood $V\subset X$ satisfying
...
\item A complex manifold with LSAP is an Oka-1 manifold.
\item A complex manifold which is densely dominable by manifolds having LSAP
is Oka-1.
\item If $h:X\to Y$ is a holomorphic fibre bundle whose fibre $E$ has LSAP,
then $X$ is an Oka-1 manifold  if and only if $Y$ is an Oka-1 manifold.
\end{enumerate}
\end{proposition}

\begin{proof}
Part (a) follows from Lemma \ref{lem:bump} and Remark \ref{rem:bump}, 
parts (b) and (c) follow from the proof of Theorem \ref{th:main2} (with  
LSAP replacing the use of Lemma \ref{lem:noncritical}), 
and part (d) follows from the proof of Theorem \ref{th:updown}.
(However, we are unable to make the same conclusion under the weaker
assumption that $h:X\to Y$ is an Oka-1 map.) 
Note that in (c) we may use different manifolds with LSAP to dominate $X$ 
at different points. 
\end{proof}
...
open in families of noncompact complex manifolds. For example, one can consider the family
$h:X=\{(z,w)\in\C^2 : |zw|<1\}\to \C$ whose fibre over $z\in \C^*$ is the disc, while the fibre
over $z=0$ is $\C$. On the other hand, we are not aware of an example of an isolated
Oka or Oka-1 fibre in a smooth family of compact complex manifolds. 
...
and in \cite[Section 7.3]{Forstneric2017E}. Comparing both lists, 
...
are pairwise equivalent. The main exceptions are the K3 surfaces
...
$K_X^{\otimes m}$ only admits the trivial (zero) section for any $m$. 
...
not dominable by $\C^{\dim X}$. 
Conjecturally, no such manifold is Oka-1 either, since it is believed that any holomorphic
line $\C\to X$ in a manifold of general type is contained in a proper complex subvariety of $X$. 
...
A {\em fibration} $f:X\to C$ of a surface $X$ onto a curve $C$ 
...
is an elliptic curve (a torus). Such a surface can have any Kodaira dimension 
$\kappa\in \{-\infty,0,1\}$. A compact complex surface $X$ is called an {\em elliptic surface} 
...
$X\to C$ with fibre $\CP^1$ over a compact curve $C$ (see \cite[p.\ 189]{BarthHulek2004}). 
...
or a torus (see \cite[Corollary 5.6.4]{Forstneric2017E}). 
By Theorem \ref{th:updown} (c), $X$ is Oka-1 if and only if $C$ is
$\CP^1$ or a torus. Note that minimal ruled surfaces over $\CP^1$ 
...
Assuming that the global spherical shell conjecture is true, 
...
\cite[Proposition 5.6.17]{Forstneric2017E}, and its unramified quotient $X\setminus P$ 
is an Oka manifold by \cite[Proposition 5.6.3]{Forstneric2017E}. 
...
singular points $q_1,\ldots,q_{16}$. Blowing up $X/\{1,\sigma\}$ at these singular 
points yields a smooth compact surface $Y$ containing mutually disjoint 
...
then the Zariski domain $Y_0=Y\setminus (B\cup C)$ and $X_0=X\setminus (A\cup P)$
...
\end{equation} 
In both cases the sum is over $p\in C$ with $n_p > 1$, and
$D=0$ if there are no multiple fibres. 

We can now state our main result concerning elliptic fibrations. 
...
when $C=\D$ is the disc is excluded by condition (b) as shown in 
\cite[proof of Theorem 3.9]{BuzzardLu2000}, and the case $C=\CP^1$ 
can be reduced to $C=\C$ by removing a point. 
...
$\Omega = X\setminus X^\sigma$ in $X$. Recall that the fibre of the Jacobian fibration
over $p\in \C$ is 
\[
...
\[
	...
	H^1(X_p,\Oscr_{X_p}^*) = \mathrm{Pic}^0(X_p) \to H^2(X_p,\Z)\to 0
\]
(see \cite[p.\ 627]{BuzzardLu2000}).
We have that $L_p:=H^1(X_p,\Oscr_{X_p}) \cong\C$ for all $p$,
...
%The same is true for every compact surface bimeromorphic to an elliptic K3 surface.
\end{corollary}

\begin{comment}
\begin{proof}
We only need to justify the second part of the corollary.
If $Y$ is a compact surface bimeromorphic to an elliptic K3 surface $X$, 
then $Y$ admits a dominating map from the complement of finitely many 
points $x_1,\ldots, x_m\in X$ in $X$. By passing to 

We shall disregard the points $x_i$ 
outside of the Zariski open domain $\Omega'=\Omega\setminus X'$,
using the notation in the proof of Theorem \ref{th:elliptic}.
The preimage of these points by the map $h:L\to \Omega$ in \eqref{eq:h8}

Proceeding as in the proof of the first part, this leads to the problem
\end{comment}
...
The equivalence of (a) and (b) in Theorem \ref{th:elliptic} gives the following.

%
%  PROPERLY ELLIPTIC SURFACES
%
\begin{corollary}\label{cor:properly elliptic}
Let $f:X\to C$ be an elliptic fibration over an elliptic curve. Then, the surface $X$ is 
densely dominable by $\C^2$ if and only if the fibration $f$ has no multiple fibres.
\end{corollary}

\begin{proof}
Let $D$ be the $\mathbb{Q}$-divisor \eqref{eq:D}. Since $\chi(C)=0$, we have that 
$\chi(C,D)=\chi(C)-\deg D=-\deg D$, so $\chi(C,D)\ge 0$ (condition (b)
in Theorem \ref{th:elliptic}) holds if and only if $D=0$, which means that there
are no multiple fibres.
\end{proof} 
...
%
%\section{Holomorphic curves in rationally connected manifolds}\label{sec:RC}
\section{Every compact rationally connected manifold is Oka-1}\label{sec:RC}

Since a complex manifold with many rational curves also contains many holomorphic lines,
rationally connected manifolds are natural candidates to be Oka-1 manifolds. 
These are complex projective manifolds $X$ with the property that any pair of points in $X$ 
can be connected by a rational curve $\CP^1\to X$. Among many excellent 
references, we mention 
the monographs by Koll\'ar \cite{Kollar1995E} and Debarre \cite{Debarre2001}.

The following main result in this section gives an affirmative answer to the 
above supposition. It is proved at the end of the section. 

%
%   RATIONALLY CONNECTED MANIFOLDS 
%
\begin{theorem}\label{th:RC}
Every compact rationally connected manifold is an Oka-1 manifold.
\end{theorem}

Theorem \ref{th:RC} includes the recent result of Campana and Winkelmann 
\cite[Theorem 5.2]{CampanaWinkelmann2020} to the effect that every rationally 
connected manifold $X$ admits an entire curve $\C\to X$ 
whose image contains a given countable subset of $X$, and one can also
prescribe finite order jets at these points \cite[Corollary 5.7]{CampanaWinkelmann2020}. 
In particular, every such manifold contains everywhere dense entire curves $\C\to X$. 
Theorem \ref{th:RC} applies to maps $R\to X$ from all open Riemann surfaces, 
and it is more precise even in the special case $R=\C$ since it gives interpolation 
on a discrete set, instead of mere hitting of a given sequence in $X$. 
Campana and Winkelmann give several further examples of complex manifolds 
having many holomorphic lines, and they discuss conjectural relationships 
between this property and several other properties of a complex manifold, 
such as being special in the sense of Campana \cite{Campana2004AIF,Campana2004AIF-2};
see \cite[Conjecture 4.2]{CampanaWinkelmann2020}.  
By extension, their discussion applies to Oka-1 manifolds.

Theorem \ref{th:RC} is new for manifolds of dimension bigger than $2$. 
In dimension $1$ the only such manifold is $\CP^1$, which is Oka, 
while in dimension $2$ every compact 
rationally connected surface is rational, % (i.e., birationally equivalent to $\CP^2$), 
and hence an Oka manifold; see Subsection \ref{ss:Kodairaminusinfinity}.

The construction of Campana and Winkelmann relies on the deformation theory for 
rational curves in a rationally connected manifold, in particular, on smoothing a  
comb of rational curves; 
see \cite[Proposition 5.1 and Lemma 5.5]{CampanaWinkelmann2020}.
(For a more general result, see Koll\'ar \cite[Theorem 7.6, p.\ 155]{Kollar1995E}. 
This technique goes back to the seminal paper
\cite{KollarMiyaokaMori1992} by Koll\'ar, Miyaoka, and Mori from 1992.)
Using these methods, we tried to show that a rationally connected manifold  
is densely dominable by tubes of lines coming from trees of rational curves, and hence
is an Oka-1 manifold by Theorem \ref{th:main1}. Our attempts in this direction
remained inconclusive; see the discussion in the sequel and Problems 
\ref{prob:RC} and \ref{prob:C2}. Instead, we obtain Theorem \ref{th:RC} by using  
Runge approximation theorem for compact pseudoholomorphic curves in 
certain compact almost complex manifolds, due to 
Gournay \cite[Theorem 1.1.1]{Gournay2012}, to which we add  
%
% AA: I added "jet"
%
jet interpolation at finitely many points (see Proposition \ref{prop:int}). Together with  
Proposition \ref{prop:weakOka1}, this provides the main inductive step in 
the proof of Theorem \ref{th:RC}. 

While it may seem unusual to obtain 
an approximation result for noncompact holomorphic curves from an approximation result
for compact curves, one should bear in mind that the holomorphic function theory on
open Riemann surfaces was first developed by Behnke and Stein \cite{BehnkeStein1949}
in 1949 by using the previously established theory of meromorphic functions
on compact Riemann surfaces.

%%%%%%%%%%%%

Before turning to the proof of Theorem \ref{th:RC}, we recall some background on
rationally connected manifolds and indicate a possible approach to finding dominating
tubes of lines in them, leading to Problem \ref{prob:RC}. 

Let $C=\bigcup_{i=1}^n C_i$ be a bouquet of $n$ rational curves $C_i=\CP^1$
intersecting at a single point $p_0$. 
We embed $C$ in $(\CP^1)^n$ such that  $C_i$ is the one-point compactification 
of the $i$-th coordinate axis in $\C^n\subset (\CP^1)^n$ 
and $p_0$ is the origin $0\in \C^n$. Let $X$ be a complex manifold of dimension $n$.
Given a neighborhood $U\subset (\CP^1)^n$ of $C$,  
the set $U\cap \C^n$ contains a spanning tube $T\subset \C^n$ around the union 
of the coordinate axes $C\cap \C^n$. Suppose that $f:C\to X$ is a morphism such that 
$df_0:T_0 C_i\to T_{f(0)} X$ is nonvanishing and 
\begin{equation}\label{eq:dominatingrational} 
\text{the complex lines $df_0(T_0 C_i)\subset T_{f(0)}X$ for $i=1,\ldots,n$ span $T_{f(0)}X$.}
\end{equation} 
Call such a map $f$ dominating at $0\in C$.
If $f$ extends to a holomorphic map $F:U\to X$ from a neighbourhood of $C$, then 
the restriction $F:T\to X$ to a tube $T\subset U\cap\, \C^n$ around $C\cap\C^n$
is a holomorphic map which is dominating at $f(0)\in X$. 
Thus, if most points $x\in X$ are the centre of a tree of lines $f:C\to X$ as above, 
then $X$ is an Oka-1 manifold by Theorem \ref{th:main1}.

Dominating trees $C\to X$ as in \eqref{eq:dominatingrational} exist in every
rationally connected $n$-manifold $X$. We recall a few basic facts about such manifolds,
referring to the monographs by Debarre \cite{Debarre2001} and Koll\'ar \cite{Kollar1995E}
for more information. For every rational curve $f:\CP^1\to X$ we have that 
\begin{equation}\label{eq:Birkhoff}
	f^*TX=\oplus_{i=1}^n \Oscr_{\CP^1}(a_i)\ \ \  \text{with $a_i\in \Z$}
\end{equation}
by the Birkhoff factorization theorem \cite{Birkhoff1909}, and $\max a_i\ge 2$ if $f$ is nonconstant
since in this case the bundle $f^*TX$ contains $T\CP^1\cong \Oscr_{\CP^1}(2)$.
A nonconstant rational curve $f:\CP^1\to X$ is said to be {\em $r$-free} for some $r\in\Z_+$ 
if $a_i\ge r$ for every $i=1,\ldots,n$. The curve is called {\em free} or {\em semipositive}
if this holds with $r=0$, and {\em very free} if it holds with $r=1$.
(See Debarre \cite[Definition 4.5 and Remark 4.6]{Debarre2001} 
or Koll\'ar  \cite[Definition 3.1]{Kollar1995E}.)
A compact projective manifold $X$ with a very free curve $f:\CP^1\to X$ is rationally connected, 
and the associated evaluation map $ev:M\times \CP^1\to X$ with $ev(m_0,\cdotp)=f$,
where $M$  (with a base point $m_0\in M$) is the moduli space of rational curves in $X$ 
homologous to $f$, is a smooth morphism with dense image 
(see \cite{AraujoKollar2003,Debarre2001,Kollar1995E}). 
In particular, every free rational curve $f:\CP^1\to X$ is {\em regular}, in the sense that the 
linearized $\dibar$-operator 
\[ % begin{equation}\label{eq:Df}
	D_f:\Cscr^\infty(\CP^1,f^*TX)\to \Cscr^\infty(\CP^1, \Lambda^{0,1}\CP^1 \otimes f^*TX)
\] % end{equation}
from the Fr\'echet space of smooth sections of the complex vector bundle 
$f^*TX\to\CP^1$ to the space of smooth $(0,1)$-forms on $\CP^1$ with values 
in the same bundle is surjective. 
An explicit formula for $D_f$ is given by \eqref{eq:Df} below in the 
more general case of a smooth curve in an almost complex manifold.
If the manifold $X$ is rationally connected, then for most points $x\in X$ and 
tangent vectors $v\in T_xX$ there is a rational curve $\CP^1\to X$ through $x$ 
in the direction $v$, so we obtain  dominating trees as in \eqref{eq:dominatingrational}. 

%
%  PROBLEM ON RATIONALLY CONNECTED MANIFOLDS
%
% AA: This problem had the same label as the previous one. I changed this one.
%
\begin{problem}\label{prob:RC}
Let $f:C\to X$  be a morphism from a tree $C=\bigcup_{i=1}^n C_i \subset (\CP^1)^n$ 
of rational curves as above.
Assuming that every rational curve $f|_{C_i}:C_i\to X$ for $i=1,\ldots,n$ is $r$-free 
for a sufficiently large $r\ge 0$, does $f$ extend to a holomorphic map $U\to X$ 
on some open neighbourhood $U\subset (\CP^1)^n$ of $C$?
\end{problem}
 
The problem in obtaining an extension by using deformation theory
is indicated by Araujo and Koll\'ar in \cite[Warning 16, p.\ 25]{AraujoKollar2003}. 
In view of Remark \ref{rem:extendedtube} (which relies on a nontrivial
result from Oka theory), a holomorphic extension 
would yield holomorphic maps $\C^2\to X$ having rank 2 at a generic point. 
While such maps exist when $\dim X=2$ since $X$ is then a rational surface
and hence an Oka manifold, this seems unknown when $\dim X>2$. 

\begin{problem}\label{prob:C2}
Let $X$ be a compact rationally connected manifold of dimension $>2$.
Does there exist a holomorphic map $\C^2\to X$ having rank $2$ at a generic point?
\end{problem}

In the proof of \cite[Theorem 5.2]{CampanaWinkelmann2020},
Campana and Winkelmann use deformation theory to obtain a 
holomorphic extension of a morphism from a bouquet of two rational curves 
$C=C_1\cup C_2$, embedded in the complex surface $Y$ 
obtained by blowing up a point in $\CP^1\times \C$, such that $C_1$ and 
$C_2$ are -1 curves in $Y$ \cite[p.\ 7]{CampanaWinkelmann2020}. 
However, neighbourhoods of $C$ in this surface $Y$ are too small to contain 
a tube of lines. Indeed, they admit a nonconstant bounded plurisubharmonic function, 
while a tube of lines does not.

We now present the scheme of Gournay's construction leading to 
\cite[Theorem 1.1.1]{Gournay2012} (see Theorem \ref{th:Gournay}). 
This is  necessary in order to explain the proof of Proposition \ref{prop:int}.
To simplify the discussion, we shall consider things in the $\Cscr^\infty$ category.
A suitable setting can be found in the monograph by McDuff and Salamon 
\cite[Section 3.1]{McDuffSalamon2012}, and we recall it briefly. 

Let $(\Sigma,j)$ be a compact Riemann surface and $(X,J)$ be a smooth compact 
almost complex manifold, where $j$ and $J$ are, respectively, 
the almost complex structure operators on $T\Sigma$ and $TX$. 
A $\Cscr^1$ map $f:(\Sigma,j) \to (X,J)$ is said to be $J$-holomorphic, or simply
holomorphic, if it satisfies the Cauchy--Riemann equation at every point $z\in \Sigma$:
%
% AA: I added \circ in the right hand side of the next equation
%
\[
	df_z\circ j_z = J_{f(z)}\circ df_z.
\]
Such {\em pseudoholomorphic curves} exist in 
great abundance even if the complex structure $J$ on $X$ is nonintegrable; 
see \cite{Gromov1985,McDuffSalamon1994,McDuffSalamon2012,Sikorav1994}
among many other sources. (Note that an almost complex structure $j$ on a surface 
$\Sigma$ is always integrable.) We endow $X$ with a complete $J$-hermitian metric 
$\hgot$ and let $\nabla$ denote the associated covariant derivative. 
(More precisely, $\hgot=\ggot-\sqrt{-1}\,\omega$ where $\ggot$ is a Riemannian
metric, $\nabla$ is the covariant derivative with respect to $\ggot$, and $\omega$ is a nondegenerate $2$-form. Note that $(X,J,\hgot)$ is a K\"ahler manifold
if and only if $J$ is integrable and $d\omega=0$, i.e., $\omega$ is a symplectic form.
However, the precise choice of the metric is not important in Gournay's proof,
a fact which we shall explore in the proof of the interpolation 
result in Proposition \ref{prop:int}.)
Given a smooth map $f:\Sigma\to X$, its differential (a $1$-form on $X$ with values in 
$f^* TX$) decomposes as $df=\di_J f + \dibar_J f$, where
\[
	\di_J f = \frac12 (df - J\circ df \circ j) \quad\text{and}\quad 
	\dibar_J f = \frac12 (df + J\circ df \circ j)
\]
are its $(1,0)$ and $(0,1)$ components. % with respect to $J$. 
Note that $f$ is $J$-holomorphic if and only if $\dibar_J f=0$. 

The space $\Bscr=\Cscr^\infty(\Sigma,X)$ of smooth maps $\Sigma\to X$ 
is an infinite dimensional Fr\'echet manifold whose tangent space
at $f\in\Bscr$ is the space $\Cscr^\infty(\Sigma,f^* TX)$ of smooth
sections of the $J$-complex vector bundle $f^* TX\to\Sigma$.
Elements of $\Cscr^\infty(\Sigma,f^* TX)$ are smooth vector fields on $X$
along the map $f$. For $z\in \Sigma$ we have the geodesic curve 
$\R \ni s\mapsto \exp_{f(z)}(s\xi(z))\in X$ passing at $s=0$ through $f(z)$ in the direction
of the vector $\xi(z)\in T_{f(z)}X$. The parallel transport along the geodesic flow 
$s \mapsto \exp_f(s\xi)\in \Bscr$ with respect to the $J$-linear connection 
$\wt \nabla=\nabla - \frac12 J \nabla J$ on $TX$ defines at $s=1$ an isomorphism 
\[
	\Phi_f(\xi): f^* TX \stackrel{\cong}{\longrightarrow} (\exp_f(\xi))^* TX
\]
of $J$-complex vector bundles over $\Sigma$, with $\Phi_f(0)=\Id$. 
(See \cite[p.\ 39]{McDuffSalamon2012} and note that $\wt \nabla = \nabla$ 
if $(X,J,\hgot)$ is a K\"ahler manifold.) Consider the map   
\[
	\Fcal_f: \Cscr^\infty(\Sigma,f^*TX) \to 
	\Cscr^\infty(\Sigma,T^{0,1}\Sigma \otimes f^*X), \quad
	\Fcal_f(\xi) = \Phi_f(\xi)^{-1}\, \dibar_J (\exp_f(\xi)).
\]
Thus, $\Fcal_f(0)=\dibar_J f$. (In the definition of $\Fcal_f$, one
can also use the metric connection $\nabla$ and apply a projection 
of $\Phi_f(\xi)^{-1}\, \dibar_J (\exp_f(\xi))$ on the space 
$\Cscr^\infty(\Sigma,T^{0,1}\Sigma \otimes f^*X)$ of $(0,1)$-forms, 
as Gournay does in \cite{Gournay2012}.) 
The linearization of $\dibar_J$ at $f\in\Bscr$ is the operator 
\[	
	D_f:\Cscr^\infty(\Sigma,f^*TX) \to \Cscr^\infty(\Sigma,T^{0,1}\Sigma \otimes f^*X)
\]
given on $\xi\in \Cscr^\infty(\Sigma,f^* TX)$ by
\[
	D_f \xi = (d\Fcal_f)_0 (\xi).
\]
Hence, we have that 
\[
	\Fcal_f(\xi)  = \dibar_J f + D_f \xi + \text{higher order terms in $\xi$}. 
\]
The explicit expression for the value of $D_f \xi$ on a vector $v\in T_z\Sigma$ is 
\begin{equation}\label{eq:Df}
	D_f \xi(v) = \frac12 \left(\nabla_{\!v} \xi(z) + J_{f(z)}\nabla_{\! j(v)} \xi(z)\right)
	- \frac12 J_{f(z)} (\nabla_{\!v} J_{f(z)}) \di_J f(v)
\end{equation}
(see McDuff and Salamon \cite[Eq.\ (3.1.4)]{McDuffSalamon2012}). 
Here, $\nabla$ is the connection on $f^*TX$ induced by the one on $TX$.
As shown in \cite[Appendix C]{McDuffSalamon2012}, $D_f$ is an elliptic 
Fredholm operator and  
\begin{equation}\label{eq:indexDf}
	\text{index} \, D_f = n (2-2g(\Sigma)) + 2c_1(f^* TX),
\end{equation}
where $n=\dim_\C X$, $g(\Sigma)$ is the genus of $\Sigma$, 
and $c_1$ is the first Chern number. 

If $(X,J,\hgot)$ is a K\"ahler manifold then $J$ is parallel with respect to the 
geodesic flow defined by the Riemannian metric $\ggot=\Re \hgot$, 
so $\nabla J=0$ and hence the second term on the right hand side of 
\eqref{eq:Df} vanishes. If in addition the map $f$ is holomorphic then 
the first term on the right hand side equals $\dibar_J \xi$, so in this case 
$D_f$ is the standard $\dibar$-operator on the holomorphic 
vector bundle $f^*TX$. (The same observation
holds on an open subset $U\subset \Sigma$ on which $f$ is holomorphic.) 
Solving the equation $D_f\xi=\eta$ locally on $\Sigma$ leads to a Cousin-I problem 
with coefficients in the holomorphic vector bundle $f^*TX$. 
By the Riemann--Roch theorem this problem is solvable if $f^*TX$ is sufficiently positive, 
depending on the genus of $\Sigma$ (cf.\ \eqref{eq:indexDf})
However,  the initial map in the proof of Theorem \ref{th:Gournay} is not 
holomorphic everywhere.

Let $\Escr\to \Bscr$ be the infinite dimensional vector bundle 
whose fibre over $f\in \Bscr$ is 
\[
	\Escr_f = \Omega^{0,1}(\Sigma,f^* TX) := 
	\Cscr^\infty(\Sigma,T^{0,1}\Sigma \otimes f^*TX)
\]
of smooth $(0,1)$-forms on $\Sigma$ with values in the complex 
vector bundle $f^* TX$. Hence, the $\dibar_J$ operator defines a section 
$\Bscr\to \Escr$ given by $\Bscr\ni f\mapsto (f,\dibar_J f)\in \Escr$.

Assume now that $f\in\Bscr$ is such 
that $\dibar_J f$ is small in a suitable norm. Assume in addition
that the linearized operator $D_f: T_f\Bscr \to \Escr_f$ in \eqref{eq:Df} has a bounded
right inverse $S_f$ so that $D_f \circ S_f=\Id$ on $T_f\Bscr=\Cscr^\infty(\Sigma,f^*TX)$.
The plan is to find a holomorphic map of the form 
$\tilde f=\exp_f (\tilde \xi):\Sigma\to X$ close to $f=\exp_f(0)$, 
where $\tilde \xi$ is the fixed point of the 
(modified) Newton iteration given by the map
\[
	\Hcal: \Cscr^\infty(\Sigma,f^*TX)\to \Cscr^\infty(\Sigma,f^*TX),
	\quad
	\Hcal(\xi)= \xi - (S_f \circ \Fcal_f)(\xi).
\]
The iteration begins at $\xi_0=0$, where 
$d\Hcal_0=\Id-S_f\circ (d\Fcal_f)_0=\Id-S_f\circ D_f=0$, and the recursion
is given by $\xi_j=\Hcal(\xi_{j-1})$ for $j\in\N$. When things are done correctly,
the sequence $\xi_j$ converges to a fixed point $\tilde \xi=\lim_{j\to\infty}\xi_j$
which solves the problem. In the context of Banach spaces, a precise 
result on the existence of a fixed point is \cite[Theorem 2]{Kuzman2018}.

To realize the first step of this plan, namely, finding an approximate solution, 
Gournay introduced the following property of an almost complex manifold 
$X$ \cite[Definition 3.1.2]{Gournay2012}.  

\smallskip
\noindent{\bf Double tangent property.} 
There is a dense subset $E\subset TX\oplus TX$ such that for every 
$(a,b)\in E_x\subset T_x X\oplus T_x X$,
$0<\epsilon <1/3$, and every small enough $r>0$ (depending on $(a,b,\epsilon)$)  
there is a holomorphic rational curve $f:\CP^1\to X$ with $f(p)=x$ for some $p\in \CP^1$ 
such that in a pair of local charts $\phi:U\to \C$ on $p\in U\subset \CP^1$,  
with $\phi$ holomorphic and $\phi(p)=0$, and $\Phi: V \to \C^n$ on $x\in V\subset X$ 
with $\Phi(x)=0$ and $\dibar \Phi_{x}=0$, we have that
\begin{equation}\label{eq:DTP}
	\Phi\circ f\circ\phi^{-1}(z) = az + \frac{br^2}{z}+ O(r^{1+\epsilon})\ \ 
	\text{on the annulus $A_r=\{\frac{r}{1+r^\epsilon} \le |z|\le  r(1+r^\epsilon)\}$}.
\end{equation}

The following is Gournay's main result \cite[Theorem 1.1.1]{Gournay2012}.
This is not precisely his statement, which assumes that $J$ is regular on all
rational curves in $X$, but is our understanding of what his proof 
actually gives. In other words, $X$ may contain rational curves on which
$J$ is not regular as long as these curves need not be used in the course
of the proof.

%
%  GOURNAY'S THEOREM
%
\begin{theorem} \label{th:Gournay}
Let $(X,J)$ be a smooth compact almost complex manifold, 
and assume that $J$ is regular on a set of holomorphic rational curves 
$\CP^1\to X$ satisfying the double tangent property. 
% \eqref{eq:DTP}.
Given a compact Riemann surface $\Sigma$, a compact subset $K\subset \Sigma$,
and a smooth map $g:\Sigma \to X$ which is holomorphic on a neighbourhood 
of $K$, we can approximate $g$ uniformly on $K$ by holomorphic maps 
$F:\Sigma\to X$.
\end{theorem}

Gournay proved (see \cite[Proposition 1.2.1]{Gournay2012} and
\cite[Theorem 1.3]{Gournay2010X}) that the double tangent property holds if there
is a dense set $\Dcal$ in the unit tangent bundle of $X$ such that for every vector 
$v\in \Dcal_x,\ x\in X$, there is a rational curve $f:\CP^1\to X$ 
with $f(p)=x$ and $v\in df_p(T_p\CP^1)$ for some $p\in \CP^1$.

In the proof of Theorem \ref{th:RC} we will need the following additions 
to Gournay's theorem.

%
%  GOURNAY'S THEOREM WITH INTERPOLATION
%
\begin{proposition}\label{prop:int}
The approximating holomorphic map $F:\Sigma\to X$ in Theorem \ref{th:Gournay} can 
be chosen such that it agrees with $g$ at finitely many given points in $K$.

If in addition the almost complex structure $J$ on $X$ is integrable on 
a neighbourhood of the compact set $g(K)\subset X$, then 
$F$ can be chosen to agree with $g$ to any given finite order at 
finitely many given points in $K$.
\end{proposition}

Theorem \ref{th:Gournay} and Proposition \ref{prop:int} can be seen as a nonlinear
version of the comb smoothing theorem, due to Koll\'ar, Miyaoka, and Mori 
\cite{KollarMiyaokaMori1992}, which says that by attaching sufficiently many semipositive
rational curves $f_i:\CP^1\to X$ to a given compact holomorphic curve $g:\Sigma\to X$
ensures that the curve $g(\Sigma)+\sum_i f_i(\CP^1)$ moves in a smooth family of 
algebraic curves in $X$ passing through a given finite set of points in 
$g(\Sigma)\setminus \bigcup_i f_i(\CP^1)$. 

We now recall the scheme of Gournay's proof of Theorem \ref{th:Gournay},
the main aim being to provide an explanation for Proposition \ref{prop:int}.

The first step is to find a smooth map $f:\Sigma\to X$ 
which agrees with the given map $g:\Sigma\to X$ 
on a neighbourhood of $K$ (hence it is holomorphic there), 
and which is also holomorphic on a large number of well spread 
discs $\Delta_1,\ldots,\Delta_m$ in $\Sigma$ with pairwise disjoint closures
$\overline \Delta_i\subset \Sigma \setminus K$ such that a 
suitable norm of $\dibar f$ on $\Sigma$ is sufficiently small. 
(The norms used in Gournay's proof are similar to those of Taubes \cite{Taubes1994}; 
see \cite[Section 2]{Gournay2012}.) The precise choice and number  
of the discs $\Delta_i$ depends on the constant $\epsilon>0$ in the desired estimate 
$\sup_{p\in K}\dist_X (F(p),g(p))<\epsilon$, and in the case of Proposition \ref{prop:int} 
also on the interpolation data. To obtain such $f$, one proceeds as follows; 
see \cite[Sect.\ 3.1]{Gournay2012}. 
In a pair of local coordinates around a point $p\in \Sigma\setminus K$ and 
of $g(p)\in X$ as in the definition of the double tangent property, in which these points are 
the origin in $\C$ and $\C^n$, respectively, we have that 
\begin{equation}\label{eq:b}
	g(z)=az+b\bar z+o(|z|)\ \ 
	\text{where $a=\frac{\di g}{\di z}(0)\in\C^n$ and $b=\frac{\di g}{\di\bar z}(0)\in\C^n$.}
\end{equation}
On the circle $|z|^2=z\bar z=r^2>0$ we have that $\bar z=\frac{r^2}{z}$, and hence 
\[
	g(z)=az+\frac{b r^2}{z} +o(r)\ \ \text{for $|z|=r>0$}.
\]
By the double tangent property there is a
rational curve $f_p:\CP^1\to X$ whose local expression in the same
coordinates is given by \eqref{eq:DTP}, so it is close to $g$ on the annulus $A(r)$ 
in \eqref{eq:DTP}. By smoothly gluing $g$ with $f_p$ on the annulus 
$r\le |z|\le r(1+r^\epsilon)$, we replace the smooth disc $g(\{|z|\le r\}) \subset g(\Sigma)$ 
by the holomorphic disc $f_p(\{|z|\le r\})$ without changing the map very much 
elsewhere. In \cite{Gournay2012} this is called {\em grafting} of the rational 
curve $f_p$ to $g(\Sigma)$. 
We repeat grafting at other points of $\Sigma\setminus K$ such that the grafted discs
are pairwise disjoint and contained in $\Sigma\setminus K$ 
until the resulting map $f:\Sigma\to X$ is holomorphic on a large subset of $\Sigma$ and 
has small enough $\dibar f$ in a suitable metric on $\Sigma$, adjusted to $f$. 
Note that $f$ agrees with $g$ near $K$ but it may be far from 
it on the discs $\Delta_i\subset \Sigma\setminus K$ $(i=1,\ldots,m)$ 
where grafts were made. Furthermore, this procedure changes the 
homology class of $f(\Sigma)$ in $H_2(X,\Z)$.

As pointed out in \cite[p.\ 334]{Gournay2012}, the heuristic idea to construct 
this approximate solution can be found in Donaldson's paper 
\cite[Section 3]{Donaldson1993}.  Comparison with the works of
Donaldson \cite{Donaldson1993} and Taubes \cite{Taubes1994} 
on instantons and anti-self-dual metrics is discussed 
in \cite[p.\ 318]{Gournay2012}.

%
% From Gournay's paper (only for my information)
%
\begin{comment}
Instantons and anti-self-dual metrics. As a last note in this introduction,
there are some differences between the case of instantons (on the sphere) or the antiself-
dual metrics and the J-holomorphic problem: the non-linearity is quadratic in
...
In the second step, Gournay finds a section $\tilde\xi\in \Cscr^\infty(\Sigma,f^*TX)$
which is uniformly small on $K$ such that the map $F=\exp_f(\tilde \xi):\Sigma\to X$ 
is $J$-holomorphic, so $F$ satisfies Theorem \ref{th:Gournay}. 
Such $\tilde \xi$ is found by a Newton iteration as described above; 
see \cite[Sections 3.4--3.5]{Gournay2012}.
The crucial ingredient is to solve the linearized equation $D_f\xi=\eta$ \eqref{eq:Df}
with good estimates. Gournay reduces this problem to solving the linearized equation 
$D_{f}\xi_i =\eta_i$ on each of the discs $U_i=\Delta_i\subset \Sigma$ $(i>0)$, which 
come from rational curves as described in the first step of the proof, 
and for $i=0$ on a neighbourhood $U_0$ of the set 
$\Sigma \setminus \bigcup_{i=1}^m \Delta_i$. Note that $U_0$ is a finite bordered
Riemann surface. The sections $\xi_i\in \C^\infty(f^*TX|_{U_i})$ 
are smoothly glued together into a section $\xi\in \Cscr^\infty(\Sigma,f^*TX)$ as 
described in \cite[p.\ 338]{Gournay2012}, and Gournay then identifies the relations 
between the pairs $(\xi_i,\eta_i)$ solving localized equations
$D_f \xi_i=\eta_i$ which must be satisfied in 
order that $\xi$ satisfies the equation $D_f\xi=\eta$. 
To solve this problem, he introduces new unknown quantities 
$h_i\in \Cscr^\infty(T^{0,1}\Sigma \otimes f^*X|_{U_i})$ related to $\xi_i$ by 
\begin{equation}\label{eq:hi}
	\xi_i=D_{f}^* h_i \ \ \text{for $i=0,1,\ldots,m$}, 
\end{equation}
and he considers the corresponding Neumann problem 
\begin{equation}\label{eq:Laplace}
	D_{f} D^*_{f} h_i = \eta_i \ \ \text{on $U_i$ for $i=0,1,\ldots,m$}
\end{equation}
(see \cite[p.\ 339]{Gournay2012}). These are second order elliptic equations
whose highest order term is a Laplacian. 
From the compatibility conditions on the $(0,1)$-forms $\eta_i$, given on 
\cite[p.\ 339]{Gournay2012} just below the definition of the multilinear map $H_0$, 
we see that $\eta_i$ for $i>0$ depends on $\eta$ and on $h_0$.
In addition, the form $\eta_0$ in the equation $D_f D^*_f h_0=\eta_0$ on $U_0$ 
depends on $h_i$ for $i\ge 0$ (cf.\ \cite[p.\ 341]{Gournay2012}).
The approach to solving this problem is by iteration whose first step is $h_0=0$
and the initial collection $(\eta_i)_{i=0,\ldots,m}$ obtained by restricting 
$\eta$ to the sets $U_i$. (The actual Newton iteration will start with 
$\eta=\dibar_J f$.) Using this data, we find $h_i$ on $\Delta_i$ for $i>0$ by solving 
\eqref{eq:Laplace}. From the compatibility conditions on \cite[p.\ 341]{Gournay2012} 
we obtain the next $\eta_0$, and hence the next $h_0$ solving \eqref{eq:Laplace} for $i=0$.
These new data are again used in \eqref{eq:Laplace} to find the next collection 
$\eta_i$, $h_i$ for $i>0$. The process continues and converges to a solution 
provided $\eta$ is small enough. (The role of the map $H_0$ defined on p.\ 339 is 
summarized in \cite[Remark 3.3.6]{Gournay2012}. The projection $\Pi_E$ which appears 
there is introduced on \cite[p.\ 321]{Gournay2012}.) 
The key result is \cite[Corollary 2.5.5]{Gournay2012},
which provides a solution $[h]=(h_i)_{i=0,\ldots,m}$ to \eqref{eq:Laplace} with 
suitable estimates. The solution $\xi$ of the linearized equation $D_f\xi=\eta$
is then obtained from the collection $\xi_i$ given by \eqref{eq:hi} since the 
required compatibility conditions were taken into account in the equations 
\eqref{eq:Laplace} for $[h]=(h_i)$. 
%Although Gournay solves the linearized equations on the entire rational curves used in grafting, the parts of solutions outside of the grafted discs $\Delta_i$ are discarded. 

The passage from the linearized problem $D_f\xi=\eta$ to the nonlinear 
problem is explained in \cite[Section 3.4]{Gournay2012} (see especially Equation (3.4.1)), 
where the author constructs an approximate $E$-quasisolution. 
Finally, in \cite[Section 3.5]{Gournay2012} it is explained how
$E$-quasisolutions lead to an authentic solution $\tilde \xi$ for which the map
$\exp_f(\tilde \xi):\Sigma\to X$ is holomorphic.

%
% FF  This paragraphs was made a remark, and the last sentence was added
%
\begin{remark}\label{rem:Gournay}
Looking at Gournay's proof, we see that the true role of the hypothesis 
that the almost complex structure $J$ is regular on the 
rational curves $f_i:\CP^1\to X$ used in grafting 
is that such grafts increase positivity of the bundle $f^*TX\to\Sigma$  
(see \eqref{eq:indexDf}), thereby ensuring the existence of solutions $\xi$ of 
the linearized problem $D_f\xi=\eta$ with good estimates. This is in line with our 
earlier observation concerning the comb smoothing theorem 
of Koll\'ar et al.\ \cite{KollarMiyaokaMori1992}. 
%In the case of interest to us when 
If $(X,J)$ is a rationally connected manifold,
it suffices to use semipositive rational curves in $X$, i.e., those for which 
the integers $a_1,\ldots,a_n$ in the factorization \eqref{eq:Birkhoff} of the 
vector bundle $f_i^*TX\to\CP^1$ are nonnegative; see the proof of
Theorem \ref{th:RC} and Corollary \ref{cor:RC} below.
\end{remark}

%
%  INTERPOLATION
%
\begin{proof}[Proof of Proposition \ref{prop:int}]
In the first part we follow the idea of Kuzman in \cite[proof of Corollary 7]{Kuzman2018}. 
Let $p_1,\ldots,p_k\in K$ be the points where we wish to interpolate. 
Choose small discs $p_i\subset \Delta'_i\subset \Sigma$ with pairwise
disjoint closures which are contained in the open 
neighbourhood of $K$ where the map $g$ is holomorphic (hence $f=g$ on these discs). 
On each of these discs, the linearized equation $D_f\xi=\eta$ admits a solution 
that vanishes at $p_i$. Indeed, we see from \eqref{eq:Df} 
that if $\xi$ solves the equation  $D_f \xi =\eta$, then for any constant section 
$c$ with $\nabla c=0$ the sum $\xi+c$ solves the same equation. 
In the K\"ahler case, which is of main interest to us, this is the standard 
non-homogeneous $\dibar$ equation which, on the unit disc $\Delta\subset \C$, 
is solved by the normalized Cauchy--Green operator
\[
	\xi(z)=\Tcal (\eta)(z) =
	\frac{1}{\pi} \int_{\zeta\in \Delta} \left(\frac{1}{z-\zeta}+\frac{1}{\zeta}\right)
			\eta(\zeta) \, du dv,\quad z\in\Delta,\ \zeta=u+\imath v.
\]
This operator satisfies $\frac{\di}{\di\bar z}\Tcal(\eta)=\eta$ and $\Tcal(\eta)(0)=0$.
For any $g\in L^p(\Delta)$, $p>2$, $\Tcal(g)$ is a bounded continuous function on $\C$ 
satisfying the uniform H\"older condition with the exponent $\alpha=1-2/p$; 
moreover, $\Tcal: L^p(\Delta)\to \Cscr^\alpha(\C)$ is a continuous linear operator
(see \cite[Lemma 1, p.\ 51]{Ahlfors2006} or \cite[Lemma 1.5]{BoivinGauthier2001}.)
It is also a bounded operator $W^{k,p}(\Delta) \to W^{k+1,p}(\Delta)$ on Sobolev
spaces for $k\in\Z_+$ and $p\ge 2$; see \cite[Section 4.3]{AstalaIwaniecMartin2009}.

We add to the discs $\Delta'_i$ $(i=1,\ldots,k)$ around the points $p_i\in K$ 
a suitable collection of discs $\{\Delta_i\}$ obtained by grafting and apply Gournay's proof 
with this enlarged collection. The closures of all these discs must be
pairwise disjoint. (As pointed out by Kuzman in \cite{Kuzman2018}, 
we can think of the discs $\Delta'_i$ as trivial grafts with $b=0$ in \eqref{eq:b}. 
The collection of discs $\Delta_i$ where grafts are made has to depend on the 
interpolation data to ensure that the estimates in \cite{Gournay2012} apply. 
Indeed, it is impossible to interpolate at more and more points of $K$
when starting the Newton iteration from a fixed map $f:\Sigma\to X$, 
as this would produce an infinite dimensional family of holomorphic curves 
$\Sigma\to X$ in the same homology class, which is impossible.)
The solutions $\xi_i$ to the linearized equation $D_f\xi_i=\eta_i$
on the discs $\Delta'_i$ vanish at $p_i\in \Delta'_i$ by the construction.
This can be incorporated into the relevant function 
spaces, in particular, in the definition of the Fr\'echet
space  $\wt \Cscr^\infty(T^{0,1}\Sigma \otimes f^*X)$ on top of \cite[p.\ 344]{Gournay2012}.
The holomorphic map $\exp_f \tilde \xi :\Sigma\to X$ obtained by 
Newton iteration then agrees with $f$, and hence with $g$, 
at the given points $p_1,\ldots,p_k$, and it approximates $g$ on $K$.

Assume now that $J$ is integrable on a neighbourhood $f(\overline\Delta'_i)$
for $i=1,\ldots,k$. We may assume that the discs $\Delta'_i$ are chosen small 
enough so that $f(\overline\Delta'_i)$ is contained in an open chart
$V_i\subset X$ biholomorphic to a domain in $\C^n$ with $n=\dim X$.
Fix holomorphic coordinates $(z_1,\ldots,z_n)$ on $V_i$ 
and choose a smooth hermitian metric $\hgot$ on $X$ which equals the 
flat metric $\sum_{j=1}^n dz_j\otimes d\bar z_j$ 
on a slightly smaller domain $V'_i\subset V_i$ with $f(\overline\Delta'_i) \subset V'_i$.
Due to smoothness of the map $f:\Sigma\to X$ and the elliptic regularity 
of the equation $D_f\xi=\eta$, all forms $\xi$ in Gournay's procedure are smooth. 
Recall that the $J$-complex vector bundle $f^*TX$ is holomorphic over 
each disc $\Delta'_i$. At every step of Gournay's procedure, 
we subtract from the solution $\xi_i$ of $D_f\xi_i=\eta_i$ on $\Delta'_i$ 
a holomorphic polynomial $P_i$ in the complex coordinate $z$ on $\Delta'_i$
to ensure that $\xi_i-P_i$ has vanishing holomorphic jet of order $s$ at $p_i$
(although its total jet of order $s$ need not vanish). Note that $\xi_i-P_i$
still solves the same equation. 
Since the coefficients of $P_i$ are controlled by the $L^p$-norm of 
$\eta_i$ on $\Delta'_i$, this changes the constants  
in Gournay's construction by a finite factor, which can be offset  
by the choice of graftings in the initial step. Proceeding as before, 
we obtain a section $\tilde\xi \in \Cscr^\infty(\Sigma,f^* TX)$ 
whose holomorphic jet of order $s$ at $p_i$ vanishes for $i=1,\ldots, k$
and the map $F=\exp_f\tilde\xi:\Sigma\to X$ is holomorphic.
Since $\hgot=\sum_{j=1}^n dz_j\otimes d\bar z_j$ on $V'_i$, we have 
in the local coordinates $(z_1,\ldots,z_n)$ on $V'_i$ that 
\[
	\exp_{f(z)}(\xi(z)) = f(z) + \xi(z) \quad
	\text{for $z\in \Delta'_i$ and $\xi(z)$ small enough.}
\]
In particular, $\exp_f\tilde\xi =f+\tilde \xi$ near $p_i$ for $i=1,\ldots,k$, 
so $F$ agrees with $f$ to order $s$ at $p_i$. 
\end{proof}
...
%   PROOF OF THEOREM RC
%
\begin{proof}[Proof of Theorem \ref{th:RC}]
Every compact rationally connected manifold $X$ is a projective manifold,
hence K\"ahler. It is simply connected \cite[Corollary 4.18]{Debarre2001}. 
Furthermore, positive (very free) 
rational curves in $X$ satisfy \cite[Proposition 1.2.1]{Gournay2012},
and hence the double tangent property holds. Indeed, on any rationally connected 
complex manifold there is a positive rational curve passing through any given 
finite set of points with preassigned tangents at these points; see 
\cite[Corollary 2.8 and Exercise 4.8.6]{Debarre2001}. Hence,
...
Theorem \ref{th:Gournay} and Proposition \ref{prop:int} hold on every 
compact rationally connected manifold $(X,J)$.
\end{corollary}

%We continue with the proof of Theorem \ref{th:RC}. 
Let $K\subset L\subset R$ and $f:K\to X$ be as in Proposition \ref{prop:weakOka1} (a). 
Choose a compact smoothly bounded domain $M\subset R$ with $L\subset \mathring M$ 
such that $L$ (and hence $K$) is a strong deformation retract of $M$. 
Thus, $f$ extends from $K$ to a smooth map $f:M \to X$. Let $\Sigma$ be a compact 
Riemann surface obtained by gluing a disc to $M$ along each Jordan curve in $bM$. 
Since $X$ is simply connected, $f$ extends from $M$ to a smooth map $\Sigma\to X$.
By Theorem \ref{th:Gournay} and Proposition \ref{prop:int}, $f$ can be approximated 
uniformly on $K$ by holomorphic maps $\tilde f:\Sigma\to X$ which agree with $f$
to a given order at the given finitely many points in $K$. 
The restriction of $\tilde f$ to $L$ then satisfies 
the conditions in Proposition \ref{prop:weakOka1}. Hence, $X$ is an Oka-1 manifold. 
\end{proof}


\begin{remark}
By using only Theorem \ref{th:Gournay} without Proposition \ref{prop:int},
we see that a compact rationally connected 
manifold $X$ satisfies the Oka-1 property with approximation.  
By Corollary \ref{cor:densecurves} this still gives holomorphic curves 
$R\to X$ with dense images from any given open Riemann surface $R$, 
in particular, from $R=\C$. 
\end{remark}
...
$M=\Delta$ obtained by Forstneri\v c and Winkelmann in \cite{ForstnericWinkelmann2005MRL}.

%
% Theorem
%
\begin{theorem}\label{th:dense}
Assume that $X$ is a connected complex manifold endowed with a complete distance function $\dist_X$, 
$\overline M=M\cup bM$ is a compact bordered Riemann surface, and $f:\overline M\to X$ is a map of class 
$\Ascr(\overline M)$. Given a compact subset $K\subset M$, a countable set $B\subset X$, and 
a number $\epsilon>0$, there is a holomorphic map $F: M\to X$ such that 
\begin{enumerate}[\rm (a)]
\item $\sup_{p\in K}\dist_X(F(p),f(p))<\epsilon$, 
 
\item $F$ agrees with $f$ to a given finite order at a given finite set of points in $M$, and
 
\item $B\subset F(M)$.
\end{enumerate}
Furthermore, $F$ can be chosen to be an immersion if $\dim X>1$, and to be an injective immersion 
if $\dim X>2$, whenever condition {\rm (b)} allows it.
\end{theorem}
%
%
%
...
It was shown by Forn\ae ss and Stout \cite{FornaessStout1977AJM,FornaessStout1982AIF} that 
...
for maps $M\to\Delta^n$. However, there seems to be no particular advantage in this reduction,
which also does not give the approximation and the general position result in 
Theorem \ref{th:dense}.

%
% Proof of Theorem 
%
\begin{proof}[Proof of Theorem \ref{th:dense}]
We assume without loss of generality that $M$ is a smoothly bounded compact domain without 
holes in an open Riemann surface $R$ (see Stout \cite[Theorem 8.1]{Stout1965TAMS}) and, 
by the Mergelyan theorem with interpolation for manifold-valued maps 
(see Theorem \ref{th:Mergelyan}), that $f_0=f$ is holomorphic on $\overline M$.
Let $K$, $B$, and $\epsilon$ be as in the statement, and assume as we may that $K_0=K$ is a 
smoothly bounded compact domain which is a strong deformation retract of $M$ and 
the given countable set $B=\{b_1,b_2,\ldots\}$ is infinite. Also, choose a finite set $C$ for the interpolation 
condition in {\rm (b)}, and assume that $C\subset \mathring K_0$ and $f(C)\cap B=\varnothing$. 
...
\item $K_{j-1}\cup\{a_j\}\subset \mathring K_j$ and $K_j$ is a strong deformation retract of $M$.
...
Assume that we have such a sequence in hand. Conditions {\rm (1$_j$)}, {\rm (2$_j$)}, {\rm (3$_j$)}, 
and \eqref{eq:cupK_jM} ensure that there is a limit holomorphic map $F=\lim_{j\to\infty}f_j:M\to X$ 
satisfying condition (a) in the statement of the theorem. Moreover, conditions {\rm (5$_j$)} 
and {\rm (4$_j$)} guarantee (b) and (c), respectively. So, the map $F$ satisfies the conclusion of 
the theorem. Note that the final statement can be granted as explained in Remark \ref{rem:transversality}; 
for this one has to take the number $\epsilon_j>0$ sufficiently small at each step of the inductive construction.

It thus remains to explain the induction. The basis is provided by $f_0=f$, $K_0=K$, $\epsilon_0=\epsilon/2$, 
and $a_0\in \mathring K_0$; note that conditions {\rm (2$_0$)} and {\rm (3$_0$)} are void. 
For the inductive step, fix $j\in\N$ and assume that we have objects $f_{j-1}$, $K_{j-1}$, $\epsilon_{j-1}$, 
and $a_{j-1}$ satisfying conditions {\rm (1$_{j-1}$)}, {\rm (4$_{j-1}$)}, and {\rm (5$_{j-1}$)}. 
Choose any $\epsilon_j>0$ meeting {\rm (2$_j$)}. If $b_j\in f_{j-1}(M)$ then we choose any point $a_j\in M$ 
with $f_{j-1}(a_j)=b_j$ and any smoothly bounded compact domain $K_j\subset M$ satisfying {\rm (1$_j$)}, 
...
$\gamma\cap \overline M=\{x\}$ and the intersection of $bM$ and $\gamma$ is transverse at $x$. 
Let $x'\in R\setminus\overline M$ denote the other endpoint of $\gamma$. 
Fix a point $y\in \gamma\setminus\{x,x'\}$ and extend $f_{j-1}$ to a continuous map 
$f_{j-1}:\overline M\cup\gamma\to X$ which is holomorphic on a neighbourhood of $\overline M$ 
and satisfies $f_{j-1}(y)=b_j$. Theorem \ref{th:Mergelyan} furnishes 
a holomorphic map $g:V\to X$ on an 
...
\end{enumerate}
For (ii) and (iv) take into account {\rm (4$_{j-1}$)} and {\rm (5$_{j-1}$)}; recall that 
$C\subset \mathring K_0\subset M$. Now, \cite[Theorem 6.7.1]{AlarconForstnericLopez2021} 
(see also \cite[Theorem 2.3]{ForstnericWold2009}) furnishes a conformal diffeomorphism 
$\phi: \overline M\to\phi(\overline M)\subset V$
such that %$\phi(x)=x'$, 
$\phi$ agrees with the identity map to a given order at the points in the finite set 
$C\cup\{a_0,\ldots,a_{j-1}\}\subset M$, $\phi$ is arbitrarily close to the identity map on $K_{j-1}$, and
\begin{equation}\label{eq:yphi(M)}
	y\in \phi(M).
\end{equation} 
In particular, $\phi$ can be chosen so that the map $f_j=g\circ\phi:\overline M\to X$ of class 
$\Ascr(\overline M)$ satisfies
\begin{enumerate}[\rm (I)]
...
\end{enumerate}
Moreover, by using Theorem \ref{th:Mergelyan} once again, 
we may assume that $f_j$ is holomorphic on $\overline M$. Finally, (II) enables us to choose a point 
$a_j\in M$ with $f_j(a_j)=b_j$, and then a smoothly bounded compact domain $K_j\subset M$ 
satisfying {\rm (1$_j$)}. It is then clear in view of these choices and conditions (i)--(iv) and (I)--(IV) 
that $f_j$, $K_j$, $\epsilon_j$ and $a_j$ satisfy the requirements in {\rm (1$_j$)}--{\rm (5$_j$)}. 
This closes the induction and completes the proof of the theorem; note that \eqref{eq:cupK_jM} 
is guaranteed since we are allowed to choose the compact domain $K_j\subset M$ as large as 
desired at each step of the inductive construction.
\end{proof}

%
% AA: I added the following
%
Combining the arguments used in \cite[proofs of Theorems 1.6 and 1.8]{AlarconForstneric2023Embedded} 
with the proof of Theorem \ref{th:dense} one may easily establish analogous hitting results for some 
classes of surfaces of infinite topology, at the cost of loosing the control on the complex structure. 
...
Let $X$ be a connected complex manifold and $B\subset X$ be a countable set. 
...
\item On every open orientable smooth surface $S$ there is a complex structure $J$ such that the open Riemann surface $M=(S,J)$ admits a holomorphic map $F:M\to X$ with $B\subset F(M)$.
% 
%\item If $R$ is a compact Riemann surface and $M=R\setminus\bigcup_{i=0}^\infty D_i$ is an open domain in $R$ whose complement is the union of countably many pairwise disjoint closed discs $D_i$, then there is a holomorphic map $F:M\to X$ with $B\subset F(M)$.
%
\end{enumerate}
...
In particular, in both cases (and up to a suitable choice of the Cantor set $C$ in assertion (i) and of 
the complex structure $J$ in assertion (ii)) there is a holomorphic map $M\to X$, which is an immersion 
if $\dim X>1$ and an injective immersion if $\dim X>2$, whose image is everywhere dense in $X$. 
...
The authors wish to thank Rafael Andrist, Fr\'ed\'eric Campana, Antoine Gournay,
Daniel Huybrechts, J\'anos Koll\'ar, Uro\v s Kuzman, Finnur L\'arusson,
and Thomas Peternell for consultations and helpful advice.  
...
\bibitem{AlarconForstneric2023Embedded}
A.~Alarc\'{o}n and F.~Forstneri\v{c}.
\newblock Embedded complex curves in the affine plane, 2023.
\newblock \url{https://arxiv.org/abs/2301.10304}.
...
C.~Araujo and J.~Koll{\'a}r.
\newblock Rational curves on varieties.
\newblock In {\em Higher dimensional varieties and rational points. Lectures of
  the summer school and conference, Budapest, Hungary, September 3--21, 2001},
  pages 13--68. Berlin: Springer; Budapest: J{\'a}nos Bolyai Mathematical
  Society, 2003.
...
\bibitem{Birkhoff1909}
G.~D. Birkhoff.
\newblock Singular points of ordinary linear differential equations.
\newblock {\em Trans. Am. Math. Soc.}, 10:436--470, 1909.

\bibitem{BoivinGauthier2001}
A.~Boivin and P.~M. Gauthier.
\newblock Holomorphic and harmonic approximation on {R}iemann surfaces.
\newblock In {\em Approximation, complex analysis, and potential theory
  ({M}ontreal, {QC}, 2000)}, volume~37 of {\em NATO Sci. Ser. II Math. Phys.
  Chem.}, pages 107--128. Kluwer Acad. Publ., Dordrecht, 2001.
...
\bibitem{CampanaWinkelmann2020}
F.~Campana and J.~Winkelmann.
\newblock Dense entire curves in rationally connected manifolds, 2019.
\newblock \url{https://arxiv.org/abs/1905.01104}.

\bibitem{CarlsonGriffiths1972}
J.~Carlson and P.~Griffiths.
\newblock A defect relation for equidimensional holomorphic mappings between
  algebraic varieties.
...
\newblock {\em Rev.\ Roumaine Math.\ Pures Appl., to appear}, 2023.
\newblock \url{http://arxiv.org/abs/2204.06841}.
...
\bibitem{Gournay2010X}
A.~Gournay.
\newblock Complex surfaces and interpolation on pseudo-holomorphic cylinders.
\newblock {\em arXiv e-prints}, 2010.
\newblock \url{https://arxiv.org/abs/1006.1775}.
...
\newblock {\em arXiv e-prints}, May 2020.
...
\bibitem{Kuzman2018}
U.~Kuzman.
\newblock On {Poletsky} theory of discs in compact manifolds.
\newblock {\em Proc. Am. Math. Soc.}, 146(11):4767--4774, 2018.
...
\bibitem{McDuffSalamon1994}
D.~McDuff and D.~Salamon.
\newblock {\em {$J$}-holomorphic curves and quantum cohomology}, volume~6 of
  {\em University Lecture Series}.
\newblock American Mathematical Society, Providence, RI, 1994.

\bibitem{McDuffSalamon2012}
D.~McDuff and D.~Salamon.
\newblock {\em {$J$}-holomorphic curves and symplectic topology}, volume~52 of
  {\em Amer. Math. Soc. Colloquium Publications}.
\newblock Amer. Math. Soc., Providence, RI, second edition, 2012.
...
\bibitem{Sikorav1994}
J.-C. Sikorav.
\newblock Some properties of holomorphic curves in almost complex manifolds.
\newblock In {\em Holomorphic curves in symplectic geometry}, volume 117 of
  {\em Progr. Math.}, pages 165--189. Birkh\"auser, Basel, 1994.
...
\newblock {\em Math. Res. Lett.}, 1(6):809--822, 1994.
