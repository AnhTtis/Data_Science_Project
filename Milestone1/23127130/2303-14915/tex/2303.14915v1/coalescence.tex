...
\keywords{ $A_\alpha$-spectrum, Coalescence, Wiener index, hyper-Wiener index, Zagreb index}
...
Conversely, if $G_1\circ_kG_2$ is Hamiltonian, then there exists a Hamiltonian cycle $u_1u_2\cdots w_1w_2\cdots w_m v_{k+1}v_{k+2}\cdots v_{n_{2}}w_{m+1}\cdots w_{k}\cdots u_{n_1}u_1$.
...
\section{Topological indices of \texorpdfstring{$k$}{}-coalescence of graphs}\label{top}
In this section, some topological indices of $k$-coalescence of graphs are computed. We calculate the Wiener index, hyper-Wiener and Zagreb indices of the Lollipop and Dumbbell graphs using the results. Also, as an application, we compute the Wiener, hyper-Wiener and Zagreb indices of the organic compound 1,2-dicyclohexylethane(\ce{C_{14}H_{26}}).

\begin{proposition}
Wiener index of $G_1\circ_kG_2$ is 
$$W(G_1\circ_kG_2)=W(G_1)+W(G_2).$$
\end{proposition}

\begin{proof}
From Definition \ref{W},
\begin{align*}W(G_1\circ_k&G_2)\\
=&\displaystyle\frac{1}{2}\left(\sum_{v\in V(G_1)}d_{G_1}(v)-\sum_{v\in \mathcal{Q}}d_{G_1}(v)+\sum_{v\in V(G_2)}d_{G_2}(v)-\sum_{v\in \mathcal{Q}}d_{G_2}(v)+\sum_{v\in \mathcal{Q}}\left(d_{G_1}(v)+d_{G_2}(v)\right)\right)\\
=&W(G_1)+W(G_2).
\end{align*}
\end{proof}
\begin{remark}
    ...
    \end{cases}$ and Wiener index of path is $W(P_n)=\frac{n(n^2-1)}{6}$. Thus the Wiener index of Lollipop graph $L(m,n-1)$ is  $$W(L(m,n-1))=\begin{cases}
        \frac{m^3}{8}+\frac{n(n^2-1)}{6} & \text{if $m$ is even}\\
        \frac{m(m^2-1)}{8}+\frac{n(n^2-1)}{6} & \text{if $m$ is odd.}
    \end{cases}$$
\end{remark}
    ...
    The Wiener index of Dumbbell graph $D_{m,m,n-3}$ is 
     $$W(D_{m,m,n-3})=\begin{cases}
        \frac{m^2}{4}+\frac{n(n^2-1)}{6} & \text{if $m$ is even}\\
        \frac{m(m^2-1)}{4}+\frac{n(n^2-1)}{6} & \text{if $m$ is odd.}
    \end{cases}$$
\end{remark}
\begin{figure}[H]
    \centering
...
\begin{example}
    The molecular graph of 1,2-dicyclohexylethane(\ce{C_{14}H_{26}}) is isomorphic to $D_{6,6,1}$. Therefore the Wiener index of 1,2-dicyclohexylethane(\ce{C_{14}H_{26}}) is 64.
    \begin{figure}[H]
        \centering
...
\begin{proposition}
Hyper-Wiener index of $G_1\circ_kG_2$ is 
$$WW(G_1\circ_kG_2)=WW(G_1)+WW(G_2)+\sum_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v).$$
\end{proposition}

\begin{proof}
From Definition \ref{WW},
\begin{align*}
    WW(G_1\circ_kG_2)&=\frac{1}{2}(W(G_1)+W(G_2))\\
    &\hspace{-2cm}+\frac{1}{2}\left(\sum_{v\in V(G_1)}d_{G_1}^2(v)-\sum_{v\in \mathcal{Q}}d_{G_1}^2(v)+\sum_{v\in V(G_2)}d_{G_2}^2(v)-\sum_{v\in \mathcal{Q}}d_{G_2}^2(v)+\sum_{v\in \mathcal{Q}}\left(d_{G_1}(v)+d_{G_2}(v)\right)^2\right)\\
    &=WW(G_1)+WW(G_2)+\sum_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v).
\end{align*}
\end{proof}

\begin{remark}
    The hyper-Wiener index of of Lollipop graph $L(m,n-1)$ is  $$WW(L(m,n-1))=\begin{cases}
        \frac{m^2(m+1)(m+2)}{48}+\frac{n^4+2n^3-n^2-2n}{24}+\frac{m^2n(n-1)}{8} & \text{if $m$ is even}\\
        \frac{m(m^2-1)(m+3)}{48}+\frac{n^4+2n^3-n^2-2n}{24}+\frac{(m-1)^2n(n-1)}{8} & \text{if $m$ is odd.}
    \end{cases}$$
    \end{remark}

    \begin{remark}
    The hyper-Wiener index of Dumbbell graph $D_{m,m,n-3}$ is 
     $$WW(D_{m,m,n-3})=\begin{cases}
        \frac{m^2(m+1)(m+2)}{24}+\frac{n^4+2n^3-n^2-2n}{24}+\frac{m^4+4m^2(m+n-1)(n-1)}{16} & \text{if $m$ is even}\\
        \frac{m(m^2-1)(m+3)}{24}+\frac{n^4+2n^3-n^2-2n}{24}+\frac{(m-1)^4+4(m-1)^2(m+n-1)(n-1)}{16} & \text{if $m$ is odd.}
    \end{cases}$$
\end{remark}

\begin{example}
    The hyper-Wiener index of 1,2-dicyclohexylethane is 423.
\end{example}

\begin{proposition}
The forgotten topological index of $G_1\circ_kG_2$ is 
$$F(G_1\circ_kG_2)=F(G_1)+F(G_2)+3\sum_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v)(d_{G_1}(v)+d_{G_2}(v)).$$
\end{proposition}

\begin{proof}
From Definition \ref{F},
\begin{align*}
F(G_1\circ_kG_2)=&\sum_{v\in V(G_1)}d_{G_1}^3(v)-\sum_{v\in \mathcal{Q}}d_{G_1}^3(v)+\sum_{v\in V(G_2)}d_{G_2}^3(v)-\sum_{v\in \mathcal{Q}}d_{G_2}^3(v)+\sum_{v\in \mathcal{Q}}\left(d_{G_1}(v)+d_{G_2}(v)\right)^3\\
=&F(G_1)+F(G_2)+3\sum_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v)(d_{G_1}(v)+d_{G_2}(v)).
\end{align*}
\end{proof}

\begin{proposition}
First Zagreb index of $G_1\circ_kG_2$ is 
$$M_1(G_1\circ_kG_2)=M_1(G_1)+M_1(G_2)+2\sum_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v).$$
\end{proposition}

\begin{proof}
From Definition \ref{M1},
\begin{align*}
M_1(G_1\circ_kG_2)&=\sum_{v\in V(G_1)}d_{G_1}^2(v)-\sum_{v\in \mathcal{Q}}d_{G_1}^2(v)+\sum_{v\in V(G_2)}d_{G_2}^2(v)-\sum_{v\in \mathcal{Q}}d_{G_2}^2(v)+\sum_{v\in \mathcal{Q}}\left(d_{G_1}(v)+d_{G_2}(v)\right)^2\\
&=M_1(G_1)+M_1(G_2)+2\sum_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v).
\end{align*}
\end{proof}

\begin{remark}
    The first Zagreb index of of Lollipop graph $L(m,n-1)$ is  $$M_1(L(m,n-1))=\begin{cases}
       4(m+n)-6+\frac{m^2n(n-1)}{4}  & \text{if $m$ is even}\\
       4(m+n)-6+\frac{(m-1)^2n(n-1)}{4}   & \text{if $m$ is odd.}
    \end{cases}$$
    \end{remark}

    \begin{remark}
    The first Zagreb index of Dumbbell graph $D_{m,m,n-3}$ is 
     $$M_1(D_{m,m,n-3})=\begin{cases}
        4(2m+n)-6+\frac{m^2n(n-1)}{4}+\frac{m^4+4m^2(m+n-1)(n-1)}{16} & \text{if $m$ is even}\\
        4(2m+n)-6+\frac{(m-1)^2n(n-1)}{4}+\frac{(m-1)^4+4(m-1)^2(m+n-1)(n-1)}{16} & \text{if $m$ is odd.}
    \end{cases}$$
\end{remark}

\begin{example}
    The first Zagreb index of 1,2-dicyclohexylethane is 490.
\end{example}

\begin{proposition}
Narumi-Katayama index of $G_1\circ_kG_2$ is 
$$NK(G_1\circ_kG_2)=NK(G_1)NK(G_2)\prod_{v\in\mathcal{Q}}\frac{d_{G_1}(v)+d_{G_2}(v)}{d_{G_1}(v)d_{G_2}(v)}.$$
\end{proposition}

\begin{proof}
From Definition \ref{NK},
\begin{align*}
NK(G_1\circ_kG_2)&=\frac{\displaystyle\prod_{v\in V(G_1)}d_{G_1}(v)\prod_{v\in V(G_2)}d_{G_2}(v)}{\displaystyle\prod_{v\in\mathcal{Q}}d_{G_1}(v)d_{G_2}(v)}\prod_{v\in\mathcal{Q}}\left(d_{G_1}(v)+d_{G_2}(v)\right)\\
&=NK(G_1)NK(G_2)\prod_{v\in\mathcal{Q}}\frac{d_{G_1}(v)+d_{G_2}(v)}{d_{G_1}(v)d_{G_2}(v)}.
\end{align*}
\end{proof}

\begin{proposition}
The first modified Zagreb index of $G_1\circ_kG_2$ is 
$$ ^mM_1(G_1\circ_kG_2)=^mM_1(G_1)+^mM_1(G_2)-\sum_{v\in\mathcal{Q}}\frac{2d_{G_1}(v)d_{G_2}(v)}{d_{G_1}(v)d_{G_2}(v)(d_{G_1}(v)+d_{G_2}(v))}.$$
\end{proposition}
...
\begin{align*}
    ^mM_1(G_1&\circ_kG_2)\\
    &=\sum_{v\in V(G_1)}\frac{1}{d_{G_1}^2(v)}-\sum_{v\in \mathcal{Q}}\frac{1}{d_{G_1}^2(v)}+\sum_{v\in V(G_2)}\frac{1}{d_{G_2}^2(v)}-\sum_{v\in \mathcal{Q}}\frac{1}{d_{G_2}^2(v)}+\sum_{v\in \mathcal{Q}}\frac{1}{\left(d_{G_1}(v)+d_{G_2}(v)\right)^2}\\
    &=^mM_1(G_1)+^mM_1(G_2)-\sum_{v\in\mathcal{Q}}\frac{2d_{G_1}(v)d_{G_2}(v)}{d_{G_1}(v)d_{G_2}(v)(d_{G_1}(v)+d_{G_2}(v))}.
\end{align*}
\end{proof}

\section{Conclusion}
This paper estimates some structural properties of a non-regular graph obtained from the $k$-coalescence of two graphs. Also, the $A_\alpha$-characteristic polynomial of $k$-coalescence of two graphs is determined. Moreover, the $A_\alpha$-spectrum and $A_\alpha$-energy of $k$-coalescence of two complete graphs are computed. In addition, some topological indices of $k$-coalescence of two graphs are estimated. The Wiener, hyper-Wiener and Zagreb indices of Lollipop and Dumbbell graphs are derived as an application. Using these, the Wiener, hyper-Wiener and Zagreb indices of the organic compound 1,2-dicyclohexylethane(\ce{C_{14}H_{26}}) are calculated.
