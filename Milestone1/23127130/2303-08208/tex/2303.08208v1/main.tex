...
%MSC:n luokittelu järjestyksessä: Radon transform, Geodesics in global differential geometry, Integral geometry, Global analysis on manifolds
...
The manifold~$M$ comes equipped with a~$C^{1,1}$ regular Riemannian metric~$g$ i.e.~$g$ is continuously differentiable and the derivative is Lipschitz.
...
The X-ray transform of tensor fields is defined section in~\ref{subsubsec:transform}.
...
%Upgrading the regularity of~$p$, a posteriori, using the equation~$f = \sigma\nabla p$ is not possible, at least not naively.
%When~$g \in C^{1,1}$, the Christoffel symbols are Lipschitz.
%Thus in principle, if~$h$ is a solution to~$\sigma\nabla h = 0$ then the derivative of~$h$ cannot be more regular than Lipschitz, which means that~$h$ cannot be more regular than~$g$.
%However, it is possible that~$h \in C^{1,1}(M)$ and~$\sigma\nabla h \in C^{1,1}(M)$, but~$h \notin C^{2,1}(M)$.
%By the metric compatibility of the Levi-Civita connection~$\nabla g = 0$ and thus the metric tensor~$g$ itself is an example of such tensor field.
%Per this example, one must exercise caution with the equation~$f = \sigma\nabla p$ and regularity of its pieces, when regularity over~$C^{1,1}$ is not accessible.
...
\begin{enumerate}
    ...
    %, could possibly be achieved applying boundary ellipticity of the normal operator of the X-ray transform to prove~$f|_{\partial M} \in C^{2,1}(\partial M)$, if one is able to formulate and prove such results and disregards the issue of regularity above~$C^{1,1}$ (see section~\ref{subsec:tensors}). We will not explore ellipticity or its consequences for the normal operator in this article.
...
These include reflecting obstacles inside the manifold~\cite{ISBRTRSCO,IPBRTORO}, attenuations and Higgs fields~\cite{SUARTSS,PSUARTCHF,GPSXRTCNC}, manifolds with magnetic flows~\cite{DPSUBRPPMF,AinsworthAMRTS,MPIPGD} and non-Abelian variations~\cite{FUXRTNACTD,PSNAXRTS,FNPCINNXT}.
...
As the level set $F^{-1}(1)$ of the~$C^{1,1}$ map~$F \colon TM \to \R$ defined by~$F(x,v) = g_x(v,v)$ the unit sphere bundle is a~$C^{1,1}$ submanifold of~$TM$.
The boundary
\begin{equation}
\partial(SM)
\coloneqq
...
Given~$z \in SM$, there is a unique unit speed geodesic~$\gamma_z$ corresponding to the initial condition~$z$.
...
\end{equation}
for all~$(x,v) \in SM$.
The X-ray transform of~$f$ is the restriction of the integral function to the inward pointing part of the boundary~$\partial(SM)$ i.e.~$If \coloneqq u^f|_{\doo{in}(SM)}$.
...
The vertical Laplacian on the sphere bundle is $\lap{v} \coloneqq -\dive{v}\grad{v}$; see Appendix~\ref{app:diff-operators} for details on the differential operators.
...
Here~$R$ is the Riemannian curvature tensor of~$g$ and it is interpreted as an~$L^\infty$ tensor field, which is reasonable since~$g \in C^{1,1} = W^{2,\infty}$.
The curvature tensor~$R \colon L^\infty(N) \to L^\infty(N)$ acts on sections of the bundle~$N$ by $R(x,v)W(x,v) \coloneqq R(W(x,v),v)v$ producing again sections of the bundle~$N$.
...
We define the Sobolev spaces~$H^1_{(0)}(SM)$ and~$H^1_{(0)}(N,X)$ respectively defined as completions of~$C^{1}_{(0)}(SM)$ and~$C^{1}_{(0)}(N)$ with respect to the norms
\begin{equation}
\begin{split}
\norm{u}^2_{H^1(SM)}
&\coloneqq
...
\end{split}
\end{equation}
The optional subindices~$0$ indicate zero boundary values.
...
%The geodesic vector field maps from spherical harmonic degree~$k$ into degrees~$k-1$ and~$k+1$ and thus we can write~$X = X_- + X_+$.
For all $m \in \N$ there are operators $X_\pm \colon \OS{1}{\infty}{m} \to \OS{0}{\infty}{m \pm 1}$
%\begin{equation}
%X_\pm
%\colon
%\OS{1}{\infty}{m}
...
%\OS{0}{\infty}{m \pm 1}
%\end{equation}
with the convention that $\OS{0}{\infty}{-1} = 0$ so that $X = X_+ + X_-$. These mapping properties and validity of this decomposition in low regularity are addressed in proposition~\ref{prop:x-mapping-property}.

\subsubsection{Simple~$C^{1,1}$ manifolds}

%In~\cite[Lemma 11]{IKPIXRTMLR}, the authors proved that there are no conjugate points on a Riemannian manifold~$(M,g)$ if the \emph{index form}~$Q$ \emph{of the entire manifold}~$(M,g)$ is positive.
%This index form is a quadratic form defined for~$W \in H^1_0(N,X)$ by
%\begin{equation}
%Q(W)
%\coloneqq
%\norm{XW}^2_{L^2(N)}
...
\iip{RW}{W}_{L^2(N)}.
\end{equation}
In~\cite[Lemma 11]{IKPIXRTMLR}, the authors proved that there are no conjugate points on a Riemannian manifold~$(M,g)$, $g \in C^\infty$, if the global index form~$Q$ of~$(M,g)$ is positive definite.

We conclude this subsection by recalling a definition of a simple manifold in the case~$g \in C^{1,1}$.
The definition is equivalent to the definition of traditional simple manifold when~$g \in C^\infty$~\cite{IKPIXRTMLR}.
...
By boundary determination lemma~\ref{lma:boundary-determination} there is a symmetric~$(m-1)$-tensor field~$p_0 \in C^{1,1}(M)$ so that~$p_0|_{\partial M} = 0$ and~$f|_{\partial M} = \sigma\nabla p_0|_{\partial M}$.
...
\end{equation}
where the summation is over all permuations~$\pi$ of~$\{1,\dots,m\}$. Note that since $\norm{\sigma \nabla f}_{L^2} \le \norm{\nabla f}_{L^2}$ the symmetrized covariant derivative is bounded between Sobolev spaces.
%{Oleellinen väite on kai seuraava: Olkoot $z_1,\dots,z_k\in\R$ ja $\bar z$ niiden keskiarvo. Jos valitaan $\hat z=(z_1-\bar z,\dots,z_k-\bar z)$, niin $\abs{\hat z}\leq\abs{z}$. Tensorisovelluksessa nämä luvut olisivat siis saman tensorikentän niitä komponentteja yhdessä pisteessä, joiden indeksit ovat permutaatiota vaille samat. Tämä on todennäköisesti helpointa nähdä tutkimalla toisen asteen polynomia $p(x)=\abs{(z_1-x,\dots,z_k-x)}^2$, jonka minimin pitäisi löytyä kohdasta $x=\bar z$. --- Päivitys: Joo, näin tosiaan menee helposti. Ja pitää ehkä muistaa laskea komponentit sopivasti monikerroittain jos on toistettuja indeksejä.}
...
%%%%
%%%%
%\subsection{The unit sphere bundle}

%Let~$M$ be a compact smooth manifold with a smooth boundary and let~$g$ be a~$C^{1,1}$ Riemannian metric on~$M$. We denote by~$TM$ the tangent bundle of~$M$.
%Elements of the tangent bundle are the tangent vectors~$(x,v) \in TM$, which we sometimes write as~$v \in T_xM$ if convenient.
%Consider the function~$F \colon TM \to \R$ defined by $F(x,v) = g_x(v,v)$.
%The (unit) sphere bundle~$SM$ is defined to be~$F^{-1}(1)$.
%The sphere bundle is a subbundle of~$TM$, but since~$F$ is only a~$C^{1,1}$ map,~$SM$ is only a~$C^{1,1}$ submanifold of~$TM$.

%Let~$\nu$ be the inward unit normal vector field to~$\partial M$.
%The boundary~$\partial(SM)$ is a disjoint union of tangential directions~$\partial_0(SM)$ and inwards and outwards pointing parts defined as
%\begin{equation}
%\doo{in}(SM)
%\coloneqq
%\{\,
...
Definitions of standard Sobolev spaces on~$SM$ are available in appendix~\ref{app:sobolev}, and we only define Sobolev spaces for scalar functions on~$SM$ of different vertical and horizontal indices.
...
\begin{proof}
See appendix~\ref{app:commutators}.
\end{proof}
...
%In this subsection we recall the well know fact that functions on the unit sphere bundle can be fiberwise decomposed into vertical Fourier modes.
%Also, we recall the identification of trace-free symmetric tensor fields and spherical harmonics i.e. the vertical Fourier modes.
In this subsection we recall the identification of trace-free symmetric tensor fields and spherical harmonics i.e. the vertical Fourier modes.
We state and prove proposition~\ref{prop:identification} in order to emphasis what changes in the these well known results when applied to a case of non-smooth Riemannian metrics.
...
%Let~$M$ be a compact smooth Riemannian manifold with a smooth boundary and let~$g$ be a~$C^{1,1}$ Riemannian metric on~$M$.
%Each function~$u \in L^2(SM)$ has a unique~$L^2$-orthogonal decomposition
%\begin{equation}
%u = \sum_{k=0}^\infty u_k.
%\end{equation}
%Here~$\lap{v} u_k = k(k+n-2)u_k$ for all~$k \in \N$ and we call~$u_k$ the~$k$th spherical harmonic component of~$u$.
%For $k \in \{0,1\}$, $l \in \N$ and $\alpha,\beta \in [0,1]$ we let
%\begin{equation}
%\OS{k,\alpha}{l,\beta}{m}
%\coloneqq
%\{\,
%u \in \C{k,\alpha}{l,\beta}{SM}
%\,:\,
%\lap{v} u
%=
%m(m+n-2)u
...
%Let~$M$ be a compact smooth Riemannian manifold with a smooth boundary and let~$g$ be a~$C^{1,1}$ Riemannian metric on~$M$.
%Let~$f$ be a symmetric~$m$-tensor field on~$M$.
%Given~$x \in M$ there is a corresponding function~$\mu_xf$ on~$T_xM$ defined by the formula~$\mu_xf(v) = f_x(v,\dots,v)$.
%We denote the restriction of~$\mu_xf$ to~$S_xM$ by~$\lambda_xf$.
%The function~$\mu_xf$ can be promoted to a function on the level of bundles.
%We define~$\mu f \colon TM \to \R$ by~$\mu f(x,v) = \mu_xf(v)$.
%The restriction of~$\mu f$ to~$SM$ is denoted by~$\lambda f$.
...
We prove that lemma~\ref{lma:boundary-determination} follows from the local construction by a partitions of unity argument.
...
\begin{enumerate}
    \item \label{item:constr-p-1} We prove $f_{j_1\cdots j_m}(\hat x,x^n) = 0$ in the coordinates in~$W$ when~$j_1,\dots,j_m \in \{1,\dots,n-1\}$.
    
    \item \label{item:constr-p-2} We verify that~$(\sigma\nabla p)_{j_1\dots j_m}|_{x^n = 0} = f_{j_1\dots j_m}$ in the coordinates in~$W$.
\end{enumerate}
...
By the product rule we have~$\nabla(\psi_ip_i) = \nabla \psi_i \otimes p_i + \psi_i(\nabla p_i)$ for all~$i$. Since~$\nabla \psi_i$ is a one-form,~$p_i$ is symmetric and symmetrization does not affect scalar functions, we have
\begin{equation}
\sigma\nabla p
=
\sum_{i = 0}^n
...
Next we prove lemma~\ref{lma:e-ae} i.e. we prove that if a Lipschitz function~$u$ on~$SM$ arising from of tensor field~$-p$ satisfies the transport equation~$Xu = -f$, then~$\sigma \nabla p = f$ holds pointwise almost everywhere.
...
%This section concludes the proofs of lemmas in section~\ref{sec:proof-of-theorem}.
%We show that the~$L^2$-estimate in lemma~\ref{lma:l2-estimate} follows from the Pestov identity by tools we have developed so far.
%Additionally, we prove two Friedrich's type estimates (lemma~\ref{lma:friedrichs}) on the unit sphere bundle, which combined with the Pestov identity lead to the proof of lemma~\ref{lma:injectivity-xplus}.
%The proofs of the Friedrich's type estimates rely on Santaló's formula in low regularity for which we give a proof in lemma~\ref{lma:santalo}.
...
It was proved that in~\cite[Lemma 9]{IKPIXRTMLR} that the Pestov identity~\eqref{eq:pestov} holds for this class of functions on simple~$C^{1,1}$ manifolds.
\end{proof}
...
If~$\omega$ is a differential~$k$-form on~$SM$, then denote by~$i_X\omega$ the contraction of~$\omega$ with the geodesic vector field~$X$ i.e. for any vector fields~$Y_1,\dots,Y_{k-1}$ on~$SM$, we let
...
\begin{proof}
First, we prove the inequality for functions. By densitiy is enough to consider the case~$u \in C^1_0(SM)$. By Santaló's formula (lemma~\ref{lma:santalo}) we can write
\begin{equation}
\norm{Xu}^2_{L^2(SM)}
=
\int_{\doo{in}(SM)}
...
%%%%
%%%%
%\section{Simple manifolds with non-positive sectional curvature}

%Let~$M$ be a compact smooth manifold with a smooth boundary and let~$g$ be a~$C^{1,1}$ Riemannian metric on~$M$.
%In this section we prove a~$C^{1,1}$ analog of the smooth result that all compact smooth Riemannian manifold with smooth strictly convex boundaries and non-positive sectional curvature are simple.
...
%Particularly, lemma~\ref{lma:non-pos-curvature-simple} implies that any~$C^{1,1}$ Riemannian metric on the closed Euclidean ball~$M$ is~$C^{1,1}$ simple, if its sectional curvature is almost everywhere non-positive and it satisfies defining conditions~\ref{a2} and~\ref{a3} of a simple~$C^{1,1}$ manifold.

\begin{proposition}
%\label{prop:non-pos-curvature-simple}
...
\section{Differential operators on the unit sphere bundle}
\label{app:diff-operators}

This appendix complements the preliminaries in section~\ref{sec:preliminaries} by providing coordinate formulas of the various differential operators on~$SM$, definitions of the standard Sobolev spaces on~$SM$ and a proof of proposition~\ref{prop:commutator-formulas} concerning commutators of the differential operators.

\subsection{Coordinate formulas}

Let $x$ be some coordinates on $M$ and $(x,v)$ the coordinates on $TM$ and $(x,v,X,V)$ the coordinates on $TTM$.
We denote
\begin{equation}
\delta_{x^i}
=
\partial_{x^i}
-\Gamma^l_{\ ik}v^k\partial_{v^l}
.
\end{equation}
The vectors~$\delta_{x^i}$ form a basis for the horizontal subbundle of~$TTM$
and the vectors~$\partial_{v^i}$ form a basis for the vertical bundle of~$TTM$.

Recall from section~\ref{subsubsec:hv} the subbundle~$N$ of~$\pi^*TM$, where $\pi\colon SM\to M$ is the unit sphere bundle.

The horizontal and vertical subbundles~$\H$ and~$\V$ of~$TSM$ are canonically identified with~$N$ via the maps $H^j\delta_{x^j} \mapsto H^j\partial_{x^j}$ and $V^k\partial_{v^k} \mapsto V^k\partial_{x^k}$.
...
The actions of vector fields on~$SM$ on functions~$u$ in~$C^1(SM)$ can be regarded as actions of vector fields on~$TM$ via the scaling map~$s \colon TM\setminus 0 \to SM$ defined by $s(x,v) = (x,v\abs{v}_g^{-1})$.
More precisely, if~$u \in C^1(SM)$, then we have the basic derivatives~$\delta_j$ and~$\partial_k$ defined by
\begin{equation}
\delta_ju = \delta_{x^j}(u \circ s)|_{SM},
\quad\text{and}\quad
\partial_ku = \partial_{v^k}(u \circ s)|_{SM}.
\end{equation}

The usual differential operators that act on functions on the unit sphere bundle and on sections of the bundle~$N$.
The geodesic vector field~$X$ is a differential operator that acts both on functions on~$SM$ and sections of the bundle~$N$.
The actions on a scalar function~$u$ and on a section~$W$ are
\begin{equation}
Xu
=
v^j\delta_ju
...
(XW^j)\partial_{x^j}
+
\Gamma^l_{\ jk}v^jW^k\partial_{x^l}.
\end{equation}
Vertical and horizontal gradients are differential operators defined respectively by
\begin{equation}
\grad{v}u
=
(g^{ji}\partial_iu)\partial_{x^j}
\quad\text{and}\quad
\grad{h}u
=
(g^{ji}\delta_iu - (Xu)v^j)\partial_{x^j}.
\end{equation}
The formal adjoint operators of~$-\grad{v}$ and~$-\grad{h}$ are the vertical and horizontal divergences
\begin{equation}
\dive{v}(W) = \partial_jW^j
\quad\text{and}\quad
\dive{h}(W) = (\delta_j + \Gamma^i_{\ ji})W^j.
\end{equation}

\subsection{Standard Sobolev spaces}
\label{app:sobolev}

Let~$M$ be a compact smooth manifold with a smooth boundary and let~$g$ be a~$C^{1,1}$ Riemannian metric on~$M$.
For two continuous functions~$u$ and~$w$ on~$SM$ define
\begin{equation}
\label{eq:inner-prod-functions}
\iip{u}{w}_{L^2(SM)}
\coloneqq
\int_{SM}
uw
\,\d\Sigma_g.
\end{equation}
Formula~\eqref{eq:inner-prod-functions} defines an inner product on~$C(SM)$ and we let~$L^2(SM)$ be the completion of~$C(SM)$ with respect to the norm induced by~$\iip{\cdot}{\cdot}_{L^2(SM)}$.
For two continuous sections~$W$ and~$Z$ of the bundle~$N$ we let
\begin{equation}
\label{eq:inner-prod-sections}
\iip{W}{Z}_{L^2(N)}
\coloneqq
\int_{SM}
g_{jk}W^jZ^k
\,\d\Sigma_g
\end{equation}
and we define~$L^2(N)$ to be the completion of~$C(N)$ with respect to the norm induced by the inner product~\eqref{eq:inner-prod-sections}.

The vertical and horizontal divergences are the formal adjoints of the vertical and horizontal gradients with respect to these inner products.
Let~$u \in C^1(SM)$ and~$W \in C^1(SM)$. If either~$u$ or~$W$ vanishes on the boundary~$\partial(SM)$, then
\begin{equation}
\iip{\grad{v}u}{W}_{L^2}
=
-\iip{u}{\dive{v}W}_{L^2}
\,\text{and}\,
\iip{\grad{h}u}{W}_{L^2}
=
-\iip{u}{\dive{h}W}_{L^2}.
\end{equation}

\subsection{Proof of commmutator formulas}
\label{app:commutators}

We prove the commutator formulas in proposition~\ref{prop:commutator-formulas}.

\begin{proof}[Proof of proposition~\ref{prop:commutator-formulas}]
By density it is enough to prove the claimed formulas for~$u \in \C{1}{2}{SM}$.
Then the commutators are well defined as a classical differential operators.
As in the smooth case (see~\cite[Appendix A]{PSUIDBTTT}) we can compute that
\begin{equation}
\label{eq:proof-of-commutators}
[\partial_j,v^k]u
=
\delta_j^ku
-
g_{jl}v^lv^ku
\quad\text{and}\quad
[\delta_j,g^{lk}\partial_k]u
=
-\Gamma^l_{\ jm}g^{mk}\partial_ku.
\end{equation}
Using the equation~\eqref{eq:proof-of-commutators} we find that (for more details see~\cite[Appendix A]{PSUIDBTTT})
\begin{equation}
[X,\grad{v}]
=
-\grad{h}u
+
v^j
([\delta_j,g^{lm}\partial_m]u
+
\Gamma^l_{\ jk}g^{km}\partial_mu)
\partial_{x^l}
=
-\grad{h}u.
\end{equation}
The claimed identity $[X,\dive{v}] = -\dive{h}$ follows from $[X,\grad{v}] = -\grad{h}$ by taking adjoints.

Similarly (again, see~\cite[Appendix A]{PSUIDBTTT} for details), we can compute that
\begin{equation}
\dive{h}\grad{v}u
-
\dive{v}\grad{h}u
=
[\delta_k,g^{kl}\partial_l]u
+
g^{kl}\Gamma^j_{\ kj}\partial_lu
+(Xu)(\partial_kv^k)
=
(n-1)Xu.
\end{equation}
Again, direct computation in local coordinates is valid since the second order derivatives of~$u$ only contain one horizontal and one vertical derivative.

The formula for~$[X,\lap{v}]$ follows from other formulas since
\begin{equation}
\begin{split}
[X,\lap{v}]u
&=
...
\\
&=
2\dive{v}\grad{h}u + (n-1)Xu,
\end{split}
\end{equation}
where each step is valid since~$u \in \C{1}{2}{SM}$.
\end{proof}
...
First, we prove item~\ref{item:constr-p-1}.
To prove that~$f_{j_1\cdots j_m}(\hat x,x^n) = 0$ for all~$j_1,\dots,j_m \in \{1,\dots,n-1\}$ it is enough to prove that~$f_x(v,\dots,v) = 0$ for all tangential directions~$v \in S_x(\partial M)$, where~$x \in W$.
Given~$v \in S_x(\partial M)$ we choose a sequence of vectors~$v_k \in S_xM$ so that~$\tau(x,v_k) > 0$,~$\tau(x,v_k) \to 0$ and~$v_k \to v$ as~$k \to \infty$.
The vectors~$v_k$ exist by~$C^{1,1}$ simplicity as explained in~\cite[Lemma 23]{IKPIXRTMLR}.
Since~$If = 0$ and the lengths of the geodesics corresponding to~$(x,v_k) \in SM$ become arbitrarily short we see that
\begin{equation}
f_x(v,\dots,v)
=
\lim_{k \to \infty}
...
0,
\end{equation}
which concludes to proof of item~\ref{item:constr-p-1}.

We proceed proving item~\ref{item:constr-p-2}.
...
There are~$(m-l)(m-1)!$ permutations~$\pi$ of~$\{1,\dots,m\}$ so that~$j_{\pi(1)} = n$, when no restrictions are set on the remaining~$j_{\pi(2)},\dots,j_{\pi(m)}$.
...
%&=
%\frac{m-l}{m}\frac{m}{m-l}f_{j_1\cdots j_ln \cdots n}(\hat x,0)
%+
%\frac{1}{m}
%\sum_{s=1}^l
%0
...
\end{equation}
We have finally used item~\ref{item:constr-p-1} of the proof, where we proved that~$f_x(v,\dots,v) = 0$ for all~$v \in S_x(\partial M)$ with~$x \in W$.
