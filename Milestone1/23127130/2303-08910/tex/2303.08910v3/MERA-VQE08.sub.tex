\documentclass[english,10pt,letterpaper,twocolumn,superscriptaddress,floatfix,allowfontchangeintitle,accepted=2025-02-03]{quantumarticle}
\pdfoutput=1
...
\section{\texorpdfstring{\!\!\!Initialization and convergence schemes}{Initialization and convergence schemes}}\label{sec:converge}
...
\end{equation}
where $U_{\tau,k}$ denotes one of the MERA tensors in layer $\tau$. The computation time follows a power law $\O(\chi^r)$ with rather large exponents $r$ especially in $D\geq 2$ spatial dimensions. This holds regardless of whether the optimization is based on Riemannian gradients \cite{Hauru2021-10,Miao2021_08} or the traditional fixed-point-iteration SVD scheme \cite{Evenbly2009-79}. The costs for EEG-based TMERA optimizations depend additionally on the number of Trotter steps $t$ needed to achieve a certain energy accuracy \eqref{eq:accuracy}. In the VMC-based fMERA and TMERA methods, energy gradients \eqref{eq:Egrad} are not determined exactly but sampled stochastically, where, in analogy to the VQE scheme, one propagates pure causal-cone states in the MERA preparation direction and applies projective measurements to the sites that leave the causal cone in the layer transitions \cite{Ferris2012-85,Barthel2025-111}. Hence, the associated cost attains an additional factor $1/\epsilon^2$ corresponding to the required number of samples.
...
\begin{figure*}[t]
	...
	\includegraphics[width=0.3055\textwidth]{fig005}&
	\includegraphics[width=0.3055\textwidth]{fig006}&
	\includegraphics[width=0.3055\textwidth]{fig007}\\[0.6em]
	...
	\includegraphics[width=0.3055\textwidth]{fig008}&
	\includegraphics[width=0.3055\textwidth]{fig009}&
	\includegraphics[width=0.3055\textwidth]{fig010}\\
	\end{tabular}
...
\begin{figure}[t]
	...
	The three lower curves show the maximum, mean value, and minimum of the energy density accuracy for a few hundred randomly initialized TMERA optimizations. The displayed average two-qubit rotation angle is obtained from the converged lowest-energy TMERA.
	}
\end{figure}
...
\begin{enumerate}[label=(\alph*)]
 ...
 \item Arguments by Kim and Swingle \cite{Kim2017_11} also apply to TMERA and suggest their resilience against noise. This is due to the fact that MERA layer transition channels are generally gapped such that noise does not proliferate but gets damped when transitioning to the next layer.
...
First experimental implementations of MERA on ion-trap quantum computers have demonstrated critical groundstate correlation functions \cite{Haghshenas2023_05} as well as a quantum phase transition and the associated change from area-law to log-area-law entanglement scaling \cite{Miao2024_12}.
...
Q. Mortier, L. Devos, L. Burgelman, B. Vanhecke, N. Bultinck, F. Verstraete, J.
  ...
  \href{https://doi.org/10.21468/SciPostPhys.18.1.012} {SciPost Phys. {\bf 18},
   012  (2025)}.
...
T. Barthel and Q. Miao, {\em Scaling of contraction costs for entanglement
  ...
  Monte Carlo}, \href{https://doi.org/10.1103/PhysRevB.111.045104} {Phys. Rev.
  B {\bf 111},  045104  (2025)}.
...
T. Barthel and Q. Miao, {\em Absence of barren plateaus and scaling of
  ...
  \href{http://arxiv.org/abs/2304.00161, accepted in Commun.\ Math.\ Phys.}
  {arXiv:2304.00161, accepted in Commun.\ Math.\ Phys.  (2023)}.
...
\bibitem{Haghshenas2023_05}
R. Haghshenas, E. Chertkov, M. DeCross, T.~M. Gatterman, J.~A. Gerber, K.
  Gilmore, D. Gresh, N. Hewitt, C.~V. Horst, M. Matheny, T. Mengle, B.
  Neyenhuis, D. Hayes, and M. Foss-Feig, {\em Probing critical states of matter
  on a digital quantum computer},
  \href{https://doi.org/10.1103/PhysRevLett.133.266502} {Phys. Rev. Lett. {\bf
  133},  266502  (2024)}.

\bibitem{Miao2024_12}
Q. Miao, T. Wang, K.~R. Brown, T. Barthel, and M. Cetina, {\em Probing
  entanglement scaling across a quantum phase transition on a quantum
  computer}, \href{http://arxiv.org/abs/2412.18602} {arXiv:2412.18602  (2024)}.
