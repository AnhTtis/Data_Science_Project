...
\item For any $t\in \{1\}\cup [2,\infty]$, there exists $f\in \H$ such that $h_{pol}(f)=t$. Moreover, $h_{pol}(f)=1$ if and only if $f$ is the compactification of a map conjugate to translation in $\R^2$. 
\end{enumerate}
\end{theorem}
...
\begin{maintheorem}\label{teoFlex}
