...
this has applications for database repairs and also for the strategy analysis of games. 
...
However, for \emph{fully idempotent} semirings, in which both addition and multiplication are idempotent, this form of counting is not possible and Hanf's locality theorem holds.
...
We emphasize that in the Boolean case, \cref{defLocalFormula} is equivalent to the standard notion, so we do not add expressive power.
...
%
We remark that the addition of ball quantifiers now makes it possible to express $d(x,y) \le r$ by a formula that only assumes values $0$ or $1$,
...
We further use the abbreviations $\Esc r {y_1,\dots y_m} \, \theta(\ty) \coloneqq \E y_1 \dots \E y_m \big( \bigland_{i < j} d(y_i,y_j) > 2r \land \theta(\ty) \big)$ and $\Asc r {y_1,\dots y_m} \, \theta(\ty) \coloneqq \A y_1 \dots \A y_m \big( \biglor_{i < j} d(y_i,y_j) \le 2r \lor \theta(\ty) \big)$ for scattered quantifiers.
...
Then $\pi_{st}\ext{\psi(a)}=t$ but $\pi_{st}\ext{\phi(a)}=st$.
...
Since $\pi_{st}(\neg Ux) = 0$ for all $x \in A$, we can view the evaluation $\pi_{st} \ext {\alpha(x)}$ as an expression built from the
semiring operations, the value $\pi_{st}(Ux)$ and constants $0,1$.
We can thus describe the value of $\alpha(x)$ in any interpretation $\pi_{st}$ by a univariate polynomial $p(X) \in K[X]$,
...
can be described by a \emph{symmetric} polynomial $q(X,Y) \in K[X,Y]$ such that
$\pi_{st}\ext{\beta} = q(s,t)$ for all $s,t \in K \setminus \{0\}$.

Local sentences can thus be represented by polynomials
...
This insight can be formalized. With a formula $\phi$ we associate its \emph{dual} $\dual \phi$, resulting from $\phi$ by swapping $\E$, $\A$ (including scattered and ball quantifiers) and also $\lor$, $\land$, e.g.:
\[
...
This is very similar to a result of Grohe and Wöhrle \cite{GroheWoe04} for standard Boolean semantics, which is literally identical the same statement.
