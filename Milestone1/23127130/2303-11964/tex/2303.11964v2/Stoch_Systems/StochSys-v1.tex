...
\usepackage{amsmath, enumerate, amssymb, wasysym, verbatim, bbm, color, graphics, graphicx, mathrsfs}
...
\RUNAUTHOR{Gonz\'alez C\'azares, Lin and Mijatovi\'c}
...
\RUNTITLE{Simulation of the first passage of a tempered stable subordinator}
...
\ARTICLEAUTHORS{%
\AUTHOR{Jorge Ignacio Gonz\'alez C\'azares}
\AFF{Department of Statistics, University of Warwick, and The Alan Turing Institute, UK, \EMAIL{jorge.i.gonzalez-cazares@warwick.ac.uk}} %, \URL{}}
\AUTHOR{Feng, Lin}
\AFF{Department of Statistics, University of Warwick, UK, \EMAIL{feng.lin.1@warwick.ac.uk}}
\AUTHOR{Aleksandar Mijatovi\'c}
\AFF{Department of Statistics, University of Warwick, and The Alan Turing Institute, UK, \EMAIL{a.mijatovic@warwick.ac.uk}}
% Enter all authors
} % end of the block
...
\KEYWORDS{exact simulation; subordinator; first passage of subordinator; overshoot and undershoot of a subordinator;  complexity} 
%\HISTORY{...}

\MSCCLASS{60G51, 65C05 (Primary); 62E15,  60E07 (Secondary)}
...
tempered stable process (cf.~\citealp{MR3500619} for examples in financial mathematics),
...
for the pair $(\tau_b,S_{\tau_b}-b(\tau_b))$. In this case, the problem is first reduced to the stable case via the Esscher transform. Then the constant barrier makes the density of $(\tau_b,S_{\tau_b}-b)$ explicit with a stable scaling property. The scaling property in turn allows for a change of variables (in the form of the variable $r=y^{-\alpha/(1-\alpha)}t^{1/(1-\alpha)}$ in~\citealp[l.11, \S4.2]{MR4122822}) that makes it easy to sample from the resulting density. This technique,
which depends on the explicit density of the random vector $(\tau_b,S_{\tau_b}-b)$,
does not generalise easily to a non-constant function~$b$, including a linear function $b(t)= a_0-a_1t$ with $a_1>0$. Part of the difficulty in generalising such methodology is that a new possibility arises: the subordinator may \emph{creep} (i.e. cross continuously~\citealp{https://doi.org/10.48550/arxiv.2205.06865,chi2016exact}) through the curve $b$, in which case $S_{\tau_b-}=b(\tau_b)=S_{\tau_b}$. Moreover, the probability of creeping is not uniform in time even for linear functions (see Proposition~\ref{prop:joint_law}(b) below), making it hard to compute even the mass of the absolutely continuous component of the joint law.
...
\begin{proposition}\label{prop:undershoot_algorithm}
...
\begin{proposition}\label{prop:psi_1}
...
\begin{proposition}\label{prop:psi_2}
...
\begin{figure}[ht]
\FIGURE{\centering
    \includegraphics[width=12cm]{Python_output.jpeg}}{Rounding errors\label{fig:python}}{Rounding errors in Python produces singular random variables.}
\end{figure}
...
\end{subfigure}}{Implementation of Algorithm~\ref{alg:triple_stable} on Julia\label{fig:stable_case}}{The average time $\E[T]$ is measured in seconds taken for every $10^4$ samples.}
\end{figure}
...
}{Implementation of Algorithm~\ref{alg:improved_triple_temper_stable}\label{fig:tempered_stable_case}
}{
...
The solution $u$ of the FPDE problem in~\eqref{eq:fpde} exists and its stochastic representation is given by (see~\citealp{hernandez2017generalised}):
\begin{equation*}
...
}{Estimated values $u_n(t,x)$ of $u(t,x)$ using $n=10^4$ samples\label{fig:fpde}}{
...
\end{equation}
where $r:=\alpha/(1-\alpha)\in(0,\infty)$. Note that $r/\alpha=r+1$ and hence $\sigma_\alpha(0+)\coloneqq\lim_{x\downarrow0}\sigma_\alpha(x)=(1-\alpha)\alpha^{\alpha/(1-\alpha)}$.  The density $g_t$ of $S_t$ can be expressed as follows~\cite[\S4.4]{uchaikin2011chance}:  for any $x>0$ we have 
...
\begin{proposition}\label{prop:joint_law}
...
\proof{Proof of Proposition~\ref{prop:joint_law}.}
...
\begin{proposition}\label{prop:undershoot}
...
\proof{Proof of Proposition~\ref{prop:undershoot}.}
...
\begin{lemma}\label{lem:sigma_linear_upperbound}
...
\proof{Proof of Proposition~\ref{prop:psi_1}.}
...
\end{multline*}
By the stopping condition of the binary search of line~\ref{alg:inversion_newton_raphson_bisection_stop} in Algorithm~\ref{alg:inversion_newton_raphson}, we only need the final expression in the previous display to be smaller than $2^{k-2}$, implying $k=\Oh(|\log(1-\alpha)|)$. Thus, this binary search requires $\Oh(|\log(1-\alpha)|)$ iterations. 
\Halmos\endproof
...
\begin{lemma}\label{lem:sigma'_upperbound}
...
\les \frac{1}{1-x}.\Halmos
\]
\endproof

Now we can complete the proof of Proposition~\ref{prop:psi_2}.
\proof{Proof of Proposition~\ref{prop:psi_2}.}
...
Again, the computational cost of Algorithm~\ref{alg:Devroye} has two parts: (I) the cost to finding the value of $a_1$ and (II) the expected cost of the accept-reject step. By~\cite[\S4]{devroye2012note}, the expected cost of (II) is bounded above by $5$. 
...
of Algorithms~\ref{alg:psi_1} and~\ref{alg:psi_2}, given in  Propositions~\ref {prop:psi_1} and~\ref{prop:psi_2}), imply the bound in Proposition~\ref{prop:undershoot_algorithm}.\Halmos
%\end{proof}
...
\proof{Proof of Lemma~\ref{lem:bound_of_expectation_complicated_thing}.}
...
\quad\text{as }\alpha\downarrow0.\Halmos
\]
\end{enumerate}
\endproof
...
\proof{Proof of Theorem~\ref{thm:expectation_of_time_complexity}.}
...
\proof{Proof of Corollary~\ref{cor:triple_stable_small}.}
...
\proof{Proof of Theorem~\ref{thm:tempered_expect_time}.}
...
\proof{Proof of Corollary~\ref{cor:improved_tempered_expected_time}.}
...
\proof{Proof of Theorem~\ref{thm:joint_law}.}
...
\begin{lemma}\label{lem:exp_moment}
...
\proof{Proof of Lemma~\ref{lem:exp_moment}.}
...
\[
\E[\tau_c]=\int_0^\infty\p[\tau_c>y]\md y\les\int_0^\infty \me^{uc-t\psi(u)}\md t=\frac{\me^{uc}}{\psi(u)}.\Halmos
\]
\endproof
...
\frac{\p[B,C,E\ges q S_t]/\p[E\ges qS_t]}{\p[C,E\ges q S_t]/\p[E\ges qS_t]}=\p[B|C, E\ges qS_t].\Halmos
\]
\endproof
...
\vspace{2mm}
\begin{breakablealgorithm}
\caption{({\citealp[Alg.~3.1]{TemperedStableDassios}}) Simulation from the law of $S_t$ under $\p$}
...
\vspace{2mm}
\begin{breakablealgorithm}
\caption{(\citealp{devroye2012note}) Simulation from a log-concave density on $[0,1]$}
\label{alg:Devroye}
