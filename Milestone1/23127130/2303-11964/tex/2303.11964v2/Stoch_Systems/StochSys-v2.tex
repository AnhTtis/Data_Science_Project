...
\AFF{Department of Statistics, University of Warwick, and The Alan Turing Institute, UK and\\ Department of Probability and Statistics, IIMAS--UNAM, Mexico, \EMAIL{jorge.gonzalez@sigma.iimas.unam.mx}} %, \URL{}}
\AUTHOR{Feng, Lin}
...
\KEYWORDS{exact simulation; subordinator; first passage of subordinator; non-increasing time-varying barrier; overshoot and undershoot of a subordinator;  complexity} 
%\HISTORY{...}
...
A subordinator $S=(S_t)_{t\in\RP}$ (where $\RP\coloneqq[0,\infty)$) is a non-decreasing process with independent and stationary increments and right-continuous paths, started at zero $S_0=0$. It is well known that $S_t=\mu t+\sum_{s\in(0,t]}J_s$, where $\mu\in\RP$ is  a non-negative drift parameter and the (positive) jumps $\{(s,J_s):s>0, J_s>0\}$ form a Poisson point process with mean measure $\md t\otimes \nu(\md x)$ on $(0,\infty)\times (0,\infty)$, where $\nu$ is the \emph{L\'evy measure} of $S$, satisfying $\int_0^\infty\min\{1,x\}\nu(\md x)<\infty$ (see 
 details in~\citealp[Def.~8.2]{ken1999levy}). %The Laplace transform of $S$ is given by the L\'evy--Khintchine formula $t^{-1}\log\E[\me^{-\lambda S_t}]=-\mathfrak{d}+\int_0^\infty (\me^{-\lambda x}-1)\nu(\md x)$, for $\lambda, t\ges 0$. 
...
\FIGURE{\centering
    ...
    }{Density of the second marginal of $\wt\psi_s^{(2)}$\label{fig:psi2_marginal}}{The figure shows the graphs of the density of the second marginal of $\wt\psi_s^{(2)}$, in a custom logarithmic-style scale, for two processes with $\alpha=0.85$ and $\alpha=0.9$, where the parameter $s$ is at its $99^{\text{th}}$ quantile.
...
\end{subfigure}}{Implementation of \hyperref[alg:triple_stable_conditional_on_time]{SFP-Alg} with $t_*=\infty$ on Julia\label{fig:stable_case}}{The average time $\E[T]$ is measured in seconds taken for every $10^4$ samples.\label{fig:large_small_alpha}}
\end{figure}
...
}{Implementation of \hyperref[alg:improved_triple_temper_stable]{TSFFP-Alg}\label{fig:tempered_stable_case}
}{
...
of Algorithms~\hyperref[alg:psi_1]{$\psi^{(1)}$-Alg} and~\hyperref[alg:psi_2]{$\psi^{(2)}$-Alg}, given in  Propositions~\ref {prop:psi_1} and~\ref{prop:psi_2}), imply the bound in Proposition~\ref{prop:undershoot_algorithm}.\Halmos
%\end{proof}
...
\begin{algorithm}
\caption{({\citealp[Alg.~3.1]{TemperedStableDassios}}, TS-Alg) Simulation from the law of $S_t$ under $\p_q$}
...
\begin{algorithm}
\caption{(\citealp{devroye2012note}, LC-Alg) Simulation from a log-concave density on $[0,1]$}
\label{alg:Devroye}
