...
\author{
Jorge Ignacio Gonz\'alez C\'azares\footnote{\texttt{jorge.i.gonzalez-cazares@warwick.ac.uk }}, Feng Lin\footnote{\texttt{feng.lin.1@warwick.ac.uk}}, 
...
For any $\theta>0$, $\alpha\in(0,1)$ and $q\in[0,\infty)$, consider a driftless, possibly, tempered stable subordinator $S$ with L\'evy measure $\nu(\md x)=\theta(\alpha/\Gamma(1-\alpha))\me^{-qx}x^{-\alpha-1}\md x$. The firs-passage event of $S$ is described by the random element 
\[
%\chi_b^S
%\coloneqq
\big(\tau_b,
...
\]
is the first crossing time of $S$ over a non-decreasing absolutely continuous function $b:[0,\infty)\to[0,\infty)$, with $b(0)\in(0,\infty)$. The variable $S_{\tau_b-}$, 
...
Algorithm~\ref{alg:improved_triple_temper_stable} below, which is the main simulation algorithm of this paper, samples from the law of the vector $(\tau_b,S_{\tau_b-},S_{\tau_b})$.

Denote by $\mT$ the running time $\mT$ of Algorithm~\ref{alg:improved_triple_temper_stable}. Here and throughout we  assume that elementary operations such as evaluation of sums, products and elementary functions $\sin$, $\cos$, $\exp$ all have constant computational cost. Further, we assume all operations use floating point precision and denote by $N\in\N$ the desired number of precision bits of the output. Then $\mT$ has finite exponential moments (up to some order) and its mean is bounded above by
\[ 
\kappa_{\ref{alg:improved_triple_temper_stable}}
(1+qb(0)/\alpha)((1-\alpha)^{-3}+|\log\alpha|+\log N)/\alpha,
\]
where the constant $\kappa_{\ref{alg:improved_triple_temper_stable}}$ depends on neither $\alpha\in(0,1)$, $q\in[0,\infty)$ nor $b$ (see Corollary~\ref{cor:improved_tempered_expected_time} below). Moreover, the constant $\kappa_{\ref{alg:improved_triple_temper_stable}}$  can be made explicit in terms of the costs of elementary operations (such as addition, multiplication,  exponentiation and trigonometric functions). 

Since the law of $(\tau_b,S_{\tau_b-},S_{\tau_b})$
the vector degenerates in any of the limiting regimes, $\alpha\uparrow1$, $\alpha\downarrow0$, $q\to\infty$ or $b(0)\to\infty$, 
the deterioration in the performance of the algorithm in these regimes is expected.
 Indeed, the subordinator $S$ converges weakly to a linear drift as $\alpha\uparrow1$, making the law of the undershoot $S_{\tau_b-}$ increasingly harder to simulate from. If instead $\alpha\downarrow0$ or $q\to\infty$, then the subordinator $S$ converges weakly to the trivial process $0$ for which the first-passage time $\tau_b$ is infinite. Finally, if $b\equiv b(0)$ increases, then the crossing time $\tau_b$ increases, extending the running time of several loops in the algorithm. The main point of the formula in the display above
...
for a general tempering parameter $q\in[0,\infty)$ (Algorithm~\ref{alg:improved_triple_temper_stable} below)
 is the stable case $q=0$, given in Algorithm~\ref{alg:triple_stable} below. %is given by the flowchart in \jorge{ Figure -- reference missing}  %~\ref{fig:flowchart_stable},
The the general case $q\in(0,\infty)$  in Algorithm~\ref{alg:improved_triple_temper_stable} requires only an additional accept-reject step based on the Esscher transform described below. In turn, the simulation of the first-passage event of a stable process is essentially reduced to the simulation of the undershoot $S_{\tau_b-}$ on the event 
$\{S_{\tau_b-}<b(\tau_b)\}$
when the first passage did not occur continuously (i.e. by creeping\footnote{Note that creeping occurs with positive probability even though the tempered stable subordinator has no drift, when the boundary function $b$ is not constant, see~\cite{https://doi.org/10.48550/arxiv.2205.06865,chi2016exact}.}). Sampling the undershoot efficiently, a deceivingly hard task (see Subsection~\ref{subsec:How_not_to_sample} below), is the main technical contribution of this paper.  
Although the density of the undershoot $S_{\tau_b-}$ admits a semi-analytic explicit form (see~\eqref{eq:undershoot_law} below), it is unbounded and does not lend itself well to direct accept-reject methods. Our main strategy to circumvent this issue is to break up the domain into regions, depending on the values of the (possibly random) parameters, and use a mix of direct accept-reject methods and Devroye's algorithm for log-concave densities~\cite{devroye2012note}, see Algorithm~\ref{alg:Devroye} in Appendix~\ref{sec:devroye} below. The algorithm in~\cite{devroye2012note} plays a key role precisely in the regions where the density of $S_{\tau_b-}$ decreases sharply to super-exponentially small values, making the standard accept-reject methods infeasible.
...
\begin{comment}
\emph{The tempering acceptance probability.} Note that, in the simulation of a marginal, the acceptance of a sample equals
\[
\E_*\big[\me^{-qS_t}\big]
=\me^{-q^\alpha\theta t}.
\]

In the case of a stable variable $S_t$ conditioned on $S_t\les s$, the acceptance probability equals 
\[
\E_*\big[\me^{-qS_t}|S_t\les s\big]
=\frac{\E_*[\me^{-qS_t}\1_{\{S_t\les s\}}]}{\p_*[S_t\les s]}
\ges\max\big\{\E_*\big[\me^{-qS_t}\big],\me^{-qs}\big\} 
=\max\{\me^{-q^\alpha\theta t},\me^{-qs}\}.
\]

\emph{Overshoot.} Let $\rho=b(0)>0$. The acceptance probability is upper bounded by $\p_*[E\ges q b(\tau_b)]$ which attains its minimal value $\p_*[E\ges q\rho]=\me^{-q\rho}$, over the class of non-increasing functions $b:[0,\infty)\to[0,\rho]$, when we take $b\equiv \rho$. On the other hand, note that $\tau_b\ges\tau_{\rho}^S$ and hence $S(\tau_b)\ges S(\tau_{\rho}^S)$. Hence $\p_*[E\ges qS(\tau_b)]\ges\p_*[E\ges qS(\tau_\rho)]$ where, by~\cite[Ex.~VIII.3]{MR1406564} and Euler's reflection formula, the density of $S(\tau_\rho)$ is
\[
t\mapsto \1_{(\rho,\infty)}(x)\frac{\alpha\sin(\pi\alpha)}{\pi}\int_0^{r}x^{\alpha-1}(t-x)^{-\alpha-1}\md x.
\]
Thus, we have
\begin{align*}
\p_*[E\ges qS(\tau_\rho)]
&=\E_*\big[\me^{-qS(\tau_\rho)}\big]
=\frac{\alpha\sin(\pi\alpha)}{\pi}
    \int_0^\rho x^{\alpha-1}\int_{r}^\infty 
   \me^{-qt}(t-x)^{-\alpha-1}\md t\,\md x\\
&=\frac{\alpha\sin(\pi\alpha)}{\pi}
    \int_0^{q\rho}x^{\alpha-1}\int_{q\rho}^\infty 
   \me^{-t}(t-x)^{-\alpha-1}\md t\,\md x\\
&\ges
\frac{\alpha\sin(\pi\alpha)}{\pi}
    \int_0^{q\rho}x^{\alpha-1}\md x\int_{q\rho}^\infty 
   \me^{-t}t^{-\alpha-1}\md t
=\frac{\sin(\pi\alpha)}{\pi}(q\rho)^{\alpha}\Gamma(-\alpha,q\rho),
\end{align*}
where $\Gamma(a,x)=\int_x^\infty\me^{-t}t^{a-1}\md t$ is the upper incomplete gamma function. In particular, taking $q\rho\to 0$, the lower bound tends to 
$\sin(\pi\alpha)/(\pi\alpha)\in(0,1)$. \jorge{This shows that the bound is not too bad.}
\end{comment}

\subsection{Comparison with the literature}

Exact simulation of the first-passage event of a tempered stable subordinator is a central topic of two articles~\cite{chi2016exact,MR4122822}. Chi~\cite{chi2016exact} was the first to develop a simulation algorithm for the first-passage event for a wide class of subordinators  over a general non-decreasing absolutely continuous boundary functions~$b$. The case of the tempered stable subordinator plays a prominent role in~\cite{chi2016exact}, because the entire general class
...
Dassios, Lim and Qu~\cite{MR4122822} 
consider a tempered stable subordinator and a constant  function $b$. They 
...
Before presenting the algorithm, we give a brief intuitive account. The measures $\p_q$ are known to be equivalent to each other (see Appendix~\ref{app:temper}). In fact, it is known that the simulation of a path function of $(S_t:t\in[0,T])$ (for some $T>0$) under $\p_q$ is equivalent to proposing the same path function under $\p_0$ and then accepting with probability $\exp(-q^\alpha \theta T)$. Thus, it is natural to expect that the simulation of the first-passage triplet 
$(\tau_b,S_{\tau_b-},S_{\tau_b})$
under $\p_q$ can  essentially be reduced to the case $q=0$ on the stochastic interval $[0,\tau_b]$. 
However, 
since $\tau_b$ is possibly not bounded above,
this could make the acceptance probability 
arbitrarily small.
Algorithm~\ref{alg:triple_temper_stable} makes use of the change-of-measure idea by introducing a time grid and sampling $S$ on the grid until the compact interval in which $\tau_b$ lies is identified. 

\begin{breakablealgorithm}
\caption{Simulation of the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ under $\p_q$}
...
\State{Set $t_*\gets(2qb(0)+1-2^{-\alpha})/((2^\alpha-1)q^\alpha\theta)$, $T\gets 0$ and $U\gets0$}
\Repeat\label{line:loop_until_tau<t_*}
\State{Sample $s$ from the law of $S_{t_*}$ under $\p_q$ via Algorithm~\ref{alg:tempered_stable}}
\If{$s<b(t_*)$}
\State{Update $T\gets T+t_*$, $U\gets U+s$ and $b(\cdot)\gets b(\cdot+t_*)-s$}
\EndIf
\Until{$s\ges b(t_*)$}
    \Repeat
    \State{Sample $(t,u,v)$ from the law of $(\tau_b,S_{\tau_b-},S_{\tau_b})|\{\tau_b\les t_*\}$ under $\p$ via Algorithm~\ref{alg:triple_stable_conditional_on_time}}
    \State{Sample $w$ from the law of $S_{t_*-t}$ under $\p$ via Algorithm~\ref{alg:stable} and $E'\sim\Exp(1)$} 
    \Until{$E'\ges q(w+v)$}\label{line:check_temper}
    \State{\Return $(T+\tau,U+u,U+v)$}
\end{algorithmic}
\end{breakablealgorithm}

Algorithms~\ref{alg:stable} and~\ref{alg:tempered_stable} are well-known simple and fast procedures for sampling stable  and tempered stable  marginals, respectively (see Appendix~\ref{app:temp_stable_marginal} below).
Algorithm~\ref{alg:triple_stable_conditional_on_time} and its analysis constitutes the main technical contribution of this paper. 
The complexity of Algorithm~\ref{alg:triple_temper_stable}
...
Algorithm~\ref{alg:triple_temper_stable} samples from the law of the triplet $(\tau_b, S_{\tau_b-}, S_{\tau_b})$ of a tempered stable subordinator $S$ under $\p_q$. Moreover, the running time of Algorithm~\ref{alg:triple_temper_stable} has exponential moments with mean bounded by
\[
\kappa_{\ref{alg:triple_temper_stable}}\me^{2qb(0)/(2^\alpha-1)}((1-\alpha)^{-3}+|\log\alpha|+\log N)/\alpha,
\]
where the constant $\kappa_{\ref{alg:triple_temper_stable}}$ does not depend on the parameters $\alpha\in(0,1)$, $\theta\in(0,\infty)$ or $q\in[0,\infty)$, and $N$ specifies the required number of precision bits of the output.
\end{thm}


The upper bound on the running time of Algorithm~\ref{alg:triple_temper_stable} has a term of order $\Oh(\exp(2qb(0)/(2^\alpha-1))$, which may be quite large if either $b(0)$, $q$ or $1/\alpha$ are large. However, the exponential dependence on these parameters of the complexity of Algorithm~\ref{alg:triple_temper_stable} can be easily turned into a linear dependence (see Cor.~\ref{cor:improved_tempered_expected_time}) by picking $R>0$ and applying  Algorithm~\ref{alg:triple_temper_stable} successively with  the boundary function  $t\mapsto \min\{b(t),R\}$.

\begin{breakablealgorithm}
\caption{Fast simulation of the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ under $\p_q$}
\label{alg:improved_triple_temper_stable}
\begin{algorithmic}[1]
\Require{Parameters $\alpha\in(0,1)$, $\theta>0$, $q\ges 0$ and function $b:[0,\infty)\to[0,\infty)$}
\State{Set $R\gets(2^\alpha-1)/(2q)$, $T\gets0$ and $U\gets0$}
\Repeat
\State{Sample $(\tau,u,v)$ via Algorithm~\ref{alg:triple_temper_stable} with function $\min\{b,R\}$}
\If{$v<b(\tau)$}
\State{$T\leftarrow T+\tau$, $U\leftarrow U+v$, $b(\cdot)\leftarrow b(\cdot+\tau)-v$}
\EndIf
\Until{$v\ges b(\tau)$}
\State{\Return ${(T+\tau, U+u, U+v)}$}
\end{algorithmic}
...
Algorithm~\ref{alg:improved_triple_temper_stable} samples from the law of $(\tau_b, S_{\tau_b-}, S_{\tau_b})$ under $\p_q$. Moreover, the running time of Algorithm~\ref{alg:improved_triple_temper_stable} has exponential moments with mean bounded by
\[ 
\kappa_{\ref{alg:improved_triple_temper_stable}}
(1+qb(0)/\alpha)((1-\alpha)^{-3}+|\log\alpha|+\log N)/\alpha,
\]
where $\kappa_{\ref{alg:improved_triple_temper_stable}}$ depends on neither $\alpha\in(0,1)$, $\theta\in(0,\infty)$ nor $q\in[0,\infty)$, and $N$ is as in Theorem~\ref{thm:tempered_expect_time}.
\end{cor}


%The key ingredients in Algorithm~\ref{alg:triple_temper_stable} are (I) Algorithm~\ref{alg:triple_stable_conditional_on_time}, which simulates the crossing triplet of a stable process conditioned on the crossing occurring before a given time $t_*>0$ and (II) Algorithms~\ref{alg:stable} and~\ref{alg:tempered_stable} which sample, respectively, the marginals of a stable and tempered stable subordinators with constant expected running time. Since the algorithms in (II) are standard in the literature (see Appendices~\ref{app:stable_marginal} and~\ref{app:temp_stable_marginal} below), the missing component is developing Algorithm~\ref{alg:triple_stable_conditional_on_time}. 

\subsection{Sampling the first passage of a stable subordinator}
\label{subsec:simulation_stable_vector}

The crucial ingredient of Algorithm~\ref{alg:triple_stable_conditional_on_time}
for sampling $(\tau_b,S_{\tau_b-},S_{\tau_b})|\{\tau_b\les t_*\}$ under the stable law $\p$
is Algorithm~\ref{alg:triple_stable} for the  sampling of the unconditional 
$(\tau_b,S_{\tau_b-},S_{\tau_b})$ under $\p$.
The basis of Algorithm~\ref{alg:triple_stable} is Proposition~\ref{prop:joint_law} below, describing the law of $(\tau_b,S(\tau_b-),S(\tau_b))$ under $\p$. Before presenting it, we give a brief account of its contents. First,  Algorithm~\ref{alg:triple_stable} samples the crossing time $\tau_b$ of the stable subordinator. Then, the algorithm determines whether the crossing occurred continuously (i.e., by creeping, with a `jump' of size $\Delta_S(\tau_b)=0$) or via a jump of size $\Delta_S(\tau_b)>0$. In the former case, the algorithm simply returns the crossing triple and, in the latter, the algorithm samples the undershoot and the overshoot.

\begin{breakablealgorithm}
\caption{Simulation of the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ under $\p$}
\label{alg:triple_stable}
\begin{algorithmic}[1]
\Require{Parameters $\alpha\in(0,1)$, $\theta>0$ and functions $b:[0,\infty)\to[0,\infty)$ and $B^{-1}:(0,\infty)\to(0,T_b)$ specified in Proposition~\ref{prop:joint_law}}
\State{Generate a stable variable $V_1\sim\mathcal{L}(S_1)$ via Algorithm~\ref{alg:stable} and set $T\gets B^{-1}(V_1)$}
\State{Generate $U_1\sim\Unif(0,1)$}
\If{$U_1\les -b'(T)/(-b'(T)+\alpha^{-1}T^{-1}b(T))$}
\State{\Return $(T, b(T), b(T))$}
\Else
\State{Generate $U_2\sim\Unif(0,1)$}
\State{Generate $V_2\sim\Unif\rmS_{\alpha}(T,w)$  with the law of $S_{T-}|\{\tau_b=T,\,b(\tau_b)=w,\,\Delta_S(\tau_b)>0\}$ via Algorithm~\ref{alg:undershoot_stable}}
...
Algorithm~\ref{alg:triple_stable} samples from the law of the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ under $\p$. The running time of Algorithm~\ref{alg:triple_stable} has exponential moments and its mean is bounded by 
\[
\kappa_{\ref{alg:triple_stable}}\left((1-\alpha)^{-3}+|\log\alpha|+\log N\right),
\]
where the constant $\kappa_{\ref{alg:triple_stable}}$ depends neither on  $\alpha\in(0,1)$ nor $\theta\in(0,\infty)$, and $N$ specifies the required number of precision bits of the output.
\end{thm}


The proof of Theorem~\ref{thm:expectation_of_time_complexity} is in  Section~\ref{subsubsec:proof_of_main_thm_stable} below. Theorem~\ref{thm:expectation_of_time_complexity} implies that the expected running time of Algorithm~\ref{alg:triple_stable} may increase as fast as $(1-\alpha)^{-3}$ as $\alpha\uparrow1$. A deterioration in the limit as $\alpha\uparrow1$ is expected since, in this limit, the stable subordinator $S$ converges to a pure-drift subordinator $\theta t$, for which the distribution of the triplet degenerates to a delta distribution. Similarly, the expected running time of Algorithm~\ref{alg:triple_stable} may increase as fast as $|\log\alpha|$ as $\alpha\downarrow0$. This is expected as well since the stable subordinator $S$ converges to the trivial process $0$ as $\alpha\downarrow 0$. Numerical evidence for these increases in complexity and the theoretical bounds in Theorem~\ref{thm:expectation_of_time_complexity} can be found in Section~\ref{sec:numerics} below. We stress that existing algorithms for the simulation of the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ have infinite expected running time for any $\alpha\in(0,1)$, see Subsection~\ref{subsec:How_not_to_sample} below for more details. 


A minor variation of Algorithm~\ref{alg:triple_stable} yields samples conditional on the event $\tau_b\les t_*$.


\begin{breakablealgorithm}
\caption{Simulation of the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ conditional on $\tau_b\les t_*$ under $\p$}
\label{alg:triple_stable_conditional_on_time}
\begin{algorithmic}[1]
\Require{Parameters $\alpha\in(0,1)$, $\theta>0$, functions $b:[0,\infty)\to[0,\infty)$ and  $B^{-1}:(0,\infty)\to(0,T_b)$ specified in Proposition~\ref{prop:joint_law}}
\State{Generate $V_1\sim\mathcal{L}(S_1)$ via Algorithm~\ref{alg:stable} until $V_1\ges B(t_*)$ and set $T\gets B^{-1}(V_1)$}\label{line:tau<t_*}
\Comment{This is the only step different from Algorithm~\ref{alg:triple_stable}}
\State{Generate $U_1\sim\Unif(0,1)$}
\If{$U_1\les -b'(T)/(-b'(T)+\alpha^{-1}T^{-1}b(T))$}
\State{\Return $(T, b(T), b(T))$}
\Else
\State{Generate $U_2\sim\Unif(0,1)$}
\State{Generate $V_2\sim\Unif\rmS_{\alpha}(T,b(T))$ with the law of $S_{T-}|\{\tau_b=T,\,b(\tau_b)=b(T),\,\Delta_S(\tau_b)>0\}$ via Algorithm~\ref{alg:undershoot_stable}}
...
\begin{cor}
\label{cor:triple_stable_small}
Algorithm~\ref{alg:triple_stable_conditional_on_time} samples from the law of $(\tau_b,S_{\tau_b-},S_{\tau_b})$ under $\p$, conditional on $\tau_b\les t_*$. Moreover, the running time of Algorithm~\ref{alg:triple_stable_conditional_on_time} has exponential moments and its mean is bounded by
\[
\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{\p[S(t_*)\ge b(t_*)]}
\les \me\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{\min\{1,\alpha^r(1-\alpha)(\theta t_*)^{r+1}b(t_*)^r\}}
\]
where the constant $\kappa_{\ref{alg:triple_stable}}$ is as in Theorem~\ref{thm:expectation_of_time_complexity} and $N$ specifies the number of precision bits of the output.
\end{cor}

As we said before, Algorithms~\ref{alg:triple_stable} and~\ref{alg:triple_stable_conditional_on_time} have three main steps. First, we generate the first-passage time $T$ in line~1. This requires evaluating, up to $N$ bits of precision, the inverse $B^{-1}$ of the function $B:t\mapsto t^{-1/\alpha}b(t)$, which is on $(0,T_b)$ where $T_b\coloneqq \inf\{t>0:b(t)=0\}\in(0,\infty]$. This step may require a numerical inversion of $B$ (or, equivalently, numerically finding the root of $t\mapsto B(t)-V_1$). Such algorithms are standard in the literature (see Appendix~\ref{sec:NewtonRaphson}), making this a simple step. Second, the algorithm determines if the first passage occurred continuously (i.e., by creeping) in line~3. If the process crept, the triple equals $(T,b(T),b(T))$, but, if it did not, then we must sample the undershoot and overshoot of the process conditional on the crossing time. The third main step is precisely the simulation of the undershoot and overshoot in lines~7 and~8. The law of the undershoot, although expressible in terms of the stable law, is complicated and hard to generate samples from. For this reason, the focus of the remainder of this section is to construct Algorithm~\ref{alg:undershoot_stable}. 

\subsection{Simulation of the stable undershoot  via domination by a mixture of densities}
\label{subsec:undershoot}

Our main objective now is to present a simulation algorithm from the undershoot law $\Unif\rmS_\alpha(t,w)$, $t,w>0$, that is, the law of $S_{\tau_b-}$ under $\p$, conditional on $\{\tau_b=t, b(\tau_b)=w, \Delta_S(\tau_b)>0\}$. Our main tool is the general rejection-sampling principle. Unlike before, the tools and algorithms used here have little probabilistic interpretation and are mostly based on analytical properties and inequalities. The derivation of these inequalities, which validate Algorithm~\ref{alg:undershoot_stable}, will be developed and explained in Section~\ref{sec:proofs} below. For now, we only present the algorithms with a brief description of how it works. The algorithm has several nontrivial components that we will slowly introduce and explain. The reason for this is that the task of simulating from the undershoot law $\Unif\rmS_\alpha(t,w)$ comes with multiple pitfalls, with several tempting approaches having severe problems, see Subsection~\ref{subsec:How_not_to_sample} for an account of these pitfalls.


First, the density $f_{t,w}$ of $\Unif\rmS_\alpha(t,w)$ involves the density of the stable law, which can be written as a non-elementary integral of elementary functions (see Proposition~\ref{prop:joint_law}). Thus, by extending the state space and applying an elementary transformation, the problem is converted into producing samples from a density $\psi$ proportional to $(x,y)\mapsto (s-x^{-1/r})^{-\alpha}\sigma_\alpha(y)\me^{-\sigma_\alpha(y)x}$ on $(s^{-r},\infty)\times(0,1)$, where $r\coloneqq \alpha/(1-\alpha)$ and $\sigma_\alpha$ is the convex function defined in terms of trigonometric functions in~\eqref{eq:zolotarev_density}. The density $\psi_s$ is difficult to sample from directly, so we first bound it with a mixture $\wt\psi_s=p\wt\psi^{(1)}_s+(1-p)\wt\psi^{(2)}_s$ of two simpler densities $\wt\psi^{(1)}_s$ and $\wt\psi^{(2)}_s$ on $(s^{-r},\infty)\times(0,1)$ proportional to, respectively,   $(x,y)\mapsto\sigma_\alpha(y)\me^{-\sigma_\alpha(y)x}$ and $(x,y)\mapsto\sigma_\alpha(y)\me^{-\sigma_\alpha(y)x}(x-s^{-r})^{-\alpha}$ (see details in Subsection~\ref{subsec:Undershoot_simulation} below). The simulation algorithms corresponding to these simpler densities will be discussed later.

\begin{breakablealgorithm}
\caption{Simulation from the law of $S_{t-}|\{\tau_b=t,\,b(\tau_b)=w,\,\Delta_b^S>0\}$ }
...
\If{$V_1<p$}
    \State{Generate $(\zeta_s,Y_s)$ with density $\wt\psi_s^{(1)}$ via Algorithm~\ref{alg:psi_1}}
\Else
    \State{Generate $(\zeta_s,Y_s)$ with density $\wt\psi_s^{(2)}$ via Algorithm~\ref{alg:psi_2}}
\EndIf
...
\begin{remark}
\label{rem:prob_p'}
The proportion $p'$ (and hence $p$) can be efficiently evaluated to arbitrary precision using Gaussian--Legendre quadrature. In fact, the numerator in the formula in~\eqref{eq:psi_p'} equals the distribution function of a stable law. The computation time for $p'$ is thus assumed to be constant for any $s=(\theta t)^{-1/\alpha}w$, see fast algorithms in e.g. \cite{nolan1997numerical,ament2018accurate}. Our simulation times agree with this assumption, see Section~\ref{sec:numerics} below.
\end{remark}

For any $a,x>0$, define $\log_a^+(x):=\max\{0, \log(x)/\log(a)\}$.
\begin{prop}\label{prop:undershoot_algorithm}
Algorithm~\ref{alg:undershoot_stable} samples from the law $\Unif\rmS_\alpha(t,w)$ of the undershoot $S_{t-}$, conditional on the event $\{\tau_b=t,b(\tau_b)=w,\Delta_b^S>0\}$. The random running time of Algorithm~\ref{alg:undershoot_stable} has exponential moments. Moreover, the expected running time of Algorithm~\ref{alg:undershoot_stable} is bounded above by 
\begin{align*}   \kappa_{\ref{alg:undershoot_stable}}\Bigl[(1-\alpha)^{-1}&\Bigl(1+\alpha(1-\alpha)^{-1}\log_2^+(s^{-1})+\log_2^+\big(1/(s^{\alpha/(1-\alpha)}-\alpha^{(1-2\alpha)/(1-\alpha)}(1-\alpha))\big) \\
&+\alpha\log_2^+(s)+|\log(\alpha)|+(1-\alpha)^{-2}\Bigr)+\log N\Bigr],
\end{align*}
where the constant $\kappa_{\ref{alg:undershoot_stable}}>0$ depends on neither $s=(\theta t)^{-1/\alpha}w\in(0,\infty)$ (see line~\ref{line_1:alg:undershoot_stable} of Algorithm~\ref{alg:undershoot_stable}), $t>0$, $\theta\in(0,\infty)$ nor $\alpha\in(0,1)$.
\end{prop}
...
The bivariate density $\wt\psi_s^{(1)}$ can be written as the product of a marginal log-concave density and an exponential density with deterministic shift and a random scale. The latter is easy to simulate and the former can also be simulated via the general Algorithm~\ref{alg:Devroye} by virtue of being log-concave.

\begin{breakablealgorithm}
\caption{Simulation from the density $\wt\psi_s^{(1)}$}
\label{alg:psi_1}
\begin{algorithmic}[1]
\Require{Parameters $\alpha\in(0,1)$ and $s>0$}
\State{Sample $Y_s$ with density $y\mapsto \exp(-\sigma_\alpha(y)s^{-r})$ via Algorithm~\ref{alg:Devroye}}
...
Algorithm~\ref{alg:psi_1} samples from the density $\wt\psi_s^{(1)}$. Moreover, the running time of Algorithm~\ref{alg:psi_1} is bounded by a multiple of $G+\alpha(1-\alpha)^{-1}\log^+(s^{-1})$ where $G$ is a geometric random variable with parameter $1/5$. In particular, the running time has exponential moments with mean bounded by 
\[
\kappa_{\ref{alg:psi_1}}\left(1+\alpha(1-\alpha)^{-1}\log^+(s^{-1})\right),
\]
where the constant $\kappa_{\ref{alg:psi_1}}>0$ depends neither on $s\in(0,\infty)$ nor $\alpha\in(0,1)$.
\end{prop}

The proof of Proposition~\ref{prop:psi_1} is given in Subsection~\ref{subsec:psi_(1)-2} below. Algorithm~\ref{alg:psi_1} does not rely on numerical inversion (cf.~Algorithm~\ref{alg:inversion_newton_raphson}). Its expected running time does therefore not depend on the number of precision bits $N$, specified \textit{a priori}. In fact, the computational complexity of Algorithm~\ref{alg:psi_1} comes solely from the pre-prosessing step (line~\ref{line_find_a_1} of Algorithm~\ref{alg:Devroye}), which depends on the values of the parameters in Algorithm~\ref{alg:psi_1}.


\subsubsection{Simulation from the density \texorpdfstring{$\wt\psi_s^{(2)}$}{psi 2}}
\label{subsec:psi_(2)}

Simulating from the density $\wt\psi_s^{(2)}$ is a delicate matter, particularly because of the sensitivity of the function on $\alpha\in(0,1)$. Algorithm~\ref{alg:psi_2} below samples from the density $\wt\psi_s^{(2)}$ by breaking up the support of $\wt\psi_s^{(2)}$ depending on the parameter $\alpha$ and using rejection-sampling with respect to various dominating (proposal) densities. Algorithm~\ref{alg:psi_2} needs to numerically invert several twice differentiable functions $f:[0,1]\to\R$ to produce samples of certain dominating (proposal) densities. This inversion is achieved via the Newton--Raphson root-finding method given in Algorithm~\ref{alg:inversion_newton_raphson}.  Each call of Algorithm~\ref{alg:inversion_newton_raphson} requires the input of a specific target function $f$ and an auxiliary function $M:[0,1]\times\N\to\R$ satisfying~\eqref{eq:newton_raphson_auxiliary} (see Appendix~\ref{sec:NewtonRaphson}). 

\begin{breakablealgorithm}
\label{alg:psi_2}
  \caption{Simulation from the density $\wt\psi_s^{(2)}$}
  \begin{algorithmic}[1]
\Require{Parameters $\alpha\in(0,1)$ and $s>0$}
\State{Set $z\gets 0$ if $\alpha s^r<\sigma_\alpha(0+)$. Else $z\gets\sigma_\alpha^{-1}(\alpha s^r)$ via
Algorithm~\ref{alg:inversion_newton_raphson} with $f=\log\sigma_\alpha$ and $M$ in~\eqref{eq:sigma_auxiliary_function}}
...
\If{$D=2$}
    \State{Generate $Y_s$ from the density proportional to $\tilde f|_{[z,1]}$ via Algorithm~\ref{alg:Devroye}}
...
\ElsIf{$D=0$}
    \Repeat
        ...
        via Algorithm~\ref{alg:inversion_newton_raphson} with $M$ in~\eqref{eq:u*sigma^alpha_auxiliary}}
...
\If{$\alpha\les1/2$}
    ...
    \Repeat
        \State{Generate $Y_s$ with distribution $f$ defined in~\eqref{eq:density_c1} via Algorithm~\ref{alg:inversion_newton_raphson} with $M$ in~\eqref{eq:c2_auxiliary}}\label{step:inversion_alpha<1/2}
        \State{Generate $V\sim\Unif(0,1)$}
    ...
    \Repeat
        \State{Generate $Y_s$ with distribution $f$ defined in~\eqref{eq:distribution_c2} via Algorithm~\ref{alg:inversion_newton_raphson} with $M$ in~\eqref{eq:sigma_auxiliary_function}}
...
for Step~\ref{step:generate_discrete_rv} in Algorithm~\ref{alg:undershoot_stable}
is thus assumed to be constant.
\end{remark}

\begin{prop}\label{prop:psi_2}
Algorithm~\ref{alg:psi_2} samples from the 
law given by the density $\wt\psi_s^{(2)}$. Moreover, the running time of Algorithm~\ref{alg:psi_2} is bounded above by a constant multiple of 
\[
G+\log^+\big(1/\big(s^{\alpha/(1-\alpha)}-\alpha^{(2\alpha-1)/(1-\alpha)}\big)\big)+\alpha\log^+(s)+|\log\alpha|+\log N,
\] 
where $N$ specifies the number of precision bits of the output and $G$ is a geometric random variable with  probability of success $\p(G=1)$ bounded below by a multiple of $(1-\alpha)^2$. In particular, the running time of Algorithm~\ref{alg:psi_2} has exponential moments and its mean is bounded by 
$$
\kappa_{\ref{alg:psi_2}} 
\left[(1-\alpha)^{-2}
    +\log^+\big(1/\big(s^{\alpha/(1-\alpha)}
    -\alpha^{(2\alpha-1)/(1-\alpha)}\big)\big)
    +\alpha\log^+(s)+|\log\alpha|+\log N\right],
$$ 
where the constant $\kappa_5$ depends neither on $s\in(0,\infty)$ nor $\alpha\in(0,1)$.
\end{prop}

The proof of Proposition~\ref{prop:psi_2} is given in Subsection~\ref{subsec:psi_(2)-2} below. The computational cost in Algorithm~\ref{alg:psi_2} comes from various sources: 
(I) applications of the inversion Algorithm~\ref{alg:inversion_newton_raphson}, which may depend on the parameters $\alpha\in(0,1)$ and $s>0$ (recall that the parameter $s$ is in fact random in Algorithm~\ref{alg:undershoot_stable}, which calls Algorithm~\ref{alg:psi_2});
(II) in the cases $D=0$ and $D=1$,
we also perform an accept-reject step where the acceptence probability depends on the parameters $\alpha\in(0,1)$ and $s>0$; (III) in the case $D=2$, the pre-processing (line~\ref{line_find_a_1} in Algorithm~\ref{alg:Devroye})
also adds to the final computational complexity since the function Algorithm~\ref{alg:Devroye} is applied to in line~\ref{step:devroye_in_psi_2}  of Algorithm~\ref{alg:psi_2} depends both on
$\alpha\in(0,1)$ and $s>0$. We note that the accept-reject step in the case $D=2$ is embedded in Algorithm~\ref{alg:Devroye} and has uniformly bounded (in parameter values) expected running time. 
...
=\frac{\varphi_\alpha(x)(s-x)^{-\alpha}\md x}{\int_0^x \varphi_\alpha(y)(s-y)^{-\alpha}\md y},
\]
...
\end{aligned}
\end{equation}
However, $\E[S_1^\eta]<\infty$ if and only if $\eta<\alpha$ by~\eqref{eq:mellin_tranform} below. Since $r=\alpha/(1-\alpha)>0$ for all $\alpha\in(0,1)$, the expected running time of the algorithm in~\cite[Sec.~7]{chi2016exact} is indeed infinite. 

We note that, similarly, Step 5 of the algorithm in~\cite[Sec.~7]{chi2016exact} also has infinite expected running time. Since Step~5 is more complex than Step~3, we made the phenomenon above explicit only for Step~3.

\subsection{Can an implemented simulation algorithm  be exact?}
\label{subsec:exact?}

The answer to the question in the title of the present subsection  depends on the definition of exact simulation.
The simplest, and in some sense most natural, definition requires the output of the algorithm to have the same law as the variable it is simulating. In this case, the answer is clearly no: working with $N\in\N$ bits of precision makes the output lie on a grid, implying that the total variation (TV) distance between a simulated output and any target law with a density equals one.\footnote{We thank Luc Devroye for pointing this out to us.} 
A way of dealing with this issue might be to understand and minimize a natural distance between the output of an implemented algorithm and a draw from the target law.  
Two natural ways of measuring the distance between algorithm's output and the actual law are as follows.
...
For any coupling $\lambda$ of the laws of $A$ and $I$, the $L^\infty$-distance is given by the essential supremum $\lambda\text{-}\esssup |A-I|$ and the $L^\infty$-Wasserstein distance equals the infimum of $L^\infty$-distances over all couplings $\lambda$.
It appears that even a simple accept-reject method, implemented on a computer cannot  reduce this distance, regardless of the precision used (i.e. for any $N\in\N$). Indeed, consider an implementation of Algorithm~\ref{alg:Devroye} (in  Appendix~\ref{sec:devroye} below), together with the natural coupling, obtained by running the algorithm on a computer and on an ideal computer (IC), which carries out all evaluations without error.  Even if the constants $a_0$, $a_1$ and $f(a_1)$ could be represented exactly on the grid, the event-detection occurring in lines $6$ and $8$ will have a chance of being misdetected by the computer, simply because the variable $V_2$ is not uniform on $(0,1)$ but instead lives on the grid. Therefore, there is a (small, but non-zero) chance that the value of $I$ originated from line $7$, while the value of $A$ came from line $9$. 
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=12cm]{Python_output.jpeg}
    \caption{Rounding errors in Python produces singular random variables.}
    \label{fig:python}
\end{figure}

The discussion in (I) and (II) suggests that it is not evident how one can quantify a deviation from the strong definition of exact simulation 
stated above. In this paper we use the following weaker definition: an algorithm implemented on a computer
is \textit{exact} if,  given exact values of the iid uniforms and assuming all the elementary operations carried out by the algorithm are exact,  the returned value has the correct law (i.e. with zero TV-distance from the target law). In this definition, elementary operations include evaluations of elementary functions and their integrals as well as inversions.  
...
In the case of our main simulation method (Algorithm~\ref{alg:improved_triple_temper_stable} above), we instead carefully analyse the cost of each Newton-Raphson call needed to perform an inversions within the error margin, smaller than what can be represented on the computer. All other elementary operations, including the evaluation of the definite  integrals in Algorithms~\ref{alg:undershoot_stable} (Step~\ref{line_1:alg:undershoot_stable})
and~\ref{alg:psi_2} (Step~\ref{step:generate_discrete_rv}) are assumed to have constant complexity (the integrands are well behaved and close to those arising in the representation of a stable density, making the numerical procedures converge fast, see remarks following the algorithms). 
...
The main objective of this section is to present some applications of our algorithms. First, we present some numerical evidence for the theoretical bounds on the expected running time of Algorithms~\ref{alg:improved_triple_temper_stable} and~\ref{alg:triple_stable}. We then show how Algorithm~\ref{alg:improved_triple_temper_stable} can be used to sample the first-passage event of a two-sided tempered stable process, which in turn can be used to price barrier options via Monte Carlo. Next, we use the probabilistic representation of the solution of a fractional partial differential equation (FPDE)~\cite{hernandez2017generalised} along with Algorithm~\ref{alg:improved_triple_temper_stable} to produce Monte Carlo estimators of the solution of such a FPDE.

\subsection{Implementation}
\label{sec:numerics}

In this section, we showcase an implementation of our algorithms in Julia computing language. Our first goal is to show that such an implementation makes the simulation of the first-passage event feasible and fast in practice. Our second goal is to show that, in practice, our bounds on the expected running time of Algorithms~\ref{alg:improved_triple_temper_stable} and~\ref{alg:triple_stable} overestimate the true complexity. 
...
\begin{subfigure}[b]{0.49\textwidth}
 ...
 \caption{By Theorem~\ref{thm:expectation_of_time_complexity}, $\E[T]=\Oh(|\log\alpha|)$ as $\alpha\downarrow0$. The picture confirms this: $\E[T]/|\log\alpha|$ decreases to a number close to $1$ as $\alpha\downarrow0$ (the reported value of $\E[T]$ for $\alpha=0.001$ is $9.30$ seconds per $10^4$ samples).}
...
\begin{subfigure}[b]{0.49\textwidth}
 ...
 \caption{By Theorem~\ref{thm:expectation_of_time_complexity}, $\E[T]=\Oh((1-\alpha)^{-3})$ as $\alpha\uparrow1$. The picture confirms this: $\log(\E[T])/|\log(1-\alpha)|$ decreases as $\alpha\uparrow1$ to a number close to $0.2$ (the reported value of $\E[T]$ for $\alpha=0.999$ is $6.28$ seconds per $10^4$ samples).}
 \label{fig:large_alpha}
\end{subfigure}
\caption{The pictures show an implementation of Algorithm~\ref{alg:triple_stable}. The average time $\E[T]$ is measured in seconds taken for every $10^4$ samples.}
\label{fig:stable_case}
\end{figure}

Observe that the expected running time appears to be smaller for $\alpha=0.999$ (close to $1$) than for $\alpha=0.001$ (close to $0$). Although this is surprising since our upper bounds are of order $\Oh((1-\alpha)^{-3})$ for large $\alpha$ and of order $\Oh(|\log\alpha|)$ for small $\alpha$, it is important to consider a few things. First, our key Algorithm~\ref{alg:psi_2}, used to simulate the stable undershoot, uses a very different simulation method for $\alpha\les1/2$ and for $\alpha>1/2$. In particular, the multiplicative constants in the $\Oh$ notation can be significantly different. Second, the bounds in Theorem~\ref{thm:expectation_of_time_complexity} are only bounds, and need not be sharp. In particular, this does not necessarily mean that the asymptotic cost as $\alpha\uparrow1$ is truly smaller than the asymptotic cost as $\alpha\downarrow0$.
...
\caption{
The picture depicts an implementation of Algorithm~\ref{alg:improved_triple_temper_stable} with parameters $\alpha=0.55$, $\theta=1$, $b(t)\equiv 1$ and $q\in\{\me^0, \me^{1/4},\ldots,\me^{11}\}$. The mean running time $\E[T]$ is measured in seconds taken for every $10^2$ samples. By Corollary~\ref{cor:improved_tempered_expected_time}, $\E[T]=\Oh(q)$ as $q\to\infty$. This is supported by the picture: the quotient $\log(\E [T])/\log(\me+q)$ appears to converge to $1$ as $q\to\infty$.}
\label{fig:tempered_stable_case}
\end{figure}
...
We start by recalling the algorithm in~\cite[Sec.~5.1]{chi2016exact}.
...
\Repeat
    \State{Sample $(s,u,v)\sim\mathcal{L}(\tau_b,Z^+_{\tau_b^+-},Z^+_{\tau_b^+})$ %via  Algorithm~\ref{alg:improved_triple_temper_stable} 
...
Let $(\tau_*^i,Z_{\tau_*^i}^i)$, $i\in\N$, be iid samples with the law of $(\tau_*,Z_{\tau_*})$ generated via Algorithm~\ref{alg:FPE_of_bounded_variation}. Then, for every $n$, we consider the Monte Carlo estimator
\[
G_n(T,R_0)\coloneqq
    \frac{1}{n}\sum_{k=1}^n 
        \max\{R_0\me^{Z_{\tau_*}^i}-K,0\}\1\{\tau_*^i=T\},
\]
of $G(T,R_0)$. Figure~\ref{fig:barrier_option} shows such an implementation for the case where $Z$ is the difference of two tempered stable subordinators. To obtain smoother graphs, we apply Algorithm~\ref{alg:FPE_of_bounded_variation} to successive values of the barrier level $\log(M/R_0)$ (i.e., by decreasing values of $R_0\in\{98,98.031,\ldots,101.999\}$), obtaining strongly correlated samples. This is a well-known procedure in Monte Carlo estimation that retains the accuracy of each estimate as if we had used independent samples for each value of $R_0$, but reduces the random fluctuations between any two estimated values, see~\cite[Sec.4.2]{MR1999614}.

\begin{figure}[ht]
\centering
\includegraphics[width=.6\textwidth]{barrier_option.png}
\caption{
Estimated values of $\me^{-\delta T}\E[\max\{R_T-K,0\}\1\{\sup_{t\in[0,T]}R_t<M\}]$ using $n=10^4$ samples with time horizons $T\in\{14/365, 30/365, 90/365\}$ and parameters $\delta=0$, $K=98$ and $M=102$. We assume $Z=Z^+-Z^-$ where $Z^+$ and $Z^-$ are both driftless tempered stable subordinators with parameters $(\alpha^+,\theta^+,q^+)=(0.66,0.1305,6.5022)$ and $(\alpha^-,\theta^-,q^-)=(0.66,0.0615,3.3088)$, respectively, as calibrated from the USD/JPY currency pair, see~\cite[Table 3]{MR3038608}.
}
...
$T_t^k$ are an iid samples of $T_t$, obtained by Algorithm~\ref{alg:improved_triple_temper_stable}, while 
$$X^{x,k}_{s}:=x\exp\left(\sqrt{s} N^k-s/2\right),$$ where 
$N^k$ are iid standard normal random variables.
Since $X^x$ is a geometric Brownian motion and Algorithm~\ref{alg:improved_triple_temper_stable} is exact, the variables $\phi(X_{T_t^k}^{x,k})$ in the estimator in~\eqref{eq:mc_estimator} have the same law as $\phi(X^x_{T_t})$.
...
\begin{comment}
\begin{breakablealgorithm}
\label{alg:MC_estimator}
\caption{Monte-Carlo estimator of the solution of FPDE~\eqref{eq:fpde}}
\begin{algorithmic}[1]
\Require{$\alpha\in(0,1)$, $q,\theta>0$, $N\in\mathbb N$}
\State{Set $u\leftarrow0$}
\For{$k=1:N$}
    \State{Sample $T_t^i$,  sample $B_1\sim N(0,1)$, and update $u\leftarrow u+\phi(x\exp((T_t^i)^{1/2}B_1-(T_k^i)^2/2))$}
\EndFor
\State{$\bar u\leftarrow u/N$}
\State{\Return $\bar u$}
\end{algorithmic}
\end{breakablealgorithm}
\end{comment}
...
\caption{
Estimated values $u_n(t,x)$ of $u(t,x)$ using $n=10^4$ samples. Here $\phi(x)=x^2$, $T_t$ has the  law of the first-passage time of a tempered stable subordinator with parameters $\alpha=0.4$, $\theta=1$, $q=1$ crossing level $b(t)\equiv 1$ and $X=(X_t)_{t\in\RP}$ is geometric Brownian motion $X_t=x\exp(B_t-t/2)$ with $X_0=x>0$.
}
\label{fig:fpde}
\end{figure}
...
\end{equation}
where $r:=\alpha/(1-\alpha)\in(0,\infty)$. Note that $r/\alpha=r+1$ and hence $\sigma_\alpha(0+)\coloneqq\lim_{x\downarrow0}\sigma_\alpha(x)=(1-\alpha)\alpha^{\alpha/(1-\alpha)}$.  The density $g_t$ of $S_t$ can be expressed as follows~\cite[Section~4.4]{uchaikin2011chance}:  for any $x>0$ we have 
...
$$
\p[S(\tau_b)=b(\tau_b)|\tau_b=t]=\p[\Delta_S(\tau_b)=0|\tau_b=t]=-b'(t)/(-b'(t)+\alpha^{-1}t^{-1}b(t)).
$$
...
\item[(ii)] The expression of the probability of creeping, conditional on $\tau_b=t$, in Proposition~\ref{prop:joint_law}(b) does not depend on the density of $S_t$. This makes the event of creeping easy to sample, see Algorithm~\ref{alg:triple_stable} below.
\item[(iii)] Conditional on not creeping, the jump $\Delta_S(\tau_b)$ is easy to sample by inverting its distribution function in~\eqref{eq:creep_law}. In contrast, sampling the undershoot from the law in~\eqref{eq:undershoot_law} in finite expected running time is delicate, see Algorithm~\ref{alg:undershoot_stable} below.
\end{enumerate}
\end{remark}



\begin{proof}[{\textbf{{Proof of Proposition~\ref{prop:joint_law}}}}]
...
$
for any $t>0$. \\
(b) We claim that $\tau_b$ is absolutely continuous and 
its density is given by
a function $K:(0,T_b)\to (0,\infty)$, 
\begin{equation}\label{eq:density_tau}
K(t)=g_t(b(t))\left(\alpha^{-1}t^{-1}b(t)-b'(t)\right)\quad\text{ for all $t\in(0,T_b)$.}
\end{equation}
...
\[
    ...
    \end{aligned}
\]
Since $\p[S(\tau_b)=b(\tau_b)|\tau_b=t]=
\p[\Delta_S(\tau_b)=0|\tau_b=t]=
(1/K(t))\frac{\md}{\md t}\p[\Delta_S(\tau_b)=0, \tau_b\leq t]$,
%-b'(t)/(-b'(t)+\alpha^{-1}t^{-1}b(t))$
the representation in~\eqref{eq:density_tau} and~\eqref{subeq:creep_b} imply $\p[S(\tau_b)=b(\tau_b)|\tau_b=t]=-b'(t)/(-b'(t)+\alpha^{-1}t^{-1}b(t))$.\\
...
\begin{figure}
    \centering
     ...
     \draw   (166,26) .. controls (166,14.95) and (200.25,6) .. (242.5,6) .. controls (284.75,6) and (319,14.95) .. (319,26) .. controls (319,37.05) and (284.75,46) .. (242.5,46) .. controls (200.25,46) and (166,37.05) .. (166,26) -- cycle ;
%Shape: Rectangle [id:dp32243798010177116] 
\draw   (61,62) -- (456,62) -- (456,199) -- (61,199) -- cycle ;
%Shape: Ellipse [id:dp44129441350834564] 
\draw   (232,510) .. controls (232,498.95) and (264.24,490) .. (304,490) .. controls (343.76,490) and (376,498.95) .. (376,510) .. controls (376,521.05) and (343.76,530) .. (304,530) .. controls (264.24,530) and (232,521.05) .. (232,510) -- cycle ;
%Straight Lines [id:da97769761452816] 
\draw    (243,46) -- (243,59) ;
\draw [shift={(243,61)}, rotate = 270] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da15074286710157292] 
\draw    (527,250) -- (640,250) -- (640,508) -- (378,509.98) ;
\draw [shift={(376,510)}, rotate = 359.57] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Flowchart: Decision [id:dp9656789601482638] 
\draw   (31.5,228) -- (82,248.5) -- (31.5,269) -- (-19,248.5) -- cycle ;
%Flowchart: Decision [id:dp9054208789700111] 
\draw   (31.5,289) -- (82,309) -- (31.5,329) -- (-19,309) -- cycle ;
%Flowchart: Decision [id:dp6008598745892606] 
\draw   (31.5,349) -- (82,369) -- (31.5,389) -- (-19,369) -- cycle ;
%Shape: Rectangle [id:dp8573734528262478] 
\draw   (100,229) -- (527,229) -- (527,269) -- (100,269) -- cycle ;
%Shape: Rectangle [id:dp37137654327687897] 
\draw   (101,279) -- (628,279) -- (628,329) -- (101,329) -- cycle ;
%Shape: Rectangle [id:dp6145863069688628] 
\draw   (101,340) -- (626,340) -- (626,399) -- (101,399) -- cycle ;
%Shape: Rectangle [id:dp6586689671380344] 
\draw   (100,409) -- (627,409) -- (627,480) -- (100,480) -- cycle ;
%Straight Lines [id:da32010212570451047] 
\draw    (241,198) -- (241,210) -- (30,209) -- (30.9,228) ;
\draw [shift={(31,230)}, rotate = 267.27] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da34212174502933435] 
\draw    (82,248.5) -- (98,248.94) ;
\draw [shift={(100,249)}, rotate = 181.59] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da18156369767566516] 
\draw    (31.5,269) -- (31.5,287) ;
\draw [shift={(31.5,289)}, rotate = 270] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da33540639470665656] 
\draw    (82,309) -- (99,309) ;
\draw [shift={(101,309)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da15378439218095696] 
\draw    (31.5,329) -- (31.5,347) ;
\draw [shift={(31.5,349)}, rotate = 270] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da322332223597523] 
\draw    (31.5,389) -- (30,441) -- (98,441) ;
\draw [shift={(100,441)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da732544812510592] 
\draw    (82,369) -- (99,369) ;
\draw [shift={(101,369)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da4683401952758066] 
\draw    (640,301) -- (627,301) ;
%Straight Lines [id:da6097291855616545] 
\draw    (640,370) -- (625,370) ;
%Straight Lines [id:da934391245095999] 
\draw    (640,443) -- (627,443) ;

% Text Node
\draw (185,16) node [anchor=north west][inner sep=0.75pt]   [align=left] {Input \ $\displaystyle \alpha ,\ \theta ,\ t,\ v\ $};
% Text Node
\draw (76,65) node [anchor=north west][inner sep=0.75pt]   [align=left] {Define $\displaystyle f( u) :=\sigma_\alpha ( u)^{\alpha }\exp\left( -\sigma_\alpha ( u) s^{-r}\right) .$\\Set $\displaystyle z\leftarrow \sigma_\alpha ^{-1}\left( \alpha s^{r}\right) 1\left\{\alpha s^{r}  >\sigma_\alpha ( 0+)\right\}$and $\displaystyle z_{*}\leftarrow z\land 1/2$.\\Generate discrete random variable $\displaystyle D$ on $\displaystyle \{0,1,2\}$ \\with weights $\displaystyle \left(\int _{0}^{z_{*}} f( u) du,\int _{z_{*}}^{z} f( u) du,\ \int _{z}^{1} f( u) du\right)$};
% Text Node
\draw (267,500) node [anchor=north west][inner sep=0.75pt]   [align=left] {Return $\displaystyle U$};
% Text Node
\draw (4,241) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle D=2?$};
% Text Node
\draw (4,302) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle D=0?$};
% Text Node
\draw (-5,359) node [anchor=north west][inner sep=0.75pt]   [align=left] {$\displaystyle \alpha < 1/2?$};
% Text Node
\draw (105,238) node [anchor=north west][inner sep=0.75pt]   [align=left] {Generate $\displaystyle U$ from density proportional to $\displaystyle f|_{[ z,1]}$ via Algorithm~\ref{alg:Devroye} };
% Text Node
\draw (103,282) node [anchor=north west][inner sep=0.75pt]   [align=left] {Generate $\displaystyle U$ from distribution proportional to $\displaystyle u\mapsto u\sigma_\alpha ( u)^{\alpha }$ via inversion method,\\and do accept-reject with probability $\displaystyle h( U) ,$ where $\displaystyle h$ is defined in~\eqref{eq:prob_h} };
% Text Node
\draw (106,348) node [anchor=north west][inner sep=0.75pt]   [align=left] {Generate $\displaystyle U$ from density proportional to~\eqref{eq:distribution_c1} by inversion via Newton--Raphson\\and do accept-reject with probability $\displaystyle h_{c,1}( U) ,$ where $\displaystyle h_{c,1}$ is defined in~\eqref{eq:h_c1} };
% Text Node
\draw (109,413) node [anchor=north west][inner sep=0.75pt]   [align=left] {Generate $\displaystyle U$ from distribution proportional to $\displaystyle \rho ( u)^{r-1} -\rho ( 1/2)^{r-1}$ via \\inversion method, and do accept-reject with probability $\displaystyle h_{c,2}( U) ,$ \\where $\displaystyle h_{c,2}$ is defined in~\eqref{eq:h_c2} };
% Text Node
\draw (75,227) node [anchor=north west][inner sep=0.75pt]   [align=left] {Yes};
% Text Node
\draw (77,346) node [anchor=north west][inner sep=0.75pt]   [align=left] {Yes};
% Text Node
\draw (76,284) node [anchor=north west][inner sep=0.75pt]   [align=left] {Yes};
% Text Node
\draw (36,267) node [anchor=north west][inner sep=0.75pt]   [align=left] {No};
% Text Node
\draw (38,329) node [anchor=north west][inner sep=0.75pt]   [align=left] {No};
% Text Node
\draw (35,406) node [anchor=north west][inner sep=0.75pt]   [align=left] {No};


\end{tikzpicture}
    \caption{Caption}
    \label{flowchart_second}
\end{figure}
...
%Shape: Ellipse [id:dp7799856252837061] 
\draw   (166,26) .. controls (166,14.95) and (200.25,6) .. (242.5,6) .. controls (284.75,6) and (319,14.95) .. (319,26) .. controls (319,37.05) and (284.75,46) .. (242.5,46) .. controls (200.25,46) and (166,37.05) .. (166,26) -- cycle ;
%Shape: Rectangle [id:dp32243798010177116] 
\draw   (61,62) -- (456,62) -- (456,199) -- (61,199) -- cycle ;
%Shape: Ellipse [id:dp44129441350834564] 
...
The task is to justify the validity of Algorithm~\ref{alg:triple_stable}, which samples the triplet $(\tau_b,S_{\tau_b-},S_{\tau_b})$ of the stable subordinator $S$ under $\p$. The analysis of its expected computational complexity has some technical prerequisites and will thus be deferred to Subsection~\ref{subsubsec:proof_of_main_thm_stable}. 

By Proposition~\ref{prop:joint_law}(a), $\tau_b$ has the same law as $B^{-1}(S_1)$ and can be sampled exactly in constant time (i.e. \textit{not} requiring a random number of steps) via the widely used Algorithm~\ref{alg:stable} given in Appendix~\ref{app:temp_stable_marginal} below. Moreover, by Proposition~\ref{prop:joint_law}(b), the probability of the stable subordinator $S$ creeping over the function $b$ (i.e. $\Delta_S(\tau_b)=0$), given $\tau_b=t$, equals $-b'(t)/\big(-b'(t)+\alpha^{-1}t^{-1}b(t)\big)$. Algorithm~\ref{alg:triple_stable} samples a Bernoulli variable taking the value $1$ with probability $-b'(t)/\big(-b'(t)+\alpha^{-1}t^{-1}b(t)\big)$. If the Bernoulli equals $1$, then, since the probability of $S$ jumping onto the function $b$ is zero $\p(S_{\tau_b-}<b(\tau_b), S_{\tau_b}=b(\tau_b))=0$, Algorithm~\ref{alg:triple_stable} returns $(\tau_b,S_{\tau_b-},S_{\tau_b})=\big(t,b(t),b(t)\big)$. If, conditional on $\tau_b=t$, creeping does not occur, Algorithm~\ref{alg:triple_stable} samples the undershoot $S(\tau_b-)$ from the law $\p[S_{\tau_b-}\in\md u|\Delta_S(\tau_b)>0,\tau_b=t]$, given in~\eqref{eq:undershoot_law} of Proposition~\ref{prop:joint_law}(c), using Algorithm~\ref{alg:undershoot_stable} given below. Then, conditional on $S_{\tau_b-}=u$, the jump size $\Delta_S(t)$ has  by~\eqref{eq:creep_law} the same law as $(b(t)-u)U^{-1/\alpha}$, where $U$ is an independent uniform on $(0,1)$. Thus, Algorithm~\ref{alg:undershoot_stable} returns $(\tau_b,S_{\tau_b-},S_{\tau_b})=\big(t,u,u+(b(t)-u)U^{-1/\alpha}\big)$.


%We now state the main result of this section. Its proof is in  Section~\ref{subsubsec:proof_of_main_thm_stable} below.
%The only difficult part is to sample the %undershoot $S(\tau_b-)$.

By the previous paragraph, for Algorithm~\ref{alg:triple_stable} to be valid, it suffices to justify that Algorithm~\ref{alg:undershoot_stable} produces samples from the law $\Unif\rmS_\alpha(t,w)$ of $S_{\tau_b-}|\{\tau_b=t, b(\tau_b)=w, \Delta_S(\tau_b)>0\}$, which is described analytically in~\eqref{eq:undershoot_law} of Proposition~\ref{prop:joint_law}(c). Note that the simulation from the law $\Unif\rmS_\alpha(t,w)$ via Algorithm~\ref{alg:undershoot_stable} is the only step in Algorithm~\ref{alg:triple_stable} that has random running time. In fact, the design of Algorithm~\ref{alg:undershoot_stable} is one of the main contributions of this paper and will be discussed in detail in the following subsection.
...
We now describe and justify Algorithm~\ref{alg:undershoot_stable} and analyse its computational complexity. Recall from Subsection~\ref{subsec:joint_law_description} above that the function $b:[0,\infty)\to[0,\infty)$ is absolutely continuous and non-decreasing and hence almost everywhere differentiable with derivative $b'$ almost everywhere equal to the density of $b$ and set to be equal to $-1$ at the points of non-differentiability of $b$. Further recall that $r=\alpha/(1-\alpha)$, $w_\alpha=\Gamma(1-\alpha)/\alpha>0$ and the definition $T_b>0$, as well as that of the functions $\sigma_\alpha$ and $\varphi_\alpha$ defined in~\eqref{eq:zolotarev_density} and~\eqref{eq:zolotarev_representation}, respectively. 

By~\eqref{eq:undershoot_law} in Proposition~\ref{prop:joint_law}(c) and the representation of the stable density $g_t(x)$ in~\eqref{eq:zolotarev_representation}, the density of $\Unif\rmS_{\alpha}(t,w)$ is given by 
\[
f_{t,w}(x)= \frac{1}{w_\alpha w\varphi_\alpha((\theta t)^{-1/\alpha}w)}
\varphi_\alpha((\theta t)^{-1/\alpha}x)((\theta t)^{-1/\alpha}w-(\theta t)^{-1/\alpha}x)^{-\alpha}, \qquad x\in(0,w),
\] 
while the density of  $\xi\sim\Unif\rmS_\alpha(1/\theta,(\theta t)^{-1/\alpha}w)$ equals
%$\xi$ has density 
\[
...
$(\theta t)^{1/\alpha}\xi\sim \Unif\rmS_{\alpha}(t,w)$. It suffices to sample $\xi$ from the law $\Unif\rmS_\alpha(1/\theta,s)$ with density 
...
\]
Define a random variable $\zeta_s:=\xi^{-r}$. Note that $\zeta_s^{-1/r}\sim\Unif\rmS_\alpha(1/\theta,s)$,
and hence $(\theta t)^{1/\alpha}\zeta_s^{-1/r}\sim\Unif\rmS_\alpha(t,w)$.
Moreover, 
...
is given by the density
 ...
 \end{equation}
 Fast exact simulation from the law given by the density $\psi_s$ is non-trivial but possible, see Algorithm~\ref{alg:undershoot_stable}.
...
(1-\alpha)/2\les c_\psi\psi_s(x,y)/\wt\psi_s(x,y)=(s-x^{-1/r})^{-\alpha}/\big((1-2^{\alpha-1})^{-\alpha} s^{-\alpha}
    +(2r)^{\alpha}s^{-r}(x-s^{-r})^{-\alpha}\big)\les 1. 
\end{equation}
\end{prop}

\begin{remark}
The numerical value of the constant $c_\psi$ in Proposition~\ref{prop:undershoot} is not required in Algorithm~\ref{alg:undershoot_stable}, which samples from the density $\psi_s$ in~\eqref{eq:scaled_law}.
...
\begin{proof}[{\textbf{{Proof of Proposition~\ref{prop:undershoot}}}}]
Denoted the total masses of the positive functions 
...
\end{align*} 
\begin{comment}
  The integral over $x$ in the denominator can be computed in by changing of variable as follows:
\[
\int_{s^{-r}}^\infty\me^{-\sigma_\alpha(u)x}(x-s^{-r})^{-\alpha}\md x
=\me^{-\sigma_\alpha(u)s^{-r}}\int_0^\infty\me^{-\sigma_\alpha(u)y}y^{-\alpha}\md y
=\me^{-\sigma_\alpha(u)s^{-r}}\sigma_\alpha(u)^{\alpha-1}\int_0^\infty\me^{-z}z^{-\alpha}\md z.
\]
\end{comment}
...
\begin{comment}
\begin{align*}
    &((1-2^{\alpha-1})^{-\alpha}+2^\alpha)^{-1}\wt\psi_s\les c_\psi\psi_s\\
    \Longleftrightarrow& ((1-2^{\alpha-1})^{-\alpha}+2^\alpha)^{-1}\big((1-2^{\alpha-1})^{-\alpha} s^{-\alpha}
    +(2r)^{\alpha}s^{-r}(x-s^{-r})^{-\alpha}\big)\les (s-x^{-1/r})^{-\alpha}\\
    \Longleftrightarrow& (1-2^{\alpha-1})^{-\alpha} s^{-\alpha}
    +(2r)^{\alpha}s^{-r}(x-s^{-r})^{-\alpha}\les ((1-2^{\alpha-1})^{-\alpha}+2^\alpha)(s-x^{-1/r})^{-\alpha}\\
    \Longleftarrow&  (2r)^{\alpha}s^{-r}(x-s^{-r})^{-\alpha}\les 2^\alpha(s-x^{-1/r})^{-\alpha}
\end{align*}
\end{comment}

Since $(1-2^{\alpha-1})^{-\alpha} s^{-\alpha}\ges (s-x^{-1/r})^{-\alpha}$ for $x\geq 2^\alpha s^{-r}$
(note that $x\mapsto (s-x^{-1/r})^{-\alpha}$ is decreasing and equal to $(1-2^{\alpha-1})^{-\alpha} s^{-\alpha}$
at $x= 2^\alpha s^{-r}$),
the inequality on the right side of~\eqref{eq:inequality_psi_tilde} holds for  $x\geq 2^\alpha s^{-r}$. Consider the case $s^{-r}<x<2^\alpha s^{-r}$. Since $x\mapsto(1-\alpha)s^{r+1}x+2\alpha x^{-1/r}$ decreases on $[s^{-r},2^\alpha s^{-r}]$, after substituting $r=\alpha/(1-\alpha)$, we obtain $(1-\alpha)s^{1/(1-\alpha)}x+2\alpha x^{-(1-\alpha)/\alpha}\les (1+\alpha)s$. This inequality is equivalent to $(2\alpha/(1-\alpha))^\alpha s^{-\alpha/(1-\alpha)}(x-s^{-\alpha/(1-\alpha)})^{-\alpha}\ges (s-x^{-(1-\alpha)/\alpha})^{-\alpha}$. Substituting 
...
\begin{proof}[{\textbf{{Proof of Proposition~\ref{prop:psi_1}}}}]
...
via Algorithm~\ref{alg:Devroye}. Then, conditional on $Y_s=y$, we sample $\zeta_s$ from the law whose density is proportional to the function $x\mapsto\sigma_\alpha(y)\exp(-\sigma_\alpha(y)x)\1_{\{x>s^{-r}\}}$ on $x\in(s^{-r},\infty)$. Note that this makes $\zeta_s$, conditional on $Y_s$, into a shifted exponential random variable. 

As explained in Section~\ref{sec:devroye} below, the computational cost of Algorithm~\ref{alg:Devroye} has two parts: (I) the cost of finding the value of $a_1$ in line~\ref{line_find_a_1} of Algorithm~\ref{alg:Devroye} and (II) the expected 
cost of the accept-reject step in Algorithm~\ref{alg:Devroye}.
By~\cite[Sec.~4]{devroye2012note}, the expected cost of (II) is bounded above by $5$. 

Note that cost (I) of finding $a_1$ within Algorithm~\ref{alg:psi_1} is not constant in the variable $s$,
which is itself random in Algorithm~\ref{alg:triple_stable}. 
Thus we need to analyse the cost of finding $a_1$ in Algorithm~\ref{alg:Devroye} as a function of $s$.
...
in the binary search in Step~\ref{step_binary} of Algorithm~\ref{alg:Devroye} is bounded above by 
$$1+\log(1/y)\les 1+\log^+\big((\pi-2\me^{-1})(\log4)^{-1}(1-\alpha)s^{-r}\big)\les\kappa_{\ref{alg:psi_1}}(1+\alpha(1-\alpha)^{-1}\log_2^+(s^{-1})),$$
where the constant in $\kappa_{\ref{alg:psi_1}}$ depends neither on $s\in(0,\infty)$ nor $\alpha\in(0,1)$,
implying the proposition.
\end{proof}

Before proving Proposition~\ref{prop:psi_2}, we first clarify in the next subsection the the time cost of applying the Newton--Raphson method in  lines~1, 8, 15 and 21 of Algorithm~\ref{alg:psi_2}. As we shall see, the costs depend on $\alpha$ and may, but need not, depend on $s$.

\subsubsection{Time cost of applying the Newton--Raphson method in Algorithm~\ref{alg:psi_2}}
...
\end{equation}
For any $s>\alpha\log\alpha+(1-\alpha)\log(1-\alpha)$, the computational cost of finding the root of $\varsigma(x)$ via numerical inversion is
...
Since $\varsigma(x)$ is convex, if our initial estimate $x_0$ of the root is larger than the true root, then the Newton--Raphson sequence
\[
...
\ges x_*^3(1-x_*-2^{-k})^2$, by the stopping condition of the binary search of line~\ref{alg:inversion_newton_raphson_bisection_stop} in Algorithm~\ref{alg:inversion_newton_raphson}, we only need 
$$
2^{1-k}\cdot\frac{1}{2\pi\alpha(1-\alpha)x_*^3(1-x_*-2^{-k})^2}\les\frac{1}{2}.
$$
implying $k=\Oh(|\log(x_*(1-x_*))+\log(\alpha(1-\alpha))|)$. Thus, this binary search requires $\Oh(|\log(x_*(1-x_*))+\log(\alpha(1-\alpha))|)$ steps.
\end{proof}
...
\begin{equation}
    ...
    +(\alpha-1)\sigma_\alpha'(x)^2/\sigma_\alpha(x)^{2}
        \big)\bigg).
\end{equation}
The total cost of numerical inversion of the function $u\mapsto u\sigma_\alpha(u)^\alpha$ on the interval $[0,z_*]\subset[0,1/2]$ is
$\Oh(|\log(1-\alpha)|+\log N)$ where $N$ specifies the number of precision bits.
\end{prop}
\begin{proof}
 On $[0,z_*]$, the function is convex and increasing with derivative $\sigma_\alpha(u)^\alpha+u\alpha\sigma_\alpha'(u)\sigma_\alpha(u)^{\alpha-1}$. Thus, given $y\in(0,\sigma(z_*))$, if our initial estimate $x_0$ of the root $x_*$ of the function $u\sigma_\alpha(x)^\alpha-y$ is larger than $x_*$ and sufficiently close to $x_*$, then the Newton--Raphson sequence
\[
x_{n+1}
...
\end{multline*}
By the stopping condition of the binary search of line~\ref{alg:inversion_newton_raphson_bisection_stop} in Algorithm~\ref{alg:inversion_newton_raphson}, we only need the final expression in the previous display to be smaller than $2^{k-2}$,
%$$
%2^{1-k}\left[1+\frac{1}{4}\bigg(-\frac{\alpha}{1-\alpha}\bigg(\frac{\alpha\pi}{\sin(\alpha\pi/2)}\bigg)^2
%    -\bigg(\frac{(1-\alpha)\pi}{\sin((1-\alpha)\pi/2) }\bigg)^2
%    +\pi^2
%    +\alpha\bigg(\frac{\alpha}{1-\alpha}\frac{\cos(\alpha\pi/2)}{\sin(\alpha\pi/2)}+\cos(\alpha\pi/2)\bigg)^2\bigg)^2\right]\les\frac{1}{2}
%$$
implying $k=\Oh(|\log(1-\alpha)|)$. Thus, this binary search requires $\Oh(|\log(1-\alpha)|)$ iterations. 
\end{proof}
...
\end{equation}
For $\alpha\in(0,1/2)$, the total cost of numerical inversion of the function $F$ is
$\Oh(|\log(1-z)|+\log N)$ where $N$ specifies the number of precision bits.
\end{prop}
\begin{proof}
Note that $F''(x)=(1-x)^{-(r+1)}(r\sin(\pi(1-\alpha))-\pi\alpha(1-\alpha)\cos(\pi\alpha)(1-r)(1-x))$ for $x\in(1/2,z)$. Given $y\in(F(z),F(1/2))$, if our initial estimate $x_0$ of the root $x_*$ of the function $F(x)-y$ is  sufficiently close to $x_*$, then the Newton--Raphson sequence
\[
x_{n+1}
...
\end{align*}
By the stopping condition of the binary search of line~\ref{alg:inversion_newton_raphson_bisection_stop} in Algorithm~\ref{alg:inversion_newton_raphson}, we only need 
$$
...
$$
implying $k\ges\Oh(|\log(1-z)|)$. Thus this binary search requires $\Oh(|\log(1-z)|)$ iterations. 
\end{proof}
...
+\sum_{j=i}^{i+\lfloor(m-i)/2\rfloor}\Bigg(\binom{m}{m-j}i-\binom{m}{m+i-j}i\Bigg)\big(c^{m-j+i}-c^{j}\big).
\]
...
For $x\in(0,1)$, we have $\sigma_\alpha'(x)\les
...
\begin{proof}[{\textbf{{Proof of Proposition~\ref{prop:psi_2}}}}]
In line~\ref{step:inversion_sigma},
 Algorithm~\ref{alg:psi_2} sets $z:=0$ when $\alpha s^r<\sigma_\alpha(0+)=(1-\alpha)\alpha^{\alpha/(1-\alpha)}$.  Otherwise the equation $\sigma_\alpha(x)=\alpha s^{r}$,
...
%Hence
%$$
%\frac{(\pi-2\me^{-1})(1-\alpha)}{\alpha %s^r-\alpha^r(1-\alpha)}\frac{4\alpha^{1-%\alpha}s^\alpha}{\sin(\alpha\pi)}
%\les\frac{ 4(\pi-2\me^{-1})s^\alpha}%{\pi\alpha^2(s^r-\alpha^{r-1}(1-%\alpha))}.
%$$
...
\]
hold on the interval $(z,1)$. Since $\sigma_\alpha$ is strictly increasing and $\sigma_\alpha''>0$, $\tilde f|_{[z,1]}$ log-concave on $[z,1]$ with mode at $z$. Thus we may apply Algorithm~\ref{alg:Devroye} to obtain a sample in the interval $[z,1]$. 

Again, the computational cost of Algorithm~\ref{alg:Devroye} has two parts: (I) the cost to finding the value of $a_1$ and (II) the expected cost of the accept-reject step. By~\cite[Sec.~4]{devroye2012note}, the expected cost of (II) is bounded above by $5$. 

To analyse the cost of finding $a_1$ in Algorithm~\ref{alg:Devroye}, our idea is to find $y\in(z,(z+1)/2)$ such that $\tilde f(y)/\tilde f(z)\ges 1/4$ since $(z+1)/2$ is the first number we check in the binary search. And then we only need to estimate the upperbound of $\log(1/(y-z))$. Recall that $\tilde f(x):=\sigma_\alpha(x)^\alpha\exp(-\sigma_\alpha(x)s^{-r})$ and $\tilde f(z)=\alpha s^r$. In fact, for $y\in(z,1)$ satisfying $\sigma_\alpha(y)=(\alpha+2\log2)s^r$, we have 
\[
...
$$\kappa_{\ref{alg:psi_2}} \left[1/(1-\alpha)^{2}+1/\log^+(s^{\alpha/(1-\alpha)}-\alpha^{(1-2\alpha)/(1-\alpha)}(1-\alpha))+\alpha\log^+(s)+|\log(\alpha)|+\log N\right],$$ 
where the constant $\kappa_{\ref{alg:psi_2}}$ depends on neither $s\in(0,\infty)$ nor $\alpha\in(0,1)$. 
\end{proof}

\subsubsection{Proof of Proposition~\ref{prop:undershoot_algorithm}}
\label{subsubsec:proof_conditional_law_complexity}
%\begin{proof}[{\textbf{{Proof of Proposition~\ref{prop:undershoot_algorithm}}}}]
In this subsection we prove Proposition~\ref{prop:undershoot_algorithm} using Propositions~\ref{prop:psi_1}, \ref{prop:psi_2} and~\ref{prop:undershoot}. Let $t$ and $w$ be as in the statement of Proposition~\ref{prop:undershoot_algorithm} and recall that $\theta>0$ is the ``intensity'' in the L\'evy measure~\eqref{eq:stable_Levy_measure} of the stable subordinator $S$. Recall also that $s=(\theta t)^{-1/\alpha}w$. As explained in the beginning of  Section~\ref{subsec:Undershoot_simulation}, if we sample $(\zeta_s,Y_s)$ with density $\psi_s$ given in~\eqref{eq:scaled_law}, then the random variable $(\theta t)^{1/\alpha}\zeta_s^{-1/r}$ follows the law of $S_{t-}|\{\tau_b=t,\,b(\tau_b)=w,\,\Delta_b^S>0\}$. By the inequalities in~\eqref{eq:inequality_psi_tilde} of Proposition~\ref{prop:undershoot}, Algorithm~\ref{alg:undershoot_stable} samples from $\wt \psi_s$ (defined in Proposition~\ref{prop:undershoot}) and performs an accept-reject step to obtain a sample $(\zeta_s,Y_s)$ from the density $\psi_s$. This implies that Algorithm~\ref{alg:undershoot_stable} indeed samples from the law $\Unif\rmS_{\alpha}(t,w)$.

It remains to prove that the expected running time of Algorithm~\ref{alg:undershoot_stable} is bounded as stated. As explained in Remark~\ref{rem:prob_p'} above, the computation of $p'$ is assumed to take a constant amount of time. By~\eqref{eq:inequality_psi_tilde}, the acceptance probability in line~\ref{step:alg_undershoot_accept} of Algorithm~\ref{alg:undershoot_stable} equals $c_\psi\psi_s(\zeta_s,Y_s)/\wt\psi_s(\zeta_s,Y_s)$ and is bounded below by $(1-\alpha)/2$, uniformly in $s$ (and all other parameters except $\alpha$). Thus the accept-reject step in Algorithm~\ref{alg:undershoot_stable} on average takes no more than $2/(1-\alpha)$ steps.
...
of Algorithms~\ref{alg:psi_1} and~\ref{alg:psi_2}, given in  Propositions~\ref {prop:psi_1} and~\ref{prop:psi_2}), imply the bound in Proposition~\ref{prop:undershoot_algorithm}.\qed
%\end{proof}


\subsection{Proofs of Theorem~\ref{thm:expectation_of_time_complexity} and Corollary~\ref{cor:triple_stable_small}}
...
With $s$ defined in line~\ref{line_1:alg:undershoot_stable} of Algorithm~\ref{alg:undershoot_stable}, we have:
...
\begin{proof}[\textbf{Proof of Lemma~\ref{lem:bound_of_expectation_complicated_thing}}]
...
&=-\int_A^{(A^r+1)^{1/r}}\varphi_\alpha(x)\log(x^r-A^r)\md x\\
&\leq -\|\varphi_\alpha\|_\infty  \int_A^{(A^r+1)^{1/r}}\log(x^r-A^r)\md x.
\end{align*}
By substituting $x=(y+A^r)^{1/r}$ we get
\begin{multline*}
-\int_A^{(A^r+1)^{1/r}}\log(x^r-A^r)\md x
...
\E\bigg[\log^+\bigg(\frac{1}{s^r-\alpha^{r-1}(1-\alpha)}\bigg)\bigg]
=-\int_A^{(A^r+1)^{1/r}}\varphi_\alpha(x)\log(x^r-A^r)\md x
\les-\frac{r}{\me}\int_A^{(A^r+1)^{1/r}}\log(x^r-A^r)\frac{\md x}{x}.
\]
By substituting $x=(y+A^r)^{1/r}$ we get
\[
...
We can now give a proof Theorem~\ref{thm:expectation_of_time_complexity} on the expected running time of Algorithm~\ref{alg:triple_stable}.


\begin{proof}[\textbf{Proof of Theorem~\ref{thm:expectation_of_time_complexity}}]
By Proposition~\ref{prop:undershoot_algorithm}, to find an upper bound on the average running time, we only need to take expectation of the average running time of Algorithm~\ref{alg:undershoot_stable} in $s$. In other words, upper bound $\E[\alpha\log_2^+(s)]$, $\E[\alpha\log_2^+(s^{-1})]$ and $\E[\log^+(1/(s^{\alpha/(1-\alpha)}-\alpha^{(2\alpha-1)/(1-\alpha)}(1-\alpha)))]$.
...
\les4\Gamma(3/2).
\]
Together with Lemma~\ref{lem:bound_of_expectation_complicated_thing}, the average running time of Algorithm~\ref{alg:triple_stable} is bounded above by 
$$
...
To show the running time of Algorithm~\ref{alg:triple_stable} has exponential moments, %besides those accept-reject steps with deterministic acceptance probability, 
we only need to verify that the running times of Algorithms~\ref{alg:psi_1} and~\ref{alg:psi_2}, with random parameter $s\eqd S_{1/\theta}$, have exponential moments. 
\begin{comment} 
Next we list the complexities of all the the subroutines of Algorithms~\ref{alg:psi_1} and~\ref{alg:psi_2} with random running time.
\begin{enumerate}
\item The cost of finding $a_1$ while applying Algorithm~\ref{alg:Devroye} in Algorithm~\ref{alg:psi_1} is bounded by 
$$
\kappa_{\ref{alg:psi_1}}\big(1+\alpha(1-\alpha)^{-1}\log^+(s^{-1})\big).
$$
\item The cost of inverting $\sigma_\alpha$ in line~\ref{line_1:alg:undershoot_stable} in Algorithm~\ref{alg:psi_2} is bounded by 
$$
\Oh\big(\log^+\big(1/(s^r-\alpha^{r-1}(1-\alpha))\big)+\alpha\log^+(s)+|\log(\alpha)| +|\log(1-\alpha)|+\log N\big).
$$
\item The cost of finding $a_1$ while applying Algorithm~\ref{alg:Devroye} in line~\ref{step:devroye_in_psi_2} Algorithm~\ref{alg:psi_2} is bounded above by 
$$
\Oh(1+\alpha\log^+(s)+(1-\alpha)^{-1}|\log(1-\alpha)|).
$$
\item The cost of inversion in line~\ref{step:inversion_D=0} in Algorithm~\ref{alg:psi_2} is bounded by $\Oh(|\log(1-\alpha)|+\log N)$. 
\item The cost of inversion in line~\ref{step:inversion_alpha<1/2} in Algorithm~\ref{alg:psi_2} is bounded by 
$
\Oh(\alpha\log_2^+(s)+\log N).
$
\item The cost of inversion in line~\ref{step:inversion_alpha>1/2} in Algorithm~\ref{alg:psi_2} is bounded by 
$
\Oh(\alpha\log_2^+(s)+|\log(1-\alpha)|+\log N).
$
\end{enumerate}
\end{comment}
By Propositions~\ref{prop:psi_1} and~\ref{prop:psi_2}, we only need to show that $\log^+(s^{-1})$, $\log^+(s)$ and $\log^+(s^r-\alpha^{r-1}(1-\alpha))^{-1})$ have exponential moments. Note from~\eqref{eq:mellin_tranform} that $\E[\exp(\eta\log^+(s^{-1}))]=\E[s^{-\eta}\1\{s<1\}]<\infty$ for any $\eta\ges 0$, $\E[\exp(\eta\log^+(s))]=\E[s^\eta\1\{s>1\}]<\infty$ for any $\eta<\alpha$, and 
\begin{multline*}
...
\begin{proof}[\textbf{Proof of Corollary~\ref{cor:triple_stable_small}}]
Algorithm~\ref{alg:triple_stable_conditional_on_time} samples $\chi_b^S$ under $\p$ conditional on $\tau_b\les t_*$ and its Line~\ref{line:tau<t_*} is the only step different from Algorithm~\ref{alg:triple_stable}: we keep sampling $\tau_b$ until $\tau_b\les t_*$. This is equivalent to $V_1\eqd S_1$ being larger or equal to $B(t_*)$, where $B$ is specified in Proposition~\ref{prop:joint_law}, and has acceptance probability equal to $\p[\tau_b\les t_*]$. The other steps are the same as in Algorithm~\ref{alg:triple_stable}.

The running time of Algorithm~\ref{alg:triple_stable_conditional_on_time} consists of two parts: (I) the naive sampling of $V_1\eqd S_1$ until $V_1\ges B(t_*)$ in Line~\ref{line:tau<t_*} of Algorithm~\ref{alg:triple_stable_conditional_on_time} and (II) the running time of Algorithm~\ref{alg:undershoot_stable} with random input $t=\tau_b$  in Line~\ref{line:undershoot_t<t_*} of Algorithm~\ref{alg:triple_stable_conditional_on_time}, where $\tau_b$ is conditioned to be smaller or equal to $t_*$. Let $T_\alpha(t)$ be the expected runtime of Algorithm~\ref{alg:undershoot_stable} with parameter $t$. Then the running time of Algorithm~\ref{alg:triple_stable_conditional_on_time} is bounded by
\[
...
\]
where $\E[T_\alpha(\tau_b)]\les\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)$ by Theorem~\ref{thm:expectation_of_time_complexity}.It remains to lower bound $\p[S_1\ges B(t_*)]$. 

Pick $\lambda>0$ and note that
\[
\p[S(t_*)< b(t_*)]
\les\E[\me^{\lambda b(t_*)-\lambda S(t_*)}\1\{S(t_*)<b(t_*)\}]
\les\E[\me^{\lambda b(t_*)-\lambda S(t_*)}]
...
Denote $\chi_b\coloneqq(\tau_b,S_{\tau_b-},S(\tau_b))$, fix some $t>0$ and recall that $\{\tau_b\les t\}\in\mF_t$. For any measurable $A\subset[0,t]\times[0,\infty)^2$ we have $\{\chi_b\in A\}\in\mF_t$ and hence %by Doob's optional sampling theorem~\cite[Thm~7.12]{MR1876169}, we have
\begin{equation*}
\p_q\big[\chi_b\in A\big]
=\E\big[\1_A(\chi_b)M(t)\big]
...
\begin{proof}[\textbf{Proof of Theorem~\ref{thm:tempered_expect_time}}]
For a fixed time $t_*$, Algorithm~\ref{alg:triple_temper_stable} first samples the event $\{\tau_b\les t_*\}$ under $\p$. Indeed, this is done by sampling $s\sim\mathcal{L}(S_{t_*})$ under $\p_q$ and noting that the event $s\les b(t_*)$ is equivalent to the corresponding tempered stable process, of which $s$ is the final value, to not cross the boundary on the time horizon $[0,t_*]$. Thus, if $s\les b(t_*)$, by the Markov property of $S$, we may simply start a new iteration from the initial time-space point $(t_*, s)$. If instead $s<b(t_*)$, we need to sample $\chi_b|\{\tau_b\les t_*\}$ under $\p_q$. Moreover, in that case, by~\eqref{eq:esscher_tau}, we need only repeatedly sample $\chi_b$ under $\p$ conditional on $\tau_b\les t_*$ and an independent $E\sim\Exp(1)$, until $E\ges q S_{t_*}$. For that, Algorithm~\ref{alg:triple_stable_conditional_on_time} samples $(t,u,v)\sim\mathcal{L}(\chi_b^S)$ under $\p$ conditional on $\tau_b\les t_*$. By the strong Markov property, to check if $E\ges qS_{t_*}$, we need only sample independently (and conditionally given $\tau_b=t$) $w\sim \mathcal{L}(S_{t_*}-S_{t})=\mathcal{L}(S_{t_*-u})$ via Algorithm~\ref{alg:stable} and set $S_{t_*}=v+w$. This justifies the validity of Algorithm~\ref{alg:triple_temper_stable}.

Now let us consider the computational complexity of Algorithm~\ref{alg:triple_temper_stable}. The expected number of iterations in Line~\ref{line:loop_until_tau<t_*} of Algorithm~\ref{alg:triple_temper_stable} is
\[
\E_q[\lfloor\tau_b/t_*\rfloor+1]\les \E_q[\tau_b]/t_*+1.
\]
In each iteration we use Algorithm~\ref{alg:tempered_stable} with expected complexity uniformly bounded by $c_0\coloneqq 4.2154$ according to~\cite[Thm~3.1]{TemperedStableDassios}. After that, we sample the triplet conditional on $\tau_b\les t_*$ via Algorithm~\ref{alg:triple_stable_conditional_on_time}. By Corollary~\ref{cor:triple_stable_small}, the expected running time of Algorithm~\ref{alg:triple_stable_conditional_on_time} is bounded by
\[
\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{\p[S(t_*)\ge b(t_*)]}.
\]

The acceptance probability in Line~\ref{line:check_temper} of Algorithm~\ref{alg:triple_temper_stable} is 
\[
\p[E\ges qS_{t_*}|S_{t_*}\ges b(t_*)]=\E[\me^{-qS_{t_*}}\1\{S_{t_*}\ges b(t_*)\}]/\p[S_{t_*}\ges b(t_*)].
\]
All in all, the expected running time of Algorithm~\ref{alg:triple_temper_stable} is bounded by 
\[
c_0\bigg(1+\frac{\E[\tau_b]}{t_*}\bigg)
+\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{\E[\me^{-qS_{t_*}}\1\{S_{t_*}\ges b(t_*)\}]}.
\]
...
\begin{align*}
\E[\me^{-qS(t_*)}\1\{S(t_*)< b(t_*)\}]
&\les\E[\me^{-2qS(t_*)+qb(t_*)}\1\{S(t_*)<b(t_*)\}]\\
&\les\E[\me^{-2qS(t_*)+qb(t_*)}]
...
\begin{multline*}
\E[\me^{-qS(t_*)}\1\{S(t_*)\ge b(t_*)\}]
=\me^{-q^\alpha\theta t_*}-\E[\me^{-qS(t_*)}
    \1\{S(t_*)< b(t_*)\}]\\
...
Denote $T_1\coloneqq qb(0)/(2^\alpha-1)$, $T_2\coloneqq qb(0)$ and set $t_*=(2qb(0)+1-2^{-\alpha})/((2^\alpha-1)q^\alpha\theta)=(2T_1+2^{-\alpha})/(q^\alpha\theta)$ and note that $2^\alpha-1\in[\alpha\log2,\alpha]$. Then the expected running time of Algorithm~\ref{alg:triple_temper_stable} is bounded by
\begin{align*}
&c_0\bigg(1+\me \frac{(1+1/(qb(0)))^{1-\alpha} qb(0)
        }{\alpha q^\alpha\theta t_*}\bigg)
+\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{((2^\alpha-1)q^\alpha\theta t_*-2qb(0))\me^{-2^\alpha q^\alpha \theta t_*+ 2qb(0)}}\\
=\,&c_0\bigg(1+\me \frac{(1+1/T_2)^{1-\alpha}T_2
        }{\alpha (2T_1+2^{-\alpha})}\bigg)
+\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{((2^\alpha-1)(2T_1+2^{-\alpha})-2T_2)\me^{-2^\alpha (2T_1+2^{-\alpha}) + 2T_2}}\\
=\,&c_0\bigg(1+\me \frac{(1+1/T_2)^{1-\alpha} T_2
        }{\alpha (2(2^\alpha-1)T_2+2^{-\alpha})}\bigg)
+\me^{1+2T_1}\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{1-2^{-\alpha}}\\
\les\,&c_0\bigg(1+\frac{\me}{\log 2}\1_{\{T_2\ges 1\}}
        +\frac{4\me}{\alpha}\1_{\{T_2<1\}}\bigg)
+2^\alpha\me^{1+2T_1}\frac{1+\kappa_{\ref{alg:triple_stable}}((1-\alpha)^{-3}+|\log\alpha|+\log N)}{\alpha\log2}\\
\les\,&\kappa_{\ref{alg:triple_temper_stable}}\me^{2T_1}((1-\alpha)^{-3}+|\log\alpha|+\log N)/\alpha,
\end{align*}
where $\kappa_{\ref{alg:triple_temper_stable}}$ is a constant that depends neither on $\alpha\in(0,1)$, $\theta\in(0,\infty)$ nor $q\in(0,\infty)$.
\end{proof}

\begin{proof}[\textbf{Proof of Corollary~\ref{cor:improved_tempered_expected_time}}]

Algorithm~\ref{alg:improved_triple_temper_stable} applies Algorithm~\ref{alg:triple_temper_stable} at most $1+\lfloor b(0)/R\rfloor$ times. In each application, the expected running time is bounded by 
\[
\kappa_{\ref{alg:triple_temper_stable}}\me^{2qR/(2^\alpha-1)}((1-\alpha)^{-3}+|\log\alpha|+\log N)/\alpha.
\]
With our choice of $R=(2^\alpha-1)/(2q)$, the average running time of Algorithm~\ref{alg:improved_triple_temper_stable} is bounded by
\[
\kappa_{\ref{alg:triple_temper_stable}}\me(1+2qb(0)/(2^\alpha-1))((1-\alpha)^{-3}+|\log\alpha|+\log N)/\alpha.
\]
...
Theorem~\ref{thm:joint_law} below and its proof are taken from~\cite{chi2016exact}, see also~\cite{https://doi.org/10.48550/arxiv.2205.06865}. This appendix is included for completeness, because Theorem~\ref{thm:joint_law} underpins our key Algorithm~\ref{alg:undershoot_stable}.

Let $S$ be a subordinator 
...
\begin{thm}
\label{thm:joint_law}
 For a subodinator $S$ satisfying assumptions in the previous paragraph and
...
\begin{proof}[{\textbf{{Proof of Theorem~\ref{thm:joint_law}}}}]
...
and the fact that $S_t$ has a density imply $\p(S_{\tau_b-}<b(\tau_b),S_{\tau_b}=b(\tau_b))=0$.
...
\begin{align*}
\{t-\epsilon<\tau_b\leq t\} & =\{S_{t-\epsilon}<b(t-\epsilon)\}\cap\{S_t\geq b(t)\} \\
...
\begin{proof}[{\textbf{{Proof of Lemma~\ref{lem:exp_moment}}}}]
...
Fix any $\theta>0$ and $\alpha\in(0,1)$. Let $S$ be a driftless stable subordinator under the probability measure $\p$ with L\'evy measure $\nu(\md x)=w_\alpha^{-1}\theta x^{-\alpha-1}\md x$. Denote by $(\mG_t)_{t\ge 0}$ be the right-continuous filtration generated by $S$. Given $q\ges 0$, let $\p$ be the probability measure with Radon--Nikodym derivative 
\[
...
More precisely, 
 ...
 and, by~\cite[Section~4.4]{uchaikin2011chance}, for any $x\in(0,\infty)$ we have
...
\begin{breakablealgorithm}
\caption{Simulation from the law of $S_t$
under $\p_0$}
\label{alg:stable}
\begin{algorithmic}[1]
...
which clearly equals the distribution function of the output of Algorithm~\ref{alg:stable}. Moreover, Algorithm~\ref{alg:stable} clearly has constant computational cost, which does not depend on the parameters $\theta,t,\alpha$.
...
\begin{breakablealgorithm}
\caption{({\cite[Alg.~3.1]{TemperedStableDassios}}) Simulation from the law of $S_t$ under $\p$}
...
\label{sec:devroye}
Devroye's extremely efficient method~\cite{devroye2012note} for the simulation from a log-concave densities is crucial in several steps of our main Algorithm~\ref{alg:triple_stable}. The general algorithm in~\cite{devroye2012note} samples from densities defined on $\R_+$. Here we phrase it for log-concave densities defined on $[0,1]$, as all the densities arising in 
Algorithm~\ref{alg:triple_stable}
...
\begin{breakablealgorithm}
\caption{(Devroye~\cite{devroye2012note}) Simulation from a log-concave density on $[0,1]$}
\label{alg:Devroye}
...
\end{algorithmic}
\end{breakablealgorithm}
Computational cost of Algorithm~\ref{alg:Devroye} consist of two parts: the computational cost of finding $a_1$,
...
In Algorithm~\ref{alg:psi_2}, we will apply Newton--Raphson method (Algorithm~\ref{alg:inversion_newton_raphson} below) to find the root $x_*$, satisfying $f(x_*)=0$, of an increasing function $f:(0,1)\to\R$. The search terminates when the computed approximation to $x_*$ coincides with the actual root $x_*$ in the first $N$ bits (e.g. in double-precision floating-point arithmetic $N=53$). 

In order to control the running time of Algorithm~\ref{alg:inversion_newton_raphson} when applied to
a twice differentiable increasing function $f:(0,1)\to\R$ with root $x_*$, we need to have access to an auxiliary function $M:[0,1]\times\mathbb N\to\R_+$ such that 
...
\end{equation}
In practice, for a specific function $f$, $M(x_0,k)$  may, but need not, depend on either $x_0$ or $k$ (see e.g. Proposition~\ref{prop:inverse_sigma}~\ref{prop:inverse_u*sigma}\ref{prop:inverse_c1}).
...
\textit{a priori} estimates on $f$, see proofs of .... above.
...
Algorithm~\ref{alg:inversion_newton_raphson}
...
finds a starting point $x_0$ for the Newton--Raphson method to have quadratic convergence. In fact, the error sequence $(\ve_n)_{n\in\N}$ (where $\ve_n:=|x_n-x_*|$ is the error at the $n$-th iteration) satisfies $|\ve_{n+1}|\les M\ve_n^2,$
implying that $|\ve_n|\les M^{2^n-1}\ve_0^{2^n}$. Thus, for the Newton--Raphson methodology to attain quadratic convergence, it suffices to find $x_0$ such that $M\ve_0\les 1/2$, as this would imply that $|\ve_n|\les (1/2)^{2^n-1}$, making $N$ bits of precision attainable in $\log N$ iterations. The computational cost of Algorithm~\ref{alg:inversion_newton_raphson} has two parts: (I) the cost to finding the value of $x_0$ and (II) the cost to compute the Newton--Raphson sequence $x_{n+1}=x_n-f(x_n)/f'(x_n)$ until the error reaches desired precision. (I) depends on $2^{1-k}M(x_0,k)$ and is in fact bounded above by $N$. (II) is bounded by $\log N$ as we explained before.
