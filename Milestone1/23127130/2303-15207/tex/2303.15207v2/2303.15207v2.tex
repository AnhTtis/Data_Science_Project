...
%v2
$ ds^2_{{\rm CP}^3}=  {d w^s d \bar w^s\ov 1 + |w|^2}   - { w_r \bar w_s   \ov (1 + |w|^2)^2 } d  w^s d   \bar w^r $
...
where  the M2 brane tension %v2
 is\foot{The  universal expression  for the M2 brane tension  in terms of  the coefficient $\kappa_{11}$ in the 11d  theory action ${1\ov 2 \kappa_{11}^2} \int d^{11} x \sqrt{ -g}\,  R + ...$  is \ci{Klebanov:1996un}:
$T_2 = ( 2\pi)^{2/3} (2 \kappa_{11}^2)^{-1/3}$.  Here $\ell_{\rm pl}$ is defined so that 
$2 \kappa_{11}^2 = (2 \pi)^8 \ell_{\rm pl}^9$  as, e.g., in \ci{Bagger:2012jb}.}
\begin{equation}
...
%We see that, as expected,
 ...
 $e^{-S_{\rm M2}^{\rm cl.}}$ precisely matches the exponential in the localization prediction (\ref{loc-largeN}).\foot{This AdS$_2\times S^1$ solution was  considered
    ...
    was demonstrated 
     in  \ci{Farquet:2013cwa}. Supersymmetries of similar M2-brane solutions  were considered also in 
 %v2
  \ci{Lietti:2017gtc}).}  
...
%can be  actually resummed  into  %follows 
  \be  \la{33}
  \langle  W_{1\ov 2} \rangle = {1  \ov 2  \sin \big( \sqrt\frac{\pi}{2}\,\frac{\gs}{\sqrt T}\big)   } \, e^{2\pi\,T}  \Big[1+ O(T^{-1})\Big] \ , \ee
...
One natural generalization of our calculation is to consider the $1\ov6$-BPS Wilson loop 
%v2
\ci{Drukker:2008zx,Chen:2008bp}. In this case the localization result derived in \cite{Klemm:2012ii}, expanded in the large $N$ fixed $k$ limit, gives
\be 
