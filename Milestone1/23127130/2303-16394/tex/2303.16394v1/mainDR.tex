...
\usepackage{color} \usepackage{a4wide} \usepackage{amsmath,amssymb,amsthm,amsfonts}
\newcommand{\R}{\mathbb{R}}
\newcommand{\DRE}{\varphi_{\gamma}^{\mbox{\tiny{DR}}}}
\newcommand{\x}{\sigma}
...
linkcolor = {green} } \usepackage[backend=biber, style=numeric, sortlocale=auto,
%natbib=true, doi=false, isbn=false, url=false,eprint=false,
giveninits=true,maxnames=6]{biblatex} \usepackage{biblatex}
...
\author{Felipe Atenas\thanks{IMECC/UNICAMP, Campinas, S\~{a}o Paulo, Brazil.
e-mail: f262900@dac.unicamp.br. The author is supported by FAPESP grant 2019/20023-1. }}
...
\noindent{{\bf Key words.} Nonsmooth nonconvex optimization, weak convexity, Douglas-Rachford splitting, problem
decomposition, Progressive hedging}


\noindent{{\bf AMS subject classifications.} 90C30, 90C15,  90C26, 49J52, 65K05, 65K10,}
...
\begin{equation} \label{EB-f} \tag{EB} d\bigl(x, (\partial
...
if \begin{equation} \label{PSIS} \tag{PSIS} \forall u,v \in (\partial
\varphi)^{-1}(0), \: \| u - v \| \le \varepsilon \implies \varphi(u) =
...
\eqref{DR:scheme}, $v^k$ corresponds to a solution to the following problem \vspace{-0.2cm} \begin{equation} \label{DRE:moreau} \min_{v} \left\{ \varphi_2(v) +
...
\R$ is defined as \vspace{-0.2cm} \begin{equation} \label{DRE} \DRE(s) =
...
from \eqref{DRE} that \vspace{-0.1cm}\begin{equation}\DRE(s^{\star}) \le \varphi_1(u^{\star}) + \langle \nabla     \varphi(u^{\star}), u^{\star} - u^{\star} \rangle + \varphi_2(u^{\star}) +
...
Lagrangian, we then have \vspace{-0.3cm}\begin{equation} \label{Lagr:aux-est} \varphi_1(u^k) +
...
large $k,$ \vspace{-0.2cm} \begin{equation*} |\varphi_1(v^k) - \varphi_1(u^k)| \le M_1 \eta +
\dfrac{L}{2}\eta^2.  \end{equation*} Therefore,  $\varphi(v^k) \le
\varphi_1(u^k) + \varphi_2(v^k) + M_1 \eta + \dfrac{L}{2}\eta^2$.  Combining
this last inequality with \eqref{Lagr:aux-est}, it holds \vspace{-0.1cm} \begin{equation*} \begin{aligned} \varphi(v^k)   & \le  \varphi^{\star} +
...
sequences are bounded, then there exists $M_2>0$ such that $\| u^k-s^k\| \le M_2.$ Therefore, for all sufficiently large $k,$ \vspace{-0.15cm} \begin{equation*} \varphi(v^k)
...
\mbox{proj}_{(\partial_L \varphi)^{-1}(0)}(v^k),$ define \vspace{-0.15cm} \[p^k = (p_v^k, p_v^k,
\gamma^{-1}(p_v^k - s^k)). \] Then, there exists $\overline{C}>0$, such that for
all $k$, \vspace{-0.15cm} \begin{equation} \label{pk-xk} \| p^k - \x^k\|^2 \le
...
Therefore,  \vspace{-0.1cm} \begin{equation*} \| p^k - \x^k\|^2 \le 2(1+ \gamma^{-2})  (\ell^2 \|
...
\begin{algorithmic}[1]
    ...
    \State \textbf{Projection}: define \vspace{-0.15cm}\begin{equation*}
        \begin{array}{rcl}
        z^k &=& \left(
    ...
    P_{\mathcal{C}}[s^k],  \\
        ...
        \end{array} 
    \end{equation*} \label{AL-up:uk}  \vspace{-0.15cm}   
    
   \State \textbf{Primal subproblems:} find a solution $x^k$ of \vspace{-0.15cm} \begin{equation}   \label{subproblem:primal} \min_{x \in \R^n} \left\{ f(x) +
   \langle w^k ,
   x\rangle + \dfrac{1}{2 \gamma} \| x - z^k \|^2 \right\}. \end{equation} \label{AL-up:vk} \vspace{-0.15cm}
    ...
    In addition, assume  $f$ is a weakly convex function,  conditions \eqref{EB-f} for and \eqref{PSIS} hold for $\varphi = f + \frac{\mu}{2}d_{\mathcal{C}}^2$, as well as condition \eqref{CQ}. Then:
        \begin{itemize}
...
critical points (resp., solutions) of \eqref{primal:2}. Our next result shows
that those limit points will be critical points (resp., solutions) of
problem \eqref{primal:1}.

 \begin{lemma} \label{lem:prim-rel}

Suppose that $\mathcal{C} \subseteq \R^n$ is a nonempty closed convex set, and $f$ a proper lower semicontinuous function. 
\begin{itemize}
    \item[(i)]  Additionally, suppose the following constraint qualification holds at any critical point $x^{\star}$ of the unconstrained problem \eqref{primal:2}: \begin{equation} \label{CQ} \tag{CQ}
        - \partial f(x^{\star}) \cap N_{\mathcal{C}}(P_{\mathcal{C}}[x^{\star}]) = \{0\}.
    \end{equation}   Then, $x^{\star}$ also a critical for the constrained problem \eqref{primal:1}.
    \item[(ii)] Any solution of problem \eqref{primal:2} is also a solution of problem \eqref{primal:1}. 
\end{itemize}
...
\begin{itemize}
    \item[(i)] {Let $x^{\star} \in \R^n$ be a critical point of \eqref{primal:2}, that is, \[0 \in \partial \left(f + \frac{\mu}{2}d_{\mathcal{C}}^2\right)(x), \quad \mbox{ and } x^{\star} \in \mbox{dom}(f).\] Since $\frac{\mu}{2}d_{\mathcal{C}}^2$ is differentiable, then from \cite[Exercise 8.8(c)]{rockafellar2009variational} and \eqref{d-derivative}, \[ \partial \left(f + \frac{\mu}{2}d_{\mathcal{C}}^2\right)(x^{\star}) = \partial f(x^{\star}) + \mu( x^{\star} - P_{\mathcal{C}}[x^{\star}]). \] Consequently,  $\mu(x^{\star} - P_{\mathcal{C}}[x^{\star}]) \in- \partial f(x^{\star}) .$ Furthermore, from the definition of normal cone, $\mu(x^{\star} - P_{\mathcal{C}}[x^{\star}]) \in N_{\mathcal{C}}(P_{\mathcal{C}}[x^{\star}])$, thus $0 \in \partial f(x^{\star}) + N_{\mathcal{C}}(P_{\mathcal{C}}[x^{\star}])$. In this way, \eqref{CQ} implies that $x^{\star} = P_{\mathcal{C}}[x^{\star}], $ from where it holds that $0 \in \partial f(x^{\star}) + N_{\mathcal{C}}(x^{\star})$.
    }
    
    \item[(ii)] Let $x^{\star}$ be any solution to \eqref{primal:2}. Then, for any $x \in
\mathcal{C}$, \begin{equation*} f(x^{\star}) + \dfrac{\mu}{2}d_{\mathcal{C}}^2(x^{\star}) \le
f(x).  \end{equation*} 

Since $\dfrac{\mu}{2}d_{\mathcal{C}}^2(x^{\star}) \ge 0,$ then $f(x^{\star})  \le
f(x)$, from which we deduce that $x^{\star}$ is a solution to \eqref{primal:1},
if and only if, $x^{\star} \in \mathcal{C}.$ In this manner, by way of
contradiction, assume that $x^{\star} \notin \mathcal{C}$, then
$d_{\mathcal{C}}(x^{\star}) > 0,$ and $x^{\star}$ do not solve problem
\eqref{primal:1}. Hence, there exists $\overline{x} \in \mathcal{C}$, such that
$f(\overline{x}) < f(x^{\star})$, and thus \begin{equation*} f(\overline{x}) + \dfrac{\mu}{2}d_{\mathcal{C}}^2(\overline{x})
< f(x^{\star}) + \dfrac{\mu}{2}d_{\mathcal{C}}^2(x^{\star}), \end{equation*}
which implies that $x^{\star}$ is not a solution of \eqref{primal:2}, a
contradiction. 
\end{itemize}
...
in the nonconvex setting. In this case, by making copies of the decision
...
Suppose that the underlying random variable of the problem has finite support, enumerated by $i \in \{1, \dots, N\} \subseteq \mathbb{N}$. 
 ...
 x_{[T-1]}) \in \R^n$ gathers the decision variables of stages $t =1, \dots, T,$
 and $x_{T} \in \R^{n^{\prime}-n}$ represents the decision variable of stage
 $t=T$. Concerning scenarios, for $i= 1, \dots, N$, $x_i \in \R^{n_i}$ denotes a
...
\begin{equation*}
    F(x,x_{T}) = \sum_{i=1}^N p_i F_i(x_i, x_{iT}),
...
\begin{itemize}
    ...
    \item for scenario $i \in \{1, \dots, N\}$, $(x_i, x_{iT})\in \R^{n_i^{\prime}}$ belongs to a nonempty convex compact set $X_i$, where $n_1^{\prime} + \dots + n_N^{\prime} = n^{\prime}$. 
\end{itemize}
Accordingly, the problem to be solved has the following structure
\begin{equation} \label{PH:primal1}\left\{
\begin{aligned}
& \underset{x,x_{T}}{\text{min}}
& & F(x,x_T) \\
& \text{s.t.}
& & x \in \mathcal{N}, \\
&&& (x_i,x_{iT}) \in X_i, \mbox{ for } i = 1, \dots, N.
\end{aligned}\right.
\end{equation}

To cast problem \eqref{PH:primal1} in the format \eqref{primal:1}, we redefine
$F$ by means of a marginal function. Specifically, for all $x \in \R^n$ and $i \in \{1, \dots, N\}$,
we let \begin{equation*}
    f_i(x_i) = \inf_{x_{iT}}\left\{ F_i(x_i,x_{iT}) : (x_i,x_{iT}) \in X_i  \right\},
\end{equation*}
noting that $f_i: \R^{n_i} \to \R$ is lower semicontinuous (by
lower semincontinuity of $F_i$, and compactness of $X_i$, see \cite[
Lemma 1.2]{bauschke2011convex}). Furthermore, 
 the problem defining $f_i$ attains its minimum, which implies that $f_i(x_i)$ is finite for each $x_i$. 
We obtain the following reformulation for problem \eqref{PH:primal1}
...
\begin{algorithmic}[1]
    ...
    \For{$k=0,1, ...$}
   \State \textbf{Primal subproblems:} for each scenario $i=1, \dots, N$, find a solution $(x^k_i, x^k_{iT})$ of  \begin{equation}   \label{subproblem:primal-s} \min_{(x_i, x_{iT}) \in X_i} \left\{ F_i(x_i,x_{iT}) +
    (w^k_i)^{\top}
   x_i + \dfrac{1}{2 \gamma} | x_i - z^k_i |^2 \right\} \end{equation}   
    ...
    \begin{proof}
        ...
        \begin{itemize}
                \item the update rule for $x^k$ in Algorithm~\ref{NonCvxAL} combined with the definition of $f$ yields the following subproblem objective function \vspace{-0.15cm}\begin{equation*}
            \begin{array}{rcl}
                 ...
                 &  = & \displaystyle\sum_{i=1}^N p_i \inf_{\substack{x_{iT} \mbox{ s.t.} \\ (x_i, x_{iT}) \in X_i }} F_i(x_i, x_{iT}) + (w^k_i)^{\top}x_i + \dfrac{1}{2 \gamma}| x_i - z^k_i |^2    
            \end{array}
        \end{equation*} Therefore, solving problem \eqref{subproblem:primal} is equivalent to separately solving \eqref{subproblem:primal-s} for all $i=1,\dots, N,$
    ...
    The convergence of Algorithm~\ref{NonCvxPH} follows from Theorem~\ref{th:AL-convergence}, since problem \eqref{PH:primal2} is a special case of problem \eqref{primal:2}.  Observe that, in particular,  \eqref{CQ} takes the following form \begin{equation} \label{CQ2} \tag{CQ$^{\prime}$}
        -\partial f(x^{\star}) \cap \mathcal{N}^{\perp} = \{0\}.
    \end{equation}

\begin{theorem} \label{th:PH-convergence}
    Suppose that $F: \R^{n} \to \R \cup \{+\infty\}$ is a  proper lower semicontinuous function, and  for each $i=1, \dots, N$, $X_i \subseteq \R^{n_i^{\prime}}$ is a nonempty convex and compact set. Then, for $\lambda \in (0,2)$, and $0 < \gamma < \frac{2-\lambda}{2L}$, any bounded sequence $\{(z^k,x^k,s^k)\}$ generated by Algorithm~\ref{NonCvxAL} satisfies:
    ...
    \begin{itemize}
        ...
        \item[(ii)] The sequence $\{z^k - x^k\}$ converges to $0$,  all cluster points of $\{z^k\}$ and $\{x^k\}$ coincide, and are also critical points of $f + \frac{\mu}{2}d_{\mathcal{N}}^2$, with critical value  $\varphi^{\star}$. Additionally, if \eqref{CQ2} holds at any of such cluster points, such points are critical for problem \eqref{PH:primal2}, and all cluster points of the sequence $\{(x^k, x^k_T)\}$ generated by solving \eqref{subproblem:primal-s}, are critical points of problem \eqref{PH:primal1}.
    \end{itemize}

    In addition, suppose that $\{F_i: \R^{n_i} \to \R \cup \{+\infty\}\}_{i=1}^N$ is a family of weakly convex functions, and   conditions \eqref{EB-f} and \eqref{PSIS} hold for $\varphi = f + \frac{\mu}{2}d_{\mathcal{N}}^2$, as well as condition \eqref{CQ2}. Then, the following holds.
    ...
    \begin{proof}
        Problem \eqref{PH:primal1} has a nonempty set of solutions, since $F$ is proper lower semicontinuous, and for $X = \Pi_{i=1}^N X_i$, $X \cap \mathcal{N}$ is a compact set, because $X$ is compact and $\mathcal{N}$ is closed. The result follows directly from Theorem~\ref{th:AL-convergence}, except the last statement of item (ii). In order to prove it, for any $i = 1, \dots, N$, take any cluster point $(x_i^{\star},x_{iT}^{\star})$ of $\{(x_i^k, x_{iT}^k)\} \subseteq X_i$. The optimality conditions of problem \eqref{subproblem:primal-s} read \begin{equation*}
             0 \in \partial_L F_i \left(x^{k}_i, x^{k}_{iT} \right) + N_{X_i} \left(x^{k}_i, x^{k}_{iT} \right)+ \begin{pmatrix} w^k_i\\ 0 \end{pmatrix} + \frac{1}{\gamma} \begin{pmatrix} x^{k}_i - z_i^k\\ 0 \end{pmatrix}.
        \end{equation*} Note that if $s^{\star}$ denotes a cluster point of $\{s^k\}$, then from the update rule of $\{w^k\}$, it follows that $ w^{\star}=\frac{\mu}{1+ \gamma \mu}(s^{\star} - P_{\mathcal{N}}[s^{\star}]) \in\mathcal{N}^{\perp}$ is a cluster point of $\{w^k\}$.   Taking the limit in the optimality conditions, up to a subsequence if necessary, it yields   \[0 \in \partial_L F \left(x^{\star}, x^{\star}_{T} \right) + N_{X} \left(x^{\star}, x^{\star}_{T} \right)+ \begin{pmatrix} w^{\star}\\ 0 \end{pmatrix}, \] that is, $0 \in \partial_L F \left(x^{\star}, x^{\star}_{T} \right) + N_{X} \left(x^{\star}, x^{\star}_{T} \right)+ \mathcal{N}^{\perp} \times \{0\}.$
    \end{proof}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{comp_overall.pdf}
...
Then, for any $y_1, y_2 \in \R^m,$ and setting $ \zeta(t,x_1,x_2) = \phi(tx_1+(1-t)x_2) +
    \frac{\rho}{2}\|tx_1 + (1-t)x_2\|^2$, it follows

    \begin{equation*} \begin{array}{rcl} \zeta(t,x_1,x_2)  & \le & \Phi(tx_1 + (1-t)x_2, ty_1 +
    ...
    y_2 \in \R^m$, it holds that

    \begin{equation*} \zeta(t,x_1,x_2)
    ...
    \begin{equation*} \begin{array}{rcl} \zeta(t,x_1,x_2) &\le& t
...
\textit{Proof of Lemma~\ref{lemma:app}}. For each $i = 1, \dots, N$, apply Lemma~\ref{marginal-w-cvx} for $\Phi = F_i + i_{X_i}$, and $\phi = f_i$. Note that $F_i$ is bounded from below, since whenever $(x_i,x_{iT}) \notin X_i $, $\Phi(x_i,x_{iT}) = + \infty$, and $\Phi(x_i,x_{iT}) = F_i(x_i,x_{iT})$ otherwise. Therefore, each $f_i $ is weakly convex on $\R^{n_i}$, and since $f(x) = \sum_{i=1}^N p_i f_i(x_i)$, it follows that $f$ is weakly convex on $\R^n.$
