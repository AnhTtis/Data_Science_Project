...
We now detail our experimental setup and demonstrate our solution in terms of privacy robustness and data usability, comparing to state-of-the-art de-identification methods and ablated baselines 
...
\noindent\textbf{Comparison.} We consider state-of-the-art de-identification methods, including DeepPrivacy \cite{hukkelaas2019deepprivacy}, CIAGAN \cite{maximov2020ciagan}, Password \cite{gu2020password}, and RePaint \cite{lugmayr2022repaint}.
We also demonstrate the impact that different transformation models and identity/utility experts can have on the proposed method.
For readibility of the tables, we denote these different versions as ``Ours ($a$, $b$, $c$)" where $a$ fixes the identity model(s) $h_\mathcal{Z}^a$ used, $b$ the transformation function $\psi_b^\epsilon$, and $c$ the utility model(s) $h_\mathcal{Y}^c$.
...
As shown in Table \ref{tab:compare-deid}, we outperform other de-identification methods in both data anonymization and image quality by a large margin. 
...
As justified in Section~\ref{sec:metho} and experimentally measured in Table \ref{tab:compare-invertibility}, $\psi_{\text{opp}}$ would suffer high re-identification. 
...
MLP-based transformations relying on multiple identity extractors, \ie, ``Ours ($mix, mlp, eye$)", perform better than versions with only one ID expert. We attribute the increased robustness to the combined knowledge of the two algorithms which capture more varied ID-related features that are then obfuscated. VED-based transformations yield better results in terms of de-identification and non-invertibility, confirming the superiority of our proposed solution.
...
\input{tables/tab-compare-utility-both}
\input{tables/tab-compare-utility-training}

\noindent\textbf{Utility corruption in anonymized images.}
Our methods demonstrate substantial advantages over previous work in preserving utility across various datasets, as shown in Table \ref{tab:compare-utility}. 
