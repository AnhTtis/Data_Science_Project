...
We proposed \textit{Disguise}, a model for face de-identification that ensures both the privacy of depicted people and  usability of altered images. 
Through experiments we showed how it can pre-process sensitive data before their use in inference or training contexts.
...
One should note that our current model is tailored for face obfuscation and does not consider other visual attributes that may leak identity (\eg, conspicuous glasses or haircut, background, \etc). This could, however, be tackled by considering broader ID-extracting methods $H_\mathcal{Z}$ \cite{bhanu2017deep}.
We also acknowledge that \textit{Disguise} could benefit from research in multi-objective learning \cite{desideri2012multiple,sener2018multi,momma2022multi} to find better optima for cases when identity and utility features overlap.
