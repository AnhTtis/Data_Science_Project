% \begin{table}[h]
% \centering
% \small
% \caption{Compare de-id performance of different face anonymization methods.}
% \label{tab:compare-deid}

% \resizebox{\linewidth}{!}{
% \begin{tabular}{l@{\hspace{0.2em}}l@{\hspace{0.15em}}l@{\hspace{0.15em}}l|ccc|c}
% \toprule
% \multicolumn{4}{c|}{\multirow{2}{*}{Methods}} & \multicolumn{3}{c|}{TPR (\%) @ FPR=1e-3 / Accuracy (\%) $\downarrow$} & FIQ $\uparrow$ \\
% \multicolumn{4}{c|}{} & FaceNet & SphereFace & AdaFace & SER \\ \midrule
% \multicolumn{4}{c|}{Original} & 93.83 / 97.1 & 87.90 / 96.2 & 95.43 / 97.7 & 0.769 \\
% \multicolumn{4}{c|}{DeepPrivacy} & 7.33 / 73.8 & 2.93 / 70.9 & 4.57 / 68.6 & 0.670 \\
% \multicolumn{4}{c|}{CIAGAN} & 1.84 / 64.5 & 1.04 / 59.0 & 5.62 / 71.0 & 0.583 \\
% \multicolumn{4}{c|}{Password} & 31.67 / 79.1 & 17.07 / 73.5 & 51.00 / 84.0 & \multicolumn{1}{l}{} \\
% \multicolumn{4}{c|}{RePaint} & 2.84 / 67.7 & 1.14 / 63.5 & 3.58 / 68.5 & \multicolumn{1}{l}{} \\ 
% \hline
% \multicolumn{4}{c|}{Random} & {0.34 / 51.4} & {0.18 / 51.1} & {0.20 / 51.1} & 0.769 \\ 
% \hline
% \multicolumn{4}{c|}{Opposite} & 0.63 / 51.5 & 0.03 / 50.0 & 0.03 / 50.0 & \multicolumn{1}{l}{} \\
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ar}$,} 
% & {\footnotesize $\psi^\epsilon_{\text{ved}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{emo})$} 
% & 0.23 / 50.1 & 0.00 / 50.0 & 0.07 / 50.0 & 0.896 \\
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ar}$,} 
% & {\footnotesize $\psi^\epsilon_{\text{ved}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{eye})$} 
% & 0.03 / 50.0 & 0.03 / 50.0 & 0.00 / 50.0 & 0.897 \\
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ar}$,} 
% & {\footnotesize $\psi_{\text{mlp}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{emo})$} 
% & 0.03 / 50.0 & 0.00 / 50.0 & 0.03 / 50.0 & 0.901 \\
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ar}$,} 
% & {\footnotesize $\psi_{\text{mlp}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{eye})$} 
% & 0.00 / 50.0 & 0.00 / 50.0 & 0.03 / 50.0 & 0.904 \\
% \hdashline
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ar}$,} 
% & {\footnotesize $\psi_{\text{mlp}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{mix})$} 
% & 0.00 / 50.0 & 0.00 / 50.0 & 0.03 / 50.0 & 0.901 \\
% \hdashline
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ar}$,} 
% & {\footnotesize $\psi_{\text{mlp}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{eye})$} 
% & 0.00 / 50.0 & 0.00 / 50.0 & 0.03 / 50.0 & 0.904 \\
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{ad}$,} 
% & {\footnotesize $\psi_{\text{mlp}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{eye})$} 
% & 3.73 / 70.8 & 0.43 / 65.5 & 2.73 / 66.3 & \multicolumn{1}{l}{} \\
% \hdashline
% Ours 
% & {\footnotesize $(h_\mathcal{Z}^\text{mix}$,} 
% & {\footnotesize $\psi_{\text{mlp}}$,} 
% & {\footnotesize $h_\mathcal{Y}^\text{eye})$} 
% & 0.07 / 50.0 & 0.00 / 50.0 & 0.07 / 50.0 & \multicolumn{1}{l}{} \\ \bottomrule
% \end{tabular}
% }
% \end{table}

\begin{table}[t]
\centering
\small
\caption{Identification / validation rate ($\downarrow$, lower = better) and image quality evaluation ($\uparrow$, higher = better) over edited LFW data.}
\label{tab:compare-deid}

\resizebox{\linewidth}{!}{
\begin{tabular}{l@{\hspace{0.2em}}l@{\hspace{0.15em}}l@{\hspace{0.15em}}r|ccc|c}
\toprule
\multicolumn{4}{c|}{\multirow{2}{*}{Methods}} & \multicolumn{3}{c|}{TPR (\%) @ FPR=1e-3 / Accuracy (\%) $\downarrow$} & FIQ $\uparrow$ \\
\multicolumn{4}{c|}{} & FaceNet & SphereFace & AdaFace & SER \\ \midrule
\multicolumn{4}{l|}{Original} & 93.83 / 97.1 & 87.90 / 96.2 & 95.43 / 97.7 & 0.769 \\
\hline
\multicolumn{4}{l|}{DeepPrivacy \cite{hukkelaas2019deepprivacy}} & 7.33 / 73.8 & 2.93 / 70.9 & 4.57 / 68.6 & 0.670 \\
\multicolumn{4}{l|}{CIAGAN \cite{maximov2020ciagan}} & 1.84 / 64.5 & 1.04 / 59.0 & 5.62 / 71.0 & 0.583 \\
\multicolumn{4}{l|}{Password \cite{gu2020password}} & 31.67 / 79.1 & 17.07 / 73.5 & 51.00 / 84.0 & 0.689 \\
\multicolumn{4}{l|}{RePaint \cite{lugmayr2022repaint}} & 2.84 / 67.7 & 1.14 / 63.5 & 3.58 / 68.5 & 0.540 \\ 
% \hline
% \multicolumn{4}{l|}{Random} & {0.34 / 51.4} & {0.18 / 51.1} & {0.20 / 51.1} & 0.769 \\ 
\hline
Ours 
& {\scriptsize (arc,} 
& {\scriptsize opp,} 
& {\scriptsize \O)}  & 0.63 / 51.5 & 0.03 / 50.0 & 0.03 / 50.0 & 0.809 \\
Ours 
& {\scriptsize (arc,} 
& {\scriptsize ved,} 
& {\scriptsize emo)} 
& 0.23 / 50.1 & \textbf{0.00} / 50.0 & 0.07 / 50.0 & 0.896 \\
Ours 
& {\scriptsize (arc,} 
& {\scriptsize ved,} 
& {\scriptsize eye)} 
& 0.03 / 50.0 & 0.03 / 50.0 & \textbf{0.00} / 50.0 & 0.897 \\
\hdashline
Ours 
& {\scriptsize (arc,} 
& {\scriptsize mlp,} 
& {\scriptsize emo)} 
& 0.03 / 50.0 & \textbf{0.00} / 50.0 & 0.03 / 50.0 & 0.901 \\
Ours 
& {\scriptsize (arc,} 
& {\scriptsize mlp,} 
& {\scriptsize eye)} 
& \textbf{0.00} / 50.0 & \textbf{0.00} / 50.0 & 0.03 / 50.0 & 0.904 \\
Ours 
& {\scriptsize (arc,} 
& {\scriptsize mlp,} 
& {\scriptsize mix)} 
& \textbf{0.00} / 50.0 & \textbf{0.00} / 50.0 & 0.03 / 50.0 & 0.901 \\
\hdashline
Ours 
& {\scriptsize (arc,} 
& {\scriptsize mlp,} 
& {\scriptsize eye)} 
& \textbf{0.00} / 50.0 & \textbf{0.00} / 50.0 & 0.03 / 50.0 & 0.904 \\
Ours 
& {\scriptsize (ada,} 
& {\scriptsize mlp,} 
& {\scriptsize eye)} 
& 3.73 / 70.8 & 0.43 / 65.5 & \cellcolor{gray!33} \color{white} NA \scriptsize{(\cf train/} & 0.870 \\
Ours 
& {\scriptsize (mix,} 
& {\scriptsize mlp,} 
& {\scriptsize eye)} 
& 0.07 / 50.0 & 0.00 / 50.0 & {\scriptsize \cellcolor{gray!33} \color{white} eval overlap)} & \textbf{0.906} \\ \bottomrule
\end{tabular}
}\vspace{-10pt}
\end{table}