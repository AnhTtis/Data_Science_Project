...
\end{align}
%
In the case of consumer households with no photovoltaic panel, the generation $E^{\text{pv}}_{t,i,j} = 0$. When $E^{\text{net}}_{t,i,j} \geq 0$ (called ``shortage'' state), it means there is extra energy needed from external sources (e.g., retailers or other households). When $E^{\text{net}}_{t,i,j} < 0$ (called ``surplus'' state), there is surplus energy available to sell back to the external power grid or other households in shortage. The equation (\ref{eq:net_equilibrium}) presents a constraint that should be satisfied as it is impossible to have both scenarios simultaneously. Finally, we define the objective function of this layer:
\begin{equation}
    \min
    \left\{
        \begin{array}{lr} 
...
\begin{align}
    ...
    \\
        E^{\text{net}}_{t,i} &= E^{\text{sp}}_{t,i} - E^{\text{st}}_{t,i}.
    \label{eq:net_l2}
\end{align}
...
\begin{table}%[htbp!]
  ...
  & CVXPY & MAHTM & COMA \\ \midrule
    ...
    \end{tabular}
    \vskip-5pt
    \caption{Average performance of households. Lower is better for all except reward. }
    \label{tab:results}
\end{table}

Table \ref{tab:results} presents our current empirical results comparing the optimal solution for our scenario using a linear solver (CVXPY), our framework, and COMA, which is considered a state-of-the-art MARL algorithm. One of the things to highlight about our approach is its training speed and simplicity versus COMA, which is very sensitive to hyperparameter tuning. Our framework reached solutions very close to the optimal within a reasonable training time.
%TODO Review update

\section{Conclusion}

The proposed framework systematically applies the MARL technique on transactive microgrids. The results are compared with one classic MARL algorithm. A customized OpenAI Gym environment was also created to serve as the test bench for this work. Our framework can help accelerate the implementation of local renewable energy markets, fostering emission reduction and more consumer engagement.
\\
\\
(The source code and demo files have been anonymized and are available in \href{https://anonymous.4open.science/r/hierarchical-rl} {\textbf{this repository}} link.)
...
In this approach, the objective of our agent is to maximize the probability of having the trajectories that show the higher sum reward. Defined as:
...
In A2C, we use an estimator (a neural network) to represent the policy $\pi_\theta(a_t|s_t)$, named Actor. The actor will map the states to the actions and, from this, will learn which are optimal. Its training follows the next steps:
...
\end{figure}
As shown in the tables above, some houses have no solar energy production (the ones in 0's), which means they need to rely on the battery to make decisions related to the energy. There's also no battery cell price so far, but this is one of the parameters planned to be incorporated in the following steps to see more dynamics in the microgrid. 


\subsection*{Hyperparameters}

In the table \ref{tab:pg_hyperparameters}, we defined the following hyperparameters for the training after fine-tuning using grid search. Since there's less variance in the Advantage Actor-Critic (A2C), the number of epochs needed is less than using a policy gradient (PG). 
...
% The main problem we found implementing the PG algorithm is that the variance creates a challenge for the agent. Learning seems more challenging as it is possible to see the results after enabling noise. We fixed this error by improving the PG: adding the advantage function and the critic, the A2C algorithm. The immediate impact was noticed in the training time and the attainable results.
