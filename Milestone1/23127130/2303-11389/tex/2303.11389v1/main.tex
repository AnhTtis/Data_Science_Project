...
learning, deep metric learning, image classification, ensembles of classifiers.
\end{IEEEkeywords}
...
\begin{table}%[ht!]
    \centering
     \caption{Accuracy results (\%), standard deviation ($\pm$), and the number of classifiers ($[.]$) used by fusion strategies MV and UMDA in a 5-folds cross-validation protocol. With \textbf{$^*$} are best baseline classifiers for each dataset.} 
...
\begin{enumerate} 
 ...
 and the creation of a stronger final classifier (ensemble of classifiers);
