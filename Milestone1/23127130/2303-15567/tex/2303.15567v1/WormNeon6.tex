...
and match those of the well-established $K^\pi=0^+$, $K^\pi=0^-$,
and certain higher rotational bands of Neon-20.}
\end{center}
...
underlying configuration space is a spatial wormhole. Spacetime
wormholes have generated much interest, but are usually regarded as a
science-fiction fantasy. Here we find a wormhole that is physically
...
well-separated, and additionally has the usual spherical polar
coordinates $(\theta,\phi)$ representing the direction of the
separation axis. Unusually, however, $r$ runs from $-\infty$
to $\infty$. The metric is that
of an Ellis--Bronnikov wormhole \cite{Ell,Bro}
\be
ds^2 = dr^2 + (r^2 + a^2)(d\theta^2 + \sin^2 \theta \, d\phi^2) \,.
\label{metric}
\ee
On this background, there is a smooth, attractive potential energy
depending symmetrically on $r$, with its minimum at $r=0$ where the
clusters merge into the bipyramid. The radius of the wormhole's
throat, $a$, is related to the linear size of the bipyramid. 

To justify this geometry, consider a simple classical motion with
$r$ monotonically decreasing
through zero, and $(\theta,\phi)$ fixed. This models an incoming
alpha-particle coalescing instantaneously with the four-alpha cluster
to form the bipyramid, and a different alpha-particle from the opposite
side of the bipyramid getting ejected. The motion can be thought of as
...
reduced mass of the two clusters at large separation, and $V_0$ is a positive constant so
...
\ee
$l$ is the usual angular momentum quantum number, and the
...
\chi = E \chi \,.
\ee
We recall that bound states of the potential $V=\mbox{const}\cdot
(r^2+a^2)^{-2}$ with zero angular momentum have, in the context of kinks
on the wormhole background, been considered in \cite{BDKK} and \cite{Alice}.
...
There is more than one motivation for our choice of potential. It is
short-ranged compared to the centrifugal repulsion proportional to
$l(l+1)$ (although not exponentially so). It is a multiple of the
Ricci scalar curvature of the wormhole, something that could arise
from an alternative treatment of canonical quantization. Finally, the
potential's algebraic form means that for $\e = 0$, eq.(\ref{radial})
usefully simplifies to the associated Legendre equation, which can be
explicitly solved. 
\vspace{4mm}
...
(\ref{1deq}) is a 1-d Schr\"odinger equation with effective potential (Fig.1)
\be
...
\begin{center}
{{\bf Figure 1.} {Effective potential with $m=9$ and $l=7$.}}
\end{center}
\end{center}
For bound states to exist, $v_{\rm eff}$ has to be negative
somewhere, which requires $m^2 - 1 > l(l+1)$. On the other hand, if $m^2 -
1 > 2l(l+1)$ then the integral of $v_{\rm eff}$ over $\R$ is negative, and a
bound state definitely exists (by a variational argument). For general
positive $m$ we therefore expect bound states, but only for $l < m$.

Rather remarkably, there are threshold bound states for all $0 < l <
...
normalisable because $S$ has degree $m-1$. Note that all the threshold
states are physically allowed provided $m$ is odd, because $m-l-1$ has
the same parity as $l$ in this case.
...
angular momenta up to $l=8$. For $m=9$, and $l$ decreasing from $8$ to $1$
...
all the threshold states, is shown in Fig.2. 
\begin{center}
\includegraphics[scale=0.6,angle=0]{osc.png}
\begin{center}
{{\bf Figure 2.} {Numerical bound state spectrum for $m=9$ and integer
...
energies are also shown in Fig.2 for $m=9$, as functions
...
0.77 \,, b_3 = 0.66$ and $b_4 = 0.54$. We see from Fig.2 that these
...
\begin{center}
{{\bf Figure 3a.} {Bound states with $n=0$.}}
\end{center}
\end{center}
\begin{center}
\includegraphics[scale=0.5,angle=0]{wn1.png}
\begin{center}
{{\bf Figure 3b.} {Bound states with  $n=1$.}}
\end{center}
\end{center}
\begin{center}
\includegraphics[scale=0.5,angle=0]{wn2.png}
\begin{center}
{{\bf Figure 3c.} {Bound states with $n=2$.}}
\end{center}
\end{center}
...
wavefunctions are similar in shape if the comparison is made
outside this region.
...
band up to $6^+$, the lowest $0^-$ band up to $7^-$,
and the $0^+_4$ band up to $4^+$. Following refs.\cite{Fuj,Mar}
...
bipyramid. In Fig.4 we show the best fit of these observed states to
our model.

\begin{center}
\includegraphics[scale=0.6,angle=0]{spectrum9_02_new.png}
\begin{center}
{{\bf Figure 4.} {Experimental energies (boxes), numerical bound
...
$\left(0,0, \quart(\sqrt{2} + z) \right)$, accompanied by a single alpha-particle at
...
The moment of inertia about any axis through the origin, and orthogonal to the $x_3$-axis
is found to be
\be
...
The ground state wavefunctions extend from $x=0$ to approximately $x =0.7$. This corresponds to a range of $R$-values, between
$3.65$ fm and approximately $5.3$  fm.
The 
1-phonon wavefunctions have a peak at about $x = 0.32$, corresponding to $R = 4.04$ fm, so the cluster separation hardly exceeds that of the ground state wavefunctions.
In the 2-phonon wavefunctions, the
node is at $x = 0.23$. The cluster separation here
is $R = 3.86$ fm, so the bipyramid is only slightly split. On the other
hand, the peak of the 2-phonon wavefunctions is around
...
We have proposed a model for the states of Neon-20 interpolating
...
short-range potential, with rather simple mathematical form. The
quantum states of the model are interpreted as rotational bands of
the $n$-phonon excitations of the lowest-frequency vibrational mode
of the bipyramid, which tends to produce a $1+4$ cluster split.  
...
shown in Fig.1, is a finite well, and it would be interesting to
