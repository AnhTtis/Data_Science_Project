...
Along with these advantages, our formulation provides a computational
framework for algorithm design. While much of the recent development in this
literature is founded upon mathematical insight, much less is said about how
to use computational resources and prior experimentation to develop new
sampling algorithms.  Our limiting dynamic program offers a novel
computational advantage: sample paths of the model are fully
\emph{differentiable} with respect to the sampling probabilities chosen by the
experimenter.  One can simulate a sample trajectory (or a mini-batch of
trajectories) of the experiment, calculate performance metrics such as
cumulative or simple regret, and compute gradients through
auto-differentiation frameworks, such as Tensorflow~\cite{TensorFlow15} or
PyTorch~\cite{PyTorch19}.  These gradients can then be used to directly
optimize over sampling policies, e.g. policy gradient, or obtain sampling
probabilities by solving a planning problem, e.g. policy iteration from a base
policy such as Thompson Sampling.  This enables novel, efficient
implementations of methods from approximate dynamic programming (ADP) and
reinforcement learning (RL) despite continuous, high-dimensional state and
action spaces.
...
As a demonstration of this framework, we highlight a model predictive control
(MPC) style policy $\algofull$, which at every epoch solves for an optimal
static allocation for the remaining horizon, using currently available
information.
...
observations are used to update the planning problem in the next period,
and can be seen as a dynamic extension of the single-batch problem analyzed
in~\cite{FrazierPo10}.
Our method is optimal among those that do not optimize for potential future
feedback and in particular, is always better than the uniform allocation.
% (This algorithmic design principle is often referred to as the
% open-loop feedback control~\cite[Chapter 6]{Bertsekas12a}.)
Through extensive empirical validation, we demonstrate in
Sections~\ref{section:algorithms-adp} and~\ref{section:experiments} that
...
designing adaptive experimentation methods. In
Section~\ref{section:algorithms}, we observe that the Gaussian sequential
...
MDP are governed by posterior updates based on new observations.


We illustrate our computation-driven paradigm to algorithm development in
Section~\ref{section:algorithms-adp}.  Our MDP formulation gives rise to
several policies derived from reinforcement learning (RL) and approximate
dynamic programming (ADP), which utilize the differentiability with respect to
policy parameters.  Through empirical benchmarking, we find that a simple MPC
policy $\algofull$ consistently outperforms standard policies as well as
carefully tuned reinforcement learning algorithms.  In
Section~\ref{section:experiments}, we perform a thorough empirical comparison
with standard batched bandit policies under different experimental settings,
e.g.  low vs high measurement noise, large vs small batches, flat vs
concentrated priors, etc., and find that $\algo$ outperforms these methods
despite relying on Gaussian approximations.
