...
identification~\cite{QinKlRu17,RussoVaKaOsWe18, ShangHeMeKaVa20,
  Russo20}. When viewed as algorithms for the original pre-limit problem,
...
\paragraph{Batching and delayed feedback in MABs}
...
\paragraph{Asymptotics in bandits and Gaussian approximations}

There is a nascent but quickly growing literature on deriving diffusion limits
for bandit policies~\cite{KalvitZe21, WagerXu21, FanGl21, Adusumilli21, AramanCa22}. These
works typically let the number of reallocation epochs grow to infinity, while
scaling down the gaps in average rewards like in our setting
(Point~\ref{item:diffusion}). \citet{WagerXu21} consider any Markovian policy,
and~\citet{FanGl21} study Thompson sampling with extensions to batched
settings where the batch size is small compared to the number of
reallocation epochs. \citet{KalvitZe21} find a sharp discrepancy between the
behavior of UCB and Thompson Sampling in a related
regime. \citet{Adusumilli21} considers an arbitrary Bayesian policy and derive
an HJB equation for the optimal policy in the limit, which can be solved by
PDE methods. \citet{AramanCa22} consider a sequential testing environment with
two hypotheses involving experiments that 
arrive according to an exogenous Poisson process. 
They derive diffusion process approximations in the limit as
intensity of arrivals grows large and the informativeness of experiments shrinks.
...
time MDP which allows us to use approximate dynamic programming and
reinforcement learning methods to develop new adaptive experimentation
policies.
...
Recently,~\citet{XiongAtBaImb23} consider non-adaptive and adapative designs
of panel data experiments, in which the experimenter assigns subjects form the
control group to the treatment arm over the course of the experiment in order
to maximize the precision of a generalized least squares (GLS) estimator of
the treatment effect.  In this two-armed setting, they consider uncertainty
estimates for the sample variance of the GLS estimator and derive adaptive
designs using Gaussian approximations.  In our work, we focus on the setting
where there are a multiple treatment arms, so an effective adaptive design
must sample strategically to reduce uncertainty for treatments which are
likely to be the best one. Our approach maintains a posterior distribution
over average rewards, rather than an estimate of the uncertainty.  % In
...
to guide the experimental design similar to the works~\cite{FosterIvMaRa21,
  JorkeLeBr22}. But unlike these works, the methods we consider at the end of
Section~\ref{section:algorithms-mdp} use \emph{pathwise} policy gradients of the
value function enabled by the 
reparameterization trick for Gaussian random variables~\cite{KingmaWe14}.
