...
Our main empirical observations inform the design of effective adaptive
experimentation methods when reallocation of sampling effort are few and
expensive. We summarize our results as follows.
...
\item For general reward distributions, the asymptotic Gaussian sequential
  ...
  experimental design, even when batch sizes are small.  Bayesian policies
  derived from the dynamic program~\eqref{eqn:opt-dp} outperform uniform
  allocation and standard batch experimentation policies, including
  specialized algorithms (e.g.  variants of Thompson sampling~\cite{Russo20})
  that require complete knowledge of the reward distribution.
\item Among Bayesian policies that utilize batch Gaussian approximations,
  $\algofull$ achieves the largest performance gain across a wide variety of
  settings.
\item When the number of reallocation epochs is small, $\algo$ calibrates the
  level of exploration to the time horizon by iteratively planning with the
  Gaussian sequential experiment~\eqref{eqn:rho}.
\item Performance gains from $\algo$ are larger in underpowered experiments
  where there are many treatment arms or high measurement noise, settings in
...
In summary, although $\algo$ relies on Gaussian approximations for the rewards
and the prior distribution, performance gains from planning with the Gaussian
sequential experiment greatly outweigh approximation errors. Our empirical
results suggest that $\algo$ excels in settings that are difficult for
standard adaptive policies, including those a limited number of reallocation
epochs, unknown reward distributions, low signal-to-noise ratios, and a large
number of treatment arms. In addition to the key dimensions we highlight in
this section, we provide a comprehensive set of experiments in the following
Streamlit app:
% \vspace{0.3cm}
...
% sampling~\cite{Russo20}) that require complete knowledge of the reward
...
% $s_a^{2} / b_t$.
 ...
 \vspace{-.2cm}
\paragraph{Setup}
As a concrete illustration of our main findings, consider a setting with
$K = \{10, 100 \}$ arms with up to $T=10$ batches. All batches have the same
number of observations in each epoch and we consider two batch sizes:
$b_{t}n = 100$ samples and $b_{t}n = 10,000$ observations per epoch. We
evaluate each policy using the Bayes simple regret~\eqref{eqn:bsr} under the
true prior. For policies that use the Gaussian sequential experiment, we train
them by approximating the true prior with a Gaussian prior with the same mean
and standard deviation in order to preserve conjugacy and utilize the MDP
...
a known $\textsf{Beta}(\alpha,\beta)$ prior.  For
each batch size $b_{t}n = 100$ or $10,000$, we scale the prior parameters
...
has an average reward of $0.5 \pm 0.03$.
...
$\mu_{a}$ are drawn independently from a known $\textsf{Gamma}(\alpha,\beta)$ prior, 
which determines the differences in the average
reward between the arms.  For each batch size $b_{t}n$, we set
$\alpha = b_{t}n$ and $\beta = 1/b_{t}n$ to preserve the difficulty of the
problem as measured by the gap between average rewards.  
...
\begin{figure}[t]
 ...
 \else
  ...
  \fi
    \centering
    \hspace{-.9cm}
      \subfloat[\centering Number of arms $\numarm = 10$.]{\label{fig:num_arm_bsr_10}{\includegraphics[height=5.7cm]{./fig/reg_Bernoulli_100_10_1_Flat.png}} }%
      \hspace{.5cm}
      \subfloat[\centering Number of arms $\numarm = 100$.]{\label{fig:num_arm_bsr_100}{\includegraphics[height=5.7cm]{./fig/reg_Bernoulli_100_100_1_Flat.png} }}%
      \vspace{.4cm}
      \caption{\label{fig:num_arm_bsr} Comparison of performance across
        ...
        Bernoulli with a $\mbox{Beta}$ prior.  $\algo$ delivers larger
...
To benchmark performance, we consider a suite of standard batch bandit
policies proposed in the literature.
\begin{itemize}[itemsep=-2pt, leftmargin=.7cm]
  \item Uniform: For each batch, assign samples uniformly across treatment arms.
  \item Successive Elimination \cite{EvenDarMaMa06, GaoHaReZh19,
  ...
  \item Oracle Thompson Sampling \cite{KalkanhOz21, KandasamyKrScPo18}: Beta-Bernoulli Thompson sampling with batch updates.
  \item Oracle Top-Two Thompson Sampling \cite{Russo20}: Beta-Bernoulli Top-Two Thompson sampling with batch updates.
...
restrict our focus to successive elimination as the key benchmark. This
...
\begin{figure}[t]
  \centering
  \vspace{-1.6cm}
  \includegraphics[height=6cm]{./fig/bar_var_Gumbel_100_100_Flat.png}
    \vspace{.4cm}
    \caption{\label{fig:bar_var} Comparison of performance across different
      ...
      $T = 10$ batches of size $b_{t}n = 100$.  We observe that $\algo$ is
...
number of arms.  This is not the case for all adaptive policies: the relative
performance of the Thompson sampling policies is slightly worse when there are
more alternatives. In these experiments and others presented in the
...
horizons. This illustrates the effectiveness of planning with the Gaussian
sequential experiment for calibrating exploration effort considering a limited
...
policies struggle to learn enough to improve sampling efficiency.  We note
...
high reward treatment arms.  In the high noise regime, the raw performance
...
\begin{figure}[t]
 ...
 \else
  ...
  \centering
  \hspace{-.9cm}
  \subfloat[\centering Batch size $b_{t}n = 100$.]{\label{fig:gumbel_batch_bsr_10}{\includegraphics[height=5.7cm]{./fig/reg_Gumbel_100_100_1_Flat.png}} }%
  \hspace{.5cm}
  \subfloat[\centering Batch size $b_{t}n = 10,000$.]{\label{fig:gumbel_batch_bsr_100}{\includegraphics[height=5.7cm]{./fig/reg_Gumbel_10000_100_1_Flat.png} }}%
  \vspace{.4cm}
  \caption{\label{fig:gumbel_batch_bsr} Comparison of performance across
    ...
    rewards are Gumbel with a $\mbox{Gamma}$ prior. Despite relying on
...
\begin{figure}[t]
    \vspace{-1.6cm}
  \centering
  \includegraphics[height=6cm]{./fig/bar_prior_Bernoulli_100_100_0.25.png}
    \vspace{.4cm}
    \caption{\label{fig:bar_prior} Comparison of performance across  different
      ...
      $\textsf{Beta-Bernoulli experiment}$ where individual rewards are
      Bernoulli with various $\mbox{Beta}$ priors.  When the prior is
...
gains compared to Thompson sampling policies that use the true prior.  This
...
\begin{figure}[t]
 ...
 \else
  ...
  \centering
  \hspace{-1.6cm}
  \subfloat[\centering Measurement variance $s_{a}^{2} = 1$.]{\label{fig:gse_bsr}{\includegraphics[height=5.2cm]{./fig/gse_reg_Gumbel_10000_100_1_Flat.png} }}%
  %\hspace{.05cm}
  \subfloat[\centering Measurement variance $s_{a}^{2} \in \{0.2, 1, 5\}$ for $T = 10$]{\label{fig:gse_var}{\includegraphics[height=5.2cm]{./fig/gse_bar_var_Gumbel_10000_100_Flat.png}} }%
  \vspace{.4cm}
...
training the policy becomes more challenging when there are many treatment
arms and when the measurement variance is low, as the gradients are noisier in
these settings. The versatile performance of $\algo$ across short and long
horizon experiments as well as low and high measurement noise levels shows
that the planning and dynamical aspects of the Gaussian sequential experiment
...
% We compare our methods with Top-Two Thompson Sampling (TTTS) \cite{Russo20}, adapted to the batch-limit setting. That is,
...
% (for the limiting Gaussian sequential experiment), and denote it as ''KG'', due to its similarity with the Knowledge Gradient (KG) policy \cite{FrazierPoDa08}. 
...
% Note that this is not identical to the KG policy in \cite{FrazierPoDa08}, which only samples arms one at a time. 
