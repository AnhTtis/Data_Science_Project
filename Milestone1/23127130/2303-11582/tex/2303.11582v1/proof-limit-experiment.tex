...
positive propensity score (this is a deterministic set).  For the arms not in
$I_{0}$,
...
$r_{0:t}$ only through the propensity scores $\pi_{t+1,a}(r_{0:t})$.
...
realizations of the propensity score
$\pi_{t+1,a}(\sqrt{n}\bar{R}_{0}^{n},...,\sqrt{n}\bar{R}_{t}^{n})$.  This is
complicated by the fact that we allow the propensity scores to be zero.  To
...
\end{align*}
Note that $\Sigma_{n,t+1}$ is not quite equal to the covariance matrix
$\Sigma_{t+1}$ appearing in the limiting Gaussian sequential
experiment~\eqref{eqn:cov}.  This is because for any finite $n$, the
covariance of this Gaussian $\Sigma_{n,t+1}(r_{0:t})$ not only includes the
variance of the arm rewards but also correlations that emerge from sampling
arms according to a random multinomial vector (rather than a deterministic
allocation).  This gap vanishes as $n\to\infty$ and the covariance
matrix converges to the diagonal covariance matrix $\Sigma_{t+1}(r_{0:t})$.
...
Recall the decomposition~\ref{eqn:start} for any bounded Lipschitz function
