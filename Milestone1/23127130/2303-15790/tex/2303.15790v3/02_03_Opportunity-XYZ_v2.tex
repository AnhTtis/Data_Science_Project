...
\item Charmonium-like hybrid candidates are also important targets to be searched for at the STCF, among which the most intriguing one is the lowest $1^{-+}$ state since the quantum numbers are prohibited for quark--antiquark states and from extensive lattice studies, it is expected to be the lowest charmonium-like hybrid. The mass is approximately $4.1\sim4.3$~GeV. In addition, one expects a hybrid supermultiplet including $(0,1,2)^{-+}$ and $1^{--}$ states with nearly degenerate masses of approximately 4.4 GeV~\cite{Liu:2012ze}. At the STCF with $E_\text{cm}\gtrsim4.5$~GeV, the
