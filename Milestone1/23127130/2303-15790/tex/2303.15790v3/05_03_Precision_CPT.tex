...
order in the perturbation expansion has different dimensions and, thus, a distinct observed quantity
...
The particles with the best measured masses are the stable electron and proton, and, according to the
PDG~\cite{ParticleDataGroup:2022pth}:
\begin{eqnarray}
...
\begin{equation}
  |M_{\Kzbar}-M_{\Kz}|\leq 5 \times 10^{-16}~{\rm MeV},
\end{equation}
...
These events are quite distinct at a center-of-mass system $\ee\rt\jpsi$ collider as shown in Fig.~\ref{fig:k0kpi-STCF}a, and occur
...
\caption{\footnotesize {\bf a)}  A simulated $\jpsi\rt K^-\pip\Kz(\tau)$; $\Kz(\tau)\rt\pipi$ event in
  ...
  for the events shown in panel~b.  Here $\phi=\phi_{\rm SW}+\delta\phi^{CPT}_{\eta}$.) 
  }
...
the decay curves for $\Kz(\tau)\rt\pipi$ and $\Kzbar(\tau)\rt\pipi$ events shown in Fig.~\ref{fig:k0kpi-STCF}b,
...
factor of ten better than previous experiments. The issue will be whether or not the systematic errors can be controlled
