...
An average of $(6.28\pm0.32)\%$ for this benchmark mode is quoted by the PDG \cite{PDG}.
The doubly Cabibbo-suppressed decay $\Lambda_c^+\to pK^+\pi^-$ has also been observed by Belle \cite{Belle:DCS} and LHCb \cite{LHCb:DCS}. It is therefore important to search for the two-body modes that are doubly Cabibbo suppressed: $pK^{0(*)}$ and $nK^{+(*)}$.

\begin{table}[hbtp]
\caption{The measured branching fractions of the Cabibbo-allowed two-body decays of $\Lambda_c^+$ (in units of \%) taken from the 2022 Particle Data Group Review of Particle Physics \cite{PDG}. We have included the new BESIII measurements of $\Lambda_c^+\to \Lambda \rho^+$, $\Sigma^{*+}\pi^0$ and $\Sigma^{*0}\pi^+$~\cite{BESIII:2022udq}.} \label{tab:BRs}
\begin{center}
...
sum rules \cite{Carvalho,Huang,Azizi} and lattice QCD \cite{Meinel:LamcLam,Meinel:Lamcn}. Many of the early predictions of
$\B(\Lambda_c^+\to\Lambda e^+\nu_e)$ are smaller than the first measurement of the absolute branching fraction of $(3.6\pm0.4)\%$ reported by BESIII \cite{BESIII:Lambdaenu}. However, the lattice QCD calculations in \cite{Meinel:LamcLam} show good agreement with the experimental results for both $\Lambda_c^+\to\Lambda e^+\nu_e$ and $\Lambda_c^+\to\Lambda \mu^+\nu_\mu$. Needless to say,
...
baryons in heavy hadron chiral perturbation theory to (i) LO \cite{Cheng97,Cheng93}, (ii) NLO \cite{Jiang} and (iii) NNLO  \cite{Wang:2018}.   } \label{tab:em}
\begin{center}
...
The CKM matrix contains a phase that implies the
