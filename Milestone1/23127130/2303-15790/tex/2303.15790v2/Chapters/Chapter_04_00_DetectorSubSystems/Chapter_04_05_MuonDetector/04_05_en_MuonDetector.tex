...
The plastic scintillator detector is more sensitive to photon and neutron particles, which are the main background contributors to neutral hadron detection and identification. The {\sc Geant4}~\cite{mud14} simulation results indicate that with the expected STCF background level described in Sec.~\ref{sec:expcon}, the muon detection efficiency in the momentum range of [0.4, 0.6] GeV/c decreases by 10-20\% if the MUD detectors are all plastic scintillators compared with that of the STCF MUD baseline design.
...
\begin{figure*}[htb]
    ...
    \vspace{0cm}
\caption{The muon detection efficiency curve from the {\sc Geant4} simulation, with a polar angle of 90 deg.
