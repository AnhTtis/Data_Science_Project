...
QCD and hadronic physics, flavor physics and CP violation and the search for new physics. We study the
benchmark process in detail for each category and give the physics sensitivity. Summarized results are
shown in Table~\ref{phyper}.
...
$\tau\to \gamma\mu$  &              -                 &  $5\times10^{-9}$                 &   $1.2\times10^{-8}$  \\
...
With the $K/\pi$ misidentification rate required to be 1\% up to 2~GeV, the statistical uncertainty for $\pi\pi+X$ is $(2\sim7)\times10^{-4}$, and the statistical uncertainty for $KK+X$ is $(7\sim20)\times10^{-4}$ for $1~\rm {ab}^{-1}$ luminosity at $\sqrt{s}=7$~GeV.
...
In addition, the selection efficiency for this process with c.m~energy from $\sqrt{s}=4.0$~GeV to 5.0~GeV, where the cross-section for $e^{+}e^{-}\to\tau^{+}\tau^{-}$ is the
...
\end{center}
\vspace{-0.5cm}
\caption{(a) Invariant mass of $K_{S}\pi^{-}$ with combined $e$-tag and $\mu$-tag from $\tau^{+}$ decay. (b) Selection efficiencies for the signal process at different c.m.~energies from $\sqrt{s}=4.0$~GeV to 5.0~GeV.}
\label{tautokspinu}
\end{figure}
...
The upper limit for
$\tau\to\gamma\mu$ is calculated to be $1.2\times10^{-8}$ with 1~ab$^{-1}$
at $\sqrt{s}=4.26$~GeV.
...
which is currently within 0.2\% ~\cite{Jegerlehner:2008rs}. With more precise cross-sections obtained in experiments, especially at low c.m.~energies, the accuracy of the VP effect will be further improved.
