...
particles (MCPs).
A variety of BSM models predict MCPs;
...
MCPs have previously been searched for
...
the previously allowed MCP parameter space for masses
in the MeV--GeV range \cite{Liu:2018jdi, Liang:2019zkb}.
At the MeV--GeV energy scale,
the existing laboratory constraints
on MCPs include constraints from colliders \cite{Davidson:1991si},
...
for MCPs (solid) and for the SM irreducible BG (dashed)
...
Because the ionization signals from MCPs are
so weak that typical detectors in particle colliders
are unable to detect MCPs directly,
MCPs can be searched for at electron colliders via
...
%are unable to detect MCPs directly.
%Searches
The analysis of searching for MCPs via the monophoton state at the STCF can be
easily extended to a variety of invisible
particles in the hidden sector.
In MCP models, monophoton events can be produced via
$e^+ e^- \to \bar \chi \chi \gamma$,
where $\chi$ is the MCP.
...
has a better sensitivity to kinematically accessible MCPs.
...
\caption{
The expected 95\% C.L.\ upper bounds on MCPs
...
The STCF sensitivity to MCPs in the MeV--GeV mass range is shown
...
electron beam dump experiment for MCPs,
from $\sim$4 MeV to 0.1 GeV.
MCPs with $\epsilon \lesssim (0.8-3) \times 10^{-4}$
...
%
The expected constraints on MCPs from the STCF
...
%
The STCF sensitivities to MCPs at three different collision
energies are compared in Fig.\ (\ref{F-STCF-limit}),
where 10 ab$^{-1}$ of data is assumed for each collision
energy.
%
Although the low-energy mode loses sensitivity to heavy MCPs,
it has better sensitivity than the high-energy mode for
probing light MCPs.
For example, 10 ab$^{-1}$ of data with $\sqrt{s}=2$ GeV can be used to probe MCPs
...
The constraints on MCPs at various collision energies are also shown in
...
\end{tabular}
\caption{
The expected 95\% C.L.\ upper bounds on $\epsilon$ for MCPs 
