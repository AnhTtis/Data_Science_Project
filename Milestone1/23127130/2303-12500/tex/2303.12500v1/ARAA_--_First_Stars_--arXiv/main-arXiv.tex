...
For a collapsing sphere, we can estimate the associated accretion rate from the Jeans mass and the free-fall time $\tau_{\rm ff} = \sqrt{3 \pi / 32 G \rho}$ via:
\begin{equation}
...
\begin{center}
\vspace*{-0.3cm}
\includegraphics[width=10cm]{Figures/klessen-glover-ARAA-fig-02.pdf}
...
We refer to stellar birth in pristine halos containing zero-metallicity gas, which have not been affected by stellar feedback from neighboring halos, as the standard Population III formation pathway. We briefly mention alternative scenarios in Section \ref{sec:alternative-PopIII-pathways}. 
...
\begin{equation}
 L_{\rm Edd} = 4 \pi M_{\bullet} \mu m_{\rm p}c / \sigma_{\rm T} \;, 
 \label{eq:Eddington-luminosity}
\end{equation}
 where $m_{\rm p}$ is the proton mass, $\mu$ is the mean molecular weight and reflects the chemical composition of the gas (see also the discussion below Equation~\ref{eq:Jeans-mass}), $c$ is the speed of light, and $\sigma_{\rm T}$ is the Thompson scattering cross section. Otherwise radiation pressure would be too strong to allow for accretion. This can be converted into a characteristic timescale by comparing with the available energy reservoir in the system coming from the rest mass energy of the black hole, $\tau_{\rm Edd} = M_{\bullet}c^2 / L_{\rm Edd} = \sigma_{\rm T}c / (4 \pi \mu m_{\rm p})$,  which is independent of the black hole mass. For typical conditions in primordial gas, $\tau_{\rm Edd} \sim 0.4\,$Gyr. 
The time taken for a black hole to grow via Eddington-limited accretion from an initial mass  $M_{\bullet,i}$ to a final mass $M_{\bullet,f}$ is given by 
\begin{equation}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=10.5cm]{Figures/klessen-glover-ARAA-fig-11.pdf}
...
Finally, we note that complementary information on the metal yields produced by the first generation of stars is potentially provided by observations of extremely metal poor foreground absorption systems in the spectra of distant quasars. The most metal-poor of these systems have metallicities comparable to EMP stars \citep[e.g.][]{Cooke2017,Robert2022,Welsh2022}. If we assume that each of these systems has been enriched by either a single Pop III supernova or a small number of such SNe, then we can use the observed metal abundances to infer their properties. In general, the results we obtain from this procedure are consistent with those coming from EMP stars. Most of the systems show elemental abundances consistent with enrichment primarily by CCSNe \citep{Welsh2019}, although a recent example has been found of a cloud in the broad-line region of a $z = 7.6$ quasar that has an abundance pattern that appears to indicate a PISN origin \citep{Yoshii2022}. This supports the idea that most Pop III supernovae are CCSNe, with PISNe occurring only rarely.
