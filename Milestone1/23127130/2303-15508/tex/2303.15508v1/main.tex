...
In this paper we address this challenge by focusing on cluster states,
which can be realized as ground states of cluster Hamiltonians and belong to a class of states called graph states~\cite{PhysRevLett.86.910}. Cluster Hamiltonians can be expressed as a sum of local terms, known as stabilizers, and are commonly used to describe condensed matter systems. As we will explain, the degree of uniformity of these states are bounded by the support of the stabilizers, i.e. the size of the neighborhood that interacts with each qubit. The ground state of these Hamiltonians can be prepared exactly on quantum computers via shallow circuits of local unitary gates \cite{choo2018measurement,azses2020identification,smith2022crossing} 
Cluster states in one and two dimensions have been studied in the context of symmetry protected topological states of matter
\cite{smacchia2011statistical,son2011quantum,azses2020identification}, and can be used in measurement based quantum computation and teleportation~\cite{briegel2009measurement,raussendorf2019computationally,NIELSEN2006147,azses2020identification,lee2022measurement}. 

We show that the cluster state in $D$-dimensions is $2D$-uniform. Despite of not containing a logical space, this state allows for quantum error
detection of $D$ individual errors. This ability of error detection stems from a connection between $m$-uniformity and QECCs that allows us to employ these states for benchmarking errors that may act on up to $ \lfloor m/2 \rfloor $ qubits in current noisy-intermediate scale quantum computers~\cite{preskill2018quantumcomputingin}. We demonstrate this procedure experimentally  on a real quantum computer for $D = 1$.  Finally, we provide examples of $m-$uniform subspaces in which a finite logical space is encoded.

The paper is organized as follows. In Sec.~\ref{sec:2} we introduce the cluster states and determine their $m-$uniformity based on the stabilizer formalism. In Sec.~\ref{sec:3} we review the connection to QECC and deduce the quantum error detection ability of cluster states. We present an experimental demonstration of this application in Sec.~\ref{sec:4}. In Sec.~\ref{sec:5} we exemplify how $m$-uniform spaces, encoding logical information and also allowing to detect errors, can be constructed. We conclude in Sec.~\ref{sec:6}.
...
\caption{One dimensional cluster model}
...
\label{sec:2}
The $D$-dimensional cluster Hamiltonian describes $n$ qubits located on the vertices of a $D$-dimensional square lattice, such that each qubit interacts with exactly $2D$ neighbours. Unless specified otherwise, we consider periodic boundary conditions (PBC) in each dimension. The one dimensional ($D=1$) cluster  Hamiltonian, which is depicted in Fig.~\ref{fig:1D_Cluster_Ising}, is defined as
\begin{equation}
\label{cluster}
...
using a lattice vector $\mathbf{v}=(i_{1},i_{2},\dots,i_{D})$, where $i_{1},i_{2},\dots,i_{D}\in \mathbb{Z}$, 
with basis vectors $\{\mathbf{e}_{i}\}$ 
having a 1 as the $i$'th entry and 0 elsewhere, and define the cluster Hamiltonian as
\be
\label{eq3}
...
\ee
Here, the $s_{\mathbf{v}}$'s are referred to as stabilizers and satisfy the following two properties~\cite{https://doi.org/10.48550/arxiv.quant-ph/0602096}: (i) they square to one, ${s_{\mathbf{v}}}^{2}=I$, because they correspond to tensor products of Pauli matrices; (ii) they commute, $[s_{\bf v},s_{\bf v'}]=0$. 

These two properties ensure that the Hamiltonian in Eq.~(\ref{eq3}) is frustration-free \cite{doi:10.1073/pnas.1519833113}, i.e. any ground state of $H$ is a simultaneous ground state of each  $s_{\mathbf{v}}$. In a more formal way, we note that the set of  $q$ stabilizers $\big\{s_{\mathbf{v}}\big\}$ generates a $\mathbb{Z}_2^q$ group called the stabilizer group $\mathcal{S}$ \cite{https://doi.org/10.48550/arxiv.quant-ph/9705052,https://doi.org/10.48550/arxiv.1508.02595}.
For the above cluster Hamiltonian with PBC, $q$ equals the number of qubits, $q=n$. Let us denote the generators of the stabilizer group by $\{ s_i\}$ ($i=1,\dots, q \le n$) and the elements of this group as $\mathcal{S}_i$ $(i=1, \dots, 2^{q})$. Since the $\mathcal{S}_i$'s commute, they have a common set of eigenvectors and their eigenvalues are all $\pm 1$ as of the first property above. Consider the unique eigenvector $|cs\rangle$ such that $s_i|cs\rangle=|cs\rangle$ for any $i$. By definition this unique ground state of the cluster Hamiltonian is the cluster state in $D$ dimensions and it has a ground state energy of $-n$. For later reference, we define the {\it support} of $\mathcal{S}_i$ as the number of non-identity local Pauli matrices of $\mathcal{S}_i$. For example, for the 1-dimensional cluster state the support of each local term is ${\rm{supp}}(s_i)= 3$, while for the $D$-dimensional cluster state we have ${\rm{supp}}(s_i)= 2D+1$. 


\subsection{\label{sec:level3}Reduced density matrices and stabilizer subgroups}
The stabilizer formalism leads to an explicit way to construct reduced density matrices. By definition, the pure density matrix $\rho$ of the cluster state corresponds to the projector into the $+1$ eigenvector of the generators of the stabilizer group and can
be written as 
\begin{equation}
\label{eq:dm_s_formula}
    \rho =\frac{1}{2^{n}} \sum_{\sigma \in \mathcal{S}} \sigma.
\end{equation}
Using this expression, one can show that the reduced density matrix over the set of qubits $A$ is \cite{https://doi.org/10.48550/arxiv.quant-ph/0602096}
\begin{equation}
...
\begin{figure}
    ...
    \caption{Circuit used to benchmark error rates on qubit $3$.}
...
{qiskit_cluster_detect_error_PBC=False_SHOTS=20000_L=5_run=RunOption.noise_avgsize=5_xzx=False_maxt=1000.0ns.pdf} &
\includegraphics[width=0.5\linewidth]
{qiskit_cluster_detect_error_PBC=False_SHOTS=20000_L=5_run=RunOption.noise_avgsize=5_xzx=True_maxt=1000.0ns.pdf}\\
(c) Experiment: ZXZ &
(d) Experiment: XZX \\
\includegraphics[width=0.5\linewidth]
{qiskit_cluster_detect_error_PBC=False_SHOTS=20000_L=5_run=RunOption.qc_avgsize=5_xzx=False_maxt=1000.0ns.pdf}&
\includegraphics[width=0.5\linewidth]
{qiskit_cluster_detect_error_PBC=False_SHOTS=20000_L=5_run=RunOption.qc_avgsize=5_xzx=True_maxt=1000.0ns.pdf}
\end{tabular}
...
On the other hand, the $X_3$ error has syndrome $00100$ and can occur due to a single $R_3$ readout error. In Fig.~\ref{noise_error}(b) we present the results from the transformed protocol. We notice that the $X$ error probability is now shifted by approximately the same amount as the $Z$ error of Fig.~\ref{noise_error}(a), and vice-versa. In contrast, we find that the slopes of the errors in the original and transformed circuits are comparable. This finding confirms our hypothesis that the shift of the curves are associated with SPAM errors, while the slopes are due to processes that occur during the waiting time. 
...
\begin{figure}[] 
\centering
\includegraphics[width=\linewidth]{main-figure2.pdf}
...
\end{figure}
The cluster state discussed so far does not encode a logical subspace. Is it possible to supplement its error detection ability, with an encoded subspace? Here, we exemplify this possibility via a measurement based protocol~\cite{https://doi.org/10.48550/arxiv.quant-ph/0602096,PhysRevA.65.012308,https://doi.org/10.48550/arxiv.quant-ph/0111080,Grassl}.
...
\appendix
\section{}\label{se:app_cs_2d_uniform}

\textbf{Proof of Proposition 1:} We present a proof by contradiction. Consider subsystem $A$ containing  $|A| \le 2D$ qubits. Let us assume that the subgroup $\mathcal{S}_A$ is not the trivial group. That is, there exists at least one matrix $\sigma \neq I$ in $\mathcal{S}_A$. Since $\mathcal{S}_A$ is a subgroup of $\mathcal{S}$, $\sigma$ is the product of  generators of $\mathcal{S}$,
...
\section{}\label{se:proof_finite_lattice}
...
\section{} \label{ minimum central qubit }
...
\begin{thebibliography}{79}%
	...
	{https://doi.org/10.1103/PhysRev.47.777} {\bibfield  {journal} {\bibinfo
		...
		{777} (\bibinfo {year} {1935})}\BibitemShut {NoStop}%
	...
	{NoStop}%
	\bibitem [{\citenamefont {Enriquez}\ \emph {et~al.}(2016)\citenamefont
		{Enriquez}, \citenamefont {Wintrowicz},\ and\ \citenamefont
		{Zyczkowski}}]{article}%
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {M.}~\bibnamefont
			{Enriquez}}, \bibinfo {author} {\bibfnamefont {I.}~\bibnamefont
			{Wintrowicz}},\ and\ \bibinfo {author} {\bibfnamefont {K.}~\bibnamefont
			{Zyczkowski}},\ }\bibfield  {title} {\bibinfo {title} {Maximally entangled
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {L.}~\bibnamefont
		...
		{author} {\bibfnamefont {E.}~\bibnamefont {Chitambar}},\ }\bibfield  {title}
	{\bibinfo {title} {A hierarchy of multipartite correlations based on
			concentratable entanglement},\ }\href@noop {} {\bibfield  {journal} {\bibinfo
			{journal} {arXiv preprint arXiv:2209.07607}\ } (\bibinfo {year}
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {D.~N.}\ \bibnamefont
			...
			subsystem},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal}
			{Physical review letters}\ }\textbf {\bibinfo {volume} {71}},\ \bibinfo
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.~R.}\ \bibnamefont
			...
			{Rains}}, \bibinfo {author} {\bibfnamefont {P.}~\bibnamefont {Shor}},\ and\
		\bibinfo {author} {\bibfnamefont {N.~J.}\ \bibnamefont {Sloane}},\ }\bibfield
	{title} {\bibinfo {title} {Quantum error correction via codes over gf (4)},\
	}\href@noop {} {\bibfield  {journal} {\bibinfo  {journal} {IEEE Transactions
				on Information Theory}\ }\textbf {\bibinfo {volume} {44}},\ \bibinfo {pages}
		{1369} (\bibinfo {year} {1998})}\BibitemShut {NoStop}%
	...
	{\bibinfo {title} {Maximally multipartite entangled states},\ }\bibfield
	{journal} {\bibinfo  {journal} {Physical Review A}\ }\textbf {\bibinfo
		{volume} {77}},\ \href {https://doi.org/10.1103/physreva.77.060304}
	{10.1103/physreva.77.060304} (\bibinfo {year} {2008})\BibitemShut {NoStop}%
	...
	{https://doi.org/10.48550/ARXIV.2006.04086} {\bibinfo {title} {Constructions
			of $k$-uniform states from mixed orthogonal arrays}} (\bibinfo {year}
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {F.}~\bibnamefont
			...
			highly entangled subspaces},\ }\href@noop {} {\bibfield  {journal} {\bibinfo
	...
	{\bibinfo {title} {Two and three-uniform states from irredundant orthogonal
			arrays},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal} {npj
				Quantum Information}\ }\textbf {\bibinfo {volume} {5}},\ \bibinfo {pages} {1}
	...
	{\bibinfo {title} {Quantum k-uniform states for heterogeneous systems from
			irredundant mixed orthogonal arrays},\ }\bibfield  {journal} {\bibinfo
		{journal} {Quantum Information Processing}\ }\textbf {\bibinfo {volume}
		{20}},\ \href {https://doi.org/10.1007/s11128-021-03040-0}
	{10.1007/s11128-021-03040-0} (\bibinfo {year} {2021})\BibitemShut {NoStop}%
	...
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
			{Borras}}, \bibinfo {author} {\bibfnamefont {A.}~\bibnamefont {Plastino}},
		\bibinfo {author} {\bibfnamefont {J.}~\bibnamefont {Batle}}, \bibinfo
		{author} {\bibfnamefont {C.}~\bibnamefont {Zander}}, \bibinfo {author}
		{\bibfnamefont {M.}~\bibnamefont {Casas}},\ and\ \bibinfo {author}
		...
		{title} {Multiqubit systems: highly entangled states and entanglement
			...
			{volume} {40}},\ \bibinfo {pages} {13407} (\bibinfo {year}
		{2007})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {Z.}~\bibnamefont
		...
		and\ \bibinfo {author} {\bibfnamefont {E.}~\bibnamefont {Barnes}},\
			...
			constructing highly entangled graph states},\ }\href@noop {} {\bibfield
		{journal} {\bibinfo  {journal} {Physical Review A}\ }\textbf {\bibinfo
			{volume} {106}},\ \bibinfo {pages} {062424} (\bibinfo {year}
		{2022})}\BibitemShut {NoStop}%
	\bibitem [{\citenamefont {Sudevan}\ and\ \citenamefont
		{Das}(2022)}]{https://doi.org/10.48550/arxiv.2201.05622}%
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {S.}~\bibnamefont
			{Sudevan}}\ and\ \bibinfo {author} {\bibfnamefont {S.}~\bibnamefont {Das}},\
	}\href {https://doi.org/10.48550/ARXIV.2201.05622} {\bibinfo {title}
		{$n$-qubit states with maximum entanglement across all bipartitions: A graph
			state approach}} (\bibinfo {year} {2022})\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {Z.}~\bibnamefont
			...
			constructing quantum codes from highly entangled states},\ }\href@noop {}
	{\bibfield  {journal} {\bibinfo  {journal} {IEEE Access}\ }\textbf {\bibinfo
			{volume} {8}},\ \bibinfo {pages} {222439} (\bibinfo {year}
		{2020})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {W.}~\bibnamefont
		...
		{title} {Absolute maximal entanglement and quantum secret sharing},\
				...
				A}\ }\textbf {\bibinfo {volume} {86}},\ \bibinfo {pages} {052335} (\bibinfo
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {K.}~\bibnamefont
			{Choo}}, \bibinfo {author} {\bibfnamefont {C.~W.}\ \bibnamefont
			{Von~Keyserlingk}}, \bibinfo {author} {\bibfnamefont {N.}~\bibnamefont
		...
		{pages} {086808} (\bibinfo {year} {2018})}\BibitemShut {NoStop}%
	...
	{\bibinfo {title} {Identification of symmetry-protected topological states on
			noisy quantum computers},\ }\href@noop {} {\bibfield  {journal} {\bibinfo
	...
	{title} {\bibinfo {title} {Crossing a topological phase transition with a
		...
		{pages} {L022020} (\bibinfo {year} {2022})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {W.-R.}\ \bibnamefont
		...
		{author} {\bibfnamefont {R.}~\bibnamefont {Raussendorf}}, \bibinfo {author}
		{\bibfnamefont {E.}~\bibnamefont {Sela}},\ and\ \bibinfo {author}
		{\bibfnamefont {V.}~\bibnamefont {Scarola}},\ }\bibfield  {title} {\bibinfo
		{title} {Measurement-based time evolution for quantum simulation of fermionic
			...
			{Physical Review Research}\ }\textbf {\bibinfo {volume} {4}},\ \bibinfo
		{pages} {L032013} (\bibinfo {year} {2022})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {W.}~\bibnamefont
		...
		{journal} {\bibinfo  {journal} {Phys. Rev. E}\ }\textbf {\bibinfo {volume}
			{96}},\ \bibinfo {pages} {020106} (\bibinfo {year} {2017})}\BibitemShut
	{NoStop}%
	...
	\bibitem [{\citenamefont {Hein}\ \emph {et~al.}(2006)\citenamefont {Hein},
		...
		\citenamefont {Nest},\ and\ \citenamefont
		{Briegel}}]{https://doi.org/10.48550/arxiv.quant-ph/0602096}%
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {M.}~\bibnamefont
		...
		{\bibfnamefont {M.~V.~d.}\ \bibnamefont {Nest}},\ and\ \bibinfo {author}
		{\bibfnamefont {H.~J.}\ \bibnamefont {Briegel}},\ }\href
	{https://doi.org/10.48550/ARXIV.QUANT-PH/0602096} {\bibinfo {title}
		{Entanglement in graph states and its applications}} (\bibinfo {year}
	{2006})\BibitemShut {NoStop}%
	\bibitem [{\citenamefont {Sattath}\ \emph {et~al.}(2016)\citenamefont
		{Sattath}, \citenamefont {Morampudi}, \citenamefont {Laumann},\ and\
		\citenamefont {Moessner}}]{doi:10.1073/pnas.1519833113}%
	\BibitemOpen
	...
	{\bibfield  {journal} {\bibinfo  {journal} {Proceedings of the National
		...
		{6433} (\bibinfo {year} {2016})},\ \Eprint
	{https://arxiv.org/abs/https://www.pnas.org/doi/pdf/10.1073/pnas.1519833113}
	{https://www.pnas.org/doi/pdf/10.1073/pnas.1519833113} \BibitemShut {NoStop}%
	\bibitem [{\citenamefont
		{Gottesman}(1997)}]{https://doi.org/10.48550/arxiv.quant-ph/9705052}%
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {D.}~\bibnamefont
			{Gottesman}},\ }\href {https://doi.org/10.48550/ARXIV.QUANT-PH/9705052}
	{\bibinfo {title} {Stabilizer codes and quantum error correction}} (\bibinfo
	...
	\bibitem [{\citenamefont {Zeng}\ \emph {et~al.}(2015)\citenamefont {Zeng},
		...
		{Wen}}]{https://doi.org/10.48550/arxiv.1508.02595}%
	\BibitemOpen
	...
	{https://doi.org/10.48550/ARXIV.1508.02595} {\bibinfo {title} {Quantum
			information meets quantum matter -- from quantum entanglement to topological
			phase in many-body systems}} (\bibinfo {year} {2015})\BibitemShut {NoStop}%
	\bibitem [{\citenamefont {Rajak}\ \emph {et~al.}(2019)\citenamefont {Rajak},
		\citenamefont {Dalla~Torre} \emph {et~al.}}]{rajak2019complete}%
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
			{Rajak}}, \bibinfo {author} {\bibfnamefont {E.~G.}\ \bibnamefont
			{Dalla~Torre}}, \emph {et~al.},\ }\bibfield  {title} {\bibinfo {title}
		{Complete characterization of spin chains with two ising symmetries},\
				...
				Letters)}\ }\textbf {\bibinfo {volume} {125}},\ \bibinfo {pages} {10008}
		(\bibinfo {year} {2019})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {B.}~\bibnamefont
			...
			[[4, 2, 2]]-concatenated toric code},\ }\href@noop {} {\bibfield  {journal}
		{\bibinfo  {journal} {arXiv preprint arXiv:1604.04062}\ } (\bibinfo {year}
		{2016})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {C.~K.}\ \bibnamefont
		...
		{Repeated quantum error detection in a surface code},\ }\href@noop {}
	{\bibfield  {journal} {\bibinfo  {journal} {Nature Physics}\ }\textbf
		{\bibinfo {volume} {16}},\ \bibinfo {pages} {875} (\bibinfo {year}
		{2020})}\BibitemShut {NoStop}%
	...
	\bibitem [{\citenamefont
		{Schlingemann}(2001)}]{https://doi.org/10.48550/arxiv.quant-ph/0111080}%
	\BibitemOpen
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {D.}~\bibnamefont
			{Schlingemann}},\ }\href {https://doi.org/10.48550/ARXIV.QUANT-PH/0111080}
	{\bibinfo {title} {Stabilizer codes can be realized as graph codes}}
	(\bibinfo {year} {2001})\BibitemShut {NoStop}%
	\bibitem [{\citenamefont {Grassl}\ \emph {et~al.}()\citenamefont {Grassl},
	...
	{\emph {\bibinfo {booktitle} {Proceedings {IEEE} International Symposium on
				Information Theory,}}}\ (\bibinfo  {publisher} {{IEEE}})\BibitemShut
	{NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {D.~V.}\ \bibnamefont
		...
		{\bibinfo  {journal} {Physical review letters}\ }\textbf {\bibinfo {volume}
			{108}},\ \bibinfo {pages} {240505} (\bibinfo {year} {2012})}\BibitemShut
	{NoStop}%
	...
	{\bibinfo {title} {Computational power of symmetry-protected topological
			phases},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal} {Physical
				review letters}\ }\textbf {\bibinfo {volume} {119}},\ \bibinfo {pages}
		{010504} (\bibinfo {year} {2017})}\BibitemShut {NoStop}%
	...
	{title} {\bibinfo {title} {Entanglement spectrum of a topological phase in
			one dimension},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal}
			{Physical review b}\ }\textbf {\bibinfo {volume} {81}},\ \bibinfo {pages}
		{064439} (\bibinfo {year} {2010})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {D.}~\bibnamefont
			...
			symmetry-protected topological phases},\ }\href@noop {} {\bibfield  {journal}
		{\bibinfo  {journal} {Physical Review B}\ }\textbf {\bibinfo {volume}
			{102}},\ \bibinfo {pages} {235157} (\bibinfo {year} {2020})}\BibitemShut
	{NoStop}%
	...
	{title} {\bibinfo {title} {Entanglement spectroscopy of non-abelian anyons:
			Reading off quantum dimensions of individual anyons},\ }\href@noop {}
	{\bibfield  {journal} {\bibinfo  {journal} {Physical Review B}\ }\textbf
		{\bibinfo {volume} {99}},\ \bibinfo {pages} {115429} (\bibinfo {year}
		{2019})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {D.}~\bibnamefont
			...
			topological order by symmetry resolution},\ }\href@noop {} {\bibfield
		{journal} {\bibinfo  {journal} {Physical Review B}\ }\textbf {\bibinfo
			{volume} {104}},\ \bibinfo {pages} {L220301} (\bibinfo {year}
		{2021})}\BibitemShut {NoStop}%
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {S.}~\bibnamefont
			...
			entanglement: exact results in 1d and beyond},\ }\href@noop {} {\bibfield
		{journal} {\bibinfo  {journal} {Journal of Statistical Mechanics: Theory and
				Experiment}\ }\textbf {\bibinfo {volume} {2020}},\ \bibinfo {pages} {033106}
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {F.}~\bibnamefont
		...
		and\ \bibinfo {author} {\bibfnamefont {P.}~\bibnamefont {Calabrese}},\
			...
			long-range free-fermion chain},\ }\href@noop {} {\bibfield  {journal}
		{\bibinfo  {journal} {Journal of Statistical Mechanics: Theory and
				Experiment}\ }\textbf {\bibinfo {volume} {2022}},\ \bibinfo {pages} {063104}
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {K.}~\bibnamefont
			...
			entanglement of c 2-symmetric topological insulators},\ }\href@noop {}
	{\bibfield  {journal} {\bibinfo  {journal} {Physical Review B}\ }\textbf
		{\bibinfo {volume} {107}},\ \bibinfo {pages} {125108} (\bibinfo {year}
	...
	\bibfield  {author} {\bibinfo {author} {\bibfnamefont {B.}~\bibnamefont
			...
			theorem},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal} {Israel
				Journal of Mathematics}\ }\textbf {\bibinfo {volume} {50}},\ \bibinfo {pages}
		{169} (\bibinfo {year} {1985})}\BibitemShut {NoStop}%
	\bibitem [{Note1()}]{Note1}%
	\BibitemOpen
