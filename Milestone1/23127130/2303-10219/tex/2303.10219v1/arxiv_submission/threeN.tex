...
In this work we assume that $C_L(P)$ can be represented as an all-orders Feynman diagrammatic expansion in a relativistic effective field theory (EFT) in which the degrees of freedom are neutrons. In the context of QCD, this means that we are considering the pionless EFT for nucleons, i.e. the theory in which the effects of pions have been integrated out. We make no restrictions on the form or strength of the interaction vertices, other than requiring that they respect Lorentz invariance and neutron number, and thus expect that the resulting formalism applies generally. What we do restrict is the value of the total CMF energy, $E_3^* = \sqrt{P^2} = \sqrt{E^2 - \boldsymbol P^2}$, in such a way that it is legitimate to neglect the degrees of freedom that have been integrated out. In particular, for isosymmetric QCD we require that
\begin{equation}
...
\end{equation}
The quantity on the left-hand side is the full three-neutron scattering amplitude, defined as the sum of all diagrams in the skeleton expansion with a standard $i \epsilon$ pole prescription in propagators. The limit on the right-hand side yields a set of infinite-volume integral equations. We do not write these out in detail as they have the same form as those for identical scalars presented in ref.~\cite{\HSQCb}, except for the presence of additional spin indices and of the Wigner rotations inherited from $\textbf G$ and $\textbf K_{\df,3}$. Dedicated studies concerning the solution to these integral equations can be found in refs.~\cite{Dawid:2023jrj,Jackura:2020bsk}, and we expect many of the practical details to carry over to the three-neutron case.
...
= \sum_{ m'_{s}(\astarlittle) , m'_{s}(\bstarlittle) } \Big \vert \boldsymbol k, \ell, m, m'_{s}(\astarlittle) , m'_{s}(\bstarlittle) , m_s(\klittle) \Big \rangle
