...
While \Cref{hex_lemma} \cite{hex} was published long time ago, it does not seem to be that widely known.
In order to keep our exposition elementary and self-contained, we provide also a short proof of \Cref{hex_lemma}.
Our proof implicitly follows kind of a ``fixed-point'' argument, quite similarly to \cite{hex}, but we stay on purely combinatorial side of the problem.
...
Although the original proof in \cite{dujmovic2021stack} is not very long, by carefully rearranging the arguments
we have succeeded in eliminating some extensive technical parts of the proof in \cite{dujmovic2021stack}
and, in particular, resolved the case of pairwise crossing paths in a direct short way.
This alternative short proof is based on the Bachelor's thesis of the second author \cite{Straka2023thesis}.
