...
We also follow the design of SR-$k$ to propose a variant of randomized block BFGS method~\cite{gower2016stochastic,gower2017randomized,kovalev2020fast} and a randomized block DFP method, resulting an explicit superlinear convergence rate of~$\OM\big((1-k/(\varkappa d))^{t(t-1)/2}\big)$.
...
$\displaystyle{\OM\big(\big({d\varkappa}/{t}\big)^{t/2}\big)}$\\ [0.1cm]
    $\displaystyle{\OM\big(\big(\exp{({d\ln(\varkappa)}/{t})}-1\big)^{t/2}\big)}$  \\  [0.1cm]
