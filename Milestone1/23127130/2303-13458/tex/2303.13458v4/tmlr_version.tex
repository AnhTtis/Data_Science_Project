...
\author{
      \name
      Oskar Nordenfors \email oskar.nordenfors@umu.se\\
...
Given representations $\rho_U$ and $\rho_Y$ on vector spaces $U$ and $V$ respectively, we may naturally define a lifted representation on $\Homm(U,V)$ as follows:
...
\end{align}
where $\ell: Y \times Y \to \R$ is some loss-function, yields an $A\in\calE$ so that $\Phi_A$ is an equivariant function. That is, a minimization of the nominal risk \eqref{eq:nomrisk} does not take advantage of the inductive bias of the equivariance of the ground truth $f$. We will in this paper analyze two strategies for doing so.
...
\begin{customass}{1}
    The group $G$ is acting on all hidden spaces $X_i$ through unitary representations $\rho_i$. The spaces $\Homm_G(X_i,X_{i+1})$ are all non-empty. \label{ass:unitary_reps}
\end{customass}
\begin{customass}{2}
...
We also note that all assumptions are in many settings quite weak. We already commented on Assumption \ref{ass:unitary_reps}.  As for Assumption \ref{ass:equiv_nonlin},  note that any non-linearity acting pixel-wise on an image will be equivariant to any representation acting by moving around the pixels of the image, for instance translations and rotations. In the same way,  any loss comparing images pixel by pixel will satisfy Assumption \ref{ass:equiv_loss}. Furthermore, if we are trying to learn an invariant function the final representation $\rho_Y$ is trivial and Assumption \ref{ass:equiv_loss} is trivially satisfied. We discuss more cases in the Appendix \ref{app:archs}.
...
\begin{remark}
    Note that Theorem \ref{theo:gradients_equal} only applies to \emph{points in $\calE$}. It does not say anything about stationarity of points that are not in $\calE$.
\end{remark}
...
\begin{proof}
    ...
    Now, to determine the dynamics of $X$, let us test the equation $\dot{A} = - \Pi_\calL\nabla R^\aug(A)$ with an arbitrary $W\in \rmT\calE$. We namely have $\sprod{\dot{A},W}=\sprod{X,W}$ for such $W$. Furthermore, $\sprod{\Pi_\calL\nabla R^\aug(X),W} = \sprod{\Pi_\calE \nabla R(X),W} = \sprod{\nabla R(X),W}$, and
    \begin{align*}
...
We perform an experiment in order to showcase the difference between a network whose layers obey the compatibility assumption and one where they do not. We made some concessions to standard practices, such as e.g., using stochastic gradient descent (SGD) instead of gradient descent (which might more accurately reflect the theoretical results).
...
We consider two nominal architectures consisting of convolutional layers with a fully connected last layer, and equivariant pooling and layer normalization layers -- see Figure \ref{fig:architecture}. The two architectures differ in the choice of support for the convolutional filters -- one is the cross support and one is the skew support, as in Figure \ref{fig:supports}.  These were trained in $\eqvi$ mode for $250$ epochs, manifestly invariant to the rotation action of $\mathbb{Z}_4$, to classify MNIST \citep{lecun1998a}. We used SGD as the optimizer, with an MSE loss with the labels as one-hot vectors; the learning rate was set to $5\cdot 10^{-4}$ and the batch size was set to 100. %The $3\times3$ convolutional filters in two of the CNNs had support in a cross pattern and the filters in the last CNN had a skew pattern (see Figure \ref{fig:supports}), with values in all other indices set to 0. The CNNs were trained for 250 epochs. 
Then we switched mode to $\augm$ for ten more epochs, with a learning rate of $2.5\cdot 10^{-4}$. The reason we lowered the learning rate for this part of the experiment was to make the SGD 'closer' to a flow. %their architectures were 'let loose' from their manifestly invariant state to allow non-equivariant layers and training was continued on augmented data for 10 more epochs with the learning rate set to $2.5\cdot 10^{-4}$. 
For the cross shaped support, we ran two experiments, with partially unitary and non-unitary embedding operator $L$ in $\augm$ mode, resulting in a total of three experiments. The non-unitary $L$ were drawn according to a Gaussian distribution among all possible embeddings. The images were normalized with respect to mean and standard deviation before being sent to the first layer.
...
The purpose of the experiment was to observe how quickly the layers of the different CNNs would drift away from $\mathcal{E}$ when using augmentation. After the $\eqvi$ mode epochs, we will likely be close to a stationary point of the equivariant flow. This point will also be stationary for the augmented flow if the compatibility condition holds. By Example \ref{ex:skewvscross}, this is the case for the cross support, but not for the skew support. Hence, we can expect the latter to drift away faster. For the two cross support architectures, we also expect to see a difference, since $\calE$ only is an invariant set when using when partially unitary $L$, by Theorem \ref{theo:gradients_equal} (part 2). Hence, the non-unitary version should be more prone to drift. %the CNN with cross support fulfills the compatibility assumption \ref{ass:compability} and the CNN with skew support does not. Hence, in the light of Theorem \ref{theo:gradients_equal}The architecture with partially unitary $L$ satisfies the conditions of the second part of Theorem \ref{theo:gradients_equal}, while the one without partially orthogonal $L$ does not.


\subsection{Results and analysis}
After each batch in the augmented training, the distance of the layers from  $\calE$ was recorded. In Figure \ref{fig:distance}, we plot on the left mean distance over each full epoch, and on the right, the distance after each batch in the first epoch. In essence, our above hypotheses are confirmed: The architecture with cross support and partially unitary $L$ stays close to $\calE$, whereas the others drift. The skew-support architecture drifts more in absolute numbers, but not too much should be read into this -- it may very well look different for other nominal architectures. Also note that the non-unitary cross architectures have some outliers -- we believe that the cause for this is numerical problems when initializing the $\augm$ training - this entails solving a linear set of equations $Lc = A_0$, with a potentially ill-conditioned $L$. %The experiment was performed 30 timesBy the above discussion, we would expect that layers of the CNN with cross-shaped support for the filters and partially orthogonal parametrization would leave $\calE$ more slowly during the augmented training than the layers of CNN with skew-shaped support for the filters, since by Theorem \ref{theo:gradients_equal} the stationary points for $\augm{}$ are not necessarily the same as the stationary points for $\eqvi{}$. We would also expect that of the two CNNs with cross-shaped support for the filters, that the layers of the CNN with partially orthogonal parametrization would leave $\calE$ more slowly during the augmented training than the layers of the CNN with non-orthogonal parametrization, since for the former $\calE$ is invariant for $\augm{}$, but not for the latter.
\begin{figure}
    \centering
    \includegraphics[width=.85\textwidth]{figures/distance_non_unitary_title.png}
    \caption{On the left: distance in 2-norm of layers from $\calE$ averaged over the batches per epoch of augmented training. On the right: distance in 2-norm of layers from $\calE$ during the first epoch of augmented training.\\
...
The cross shaped architecture in this case seems to be relatively stable -- that it initially drifts in the first epoch is probably due to the stochastic nature of $SGD$ -- understanding this better is interesting future work. It should however be noted that although it appears so in the left figure, it does not converge to $0$, but rather to  around 1e-3. This is not a contradiction -- Theorem \ref{th:stable} indicates that $\calE$ may be unstable for $\augm$ also in the cross-shaped case.
...
\paragraph{Hardware}
The $30\cdot 3$ trials of the experiment were run in parallel on a super computer using NVIDIA Tesla T4 GPUs with 16GB RAM. We estimate the total computation time to be about 270 hours.
