...
\[\mathcal{F} =\{(p, \theta) \mapsto \langle \phi, \sigma(p, \theta) \rangle : \phi \in B^d_2\},\]
where $\sigma$ is a fixed feature map, then the Vovk-Azoury-Warmuth forecaster achieves $\est(T) \lesssim d \cdot \log(T)$~\cite{vovk1997competitive,azoury2001relative}.
...
With these results in mind, we can derive the exact regret bounds for various instances function class $\mathcal{F}$.
...
\begin{itemize}
    ...
    We can explicitly construct an $\varepsilon$-covering so that $\log \mathcal{N}_2(\mathcal{F}, \varepsilon) \lesssim \varepsilon^{-m-1} $ (see Example 5.10 and 5.11 in~\cite{wainwright2019high}).
...
We can analogously work out the regret bounds if each of the examples are miss-specified.
...
\begin{align*}
    ...
    &= \sum_{t=1}^T \EE_{p_t \sim \Delta_t} \left[ |\bar{x}(p_t; \theta_t) - d_t| \mid \mathcal{H}_{t-1} \right] - |\bar{x}( \bar{p}_t; \theta_t) - d_t| + 2\varepsilon T + O(T/K)
\end{align*}
%
...
\begin{itemize}
    ...
    \[(|\bar{x}(p_t; \theta_t) - d_t| - |\hat{f}_t(p_t; \theta_t) - d_t|)^2 = (\bar{x}(p_t; \theta_t) - \hat{f}_t(p_t; \theta_t))^2 \]
\end{itemize}
Therefore,
...
\begin{aligned}
    & \sum_{t=1}^T (\bar{x}(p_t) - \hat{f}_t(p_t))^2 \\
...
\begin{align*}
    ...
    \le{}& \sum_{t=1}^T (x_t - x^*(p_t; \theta_t))(\hat{f}_t(p_t; \theta_t)  - \bar{x}(p_t; \theta_t)) + \sum_{t=1}^T 6 \varepsilon^2 + \frac{1}{12}(\hat{f}_t(p_t; \theta_t)  - \bar{x}(p_t; \theta_t))^2
\end{align*}
...
\begin{align*}
    \preg(T)
    &\lesssim \frac{KT}{\gamma} + \gamma \cdot \est(T) + \varepsilon^2 T + \log(1/\delta)) \\
