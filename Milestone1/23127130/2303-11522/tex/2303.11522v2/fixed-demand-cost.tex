...
The proof of Theorem~\ref{thm:IdenticalResult} relies on the following Lipschitzness condition between the optimal supplier production and the prices set by the market operator.
...
Due to the monotonicity of the optimal supplier production in the prices, both payment and cost regret are only accumulated when $p_t>p^*$. Further, since $p_t>p^*$ at most once in each sub-phase, the total payment and cost regret are $O(\log \log T)$.
