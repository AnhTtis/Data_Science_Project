...
So, the precision parameter $\epsilon_k$ is shrunk each time payment or cost regrets are accumulated.
...
\begin{itemize}
    \item $|\S_{k_t}| \le 1/\sqrt{T}$: From the construction of our algorithm, as all demands in each sub-interval are considered equal to the lower bound of that interval, the condition $p^*(a_{k_t}) \in \S_{k_t}$ is always satisfied. 
