...
\label{sec:method}
We write $X$ to denote a random variable that can be scalar-, vector-, or matrix-valued (and likewise for $\Xt{1},\ldots,\Xt{K}$). 
...
Theorem~\ref{thm:generalized-thinning} is proven in Supplement~\ref{app:pf-sufficiency-thinning}. 
Further, there is a simple algorithm for finding families of distributions $\mathcal{P}$ and functions $T(\cdot)$ such that $\mathcal P$ can be thinned by $T(\cdot)$.  
