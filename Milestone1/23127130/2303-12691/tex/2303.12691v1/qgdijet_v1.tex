...
%
\begin{figure}[htb]
 \includegraphics[width=\columnwidth]{feynman_bef.pdf}
 \includegraphics[width=\columnwidth]{feynman_in.pdf}
\caption{Diagrams with photon splitting before (top) or interacting inside (bottom) the target. Momenta, color and polarisation indices, and coordinates separated by a semicolon, are indicated. The target hadron is shown as an orange band.}
\label{fig:diag}
\end{figure}
%
...
\ea
%
for arbitrary polarization $\lambda$ of the photon. Here, $\Psi(z)$ is the quark background field,  $S_F$ and $G_F$ are the Feynman quark and gluon propagators in the gluon background field of the target at eikonal accuracy, whereas $S_{F,0}$ and $G_{F,0}$ are the corresponding vacuum propagators. Using the formalism developed in \cite{Altinoluk:2020oyd, Altinoluk:2021lvu, Altinoluk:2022jkk}, Eqs. (\ref{bef-def}) and (\ref{in-def}) can be evaluated to give\footnote{In the case of longitudinal polarization of the photon, the contribution with photon splitting inside the target (\ref{in-def}) is found to vanish, from the evaluation of its Dirac structure together with the longitudinal  photon polarization vector in light-cone gauge.} 
%
\ba
S^{\rm bef}_{\gamma_{T,L}\to q_1g_2}&=&i e e_fg  2\pi \delta(p_1^+\!+\!p_2^+\!-\!q^+)\int_{\v,\z}e^{-i\v\cdot\p_1-i\z\cdot\p_2}\int\frac{d^2\K}{(2\pi)^2}\frac{e^{i(\v-\z)\cdot\K}}{\left[ \K^2+m^2+\frac{p_1^+p^+_2}{(q^+)^2}Q^2\right]}\nonumber\\
&&\times 
...
%%
%\ba
S^{\rm in}_{\gamma_T\to q_1g_2}&=&i \frac{e e_fg  2\pi \delta(p_1^+\!+\!p_2^+\!-\!q^+)}
{\left[ \left( \p_1-\frac{p_1^+}{p_2^+}\p_2\right)^2+m^2\right]}
...
\ea
%
where we denote the components of a four momentum $p=(p^+,p^-,{\bf p})$, $p^\pm=(p^0\pm p^3)/\sqrt{2}$, $m$ is the quark mass, $Q^2=-q^2$ is the photon virtuality, and
% 
\be
...
\begin{widetext}
\ba
S^{\rm in}_{\gamma_T\to q_1g_2}&=&i \frac{e e_f g\,  2\pi \delta(p_1^+\!+\!p_2^+\!-\!q^+)}
{\left[ \P^2+(1-z)^2m^2\right]}\, (1-z)^2\, {\bar u}(1)\frac{\gamma^+\gamma^-}{2} \Gamma^{\rm in}_T
...
\ea 
%
Using the approximation (\ref{corr_lim_Color_Bef}), the integration over $\r$ can be performed trivially and sets $\K=\P$.  All in all, the "before" contribution (\ref{bef_S_gen}) to the S-matrix reduces to 
%
 \begin{widetext}
\ba
S^{\rm bef}_{\gamma_{T,L}\to q_1g_2}&\simeq&i \frac{e e_fg\,  2\pi \delta(p_1^+\!+\!p_2^+\!-\!q^+)}{\left[ \P^2+{\bar Q}^2\right]}\, 
{\bar u}(1)\frac{\gamma^+\gamma^-}{2} \Gamma^{\rm bef}_{T,L}
\int_{\b}e^{-i\b\cdot\k}
\int_{z^+} 
t^{a_2}U_F\left(+\infty,z^+;\b\right)\Psi(z^+,\b)
...
\ea
 %\end{widetext}
%
in the correlation limit $\P^2\gg\k^2$, using the notation
...
$\bar Q^2=m^2+z(1-z)Q^2$. Note that Eqs.~(\ref{in_S_corr}) and (\ref{bef_S_corr}) contain the same $\P$-independent color structure, and differ only by a $\P$-dependent kinematical factor.
...
%\ee
where the hard factors for the longitudinal and the transverse photon polarizations are 
%
\ba
%{\cal H}_{L}=  \frac{4Q^2z^3(1-z)^2}{[\P^2+{\bar Q}^2]^2}\,, \ 
{\cal H}_{L}=  \frac{4Q^2z^3(1-z)^2}{G(\P,{\bar Q})^2}\,, \ 
%{\cal H}_{T}=  z 
...
{\cal H}_{T}=  z 
\left\{ \frac{(1+z^2)\P^2+(1-z)^4m^2}{F(\P,z,m)^2}+\frac{[z^2+(1-z)^2]\P^2+m^2}{G(\P,{\bar Q})^2}-\frac{2z^2\P^2}{G(\P,{\bar Q})F(\P,z,m)}\right\}
\ea
%
with $F(\P,z,m)=\P^2+(1-z)^2m^2$ and $G(\P,{\bar Q})=\P^2+{\bar Q}^2$. The target-averaged color operator ${\cal T}(\k)$ reads 
%
\be
...
\ee
%
Note that the unpolarized TMD quark distribution is defined as 
%
\be
f_1^q(x,\k)=\frac{1}{(2\pi)^3}\int_{\b} e^{i\k\cdot\b}\int_{z^+}e^{-iz^+xP_{{tar}}^-}\langle P_{{tar}}|\bar \Psi(z^+,\b)\frac{\gamma^-}{2}U_F^\dagger(+\infty, z^+; \b)U_F(+\infty, 0; {\bf 0})\Psi(0,{\bf 0})|P_{{tar}}\rangle,
\label{TMD_def}
\ee
%
where we have neglected the transverse gauge link. Here, $P_{tar}$ is the 4-momentum of the target and $|P_{{tar}}\rangle$ represents the quantum state of the target. Relating the CGC-like target average present in Eq.~(\ref{T_op_def}) with the expectation value in the target quantum state in Eq.~(\ref{TMD_def}) (see Supplemental material), one obtains the relation   
%
\be
{\cal T}(\k)=\frac{(2\pi)^3}{P_{{tar}}^-}f^q_1(x=0,\k).
\label{rel_T_op_vs_TMD}
\ee
%
Eq.~(\ref{cross_sec_res_1}) is thus a factorization formula involving the quark TMD distribution at $x=0$ target momentum fraction.
...
%
\be
\frac{d\sigma^{\gamma_{T,L}\to q_1g_2}}{dz\, dM^2_{jj}\,  d^2\k }\bigg|_{\rm corr. lim.}=\, (2\pi)\, \frac{\alpha_{\rm em}\, \alpha_s\, C_F\, e_f^2}{W^2}\, \widetilde{\cal H}_{T,L}(M_{jj},z,Q)f_1^q(x=0, \k)
\,  ,
\label{eq:corrlim2}
\ee
%
...
$H_T$ is much larger than $H_L$, hard factors decrease  with $M_{jj}$, and $H_T$ is notably flat with $Q$. 
...
Note that in the denominator of Eq.~(\ref{eq:corrlim2}) we have made the high-energy approximation $2q^+P_{{tar}}^-\simeq W^2=(q+P_{tar})^2$ with $W$ being the center of mass energy of the photon-target collision. This suppression of Eq.~(\ref{eq:corrlim2}) as $1/W^2$ at high energy is characteristic of a next-to-eikonal contribution, and here it is due to the exchange of quarks in the $t$-channel instead of only gluons.  
%
...
%%
 ...
 %
\begin{figure}[htb]
 \includegraphics[width=\columnwidth]{HardFactors_vs_Mjj.pdf}
 \vskip 0.2cm
 \includegraphics[width=\columnwidth]{HardFactors_vs_z.pdf}
 \vskip 0.2cm
\includegraphics[width=\columnwidth]{HardFactors_vs_Q.pdf}
...
While additional studies are required, we think that with these experimental cuts the process of quark-gluon dijet production in DIS at the EIC is a promising new method to probe the quark TMD distribution in addition to semi-inclusive DIS and Drell-Yan production, see e.g.~\cite{Angeles-Martinez:2015sea} and refs. therein.
...
the research grant no. 2018/31/D/ST2/00666 (SONATA 14). NA has received financial support
...
\subsection{Definition of the Wilson lines}

The Wilson lines that account for the multiple scattering of the projectile partons off the target are defined in the standard way as 
%
\ba
\hspace{-0.5cm}
...
In order to setup our calculation, we need to specify the scaling behavior of the quark background field $\Psi(z)$ of the target. It is convenient to introduce the projections
%
\ba
\Psi^{(-)}(z)
&\equiv &
...
In our calculation at next-to-eikonal (NEik) accuracy, only the components $\Psi^{(-)}(z)$ matter, whereas the components $\Psi^{(+)}(z)$ would contribute only to corrections which are further suppressed at high energy (next-to-next-to-eikonal (NNEik) and beyond).  
 
At $S$-matrix level, both contributions (\ref{bef_S_gen}) and (\ref{in_S_gen}) have an overall scaling as $(\gamma_t)^{-\frac{1}{2}}$ under a large boost of the target along the $x^-$ direction, resulting from the enhancement as  $(\gamma_t)^{\frac{1}{2}}$ of $\Psi^{(-)}(z)$  and from the suppression as $(\gamma_t)^{-1}$ due to the integration in $z^+$ over the Lorentz contracted support of $\Psi(z)$, representing the width of the target (see Refs.~\cite{Altinoluk:2020oyd, Altinoluk:2021lvu, Altinoluk:2022jkk}).
...
As a remark, from Eqs.~(\ref{bef-def}) and (\ref{in-def}), one obtain as well contributions induced by the instantaneous part of the intermediate quark propagator $S_F(w,z)$. In that case, the photon annihilation and the gluon emission happens simultaneously (meaning $w^+=z^+$). However, due to the structure of the instantaneous part of the quark propagator and to the light-cone gauge polarization vectors for QCD and QED, the enhanced components $\Psi^{(-)}(z)$ of the quark background field are projected out, and only the suppressed components $\Psi^{(+)}(z)$ survive. For that reason, the diagrams with simultaneous photon annihilation and gluon emission contribute only at NNEik accuracy at cross section level instead of NEik for the terms calculated in the present paper.



\subsection{Propagators from inside to outside of the target}

The quark propagator from the point $y$ inside the target to the point $x$ after the target is derived in Ref.~\cite{Altinoluk:2022jkk} at eikonal accuracy. The final result reads
%
\ba
&&
...
\ea
%
where $p^-\equiv (\p^2+m^2)/(2p^+)$ and the covariant derivatives are defined as 
%
\ba
...
\ea
%
In a similar way, the quark propagator from the point $y$ inside the target to the point $x$ before the target can be calculated at eikonal accuracy \cite{Altinolukxyz}. The final result reads
\ba
&&
...
\ea
%
Finally, the gluon propagator from the point $y$ inside the target to the point $x$ after the target can be calculated at eikonal accuracy \cite{Altinolukxyz} and the result reads
 \begin{widetext}
...
\left[ g^\nu_{\;\;  j}+\frac{g^{\nu +}}{p^+}\left(\p^j+i\, \overleftarrow{{\cal D}^A_{\y^j}}\right)
 \right] e^{iy^-p^+}e^{-i\y\cdot \p}\, . 
\ea
These three propagators are used in Eqs. (\ref{bef-def}) and (\ref{in-def}) to compute the $S$-matrix element explicitly. 

%relation between the S-matrix, amplitude and cross section in the $z^-$ independent case.\\


%

 \subsection{Cross section in general kinematics}
 The quark-gluon dijet production cross section in DIS for the longitudinal photon case at next-to-eikonal accuracy can be written in general kinematics (without assuming the correlation limit) as 
%
\ba
&&
(2\pi)^6\, (2p_1^+)\, (2p_2^+)\, 
...
=
(2q^+) \, (2\pi) \delta(p_1^++p_2^+-q^+)\, \sum_{h_1,\lambda_2}\sum_{\rm col.} \big| {\cal M}_{bef}^L\big|^2,
\ea
...
%
\ba
 \sum_{h_1,\lambda_2}\sum_{\rm col.} \big| {\cal M}_{bef}^L\big|^2&=& \frac{e_f^2\, \alpha_{\rm em}\, \alpha_s}{(2q^+)}
 16 Q^2\, z^3(1-z)^2
...
= 2q^+ \, (2\pi)\, \delta(p_1^++p_2^+-q^+)\, \frac{1}{2}\sum_{\lambda, \lambda_2h_1}\sum_{ {\rm col.}}\big| {\cal M}^T_{bef}+{\cal M}^T_{in}\big|^2,
\ea
%
with 
%
\ba
\frac{1}{2}\sum_{\lambda, \lambda_2h_1}\sum_{ {\rm col.}}\big| {\cal M}^T_{in}\big|^2&=&
...
\ba
&&
\frac{1}{2}\sum_{\lambda, \lambda_2h_1}\sum_{ {\rm col.}}\big| {\cal M}^T_{bef}\big|^2=\frac{e_f^2\, \alpha_{\rm em}\, \alpha_s \, 4z}{(2q^+)}
...
\ba
&&
\frac{1}{2}\sum_{\lambda, \lambda_2h_1}\sum_{ {\rm col.}}\big( {\cal M}^{T\dagger}_{bef}{\cal M}^T_{in}+{\cal M}^T_{bef}{\cal M}^{T\dagger}_{in} \big)=-
...
%
%
\subsection{CGC-like target average vs. quantum expectation value}
