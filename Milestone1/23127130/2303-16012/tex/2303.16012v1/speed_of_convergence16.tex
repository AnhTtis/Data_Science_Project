...
\maketitle
\begin{abstract}
The Fourier cosine expansion (COS) method is used for pricing European
options numerically very efficiently. To apply the COS method, one
has to specify two parameters: a truncation range for the density
...
an exponential order of convergence if the density is smooth and decays
exponentially. But, if the density is smooth and has heavy tails like
in the Finite Moment Log Stable model, the COS method has not an exponential
...
options very fast because stock price models are typically calibrated
...
routine, model prices of call and put options need to be evaluated
very often for different model parameters, see \cite{fang2009novel,junike2022precise}. 

To compute the price of a European option, one has to solve an integral
involving the product of the density of the log-returns at maturity
and the payoff function. But, for many financial models, the density
...
be used to retrieve the density. 
...
option prices. COS method requires two parameters: a truncation range
for the density of the log-returns and a number of terms $N$ to approximate
the truncated density by a cosine series. While it is known how to
choose the truncation range, see \cite{junike2022precise}, the choice
of $N$ is based largely on trial and error. The COS method has been
extended and applied extensively, see \cite{fang2009pricing, fang2011fourier, grzelak2011heston,ruijter2012two,zhang2013efficient, leitao2018data, liu2019neural, liu2019pricing, oosterlee2019mathematical,bardgett2019inferring}.
Other Fourier pricing techniques are discussed e.g. by \cite{carr1999option, lord2008fast, ortiz2013robust, ortiz2016highly}.

Relative to these papers we make two main contributions: a) we develop
an explicit, useful and rigorous bound for $N$ and b) we analyze
the order of convergence of the COS method in detail.

\cite{fang2009novel} proposed to approximate the (unknown) density
in three steps: i) truncate the density $f$, i.e. approximate $f$
by a function $f_{L}$ with finite support on some (large enough)
...
from the characteristic function of $f$. Hence, to apply the COS
method, two choices need to be made: find a suitable truncation range
...
\end{equation}
The first, second and third term at the right-hand-side of Inequality
(\ref{eq:error_bound}) corresponds to approximations due to i), ii)
and iii), respectively. 

\cite{junike2022precise} developed explicit expressions for bounds
for the first and third term on the right-hand-side of Inequality
(\ref{eq:error_bound}), which only depend on the choice of $L$,
not on the choice of $N$. If $f$ has semi-heavy tails, i.e. the
...
Recently, \cite{aimi2023fast} proposed some indications how to choose
$N$. But, to the best of our knowledge, a rigorous bound for $N$
to ensure a certain precision of the COS method is not known yet.
...
In this article, we use that idea to find an explicit and useful bound
for $N$, which involves the bound of the $j^{th}$-derivative of
the density $f$. 

For some models like the Black-Scholes model, see \cite{black1973pricing},
the normal inverse Gaussian (NIG) model, see \cite{barndorff1997normal}
or the Finite Moment Log Stable model (FMLS), see \cite{carr2003finite},
we find sharp bounds for all derivatives of the density of $f$ in
closed-form and obtain an explicit and useful bound for $N$, which
is provable large enough to ensure that the COS method converges within
...
COS method with focus on the second term on the right-hand-side of
...
for smooth density functions and compares favorable to the Carr-Madan
formula, see \cite{carr1999option}. 

In this article, we also take into account the error introduced by
the truncation range, i.e. errors due to i), ii) and iii) and establish
upper bounds for the order of convergence of the COS method. We confirm
both theoretically and empirically that the COS method indeed converges
(at least) exponentially for smooth density functions if additionally
the tails of the density decay at least exponentially. 

But, for fat tailed and smooth densities like the Cauchy density or
the density of the log-returns in the FMLS model the truncation error
due to i) and iii) becomes much more relevant compared to densities
with semi-heavy tails. We show theoretically that the COS method converges
at least as $O(N^{-\alpha})$ for $N\to\infty$, where $\alpha>0$
...
that the COS method converges like $O(N^{-\alpha})$, i.e. the theoretical
bound is sharp.

This article is structured as follows: Section \ref{sec:Overview:-the-COS}
provides an overview of the technical details of the COS method. Section
\ref{sec:How-to-find} provides explicit formulas for the truncation
range $L$ and the number of terms $N$. Section \ref{sec:Convergence-rate-of}
analyzes the order of convergence of the COS method. We distinguish
in Sections \ref{sec:How-to-find} and \ref{sec:Convergence-rate-of}
models with semi-heavy tails and models with heavy tails. Section
\ref{sec:Numerical-experiments} provides numerical experiments confirming
the theoretical findings. Section \ref{sec:Conclusions} concludes. 
...
on some filtered probability space. We assume there is a bank account
paying continuous compounded interest $r\in\mathbb{R}$. The market
...
at $T$, where $g:[0,\infty)\to\mathbb{R}$. For instance, a European
put option with strike $K>0$ can be described by $g(x)=\max(K-x,0)$,
$x\geq0$. 

At several places, we assume that the payoff function is bounded.
Prices of European call options are not bounded but can be obtained
from prices of put options by the put-call parity.

Provided the characteristic function $\varphi_{\log(S_{T})}$ of $\log(S_{T})$
is given in closed-form, the COS method is able to price the European
option numerically very fast as follows: we denote by
\[
...
\]
We assume, $X$ has a density $f$ but the precise structure of $f$
does not need to be known. Because the density of $X$ is centered
around zero, it is justified to truncate the density $f$ on a symmetric
...
\end{align*}
We need a few abbreviations to discuss the COS method: for $L>0$
let $f_{L}:=1_{[-L,L]}f$. Define the basis functions 
\[
...
\end{equation}
To keep the notation simple, we suppress the dependency of $e_{k}$
and $c_{k}$ on $L$ and the dependency of $v_{k}$ on $M$. The COS
methods states that the price of the European option can be approximated
by
\begin{equation}
...
if $\varphi$ is given analytically and the coefficients $v_{k}$
can be computed explicitly as well in important cases e.g. for plain
vanilla European put or call options and digital options, see e.g.
appendix in \cite{junike2022precise}. This makes the COS method numerically
very efficient and robust. 

To give the Approximation (\ref{eq:COS_method}) a precise meaning,
we need a bound for the term
\begin{equation}
...
The class of COS-admissible densities is very large, it includes in
particular bounded densities with existing first and second moments.
...
\end{proof}
Often, it is fine to choose $M=L$, e.g. if we apply the COS method
to densities with semi-heavy tails. But, if the density has heavy
tails, it is usually numerically more efficient to choose $L$ and
$M$ differently.

The main requirement to apply the COS method is the availability of
the characteristic function in closed-form. Many models satisfy this
requirement and we list a few examples: 
\begin{description}
...
\end{description}
To apply the COS method, one has to specify two parameters: $L>0$
to define the truncation range and $N\in\mathbb{N}$ to set the number
of terms. The former can be estimated using assumptions on the tail-behavior
of the density. The latter depends on the smoothness of the density. 
...
in the models BS, NIG, Heston and CGMY with parameter $Y\in(0,1)$,
are infinitely many times differentiable and have semi-heavy tails,
...
The stable law has been used for stock price modeling since \cite{mandelbrot1997variation}
...
It is infinitely many times differentiable and has heavy tails at
the left with Pareto index $\alpha$, i.e. the tails decay like $|x|^{-1-\alpha}$,
$x\to-\infty$, but the right tails decay exponentially, see \cite{carr2003finite}.
This makes the FMLS very appealing from a theoretic point of view:
prices of put and call options and all moments of the underlying stock
price $S_{T}$ exists. The expectation of the log-returns also exists
but the log-returns do not have finite variance. Fitting the FMLS
model to real market data shows very satisfying results, see \cite{carr2003finite}. 

\section{\label{sec:How-to-find}How to find $M$, $L$ and $N$}

We summarize assumptions on the density $f$ of the log-returns in
order to find explicit expressions for $L$ and $N$.
...
large enough. We suppose that $f$ satisfies one of the following
assumptions:
\begin{description}
...
only depends on the choice of $N$. It is in similar form well known
to literature, see e.g. Theorem 1.39 in \cite{plonka2018numerical},
...
functions with range $[-1,1]$ or $[0,2\pi]$. Here, we need explicitly
the dependency of the series-truncation error on the truncation range
$[-L,L]$, therefore we state the full proof. 
\begin{lem}
...
= & \frac{1}{L}\bigg|\Re\bigg\{\sum_{j=0}^{J}i^{j+1}\frac{(2L)^{j+1}}{(k\pi)^{j+1}}\left(f^{(j)}(-L)-(-1)^{k}f^{(j)}(L)\right)\\
 & +e^{i\frac{k\pi}{2}}i^{J+1}\frac{(2L)^{J+1}}{(k\pi)^{J+1}}\int_{-L}^{L}f^{(J+1)}(x)e^{ik\pi\frac{x+L}{2L}}dx\bigg\}\bigg|\\
...
$L$ can be found applying Markov's inequality, see \cite{junike2022precise}
or Theorem \ref{thm:find_N_A_B} below. Next, we provide ideas and
examples how to estimate $H_{J+1}$. If $f\in C_{b}^{J+1}(\mathbb{R})$
is a density with characteristic function $\varphi$ it holds that%
\begin{comment}
\[
...
with mean zero and variance $\sigma^{2}T$. As the normal distribution
is a member for the stable distributions, we obtain with help of Equation
(\ref{eq:stable_bound}) that
\begin{align*}
...
and $\sigma>0$, the centralized log-returns at time $T>0$ are stable
distributed with stability parameter $\alpha$, skewness $\beta=-1$,
scale $c=\sigma T^{\frac{1}{\alpha}}$ and location $\theta=0$, see
\cite{carr2002fine}. An upper bound for the density $f_{\text{FMLS}}$
of the log-returns and its derivatives can directly be obtained from
...
$N$ and $L$ if $f$ satisfies Assumption A, to ensure that the COS
method approximates the true price within a predefined error tolerance
...
small enough. Assume $J\in\mathbb{N}_{0}$ and $f$ satisfies Assumption
...
\begin{equation}
\int_{\mathbb{R}\setminus[-M,M]}\big|v(x)f(x)\big|dx\leq K\int_{\mathbb{R}\setminus[-M,M]}f(x)dx=K\frac{\mu_{n}}{M^{n}}\leq\frac{\varepsilon}{2}.\label{eq:Markov_semi}
\end{equation}
...
to choose $n\in\{4,6,8\}$ for $\mu_{n}$. By Theorem \ref{thm:find_N_A_B}
...
But that could be time-consuming and in Sections (\ref{subsec:BS-model})
...
provides examples indicating that the bound in Equation (\ref{eq:N_J>0})
...
offers a bound for $B(L)$, but we cannot apply it to all densities
of the stable distributions because the assumptions in that Proposition
are not satisfied for all stability parameter $\alpha\in(0,2]$ of
...
\]
If the mode of $f$ is inside $[-L,L]$ then 
\[
...
tails). Let $v:\mathbb{R}\to\mathbb{R}$ be bounded, with $|v(x)|\le K$
for all $x\in\mathbb{R}$ and some $K>0$. Let $\varepsilon>0$ be
small enough. Assume $J\in\mathbb{N}$ and $f$ satisfies Assumption
...
\[
\int_{\mathbb{R}\setminus[-M,M]}\big|v(x)f(x)\big|dx\leq K\int_{\mathbb{R}\setminus[-M,M]}f(x)dx=\frac{2KC_{3}}{\alpha}M^{-\alpha}\leq\frac{\varepsilon}{2}
\]
...
\end{proof}
\begin{example}
\label{exa:C3}Recall that the log-returns of the FMLS model are stable
distributed. Denote by $F_{\text{Stable}}$ the cumulative distribution
...
(\ref{eq:N_stable}) and the bounds $H_{j}$ are given explicitly
...
$M$ and $L$ depending on $N$. We only describe the asymptotic behavior
...
\end{proof}
\begin{rem}
The COS method converges exponentially, i.e. faster then $O(N^{-q})$
...
does not converges faster than $O(N^{-\alpha})$ for the FMLS model,
...
All numerical experiments are compared to the Carr-Madan formula,
see \cite{carr1999option}, which is applicable if the characteristic
function of the log-returns is given in closed-form and if $E[S_{T}^{1+\gamma}]$
is finite for some $\gamma>0$ called \emph{damping factor}. If not
stated otherwise, we use the Carr-Madan formula with $N=2^{17}$ terms,
we set the damping factor equal to $\gamma=0.1$ and use a Fourier
...
All numerical experiments are carried out on a modern laptop (Intel
...
For the BS model, we compute $L$ using Equation (\ref{eq:M}) and
$N$ as in Equation (\ref{eq:N_J>0}) and compare $N$ with $N_{\min}$,
which is the minimal number of terms such that the absolute difference
of the price of the COS method and the Black-Scholes price are less
than the error tolerance. Table \ref{tab:BS} shows that $N$ is only
about $40\%$ larger than $N_{\min}$. 

How could the number of terms $N$ be estimated if there are no closed-form
solutions available for the bounds of the derivatives of the density
of the log-returns? We propose to solve the right-hand-side of Inequality
(\ref{eq:H_j}) numerically to obtain $H_{j+1}$. Here, we use R's
function \emph{integrate} with default values. The CPU time of the
COS method and the numeric integration routine to obtain $H_{j+1}$
are of similar magnitude, see Table \ref{tab:BS}. 

The value for $N$ does not change when using numeric integration
...
Also in Section \ref{subsec:FMLS-model} we use $j=40$. 
...
\ref{thm:find_N_A_B} does not help much to find the number of terms
$N$ for the COS method if the density of the log-returns is not sufficiently
smooth. 

By $f_{\text{VG}}$ we denote the density of the log-returns in the
VG model at maturity $T>0$ with parameters $\sigma>0$, $\theta=0$
and $\nu>0$. If $T<\frac{\nu}{2}$, the density $f_{\text{VG}}$
is unbounded and Theorem \ref{thm:find_N_A_B} cannot be applied to
find $N$. If $T\in\left(\nu,\frac{3\nu}{2}\right)$, the derivative
of $f_{\text{VG}}$ is continuous but the second derivative of $f_{\text{VG}}$
...
Optimizing the derivative of the density $f_{\text{VG}}$ numerically,
we obtain $H_{1}=218$ and Theorem \ref{thm:find_N_A_B} suggests
$N\approx8\cdot10^{12}$. 

We computed a reference price $\pi_{CM}=1.809833$ by the Carr-Madan
...
at time $T>0$ have heavy tails and are stable distributed with stability
...
for $L$ and $N$ for the FMLS model with those parameters for a European
...
$x\mapsto C_{3}|x|^{-1-\alpha}$. The left tails decay indeed like
Pareto tails. The right tails decay much faster, they decay exponentially,
...
$N=10^{7}$. We use the explicit formula for $H_{j+1}$ from Example
\ref{exa:FMLS} and Equation (\ref{eq:N_stable}) find $N=5451$,
where we set $j=40$. By Theorem \ref{thm:find_N_C}, any other choice
for $j$ is possible but another value for $j$ does not significantly
improve $N$.

Applying Equations (\ref{eq:M_stable}) and (\ref{eq:L_stable}),
we obtain $M=69$ and $L=176$. With these parameters, the COS method
...
We also apply the Carr-Madan formula with the ``standard parameters'',
...
truncation range of $1024$. With those standard parameters, the Carr-Madan
formula prices the option within the error tolerance in about 1.1
milliseconds. 

The value for the CPU time is about three times larger applying Theorem
\ref{thm:find_N_C} to get $N$ compared to the optimized value for
$N$, which is acceptable in our point of view. The computational
...
\subsection{\label{subsec:Convergence-Rate}Convergence Rate}

In this section, we analyze empirically the order of convergence of
...
we analyze how the error of the COS method in the BS model behaves
...
constant at a certain value of $N$. This can be explained by Inequality
...
Inequality (\ref{eq:error_bound}) do not improve when increasing
$N$ but leaving $L$ constant.

For large $L$, like $L=20\sigma$, this effects somewhat vanishes
because the tails of the Gaussian distribution decay so fast that,
up to fixed-precision arithmetic\footnote{The software package R works with a precision of 53 bits conforming
to the IEC 60559 standard, see R Core Team (2021). R: A language and
environment for statistical computing. R Foundation for Statistical
Computing, Vienna, Austria. URL https://www.R-project.org/.}, the Gaussian density could be seen as a density with finite support.
Using arbitrary-precision arithmetic instead should show that also
for $L=20\sigma$, the error of the COS method does not converge to
zero for $N\to\infty$. 
...
case of the Cauchy density, we set
\[
...
those values are taken from \cite{carr2003finite}, who calibrated
the FMLS model to real market data. Then we compute the price of a
European call option with maturity $T=1$, $S_{0}=K=100$ and $r=0$
by the Carr-Madan formula.
...
\]
is a sufficient fine grid of the interval $[1,10^{6}]$. Figure \ref{fig:L_optimal}
shows $L_{\text{optimal}}$ over $N$ for the Cauchy and the FMLS
model. We see that the relationship between $\log(N)$ and $\log(L_{\text{optimal}})$
is approximately linear with slope $0.95$ for the Cauchy model and
slope $0.86$ for the FMLS model. 
...
For the Cauchy model, the order of convergence can be estimated to
$O(N^{-0.92})$ from Figure \ref{fig:Convergence-rate-cauchy} if
we choose $L=\frac{1}{10}N$. The order of convergence using $L_{\text{optimal}}$
is of similar size. This order of convergence is very close to the
theoretical bound of $O(N^{-1})$ predicted by Theorem \ref{thm:asy}(ii). 
...
theoretical bound $O(N^{-1.5597})$ and does not differ much if we
use $L_{\text{optimal}}$ instead. 
...
with damping factor $0.1$ and Fourier truncation range of $1200$.
...
and showed in numerical experiments the usefulness of those formulas
in applications. The densities of the log-returns for many models
in finance are smooth, examples include the BS, NIG, Heston or FMLS
models.

If the density is non-differentiable, neither Theorem \ref{thm:find_N_A_B}
nor \ref{thm:find_N_C} can be applied to find a bound for $N$. If
the density is only a few times differentiable, this holds e.g. for
...
could be to use the NIG model instead of the VG model. Both are flexible,
three parameter Lévy models.

We further analyzed the order of convergence of the COS method and
observed both theoretically and empirically that models enjoy exponential
convergence if the densities of the log-returns are smooth and have
semi-heavy tails. But, if the density of the log-returns is smooth
and has \emph{heavy} \emph{tails}, the order of convergence of the
COS method can be bounded by $O(N^{-\alpha})$, where $\alpha>0$
is the Pareto tail index. This is the case e.g. for the FMLS model
where $\alpha\in(1,2)$. Numerical experiments indicate that the bound
$O(N^{-\alpha})$ is sharp.
