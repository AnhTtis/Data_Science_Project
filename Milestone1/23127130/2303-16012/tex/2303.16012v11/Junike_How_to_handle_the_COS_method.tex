...
Germany, ORCID: 0000-0001-8686-2661, E-mail: gero.junike@uol.de}}
\maketitle
\begin{abstract}
...
\\
\textbf{Keywords: }COS method, option pricing, Greeks, number of terms,
order of convergence, heavy tails\\
...
To calibrate stock price models, it is crucial to price European options quickly
...
often for different model parameters, see \citet{fang2009novel,junike2022precise}. 
...
The COS method has been extensively extended and applied, see \citet{fang2009pricing, fang2011fourier, grzelak2011heston,ruijter2012two,zhang2013efficient, leitao2018data, liu2019neural, liu2019pricing, oosterlee2019mathematical,bardgett2019inferring,junike2022precise}.
...
\citet{junike2022precise} apply a simple triangle inequality  to bound
...
\citet{junike2022precise} developed explicit expressions for bounds
...
tails decay exponentially, \citet{junike2022precise} apply Markov's
inequality to obtain an explicit and useful expression for $L$. 

Recently, \citet{aimi2023fast} proposed some hints on how to choose
...
an explicit and useful bound for $N$, which is provably large enough
...
as the density of the log-returns in the Finite Moment Log Stable
(FMLS) model (see \citet{carr2003finite}), the truncation error due
to i) and iii) becomes much more relevant compared to densities with
semi-heavy tails. We show theoretically that the COS method converges
...
\citet[Sec. 2.2.1]{ruijter2015application} or \citet{leitao2018data}.
However, to the best of our knowledge, there is still no rigorous
understanding of how to approximate the Greeks using the COS method.
Another contribution of this article is thus to provide conditions
under which Delta and Gamma can be safely approximated by the COS
method. 
...
interval $[-L,L]$, see \citet{junike2022precise}. Define
\[
...
is weighted by one-half. A little analysis, see \citet{fang2009novel},
shows that 
\[
...
in \citet{junike2022precise}. This makes the COS method numerically
...
\end{equation}
\citet{junike2022precise} call integrable functions $f$ with $B_{f}(L)\to0$,
...
see proof of Theorem 7 in \citet{junike2022precise}. 
...
and stable densities, see \citet{junike2022precise}. 
\begin{lem}
...
do not know the exact behavior of the tails. For such densities, \citet{junike2022precise}
...
tails, i.e., the tails decay exponentially, see \citet{kuchler2013tempered, asmussen2022role,albin2009asymptotic,schoutens2003levy,dragulescu2002probability}. 
...
and \citet{fama1965behavior}. The FMLS model models the log-returns
...
exponentially, see \citet{carr2003finite}. This makes the FMLS very
...
shows very satisfactory results, see \citet{carr2003finite}. 
...
differentiable with bounded derivatives. By $f^{(j)}$ we denote the
$j^{th}$-derivative of $f\in C_{b}^{J+1}(\mathbb{R})$. We use the
convention $f^{(0)}\equiv f$. By $\left\Vert .\right\Vert _{\infty}$
...
\end{assumption}
\begin{rem}
Using $f(x)=\frac{1}{2\pi}\int_{\mathbb{R}}e^{iux}\varphi(u)du$ and
the dominated convergence theorem, it is easy to see that $f\in C_{b}^{J+1}(\mathbb{R})$
if the functions $u\mapsto|u^{j}\varphi(u)|$, $j=0,...,J+1$, are
integrable. This holds for instance if $\varphi$ decays exponentially
for $u\to\pm\infty$. 
\end{rem}
...
\begin{rem}
In Theorem \ref{thm:find_N_C} we treat models with Pareto-tails;
i.e., the density for the log-returns behaves like
\begin{equation}
f(x)\sim|x|^{-1-\alpha},\quad x\to\pm\infty.\label{eq:sym_Pareto}
\end{equation}
The right-hand side of Equation (\ref{eq:B}) is obtained by differentiating
the right-hand side of (\ref{eq:sym_Pareto}). We assume in Theorem
\ref{thm:find_N_C} that $C_{3}$ and $\alpha$ are known. The exact
tail-behavior of the density of the log-returns is indeed known for
the stable law, in particular for the FMLS model; see Example \ref{exa:C3}.
\end{rem}
...
\end{align*}
As $(e_{k}^L)_{k\in\mathbb{N}}$ is an orthonormal basis it follows that $\langle e_{k}^L,e_{l}^L\rangle=L \delta_{k,l}$ for $(k,l)\ne(0,0)$ and hence
\begin{equation}
...
$L$ can be found by applying Markov's inequality, see \citet{junike2022precise},
...
\citet{carr2003finite}. An upper bound for the density $f_{\text{FMLS}}$
...
\citet[Cor. 9 and Remark 11]{junike2022precise}. 
...
satisfying Assumption \ref{A1}. For some even $n\in\mathbb{N}$ let
\begin{equation}
...
\end{equation}
Let $\xi=\sqrt{2M}K$. If $J\geq1$, let $\ell\in\{0,...,J-1\}$ and
$k\in\{1,...,J-\ell\}$ and
\begin{align}
...
\begin{equation}
N\geq\left(\frac{4\left\Vert f^{(1)}\right\Vert _{\infty}L^{\frac{3}{2}}}{\pi}\frac{6\xi}{\varepsilon}\right)^{2}.\label{eq:N_j=00003D0}
\end{equation}
...
\ref{A1} holds, i.e., $L_{0}\leq M=L$. It follows that
\begin{equation}
...
By Inequality (\ref{eq:N>=00003D3LC2/pi}) it follows that
\begin{align*}
...
\end{proof}
\begin{rem}
The formula in Equation (\ref{eq:M}) for $M$ and $L$ is taken from
\citet[Cor. 9]{junike2022precise} and based on Markov's inequality:
It holds by the definition of $M$ that
\[
\int_{\mathbb{R}\setminus[-M,M]}\big|v(x)f(x)\big|dx\leq K\int_{\mathbb{R}\setminus[-M,M]}f(x)dx\leq K\frac{\mu_{n}}{M^{n}}\leq\frac{\varepsilon}{2},
\]
i.e., Inequality (\ref{eq:Cor8_1}) in Lemma \ref{lem:cor8} is satisfied
for $f$ for \emph{any} $\varepsilon>0$. We do not need to know the
exact tail behavior of $f$ to obtain a bound for $M$. We use Assumption
\ref{A1} to ensure that Inequality (\ref{eq:Cor8_1}) is also satisfied
...
to a fixed value, i.e., $k=40$. Bounds for $\|f^{(k+1+\ell)}\|_{\infty}$
are explicitly known for some models, see Examples \ref{exa:Gauss},
\ref{exa:NIG} and \ref{exa:FMLS}. These bounds can also be obtained
numerically, see Section \ref{subsec:BS-model}. Section \ref{sec:Numerical-experiments}
contains examples indicating that the bound in Inequality (\ref{eq:N_J>0})
for $N$ is often sharp and very useful in applications.
\end{rem}

To extend Theorem \ref{thm:find_N_A_B} to the case in which $f$
satisfies Assumption \ref{A2}, i.e., $f$ has heavy tails, we need
a bound for the term $B_{f}(L)$, defined in Equation (\ref{eq:B(L)}).
Proposition 2 in \citet{junike2022precise} provides a bound for $B_{f}(L)$,
but we cannot apply it to all densities of the stable distributions
because the assumptions in that Proposition are not satisfied for
all stability parameters $\alpha\in(0,2]$ of the stable law. Therefore,
we need the following lemma. We use the following abbreviation: the
maximum of two real numbers $x,y$, is denoted by $x\lor y$.
\begin{lem}
\label{lem:COS}Let $f\in C_{b}^{1}(\mathbb{R})$ be integrable such
that 
\[
...
To see this, note that the unimodality implies the assertion in line
(\ref{eq:lemma_monotone}). Further,
\[
...
of the stable law is unimodal, see \citet{yamazato1978unimodality}. 
\begin{thm}
...
Let $k\in\{1,...,J\}$ and
\begin{align}
N\geq\left(\frac{2^{k+2}\left\Vert f^{(k+1)}\right\Vert _{\infty}L^{k+\frac{3}{2}}}{k\pi^{k+1}}\frac{12\xi}{\varepsilon}\right)^{\frac{1}{k}}.\label{eq:N_stable}
\end{align}
Then it holds that
\[
\left|\int_{\mathbb{R}}v(x)f(x)dx-\sum_{k=0}^{N}{}^{\prime}c_{k}v_{k}\right|\leq\varepsilon.
...
\end{equation}
Use Inequality (\ref{eq:1/N}),
the definition of $L$ and $\frac{96}{\pi^2}\leq12$,
to see that
\begin{equation}
...
definition of $N$ in Inequality (\ref{eq:N_stable}), we also have 
\[
...
\]
Let $L=L(N)$. We say the error of the COS method converges \emph{with
...
(i) Let $f$ be a density satisfying Assumption \ref{A1}. Let $\beta\in\left(0,\frac{J}{J+3}\right)$
...
\end{align}
$\beta<\frac{J}{J+3}$ implies $-J(1-\beta)+2\beta<0$
...
\end{align*}
The last inequality can be seen as follows: as $\beta\leq\frac{J}{J+2+\alpha}$, it follows
that
\[
...
like $O(N^{-\alpha})$ as $N\to\infty$ if $f$ is smooth and has
heavy tails with Pareto index $\alpha>0$. Numerical experiments indicate
...
the approximation of Delta and Gamma by the COS method. We proof these
formulas and discuss how to choose $M$, $L$ and $N$ for the Greeks. 
...
$f$ is a density satisfying Assumption \ref{A1}. It follows that
...
the Greeks. We define $v_{k}:=\int_{-M}^{M}v(x,S_{0})e_{k}^L(x)dx$
...
Let $\varepsilon>0$ be small enough. Let $\gamma=\min\big\{\varepsilon,\,\varepsilon S_{0},\frac{\varepsilon S_{0}^{2}}{2}\big\}$.
...
We set $k=40$ to obtain $N=258$ by Inequality (\ref{eq:N_J>0-1}).
Other values for $k$ and $N$ for the price are reported in Table \ref{tab:BS}.
...
than the error tolerance. $N$ is about twice as larger as $N_{\min}$. 
...
\hline 
$N$  & 988 & 284 & 207 & 183 & 175 & 173 & 173\tabularnewline
\hline 
...
$C_{3}$ in Equations (\ref{eq:M_stable}) and (\ref{eq:L_stable})
to
\[
...
(\ref{eq:M_stable}) and (\ref{eq:L_stable}). We set $N=5815$, using
...
The value for the CPU time is about five times higher when using
...
We consider a call options with parameters $\sigma=0.2,$ $S_{0}=K=100$,
...
each $N\in\{2^{i},\,i=4,5,...,20\}$ we define
\[
...
nor Theorem \ref{thm:find_N_C} can be used to find a bound for $N$.
If the density is only differentiable a few times, which is the case
for the VG model for some parameters and short maturities, our bound
for $N$ is too large to be useful in most practical applications.
A solution might be to use the NIG model instead of the VG model.
Both are flexible, three-parameter Lévy models.
