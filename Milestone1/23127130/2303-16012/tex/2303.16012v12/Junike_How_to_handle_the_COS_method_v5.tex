...
The COS method has been extensively extended and applied, see \citep{fang2009pricing,fang2011fourier,grzelak2011heston,ruijter2012two,zhang2013efficient,leitao2018data,liu2019neural,liu2019pricing,oosterlee2019mathematical,bardgett2019inferring}.
Other Fourier pricing techniques are discussed e.g., by \citep{carr1999option,lord2008fast,ortiz2013robust,ortiz2016highly}.
...
and the CGMY model with parameter $Y\in(0,1)$, see \citep{carr2002fine,albin2009asymptotic,kuchler2013tempered,asmussen2022role}.
...
The filtration $(\mathcal{F}_{t})_{t\geq0}$ satisfyies the usual
conditions and $\mathcal{F}_{0}=\{\Omega,\emptyset\}$. We assume
that there is a bank account paying continuous compound interest $r\in\mathbb{R}$
...
The prices of European call options are not bounded, but they can
be obtained from the prices of put options by the put-call parity.
...
need not be known. Since the density of $X_{T}$ is centered around
...
\end{assumption} 
\begin{rem}
Assume the functions $u\mapsto|u^{j}\varphi(u)|$, $j=0,...,J+1$,
are integrable. By Fourier inversion we have that $f(x)=\frac{1}{2\pi}\int_{\mathbb{R}}e^{iux}\varphi(u)du$.
...
We provide ideas and examples for estimating $\|f^{(j)}\|_{\infty}$.
It follows by the inverse Fourier transform and Equation (\ref{eq:Fourier_transofrm})
that 
\begin{equation}
...
\citet[Rem. 3.2]{fang2009novel} state formulas for the approximation
...
\hline 
$N$  & 988  & 284  & 206  & 183  & 175  & 173  & 173\tabularnewline
\hline 
...
satisfying
\begin{equation}
|a_{N}|\approx|c_{N}|=\frac{2}{b-a}e^{-\frac{1}{2}(\frac{\sigma\pi}{b-a})^{2}TN_{F}^{2}}\leq\varepsilon_{N_{F}},\label{eq:NF}
\end{equation}
...
\hline 
\hline 
$10^{-8}$ & $0.2$ & $6.94$ & $127$ & $179$ & $120$\tabularnewline
\hline 
$0.1$ & $1$ & $10$ & $0$ & $33$ & $10$\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
\emph{integrate }with default values\emph{. }The CPU time of the COS
method and the numerical integration routine are of similar magnitudes.
...
set $\varepsilon=10^{-4}$, $n=4$, $k=20$, $S_{0}=100$ and $r=0$
