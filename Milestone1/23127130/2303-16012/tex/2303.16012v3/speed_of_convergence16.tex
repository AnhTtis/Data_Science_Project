...
for $N$ as well. We further show that the COS method has an exponential
...
The COS method has been extensively extended and applied, see \cite{fang2009pricing, fang2011fourier, grzelak2011heston,ruijter2012two,zhang2013efficient, leitao2018data, liu2019neural, liu2019pricing, oosterlee2019mathematical,bardgett2019inferring}.
...
\end{align}
We need some abbreviations to discuss the COS method for solving the
integral at the right-hand-side of Equation (\ref{eq:price}) numerically:
for $L>0$ let $f_{L}:=1_{[-L,L]}f$. Define the basis functions 
\[
...
\begin{equation}
\left\Vert f-f_{L}\right\Vert _{2}\leq\frac{\varepsilon}{6\xi},\quad\text{and}\quad\sqrt{B(L)}\leq\frac{\varepsilon}{6\xi}.\label{eq:Cor8_2}
\end{equation}
...
density of the log-returns is known and the inequalities in (\ref{eq:Cor8_1})
...
this requirement and we list some examples: 
\begin{description}
...
$x,y$, is denoted by $x\lor y$. By $\Gamma$ we denote the gamma
function
\[
...
$\varepsilon>0$. Theorem \ref{thm:find_N_A_B} extends \citet[Cor. 9, Rem 11]{junike2022precise}.
\begin{thm}
...
satisfying Assumption A. For some even $n\in\mathbb{N}$ let
\begin{equation}
...
\begin{align}
N & \geq\frac{4L}{\pi}\lor\left(\frac{2^{j+2}H_{j+1}L^{j+1}}{j\pi^{j+1}}\frac{12\xi}{\varepsilon}\right)^{\frac{1}{j}}.\label{eq:N_J>0}
\end{align}
If $J=0$ let
\begin{equation}
...
tails). Let $v:\mathbb{R}\to\mathbb{R}$ be bounded, with $|v(x)|\le K$
for all $x\in\mathbb{R}$ and some $K>0$. Let $\varepsilon>0$ be
small enough. Suppose $J\in\mathbb{N}_{0}$. Let $f$ be a unimodal
density satisfying Assumption B. Let
\begin{align}
...
Let $j\in\{1,...,J\}$ and
\begin{align}
N\geq\frac{4L}{\pi}\lor\left(\frac{2^{j+2}H_{j+1}L^{j+1}}{j\pi^{j+1}}\frac{12\xi}{\varepsilon}\right)^{\frac{1}{j}}.\label{eq:N_stable}
\end{align}
Then
\begin{equation}
...
Example \ref{exa:f B uni}. Using the bound for $B(L)$ from Lemma
