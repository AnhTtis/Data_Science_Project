...
\usepackage{breakurl}
\hypersetup{
    breaklinks=true, % splits links across linescolorlinks=false, % displays links as colored text instead of blockspdfusetitle=true, % \title and \author values into pdf metadata
}
...
for $N$ as well for pricing and for the Greeks. We further show that
the COS method has an exponential order of convergence when the density
is smooth and decays exponentially. However, when the density is smooth
and has heavy tails, as in the Finite Moment Log Stable model, the
COS method does not have exponential order of convergence. Numerical
...
Another contribution of this article is to consider also the errors
...
exponentially. However, for fat-tailed and smooth densities such as
...
important role in risk management. The Greeks Delta or Gamma are the
first and second derivatives of the price of the option with respect
to the current price of the underlying. There are ideas in the literature
on how to approximate the Greeks Delta and Gamma of the option using
the COS method, see \citet[Rem. 3.2]{fang2009novel} or \citet{leitao2018data}.
However, to the best of our knowledge, there is still no rigorous
understanding yet how to approximate the Greeks using the COS method.
Another contribution of this article is to show under which conditions
the Greeks Delta and Gamma can be safely approximated by the COS method. 
...
discusses the numerical computation of the Greeks by the COS method.
Section \ref{sec:Numerical-experiments} contains numerical experiments
which confirm the theoretical results. Section \ref{sec:Conclusions}
concludes. 
...
approximate the derivatives of $f$ by cosine functions, in order
to approximate the Greeks, i.e. the sensitivities of the option. 
...
\]
i.e. the coefficients $c_{k}^{j}$ can be obtained explicitly if $\varphi$
...
or call options and digital options, see for instance appendix in
\cite{junike2022precise}. This makes the COS method numerically very
...
\end{equation}
\cite{junike2022precise} called integrable functions $f$ with $B_{f}(L)\to0$,
$L\to\infty$ \emph{COS-admissible.} The term $\sqrt{B_{f}(L)}$ is
equal to third term at the right-hand-side of Inequality (\ref{eq:error_bound}). 
...
and stable densities, see \cite{junike2022precise}. Theorem \ref{thm:find_N_A_B}
and Theorem \ref{thm:find_N_C} apply the next Lemma \ref{lem:cor8}
and provide explicit formulas for the parameters $M$, $L$ and $N$
of the COS method for densities with semi-heavy tails and heavy tails,
respectively.
\begin{lem}
...
For some models, e.g. the FMLS model, the exact tail behavior of the
density of the log-returns is known and the Inequalities (\ref{eq:Cor8_1})
...
proposed to use Markov's inequality to obtain explicit bounds for
$M$ and $L$.
...
by $C_{b}^{J+1}(\mathbb{R})$ the $(J+1)$-times, continuously differentiable
functions from $\mathbb{R}$ to $\mathbb{R}$ which are bounded with
bounded derivatives. By $f^{(j)}$ we denote the $j^{th}$-derivative
...
\begin{description}
\item [{Assumption~A:}] $f\in C_{b}^{J+1}(\mathbb{R})$ and $f$ has semi-heavy
tails, i.e.
\begin{equation}
|f^{(j)}(x)|\leq C_{1}C_{2}^{j}e^{-C_{2}|x|},\quad j=0,...,J+1,\quad|x|\geq L_{0}.\label{eq:A}
\end{equation}
\item [{Assumption~B:}] $f\in C_{b}^{J+1}(\mathbb{R})$ and $f$ has heavy
tails with index $\alpha>0$, i.e.
\begin{equation}
...
\end{equation}
The Inequality (\ref{eq:H_j}) provides explicit expressions to find
a bound for the term $\|f^{(j)}\|_{\infty}$, $j=0,..,J+1$ for several
models as the next examples show. The right-hand-side of Inequality
(\ref{eq:H_j}) can also be solved numerically, this is done in Section
\ref{subsec:BS-model}.
\begin{example}
...
see \cite{carr2003finite}. An upper bound for the density $f_{\text{FMLS}}$
...
$M$, $N$ and $L$ when $f$ satisfies Assumption A, to ensure that
the COS method approximates the true price within a predefined error
tolerance $\varepsilon>0$. Theorem \ref{thm:find_N_A_B} extends
...
is useful for the Greeks Delta and Gamma.
\begin{thm}
...
satisfying Assumption A. Let $\ell\in\{0,...,J-1\}$. For some even
$n\in\mathbb{N}$ let
\begin{equation}
...
\begin{align}
N & \geq\frac{3LC_{2}}{\pi}\lor\left(\frac{2^{k+2}\left\Vert f^{(k+1+\ell)}\right\Vert _{\infty}L^{k+1}}{k\pi^{k+1}}\frac{12\xi}{\varepsilon}\right)^{\frac{1}{k}}.\label{eq:N_J>0}
\end{align}
If $J=0$, let
\begin{equation}
...
of $f$. For $\varepsilon$ small enough, $M$ is large enough so
that Assumption A holds, i.e. $L_{0}\leq M=L$. It follows that,
\begin{equation}
...
\end{equation}
Assume $J\geq1$. By Equation (\ref{eq:N_J>0}) it follows that
\begin{align*}
...
are indeed satisfied because the right-hand-sides of those Inequalities
are of order $\log(\varepsilon)$. By the definition of $N$ in Equation
...
Now assume $J=0$. By the definition of $N$ in Equation (\ref{eq:N_j=00003D0}),
...
in \cite{junike2022precise} suggest to choose $n\in\{4,6,8\}$ for
$\mu_{n}$. According to Theorem \ref{thm:find_N_A_B}, any $k\in\{1,...,J-\ell\}$
is allowed to define $N$ by Equation (\ref{eq:N_J>0}). In the applications,
one could minimize $N$ over all admissible $k$. But this could be
time-consuming and in Sections \ref{subsec:BS-model} and \ref{subsec:FMLS-model}
we set $k$ to a fixed value, i.e. $k=40$. Bounds for $\|f^{(k+1+\ell)}\|_{\infty}$
are explicitly known for some models, see Examples \ref{exa:Gauss},
\ref{exa:FMLS} and \ref{exa:NIG}. Those bounds can also be obtained
...
\begin{example}
In Theorem \ref{thm:find_N_A_B} we require that $\varepsilon>0$
is \emph{small enough}. Based on the example of the Laplace model,
see \cite{madan2016adapted}, we illustrate this point. Let $\sigma>0$.
In the Laplace model, the log-returns at maturity $T>0$ have the
density
\[
...
see \citet[Exa. 3]{guillaume2019implied}. It holds that
\[
...
satisfies Equation (\ref{eq:A}) in Assumption A. Consider the following
parameters: maturity $T\in\{\frac{1}{250},1,10\}$, strike $K=100$
and $\sigma=0.2$. Use $n=8$ moments and the $k^{th}$-derivative
with $k=40$ to obtain $M$, $L$ and $N$. If $\ell=0$, set $\varepsilon=10^{-4}$.
If $\ell=1$, set $\varepsilon=10^{-6}$. If $\ell=2$, set $\varepsilon=10^{-7}$.
Inequalities (\ref{eq:M_semi}), (\ref{eq:L1_semi}), (\ref{eq:L_B_semi})
and (\ref{eq:L_N_semi}) are then satisfied, i.e. $\varepsilon$ is
small enough.
\end{example}
...
$B_{f}(L)$, see Equation (\ref{eq:B(L)}). Proposition 2 in \cite{junike2022precise}
provides a bound for $B_{f}(L)$, but we cannot apply it to all densities
...
B. Then $f$ also satisfies the assumption of Lemma \ref{lem:COS}.
To see this, note that $f$ is bounded and therefore square-integrable.
The unimodality implies Equation (\ref{eq:lemma_monotone}). Further,
\[
...
B holds, i.e. $L_{0}\leq M\leq L$. The inequality 
\[
...
definition of $N$ in Equation (\ref{eq:N_J>0}), we also have 
\[
...
Equation (\ref{eq:N_stable}) and bounds for $\|f^{(j)}\|_{\infty}$
...
(i) Let $f$ be a density satisfying Assumption A. Let $\beta\in\left(0,\frac{2J}{2J+3}\right)$
and $\gamma>0$. If $L=\gamma N^{\beta}$ it holds that
\[
\text{err}(L(N),N)\leq O\left(N^{-J(1-\beta)+\frac{3}{2}\beta}\right),\quad\text{as }N\to\infty.
\]
(ii) Let $f$ be a unimodal density satisfying Assumption B. Let $\beta\in\left(0,\frac{2J}{2J+3+2\alpha}\right)$
and $\gamma>0$. If $L=\gamma N^{\beta}$ it holds that
\[
...
$f$ satisfies Assumption A. It follows that
...
COS method approximates the price and the Greeks Delta and Gamma within
a predefined error tolerance $\varepsilon>0$. One can use the same
parameters $M$, $N$ and $L$ to obtain the price and the Greeks. 
\begin{thm}
\label{thm:Greeks}($M$, $L$ and $N$ for the price and Delta and
Gamma). Let $g$ be bounded. Let $v$ be defined as in Equation (\ref{eq:v_Greek}).
Let $\varepsilon>0$ be small enough. Let 
\[
\gamma=\varepsilon\land\varepsilon S_{0}\land\frac{\varepsilon S_{0}^{2}}{2}.
\]
Suppose $J\geq3$. Let $f$ be a density satisfying Assumption A.
...
\end{align*}
Using the triangle equation, we can see that
\begin{align*}
...
We set $k=40$ to obtain $N=179$ by Equation (\ref{eq:N_J>0-1}),
compare with Table \ref{tab:BS}. Theorem \ref{thm:Greeks} indicates
that $M,$ $L$ and $N$ serves to approximate both the price and
the Greeks Delta and Gamma by the COS method. This can be confirmed
...
and the closed-form solution by the Black-Scholes formula for price,
Delta and Gamma, are less than the error tolerance. $N$ is only about
...
(\ref{eq:H_j}) numerically to find a bound for $\|f^{(k+1)}\|_{\infty}$.
...
to obtain a bound for $\|f^{(k+1)}\|_{\infty}$ compared to the closed-form
solution for the bound of $\|f^{(k+1)}\|_{\infty}$ from Example \ref{exa:Gauss}.
In this example, $k=40$ works well. We will also use $k=40$ in Section
\ref{subsec:FMLS-model}.
...
method: $N$ depending on $k$ by Equation (\ref{eq:N_J>0}). CPU
...
Motivated by Example \ref{exa:C3}, we suggest to set the variable
$C_{3}$ appearing in Equation (\ref{eq:M_stable}) to
\[
...
Example \ref{exa:FMLS} and Equation (\ref{eq:N_stable}) with $k=40$.
...
option pricing. Also the sensitivities, i.e. the Greeks, can be efficiently
...
in applications to obtain the price of an option the Greeks Delta
and Gamma. The densities of the log-returns are smooth for many models
in finance, such as the BS, NIG, Heston or FMLS model.
