...
is thus to provide conditions under which Delta and Gamma can be safely
approximated by the COS method. 
...
i.e., the density for the log-returns behave like
\begin{equation}
...
\}
\end{comment}
We use the following abbreviation: the maximum of two real numbers
$x,y$, is denoted by $x\lor y$.
\begin{lem}
...
order $\rho>0$}, if there is a constant $\kappa>0$ such that
\begin{equation}
...
as $N\to\infty$ for any $q>0$ if $f$ is smooth and has semi-heavy
...
for the FMLS model, see Section \ref{subsec:Convergence-Rate}, i.e.,
the bound in Theorem \ref{thm:find_N_C}(ii) is sharp.
\end{rem}
...
\[
\gamma=\min\left\{ \varepsilon,\,\varepsilon S_{0},\frac{\varepsilon S_{0}^{2}}{2}\right\} .
\]
...
for $L$ and $N$ for the FMLS model with these values for $\alpha$
and $\sigma$ for a European call option with the following parameters:
\[
