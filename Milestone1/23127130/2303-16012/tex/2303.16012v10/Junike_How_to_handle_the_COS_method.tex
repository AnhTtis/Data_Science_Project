...
\[
e_{k}(x)=\cos\left(k\pi\frac{x+L}{2L}\right),\quad x\in\mathbb{R},\quad k=0,1,...
\]
...
\end{align*}
As $(e_{k})_{k\in\mathbb{N}}$ is an orthonormal basis it follows
that
\begin{equation}
...
of Inequality (\ref{eq:N_J>0}) is of order $\varepsilon^{-(\frac{1}{n}+\frac{3}{2kn}+\frac{1}{k})}$.
...
\end{equation}
Let $\varepsilon$ be small enough, use Inequality (\ref{eq:1/N}),
the definition of $L$ and $\frac{1}{1+\alpha}\leq\frac{2}{1+2\alpha}$,
to see that
\begin{equation}
L\geq\left(\frac{96C_{3}\xi}{\pi^{2}\varepsilon}\frac{\alpha+k}{N-\frac{2(\alpha+k)}{\pi}}\right)^{\frac{1}{1+\alpha}}.\label{eq:1/N_L}
\end{equation}
Using $\prod_{m=1}^{j}(\alpha+m)\leq(\alpha+k)^{j}$ for $j\leq k$,
it follows that
\begin{align*}
 & \sum_{j=1}^{k}\frac{2^{j+1}}{j\pi^{j+1}}\frac{L^{j}}{N^{j}}\left(|f^{(j)}(-L)|+|f^{(j)}(L)|\right)\\
\leq & \sum_{j=1}^{k}\frac{2^{j+1}}{j\pi^{j+1}}\frac{L^{j}}{N^{j}}\left(2C_{3}(\alpha+k)^{j}L^{-1-\alpha-j}\right)\\
\leq & \frac{8}{\pi^{2}}C_{3}L^{-1-\alpha}\frac{\alpha+k}{N}\sum_{j=0}^{\infty}\left(\frac{2(\alpha+k)}{N\pi}\right)^{j}\\
\leq & \frac{8}{\pi^{2}}C_{3}L^{-1-\alpha}\frac{\alpha+k}{N-\frac{2(\alpha+k)}{\pi}}\\
...
that
\begin{align*}
A_{2}(L,N)\leq & \sum_{j=1}^{J}a_{j}N^{\beta(-1-\alpha)-j}+\frac{2^{J+2}\left\Vert f^{(J+1)}\right\Vert _{\infty}}{J\pi^{J+1}}\frac{L^{J+1}}{N^{J}}.
\end{align*}
...
the Greeks. We define $v_{k}:=\int_{-M}^{M}v(x,S_{0})e_{k}(x)dx$
