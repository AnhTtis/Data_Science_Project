...
\noindent \textbf{Vision.} We extract two types of visual features, \ie, Swin Transformer \cite{liu2021swin} and GH-Feat \cite{xu2021generative}.
...
\begin{table*}[t]
    ...
    \toprule
    \rowcolor{Gray}
    Method & 1 & 2 & 4 & 6 & 7 & 10 & 12 & 15 & 23 & 24 & 25 & 26 & \textbf{Avg.} \\
    \midrule
    ...
    + Smooth + Threshold & 52.6 & 49.5 & 50.5 & 63.2 & 74.9 & 76.1 & 75.0 & 32.9 & 14.6 & 15.3 & 84.2 & 38.5 & 52.3 \\
    \bottomrule
    \end{tabular}}
    \label{tab:result}
\end{table*}
...
While our models are trained with respect to frame-level Action Unit labels, facial expressions should be temporally consistent. Therefore, we apply smoothing with a sliding window size of $16$ (frames) to the prediction logits produced by our trained model. We further adjust the positive thresholds with the logits produced by our model on our validation set ($10\%$ of the official train set) and apply the optimized thresholds on our test set (official validation set).
...
\subsection{Implementation and Training Details}
All methods are implemented in PyTorch \cite{paszke17}. Code and model weights are available, for the sake of reproducibility\footnote{https://github.com/intelligent-human-perception-laboratory/ABAW5}. We use a machine with two Intel(R) Xeon(R) Gold 5218 (2.30GHz) CPUs with eight NVIDIA Quadro RTX8000 GPUs for all the experiments.

The input image is first resized to $256\times256$. For data augmentation, each face is randomly cropped into $224\times224$ and randomly flipped in the horizontal direction. We train the model with the AdamW optimizer \cite{loshchilov2017decoupled} for 15 epochs with a batch size of 256. The learning rate is 1e-5. The weight decay is 1e-5. The gradient clipping is 1.0. 

Following Jiang \etal \cite{jiang2022facial}, the loss weights of the BCE loss $W_1$ and the multi-label loss $W_2$ are [1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5] and [1, 2, 1, 1, 1, 1, 1, 6, 6, 6, 1, 2] respectively.

The thresholds for the 12 AUs are [0.5, 0.55, 0.5, 0.4, 0.45, 0.45, 0.45, 0.5, 0.5, 0.55, 0.4, 0.5].

\subsection{Results and Discussions}
We show the experimental results in Table \ref{tab:result}. F1 score ($\uparrow$) is the evaluation metric. The results show that our method outperforms the baseline by more than 10\% in terms of the F1 score, showing the effectiveness of our method. In addition, we find that with label smoothing and threshold fine-tuning, our model's performance gets further improved. Especially for AU24 with threshold fine-tuning, the performance gets 7\% improvement.
