...
\title{Eigenvalue estimates for the fractional Laplacian on lattice subgraphs}
...
where $C_d = (2\pi)^2V_d^{-\frac{2}{d}}$. For any planar domain $\Omega$ which tiles $\mathbb{R}^2$, P\'{o}lya \cite{G1961ON} gave the lower bound estimate that
$$\lambda_k(\Omega) \geqslant C_d(\frac{k}{V(\Omega)})^{\frac{2}{d}}.$$
His proof actually works in any dimension $d\geqslant 2$, and he conjectured that this estimate holds for all bounded domains in $\mathbb{R}^d$. To solve this conjecture, Lieb \cite{1979The} obtained the result that
$$\lambda_k(\Omega) \geqslant D_d(\frac{k}{V(\Omega)})^{\frac{2}{d}}.$$
Here the constant $D_d < C_d$ which is proportional to $C_d$. Later in 1983, Li-Yau \cite{1983On} derived the sharp estimate that
$$\frac{1}{k}\sum_{i=1}^k \lambda_i(\Omega) \geqslant \frac{d}{d+2}C_d(\frac{k}{V(\Omega)})^{\frac{2}{d}},$$
called the Berezin-Li-Yau inequality, improving the constant $D_d$ to $\frac{d}{d+2}C_d$. The upper bound estimate was proved by Kr\"{o}ger \cite{1994Estimates} for large $k$ in 1994, which includes an extra term depending on the geometry of $\Omega$.

For the fractional Laplacian $(-\Delta)^s$, $0<s< 1$, we denote by $\lambda_{s,i}(\Omega)$ the $i$-th eigenvalue of $(-\Delta)^s$, and the Berezin-Li-Yau type inequality was generalized in \cite{2013ESTIMATES} as
$$\frac{1}{k}\sum_{i=1}^k \lambda_{s,i}(\Omega) \geqslant \frac{d}{d+2s}C_{d}^{s}(\frac{k}{V(\Omega)})^{\frac{2s}{d}}.$$

In recent years, people paid attention to the analysis on graphs. The graph Laplacian is studied in the literature, e.g. \cite{1990The,2000Graph,2013Harmonic,BL21}. There are some results for the fractional graph Laplacian, for example \cite{2022Optimal,2022Spectral,Benzi2020,21Decay}. However, the fractional Laplacian in these work is always defined on the whole graph, without reference to the Dirichlet eigenvalues of this operator. In this paper, for a locally finite, simple, undirected, connected graph $G$, we introduce the definition of the fractional Laplacian on a subgraph with Dirichlet boundary condition.

We recall the setting of graphs. A locally finite, simple, undirected and connected graph $G=(V,E)$ consists of the set of vertices $V$ and the set of edges $E$. We call two vertices $x, y$ neighbours, denoted by $x\sim y$, if there exists $e \in E$ connecting $x$ and $y$. A subgraph $G_1=(V_1, E_1)$ of $G$ means $V_1\subset V$ and $E_1\subset E$. In this paper we always consider locally finite, simple, undirected and connected graphs. We write $x\in G$ instead of $x\in V$ and denote by $|G|$ the number of vertices in $G$.
...
where $u:G \rightarrow \mathbb{C}$, and $e^{t\Delta}$ denotes the heat semigroup, see Definition~\ref{def:b}. Given a finite subgraph $G_1\subsetneq G$, we introduce the fractional Laplacian operator with Dirichlet boundary condition, denoted by $L^\alpha_{G_1}$, which is defined as
...
See Section~\ref{2} for more properties about this operator.
...
Consider the infinite d-dimensional integer lattice $\mathbb{Z}^d$ as a graph. We study the fractional Laplacian on a finite subgraph $\Omega$ of $\mathbb{Z}^d$ by the Fourier transform. In 2021, Bauer and Lippner \cite{BL21} studied the graph Laplacian eigenvalues with Dirichlet boundary condition on $\Omega$. For the average of the first k Dirichlet eigenvalues, they obtained the upper estimate in the spirit of Kr\"{o}ger \cite{1994Estimates} and the lower estimate in the spirit of Li-Yau \cite{1983On}. In this paper, we follow the proof strategies in \cite{1994Estimates,1983On,BL21} to derive the upper and lower bounds of Dirichlet eigenvalues for the fractional Laplacian.

For a finite subgraph $\Omega \subset \mathbb{Z}^d$, we define a boundary term
...
This could be regarded as the size of the boundary associated with $L^\alpha_{\Omega}$. Here $Q_\alpha (x,y) = \frac{1}{|\Gamma(-\frac{\alpha}{2})|}\int^{\infty}_0 t^{-1-\frac{\alpha}{2}}p(t,x,y)dt$, and $p(t,x,y)$ is the heat kernel on $\mathbb{Z}^d$, see Definition~\ref{def:b}. The following are main results of this paper.
...
The proof strategy is as follows. The upper bound estimate follows from Rayleigh-Ritz formula. Let $\{\phi_j\}_{1\leqslant j\leqslant |\Omega|}$ be the corresponding eigenfunctions, $V_k$ be the subspace of $\mathbb{C}^\Omega$ spanned by $\phi_1,\ldots,\phi_k$ and $P_k$ be the orthogonal projection to $V_k$. First by the Rayleigh quotient, we write $\lambda_{k+1}= \inf_{0\neq w\perp V_k}\frac{\langle w,L^\alpha_{\Omega}w\rangle _{\Omega}}{\langle w,w\rangle _{\Omega}}$. For any $g:\Omega\rightarrow\mathbb{C}$, set $g= P_k g+g-P_k g$ and we have Lemma~\ref{lm4}. With the Fourier transform, we choose $g(x)=h_z(x) = e^{i\langle x,z\rangle}$ and integrate in a measurable subset $B$ of $[-\pi,\pi]^d$ to prove Lemma~\ref{lm5}. Finally, we prove (a) by choosing proper set $B$. For the lower bound estimate, the key is to prove an integral inequality (Lemma~\ref{lm6}) which is an adaption of Li and Yau's method. To prove this lemma, we construct a radially symmetric function $\varphi(z):[-\pi,\pi]^d\rightarrow\mathbb{C}$ which satisfies $0\leqslant \varphi(z)\leqslant (\Phi(z))^{\frac{\alpha}{2}}$ and
...
Note that on $\Omega$, comparing to the Laplacian, we know the fractional Laplacian is a global operator, while the Laplacian is a local operator. Recall that
...
In next section, we introduce the fractional Laplacian on a locally finite connected graph $G$, and define the Dirichlet eigenvalue problem on a finite subgraph. In Section~\ref{sec:pro}, we prove Theorem~\ref{thm:main1}.
...
%Penny graphs are the contact graphs of unit circles [1, 2] ¡ª they are formed from
%non-overlapping sets of unit circles by creating a vertex for each circle and an edge
%for each tangency between two circles ¡ª and as such, fit into a long line of graph
...
We always consider stochastically complete graphs. With these properties, we give the definition of the fractional Laplacian.
...
and \ $\sum_{\substack{y\in G\\ y\neq x}}Q_\alpha(x,y)<+\infty$. For any $u \in H_\alpha(G):=\{u\in C(G)|\sum_{\substack{y\in G\\ y\neq x}} Q_\alpha(x,y)|u(y)|<+\infty\}$, the fractional Laplacian is well-defined.
\end{definition}

\begin{proof}
For any $u \in H_\alpha$, we have
\begin{equation*}
\begin{aligned}
(-\Delta)^{\frac{\alpha}{2}}u(x) &= \frac{1}{|\Gamma(-\frac{\alpha}{2})|}\int^{\infty}_0 (u(x)-e^{t\Delta}u(x))t^{-1-\frac{\alpha}{2}}dt\\
...
$$|\sum_{\substack{y\in G\\ y\neq x}}p(t,x,y)|=|1-p(t,x,x)|=|p(0,x,x)-p(t,x,x)| = |t\cdot \partial_t p(\xi,x,x)|\leqslant ct,$$
 we know $\sum_{\substack{y\in G\\ y\neq x}}Q_\alpha(x,y)<+\infty$. Since $u \in H_\alpha(\Omega)$, by above calculation one easily sees that $(-\Delta)^{\frac{\alpha}{2}}$ is well-defined.
\end{proof}
...
$L^\alpha_{G_1}$ becomes a finite dimensional operator, and then has $|G_1|$ eigenvalues. Note that for any $ u\in C(G_1)$, we have $u^*\in l^\infty(G)\subset H_\alpha(G)$. Further the quadratic form $\langle u,L^\alpha_{G_1} u\rangle _{G_1} = \langle u^*,(-\Delta)^{\frac{\alpha}{2}} u^{*}(x)\rangle _{G}$, with the natural Hermitian inner product, is well-defined. We give some properties as follows.
...
\item The eigenvalues are positive real numbers, labelling as
$$0<\lambda_1(G_1)< \lambda_2(G_1)\leqslant\ldots\leqslant\lambda_{|G_1|}(G_1).$$
...
$(b)$: The property (a) tells us that the operator $L^\alpha_{G_1}$ is self-adjoint and positive definite, which contains that the eigenvalues are positive and real numbers. To prove $\lambda_1$ is single, we suppose that there exist real eigenfunctions $\phi_1$ and $\varphi_1$ corresponding to $\lambda_1$, which satisfy $\phi_1 \neq c\varphi_1$. Select a constant $k$ to make $\phi_1 - k\varphi_1$ a sign changing eigenfunction, which contradicts the property (c).
...
The above argument derives that $\phi_1\geqslant 0$. We assume $G_1\supset M = \{x\in G_1:\phi_1(x)=0\}\neq\emptyset$. Let
...
Let $\mathbb{Z}^d$ be the infinite d-dimensional integer lattice, and we choose $S = \{e_1,\ldots,e_d,-e_1,\ldots,-e_d\}$ as the generating set. Here $e_i\in \mathbb{Z}^d$ is the vector whose i-th component is $1$ and the rest is $0$. Thus we can think $\mathbb{Z}^d$ as the Cayley graph generated by $S$. From Definition~\ref{def:b} and (3), we are clear about the definitions of the fractional Laplacian and the Dirichlet eigenvalue problem on $\mathbb{Z}^d$. In this subsection we introduce the Fourier transform on $\mathbb{Z}^d$, and derive some critical equalities and inequalities on a finite subgraph $\Omega$ of $\mathbb{Z}^d$.

For $z\in [-\pi,\pi]^d$, let us define $h_z(x):\mathbb{Z}^d\rightarrow \mathbb{C}$ by $ h_z(x)= e^{i\langle x,z\rangle}$, and $\langle x,z\rangle = \sum_{i=1}^d x_iz_i$. Thus the Fourier transform on $\mathbb{Z}^d$ is defined by
\begin{equation*}
\begin{aligned}
...
$$\widehat{u}(z) = (2\pi)^{-\frac{d}{2}}\sum_{x\in\mathbb{Z}^d} e^{-i\langle x,z\rangle}u(x)=(2\pi)^{-\frac{d}{2}}\langle u,h_z\rangle_{\mathbb{Z}^d},$$
$$u(x) = (2\pi)^{-\frac{d}{2}}\int_{[-\pi,\pi]^d}e^{i\langle x,\xi\rangle}\widehat{u}(\xi)d\xi.$$
...
To prove Theorem~\ref{thm:main1}, we introduce the function $\Phi (z) = \sum_{i=1}^d(2-2\cos z_i)$, $z\in[-\pi,\pi]^d$, and prove some relevant lemmas as follows.
...
\begin{proof}
By Definition~\ref{def:b}, we apply the Fourier transform operator to both sides of the equation (2) and get
...
For the heat equation (1), we also apply the operator $\mathscr{F}$ to both sides, and the equations are transformed into
\begin{equation*}
\left\{
\begin{aligned}
...
\right.
\end{equation*}
The solution is given as $\widehat{e^{t\Delta}u}(\xi)=\widehat{v}(t,\xi)=e^{-\Phi(\xi)t}\widehat{u}(\xi)$. Hence, by integrating by parts for $\Gamma(1-\frac{\alpha}{2})$, we derive that
$$\widehat{(-\Delta)^{\frac{\alpha}{2}} u}(\xi) = \widehat{u}(\xi)\cdot\frac{1}{|\Gamma(-\frac{\alpha}{2})|}\int^{\infty}_0 (1-e^{-\Phi(\xi)t})t^{-1-\frac{\alpha}{2}}dt
= (\Phi (\xi))^\frac{\alpha}{2}\widehat{u}(\xi).$$
...
Let $\Omega$ be a finite subgraph of $\mathbb{Z}^d$. For any $u\in C(\Omega)$, $0<\alpha< 2$, there holds
\begin{enumerate}[(a)]
...
$(b)$: From (4), let us replace v with u, then we have
\begin{equation*}
\begin{aligned}
...
Let $\Omega$ be a finite subgraph of $\mathbb{Z}^d$. For $0<\alpha< 2$, there holds
\begin{enumerate}[(a)]
...
Letting $\alpha\rightarrow0$ on $\mathbb{Z}^d$, we know $(-\Delta)^{\frac{\alpha}{2}}$ tends to the identity operator. We actually have
...
which is well-defined for $u,v\in l^1(\mathbb{Z}^d)$. There have been a lot of researches about the logarithmic Laplacian in the continuous case, and the above properties of the fractional Laplacian may provide some information on the logarithmic Laplacian in the discrete case.
\end{remark}
\section{Proof of Theorem~\ref{thm:main1}}\label{sec:pro}

Let $\Omega$ be a finite subgraph of $\mathbb{Z}^d$. The eigenvalues of the operator $L^\alpha_{\Omega}$ label as
$$0< \lambda_1<\ldots\leqslant\lambda_{|\Omega|},$$
and $\{\phi_j\}_{1\leqslant j\leqslant |\Omega|}$ are the corresponding standardized eigenfunctions. In this section, we follow the methods in \cite{1983On,BL21,1994Estimates} to derive the upper and lower bounds of the Dirichlet eigenvalues of the fractional Laplacian, proving Theorem~\ref{thm:main1}.
...
We start from a general lemma (proved in \cite{BL21}) of eigenvalues.
...
Let $\Omega$ be a finite subgraph of $\mathbb{Z}^d$. For any $1\leqslant k \leqslant |\Omega|$, $0<\alpha< 2$, and $B \subset [-\pi,\pi]^d$ measurable, we have
\begin{equation*}
\begin{aligned}
...
From Proposition~\ref{pro:c} we know the above choice satisfies the conditions of Lemma~\ref{lm4}. Let us integrate both sides of the inequality (5) and we have
\begin{equation*}
\begin{aligned}
...
\begin{equation*}
\begin{aligned}
\Rightarrow \int_{z\in B}(\Phi(z))^{\frac{\alpha}{2}}dz \leqslant \int_{z\in B}|z|^\alpha= dV_d\int^R_0 r^\alpha\cdot r^{d-1}dr = \frac{dV_d}{d+\alpha}R^{d+\alpha}.
\end{aligned}
\end{equation*}
Apply the above inequality into (6), and we obtain
\begin{equation*}
\begin{aligned}
...
Similarly, if $1\leqslant k\leqslant\min\{1,\frac{V_d}{2^{d+1}}\}|\Omega|$, we choose $R'=2^{\frac{1}{d}}R$ , which means $R' = 2\pi(\frac{2k}{V_d|\Omega|})^{\frac{1}{d}} \leqslant \pi$ and $|B| = \frac{2k(2\pi)^d}{|\Omega|}$. Then we repeat the above proof and ignore the effect of $\sum_{j=1}^k\lambda_j$, obtaining that
\begin{equation*}
\begin{aligned}
...
$$ \Rightarrow\lambda_{k+1} \leqslant (2\pi)^\alpha\frac{d\cdot2^{\frac{d+\alpha}{d}}}{d+\alpha}(\frac{k}{V_d|\Omega|})^{\frac{\alpha}{d}}+2\frac{|\partial^\alpha \Omega|}{|\Omega|},$$
which is what we want.
\end{proof}

\begin{remark}\label{rm1}
Note that letting $\alpha\rightarrow2$, the fractional Laplacian tends to the Laplacian, and the upper bound estimate in Theorem~\ref{thm2} also holds, which is the result in \cite{BL21}.
\end{remark}
\
...
$$(\frac{K}{MV_d})^{\frac{1}{d}}\leqslant a=2^{1-\frac{1}{\alpha}} <\pi.$$
...
$0 \leqslant \varphi(z)\leqslant(\Phi(z))^{\frac{\alpha}{2}}$. By the Taylor expansion, we observe that if $|z|\leqslant a=2^{1-\frac{1}{\alpha}} <\pi$, there holds
\begin{equation*}
\begin{aligned}
&\quad\quad(\Phi(z))^{\frac{\alpha}{2}}= (\sum_{i=1}^d(2-2\cos z_i))^{\frac{\alpha}{2}}\\
...
\end{aligned}
\end{equation*}
where the last step comes from $(1-x)^{\frac{\alpha}{2}}\geqslant 1-x^{\frac{\alpha}{2}}$ for any $0\leqslant x\leqslant1$. For $|z|\leqslant a $, one easily sees that
...
Step 2: With the function $\varphi(z)$, we can estimate the lower bound of $\int_{[-\pi,\pi]^d}(\Phi(z))^{\frac{\alpha}{2}}F(z)dz$. Since $F\geqslant 0$, we get
\begin{equation}
\begin{aligned}
\int_{[-\pi,\pi]^d}(\Phi(z))^{\frac{\alpha}{2}}F(z)dz \geqslant \int_{[-\pi,\pi]^d}\varphi(z)F(z)dz.
\end{aligned}
...
Choose the constant $R = (\frac{K}{MV_d})^{\frac{1}{d}}\leqslant a=2^{1-\frac{1}{\alpha}} <\pi$ to satisfy that $MR^dV_d = K$, and we define
\begin{equation*}
\widetilde{F}(z) = \left\{
\begin{aligned}
& M \ &,\ |z|\leqslant R,\\
...
\right.
\end{equation*}
$$\Rightarrow \int_{[-\pi,\pi]^d}\widetilde{F}(z)dz= \int_{[-\pi,\pi]^d}F(z)dz= K.$$
...
\begin{remark}
Similar to Remark~\ref{rm1}, letting $\alpha \rightarrow 2$, the fractional Laplacian tends to the Laplacian, and the above lower bound estimate also holds, which is the result proved in \cite{BL21}.
\end{remark}
