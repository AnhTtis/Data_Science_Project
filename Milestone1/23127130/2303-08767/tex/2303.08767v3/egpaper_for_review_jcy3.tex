...
\begin{figure*}[!t]
    ...
    The source text is ``a standing dog' and the target text is ``a sitting dog". Cross Attention maps (a) conditioned with ${\eb}_{src}$ (b) conditioned with $[{\eb}_{src}', {\eb}_{hper}]$, (c) conditioned with $[{\eb}_{tgt}', {\eb}_{hper}]$. (d) Cross attention maps by Imagic \cite{kawar2022imagic} with Stable Diffusion.}
    \label{fig_attention}
\end{figure*}
...
\begin{figure*}[!t]
    \centerline{\includegraphics[width=1.0  \textwidth]{./Figures/fig_comparison.jpg}}
    \caption{The qualitative comparison results. Compared with three stable diffusion-based text-guided image manipulation methods, our method shows its superiority. It could preserve the identities of the subject in source images, while appropriately transforming the semantic information to align with the CLIP embedding of the target text.
    Original Imagic results with Imagen show comparable results by using
    proprietary text embedding scheme.}
    \vspace{0.5cm}
...
We compared our model with three diffusion model based image manipulation baselines, i.e. Imagic~\cite{kawar2022imagic}, DreamBooth~\cite{ruiz2022dreambooth}, and Textual Inversion~\cite{gal2022image}. Although DreamBooth and Textual Inversion require three to five images for optimal training, we trained them with a single image because of their unavailability. We compared our method with these three methods which were implemented based on Stable Diffusion~\cite{rombach2022high}. Additionally, the Imagic authors provided us with the official Imagic results that were obtained using Imagen, as a model that is not publicly available. We also included them for comparison. The qualitative results are illustrated in Figure \ref{fig_comparison}. 

It is noticed from the results that the implementation of Imagic using the Stable Diffusion model exhibits inferior content preservation capabilities when compared to the official Imagic result and the other results. Although Imagic was proposed to edit semantic information while preserving the other overall shapes, it showed inferior performance with Stable Diffusion model. In the first row, the bird's head is different from the source image and the dog in the second row has lots of hair around its head in the generated image which is different from the source image. It is due to interpolation in the inference step, which approximates between the target and the optimized embedding. The Imagen model-based implementation of Imagic yields significantly improved image manipulation results, preserving the original source image while making targeted edits based on specified textual inputs. However, due to the Imagic model's strong tendency to preserve the object's position and background of the source image, it results in poor performance when the source image undergoes significant changes due to target textual inputs. As shown in the last row of the figure, when the white bear in source image undergoes substantial transformations due to textual inputs, the model fails to preserve its identity. 
...
In Figure~\ref{fig_attention}(d), the attention map of Imagic with Stable Diffusion is shown, revealing no discernible organized attention pattern.
...
\begin{tabular}{c|p{1.5cm}p{1.5cm}|p{2cm}p{2cm}|p{2cm}}
    ...
    \rule{0pt}{1\normalbaselineskip}
    Imagic w/ Stable Diffusion             &\hfil \underline{3.731}   &\hfil 3.251                    &\hfil \bf{0.2148} &\hfil  0.8223    &\hfil 14.08 min\\
...
\caption{Quantitative results on comparative studies. The bold text denotes the best score, and the underline indicates the second-best score. All of the models were implemented using the Stable Diffusion model.}
...
\begin{figure*}[!t]
    %\vspace{1.3cm}
    \centerline{\includegraphics[width=1.0 \textwidth]{./Figures/sup_control.jpg}}
...
\begin{figure*}[!t]
    \centerline{\includegraphics[width=1.0\textwidth]{./Figures/sup_control_rabbit.jpg}}
...
For further comparison, we generated comparative results for additional result images (Figure \ref{sup_control} and \ref{rabbit_control}). From the manipulated images, which are obtained with the Stable Diffusion, it is confirmed that our model produces better results than other models in maintaining the appearance of objects in the source image and reflecting the given text in the resulting image.
...
We would like to express our gratitude to Shiran Zada and the other authors from Imagic for their help with the official Imagic results.
