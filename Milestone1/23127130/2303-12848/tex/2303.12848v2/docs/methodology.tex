...
In this section, we define the problem and present the details of each component in the pipeline of \sys, including test-time reconstruction and detection of attacks.
...
During the inference time, an adversarial input $u_{\rm adv}$ is created by adding an adversarial perturbation $\delta_{\rm adv}$ to a clean sample $u\in\mathcal{Z}$, resulting in a classifier $C$ output $\hat{y} = C(u_{\rm adv})$ that does not match the ground truth label $y$. 
...
To mitigate these adversarial attacks, we propose using a reverse vector $\delta_{\rm rev}$ to minimize the difference between the reconstructed image and the original input, effectively generating an adversarial attack for correction. Our goal is to find an optimal reverse vector $\delta_{\rm rev}$ with a small $\epsilon$ range that can minimize the reconstruction loss of MAE and restore the correct prediction, such that $C(u)=y = C(u + \delta_{\rm adv}+ \delta_{\rm rev})$. In our paper, We aim to (1) detect the occurrence of adversarial attacks for $u \in \mathcal{Z}$, (2) find a reverse vector $\delta_{\rm rev}$ with a small $\epsilon$ range to restore the prediction as $(h_\psi \circ f_\theta)(u)=y = (h_\psi \circ f_\theta)(u + \delta_{\rm adv}+ \delta_{\rm rev}) $.
...
Self-supervised learning (SSL) help capture the natural inputs manifold, yet standard self-supervision pretext task such as rotation prediction, re-colorization, and contrastive learning are not sensitive enough to adversarial attacks. The MAE is one of the strongest self-supervision learning tasks, which can reconstruct the randomly masked-out patches of input by minimizing the reconstruction loss of predicted patches. 
...
\subsection{Test-time Adversarial Detection}
After obtaining a well-trained MAE ($f_{\theta}, g_{\phi}$), the detection of anomalous adversarial attacks is straightforward by using the reconstruction loss $\mathcal{L}_{ mse}(\cdot)$. The rationale lies in our training for $f_{\theta}$ and $ g_{\phi}$ being on the clean sample sets $\mathcal{S}$, which can be viewed as the manifold learning on in-distribution data. Thus, the magnitude of reconstruction loss enlarges while encountering adversarial samples $u$ from $\mathcal{Z}$, thus triggering the detection.  
...
Here, we utilize reconstruction loss $\mathcal{L}_{ mse}$ with a statistic-based method for detection in \sys, the Kolmogorov-Smirnov test (KS test)~\cite{kstest}.
We hypothesize that loss values associated with non-attacked or "clean" images will be distinctly lower on average compared to those associated with attached images. Also, the sampled distributions of the loss values form the histogram in Figure~\ref{fig:mse_loss_dist}. We can delineate the difference in distribution between non-attacked and attacked images. We find that combining SSL loss with the KS test for adversarial detection is better than other detection methods that calculate the distance metric for logit outputs. In Section~\ref{subsec:detection_result}, we  illustrate the details of other baselines and demonstrate the detection results.
...
\subsection{Reconstruction for Adversarial Attacks}

The second part of the \sys method involves repairing adversarial attack inputs from malicious to benign. Unlike the existing method, the reconstruction is fully unsupervised and can be done parallel to detection. 
...
To repair the potential adversarial samples $u_{adv}$, we inject a reverse vector $\delta_{rev}$ into $u_{adv}$ such that $\hat{u} = u_{adv} + \delta_{rev}$ minimizes the reconstruction loss $\mathcal{L}_{mse}$. We initialize the $\delta_{rev}$ under the uniform distribution following a specific $\epsilon$ range. The $\delta_{rev}$ can be either a random gaussian noise or crafted perturbation. After optimization, the reconstructed sample $\hat{u} = u_{adv} +\delta_{rev}$ is fed to the classifier $C$ for prediction, yielding classification scores $\hat{y} = C(\hat{u})$.
...
\begin{align}
    ...
    \vspace{-4mm}
\end{align} 
We aim to find an optimal vector $\delta_{rev}$ for generating a reconstructed sample $\hat{u} = u_{adv} + \delta_{rev}$ such that the gap between masked patches $(1-b) \odot \hat{u}$ and predicted patches $(g_{\phi} \circ f_{\theta})(b \odot \hat{u})$ of the reconstructed sample can be reduced. Specifically, we update $\delta_{rev}$ using projected gradient descent, subject to $\ell_{\infty}$ with a specific $\epsilon$ range. The update of $\delta_{rev}$ is described in Eq.~\ref{eqn:deltaupdate}. 
\begin{align}
...
%\nonumber
\end{align}
Here, $t$ is the timestamp, $\alpha$ is the step size, and $\epsilon$ is the perturbation range. The update of $\delta_{rev}$ is performed by taking the gradient of the reconstruction loss $\mathcal{L}_{mse}$ with respect to the input and clipping the update of $\delta_{rev}$ to the $\epsilon$ range. 

\iffalse

,where the $\delta_{rev}$ is updated via using projected gradient descent, subjected to $\ell_{\infty}$ with a specific $\epsilon$ range. To be more specific, we aim to find an optimal vector $\delta_{rev}$ for generating reconstructed sample $\hat{u} = u_{adv} + \delta_{rev}$, such that the gap between masked patches $(1-b) \odot \hat{u}$  and predicted patches $(g_{\phi} \circ f_{\theta})(b \odot \hat{u})$ of the reconstructed can be reduced. The update of $\delta$ is as Eq.~\ref{eqn:deltaupdate}:
...
\subsection{Defend against Adaptive Attack}
\label{sec:dda_method}
We show that \sys is able to defend against the adaptive attack by assuming the attackers have knowledge about our defense method, including the MAE architecture, adversary detection, and reconstruction. To launch an adaptive attack that can fool both MAE and Target classifier, an attacker can  generate adversarial examples by jointly optimizing reconstruction loss for MAE and cross-entropy loss for the classifier. We change the optimization problem of $\delta$ as the following formulation~\ref{eqn:optim_da} for the adaptive attack. 
\begin{multline}
    \label{eqn:optim_da}
    \text{max } 
...
\begin{align}
    ...
    \vspace{-6mm}
\end{align}
where the $\lambda$ is the parameter to control the adversarial budget. If the $\lambda$ increases, the optimization gradually shifts the budget for attacking the classification loss $\mathcal{L}_{ce}$ to the reconstruction loss $\mathcal{L}_{mse}$. By optimizing with $\mathcal{L}_{da}$, the optimal $\delta$ can be found to generate the defense-aware adversarial samples. We empirically found the trade-off between $\mathcal{L}_{mse}$ and $\mathcal{L}_{ce}$, where the joint optimization on  these two objective functions leads to a degradation of the attack success rate for the defense-aware adversarial samples. Therefore, our defense method is robust against adaptive attacks.
