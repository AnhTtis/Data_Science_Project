...
\end{tabular}
\caption{The reconstruction results on eight kinds of attack. We compare \sys with the other two SSL baselines, including contrastive learning task (SimCLR~\cite{simclr}) and rotation prediction~\cite{gidaris2018unsupervised}. We show the robust accuracy (\%) on two Resnet50~\cite{he2016deep} models pre-trained with Standard training and $\ell_\infty$ robust training (MADRY~\cite{madry2017towards}). The numbers in bold represent the best performance.}
\label{tab:reconstruction}
\end{table*}

\paragraph{Dataset Detail}
We train and test \sys by using the ImageNet dataset~\cite{deng2009imagenet}, which is a 1K-class large-scale vision dataset. Notice that the training process of \sys only uses the original ImageNet train set with $\sim$1M images.
To evaluate \sys, we generate the adversarial samples on the ImageNet test set~\cite{deng2009imagenet} with 50K images. Here, we consider 8 kinds of attacks and generate them from two kinds of models, the  Standard ~\cite{he2016deep} and Robust~\cite{madry2017towards} ResNet50. 
...
\paragraph{Baseline} We compare the detection part of \sys with the following four kinds of baselines:
...
\vspace{-4mm}
         \caption{Histogram plot of self-supervised losses $\mathcal{L}_{mse}$ for eight adversarial attacks. We show the loss distribution of original images with the color blue, adversarial images as red, and reconstructed images as green. The figure shows that adversarial images cause a large shift in reconstruction loss (from blue to red). After using \sys to reconstruct, the loss distribution of reconstructed images shifts back and gets closer to the original images (red to yellow).}
...
Table~\ref{tab:detection_result} shows the detection rate of our method against eight kinds of adversarial attacks on ImageNet data. Our method combines the $\mathcal{L}_{mse}$ loss with a two-sample KS test and set the p-value threshold based on a fixed FPR rate of 0.2. Here, we evaluate the detectors for every baseline using the same FPR 0.2.
Compared with feature squeezing~\cite{xu2017feature}, noise-based detection~\cite{hu2019new}, targeted detection~\cite{hu2019new}, and two SSL tasks, our detection rate hovers around 82\%, which outperforms all other baselines.  
...
We hypothesize that the KS-test with MAE loss $\mathcal{L}_{mse}$ outperforms other baseline detection methods because the MAE loss is more sensitive to variance in distributions, compared to rotation prediction and contrastive learning, Moreover, the two-sample KS-test has the additional benefit of being able to better control the TPR-FPR tradeoff compared to most other baselines by varying the p-value threshold for class prediction. Tuning this threshold parameter is important for adversarial detection since the cost of false positives is larger than false negatives, given the context of the detection problem.


\begin{figure}[t]
\centering
\includegraphics[scale=0.52]{figs/daa_robust_acc.png}
\vspace{-3mm}
...
\subsection{Adversarial Reconstruction Results}
We compare our reconstruction method with two kinds of SSL baselines: (1) Contrastive learning learns the representations that map the feature of the transformations of the same image into a nearby place. Here, we follow the SimCLR~\cite{simclr} to implement the contrastive learning task. We create the positive/negative pairs for images by applying some transformations such as random cropping/flipping, random rotation, ...etc. (2) The rotation prediction~\cite{gidaris2018unsupervised} learns to predict the rotation degree of upcoming input samples. Here, we train an auxiliary 4 classes ($0^{\circ}$, $90^{\circ}$,$180^{\circ}$, $270^{\circ}$) rotation prediction task as one of our SSL baselines.
...
As Section~\ref{sec:method} mentioned, we optimize the perturbations for a given input by using the PGD algorithm based on the reconstruction loss $\mathcal{L}_{\rm mse}$ extracted from MAE. As Table~\ref{tab:reconstruction} shows, we evaluate the robust accuracy on those target classifiers $\mathcal{C}$ which our adversarial samples generated from. \sys successfully repairs the adversarial images and improves the robust accuracy without knowing the ground truth labels during the inference time. The target classifiers are pre-trained with two different methods, including standard training and robust training.
...
Compared with two SSL tasks, \sys improves the RA after reconstruction by $6\%\sim40\%$ for those attack samples generated from Standard ResNet50 model. However, contrastive learning (SimCLR) and rotation prediction only improve by 1\%$\sim$7\%. For the Robust ResNet50 model, \sys improves the RA by 1\%$\sim$8\% for every attack, outperforming other baselines.
...
As Section~\ref{sec:dda_method} mentioned, we launch the adaptive attack on models and demonstrate the effectiveness of \sys on this stronger attack which assumes the attacker knows our defense method and the MAE model. Figure~\ref{fig:daa_robust_acc} shows the result of the defense-aware attack on two target classifiers, including Standard ResNet50 and Robust ResNet50~\cite{madry2017towards}. While increasing the self-supervised attack strength $\lambda$ for DAA from 0 to 6, the attacks are gradually weakened, and the robust accuracy increases. If the adversary attempts to attack the MAE, the attack will be less effective on the backbone classifiers. On the other hand, if the adversary attempts to generate samples with high attack success rates, it is difficult to attack both the target classifier and our MAE. While the dotted line shown in Figure~\ref{fig:daa_robust_acc} indicates the result of DAA, the solid line shows the reconstruction result of \sys on top of DAA, which indicates that our method has optimal results when $\lambda$ is set as 0 for both the standard and robust model. 

\vspace{-2mm}

\subsection{Loss Analysis on Attack Detection}
\label{ablation:loss}
To better understand the ability of reconstruction for MAE, we visualize the histogram on the self-supervised loss distribution for adversarial examples.  We compare the loss variance between clean samples and different kinds of attack samples by collecting the loss on every batch during the testing-time inference. As Figure~\ref{fig:mse_loss_dist} shows, the loss variance on clean samples (colored with blue region) and the other eight attack types' samples (red curve) are large before reconstruction. The orange curve shows the loss distribution of samples after reconstruction. Here, \sys uses MSE loss to reconstruct the samples and demonstrates a huge loss shifting after reconstruction (colored with yellow region), which is closer to the loss distribution of clean samples. As the result, this inspires us to leverage the MAE loss to detect the adversarial sample with KS-test.
