...
%\topmargin=-1cm
\textwidth=15cm \textheight=21.6cm \oddsidemargin 0.46cm
...
K_t(y_1,\ldots ,y_m):=(1/t^{mn})K(y_1/t,\ldots ,y_m/t)$, for any $ t\in 
...
\begin{align*}
\left(\frac{1}{|Q|}\int_Q\omega(y)\right)^{\frac{1}{p}}
\left(\frac{1}{|Q|}\int_Q\omega(y)^{\frac{-1}{p-1}}\right)^
{\frac{1}{p'}}\leq C.
\end {align*}
When $ p=1 $, $\omega \in A_1$ if and only if there exists a positive constant 
$C$ such that
\begin{align*}
\left(\frac{1}{|Q|}\int_Q\omega(y)\right)\leq C\mathop{\inf}\limits_{x\in 
{\rm Q}}\omega(x).
\end {align*}
Via the classical Muckenhoupt's class weight functions, 
numrous authors have stuied the extrapolation of $ A_p$. In 2012, Tang 
...
\begin{align*}
\left(\frac{1}{\varphi(Q)^{\theta}|Q|}\int_Q\omega(y)\right)^{\frac{1}{p}}
\left(\frac{1}{\varphi(Q)^{\theta}|Q|}\int_Q\omega(y)^{\frac{-1}{p-1}}\right)^
{\frac{1}{p'}}\leq C,
\end {align*}
for all cubes $Q$. Especially, when $p=1$, we obtain
\begin{align*}
\left(\frac{1}{\varphi(Q)^{\theta}|Q|}\int_Q\omega(y)\right)\leq 
C\mathop{\inf}\limits_{x\in {\rm Q}}\omega(x),
\end {align*}
where $Q(x,r)$ is the cube centered at $x$ with the sidelength $r$,  
$\varphi(Q)^{\theta}:=(1+r)^{\theta}$, $|E|$ denotes the 
...
strictly large than the class $A_p$ for all $1\leq p<\infty$. Moreover, taking  
for instance,  the weight $ \omega \in A_{p}^{\theta } (\varphi ) $, for $ 
\omega (x):=1+|x|^2 $, $ p> 1$, and $0\le \theta \le 2n $, but $ \omega \notin 
...
Q}\left(\frac{1}{|Q|}\int_Qv_{\vec{\omega}}\right)^{\frac1p}
\prod_{j=1}^{m}\left(\frac{1}{|Q|}\int_Q\omega_j(x)^{1-p'_j}\right)^{\frac{1}{p'_j}}<\infty,
\end {align*}
...
\omega_j(x)^{1-p'_j}\right)^{\frac{1}{p'_j}}$ is considered to be 
...
$ functions, where the weights are $ A_{\vec p}^{\infty } (\varphi ) $.

We say that $\vec\omega\in A_{\vec{p}}^{\theta}(\varphi)$, if and only if
\begin{align*}
\mathop{\sup}\limits_{\rm Q} 
\left(\frac{1}{\varphi(Q)^{\theta}|Q|}\int_Qv_{\vec{\omega}}\right)^{\frac1p}
\prod_{j=1}^{m}\left(\frac{1}{\varphi(Q)^{\theta}|Q|}\int_Q\omega_j(x)^{1-p'_j}\right)^{\frac{1}{p'_j}}<\infty,
\nonumber
\end{align*}
...
\omega_j(x)^{1-p'_j}\right)^{\frac{1}{p'_j}}$ is considered to be 
$\left(\mathop{\inf}\limits_{x\in {\rm Q}}\omega_j(x)\right)^{-1}$. 
For some other studies of $A_{\vec p}^{\infty}(\varphi)$, one can see, for 
instance, \cite{b38,b39}.  
\begin{remark}
...
on the weighted Morry spaces is established, where the new Morry space is 
acquired for the new multilinear square operator with generalized kernel. The 
boundedness of multilinear commutators and multilinear iterative commutators 
generated by new class of multilinear square operators and new $ BMO $  
functions on the weighted Lebesgue spaces and weighted Morrey spaces  
will be presented in a forthcoming article.


\section{New weighted norm inequalities for new multilinear square operators 
with generalized kernels on Lebesgue spaces}\label{sec2}
...
\begin{definition}
Let $0<\eta<\infty$, the maximal operator $M_{\varphi,\eta}$ is defined by
\begin{align*}
M_{\varphi,\eta}f(x):=\mathop{\sup}\limits_{ x\in {\rm 
Q}}\frac{1}{\varphi(Q)^{\eta}|Q|}\int_Q|f(y)|dy.
\end{align*}
Let $ Q $ be a dyadic cube, the dyadic maximal function 
$M_{\varphi,\eta}^\triangle$ is defined by 
\begin{align*}
M_{\varphi,\eta}^\triangle f(x):=\mathop{\sup}\limits_{ x\in {\rm 
Q}}\frac{1}{\varphi(Q)^{\eta}|Q|}\int_Q|f(y)|dy.
\end{align*}
...
&:=\mathop{\sup}\limits_{x\in {\rm 
Q},r<1}\frac{1}{|Q|}\int_{Q}|f(y)-f_Q|dy+\mathop{\sup}\limits_{x\in 
{\rm Q},r\geq1}\frac{1}{\varphi(Q)^{\eta}|Q|}\int_{Q}|f(y)|dy\\
&\simeq\mathop{\sup}\limits_{x\in {\rm 
Q},r<1}\mathop{\inf}\limits_{\rm 
c}\frac{1}{|Q|}\int_{Q}|f(y)-c|dy+\mathop{\sup}\limits_{x\in {\rm 
Q},r\geq1}\frac{1}{\varphi(Q)^{\eta}|Q|}\int_{Q}|f(y)|dy,
\end{align*}
where $f_{Q} :=\frac{1}{\left | Q \right | } \displaystyle \int_Qf(y)dy$. 
\end{definition}
...
$Q$,
\begin{align*}
\left(\frac{1}{|Q|}\int_Q\omega^{1+\delta}(x)\right)^{\frac{1}{1+\delta}}\leq
C\left(\frac{1}{|Q|}\int_Q\omega(x)\right)(1+r)^l.
\end{align*}
\end{itemize}
\end{lemma}
...
then
\begin{align*}
\frac{1}{\varphi(Q)^{\theta}|Q|}\int_Q |f(x)| \leq 
C\left(\frac{1}{\omega(5Q)}\int_Q|f(x)|^p\omega(x)\right)^{\frac1p}.
\end{align*}
\end{itemize}
...
\Vert_{L^p(\omega)}\leq\Vert M_{\varphi,\eta}^{\sharp, \triangle} f 
\Vert_{L^p(\omega)}.
\end{align*}
\end {lemma}
...
\begin{align*}
\mathcal{M}_{\varphi,\eta}(\vec{f})(x):=\mathop{\sup}\limits_{x\in {\rm 
Q}}\prod_{j=1}^m\frac1{\varphi(Q)^\eta \left | Q \right | } \int_Q 
|f_j(y_j)| dy_j
\end{align*}
and
\begin{align*}
...
^{\frac{1}{\delta } }
=\left(\mathop{\sup}\limits_{x\in 
{\rm Q}}\prod_{j=1}^m\frac1{\varphi(Q)^\eta \left | Q \right |} \int_Q 
...
multilinear square operators with generalized kernels. The main theorem of this 
...
For $0<\delta<1/m$, $0<\eta<\infty$ and all bounded measurable functions $\vec 
f=(f_1,\ldots,f_m)$ with compact support, there exists a constant $C>0$ 
such that 
...
\noindent{\it \rm \textbf{Case 1}:} $r<1$. Since $0<\delta<1/m< 1$, $\eta>0$ 
...
$. This yields
\begin {align*}
\prod_{j=1}^mf_j(y_j)&=\sum_{\alpha_1,\ldots,\alpha_m\in\{0,\infty\}}f_1^{\alpha_1}(y_1)\cdots
 f_m^{\alpha_m}(y_m)\\
...
Let $A:=\sum_{(\alpha_1,\ldots,\alpha_m)\in 
\mathscr{L}}A_{\alpha_1,\ldots,\alpha_m}$, we conclude that
\begin  {align*}
...
\mathscr{L}}\left(\frac1{|Q|}\int_Q|T(f_1^{\alpha_1},\ldots,f_m^{\alpha_m})(z)-
A_{\alpha_1,\ldots,\alpha_m}|^\delta dz\right)^{\frac1\delta}\\
&:=\uppercase\expandafter{\romannumeral+1}+\uppercase\expandafter{\romannumeral+2}.
\end {align*}

As $T:L^{s_1}\times\cdots\times L^{s_m}\to L^{s,\infty}$, using Lemma 
...
for $z_0\in 4Q\setminus 3Q$ and $\Delta_k:=Q(z_0, 2^{k}\sqrt{mn}|z-z_0|)$, $  
z\in Q $, $k\in \mathbb{N}_+$. Note that  $\Delta_2 \subset Q^*$, so 
$(\mathbb{R}^n)^m\setminus 
(Q^*)^m\subset (\mathbb{R}^n)^m\setminus (\Delta_2)^m$. When $z\in Q$, $z_0\in 
4Q\setminus 3Q$, we obtain that $|z-z_0|\sim r$. Because each term of 
$\mathscr{L}$ contains at least one $\alpha_j=\infty, j=1, \ldots,m$, we choose 
one of them for discussion. Taking $N\geq m\eta/q'$, by H\"{o}lder's inequality 
...
,\vec{y}\right)\right| \cdot \prod_{j=1}^{m} 
...
%\left|f_{j}\left(y_{j}\right)\right| d \vec{y} \right ) ^{2}  \frac{d 
%	t}{t}\right)^{\frac{1}{2}} d z\\
&\leq \frac{1}{|Q|} \int_{Q} \sum_{k=1}^{\infty }\left(\int_{0}^{\infty}  \left 
( \int_{\left(\Delta_{k+2}\right)^{m} 
	\backslash\left(\Delta_{k+1}\right)^{m}}\left|K_{t}(z, 
\vec{y})-K_{t}\left(z_{0} ,\vec{y}\right)\right| \cdot \prod_{j=1}^{m} 
\left|f_{j}\left(y_{j}\right)\right| d \vec{y} \right ) ^{2}  \frac{d 
...
\backslash\left(\Delta_{k+1}\right)^{m}} \prod_{j=1}^{m} 
...
\prod_{j=1}^m\left(\frac{1}{\varphi(\Delta_{k+2})^{\eta}|\Delta_{k+2}|}
\int_{\Delta_{k+2}}|f_j(y_j)|^{q'}
dy_j\right)^{\frac 1{q'}}dz\\
&\leq C\frac{1}{\left | Q \right | } \int_Q \sum_{k=1}^\infty C_{k} \cdot 
(1+2^{k}r)^{\frac{m\eta }{q'}- N } dz \cdot  \mathcal{M}_{q',\varphi,\eta}(\vec 
f)(x)\\
&\leq C\mathcal{M}_{q',\varphi,\eta}(\vec f)(x).
\end{align*}
...
\noindent{\it \rm \textbf{Case 2}:} $r\geq1$. Since $0<\delta<1/m< 1$ and 
$\eta>0$, we obtain
\begin {align*}
&\left(\frac1{\varphi(Q)^\eta|Q|}\int_Q |T(\vec f)(z)|^\delta 
dz\right)^{\frac1\delta}\\
&\leq C\frac{1}{\varphi(Q)^{\eta/\delta}}\left(\frac1{|Q|}\int_Q 
|T(f_1^0,\ldots,f_m^0)(z)|^\delta dz\right)^{\frac1\delta}\\
&\ \ \ \ +C\frac{1}{\varphi(Q)^{\eta/\delta}}\sum_{\alpha_1,\ldots,\alpha_m\in 
\mathscr{L}}\left(\frac1{|Q|}\int_Q|T(f_1^{\alpha_1},\ldots,f_m^{\alpha_m})(z)|^\delta
 dz\right)^{\frac1\delta}\\
&:=\uppercase\expandafter{\romannumeral+3}+\uppercase\expandafter{\romannumeral+4}.
\end {align*}

According to $T:L^{s_1}\times\cdots\times L^{s_m}\to L^{s,\infty}$, using Lemma 
\ref{Lemma2.8} and H\"{o}lder's inequality, we deduce that
...
\setlength\belowdisplayskip{10pt}
\begin {align*}
\uppercase\expandafter{\romannumeral+4}_{\alpha_1,\ldots,\alpha_m}&:=\frac{1}
{\varphi(Q)^{\eta/\delta}}\left(\frac1{|Q|}\int_Q|T(f_1^{\alpha_1},\ldots,f_m^
{\alpha_m})(z)|^\delta dz\right)^{\frac1\delta}\\
&\leq \frac 1{|Q|}\int_Q|T(f_1^{\alpha_1},\ldots,f_m^{\alpha_m})(z)|dz\\
...
For $z\in Q$ and $(y_1,\ldots,y_m)\in (\Delta_{k+2})^m \setminus 
(\Delta_{k+1})^m$, we obtain $ {\textstyle \sum_{j=1}^{m}} 
|z-y_j|\geq2^{k-1}|z-z_0|$. Since $z\in Q, z_0\in 4Q\setminus 3Q$, we can get 
that $|z-z_0|\sim r$. Taking $N >m\eta/q'$, by Minkowski's inequality 
...
\backslash\left(\Delta_2\right)^{m}}\left(\int_{0}^{\infty} \left |  
K_{t}(z, \vec{y})\right|^{2} \frac{d 	t}{t}\right)^{\frac{1}{2} }\cdot 
...
%\vec{y} d z \\
&\leq \frac{1}{|Q|} \int_{Q} \sum_{k=1}^{\infty } 
\int_{(\Delta_{k+2})^m\setminus (\Delta_{k+1})^m}\frac{C}{\left ( 
\sum_{j=1}^{m} \left | z- y_{j}  \right |  \right ) ^{mn}\cdot \left ( 1+  
\sum_{j=1}^{m} \left | z- y_{j}  \right |  \right ) ^{N} }  \\
&\ \ \ \ \times \prod_{j=1}^{m} \left|f_{j}\left(y_{j}\right)\right| d 
\vec{y} dz \\
&\leq C \frac1{|Q|}\int_Q \sum_{k=1}^\infty 
(2^{k-1}|z-z_0|)^{-mn}(1+2^{k-1}|z-z_0|)^{-N}\\
&\ \ \ \ \times \left(\int_{(\Delta_{k+2})^m}\prod_{j=1}^m|f_j(y_j)|d\vec y 
\right) dz \\
&\leq C \frac1{|Q|}\int_Q \sum_{k=1}^\infty 
\frac{|\Delta_{k+2}|^m}{(2^kr)^{mn}(1+2^kr)^{N}}\prod_{j=1}^m\left(\frac{1}
{|\Delta_{k+2}|}\int_{\Delta_{k+2}}|f_j(y_j)|^{q'}dy_j\right)^{\frac1{q'}}dz\\
&\leq C \frac1{|Q|}\int_Q \sum_{k=1}^\infty 
\frac{|\Delta_{k+2}|^{m}\varphi(\Delta_{k+2})^{m\eta/q'}}{(2^kr)^{mn}
(1+2^kr)^N}
\prod_{j=1}^m\left(\frac{1}{\varphi(\Delta_{k+2})^{\eta}|\Delta_{k+2}|}
\int_{\Delta_{k+2}}|f_j(y_j)|^{q'}dy_j\right)^{\frac 1{q'}} dz\\
&\leq C \frac1{|Q|}\int_Q \sum_{k=1}^\infty (1+2^kr)^{\frac{m\eta }{q'} - N} 
\prod_{j=1}^m\left(\frac{1}{\varphi(\Delta_{k+2})^{\eta}|\Delta_{k+2}|}
\int_{\Delta_{k+2}}|f_j(y_j)|^{q'}dy_j\right)^{\frac 1{q'}} dz\\
&\leq C\mathcal{M}_{q',\varphi,\eta}(\vec f)(x).
\end{align*}
...
operator with generalized kernel on the weighted Lebesgue spaces. Choose a $ 
\delta  $ such that $ 0< \delta < 1/m $, and $ \eta =\eta _0 $ in Lemma 
\ref{Lemma2.6} for $ \vec \omega \in A_{\vec p/q'}^\infty(\varphi ) $. It 
...
&\leq \| M_{\delta,\varphi,\eta _0}^{\sharp, \triangle}(T(\vec f)) 
\|_{L^p(v_{\vec\omega})}
...
\le \left \| \mathcal{M}_{\varphi ,\theta _0}^{\bigtriangleup} \left ( \left | 
T( \vec f  ) \right | ^\delta \right )   \right \|_{L^{\frac{p}{\delta } 
,\infty } \left ( \upsilon _{\vec 
\omega }  \right ) }^{\frac{1}{\delta } } \\
&=\sup_{\lambda > 0}\left [\lambda \left |\nu _{\vec\omega }\left \{ x\in 
\mathbb{R}^n: \mathcal{M}_{\varphi ,\theta _0}^{\bigtriangleup} \left ( \left 
|  T ( \vec f  ) ( x  ) \right |^\delta \right )  >\lambda   \right \} \right 
|^\frac{\delta }{p}\right ]^\frac{1}{\delta }\\
&=\sup_{\lambda > 0}\lambda \left |\nu _{\vec\omega }\left 
\{ x\in \mathbb{R}^n: \mathcal{M}_{\varphi ,\theta _0}^{\bigtriangleup}  \left 
( \left |T ( \vec f  ) ( x  ) \right |^\delta  \right ) >\lambda ^\delta    
\right \} \right |^\frac{1 }{p} \\
&=\sup_{\lambda > 0}\lambda \left |\nu _{\vec\omega }\left 
\{ x\in \mathbb{R}^n: \left (\mathcal{M}_{\varphi ,\theta _0}^{\bigtriangleup}  
...
Next, we estimate $ \Psi $. Let $ \varphi (\lambda ):=\lambda ^{p} $, then $ 
\varphi (2\lambda )=2^{p} \lambda ^{p} \le 2^{p}\varphi (\lambda ) $, so $ 
\varphi $ satisfies the double condition. Using Lemma \ref{Lemma2.5}, Lemma 
...
\{ x\in \mathbb{R}^n:\mathcal{M}_{\delta,\varphi,\theta 
_0}^{\sharp,\bigtriangleup}  
\left ( T ( \vec f  ) \right )  ( x  ) >\lambda   \right \} \right |^\frac{1 
}{p} \\
&=C\left \| \mathcal{M}_{\delta ,\varphi ,\theta _0}^{\sharp ,\bigtriangleup} 
T( \vec f  ) \right \|_{L^{p,\infty } \left ( \upsilon _{\vec \omega }  \right 
...
\section{New weighted norm inequalities for new multilinear square operators 
with generalized kernels on new Morry spaces}\label{sec3}
...
_{r>0}\left[\int_{\mathbb{R}^{n}}\left(\varphi ^{\alpha} 
(Q(z,r))u^{-\lambda}(Q(z,r))\left\|f\chi_{Q(z,r)}\right\|_{L^{p}(w)}\right)^{l} 
d z\right]^{1 / l}<\infty.
\end{align*}
...
_{r>0}\left[\int_{\mathbb{R}^{n}}\left(\varphi ^{\alpha} 
(Q(z,r))u^{-\lambda}(Q(z,r))\left\|f \chi_{Q(z,r)}\right\|_{L^{p,\infty 
}(w)}\right)^{l}dz\right]^{1 / l}<\infty.
\end{align*}
\end{definition}
...
In 2021, Zhao and Zhou \cite{b39} studied new weighted norm inequalities for 
...
given below.
\begin{definition}{\rm(\cite{b39})}
Let $ u $ be a weight, $ \vec w=(w_1,\ldots ,w_m) $ be a vector weight and $ 
\lambda \in [0,1) $, $ 1\le p< l\le \infty  $, $ \alpha \in (- \infty ,\infty ) 
$, $ \vec p=(p_1, \ldots ,p_m) $, $ Q:=Q(z,r)$. The new strong Morrey space $ 
M_{\alpha ,\lambda }^{\vec p,l} (u,\vec w) $ is defined as the set of 
...
_{r>0}\left[\int_{\mathbb{R}^{n}}\left(\varphi ^{\alpha} 
(Q(z,r))u^{-\lambda}(Q(z,r))\prod_{j=1}^{m} \left\|f_j 
\chi_{Q(z,r)}\right\|_{L^{p_j}(w_j)}\right)^{l} d z\right]^{1 / l}<\infty.
\nonumber
\end{align*}

The new weak Morrey space $ WM_{\alpha ,\lambda }^{\vec p,l} (u,\vec w) $ is 
defined as the set of vector-valued measurable functions $ \vec f=(f_1,\ldots 
,f_m) $ satisfying $\| \vec f  \| _{WM_{\alpha ,\lambda }^{\vec p,l} (u,\vec 
w)} < \infty$, where 
\begin{align*}
\|\vec f\|_{WM_{\alpha, \lambda}^{\vec p, l}(u, \vec w)}
&:=\sup _{r>0}\left[\int_{\mathbb{R}^{n}}\left(\varphi ^{\alpha} 
(Q(z,r))u^{-\lambda}(Q(z,r))\prod_{j=1}^{m} \left\|f_j 
...
\begin{lemma}\label{Lemma3.1} {\rm(\cite{b39})}  
...
\end{align*}
\end {lemma}
\begin{lemma}\label{Lemma3.2} {\rm(\cite{b39})}  
Let $ 0< \theta < \infty ,1\le p< \infty  $. If $ w\in A_{p}^{\theta }(\varphi 
)   $, then there exists a positive constant $ C $, such that for any $\rho > 1 
$,
\begin{align*}
...
\quad\quad  The boundedness on the new Morry space is established for the 
...
\tilde{\alpha_i}:=p\beta/p_i$, and $ \beta :=\alpha + p\lambda \theta $ with $ 
\theta \ge 0 $.
\end {theorem}
...
each $f_j^0=f_j \chi_{Q^*}$, $ f_j^\infty=f_j-f_j^0 $. Then
\begin{align*}
\prod_{i=1}^{m} f_{i}\left(y_{i}\right) 
&=\prod_{i=1}^{m}\left(f_{i}^{0}\left(y_{i}\right)+
f_{i}^{\infty}\left(y_{i}\right)\right) 
=\sum_{\alpha_{1}, \ldots, \alpha_{m}\in \{0, \infty\}} 
f_{1}^{\alpha_{1}}\left(y_{1}\right) \cdots 
f_{m}^{\alpha_{m}}\left(y_{m}\right) \\
&=\prod_{i=1}^m f_i^{0}(y_i)+\sum_{(\alpha_1,\ldots,\alpha_m)\in 
\mathscr{L}}f_1^{\alpha_1}(y_1)\cdots f_m^{\alpha_m}(y_m),
\end{align*}
where  $\mathscr{L}:=\{(\alpha_1,\ldots,\alpha_m)$: there is at least one 
$\alpha_i=\infty\}$. For $ x\in Q(z,r)$, we have
\begin{align*}
\left|T\left(f_{1},\ldots, f_{m}\right)(x)\right|
&=|T\left(f_{1}^{0}, \ldots, 
f_{m}^{0}\right)(x)+\sum_{(\alpha_1,\ldots,\alpha_m)\in 
\mathscr{L}} T\left(f_{1}^{\alpha_{1}},\ldots,f_{m}^{\alpha_{m}}\right)(x)|  \\
...
&:=\left |  T\left(f_{1}^{\alpha_{1}}, \ldots,f_{m}^{\alpha_{m}}\right)(x) 
\right | \nonumber\\
&\le \left(\int_{0}^{\infty} \left ( \int_{(\mathbb{R}^{n})^{m}\backslash 
(Q^\ast )^{m}} \left|K_{t}(x, \vec{y})\right| \cdot \prod_{j=1}^{m} 
...
}\cdot \prod_{j=1}^{m} \left|f_{j}\left(y_{j}\right)\right| d \vec{y} \nonumber 
\\
...
\prod_{i=1}^{m}\left\|f_{i} \chi_{3^{k+1} Q}\right\|_{L^{p_i} (w_i)}.
\end{align*}
Multiplying both sides of the above inequality by $ \varphi(Q)^{\alpha} 
v_{\vec{w}}(Q)^{-\lambda} $, by Lemma \ref{Lemma2.3}, Lemma \ref{Lemma3.1} and 
Lemma \ref{Lemma3.2}, and taking $ N\ge m\theta/q'+\eta/p+mp\lambda\theta  
/q'-p\lambda\theta$, we obtain
\begin{align*}
&\varphi(Q)^{\alpha} v_{\vec{w}}(Q)^{-\lambda}\left\|T(\vec{f}) \chi_{Q(z, 
r)}\right\|_{L^{p}\left(v_{\vec{w}}\right)} \\
&\quad \leq C \sum_{k=0}^{\infty} \frac{1}{3^{n k \delta / p}} 
\varphi\left(3^{k+1} Q\right)^{\alpha+p \lambda \theta } 
...
\prod_{i=1}^{m}\left\|\varphi\left(3^{k+1} Q\right)^{(\alpha+p \lambda 
\theta) 	\frac{p}{p_i} } w_{i}\left(3^{k+1} Q\right)^{-\frac{\lambda p}{p_i} 
}|| f_{i} \chi_{3^{k+1} Q}||_{L^{p_{i}}\left(w_{i}\right)}\right\|_{L^{l p_{i} 
/p}\left(\mathbb{R}^{n}\right)}.
\end{align*}
Set $\tilde{l_i}:=lp_i/p$, $\tilde{\lambda_i }:=p\lambda/p_i$, $\tilde{\alpha 
_i }:=p\beta/p_i$, and $ \beta :=\alpha +p\lambda \theta$. Then Theorem 
...
For the term $ \mathrm{III} $, by Theorem \ref{Theorem2.1} $ \left ( ii \right 
) $, we obtain
\begin{align*}
...
\sum_{k=1}^{\infty }}  C_k< \infty$. Let $ \vec p=(p_1,\ldots ,p_m) $, $ \alpha 
\in (- \infty ,\infty ) $, $ \lambda \in [0,1) $, $ 1\le p<l\le \infty $,  $ 
1/p = {\textstyle \sum_{j=1}^{m}}  1/p_j$, $ \vec w=(w_1,\ldots ,w_m)\in 
A_{\vec p /q'}^{\theta }(\varphi ) $, $ v_{\vec w}= {\textstyle 
\prod_{j=1}^{m}}  w_{j}^{p/p_j}$, and $\beta =\alpha +p\lambda \theta  $ with $ 
\theta \ge 0 $. Then the following statements hold.
\begin{itemize}
...
Similar to the proof of Theorem \ref{Theorem3.1}, we omit the details here.
...
\bibitem[4]{b4}X. Chen, Q. Xue and K. Yabutam, On multilinear Littlewood-Paley 
...
pseudo-differentiels, {Bull. Soc. Math. France.} {57} (1978), 1-185.
...
\bibitem[39]{b39}N. Zhao and J. Zhou, New Weighted Norm Inequalities for 
