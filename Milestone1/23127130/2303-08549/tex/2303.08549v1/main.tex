...
\begin{document}
	...
	\title{A Bregman-Kaczmarz method for nonlinear systems of equations}
        ...
        \author{Robert Gower\thanks{CCM, Flatiron Institute, New York, USA, \href{mailto:gowerrobert@gmail.com}{gowerrobert@gmail.com}, \orcid{0000-0002-2268-9780}} \and Dirk A. Lorenz\thanks{Institute of Analysis and Algebra, TU Braunschweig, Braunschweig, Germany, \href{mailto:d.lorenz@tu-braunschweig.de}{d.lorenz@tu-braunschweig}, \orcid{0000-0002-7419-769X}} \and Maximilian Winkler\thanks{Insitute of Analysis and Algebra, TU Braunschweig, Braunschweig, Germany, \href{mailto:maximilian.winkler@tu-braunschweig.de}{maximilian.winkler@tu-braunschweig.de}, \orcid{0000-0002-8501-5556}}}
	\maketitle
	...
	\end{equation} 
	% \begin{align} \label{eq:ourmethodintro}
	%     x_{k+1} = \arg&\min_{y }  \;D_\varphi^{x_k^*}(x_k,y)  \\
	%     &\mbox{subject to}\quad y \in  H_k := \big\{x\in\RR^d: f_{i_k}(x_k) + \langle \nabla f_{i_k}(x_k), x-x_k\rangle = 0\big\}, \nonumber
	% \end{align}
	...
	%, that is
	%We denote the Bregman projection in short as $x_{k+1} = \Pi_{\varphi, H_k}^{x_k^*}(x_k)$.
	%We face a fundamental problem when dealing with the algorithm given by~\eqref{eq:ourmethodintro}:
	...
	%\begin{align*}
	%    x_{k+1} = \nabla\varphi^*(x_k^*-t_k\nabla f_{i_k}(x_k)), \qquad\text{with some } x_k^*\in\partial\varphi(x_k) \text{ and } t_k = \sigma \frac{f_{i_k}(x_k)}{\|\nabla f_{i_k}(x_k)\|_2^2},
	%\end{align*}
	%where $\varphi^*$ is the convex conjugate function of $\varphi$ (see chapter 3) and $\sigma>0$. In~\cite{LLMO21} and in our convergence analysis, we will see that this makes most sense if $\varphi$ is strongly convex with modulus $\sigma$. The new point $x_{k+1}$ then does not lie on the hyperplane, but still lies in $\dom \partial \varphi$ by construction. Indeed, subdifferential inversion shows that $x_{k+1}^* := x_k^*-t_k\nabla f_i(x_k)\in\partial\varphi(x_{k+1})$, which serves as a new subgradient for the next step. 
	%Summarizing, the proposed update reads as follows:
	%\vspace{0.3cm}
	%\begin{itemize}
	%   \item Set $x_{k+1} = \begin{cases}
	%        \Pi_{\varphi, H_k}^{x_k^*}(x_k), \qquad \text{if }~\eqref{eqn:Qualif_constraint_hyperplane} \text{ holds, } \\[0.5cm]
	%        \nabla\varphi^*\big(x_k^*-\sigma \frac{f_{i_k}(x_k)}{\|\nabla f_{i_k}(x_k)\|_2^2}\nabla f_{i_k}(x_k)\big), \qquad \text{otherwise.}
	%    \end{cases} $
	%    \item Choose $x_{k+1}^* \in \partial\varphi (x_{k+1})$.
	%\end{itemize}
	...
	We demonstrate that our proposed method can be reinterpreted as mirror descent with a novel adaptive step size in case that the $f_i$ are nonnegative. Moreover, for $\varphi(x)=\frac12\|x\|_2^2$, we obtain back the SGD method with the stochastic Polyak step size. For general $\varphi$, computing the step size requires the solution of a convex one-dimensional minimization problem. This is a similar situation as in the update of the SDCA method~\cite{shalev2013stochastic}, a popular stochastic variance reduced method  for minimizing regularized general linear models.
	
	Recently, in~\cite{LLMO21} it was proposed to use the stochastic Polyak step size from SGD in mirror descent. This update has the advantage that it is always defined and it is relatively cheap to compute. Follow-up works demonstrated good numerical performance in an example in quantum information theory \cite{CLY22} and proposed extensions to the case with unknown objective value \cite{LY22}. However, we prove that for convex functions, our proposed method
	...
	The function $\varphi$ is called $\sigma$-\emph{strongly} convex w.r.t. a norm $\|\cdot\|$ for some $\sigma>0$, if for all $x,y\in\mathrm{dom} \ \partial\varphi$ it holds that $\frac{\sigma}{2}\|x-y\|^2\leq D_\varphi^{x^*}(x,y)$. 
	...
	\begin{align}
		...
		t_{k,\sigma} = \sigma \frac{f_{i_k}(x_k)}{\|\nabla f_{i_k}(x_k)\|_*^2} \qquad \text{with some norm } \|\cdot\|_*.
	\end{align}
	...
	\begin{algorithm}[htb]
		\begin{algorithmic}[1]
			\State Input: $\sigma>0$ and probabilities $p_i>0$ for $i=1,...,n$ and $\sigma>0$
			\State Initialization: $x_{0}^*\in \RR^d, x_0 = \nabla \varphi^*(x_0^*)$
			\For{$k=0,1,...$}
			\State choose $i_k\in\{1,...,n\}$ according to the probabilities $p_1,...,p_n$              
			\If{$f_{i_k}(x)\neq 0$ and $\nabla f_{i_k}(x_k)\neq 0$}
	...
	Note that the problem~\eqref{eqn:BregProj_stepsize} is convex and one-dimensional and can be solved with the bisection method, if $\varphi^*$ is a $C^1$-function, or (globalized) Newton methods, if $\varphi^*$ is a $C^2$-function, see Appendix A. In the following, we show how to implement the steps of Algorithm~\ref{alg:NBK} for typical examples. 
	...
	\begin{bsp} \textup{(Unconstrained case and sparse Kaczmarz)}
		...
		\begin{align}
			...
			\varphi=\lambda\|x\|_1 + \frac12 \|x\|_2^2 
		\end{align} 
		...
		\begin{align*} 
			&\text{find } t_k\in \argmin_{t\in\RR} \frac12 \|S_\lambda(x_k^*-t\nabla f_{i_k}(x_k))\|_2^2 +  t\cdot \big( \langle \nabla f_{i_k}(x_k),x_k\rangle - f_{i_k}(x_k) \big), \\
	...
	\begin{bsp}\textup{(Simplex constraints)}
		...
		We can characterize condition~\eqref{eqn:Condition_hyperplane_nonempty_intersection} easily as follows: The hyperplane $H(\alpha,\beta)$ intersects $\mathrm{dom}(\partial\varphi)=\Delta^{d-1}_+$ if and only if
		\begin{itemize}
		...
		\begin{align} 
			...
			t_k&\in\argmin_{t\in\RR} \beta_k t + \log\big(\sum_{i=1}^d x_{k,i} e^{-t\partial_i f_{i_k}(x_k)}\big), \\
			\label{eq:expgrad}
			x_{k+1} &= x_k \cdot \frac{e^{-t_k\nabla f_{i_k}(x_k)}}{\|e^{-t_k\nabla f_{i_k}(x_k)}\|_1},
		\end{align}
	...
	\begin{bsp}\textup{(Cartesian products of constraints)}
		...
		\begin{align*}
			\mathrm{dom}(\partial\varphi) = \bigtimes_{i=1}^m \mathrm{dom}(\partial\varphi_i) \quad \text{and} \quad \partial\varphi(x) =  \bigtimes_{i=1}^m \partial\varphi_i(x_i) \text{ for all } x_i\in\mathrm{dom}(\partial\varphi_i).
		\end{align*} 
		...
		\begin{align*}
			t_k&\in \argmin_{t\in\RR} \beta_k t + \sum_{i=1}^m \varphi_i^*\big(x_{k,i}^*-t\nabla_{i}f_{i_k}(x_k)\big), \\
	...
	In this section we do the convergence analysis of Algorithm~\ref{alg:NBK}. At first, we characterize fixed points of Algorithm~\ref{alg:NBK} and Algorithm~\ref{alg:NBK_relaxed} and provide necessary lemmas for the subsequent analysis. In Section~\ref{subsec:star-convex}, we prove that for nonnegative star-convex functions $f_i$, condition~\eqref{eqn:Condition_hyperplane_nonempty_intersection} is always fulfilled and the step size~\eqref{eqn:BregProj_stepsize} is better than the relaxed step size~\eqref{eqn:mSPS_like_stepsize} in the sense that it results in an iterate with a smaller Bregman distance to all solutions of~\eqref{eqn:problem}. Finally, we present convergence results for Algorithm~\ref{alg:NBK} for this setting. 
	...
	As a first result, we determine the fixed points of Algorithm~\ref{alg:NBK}. The proposition states in particular that, in the unconstrained case $\mathrm{dom}(\varphi)=\RR^d$, fixed points are exactly the stationary points of the least-squares function $\|f(x)\|_2^2$.
	
	
	\begin{prop}
		\label{prop:Fixed_points}
		Let Assumption~\ref{as:Standard_assumptions} hold and let $x\in\mathrm{dom}(\partial\varphi)$ and $x^*\in\partial\varphi(x)$. The pair $\big(x,x^*\big)$ is a fixed point of Algorithm~\ref{alg:NBK} if and only if for all $i\in\{1,...,n\}$ it holds that $f_i(x) = 0$ or $\nabla f_i(x)=0$.
	\end{prop}
	...
	We will also use the following celebrated lemma.
	
	\begin{lem}[Robbins-Siegmund~\cite{Robbins71}]
		\label{lem:Robbins_Siegmund_lemma}
		Let $(\Omega,\FF,\PP)$ be a probability space and $\FF_k$ a filtration, i.e. a sequence of $\sigma$-algebras $\FF_k\subset\FF$ with $\FF_{k+1}\subset \FF_k$. Let $u_k,\theta_k,\xi_k,\zeta_k$ be nonnegative $\FF_k$-measurable random variables such that $\sum_{k=1}^\infty \theta_k<\infty$, $\sum_{k=1}^\infty \xi_k<\infty$ and
		\[ \EE(u_{k+1}\mid \FF_k) \leq (1+\theta_k)u_k + \xi_k - \zeta_k. \]
		Then, $u_k$ converges a.s. and it holds that $\sum_{k=1}^\infty \zeta_k<\infty$ a.s.
	\end{lem}
	...
	\begin{itemize}
		...
		Furthermore, we will converge to this intersection point by applying Algorithm~\ref{alg:NBK} to the nonnegative function $\tilde f$ where $\tilde f_i = f_i|_C - \hat f_i$. When  $n=1$, we cover the setting of mirror descent for the problem
		\begin{align}
	...
	\begin{thm}
		...
		\end{align*}
		If $\varphi$ is $\sigma$-strongly convex, the inequality also holds for the mSPS-step size $t=t_{k,\sigma}$.
	\end{thm}
	...
	\begin{lem}
		...
		\begin{align}
			...
			\leq  D_\varphi^{x_k^*}(x_k,\hat x) - \frac{\sigma}{2} \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2}. 
		\end{align}
	\end{lem}
	\begin{proof}
		...
		\[ \varphi^*(x_k^*) - \hat t f_{i_k}(x_k) + \frac{1}{2\sigma} \hat t^2\|\nabla f_{i_k}(x_k)\|_*^2 = \varphi^*(x_k^*) - \frac{\sigma}{2} \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2}. \]
		...
		\begin{align} 
			...
			\varphi^*(x_k^*-t\nabla f_{i_k}(x_k)) + t\beta_k \leq \varphi^*(x_k^*) - \frac{\sigma}{2} \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2} \end{align}
		holds for $t\in\{t_{k,\sigma},t_{k,\varphi}\}$. If Assumption 2(i) or 2(ii) are fulfilled, Lemma~\ref{lem:sign_of_stepsize} and star-convexity of $f_{i_k}$ show that
		\begin{align} 
			\label{eqn:Estimate_leq_convexity_estimate}
			t_k\big(f_{i_k}(x_k)+\langle \nabla f_{i_k}(x_k), \hat x-x_k\rangle\big) 
			\leq 0. 
		\end{align}
		Under Assumption 2(iii), we have equality in~\eqref{eqn:Estimate_leq_convexity_estimate}. 
		Inserting this inequality into Lemma~\ref{lem:Basic_Dphi_descent_estimate}, we obtain the claimed bound 
		\begin{align*}
			D_\varphi^{x_{k+1}^*}(x_{k+1},\hat x)
			&\leq \varphi^*(x_k^*) - \frac{\sigma}{2} \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2} - \langle x_k^*,\hat x\rangle + \varphi(\hat x) \\
			&= D_\varphi^{x_k^*}(x_k,\hat x) - \frac{\sigma}{2} \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2},
		\end{align*}
		where we used~\eqref{eqn:BregDist_with_conjugate} in the last step. \\
	\end{proof}
	
	
	We can now establish almost sure convergence of Algorithm~\ref{alg:NBK}. 
	...
	\begin{thm} 
		...
		\begin{align*} \EE\big[\min_{l=1,...,k} \sum_{i=1}^n p_i \big(f_i(x_l)\big)^2\big] 
			\leq \frac{2C}{\sigma k} \end{align*}
		with some constant $C>0$. Moreover, the iterates $x_k$ of Algorithm~\ref{alg:NBK} converge a.s. to a point in the solution set~$S$. 
	\end{thm}
	\begin{proof}
		Taking expectation in~\eqref{eqn:Descent_estimate_in_thm_proof_leq} and rearranging, we obtain for all $l\in\NN$ that
		\begin{align*}
			\EE\Big[\sum_{i=1}^n p_i(f_i(x_l))^2\Big] \leq \frac{2c}{\sigma} \cdot \big(\EE\big[D_\varphi(x_{l},\hat x)\big]- \EE\big[D_\varphi(x_{l+1},\hat x)\big]\big), 
		\end{align*}
		The convergence rate now follows with $C=c\cdot D_{\varphi}^{x_0^*}(x_0,\hat x)$ for any $\hat x\in S$ by averaging over $l=0,...,k$ and telescoping. 
		
		Next, we prove the almost sure iterate convergence. By Lemma~\ref{lem:Dphi_descent_estimate} and $\sigma$-strong convexity of $\varphi$, we have that
		...
		\[ \|\nabla f_{i_k}(x_k)\|_*^2 \leq c \] 
		with some constant $c>0$. Inserting this into~\eqref{eqn:Dphi_descent_estimate_leq} from Lemma~\ref{lem:Dphi_descent_estimate} and taking conditional expectation w.r.t. $i_0,...,i_{k-1}$ yields that
		\begin{align}
			\EE\big[D_\varphi^{x_{k+1}^*}(x_{k+1},\hat x) \mid i_0,...,i_{k-1}\big]  
			&\leq
			D_\varphi^{x_k^*}(x_{k},\hat x) - \frac{\sigma}{2c} \cdot \sum_{i=1}^m p_i \big(f_i(x_k)\big)^2 
			\label{eqn:Descent_estimate_in_thm_proof_leq} \\
			&\leq 
			D_\varphi^{x_k^*}(x_{k},\hat x) - \frac{\sigma \cdot \min_i p_i}{2c} \cdot \|f(x_k)\|_2^2. \nonumber
		\end{align}
		The Robbins-Siegmund-Lemma proves that $f(x_k)\to 0$ with probability $1$. In this case, all cluster points of the sequence $x_k$ lie in $S$. The assertion now follows by standard arguments as made in \cite[Proposition 2.9]{BC03}: For two subsequences $x_{l_k}$ with $x_{l_k}\to x$ and $x_{m_k}$ with $x_{m_k}\to y$, Assumption \ref{as:Standard_assumptions}(iv) gives that $D_\varphi^{x_{l_k}^*}(x_{l_k},x)\to 0$ and $D_\varphi^{x_{m_k}^*}(x_{m_k},y)\to 0$. But, as it holds that $x,y\in S$, Lemma~\ref{lem:Dphi_descent_estimate} shows that also $D_\varphi^{x_{l_k}^*}(x_{l_k},y)\to 0$ and $D_\varphi^{x_{m_k}^*}(x_{m_k},x)\to 0$. By strong convexity of $\varphi$, this implies that $\|x_{l_k}-y\|_2\to 0$ and $\|x_{m_k}-x\|_2\to 0$, and hence, $x=y$. Since the sequence $x_k$ is bounded, this proves that $x_k$ converges a.s. to a point in $S$. \\
	\end{proof}
	...
	\begin{proof}
		...
		\[ \overline f(x_k) = \overline f(x_k) - \overline f(\hat x) - \langle \nabla \overline f(\hat x), x_k-\hat x\rangle \geq \mu D_\varphi(\hat x,x_k) \geq \frac{\mu\sigma}{2}\|x_k-\hat x\|^2\geq \frac{\mu\sigma}{M}D_\varphi^{x_k^*}(x_k,\hat x). \]
	\end{proof}
	
	Since the proofs of Theorem \ref{thm:Convergence_nonnegative_convex_C1_leq}, Theorem \ref{thm:Convergence_nonnegative_convex_Li_smooth} and Theorem \ref{thm:Convergence_nonnegative_convex_Li_smooth_mu_sc} basically rely on Lemma \ref{lem:Dphi_descent_estimate}, they also hold for Algorithm \ref{alg:NBK_relaxed}.
	...
	Inspired by~\cite{WLBG22}, we consider functions fulfilling the so-called tangential cone condition, which was introduced by~\cite{HNS95} as a sufficient condition
	...
	\begin{lem}
		...
		\[ D^{x_{k+1}^*}_\varphi(x_{k+1},\hat x) \leq D^{x_k^*}_\varphi(x_k,\hat x) - \tau \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2}, \] 
	...
	\begin{proof}
		...
		\begin{align*}
			t_{k,\sigma} \big( f_{i_k}(x_k) + \langle f_{i_k}(x_k),\hat x-x_k\rangle \big) \leq \eta \sigma \frac{f_{i_k}(x_k)^2}{\|\nabla f_{i_k}(x_k)\|_*^2}.
		\end{align*}
	...
	\begin{thm} 
		...
		Let Assumption~\ref{as:Standard_assumptions} hold and let $\varphi$ be $\sigma$-strongly convex. Moreover, let Assumption~\ref{as:tangential_cone_condition} hold with some $\eta>0$ and $\hat x\in S$ and let $x_0\in B_{r,\varphi}(\hat x) \cap C$.
		\begin{enumerate}[(i)]
			\item If $\eta<\frac12$, then the iterates $x_k$ of Algorithm~\ref{alg:NBK_relaxed} converge a.s. to $\hat x$ and fulfill that 
			\begin{align*} 
				\EE\big[\min_{l=1,...,k} \sum_{i=1}^n p_i \big(f_i(x_l)\big)^2\big] 
				\leq 
				\frac{C \cdot D_\varphi^{x_0^*}(x_0,\hat x)}{\sigma\big(\frac12-\eta\big) k}.
			\end{align*}
			\item Let $\varphi$ be additionally $M$-smooth and $\eta < \frac{\sigma}{2M}$. Assume that $x_k$ are the iterates of Algorithm~\ref{alg:NBK} and the condition $H_k\cap\mathrm{dom}(\varphi)\neq\emptyset$ is fulfilled for all $k$. Then the $x_k$ converge a.s. to $\hat x$ and it holds that 
			\begin{align*} 
				\EE\big[\min_{l=1,...,k} \sum_{i=1}^n p_i \big(f_i(x_l)\big)^2\big] 
				\leq 
	...
	\begin{proof}
		First, we observe that $\eta$-TCC implies uniqueness of $\hat x$ as a zero of $f_i$ in $B_{r,\varphi}$ for all $i\in\{1,...,n\}$: Assume on the contrary there exists $\hat x'\in B_{r,\varphi}(\hat x)$ with $f_i(\hat x')=0$. But then, Rolle's theorem gives a point $x'\in B$ on the line segment between $\hat x$ and $\hat x'$ with vanishing directional derivative $Df_i(x')[\hat x - x']$ and we obtain the equality 
		\[ \big|f_i(x') + \langle \nabla f_i(x'), \hat x - x'\rangle - f_i(\hat x)\big| = 
		\big|f_i(x') + Df_i(x')[\hat x - x'] - f_i(\hat x)\big| =
		|f_i(x')-f_i(\hat x)|,\]
		which is a contradiction to $\eta <1$ in $\eta$-TCC. \\
		Hence, the set $S\cap B_{r,\varphi}(\hat x)$ contains only one element. By Lemma~\ref{lem:Dphi_descent_estimate_tcc}, the $x_k$ stay in~$B_{r,\varphi}(\hat x)$.
	...
	\begin{thm} 
		...
		Let Assumption~\ref{as:Standard_assumptions} hold true and let $\varphi$ be $\sigma$-strongly convex. Let Assumption~\ref{as:tangential_cone_condition} hold with some $\eta>0$ and $\hat x\in S$ and let $x_0\in B_{r,\varphi}(\hat x) \cap C$. Moreover, assume that the Jacobian $Df(x)$ has full column rank for all $x\in B_{r,\varphi}(\hat x)$ and further that
		\[ \kappa_{\min}  := \min_{x\in B_{r,\varphi}(\hat x)} \min_{\|y\|_2=1} \frac{\|Df(x)\|_F}{ \|Df(x)y\|_2 } > 0, \qquad p_{\min}=\min_{i=1,...,n} p_i>0. \]
		...
		\begin{enumerate}
			...
			\item[(ii)] Let $\varphi$ be additionally $M$-smooth and $\eta < \frac{\sigma}{2M}$. Assume that $x_k$ are the iterates of Algorithm~\ref{alg:NBK} and the condition $H_k\cap\mathrm{dom}(\varphi)\neq\emptyset$ is fulfilled for all $k$. Then it holds that 
			\begin{align} 
				\label{eqn:tcc_linear_Alg2}
				\frac{\sigma}{2}\EE\big[\|x_k-\hat x\|_2^2] 
				\leq \EE\big[D_\varphi(x_{k},\hat x)\big] 
				\leq \Big( 1- \frac{ \sigma\big(\frac12-\eta\frac{M}{\sigma}\big)p_{\min}}{(1+\eta)^2\kappa_{\min}^2} \Big)^k \EE\big[D_\varphi(x_0,\hat x)\big].
			\end{align}
		\end{enumerate}
	\end{thm}
	
	For the proof, as in~\cite{WLBG22} we use the following two auxiliary lemmas.
	
	\begin{lem}[\cite{WLBG22}]
		\label{lem:tcc_slope_bound}
		If a function $f$ fulfills $\eta$-TCC on $U$ with constant $\eta\in ]0,1[$, it holds for all $x,y\in U$ that \[ |f(x)-f(y)| \geq \frac{1}{1+\eta}|\langle \nabla f(x), y-x\rangle|. \]
	\end{lem}
	...
	\begin{proof}[Proof of Theorem~\ref{thm:Convergence_tcc_linear_rate}]
		In all three cases, by Lemma~\ref{lem:Dphi_descent_estimate_tcc}, Lemma~\ref{lem:tcc_slope_bound} and the fact that $f(\hat x) = 0$, we can estimate
		\begin{align*} 
		...
		\begin{align*} 
			...
			\EE\big[D_\varphi(x_{k},\hat x)\big] - \frac{\tau}{(1+\eta)^2} \cdot \sum_{i=1}^n p_i \frac{|\langle \nabla f_i(x_k), \hat x-x_k\rangle|^2}{\|\nabla f_i(x_k)\|_2^2} \\
			&\leq 
			\EE\big[D_\varphi(x_{k},\hat x)\big] - \frac{\tau p_{\min}}{(1+\eta)^2}  \cdot \frac{\|Df(x_k)(\hat x-x_k)\|_2^2}{\|Df(x_k)\|_F^2} \\
			&\leq \EE\big[D_\varphi(x_{k},\hat x)\big] - \frac{\tau p_{\min}}{(1+\eta)^2 \kappa_{\min}^2} \cdot \|x_k-\hat x\|_2^2 \\
	...
	In this section, we evaluate the performance of the NBK method. In a first experiment we use NBK to find sparse solutions with the nonsmooth DGF $\varphi(x) = \frac{1}{2}\|x\|_2^2+\lambda\|x\|_1$ for unconstrained quadratic equations, that is, with $C=\RR^d$. Next, we employ the negative entropy DGF over the probability simplex $C=\Delta^{d-1}$ to solve simplex-constrained linear equations as well as the \emph{left-stochastic decomposition problem}, a quadratic problem over a product of probability simplices with applications in clustering~\cite{AFGK13}. All the methods were implemented in MATLAB. and can be downloaded from %\href{https://github.com/MaxiWk/Bregman-Kaczmarz}{\texttt{https://github.com/MaxiWk/Bregman-Kaczmarz}}.
	\url{https://github.com/MaxiWk/Bregman-Kaczmarz}.
	
	
	\subsection{Sparse solutions of quadratic equations}
	As a first example, we consider multinomial quadratic equations
	\[ f_i(x) = \frac12\langle x,A^{(i)}x\rangle + \langle b^{(i)},x\rangle + c^{(i)}=0 \]
	with $A^{(i)}\in\RR^{d\times d}$, $b^{(i)}\in\RR^d$ and $c^{(i)}\in\RR$. We investigate if Algorithm~\ref{alg:NBK} (NBK method) and Algorithm~\ref{alg:NBK_relaxed} (relaxed NBK method) are capable of finding a sparse solution $\hat x\in\RR^d$ by using the DGF $\varphi(x)=\lambda\|x\|_1+\frac12\|x\|_2^2$ and test both methods against the euclidean nonlinear Kaczmarz method (NK). As it holds $\mathrm{dom} \ \varphi = \RR^d$, it is always possible to choose the step size $t_{k,\varphi}$ from~\eqref{eqn:BregProj_stepsize} in the NBK method. Moreover, the step size can be computed exactly by a sorting procedure, as $\varphi^*$ is a continuous piecewise quadratic function, see Example~\ref{ex:varphi_from_sparse_Kaczmarz}. In order to guarantee existence of a sparse solution, we choose a sparse vector $\hat x\in\RR^d$, sample the data $A^{(i)}, b^{(i)}$ randomly with entries from the standard normal distribution and set 
	\[ c^{(i)} = - \Big( \frac12\langle x,A^{(i)}x\rangle + \langle b^{(i)},x\rangle \Big). \]
	In all examples, the nonzero part of $\hat x$ and the initial subgradient $x_0^*$ are sampled from the standard normal distribution. The initial vector $x_0$ is computed by $x_0=\nabla\varphi^*(x_0^*) = S_\lambda(x_0^*)$.
	
	From the updates it is evident that computational cost per iteration is cheapest for the NK method, slightly more expensive for the rNBK method and most expensive for the NBK method. To examine the case $d<n$, we choose $A^{(i)}\sim\mathcal N(0,1)^{500\times 500}$ for $i=1,...,1000$, $\hat x$ with $25$ nonzero entries and $\lambda=10$ and perform 20 random repeats. Figure~\ref{fig:sparse_quadratic_overdet} shows that the NBK method clearly outperforms the other two methods in this situation, even despite the higher cost per iteration. 
	...
	\begin{figure}[ht]
		...
		\includegraphics{sparse_quadratic/fig11}
		&
		\includegraphics{sparse_quadratic/fig12}
			\end{tabular}
		\includegraphics{sparse_quadratic/fig1_legend}
	...
	\begin{figure}[htb]
		\begin{center}
			\begin{tabular}{rl} 			
	\includegraphics{sparse_quadratic/fig21}
	&
	\includegraphics{sparse_quadratic/fig22}
\end{tabular}

% legend
\includegraphics{sparse_quadratic/fig2_legend}
	...
	In our experiments we have noticed that in large dimensions, such as $d\geq 100$, solving the Bregman projection~\eqref{eqn:BregProj_stepsize} up to a tolerance of $\epsilon=10^{-9}$ takes less than half as much computation time as the simplex projection. As these two are the dominant operations in these methods, the NBK updates are computationally cheaper than the NK updates in the high dimensional setting. However, the examples will show that convergence quality of the methods depends on the distribution of the entries of $A$. All methods were observed to converge linearly.
	
	In the following experiments, we take different choices of $A$ and set the right-hand side to $b=A\hat x$ with a point $\hat x$ drawn from the uniform distribution on the probability simplex $\Delta^{d-1}$. All methods are initialized with the center point $x_0=(\frac{1}{d}, ..., \frac{1}{d}).$ 
	
	For our first experiment, we choose standard normal entries $A\sim\mathcal N(0,1)^{n\times d}$ with $(n,d) = (500,200)$ and $(n,d)=(200,500)$. Figure~\ref{fig:simplex_linear_stdn_distr} shows that in this setting, the POCS method achieves much faster convergence in the overdetermined case $(n,d) = (500,200)$ than the NBK method, whereas both methods perform roughly the same in the underdetermined case $(d,n) = (200,500)$. The rNBK method was considerably slower than the other two methods, which shows that the computation of the $t_{k,\varphi}$ step size for NBK pays off. 
	
	In our second experiment, we build up the matrix from uniformly distributed entries $A\sim\mathcal U([0,1])^{n\times d}$ and $A\sim\mathcal U([0.9,1])^{n\times d}$ with $(n,d)=(200,500)$. The results are summarized in Figure~\ref{fig:simplex_linear_unif_distr}. For the Kaczmarz method it has been observed in practice that so called 'redundant' rows of the matrix $A$ deteriorate the convergence of the method~\cite{JNY22}. This effect can also occur with the POCS method, as it also relies on euclidean projections. Remarkably, this is not the case for the NBK method and it clearly outperforms the POCS method and the rNBK method. This in particular shows that the multiplicative update used in both the rNBK method and the NBK method is not enough to overcome the difficulty of redundancy- to achieve fast convergence, it must be combined with the appropriate step size which is used by the proposed NBK method.
	
	Finally, we illustrate the effect of the accuracy $\epsilon$ in step size computation for the NBK method. We choose $A\sim U([0,1])^{n\times d}$ and $\epsilon=10^{-9}$ and compare with the larger tolerance $\epsilon=10^{-5}$. Figure~\ref{fig:simplex_linear_effect_of_inaccuracy} shows that, with $\epsilon=10^{-5}$, the residual plateaus 
	...
	\begin{figure}[htp]
		\begin{center}
	\begin{tabular}{rl} 
		\includegraphics{linear_equations_on_simplex/fig31}
		&
		\includegraphics{linear_equations_on_simplex/fig32}
		\\[0.8cm]
		\includegraphics{linear_equations_on_simplex/fig33}
		&
		\includegraphics{linear_equations_on_simplex/fig34}
		
	\end{tabular}
	
	\includegraphics{linear_equations_on_simplex/fig3_legend}
...
\end{center}
	...
	\begin{figure}[htb]
		\begin{center}
	\begin{tabular}{rl} 
		\includegraphics{linear_equations_on_simplex/fig41}
		&
		\includegraphics{linear_equations_on_simplex/fig42}
		\\
		\includegraphics{linear_equations_on_simplex/fig43}
		&
		\includegraphics{linear_equations_on_simplex/fig44}
	\end{tabular}
	
	\includegraphics{linear_equations_on_simplex/fig4_legend}
...
\end{center}
	...
	\begin{figure}[htb]
		\begin{center}
	\begin{tabular}{rl} 
		\includegraphics{linear_equations_on_simplex/fig51}
		&
		\includegraphics{linear_equations_on_simplex/fig52}
	\end{tabular}
	
	\includegraphics{linear_equations_on_simplex/fig5_legend}
...
\end{center}
	...
	\end{align}
	where \[L^{r,m}:=\big\{P\in\RR^{r\times m}_{\geq 0}: P^T\mathbbm{1}_r = \mathbbm{1}_m\}\] is the set of left stochastic matrices and $A\in\RR^{r\times m}$ is a given nonnegative matrix. The problem is equivalent to the so-called \emph{soft-K-means} problem and hence has applications in clustering~\cite{AFGK13}. We can view~\eqref{eqn:LSD_problem} as instance of problem~\eqref{eqn:problem} with component equations
	\begin{align*}
		f_{i,j}(X) = \langle X_{:,i}, X_{:,j} \rangle - A_{i,j} = 0 \qquad \text{for } i=1,...,r, \ j=1,...,m
	\end{align*}
	and $C=L^{r\times m}\cong \left(\Delta^{r-1}\right)^m$, where $X_{:,i}$ denotes the $i$'th column of $X$. For Algorithm~\ref{alg:NBK} we choose the DGF from Example~\ref{ex:DGF_and_NBK_for_cartesian_products} with the simplex-restricted negative entropy $\varphi_i=\varphi$ from Example~\ref{ex:Simplex_entropy}.
	Since $f_{i,j}$ depends on at most two columns of $X$, Algorithm~\ref{alg:NBK} acts on $\Delta^{r-1}$ or $\Delta^{r-1}\times \Delta^{r-1}$ in each step. For the update this implies that we can apply the steps from Example~\ref{ex:Simplex_entropy} in the first case, and from Example~\ref{ex:Double_simplex_entropy} in the second case.
	
	We compare the performance of Algorithm~\ref{alg:NBK} and Algorithm~\ref{alg:NBK_relaxed} to a projected nonlinear Kaczmarz method given by Algorithm~\ref{alg:EuclProj_LSD}. Here, by $X_{k,:,i}$ we refer to the $i$'th column of the $k$'th iterate matrix. In all examples, we set $A=\hat X^T \hat X$, where the columns of $\hat X$ were sampled according to the uniform distribution on~$\Delta^{r-1}$.
	...
	We have observed that Algorithm~\ref{alg:NBK} (NBK method) gives fastest convergence, if $r$ is not much smaller than $m$, see Figure~\ref{fig:LSD_100x50} and Figure~\ref{fig:LSD_50x100}. In both experiments, we noticed that condition~\eqref{eqn:Condition_hyperplane_nonempty_intersection} was actually fulfilled in each step, but checking did not show a notable difference in performance. The most interesting setting for clustering is that $r$ is very small and $m$ is large, as $r$ is the number of clusters~\cite{AFGK13}. However, it appears unclear if the NBK or the PNK method are a better choice for this problem size, as Figure~\ref{fig:LSD_3x100} shows. In this experiment, condition \eqref{eqn:Condition_hyperplane_nonempty_intersection} was not always fulfilled in the NBK method and we needed to employ the globalized Newton method together with an additional condition to the step size approximation, see Appendix for details. Algorithm~\ref{alg:NBK_relaxed} again was clearly outperformed by the other two methods in all experiments.
	...
	\begin{figure}[htb]
		...
		\begin{center}
			...
			\begin{tabular}{rl} 
	\includegraphics{left_stochastic_decomposition/fig61}
	&
	\includegraphics{left_stochastic_decomposition/fig61}
\end{tabular}
\includegraphics{left_stochastic_decomposition/fig6_legend}
			\vspace{0.3cm}
	...
	\begin{figure}[htb]
		...
		\begin{center}
			...
			\begin{tabular}{rl}
	\includegraphics{left_stochastic_decomposition/fig71}
	& \hspace{0.5cm}
	\includegraphics{left_stochastic_decomposition/fig72}
\end{tabular}
\includegraphics{left_stochastic_decomposition/fig7_legend}
			\vspace{0.3cm}
	...
	\begin{figure}[htb]
		...
		\begin{center}
			...
			\begin{tabular}{rl}
	\includegraphics{left_stochastic_decomposition/fig81}
	& \hspace{0.5cm}
	\includegraphics{left_stochastic_decomposition/fig82}
\end{tabular}
\includegraphics{left_stochastic_decomposition/fig8_legend}
			\vspace{0.3cm}
			
			\caption{Experiment 'Left stochastic decomposition problem' with $r=3, m=100$, residuals $\|f(X_k)\|_2$ against computation time. Left and right: Two random examples with different convergence behaviour. Thick line shows median over all trials, light area is between min and max, darker area indicates 25th and 75th quantile.}
			\label{fig:LSD_3x100}
	...
	\section{Newton's method for linesearch problem \eqref{eqn:BregProj_stepsize}}
