...
\usepackage[portrait,margin=2.9cm]{geometry}
...
\crit_i(\lambda)
:= 
\cG(2\tilde \ve_{\gamma_i(\lambda)}, \tilde \ve_{\gamma_i(\lambda)}, \cP_i)\, ,  
\ \ \
i\geq 1\, .
\end{equation}
...
If $W$ has weak partial derivatives $D_x W$ and $D_y W$ on the region $\{0<y<x<1\}$, then by the Poincar\'e inequality for functions in Sobolev spaces, for any $1\leq j<i\leq n$,
