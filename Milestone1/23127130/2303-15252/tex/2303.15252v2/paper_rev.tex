...
Similar as in the spin-$1/2$ Heisenberg model it has been observed that correlation functions of this model on
short segments can be `factorized': they are completely determined by
a single nearest-neighbour two-point function $\omega$ and a set of
structure functions.  While $\omega$ captures the
dependence on the system size and the state of the system the structure functions can be expressed in terms of the
...
Recently, we have expressed reduced density matrices in general face models in terms of their local Boltzmann weights \cite{FrWe21}.  Following a similar construction for the Heisenberg spin chain \cite{AuKl12} this allows to derive discrete functional equations called `reduced' qKZ equations satisfied by the RDMs.  A study of these equations for the critical RSOS models has been initiated in \cite{FrWe21}: based on finite size studies of these models with a small number of allowed local heights we found that their two- and three-site RDMs can be written in factorized form similar to (\ref{XXXfac}) for \emph{all} states from a particular topological sector. In this paper we continue this work: after a short review of the definition of the models we propose the functional equations satisfied by the RDMs of the RSOS models. Complementing the restricted qKZ equation with a set of recurrence and reduction relations resulting from the properties of the Boltzmann weights together with an identity for the asymptotics of the RDMs observed to hold in a particular topological sector allows to compute the algebraic part of the two- and three-site density matrices which then can be expressed in terms of the generators of the Temperley-Lieb algebra underlying the RSOS model.  For the physical part we solve the restricted qKZ equation for the nearest-neighbour two-site RDM for the ground state of the  model in the thermodynamic limit and compare our result to those for finite systems.  Finally, we apply our expressions to compute multi-point LHPs for the critical RSOS models.
...
\begin{equation}
\label{eq:transfer}
    t(u) = \sum_{\alpha\gamma} T^{\alpha\alpha}_{\gamma\gamma}(u)\,.
\end{equation}
...
% \draw[dotted] (0.5,0.5)--(1,0.5);
% \draw[dotted] (0.5,-0.5)--(1,-0.5);
% \draw (1,0)--(1,0.5)--(2,0.5)--(2,0)--(1,0)--(1,-0.5)--(2,-0.5)--(2,0);
% \node[draw,circle,inner sep=1pt,fill] at (1,0) {};
%
%  \node [left,font=\scriptsize] at (0,0) {$a$} ;
%  \node [above,font=\scriptsize] at (0.5,0.5) {$f$} ;
%  \node [above,font=\scriptsize] at (1,0.5) {$f$} ;
%  \node [above,font=\scriptsize] at (2,0.5) {$e$} ;
%  \node [right,font=\scriptsize] at (2,0) {$d$};
%  \node [below,font=\scriptsize] at (2,-0.5) {$c$};
%  \node [below,font=\scriptsize] at (1,-0.5) {$b$};
%  \node [below,font=\scriptsize] at (0.5,-0.5) {$b$};
%  \node [below right=0cm, font=\tiny] at (1,0) {$g$};
%  
%  \node [font=\scriptsize] at (0.5,0) {$u-v$};
%  \node [font=\scriptsize] at (1.5,0.25) {$v$};
%  \node [font=\scriptsize] at (1.5,-0.25) {$u$};
%\end{tikzpicture} }=
%\scalebox{1.5}{
%\begin{tikzpicture}[baseline=(current bounding box.center)]
% \draw (0,0)--(0,0.5)--(1,0.5)--(1,0)--(0,0)--(0,-0.5)--(1,-0.5)--(1,0);
% \draw (1,0)--(1.5,0.5)--(2,0)--(1.5,-0.5)--(1,0);
% \draw [dotted] (1,0.5)--(1.5,0.5);
% \draw[dotted] (1,-0.5)--(1.5,-0.5);
% \node[draw,circle,inner sep=1pt,fill] at (1,0) {};
% 
%  \node [left,font=\scriptsize] at (0,0) {$a$} ;
%  \node [above,font=\scriptsize] at (0,0.5) {$f$} ;
%  \node [above,font=\scriptsize] at (1,0.5) {$e$} ;
%  \node [above,font=\scriptsize] at (1.5,0.5) {$e$} ;
%  \node [right,font=\scriptsize] at (2,0) {$d$};
%  \node [below,font=\scriptsize] at (1.5,-0.5) {$c$};
%  \node [below,font=\scriptsize] at (1,-0.5) {$c$};
%  \node [below,font=\scriptsize] at (0,-0.5) {$b$};
%  \node [below left=0cm, font=\tiny] at (1,0) {$g$};
%  
%  \node [font=\scriptsize] at (0.5,0.25) {$u$};
%  \node [font=\scriptsize] at (0.5,-0.25) {$v$};
%  \node [font=\scriptsize] at (1.5,0) {$u-v$};
%
%\end{tikzpicture}}.
% \end{equation}
%------------------------------------------------------
...
\end{equation}
in terms of the Yang-Baxter operators $W_j(u)$ acting on $\mathcal{H}^L$ as
\begin{equation}
\label{eq:YB-op}
        \bra{a_0\dots a_L}W_j(u)\ket{b_0\dots b_L} \equiv \prod_{k\ne j}\delta_{a_kb_k}\,W \left(
    \begin{matrix}
      a_{j-1} & a_j\\
      b_j & a_{j+1}
    \end{matrix}\,\middle|\, 
    u\right)\,.
\end{equation}
With  (\ref{eq:rsos_weights}) these operators can be expanded as
\begin{equation}
    W_j(u) = \rho(u)\,\mathbf{1} + \rho(u+\lambda)\,e_j,
\end{equation}
where $\{\mathbf{1},e_1,\dots,e_{n-1}\}$ is a representation of the generating elements of the Temperley-Lieb algebra $\text{TL}_n(\beta)$
\begin{equation}
    \label{eq:TL}
    \begin{aligned}
...
\end{equation}
and generalized RDMs $D_N$ depending on a set of auxiliary spectral parameters $\lambda_j$, $j=1,\dots,N$,
\begin{equation}
    \label{eq:def_D}
...
\end{equation}
where $\underline{\bm{\alpha}} = (\alpha_0,\dots,\alpha_N)$ and $\underline{\bm{\beta}} = (\beta_0,\dots,\beta_N)$ are sequences of heights labeling the basis of the space $\mathcal{V}^N=\text{span}\{\ket{\alpha_0\dots\alpha_N}: |\alpha_{j+1}-\alpha_j|=1\}$ (for a graphical representation of this object see Appendix~\ref{app:DN_graphical}).
...
\end{equation}
In view of this relation we can complement (\ref{eq:def_D}) by
\begin{equation}
  \label{def_D0}
  D_0^{\alpha,\alpha} = \frac{1}{\braket{\Phi}{\Phi}}\,\bra{\Phi}\,E^{\alpha_{0}}_{\alpha_{0}}  \ket{\Phi} \equiv P_\alpha
\end{equation}
being the local height probability (LHP) of the critical RSOS model \cite{AnBF84}
\begin{equation}
\label{LHP1}
  P_\alpha = \frac{2\lambda}{\pi}\,\sin^2\alpha\lambda\,.
\end{equation}
%$D_0^{\alpha,\alpha}=\bra{\Phi}E^{\alpha_0}_{\alpha_0}\ket{\Phi}/\braket{\Phi}{\Phi}$.

As a consequence of the state $\ket{\Phi}$ satisfying periodic boundary conditions the matrix elements $D_N(\lambda_1,\dots,\lambda_{N})^{\{\underline{\bm{\alpha}}\}\{\underline{\bm{\beta}}\}}$ vanish for $\alpha_0\neq\beta_0$, $\alpha_N\neq\beta_N$. Therefore $D_N$ can be decomposed into blocks labeled $[\alpha_{0},\alpha_{N}]$, i.e.\
\begin{equation}
  \label{eq:def_Dblock}
...
%---------------------------------------------------------------------- 
In \cite{FrWe21} a factorization of correlation functions similar to (\ref{XXXfac}) has been observed to hold for the generalized two- and three-site density matrices of RSOS models with $r=4$ and $5$ in the topological sectors with quantum dimension $d_q(j)=1$: 
\begin{equation}
\label{RSOSfac}
\begin{aligned}
...
%
Moreover, $D_1(\lambda_1)$ has been shown to be independent of the spectral parameter $\lambda_1$ and can be obtained from the LHP (\ref{LHP1}), see Ref.~\cite{FrWe21}: using the symmetry $P_{a,a+1}=P_{a+1,a}$ of the two-site LHPs (i.e.\ the probabilities that the heights at two adjacent sites are equal to $a$ and $a+1$) and the identity $\sum_b P_{a,b}=P_a$ the non-zero elements of $D_1=F_{1;\varnothing,\varnothing}$ are found to  be ($1\leq a \leq r-2$)
\begin{equation}
\label{D1}
...
%
%Moreover, among them $F_{N;\varnothing,\varnothing}$ and $F_{2;(1),(2)}$ are found to be independent of the spectral parameters $\lambda_j$.

In  the following we collect several properties of the reduced density matrices constructed in the previous section and relations satisfied by  them which will be used below for their computation:
...
\paragraph{} The $N$-site generalized RDM can be restricted to the subspace diagonal in one of the spins, e.g.\ with matrix elements with $\alpha_j=\beta_j$ for the $j$-th index, by inserting the projector $\mathcal{P}_{\text{per}}$ onto states in $\mathcal{H}^L$ obeying \emph{periodic} boundary conditions in (\ref{eq:def_D}) as
\begin{equation}
    \label{eq:def_D-restricted}
    D_N^{(j)}(\lambda_1,\dots,\lambda_{N})^{\{\underline{\bm{\alpha}}\}\{\underline{\bm{\beta}}\}}   
    = %\sum_{\alpha_0,\alpha_N} \delta_{\alpha_0,\beta_0}\,\delta_{\alpha_N,\beta_N}\, 
      \frac{\bra{\Phi} \left(\prod_{k=1}^{j} T^{\alpha_{k-1} \beta_{k-1}}_{\alpha_k \beta_k}(\lambda_k)\right) \mathcal{P}_{\text{per}}\,
      \left(\prod_{k=j+1}^{N}  T^{\alpha_{k-1} \beta_{k-1}}_{\alpha_k \beta_k}(\lambda_k)\right) \ket{\Phi}}
      {{\braket{\Phi}{\Phi}}\,\prod\limits_{k=1}^{N} \Lambda(\lambda_k)}\,.
\end{equation}
Performing partial traces of $D_N^{(1)}$ ($D_N^{(N-1)}$) over the spin $\alpha_0$ ($\alpha_{N}$) one obtains the follwoing relations between RDMs of different order:
\begin{equation}
\label{eq:parttr}
\begin{aligned}
    &\mathrm{tr}_0 \left(D_N^{(1)}(\lambda_1,\dots,\lambda_N)\right) = \sum_{\alpha_0} \delta_{\alpha_0\beta_0}\,\delta_{\alpha_1\beta_1}\, \left[D_{N}(\lambda_1,\dots,\lambda_N)\right]^{\alpha_0\dots\alpha_N,\beta_0\dots\beta_N} = D_{N-1}(\lambda_2,\dots,\lambda_{N})\,,\\
    &\mathrm{tr}_N \left(D_N^{(N-1)}(\lambda_1,\dots,\lambda_N)\right) = \dots = D_{N-1}(\lambda_1,\dots,\lambda_{N-1})\,.
\end{aligned}
\end{equation}
To prove these relations one uses the fact that $\sum_\alpha T_{\gamma\gamma'}^{\alpha\alpha}(u)\, \mathcal{P}_{\text{per}}\,T^{\gamma\gamma'}_{\dots}(v)= \delta_{\gamma\gamma'}\,t(u)\,T^{\gamma\gamma}_{\dots}(v)$.
...
% &\qquad=\frac{\delta_{\alpha_0\beta_0}\delta_{\alpha_N\beta_N}}{\prod_{i=1}^N \rho(\lambda_i-\lambda_N)\rho(\lambda_N-\lambda_i)}\times\\
% &\qquad\qquad\times\sum_{\{\underline{\bm{\gamma}}\}\{\underline{\bm{\delta}}\}}\delta_{\alpha_{N-1}\gamma_N}\prod_{i=1}^{N-1}
%  W \left(
%    \begin{matrix}
%      \gamma_{i-1} & \gamma_i\\
%      \alpha_{i-1} & \alpha_i
%    \end{matrix}\,\middle|\, 
%    \lambda_N-\lambda_i\right)\,B^{\{\underline{\bm{\gamma}}\}\{\underline{\bm{\delta}}\}}\times\\
% &\qquad\qquad\qquad\qquad\times \prod_{i=1}^{N-1}W \left(
%    \begin{matrix}
%      \delta_{i-1} & \beta_{i-1}\\
%      \delta_{i} & \beta_i
%    \end{matrix}\,\middle|\, 
%    \lambda_i-\lambda_N\right)P_- \left(
%    \begin{matrix}
%      \delta_{N-1} & \beta_{N-1}\\
%      \delta_{N} & \beta_N
%    \end{matrix}\right)\\
...
\bra{\alpha_0\alpha_1\alpha_2} P_-\ket{\beta_0 \beta_1 \beta_2} = \begin{tikzpicture}[baseline=(current bounding box.center)]
  \def \d {0.5 * 1.41421356237}
  \draw (0,0)--(\d,\d)--(2*\d,0)--(\d,-\d)--(0,0);
  ...
  \node at (\d,0) {$P_-$};
\end{tikzpicture} \equiv \delta_{\alpha_0 \beta_0} \delta_{\alpha_2 \beta_2} \, \sqrt{\frac{g_{\alpha_0} {g_{\alpha_2}}}{g_{\alpha_1} g_{\beta_1}}}\, \begin{tikzpicture}[baseline=(current bounding box.center)]
  \def \d {0.5 * 1.41421356237}
  \draw (0,0)--(\d,\d)--(2*\d,0)--(\d,-\d)--(0,0);
  \node [left] at (0,0) {$\alpha_1$};
...
\paragraph{} Based on numerical results for small $r$ and $N$ we have proposed an identity relating the asymptotics of $D_N$ to  $D_{N-1}$ in the topological sectors with $d_q(j)=1$ when $\lambda_N$ is sent to $i\infty$ \cite{FrWe21}:
...
\end{aligned}
\end{equation}
(Recall that $D_1$ is independent of the spectral parameter $\lambda_1$, see (\ref{D1}).)
...
% &\quad
% = \delta_{\alpha_N\alpha_{N-2}}\delta_{\beta_N\beta_{N-2}}
% \bra{\alpha_0..\alpha_{N-2}}D_{N-2}(\lambda_1,\dots,\lambda_{N-2})\ket{\beta_0..\beta_{N-2}} \,\frac{\prod_{k=1}^L\rho(u-u_k)\rho(u_k-u)}{\Lambda(u)\Lambda(u+\lambda)}\\
 &\quad \times
...
Assuming that the factorization (\ref{RSOSfac}) holds for general $\lambda=\pi/r$ we can  apply (\ref{eq:thm2}) to the two-site density matrix $D_2$ of the RSOS model.  This gives rise to a discrete difference equation  satisfied by the scalar function $\omega(u,v)$ for a given value of $r$.  Based on explicit results for $r\leq7$ we propose the following functional equation for $\omega(u,v)$ for $v\in \{u_k\}$ and general values of $\lambda=\pi/r$
\begin{equation}
\label{eq:func}
    \omega(u,v+\lambda)
    %=\frac{\lambda\,\sin\lambda\,\sin3\lambda\,  \tan ^2\lambda}{\pi
    =\frac{\sin^2\lambda}{\cos2 (u-v)-\cos2 \lambda}
    -\frac{\cos (2 (u-v-\lambda ))-\cos (2 \lambda )}{\cos (2 (u-v))-\cos (2 \lambda )}\, \omega(u,v)\,.
\end{equation}
We have checked (\ref{eq:func}) for values of $r$ up to $12$.
Note that, as a consequence of (\ref{eq:asympt}), the function $\omega(u,v)$ vanishes for ${v\to i\infty}$.


%---------------------------------------------------------------------- 
\section{\texorpdfstring{$N$}{N}-site density matrices - the algebraic part}
%---------------------------------------------------------------------- 
%As mentioned above, the factorization of the reduced density matrices takes a particularly simple form in the topological sectors with quantum dimension $d_q(j)=1$, where it is given in terms of a single two-site function and many of the structure functions only depend on $\lambda=\pi/r$.  

%\subsection{The single-site density matrix} 
To begin our analysis of the algebraic part we note that, using the asymptotic relation (\ref{eq:asympt}), the elements of $F_{N;\varnothing,\varnothing}$ for $N\geq2$ can be obtained recursively
\begin{equation}
...
\end{align}
with $F_{2;\varnothing,\varnothing}$ given  in (\ref{mel_aN}). Numerically we find that the matrix $F_{2;(1),(2)}=F_{2;\varnothing,\varnothing}\,B_2$ is also constant (i.e.\ independent of the spectral parameters).  Therefore it can be obtained by utilizing (\ref{eq:thm2}): since all matrix elements of $D_2$ are given in terms of the unknown elements of $B_2$ and the single function $\omega(\lambda_1,\lambda_2)$ this equation determines the former.
%By filling the matrix with every already known element as well as placeholders for the remaining ones, it is possible to gather information about the unknown elements through (\ref{eq:thm2}) for $D_2$. Since every matrix element satisfies this functional equation, we can compare the equations for different matrix elements and solve for the desired factor. %{\color{red}
Based on this process we have computed $B_2$ for $r=4,\dots,12$.
%\begin{equation}
%\label{mel_b2}
%\begin{aligned}
%    &\langle{a\pm 1,a,a\pm 1}|B_2|{a\pm1,a,a\pm1}\rangle = -\langle{a\pm1,a,a\mp1}|B_2|{a\pm1,a,a\mp1}\rangle
%    = \frac{g_{a-1}\,g_{a+1}}{g_1 g_3}
    %\frac{\sin((a-1)\lambda)\,\sin((a+1)\lambda)}{\sin\lambda\,\sin3\lambda}
%    \,,\\
%    &\langle{a,a\pm1,a}|B_2|{a,a\mp1,a}\rangle= 2\,\cos\lambda \frac{g_a\sqrt{g_{a-1}\,g_{a+1}}}{g_1 g_3}
    %frac{\cos\lambda\,\sin a\lambda}{\sin\lambda\,\sin3\lambda}\,\sqrt{\sin((a-1)\lambda)\,\sin((a+1)\lambda)}
%    \,.
%\end{aligned}
%\end{equation}
%}
...
\begin{equation}
\label{eq:B2_TL}
    %D_2(\lambda_1,\lambda_2) = A_2 \left(\mathbf{1}+A_2^{-1}B_2 \,\omega(\lambda_1,\lambda_2) \right)
    %= A_2 \left(\mathbf{1} + 
    B_2=2\left(2\cos\lambda\,e_1-\mathbf{1}\right)\,,
    %\omega(\lambda_1,\lambda_2) \right)
\end{equation}
...
\begin{aligned}
    ...
    &B_2^{[aa]} = \frac2{\sin a\lambda}\,\begin{pmatrix}
      \sin(a-2)\lambda & %\sin((a-2)\lambda) &
      2\cos\lambda\sqrt{\sin(a-1)\lambda\,\sin(a+1)\lambda} \\ %\sin(a-1)\lambda\,\sin(a+1)\lambda}\\
...
\end{aligned}
\end{equation}  
with constant matrices $f^1_{ij}=\left(f^1_{ij}\right)^\top$, $f^2=-\left(f^2\right)^\top$ and $f^4=\left(f^4\right)^\top$.  $f^1_{ij}$ and $f^2$ can be computed using Eq.~(\ref{eq:asympt})
together with the YB relation (\ref{eq:func_eq_ybe}): 
%
%the asymptotics (\ref{eq:asympt}) for $\lambda_j\to i\infty$ leads to additional relations between the constants in (\ref{B3_par}).  S
since $\cot x\to-i$ and $\omega(y,x)\to0$ in the limit $x\to i\infty$ one can express $f^1_{12}$ in terms of $B_2$:
\begin{equation}
\label{eq_red12}
\begin{aligned}
...
%We have also considered several examples to check that the $4$-site reduced density matrix $D_4(\lambda_1,\lambda_2,\lambda_3,\lambda_4)$ allows for a factorisation similar to that in the Heisenberg model (\ref{XXXfac}).
%Using a generalisation of the algorithm presented in \cite{FrWe21} and equation \ref{XXXfac} for the factorisation of the Heisenberg model, we find the following expression for a single element of the density Matrix $D_4$:

%with $f_{i,j}=f_{ij} \,\omega(\lambda_i,\lambda_j)$ and $f_{ij,kl}=f_{ijkl} \,\omega(\lambda_i,\lambda_j) \,\omega(\lambda_k,\lambda_l)$. Where $f_{ij}$ and $f_{ijkl}$ are rational elementary functions of $e^{2i\lambda_{12}}$ , $e^{2i\lambda_{13}}$, $e^{2i\lambda_{14}}$ , $e^{4i\lambda_{12}}$ , $e^{4i\lambda_{13}}$ and $e^{4i\lambda_{14}}$ ($\lambda_{kl}\equiv\lambda_k-\lambda_l$) and $\omega(u,v)$ the nearest neighbour function as introduced above.\\
%Note that equation (\ref{d4fac}) differs from the equation of the Heisenberg model by one missing term. Furthermore, the system of equations was only uniquely solvable for chain lengths $L>2$, e.g. $L=4$. The reason seems to be an identity fulfilled by products of two-point functions for $L=2$. For small system sizes we were able to compute all structure functions using Fourier methods. 

%In this case the matrix elements contain terms bilinear in the nearest neighbour function $f(\lambda_j,\lambda_k)$, see e.g.\ \cite{BoST05,SaST05}. 
%For the constant term the recursion relation (\ref{eq:recursion_A}) gives
%\begin{align}
%    \langle{abcde}|A_4|{ab'c'd'e}\rangle = \delta_{bb'}\delta_{cc'}\delta_{dd'}\frac{\sin a\lambda\,\sin e\lambda}{8r\cos^4\lambda}\,,
%    \quad |a-b|=|b-c|=|c-d|=|d-e|=1\,.
%\end{align}
%In principle it is possible to determine the coefficients of the other contributions to $D_4$ similar as those for $D_3$ above.  Unfortunately, we did not find a way to present them in a compact way.  Clearly, an alternative way to obtain these, e.g.\ via recursion relations, is desirable. 
...
%To obtain a solution for arbitrary values of $v$ we use analytic continuation, i.e.\ shifting the integration contour by $i \delta$ with $\delta\in\mathbb{R}$ gives a solution for $v\in(-\delta,\lambda-\delta)$. 
...
\end{equation}
%\begin{align*}
%    \mathrm{trace} \left(A_2\,e_j\right) &= \frac1{r\cos^2\lambda} \sum_{a=1}^{r-2} \sin a\lambda\,\sin(a+1)\lambda
%    = \frac{1}{2\cos\lambda}\,,\\
%    \mathrm{trace} \left(B_2\,e_j\right) &= \frac2{\sin\lambda\,\sin3\lambda}
%    \sum_{a=1}^{r-3}\sin a\lambda\left(2\sin(a+1)\lambda+\sin(a+3)\lambda \lambda\right)
%    = r\,\frac{\cos\lambda}{\sin^2\lambda}\,.
%\end{align*}
...
% 
In principle, an additional check of (\ref{LHP3}) would be possible by comparison with the trigonometric limit of the corresponding expression for the massive regimes III and IV of the RSOS model from \cite{LuPu96} which are given in terms of a two-fold integral.  While we have not tried this it might also lead to insights on how to factorize the multiple integral representations of $n>3$-point LHPs.
...
\end{aligned}
\end{equation}
%($\partial_n\omega(x_1,x_2)$ denotes the derivative of $\omega$ w.r.t.\ the $n$-th argument at $(x_1,x_2)$).  
%In Table~\ref{tab:p1234} we present the analytical expressions for $P_{1234}$ in the thermodynamic ground state of (\ref{eq:HqRSOS}) for small $r$.
...
%
%\begin{table}
%\caption{\label{tab:p1234}Probability $P_{1234}$ for a segment of four consecutive sites with spins increasing from $1$ to $4$ in the ground state of the infinite length quantum RSOS model (\ref{eq:HqRSOS}).}
%\begin{ruledtabular}
%\begin{tabular}{rll}
%$r$ & $P_{123}$ & $P_{1234}$ \\\hline
%$5$ & $\frac7{20}\sqrt{5}-\frac34$ & $\frac{123}{20}\,\sqrt{5} - \frac{55}{4}$\\ 
%$6$ & $\frac2{216}\,\left(4-\frac{3\,\sqrt{3}}{\pi}\right) $ & %$\frac1{216}\left(2-\frac{3\,\sqrt{3}}{\pi}\right)$ \\
%$7$ & &  \\
%$8$ & & $(5\pi-12)(29\sqrt{2}-41)/(16\pi)$\\
%$9$ & & \\
%$10$ & &
%\end{tabular}
%\end{ruledtabular}
...
\section{The generalized reduced density matrix}
\label{app:DN_graphical}
Using the graphical notation introduced in Eqs.~(\ref{eq:rsos_weights}) and (\ref{eq:def_monodromy}) the generalized reduced density matrix (\ref{eq:def_D}) can be depicted as:
\begin{equation}
 \begin{aligned}
     D_N(\lambda_{1},\dots,\lambda_{N})^{\{\underline{\bm{\alpha}}\}\{\underline{\bm{\beta}}\}} &=
     \frac{\bra{\Phi} \prod_{k=1}^{N}  T^{\alpha_{k-1} \beta_{k-1}}_{\alpha_k \beta_k}(\lambda_k) \ket{\Phi}}
      {{\braket{\Phi}{\Phi}}\,\prod\limits_{k=1}^{N} \Lambda(\lambda_k)}\\
 &=\begin{tikzpicture}[baseline=(current bounding box.center)]
  \def \b {5} %Breite
  \def \c {3} % Länge der vert. Linien
  \def \n {\b/3} %Abstand der vert Linien
 \draw (0,0)--(\b,0);
 \draw (0,-\c/2)--(\b,-\c/2);
  \draw (0,\c/2)--(\b,\c/2);
   \foreach[evaluate=\x as \yeval using \n*\x] \x in {0,1,2,3}
  \draw (\yeval,-\c/1)--(\yeval,\c/2);
  \node [left] at (0,\c/2) {$\alpha_{0}$};
    \node [right] at (\b,\c/2) {$\beta_{0}$};
 \node [left] at (0,0) {$\vdots$};
 \node [left] at (0,-\c/2) {$\vdots$};
  \node [left] at (0,-\c/1) {$\alpha_{N}$};
  \node [right] at (\b,0) {$\vdots$};
 \node [right] at (\b,-\c/2) {$\vdots$};
  \node [right] at (\b,-\c/1) {$\beta_{N}$};
 \draw (0,\c/2)--(\b,\c/2)--(\b/2,\c/2+\c/4)--(0,\c/2);
   \draw (0,-\c/1)--(\b,-\c/1)--(\b/2,-\c/1-\c/4)--(0,-\c/1);
  \node at (\b/2,\c/2+\c/8) {$\Phi$};
  \node at (\b/2,-\c/1-\c/8) {$\Phi$};

    \node  at (\n/2,\c/4) {\scriptsize $\lambda_{1}-u_1$};
    \node  at (\n/2+\n,\c/4) {\dots};
     \node  at (\n/2+\n,-\c/4) {\dots};
     \node  at (\n/2+\n,-9/4) {\dots};
     \node  at (\n/2+2*\n,\c/4) {\scriptsize $\lambda_{1}-u_L$};
     \node  at (\n/2+2*\n,-\c/4) {\scriptsize $\dots$};
    \node  at (\n/2,-\c/4) {\scriptsize$\dots$};
    \node at (\n/2,-9/4){\scriptsize $\lambda_{N}-u_1$};
     \node  at (\n/2+2*\n,-9/4) {\scriptsize $\lambda_{N}-u_L$};
  %  \node[draw,circle,inner sep=1pt,fill] at (0,0) {};
    \node[draw,circle,inner sep=1pt,fill] at (\n,0) {};
    \node[draw,circle,inner sep=1pt,fill] at (2*\n,0) {};
     \node[draw,circle,inner sep=1pt,fill] at (\n,-3/2) {};
      \node[draw,circle,inner sep=1pt,fill] at (2*\n,-3/2) {};
  %  \node[draw,circle,inner sep=1pt,fill] at (\b,0) {};
 
    \end{tikzpicture}\cdot \frac{1}{\braket{\Phi}{\Phi}\prod\limits_{k=1}^{N} \Lambda(\lambda_k)}
 \end{aligned}\,
\end{equation}
where the projection onto the eigenstate $\ket{\Phi}$ of the transfer matrix is indicated by sandwiching of $\prod_{k=1}^N T_{\alpha_k\beta_k}^{\alpha_{k-1}\beta_{k-1}}$.
...
\bibitem [{\citenamefont {Gils}\ \emph {et~al.}(2009)\citenamefont {Gils},
  ...
  {volume} {103}},\ \bibinfo {pages} {070401} (\bibinfo {year} {2009})},\
  \Eprint {http://arxiv.org/abs/0810.2277} {arXiv:0810.2277} \BibitemShut
  {NoStop}%
\bibitem [{\citenamefont {Gils}\ \emph {et~al.}(2013)\citenamefont {Gils},
  ...
  {87}},\ \bibinfo {pages} {235120} (\bibinfo {year} {2013})},\ \Eprint
  {http://arxiv.org/abs/1303.4290} {arXiv:1303.4290} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {Finch}\ and\ \citenamefont {Frahm}(2013)}]{FiFr13}%
  ...
  }\textbf {\bibinfo {volume} {15}},\ \bibinfo {pages} {053035} (\bibinfo
  {year} {2013})},\ \Eprint {http://arxiv.org/abs/1211.4449} {arXiv:1211.4449}
  \BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Levy-Bencheton}\ \emph {et~al.}(2016)\citenamefont
  ...
  {http://arxiv.org/abs/1507.03404} {arXiv:1507.03404} \BibitemShut {NoStop}%
...
\bibitem [{\citenamefont {Aufgebauer}\ and\ \citenamefont
  ...
  {NoStop}%
\bibitem [{\citenamefont {Kl\"umper}\ and\ \citenamefont
  {Pearce}(1992)}]{KlPe92}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
  {Kl\"umper}}\ and\ \bibinfo {author} {\bibfnamefont {P.~A.}\ \bibnamefont
  {Pearce}},\ }\href {\doibase https://doi.org/10.1016/0378-4371(92)90149-K}
  {\bibfield  {journal} {\bibinfo  {journal} {Physica A}\ }\textbf {\bibinfo
  {volume} {183}},\ \bibinfo {pages} {304} (\bibinfo {year}
...
\bibitem [{\citenamefont {Boos}\ \emph {et~al.}(2005)\citenamefont {Boos},
  ...
  {http://arxiv.org/abs/hep-th/0410039} {hep-th/0410039} \BibitemShut {NoStop}%
\bibitem [{\citenamefont {Sato}\ \emph {et~al.}(2005)\citenamefont {Sato},
  ...
  {http://arxiv.org/abs/hep-th/0507290} {hep-th/0507290} \BibitemShut {NoStop}%
