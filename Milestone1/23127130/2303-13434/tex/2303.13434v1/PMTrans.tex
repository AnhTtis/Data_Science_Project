...
%      \end{subfigure}
%      \vspace{-5pt}
%     \captionof{figure}{(a) The classification accuracy of our PMTrans surpasses the SoTA methods by \textbf{+17.7\%} on the most challenging DomainNet dataset. Note that the target tasks treat one domain of DomainNet as the target domain and the others as the source domains. (b) PMTrans builds up the intermediate domain (\textcolor{green(pigment)}{green patches}) via a novel PatchMix module by learning to sample patches from the source (\textcolor{blue}{blue patches}) and target (\textcolor{pink}{pink patches}) domains. PatchMix tries to maximize the CE (\textcolor{green(pigment)}{$\uparrow$}) between the intermediate domain and source/target domain, while other players try to minimize it (\textcolor{red}{$\downarrow$}) by aligning their features.}
%     \vspace{-5pt}
% \label{fig:Intro}
% \end{center}
% }] 
...
\begin{figure}[t]
    ...
    \vspace{-8pt}
    \caption{The classification accuracy of our PMTrans surpasses the SoTA methods by \textbf{+17.7\%} on the most challenging DomainNet dataset. Note that the target tasks treat one domain of DomainNet as the target domain and the others as the source domains.}
...
%      \end{subfigure}
%      \vspace{-5pt}
% \caption{(a)The classification accuracy of our PMTrans surpasses the SoTA methods by \textbf{+17.7\%} on the most challenging DomainNet dataset. Note that the target tasks take one domain of DomainNet as the target domain and other domains as source domains. (b) PMTrans builds up the intermediate domain (\textcolor{green}{green patches}) via a novel PatchMix module by learning to sample patches from the source (\textcolor{blue}{blue patches}) and target (\textcolor{red}{red patches}) domains. PatchMix tries to maximize the CE (\textcolor{green}{$\uparrow$}) between the intermediate domain and source/target domain, while other players try to minimize it (\textcolor{red}{$\downarrow$}) by aligning their features.}
% \label{fig:Intro}
% \vspace{-10pt}
% \end{figure*}
\begin{figure}
...
The results show that the performance of PMTrans significantly surpasses that of the ViT-based \cite{abs-2204-07683,abs-2109-06165,abs-2108-05988} and CNN-based SoTA methods~\cite{NaJCH21,LiXGLWL21,SharmaKC21} by \textbf{+3.6\%} on Office-Home, \textbf{+1.4\%} on Office-31, and \textbf{+17.7\%} on DomainNet (See Fig.~\ref{fig:result_cover}), respectively. 
...
\begin{figure*}[t!]
    ...
    % learnable parameters of Beta distribution (Beta), Patch Embedding Network (Emb) 
...
% Mixup was first proposed by Zhang \etal\cite{ZhangCDL18} to regularize the network to avoid undesirable memorization and sensitivity to adversarial examples. 
% reduce the sensitivity of domain shift
...
% \begin{equation}
% \begin{aligned}
% &\min _{E, D_d, D_s, D_f} L\left(X_s, Y_s\right) \\
% &\min _{E, D_d, D_s} L\left(X_s, Y_s\right)+\lambda L_d\left(X_t\right) \\
% &\min _{D_f} L\left(X_s, Y_s\right)+\lambda (L_d\left(X_s\right)+\max \left(0, m-L_d(X_t)\right)) \\
% &L_d\left(X_t\right)=\left\|(P_s^t-P_f^t)\bigoplus gate \right\|_2^2 .
% \end{aligned}
% \end{equation}
...
% which shares information from both the source domain $\mathcal{D}_{s}$ and the target domain $\mathcal{D}_{t}$.
...
%In detail, let $P_{S}$ and $P_{T}$ be the empirical distributions defined by $\mathcal{D}_{s}$ and $\mathcal{D}_{t}$, respectively. %$D(P_{S},P_{T})$ measures the divergence between the source and target domains, and can be defined as
...
% Let $\mathcal{H}^{s}$, $\mathcal{H}^{t}$, and $\mathcal{H}^{i}$ be a vector space of dimension $\operatorname{dim}(\mathcal{H})$ for the source, target, and intermediate domains, respectively. 
...
%
% For each domain image, the Patch Embedding layer splits it into a token sequence, which serves as the input for transformer blocks. The classifier takes the output of transformer blocks and outputs label predictions. To bridge the source and target main, we construct an intermediate domain to handle the issue of a large domain gap.
...
% \begin{definition}
% (Nash Equilibrium) : A point $\left(w_{i}^{*}, w_{-i}^{*}\right) \in \Omega$ is said to be a Nash Equilibrium of the domain-adversarial game if there exists some $\delta>0$ such that:
% $
% \forall i \in\{1,2,3\}, \quad J_{i}\left(w_{i}^{*}, w_{-i}^{*}\right) \leq J_{i}\left(w_{i}, w_{-i}^{*}\right), \quad \text { s.t. }\left\|\omega_{i}-\omega_{i}^{*}\right\|_{2}<\delta
% $
% \end{definition}

% To clarify the min-max process, we introduce the game's vector field $v(w)$, which is identical to the gradient with respect to every players.
% \begin{definition}
% \label{Def:vector field}
% (Vector field): .
% \[
% v(w) := (\bigtriangledown_{w_1}J_1, \bigtriangledown_{w_2}J_2, \bigtriangledown_{w_3}J_3)
% \]
% \end{definition}
% By examining Definition.\ref{Def:vector field} with respect to Eqn.(\ref{eq:objective}), the process can be categorized into both cooperation and competition\cite{DavidAcuna2022DOMAINAT}. 
% \begin{equation}
%     v(w)=
%     \begin{pmatrix}
%     \bigtriangledown_{w_1}\ell\\
%     \bigtriangledown_{w_2}\ell\\
%     0
%     \end{pmatrix}
%     +
%     \begin{pmatrix}
%     \alpha\bigtriangledown_{w_2}{CE}_{s,i, t}(\omega)\\
%     \alpha\lambda\bigtriangledown_{w_2}{CE}_{s,i, t}(\omega)\\
%     -\bigtriangledown_{w_3}{CE}_{s,i, t}(\omega)
%     \end{pmatrix},   
% \end{equation}
% %
% where the left part related to the gradient of $l(w_1, w_2)$, and the right part denotes the adversarial behavior on producing or consuming CE in the network, where player P compete with both players F and C.
%
%
\begin{definition}
\label{Def:NE}
...
% To clarify the min-max process, we introduce the game's vector field $v(\boldsymbol{\omega})$, which is identical to the gradient \wrt~ each player. Here, $v(\boldsymbol{\omega})$ is defined as:
% \begin{definition}
% \label{Def:vector field}
% (Vector field): .
% \[
% v(w) := (\bigtriangledown_{\boldsymbol{\omega}_1}J_1, \bigtriangledown_{\boldsymbol{\omega}_2}J_2, \bigtriangledown_{\boldsymbol{\omega}_3}J_3)
% \]
% \end{definition}
% By examining Definition~\ref{Def:vector field} \newcommand{\aka}{\textit{a.k.a.}}cuna2022DOMAINAT}. 
%
% \begin{equation}
% \label{eq:vector space}
%     v(\boldsymbol{\omega})=
%     \begin{pmatrix}
%     \bigtriangledown_{{\boldsymbol{\omega}}_1}\ell\left(\boldsymbol{\omega}_{\mathcal{F}}, \boldsymbol{\omega}_{2}\right)\\
%     \bigtriangledown_{{\boldsymbol{\omega}}_2}\ell\left(\boldsymbol{\omega}_{\mathcal{F}}, \boldsymbol{\omega}_{2}\right)\\
%     0
%     \end{pmatrix}
%     +
%     \begin{pmatrix}
%     \alpha\bigtriangledown_{{\boldsymbol{\omega}}_2}{CE}_{s, i,t}(\boldsymbol{\omega})\\
%     \alpha\bigtriangledown_{{\boldsymbol{\omega}}_2}{CE}_{s, i,t}(\boldsymbol{\omega})\\
%     -\bigtriangledown_{{\boldsymbol{\omega}}_3}{CE}_{s, i,t}(\boldsymbol{\omega})
%     \end{pmatrix},   
% \end{equation}
% %
% where the left part is related to the gradient of $\ell(w_1, w_2)$, and the right part denotes the adversarial behavior on maximizing or minimizing CE. Based on the gradient optimization, a NE solution of the game is equivalent to searching the minima for all the players asymptotically.


% In this min-max CE game, each of the three players behaves selfishly to reduce their cost function, and this competition on CE will possibly end with a situation where no one has anything to gain by changing only one's strategy, which is called Nash Equilibrium.  Therefore, a NE solution of the game is equivalent to search the minima for all the players asymptotically.

% Minimizing the cost ${CE}_{s,i, t}(\omega)$ is to decrease the domain discrepancy between the intermediate domain with two domains, which facilitate decreasing the domain divergence between the source and the target domain.
...
% Accordingly, we propose two 
% As PatchMix tries to maximize the CE between the intermediate domain and source/target domain, we need to find a way to minimize the CE in the game.
% To achieve this, we compute the normalized cosine distance between the intermediate domain (for each column), \ie the mixed-up patches and source/target domain (for each row), as shown in Fig. \ref{fig:align-feature}{\color{red}(a)}. The normalized scores in each row denotes the similarity between a sample from the intermediate domain and another from the source/target domain.
% Through Theorem~\ref{theorem:Distribution estimation}, \textit{we conclude that the feature similarity can be equivalent to the label similarity.} 
% Intuitively, for the intermediate and source domains, we define the label similarity $y^{is} = y^s({y^s})^{\intercal}$, as shown in Fig.\ref{fig:align-feature} {\color{red}(b)},
% the diagonals (pink) show samples with the same labels. 
% The value of the diagonal can be used as supervision for both source and target domains. Furthermore, for source domain, we can exploit semantic labels, \eg, the samples with label 'clock' (yellow) to extend its supervision (yellow and pink). 
% %
% Finally, we utilize the CE to measure the domain discrepancy between the feature similarity and label similarity.
% % as shown in Theorem~\ref{theorem:Distribution estimation}.
...
% 
% and  the similarity between the intermediate image and other classes is 0.
...
% the similarity between the intermediate image and other images is 0.
...
%
%  and the two semi-supervised mixup losses are defined as
% \[
%  \mathcal{L}_{f}(\boldsymbol{\omega}_{\mathcal{F}}, \boldsymbol{\omega}_{3}) = \mathcal{L}_{f}^{I,S}(\boldsymbol{\omega}_{\mathcal{F}}, \boldsymbol{\omega}_{3}) + \mathcal{L}_{f}^{I,T}(\boldsymbol{\omega}_{\mathcal{F}}, \boldsymbol{\omega}_{3}), \ 
% \mathcal{L}_{l}(\boldsymbol{\omega}) = \mathcal{L}_{l}^{I,S}(\boldsymbol{\omega}) + \mathcal{L}_{l}^{I,T}(\boldsymbol{\omega}).    
% \]
% %
...
% \textcolor{red}{As such, we can obtain the solution of the game with only one-step optimization, which is more efficient than that proposed in CDTrans~\cite{abs-2109-06165}.}
...
%  the backbone (\eg, ResNet-50, ViT-based) means the models are 
% Due to the data imbalance and noise labels of datasets, 
...
Avg  & 59.9 &25.3 &52.2& 19.6& 65.9 &48.4& \colorbox{lightgray}{45.2}&Avg&60.0& 28.2& 53.3& 13.7& 65.3 &50.4& \colorbox{lightgray}{45.2}&     Avg&67.9&30.7&59.1&27.0&72.8&56.9&\colorbox{lightgray}{\textbf{62.9}}  \\
\bottomrule
\end{tabular}}
\vspace{-8pt}
...
% \hline
% $\mathcal{L}_{cls}^{S}$&$\mathcal{L}_{f}$&$\mathcal{L}_{l}$&A$\to$ C& A$\to$ P & A $\to$ R & C $\to$ A &C $\to$ P &C $\to$ R &P$\to$ A& P$\to$ C & P$ \to$ R & R$ \to$ A &R$ \to$ C &R$ \to$ P& \colorbox{lightgray}{Avg}    \\
% \hline
% \hline
% \checkmark&&&72.7   &87.1   &90.6   &84.3   &87.3   &89.3   &80.6 &68.6 &90.3&84.8&69.4&91.3& \colorbox{lightgray}{83.6} \\
% \checkmark& \checkmark& &73.3&87.2&90.8&84.8&87.5&89.5&81.5&71.1&90.5&85.2&72.9&92.0&\colorbox{lightgray}{83.9}\\
% \checkmark& &\checkmark&79.2&91.8&92.3&88.0&92.6&\textbf{93.0}&87.1&77.8&92.5&88.2&78.4&93.9&\colorbox{lightgray}{87.9}\\
% \hline
%  \checkmark & \checkmark& \checkmark&\textbf{79.7}   &\textbf{92.3}   &\textbf{92.6}&\textbf{88.3}  &\textbf{93.1}&92.8  &\textbf{87.3} &\textbf{80.0}&\textbf{92.8}&\textbf{88.8} &\textbf{79.8} &\textbf{94.6} & \colorbox{lightgray}{\textbf{88.5}}   \\  
% \hline
% \end{tabular}}
% \vspace{-10pt}
% \end{table*}
...
%  \textbf{79.7}   &\textbf{92.3}   &\textbf{92.6}&\textbf{88.3} &\textbf{93.1}&92.8  &\textbf{87.3} &\textbf{80.0}&\textbf{92.8} &\textbf{88.8} &\textbf{79.8} &\textbf{94.6} & \colorbox{lightgray}{\textbf{88.5}}  \\  
\bottomrule
\end{tabular}}
\vspace{-5pt}
\caption{Effect of semi-supervised loss. The best performance is marked as \textbf{bold}.}
...
% &79.7   &\textbf{92.3}   &\textbf{92.6}&88.3  &\textbf{93.1}&\textbf{92.8}  &\textbf{87.3} &\textbf{80.0}&\textbf{92.8} &\textbf{88.8} &\textbf{79.8} &\textbf{94.6} & \colorbox{lightgray}{\textbf{88.5}}   \\ 
\bottomrule
\end{tabular}}
\vspace{-6pt}
...
\end{table*}
\noindent\textbf{Results on DomainNet.} PMTrans achieves a very high average accuracy on the most challenging DomainNet dataset, as shown in Tab. \ref{tab:domainnet}. Overall, our proposed PMTrans outperforms the SoTA methods by \textbf{+17.7\%} accuracy. Incredibly, PMTrans surpasses the SoTA methods in all the \textbf{30 sub-tasks}, which demonstrates the strong ability of PMTrans to alleviate the large domain gap. Moreover, transferring knowledge is much more difficult when the domain gap becomes significant. 
...
% and our proposed intermediate domain successfully address this effect of large domain gaps.
\subsection{Ablation Study}

% In this section, we ablate important design elements in PMTrans on the Office-Home dataset.
% \textbf{PatchMix.}
% Ablations of PatchMix on the Office-Home are shown in Table \ref{Patch_Mixup}. 

\noindent\textbf{Semi-supervised mixup loss.}
% The ablation study for the losses is 
