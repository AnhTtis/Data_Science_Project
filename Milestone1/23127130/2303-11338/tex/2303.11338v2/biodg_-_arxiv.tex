...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\columnwidth]{figures/sresnet_model_rev2_vertical}
	...
	%Once again, as in Figures \ref{fig:backbones} and \ref{fig:resnet18}
...
\begin{itemize}
	\item We introduce a DG evaluation benchmark, namely \emph{BioDG}\footnote{
		The code will be published and publicly available upon acceptance.}, for 12-lead
...
%trained on samples $(\textbf{x}^{(s)}, y^{(s)})$, which is able to generalize on previously unseen data $(\textbf{x}^{(t)}, y^{(t)})$ drawn from N \emph{source} $(N\geq1)$ and K $(K\geq1)$ \emph{target} domains respectively.
%the target domains $\{T_1, T_2, \dotsc, T_K\}$.
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=\linewidth, ]{figures/backbones_rev3}
...
\begin{figure*}[htbp]
	...
	\caption{Illustration of the proposed BioDG Benchmark, for ECG and EEG classification.
		Each dataset is split into Source and Target domains. The trained models are then evaluated on both held-out intra-distribution (drawn from Source domains) and OOD (drawn from Target domains) data.}
	\label{fig:benchmark}
\end{figure*}
...
\begin{figure*}[htbp]
	\centering
	\includegraphics[width=\linewidth, ]{figures/dg_resnet18_rev}
...
\begin{figure}
	\centering
	\includegraphics[width=\columnwidth, ]{figures/convergence}
	\caption{Plots of the training and validation losses of all methods for the BioDG experimental setup. Both training and validation data originate from the \textit{Source} Domains. On the top plots, we illustrate convergence analysis for the ECG datasets for all models utilizing both the ResNet-18 and S-ResNet backbone networks. The bottom plots depict the train and validation loss progression for the EEG classification task. As in Table \ref{tab:eeg_results}, the labels on the right side of the EEG analysis indicate the source domains on which each model was trained on.}
	\label{fig:convergence}
\end{figure}

%\begin{figure*}
%	\centering
%	\includegraphics[width=\linewidth, ]{figures/eeg_convergence_para}
%	\caption{Plots of the training and validation losses of all methods for the EEG classification setting. The labels above each plot indicate the EEG domains on which the models were trained on.}
%	\label{fig:eeg_convergence}
%\end{figure*}

\begin{figure}[h]
	\centering
	\includegraphics[width=\columnwidth, ]{figures/tsne_eeg3}
...
%\begin{figure*}[hbp]
%	\centering
%	\includegraphics[width=\linewidth, ]{figures/EEG Convergence}
%	\caption{Convergence analysis of the proposed BioDG method on the intra-distributions of the EEG SEED datasets. The titles above each plot indicate the data on which the models were trained on. We plot the training and validation losses for each epoch.}
%	\label{fig:eeg_convergance}
%\end{figure*}
...
are depicted in Table \ref{tab:eeg_results}. For our experiments, we also provide convergence analysis plots (Fig. \ref{fig:convergence}) of the the training and validation losses for all models in both settings, along with t-SNE feature vector visualizations for the EEG biosignal classification task\footnote{We select to provide t-SNE visualizations only for the the EEG data as the ECG signals are multi-labeled and only a very small subset of them hold a single label and can therefore be attributed to a single feature cluster.}. The experimental results are discussed in the following section.
	...
	\begin{center}
		...
		\begin{adjustbox}{width=1\linewidth}
			\begin{tabular}{|c|c|c|c|c|}
				...
				%IRM   & 12.49 & 10.3  &  & 1.755  \\
				%MMD   & 12.28 & 10.3  &   & 1.755  \\
				%RSC   & 12.32 & 10.3  &  & 1.755  \\
				%CORAL & 12.29 & 10.3  &  & 1.755  \\
