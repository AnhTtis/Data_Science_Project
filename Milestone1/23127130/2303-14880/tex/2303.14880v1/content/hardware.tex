...
\subsection{Stereo and Depth Camera}
...
\subsection{360\textdegree~Camera}
In addition to the forward facing stereo and depth camera, we also collect 360\textdegree~RGB video to provide better situational awareness of the surrounding. The 360\textdegree~vision also includes all possible sensory information that can be provided by active pan-tilt cameras onboard many mobile robot platforms. 
We use a Kodak Pixpro Orbit360 4K VR Camera, mounted on top of the 3D LiDAR, to collect 360\textdegree~images. The camera has a very compact form factor with two lenses integrated in one camera body to provide spherical 360\textdegree~view and does not introduce extra weight compared with other bulky 360\textdegree~cameras. Note that due to software limitations the camera's webcam mode does not allow both lenses to stream live video to the laptop, so we save the spherical 360\textdegree~view from both lenses to an SD card in the camera. 
...
% \subsection{Mounting}
% 3D Printing and maybe add a 3D blueprint of the design
