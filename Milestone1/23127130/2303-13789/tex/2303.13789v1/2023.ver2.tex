...
%abbreviation of jounal name
\DeclareSourcemap{
    \maps[datatype=bibtex, overwrite=true]{
        \map{
            \step[fieldsource=journal,
                match=\regexp{Acta\sPolytechnica },
                replace={Acta Polytech.}]
            \step[fieldsource=journal,
                match=\regexp{Advances\sin\sTheoretical\sand\sMathematical\sPhysics},
                replace={Adv.Theor.Math.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Annals\sof\sPhysics},
                replace={Annals Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Communications\sin\sMathematical\sPhysics},
                replace={Commun.Math.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Fortschritte\sder\sPhysik},
                replace={Fortsch.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{International\sJournal\sof\sGeometric\sMethods\sin\sModern\sPhysics},
                replace={Int.J.Geom.Meth.Mod.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{International\sJournal\sof\sModern\sPhysics\sA},
                replace={Int.J.Mod.Phys.A}]
            \step[fieldsource=journal,
                match=\regexp{Journal\sof\sHigh\sEnergy\sPhysics},
                replace={JHEP}]
            \step[fieldsource=journal,
                match=\regexp{Journal\sof\sthe\sKorean\sPhysical\sSociety},
                replace={J.Korean Phys.Soc.}]
            \step[fieldsource=journal,
                match=\regexp{Journal\sof\sMathematical\sPhysics},
                replace={J.Math.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Letters\sin\sMathematical\sPhysics},
                replace={Lett.Math.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Modern\sPhysics\sLetters\sA},
                replace={Mod.Phys.Lett.A}]
            \step[fieldsource=journal,
            ...
            \step[fieldsource=journal,
                ...
                replace={Phys.Rev.D}]
            \step[fieldsource=journal,
                match=\regexp{Proceedings\sof\sScience},
                replace={PoS}]
            \step[fieldsource=journal,
                match=\regexp{Progress\sof\sTheoretical\sPhysics},
                replace={Prog.Theor.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Progress\sof\sTheoretical\sPhysics\sSupplement},
                replace={Prog.Theor.Phys.Suppl.}]
            \step[fieldsource=journal,
                match=\regexp{Progress\sof\sTheoretical\sand\sExperimental\sPhysics},
                replace={PTEP}]
            \step[fieldsource=journal,
                match=\regexp{Proceedings\sSupplements},
                replace={Proc.Suppl.}]
            \step[fieldsource=journal,
                match=\regexp{Reviews\sin\sMathematical\sPhysics},
                replace={Rev.Math.Phys.}]
            \step[fieldsource=journal,
                match=\regexp{Theoretical\sand\sMathematical\sPhysics},
                replace={Theor.Math.Phys.}]
            }
        }
    }
%------------------------------
%input bibliography
\bibliography{SFT.bib,SFT(review).bib}
...
\title{\textbf{Energy from Ellwood invariant for solutions involving $X^0$ variables}}
\author{Yuji Ando\thanks{E-mail: ando@het.ph.tsukuba.ac.jp}\quad and Tomoya Suda\thanks{E-mail: suda@het.ph.tsukuba.ac.jp}\\
\\
\textit{Degree Programs in Pure and Applied Sciences,}\\
\textit{Graduate School of Science and Technology, University of Tsukuba,}\\
\textit{Tsukuba, Ibaraki 305-8571, Japan}}
\date{}
...
\begin{align}
    \Psi=\sqrt{F_{11}}c\frac{B}{H_1}c\sqrt{F_{21}}\qc H_1=\frac{1-F_{11}}{K}\qand F_{11}=F_{12},
\end{align}
and ghost brane solution \cite{Masuda:2012kt} is given by
\begin{align}
    \Psi=\sqrt{F}_{11}c\frac{B}{H_1}c\sqrt{F_{12}}+\sqrt{F}_{21}c\frac{B}{H_2}c\sqrt{F_{22}}\qc H_i=\frac{1-F_{1i}}{K}\qand F_{1i}=F_{2i}\qand F_{12}F_{21}=0.
\end{align}
...
\begin{align}
    \frac{1}{2}\Tr(\Psi_\mathrm{sol}Q\Psi_\mathrm{sol})&=\Tr((\mathcal{G}\Psi_\mathrm{sol})\Psi_\mathrm{sol})-\frac{1}{2}\Tr(\Psi_\mathrm{sol}\comm{Q}{\mathcal{G}}\Psi_\mathrm{sol}),\label{cond:1a}\\
...
\begin{align}
    \chi\coloneqq\lim_{(\Lambda,\delta)\to(\infty,0)}\qty(\int_{i\delta}^{i\Lambda}\frac{\dd{z}}{2\pi i}4\partial X^0(z)\bar{c}\bar{\partial}X^0(\bar{z})-\int_{-i\delta}^{-i\Lambda}\frac{\dd{\bar{z}}}{2\pi i}4\bar{\partial}X^0(\bar{z})c\partial X^0(z)+\frac{c(0)}{2\pi\delta}).
\end{align}
...
\end{align}
Therefore the energy is proportional to the Ellwood invariant.
...
\begin{align}
    ...
    &\mathcal{C}_\mathrm{I}(L,\Lambda,\delta)\coloneqq\int_{P_{L,\Lambda,\delta}}\frac{\dd{z}}{2\pi i}4\partial X^0(z)\bar{c}\bar{\partial}X^0(\bar{z})-\int_{\bar{P}_{L,\Lambda,\delta}}\frac{\dd{\bar{z}}}{2\pi i}4\bar{\partial}X^0(\bar{z})c\partial X^0(z),\\
    &\mathcal{C}_{\mathrm{I}\hspace{-1.2pt}\mathrm{I}\mathrm{A}}(L,\Lambda,\delta)\coloneqq-2\qty(c\partial X^0(i\infty)+\bar{c}\bar{\partial}X^0(-i\infty))\qty(\int_{P_{L,\Lambda,\delta}}\frac{\dd{z}}{2\pi i}\partial X^0(z)-\int_{\bar{P}_{L,\Lambda,\delta}}\frac{\dd{\bar{z}}}{2\pi i}\bar{\partial}X^0(\bar{z})),\\
    &\mathcal{C}_{\mathrm{I}\hspace{-1.2pt}\mathrm{I}\mathrm{B}}(L,\Lambda,\delta)\coloneqq\int_{P_{L,\Lambda,\delta}}\frac{\dd{z}}{2\pi i}\frac{1}{2}\partial^{2}c-\int_{\bar{P}_{L,\Lambda,\delta}}\frac{\dd{\bar{z}}}{2\pi i}\frac{1}{2}\bar{\partial}^{2}\bar{c}+\int_{P_{L,\Lambda,\delta}}\dd{z}\partial\kappa(z,\bar{z})+\int_{\bar{P}_{L,\Lambda,\delta}}\dd{\bar{z}}\bar{\partial}\kappa(z,\bar{z}),
\end{align}
with
\begin{align}
...
\begin{align}
    \mathcal{T}_\mathrm{I}&=\lim_{(\Lambda,\delta)\to(\infty,0)}\left[\ev{\psi(L+L')\int_{i\delta}^{i\Lambda}\frac{\dd{z}}{2\pi i}4\partial X^0(z)\bar{c}\bar{\partial}X^0(\bar{z})\psi(L)}_{C_{L+L'}}+\ev{\psi(L+L')\psi(L)\int_{i\delta}^{i\Lambda}\frac{\dd{z}}{2\pi i}4\partial X^0(z)\bar{c}\bar{\partial}X^0(\bar{z})}_{C_{L+L'}}\right.\\
    &\qquad\left.+\ev{\psi(L+L')\int_{-i\delta}^{-i\Lambda}\frac{\dd{\bar{z}}}{2\pi i}4\bar{\partial}X^0(\bar{z})\partial X^0(z)\psi(L)}_{C_{L+L'}}+\ev{\psi(L+L')\psi(L)\int_{-i\delta}^{-i\Lambda}\frac{\dd{\bar{z}}}{2\pi i}4\bar{\partial}X^0(\bar{z})\partial X^0(z)}_{C_{L+L'}}\right].
\end{align}
...
Hence we obtain
\begin{align}
    \ev{\psi(L+L')\mathcal{C}_{\mathrm{I}\hspace{-1.2pt}\mathrm{I}\mathrm{B}}(L,\Lambda,\delta)\psi(L)}_{C_{L+L'}}&=-\ev{\psi(L+L')\psi(L)\qty(\frac{1}{4\pi i}(\partial c(i\delta)-\bar{\partial}c(-i\delta))+\kappa(i\delta,-i\delta))}_{C_{L+L'}}\\
...
\begin{align}
    \mathcal{A}_2&=\int_0^\infty\dd{L'}\int_0^\infty\dd{L}\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'}}\Tr(e^{-L'K}\psi(L')\partial_L(e^{-LK}\alpha(L)))\\
    &\qquad+\int_0^\infty\dd{L'}\int_0^\infty\dd{L}\eval{\partial_t\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'+t}}}_{t=0}\Tr(e^{-L'K}\psi(L')e^{-LK}\alpha(L))\\
    &=\int_0^\infty\dd{L'}\int_0^\infty\dd{L}\qty(\eval{\partial_t\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'+t}}}_{t=0}-\partial_L\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'}})\Tr(e^{-L'K}\psi(L')e^{-LK}\alpha(L)))\\
    &\qquad+\int_0^\infty\dd{L'}\eval{\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'}}\Tr(e^{-L'K}\psi(L')e^{-LK}\alpha(L))}_{L=0}^{L=\infty}.
\end{align}
With the help of \eqref{correlation:G}, we can derive the following.
\begin{align}
    \eval{\partial_t\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+t+1}}}_{t=0}-\partial_L\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'}}=-\frac{1}{L+1}
\end{align}
...
\begin{align}
    \Psi&=\int_0^\infty\dd{L}e^{-LK}\psi(L),\\
    &\psi(L)\coloneqq\sum_i\int\dd{L_{1i}dL_{21}dL_{3i}dL_{4i}dL_{5i}}\delta(L-L_{1i}-L_{21}-L_{3i}-L_{4i}-L_{5i})\\
...
\begin{gather}
    \Psi_\mathrm{int}=\sqrt{F_{11}}c\sqrt{F_{21}}G_{11}c\frac{B}{H_1}G_{21}\sqrt{F_{21}}c\sqrt{F_{11}}+\sqrt{F_{12}}c\sqrt{F_{22}}G_{12}\frac{B}{H_2}G_{22}\sqrt{F_{22}}c\sqrt{F_{12}},\\
    G_{11}=G_{21}=F_{21}=1\qc F_{11}=F_{12}=H_1=\frac{1}{1+K}\qc F_{22}=(1+K)^2\qc H_2=-(1+K)\qc G_{12}=\sigma\qc G_{22}=\bar{\sigma},
\end{gather}
where $\sigma,\bar{\sigma}$ are defined as an infinitesimally thin strip with the respectively operators insertion by
\begin{align}
    \sigma=\sigma_*e^{i\sqrt{h}X^0}\qc\bar{\sigma}=\bar{\sigma}_*e^{-i\sqrt{h}X^0},
\end{align}
...
In this section, we confirm
\begin{align}
    \mathcal{A}_1+\mathcal{A}_2=\Tr(\Psi(\chi-\chi^\dag)\Psi),
\end{align}
for the solution \eqref{def:KBcmatterstring}. In appendix \ref{cond:1}, it is given that \eqref{cond:1a} and \eqref{cond:1b} are not problematic for this case also.
\subsection{Evaluation of \texorpdfstring{$\mathcal{A}_1$}{A1}}
...
\begin{align}
    ...
    &=\lim_{\delta\to0}\qty(\ev{g_z(i\delta,-i\delta)}_{C_{L+L'}}\ev{c(i\delta)e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_{L+L'}}-\ev{g_{\bar{z}}(i\delta,-i\delta)}_{C_{L+L'}}\ev{\bar{c}(-i\delta)e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_{L+L'}}).
\end{align}
...
\begin{align}
    \mathcal{A}_1=\int_0^\infty\dd{L'}\int_0^\infty\dd{L}\Tr(e^{-L'K}\psi(L')\qty(\chi e^{-LK}\psi(L)+e^{-LK}\psi(L)\chi+\frac{1}{1+L}e^{-LK}\alpha(L))).
\end{align}
\subsection{Evaluation of \texorpdfstring{$\mathcal{A}_2$}{A2}}
...
\begin{align}
    &\eval{\partial_t\ev{\psi(L+L')\mathcal{G}(L,\Lambda,\delta)e^{-tK}\alpha(L+t)}_{C_{L+L'+t}}}_{t=0}\\
    &=\eval{\partial_t\qty(\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'+t}}\ev{\psi(L+L')e^{-tK}\alpha(L+t)}_{C_{L+L'+t}})}_{t=0}\\
    &\quad+\sum_{i,n}\int\dd{L_{1n}dL_{2n}dL_{3n}dL_{4n}dL_{5n}dL_{1i}dL_{2i}dL_{3i}dL_{4i}dL_{5i}}\\
    &\quad\quad\times f_{1n}(L_{1n})f_{2n}(L_{2n})h_n(L_{3n})f_{3n}(L_{4n})f_{4n}(L_{5n})f_{1i}(L_{1i})f_{2i}(L_{2i})h_i(L_{3i})f_{3i}(L_{4i})f_{4i}(L_{5i})\\
    &\quad\quad\times\partial_t\left(\Delta(L_{1i}+L_{2i}+L_{3i}+L_{4i}+L_{5i},\Lambda,\delta,s+t)\right.\\
    &\quad\quad\quad\times\eval{\left.\Tr(e^{-L_{1n}K}ce^{-L_{2n}K}G_{1n}e^{-L_{3n}K}BG_{2n}e^{-L_{4n}K}ce^{-L_{5n}K}e^{-tK}e^{-L_{1i}K}ce^{-L_{2i}K}G_{1i}e^{-L_{3i}K}G_{2i}e^{-L_{4i}K}ce^{-L_{5i}K})\right)}_{t=0},
\end{align}
where $\Delta$ id defined by \eqref{def:Delta}. The first term can be written by
\begin{align}
    \eval{\partial_t\qty(\ev{\mathcal{G}(L,\Lambda,\delta)}_{C_{L+L'+t}}\ev{\psi(L+L')e^{-tK}\alpha(L+t)}_{C_{L+L'+t}})}_{t=0}=-\int_0^\infty\dd{L'}\int_0^\infty\dd{L}\frac{1}{1+L}\Tr(e^{-L'K}\psi(L')e^{-LK}\alpha(L))
\end{align}
...
\begin{align}
    &\Tr(e^{-L_{1n}K}ce^{-L_{2n}K}G_{1n}e^{-L_{3n}K}BG_{2n}e^{-L_{4n}K}ce^{-L_{5n}K}e^{-tK}e^{-L_{1i}K}ce^{-L_{2i}K}G_{1i}e^{-L_{3i}K}G_{2i}e^{-L_{4i}K}ce^{-L_{5i}K})\\
    &=\frac{(s+t)^2}{4\pi^3}\left((L_{1n}+L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i}+L_{5i})\sin\frac{2\pi(L_{2i}+L_{3i}+L_{4i})}{s+t}\right.\\
    &\qquad+(L_{2i}+L_{3i}+L_{4i})\sin\frac{2\pi(L_{1n}+L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i}+L_{5i})}{s+t}\\
    &\qquad-(L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i})\sin\frac{2\pi(L_{1n}+L_{2i}+L_{3i}+L_{4i}+L_{5i})}{s+t}\\
    &\qquad-(L_{1n}+L_{2i}+L_{3i}+L_{4i}+L_{5i})\sin\frac{2\pi(L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i})}{s+t}\\
    &\qquad+(L_{5n}+t+L_{1i})\sin\frac{2\pi(L_{1n}+L_{5i})}{s+t}\\
    &\qquad\left.+(L_{1n}+L_{5i})\sin\frac{2\pi(L_{5n}+t+L_{1i})}{s+t}\right)\\
    &\times\Tr(e^{-(L_{1n}+L_{2n})K}G_{1n}e^{-L_{3n}K}G_{2n}e^{-(L_{4n}+L_{5n}+t+L_{1i}+L_{2i})K}G_{1i}e^{-L_{3i}K}G_{2i}e^{-(L_{4i}+L_{5i})K}).
\end{align}
On the other hand, we cannot evaluate $\Delta$. Hence it is not clear whether \eqref{cond:2} does not hold for the solution. If $\Delta$ does not vanishes, the difference between the energy and the Ellwood invariant is given by
\begin{align}
    &-S[\Psi_\mathrm{sol}]-\Tr(\mathcal{V}\Psi_\mathrm{sol})\\
    &=\sum_{i,n}\int\dd{L_{1n}dL_{2n}dL_{3n}dL_{4n}dL_{5n}dL_{1i}dL_{2i}dL_{3i}dL_{4i}dL_{5i}}\\
    &\qquad\times f_{1n}(L_{1n})f_{2n}(L_{2n})h_n(L_{3n})f_{3n}(L_{4n})f_{4n}(L_{5n})f_{1i}(L_{1i})f_{2i}(L_{2i})h_i(L_{3i})f_{3i}(L_{4i})f_{4i}(L_{5i})\\
    &\qquad\times\partial_t\left(\Delta(L_{1i}+L_{2i}+L_{3i}+L_{4i}+L_{5i},\Lambda,\delta,s+t)\right.\\
    &\qquad\qquad\times\frac{(s+t)^2}{4\pi^3}\left((L_{1n}+L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i}+L_{5i})\sin\frac{2\pi(L_{2i}+L_{3i}+L_{4i})}{s+t}\right.\\
    &\qquad+(L_{2i}+L_{3i}+L_{4i})\sin\frac{2\pi(L_{1n}+L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i}+L_{5i})}{s+t}\\
    &\qquad-(L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i})\sin\frac{2\pi(L_{1n}+L_{2i}+L_{3i}+L_{4i}+L_{5i})}{s+t}\\
    &\qquad-(L_{1n}+L_{2i}+L_{3i}+L_{4i}+L_{5i})\sin\frac{2\pi(L_{5n}+t+L_{1i}+L_{2i}+L_{3i}+L_{4i})}{s+t}\\
    &\qquad+(L_{5n}+t+L_{1i})\sin\frac{2\pi(L_{1n}+L_{5i})}{s+t}\\
    &\qquad\qquad\left.+(L_{1n}+L_{5i})\sin\frac{2\pi(L_{5n}+t+L_{1i})}{s+t}\right)\\
    &\qquad\left.\times\Tr(e^{-(L_{1n}+L_{2n})K}G_{1n}e^{-L_{3n}K}G_{2n}e^{-(L_{4n}+L_{5n}+t+L_{1i}+L_{2i})K}G_{1i}e^{-L_{3i}K}G_{2i}e^{-(L_{4i}+L_{5i})K})\right).
\end{align}
\section{Summary\label{summary}}
We examine condition \eqref{cond:2} for the solution which is constructed by $K,B,c$ and matter operators involving $X^0$. As a result, we obtain that $X^0$ presents an obstruction. If the solution involves $X^0$, we need to calculate $\Delta$. Hence \eqref{cond:2} may not hold for the solution. Because we confirm that \eqref{cond:1a} and \eqref{cond:1b} are not problematic in appendix \ref{cond:1}, if we can evaluate $\Delta$, it will be clear whether the energy is proportional to the Ellwood invariant. Unfortunately, $\Delta$ depends on $X^0$ included in the solution and we cannot evaluate $\Delta$. Thus at present, it is not clear whether the energy is proportional to the Ellwood invariant.
...
The authors would like to thank Nobuyuki Ishibashi for reading a preliminary draft and helpful comments.
...
\end{align}
In general, correlation functions of $X$ variables can be evaluated from above result. We give two important correlation functions
\begin{align}
    &\ev{X^\mu(z')X^\nu(z)e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s}\\
    &=\eval{\frac{1}{i}\pdv{k'_\mu}\frac{1}{i}\pdv{k_\nu}\ev{e^{ik'\cdot X(z')}e^{ik\cdot X(z)}e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s}}_{k=k'=0}\\
    &=-\qty[\sum_{\substack{1\le m\le n\\1\le o\le n}}(k_m)^\mu(k_o)^\nu\ln(\frac{\sin\frac{\pi}{s}(z'-z_m)}{\cos\frac{\pi}{s}z'\cos\frac{\pi}{s}z_m})\ln(\frac{\sin\frac{\pi}{s}(z-z_o)}{\cos\frac{\pi}{s}z\cos\frac{\pi}{s}z_o})+\eta^{\mu\nu}\ln(\frac{\sin\frac{\pi}{s}(z'-z)}{\cos\frac{\pi}{s}z'\cos\frac{\pi}{s}z})]\ev{e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s},\\
    &\ev{\partial X^\mu(z')\partial X^\nu(z)e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s}\\
    &=\pdv{z'}\pdv{z}\ev{X^\mu(z')X^\nu(z)e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s}\\
...
\begin{align}
    ...
    &\qquad-\frac{\pi}{s}\sum_{\substack{1\le m\le n\\1\le o\le n}}(k_m)^\mu(k_o)^\nu\ev{e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s}\\
    &\qquad\qquad\times\qty(\tan\frac{\pi}{s}z+\cot\frac{\pi}{s}(z-z_o))\ln(\frac{-\cos\frac{2\pi}{s}(\Re z-z_m)+\cos\frac{2\pi i}{s}\Im z}{\cos\frac{2\pi}{s}\Re z+\cos\frac{2\pi i}{s}\Im z}\frac{\cos\frac{2\pi}{s}\Re z_0+\cos\frac{2\pi i}{s}\Im z_0}{-\cos\frac{2\pi}{s}(\Re z_0-z_m)+\cos\frac{2\pi i}{s}\Im z_0}).
\end{align}
Since for
\begin{align}
...
\begin{align}
    ...
    &=\ev{(X^\mu(z,\bar{z})-X^\mu(i\infty,-i\infty))\partial X^\nu(z)}_{C_s}\ev{e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s}\\
    &\qquad-\frac{\pi}{s}\sum_{\substack{1\le m\le n\\1\le o\le n}}(k_m)^\mu(k_o)^\nu\qty(\tan\frac{\pi}{s}z+\cot\frac{\pi}{s}(z-z_o))\ln(\frac{-\cos\frac{2\pi}{s}(\Re z-z_m)+\cos\frac{2\pi i}{s}\Im z}{\cos\frac{2\pi}{s}\Re z+\cos\frac{2\pi i}{s}\Im z})\ev{e^{ik_1\cdot X(z_1)}\dots e^{ik_n\cdot X(z_n)}}_{C_s},
\end{align}
and
\begin{align}
...
Since for
\begin{align}
    \tan\frac{\pi}{s}(x+iy)+\cot\frac{\pi}{s}(x+iy-z_o)&=-4ie^{\pi i(2x-z_o)/s}\cos(\frac{\pi}{s}z_o)e^{-2\pi y/s}+\order{e^{-4\pi y/s}},\\
...
Finally, using the above results, we give the correlation function of $\mathcal{G}$ and plane wave vertex operators
\begin{align}
...
\end{align}
On the other hand, the vertical part of the integration does not vanish.
...
\begin{figure}
    ...
    \begin{tikzpicture}[baseline=0.5cm]
        ...
        %operator
        \fill ({1.4*cos(pi*(1/2+0.02) r)},{0.4*sin(pi*(1/2+0.02) r)}) circle (2pt) node[above left] {$\small \Psi_1$};
        %operator
