...
\par\vfill\noindent
{\bf Keywords:} Deep neural networks, intractable likelihoods, sequential, time-series
\par\medskip\noindent
...
Several works have proposed procedures for designing summary statistics \citep{fearnhead2012constructing, jiang2017learning}.

%\paragraph{Parameter estimation using NN} 
Despite its popularity, ABC is not scalable to large numbers of observations, since
%inefficiency in high dimensional problems since the simulated samples needs to explore the entire parameter space; 
%loss in information and statistical power due to the reduction of the data to low-dimensional summary statistics about the parameters; and finally 
inference for new data requires repeating most steps of the procedure. %, so the `5method is not scalable to large numbers of observations.
To address this issue, recent progress in deep learning capabilities, such as the integration of advanced automatic differentiation and probabilistic programming within the modeling workflow, %can significantly improve these aspects.
%These developments 
were used, e.g., to approximate density ratios proportional to the likelihood estimation using a classifier in \citet{hermans2020likelihood}, parameter estimation in Gaussian processes \citep{gerber2021fast} and in multivariate extremes \citep{lenzi2021neural, sainsbury2022fast}.
Specifically for parameter estimation of spatial covariance functions, \cite{gerber2021fast} used maximum likelihood estimators (MLEs) for training convolutional neural networks (CNNs).
%The idea is to train deep neural networks (DNNs) to learn the mapping between data from model simulations and model parameters.
...
Alternatively, \cite{lenzi2021neural} avoided the shortcoming of using MLEs and proposed to computed parameter estimates from approximate likelihood methods and used those to design training data for the CNN.
...
In such applications, where early access to results may carry a premium, our deep neural network (DNN)  estimator is particularly advantageous. The network is trained ahead of time, and estimates are obtained instantaneously when new data becomes available. 
...
We leverage our statistical knowledge about scaling data and parameters to effectively train the DNNs by reducing computational costs and improving convergence. %We always aim for properties such as constant Fisher information, unbounded parameter space, and distribution close to Gaussian. 
Our approaches take advantage of modern and powerful computational resources for DNNs, such as graphical processing units (GPUs). At the same time, unlike classical deep learning for prediction, they preserve the statistical model and parameter interpretability. 
...
The remainder of this paper is organized as follows. First, in Section~\ref{sec:parm-est}, we outline the methodologies we develop for designing training data to train DNNs, along with some practical considerations. 
In Section~\ref{sec:iter}, we introduce the construction of the automatic iterative approach and conduct simulation studies for Gaussian i.i.d and spatial extremes model, whereas in Section~\ref{sec:ts} we outline our unified database approach applied to time series data from an intractable model.
In Section~\ref{sec:conclusion}, we conclude and outline avenues for future research.
...
To describe such a process, it is common practice to assume a statistical model $\mathbb{P}_{\boldsymbol{\theta}_0} \in \mathcal{P}(\mathcal{X})$ with probability density function $p(\cdot; \boldsymbol{\theta}_0)$ parameterized by a finite number of parameters $\boldsymbol{\theta}_0 \subset \Theta \in \mathbb{R}^P$. 
To characterize this distribution, one needs to estimate $\boldsymbol{\theta}_0$ using the observations through the log-likelihood function $l(\boldsymbol{\theta}_0; \mathbf{x}_0) \equiv \mbox{log}\{p(\mathbf{x}_0; \boldsymbol{\theta}_0)\}$.
...
Recently, parameter estimation using DNNs have opened doors to solving previously intractable statistical estimation problems (see e.g., and \citet{lenzi2021neural} and \citet{sainsbury2022fast}).
As shown in these papers, the key to efficiency is to avoid altogether learning likelihood functions and directly learn the mapping between data and parameters through DNNs by carrying out simulations.
...
\begin{enumerate}[label=(\Alph*)]
    \item {\textit{A fully automatic iterative approach:}}
        ...
        After a few iterations, we reach our goal, and the training data and the bootstrapping samples are concentrated around the true parameters.
...
(1) approaches the MLE even when the initial training data is relatively far from the actual value;
(2) learns independence in the data by permuting i.i.d samples
(3) reaches the truth quicker when using meaningful parametrizations and orthogonal parameters.
...
We consider an MLP for $\mathcal{F}_{\phi}$ taking output in $\mathbb{R}^P$ and input in $\mathbb{R}^J$ (see line 5 of Algorithm~\ref{alg2}).
...
When the estimates are close to stabilizing, and the uncertainty has decreased, the number of samples in the training data is set to increase by $5\%$ (see line 9 of Algorithm~\ref{alg2}). 
...
We set $L=10$ and $N=1000$ and uniformly generate training output samples $\{\theta_n\}_{n=1}^{N} \sim {\mbox{Unif}}(-2, 1)$ and corresponding inputs $\{\mathbf{x}_{n}\}_{n=1}^{N} \sim N\{1, \mbox{exp}(\theta_n)\}$, where the elements in the later are permuted $L=10$ times for each $n$.
...
The left panel in this figure shows that after five iterations, the algorithm approaches the MLE (green dashed line) with low uncertainty (see narrow blue boxes).
To quantify the appropriateness of our method and as a calibration measure, we compare $95\%$ central intervals from the bootstrapping estimates with the same interval from the empirical variance in the data. 
 ...
 \begin{figure}[htb!]
	\centering
			\begin{subfigure}[b]{0.8\textwidth}
			 %     			\caption*{\scriptsize{(c) Transformed smoothness}}
			%	~~~~~ \caption{}
		%\includegraphics[width=5.7cm, height=5cm]{Figures/iter_logvarGauss_N20_ntrain1000_nboot100_lopars-2to1_logtest1_crit05_inc_per.png}
      	\includegraphics[width=0.5\textwidth]{iter_logvarGauss_N20_ntrain500_nboot1000_lopars-05to05_logtest1_crit05_incut.png}
      	\includegraphics[width=0.5\textwidth]{95ic_addstep_ntrain100_nlen20_nboot100_ct20.png}
			%\includegraphics[width=5.5cm, height=4.5cm]{Figures/95ic_addstep_ntrain50_nlen20_nboot10k_ct6.png}
	\end{subfigure}
	\caption{Results from estimating $\mbox{log}(\sigma_0^2) = 1$ from a zero-mean Gaussian distributed sample of size $J=20$ using Algorithm~\ref{alg2}.
    The left plot shows boxplots of training data (grey boxes), fitted values (red line), and bootstrapped samples (blue boxes) as iterations progress.
    The horizontal dashed line corresponds to the MLE. 
 Training datasets were initially simulated in the uniform interval, with $N=1000$ samples each permuted $L=10$ times to prevent the MLP from learning incorrect dependencies.
...
Similarly to the single parameter estimation, we fix $N=1000$, $L=10$, and $J=20$. 
...
%	\\
 ...
 Training output data were initially simulated using $N = 1000$ training samples each of length $J=20$ permuted $L=10$ times as:
...
\paragraph{Re-using data} We reuse training data from the previous iteration since simulations of Brown-Resnick processes are relatively expensive.
Among the samples not contained in the updated uniform interval of the current iteration, we randomly select $40\%$ and add them to the training data of the current step (see line 9 of Algorithm~\ref{alg3}). 
...
\begin{figure}[htb!]
	\centering
		\includegraphics[width=0.7\textwidth]{scatterplot_brownres_nn31_nsize31_nboot100_ntrain6000smoothtrain01to19_rangetrainmeanvariog05_smooth1_range62_transf_agg05_comb2.png}
...
\begin{figure}[htb!]
	\centering
		\includegraphics[width=0.85\textwidth]{boxplot_ex.png}
...
Next, since $\mathbf{x}_0$ is shorter than the training data, we create a new time series $\mathbf{x}^{*}_0$ by concatenating $\mathbf{x}_0$ to achieve the desired length.
...
\begin{figure}[htb!]
\centering
\includegraphics[width=1\textwidth]{eg_arima101_09_02.png}
	\caption{Data from an AR(1) process of length $T=50$ with AR coefficient equal to 0.9, augmented five times to achieve the training data length $T_k = 250$.}
\label{fig:eg-ts}
\end{figure}
...
(iii) holds without requiring any particular dependence structure assumption as long as the data is stationary and Markov.
...
One can for instance replicate the data into $m$ blocks, where $m = \lfloor \frac{T_k}{T} \rfloor$, and complete the remaining values with a random block from $\mathbf{x}_0$ of size $T_k \mbox{mod} T$.
...
\end{aligned}
    \label{eq:sv}
\end{equation}
where $\epsilon(t)$ and $\xi(t)$ are independent noises and $T$ is the number of observations. The volatility variable $h(t)$ is latent with an autoregressive of order one structure, and only $x(t)$ is observed. 
...
Extending our database approach to the spatial case would require more research because of an increase of the edge effect due to the replications.
