...
The main steps of the hypothetical QKD protocol can be summarised as follows
...
In summary, the physical protocol is given by
...
\begin{equation}
\ell\leq H^{\epsilon^\mathrm{phys}}_{\min}(\hat{Z}_1^n|R_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{phys,QKD}|_{ \Omega^\mathrm{phys}_{\mathrm{NA}}}}-\mathrm{leak}_\mathrm{EC}-2\log\frac{1}{\epsilon^\mathrm{phys}},
\end{equation}
...
\begin{align}
%&H^\epsilon_{\min}(\hat{Z}_1^n|R_1^n\tilde{X}_1^nLE)_{\omega^\mathrm{QKD}|_{{\Omega}}}\\
& H^\epsilon_{\min}(\hat{Z}_1^n|R_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NA}}}}}\nonumber\\
&\geq H^{\epsilon/4}_{\min}(\hat{Z}_1^n\tilde{X}_1^n|R_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NA}}}}}-H^{\epsilon/4}_{\max}(\tilde{X}_1^n|R_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NA}}}}}\nonumber\\
...
&\geq H^{\epsilon/16}_{\min}(\tilde{X}_i^n {X'}_i^n \hat{Z}_i^n \tilde{Z}_i^n|R_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NA}}}}\nonumber\\
&-H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|\hat{Z}_1^n\tilde{X}_1^nR_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NA}}}}-3\Gamma(\epsilon/16)\label{eq:22}.
\end{align} 
...
\begin{align}
 ...
 &+\sqrt{\frac{n}{2}\ln{\frac{512}{\epsilon^2 \Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NA}}]}}}\log{(17(m+1))}.
\end{align}
...
\begin{align}
   ...
   &-(a-1)^2K_a-p^\mathrm{PE}\log{5}-(1-p^\mathrm{key})\log(17(m+1))\nonumber\\
...
\begin{align}
    ...
    &\left.-(a-1)^2K_a-p^\mathrm{PE}\log{5}-(1-p^\mathrm{key})\log(17(m+1))\right]\nonumber\\
...
\begin{equation}\label{eq:deltaPE}
    \delta^\mathrm{tol}_\mathrm{PE}= 2\sqrt{\log\(\frac{n}{\epsilon_\mathrm{PE}^c}\)\sum_{i=1}^{4m}\frac{\gamma_ic^2_i}{n}}+\frac{3D}{n}\log\frac{2}{\epsilon_\mathrm{PE}^c},
\end{equation}
...
\begin{align}
    ...
    &\leq 2\sqrt{\log\(\frac{n}{\epsilon_\mathrm{PE}^c}\)\sum_{i=1}^{4m}\frac{\gamma_ic^2_i}{n}}+\frac{3D}{n}\log\frac{2}{\epsilon_\mathrm{PE}^c}.
\end{align}
...
&
-\varepsilon' \sum_{z'=1}^{4m + 16}\mu_{z'},
\end{align}
...
\centerline{\input{DW_b1_PRX.tex}}
    \caption{Asymptotic secret key generation rate according to \eqref{eq:asymptoticLimit} in terms of distances $D$ and excess noise $\xi$ with a modulation $(\Delta, \delta)=(0.9,0.9)$, ideal error correction $f=0 \%$ and cutoff $N_c=15$. The amplitude of the coherent states was optimized with respect to the distance.}% The amplitude of the coherent states was optimized with respect to the distance.}%, resulting in values between $1.07$ for $D=0$ km to $0.71$ beyond $D=65$ km.}
    \label{fig:AsymptoticRate}
\end{figure}

Switching to the finite-size regime, we can use our crossover min-tradeoff function $\tilde{g}_0$ in Theorem \ref{FinteSizePhys} together with the asymptotic rate \ref{fig:AsymptoticRate} to observe the finite key generation rates. Choosing the parameters $\xi=1\%$, $f=1\%$, $\epsilon = 10^{-9}$, $\epsilon^\mathrm{phys}_{\mathrm{NA}} = 10^{-3}$, $\epsilon^\mathrm{tom} = 10^{-8}$ and $\epsilon_\mathrm{PE}^c = 10^{-6}$ together with a grid search optimisation over $a$ and $p^\text{key}$, we were able to obtain non-zero key rates for $n\geq10^{12}$ rounds and distances $D\geq15$ km. The outcomes of this process are illustrated in Figure \ref{fig:FiniteRate}, where the curves represent different values for the finite key generation rates with respect to the number of rounds taken for the protocol.
...
\begin{figure}[h]
    ...
    \centerline{\input{FRates_9_27}}
    \caption{Finite-size secret key generation rate according to \eqref{eq:FiniteKeyRate} for $\xi=1\%$, $f=1\%$, $N_c=15$, $n$ rounds for the protocol and a modulation $(\Delta, \delta)=(0.9,0.9)$. The parameters $a$, $p^\mathrm{key}$ and $\tilde{p}^\mathrm{PE}$ were optimised according to a grid search, and we set $\epsilon = 10^{-9}$, $\epsilon^\mathrm{phys}_{\mathrm{NA}} = 10^{-3}$, $\epsilon^\mathrm{tom} = 10^{-8}$ and $\epsilon_\mathrm{PE}^c = 10^{-6}$.}% and $ \epsilon_\text{EC}=10^{-8} $.}
...
\begin{figure}[h]
    \centerline{\input{FRates_n12_9_27}}
...
Our security analysis can be improved in several directions. As mentioned, being a prepare-and-measure protocol, it is natural to consider the application of GEAT. This may not only provide larger finite-key generation rates, but also allow one to study variants of the protocol using homodyne measurements, which we were unable to accommodate within our security analysis. Another related question is to analyse using GEAT how the obtained rates vary with the number of states prepared by Alice and, in particular, how they approximate the rates of Gaussian modulated protocols. A second relevant improvement is to study how to remove the cutoff assumption in the computation of the asymptotic key rates. This has been achieved for collective attacks in the case where the information used in parameter estimation is made of moments of Bob's quadratures \cite{Upadhyaya2021,kanitschar2023finite}, but seems trickier when dealing with coarse-grained probabilities as well as in the presence of coherent attacks. In our view, our numerical results provide strong evidence that the rates without the cutoff assumption will be practically the same as those reported in our work for large enough value of the cutoff, but this deserves to be confirmed.

We also note that whereas the EAT does not require an explicit bound on the Hilbert space dimension of Eve, the Hilbert space dimensions are assumed finite \cite{dupuis2016entropy}. Whereas this is a reasonable assumption in realistic scenarios, where Eve is only assumed to be able create 'physical' states, i.e. states with finite energy \cite{tomamichel2017largely}, such an assumption has been overcome in the case of collective attacks, using a version of the asymptotic equipartition theorem allowing for infinite dimensional Hilbert spaces \cite{furrer2011min}. Given recent advancements towards a similar generalisation of the EAT to infinite dimensional Hilbert spaces \cite{fawzi2022asymptotic}, we intend to extend our security framework to infinite energy attacks in future works. 


Finally, while presented for a specific protocol consisting of four coherent states, our security proof can be adapted to other constellation of coherent states. It deserves further investigation to study how the rate changes when using more states and whether and how one can approximate the rate of protocols using Gaussian modulation. It is in fact expected that, as it happens for four coherent states, see Fig~\ref{fig:Nc_Test}, the rates obtained when using coarse-grained probabilities will be very close to those obtained when using moments~\cite{lin2019asymptotic}.
