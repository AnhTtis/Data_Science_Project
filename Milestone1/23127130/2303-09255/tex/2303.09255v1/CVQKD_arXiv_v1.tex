...
Having overcome these challenges, we are able to derive a min-tradeoff function using the numerical approach presented in \cite{winick2018reliable}, which, as said, requires a cutoff assumption. It involves a linearisation of the objective function and the use of duality, finally reducing the problem to a semi-definite programming optimisation, which can be efficiently handled numerically. Our numerical analysis also suggests that the values of the key rate do not significantly vary once the cutoff becomes large enough. Using this approach, we are able to obtain non-trivial key rates in finite-size setting of $n=10^{12}$ rounds.
...
For two subnormalised states $\rho,\sigma\in\mc{D}_{\leq}(\mc{H}_A)$, we define the generalised fidelity as
\begin{equation}
...
\begin{equation}
    ...
    \end{cases}
    \end{equation}
for positive semidefinite $\rho$ and $\sigma$, we make use of min and max conditional entropies, defined for a subnormalised quantum state $\rho_{AB}\in\mc{D}_{\leq}(\mc{H}_{AB})$ by \cite{tomamichel2015quantum},
\begin{align}
...
\begin{equation}
    ...
    \end{cases}
    \end{equation}
and the conditional  R\'enyi entropy as
\begin{equation}
...
A proof of security of a QKD protocol then involves two parts: Firstly, it has to be shown that it results in a state that is secret and correct, i.e. close to a perfect ccq-state. Formally,  for $\epsilon^\mathrm{sou}>0$, a QKD protocol is said to be \emph{$\epsilon^\mathrm{sou}$-sound}, if it results in a state $\rho^\mathrm{QKD}_{K_AK_BE}$, such that if we condition on the event ${\Omega_\text{NonAbort}}$ of not aborting the protocol it holds
\begin{equation}
\Pr_{\rho^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]\frac{1}{2}\left\|\rho^\mathrm{QKD}_{K_AK_BE}|_{\Omega_\text{NonAbort}}-\rho^\text{perfect ccq}_{K_AK_BE}\right\|_1\leq\epsilon^\mathrm{sou}.
\end{equation}
...
\begin{equation}
\Pr_{\rho^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]\frac{1}{2}\left\|\rho^\mathrm{QKD}_{K_AE}|_{\Omega_\text{NonAbort}}-\rho^\text{perfect ccq}_{K_AE}\right\|_1\leq\epsilon^\mathrm{sec}.
\end{equation}
The protocol is further said to be \emph{$\epsilon^\mathrm{cor}$-correct} if
\begin{equation}
    \Pr_{\rho^\mathrm{QKD}}[K_A\neq K_B\land\Omega_ \mathrm{NonAbort}]\leq \epsilon^\mathrm{cor}.
\end{equation}
...
\begin{equation}
1-\Pr_\mathrm{hon}[\Omega_{\mathrm{NonAbort}}]\leq\epsilon^\mathrm{com},
\end{equation}
...
In each round of the protocol, Alice prepares one of four coherent states $\ket{\varphi_x} \in \{\alpha,-\alpha,i \alpha, -i\alpha\}$ for some predetermined $\alpha\in\R$, with probability $\frac{1}{4}$. The state is then sent to Bob via a noisy channel that is potentially compromised by Eve. Bob then performs a heterodyne measurement. 
...
\end{equation}
where $\varphi_x \in \{\alpha,-\alpha,i \alpha, -i\alpha\}$. Alice sends the $A'$ subsystem to Bob via a noisy quantum channel, keeping the $A$ subsystem. Alice and Bob then both perform measurements on their respective subsystems.  
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{PhSpaceDouble.eps}
...
\begin{align}
    &\Omega_\mathrm{PE}:=\left\{c_1^n\in\mc{C}:\freq_{c_1^n}\in\mc{P}_{\Omega_\mathrm{PE}}\right\},\label{eq:OmegaPE}\\
    &\Omega_\mathrm{tom}:=\left\{c_1^n\in\mc{C}:\freq_{c_1^n}\in\mc{P}_{\Omega_\mathrm{tom}}\right\},\label{eq:Omegatom}\\
...
\Omega_{\mathrm{NonAbort}} = \Omega_{\mathrm{EA}} \cap \Omega_{\mathrm{EC}}.
...
\begin{align*}
    \sigma^\mathrm{QKD}_{\bar{X}_1^n O_1^n S_1^n C_1^nHH'LE}=\Pr[\Omega_{\mathrm{NonAbort}}]\sigma^\mathrm{QKD}_{\bar{X}_1^n O_1^n S_1^n C_1^nHH'LE}|_{ \Omega_{\mathrm{NonAbort}}}+(1-\Pr[\Omega_{\mathrm{NonAbort}}])\sigma^\mathrm{QKD}_{\bar{X}_1^n O_1^n S_1^n C_1^nHH'LE}|_{ \neg\Omega_{\mathrm{NonAbort}}}.
\end{align*}



\subsection{The physical QKD protocol}
Finally, let us describe the physical QKD protocol, which is actually performed in the lab. The physical QKD protocol is essentially equal to the hypothetical protocol except for the following two differences: Firstly, in step (1) of the protocol, if $R_i=2$, Alice does not perform tomography. The round is simply discarded. Hence, the random variable $X'_i$ will be either $\perp$ or undefined. Secondly, at the end of the protocol, abortion or non-abortion will only be determined by parameter estimation and error correction, i.e. the event $\Omega_\mathrm{NonAbort}$ will be replaced by $\Omega^\mathrm{phys}_\mathrm{NonAbort}=\Omega_\mathrm{PE}\cap\Omega_\mathrm{EC}$. 
...
\begin{equation}
\ell\leq H^{\epsilon^\mathrm{phys}}_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{phys,QKD}|_{ \Omega^\mathrm{phys}_{\mathrm{NonAbort}}}}-\mathrm{leak}_\mathrm{EC}-2\log\frac{1}{\epsilon^\mathrm{phys}},
\end{equation}
which is $3\epsilon^\mathrm{phys}+\epsilon_\mathrm{EC}$-sound, in the sense that $\Pr_{\sigma^\mathrm{phys,QKD}}[\Omega^\mathrm{phys}_\mathrm{NonAbort}]\frac{1}{2}\|\sigma^\mathrm{phys,QKD}|_{ \Omega^\mathrm{phys}_{\mathrm{NonAbort}}}-\sigma^\mathrm{perfect \ ccq}\|_1\leq 3\epsilon^\mathrm{phys}+\epsilon_\mathrm{EC}$.
\end{proposition}
...
In order to apply Proposition \ref{renner}, we need to lower bound the smooth min-entropy using the entropy accumulation theorem. However, as noted in the introduction, we are able to apply the EAT directly to our physical QKD protocol, due to the need to characterise Alice's marginal system in a prepare-and-measure scenario. To that purpose we first consider the hypothetical QKD protocol that includes additional tomography measurements. However, due to issues with the Markov condition, we will not be able to directly apply the EAT to our hypothetical protocol, either. Instead, we will make use of various chain rules for smooth entropies, in order to relate the output of our hypothetical QKD protocol to that of a series of $n$ EAT channels, which we call the `EAT process', and then apply the EAT to the EAT process, while dealing with the remaining terms separately. 
 ...
 We begin by considering the hypothetical QKD protocol. Let $n\in\mathbb{N}$ and $\epsilon>0$. Conditioned on not aborting, the hypothetical QKD protocol results in the state $\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}$. By application of chain rules for smooth entropies (eq. (6.63) and eq. (6.56) in \cite{tomamichel2015quantum}), it holds
\begin{align}
%&H^\epsilon_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nLE)_{\omega^\mathrm{QKD}|_{{\Omega}}}\\
& H^\epsilon_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}\geq H^{\epsilon/4}_{\min}(\hat{Z}_1^n\tilde{X}_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}-H^{\epsilon/4}_{\max}(\tilde{X}_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}-2\Gamma(\epsilon/4),\label{eq:20}%-\log{|L|}
\end{align} 
where $\Gamma(x):=-\log\(1-\sqrt{1-x^2}\)$. By another application of a chain rule (eq. (6.57) in \cite{tomamichel2015quantum}), we obtain
\begin{align}
&H^{\epsilon/4}_{\min}(\hat{Z}_1^n\tilde{X}_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}\geq H^{\epsilon/16}_{\min}(O_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}-H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|\hat{Z}_1^n\tilde{X}_1^nS_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}-3\Gamma(\epsilon/16)\label{eq:22}.
\end{align} 


We can now apply the same argument as used in \cite{dupuis2016entropy} to upper bound the max entropy terms in eqs. (\ref{eq:20}) and (\ref{eq:22}). We begin by upper bounding the term $H^{\epsilon/4}_{\max}(\tilde{X}_1^n|R_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}$ in eq. (\ref{eq:20}). We note that by the strong subadditivity of the smooth max entropy \cite{tomamichel2015quantum}, it holds
\begin{equation}
H^{\epsilon/4}_{\max}(\tilde{X}_1^n|R_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}\leq H^{\epsilon/4}_{\max}(\tilde{X}_1^n|R_1^n)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}},
\end{equation}
...
\begin{align}
 H^{\epsilon/4}_{\max}(\tilde{X}_1^n|R_1^n)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}&\leq  H^{\epsilon/4}_{\max}(\tilde{X}_1^n|\tilde{R}_1^n)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}\leq np^\text{PE}\log{5} + \sqrt{\frac{n}{2}\ln{\frac{32}{\epsilon^2 \Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]}}}\log{5}.
\end{align}
In a similar way we can provide an upper bound on the term $H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|\hat{Z}_1^nR_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}$ in eq. (\ref{eq:22}). Again, it holds by strong subadditivity,
\begin{equation}
H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|\hat{Z}_1^n\tilde{X}_1^nR_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}\leq H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|R_1^n)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}},
\end{equation}
...
\begin{align}
 H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|R_1^n)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}&\leq  H^{\epsilon/16}_{\max}({X'}_1^n\tilde{Z}_1^n|\bar{R}_1^n)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}\\
 &\leq n(1-p^\text{key})\log{\(17(S+1)\)}+\sqrt{\frac{n}{2}\ln{\frac{512}{\epsilon^2 \Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]}}}\log{(17(S+1))}.
\end{align}

What remains to be done is to lower bound the term $H^{\epsilon/16}_{\min}(O_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}}$ in eq. (\ref{eq:22}) using the EAT.
...
In order to apply the EAT we wish to condition on an event that is only defined on $C_1^n$. For that purpose, note that we can write $\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}$ as $(\sigma^\mathrm{QKD}|_{\Omega_{\mathrm{EA}}})|_{\Omega_{\mathrm{EC}}}$ where the probability of the event $\Omega_{\mathrm{EC}}$ with respect to the state $\sigma^\mathrm{QKD}|_{\Omega_{\mathrm{EA}}}$ is given by $\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EC}} | \Omega_{\mathrm{EA}}]$.
Now we use Lemma B.5 of \cite{dupuis2016entropy} to show that for $a\in(1,\infty)$
\begin{align}
    H^{\epsilon/16}_{\min}(O_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}} 
    &\geq H_{a}^{\uparrow}(O_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{NonAbort}}}} - \frac{\Gamma(\epsilon/16)}{a-1} \\
    &\geq H_{a}^{\uparrow}(O_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{ \Omega_{\mathrm{EA}}}} - \frac{\Gamma(\epsilon/16)}{a-1} - \frac{a}{a-1} \log\left(\frac{1}{\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EC}} | \Omega_{\mathrm{EA}}]}\right).\label{eq23}
\end{align}
...
%&=\sigma^\mathrm{QKD}_{O_1^nS_1^nC_1^nE}|_{\Omega_{\mathrm{NonAbort}}},
\end{align}
...
H^{\uparrow}_{a}(O_1^n|S_1^nE)_{\sigma^\mathrm{EAT}|_{ \Omega_{\mathrm{EA}}}}\geq nf(p_0)-n\(\delta^\mathrm{tol}+\frac{(a-1)\ln2}{2}V^2\)-\frac{a}{a-1}\log\frac{1}{\Pr_{\sigma^\mathrm{EAT}}[\Omega_{\mathrm{EA}}]}-n(a-1)^2K_a,
\end{equation}
where we have defined
\begin{align}
V&=\sqrt{\mathrm{Var}(f)+2}+\log(2d_O^2+1),\label{eq:V}\\
K_a&=\frac{1}{6(2-a)^3\ln2}2^{(a-1)(2\log d_O+\max(f)-\min_\Sigma(f))}\ln^3\(2^{2\log d_O+\max(f)-\min_\Sigma(f)}+e^2\),\label{eq:Ka}
\end{align}
...
\begin{equation}
\gamma'_i:=\frac{\pi'_i(1-\sum_{j=1}^i\pi'_j)}{1-\sum_{j=1}^{i-1}\pi'_j}, \; c'_i:=h'_i-\frac{\sum_{j=i+1}^{17}h'_j\pi'_j}{1-\sum_{j=1}^{i}\pi'_j},\; D':=\max_{i,j\in\{1,...,17\}}|h'_i-h'_j|,
\end{equation}
...
\begin{equation}
    \left|f^\mathrm{tom}(\freq_{c_1^n})-f^\mathrm{tom}(p_0)\right|=\left|(\hat{\pi}'-\pi')^Th'\right|\leq 2\sqrt{\log\(\frac{n}{\epsilon^\mathrm{tom}}\)\sum_{i=1}^{16}\frac{\gamma'_ic'^2_i}{n}}+\frac{3D'}{n}\log\frac{2}{\epsilon^\mathrm{tom}},
\end{equation}
...
\begin{equation}\label{eq68}
    H^{\epsilon^\mathrm{phys}}_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{phys,QKD}|_{\Omega^\mathrm{phys}_{\mathrm{NonAbort}}}}\geq H^\epsilon_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{QKD}|_{\Omega_{\mathrm{NonAbort}}}}.
\end{equation}
\end{lemma}
...
\end{align}
Further, by the triangle inequality, the $\epsilon$-ball (in terms of purified distance) around $\sigma_{\hat{Z}_1^nS_1^n\tilde{X}_1^nE}^\mathrm{QKD}|_{\Omega_{\mathrm{NonAbort}}}$ is contained in the $\epsilon^\mathrm{phys}$-ball around $\sigma_{\hat{Z}_1^nS_1^n\tilde{X}_1^nE}^\mathrm{QKD}|_{\Omega^\mathrm{phys}_{\mathrm{NonAbort}}}$, implying eq. (\ref{eq68}).
...
Let  $n\in\mathbb{N}$. Let $\epsilon^\mathrm{phys}_{\mathrm{NonAbort}},\epsilon^\mathrm{tom},\epsilon_\text{EC}\in(0,1)$ such that $\epsilon^\mathrm{tom}<\frac{1}{2}\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$. Let $\epsilon\in\(0,1-\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}\)$, and define $\epsilon^\mathrm{phys}=\epsilon+\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}$. Let $0\leq  p^\mathrm{key},p^\mathrm{PE},p^\mathrm{tom}\leq1$ such that $p^\mathrm{key}+p^\mathrm{PE}+p^\mathrm{tom}=1$. Let $f$ be an affine min-tradeoff function of the form $f(p)=f^\mathrm{PE}(p)+f^\mathrm{tom}(p)+\mathrm{const}$. Let $p_0$ be given by eqs. (\ref{eq:p0PE},\ref{eq:p0tom}). Let $\delta^\mathrm{tol}_\mathrm{PE}>0$ and define 
...
\end{equation}
Let further $\Omega_\mathrm{PE}$, $\Omega_\mathrm{tom}$, $\Omega_\mathrm{EC}$, and $\Omega_\mathrm{NonAbort}$ be defined by eqs. (\ref{eq:OmegaPE}-\ref{eq:OmegaNonAbort}). Let $\text{leak}_\text{EC}$ be the amount of information leaked during error correction. Then, if $\Pr_{\sigma^\mathrm{phys,QKD}}[\Omega_{\mathrm{PE}}\cap\Omega_{\mathrm{EC}}]\geq\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$, %Then, either the physical QKD protocol aborts with probability greater than  $1-\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$, or, 
...
\begin{align}
   r^\mathrm{phys}|_{\Omega^\mathrm{phys}_{\mathrm{NonAbort}}}&\geq f(p_0) -\delta^\mathrm{tol}_\mathrm{PE}-\delta^\mathrm{tol}_\mathrm{tom}-\frac{(a-1)\ln2}{2}V^2-(a-1)^2K_a-p^\mathrm{PE}\log{5}-(1-p^\mathrm{key})\log(17(S+1))\nonumber\\
    &-\frac{1}{\sqrt{n}}\left[\sqrt{\frac{1}{2}\ln\frac{32}{\epsilon^2 ( \epsilon^\mathrm{phys}_{\mathrm{NonAbort}}-\epsilon^\mathrm{tom})}}\log{5}+\sqrt{\frac{1}{2}\ln\frac{512}{\epsilon^2 ( \epsilon^\mathrm{phys}_{\mathrm{NonAbort}}-\epsilon^\mathrm{tom})}}\log{(17(S+1))}\right]\nonumber\\
    &-\frac{1}{n}\left[\frac{\Gamma(\epsilon/16)}{a-1}+\frac{a}{a-1}\log\frac{1}{ \epsilon^\mathrm{phys}_{\mathrm{NonAbort}}-\epsilon^\mathrm{tom}}+\mathrm{leak}_\mathrm{EC}+2\log{\frac{1}{\epsilon^\mathrm{phys}}}+2\Gamma(\epsilon/4)+3\Gamma(\epsilon/16)\right]\label{eq:FiniteKeyRate}.
\end{align}
...
\begin{proof}
We begin by lower bounding $H^\epsilon_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{QKD}|_{{ \Omega_{\mathrm{NonAbort}}}}}$ using eqs. (\ref{eq:20} - \ref{eq23}). We then note that by eq. (\ref{eq_32}) it holds $H_a^\uparrow(O_1^n|S_1^nE)_{\sigma^\mathrm{QKD}|_{\Omega_{\mathrm{EA}}}}=H_a^\uparrow(O_1^n|S_1^nE)_{\sigma^\text{EAT}|_{\Omega_{\mathrm{EA}}}}$, allowing us to apply Proposition \ref{EAT2alpha}. Since  $\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EC}}|\Omega_{\mathrm{EA}}]\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EA}}]=\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]$, the terms $\frac{a}{a-1}\log\frac{1}{ \Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EC}}|\Omega_{\mathrm{EA}}]}$ and $\frac{a}{a-1}\log\frac{1}{ \Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EA}}]}$ in eqs (\ref{eq23}) and (\ref{eq39}), can be merged, resulting in a term that  dependents only on $\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]$. Formally, we obtain 

\begin{align}
    H^\epsilon_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{QKD}|_{\Omega_{\mathrm{NonAbort}}}}&\geq \nonumber\\
    & n\left[ f(p_0) -\delta^\mathrm{tol}_\mathrm{PE}-\delta^\mathrm{tol}_\mathrm{tom}-\frac{(a-1)\ln2}{2}V^2-(a-1)^2K_a-p^\mathrm{PE}\log{5}-(1-p^\mathrm{key})\log(17(S+1))\right]\nonumber\\
    &-\sqrt{n}\left[\sqrt{\frac{1}{2}\ln\frac{32}{\epsilon^2 {\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]}}}\log{5}+\sqrt{\frac{1}{2}\ln\frac{512}{\epsilon^2 {\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]}}}\log{(17(S+1))}\right]\nonumber\\
    &-\frac{\Gamma(\epsilon/16)}{a-1}-\frac{a}{a-1}\log\frac{1}{\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]}-2\Gamma(\epsilon/4)-3\Gamma(\epsilon/16).%\label{eq:FiniteKeyRate}
\end{align}
Further, it holds
\begin{align}
\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]&=\Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{EA}}\cap\Omega_{\mathrm{EC}}]\\
...
\begin{equation}
    \Pr_{\sigma^\mathrm{QKD}}[\Omega_{\mathrm{NonAbort}}]\geq \epsilon^\mathrm{phys}_{\mathrm{NonAbort}}-\epsilon^\mathrm{tom}.
\end{equation}
Now we can apply Lemma \ref{lem:HypPhys} to show that for our choice of $\epsilon$, and $\epsilon^\mathrm{phys}=\epsilon+\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}$, it holds
\begin{equation}
 H^{\epsilon^\mathrm{phys}}_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{phys,QKD}|_{\Omega^\mathrm{phys}_{\mathrm{NonAbort}}}}\geq  H^\epsilon_{\min}(\hat{Z}_1^n|S_1^n\tilde{X}_1^nE)_{\sigma^\mathrm{QKD}|_{\Omega_{\mathrm{NonAbort}}}}
\end{equation}
...
In this section we show that the physical QKD protocol is complete, i.e. we provide a lower bound on the probability $\Pr_\mathrm{hon}[\Omega^\mathrm{phys}_{\mathrm{NonAbort}}]$ of an honest application not aborting. 
...
%\end{equation}
Let further $\epsilon_\mathrm{EC}^c\in(0,1)$ be a suitable completeness parameter for the error correction protocol used. Then the physical QKD protocol is $\epsilon_\mathrm{PE}^c+\epsilon_\mathrm{EC}^c$-complete, i.e. $\Pr_\mathrm{hon}[\Omega^\mathrm{phys}_\mathrm{NonAbort}]\geq1-\epsilon_\mathrm{PE}^c+\epsilon_\mathrm{EC}^c$.
\end{theorem}
...
\begin{equation}
    1-\Pr_\mathrm{hon}[\Omega^\mathrm{phys}_{\mathrm{NonAbort}}]\leq1- \Pr_\mathrm{hon}[\Omega_\mathrm{PE}]+1-\Pr_\mathrm{hon}[\Omega_\mathrm{EC}].
\end{equation}
...
\begin{equation}
    \left|f^\mathrm{PE}(\freq_{c_1^n})-f^\mathrm{PE}(p_0)\right|=\left|(\hat{\pi}-\pi)^Th\right|\leq 2\sqrt{\log\(\frac{n}{\epsilon_\mathrm{PE}^c}\)\sum_{i=1}^{4S}\frac{\gamma_ic^2_i}{n}}+\frac{3D}{n}\log\frac{2}{\epsilon_\mathrm{PE}^c}.
\end{equation}
...
For a given empirical distribution $p_0$ and a suitable choice of a min-tradeoff function, Theorems \ref{FinteSizePhys} and \ref{theo:complete} combined show the security of the finite round physical QKD protocol. In order to obtain the best finite size key rate, for given $n\in\mathbb{N}$, as well as some choice for the parameters $\epsilon^\mathrm{phys}_{\mathrm{NonAbort}},\epsilon^\mathrm{tom},\epsilon_\text{EC},\epsilon_\mathrm{EC}^c,\epsilon_\mathrm{PE}^c\in(0,1)$, such that $\epsilon^\mathrm{tom} \ll \frac{1}{2}\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$, as well as $\epsilon\in\(0,1-\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}\)$ and $\epsilon^\mathrm{phys}=\epsilon+\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}$, we can set the tolerance parameters as in eqs. (\ref{eq:deltatom2},\ref{eq:deltaPE}), and maximise the key rate given by (\ref{eq:FiniteKeyRate}) over probabilities $0\leq  p^\text{key},p^\text{PE},p^\text{tom}\leq1$ such that $p^\text{key}+p^\text{PE}+p^\text{tom}=1$, %over tolerance parameters $\delta^\mathrm{tol}_\mathrm{tom}>0$ fulfilling eq. (\ref{eq:deltatom}), $\delta^\mathrm{tol}_\mathrm{PE}$ fulfilling eq. (\ref{eq:deltaPE}), 
and over $a\in(1,2)$. We note that in order to get a non-trivial result, we will have to choose $\epsilon_\mathrm{PE}^c$ and $\epsilon_\mathrm{EC}^c$ such that the success probability of the honest implementation meets the threshold $\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$ used in Theorem \ref{FinteSizePhys}, i.e. we need $1-\epsilon_\mathrm{PE}^c-\epsilon_\mathrm{EC}^c>\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$. 
...
\begin{align}
\mc{M}^\text{EAT}_{i}(\cdot)&=p^\text{key}\mc{M}^\text{EAT,key}_{i}(\cdot)\otimes\pro{\perp}_{C_i}+(1-p^\text{key})\mc{M}^\text{EAT,test}_{i}(\cdot).
\end{align}
...
\begin{align}
%&\rho^{\text{EAT},i}_{O_iS_iC_iQ_i\tilde{E}}=\id_{\tilde{E}}\otimes\mc{M}^\text{EAT}_i(\rho_{Q_{i-1}\tilde{E}})=\id_{Q_i\tilde{E}}\otimes\mc{M}^\text{EAT}_{A_iB_i\to O_iS_iC_i}\(\rho_{A_iB_iQ_i\tilde{E}}\),\\
&\rho^{\text{EAT,test},i}_{O_iS_iC_iQ_i\tilde{E}}=\id_{\tilde{E}}\otimes\mc{M}^\text{EAT,test}_i(\rho_{Q_{i-1}\tilde{E}})=\id_{Q_i\tilde{E}}\otimes\mc{M}^\text{EAT,test}_{A_iB_i\to O_iS_iC_i}\(\rho_{A_iB_iQ_i\tilde{E}}\).
\end{align}
...
\begin{align}
V\leq\tilde V&=\sqrt{\frac{1}{1-p^\text{key}}\(\max(g)-\min(g)\)^2+2}+\log(2d_O^2+1),\label{eq:96}\\
K_a\leq\tilde K_a&=\frac{1}{6(2-a)^3\ln2}2^{(a-1)(2\log d_O+\max(g)-\min(g))}\ln^3\(2^{2\log d_O+\max(g)-\min(g)}+e^2\).\label{eq:97}
\end{align}
...
\begin{align}
\inf_{\substack{\mc{H}_{\hat{E}}\simeq\mc{H}_{Q_1}\\\ket{\rho}\in\Sigma_{\hat{E}}(\tilde p)}}H(O|S\hat{E})_{\rho^\text{EAT}}&\geq\inf_{\substack{\mc{H}_{\hat{E}}\simeq\mc{H}_{Q_1}\\\ket{\rho}\in\Sigma_{\hat{E}}(\tilde p)}}\( H(\hat{Z}|S\hat{E})_{\rho^\text{EAT}}+H(\tilde{Z}\tilde{X}X'|\hat{Z}S\hat{E})_{\rho^\text{EAT}}\)\label{eq:36}\\
...
\begin{align}
g(\tilde p)=&\inf_{\substack{\mc{H}_{\hat{E}}\simeq\mc{H}_{Q_1}\\\ket{\rho}\in\Sigma_{\hat{E}}(\tilde p)}} H(\hat{Z}|S\hat{E})_{\rho^\mathrm{EAT}}=p^\mathrm{key}\inf_{\rho\in\Sigma(\tilde p)}D(\mc{G}(\rho_{AB})||\mc{Z}(\mc{G}(\rho_{AB})),\label{eq:MinTradeOff2}
\end{align}
...
We then consider the dual of the SDP, which for any dual feasible point provides an affine lower bound on the orginal SDP. By the nature of duality, which roughly speaking incorporates the constraints into the objective, the objective of the dual problem will explicitly depend on $\tilde p$ in an affine way, as will the entire expression lower bounding  the optimising problem in eq. (\ref{115}). Thus, for any given state $\tilde\rho_{AB}\in\mc{D}(\mc{H}_{AB})$, as well any dual feasible point, we can obtain an affine crossover min-tradeoff function.
...
\begin{equation}
r(\rho) = H(\tilde{\mathcal{Z}}(\tilde{\mc{G}}(\rho)) - H(\tilde{\mc{G}}(\rho)). \label{newobj}
\end{equation}
...
\begin{align}
g(\tilde p)=p^\text{key}r(\rho^*_{\tilde p})&\geq p^\text{key}\(r(\tilde\rho)-\Tr\left[\tilde\rho^T\nabla r(\tilde\rho)\right]+\max_{\vec{\nu}\in\Sigma^*_{\tilde\rho}}\ell_{\tilde p}(\vec{\nu})\) \label{eq:lowerbound}\\
...
\end{align}
Such minimization can be approximately computed in MATLAB with the function \texttt{fminbnd}. Then, we set a new density matrix $\tilde\rho^{(i)} = \tilde\rho^{(i-1)} + \kappa^* \Delta\tilde\rho^{(i)}$, with the optimal coefficient $\kappa^*$, and repeat the optimisation \eqref{eq:SDP2a}. After $n^\mathrm{iter}$ we set $\tilde\rho_0=\tilde\rho^{(n^\mathrm{iter})}$.
...
\begin{align}
&\max(\tilde{g}_{0})=p^\text{key}G_0+p^\text{key}\underbrace{\max\left\{\frac{\nu_{0,xz}}{\tilde{p}^\text{PE}}:x=0,\ldots,3,z=0,\ldots, S-1\right\}\cup\left\{\frac{\nu'_{0,x'}}{\tilde{p}^\text{tom}}:x'=0,\ldots15,\right\}}_{=:\max(\nu_0)},\\
&\min(\tilde{g}_{0})=p^\text{key}G_0+p^\text{key}\underbrace{\min\left\{\frac{\nu_{0,xz}}{\tilde{p}^\text{PE}}:x=0,\ldots,3,z=0,\ldots, S-1\right\}\cup\left\{\frac{\nu'_{0,x'}}{\tilde{p}^\text{tom}}:x'=0,\ldots,15\right\}}_{=:\min(\nu_0)}. \label{Ming}
\end{align}
...
\begin{align}
f(p)&=\sum_{c\in\tilde{\mathcal{C}}}p(c)\(\max(\tilde{g}_0)+\frac{1}{1-p^\text{key}}\(\tilde{g}_0(\delta_c)-\max(\tilde{g}_0)\)\)+p(\perp,\perp,\perp)\max(\tilde{g}_0)\\
&=\max(\tilde{g}_0)+\sum_{c\in\tilde{\mathcal{C}}}\frac{p(c)}{1-p^\text{key}}\(\tilde{g}_0(\delta_c)-\max(\tilde{g}_0)\)\\
&=\underbrace{p^\mathrm{key}\(G_0+\max(\nu_0)\)}_{=:\mathrm{const}}+\sum_{x=0}^3\sum_{z=0}^{S-1}\underbrace{p^\text{key}\frac{\nu_{0,xz}/\tilde{p}^\text{PE}-\max(\nu_0)}{1-p^\text{key}}}_{=:h_{x,z,\perp}}p(x,z,\perp)+\sum_{x'=0}^{15}\underbrace{p^\text{key}\frac{\nu'_{0,x'}/\tilde{p}^\text{tom}-\max(\nu_0)}{1-p^\text{key}}}_{=:h_{\perp,\perp, x'}}p(\perp,\perp,x')\\
...
\begin{align}
f(p_0)&=p^\mathrm{key}\(G_0+\max(\nu_0)+ \sum_{x=0}^3\sum_{z=0}^{S-1}\(\nu_{0,xz}-\tilde{p}^\mathrm{PE}\max(\nu_0)\)p_0^\mathrm{sim}(x,z)+\sum_{x'=0}^{15}\(\nu'_{0,x'}-\tilde{p}^\mathrm{tom}\max(\nu_0)\)p_0^\mathrm{tom}(x')\)\\
&=p^\mathrm{key}\(G_0+ \sum_{x=0}^3\sum_{z=0}^{S-1}\nu_{0,xz} p_0^\mathrm{sim}(x,z)+\sum_{x'=0}^{15}\nu'_{0,x'}p_0^\mathrm{tom}(x')\)\label{146}\\
&=\tilde{g}_0(\tilde{p}_0).
\end{align}
...
Let $n\in\mathbb{N}$. We begin by noting that, for fixed $S\in\mathbb{N}$, our numerically obtained values $\nu_{0,xz}$, for $x=0,...,3,z=0,...,S-1$, and $\nu'_{0,x'}$, for $x'=0,...,15$, are constant, i.e. independent of $n$. Let us consider some fixed values for the parameters $\epsilon^\mathrm{phys}_{\mathrm{NonAbort}},\epsilon^\mathrm{tom},\epsilon_\text{EC},\epsilon_\mathrm{EC}^c,\epsilon_\mathrm{PE}^c\in(0,1)$, such that $\epsilon^\mathrm{tom}<\frac{1}{2}\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}$, as well as $\epsilon\in\(0,1-\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}\)$,  $\epsilon^\mathrm{phys}=\epsilon+\sqrt{2\epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}$. Let us also consider some constant $0\leq\tilde{p}^\mathrm{PE}\leq1$, and $\tilde{p}^\mathrm{tom}=1-\tilde{p}^\mathrm{PE}$. 
...
\begin{theorem}{\bf (Asymptotic rate)}\label{Asymptotoc}
 ...
 \begin{align}
r^\mathrm{phys}\big|_{\Omega^\mathrm{phys}_\mathrm{NonAbort}}&\geq G_0+\sum_{x=0}^3\sum_{z=0}^{S-1}\nu_{0,xz}p_0^\mathrm{sim}(x,z)+\sum_{x'=0}^{15}\nu'_{0,x'}p_0^\mathrm{tom}(x') -\frac{1}{n}\mathrm{leak}_\mathrm{EC}+\mathcal{O}((\log n)^\frac{1}{2}n^{-\frac{1}{4}})\\
\lim_{n\to \infty}r^\mathrm{phys}\big|_{\Omega^\mathrm{phys}_\mathrm{NonAbort}}&\geq G_0+\sum_{x=0}^3\sum_{z=0}^{S-1}\nu_{0,xz}p_0^\mathrm{sim}(x,z)+\sum_{x'=0}^{15}\nu'_{0,x'}p_0^\mathrm{tom}(x') -\lim_{n\to \infty}\frac{1}{n}\mathrm{leak}_\mathrm{EC}\label{rate}.
\end{align}
\end{theorem}
...
For the classical information leaked during error correction, we can assume an honest, iid implementation of the protocol. Assuming a perfect implementation of an $\epsilon_\mathrm{EC}$-secure and robust error correction, it holds \cite{renner2008security}
\begin{equation}
       \frac{1}{n}\mathrm{leak}_\mathrm{EC} \leq  \chi_\mathrm{EC} H(\hat{Z}|\hat{X}) + (1-\chi_\mathrm{EC})H(\hat{Z}) +\sqrt{\frac{3\log(2/\epsilon_\mathrm{EC})}{n}}\log(|\hat{Z}|+3),
\end{equation}
where $\hat{X}$, $\hat{Z}$ represent the key string bits (after removing the symbol $\perp$) of Alice and Bob respectively, and $\chi_\mathrm{EC}$ is the error correction efficiency, where $\chi_\mathrm{EC}=1$ corresponds to the Shannon limit. %Taking into account the inefficiency of error correction, we can upper this by \cite{lin2019asymptotic}
%\begin{equation}
...
$H(\hat{Z}|\hat{X})$ can be computed numerically according to the distribution \eqref{eq:doubleint} adapted for the modulation of the key rounds, namely
\begin{equation}
...
With the error correction cost, it is not only possible to calculate the asymptotic secret key rate, but also optimise the amplitude $\alpha$ that Alice chooses for her coherent states, which is not attainable with only the results from the SDP. The outcomes of such optimisation in amplitudes are included for completeness in Figure \ref{fig:AmpCurves}, which indicates in particular how $\alpha = 0.71$ becomes the optimal amplitude for distances beyond $D=65$ km in the case of $\chi_\mathrm{EC}=1$.%, in accordance with the results in \cite{lin2019asymptotic}.
...
%%%%%%%%%%%%%%
\begin{figure}[h]
    \centerline{\input{Inset}}
    \caption{Asymptotic secret key generation rate with $\xi=2\%$ and $\chi_\mathrm{EC}=0.95$ according to the parameter estimation described in Lin et al. \cite{lin2019asymptotic} and our modulation $(\Delta, \delta)=(0.9,0.9)$, with diverse values for the cutoff $N_c$. The amplitude, taken to be the same for all curves, was optimised with respect to the distance for the modulation employed in \cite{lin2019asymptotic}. The inset shows the convergence of our modulation.}
     \label{fig:Nc_Test}
\end{figure}
%%%%%%%%%%%%%%


Figure \ref{fig:AsymptoticRate} shows the asymptotic key rates according to eq. (\ref{eq:asymptoticLimit}), where a modulation $(\Delta, \delta)=(0.9,0.9)$ was employed together with a cutoff $N_c = 15$. In order to obtain a more numerically stable set of dual points for the finite analysis, the amplitude of the coherent states was optimised for the Shannon limit, and then a correction was applied to reach the scenario $\chi_\mathrm{EC}=0.95$. For distances below $150$ km, the algorithm typically needs 120 or less iterations to converge. For larger lengths, it was necessary to reach the limit of 300 iterations before using eq. \eqref{eq:maximization} to obtain the reliable lower bound. On the other hand, we observed a numerical error at the constraints $\varepsilon'$ typically between $10^{-10}$ for small lengths and $10^{-15}$ for very long distances, which ensures both the reliability of the code and the tightness of the key rates.

\begin{figure}[h]
\centerline{\input{DW_b1_20_2.tex}}
    \caption{Asymptotic secret key generation rate according to \eqref{eq:asymptoticLimit} in terms of distances $D$ and excess noise $\xi$ with a modulation $(\Delta, \delta)=(0.9,0.9)$ and an error correction efficiency $\chi_\mathrm{EC}=0.95$.}% The amplitude of the coherent states was optimized with respect to the distance.}%, resulting in values between $1.07$ for $D=0$ km to $0.71$ beyond $D=65$ km.}
    \label{fig:AsymptoticRate}
\end{figure}

Switching to the finite-size regime, we can use our crossover min-tradeoff function $\tilde{g}_0$ in Theorem \ref{FinteSizePhys} together with the asymptotic rate \ref{fig:AsymptoticRate} to observe the finite key generation rates. Choosing the parameters $\epsilon = 10^{-5}$, $\epsilon^\mathrm{phys}_{\mathrm{NonAbort}} = 10^{-2}$, $\epsilon^\mathrm{tom} = 10^{-8}$, $ \epsilon^\mathrm{phys} = \epsilon + \sqrt{2 \epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}$, $\epsilon_\mathrm{PE}^c = 10^{-6}$ and $ \epsilon_\text{EC}=10^{-8} $ together with a grid search optimisation over $a$ and $p^\text{key}$, we were able to obtain non-zero key rates for $n\geq10^{12}$ rounds and distances $D\geq20\text{km}$. The outcomes of this process are illustrated in Figure \ref{fig:FiniteRate}, where the curves represent different values for the finite key generation rates with respect to the number of rounds taken for the protocol.



% $\epsilon=\epsilon_{\mathrm{NonAbort}}=10^{-5}$, and  $\epsilon_\text{EC} = 10^{-8}$, and performing a grid search optimisation over $a$ and $p^\text{key}$, as well as an optimisation over $\tilde{p}^\text{PE}$, we are able to obtain non-zero key rates for $n=10^{11}$ rounds and distances $D\leq21\text{km}$.
% The outcomes of this process are illustrated in Figure \ref{fig:FiniteRate}, where the plots represent different values for the finite key generation rates with respect to the number of rounds taken for the protocol.} %The calculations show how it is possible to generate finite key rates even with only $n=10^{11}$ rounds for distances $D \leq 21$ km.
% we calculated the finite key generation rate given at \eqref{eq:FiniteKeyRate}, %we took the secret key rate for the asymptotic limit as the min-tradeoff function. 
%which also required an optimisation for $\tilde{p}^\text{PE}$ and $\tilde{p}^\text{tom}$ to minimize the term $\max(\tilde{g}_0)-\min(\tilde{g}_0)$, as well as adjusting the values of the coefficients $a$ and $p^\text{key}$ according to a grid search in order to maximise the value of the lower bound.


\begin{figure}[h]
    \centerline{\input{FRates_b095_20_2}}
    \caption{Finite-size secret key generation rate according to \eqref{eq:FiniteKeyRate} for $\xi=1\%$, $\chi_\mathrm{EC}=0.95$, $n$ rounds for the protocol and a modulation $(\Delta, \delta)=(0.9,0.9)$. The parameters $a$, $p^\mathrm{key}$ and $p^\mathrm{PE}$ were optimised according to a grid search, and we set $\epsilon = 10^{-5}$, $\epsilon^\mathrm{phys}_{\mathrm{NonAbort}} = 10^{-2}$, $\epsilon^\mathrm{tom} = 10^{-8}$, $ \epsilon^\mathrm{phys} = \epsilon + \sqrt{2 \epsilon^\mathrm{tom}/\epsilon^\mathrm{phys}_{\mathrm{NonAbort}}}$, $\epsilon_\mathrm{PE}^c = 10^{-6}$ and $ \epsilon_\text{EC}=10^{-8} $.}
...
%were obtained by maximizing the lower bound at \eqref{eq:FiniteKeyRate} according to a grid search for the coefficients $\alpha$ and $\beta$ and different values for the number of signals sent by Alice to Bob.
...
the AXA Chair in Quantum Information Science, European Union's Horizon 2020 research and innovation programme under grant agreements No. 820466  (project CiviQ), No. 101114043 (project QSNP), No. 101017733 (project Veriqtas) within the QuantERA II Programme, and No. 801370 (2019 BP 00097) within the  Marie Sklodowska-Curie Programme.%\deleted{, the postdoctoral fellowships programme Beatriu de Pin\'{o}s, funded by the Secretary of Universities and Research (Government of Catalonia) and by the Horizon 2020 programme of research and innovation of the European Union under the Marie Sklodowska-Curie grant agreement No. 801370 (2019 BP 00097). C.P. acknowledges the ERC AdG CERQUTE. O.F. acknowledges support from the European Research Council (ERC Grant AlgoQIP, Agreement No. 851716) and from the European Unionâs Horizon 2020 research and innovation programme under Grant Agreement No 101017733 within the QuantERA II Programme}.
}
