...
\mathbb{E}[f(H_\ell)] = \frac{1}{Z_G(g,T)}\int_{G^{\mathbb{E}^+}} f(h_\ell) \prod_{F\in\mathbb{F}} p_{\vol(F)}(h_{\partial F}(g))dg,
\end{equation}
...
\]
The expectation in the RHS can be bounded by Cauchy--Schwarz inequality
\begin{align*}
