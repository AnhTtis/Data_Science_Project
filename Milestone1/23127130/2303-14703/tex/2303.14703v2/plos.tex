...
Molecular subtypes in colorectal cancer (CRC) are crucial for treatment decisions. Recently, convolutional neural networks (CNNs) have been used to automate CRC subtype identification from H\&E stained histopathological images. However, the relationship between genomic variants and cellular morphology in imaging phenotypes remains underexplored. This study aimed to explore these correlations by integrating genomic variants into CNN models for CRC subtype classification from H\&E images. The TCGA-CRC-DX dataset, containing whole slide images from 360 CRC-diagnosed patients, was used. CNN models were trained to account for correlations between genomic variations within CRC subtypes and cellular morphology patterns. The interplay was assessed through stratified 5-fold cross-validation, using AUROC and AP as performance metrics. CNN models that considered correlations between genomic variations and cellular morphology outperformed baseline models that didn't account for genomic variations. Using single-nucleotide-polymorphism (SNP) features (AUROC: 0.824$\pm$0.02 vs. 0.761$\pm$0.04, p$<$0.05, AP: 0.652$\pm$0.06 vs. 0.58$\pm$0.08) or CpG-Island methylation phenotype (CIMP) features (AUROC: 0.834$\pm$0.01 vs. 0.787$\pm$0.03, p$<$0.05, AP: 0.687$\pm$0.02 vs. 0.64$\pm$0.05) yielded improved accuracy. Combining models for CIMP and SNP further enhanced accuracy (AUROC: 0.847$\pm$0.01 vs. 0.787$\pm$0.03, p=0.01, AP: 0.68$\pm$0.02 vs. 0.64$\pm$0.05). These findings demonstrate that accounting for genomic variations in CRC subtypes improves the accuracy of CNN models in CRC subtype classification from histopathological images, potentially shedding light on biological cues affecting cancer histopathological imaging phenotypes.
...
% Therefore, it is plausible that changes in DNA methylation profiles might be associated with varied cellular compositions, which in turn could be reflected in the morphological features observed in whole slide images

% Specifically, Zheng et al. \cite{Zheng2020} hypothesized that DNA methylation patterns can be discerned through whole slide images, owing to their influence on cellular morphology in various dimensions including chromatin organization \cite{Zhang2017}, shaping cell identity \cite{Lokk2014}. Consequently, it stands to reason that alterations in DNA methylation profiles could correlate with distinct mixes of cell types, thereby manifesting in the morphometric characteristics captured by whole slide imaging.

% Of even greater significance is the realization that changes in DNA methylation within key driver genes implicated in cancer can trigger the dysregulation of these genes, subsequently giving rise to transcriptomic and proteomic shifts. These molecular changes cascade into the transformation of critical cellular processes, encompassing cell-cycle regulation, metabolism, and angiogenesis. The cumulative effect of such transformations is substantial enough to be discerned in the morphological patterns captured within whole slide images.


% Previous studies suggested that high single-nucleotide-polymorphism (SNP) rate and high CpG-Island methylation phenotype (CIMP) dominant molecular features of the MSI class 
% \cite{Liu2018ComparativeAdenocarcinomas}. 



% Bilat et al. \cite{Bilal2021DevelopmentStudy} segmented the cells in chosen patches using Hover-net \cite{Graham2019}. They state, in their cell composition analysis, that there exists a strong correlation between MSI and
% hypermutation (and hypermutation is mainly due to SNPs) and a moderate correlation between
% MSI and CIMP-H. The cell composition of all of these phenotypes consists a large immune response. 



%and a high infiltrate of inflammatory cells in microsatellite instable tumours9,32,34  are in line with existing literature.  
%Another study, suggests high correlation of tumor slides with immune response. Immune response is activated dominantly by hypermutation. https://www.nature.com/articles/s41598-022-13541-2
% All of these studies are evident to the influence of these molecular features to the image phenotype.  
% Not only are they dominant but they also present a range of values among the MSI class. 

% We therefore hypothesize that the diversity within these features in the MSI class limits the learning capacity of the CNN. 


In this study, we aim to leverage CNN-based classification models to investigate the interplay between molecular and morphological levels. Our main hypothesis is that genomic variations within CRC subtypes of MSI and MSS may impact the H\&E image phenotype. We examined this hypothesis by developing and evaluating CNN classification models that account for the potential correlation between the genomic variations and the imaging phenotype. We particularly focused on single-nucleotide-polymorphism (SNP) and CpG-Island methylation phenotype (CIMP) within the MSI and MSS CRC subtypes because of their significant heterogeneity observed within the MSI subtype as indicated by Liu et al. \cite{Liu2018ComparativeAdenocarcinomas}.
...
It is important to highlight that while our training phase incorporates molecular subtype information, the inference process depends exclusively on the H\&E images, with no additional data used. 
...
% The rest of the paper is organized as follows: Section~\ref{sec:related} summarized related work. Section~\ref{sec:methods} presents the data used in the study, the methodological contribution, and the experimental setup. The experimental results are described in Section~\ref{sec:results}. Finally, Section~\ref{sec:discussion} discusses our findings and concludes the work.
...
% The CNN model generates patch-level classifications. To obtain patient-level classification, we average the classification probabilities of the patches. For instance, given a classifier $F$, the output for a specific patch $x$ is the probability of being classified as MSI or MSS, denoted as $(0 \leq F(x) \leq 1$. For an H\&E image $W$ with $N$ extracted patches $((x_1,x_2, \hdots, x_N) \in W)$, we compute the patient-level MSI probability as follows:
...
We delved into the possible presence of phenotypic variations within classes, theorizing that such diversity might hinder the learning and generalization proficiency of the CNN. Our exploration centered on two dominant attributes: SNPs and the CIMP. Drawing from Liu et al.'s findings \cite{Liu2018ComparativeAdenocarcinomas}, MSI samples manifest significant disparities in SNP density (ranging dramatically from 10-200) and CIMP distinctions (with 60\% of MSI manifesting as CIMP-H, leaving 40\% otherwise). This led us to speculate that such intrinsic variances could find reflection in the H\&E imaging phenotypes. To maintain a consistent benchmark, we used CNV as a control variable due to its stable nature within MSI tumors.

By charting the biological attribute frequencies from patch results of our baseline model, our objective was to discern potential characteristic patterns for misclassified patches. We observed that such inaccurately classified patches either aligned closely with the contrasting class or spanned a diverse set of feature values. This observation paved the way for the hypothesis that the CNN might have acquired a confined phenotypic spectrum for the classes. To enhance this spectrum for the CNN, we suggested segregating the classes based on these characteristic patterns.


% It's a well-established fact that tumors exhibiting MSI often present a high prevalence of SNPs due to a malfunctioning DNA mismatch-repair mechanism. Furthermore, they're frequently classified as highly methylated \cite{Liu2018ComparativeAdenocarcinomas}. As a control experiment, we also explored the feature of copy-number variation (CNV), which is more common in MSS tumors and less frequent in MSI tumors.
...
%Steps for generating the MSI sub-class label. $y_i$ are the database original labels. $s_i$ are the chosen feature rates of each patient, given by the database. $y'_i$ are the new inferred labels according to the feature threshold.
}
\label{alg:snp}
\end{algorithm}
...
% The CNN here classifies the patches to 3 classes - $MSI_1$, $MSI_2$ and MSS. To perform patient level classification we consider the patch probability of MSI as the higher probability among $MSI_1$, $MSI_2$. 

% Formally, for a patch $x$ and a classifier $F$, the MSI probability is defined as:
% \begin{equation}
%     P_x(MSI) = max\left(F(x=MSI_1),F(x=MSI_2)\right)
% \end{equation}
% In a manner similar to the baseline model, we use equation \ref{eq:agg} to calculate the patient-level classification from the H\&E image. 

% Similarly to the baseline model we calculate the patient-level classification for a H\&E image with equation \ref{eq:agg}. 
...
\end{figure}
% We began our investigation by evaluating the performance of a baseline model, a slightly modified replication of the one developed by Kather et al.\cite{Kather2019DeepCancer}. 
...
The boxplot of the SNP rates concerning the test set's baseline classification results is displayed in Figure~\ref{fig:feature_boxplot}a. The MSS class consistently shows a low SNP rate, regardless of the model classification outcome. This aligns with prior research indicating that MSS patients usually lack a deficiency in the DNA mismatch repair mechanism \cite{Liu2018ComparativeAdenocarcinomas}. Conversely, for the MSI class, there's a noticeable difference in the SNP distribution between true-positive and false-negative classifications. The true-positive group has a marginally higher median with limited variance, whereas the false-negative group demonstrates a wider range of variation.
The threshold of the optimal split was found to be 1200.
Figure~\ref{fig:feature_boxplot}b showcases the distribution of methylation types based on the baseline classification of the test set. While CIMP-H appears infrequently in MSS (present in 1\% of MSS patches), it is prevalent in MSI (accounting for 59\% of MSI patches). Notably, a large portion of Non-CIMP-H patches in MSI (76\%) were mistakenly classified as the MSI class.
...
%     The distribution of molecular features within the test set, separated according to the baseline classification of the patches is presented as follows:
...
% We devised three biologically-primed models. The first, BP-CNN\textsubscript{SNP}, partitioned MSI patients into two subgroups based on their SNP rate, using a threshold of 1200. The second, BP-CNN\textsubscript{CIMP}, separated the MSI group into CIMP-H versus non-CIMP-H subgroups. The third, BP-CNN\textsubscript{CNV}, divided the MSI group into two subgroups based on each patient's CNV rate, with a threshold of 0.005.
...
\end{figure}
 ...
 \begin{figure}[ht!]
    ...
    % Patient level confusion matrices. Each matrix is an average on the test set over the different training sessions. BP-CNN SNP model (a) BP-CNN CIMP model (b). Threshold for MSI prediction is the best F1 score threshold.
    }
    \label{fig:confusion}
\end{figure}
...
%     \begin{tabular}{rrr}
%     \hline
%          Method & AUC(95\% CI) & AP(95\% CI)\\
%          \hline
%          Baseline &0.8(0.78-0.81)  &  0.66(0.61-0.70)\\
%          BP-CNN\textsubscript{SNP} &0.82(0.79-0.86)  & 0.65(0.58-0.72)\\ 
%          BP-CNN\textsubscript{CIMP} &0.83(0.81-0.85)  &0.687(0.65-0.72) \\
%          BP-CNN\textsubscript{CNV} &0.79(0.75-0.85) &0.63(0.51-0.76)\\
%          BP-CNN\textsubscript{Combined} & 0.85(0.82-0.87) & 0.68(0.63-0.71)\\ 
%          \hline
%     \end{tabular}
%     \caption{Performance of our BP-CNN models - SNP, CIMP, and the Combined (CIMP and SNP), averaged over 5 folds with 95\% confidence intervals, compared to the Baseline model. }
%     \label{tab:prev_results1}
% \end{table}
...
% Our results present a substantial improvement in classification accuracy, as gauged by the average AUC, when compared to earlier studies that utilized CNN models and transfer-learning for CRC subtype classification, such as the study conducted by Kather et al. \cite{Kather2019DeepCancer}. This advancement underscores the influence of molecular subtypes like SNP and CIMP on the phenotype of H\&E stained histopathological images. Yet,  

% While the cutting-edge 'draw and rank' method by Bilal et al. \cite{Bilal2021DevelopmentStudy} demonstrates superior classification accuracy (average AUC of 0.9$\pm$0.01 using a repeated cross-validation setup with three repetitions and four folds), it considers the CNN model for image classification as a 'black box', and hence, fails to offer insights into how molecular features affect the phenotype of H\&E stained histopathological images.
...
M.F. acknowledges funding from the Israel Innovation Authority (grant number 73249) and from Microsoft Education and the Israel Inter-university Computation Center (IUCC). 
