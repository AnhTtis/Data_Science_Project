\pgfplotsset{/pgfplots/horizontal line legend/.style={legend image code/.code={\draw[ultra thick] (0cm,0cm)-- (.35cm,0cm);},},}

\begin{tikzpicture}
    \begin{axis} [ybar,
    ylabel=Time per time step (ms), ymin=0, ymax=45,
    ...
    \addplot[blue,horizontal line legend,sharp plot,update limits=false,ultra thick, dashed] coordinates { ([normalized]-0.4,23.9) ([normalized]0.4,23.9) };
    \addplot[black,horizontal line legend,sharp plot,update limits=false,ultra thick] coordinates { ([normalized]-0.4,17.4) ([normalized]0.4,17.4) };
    \legend{Dilute case, Dense case, PSM without particles, LBM roofline $T_\min$}
    \end{axis}
\end{tikzpicture}
