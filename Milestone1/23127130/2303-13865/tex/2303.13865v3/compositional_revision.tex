...
With the recent advances in computing power and architecture, we have witnessed with it an increase in the scale and complexity of models and algorithms utilised in statistics and machine learning. A key technique to understand and analyse complex algorithms is to understand their compositional structure: namely what are the fundamental building blocks that constitute an algorithm, and how are they related?
...
One key underlying idea is to focus on compositionality and abstract away specific implementational details. 
...
{\it (ii)} forward decomposition; {\it (iii)} backward decomposition. The computations involved can only be performed in closed-form in very specific settings, such as when the latent states take values only in a finite set. The celebrated Kalman filter-smoother for linear Gaussian systems is another well-known example (\cite{Bishop07}).  \cite{van2020automatic} introduced the {\it Backward Filtering Forward Guiding (BFFG)} algorithm, which uses the forward decomposition with  simplified dynamics in the backward filtering pass for $X$.   BFFG provides a general framework for dealing with the smoothing problem, encompassing both the Kalman and finite-state space settings as special cases.  
...
In particular, in Theorems~\ref{thm:functor} and \ref{thm:lax} we provide compositionality results for Kalman smoothing. More precisely, we show that the BFFG algorithm defines a {functor} from a category of Markov kernels into a category of optics, which in certain settings can even be \textit{lax monoidal}. In other words, the optic obtained from parallel or sequential composition of Markov kernels is equivalent to the corresponding composition of optics of the separate Markov kernels. Compositionality of the well-known Forward Filtering Backward Sampling (FFBS) algorithm (\cite{fruhwirth1994data}, \cite{carter1994gibbs}) is a direct  consequence of our results. 
 

This categorical perspective on BFFG enables prospective users to break down this complex algorithm into its constituent parts, to aid to implementation and comprehensibility. It demonstrates there is a higher-order structure preserving core within such smoothing algorithms. 
...
Optics as models of bidirectional data flow have also found numerous applied uses. For instance, in the recent works of \cite{smithe2020bayesian, Braithwaite2023}, a category of optics termed \textit{Bayesian lenses} is used to give a compositional account of Bayesian inference itself.
...
Using Markov kernels, we can define a ``computational graph'' of the stochastic process evolving on a  \textit{directed tree}. This is most easily done by an example. Consider the  tree depicted in Figure \ref{fig:directed_tree}. Though this visualisation is popular in the literature on Bayesian networks -- highlighting vertices at which random quantities ``sit''-- from a categorical point of view it is more natural to convert this figure into the \textit{string diagram} given in Figure \ref{fig:string_directed_tree}. Here, for any measurable space $S$, the duplication kernel $\Dup\colon S\rightarrowtriangle S\otimes S$ is a Markov kernel defined by
...
We assume that for the final vertex $\kappa_{n+1}(x,\dd y) = p_{n+1}(x,y) \nu(\dif y)$ for a dominating measure $\nu$, where $p$ is jointly measurable. 
...
\begin{equation}\label{eq:kappa_to_kappastar}
    \kappa^\star_i(x,\dd y) = \frac{\kappa_i(x,\dd y) h_i(y)}{\int \kappa_i(x,\dd y') h_i(y')}. 
\end{equation} 
...
Therefore, if $(h_i)_{i=0}^n$ are known, we can pushforward $\delta_{x_0}$, via
\begin{equation}\label{eq:linegraph*} \delta_{x_0} \kappa_1^\star \kappa_2^\star \cdots \kappa_i^\star, \end{equation}
...
\[ P^\star(\dif x_0,\ldots, \dd x_n) = \delta_{x_0}(\dif x_0) \kappa^\star_1(x_0, \dd x_1) \cdots \kappa^\star_{n-1}(x_{n-1},  \dd x_{n}). \]
...
\[ \kappa^\circ_i(x,\dd y) = \frac{\kappa_i(x,\dd y) g_i(y)}{\int \kappa_i(x,\dd y') g_i(y')}. \]
If chosen properly, the sequence of maps  $\{g_i\}_{i=0}^n$ ensures that realisations of the process under $\kappa^\circ_i$ are {\it guided} towards $x_{n+1}$ at vertex $n+1$.
Clearly, we have Radon--Nikodym derivatives
\[ \frac{\kappa^\star_i(x,\dd y)}{\kappa^\circ_i(x,\dd y)} = \frac{h_i(y)}{g_i(y)} \frac{\int\kappa_i(x,\dd y') g_i(y')}{\int\kappa_i(x,\dd y') h_i(y')}.\]
The likelihood ratio of paths samples under $\kappa^\star_i$ relative to $\kappa_i^\circ$ is therefore given by the likelihood ratio
\begin{align*}
    L(x_1,\ldots, x_n) &= \prod_{i=1}^{n} \frac{h_i(x_{i})}{g_i(x_{i})} \frac{\int \kappa_i(x_{i-1},\dd y) g_i(y)}{\int \kappa_i(x_{i-1},\dd y) h_i(y)}\\ &= \frac{h_{n}(x_{n})}{h_0(x_0)}\prod_{i=1}^{n} \frac{\int \kappa_i(x_{i-1},\dd y) g_i(y)}{g_i(x_i)} \\
    &= \frac{h_{n}(x_{n})}{g_n(x_n)}\frac{g_0(x_0)}{h_0(x_0)}\prod_{i=1}^{n} \frac{ \kappa_ig_i(x_{i-1})}{\tilde\kappa_{i}g_i (x_{i-1})} .
\end{align*}  
Note the cancellation of terms due to the recursive relation in \eqref{eq: hn2}. This implies that for bounded functions $f$,
...
\[ P^\circ(\dif x_0,\ldots, \dd x_n) = \delta_{x_0}(\dif x_0) \kappa^\circ_1(x_0, \dd x_1) \cdots \kappa^\circ_{n-1}(x_{n-1},  \dd x_{n}). \]


Assuming $h_n=p_{n+1}(\cdot, x_{n+1})$ to be tractable and $x_0$ being fixed and known, the likelihood ratio does not involve $h_1,\ldots, h_{n-1}$ which may be hard or expensive to compute. It does however depend on  $h_0(x_0)$, the \emph{evidence}, which can usually not be evaluated. Fortunately,  Markov Chain or Sequential Monte Carlo methods that exploit the relation in \eqref{eq:condexp} only require evaluation of $L$ up to a fixed multiplicative constant. For instance, \eqref{eq:condexp} implies
\[
...
\]
where $\{(X_{1,i}, \ldots, X_{n,i})\}_{i=1}^I$ are independent samples under $\mathbb{P}^\circ$. We see that $T$ does not depend on $h_0(x_0)$.
...
The main contribution of this work is to connect the BFFG algorithm described above with well-established concepts from category theory. In this section, we introduce the key categorical definitions.
...
\begin{defn}
    We say that a category $\mathcal C$ is a \textbf{monoidal category}, if in addition to Definition~\ref{def:cat}, we have
    \begin{enumerate}
...
\]
        ...
        \end{tikzcd};
        \]
        \item the category is furthermore a \textbf{symmetric} monoidal category if in addition there are natural isomorphisms $s_{A,B}\colon{}A\otimes B \to B\otimes A$ such that 
...
Optics were introduced in \cite{Riley2018} for modelling systems with bidirectional data flow. Here, we present the formulation as given in  \cite{gavranovic2022space}.  Note that an optic models a transformation in which a forward pass is followed by a backward pass. The first pass of the BFFG algorithm is the backwards filter, so this will actually constitute the \textit{forward map} of the optic.  
...
\begin{itemize}
	...
	\item $\fw \colon A \to M \otimes B$, a morphism referred to as the \textit{forward map};
	\item $\bw \colon M \otimes B' \to A'$, a morphism referred to as the \textit{backward map};
\end{itemize}
and two such triplets $(M, f, f')$ and $(N,g,g')$ are equivalent, denoted $(M, f, f') \sim (N,g,g')$ if there exists a \textit{residual morphism} $r\colon M \to N$ such that 
...
\end{tikzpicture}
\end{equation}
In the forward pass, $\fw$ produces a {\it message} of type $M$, the internal state, which is consumed in the backward pass by $\bw$. 
...
\begin{align*}
	M & := M_1 \otimes M_2 \\
	\fw &:=   (\id_{M_1} \otimes\, \fw_2)\circ \fw_1 \colon \quad A \rightarrow M_1 \otimes (M_2 \otimes C) \\
  	\bw &:= \bw_1 \circ (\id_{M_1} \otimes \,\bw_2) \colon \quad M_1\otimes (M_2 \otimes C') \to A',
 \end{align*}
\end{defn}
...
\begin{defn}[Identity optic]
    ...
    we take as internal state $1$, the monoidal unit, then the forwards map is $\lambda^{-1}_A\colon{}A\to 1\otimes A$ and the backwards map is $\lambda_{A'}\colon{}1\otimes A' \to A'$, the left unitors.
\end{defn}

See \cite[Theorem 2.0.12]{Riley2018} for a formal proof of the following.
...
\begin{itemize}
    ...
    \item then in the \textit{forward pass} we iteratively apply the  $h$-transform from Definition \ref{def:htransform} to $(\kappa_i, g_i)$.
\end{itemize}
In this section, we will formalise both steps by defining formally backward- and forward maps, using the framework of Optics as defined in Section~\ref{sec:optics}. 
...
For a measurable space $S = (E, \fB)$, let $\bM_+(S)$ denote the set of strictly positive measurable functions on $S$. Let $\calM(S)$ denote the set of s-finite measures on $S$. s-finite measures are a convenient class to work with, since it is closed under \textit{pushforwards} of measurable maps and Markov kernels \cite[Proposition~7]{Staton2017}.
...
Intuitively, we should imagine that the kernel $\tilde\kappa$ is an approximation to the kernel $\kappa$, and then $g$ represents a an approximation to the functions $h$ defined through \eqref{eq: hn2}.

This map returns both the  pullback $\tilde \kappa g$  and an appropriate {\it message} $g$ for the map $\forw{\kappa,\tilde\kappa}$ specified in the following definition. This corresponds to the backwards map in BFFG, but will be $\fw$ in terms of the optic (Definition~\ref{def:optic}).
...
\begin{equation}\label{eq:forw}
 \forw{\kappa,\tilde\kappa}(g, \mu)  = \nu, \quad \text{with} \quad \nu(\!\dd y) := \int \frac{g(y)}{\tilde\kappa g(x)} \mu(\!\dd x) \kappa(x, \dd y). 
 \end{equation}
...
This corresponds to the forwards pass in BFFG, and in terms of the optic will be $\bw$ (Definition~\ref{def:optic}).
\end{defn}


The pairing of the backward- and forward maps can be conveniently summarised by the following diagram, which is analogous to \eqref{eq:optic}:
...
Then, we can reinterpret $\forw{\kappa,\tilde\kappa}$ as producing a {\it weighted} probability measure: 
...
\begin{split}
 w_{\kappa,\tilde\kappa}(g,\mu) &= \iint  \frac{g(y)}{\tilde\kappa g(x)} \kappa(x,\dd y) \mu(\dif x) =  \int \frac{(\kappa g)(x)}{(\tilde \kappa g)(x)} \mu(\dif x)\\
\nu(\!\dd y) &=  w^{-1}_{\kappa,\tilde\kappa}(g, \mu) \int  \frac{g(y) }{(\tilde \kappa g)(x)  }  \mu(\!\dd x) \kappa(x, \dd y), 
 \end{split}
 \end{equation}
...
\begin{thm}\label{thm:functor}
    ...
    \begin{itemize}
        \item on objects, given a measurable space $S$, we have $\scr{O}(S)= \bM_+(S)\otimes \calM(S)$;
...
\begin{proof}
For the first construction, we see that it has internal state $M:= \bM_+(S'')$, with
\begin{equation*}
...
\begin{split}
    \forw{0,2} (g,\mu) &=\nu,\\
    \nu(\dif y) &= \int \frac{g(y)}{\tilde\kappa_{0,2}g(x)} \mu(\dif x) \kappa_{0,2}(x,\dif y).
\end{split}
\end{equation*}

On the other hand, with the second construction by composing optics we find we have inner state $N:= \bM_+(S'')\otimes \bM_+(S') $ and maps
\begin{equation*}
...
\end{lem}
\begin{proof}
    For the backwards pass, we have $\backw{\id_S}(g)=(g,g)$.
    For the forwards pass, we have $\forw{\id_S,\id_S}(g,\mu)=\nu$ with $\nu(\dif x)=\mu(\dif x)$.
...
In the previous section we established that the BFFG is functorial, namely it preserves sequential composition. It turns out it can also preserve parallel composition: in certain settings, $\scr{O}$ defines a \textit{lax monoidal functor} as in Definition~\ref{defn:laxmf}.
...
Under this condition, BFFG actually coincides with BFFS as in Algorithm~\ref{alg:bffs}.
...
Since we have already established in Theorem in Theorem~\ref{prop:seq} that $\scr{O}\colon{} (\Stoch_2)^{\mathrm{Op}}\to \Optic(\Set)$ defines a functor, what remains to establish lax monoidal functoriality is defining an appropriate morphism $\epsilon$ and natural transformation $\mu$, which together satisfy associativity and unitality.
...
$${\mu_{S,T}}\colon{}\scrO(S)\otimes\scrO(T)\to\scrO\left(S\otimes T\right)$$ as follows: we take trivial internal state, and the following forwards and backwards maps:
\begin{equation*}
    \begin{split}
...
In order to prove naturality of $\mu$, we would need to establish the following naturality square: for kernels $\kappa_1, \tilde\kappa_1 \colon{} S\rightarrowtriangle S'$ and $\kappa_2, \tilde\kappa_2 \colon{} T\rightarrowtriangle T'$ ,
...
\begin{itemize}
    ...
    \begin{equation*}
        \begin{split}
            ...
            \text{fw: }\bM_+(S')\otimes \bM_+(T')&\to N\otimes \bM_+(S\otimes T),\\
            (g_1,g_2) &\mapsto \left( g_1 \cdot g_2,\tilde \kappa_1 g_1 \cdot \tilde\kappa_2 g_2 \right);\\
            \text{bw: } N\otimes \calM(S\otimes T)&\to \calM(S')\otimes \calM(T'),\\
            \left( \bar g(s',t'),\bar \mu(\dif s,\dif t) \right) \mapsto \bigg( \int_{S\otimes T}\int_{T'}&\frac{\bar{g}(s',t')}{\left[\left(\tilde{\kappa}_{1}\otimes\tilde{\kappa}_{2}\right)\bar{g}\right](s,t)}\bar{\mu}(\dif s,\dif t)\kappa_{1}(s,\dif s')\kappa_{2}(t,\dif t') ,\\
    ...
    \begin{split}
        ...
        \text{fw: }\bM_+(S')\otimes \bM_+(T')&\to M\otimes \bM_+(S\otimes T),\\
            (g_1,g_2) &\mapsto \left( g_1 , g_2,\tilde \kappa_1 g_1 \cdot \tilde\kappa_2 g_2 \right);\\            
            \text{bw: } M\otimes \calM(S\otimes T)&\to \calM(S')\otimes \calM(T'),\\
...
\end{multline}
In general, these are \textit{not equal}. A sufficient condition for this equality is that the two terms in the square brackets are identically equal to 1; this is implied by choosing $\tilde\kappa_1=\kappa_1$ and $\tilde\kappa_2 = \kappa_2$. (Note that also when $\bar\mu$ has product form, both expressions are equal up to a constant of proportionality. This constant cancels for example in self-normalized importance sampling.)
...
To this end, define the morphism $\epsilon\colon{} 1_{\Optic(\Set)}\to \scr{O}(1_\Stoch)=\bM_+(\{*\})\otimes \calM(\{*\})$ to be the optic with trivial internal state and the following forwards and backwards maps:
\begin{equation*}
    \begin{split}
    ...
    \end{split}
\end{equation*}
that is, forwards $*$ is mapped to the constant function 1 and backwards the measure is simply deleted.
...
Consider a hidden Markov model and suppose that at a hidden node $t$ an observation is not recorded,  perhaps due to a failing sensor. In this case can we can collapse the edges $(\pa(t), t)$ and $(t,\ch(t))$ to $(\pa(t), \ch(t))$ in the latent process. Theorem \ref{thm:functor} ensures that BFFG is not affected by this. 
\end{ex}
...
{\bf Acknowledgment:} We thank Keno Fischer,  Philipp Gabler,  Evan Patterson, Chad Scherrer and Bruno Gavranovi\'c for helpful discussions.  We acknowledge valuable input from the associate editor and anonymous referees on a preliminary version of this paper. 
