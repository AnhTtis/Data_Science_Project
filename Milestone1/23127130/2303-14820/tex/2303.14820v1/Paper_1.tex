...
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage[bookmarks=true,bookmarksnumbered=true,bookmarksopen=true,bookmarksopenlevel=0,
...
\newref{subsec}{refcmd = {Subsection \ref{#1}}}
...
\subjclass[2020]{05C63, 20F65, 37B10, 03D99}
...
two ends. This strenghens a theorem by Brandon Seward.
...
\begin{thm}[Geometric Burnside's problem, \cite{seward_burnsides_2014}]
...
In order to prove \thmref{seward}, Brandon Seward proved a more general
graph theoretic result. 
\begin{thm}[{\cite[Theorem 1.6]{seward_burnsides_2014}}]
...
This result proves \thmref{seward} for one ended groups, and indeed
it says more, as the translation-like action obtained is transitive.
...
\begin{thm}
\label{thm:t2} Let $\G$ be an infinite graph which whose vertices
have finite degree. Then $\G$ admits a transitive translation-like
action by $\Z$ if and only if it is connected and has one or two
ends.
...
A problem left in \cite[Problem 3.5]{seward_burnsides_2014} was to
...
graphs. It follows from Seward's theorem that every finitely generated
group with one or two ends has at least one set of generators $S$
for which $\cay(G,S)$ admits a bi-infinite Hamiltonian path \cite[Theorem 1.8]{seward_burnsides_2014}.
It is not known whether this holds for every Cayley graph \cite[Problem 4.8]{seward_burnsides_2014},
...
actions. This readiliy implies \thmref{seward}. 
\begin{thm}
...
\thmref{t2} generalizes this to infinite graphs, that is, the cube
of a locally finite connected graph with one or two ends admits a
bi-infinite Hamiltonian path. In our proofs we make use of Karaganis
result: we will define bi-infinite Hamiltonian paths locally. 

We mention that \thmref{t1} has been proved in \cite[Section 4]{cohen_strongly_2021},
...
solution is that of finding infinite paths. K\H{o}nig's infinity lemma
...
In \secref{Computable-translation-like-acti} we will provide a computable
version of \thmref{t2}, from which follows that a highly computable
graph admits a transitive translation-like action by $\Z$ if and
only if it admits a computable one. The restriction on ends is essential
to make the proof computable, and indeed we can not show a computable
version of \thmref{t1}. 
...
application is discussed below (\thmref{aplication}).
...
if $H$ has decidable subgroup membership problem (\propref{obvio}). 

The proof of \thmref{maintheorem} is as follows. For groups with
one or two ends, we will show the existence of a computable and transitive
translation-like action by $\Z$, i.e. a computable version of \thmref{t2}.
This has decidable orbit membership problem for the trivial reason
that has only one orbit. For groups with two or more ends, we will
obtain the desired translation-like action from a subgroup isomorphic
to $\Z$ with decidable membership problem. The existence of this
subgroup is a consequence of the computability of the normal form
from Stalling's structure theorem on groups with decidable word problem
(\propref{stallings-descomposicion-calculable}). 
...
this and the following concepts are given in \secref{medvedev}. Intuitively,
the Medvedev degree of a set $P\subset A^{\N}$ measures how hard
is to compute a point in $P$. For example, a set has zero Medvedev
degree if and only if it has a computable point. Note that this becomes
meaningful if we regard $P$ as the set of solutions to some problem. 
...
Hall's matching theorem, and others \cite[Chapter 13]{ershov_handbook_1998-2}.
...
Then \thmref{maintheorem} ensures that this construction preserves
...
of finite type from one group to another \cite{jeandel_translation-like_2015},
to produce aperiodic subshifts of finite type on new groups \cite{jeandel_translation-like_2015,cohen_strongly_2021},
and to study the entropy of subshifts of finite type on some amenable
groups \cite{barbieri_entropies_2021}.

\subsection*{Paper structure}

In \secref{Preliminaries} we fix some notation, and recall some basic
facts on graph and group theory. We also fix our computability setting
for countable sets. In \secref{Translation-like-actions-by} we show
\thmref{t2} and \thmref{t1}. In \secref{Computable-translation-like-acti}
we show \thmref{maintheorem}, and apply this in \secref{medvedev}
to prove \thmref{aplication}.

\subsection*{Acknowledgements}

This paper would not have been possible without the help, guidance,
and careful reading of my two advisors. I am grateful to Sebasti‡n
Barbieri for suggesting that translation-like actions would work to
prove \thmref{aplication}, and for helping me with many details.
I am also grateful to Crist—bal Rojas for providing the computability
background which took me to Medvedev degrees of complexity, and finally
to ask \thmref{aplication}.

This research was supported by ANID 21201185 doctorado nacional, and
the project CID\textit{ }computing with infinite Data. 

\section{Preliminaries\protect\label{sec:Preliminaries}}
...
\subsection{Words and finitely generated groups \protect\label{subsec:preliminaries-Finitely-generated-groups}}
...
be among the numbers $\{0,1,2,\infty\}$, as proved in \cite{freudenthal_uber_1945,hopf_enden_1944}. 
...
\subsection{Computability on countable sets via numberings\protect\label{subsec:computability-on-countable}}
...
with computability notions on the symbolic space $A^{G}$ in \secref{medvedev}.
We consider a unified approach using numberings, a generalization
of Gšdel numberings introduced by Yuri Ershov. A survey on the subject
...
We obtain a numbering of $G$ by \propref{enumerados-sobre}. Indeed,
this is a computable numbering, and it is the only one up to equivalence:
...
relation $s(n)=s(m)$ is decidable, \propref{enumerados-sobre} can
be applied. Thus a numbering of a set $X$ induces a unique numbering
...
\begin{proof}[Proof of \propref{enumerados-sobre}]
...
\end{proof}
%
\begin{proof}[Proof of \propref{numberings-equivalent}]
...
\end{proof}
%
The proof of \propref{isom-calculable} is left to the reader, as
it is identical to the previous one. 

\section{Translation-like actions by $\protect\Z$ on locally finite graphs\protect\label{sec:Translation-like-actions-by}}

In this section we prove \thmref{t2} and \thmref{t1}. For this we
will work with finite and bi-infinite 3-paths. 
\begin{defn}
...
Using this we will construct bi-infinite 3-paths in an appropiate
manner, and this will provide a proof of \thmref{t1} and \thmref{t2}.
...
\begin{proof}[Proof of \lemref{lema-tecnico}]
...
adjacent to some vertex outside $\L$. By \lemref{lema-tecnico} there
is a 3-path $f'$ which is Hamiltonian on $\L$, starts at $u$ and
ends in $w$. We claim that this $f'$ is right-extensible. Indeed,
...
is a vertex $v$ in $\L$ adjacent to $u$. Now we invoke \lemref{lema-tecnico}
...
in \lemref{lema-tecnico} will be important in the following proof. 
...
then then the claim is easily obtained by applying \lemref{existence-right-extensible}
on each of these components. Indeed, by \lemref{existence-right-extensible}
...
\lemref{lema-tecnico}, we will split $p$ aproppiately to obtain
$g$ and $h$. Let $w_{1}$ be a vertex in $\L'$ which is adjacent
to some vertex $v'$ in $\L-\L'$, which exists as $\L$ is connected.
Now we need the second condition in \lemref{lema-tecnico}, which
says that there is a vertex $w_{2}$ in $\L'$ whose distance from
$w_{1}$ is at most 2, and such that $p$ visits consecutively $\{w_{1},w_{2}\}$.
We will assume that $p$ visits $w_{2}$ after visiting $w_{1}$,
the other case being symmetric. As $\L-\L'$ has no finite connected
component, there is a vertex $u'$ in $\L$ which is outside $\L'$
and is adjacent to $v'$. Thus, $w_{1}$ is at distance at most 2
from $u'$, and $w_{2}$ is at distance at most 3 from $v'$. 
...
\end{proof}
%
Observe that when $\G$ has one or two ends, the hypothesis of \lemref{double-extensible-paths}
...
of \corref{one-two-ends}. When we deal with bi-infinite 3-paths,
...
\lemref{double-extensible-paths}. Now assume that we have defined
the bi-extensible 3-path $f_{i}$, and that it visits $v_{i}$. We
define $f_{i+1}$ as a bi-extensible 3-path on $\G$ which extends
$f_{i}$, its domain extends the domain of $f_{i}$ in both directions,
and it visits $v_{i}$. This is possible by applying \corref{one-two-ends}
...
\begin{proof}
By first applying \lemref{existence-bi-extensible} and then iterating
\lemref{double-extensible-paths}, we obtain a sequence of bi-extensible
3-paths $(f_{i})_{i\in\N}$ on $\G$, such that $f_{i+1}$ extends
$f_{i}$ for all $i\geq0$, and such that their domains exhaust $\Z$.
We define a bi-infinite 3-path $f:\Z\to\G$ by setting $f(n)=f_{i}(n)$,
for $i$ big enough. 
...
\end{proof}
%
Now the proof of the following result is by iteration of \lemref{bi-infinte-3-trail-connected-complement}. 
\begin{prop}
...
\lemref{bi-infinte-3-trail-connected-complement} to obtain a bi-infinite
3-path $f_{0}$ such that $\G-f_{0}$ has no finite connected component.
Each connected component of $\G-f_{0}$ is an infinite, connected,
and locally finite graph, so we can apply \lemref{bi-infinte-3-trail-connected-complement}
...
graph $\G_{w}$. By \lemref{bi-infinte-3-trail-connected-complement},
...
\end{comment}
\end{proof}
We now derive \thmref{t1} and \thmref{t2} from this statements in
terms of bi-infinite 3-paths.
%
\begin{proof}[Proof of \thmref{t1}]
 Let $\G$ be a graph as in the statement, and let $f_{i},i\in I$
as in \propref{existencia-3-paths}. We define $\ast:V(\G)\times\Z\to V(\G)$
by the expression
\begin{align*}
...
\begin{proof}[Proof of \thmref{t2}]
 Let $\G$ be a graph as in the statement. By \propref{hamiltonian-3-trail-one-two-ends},
$\G$ admits a Hamiltonian bi-infinite 3-path $f$. This defines a
translation-like action by $\Z$ as in the proof of \thmref{t1},
...
This is stated in \cite[Theorem 3.3]{seward_burnsides_2014} for graphs
...
the proof.
\end{proof}
We end this section by repeating a problem left in \cite[Problem 3.5]{seward_burnsides_2014}.
\begin{problem}
Find necessary and sufficient conditions for a graph to admit a transitive
translation-like action by $\Z$. 
\end{problem}

\thmref{t2} shows that the number of ends and connectedness completely
classify locally finite graphs which admit a transitive translation-like
action by $\Z$. The case of graphs with vertices of infinite degree
may be different, as there is not a uniquely defined notion of ends
there. This is discussed in detail in \cite{diestel_graph-theoretical_2003}. 

\section{Computable translation-like actions by $\protect\Z$ \protect\label{sec:Computable-translation-like-acti}}
...
with decidable orbit membership problem (\thmref{maintheorem}). Let
us recall here the proof scheme. For groups with at most two ends
this is obtained from a computable version of \thmref{t2}. For the
remaining case, we show that the group contains a subgroup isomorphic
to $\Z$ with decidable subgroup membership problem. 

The reader may observe that in the case where the group has two ends
we provide two different proofs for \thmref{maintheorem}. Indeed
a group with two ends is virtually $\Z$, and it would be easy to
give a more direct proof, but the intermediate statements have independent
interest (\thmref{transitive-computable-action-on-computable-graph}
and \propref{stallings-descomposicion-calculable}). 
...
it \thmref{maintheorem} for groups with one or two ends.

\begin{thm}[Computable \thmref{t2}]
...
Joining this with \thmref{t2} we obtain the following corollary. 
\begin{cor}
...
Recall that \thmref{t2} was obtained by constructing a bi-infinite
Hamiltonian 3-path (\propref{hamiltonian-3-trail-one-two-ends}).
...
\begin{proof}
We prove the first claim. By \lemref{semidecidible-complemento-con-componente-finita},
...
\begin{proof}
We now prove the first claim. By \lemref{semidecidible-complemento-con-componente-finita},
...
\end{proof}
%
We can now show tht the bi-infinite Hamiltonian path in \propref{hamiltonian-3-trail-one-two-ends}
can be computed. 
\begin{prop}[Computable \propref{hamiltonian-3-trail-one-two-ends}]
\label{prop:3-trail-hamiltonian-calculable}Let $\G$ be a highly
computable graph satisfying the hypothesis in \thmref{t2}. Then it
admits a Hamiltonian bi-infinite 3-path which is computable.
\end{prop}
...
by applying \corref{one-two-ends} to the graph $\G$ and the 3-path
$f_{i}$. Moreover, it can be computed by searching exhaustively,
the conditions imposed on $f_{i+1}$ are decidable thanks to \lemref{decidible-complemento-conexo-1-end}
and \lemref{decidible-complemento-conexo-2-end}.
...
is computable. 
\end{proof}
We can now prove \thmref{transitive-computable-action-on-computable-graph}.

\begin{proof}[Proof of \thmref{transitive-computable-action-on-computable-graph}]
Let $f:\Z\to V(\G)$ be a bi-infinite 3-path on $\G$ which is Hamiltonian
and computable, which exists thanks to \propref{3-trail-hamiltonian-calculable}.
...
\end{proof}
%
As mentioned before, \thmref{transitive-computable-action-on-computable-graph}
implies inmediately that \thmref{maintheorem} holds for groups with
one or two ends. 
\begin{proof}[Proof of \thmref{maintheorem} for groups with one or two ends]
...
Now \thmref{transitive-computable-action-on-computable-graph} yields
a computable and transitive translation-like action on $(V(\G),d_{\G})$,
...
In this subsection we prove \thmref{maintheorem} for groups with
two or more ends. For this we will show that a finitely generated
...
theorem (see \propref{stallings-descomposicion-calculable} below).

For the reader's convenience, we recall some facts about HNN extensions,
amalgamated products, and normal forms. We refer the interester reader
...
\item Succesive $c_{i}$ alternate between $T_{A}$ and $T_{B}$
\end{enumerate}
...
\end{proof}
%
We can now finish the proof of \thmref{maintheorem}, where the only
remaining step is to verify the computability of the action. 
\begin{proof}[Proof of \thmref{maintheorem} for groups with two or more ends]
Let $G$ be a finitely generated infinite group with decidable word
problem. By \corref{containment-Z-decidable-membership-problem} there
is an element $c\in G$ such that $\langle c\rangle$ is isomorphic
to $\Z$, and has decidable subgroup membership problem in $G$. Thus
the right action $\ast:G\times\Z\to G,$ $(g,n)\mapsto(gc^{n})$ has
decidable orbit membership problem by \propref{obvio}.
...
loss of generality (\propref{numberings-equivalent}) we can endow
$G$ with the numbering obtained from the surjection $(S\cup S^{-1})^{*}\to G,w\mapsto w_{G}$,
and the previous paragraph makes it clear that the fuction $\ast$
is computable.
\end{proof}
%

\section{Medvedev degrees of effective subshifts\protect\label{sec:medvedev}}

In this section we prove \thmref{aplication}. For this we need some
definitions. Let $G$ be a finitely generated group, and $A$ a finite
set. A \textbf{subshift }is a subset $X\subset A^{G}$ which is closed
in the prodiscrete topology, and invariant under the group action
$G\act A^{G}$ by left translations $(g\ast x)(h)\mapsto x(g^{-1}h).$
...
of the set of all patterns as in \propref{enumerados-sobre}, and
we can say that a set of patterns $\F$ is computably enumerable.
...
\subsection{Computability on $A^{\protect\N}$, and Medvedev degrees \protect\label{subsec:Computability-on-A^n}}
...
\subsection{Computability on $A^{G}$ \protect\label{subsec:Computability-on-A^g} }
...
in \subsecref{computability-on-countable}. We recall the following
...
\begin{comment}
\marginpar{cristobal: zparece correcto llamar a esto una representaci—n?}
\end{comment}
...
Recall from \secref{Preliminaries} that a group as in the statement
admits a computable numbering. The next proposition shows that the
computability notions that we obtain on $A^{G}$ do not depend on
the choice of $\nu$. 
\begin{prop}
...
function by \propref{biycalculable} and the fact that $\nu^{-1}\circ\nu':\N\to\N$
...
for $A^{G'}$ as in \defref{gdelta}. If $f:G\to G'$ is a group isomorphism,
then the associated function $F:A^{G'}\to A^{G}$, $x\mapsto x\circ f$
is computable.
\end{prop}

The proof is omited, as it is identical to the previous one by applying
\propref{isom-calculable}. This means that the computability notions
being considered on $A^{G}$ are preserved if we rename group elements
(for example, by taking different presentations of the same group). 
...
computably enumerable. Joining this with \propref{ef-cerrado}, we
obtain:
\begin{prop}
...
\begin{proof}
Denote by $F$ the function above. By \propref{isom-calculable},
...
\]
 This is a computable function in $A^{\N}$ by \propref{biycalculable}.
\end{proof}
\end{comment}
...
as a subshift, and then we use this to prove \thmref{aplication}.
...
\begin{figure}
\begin{center}\includegraphics[width=1\columnwidth]{translation-like-sinpintar}\end{center}
...
$S=\{(\pm1,0),(0,\pm1)\}$.\protect\label{fig:subshift-translation-like-action}}
\end{figure}
...
arrow from $gl$, see \figref{subshift-translation-like-action}.
...
\end{proof}
\begin{figure}[h]
\begin{center}\includegraphics[width=1\columnwidth]{translation-like-pintada-3}\end{center}
...
in \cite{barbieri_entropies_2021,jeandel_translation-like_2015}.
We will write $X_{J}(H,G)$ and $Y[X_{J}(\Z,G)]$ with the same meaning
...
\begin{enumerate}
\item In \cite{jeandel_translation-like_2015}, E. Jeandel proved that when
...
folklore. In the following result we use \thmref{maintheorem} to
show that this construction also preserves the Medvedev degree of
a subshift $Y$ for $H=\Z$, and $J$ big enough. This shows \thmref{aplication},
...
action as in \thmref{maintheorem}. 
...
\end{proof}
\begin{comment}
One way to construct an effectively closed set (maybe not shift invariant)
$X\subset A^{\N}$ is to take a computable numbering of all partial
computable functions $\phi_{e}:\N\to A$ (which is possible because
there is a universal one), and define $A^{\N}-X=\cup[w_{i}]$, where
$w_{i}$ are segments of $\phi_{e}$ in an appropiate manner. If done
appropiately, the set $X$ is nonempty and without computable points.
This idea can be used to construct an effectively closed subshift
without computable configurations \cite[theorem 4]{cenzer_computable_2008},
and this argument has an extension (personal comunication) to finitely
generated groups by S. Barbieri. A construction of this kind leaves
us with a set $X$ such that $X>_{\M}0_{\M}$, but we have no idea
of its degree. Can we relate this degree to some other property in
a meaningful manner?
\begin{enumerate}
\item It is known that for an amenable group $G$, most of subshifts have
low entropy, that is, zero entropy subshifts are generic \cite{frisch_symbolic_2017}.
What is the typical complexity, if we now measure complexity with
Medvedev degrees?
\item The previous question still makes sense, if now ask for the complexity
of the language of subshift. For $X\subset A^{G}$, we define its
language as $L=\{p:K\Subset G\to A|[p]\cap X\ne\emptyset\}$. Thus
effective subshifts are exactly those such that $L^{c}$ is a computably
enumerable set. What is the typical complexity if we now ask for the
language complexity?
\item El subshift de traslacion acciones vale en un grupo recursivamente
presentado. Sera que el subshift de traslacion acciones por Z tiene
un punto calculable
\item No lo probamos aqui, pero el subshift de traslacion acciones por Z
es efectivamente cerrado para un grupo recursivamente presentado.
Hemos probado en ..tla que este subshift tiene un punto calculable,
cuando el grupo tiene wp decidible. Ser{[}a esto ciert?
\item El halls matching theorem, realiza todos los grados Pi1?
\end{enumerate}
\end{comment}

\bgroup\inputencoding{utf8}
