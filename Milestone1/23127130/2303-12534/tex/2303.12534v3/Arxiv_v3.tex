...
% \cite{khoo_solving_2018, wang_pastfuture_2019, li_computing_2019, li_semigroup_2022, strahan2023predicting, hasyim_supervised_2022} 
...
\begin{equation}
    \mathcal{S}^\tau f(x) = \E_x\left[ f(X^{\tau\wedge T}) \right]
\end{equation}
is the ``stopped'' transition operator \cite{strahan_long-time-scale_2021}. We use the notation $\tau\wedge T = \min\{\tau, T\}$.  
The parameter $\tau$ is known as the lag time. While
it is an integer corresponding to a number of integration steps, in
our numerical examples, we often express it in terms of equivalent
time units.

% Computing prediction functions can enable detailed study of kinetics and mechanisms and insight into transition dynamics of complex systems. \cite{strahan2023predicting, finkel_learning_2021, antoszewski_kinetics_2021, finkel_revealing_2022, guo_dynamics_2022}
...
\end{equation}
where $r(x)$ is the right-hand side of \eqref{eq:FK} and $\mu$ is an arbitrary distribution of initial conditions $X^0$ (boundary conditions were enforced by an additional term) \cite{strahan2023predicting}. 
The norm $\lVert\cdot\rVert_\mu$ is defined by $\lVert f\rVert_\mu^2=\langle f,f\rangle_\mu$, where $\langle f,g\rangle_\mu=\int f(x)g(x)\mu(dx)$ for arbitrary functions $f$ and $g$.
The gradient of the residual norm in \eqref{eq:Res} with respect to neural-network parameters $\theta$ can be written
...
% , the appeal of an inexact subspace iteration relative to inexact power iteration is self-evident: rather than finding only the single dominant eigenfunctions/values of the transition operator, we can find the dominant $k$ eigenfunctions/values. 
...
%{\color{red}
%To derive our grid based scheme, we first note that the generator of a reversible overdamped diffusion can be written as:
%\begin{equation}
%    \mathcal{L}f=\beta^{-1}\nabla \ln \pi \nabla f + \beta^{-1}\nabla^2f
%\end{equation}

%To discretize this on a grid with grid vector $\Delta x_i$ for the ith coordinate, we approximate the drift term with the average of a forward and backward difference quotient:
%\begin{widetext}
%\begin{multline}
%    \beta^{-1}\nabla \ln \pi(x^j)\nabla f(x^j) =
%    \sum_i\frac{1}{\beta\Delta x_i} \bigg(\frac{\pi(x^j+\Delta x_i)-\pi(x^j)}{\pi(x^j+\Delta x_i)+\pi(x^j)}\frac{f(x^j+\Delta x_i)-f(x^j)}{\Delta x_i}+\\
%    \frac{\pi(x^j)-\pi(x^j-\Delta x_i)}{\pi(x^j)+\pi(x^j-\Delta x_i)}\frac{f(x^j)-f(x^j-\Delta x_i)}{\Delta x_i}\bigg)
%\end{multline}
%and the diffusion term with a central difference:
%\begin{equation}
%    \beta^{-1}\nabla^2 f(x^j)=\sum_i \frac{f(x^j+\Delta x_i)+f(x^j-\Delta x_i)-2f(x^j)}{\beta\Delta x_i^2}
%\end{equation}
%\end{widetext}
%Adding the above, collecting terms, and substituting the $\pi(x^j)=e^{-\beta V(x^j)}$ gives the expression for the transition matrix we use.
%}
...
\begin{figure}[tb]
    ...
    \caption{Helix-to-helix transition of AIB\textsubscript{9}.  (left) Left- and right-handed helices, which we use as states $A$ and $B$, respectively, when computing predictions.  Carbon, nitrogen, and oxygen atoms are shown in yellow, blue, and red, respectively; hydrogen atoms are omitted.
...
In this section, we provide some further numerical details for the application of our method to spectral estimation and demonstrate the method on the test problems.  For our subspace iteration with ${\cal A}={\cal T}^\tau$, we require estimators for inner products of the form $\langle f, \T{\tau} g\rangle_\mu$.
...
%\begin{equation}
%K_{1:i,i}=\arg \min_{c} \norm[\Bigg]{\sum_{a=1}^i\varphi_{\theta}^{a}c_a-\mathcal{T}^{\tau}\tilde{\varphi}_s^a}_\mu+\gamma_2\sum_{a=1}^{i-1}c_a^2.
%\end{equation}
\begin{equation}
\begin{aligned}
K_{1:i,i}=\arg \min_{c}\frac{1}{n}&\sum_{j=1}^n\left(\sum_{a=1}^i\varphi_{\theta}^{a}(X_j^0)c_a-\tilde{\varphi}_s^a(X_j^{\tau})\right)^2 \\
&+\gamma_2\sum_{a=1}^{i-1}c_a^2.
\end{aligned}
\end{equation}
...
\For{$s = 1\dots S$}
    ...
    \State $K_{1:i,i}\gets\arg \min_{c}\frac{1}{n}\sum_{j=1}^n\left(\sum_{a=1}^i\varphi_{\theta}^{a}(X_j^0)c_a-\tilde{\varphi}_s^a(X_j^{\tau})\right)^2+\gamma_2\sum_{a=1}^{i-1}c_a^2 $
...
\begin{figure*}[hbt]
    ...
    % [ $\tau$ IS NOW AN INTEGER, BUT MAYBE WE CAN KEEP THE PLOTS AS IS IF WE USE THE LANGUAGE ``$\tau$ corresponds to'' IN THE TEXT]
    }
...
% We expect that learning $\lambda_5$ is challenging because of the small gap between $\lambda_4$ and $\lambda_5$.
...
When the boundary conditions are non-zero, the orthogonalization procedure is only applied to the basis functions ${\{\varphi^a_{\theta}\}}_{a=2}^k$, and $K_{a 1} = I_{a 1}$,  the $a1$ element of the identity matrix. 
...
\For{$s = 1\dots S$}
    ...
    \State $K_{1:i,i}\gets\arg \min_{c}\frac{1}{n}\sum_{j=1}^n\left(\sum_{a=1}^i\varphi_{\theta}^{a}(X_j^0)c_a-\tilde{\varphi}_s^a(X_j^{\tau \bmin T_j})\right)^2+\gamma_2\sum_{a=1}^{i-1}c_a^2 $
...
For the M\"uller-Brown model with a deepened intermediate basin, the second eigenvalue of $\mathcal{S}^\tau$ is of order $10^{-4}$ for a lag time of $\tau=1000$ steps or $1\,{\delta_t}^{-1}$  (while the first is near one as discussed above). We therefore choose $k=2$ with $\tilde{\varphi}^2_1$ initialized as a random linear combination of coordinate functions as in previous examples. 
...
\For{$s = 0\dots (\tau / \tau_{\rm mem}) - 2$}
    \State $M^s\gets C^{s+2}-C^{s+1}-A(C^0)^{-1}C^{s+1}-\sum_{j=0}^{s}M^j(C^0)^{-1}C^{s-j}$
\EndFor
...
%{
%Motivated by the maximum-likelihood procedures discussed in Section \ref{sec:softplus} \cite{peters_obtaining_2006, peters2007extensions, jung_artificial_2019, jung_machine-guided_2023}, we also compare with logistic regression to learn a model for the binary variable $\mathbbm{1}_B(X^{T})$, where $T$ is the time the process enters $A\cup B$, and the inputs to the model are the coordinates $y$ and $z$.  The data for the logistic regression is a single long trajectory of length 125,000 timesteps, which is equal to the total number of timesteps in our $\tau=2.5\,\delta_t^{-1}$ short-trajectory data set. 
%In the limit of infinite lag time, a single Richardson iteration using the softplus loss function is equivalent to logistic regression, and so we use the same hyperparameters and architecture as for the other Richardson iteration test in this section (Table~\ref{tab:params}).   The results are shown in Figure \ref{fig:Logistic}; the RMS error is 0.18, and the logit error is 13.8, compared with values of 0.028 and 1.94 respectively for the subspace iteration (Figure~\ref{fig:SI_Q_error}).  The logistic regression is thus unable to achieve a good estimate of the committor for this system either in the sense of RMS or logit error.  
%}
...
% {[WHAT DOES ``random'' INITIALIZATION MEAN IN THE TABLE? SPECIFY HERE. WHY NOT $\mathbbm{1}_B$?]}
...
The
overall error in the committor estimate is defined as
\clearpage
\begin{equation}
\label{eq:emp_error}
q\ \textrm{error}=
...
We find that the performance steadily drops as the number of trajectories is reduced and degrades rapidly for the data sets subsampled more than 20-fold (Figure~\ref{fig:emp_error}, bottom), corresponding to about 7 $\mu$s of total sampling.}
...
In Figure~\ref{fig:aib_mfpt}, we show the MFPT obtained from Algorithm \ref{alg:forecast} with $k=5$ and the $L^2_\mu$ loss function. 
Initially we set $\tilde{\varphi}^1_1$ equal to an arbitrary positive function (we use $5\mathbbm{1}_A$) and $\tilde{\varphi}^a_s$ for $a>1$ to a random linear combination of coordinate functions. In Figure~\ref{fig:aib9_mfpt} we examine the convergence of the MFPT from the left-handed helix to the right-handed helix for the MFPT computed with $k=1$ (pure Richardson iteration)
and $k=5$.
The horizontal line indicates a MFPT of about 56~ns estimated from the long reference trajectories.
...
\begin{figure*}[hbt]
    ...
    %Projection onto the dPCs of (left) the empirical MFPTs (i.e. a 2-D histogram on the dPCs of the random variable $T(X_t)$ obtained from our long trajectories) (middle) representative neural network MFPTs using $\tau$ corresponding to 400~ps.
    %(right) Comparison between empirical MFPTs and neural network MFPTs trained with $\tau$ corresponding to 400~ps. 
...
%{
%While the ability to compute from short trajectories is itself an advantage, even in cases when trajectories that reach the reactant and product states are available, 
%We compared our inexact iterative scheme to committor approximation based on maximum-likelihood estimation and related approaches \cite{Ma2005nn, peters_obtaining_2006, peters2007extensions, hu2008two, jung_artificial_2019, jung_machine-guided_2023, miloshevich_probabilistic_2023}.
%Maximum-likelihood estimation is closely related to ``Monte-Carlo'' approximation in reinforcement learning \cite{sutton_reinforcement_2018}, and, as noted previously, to the inexact Richardson iteration that we propose here with $\tau \rightarrow \infty$.
%We found that our scheme outperformed maximum-likelihood estimation significantly in the case of the M\"uller-Brown model but not AIB$_\textsubscript{9}$.  The difference between these systems is the number of trajectories reaching states $A$ and $B$:  there are few for the former relative to the latter.  We thus expect our scheme to be preferable to maximum-likelihood estimation when generating such trajectories is difficult, as is often the case in complex systems.  This hypothesis deserves further investigation in the contexts of specific path-sampling algorithms.
%}
