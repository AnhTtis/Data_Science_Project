...
% \subsection*{Proof of \cref{thm::OptSolf2}:}
...
% That is, reaching a state $X''$ from $X'$, such that $X''_1 = V^*$ has a probability of at least $\Omega(2^{-2 \cdot \opt \cdot W})$, where  selecting $X'$ in $\P$ has probability $\Omega(1/n^2)$.
% Thus, once $X'$ is contained in $\P$ it takes in expectation $\O\left(n^2 \cdot 4^{\opt \cdot W} \right)$ iterations reaching $X''$.
% As a result, the algorithm \algGlobalSemoAlt needs in total $\O\left(n^3(\log n + \opt) + n^2 \cdot 4^{\opt \cdot W} \right)$ iterations finding an optimal $W$-separator in expectation.


\subsection*{Proof of \cref{lemma::balancedExpansionStrictlyReducibleExist}}
%To prove \cref{lemma::balancedExpansionStrictlyReducibleExist} 
...
%a relation of the LP-objective to a sequence of minimal strictly reducible pair.
