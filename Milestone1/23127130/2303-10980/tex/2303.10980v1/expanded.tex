...
logic has available $k$ ``blue'' variables to address edges, an unbounded number of
``red'' variables to address vertices of a hypergraph, and atomic formulas $E(e,v)$ to express that
vertex $v$ is contained in edge $e$.
...
The design of the logic $\GCk$ is somewhat inspired by the \emph{guarded fragment of
...
This is somewhat analogous (although not tightly related) to the following classical results.
...
graphs used in Dvořák's proof, taylored towards our setting.
...
\paragraph*{Generalised Hypertree Decompositions:}
...
\hom{\J'}{\I'}$. 
The remainder of this section is devoted to showing that such a $\J'$ indeed exists.

The overall approach is as follows: Consider a ghd
$D=(T,\DBag,\DCover)$ of $J$ with $\w{D}\leq k$.
For each tree-node $t\in \V{T}$ and each blue node $e\in \bVJ$
consider the set of red nodes $s(t,e)\deff N_{\J}(e)\cap\DBag(t)$.
Let $S$ be the set consisting of all these sets $s(t,e)$, i.e.,
$S\deff \setc{s(t,e)}{t\in\V{T},\; e\in \bVJ}$.
For each $s\in S$ we create a new blue node
$e'_{s}$ along with edges from $e'_{s}$ to every
red node $v\in s$; i.e., $N(e'_s)=s$.
For every
$t\in \V{T}$ we let $\DCover'(t)\deff \setc{e'_{s(t,e)}}{e\in\DCover(t)}$.
Clearly, $D'\deff (T,\DBag,\DCover')$ satisfies
the conditions
\ref{def:hypertree_decomposition:connectedness_of_vertices} and
\ref{def:hypertree_decomposition:covering} of
Definition~\ref{def:hypertree_decomposition} and
condition
\ref{def:entangled_hypertree_decomposition:precision} of
Definition~\ref{def:entangled_hypertree_decomposition}.
But
condition~\ref{def:entangled_hypertree_decomposition:connectedness_of_edges}
might still be violated.
To fix this let us, for each $s\in S$, write $m_s$ to denote the
number of connected components of the subgraph
$T_{e'_s}$, and let $V_{s,1},\ldots,V_{s,m_s}$
be the sets of tree-nodes of these connected components.
For each $s\in S$ with $m_s\geq 2$, we replace the single blue node
$e'_s$ with $m_s$ new blue nodes $e'_{s,1},\ldots,e'_{s,m_s}$, and connect
each of them to all red nodes in $s$, i.e., $N(e'_{s,i})=s$ for all
$i\in[m_s]$. For each $t_i\in V_{s,i}$ we replace $e'_s$ with
$e'_{s,i}$ in $\DCover'(t_i)$. 

Let us write $\DCover''$ to denote the accordingly modified version of
$\DCover'$.
It is straightforward to verify that $D''\deff (T,\DBag,\DCover'')$  satisfies
the conditions
\ref{def:hypertree_decomposition:connectedness_of_vertices} and
\ref{def:hypertree_decomposition:covering} of
Definition~\ref{def:hypertree_decomposition} and
the conditions
\ref{def:entangled_hypertree_decomposition:precision} and
\ref{def:entangled_hypertree_decomposition:connectedness_of_edges}
of Definition~\ref{def:entangled_hypertree_decomposition}.

Let $J''$ be the extension of $J$ by all the new blue nodes and their
associated edges. We can turn $D''$ into an ehd of $J''$ as follows:
We consider every blue node $e$ of $J''$ for which there does not exist any
$t\in\V{T}$ with $e\in\DCover''(t)$. Since $D$ is a ghd of $J$ we know
that there exists a $t\in\V{T}$ with $N_{J''}(e)\subseteq\DBag(t)$. We fix
one particular such $t$ and
create a new tree-node $t'_e$ that we insert into $T$ as a new leaf
adjacent to $t$, and we let $\DBag(t'_e)\deff N_{J''}(e)$ and
$\DCover''(t'_e)\deff \set{e}$.
Let us write $T^*$, $\DBag^*$, and $\DCover^*$ for the accordingly
modified versions of $T$, $\DBag$, and $\DCover''$. It is
straightforward to verify that $D^*\deff (T^*,\DBag^*,\DCover^*)$ is an
ehd of $J''$. Furthermore, $\w{D^*}\leq\w{D}$. Hence, $J''\in\IEHW{k}$.

Recall that our aim is to find a $J'\in\IEHW{k}$ with $\hom{\J'}{\I}
\neq \hom{\J'}{\I'}$.
In case that $\hom{\J''}{\I}\neq \hom{\J''}{\I'}$, we are done by
letting $J'\deff J''$.
But there is no guarantee that indeed
$\hom{\J''}{\I}\neq \hom{\J''}{\I'}$.
To provide such a guarantee, for each $s\in S$ we do not just insert
$m_s$ copies of $e'_s$, but we insert a suitable number $n_s\geq m_s$
of copies of $e'_s$ (each of them connected with all the red nodes in $s$).
With the same reasoning as above, the resulting incidence graph $\J'$
belongs to $\IEHW{k}$.
The remainder of this section is devoted to showing that there exist suitable
numbers $n_s\geq m_s$, for all $s\in S$, such that the
resulting incidence graph $\J'$ satisfies $\hom{\J'}{\I}\neq
\hom{\J'}{\I'}$.
We obtain these numbers $n_s$ by successively applying the following
Lemma~\ref{lemma:adding_edges_keeps_distinguishing} for each $s\in S$.
For formulating and proving the lemma, the following notation is
convenient: For an arbitrary incidence graph $\J$, 
...
\noindent
From Claim~1
(for a proof see Appendix~\ref{appendix:generalization_to_hw})
we obtain: 
...
\noindent
See Appendix~\ref{appendix:generalization_to_hw} for a proof of
Claim~2.
...
Lemma~\ref{lemma:adding_edges_keeps_distinguishing} and
the proof of Theorem~\ref{thm:EHWandIGHW}.
\end{proof}  
...
And
$f \union g$ denotes the partial function $h$ with
...
To address \emph{blue} nodes, we have available $k$ \emph{blue
  variables} $\vare_1,\ldots,\vare_k$. To address \emph{red} nodes, we have
available countably many \emph{red variables} $\varv_1,\varv_2,\varv_3,\ldots{}$.
This leaves us with the following sets of variables:
$\VarB\deff \set{\vare_1,\ldots,\vare_k}$, $\VarR\deff \setc{\varv_i}{i\in\NNpos}$, and $\Var\deff \VarB\cup\VarR$.
...
The formulas of our logic will be annotated with \emph{guard
  functions}, i.e., \emph{partial functions} $g:\NNpos\pto[k]$ with
finite domain $\Dom{g}$.
...
We define certain operations on $k$-labeled incidence graphs. First,
we provide a way to modify the labels (observe that these
modifications do not necessarily preserve real guards). 
...
\begin{enumerate}[$-$]
\item
  $\reclaimR{\kLI}{\myR}\deff(\I,r',b,g')$ with $r'\deff r-\myR$, $g'\deff g-\myR$
  (i.e., we remove from the red nodes all the labels in $\myR$).
  
\item
  $\reclaimB{\kLI}{\myB}\deff(\I,r,b',g)$ with $b'\deff b-\myB$\\
  (i.e., we remove from the blue nodes all the labels in $\myB$).
...
We want to use the labels to combine two $k$-labeled incidence graphs
into a single one. This is achieved by merging nodes that carry the
same label: 
...
The following lemma's proof is straightforward.
...
\noindent \textbf{Recursive cases}:
Let $\kLI = (\I, r, b, g)\in \GLIk$.
\begin{enumerate}
\setcounter{enumi}{\value{KGuardedCounter}}
\item $\reclaimR{\kLI}{\myR} \in \GLIk$ for every 
$\myR \subseteq \Def{r}$.
\item\label{item:GLI-reclaimB} $\reclaimB{\kLI}{\myB} \in \GLIk$ for every 
$\myB \subseteq \Def{b} \setminus \Img{g}$.
\item\label{item:GLI-switch} $\switch{\kLI}{f} \in \GLIk$ for every transition $f$ for $g_\kLI$.
\item $\glue{\kLI}{\kLI'} \in \GLIk$ for every $\kLI'=(\I',r',b',g')\in\GLIk$ such that $g$ and $g'$ are compatible.
\end{enumerate}
\end{definition} 
...
of the 
\emph{quantum graphs} of Lovász and Szegedy \cite{Lovasz2008}, taylored towards our setting.
...
This section's goal is to prove Theorem~\ref{thm:GCkVsHomomIndist}.
The directions ``$\Longleftarrow$'' and ``$\Longrightarrow$'',
respectively, are achieved by the following
Lemmas~\ref{lemma:HomToFormula} and \ref{lemma:FormulaToHom}. 
For formulating and proving the lemmas, we need the following
notation. 
...
We proceed by induction based on Definition~\ref{def:k_guarded_incidence_graphs}.
...
We proceed with the second lemma necessary for the proof of
Theorem~\ref{thm:GCkVsHomomIndist}.
...
Now, the proof of Theorem~\ref{thm:GCkVsHomomIndist} can easily be achieved 
by using Theorem~\ref{thm:IEHWkIsGLIk}
and the Lemmas~\ref{lemma:HomToFormula} and \ref{lemma:FormulaToHom}.
...
For our proofs it was crucial to consider the (severely restricted) \emph{entangled
hypertree decompositions} (ehds, for short) instead of the usual
generalised hypertree decompositions.  To the best of our knowledge,
ehds have not been studied before. By definition, $\IEHW{k}$ is a
subclass of $\IGHW{k}$; and according to Theorem~\ref{thm:EHWandIGHW}
homomorphism indistinguishability coincides for both classes.
For $k=1$ it is easy to see that $\IEHW{1}=\IGHW{1}$, which is known
to be precisely the class of $\alpha$-acyclic hypergraphs.
But many further questions concerning ehds remain open:

For $k\geq 2$: \,Is $\IEHW{k}$ a strict subclass of $\IGHW{k}$?
We know that  $\ghw{H}\leq \ehw{H}$ for all hypergraphs $H$.
Is $\ehw{H}$ tightly bounded by $\hw{H}$, i.e., 
can we find a function $f$ such that $\ehw{H} \leq  f(\hw{H})$?  
...
believe that there should also be a generalisation of Cai, Fürer and
Immerman's result \cite{Cai1992}, i.e., 
a variant of the Weisfeiler-Leman
algorithm working on hypergraphs, whose distinguishing power matches
...
\section{Details omitted in Section~\ref{sec:generalization_to_regular_homomorphisms}}
\label{appendix:generalization_to_regular_homomorphisms}


\subsection*{Details on Böker's Theorem}
{
...
\begin{enumerate}[wide]
	...
	\item Let $\kLI$, $\kLI'$ be $k$-labeled incidence graphs and
	...
	\item Let $\kLI$, $\kLI'$ be $k$-labeled incidence graphs and let $\myB \subseteq \Dom{b_{\kLI}}$ and $\ell \isdef |\myB|$. It holds that 
	\[
...
\begin{enumerate}
	...
	\item The same argument as in the previous case works for this claim as well.
	\item This follows directly from 1) and 3).\qed\textsubscript{Lemma \ref{lem:hom_of_glued_is_product}}
\end{enumerate} 
\medskip
...
\begin{proof}[\textbf{\upshape{Proof of Lemma~\ref{lemma:HomToFormula}}}] \ \\
We proceed by induction based on Definition~\ref{def:k_guarded_incidence_graphs}.
\smallskip
