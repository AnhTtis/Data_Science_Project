...
\newcommand\map[1]{\textcolor{magenta}{#1}}
...
\affiliation{University of California, Los Angeles and Santa Fe Institute}
...
The computational investigation of Fermi, Pasta, Ulam, and Tsingou of arrays of nonlinearly coupled oscillators has led to a wealth of studies in nonlinear dynamics. Most of these studies have considered arrays of homogeneous oscillators, even though there are inherent heterogeneities between the oscillators in real-world arrays. Well-known FPUT phenomena, such as energy recurrence, can break down in such heterogeneous systems. In this paper, we present an approach to recover recurrence in FPUT systems in the presence of oscillator heterogeneities. We examine oscillator variabilities in FPUT systems with cubic nonlinearities, and we demonstrate that centrosymmetry in oscillator arrays may be an important source of recurrence.
\end{abstract}
...
The Fermi--Pasta--Ulam--Tsingou (FPUT) system has been studied widely since the original investigation of one-dimensional (1D) FPUT systems in the 1950s~\cite{Fermi1955,Dauxois2005,Flach2005,Miloshevich2009,Midtvedt2014,Lvov2018,Lewis2018,Kuzkin2020}. A 1D FPUT system consists of a chain of harmonic oscillators that are coupled to each other by nonlinear springs. In the original FPUT numerical experiments~\cite{Fermi1955}, the system was excited in the first mode, with the expectation that the energy of the first mode would spread to higher modes and lead to equipartition of energy among the modes. Instead, however, it was observed numerically that the energy eventually returned to the first mode after initially spreading to higher modes.
Based on these pioneering numerical computations, it also seemed that such ``recurrence'' of energies may continue indefinitely. 
Numerous studies have investigated this seemingly paradoxical recurrence \cite{Tuck1972,Berman2005,Zabusky1965,VanSimaeys2001}, and researchers have even observed very long ``superecurrences'' in  numerical studies of FPUT systems~\cite{Tuck1972,Pace2019}

Despite the wealth of theoretical and computational studies of FPUT systems, there are very few studies of such systems in laboratory experiments~\cite{Goossens2019,Pierangeli2018}. One possible reason for this, as discussed in the numerical study of Nelson et al.~\cite{Nelson2018}, is the inherent variability of objects in real life. It is very difficult to build homogeneous elements for laboratory experiments, so one can expect individual oscillators to have heterogeneities even when one attempts to make them homogeneous. Nevertheless, most of studies of FPUT systems use homogeneous oscillators arrays, with all oscillators assumed to be identical.
...
uses to investigate the best-case and worst-case performance of circuits \cite{Gurnett1957}. Such variability, which is also often called a ``mismatch'', introduces heterogeneities in component values within some bounds.
Nelson et al.~\cite{Nelson2018} studied the effects of such variabilities, which they modeled by incorporating random discrepancies in parameter values, in a 1D FPUT array with a quadratic nonlinearity. 
They observed that energy recurrence breaks down as variability levels reach typical real-world values (such as those in electronic systems). Zulkarnain et al.~\cite{ZULKARNAIN2022} extended these results by conducting a mathematical analysis of the relationship between chaotic behavior and variabilities in FPUT systems.

The presence of typical variabilities in most experimental settings limits the ability to observe recurrences in FPUT systems. Therefore, in the present paper, we explore an approach to recover recurrences in heterogeneous FPUT arrays, despite the presence of variabilities.  We demonstrate our approach using a 1D FPUT system with a cubic nonlinearity (i.e., the FPUT-$\beta$ model). We analyze the proposed approach to recurrence recovery by examining centrosymmetric FPUT systems with heterogeneous oscillators.
...
\end{align}
where $x_i$ denotes the displacement of the $i$th oscillator from its original position and the parameter $\beta$ describes the coupling strength between neighboring oscillators. In Fig.~\ref{fig:Idealsituation}, we show a simulation that has the well-known recurrence. In this simulation, in which we excite only the first mode of the system, the initial energy of the first mode of the system spreads to its higher odd-order modes; it then eventually returns to the first mode \cite{Fermi1955}. This cycle then repeats. These simulations use the initial condition  $x_i = \sin\left[\pi i/(N+1)\right]$ and $\dot x_i = 0$ (with $i \in \{1, \ldots, N\}$).


\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{ideal.pdf}
	\caption{Recurrence of a cubic FPUT system with $N = 128$ oscillators, a nonlinear coupling strength of $\beta = 8$, and no energy in the even modes.
	}
	\label{fig:Idealsituation}
\end{figure}

The recurrence in Fig.~\ref{fig:Idealsituation} arises from an FPUT array in which all oscillators are identical. However, in the real world, it is extremely difficult (and, in fact, essentially impossible) to build uniform systems. To model a real-world FPUT array, one can introduce tolerances (and hence variabilities) in the individual oscillators of the array. We thus consider the heterogeneous real-world FPUT-$\beta$ system
...
In our simulations of the heterogeneous FPUT-$\beta$ array~\eqref{cubictol}, we use the initial condition $x_i = \sin\left[\pi i/(N+1)\right]$ and $\dot x_i = 0$, which excites the system in its first mode. We use tolerance values from electrical engineering, in which typical components have reported tolerances of $\pm 0.1\%$, $\pm 1\%$, $\pm 5\%$, and $\pm 10\%$~\cite{Horowitz2015}. Manufacturing-induced variabilities generally follow a Gaussian distribution \cite{Nelson2018,James1956,IEC2016}. Therefore, we assume that the variabilities in our oscillators follow Gaussian distributions. Again using industry standards from electrical engineering, a $\tau\%$ tolerance signifies that the tolerance $t_i$ has a Gaussian distribution with a mean of $1$ and a standard deviation of $\sigma = (1/3) \times 0.01 \tau$. Therefore, with the $6 \sigma$ bounds, which are used widely in manufacturing, $99.73\%$ of the values of manufactured devices are in the interval $[1 - 0.01 \tau, 1 + 0.01 \tau]$.

We conduct numerical simulations of the FPUT-$\beta$ system of equation~\eqref{cubictol} by using a 4th-order Runge--Kutta algorithm from the built-in MATLAB code libraries and extending the code of Dauxois et al. \cite{Dauxois2005}. In a typical cohort of oscillators, the tolerance parameter values are distributed randomly, so a deterministic study is impractical. Therefore, we use Monte Carlo techniques to investigate the effects of variabilities. We do a large number of simulations of the same system, however, with different parameter values. We draw these parameter values from a Gaussian distribution, which is akin to the typical process that is used in design verification of integrated circuits \cite{Kim2010}. After simulating equation \eqref{cubictol} 100 times, with different sets of tolerance values $t_i$ with a known spread, we plot the energy of various modes in each of these simulations, analogously to the homogeneous FPUT-$\beta$ array of equation \eqref{cubic} of Fig.~\ref{fig:Idealsituation}. Nelson et al.~\cite{Nelson2018} investigated variability in 1D FPUT systems with quadratic coupling for a variety of array sizes and tolerance values. They observed a breakdown in energy recurrence at typical real-world tolerance values. Therefore, it is unsurprising that we observe a similar recurrence breakdown in 1D FPUT arrays with cubic coupling. 

In Fig.~\ref{fig:tol101}(a), we show one typical simulation of the FPUT-$\beta$ array \eqref{cubictol} with $N = 128$ oscillators, a nonlinear coupling strength of $\beta = 8$, and a tolerance of 5\%. In contrast to the homogeneous FPUT-$\beta$ array \eqref{cubic}, we no longer observe much energy recurrence. This lack of recurrence is characteristic of 85 of the 100 total simulations. With tolerances of 10\%, only 3 of the 100 simulations of \eqref{cubictol} still have notable energy recurrence. However, with tolerances of 0.1\% (i.e., very low tolerances), we observe energy recurrences in most of our simulations. 

In a homogeneous 1D FPUT array with cubic coupling, we do not expect even modes to receive energy during recurrence (see Fig.~\ref{fig:tol101}(c,d))~\cite{Fermi1955}. This differs from homogeneous 1D FPUT array with quadratic coupling, where both even and odd modes receive energy during recurrence. However, in the heterogeneous cubic FPUT array, the even modes also receive energy (see Fig.~\ref{fig:tol101}(b)). This observation is not surprising, but it is still worth illustrating.

\begin{figure*}[htbp]
	\centering
	\includegraphics[width=1\textwidth]{evenmode.pdf}
	\caption{Energy recurrence in a 1D FPUT-$\beta$ array with $N = 128$ oscillators and a nonlinear coupling strength of $\beta = 8$. In panels (a) and (b), we show results for a heterogeneous array with a tolerance of $\pm 5\%$; in panels (c) and (d), we shows results for a homogeneous array.  We show the energy of (a, c) odd modes and (b, d) even modes (with the first mode as a reference). 
	}
	\label{fig:tol101}
\end{figure*}	
...
In our earlier simulations, as well as in those by Nelson et al., \cite{Nelson2018}, there was no predetermined order in the exact locations of the tolerance values in the oscillator array. Indeed, one does not expect any specific ordering of tolerance values in a typical real-world system. We hypothesize that the arbitrariness of the locations of the tolerance values, in tandem with the magnitudes of these values, may be a source of recurrence breakdown. To evaluate this hypothesis, we examine whether or not ordered tolerance values can help restore energy recurrence. This approach of structured heterogeneities was also used by Fraternali et al.~\cite{fraternali2010} in a computational study of chains of Hertzian particles.

We again conduct Monte Carlo simulations of equation \eqref{cubictol}. In Fig.~\ref{fig:recover}(a), we show a typical example with a tolerance of 5\%. In Fig.~\ref{fig:recover}(b), we show the locations of individual tolerances values across the 128 oscillators of this FPUT array. To explore our hypothesis, we rearrange the array with the same tolerance values, but now we place them in an ascending order (see Fig.~\ref{fig:recover}(d)). Simulating this system with the oscillators in this order recovers much of the energy recurrence (see Fig.~\ref{fig:recover}(c)). This result suggests that a key reason for the lack of recurrence may the arbitrary locations of the tolerance values. It also suggests that one should be able to recover much of the energy recurrence of an FPUT array by measuring an appropriate characteristic, such as the tolerance value, of its individual oscillators and then ordering them based on this characteristic. Recall that 85 of the 100 simulations with arbitrarily placed tolerance of 5\%, do not exhibit notable energy recurrence. By placing the tolerances in ascending order, we are able to recover recurrence in 23 of these 85 simulations. Consequently, with the ascending order of tolerances, we observe recurrence in 38 of the 100 simulations.


\begin{figure*}[htbp]
	\centering
	\includegraphics[width=1\textwidth]{retechnique.pdf}
	\caption{Simulations of 1D cubic FPUT arrays with $N = 128$ oscillators, a nonlinear coupling strength of $\beta = 8$, and $\pm 5\%$ tolerance with (a, b) randomly located and (c, d) ordered tolerance values.}
	\label{fig:recover}
\end{figure*}
...
We see that perturbing the oscillators at locations 1, 32, 97, and 128 of the array does not lead to a breakdown in recurrence. This is interesting but unsurprising, as these locations are the nodes of the second and fourth mode of a corresponding linearly coupled array. Ideally, the even modes of homogeneous 1D FPUT-$\beta$ array should not receive energy during recurrence. Therefore, we expect that perturbing the oscillators at the nodes of the modes should not have a major effect on energy recurrence. This discussion also suggests that perturbing the system at some locations may significantly increase the energy transfer to higher modes. This may help explain the breakdown of the energy recurrence.

%%%%%

\section{Centrosymmetric distribution of oscillators} \label{centro}

The energy transfers from the first mode to higher modes (see Fig.~\ref{fig:energychange}) is symmetric around the center location of a 1D FPUT-$\beta$ array. 
 The reordered ascending configuration, which is ordered by tolerance values, is close to being centrosymmetric. We hypothesize that centrosymmetry can lead to a recovery of recurrence.
 In a centrosymmetric $N$-oscillator array with even $N$, if the oscillator at location $N/2 - i$ has a parameter value that differs from the mean by $+\varpi$, then the associated parameter value of oscillator at location $N/2 + i$ differs from the mean by $-\varpi$. In Fig.~\ref{fig:centrosy}, we show a centrosymmetric distribution for an array of $N = 128$ oscillators. This distribution has a mean of 1 and centrosymmetry around oscillators 64 and 65.
 
 
 \begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{distri.pdf}
	\caption{Energy spread from the first mode with a 10\% tolerance perturbation at different locations of a 128-oscillator 1D FPUT-$\beta$ array with a nonlinear coupling strength of $\beta = 8$.}
...
We conduct simulations to investigate the use of centrosymmetry in the recovery of recurrence in FPUT arrays with tolerance. We again use FPUT-$\beta$ arrays with $N = 128$ oscillators, a nonlinear coupling strength of $\beta = 8$, and a tolerance of 5\%. With arbitrarily place tolerance values from a Gaussian distribution, only 15 of 100 total simulations have notable energy recurrence. When we arrange the oscillators in ascending order of their tolerance values, 38 of 100 total simulations have notable energy recurrence. When we arrange the oscillators centrosymmetrically, we observe energy recurrence in 76 of 100 total simulations. The centrosymmetric distribution has a stronger effect on energy recurrence than ordering them by tolerance values.

For tolerance levels of 10\%, only 3 of 100 total simulations with arbitrarily distributed tolerances have notable energy recurrence. However, rearranging the oscillators in ascending order of tolerance values or using a centrosymmetric arrangement yields recurrence in 25 of 100 total simulations. This recovery is not as good as the recovery at a 5\% tolerance. Nevertheless, even with very large tolerance levels, both the centrosymmetric distribution and ascending order of tolerance values can reduce the impact of tolerance on energy recurrence of the FPUT arrays.

%%%%%

\section{Conclusions} \label{conc}

We studied 1D Fermi--Pasta--Ulam--Tsingou (FPUT) arrays with cubic nonlinear coupling between oscillators and tolerances in the oscillators. Increasing tolerances, which increases the amount of heterogeneity of an array, breaks the energy recurrence of these FPUT-$\beta$ arrays. With tolerances, we also observed that energy can transfer to even modes in an FPUT-$\beta$ array; this cannot occur in the associated homogeneous FPUT system. Given the tolerance values of a set of oscillators, we rearranged them (1) by ordering them from smallest to largest and (2) centrosymmetrically, and we showed in both cases that such rearrangement can mitigate the loss of energy recurrence. We also observed that the tolerance of individual oscillators can play a significant role in the breakdown of recurrence.
...
ZL and BC thank German Federal Ministry of Education and Research (Bundesministerium f\"ur Bildung und Forschung) and European Commission for funding using the Distributed Artificial Intelligence Systems (DAIS project) (Grant agreement ID 101007273).

%%%%%

%\renewcommand\refname{Reference}
%\bibliographystyle{apsrev4-1}
%\bibliography{FPUTre-v07.bib}
