...
\end{align}
In the above expression, matrix operations are modulo $2$. For simplicity, we are not concerned with normalising codeword states. 
...
\end{enumerate}
\end{framed}
The above algorithm requires that $B|\mathbf{0}\rangle =|\mathbf{0}\rangle$. If this is not the case, let $B|\mathbf{0}\rangle = \omega^p|\mathbf{0}\rangle$, run the algorithm using $B':=\omega^{-p}B$ and adjust for phase on the result. 
...
\end{enumerate}
\end{framed}
\begin{example}[Constructing CSS Codes with Transversal Logical Controlled Phase Operators]
