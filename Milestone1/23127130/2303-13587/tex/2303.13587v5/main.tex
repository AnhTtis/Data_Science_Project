\documentclass[a4paper,onecolumn,11pt,accepted=2023-03-05]{quantumarticle}
...
\author{Ruge Lin}
\affiliation{Quantum Research Centre, Technology Innovation Institute, United Arab Emirates.}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/bounds.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/EC_bis.pdf}
...
\begin{figure}
\centering
\includegraphics[scale=0.9]{Graph/EC_circuit.jpg}
...
\begin{figure}
\centering
\includegraphics[scale=0.9]{Graph/Hash_circuit.jpg}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/Grover_bis.pdf}
...
In this section, we investigate the entanglement trajectory of the Shor algorithm with the most up-to-date version for minimizing the number of qubits provided by Beauregard \cite{griffiths1996semiclassical,parker2002entanglement,beauregard2002circuit}, which only needs $2n+3$ qubits. The QFT is performed semi-classically with one controlling qubit, as shown in FIG. \ref{Shor_circuit}. The numerical simulation with Qibo is also available on GitHub \cite{Github_shor}. Since the space of QFT has been reduced to one qubit, the entanglement between this qubit and the rest of the system is upper bounded by $1$, and it would be more convincing to study this circuit with another partition. We use the bi-partition between the register that stores the output of the QME ($n$ qubits) and the rest of the qubits to track the entanglement trajectory and sample $\rho_A$ in the middle of each QME, as shown in FIG. \ref{Shor_circuit_ME}. The size of the subsystem is $\alpha=2^{n}$, and the size of the complementary system is $\beta=2^{n+3}$. The result is shown in FIG. \ref{Shor_bis}. The entanglement trajectories concentrate on a particular path connecting $\lambda_0=\frac{1}{2}$ with an interval of $\frac{1}{2}\ln{2}$ and

\begin{equation}
f_{shor}=-x\ln{x}-\left(1-x\right)\ln\left(x-\frac{1}{2}\right),
\end{equation}
%
despite the difference between system sizes and inputs. The maximum entanglement is located at $\lambda_0=\frac{1}{2}$. We do not have an explicit interpretation of the $\frac{1}{2}\ln{2}$ interval nor $f_{shor}$. As we can see from the numerical output, some data points are located quite far from this approximation. The interval is derived from the fact that the eigenvalues of $\rho_A$ after the controlled SWAP operator frequently consist of $\lambda_0=\frac{1}{2}$ and $2^m$ non-zero elements of $\frac{1}{2^m}$, with positive integer $m$. And the formula $f_{shor}$ is derived from the fact that the eigenvalues of $\rho_A$ before the controlled SWAP operator frequently consist of $\lambda_0=\frac{m+1}{2m}$ and $m-1$ non-zero elements of $\frac{1}{2m}$, with positive integer $m$.

\begin{figure}
\centering
\includegraphics[scale=0.9]{Graph/Shor_circuit.jpg}
...
\begin{figure}
\centering
\includegraphics[scale=0.9]{Graph/QME_circuit.jpg}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/Shor_bis.pdf}
...
We introduce the Marchenko Pastur distribution \cite{nechita2007asymptotics,feier2012methods,livan2018introduction,bai1999kmethodologies,haagerup2003random,potters2020first}, which describes the asymptotic behaviour of eigenvalues of a Wishart matrix. The theorem is named after mathematicians Vladimir Marchenko and Leonid Pastur, who proved it in 1967 \cite{marvcenko1967distribution}.
...
\begin{figure}
\centering
\includegraphics[width=0.60\linewidth]{Graph/MPD.pdf}
...
Finally we have $\lambda_0=\lambda_{max}\left(Y_\beta\right)=\lambda_{max}\left(\frac{1}{\beta}XX^\dagger\right)=\alpha|\gamma|^2$, as shown in FIG. \ref{mean_MPD}, while the remaining eigenvalues still follow the MPD \cite{bouchaud2015financial}. We keep the notation $\gamma^2$ instead of $|\gamma|^2$ for simplicity.

\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/mean_MPD.pdf}
\caption{The convergence of $\lambda_0$ towards $\alpha\gamma^2$ as $\alpha=100$, $\beta=200$ and $\gamma$ varies from $0$ to $1$. Each $\lambda_0$ is the mean (raw data are plotted in blue) of the dominant eigenvalues over $500$ random matrices $Y_\beta$ generated by the entries of $X$ following the normal distribution $\mathcal{N}\left(\gamma,1\right)$. We have $\lambda=\frac{1}{2}$, and $\lambda_+=\left(1+\frac{\sqrt{2}}{2}\right)^2$ is plotted in gray dotted line. The minor difference lies between $\lambda_0$ and $\lambda_+$ when $\gamma\sim 0$ is due to the effect of the Tracy-Widom distribution \cite{tracy1996orthogonal,tracy2002distribution,johnstone2001distribution,chiani2014distribution} and can be generalized with a phase transition \cite{baik2005phase,vznidarivc2012subsystem,pandey2010correlated,vinayak2014spectral}. Since this phenomenon is highly non-analytical also numerically negligible for our study, we will not elaborate in detail.}
\label{mean_MPD}
\end{figure}
...
\begin{equation}
\begin{aligned}
&\int^b_a \ln{x} \sqrt{\left(b-x\right)\left(x-a\right)}dx\\
=&\frac{\pi}{16}\left(a^2+6ab+b^2-4\sqrt{ab}\left(a+b\right)-4\left(a-b\right)^2\ln{2}+2\left(a-b\right)^2\ln\left(a+b+2\sqrt{ab}\right)\right).
\end{aligned}
\label{eq_A2}
\end{equation}
\end{widetext}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/alphabeta.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/Grover.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/Shor.pdf}
...
&=\frac{1}{\sqrt{2}}\left(\ket{1\underbrace{0...0}_{\frac{n}{2}-1}}\ket{ \underbrace{0...0}_{\frac{n}{2}}}+\ket{1 1\underbrace{0...0}_{\frac{n}{2}-2}}\ket{ \underbrace{0...0}_{\frac{n}{2}}}\right),
\end{aligned}
\end{equation}
%
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/Prime_Almost.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/QFT.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/Gap.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{Graph/AME_gap.pdf}
...
\centering
\includegraphics[width=0.98\linewidth]{Graph/EC_gap.pdf}
...
\hfill
\begin{subfigure}[t]{0.48\linewidth}
\includegraphics[width=1\linewidth]{Graph/Grover_gap.pdf}
\caption{Corresponding to FIG. \ref{Grover}, Grover algorithm to solve EC problem and the Hash function.}
\label{Grover_gap}
\end{subfigure}
\\
\begin{subfigure}[t]{0.48\linewidth}
\includegraphics[width=1\linewidth]{Graph/Shor_gap.pdf}
\caption{Corresponding to FIG. \ref{Shor}, Shor algorithm. Gray horizontal lines are plotted with interval $\ln{2}$.}
\label{Shor_gap}
\end{subfigure}
%
\hfill
\begin{subfigure}[t]{0.48\linewidth}
\includegraphics[width=1\linewidth]{Graph/Prime_Almost_gap.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.48\linewidth]{Graph/Renyi.pdf}
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth]{Graph/3D.pdf}
...
\newblock {\em Proceedings of the Royal Society of London. Series A: Mathematical, Physical and Engineering Sciences}, \href{https://doi.org/10.1098/rspa.2002.1097}{DOI: 10.1098/rspa.2002.1097}.
...
\newblock {\em Physical Review A}, \href{https://doi.org/10.1103/PhysRevA.69.052308}{DOI: 10.1103/PhysRevA.69.052308}.

\bibitem{vidal2003efficient}
Guifr{\'e} Vidal.
\newblock Efficient classical simulation of slightly entangled quantum computations.
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.91.147902}{DOI: 10.1103/PhysRevLett.91.147902}.

\bibitem{gross2009most}
David Gross, Steve T Flammia, and Jens Eisert.
\newblock Most quantum states are too entangled to be useful as computational resources.
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.102.190501}{DOI: 10.1103/PhysRevLett.102.190501}.
...
\newblock {\em Cambridge University Press}, \href{https://doi.org/10.1017/CBO9780511535048}{DOI: 10.1017/CBO9780511535048}. 

\bibitem{qibo_paper}
Stavros Efthymiou, Sergi Ramos-Calderer, Carlos Bravo-Prieto, Adri{\'{a}}n P{\'{e}}rez-Salinas, Diego Garc{\'{\i}}a-Mart{\'{\i}}n, Artur Garcia-Saez, Jos{\'{e}} Ignacio Latorre, and Stefano Carrazza.
\newblock Qibo: a framework for quantum simulation with hardware acceleration.
\newblock {\em Quantum Science and Technology}, \href{https://doi.org/10.1088/2058-9565/ac39f5}{DOI: 10.1088/2058-9565/ac39f5}.
...
\newblock {\em {Quantum}}, \href{https://doi.org/10.22331/q-2022-09-22-814}{DOI: 10.22331/q-2022-09-22-814}.

\bibitem{Github}
Ruge Lin.
\newblock \href{https://github.com/gogoko699/random-density-matrix}{https://github.com/gogoko699/random-density-matrix}.

\bibitem{albash2018adiabatic}
Tameem Albash and Daniel A Lidar.
\newblock Adiabatic quantum computation.
\newblock {\em Reviews of Modern Physics}, \href{https://doi.org/10.1103/RevModPhys.90.015002}{DOI: 10.1103/RevModPhys.90.015002}.
...
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.106.050502}{DOI: 10.1103/PhysRevLett.106.050502}.

\bibitem{vznidarivc2006exponential}
Marko {\v{Z}}nidari{\v{c}} and Martin Horvat.
\newblock Exponential complexity of an adiabatic algorithm for an np-complete problem.
\newblock {\em Physical Review A}, \href{https://doi.org/10.1103/PhysRevA.73.022329}{DOI: 10.1103/PhysRevA.73.022329}.

\bibitem{Github_EC}
Sergi Ramos-Calderer.
\newblock \href{https://github.com/qiboteam/qibo/tree/master/examples/adiabatic3sat}{https://github.com/qiboteam/qibo/tree/master/examples /adiabatic3sat}.

\bibitem{grover1996fast}
Lov K Grover.
\newblock A fast quantum mechanical algorithm for database search.
\newblock {\em Proceedings of the twenty-eighth annual ACM symposium on Theory of computing}, \href{https://doi.org/10.1145/237814.237866}{DOI: 10.1145/237814.237866}.

\bibitem{Grover_EC}
Sergi Ramos-Calderer.
\newblock \href{https://github.com/qiboteam/qibo/tree/master/examples/grover3sat}{https://github.com/qiboteam/qibo/tree/master/examples /grover3sat}.

\bibitem{dalzell2022mind}
Alexander M Dalzell, Nicola Pancotti, Earl T Campbell, and Fernando GSL Brand{\~a}o.
\newblock Mind the gap: Achieving a super-grover quantum speedup by jumping to the end.
\newblock {\em Proceedings of the 55th Annual ACM Symposium on Theory of Computing}, \href{https://doi.org/10.1145/3564246.3585203}{DOI: 10.1145/3564246.3585203}.
...
\newblock {\em Proceedings of the 51st Annual ACM SIGACT Symposium on Theory of Computing}, \href{https://doi.org/10.1145/3313276.3316359}{DOI: 10.1145/3313276.3316359}.

\bibitem{ramos2021quantum}
Sergi Ramos-Calderer, Emanuele Bellini, Jos{\'e} I Latorre, Marc Manzano, and Victor Mateu.
\newblock Quantum search for scaled hash function preimages.
\newblock {\em Quantum Information Processing}, \href{https://doi.org/10.1007/s11128-021-03118-9}{DOI: 10.1007/s11128-021-03118-9}.

\bibitem{bernstein2008chacha}
Daniel J Bernstein.
\newblock Chacha, a variant of salsa20.
\newblock \href{https://cr.yp.to/chacha/chacha-20080120.pdf}{\em Workshop record of SASC}.

\bibitem{Github_hash}
Sergi Ramos-Calderer.
\newblock \href{https://github.com/qiboteam/qibo/tree/master/examples/hash-grover}{https://github.com/qiboteam/qibo/tree/master/examples /hash-grover}.
...
\newblock {\em SIAM review}, \href{https://doi.org/10.1137/S0097539795293172}{DOI: 10.1137/S0097539795293172}.

\bibitem{kendon2004entanglement}
Vivien M Kendon and William J Munro.
\newblock Entanglement and its role in Shor's algorithm.
\newblock \href{https://arxiv.org/abs/quant-ph/0412140}{\em arXiv:quant-ph/0412140}.

\bibitem{Github_shor}
Sergi Ramos-Calderer.
\newblock \href{https://github.com/qiboteam/qibo/tree/master/examples/shor}{https://github.com/qiboteam/qibo/tree/master/examples/shor}.
...
\newblock {\em Physical Review Letters}, \href{https://doi.org/10.1103/PhysRevLett.76.3228}{DOI: 10.1103/PhysRevLett.76.3228}.

\bibitem{parker2002entanglement}
S Parker and MB Plenio.
\newblock Entanglement simulations of Shor's algorithm.
\newblock {\em Journal of Modern Optics}, \href{https://doi.org/10.1080/09500340110107207}{DOI: 10.1080/09500340110107207}.

\bibitem{beauregard2002circuit}
Stephane Beauregard.
\newblock Circuit for Shor's algorithm using $2n+3$ qubits.
\newblock \href{https://arxiv.org/abs/quant-ph/0205095}{\em arXiv:quant-ph/0205095}.

\bibitem{braunstein1996geometry}
Samuel L Braunstein.
\newblock Geometry of quantum inference.
\newblock {\em Physics Letters A}, \href{https://doi.org/10.1016/0375-9601(96)00365-9}{DOI: 10.1016/0375-9601(96)00365-9}.
...
\newblock {\em Journal of Physics A: Mathematical and General}, \href{https://doi.org/10.1088/0305-4470/37/35/004}{DOI: 10.1088/0305-4470/37/35/004}.

\bibitem{nechita2007asymptotics}
Ion Nechita.
\newblock Asymptotics of random density matrices.
\newblock {\em Annales Henri Poincar{\'e}}, \href{https://doi.org/10.1007/s00023-007-0345-5}{DOI: 10.1007/s00023-007-0345-5}.

\bibitem{majumdar2010extreme}
Satya N Majumdar.
\newblock Extreme eigenvalues of Wishart matrices: application to entangled bipartite system.
\newblock {\em Oxford Academic}, \href{https://doi.org/10.1093/oxfordhb/9780198744191.013.37}{DOI: 10.1093/oxfordhb/9780198744191.013.37}.

\bibitem{feier2012methods}
Adina Roxana Feier.
\newblock Methods of proof in random matrix theory.
\newblock \href{https://www.math.harvard.edu/media/feier.pdf}{https://www.math.harvard.edu/media/feier.pdf}.
...
\newblock {\em Springer Cham}, \href{https://doi.org/10.1007/978-3-319-70885-0}{DOI: 10.1007/978-3-319-70885-0}.

\bibitem{bai1999kmethodologies}
Z D Bai.
\newblock Methodologies in spectral analysis of large dimensional random matrices, a review.
\newblock {\em Advances in Statistics}, \href{https://doi.org/10.1142/9789812793096_0015}{DOI: 10.1142/9789812793096\_0015}.

\bibitem{haagerup2003random}
Uffe Haagerup and Steen Thorbj{\o}rnsen.
\newblock Random matrices with complex Gaussian entries.
\newblock {\em Expositiones Mathematicae}, \href{https://doi.org/10.1016/S0723-0869(03)80036-1}{DOI: 10.1016/S0723-0869(03)80036-1}.

\bibitem{potters2020first}
Marc Potters and Jean-Philippe Bouchaud.
\newblock A First Course in Random Matrix Theory: For Physicists, Engineers and Data Scientists.
\newblock {\em Cambridge University Press}, \href{https://doi.org/10.1017/9781108768900}{DOI: 10.1017/9781108768900}.
...
\newblock {\em Mathematics of the USSR-Sbornik}, \href{https://doi.org/10.1070/SM1967v001n04ABEH001994}{DOI: 10.1070/SM1967v001n04ABEH001994}.
...
\newblock {\em Biometrika}, \href{https://doi.org/10.1093/biomet/20A.1-2.32}{DOI: 10.1093/biomet/20A.1-2.32}.

\bibitem{anderson2010introduction}
Greg W Anderson, Alice Guionnet, and Ofer Zeitouni.
\newblock An introduction to random matrices.
\newblock {\em Cambridge University Press}, \href{https://doi.org/10.1017/CBO9780511801334}{DOI: 10.1017/CBO9780511801334}.

\bibitem{meyer2000matrix}
Carl D Meyer.
\newblock Matrix analysis and applied linear algebra.
\newblock {\em SIAM}, \href{https://doi.org/10.1137/1.9781611977448}{DOI: 10.1137/1.9781611977448}.

\bibitem{belitskii2013matrix}
G. R. Belitskii , Yurii I. Lyubich.
\newblock Matrix norms and their applications.
\newblock {\em Birkh{\"a}user}, \href{https://doi.org/10.1007/978-3-0348-7400-7}{DOI: 10.1007/978-3-0348-7400-7}.

\bibitem{bouchaud2015financial}
Jean-Phillipe Bouchaud and Marc Potters.
\newblock Financial applications of random matrix theory: a short review.
\newblock {\em Oxford Academic}, \href{https://doi.org/10.1093/oxfordhb/9780198744191.013.40}{DOI: 10.1093/oxfordhb/9780198744191.013.40}.

\bibitem{tracy1996orthogonal}
Craig A Tracy and Harold Widom.
\newblock On orthogonal and symplectic matrix ensembles.
\newblock {\em Communications in Mathematical Physics}, \href{https://doi.org/10.1007/BF02099545}{DOI: 10.1007/BF02099545}.

\bibitem{tracy2002distribution}
Craig A Tracy and Harold Widom.
\newblock Distribution functions for largest eigenvalues and their applications.
\newblock \href{https://arxiv.org/abs/math-ph/0210034}{\em arXiv:math-ph/0210034}.

\bibitem{johnstone2001distribution}
Iain M Johnstone.
\newblock On the distribution of the largest eigenvalue in principal components analysis.
\newblock {\em The Annals of statistics}, \href{https://doi.org/10.1214/aos/1009210544}{DOI: 10.1214/aos/1009210544}.

\bibitem{chiani2014distribution}
Marco Chiani.
\newblock Distribution of the largest eigenvalue for real Wishart and Gaussian random matrices and a simple approximation for the Tracy-Widom distribution.
\newblock {\em Journal of Multivariate Analysis}, \href{https://doi.org/10.1016/j.jmva.2014.04.002}{DOI: 10.1016/j.jmva.2014.04.002}.
...
\newblock {\em Annals of Probability}, \href{https://doi.org/10.1214/009117905000000233}{DOI: 10.1214/009117905000000233}.

\bibitem{vznidarivc2012subsystem}
Vinayak and Marko {\v{Z}}nidari{\v{c}}.
\newblock Subsystem dynamics under random Hamiltonian evolution.
\newblock {\em Journal of Physics A: Mathematical and Theoretical}, \href{https://doi.org/10.1088/1751-8113/45/12/125204}{DOI: 10.1088/1751-8113/45/12/125204}.

\bibitem{pandey2010correlated}
Vinayak and Akhilesh Pandey.
\newblock Correlated Wishart ensembles and chaotic time series.
\newblock {\em Physical Review E}, \href{https://doi.org/10.1103/PhysRevE.81.036202}{DOI: 10.1103/PhysRevE.81.036202}.

\bibitem{vinayak2014spectral}
Vinayak.
\newblock Spectral density of the noncentral correlated Wishart ensembles.
\newblock {\em Physical Review E}, \href{https://doi.org/10.1103/PhysRevE.90.042144}{DOI: 10.1103/PhysRevE.90.042144}.

\bibitem{page1993average}
Don N Page.
\newblock Average entropy of a subsystem.
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.71.1291}{DOI: 10.1103/PhysRevLett.71.1291}.

\bibitem{sen1996average}
Siddhartha Sen.
\newblock Average entropy of a quantum subsystem.
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.77.1}{DOI: 10.1103/PhysRevLett.77.1}.

\bibitem{pal2020probing}
Rajarshi Pal and Arul Lakshminarayan.
\newblock Probing the randomness of ergodic states: extreme-value statistics in the ergodic and many-body-localized phases.
\newblock \href{https://arxiv.org/abs/2002.00682}{\em arXiv:2002.00682 [cond-mat.dis-nn]}.
...
\newblock {\em Journal of Physics A: Mathematical and General}, \href{https://doi.org/10.1088/0305-4470/34/35/335}{DOI: 10.1088/0305-4470/34/35/335}.

\bibitem{hayden2006aspects}
Patrick Hayden, Debbie W Leung, and Andreas Winter.
\newblock Aspects of generic entanglement.
\newblock {\em Communications in mathematical physics}, \href{https://doi.org/10.1007/s00220-006-1535-6}{DOI: 10.1007/s00220-006-1535-6}.
...
\newblock \href{https://arxiv.org/abs/1306.2536}{\em arXiv:1306.2536 [quant-ph]}.

\bibitem{goyeneche2015absolutely}
Dardo Goyeneche, Daniel Alsina, Jos{\'e} I Latorre, Arnau Riera, and Karol {\.Z}yczkowski.
\newblock Absolutely maximally entangled states, combinatorial designs, and multiunitary matrices.
\newblock {\em Physical Review A}, \href{https://doi.org/10.1103/PhysRevA.92.032316}{DOI: 10.1103/PhysRevA.92.032316}.

\bibitem{AME_table}
F. Huber and N. Wyderka.
\newblock Table of AME states.
\newblock \href{https://tp.nt.uni-siegen.de/ame/ame.html}{https://tp.nt.uni-siegen.de/ame/ame.html}.

\bibitem{latorre2013quantum}
Jos{\'e} I Latorre and Germ{\'a}n Sierra.
\newblock Quantum Computation of Prime Number Functions.
\newblock \href{https://arxiv.org/abs/1302.6245}{\em arXiv:1302.6245 [quant-ph]}.

\bibitem{latorre2014there}
Jos{\'e} I Latorre and Germ{\'a}n Sierra.
\newblock There is entanglement in the primes.
\newblock \href{https://arxiv.org/abs/1403.4765}{\em arXiv:1403.4765 [quant-ph]}.

\bibitem{garcia2020prime}
Diego Garcia-Martin, Eduard Ribas, Stefano Carrazza, Jos{\'e} I Latorre, and Germ{\'a}n Sierra.
\newblock The Prime state and its quantum relatives.
\newblock {\em Quantum}, \href{https://doi.org/10.22331/q-2020-12-11-371}{DOI: 10.22331/q-2020-12-11-371}.

\bibitem{rosenblatt1956central}
Murray Rosenblatt.
\newblock A Central Limit Theorem and a Strong Mixing Condition.
\newblock {\em Proceedings of the National Academy of Sciences of the United States of America}, \href{https://doi.org/10.1073/pnas.42.1.43}{DOI: 10.1073/pnas.42.1.43}.

\bibitem{li2008entanglement}
Hui Li and F Duncan M Haldane.
\newblock Entanglement spectrum as a generalization of entanglement entropy: Identification of topological order in non-abelian fractional quantum hall effect states.
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.101.010504}{DOI: 10.1103/PhysRevLett.101.010504}.

\bibitem{cirac2011entanglement}
J Ignacio Cirac, Didier Poilblanc, Norbert Schuch, and Frank Verstraete.
\newblock Entanglement spectrum and boundary theories with projected entangled-pair states.
\newblock {\em Physical Review B}, \href{https://doi.org/10.1103/PhysRevB.83.245134}{DOI: 10.1103/PhysRevB.83.245134}.

\bibitem{roy2021bulk}
Sudipto Singha Roy, Silvia N Santalla, Javier Rodr{\'\i}guez-Laguna, and Germ{\'a}n Sierra.
\newblock Bulk-edge correspondence in the Haldane phase of the bilinear-biquadratic spin-$1$ Hamiltonian.
\newblock {\em Journal of Statistical Mechanics: Theory and Experiment}, \href{https://doi.org/10.1088/1742-5468/abf7b4}{DOI: 10.1088/1742-5468/abf7b4}.

\bibitem{alba2021entanglement}
Vincenzo Alba.
\newblock Entanglement gap, corners, and symmetry breaking.
\newblock \href{https://arxiv.org/abs/2010.00787}{\em arXiv:2010.00787 [cond-mat.stat-mech]}.
...
\newblock {\em Physical Review A}, \href{https://doi.org/10.1103/PhysRevA.78.032329}{DOI: 10.1103/PhysRevA.78.032329}.

\bibitem{lauchli2010disentangling}
Andreas M L{\"a}uchli, Emil J Bergholtz, Juha Suorsa, and Masudul Haque.
\newblock Disentangling entanglement spectra of fractional quantum hall states on torus geometries.
\newblock {\em Physical review letters}, \href{https://doi.org/10.1103/PhysRevLett.104.156404}{DOI: 10.1103/PhysRevLett.104.156404}.

\bibitem{nielsen2002quantum}
Michael A Nielsen and Isaac Chuang.
\newblock Quantum Computation and Quantum Information.
\newblock {\em Cambridge University Press}, \href{https://doi.org/10.1017/CBO9780511976667}{DOI: 10.1017/CBO9780511976667}.

\bibitem{nielsen2011r}
Frank Nielsen and Richard Nock.
\newblock On T{\'e}nyi and Tsallis entropies and divergences for exponential families.
\newblock \href{https://arxiv.org/abs/1105.3259}{\em arXiv:1105.3259 [cs.IT]}.
