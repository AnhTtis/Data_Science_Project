...
\usepackage{soul}
\usepackage{xcolor}
\newcommand{\aarti}[1]{\textcolor{red}{[#1]}}
\newcommand{\drew}[1]{\textcolor{purple}{[#1]}}
\newcommand{\albert}[1]{\textcolor{blue}{#1}}
\newcommand{\sh}[1]{\textcolor{green}{#1}}
\newcommand{\vj}[1]{\textcolor{orange}{#1}}
...
\icmltitlerunning{Predicting the Initial Conditions of the Universe using Deep Learning 
% \sh{a better title needed? Inferring the Initial Conditions of the Universe directly with Deep Learning? Direct Inference of Initial Conditions of the Universe with Deep Learning} 
}

\begin{document}

\twocolumn[
\icmltitle{Predicting the Initial Conditions of the Universe using Deep Learning}
...
\icmlauthor{Vaibhav Jindal}{yyy}
\icmlauthor{Drew Jamieson}{zzz}
\icmlauthor{Albert Liang}{yyy}
\icmlauthor{Aarti Singh}{yyy}
\icmlauthor{Shirley Ho}{comp1,comp2,comp3}
%\icmlauthor{}{sch}
%\icmlauthor{}{sch}
\end{icmlauthorlist}

\icmlaffiliation{yyy}{Machine Learning Department, Carnegie Mellon University, Pittsburgh, PA, USA}
\icmlaffiliation{comp1}{Center for Computational Astrophysics, Flatiron Institute, New York, NY, USA;}
\icmlaffiliation{comp2}{Department of Physics \& Center for Data Science, New York University, New York, NY, USA;}
\icmlaffiliation{comp3}{Department of Astrophysical Sciences, Princeton University, Princeton, USA;}

\icmlaffiliation{zzz}{Max Plank Institute for Astrophysics, Garching, Germany}

\icmlcorrespondingauthor{Aarti Singh}{aarti@cs.cmu.edu}
\icmlcorrespondingauthor{Shirley Ho}{shirleyho@flatironinstitute.org}
...
We train a V-Net based convolutional neural network, which outputs the linear displacement of an N-body system, given the current time nonlinear displacement and the cosmological parameters of the system.
%\aarti{Since the mapping between the nonlinear displacements to the linear displacements is one-to-many, training a neural network for such predictions is essentially ill-defined because of the one-to-one nature of neural network mappings. 
%Surprisingly, - I wonder if we should mention this without explanation? either we explain it or only mention in discussion? Shirley?} 
We demonstrate that this neural network accurately recovers the initial linear displacement field over a wide range of scales ($<1$-$2\%$ error up to nearly $k = 1\ \mathrm{Mpc}^{-1}\,h$), despite the ill-defined nature of the inverse problem at smaller scales. Specifically, %the reverse model is very good at large scales  and only slightly worse than the forward model at smaller scales, despite the fact that  
smaller scales are dominated by nonlinear effects which makes the backward dynamics much more susceptible to numerical and computational errors leading to highly divergent backward trajectories and a one-to-many backward mapping. 
%\drew{There is a little subtlety here. The gravitational dynamics that we are modeling is, in principle, time reversible. What is irreversible is the particular discretization/integration schemes used by all accurate N-body solvers. This is made more severe due to both the nonlinear and the attractive nature of gravity; nearby trajectories can diverge wildly into the past, so while the forward dynamics may be fairly robust to noise or floating point rounding errors, the backwards dynamics will be more sensitive to these. Importantly though, the dynamics on large scales is fairly linear, so it is not surprising we recover the correct mapping on large scales. The impressive and surprising thing is how well this method works down to scales where we know nonlinear effects become important. The nonlinear effects become important at $k=0.1\ \mathrm{Mpc}^{-1}\,h$, and our model continues to have percent-level accuracy down to $k=0.8$--$0.9\ \mathrm{Mpc}^{-1}\,h$. This is comparable (only slightly worse) than the forward model. We should emphasize this.}
...
%\sh{what Drew said above is quite important, I hope this is reflected elsewhere in the manuscript?}
The results of our method motivate that neural network based models can act as good approximators of the initial linear states and their predictions can serve as good starting points for sampling-based methods %such as Hamiltonian Monte Carlo
 to infer the initial states of the universe. 
%\aarti{How? If the NN is only finding one of the suboptimal modes, then this wont help}
%\drew{I think it is probably an improvement even in this case. The Hamiltonian Monte Carlo methods that are used to sample initial conditions directly start from a totally random configuration and have long(ish) burn-in times before they converge on a realization of the linear field consistent with the final state. Once there, they may have landed in such a suboptimal configuration too, but its probably a better place to start the search than the initial random configuration.}
...
\begin{figure*}[h!]
    ...
    \begin{subfigure}[b]{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/ood_dis/0.png}
        \caption{0th percentile ($\Omega_m = 0.296, \Omega_b = 0.067, h = 0.523, n_s = 1.091, \sigma_8 = 0.806$)}
        \label{ood_dis:0}
    \end{subfigure}
    \begin{subfigure}[b]{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/ood_dis/20.png}
        \caption{20th percentile ($\Omega_m = 0.311, \Omega_b = 0.067, h = 0.673, n_s = 0.993, \sigma_8 = 0.977$)}
        \label{ood_dis:20}
    \end{subfigure}
    \begin{subfigure}[b]{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/ood_dis/40.png}
        \caption{40th percentile ($\Omega_m = 0.373, \Omega_b = 0.054, h = 0.869, n_s = 0.922, \sigma_8 = 0.678$)}
        \label{ood_dis:40}
    \end{subfigure}
    \begin{subfigure}[b]{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/ood_dis/60.png}
        \caption{60th percentile ($\Omega_m = 0.412, \Omega_b = 0.036, h = 0.840, n_s = 0.942, \sigma_8 = 0.680$)}
        \label{ood_dis:60}
    \end{subfigure}
    \begin{subfigure}[b]{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/ood_dis/80.png}
        \caption{80th percentile ($\Omega_m = 0.491, \Omega_b = 0.069, h = 0.518, n_s = 0.932, \sigma_8 = 0.820$)}
        \label{ood_dis:80}
    \end{subfigure}
    \begin{subfigure}[b]{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth]{images/ood_dis/100.png}
        \caption{100th percentile ($\Omega_m = 0.497, \Omega_b = 0.044, h = 0.805, n_s = 0.910, \sigma_8 = 0.600$)}
        \label{ood_dis:100}
    \end{subfigure}
