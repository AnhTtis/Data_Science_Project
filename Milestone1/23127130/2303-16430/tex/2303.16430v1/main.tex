...
In this paper, we devise a bandit online learning algorithm that integrates the optimistic mirror descent scheme and multi-point pseudo-gradient estimates. 
We further demonstrate that the generated actual sequence of play can converge a.s. to a critical point if the game under study is merely coherent, without resorting to extra Tikhonov regularization terms or additional norm conditions. 
...
In particular, in the Nash equilibrium problem (NEP), this group of players seeks to reach a stationary point known as Nash equilibrium (NE), where no player has any incentive to unilaterally deviate from it. 

In order to devise a solution algorithm for the NEP or its variants, it is crucial to have access to the first-order information, i.e., the partial gradient of the local objective function of each player, the evaluation of which nevertheless usually requires the action profile from all players. 
In view of this, in some studies \cite{mertikopoulos2019learning,yi2019operator,tatarenko2020geometric}, the availability of first-order oracles is taken as a given, whereas some other studies \cite{pavel2019distributed, bianchi2022fast, huang2022distributed} investigate scenarios where a communication network exists and players are willing to communicate with their trusted neighbors and keep local estimates of others' action profiles. 
...
Our primary objective in this work is to develop an online learning algorithm for multi-player continuous games that merely possess pseudo-monotonicity or mere coherence with 
bandit information. 

\textit{Related Work: }
There have been several recent notable contributions to the field of bandit learning in games. 
...
In a recent study \cite{gao2022bandit}, Gao et al. introduced an algorithm that integrates a second-order learning dynamics and Tikhonov regularization and proved that the resulting sequence of play converges a.s. to a variationally stable state (VSS) of the game. 
...
In the literature of variational inequalities (VIs) and their stochastic versions (SVIs), Mertikopoulos et al. \cite{mertikopoulos2018optimistic} showed that vanilla MD converges when the problem is strictly coherent, a relaxed variant of strict monotonicity, but not otherwise. 
By contrast, the extra-gradient (EG) method is capable of achieving convergence to a solution in all coherent VIs, but this requires exact operator values. 
...
\textit{Contributions: }
In this work, we develop a bandit online learning algorithm and establish the a.s. convergence of the generated sequence of play under the regularity that the game is merely coherent.  
The proposed algorithm leverages optimistic mirror descent (OMD) \cite{azizian2021last, hsieh2019convergence}, a single-call extra-gradient scheme, as the backbone, which enables us to contend with the absence of strict coherence and reduces the query cost induced by the extra step. 
...
We let $\norm{\cdot}_2$ denote the $\ell_2$-norm and $\langle, \rangle$ represent the dot product. 
...
\end{align}
where $x^{-i} \coloneqq [x_j]_{j \in \mathcal{N}_{-i}}$ denotes the stack action of other players that parameterizes the objective $J^i$ with $\mathcal{N}_{-i} \coloneqq \mathcal{N}\backslash\{i\}$ and $x \coloneqq [x_j]_{j \in \mathcal{N}}$; 
...
Moreover, its individual strategy space $\mathcal{X}^i$ is compact and convex. 
\end{assumption}
...
Our rationale for working in $\mathcal{H}$ rather than the Euclidean space is that the induced flexibility may enable us to accommodate scenarios like heterogeneous updating step sizes among players in future work. 

Before proceeding, we remark that under Assumption~\ref{asp:objt-set}, it is implicitly implied that $F$ is Lipschitz continuous on $\mathcal{X}$ with some constant $L$, i.e., for any $x$ and $x^\prime \in \mathcal{X}$, we have 
\begin{align}
\norm{F(x) - F(x^\prime)}_{\mathcal{H}} \leq L\norm{x - x^\prime}_{\mathcal{H}}. 
\end{align}
...
Every critical point $x_*$ of $\mathcal{G}$ is merely variationally stable, i.e., $\langle F(x), x - x_*\rangle \geq 0$ for all $x \in \mathcal{X}$. 
\end{assumption}
...
Nonetheless, the latter is generally the more readily verifiable assumption in practical applications.
...
In this subsection, we shall provide a brief overview of the optimistic mirror descent algorithm, as well as related concepts and proven outcomes. 
As an extension of projection in general spaces, the mirror map $\nabla \psi^*: \mathcal{H}_* = \mathcal{H} \to \mathcal{H}$ is defined as: 
\begin{align}
\nabla \psi^*(z) = \argmax_{x \in \mathcal{X}} \{\langle z, x\rangle - \psi(x)\}, 
\end{align}
where $\psi:\dom\psi \to \rset{}{}$ is a so-called distance-generating function (DGF) that fulfills the following conditions:
...
\end{align}
To let $D(p, \cdot)$ reflect a neighborhood of $p$, we make the following assumption. 
\begin{assumption}\label{asp:recip}
(Bregman Reciprocity) The chosen DGF $\psi$ satisfies that when the sequence $(x_k)_{k \in \nset{+}{}}$ converges to some point $p$, i.e., $\norm{x_k - p}_{\mathcal{H}} \to 0$, we have $D(p, x_k) \to 0$.
\end{assumption}
Then, the Bregman divergence generates the prox-mapping $P_{x, \mathcal{X}}: \mathcal{H} \to \dom{\psi} \cap \mathcal{X}$ for some fixed $x \in \mathcal{X} \cap \dom{\psi}$ that serves as a crucial operator in mirror descent and its variants:
\begin{align}
P_{x, \mathcal{X}}(y) = \argmin_{x^\prime \in \mathcal{X}}\{\langle y, x - x^\prime\rangle + D(x^\prime, x)\}.
\end{align}

With all these in hand, the optimistic mirror descent (OMD) can be expressed as below. 
\begin{align}\label{eq:omd}
\begin{split}
X_{k+1/2} &= P_{X_k, \mathcal{X}}(-\tau_k g_{k-1/2}) \\
X_{k+1} &= P_{X_k, \mathcal{X}}(-\tau_k g_{k+1/2})
\end{split}
\end{align}
The update consists of the following two steps. 
Given the base state $X_k$ at step $k$, in the look-forward step, the leading state $X_{k+1/2}$ is procured by updating $X_k$ with the proxy $g_{k-1/2}$ queried at $X_{k-1/2}$ rather than the exact pseudo-gradient queried at $X_k$ to reduce the oracle call per iteration. 
This step plays an essential role in anticipating the landscape of $F$ and facilitates the convergence when $F$ is merely monotone. 
In the state-updating step, the base state $X_k$ is revised to $X_{k+1}$ following the pseudo-gradient information $g_{k+1/2}$ queried at the leading state $X_{k+1/2}$. 
...
A prevalent technique in the literature of first-order information estimation methods is the simultaneous perturbation stochastic approximation (SPSA) approach. 
At each iteration $k$, to implement this one-shot estimation approach, initially, it is necessary to undertake the following perturbation step: 
...
Nevertheless, as previously noted in the literature \cite{bravo2018bandit}, the SPSA approach incurs a larger estimation variance with a decrease in query radius aimed at improving estimation accuracy, which results in conservative choices of updating step sizes and significant degradation of the convergence rate. 
...
At every iteration $k$, each player $i$ executes the perturbation step in \eqref{eq:perb} $(T_k + 1)$ times in an independent manner, takes the action $\hat{X}^i_{k+1/2,t}$, and observes the associated realized objective function values $J^i(\hat{X}_{k+1/2,t})$, where the variable $t$ is an index of the multiple samples taken per iteration.
...
The filtration $(\mathcal{F}_k)_{k \in\nset{}{+}}$ is constructed as $\mathcal{F}_k \coloneqq \sigma\big\{X_0, \{u_{1,t}\}_{t=0}^{T_1}, \ldots, \{u_{k-1,t}\}_{t=0}^{T_{k-1}}\big\}$, which captures the entire information up to and including the update that results in $X_k$. 
...
\end{align*}
where, for brevity, we denote $B^i_k \coloneqq \expt{}{G^i_k \mid \mathcal{F}_k} - \nabla_{x^i} J^i(X_{k+1/2})$ to represent the systematic error and $V^i_k \coloneqq G^i_k - \expt{}{G^i_k \mid \mathcal{F}_k}$ the stochastic error. 
...
\end{align}
where $\mathbb{B}_i \subseteq \rset{n^i}{}$ and $\mathbb{S}_j \subseteq \rset{n^j}{}$ denote the unit ball and the unit sphere centered at the origin; 
$\mathbb{S}_{-i} \coloneqq \prod_{j \in \mathcal{N}^{-i}} \mathbb{S}_j \subseteq \rset{n^{-i}}{}$; 
...
Moreover the systematic error $B_k \coloneqq [B^i_k]_{i \in \mathcal{N}}$ possesses a decaying upper bound $\norm{B_k}_{\mathcal{H}} \leq \alpha_B \delta_k$ for some positive constant $\alpha_B$.
\end{lemma}
\begin{proof}
See Appendix~\ref{pf:bias}. 
\end{proof}

In contrast to the single-point or two-point estimates, the advantage of utilizing \ref{eq:mpg} is primarily demonstrated in the following lemma, which measures the decaying rate of the estimation variance regarding the number of samples. 
\begin{lemma}\label{le:variance}
Suppose that Assumption~\ref{asp:objt-set} holds. 
Then at each iteration $k$, the squared dual norm of $V_k \coloneqq [V^i_k]_{i \in \mathcal{N}}$ satisfies $\expt{}{\norm{V_k}^2_{\mathcal{H}} \mid \mathcal{F}_k} \leq \alpha_V / T_k$ for some positive constant $\alpha_V$. 
\end{lemma}
\begin{proof}
...
In view of the convergence properties of OMD introduced in Sec.~\ref{subsec:omd} under deterministic and stochastic scenarios, we design a zeroth-order algorithm for merely monotone games by incorporating \ref{eq:mpg} into OMD, the precision of which can be controlled by adjusting the sample size per iteration. 
...
\For{$t = 0, \ldots, T_k$}
    \State Randomly sample the direction $u^i_k$ from $\mathbb{S}^{n^i}$
...
The Robbins-Siegmund (R-S) theorem serves as a heavy-lifting tool in the field of stochastic optimization to examine the convergence of sequences, the formal statement of which is presented as follows. 
...
To employ the theorem, it is necessary to guarantee that $\sum_{k \in \nset{}{+}}\xi_k$ is a.s. finite. 
Recall from Lemma~\ref{le:variance}, in the variance reduction scenario, the decaying upper bound is constructed for $\expt{}{\norm{V_k}^2_{\mathcal{H}} \mid \mathcal{F}_k}$ rather than the random variable $\norm{V_k}^2_{\mathcal{H}}$. 
In the meantime, unlike the typical extra-gradient method, OMD leverages the pseudo-gradient $F(X_{k-1/2})$ from the last iteration when updating the leading state $X_{k+1/2}$. 
This approximation brings the stochastic error $\norm{V_{k-1}}^2_{\mathcal{H}}$ into the recurrent inequality, which, due to the absence of averaging effect, does not possess a decaying upper bound and prevents us from applying the R-S theorem. 
Motivated by the consideration above, our next step will involve establishing a variant of the R-S theorem by relaxing the condition imposed upon the sequence $(\xi_k)_{k \in \nset{}{+}}$.  
...
In the numerical simulation, we choose $\tau = 0.1$, the decaying query radius $\delta_k = 0.1 \times (k + 100)^{-1.1}$, and the increasing number of queries per iteration $T_k = \ceil{0.1 \times (k + 50)^{1.1}}$. 
Since the negative entropy $h(x) = \sum_{i=1,2,3} -[x]_i\log[x]_i$ is $1$-strongly convex in $\norm{\cdot}_2$ and satisfies all the requirements discussed in Sec.~\ref{subsec:omd}, it can be chosen as a DGF for player $a$ and $b$. 
The simulation results are illustrated in Fig.~\ref{fig:rps}, with Fig.~\ref{fig:rps} (a) and (b) visualizing the actual sequences of play of player $a$ and $b$ and Fig.~\ref{fig:rps} (c) reporting the relative distance $\norm{\hat{X}_{k+1/2} - x_*}_2/\norm{x_*}_2$ to the CP/NE $x_*$. 
It can be observed that the realized sequence $\hat{X}_{k+1/2}$ converges to $x_*$ at an exponential rate for this bilinear game. 
\begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth]{figs/rps_vared_1673374000.png}
...
Compare the results with different choices of $\delta_k$, we note that for this problem when $\delta_k$ decays comparable to or faster than $O(k^{-1.1})$, further increasing the decaying rate contributes little to speed up the convergence rate of the sequence. 
...
\begin{figure}
    ...
    \caption{Performance of Algorithm~\ref{alg:vr-bdt-omd} in the Least Square Estimation: the OMD solution in Panel (a) corresponds to the case when $\delta_k = O(k^{-1.1})$ and $T_k = \ceil{0.1 \times (k+50)^{-1.1}}$; Panel (b) and (d) visualize the relative distance to the unique CP, i.e., the metric is given by $\norm{\hat{X}_{k+1/2} - x_*}_2/\norm{x_*}_2$; Panel (c) and (e) report the relative updating step sizes per iteration, i.e., $\norm{\hat{X}_{k+1/2} - \hat{X}_{k-1/2}}_2/\norm{\hat{X}_{k-1/2}}_2$. The rolling averages with a window size of 100 and the original fluctuations are illustrated in solid curves and semi-transparent curves, respectively. }
    \label{fig:lse}
\end{figure}

\section{CONCLUSION}
In this work, we investigate bandit learning in multi-player continuous games with an emphasis on handling merely monotone cases.  
