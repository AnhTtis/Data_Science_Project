...
We next turn to a concrete setting. Fix a positive integer $n$ and $k$ positive integers $\vec{d}:=(d_1,...,d_k)$. For general hypersurface $S_{d_j}$ of degree $d_j$, if $\sum_{j=1}^k d_j< n+1$, then the complete intersection $\cap_{j=1}^kS_{d_j}$ gives a K-semistable Fano manifold (e.g. \cite{AGP06}). We denote $\mM^K_{n, \vec{d}}$ (resp. $M^K_{n, \vec{d}}$) to be the K-moduli stack (resp. K-moduli space) generically parametrizing such Fano manifolds. Let $\mM^K_{n, \vec{d}, \vec{x}}$ (resp. $M^K_{n, \vec{d}, \vec{x}}$) be the K-moduli stack (resp. K-moduli space) generically parametrizing K-semistable log Fano pairs of the form  $(\bP^n, \sum_{j=1}^kx_jS_{d_j})$, where $\vec{x}:=(x_1,...,x_k)$. Then it is natural to ask what is the relationship
between $\mM^K_{n, \vec{d}}$ and $\mM^K_{n, \vec{d}, \vec{x}}$. First recall the following result.

\begin{theorem}{\rm{(\cite[Theorem 8.1]{Zhou23a})}}
Consider the log pair $(\bP^n, S_{d_1}+S_{d_2}+...+S_{d_k})$, where $S_{d_j}, j=1,...,k,$ are general smooth hypersurfaces in $\bP^n$ of degrees $d_j$ with $n\geq 2$ and $d_j\leq n+1$. Then $\Kss(\bP^n, S_1+...+S_k)$ is a polytope generated by the following equations
\begin{equation*}
\begin{cases}
...
For a log pair $(\bP^n, \sum_{j=1}^kS_{d_j})$, where $S_{d_j}$ is a hypersurface of degree $d_j$ {\rm{(}}not necessarily in general position{\rm{)}} with $\sum_{j=1}^kd_j<n+1$, the log Fano pair $(\bP^n, \sum_{j=1}^ka_jS_{d_j})$ is K-semistable if and only if the complete intersection $\cap_{j=1}^{k}S_{d_j}$ is a K-semistable Fano variety of dimension $n-k$. Moreover, there exists a morphism 
...
In this section, we fix a K-polystable Fano variety $X$ of dimension $n$ with $(-K_X)^n=v$ and $k$ sufficiently divisible positive integers $l_1,...,l_k$. Denote $\bP_j:=\frac{1}{l_j}|-l_jK_X|,\ j=1,...,k$.
...
It suffices to compute ${\pi_{\vec{c}}}_*((-K_{X\times T/T})^n\mB_j)$. Recall that $\mB_j:=\bP_1\times...\times\bP_{j-1}\times \mD_j\times \bP_{j+1}\times...\times\bP_k$, and ${\pi_j}_*((-K_{X\times \bP_j/\bP_j})^n\mD_j)=\mO_{\bP_j}(\frac{v}{l_j})$ for $\pi_{j}: X\times \bP_j\to \bP_j$. Thus we have
...
In this section, we prove Theorem \ref{thm: main2}. Throughout the section, we fix a positive integer $n$ and $k$ positive integers $\vec{d}:=(d_1,...,d_k)$ with $\sum_{j=1}^kd_j<n+1$. We  exclude the case $d_1=d_2=...=d_k=1$ since it is not interesting. Denote $\vec{a}:=(a_1,...,a_k)$ for
$$a_j=\frac{\sum_{i=1}^{k}d_i+(n-k+1)d_j-(n+1)}{(n-k+1)d_j}. $$
Let $\mM^K_{n, \vec{d}}$ (resp. $M^K_{n, \vec{d}}$) be the K-moduli stack (resp. K-moduli space) which generically parametrizes $n-k$ dimensional Fano manifolds of the form $\cap_{j=1}^{k}S_{d_j}$, and $\mM^K_{n, \vec{d}, \vec{x}}$ (resp. $M^K_{n, \vec{d}, \vec{x}}$) be the K-moduli stack (resp. K-moduli space) which generically parametrizes K-semistable log Fano pairs of the form  $(\bP^n, \sum_{j=1}^kx_jS_{d_j})$, where $\vec{x}:=(x_1,...,x_k)$ and  $S_{d_j}$ is a hypersurface of degree $d_j$ for each $1\leq j\leq k$.


\begin{proposition}\label{prop: good S}
Suppose $(\bP^n, \sum_{j=1}^ka_jS_{d_j})$ is a K-semistable log Fano pair, then $S_{d_j},\ j=1,...,k,$ are different from each other and every $S_{d_j}$ is irreducible.
\end{proposition}

\begin{proof}
Suppose $S_{d_{k-1}}=S_{d_k}$, then the log pair $(\bP^n, \sum_{j=1}^{k-1}c_jS_{d_j})$ is K-semistable, where 
$$c_1=a_1,...,\ c_{k-2}=a_{k-2},\ c_{k-1}=a_{k-1}+a_k. $$
Suppose $S_{d_j}, j=1,...,k-1$, are in general position, then by \cite[Theorem 8.1]{Zhou23a}, the K-semistable domain $\Kss(\bP^n, \sum_{j=1}^{k-1}S_{d_j})$ is a polytope generated by the following equations
\begin{equation*}
\begin{cases}
0\leq x_i\leq 1, \ \ \  1\leq i\leq k-1\\
\beta_{\bP^n, \sum_{j=1}^{k-1}x_jS_{d_j}}(S_{d_i})\geq 0, \ \ \  1\leq i\leq k-1.
\end{cases}
\end{equation*}
The equations 
$$\beta_{\bP^n, \sum_{j=1}^{k-1}x_jS_{d_j}}(S_{d_i})= 0, \ \ \  1\leq i\leq k-1 $$
admit a unique solution $(a_1',...,a_{k-1}')$ for
$$a_j'=\frac{\sum_{i=1}^{k-1}d_i+(n-k+2)d_j-(n+1)}{(n-k+2)d_j}<c_j, $$
which is an extremal point of $\Kss(\bP^n, \sum_{j=1}^{k-1}S_{d_j})$ having the maximal distance from the original point. Thus $(c_1,...,c_{k-1})$ is not contained in $\Kss(\bP^n, \sum_{j=1}^{k-1}S_{d_j})$, a contradiction. 

Next we show that every $S_{d_j}$ is irreducible. Suppose $S_{d_1}$ is reducible, and $D$ is a irreducible component of $S_{d_1}$. Let $S_{d_1}'$ be a general hypersurface of degree $d_1$, then
\begin{align*}
&\ \  \beta_{\bP^n, \sum_{j=1}^ka_jS_{d_j}}(D)\\
=&\ \  A_{\bP^n, \sum_{j=1}^ka_jS_{d_j}}(D)-S_{\bP^n, \sum_{j=1}^ka_jS_{d_j}}(D) \\
<&\ \   A_{\bP^n, a_1S_{d_1}'+\sum_{j=2}^ka_jS_{d_j}}(S_{d_1}')-S_{\bP^n, a_1S_{d_1}'+\sum_{j=2}^ka_jS_{d_j}}(S_{d_1}')\\
=&\ 0.
\end{align*}
The strict inequality holds since 
$$A_{\bP^n, \sum_{j=1}^ka_jS_{d_j}}(D)= A_{\bP^n, a_1S_{d_1}'+\sum_{j=2}^ka_jS_{d_j}}(S_{d_1}')=1-a_1$$
and
$$S_{\bP^n, \sum_{j=1}^ka_jS_{d_j}}(D) > S_{\bP^n, a_1S_{d_1}'+\sum_{j=2}^ka_jS_{d_j}}(S_{d_1}'). $$
This is a contradiction to the K-semistability of $(\bP^n, \sum_{j=1}^ka_jS_{d_j})$, concluded.
\end{proof}


\begin{proposition}\label{prop: if}
Suppose $(\bP^n, \sum_{j=1}^ka_jS_{d_j})$ is a K-semistable log Fano pair, then $\cap_{j=1}^k S_{d_j}$ is a K-semistable Fano variety of dimension $n-k$.
\end{proposition}
...
Step 3. 
Continue the same process, we get that the log pair 
...
Suppose $\cap_{j=1}^k S_{d_j}$ is a K-semistable Fano variety of dimension $n-k$, then $(\bP^n, \sum_{j=1}^ka_jS_{d_j})$ is a K-semistable log Fano pair.
\end{proposition}
\begin{proof}
The proof of Proposition \ref{prop: if} can be reversed. Concluded.
\end{proof}


\begin{remark}\label{rem: degeneration}
Suppose the log pair $(X, \sum_{j=1}^kB_j)$ is a degeneration of log pairs of the form $(\bP^n, \sum_{j=1}^kS_{d_j})$ via a $\bQ$-Gorenstein flat family, i.e. there exists a family over a smooth pointed curve $0\in C$, denoted by $(\mX, \sum_{j=1}^k\mG_j)\to C$, such that 
\begin{enumerate}
...
\item $(\mX_{t}, \sum_{j=1}^k\mG_{j, t})$ is of the form $(\bP^n, \sum_{j=1}^kS_{d_j})$ for $t\in C\setminus \{0\}$,
\end{enumerate}
then all the propositions stated for $(\bP^n, \sum_{j=1}^kS_{d_j})$ also applies for $(X, \sum_{j=1}^kB_j)$ and the proof is similar.
\end{remark}

\begin{proof}[Proof of Theorem \ref{thm: main2}]
By Propositions \ref{prop: if} and \ref{prop: only if}, the log pair $(\bP^n, \sum_{j=1}^ka_jS_{d_j})$ is K-semistable if and only if $S_{d_1}\cap...\cap S_{d_k}$ is K-semistable of dimension $n-k$. By Remark \ref{rem: degeneration}, this property also applies to any $(X, \sum_{j=1}^ka_jB_j)\in \mM^K_{n, \vec{d}, \vec{a}}$. Thus we could define the morphism
...
We aim to show that $\phi$ is surjective. Suppose $[Y]\in M^K_{n, \vec{d}}$  is of the form $S_{d_1}\cap...\cap S_{d_k}$,  by 
Proposition \ref{prop: only if}, $[(\bP^n, \sum_{j=1}^ka_jS_{d_j})]$ is a preimage. Suppose $[Y]$ is not of the form $S_{d_1}\cap...\cap S_{d_k}$. Then there is a flat family $\mY\to C$ over a smooth pointed curve $0\in C$ such that $-K_{\mY/C}$ is ample over $C$, $\mY_0\cong Y$, and $\mY_t$ is of the form $S_{d_1}\cap...\cap S_{d_k}$. Let $\mY^*\to C^*:=C\setminus \{0\}$ be the family obtained via base change under the inclusion $C^*\to C$. Then there exists a family $(\mX, \sum_{j=1}^k\mS_{d_j})\to C^*$ such that for any $t\in C^*$, the fiber $(\mX_t, \sum_{j=1}^k\mS_{d_j, t})$ is isomorphic to $(\bP^n, \sum_{j=1}^k S_{d_j})$ for some hypersurfaces $S_{d_j}$ and $\cap_{j=1}^k \mS_{d_j, t}\cong \mY_t$. Note that $(\mX, \sum_{j=1}^ka_j\mS_{d_j})\to C^*$ is a family of K-semistable log Fano pairs, by the properness of K-moduli space, we could find a complete family $(\tilde{\mX}, \sum_{j=1}^ka_j\tilde{\mS}_{d_j})\to C$ up to a finite base change and the central fiber $(\tilde{\mX}_0, \sum_{j=1}^ka_j\tilde{\mS}_{d_j,0})$ is also K-semistable. By Remark \ref{rem: degeneration} and Proposition \ref{prop: if}, $\cap_{j=1}^k \tilde{\mS}_{d_j,0}$ is K-semistable and $[Y]=[\cap_{j=1}^k \tilde{\mS}_{d_j,0}]$ by the separatedness of K-moduli space (e.g. \cite{BX19}). Thus $[(\tilde{\mX}_0, \sum_{j=1}^ka_j\tilde{\mS}_{d_j,0})]$ is a preimage of $[Y]$ and $\phi$ is surjective.
\end{proof}
...
|\mO_{\bP^n}(d_k)|$$ 
under $\Aut(\bP^n)$-action with respect to the linearization $\mO(\frac{a_1}{d_1},...,\frac{a_k}{d_k})$. By Proposition \ref{prop: GIT=K}, we have 
