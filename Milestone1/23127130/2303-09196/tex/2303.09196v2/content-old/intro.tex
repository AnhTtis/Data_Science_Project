...
In this work we consider \emph{expected risk minimization}
...
and refer to it as a \emph{risk measure} \cite[\S6.3]{Shapiro2021}. When $\amb$ is permutation invariant 
(i.e. for any $\mu \in \amb$ and permutation $\pi$, $\pi\mu \in \amb$) we show
\begin{equation} \label{eq:well-callibrated}
    \prob\left[\rho[L(\theta)] \dfn \sup_{\mu \in \amb} \<\mu, L(\theta)\> \geq \E[\ell(\theta, \xi)]\right] \geq 1 - \varepsilon
\end{equation}
...
Solving \cref{eq:robustified-erm} instead of a sample average is nothing new and several sets $\amb$ have been considered under the framework of \emph{distributionally robust optimization (DRO)} \cite{Shapiro2021}.
...
\ilpub{Throughout this paper we highlight the main results and provide only high-level proofs. 
A more exhaustive treatise is contained in a technical report \cite{TR}.}

\paragraph*{Notation} Let $\Re$ denote the reals\ilarxiv{\,and $\eRe$ the extended reals}. 
For some convex function $\phi \colon \Re^n \to \ilarxiv{\eRe}\ilpub{\Re}$ let $\phi^*$ denote the convex conjugate\ilarxiv{, $\partial \phi$ the subgradient} and
$\dom \phi$ its domain. 
\ilarxiv{For a set $\set{X}$ let $\iota_{\set{X}}(x) = 0$ if $x \in \set{X}$ and $+\infty$ otherwise be the indicator function of $\set{X}$ and $\ri \set{X}$ the relative interior.}
...
\ilarxiv{For a cone $\set{K}$ let $\set{K}^* \dfn \{y \colon \<x, y\> \geq 0, \forall x \in \set{K}\}$ denote its dual and $\set{K}^\circ \dfn -\set{K}^*$ its polar cone.}
\ilpub{For a cone $\set{K}$ let $\set{K}^\circ \dfn \{y \colon \<x, y\> \leq 0, \forall x \in \set{K}\}$ denote its polar cone.}
Let $\Pi^n$ denote the permutations of $[n]$ (i.e. all bijections $[n] \to [n]$). We write $\pi x = (x_{\pi(1)}, \dots, x_{\pi(n)})$
for $\pi \in \Pi^n$, $x \in \Re^n$ and similarly for sets $\pi \set{X} = \{\pi x \colon x \in \set{X}\}$. 
...
denote the monotone cone and $(\M^n)^\circ$ its polar (cf. \ilarxiv{\cref{prop:dual-monotone-cone}}\ilpub{\cite[Lem.~A.1]{TR}}). 
...
\ilarxiv{For $x, y \in \Re^n$ let $x \slt y$ denote that $y$ majorizes $x$ \cite{Marshall2011}}.
For sets let $A + B \dfn \{a + b \colon a \in \set{A}, b \in \set{B}\}$
denote the Minkowski sum and for $x \in \Re^n$, $x + B = \{x\} + B$. 
