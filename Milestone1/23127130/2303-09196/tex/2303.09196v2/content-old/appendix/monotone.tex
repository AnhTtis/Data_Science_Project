...
\end{proposition}
\begin{proof}
    Let $x \in \set{A}$. Since the identity map is also in $\Pi^n$, \cref{prop:permuto-hull} implies $\hull(\Pi^n x) \subseteq x_{\uparrow} + (\Re^n_{\uparrow})^\circ$.
    Therefore we have $\pi(x) \in x_{\uparrow} + (\M^n)^\circ$ for all $\pi \in \Pi^n$. 
    Noting that $x_{\uparrow} \in \set{A}$ by permutation invariance gives $\pi(x) \in \set{A} + (\Re^n_{\uparrow})^\circ$ for all $\pi \in \Pi^n$. 
    For every $\pi \in \Pi^n$, also $\pi^{-1} \in \Pi^n$. We can apply this inverse to both sides of the inclusion.
    Thus $x \in \cap_{\pi} \pi(\set{A} + (\M^n)^\circ)$ and $\set{A} \subseteq \cap_{\pi} \pi(\set{A} + (\M^n)^\circ)$.

    For the converse let $x \in \cap_{\pi} \pi(\set{A} + (\M^n)^\circ)$. Then, again since $\pi^{-1} \in \Pi^n$, $\pi(x) \in \set{A} + (\M^n)^\circ$ for all $\pi \in \Pi^n$.
