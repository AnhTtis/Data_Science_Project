...
The underlying distribution encodes the fact that the samples are exchangeable and has 
the consequence that identically distributed random variables are characterized as permutations (cf. \cref{app:distortion-representation})
...
We consider coherent risks, which can be represented as support functions \cite{Pichler2021}
...
definition of a coherent risk measure in \cref{app:distortion-representation}.
...
\begin{proof}
    ...
    \end{equation*}
    The second and third equality follow from permutation invariance and the definition of the polar cone respectively. 
    So $\amb + (\M^n)^\circ $ is valid. The remainder is shown in \cref{app:distortion-representation}.
\end{proof}

The full derivation of \cref{eq:distort-ambiguity-relation} in \cref{app:distortion-representation} makes extensive use of 
...
\begin{figure}
    \input{assets/transform.tex}
    \vspace{0.5em}
    \centering
    \caption{Graphical depiction of \cref{prop:fundamental-equivalence-lemma} for $n = 3$.
    The intersection with the simplex $\Delta^n$ is shown.} \label{fig:graphical-equivalence} \vspace{-2em}
\end{figure}
