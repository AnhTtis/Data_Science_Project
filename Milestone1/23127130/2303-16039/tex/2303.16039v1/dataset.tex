...
Our out-of-the-lab data collection did not control where, when, how long and via which laptop or desktop participants could join, allowing more natural behaviour~\cite{mazilu2015wearable,vaizman2018extrasensory,petersen2021pedagogical}.
...
The former only counts the time spent on tasks.
...
On average, they reported having used a mouse and keyboard for 13.64 years ($\mathrm{SD}=6.80$).
...
In \textit{text entry and editing}, participants wrote %
a piece of text in English in a text editor\footnote{\url{https://github.com/tinymce/tinymce}} for one trial (Fig.~\ref{fig:main-tasks}a).
...
We asked participants to write at least 200 words and apply at least 15 text formatting rules, e.g. change font colour, size or alignment.
...
In \textit{image editing}, participants were presented with two images shown side-by-side in an image editor\footnote{\url{https://github.com/nhn/tui.image-editor}} (Fig.~\ref{fig:main-tasks}b).
The image on the left was a real photograph, whereas the image on the right was a sketch.
Participants edited the image on either or both sides as they wanted by drawing, cropping, flipping, rotating, adding icons or adding filters.
...
To reduce potential effects of task order, half of the initial 52 participants performed the text entry and editing task first, followed by the image editing task, while the other half %
...
\textit{Text typing} involved copying a short piece of text ($\sim$100 words, Fig.~\ref{fig:calibration-tasks}a) as quickly as possible~\cite{connelly2007comparison,grabowski2008internal}.
The average duration of key presses and the number of keys pressed per minute were calculated as keyboard metrics~\cite{connelly2007comparison,grabowski2008internal}.
