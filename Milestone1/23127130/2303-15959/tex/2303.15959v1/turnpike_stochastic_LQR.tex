...
%Further, we make the following admissibility assumption.
%\begin{definition} \label{defn_admissible}
    %A control $U(\cdot)$ is called admissible, if 
    %\begin{enumerate}
        %\item there is a $\alpha > 0$ such that $\E [\norm{X_{U}(k,X_0)}^2] < \alpha$ for all $k \in \N_0$
        %\item $X(k) \in \Ltwo{\R^n}$ and $U(k) \in \Ltwo{\R^l}$ for all $k \in \N_0$, where $\Omega$ is the set of realizations, $\Prob$ is the probability measure, $\F$ is a $\sigma$-algebra and $\Filtr$ is a filtration according to \cite{Protter2005}.
    %\end{enumerate}
    %The set of admissible controls is denoted by $\U$.
%\end{definition}
...
\begin{equation} \label{eq_sOCP}
    \min_{U(\cdot)} J_N(X_0,U) := \sum_{k=0}^{N-1} \Exp{\ell(X(k),U(k))}
\end{equation}
...
%Before investigating stochastic turnpike properties, we will briefly recall the basic concepts of dissipativity and turnpike in the deterministic case.
%For this purpose, we denote the deterministic counterpart to system \eqref{eq_StochSystem} by
...
\begin{definition} \label{defn_DissiDeterm}
    ...
    \begin{equation}\label{eq:sDI}
        ...
        \end{split}
    \end{equation}
    holds for all $(x,u) \in \R^n \times \R^l$.  %If $\lambda$ is not bounded from below, it is called \textit{strict $(x,u)$-pre-dissipative}.
\end{definition}

%The connection between strict dissipativity and the turnpike property is well studied, see \cite{Faulwasser2017,Faulwasser2022,Gruene2016,Gruene2017,Gruene2018}. 
...
\begin{lemma} \label{lem_TurnpikeDeterm}
    ...
    %sequence $\mathbf{u} := (u(0),\ldots,u(N-1)) \in (\R^l)^N$ 
...
and use the following definition of stationary processes. % regarding the random variables.
\begin{definition} \label{defn_StationaryPair}
    ...
    %it has an invariant probability measure $\varrho^s$, i.e. if 
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    with $V(\cdot) \in \U$. %\Ltwo{\R^l}$. 
    %The function $V(\cdot)$ can be interpreted as a measurement how far the control $U(\cdot)$ differs from $U^*(\cdot)$.
    ...
    \begin{equation} \label{eq_solX}
        \begin{split}
            %X^*(k) &= A_L^k X_0 + \sum_{j=0}^{k-1} A_{\RF}^j W(k-1-j) \\
    ...
    %         &+ \E \bigg[ X^*(k) \big(\sum_{j=0}^{k-1} A_{\RF}^j B V(k-1-j) \big)^T \bigg] \\
    %         &+ \E \bigg[ \big( \sum_{j=0}^{k-1} A_{\RF}^j B V(k-1-j) \big) X^*(k)^T \bigg] \\
    %         &+ \E\bigg[ ( \sum_{j=0}^{k-1} A_{\RF}^j B V(k-1-j) ) ( \sum_{j=0}^{k-1} A_{\RF}^j B V(k-1-j) )^T \bigg] .
    %     \end{split}
    % \end{equation*}
    \begin{equation*}
        \begin{split}
...
\begin{lemma} \label{lem_DissiPosDef}
    ...
    for every control $U(\cdot)$ %and corresponding state $X(\cdot) = X_{U}(\cdot,X_0)$ 
    it holds that 
    \begin{equation*}
        \begin{split}
...
\begin{proof}
    ...
    \begin{equation} \label{eq_turnpikeContr}
        ...
        \end{split}
    \end{equation}
    Now assume that $Q_{\eps} < N-(\delta+C)/\eps$. This means there is a set $\mathcal{\Bar{M}} \subset \lbrace 0,\ldots,N-1 \rbrace$ of $N-Q_{\eps} > (\delta+C)/\eps$ time instants such that $\E [ \Vert (\widetilde{X}(k),\widetilde{U}(k)) \Vert_H^2 ] \geq \eps$ for all $k \in \mathcal{\Bar{M}}$. Using Lemma \ref{lem_DissiPosDef}, this implies 
    % \begin{equation*}
    %     \begin{split}
    %         \tilde{J}_N(X_0,U) &= \sum_{k=0}^{N-1} \E\big[\Vert \widetilde{X}(k),\widetilde{U}(k)) \Vert_H^2\big] \\
    %         &~\geq (N-Q_{\eps}) \eps > \delta + C.
    %     \end{split}
    % \end{equation*}
    %\begin{equation*}
        $\tilde{J}_N(X_0,U) = \sum_{k=0}^{N-1} \E\big[\Vert \widetilde{X}(k),\widetilde{U}(k)) \Vert_H^2\big] > \delta + C$,
    %\end{equation*}
    which contradicts \eqref{eq_turnpikeContr} and, thus, proves the theorem.
\end{proof}
...
\begin{proof}
    ...
    Using equation \eqref{eq_MarkovIneq}, this gives
    %\begin{equation*}
        $\Prob\left(\Vert (\widetilde{X}(k),\widetilde{U}(k)) \Vert_H^2 \geq \epsilon \right) \leq \eta$
    %\end{equation*}
    for all these time instants and, thus, proves the claim.
\end{proof}
...
\begin{figure}[ht] 
    \centering
    \includegraphics[width=8.3cm,height=4.2cm]{turnpike_pathwise_new.pdf}
