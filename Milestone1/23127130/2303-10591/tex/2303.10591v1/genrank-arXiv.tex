...
%\subtitle{with an application to the cluster multiplications}
\author{JiaRui Fei}
\address{}
\email{}
\thanks{}
...
We find an explicit mutation formula for the general rank, and discover several interesting related mutation invariants.
\end{abstract}
...
A general representation in $\PC(\delta)$ is called a general representation of weight $\delta$.
...
Thanks to Derksen-Weyman-Zelevinsky's mutation, we are able to give an algorithm for computing the general ranks. Our algorithm is expected to work for any pair $(\delta,\ep)$ but we can only show it works under some conditions. Those conditions are trivially satisfied if one of $\delta$ and $\ep$ is (negative or positive) {\em reachable}. 
...
It turns out that if there is a sequence of mutations $\mub$ such that $(\mub(\delta),\mub(\ep))$ has completely extremal rank, then $r_\ep(\delta)$ and $l_\ep(\delta)$ commute with any sequence of mutations and $\tau^i$ (Lemma \ref{L:rlmu}). 
...
\begin{enumerate}
	...
	\begin{equation*} \cdots \to  \tau^{-1} M\xrightarrow{f_{-1}} \tau^{-1} R \xrightarrow{g_{-1}} \tau^{-1} E\xrightarrow{h_{-1}} M\xrightarrow{f_0} R \xrightarrow{g_0} E \xrightarrow{h_0} \tau M \xrightarrow{f_1} \tau R \xrightarrow{g_1} \tau E \xrightarrow{h_1} \tau^2 M\to \cdots,\end{equation*}	
	where $R$ is general of weight $r_\ep(\delta)$, $(E,M)$ is general as a pair of weights $\delta$ and $\ep$, and $h_i$ is a general homomorphism in $\Hom_J(\tau^{i}E, \tau^{i+1} M)$.
	...
	\begin{equation*} \cdots \to  \tau^{-1} L\xrightarrow{f_{-1}} \tau^{-1} M \xrightarrow{g_{-1}} \tau^{-1} E\xrightarrow{h_{-1}} L\xrightarrow{f_0} M \xrightarrow{g_0} E \xrightarrow{h_0} \tau L \xrightarrow{f_1} \tau M \xrightarrow{g_1} \tau E \xrightarrow{h_1} \tau^2 L\to \cdots,\end{equation*}	
	where $L$ is general of weight $l_\ep(\delta)$, $(M,E)$ is general as a pair of weight $\delta$ and $\ep$, and $g_i$ is a general homomorphism in $\Hom_J(\tau^{i} M, \tau^{i} E)$.
\end{enumerate}
\end{theorem}
\noindent Here, $(E,M)$ is general as a pair of weight $\delta$ and $\ep$ means that $(E,M)$ can be chosen in an open subset of $\PC(\delta)\times\PC(\ep)$.
...
If $\ep$ is (negative or positive) reachable, then Theorem \ref{T:rl} and Algorithm \ref{a:rl} take a particularly nice form (Corollaries \ref{C:rlrigid} and \ref{C:rle}). In this case, the operators $r_\ep$ and $l_\ep$ are inverse of each other (Proposition \ref{P:rlid}).
...
\begin{align*}
	...
	& P_u && \text{the projective cover of $S_u$} &\\
	& I_u && \text{the injective envelope of $S_u$} &\\
...
Note that this definition agrees with the one in \cite{DF}.
...
\end{equation}
for any $\Aut_A(P_-)\times \Aut_A(P_+)$-stable subvariety $Y$ of $\Hom_A(P_1, P_0)$ and $\GL_\alpha$-stable subvariety $X$ of $\rep_\alpha(A)$.
It comes with two projections $p_1: Z\to \Hom_A(P_-,P_+)$ and $p_2:Z\to \rep_\alpha(A)$.
\begin{theorem}[{\cite[Theorem 2.3, Corollary 2.6]{DF}}] \label{L:P2R} The projection $p_2$ is open.
Moreover, the map $p_2 p_1^{-1}$ gives a bijection between $\Aut_A(P_-)\times \Aut_A(P_+)$-stable subvariety of $\img p_1$ and $\GL_\alpha$-stable subvariety of $\img p_2$, preserving openness, closure, and irreducibility.
\end{theorem}

As discussed before, there is an open subset $U\subset\PHom_A(\delta)$ such that $W=p_2(p_1^{-1}(U))$ lies entirely in $\rep_\alpha(A)$.
In particular, there is an irreducible component $C$ of $\rep_\alpha(A)$ such that $p_1(p_2^{-1}(W\cap C))$ is dense in $\PHom_A(\delta)$.
Since $p_2$ is open, $W$ (and thus $W\cap C$) is open. So $p_1(p_2^{-1}(W\cap C))$ is constructible, and thus contains an open subset of $\PHom_A(\delta)$.
We conclude that we can shrink $U$ such that $W=p_2(p_1^{-1}(U))$ lies in a single component of $\rep_\alpha(A)$.
...
When we say a general representation of weight $\delta$, we mean a general representation in $\PC(\delta)$.
...
\begin{lemma} \label{L:U0} For any $\Aut_A(\delta)$-stable subvariety $X\subset\PHom_A(\delta)$, there is a nonempty open $\Aut_A(\delta)$-stable subset $U_0$ of $X$ such that there is morphism 
	$\pi: U_0 \to \PC(\delta)$ such that $\pi(d)$ is isomorphic to $\coker(d)$.	
\end{lemma}
\begin{proof} This follows from \cite[Lemma 2.11]{P}.
\end{proof}
...
The following lemma is implied in the proof of \cite[Theorem 3.21]{P}. It can also be proved in a similar fashion as our Theorem \ref{T:genpi}.
\begin{lemma} \label{L:genmu} There is an open subset $U$ in $\PC(\delta)$ and an open subset $U'$ in $\PC(\mu_u(\delta))$ such that the orbit of $\mu_u(M)$ lies in $U'$ if and only if the orbit of $M$ lies in $U$.
...
\begin{equation} \label{eq:long} \cdots \to \tau^{-1} L\to \tau^{-1} M\to \tau^{-1} N \xrightarrow{\theta'} L\to M\to N \xrightarrow{\theta} \tau L \to \tau M \to \tau N \to \tau^2 L \to \cdots.
\end{equation}
with $\delta_{\mc{M}} = \delta_{\mc{L}} + \delta_{\mc{N}} + \rank(\theta) B$ and $\dtc_{\mc{M}} = \dtc_{\mc{L}} + \dtc_{\mc{N}} - \rank(\theta') B$.	
\end{corollary}
\begin{proof} We apply the functor $F=\C(\T,-)$ to the above triangle, and note that $\C(\T,\Sigma^i \b{M}) \cong \tau^i M$.
So we get the desired long exact sequence. Since $\delta_{\mc{M}} = \ind\b{M}$, the last formula then follows from lemma \ref{L:index}.
\end{proof}
\noindent We had a direct explicit proof of this corollary without referring to the category $\C$. We'd like to thank 
...
\begin{definition} \label{D:rlgrank} For any decorated representation $\mc{E}=(E,E^-)$ of weight $\ep$, we define the two operators $r_{\mc{E}}$ and $l_{\mc{E}}$ on the set of $\delta$-vector as follows:
\begin{align} \label{eq:re}	r_{\mc{E}} (\delta) &= \delta+\ep +\rank(E, \tau\delta) B; \\
	\label{eq:le} l_{\mc{E}} (\delta) &= \delta-\epc +\rank(\delta,E) B. 
\intertext{We also define the two operators $r^{\mc{E}}$ and $l^{\mc{E}}$ on the set of $\dtc$-vector}
\notag   r^{\mc{E}} (\dtc) & =  \dtc - \tau^{-1}\ep -\rank(\tau^{-1}E, \dtc) B;	\\	
\notag	 l^{\mc{E}} (\dtc) &= \dtc + \tau^{-1}\epc - \rank(\tau^{-1}\dtc, \tau^{-1}E) B.
\end{align}	
...
By Lemma \ref{L:gendes} (for void mutation) $\b{g}$ will descend to a general morphism $g\in \Hom_{J}(E, \tau M)$.
...
$$\dtc_{\mc{R}} = \dtc_{\mc{M}} + \dtc_{\mc{E}} - \rank(g_{-1})B = \dtc + \epc - \rank(\tau^{-1}E,\delta)B = r^{\mc{E}}(\dtc).$$
...
\begin{equation}  \cdots \to \tau^{-1} L\to \tau^{-1} M\xrightarrow{\tau^{-1}g} \tau^{-1} E \xrightarrow{} L\to M\xrightarrow{g} E \xrightarrow{} \tau L \to \tau M \xrightarrow{\tau g} \tau E \to  \cdots. 
\end{equation}
...
By Theorem \ref{T:subp} a general presentation $d_L$ in $\Hom_J(P([-\tau^{-1}\ep]_++[-\delta]_+), P([\tau^{-1}\ep]_++[\delta]_+]))$
has a quotient presentation $d_M$ of weight $\delta$, and thus a subpresentation $d_1$ of weight $\tau^{-1}\ep$.
Note that the presentation $d_L$ is homotopy equivalent to a general presentation of weight $l_\ep(\delta) = \delta+\tau^{-1}\ep$ by Lemma \ref{L:homotopy}.
Moreover, by Lemma \ref{L:sqgen} we may assume $d_M$ and $d_{\tau^{-1}E}$ are general as a pair, and
$\eta$ is general in $\E(d_M,d_{\tau^{-1}E})$ .
We lift the quotient presentation $d_L \to d_M$ to a nine diagram as in Lemma \ref{L:lift233}:
$$\xymatrix{
...
P_-^L \ar@{^(->}[r] \ar[d]_{d_{L}} & P_-^M \ar[d]_{d_M} \ar@{>>}[r]^{\pi_-} & P_-^E\ar[d]_{d_E} \\
P_+^L \ar@{^(->}[r] \ar@{>>}[d]& P_+^M \ar@{>>}[r]^{\pi_+} \ar@{>>}[d]_{} & P_+^E \ar@{>>}[d]_{} \\
	L \ar[r] & M\ar@{>>}[r]^{g} & E}
\end{equation*}
In particular, we get for a pair of general presentations $d_M$ and $d_E$ of weight $\delta$ and $\ep$,
a general morphism from $d_M$ to $d_E$ is surjective.
Finally, a similar argument as in the proof of Lemma \ref{L:sqgen} shows that the kernel $d_L$ of such a morphism is general as well. That is equivalent to say $\b{L}$ may be assumed to be general of index $\delta-\ep$.
...
\begin{enumerate}
\item Suppose that there is an extended sequence of mutation $\mub$ such that $(\mub(\ep), \mub(\tau\delta))$ has completely extremal rank $r$. Then there is an exact sequence
\begin{equation}\label{eq:rseq} \cdots \to  \tau^{-1} M\xrightarrow{f_{-1}} \tau^{-1} R \xrightarrow{g_{-1}} \tau^{-1} E\xrightarrow{h_{-1}} M\xrightarrow{f_0} R \xrightarrow{g_0} E \xrightarrow{h_0} \tau M \xrightarrow{f_1} \tau R \xrightarrow{g_1} \tau E \xrightarrow{h_1} \tau^2 M\to \cdots,\end{equation}	
where $R$ is general of weight $r_\ep(\delta)$, $(E,M)$ is general as a pair of weights $\delta$ and $\ep$, and $h_i$ is a general homomorphism in $\Hom_J(\tau^{i}E, \tau^{i+1} M)$.
...
\item Suppose that there is an extended sequence of mutation $\mub$ such that $(\mub(\delta), \mub(\ep))$ has completely extremal rank $r$.  Then there is an exact sequence
\begin{equation}\label{eq:lseq} \cdots \to  \tau^{-1} L\xrightarrow{f_{-1}} \tau^{-1} M \xrightarrow{g_{-1}} \tau^{-1} E\xrightarrow{h_{-1}} L\xrightarrow{f_0} M \xrightarrow{g_0} E \xrightarrow{h_0} \tau L \xrightarrow{f_1} \tau M \xrightarrow{g_1} \tau E \xrightarrow{h_1} \tau^2 L\to \cdots,\end{equation}	
where $L$ is general of weight $l_\ep(\delta)$, $(M,E)$ is general as a pair of weight $\delta$ and $\ep$, and $g_i$ is a general homomorphism in $\Hom_J(\tau^{i} M, \tau^{i} E)$.
\end{enumerate}
\end{theorem}
...
\begin{remark} \label{r:genhom} (1). In fact we can also say something about $f_i$ and $h_i$ in \eqref{eq:lseq}. If $r=0$ or $\dv(\mub(\delta))$, then we may assume that $f_i$ is general in $\Hom_J(\tau^i L, \tau^{i}M)$; if $r=\dv(\mub(\ep))$, then we may assume that $h_i$ is general in $\Hom_J(\tau^iE, \tau^{i+1}L)$ (see Remarks \ref{r:rltri} and \ref{r:rltri1}).
...
Step 2. According to Proposition \ref{P:rlmu} we have that $$l_{\ep}(\delta) = \begin{cases} \mub^{-1}(\delta'-\epc') & \text{if $r=0$;}  \\
...
For completeness, we list other formulas below \begin{align*}
r_{\ep}(\delta) &= \begin{cases} \mub^{-1}(\delta'+\ep') & \text{if $\rank(\ep',\tau\delta')=0$;}  \\
		\mub^{-1}(\delta'+\epc') & \text{if $\rank(\ep',\tau\delta')=\dv(\ep')$;} \\
		\mub^{-1}(-\tau\delta'+\ep') & \text{if $\rank(\ep',\tau\delta')=\dv(\tau\delta')$.}
	\end{cases}\\
l^{\ep}(\dtc) &= \begin{cases} \mub^{-1}(\dtc'+\tau^{-1}\epc') & \text{if $\rank(\tau^{-1}\dtc',\tau^{-1}\epc')=0$;}  \\
	\mub^{-1}(\dtc'-\epc') & \text{if $\rank(\tau^{-1}\dtc',\tau^{-1}\epc')=\dv(\tau^{-1}\epc')$;} \\
	\mub^{-1}(-\tau^{-1}\dtc'+\tau^{-1}\epc') & \text{if $\rank(\tau^{-1}\dtc',\tau^{-1}\epc')=\dv(\tau^{-1}\dtc')$.}
\end{cases}\\
r^{\ep}(\dtc) &= \begin{cases} \mub^{-1}(\dtc'+\epc') & \text{if $\rank(\tau^{-1}\epc',\dtc')=0$;}  \\
	\mub^{-1}(\dtc'-\tau^{-1}\epc') & \text{if $\rank(\tau^{-1}\epc',\dtc')=\dv(\tau^{-1}\epc')$;} \\
	\mub^{-1}(\delta'+\epc') & \text{if $\rank(\tau^{-1}\epc',\dtc')=\dv(\dtc')$.}
\end{cases}
\end{align*}
\end{algorithm}
...
We suspect that $\rank(\delta,\ep)=\dim(\ep)$ might imply $\e(\delta,\ep)=0$ so that Conjecture \ref{c:cer} imply Conjecture \ref{c:hev}.
...
It is also easy to show that $\rank(\delta,\ep)=\dim(\ep)$ implies $\e(\delta,\ep)=0$ if $\ep$ is rigid.
...
As $\delta(\dv(\delta))$ and $\ep(\dv(\ep))$ are not positive, $\delta$ and $\epc$ are not rigid (in particular not reachable).
...
If $\ep$ is (negative or positive) reachable, the rank condition in Theorem \ref{T:rl} is trivially satisfied. In addition, the rigidity of $\ep$ makes the situation particularly nice. The following two corollaries will play a crucial role in \cite{Fc}.
\begin{corollary} \label{C:rlrigid} Assume that $\ep$ is (negative or positive) reachable. Then \begin{enumerate}
...
In particular, the rank of each $g_i$ is the general rank from $\tau^{i}(r_\ep(\delta))$ to $\tau^{i}\ep$.
	...
	r_{\ep}(\delta) = \delta + \epc - \rank(g_0) B\ &\text{ and }\ 
		\hom_J(r_{\ep}(\delta), E) = \hom_J(\delta, E) + \epc(\rank(g_0)),\\
	r_{\ep}(\delta) = \delta + \ep + \rank(h_0) B\ &\text{ and }\  
		\e_J(r_{\ep}(\delta),E) = \e_J(\delta, E)  - \ep(\rank(h_0)),\\
	r^{\ep}(\dtc) = \dtc - \tau^{-1}\ep - \rank(h_{-1}) B\ &\text{ and }\  
		\hom_J(\tau^{-1}E, r^{\ep}(\dtc)) = \hom_J(\tau^{-1}E, \dtc)  - \tau^{-1}\ep(\rank(h_{-1})),\\
	r^{\ep}(\dtc) = \dtc - \tau^{-1}\epc + \rank(g_{-1}) B\ &\text{ and }\  
		\ec_J(\tau^{-1}E, r^{\ep}(\dtc)) = \ec_J(\tau^{-1}E, \dtc)  + \tau^{-1}\epc(\rank(g_{-1})).	
	\end{align*}		
...
In particular, the rank of each $h_i$ is the general rank from $\tau^{i}\ep$ to $\tau^{i+1}(l_\ep(\delta))$.
	...
	l_{\ep}(\delta) = \delta - \epc + \rank(g_0) B\ &\text{ and }\ 
		\hom_J(l_{\ep}(\delta),E) = \hom_J(\delta,E) - \epc(\rank(g_0)),\\
	l_{\ep}(\delta) = \delta - \ep - \rank(h_0) B\ &\text{ and }\  
		\e_J(l_{\ep}(\delta),E) = \e_J(\delta,E)  + \ep(\rank(h_0)),\\
	l^{\ep}(\dtc) = \dtc + \tau^{-1}\ep + \rank(h_{-1}) B\ &\text{ and }\  
		\hom_J(\tau^{-1}E, l^{\ep}(\dtc)) = \hom_J(\tau^{-1}E, \dtc)  + \tau^{-1}\ep(\rank(h_{-1})),\\
	l^{\ep}(\dtc) = \dtc + \tau^{-1}\epc - \rank(g_{-1}) B\ &\text{ and }\  
		\ec_J(\tau^{-1}E, l^{\ep}(\dtc)) = \ec_J(\tau^{-1}E, \dtc)  - \tau^{-1}\epc(\rank(g_{-1})).
	\end{align*}
\end{enumerate}
\end{corollary}
...
So we may assume that both $(M,E)$ and $(E,\tau L)$ are general pairs.
...
$\Hom_J(\img(h_{-1}),E)$ vanishes because $\Hom_J(\tau^{-1}E,E)=\Ec_J(E,E)=0$.
...
and $r_{\ep} l_{\ep}$ is the identity if and only if $\rank(E,\tau L)=\rank(\ep, \tau(l_{\ep}(\delta)))$.
...
\begin{remark} If $\delta$ is real, then the above invariants always vanish as seen in Corollary \ref{C:rle}.
If $\delta$ is not real, they may not be zero (but conjecturally always positive).
\end{remark}
...
\bibitem{Ft} J. Fei, \textit{Tropical $F$-polynomials and general presentations,}~to appear J. London Math. Soc..
