...
\caption{Illustrating the required margin between a previous low-confidence region ($y$) and current low-confidence region ($z$) for a scheduling solution. The worst-case scenario from operator corrections occurs when Robot 1 takes shorter than scheduled on $x$ and Robot 2 takes longer than scheduled on $y$.}
\label{fig:rtamargin}
\vspace{-15pt}
\end{figure}
...
After each iteration of running the two-robot system, we acquire two new demonstrations of the task and two new Boolean arrays of whether corrections were given during low-confidence samples. With these new data, we can shift the mean behavior and append our correction observations.
\begin{equation}
\label{eq:dagger}
\begin{aligned}
