...
\begin{document}
	...
	\begin{abstract}
     We analyze  the finite-strain   Poynting-Thomson viscoelastic
...
%	\tableofcontents
 ...
 Viscoelastic solids appears ubiquitously in
...
of Continuum Mechanics. In the linearized, infinitesimal-strain setting, two
...
accurate description of viscous creep and relaxation.
...
{\it Zener} models (elastic and Kelvin-Voigt elements in parallel) at finite strains is discussed, and to 
...
of en energy inequality, an elastic semistability inequality, and an
...
known, the PDE
setting is local in nature and, as such, does not allow to consider
...
analogous to ours in terms of solution notion.  In contrast with
...
\section{The finite-strain Poynting-Thomson model}\label{sec:model}
  ...
  % namely the Zener and  Poynting-Thompson  ones
  ...
  % combined in series (see Figure \ref{fig:Poynting Thompson}). 
...
\end{align}
where $f$ stands for a given body force. The reader is referred to the
  monographs \cite{Fabrizio92,Kruzik Roubicek,Visintin} for a
  comprehensive collection of analytical results.
...
settings and refer to \cite{Melching,Stefanelli1} for some parallel theory in the frame of
  ...
  choosing ${\mathbb
    ...
    PDE1}-\eqref{linear PDE1} reduces to the {\it Maxwell} fluidic
...
techniques, which in turn ask for some   some specification on the class of
...
different settings. %There are several papers dealing with the
...
$y=y_{\rm el,\varepsilon}\circ y_{\rm
  vi,\varepsilon}$ and
the corresponding rescaled displacements $u_\varepsilon=(y{-} {\rm
...
%\cite{Kruzik Roubicek} (see Figure \ref{fig:Poynting Thompson}), whose equations are
...
is actually a homeomorphism with inverse $y_{\text{\rm vi}}^{-1}\in W^{1,p_{\text{\rm vi}}/(d-1)}$ (see \cite{fonseca_gangbo_2018}).
...
i.e. $\ell(t)\equiv 0$ for every $t\in [0,T]$. In this setting, the
discrete incremental problem \eqref{i min probl} is  specified as
 
  \begin{equation}\label{example}
    \min_{F\in \R, F_{\rm vi}>0}\left(\frac{1}{2}\left|F F_{\rm vi}^{-1}{-}1\right|^2+\frac{1}{2} |F_{\rm vi}{-}1|^2+\frac{1}{2\tau}\left| (F_{\rm vi}{-}F_{\rm vi}^{i-1})(F_{\rm vi}^{i-1})^{-1}\right|\right) \quad \text { for } i=1, \ldots, N,
  \end{equation}
  where  $F\in \R$  and $F_{\rm vi}>0$ represent the  total and
  viscous (scalar) strains, 
  respectively.  Take now  initial values $(F^0,F^0_{\rm vi})$ with
...
it is easy to  check  that the constant in time
  ...
  the the viscous strain $F_{\rm vi}$ corresponding to an approximable
...
\subsection{ ${\Gamma}$-$\liminf$ inequalities}\label{sec:ga}
...
for almost every $t\in [0,T]$.
    Moreover notice that, since $\E^{\varepsilon}(t,u_{\varepsilon}(t),v_{\varepsilon}(t))<\infty$ for every $t\in [0,T]$, from assumption \ref{L4} it follows $
  I+\varepsilon \nabla v_{\varepsilon}\in K$ for almost every $x \in \Omega$ and for every $t\in [0,T]$.
...
The  linearized  semistability condition \eqref{semistability
  0} on the other hand is more delicate, since it requires to pass to
...
limit in the loading terms, which is straightforward. 
 ...
 project I5149 (GA\v{C}R 21-06569K) and the OeAD-WTZ project CZ
 09/2023. US is partially funded by projects I4354 and P32788. 
...
P.~G. Ciarlet, J.~Ne\v cas. Injectivity and self-contact in nonlinear elasticity. \emph{Arch. Rational Mech. Anal.} 97 (1987), 171--188.
...
linearized viscoelasticity. {\it SIAM J. Math. Anal.} 50 (2010),
4426--4456.
...
\bibitem{Nahn}
N.~Phan-Thien. {\it 
 Understanding Viscoelasticity: Basics of Rheology}.  Advanced Texts
