...
This robustness verification method \cite{randomized_smoothing} computes the $\ell_2$-norm certificates around an input sample $x$ by counting which class is most likely to be returned when $x$ is perturbed by isotropic Gaussian noise. 
...
\citet{salman} show that randomized smoothing can postprocess the network to make it locally Lipschitz continuous.
...
Following this trend~\citet{dds} make use of a diffusion model as one-shot denoiser achieving state-of-the-art performances.
