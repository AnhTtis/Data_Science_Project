...
\section{Introduction} \label{sec:INTRO}
    ...
    To the best of our knowledge, we are the first work to propose federated learning-enabled LEO constellation (FedLEO) consisting of only operating LEO constellation.
    Note that the GS is only responsible for initial signal controlling with considerably lower overhead compared to raw data processing, FedLEO considers all data process on the orbit, which has well-addressed the bottleneck of LEO-GS transmission delay overhead. 
    ...
    We evaluate the performance of FedLEO under mega-constellation of around 700 satellites under practical constellation setting. 
    Simulation results show that the proposed LESC in FedLEO outperforms the conventional centralized and distributed learning architectures in terms of accuracy and training overhead. 
    
    The rest of the paper is outlined as follows. 
    In Section \ref{sec:SYS}, we introduce the system model, signaling and LEO constellation of FedLEO.
    In Section \ref{sec:PROS}, we describe the proposed LESC algorithm in FedLEO. 
    In Section \ref{sec:SR}, we demonstrate the performance evaluation of FedLEO.
    Finally, conclusions are drawn in Section \ref{sec:CON}.
    
\section{System Architecture and Model} \label{sec:SYS}
\subsection{Architecture and Signalling of FedLEO} \label{sub:SYSFL}
    
    \begin{figure}[t]
        \centerline{\includegraphics[width=8cm]{system1.eps}}
        \caption{System model for FedLEO.}
        \label{fig:sys1}
    \end{figure}
    
    The system architecture of proposed federated learning over LEO constellation (FedLEO) is shown in Fig. \ref{fig:sys1}.  The whole system consists of three parts, including the terrestrial GS, LEO groups of a single edge and several client nodes. 
    ...
    \begin{figure}[htbp]
        ...
        \caption{The signalling of the proposed FedLEO system.}
        \label{fig:sys2}
    \end{figure}
...
\section{Proposed LEO Edge Selection and Clustering (LESC) in FedLEO} \label{sec:PROS}

    FedLEO system contains two main procedures, including LEO edge selection and client clustering as well as generic federated task learning. The concrete algorithm is elaborated in Algorithm \ref{alg:cluster}.
    ...
    \begin{algorithm}[!tb]
    \caption{Proposed LEO Edge Selection and Clustering (LESC) in FedLEO}
    \SetAlgoLined
    \DontPrintSemicolon
    \label{alg:cluster}
    \begin{algorithmic}[1]
...
\section{Performance Evaluation} \label{sec:SR}
    We demonstrate the performance of proposed LESC in FedLEO system.
    ...
    Compared to FedLEO, the main difference in CL is that all data collected from each client will be transmitted to a chosen centralized LEO server performing CL method. 
...
\subsection{Comparison with Benchmark Architecture}
    ...
    We can observed that FedLEO outperforms both CL and DL with the highest accuracy. 
    ...
    \begin{subfigure}{0.245\textwidth}
        ...
        \label{fig:R_m}
    \end{subfigure}
    \caption{Performance comparison of proposed FedLEO system and CL/DL architectures in terms of (a) convergence and (b) accuracy with different distance thresholds of $\delta_d$.}
    \label{fig:R}
    \end{figure}
    ...
    \begin{subfigure}{0.245\textwidth}
        ...
        \label{fig:SNR_m}
    \end{subfigure}
    \caption{Performance comparison of proposed FedLEO system and CL/DL architectures in terms of (a) convergence and (b) accuracy with different SNR thresholds of $\delta_\gamma$.}
    \label{fig:SNR}
    \end{figure}

    In Fig. \ref{fig:SNR}, we cluster clients by the SNR thresholds of  $\delta_\gamma \in \{18, 20, 22, 24\}$ dB. 
    With higher SNR threshold $\delta_\gamma$, FedLEO will cluster LEO clients with comparatively smaller path loss. 
    ...
    \begin{subfigure}{0.245\textwidth}
        ...
        \label{fig:Trc_m}
    \end{subfigure}
    \caption{Performance comparison of proposed FedLEO system and CL/DL architectures in terms of (a) convergence and (b) accuracy with different re-clustering periods of $T_{rc}$.}
    \label{fig:Trc}
    \end{figure}

    Furthermore, the re-clustering is a key mechanism for FedLEO to deal with the time-varying channel. 
    ...
    The result shows that if without re-clustering, the classification accuracy of both FedLEO and CL degrades significantly. 
    By contrast, more frequent re-clustering with smaller $T_{rc}$ provides better performance, which shows the necessity of re-clustering mechanism in FedLEO.
    ...
    \begin{subfigure}{0.245\textwidth}
        ...
        \label{fig:ps_m}
    \end{subfigure}
    \caption{Performance comparison of proposed FedLEO system and CL/DL architectures in terms of (a) convergence and (b) accuracy with different pointing error angle SD of $\sigma_\theta$.}
    \label{fig:ps}
    \end{figure}
    ...
    However, the accuracy of FedLEO outperforms CL, benefited by re-clustering of appropriate candidates of LEO clients.
...
\subsection{LEO Overhead Analysis}
    ...
    The delay time $T_{tr, x}$ accounts for transmission time over ISL $T_{s, x}$, training time per epoch $T_{e, x}$ and FL parameter aggregation time $T_{a, x}$, where $x\in\{F, C, D\}$ indicates FedLEO, CL and DL systems, respectively. 
    The total system delay $T_{tr, F}$ of the proposed FedLEO can be derived as
    \begin{equation}
        \label{eq:TFL}
    ...
    \end{align}
    where $T_{s,C}$ stands for the time of raw data transfer, whilst $T_{e,C}$ is required to perform comparably longer training than either FedLEO or DL due to higher data amount in CL.
    The result is shown in Table \ref{table:FCCOM} with 20 LEO clients in the cluster for fair comparison. 
    The proposed FedLEO system requires total delay time of 2.36 s, which has comparably lower delay than CL of 15.67 s. 
    The bottleneck of CL comes from training and transmission of raw data. 
    Also, the memory required for LEO edge server in FedLEO is much smaller than that in CL.
    Though with a asymptotic overhead with FedLEO, the non-cooperative architecture of DL degrades the accuracy performance without moderate information exchange.
    ...
    \footnotesize
     ...
     \begin{tabular}{L{8em} R{6em} R{6em} R{6em}}%{l*{2}{l}r}
      ...
      & $\textbf{FedLEO}$ & $\textbf{Centralized}$ & $\textbf{Distributed}$\\
      \hline
...
\section{Conclusion} \label{sec:CON}
    In this paper, we conceive an FedLEO system, which enables computing in space without comparably high delay overhead of processing at GS owing to long distance. 
    In FedLEO, LEOs communicate with each other under optical ISLs, which significantly reduces the communication overhead.
    ...
    We have evaluated the proposed FedLEO with LESC in terms of various parameters of clustering thresholds, re-clustering periods, and misalignment effects. 
    Benefited from both ISL and FL, the results have shown that FedLEO with LESC outperforms the benchmarks of conventional centralized and distributed LEO architectures in terms of the highest classification accuracy as well as the lowest overhead of total system delay and required memory footprint.
