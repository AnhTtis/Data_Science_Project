...
%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%
If one takes Sangiorgi's \cbn normal form similarity $\leqcbn$ (\refdef{cbn-nfs}) and simply replaces weak head reduction with one of the weak \cbv reductions (weak, left, or right) then one obtains notions of \cbv normal form similarity. Their compatibility can be proved with the same method that shall be used in \refsect{toy}, obtaining soundness with respect to contextual equivalence. 
...
\begin{definition}[Naive normal form bisimulation for Call-by-Value]
A relation $\relsym$ is a \emph{naive (\cbv) normal form simulation} if $\relsym\subseteq\relncbv$, where $\tm \relncbv \tmp$ holds whenever $\tm,\tmp$ satisfy one of the following clauses:
	\begin{center}
...
%		\item $\tm \bsweak \ntm = \ntmONE\ntmTWO$ and $\tmp \bsweak \ntmtwo = \ntmONEtwo\ntmTWOtwo$ with $\ntmONE\rel\ntmONEtwo$ and $\ntmTWO\rel\ntmTWOtwo$.
%	\end{enumerate}
	\emph{Naive normal form similarity} $\leqncbv$ is the largest naive normal form simulation.
	\end{definition}

Naive (bi)similarity seems defined very naturally, and yet it does not validate \emph{any} of the equivalences of the previous section. Let's discuss $\equivscr$ and $\equivlid$.
\begin{enumerate}
\item \emph{\cbv scrutability}: the scrutable equivalence $\equivscr$ is not validated by $\leqncbv$, and (\cbv) inscrutable terms are not minimal for $\leqncbv$, in contrast with the fact that \cbn inscrutable terms are minimal for Sangiorgi's similarity $\leqcbn$. For instance, $\Omega$ is inscrutable and $\leqncbv$-minimal, but $\Omega^L$ is also inscrutable and one has $\Omega \leqncbv \Omega^L$, because $\Omega\bsweakdiv$, but not $\Omega^L \not\leqncbv \Omega$, because $\Omega^L$ is $\tow$-normal.
...
\paragraph{Naive Similarity and Fix-Points} It is nonetheless possible to prove that the usual \cbv variants of Curry's and Turing's fix-point combinators $\curryfix$ and $\turingfix$ are naively similar, as we now show.
\begin{center}
