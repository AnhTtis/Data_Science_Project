...
%
%
%\section{VSC with practical values big step system completeness proof}
%
...
%
%\begin{lemma}
%	$\tm \tovscp \tmp$ and  $\tmp \bsvscp k \ntm \Rightarrow \tm \bsvscp {k+1} \ntm$
%\end{lemma}
%
%\begin{proof} \label{proof:proof-completeness-big-step-vsc-practical}
%By induction on $(k,d)$ (where $d :\tm \tovscp \tmp$) and case analysis on $\tmp \bsvscp k \ntm$, we build a derivation for $\tm \bsvscp {k+1} \ntm$. Cases of $\tm \tovscp \tmp$:
...
%
%\section{VSC with theoretical values big step system modulo $\sigma_1$ completeness proof}
%
%\paragraph{Small steps.} The same small steps system is used as in the previous section but with theoretical values instead of practical values.
%
%\begin{lemma}
%	$\tm \tovsct \tmp$ and  $\tmp \bsvsct k \fire \Rightarrow \tm \bsvsct {k+1} \fire$
%\end{lemma}
%
%\begin{proof} \label{proof:proof-completeness-big-step-vsc-theoretical}.
%	By induction on $(k,d)$ (where $d :\tm \tovsct \tmp$) and case analysis on $\tmp \bsvsct k \fire$, we build a derivation for $\tm \bsvsct {k+1} \fire$. Cases of $\tm \tovsct \tmp$:
%	\input{VSC-theoretical-sigma1-bigstep-completeness-proof}
%	
%\end{proof}

\section{Proof of Equivalence of Small-Step and Big-Step Operational Semantics}
In this chapter, we give the details for the proof of completeness for the big step systems we introduced using the diamond property. We omit the details for the second system, the proof adapts easily.
...
\subsection{Big-Steps/Small-Steps for VSCp }
\label{proof:proof-completeness-big-step-vsc-practical}
\paragraph{A constrained reduction to model the big steps semantics}
In the Value Substitution Calculus with practical values (VSCp),we define a subreduction $\tos \subseteq \tovscp$ is defined as follows. 
\begin{center}
	$\begin{array}{r@{\hspace{.5cm}}rlll}
		%		\textsc{Terms} & \tm, \tmtwo, \tmthree & \grameq & \var\mid \valp \mid \tm\tmtwo \mid \tm\esub\var\tmtwo
		%		\\
		%		\textsc{Values} & \valp  & \grameq  & \la\var\tm
		%		\\
...
\begin{center}
	$\begin{array}{c@{\hspace{.5cm}}rcc}
		...
		\\
		\tm\esub\var{\isctxp{\valp}} \rtos \isctxp{\tm\isub\var\valp} &
		\ievctxp \tm \tos \ievctxp \tmtwo \textrm{~~~if } \tm \rtos \tmtwo
	\end{array}
	$\end{center}
...
\begin{lemma}
	\label{l:tos-normal-is-tovscp-normal}
	If $\tm$ is $\tos$-normal, then $\tm$ is $\tovscp$-normal.
\end{lemma}
\begin{proof}
	By induction on the structure of $\tm$.
	\begin{itemize}
		\item $\tm =\var$ or $\tm = \la\var\tmp$, the result is immediate
		\item $\tm = \tmrone\tmrtwo$, then $\tmrone$ and $\tmrtwo$ are $\tos$-normal, and by induction are $\tovscp$-normal. There is only one possibility for $\tm$ to $\tovscp$-reduce.
		
		Suppose $\tmrone\tmrtwo \tom \tmp$ then $\tmrone =\sctxp{\la\var\tmronep}$.
		Since $\tmrone$ is a $\tovscp$ normal form, by \reflemma{sctx-normal-is-isctx} we have that $\tmrone = \isctxp{\la\var\tmronep'}$, which contradicts the assumption that $\tm$ is a $\tos$-normal form.
		\item $\tm = \tmrone\esub\var\tmrtwo$, , then $\tmrone$ and $\tmrtwo$ are $\tos$-normal, and by induction are $\tovscp$-normal. There is only one possibility for $\tm$ to $\tovscp$-reduce.
		
		Suppose $\tmrone\esub\var\tmrtwo \toeabs \tmp$ then $\tmrtwo =\sctxp{\valp}$.
		Since $\tmrtwo$ is a $\tovscp$ normal form, by \reflemma{sctx-normal-is-isctx} we have $\tmrtwo = \isctxp{\valptwo}$, which contradicts the assumption that $\tm$ is a $\tos$-normal form.\qedhere
	\end{itemize}
...
\begin{corollary}
	\label{cor:tovscp-and-tos-normal-forms}
	$\tovscp$ and $\tos$ have the same normal forms.
\end{corollary}

\begin{corollary}[Completeness of $\tos$] \label{prop:S_completeness}$ \tm \tovscp^k \tmn  \iff \tm \tos^k \tmn $ 
\end{corollary}
\begin{proof} 
Direct corollary of \Cref{lem:RD_completeness} and Corollary \ref{cor:tovscp-and-tos-normal-forms}.
	\end{proof}
	...
	\begin{lemma}
		...
		$\tm \tos \tmp$ and  $\tmp \bsvscp k \ntm \Rightarrow \tm \bsvscp {k+1} \ntm$
	\end{lemma}
	
	\begin{proof}
		Straightfoward proof by structural induction. Note that substitutivity is never  required.
	\end{proof}
...
\gettoappendix{l:ss-bs-equivalence_vsc}
	...
	\begin{proof}
		$(\Rightarrow)$ by induction on the $(\tm \bsvscp k \ntm)$ derivation.
		
		$(\Leftarrow)$ by induction on $k$ using the fact that $\tm \tovscp^k \tmn$ implies $\tm \tos^k \tmn$  (by \Cref{prop:S_completeness}), and the \reflemma{ss-bs-equivalenceaux1_vsc}.
	\end{proof}
	
	
	

\subsection{Big-Steps/Small-Steps for VSCmodulo }
\label{proof:proof-completeness-big-step-vsc-modulo}

\gettoappendix{l:ss-bs-equivalence_vsce}

\begin{proof}
	Same argument as for VSCp easily adapts, using the same reduction strategy but including variables in the definition of values.
\end{proof}
