...
\section{Removed Proofs from Section 12 About the Type Preorder}
...
\begin{enumerate}
	...
	\newcommand{\ntmleft}{\ntm_{\mathsf{left}}}
	\newcommand{\ntmlefttwo}{\ntmtwo_{\mathsf{left}}}
	The term $\tm$ is typable by a derivation $\typeder: \typectx \types \tm \hastype \mtype$ therefore it is normalizable by \refthm{invariance-and-adequacy}. Hence we have $\tm\tovsc^k\ntm$. By the fact that all VSC terminating terms are Plotkin's left terminating, we have that $\tm\tolas\ntmleft$ and therefore (since $\relsym$ is an enf bisimulation) $\tmp\tolas^*\ntmlefttwo$ with $\ntmleft \relenf\ntmlefttwo$. Instead of looking for a derivation $\typederp$  of $\tmp$, we can look for a derivation $\typederp_1$ of $\ntmlefttwo$ and conclude by (typability) expansion for the $\tolas$ reduction.
