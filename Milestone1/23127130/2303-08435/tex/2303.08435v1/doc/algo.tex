...
\begin{equation}
 m = (W \times \frac{2 * {N\!A}}{\lambda}) \times 2 +1,~
 n = (H \times \frac{2 * {N\!A}}{\lambda}) \times 2 +1,
 \label{eq:kernel_mn}
\end{equation}
...
\begin{algorithm}[h]
  \caption{\fname~Forward Training Procedure}
    \begin{algorithmic}[1]
        ...
        \State $\mathcal{F}(\vec{M}) \gets \texttt{fftshift}(\texttt{fft2}(\texttt{ifftshift}(\vec{M})))$ \label{ag1:line:fftm}
