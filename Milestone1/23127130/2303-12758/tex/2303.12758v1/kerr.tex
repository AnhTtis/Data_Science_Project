...
\begin{align}
    \begin{split}\label{old1.3}
        g_{ij}&=\left(1-\frac{2M}{r}\right)^{-1} dr^2+ r^2 d\si_{\mathbb{S}^2}+o_4(r^{-\frac{s-1}{2}}),\\
        k_{ij}&=o_3(r^{-\frac{s+1}{2}}).
    \end{split}
\end{align}
\end{definition}
...
\end{itemize}
\subsection{Acknowledgements}\label{acknow}
The author is very grateful to J\'er\'emie Szeftel for his support and encouragements.
...
We denote $(\MM,\bg)$ a spacetime $\MM$ with the Lorentzian metric $\g$ and $\D$ its Levi-Civita connection. Let $u$ and $\ub$ be two optical functions on $\MM$, that is
\begin{equation*}
...
\end{equation}
which is a space-like $2$--sphere. We define the area radius of $S(u,\ub)$:
\begin{equation}
...
The spacetime metric $\g$ induces a Riemannian metric $\ga$ on $S(u,\ub)$. We use $\nab$ to denote the Levi-Civita connection of $\ga$ on $S(u,\ub)$. Using $(u,\ub)$, we can introduce a local coordinates system $(u,\ub,x^A)$ on $\MM$ with $e_4(x^A)=0$. The metric $\g$ takes the form:
...
Given $\Si_0$ a spacelike hypersurface, called the \emph{initial hypersurface}, we construct an \emph{initial layer region} $\kk_{(0)}$ near $\Si_0$ endowed with a double null foliation $(u_{(0)},\ub)$ as in Section \ref{doublenullsection} such that
\begin{align}
    \Si_0=\big\{p\in \kk_{(0)}/\, u_{(0)}(p)+\ub(p)=0\big\},
\end{align}
and
\begin{equation}
    0\leq u_{(0)}(p)+\ub(p)\leq 2\de_0,\qquad \forall\, p\in\kk_{(0)},
\end{equation}
...
\end{equation}
where $(C_{(0)})_{u_{(0)}}$ is the outgoing null cone emanating from $S_{(0)}(u_{(0)},-u_{(0)})$ and $\Cb_\ub$ is the ingoing null cone emanating from $S_{(0)}(-\ub,\ub)$. Also, as in Section \ref{doublenullsection}, $\kk_{(0)}$ is covered by coordinates systems $(u_{(0)},\ub,x^A_{(0)})$ with $L_{(0)}(x^A_{(0)})=0$ and the metric $\g$ in $\kk_{(0)}$ has the form:
\begin{equation}
    \g=-2\Om_{(0)}^2(d\ub\otimes du_{(0)}+du_{(0)}\otimes d\ub)+(\ga_{(0)})_{AB}(dx^A_{(0)}-\bu_{(0)}^A du_{(0)})\otimes(dx^B_{(0)}-\bu_{(0)}^B du_{(0)}).
\end{equation}
We also define
\begin{equation}
    K:=\big\{p\in \Si_0/\, w(p)\leq R_0\big\},
\end{equation}
where $R_0>0$ is the constant appearing in \eqref{R0constant} and
\begin{equation}
    w:=\frac{\ub-u_{(0)}}{2}.
\end{equation}
...
\begin{align}
    \begin{split}\label{dddd}
        d_1^*d_1&=-\De_1+\mathbf{K},\qquad \qquad d_1d_1^*=-\De_1,\\
...
We have the following formulae:
\begin{align*}
\tr_K\chi_{K}-\frac{2}{r_K}&=\O_2^1,\qquad\qquad\,\tr_K\chib_{K}+\frac{2}{r_K}=\O_2^1,\\
\om_{K}&=\O_2^1,\qquad\qquad\qquad\;\,\qquad\omb_K=\O_2^1,\\
\hch_{K}&=\O_3^2,\qquad\qquad\qquad\qquad\;\, \hchb_{K}=\O_3^2,\\
\nab_K\tr_K\chi_K&=\O_4^2,\qquad\qquad\quad \nab_K\tr_K\chib_K=\O_4^2,\\
\eta_{K}&=\O_3^2,\qquad\qquad\qquad\qquad\,\,\,\etab_{K}=\O_3^2,\\
\bu_{K}&=\O_2^2,\qquad\qquad\qquad\;\, \Om_{K}-\f12=\O_1^1.
\end{align*}
We also have:
\begin{align*}
    \a_{K} &=\O_5^3, \qquad\qquad\qquad\aa_{K}=\O_5^3,\\
    \b_{K} &=\O_4^2, \qquad\qquad\qquad \bb_{K} =\O_4^2,\\
    \rho_{K}&=\O_3^1, \qquad\qquad\qquad \si_{K} =\O_4^2.
\end{align*}
\end{proposition}
\begin{proof}
...
\begin{lemma}\label{expressionGaR}
    ...
    \begin{align*}
        ...
        \eta&=\O^2_3+\Gag,\qquad\qquad\qquad\;\,\etab=\O_3^2+\Gag,\\
        \om&=\O^1_2+\Gag,\qquad\qquad\qquad\;\,\omb=\O^1_2+\Gab,\\
        \bu&=\O_2^2+r\Gag,\qquad\qquad \Om-\frac{1}{2}=\O_2^1+r\Gag,\\
        \ga&=\O_0^0+r\Gag,\qquad\qquad\qquad\; \in=\O_0^0+r\Gag.
    \end{align*}
    We also have:
    \begin{align*}
        \a&=\O^3_5+\ac,\qquad \quad\qquad\quad \aa=\O^3_5+\aac,\\
        \b&=\O^2_4+\bc,\qquad\quad \qquad\quad \,\bb=\O_4^2+\bbc,\\
        \rho&=\O_3^1+\rhoc,\qquad \quad\quad\quad\;\quad \si=\O_4^2+\sic.
    \end{align*}
\end{lemma}
\begin{proof}
    It follows directly from Definitions \ref{gammag}, \ref{KerrO} and Proposition \ref{decayGamma}.
\end{proof}
The following lemma allows us to compute the derivatives of $\O_q^p$.
\begin{lemma}\label{newuseful}
    ...
    Then, we have
    \begin{align*}
        \Om\nab_3 X_K&=\O^p_{q+1}+\O_q^p\c\Gag+\O_{q}^p\c\Jbc+\O_q^p\c\Lc,\\
        \Om\nab_4 X_K&=\O^p_{q+1}+\O_{q}^p\c\Jc,\\
        \nab X_K&=\O^p_{q+1}+\O_{q}^p\c\Lc.
    \end{align*}
\end{lemma}
\begin{proof}
    ...
    \begin{align*}
        ...
        &=\bcc^A(\nab-\nab_K)_{\pr_{x^A}}X_K+\bcc^A(\nab_K)_{\pr_{x^A}}X_K+\Jbc\c\O_q^p+\Lc\c\O_q^p\\
        &=\Gag\c\O_q^p+\Jbc\c\O_q^p+\Lc\c\O_q^p,
    \end{align*}
    ...
    \begin{equation*}
        \Om\nab_3 X_K=\O_{q+1}^p+\Gag\c\O_q^p+\Jbc\c\O_q^p+\Lc\c\O_q^p.
    \end{equation*}
...
\begin{corollary}\label{newuse}
    ...
    \begin{equation*}
        \nab(\O_q^p\c\Gag)=\O_{q+1}^p\c\Gag^{(1)},\qquad \nab(\O_q^p\c\Gab)=\O_{q+1}^p\c\Gab^{(1)}.
    \end{equation*}
\end{corollary}
\begin{proof}
    We have from Lemma \ref{newuseful}
    \begin{align*}
        \nab(\O_q^p\c\Ga_i)&=\O_q^p\c\nab\Ga_i+\nab\O_q^p\c\Ga_i\\
        &=\O_{q+1}^p\c\Ga_i^{(1)}+(\O_{q+1}^p+\O_q^p\c\Lc)\c\Ga_i\\
        &=\O_{q+1}^p\c\Ga_i^{(1)},
    \end{align*}
...
\begin{align*}
\nab_4\,\widecheck{\Om\trch}+\tr\chi\,\widecheck{\Om\trch}=&\O_1^0\c\widecheck{\Om\om}+\fl[\nab_4\,\trchc]+\err[\nab_4\,\trchc],\\
\fl[\nab_4\,\trchc]:=&\Gag\c\O_2^1,\\
\err[\nab_4\,\trchc]:=&\Gag\cdot\Gag,\\\\
\nab_3\,\widecheck{\Om\trchb}+\tr\chib\,\widecheck{\Om\trchb}=&\O_1^0\c\widecheck{\Om\omb}+\fl[\nab_3\,\trchc]+\err[\nab_3\,\trchc],\\
\fl[\nab_3\,\trchbc]:=&\Gag\c\O_2^1,\\
\err[\nab_3\,\trchbc]:=&\Gab\c\Gab,\\\\
\nab_4(\widecheck{\Om\trchb})=&-2\Om\mubmc+\fl[\nab_4\,\trchbc]+\err[\nab_4\,\trchbc],\\
\fl[\nab_4\,\trchbc]:=&\Gag\c\O_3^2,\\
\err[\nab_4\,\trchbc]:=&\Gag\c\Gag,\\\\
\nab_3(\widecheck{\Om\trch})=&-2\Om\mumc+\fl[\nab_3\,\trchc]+\err[\nab_3\,\trchc],\\
\fl[\nab_3\,\trchc]:=&\Gag\c\O_3^2,\\
\err[\nab_3\,\trchc]:=&\Gag\c\Gag.
\end{align*}
We also have the linearized Codazzi equations:
\begin{align*}
\sdiv(\hchc)=&-\bc+\O_0^0\c\nab\trchc+\O_1^0\c\zec+\fl[\sdiv(\widecheck{\hch})]+\err[\sdiv(\widecheck{\hch})],\\
\fl[\sdiv(\widecheck{\hch})]:=&\Gag\c\O_3^2+\O_3^2\c\Lc,\\
\err[\sdiv(\widecheck{\hch})]:=&\Gag\c\Gag,\\ \\
\sdiv(\widecheck{\hchb})=&\bbc+\O_0^0\c\nab\trchbc+\O_1^0\c\zec+\fl[\sdiv(\widecheck{\hchb})]+\err[\sdiv(\widecheck{\hchb})],\\
\fl[\sdiv(\widecheck{\hchb})]:=&\Gab\c\O_3^2+\O_3^2\c\Lc,\\
...
\fl[\nab_4\mumc]:=&\O_3^2\c (d_2^*\etac,\nab\trchc,\bc)+\O_3^1\c(\Gag,\Lc),\\
...
\fl[\nab_3\mubmc]:=&\O_3^2\c (d_2^*\etabc,\nab\trchbc,\bbc)+\O_3^1\c(\Gag,\Lc),\\
...
\fl[\nab_3(\rhoc,\sic)]:=&\Gag^{(1)}\c\O_3^1+\O_3^2\c(\aac,\bbc), \\
\err[\nab_3(\rhoc,\sic)]:=&\Gag\c(\aac,\bbc),\\ \\
\nab_4(\rhoc,-\sic)+\frac{3}{2}\trch(\rhoc,-\sic)
=&d_1\bc+\fl[\nab_4(\rhoc,-\sic)]+\err[\nab_4(\rhoc,-\sic)],\\
\fl[\nab_4(\rhoc,-\sic)]:=&\Gag^{(1)}\c\O_3^1+\O_3^2\c(\ac,\bc),\\
...
\begin{align*}
        \nab_4(\gac_{AB})-\trch(\gac_{AB})=&\O_{-2}^0\c(\hchc,\trchc)+O(r)\Omc+r^3\Gag\c\Gag,\\
        \nab_4(\Omc)=&\O_0^0\c\widecheck{\Om\om}+\O_1^1\c\Gag+r\Gag\c\Gag,\\
        \nab_4(\bcc^A)=&\O_1^0\c\zec+\O_3^2\c\Gag+\Gag\c\Gag,\\
        \nab_4(\inc_{AB})-\trch(\inc_{AB})=&\O_{-2}^0\c\trchc+r^3\Gag\c\Gag.
\end{align*}
...
\begin{align*}
    \nab_3(\gac_{AB})-\trchb(\gac_{AB})=&\O_{-2}^0\c(\widecheck{\trchb},\hchbc)+\O_3^2\c\gac_{AB},\\
    \nab_3(\inc_{AB})-\trchb(\inc_{AB})=&\O_{-2}^0\c\trchbc+\O^2_3\c\inc_{AB}.
\end{align*}
\end{proposition}
\begin{proof}
...
\begin{align*}
         ...
         \RRb_{q}[(\rhoc,\sic)](u,\ub)&:=|u|^{\frac{s-4}{2}}r^2\Vert (r\nab)^q(\rhoc,\sic) \Vert_{2,\ucuv},\\
         \RRb_{q}[\bbc](u,\ub)&:=|u|^{\frac{s-2}{2}}r \Vert (r\nab)^q\bbc\Vert_{2,\ucuv},\\
...
\end{align*}
We can extend the foliation $(u,\ub)$ to a neighborhoood of $\Si_0\sm K$ in $J^-(\Si_0\sm K)$ such that it is well defined on $\Si_0\sm K$. The curvature flux on $\Si_0\sm K$ is defined by:
\begin{equation*}
...
\end{split}
\end{align}
Then, $\KK_{(0)}$ has a unique future development $(\MM,\g)$, defined outside its future domain of influence $I^+(\kk_{(0)})$ with the following properties:
\begin{enumerate}
...
\end{equation}
\end{thmM1}
Theorem M1 is proved in Section \ref{curvatureestimates}. The proof is based on the $r^p$--weighted estimate method introduced by Dafermos and Rodianski in \cite{Da}.
\begin{thmM2}
...
\end{lemma}
\begin{proof}
It follows directly from the assumption \eqref{asm1}, Definition \ref{gammag} and the Sobolev inequalities in Propositions \ref{sobolevkn} and \ref{standardsobolev}.
\end{proof}
...
\begin{split}\label{div}
       ...
       =&2k r^p d_{1}(\psi_{(1)}\cdot\psi_{(2)})
...
\end{equation}
which is a direct consequence of Propositions \ref{Bianchieq}, \ref{Bianchieqdkb} and \ref{sobolevkn}.
\end{proof}
...
\end{split}
\end{align}
Recall the linearzied torsion equation in Proposition \ref{nullstructure}:
\begin{align}
...
Notice that we have
\begin{align*}
    \widecheck{\Om\trchb}=\Om^2(\Om^{-1}\trch)-\Om^2_K(\Om^{-1}_K\tr_K\chib_K)=\widecheck{\Om^2}(\Om^{-1}\trchb)+\Om^2_K(\widecheck{\Om^{-1}\trchb}),
\end{align*}
which implies
\begin{align*}
...
\end{align*}
Integrating it along $e'_4-e'_3$, we deduce\footnote{Recall that ${x'}^A=x^A$ on $\Cb_*\cap\Si_0$ which is chosen in Section \ref{bootregion}.}
\begin{align*}
    |{x'}^A-x^A|_{\infty,S'}\les \int_{w}^{\ub_*} \frac{\ep_0}{{r}^{\frac{s+1}{2}}}dw\les \frac{\ep_0}{r^\frac{s-1}{2}}.
\end{align*}
...
\begin{align}
    s_0:=\min\left\{s,\frac{15}{2}\right\}.
\end{align}
We define for $q=0,1$:
\begin{align}
...
&\Div(r^p|\amrc|^2e_3)+2\Div(r^p(|\asc|^2-2|\ac|^2)e_4)\\
&+(p+2)r^{p-1}|\amrc|^2+2(8-p)r^{p-1}(|\asc|^2-2|\ac|^2)\\
=&4r^pd_1(\amrc\c\asc)+\O_{5-p}^3\c\Gag^{(1)}\c\ac+r^p(\amrc,\asc)\c(\fl[\a]+\err[\a])\\
&+r^p(\O_2^1+\Gab)\c(|\amrc|^2,|\asc|^2,|\ac|^2).
\end{split}
\end{align}
\end{lemma}
\begin{proof}
...
\begin{align*}
    ...
    =&4r^pd_1(\amrc\c\asc)+2r^p\amrc\c h_{(1)}+4r^p\asc\c h_{(2)}+r^p(\O_2^1+\Gab)\c(|\amrc|^2,|\asc|^2),
\end{align*}
which implies
\begin{align*}
    &\Div(r^p|\amrc|^2e_3)+2\Div(r^p|\asc|^2e_4)+(p+2)r^{p-1}|\amrc|^2+2(8-p)r^{p-1}|\asc|^2\\
    =&4r^pd_1(\amrc\c\asc)+8r^{p-1}\amrc\c\ac+r^p\amrc\c(\fl[\a]+\err[\a])+r^p(\O_2^1+\Gab)\c(|\amrc|^2,|\asc|^2).
\end{align*}
Next, we compute
\begin{align*}
    4\Div(r^p|\ac|^2e_4)&=4r^p|\ac|^2\Div(e_4)+4\nab_4(r^p|\ac|^2)\\
    &=4r^p|\ac|^2(\trch-2\om)+4\nab_4(r^{p-10}|r^5\ac|^2)\\
    &=8r^{p-1}|\ac|^2+8\nab_4(r^5\ac)\cdot r^{p-5}\ac+4(p-10)r^{p-1}|\ac^2|+r^p(\O_2^1+\Gag)\c|\ac|^2\\
    &=4(p-8)r^{p-1}|\ac|^2+8r^{p-1}\amrc\cdot\ac+r^p(\O_2^1+\Gag)\cdot|\ac|^2+\O_{5-p}^3\c\Gag^{(1)}\c\ac,
\end{align*}
where we used
\begin{align}
\begin{split}\label{totallye4a}
    \amrc&=\frac{1}{r^4}\nab_4(r^5\a)-\frac{1}{r_K^4}(\nab_K)_{(e_4)_K}(r_K^5\a_K)\\
    &=\frac{1}{r^4}\nab_4(r^5\a)-\frac{1}{r^4}\nab_4(r^5\a_K)+\frac{1}{r^4}\nab_4(r^5\a_K)-\frac{1}{r_K^4}(\nab_K)_{(e_4)_K}(r_K^5\a_K)\\
    &=\frac{1}{r^4}\nab_4(r^5\ac)+\O_4^3\c\Gag^{(1)}.
\end{split}
\end{align}
Hence, we have
\begin{align*}
&\Div(r^p|\amrc|^2e_3)+2\Div(r^p(|\asc|^2-2|\ac|^2)e_4)+(p+2)r^{p-1}|\amrc|^2+2(8-p)r^{p-1}(|\asc|^2-2|\ac|^2)\\
=&4r^pd_1(\amrc\c\asc)+\O_{5-p}^3\c\Gag^{(1)}\c\ac+r^p\amrc\c(\fl[\a]+\err[\a])+r^p(\O_2^1+\Gab)\c(|\amrc|^2,|\asc|^2,|\ac|^2).
\end{align*}
This concludes the proof of Lemma \ref{keyidentity}.
\end{proof}
...
\begin{proposition}\label{poincare}
    ...
    \begin{align*}
        |rd_2\a|^2_{L^2(S)} \geq c_2 |\a|^2_{L^2(S)},
    \end{align*}
...
\end{proposition}
\begin{proof}
See Proposition C.1 in \cite{ShenMink}.
\end{proof}
\begin{proposition}\label{keyint}
We have the following estimate:
\begin{align*}
    \int_{\cuv}r^{s_0}|\amrc|^2 +\int_{\ucuv}r^{s_0}|\asc|^2+\int_V r^{s_0-1}\left(|\amrc|^2 +|\asc|^2+|\ac|^2\right)\les\frac{\ep_0^2}{|u|^{s-s_0}}.
\end{align*}
\end{proposition}
\begin{proof}
    Integrating \eqref{integralkey} with $p=s_0$ and proceeding as in Proposition \ref{estab}, we obtain
    \begin{align*}
        &\int_{C_u}r^{s_0}|\amrc|^2 +\int_{\Cb_\ub}r^{s_0}(|\asc|^2-2|\ac|^2)+\int_V r^{s_0-1}\left(|\amrc|^2+|\asc|^2-2|\ac|^2\right)\\
        \les &\int_{\Si_0\cap V} r^{s_0}(|\amrc|^2+|\asc|^2)+\int_V r^{s_0}|(\amrc,\asc)||\fl[\a]+\err[\a]|+M^3r^{s_0-5}|\Gag^{(1)}||\ac|\\
        &+\int_V r^{s_0}(\O_2^1+\Gab)(|\amrc|^2+|\asc|^2).
    \end{align*}
    First, we have
    \begin{align*}
...
Next, we estimate
\begin{align*}
    &\int_V r^{s_0}|(\amrc,\asc)||\fl[\a]|+M^3r^{s_0-5}|\Gag^{(1)}||\ac|\\
    \les & \int_V r^{s_0} |(\amrc,\asc)|\left|\O_4^2\c\Gag^{(1)}+\O_2^1\c\bc^{(1)}\right|+r^{s_0}|\ac||\O_5^3\c\Gag^{(1)}|\\
    \les &\int_V M^2 r^{s_0-4} |(\amrc,\asc)||\Gag^{(1)}|+ Mr^{s_0-2} |(\amrc,\asc)||\bc^{(1)}|\\
...
\end{equation*}
Moreover, we have from Proposition \ref{poincare}
\begin{align*}
    |rd_2\ac|_{L^2(S)}^2\geq c_2|\ac|^2_{L^2(S)}\geq\frac{7}{2}|\ac|^2_{L^2(S)},
\end{align*}
for $\ep$ small enough. Combining the above estimates, we deduce
\begin{align*}
...
\end{equation}
Recalling from \eqref{totallye4a} that
    \begin{align*}
        \amrc=\frac{1}{r^4}\nab_4(r^5\ac)+\O_4^3\c\Gag^{(1)},
    \end{align*}
...
We compute
\begin{align*}
4\Div(r^{-2}|\aac|^2e_3)&=4e_3(r^{-2}|\aac|^2)+4r^{-2}|\aac|^2\Div(e_3)\\
&=4e_3(r^{-12}|r^5\aac|^2)+4r^{-2}|\aac|^2(\trchb-2\omb)\\
&=40 r^{-3}|\aac|^2+8r^{-3}\aamrc\cdot\aac+r^{-2}(\O_2^1+\Gab)|\aac|^2+\O_7^3\c\Gab^{(1)}\c\aac,
\end{align*}
 where we used
\begin{align}
\begin{split}\label{totallye3aa}
    ...
    &=\frac{1}{r^4}\nab_3(r^5\ac)+\O_4^3\c\Gab^{(1)}.
\end{split}
\end{align}
Combining the above identities, we deduce
\begin{align*}
    &2\Div(r^{-2}(|\aasc|^2-|\aac|^2)e_3)+\Div(r^{-2}|\aamrc|^2)+10r^{-2}\trchb|\aasc|^2+40r^{-3}|\aac|^2\\
    =&2r^{-2}d_1(\aasc\c\aamrc)+r^{-2}(\aasc,\aamrc)\c(\fl[\aa]+\err[\aa])+\O_7^3\c\Gab^{(1)}\c\aac+r^{-2}(\O_2^1+\Gab)|\aac|^2.
\end{align*}
Integrating it in $V$, we obtain
\begin{align*}
    &\int_{\cuv}r^{-2}|\aasc|^2 +\int_{\ucuv}r^{-2}|\aamrc|^2\\
    \les&\int_{\Si_0\cap V} r^{-2}(|\aasc|^2 +|\aamrc|^2)+\int_V r^{-3}|\aasc|^2+r^{-2}|(\aamrc,\aasc)||\fl[\aa]+\err[\aa]|\\
    &+\int_V M^3r^{-7}|\Gab^{(1)}||\aac|+ r^{-2}(\O_2^1+\Gab)|\aac|^2.
\end{align*}
We first have
\begin{align*}
...
Notice that
\begin{align*}
\int_V r^{-3}|\aasc|^2 \les \int_{|u|}^\ub \frac{d\ub}{r^3}\int_\ucuv |\aasc|^2\les \ep_0^2\int_{|u|}^\ub \frac{d\ub}{r^3|u|^s}\les\frac{\ep_0^2}{|u|^{s+2}}.
\end{align*}
We also have
\begin{align*}
    &\int_V r^{-2}|\aamrc,\aasc||\fl[\aa]|\\
    \les&\int_V r^{-2}|\aamrc,\aasc||\O_4^2\c\Gab^{(1)}+\O_2^1\c\aac^{(1)}|\\
    \les&\int_V M^2 r^{-6}|\aamrc||\Gab^{(1)}|+ M^2 r^{-6}|\aasc||\Gab^{(1)}|+Mr^{-4}|\aamrc||\aac^{(1)}|+Mr^{-4}|\aasc||\aac^{(1)}|\\
    \les& M^2\int_{|u|}^\ub \frac{d\ub}{r^5}\left(\int_\ucuv r^{-2}|\aamrc|^2 \right)^\f12\left(\int_\ucuv |\Gab^{(1)}|^2\right)^\f12+ M^2\int_{-\ub}^u \frac{du}{r^4}\left(\int_\cuv r^{-2}|\aasc|^2 \right)^\f12\left(\int_\cuv |r^{-1}\Gab^{(1)}|^2\right)^\f12\\
    &+M\int_{|u|}^\ub \frac{d\ub}{r^4}\left(\int_\ucuv |\aamrc|^2\right)^\f12\left(\int_\ucuv |\aac^{(1)}|^2\right)^\f12+M\int_{-\ub}^u \frac{du}{r^3}\left(\int_\cuv r^{-2}|\aasc|^2 \right)^\f12\left(\int_\cuv |\aac^{(1)}|^2\right)^\f12\\
    \les& M^2\int_{|u|}^\ub \frac{d\ub}{r^5} \frac{\ep}{|u|^\frac{s+2}{2}}\left(\int_{-\ub}^u du|\Gab^{(1)}|^2_{2,S}\right)^\f12+M^2\int_{-\ub}^u \frac{du}{r^4}\frac{\ep}{|u|^\frac{s+2}{2}}\left(\int_{-\ub}^u du |r^{-1}\Gab^{(1)}|^2_{2,S}\right)^\f12\\
    &+M\int_{|u|}^\ub \frac{d\ub}{r^4}\frac{\ep}{|u|^\frac{s}{2}}\frac{\ep}{|u|^\frac{s}{2}}+M\int_{-\ub}^u \frac{du}{r^3}\frac{\ep}{|u|^\frac{s+2}{2}}\frac{\ep}{|u|^\frac{s}{2}}\\
    \les&\frac{M^2\ep^2}{r^4|u|^s}+\frac{M\ep^2}{r^3|u|^s}\\
    \les&\frac{\ep_0^2}{|u|^{s+2}}.
\end{align*}
...
Moreover, we have
\begin{align*}
    \int_V M^3r^{-7}|\aac||\Gab^{(1)}|&\les M^3\int_{|u|}^\ub \frac{d\ub}{r^7}\left(\int_\ucuv |\aac|^2\right)^\f12 \left(\int_\ucuv |\Gab^{(1)}|^2\right)^\f12 \\
...
\begin{align*}
    \int_\cuv r^{-2}|\aasc|^2+\int_\ucuv r^{-2}|\aamrc|^2\les \frac{\ep_0^2}{|u|^{s+2}},
\end{align*}
...
\bibitem{choquet} Y.~Choquet-Bruhat, \emph{Th\'eor\`eme d'existence pour certain syst\`emes d'\'equations aux d\'eriv\'ees partielles non lin\'eaires}, Acta Matematica \textbf{88}, 141--225, 1952.
...
\bibitem{Hintz} P.~Hintz, \emph{Exterior stability of Minkowski space in generalized harmonic gauge}, arXiv:2302.13804.
...
\bibitem{Kerr} R.~P.~Kerr, \emph{Gravitational field of a spinning mass as an example of algebraically special metrics}, Physical Review Letters \textbf{11}, 237, 1963.
...
\bibitem{knpeeling} S.~Klainerman and F.~Nicol\`o, \emph{Peeling properties of asymptotically flat solutions to the Einstein vacuum equations}, Class. Quantum Grav. \textbf{20}, 3215--3257, 2003.
...
\bibitem{KS:main} S.~Klainerman and J.~Szeftel, \emph{Kerr stability for small angular momentum}, arXiv:2104.11857.
\bibitem{KS:Survey} S.~Klainerman and J.~Szeftel, \emph{Brief introduction to the nonlinear stability of Kerr}, arXiv:2210.14400.
...
\bibitem{Shen} D.~Shen, \emph{Construction of GCM hypersurfaces in perturbations of Kerr}, arXiv:2205.12336.
\bibitem{ShenMink} D.~Shen, \emph{Stability of Minkowski spacetime in exterior regions}, arXiv:2211.15230.
\bibitem{teukolsky} S.~A.~Teukolsky, \emph{Rotating black holes: Separable wave equations for gravitational and electromagnetic perturbations}, Physical Review Letters \textbf{29}, No. 16, 1114--1118, 1992.
\end{thebibliography}
\end{document}