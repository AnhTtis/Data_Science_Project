...
\title{Optimal Estimation of the Binned Mask-Free Power Spectrum, Bispectrum, and Trispectrum on the Full Sky}
%\date{\today}
...
\beq\label{eq: gaunt-spin-def}
    w^{LM}_{\ell_1\ell_2m_1m_2} &\equiv &\sqrt{\frac{(2\ell_1+1)(2\ell_2+1)(2\ell_3+1)}{4\pi}}\tj{\ell_1}{\ell_2}{\ell_3}{m_1}{m_2}{m_3}\tj{\ell_1}{\ell_2}{\ell_3}{-1}{-1}{2}\\\nonumber
    &\equiv& \int d\hn\,{}_{+1}Y_{\ell_1m_1}(\hn){}_{+1}Y_{\ell_2m_2}(\hn){}_{-2}Y_{\ell_3m_3}(\hn), 
\eeq
which we have written as an integral over three spin-weighted spherical harmonics, ${}_{s}Y_{\ell m}(\hn)$, in the final line. Note that this is symmetric under interchange of the $(\ell_i,m_i)$ pairs. The reason for the spin-weighting in $w$ will be explained in \S\ref{subsubsec: t4-ideal}.
...
\begin{empheq}[box=\widefbox]{align}\label{eq: binned-tl}
  \av{a_{\ell_1m_1}a_{\ell_2m_2}a_{\ell_3m_3}a_{\ell_4m_4}}_c &\approx \sum_{LM}(-1)^Mw^{L(-M)}_{\ell_1\ell_2m_1m_2}w^{LM}_{\ell_3\ell_4m_3m_4}\frac{t(\vec b,B)}{\Delta_4(\vec b)}\Theta_L(B)\\\nonumber
    &\qquad\,\times\,\left[\Theta_{\ell_1}(b_1)\Theta_{\ell_2}(b_2)\Theta_{\ell_3}(b_3)\Theta_{\ell_4}(b_4)+\text{7 perms.}\right]+(\ell_2\leftrightarrow\ell_3)+(\ell_2\leftrightarrow\ell_4),
\end{empheq}
...
Finally, let us consider optimal estimation of the full-sky trispectrum. Unlike for lower order statistics, this has rarely been considered previously (though see \citep{2015arXiv150200635S,2011MNRAS.412.1993M,Regan:2010cn} for notable examples) and the impact of masks has not been carefullly assessed. Furthermore, as noted in \S\ref{sec: ideal-binned}, the trispectrum contains both a parity-even and a parity-odd part: the estimators below are the first to measure the latter part. 
...
\eeq
where we have note that all 24 permutations are equivalent (due to the symmetry of the four $h$ fields), and explicitly restricted to even or odd $\ell_{1234}\equiv \ell_1+\ell_2+\ell_3+\ell_4$. By expanding the square bracket, this can be split into two coupled pieces:
...
\beq\label{eq: t-general-spin}
    \sum_{m_1m_2}\tj{\ell_1}{\ell_2}{L}{m_1}{m_2}{M}h_{\ell_1m_1}h_{\ell_2m_2} &=& \tj{\ell_1}{\ell_2}{L}{-s_1}{-s_2}{s_{12}}^{-1}\sqrt{\frac{4\pi}{(2\ell_1+1)(2\ell_2+1)(2\ell_3+1)}}\\\nonumber
...
\eeq
On the RHS, we have inserted the spin-weighted Gaunt factor definition \eqref{eq: gaunt-spin-def} for a general set of spins $\{s_1,s_2,s_3\}$. This allows the $m_i$ summations to be rewritten as an integral (or equivalently, a set of spin-weighted harmonic transforms); given an appropriate definition for the \textit{reduced} trispectrum coefficients, it also allows us to separate the $\ell_i$ summations.
...
\beq
    ...
    &&\,\times\,\left\{\left[w^{L'(-M')}_{\ell_1\ell_2m_1m_2}w^{L'M'}_{\ell_3\ell_4m_3m_4}\Theta_{\ell_1}(b_1')\cdots\Theta_{\ell_4}(b_4')+\text{7 perms.}\right]\,+\,(\ell_2\leftrightarrow\ell_3)+(\ell_2\leftrightarrow\ell_4)\right\},
\eeq
...
\beq
    ...
    &&\,+\,(\ell_2\leftrightarrow\ell_3)+(\ell_2\leftrightarrow\ell_4).
\eeq
