...
\STATE Optimize \begin{equation}
    \pi_i \gets \argmin_{\pi \in \Pi} \mathbb{E}_{s \sim \mathcal{D}, a \sim \pi(s)}[\mathbb{E}[\hat{Q}_t|s_t = s, a_t = a]] - \alpha H(\pi).
\end{equation}
\ENDFOR
...
\begin{equation}
    \pi_{i} = \argmin_{\pi \in \Pi} \sum_{j=0}^{i-1} \mathbb{E}_{\substack{t \sim U[0, T], s \sim \eta^t}}[Q^{\pi_j}(s, \pi(s))] - \alpha H(\pi). \nonumber
\end{equation}
