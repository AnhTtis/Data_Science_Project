...
Note that in our framework, these methods can be regarded as using the classification head only (except \cite{jiang2021layercam}) for $\mathbf f_\text{explain}$, see \cref{sec:method:intermediate}. To evaluate them at earlier layers, we simply expand $\mathbf f_\text{explain}$ accordingly to include more network layers.
