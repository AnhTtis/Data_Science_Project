...
Note that this setting differs from pixel removal metrics (\eg \cite{samek2016evaluating,srinivas2019full}), where `removing' a patch of pixels at the input and replacing it with a baseline (\eg zero) values may still result in the patch influencing the network's decision, for example, based on the shape and the location of the patch. In contrast, we effectively make the \emph{weights} between the CNN backbone and the classification heads for other grid cells zero, which ensures no influence from pixels in those grid cells to the output.
