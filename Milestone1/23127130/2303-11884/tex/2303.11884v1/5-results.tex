...
\subsection{Evaluation on Layer-wise Relevance Propagation Configurations}
\label{subsec:lrp}
\input{fig_box_lrp}
\input{fig_vis_lrp}
...
In this section, we study the impact of the choice of rules in attribution performance of \lrp. In particular, we evaluate on four configurations. First, the configuration used in \cite{arias2021explains} and the previous sections, hereafter referred to as LRP-Focus. Second, the composite configuration proposed in \cite{montavon2019layer}, that applies the $\epsilon$-rule on fully connected layers, the $\gamma$-rule on convolutional layers except the first layer, and the $z^{\mathcal{B}}$-rule on the first convolutional layer. The third and fourth configurations involve applying the $z^+$-rule on all layers (equivalent to Excitation Backprop) and the $\epsilon$-rule on all layers (equivalent to \ixg) respectively. Quantitative results can be found in \cref{fig:lrpbox}.

We find that LRP-Focus outperforms all other configurations on both models for \gridpg. Both the $\gamma$-rule (used in LRP-Composite) and the $z^+$-rule (used in LRP-Focus) favour activations that contribute positively to the output while propagating relevance, but to different extents. In fact, the $z^+$-rule is equivalent to the $\gamma$-rule as $\gamma\to\infty$, with only positive contributions being considered for relevance propagation. This would cause the pixels that contribute the most positively to the output logit to get most of the attributions, and lead to high localization scores. The other three configurations localize pooly, particularly on Resnet152, which shows that the performance of LRP is highly sensitive to the choice of configuration.


\myparagraph{Impact of Network Architecture:}
LRP-Composite is popularly used \cite{montavon2019layer,anders2021software} as a standard LRP configuration for image classification tasks. As discussed in \cite{montavon2019layer} and seen in \cref{fig:lrpbox}, it generates attributions that localize well and are visually pleasing for VGG models. However, on Resnet152, the performance drops significantly. Qualitative AggAtt results are shown in \cref{fig:vis:lrp}. We find that while well localized on VGG19 (similar to LRP-Focus, as seen in \cref{fig:lrpbox} and \cref{fig:vis:pg}), the attributions are sparse and often imperceptible on Resnet152. This shows that a configuration that performs well on one model might not necessarily do so on another.

Overall, we find that the performance of \lrp is highly sensitive to the choice of rules, and also varies based on the model. While it is possible to obtain attributions that localize well, by carefully selecting the rules at each layer, it also suggests that caution should be employed before using LRP on a new model and setting.
