...
\begin{proof}
	$ $
	\begin{itemize}
		\item \underline{Strongly connection} Let $t_1, t_2$ s.t. $t_1 \not \leq^h t_2$. If $t_2 \ [\so \cup \wro]^* t_1$, then $t_2 \leq^h t_1$. Otherwise, as $\lnot(t_1 \ [\so \cup \wro]^* \ t_2)$ and $\minimalDependency$ halts (lemma \ref{lemma:minimalDependency-halts}) either \linebreak $\minimalDependency(h, t_1, t_2, \bot)$ or $\minimalDependency(h,t_2, t_1, \bot)$ holds. But as $t_1 \not\leq^h t_2$, $t_2 \leq^h t_1$.
