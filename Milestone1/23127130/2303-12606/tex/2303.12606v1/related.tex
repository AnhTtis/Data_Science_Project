...
For instance, reasoning about the completeness and the (strong) optimality of existing DPOR algorithms for shared-memory is agnostic to the scheduler ($\genericNext$ function) while the strong optimality of our $\textsc{explore-ce}$ algorithm relies on the scheduler keeping at most one transaction pending at a time. \nver{In addition, unlike TruSt, \textsc{explore-ce} invariants guarantee no swapped events can be swapped again and that $<$ is an extension of $\so \cup \wro$.} This makes our completeness and optimality proofs radically different. \nver{Moreover, even for transactional programs with one access per transaction, where $\SER$ and $\SC$ are equivalent, TruSt under $\SC$ and $\textsc{explore-ce}^*$ under $(I_0,\SER)$ do not coincide (for $I_0 \in \{\RA, \RA, \CC\}$). In this case, TruSt enumerates only $\SC$-consistent histories at the cost of solving a NP-complete problem at each step while $\textsc{explore-ce}^*$ step cost can be reduced to polynomial time at the price of not being strongly-optimal.}
