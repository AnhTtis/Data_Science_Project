...
\label{proof:impossibility}
	...
	\nver{From $\hist_1$, depending on the order defined by $\genericNext$ between $\erd{y}$ and $\ewrt{y, 1}$, $\textsc{explore}$ can be called recursively either on $\hist_1$, on $\hist_{2}$ in Figure~\ref{fig:ser-non-optimal:2}, or on $\hist_{3}$ in Figure~\ref{fig:ser-non-optimal:3} before adding $\erd{y}$. From $\hist_1$ and $\hist_2$, $\textsc{explore}$ explores $\hist_{11}$ in Figure~\ref{fig:ser-non-optimal:11} and $\hist_{21}$ in Figure~\ref{fig:ser-non-optimal:21} respectively; while from $\hist_3$ two alternative histories may be explored: $\hist_{31}$ and $\hist_{32}$ in Figure~\ref{fig:ser-non-optimal:31} and Figure~\ref{fig:ser-non-optimal:32} respectively.} 

	\nver{However, from histories $\hist_{11}$, $\hist_{21}$ or $\hist_{31}$ $\textsc{explore}$ will necessarily be called recursively on a history $\hist$ like in Figure~\ref{fig:ser-non-optimal:h} which does not satisfy $I$, thereby violating strong optimality. Thus, any $\textsc{explore}$ implementation that is strong optimal should only explore $\hist_{32}$. In such case, by the restrictions on the $\genericSwap$ function (defined in Section~\ref{sec:algs}), any extension of $\hist_{32}$ does not allow to explore the history where $\erd{x}$ reads from $\ewrt{x,2}$: any outcome of a re-ordering between two contiguous subsequences $\alpha$ and $\beta$ must be prefix of $\hist_e$ when the events in $\alpha$ are taken out. In particular, for any extension $\hist'$ of $\hist_{32}$ and pair of contiguous sequences $\alpha, \beta$ such that $\hist' \setminus \alpha$ is a prefix of $\hist'$, if an event from the second transaction belongs to $\beta$, $\erd{y}$ must also be in $\beta$. Therefore, $\ewrt{x, 2}$ must also be in $\beta$, and so $\erd{x}$ must be. Analogously, if $\erd{x}$ belongs to $\beta$, $\init$ belongs to it. Altogether, if $\beta$ contains any element, then $\alpha$ must be empty; so no swaps can be produced from $h_{32}$. To conclude, in this case $\textsc{explore}$ violates $I$-completeness.}
