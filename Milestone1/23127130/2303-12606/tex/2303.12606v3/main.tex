...
%\declaretheorem[name=Theorem,numberwithin=theorem]{thm}
\usepackage{wrapfig}
\usepackage{listings}
...
%\definecolor{newVersionColor}{HTML}{29a3a3}
\definecolor{newVersionColor}{HTML}{000000}
%\definecolor{oldVersionColor}{HTML}{dc2361}
%\definecolor{appendixVersionColor}{HTML}{dcb123}
\definecolor{appendixVersionColor}{HTML}{000000}

%\definecolor{noAppendixVersionColor}{HTML}{6423dc}
\definecolor{noAppendixVersionColor}{HTML}{000000}
...
%\newcommand\nver[1]{{\color{newVersionColor}#1}}
\newcommand*\nver{\textcolor{newVersionColor}}
\newcommand\noAppendixver[1]{\iftrue\textcolor{noAppendixVersionColor}{#1}\fi}


\newcommand\oldver[1]{}
%\newcommand\oldver[1]{\iftrue{\textcolor{oldVersionColor}{\sout{#1}}}\fi}
\newcommand\appendixver[1]{\iffalse{\leavevmode\textcolor{appendixVersionColor}{#1}}\fi}
...
\newcommand{\transC}[1]{\mathsf{commTrans}({#1})}
...
\newcommand{\alter}[3]{\textup{\textsf{alter}}_{#1, #2}(#3)}
\newcommand{\canSwap}{\textup{\textsc{canSwap}}}
\newcommand{\isConsistent}[2][\mathcal{M}]{\textup{\textsc{isConsistent}}_{#1}(#2)}
\newcommand{\isMaximallyAdded}[4][I]{\mathsf{readLatest}_{#1}(#2, #3, #4)}
\newcommand{\readsCasuallyLatest}[3][I]{\mathsf{readsCLatest}_{#1}(#2, #3)}
...
%%% The following is specific to PLDI '23 and the paper
%%% 'Dynamic Partial Order Reduction for Checking Correctness against Transaction Isolation Levels'
%%% by Ahmed Bouajjani, Constantin Enea, and Enrique Roman-Calvo.
%%%
\setcopyright{rightsretained}
\acmPrice{}
\acmDOI{10.1145/3591243}
\acmYear{2023}
\copyrightyear{2023}
\acmSubmissionID{pldi23main-p143-p}
\acmJournal{PACMPL}
\acmVolume{7}
\acmNumber{PLDI}
\acmArticle{129}
...
\begin{document}
	...
	%% Title information
	\title[Dynamic Partial Order Reduction for Checking Correctness against Transaction Isolation Levels]{Dynamic Partial Order Reduction for Checking Correctness against Transaction Isolation Levels}       %% [Short Title] is optional;
...
%LIX, CNRS, Ecole Polytechnique
	...
	\affiliation{
		\institution{Université Paris Cité, CNRS, IRIF}
	...
	\affiliation{
		\institution{LIX, Ecole Polytechnique, CNRS and Institut Polytechnique de Paris}
		\country{France}                    %% \country is recommended
	}
	\email{cenea@lix.polytechnique.fr}          %% 

	\author{Enrique Rom\'an-Calvo}
	\affiliation{
		\institution{Université Paris Cité, CNRS, IRIF} 
		\country{France}                    %% \country is recommended
	}
...
%	\ccsdesc[300]{Information systems~Relational database model}
	...
	\keywords{Applications of Storage Systems, Transactional Databases, Weak Isolation Levels, Dynamic Partial-Order Reduction}
	%% Keywords
...
%	\input{implementation}
	...
	\input{data-availability-statement}
...
%	\end{acks}
	...
	\input{appendix/diagrams/rc-ra}


	\newpage
	\input{appendix/diagrams/rules}
	\newpage
	

	\input{appendix/proofs/causal-extensibility}
	\input{appendix/proofs/impossibility}

	\newpage
	\input{appendix/proofs/sound-complete-optimal}

	\newpage
	\input{appendix/experiments/experiments}
	%\input{appendix}
	%
