...
the values of $f_i(k), \ i\in \mathcal V$  at all sampled points in $[-2,4]$ for $k=1000$  and $k=100000$ iterations, respectively. It can be seen that the  estimations of all nodes: $f_i(k), \ i\in \mathcal V$ converge to the unknown true function $f^{*}$ pointwisely almost surely as $k$ increases and the numerical result  is consistent with  Corollary \ref{vnlllleleeemmem}.
...
&&=\left\langle \left(\sum_{i=1}^NH_i\right)a,a\right\rangle _{\X}
 >0.
\ena
\item If there exist $1\leq i_0\neq j_0\leq N$, such that $x_{i_0}\neq x_{j_0}$. It follows from $H_i\ge 0$, $i=1,\cdots,N$ that $\text{diag}\{H_1,\cdots,H_N\}\ge 0$. Noting that the graph is undirected and connected, then there exist nodes $\{ i_{1},i_{2},\ldots, i_{m}\}$ such that
...
&&=\frac{1}{2}\sum_{i=1}^N\sum_{j=1}^Na_{ij}\|x_i-x_j\|^2_{\X}\cr
 && \ge \frac{1}{2}\sum_{l=0}^{m-1} a_{i_{l}i_{l+1}}\|x_{i_{l}}-x_{i_{l+1}}\|_{\X}^2 + \frac{1}{2}a_{i_{m}j_{0}}\|x_{i_{m}}-x_{j_{0}}\|_{\X}^2\geq 0.
\ena
 If  $\frac{1}{2}\sum\limits_{l=0}^{m-1} a_{i_{l}i_{l+1}}\|x_{i_{l}}-x_{i_{l+1}}\|_{\X}^2 + \frac{1}{2}a_{i_{m}j_{0}}\|x_{i_{m}}-x_{j_{0}}\|_{\X}^2=0$, then by  $a_{i_{0}i_{1}}>0,  a_{i_{1}i_{2}}>0, \ldots, a_{i_{m}j_{0}}\\ >0$,  we get $x_{i_0}=x_{i_1}=\ldots=x_{i_{m}}=x_{j_{0}}$, which leads to a contradiction with  $x_{i_0}\neq x_{j_0}$. Thus, we have $\frac{1}{2}\sum\limits_{l=0}^{m-1} a_{i_{l}i_{l+1}}\|x_{i_{l}}-x_{i_{l+1}}\|_{\X}^2 + \frac{1}{2}a_{i_{m}j_{0}}\|x_{i_{m}}-x_{j_{0}}\|_{\X}^2>0$. Then, by (\ref{ofunc1}), we have
