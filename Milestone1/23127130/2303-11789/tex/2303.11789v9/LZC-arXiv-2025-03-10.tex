...
%\vskip 0.2cm
Combining Definition \ref{tuopukongjian} and Corollary 1.4.7, Proposition 1.1.28 and Corollary 1.1.29 in \cite{hy}, we directly obtain the following properties of operator-valued random elements.
...
\indent  (a). If $f_0$ is a random element with values in $(\mathscr L(\mathscr Y,\mathscr Z),\tau_{\text{S}}(\mathscr L(\mathscr Y,\mathscr Z)))$, then the mapping $f_0y:\omega\mapsto f_0(\omega)y$ is a random element with values in $(\mathscr Z,\tau_{\text{N}}(\Z))$ for arbitrary $y\in \Y$.\\
