...
In all our experiments, while our vision encoders differed in size, the text encoder is the same \ie, a pretrained 
\xlmr Base~\cite{conneau2020unsupervised} model with 270M parameters. Our \lit training hyperparameters are shared in 
...
{\bf \noindent \mae Scaling results for \inetOneKShort linear.} We show the performance of \mae on \inetOneKShort linear
probing using \igSizeShort and \inetOneKShort pretraining in \cref{fig:mae_scaling_in1k_lin}. Different from the results in
\cref{fig:mae_scaling}, the performance with \inetOneKShort pretraining starts off significantly better. This is because
with \mae-\inetOneKShort pretraining, the trunk is pretrained on the same dataset we use for linear probing, which isn't the
case for \igSizeShort pretraining, making the comparison unfair. Excitingly, even here we see that \mae on \igSizeShort scales
much better, and based on the trends, would outperform \mae pretraining on \inetOneKShort if scaled out further.


{\bf \noindent Analyzing detection performance.}
We report a TIDE~\cite{tide-eccv2020} analysis of the types of errors done by \vitL models trained with \ce, \mae or \ours.
The results are reported in \cref{tab:detection_tide_all}.
Overall, it shows that \mae, because it is pretrained on lower level representation, is much better at finding and localising objects than \ce.
For the same reasons, \mae is worse at classifying the content of bounding boxes than \ce.
Using \mae as a \pptraining phase to \ce leads to a significant improvement in finding an localising objects,
while still retaining good classification abilities, demonstrating the usefuless of our \pptraining approach.




{\bf \noindent Tables for figures in main paper.} 
Lastly, \cref{tab:mae_scaling_numbers} shares the results showcasing the scalability of \mae (earlier shown in 
\cref{fig:mae_scaling})
in tabular form for readability. Similarly, the results comparing \mae, \ce, and \ours performance when using \igSizeShort 
(\cref{fig:mae_vs_ce_vs_mae_ce_ig}) and \inetFullShort (\cref{fig:mae_vs_ce_vs_mae_ce_in21k}) are tabulated in 
\cref{tab:radar_numbers}.
