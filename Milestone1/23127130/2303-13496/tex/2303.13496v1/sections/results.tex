...
the mean top-1 accuracy over all the splits.

We evaluate two protocols for low-shot finetuning -- linear classifiers and Adapters~\cite{houlsby2019parameter}, both 
of which keep the entire model parameters frozen and introduce few trainable parameters.
In our preliminary experiments, we evaluated multiple Adapters proposed for ViTs -- LoRA~\cite{hu2022lora}, 
AdaptFormer~\cite{chen2022adaptformer}, and VPT~\cite{jia2022visual}. We found that VPT performed the best while being 
robust to the choice of training hyperparameters, and ourperforms linear classifiers for our models.
...
We initialize the text encoder from a relatively small \xlmr Base~\cite{conneau2020unsupervised} model with 270M parameters.
\cref{tab:zero_shot} shows the zero-shot
transfer performance of our models on \inetOneKShort, and \foodShort. Our \vitTwoB attains 80.8\% accuracy on 
...
\jftThreeB provides best \inetOneKShort accuracy. On \food we attain the highest zero-shot transfer accuracy of 96.0\%. 
...
Unlike other tasks like image / video classification, scaling the model size using only \ce pretraining does \emph{not} improve 
detection performance.
However, adding \prept with \mae provides consistent gains and allows \ce to scale with model size.
...
\ours improves over \mae at detecting rare objects, presumably because of the diversity in the \igSizeShort labels,
and improves at detecting frequent classes over \ce due to \mae \pptraining.
