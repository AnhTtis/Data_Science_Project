...
\begin{enumerate}[label={\bfseries (KS\arabic*):}]
    \item $\exists \, a,C_1>0$ s.t. $\forall \, \epsilon>0$, $\mu(B_\varepsilon(S)) \leq C_1 \varepsilon^a$.
    \item $\exists \, b,C_2>0$ s.t. $\forall \, x \in X \setminus S$, $||D^2_xf|| \leq C_2 \,d(x,S)^{-b}$.
...
\begin{enumerate}
    ...
    \item $\| D_xf(v) \| \geq \Lambda \| v \|$, $\forall v \in C_x^u$ and $\| D_xf^{-1}(v) \| \geq \Lambda \| v \|$, $\forall v \in C_x^s$.
