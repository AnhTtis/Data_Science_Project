...
\vspace{-0.05in}
In this section, we design a distributed, uncoordinated algorithm that leads to mixed-strategies NE for the ESG when there does not exist a dominant-strategy for each consumer, i.e., for the consumers in the set $\Sigma_{2,2}$ of Case 2. Note that based on the information they have (Section \ref{sec:game_def}), the consumers can know whether they have a dominant strategy and in such a case they can directly compute it. The proposed distributed iterative algorithm, Algorithm \ref{algorithm}, is based on a best response scheme and requires minimum information exchange among the consumers. In particular, there is no central coordinator or direct communication channels between consumer pairs since the information that is required to be exchanged among consumers is just broadcasted from the consumer that has performed the most recent computation to the remaining ones. %The steps of the proposed algorithm are summarized in Algorithm \ref{algorithm} below.
...
%%
As we further aim to limit information exchange, the chosen strategies are not communicated. Instead the consumers update and broadcast three common variables, which encode this information:

\noindent $1)$ the variable $X_\Sigma$ that is equal to the total current average energy demand for $RES$;\\
$2)$ the vector $EQT$ that indicates which consumer types have played in the passed iterations of an algorithm's step ($EQT(\ell)=1$ if type $\ell \in \Theta$ has played), and is re-initialized to $\bm{0_M}$ at the beginning of each outer loop;\\
...
\begin{algorithm}[t] 
    ...
    $(p_{RES,\ell}, p_{grid,\ell}) \gets (0,1), ~\forall \ell  \in \Theta$ \;
    $X_\Sigma \gets N \sum_{ \ell \in \Sigma_1  } r_{\ell} E_{\ell}$, ~~$\Sigma \gets \Sigma_{2,2}$ \;
   ...
   % $X_\Sigma \gets \sum_{ \vartheta_j \in \Sigma_1 \setminus \{ \vartheta_i \} } r_{\vartheta_j} (N-1)E_{\vartheta_j}$, $\Sigma \gets$ all consumer with types in $\Sigma_2$ \label{line:sigma3}\; 
    ...
    $EQP \gets$ vector of size $M$ with zero entries for $\ell \in \Sigma$ and unary entries for $\ell \in \Sigma_{1}$\;
    \For{$step \gets 1$ \KwTo $N_{step}$ \label{line:external_round}}{
        $EQT \gets \mathbf{0}_M$\;
        $EQP_{old} \gets EQP$\;
        \For{each consumer $i \in \Sigma$}{
            \If{$EQT(\vartheta_i) = 1$}{
            Consumer $i$ retrieves  $p_{RES,\vartheta_i}^{}$ from $EQP(\vartheta_i)$\; \label{line:retrievestrategy}}
            \Else{
                %\uIf{$x=PA$}{
                {\small$rse_{\vartheta_i}^{PA}(p_{RES,\vartheta_i}^{})\leftarrow \frac{E_{\vartheta_i}\cdot \mathcal{ER}}{X_\Sigma +(N-1) r_{\vartheta_i} p^{}_{RES, \vartheta_i} E_{\vartheta_i} + E_{\vartheta_i}}$}\label{line:PAconstr} \;
               %\;
               %}
                %\ElseIf{$x=ES$}{ $rse_{\vartheta_i}^{ES}(p_{RES,\vartheta_i}^{PA})=\frac{\mathcal{ER}}{X_\Sigma +(N-1) r_{\vartheta_i} p^{PA}_{RES, \vartheta_i} + 1}$\label{line:ESconstr} \;}
               {\small$p_{RES,\vartheta_i}^{*}\gets \,\underset{p_{RES,\vartheta_i}} {\mathrm{arg \min}}\, cost(p_{RES,\vartheta_i}^{})$}\label{line:bestresponse}, from \eqref{eq:personalcost}\;
                {\small$p_{RES,\vartheta_i}^{cap}\gets f_{cap}(p_{RES,\vartheta_i}^{*})$}, from \eqref{eq:cap} \label{line:cap}\;
                {\small$EQT(\vartheta_i) \gets 1$\label{line:eqtgets1}}, \; {\small$EQP(\vartheta_i) \gets EQP(\vartheta_i)+p_{RES,\vartheta_i}^{cap}$}\label{line:probincrease}\;
                 \label{line:pacomputed}
                %\uIf{$x=PA$}{
                    $X_\Sigma \gets \,X_\Sigma+ (N-1)~r_{\vartheta_i}~p_{RES,\vartheta_i}^{cap}~E_{\vartheta_i}$
                    % ;}
                %\ElseIf{$x=ES$}{
                   % $X_\Sigma \gets \,+ (N-1)r_{\vartheta_i}p_{RES,\vartheta_i}^{x,cap}$\;}
                }
