...
The goal of training is to use the dataset~$\dataset$ to find weights~$\ws$ such that~$f$ approximates~$\hypothesis$ well over input regions with high probability density.
 ...
 Gradients of $\lossfn$ with respect to the network's weights can then be used to update the weights during training. %Common examples of loss functions are \emph{mean of squares error} and \emph{cross entropy}~\cite{Goodfellow-et-al-2016}.   
...
\end{equation}
\end{definition}
\noindent The problem of verifying the robustness of neural networks has received significant attention from the verification community~\cite{CKDKKAE22,SinghGPV19,WangZXLJHK21}, and it is known to be difficult both theoretically~\cite{KaBaDiJuKo17Reluplex} and  in practice~\cite{BaLiJo21}. 
...
the former fell short of introducing quantifiers as part of the language (and thus modelled $\epsilon$-robustness semi-formally in all experiments), 
and the latter only covered propositional fragment (which is not sufficient to reasoning about robustness).

Solutions were offered from the perspective of formal logic. In ~\cite{KriekenAH20,VANKRIEKEN2022103602} it was shown that one can use various propositional fuzzy logics to create loss functions. However, these fuzzy DLs did not stretch to cover the features of the DLs that came from machine learning community, and in particular could not stretch to express the above $\epsilon-$robustness property, which needs not just quantification over infinite domains (quantifiers for finite domains were given in~\cite{KriekenAH20}), but also a formalism to express properties concerning vectors and functions over vectors.% and probability distributions.
...
\emph{In many of the existing DL approaches  syntax, semantics and pragmatics are not well-separated, which obscures their formal analysis.} We have already given an example of DL2 treating quantifiers empirically outside of the language.
But the problem runs deeper than modelling quantifiers. To illustrate, let us take a fragment of syntax on which all DLs are supposed agree. %\paragraph{Different DLs}
It will give us a toy propositional language $$a := p\ |\ a \wedge a$$
 
They assume that each propositional variable $p$ is interpreted in a domain $\domainSymbol \subseteq \Real$. The domains vary vastly across the DLs (from fuzzy set $[0,1]$ to $(\infty, \infty)$ in STL) and the choice of a domain becomes an important factor in both the syntax and the semantics.  Thus DL2's domain $[0, \infty]$ severely restricts the translation of negation compared to other DLs. % (it is defined only on atomic propositions).  
Conjunction is interpreted by $+$ in DL2~\cite{fischer2019dl}, by $min$, $\times$ and other more complex operations in different fuzzy DLs \citet{VANKRIEKEN2022103602}.  
...
As consequence of the above two problems, the third problem is to \emph{create a general syntax and semantics that can express different DLs and are modular on the choice of a concrete DL, giving flexibility to choose one more suited for a concrete task, or to use a combination of the ``best" features from several DLs}.
...
it is a typed first-order language with negation and universal and existential quantification that can express properties of functions and vectors.
 Moreover, the language has an implicit formal treatment of neural networks via a special kind of context -- a solution that we found necessary in achieving a sufficient level of generality in the semantics.  
 ...
 On the semantic side (Section~\ref{sec:semantics}), interpretation is defined to be parametric on the choice of the interpretation domain $\domainSymbol$ or a particular choice of logical connectives. 
  This parametric nature of interpretation simplifies both the theoretical study and implementations that compare different DLs. Moreover, the semantics formally introduces the notion of a probability distribution that corresponds to the data. %(being approximated via training). 
  ...
  on improving $\epsilon$-ball robustness.
...
Each element of the vector $x_i$ is assumed to be drawn from a random variable $X_i$ over domain $D_i$. The \emph{joint PDF} is a function
...
\end{equation*}
%From here on, 
For brevity we will write $\distr_\X$ instead of $\distr_{X_1, \ldots, X_n}$ and $\int_\D ... d\x$ rather than the full integral above.
%
% Secondly, 
...
%\end{equation}
Throughout the paper we assume that random variables are independent. 
...
\qquad \lamText\ (v : \VecType{\exsize})\ .
\ \lamText\ (u : \VecType{\exsize})\ .
\ \lamText\ (a : \RealType)\ . \\
	 
\quad\qquad \forall\ (i : \FinType{\exsize} )\ .
\ \letText\ (d : \RealType) = v\ !\ i - u\ !\ i \ \textrm{in}
...
\begin{figure}[t]
	...
	\semSpace
	\infer[(bool)]{\hcTypeRel{\elBool}{\BoolType}}{\elBool \in \Bool}
	\\
	\\
	\infer[(app)]{\hcTypeRel{\App{e_1}{e_2}}{\tau_2}}{\hcTypeRel{e_1}{\FunType{\tau_1}{\tau_2}} & \hcTypeRel{e_2}{\tau_1}}
	\\
	\\
	\infer[(lam)]{\hcTypeRel{\Lam{\id}{\tau_1}{e}}{\FunType{\tau_1}{\tau_2}}}{\hTypeRel{\hcNetCtx,\cons{(\id,\tau_1)}{\hcVarCtx}}{e}{\tau_2}}
	\semSpace
	\infer[(let)]{\hcTypeRel{\Let{\id}{\tau_1}{e_1}{e_2}}{\tau_2}}{\hcTypeRel{e_1}{\tau_1}  & \hTypeRel{\hcNetCtx,\cons{(\id,\tau_1)}{\hcVarCtx}}{e_2}{\tau_2}}
	\\
	\\
	...
	\\
	\\
	\infer[(forall)]{\hcTypeRel{\Forall{\id}{\tau}{e}}{\BoolType}}{\hTypeRel{\hcNetCtx,\cons{(v,\tau)}{\hcVarCtx}}{e}{\BoolType} & \tau \neq \FunType{\tau_1}{\tau_2}}
	\quad
	\infer[(exists)]{\hcTypeRel{\Exists{\id}{\tau}{e}}{\BoolType}}{\hTypeRel{\hcNetCtx,\cons{(v,\tau)}{\hcVarCtx}}{e}{\BoolType} & \tau \neq \FunType{\tau_1}{\tau_2}}
	\end{gather*}
	\setlength{\belowcaptionskip}{-15pt} 
...
We now define a typing relation for well-typed expressions in LDL. A \emph{bound variable context}, $\Delta$, is a list of pairs $x : \tau$ of variables and their types. 
A \emph{network context}, $\Xi$, is a list of pairs $f: \VecType{n} \rightarrow \VecType{m}$ of neural network variables and their associated types. 
%In later sections  we will call $(\Xi, \Delta)$ the \emph{syntactic context} of LDL.
 The set of \emph{well-typed expressions} is the collection of all $e$ with $\haTypeRel$ for some contexts $\Xi$ and $\Delta$ and type $\tau \in \langle type \rangle$, where $\haTypeRel$ is defined inductively in Figure~\ref{fig:types}. 
% When a well-formed expression $e$ has type $\BoolType$, we will call it \emph{a formula}. 

When $e' = \Forall{\id}{\tau}{e}$ is well typed and $\tau = \RealType$ or $\tau = \VecType{n}$, we will say that the quantifier $\forall$ is \emph{infinite} in $e'$, otherwise the quantifier is \emph{finite}; similarly for $\exists$. 
...
\end{equation*}}}
\noindent Note that the type \textit{Bool} is absent as it is dependent on the choice of DL, given in Figure~\ref{fig:expr}. % for the interpretation for each DL. 

\subsection{Semantics of LDL Expressions}
\label{subsec:expressions}

The next step is to interpret well-typed expressions, which  %, see Figure~\ref{fig:generic-semantics}. 
...
\begin{itemize}
	\item The bound context $\hVarCtx$ is a partial function that assigns each bound variable currently in scope $x : \tau$  a semantic value in~$\ttype{\tau}$. We will use the notation $\hVarCtx[x\rightarrow y]$ to represent updating $\hVarCtx$ with a new mapping from $x$ to $y$.
	\item The network context $\hNetCtx$ is a function that maps each network variable $f: \VecType{m} \rightarrow \VecType{n}$ to $\mathtt{f} : \Real^m \rightarrow \Real^n$, the actual (external) implementation of the neural network.
	
	\item The quantifier context $\hRandCtx$ is a function from each syntactic variable $x: \typeClass$ bound by an infinite quantifier to $(X, \gamma)$, where $X$  is a random variable from which values for $x$ are sampled from (by discussion of Section~\ref{sec:datasets-and-classifiers} this also extends to cover random vector variables) and $\gamma \in \Real$  is a precision parameter.
	%describes the level of precision used when searching for extrema of a function over $X$.

\end{itemize}
\begin{example}[Semantic context]
Consider the LDL expression in Example~\ref{ex:wte}: $\robustexpr$ contains a single network variable~$f$, and therefore the network context will be of the form: $\hNetCtx = [ f \mapsto (\mathtt{f}: \Real^{784} \rightarrow \Real^{10}) ]$ where $\mathtt{f}$ is the actual neural network (e.g.~think the Tensorflow/PyTorch object). The expression contains a single infinitely quantified variable $x$, and therefore the quantifier context $\hRandCtx$ will be of the form $[x \mapsto (\X, 0.1)]$, where $\X$ is random vector  variable of size $784$, and represents the underlying distribution that the input images are being drawn from. At the top-level of the expression, no bound variables in scope and therefore $\hVarCtx$ is empty. However, when interpreting the subexpression $bounded\ x\ \hat{x}\ \epsilon$,  the $\hVarCtx$ will be of the form:
%\begin{equation*}
$[bounded \mapsto v_1, \: \epsilon \mapsto v_2,\:  \delta \mapsto v_3, \: \hat{x} \mapsto v_4, \: x \mapsto v_5]$
%\end{equation*} 
where $v_1$ is the interpretation of the let bound expression \emph{bounded}, $v_2, v_3 \in \Real$ are the concrete values of $\epsilon$ and $\delta$, and $v_4, v_5 \in \Real^{784}$ are the value passed in for $\hat{x}$ and the current value of the quantified variable $x$ respectively. 
\end{example}
...
\begin{figure}[t]
	...
	$ \tempty{==}_{L} $ &
	$ \lam{\val_1, \val_2}{- | \val_1 - \val_2 | } $ &
	\coloured{$\lambda \val_1, \val_2. 1 -  |\frac{\val_1-\val_2}{\val_1 + \val_2}|$} &
	\coloured{$\lam{\val_1, \val_2}{-|\val_1 - \val_2|}$}
	\\ \hline
	{$ \tempty{\leq}_{L} $} &
	$\lam{\val_1, \val_2}{- \max(\val_1 - \val_2, 0)}$ &
	\coloured{$\lam{\val_1, \val_2}{1-\max(\frac{\val_1 -\val_2}{\val_1 + \val_2}, 0)}$} &
...
\citet{fischer2019dl} were the first to interpret universally quantified formulae via expectation maximisation methods, in which case the optimised parameters were neural networks weights, and the quantified properties concerned $\epsilon$-ball robustness of the given neural network. %The rest of DL literature resorts to propositional fragments of logics, with the exception of~\cite{}.\nnote{Not sure about that, the fuzzy and DL both handle quantifiers in a way, just STL doesn't}
...
%and will not be restricted to properties that concern neural networks and their weights.
 %unlike DL2, 
and does not make any restricting assumptions about the universal properties that the language can express.
For example, we defined LDL to admit expressions that may or may not refer to neural networks (or weights), and may express properties more general than $\epsilon$-robustness.  
...
In particular, for a quantified formula $\forall x. P(x)$, where $P$ is an $\epsilon$-robustness property,  ~\citet{fischer2019dl} take $|\x - \hat{\x}| \leq \epsilon$  and empirically compute the ``worst perturbation" in the $\epsilon$-interval from $\hat{x}$ using a ``PGD adversarial attack" projected within that interval. 
...
An alternative would be to use just the global minimum (or maximum) of a function directly. 


\begin{example}[Semantics of Quantified Expressions]
	We calculate a loss function for the robustness property in Example~\ref{ex:wte} with respect to DL2 logic in the context $(\semCtx)$ where $\hNetCtx = [ f \mapsto (\mathtt{f}: \Real^{784} \rightarrow \Real^{10}) ]$, $\hRandCtx = [x \mapsto (\X, 0.1)]$ and $\hVarCtx$ is empty. Since DL2 has no separate definition of implication we translate the implication in robustness property in a standard manner using negation and disjunction and pushing the negation inwards  to the level of comparisons: % as previously defined.
	\begin{align*}
	\texpr{e^*}{\hNetCtx}{\hRandCtx}{\hVarCtx}{DL2} & = \lambda\ \epsilon.\ \lambda\ \delta.\ \lambda\ \hat{\x}\ .\ \mathbb{E}_{min}^{0.1}[(\lambda \x. (bounded\ \x\ \hat{x}\ \epsilon) (bounded\  (f(\x))\ (f(\hat{\x}))\ \delta))(\X)] \\
	& \text{where} \quad bounded = \lambda \textbf{v} . \lambda \textbf{u} . \lambda a . \sum_{i=0}^{10} (a (v_i - u_i)) + ((v_i - u_i) a)
	\end{align*}
%\matthew{Natalia, this relies on having a DL2 translation for implication in Figure~\ref{fig:expr}. Can you add it and use it to complete this example? I also don't think the comment below about negation being pushed inward applies here?}
	%Note that for the translation to work the negation had to be pushed inwards to the level of comparisons ($\leq$).
\end{example}

The final problem to consider is whether the interpretation of infinite quantifiers via global minima and maxima commutes with the interpretation for $\land$ and $\lor$: in general it does not. Section~\ref{sec:properties} considers this problem in detail.
...
%\matthew{Okay so I'm a bit lost here. There are several things we might be trying to prove here:
%\begin{enumerate}
%\item For each DL we could prove soundness/completeness for the the minimal subset of LDL syntax that the original DL defines.

%\item Or for each DL we could prove soundness/completeness for the maximal subset of LDL syntax to which the DL can be extended to.
%\end{enumerate}
%We appear to be doing a bit of both. The proofs include cases for quantifiers (in LDL, but not in original DLs), but doesn't consider much of the LDL syntax such as vectors, lambdas etc. We need to be much more explicit about what exactly our goal is here.
%}


In all existing work, soundness and completeness of DLs was given for propositional fragments of those logics. The standard Boolean semantics of propositional classical logic is taken as a decidable procedure for a set of all true formulae. The main soundness result establishes that if $\texpr{e}{\hNetCtx}{\hRandCtx}{\hVarCtx}{L} \in \texpr{\top}{\hNetCtx}{\hRandCtx}{\hVarCtx}{L}$ then $e$ is \emph{true} in the propositional logic.
...
However, as LDL is a richer language, we may no longer rely on this method.  
For a typed FOL, one could define a Kripke-style semantics~\cite{sorensen2006lectures,LIPTON20181} for characterising the set of all true formulae, or take provable FOL formulae to characterise the set of true FOL formulae. We take the latter approach. %We leave this approach for future work. 
%
...
%\matthew{What is the relationship between adequacy and soundness? And how do we recover soundness from adequacy? I think this section would be improved by ``Definition" environments for Soundness, Completeness, Adequacy etc. in the same way we have them for Shadow Lifting etc. in the next subsection.}

%\matthew{I think the meaning of the turnstile relation needs to be explicitly mentioned in the text. While I'm not deeply familiar with sequent calculi, I think I know what $\SequentCLJ(){e}$  means, but I'm more unsure about the meaning of $\SequentCLJ(e){}$. }
%\matthew{Also if I understand correctly, we're defining a variant of \LJ\ over the syntax of LDL? In which case, don't the rules of Figure~\ref{fig:rules-CLJ} need to be defined with respect to the LDL syntax. For example, in the exists and forall rules, we need to have types present.
%\\
%There's a subtly here unfortunately. In particular the syntactic operators aren't really binary, they're nullary. So this definition of \LJ{} doesn't align with terms we have in our language. For example it's perfectly possible to have partially terms (e.g. $\wedge\ x_1$ is a well-defined term of type $\BoolType{} \rightarrow \BoolType{}$). This has serious implications for the proof strategy.}

\subsubsection{Type Soundness of Interpretation}


We start with type soundness of LDL.
...
Given that: \begin{inparaenum}
\item For all $f: \VecType{m} \to \VecType{n} \in \Xi$, $N[f] \in (\Real^n)^{\Real^m}$, and
\item For all $x:\tau \in \Delta$, $\Gamma[x] \in \ttype{\tau}$, then:
\end{inparaenum}
	If $\Xi, \Delta \Vdash  e: \tau$ then $\tempty{e}^{\hNetCtx,\hRandCtx,\hVarCtx}_L \in \ttype{\tau}$.
\end{theorem}


%To denote that an expression $e$ is in $\beta-$normal form, we will write  $e^{\beta}$.
In the next section, we will use the fact that all expressions of type $\RealType$ can be interpreted as real numbers. %, with that in mind,
...
The following Lemma is obtained as a corollary of Theorem~\ref{th:type-sound}:  %is proven by induction on $\Xi, \Delta \Vvdash e$:
% (note the empty $\Delta$):

\begin{lemma}[Arithmetic evaluation of real expressions]\label{lem:expr-real}
	If $\Xi, \Delta \Vdash e: \RealType$ then $\tempty{e}^{\hNetCtx,\hRandCtx,\hVarCtx}_L \in \Real$.
\end{lemma}
...
We will assume that for each comparison relation $\bowtie$ in the language, we have a corresponding oracle $\bowtie^*$ that decides, for any pair  $r_1, r_2$ of real numbers whether $r_1 \bowtie^* r_2$ holds. Formally, $ r_1 \bowtie^* r_2$ returns $\top$ if the relation holds, and $\bot$ otherwise. 

We define the set of \emph{FOL formulae} in a standard way (see Figure~\ref{fig:terms} in Appendix). %, intuitively they are well-typed expressions of type $\BoolType$.
% so therefore include logical connectives, comparisons, quantifiers and applications. 
...
\begin{figure}[bt]
	...
	\caption{\emph{\footnotesize{The rules for \LJ{}, standard structural rules are assumed. In the rules we use $w$ to denote any closed expression that is well-typed with empty context $\Delta$, that is $\cTypeRel$.}}}
	\label{fig:rules-CLJ}
	\vspace*{-1.5em}
\end{figure}


\subsubsection{Fuzzy DLs are sound but incomplete}
\label{subsec:soundness}

We start with showing that fuzzy DL comparison operators are valid; this result uses Lemma~\ref{lem:expr-real}. 

\begin{lemma}[Soundness of FDL comparisons]\label{lem:eval} If  $\Xi, \Delta \Vdash e_1 \bowtie e_2$, then the following holds:

If $\tempty{e_1 \bowtie e_2}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} = \tempty{\top}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL}$ then $\tempty{e_1^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL}  = \top$. 

If $\tempty{e_1 \bowtie e_2}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} = \tempty{\bot}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL}$ then $\tempty{e_1^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL}  = \bot$. 
\end{lemma} 

\noindent The auxiliary lemma is proven by case analysis on $e$, mutual induction, and relies on Lemma~\ref{lem:eval}:
...
\end{enumerate}
\end{lemma}
Soundness is then obtained as a corollary:
%\nnote{cleaning in progress}
\begin{theorem}[Soundness of G\"{o}del DL]\label{th:s}
	Given a formula $e$, for any contexts $N, \Gamma, Q$ if $\tGodel{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = \tGodel{\top}^{\hNetCtx,\hRandCtx, \hVarCtx}$ then $\SequentCLJ(){e}$.
\end{theorem}
...
\noindent All other fuzzy DLs are also sound with respect to \LJ, with proofs following the same scheme. Of notable exception is the Łukasiewicz DL, for which we cannot reconstruct the proof unless we remove either negation or implication from the syntax. 
Intuitively, this is because Łukasiewicz implication is not strong enough: e.g. $ \tempty{e_1 \impl e_2} $ evaluates to $1$ as long as $\tempty{e_1} \leq \tempty{e_2}$. Proper study of this phenomenon is left for future work.  
%, cf. 
...
A similar counter-example can be replicated in STL. %cannot be used to obtain the result. Firstly, implication is undefined in the language, and secondly, 
 

\subsubsection{DL2 is sound}\label{subsec:soundness2}
...
Firstly, a variant of Lemma~\ref{lem:eval} holds for DL2. 
 Next, taking DL2 with just connectives $\land, \lor$ and quantifiers, we obtain its soundness: 
...
follows the structure of similar proofs for FDLs. This is a welcome simplification, and will be useful in future computer formalisations of these logics. Seeing the result is given for an incomplete set of connectives, we omit discussion of completeness for DL2.  
...
Also note that its connectives and quantifiers lack properties that are provable in \LJ: e.g.~associativity of conjunction, or commutativity of a universal quantifier with respect to conjunction. 
...
Soundness for STL would be stated as: \emph{``Given a formula $e$, 
	if $\tSTL{e}^{\hNetCtx,\hRandCtx, \hVarCtx} \in \tSTL{\top}^{\hNetCtx,\hRandCtx, \hVarCtx}$  then $\SequentCLJ(){e}$."}  \citet{varnai} do 
...
%Other types of smoothness are known in the literature~\cite{LeeRY23}. 

%\begin{definition}[Min-max boundedness]
%	The  $\tempty{e}$ is \textit{min-max bounded} (by $a$ and $b$) if
%	\begin{equation*}
%	a \leq \tempty{e} \leq b
%	\end{equation*}
%\end{definition}
...
%\begin{definition}[Min-max boundedness]
%	The operator $\tempty{\conjM}$ is \textit{min-max bounded} if
%	\begin{equation*}
%	\min(\tempty{A_1}, ..., \tempty{A_M}) \leq \tempty{\conjM(A_1, ..., A_M)} \leq \max(\tempty{A_1}, ..., \tempty{A_M})
%	\end{equation*}
%\end{definition}

%The min-max boundedness ensures closure of the translation with respect to the domain.
...
Table \ref{tab:properties} summarises how different DLs compare and contains results already established in literature (as cited) as well as provides new results. 
All proofs for new results can be found in Appendix~\ref{app:properties}. The table inspires the following conclusions. 
...
%\begin{itemize}
\begin{inparaenum}[i)]
\item Starting with geometric properties, smoothness is traditionally important for differentiation~\cite{LeeRY23}, and connectives for DL2 and Product DL satisfy the property. However, in the FOL extension only STL remains smooth, see also discussion of \S~\ref{subsec:STL-S}.
...
\item Since quantifiers are defined as global minima and maxima, only connectives of G\"{o}del DL commute with respect to them.
This is a highly desirable property, that has not been previously discussed in the literature. It impacts completeness of DLs (cf. \S~\ref{subsec:soundness}).
...
Taking the specification of Example~\ref{ex:wte}, it is compiled by Vehicle to a loss function in Python where it can be used to train a chosen neural network. The loss function resulting from the LDL semantics is typically used in combination with a choice of standard loss function such as cross-entropy.
...
\begin{figure}
	...
	\begin{subfigure}{.4\textwidth}
		\includegraphics[width=1\linewidth]{all-DLs-accuracy.png}
		%\caption{Accuracy of the network trained with different custom losses with varying parameters $\alpha, \beta \in \Real$}
		%\label{fig:acc}
	\end{subfigure}
		\begin{subfigure}{.4\textwidth}
		\includegraphics[width=1\linewidth]{all-DLs-constraint-satisfaction1.png}
...
\textbf{Adversarial training.} Starting with the seminal paper of ~\cite{szegedy2013intriguing}, thousands of citations in machine learning literature have been devoted to adversarial attacks and robust training of neural networks. Majority of those papers does not use a formal logical language for attack or loss function generation. LDL opens new avenues for this community, as allows one to formulate an arbitrary property of interest, not necessarily robustness, see e.g.~\cite{CKDKKAE22,fomlas22}.%\textit{NOTE: I don't know which one you mean here, Luca's talk at FOMLAS was on Vehicle as language? But it doesn't have anything about non-robustness properties as far as I know? I cited it here in case you meant it though it's worth noting it wasn't published anywhere, it's just as a talk.}
...
We intend to use LDL to further study the questions of a suitable semantics for DL,
 both proof-theoretic and denotational. For proof-theoretic semantics, we need to find new DLs with tighter correspondence to \LJ; and moreover new calculi can be developed on the basis of \LJ ~to suit the purpose. Since loss functions are used in computation, we conjecture that constructive logics will be useful in this domain. For denotational semantics, we intend to explore Kripke frame semantics.  
Future work could also include finding the best combination of logical and geometric properties in a DL and formulating new DLs that strike the right balance. Thorough evaluation of performance of all the DLs is also left to future work: it includes formulating heuristics for finding local/global maxima for quantifier interpretation, and evaluation of LDL effectiveness within the continuous verification cycle. Finally,
...
We have defined both $\leq$ and $==$ in Section~\ref{subsec:expressions}. Normally other comparisons could be expressed in terms of $\leq$, $==$ and logical connectives. However as not all DLs presented have negation this approach would be flawed and therefore we define all the remaining comparisons in Figure~\ref{fig:more-comparisons}.
...
\begin{figure}
	...
	\begin{tabular}{|c|c|c|c|}
		...
		$\tempty{\neq}_{L}$ &
		$ \lam{\val_1,\val2}{-\xi[\val_1=\val_2]}  $&
		\coloured{$ \lam{\val_1,\val2}{1-[\val_1=\val_2]}  $}&
		\coloured{$ \lam{\val_1,\val2}{-\xi[\val_1=\val_2]}  $}\\
		\hline
		$ \tempty{<}_{L} $ &
		...
		\hline
	\end{tabular}
\caption{Semantics of comparisons between terms in LDL that are dependant on the choice of DL. Interpretations in \coloured{colour} have been defined for the purposes of LDL and were not present in the original semantics.  $\xi > 0$ is a constant and $[\cdot]$ an indicator function.}
...
DL2 does not have an interpretation for negation - instead negation of a term is pushed inwards to the level of comparisons between terms. While this cannot be expressed as a single lambda function LDL does provide syntax rich enough to express the method of pushing negation inwards used in DL2, as defined in Figure~\ref{eq:lowerNot}.
...
\subsection{Well Formed FOL Formulae}
\label{app:wff}

%Figure~\ref{fig:real-expr} defines real expressions.



%We mentioned before that a well formed formula is an expression of type Bool. For clarity we introduce in 
...
\begin{figure}[!]
	...
	\end{spreadlines}
	\vspace*{-1em}
	\caption{\footnotesize{(Well-formed) FOL Formulae in LDL.}}% \textbf{Bottom:} Well-formed Formulae.}}}
	\label{fig:terms}
\end{figure}
...
\LJ{} has four structural rules: weakening, exchange and contraction on the left, and weakening on the right.
%We omit the latter, as we extend the {\footnotesize{$\LJAxiom$}} rule in a way that renders right weakening opaque. 
We need to add the structural rules {\footnotesize{\LJWeakLT{}}}, {\footnotesize{\LJWeakRT{}}}, {\footnotesize{\LJExchLT{}}}
and {\footnotesize{\LJCtrLT{}}} :
...
%
%We prove by induction on the typing judgment the Theorem~\ref{th:type-sound}.
%
%\begin{proof}
%\textbf{Base Case 1.} 
%Suppose $e$ is a networkVar. Then we have $e: \VecType{m} \to \VecType{n} \in \Xi$. But then we have $N[e] \in (\Real^n)^{\Real^m}$ by assumption.
%
%\textbf{Base Case 2.}
%Suppose $e$ is  
%\end{proof}
...
We start with a lemma needed for base case of adequacy proofs for all FDLs.
 In the proofs, we reduce $\bowtie$ to $\{== , \leq ,\neq\}$, other comparison operators can be obtained analogously.
...
\begin{proof}
The proof of Lemma~\ref{lem:expr-real} proceeds by case-reasoning on real expressions as defined in Figure~\ref{fig:real-expr}.

\textbf{Base Case 1.}
Suppose we have $e = r :\RealType$ and $\Xi, \Delta \Vvdash e$. Then $\tempty{e}_L^{\semCtx} = r \in \Real$ by definition of the interpretation function.
%an expression of type $\Real$.

\textbf{Base Case 2.}
If $e$ is of the form $ e_1 \> e_2  $ 
%by the fact that the context $\Delta$ is empty, 
we know that both 
$\Xi, \Delta  \Vdash e_1: \tau \to \RealType  $ and $\Xi, \Delta \Vdash e_2: \tau$. But then, we have $\Xi, \Delta \Vdash e:  \RealType$ by definition of $\Vdash$. 
But then, by definition of the interpretation function, we have 
$\tempty{e }_L^{\semCtx} \in \Real$. 

\textbf{Base Case 3.}
If we have $e = v ! i$, then we know  that both 
$\Xi, \Delta \Vdash v:\VecType \RealType $ and $\Xi, \Delta \Vdash i: \FinType$. 
 But then, we have $\Xi, \Delta \Vdash e:  \RealType$ by definition of $\Vdash$. 
%where $v$ has type $\VecType \RealType$ and $i$ has type $\FinType$. 
But then, by definition of the interpretation function, $\tempty{e}_L^{\semCtx} \in \Real$.

%then we know $e$ is a value of type $\RealType$ for which we have $\tempty{e}_L \in \Real$ by base case.

\textbf{Inductive Case 1.}
If $e = e_1 + e_2$ or $e = e_1 \times e_2$ then 
we have $\Xi, \Delta \Vvdash e_1$ and $\Xi, \Delta \Vvdash e_2$.
By the inductive assumption, we have $\tempty{e_1}_L^{\semCtx} \in \Real$ and $\tempty{e_2}_L^{\semCtx} \in \Real$. But then, by definition of the interpretation function,
%by definition, considering we assume all arithmetic operations are applied, we have $e : \RealType$ and therefore 
$\tempty{e}_L^{\semCtx} \in \Real$.

\textbf{Inductive Case 2.}
If we have $e$ of the form $\text{let }x = e_1 \text{ in } e_2$ then we know that $\Xi,\Delta \Vdash e_1:\tau_1$ and $ \Xi, (x, \tau_1) ::\Delta \Vvdash e_2 : \RealType$. 
By definition of the interpretation function, we know that $\tempty{\text{let }x = e_1 \text{ in } e_2}_L^{\semCtx} = \tempty{e_2}_L^{N,Q,\Gamma[x \to \tempty{e_1}_L^{\semCtx} ]}$. By the induction hypothesis, we know that
 $\tempty{e_2}_L^{\semCtx'} \in \Real$, where $\Gamma'$ is $\Gamma$ that is extended with interpretation for $x$. And in particular, it can be $x \mapsto \tempty{e_1}_L^{\semCtx}$ .
%But then, we have 
%$\Xi \Vdash e:  \RealType$ by definition of $\Vdash$. 
%But then, by definition of the interpretation function, we have 
But then, $\tempty{e}_L^{\semCtx} \in \Real$. 

%Then $e$ is a value of type $\RealType$ for which we have $\tempty{e}_L \in \Real$ by base case.

%Lastly we have $\text{let } e:\RealType = e_1 \text{ in } e_2$. Then we have 
\end{proof}

%\begin{lemma}%[Soundness of intervals and arithmetic operations in FDL]\label{lem:eval} 
%For all well-typed expressions $e_1 \bowtie e_2$, the following holds:

%If $\tempty{e_1 \bowtie e_2}_{FDL}^{\semCtx} = 1$ then $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \top$. 

%If $\tempty{e_1 \bowtie e_2}^{\semCtx}_{FDL} = 0$ then $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$. 
%\end{lemma}

%Before we prove this lemma another one is needed together with a definition of well-formed expression of type Real that can be found in Figure~\ref{fig:real-expr} for clarity.
\end{comment}


We now prove Lemma~\ref{lem:eval}.
...
If $\bowtie$ is $==$, then  $\tempty{e_1 == e_2}^{\semCtx}_{FDL} = 1$ means $\tempty{e_1}^{\semCtx}_{FDL} - \tempty{e_2}^{\semCtx}_{FDL} = 0$,
...
%\matthew{$==^*$ and $==^*$ aren't defined.}
	...
	If $\bowtie$ is $\leq$, then  $\tempty{e_1 \bowtie e_2}^{\semCtx}_{FDL} = 1$ means $\tempty{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} - \tempty{e_2}^{\semCtx}_{FDL} \leq 0$, that is, $\tempty{e_1}^{\semCtx}_{FDL} \leq \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \leq^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \top$. Similarly, $\tempty{e_1 \leq e_2}^{\semCtx}_{FDL} = 0$ means $\tempty{e_1}^{\semCtx}_{FDL} - \tempty{e_2}^{\semCtx}_{FDL} > 0$, that is, $\tempty{e_1}^{\semCtx}_{FDL} > \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \leq^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$. 
	
	If $\bowtie$ is $\neq$, then  $\tempty{e_1 \bowtie e_2}^{\semCtx}_{FDL} = 1$ means $[\tempty{e_1}^{\semCtx}_{FDL} = \tempty{e_2}^{\semCtx}_{FDL}] = 0$, that is, $\tempty{e_1}^{\semCtx}_{FDL} \neq \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \neq^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \top$. Similarly, $\tempty{e_1 \leq e_2}^{\semCtx}_{FDL} = 0$ means $[\tempty{e_1}^{\semCtx}_{FDL} = \tempty{e_2}^{\semCtx}_{FDL}] = 1$, that is, $\tempty{e_1}^{\semCtx}_{FDL} = \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \neq^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$. 
...
\subsubsection{Proof of Soundness for G\"{o}del  DL}
...
\begin{proof}
	...
	\textbf{Base Case 1.} 
	Suppose $e = \top$, and $\tGodel{e}^{\semCtx}  = 1$. But we know $\SequentCLJ(){\top}$ by the rule $(\top)$.
	
	\textbf{Base Case 2.} 
	Suppose $e = e_1 \bowtie e_2$. Moreover, $\tGodel{e_1 \bowtie e_2}^{\semCtx} = 1$. 
	...
	Tt means that $\tGodel{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 1$ for at least one input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1[w / x]}$. We therefore can deduce $\SequentCLJ(){\exists x. e_1}$.
	...
	\textbf{Base Case 1.} 
	Suppose $e = \bot$ and $\tGodel{e}^{\semCtx}  = 0$. But we know $\SequentCLJ(\bot){}$ by the $(\bot)$.
	
	\textbf{Base Case 2.} 
	Suppose $e = r_1 \bowtie r_2$, where $\bowtie$ is a comparison operator. Moreover, $\tGodel {  r_1 \bowtie r_2}^{\semCtx} = 0$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$.
	...
	Then $\tGodel{e_1}^{\semCtx} \in \tGodel{\top}^{\semCtx}$. Then by Lemma~\ref{l:s1} Lemma~\ref{l:s1} (Part 1) we have $\SequentCLJ(){e_1 }$. But then by $\LJNegL$ we can derive $\SequentCLJ(\neg e_1){ }$.
	...
	Suppose $e =  e_1 \impl e_2$, and therefore $\tGodel{e}^{\semCtx} = \max (1-\tGodel{e_1}^{\semCtx}, \tGodel{e_2}^{\semCtx}) = 0$. This means that both $1-\tGodel{e_1} = 0$ and $\tGodel{e_2}^{\semCtx}  = 0$. 
...
\subsubsection{Proof of Soundness for Łukasiewicz DL}
We move on the defining analogous theorems for another fuzzy logic - $\tlukasiewicz{}^{\semCtx}$.
For proof to go through, we have to exclude either negation or implication from the syntax. The study of this phenomenon is left for future work.

%We need to state a different version of Lemma~\ref{lem:eval}. 

\begin{comment}
\begin{lemma}[Soundness of FDL comparisons]\label{lem:eval2} If  $\Xi, \Delta \Vvdash e_1 \bowtie e_2$, then the following holds:
If $\tempty{e_1 \bowtie e_2}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} < \tempty{\top}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL}$ then $\tempty{e_1^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{FDL}  = \bot$. 
\end{lemma} 
\begin{proof}
If $\tempty{e_1 == e_2}_{FDL} < 1$ means $|\tempty{e_1}^{\semCtx}_{FDL} - \tempty{e_2}^{\semCtx}_{FDL}| > 0$, that is, $\tempty{e_1}^{\semCtx} \neq \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1}^{\semCtx}_{FDL} ==^* \tempty{e_2}^{\semCtx}_{FDL} = \bot$. 
	
	If  $\tempty{e_1 \leq e_2}^{\semCtx}_{FDL} < 1$ means $\tempty{e_1}^{\semCtx}_{FDL} - \tempty{e_2}^{\semCtx}_{FDL} > 0$, that is, $\tempty{e_1}^{\semCtx}_{FDL} > \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \leq^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$. 
	
	If  $\tempty{e_1 \leq e_2}^{\semCtx}_{FDL} < 1 $ means $[\tempty{e_1}^{\semCtx}_{FDL} = \tempty{e_2}^{\semCtx}_{FDL}] = 1$, that is, $\tempty{e_1}^{\semCtx}_{FDL} = \tempty{e_2}^{\semCtx}_{FDL}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \neq^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$. 
\end{proof}
\end{comment}
...
\begin{lemma}\label{th:l1}
	Given a formula $e$, for any contexts $N, \Gamma, Q$ the following hold:
	\begin{enumerate}
		\item if $\tlukasiewicz{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = \tlukasiewicz{\top}^{\hNetCtx,\hRandCtx, \hVarCtx}$   then $\SequentCLJ(){e}$.
		\item if   
		$\tlukasiewicz{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = \tlukasiewicz{\bot}^{\hNetCtx,\hRandCtx, \hVarCtx}$ 
		 then $\SequentCLJ(e){}$. 
	\end{enumerate}
\end{lemma}
Its corollary is:
%\nnote{But then this needs to be weaker...}
\begin{theorem}[Soundness of Łukasiewicz DL]
Given a formula $e$, for any contexts $N, \Gamma, Q$ if $\tlukasiewicz{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = \tlukasiewicz{\top}^{\hNetCtx,\hRandCtx, \hVarCtx}$ then $\SequentCLJ(){e}$.% \tlukasiewicz{\top}^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$   then $\SequentCLJ(){e}$.
\end{theorem}
...
\begin{proof}
	...
	\textbf{Base Case 1.} 
	Suppose $e = \top$, and $\tlukasiewicz{e}^{\semCtx}  = 1$. But we know $\SequentCLJ(){\top}$ by the rule $(\top)$.
	
	\textbf{Base Case 2.} 
	Suppose $e = e_1 \bowtie e_2$. Moreover, $\tlukasiewicz {e_1 \bowtie e_2}^{\semCtx} = 1$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \top$.
	But then, we can derive $\SequentCLJ(){ r_1 \bowtie r_2}$ by the rules $ \LJArithR$ and $(\top)$. 
	
	\textbf{Inductive Case 1.} Suppose $e = \neg e_1$, and therefore $\tlukasiewicz{e}^{\semCtx} = 1-\tlukasiewicz{e_1}^{\semCtx} = 1$. This is only possible when $\tlukasiewicz{e_1}^{\semCtx} = 0$. 
	Then, by Lemma~\ref{th:l1} (Part 2), 
	we have
	$\SequentCLJ(e_1){}$. 
	But then, by the rule $\LJNegR$, we can derive 
	$\SequentCLJ(){\neg e_1}$.
	%NOT FINISHED
	
	\textbf{Inductive Case 2.} Suppose $e =  e_1 \land e_2$, and therefore $\tlukasiewicz{e}^{\semCtx} = \max (\tlukasiewicz{e_1}^{\semCtx}+ \tlukasiewicz{e_2}^{\semCtx}-1,0) = 1$. This means that  
	$\tlukasiewicz{e_1}^{\semCtx}+ \tlukasiewicz{e_2}^{\semCtx}-1 = 1 $, or $\tlukasiewicz{e_1}^{\semCtx}+ \tlukasiewicz{e_2}^{\semCtx} = 2$. That means we have $\tlukasiewicz{e_1}^{\semCtx} = \tlukasiewicz{e_2}^{\semCtx} = 1 $ And, by the induction hypothesis, we have that $\SequentCLJ(){e_1}$ and $\SequentCLJ(){e_2}$. But then, by the rule $\LJConjR$, we have $\SequentCLJ(){e_1 \land e_2}$.
	
%	\textbf{Inductive Case 3.} Suppose $e =  e_1 \impl e_2$, and therefore $\tlukasiewicz{e}^{\semCtx} = \min (1-\tlukasiewicz{e_1}^{\semCtx}+ \tlukasiewicz{e_2}^{\semCtx},1) = 1$.
%	This means that $1-\tlukasiewicz{e_1}^{\semCtx}+ \tlukasiewicz{e_2^{\semCtx}} \geq 1$. This means that $\tlukasiewicz{e_1}^{\semCtx}\leq \tlukasiewicz{e_2}^{\semCtx}$. This means we have two cases to consider. 
%	First that $\tlukasiewicz{e_2}^{\semCtx}=1$ but then by the induction hypothesis, we have that $\SequentCLJ(){e_2}$.
%	Second we have that $\tlukasiewicz{e_2}^{\semCtx}<1$, $\tlukasiewicz{e_1}^{\semCtx} <1$ and $\tlukasiewicz{e_1}^{\semCtx}\leq \tlukasiewicz{e_2}^{\semCtx}$. Then, by Lemma~\ref{th:l1} (Part 2), we have $\SequentCLJ(){\neg e_1}$.
...
%	In the first case, we use weakening to get $\SequentCLJ(e_1){e_2}$. This allows us to use the rule $\LJImplR$ to derive $\SequentCLJ{e_1 \impl e_2}$.
%	 In the second case, we can derive $\SequentCLJ(\neg\neg e_1){}$, and then by the weakening and the rule $\LJImplR$ we obtain $\SequentCLJ(){\neg \neg e_1 \impl e_2}$. At the same time, $\SequentCLJ(\neg \neg e_1  \impl e_2){ e_1  \impl e_2}$ is provable in \LJ.
%	 Then we can use the cut rule to obtain  $\SequentCLJ{e_1 \impl e_2}$. 
...
%%	Now using one of the weakening rules, we can obtain $\SequentCLJ(\Gamma_T, e_1){e_2}$. This allows us to use the rule $\LJImplR$ to derive $\SequentCLJ{e_1 \impl e_2}$.
	...
	\textbf{Base Case 1.} 
	Suppose $e = \bot$ and $\tlukasiewicz{e}^{\semCtx}  0$. But we know $\SequentCLJ(\bot){}$ by the $(\bot)$. 
	%By $\LJNegR$ we get $\SequentCLJ(){\neg \bot}$.  
	
	\textbf{Base Case 2.} 
	Suppose $e = e_1 \bowtie e_2$, where $\bowtie$ is a comparison operator. Moreover, $\tlukasiewicz {  r_1 \bowtie r_2}^{\semCtx} < 1$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$.
	But then we obtain a proof for $\SequentCLJ(r_1 \bowtie r_2){ }$ by the rules $ \LJArithL $ and ($\bot$). 
	%By $\LJNegR$ we get $\SequentCLJ(){\neg  r_1 \bowtie r_2 }$.
	
	\textbf{Inductive Case 1.} 
                        
	Suppose $e = \neg e_1$, and therefore $\tlukasiewicz{e}^{\semCtx} = 1-\tlukasiewicz{e_1}^{\semCtx} = 0$. This is only possible %when $\tlukasiewicz{e_1} > 0$. Therefore we have two cases - either 
	$\tlukasiewicz{e_1} = 1$ . %or $0<\tlukasiewicz{e_1} < 1$. 
	%In the first case 
	Then we have $\tlukasiewicz{e_1}^{\semCtx} \in \tlukasiewicz{\top}^{\semCtx}$. Then by Lemma~\ref{th:l1} (Part 1) we have $\SequentCLJ(){e_1 }$.
	 But then by $\LJNegL$  we can derive $\SequentCLJ(\neg e_1){}$.
	%In the second case, we use the induction hypothesis and get  $\SequentCLJ(){\neg e_1 }$
	
	%we have $\SequentCLJ(e_1){}$ Lemma~\ref{th:l1} (Part 2). POSSIBLY WEAKENING RULE?% Then using one of the weakening rules we 
%	
%	
%
...
%
	...
	It means that $\tlukasiewicz{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 0$ for some input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(e_1[y / w]){}$. We therefore can deduce $\SequentCLJ(\forall x. e_1){}$. 
...
\subsubsection{Proof of Soundness for Product DL}
We first define analogous theorem for another fuzzy logic - $\tproduct{}$.
...
\begin{proof}
	...
	\textbf{Base Case 1.} 
	Suppose $e = \top$, and $\tproduct{e}^{\semCtx}  = 1$. But we know $\SequentCLJ(){\top}$ by the rule $(\top)$.
	...
	\textbf{Inductive Case 5.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tproduct{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$. This means that the maximum expected value of $\lambda y. \tproduct{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $1$. % Note that $x$ no longer appears in $\tGodel{e_1}^{\hNetCtx,\hRandCtx, y::\hVarCtx}$.
	It means that $\tproduct{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 1$ for at least one input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1[w / x]}$. We therefore can deduce $\SequentCLJ(){\exists x. e_1}$.
	...
	\textbf{Base Case 1.} 
	Suppose $e = \bot$ and $\tproduct{e}^{\semCtx}  = 0$. But we know $\SequentCLJ(\bot){}$ by the $(\bot)$.
	
	\textbf{Base Case 2.} 
		Suppose $e = r_1 \bowtie r_2$, where $\bowtie$ is a comparison operator, and $r_1, r_2$ are real numbers. Moreover, $\tproduct {  r_1 \bowtie r_2}^{\semCtx} = 0$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$.
	But then we obtain a proof for $\SequentCLJ(\Gamma_T,r_1 \leq r_2){ }$ by the rules $ \LJArithL $ and ($\bot$).
	...
	It means that $\tproduct{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 0$ for some input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(e_1[y / w]){}$. We therefore can deduce $\SequentCLJ(\forall x. e_1){}$. 
	%, note that $x \notin FV(\Gamma_T)$, as $\Gamma_T$ is empty. %thanks to an assumption on uniqueness of variables we made previously.%, which must be the case, as, in $\SequentCLJ{e_1}$, the formula $e_1$ does not contain variable $x$. 
	
	\textbf{Inductive Case 5.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tproduct{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 0$. This means that the maximum expected value of $\lambda y. \tproduct{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $0$. % Note that $x$ no longer appears in $\tGodel{e_1}^{\hNetCtx,\hRandCtx, y::\hVarCtx}$.
...
\subsubsection{Proof of Soundness for Yager DL}

We first define analogous theorem for another fuzzy logic - $\tyager{}^{\semCtx}$. It is important to note that the Yager DL does not include implication and so the proof is done for a restricted version of \LJ.

\begin{lemma}\label{th:y1}
	Given a formula $e$, for any contexts $N, \Gamma, Q$ the following hold:
	\begin{enumerate}
		\item if $\tyager{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = \tyager{\top}^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$  then $\SequentCLJ(){e}$.
		\item if   
		$\tyager{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = 0$ 
		 then $\SequentCLJ(e){}$.  
	\end{enumerate}
\end{lemma}

\begin{theorem}[Soundness of Yager DL]
	Given a formula $e$, for any contexts $N, \Gamma, Q$ if $\tyager{e}^{\hNetCtx,\hRandCtx, \hVarCtx} = \tyager{\top}^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$   then $\SequentCLJ(){e}$.
\end{theorem}



\begin{proof}
	The mutually  inductive proof proceeds by case-reasoning on the shape of the formula $e$, and by induction on the structure of $\tyager{}^{\semCtx}$, a choice of one of the DLs.
	As the proof is mutually inductive between two parts of Lemma~\ref{th:p1}, we will refer to them accordingly as 
	Lemma~\ref{th:y1} (Part 1) and Lemma~\ref{th:y1} (Part 2). 
	
	We start with the first part of Lemma~\ref{th:y1}.
	The case of $e = \bot$ is automatically excluded, so our first base case is:
	
	\textbf{Base Case 1.} 
	Suppose $e = \top$, and $\tyager{e}^{\semCtx}  = 1$. But we know $\SequentCLJ(){\top}$ by the rule $(\top)$.
	
	\textbf{Base Case 2.} 
	Suppose $e = e_1 \bowtie e_2$. Moreover, $\tyager {e_1 \bowtie e_2}^{\semCtx} = 1$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \top$.
	But then, we can derive $\SequentCLJ(){ r_1 \bowtie r_2}$ by the rules $ \LJArithR$ and $(\top)$. 
	
	\textbf{Inductive Case 1.} Suppose $e = \neg e_1$, and therefore $\tyager{e}^{\semCtx} = 1-\tyager{e_1}^{\semCtx} = 1$. This is only possible when $\tyager{e_1} = 0$. 
	Then, by Lemma~\ref{th:y1} (Part 2), we have
	$\SequentCLJ(e_1){}$. But then, by the rule $\LJNegR$, we can derive $\SequentCLJ(){\neg e_1}$.
	%NOT FINISHED
	
	\textbf{Inductive Case 2.} Suppose $e =  e_1 \land e_2$, and therefore $\tyager{e}^{\semCtx} = \max (1 - ((1-\tyager{e_1}^{\semCtx})^p + (1-\tyager{e_2}^{\semCtx})^p)^{1/p},0) = 1$ where $p\geq1$.
	 Since $1$ is the top value that means we have $\tyager{e_1}^{\semCtx} = \tyager{e_2}^{\semCtx} = 1 $ And, by the induction hypothesis, we have that $\SequentCLJ(){e_1}$ and $\SequentCLJ(){e_2}$. But then, by the rule $\LJConjR$, we have $\SequentCLJ(){e_1 \land e_2}$.
	
	
	\textbf{Inductive Case 3.} Suppose $e =  \forall x. e_1$, and therefore we have $\tyager{\forall x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$.  This means that the minimum expected value of $\lambda y. \tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $1$. 
	%Note that $x$ no longer appears in $\tGodel{e_1}^{\hNetCtx,\hRandCtx, y::\hVarCtx}$.  
	Seeing that $1$ is the top value, it means that $\tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 1$ for all inputs $y$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1}$. We therefore can deduce $\SequentCLJ(){\forall x. e_1}$ ($x \notin FV(\Gamma_T)$, as $\Gamma_T$ is empty). %thanks to an assumption on uniqueness of variables we made previously.%, which must be the case, as, in $\SequentCLJ{e_1}$, the formula $e_1$ does not contain variable $x$. 
	
	\textbf{Inductive Case 4.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tyager{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$. This means that the maximum expected value of $\lambda y. \tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $1$. % Note that $x$ no longer appears in $\tGodel{e_1}^{\hNetCtx,\hRandCtx, y::\hVarCtx}$.
	It means that $\tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 1$ for at least one input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1[w / x]}$. We therefore can deduce $\SequentCLJ(){\exists x. e_1}$.
	
	We now move on to the proof of the second part of Lemma~\ref{th:y1}. The case of $e = \top$ is automatically excluded, so our first base case is:
	
	\textbf{Base Case 1.} 
	Suppose $e = \bot$ and $\tyager{e}^{\semCtx}  = 0$. But we know $\SequentCLJ(\bot){}$ by the $(\bot)$.
	
	\textbf{Base Case 2.} 
	Suppose $e = r_1 \bowtie r_2$, where $\bowtie$ is a comparison operator, and $r_1, r_2$ are real numbers. Moreover, $\tyager {  r_1 \bowtie r_2}^{\semCtx} = 0$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1^{\beta}}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{FDL} = \bot$.
	But then we obtain a proof for $\SequentCLJ(\Gamma_T,r_1 \leq r_2){ }$ by the rules $ \LJArithL $ and ($\bot$).
	
	\textbf{Inductive Case 1.} 
	Suppose $e = \neg e_1$, and therefore $\tyager{e}^{\semCtx} = 1-\tyager{e_1}^{\semCtx} = 0$. This is only possible when $\tyager{e_1}^{\semCtx} = 1$. % or $\tGodel{e_1} \in (0,1)$. 
	Then $\tyager{e_1}^{\semCtx} \in \tyager{\top}^{\semCtx}$. Then by Lemma~\ref{th:y1} (Part 1) we have $\SequentCLJ(){e_1 }$. But then by $\LJNegL$ we can derive $\SequentCLJ(\neg e_1){ }$.
	
	
	
	\textbf{Inductive Case 2.} 
	Suppose $e =  e_1 \land e_2$, and therefore $\tyager{e}^{\semCtx} = \tyager{e}^{\semCtx} = \max (1 - ((1-\tyager{\val_1}^{\semCtx})^p + (1-\tyager{\val_2}^{\semCtx})^p)^{1/p},0) = 0$ where $p\geq1$. This means that $((1-\tyager{\val_1}^{\semCtx})^p + (1-\tyager{\val_2}^{\semCtx})^p)^{1/p} = 1$, from which we know that
at east one of the following holds - $ \tyager{e_1}^{\semCtx} = 0 $ or $ \tyager{e_2}^{\semCtx} = 0 $. In both cases respectively by induction hypothesis we have $\SequentCLJ(e_1){}$ or $\SequentCLJ(e_2){}$. But then, by the rule $(\LJConjL)$, we have $\SequentCLJ(e_1 \land e_2){}$.

	\textbf{Inductive Case 3.} Suppose $e =  \forall x. e_1$, and therefore we have $\tyager{\forall x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 0$.  This means that the minimum expected value of $\lambda y. \tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $0$. 
	It means that $\tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 0$ for some input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(e_1[y / w]){}$. We therefore can deduce $\SequentCLJ(\forall x. e_1){}$. 

	
	\textbf{Inductive Case 4.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tyager{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 0$. This means that the maximum expected value of $\lambda y. \tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $0$. % Note that $x$ no longer appears in $\tGodel{e_1}^{\hNetCtx,\hRandCtx, y::\hVarCtx}$.
	Seeing that $0$ is the bottom value, it means that $\tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 0$ for all inputs. But then, by the induction hypothesis, we have $\SequentCLJ(e_1){}$. We therefore can deduce $\SequentCLJ(\exists x. e_1){}$, ($x \notin FV(\Gamma_T)$, as $\Gamma_T$ is empty).
\end{proof}

\subsubsection{Proof of Soundness for DL2}
\label{app:dl2-adequacy}


We now provide the proof for Theorem~\ref{th:d1}, $\tDLtwo{}^{\semCtx}$. It is important to remember that DL2 translation does not include a stand-alone negation operator or implication - therefore this proof is done for a limited version of \LJ.	We start with a helper lemma.

\begin{lemma}[Adequacy of intervals and arithmetic operations in DL2]\label{lem:eval1} 
If  $\Xi, \Delta \Vdash e_1 \bowtie e_2$, then the following holds: 	
	if $\tDLtwo{e_1 \bowtie e_2}^{\semCtx} = 0$ then $\tempty{e_1^{\beta}}^{\semCtx}_{DL2} \bowtie^* \tempty{e_2^{\beta}}^{\semCtx}_{DL2} = \top$. 

\end{lemma}
\begin{proof} 
 Since $e_1 \bowtie e_2$ is well-typed each of $e_1, e_2$ has type $\RealType$. In that case, $\tDLtwo{e_i}^{\semCtx} = e_i^{\beta}$ by Lemma~\ref{lem:expr-real}.
  If $\bowtie$ is $==$, then  $\tDLtwo{e_1 == e_2}^{\semCtx} = 0$ means $\tDLtwo{e_1}^{\semCtx} - \tDLtwo{e_2}^{\semCtx} = 0$, that is, $\tDLtwo{e_1}^{\semCtx} = \tDLtwo{e_2}^{\semCtx}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{DL2}  ==^* \tempty{e_2^{\beta}}^{\semCtx}_{DL2}  = \top$. 
	
	If $\bowtie$ is $\leq$, then  $\tDLtwo{e_1 \bowtie e_2}^{\semCtx} = 0$ means $\tDLtwo{e_1}^{\semCtx} - \tDLtwo{e_2}^{\semCtx} \leq 0$, that is, $\tDLtwo{e_1}^{\semCtx} \leq \tDLtwo{e_2}^{\semCtx}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{DL2}  \leq^* \tempty{e_2^{\beta}}^{\semCtx}_{DL2}  = \top$.
	
	If $\bowtie$ is $\neq$, then  $\tDLtwo{e_1 \neq e_2}^{\semCtx} = 0$ means $[\tDLtwo{e_1}^{\semCtx} == \tDLtwo{e_2}^{\semCtx}] = 0$, that is, $\tDLtwo{e_1}^{\semCtx} \neq \tDLtwo{e_2}^{\semCtx}$. But then, $\tempty{e_1^{\beta}}^{\semCtx}_{DL2}  \neq^* \tempty{e_2^{\beta}}^{\semCtx}_{DL2}  = \top$.  
...
\begin{proof}
	\textbf{Base Case 1.} 
	Suppose $e = \top$, and $\tDLtwo{e}^{\semCtx}  = 0$. But we know $\SequentCLJ(){\top}$ by the rule $(\top)$.
	
	\textbf{Base Case 2.} 
	Suppose $e = r_1 \bowtie r_2$, where $\bowtie$ is a comparison operator, and $r_1, r_2$ are real numbers. Moreover, $\tDLtwo {  r_1 \bowtie r_2}^{\semCtx} = 0$. 
	By Lemma~\ref{lem:eval1}, we have $\tempty{e_1^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{DL2} \bowtie^* \tempty{e_2^{\beta}}^{\hNetCtx,\hRandCtx, \hVarCtx}_{DL2} = \top$.
	But then we obtain a proof for $\SequentCLJ{r_1 \bowtie r_2}$ by the rules $ \LJArithR $ and ($\top$).
	
	\textbf{Inductive Case 1.} Suppose $e =  e_1 \land e_2$, and therefore $\tDLtwo{e}^{\semCtx} = \tDLtwo{e_1}^{\semCtx} + \tDLtwo{e_2}^{\semCtx} = 0$.That means we have $\tDLtwo{e_1}^{\semCtx} = \tDLtwo{e_2}^{\semCtx} = 0 $ And, by the induction hypothesis, we have that $\SequentCLJ(){e_1}$ and $\SequentCLJ(){e_2}$. But then, by the rule $\LJConjR$, we have $\SequentCLJ(){e_1 \land e_2}$.
	...
	It means that $\tDLtwo{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 0$ for at least one input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1[w / x]}$. We therefore can deduce $\SequentCLJ(){\exists x. e_1}$.
\end{proof}

%\subsection{STL}\label{app:stl-adequacy}
%
%While in general adequacy does not hold for STL based DL, as explained in Section~\ref{subsec:soundness2} we provide the proof for the following lemma.
%
%\begin{lemma}[Adequacy of intervals and arithmetic operations in STL]\label{lem:eval2} For all well-typed expressions $e_1 \bowtie e_2$, the following holds:
%
%If $\tSTL{e_1 \bowtie e_2}^{\semCtx} \in \tSTL{\top}^{\semCtx}$ then $\tSTL{e_1^{\beta}}^{\semCtx} \bowtie^* \tSTL{e_2^{\beta}}^{\semCtx} = \top$.
%
%If $\tSTL{e_1 \bowtie e_2}^{\semCtx} \in \tSTL{\bot}^{\semCtx}$ then $\tSTL{e_1^{\beta}}^{\semCtx} \bowtie^* \tSTL{e_2^{\beta}}^{\semCtx} = \bot$.
%\end{lemma}
%\begin{proof}
%	 Since $e_1 \bowtie e_2$ is well-typed each of $e_1, e_2$ has type $\RealType$. In that case, $\tSTL{e_i}^{\semCtx} = e_i^{\beta}$. If $\bowtie$ is $==$, then  $\tSTL{e_1 == e_2}^{\semCtx} = \infty$ means $-|\tSTL{e_1}^{\semCtx} - \tSTL{e_2}^{\semCtx}| = -\infty$. %Effectively, it means $-|\tSTL{e_1}^{\semCtx} - \tSTL{e_2}^{\semCtx}| = 0$.
%	 \nnote{Not sure how to proceed from here - we need to get $\tSTL{e_1}^{\semCtx} = \tSTL{e_2}^{\semCtx}$ but with the infinity it's tricky}
%	That is, $\tSTL{e_1}^{\semCtx} = \tSTL{e_2}^{\semCtx}$. But then, $e_1^{\beta} ==^* e_2^{\beta} = \top$. Similarly, $\tSTL{e_1 == e_2}^{\semCtx} \in (-\infty, 0)$ means $\tSTL{e_1}^{\semCtx} - \tSTL{e_2}^{\semCtx} \neq 0$, that is, $\tSTL{e_1}^{\semCtx} \neq \tSTL{e_2}^{\semCtx}$. But then, $\tSTL{e_1^{\beta}}^{\semCtx} ==^* \tSTL{e_2^{\beta}}^{\semCtx} = \bot$. 
	
%	If $\bowtie$ is $\leq$, then  $\tSTL{e_1 \bowtie e_2}^{\semCtx} \in  [0, \infty)$ means $\tSTL{e_2}^{\semCtx} - \tSTL{e_1}^{\semCtx} \in  [0, \infty ]$, that is, $\tSTL{e_1}^{\semCtx} \leq \tSTL{e_2}^{\semCtx}$. But then, $e_1^{\beta} \leq^* e_2^{\beta} = \top$. Similarly, $\tSTL{e_1 \leq e_2}^{\semCtx} \in (-\infty, 0)$ means $\tSTL{e_1}^{\semCtx} - \tSTL{e_2}^{\semCtx} > 0$, that is, $\tSTL{e_1}^{\semCtx} > \tSTL{e_2}^{\semCtx}$. But then, $\tSTL{e_1^{\beta}}^{\semCtx} \leq^* \tSTL{e_2^{\beta}}^{\semCtx} = \bot$. 
%	
%	If $\bowtie$ is $\neq$, then  $\tSTL{e_1 \bowtie e_2}^{\semCtx} \in  [0, \infty)$ means $\tSTL{e_2}^{\semCtx} - \tSTL{e_1}^{\semCtx} \in  [0, \infty ]$, that is, $\tSTL{e_1}^{\semCtx} \leq \tSTL{e_2}^{\semCtx}$. But then, $e_1^{\beta} \leq^* e_2^{\beta} = \top$. Similarly, $\tSTL{e_1 \leq e_2}^{\semCtx} \in (-\infty, 0)$ means $\tSTL{e_1}^{\semCtx} - \tSTL{e_2}^{\semCtx} > 0$, that is, $\tSTL{e_1}^{\semCtx} > \tSTL{e_2}^{\semCtx}$. But then, $\tSTL{e_1^{\beta}}^{\semCtx} \leq^* \tSTL{e_2^{\beta}}^{\semCtx} = \bot$. 
%\end{proof}


\section{Proof Sketches for Logical and Geometric Property Proofs}
...
%	\label{lem:prop}
	...
	\begin{itemize}
		\item $\tDLtwo{}$ is commutative, scale-invariant, associative, sound (for a limited \LJ) and has shadow-lifting. It is not idempotent and does not have quantifier commutativity. While its interpretation for logical connectives is weakly smooth, $\tDLtwo{}$ is not weakly smooth due to the presence of $\min$ and $\max$ in translation of comparisons. While no proofs about the properties were provided in~\citet{fischer2019dl} the translation of conjunction in DL2 and product based fuzzy logic is standard addition. The more common properties including idempotence, commutativity, associativity and min-max boundedness in given domain are known and proven. Considering the interpretation of conjunction is addition it is simple enough to reason about its partial derivatives and therefore shadow-lifting. %It is simple enough to reason about remaining properties as shadow-lifting since the properties of partial derivatives for simple arithmetic operations are well known as well. WHAT ARE THEY?
		\item $\tGodel{}$ is idempotent, commutative, scale-invariant, associative, sound and has quantifier commutativity. It does not have shadow-lifting and it is not weakly smooth.   The interpretation of conjunction is a minimum between the two elements which prohibits shadow-lifting as well as smoothness by definition. The simpler properties are well known and proved in fuzzy logic literature as it is an established fuzzy logic.
		\item $\tlukasiewicz{}$ is  commutative, sound (for a syntax excluding either negation or implication) and associative. It is not idempotent, scale-invariant, weakly smooth and does not have shadow-lifting or quantifier commutativity.  The presence of maxima in the interpretation of conjunction however naturally prohibits shadow lifting and smoothness. Similarly to Gödel it is an established fuzzy logic and many of the properties of its interpretation of conjunction have been proven.
		\item $\tyager{}$ is  commutative, sound (for a limited syntax) and associative. It is not idempotent, scale-invariant, weakly smooth and does not have shadow-lifting or quantifier commutativity.  Similarly to the previous two the presence of maxima prohibits shadow lifting and smoothness. This is another one of the well established fuzzy logics for which a lot of the properties have been already investigated by the community.
		\item $\tproduct{}$ is  commutative, associative, sound and has shadow-lifting. It is not idempotent,  scale-invariant, does not have quantifier commutativity.  While its interpretations logical connectives are weakly smooth $\tproduct{}$ is not weakly smooth due to the interpretation of comparisons.  Furthermore as its interpretation of conjunction is the arithmetic operation of multiplication it is easy to reason about its partial derivates and therefore shadow lifting. Since it is a well-established fuzzy logic its properties the more common logical properties have been extensively studied already.
...
Given two well-typed expressions $e_1$ and $e_2$, if $e_1 \conv e_2$, then $\tempty{e_1^{\beta}}_{\mathcal{DL}}^{a} = \tempty{e_2^{\beta}}_{\mathcal{DL}}^{a}$.
\end{lemma}
...
$\tempty{e^{\beta}}^{a}$ when we write $\tempty{e}$. 
...
\begin{proof}
	...
	By Lemma~\ref{lem:eval2}, we have $e_1^{\beta} \bowtie^* e_2^{\beta} = \top$.
