...
\noindent All other fuzzy DLs, with the exception of Łukasiewicz, are also sound with respect to \LJ, with proofs following the same scheme. For the Łukasiewicz DL, we cannot prove the equivalent of Lemma~\ref{l:s1} unless we remove either negation or implication from the syntax. 
Intuitively, this is because Łukasiewicz implication is not strong enough: e.g. $ \tempty{e_1 \impl e_2} $ evaluates to $1$ as long as $\tempty{e_1} \leq \tempty{e_2}$. Proper study of this phenomenon is left for future work.  We note that the recent work by \citet{BMPP23} has shed light on logical properties of Łukasiewicz logic, and we intend to build upon those results.
%, cf. 
...
\subsection{Proof of Soundness for Łukasiewicz DL}
We move on the defining analogous theorems for another fuzzy logic - Łukasiewicz, denoted by $\tlukasiewicz{}^{\semCtx}$.
...
\begin{proof}
	...
	\textbf{Inductive Case 3.} Suppose $e =  \forall x. e_1$, and therefore we have $\tlukasiewicz{\forall x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$.  This means that the minimum expected value of $\lambda y. \tlukasiewicz{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $1$. 
	...
	It means that $\tlukasiewicz{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 1$ for at least one input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1[y / x]}$. We therefore can deduce $\SequentCLJ(){\exists x. e_1}$.
	...
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2}^{\semCtx}_{FDL} = \bot$.
	But then we obtain a proof for $\SequentCLJ(e_1 \bowtie e_2){ }$ by the rules $ \LJArithL $ and ($\bot$). 
	%By $\LJNegR$ we get $\SequentCLJ(){\neg  r_1 \bowtie r_2 }$.
	
	\textbf{Inductive Case 1.} Suppose $e = \neg e_1$, and therefore $\tlukasiewicz{e}^{\semCtx} = 1-\tlukasiewicz{e_1}^{\semCtx} = 0$. This is only possible $\tlukasiewicz{e_1} = 1$. %or $0<\tlukasiewicz{e_1} < 1$. 
	%In the first case 
...
%
	...
	\textbf{Inductive Case 4.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tlukasiewicz{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} < 1$. This means that the maximum expected value of $\lambda y. \tlukasiewicz{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $0$. 
...
\subsection{Proof of Soundness for Yager DL}

We first define analogous theorem for another fuzzy logic - Yager, denoted $\tyager{}^{\semCtx}$. It is important to note that the Yager DL does not include implication and so the proof is done for a restricted version of \LJ.
...
\begin{proof}
	...
	\textbf{Inductive Case 4.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tyager{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 1$. This means that the maximum expected value of $\lambda y. \tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $1$. 
	It means that $\tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 1$ for at least one input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(){e_1[y / x]}$. We therefore can deduce $\SequentCLJ(){\exists x. e_1}$.
	...
	\textbf{Base Case 2.} 
	Suppose $e = e_1 \bowtie e_2$, where $\bowtie$ is a comparison operator, and $e_1, e_2$ are real numbers. Moreover, $\tyager {  e_1 \bowtie e_2}^{\semCtx} = 0$. 
	By Lemma~\ref{lem:eval}, we have $\tempty{e_1}^{\semCtx}_{FDL} \bowtie^* \tempty{e_2}^{\semCtx}_{FDL} = \bot$.
	But then we obtain a proof for $\SequentCLJ(\Gamma_T,e_1 \leq e_2){ }$ by the rules $ \LJArithL $ and ($\bot$).
...
at east one of the following holds - $ \tyager{e_1}^{\semCtx} = 0 $ or $ \tyager{e_2}^{\semCtx} = 0 $. In both cases respectively by induction hypothesis we have $\SequentCLJ(e_1){}$ or $\SequentCLJ(e_2){}$. But then, by the rule $(\LJConjL)$, we have $\SequentCLJ(e_1 \land e_2){}$.
	...
	It means that $\tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]} = 0$ for some input $w$. But then, by the induction hypothesis, we have $\SequentCLJ(e_1[y / x]){}$. We therefore can deduce $\SequentCLJ(\forall x. e_1){}$. 

	
	\textbf{Inductive Case 4.} Suppose $e =  \exists x. e_1$,  and therefore we have $\tyager{\exists x: \tau .\ e_1 }^{\hNetCtx,\hRandCtx, \hVarCtx} = 0$. This means that the maximum expected value of $\lambda y. \tyager{e_1}^{\hNetCtx,\hRandCtx, \hVarCtx[x\rightarrow y]}$ is $0$.
