...
\vspace{-0.2cm}
\author{
 Wenhang Ge$^1$ \quad Tao Hu $^2$ \quad  Haoyu Zhao $^1$ \quad Shu Liu $^3$ \quad Ying-Cong Chen$^{1, *}$\\
 $^1$ The Hong Kong University of Science and Technology (Guangzhou)\\
 $^2$ The Chinese University of Hong Kong \qquad
 $^3$ SmartMore\\
...
\begin{abstract}
   ...
   To overcome this issue, we propose Ref-NeuS, which aims to reduce ambiguity by attenuating the importance of reflective surfaces.
...
existing methods tend to produce erroneous results in reflective surfaces (such as specular highlights). Since these approaches infer geometry information with multi-view consistency, which is compromised due to the ambiguous surface prediction of the geometry network on reflective surfaces. As a result, their practicality becomes limited in scenarios where reflection is unavoidable.
...
%\end{minipage}
%}
\caption{Our ambiguity-reduced framework significantly improves explicit surfaces geometry (top row), surface normals (middle row) and rendering realism (bottom row) for modeling reflective surfaces compared to NeuS and Ref-NeRF, our baseline and the previous top-performing neural view synthesis model for reflection, respectively. Reconstruction accuracy  (lower is better), surface normal mean angular error, i.e., MAE (lower is better) and PSNR (higher is better) are compared.}
...
To obtain the multi-view pixel colors $\left\{\mathbf{C}_i(\mathbf{r})\right\}_{i=1}^{N}$, we project the surface point $\mathbf{x}$ onto all $N$ images $\left\{\mathbf{I}_{i}\right\}_{i=1}^{N}$ and use bilinear interpolation to obtain the corresponding pixel colors $\left\{\mathbf{C}_i(\mathbf{r})\right\}_{i=1}^{N}$, where $\left\{\mathbf{C}_i(\mathbf{r})\right\}_{i=1}^{N} = \left\{\mathbf{C}_i(\mathbf{r}), \left\{\mathbf{C}_j(\mathbf{r})\right\}_{j=1}^{N-1}\right\}$ denotes the reference pixel color and source pixel colors.  For simplicity, omitting the subscript, the pixel color $\mathbf{C}$ is  obtained by
...
partial images contains reflection, we treat reflection localization as an anomaly detection problem, with the expectation that reflective surfaces will be regarded as the anomaly and assigned a high reflection score. To this end, we  estimate a view-dependent reflection score $\overline{\beta}^2(\mathbf{r})$ by an anomaly detector empirically, which uses the Mahalanobis distance \cite{mahalanobis1936generalised} as the reflection score (i.e., variance) as follows:
\begin{equation}\label{ucc_view}
    \small
    \overline{\beta}^2(\mathbf{r}) = \gamma \frac{1}{N-1} \sum_{j=1}^{N-1} 
    \sqrt{
...
\end{equation}
where $\gamma$ is the a scale factor to control the scale of the reflection score and $\mathbf{\Sigma}^{-1}$ is the empirical covariance matrix.  %The distribution of pixel colors are assumed to be independent. The reflection score is further simplified as
%\begin{equation}
 %   \label{ucc_view}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.47\textwidth]{figure/spongy20230307final.pdf}
...
After capturing the predicted surface point $\mathbf{x}_{i}^{*}$, we can calculate the distances $\left\{d_j^*\right\}_{j=1}^{N-1}$ between this point and all source camera locations $\left\{\mathbf{o}_j\right\}_{j=1}^{N-1}$ as follows:

\begin{equation}
    d_j^* = \frac{\mathbf{x}_{i}^{*} - \mathbf{o}_j}{\operatorname{norm}(\mathbf{x}_{i}^{*} - \mathbf{o}_j)},
\end{equation}
where $\operatorname{norm}$ indicates normalization operation to transfer a vector to a unit vector. Meanwhile, we compute the distances $\left\{d_j\right\}_{j=1}^{N-1}$ from all source camera locations $\left\{\mathbf{o}_j\right\}_{j=1}^{N-1}$ to the first intersection of the intermediate reconstructed meshes by ray casting \cite{roth1982ray}. Based on these two distances, the visibility of source images $\mathbf{I}_j$ is estimated by
\begin{equation}
    \label{visi}
...
Based on the approximation of visibility, we eliminate the invisible pixel colors $\{\mathbf{C}_j(\mathbf{r}) \mid v_j=0\}$ that were used in the calculation of the reflection score in Eq. \eqref{ucc_view}. We then refine the reflection score as follows:
\begin{multline}\label{ucc_form}
    \small
    {\overline{\beta}^2(\mathbf{r})} = 
    \gamma \frac{1}{\sum_{j=1}^{N-1} v_j} \sum_{j=1}^{N-1} v_j\text{Mdis},
     \\
    \small
...
\begin{equation}\label{col}
    \mathbf{c} = c(\mathbf{x}, \hat{\mathbf{d}}),
\end{equation}
where $\hat{\mathbf{d}}$ is the reflection direction calculated by
\begin{equation}
    \hat{\mathbf{d}} = 2(-\mathbf{d} \cdot \mathbf{\hat{n}} )\mathbf{\hat{n}} + \mathbf{d},
\end{equation}
...
We used the glossy cans and corncho1 from \cite{park2020seeing}, which provides meshes scanned by RGBD sensors for evaluation. Since there are more than 2000 images, we only sampled half of them for training due to limited memory. \footnote{The number of training images does not necessarily affect the reconstruction results with hundreds of uniformly-distributed views.}
...
\begin{figure*}
    ...
    \begin{minipage}{0.16\textwidth}
    \centering
    \includegraphics[width=\columnwidth]{figure/r_163_normal-removebg-preview.pdf}
...
We conducted an ablation study on the Shiny Blender dataset to evaluate the effectiveness of each component in our model. We used NeuS \cite{wang2021neus} as our baseline, and the experimental results are reported in Table \ref{ablation}.
``NeuS w/ RS'' indicates that we computed the reflection-aware photometric loss with the reflection score estimated by Eq. \eqref{ucc_view}, without accounting for visibility. This leads to a smaller improvement in performance over the baseline, suggesting that using the reflection score as a source of variance can still be beneficial in improving geometry.
...
The radiance network comprises 4 hidden layers of size 256, which parametrize view-dependent radiance. It takes as input the spatial position $\mathbf{x}$, the normal vector $\mathbf{\hat{n}}$, the reflection direction $\hat{\mathbf{d}}$, and the 256-dimensional geometry feature vector. We applied positional encoding with 6 frequencies to the spatial location $\mathbf{x}$ and 4 frequencies to the view direction $\hat{\mathbf{d}}$.
\begin{figure}
    \centering
    \begin{minipage}{0.48\textwidth}
