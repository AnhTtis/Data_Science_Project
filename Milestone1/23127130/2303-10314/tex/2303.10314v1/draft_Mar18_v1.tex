...
correlations or not is essential: typically for the chaotic systems, the energy
...
the random matrix theory 
 (RMT) as well
and therefore 
are regarded as typical behaviors of the chaotic systems. 
...
in the the small $\hbar$ expansion, where 
\begin{align}
...
decay exponentially as a function of $\tau$. This is in contrast to the one-pint function which decays $\sim e^{ - \tau^2}$ rather than $\sim e^{- \tau}$.  
...
Z_n(\{\beta_i\})
 ...
 &=\Tr\log\left(
   1+\left[-1+\prod_{i=1}^n(1+z_i e^{\beta_iQ})\right]\Pi\right)
   \Bigg|_{{\cal O}(z_1\cdots z_n)}\\
 &=\Tr\log\left(
   1+\sum_{k=1}^n\sum_{i_1<\cdots<i_k}z_{i_1}\cdots z_{i_k}
...
Eq.~\eqref{ftauasconnectedtwopt} manifestly shows that $f(\tau)$
...
\end{align}
%
for which $\tau^{(\mathrm{max})}$ is unambiguously determined.
...
\acknowledgments
This work was supported in part by JSPS KAKENHI Grant Number 21J20906 (TA), 18K03619 (NI), 19K03856 (KS), 22K03594 (KO). This work was also supported by MEXT KAKENHI Grant-in-Aid for Transformative Research Areas A ``Extreme Universe'' No.~21H05184 (NI) and  No.~21H05187 (KO).
