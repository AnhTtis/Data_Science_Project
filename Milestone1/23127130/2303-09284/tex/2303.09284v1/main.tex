...
\newcommand*{\mkgreen}[1]{{{#1}}}
\newcommand*{\mkcyan}[1]{{{#1}}}
\newcommand*{\mkmag}[1]{{{#1}}}
...
% \title[Constrain the DM spike profile around GC with stellar orbits]{Constrain the dark matter spike profile around the Galactic centre with stellar orbits}
...
In this work, we investigate the DM spike profile using updated data from the Keck and VLT telescopes considering that the presence of such an extended mass component may affect the orbits of the S-stars in the Galactic center.
...
where the density of DM peaks and certain excesses have been reported and investigated~\citep{Hooper2011,DiMauro2021,Cholis2022,Bringmann2012,2015PhRvD..91l3010Z,Ackermann2015,DAMPE:2021hsz}. 
...
with the spike slope range $2.25<\gamma_{\rm sp}<2.5$~\citep{Gondolo:1999ef,Sadeghian2013,Ferrer2017}.
This spike profile has been widely adopted in previous studies, 
leading to strong constraints on the DM annihilation rate~\citep[e.g.][]{Gondolo2000,Bertone2002,Fields2014,Lacroix2017,Xia2021,Liu2022}.




The formation of SMBHs is a complex process, and it may not always be adiabatic or initially located at the center of the galaxy, leading to a flattening of the DM spike.
...
Additionally, the spike density can be weakened if the seed BH is massive enough and located off-center~\citep{Ullio2001}. 
...
There are two options: the stellar orbits of S-stars~\citep{Weinberg2005,Lacroix:2018zmg} and the gravitational wave~\citep{Eda2013,Li2022}, while our work only focus on the former one.
...
\begin{equation}\label{eq::einasto_halo}
    \rho_{\rm ein,halo}(r)=\rho_{0} \exp \left \{-\frac{2}{\alpha}\left[\left(\frac{r}{r_{s}}\right)^{\alpha}-1\right]\right\},
\end{equation}
...
In the GS model, the SMBH accretes the interstellar medium from scratch in the centre, therefore $M_{{\rm tot},i}^{\rm GS}(r)=M_{{\rm dm},i}(r)$ and $M_{{\rm tot},f}^{\rm GS}(r)=M_{{\rm dm},f}^{\rm GS}(r)+M_{\rm BH}$.
...
The spike slopes barely change within the spike radius and equal to $2.26$ and $2.28$ for the \einn and \einw, respectively.
...
To release the assumptions of the GS model, we choose the spike radius $R_{\rm sp}$ as the free parameter.
To obtain the enclosed mass profile given $R_{\rm sp}$, we use
\begin{equation}
    M_{{\rm tot},f}(r) = M_{{\rm dm},f}(r) + M_{\rm acc}(R_{\rm sp}),%M_{{\rm dm},i}(2 R_{\rm sp}),
\end{equation}
and
\begin{equation}
...
Since the S2 orbit only covers a small range of the spike (the dark gray band in Fig.~\ref{fig::profile}), it is difficult to determine the two parameters at the same time.
...
The orange hollow and blue solid points are the astrometic measurements from the VLT and Keck telescopes respectively, while the black line represents the MAP orbit of S2 which moves clockwise.
...
In the left panel of Fig.~\ref{fig::gnfw_Rsp_s2}, we show the 68\% credible interval and 95\% upper limit of $R_{\rm sp}$ with the color band and solid line.
...
In Fig.~\ref{fig::gnfw_gammasp}, we present the 68\% credible interval and 95\% upper limit of $\gamma_{\rm sp}$ with the color band and solid line.
...
We draw the spike slopes predicted by the GS spike model \mkblue{(Eq.(\ref{eq::gs_spike_slope}))} in the black dashed line.
The GS spike profile with $\gamma \geq 0.92$ is excluded at 95\% level.

The DM spike slope $\gamma_{\rm sp}$ could be flatten to 3/2 when the DM particles are efficiently heated by the dynamical processes~\citep{Merritt2002a,Gnedin:2003rj,Shapiro2022}.
...
\begin{figure*}
    \includegraphics[width=0.85\textwidth]{./fig_best_all_4stars.pdf}
...
\begin{figure}
    \includegraphics[width=\columnwidth]{./fig_Rsp_gamma_4stars.pdf}
    \caption{\label{fig::gnfw_Rsp_4stars}
        The constraints of the gNFW spike radius $R_{\rm sp}$ with the measurements of S2 from Keck and S2, S1, S9 and S13 from VLT.
        The spike slope $\gamma_{\rm sp}$ is fixed to the GS model value.
        The median (green dotted line), 68\% credible interval (green band) and 95\% upper limit (green solid line) of the spike radius are shown. 
        The 95\% upper limit from~\citet{Lacroix:2018zmg} is shown in pink dot-dashed line.
        The expected spike radius in GS spike model is shown with the black dashed line.
        The GS spike model is not favored by data when $\gamma \geq 0.90$ by 95\% chance.
    }
\end{figure}
...
\subsection{Joint analysis with the S2, S1, S9 and S13 measurements}\label{sec::discussion:4stars}
...
We find that the GS spike model with $\gamma>0.92$ is excluded at 95\% level.
If we combine the data from S2, S1, S9 and S13, the limits of the GS model are slightly improved: 
$\gamma < 0.90$ (see Fig.~\ref{fig::gnfw_Rsp_4stars}).
...
\begin{thebibliography}{}
    ...
    \bibitem[\protect\citeauthoryear{{Abuter} et~al.,}{{Abuter}
      et~al.}{2018}]{GRAVITY:2018ofz}
    {Abuter} R.,  et~al., 2018, \mn@doi [\aap] {10.1051/0004-6361/201833718}, \href
      {https://ui.adsabs.harvard.edu/abs/2018A&A...615L..15G} {615, L15}
    ...
    \bibitem[\protect\citeauthoryear{{Straub} et~al.,}{{Straub}
      et~al.}{2023}]{GRAVITY2023}
    {Straub} O.,  et~al., 2023, preprint, \href
      {https://ui.adsabs.harvard.edu/abs/2023arXiv230304067T} {} (\mn@eprint
      {arXiv} {2303.04067})
...
\begin{table}
    ...
    \caption{
        ...
        The fourth column presents the constraints using the combined observations of 4 S-stars.
        See Sec.~\ref{sec::results} and Sec.~\ref{sec::discussion:4stars} for detail.
    }\label{tab::constraints}
    \begin{tabular}{lccc|cc}
     \hline
     $\gamma$ & $R_{\rm sp}^{\rm GS}$ & $R_{\rm sp,95\%,S2}$ & $R_{\rm sp,95\%,4stars}$ & $\gamma_{\rm sp}^{\rm GS}$ & $\gamma_{\rm sp,95\%,S2}$ \\
              &      (pc)       &      (pc)            & (pc) & & \\
     \hline
     0.5 & 37.9 & 71.9 & 69.8 & 2.285 & 2.45 \\
     0.6 & 34.3 & 55.4 & 51.6 & 2.294 & 2.42 \\
     0.7 & 30.3 & 44.9 & 41.8 & 2.303 & 2.37 \\
     0.8 & 26.3 & 32.5 & 29.3 & 2.313 & 2.36 \\
     0.9 & 22.5 & 23.5 & 22.3 & 2.323 & 2.33 \\
     1.0 & 18.6 & 15.7 & 14.2 & 2.333 & 2.32 \\
     1.1 & 14.8 & 10.2 &  9.5 & 2.345 & 2.28 \\
     1.2 & 11.4 &  5.7 &  5.5 & 2.357 & 2.25 \\
     1.3 &  8.4 &  3.0 &  3.0 & 2.370 & 2.23 \\
     1.4 &  5.9 &  1.4 &  1.4 & 2.385 & 2.18 \\
     1.5 &  4.0 &  0.6 &  0.5 & 2.400 & 2.14 \\
     \hline
    \end{tabular}
\end{table}
