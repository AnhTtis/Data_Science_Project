...
model fine-tuning. 
Another line of work~\cite{thrush2022winoground,parcalabescu2021valse,zhao2022vl} probe VLMs on similarity measure between image-text inputs in a zero-shot manner, by designing stress tests 
on image-text pairs with minimal \emph{caption} semantic changes while keeping images intact. 
To the best of our knowledge, \benchname is the first to test whether the inherent image-text similarity measure in existing VLMs is sensitive to visual semantic changes. 
Compared to 
caption-centric benchmarks, \benchname explores both natural and synthetic ways to generate image pairs with minimal semantic change, making the data generation process 
...
However, the equivariance property still plays an important role in various fields, such as self-supervised learning~\cite{dangovski2021equivariant, xie2022should, wang2021self}, representation learning~\cite{qi2020learning}, and language understanding~\cite{gordon2019permutation}.
