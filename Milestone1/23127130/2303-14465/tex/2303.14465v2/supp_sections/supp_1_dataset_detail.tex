...
% \wangtan{good points, have added sentences below}
...
% \linjie{In general, the subscripts used for each dataset are inconsistent. It is quite confusing what they mean.Especially we used $I_i$, $I_j$ for AG and $I_i$, $I_{i+1}$ for GEBC. May need a paragraph or a few sentence to explain notations.}
...
% \wangtan{yes, fixed} \linjie{can we add a sentence on why we do not want two samples to be too similar? As they will be seen as duplicates?}
...
\begin{figure}[h!]
    ...
    % \wangtan{fix}
    % \linjie{there is no frame filter in this figure. maybe correct the caption or add it to the figure?}
    }
...
Intuitively, 
% the annotation format of GEBC has satisfied our requirements. 
...
% Therefore, it is difficult for the distinction with the ``loss of information'' (from two video segments to two frames).
...
% \underline{\emph{Practical Problem}}: We find that for the cooking video, the choosing frame may contain the view of chef rather than the cooking step. This leads to the mismatch between the image and the caption of receipt.
% \underline{\emph{Solution}}: We apply a extremely simple yet effective xxx. Since the mismatch frame is all about the chef, we discard the frames with human faces by utilizing the mature face recognition detector~\cite{xx}.
%The filtering process leads to our final \textsc{Eq-YouCook2} subset with xxx image-text pairs.
...
% \wangtan{fix to avoid confusion} \linjie{shouldn't this example be scene change as object remains the same?}
