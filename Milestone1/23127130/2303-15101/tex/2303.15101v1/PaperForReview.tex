...
% The specular reflectance is crucial to resolving the GBR ambiguity~\cite{belhumeur1999}. 
Therefore, recent advances in UPS (\eg,~\cite{li2022self, zheng2019spline, li2022neural}) adopt the isotropic reflectance model accounting for non-Lambertian effects to solve UPS. Nonetheless, such a model restricts methods' performance on objects with more general (\eg, anisotropic) materials, while modeling general reflectance is challenging due to extra unknowns, which eventually make UPS intractable. 
Other works (\eg,~\cite{yang2022ps, yang2022s, li2022neural, li2022self}) notice the benefits of the shadow cues in utilizing global shape-light information to solve PS because the shadow reflects the interaction of shape and light~\cite{yu2002shadow, kriegman2001shadows}.
However, these methods either fail to exploit the shadow cues due to the lack of a differentiable path from the shadow to the concerned unknowns like shape~\cite{li2022neural, yang2022ps, li2022self}, or the shadow cues have limited effects on the visible shape reconstruction due to the implicit shape representation~\cite{yang2022s}. 
...
\begin{itemize}[itemsep=2.pt,topsep=2.pt,parsep=2.pt]
    ...
    DANI-Net achieves state-of-the-art performance on several real benchmark datasets.
\end{itemize}
...
\Tref{tab:method} summarizes differences between representative existing PS methods and the proposed DANI-Net. We also briefly review recent advances on neural reflectance representation in 3D vision. 
...
That is, some works~\cite{li2022self, li2022neural, yang2022ps} calculate the shadow map without simultaneously optimizing shadow and other unknowns due to the discontinuity of the binary representation of the shadow map; the others either need the ground truth of shadow maps and ignore the shading cue~\cite{karnieli2022deepshadow}, or rely on priors of scene and light to ensure the accuracy~\cite{yang2022s}.  
...
Different from the above methods that work on multi-view inputs and focus on the full but coarse geometry recovery, DANI-Net takes single-view inputs under a PS setup and focuses on the fine-detailed recovery of depth and surface normal. 
...
However, general reflectance introduces more unknowns, which requires extra cues to solve. Another ambiguity is reflectance-light ambiguity denoted as a non-zero scalar between $e_j$ and other terms in \eref{eq:general_brdf}. 
Researchers focus on resolving the GBR ambiguity while ignoring reflectance-light ambiguity, as the former determines the accuracy of the estimated surface normal. This paper also focuses on resolving GBR ambiguity.
...
Finding the differentiable relationship between shadow, shape, and light is beneficial for solving UPS since 1) it generates a more accurate shadow map through iterative optimization which leads to a more accurate shape-light estimation; 2) compared to previous UPS methods~\cite{li2022neural, li2022self} lacking usage of global shape-light information, it facilitates fully exploiting shadow cues containing that information to solve UPS.
While the attached shadow is already differentiable to the surface normal and light direction in a specific domain, finding the differentiable relationship for cast shadow is more tricky, since it directly associates with light and the entire depth map instead of the surface normal at a particular point. The latter rises two key technical challenges to optimizing DANI-Net. First, since the pixel-wise cast shadow calculation increases the computational cost, it is necessary to introduce a technique to improve the training efficiency. Second, since there is no direct connection between the cast shadow and the surface normal, an effective fitting method that calculates the surface normal from the depth map of limited resolution is another required technique to ensure the backpropagation efficiency and the connection from the global shape information (\ie, the depth map) to the reflectance model for inverse rendering.
...
Although we can directly infer $\hat{w}^k_{ij}$ through a MLP, it significantly increases the training time.
To this end, we apply {\it grid bi-linear interpolation} to interpolate $\hat{w}^k_{ij}$ by retrieving its four neighboring points' depth values from $W$, as shown in \Fref{fig:interpolate}.  The grid bi-linear interpolation is much faster\footnote{The inference time of grid bi-linear interpolation vs. depth inference (MLP) is 0.002 vs. 0.212 seconds on average, for 64 sampled points and a batch size of 1 on {\sc DiLiGenT} dataset~\cite{shi2016benchmark}} than inferring $\hat{w}^k_{ij}$, which speeds up the training process.
...
they either use excessive neighbor points ($> 4$)~\cite{qi2018geonet, nehab2005efficiently} that reduces the precision of the fitted surface normal, or use just a few points~\cite{nakagawa2015estimating} ($<4$) that brings ineffective backpropagation since the gradients affect a limit number of points on the depth map, or use a proper number of points ($=4$) while ignoring the information of the query point~\cite{li2022self}, as shown in \Fref{fig:fit}.
...
\begin{aligned}
\label{eq:xy}
    \boldsymbol{x}_i & = \boldsymbol{V}_d - \frac{\boldsymbol{n}_i}{\boldsymbol{V}_d \cdot \boldsymbol{n}_i},~~~
...
As shown in \Fref{fig:overview}, our neural inverse rendering framework integrates the differentiable shadow handling method and anisotropic reflectance model to two Multi-Layer Perceptrons (MLPs)\footnote{More implementation details such as the network architecture could be found in the supplementary material.}
...
The DepthMLP estimates $w_{i}$ pixel-wisely, while the MaterialMLP outputs $c^k_i$ and $\rho^d_i$ for spatially varying reflectance.
...
We initialize $\boldsymbol{l}_j$ and $e_j$ using outputs of a pre-trained light calibration model, \ie, we apply the CNN structures in~\cite{li2022neural} and train on the Blobby and Sculpture dataset~\cite{chen2018ps}.
...
\begin{table*}[t]
    ...
    \caption{Quantitative comparison in terms of MAE of surface normal  on {\sc DiLiGenT} benchmark dataset~\cite{shi2016benchmark}. \textbf{Bold numbers} and \underline{underlined numbers} indicate the best and the second-best results, respectively.}
    \vspace{-10pt}
...
The silhouette loss function $\mathcal{L}_\text{Si}$ is similar to those in~\cite{hashimoto2019uncalibrated, chen2020learned, li2022self} by calculating the cosine similarity between estimated and fitted silhouette's normal\footnote{Fitted silhouette's normal is pre-computed based on the geometry intuition, \ie, the projection of silhouette's normal to xy-plane is likely to be perpendicular to the silhouette if we assume the silhouette is occluding. For non-occluding silhouette, we apply a flexible strategy to use $\mathcal{L}_{\mathrm{Si}}$, please refer to the supplementary material for more details.}.
...
\begin{align}{\footnotesize
\begin{aligned}
     \mathcal{L}_{\text{smooth}} =&\mathcal{L}_{R^d}+ \mathcal{L}_{W}+\lambda_N \mathcal{L}_N\\
    = & \frac{1}{\#\mathbb{P}} \sum_{i=1}^{\#\mathbb{P}}|\frac{\partial R^d}{\partial u} + \frac{\partial R^d}{\partial v}|
     +  \frac{1}{\#\mathbb{P}} \sum_{i=1}^{\#\mathbb{P}}|\frac{\partial W}{\partial u} + \frac{\partial W}{\partial v}|\\
...
\begin{align}
\begin{aligned}
      \mathcal{L}_\text{stage1}=&\mathcal{L}_\text{IR}+ \lambda_\text{Si} \mathcal{L}_\text{Si}+\lambda_{\text{smooth}} \mathcal{L}_{\text{smooth}},\\
       \mathcal{L}_\text{stage2}=&\mathcal{L}_\text{IR}  + \lambda_\text{Si}\mathcal{L}_\text{Si}+ \lambda_{\text{smooth}}\lambda_N\mathcal{L}_N,\\
       \mathcal{L}_\text{stage3}=&\mathcal{L}_\text{IR} + \lambda_\text{Si}\mathcal{L}_\text{Si},
    \end{aligned}
\end{align}
}where 
$\lambda_N=0.01$, $\lambda_\text{smooth}=0.01$, and $\lambda_\text{Si}=0.01$.
...
As can be observed from \Tref{tab:normal_diligent} and \Tref{tab:light_diligent}, the performance of DANI-Net {\it w/o $s$} and DANI-Net {\it w} \cite{li2022neural} drops on surface normal estimation (the average MAE increases $0.33^\circ$ and $0.53^\circ$, respectively), while remains similar on light calibration compared with DANI-Net. However, as shown in \Fref{fig:shadow_comp}, DANI-Net generates much more realistic and smooth shadow maps than its alternatives. 
...
In contrast, DANI-Net overcomes these limitations by explicitly calculating shadow maps based on iteratively optimized depth and light, free from impact from surface points with dark color and accumulating errors in the depth map, leading to high-quality shadow maps.
...
Our unsupervised training manner and anisotropic reflectance modeling are beneficial to generalize DANI-Net to data from different sources.
...
\textbf{Network structure.} We show the details of the DepthMLP and MaterialMLP in \Fref{fig:network}. We use the same pre-trained light model as~\cite{li2022neural,chen2020learned} for light initialization. More details of the pre-trained light model can be found in the supplementary materials of SCPS-NIR~\cite{li2022self}.
\begin{figure}[h]
    \centering
...
From \Fref{fig:diligent1} to \Fref{fig:diligent4} we show the normal map, error map, and light map of 10 objects from {\sc DiLiGenT} Dataset~\cite{shi2016benchmark} predicted by DANI-Net and its alternatives (\ie, DANI-Net {\it w} [\textcolor{green}{25}] and DANI-Net {\it w/o} $s$). We further compare DANI-Net with recent UPS methods~\cite{li2022self, chen2020learned} and state-of-the-art unsupervised PS methods~\cite{li2022neural}. The results illustrate the effectiveness of the differentiable shadow handling method in DANI-Net.
\label{sec:comp_diligent}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{"Images/sec2/DiLiGenT2_c.pdf"}
    \caption{The visual quality comparison among DANI-Net, DANI-Net {\it w} [\textcolor{green}{25}], DANI-Net {\it w/o} $s$, SCPS-NIR~\cite{chen2019self}, CW20~\cite{chen2020learned},  and LL22~\cite{li2022neural} on {\sc Ball} and {\sc Buddha} from {\sc DiLiGenT}~\cite{shi2016benchmark} in terms of normal map (row 1,4), error map (row 2, 5), and light map (row 3, 6). Numbers indicate the MAE (for surface normal map or light directions) or scale-invariant relative error (for light intensity).}
    \label{fig:diligent1}
\end{figure}
\newpage
\vspace*{\fill}
\begin{figure}[h]
    ...
    \caption{The visual quality comparison among DANI-Net, DANI-Net {\it w} [\textcolor{green}{25}], DANI-Net {\it w/o} $s$, SCPS-NIR~\cite{chen2019self}, CW20~\cite{chen2020learned},  and LL22~\cite{li2022neural} on {\sc Bear}, {\sc Cat}, and {\sc Harvest} from {\sc DiLiGenT}~\cite{shi2016benchmark} in terms of normal map (row 1, 4, 7), error map (row 2, 5, 8), and light map (row 3, 6, 9). Numbers indicate the MAE (for surface normal map or light directions) or scale-invariant relative error (for light intensity).} 
    \label{fig:diligent2}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{The visual quality comparison among DANI-Net, DANI-Net {\it w} [\textcolor{green}{25}], DANI-Net {\it w/o} $s$, SCPS-NIR~\cite{chen2019self}, CW20~\cite{chen2020learned},  and LL22~\cite{li2022neural} on  {\sc Goblet}, {\sc Pot1}, and {\sc Pot2} from {\sc DiLiGenT}~\cite{shi2016benchmark} in terms of normal map (row 1, 4, 7), error map (row 2, 5, 8), and light map (row 3, 6, 9). Numbers indicate the MAE (for surface normal map or light directions) or scale-invariant relative error (for light intensity).} 
    \label{fig:diligent3}
\end{figure}
\vspace*{\fill}\clearpage
...
\begin{figure}[h]
    ...
    \caption{The visual quality comparison among DANI-Net, DANI-Net {\it w} [\textcolor{green}{25}], DANI-Net {\it w/o} $s$, SCPS-NIR~\cite{chen2019self}, CW20~\cite{chen2020learned},  and LL22~\cite{li2022neural} on  {\sc Reading} and {\sc Cow} from {\sc DiLiGenT}~\cite{shi2016benchmark} in terms of normal map (row 1,4), error map (row 2, 5), and light map (row 3, 6). Numbers indicate the MAE (for surface normal map or light directions) or scale-invariant relative error (for light intensity).} 
    \label{fig:diligent4}
\end{figure}
\vspace*{\fill}\clearpage
...
Although the fitted silhouette normal is only reliable at the occluding boundaries, the silhouette loss $\mathcal{L}_{\mathrm{Si}}$ is crucial in alleviating GBR ambiguity that facilitates DANI-Net's convergence and prevents it from falling into the local optimum. We compare DANI-Net with `DANI-Net {\it w/o} $\mathcal{L}_{\mathrm{Si}}$' (or $\mathcal{L}_{\mathrm{Si}}$) that drop $\mathcal{L}_{\mathrm{Si}}$ at the beginning (or at stage 3) on {\sc DiLiGenT} dataset~\cite{shi2016benchmark}. As shown in \Tref{tab:silhouette}, the average MAE of normal estimation on 10 objects increases $0.52^{\circ}$ (or $0.14^{\circ}$), indicating the necessity of silhouette loss. 

For objects in {\sc Light Stage Data Gallery}~\cite{chabert2006relighting} and {\sc DiLiGenT} $10^2$~\cite{ren2022diligent102} with non-occluding silhouette, DANI-Net uses silhouette loss in a more flexible way. That is, we either drop the silhouette loss at stage 3 ({\sc Knight Kneeling}, {\sc Knight Fighting}, and {\sc Knight Standing} from {\sc Light Stage Data Gallery}~\cite{chabert2006relighting}), or roughly estimate the silhouette's normal instead of pre-computing it through silhouette's perpendicular vectors ({\sc Bunny, Hexagon, Nut, Pentagon, Propeller, Square,} and {\sc Turbine} from {\sc DiLiGenT} $10^2$~\cite{ren2022diligent102})\footnote{It is easy to estimate the silhouette normal of those objects as $[0, 0, 1]$. We apply the same measurement to other methods involving silhouette loss (\ie, SCPS-NIR~\cite{li2022self}) when comparing DANI-Net with those methods.}. To clarify the necessity of this strategy, we visualize the estimated normal map of DANI-Net and `DANI-Net {\it w} $\mathcal{L}_\mathrm{Si}$' that keeps $\mathcal{L}_\mathrm{Si}$ in all stages on {\sc Knight Fighting} with partially non-occluding silhouettes on the sword (the top row of \Fref{fig:silh}), and {\sc Nut Pp} with predominantly non-occluding silhouettes on its base (the bottom row of \Fref{fig:silh}). It can be observed that keeping $\mathcal{L}_{\mathrm{Si}}$ in all stages does not significantly affect the normal estimation results for objects with partially occluding silhouettes like {\sc Knight Fighting}, but it does degrade the performance for objects with predominantly non-occluding silhouettes (MAE on normal estimation increases $1.16^{\circ}$ on {\sc Nut Pp}). This validates that objects with non-occluding silhouettes should be handled differently.
% However, a more generic solution for objects that may contain non-occluding boundaries that can be easily distinguished from images is to drop $\mathcal{L}_{\mathrm{Si}}$ at stage 3, we denote the alternative of DANI-Net that implements such strategy as `DANI-Net {\it w/o} $\mathcal{L}_{\mathrm{Si}}$-3' (see \Tref{tab:silhouette}). Although the average MAE increases $0.14^{\circ}$, it helps suppress artifacts for objects with dominant non-occluding boundaries.
...
% `($x$)' indicates $x$ lights are used in the certain methods.}
    ...
    Method & \sc{Ball}  & \sc{Bear}  & \sc{Buddha} & \sc{Cat}   & \sc{Cow}   & \sc{Goblet} & \sc{Harvest} & \sc{Pot1}  & \sc{Pot2}  & \sc{Reading} & AVG    \\ 
    \hline
    DANI-Net {\it w/o} $\mathcal{L}_{\mathrm{Si}}$ & 1.68  & 4.22  & 9.44  & 5.30  & 5.79  & 8.12  & 14.18  & 7.43  & 5.53  & 8.89  & 7.06  \\
    DANI-Net {\it w/o} $\mathcal{L}_{\mathrm{Si}}$-3 & 1.81  & 4.07  & 8.72  & 5.15  & 5.69  & 7.10  & 13.77  & 6.95  & 5.26  & 8.30  & 6.68  \\
...
\begin{figure}[h]
    ...
    \caption{Visualization of the estimated normal map on {\sc Knight Fighting} (top), and {\sc Nut Pom} (bottom) from {\sc Light Stage Data Gallery}~\cite{chabert2006relighting} and {\sc DiLiGenT}$10^2$~\cite{ren2022diligent102}. Sub-figures in the left (or right) column are normal maps estimated by DANI-Net (or `DANI-Net {\it w} $\mathcal{L}_\mathrm{Si}$') Red boxes highlight prominent regions for easy comparison. Red arrows point to the silhouette of objects. Numbers at the bottom indicate the MAE of normal estimation.} 
    \label{fig:silh}
\end{figure}
...
\textbf{Quantitative results of light calibration.} \Fref{fig:diligent100_dir} and \Fref{fig:diligent100_int} present the light direction and intensity calibration results, respectively, compared with CW20~\cite{chen2020learned}, SCPS~\cite{chen2019self}, and SCPS-NIR~\cite{li2022self}. DANI-Net reaches the second-best performance on light direction calibration and the best on light intensity calibration. By cross-comparing the results of normal estimation, we observe that the cases with high normal estimation MAE also result in particularly high MAE in light calibration. 
\begin{figure}[h]
    \vspace{-5pt}
    \centering
...
and magnify the pixel intensity 5 times on objects except {\sc Plant} because the original pixel intensities on those objects are too low, leading to strong artifacts on the predicted normal map. We also downsample the {\sc Plant}'s observed images to 0.5 of their original size for efficient training.
...
\begin{figure}[h]
    ...
    \caption{The visual quality comparison among DANI-Net, SCPS-NIR, and CW20~\cite{chen2020learned} on {\sc Apple}, {\sc Gourd1}, and {\sc Gourd2} from {\sc Apple \& Gourd} and {\sc Plant} from {\sc Light Stage Data Gallery} in terms of normal map (column 2) and light map (column 3 - 6). Numbers indicate the MAE (for light directions).}
    \label{fig:gallery}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{The visual quality comparison among DANI-Net, SCPS-NIR, and CW20~\cite{chen2020learned} on {\sc Helmet Left}, {\sc Helmet Front}, {\sc Knight Kneeling}, {\sc Knight Standing} and {\sc Knight Fighting} from {\sc Light Stage Data Gallery} in terms of normal map (column 2) and light map (column 3 - 6). Numbers indicate the MAE (for light directions).}
    \label{fig:gallery2}
\end{figure}
\end{document}
