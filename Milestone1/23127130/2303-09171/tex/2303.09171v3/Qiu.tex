...
\affiliations{
    ...
    changqing\_qiu@bit.edu.cn, jfs21cn@bit.edu.cn,
    zhangyining@stu.pku.edu.cn
...
FG-CAM outperforms existing CAM-based methods significantly in both shallow and intermediate layers, and outperforms LRP and its variants significantly in the input layer.
...
Layer-CAM, on the other hand, although outperforming other CAM-based methods on shallow layers, suffers from shattered gradient problem~\cite{A13} as the model gets deep and the target layer becomes shallow, which can lead to problems such as making explanation noisy and reduced faithfulness (not as dramatically as other CAM-based methods, but still not negligible).
...
FG-CAM\textsuperscript{D} represents FG-CAM with denoising.
...
The experimental results of FG-CAM have been bolded.
...
The A.D. is expressed as $\sum_{i}^{N}\frac{\max(0,y_i^c-o_i^c)}{y_i^c}\times \frac{100}{N}$, and the A.I. is expressed as $\sum_{i}^{N}\frac{sign(y_i^c<o_i^c)}{N}\times 100$
...
\begin{table}
	\centering
	\normalsize
		\begin{tabularx}{\linewidth}{@{\hspace{0em}}l@{\hspace{0.4em}} XXXXXX}
			\toprule
			...
			\hline
			FG-Grad-CAM\textsuperscript{D} & \textbf{18.00} & \textbf{33.85} &
			 \textbf{17.75} & \textbf{33.55} & \textbf{17.90} & \textbf{34.00} \\
			\hline
			...
			\hline
			FG-Score-CAM\textsuperscript{D} & \textbf{19.19} & \textbf{31.70} & \textbf{18.99} & \textbf{32.00} & \textbf{19.24} & \textbf{30.50} \\
			\hline
...
\begin{table*}[t]
	\centering
		\begin{tabular}{lcccccccc}
			\toprule
			&&Grad-CAM&\makecell{FG-Grad-\\CAM}&\makecell{FG-Grad-\\CAM\textsuperscript{D}}&Score-CAM&\makecell{FG-Score-\\CAM}&
			\makecell{FG-Score-\\CAM\textsuperscript{D}}&Layer-CAM\\
			\midrule
...
\begin{table*}
	...
	\small
		\begin{tabular}{lccccccc}
			\toprule
			&Grad-CAM&FG-Grad-CAM&FG-Grad-CAM\textsuperscript{D}&Score-CAM&FG-Score-CAM&FG-Score-CAM\textsuperscript{D}&Layer-CAM\\
			\midrule
...
\begin{table*}[t]
	...
	\begin{tabular}{lccccccc}
		\toprule
		&LRP&CLRP&SGLRP&FG-Grad-CAM&\makecell{FG-Grad-CAM\textsuperscript{D}}&FG-Score-CAM&\makecell{FG-Score-CAM\textsuperscript{D}}\\
		\midrule
		A.D.&25.48&30.64&40.62&\textbf{16.81}&\textbf{16.13}&\textbf{20.28}&\textbf{16.84}\\
		\hline
		A.I.&25.00&24.15&21.75&\textbf{36.45}&\textbf{36.55}&\textbf{30.15}&\textbf{31.80}\\
		\bottomrule
	\end{tabular}
...
\subsection{Comparison with LRP and Its Variants}
...
\begin{table*}[!h]
	...
	\begin{tabular}{lccccccc}
		...
		&LRP&CLRP&SGLRP&FG-Grad-CAM&\makecell{FG-Grad-CAM\textsuperscript{D}}&FG-Score-CAM&\makecell{FG-Score-CAM\textsuperscript{D}}\\
		\midrule
		Proportion&0.48385&0.59515&0.52136&\textbf{0.49503}&\textbf{0.65496}&\textbf{0.54000}&\textbf{0.66983}\\
		\bottomrule
	\end{tabular}
...
FG-CAM greatly outperforms CAM-based methods in both shallow and intermediate layers, and outperforms LRP and its variants in the input layer significantly.
