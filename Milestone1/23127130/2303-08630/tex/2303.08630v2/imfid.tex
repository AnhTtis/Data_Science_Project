...
\newtheorem*{asmp}{Model Assumptions}
...
\begin{equation}
\label{eq:eta}
R(x,\theta) = \frac{L_x(\theta)}{\sup_{\vartheta \in \TT} L_x(\vartheta)}, \quad \theta \in \TT. 
\end{equation}
Next, define the {\em possibility contour} 
\begin{equation}
\label{eq:contour}
\pi_x(\theta) = \prob_\theta\{ R(X,\theta) \leq R(x,\theta)\}, \quad \theta \in \TT. 
\end{equation}
...
The relative likelihood function $R(x,\theta)$ in \eqref{eq:eta} depends on $(\theta^{-1} \circ g, u)$ only, where $t(x)=(g,u)$.  In particular, if $t(x) = (g,u)$, then 
\[ R(x,\theta) = d_u \, f(\theta^{-1} \circ g, u), \]
where $d_u$ is a constant that depends only on $u$.  
\end{prop}

\begin{proof}
Since $x \mapsto t(x)$ is a bijection, by \eqref{eq:GU.joint} we have 
\[ R(x,\theta) = \frac{p_\theta(g,u)}{\sup_{\vartheta \in \TT} p_\vartheta(g,u)} = \frac{f(\theta^{-1} \circ g, u)}{\sup_{\vartheta \in \TT} f(\vartheta^{-1} \circ g, u)}. \]
...
& = \{2\pi I_0(\kappa u)\}^{-1} \exp\{\kappa u \cos(g - \theta)\} \, p(u),
\end{align*}
...
\[ q_x(\theta) \propto \exp\{\kappa u \cos(g - \theta)\}, \quad \theta \in (0,2\pi], \]
