% "VERSION 1: last correction: 14. März 2023 by PK"
% 
\tolerance = 10000
% final prl-format:
...
\begin{document}
	...
	1970s  and 1980s both theoretically \cite{Lee73,Lee74,Rice75,Rice76,Rice78}  and experimentally \cite{Gruener88,Gruener94}. 
	For temperatures below a critical temperature $T_c$, CDW-materials exhibit
	...
	Recent progress in optical-pump terahertz-probe spectroscopy \cite{Schaefer10,Schaefer14,Thomson17,Thomson23}
	...
	in the literature. In this work, we will address another relevant question which has not been completely settled: what is the effect of long-range Coulomb interactions on the spectrum of collective modes in an incommensurate CDW? Older publications addressing this problem are partially contradictory \cite{Lee78,Wong87,Virosztek93}. In particular,  Virosztek and Maki \cite{Virosztek93} found 
	that the amplitude modes are not affected by the Coulomb interaction, while the phase mode splits at finite temperature into an optical and an acoustic branch; at zero temperature  $T=0$ only the optical branch survives so that at $T=0$ Coulomb interactions destroy gapless phase fluctuations.
	In this work, we will critically re-examine this result and show
	...
	in a crystal) is essential to protect the gapless nature of the lowest phase mode. 
	
	The rest of this paper is organized as follows. In Sec.~\ref{sec:models} we introduce two model Hamiltonians describing  incommensurate charge-density waves in quasi-one-dimensional conductors: the multi-phonon Fr\"{o}hlich Hamiltonian and its extension including long-range Coulomb interactions. In Sec.~\ref{sec:MF} we recapitulate the mean-field theory for the CDW state. In the following Sec.~IV we compute the collective modes in the CDW state without long-range Coulomb interactions using an effective action approach for the Fr\"{o}hlich Hamiltonian within the Gaussian approximation (which is equivalent to the random-phase approximation). Then, in Sec.~V we  derive the dispersions of collective modes including long-range Coulomb interactions and show that the lowest phase mode remains gapless. In Sec.~VI we compare our predictions for the temperature dependence  of the collective modes with experimental results for K$_{0.3}$MoO$_3$  \cite{Schaefer10,Schaefer14,Thomson17,Thomson23}. The concluding Sec.~VII gives a summary of your main results and an outlook. Additional technical details of the calculations presented in this work are given in two appendices.
	...
	CDW state in quasi-one-dimensional conductors such as K$_{0.3}$MoO$_3$.	We also point out some subtleties related to phonon renormalization and the screening	of the Coulomb interactions which will play an important role in the calculation of the collective modes in the CDW state.
	
	
	\subsection{Multi-phonon Fr\"{o}hlich model}
	
	The established  minimal model describing the CDW instability in conductors	is the Fr\"{o}hlich Hamiltonian \cite{Froehlich52}
	\begin{eqnarray}
	...
	$b_{\bd{q} \lambda}$ annihilates a phonon of type $\lambda$ with momentum $\bd{q}$ and energy $\omega_{\bd{q} \lambda}$.  Assuming that the Fermi surface can be approximated by two parallel flat sheets at $ \pm \bd{k}_F = \pm k_F \bd{e}_x$, where $\bd{e}_x$ is a unit vector 
	...
	In the second line of Eq.~(\ref{eq:Froehlich})
	...
	so that to leading order  long-wavelength phonons with $ | \bd{q} | \ll k_F$
	...
	$\gamma_{\bd{q} \lambda}$ in the Fr\"{o}hlich Hamiltonian (\ref{eq:Froehlich}) would 
	lead to a lattice instability \cite{Froehlich52} which has recently been discussed by
	serveral authors \cite{Kumar08,Murakami14,Ohgoe17,Esterlis18,Esterlis19,Chubukov20,Wang20,Sadovskii21,Cichutek22}. In this work we eliminate  
	possible instabilities competing with the CDW 
	by assuming that $\gamma_{\bd{q} \lambda}$ is only finite for momenta $\bd{q}$
	...
	supplement the Fr\"{o}hlich Hamiltonian  (\ref{eq:Froehlich})  by the quantized interaction energy 
	...
	\begin{eqnarray}
		...
		\label{eq:FroehlichC}
	\end{eqnarray}
	where $f_{\bd{q}}$ is the Fourier transform of the Coulomb interaction. 	For a quasi-one-dimensional system of coupled chains with transverse lattice spacing $a_{\bot}$ this is \cite{Kopietz95,Kopietz97}
	\begin{equation}
	...
	\end{equation} 
	in Eq.~(\ref{eq:FroehlichC}) represents the Fourier components of the 
	total density consisting of the sum of the electronic density
	\begin{equation}
	...
	\end{equation}
	The coupling  between longitudinal phonons and the
	fluctuations of the ionic density is for small momenta of the form
	\begin{equation}
		\alpha_{\bd{q} \lambda} 
		= \alpha_{\lambda} | \bd{q} |,
		\label{eq:alphadef}
	...
	only one longitudinal acoustic phonon, so that in this case, we may omit the flavor label $\lambda$. The corresponding coupling $\alpha_{\bd{q} }$  can in this case be
	...
	for the calculation of the dispersions of the collective modes of CDW systems in the presence of Coulomb interactions, it is crucial to retain also the ion-ion interaction
	in Eq.~(\ref{eq:HCoulomb}). 
To give an  intuitive  argument for the importance 
of the ion-ion interaction consider for simplicity a  
lattice supporting only a single longitudinal acoustic
	...
	At the first sight Eq.~(\ref{eq:cbii}) seems to suggest that long-range Coulomb
	...
	the ionic charge is screened by the electrons so that acoustic phonons have the squared dispersion $ \Omega_i^2 / ( 1 + \kappa^2 / q^2 )$, where $\kappa$ is the Thomas-Fermi screening wavevector. For $ q \ll \kappa$ we thus recover the  linear dispersion of acoustic phonons with velocity $ \Omega_i / \kappa$,  which is the well-known  Bohm-Staver 
	relation \cite{Bohm51,Ashcroft76}.
	From the above considerations, it should be clear that by simply dropping the ionic contribution to the Coulomb interaction in Eq.~(\ref{eq:HCoulomb}) we violate the balance between electronic and ionic charge fluctuations and therefore cannot properly describe screening effects.
	To calculate the effect of Coulomb interactions on the collective modes in CDW systems it is,
	therefore, crucial to retain also the ionic contribution to the Coulomb 	Hamiltonian (\ref{eq:HCoulomb}). In a different context, the importance of this contribution has been noted previously 
in Refs.~[\onlinecite{Kopietz96a,Kopietz96b}].
	...
	by its expectation value describing  a CDW with wave-vector $2 \bd{k}_F$,
	\begin{equation}
		X_{Q \lambda} \rightarrow  X^0_{Q \lambda} = \beta  {\cal{V}}  \delta_{ \nu , 0}   [   
		\delta_{\bd{q} ,  \bd{Q} }  {X}_{ \bd{Q} \lambda}^0 
		+   \delta_{\bd{q} , -  \bd{Q} }  {X}^0_{- \bd{Q} \lambda} ],
	\end{equation}
	where $\bd{Q} = 2 \bd{k}_F$ is the ordering momentum of the CDW.
	With the notation
	\begin{equation}
		\Delta = \sum_{\lambda} \gamma_{ \bd{Q} \lambda} X^0_{\bd{Q} \lambda},
		\; \; \; 
		\Delta^{\ast} = \sum_{\lambda} \gamma_{ -\bd{Q} \lambda} X^0_{- \bd{Q} \lambda}
	\end{equation}
	...
	\begin{eqnarray}
		...
		\nonumber
		\\
		& + &   \Delta \bar{c}_{K +  \bd{Q} \sigma} 
		c_{K \sigma} +   \Delta^{\ast} \bar{c}_{K -  \bd{Q} \sigma} 
		c_{K \sigma}  \Bigr] 
		\nonumber
		\\
		& + &
		\beta {\cal{V}} \sum_{\lambda}  \omega_{ \bd{Q} \lambda}^2 
		X^0_{ - \bd{Q} \lambda} X^0_{ \bd{Q} \lambda}.
		\label{eq:SF0}
	\end{eqnarray}
	This is the action 
	...
	periodic potential 
	$
	U ( \bd{r} ) =  \Delta  e^{i \bd{Q} \cdot \bd{r}}
	+ \Delta^{\ast} e^{- i \bd{Q} \cdot \bd{r}}$.
	...
	With  $\bd{Q} = 2 k_F  \bd{e}_x$ the first Brillouin zone is  $ | k_x | \leq k_F$.
	...
	\begin{widetext}
		\begin{eqnarray}
			...
			\left(  \bar{c}_{ K + \frac{\bd{Q}}{2} \sigma } ,  \bar{c}_{ K - \frac{\bd{Q}}{2} \sigma } 
			\right)
			\left(  \begin{array}{cc} \epsilon_{\bd{k} + \frac{\bd{Q}}{2} }  - \mu  - i \omega  &  \Delta \\
				\Delta^{\ast} &   \epsilon_{\bd{k} - \frac{\bd{Q}}{2} }   - \mu  - i \omega \end{array} \right)
			\left( \begin{array}{c}  c_{ K + \frac{\bd{Q}}{2} \sigma } \\
				c_{K - \frac{\bd{Q}}{2} \sigma } \end{array} \right)
			+ \beta {\cal{V}} \sum_{\lambda} \omega_{\bd{Q} \lambda}^2 X^0_{- \bd{Q} \lambda} 
			X^0_{\bd{Q} \lambda},
			\nonumber
			\\
			& &
	...
	$ K \pm \bd{Q} /2 = ( i \omega , \bd{k} \pm \bd{Q} / {2}) $.
	The quadratic form in Eq.~(\ref{eq:SFMF})
	can be diagonalized with a canonical  transformation to new fermionic fields $d^+_{K  \sigma }$ and
	$d^-_{K  \sigma  }$,
	\begin{equation}
		\left( \begin{array}{c}  c_{ K + \frac{\bd{Q}}{2} \sigma } \\
			c_{K - \frac{\bd{Q}}{2} \sigma } \end{array} \right) =
	...
	\begin{eqnarray}
		...
		\; \; \; 
		\xi_{\bd{k}}  =  \frac{ \epsilon_{\bd{k} + \frac{\bd{Q}}{2} } -    \epsilon_{\bd{k} - \frac{\bd{Q}}{2} }}{2}.
	\end{eqnarray}
	...
	\begin{eqnarray}
		...
		\omega_{\bd{Q} \lambda}^2 X^0_{- \bd{Q} \lambda} X^0_{\bd{Q} \lambda} ,	\end{eqnarray}
	where
	\begin{equation}
		E_{\bd{k}}^{ \pm} =   \pm E_{\bd{k}} 
		+   \frac{ \epsilon_{\bd{k} + \frac{\bd{Q}}{2} } +    \epsilon_{\bd{k} - \frac{\bd{Q}}{2} }}{2}.
	\end{equation}
	...
	\begin{eqnarray}
		...
		\nonumber
		\\
		& +  &   {\cal{V}} \sum_{\lambda} \omega_{\bd{Q} \lambda}^2  
		X^0_{- \bd{Q} \lambda} X^0_{\bd{Q} \lambda} ,
	\end{eqnarray}
	where  
	\begin{equation}
	...
	with respect  $X^0_{ - \bd{Q}}$ we obtain the self-consistency  conditions
	\begin{equation}
		X^0_{\bd{Q} \lambda}  =  - \frac{ \gamma^{\ast}_{\bd{Q} \lambda}}{
			\omega^2_{\bd{Q} \lambda} }\frac{ s}{{\cal{V}}} \sum_{\bd{k} \alpha \lambda^{\prime}} 
		\frac{ \alpha}{ e^{\beta   ( E^{\alpha}_{\bd{k}} - \mu )  }+1}
		\frac{ \gamma_{\bd{Q} \lambda^{\prime}} X^0_{\bd{Q} \lambda^{\prime}} }{2 E_{\bd{k}}}.
		\label{eq:selfconXs}
	\end{equation}
	Keeping in mind that we consider electronically quasi-one-dimensional systems we may expand the electronic energy dispersion to linear order around the Fermi momentum, $\epsilon_{ \bd{k} \pm \frac{\bd{Q}}{2} } =	\epsilon_{ \bd{k} \pm \bd{k}_F } \approx   \epsilon_{\bd{k}_F} \pm  v_F k_x $, so that
	\begin{equation} 
	...
	\end{equation}
	Defining 
	$\Delta_{\lambda} = \gamma_{\bd{Q} \lambda} X^0_{\bd{Q} \lambda }$ the self-consistency equation (\ref{eq:selfconXs}) can then be written as
	\begin{equation}
	...
	\begin{eqnarray}
		d_{\lambda} &  = &  -  \frac{ | \gamma_{\bd{Q} \lambda} |^2 }{
			\omega_{\bd{Q} \lambda}^2 } 
		\frac{ s }{\cal{V}} \sum_{\bd{k} \alpha}
		\frac{ \alpha}{ e^{\beta ( E_{\bd{k}}^{\alpha} - \mu )}+1} 
		\frac{ 1}{ 2 E_{\bd{k}}}
		\nonumber
		\\
		& = & \frac{ | \gamma_{\bd{Q} \lambda} |^2 }{
			\omega_{\bd{Q} \lambda}^2 } 
	...
	\begin{equation}
		1 =   \sum_{\lambda} d_{\lambda} =   \left[ \sum_{\lambda} \frac{ | \gamma_{\bd{Q} \lambda} |^2 }{
			\omega_{\bd{Q} \lambda}^2 } \right]
		\frac{ s }{\cal{V}} \sum_{\bd{k} } 
		\frac{ \tanh ( \beta E_{\bd{k}} / 2 ) }{ 2 E_{\bd{k}}}.
		\label{eq:selfcon}
	\end{equation}
	...
	\end{equation}
	where $\omega_c$ is an ultraviolet cutoff \cite{Fetter71,Mahan10,Khomskii10} typically taken of the order of the bandwidth \cite{Rice75,Gruener88} and
	we have introduced the dimensionless electron-phonon coupling
	\begin{equation}
		g_0 =  \left[ \sum_{\lambda} \frac{ | \gamma_{\bd{Q} \lambda} |^2 }{
			\omega_{\bd{Q} \lambda}^2 } \right] \frac{\nu_3 }{2}.
		\label{eq:g0def}
	\end{equation} 
	Here
	\begin{equation}
...
on blue bronze.}
	...
	\end{equation}
	Separating the mean-field part of the fluctuating gap,
	$\Delta_Q = \Delta_Q^0 + V_Q$, where $\Delta^0_Q = 
	\delta_{Q,0} \Delta = 
	\beta {\cal{V}} \delta_{\nu,0}
	\delta_{\bd{q},0} \Delta$,
	we obtain within Gaussian approximation
	\begin{eqnarray}
		& &  - s{\rm Tr} \ln [ \mathbf{G}_0^{-1} - \mathbf{\Delta } ] =
	...
	in the Fr\"{o}hlich Hamiltonian is only finite for momenta close to
	$\pm 2 \bd{k}_F$.
	To explicitly separate the long-wavelength phonon displacements with momenta $ | \bd{q} | \ll k_F$
	from the  short-wavelength displacements with momenta close to $ \pm 2 \bd{k}_F$, we set
	\begin{subequations}
		\label{eq:UVlow}
		\begin{eqnarray}
	...
	\begin{widetext}
		...
		\end{subequations}
	\end{widetext}
	For simplicity, let us choose the mean-field order parameter $\Delta$ to be real. 
	It is then convenient to parametrize the complex field $V_{Q}$ in terms of two real fields
	...
	$\bd{q}$ so that $V_{{Q}} = \sum_{\lambda} \gamma_{\lambda} V_{Q \lambda}$.
	Similarly, we expand $A_Q = \sum_{\lambda} \gamma_{\lambda} A_{Q \lambda}$ and
	...
	& & 	\bar{D}_0^{-1} ( 2 k_F , \omega  )-\bar{D}_0^{-1} ( 2 k_F , 0 ) \nonumber
 	...
 	\label{eq:ampliphon}
	\end{eqnarray}
	Anticipating that for K$_{0.3}$MoO$_3$ the solutions of this equation are adiabatically related to the  bare phonon frequencies $\omega_{\lambda}$, we may approximate the  inverse phonon propagator in Eq.~(\ref{eq:ampliphon}) in the regime  $ | \omega - \omega_{\lambda} | \ll |\omega_{  \lambda}-\omega_{  \lambda\pm1}|$ by
	\begin{equation}
	...
	\end{equation}
	which is sufficient to obtain the dispersion of the lowest phase mode for  
	...
	Eq.~(\ref{eq:g0def}).
	Our result  (\ref{eq:omegaAres}) for the squared frequency of the amplitude mode then reduces to
	\begin{equation}
...
in units of the squared Fermi velocity $v_F^2$  as a function of the
	...
	by Virosztek and Maki \cite{Virosztek93}, is essential to correctly describe the effect of Coulomb interactions on the phase modes in a CDW. 
 	...
 	Note that  pure Coulomb interactions in one-dimensional electronic systems  are generally non-perturbative for low energies due to Luttinger liquid physics  \cite{Giamarchi_Book}, albeit a growing body of recent evidences 
indicates that the formation of a charge-density wave  
in these systems is a generic phenomenon that remains robust even when high-energy states in the  whole energy band are taken into account~\cite{Tsyplyatyev15,Jin19,Vianez22,Tsyplyatyev22}. Nevertheless, the use of the Gaussian approximation is justified in our calculation by the effect of
the electron-phonon interaction which triggers the 
formation of the correlated CDW state. At low temperatures  
the inverse of the large value of the  order parameter $\Delta$ 
in Eq.~\eqref{Delta_order_parameter}  
provides  a small parameter justifying the perturbative treatment of 
the electron-electron interaction even in one dimension. We will return to this point in Sec.~\ref{sec:experiment} where
we compare the results of this section with the experimental data. 
	
 	Following the procedure outlined in Sec.~\ref{sec:modesF} we may now integrate over the
	fermions in two-band approximation to obtain the effective action of the	phonons and the Coulomb field,
	\begin{eqnarray}
	...
	The next few steps are analogous to the manipulations
	...
	\begin{equation}
		...
		\frac{ \alpha^2_{ \bd{q} \lambda}}{ \nu^2 + \omega^2_{\bd{q} \lambda} }	\end{equation}
	...
	After analytic continuation to real frequencies the second term in the square braces 
 ...
 \label{eq:Omegaionren}
	...
	\noindent In the weak coupling regime where  $2\Delta \ll \Omega_e$, which is the relevant regime for blue bronze \cite{Ando05,Sing99}, the energy of the upper branch for $\bd{q}=0$ is  given by
	...
	\end{equation}
	Using the Bohm-Staver
	relation \cite{Bohm51,Ashcroft76}
	\begin{equation}
		v_s = \sqrt{ \frac{ z m}{3M}} v_F
	\end{equation}
	to express the sound velocity  
	in terms of the Fermi velocity, we 
	obtain in the adiabatic regime and for weak electron-phonon coupling
...
%	
 ...
 \label{sec:experiment}
	...
	The multi-phonon Fr\"{o}hlich Hamiltonian ${\cal{H}}_F$ defined in Eq.~(\ref{eq:Froehlich}) with $\lambda_{\rm max}$ phonons depends on $2\lambda_{\rm max}$ free parameters, the phonon frequencies $\omega_{ 2 \bd{k}_F  \lambda}$ and the electron-phonon couplings $\gamma_{ \lambda}$. In principle, these parameters can be determined by measuring the energies of the $\lambda_{\rm max}$ amplitude modes and the associated $\lambda_{\rm max}$ phase modes at some fixed momentum $\bd{q}$. Unfortunately, such a measurement is not available at this point. Moreover, in contrast to our results for the  amplitude modes, our experimental data for the phase modes exhibit an unexplained temperature dependence which prevents us from fitting our theoretical results to the raw data for the  $2\lambda_{\rm max}$ collective modes. Instead, we fix the $ 2 \lambda_{\rm max}$ free parameters of our multi-phonon Fr\"{o}hlich model using the frequencies $\omega_{\lambda} \equiv \omega_{ 2 \bd{k}_F \lambda}$ of the bare phonon modes from a measurement at $T>T_c$ and fitting the measured  amplitude modes at $T=0$, which can then be used to fix the squared electron-phonon couplings $\gamma_\lambda^2$. Remarkably, if we neglect the Coulomb interaction 
none of the subsequent findings are sensitive to the precise value of the gap $\Delta_0\equiv\Delta ( T=0)$ even though $\Delta_0$ is an additional free parameter and must be specified for rescaling purposes. This is because the solutions to the mode equations~\eqref{eq:eigenA2} 
and \eqref{eq:eigenB2} are of the order of the phonon frequencies
$\omega_{ 2 \bd{k}_F \lambda}$, which means that our calculation of the
 mode energies is  not only controlled by the dimensionless electron-phonon 
couplings $g_{0,\lambda}=\gamma_{ \lambda}^2\nu_3/2\omega_{ 2 \bd{k}_F  \lambda}^2$, but also by  the small parameter $\omega/2\Delta_0\sim\omega_{ 2 \bd{k}_F \lambda}/2\Delta_0$. Hence, the auxiliary function $F (0, \omega)$ in Eq.~\eqref{eq:deff} can be evaluated for small $\omega$ and, therefore, all corrections which explicitly depend on $\Delta_0$ 
are at least of order  $\omega^2/2\Delta_0^2 \ll 1$.

	\begin{figure}
		\centering
		\includegraphics[width=1\linewidth]{fig6.pdf}
		\caption{Lowest three amplitude modes of K$_{0.3}$MoO$_3$. The black dots are the measurements from  Refs.~\cite{Schaefer10,Schaefer14}, reanalyzed in \cite{Thomson23}, while the solid lines are the predictions of the amplitude mode equation Eq.~\eqref{eq:eigenA2} for $T>0$, where we matched the coupling constants to the $T=0$ values taking the phonon frequencies from Refs.~\cite{Schaefer10,Schaefer14,Thomson17}, see Appendix B for the explicit values.}
		\label{fig:ExAmplidude}
	\end{figure}
	
	In this work, we retain only the lowest three phase and amplitude modes of K$_{0.3}$MoO$_3$. The frequencies of bare phonon modes for $T > T_c$ are taken from Refs.~\cite{Schaefer10,Schaefer14,Thomson17}, the frequencies of amplitude modes from \cite{Schaefer10,Schaefer14,Thomson17,Thomson23}, and the value of the CDW gap  $2\Delta_0 \approx 100 $ meV is estimated from angle-resolved photoemission measurements \cite{Ando05}, which fixes all the electron-phonon couplings $\gamma_\lambda^2$ for the three relevant phonon modes.  The numerical values of our fit parameters $\omega_{\lambda}$ and $\gamma_{\lambda}^2$ are given in Appendix~B. Then we solve the mean-field gap equation~\eqref{eq:selfcon} to obtain  the temperature-dependent gap $\Delta(T)$. Cross-checking with Refs.~\cite{Sato83,Girault89} reveals that $\Delta(T)$ can be approximated on the mean-field level. Using $\Delta(T)$ as an input for the numerical solution of the function $F(0,\omega )$ defined in Eq.~\eqref{eq:deffnum} enables us to solve for the roots of the mode-frequency equations~\eqref{eq:eigenA2} and \eqref{eq:eigenB2} in the absence of Coulomb interactions. Our results are presented in Figs.~\ref{fig:ExAmplidude} and~\ref{fig:ExPhase} where we plot the lowest three temperature-dependent amplitude and phase modes for temperatures up to $T < 0.98 T_c$. Our procedure reproduces the data for the amplitude modes from Refs.~\cite{Schaefer10,Schaefer14,Thomson17,Thomson23} as long as $T$ is not too close to $T_c$, where thermal fluctuations become important and the order parameter $\Delta(T)$ becomes small so that the Gaussian approximation used in this work is expected to break down. In this regime higher-order processes need to be taken into account, like for example in the theory of superconductivity for temperatures close to  $T_c$ \cite{Varlamov_book}.
	
	The experimental data represented as black dots in Figs.~\ref{fig:ExAmplidude} and~\ref{fig:ExPhase} were taken from  Refs.~\cite{Schaefer10,Schaefer14,Thomson17,Thomson23}, and from Ref.~\cite{Degiorgi91} for the lowest phase mode shown in Fig.~\ref{fig:ExPhase}. With the exception of the lowest  phase mode, our theoretical results for the Fr\"{o}hlich model  in Eqs.~(\ref{eq:eigenA2}) and (\ref{eq:eigenB2})  (neglecting long-range Coulomb interactions) reproduce the experimentally-observed temperature dependence of all modes within the experimental uncertainties. However, a closer look of our finite-temperature results for the phase modes (see Appendix~B) reveals a discrepancy in the magnitude of the temperature dependence: while our solution of Eq.~\eqref{eq:eigenB2} predicts a  $\mathcal{O}(10^{-4})$ THz variation in the temperature dependence of the finite frequency phase modes (see the inset of Fig.~\ref{fig:ExPhase}), the experimentally observed  variation, while being generally weak, is still two orders of magnitude larger, i.e., $\mathcal{O}(10^{-2})$ THz for the finite frequency phase modes shown in Fig. \ref{fig:ExPhase}. 
	%	
	\begin{figure}[tb]
		\centering
		\includegraphics[width=1\linewidth]{fig7.pdf}
		\caption{Temperature dependence of the lowest three phase modes of K$_{0.3}$MoO$_3$. The black dots are data points from Ref.~[\onlinecite{Thomson17, Thomson23,Degiorgi91}], while the solid lines are the predictions of the mode equation Eq.~\eqref{eq:eigenB2}, taking the fitted $\gamma_{\lambda}^2$ and $\omega_{ 2 \bd{k}_F \lambda}$ from Appendix B. We have fixed the lowest frequency mode to its pinning frequency following Ref.~\cite{Degiorgi91}, since the measurements of \cite{Thomson17,Thomson23} cannot reliably resolve frequencies $\omega 
			\leq 1$ THz. The dashed grey line indicates the controlled temperature behavior of the phase-plasmon-hybrid for $\Omega_e\approx 1.5$ eV and $\Omega_i^2/\Omega_e^2\approx 1/2000$ predicted by Eq.~\eqref{eq:SeffABCB} up to a certain temperature $T_*$ estimated in Eq.~(\ref{eq:P1exp}), while the dotted grey line indicates its trend for $T \gtrsim T_*$ where our approximations break down.
The inset shows the relative change $[\omega(T) -\omega(0)]/\omega (0)$ in percent $[\%]$ of the highest frequency phase mode predicted without Coulomb interactions by Eq.~\eqref{eq:eigenB2}.}
		\label{fig:ExPhase}
	\end{figure}
	%
A possible explanation for this discrepancy between theory and experiment is that the Gaussian approximation made in deriving Eq.\ \eqref{eq:eigenB2} is not sufficient to explain the temperature dependence of the phase modes. Note that the Gaussian approximation (which is equivalent to the random-phase approximation) retains only the lowest order in the electron-phonon coupling by including only the contribution of a  single electron-bubble to  the phonon self-energy which is therefore  quadratic in the dimensionless electron-phonon coupling $\sqrt{g_0}$ defined in Eq.~\eqref{eq:gnull}. To this order, there is no mixing between the amplitude and phase modes, see Eq.\ (\ref{eq:SeffAB}). However, the next-order contribution in the collective fields which is quartic in $\sqrt{g_0}$, already introduces such a mixing that would lead to a stronger temperature variation of the phase modes. Since in K$_{0.3}$MoO$_3$ the dimensionless electron-phonon coupling  $g_0\sim 0.3$ is small, the effect of such mixing can be estimated as $g_0^2 \delta\omega_{A}\sim 0.01$ THz, where we used $\delta \omega_{A} \sim 0.1$ THz  as the variation of the higher frequency amplitude modes in Fig.~\ref{fig:ExAmplidude}. This estimate matches the experimentally observed order of magnitude of the phase mode variation. Hence, a systematic calculation of this effect for the phase modes can be done by considering higher order terms up to the fourth order of the expansion \eqref{eq:expansion1}, posing a route for further improving the accuracy of the theory presented in this work.

While the experimental data for the amplitude modes and the higher-frequency phase modes can be
fitted by our  results for the  multi-phonon Fr\"{o}hlich Hamiltonian neglecting Coulomb interactions, the
behavior of the lowest phase mode remains somewhat mysterious. In fact, in experiments this mode so far has not been unambiguously confirmed;
for example, with  the detectors used 
in Ref.~\cite{Thomson17,Thomson23} any mode with frequency of the order of  $1$~THz or lower cannot be reliably detected.
Given the fact that according to our analysis the lowest phase mode remains gapless in the presence of
long-range Coulomb interactions, the physical mechanism inducing a gap in the  
lowest phase mode, seen in an earlier experiment by
Degiorgi {\it{et al.}}~\cite{Degiorgi91}, remains unclear.
One possible explanation is 
pinning by impurities, which are not included in our model.
Another possibility is that the gapped lowest phase mode detected
in the experimental data reproduced in  Fig.~\ref{fig:ExPhase} is actually the upper (gapful) branch of the 
hybrid phason-plasmon mode due to  the Coulomb interaction discussed in Sec.~\ref{sec:Coulomb}. 
To further examine this possibility, we have solved the equation (\ref{eq:hybridpp}) for the collective 
phase modes in the CDW including  the effect of  long-range Coulomb interactions.  
The collective modes now depend on two additional parameters which can be chosen to be the electronic plasma frequency $\Omega_e^2 = 4 \pi  e^2 n /m$ and the  long-wavelength electron-phonon couplings $\alpha_{\bd{q} \lambda} = \alpha_{\lambda} | \bd{q} |$ defined in Eq.~(\ref{eq:alphadef}). For simplicity, we work here with a minimal model where the fluctuations of the ionic density are described by a single acoustic phonon. Given the fact that hybridization with other phonons can change the actual value of the electron-phonon coupling,  we use the square of the ionic plasma frequency $f_{\bd{q}} \alpha^2_{\bd{q}}=\Omega_i^2$ as an additional fit parameter which can be obtained from experiments. With $\Omega_e \approx 1.5$ eV and $\Omega_i^2/\Omega_e^2\approx 1/2000$ we obtain the lowest hybrid phason-plasmon mode indicated by the dashed gray line in Fig.~\ref{fig:ExPhase}.
At the same time, the amplitude modes are not affected by the Coulomb interaction within our approximation and the
finite frequency phase modes are only weakly renormalized by the self-energy correction \eqref{eq:Pi1} due to Coulomb interactions. 

%Lastly, the inclusion of Coulomb interactions raises the importance of the precise value of the CDW gap $\Delta$, because the hybrid plasmon scales $\omega_{+} \propto \Delta$ according to Eq.~\eqref{eq:omegaplusdelta}. 

	
Obviously, the lowest hybrid phason-plasmon mode with frequency $\omega_{+} (0)$ 
predicted by our theory has not been seen in the experiment \cite{Thomson17,Thomson23}. A possible explanation is that
the experimental detector used in Ref.~\cite{Thomson17,Thomson23} is not sensitive in  this frequency range. Furthermore,
the temperature dependence of this mode, shown by the gray line in Fig.~\ref{fig:ExPhase}, exhibits a 
strong upturn for $T \gtrsim T_{\ast}$. In this regime
the polarization function $\Pi^{\phi\phi}_0(Q)$ defined in Eq.~\eqref{eq:Piphiphiq2} (which reduces to the usual Lindhard function \cite{Fetter71,Mihaila11}  in the normal metallic state)  dominates the temperature-dependence of the hybrid mode.
Critically, for $T>T_*$ this would lead to hybridization and avoided level crossings between the lowest phason-plasmon mode and the upper phase modes. The latter behavior is not supported by the measurements in 
Ref.~\cite{Thomson17,Thomson23}. Hence, we indicate this behavior with a dotted line in Fig.~\ref{fig:ExPhase}  only  for a small temperature range above $T_*$. Clearly, the term responsible for this behavior was already present in the calculations including long-range Coulomb interactions by Visosztek and Maki \cite{Virosztek93}, yet because they neglect the ionic response it plays a different role there.
	
	One possible explanation for the above discrepancy between theory and experiment is the breakdown of the Gaussian approximation (which is equivalent to the random-phase approximation)  to properly describe the formation of 
charge-density waves  in the presence of  Coulomb interactions for $T>T_*$, where is temperature $T_{\ast}$ is estimated in Appendix~B. Since the Luttinger liquid physics is generally expected to become important in the electronic one-dimensional systems with Coulomb interaction~\cite{Giamarchi_Book}, higher-order interaction processes neglected in Gaussian approximation are expected to stabilize the CDW state~\cite{Anderson70,Dzyaloshinskii74,Solyom79} in the temperature range $T>T_*$, and suppress the Fermi liquid physics  including metallic screening described by the temperature dependence of polarization function $\Pi^{\phi\phi}_0(Q)$ for $T>T_*$. We therefore  suggest that the proper  theory of collective modes in the presence of long-range Coulomb interactions for $T>T_*$ would need to include interactions between the collective modes, which is a possible further theoretical development but goes beyond the scope of the present work.
	...
	Comparison with experiments  on K$_{0.3}$MoO$_3$ (blue bronze) \cite{Thomson17,Schaefer14,Schaefer10,Thomson23} shows that the temperature-dependent mode frequencies of multiple amplitude modes are already well described by the multi-phonon Fr\"{o}hlich Hamiltonian in Gaussian approximation derived in Sec.~\ref{sec:modesF}. At this level of approximation, our effective action approach is essentially equivalent to the random-phase approximation adopted a long time ago by Lee, Rice, and Anderson~\cite{Lee73,Lee74,Rice75,Rice76,Rice78}. While this  model is fairly successful in describing the order of magnitude for the higher frequency phase modes, our calculations for the multi-phonon Fr\"{o}hlich model (\ref{eq:Froehlich}) and its generalization (\ref{eq:FroehlichC}) including the long-range Coulomb interaction fail to ascertain the nature of the lowest phase mode detected in Ref.~[\onlinecite{Thomson17}, \onlinecite{Thomson23,Degiorgi91}] and exhibit a temperature dependence far beneath the experimental values. At this point, we can only speculate about the nature of the experimentally observed lowest finite frequency phase  mode and if  a stabilized version of the $T=0$ phason-plasmon hybrid mode predicted by Eq.~\eqref{eq:SeffABCB} exists. We note however, that pinning by impurities would raise the zero-frequency phase mode to finite frequencies.
	...
	\begin{widetext}
		...
		\begin{eqnarray}
			...
			\label{eq:SeffUVcb}
		\end{eqnarray}
		At this point we choose $\Delta$ to be real and 
	...
	In this appendix  we specify the parameters in the multi-phonon Fr\"{o}hlich Hamiltonian (\ref{eq:Froehlich}) and in the Fr\"{o}hlich-Coulomb Hamiltonian (\ref{eq:FroehlichC}) which we have used in Sec.~\ref{sec:experiment} to compare our calculations  with  the experimental data. 
The multi-phonon Fr\"{o}hlich Hamiltonian~\eqref{eq:Froehlich} with $\lambda_{\rm max}$ phonons
depends
on the bare phonon frequencies $\omega_{ 2 \bd{k}_F \lambda}$ and  the electron-phonon 
couplings $\gamma_{\lambda}$, which form a set of $2 \lambda_{\rm max}$ parameters.
 In addition, out Hamiltonian depends also on the ultraviolet  cutoff $\omega_c$ 
in the mean-field gap equation~(\ref{eq:gapequation}); actually, we may
use the gap equation (\ref{eq:gapequation})
to eliminate $\omega_c$ in favor of
the experimentally measured 
zero-temperature gap $2\Delta (T=0) \approx 100$~meV$ \approx  24$ THz. 
...
specified in the caption of Fig.~\ref{fig:ExPhase}.
\vspace{2mm}
\begin{table}[h!]
 \begin{center}
 ...
 \hline
	...
	\end{tabular}
 \end{center}
	\caption{Summary of the values of the parameters of  the multi-phonon Fr\"{o}hlich Hamiltonian (\ref{eq:Froehlich}) and the 
...
dotted line in Fig.~\ref{fig:ExPhase}.
		...
		%
		\bibitem{Tsyplyatyev15}
		O. Tsyplyatyev, A. J. Schofield, Y. Jin, M. Moreno, W. K. Tan, C. J. B. Ford, J. P. Griffiths, I. Farrer, G. A. C. Jones, and D. A. Ritchie,
		Hierarchy of Modes in an Interacting One-Dimensional System,
		Phys. Rev. Lett. {\bf 114}, 196401 (2015).
		%
		...
		%
		\bibitem{Jin19}
		Y. Jin, O. Tsyplyatyev, M. Moreno, A. Anthore, W. K. Tan, J. P. Griffiths, I. Farrer,  and D. A. Ritchie, L. I. Glazman, A. J. Schofield, and C. J. B. Ford,
		Momentum-dependent power law measured in an interacting quantum wire beyond the Luttinger limit,
		Nat. Commun. {\bf 10}, 2821 (2019).
		%
		\bibitem{Vianez22}
		...
		%
		\bibitem{Sing99}
		M. Sing, V. G. Grigoryan, G. Paasch, M. Knupfer, J. Fink, F. Levy, H. Berger, B. Lommel, and W. A{\ss}mus,
		Plasmon Dispersion in Quasi-One-Dimensional (TaSe$_4$)$_2$I and K$_{0.3}$MoO$_3$,
		Synthetic Metals {\bf102} (1999). 
		%
		\bibitem{Sato83}
