...
\begin{enumerate}[label = (T\arabic*)]
	...
	\item \label{def_T2} For every sphere $S$ in $\Sp^3$ transverse to $G$, if $|C(S \cap G)| < n$ then exactly one of the two closed balls $\bar B_1 \in \T$ or $\bar B_2$ is in $\T$, where $\Sp^3 \smallsetminus S = \ens{B_1, B_2}$. 
...
Plugging (\ref{eq_prop_contrad_torus_disjoint_1}) and (\ref{eq_prop_contrad_torus_disjoint_2}) concludes the proof: $2\oper{\sum}{e \in E(\Gamma)}{}{|C(e \cap G)|} < 2 \comprep (G,\Sigma)$.
\end{proof}
