...
The \ac{MV}-solution from Section \ref{sec:peak_lp} involve a free terminal time, multiple histories, and zero running cost $(J=0)$.
...
The time-slack measure is set to $\nu=0$ because of the fixed-terminal-time setting ($t^*=T$). The symbol $\mu_{x_h(\cdot)}$ is the occupation measure of $x_h$:
\begin{align}
    \forall \phi &\in C([-\tau, 0] \times X) & \inp{\phi}{\mu_{x_h(\cdot)}} = \int_{t=-\tau}^{0} \phi(t, x_h(t)) dt.
\end{align}
...
\label{eq:u_cont}
\begin{align}
    d^* = & \ \sup \quad v(0, x_h(0)) +\textstyle \textstyle\int_{-\tau}^{0} \phi(t + \tau_i, x_h(t)) dt\label{eq:u_cont_obj}& \\
...
\begin{align}
    ...
    & \dot{x} =  f(t, x(t - \tau_0), \ldots, x(t - \tau_r), u(t)) & & \forall t \in [0, T] \nonumber\\
    & x(t') = w(t') & & \forall t \in [t-\tau_r, t) \nonumber\\
    & x(t) = z \\
        & x(T) \in X_T \nonumber \\
    & u(t) \in U& & \forall  t \in [0, T]. \nonumber
\end{align}
...
The Hamilton Jacobi-Bellman equation of optimality is \cite{liberzon2011calculus}:
\begin{subequations}
\label{eq:hjb}
\begin{align}
    0 &= J_T(x(T)) - V^*(T, x(T)) \label{eq:hjb_term_true}\\
    0 &= \inf_{u \in U} \left(\dot{V}^*(t, x(t), x_{\tau_r}(\cdot), u) + J(t, x(t), u(t)) \right) & \forall t \in [0, T]. \label{eq:hjb_traj_true}
\end{align}
\end{subequations}
...
Let $\tilde{u} \in \mathcal{U}$ be an arbitrary control policy starting at the initial condition $(t_0, z, w)$, resulting in a trajectory $\tilde{x}(t)$. Denote $\tilde{x}_\tau(\cdot)$ as the history function $\tilde{x}_t(t') = \tilde{x}(t + t') \ \forall t' \in [-\tau_r, 0]$. 
...
The terminal value of the value function is
\begin{equation}
    V(T) = V(t, x(T), x([T-\tau_r, T])) = v(T, x(T)) + \int_{T}^{T} \phi_i(s, x(s-\tau))ds = v(T, x(T)). \\ \label{eq:value_term}
\end{equation}

The objective in \eqref{eq:u_cont_obj} is the evaluation of the value functional at time $t=0$ along the optimal controlled trajectory $x^*(t)$ with
...
\begin{align}
\label{eq:v_dot}
    \dot{V}(t, z, w(\cdot), u) &= \Lie_f v(t, x(t)) + I_{[0, T-\tau]}(t) \phi(t+\tau_i, z) -\phi(t, w(- \tau_i)).
\end{align}
...
The value functional evaluated along the optimal trajectory $V^*(t) = V(t, x^*(t), x^*([t-\tau_r, t])$ is $C^0$-continuous in $t$, but is not necessarily $C^1$-continuous in $t$. At the time $t=T-\tau$, define the value functional evaluations of
\begin{align}
...
The difference in the derivative evaluations for every lag $i^*$ is, 
\begin{align}
...
\begin{align}
    \label{eq:u_recovery}
    u(t) &= \argmin_u  \Lie_f v(t, x(t)) +  I_{[0, T-\tau]}(t) \phi(t+\tau, x(t))* - \phi(t, x(t - \tau_i)) + J(t, x(t), u(t))\\
...
%     & \pi^{t x_i}_\# \bar{\mu} = (t + \tau_i, x)_\# \left( \textstyle\sum_{j=-i}^{r-i}\nu_j  \right) & & \forall i = 0, \ldots, r\label{eq:box_u_meas_delay} \\
...
% \end{align}
% The controllers extracted from \eqref{eq:box_control_law} are polynomial in terms of time $t$, the current state $x(t)$, and delayed states $x(t - \tau_i)$. These are approximate controllers that may not be strictly feasible solutions of SOS constraints.
...
% % For state delays $\tau_i$ and input delays $\tilde{\tau}_k$, dynamics are, 
% % \begin{equation}
% % \label{eq:dynamics_dead_time}
% %     x'(t) = f(t, \cup_{i=0}^r x(t - \tau_i), \cup_{i=k}^{\tilde{r}} u(t - \tilde{\tau}_k))
% % \end{equation}
...
% % The lags $\tau_i$ and $\tilde{\tau}_k$ do not necessarily need to match.

% % % Generated bounds may be tighter if the sequences $\{\tau_i\}$ and $\{\tilde{\tau}_k\}$ are the 
...
% %     \bar{\mu}\left(A \times \prod_{i=0}^r B_i \times  \prod_{k=0}^{\tilde{r}} C_k\mid x_h, u_h\right) = \int_{A} \left(\prod_{i=0}^r I(x(t - \tau_i \mid x_h) \in B_i)\right)\left(\prod_{k=0}^{\tilde{r}} I(u(t - \tilde{\tau}_k \mid u_h) \in C_i)\right) dt.
% % \end{equation}
...
% %     & \pi^{t x_i u_i}_\# \bar{\mu} = S^{\tau_i}_\# \left(\textstyle \sum_{j=-i}^{r-i} \nu_j \right)  & & \forall i = 0, \ldots, r\label{eq:u_dead_s_meas_delay} \\
...
\begin{table}[h]
    \centering
    \caption{\label{tab:linear_bounds}LMI approximation bounds to program \eqref{eq:u_meas}}
