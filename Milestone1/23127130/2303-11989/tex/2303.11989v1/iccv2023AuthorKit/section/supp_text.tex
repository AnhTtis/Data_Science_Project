...
First, our completion stage (see Section~\ref{subsec:Trajectory Generation}) might not be able to inpaint all holes (Figure~\ref{fig:supp-limitation}b).
...
Second, our mesh fusion stage (see Section~\ref{subsec:3D Mesh Generation}) might not remove all stretched-out faces (Figure~\ref{fig:supp-limitation}c).
...
We create the complete scene layout and furniture in the first stage of our tailored two-stage viewpoint selection scheme (see Section~\ref{subsec:Trajectory Generation}).
...
For each camera pose in both stages, we follow an iterative scene generation scheme (see Section~\ref{subsec:Perpetual Generation}).
After generating novel content, we predict its depth in our depth alignment stage (see Section~\ref{subsec:Depth Alignment}).
...
We complete the scene in the second stage of our tailored two-stage viewpoint selection scheme, by filling in remaining holes in the mesh (see Section~\ref{subsec:Trajectory Generation}).
To this end, we first select suitable camera poses that look at these holes (Figure~\ref{fig:supp-mask-dilation}a).
We then follow the iterative scene generation scheme to fill in the holes in the mesh (see Section~\ref{subsec:Perpetual Generation}).
...
To combine the new content with the existing mesh, we apply our triangulation scheme (see Section~\ref{subsec:3D Mesh Generation}).
...
Our two-stage tailored viewpoint selection (see Section~\ref{subsec:Trajectory Generation}) first creates the general scene layout and furniture from predefined trajectories.
We choose these trajectories such that the camera poses do not intersect with generated geometry (see Section~\ref{subsec:Trajectory Generation} for more details).
