...
\subsection{Spatially Varying Scene Generation}
\label{subsec:cont-gen}
\input{iccv2023AuthorKit/tables/fig_ours_mix_appl}
\input{iccv2023AuthorKit/tables/fig_layout_control}
...
This can be desired to avoid repeating elements in a complete scene (e.g., multiple couches could be spread out over the whole room when using the same prompt for every camera).
Instead, users can specify different object positions through different camera poses and text prompts.
...
We show results that combine multiple text prompts in Figure~\ref{fig:ours-mix-appl} and Figure~\ref{fig:layout-control}.
...
We believe this demonstrates an exciting application of our method, that can be further explored in future work.
