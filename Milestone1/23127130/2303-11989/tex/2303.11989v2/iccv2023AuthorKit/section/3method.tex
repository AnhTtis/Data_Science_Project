...
Input to our method is a set of arbitrary text prompts $\{P_t\}_{t=1}^T$ that corresponds to our selected poses $\{E_t\}_{t=1}^T \in \mathbb{R}^{3\times 4}$ in both stages.
...
\begin{equation}
\label{eq:step-3}
    \hat{d}_t = \textit{predict-and-align}(\mathcal{F}_{d}, I_t, d_t, m_t).
\end{equation}
...
We can then extract the aligned depth as $\hat{d} = (\frac{\gamma}{\hat{d}_{p}} + \beta)^{-1}$.
...
We subsequently render \emph{predefined} trajectories in different directions that eventually cover the whole room.
We found generation works best, if each trajectory starts off from a viewpoint with mostly unobserved regions.
This generates the outline of the next chunk, while still being connected to the rest of the scene (e.g., see Figure~\ref{fig:iterative-gen}).
...
We translates camera positions $T_0{\in}\mathbb{R}^3$ along the look-at direction $L{\in}\mathbb{R}^3$ uniformly: $T_{i+1}{=}T_{i}{-}0.3 L$.
We stop if the mean rendered depth is larger than $0.1$ or discard the camera after $10$ steps.
This avoids views too close to existing geometry.
