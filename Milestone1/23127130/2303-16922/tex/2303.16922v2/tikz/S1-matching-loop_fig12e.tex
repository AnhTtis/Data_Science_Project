...
\begin{feynman}
	\vertex (loop1);
    \vertex[left=1cm of loop1] (e1) {$e$};
    \vertex[right=2cm of loop1] (loop2);
    \vertex[right=1cm of loop2] (a1);
    \vertex[right=1cm of a1] (l1) {$\ell$};
    ...
    \diagram*[baseline=(e1.base)] {
      {[edges=fermion]
        (e1) -- (loop1),
        (loop2) -- [quarter right, edge label'=$u$] (loop3) -- [quarter right, edge label'=$u$] (loop1),
        (loop2) -- [edge label=$e$] (a1) -- (l1),
      },
      (loop2) -- [charged scalar,thick, edge label'=$S_1$] (loop1),
      (h1) -- [scalar] (a1),
      (loop3) -- [boson] (w1),
    };
