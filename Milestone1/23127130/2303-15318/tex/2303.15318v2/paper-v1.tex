...
\author{%
    Steven~Dahdah,~\IEEEmembership{Student~Member,~IEEE,}
    James~Richard~Forbes,~\IEEEmembership{Member,~IEEE}%
    \thanks{This work is supported by the Natural Sciences and Engineering
    Research Council of Canada (NSERC) Discovery Grants program, the
    \textit{Institut de valorisation des donn{\'e}es} (IVADO), the Canadian
    Institute for Advanced Research (CIFAR), and the \textit{Centre de
    recherches math{\'e}matiques} (CRM), as well as by Mecademic through the
    Mitacs Accelerate program.}%
    \thanks{S. Dahdah and J. R. Forbes are with the
    Department of Mechanical Engineering, McGill University, Montreal QC
    H3A~0C3, Canada (e-mail:
    \href{mailto:steven.dahdah@mail.mcgill.ca}{steven.dahdah@mail.mcgill.ca};
    \href{mailto:james.richard.forbes@mcgill.ca}{james.richard.forbes@mcgill.ca}).}%
}

\markboth{}{Dahdah and Forbes: Closed-Loop Koopman Operator Approximation}
\IEEEpubid{}
...
\begin{lrbox}{\closedloop}
    ...
    \begin{tikzpicture}
        ...
        \node[
            ...
            \begin{overpic}[width=2.5cm]{./fig_tikz/state_2.pdf}
               \put(-10, -10){\includegraphics[width=2.5cm]{./fig_tikz/state_1.pdf}}
               \put(-20, -20){\includegraphics[width=2.5cm]{./fig_tikz/state_0.pdf}}
            \end{overpic}
        };
        \node[
            below right = 12ex and 0ex of plant.south,
        ] (pic3) {%
            \begin{overpic}[width=2.5cm]{./fig_tikz/posvel_2.pdf}
               \put(-10, -10){\includegraphics[width=2.5cm]{./fig_tikz/posvel_1.pdf}}
               \put(-20, -20){\includegraphics[width=2.5cm]{./fig_tikz/posvel_0.pdf}}
            \end{overpic}
        };
        \node[
            below right = 13.6ex and 0ex of i_bar.south,
        ] (pic1) {%
            \begin{overpic}[width=2.5cm]{./fig_tikz/ref_2.pdf}
               \put(-10, -10){\includegraphics[width=2.5cm]{./fig_tikz/ref_1.pdf}}
               \put(-20, -20){\includegraphics[width=2.5cm]{./fig_tikz/ref_0.pdf}}
            \end{overpic}
        };
        \node[
            above = 0ex of pic1,
...
\begin{abstract}
    ...
    method are demonstrated experimentally using a rotary inverted pendulum
    system.
    %
...
\begin{IEEEkeywords}
    Koopman operator theory, closed-loop systems, system identification, linear
    systems theory, linear matrix inequalities, asymptotic stability,
    regularization.
\end{IEEEkeywords}

\section{Introduction}\label{sec:introduction}

\IEEEPARstart{U}{sing} Koopman operator theory~\cite{koopman_hamiltonian_1931,
...
dynamics of the system~\cite{abraham_active_2019, mamakoukas_local_2019}, while
in others, they are taken from a standard set of basis functions, like
polynomials, sinusoids, or radial basis functions~\cite{bruder_modeling_2019,
abraham_model-based_2017}. Lifting functions can also be chosen to approximate a
...
well~\cite{korda_2018_linear, bruder_modeling_2019, pan_2020_structure}. Once
the chosen lifting functions are applied to the data, linear regression is used
to approximate the Koopman matrix, often with some form of
regularization~\cite{bruder_modeling_2019}.
%
\IEEEpubidadjcol{}% Must be somewhere in fist column
...
methods~\cite{korda_2018_linear, ljung_2019_a}, most notably subspace
identification methods~\cite[\S3]{brunton_2022_modern}.
%
...
controller.
%
\IEEEpubidadjcol{}% Must be somewhere in second column
%
...
%
The advantages of the proposed method are demonstrated on a rotary inverted
pendulum system which, due to its instability, would be difficult to identify in
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            ...
            \node[
                ...
                \begin{overpic}[width=3cm]{./fig_tikz/lift_2.pdf}
                   \put(-10, -10){\includegraphics[width=3cm]{./fig_tikz/lift_1.pdf}}
                   \put(-20, -20){\includegraphics[width=3cm]{./fig_tikz/lift_0.pdf}}
                \end{overpic}\\[3ex]
                \usebox{\arrow}
                \\
...
%
Section~\ref{sec:results} analyzes the experimental results of the closed-loop
Koopman operator approximation method.
%
...
%
The benefits of the proposed closed-loop Koopman operator approximation method,
referred to in this section as closed-loop EDMD (CL EDMD),
are demonstrated on a dataset collected from the Quanser
\textit{QUBE-Servo}~\cite{qube}, a rotary inverted pendulum system. Pictured in
Figure~\ref{fig:qube}, this system has an unstable equilibrium point when the
pendulum is upright.
%
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth, angle=-90]{./pic/quanser.jpg}
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
where $K_1^\mathrm{p}=6$ and $K_2^\mathrm{p}=30$ are the proportional gains,
$K_1^\mathrm{d}=1.8$ and $K_2^\mathrm{d}=2.5$ are the derivative gains, and
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=\linewidth]{./fig/inputs_cl.pdf}
...
The Koopman lifting functions chosen for the rotary inverted pendulum are second
order monomials followed by \num{10} time delays. As required by the proposed
method, the inputs and controller states are not lifted.
%
...
\begin{figure*}[htbp]
    ...
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/spectral_radius_cl.pdf}
        \caption{Relationship between the regularization coefficient $\alpha$
        and the spectral radii of the closed-loop systems identified with EDMD
        and closed-loop EDMD.\@ As $\alpha$ increases, the closed-loop system
        identified with EDMD becomes unstable, while the closed-loop system
        identified with the proposed method remains asymptotically
        stable.}\label{fig:spectral_radius_cl}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/spectral_radius_ol.pdf}
        \caption{Relationship between the regularization coefficient $\alpha$
        and the spectral radii of the plant systems identified with EDMD and
        closed-loop EDMD.\@ As $\alpha$ increases, both plant systems are
        incorrectly stabilized by the
        regularization.}\label{fig:spectral_radius_ol}
    \end{subfigure}
    \caption{Relationships between the regularization coefficients and
    the spectral radii of the closed-loop and plant systems.}\label{fig:spectral_radius}
\end{figure*}
\begin{figure*}[htbp]
    \centering
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/cross_validation_cl.pdf}
        \caption{Relationship between the regularization coefficient $\alpha$
        and the test set $R^2$ scores of the closed-loop systems identified with
        EDMD and closed-loop EDMD.\@ As $\alpha$ increases, both scores
        decrease. However, the system identified with EDMD becomes unstable
        around $\alpha=1$, and its prediction score
        diverges.}\label{fig:cross_validation_cl}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/cross_validation_ol.pdf}
        \caption{Relationship between the regularization coefficient $\alpha$
        and the test set $R^2$ scores of the plant systems identified with EDMD
        and closed-loop EDMD.\@ The plant prediction scores are only finite for
        large regularization coefficients, indicating that is not a good metric
        for tuning the regularizer.}\label{fig:cross_validation_ol}
    \end{subfigure}
    \caption{Relationships between the regularization coefficients and the test
    set $R^2$ scores of the closed-loop and plant
    systems.}\label{fig:cross_validation}
\end{figure*}
...
%
Both plots in Figure~\ref{fig:spectral_radius} indicate that a low
regularization coefficient is appropriate for this system.
...
Figure~\ref{fig:spectral_radius}, a good scoring metric for the
\textit{QUBE-Servo} system should have its peak at a low regularization
coefficient.
%
...
Figures~\ref{fig:spectral_radius} and~\ref{fig:cross_validation} demonstrate two
key conclusions. First, the closed-loop prediction error should be used for
assessing the accuracy of identified models, as the best closed-loop scores
correspond to Koopman models with the expected stability properties.
%
...
\begin{figure*}[htbp]
    ...
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/eigenvalues_cl.pdf}
        \caption{Eigenvalues of the closed-loop systems identified with EDMD and
        closed-loop EDMD.\@ Unlike EDMD, CL EDMD correctly identifies a stable
        closed-loop system for both regularization
        coefficients.}\label{fig:eigenvalues_cl}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/eigenvalues_ol.pdf}
        \caption{Eigenvalues of the plant systems identified with EDMD and
        closed-loop EDMD.\@ For small regularization coefficients, both EDMD and
        CL EDMD correctly identify unstable plant
        systems.}\label{fig:eigenvalues_ol}
    \end{subfigure}
    \caption{Eigenvalues of the closed-loop and plant
    systems.}\label{fig:eigenvalues}
\end{figure*}
\begin{figure*}[htbp]
    \centering
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/errors_cl.pdf}
        \caption{Prediction errors of the closed-loop systems identified with
        EDMD and closed-loop EDMD.\@ Unlike CL EDMD, EDMD incorrectly identifies
        an unstable closed-loop system when using a large regularization
        coefficient.}\label{fig:predictions_cl}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/errors_ol.pdf}
        \caption{Prediction errors and controller output of the plant systems
        identified with EDMD and closed-loop EDMD.\@ Despite the diverging
    ...
    \end{subfigure}
    \caption{Prediction errors of the closed-loop and plant
...
Consider the closed-loop and plant eigenvalues in Figure~\ref{fig:eigenvalues}.
The naive approach, which uses the plant for regularization and scoring,
identifies an unstable closed-loop system with two eigenvalues clearly outside
the unit circle. Due to the large regularization coefficient, the plant system
it identifies is asymptotically stable, which is also inconsistent with the
...
\begin{table}[ht]
    ...
    \begin{tabular*}{\linewidth}{ll@{\extracolsep{\fill}}cccc}
        \toprule
        Reg. &
        Score &
        $R^2$ avg. &
...
Table~\ref{tab:score} shows that the most important factor for attaining a high
prediction score is the use of the closed-loop prediction error for scoring. For
the \textit{QUBE-Servo} system, the choice to use the closed-loop or plant
Koopman matrix for regularization is not critical, as long as a sufficiently
small regularization coefficient is chosen.
%
...
knowledge of the controller and measurements of the controller output, which may
not be available. If knowledge of the controller is already assumed, then using
the proposed closed-loop method is preferable, as it avoids the possibility of
bias in the models.
%
...
particular region~\cite{chilali_1999_robust}. The advantages and disadvantages
of each approach are summarized in Table~\ref{tab:pro_con}.

\begin{table}[ht]
    \centering
    \caption{Comparison of Koopman ID methods}\label{tab:pro_con}
    \begin{tabular*}{\linewidth}{ll@{\extracolsep{\fill}}cccc}
        \toprule
        Reg. &
        Score &
...
%
The advantages of this method is demonstrated experimentally using an unstable
rotary inverted pendulum system. In particular, the proposed closed-loop method
identifies Koopman models that align with the prior knowledge that the
closed-loop system is asymptotically stable, while the plant is unstable. The
closed-loop method also naturally allows the closed-loop prediction error to be
used as a goodness-of-fit metric, which is crucial for selecting lifting
functions and an appropriate regularization coefficient. Finally, the proposed
closed-loop Koopman operator approximation method allows both the closed-loop
and plant Koopman matrices to be used in regularizers or constraints. Using
Extended DMD, only the plant's Koopman matrix can be used in a regularizer,
leading to potentially unstable closed-loop systems for large regularization
coefficients. The additional flexibility provided by the proposed closed-loop
Koopman operator approximation method will be explored in future work, as the
ability to incorporate known information about the closed-loop system or plant
into the regression problem could prove useful in identifying more accurate or
useful Koopman models. Extensions to address the use of nonlinear controllers,
or to address the situation where the controller is also unknown could also
prove valuable in broadening the applicability of the proposed method.

\appendix[Recovery of the plant using least-squares]\label{sec:lstsq}

\begin{figure*}[htbp]
    \centering
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/controller_rewrap_eig_lstsq.pdf}
        \caption{Closed-loop eigenvalues identified with the least-squares
        version of CL EDMD, before and after the plant is extracted and
        the controller is re-connected. When the closed-loop system is
        reconstructed from the plant, the eigenvalues move, and some of them
        leave the unit circle, making the closed-loop system
        unstable.}\label{fig:controller_rewrap_eig_lstsq}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/controller_rewrap_eig_const.pdf}
        \caption{Closed-loop eigenvalues identified with the constrained
        version of CL EDMD, before and after the plant is extracted and
        the controller is re-connected. When the closed-loop system is
        reconstructed from the plant, the eigenvalues do not change, indicating
        that the constrained regression procedure respects the structure of the
        closed-loop system.}\label{fig:controller_rewrap_eig_const}
    \end{subfigure}
    \caption{Closed-loop eigenvalues identified with the least-squares and
    constrained versions of CL EDMD.}\label{fig:controller_rewrap_eig}
\end{figure*}
\begin{figure*}[htbp]
    \centering
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/controller_rewrap_error_lstsq.pdf}
        \caption{Closed-loop prediction errors of models identified with
        the least-squares version of CL EDMD, before and after the plant is
        extracted and the controller is re-connected. Reconstructing the
        closed-loop system destabilizes it, as indicated by the diverging
        predictions of the reconstructed
        model.}\label{fig:controller_rewrap_pred_lstsq}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{3.5in}
        \centering
        \includegraphics[width=\linewidth]{./fig/controller_rewrap_error_const.pdf}
        \caption{Closed-loop prediction errors of models identified with
        the constrained version of CL EDMD, before and after the plant is
        extracted and the controller is re-connected. Reconstructing the
        closed-loop system does not change its predictions, indicating that the
        constrained regression procedure respects its
        structure.}\label{fig:controller_rewrap_pred_const}
    \end{subfigure}
    \caption{Closed-loop prediction errors of models identified with the
    least-squares and constrained versions of CL
...
%
Figure~\ref{fig:controller_rewrap_pred} shows the prediction errors of each
