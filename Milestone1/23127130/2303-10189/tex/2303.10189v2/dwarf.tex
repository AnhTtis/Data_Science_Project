...
thinness allows unobscured views of sub-$\mu$m to mm-sized grains 
%dominating the brightness 
in optical to radio-wavelength images.
...
Apsidal alignment can also result from a giant impact.  %one which produces enough dust to change the disk's complexion. 
...
the impact originally occurred. 
%Debris is released from this collision point at a more-or-less common orbital velocity equal to the progenitor's at the time of impact. 
For many orbital periods after the impact, dust continues to be generated at the collision
point, as fragments repeatedly
return there and grind down to smaller sizes (\citealt{jackson14}; \citealt{kral15}). ``The gift that keeps on giving,'' the collision point acts as a delta-function source of dust
%on apsidally aligned orbits, 
as long as it is not smeared out by differential precession of fragment orbits.
...
%We seek here to determine whether the non-axisymmetric structures in HD 61005, HD 32297, HD 106906, HD 15115, and HD 111520 might also be pointing to giant impacts.
...
Step 1 of our procedure 
%for generating scattered light images 
%of a giant impact debris field
is to compute a set of $N_{\rm p} = 200$ orbits (sometimes higher for greater resolution) of ``parent bodies''---large collision fragments comprising the pinched ring, unaffected by stellar radiation pressure or stellar wind drag---launched from
...
a circular progenitor orbit around a solar-mass star at $|\mathbf{R}| = 60$ au, implying $|\mathbf{V}_0| \simeq 4$ km/s and $|\mathbf{\delta V}| \simeq 100$ m/s; the last quantity is 
%comparable to 
of order the 
...
%Our mock infrared SPF is just a guess at the actual scattering behavior at longer wavelengths; we use it to identify trends and not to perform detailed quantitative comparisons with observations.}
...
%The dust maps derive from $N_{\rm p} = 200$ parent bodies, $N_\beta = 5000$ dust orbits per parent body, and $N_{\rm dust-per-orbit} = 100$ dust grains per dust orbit, for a total of $N_{\rm dust} = 10^8$ dust particles. Colors in the dust maps denote surface brightness on a log scale, plotted in the `bb' color scheme of the \texttt{ds9} software package. At least three orders of magnitude are spanned from white to dark red; regions nearest the collision point have extremely steep brightness gradients, but are shown here saturated in white.
...
%We have used the Hedman-Stark Saturn G ring SPF which is known to not match the SPF inferred for the debris disk HR 4796A (Chen et al.~2020), but which appears consistent with SPFs measured (less well) for other debris disks (Hughes et al.~2018). 
...
Figure \ref{fig:hd106906}, left panel, shows a composite image of the HD 106906 system in two wavebands. At visible wavelengths (HST/STIS) and large spatial scales, diffuse emission from the nearly edge-on disk extends farther to the west than to the east (\citealt{kalas15}; Esposito et al.~2023, in preparation). At longer infrared wavelengths and smaller spatial scales (Gemini Planet Imager, GPI), 
%EC: Kral et al. 2020 reports NO asymmetry in mm continuum, but a CO bright spot 3-4 arcsec to the SE of the star
the western extension is absent, and the central ring instead appears brighter to the east by $\sim$10--30\% \citep{kalas15,crotts21}. 
%The longer the wavelength and the smaller the scale, the more the image should be dominated by larger particles relatively immune to radiation pressure.
...
%Note our zoomed-in and re-scaled model image is just a proxy for a proper infrared image, because we are not accounting for how the scattering phase function changes from optical to infrared wavelengths. Modulo this shortcoming, 
The zoomed-in infrared ring is brighter on its forward-scattering northern half, and most bright in its northeast quadrant where orbital streamlines converge on the collision point and line-of-sight column densities are highest (we chose Alt/Az = -4$^\circ$/120$^\circ$ to situate the collision point in this quadrant). For simplicity we have assumed the progenitor to be on a circular orbit, but there is evidence from the GPI data that the host star (actually a binary) is not centered on the inner disk, implying the latter has an eccentricity of $\sim$0.16, possibly higher (\citealt{crotts21}, their figure 7). We could account for this by allowing for an eccentric progenitor; the resultant disk eccentricity could further brighten the northeast quadrant.

%In the mm continuum as observed by ALMA, HD 106906 presents a radially broad, nearly edge-on disk that appears axisymmetric (apart from a tentative stellocentric offset; \citealt{fehr22,kral20}). 
%%These observations motivate our choice in Fig.~\ref{fig:hd106906} to place the collision progenitor on a circular orbit. 
%At a spatial resolution of $\sim$39 au \citep{fehr22}, the ALMA beam cannot resolve the collision point of the pinched ring of radius $\sim$70 au (cf.~our Fig.~\ref{fig:thermal} showing sample thermal images, and $\beta$ Pic where the collision point is claimed to be detected at mm wavelengths).
...
%To summarize, modulo the uncertainty regarding the origin of HD 106906b, the morphology of the HD 106906 disk seems more completely and economically explained by gravitational perturbations from the known companion than by a giant impact.}



%There appears to be more optical emission to the east in the HST/STIS image than in our model. This could be remedied by accounting for the extra dust created at the ``anti-collision line'' and not just the collision point (see the beginning of \S\ref{subsec:procedure}). Or we could try adding light from a ``background'' parent body disk separate from the giant impact debris---more on this background disk below.




%%The model image assumes a circular progenitor orbit, as suggested by the lack of non-axisymmetry seen
%%but an eccentric progenitor could probably be accommodated as well, since many of the halo sub-structures that depend on progenitor eccentricity are washed out for our viewing geometry.


%This misalignment appears significant, and incompatible with the first scenario which posits that all particles seen at all wavelengths derive from one progenitor and therefore share a common  orbital plane. By contrast, the second scenario can accommodate the misalignment, if we say that the giant impact debris seen at optical wavelengths on large scales derives from a disrupted progenitor whose orbit was inclined by $\sim$5 degrees relative to the background disk seen at infrared wavelengths on small scales. 

%A giant impact origin for the scattered-light needle asymmetry in HD 106906 would seem an explanation to be preferred over others. The disk orbits a stellar binary which could in principle force an eccentricity on disk particles and generate a needle. But as we have noted, the mm-wave disk shows no evidence for eccentricity, aside from a stellocentric offset which can be attributed to the external substellar companion HD 106906b shifting the system barycenter. HD 106906b otherwise has no significant dynamical effect on the disk, as the orbital precession the companion forces is insignificant compared to the precession driven by the central binary \citep{fehr22,farhat22}.
...
Unfortunately, other details in the observations do not match those in the model. On the small scales probed by GPI (top inset in Fig.~\ref{fig:hd111520}), the disk appears brighter to the north than to the south. Our model on these scales is instead brighter to the south, at the periapse of the eccentric disk; here the disk is vertically thinner and line-of-sight column densities are larger (bottommost image in Fig.~\ref{fig:hd111520}). A second problem is that the disk seen by GPI aligns with only the lower fork---see the horizontal dotted line drawn in Fig.~\ref{fig:hd111520}, and also figure 5 of \citet{crotts22}. From the model, we would instead expect the spine of the GPI disk to cut through the middle of the fork.
%since in the model, the compact eccentric disk of larger infrared-scattering particles aligns perfectly with the extended fork of smaller visible-scattering particles. 
A third feature of the HST data not reproduced by the model is that the southern ansa warps downward to the west, by practically the same amount that the upper fork warps upward to the east \citep[][their figure 5]{crotts22}.
...
%The short (southern) ansa of our model is too short compared to the observations, but could be lengthened  by including dust grains launched from parent bodies located closer to their apastra (we assumed launch true anomalies were uniformly distributed between $-\pi/2$ and $\pi/2$, which sends grains farther north than south; \S\ref{subsubsec:proc_fam}). The short ansa could also be extended by including dust generated from a separate ``background'' belt that would serve as a backdrop to the collisional family.
...
%(see also HD 111520 whose southern ansa is similarly more extended than predicted from our simplified giant impact model). 

%In the Alt/Az = -1$^\circ$/60$^\circ$ view of dust from an eccentric progenitor (Fig.~\ref{fig:hd15115} bottom panel),  %%shows a blow up of the relevant image in Fig.~\ref{fig:pano_ecc})
%the short side of our model needle curves upward, appearing somewhat like the shorter needle 2 of HD 15115. However, it is not bifurcated as clearly as in the observations. Another problem is that whereas the observed needle 2 is straight and points radially back toward the host star, in the model image the spine of the disk on the short ansa curves gently (there is a second derivative), and a local tangent to the spine at its far end does not point back toward the star.

%Invoking two giant impacts to explain the two needles of HD 15115 might seem to some a bit ad hoc. 
...
In the HST image, the primary wing is brighter than the secondary by factors of $\sim$2--3. In our model image using a progenitor on a circular orbit, the brightness contrast between primary and secondary wings is a factor of 1.5--2, lower than desired because the primary tends to be seen in weaker back-scattered light. Ways to remedy this discrepancy include: changing the scattering phase function; allowing for some differential precession of fragment orbits to smear out the collision point, thereby diffusing the jets and by extension the secondary wing; and adding light to the primary wing from a separate background disk. 
%EC: which could be eccentric
...
%(see the discussion for HD 106906, whose favored second scenario applies here as well).
...
%We found that we could dim and thereby shorten the primary wing relative to the secondary wing by reducing the velocity dispersion $|\delta V_0|/|V_0|$ of fragment ejecta (\S\ref{subsubsec:proc}); see the third panel from the top of Fig.~\ref{fig:hd61005}. The shortening, however, is modest and not adequate to reproduce the observations.

\section{A Hidden Population of Giant Impact Progenitors}\label{sec:hidden}

%\subsection{Progenitor Mass}
...
%The progenitor sizes $R$ underlying giant impact candidate disks need to be determined precisely in future work. %Doing so, and further developing the case for catastrophic disruptions, would enable powerful constraints on theories of planet formation.

% simulations orders of magnitude larger than the present-day mass of the Kuiper belt, and presents a picture quite different from standard coagulation simulations which produce many fewer Pluto-sized objects at $\sim$100 au (e.g. Kenyon \& Bromley 2005, and references therein). 

%This implies a gravitationally focussed collision, one which occurs nearly head-on at velocity $v_{\rm coll} \simeq v_{\rm esc}$. Such a collision has high accretion efficiency, with at most $\xi_{\rm d} \sim 10$\% of the colliding mass escaping as debris from the gravitational well of the merger product (see, e.g., figure 5 of \citealt{emsenhuber20} for $v_{\rm coll}/v_{\rm esc} \simeq 1$ and impact angles $\theta_{\rm coll} \ll 1$ rad). Then we estimate $M_{\rm p} \sim M_{\rm d}/\xi_{\rm d} \sim 0.1$--$1 M_\oplus$.

%We estimate a minimum mass for the collision progenitor by assuming it was catastrophically disrupted into fragments less than half its original size. Under these conditions, the effects of gravity should be minimized; the progenitor's surface escape velocity $v_{\rm esc}$ should be less than the collision velocity $v_{\rm coll}$ (the collision is gravitationally unfocussed) and the impact angle $\theta_{\rm coll}$ should be $\lesssim 1$ rad, to avoid too grazing a collision \citep{emsenhuber20}. Assuming the colliding bodies are drawn from a reservoir $\sim$100 au from a $2 M_\odot$ star with eccentricities and inclinations $< 0.3$, gravitationally unfocussed collisions have $v_{\rm coll} \lesssim 1.3$ km/s. Then for a bulk density of 2 g/cc, $v_{\rm esc} \lesssim v_{\rm coll}$ implies a progenitor mass $\lesssim 0.002 M_{\oplus}$ or less than Pluto's.




%The total dust mass needed to reproduce the observed fluxes in HD 106906 from the near-infrared to the millimeter is estimated to range from $\sim$0.01 to $0.1 M_\oplus$ \citep{fehr22,crotts21,kral20,chen11}. We can think of two ways to interpret this mass in the context of a giant impact. In one endmember interpretation, the entirety of the disk, of mass $M_{\rm d} \sim 0.01$--$0.1 M_\oplus$, is the unaccreted debris from the destruction of a progenitor of mass $M_{\rm p} > M_{\rm d}$ which collided with a projectile of comparable mass. The collision would have been gravitationally focussed, a conclusion reached as follows. Orbital velocities at a characteristic radius of $\sim$70 au in the HD 106906 disk are $\sim$6 km/s, implying unfocussed relative velocities $v_{\rm unfocus}$ of at most $\sim$1 km/s (the value obtained by assuming bodies have inclination and/or eccentricity dispersions on the order of 0.2). A body whose surface escape velocity matches $v_{\rm unfocus} \sim 1$ km/s is Pluto, whose mass is $0.002 M_\oplus < M_{\rm d}$. Since $M_{\rm p} > M_{\rm d}$ in this scenario, and surface escape velocities increase with increasing mass, we deduce that the progenitor's surface escape velocity $v_{\rm esc} > v_{\rm unfocus}$, i.e. $M_{\rm p} \gg$ Pluto. 

%This implies a gravitationally focussed collision, one which occurs nearly head-on at velocity $v_{\rm coll} \simeq v_{\rm esc}$. Such a collision has high accretion efficiency, with at most $\xi_{\rm d} \sim 10$\% of the colliding mass escaping as debris from the gravitational well of the merger product (see, e.g., figure 5 of \citealt{emsenhuber20} for $v_{\rm coll}/v_{\rm esc} \simeq 1$ and impact angles $\theta_{\rm coll} \ll 1$ rad). Then we estimate $M_{\rm p} \sim M_{\rm d}/\xi_{\rm d} \sim 0.1$--$1 M_\oplus$.

%Alternatively, we may interpret $M_{\rm d}$ as the mass belonging to a ``background'' axisymmetric disk not directly related to the eastern brightening and long western extension seen in the infrared and optical. The non-axisymmetric features would be due to the catastrophic, i.e. near-complete disruption of a mass $M_{\rm p}$ which in this scenario would be $\ll M_{\rm d}$. 

%The second of the two above interpretations seems more plausible as it requires less mass to be involved in the collision (but see \S\ref{sec:sum} where we give a more full accounting of the total mass reservoir). 
...
%\tablewidth{0pt}
 ...
 $\beta$ Pic & Y & & Y & needle from giant impact; warp from $\beta$ Pic b & 7, 8, 9, 10 \\
  HD 106906 & Y &  & Y & Occam's razor prefers HD 106906b & 1, 11, 12, 13 \\
 HD 111520 &  &  & Y & warp analogous to $\beta$ Pic warp & 1, 9 \\
 \enddata

%\tablecaption{}
% \vspace{-0.5cm}
\tablecomments{Y = Yes, this seems a viable explanation. ? = Possibly relevant but probably not a dominant effect.}
\tablerefs{1.~This paper 
2.~\cite{mazoyer14} 
3.~\cite{chiang17} 4.~\cite{debes09}
5.~\cite{maness09} 
6.~\cite{olofsson16} 
7.~\cite{dent14} 8.~\cite{janson21} 9.~\cite{mouillet97} 10.~\cite{dupuy19} 
11.~\cite{lee16} 
12.~\cite{rodet17}
13.~\cite{moore23}
}
...
%What is needed to make progress is a model that explicitly connects progenitor mass to the evolving size distributions of fragments and dust, and computes flux-calibrated images vs.~time (cf.~\citealt{kral15}).


%\subsection{Inferring the Reservoir of Large Bodies from Giant Impacts}\label{subsec:reservoir}
...
\section{{  For our eyes only}}

\subsection{Thermal emission images}

\subsubsection{Wavelength and $\beta$-dependent emissivity $\varepsilon$}
Model each dust particle of size $s$ as having an emissivity $\varepsilon$ at wavelength $\lambda$:
\begin{equation}
\varepsilon (s, \lambda) = \begin{cases}
1 & {\rm if} \,s > \lambda/(2\pi) \\
2\pi s/\lambda & {\rm if} \,s \leq \lambda/(2\pi)\,. \nonumber
\end{cases}
\end{equation}
Relate $s$ to $\beta$ using:
\begin{equation}
s = \frac{3}{16\pi} \frac{L_\star Q_{\rm rad}}{GM_\star c\rho \beta} \,. \nonumber
\end{equation}
Let's scale to parameters specific for HD 106906: $L_\star = 6.6 L_\odot$, $M_\star = 2.7 M_\odot$, $\rho = 1$ g/cc, $c = 3 \times 10^{10}$ cm/s, $Q_{\rm rad} = 1$, $G = 6.67 \times 10^{-8}$ in cgs units. After plugging in, {  GRAIN SIZE $s_{\mu{\rm m}}$ IN UNITS OF $\mu$m IS COMPUTED FROM $\beta$ USING:}
\begin{equation}
s_{\mu{\rm m}} = c_1 / \beta \label{eqn:s_beta_simple}
\end{equation}
{  WITH $c_1 = 1.5$ (a dimensionless number)}. Also {  OUR EQUATION FOR THE WAVELENGTH-DEPENDENT EMISSIVITY  $\varepsilon$ BECOMES}:
\begin{equation}
\varepsilon (\beta,\lambda)= %\begin{cases}
\label{eqn:vareps_simple}
\min [1, 2\pi c_1/(\beta \lambda_{\mu{\rm m}})]
%1 & {\rm if} \,\beta < 2\pi c_1/ \lambda_{\mu{\rm m}} \\
%2\pi c_1/(\beta \lambda_{\mu{\rm m}}) & {\rm if} \,\beta \geq 2\pi c_1/\lambda_{\mu{\rm m}} 
%\end{cases}
\end{equation}
{  WHERE $\lambda_{\mu{\rm m}}$ MEASURES WAVELENGTH $\lambda$ IN UNITS OF $\mu$m. WE SET $\lambda_{\mu{\rm m}}$ = 1000 TO COMPUTE mm-WAVE IMAGES.}

\subsubsection{Dust grain temperature $T$}
The dust grain temperature is given by:
\begin{equation}
T = \left( \frac{L_\star}{16\pi \sigma r^2} \right)^{1/4} \left( \frac{1}{\langle\varepsilon\rangle_T} \right)^{1/4} \nonumber
\end{equation}
(equation 3 of Chiang et al.~2000). The factor of 16 = $4\cdot 4$ where the second factor of 4 comes from considering a spherical grain of geometric absorption cross-section $\pi s^2$ and emitting geometric cross section $4\pi s^2$. Approximate $\langle \varepsilon \rangle_T = 2\pi s/[hc/(3kT)]$ if $2\pi s/[hc/(3kT)] < 1$, and $\langle \varepsilon \rangle_T = 1$ otherwise (recognize $hc/(3kT)$ as the wavelength where the Planck function $B_{\nu}$ peaks). Another way to write this is $\langle \varepsilon \rangle_T = \min (2\pi s/[hc/(3kT)], 1)$.

From the above, {  CALCULATE $T$ USING THE FOLLOWING ALGORITHM: first, assign $T$ to be the value of a spherical blackbody:}
\begin{align}
T &= \left( \frac{L_\star}{16\pi \sigma r^2} \right)^{1/4} \nonumber \\
T_{\rm K} &= 450.4 \cdot r_{\rm au}^{-1/2}
\label{eqn:starter_T}
\end{align}
where $T_{\rm K}$ is $T$ measured in units of deg K, and stellocentric radius $r_{\rm au}$ is measured in units of au. 
Equation (\ref{eqn:starter_T}) assumes $\langle \varepsilon \rangle_T = 1$ (blackbody). {  Check {\it a posteriori} that this assumption is valid by computing}
\begin{align}
{\rm check} &= \frac{6\pi s k T}{hc} \nonumber \\
 &= 0.0013 \cdot s_{\mu{\rm m}} \cdot T_{\rm K} 
\label{eqn:check}
\end{align}
where $s_{\mu{\rm m}}$ is measured in $\mu$m, and $T$ is measured in degrees K. {  If check $\geq 1$ then stop: the assigned blackbody $T$ is correct. If check $< 1$ then re-assign $T$ to:}
\begin{align}
T &= \left( \frac{L_\star}{16\pi \sigma r^2} \right)^{1/4} \left( \frac{hc}{6\pi s kT} \right)^{1/4} \nonumber \\
T & = \left( \frac{L_\star}{16\pi\sigma r^2} \right)^{1/5} \left( \frac{hc}{6\pi s k} \right)^{1/5} \nonumber \\
T_{\rm K} &= 500.7 \cdot r_{\rm au}^{-0.4} \cdot s_{\mu{\rm m}}^{-0.2} \,. \label{eqn:alternate_T}
\end{align}

\subsubsection{Contribution to the surface brightness}
{  Recall that in our scattered light images, each grain contributes a surface brightness proportional to $\phi(\theta)/(\beta^2 r^2)$, where $\phi$ is the scattering phase function and $\theta$ is the scattering angle. Find the piece of code that evaluates this quantity. REPLACE $\phi(\theta)/(\beta^2 r^2)$ with the following to model thermal emission at mm-wavelengths:}
\begin{align}
\phi(\theta)/(\beta^2 r^2) \rightarrow &
%T_{\rm K} (\beta,r_{\rm au}) 
\, [\exp( 14391 \lambda_{\mu{\rm m}}^{-1} T_{\rm K}^{-1} ) - 1]^{-1}\nonumber \\
& \cdot \varepsilon (\beta,\lambda_{\mu{\rm m}}) \cdot [s_{\mu{\rm m}}(\beta)]^2 \label{eqn:replacement}
\end{align}
The prefactor in square brackets comes from the Planck function $B_{\nu}$. We have dropped the factor of $1/\lambda^3$ from $B_{\nu}$ since it does not change from grain to grain.
%The linear scaling with $T$ assumes the specific intensity is on the Rayleigh-Jeans tail of the Planck spectrum, which should be OK as long as $T$ does not fall much below $\sim$6 K. 

{  TO EVALUATE (\ref{eqn:replacement}), USE EQUATIONS (\ref{eqn:s_beta_simple}), (\ref{eqn:vareps_simple}), (\ref{eqn:starter_T}), (\ref{eqn:check}), and (\ref{eqn:alternate_T}), setting $\lambda_{\mu{\rm m}} = 1000$ (ALMA), and for a separate set of images, $\lambda_{\mu{\rm m}} = 25$ (JWST).}

\subsection{Mock Infrared Scattering Phase Function}
At longer than visible wavelengths, we expect scattering to be more isotropic, insofar as diffraction predicts the forward-scattered beam to scale in angular width as wavelength divided by grain size. We mock up an infrared scattering phase function (SPF) by taking the Hedman-Stark SPF and adjusting its weighting coefficients. {  We decrease the weight of the most strongly forward-scattering component ($w_1 = 0.643 - 0.13 = 0.513$ instead of $w_1 = 0.643$) and increase correspondingly the weight assigned to the most isotropically scattering component ($w_3 = 0.181 + 0.13 = 0.311$ instead of $w_3 = 0.181$).} Compared to the original visible-wavelength Hedman-Stark SPF which beams 50\% of its power into scattering angles $\theta < 1.24^\circ$ and 75\% into $\theta < 34.1^\circ$, our mock infrared SPF beams those relative powers into larger angles $\theta < 6.28^\circ$ and $\theta < 64.9^\circ$, respectively. The overall shape of this infrared SPF is otherwise similar to the visible SPF (cf. Chen et al.~2020). Our mock infrared SPF is just a guess at the actual scattering behavior at longer wavelengths; we use it to identify trends and not to perform detailed quantitative comparisons with observations.
