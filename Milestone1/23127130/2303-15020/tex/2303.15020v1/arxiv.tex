...
We prove continuous relative completeness of the logic w.r.t. FOD, as well as discrete relative completeness in the sense  that continuous behaviour can be arbitrarily approximated by discretization. Besides, we discuss how to simplify proofs using the logic by providing a simplified assertion language and  a 
set of sound and complete rules for differential invariants for ODEs. Finally,  we  
implement a proof assistant for the logic  
in Isabelle/HOL, and apply it to 
 verify two case  studies to illustrate the power and scalability of our logic. 
 \oomit{an example of a lunar lander and an example of a scheduler controlling two parallel tasks, which are too complicated to be verified with 
 the DC-based HHL Prover. }
 %, that  
%demonstrates the power and scalability of the new proof system. 
%approach by verifying a lunar lander example.   
\oomit{
Deductive verification of hybrid systems increasingly attracts more attention in recent years because of its power and scalability. A powerful specification logic for hybrid systems is the cornerstone of deduction-based  approaches. However, the existing specification logics cannot specify and reason about properties related to complicated features of hybrid systems very well. In this paper, we revisit hybrid Hoare logic by proposing a much simpler proof system, and proving its 
continuous relative completeness in the sense that the first-order theory of differential equations 
is complete, and its discrete relative completeness in the sense that 
continuous behaviour can be arbitrarily approximated by discrete actions and the corresponding discrete part is complete. Our logic still use Hybrid CSP as its modeling language, but use first-order theory of differential equations together with specific predicates for timed traces and readiness as its assertion logic. }
\end{abstract}
...
\keywords{Hybrid systems, hybrid Hoare Logic, Hybrid CSP, proof system, relative completeness, differential invariants}
...
%, specifically, if the ODEs of the problems can be  solved explicitly.
% Certainly, applying abstraction or approximation \cite{ADI06,ABDM00}  can improve the scalability, but as a pay we have to
...
\oomit{ although several hybrid logics have been proposed, e.g., Differential (Algebraic) Dynamic Logic (D(A)DL) \cite{Platzer08,PlatzerClarke08}, Extended Duration Calculus (EDC) \cite{ChaochenRH92}, Hybrid Hoare Logic (HHL) \cite{Liu10,WZG12}, \emph{etc}., as they are either not expressive enough to
handle properties related to different features of hybrid systems or too complicated. For instance, D(A)DL cannot deal with communication and concurrency, EDC depends on some existing mathematical theory to reason about continuous properties, while previous versions of HHL is either noncompositional \cite{LLQZ10} or too complicated \cite{WZG12,GWZZ13}, particular, it lacks logical foundations.}

In this paper, \oomit{we consider the problem of verifying systems with hybrid behavior as well as explicit concurrency and communication. To express the formal model of the system, we choose Hybrid CSP (HCSP)~\cite{Jifeng:1994,Zhou:1996}, an extension of CSP to include continuous evolution according to an ODE, as well as interruption of continuous evolution by communication.}  we re-investigate proof theory for HCSP by providing a much simpler proof system with continuous and discrete relative completeness. 
...
The bare version of the proof system, while satisfying good completeness properties, is still tedious to use in practice, as the weakest preconditions consist of several cases even for simple communications, and moreover, the rules for ODEs require the explicit solutions, that may even not exist. To simplify proofs in practice, we define on top of the existing assertion language a set of simplified constructs. We demonstrate that, using the defined syntactical sugar, the proof system can be simplified,  and thus verification of HCSP programs can be done in a relatively concise and much simpler manner. Second, we formulate the differential invariant rules for reasoning about ODEs, and prove the soundness and completeness of semi-analytic invariants. Finally, we implement a proof assistant for our proof system with the above simplification in Isabelle/HOL, and apply it verify an example of a lunar lander and  a scheduler controlling two modules in parallel. The latter 
is too complicated to be proved with  DC-based HHLProver \cite{WZZ15},  
%a lunar lander example using this system. 
which demonstrates the power and scalability of our proof system. 
%use of differential invariants, communication synchronization and compositional reasoning. \oomit{The proof system is implemented in Isabelle/HOL in a foundational way: we define both big-step and small-step semantics of HCSP, prove their equivalence, and prove all deduction rules to be sound in terms of the big-step semantics. This is then used to mechanically check all our examples.}
...
%\vspace*{-3mm} 
    ...
    \item We further simplify the proof system by providing a simplified language of assertions on traces and a set of  differential invariant rules for ODEs, 
...
\oomit{
In this paper, we will revisit Hybird Hoare logic  proposed in  \cite{Liu10,WZG12} by giving a much simpler compositional proof system. %Similar to, we will still use Hybrid CSP (HCSP) as the modeling language. 
In order to simplify the logic, 
inspired by Hooman's real-time Hoare logic \cite{Hooman94}, we only use  first-order logic together with some  specific predicates to timed traces and readiness as the assertion logic, rather that 
first-order logic together with Duration Calculus \cite{ZHR91} like in \cite{Liu10,WZG12}, that complicates the logic very much. Additionally, our proof system is a conservative extension of  Hoare proof system for CSP proposed in \cite{Apt80,Apt83}. Actually, if we consider to discretize the continuous behaviours in HCSP, i.e., we remove rules and axioms related to continuous evolution in HCSP, then the resulted fragment of our proof system  is equivalent to Apt \emph{et al}'s \cite{Apt83}, therefore, we can conclude the relative completeness of our proof system in the sense of discrete semantics as discussed in \cite{Platzer12a}. 

Also, similar to \cite{Platzer12a}, we prove our proof system is complete with respect to continuous semantics in the Cook's sense \cite{Cook78}, 
i.e., we assume all valid assertions related to first-order theory of differential equations. 

 In addition, we implement the logic in Isabelle/HOL, and some case studies  will be used to illustrate its effectiveness and power.
}
...
%and the extended DC (called EDC) tries to make up the gap but the logical reasoning with EDC is quite complicated.

 
\oomit{The join and magic-wand operators introduced in our simplified assertion language are similar to notions in separation logic~\cite{Reynolds02} and duration calculus~\cite{ZHR91}. An operator for separation in time in the semantics of CSP-like languages is also introduced in~\cite{HoareO08}.}

\oomit{
There are many other extensions of Hoare logic, we just name a few as follows. To deal with mutation data structures, Reynolds proposed separation logic \cite{Reynolds02}, O'Hearn \emph{et al.} developed the logic, even extended it to concurrency    \cite{OYR09,COY07,CDOY11}.  
Hartog and Vink introduced a Hoare-like proof system for partial correctness of probabilistic programs which are defined as (deterministic) while programs extended by the probabilistic
choice \cite{HartogV02}.  In \cite{Benton04}, Benton extended Hoare logic to relational Hoare logic (RHL) for comparing two programs, and Barthe further extended RHL to probabilistic RHL (pRHL)  \cite{Barthe13} for specifying and reasoning about security  with a proof assistant EasyCrypt \cite{Barthe13b}. In addition, Ying extended Hoare logic to quantum computation and established 
quantum Hoare logic (qHL) \cite{Ying11}, and recently, Barthe \emph{et al.} combined pRHL and qHL together  
and established probabilistic quantum Hoare logic (pQHL) \cite{Barthe20}  for security and quantum program comparison in quantum computation. Besides, recently Hoare logic was revised for detecting bugs and 
established incorrect logic \cite{OHearn20} due to O'Hearn and  incorrect separation logic \cite{RaadBDDOV20} due to O'Hearn \emph{et al.}
}
...
\oomit{
Hybrid automata~\cite{Alur1993} are a popular formalism to model hybrid systems, but composition of automata results in an exponential product automaton which is intractable to analyze in practice. \emph{I/O hybrid automata}~\cite{LynchSV03} is an extension of hybrid automata with explicit inputs and outputs. Assume-guarantee reasoning~\cite{Henzinger2001} on such automata tackles composability to prevent state-space explosion. Yet, use is in practice restricted to linear hybrid automata. }
...
All code related to the implementation and case studies can be found at https://github.com/AgHHL/lics2023.git. 
...
Throughout this paper, we use the following notations and conventions: 
 $\mathbb{N}$, $\mathbb{R}$ and $\mathbb{R}^+$ stand for the set of natural numbers, real numbers, 
 nonnegative real numbers, respectively. $x,y,z, \ldots$ possibly with superscript and/or subscript denote 
 variables over real numbers. $\vec{x}_0$ stands for a vector of variables, whose $i$-th element is denoted by 
  $x_i$. Similarly, $\vec{e}$ stands for a vector of expressions, and its $i$-th element is denoted by 
   $e_i$. $\dot{x}$ stands for the derivative of $x$ with respect to time.} 
...
\begin{itemize}
	...
	%\item The process variable $X$ in isolation does nothing, keeps idle for ever. But when it is used in a recursion,  the execution of every of its occurrences  is replaced by the recursion  itself.   
	\item Process variable $X$ and recursion \sm{\mu X. c} are defined as usual.  \sm{\mu X. c} behaves like  the 
	process derived from  $c$ by replacing all occurrences $X$ in it  with \sm{\mu X. c}. 
	%executes $c$ for a nondeterministic finite number of times.
...
the right side $\vec{e}$ satisfies the \emph{local Lipschitz condition}. 
	%For easing our discussion later, we assume $B$ to be open in what follows. But such assumption can be dropped as any closed set can be  represented as the  interaction of infinite many open sets. 
...
The other constructs of HCSP in \cite{Jifeng:1994,Zhou:1996} are definable, e.g., $\pwait\ d$, repetition, and external choice etc. 
%%\begin{eqnarray*}
\oomit{ \sm{\pwait \ d  \pdef  t:=0; \langle \dot{t}=1\& t < d\rangle,}
 ...
 %and the timeout
 %\sm{ \langle  \vec{\dot{x}}=\vec{e}\& B \unrhd_d Q \rangle} can be defined
 %   by \sm{t:=0; \langle  \wedge \vec{\dot{x}}=\vec{e}, \dot{t}=1 \& t < d \wedge B\rangle;
 %    t\geq d \rightarrow Q.}
 %   \oomit{, so the transition rules for these two constructs are derivable from
 %    those of other constructs, and thus omitted here for saving space. }
\oomit{
In the sequel, we use \sm{\Var(P)} to stand for the set of local variables and \sm{\Chan(P)} for the set of channels
 of a process \sm{P}.
%, and \sm{\actf(P)} for the set of input and output events, occurring in \sm{P} respectively.
 \sm{ch*} stands for a communication event, i.e., either \sm{ch?x} or \sm{ch!e,} while
  \sm{\overline{ch*}} for its dual,  i.e., \sm{ch!e} (\sm{ch?x}) if \sm{ch*} is \sm{ch?x} (
   \sm{ch!e}), and \sm{\textit{comm}(ch*, \overline{ch*})} for a communication \sm{ch.e} when
   \sm{x} is identical to \sm{e}. } 
...
\begin{itemize}
	...
	%As usual, we assume a communication event itself does not take any time, i.e., we adopt \emph{super-dense computation} as normal. 
...
%For brevity, hereafter we use \emph{event} to refer to \emph{generalized event} if no otherwise stated. 
...
Rules SyncWait1-22 define the cases when both sides are wait events, i.e. waiting for a communication or evolving w.r.t. an ODE, then the wait events of the same length will synchronize if they have compatible ready sets.  
...
{\langle d,\vec{p}_1,\rdy_1\rangle^\chop tr_1 \|_{cs} \langle d,\vec{p}_2,\rdy_2\rangle^\chop tr_2 \Downarrow \langle d, \vec{p}_1\uplus \vec{p}_2,\rdy_1\cup \rdy_2\rangle^\chop \textit{tr}} & \\[1mm] 
& \prftree[r]{SyncWait2 }
{\begin{array}{cc}
		d_1>d_2>0 \quad \langle d_1-d_2,\vec{p}_1(\cdot+d_2),\rdy_1 \rangle^\chop tr_1\|_{cs} tr_2\Downarrow \textit{tr} \quad
		\compat(\rdy_1,\rdy_2)
\end{array}}
{\langle d_1,\vec{p}_1,\rdy_1\rangle^\chop tr_1 \|_{cs} \langle d_2,\vec{p}_2,\rdy_2\rangle^\chop tr_2 \Downarrow \langle d_2,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup\rdy_2 \rangle^\chop \textit{tr}} & 
\end{eqnarray*} }
\vspace{-4mm} 
...
\oomit{
\begin{remark}
Note that in the above rules, we assume the maximal synchronization property, i.e., two processes can perform time synchronization if and only if there is not a pair of matched communication events both are ready, see 
the antecedent $\compat(\rdy_1,\rdy_2)$ in SyncWait1 and SyncWait2. 
\oomit{Concretely speaking, SyncWait2 rule handles the case when the waiting time $d_1$ of the left process is longer than the waiting time $d_2$ of the right process. Then, we synchronize only the time interval of length $d_2$ at this step. The second antecedent of SyncWait2 means that the remaining portion of waiting time ($d_1-d_2$, with path shifted) together with the remaining trace on the left synchronizes with the remaining trace on the right. This serves the same role as the first antecedent of SyncWait1, and the second antecedent of rules SyncIO and NoSyncIO. }
\end{remark} 
}
...
\begin{eqnarray*} 
 ...
 \prftree[r]{}{(ch?x, s) \Rightarrow (s[x \mapsto v], \langle ch?, v\rangle) }& \\[1mm] 
  ...
  %\prftree[r]{}{(ch?x, s) \Rightarrow (s[x \mapsto v], \langle ch?, v\rangle) }
   \prftree[r]{}{d>0}{(ch?x, s) \Rightarrow (s[x \mapsto v], \langle d, I_s, \{ch?\} \rangle^\chop \langle ch?, v\rangle)}  \quad \prftree[r]{}{(ch?x,s)\Rightarrow (s, \langle \infty,I_s,\{ch?\})}  \quad \prftree[r]{}{ }
	{(X, s) \Rightarrow (s, \langle \infty,I_s,\{ \, \} \rangle )} 
	\quad\prftree[r]{}{ (c[\mu X. c/X], s)  \Rightarrow (s', \textit{tr})  }
	{ (\mu X. c, s) \Rightarrow (s', \textit{tr})} & \\[1mm]
	& \prftree[r]{}{
		\begin{array}{cc}
...
\begin{itemize}
	...
	\item $X$ and $\mu X. c$ can be understood in a standard way. 
...
%The communication by synchronizing the traces \sm{tr_2'=\langle ch!,3\rangle} and \sm{tr_3'=\langle ch?,3\rangle}
...
\oomit{
Some selected rules for small-step semantics  are shown in Fig.~\ref{fig:small-step} (The full semantics is in Appendix~\ref{app:full-semantics}). 
\begin{figure*} \centering \vspace*{-2mm}
{\small 
 \begin{eqnarray*} 
 & \prftree[r]{}{}{(ch!e, s) \xrightarrow{\langle ch!,s(e)\rangle} (\pskip, s)}
\qquad
   \prftree[r]{}{}{(ch!e, s) \xrightarrow{\langle d, I_s,\{ch!\}\rangle} (ch!e, s)}
		\qquad
		\prftree[r]{}{}{(ch!e, s) \xrightarrow{\langle \infty,I_s,\{ch!\}\rangle} (\pskip, s)} \qquad
		\prftree[r]{}{}
		{(X, s) \xrightarrow{\langle d,I_s,\{\}\rangle}
			(X, s)}
%		\qquad \prftree[r]{}{(ch?x, s) \xrightarrow{\langle ch?, v\rangle } (\pskip, s[x\mapsto v])} & \\[1mm]
%		& 	\prftree[r]{}{(ch?x, s) \xrightarrow{\langle d,I_s,\{ch?\}\rangle}
%			 (ch?x, s)}
%		\qquad
%		\prftree[r]{}{(ch?x, s) \xrightarrow{\langle \infty,I_s,\{ch?\}\rangle}  (\pskip, s)} 
  & \\%[1mm]
	& 			\prftree[r]{}
		{ (c[\mu X. c/X],s)\xrightarrow{e}(c',s')  }
		{(\mu X. c, s) \xrightarrow{e} (c', s')}
		\quad \prftree[r]{}
		{\begin{array}{cc}
				\vec{p} \mbox{ is a solution of the ODE $\vec{\dot{x}}=\vec{e}$} \\
		...
		\prftree[r]{}
		{\neg s(B)}
		{(\evo{x}{e}{B}, s) \xrightarrow{\tau} (\pskip, s)} & \\%[1mm]
	& 	
	%\prftree[r]{}
	%	{\begin{array}{cc}
%				\vec{p} \mbox{ is a solution of the ODE $\vec{\dot{x}}=\vec{e}$} \\
%				\vec{p}(0) = s(\vec{x}) \quad \forall t\in[0,d).\, s[\vec{x}\mapsto \vec{p}(t)](B)
%		\end{array}}
%		{(\exempt{\evo{x}{e}{B}}{i\in I}{ch_i*}{c_i}, s) \xrightarrow{\langle d,\vec{p},\rdy(\cup_{i\in I} ch_i*) \rangle}
%			(\exempt{\evo{x}{e}{B}}{i\in %I}{ch_i*}{c_i}, s[\vec{x}\mapsto \vec{p}(d)])} & \\%[1mm]
%	& 	 \prftree[r]{}
%		{\neg s(B)}
%		{(\exempt{\evo{x}{e}{B}}{i\in I}{ch_i*}{c_i}, s) \xrightarrow{\tau} 
%			(\pskip, s)} \quad
%		 \prftree[r]{}
%		{i\in I}{ch_i* = ch!e}
%		{(\exempt{\evo{x}{e}{B}}{i\in I}{ch_i*}{c_i}, s) \xrightarrow{\langle ch!,s(e)\rangle}  {(c_i, s)}} 
%		\quad & \\[1mm]
 \prftree[r]{ }
{(c_1,s_1) \xrightarrow{\tau} (c_1',s_1')}
{(c_1\|_{cs} c_2,s_1\uplus s_2) \xrightarrow{\tau}  (c_1'\|_{cs} c_2, s_1'\uplus s_2)} \quad
\prftree[r]{}
...
{(c_1\|_{cs} c_2, s_1\uplus s_2) \xrightarrow{\langle ch\triangleright,v\rangle}
	(c_1'\|_{cs} c_2, s_1'\uplus s_2)}  \quad
	\prftree[r]{}
...
& \prftree[r]{ }
{\begin{array}{c}
		\compat(\rdy_1, \rdy_2) \quad 
		(c_1,s_1) \xrightarrow{\langle d,\vec{p}_1,\rdy_1\rangle} (c_1',s_1')
		\quad 
		(c_2,s_2) \xrightarrow{\langle d,\vec{p}_2,\rdy_2\rangle} (c_2',s_2')
\end{array}}
{(c_1\|_{cs} c_2, s_1\uplus s_2) \xrightarrow{\langle d,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup \rdy_2\rangle}
	 (c_1'\|_{cs} c_2', s_1'\uplus s_2')}  & 
\end{eqnarray*}
} \vspace*{-4mm} 
\caption{Selection of rules in small-step operational semantics}
\label{fig:small-step} \vspace*{-2mm}
\end{figure*} }
...
The following theorem shows the equivalence between big-step and small-step semantics. %Full proofs are given in Appendix~\ref{app:full-semantics}.
%First, we need some preliminary concepts.
\begin{theorem} \label{thm:small-big-semantics}
%\begin{itemize}
    %\item 
    \vspace*{-3mm}
...
We first present the syntax for terms. The language consists of terms of several types. In the proof of relative completeness w.r.t FOD, we will describe how to encode them  %each type into real numbers, and define each of the special constructs 
to FOD.
{\small \[ \vspace*{-2mm}
\begin{array}{rll}
...
\quad \prftree[r]{SyncUnPairE4}{ch\in cs}{\langle ch\triangleright,v\rangle^\chop tr_1 \|_{cs} \langle d,\vec{p},\rdy \rangle^\chop tr_2 \Downarrow \delta} & \\
%\qquad\qquad 
...
\quad \prftree[r]{SyncWaitE2}{\langle d,\vec{p}_1,\rdy_1\rangle^\chop tr_1 \|_{cs} \langle d,\vec{p}_2,\rdy_2\rangle^\chop tr_2 \Downarrow \textit{tr}}{\compat(\rdy_1,\rdy_2)}
{\exists \textit{tr}'.\, \textit{tr} = \langle d,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup\rdy_2\rangle ^\chop \textit{tr}' \wedge tr_1\|_{cs} tr_2 \Downarrow \textit{tr}'} & \\%[1mm] 
...
{\exists \textit{tr}'.\, \textit{tr} = \langle d_1,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup\rdy_2\rangle ^\chop \textit{tr}' \wedge tr_1\|_{cs} \langle d_2-d_1,\vec{p}_2(\cdot+d_1),\rdy_2\rangle^\chop tr_2 \Downarrow \textit{tr}'} & \\%[1mm] 
&  
\oomit{\quad
...
\oomit{
& \prftree[r]{SyncEmpE1}{\epsilon \|_{cs} \epsilon \Downarrow \textit{tr}}{\textit{tr} = \epsilon} \quad
\prftree[r]{SyncEmpE2}{\langle d,\vec{p},\rdy\rangle^\chop tr_1 \|_{cs} \epsilon \not\Downarrow \textit{tr}} & \\
%\quad 
& \prftree[r]{SyncEmpE3}{\langle ch\triangleright,v\rangle^\chop tr_1 \|_{cs} \epsilon \Downarrow \textit{tr}}{ch\notin cs\wedge \exists \textit{tr}'.\, \textit{tr}=\langle ch\triangleright,v\rangle^\chop \textit{tr}' \wedge tr_1 \|_{cs} \epsilon \Downarrow \textit{tr}'} & }
...
\oomit{
\begin{enumerate}
	\item If both sides are communication events, where the channel lies in \sm{cs}, then the two events must synchronize with each other. That is, they have the same channel and value, one of them is input and the other is output (rule SyncPairE).
	
	\item If one side is a communication event with channel in \sm{cs}, and the other side is a communication event with channel not in \sm{cs}, then the event with channel not in \sm{cs} has 
	a priority according to rule SyncUnpairE1.
	
	\item If both sides are communication events with channels not in \sm{cs}, then either of them can 
	happen first, see rule SyncUnpairE2.
	
	
	
	\item If one side is a continuous event, and the other side is a communication event with channel not in \sm{cs}, then the communication event should  
	happen first (rule SyncUnpairE3).

 \item If one side is a continuous event, and the other side is a communication event in \sm{cs}, then this produces a deadlock  (rule SyncUnpairE4).
	
	\item If both sides are continuous events, then the two ready sets must be compatible (rule SyncWaitE1). Then there are two cases depending on the comparison between durations of the two continuous events. If the two durations are equal, they can be synchronized with each other (rule SyncWaitE2). Otherwise, the shorter one synchronizes with the initial part of the longer one first (rule SyncWaitE3).
	
	\item Finally, if one side is the empty trace, then the only possibility is that the other side consists of communication events with channel not in $cs$ (rules SyncEmptyE1-3).
\end{enumerate}
}
...
\begin{eqnarray*}
	...
	\qquad \prftree[r]{ Output }{\left\{
 	...
 	& \prftree[r]{ Int }{
 		\begin{array}{l}
 			...
 			P\rightarrow (\neg B \rightarrow R)  \\
 			P\rightarrow \forall d>0.\, (\forall t\in[0,d).\, B[\vec{p}_{\vec{x}_0}(t)/\vec{x}]  \wedge \neg B[\vec{p}_{\vec{x}_0}(t)/\vec{x}] ) \to   R[\vec{p}_{\vec{x}_0}(d)/\vec{x}, \tracev^\chop \langle d, \vec{p}_{\vec{x}_0}, \rdy(\cup_{i\in I} ch_i*)\rangle/\tracev]
 		\end{array}
 	}
 ...
 & \\[1mm]	 
  & 	 \prftree[r]{Recursion}{\spec{P}{\mu X. c}{Q} \vdash \spec{P}{c[\mu X. c/X]}{Q}}
 	{\spec{P}{\mu X. c}{Q}}
  \quad
 	\prftree[r]{Subst}{\{\overline{z}\} \cap \var(c) = \emptyset}{\forall i. y_i \in \var(c) \rightarrow z_i \notin  \freev(Q)}{\spec{P}{ c}{Q}}{\spec{P[\overline{y}/\overline{z}]}{c}{Q[\overline{y}/\overline{z}]}}&\\
 ...
 %	\\
 %	&{\color{red}\prftree[r]{TrInd}{\spec{P \wedge tr = \epsilon}{c}{Q \wedge tr = h_1}}{tr \notin \textit{free}(P) \cup \textit{free}(Q)}{\spec{P \wedge tr = h_0}{c}{Q \wedge tr = {h_0}^\chop h_1}}} &  
 \vspace*{-6mm} 
...
\item \oomit{As discussed in \cite{Lamport84}, communication events can  essentially be encoded as assignment semantically, thus  can be axiomatized by an axiom similar to  
assignment axiom in classical Hoare logic. But for 
HCSP,}
...
possibilities,
  %For communication events, as the environment is unknown, the weakest precondition must cover all : 
%the event occurs immediately, waiting for some time before occurring, and waiting indefinitely, 
see axioms Output and Input. These axioms also provide a way 
to compute the weakest precondition 
w.r.t. a given postcondition. % and an HCSP process. 
%as in classical Hoare logic. 
...
\item  %{\color{red}
The rule for recursion (rule Recursion) is standard~\cite{DBLP:series/lnm/Hoare71}, for which the conclusion for the recursion is assumed as a hypothesis in the proof of the body.  
%The rule for repetition using a loop invariant can be derived from the  recursion rule.

\item Moreover, for completeness, several  general rules including invariance, conjunction,  substitution and consequence are added (rules Inv, Conj, Subst, and Conseq). 
\end{itemize}
...
Reasoning recursions, particular loops, needs invariants. Although continuous evolution can be reasoned about by explicitly using  its solution as indicated in rule Cont, differential invariants can 
ease the reasoning very much as obtaining a solution of  a ODE is mathematically  difficult. As we will show in Sect.~\ref{sec:simplified-rules}, invariant-based rules for reasoning ODEs are derivable from HHL. 

So, as in classical Hoare logic, invariant generation plays a central role in deductive verification of HSs. 
...
%with respect to a so-called \emph{approximate bisimulation}. 
...
\section{Continuous relative completeness}
...
%such that \sm{\wp(c,Q)(s,\textit{tr})} holds if and only if for all runs  from \sm{(c,s)} to some \sm{(s',\textit{tr}')} according to big-step semantics, \sm{Q(s',\textit{tr}^\chop \textit{tr}')} holds.
...
The only $\wp$ rule that does not allow direct computation is that for recursion (repetition). Instead it satisfies the following recursive equation:
\[  
\wp(\mu X. c,Q) = \wp(c[\mu X. c/X],Q).
\]
%This means, executing \sm{\mu X. c} is equivalent to executing unfolding of \sm{\mu X.c}. 
The recursive  equation is not solvable in general, but it provides a way to approximate $\wp(\mu X. c,Q)$ according to provided invariants for $\mu X. c$. So,  as the verification of programs with classical Hoare logic, invariant generation plays a central role in the verification of HSs with HHL. 
...
\oomit{
Note that \sm{P} is a predicate on the global state only, while \sm{\sp(s,P)} is a predicate on the global state and trace. This asymmetry is due to the consideration that there is no sequential composition of parallel processes, so 
no trace is accumulated before the parallel composition starts.}
...
\oomit{
This proves the following theorem, under the assumption of expressibility of predicates and provability of entailments in the underlying logical system:
\begin{theorem}\label{thm:parallel-complete}
	For a parallel process \sm{c} and a predicate \sm{P} on the global state, that can be written as a conjunction of predicates on individual processes of \sm{c}, and if a Hoare triple \sm{\{P\}\ c\ \{Q\}} is valid, then it is provable in the above system.
\end{theorem}}
...
{\small  \[
	\hspace*{-2mm} \begin{array}{ll}
		 & Step_{cs}(tr_1,tr_1',tr_2,tr_2',\textit{blk}) \\	
		= & \exists ch,\,v.\, (ch\in cs\wedge tr_1=\langle ch!,v\rangle^\chop tr_1'\,\wedge 
		%\\ & & \quad \quad 
		tr_2=\langle ch?,v\rangle^\chop tr_2' \wedge \textit{blk}=\langle ch,v\rangle)\,\vee \\
	   & \exists ch,\,v.\, (ch\in cs\wedge tr_1=\langle ch?,v\rangle^\chop tr_1'\,\wedge 
	  %\\& & \quad \quad 
	  tr_2=\langle ch!,v\rangle^\chop tr_2' \wedge \textit{blk}=\langle ch,v\rangle)\,\vee \\
	   & \exists {ch\triangleright},\,v.\, (ch\notin cs\wedge tr_1=\langle ch\triangleright,v\rangle^\chop tr_1'\, \wedge 
	  %\\& & \quad \quad 	  
	  tr_2=tr_2'\,\wedge \textit{blk}=\langle ch\triangleright,v\rangle)\,\vee \\
	 &  \exists {ch\triangleright},\,v.\, (ch\notin cs\wedge tr_1=tr_1'\,\wedge 
	 %\\& & \quad \quad 
	tr_2=\langle ch\triangleright,v\rangle^\chop tr_2'\wedge \textit{blk}=\langle ch\triangleright,v\rangle)\,\vee \\
	 & \exists d,\,\vec{p}_1,\,\rdy_1,\,\vec{p}_2,\,\rdy_2.\, (\operatorname{compat}(\rdy_1,\rdy_2)\,\wedge 
	%\\& & \quad \quad 
	tr_1=\langle d,\vec{p}_1,\rdy_1\rangle^\chop tr_1'\,\wedge tr_2=\langle d,\vec{p}_2,\rdy_2\rangle^\chop tr_2'\,\wedge 
	 \\
	 & \qquad \qquad 
	\textit{blk}=\langle d, \vec{p}_1\uplus \vec{p}_2,\rdy_1\cup \rdy_2\rangle)\,\vee \\
		& \exists d_1,\,d_2,\,\vec{p}_1,\,\rdy_1,\,\vec{p}_2,\,\rdy_2,\,tr_1''.\,
		(d_1>d_2\,\wedge  
		%\\& & \quad \quad  
		\operatorname{compat}(\rdy_1,\rdy_2)\,\wedge tr_1=\langle d_1,\vec{p}_1,\rdy_1\rangle^\chop tr_1''\,\wedge  \\
		&\qquad\quad tr_1'=\langle d_1-d_2,\vec{p}_1(\cdot+d_2),\rdy_1\rangle^\chop tr_1''\,\wedge 
	%\\& & \quad \quad 
	tr_2=\langle d_2,\vec{p}_2,\rdy_2\rangle^\chop tr_2'\,\wedge 
	%\\ & & \quad \quad 	
	\textit{blk}=\langle d_2,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup \rdy_2\rangle)\, \vee \\
		& \exists d_1,\,d_2,\,\vec{p}_1,\,\rdy_1,\,\vec{p}_2,\,\rdy_2,\,tr_2''.\,
		(d_1<d_2\,\wedge 
		%\\& & \quad \quad 
		\operatorname{compat}(\rdy_1,\rdy_2)\wedge 
		 tr_1=\langle d_1,\vec{p}_1,\rdy_1\rangle^\chop tr_1'\, \wedge \\
		&\qquad\quad 
		tr_2=\langle d_2,\vec{p}_2,\rdy_2\rangle^\chop tr_2''\,\wedge 
		%\\& & \quad \quad 
		tr_2'=\langle d_2-d_1,\vec{p}_2(\cdot+d_1),\rdy_2\rangle^\chop tr_2''\, \wedge 
		%\\& &\quad \quad 
		\textit{blk} = \langle d_1,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup \rdy_2\rangle).
	\end{array} \]
	 }
}
...
%The conclusion  indicates that $Q$ should hold on the ending state and with $\langle d,\vec{p}_{\vec{x}_0},\emptyset\rangle$ appended to the trace in case such $d$ does exist.  
...
\oomit{
We conjecture that \oomit{if we introduce the auxiliary rule as in \cite{Apt83,Owicki76}\footnote{Currently, we do not know whether the rule for auxiliary variables is derivable or not, but it is not necessary in the current proof system.},  then}  the discrete fragment of our proof system is equivalent to the proof system for CSP given in \cite{Apt83}, therefore it is complete in Cook's sense, i.e.,
}
...
\paragraph{Implementation} The proof system with the above simplification is implemented in Isabelle/HOL. It depends upon the formalization of ODEs in Isabelle/HOL~\cite{ImmlerH12}. The following two examples are verified using the implementation.

\oomit{The first example uses explicit solutions of ODEs. In the second example, we show the use of an invariant, together with compositional reasoning.

\oomit{The proof system is implemented in Isabelle/HOL, including the proof of equivalence between big-step and small-step semantics, statement of the Hoare rules, and soundness of the proof system in terms of big-step semantics. The definition of the proof system is built upon the formalization of ordinary differential equations in Isabelle/HOL~\cite{ImmlerH12}. The example given in this section is checked using this implementation.
}
\oomit{
Similarly, the assertions involving waiting and ODE evolution are:
\[ \textsf{wait}(\vec{x}_0,d,\rdy) \triangleq \textit{tr} = \langle d,I_{\vec{x}_0},\rdy\rangle \]
\vspace{-0.7cm}
\begin{align*}
...
\begin{align*}
	...
	&\quad (\exists v.\, \textit{tr} = \langle ch?, v \rangle \wedge \vec{x}_2 = \vec{x}_0[y\mapsto v])\, \vee \\
	&\quad \exists v.\, \exists d>0.\, (\forall t\in[0,d).\, B[\vec{p}_{\vec{x}_0}(t)/\vec{x}]) \,\wedge \\
	&\qquad\quad \textit{tr} = \langle d,\vec{p}_{\vec{x}_0},\rdy\rangle^\chop \langle ch?,v\rangle \wedge \vec{x}_2=\vec{p}(d)[y\mapsto v]
\end{align*}
\vspace{-0.7cm}
\begin{align*}
...
We further define two similar assertions, that specify that the trace consists of evolution along a path followed by an immediate communication:
\begin{align*}
	&\textsf{pathOut}(d,\vec{p},ch,e,\rdy) \triangleq \\
	&\quad \mbox{if } d = 0 \mbox{ then } \textit{tr} = \langle ch!, e(\vec{p}(0)) \rangle
	 \\
...
\[\begin{array}{l}
	\textsf{wait}(\vec{x}_0,d) \triangleq \textsf{wait}(\vec{x}_0,d,\emptyset) \\
...
With the above definitions, we can simplify the weakest liberal precondition rules stated in Section \ref{sec:hoare}. 
The set of full calculus is given in the appendix, here we just list the rule for 
the interrupt below as an example. 
\oomit{
For the rules for external choice and interrupt, with a fixed configuration of communications, there are equivalent rules that are more clearly in weakest liberal precondition form. We give two examples.

For external choice, if there are two input communications, the rule is:
\[ \prftree
{\spec{Q_1}{c_1}{R}}
{\spec{Q_2}{c_2}{R}}
{\begin{array}{l}
	\left\{\begin{array}{l}
		\forall v.\,\wand{\textsf{in}(\vec{x}_0,ch_1,v,\{ch_1?,ch_2?\})}{Q_1(\vec{x}_0[x\mapsto v])}
		\,\wedge \\
		\forall v.\,\wand{\textsf{in}(\vec{x}_0,ch_2,v,\{ch_1?,ch_2?\})}{Q_2(\vec{x}_0[y\mapsto v])} \,\wedge \\
		\textsf{wait}(\vec{x}_0,\infty,\{ch_1?,ch_2?\}) \wandop R
	\end{array}
	\right\}\,\\
	\quad ch_1?x\to c_1\lr ch_2?y\to c_2 \, \\
	\{R\}	 
 \end{array}}
\] }

For interrupt, consider the case with one input communication, the rule is:
\[ \prftree
{\spec{Q}{c}{R}}
{
	\begin{array}{l}
		\left\{\begin{array}{l}
			\forall \vec{x_2}.\,\wand{\textsf{odein}(\vec{x}_0,\vec{\dot{s}}=\vec{e},B,\vec{x}_2,ch,x,\{ch?\})}{Q(\vec{x_2})}
			\,\wedge \\
			\forall \vec{x_2}.\,\wand{\textsf{ode}(\vec{x}_0,\vec{\dot{s}}=\vec{e},B,\vec{x}_2,\{ch?\})}{R(\vec{x_2})}
		\end{array}
		\right\}\, \\
...
Similarly, we can define a simplified strongest postcondition calculus for HHL using the assertion language above. The most important rules are for parallel and communication interrupt. The rule for the former is same as 
the one in Fig~\ref{fig:hoarelogic}, the rule for the latter is given below. 
\oomit{
To make the statement of the rules more intuitive, we restrict to the case where the state is fixed in the precondition, and the trace satisfies some assertion $P$ on traces. The simplified strongest postcondition rules are presented in Fig.~\ref{fig:simpsp}. 
Again, with fixed configuration of communications, we can obtain simpler rules.}
...
{\{\vec{x}_0 = \vec{x}_0 \wedge P\} ch_1?x\to c_1\lr ch_2?y\to c_2\\
	\{Q_1 \vee Q_2 \}}
\]
For interrupt with a single input, we have:
\[ \prftree
...
\oomit{
Since the strongest postcondition rules only deal with the case where the initial state is fixed, we also need rules for situations where the precondition or postcondition contains an existence quantifier. The following rules are clear.
\[ \prftree[r]{ExPre}
{\forall v.\, \spec{P(v)}{c}{Q}}
{\spec{\exists v.\, P(v)}{c}{Q}}
\]
\[ \prftree[r]{ExPost}
{\exists v.\, \spec{P}{c}{Q(v)}}
{\spec{P}{c}{\exists v.\, Q(v)}}
\] }

\oomit{
\subsection{Example with loop and parallel}

First, we consider following process:
\[ (x := x + 1; ch!x)^* \| (ch?y)^* \]

For the left side, the overall Hoare triple is:
{\small
\begin{align*}
...
For the right side, the overall Hoare triple is:
{\small
\begin{align*}
...
\[ \mathit{plant} \triangleq (t := 0; \langle \dot{v}=a, \dot{t}=1 \& t < 1\rangle; p2c!v; c2p?a)^* \]
This models a vehicle that periodically sends its velocity, receives acceleration command $a$, and evolves according to the ODE $\dot{v}=a$. Suppose we wish to keep the velocity around 10, then a simple control that achieves this is as follows:
\[ \mathit{ctrl} \triangleq (p2c?v; \IFE{v < 10}{c2p!1}{c2p!(-1)})^* \]

The combined system is:
\[ \mathit{system} \triangleq \mathit{plant}\,\|_{\{p2c,c2p\}}\,\mathit{ctrl} \]

For the invariant of process \textit{plant}, we define the function \sm{\textsf{plant}(v_0,a_0,as)}, representing the behavior of \textit{plant} when the starting velocity and acceleration are $v_0$ and $a_0$, and the sequence of values received via $c2p$ is $as$.
\[ \hspace*{-6mm} \begin{array}{rl}
&\textsf{plant}(v_0,a_0,[]) =\textsf{emp} \\
&\textsf{plant}(v_0,a_0,a\cdot as') =
\textsf{traj}(1, [v\mapsto v_0+a_0t, a\mapsto a_0, t\mapsto t]) \joinop \\
& \quad \textsf{out}([v\mapsto v_0+a_0, a\mapsto a_0, t\mapsto 1], p2c, v_0+a_0) \joinop \\
& \quad \textsf{in}([v\mapsto v_0+a_0, a\mapsto a_0, t\mapsto 1], c2p, a) \joinop \\
& \quad \textsf{plant}(v_0+a_0,a,as')
\end{array} \]
We also define a function $s_{\textit{plant}}(v_0,a_0,as)$ giving the ending state of \textit{plant}. The details are omitted here. Applying the repetition rule and the above strongest postcondition rules, we get the following:
\begin{align*}
	\{ & s = [v\mapsto v_0, a\mapsto a_0] \wedge \textsf{emp} \} \\
	& (t := 0; \langle \dot{v}=a, \dot{t}=1 \& t < 1\rangle; p2c!v; c2p?a)^* \\
	\{ & \exists as.\, s = s_{\textit{plant}}(v_0,a_0,as) \wedge \textsf{plant}(v_0,a_0,as) \}
\end{align*}
For the invariant of \textit{ctrl}, we define the function $\textsf{ctrl}(v_0,vs)$, representing the behavior of \textit{ctrl} when the starting value of $v$ is $v_0$, and the sequence of values received via $c2p$ is $vs$. We use the notation $b?v:w$ to denote the value that equals $v$ if $b$ is true and $w$ otherwise.
\[
\begin{array}{ll}
	&\textsf{ctrl}(v_0,[]) = \textsf{emp} \\
	&\textsf{ctrl}(v_0,v\cdot vs') = \textsf{in}([v\mapsto v_0],p2c,v) \joinop\\
	&\quad \textsf{out}([v\mapsto v],c2p,v<10?1:-1) \joinop \\ &\quad \textsf{ctrl}(v,vs')
\end{array} \]

We likewise define a function $s_{\textit{ctrl}}(v,vs)$ giving the ending state of control. Applying the repetition rule and strongest postcondition rules gives the following:
\begin{align*}
	\{ & s = [v\mapsto v_0] \wedge \textsf{emp} \} \\
	& (p2c?v; \IFE{v < 10}{c2p!1}{c2p!-1})^* \\
	\{ & \exists vs.\, s = s_{\textit{ctrl}}(v_0,vs) \wedge \textsf{ctrl}(v_0,vs) \}
\end{align*}

The final step is to compute the synchronization of the postconditions. Define an invariant on velocity and acceleration as follows:
\[ \textrm{inv}(v,a) \triangleq v \in [9,11] \wedge a = (v<10?1:-1). \]
The invariant for the whole system is
\[
\begin{array}{ll}
	&\textsf{system}(0) = \textsf{emp} \\
	&\textsf{system}(n + 1) = \exists v_0,\,a_0.\, \textrm{inv}(v_0,a_0)\,\wedge \\
	&\quad \textsf{traj}(1,[v\mapsto v_0+a_0t, a\mapsto a_0, t\mapsto t]) \joinop \\
	&\quad \textsf{IO}(p2c,v_0+a_0) \joinop
	\textsf{IO}(c2p, v_0+a_0<10?1:-1) \joinop \\ &\quad \textsf{system}(n).
\end{array} \]

We now show the computation of synchronization. The lemma to be proved by induction is (taking $cs=\{p2c,c2p\}$):
\[ \textrm{inv}(v_0,a_0)\to \textsf{plant}(v_0,a_0,as) \|_{cs} \textsf{ctrl}(v_0',vs) \rightarrow \textsf{system}(|as|) \]
We show the interesting part of the proof below, for the case where both \sm{as} and \sm{vs} are nonempty.
\begin{align*}
	& \textsf{plant}(v_0,a_0,a\cdot as) \|_{cs} \textsf{ctrl}(v_0',v\cdot vs) \\
	\rightarrow\ & \textsf{traj}(1, [v_0+a_0t,a_0,t]) \joinop
	\textsf{out}([v_0+a_0,a_0,1], p2c, v_0+a_0) \joinop\\
	&\quad
	\textsf{in}([v_0+a_0,a_0,1], c2p, a) \joinop \textsf{plant}(v_0+a_0,a,as) \|_{cs} \\
	& \textsf{in}([v_0],p2c,v) \joinop
	\textsf{out}([v],c2p,v<10?1:-1) \joinop \textsf{ctrl}(v,vs') \\
	\rightarrow\ & \textsf{traj}(1, [v_0+a_0t,a_0,t]) \joinop \\
	&(\textsf{out}([v_0+a_0,a_0,1], p2c, v_0+a_0) \joinop
	\textsf{in}([v_0+a_0,a_0,1], c2p, a) \joinop \\
	&\ \textsf{plant}(v_0+a_0,a,as) \|_{cs} \\
	&\ \textsf{in}([v_0],p2c,v) \joinop
	\textsf{out}([v],c2p,v<10?1:-1) \joinop \textsf{ctrl}(v,vs')) \\ 
	\rightarrow\ & v = v_0+a_0 \wedge \textsf{traj}(1, [v_0+a_0t,a_0,t]) \joinop
	\textsf{IO}(p2c,v) \joinop \\
	&(\textsf{in}([v_0+a_0,a_0,1], c2p, a) \joinop \textsf{plant}(v_0+a_0,a,as) \|_{cs} \\ 
	&\ \textsf{out}([v],c2p,v<10?1:-1) \joinop \textsf{ctrl}(v,vs')) \\ 
	\rightarrow\ & v = v_0+a_0 \wedge a = v<10?1:-1 \wedge \textsf{traj}(1, [v_0+a_0t,a_0,t]) \joinop\\
	& \textsf{IO}(p2c,v) \joinop \textsf{IO}(c2p,a) \joinop (\textsf{plant}(v_0+a_0,a,as) \|_{cs} \joinop \textsf{ctrl}(v,vs')) \\ 
	\rightarrow\ & v = v_0+a_0 \wedge a = v<10?1:-1 \wedge \textsf{traj}(1, [v_0+a_0t,a_0,t]) \joinop\\
	& \textsf{IO}(p2c,v) \joinop \textsf{IO}(c2p,a) \joinop \textsf{system}(|s|) \\
	\rightarrow\ & \textsf{system}(|a\cdot as|) 
\end{align*}
The first step expands the definition of \sm{\textsf{plant}} and \sm{\textsf{ctrl}}. The second step uses synchronization of \sm{\textsf{traj}} and \sm{\textsf{in}}. The third and fourth steps use synchronization between \sm{\textsf{in}} and \sm{\textsf{out}}, obtaining the values of $v$ and $a$ in the process. In the fifth step, we use the inductive hypothesis, along with the arithmetic property that \sm{\textrm{inv}(v_0,a_0)} implies \sm{\textrm{inv}(v_0+a_0,v_0+a_0<10?1:-1)}. The final step uses the definition of \sm{\textsf{system}}. The final Hoare triple obtained is:
\[ \spec{ s = [v\mapsto v_0, a\mapsto a_0]}{\textit{system}}
	{\exists n.\, \textsf{system}(n)} \]
This shows that at the beginning of every iteration, the state of \textit{plant} satisfy the invariant $\mathrm{inv}$. It is not difficult to derive from this that $\mathrm{inv}$ also holds during the entirety of continuous evolution.

\subsection{Applying differential invariants}

We now show an example of proof using differential invariants. This also gives an example of advantages of compositional reasoning. Consider the following process:
\[
\textit{plant} \triangleq (t := 0; \langle\dot{x}=ax^2, \dot{y}=byx, \dot{t}=1 \& t < 1\rangle; p2c!x; c2p?a; c2p?b)^*
\]

This models a system following differential equation $\dot{x}=ax^2, \dot{y}=byx$ for time intervals of length 1, then sends the value of $x$ and receives new values of two parameters $a$ and $b$. If $a=b$, then the system maintains the invariant $x+y=0$ (Lie derivative of $x+y$ is $ax^2+byx=ax(x+y)$ when $a=b$.\footnote{This example is adapted from \url{https://github.com/LS-Lab/KeYmaeraX-projects/blob/master/benchmarks/basic.kyx}}

The control is given by:
\[
\textit{ctrl} \triangleq (\pwait(t_1)\sqcup \pwait(t_2); p2c?x; c2p!f(x); c2p!f(x))^*
\]

The control periodically reads the value of $x$, then outputs the value of two parameters, both of which computed as $f(x)$ (it does not matter which function $f$ is here). We assume $t_1<1$ and $t_2<1$, so the control side is always ready to receive input before one unit of time, but the exact amount of time is nondeterministic.

As before, the combined system is:
\[ \mathit{system} \triangleq \mathit{plant}\,\|_{\{p2c,c2p\}}\,\mathit{ctrl} \]

The invariant on trace satisfied by plant, given the initial values of $a,b$, and list of control inputs, is as follows:
\[
\begin{array}{ll}
&\textsf{plant}(a,b,[]) = \textsf{emp} \\
&\textsf{plant}(a,b,(a',b')\cdot as') = \\
&\quad (a=b \longrightarrow \textsf{traj}_\textsf{inv}(1,x+y=0) \joinop \\
&\qquad\qquad\quad\ \ \textsf{out}_\textsf{inv}(p2c, x) \joinop
\textsf{in}_\textsf{inv}(c2p, a') \joinop
\textsf{in}_\textsf{inv}(c2p, b') \joinop \\
&\qquad\qquad\quad\ \ \textsf{plant}(a',b',as'))
\end{array} \]
This invariant states the fact that if the parameters $a$ and $b$ are the same, then the system maintains the invariant $x+y=0$. If $a\neq b$, then no condition is imposed on the trace. Hence, the system makes an assumption on the control to always return two equal values for $a$ and $b$. The following Hoare triple can be shown for \textit{plant}:
\begin{align*}
	\{ & a = a_0 \wedge b = b_0 \wedge x + y = 0 \wedge \textsf{emp} \} \\
	& \textit{plant} \\
	\{ & \exists vs.\, \textsf{plant}(a_0,b_0,vs) \}
\end{align*}

The invariant satisfied by \textit{ctrl} is as follows:
\[
\begin{array}{ll}
&\textsf{ctrl}([]) = \textsf{emp} \\
&\textsf{ctrl}(x\cdot xs') = \\
&\quad \exists d<1.\,
\textsf{traj}_{\textsf{inv}}(d,\textsf{true})\joinop \\
&\qquad\qquad\ \textsf{in}_\textsf{inv}(p2c,x) \joinop
\textsf{out}_\textsf{inv}(c2p, f(x)) \joinop
\textsf{out}_\textsf{inv}(c2p, f(x)) \joinop \\
&\qquad\qquad\ \textsf{ctrl}(xs')
\end{array}
\]
This invariant only guarantees that the two outputs along channel $c2p$ are the same. The following Hoare triple can be shown for \textit{ctrl}:
\begin{align*}
	\{ \textsf{emp} \}\ \textit{ctrl}\ 
	\{ \exists xs.\, \textsf{ctrl}(xs) \}
\end{align*}

The two sides can be combined to form the invariant of the system:
\[
\begin{array}{ll}
&\textsf{system}([]) = \textsf{emp} \\
&\textsf{system}((x,v)\cdot xs') = \\
&\quad \textsf{traj}_{\textsf{inv}}(1, x+y=0) \joinop \\
&\quad \textsf{IO}(p2c,x) \joinop
\textsf{IO}(c2p,v) \joinop
\textsf{IO}(c2p,v) \joinop \\
&\quad \textsf{system}(xs')
\end{array}
\]

The Hoare triple for the whole system is:
\begin{align*}
	\{ & a = a_0 \wedge b = b_0 \wedge x + y = 0 \wedge \textsf{emp} \} \\
	& \textit{system} \\
	\{ & \exists xs.\, \textsf{system}(xs) \}
\end{align*}

We will only give the basic idea of the derivation here. The guarantee from \sm{\textsf{control}} that the output parameters for $a$ and $b$ are the same is used to discharge the assumption $a=b$ in \sm{\textsf{plant}}. The condition that control waits for at most one time unit means it will always be the waiting side for the communication on $p2c$. Note the Hoare triples we proved for \textit{plant} and \textit{ctrl} are far from specifying their exact behavior, but instead are abstractions that are just enough for proving the desired property of the system (that $x+y=0$ is an invariant). This gives an example of compositional reasoning that is permitted by the logical framework.
}
\paragraph{Case 1: Lunar lander}
Firstly,  we demonstrate how our proof system works in practice by showing a simplified lunar lander example adapted from 
\cite{Zhao14}.  
...
\vspace{-4mm} \noindent where $v_{s}$ is the target velocity we want to maintain, i.e. -1.5m/s here. Since the origin ODE is non-polynomial, we replace $\frac{F_c}{m}$ by a new variable $w$. 
...
\[
\begin{array}{ll}
\textit{plant} \triangleq &(\langle\dot{v}=w-3.732, \dot{w}=\frac{w^{2}}{2500},\dot{t}=1 \rangle \unrhd \talloblong\\
...
\vspace{-4mm} \noindent The controller updates the thrust every period $T$. After receiving the current velocity and thrust from plant, it updates the thrust according to  $f(v,w) \triangleq -(w - 3.732) * 0.01 + 3.732 - (v + 1.5) * 0.6$. 
...
{\small
\begin{align*} %{rl}
\textsf{plant}(v,w,[]) = & \textsf{emp} \\
\textsf{plant}(v,w,w'\cdot ws') =  & \exists \vec{x}.\,
\textsf{trout}([v,w,0],\vec{x},\textit{chv},\vec{x}\vert_{v},\{\textit{chv}\})\joinop \\[-1mm]
& \hspace*{-8mm}  \textsf{out}(\textit{chw},\vec{x}\vert_{w}) \joinop
\textsf{in}(\textit{chc}, w') \joinop  \textsf{plant}(\vec{x}\vert_{v},w',ws') \\[-1mm]
  %\]
%\[ \vspace*{-2mm} 
%\begin{array}{rl}
 \textsf{trout}(\vec{x}_{0},\vec{x}_{1},\textit{chv},v,\textit{rdy}) 
\triangleq &\exists d \textsf{ } \vec{p}.\, \tracev = \langle d,\vec{p},\rdy\rangle^{\chop} \langle ch!,v \rangle \\[-1mm]
&\wedge \vec{p}(0) = \vec{x}_{0} 
\wedge \vec{p}(d) = \vec{x}_{1} \\[-1mm]
%\end{array}
%\end{array}
%\]
...
& \hspace*{-2.5cm} 	\{  v = v_0 \wedge w = w_0 \wedge t = 0 \wedge \textsf{emp} \}  
  \textit{plant}  
	\{   \exists ws.\, \textsf{plant}(v_0,w_0,ws) \} \vspace*{-2mm} 
\end{align*} }

\vspace{-4mm}
Similar for \textsf{ctrl}, it follows 
{\small  \vspace*{-2mm} 
\begin{align*} \vspace*{-2mm}  %{ll}
&\textsf{ctrl}([]) = \textsf{emp} \\
&\textsf{ctrl}(v,w,(v',w')\cdot \textit{vws}') =   \textsf{traj}(T,I_{[v,w]})\joinop \textsf{in}(\textit{chv},v') \joinop\\
&\qquad\qquad\ 
\textsf{in}(\textit{chw}, w') \joinop
\textsf{out}(\textit{chc}, f(v',w')) \joinop  \textsf{ctrl}(v',w',\textit{vws}') \\ %\vspace*{-2mm} 
%\end{array}
%\]
%\begin{align*} \vspace*{-4mm} 
&	\{   v = v_0 \wedge w = w_0 \wedge \textsf{emp} \}  \textit{ctrl}  
	\{   \exists vws.\, \textsf{ctrl}(v_0,w_0,\textit{vws}) \}
\end{align*}
}
...
%we can get 
a trace invariant of the whole system as 

{\small \hspace*{-8mm} \[\hspace*{-5mm}
\begin{array}{ll}
&\textsf{system}(v0,w0,v1,w1,0) = \textsf{emp} \\
&\textsf{system}(v0,w0,v1,w1,n+1) = 
%&\quad
\exists \vec{p} . \, \textsf{traj}(T, \vec{p}_{[v0,w0,0]}\uplus I_{[v1,w1]},\{chv\}) \joinop \\
&\qquad
\textsf{IO}(\textit{chv},\vec{p}(T)\vert_{v}) \joinop
\textsf{IO}(\textit{chw},\vec{p}(T)\vert_{w}) \joinop
%\\
%&\qquad
\textsf{IO}(\textit{chc},f(\vec{p}(T)\vert_{v},\vec{p}(T)\vert_{w})) \joinop \\
&\qquad\textsf{system}(\vec{p}(T)\vert_{v},f(\vec{p}(T)\vert_{v},\vec{p}(T)\vert_{w}),\vec{p}(T)\vert_{v},\vec{p}(T)\vert_{w},n)
\end{array}
\] }

\vspace{-3mm}
To prove the goal, i.e.,  the velocity of lander keeps within $(-1.45,$ $-1.55)$, we invoke the ODE invariant generation tool and obtain 
an invariant $\textit{p}(t,v,w)\le 0$, where $\textit{p}(t,v,w)$ is a polynomial, with
%satisfying the following four conditions: % is obtained:

\vspace{-2mm}
{\small \[
\begin{array}{ll}
 & \textit{p}(0,-1.5,5670/1519)\le0 \wedge 
 0\le t \le T \rightarrow \textit{p} = 0 \rightarrow \mathcal{L}_{\overline{e}}(\textit{p})<0\\
 \wedge & 
 \textit{p}(T,v,w)\le 0 \rightarrow \textit{p}(0,v,f(v,w))\le 0 \\
  \wedge &
 (0\le t \le T \wedge\textit{p}\le 0) \rightarrow (v+1.45)*(v+1.55)<0 
\end{array}
\] \vspace*{-3mm} }
\iffalse
Using the differential invariant  rule with some modifications to its form 
...
By the differential invariant rule we can prove that in each period, the trajectory of the ODE satisfies $\textit{p} \le 0$, and  $-1.55\le v\le -1.45$. 

The property to be verified for the whole  system is:

\vspace{-2mm}
{\small \begin{align*}
	\{ & (v = -1.5 \wedge w = 5670/1519 \wedge t = 0) \uplus ( v = v1 \wedge w = w1)\wedge \textsf{emp} \} \\
	& \quad \quad  \mathit{plant}\,\|_{\{\textit{chv,chw,chc}\}}\,\mathit{ctrl} \quad 
	\{  \exists n.\, \textsf{system}(-1.5,5670/1519,v1,w1,n) \} \vspace*{-4mm}
\end{align*}}

\vspace{-3mm}
Compared to the proof in \cite{Zhao14}, the above proof for  
the parallel of  $\mathit{plant}$ and $\mathit{ctrl}$ 
 with communications is conducted in a much easier and more naturally compositional way. 

\paragraph{Case 2: Scheduler with two tasks}
Next, we apply the proof system to analyze a scheduler process controlling two modules. This scheduling problem  comes from translation of the combination of  AADL and Simulink/Stateflow  in~\cite{DBLP:journals/tcs/XuWZJTZ22}. 
We focus on the interactions of the modules with the scheduler, and prove the correctness of the scheduler,  i.e., at at any time, the running module is always with the highest priority among modules that are ready. This case study demonstrates the capability of our proof system, as it cannot be proved with the DC-based HHLProver \cite{WZZ15}.
...
\begin{array}{ll}
     &\mathit{module} := (\\
     &\textrm{if}\ \textit{state}=\textit{WAIT} \ \textrm{then}\   \langle \dot{T}=1\& T\le \textit{period}\rangle;T:=0;
     %& \qquad 
     ...
     & \qquad\langle \dot{T}=1\& T< \textit{period}\rangle\unrhd (\textit{\textit{run\_ch}}?x \rightarrow \textit{state} := \textit{\textit{RUN}});\\
     ...
     & \qquad\langle \dot{T}=1,\dot{C}=1\& T< \textit{period}\land C< \textit{cost} \rangle\unrhd %\\
     %& \qquad 
...
\vspace*{-2mm}
\noindent In each round, depending on the value of $\textit{state}$: if it is  $\textit{WAIT}$, the module waits for $\textit{period}$ time, then resets $T$ and $\textit{ent}$, turns to \textit{READY}; if it is  $\textit{\textit{READY}}$, the module first sends a  request to the scheduler with its priority  $\textit{\textit{req\_ch}}!\textit{prior}$, then waits for a running command $\textit{\textit{run\_ch}}?x$ until the end of this period. Once a running command is received,  turn to  $\textit{RUN}$; if it is  $\textit{RUN}$, $C$ will be reset if this module is not executed. Then the module starts running and can be preempted at any time by $\textit{\textit{pr\_ch}}$
...
&\textsf{s}(k+1,L,ri,rp) =\\
&\quad \exists p. \textsf{in}(req\_ch_i,p,(\{\},\{req\_ch_{i\in I},exit\_ch_{i\in I},free\_ch_{i\in I}\}))\\
...
&\quad \exists g. \textsf{in}(free\_ch_i,g,(\{\},\{req\_ch_{i\in I},exit\_ch_{i\in I},free\_ch_{i\in I}\}))\\
...
\textsf{traj}(\min(\textit{period}_1-T_1,\textit{cost}_1-\textit{ent}_1*C_1))\\
...
\vspace{-2mm} \noindent The first case states that when $\textit{module1}$ is in $\textit{\textit{READY}}$  and $\textit{module2}$ is in $\textit{RUN}$, the operation of $\textit{module2}$ is immediately interrupted by $\textit{module1}$.
...
{\small \begin{align*} 
 ...
 %& \quad \quad \quad  \|_{\{\textit{req\_ch}_2,\textit{free\_ch}_2,\textit{exit\_ch}_2,\textit{run\_ch}_2,\textit{pr\_ch}_2\}}\,\mathit{module2}  
  \quad \quad \{  f(\textit{WAIT}_1,\textit{WAIT}_2) \}
\end{align*} }

\vspace{-4mm}
The proof needs to consider each combination of states in the scheduler and modules (54 cases in total). During synchronization when both processes are waiting, we need to consider three cases depending on the comparison of waiting time on the two sides. Some global invariants need to be shown: the value of $T$ and $C$ in the module will not exceed $\textit{period}$ and $\textit{cost}$ respectively, $\textit{ri}$ indeed represents which module is running,
and so on\oomit{$\textit{prior}$ of module 1 does not enter $L$, 
and $\textit{prior}$ of module 2 can only enter $L$ in the $\textit{\textit{READY}}$ state}. Our proof system is able to deal with these complexities efficiently, and complete the entire proof in around 10,000 lines of code.
...
& \prftree[r]{SkipB}{(\pskip,s) \Rightarrow (s,\epsilon)} 
	...
	\quad
	\prftree[r]{PVarB}{ }
	{(X, s) \Rightarrow (s, \langle \infty,I_s,\{ \, \} \rangle )} & \\[1mm]
 	& 
	\prftree[r]{RecB}{ (c[\mu X. c/X], s)  \Rightarrow (s', \textit{tr})  }
	{ (\mu X. c, s) \Rightarrow (s', \textit{tr})} 
...
\prftree[r]{IntB1}{i\in I}{ch_i*=ch!e}{(c_i,s_1) \Rightarrow (s_2,\textit{tr})}
	...
	{(\exempt{\evo{x}{e}{B}}{i\in I}{ch_i*}{c_i}, s_1) \Rightarrow (s_2,
 	...
 	& \prftree[r]{IntB5}{\neg s(B)
	}{(\exempt{\evo{x}{e}{B}}{i\in I}{ch_i*}{c_i}, s) \Rightarrow (s, \epsilon)}
\quad
	\prftree[r]{IntB6}{
		\begin{array}{cc}
			\vec{p} \mbox{ is a solution of the ODE $\vec{\dot{x}}=\vec{e}$} \\
			\vec{p}(0)=s_1(\vec{x}) \quad \forall t\in[0,d).\,s_1[\vec{x}\mapsto \vec{p}(t)](B) \quad \neg s_1[\vec{x}\mapsto \vec{p}(d)](B)
	\end{array}}
	{(\exempt{\evo{x}{e}{B}}{i\in I}{ch_i*}{c_i}, s_1) \Rightarrow (s_1[\vec{x}\mapsto \vec{p}(d)],\langle d,\vec{p},\rdy(\cup_{i\in I} ch_i*)\rangle )}
	& \\[1mm]
 	& 
	\oomit{
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Recursion: there are two rules PVarB and RecB in big-step semantics. PvarB corresponds to applying PVarS for infinite times. RecB corresponds to applying RecS. 
  ...
  %The case NoSyncIO' is similar, where we only need to work on the right side, and the corresponding small-step rule is ParUnpairS2.
	 ...
	 \item SyncWait2: We have $d_1>d_2$, $tr_1=\langle d_1,\vec{p}_1,\rdy_1\rangle^\chop tr_1'$, $tr_2=\langle d_2,\vec{p}_2,\rdy_2\rangle^\chop tr_2'$, $tr=\langle d_2,\vec{p}_1\uplus \vec{p}_2,\rdy_1\cup\rdy_2\rangle^\chop tr'$, $\compat(\rdy_1,\rdy_2)$ and
...
\begin{proof}[Proof for Theorem~\ref{thm:soundness}]	
	...
	\oomit{	\item Repetition: By induction, we have \sm{\vDash \spec{P}{c}{P}}. According to the operational semantics, there are two cases. The first case is \sm{h'=\epsilon} (rule \sm{\textrm{RepB1}}). Then \sm{\nseman{P}{s}{h}} holds directly. In the second case, there exist \sm{s_1, h_1} and \sm{h_2} such that
...
$\spec{P \wedge \tracev = h'}{c}{Q\wedge \tracev=h'^\chop h}$, as $\vec{x}$ do not occur in $h'$.
...
\item  Rules Cond, IChoice, Recursion, Conj, Inv, Conseq, Subst can be proved easily by induction.  
...
{\small 
	...
	\begin{array}{ll}
		...
		&\qquad (\neg B \rightarrow R)\, \wedge   \forall d>0.\,(\forall t\in[0,d).\,B[\vec{p}_{\vec{x}_0}(t)/\vec{x}])\wedge \neg B[\vec{p}_{\vec{x}_0}(d)/\vec{x}] \to  R[\vec{p}_{\vec{x}_0}(d)/\vec{x}, \tracev^\chop \langle d, \vec{p}_{\vec{x}_0}, \rdy(\cup_{i\in I} ch_i*)\rangle/\tracev]
	\end{array}
	\]
...
The computation of weakest liberal precondition is given in Fig.~\ref{fig:wp}. Justification of this computation is given as part of the soundness proof in Appendix~\ref{app:soundness}. We first introduce a lemma stating that the weakest liberal precondition for recursion is able to deduce all valid facts for recursion according to our proof system.
...
\end{lemma}
\begin{proof}
    The proof of this lemma can be seen from~\cite{DBLP:journals/toplas/Apt81}. During the proof of this lemma, Rules Recursion, Substitution, Conj, Inv and Conseq are all needed. 
\end{proof}

We then present the proof for Lemma~\ref{lem:wp-complete} stating that the weakest liberal precondition for HCSP is derivable from our proof system.
...
\begin{proof}[Proof for Theorem~\ref{thm:continuous-complete}]
	For sequential case, if $\{P\}\ c\ \{Q\}$ is valid, then if $c$ is not recursion, $P\rightarrow \wp(c,P)$ holds, and by assumption provable. Combining with Lemma~\ref{lem:wp-complete}, we have $\{P\}\ c\ \{Q\}$ is provable.  If $c$ is recursion, then by Lemma~\ref{lemma:recursion} and Lemma~\ref{lem:wp-complete}, $\{P\}\ c\ \{Q\}$ is provable. For parallel case, if $\{P\}\ c\ \{Q\}$ is valid, then $\sp(c,P)\rightarrow Q$ holds, and by assumption provable. Combining with Lemma \ref{lem:sp-parallel-complete}, we have $\{P\}\ c\ \{Q\}$ is provable.
\end{proof}
...
\begin{figure*}[ht!]
\begin{eqnarray*}
&\textsf{emp} \|_{cs} \textsf{emp} \rightarrow \textsf{emp} \qquad \textsf{emp} \|_{cs} \textsf{in}(\vec{x}_0,ch,v) \joinop P \rightarrow \textsf{false}
\qquad
\textsf{emp} \|_{cs} \textsf{out}(\vec{x}_0,ch,v) \joinop P \rightarrow \textsf{false} & \\[1mm]
 	& 
\textsf{emp} \|_{cs} \textsf{traj}(d,\vec{p}) \joinop P \rightarrow \textsf{traj}(d,I_\bot\uplus\vec{p}) \joinop (\textsf{emp} \|_{cs} P) 
\qquad
(\textsf{traj}(d,\vec{p}) \joinop P) \|_{cs} (\textsf{traj}(d,\vec{q}) \joinop Q) \rightarrow
\textsf{traj}(d,\vec{p}\uplus \vec{q}) \joinop (P \|_{cs} Q)
& \\[1mm]
 	& 
\prftree{d_1<d_2}{
\begin{array}{l}
  (\textsf{traj}(d_1,\vec{p}) \joinop P) \|_{cs}
  (\textsf{traj}(d_2,\vec{q}) \joinop Q) \rightarrow \\
  \textsf{traj}(d_1,\vec{p}\uplus\vec{q}) \joinop (P\|_{cs} \textsf{traj}(d_2-d_1,\vec{q}(\cdot+d_1)) \joinop Q)
\end{array}}
\quad
\prftree{d_1>d_2}{
\begin{array}{l}
  (\textsf{traj}(d_1,\vec{p}) \joinop P) \|_{cs}
  (\textsf{traj}(d_2,\vec{q}) \joinop Q) \rightarrow \\
  \textsf{traj}(d_2,\vec{p}\uplus\vec{q}) \joinop (\textsf{traj}(d_1-d_2,\vec{q}(\cdot+d_2)) \joinop P\|_{cs} Q)
\end{array}}
& \\[1mm]
 	& 
\prftree{ch\in cs}{
	\begin{array}{l}
		...
		\qquad v = w \wedge \textsf{IO}(ch,v) \joinop (P \|_{cs} Q)
	\end{array}} 
	\quad\prftree{ch\in cs}{
	\begin{array}{l}
		(\textsf{traj}(d,\vec{p}) \joinop P) \|_{cs} (\textsf{in}(\vec{x}_0,ch,v) \joinop Q) \rightarrow \\
		\qquad\textsf{traj}(d,\vec{p}\uplus I_{\vec{x}_0}) \joinop (P \|_{cs} \textsf{in}(\vec{x}_0,cs,v) \joinop Q)
	\end{array}}
& \\[1mm]
 	& 
 \prftree{ch\in cs}{
	\begin{array}{l}
		(\textsf{traj}(d,\vec{p}) \joinop P) \|_{cs} (\textsf{out}(\vec{x}_0,ch,v) \joinop Q) \rightarrow \\
		\qquad\textsf{traj}(d,\vec{p}\uplus I_{\vec{x}_0}) \joinop (P \|_{cs} \textsf{out}(\vec{x}_0,cs,v) \joinop Q)
	\end{array}} & 
	\end{eqnarray*}
...
Fig.~\ref{fig:elimination_rules} lists the elimination rules for synchronization of simplified assertions. They are consistent with Fig.~\ref{fig:elim-synchronization}. We explain the last two rules in detail. They indicate that when one side is waiting for $d$ time units followed by a trace satisfying $P$, while the other side is ready for an input that should be synchronized, the only option is to wait for $d$ time units, and then (possibly) perform the communication later in the trace satisfying $P$. For the detailed proof, consider a case analysis on the time of communication compared to $d$. If communication happens in less than $d$ time units, there cannot be any synchronization with the left side. If communication happens in exactly or more than $d$ time units, any possible synchronization is covered by the right side of the entailment.
