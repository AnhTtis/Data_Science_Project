...
%TCIDATA{LastRevised=Friday, March 10, 2023 07:56:48}
...
memory of late Peter Fishburn. Peter was an exceptional applied
mathematician who made pioneering contributions to mathematical decision
theory within economics and psychology and was one of the leading figures
in order theory at large. In fact, the present paper is firmly built on a
...
thank Mauricio Ribeiro, Gil Riella, John Quah, and the seminar participants
at New York University and Johns Hopkins University for their suggestions
and feedback.}}}}
...
\begin{abstract}
\noindent We develop a partial ordering approach toward comparing stochastic
choice functions on the basis of their rationality. We do this by assigning
to any stochastic choice function a one-parameter class of deterministic
choice correspondences, and then checking for the rationality (in the
standard sense of revealed preference) of these correspondences for each
parameter value. Similar to the classical notion of deterministic
rationality, our rationality ordering detects violations of (stochastic)
transitivity as well as inconsistencies between choices from nested menus. We
obtain an easy-to-use characterization of this ordering, and then use that
characterization to introduce an index of stochastic rationality. We apply
our ordering and index to some well-known stochastic choice models and
provide an empirical application using a famous experiment by Amos Tversky.
...
utility, the Luce model, the tremble model, Tversky's nontransitivity
experiment.
...
simpler, and it provides clear criteria about when a particular choice
...
often conflicting, meanings to it. We are pragmatic about this concept in
this paper. Behaviorally speaking, by the \textquotedblleft
rationality\textquotedblright\ of an individual's\ choice behavior, we
understand that her choices in large menus are consistent with those in
pairwise choice situations; this corresponds to preference maximizing
behavior.} However, despite its time-honored stance, this approach is hard
to connect to empirical choice data, and is too coarse to model empirically
relevant deviations from preference maximization in a unified manner. In
this regard, stochastic choice theory has significant advantages. It can
readily be inferred from repeated choice data, and it is rich enough to
capture choice anomalies with ease.\footnote{%
On the former point, see Balakrishnan, Ok and Ortoleva (2021), and on the
latter, see Gul, Natenzon, and Pesendorfer (2014), Manzini and Mariotti
(2014), Fudenberg, Iijima, and Strzalecki (2015), Aguiar (2017), Echenique,
Saito, and Tserenjigmid (2018), and Natenzon (2019), among others.} On the
downside, it is much less clear when a stochastic choice model of \emph{individual decision-making} can be
classified as rational, and how one can make comparative rationality
statements about such rules. For instance, is any random utility model (RUM)
a rational model of \emph{individual decision-making}? If not, is it at least the case that any such model
is more rational than some of its alternatives? Are constrained versions of
the Luce model rational? Which sorts of constraints do not detract from the
rationality of the classical Luce model? Are the tremble choice models
rational? Does such a model necessarily become less rational as its tremble
likelihood gets smaller?

The primary goal of this paper is to introduce a methodology to make
rationality comparisons between stochastic choice functions, thereby
providing concrete answers to such questions. We begin with assigning to
each stochastic choice function a one-parameter family of deterministic
choice correspondences. Put precisely, for any stochastic choice function $%
\mathbb{P}$ and $\lambda \in (0,1],$ we define $C_{\mathbb{P},\lambda }$ as
the choice correspondence that chooses in a menu $S$ all options whose
choice probabilities are at least the 100$\times \lambda $ percent of the
maximum likelihood of choice in that menu. (So, for small enough $\lambda $, 
$C_{\mathbb{P},\lambda }$ selects everything that is chosen in $S$ with
positive probability, while $C_{\mathbb{P},1}$ selects only the items in $S$
with maximum likelihood of being chosen.) We think of $C_{\mathbb{P},\lambda
}$ as a deterministic \textquotedblleft approximation\textquotedblright\ of $%
\mathbb{P}$. Of course, it is not at all clear which particular $\lambda $
one should use in this approximation; depending on the specifics of the
choice environment, such as the size of the alternative space, the duration
of time one needs to make a decision, etc., it may be more appropriate to
use different values for $\lambda $. (This issue was explored in
Balakrishnan, Ok and Ortoleva (2021)). In this paper, we do not subscribe to
any particular value of $\lambda ,$ but rather consider the family $\{C_{%
\mathbb{P},\lambda }:0<\lambda \leq 1\}$, which was introduced by Fishburn
(1978), in its entirety.
...
measurement, or stochastic orders of risk analysis), it is difficult to
apply $\trianglerighteq _{\text{rat}}$ using its definition alone. After
going through some preliminaries of individual choice theory in Section 2,
we thus obtain a parameter-free characterization of $\trianglerighteq _{%
\text{rat}}$ in Section 3. This characterization assigns to each stochastic
choice function $\mathbb{P}$ a set $\Lambda (\mathbb{P})$, the union of a
finite collection of right-semiclosed intervals, and shows that $\mathbb{P}%
\trianglerighteq _{\text{rat}}\mathbb{Q}$ iff $\Lambda (\mathbb{P})\subseteq
\Lambda (\mathbb{Q})$. This not only makes working with $\trianglerighteq _{%
\text{rat}}$ easier (which is illustrated by our subsequent applications),
but it also lets us define a natural index of rationality that is consistent
with $\trianglerighteq _{\text{rat}}$ (by using the size of $\Lambda (%
\mathbb{P})$). This index, which we denote by $I_{\text{rat}}$, quantifies
the extent to which one stochastic choice function is more rational than
another. Moreover, it can be used to rank any two stochastic choice
functions that happen to be incomparable by means of $\trianglerighteq _{%
\text{rat}}$ (just like one can use, say, the Gini index to compare two
income distributions whose Lorenz curves cross).

Equipped with this methodology, one can address the kinds of questions we
have posed above formally, and at times, obtain answers that are not
self-evident. In our first application (Section 4.1), we investigate the
rationality of the random utility model. There is a sense in which this
model is the stochastic analogue of the classical (deterministic) rational
choice model. Presumably, for this reason, many decision theorists seem to
view \textit{any }RUM as corresponding to the stochastic choice behavior of
a rational individual. On closer scrutiny, however, this position is
suspect. For example, while transitivity is the key property of
deterministic rational choice model, a RUM may fail to satisfy even the
weakest form of stochastic transitivity.\footnote{%
This is not a theoretical anomaly. In the empirical application, we provide
in Section 4.4 we actually find that 4 of the 26 subjects exhibit weakly
stochastically \textit{non}-transitive behavior that is nonetheless
consistent with the RUM model.} Similarly, while the axiomatic basis of the
former model is very much based on rationality axioms (such as the weak
axiom of revealed preference), none of the known axioms for the RUM has the
makeup of a rationality postulate.
...
The issue here is not one's tastes, but the \textit{consistency }of one's
tastes. After all, the behavior of this politician is markedly different
...
rat}}$. The point is that the set of utility functions of a
\textquotedblleft rational\textquotedblright\ RUM should be coherent in some
sense. In Section 4.1, we characterize what this means for the RUM with two
utility functions. In fact, $\trianglerighteq _{\text{rat}}$ is able to rank
any two such RUMs, and classifies any such model with this consistency
condition as maximally rational. The rationality measurement methodology we
develop here is thus able to behaviorally distinguish between various random
utility models.
...
rational. However, we show in Section 4.2 that every 2-stage Luce rule is
maximally rational, provided that it satisfies a simple consistency
condition.

These observations demonstrate that if an analyst wishes to adopt either a
RUM or a general Luce-type model in economic applications, and if (s)he
wishes to investigate the consequences of rational, yet random, choice
behavior, (s)he needs to impose suitable consistency conditions on the
primitives of these models. The preorder $\trianglerighteq _{\text{rat}},$
as well as the index $I_{\text{rat}}$, can be used to identify the needed
consistency condition. Even better, these measures let one perform
comparative statics exercises in which one studies how the results of an
economic application alter as the adopted model of random choice is made
more rational. This is the manner in which our theoretical contribution may
be used in applied microeconomic modeling.

The next application we study in this paper pertains to the tremble model of
choice (Section 4.3). Unlike the previous applications, we find that no two
such models with distinct tremble probabilities are comparable by $%
\trianglerighteq _{\text{rat}}$. Instead, we compare the rationality of such
random choice models by using the index $I_{\text{rat}}$. We find that a
tremble model gets more rational (with respect to $I_{\text{rat}}$) as the
likelihood of the trembles, which are smaller than a threshold, gets
smaller. In the limit where this likelihood is zero, the model becomes the
standard utility-maximization model, which is, of course, maximally
rational. However, this is valid only for tremble probabilities that are
small enough. When these are larger than the said threshold, the opposite
conclusion is obtained. In that case, our measurement methodology picks up
the fact that there is no reason to view trembles as \textquotedblleft
mistakes\textquotedblright\ when they occur in high frequency. When tremble
probability is large enough, increasing it further makes the model get
closer to the one that chooses everything in every menu with equal
probabilities, thereby increasing its rationality (at least in the eyes of $%
I_{\text{rat}}$). This makes good sense because the latter model is
maximally rational, as it simply corresponds to the behavior of a
utility-maximizer who is indifferent between all alternatives.

These three applications are of a theoretical nature as they pertain to
random choice models. In Section 4.4 we turn to an empirical application. We
use the pooled choice data of Tversky (1969) and Regenwetter, Dana and
Davis-Stober (2011) to determine which of the involved subjects are
maximally rational according to our rationality ordering. We also make
rationality comparisons between the subjects by using the ordering $%
\trianglerighteq _{\text{rat}}$, and determine the value of $I_{\text{rat}}$
for each of them. We found that our methodology yields some novel insights
about the data, and it certainly highlights nuances that were previously
not apparent.

In Section 5, we conclude by pointing to a number of directions for research
that may refine and improve the measurement methodology we introduce here.
...
environments, there is also a good literature on measuring the rationality
of choice behavior in general environments; see, \textit{inter alia},
Houtman and Maks (1985), Apesteguia and Ballester (2015), Caradonna (2020),
and Riberio (2021).

Other than Riberio (2021), none of these papers develop a partial ordering
approach, and other than Apesteguia and Ballester (2015), none of them
applies to the context of stochastic choice functions. And even the powerful
work of Apesteguia and Ballester (2015), while applicable on the random
choice domain, targets predominantly the deterministic choice functions.
Unlike ours, that approach does not allow indifferences or
incomparabilities, which forces it to conclude that uniform randomization
due indifferences is the \textit{least} rational stochastic choice. In
addition, the Apesteguia-Ballester measure does not detect violations of
stochastic transitivity, nor is it designed to do this. More on this later
in the paper.
...
literature on measuring rationality. As our main goal is to introduce a
binary relation (or a measure) to compare stochastic choice functions and
there is a reasonably large literature on measuring rationality in
deterministic choice environments, we essentially abstract away from
comparing one deterministic choice with another deterministic choice or
stochastic choice. If one specializes our rationality ordering to the
context of deterministic choice correspondences, one gets an extremely
coarse ordering that renders one choice correspondence more rational than
another iff the first one is rational (in the sense of preference
maximization) and the other is not. Thus, unlike that of Apesteguia and
Ballester (2015), our method is useless for studying the comparative
rationality of deterministic choice behaviors, and conversely, the
literature on measuring rationality in the context of deterministic choice
theory does not apply to the present setting of random choice. Having said
this, however, one may combine the partial ordering method we introduce here
with the deterministic methods of the earlier literature to obtain a
comprehensive approach to measuring rationality in decision making. We
discuss this possibility in Section 3.
...
every $S\in \mathfrak{X}$.\footnote{%
A \textit{preorder} $\succsim $ on $X$ is a reflexive and transitive binary
relation on $X.$ (The asymmetric part of $\succsim $ is denoted by $\succ ,$
and its symmetric part by $\sim $.) For any $S\in \mathfrak{X},$ we denote
the set of all $\succsim $-maximal elements in $S$ by \textrm{{\footnotesize 
...
$y\succsim x$ for every $x,y\in X,$ we say that $\succsim $ is a \textit{%
...
relation is always transitive, but it need not be complete.} Thus:
...
relation that rationalizes a choice correspondence to be complete. The
completeness of a preference relation is all about being \textit{decisive},
...
two options due to insufficient information, or due to the multiplicity of
attributes relevant to choice. (As a matter of fact, this may well be the
reason for one's randomized choice behavior; see Ok and Tserenjigmid
(2022).) We will not discuss this point any further here. Many authors, such
as Aumann, Bewley, Karni, Luce and Raiffa, have written extensively on this
point, and there is now quite a substantive literature in decision theory on
...
completeness of the rationalizing preferences goes beyond these general
...
\noindent \textsc{Theorem 2.2.} [Riberio and Riella, 2017] \textit{A choice
correspondence $C$ on $\mathfrak{X}$\textsl{\ }is rational if, and only if,
it satisfies the Chernoff, Condorcet, and No-Cycle Axioms.}
...
From an individualistic perspective, we may interpret this distribution by
imagining that an agent has been observed to make choices from the feasible
menu $S$ multiple times, and $\mathbb{P}(x,S)$ is simply the relative
frequency of the times that she has chosen $x$ from $S.$ This is the \textit{%
empirical} interpretation of $\mathbb{P},$ and is very much in line with the
classical revealed preference theory. Alternatively, one can imagine that
the choice behavior of the individual is intrinsically probabilistic, or
that their choice behavior is deterministic over $S$, but the modeler has
only partial information about it. As such, $\mathbb{P}(x,S)$ is considered
as the actual probability of the agent choosing $x$ from $S;$ this is the 
\textit{theoretical} interpretation of $\mathbb{P}$.
...
\end{equation*}%
if $\lambda =0.$ We refer to the collection $\{C_{\mathbb{P},\lambda }:0\leq
\lambda \leq 1\}$ as the \textbf{Fishburn family} \textbf{associated with} $%
\mathbb{P}$.$\footnote{$\lambda $-choice correspondences induced by a $%
\mathbb{P}\in $ \textsf{\textbf{scf}}$(X)$ were first considered by Fishburn
(1978) who sought the characterization of $\mathbb{P}$ such that for every $%
\lambda \in \lbrack 0,1]$, there is a (utility) function $u_{\lambda
}:X\rightarrow \mathbb{R}$ with $C_{\mathbb{P},\lambda }(S)=\arg \max
u_{\lambda }(S)$ for every $S\in \mathfrak{X}$. Ok and Tserenjigmid (2022)
introduce methods of identifying if, and when, a stochastic choice model may
be thought of as arising due to indifference, incomparability, or
experimentation, by studying properties of choice correspondences $C_{%
\mathbb{P},0}$ and $C_{\mathbb{P},1}$.}$ Balakrishnan, Ok and Ortoleva
(2021) have recently used this family to \textquotedblleft
infer\textquotedblright\ a deterministic choice correspondence from $\mathbb{%
P}$, and provided an axiomatic characterization for it. Moreover, $C_{\mathbb{P}, \lambda}$ can be seen as the best deterministic-approximation of $\mathbb{P}$.\footnote{For each $\lambda\in (0, 1]$, there $\alpha>0$ for which $C_{\mathbb{P}, \lambda}$ is the unique solution of the following minimization problem
\[\arg\min_{C} \sum_{S\in\mathfrak{X}}\sum_{x\in S}\Big{|}\big(\mathbf{1}\{x\in C(S)\}\big)^\alpha-\big(\mathbb{P}^*(x, S)\}\big)^\alpha\Big{|}.\]}
...
eliminating, for instance, items that are chosen by mistake, in a rush,
...
\mathbb{P},0}\{x,y\}=\{x,y\}$ to this end. This is the main reason why
Fishburn considered a multitude of approximations.
...
as acceptable, it makes good sense to qualify $\mathbb{P}$ as \textquotedblleft
...
x,y,z\in X,$%
\begin{equation*}
\mathbb{P}(y,\{x,y\})<\lambda\, \mathbb{P}(x,\{x,y\})\hspace{0.2in}\text{and%
\hspace{0.2in}}\mathbb{P}(z,\{y,z\})<\lambda\, \mathbb{P}(y,\{y,z\})
\end{equation*}%
imply $\mathbb{P}(z,\{x,z\})<\lambda\, \mathbb{P}(x,\{x,z\})$.
...
stochastic choice function $\mathbb{P}$ on }$\mathfrak{X}$\textit{\ is $%
\lambda $-rational if, and only if, it satisfies the $\lambda $-Stochastic
Chernoff, Condorcet and Transitivity Axioms.}
...
\noindent \textsc{Example 2.1}. Let $X:=\{x,y,z\}$ and $\mathbb{P}$ be the stochastic choice function on $\mathfrak{X}$ such that 
\begin{equation*}
\mathbb{P}(x, \{x,y\})=0.8,\, \mathbb{P}(y, \{y,z\})=\mathbb{P}(z, \{x,z\})=\frac{2}{3},\text{ and }\mathbb{P}(x, X)=\mathbb{P}(z, X)=\frac{6}{13}.
\end{equation*}%
The set of $\lambda$ for which $\mathbb{P}$ is $\lambda$-rational is $(0, \frac{1}{6}]\cup (\frac{1}{4}, \frac{1}{2}]$. Hence, $\mathbb{P}$ is $0$-rational and $\frac{1}{2}$-rational, but not $\frac{1}{5}$-rational. Furthermore, when $\lambda\in (\frac{1}{2}, 1]$, $\mathbb{P}$ does not obey the $\lambda-$Stochastic Transitivity Axiom, capturing the fact the weakest form of stochastic transitivity is violated. Moreover, when $\lambda\in (\frac{1}{6}, \frac{1}{4}]$, $\mathbb{P}$ does not obey the $\lambda-$Stochastic Condorcet Axiom, capturing inconsistencies in choices across nested menus.\, $\square $
...
follows that $\mathbb{P}$ is $\lambda $-rational for every $\lambda \in
(0,1] $ iff $C_{\mathbb{P}\text{,}1}$ is rational. Thus, any $\mathbb{P}\in $
...
stochastic choice functions that belong to these sets in Section 4.} $%
\square $
...
\noindent \textsc{Example 2.1 (continued)}. For the stochastic choice function in this example, we find that $\Lambda(\mathbb{P})=(\frac{1}{6}, \frac{1}{4}]\cup (\frac{1}{2}, 1]$ and $I_{\text{rat}}(\mathbb{P})=\frac{5}{12}$. 
...
\textit{b. $\mathbb{P}$ is $\lambda $-rational for every $\lambda \in
\lbrack 0,1]$};\textit{\ }
...
\textquotedblleft s-transitivity\textquotedblright\ for brevity, we next recall the
...
moderate s-transitivity are truly minor weakenings of these stochastic
...
is 1-rational, it is clear that every such stochastic choice function is
almost weakly s-transitive. Less trivially, the following stronger result is
true.
...
maximally rational. In particular, it readily follows from this observation
that not every RUM is maximally rational.
...
function may still exhibit inconsistencies in choices across nested menus,
thereby violating either $\lambda $-Stochastic Chernoff or $\lambda $%
-Stochastic Condorcet Axioms for some $\lambda .$ However, as we will see in
Section 3.4, if we restrict our attention to pairwise choice sets (as it is
often done in within-subject choice experiments), then every moderately
...
stochastic choice function need not be strongly s-transitive. This is hardly a
cause for concern. As discussed at length by He and Natenzon (2022), strong
s-transitivity is a very demanding condition. Not only that it is not
...
condition. We consider one such illustration in Example 3.5 below.
...
fact, as we shall see in Section 4.2, every Luce model is maximally
rational. $\square $
...
because probabilistic choice behavior of a rational individual may well come
about due to her indecisiveness about the appeal of various outcomes (Ok and
Tserenjigmid (2022)). To drive this point home, let us say that a choice
...
to Paris ($x$) and a trip to Rome ($y$). Suppose she is conflicted between
these options, to the extent that she is ready to settle the problem by
...
to Paris with a discount of \$1. It is plain that she prefers $x_{+}$ over $x
...
two trips. It seems only natural that she settles the choice between $x_{+}$
and $y$ exactly as she settled that problem between $x$ and $y,$ that is, by
using a fair coin toss: $\mathbb{P}(x_{+},\{x_{+},y\})=\frac{1}{2}=\mathbb{P}%
(y,\{x_{+},y\})$.\footnote{%
For another parable that justifies this stochastic choice function from the
perspective of social choice, see Eliaz and Ok (2006).} This random choice
behavior seems quite sensible; there seems to be nothing \textquotedblleft
irrational\textquotedblright\ about it.\ And yet, we have $C_{\mathbb{P}%
,\lambda }\{x,y\}=\{x,y\},$ $C_{\mathbb{P},\lambda }\{x,x_{+}\}=\{x\},$ and $%
C_{\mathbb{P},\lambda }\{x_{+},y\}=\{x_{+},y\},$ so $C_{\mathbb{P},\lambda }$
is not \emph{totally} rational for any $\lambda \in (0,1]$. It follows that $%
\trianglerighteq _{\text{rat}}^{\prime \prime }$ declares $\mathbb{P}$ less
rational than \textit{any }stochastic choice function on $\mathfrak{X}$, an
absurd conclusion.\footnote{Indeed, we obtain a very similar conclusion even if $\mathbb{P}(x_{+}, \{x_{+}, y\})>\frac{1}{2}$.}
...
rat}}$ declares $\mathbb{P}$ as maximally rational, which makes perfect
sense in this instance.\footnote{%
...
s-transitive, but that this is for a good reason. He and Natenzon (2022)
reviews other such examples from the literature.}\,$\square $
...
models, Lemma 3.4 also applies to many well-known special cases of RUM such as probit, nested logit, tremble model, and
Tversky's elimination-by-aspects model.
...
\subsection{Pairwise Choice Situations}
...
models of random choice, but it becomes too demanding when it comes to
empirical applications where one has only limited data sets. Indeed, other
than very few exceptions, all within-subject repeated choice experiments
use menus that contain only two options. Our analysis not only easily adapts
to this case, but it actually becomes easier to apply.

Let $\mathfrak{X}_{2}$ stand for the collection of all nonempty subsets of $X
$ that contain at most two alternatives. By a \textit{choice correspondence}
on $\mathfrak{X}_{2},$ we mean a set-valued map $C:\mathfrak{X}%
_{2}\rightarrow \mathfrak{X}_{2}$ such that $C(S)\subseteq S.$ As soon as we
replace $\mathfrak{X}$ with $\mathfrak{X}_{2}$, the notion of being rational
and the No-Cycle Axiom apply to such correspondences \textit{verbatim}.
Moreover, Theorem 2.2 in this case takes the following form: \textit{A
choice correspondence $C$ on $\mathfrak{X}_{2}$\textsl{\ }is rational if,
and only if, it satisfies the No-Cycle Axiom. }(The proof is an easy
exercise.)

By a \textit{stochastic choice function} on $\mathfrak{X}_{2},$ we mean a
function $\mathbb{P}:X\times \mathfrak{X}_{2}\rightarrow \lbrack 0,1]$ such
that $\mathbb{P}(x,\{x,y\})+\mathbb{P}(y,\{x,y\})=1$ and $\mathbb{P}%
(z,\{x,y\})=0$ for every $x,y\in X$ and $z\in X\backslash \{x,y\}.$ The
collection of all such functions is denoted by \textsf{\textbf{scf}}$_{2}(X).
$ In experimental work where a subject chooses from all pairwise choice
menus repeatedly, the ensuing relative choice frequencies form such a
stochastic choice function.

For any $\mathbb{P}\in $ \textsf{\textbf{scf}}$_{2}(X)$ and $\lambda \in
(0,1],$ $C_{\mathbb{P},\lambda }$ is a choice correspondence on $\mathfrak{X}%
_{2};$ we say that $\mathbb{P}$ is $\lambda $-rational iff $C_{\mathbb{P}%
,\lambda }$ is rational. In view of the above-mentioned modification of
Theorem 2.2, we have the following modification of Theorem 2.3: \textit{For
any }$\lambda \in (0,1],$\textit{\ a} $\mathbb{P}\in $ \textsf{\textbf{scf}}$%
_{2}(X)$\textit{\ is }$\lambda $\textit{-rational if, and only if, it
satisfies the }$\lambda $\textit{-Stochastic Transitivity Axiom.}

Next, we define the \textit{comparative rationality ordering}\textbf{\ }$%
\trianglerighteq _{\text{rat}}$ on \textsf{\textbf{scf}}$_{2}(X)$ by (\ref%
{ratt}). Given the observation above, this preorder has a very simple
characterization:%
\begin{equation*}
\mathbb{P}\trianglerighteq _{\text{rat}}\mathbb{Q}\hspace{0.2in}\text{iff%
\hspace{0.2in}ST}(\mathbb{P})\subseteq \text{ST}(\mathbb{Q})
\end{equation*}%
for every $\mathbb{P},\mathbb{Q}\in $ \textsf{\textbf{scf}}$_{2}(X)$. (Here,
of course, ST$(\cdot )$ is defined on \textsf{\textbf{scf}}$_{2}(X)$ by (\ref%
{st}), where $\mathbb{P}^{\ast }:X\times \mathfrak{X}_{2}\rightarrow \lbrack
0,1]$ is defined by (\ref{star}).) Consequently, our rationality index takes
the form%
\begin{equation*}
I_{\text{rat}}(\mathbb{P}):=1-\text{Leb}(\text{ST}(\mathbb{P}))
\end{equation*}%
for every $\mathbb{P}\in $ \textsf{\textbf{scf}}$_{2}(X)$. By Lemma 3.4,
therefore, any moderately s-transitive stochastic choice function on $%
\mathfrak{X}_{2}$ is maximally rational. Combining this fact with
...
\end{equation*}%
for any $\mathbb{P}\in $ \textsf{\textbf{scf}}$_{2}(X)$. Apparently, the
difference between maximal rationality and moderate s-transitivity is really
minor in the case of pairwise choice situations. The following is an
immediate application of this observation.

\bigskip 

\noindent \textsc{Example 3.6}. The \textbf{moderate utility model} (MUM) of
He and Natenzon (2022) is any $\mathbb{P}\in $ \textsf{\textbf{scf}}$_{2}(X)$
of the form 
\begin{equation*}
\mathbb{P}(x,\{x,y\})=F\left( \frac{u(x)-u(y)}{d(x,y)}\right) ,\hspace{0.1in}%
\hspace{0.1in}\hspace{0.1in}x,y\in X,
\end{equation*}%
where $F:\mathbb{R}\rightarrow \lbrack 0,\infty )$ is any strictly
increasing function with $F(t)=1-F(-t)$ for every $t\in \mathbb{R},$ $%
u:X\rightarrow \mathbb{R}$ is any (utility) function, and $d$ is any metric
on $X.$ It is not difficult to show that any MUM is moderately s-stochastic.
(He and Natenzon (2022) in fact prove that the converse of this is also
true, that is, every moderately s-stochastic $\mathbb{P}\in $ \textsf{%
\textbf{scf}}$_{2}(X)$ is a MUM.) In view of the discussion above,
therefore, we conclude: \textit{Every MUM is maximally rational.} 

Consequently, all Fechnerian models (e.g., RUM with i.i.d. errors and the weak APU model) and many well-known RUMs
(e.g., probit, nested logit model, tremble model, and
Tversky's elimination-by-aspects model) are maximally rational on $\mathfrak{X}_2$ as they are MUM.
$\square $

\bigskip 


In an iterated choice experiment in which a subject chooses from all
doubleton subsets of $X,$ one obtains empirical relative frequencies of her
choices in every such menu. These frequencies make up a stochastic choice
function on $\mathfrak{X}_{2},$ and thus our modified comparative
rationality measurement procedures immediately apply. In Section 4.4, we
will provide such an empirical application in the context of a famous
experiment by Amos Tversky.
...
rational\textquotedblright\ choice model.
...
(2004), and Apestguia, Ballester and Lu (2014).} To add insult to injury, a
RUM may fail even weak s-transitivity, the most basic stochastic
transitivity requirement considered in the literature.\footnote{%
For example, let $X:=\{x,y,z\},$ and define $\mathbb{P}\in $ \textsf{\textbf{%
scf}}$(X)$ with $\mathbb{P}(x,\{x,y\})=\mathbb{P}(y,\{y,z\})=\mathbb{P}%
(z,\{x,z\}):=\frac{2}{3}$, and $\mathbb{P}(x,X)=\mathbb{P}(y,X):=\frac{1}{3}$%
. This is a RUM (induced by three individuals whose rankings of $x,$ $y$, and 
$z$ form a Condorcet cycle), but it is not weakly s-transitive.}
...
that is, 
\begin{equation*}
\mathbb{P}(x,S)=\theta \mathbf{1}_{\arg \max u(S)}(x)+(1-\theta )\mathbf{1}%
_{\arg \max v(S)}(x)
\end{equation*}%
...
\mathbb{P}^{\ast }(z,\{y,z\})$ are zero, but the latter means $u(x)>u(y)>u(z)
$ and $v(x)>v(y)>v(z),$ whence $\mathbb{P}^{\ast }(z,\{x,z\})=0,$ so this
interval must be empty. Thus, we have ST$(\mathbb{P})=\varnothing $ when $%
\theta =\frac{1}{2}$ as well.
...
\noindent \textsc{Proposition 4.1.} \textit{Let $\mathbb{P}$ be the uniform
...
\noindent \textsc{Corollary 4.2.} \textit{a. Any two uniform dRUMs on} $%
...
The last two parts of Corollary 4.2 go against the conventional wisdom that
every random utility model is \textquotedblleft rational.\textquotedblright\
Corollary 4.2.d says that a uniform dRUM may be strictly less rational than
...
be rational in a very strong sense. Unless its inducing utility functions
...
\noindent \textsf{\textbf{On Maximal Rationality. }}Another insight provided
by Corollary 4.2 is about maximally rational stochastic choice functions. If
an agent makes her choices across feasible menus by maximizing an injective
utility function, it is in the nature of things that we declare her as
maximally rational. One may imagine that only such choice procedures should
be considered as maximally rational. Indeed, this is an axiom in Apesteguia
and Ballester (2015). By contrast, Corollary 4.2.b says that certain types
of uniform dRUMs are also declared maximally rational by $\trianglerighteq _{%
\text{rat}}$. (We will later show that certain types of non-RUM Luce models
have this property as well.) A brief discussion about the plausibility of
this assessment is in order.

Consider a social planner who makes choices on the part of a group of two
individuals (such as a parent of two children). Suppose the utility
functions of the constituents of this society are given by injective utility
functions $u$ and $v$ on $X.$ When both of these utilities say that an
option is best in a menu, the planner chooses that option. If they disagree,
she decides among the most desired options of the two people by flipping a
fair coin. Is the planner rational? It seems to us that her procedure is
duly rational (on the basis of the provided information), but whether or not
it yields \textit{consistent} choice probabilities across menus (so that we
qualify the resulting stochastic choice function as fully rational) depends
on the \textquotedblleft coherence\textquotedblright\ of $u$ and $v$. The
viewpoint that $u$ and $v$ are \textquotedblleft coherent\textquotedblright\
iff they rank all alternatives the same way forces the conclusion that
choice probability distributions across menus can never be considered as
consistent unless each of those distributions is degenerate. This seems
unnecessarily demanding. The present approach, instead, understands from the
said \textquotedblleft coherence\textquotedblright\ the property of
consistency over triplets. This entails that the choice probability
distributions are consistent in the sense that \textit{every} approximation
of them by a deterministic choice correspondence from the Fishburn family
arises upon the maximization of a (possibly incomplete) preference relation.%
\footnote{%
The point is not that consistency over triplets is the \textquotedblleft
right\textquotedblright\ coherence condition to use, and that one should
model the consistency of choice probability distributions the way we do here
(through the Fishburn family). Rather, we simply take issue with the
contention that full rationality of a stochastic choice function must be
equated with the maximization of a single strict complete preference
relation.}
...
The following result generalizes the second part of Proposition 4.1.

\bigskip

\noindent \textsc{Proposition 4.3.} \textit{Let $\mathbb{P}$ be the RUM on} $%
...
in Proposition 4.3 are not sufficient for maximal rationality.

\bigskip

\noindent \textsc{Example 4.1.} Let $X:=\{x,y,z\}$, take any maps $u_{1},$ $%
...
\noindent \textsc{Proposition 4.4.} \textit{Let $\mathbb{P}$ be the RUM on} $%
\mathfrak{X}$\textit{\ induced by $(u_{i},\theta _{i})_{i\in N}$ with $%
...
\noindent \textsc{Example 4.2.} Let $f:X\rightarrow \mathbb{R}$ and define $%
\Gamma :\mathfrak{X}\rightarrow \mathfrak{X}$ by $\Gamma (S):=\arg \max
f(S). $ Then, any general Luce model with the constraint correspondence $%
\Gamma $ is maximally rational. $\square $

\bigskip

\noindent \textsc{Example 4.3.} Let $X:=\{x,y,z\},$ $u(x)>u(y)>u(z),$ $%
...
\noindent \textsc{Proposition 4.5.} \textit{Any proper 2-stage Luce model is
maximally rational.}
\medskip

Since some proper 2-stage Luce models are not RUM, Propositions 4.3 and 4.5 show that RUM is neither necessary nor sufficient for maximal rationality. 
...
contrast to uniform dRUM (Corollary 4.2.a). To examine the comparative
...
\noindent \textsc{Proposition 4.6.} \textit{a. No two tremble models with
...
maximally rational. Proposition 4.6 captures this idea, but also warns us
...
methodology in the context of within-subject repeated choice experiments. To
this end, we use Experiment 1 of Tversky (1969) which was originally
...
obtain a choice data set of 26 individuals (with Subjects 17-26 coming from
...
\end{tikzpicture}
\end{center}
\caption{Distribution of $\Lambda$-sets}
\end{figure}
...
do not. But one is often interested in the degree to which rationality is
...
on binary choices is notoriously difficult. Fortunately, when the total
number of alternatives is at most 5, that is, $\left\vert X\right\vert \leq
5,$ Theorem 1 of Fishburn (1992) establishes that we only need to check
the following \textit{triangular condition} to this effect:%
\begin{equation*}
...
worth noting that four of these subjects, namely, S13, S1, S12 and S25, violate
weak s-transitivity. This is but an empirical testament to the fact that a
RUM need not be weakly s-transitive. Moreover, it shows that the RUM cannot
distinguish the choice behavior of S1, S12, and S25 from those of S2,
S7, and S9 who satisfy weak s-transitivity. In contrast, as we have seen in Figure 2, the present method
of measuring stochastic rationality renders sharper comparisons. For
instance, it follows from Figure 2 that any one of S2,
S7, and S9 is more rational than S1, S12 and S25 with respect to $I_{\text{rat}}$. In
fact, S9 is more rational than S12 and S25 even relative to $%
\trianglerighteq _{\text{rat}}$. 
...
discussed in Section 1, this is not an unexceptionable viewpoint, absent
suitable consistency conditions among the utilities of the model.\footnote{%
\linespread{1.1} \selectfont One might think that deliberately stochastic
choice models (e.g., studied in Machina (1985), Fudenberg, Iijima, and
Strzalecki (2015), and Cerreia-Vioglio, Dillenberger, Ortoleva, and Riella
(2019)) are trivially maximally rational as in these models a decision maker
maximizes a preference over probability distributions over alternatives.
Deliberately stochastic choice models may or may not be maximally rational
by $\trianglerighteq _{\text{rat}}$ since, in general, the preference over
probability distributions and the preference over alternatives are not
required to be consistent with each other. Moreover, as shown in
Cerreia-Vioglio, Dillenberger, Ortoleva, and Riella (2019), some
deliberately stochastic choice models may violate weak stochastic
transitivity. We leave characterizations of sufficient consistency
conditions on the preference over probability distributions for maximal
rationality for future research.}

In this paper, we have thus developed a rigorous method of comparing the
...
empirically by using the choice data generated by repeated choice
experiments.
...
{\small \noindent \textsc{Proof of Theorem 3.1.} Let us show that $\mathbb{P}
$ satisfies $\lambda $-Stochastic Chernoff Axiom iff $\lambda\not\in\text{Ch}%
(\mathbb{P})$. Suppose $\mathbb{P}$ satisfies $\lambda $-Stochastic Chernoff
Axiom. By way of contradiction, suppose $\lambda\in \text{Ch}(\mathbb{P})$;
i.e., there are $S,T\in \mathfrak{X}$ and $x\in S$ such that $S\subseteq T$
and $\mathbb{P}^{\ast }(x,S)<\lambda\le \mathbb{P}^{\ast }(x,T)$. Hence, $%
\mathbb{P}$ violates $\lambda $-Stochastic Chernoff Axiom. In other words,
if $\mathbb{P}$ satisfies $\lambda $-Stochastic Chernoff Axiom, then $%
\lambda\not\in\text{Ch}(\mathbb{P})$. Conversely, suppose $P$ violates $%
\lambda $-Stochastic Chernoff Axiom; i.e., there are $S,T\in \mathfrak{X}$
with $S\subseteq T,$ and $x\in S$, $\mathbb{P}^{\ast }(x,T)\geq \lambda> 
\mathbb{P}^{\ast }(x,S)$. Hence, $\lambda\in \Lambda(\mathbb{P})$. In other
words, if $\lambda\not\in \Lambda(\mathbb{P})$, then $P$ satisfies $\lambda $%
-Stochastic Chernoff Axiom. }

{\small Similarly, it is not difficult to prove that $\mathbb{P}$ satisfies $%
\lambda $-Stochastic Condercet (Transitivity) Axiom iff $\lambda \not\in 
\text{Con}(\mathbb{P})$ (resp., $\lambda \not\in \text{ST}(\mathbb{P})$).
Hence, by Theorem 2.3, $\mathbb{P}$ is $\lambda $-rational iff $\lambda
\not\in \Lambda (\mathbb{P})$. Then $\mathbb{P}\trianglerighteq _{\text{rat}}%
\mathbb{Q}$ iff $\mathbb{P}$ is $\lambda $-rational for every $\lambda \in
(0,1]$ for which $\mathbb{Q}$ is $\lambda $-rational; equivalently, $\Lambda
(\mathbb{P})\subseteq \Lambda (\mathbb{Q})$. \ $\blacksquare $}
...
{\small \noindent \textsc{Proof of Proposition 4.1.} Let us write $I_{S,x}$
...
{\small \noindent \textsc{Proof of Proposition 4.3.} (a) Let $\frac{1}{2}%
...
{\small \noindent \textsc{Proof of Proposition 4.4.} \textit{Claim 1. }Ch$(%
...
). If there is a $z\in T$ with $u_1(y)=\max u_1(T)>u_1(z)>u_1(x)$, then by
hypothesis, $u_{i}(z)>u_{i}(x)$ for all $i\in N,$ whence $\mathbb{P}^{\ast
}(x,T)=0,$ contradicting $I$ being nonempty. Assume, then, $%
u_1(y)>u_1(x)>u_1(z)$ for each $z\in T\setminus\{x, y\}$. Then, it must be
that $y\in S$. Moreover, by hypothesis, $u_{i}(y)>u_{i}(z)$ for all $i\in N$
and $z\in T\backslash \{y\}.$ Finally, it is easy to check that $\mathbb{P}%
(y, \{x, y\})=\mathbb{P}(y, S)=\mathbb{P}(y, T)$ and $\mathbb{P}(x, \{x,
y\})=\mathbb{P}(x, S)=\mathbb{P}(x, T)$, which means $I$ is empty, a
...
{\small \noindent \textsc{Proof of Proposition 4.5.}\footnote{%
...
%\noindent Agranov, M., and P. Ortoleva, Ranges of preferences and
%randomization, \textit{mimeo}, Princeton Univ., 2020.
...
\noindent Aumann, R.J., Utility theory without the completeness axiom, \textit{Econometrica.}
30 (1962), 445-462.
...
\noindent Bewley, T.F., Knightian Decision Theory: Part I, Cowles Foundation Discussion Paper No. 807, 1986.
...
\noindent Cerreia-Vioglio, Simone, David Dillenberger, Pietro Ortoleva, and
Gil Riella, Deliberately stochastic, \textit{American Economic Review,} 109
(2019), 2425-2445.
...
\noindent He, J., and P. Natenzon, Moderate utility, \textit{mimeo},
University of Washington at St. Louis, 2022.
...
\noindent Machina, Mark, Stochastic choice functions generated from
deterministic preferences over lotteries, \textit{Economic Journal,} 95
(1985), 575-594.
...
experimentation and stochastic choice, \textit{Theoretical Econ.}%
17 (2022), 378-413.
...
\noindent Schmeidler, D., Subjective probability and expected utility without additivity, \textit{Econometrica.}
57 (1989), 571-587.
...
\noindent von Neumann, J., and O. Morgenstern, \textit{Theory of Games and Economic Behavior},  Princeton Univ. Press, (1947).
