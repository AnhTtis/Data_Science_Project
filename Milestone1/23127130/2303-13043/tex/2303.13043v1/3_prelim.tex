...
\citet{shi2022visual} show that a sparse reconstruction (SR) module functionally resembles visual attention. An SR module takes an input $\bfx \in \bbR^d$ %\footnote{For example, if $\bfx$ is an image or a feature map, then $d = c \cdot h \cdot w$.} 
...
\end{equation}
where the optimization is over an auxiliary variable $\bfu$ and $\tbfu = \mathit{g}_\lambda (\bfu) = \mathit{sgn}(\bfu) (|\bfu| - \lambda)_+$ with $\mathit{sgn}(\cdot)$ as the sign function and $(\cdot)_+$ as ReLU %\footnote{\cref{eq:sr_dyn} is proposed in \cite{rozell2008sparse} as a biologically plausible algorithm for sparse reconstruction in V1. Specifically, one may view \cref{eq:sr_dyn} as a simple ODE description of dynamics of biological neurons, \ie, $\bfu$ represents the membrane potential which is charged by input current $\bfP^T \bfx$ and discharged by lateral inhibition $- (\bfP^T \bfP - \mathbf{I}) \tbfu$ and self-leakage $-\bfu$.}. 
