...
\author{%
Yi Xie $^{1}$ \quad Huaidong Zhang $^{1}$\thanks{Corresponding author} \quad Xuemiao Xu$^{1,2,3,4}$ \quad Jianqing Zhu $^{5}$  \quad Shengfeng He $^{6}$\\
$^{1}$South China University of Technology \quad $^{2}$State Key Laboratory of Subtropical Building Science \\ $^{3}$Ministry of Education Key Laboratory of Big Data and Intelligent Robot\\ \quad $^{4}$Guangdong Provincial Key Lab of Computational Intelligence and Cyberspace Information\\
\quad $^{5}$ Huaqiao University \quad $^{6}$ Singapore Management University\\
