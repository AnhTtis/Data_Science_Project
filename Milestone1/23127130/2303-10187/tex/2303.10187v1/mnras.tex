...
\newcommand{\poojan}[1]{\textbf{\textcolor{Purple}{[Poojan: #1]}}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$^{5}$\,Institute of Astronomy, Department of Physics and Astronomy, KU Leuven, Celestijnenlaan 200 D, 3001 Leuven, Belgium.
}
...
We combine this updated version of METISSE with the Binary Stellar Evolution \citep[BSE;][]{Hurley:2002} code. 
...
In Section~\ref{sec:mass_transfer_binary} we briefly review the physics of mass transfer in binary systems while in Section~\ref{sec:bse} we describe  the treatment of binary evolution in BSE.
...
% While 1-D stellar evolution codes are useful for studying the properties of a few stars, they are computationally expensive and not meant for direct use in computing the evolution of a population of stars. Consequently, many population synthesis codes rely on fitting formulae for single stellar evolution. 
% Large uncertainties in the evolution of massive stars, both physical and numerical, also keeps us from an understanding of massive close binary systems \citep{DornWallenstein2020, Belczynski2021}.
...
For massive stars ($M_{\rm ini} > 10$\Msun{}), mass loss due to stellar winds can be quite high ($\sim$\tento{-4}\Msun{}yr$^{-1}$) and the star can lose its envelope before nuclear burning is completed. Depending on its mass and the evolutionary stage, a stripped star in METISSE can either become a helium white dwarf or a naked helium star. We currently revert to using SSE formulae for further evolution of the naked helium stars. 
...
\begin{figure}
    \includegraphics[width=\columnwidth]{plots/Core_mass_sse_metisse.pdf}
    \caption{Evolution of core mass with phase as predicted by SSE (dashed-dotted) and METISSE (solid lines). 
    Here the core mass represents the mass of the helium core up until the AGB and the mass of the carbon-oxygen core thereafter.}
...
Following the SSE convention, the core mass here represents the mass of the helium-rich core for phases up to the AGB and the mass of the carbon-oxygen core for phases beyond the AGB (including naked helium star phases). 
From the figure, we see that the core masses at the end of each evolutionary phase for stars up to 20\Msun{} show a good agreement between METISSE and SSE, with the maximum difference being 0.2\Msun{} for a 15\Msun{} star at the end of the HG.
...
For stars with initial masses of 40\Msun{} and above, where both SSE and METISSE predict the formation of a naked helium star, there can be a difference of up to 1.4\Msun{} in the core masses (helium core) during the HG between the two codes.
...
These differences suggest that depending on the mass-loss rates, the convective cores of massive stars can be insensitive to the effects of mass loss. In general, at high mass-loss rates present in massive stars, the removal of the outer layers can destroy its equilibrium.
...
In this section we provide a brief overview of mass transfer and its role in binary evolution.
...
The binary is now in a semi-detached state and is said to be undergoing mass transfer through Roche-lobe overflow (RLOF).
...
Following \citet{Kippenhahn1967}, mass transfer in an interacting binary system can be classified as one of three cases based on the evolutionary state of the donor star when it first fills its Roche lobe. 
\begin{enumerate}
...
% , or on the timescale for angular momentum loss, whichever is shorter. 
...
For example, if the expansion of the donor occurs beyond the outer Lagrangian point of the system, or the donor transfers more mass than the accretor can accrete (which is limited by its thermal timescale), then the outer layers of the star form a common envelope (CE) around the binary \citep{Paczynski:1976,Iben1993}. 
...
BSE is a popular binary population synthesis code that can rapidly compute the evolution of stars in a binary system, along with providing algorithms to model the necessary binary physics. 
...
The first part is for detached binary systems where neither of the stars have filled their Roche lobes, and they interact (if at all) solely through wind accretion and tidal interactions. Interaction of detached binaries through tides is treated using parametrized formulae from \citet{Hut1981}, \citet{Zahn1977} and \citet{Campbell1984}, while wind-accretion from a companion star is estimated via the Bondi-Hoyle mechanism \citep{BondiHoyle1944}.

The second part of the evolution is where one or both stars fill their Roche lobes. 
The treatment of RLOF closely follows \citet{Tout:1997}
and the stability of mass transfer through RLOF is determined using the critical mass ratio, $q_{\text {crit}}$. For donors that are in the giant phase of the evolution (such as the RGB stars or the AGB stars), $q_{\text {crit}}$ is calculated  using \citep{Hjellming:1987,Webbink:1988},
...
where $M_{\rm d}$ and $M_{\rm c,d}$ denote the total mass and core mass of the donor, respectively.
For other nuclear-burning phases of the donor, constant values of $q_{\text {crit}}$ from \citet{Webbink:1985} are used. At any point during the mass transfer through RLOF, if the mass ratio of the donor to the accretor exceeds $q_{\text {crit}}$, then mass transfer is deemed unstable. 

If the binary reaches the state of a common envelope, the outcome of the common envelope evolution is determined by comparing the total binding energy of the envelope and the orbital energy of the cores \citep{Paczynski:1976}. The efficiency of energy transfer from the orbit to the common envelope is determined through the parameter $\alpha$ \citep{Webbink:1984} whereas the binding energy of the envelope depends on the structure parameter $\lambda$ \citep{deKool1990}, both of which are treated as free parameters in BSE due to large uncertainties in their values. 

Angular momentum losses through gravitational radiation and magnetic braking are estimated using the weak-field approximation of general relativity \citep{LandauLifshitz1975} and parameterizations from \citet{Rappaport1983} and \citet{Skumanich1972} respectively. If either of the stars undergoes a supernova explosion, a kick velocity is taken randomly from a Maxwellian distribution following \citet{Hansen1997} with dispersion or root-mean-square velocity of 190\,${\rm km\,sec^{-1}}$, to calculate the loss in angular momentum from the system.
...
In this section, we compute the evolution of the 25\Msun{} and 15\Msun{} binary system described above with BSE-METISSE using \citet{Pols:1998} tracks and compare the results obtained using the traditional method of using SSE formulae in BSE. The evolution of the binary system as predicted by both methods is summarized in the top two panels (panels a and b) of Figure~\ref{fig:summary_bse_metisse}.  

Panel (a) of Figure~\ref{fig:summary_bse_metisse} depicts the evolution of the binary computed using SSE-POLS98. The system starts with both stars on the main sequence with an orbital separation of 1800\,days ($\approx$ 2128\Rsun{}). 
...
The mass loss through stellar winds increases the orbital separation of the binary to 2193\Rsun{}.
As the primary evolves, it expands and eventually fills its Roche lobe during the core helium burning phase at 7.57\,Myr, while the secondary is still burning hydrogen on the main sequence (Figure~\ref{fig:comp_bse_metisse}).
RLOF ensues and mass is transferred from the now 23.59\Msun{} primary onto the 15.01\Msun{} secondary.
This mass transfer initially decreases the orbital separation as mass is transferred conservatively from the more massive primary to the less massive secondary. The mass ratio reverses at the orbital separation of 2183\Rsun{}, and further mass transfer only widens the binary.

Mass transfer through RLOF continues on the nuclear timescale as the primary evolves through the giant phases and ends at an orbital separation of 2573\Rsun{}, when the primary undergoes a supernova explosion at 8.26\,Myr.
The primary, with a pre-supernova mass of 11.84\Msun{} and a carbon-oxygen core mass of 7.10\Msun{}, forms a 9.00\Msun{} BH. 
The secondary, on the other hand, has become a blue straggler star with a total mass of 20.15\Msun{}. 
The evolution of the secondary continues while the companion BH grows through wind accretion to a final mass of 9.44\Msun{}. The secondary evolves to become a 13.95\Msun{} giant with a 5.38\Msun{} carbon-oxygen core before exploding in a supernova at 13.89\,Myr to form a 2.23\Msun{} neutron star. By construction, no natal kick is imparted to the newly formed neutron star, although the large amount of mass lost during the supernova explosion disrupts the binary and the system becomes unbound. 

METISSE-POLS98 predicts a similar evolutionary path for the binary, as shown in panel (b) of Figure~\ref{fig:summary_bse_metisse}.
The primary evolves from a 25\Msun{} main-sequence star to a 23.37\Msun{} core helium burning giant as the 15\Msun{} secondary accretes material from the wind of the primary to become a 15.03\Msun{} star. 
The primary fills its Roche lobe and mass transfer from the primary to the secondary through RLOF begins at 7.77\,Myr, about 0.2\,Myr later compared to SSE.
Similar to SSE, mass is transferred on the nuclear timescale until the primary explodes in a supernova to become an 8.66\Msun{} BH. During this phase of mass transfer, the secondary accretes 2.66\Msun{} from the primary to become a 17.69\Msun{} blue straggler star as the orbital separation increases from 2195\Rsun{} to 2598\Rsun{}. 
The secondary evolves to become a giant star losing mass through winds, some of which is accreted by the companion BH which grows to 8.89\Msun{}. The secondary ends its life in a supernova with a pre-supernova mass of 13.71\Msun{}, and a core mass of 4.35\Msun{} to form a 1.36\Msun{} neutron star. Similar to SSE, the system loses about 10\Msun{} mass during the supernova, which is enough to disrupt the binary. 


While the overall evolutionary path for the binary predicted by METISSE agrees well with SSE, there are also some discrepancies. Both SSE and METISSE predict mass transfer due to RLOF on the nuclear timescale until the primary explodes in a supernova. However, the amount of mass transferred from the primary to the secondary with the SSE formulae (5.14\Msun{}) is twice as much compared to METISSE (2.66\Msun{}). This is due to the differences in the implementation of mass transfer in both codes during the core helium burning phase of high-mass giant stars (Section~\ref{sec:hrd_metisse_sse}). 
SSE continuously predicts a higher stellar radius compared to METISSE during this phase as the primary loses mass (see Figure~\ref{fig:comp_bse_metisse}). 
The ratio of stellar radius to the Roche-lobe radius is similarly higher. 
Since the amount of mass transferred on the nuclear timescale in BSE varies as the cube of the ratio of stellar radius to the Roche-lobe radius \citep[Eqn 58 of][]{Hurley:2002}, therefore more matter is transferred from the primary to the secondary in SSE's case as compared to METISSE for a similar amount of time ($\sim$ 0.7\,Myr). 

Another unrelated discrepancy can be seen between the METISSE and SSE predictions when each star undergoes a supernova explosion to form a compact remnant (NS or BH). SSE, despite predicting a lower pre-supernova mass for the primary, predicts the formation of a more massive BH compared to METISSE. Similarly, despite the similarity between the pre-supernova masses for the secondary, SSE predicts the formation of a comparatively more massive NS. This discrepancy is a result of the carbon-oxygen core mass predicted by each code. METISSE relies on the underlying input tracks to provide information about the carbon-oxygen core mass, and therefore can better capture its evolution. SSE, on the other hand, uses a simplified method where the carbon-oxygen core mass grows until a critical value is reached \citep[see Section 5.2 of][for details]{Agrawal:2020}, resulting in a higher carbon-oxygen core mass than METISSE. 
...
Hereafter we refer to the three sets of MESA tracks as MESA-SET1, MESA-SET2 and MESA-SET3 respectively. The evolutionary tracks for a sample of masses from each set are shown in Figure~\ref{fig:HR_MESA_OVS}
...
For METISSE-MESA-SET1, RLOF starts at 7.47\,Myr during core helium burning at a similar primary mass and evolutionary phase as METISSE-POLS98. 
As the primary evolves beyond core helium burning to the AGB phase, the mass transfer becomes dynamically unstable (according to Equation~\ref{eq:qcrit}), and a CE forms around the binary. 
Following CE, the primary loses its envelope to form an 8.37\Msun{} naked helium star with a 6.18\Msun{} carbon-oxygen core that shortly afterwards collapses to form a 4.46\Msun{} BH. The secondary, now a 15.02\Msun{} main-sequence star, evolves and fills its Roche lobe at about 12.57\,Myr during core helium burning. 
The mass ratio of the binary again renders mass transfer unstable during the core helium burning phase and the system undergoes CE evolution. During the CE, the secondary loses its envelope to form a 4.40\Msun{} naked helium star and eventually forms a 1.35\Msun{} NS. The two CE episodes significantly reduce the separation, leaving a final separation between the two stars of just 25\Rsun{}. The end products, a BH and a NS in a tight binary, can eventually merge and can give rise to gravitational-wave emission. 
 % Note that the secondary almost fills its Roche-lobe at the beginning of core helium burning, but as luminosity and temperature increase due to core helium burning, its radius decreases and the star undergoes a blue loop.
 
 
 In the case of METISSE-MESA-SET2, owing to larger overshooting and therefore larger radius, RLOF begins at about 7.46\,Myr when the primary is still on the HG. 
 This mass transfer remains stable, however, RLOF ends and restarts multiple times as the stellar radius changes as the primary evolves beyond core helium burning. 
 After transferring 6.60\Msun{} to the secondary, the primary finally ends its life in a supernova, forming an 11.43\Msun{} BH. 
 The secondary effectively evolves as a 21.58\Msun{} main-sequence star and, similar to the primary, starts RLOF during the HG phase. 
 The mass transfer continues until the secondary evolves to become a 15.86\Msun{} AGB star. During the AGB evolution, the mass ratio of the two stars causes the mass transfer to become dynamically unstable (Equation~\ref{eq:qcrit}) and the stars undergo CE evolution, which 
 strips the secondary of its envelope, leaving behind an 8.51\Msun{} helium star with a 6.51 carbon-oxygen core. 
 The naked helium star evolves to form a 5.27\Msun{} BH. 
 The CE phase reduces the orbital separation only a little, ultimately resulting in a wide (1500\Rsun{}) binary black hole system.
 

The case of METISSE-MESA-SET3 closely resembles the case of METISSE-MESA-SET2. The 25\Msun{} primary fills its Roche lobe during the HG phase, although at a later time of 8.03\,Myr, as the larger overshooting value extends the main-sequence life of the star. 
It also leads to the formation of a more massive helium core and at 8.57\,Myr, the primary becomes a naked helium star with a 10.80\Msun{} carbon-oxygen core after transferring 6.35\Msun{} to the secondary. It then ends its life as a 10.72\Msun{} BH. The secondary, which is now 21.31\Msun{}, evolves and fills its Roche lobe during the HG phase, similar to METISSE-MESA-SET2. However, unlike METISSE-MESA-SET2, a high core mass to total mass ratio in this case keeps mass transfer stable (Equation~\ref{eq:qcrit}) and despite reaching a similar mass ratio of primary mass to secondary mass as METISSE-MESA-SET2 during the AGB phase, the binary avoids the CE phase.
The mass transfer remains stable but discontinuous as the star evolves through core helium burning and the AGB phase.
Finally, the secondary ends its life in a supernova at 14.28\,Myr to form a 10.89\Msun{} BH. 
The orbital separation between the two stars remains quite close to the initial value, 2092\Rsun{}, and the two BHs form a wide binary. 

In all three cases, changes in core overshooting changes the stellar radius. Therefore, RLOF can be initiated at a different time and evolutionary phase in the binary system which, in turn, determines the amount of mass transferred to the secondary. Similarly, the larger size of the helium core -- and consequently of the carbon-oxygen core -- leads to large variations in the mass of the remnant formed by the secondary, from a 10.89\Msun{} BH in the case of METISSE-MESA-SET3 (with $\alpha_{\rm OVS} =0.55$), to a 1.36\Msun{} NS in the case of METISSE-MESA-SET1 ($\alpha_{\rm OVS} =0.11$).  
The same binary can undergo 0, 1 or 2 CE events depending on the value of overshooting used in the underlying stellar models, remarkably altering the predictions of final separation between the systems.
The evolutionary predictions in all three cases with MESA tracks are again different from the evolution computed using the \pols tracks in Section~\ref{sec:comparing_sse_metisse_with_bse}.
...
Mass transfer in a binary occurs when either star fills its Roche lobe and depending on the amount and the stability of mass transfer, it can completely alter the life of the component stars. 
...
This is evident in Figure~\ref{fig:pop_synth}, where we show the evolutionary state of the donor when the binary experiences mass transfer via RLOF for the first time and the different cases of ensuing mass transfer described in Section~\ref{sec:mass_transfer_binary}.
...
% \begin{table}
% \centering
% \begin{tabular*}{0.768\columnwidth}{|l|r|r|r|}
% \hline
% & BNS & BH-NS & BBH \\
% \hline

% SSE-POLS98 & 21.99 & 12.95 & 17.31\\
% METISSE-POLS98 & 22.47 & 9.92 & 19.60 \\
% METISSE-MESA-SET1 & 9.27 & 12.55 & 19.47\\
% METISSE-MESA-SET2 & 22.19 & 9.11 & 26.42\\
% METISSE-MESA-SET3 & 25.51 & 9.80 & 31.66\\

% \hline
% \end{tabular*}
% \caption{Percentage of different remnant systems, as predicted by SSE and METISSE with different sets of stellar models, for the population of isolated binaries uniformly distributed in mass and orbital periods shown in Fig.~\ref{fig:pop_synth_rem}.}
% \label{tab:rem_percentage}
% \end{table}
