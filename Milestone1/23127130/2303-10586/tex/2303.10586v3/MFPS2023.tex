...
%%
\def\copynames{J.-S. P. Lemay, J.-B. Vienney, \ldots}    %% Fill in the first initial and last name of the authors
...
\end{theorem}
\begin{proof} The proof is mostly the same as that of \cite[Thm 4.12]{blute2006differential}. Essentially, one uses the rule for a deriving transformation (resp. codereliction) to prove the rule of the same name for a codereliction (resp. deriving transformation). The only tricky one might be the product rule, so let us prove it here. Starting with a deriving transformation $\partial$, we use the product rule at $0$ and (\ref{bialgeq}), to prove the product rule for the codereliction: 
\begin{gather*}
...
Now suppose that we are also in an additive symmetric monoidal category. Then equalizer symmetric powers always give an $\mathbb{N}$-graded monoidal additive bialgebra differential modality where $\overline{\mathsf{d}} = 1_A$, $\overline{\mathsf{w}} = 1_I$, and $\overline{\mathsf{c}}_{n,m}$ is defined using the equalizer universal property, so is the unique map such that:
\begin{align}
...
\begin{equation}\begin{gathered} \xymatrixrowsep{1.75pc}\xymatrixcolsep{5pc}\xymatrix{ 
        ...
        \oc_r \oc_1 A \ar[r]_-{\oc_r(\mathsf{d})} & \oc_r A  & \oc_r \oc_{st} A \ar[r]_-{\oc_r(\mathsf{p}_{s,t})} & \oc_r \oc_s \oc_t A} \tag{\ref{comonadeq}} \end{gathered}\end{equation}
\begin{equation}\begin{gathered} \xymatrixrowsep{1.75pc}\xymatrixcolsep{2.5pc}\xymatrix{\oc_{r+s+t} A  \ar[r]^-{\mathsf{c}_{r+s,t}} \ar[d]_-{\mathsf{c}_{r,s+t}} & \oc_{r+s} A \! \otimes \! \oc_t A \ar[d]^-{\mathsf{c}_{r,s} \otimes 1} & \oc_r A \ar@{=}[dr] \ar[r]^-{\mathsf{c}_{r,0}}  \ar[d]_-{\mathsf{c}_{0,r}}   & \oc_r A \! \otimes \! \oc_0 A \ar[d]^-{1 \otimes \mathsf{w}}  & \oc_{r+s} A  \ar[r]^-{\mathsf{c}_{r,s}}  \ar[dr]_-{\mathsf{c}_{s,r}} & \oc_r A \! \otimes \! \oc_s A  \ar[d]^-{\sigma}\\
      \oc_r A\! \otimes \! \oc_{s+t} A \ar[r]_-{1 \otimes \mathsf{c}_{s+t}} & \oc_r A \! \otimes \! \oc_s A \! \otimes \! \oc_t A& \oc_0 A\! \otimes \! \oc_r A \ar[r]_-{\mathsf{w} \otimes 1}  & \oc_r A & & \oc_s A \! \otimes \! \oc_r A } \tag{\ref{comonoideq}} \end{gathered}\end{equation}   
      ...
      \oc_{rt}A \otimes \oc_{st} A \ar[r]_-{\mathsf{p}_{r,t} \otimes \mathsf{p}_{s,t}} & \oc_{r} \oc_t A \otimes \oc_r \oc_s A & & I } \tag{\ref{deltacomonoideq}}  \end{gathered}\end{equation}
...
\begin{equation}\begin{gathered} \xymatrixrowsep{1.75pc}\xymatrixcolsep{2pc}\xymatrix{\oc_r A \! \otimes \! \oc_r B \! \otimes \! \oc_r C  \ar[r]^-{\mu^\otimes_r \otimes 1} \ar[d]_-{1 \otimes \mu^\otimes_r} & \oc_r(A \!\otimes\! B) \! \otimes \! \oc_r C \ar[d]^-{\mu^\otimes_r}& \oc_r A  \ar@{=}[dr] \ar[d]_-{\mu^I_r \otimes 1} \ar[r]^-{1 \otimes \mu^I_r} & \oc_r A \! \otimes \! \oc_r I \ar[d]^-{\mu^\otimes_r} &  \oc_r A \! \otimes \! \oc_r B  \ar[r]^-{\sigma}  \ar[d]_-{\mu^\otimes_r} & \oc_r B \! \otimes \! \oc_r A \ar[d]^-{\mu^\otimes_r}\\  
      \oc_r A \! \otimes \! \oc_r(B \! \otimes\! C) \ar[r]_-{\mu^\otimes_r} & \oc_r(A \!\otimes\! B \!\otimes\! C) & \oc_r I\! \otimes \! \oc_r A \ar[r]_-{\mu^\otimes_r} & \oc_r A &  \oc_r(A \!\otimes\! B) \ar[r]_-{\oc_r(\sigma)} &  \oc_r(B \!\otimes\! A)  } \tag{\ref{smfeq}}  \end{gathered}\end{equation}
      %
\begin{equation}\begin{gathered} \xymatrixrowsep{1.75pc}\xymatrixcolsep{1.75pc}\xymatrix{\oc_{rs} A \! \otimes \! \oc_{rs} B  \ar[d]_-{\mathsf{p}_{r,s} \otimes \mathsf{p}_{r,s}} \ar[r]^-{\mu^\otimes_{rs}}  & \oc_{rs}(A \! \otimes \! B) \ar[dd]^-{\mathsf{p}_{r,s}} & I \ar[r]^-{ \mu^I_{rs}} \ar[d]_-{\mu^I_r} & \oc_{rs} I \ar[d]^-{\mathsf{p}_{r,s}}  & \oc_1 A \! \otimes \! \oc_1 B \ar[dr]_-{\mathsf{d} \otimes \mathsf{d}} \ar[r]^-{\mu^\otimes_1} & \oc_1(A \! \otimes \! B) \ar[d]^-{\mathsf{d}} & I \ar@{=}[dr]^-{} \ar[r]^-{ \mu^I_1} & \oc_1 I \ar[d]^-{\mathsf{d}} \\
  ...
  } \tag{\ref{demeq}}  \end{gathered}\end{equation}
  %
 ...
 \oc_{r+s}(A \! \otimes \! B) \ar[r]_-{\mathsf{c}_{r,s}} & \oc_r(A \! \otimes \! B) \!\otimes\! \oc_s(A \! \otimes \! B)  } \tag{\ref{DEmeq}}  \end{gathered}\end{equation}
 %
  ...
  }  \tag{\ref{DE!eq}}  \end{gathered}\end{equation}
