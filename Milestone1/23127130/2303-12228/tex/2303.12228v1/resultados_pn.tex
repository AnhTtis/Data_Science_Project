...
More generally, 
in order to simplify writing and notation, if we have a sequence of c\`adl\`ag processes with values in $\mathbb{R}^d$ of the form $\Sigma^{n}_t = \Sigma_{\lfloor n t \rfloor}$, we denote by $\Hat{\Sigma}^{n}_t$ its linearly interpolated version, i.e., 
\begin{equation*}
\Hat{\Sigma}^{n}_t := \Sigma_{\lfloor n t \rfloor} + (nt - \lfloor nt \rfloor)(\Sigma_{\lfloor nt \rfloor + 1} - \Sigma_{\lfloor nt \rfloor}) \,, \ t\ge 0 \,,
\end{equation*}
which is a random element of $C_{\Rs^d}[0, \infty)$. Moreover by ~\cite[Proposition 10.4, Chapter 3]{ethier2009markov} if we have convergence in distribution in the Skorohod space of $\{\Sigma_{\cdot}^n\}_{n\ge 1}$ to a continuous process, then we also have convergence in distribution in $C_{\Rs^d}[0, \infty)$ of $\{\Hat{\Sigma}_{\cdot}^n\}_{n\geq 1}$ to the same limit. Still about notation, we can also have $\Sigma^{n}_t = \Sigma_{\lfloor n t \rfloor}$ as a set value function and coherently we set
\begin{equation*}
\Hat{\Sigma}^{n}_t := |\Sigma_{\lfloor n t \rfloor}| + (nt - \lfloor nt \rfloor)(|\Sigma_{\lfloor nt \rfloor + 1}| - |\Sigma_{\lfloor nt \rfloor}|) \,, \ t\ge 0 \, .
\end{equation*}
...
\begin{remark}\label{rem:conver}
 ...
 \begin{itemize}
     \item[a)] If a sequence of processes converges in probability with respect to the uniform norm in $C_{\Rs^d}[0, T]$ for all $T >0$, then it  converges in probability in $C_{\Rs^d}[0, \infty)$ under the metric $\rho$ defined in \eqref{def:rho} (this is a well-known result which can be easily proved).
     \item[b)] If a sequence of processes is tight  in $C_{\Rs^d}[0, T]$ for all $T>0$ with the topology of uniform convergence in the compacts, then the sequence is  tight  in $C_{\Rs^d}[0, \infty)$ (see, e.g., \cite[Theorem 4.10,  Chapter 2]{karatzas2012brownian}). 
     \item[c)] \cite[Theorem 7.3]{billingsley1999probability}: A sequence of processes $\{\Hat{\Sigma}_{\cdot}^n\}_{n\geq 1}$ is tight in $C_{\Rs^d}[0, T]$  if and only if the following two conditions are satisfied:
     \begin{enumerate}[1.]
...
\subsection{Case $\beta>1/2$; proof of Theorem~\ref{pn-WGERW-Gauss}}\label{prova-pn-WGERW}
...
Recall that for the $p_n$-\Name*, the variables $\{U_i\}_{i \geq 1}$ are uncorrelated, rather than independent, with both sequences $\{\gamma_i\}_{ i\geq 1}$ and $\{\xi_i\}_{i \geq 1}$. 
...
We start stating a result that will be used in the proof of Theorem~\ref{pn-WGERW-Gauss}. %\cm{Due to this following Lemma it will be possible to see that the drift push through time will not make a difference for the process.}
...
\end{equation*}
Then $\{\Hat{D}^n_\cdot\}_{n\ge 1}$ as a sequence of random elements of $C_{\Rs^d}[0, \infty)$,  converges in probability to the identically zero function.
\end{lemma}
...
\begin{proof}[Proof of Theorem~\ref{pn-WGERW-Gauss}]
...
converges in probability to the identically zero function (as a sequence of random elements of $C_{\Rs^d}[0, \infty)$). For the first term in~\eqref{p_n-WGERW_incrementos} we  use~\cite[Theorem 7.1.4, 7.1.1]{ethier2009markov} to obtain that
...
The proof of Corollary~\ref{pnESRW->BM}(stating that the $p_n$-\Nametwo{} converges in distribution to a standard Brownian Motion) follows the same lines of the proof of Theorem~\ref{pn-WGERW-Gauss}. The main difference is  in~\eqref{xi_i->Z} where instead of using ~\cite[Theorem 7.1.4]{ethier2009markov}, we can apply Donsker's Theorem, since the $\{\xi\}_{i\geq 1}$ corresponding to  $p_n$-\Nametwo{} are i.i.d. with zero-mean vector and finite covariance matrix (see, e.g., \cite[Theorem 8.2]{billingsley1999probability} or~\cite[Theorem 5.1.2]{ethier2009markov}).
...
We begin showing  that $\{\Hat{D}_\cdot ^{n,\gamma}\}_{n\ge 1}$ converges in probability to the identically zero function in $C_{\Rs^d}[0, T]$ for all $T > 0$ (recall that $\Hat{D}_t ^{n,\gamma}$ is the linearly interpolated version of   $D_{\lfloor nt \rfloor}^{\gamma}$). Note that
...
\end{split}
\end{align*}
for some $c' > 0$, since $\sum_{i=1}^{k} \frac{1}{i^{\beta}} = O(k^{1-\beta})$. Since $\beta >1/2+\theta$, we obtain that $\{\Hat{D}^{n,\gamma}_{\cdot}\}_{n\ge 1}$ converges uniformly in probability to the identically zero function for all $T >0$.
%\begin{equation*}
...
computations as above, we show that $\{\Hat{D}^{n,\xi}_{\cdot}\}_{n\ge 1}$ also converges uniformly in probability to the identically zero function for all $T >0$.
...
We begin introducing some auxiliary results required to analyze the asymptotic behavior of the $p_n$-\Nametwo{} on $\ZZ^d$, with $\beta = 1/2$ and $d\geq 2$.
...
We now state two known results about the range of a random walk on $\ZZ^d$ with i.i.d. increments which is instrumental in our proofs. Henceforth, we denote by $\pi_d$ the probability that $\{Y_n\}_{n \geq 0}$ never returns to the origin (as in the statement of Proposition~\ref{prop:RangeERW}).
\begin{theorem}[From {\cite[pages 38-40]{spitzer2001principles}}\&{\cite[Theorem 1]{hamana2001large}}]\label{teo: RnZ>}
...
\begin{align*}
\tag{LLG}\frac{|\Rr_n^Y|}{n} \xrightarrow[n \to \infty]{} \pi_d \ \text{ a.s.}\,.
\end{align*}
...
\begin{enumerate}[i)]
    ...
    \item For $d= 2$, $\{\Hat{K}^n_{\cdot}/n^{1/2}\}_{n\geq 1}$ converges in probability as random elements of $C_{\Rs}[0, \infty)$ to the identically zero function.
\end{enumerate}
...
If $\lim_{n \to \infty} |K_{\lfloor nt \rfloor}|< \infty$,  then there exists a positive constant $L$ such that 
\begin{equation}\label{eq:prob02finito}
\lim_{n \to \infty} \sum_{i \in K_{\lfloor nt \rfloor}} \frac{ \Vert \gamma_i - \xi_i \Vert}{|K_{\lfloor nt \rfloor}|} \leq 2LK\,,
\end{equation}
...
%
This dimensional constraint   is crucial in our proof technique and it is due to how we use Theorem~\ref{teo: RnZ>} (LLN) in the proof of Proposition~\ref{prop: bound_Kn} (see, Remark~\ref{rem:restriction}).
...
\begin{corollary}\label{B'n->p}
Let $\{J_n(\delta)\}_{n \geq 1}$ be defined in~\eqref{Bn'} with $\delta \in (\pi_d, 1)$ and  $\{V_n(\delta')\}_{n \geq 1}$ be defined in~\eqref{Fn'} with $\delta' \in (0, \pi_{d-k})$. Then, it holds that: 
    \begin{equation*}
...
\begin{lemma}\label{lem: tightaux}
...
If Conjecture~\ref{conj_range} were true, we would be able to strengthen the claim of Proposition~\ref{prop: bound_Kn} and obtain that
\begin{equation*}
...
\frac{1}{\phi} \exp(-c n^{\frac{1}{2}})\exp\bigg(2(e-1)\sqrt{n}\left(\sqrt{t + \phi} - \sqrt{t} + 2 \right) \bigg) \le \eta \quad \forall n \ge n_0 \,.
\end{equation}
...
Item $ii)$: 
 By   Remark~\ref{rem:conver}-$a)$,  it is enough to prove that the sequence of processes $\{\Hat{K}^n_{\cdot}/n^{1/2}\}_{n\geq 1}$ converges in probability as random elements of $C_{\Rs}[0, T)$ to the identically zero function, for all $T > 0$. For the latter it suffices to prove  
...
\end{split}
\end{equation}
Since $\delta>0$ is arbitrary,  $\limsup_{n \to \infty} \PP[ G_n ] = 0$ for every $\varepsilon$ fixed. Therefore, for all $T > 0$ the sequence of processes $\left\{\Hat{K}^n_{\cdot}/n^{1/2}\right\}_{n\geq 1}$ converges in probability, as random elements of $C_{\Rs}[0,T]$, to the identically zero function.  %(see Lemma~\ref{lem: convprob}) \cm{pensar em uma referencia ou não ha necessidade?}. 
...
\begin{proof}[Proof of Lemma~\ref{lem: tightaux}.] To avoid clutter in the notation we omit the dependence on $\delta$ e $\delta'$. By Corollary~\ref{B'n->p} we already have the convergence of the finite-dimensional distributions. Therefore,  by~\cite[Theorem 7.1]{billingsley1999probability},  to prove points $i)$ and $ii)$ it only remains to prove that both sequences of processes are  tight in $C_{\Rs^d}[0, \infty]$. By Remark~\ref{rem:conver}-$b)$ we only need to prove tightness in $C_{\Rs^d}[0, T]$ for all $T>0$. The proof strategy is analogous to the one used in the proof of Theorem~\ref{pn-ERW-d=>4} which relies on Remark~\ref{rem:conver}-$c)$.  %
...
\end{split}    
\end{equation*}
and we obtain \eqref{eq: PnA2} choosing $\phi \in (0,1)$ sufficiently small such that $\sqrt{t+\phi} - \sqrt{t}< \varepsilon/4\sqrt{\delta}(e-1)$. 
% \begin{equation*}
...
\begin{proof}[Proof of Proposition~\ref{prop: bound_Kn}.] Note that the statement follows if
...
%
Hence on the right-hand side of~\eqref{eq: A_t^cM<} we have an intersection of two events whose probability converges to 1 as $n$ goes to infinity. Thus, for every $M > c >0$ 
\begin{equation*}
...
%\end{equation*}
%
Now suppose that we have monotonic decreasing and increasing  sequences $\{c_j\}_{j \ge 1}$ and $\{M_j\}_{j \ge 1}$ respectively, such that  $c_j \to 0$ and $M_j \to \infty$ as $j$ goes to infinity. Let $\{
...
\begin{equation*}
A:= \left\{\forall t \in [0, \infty): H_t \le 2(\delta t)^{1/2}  \right\} \,,   
\end{equation*}
and 
$$
A^{c_i, M_i} = \left\{\forall t \in [c_i, M_i]: H_t \le 2(\delta t)^{1/2}  \right\}\,,
$$
and $A = \cap_{i=1}^{\infty} A^{c_i, M_i}$.
%
Then,  by Portmanteau Theorem we have that for every $i \in \ZZ^+$
\begin{equation*}
\PP[A^{c_i, M_i}] \ge \limsup_{n \to \infty} \PP[A_n^{c_i, M_i}] =1 \,.    
\end{equation*}
%
Hence, $\PP[A^{c_i, M_i}] = 1$ for all $i \in \ZZ^+$ and we obtain the desired result, namely
\begin{equation}\label{eq:At_1}
\PP[A] = \PP\Big[ \bigcap_{j=1}^{\infty} A^{c_j, M_j}\Big] = 1 \,. 
\end{equation}
%
As far as the lower bound is concerned, 
...
\subsection{On the range of $p_n$-\Nametwo{} for $\beta=1/2$ and $d\geq 2$}\label{sec:rangeERW}
In this section, we provide the proof of  Proposition~\ref{prop:RangeERW}.  
...
% \end{remark}
The heuristic of the proof is the following: 
suppose that the $p_n$-\Nametwo{} at time $j$ visits a new site and gets excited. Then, after $k$ further steps it visits another new site and gets excited again. No excitation occurs between time $j+1$ and $j+k-1$. Thus, by the description of the model, we know that between time $j+2$ and $j+k$ the process evolves as a random walk with i.i.d. increments. The main idea behind the proof of  Proposition~\ref{prop:RangeERW} relies on this observation. Specifically, in each of these time windows, we can use the range of the random walk with i.i.d. increments to the upper bound of the range of the $p_n$-\Nametwo{}.
...
Denote by $\{N_i\}_{i \geq 0}$ the sequence of times 
...
\begin{equation} \label{pR_DNj}
|\Rr_{[n^{\varepsilon}, n]} ^X|  \leq
...
%
We pointed out that to deal properly with the rightmost sum in~\eqref{pR_DNj}, we need to keep in mind that $Y$ is independent of times $\{N_j\}_{j\ge 1}$.
...
|\Rr_{[n^{\varepsilon}, n]} ^X| \le \frac{n}{k} + k |A_{n,k}| + \sum_{j \in A_{n,k}^c} |\Rr_{[N_{j-1} + 2, N_j]}^Y|\,.
\end{equation}
...
Since $|A_{n,k}| = \sum_{j=1}^{M_n} \um_{\{\Delta N_j \leq k \}}$, using \eqref{eq: DNj<k},   for every $a>0$ it holds that
\begin{align*}
...
%\\
 \leq \binom{n}{a} \PP[G \leq k]^a
\\
& \leq \Big( \frac{ne}{a} \Big)^a \Big( 1 - \Big( 1 - \frac{1}{n^{\frac{\varepsilon}{2}}} \Big)^k \Big)^a
 \leq \Big( \frac{ne}{a} \Big)^a \Big( 1 - \exp{-\frac{3}{2}\frac{k} {n^{\frac{\varepsilon}{2}}}} \Big)^a \leq \Big( \frac{ne}{a} \times \frac{3k}{2n^{\frac{\varepsilon}{2}}} \Big)^a \,,
\end{align*}
...
\PP\Big[  \sum_{j=1}^{M_n} \um_{\{\Delta N_j \leq k \}}  > n^{1-\varepsilon/4} \Big] \leq \Big( \frac{3ek}{2n^{\frac{\varepsilon}{4}}} \Big)^{n^{1-\frac{\varepsilon}{4}}} \,.  
\end{equation}
...
%\end{align}
%
First, we observe that by a simple counting argument  
\begin{equation}\label{eq: R_DNj>y3}
|A_{n, \lceil \log^2 (n) \rceil}^c| \leq  \frac{n}{\lceil \log^2 (n) \rceil} , \ \forall \, n \,.    
\end{equation}
...
$$
\left\{ \exists j \in A_{n, \lceil \log^2 (n) \rceil}^c : |\Rr_{[N_{j-1} + 2, N_j]}^Y| > \gamma \Delta N_j\right\} \supseteq
...
%
Hence,  setting $\delta = \gamma$, the proof is complete. 
\end{proof}
