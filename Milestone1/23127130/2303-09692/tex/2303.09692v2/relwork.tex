...
Using this semantics, Kaminski~\cite{Kaminski2019a} developed an advanced weakest precondition calculus. 
Our probabilistic relations use a similar notation to the square bracket, called Iverson brackets. The predicates in probabilistic relations are UTP's alphabetised relations which have been used to establish program correctness using the weakest precondition calculus~\cite{Woodcock2004}. For this reason, our probabilistic relations can also describe the weakest pre-expectation semantics. 
...
denHartog2002, 
Chadha2007}, and 
VPHL~\cite{Rand2015} uses a weighted tree structure to represent probability distributions in its semantics and can reason about the partial correctness of probabilistic programs. The probabilistic relational Hoare logic (pRHL)~\cite{Barthe2009} is a Hoare quadruple that establishes the equivalence of two programs and the usual Hoare logic to relate programs as pre- and post-conditions. Probabilistic relations use UTP's alphabetised relations, which have been used to establish program correctness using Hoare logic~\cite{Woodcock2004}. For this reason, our probabilistic relations can also describe probabilistic Hoare logic semantics. The semantics of probabilistic relations are denotational, and two programs are equivalent if they are equal functions over the same state space.
...
Morgan and McIver's early work~\cite{Morgan1996b,McIver2005an} uses general techniques invariants and variants for reasoning about loops. Invariants for probabilistic loops are now expectations. Variants are still integer-valued expressions but 
\begin{inparaenum}[a)]
...
\end{align*}
\noindent
The expected run-time is the expectation of $t'$, simply the sequential composition of $\left(H ; t'\right) = t + 6$, denoting, on average, it takes six throws to have their outcomes equal.
