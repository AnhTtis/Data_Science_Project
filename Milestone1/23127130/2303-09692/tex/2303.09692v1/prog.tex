...
\label{sec:program}
This section is concerned with syntax and denotational semantics of our probabilistic programming language in Sect.~\ref{ssec:syntax_semantics}. Probabilistic recursion is not considered here and its syntax and semantics will be introduced in Sect.~\ref{sec:rec}. 

Before presenting the semantics, we define a notation of Iverson brackets in Sect.~\ref{ssec:prob_iverson} and introduce various expression types used in our language to constrain programs in Sect.~\ref{prob:type_abbr}. Our probabilistic programs are functions and they are characterised as probabilistic, distributions, or subdistributions in Sect.~\ref{prob:dist_functs}. Our definition of Iverson brackets is real-valued functions, but probabilistic programs are $\ureal$-valued functions. In order to use Iverson brackets in our semantics, we need to convert between these functions. The conversion is defined in Sect.~\ref{prob:type_abbr}.
...
Iverson brackets establish a correspondence between the predicate calculus and arithmetic, generalising the Kronecker delta.\footnote{Iverson brackets are a notation for the characteristic function on predicates. The convention was invented by Kenneth Eugene Iverson in 1962. The specific use of square brackets was advocated by Donald Knuth to avoid ambiguity in parenthesised logical expressions.}
\begin{definition}[Iverson bracket]
    The Iverson bracket of a predicate $P$ of type $[S]\upred$ defines a function $S \fun \real$, which gives a real number 0 or 1 if $P$ is false or true for particular state $s$ (of type $S$). \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_iverson_bracket.thy\#L45}
   \begin{align*}
       & \ibracket{P} \defs \usexpr{\IF P \THEN 1 \ELSE 0}
   \end{align*}
\end{definition}

A number of laws follow immediately from this definition.
\begin{thm}
    \label{thm:ib}
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_iverson_bracket.thy\#L63}
    \begin{align}
...
Laws~(\ref{thm:ib_neg}) to (\ref{thm:ib_plus}) establish direct correspondence between arithmetic, logic, and set operations. Laws~(\ref{thm:ib_max}) and (\ref{thm:ib_min}) show the maximum and mininum operations can be implemented using the Iverson bracket. Law~(\ref{thm:ib_summation}) shows summation over a subset of indices characterised by $P(k)$ can be expressed as summation over whole indices with the summation function $f(x)$ mulplified with the Iverson bracket of $P$. According to Donald E. Knuth~\cite{Knuth1992}, it is not easy to make a mistake when dealing with indices of summation by using the notation of the right side of the law. 
We omit other properties of Iverson brackets here for simplicity.

\subsection{Type Abbreviations}
\label{prob:type_abbr}
We define a number of type abbreviations for real-valued and $\ureal$-valued functions, and these abbreviations are used to type constructs in our language.
 \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L175}
\begin{align*}
...
We define two functions $\prrvfun$ (\isaref{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L184}) and $\rvprfun$ (\isaref{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L181}) to convert $P$ of type $[S_1, S_2] \prfun$ to an expression of type $[S_1, S_2] \rvfun$, and $f$ of type $[S_1, S_2] \rvfun$ to an expression of type $[S_1, S_2] \prfun$.
\begin{definition}[Conversion of relational real-valued and $\ureal$-valued functions]
    \label{def:rvfun2prfun}
    \begin{align*}
        & \prrvfun(P) \defs \usexpr{(P)^r} \tag*{(Probabilistic programs to real-valued functions)} \label{def:rvfun_of_prfun}\\
        & \rvprfun(f) \defs \usexpr{(f)^u} \tag*{(Real-valued functions to probabilistic programs)} \label{def:prfun_of_rvfun}
    \end{align*}
\end{definition}
The notations $p^r$ and $r^u$ (Definition~\ref{def:u2r_r2u}) convert a $\ureal$ number to a real number and a real number to a $\ureal$ number respectively. In this paper, we use symbols $\prrvfunsym{P}$ and $\rvprfunsym{f}$ for $\prrvfun(P)$ and $\rvprfun(f)$.

\begin{rmk}
   We define two types $\prfun$ and $\rvfun$ due to two reasons.
   First, infinite summation and limits are defined over topological space, and so over real numbers which form a banach space, but not over $\ureal$. We, therefore, need to convert probabilistic programs into real-valued functions in order to calculate summation and limits. After calculation, the results are converted back to probabilistic programs. Second, two functions in parallel composition or joint probability are not necessary to be probabilistic, and they can be more general real-valued functions.
\end{rmk}
...
\begin{definition}[Nonnegative] \label{def:nonneg} 
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L15}
\begin{align*}
    & \isnonneg(p) \defs \tautology{p \geq 0}
\end{align*}
where $\tautology{e}$ is a tautology on predicate $e$ and is expanded to $\forall s @ e(s)$.
\end{definition}

A real-valued expression $p$ is \emph{probabilistic} if its range is real numbers between 0 and 1 inclusive. This kind of expressions is characterised by a function $\isprob$ of type $[S]\rexpr \fun \bool$.

\begin{definition}[Probability expression] \label{def:isprob} 
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L18}
\begin{align*}
...
\begin{thm}[Iverson bracket is probabilistic]
    ...
    $\isprob(\ibracket{p})$ 
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L76}
\end{thm}
...
\begin{definition}[Probabilistic distributions] \label{def:isdist} 
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L29}
    \begin{align*}
...
A probabilistic function is called a \emph{subdistribution} function if the probabilities of all states sum to less than or equalt o 1, which is characterised by a function $\issubdist$.
\begin{definition}[Probabilistic subdistributions] \label{def:issubdist} 
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L32}
    \begin{align*}
...
\begin{lem}
   $\isdist(p) \implies \issubdist(p)$ 
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L79}
\end{lem}

For relational real-valued expressions $p$ of type $[S_1, S_2]\rvfun$, we define three functions to specify if the final states of a program characterised by the expression $p$ is probabilistic, distributions, or subdistributions.
In order to specify these functions, we define a curried operator $\curried{p}$ ($\defs \lambda s~s' @ p(s, s')$) to turn $p$ into lambda terms, and so $\curried{p}(s)$ is a function from the final state $s'$ to real numbers. 
\begin{definition}[Final states are probabilistic, distributions, and subdistributions]
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L36}
    \begin{align*}
...
\begin{definition}[Summable on final states]
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L188}
    ...
    \end{align}
\end{definition}
The function $\summableonfinal$ characterises the relational expression $p(s)$ over final states are summable on the universe $\univ$ of state space for any initial state $s$. The function $\summableonfinals$ characterises the product of the expressions $p(s)$ and $q(s)$ over final states are summable on $\univ$.
...
\begin{definition}[Reachable final states]
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L194}
    \begin{align}
...
The $\finalreachables(p,q)$ characterises the possibility for $p(s)$ and $q(s)$ to reach a same state $s'$ from any initial state $s$.

Convergence and reachability of a relational expression $p$ can be derived from whether $p$ are a distribution or subdistribution over its final states, as shown below.
\begin{thm}\label{thm:final_distribtion}
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L377}
    \begin{align}
    ...
    \end{align}
\end{thm}
Law~\ref{thm:final_distribtion_1} shows if $p$ is a distribution over its final states, then $p$ is probabilistic, summable over its final states, and reachable. The second conjunct restates $p$ is a distribution over its final states.
Law~\ref{thm:final_subdistribtion} shows if $p$ is a subdistribution over the final states, then $p$ is probabilistic, the summation of $p$ over its final states is larger than 0, and less than or equal to 1, and $p$ is summable over its final states and reachable. The second and third conjuncts restate $p$ is a subdistribution over its final states.
...
\begin{definition}[Normalisation]
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L55}
\begin{align*}
    \norm(p) \defs \usexpr{p / \left(\infsum s: S @ p(s)\right)}
\end{align*}
\end{definition}
The $\norm(p)$ gives the distribution of the state space (both the initial state and the final state for a relational expression) in $p$. For example, suppose that $x$ is in the range of $1..n$. 
...
\begin{definition}[Normalisation of the final state]
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L58}
\begin{align*}
...
\begin{thm}
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L2109}
\end{thm}
...
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L554}
\end{thm}
...
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L574}
\end{thm}
...
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L561}
\end{thm}
...
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L582}
\end{thm}
...
\begin{definition}[Probabilistic programs]
 ...
 \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L224}
    \begin{align*}
...
%        & \pparallel{P}{Q} \defs \rvprfunsym{\norm \usexpr{\prrvfunsym{P} * \prrvfunsym{Q}}}\tag*{(Parallel composition or joint probability of probabilistic programs)} \label{def:prog_parallel_pp} \\ 
 %       & \pparallel{R}{Q} \defs \rvprfunsym{\norm \usexpr{R * \prrvfunsym{Q}}}\tag*{(Parallel composition or joint probability)} \label{def:prog_parallel_fp} \\ 
    ...
    \end{align*}
    %\begin{align*}
    %    \pskip & \defs \rvprfunsym{\ibracket{\II}} \tag*{(skip \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L224})} \label{def:prog_skip} \\
    %    \left(\passign{x}{e}\right) & \defs \rvprfunsym{\ibracket{x := e}} \tag*{(assignment \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L239})} \label{def:prog_assign} \\
    %    \left(\ppchoice{r}{P}{Q}\right) & \defs \rvprfunsym{\usexpr{\prrvfunsym{r} * \prrvfunsym{P} + \left(\rfone - \prrvfunsym{r}\right) * \prrvfunsym{Q}}}\tag*{(probabilistic choice \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L253})} \label{def:prog_pchoice} \\ 
    %    \left(\pcchoice{b}{P}{Q}\right)& \defs \rvprfunsym{\usexpr{\IF b \THEN \prrvfunsym{P} \ELSE \prrvfunsym{Q}}}\tag*{(conditional choice \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L283})} \label{def:prog_condchoice} \\ 
    %    \pseq{P}{Q} & \defs \rvprfunsym{\usexpr{\infsum v_0 @ \prrvfunsym{P}[v_0/\vv'] * \prrvfunsym{Q}[v_0/\vv]}}\tag*{(sequential composition \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L299})} \label{def:prog_seq} \\ 
%   %     & \pparallel{P}{Q} \defs \rvprfunsym{\norm \usexpr{\prrvfunsym{P} * \prrvfunsym{Q}}}\tag*{(Parallel composition or joint probability of probabilistic programs)} \label{def:prog_parallel_pp} \\ 
 %  %     & \pparallel{R}{Q} \defs \rvprfunsym{\norm \usexpr{R * \prrvfunsym{Q}}}\tag*{(Parallel composition or joint probability)} \label{def:prog_parallel_fp} \\ 
    %    \pparallel{R}{T} & \defs \rvprfunsym{\normf \usexpr{R * T}}\tag*{(parallel composition \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L327})} \label{def:prog_parallel_ff}
    %\end{align*}
\end{definition}
We note that the semantics of all these constructs are the conversion of the corresponding real-valued expressions to the $\ureal$-valued expressions by $\rvprfun$. 

The probability skip $\pskip$ is the $\ureal$ version of the Iverson bracket of the relational skip $\II$. It changes no variables and terminates immediately. On termination, the final state is equal to the initial state with probability 1; all other assignments to the final state have probability 0.
Similarly, the probability assignment ($\passign{x}{e}$) is the $\ureal$ version of the Iverson bracket of the relational assignment ($x := e$). Assignment is a one-point distribution of the final state. 
...
Sequential composition $(\pseq{P}{Q})$ is the serial composition of $\prrvfunsym{P}$ with $\prrvfunsym{Q}$. It is the conditional probability of $\prrvfunsym{Q}$, given $\prrvfunsym{P}$. $P$ and $Q$ have the same type of $[S]\prhfun$. $(\pseq{P}{Q})$ is the summation of the product of $\prrvfunsym{P}[v_0/\vv']$, the substitution of $v_0$ for $\vv'$ in $\prrvfunsym{P}$, and $\prrvfunsym{Q}[v_0/\vv]$, the substitution of $v_0$ for $\vv$ in $\prrvfunsym{Q}$, over their intermediate states $v_0$. Its semantics can be interpreted as: starting from an initial state $s$, the probability of $(\pseq{P}{Q})$ reaching a final state $s'$ is equal to the summation of the probability of $\prrvfunsym{Q}$ reaching state $s'$ from $v_0$, given the probability of $\prrvfunsym{P}$ reaching $v_0$ from $s$, over all intermediate states $v_0$. 
...
In Bayesian inference, the posterior probability of $A$ given $B$ is computed based on a prior probability, which is estimated before $B$ is observed, a likelihood function over $B$ given fixed $A$, and a model evidence $B$ according to Bayes' theorem given below.
\begin{align*}
...
In our programming language, the update of the posterior probability is modelled using parallel composition for learning new facts and sequential composition for making actions, such as the movement of robots. This is illustrated in the forgetful Monty, the robot localisation, and the cancer diagnosis examples in Sects.~\ref{ssec:cases_forgetful_monty}, \ref{ssec:cases_robot_localisation}, and \ref{ssec:cancer_diagnosis}.

\subsection{Top and bottom}
\label{ssec:prog_top_bot}
According to Theorem~\ref{thm:ureal_func_complete}, the set of probabilistic programs is a complete lattice under $\leq$. The top and bottom element of the lattice satisfy the properties below. 
\begin{thm}
    \label{thm:top_bot}
Provided that $P$ is a probabilistic program and $p$ is a real-valued function.
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L863}
    \begin{align*}
...
The constant $\ufone$ and $\rfone$ mutually correspond to each other in $\ureal$-valued and real-valued functions and they can be converted to each other. This is similar for $\ufzero$ and $\rfzero$.
...
\begin{thm}
    \label{thm:pskip}
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L909}
    \begin{align}
...
Probabilistic assignment is a distribution.
\begin{thm}
    \label{thm:prob_assign_finaldist}
    $\isfinaldist\left(\prrvfunsym{\passign{x}{e}}\right)$
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L946}
    %\begin{align*}
...
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L972}
    \begin{align}
...
\isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice.thy\#L274}
    \begin{align*}
...
\begin{thm}[Quasi-associativity]
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1148}
    \begin{align*}
...
\begin{thm}
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1252}
    \begin{align}
    ...
    \end{align}
\end{thm}
Law~\ref{thm:cchoice_final_dist} shows if $P$ and $Q$ are distributions, then the conditional choice is also a distribution. Laws~\ref{thm:cchoice_id} shows the conditional choice between $P$ and $P$ is $P$ itself. The conditional choice is a special form of probabilistic choice, as given in Law~\ref{thm:cchoice_pchoice}, with its weight $\ibracket{b}$ being the Iverson bracket of $b$: either 0 (if $b$ is evaluated to false) or 1 (if $b$ is evaluated to true). Conditional choice is also monotonic, shown in Law~\ref{thm:cchoice_mono}.
...
\begin{thm}
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1560}
    \begin{align}
...
An interesting Law~\ref{thm:pseq_one} (\isaref{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1698}) states if $P$ is a distribution over the final state, then sequence composition of $P$ with $\ufone$ is $\ufone$. Its proof is given below.
\begin{proof}
\begin{align*}
  & \pseq{P}{\ufone} \\
...
Sequential composition of two Iverson bracket expressions can be simplified to the summation of the Iverson bracket of a conjunction, as shown in Law~\ref{thm:pseq_ibracket} (\isaref{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1371}). This law is proved below.
\begin{proof}
\begin{align*}
...
A corollary of this law, given below, states that if the two expressions cannot agree on an intermediate state $v_0$, then the sequence is just $\ufzero$. 
\begin{thm}
    \label{thm:pseq_ibracket_contradictory}
   $c_1 \neq c_2 \implies \pseq{\rvprfunsym{\ibracket{x'=c_1}}}{\rvprfunsym{\ibracket{x=c_2}}} = \ufzero$ 
   \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1377}
\end{thm}
If the two expressions agrees on one intermediate state $c_1$, then the intermediate state can be ignored.
\begin{thm}
   ...
   \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1392}
\end{thm}
If the second expression is also a point distribution, then the intermediate state can be ignored but its final state is still specified.
\begin{thm}
   ...
   \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L1413}
\end{thm}
...
% \begin{definition}[Normalisation]
% Normalisation $\norm$ of an expression $P$ of type $[S_1, S_2]\relexpr$ is also an expression of the same type, but with the value of $P$ for each intial state $s$ and final state $s'$ pair divided by the summation of the values of $e$ over all reachable final states from $s$.
% \begin{align*}
%     %\norm(e) \defs \lambda (s, s') @ e (s, s') / \left(\infsum ss: S_2 @ e (s, ss)\right)
...
\begin{definition}[Alphabetised normalisation]
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L63}
\begin{align*}
...
\begin{definition}[Uniform distributions]
    ...
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_distribution.thy\#L69}
\begin{align*}
...
\end{align*}
\end{definition}
Basically, a uniform distribution of $x$ from a finite set $A$ is an alphabetised normalisation of a program $\ibracket{\Inf v \in A @ x := v }$, nondeterministic choice $\Inf$ of the value of $x$ from $A$, over $x'$. We note that here $\Inf$ is inside the Iverson bracket and is a UTP relation operator infimum which is simply disjunction $\Union$.

The uniform distribution operator satisfies properties below.
\begin{thm}
    \label{thm:uniform_dist}
    We fix $P$ of type $[S_1,S_2]\prfun$,
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L2145}
    \begin{align}
...
The \ref{def:prog_parallel_ff} is defined over real-valued functions and specifies a $\ureal$-valued function. It satisfies properties below.
\label{ssec:prog_parallel_comp}
\begin{thm}
    Fix $p$, $q$, and $r$ of type $[S_1, S_2] \rvfun$, and $P$, $Q$, and $R$ of type $[S_1, S_2] \prfun$.
    \isalink{https://github.com/RandallYe/probabilistic_programming_utp/blob/336ff3c17af12fd71446a50244ff04966bfa71e8/probability/probabilistic_relations/utp_prob_rel_lattice_laws.thy\#L2508}
    \begin{align}
...
Law~\ref{thm:pparallel_assoc} shows if $p$, $q$, and $r$ are nonnegative, both $p$ and $q$ are summable and reachable on their product, and both $q$ and $r$ are summable and reachable on their product, then parallel composition is associative. If, however, $p$, $q$, and $r$ are converted from probabilistic programs $P$, $Q$, and $R$, and also $Q$ is summable on its final state, then parallel composition is also associative (Law~\ref{thm:pparallel_assoc2}).

Law~\ref{thm:pparallel_uniform} shows if $A$ is finite and not empty, then parallel composition of a uniform distribution over $x$ from $A$ and $p$ can be simplified to a division whose numerator denotes the value of $p$ reaching a final state with $x$ being a particular value $v$ and whose denominator denotes the summation of the values of $p$ reaching final states with $x$ being any value $v$ from $A$.
