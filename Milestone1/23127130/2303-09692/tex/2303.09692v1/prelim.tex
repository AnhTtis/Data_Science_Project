...
Alphabetised predicates are presented in this representation of UTP through alphabetised expressions $[V,S] \uexpr$, which are parametric over the value type $V$ and the observation space $S$, and defined as total functions $S \fun V$. Predicates\footnote{In the rest of the paper, we use expressions, predicates, and relations to refer to alphabetised counterparts for simplicity.} are boolean expressions: $[S] \upred \defs [\bool, S] \uexpr$, an expression whose value type is boolean. Relations are predicates over a product space: $[S_1, S_2] \urel \defs [S_1 \cross S_2] \upred$, where $S_1$ and $S_2$ are the initial and final observation space, corresponding to undashed variables (input alphabet) and dashed variables (output alphabet). \emph{Homogeneous} relations have the same initial and final observation space: $[S] \hrel \defs [S, S] \urel$.
...
\begin{definition}[Constructs of sequential programs]
    ...
    \begin{align*}
        ...
        & \II \defs \left(x := x\right) \tag*{(skip)} \label{def:uskip} \\
        & P ; Q \defs \left(\exists s_0 @ P[s_0/s'] \land Q[s_0/s]\right)\tag*{(sequential composition)} \label{def:useq} \\
        & P \sqcap Q \defs \left(P \lor Q\right)\tag*{(nondeterminism)} \label{def:unondeter}
    \end{align*}
\end{definition}

Particularly, we need to emphasise the type of programs and their alphabets. In \ref{def:ucond}, $b$, $P$, and $Q$ are of type $[S]\upred$ and $\alpha b \subseteq \alpha P = \alpha Q$. In \ref{def:uassign}, $A$ is the observation space of a program, including variable $x$ and a set $w$ of other variables. We also use a simple syntax $w'=w$ here to denote conjunctions of equations over each variable in $w$. For example, if $w = \{y, z\}$, then $\left(w' = w\right) \defs \left(y' = y \land z' = z\right)$. The subscript $A$ in $:=_A$ is usually omitted because it can be derived from its context automatically. Skip $\II$ (\ref{def:uskip}) is a special assignment where no variable is changed and so the observation space stays the same. In sequential composition $P ; Q$ (\ref{def:useq}), two relations $P$ of type $[S_1, S_2]\urel$ and $Q$ of type $[S_2, S_3]\urel$ are composed because the output alphabet $S_2$ of $P$ is the same as the input alphabet $S_2$ of $Q$. The relational composition gives a program of type $[S_1, S_3]\urel$ with $s_0:S_2$ denotes the entire state, $P[s_0/s']$ for the substitution of the final observation $s'$ of $P$ by $s_0$, and $Q[s_0/s]$ for the substitution of the initial observation $s$ of $Q$ by $s_0$. Nondeterministic choice $P \sqcap Q$ (\ref{def:unondeter}) is simply a disjunction of relations.
...
In this model, an alphabetised predicate $\left(x = y + 1\right)$ with two observable variables $x$ and $y$ is expressed as $\left(\lambda s. \lget{x} s = \lget{y} s + 1\right)$. A relation is predicates over a product space, manipulated through a product lens $S_1 \lprod S_2$. The early relational example $\left(x' := x + 1\right)$, therefore, can be expressed as 
%
\begin{align*}
...
\end{align*}
%
where $\lfst$ and $\lsnd$ are the lens to project the first and the second element of a product space. By substituting the state $s$ with a pair $(s, s')$, this expression can be simplified to 
\begin{align*}
...
\end{align*}
%
To write UTP expressions in this way, however, is tedious and not very useful and intuitive for a good practice of programming because too many implementation details are presented. For this reason, Isabelle/UTP implemented a lifted parser to provide a transparent conversion between the lenses representation and the programming syntax like $\left(x' := x + 1\right)$. We denote this representation of UTP expressions as $\usexpr{expr}$, such as $\usexpr{x' : = x + 1}$ which is converted to \ref{eqn:lens_representation}.
...
Predicative programming~\cite{Hehner1984a,Hehner1984}, similar to programs-as-predicates, describes programs using first-order semantics or relational semantics as boolean expressions (predicates). A program has its input denoted by undashed variables and output denoted by dashed variables. Predicative programming also uses refinement to concern with program correctness.

Probabilistic predicative programming~\cite{Hehner2004,Hehner2011} generalises predicative programming from boolean to probabilistic. Notations are introduced for the probabilistic programming such as skip, assignment, conditional choice, probabilistic choice, sequential composition (conditional probability), parallel composition (joint probability), and recursion. Basically, these constructors, except parallel composition, deal with probabilistic programs whose outputs are distributions, or distribution programs. Parallel composition can deal with probabilistic programs whose outputs might not be distributions (non-distribution programs), and uses normalisation to give a distribution program. 
...
To reason about the termination of loops, a time variable is introduced to count iterations. Actually, this gives more information (time) than just termination~\cite{Hehner1999}. In this programming, the expected value of a number expression $e$ according to a distribution program $P$ is just the sequential composition of $P$ and $e$. If $e$ is a boolean expression, the sequential composition gives the probability that $e$ is true after the execution of $P$. 
With the time variable, the programming allows reasoning about the average number of the termination time. For example, on average it takes 
% Moreover, the expected number of iterations is given by $\Sigma_n^{\infty} n * 2 ^ {(-n)} = 2$
2 
flips of two fair coins to get the same outcome: both heads or both tails. 
...
% \end{inparaenum}
% 
% $\omega$ is the smallest limit ordinal, and larger than any natural numbers. It is a least upper bound of natural numbers. $\omega$ is also a smallest infinite ordinal. 
% 
...
% \textbf{Pre-order} satisfies Laws~\ref{law:reflexive} and \ref{law:transitive}. A \textbf{partial order} is a pre-order that also satisfies Law~\ref{law:antisym}. A \textbf{total} partial order additionally satisfies Law~\ref{law:total}.
% 
...
% \end{thm}
% \begin{proof}
% Suppose $(X,\leq)$ is a totally ordered sets, and so for any two elements $a, b \in X$, then 
% 
% \begin{align*}
...
Monotonic and anti-montonic functions in order theory are characterised using $\mono$ and $\antimono$ defined below.
...
Ascending and descending chains are monotonic and antimontonic functions whose domain is natural numbers. 
\begin{definition}[Chains]
    Provided $(X, \leq)$ is a poset and $f$ is a function of type $\nat \fun X$, then
    \begin{align*}
...
\begin{thm}[Knaster–Tarski fixed-point theorem]
    ...
    Provided $(X, \leq)$ is a complete lattice and $F: X \fun X$ is monotonic, then the set of fixed points of $F$ also forms a complete lattice. The least fixed point is the infimum of the pre-fixed points. 
    \begin{align*}
...
In the original definition of Scott continuity, both $X$ and $X'$ are directed-complete partial orders (dcpo). We fix them be complete lattices because every complete lattice is a dcpo~\cite{Abramsky1995} and we only consider complete lattices in this paper. If $X$ and $X'$ are the same lattice, the subscript of $\thsup{}$ in Definition~\ref{law:continuity} can be omitted.
...
Similarly, we prove $\thgfp{}~F$ is a fixed point of $F$ and it is also the greatest one.
\end{proof}
