...
% Define new colours.
\definecolor{morange}{RGB}{255,104,25}
\definecolor{mpurple}{RGB}{245,0,228}
\definecolor{mblue}{RGB}{19,15,255}
\definecolor{mgreentwo}{RGB}{25,125,25}
...
% Define a shorter command for \mathrm for use in indices.
\newcommand{\mr}[1]{\mathrm{#1}}
% Define other helpful shortenings.
\newcommand{\ci}{{\mr{c}, i}}
% Define argmin
...
\author{Matthias K{\"o}hler$^{1}$, Lisa Kr{\"u}gel$^{2}$, Lars Gr{\"u}ne$^{2}$, Matthias A. M{\"u}ller$^{3}$, and Frank Allg{\"o}wer$^{1}$% <-this % stops a space
\thanks{%
*%
The authors are thankful that this work was funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- AL 316/11-2 - 244600449; GR 1569/13-2 - 244602989; and under Germany's Excellence Strategy -- EXC 2075 -- 390740016.
%
}% <-this % stops a space
%
\thanks{$^{1}$Matthias K{\"o}hler and Frank Allg{\"o}wer are with the University of Stuttgart, Institute for Systems Theory and Automatic Control, Stuttgart, Germany
        {\tt\small \{koehler, allgower\}@ist.uni-stuttgart.de}}%
\thanks{$^{2}$Lisa Kr{\"u}gel and Lars Gr{\"u}ne are with the Universit{\"a}t Bayreuth, Mathematical Institute, Chair of Applied Mathematics, Bayreuth, Germany
        {\tt\small \{lisa.kruegel, lars.gruene\}@uni-bayreuth.de}}%
\thanks{$^{3}$Matthias A. M{\"u}ller is with the Leibniz University Hannover, Institute of Automatic Control, Hanover, Germany
        {\tt\small mueller@irt.uni-hannover.de}}%
}
...
\begin{abstract}
    ...
    It has been shown that such a scheme guarantees closed-loop stability and recursive feasibility for any reference, even if it is unreachable or time-varying.
...
In comparison to standard MPC, it retains feasibility despite changes in the reference, it possibly allows for shorter prediction horizons, and it guarantees convergence to the best reachable equilibrium with respect to the reference, which may not be an equilibrium of the plant or unreachable. 
...
Regarding performance, it has been shown for linear systems that MPC for tracking is optimal in a neighbourhood of the reference~\cite{Ferramosca2009, Ferramosca2011a} for a suitably chosen offset cost.
Apart from this, the degradation of the closed-loop performance when using MPC for tracking has not been examined.
...
\begin{assumption}\label{asm:offset_cost_indication}
    ...
    \end{equation}
\end{assumption}
For example, $T(r) = \Vert r - r_{\mathrm{e}} \Vert^2_S - \bar{T}$ with $S\succ 0$ and $\bar{T} = \Vert r_{\mathrm{d}} - r_{\mathrm{e}} \Vert^2_S$ satisfies this on bounded constraint sets.
...
In this section, we adapt the standard MPC for tracking scheme for nonlinear systems~\cite{Limon2018} and show that the best reachable steady state is asymptotically stabilised.
...
\begin{assumption}\label{asm:stage_cost_lower_and_upper_bound}
    ...
    \begin{equation}\label{eq:stage_cost_lower_and_upper_bound}
        c_1^{\ell} \Vert x - x_r \Vert_{2}^2 \le \bar{\ell}(x, r) \le c_2^{\ell} \Vert x - x_r \Vert_{2}^2
    \end{equation}
    with
    $
...
\begin{remark}
    ...
    satisfies Assumptions~\ref{asm:stage_cost_difference_bound} and~\ref{eq:stage_cost_difference_bound_with_linear_term} on bounded constraint sets due to:
    \begin{align*}
        &\ell(x, u, r_1) \le 2\ell(x, u, r_{2}) + 2\mu\vert r_{1} \vert_{r_{2}}^2,
            \\%%%
        &\ell(x, u, r_1) \le \ell(x, u, r_{2}) + \mu\vert r_{1} \vert_{r_{2}}^2 + \mu\delta\vert r_{1} \vert_{r_{2}}
    \end{align*}
    with $\mu = \max(\mu_{\mathrm{max}}(Q), \mu_{\mathrm{max}}(R))$ and $\delta = \sqrt{2}\max(\sup_{x\in X, r\in\mathcal{R}} \vert x \vert_{x_r}, \sup_{u\in U, r\in\mathcal{R}} \vert u \vert_{u_r})$.
\end{remark}
...
\begin{assumption}\label{asm:terminal_ingredients}
    ...
    \end{subequations}
\end{assumption}
Conditions~\eqref{eq:terminal_invariance}-\eqref{eq:terminal_constraint_satisfaction} and~\eqref{eq:terminal_cost_upper_bound} (combined with~\eqref{eq:stage_cost_lower_and_upper_bound}) are standard in MPC for tracking (cf.~\cite[Assm. 3]{Limon2018}, \cite[Assm. 2]{Koehler2020b}, \cite[Assm. 2]{Koehler2020a}).
The non-degeneracy condition~\eqref{eq:non-trivial_terminal_set} (cf.~\cite[Lem. 5]{Koehler2020b}, \cite[Assm. 2]{Koehler2020a}) allows us to conclude that a state is in the terminal set if it is sufficiently close to the artificial steady state. See, e.g.~\cite{Koehler2020a} for the design of suitable ingredients.
...
%it is possible to incrementally move the artificial reference once the state is close and the incurred increase in the tracking part of the objective function is proportionally bounded (cf.~\cite[Assm. 2]{Koehler2020b}).
...
\begin{proof}
    ...
    $
        c_{1}^{\ell} \Vert \hat{x} - x_{\hat{r}} \Vert_{2}^2 \le \bar{\ell}(\hat{x}, \hat{r}) \le \varepsilon \le c_1^{\ell}c_{\mathrm{b}}^2
    $
    which implies $\Vert \hat{x} - x_{\hat{r}} \Vert_{2} \le c_{\mathrm{b}}$, hence, from Assumption~\ref{asm:terminal_ingredients}, $\hat{x} \in \mathcal{B}_{c_{\mathrm{b}}}(x_{\hat{r}}) \subseteq \mathcal{X}^{\mathrm{f}}(\hat{r})$.
...
\begin{proof}
    ...
    \begin{align*}
        &J_N^{\mathrm{tc}}(\hat{x}, \hat{u}, \hat{r}) - J_N^{\mathrm{tc}}(x, u_N^*, r_N^*) + \lambda(N)(T(\hat{r}) - T(r_N^*)) 
            \\%%%
...
In the following, we show stability of the closed loop~\eqref{eq:closed-loop_system}, using the value function $V_N$ as a Lyapunov candidate.
...
\begin{proof}
    ...
    Let $x \in X$ such that $c_2^{\ell} \Vert x - x_{\mathrm{d}} \Vert_{2}^2 \le \varepsilon$.
    ...
    \begin{align}
        ...
        &\phantom{\le{}} - J_N^{\mathrm{tc}}(x(t), u_N^*(\cdot \vert t), r_N^*(x(t))) - \lambda(N)T(r_N^*(x(t)))
            \notag
            \\%%%
        &\le - \ell({x}(t), u_N^*(0 \vert t), r_N^*(x(t))) \stackrel{\eqref{eq:value_function_lower_bound}}{\le} 
        - a_{\mathrm{lo}}\vert x(t) \vert_{x_{\mathrm{d}}}^2
        \label{eq:value_function_decrease}
    \end{align}
    where the second inequality follows from the definition of $(\tilde{u}(\cdot \vert t+1), \tilde{r}(t+1))$ and~\eqref{eq:terminal_cost_decrease}.
...
%-------------------------------------------------------------------------------
% \subsection{Bound on the value function}
...
\begin{proposition}\label{prop:standard_MPC_stability}
    ...
    $c_{\mathrm{std}} > 0$%
    such that for all $t \in \mathbb{N}_0$ we have
...
\begin{proof}
    ...
    \begin{align*}
        ...
        c_{\mathrm{f}} c_2^{\ell} \vert x \vert_{x_r}^2,
            \\%%%
    ...
    %
    In the other case, $\delta_r \ge \vert r \vert_{r_{\mathrm{d}}} > \frac{c_{\mathrm{b}}}{2}$, where $\delta_r = \sup_{r,\hat{r}\in\mathcal{R}} \vert r \vert_{\hat{r}}$. 
    Pick $\theta$ and $\hat{r}$ from Assumption~\ref{asm:better_candidate_reference}.
    Then, from~\eqref{eq:candidate_reference_upper_bound}, $\vert x_{u_{\mathrm{f}}^r}(\tau, x) \vert_{x_{\hat{r}}} \le \vert x_{u_{\mathrm{f}}^r}(\tau, x) \vert_{x_r} + \vert r \vert_{\hat{r}} \le c_{\mathrm{b}}$ if $\theta\le\frac{c_{\mathrm{b}}}{2\delta c_1^{\mathrm{r}}}$. 
    ...
    we converge into $\mathcal{X}^{\mathrm{f}}(r^\prime)$ with $\vert r^\prime \vert_{r_\mathrm{d}} \le \frac{c_{\mathrm{b}}}{2}$ after $\tau(1 + c_{\tau})$ steps with $c_{\tau} \ge 4\frac{\alpha_{\mathrm{up}}^T(\delta_r) - {\alpha_{\mathrm{lo}}^T}^{-1}(\frac{c_{\mathrm{b}}}{2})}{c_2^{\mathrm{r}}\theta c_{\mathrm{b}}^2}$. 
    %
...
\begin{proof}
    ...
    \begin{align*}
        &J_K^{\mathrm{d}}(x, \mu_N) = \sum_{k=0}^{K-1} \ell(x_{\mu_{N}}(k), \mu_{N}({x}_{\mu_{N}}(k)), r_{{\mathrm{d}}}) 
            \notag
            \\%%%
        &\stackrel{\eqref{eq:stage_cost_difference_bound_with_linear_term}}{\le} 
...
Compared to the transient performance bound~\eqref{eq:standard_MPC_value_function_performance_bound} for standard MPC (see~\cite[Chapter 8.4]{Gruene2017} for a discussion of~\eqref{eq:standard_MPC_value_function_performance_bound}), 
the bound~\eqref{eq:transient_performance_bound} contains an additional error term, which depends on the structure of the stage cost ($c_5^{\ell}$, $c_6^{\ell}$) and on the shape of the set of equilibria ($\vert r_N^* \vert_{r_{{\mathrm{d}}}}$).
...
In this section, we examine whether the error terms in~\eqref{eq:transient_performance_bound} stay bounded if $K \to \infty$, and vanish if $N \to\infty$.
The former is the case, since the closed loop is exponentially stable.
...
As we show in the following example, scaling the offset cost with the horizon, i.e. keeping it in relation to the $N$ terms of the stage cost, is pivotal for the error terms in~\eqref{eq:transient_performance_bound} to vanish if the horizon grows to infinity.
%-------------------------------------------------------------------------------
\begin{example}
    We present a simple example showing that the artificial reference does not, in general, converge in \emph{open-loop} to the best reachable one for arbitrarily large horizons,
    if $\lambda(N)$ is chosen constant, e.g. $\lambda(N) \equiv 1$.
    ...
    $
        \min_{u, r} \sum_{k=0}^{N-1} (x_u(k) - x_r)^2 + u(k)^2 + x_r^2
...
\begin{theorem}\label{thm:asymptotic_performance_bound}
    ...
    \begin{equation*}%\label{eq:asymptotic_performance_bound}
...
We choose a quadratic stage cost with $Q = I_2$ and $R = 0.01$, and $T(r) = 0.01\vert x_{r,1} \vert_{x_{\mathrm{d},1}}^2 + 1000\vert x_{r,2} \vert_{x_{\mathrm{d},2}}^2 + \vert u_r \vert_{u_{\mathrm{d}}}$.
...
It can be observed that a larger horizon allows a more optimal trajectory further from the steady state manifold, since the MPC for tracking has more steps to reach its neighbourhood, and
the infinite horizon solution is approximated.
Note that a standard MPC is infeasible with $N \le 500$, whereas in this simulation, MPC for tracking already shows a good performance for $N=100$.
...
\begin{figure}[bp]
    \setlength\axisheight{0.73\linewidth}
    \setlength\axiswidth{0.95\linewidth}
    \centering
    \input{fig_state_space_evolution_arxiv.tex}
    \caption{Closed-loop evolution of MPC for tracking with different horizons. The infinite horizon solution is dashed and the steady state manifold dotted.}
