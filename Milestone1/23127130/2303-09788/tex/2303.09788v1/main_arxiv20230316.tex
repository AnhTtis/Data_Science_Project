...
\newcommand{\transscale} {Trans-scale Quantum Science Institute, The University of Tokyo, Bunkyo-ku, Tokyo 113-0033, Japan}
...
\begin{abstract}
State-of-the-art algorithms for Hamiltonian simulation promise the ability to simulate a wide range of quantum systems. 
However, existing techniques rely on having a classical description of the desired Hamiltonian, preventing the manipulation of Hamiltonians given as black boxes. 
In this work, we present a quantum algorithm to achieve higher-order transformations of Hamiltonian dynamics, the input of which is a finite number of queries to the dynamics of a seed Hamiltonian given as a black box. The algorithm efficiently simulates the dynamics of any physically realizable Hamiltonian that can be obtained from the seed Hamiltonian by a linear transformation, using only Clifford operations with correlated randomness in addition to the seed Hamiltonian dynamics. This algorithm is an instance of what corresponds to a quantum version of functional programming, where the desired function is specified as a concatenation of higher-order quantum transformations.
As examples of our algorithm, we construct general functions that can simulate the negative time-evolution and the time-reversal of any Hamiltonian dynamics, 
and apply our algorithm to a Hamiltonian learning task for efficiently estimating a single parameter of a multi-parameter Hamiltonian. 
\end{abstract}
\maketitle
...
In this work, we study which Hamiltonian dynamics can be implemented given a \textit{seed Hamiltonian} $H$, without using a classical description of $H$.
...
\begin{algorithm}[H]
        ...
        \begin{itemize}
       	...
       	\begin{equation}
        		...
        		\label{single_pauli_transfer}
       			\vspace{-0.5em}
        	\end{equation}
            for any tensor products of Pauli operators $\sigma_{\vec{v}}:=\sigma_{v_1}\otimes \cdots \otimes \sigma_{v_n}$, where $\sigma_0 \!=\!I,\ \sigma_1\!=\!X,\ \sigma_2\!=\!Y,\ \sigma_3\!=\!Z$ and $\vec{u},\ \vec{v},\ \vec{w} \in \{0,1,2,3\}^n$ are the Pauli index vectors
    ...
    \Statex \hrulefill
    	...
    	\Statex \hskip1.0em Gates: $e^{-iH\tau}\ (\tau>0)$ and Clifford gates on $\mathcal{H}_c\otimes\mathcal{H}$
    	\Statex \hrulefill
...
We assume that we can invoke the Hamiltonian evolution $e^{-i H \tau}$ of a seed Hamiltonian $H \in \mathcal{L}(\mathcal{H})$ for any time $\tau>0$.
Given a hermitian-preserving linear map $f:\mathcal{L}(\mathcal{H})\to \mathcal{L}(\mathcal{H})$ represented in terms of the Pauli transfer matrices $\gamma$ \cite{chow2012universal} as in Eq.~(\ref{ptm}), our algorithm simulates the Hamiltonian evolution $e^{-if(H)t}$ for any $t >0$ representing the time for the transformed Hamiltonian dynamics up to an error $\epsilon >0$ and variance $8\epsilon$ (see proof in Appendix A, which relies on more general results proven in Appendix B). 
...
\begin{align}
    ...
    \end{cases}
\end{align}
since $f(H)$ does not depend on values of $\gamma_{\vec{w},\vec{u}}$ for $\vec{u}\notin J$. In this case, $\beta$ is calculated as $\beta = 2|J|$ and the runtime $O(\beta ^2t^2 n/ \epsilon)$ can be reduced depending on the size of $J$. We note that the runtime scales as $t^2$, meaning that in order to perform the time-reversal or negative time-evolution by this algorithm, the dynamics is slowed down quadratically. An application of simulating the negative time-evolution to Hamiltonian block encoding \cite{lloyd2021hamiltonian} is described in Appendix D.

Finally, we consider an application of our algorithm to Hamiltonian learning \cite{granade2012robust}.  Estimation techniques of parameters of unknown Hamiltonian for Hamiltonian learning have many applications in quantum sensing \cite{de2005quantum}, analyzing properties of quantum many-body physics \cite{wiebe2014quantum}, and quantum device calibration \cite{boulant2003robust}.  Recently, an estimation technique achieving the Heisenberg limit for the precision scaling in the estimation of parameters of a {\it low-interaction Hamiltonian} has been proposed \cite{huang2022learning}. Our algorithm can be used to extend this technique to a more general class of $n$-qubit Hamiltonians. In particular, given access to the dynamics $e^{-iH\tau}$ with an arbitrary time $\tau >0$ of any Hamiltonian $H=\sum_{\vec{v}\in \{0,1,2,3\}^n}c_{\vec{v}}\sigma_{\vec{v}}$ where each coefficient satisfies $|c_{\vec{v}}|\leq 1$, but is not necessarily positive as in the case of \cite{huang2022learning}, and an upper bound of its operator norm $\|H\|_{\mathrm{op}}$ is known, we can find an estimate $\hat{c}_{\vec{v}}$ of a {\it single} parameter represented by a Pauli coefficient $c_{\vec{v}}$ for a chosen $\vec{v}\in \{0,1,2,3\}^n$ with a standard deviation smaller than or equal to $s>0$ in a total evolution time $O(1/s)$. According to the Chebyshev inequality, the task of this protocol can be regarded as an estimation of ${c}_{\vec{v}}$ with an error smaller than or equal to $\epsilon = ks$ with the failure probability smaller than or equal to $1/k^2$ for an arbitrary $k>0$.

Our estimation algorithm consists of two steps. The first step simulates $e^{-if_{\vec{v}}(H)t}$ $(t>0)$ using the Hamiltonian dynamics $e^{-iH\tau}$ $(\tau >0)$ where $\vec{v}$ specifies $c_{\vec{v}}$ that we want to estimate and $f_{\vec{v}}$ is a hermitian preserving linear map chosen as 
$f_{\vec{v}}(H)=c_{\vec{v}}Y\otimes I\otimes \cdots \otimes I$.
This function $f_{\vec{v}}$ ``filters'' to keep only the coefficient $c_{\vec{v}}$ and changes all other coefficients to be zero and then send the coefficient $c_{\vec{v}}$ to the coefficient of $Y\otimes I\otimes \cdots \otimes I$, which is chosen for the convenience of the second step. The corresponding $\gamma$ is given by $\gamma_{\vec{w}\vec{u}}:=\delta_{\vec{w},(2,0,\ldots ,0)}\delta_{\vec{u},\vec{v}}$.   
The second step performs robust phase estimation \cite{kimmel2015robust} using $e^{-if_{\vec{v}}(H)t}$ similarly to the technique in \cite{huang2022learning} to obtain an estimate for $c_{\vec{v}}$, by measuring only the first qubit in our algorithm. The detailed procedure and an analysis of the total evolution time are given in Appendix E.

{\it Summary and outlook.---}
We presented a universal algorithm that can simulate any linear physically realizable hermitian preserving transformation of any Hamiltonian dynamics given as a black box. Our algorithm requires only a finite number of calls to the black box Hamiltonian dynamics and random pairs of correlated Clifford gates. We showed how our algorithm can simulate both the time-reversal and negative time-evolution of any unknown Hamiltonian dynamics, as well as an application to Hamiltonian learning, efficiently estimating a single parameter of a multi-parameter Hamiltonian. 
...
\begin{thebibliography}{40}%
    ...
    \bibitem [{\citenamefont {Huang}\ \emph {et~al.}(2022)\citenamefont {Huang},
      ...
      scaling},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal} {arXiv
      preprint arXiv:2210.03030}\ } (\bibinfo {year} {2022})}\BibitemShut {NoStop}%
...
Before presenting the sketch of the proof, we describe the error and variance of a simulation technique shown in the circuit (\ref{uhuuhu}), which is a key element in our algorithm.  Similar randomization techniques are also used in \cite{campbell2019random} (QDrift) and \cite{huang2022learning} (Hamiltonian learning).  The simulation error is evaluated based on \cite{campbell2019random}.  In addition, the variance is also evaluated for our algorithm.
...
with $h_j = \lambda p_j >0$
    in an error smaller than or equal to $\epsilon>0$ and the variance smaller than or equal to $8\epsilon$. 
...
To prove Lemma \ref{lem:reduced}, we need a lemma given by
\begin{lem}\label{simesi1}
        For a density operator $\rho\in \mathcal{L}(\mathcal{H})$, a pure state $\ket{\psi}\in\mathcal{H}$, and $a:=\bra{\psi}\rho \ket{\psi}$, the inequality
\begin{align}
...
\Proof
When $a\leq\frac{1}{2}$, since the inequality $4\sqrt{1-a}\geq 2\sqrt{2}\geq \|\rho \|_1 +\|\ket{\psi}\bra{\psi}\|_1$ holds, Eq.~(\ref{simesi1}) always holds. Thus, we only need to check Eq.~(\ref{simesi1}) for $a> \frac{1}{2}$.
Denoting the diagonal representation of $\rho$ as
        \begin{align}
            \rho =
            \sum_{k=0} \lambda_k\ket{k}\bra{k}\quad,
            \nonumber
        \end{align}
        where $\{ \ket{k} \}$ is the eigenbasis of $\rho$ and the corresponding eigenvalues $\{ \lambda_k \}$ are defined in a decreasing order  $\lambda_0\geq \lambda_1\geq \cdots$.  By noting $\lambda_0\geq a$, the left hand side of Eq.~(\ref{simesi1}) can be written as
        \begin{align}
            \| \rho -\ket{\psi}\bra{\psi}\|_1 &\leq 
            \| \rho -\ket{0}\bra{0}\|_1+
            \| \ket{0}\bra{0}-\ket{\psi}\bra{\psi}\|_1
            \nonumber\\
            &\leq 
            \label{kongo1}
            2(1-a)+\| \ket{0}\bra{0}-\ket{\psi}\bra{\psi}\|_1,
        \end{align}
using $\| \rho -\ket{0}\bra{0}|_1=\|(\lambda_0 -1)\ket{0}\bra{0}+\sum_k\lambda_k\ket{k}\bra{k}\|_1 = 2(1-\lambda_0)\leq 2(1-a)$.
        
        In general, for a normalized vector $\ket{\beta}, \ket{\gamma}\in \mathcal{H}$, $\|\ket{\beta}\bra{\beta}-\ket{\gamma}\bra{\gamma}\|_1=2\sqrt{1-|\bra{\beta}\gamma \rangle|^2}$ holds. Defining $\ket{\gamma ^{\prime}}=:e^{i\theta} \ket{\gamma}$ such that $\langle \beta\ket{\gamma ^{\prime}}\in \mathbb{R}$ holds and 
        $\ket{\delta ^{\pm}}:= \frac{\ket{\beta}\pm \ket{\gamma ^{\prime}}}{\sqrt{2}}$ satisfying $\bra{\delta ^+} \delta ^- \rangle=0$, we have
        \begin{align}
            &\ket{\beta}\bra{\beta}-\ket{\gamma}\bra{\gamma}=
            \ket{\beta}\bra{\beta}-\ket{\gamma ^{\prime}}\bra{\gamma ^{\prime}}
            \nonumber\\
            &=
            \frac{1}{2}
            \left[
                (\ket{\delta ^+}+\ket{\delta ^-})(\bra{\delta ^+}+\bra{\delta ^-})\right.  \nonumber\\
                &\quad -\left.
                (\ket{\delta ^+}-\ket{\delta ^-})(\bra{\delta ^+}-\bra{\delta ^-})
            \right]
            \nonumber\\
            &=\ket{\delta ^+}\bra{\delta ^-}+
            \ket{\delta ^-}\bra{\delta ^+}
            \nonumber\\
            &=\sqrt{\bra{\delta ^+}\ket{\delta ^+}\cdot \bra{\delta ^-}\ket{\delta ^-}}\cdot\nonumber\\
            &\left[
                \frac{\ket{\delta ^+}}{\sqrt{\bra{\delta ^+}\ket{\delta ^+}}}
                \frac{\bra{\delta ^-}}{\sqrt{\bra{\delta ^-}\ket{\delta ^-}}}+
                \frac{\ket{\delta ^-}}{\sqrt{\bra{\delta ^-}\ket{\delta ^-}}}
                \frac{\bra{\delta ^+}}{\sqrt{\bra{\delta ^+}\ket{\delta ^+}}}
            \right] .
            \nonumber
        \end{align}
        The last equation has a form similar to (constant)$\times X$, thus its one-norm can be calculated as
        \begin{align}
            &2(\sqrt{\bra{\delta ^+}\ket{\delta ^+}\cdot \bra{\delta ^-}\ket{\delta ^-}})
            \nonumber\\
            &=2(\sqrt{(1+|\bra{\beta}\ket{\gamma ^{\prime}}|)(1-|\bra{\beta}\ket{\gamma ^{\prime}}|)})
            \nonumber\\
            &=2\sqrt{1-|\bra{\beta}\ket{\gamma}|^2} \, .
            \nonumber
        \end{align}
        
Therefore, the last term of Eq.\ (\ref{kongo1}) can be written as $\|\ket{0}\bra{0}-\ket{\psi}\bra{\psi}\|_1=2\sqrt{1-|\bra{0}\ket{\psi}|^2}$.   By decomposing $\ket{\psi}$ as  $\ket{\psi}=b\ket{0}+\sqrt{1-|b|^2}\ket{0^{\perp}}$ using $b\in \mathbb{C}$ and $\ket{0^{\perp}}$ satisfying $\braket{0|0^{\perp}}=0$, we have
  \begin{align}
            &\bra{\psi}\rho\ket{\psi}=a
            \nonumber\\
            &\leq 
            |b|^2\lambda_0+
            (1-|b|^2)(1-\lambda_0)
            ...
            &=
            \left(
                \sqrt{2}\lambda_0-\frac{1}{\sqrt{2}}
            \right)
            \left(
                \sqrt{2}|b|^2-\frac{1}{\sqrt{2}}
            \right)+\frac{1}{2}
            \nonumber\\
            &\leq 
            \left(
            ...
            \nonumber
        \end{align}       
  where in the second inequality, the condition $\lambda_0\geq a> \frac{1}{2}$ is used. We also used the property $|b^2|> \frac{1}{2}$, which is a necessary condition for $a>\frac{1}{2}$.  Therefore, we obtain
        \begin{align}
            \label{ireru}
            |\braket{0|\psi}|^2=|b|^2\geq a \, ,
        \end{align}
        By substituting Eq.~(\ref{ireru}) to Eq.~(\ref{kongo1}), we obtain
        \begin{align}
            \| \rho -\ket{\psi}\bra{\psi}\|_1 &\leq 
            2(1-a)+\| \ket{0}\bra{0}-\ket{\psi}\bra{\psi}\|_1
            \nonumber\\
...
Using Lemma \ref{simesi1}, the left hand side of Eq.~(\ref{kaisimesi}) can be bound as
\begin{align}
    \sum_j p_j\|\ket{\psi}\bra{\psi}-
    \mathcal{G}_j(\ket{\psi}\bra{\psi})\| _1 ^2
    &\leq 
    \sum_j p_j (4\sqrt{1-a_j})^2
    \nonumber\\
    \label{hutatsume}
    &=16(1-\sum_jp_ja_j).  
\end{align}
...
\section{Appendix E: Runtime and total evolution time analysis on Hamiltonian learning}

In this appendix, we present the runtime and total evolution time analysis of our algorithm for the Hamiltonian learning task of efficiently estimating a single parameter from the dynamics of a $n$-qubit unknown Hamiltonian $H=\sum_{\vec{u}} c_{\vec{u}} \sigma_{\vec{u}}$ satisfying $(|c_{\vec{u}}|\leq 1)$ with the Pauli vectors $(\vec{v}\in \{0,1,2,3\}^n)$ presented in the main text. 

In the Hamiltonian transformation algorithm (Algorithm 1) applied for the Hamiltonian learning task, we choose the transformation of the Hamiltonian given by 
\begin{align}
f_{\vec{v}}(H)=c_{\vec{v}}Y\otimes I\otimes \cdots \otimes I.
\label{extract}
\end{align}
