%\begin{proof}
    ...
    \begin{itemize}
        \item Case $b+m = 0$. Then $b=m=0$, therefore $t \in \normal$, by \cref{lem:zero-counters-size-store}.\ref{lem:zero-counters}, and  $d = \size{t}$, by~\cref{lem:zero-counters-size-store}.\ref{lem:zero-size-store}. Let $u = t$ and $q=s$, then  we can conclude since $\size{(u,q)} = \size{u} =\size{t} = d$.
