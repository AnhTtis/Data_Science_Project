...
\textbf{Implementation.} RGI and all neural networks are implemented in PyTorch \cite{paszke2017automatic} and PyTorch Geometric \cite{Fey/Lenssen/2019}. \cref{RGI_pytorch} shows a PyTorch-like pseudo-code implementation of RGI. All our experiments are run in a single 16GB GPU. For linear evaluation, we have employed Sci-kit Learn library \cite{scikit-learn} for \textit{Amazon Computers}, \textit{Amazon Photos}, \textit{Coauthor CS} and \textit{Coauthor Physics} datasets and PyTorch for the \textit{PPI} dataset, taking the implementation from \cite{thakoor2021bootstrapped}.

\textbf{Optimization.} All models have been trained with Adam optimizer \cite{DBLP:journals/corr/KingmaB14} and a weight decay of $10^{-5}$. Additionally, we have employed a learning rate scheduler with linear warmup for $n_{warmup}$ epochs and cosine decay for the remaining $n_{epochs} - n_{warmup}$, where $n_{epochs}$ is the total number of epochs. The values of $n_{warmup}$ and $n_{epochs}$ for the different datasets can be found in \cref{hyperparameters}. In transductive settings, we perform full-graph optimization at each gradient step. Alternatively, for the \textit{PPI} dataset, which is multi-graph, we set a batch size of 1 graph for each gradient step due to memory constraints of the GAT encoder. 

\textbf{Node feature normalization.} Input node features are normalized row-wise with the L1 norm for transductive settings whereas no normalization is applied to \textit{PPI} node features. Before fitting the linear classifier with the obtained representations, they are row-wise L2 normalized.

\textbf{Hyperparamters} Some of the hyperparameters have been tuned with a small search whereas others have been fixed. \cref{hyperparameters} shows the hyper-parameters settings employed to obtain the results in \cref{results_transductive} and \cref{results_ppi}. The search space of the optimized hyper-parameters is the following:

\begin{itemize}
    \item Shift operator $(\mathbf{S})$ : $\{ \mathbf{D}^{-1} \mathbf{A}, \mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}, \mathbf{I} - \mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}\}$
    ...
    \item Dropout probability input graph $(p_{input})$ : $\{ 0.0, 0.5\}$
    \item Dropout probability graph local views $(p_{local})$ : $\{ 0.0, 0.5\}$  
\end{itemize}
The number of GNN layers $(L)$ and embedding size $(D)$ have been fixed according to previours reports for better comparability. The number of warmup epochs is set to be $n_{epochs} / 10$.
...
$\mathbf{S}$ & $\mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}$ & $\mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}$ & $\mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}$ & $\mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}$ & $\mathbf{D}^{-\frac{1}{2}} {\mathbf{A}} {\mathbf{D}}^{-\frac{1}{2}}$ \\
\midrule
