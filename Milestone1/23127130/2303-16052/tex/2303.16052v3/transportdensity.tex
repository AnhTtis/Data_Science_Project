...
% Note that in the Riemannian setting, if either 
% $\mu$ and $\nu$ is absolutely continuous with respect to the volume measure, the transport density does not depend on the choice of the transport plan (see \cite{Feldman}). See also \cite{Ambrosio3} and \cite{Feldman2} for the analogous result in $\mathbb{R}^n$. 
