...
% 	0\leq |L_{\phi}(\sigma)|\leq\|\phi\|_{\infty}\ell_H(\sigma).
% \end{equation} 
...
We assume that $\exists a,b\in\mathbb{R}_+$ such that the congestion function satisfies
...
% Given $Q\in\mathcal{Q}^p_{\mathbb{H}}(\mu,\nu)$, two points $x,y\in\overline{\Omega}$ and a horizontal curve $\sigma \in H^{x,y}$, the quantity $L_{\phi_{Q}}(\sigma)$ is the length of the curve weighted by the traffic intensity and the congestion function, defined as in Theorem \ref{prolLxi} and . We call \textit{congested metric} the function $c_{\phi_Q}$ defined as in section $4$.
...
% Minimizing \eqref{lepbme1} with the constraint $(e_0)_{\#}Q=\mu$ and $(e_1)_{\#}Q=\nu$ means that we only know the initial and  final  cortical activity (or the  distributions of vehicles and their destination), and we are interested in  minimizing the total cost: this corresponds to say that the set of admissible couplings $(e_0, e_1)_{\#}Q$ coincides with the whole set of transport plans $\Pi(\mu,\nu)$. This is often referred as SO (System Optimum) type of movement. 

Additional constraints can be imposed on the problem. Specifically, one could minimize the total cost under the constraint $(e_0,e_1)_{\#}Q\in\Pi\subset\Pi(\mu,\nu)$ and, as a particular case, consider $\Pi=\{\overline{\gamma}\}$. 
% that is the coupling is given and, roughly speaking, we a priori know the probability $\overline\gamma(x,y)$ for a vehicle in $x$ (or the signal starting from $x$) to reach a destination $y$. This is often called User Equilibrium (UE) type of movement. 
In this scenario, the set of horizontal traffic plans is defined as:
\begin{equation*}
	\mathcal{Q}_{\mathbb{H}}(\overline{\gamma}):=\left\{Q\in \mathcal{P}(H) \mid (e_0,e_1)_\# Q=\overline{\gamma}\right\}
\end{equation*}
Wardrop equilibria are horizontal traffic plans that belong to:
