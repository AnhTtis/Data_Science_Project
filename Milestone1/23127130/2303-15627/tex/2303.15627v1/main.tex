...
\author{Arun Jambulapati\thanks{University of Washington, {\tt jmblpati@uw.edu}} \and Kevin Tian\thanks{Microsoft Research, {\tt tiankevin@microsoft.com}}}
\date{}
\begin{document}
...
\begin{enumerate}
    ...
    \item The relationship between area convexity and classical analyses of extragradient methods \cite{Nemirovski04, Nesterov07} is unclear. Can we obtain a unified perspective on extragradient convergence?\label{item:relate}
\end{enumerate}

A central goal of this work is to answer Question~\ref{item:relate} by revisiting the area convexity technique of \cite{Sherman17}, and developing a deeper understanding of its relationship to more standard tools in optimization theory such as relative smoothness \cite{BauschkeBT17, LuFN18} and the classical extragradient methods of \cite{Nemirovski04, Nesterov07}. As byproducts, our improved understanding of area convexity results in new state-of-the-art runtimes for \eqref{eq:boxsimplex} by affirmatively answering Question~\ref{item:nolog}, and the first accelerated solver for its natural matrix generalization by affirmatively answering Question~\ref{item:unstable}.
...
In several applications with nonnegative constraint matrices \cite{JambulapatiST19, AltschulerP20, AssadiJJST22}, this obstacle was circumvented by recasting the problem as a box-simplex game \eqref{eq:boxsimplex}, for which faster, deterministic, and efficiently-parallelizable solvers exist.\footnote{Two notable additional problems captured by box-simplex games, maximum flow \cite{Sherman17} and transshipment \cite{AssadiJJST22}, do not have a nonnegative constraint matrix (as flows are signed), so positive LP solvers do not apply.} This typically proceeds by writing $\ma x \le b$ as a soft constraint by placing $\norm{\ma x - b}_\infty$ in the objective. Then, structure of the problem is used to argue that the minimizer cannot obtain good objective value without approximately satisfying $\ma x \le b$ (see e.g.\ Section 4.1 of \cite{AssadiJJST22} for the reduction in the bipartite matching case). Careful use of this strategy with a binary search on $\opt$ can often then lead to strong multiplicative approximation guarantees required by applications \cite{Sherman13, AltschulerP20, AssadiJJST22}.

The current state-of-affairs in applications of fast SDP solvers is very similar. For a variety of problems in robust statistics (where the goal is to approximate a covariance matrix or detect a corruption spectrally) \cite{ChengDG19, CherapanamjeriM20, Jambulapati0T20} and numerical linear algebra (where the goal is to reweight or sparsify a matrix sum) \cite{Lee017, ChengG18, JambulapatiLMST21}, positive SDP solvers have found utility. However, these uses appear to be quite brittle: current positive SDP solvers \cite{Allen-ZhuLO16, PengTZ16, Jambulapati0T20} only handle the special case of packing SDPs, preventing their application in more challenging settings. In an effort to bypass this potential obstacle, we develop a general-purpose nearly-linear time solver for box-spectraplex games in Theorem~\ref{thm:boxspectraplex}. Beyond the fundamental nature of the problem it solves, we are optimistic that it will find use in applications captured by structured SDPs.
...
with probability $\ge 1 - \delta$, where $L \defeq \|\sum_{i \in [n]} |\ma_i|\|_{\textup{op}} + \normop{\mb}$.
\end{restatable}

Theorem~\ref{thm:boxspectraplexintro} follows as a special case of a general result we give as Theorem~\ref{thm:boxspectraplex}, with more refined dependences and is stated in terms of the cost to perform certain queries of matrix exponentials required by our algorithmic framework, typical of first-order methods over $\Delta^{d \times d}$. 

To put Theorem~\ref{thm:boxspectraplexintro} in context, we compare it to existing solvers for simplex-spectraplex games, a relative of \eqref{eq:boxspectraplex} where both players are $\ell_1$ constrained. Simplex-spectraplex games are more well-studied in the literature \cite{KalaiV05, WarmuthK06, AroraK07, BaesBN13, Allen-ZhuL17, CarmonDST19}, and the state-of-the-art algorithms \cite{BaesBN13, Allen-ZhuL17, CarmonDST19} query $\tO(L^{2.5}\eps^{-2.5})$ vector products in $\{\ma_i\}_{i \in [n]}$ and $\mb$. On the other hand, Theorem~\ref{thm:boxspectraplexintro} requires $\tO(L^{3.5}\eps^{-3.5})$ products, and also assumes access to $\{|\ma_i|\}_{i \in [n]}$. In important applications of interest, such as robust statistics and spectral sparsification (where $\{\ma_i\}_{i \in [n]}$ are rank-one), this additional assumption is not restrictive. Nonetheless, we believe it is interesting to understand whether access to $\{|\ma_i|\}_{i \in [n]}$, and the $\tO(L\eps^{-1})$ overhead incurred by Theorem~\ref{thm:boxspectraplexintro}, can be avoided; we discuss the latter point in the following Section~\ref{ssec:approach}.
...
\paragraph{Improved subproblem solvers.} Our first observation is that the Hessian domination condition used by \cite{Sherman17} to ensure rapid convergence of subproblems can be naturally viewed through the lens of \emph{relative smoothness} \cite{BauschkeBT17, LuFN18}, a more standard condition in optimization theory. Simple facts from convex analysis (Section~\ref{ssec:convex}) imply that after minimizing over the box variable, the induced function $\tf_v(y) \defeq \min_{x \in [-1, 1]^n} f_v(x, y)$ is relatively smooth with respect to $h$. By tuning $\alpha$ in \eqref{eq:reg_intro}, we can further ensure $\tf_v$ is relatively strongly convex in $h$, and use methods for relatively well-conditioned optimization from \cite{LuFN18} off-the-shelf to minimize $\tf_v$ (and hence also $f_v$). 
...
\min_{x \in [-1, 1]^n} \max_{y \in \Delta^d} x^\top \ma y - b^\top y + c^\top x,
\]
for some $\ma \in \R^{n \times d}$, $b \in \R^d$, and $c \in \R^n$. We will design methods computing approximate saddle points to \eqref{eq:boxsimplex} which leverage the following family of regularizers:
...
Analogously to the box-simplex setting, we use the following two-parameter family of regularizers:
...
\subsection{Convex analysis tools}\label{ssec:convex}

In this section we provide several tools on convex analysis, particularly on partial optimization of functions over multiple variables, which are used throughout the paper.

\begin{lemma}\label{lem:partialderiv}
Let $\xset \subseteq \R^m$ and $\yset \subseteq \R^n$ be convex compact subsets. Suppose $F: \xset \times \yset \to \R$ is jointly convex over its argument $(x, y) \in \xset \times \yset$. For $y \in \yset$, define $x(y) \defeq \argmin_{x \in \xset} F(x, y)$ and $f(y) \defeq F(x(y), y)$. Then for all $y \in \yset$, $\partial_y F(x(y), y) \subset \partial f(y)$.
\end{lemma}
\begin{proof}
Let $x \defeq x(y)$, $z \in \yset$, and $w \defeq x(z)$. We first claim that $0 \in \partial_x F(x, y)$. To see this, the definition of the subgradient set implies that it suffices to show for all $x' \in \xset$, $F(x, y) \le F(x', y)$, which is true by definition. Hence by convexity of $F$ from $(x, y)$ to $(w, z)$, we have the desired
\[f(z) = F(w, z) \ge F(x, y) + \inprod{\partial_y F(x, y)}{z - y} = f(y) + \inprod{\partial_y F(x, y)}{z - y}.\]
\end{proof}

\begin{lemma}\label{lem:relativesmooth}
In the setting of Lemma~\ref{lem:partialderiv}, suppose for any $x \in \xset$, $F(x, \cdot)$ (as a function over $\yset$) always is $r: \yset \to \R$ plus a linear function (where the linear function may depend on $x$). Then $r - f$ is convex, and $f - q$ is convex for any $q: \yset \to \R$ such that $F - q$ (as a function over $\xset \times \yset$) is convex.
\end{lemma}
\begin{proof}
For the first claim, for any $y, z \in \yset$, letting $x \defeq x(y)$, we have
\begin{gather*}
...
\end{gather*}
The first equality used that the first-order expansion of $F(x, \cdot)$ agrees with the first-order expansion of $r$ (as they only differ by a linear term), and the second equality used $F(x, y) = f(y)$ and Lemma~\ref{lem:partialderiv}. The only inequality used the definition of $f$. For the second claim, note that $x(y)$ also minimizes $F - q$ over $\xset$ for any fixed $y$, as $q(y)$ is a constant in this objective. Since $F - q$ is convex and partial minimization of a convex function preserves convexity, we have the conclusion.
\end{proof}

We now introduce a notion of stability for operators $g$, which we call \emph{relaxed relative Lipschitzness}. We will use it to analyze algorithms for approximating \emph{variational inequalities} in Appendix~\ref{app:xgrad_convergence}, i.e.\ problems asking to find $z$ with small $\inprod{g(z)}{z - u}$ for all $u$ in the domain.
...
\subsection{Analysis tools}\label{ssec:oracles_def}

We collect definitions and facts (some from prior work) which will help in our analysis. We begin by stating two definitions whose guarantees, when combined with Definition~\ref{def:areaconvex}, will effectively give our conceptual algorithm for \eqref{eq:boxsimplex}. We then develop subroutines which satisfy these definitions.

\begin{definition}[Gradient step oracle]\label{def:Ograd}
For a problem \eqref{eq:boxsimplex}, we say $\Ograd: \zset \times \zset^* \to \zset$ is an $(\alpha, \beta)$-\emph{gradient step oracle} if on input $(z, g)$, it returns $z' $ such that for all $u \in \zset$, 
\[\inprod{g}{z' - u} \le V^{(\alpha + \beta)}_z(u) - V^{(\alpha)}_{z'}(u) - V^{(\alpha)}_z(z').\]
\end{definition}
\begin{definition}[Extragradient step oracle]\label{def:Oxgrad}
For a problem \eqref{eq:boxsimplex}, we say $\Oxgrad: \zset \times \zset^* \times \yset \to \zset$ is an $(\alpha, \beta)$-\emph{extragradient step oracle} if on input $(z, g, \by)$, it returns $(z^+, \by^+)$ such that
\begin{align*}
\inprod{g}{z^+ - u} &\le V^{(\alpha)}_z(u) - V^{(\alpha)}_{z^+}(u) - V^{(\alpha)}_z(z^+) \\
&+ V^{\beta h}_{\by}(u\y) - V^{\beta  h}_{\by^+}(u\y) \text{ for all } u = (u\x, u\y) \in \zset .
\end{align*}
\end{definition}

Finally, we state some general useful properties of $\rlp{\alpha}$.
...
We recognize the conclusion of Lemma~\ref{lem:combine} as resembling the typical guarantee required to make progress by extragradient methods \cite{Nemirovski04, Nesterov07} (see Section 3 and Appendix D.2 of \cite{CohenST21} for a recent tutorial). In particular, a scaling of the left-hand side upper bounds duality gap of the point $z'$, and the right-hand side telescopes (and may be bounded using the following fact).

\begin{lemma}\label{lem:rdivbound}
Let $\alpha ,\gamma \ge 0$, and let $z_0 = (x_0, y_0)$ where $x_0 = \0_n$ and $y_0 = \frac 1 d \1_d$. Then $z_0$ is the minimizer of $\rlp{\alpha}$ over $\zset$, and
\[V^{(\alpha)}_{z_0}(u) \le 1 + \alpha \log d,\; V^{\gamma h}_{y_0}(u\y) \le \gamma \log d \text{ for all } u = (u\x, u\y) \in \zset.\]
\end{lemma}
\begin{proof}
...
\begin{align*}
V^{(\alpha)}_{z_0}(u) &= \rlp{\alpha}(u) - \rlp{\alpha}(z_0) - \inprod{\nabla \rlp{\alpha}(z_0)}{u - z_0} \\
&= \rlp{\alpha}(u) - \rlp{\alpha}(z_0) \le 1 + \alpha \log d,
\end{align*}
...
In this section, we give generic constructions of gradient and extragradient step oracles. 

\begin{algorithm2e}[H]
	\caption{\textsf{GradStepOracle}($z, g, \alpha, \beta$)}
	\label{alg:gradstep}
	\DontPrintSemicolon
	{\bfseries Input:} $z = (x, y) \in \zset$, $g = (g\x, g\y) \in \zset^*$, $\alpha, \beta \ge 0$ \;
        $y' \gets \argmin_{\hy \in \yset} \inprod{g\y + \nabla_y \rlp{\alpha}(x(y), y) - \nabla_y \rlp{\alpha}(z) }{\hy} + V^{\beta h}_{y}(\hy)$, where for all $\hy \in \yset$,
        \begin{equation}\label{eq:bestx}x(\hy) \defeq \argmin_{\hx \in \xset} \inprod{g\x - \nabla_x \rlp{\alpha}(z)}{\hx} + \rlp{\alpha}(\hz) \text{ where } \hz \defeq (\hx, \hy), \end{equation} \;
        $x' \gets x(y')$ \;
	 {\bfseries Return:} $(x', y')$
\end{algorithm2e}
...
\begin{aligned}
\inprod{\nabla_y \rlp{\alpha}(x(\hy), \hy) - \nabla_y \rlp{\alpha}(x(y), y)}{\hy - u\y} &= \inprod{\partial f(\hy) - \partial f(y)}{\hy - u\y}\\
...
so combining with \eqref{eq:step3ograd} we have for $u = (u\x, u\y)$
\begin{align*}
\inprod{g + \nabla \rlp{\alpha}(z') - \nabla \rlp{\alpha}(z)}{z' - u} \le V^{\beta h}_{y}(u\y).
\end{align*}
The conclusion follows by using the identity \eqref{eq:threepoint} to rewrite $\langle \nabla \rlp{\alpha}(z') - \nabla \rlp{\alpha}(z), u - z'\rangle$.
\end{proof}

\begin{algorithm2e}[H]
	\caption{\textsf{XGradStepOracle}($z, g, \by, \alpha, \beta$)}
	\label{alg:xgradstep}
	\DontPrintSemicolon
	{\bfseries Input:} $z = (x, y) \in \zset$, $g = (g\x, g\y) \in \zset^*$, $\by \in \yset$, $\alpha, \beta \ge 0$ \;
        $y^+ \gets \argmin_{\hy \in \yset} \inprod{g\y + \nabla_y \rlp{\alpha}(x(\by), \by) - \nabla_y \rlp{\alpha}(z) }{\hy} + V^{\beta  h}_{\by}(\hy)$ (following notation \eqref{eq:bestx}) \;
        $x^+ \gets x(y^+)$ \;
        $\by^+ \gets \argmin_{\hy \in \yset} \inprod{g\y + \nabla_y \rlp{\alpha}(x^+, y^+) - \nabla_y \rlp{\alpha}(z) }{\hy} + V^{\beta h}_{\by}(\hy)$\;
...
&+ \inprod{\nabla_y \rlp{\alpha}(z^+) - \nabla_y \rlp{\alpha}(x(\by), \by)}{y^+ - \by^+},
\end{aligned}
\end{equation}
where we observe that
\begin{align*}
\inprod{g\y + \nabla_y \rlp{\alpha}(z^+) - \nabla_y \rlp{\alpha}(z)}{y^+ - u\y} - \inprod{\nabla_y \rlp{\alpha}(z^+) - \nabla_y \rlp{\alpha}(x(\by), \by)}{y^+ - \by^+} \\
= \inprod{g\y + \nabla_y \rlp{\alpha}(z^+) - \nabla_y \rlp{\alpha}(z)}{\by^+ - u\y} + \inprod{g\y + \nabla_y \rlp{\alpha}(x(\by), \by) - \nabla_y \rlp{\alpha}(z)}{y^+ - \by^+}.
\end{align*}
...
\begin{aligned}\inprod{\nabla_y \rlp{\alpha}(z^+) - \nabla_y \rlp{\alpha}(x(\by), \by)}{y^+ - \by^+} &= \inprod{\partial f(y^+) - \partial f(\by)}{y^+ - \by^+} \\
...
\begin{algorithm2e}[H]
	\caption{\textsf{BoxSimplex}($\ma, b, c$)}
...
\restateboxsimplex*
\begin{proof}
First, clearly the rescaling in Line~\ref{line:rescale} multiplies the entire problem \eqref{eq:boxsimplex} by $\frac 1 L$, so an $\frac \eps L$-approximate saddle point to the new problem becomes an $\eps$-approximate saddle point for the original. Throughout the rest of the proof it suffices to treat $L = 1$. We denote for all $0 \le t \le T$, $z_t \defeq (x_t, y_t)$, $z'_t \defeq (x'_t, y'_t)$, and let $\alpha = \beta = 2$, $\gamma = 4$, and $\eta = \frac 1 3$. By observation, Lines~\ref{line:ogradbeg} to~\ref{line:ogradend} implement Algorithm~\ref{alg:gradstep} (used in Lemma~\ref{lem:Ograd_alg}), with the parameters required by Lemma~\ref{lem:combine}. Similarly, Lines~\ref{line:oxgradbeg} to~\ref{line:oxgradend} implement Algorithm~\ref{alg:xgradstep} (used in Lemma~\ref{lem:Oxgrad_alg}), with the parameters required by Lemma~\ref{lem:combine}. Telescoping Lemma~\ref{lem:combine} and averaging, we have for $z_0 = (x_0, y_0)$ and any $u = (u\x, u\y) \in \zset$
\begin{align*}
\frac 1 T \sum_{t = 0}^{T - 1} \inprod{g(z'_t)}{z'_t - u} \le \frac{6\Par{V^{(4)}_{z_0}(u) + V^{4Lh}_{\by_0}(u\y)}}{T} \le \eps.
\end{align*}
The last inequality used the bounds in Lemma~\ref{lem:rdivbound} and the definition of $T$. Moreover since $g$ is bilinear, and $\hz \defeq (\hx, \hy)$ is clearly the average of the $z'_t$ iterates, we have $\inprod{g(\hz)}{\hz - u} \le \eps$. Taking the supremum over $u \in \zset$ bounds the duality gap of $\hz$ and gives the conclusion. For the runtime, without loss of generality $\nnz(\ma) \ge \max(n, d)$ (else we may drop columns or rows appropriately), and each of $T$ iterations is dominated by a constant number of matrix-vector multiplications.
\end{proof}
...
In this section, we state several useful properties about our regularizer $\rsdp{\alpha, \mu}$, with proofs deferred to \Cref{app:regularizer}. The main result of this section is the following: 
...
In Appendix~\ref{sec:matexpvec}, we give an implementation of $\Omatexp$ whose runtime depends polynomially on $\eps$ and polylogarithmically on $\gamma$.
...
For a problem \eqref{eq:boxspectraplex}, we say $\tOgrad: \zset \times \zset^* \to \zset$ is an $(\alpha, \beta, \mu, \Delta)$-approximate gradient step oracle if on input $(z, g)$, it returns $z'$ such that for all $u \in \zset$,
\[\inprod{g}{z' - u} \le V^{(\alpha + \beta, \mu)}_z(u) - V^{(\alpha, \mu)}_{z'}(u) - V^{(\alpha, \mu)}_z(z') + \Delta.\]
\end{definition}

\begin{definition}[Approximate extragradient step oracle]\label{def:approx_xgrad}
For a problem \eqref{eq:boxspectraplex}, we say $\tOxgrad: \zset \times \zset^* \times \yset \to \zset$ is an $(\alpha, \beta, \mu, \Delta)$-approximate extragradient step oracle if on input $(z, g, \bmy)$ it returns $(z^+, \bmy^+)$ such that
\begin{align*}
\inprod{g}{z^+ - u} &\le V^{(\alpha, \mu)}_z(u) - V^{(\alpha, \mu)}_{z^+}(u) - V^{(\alpha, \mu)}_z(z^+) \\
&+V^{\beta H}_{\bmy}(u\y) - V^{\beta H}_{\bmy^+}(u\y) \text{ for all } u = (u\x, u\y) \in \zset.
\end{align*}
\end{definition}
...
\begin{algorithm2e}[H]
	\caption{\textsf{ApproxGradStepOracle}($z, g, \alpha, \beta, \mu, \tx, \tnabla_x \rsdp{\alpha,\mu}(\cdot, \my)$)}
	\label{alg:approxgradstep}
	\DontPrintSemicolon
	{\bfseries Input:} $z = (x, \my) \in \zset$, $g = (g\x, g\y) \in \zset^*$, $\alpha, \beta, \mu \ge 0$, $\tx$, a $\Delta$-approximate best response oracle for $\rsdp{\alpha,\mu}$ and $\my$ or $\my'$ (defined below), $\tnabla_x \rsdp{\alpha,\mu}(\cdot, \my)$, a $\Delta$-approximate gradient oracle for $\nabla_x \rsdp{\alpha,\mu}(\cdot, \my)$ \;
        $v \gets \tnabla_x \rsdp{\alpha,\mu}\Par{x, \my}$ \;
        $\hx \gets \tx(\my, g\x - v)$\;
        $\my' \gets \argmin_{\hmy \in \yset}\inprod{g\y + \nabla_y \rsdp{\alpha, \mu}\Par{\hx, \my} - \nabla_y\rsdp{\alpha, \mu}(x, \my)}{\hmy} + V^{\beta H}_{\my}\Par{\hmy}$ \;
        $x' \gets \tx\Par{\my', g\x - v}$ \;
...
\end{corollary}
\begin{proof}
The proof is the same as Lemma~\ref{lem:Ograd_alg}, but we incur error due to the approximate computations of $v$, $\hx$, and $x'$. Let $r \defeq \rsdp{\alpha, \mu}$ for convenience. Let $x_\star$ minimize the subproblem defining $\hx$, and $x_\star'$ minimize the subproblem defining $x'$.
...
\begin{align*}\inprod{g\y + \nabla_y r\Par{x_\star, \my} - \nabla_y r\Par{x, \my}}{\my' - u\y} &= \inprod{g\y + \nabla_y r\Par{\hx, \my} - \nabla_y r\Par{x, \my}}{\my' - u\y} \\
...
\begin{align*}
\inprod{g\x + \nabla_x r(x', \my') - v}{x' - u\x} &\le \Delta \\
\implies \inprod{g\x + \nabla_x r(x', \my') - \nabla_x r(x, \my)}{x' - u\x} &\le \inprod{g\x + \nabla_x r(x', \my') - v}{x' - u\x} \\
...
\begin{algorithm2e}[H]
	\caption{\textsf{ApproXGradStepOracle}($z, g, \bmy, \alpha, \beta, \mu, \tx, \tnabla_x \rsdp{\alpha,\mu}(\cdot, \my)$)}
	\label{alg:approxxgradstep}
	\DontPrintSemicolon
	{\bfseries Input:} $z = (x, \my) \in \zset$, $g = (g\x, g\y) \in \zset^*$, $\bmy \in \yset$, $\alpha, \beta, \mu \ge 0$, $\tx$, a $\Delta$-approximate best response oracle for $\rsdp{\alpha,\mu}$ and $\bmy$ or $\my^+$ (defined below), $\tnabla_x \rsdp{\alpha,\mu}(\cdot, \my)$, a $\Delta$-approximate gradient oracle for $\nabla_x \rsdp{\alpha,\mu}(\cdot, \my)$ \;
        $v \gets \tnabla_x \rsdp{\alpha,\mu}\Par{x, \my}$ \;
        $\bx \gets \tx(\bmy, g\x - v)$\;
        $\my^+ \gets \argmin_{\hmy \in \yset} \inprod{g\y + \nabla_y \rsdp{\alpha,\mu}(\bar{x}, \bmy) - \nabla_y \rsdp{\alpha,\mu}(x,\my)}{\hmy} + V^{\beta H}_{\bmy}(\hmy)$ \;
        $x^+ \gets \tx\Par{\my^+, g\x - v}$ \;
        $\bmy^+ \gets \argmin_{\hmy \in \yset} \inprod{g\y + \nabla_y \rsdp{\alpha,\mu}(x^+, \my^+) - \nabla_y \rsdp{\alpha,\mu}(x,\my)}{\hmy} + V^{\beta H}_{\bmy}(\hmy)$\;
...
\end{corollary}
\begin{proof}
The proof is the same as Lemma~\ref{lem:Oxgrad_alg}, but we incur error due to the approximate computations of $v$, $\bar{x}$, and $x^+$. Let $r \defeq \rsdp{\alpha,\mu}$ for convenience. Let $\bar{x}_\star$ minimize the subproblem defining $\bx$ and let $x^+_\star$ minimize the subproblem defining $x^+$.
...
\begin{align*}
\inprod{g\y + \nabla_y r(x^+, \my^+) - \nabla_y r(x, \my)}{\my^+ - u\y} &= \inprod{g\y + \nabla_y r(\bx, \bmy) - \nabla_y r(x, \my)}{\my^+ - \bmy^+} \\
...
We will actually prove a generalization of our claimed result Theorem~\ref{thm:boxspectraplexintro}, phrased in terms of MEQ oracles. Theorem~\ref{thm:boxspectraplexintro} then follows by combining Theorem~\ref{thm:boxspectraplex} with the oracle implementation in Proposition~\ref{prop:approxgrad}, applied with the stated required parameters in the following claim.

For problems exhibiting additional structure, e.g.\ graph-structured applications where fast linear system solvers \cite{SpielmanT14} can be used in conjunction with rational approximations to the exponential, one can improve MEQ oracle runtimes (an exposition can be found in Chapter 6, \cite{SachdevaV14}).
...
A combination of Theorem~\ref{thm:boxspectraplex} and Proposition~\ref{prop:approxgrad} then subsumes and slightly refines Theorem~\ref{thm:boxspectraplexintro}.
...
\end{equation}
to additive error $\epsilon t$, where $t \geq 0$ is a parameter, $b = \mr d$, and 
\[
\ma = \begin{pmatrix}
	\mw^{-1} \mb^\top \mr^\top \\
	- \mw^{-1} \mb^\top \mr^\top 
...
We would like thank our long-term collaborators Yujia Jin and Aaron Sidford for many helpful discussions at earlier stages of this project, which improved our understanding of area convexity. We also thank Victor Reis for his collaboration on related ideas to this work.
...
\newblock {\em {SIAM} J. Matrix Anal. Appl.}, 35(3):835--885, 2014.
...
In this section, we show that our notion of relaxed relative Lipschitzness extends area convexity and relative Lipschitzness and demonstrate that it enables convergence of an extragradient method for variational inequalities. We begin by comparing our notion with existing conditions.
...
\[
\eta \l g(z_t) , w_t - z_{t+1} \r \leq V_{z_t}^r(z_{t+1}) - V_{w_t}(z_{t+1}) - V_{z_t}(w_t)
\]
and 
\[
\frac{\eta}{2} \l g(w_t), z_{t+1} - u \r \leq V_{z_t}(u) - V_{z_{t+1}}(u) - V_{z_t}(z_{t+1}). 
\]
Doubling the second inequality and adding it to the first, we have 
\[
\eta \left( \l g(w_t), z_{t+1} - u \r + \l g(z_t) , w_t - z_{t+1} \r \right) \leq 2 V_{z_t}(u) - 2 V_{z_{t+1}}(u) - V_{z_t}^r(z_{t+1}) - V_{w_t}(z_{t+1}) - V_{z_t}(w_t).
\]
Rearranging the above yields 
\begin{align*}
\eta \l g(w_t), w_t - u \r &\leq 2 V_{z_t}(u) - 2 V_{z_{t+1}}(u) \\
&- V_{z_t}^r(z_{t+1}) - V_{w_t}(z_{t+1}) - V_{z_t}(w_t)+ \eta \l g(w_t) - g(z_t), w_t - z_{t+1} \r \\
&\leq 2 V_{z_t}(u) - 2 V_{z_{t+1}}(u) 
\end{align*}
...
\section{Proofs from \Cref{ssec:regprop}}\label{app:regularizer}
...
\end{align*}
The second-to-last equality above follows from Theorem 3.3 of \cite{LewisS01} and the fact that $\Phi(\my)$ and $\Phi(\my + t\mm)$ commute. Combining the above displays shows the desired claim.
\end{proof}
 %
