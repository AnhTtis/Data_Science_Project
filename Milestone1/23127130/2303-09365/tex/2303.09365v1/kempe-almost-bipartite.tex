...
\label{main-conj}
If $G$ is a $(k-1)$-colorable $B + E_l$ graph with $k \ge 4$ and $l <
\binom{k}{2}$, then $\Kc(G, k) = 1$.
\end{conj}

The assumptions on $l$ and the chromatic number of $G$ in Conjecture \ref{main-conj} are  necessary \cite{HM}. Unfortunately, we show that Conjecture~\ref{main-conj} is false for
all $k\ge 8$ (we suspect that it is true for $k\in\{5,6,7\}$). Let $\Ix{x}$ denote the function on positive integers $x$ that is 1 if $x$
...
class is 2-connected.  Therefore, if $J$ has a frozen coloring, then so does $J'$. 

Let $\wh{H}$ be as in Lemma~\ref{lem:h} and form graph $G_k$ from $K_{9,k-4}$
by adding the edges of $\wh{H}$ on the part of size 9.
Since $\Kc(\wh{H},4)\ge 2$, also $\Kc(G_k,k)\ge 2$, for all $k\ge 4$.
So it suffices to choose $k$ such that $G_k$ is an almost bipartite graph
...
We first show that 
if there exists a pair of colors $i,j$ such that $E_{\ell}$ does not have any
edge with endpoints colored $i$ and $j$, but color $i$ or $j$ (or both) appears
in both parts $S$ and $T$, then there exists a sequence of Kempe swaps such
that after performing them
%that $i$ and $j$ each appear on at most one part. To see this, simply exhibit
%as long as possible $i,j$-swaps to make 
colors $i$ and $j$ each
appear on at most one part.  (Specifically, we make color $i$ appear only on $S$
and color $j$ appear only on part $T$.)  To do this, whenever $i$ appears on $T$
or $j$ appears on $S$, we use an $i,j$-swap.  Because $E_{\ell}$ contains no
$i,j$-edge, this process achieves the desired
property.  

Each time that we perform the steps in the previous paragraph, the number
of colors used on at most one part increases, so this process halts after at
most $k$ times.
%
...
\frac{2k^2-2k-(k^2-2k-4+\Ix{k})}4 = \frac{k^2+4-\Ix{k}}4$.   If such a pair cannot be merged,
then colors $i$ and $j$ must be in opposite parts of $B$.
...
identify all vertices colored $i$ under $\vph$ (by the previous paragraph, all
...
\end{clm}
\begin{clmproof}
Assume instead (by symmetry), that there exist $x,y\in S$ such that $xy\notin
E_{\ell}$.  Consider arbitrary $(k-1)$-colorings $\alpha$ and $\beta$ of $G$,
each avoiding color $k$. 
...
In particular, $\alpha'$ and $\beta'$ are $k$-equivalent.  This implies that
$\alpha$ and $\beta$ are $k$-equivalent in $G$.  Since every $k$-coloring of $G$
is $k$-equivalent to a $(k-1)$-coloring of $G$ by Claim \ref{claim:1}, it follows  
that $\Kc(G,k)=1$. This contradiction proves the claim.  
\end{clmproof}

\medskip

Since $G[S]$ and $G[T]$ are both complete, we have $\ell =
\binom{|S|}2+\binom{|T|}2$.  By the convexity of $\binom{x}2$, we have
$\ell\ge\binom{\ceil{(|S|+|T|)/2}}2+\binom{\floor{(|S|+|T|)/2}}2$.
By hypothesis, we have $\ell<\binom{\ceil{k/2}}2+\binom{\floor{k/2}}2$.
...
$\overline{G}$ is a complete graph, which includes the base case $|V(G)|\le 2$.
...
If $G$ has a dominating vertex $v$, then $\chi(G,\chi(G)+1)=1$ if and only if
$\chi(G-v,\chi(G))=\chi(G-v,\chi(G-v)+1)=1$, so we are again done by the
induction hypothesis.  However, we prove something stronger: If there exist
...
induction hypothesis.  In the complement, $\overline{G}$, vertex $v$ is a
...
%Add these.]}  
(ii) Fix an arbitrary 4-coloring $\beta$.  Starting from a 5-coloring $\alpha$,
...
$k\in\{2,3\}$.  (iii) Each 5-coloring has either (a) four color classes of size
...
the possibilities in (a).  So it suffices to note that to permute colors $i$
and $j$ in a 4-coloring of $\overline{C_8}$, we simply use an $(i,j)$-swap.
...
If any vertex $v$ has all neighbors in $\overline{G}$ in $S$, then $v$ is
...
that uses no common color on endpoints of any edge in $S$.  However, two of its
color classes induce a 4-cycle in $\overline{G}$.  So after one Kempe swap, we
reach a coloring with a common color on endpoints of some edge in $S$, and we
are done.  
...
If $G$ is a $B + E_l$ graph with $k \ge 4$ and $l <
\frac{k^2+8k-41+\Ix{{k-1}}}4$, then $\Kc(G, k) = 1$.
\end{conj}
