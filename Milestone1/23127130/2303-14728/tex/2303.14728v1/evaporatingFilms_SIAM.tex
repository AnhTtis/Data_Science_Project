...
% \newcommand{\blue}[1]{\ifmmode\mathbf{\textcolor{blue}{#1}}\else \textbf{\textcolor{blue}{#1}}\fi}
\newcommand{\blue}[1]{\ifmmode\mathbf{\textcolor{blue}{#1}}\else \textbf{\textcolor{blue}{#1}}\fi}
...
% shifted pressure \pps = \bar{p} + \pstar
%% seems like these end up as roman or mathrm...
% \def\pps{\red{P}}
\def\pps{\mathbf{P}}
\def\X{\mathbf{X}}
% \def\X{\red{X}}
...
% \title{Coarsening dynamics of volatile thin films}
...
that
occur due to interaction of the droplets. The edges of adjacent drops can {collide} leading to them merging together or large drops can grow by drawing fluid from smaller ones, causing the latter to {collapse}. Both of these mechanisms lead to a monotone decrease in the number of drops present as a function of time, $N(t)$; this is often called \emph{coarsening}. Heuristic scaling arguments were used to obtain power-law behaviors for $N$, $N(t)=O(t^{-2/5})$ for the one-dimensional problem \cite{glasner2003coarsening,glasner2005collision,glasner2009ostwald}. Further rigorous analysis has established upper bounds on the coarsening rate  \cite{otto2006coarsening,dai2010mean,dai2011ostwald}. 
...
the disjoining pressure can compete with phase change effects yielding complex dynamics. In this range, the problem is bistable with two flat-film steady states $\Hm$ and $\Hc$  \cite{ji2018instability}, see Fig~\ref{fig1}.  For $\eps\to 0$, the smaller flat-film steady state $\Hm$ defines a stable minimum-thickness adsorbed layer,
\begin{equation}
\Hm = \eps + \pstar\eps^2+4\pstar^2\eps^3 + O(\eps^4),
\end{equation}
and the larger flat-film steady state $\Hc$ is given by
$\Hc = \pstar^{-1/3}\eps^{2/3}+O(\eps)$. Setting $p=0$ in \eqref{defp}, these states are fixed points in the phase plane describing a family of spatially periodic steady solutions (about the $\Hc$ center point) with the largest amplitude solution being a homoclinic orbit through the hyperbolic saddle $\Hm$; this solution uniquely defines a critical steady droplet profile. It is unstable and perturbations can drive it towards either condensing or evaporating dynamics. 
\par
...
\par
In section~2 we extend the approach from \cite{glasner2003coarsening} to reduce the governing PDE to a dynamical system for the evolution of interacting quasi-steady droplets. In sections 3 and 4, the behaviors for this system for the low-dimensional cases of a single drop or a pair of drops in periodic domains will be examined in detail. This background will then allow us to understand the problem of dynamics for larger sets of droplets (section~5) and ultimately the statistical scalings that describe the stages of coarsening that can occur (section~6).



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Derivation of the droplet dynamics model}
We begin by reducing the PDE \eqref{Mainpde} for general solutions to a lower-dimensional dynamical system describing interacting fluid droplets that form after initial transients \cite{glasner2003coarsening,glasner2005collision,kit2011,witelski2020}. We focus on the limit $\beta\to 0$, where the mass evolves slowly, with rate $O(\beta)$. 
For $\beta\to 0$, the influence of fluid volatility has a weak influence on the dissipation of energy and in this limit a larger family of quasi-steady drops exists, for any spatially uniform pressure in $0<\pps<\pmax$. 
\par
...
$$
and mass fluxes, $J\equiv -h^3 \partial_x p$, are imposed at the boundaries,
$$
...
x}\right).
\end{equation}
and the adjoint operator of this operator 
is $\mathcal{L}_0^{\dag} \Phi\equiv \mathcal{S}\mathcal{R}_0\Phi$.


To address the influence of condensation effects on the droplet dynamics, we select the timescale $\sigma\sim\beta\ll 1$ so that the non-conservative term is included in the leading order.
We also define a reduced evaporation rate and corresponding scaling for the spatial fluxes as
    $\beta= \ba \eps^3$ and $\sigma=\eps^3$ respectively.
Then the leading-order equation \eqref{Eq:leading-order} reduces
to the $O(\beta)$ equation
\begin{equation}
...
derived in \cite{glasner2003coarsening}. The same form of the quasi-steady droplets is being used, however we will see that the non-conservative effects will dramatically change the thin films and fluxes between drops.
...
in the core region of the droplet,  $\Hd\gg\eps$, and the disjoining pressure $\Pi(\Hd)$ is negligible. Therefore, using the balance of the surface tension $\Hd_{xx}$ and the pressure $\pps$, we  approximate the core of the droplet by a parabola \cite{bertozzi2001dewetting,glasner2003coarsening}, 
\begin{equation}
    \Hd(x;\pps)\approx \frac{1}{2}\pps(w^2-x^2)\quad \mbox{for } |x|\ll |w|,
\lbl{eq:drop_parabolic}
\end{equation}
and $\Hd\approx \Hm$ elsewhere, with $w$ being the half-width or radius of the droplet. Fig.~\ref{fig:schematic} (right) shows two typical quasi-static droplets approximated by parabolas for their core regions.

From the first integral of \eqref{eq:quasi-static} and the estimate that the inter-droplet film $\Hd\approx \Hm$, the maximum height of the droplet $h_{\max}$ satisfies $U(h_{\max}) - U(\Hm) = \pps (h_{\max}-\Hm)$. In the limit $\eps\to 0$, we get the leading-order estimate $h_{\max}\sim -U(\Hm)/\pps$. In this limit, the half-width of the droplet is given by \cite{glasner2003coarsening}
\begin{equation}
w(\pps)\sim A/\pps\qquad \mbox{with}\quad A = \sqrt{2|U(\Hm)|},
\lbl{eq:drop_width}
\end{equation}
and for \eqref{pi} this yields $A=1/\sqrt{3}$.
The mass of the droplet then can be approximated by the mass of the core region as
\begin{equation}
{M}(\pps) = \int_{-w}^{w} \Hd(x;\pps)~dx \sim \frac{2A^3}{3\pps^2}.
\lbl{eq:drop_mass}
\end{equation}
...
Then, we obtain the estimate for the non-conservative flux based on the approximation of the core of the droplet \eqref{eq:drop_parabolic},
\begin{equation}
    \Jnc 
    \approx\int_{-w}^{w}
...
\end{equation}
Equation \eqref{eq:non-cons_flux} gives an estimate of $\Jncl$ \eqref{Jnc29} on the domain restricted to the core of the droplet. We will account for the non-conservative flux on the surrounding thin films separately.



\par 
We can evaluate this integral to get the estimate
\begin{equation}
\Jnc(\pps)\approx {4\over A}(\pps-\pstar)\bb(\pps)
{\mbox{arctanh}[\bb(\pps)]},
\qquad \bb(\pps) =\left[1+{2K\pps\over A^2} \right]^{-1/2}.
\lbl{eq:non_cons_flux}
\end{equation}
These forms make clear that the sign of the non-conservative flux $\Jnc$ is determined by the relation between $\pps$ and $\pstar$. For an isolated quasi-static droplet with pressure $\pps > \pstar$, 
...
\begin{figure}
    ...
    \caption{(Left) A schematic figure of a system of three quasi-static droplets placed in a periodic domain $0 \le x \le 150$. (right) Two quasi-static droplets (solid curve) satisfying \eqref{eq:quasi-static} 
    with the pressures $\pps=\pps_{1,2}$, 
    where the core of droplets are approximated by parabola (dot-dashed curves) defined by \eqref{eq:drop_parabolic}. The shifted pressure $\pps=p(x)+\pstar$ (dashed curve) for the precursor layer is given by \eqref{eq:pressure_precursor} over $x_L\le x\le x_R$ where the fluxes at the edges of droplets are $J^{R,L}$. 
    }
    \lbl{fig:schematic}
\end{figure}
\par 
To model the fluxes between neighboring droplets, we focus on the thin film that carries the flux between the drops.
...
$$ {(h^3 p_x)}_x - {\beta p\over h+\K}=0,$$
can be approximated by
\begin{equation}
 \Hm^3 p_{xx} - {\beta\over \Hm+\K}\, p=0\qquad \mbox{on $x_L\le x\le x_R$},
 \end{equation} 
...
\end{equation}
where $D=x_R-x_L$ is the distance between contact lines of adjacent droplets, see Fig.~\ref{fig:schematic}.
\par
...
to small droplets effectively vanishing into the $\Hm$ adsorbed film.
 It was shown that this collapse happens in finite time with $\pps= O((T_c-t)^{-1/3})$ \cite{glasner2003coarsening}. 
     In practice, we define a droplet collapse event to occur the droplet pressure reaches a fixed critical pressure
    \begin{equation}
        \pps_k(t)=\pmaxh \qquad \mbox{with $\pmaxh:= (1-\eta)\peps$},
    ...
    \item[(b)] \emph{Droplet collision}: 
        When the separation distance between the edges of adjacent drops vanishes, after a rapid transition, the two droplets will merge together to produce a single combined drop. 
    This has previously been called droplet collision and occurs when the flux \eqref{JRkkp} between drops diverges with $D_{k,k+1}\searrow 0$.
   \par
...
For all the numerical studies in this paper, we set $\eps=0.1, K = 0.1, \pstar = 0.5$ unless otherwise specified. 
...
\begin{figure}
    \centering
    \includegraphics[height=5cm]{fig/fig3a.pdf}
    \hspace{0.2in}
    \includegraphics[height=5cm]{fig/fig3b.pdf}
    \caption{(Left) PDE simulation of \eqref{Mainpde} for a slowly condensing droplet 
    in a periodic domain $0\le x \le L$; (Top right) the evolution of the droplet pressure in the PDE model (dots) compared against predictions from the dynamical model \eqref{Dpdt_singleDropCondense} (solid curve). 
    The 
    last stage of the droplet growth is given by \eqref{eq:condensationLimit} (dot-dashed curve) as $\pps \to \pinf$, followed by film-wise condensation after the domain is filled at $t=t_f$. (Bottom right) The corresponding evolution of the mass $\M(t)$ with a transition from the scaling $\M = O(t)$ in the dropwise condensation stage to the scailng $\M = O(\sqrt{t})$ in the filmwise condensation stage. 
    The domain size is $L=100$ and $\beta = 10^{-7}$.}
       \label{fig:SingleDropCondense}
\end{figure}
First, we consider a single droplet $(N=1)$ with the position $\X$ and pressure $\pps$ on a periodic domain $0 \le x \le L$. In this case, the left and right neighbors of the droplet are identical, and (\ref{JRkkp}, \ref{JLkmk}) reduce to the leading-order symmetric inter-drop fluxes
\begin{equation}
...
\eqref{eq:singleDrop_flux}, one arrives at an ODE for the 
the pressure of the droplet in time
\begin{equation}
...
As $t\to T_c$, the droplet pressure blows up, $\pps \to \infty$, and the droplet collapses due to evaporation, \eqref{eq:collapseCondition}.

\par

For the case $\pps < \pstar$,  equation \eqref{Dpdt_singleDropCondense} describes the evolution of a droplet subject to condensation.
Figure \ref{fig:SingleDropCondense} (left) presents a typical simulation of the PDE \eqref{Mainpde} with $\beta=10^{-7}$ for a slowly condensing droplet placed at the center of a periodic domain $0 \le x \le 100$. The initial condition is the quasi-static droplet profile $h(x,0)=\Hd(x-\X(0);\pps(0))$ with
$\pps(0)=0.1 < \pstar$ and $\X(0)=50$ at time $t = 0$. 
Figure \ref{fig:SingleDropCondense} (top right) shows that the evolution of the pressure $\pps$ obtained from the PDE simulation (marked by dots) agrees well with the prediction from the dynamical model \eqref{Dpdt_singleDropCondense} (solid curve). This comparison shows that the leading-order linear approximation for the inter-drop fluxes \eqref{eq:singleDrop_flux} provides a good prediction for the long-time droplet dynamics under weak condensation. 


Both righthand side terms in \eqref{Dpdt_singleDropCondense} give contributions to the evolution of the droplet pressure due to condensation. The first term is condensation on the film between drops while the second describes condensation in the drop core region. This evolution goes through several stages.
The rate of condensation is initially dominated by the contributions from the long films between drops, $D\gg 1$. Later, the rate slows as the drops grow and the inter-drop films shrink, $D\searrow \Dmin$.
When both terms contribute at leading order, the evolution can be approximated by
\begin{equation}
    {d\pps\over dt} \propto - {3\beta\pstar\over 4A^3} \pps^3 \qquad \implies \qquad 
    \pps=O(t^{-1/2}) .
    \lbl{new36}
\end{equation}
In the final stage, the condensing drop fills almost the entire domain, with $D\to 0$ and $P\searrow P_L$. 
In this limit,  \eqref{Dpdt_singleDropCondense} reduces to the leading-order linear problem for $\pps$ 
\begin{equation}
...
The dot-dashed curve in Fig.~\ref{fig:SingleDropCondense} confirms the exponential decay of $\pps$ as the contact lines of the condensing drop approach the boundary.
At a critical time $t = t_f \approx 2.5\times 10^8$, the entire domain is filled with liquid, and the dynamical model \eqref{Dpdt_singleDropCondense} no longer applies as the solution approaches a spatially-uniform condensing film \cite{ji2018instability}. 
Figure \ref{fig:SingleDropCondense} (bottom right) plots the total mass $\M(t)$. Using \eqref{new36} and \eqref{eq:drop_mass} we can estimate the growth of the droplet mass as $M=O(t)$. After $t_f$, the droplet fills up the entire domain, and 
the scaling law $\M(t) = O(\sqrt{t})$  for the filmwise condensation stage applies  \eqref{flood}. 
...
\end{equation}
This analysis shows that if $\pps<\pstar$, then the magnitude of $\Delta D$ decreases in time, making the droplets asymptotically approach equal spacing for long times. However, it can be shown that the $dD/dt$ evolution equations allow for merging of droplets at finite times, $D\to 0$ as in the single drop case from section~\ref{sec:singleDrop}. We note that the differences from the mass-preserving linear instability of periodic solutions described by \cite{LP} stems from the difference in the form of the non-conservative flux term in \eqref{Mainpde}. Here periodic droplet arrays are unstable to filmwise condensation.
\par

The PDE simulation in Fig.~\ref{fig:2Drop_Dx} (left) shows an example of the dynamics of two equally-sized droplets with unequal spacing. As the droplets condense,  they drift away from each other, become more equally spaced, approaching $\X_1\to L/4$ and $\X_2\to 3L/4$, and eventually merge into a single large droplet via droplet collision. The plot in Fig.~\ref{fig:2Drop_Dx} (right) traces the separation of the centers of the droplets at $\X_1$, $\X_2$, as well as the contact line motions of the droplets $\X_{1,2} \pm w$ in time. The results from the PDE simulation (dots for the peak positions and solid curves for the contact line positions) and from the reduced ODE system \eqref{eq:equalPressureSystem} (dashed curves) show good agreement.
...
\begin{enumerate}
    \item[(A)] \emph{Pairwise growing}: where both droplets grow in time.
    \item[(B)] \emph{Growing -- shrinking}: where one droplet shrinks and collapses in finite time, and the other droplet grows.
    \item[(C)]\emph{Pairwise shrinking}: where both droplets shrink.
\end{enumerate}
For cases (A) and (C), non-conservative effects (condensation or evaporation respectively) must play a role since the overall mass is changing. For case (B), it may not be clear if the dynamics are dominated by conservative or non-conservative fluxes.
...
In this form, the system has a hyperbolic saddle point at $\pps_2=\pps_1=\pinf$. The stable manifold of this point is the line $\pps_2=\pps_1$, and the unstable manifold is the hyperbola given by $D(\pps_1,\pps_2)=0$.
...
The dynamic regions are also sensitive to the domain size $L$. Fig.~\ref{fig:twoDropTrajectories}c shows a comparison of region (A) (pairwise growth) for a varying domain size $L = 100,200,300$ (with other parameters fixed).
...
It is also worth noting that the monotonicity of the mass difference does not imply that the pressure difference between droplet pairs also monotone. 
...
As the droplets grow in time, spatial drifts also occur, hence the drops are no longer equally spaced with a constant peak-to-peak spacing. However, since the droplets are still widely separated, we approximate this many-droplet system by $6$ pairwise 
...
\begin{figure}
    ...
    (Left) The coarsening rate shows a transition from $N = O(t^{-2/5})$ to $N = O(t^{-1/2})$, followed by logarithmic behaviors governed by \eqref{eq:N_log} as $N\searrow  1$. (Right) The total mass of droplets in the system $\mathcal{M}(t)$ increases in time shows a transition from $\mathcal{M}= O(\sqrt{t})$ to a logarithmic law \eqref{eq:M_log}. The dashed curve corresponds to filmwise condensation with $\mathcal{M}= O(\sqrt{t})$. 
...
peak-to-peak distances $\lambda_k=X_{k+1}-X_k$ and contact-line distances $D_k$, from the simulation in Fig.~\ref{fig:N_M_t}. (Right) Arithmetic average $\bra D_A\ket$ and harmonic average $\bra D_H\ket$ of contact-line distances $D$ in time.}
...
We have shown in section~\ref{sec:twoDropEqualSpace} the droplet dynamics strongly depend on the peak-to-peak spacing $\lambda$, with pairwise droplet condensation (previously called case (A)) becoming the expected mode of dynamics when $\lambda$ is large (see Fig.~\ref{fig:twoDropTrajectories} (c,d)).  In Fig.~\ref{fig:D_lamb_dist} (left) we have indicated a line $\lambda=\ell_c\approx 225$ that was observed to be correspond to a strong transition in the coarsening to yield only droplet collision events for all later times (with $\bra \lambda \ket$ also continuing to monotone increase).


\par
For what follows, it is useful to more carefully examine the distribution of contact-line separation distances.
Fig.~\ref{fig:D_lamb_dist} (right) displays both the arithmetic average $\bra D_A\ket = (\sum_{k=1}^N D_k)/N$ and the harmonic average $\bra D_H\ket = N{\left(\sum_{k=1}^N D_k^{-1}\right)^{-1}}$ of the contact line distances $D_k$ in a drastic comparison during the later stage. For $t > 10^{12}$,  $\bra D_A \ket$ increases while $\bra D_H\ket$ decreases, which is consistent with the distribution plots of $D_k$ in Fig.~\ref{fig:D_lamb_dist} (left), revealing the coexistence of a small number of extremely large $D_k$ values with the majority of small distances.
...
\underline{\emph{Stage I: Quasi-conservative dynamics.}}
 In the early stage for $t \ll O(\beta^{-1})$, the system behaves like the mass-conserving coarsening case discussed in \cite{glasner2003coarsening,glasner2005collision,glasner2009ostwald}, as condensation has not yet had enough time to contribute at leading order. 
...
which leads to the heuristic scaling law
$N=O(\left<\pps\right>^2)$ and $N=O(t^{-2/5})$ \cite{glasner2003coarsening}.



\underline{\emph{Stage II: Drop-wise-condensing dynamics.}}
For $t\gg O(\beta^{-1})$, the remaining droplets are significantly larger in size with large peak-to-peak spacing $\lambda$ due to the early-stage coarsening (see Fig.~\ref{fig:D_lamb_dist} (left)). We have shown in Fig.~\ref{fig:twoDropTrajectories} that pairwise growth dynamics (region A) are the predominant events in this case, leading to more droplet collisions. Unlike the mass-conserving coarsening which preserves the total mass of droplets, the non-conservative coarsening modified by condensation is constrained by the finite domain size. When an array of large droplets with average half-width $\left<w(\pps)\right>$ are placed in the domain of size $L$, the number of droplets $N$ satisfies $2N\bra w(\pps)\ket  + N\bra D\ket =L$. Therefore, $N$ satisfies the constraint $N<L/(2\left<w\right>)$.
...
\begin{figure}
    ...
    \caption{(Left) Coarsening modified by weak condensation with $\beta=10^{-7}, 10^{-8}, 10^{-9}$ compared against the mass-conserving case with $\beta=0$, all starting from an identical initial droplet configuration with $N=10^4$ droplets. (Right) The average droplet pressure $\left<\pps\right>$ shows a scaling transition from $N = O( \left<\pps\right>^2)$ to $N = O( \left<\pps\right>)$ in time as the number of droplets $N$ decreases (indicated by arrows). 
    }
...
Figure \ref{fig:N_P_relation} compares the mass-conserving coarsening dynamics ($\beta=0$) with the coarsening dynamics modified by weak condensation effects ($\beta=10^{-9}, 10^{-8},10^{-7}$), all starting from an identical initial droplet configuration with $N=10^4$ droplets.  With the presence of weak condensation effects, the augmented dynamical system exhibits
...
To heuristically obtain a scaling law for the long-time behavior of an array of $N$ weakly condensing droplets in the spirit of mean-field models, we consider the dynamics of a single typical droplet with index $k$ at pressure $\pps_k$ in the array. We assume that this droplet has both of its neighboring droplets at the average pressure $\pavg$ but retain the contact line distances $D_{k+1,k}$ and $D_{k, k-1}$ satisfying $(D_{k+1,k}^{-1}+D_{k,k-1}^{-1})/2 = \bra D_H\ket^{-1}$ and $(D_{k+1,k}+D_{k,k-1})/2 = \bra D_A\ket$. 
...
\end{equation}
Our numerical evidence in Fig.~\ref{fig:D_lamb_dist}~(right) suggests that in the later stage, the harmonic average satisfies $\bra D_H\ket = O(1)$ and the arithmetic average satisfies $\bra D_A\ket \gg \bra D_H\ket$. Therefore, for large $t$, the leading-order equation of \eqref{eq:meanfield_reduced_0} becomes
\begin{equation}
...
\underline{\emph{Stage III: filmwise condensation dynamics.}} After the final droplet fills up the entire domain, the system undergoes filmwise condensation following the scaling law $\M(t)\sim L\sqrt{2\beta\pstar t}$ \eqref{flood} (see the dashed curve in Fig.~\ref{fig:N_M_t} (right)). This is similar to the filmwise condensation observed in Fig.~\ref{fig:SingleDropCondense}.
...
\par
Heuristic scaling laws were presented for the coarsening dynamics with weak condensation. Rigorous justification on the arguments sketched out here are needed. For example, more thorough analysis should be carried out for the transition from the stage (I) to stage (II) in the coarsening dynamics.
For future work, we are interested in exploring the coarsening of thin films with strong condensation effects (with $\beta$ large). In this case, the thin film between drops has a pressure profile $p(x)$ with boundary layers, which would give rise to a distinct dynamical system for droplets. Additionally, it would also be interesting to investigate the coarsening dynamics affected by the interplay between condensation, Marangoni effects, and gravity \cite{gratton2008coarsening} and  extend our findings from this study to a two-dimensional spatial domain is expected to yield more intricate coarsening dynamics. Finally, thin films with weak condensation may the simplest dynamic regimes for volatile films -- evaporation-dominated dynamics may require different analytical descriptions.



\section{Acknowledgments}

TW acknowledges support from grant NSF DMS 2008255. HJ acknowledges support from NCSU Faculty Research and Professional Development Program.
