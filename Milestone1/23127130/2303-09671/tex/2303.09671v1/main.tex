...
\subsection{Determining the source terms $S_s$ and $S_l$}
Typically the source terms should be determined by performing an asymptotic analysis of the model. However, thin-interface asymptotic analysis (i.e. up to second order) of the model remains a mathematically challenging task. To make the problem mathematically amenable and for the purpose of estimating the choice of $S_s$ and $S_l$, we {\sl temporarily} assume pointwise equality of the diffusion potential i.e. $\mu_s = \mu_l$ in Eqs.~(\ref{model18}), (\ref{eq18.1}), (\ref{eq18.2}) and (\ref{model19}). This allows for substantial progress in the thin-interface asymptotics, and a choice of source term can be deduced. The details of the asymptotic analyses (up to first order for the model $\mu_s \neq \mu_l$ and up to second order for the model $\mu_s = \mu_l$) are presented in the supplementary materials along with numerical comparisons of the solution obtained for both the cases (to ascertain the range of the validity of the assumption). We emphasize that $\mu_s = \mu_l$ is {\sl not} a constraint when solving the dynamic equations. The source term in the phase field model provides an additional degree of freedom to recover a desired sharp-interface relation. From the analysis it is evident that the choice of source terms $S_s = S_l = -A(c_s - c_l)$ is sufficient to impart a degree of control over solute trapping in the phase field model. 
 
 With the above choice, the diffusion potential jump up to first order in asymptotics can be written as,
 \begin{equation}\label{jump}
     \mu^- - \mu^+ = \frac{WV}{D_l\partial c_{l,0}/\partial\mu_{l,0}}(c_{l,0}-c_{s,0})(F^- - F^+) + Wq_n^-\frac{\partial\mu_s}{\partial r}\Big|^-(G^- - G^+),
 \end{equation}
 where,
 ...
 \begin{equation}\label{integral2}
     \tilde{p}(\phi_0) = \frac{g(\phi_0)-1-(1-A)a(\phi_0)}{q_n(\phi_0)}
 \end{equation}
 and
 \begin{equation}\label{integral3}
     G^{\pm} = \int_0^{\pm\infty}\Bigg[ \frac{1}{q_n(\phi)} - \frac{1}{q_n^{\pm}}\Bigg]\mathrm{d}\eta.
 \end{equation}
