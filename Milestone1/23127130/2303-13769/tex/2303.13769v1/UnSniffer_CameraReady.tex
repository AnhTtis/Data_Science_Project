...
\begin{document}
	...
	\begin{abstract}
		The recently proposed open-world object and open-set detection achieve a breakthrough in finding never-seen-before objects and distinguishing them from class-known ones.
		However, their studies on knowledge transfer from known classes to unknown ones need to be deeper,
		leading to the scanty capability for detecting unknowns hidden in the background.
		...
		which only uses class-known samples for supervision and avoids improper suppression of unknowns in the background.
		Significantly, such confidence score learned from class-known objects can be generalized to unknown ones.
		Additionally, we propose a negative energy suppression loss to further limit the non-object samples in the background.
		...
		the first publicly benchmark that encompasses precision evaluation for unknown object detection to our knowledge.
	...
	Detecting objects with a limited number of classes in the closed-world setting \cite{ren2015faster,redmon2016you,liu2016ssd,he2017mask,lin2017feature,lin2017focal,redmon2017yolo9000,cai2018cascade,carion2020end,zhu2020deformable} has been the mainstream over the years.
	Recently,
	the popularity of autonomous driving \cite{pinggera2016lost,ramos2017detecting,dosovitskiy2017carla,8126154,9210191,8794279,janai2020computer,YuZhou-IJCV2016-SFVT,NIPS2012_3e313b9b,ZheLiu-AAAI2019-TANet,2014ONLINE} has put forward higher requirements for object detection.
	That is,
	the detector is not only required to find the objects of the pre-defined categories, called `\textbf{known objects}',
	but also needs to capture the objects that the detector has never seen during training, namely, `\textbf{unknown objects}',
	so that it can cope with more challenging environments,
	such as detecting hazards in autonomous driving scenes.
	...
	Since unknown objects are not labeled in the training set,
	...
	Several OSD works have proposed uncertainty measures to distinguish unknown objects from known ones.
	However, they primarily focus on improving the discriminative power of uncertainty and tend to suppress non-objects along with many potential unknowns in the training phase.
	As a result, these methods can only capture a small number of unknown objects.
	...
	which describes the unfamiliar things by the familiar ones.
	We argue that \textit{despite being ever-changing in appearance,
	the unknown objects are often visually similar to the objects with pre-defined classes},
	as observed in Fig. \ref{fig:first_page} (d).
	The t-SNE visualization shows that the unknown objects tend to be between several pre-defined classes,
	while the non-objects are far away from them.
	Those inspire us to express a unified concept of `object' by the proposed generalized object confidence (GOC) score learned from the known objects only.
	To this end,
	we first discard the background bounding boxes and only collect the object-intersected boxes for training to prevent potential unknown objects from being supervised as backgrounds.
	...
	Different from `objectness',
	non-object boxes are not used as the negative samples for supervision.
	Fig. \ref{fig:first_page} (e) shows that the GOC score distinctly separates non-object and `object'.
	In addition,
	we design a negative energy suppression loss on the top of VOS's energy calculation \cite{vos} to further widen the gap between the non-object and the `object'.
	Fig. \ref{fig:first_page} (f) shows three distinct peaks for the known, unknown and non-object.
	Next,
	due to the absence of the unknown's semantic information in training,
	the detector hardly determines the best bounding box by a constant threshold when the number of objects cannot be prophesied ahead of time.
	In our model,
	...
	As far as we know,
	the existing methods are evaluated on the COCO \cite{lin2014microsoft} and Pascal VOC benchmark \cite{voc} that do not thoroughly label unknown objects.
	Therefore, the precision of unknown object detection cannot be evaluated.
	...
	(2) COCO-Mix containing both unknown and known objects.
	They are collected from the original COCO dataset \cite{lin2014microsoft} and annotated with the COCO's instance labeling standard.
	In addition,
	...
	\begin{itemize}
		...
		\item The graph-based box determination is designed to adaptively select the best bounding box for each object in inference for higher unknown detection precision.
	...
	\begin{figure*}
		...
		\vspace{-16pt}
		\caption{
			\textbf{The framework of UnSniffer}
	...
	And the $N$ input RGB images are denoted as $\{\mathbf{I}_1,...,\mathbf{I}_N\}$,
	with corresponding labels $\{\mathbf{Y}_1,...,\mathbf{Y}_N\}$.
	Each one $\mathbf{Y}_i=\{\mathbf{y}_1,...,\mathbf{y}_K\}$ contains a set of object instances with $\mathbf{y}_k=[l_k,x_k,y_k,w_k,h_k]$,
	...
	we use two detectors for known and unknown objects, respectively. 
	...
	We select the one with the highest GOC score in each group to form a set of unknown predictions.
	...
	and remove the low-negative-energy proposals in inference.
	Unlike \cite{vos},
	...
	It pulls down the feature response inside non-object boxes and boosts the discriminative power of both detectors.
	...
	We directly concatenate the two results and remove the unknown-class predictions whose IoU with any known-class predictions exceeds a constant threshold $\beta$.
	...
	\subsection{Generalized Object Confidence Score}
	\label{sec:MyScore}
	
	\noindent\textbf{Uncertainty Score} is usually modeled as either the maximum known-class conﬁdence score \cite{hendrycks2016baseline,liang2017enhancing} or the entropy of the classification result \cite{liu2020energy, vos, du2022unknown}.
	It can be used to distinguish unknowns from known objects according to the high uncertainty score of the unknown objects.
	However,
	...
	\noindent\textbf{Objectness Score} is usually used to judge whether a bounding box containing an object \cite{OLP,edgeboxes,ren2015faster},
	...
	However,
	learning-based object proposal methods cannot avoid the misuse of unknown samples as negative samples,
	...
	It can judge whether a proposal contains an object (including unknown and known classes),
	...
	And we define the intersection over the predicted bounding box ($IoP$) and intersection over the correct bounding box ($IoC$) for collecting training samples from $B_n$ as follows:
	\vspace{-6pt}
	\begin{equation}
	...
	we find the object instance with the maximum IoU with $b_i$.
	...
	as shown in Fig. \ref{fig:box}.
	In order to prevent the potential unknown objects from being taken as background during training,
	we only use the first three groups in Eq. \ref{eq:S_plsn} to train the module $\Phi$ by three losses.
	In the first loss,
	...
	\begin{figure*}
		...
		\caption{The illustration and more examples for the graph-based top-scoring box determination.
			The white rectangles denotes the proposals with top GOC scores in the image.
	...
	\end{figure*}
		...
		\begin{equation}
			...
			\label{eq:negloss}
	\end{equation}
where $B^{k,\mathbf{po}}_{n}=B^{k,\mathbf{p}}_{n}\cup B^{k,\mathbf{o}}_{n}$.
	Next,
	...
	\end{equation}
where $\alpha = 1$ when $IoU(b_j, \mathbf{y}_k) > IoU(b_i, \mathbf{y}_k)$,
	otherwise $\alpha = -1$.
	...
	while others are unknown predictions.
	...
	Specifically,
	we observe that most non-object boxes have lower negative energy scores than the boxes of the known and unknown classes,
	...
	using the Pascal VOC dataset \cite{voc} as the training data that contains annotations of 20 object categories.
	For testing,
	since the MS-COCO dataset \cite{lin2014microsoft} extends the PASCAL VOC categories to 80 object categories,
	we naturally employ MS-COCO for evaluating unknown objects.
	However, MS-COCO does not thoroughly label potential unknown objects in images.
	To address this issue,
	we propose two datasets, i.e., COCO-OOD and COCO-Mixed, which fully labels the unknown objects.
	Firstly,
	...
	\noindent\textbf{COCO-OOD} dataset only contains unknown categories,
	...
	\noindent\textbf{COCO-Mixed} dataset contains both the known and unknown categories,
	consisting of 897 images with ﬁne-grained annotations of 2533 unknown objects and 2658 known objects.
	The original annotations in COCO that overlap with Pascal VOC object categories are employed as the label of known objects.
	And we add annotations for unambiguous unlabelled objects simultaneously.
	Since each image in COCO-Mixed contains more object instances with complex categories and concentrated locations,
	it is more challenging to evaluate the model's reliability in detecting both unknown and known objects. 
	...
	\begin{table}[t!]
		...
		\end{tabular}
		\caption{\textbf{The Statistics of datasets} including the number of images, the average number of known and unknown instances per image.
	...
	\begin{itemize}
		...
		\item The Recall Rate (U-REC) and Precision Rate of Unknown (U-PRE) are defined: $U\mbox{-}REC = \frac{TP_u}{TP_u+FN_u}, U\mbox{-}PRE = \frac{TP_u}{TP_u+FP_u}$.
	...
	Noted that we measure mAP over different IoU thresholds from 0.5 to 0.95.
	...
	\begin{table*}
		...
		\resizebox{\linewidth}{!}{
				\begin{tabular}{c|l|c|cccc|ccccccc}
					...
					\hline
					\multirow{2}{1cm}{\centering\ding{175}} &VOS \tnote{1} \cite{vos} & \textbf{0.485} & 0.135 &0.196 & \underline{0.342} & 0.137 & \underline{0.377} & 0.040 &0.101 & \textbf{0.262} & 0.062 & 640 & 0.152\\
					&VOS \tnote{2} \cite{vos} & 0.469 & 0.205 &\underline{0.317} & 0.291 & 0.348 & 0.364 & 0.051 & 0.172 & 0.184 & 0.163 & \underline{409} &0.124 \\
					\hline
	...
	Both VOS \cite{vos} and OWOD \cite{owod} determine threshold before inference,
	...
	which selects of 200 images in the training set that do not contain any potential unknown objects for threshold determination.
	...
	\begin{table}
		...
		\caption{
			...
			If `GBD' is $\times$, we use NMS+top-$k$ with threshold of the known detector.}
		\vspace{-1em}
		\label{tab:Ablation}
\end{table}
	...
	we show UnSniffer's result on the UOD-Benchmark,
	...
	which owes to our GOC learning the overall confidence of objects from finite known objects.
	...
	\begin{figure*}[t!]
		...
		\caption{\textbf{Example results on COCO-Mix (first two rows) and COCO-OOD datasets (last three rows)}.
			...
			we only draw the top-$10$ boxes for each image.
			And other methods draw all predicted boxes.}
	...
	To investigate the contribution of each component in UnSniffer,
	...
	{\small
		...
		\bibliography{UnSniffer_CameraReady}
	}
