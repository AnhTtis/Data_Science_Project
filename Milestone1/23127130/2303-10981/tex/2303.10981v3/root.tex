...
\begin{equation}
 \sup_{u \in \mathcal{U}} \dot{h}(x,u)=\sup_{u \in \mathcal{U}} [ L_{f}h(x)+L_g h(x)u ] \geq -\alpha(h(x))   
\end{equation}
...
\        \end{bmatrix}\nu\\
    ...
    \end{cases}
\end{equation}
With a slight abuse of notation we denote $D$ in (\ref{eq:mech_ph_passive}) the dissipation matrix that possibly includes a damping injection component, and consistently with the notation in Theorem \ref{th:main} we indicate $\bar{d}_p(q,p)=\frac{\partial h}{\partial p}^{\top}D\frac{\partial S_{cl}}{\partial p}$.
...
\begin{equation}
    \Psi(q,p;\beta)=\{h,S_{cl}\} + \bar{d}_p(q,p)+ \alpha(h(q,p))
\end{equation}
