...
In the case of a Rayleigh fading channel, the received signal can be modeled as $\mathbf{y}=\alpha\mathbf{c}+\mathbf{n}$, where $\alpha$ is the fading coefficient following a Rayleigh distribution as
\begin{align}
%f_A(\alpha)=\frac{2\alpha}{\Omega}{\rm e}^{-\alpha^2/\Omega},
f_A(\alpha)=\frac{2\alpha}{\Omega}{\rm exp}(-\alpha^2/\Omega),
\end{align}
where $\Omega=\E[A^2]$. 
To make sure that fading neither amplifies nor attenuates the signal power (i.e., it only models the fluctuations), we set $\Omega=\E[A^2]=1$.
Note that the amplitude $A$ of a complex number $U+iV$, whose real and imaginary parts $U$ and $V$ are Gaussian random variables with mean zero and variance $\sigma^2$, has a Rayleigh distribution with parameter $\Omega=2\sigma^2$. Therefore, we can generate Rayleigh fading samples by picking independent samples $u$ and $v$ from a Gaussian 
%random variables $U$ and $V$, each 
distribution
with mean zero and variance $1/2$, and then obtaining $\alpha=\sqrt{u^2+v^2}$.
