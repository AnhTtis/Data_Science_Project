...
\subsection{Ab initio method for quantum plamsonics}
...
TDDFT is the formal extension of the Hohenberg-Kohn-Sham density functional theory (DFT). Within the DFT, The Hohenberg-Kohn (HK) theorem~\cite{HK1964} states that the ground-state electron density unambiguously defines the many-electron ground state for an $N-$electron system under the influence of an external potential. And there exists an energy functional that guarantees the ground state energy can be reached by the variational principle though the exact functional is an unsolved problem,
\begin{equation}
...
\begin{enumerate}
    ...
    %\item 1-3 are treated on equal footing, so the competition between different mechanisms can be distinguished. 
\end{enumerate}
...
With the Born-Oppenhermier approximation, the electronic wave function $\Theta(\mathbf{r},\mathbf{R})$ is expanded in the basis of adiabatic BO states, which depend on the electronic coordinates $\mathbf{r}$ and the nuclear coordinates $\mathbf{R}(t)$ according to
\begin{equation}
...
%The FSSH algorithm has been implemented in a semiempirical NEXMD package, where the EOMs in Eqs.~\ref{eq:2} and \ref{eq:3} are propagated via Velocity Verlet and Runge-Kutta-Verner fifth- and sixth-order algorithm,~\cite{42} respectively. More details about the implementation can be found in the reference~\cite{12}.
...
\end{equation}
Where $\ket{\phi^n_I(\br,\bR(t))}$ is the adiabatic state of configuraiton $n$. $\ket{\chi_n}$ are Gaussian nuclear basis functions
\begin{equation}
...
\iffalse
Within the MCE formalism, the wavefunction is presented as
\begin{equation}
\ket{\Psi(r,\bR,t)}=\sum_n c_n(t)\ket{\psi_n(r,\bR,t)}.    
\end{equation}
Where $\ket{\psi_n(r,\bR,t)}$ are TBFs, including both nuclear and electronic parts, 
\begin{equation}
    \ket{\psi_n(r,\bR,t)}=\ket{\chi_n(\bR,t)}\sum_I a^n_I\ket{\phi_I(r,\bR)}.
\end{equation}
$\ket{\phi_I}$ are electronic WFs. And $\ket{\chi_n}$ are Gaussian nuclear basis functions
\begin{equation}
    \ket{\chi_n}=\left(\frac{2\alpha}{\pi}\right)^{N_d/4}e^{\left\{-\alpha(R-\bar{R})+\frac{i}{\hbar}P(R-\bar{R})+\frac{i}{\hbar}\gamma_n(t)\right\}}.
\end{equation}
where $N_d$ is the number of degrees of freedom (DOFs).
\fi
...
\iffalse
\begin{figure}[!htb]
\includegraphics{figures/Au20relaxation.pdf}% Here is how to import EPS art
\caption{\label{fig:au20relax} (a) Relaxation time-energy 2D map. 
(b) The evolution of the average electronic energy of the Au$_{20}$.
}
\end{figure}

{\bf Plasmon-mediated CO bond activation}
[from conclusion]

\begin{figure}[htb]
   \centering
   \includegraphics[width=0.5\textwidth]{figures/Au20CO.pdf}
   \caption{
   An analysis of a characteristic trajectory. From top to bottom: (a) potential energy with time, (b) active excited state, (c) transition charges on CO, and (d) variation of bond length of CO. (e) Autocorrelation function of velocity. (f) Phonon spectra obtained by Fourier transform of (e). Inset in (a) shows an enlarged view of potential energy to highlight the high-frequency oscillations (\YZ{may remove this figure as it is not published yet}). 
   }
   \label{Fig:au20CO}
\end{figure} 
\fi