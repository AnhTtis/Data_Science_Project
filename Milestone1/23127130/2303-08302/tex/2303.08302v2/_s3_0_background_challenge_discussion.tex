...
From a practical perspective, we prefer lower quantization sensitivity (accuracy loss) and \classone can be (almost) viewed as the optimal-loss post-training quantization.
...
For INT4 quantization, the asymmetric method has better accuracy than the symmetric method since asymmetric quantization has better utilization of the quantization range.
...
We also find another possible reason that the Layer Norm of \opt-family is not well trained (except \opt-350M): the weight and the bias are all 1's and 0's, respectively.}
...
Similar to weight-only quantization, asymmetric quantization here has better performance than symmetric quantization, e.g., \opt-6.7B has a totally different behavior with asymmetric and symmetric quantization.
...
Note that for models with model size larger than 10B, all of them belong to \classthree, which has more than 0.5 \ppl points degradation. 
