...
Due to the cost of training/finetuning \llms, it is challenge for practitioners/researchers to do finetuning/distillation on those \llms, particularly for models with hundreds of billions parameters, like GPT-3-175B~\cite{brown2020language} and \bloom-176B~\cite{scao2022bloom}. 
...
However, the inference speed of \cite{dettmers2022llm} is generally even slower than FP16 baseline~\cite{bloom_inference} due to the difficulty to implementing mixed-precision within a single tensor.
More recently, \cite{frantar2022gptq} extends OBQ~\cite{frantar2022optimal} on \llms for INT4 weight-only quantization and shows great efficiency on quantization and latency,
...
However, \cite{xiao2022smoothquant} can only work for W8A8 as lower weight precision (INT4) itself already leads to significant accuracy, and the accuracy drop is larger than 0.1 \ppl points, which as discussed in later section is sub-optimal.
