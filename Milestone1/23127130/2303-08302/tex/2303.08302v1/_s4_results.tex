...
% Particularly, for small models (e.g., \opt-125m and \bloom-560m), block size 32/64 can bring around 1 point gain; for larger models (e.g., \opt-66B and \bloom-176B), block size 32/64 can bring around 0.1 gain.
...
To answer this idea, we select the most robust model, i.e., \bloom-176B, to perform 3 to 8 bits asymmetric weight-only quantization.
...
As can be seen, finer-grained quantization cannot have better performance than more bits quantization for all cases. 
...
By testing the optimization-based \ptq method with fine-grained quantization scheme, here is a short summary:
\begin{itemize}
    \item Larger models ($\ge$10B) can achieve \classone quantization error for 4-bit quantization.
    They can benefit from low-precision as the model size is 2x smaller and the accuracy is better than the 8-bit 2x smaller models with high probably.
    \item Smaller models ($\le$10B) usually can only achieve \classtwo or \classthree quantization error.
    As such, the usage of 4-bit quantization needs to be careful for those models.
    \item Fine-grained quantization cannot match the accuracy of more-bit quantization even if the real model size is similar.
    However, how to utilize non-standard bit-precision is still challenge.
\end{itemize}
...
% , but also provide the effect of different block sizes in~\tref{tab:bloom-176-different-blocks} on \bloom-176B.
...
%
For smaller models, fine-grained activation quantization plus per-row weight quantization usually has larger accuracy drop (around $0.1$ \ppl drop) than per-row weight-only quantization. 
% However, applying fine-grained quantization on both has smaller accuracy degradation as compared to weight-only quantization.
...
As expected, smaller block sizes bring better accuracy compared to larger block size.
The performance improvement plateaus after the size reaches 256, which matches the numbers that INT8 can represents. 
However, note that although INT8 can represent 256 different numbers, there is still activation quantization error since we use uniform quantization.
...
\begin{itemize}
    ...
    \item The benefit from fine-grained activation quantization vanishes when the block size is 256.
\end{itemize}