...
% More importantly, such block-k quantization can be implemented on modern GPU (one of the most popular deep learning architecture) since the compute unit (streaming multiprocessor) of GPUs process small chunk of matrix (e.g., 128 by 128) for calculation.
% \zhewei{@Cheng, help re-write the sentence a bit}
More importantly, such block-k quantization can be implemented on modern GPU (one of the most popular deep learning architecture) since the compute unit (streaming multiprocessor) of GPUs process tiles of data (e.g., 128 by 128 tiling size) for matrix computation.

% \zhewei{Why it is doable for kernel implementation? Add some explanation even though we did not provide the kernels :)}
...
For hyper-parameters used for \zqglobal and \zqlocal, we choose the top-three used in~\sref{sec:evaluation_of_existing_methods} for all models except for \bloom-176B, for which we only use the top-one hyperparameter, to reduce the training cost.