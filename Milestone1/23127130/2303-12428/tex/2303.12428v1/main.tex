...
It has been argued that the gain of quantum entanglement between the masses indicates non-classical states of the whole tripartite system.
Here, we focus on non-classical properties of the involved interactions rather than the involved states.
...
For example, unpaired spins interact via spin chains in solids~\cite{Sahling2015}, light modes interact via mechanical membranes~\cite{Thompson2008},
fundamentally electric charges are coupled via electromagnetic field, etc.
All these scenarios share a common structure where systems $A$ and $B$ do not interact directly, but are solely coupled via mediator system $M$, see Fig.~\ref{FIG_SETUP}.
...
Along this line, methods have been proposed to witness non-classicality of mediator's state from correlation dynamics of the coupled probes.
In particular, conditions were derived under which gain of quantum entanglement implies that the mediator must have explored non-orthogonal states during the dynamics~\cite{Krisnanda2017,Pal2021}.
...
They are independent of the initial state, dimensions of involved systems or explicit form of interactions and also work in the presence of local environments.
Accordingly, they are applicable in variety of fields, see e.g.~\cite{Krisnanda2018} for an example in quantum biology and~\cite{Kon2019} in solid state physics.
...
Our main contribution here is the development of quantitive methods.
...
Non-trivial bounds are derived for any continuous correlation measure and the only additional assumption we had to add to the minimalistic list mentioned above is the dimension of the mediator.
...
\begin{figure}[!t]
	...
	we only need to know the dimension of the mediator, even the quantified Hamiltonians can remain unknown.
...
but there the notion of non-classicality is different, based on impossibility of simulating the dynamics via local operations and classical communication.
...
Our tools show that correlations between the masses exclude gravity as a classical interaction (commuting particle-field couplings) via any finite-dimensional mediator.
...
U = U_{BM} U_{AM}.
\end{equation}
Intuitively, decomposable unitaries are those that can be simulated by coupling system $A$
to the mediator $M$, and \emph{then} coupling $B$ to the mediator.
...
Clearly, for classical interactions $[H_{AM}, H_{BM}] = 0$, the unitary operator $U = e^{-itH}$ is decomposable.
...
Therefore, the unitary generated by classical interactions is decomposable, with the added property that the decomposition must commute, i.e. $[ U_{AM}, U_{BM} ] = 0$.
...
In this case, the evolution is described by a map $\Lambda$ giving the state of the system at time $t$, i.e. $\rho = \Lambda_t(\rho_0)$.
...
Since dimensions of $A$ and $B$ are larger than the dimension of the mediator, maximally entangled state between $AB$ cannot be produced.
...
Indeed, from the properties of $g$ and $d$ is it easy to verify that this quantity is monotonic under local operations and it is zero if and only if $\rho$ is a product state across $AM:B$ partition.
...
This scenario is particularly good for demonstrating the violation because the dimension of the mediator is as small as it can be whereas the dimensions of the probes are in principle unbounded.
...
\begin{eqnarray}
  \mathrm{ND}(\Lambda) &\ge& g^{-1} ( Q_{A:MB}(\Lambda(\rho)) - B(\rho_0) ),
\label{EQ_VIOL_AMB}
\end{eqnarray}
...
\begin{equation}
|| e^{-itH} - \left( e^{-i \frac{t}{r} H_{AM}} e^{-i \frac{t}{r} H_{BM}} \right)^r ||_{\infty}
\end{equation}
...
Furthermore, since the trace distance is contractive, Eq.~
(\ref{EQ_ABM}) holds for any distance for finite-dimensional systems, at the cost of constants in function $g$.
...
\begin{eqnarray}
Q_{A:B} (\rho_{t}) \le \sup_{\sigma_{AM}} Q_{A:M} (\sigma_{AM}) + I_{A:B}(\rho_{0}),
\label{EQ_AB_DET}
\end{eqnarray}
where $\rho_t = \lambda(\rho_0)$ and we emphasise that $\lambda \in \overline{\texttt{DEC}}(m)$ acts on $AB$ only.
The supremum on the right-hand side runs over all $AM$ states with $d_M \le m$,
...
This leads to the peculiar conclusion that classical interactions cannot produce a $\texttt{SWAP}$.
...
\begin{equation}
E_{A:B}(\rho_t) \le \sup_{\sigma_{AM}} E_{A:M}(\sigma_{A:M}) + I_{A:B}(\rho_0),
\label{EQ_APP_AB}
\end{equation}
where supremum is over all the states allowed in the theory, here $d_M \le m$ and $D_{AB|M} = 0$.
...
Furthermore, all states admitting $D_{AB|M} = 0$ are disentangled across $A:M$ and therefore the supremum runs over states for which $E_{A:M}(\sigma_{A:M}) = 0$.
...
Therefore, observation of non-zero value of $E_{A:B}$ means that the supremum on the right-hand side is at least this non-zero value,
i.e. the mediator must be capable of being entangled to $A$, and in fact to $AB$ due to monotonicity, to at least the degree that has been measured.
...
If we take two nearby masses prepared in squeezed states with squeezing parameters $s_A$ and $s_B$,
it has been shown that the maximal gravitational entanglement in terms of logarithmic negativity equals $E_{A:B}^{\max} = |s_A + s_B| / \ln 2$, which holds for large squeezing~\cite{Krisnanda2020}.
Since in principle $s_i \to \infty$, gravity cannot be understood as classical interaction with any finite-dimensional mediator.
...
It is rather unlikely that this amount of entanglement will be observed in near future, as it would take tens of hours to produce it via gravity in typical setups.
...
Along the same line, we showed that swapping does not admit decomposable dilation even for unbounded mediator.
Accordingly, we leave it as an open problem to verify if gravity could implement the $\texttt{SWAP}$ gate.
...
and that gravitational interaction cannot be understood as classical (commuting particle-field couplings) via any finite-dimensional mediator.
...
The definition of classicality of interactions in terms of commutativity is justified by the following lemma.
...
\begin{eqnarray}
e^{-i t H} \rho_0 e^{i t H} & = & \Pi(e^{-i t H} \rho_0 e^{i t H}), \\
e^{-i t H} [\rho_0, H] e^{i t H} & = & \Pi(e^{-i t H} [\rho_0, H] e^{i t H}),
\end{eqnarray}
...
In particular, we show the equivalence between classicality $[H_{AM},H_{BM}] = 0$ and the existence of a commuting decomposition $U = U_{AM} U_{BM} = U_{BM} U_{AM}$.

\begin{proposition}\label{PROP_DECOMP}
A unitary $U = e^{-itH}$ has a commuting decomposition $U = U_{BM} U_{AM} = U_{AM} U_{BM}$
if and only if there exist Hamiltonians $H_{AM}$ and $H_{BM}$ such that $H = H_{AM} + H_{BM}$ and $[H_{AM}, H_{BM}] = 0$.
\end{proposition}
\begin{proof}
  Using the Baker-Campbell-Haussdorf (BCH) formula~\cite{Hall_2015}, one easily sees that if such $H_{AM}, H_{BM}$ exists, then $U = e^{-i t H_{AM}} e^{i t H_{BM}} = e^{-i t H_{BM}} e^{-i t H_{AM}}$, showing that the unitary has a commuting decomposition.

	To show the other direction, suppose the unitary $e^{-i t H}$ has a commuting decomposition.
	This means there exist $U_{AM}, U_{BM}$ such that $[U_{AM}, U_{BM}] = 0$ and $e^{-i t H} = U_{AM} U_{BM}$.
	Let $H_{AM} = (i/t) \log{U_{AM}}$ and $H_{BM} = (i/t) \log{U_{BM}}$.
	These interaction Hamiltonians must commute, because we can write the commutator using the series representation $\log{\left(\identity - X\right)} = - \sum_{n=1}^{\infty} \frac{1}{n} X^n$ to get
	\begin{align}
		[H_{AM}, H_{BM}]
		&=
...
The following proposition proves the `correlation capacity' bound when the initial state is product $\rho_0 = \rho_{AM} \otimes \rho_M$.
\begin{proposition}
\label{TH_CAPACITY}
