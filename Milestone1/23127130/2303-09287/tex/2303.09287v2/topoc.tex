...
\subsection{Why heterogeneous consensus?}
...
\begin{rmrk}
\label{rmrk.as.defined.in.this.paper}
...
Correspondingly there is no requirement in this paper that a system in consensus be in agreement amongst \emph{all} participants (see the discussions in Remarks~\ref{rmrk.as.defined.in.this.paper} and~\ref{rmrk.continuity=consensus}), 
but there \emph{is} a requirement that participants that are close to one another --- in a sense we make formal with a novel `intertwined' property which we discuss next --- should agree.
...
We will see that the kernel dictates consensus at $p$, in a sense we make formal (see discussion in Remark~\ref{rmrk.arrow}).
\item
...
\item
As a lattice structure, a semitopology is a 
...
The only nonempty quorum is $\ns P$ itself, reflecting a notion of quorum that requires unanimous agreement. 
...
Its notion of quorum captures an idea that a quorum is a set that may not be all of $\ns P$, but does at least biject with it.
...
The notion of quorum here is that there may be at most one objector (but not two).
...
This notion of quorum reflects a security principle in banking and accounting (and elsewhere) of \emph{separation of duties}, that functional responsibilities be separated such that at least two people are required to complete an action --- so that errors (or worse) cannot be made without being discovered by another person.
\item
...
But this use of language also reflects a real mathematical connection.
Semitopologies \emph{are} semilattices \emph{are} semigroups, in standard ways which we take a moment to spell out: 
\begin{itemize*}
\item
...
We can make some prediction about the theory of semitopologies, just from general mathematical principles.
Fewer axioms means: \emph{more} models, \emph{finer discrimination} between definitions, and (because there are more models) \emph{more counterexamples}.
So we can expect a theory with the look-and-feel of topology, but with new models, finer distinctions between different definitions that in topology may be equivalent, and some new definitions, theorems, and counterexamples --- and this indeed will be the case.
...
However, the theory is so strong, and the class of its models is so restricted --- just the singleton model with one element --- that there is not much left for us to say. 
In particular, additional assumptions we may make on elements add literally nothing of value, because there was only one element to begin with!  So a `stronger' theory is not a `better' one, and conversely a `weaker' set of axioms is not necessarily a `less useful' one.  It all depends on what we want to do --- and on how the maths turns out.}
\end{rmrk}
...
So we can ask:
\begin{quote}
\emph{Within our semitopological framework, what semitopological property or properties can identify the essential nature of the differences between the semitopologies in Example~\ref{xmpl.cc}?}
\end{quote}
...
\item
It suffices to provide a counterexample.
Consider the semitopology illustrated in the lower diagram in Figure~\ref{fig.012} (which is a topology), and set $\atopen=\{0,4\}$.
...
\item
It suffices to provide a counterexample.
Consider the semitopology illustrated in the right-hand diagram in Figure~\ref{fig.012}, and set $\atopen=\{0,1\}$.
...
\end{itemize*}
In this very natural special case, strong transitivity, transitivity, and being hyperconnected, all become equivalent: 
\begin{lemm}
...
\item
Suppose $\atopen$ is hyperconnected, so every pair of nonempty open subsets of $\atopen$ intersect, and suppose $O,O'\in\opens_{\neq\varnothing}$ and $O\between\atopen\between O'$.
Then also $(O\cap\atopen) \between \atopen \between (O'\cap\atopen)$.
Now $O\cap\atopen$ and $O'\cap\atopen$ are open, because $\atopen$ is open we assumed $\ns P$ is a topology (not just a semitopology) so that intersections of open sets are open.
...
\item
By Lemma~\ref{lemm.closure.closed} $\bigcap\{C\in\closed \mid R\subseteq C\}\subseteq\closure{R}$.
By construction $R\subseteq\bigcap\{C\in\closed \mid R\subseteq C\}$, and using Lemma~\ref{lemm.closure.monotone}(\ref{closure.monotone}) and part~2 of this result we have
$$
\closure{R} 
\stackrel{L\ref{lemm.closure.monotone}(\ref{closure.monotone})}\subseteq 
\closure{\bigcap\{C\in\closed \mid R\subseteq C\}} 
\stackrel{pt.2}= 
\bigcap\{C\in\closed \mid R\subseteq C\} .
$$ 
\qedhere\end{enumerate}
\end{proof}
...
Thus, Proposition~\ref{prop.open.strong-consensus} suggests that, to convince a topen to agree on a value, we can first convince an open neighbourhood that intersects the topen, and then use Grade-Cast to convince the whole topen. 
\end{rmrk}
...
Lemma~\ref{lemm.might.as.well} is an easy observation that expresses a precise mathematical sense in which this choice does not really matter; so we can choose whatever is most convenient for a particular case.
We will use Lemma~\ref{lemm.might.as.well} later, to prove Lemma~\ref{lemm.more-than-one}.
\end{rmrk}
...
Suppose $(\ns P,\opens)$ is a \emph{finite} semitoplogy (so $\ns P$ is finite, and so is $\opens$). 
...
\begin{itemize*}
\item
For $j\not\in\{i\minus 1,i\plus 1\}$ we use witness $\{j-1,j+1\}$; 
\item
for $j=i\minus 1$ we use witness $\mathbb Z\setminus \{j\plus 1\}$; and 
\item
for $j=i\plus 1$ use witness $\mathbb Z\setminus \{j\minus 1\}$.
\end{itemize*}
\item
...
\begin{rmrk}
Intuitively, $X\witno X'$ when $X'$ extends $X$ with (at least) one witnesses for every element $p\in X$.
\end{rmrk} 
...
\item
Given $R_i$, for each $p\in R_i$ nondeterministically pick some witness $w(p)\in\witness(p)$ and set $R_{i\plus 1}=R_i\cup\bigcup_{p\in R_i}w(p)$.
\item
...
Then there exists some $p\in O\in\opens(\witness)$ such that $O\cap R=\varnothing$.
Thus by Lemma~\ref{lemm.O.between.lim.R}(3) also $O\cap\f{lim}(R)=\varnothing$. 
\item
\emph{Suppose $p\not\in \f{lim}(R)$.}\quad

By Definition~\ref{defn.closure} we need to exhibit an $p\in O\in\opens(\witness)$ that is disjoint from $R$, and since $R\subseteq\f{lim}(R)$ by Lemma~\ref{lemm.R.subset.lim.R}, 
it would suffice to exhibit $p\in O\in\opens(\witness)$ that is disjoint from $\f{lim}(R)$.
We set 
$$
O=\ns P\setminus \f{lim}(R).
$$
...
\begin{enumerate*}
\item
Nondeterministically choose an initial $R_0$ --- in particular, to compute a closed set containing $p\in\ns P$ we can set $R_0=\{p\}$.
\item
Given $R_i$, for every $p\in \ns P$ check if $w\between R_i$ for every witness $w\in\witness(p)$ and collect these $p$ into a set $B_i$.
Set $R_{i\plus 1}=R_i\cup B_i$.
\item
If $R_{i\plus 1}=R_i$ then terminate with result $R_i$; otherwise loop back to~2.
\end{enumerate*}
...
\item
Call a witnessed set $(\ns P,\witness)$ (Definition~\ref{defn.witnessed.set}(\ref{witness.function})) \deffont{deterministic} when for each point $p$, $\witness(p)$ is a singleton set; thus $\witness(p)=\{W_p\}$.\footnote{$W_p$ (the witness to $p$) is not necessarily equal to $M_p$ (the least open set containing $p$).  The witness function \emph{generates} a witness semitopology, but is not necessarily \emph{equal} to it.}
In words: $\witness$ is deterministic when \emph{every point has precisely one (possibly empty) witness}. 
\end{enumerate*}
\end{defn}
...
When $\witness$ is deterministic, the algorithms simplify: there is precisely one witness to each point, and this removes the \emph{nondeterminism} from the algorithms and they become deterministic --- as our choice of name in Definition~\ref{defn.deterministic} suggests. 
\end{rmrk}
...
In particular, it is possible to create a non-deterministic witness function that generates a topology; intuitively, just because there might be a choice of witnesses, does not mean that the choice makes any difference to the final result.
...
We speculate that Proposition~\ref{prop.det} could be strengthened to show that topologies correspond to Horn clause theories that (may not be deterministic in the sense of Definition~\ref{defn.deterministic}, but which) do not require backtracking.
...
Call a semitopology \deffont{chain-complete} when for every nonempty descending chain of open sets $\mathcal O\subseteq\opens$ (Definition~\ref{defn.ascending.chains}), its intersection $\bigcap\mathcal O$ is open.
\item\label{item.chain-bounded}
Call a semitopology \deffont{chain-bounded} when for every nonempty descending chain of open and nonempty sets $\mathcal O\subseteq\opens_{\neq\varnothing}$, its intersection $\bigcap\mathcal O$ is open and nonempty.
\end{enumerate*}
\end{defn}
...
Suppose $(\ns P,\opens)$ is a chain-bounded semitopology and $O\in\opens_{\neq\varnothing}$ is a nonempty open set.
...
For example: topology has one notion of \emph{dense subset of} and, as discussed in Remark~\ref{rmrk.top.ce}, when we try to transfer this to semitopologies this splits naturally into two distinct notions --- \emph{weakly} dense and \emph{strongly} dense (Definition~\ref{defn.dense}) --- both of which turn out to be important.
...
\item
Semitopological questions such as \emph{`is this a topen set'} or \emph{`are these two points intertwined'} or \emph{`does this point have a topen neighbourhood'} --- and many other definitions in this paper, such as our taxonomy of points into \emph{regular}, \emph{weakly regular}, \emph{quasiregular}, and \emph{conflicted}, or the notions of \emph{witnessed set}, and \emph{kernel} \dots and so on --- appear to be novel.
...
Then the \deffont{semitopological consensus task} is the triple $(\tf{In}, \tf{Out}, \Delta)$ where:
\begin{itemize}
\item $\tf{In}$ is the (pure) simplicial complex with facets simplices $\{(p_1,v_1),...,(p_n,v_n)\}$ where $n=\#\ns P$, $p_i\in \ns P$ and $v_i\in \ns V$ for every $1\leq i\leq n$, and $p_i\neq p_j$ for every $i\neq j$.

\item $\tf{Out}$ is the (pure) simplicial complex with facets simplices $\sigma_{out}=\{(p_1,v_1),...,(p_n,v_n)\}$ where $n=\#\ns P$,  $p_i\in \ns P$ and $v_i \in \ns V$ for every $1\leq i\leq n$, $p_i\neq p_j$ for every $i\neq j$, and $\sigma_{out}$, when seen as a function from $\ns P$ to $\ns V$, is a continuous function on the semitopology $(\ns P, \opens)$.

\item $\Delta$ maps a simplex $\sigma_{in}\in \tf{In}$ to the subcomplex $\Delta(\sigma_{in})$ of $\tf{Out}$ such that $\Delta(\{(p_1,v_1),...,(p_m,v_m)\})$, $1\leq m\leq\#\ns P$, is the simplicial complex with facets simplices $\sigma_{out}=\{(p_1,w_1),...,(p_m,w_m)\}\in \tf{Out}$ where, for every $1\leq i\leq m$, there exist a \emph{cover} $O\in\opens$ of $p_i$ (minimal open set containing $p_i$) and $1\leq j\leq m$ such that $p_j\in O$ and $w_i=v_j$.
\end{itemize}
\end{defn}
...
This could include an arbitrary semitopology on the right (for greatest generality), but even for our intended special case of consensus it would be interesting to try to endow $\tf{Val}$ with a semilattice structure (or something like it), at least, e.g. to model merging of distinct updates to a ledger.
