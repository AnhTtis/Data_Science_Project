...
Our method applies some singularity theory of smooth maps, some theory on differential topology of manifolds, and some elementary real algebraic arguments and it is interdisciplinary. Our study also adds to our related pioneering studies \cite{kitazawa3, kitazawa7}. \cite{bodinpopescupampusorea, kohnpieneranestadrydellshapirosinnsoreatelen, 
...
$D^k:=\{x \in {\mathbb{R}}^{k} \mid ||x|| \leq 1\}$ is the $k$-dimensional unit disk. It is a $k$-dimensional smooth compact and connected submanifold of ${\mathbb{R}}^{k}$ for any non-negative integer $k \geq 1$.
...
\item (Different from "the original $U_D$",) $U_D$ can be chosen as the open set defined as the union of the finitely many open sets chosen before instead. It can and must be chosen as a connected set by our definitions, conditions and construction. Instead of this, we may also choose $U_D$ as an arbitrary open connected subset of the original "$U_D$" and "this new $U_D$" being also an open neighborhood of $\overline{D}$.
\end{itemize}
...
In the case where the hypersurfaces do not intersect, the obtained map is, {\it at least topologically}, a so-called {\it special generic} map. In other words, there exist such a nice map $f_0:M \rightarrow {\mathbb{R}}^n$ and a homeomorphism $\Phi:M \rightarrow M$ enjoying the relation $f_0 \circ \Phi=f$.
It is first presented as a remark which is not essential in the paper.
Considering this rigorously and carefully, the author has found that this may not be trivial and \cite{kitazawa8} proves this rigorously first, after earlier versions of the present paper have appeared.

In the case where the hypersurfaces may intersect, the map is, {\it at least topologically}, locally regarded as a so-called {\it moment map}.
...
\item These disks are centered at points $(x_1,x_2)$ satisfying $x_1<0$ and $x_2 \geq 0$. One of the disks is centered at $(-R,0)$ or sufficiently close to $(-R,0)$. They are also sufficiently small enough to argue as in the following.
\end{itemize}	
...
\bibitem{kitazawa7} N. Kitazawa, \textsl{Construction of real algebraic functions with prescribed preimages}, submitted to a refereed journal, arXiv:2303.00953.
\bibitem{kitazawa8} N. Kitazawa, \textsl{A short note on real algebraic maps which are topologically special generic maps}, arXiv:2312.10646.
