...
\begin{document}
\title[New explicit real algebraic maps of non-positive codimensions]{Natural real algebraic maps of non-positive codimensions with prescribed images whose boundaries consist of non-singular real algebraic hypersurfaces satisfying transversality}
...
\maketitle
\begin{abstract}
We present new real algebraic maps of non-positive codimensions with prescribed images whose boundaries consist of explicit non-singular real algebraic hypersurfaces satisfying so-called "transversality". We also give explicit information on important real polynomials. This gives new construction of explicit real algebraic maps of non-positive codimensions with explicit information on images, preimages and important real polynomials.

Thanks to celebrated theory of Nash and Tognoli, smooth closed manifolds are {\it non-singular} real algebraic manifolds and the zero sets of some real polynomial maps. In considerable cases, we can approximate smooth functions or more generally, maps, by real algebraic ones. It is in general difficult to have explicit examples. We have constructed some previously. We have ones of some new type with explicit information on global structures of the maps.  
...
Our method applies some singularity theory of smooth maps, some theory on differential topology of manifolds, and some elementary real algebraic arguments. Our method is also interdisciplinary. Our study also adds to our related pioneering studies \cite{kitazawa3, kitazawa7}. \cite{bodinpopescupampusorea, kohnpieneranestadrydellshapirosinnsoreatelen, 
	sorea1, sorea2}, which are studies on domains formed by real algebraic hypersurfaces, also motivate us.
...
$D^k:=\{x \in {\mathbb{R}}^{k} \mid ||x|| \leq 1\}$ is the $k$-dimensional unit disk. It is a $k$-dimensional smooth compact and connected submanifold of ${\mathbb{R}}^{k}$ for any positive integer $k \geq 1$.

Let $c:X \rightarrow Y$ be a differentiable map from a differentiable manifold $X$ into another one $Y$. $x \in X$ is a {\it singular} point of $c$ if the rank of the differential at $x$ is smaller than the minimum of the (multi)set $\{\dim X, \dim Y\}$. $c(x)$ is a {\it singular value} of $c$ if $x$ is a singular point of $c$. Let $S(c)$ denote the set of all singular points of $c$. 
Let ${dc}_x:T_x X \rightarrow T_{c(x)}Y$ denote the differential of $c$ at $x \in X$: this is a linear map from the tangent vector space $T_x X$ at $x$ to the tangent vector space $T_{c(x)} Y$ at $c(x)$.
...
\begin{MainThm}
	...
	Let $D$ be a connected open subset of ${\mathbb{R}}^n$ whose closure $\overline{D}$ is compact and connected. Suppose also that the following are enjoyed.
	\begin{enumerate}
		\item \label{mthm:1.1} $S_j \bigcap D$ is empty. $S_j \bigcap \overline{D}$ is non-empty. % It is also connected. %The intersection of $U_D$ and the zero set of $f_j$ is $U_D \bigcap S_j$.
For an arbitrary sufficiently small connected open neighborhood $U_D$ of $\overline{D}$, $D=U_D \bigcap {\bigcap}_{j=1}^{l_1} \{x \mid f_j(x)>0\}$, $\overline{D}=U_D \bigcap {\bigcap}_{j=1}^{l_1} \{x \mid f_j(x) \geq 0\}$ and $\overline{D}-D \subset {\bigcup}_{j=1}^{l_1} S_j$ hold. 
		\item \label{mthm:1.2} ${\bigcap}_{i=1}^{i_0} S_{j_i}$ is either empty or a non-singular real algebraic submanifold of dimension $n-i_0$ where $\{j_i\}_{i=1}^{i_0}$ is an increasing sequence. The real algebraic submanifold is also with no boundary of course.
		  \item \label{mthm:1.3} ${\bigcap}_{i=1}^{i_0} S_{j_i} \bigcap \overline{D}$ is either empty or an {\rm (}$n-i_0${\rm )}-dimensional smooth compact manifold of ${\bigcap}_{i=1}^{i_0} S_{j_i}$ where $i_0$ is as just before. 
%The {\rm (}$n-i_0${\rm )}-dimensional manifold is cornered in general.
\item \label{mthm:1.4} A kind of "transversality" holds{\rm :} for each point $p$ in the intersection ${\bigcap}_{i=1}^{i_0} S_{j_i}$, the following two agree.
\begin{enumerate}
\item The intersection ${\bigcap}_{i=1}^{i_0} {d {e_{j_i}}}_{q_i}(T_{q_i} S_{j_i})$ of the images of the differentials at unique single points $q_j \in {e_{j_i}}^{-1}(p)$ of the canonical embeddings $e_{j_i}:S_{j_i} \rightarrow {\mathbb{R}}^n$.
\item The image of the differential of the canonical embedding $e_{\{j_i\}_{i=1}^{i_0}}$ of ${\bigcap}_{i=1}^{i_0} S_{j_i}$ at the point of the one-point set ${e_{\{j_i\}_{i=1}^{i_0}}}^{-1}(p)$.
\end{enumerate}
\item \label{mthm:1.5} A surjective map $m_{l_1,l_2}:{\mathbb{N}}_{l_1} \rightarrow {\mathbb{N}}_{l_2}$ enjoying the following properties exists: for any non-empty intersection ${\bigcap}_{i=1}^{i_0} S_{j_i}$ represented by using some positive integer $i_0$ and some increasing sequence $\{j_i\}_{i=1}^{i_0}$, the restriction of $m_{l_1,l_2}$ to the set $\{j_i\}_{i=1}^{i_0}$ is always injective.
	\end{enumerate}
...
This is an extension or a variant of main results of the author, presented in \cite{kitazawa3, kitazawa7}. There the hypersurfaces do not intersect. See also \cite{kitazawa4, kitazawa9}. Additional Main Theorems, presented in the third section, are also our related new result. They are on the functions represented as the compositions of the maps with canonical projections and their global structures. \cite{kitazawa3} also studies this kind of problems.
...
See \cite{golubitskyguillemin} for transversality. In addition, some conditions will be dropped. However, we do not omit them here. Related extensions are presented in Remark \ref{rem:2} and Example \ref{ex:1}, in the fourth section, shortly. The next section is on Main Theorem \ref{mthm:1}.
Main Theorems \ref{mthm:2} and \ref{mthm:3} are presented as an application in the third section. The fourth section presents additional remarks on our study. \\
\ \\
...
Data essentially supporting our present study are all in the
 paper. We also note that the present version is a revised version of the previously submitted version \cite{kitazawa8}. We have revised based on our consideration. We have corrected errors. This does not affect our main ingredients. We have added Main Theorem \ref{mthm:3} and related exposition.  
...
\setcounter{enumi}{5}
	...
	\item Let $p$ be a point of ${\bigcap}_{i=1}^{i_0} S_{j_i} \bigcap \overline{D}$ such that for any increasing sequence $\{{j^{\prime}}_i\}_{i=1}^{i_0+1}$ containing the increasing sequence $\{j_i\}_{i=1}^{i_0}$ as a subsequence, $p \notin {\bigcap}_{i=1}^{i_0+1} S_{{j^{\prime}}_i}$ holds. Then it is diffeomorphic to ${\prod}_{j \in {\mathbb{N}}_{l_2}-\{m_{l_1,l_2}(j_i)\}_{i=1}^{i_0}} S^{m_{l_2}(j)}$ if ${\mathbb{N}}_{l_2}-\{m_{l_1,l_2}(j_i)\}_{i=1}^{i_0}$ is not empty and the one-point set if ${\mathbb{N}}_{l_2}-\{m_{l_1,l_2}(j_i)\}_{i=1}^{i_0}$ is empty.
\end{enumerate}
\end{Thm}
...
We define $S_0:=\{(x,y_1,\cdots,y_{l_2}) \in U_D \times {\prod}_{i=1}^{l_2} {\mathbb{R}}^{m_{l_2}(i)+1} \subset {\mathbb{R}}^n \times {\prod}_{i=1}^{l_2} {\mathbb{R}}^{m_{l_2}(i)+1}={\mathbb{R}}^{m+l_2} \mid {\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)} (f_{j}(x_1,\cdots,x_n))-{||y_i||}^2=0, i \in {\mathbb{N}}_{l_2}\} \subset {\mathbb{R}}^{m+l_2}$. Here $y_i:=(y_{i,1},\cdots,y_{i,m_{l_2}(i)+1})$. 
We consider the condition (\ref{mthm:1.5}) and the map  $m_{l_1,l_2}:{\mathbb{N}}_{l_1} \rightarrow {\mathbb{N}}_{l_2}$. ${m_{l_1,l_2}}^{-1}(i)$ is not empty for any $i \in {\mathbb{N}}_{l_2}$ since $m_{l_1,l_2}$ is assumed to be surjective. \\
...
As an important tool, we consider the partial derivatives of the real polynomial function defined by the real polynomial ${\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)}  (f_{j}(x_1,\cdots,x_n))-||y_i||^2={\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)}  (f_{j}(x_1,\cdots,x_n))-{\Sigma}_{j^{\prime}=1}^{m_{l_2}(i)+1} {y_{i,j^{\prime}}}^2$ for variables $x_j$ and $y_{i,j^{\prime}}$ and for each integer $1 \leq i \leq l_2$. In STEP 1-1 and STEP 1-2, we apply implicit function theorem around each point of $S_0$. After that, in STEP 1-3, we give additional arguments to complete our proof of the fact that $S_0$ is a smooth compact submanifold with no boundary. \\
\ \\
...
We have $f_{j}(x_0)>0$ for any $1 \leq j \leq l_1$. This is thanks to our first condition (\ref{mthm:1.1}) in the listed conditions on the sets $S_j$, $D$ and $U_D$. As a result ${\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)}  f_{j}(x_0)>0$ for each integer $1 \leq i \leq l_2$. 
%We use the notation $x_0:=(x_{0,1},\cdots,x_{0,n})$ and $y_0:=(y_{0,1},\cdots,y_{0,n^{\prime}-n})$ for example as before. 
We have $2y_{i,{j_0}^{\prime}}=2y_{0,i,{j_0}^{\prime}} \neq 0$ as the value of the partial derivative at the point $(x_0,y_0)$ for some variable $y_{i,{j_0}^{\prime}}$ where $y_{0,i,{j_0}^{\prime}}$ is from $y_{0,i}:=(y_{0,i,1},\cdots,y_{0,i,m_{l_2}(i)+1})$ with $y_0:=(y_{0,1},\cdots,y_{0,l_2})$.
...
The point is not a singular point of the map. Moreover, around the point $(x_0,y_0)$, $S_0$ is regarded as the graph of a smooth function around $x_0 \in D$. \\
\ \\
...
Let $i_y \in \mathbb{N}_{l_2}-m_{l_1,l_2}(\{j_i\}_{i=1}^{i_0})$. We consider partial derivatives of the function defined by the real polynomial ${\prod}_{j \in {m_{l_1,l_2}}^{-1}(i_y)} (f_{j}(x_1,\cdots,x_n))-||y_{i_y}||^2={\prod}_{j \in {m_{l_1,l_2}}^{-1}(i_y)} (f_{j}(x_1,\cdots,x_n))-{\Sigma}_{j^{\prime}=1}^{m_{l_2}(i_y)+1} {y_{i_y,j^{\prime}}}^2$. More precisely, we consider the partial derivative for each variable $y_{i_1,j^{\prime}}:=y_{i_y,j^{\prime}}$ at $(x_{\rm O},y_{\rm O})$.
...
We consider an integer $i_x \in m_{l_1,l_2}(\{j_i\}_{i=1}^{i_0})$. We discuss partial derivatives of the function defined canonically from the real polynomial ${\prod}_{j \in {m_{l_1,l_2}}^{-1}(i_x)} (f_{j}(x_1,\cdots,x_n))-||y_{i_x}||^2={\prod}_{j \in {m_{l_1,l_2}}^{-1}(i_x)} (f_{j}(x_1,\cdots,x_n))-{\Sigma}_{j^{\prime}=1}^{m_{l_2}(i_x)+1} {y_{i_x,j^{\prime}}}^2$.
...
We consider the partial derivatives for the variables $y_{i_1,{j_0}^{\prime}}$ and $y_{i_2,{j}^{\prime}}$, defined just before. We always have $0$ as the values by our definition. 
...
We also discuss the partial derivative for each variable $x_{i_3}$. For the integer $i_x$, we have the unique number $j_{i_x} \in \{j_i\}_{i=1}^{i_0}$ satisfying $i_x=m_{l_1,l_2}(j_{i_x})$. This follows from the assumption that the restriction of $m_{l_1,l_2}$ to $\{j_i\}_{i=1}^{i_0}$ is injective. 
...
\begin{itemize}
	...
	\end{itemize}
In addition, remember the assumption on the transversality on intersections for hypersurfaces $S_j$ (the conditions (\ref{mthm:1.4}) with (\ref{mthm:1.2}, \ref{mthm:1.3})). 
...
This also counts "$i_0$" of the rank independently from the "$l_2-i_0$" before. This is thanks to the following.

\begin{itemize} 
\item At $(x_{\rm O},y_{\rm O})$, $y_{i_x}$ is the origin.
\item For the $i_0$ polynomials the values of the partial derivatives for the variables $y_{i_1,{j_0}^{\prime}}$, $y_{i_2,{j}^{\prime}}$ and an arbitrary variable $y_{i,j}$ are always $0$.
\end{itemize} 
...
\begin{itemize}
\item $p$ is a point of ${\bigcap}_{i=1}^{i_{p,0}} S_{j_i} \bigcap \overline{D}$ for some increasing sequence $\{j_i\}_{i=1}^{i_{p,0}}$.
\item $p \notin {\bigcap}_{i=1}^{i_{p,0}+1} S_{{j^{\prime}}_{i}}$ holds for any increasing sequence $\{{j^{\prime}}_i\}_{i=1}^{i_{p,0}+1}$ containing the sequence $\{j_i\}_{i=1}^{i_{p,0}}$ as a subsequence. 
\end{itemize}
...
\item Consider the open set $D \bigcup {\bigcup}_{p \in \overline{D}-D} D_p$ and the open cover $\{D\} \sqcup \{D_p\}_{p \in \overline{D}-D}$ of the compact and connected subset $\overline{D}$. We can choose finitely many open sets to cover this compact and connected set $\overline{D}$. We choose such sets.
\item (Different from "the original $U_D$",) $U_D$ can be chosen as the open set $U_D:={U_{D}}^{\prime}$ defined as the union of the finitely many open sets chosen before instead. It can and must be chosen as a connected set by our definitions, conditions and construction. We may also choose our new $U_D:={U_D}^{\prime \prime}$ as an arbitrary open connected subset of ${U_D}^{\prime}$ if it is also an open neighborhood of $\overline{D}$.
\end{itemize}
...
\item There exists an integer $1 \leq i_{x_{U,0}} \leq l_2$.
\item We have two relations $f_{j}(x_{U})>0$ for each $j \in  {m_{l_1,l_2}}^{-1}(i_{x_{U,0}})$ except one $j=j_{x_{U,0}} \in {m_{l_1,l_2}}^{-1}(i_{x_{U,0}})$ and $f_{j_{x_{U,0}}}(x_{U})<0$.
\end{itemize}
...
We consider the canonical projection ${\pi}_{m+l_2,n}$ and its restriction to $M$. Thus we have a smooth real algebraic map $f:M:=S_0 \rightarrow {\mathbb{R}}^n$. We check our desired fact that $f$ is a desired map.
...
\begin{enumerate}
\item For the image $f(S(f))$ of the singular set of $f$, $f(S(f))=\overline{D}-D$. 
\item For any point $p \in {\bigcap}_{i=1}^{i_0} S_{j_i}$ such that for any increasing sequence $\{{j^{\prime}}_i\}_{i=1}^{i_0+1}$ containing the sequence $\{j_i\}_{i=1}^{i_0}$, the condition $p \notin {\bigcap}_{i=1}^{i_0+1} S_{{j^{\prime}}_i}$ is satisfied, the following two agree.
\begin{enumerate}
\item The image of the differential of the map $f$ at each point of $f^{-1}(p)$.
\item The tangent vector space of ${\bigcap}_{i=1}^{i_0} S_{j_i}$ at $p$.
\end{enumerate}
\end{enumerate}
\end{Thm}

Note that \cite{kitazawa3} considers a specific case for Main Theorem \ref{mthm:1}. These hypersurfaces $S_j$ do not intersect and $l_2=1$ there.
...
Considering this rigorously and carefully, the author has found that this may not be trivial. \cite{kitazawa8} proves this rigorously first, after earlier versions of the present paper have appeared.
...
\subsection{Main Theorems \ref{mthm:2} and \ref{mthm:3} and their proofs.}


\begin{MainThm}
	\label{mthm:2}
	Consider a graph $G:=G_0$ with exactly one edge and two vertices or a graph $G:=G_{{\rm l,r},l}$ in the following.
	\begin{itemize}
...
\item There exist no other edges.
	\end{itemize}
	 We have a family $\{{f_{G,j}}:M_{G,j} \rightarrow \mathbb{R}\}_{j=1}^{\infty}$ of countably many smooth real algebraic functions ${f_{G,j}}:M_{G,j} \rightarrow \mathbb{R}$ on some non-singular real algebraic closed and connected manifolds $M_{G_j}$ enjoying the following.
\begin{enumerate}
\item The dimension of each manifold $M_{G_j}$ is sufficiently large where the dimensions may be mutually distinct for distinct manifolds.
\item The Reeb graphs $W_{f_{G,j}}$ of the functions $f_{G,j}$ collapse to a graph isomorphic to $G$ and here the Reeb graphs of distinct functions are not isomorphic.
\end{enumerate}
\end{MainThm}
...
We can choose $l-1$ circles the $j$-th circle in which is centered at some point $(0,t_j)$ ($t_j \in \mathbb{R}$) and whose radius is $1$ in such a way that the circles are the boundaries of disks in ${\mathbb{R}}^2$ of radii $1$ and mutually disjoint. We choose such $l-1$ circles in the case $G:=G_{{\rm l,r},l}$.

For any positive integer $k>0$, we can choose $k$ disks of suitably chosen radii enjoying the following properties and we do.
\begin{itemize}
...
\item These disks are centered at points $(x_1,x_2)$ satisfying $x_1<0$ and $x_2 \geq 0$. One of the disks is centered at $(-R,0)$ or sufficiently close to $(-R,0)$. They are also sufficiently small enough to argue as follows.
\end{itemize}	
We discuss our desired family of functions and their Reeb graphs. We can construct maps into ${\mathbb{R}}^2$ by applying Main Theorem \ref{mthm:1} respecting the situations. We compose the canonical projection ${\pi}_{2,1}$ to the first component to have our desired functions. 

We discuss singular points of the functions and their Reeb graphs shortly. We abuse the notation of Main Theorem \ref{thm:1}. According to Theorem \ref{thm:2}, singular points of the functions exist in the preimage $f^{-1}(p)$ ($p \in {\mathbb{R}}^2$) in Main Theorem \ref{mthm:1} if and only if $p$ is contained in two distinct circles $S_j$ or regarded as a "pole" of some circle $S_j$.
We discuss such points $p$ the preimages $f^{-1}(p)$ of which contain some singular points of the resulting functions. 
We can see that the number of such points is $3k+1$ in the case $G:=G_0$. We can see that the number is $3k+2(l-1)+1$ in the case $G:=G_{{\rm l,r},l}$. The number of the vertices of the Reeb graph of the resulting function is $3k+1$ in the case $G:=G_0$.
The number is $3k+3$ in the case $G:=G_{{\rm l,r},l}$.
...
We discuss more. We consider two distinct disks and the circles of their boundaries here. The three points whose preimages (for $f$)
...
contain singular points of the resulting function in the $l-1$ circles chosen first and the point $(R,0)$, whose preimage contains singular points of the resulting function, are all in the right of the points before. 

This is also thanks to the listed properties: each of the $k$ chosen disks is assumed to be sufficiently small.
...
We discuss our natural generalization of Main Theorem \ref{mthm:2}.
Hereafter, a {\it circle} means a circle in ${\mathbb{R}}^2$ centered at a point $p_0:=(p_{0,1},p_{0,2}) \in {\mathbb{R}}^2$ and of a radius $r_0>0$ unless otherwise stated. This is represented as the zero set of the real polynomial ${(x_1-p_{0,1})}^2+{(x_2-p_{0,2})}^2-r_0$ ($x=(x_1,x_2) \in {\mathbb{R}}^2$). Of course this is non-singular. We define the point $(p_{0,1} \pm r,p_{0,2})$ ($(p_{0,1},p_{0,2} \pm r)$) a {\it vertical} (resp. {\it horizontal}) {\it pole} of the circle. 

\begin{Prop}
\label{prop:1}
In Main Theorem \ref{mthm:1} and Theorems \ref{thm:1} and \ref{thm:2}, let $n=2$ and suppose that the family $\{S_j\}_{j=1}^{l_1} \subset {\mathbb{R}}^2$ is a family of mutually disjoint circles and that the relation $\overline{D}-D={\sqcup}_{j=1}^{l_1} S_j$ holds. We consider our construction of $f:M \rightarrow {\mathbb{R}}^2$ as presented in our proof. The function $f_0:={\pi}_{2,1} \circ f:M \rightarrow \mathbb{R}$ enjoys the following.
\begin{enumerate}
...
\item $f_0$ is a Morse-Bott function.
\end{enumerate}

\end{Prop}

\begin{proof}
Most of our idea for our proof is presented in our preprint \cite{kitazawa9} and the proof of Main Theorem \ref{mthm:2}.
...
By the presented arguments, $S(f_0)$ is represented as a subset of $f^{-1}({\sqcup}_{j=1}^{l_1} \{(p_{j,1}-r_j,p_{j,2}),(p_{j,1}+r_j,p_{j,2})\})$.
...
Furthermore, the identity map on ${\prod}_{j \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_0)\}} S^{m_2(j)}$ comes from a natural map mapping $(x,y) \in M \subset \overline{D} \times {\mathbb{R}}^{m-n+l_2}=\overline{D} \times {\mathbb{R}}^{m_2 \circ m_{l_1,l_2}(j_0)+1} \times {\mathbb{R}}^{m-n+l_2-m_2 \circ m_{l_1,l_2}(j_0)-1} \subset {\mathbb{R}}^{m+l_2}$ onto an element of $\overline{D} \times {\mathbb{R}}^{m_2 \circ m_{l_1,l_2}(j_0)+1} \times {\prod}_{j \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_0)\}} S^{m_2(j)}$ defined as follows.
We remember the notation $y=(y_1,\cdots y_{l_2})$. For $i \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_0)\}$, the component $y_i$ is mapped onto the component $\frac{1}{\sqrt{{\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)} (f_{j}(x_1,x_2))}} y_i \in S^{m_2(i)}$ at $(x,y)$. $x$ is mapped to $x$ itself and $y_{m_{l_1,l_2}(j_0)}$ is mapped to $y_{m_{l_1,l_2}(j_0)}$ itself. This map also gives a diffeomorphism.
...
Considering natural local coordinates completes the proof. 

\end{proof}

Main Theorem \ref{mthm:3} gives a kind of generalizations of Main Theorem \ref{mthm:2}. This gives some fundamental families of real algebraic functions which are obtained as compositions of real algebraic maps into ${\mathbb{R}}^2$ whose images are bounded and connected regions in ${\mathbb{R}}^2$ surrounded by circles intersecting satisfying transversality with ${\pi}_{2,1}$ and whose Reeb graphs collapse to given graphs.

\begin{MainThm}
\label{mthm:3}
As in Proposition \ref{prop:1}, in Main Theorem \ref{mthm:1} and Theorems \ref{thm:1} and \ref{thm:2}, let $n=2$ and suppose that the family $\{S_j:=S_{0,j}\}_{j=1}^{l_{0,1}} \subset {\mathbb{R}}^2$ {\rm (}$l_1:=l_{0,1}${\rm )} is a family of mutually disjoint circles and that the relation $\overline{D_0}-D_0={\sqcup}_{j=1}^{l_{0,1}} S_j$ {\rm (}$D:=D_0${\rm )} holds. We consider our construction of $f:=f_{M_0}:M_0 \rightarrow {\mathbb{R}}^2$ as presented in our proof. For the Reeb graph $W_{f_{0,M_0}}$ of the function $f_{0,M_0}:={\pi}_{2,1} \circ f_{M_0}$, let $n_e$ denote the number of the edges of $W_{f_{0,M_0}}$ and $\{e_j\}_{j=1}^{n_e}$ the family of all edges of the graph.

For each map $l_{\{e_j\}_{j=1}^{n_e}}:{\mathbb{N}}_{n_e} \rightarrow {\mathbb{N}} \sqcup \{0\}$, there exist a non-singular real algebraic manifold $M_{l_{\{e_j\}_{j=1}^{n_e}}}$ which is also a closed and connected manifold and whose dimension is sufficiently large and a smooth real algebraic map $f_{l_{\{e_j\}_{j=1}^{n_e}}}:M_{l_{\{e_j\}_{j=1}^{n_e}}} \rightarrow {\mathbb{R}}^2$ with the following properties. 
\begin{enumerate}
\item \label{mthm:3.1} Each map $f:=f_{l_{\{e_j\}_{j=1}^{n_e}}}$ is constructed via "our construction of the map $f$ in the proof of Main Theorem \ref{mthm:1} and Theorems \ref{thm:1} and \ref{thm:2}" by choosing each $S_j$ suitably satisfying the following conditions in the construction here. Here we abuse the notation in these theorems and proofs.
\begin{enumerate}
\item $l_1:=l_{0,1}+{\Sigma}_{j^{\prime}=1}^{n_e} l_{\{e_j\}_{j=1}^{n_e}}(j^{\prime})$.
...
\item \label{mthm:3.2} Each Reeb graph $W_{f_{0,l_{\{e_j\}_{j=1}^{n_e}}}}$ collapses to the original Reeb graph $W_{f_{0,M_0}}$. 
...
\item \label{mthm:3.4}
For any pair of two distinct cases $M_{l_{\{e_j\}_{j=1}^{n_e},1}}$ and $M_{l_{\{e_j\}_{j=1}^{n_e},2}}$ such that the values of the maps $l_{\{e_j\}_{j=1}^{n_e}}:=l_{\{e_j\}_{j=1}^{n_e},1}$ and $l_{\{e_j\}_{j=1}^{n_e}}:=l_{\{e_j\}_{j=1}^{n_e},2}$ are same except at exactly one number of ${\mathbb{N}}_{n_e}$, the Reeb graphs $W_{f_{0,l_{\{e_j\}_{j=1}^{n_e},1}}}$ and $W_{f_{0,l_{\{e_j\}_{j=1}^{n_e},2}}}$ are not isomorphic.
\item \label{mthm:3.5} Each function $f_{0,l_{\{e_j\}_{j=1}^{n_e}}}$ is a Morse-Bott function.

\end{enumerate}
 We can also choose the dimensions of the manifolds $M_{l_{\{e_j\}_{j=1}^{n_e}}}$ as a fixed dimension greater than or equal to $4$. 
\end{MainThm}
\begin{proof}
We first show the former statement with the five properties.

We choose circles $S_j$ for our proof.
We can choose our circles enjoying the following properties.
\begin{enumerate}\setcounter{enumi}{5}
\item \label{mthm:3.6} For $1 \leq j \leq l_{0,1}$, $S_j:=S_{j,0}$.

\item \label{mthm:3.7} Circles in the family $\{S_{l_{0,1}+j}\}_{j=1}^{l_1-l_{0,1}}$ are centered at points in some circles in the family $\{S_{j,0}\}_{j=1}^{l_{0,1}}$ which are not vertical or horizontal poles. These circles are sufficiently small and mutually disjoint.
Furthermore, consider the preimage $P_{e_{i}}:={q_{f_{0,M_0}}}^{-1}(e_{i}) \subset M_0$, the image $f_{0,M_0}(P_{e_i})$ and the intersection $f_{0,M_0}(P_{e_i}) \bigcap {\sqcup}_{j=1}^{l_{0,1}} S_j$. The intersection is not empty and it is a disjoint union of two curves diffeomorphic to $\mathbb{R}$. We choose one connected curve $L$ from these curves.
The circle $S_{l_{0,1}+{\Sigma}_{j^{\prime}=1}^{i-1} l_{\{e_j\}_{j=1}^{n_e}}(j^{\prime})+j^{\prime \prime}}$ is centered at a point in $L$ for $1 \leq j^{\prime \prime} \leq l_{\{e_j\}_{j=1}^{n_e}}(i)$.
...
\item \label{mthm:3.8} We can apply Main Theorem \ref{mthm:1} with Theorems \ref{thm:1} and \ref{thm:2} and our proof to have our desired map $f_{l_{\{e_j\}_{j=1}^{n_e}}}:M_{l_{\{e_j\}_{j=1}^{n_e}}} \rightarrow {\mathbb{R}}^2$. For $p \in \overline{D}$, ${f_{l_{\{e_j\}_{j=1}^{n_e}}}}^{-1}(p)$ contains some singular points of the function $f_{0,l_{\{e_j\}_{j=1}^{n_e}}}$ if and only if it is a vertical pole $v_j$ of some circle $S_j$ satisfying $v_j \in \overline{D}$ or a point contained in exactly two disjoint circles in $\{S_j\}_{j=1}^{l_1}$.
\item \label{mthm:3.9} Each circle $S_{l_{0,1}+j}$ in $\{S_{l_{0,1}+j}\}_{j=1}^{l_1-l_{0,1}}$ contains exactly one vertical pole $v_{S_{l_{0,1}+j}}$ of the circle $S_{l_{0,1}+j}$ satisfying $v_{S_{l_{0,1}+j}} \in \overline{D}$ and two points contained in exactly two disjoint circles in $\{S_j\}_{j=1}^{l_1}$. These three points are mutually distinct and the values of the first components are also mutually distinct.
\item \label{mthm:3.10} Each circle $S_{j,0}$ in $\{S_{j,0}\}_{j=1}^{l_{0,1}}$ contains exactly two vertical poles $v_{S_{j,0},1}$ and $v_{S_{j,0},2}$ of it satisfying $v_{S_{j,0},1}, v_{S_{j,0},2} \in \overline{D}$. These vertical poles are different from the points discussed in (\ref{mthm:3.9}). The values of the first components of these vertical poles are always different from those of these points discussed in (\ref{mthm:3.9}).
\end{enumerate}

We constructing our map $f_{l_{\{e_j\}_{j=1}^{n_e}}}:M_{l_{\{e_j\}_{j=1}^{n_e}}} \rightarrow {\mathbb{R}}^2$ in (\ref{mthm:3.8}). We can check the first three properties (\ref{mthm:3.1}, \ref{mthm:3.2}, \ref{mthm:3.3}) here easily from the construction. 
The property (\ref{mthm:3.4}) follows from the difference of numbers of vertices. This follows from exposition on singularities of the function in (\ref{mthm:3.8}) with (\ref{mthm:3.9}, \ref{mthm:3.10}).
...
For the last property (\ref{mthm:3.5}), we discuss local singularities of the function $f_{0,l_{\{e_j\}_{j=1}^{n_e}}}$. 

First, we can remember our proof of Proposition \ref{prop:1} to singularities of the function at vertical poles. These singular points are for singular points of Morse-Bott functions
...
Around each point of the discrete set $S_{j_{0,1}} \bigcap S_{j_{0,2}}$, our map $f:M \rightarrow {\mathbb{R}}^2$ is represented as the product map of a natural Morse function on the disk $D^{m_2 \circ m_{l_1,l_2}(j_{0,1})+1}$, a natural Morse function on the disk $D^{m_2 \circ m_{l_1,l_2}(j_{0,2})+1}$ and the identity map on a sphere diffeomorphic to the product ${\prod}_{j \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_{0,1}),m_{l_1,l_2}(j_{0,2})\}} S^{m_2(j)}$ in the case $l_2>2$ and the product map of a natural Morse function on the disk $D^{m_2 \circ m_{l_1,l_2}(j_{0,1})+1}$ and a natural Morse function on the disk $D^{m_2 \circ m_{l_1,l_2}(j_{0,2})+1}$ in the case $l_2=2$. 

The identity map on ${\prod}_{j \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_{0,1}),m_{l_1,l_2}(j_{0,2})\}} S^{m_2(j)}$ comes from a natural map mapping $(x,y) \in M \subset \overline{D} \times {\mathbb{R}}^{m-n+l_2}=\overline{D} \times {\mathbb{R}}^{m_2 \circ m_{l_1,l_2}(j_{0,1})+1} \times {\mathbb{R}}^{m_2 \circ m_{l_1,l_2}(j_{0,2})+1}  \times {\mathbb{R}}^{m-n+l_2-m_2 \circ m_{l_1,l_2}(j_{0,1})-m_2 \circ m_{l_1,l_2}(j_{0,2})-2} \subset {\mathbb{R}}^{m+l_2}$ onto an element of $\overline{D} \times {\mathbb{R}}^{m_2 \circ m_{l_1,l_2}(j_{0,1})+1} \times {\mathbb{R}}^{m_2 \circ m_{l_1,l_2}(j_{0,2})+1} \times {\prod}_{j \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_{0,1}), m_{l_1,l_2}(j_{0,2})\}} S^{m_2(j)}$ defined as follows.
We remember the notation $y=(y_1,\cdots y_{l_2})$. For $i \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_{0,1}),m_{l_1,l_2}(j_{0,2})\}$, the component $y_i$ is mapped onto the component $\frac{1}{\sqrt{{\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)} (f_{j}(x_1,x_2))}} y_i \in S^{m_2(i)}$ at $(x,y)$. $x$ is mapped to $x$ itself and $y_{m_{l_1,l_2}(j_{0,a})}$ is mapped to $y_{m_{l_1,l_2}(j_{0,a})}$ itself for $a=1,2$. This map also gives a diffeomorphism.
%Note that we can generalize this to higher dimensional cases of course. However we do not need this here.

We respect these arguments and local coordinates of the functions and the maps. 
We also remember the property on singularities (\ref{mthm:3.8}) with related properties (\ref{mthm:3.9}, \ref{mthm:3.10}).
This complete the proof of the last property (\ref{mthm:3.5}).
...
Note that for example, the Reeb graph of FIGURE \ref{fig:2} is a bit different from Reeb graphs obtained in this proof. We can find some difference in the left part of the graph. 
...
\begin{enumerate}
\item We can argue similarly in the case $D$ may not be compact. In such a case, $M$ may not be compact. We can argue similarly in the case $D$ may not be connected. In such a case, $M$ may not be connected.
\item We can argue similarly in the case $S_j$ may not be connected and may be a union of connected components of the zero set of a polynomial $f_j$.
...
\end{itemize} 
\item In the complementary set ${\mathbb{R}}^k-\overline{D}$, we do not need the assumption on the transversality for intersections of the hypersurfaces $S_j$.
\end{enumerate}
\end{Ex}
...
\bibitem{kitazawa7} N. Kitazawa, \textsl{Construction of real algebraic functions with prescribed preimages}, a positive report for major revision has been sent, we have found no errors and also found that we need more precise arguments, which is presented in \cite{kitazawa9}, and a revised version of this will be submitted again to a refereed jounral, arXiv:2303.00953v2.
\bibitem{kitazawa8} N. Kitazawa, \textsl{Natural real algebraic maps of non-positive codimensions with prescribed images whose boundaries consist of non-singular real algebraic hypersurfaces satisfying transversality}, submitted to a refereed journal and a previous version of the present paper, arXiv:2303.10723v4.
...
\bibitem{kitazawa9} N. Kitazawa, \textsl{Some remarks on real algebraic maps which are topologically special generic maps}, arXiv:2312.10646. 
