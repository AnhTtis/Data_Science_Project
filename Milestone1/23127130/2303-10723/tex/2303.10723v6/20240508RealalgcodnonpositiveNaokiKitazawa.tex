...
\maketitle
\begin{abstract}
We present new real algebraic maps of non-positive codimensions with prescribed images whose boundaries consist of explicit non-singular real algebraic hypersurfaces satisfying so-called "transversality" as follows. Explicit information on important real polynomials is given. Preimages are  one-point sets or products of spheres. They are locally like so-called {\it moment} maps. 
...
Our method applies some singularity theory of smooth maps, some theory on differential topology of manifolds, and some elementary real algebraic arguments. Our method is also interdisciplinary. Our study also adds to our related pioneering studies \cite{kitazawa3, kitazawa7}. \cite{bodinpopescupampusorea, kohnpieneranestadrydellshapirosinnsoreatelen, 
	sorea1, sorea2}, which are studies on domains formed by real algebraic curves, also motivate us.
We also investigate functions represented as compositions of our new maps with the canonical projections.
...
\setcounter{enumi}{3}
	...
	\item Let $\{j_i\}_{i=1}^{l_3} \subset {\mathbb{N}}_l$ be an increasing sequence of integers {\rm (}$1 \leq l_3 \leq l_1${\rm )}. Let $p$ be a point of the set ${\bigcap}_{i=1}^{l_3} S_{j_i} \bigcap \overline{D}$ such that for any increasing sequence $\{{j^{\prime}}_i\}_{i=1}^{l_3+1}$ containing the increasing sequence $\{j_i\}_{i=1}^{l_3}$ as a subsequence, it holds that $p \notin {\bigcap}_{i=1}^{l_3+1} S_{{j^{\prime}}_i}$. Then it is diffeomorphic to ${\prod}_{j \in {\mathbb{N}}_{l_2}-\{m_{l_1,l_2}(j_i)\}_{i=1}^{l_3}} S^{m_{l_2}(j)}$ if ${\mathbb{N}}_{l_2}-\{m_{l_1,l_2}(j_i)\}_{i=1}^{l_3}$ is not empty and the one-point set if ${\mathbb{N}}_{l_2}-\{m_{l_1,l_2}(j_i)\}_{i=1}^{l_3}$ is empty.
\end{enumerate}
\end{Thm}
...
We can know the following easily by the construction and local structures of the functions and the maps and an argument presented in STEP 1-2 and announced to be essential. 
...
Note that \cite{kitazawa3} presents a specific case for Main Theorem \ref{mthm:1}. These hypersurfaces $S_j$ do not intersect and $l_2=1$ there.
...
We note that for Morse-Bott functions, see \cite{bott} for example. Some Morse-Bott functions are generalized to the presented maps as higher dimensional versions. 
...
\section{Applications of Main Theorem \ref{mthm:1} and Theorems \ref{thm:1} and \ref{thm:2} and functions represented as the compositions of maps obtained through (arguments in our proof of) them.}
...
\subsection{Functions represented as the compositions of maps obtained through Main Theorem \ref{mthm:1} and Theorems \ref{thm:1} and \ref{thm:2}.}
...
\item $f_0$ is a Morse-Bott function. In the case $l_2=1$, we can have our function $f_0$ as a Morse function.
\end{enumerate}
...
By the presented arguments, the singular set $S(f_0)$ is represented as a subset of $f^{-1}({\sqcup}_{j=1}^{l_1} \{(p_{j,1}-r_j,p_{j,2}),(p_{j,1}+r_j,p_{j,2})\})$.
...
We remember the notation $y=(y_1,\cdots y_{l_2})$. For $i \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_0)\}$, the component $y_i$ is mapped onto the component $\frac{1}{\sqrt{{\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)} (f_{j}(x_1,x_2))}} y_i \in S^{m_2(i)}$ at $(x,y)$. The component $x$ is mapped to $x$ itself and the component $y_{m_{l_1,l_2}(j_0)}$ is mapped to $y_{m_{l_1,l_2}(j_0)}$ itself. This map also gives a diffeomorphism.
...
\end{proof}
We present  Main Theorem \ref{mthm:2}. This gives some fundamental families of real algebraic functions enjoying the following properties.
\begin{itemize}
\item They are obtained as compositions of real algebraic maps into ${\mathbb{R}}^2$ in Main Theorem \ref{mthm:1} and Theorems \ref{thm:1} and \ref{thm:2} with ${\pi}_{2,1}$.
...
\item The Reeb graphs of the functions collapse to given graphs.
\end{itemize}
...
Hereafter, we respect the case each $S_j$ is a circle unless otherwise stated. We use the notation for "the set $S_j$" instead of that for "the polynomial $f_j$" in such a case.
\begin{MainThm}
\label{mthm:2}
...
\item \label{mthm:2.4}
For any two distinct cases such that the values of the maps $l_{\{e_j\}_{j=1}^{n_e}}:=l_{\{e_j\}_{j=1}^{n_e},1}$ and $l_{\{e_j\}_{j=1}^{n_e}}:=l_{\{e_j\}_{j=1}^{n_e},2}$ are same except at exactly one number of ${\mathbb{N}}_{n_e}$, the Reeb graphs $W_{f_{0,l_{\{e_j\}_{j=1}^{n_e},1}}}$ and $W_{f_{0,l_{\{e_j\}_{j=1}^{n_e},2}}}$ are not isomorphic.
...
Furthermore, take the preimage $P_{e_{i}}:={q_{f_{0,(D_0, \{S_{0,j}\}_{j=1}^{l_{0,1}},m_{l_{0,1},l_{0,2}},m_{l_{0,2}})}}}^{-1}(e_{i}) \subset M_{(D_0, \{S_{0,j}\}_{j=1}^{l_{0,1}},m_{l_{0,1},l_{0,2}},m_{l_{0,2}})}$, the image $f_{0,(D_0, \{S_{0,j}\}_{j=1}^{l_{0,1}},m_{l_{0,1},l_{0,2}},m_{l_{0,2}})}(P_{e_i})$ and the intersection \\ $f_{0,(D_0, \{S_{0,j}\}_{j=1}^{l_{0,1}},m_{l_{0,1},l_{0,2}},m_{l_{0,2}})}(P_{e_i}) \bigcap {\sqcup}_{j=1}^{l_{0,1}} S_j$. The intersection is not empty and it is a disjoint union of two curves diffeomorphic to $\mathbb{R}$. We choose one connected curve $L_i$ from these curves.
The circle $S_{l_{0,1}+{\Sigma}_{j^{\prime}=1}^{i-1} l_{\{e_j\}_{j=1}^{n_e}}(j^{\prime})+j^{\prime \prime}}$ is centered at a point in $L_i$ for $1 \leq j^{\prime \prime} \leq l_{\{e_j\}_{j=1}^{n_e}}(i)$.
...
\item \label{mthm:2.7} We can apply Main Theorem \ref{mthm:1} with Theorems \ref{thm:1} and \ref{thm:2} and our proof to have our new map $f_{l_{\{e_j\}_{j=1}^{n_e}}}:M_{l_{\{e_j\}_{j=1}^{n_e}}} \rightarrow {\mathbb{R}}^2$. More explicitly we set as follows. We put $l_2:=2$. We set $m_{l_{\{e_j\}_{j=1}^{n_e}},l_1,l_2}$ as a function onto the set $\{1,2\}$ and $m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}$ as a function with the following conditions.
\begin{enumerate}
...
\item \label{mthm:2.9} Each circle $S_{j,0}$ in $\{S_{j,0}\}_{j=1}^{l_{0,1}}$ contains exactly two vertical poles $v_{S_{j,0},1}$ and $v_{S_{j,0},2}$ of it satisfying $v_{S_{j,0},1}, v_{S_{j,0},2} \in \overline{D}$. %These vertical poles are different from the points discussed in (\ref{mthm:3.9}). 
The values of the first components of these vertical poles are always distinct from those of points discussed in (\ref{mthm:2.8}): the vertical poles $v_{S_{l_{0,1}+j}}$ and points $p_{S_{l_{0,1}+j},1}$ and $p_{S_{l_{0,1}+j},2}$ contained in exactly two distinct circles in $\{S_j\}_{j=1}^{l_1}$.
\end{enumerate}
...
We construct our map $f_{l_{\{e_j\}_{j=1}^{n_e}}}:M_{l_{\{e_j\}_{j=1}^{n_e}}} \rightarrow {\mathbb{R}}^2$ in (\ref{mthm:2.7}). We can check the first three properties (\ref{mthm:2.1}) here easily from the construction. 
...
We remember the notation $y=(y_1,\cdots y_{l_2})$. For $i \in {\mathbb{N}_{l_2}}-\{m_{l_1,l_2}(j_{0,1}),m_{l_1,l_2}(j_{0,2})\}$, the component $y_i$ is mapped onto the component $\frac{1}{\sqrt{{\prod}_{j \in {m_{l_1,l_2}}^{-1}(i)} (f_{j}(x_1,x_2))}} y_i \in S^{m_2(i)}$ at $(x,y)$. The component $x$ is mapped to $x$ itself and the component $y_{m_{l_1,l_2}(j_{0,a})}$ is mapped to $y_{m_{l_1,l_2}(j_{0,a})}$ itself for $a=1,2$. This map also gives a diffeomorphism.
...
This complete the proof of the property (\ref{mthm:2.2}).

We show the properties (\ref{mthm:2.3}, \ref{mthm:2.4}). We also remember the properties (\ref{mthm:2.7}, \ref{mthm:2.8}, \ref{mthm:2.9}).
We investigate the preimage ${f_{0,l_{\{e_j\}_{j=1}^{n_e}}}}^{-1}(t)$ containing no singular points of the function $f_{0,l_{\{e_j\}_{j=1}^{n_e}}}$. We investigate the preimage ${{\pi}_{2,1}}^{-1}(t)$, which is the straight line $L_t \subset {\mathbb{R}}^2$ represented as the zero set of the real polynomial $x_1-t$ ($(x_1,x_2) \in {\mathbb{R}}^2$). We investigate each connected component $L_{e,t}$ of the intersection of the straight line $L_t$ and $f_{l_{\{e_j\}_{j=1}^{n_e}}}(M_{l_{\{e_j\}_{j=1}^{n_e}}})$. The interior ${\rm Int}\ L_{e,t}$ is in the interior ${\rm Int}\ f_{l_{\{e_j\}_{j=1}^{n_e}}}(M_{l_{\{e_j\}_{j=1}^{n_e}}})$ of the image and the boundary consists of exactly two points $p_{e,1}$ and  $p_{e,2}$ contained in some circle $S_j$. On this, either of the following holds.


\begin{itemize}
\item These points $p_{e,1}$ and $p_{e,2}$ are in circles represented as $S_{0,j}=S_j$ ($1 \leq j \leq l_{0,1}$). The preimage ${f_{l_{\{e_j\}_{j=1}^{n_e}}}}^{-1}(L_{e,t})$ is diffeomorphic to $S^{m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}(1)+1} \times S^{m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}(2)}$. This is also regarded as a manifold diffeomorphic to one obtained by gluing two copies of $D^{m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}(1)+1} \times S^{m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}(2)}$ along the boundaries  in a canonical way. In other words, we take a double of the copy here.
...
These preimages are also connected components of the preimage ${f_{0,l_{\{e_j\}_{j=1}^{n_e}}}}^{-1}(t)$. See also Remark \ref{rem:1} for a kind of counterexamples.


We easily have the property (\ref{mthm:2.3}).
...
\begin{figure}
	...
	\includegraphics[height=75mm, width=100mm]{20240503MT2_2.eps}

	\caption{An example for Main Theorem \ref{mthm:2} following FIGURE \ref{fig:1}. 
The image of a moment-like map $f_{(D_{l_{\{e_j\}_{j=1}^{n_e}}},\{S_{l_{\{e_j\}_{j=1}^{n_e}},j}\}_{j=1}^{l_1},m_{l_{\{e_j\}_{j=1}^{n_e}},l_1,l_2},m_{l_{\{e_j\}_{j=1}^{n_e}},l_2})}$ into ${\mathbb{R}}^2$ for a region $D_{l_{\{e_j\}_{j=1}^{n_e}}}$ surrounded by circles and colored in gray: here FIGURE \ref{fig:1} is respected with $(l_{0,1},l_{0,2})=(3,1)$, $l_1=5$, $l_2=2$, $n_e=5$, and $l_{\{e_j\}_{j=1}^{n_e}}$ is a function whose value is $2$ at an edge and whose values are $0$ at the remaining edges. The Reeb graph of the function $f_{0,(D_{l_{\{e_j\}_{j=1}^{n_e}}},\{S_{l_{\{e_j\}_{j=1}^{n_e}},j}\}_{j=1}^{l_1},m_{l_{\{e_j\}_{j=1}^{n_e}},l_1,l_2},m_{l_{\{e_j\}_{j=1}^{n_e}},l_2})}:={\pi}_{2,1} \circ f_{(D_{l_{\{e_j\}_{j=1}^{n_e}}},\{S_{l_{\{e_j\}_{j=1}^{n_e}},j}\}_{j=1}^{l_1},m_{l_{\{e_j\}_{j=1}^{n_e}},l_1,l_2},m_{l_{\{e_j\}_{j=1}^{n_e}},l_2})}$. Black dots are for singular points of the function.}
	\label{fig:2}
\end{figure}
...
Then, for each edge $e_j$ and an arbitrary integer $m_{e_i}>{\Sigma}_{j=1}^{l_{1,2}} (m_{e_i,l_{1,2}}(j))+2$, there exist a non-singular real algebraic manifold $M_{e_i}$ which is also a closed and connected manifold and whose dimension is $m_{e_i}$ and a smooth real algebraic map $f_{e_i}:M_{e_i} \rightarrow {\mathbb{R}}^2$ with the following properties. 
\begin{enumerate}
\item \label{mthm:3.1} The map $f:=f_{e_i}$ is  the moment-like map reconstructed from some data $(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2}):=(D_{e_i,1},\{S_{e_i,j}\}_{j=1}^{l_1},m_{e_i,l_1,l_2},m_{e_i,l_{2}})$ by choosing each $S_{e_i,j}$ suitably satisfying the following conditions.
\begin{enumerate}
...
\item \label{mthm:3.3} The Reeb graph $W_{f_{0,e_i}}$ is isomorphic to the following graph obtained in the following way.
\begin{enumerate}
\item Choose the edge $e_i$ of $W_{f_{0,(D_1, \{S_{1,j}\}_{j=1}^{l_{1,1}},m_{l_{1,1},l_{1,2}},m_{l_{1,2}})}}$ and two new vertices $v_{e_i,1}$ and $v_{e_i,2}$ in the interior of $e_i$.
\item We add an edge to the previous new graph which contains $v_{e_i,1}$ or $v_{e_i,2}$ as a vertex and another newly added vertex. This graph is our new graph collapsing to the Reeb graph $W_{f_{0,(D_1, \{S_{1,j}\}_{j=1}^{l_{1,1}},m_{l_{1,1},l_{1,2}},m_{l_{1,2}})}}$. We remember that the number of edges is greater than that of the original graph by $2$ and that the number of vertices is greater than that of the original graph by $3$.
\end{enumerate} 
\end{enumerate}
\end{MainThm}
\begin{proof}[A sketch of the proof]
Most of important arguments is discussed in the proof of Main Theorem \ref{mthm:2}.
...
We can abuse most of the notation. We can have our map like one presented through (\ref{mthm:2.5})--(\ref{mthm:2.9}).
...
\begin{itemize}
\item These points $p_{e,1}$ and $p_{e,2}$ are in circles represented as $S_{j_1}$ and $S_{j_2}$. 
Suppose that the relation $m_{e_i,l_1,l_2}(j_1)=m_{e_i,l_1,l_2}(j_2)$ holds.
...
\item These points $p_{e,1}$ and $p_{e,2}$ are in circles represented as $S_{j_1}$ and $S_{j_2}$. 
Suppose that the relation $m_{e_i,l_1,l_2}(m_{e_i,l_1,l_2}(j_1)) \neq m_{e_i,l_1,l_2}(m_{e_i,l_1,l_2}(j_2))$ holds.
...
These preimages are also connected components of the preimage ${f_{0,l_{e_i}}}^{-1}(t)$. 
\end{proof}
...
\begin{figure}
	...
	\caption{An example for Main Theorem \ref{mthm:3} following FIGURE \ref{fig:3}. The image of a moment-like map $f_{(D_{e_i},\{S_{e_i,j}\}_{j=1}^{l_1},m_{e_i,l_1,l_2},m_{e_i,l_2})}$ into ${\mathbb{R}}^2$ for a region $D=D_{e_i}$ surrounded by circles and colored in gray: here FIGURE \ref{fig:3} is respected and $(l_{1,1},l_1,l_2)=(2,3,l_{1,2}+1)$ ($l_{1,2}=2$). The Reeb graph of the function $f_{0,(D_{e_i},\{S_{e_i,j}\}_{j=1}^{l_1},m_{e_i,l_1,l_2},m_{e_i,l_2})}:={\pi}_{2,1} \circ f_{(D,\{S_j\}_{j=1}^{l_1},m_{e_i,l_1,l_2},m_{e_i,l_2})}$. Black dots are for singular points of the function.}
	\label{fig:4}
\end{figure}
...
\begin{figure}
	...
	\caption{An example for Remark \ref{rem:1}. This follows FIGURE \ref{fig:1}. This shows a graph which is a bit different from that of FIGURE \ref{fig:2}. Here we replace a condition on the value $m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}(2)$ by $m_{l_{\{e_j\}_{j=1}^{n_e}},l_2}(2)=0$.}
	\label{fig:5}
\end{figure}
...
\begin{Rem}
\label{rem:4}
We review some important arguments from \cite{kitazawa3, kitazawa9}. Prepare a finite and connected graph $G$ enjoying the following properties.
\begin{itemize}
...
\end{itemize}
Then we have a natural moment-like map $f_{(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}:M_{(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})} \rightarrow {\mathbb{R}}^2$ with the following properties.
\begin{enumerate}
\item We can know the existence of the data $(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})$ such that  the non-singular real algebraic curves $S_j$ are mutually disjoint and that the graph $e_G(G)$ is regarded as a Poincar\'e-Reeb graph of $D_G$ .
\item The Reeb graph $W_{f_{0,(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}}=W_{{\pi}_{2,1} \circ f_{(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}}$ of the function ${\pi}_{2,1} \circ f_{(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}= f_{0,(D_G,\{S_{j}\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}$ is isomorphic to $G$. We can have this function as a Morse function. We can show that this is a Morse function by applying (a slight generalization of) Proposition \ref{prop:1}.
\end{enumerate}
Note that the existence of a region $D_G$ for the graphs $G$ and $e_G(G)$ is thanks to \cite{bodinpopescupampusorea}. The hypersurfaces $S_j$ are obtained by approximation of Weierstrass type respecting derivatives.
...
\begin{Ex}
\label{ex:2}
We first give an explicit graph $G$ and its embedding $e_G$  in Remark \ref{rem:3}. This is shown in blue in FIGURE \ref{fig:6}.
FIGURE \ref{fig:6} shows the image of a moment-like map $f_{(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}$ into ${\mathbb{R}}^2$ reconstructed from some data $(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})$. Here the region $D$ is a connected open set of ${\mathbb{R}}^2$ surrounded by three circles. The Reeb graph of the function $f_{0,(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}$ is isomorphic to the graph $G$.
For a method to find a region $D_G$ whose Poincar\'e-Reeb graph is $G$ ($e_G(G)$), see the original article \cite{bodinpopescupampusorea}.
Note also that this shows an example which is not from Main Theorem \ref{mthm:2} or \ref{mthm:3}: see the four points for singular points of the function $f_{0,(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}$. 
\end{Ex}
\begin{figure}
	
	\includegraphics[height=75mm, width=100mm]{Ex2.2.eps}

	\caption{Example \ref{ex:2}. The image of a moment-like map $f_{(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}$ into ${\mathbb{R}}^2$ reconstructed from the data $(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})$ where a region $D$ is surrounded by circles and colored in gray. The Reeb graph of the function $f_{0,(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}:={\pi}_{2,1} \circ f_{(D,\{S_j\}_{j=1}^{l_1},m_{l_1,l_2},m_{l_2})}$, isomorphic to the blue colored graph $G$ ($e_G(G)$). Black dots are for singular points of the function. Note that this shows a case which is not for Main Theorem \ref{mthm:2} or Main Theorem \ref{mthm:3}.}
	\label{fig:6}
\end{figure}
We present a related problem.
\begin{Prob}
Given a graph $G$ (with an embedding $e_G$) in Remark \ref{rem:3}. 
\begin{enumerate}

\item Can we define meaningful classes of  natural moment-like maps $f_G$ such that the Reeb graphs of ${\pi}_{2,1} \circ  f_G$ are isomorphic to $G$ and $e_G(G)$? Can we classify such maps of certain classes we define?
\item What is the most natural moment-like map $f_G$ such that the Reeb graph of ${\pi}_{2,1} \circ  f_G$ is isomorphic to $G$ and $e_G(G)$? Can we define (or formulate problems of defining) such maps under certain conditions.
\end{enumerate}
\end{Prob}
...
\bibitem{kitazawa8} N. Kitazawa, \textsl{Natural real algebraic maps of non-positive codimensions with prescribed images whose boundaries consist of non-singular real algebraic hypersurfaces satisfying transversality}, submitted to a refereed journal and a previous version of the present paper, a previous version of the present paper, arXiv:2303.10723v5.
