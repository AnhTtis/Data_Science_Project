...
Consider overlap $k  \in \{ 1, \dots, n\}$ and $m \in \{ 1,\dots, n-1\}$. Let $k' \equiv k +m \text{ (mod $n$)}$. Then, from optimality of $u^k$ at overlap $k$, 
...
\begin{lem}
\label{lem:nn4}
Suppose that there exist $\lambda \in\mathbb{R}^n \setminus \{\mathbf{0}\}$ and two points $u$, $\tilde{u} \in V$ that satisfy Condition 1 and 2 in \autoref{lem:nn3}. Then, for any $\Delta \in (0,1)$, $W^*_\lambda (\Delta )>\lambda \cdot u$ holds. 
\end{lem}
\begin{proof}
...
Observe that \autoref{lem:nn3} and \autoref{lem:nn4} imply the following: $V\neq F^*$ holds if and only if for any $\Delta  \in (0,1)$, $F(\Delta )\neq V$ holds.	
