...
The monotonicity result of the feasible payoff set can also shed light on the equilibrium (Nash equilibrium or subgame perfect equilibrium) payoff set. It is almost immediate from our result that the set of ``stationary'' equilibrium (i.e., each generation of a player employs the same strategy) payoffs is, in general, not increasing in $\delta$. Although obvious, we leave it as a proposition.\footnote{This contrasts from the case of repeated games with infinitely-lived players with a PRD \cite[]{APS_1990_ECMA} where the subgame perfect equilibrium payoff set monotonely increasing in players' discount factor. When there is no PRD, this monotonicity might not hold (see \cite{MOS_2002_GEB,Yamamoto_2010_IJGT}).} 



\begin{prop}
Consider an OLG repeated game, where the stage game is $G$. Suppose that any extreme point of $V$ is a payoff vector of a static Nash equilibrium of $G$. Then the stationary subgame perfect equilibrium payoff set is non-increasing in $\delta$ and non-decreasing in $T$. 
\end{prop}
