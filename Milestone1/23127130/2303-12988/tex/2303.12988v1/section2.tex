...
A stage game $G=(N,(A_i)_i,(u_i)_i)$ is defined as follows. $N=\{ 1,2,\cdots ,n\}$ for $n\geq 2$ is a set of one-shot players. Each player $i\in N$ is endowed with a finite set $A_i$ of available pure actions.\footnote{As long as a Public Randomizing Device is available, our result by mixed actions is the same with the one by pure actions. Therefore, we only consider pure actions throughout this paper.} Let $A \equiv \prod_{i \in N} A_i$. Player $i$'s one-shot payoff function is given by $u_i: A \to \mathbb{R}$.  
...
Given a stage game $G$ defined above, $\delta \in (0,1]$ and $T \in \mathbb{N}$, we define the OLG game $OLG(G,\delta ,T)$ as follows (also see \autoref{o1}):
\begin{itemize}
...
\caption{Structure of OLG game with $n=3$}
\label{o1}
\end{table}
...
We assume that players can access a Public Randomizing Device (henceforth PRD) uniformly distributed over the unit interval at the beginning of every period.\footnote{This assumption is employed also by \cite{Chen_2007_EL} and \cite{CF_2013_IJGT}.} As a monitoring structure, we assume at least that each player can observe players' past pure actions and realization of the PRD at each period after her/his birth.
