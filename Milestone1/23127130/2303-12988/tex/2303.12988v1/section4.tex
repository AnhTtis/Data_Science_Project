...
In this section, we study the comparative statics of the feasible set w.r.t. both $T$ and $\delta$. From the result in the previous section, we know that the feasible payoff set only depends on $\delta^T$. Our second main result says that the feasible payoff set is non-increasing in $\delta^T$, implying, perhaps surprisingly, it is \emph{non-increasing} in $\delta$.
...
The following hold:
\begin{enumerate}
	\item For any $\delta, \delta'$ with $0< \delta <\delta ' \leq 1$, $F(\delta ',\ T)\subset F(\delta ,\ T)$.
\item Given any $\delta \in (0,1]$, $F(\delta ,\ T)\subset F(\delta ,\ T+1)$.	
\end{enumerate} \end{them}

The rest of this section devotes to proving \autoref{them:2}. In doing so, we shall also provide a characterization of players' optimal play to maximize the welfare given some weights for players, which might have some independent interest. 
