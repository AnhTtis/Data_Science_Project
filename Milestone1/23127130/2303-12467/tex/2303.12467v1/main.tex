...
Primordial galaxies evolves into the various types of galaxies we see today involving complex interplay between different processes: accretion of cold gas, minor and major mergers, stellar and active-galactic-nuclei feedback.
...
However, whether primordial galaxies can form stable disk is still debatable, as low-mass galaxies may not maintain stable disk which can be easily disrupted by inflow or outflow.
Simulations of \cite{Dekel+20} suggest a critical dark matter (DM) halo mass of $M_\mathrm{h}<2\times10^{11}~\mathrm{M_\odot}$, below which disk cannot survive and the galaxy becomes dispersion-dominated.
...
The paper is structured as follows. Section \ref{data} explains our observations and dataset. Section \ref{results} describes how we derive rotation velocity, velocity dispersion and gas mass fraction. We discuss and summarize our findings in Sections \ref{discuss} and \ref{sum}, respectively. Throughout the paper we adopt a cosmological model with $H_0=70~\mathrm{km~s^{-1}~Mpc^{-1}}$, $\Omega_\Lambda =0.7$, and $\Omega_\mathrm{m} = 0.3$.
...
There were thin clouds in the beginning of observations on April 21st.
...
From 5 EMPGs which are J0057-0941, J2104-0035, J2115-1734, DDO68, and IZw18, we extract 2 components.
For J0125+0759, we extract 3 components.
We label the components with suffices, where the brightest one is labeled with \#1.
For IZw18, we separate IZw18-NW and IZw18-SE.
For DDO68, we use the labels of \#2 and \#3 in consistency with the notations in previous studies \citep[e.g.,][]{Pustilnik+05}.
...
The value of $r_\mathrm{e}$ is derived by fitting rotation disk models for the 15 spatially-resolved EMPGs in Section \ref{kinematics}.
We fit a Sersic profile to the H$\alpha$ flux maps of the other 18($=33-15$) EMPGs to derive the effective radii. 
...
As a comparison, dwarf galaxies studied in \cite{Wheeler+17} and \cite{Barat+20} have average $\voversigma$ values of $0.50$ and $0.84$, respectively.
It is curious whether the correlation between $\voversigma$ and $M_*$ are still present for young galaxies like EMPGs.
...
For the 6 galaxies (HS0822+3542, DDO68-\#2, -\#3, LeoP, J1452+0241, and J1631+4426) with $M_* < 10^6~M_\odot$, we find only weak velocity gradient in the velocity maps shown in Figure \ref{fig:maps}, consistent with the conclusion that they may not present any rotation.
For local group dwarfs with $10^{3.5}~\mathrm{M_\odot}<M_*<10^8~\mathrm{M_\odot}$, \cite{Wheeler+17} study the stellar kinematics and find no clear correlation between $\voversigma$ and $M_*$.
\cite{delosReyes+23} also obtain low $\voversigma$ based on the stellar kinematics of local dwarf galaxies and suggest a positive correlation that is independent of environmental effect.
We note that our EMPGs are different from the dwarf galaxies investigated by \cite{Wheeler+17} and \cite{delosReyes+23} as EMPGs are young systems that probably undergo the first star formation activity.
...
Whether stable rotation disk can build up in low-mass star-forming galaxies like EMPGs needs to be tested with simulations.
...
However, it is still a difficult task to resolve the profile of mass concentration \citep[e.g.,][]{Genzel+20} for low-mass young galaxies by observations.
...
We find Leo P may be relatively gas-deficient, where the stellar mass contributes more to the dynamical mass than gas or DM mass in the small radius.
\cite{Bernstein-Cooper+14} conduct H{\sc i} observations and obtain $\fgas\sim0.7$ for Leo P.
They claim the $\fgas$ value is relatively small among EMPGs, in consistent with our results.
...
To compare with previous kinematics studies, we calculate the average of Q within a disk that is called the global Q \citep[e.g.,][]{Aumer+10} from the equation:
\begin{equation}
...
It is possible that EMPGs have a large variety of dynamics, or the global Q does not provide a good constrain on the gravitational stability of EMPGs.
It is difficult to conclude whether or not EMPGs generally have stable disk without high-resolution observations for neutral hydrogen, ionized hydrogen, and stellar component \citep[e.g.,][]{Romeo20,Romeo+20}.
...
Y.H. is supported by JSPS KAKENHI Grant Nos. 21J00153, 20K14532, 21H04499, 21K03614, and 22H01259.
...
\software{FOCAS IFU pipeline \citep{Ozaki+20}, PyRAF \citep{IRAF}, Photutils \citep{photutils},  Astropy \citep{astropy:2013,astropy:2018,astropy:2022}, GalPaK$^\mathrm{3D}$ \citep{Bouche+15}}
