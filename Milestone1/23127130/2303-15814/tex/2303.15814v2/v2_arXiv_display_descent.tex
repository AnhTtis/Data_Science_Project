...
The category of (bounded) prisms with flat topology is called the absolute prismatic site.
...
$\mathcal{P}$
is \textit{of type} $\mu$
if, $(p, I)$-completely flat locally on $A$,
...
\]
of
\textit{$G$-$\mu$-displays over a bounded prism $(A, I)$}.
It turns out that
$G\mathchar`-\mathrm{Disp}_\mu(A, I)$
...
We also introduce the \textit{Hodge filtration} and the \textit{underlying $\phi$-$G$-torsor} of a prismatic $G$-$\mu$-display.
...
\end{rem}
    ...
    We will also discuss (in the case where $\O_E=\Z_p$) the relation between prismatic $G$-$\mu$-displays and prismatic $F$-gauges in vector bundles introduced by Drinfeld and Bhatt--Lurie.
    In particular, for a quasisyntomic ring $S$ over $W(k)$, we introduce the groupoid
    \[
...
We consider the pair $(W[[t]]/t^n, (\mathcal{E}))$, which is naturally a bounded prism for every $n \geq 1$.
In \cite{Lau14},
...
In Section \ref{Section:prismatic G display}, we introduce prismatic $G$-$\mu$-displays, give some basic definitions (e.g.\ Hodge filtrations and underlying $\phi$-$G$-torsors), and establish several descent results.
...
We refer to \cite[Notation 1.2]{BS} and \cite[Tag 091N]{SP} for the notion of derived complete modules (or complexes).
...
\begin{rem}\label{Remark:flat locally orientable}
    ...
    Indeed, by \cite[Lemma 3.4]{Marks}, there exists a faithfully flat homomorphism $A \to A'$ of $\delta_E$-rings such that $IA'$ is principal.
...
%This quadruple is a \textit{frame} in the sense of Zink and Lau; see for example \cite[Section 2]{Lau10} for the notion of frames.
...
%$\Phi_i \colon \Fil^i(\phi^*M) \to \phi^*M$ are the $\phi$-linear homomorphisms defined by
%$x \mapsto 1 \otimes (d^{-i}F_M(x))$.
%This construction gives an equivalence between the category of displayed Breuil--Kisin modules over $(A, (d))$ and the category of displays over the frame $(A, (d), \phi, \phi_1)$.
...
\begin{proof}
    ...
    Then the isomorphism $F_M$ restricts to $\Fil^m(\phi^*M) \overset{\sim}{\to} M \otimes_A I^{m}$.
    ...
    \[
        \Fil^i(\phi^*M)= \phi^*M \cap (\Fil^m(\phi^*M) \otimes_A I^{i-m})
    \]
...
\]
holds
in $(\phi^*(M_{A'}))[1/I']$ for any $i$.
This follows from the following chain of equalities:
\begin{equation*}
\begin{split}
\Fil^i(\phi^*(M_{A'})) &= \phi^*(M_{A'}) \cap (\Fil^m(\phi^*(M_{A'})) \otimes_{A'} I'^{i-m}) \\
&=\phi^*(M_{A'}) \cap (\Fil^m(\phi^*M) \otimes_{A} I'^{i-m}) \\
&= \phi^*(M_{A'}) \cap (\bigoplus_{j \leq m}  (L'_j  \otimes_{A'} I'^{{i-j}})) \\
&= (\bigoplus_{j \geq i} L'_j) \oplus (\bigoplus_{j < i} I'^{i-j}L'_j),
\end{split}
\end{equation*}
where the first two equalities follow from the above remark,
and for the third one, we use the fact that $L_j=0$ for $j \geq m+1$, which follows from Lemma \ref{Lemma:Brauil-Kisin module, filtration and height} and Nakayama's lemma.

(2) We note that $\Fil^i(\phi^*(M_{A'}))$ is a finite projective $A'$-module for any $i$ by Lemma \ref{Lemma:normal decomposition for displayed BK module}.
By Proposition \ref{Proposition:flat descent for finite projective modules}, 
...
We see that $F_M$ restricts to $\Fil^m \overset{\sim}{\to} M \otimes_A I^{m}$, and hence $\Fil^m=\Fil^m(\phi^*M)$.
...
we can conclude that $\Fil^i=\Fil^i(\phi^*M)$ for any $i$ by Remark \ref{Remark:filtration of Breuil-Kisin module}.
...
%any Breuil--Kisin module over $(A, I)$ of type $\mu$ arises from a prismatic $\GL_n$-$\mu$-display.
...
Let $h_d \colon A^n \overset{\sim}{\to} M$ denote the composition
\[
A^n \overset{\sim}{\to} \Fil^{m_1}_\mu \overset{\sim}{\to} \Fil^{m_1}(\phi^*M) \overset{\sim}{\to} M
\]
where the first isomorphism is given by $d^{m_1}\mu(d)^{-1}$, the second one is induced by $h$, and the third one is defined by $x \mapsto d^{-m_1}F_M(x)$.
We define $\alpha(h)_d \in \GL_n(A)$ to be the invertible matrix corresponding to the following composition:
\[
A^n = \phi^*(A^n) \overset{\phi^*h_d}{\to} \phi^*M \overset{h^{-1}}{\to} A^n.
\]
Then $h_d \colon A^n \overset{\sim}{\to} M$ gives an isomorphism $(A^n, F_{\alpha(h)_d}) \overset{\sim}{\to} M$ of Breuil--Kisin modules.
\end{proof}
...
Then we may identify $G^{\geq m}_\mu(A, I)$ with
\[
((\bigoplus_{1 \leq i \leq m} \mathfrak{g}_i \otimes_\O I^m)  \oplus  (\bigoplus_{i \geq m+1} \mathfrak{g}_i \otimes_\O I^i))\times P^{\geq m}_\mu(A)
\]
by
Remark \ref{Remark:Umu identify} (1)
and
Proposition \ref{Proposition:decomposition of display group}, where
...
$G^{\geq m}_\mu(A, I)/G^{\geq m+1}_\mu(A, I)=(\bigoplus_{i \leq m} \mathfrak{g}_i)
\otimes_\O I^m/I^{m+1}$.
\end{proof}
...
a functor
\[
X_{\Prism, A} \colon (A, I)_\Prism \to \mathrm{Set},\quad (B, IB) \mapsto X(B).
\]
...
a sheaf
\[
X_{\overline{\Prism}, A} \colon (A, I)_\Prism \to \mathrm{Set}, \quad (B, IB) \mapsto X(B/IB).
\]
...
which associates to each $(B, IB) \in (A, I)_\Prism$ the groupoid of $H_{B/IB}$-torsors over $\Spec B/IB$
...
\]
is exact.
For the proof of this fact, see \cite[Lemma 2.3.9]{PappasRapoport21} for example.

(2) This can be proved in the same way as (1).
\end{proof}


We now assume that $(A, I)$ is orientable.
...
(2) 
We may assume that $\mathcal{Q}$ is a trivial $G_{\mu, A, I}$-torsor.
...
The same assertion holds for any $B \in (A, I)_\et$, and hence
...
The isomorphism $F_M$ induces
$
F_\mathcal{P} \colon (\phi^*\mathcal{P})[1/I] \overset{\sim}{\to} \mathcal{P}[1/I],
$
so that $(\mathcal{P}, F_\mathcal{P})$
is a $\GL_n$-Breuil--Kisin module over $(A, I)$.
...
if there exists a flat covering $(A, I) \to (B, J)$ such that $(B, J)$ is orientable, the base change $\mathcal{P}_B$ is a trivial $G_B$-torsor, and via some (and hence any) trivialization
    $\mathcal{P}_B \simeq G_B$,
    the isomorphism $F_\mathcal{P}$ is given by $g \mapsto Yg$ for an element $Y$ in the double coset
    \[
    G(B)\mu(d)G(B) \subset G(B[1/J])
    \]
    where $d \in J$ is a generator.
    If these conditions are satisfied for $(B, J)=(A, I)$, then we say that $(\mathcal{P}, F_\mathcal{P})$ is \textit{banal}.
\end{defn}
...
\begin{ex}\label{Example:GLn BK module of type mu}
    Assume that $G=\GL_n$ and
    let
    $\mu \colon \G_m \to \GL_{n, \O}$
    be a cocharacter.
    Let $M$ be a Breuil--Kisin module of rank $n$ over $(A, I)$ and
    let $\mathcal{P}$ be the associated $\GL_n$-Breuil--Kisin module over $(A, I)$; see Example \ref{Example:GLn BK module}.
    Then $M$ is of type $\mu$ in the sense of Definition \ref{Definition:type of displayed BK module} if and only if $\mathcal{P}$ is of type $\mu$.
    The ``if'' direction follows from Proposition \ref{Proposition:displayed condition base change}.
    Conversely, assume that $M$ is of type $\mu$.
    Then there exists a flat covering $(A, I) \to (B, J)$ such that $(B, J)$ is orientable and
    the base change $M_B$ is banal in the sense of Definition \ref{Definition:type of displayed BK module}.
    Now Lemma \ref{Lemma:banal BK type mu standard form} implies that $\mathcal{P}$ is of type $\mu$.

    Consequently, we have an equivalence
\[
\mathrm{BK}_\mu(A, I)^{\simeq} \overset{\sim}{\to} \GL_n\mathchar`-\mathrm{BK}_\mu(A, I),
\]
where $\mathrm{BK}_\mu(A, I)^{\simeq}$ is the groupoid of Breuil--Kisin modules of type $\mu$ over $(A, I)$.
If $(A, I)$ is orientable, then we have
\[
\mathrm{BK}_\mu(A, I)^{\simeq}_{\mathrm{banal}} \overset{\sim}{\to} \GL_n\mathchar`-\mathrm{BK}_\mu(A, I)_{\mathrm{banal}},
\]
where $\mathrm{BK}_\mu(A, I)^{\simeq}_{\mathrm{banal}}$ is the groupoid of banal Breuil--Kisin modules of type $\mu$ over $(A, I)$.
\end{ex}
...
\]
This is (non-canonically) isomorphic to $G_{\Prism, A}$, and hence forms a sheaf.
The sheaf $G_{\Prism, A, I}$ is equipped with a natural action of $G_{\mu, A, I}$.
(See Definition \ref{Definition:display group sheaf} for the group sheaves $G_{\Prism, A}$ and $G_{\mu, A, I}$.)
...
The functor
$(A, I)_\et \to (A', I')_\et$
sending $B \in (A, I)_\et$ to the $(\pi, I)$-adic completion $B'$ of $B \otimes_A A'$ induces
...
$
of group sheaves.
Moreover, we can extend the above map 
$f \colon G(A)_I \to G(A')_{I'}$ to a morphism
$
...
Let
$(A, I)_{\Prism, \ori}$
be the category of orientable and bounded $\O_E$-prisms $(B, J)$ with a map $(A, I) \to (B, J)$.
The groupoid
\[
G\mathchar`-\mathrm{Disp}_\mu(A, I):= {2-\varprojlim}_{(B, J) \in (A, I)_{\Prism, \ori}} G\mathchar`-\mathrm{Disp}_\mu(B, J)
\]
is called the \textit{groupoid of $G$-$\mu$-displays over $(A, I)$}.
A \textit{$G$-$\mu$-display} over $(A, I)$ is defined to be an object of this groupoid.
\end{defn}


A map $f \colon (A, I) \to (A', I')$ of bounded $\O_E$-prisms over $\O$ induces a natural functor
\[
...
In the following, we relate
$G$-$\mu$-displays to $G$-Breuil--Kisin modules of type $\mu$.
The following construction is also discussed in \cite[Section 5.2.2]{IKY}.

\begin{prop}\label{Proposition:functor from G-displays to G-BK}
    Let $(A, I)$ be a bounded $\O_E$-prism over $\O$.
...
\begin{proof}
    The natural functor
    \[
    G\mathchar`-\mathrm{BK}_\mu(A, I) \to {2-\varprojlim}_{(B, J) \in (A, I)_{\Prism, \ori}} G\mathchar`-\mathrm{BK}_\mu(B, J)
    \]
is an equivalence by Remark \ref{Remark:flat locally orientable} and Remark \ref{Remark:flat descent for G-BK modules}.
Thus it suffices to construct,
for an orientable and bounded $\O_E$-prism $(A, I)$ over $\O$,
a natural functor
$G\mathchar`-\mathrm{Disp}_\mu(A, I) \to G\mathchar`-\mathrm{BK}_\mu(A, I)$
which is compatible with base change.

To a $G$-$\mu$-display $\mathcal{Q}$ over $(A, I)$, we attach a $G$-Breuil--Kisin module $\mathcal{Q}_{\mathrm{BK}}$ of type $\mu$ over $(A, I)$ as follows.
Let $d \in I$ be a generator.
Let
    $
...
$G_{\mu, A, I} \hookrightarrow G_{\Prism, A}$ defined by $g \mapsto \mu(d)g\mu(d)^{-1}$.
By Proposition \ref{Proposition:equivalences of pi completely etale torsors},
we can regard $\mathcal{Q}_{\mathrm{BK}, d}$ as a $G_A$-torsor over $\Spec A$.
Let $d' \in I$ be another generator
and let $u \in A^\times$ be the unique element such that $d=ud'$.
We define
$\ad(u) \colon G_{A} \overset{\sim}{\to} G_{A}$
by $g \mapsto \mu(u)g\mu(u)^{-1}$.
The pushout $(\mathcal{Q}_{\mathrm{BK}, d'})^{\ad(u)}$ of $\mathcal{Q}_{\mathrm{BK}, d'}$ along
$\ad(u)$ can be identified with $\mathcal{Q}_{\mathrm{BK}, d}$.
The following composition
\[
\mathcal{Q}_{\mathrm{BK}, d'} \to (\mathcal{Q}_{\mathrm{BK}, d'})^{\ad(u)}=\mathcal{Q}_{\mathrm{BK}, d} \overset{x \mapsto x \cdot \mu(u)}{\longrightarrow}\mathcal{Q}_{\mathrm{BK}, d}
\]
is an isomorphism of $G_{A}$-torsors.
Then we define the $G_A$-torsor $\mathcal{Q}_{\mathrm{BK}}$ over $\Spec A$ by
\[
...
\[
F \colon (\phi^*\mathcal{Q}_{\mathrm{BK}})[1/I] \overset{\sim}{\to} \mathcal{Q}_{\mathrm{BK}}[1/I].
\]
Assume first that $\mathcal{Q}$ is banal.
We choose an isomorphism
$\mathcal{Q} \simeq \mathcal{Q}_X$
for some $X \in G(A)_I$.
Since
$(\mathcal{Q}_X)_{\mathrm{BK}, d}=G_A$
for a generator $d \in I$,
we may identify
$\mathcal{Q}_{\mathrm{BK}}$
with $G_A$ by
\[
\mathcal{Q}_{\mathrm{BK}} \simeq \mathcal{Q}_{\mathrm{BK}, d} \simeq (\mathcal{Q}_X)_{\mathrm{BK}, d}=G_A.
\]
Via this identification, we define the isomorphism $F$ by
\[
(\phi^*G_A)[1/I]=G_A[1/I] \overset{\sim}{\to} G_A[1/I], \quad g \mapsto (\mu(d)X_d)g.
\]
(Here $X_d \in G(A)=G(A)_d$ is the image of $X \in G(A)_I$.)
...
Since $\mathcal{Q}_{\mathrm{BK}}$ is affine and flat over $A$, it follows from Remark \ref{Remark:structure sheaf} that there exists a unique isomorphism $F$ whose base change to $B[1/IB]$ agrees with the one constructed above.
Moreover $F$ does not depend on the choice of $B$.
By construction, we see that $\mathcal{Q}_{\mathrm{BK}}=(\mathcal{Q}_{\mathrm{BK}}, F)$ is a $G$-Breuil--Kisin module of type $\mu$.

The construction
$\mathcal{Q} \mapsto \mathcal{Q}_{\mathrm{BK}}$
induces a functor 
$G\mathchar`-\mathrm{Disp}_\mu(A, I) \to G\mathchar`-\mathrm{BK}_\mu(A, I)$,
and it is clear that the functor is compatible with base change.
\end{proof}


We will see later that the functor $(\ref{equation:functor from G-displays to G-BK})$ is an equivalence; see Proposition \ref{Proposition:G-displays and G-BK modules} below.
Here we just prove the following.

\begin{lem}\label{Lemma:banal G displays and banal G-BK module of type mu}
...
For $X \in G(A)_I$, the trivial $G_A$-torsor $G_A$ with the isomorphism
\[
(\phi^*G_A)[1/I]=G_A[1/I] \overset{\sim}{\to} G_A[1/I], \quad g \mapsto (\mu(d)X_d)g.
\]
is a banal $G$-Breuil--Kisin module of type $\mu$ over $(A, I)$, which is denoted by $\mathcal{P}_{X_d}$.
By construction, we have
$(\mathcal{Q}_X)_{\mathrm{BK}} \overset{\sim}{\to} \mathcal{P}_{X_d}$, and similarly
...
We can prove the following flat descent result, which is an analogue of \cite[Lemma 5.4.2]{Lau21}.
...
We can define sheaves $G_{\Prism, A, I}$ and $G_{\mu, A, I}$ on the site $(A, I)^{\op}_{\Prism}$ in the same way, and $G_{\mu, A, I}$ acts on $G_{\Prism, A, I}$ similarly.
It then suffices to prove that
$
G\mathchar`-\mathrm{Disp}_\mu(A, I)
$
...
We claim that every $G_{\mu, A, I}$-torsor $\mathcal{Q}$ on $(A, I)^{\op}_{\Prism}$
is trivialized by a $(\pi, I)$-completely \'etale covering $A \to B$.
Indeed, in the same way as Corollary \ref{Corollary:trivial Gmu torsor}, we see that if the pushout of $\mathcal{Q}$ along
$G_{\mu, A, I} \to (P_\mu)_{\overline{\Prism}, A}$
is trivial as a $(P_\mu)_{\overline{\Prism}, A}$-torsor on $(A, I)^{\op}_{\Prism}$, then $\mathcal{Q}$ is itself trivial.
By Proposition \ref{Proposition:equivalences of pi completely etale torsors}, any $(P_\mu)_{\overline{\Prism}, A}$-torsor $\mathcal{P}$ on $(A, I)^{\op}_{\Prism}$ arises from a $(P_\mu)_{A/I}$-torsor over $\Spec A/I$ with respect to the \'etale topology, which in turn implies that $\mathcal{P}$ is trivialized by a $(\pi, I)$-completely \'etale covering $A \to B$ (see also Lemma \ref{Lemma:etale morphism and reduction}).
The claim then follows.


By the claim,
the restriction of each pair $(\mathcal{Q}, \alpha_{\mathcal{Q}}) \in G\mathchar`-\mathrm{Disp}^{\fl}_\mu(A, I)$ to $(A, I)_\et$
...
sending
$f \colon (A, I) \to (B, IB) \in (A, I)_\Prism$
...
\begin{proof}
    By virtue of Proposition \ref{Proposition:flat descent of G display} and Remark \ref{Remark:flat descent for G-BK modules},
    this follows from Lemma \ref{Lemma:banal G displays and banal G-BK module of type mu}.
\end{proof}
...
\begin{cor}\label{Corollary:G-BK module etale banal}
    Let $(A, I)$ be an orientable and bounded $\O_E$-prism over $\O$.
    Let $\mathcal{P}$ be a $G$-Breuil--Kisin module of type $\mu$ over $(A, I)$.
    Then there exists a $(\pi, I)$-completely \'etale covering $A \to B$ such that the base change of $\mathcal{P}$ to $(B, IB)$ is banal.
\end{cor}

\begin{proof}
    By Proposition \ref{Proposition:G-displays and G-BK modules},
    we may assume that $\mathcal{P}=\mathcal{Q}_{\mathrm{BK}}$ for a $G$-$\mu$-display $\mathcal{Q}$ over $(A, I)$.
    There exists a $(\pi, I)$-completely \'etale covering $f \colon A \to B$ such that $f^*\mathcal{Q}$ is banal.
    It follows that the base change of $\mathcal{P}$ to $(B, IB)$ is banal as well.
\end{proof}

\begin{ex}\label{Example:GLn displays}
    Let $(A, I)$ be a bounded $\O_E$-prism over $\O$.
    Assume that $G=\GL_n$ and let
    $\mu \colon \G_m \to \GL_{n, \O}$
    be a cocharacter.
    By Example \ref{Example:GLn BK module of type mu} and Proposition \ref{Proposition:G-displays and G-BK modules}, we obtain an equivalence
    $
\mathrm{BK}_\mu(A, I)^{\simeq} \simeq \GL_n\mathchar`-\mathrm{Disp}_\mu(A, I).
$

Let us give a more direct construction of such an equivalence when $(A, I)$ is orientable.
Let
$M$
...
we define 
$\alpha(h)_d \colon B^n \overset{\sim}{\to} B^n$
as in the proof of Lemma \ref{Lemma:banal BK type mu standard form}.
The element $\alpha(h) \in \GL_n(B)_{IB}$
corresponding to $\alpha(h)_d \in \GL_n(B)_d$
does not depend on the choice of $d$.
In this way, we obtain a $(\GL_n)_{\mu, A, I}$-equivariant map
$\alpha \colon \mathcal{Q}(M) \to G_{\Prism, A, I}$, so that
$\mathcal{Q}(M)$ with the map $\alpha$
...
the equivalence
$\GL_n\mathchar`-\mathrm{Disp}_\mu(A, I) \overset{\sim}{\to} \GL_n\mathchar`-\mathrm{BK}_\mu(A, I)$
given in Proposition \ref{Proposition:G-displays and G-BK modules}
is isomorphic to the equivalence
$
\mathrm{BK}_\mu(A, I)^{\simeq} \overset{\sim}{\to} \GL_n\mathchar`-\mathrm{BK}_\mu(A, I)
$
given in Example \ref{Example:GLn BK module of type mu}.
\end{ex}
\begin{comment}
...
\subsection{$\phi$-$G$-torsors of type $\mu$}\label{Subsection:phi G torsor of type mu}

Let $(A, I)$ be an orientable and bounded $\O_E$-prism over $\O$.
We set $A[1/\phi(I)]:=A[1/\phi(d)]$ for a generator $d \in I$, which does not depend on the choice of $d$.
Let us introduce the following variant of the notion of $G$-Breuil--Kisin modules.
...
\begin{ex}\label{Example:G-BK module and phi G-torsor}
Let $(\mathcal{P}, F_\mathcal{P})$ be a $G$-Breuil--Kisin module over $(A, I)$ in the sense of Definition \ref{Definition:G-BK module}.
Then the base change
$
(\phi^*\mathcal{P}, \phi^*(F_\mathcal{P}))
$
is a $\phi$-$G$-torsor over $(A, I)$.
\end{ex}
...
We attach a $\phi$-$G$-torsor over $(A, I)$ to a $G$-$\mu$-display $\mathcal{Q}$ over $(A, I)$ as follows.
We write
\[
...
By Proposition \ref{Proposition:equivalences of pi completely etale torsors}, we regard $\mathcal{Q}_A$ as a $G_A$-torsor over $\Spec A$.
We shall construct a natural isomorphism
\[
...
As in the proof of Proposition \ref{Proposition:functor from G-displays to G-BK}, it suffices to consider the case where $\mathcal{Q}$ is banal.
We choose an isomorphism
$\mathcal{Q} \simeq \mathcal{Q}_X$
for some $X \in G(A)_I$.
We may identify
...
with $G_A$ by
$
\mathcal{Q}_{A} \simeq (\mathcal{Q}_X)_{A}=G_A.
$
We then define the isomorphism $\phi_{\mathcal{Q}_A}$ by
\[
(\phi^*G_A)[1/\phi(I)]=G_A[1/\phi(I)] \overset{\sim}{\to} G_A[1/\phi(I)], \quad g \mapsto X_\phi g.
\]
The isomorphism
$\phi_{\mathcal{Q}_A}$
is independent of the choice of $\mathcal{Q} \simeq \mathcal{Q}_X$.
...
\]
over $(A, I)$
is called the \textit{underlying $\phi$-$G$-torsor} of $\mathcal{Q}$.
\end{defn}


%The underlying $\phi$-$G$-torsor $\mathcal{Q}_{\phi}$ of $\mathcal{Q}$ is related to the associated $G$-Breuil--Kisin module $\mathcal{Q}_{\mathrm{BK}}$ as follows:


\begin{prop}\label{Proposition:underlying phi G torsor and underlying G-BK module}
Let
$\mathcal{Q}$
be a $G$-$\mu$-display over $(A, I)$.
Then $\mathcal{Q}_{\phi}$ is isomorphic to
$\phi^*(\mathcal{Q}_{\mathrm{BK}})$
as a $\phi$-$G$-torsor over $(A, I)$.
\end{prop}

\begin{proof}
We first assume that $\mathcal{Q}$ is banal.
We choose an isomorphism
$\mathcal{Q} \simeq \mathcal{Q}_X$ for some $X \in G(A)_I$.
Let $d \in I$ be a generator.
The underlying $G_A$-torsors of $\phi^*((\mathcal{Q}_X)_{\mathrm{BK}, d})$ and $(\mathcal{Q}_X)_\phi$ can be identified with $G_A$; see the proof of Proposition \ref{Proposition:functor from G-displays to G-BK} for $(\mathcal{Q}_X)_{\mathrm{BK}, d}$.
The isomorphism $G_A \overset{\sim}{\to} G_A$ defined by $g \mapsto X_d g$ induces an isomorphism
$\phi^*((\mathcal{Q}_X)_{\mathrm{BK}, d}) \overset{\sim}{\to} (\mathcal{Q}_X)_{\phi}$
of $\phi$-$G$-torsors over $(A, I)$.
One can check that the composition
\[
\phi^*(\mathcal{Q}_{\mathrm{BK}}) \simeq \phi^*((\mathcal{Q}_X)_{\mathrm{BK}}) \simeq \phi^*((\mathcal{Q}_X)_{\mathrm{BK}, d}) \simeq (\mathcal{Q}_X)_{\phi} \simeq  \mathcal{Q}_{\phi}
\]
does not depend on the choices of $\mathcal{Q} \simeq \mathcal{Q}_X$ and $d \in I$.

    In general, the isomorphisms in the banal case glue together to an isomorphism
...
We also introduce a variant of the notion of $G$-Breuil--Kisin modules of type $\mu$.
...
We discuss some $p$-complete $\arc$-descent results for $G$-$\mu$-displays over the $\O_E$-prism
$
(W_{\O_E}(R^\flat), I_R).
$

Following \cite{PappasRapoport21} and \cite{Bartling},
we work with $\phi$-$G$-torsors over $(W_{\O_E}(R^\flat), I_R)$ instead of $G$-Breuil--Kisin modules over $(W_{\O_E}(R^\flat), I_R)$, although they are essentially equivalent (cf.\ Proposition \ref{Proposition:equivalence of G displays and phi G torsors perfectoid}).
A $\phi$-$G$-torsor over $(W_{\O_E}(R^\flat), I_R)$ is also called a
\textit{$G$-Breuil--Kisin--Fargues module}
(or simply a $G$-BKF module)
for $R$.
In Definition \ref{Definition:phi-G-torsor of type mu}, we introduced the notion of $G$-BKF modules of type $\mu$.


\begin{rem}\label{Remark:G-BKF of type mu}
Assume that $\O_E=\Z_p$.
In \cite{Bartling}, the notion of $G$-BKF modules \textit{of type $\mu$} was introduced in a different way.
Namely, in \cite{Bartling},
a $G$-BKF module $\mathcal{P}$ for $R$ is said to be of type $\mu$ if for any homomorphism $R \to V$ with $V$ a
...
\ 
\begin{enumerate}
    \item We have the following equivalence of groupoids:
    \[
    G\mathchar`-\mathrm{BK}_\mu(W_{\O_E}(R^\flat), I_R)
    \overset{\sim}{\to}
    \left.
\left\{
\begin{tabular}{c}
     $G$-BKF modules of type $\mu$ \\ for $R$
\end{tabular}
\right\}
\right., \quad \mathcal{P} \mapsto \phi^*\mathcal{P}.
\]
    \item We have the following equivalence of groupoids:
    \[
    G\mathchar`-\mathrm{Disp}_\mu(W_{\O_E}(R^\flat), I_R)
    \overset{\sim}{\to}
    \left.
\left\{
\begin{tabular}{c}
...
\end{tabular}
\right\}
\right., \quad \mathcal{Q} \mapsto \mathcal{Q}_\phi.
\]
\end{enumerate}
\end{prop}
...
\begin{proof}
(1) Since $\phi \colon W_{\O_E}(R^\flat) \to W_{\O_E}(R^\flat)$ is bijective, the functor
$\mathcal{P} \mapsto \phi^*\mathcal{P}$
from the groupoid of $G$-Breuil--Kisin modules over $(W_{\O_E}(R^\flat), I_R)$
to the groupoid of $G$-BKF modules for $R$
is an equivalence.
Then it suffices to prove that a $G$-Breuil--Kisin module $\mathcal{P}$
over $(W_{\O_E}(R^\flat), I_R)$ is of type $\mu$ if and only if $\phi^*\mathcal{P}$ is of type $\mu$ as a $G$-BKF module.
The ``only if'' direction follows from Corollary \ref{Corollary:G-BK module etale banal}.
By Example \ref{Example:perfectoid ring etale morphism}, any $(\pi, I_R)$-completely \'etale $W_{\O_E}(R^\flat)$-algebra $B$ is of the form $W_{\O_E}(S^\flat)$ for a perfectoid ring $S$.
In particular, the Frobenius $\phi \colon B \to B$ is bijective.
The ``if'' direction follows from this fact.

(2) This follows from (1), Proposition \ref{Proposition:G-displays and G-BK modules}, and Proposition \ref{Proposition:underlying phi G torsor and underlying G-BK module}.
\end{proof}
...
For
a $G$-BKF module $\mathcal{P}$ for $R$, the following conditions are equivalent.
\begin{enumerate}
    \item $\mathcal{P}$ is of type $\mu$ (in the sense of Definition \ref{Definition:phi-G-torsor of type mu}).
...
Arguing as in Remark \ref{Remark:flat descent for G-BK modules}, we see that
the fibered category over $\Perfd^{\op}_R$ which associates to each $S \in \Perfd_R$ the groupoid of $G$-BKF modules for $S$ is a stack with respect to the $\pi$-complete $\arc$-topology.
This fact, together with Proposition \ref{Proposition:equivalence of G displays and phi G torsors perfectoid} and Corollary \ref{Corollary:arc descent for G-displays},
implies that
$\mathcal{P} \simeq \mathcal{Q}_\phi$
for a $G$-$\mu$-display $\mathcal{Q}$ over
$(W_{\O_E}(R^\flat), I_R)$, and hence $\mathcal{P}$ is of type $\mu$.
...
\begin{proof}
    ...
    it suffices to prove that the groupoid of $G$-$\mu$-adapted pairs over $(A, I)$ is equivalent to the groupoid $G\mathchar`-\mathrm{BK}_\mu(A, I)$.
    Let
    $(M, \{  s_{\alpha, M} \}_{\alpha \in \Lambda})$
    be a $G$-$\mu$-adapted pair over $(A, I)$.
    The functor
    \[
    \mathcal{P}:= \underline{\mathrm{Isom}}_{\{ s_\alpha \}}(L_A, M)
    \colon (A, I)_\et \to \mathrm{Set}
    \]
    sending $B \in (A, I)_\et$ to the set of isomorphisms
    $L_B \overset{\sim}{\to} M_B$
    carrying $s_\alpha$ to $s_{\alpha, M}$ for each $\alpha \in \Lambda$
    is a $G_{\Prism, A}$-torsor.
    By Proposition \ref{Proposition:equivalences of pi completely etale torsors}, we regard $\mathcal{P}$
    as a $G_A$-torsor over $\Spec A$.
    The isomorphism
    $F_M$ endows $\mathcal{P}$ with the structure of a $G$-Breuil--Kisin module over $(A, I)$.
    We claim that $\mathcal{P}$ is of type $\mu$.
    Once this claim is established, it is easy to see that the above construction induces an equivalence
    between the groupoid of $G$-$\mu$-adapted pairs over $(A, I)$
    and $G\mathchar`-\mathrm{BK}_\mu(A, I)$.

    We shall show that $\mathcal{P}$ is of type $\mu$.
    We may assume that there exists an isomorphism
    ...
    Under the isomorphism $h$, we have 
    \[
    \{ \Fil^i_\mu \}_{i \in \Z}=\{ \Fil^i(\phi^*M) \}_{i \in \Z},
    \]
    where $\{ \Fil^i_\mu \}_{i \in \Z}$ is the filtration of $L_A=A^n$ given in Remark \ref{Remark:standard filtration}.
    ...
    Let $d \in I$ be a generator.
    We define
    $\alpha(h)_d \colon L_A \overset{\sim}{\to} L_A$
    as in the proof of Lemma \ref{Lemma:banal BK type mu standard form}.
    By construction, it is clear that $\alpha(h)_d$ stabilizes $s_\alpha$ for each $\alpha \in \Lambda$, and hence $\alpha(h)_d \in G(A)$.
    By the proof of Lemma \ref{Lemma:banal BK type mu standard form}, there exists an isomorphism
    $\mathcal{P} \simeq G_A$ under which $F_\mathcal{P}$ is given by $g \mapsto \mu(d)(\alpha(h)_d)g$.
    Therefore $\mathcal{P}$ is of type $\mu$.
\end{proof}

\begin{rem}\label{Remark:Hodge type integral model of Shimura variety}
    In \cite[Proposition 1.3.4]{Kisin10}, \cite[Theorem 2.5]{Kim-MadapusiPera}, and \cite[Section 5.3]{IKY},
...
$J:=(d, x_1, \dotsc, x_n) \subset B'$; see Proposition \ref{Proposition:prismatic envelope}.
The map $(A, I) \to (A', I')$
is a flat covering.
...
\begin{proof}
    ...
    It then follows from Example \ref{Example:GLn displays} and
...
\begin{proof}
    ...
    Thus, by virtue of \cite[Lemma 4.28]{BMS2} and \cite[Proposition 7.11]{BS}, we may assume that $R$ is quasiregular semiperfectoid.
...
we discuss the relation between our prismatic $G$-$\mu$-displays and \textit{prismatic $F$-gauges} introduced by Drinfeld and Bhatt--Lurie (cf.\ \cite{Drinfeld22}, \cite{BL}, \cite{BL2}, \cite{BhattGauge}).
For simplicity,
we assume that
...
\begin{proof}
    ...
    (Moreover
    $\phi^*M=\bigoplus_{j \in \Z} L_j$
    is a normal decomposition in the sense of Lemma \ref{Lemma:normal decomposition for displayed BK module}.)
...
\begin{proof}
    ...
    is faithfully flat
    and the $(p, I)$-adic completion of
...
By Proposition \ref{Proposition:quasisyntomic descent for prismatic F-gauge},
the fibered category
...
\]
over
the category $\qsyn^{\op}$ that satisfies descent with respect to the quasisyntomic topology; see \cite[Proposition 4.31]{BMS2}.
...
\begin{rem}\label{Remark:on Drinfeld's definition of G-gauge of type mu}
    ...
    It should be possible to define prismatic $G$-$F$-gauges of type $\mu$ over any $p$-adic formal scheme by using certain torsors on syntomifications, but we will not discuss this here.
\end{rem}
...
\begin{proof}
    ...
    the base change
    $M_{(W(R^\flat_i), I_{R_i})}$ is of type $\mu$ for some $\mu$.
...
Finally, the author would like to thank the referee for helpful comments and especially for the suggestion that a discussion of the relation between prismatic $G$-$\mu$-displays and prismatic $F$-gauges should be included.
