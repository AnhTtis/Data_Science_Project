...
\thanks{Manuscript under review. \textit{(Corresponding author: Yuzhi Zhao.)}}
...
%Previous video colorization methods are mainly based on additional color hints \cite{reinhard2001color, welsh2002transferring, levin2004colorization, yatziv2006fast, sheng2013video}, which guide the colorization algorithms to produce realistic results. Recently, deep learning automates the video colorization process \cite{lai2018, lei2019fully, kouzouglidis2019automatic, zhao2022vcgan}. Recovering realistic and colorful videos with deep neural networks becomes plausible.

%In addition, there are wide applications of colorization such as compression \cite{baig2017multiple}, manga colorization \cite{qu2006manga}, infrared image colorization \cite{nyberg2018unpaired}, and medical image colorization \cite{mathur20212d}.
%To accelerate the inference speed and maintain high-quality colorization, we propose a scribble-based video colorization framework with pyramid networks (SVCNet).
...
% To aggregate the temporal information, SSNet receives the current colorized frame, previous frames, leading frames, and the first colorized frame.
...
% Normally, exemplar images have similar statistics with grayscale images; otherwise, the colors of results will be distorted \cite{reinhard2001color, welsh2002transferring, tai2005local}.

%The fully-automatic methods directly learn end-to-end colorization on large datasets with neural networks. Cheng \emph{et al.} \cite{cheng2015deep, cheng2017colorization} proposed the first neural-network-based architecture for automatic colorization; however, they used the combinations of hand-crafted features and simple MLP layers, thus leading to unrealistic results. The following methods replaced the hand-crafted features with semantics from pre-trained networks such as VGGNet \cite{simonyan2014very} and ResNet \cite{he2016deep}, e.g., hyper-column \cite{larsson2016learning}, backbone networks \cite{larsson2016learning, zhang2016colorful, iizuka2016let, vitoria2020chromagan, zhao2020scgan}. To further improve generation quality, some regularization items were leveraged such as classification information \cite{iizuka2016let, vitoria2020chromagan}, semantic segmentation \cite{zhao2020pixelated}, instance segmentation \cite{su2020instance}, and saliency map \cite{zhao2020scgan}. The classification information promotes the focus on the global context, while the segmentation and saliency maps facilitate the learning of key objects in an image.
...
% There are two sources of saliency maps: fixation prediction and salient object detection. The former records the eye fixations of humans and the latter focuses on entire key objects. Compared with saliency maps from fixation prediction, those from salient object detection are more continuous and have clearer boundaries.
...
%\textbf{Generative Adversarial Network.} The generative adversarial network (GAN) was developed by Goodfellow \emph{et al.}  both \cite{goodfellow2014generative}, which includes a generator and a discriminator. The generator produces realistic samples; whereas the discriminator distinguishes whether the input samples are from ground truth or generated. In colorization region, GAN was utilized for improving perceptual quality \cite{isola2017image, vitoria2020chromagan} or obtaining diverse output \cite{cao2017unsupervised}. The vanilla GAN is trained to minimize JS-divergence of generated samples and ground truth. However, it is hard to converge and often comes to mode collapse. To address the issue, some new divergences were introduced such as f-GAN \cite{nowozin2016f}, LSGAN \cite{mao2017least}, WGAN \cite{arjovsky2017wasserstein} and WGAN-GP \cite{gulrajani2017improved}. To further stabilize GAN training, spectral normalization (SN) \cite{miyato2018spectral} was proposed to enforce 1-Lipschitz continuity of discriminator. In this paper, we apply WGAN with SN to train SVCNet framework.
...
%The results are shown in Figure \ref{net} (c.I). After the 1st stage training, CPNet (red line in Figure \ref{net} (c.I)) obtains higher PSNR values on 10000 ImageNet validation set \cite{zhang2016colorful} than the state-of-the-art algorithm RUIC \cite{zhang2017real} (yellow line in Figure \ref{net}) under different numbers of given color scribbles for a specific grayscale input. In addition, we notice that adding the short-cut connections of semantic feature encoder to pyramid fusion decoder (blue line in Figure \ref{net}) decreases the performance. We assume such connections weaken the function of given color scribbles and make the network difficult to converge. Although CPNet trained without segmentation labels (green line in Figure \ref{net}) achieves better performance on the ImageNet validation set, it leads to worse performance at the next training stage (see Figure \ref{net} (c.II)) and we will discuss more results in the follows.

% The finetuning results are shown in Figure \ref{net} (c.II)-(c.IV), which are computed on single frames in the DAVIS \cite{perazzi2016benchmark} and Videvo \cite{Videvo} validation sets with 256$\times$448 resolution. From (c.II), we observe that CPNet performs better than it without segmentation branch or with short-cut connections of semantic feature encoder. From both (c.II) and (c.III), we observe that CPNet is still superior to RUIC \cite{zhang2017real} on video validation sets, even though we also finetune RUIC on those video training sets. From (c.IV), we find that a larger training resolution is helpful for colorization quality (e.g., higher PSNR values). Therefore, we finetune the CPNet on 256$\times$448 resolution. If there is a high demand for running speed and memory cost, we can adjust the input resolution of CPNet to 64$\times$128 with approximately 0.5dB decrease when no guidance and 2.5dB decrease when given 40 color scribbles.
...
%\multirow{2}{*}{3) joint training stage} & \multirow{2}{*}{CPNet+SSNet} & \multirow{2}{*}{$L^{joint}$} & \multirow{2}{*}{DAVIS+Videvo} & \multirow{2}{*}{312,000} & 1$\times$10$^{-6}$ for CPNet & \multirow{2}{*}{156,000} \cr & & & & & 5$\times$10$^{-5}$ for SSNet & \\
%\multirow{2}{*}{Pipeline) Method} & \multirow{2}{*}{Type} & Color & \multicolumn{4}{c}{DAVIS} & \multicolumn{3}{c}{Videvo} \cr & & Scribble & PSNR $\uparrow$ & SSIM $\uparrow$ & Warp Error $\downarrow$ & mIoU $\uparrow$ & PSNR $\uparrow$ & SSIM $\uparrow$ & Warp Error $\downarrow$ \\
\hline
\end{tabular}
\end{center}
\end{table*}
...
\label{loss_cpnet}
\end{equation}
%where $\lambda_c$ and $\lambda_s$ are trade-off parameters.
...
%For the joint training, we maintain CPNet loss function and add SSNet loss function. Firstly, we adopt a colorization loss:
%\begin{equation}
%L_c^{SS} = \mathbb{E} [|| z_i^{ab} - x_i^{ab} ||_1 ],
%\label{2_c}
%\end{equation}
%where $z_i^{ab}$ is the final generated result of SSNet. Secondly, we adopt a total variation loss \cite{aly2005image} to smooth $z_i^{ab}$ to encourage neighbouring pixels having similar colors:
%\begin{equation}
%L_{tv}^{SS} = \sum\limits_{h,w} \mathop{} \sqrt{ (z_{i (h+1,w)}^{ab} - z_{i (h,w)}^{ab})^2 + (z_{i (h,w+1)}^{ab} - z_{i (h,w)}^{ab})^2 }.
%\label{2_tv}
%\end{equation}

%Furthermore, we adopt the temporal loss to impose pixel-level constrain on different generated frames. For two frames, we utilize forward optical flow to align the previous frame to the position of the current frame (i.e., $z_i^{ab}$). Then, we compute the temporal loss only on non-occluded regions marked by a visual mask (defined in Equation \ref{ssnet1}). It is defined as follows:
%\begin{equation}
%L_t^{SS} (n) = \mathbb{E} [ M_{n \rightarrow i} \cdot || z_i^{ab} - \mathbb{W} ( z_{n}^{ab}, O_{n \rightarrow i} ) ||_1 ],
%\label{2_t}
%\end{equation}
%where $z_i$, $z_{n}$, $M_{n \rightarrow i}$, and $O_{n \rightarrow i}$ represent the current output, last outputs (e.g., $n=i-1$ or $n=1$), visual masks, and forward optical flows, respectively. Note that $L_t^{SS} (n)$ is computed between $n$-th and $i$-th generated images within a batch where the motion is not large; therefore, the optical flow is relatively accurate. The total loss for SSNet is given by:
%\begin{equation}
%L^{SS} = \lambda_c L_c^{SS} + \lambda_{tv} L_{tv}^{SS} + \lambda_{st} L_t^{SS} (i-1) + \lambda_{lt} L_t^{SS} (1),
%\label{2_loss}
%\end{equation}
%where $\lambda_c$, $\lambda_{st}$ and $\lambda_{lt}$ are trade-off parameters. $L_t^{SS} (i-1)$ and $L_t^{SS} (1)$ represent the short-term temporal loss and long-term temporal loss, respectively. The full loss function of joint training stage is the sum:
%\begin{equation}
%L^{joint} = L^{CP} + L^{SS}.
%\label{3_loss}
%\end{equation}
...
%The evaluation experiment is performed in the \emph{CIE Lab} color space since the grayscale $L$ channel should be unchanged for the colorization task.

%\noindent \textbf{Temporal Consistency.} We adopt Warp Error to measure color similarities between every two nearby frames in every generated video. It is calculated by warping the previous frame to the current frame's position and then computing the disparity between them. We follow the protocol in \cite{ruder2016artistic, zhao2022vcgan}, i.e., only the non-occlusion regions are computed and the PWC-Net \cite{sun2018pwc} is used as the flow estimator.
%However, we compute it only on the \emph{ab} channels.
...
%The videos for the human preference study are included in the supplementary material.
...
%\begin{table*}[t]
%\begin{center}
%\caption{Comparison of video colorization pipelines and the proposed SVCNet on DAVIS and Videvo datasets. The \textcolor{red}{\textbf{red}}, \textcolor{blue}{\textbf{blue}}, and \textcolor{green}{green} colors represent the best, the second-best, and the third-best performance, respectively. The ``IC'' and ``VC'' represent the ``image colorization method'' and ``video colorization method'' in the second column, respectively.}
%\label{table_sota}
%\begin{tabular}{lccccccc}
%\hline
%\multirow{2}{*}{Method} & \multirow{2}{*}{Type} & Color & \multicolumn{3}{c}{DAVIS} & \multicolumn{2}{c}{Videvo} \cr & & scribble & PSNR $\uparrow$ & SSIM $\uparrow$ & mIoU $\uparrow$ & PSNR $\uparrow$ & SSIM $\uparrow$ \\
...
%4) RUIC + DEVC \cite{zhang2017real, zhang2019deep} & VC & \checkmark & 24.85 & \textcolor{blue}{0.9524} & 0.9002 & \textcolor{blue}{25.66} & \textcolor{blue}{0.9583} \\
%\hline
%SVCNet & VC & \checkmark & \textcolor{blue}{25.71} & \textcolor{red}{0.9565} & \textcolor{red}{} & \textcolor{red}{26.30} & \textcolor{red}{0.9615} \\
%\hline
%\end{tabular}
%\end{center}
%\end{table*}

% RGB domain, CPNet variants
%\begin{table*}[t]
%\begin{center}
%\caption{Comparison of SVCNet and CPNet-based methods on DAVIS and Videvo datasets.}
%\label{table_sota}
%\begin{tabular}{lccccccc}
%\hline
%\multirow{2}{*}{Method} & \multirow{2}{*}{Type} & Color & \multicolumn{3}{c}{DAVIS} & \multicolumn{2}{c}{Videvo} \cr & & scribble & PSNR $\uparrow$ & SSIM $\uparrow$ & mIoU $\uparrow$ & PSNR $\uparrow$ & SSIM $\uparrow$ \\
%\hline
%CPNet + BTC \cite{zhang2017real, lai2018} & VC & \checkmark & 19.35 & 0.8905 & 0.8830 & 16.17 & 0.8259 \\
%CPNet + DEVC \cite{zhang2017real, zhang2019deep} & VC & \checkmark & 24.84 & 0.9521 & 0.8991 & 25.69 & 0.9588 \\
%\hline
%SVCNet & VC & \checkmark & \textcolor{red}{25.71} & \textcolor{red}{0.9565} & \textcolor{red}{0.9104} & \textcolor{red}{26.30} & \textcolor{red}{0.9615} \\
%\hline
%\end{tabular}
%\end{center}
%\end{table*}
...
%\begin{center}
%%\caption{Comparison of video colorization pipelines and the proposed SVCNet on DAVIS and Videvo datasets. The \textcolor{red}{\textbf{red}}, \textcolor{blue}{\textbf{blue}}, and \textcolor{green}{green} colors represent the best, the second-best, and the third-best performance, respectively. The ``IC'' and ``VC'' represent the ``image colorization method'' and ``video colorization method'' in the second column, respectively.}
%\caption{Comparison of video colorization pipelines and the proposed SVCNet on DAVIS and Videvo datasets (in $ab$ space).}
%\label{table_sota}
%\begin{tabular}{lccccccc}
%\hline
%\multirow{2}{*}{Method} & \multirow{2}{*}{Type} & Color & \multicolumn{3}{c}{DAVIS} & \multicolumn{2}{c}{Videvo} \cr & & scribble & PSNR $\uparrow$ & SSIM $\uparrow$ & mIoU $\uparrow$ & PSNR $\uparrow$ & SSIM $\uparrow$ \\
%\hline
%1) CIC \cite{zhang2016colorful} & IC & / & 30.68 & 0.9176 & 0.8419 & 30.17 & 0.9200 \\
%1) LTBC \cite{iizuka2016let} & IC & / & 31.39 & 0.9266 & 0.8945 & 31.34 & 0.9336 \\
%1) ChromaGAN \cite{vitoria2020chromagan} & IC & / & 31.02 & 0.9221 & 0.8879 & 30.91 & 0.9291 \\
%%1) SCGAN \cite{zhao2020scgan} & IC & / & 31.48 & 0.9291 & 0.8743 & 31.27 & 0.9371 \\
%1) IAC \cite{su2020instance} & IC & / & 31.69 & 0.9308 & 0.8836 & 31.55 & 0.9371 \\
%1) CIC + BTC \cite{zhang2016colorful, lai2018} & VC & / & 30.43 & 0.9192 & 0.8419 & 30.09 & 0.9215 \\
%1) LTBC + BTC \cite{iizuka2016let, lai2018} & VC & / & 31.11 & 0.9272 & 0.8899 & 30.96 & 0.9341 \\
%1) ChromaGAN + BTC \cite{vitoria2020chromagan, lai2018} & VC & / & 31.09 & 0.9222 & 0.8796 & 30.97 & 0.9280 \\
%%1) SCGAN + BTC \cite{zhao2020scgan, lai2018} & VC & / & 31.42 & 0.9286 & 0.8635 & 31.02 & 0.9356 \\
%1) IAC + BTC \cite{su2020instance, lai2018} & VC & / & 31.44 & 0.9292 & 0.8698 & 31.36 & 0.9341 \\
%1) CIC + DVP \cite{zhang2016colorful, lei2020blind} & VC & / & 30.90 & 0.9114 & 0.8724 & 30.23 & 0.9245 \\
%1) LTBC + DVP \cite{iizuka2016let, lei2020blind} & VC & / & 31.24 & 0.9172 & 0.8886 & 31.45 & 0.9352 \\
%1) ChromaGAN + DVP \cite{vitoria2020chromagan, lei2020blind} & VC & / & 31.01 & 0.9138 & 0.8800 & 30.96 & 0.9326 \\
%%1) SCGAN + DVP \cite{zhao2020scgan, lei2020blind} & VC & / & 31.37 & 0.9253 & 0.8802 & 31.25 & 0.9370 \\
%1) IAC + DVP \cite{su2020instance, lei2020blind} & VC & / & 31.72 & 0.9172 & 0.8936 & 31.61 & 0.9360 \\
%2) CIC + DEVC \cite{zhang2016colorful, zhang2019deep} & VC & / & 30.51 & 0.9262 & 0.8661 & 30.12 & 0.9286 \\
%2) LTBC + DEVC \cite{iizuka2016let, zhang2019deep} & VC & / & 31.11 & 0.9297 & 0.8688 & 31.49 & 0.9382 \\
%2) ChromaGAN + DEVC \cite{vitoria2020chromagan, zhang2019deep} & VC & / & 30.86 & 0.9302 & \textcolor{green}{0.9026} & 30.69 & 0.9351 \\
%%2) SCGAN + DEVC \cite{zhao2020scgan, zhang2019deep} & VC & / & 31.46 & 0.9298 & 0.8806 & 31.07 & 0.9375 \\
%2) IAC + DEVC \cite{su2020instance, zhang2019deep} & VC & / & 31.37 & 0.9294 & 0.8712 & 31.30 & 0.9374 \\
%3) 3DVC \cite{kouzouglidis2019automatic} & VC & / & 31.47 & 0.9309 & 0.8948 & 31.70 & 0.9375 \\
%3) FAVC \cite{lei2019fully} & VC & / & 30.77 & 0.9245 & 0.8889 & 30.51 & 0.9335 \\
%3) VCGAN \cite{zhao2022vcgan} & VC & / & 31.68 & 0.9310 & 0.8954 & \textcolor{green}{31.95} & \textcolor{green}{0.9385} \\
%\hline
%4) RUIC \cite{zhang2017real} & IC & \checkmark & \textcolor{green}{32.88} & \textcolor{green}{0.9361} & 0.8995 & 31.89 & 0.9364 \\
%4) RUIC + BTC \cite{zhang2017real, lai2018} & VC & \checkmark & 32.86 & 0.9348 & 0.8900 & 31.75 & 0.9347 \\
%4) RUIC + DVP \cite{zhang2017real, lei2020blind} & VC & \checkmark & 32.26 & 0.9269 & \textcolor{blue}{0.9075} & 31.48 & 0.9349 \\
%4) RUIC + DEVC \cite{zhang2017real, zhang2019deep} & VC & \checkmark & \textcolor{red}{34.37} & \textcolor{blue}{0.9380} & 0.9002 & \textcolor{blue}{32.66} & \textcolor{blue}{0.9396} \\
%\hline
%SVCNet & VC & \checkmark & \textcolor{blue}{33.88} & \textcolor{red}{0.9381} & \textcolor{red}{0.9102} & \textcolor{red}{33.23} & \textcolor{red}{0.9418} \\
%\hline
%\end{tabular}
%\end{center}
%\end{table*}
...
%5) CPNet post-processed by temporal smoothing method \cite{lei2020blind} or color propagation method \cite{zhang2019deep}. Note that, CPNet weights are not refined by SSNet, i.e., from warm-up pre-training.
...
%\noindent \textbf{Temporal Continuity.} For pipeline 1), the videos generated by image colorization and temporal smoothing methods are not continuous enough, e.g., ``CIC + BTC'', ``CIC + DVP'', ``LTBC + BTC'', and ``LTBC + DVP'' results in Figure \ref{video}. Since image colorization and temporal smoothing are learned individually, the final outputs are still close to the single-frame colorized results. For instance, the human is colorized to red in frame 160 for the ``CIC'' results while it turns to gray for frame 200; but the ``CIC + BTC'' cannot alleviate this issue, i.e., the results are not temporal consistent enough. For pipeline 2), the videos are smoother than the results from pipeline 1). However, it exists a similar problem since image colorization methods and ``DEVC'' are not jointly trained. From Table \ref{table_sota}, we observe that ``LTBC + DEVC'' obtains good Warp Error performance. But it is still inferior to the proposed SVCNet. In addition, since pipeline 2) highly relies on the information from the first colorized frame, the colorization results for far frames are not reasonable. For instance, the human is colorized to red in frame 245 for the ``CIC + DEVC'' results since it is red in the first frame. The ``DEVC'' simply copies the wrong colorized results. For pipeline 3), the video continuity is good in all compared with other methods; but the results of pipeline 3) are less colorful, as discussed before. For pipeline 4), ``RUIC + DEVC'' outperforms the other two methods ``RUIC'' and ``RUIC + DVP''. However, it remains the common problem of DEVC-based methods, i.e., results highly rely on the first colorized frame. Also, both quantitative results and qualitative samples of ``RUIC + DEVC'' are inferior to SVCNet. Finally, SVCNet aggregates both short-range connections and the long-range connection. It obtains better temporal consistency than other methods.
...
% Lab space
%CIC \cite{zhang2016colorful} & / & 26.22 & 0.9511 \\
%LTBC \cite{iizuka2016let} & / & 28.31 & 0.9584 \\
%LRAC \cite{larsson2016learning} & / & 28.48 & 0.9596 \\
%Pix2Pix \cite{isola2017image} & / & 26.66 & 0.9516 \\
%DeOldify \cite{DeOldify} & / & 27.74 & 0.9516 \\
%FAVC \cite{lei2019fully} & / & 27.25 & 0.9543 \\
%ChromaGAN \cite{vitoria2020chromagan} & / & 27.28 & 0.9538 \\
%SCGAN \cite{zhao2020scgan} & / & 27.42 & 0.9556 \\
%IAC \cite{su2020instance} & / & \textcolor{green}{31.41} & \textcolor{green}{0.9663} \\
%%VCGAN \cite{zhao2022vcgan} & / & 28.11 & 0.9561 \\
%RUIC \cite{zhang2017real} (w/o $s$) & / & 29.21 & 0.9624 \\
%CPNet of SVCNet (w/o $s$) & / & 29.31 & 0.9636 \\
%\hline
%RUIC \cite{zhang2017real} (with $s$) & \checkmark & \textcolor{blue}{32.51} & \textcolor{blue}{0.9678} \\
%CPNet of SVCNet (with $s$) & \checkmark & \textcolor{red}{34.89} & \textcolor{red}{0.9736} \\
