...
%-- abstract ---
\begin{abstract}
We present the numerical equivalence between the Wilson flow and stout-link smearing, both of which are known 
to be a relatively new technique for smoothing the gauge fields on the lattice.
...
Recently, two of our collaborators had found that there is numerical equivalence between the spatial Wilson flow~\footnote{The diffusion is restricted only to spatial directions.} and the stout-link smearing in the glueball spectroscopy~\cite{{Sakai:2022mgd},{Sakai:2022zdc}}. In Ref.~\cite{Sakai:2022zdc}, an %\sout{\color{red}rigorous} 
analytical proof of equivalence between the Wilson flow and the stout-link smearing was also provided in the zero limits of the lattice spacing $a$ and the stout-smearing parameter $\rho$.
However, it is not obvious that they remain equivalent even with finite parameters within some numerical precision. 
We revisit an analytical proof of the equivalence between the Wilson flow and the stout-link smearing 
at the zero limits of both the lattice spacing $a$ and the stout-smearing parameter $\rho$ as given in Ref.~\cite{Sakai:2022zdc}, and give another proof that the equivalence of the two methods is satisfied {\it even if the lattice spacing $a$ is finite.}
...
In this section, we provide another derivation of the Wilson-flow differential equation
from the stout-link smearing {\it at finite lattice spacing $a$} in the limit of $\rho\to0$. 
...
Since the variable $s=k\rho$ directly corresponds to the Wilson flow time $t$, the perturbative matching relation of the flow time $t$, and the smearing parameters $\rho$ and $n_{\rm st}$ as $t=\rho n_{\rm st}$ found in Ref.~\cite{Alexandrou:2017hqw} is also rigorously proved.
...
start at ${\cal O}(\rho)$, which induces ${\cal O}(a^2)$ corrections since the flow time $t=\rho n_{\rm st}$ 
has dimension length squared.
