...
Simultaneously, many pure language-only models have adopted simple decoder-only architectures with great success~\cite{liu2018generating,gpt3,glam} and with the added benefit of significant reduction in parameters~\cite{liu2018generating}.
...
This provides a simpler alternative to previous approaches~\cite{ albef,yu2022coca,singh2022flava,yuan2021florence}, where our model architecture is more compact and shared more broadly across tasks in the model. % and further allows for language-only decoder reuse.
...
We call the model as \ours (\textbf{M}ulti-T\textbf{a}sk \textbf{M}ulti\textbf{M}odal \textbf{U}nified \textbf{T}ransformer) to indicate that this simple approach is applicable across multiple multimodal tasks (Figure~\ref{fig:teaser}).
...
%and on the other hand, it represents a union of tasks that other models find it challenging to put together, or need more specialized of complex adaptations for. 
 ...
 \begin{figure*}
    \centering
   % \vspace{-0.7cm}
   \includegraphics[width=0.9\linewidth]{figures/Mammut_Teaser_video.png}
   % \vspace{-0.3cm}
...
With \ours, we enable all these tasks. 
...
%\begin{itemize}
  ...
  Extensions to these approaches, where masking is done in the image space~\cite{bao2021beit}, have also been considered. 
    ...
    %\item Combining contrastive and generative VLP.
      ...
      Furthermore, a second decoder uses cross-attention to connect image representations with text to generate the output text.
     ...
     demonstrate that large vision-language models are advantageous for detecting novel object categories. Inspired by this work, while taking a different approach, we show that open-vocabulary detection is an easy extension to our model with excellent performance. 
%\end{itemize}
%---------
\begin{figure*}
    \centering
...
In contrast to other architectures, Figure~\ref{fig:main_arch}, middle and right, we use a single vision encoder and a single text decoder. Unlike encoder-decoder models~\cite{pali,wang2022git,wang2022image} and contrastive alignment and captioning~\cite{albef,yu2022coca}, this is simpler and more flexible architecture. While using an encoder-decoder is very popular, it comes with disadvantages. For example, the pretrained visual encoder is a required component of the model, and contrastive tasks are not easily feasible. %, and adaptation to video is on a per-frame basis. % Additionally often an encoder-decoder is trained for text is adapted to an additional input.   
...
We note that this change does not need any additional video data pre-training, and all experiments conducted in the paper are done by directly fine-tuning of the \ours image-text model on a video dataset. % We also compare adding further video training (e.g. VideoCC-3M 
