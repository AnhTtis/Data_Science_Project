...
Notably, $\Delta$ is a constant in the 3D hybrid model.
...
In \cite{amoebot3Dcoating}, the authors solve the coating problem in the 3D Amoebot model.
In their approach, the object's surface is greedily flooded by amoebots that remain connected in a tree structure.
The process terminates for each amoebot when there are no more surrounding empty positions to move to.
Notably, given our agent's requirement to retrieve a tile after each placement, their approach cannot be applied or transferred to the hybrid model.

Deterministic finite automata that navigate the infinite 2D grid graph $\mathbb{Z}^2$ are considered in \cite{Czyzowicz2021NestFormation,Kant2021FortFormation}.
The authors of \cite{Czyzowicz2021NestFormation} address the challenge of building a compact structure, termed a nest, using available tiles.
They present a solution with a time complexity of $O(s^n)$, where $s$ denotes the initial span, and $n$ denotes the number of tiles.
In \cite{Kant2021FortFormation}, the authors focus on constructing a fort, a shape with minimal span and maximum covered area, in $\mathcal{O}(n^2)$ time.
