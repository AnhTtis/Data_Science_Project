...
Notably, this assumption does not weaken our results, since there are surfaces graphs in which properties (1) and (2) naturally arise, and otherwise we can emulate the properties using additional tile types (see \cref{sec:construction}).
...
\begin{figure}[t]
    ...
    \begin{subfigure}[c]{0.49\linewidth}
        ...
        \label{fig:idea2}
    \end{subfigure}%
    \caption{(a) Example of a triangulation $\tri{}$. (b) The simple path $\tau$ that starts at the material depot (hexagon) along the boundary of tiled nodes (opaque surface area). Links are depicted as circles. The blue area ($i$) is the \emph{range} explored by the agent. In the yellow area ($ii$), $\tau$ is `exposed' to nodes that are not tiled and not links. The agent explores the blue area to find an overlap with the yellow area, in which case there is a link that can be tiled while preserving connectivity of $\tri(\emp)$.}
    \label{fig:idea}
\end{figure}
...
Instead, our strategy is to place tiles such that a `narrow tunnel' of empty nodes remains to the tile depot, and use the left- and right-hand-rule (LHR, RHR) to navigate the resulting tile structure.
Essentially, we `mark' empty nodes by disconnecting tiles in their boundary such that they become part of the tunnel, which is formalized by the notion of \emph{links}:
...
The presence of a link in that range indicates that we have found one of the above mentioned `exposures' of $\tau$ that was already visited before, i.e., there exists a link in some cycle of $\tri{}(\emp{})$ that can safely be tiled (e.g., topmost link in \cref{fig:idea2}).
...
For simplicity, we treat $\bp$ and $\lp$ as ordered multisets, e.g., we write $v \in \lp$ if $\lp$ contains node $w$.
...
\begin{definition}
    \label{def:range}
    The $i$-\emph{range} of $p$ w.r.t. $q$ is the set of nodes $$R_i(p,q) \coloneqq \bigcup_{v \in N_i(p) \text{ w.r.t. $\tri(\emp \setminus \!\{q\})$}  }\bigcup_{\;\;w \in \bo(v)} \seg(v) \text{ w.r.t. $\tri(\emp \setminus \!\{q\})$ and anchor $w$.}$$
\end{definition}
...
With a slight abuse of notation we will use a superscript $^+$ ($^-$) to denote the next (previous) step $t^+$ ($t^-$) in which the agent gathers material at $p^0$ or terminates.%, e.g., denote by $\emp{}^+$ the set of empty nodes in step $t^+$.
...
Essentially, we make a case distinction on the type of node $v$ that is tiled in step $t$, i.e., whether $v$ is not a link, a link but no generator, or a link and a generator, and finally whether $v + \rhr{} \in \links \cup \{\start\}$ (see line 9).
...
We can then use the remaining properties that still hold in step $t^+$ (and the following corollary) to precisely determine at which node the next tile is placed (with $\noCheck{} = true$), and that this tile placement restores $P2$ in step $t^{++}$.
In the other cases the properties mostly follow from the following lemma and corollary:


\begin{lemma}
    \label{lem:generatorSharedTiled}
...
All properties hold in an intial configuration, and they are maintained as invariants by \cref{lem:invariant}.
