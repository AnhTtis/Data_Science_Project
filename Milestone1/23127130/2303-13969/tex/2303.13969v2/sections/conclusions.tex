...
If we consider an arbitrary function, the only error this algorithm makes is the initial discretization of said function as a finite sum of modulated gaussians. Moreover, the algorithm only relies on a small number of parameters whose time-evolution is explicit, making it very fast and computationally efficient.
...
Finally, our numerical examples show that the bubbles algorithm globally outperforms a spectral method combined with time splitting, where each splitting step is solved exactly.
