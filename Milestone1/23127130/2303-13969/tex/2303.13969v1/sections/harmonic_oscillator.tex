...
\cite{killipVisan2013,tao2006nonlinear}).  
...
\begin{proof}
    ...
    .\end{equation*}
    %
    Derivating this identity with respect to \( \lambda \) and evaluating the result at \( \lambda = 1 \) yields 
    \begin{equation*}
...
\begin{lemma}[Conserved quantities in dimension \( d=2 \)]
    ...
    The (radial) conservation laws are mass \( \|u\|_{ \mathbb{L}^2 } \), energy
    \begin{equation*}
        E_0 = \frac{1}{2} \langle H\psi, \psi \rangle, \quad \text{ where } H = -\Delta + |x|^2
    ,\end{equation*}
    %
    and momentum
    \begin{equation*}
        M_0 = -2E_0 \| x\psi\|^2_{ \mathbb{L}^2 } + \|x\psi\|^4_{ \mathbb{L}^2 } + \left( \Im \int x\cdot \nabla \psi(x)\bar{\psi}(x) dx \right)^2
    ,\end{equation*}
    %
    and the same applied to any power \( (-H)^s \psi \). There also holds the non radial conservation law 
    \begin{equation*}
        \mathcal{P}_j = \left( \Im \int \partial_{j} \psi\bar{\psi} \right)^2 + \left( \int x_j |\psi(x)|^2 dx \right)^2, \quad j=1, 2
    .\end{equation*}
    %
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        = \int |\nabla \psi|^2 - \int |x|^2 |\psi|^2 + \Re\left( \int x\cdot \nabla \psi \Delta \bar{\psi} \right)
        %2E_0 - 2\int |x|^2 |\psi|^2
    .\end{equation*}
    %
    We then use the Pohozaev identity \eqref{eqn: pohozaev identity}, which yields
    \begin{equation*}
        \frac{1}{2} \frac{\textd}{\textd t} \Im \int x\cdot \nabla \psi \bar{\psi} = \int |\nabla \psi|^2 - \int |x|^2 |\psi|^2 = 2E_0 - 2\int |x|^2 |\psi|^2
    .\end{equation*}
    %
    From this we obtain
    \begin{equation*}
        \frac{\textd^2}{\textd t^2} \Im \int x\cdot \nabla \psi \bar{\psi} = -16 \Im \int x\cdot \nabla \psi \bar{\psi}
    .\end{equation*}
    %
    Hence, the conservation laws
    \begin{align*}
        &\frac{1}{16} \left( \frac{\textd}{\textd t} \left[ \Im \int x\cdot \nabla \psi \bar{\psi} \right] \right)^2 +\left( \Im \int x\cdot \nabla \psi \bar{\psi}  \right)^2 \\
        &= \left( E_0 - \|x\psi\|^2_{ \mathbb{L}^2 } \right)^2 + \left( \Im \int x\cdot \nabla \psi \bar{\psi} \right)^2\\
        &= E_0^2 - 2E_0\|x\psi\|^2_{ \mathbb{L}^2 } + \|x\psi\|^4_{ \mathbb{L}^2 } + \left( \Im \int x\cdot \nabla \psi \bar{\psi} \right)^2.
    \end{align*}
    %
    Then
    \begin{equation*}
        ...
        = 2\Re \int |x|^2 \psi \overline{ \partial_{j} \psi }
        = -2 \int x_j |\psi|^2
    ,\end{equation*}
    %
    and 
    \begin{equation*}
        ...
        = \Im \int x_j i \partial_{t} \psi \bar{\psi} = \Im \int -\Delta \psi x_j \bar{\psi}
        = \Im \int \partial_{j} \psi \bar{\psi}
    .\end{equation*}
    %
    Hence the relations
    \begin{equation*}
        \left|
        \begin{aligned}
            \frac{\textd}{\textd t} \int x_j |\psi|^2 &= 2\Im \int \partial_{j} \psi \bar{\psi} \\
            \frac{\textd}{\textd t} \Im \int \partial_{j} \psi \bar{\psi} &= -2\int x_j |\psi|^2,
        \end{aligned}
        \right.
    \end{equation*}
    %
    ...
    \begin{equation*}
        \mathcal{P}_j = \left( \Im \int \partial_{j} \psi \bar{\psi} \right)^2 + \left( \int x_j |\psi|^2 \right)^2
    .\end{equation*}
    %
\end{proof}
...
\begin{align*}
    ...
    &\quad \times \left[ \partial_{k}^2 v + i \left( \left[ L\beta_k - \frac{B}{2} y_k \right] \partial_{k} v - i\frac{B}{2} v + i\left( L\beta_k - \frac{B}{2} y_k \right) \left[ \partial_{k} v + i \left( L\beta_k - \frac{B}{2} y_k \right) v \right]  \right) \right] \\
...
\begin{align*}
    -|x|^2 u
    &= -\frac{A}{L} e^{i\gamma + iL\beta\cdot y - i \frac{B}{4} |y|^2} \left| Ly + X(t) \right|^2 v \\
...
In pratice, one would know the parameters \( (A, L, B, X, \beta, \gamma) \) and would apply equation \eqref{eqn: lemma -- HO exact integration of parameters - update of parameters - with v} in order to update them. We have the following result, which allows to obtain action-angle variables from the bubble's parameters:
\begin{lemma}
...
\begin{lemma} 
    ...
    \begin{equation}
        ...
        \begin{aligned}
            \gamma(T) &= 
                \gamma(0) + \sum_{l=1}^d a_l(0) \left[ \sin(2\theta_l(T)) - \sin(2\theta_l(0)) \right] \\
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \sum_{l=1}^d a_l \left[ \sin(2\theta_l(T)) - \sin(2\theta_l(0)) \right]
    \end{align*}
    %
    and owing to \eqref{eqn: lemma -- HO exact integration of parameters - update of parameters - with v}:
    \begin{align*}
