...
\small
    ...
    \hline
      \textbf{Dataset} & \textbf{Synthetic} & \textbf{Captured} & \textbf{Videos} \\
    \hline \hline
    LLFF~\cite{llff} &   \xmark &\cmark & \xmark  \\
    \hline
    Blender~\cite{blender} &   \cmark &\xmark &  \xmark \\
    \hline
   Shiny~\cite{NeX} &   \xmark &\cmark & \xmark\\
    \hline
    DeepView’s Spaces~\cite{deepview} &  \xmark &\cmark & \xmark\\
    \hline
    Synthetic-NSVF~\cite{NSVF} &  \cmark &\xmark &  \xmark \\
    \hline
    Blend-MVS~\cite{blendedmvs} &   \cmark &\xmark &  \xmark\\
    \hline
    DTU~\cite{dtu} &  \xmark  &  \cmark &  \xmark\\
    \hline
    Ours  &  \cmark &\cmark & \cmark\\
    \hline
    \end{tabular}
    \caption{A comparison of \datasetName{} dataset (Ours) with other datasets widely used in the evaluation of NVS methods. Our dataset consists of scenes collected in a lab environment as well as natural indoor and outdoor environments with smooth reference videos and accurate camera poses for each frame. 
    \RM{I do not think this table is very useful in the current format. To make it more useful, include the number of scenes, the number of training and testing views. You should mention 360 and FF. Ticks take too much space --- make one column with short labels: syn-real-img (or similar). If we lack space, I would remove this table.} \CO{Agreed, we should include details on the datasets. Also, I don't think we introduce a new synthetic dataset.} \FB{Here we still have real-world; fieldwork here too? I changed from real-world to captured}}
    \label{tab:data}
\end{table}
 \fi
...
\iffalse
\begin{table}
\caption{NVS methods used in our evaluation. \FB{we should add either here or in 3.3 what GNT-C, GNT-S, IBRNet-C, IBRNet-S are.}}
\resizebox{\columnwidth}{!}{
\begin{tabular}{|c|l|}
\hline
\textbf{Method} & \textbf{Attributes} \\
      \hline \hline

NeRF &Differentiable volume rendering with MLP to map \\
\cite{original_nerf} & position and direction into density and radiance \\ \hline
MipNeRF & Anti-aliased conical frustums that reduce aliasing  \\
\cite{mipnerf} &  artifacts and represent finer details\\ \hline
Plenoxel &Explicitly store density and learned radiance in \\
\cite{plenoxel} & 3D grid \\ \hline
DVGO & A hybrid 3D representation to estimate density and\\
\cite{dirctvoxgo} & radiance space \\ \hline
NeX &Learnable spherical basis functions in an implicit  \\
\cite{NeX} & multi-plane image representation \\ \hline
LFNR &Light field representation and epipolar constraint  \\
\cite{lfnr} & to guide rendering \\ \hline
IBRNet &MLP and ray transformer for generalizable models  \\
\cite{ibrnet} & across different scenes \\ \hline
GNT &A unified transformer-based architecture for \\
\cite{gnt} &  generalizable NeRF \\ \hline
\end{tabular}
}
    \label{tab:nerfmethods}
\end{table}
\fi
...
We scaled the results of the pairwise comparison and expressed the subjective evaluation score in Just-Objectionable-Difference (JOD) units using the Thurstone Case V observer model \cite{pwcmp}. A difference of 1\,JOD unit here means that 75\% participants preferred one method over another. The model assumes that participants made their selections by assigning a single quality value to each video and approximates this quality by a normally-distributed random variable with the same inter- and intra-observer variance.
...
It is noteworthy that the discrepancies among the methods are more noticeable in the more  challenging \dsfieldwork{} dataset, which implies that a challenging dataset is essential to distinguish between different methods.
...
\includegraphics[width=\columnwidth]{img/scatter_main.pdf}
    \caption{Selected metric correlations for our \dslab{} (top row) and \dsfieldwork{} (bottom row) datasets. On both two datasets, we observe a stronge correlation between .}
...
\begin{figure*}%[tb]
    ...
    \includegraphics[width=0.9\linewidth]{img/jod_correlations.pdf}
    \caption{Bootstrapped distributions of correlation coefficients for all metrics
    computed on (a) \dslab{}, (b) \dsfieldwork{}, and (c) \dsllff{}. The ``+" in black denotes unbiased mean correlation ~\cite{unbiased}, and ``{\color{red}-}" in red denotes the 5th percentile (an estimate of the bad-case performance). Full-reference video metrics are missing for \dsllff{} because this dataset has no groudtruth videos. The lines connect metrics where differences cannot be deemed statistically significant in a non-parametric test, with a $p$-value threshold of 0.05.}
...
Specifically, PSNR tends to be less sensitive to the foggy artifact produced by NVS methods, as shown in the top row middle column,
%middle column of 
