...
\begin{document}
\title[Real algebraic functions which may have non-compact preimages]{Real algebraic functions which may have both compact and non-compact preimages}
...
Our answers are new in having real algebraic functions on non-compact manifolds with no boundaries and considering real algebraic functions on manifolds which may not be real algebraic, for example.  
...
\label{sec:1}
\subsection{A short presentation on real algebraic manifolds and maps and Nash ones.}
...
\begin{MainThm}
	...
	Let $l>1$ be an integer, $\{t_j\}_{j=1}^l$ an increasing sequence of real numbers, and $l_{{\mathbb{N}}_{l-1}}:{\mathbb{N}}_{l-1} \rightarrow \{0,1\}$ be a map. Then we have a suitable smooth connected Nash manifold $M$ with no boundary and a smooth Nash map $f:M \rightarrow \mathbb{R}$ enjoying the following properties.
	\begin{enumerate}
	...
	\end{enumerate}
	\end{MainThm}
For this, especially, a smooth case \cite{kitazawa2}, in which we have constructed smooth functions with prescribed preimages on non-compact manifolds with no boundaries, has motivated. Note that \cite{kitazawa3, kitazawa4} are cases where manifolds are smooth, real algebraic and closed.
...
\subsection{Important subsets in the real affine space ${\mathbb{R}}^2$.}
...
\subsubsection{$C_{{\rm E},a,b,c_1,c_2}$.}  Let $a$ and $b$ be real numbers. Let $c_1>0$ and $c_2>0$ be positive numbers.
We consider the subset in ${\mathbb{R}}^2$ represented
as $\{(x_1,x_2) \in {\mathbb{R}}^2 \mid \frac{{(x_1-a)}^2}{c_1}+\frac{{(x_2-b)}^2}{c_2}=1\}$. Let it be denoted by $C_{{\rm E},a,b,c_1,c_2}$. This is a $1$-dimensional smooth real algebraic manifold and a curve and has no boundary. This is connected. This is for an ellipse.
...
We also prove the following theorem as another main result.
\begin{MainThm}
\label{mthm:2}	
...
\item \quad $t_2+a<t_3$.
\item \quad $(t_2-t_3)(b_3+\frac{c_2}{t_2-(t_2+a)}-b_2)=c_2$. It is simplified as $(t_3-t_2)(b_3-b_2+\frac{c_2}{a})=c_2$. This means that the point $(t_2,b_3+\frac{c_2}{t_2-(t_2+a)})$ is a point in the curves $C_{{\rm H}_{+},[t_3,\infty),b_2,c_2}$ and $C_{{\rm H}_{+},[t_2+a,\infty),b_3,c_2}$.
\item \quad $b_3+\frac{c_2}{t_4-t_2-a}<b_1$. This respects the definitions of $C_{{\rm H},[t_2+a,\infty),b_3,c_2}$ and $C_{{\rm H}_{+},[t_2+a,\infty),b_3,c_2}$. Remember also that $c_2<0$ is chosen as a sufficiently small number. 
\end{itemize}
...
\indent We apply Case 1 by replacing "$t_2$" by "$t_l$" with "$l_{{\mathbb{N}}_{1}}(1)=0$" there or Case 2 by replacing "$t_2$" and "$t_3$" by "$t_{l-1}$" and "$t_l$", respectively. 
For each integer $2 \leq j \leq l-2$ in Cases 1 and 2 with $l \geq 4$, we choose $C_{\rm E,\frac{t_j+t_{j+1}}{2},b_{j,t_j},\frac{(t_{j+1}-t_{j})^2}{4},c_{j,t_j}} \times [-R,R]$ for a suitable real number $b_{j,t_j}$ and a sufficiently small number $c_{j,t_j}>0$ making the set as a sufficiently small subset contained in the interior of the set "$D_0$" in these cases. We remove the interiors of the subsets to have a new set $D_1$. By defining $D$ as the interior of $D_1$ and applying Proposition \ref{prop:1}, this completes the case in the case $l_{{\mathbb{N}}_{l-1}}(j)=0$ for any $j$. To prove in the general case, for some $j$, we need to choose another set $C_{\rm E,\frac{t_j+t_{j+1}}{2},{b_{j,t_j}}^{\prime},\frac{(t_{j+1}-t_{j})^2}{4},{c_{j,t_j}}^{\prime}} \times [-R,R]$ for a suitable real number ${b_{j,t_j}}^{\prime}$ and a sufficiently small number ${c_{j,t_j}}^{\prime}>0$ making the set as a sufficiently small subset contained in the interior of the set $D_1$ here. We also need to remove this closed set for each $j$ to make the situation $l_{{\mathbb{N}}_{l-1}}(j)=1$. We have an open subset $D_2 \subset {\mathbb{R}}^3$ and we set $D:=D_2$ to complete the proof in this case. \\
\ \\
...
In applying Case 1,  we apply by replacing "$t_2$" by "$t_l$" with "$l_{{\mathbb{N}}_{1}}(1)=0$" there.
...
In applying Case 3,  we replace "$t_3$" by "$t_l$" and we do not change the role of "$t_2$" there.
We can argue as before for each integer $2 \leq j \leq l-2$ to show the case $l \geq 4$ with the condition $l_{{\mathbb{N}}_{l-1}}(1)=0$ and $l_{{\mathbb{N}}_{l-1}}(l-1)=1$. For Case 4, we can do similarly to show the case $l \geq 4$ with the condition $l_{{\mathbb{N}}_{l-1}}(1)=1$ and $l_{{\mathbb{N}}_{l-1}}(l-1)=0$ being satisfied.

In applying Case 6, we  replace "$t_j$" by "$t_{j+l-4}$"for $j=3,4$ and we
do not change the role of "$t_2$" there.
...
\ \\
We have a desired Nash map for Main Theorem \ref{mthm:2} and we also have Main Theorem \ref{mthm:1} by our construction. This completes the proof. 
...
\bibitem{saeki} O. Saeki, \textsl{Reeb spaces of smooth functions on manifolds}, International Mathematics Research Notices, maa301, Volume 2022, Issue 11, June 2022, https://doi.org/10.1093/imrn/maa301, arXiv:2006.01689.
