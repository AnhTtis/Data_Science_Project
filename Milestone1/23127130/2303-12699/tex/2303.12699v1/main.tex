...
\begin{document}
	...
	\end{theorem}
	We also have the following equivalence result between categories of higher sheaves on derived manifolds and dg and categories of higher sheaves on the derived Cartesian site \(d\Cart.\)
	\begin{theorem}[{Theorem \ref{thm:eq_dMan}, Proposition \ref{pro:equiv_dg_Cart}}]
		The topos of higher sheaves on the relative site \(\dMan\) of derived manifolds in the sense of Behrend--Liao-Xu \cite{behrend2020derived} is Quillen equivalent to the topos of sheaves on the derived Cartesian site \(d\Cart.\) This topos is also Quillen equivalent to the higher sheaf topos over the site of non-negatively graded dg manifolds in the sense of \AKSZ\;\cite{alexandrov1997geometry}.
	\end{theorem}
	...
	\par Section \ref{sec:apps} contains applications of Theorem \ref{thm:site_eq} to concrete Fermat theories. In \S\ref{subsec:eq_synth}, we show that all models of smooth infinitesimal analysis of Moerdijk--Reyes \cite[Appendix 2]{moerdijk2013models} can be promoted to simplicial, DG, and semifree DG models and define the same (up to Quillen equivalence) categories of sheaves. In \S\ref{subsec:eq_dg}, we show that sites of non-negatively graded dg manifolds and \(d\Cart\) produce Quillen equivalent topoi of higher sheaves. \S\ref{subsec:eq_der} establishes the equivalence of our approach to derived differential geometry with that of Behrend--Liao-Xu \cite{behrend2020derived} (Theorem \ref{thm:eq_dMan}). In particular, we prove that weak equivalence between derived manifolds in both senses coincide (Corollary \ref{cor:conj}). Finally, Section \ref{sec:dif_coh} contains a straightforward definition of differential cohomology for derived differentiable stacks. 
	\subsection*{Acknowledgements}
	First, we want to express our deep gratitude to our advisor Dmitri Pavlov for posing the problem and explaining how to solve it. Second, we want to thank our partner Alisa Chistopolskaia for providing constant moral support. Third, we want to thank Sander Kupers for discussing Quillen's "Rational Homotopy Theory" with us. Finally, we want to recognize the government of Canada and IRCC specifically for helping us to remain unburdened by such irrelevant things as seeing our family or higher education while preparing this text. 
	...
	\begin{definition}\label{def:coverage}
		...
		\begin{itemize}
			\item If \(\{f_i:X_i\to X\}_{i\in I}\) is a covering family and \(g:Y\to X\) is a morphism, then there exists a covering family \(\{h_j:Y_j\to Y\}\) such that each composite morphism \(gh_j\) factors through some \(f_i.\)
			\[\begin{tikzcd}
				{Y_j} && {X_i} \\
				\\
				Y && X
	...
	\begin{definition}[{\cite[Definition A.1.1, Definition  4.1.1]{toen2005homotopical}}]\label{def:(pseudo)MC} For a universe \(\U,\) a \(\U\)-category (see \cite{artin1973theorie}) is a category with all sets of morphisms belonging to \(\U\) and the set of objects being a subset of \(\U.\) A {\it \(\U\)-small model category} \(M\) is a category with a closed model structure in the sense of \cite[Definition 1.1.3]{hovey2007model} such that \(M\) is a \(\U\)-category and there are all \(\U\)-small limits and colimits.
	\end{definition}
	\begin{remark}
	...
	\begin{definition}[{\cite[Corollary 4.6.2]{toen2005homotopical}}]\label{def:sheaves} Let \((C,S)\) be a small relative category. If \((C,S)\) carries a homotopical coverage \(K\) the left Bousfield localization of \((C,S)^\wedge\) in the class
		\[
		\L=\{\y(\F_\bullet)\to \y(x)\mid x\in C_f\}
		\]
		where \(\F_\bullet\to \y(x)\) is a homotopical hypercover will be referred to as the {\it category of stacks on \((C,S)\)} and will be denoted by \(\Sh((C,S);\K).\) By \cite[Theorem 4.6.1]{toen2005homotopical} this localization exists. 
	\end{definition}
	\begin{remark}
		A classical account of descent for hypercover on sites is in \cite[F. 1, E. 5, App.]{artin1973theorie} and \cite[\S 8]{artin2006etale}. A more modern exposition relevant for homotopy theory can be found in~\cite[\S 4 onwards]{dugger2004hypercovers} and \cite[\S 4.4]{toen2005homotopical}.
	\end{remark}
	Now we show that the choice of either injective or projective model structure\footnote{The same result with essentially the same proof holds for arbitrary intermediate model structures on presheaves in the sense of \cite{jardine2006intermediate}.} on the category of prestacks is irrelevant to the definition of sheaf topoi. 
	\begin{lemma}\label{pro:inj_proj_equiv}
		...
		The identity functors induce an equivalence between the following model categories
		\begin{enumerate}
	...
	\end{lemma} 
	\begin{proof}
		(1) is a standard result about model structures on presheaf categories (see, e.g., \cite[Lemma A.2.8.3]{lurie2009higher}). All projective cofibrations are objectwise (injective) cofibrations and hence the identity functor is a left Quillen functor. Since it also reflects equivalences it is a Quillen equivalence. 
		\par (2) follows from the following standard result about the functoriality of Bousfield localization (see, e.g., Hirschhorn \cite[Theorem 3.3.20]{hirschhorn2003model}). Let \(M\overset{F}{\underset{U}{\rightleftarrows}} N\) be a Quillen equivalence. Assume that \(S\) is a class of morphisms in $M.$ Then there is an induced Quillen equivalence:
		\[
	...
	\begin{definition}
		...
		\{\beta_i:V_i\to f(X)\} 
		\) in \(L^{-1}D\)
		there exists a covering family \(\{\alpha_j:U_j\to X\}\) in \(S^{-1}C\) such that each \(\alpha_j\) factors through some \(\beta_i\) in~\(L^{-1}D.\)
	\end{definition}
	...
	\begin{theorem}\label{thm:eq_sh_top}
		...
		\begin{enumerate}
			\item \(f\) is a Dwyer--Kan equivalence (see \cite{dwyer1980simplicial}) of relative categories.
	...
	\end{theorem}
	\begin{proof}
		By T\"{o}en--Vezzosi \cite[Corollary 2.3.6]{toen2005homotopical} (original ideas are due to Dwyer--Kan \cite{dwyer1987equivalences}) the first condition implies that there is a Quillen equivalence between prestack categories 
		\[
	...
	\begin{definition}[{\cite[\S 1]{dubuc19841}}]\label{def:fermat}
		...
		\]
		Here \(l_1,l_2:\iota\AA^2_\Z\to\iota\AA^1_\Z\) are the images of two projections and the ring operations are the images of corresponding operations in \(\Com_\Z.\)
	\end{definition}
	...
	\begin{proposition}[{\cite[Proposition 1.2]{dubuc19841}}]
		For any ring-theoretic ideal \(I\) in a \(\Tc\)-algebra \(A\) there is a natural structure of a \(\Tc\)-algebra on \(A/I\) that makes \(A\to A/I\) an \(\Tc\)-homomorphism.
	\end{proposition}
	...
	\end{definition}
	\begin{remark}
		The peculiar choice of the category \(\Tc\Alg_r\) is justified by our construction of the left adjoint functor \(\Qc\) in the Dold--Kan correspondence (see \S\ref{sec:DK_Fermat}). Morally, we just want the category \(\Tc\Alg_r\) to be small so that constructions with sheaf topoi work as planned.  
	\end{remark}
	...
	\end{lemma}
	\begin{proof}
		This follows from the fact that this statement is true for commutative algebras and the completion functor (Construction \ref{con:completion}) is left adjoint, hence it preserves colimits.
	\end{proof}
	\subsubsection{Examples}\label{sec:Fermat_examples}
	There is a vast array of different examples and applications of our result. We list only the three most fundamental ones, other examples can be found in \cite[\S 2.2.3]{carchedi2012theories}. The key reason why Fermat theories are so useful is that they allow a uniform construction of complex geometric objects (e.g., higher sheaves) from "affine models", for more detail see \cite[Introduction]{carchedi2012theories}. 
	\begin{example}\label{ex:ag}
		The simplest example of a Fermat theory that we already encountered is that of \(\Com_R\) (see Definition \ref{def:RCom}). This is the theory of commutative \(R\)-algebras for a commutative ring \(R.\) In particular, the theory \(\Com_\Z\) plays the role of the initial Fermat theory. Geometry over \(\Com_\Z\) reproduces the standard scheme-theoretic algebraic geometry.
	\end{example}
	\begin{example}\label{ex:cinf}
		Another very important example is provided by the Fermat theory \(\Cc^\infty\) defined via
		\[
	...
	\begin{example}\label{ex:hol}
		The final theory we discuss is the theory of holomorphic functions \(\O:\)
		\[
		\O(m,n)=\O(\C^m,\C^n).
		\]
	...
	\begin{construction}
		...
		\]
		The set \(\Specm A\) (called the {\it maximal spectrum} of \(A\)) can be topologized in a natural way using the {\it Zariski topology}, i.e., we declare the generating collection of open sets for this topology to be
		\[
	...
	\end{construction}
	\begin{remark}
		Construction \ref{con:specm_topology} is relevant for Examples \ref{ex:cinf}, \ref{ex:hol}, and less so in Example~\ref{ex:ag}. 
	\end{remark}
	Another construction is more convenient for algebro-geometric purposes.
	\begin{definition}
		A ring-theoretic ideal \(I\triangleleft A\) in an algebra \(A\) over a Fermat theory \(\Tc\) is called {\it radical} if 
		\[
		I=\sqrt{I}:=\{a\in A\mid \exists n\in \Z_{\ge 0},\; a^n\in I\}.
		\]
	\end{definition}
	...
	\end{construction}
	\begin{lemma}[{\cite[Proposition 3.1]{tierney1976spectrum}}]
		A poset \(\Zar(A)\) of radical ideals for an algebra \(A\) over a Fermat theory \(\Tc\) is a frame in the sense of \cite[\S IX.1]{maclane2012sheaves}. The construction \(\Zar(A)\) can be promoted to a functor
		\[
	...
	\begin{construction}\label{con:spec_topology}
		...
		A set of morphisms \(\{f_i:B_i\to A\vert i \in I\}\) is a {\it covering family} for a \(\Tc\)-algebra \(A\) if it is dual to an open cover of locales (in the sense of \cite[\S IX]{maclane2012sheaves}):
		\[
	...
	\begin{proposition}\label{prop:model_struct_on_sFermat}
		...
		\)
		Denote by \(\ol{R}[-]\) the reduced simplicial chains functor \(\sSet_*\to s\Mod_R.\) Then there is a model structure on \(s\Tc\Alg\) with weak equivalences created by the forgetful functor \(\UU:s\Tc\Alg\to \sSet_*\) and generating cofibrations given by inclusions:
		\[
	...
	\end{proposition}
	\begin{proof}
		For DGAs this is the standard result that can be found in, e.g., \cite[\S V.3]{gelfand2013methods}. For simplicial algebras this is also standard and follows from, e.g., \cite[Remarks II.4.2]{quillen2006homotopical}.
	\end{proof}
	...
	\begin{definition}[{\cite[Definition 6.10]{quillen1970co}}] Let \(A\) be a commutative ring and let \(\Lambda_A\) denote the exterior \(A\)-algebra functor on the category of \(A\)-modules. 
	...
	\begin{proof}
		...
		\end{tikzcd}\]
		Here \(\EZ\) is the Eilenberg--Zilber map. The associativity and graded-commutativity properties of the Eilenberg--Zilber map yield associativity and graded commutativity of \(\wh{\mu}.\) The unit of \(\N(\A)\) is defined as the image of \(1_\A\) in \(\N(\A)_0=\A_0.\)
	\end{proof}
	...
	\begin{proposition}\label{prop:existence_and_freeness}
		The functor \(\N\) has a left adjoint \(\Qc.\) Consider the standard cofibrantly generated model structures of Proposition \ref{prop:standard_model_struct} on \(\DGCAlg\) and \(\sCAlg.\) Then the functor \(\Qc\) carries cellular objects in \(\DGCAlg\) into cellular (and free) objects in \(\sCAlg.\)
	\end{proposition}
	\begin{proof}
	...
	\begin{proof}
		...
		\]
		By Proposition \ref{prop:existence_and_freeness} the object \(\Qc\A\) is free as a graded algebra and thus
		\[
		...
		\par Here we adopt the terminology of \cite[\S 3]{gwilliam2018enhancing}. Let \(\ol{\A}^0=\A\) and \(\ol{\N\Qc\A}^0=\N\Qc\A\). We have two cofibrant sequences:
		\begin{gather*}
			\ul{\A}(n)=\begin{cases}
				\A,& n\ge 0;\\
				\ol{\A}^{-n},& n<0
	...
	\begin{notation}
		Let \(\Tc\) be a Fermat theory over \(K\) (Definition \ref{def:Fermat_over_R}), i.e., an object of the undercategory \(\Com_K/\FTh.\) Let \(\dg\Tc\Alg\) denote the subcategory of \(\DGCAlg\) consisting of objects with a structure of \(\Tc\Alg\) in degree \(0\) and morphisms that respect this structure. Also denote by \(T_0\) the {\it ring of coefficients} for a Fermat theory \(\Tc,\) i.e., the image under the Yoneda embedding of the \(0\)'th object in \(\Tc.\) 
	\end{notation}
	...
	\end{proposition}
	\begin{proof}
		This is proved in \cite[\S 6]{carchedi2012homological}.
	\end{proof}
	\begin{proposition}
	...
	\end{proposition}
	\begin{proof}
		If \(\A\) is a dg-algebra over \(\Tc.\) If \(x\in \A_p,\) then \(\Gamma x\in \Gamma\UU\A\sse \FF\Gamma\UU\A\) is the element corresponding to \(x\) under the identification \(N\Gamma\UU\A=\UU\A.\) Clearly for any object \(\B\) in \(\s\Tc\Alg\) there is a bijection between morphisms \(\phi:\A\to \N\B\) in \(Ch_K\) and morphisms \(\theta:\FF\Gamma\UU\A\to \B\) in \(\s\Tc\Alg\) such that
		\[
		...
		\end{equation}
		Here \(\II(\A)\) is the simplicial \(\Tc\)-ideal generated by \[\II(\A)=(\wh{f}(\Gamma x_1,\ldots, \Gamma x_n)-\Gamma f(x_1,\ldots,x_n)\mid x_1,\ldots,x_n\in \A)\] for all operations \(f\) of \(\Tc\) that can be transferred along \(\N\) (i.e., all operations in degree \(0\) and multiplications in positive degrees). Observe that the adjunction morphism \(\beta:1_{\dg\Tc\Alg}\Rightarrow\N\Qc\) is given by
		\[
		\A\xrightarrow{\beta} \N\Qc\A,\quad x\xmapsto{\beta}\Gamma x+\II(\A).
		\]
	...
	\end{proposition}
	\begin{proof}
		First, observe that the equality \(T_0\otimes_K\pi_*(\SS V)=\pi_*(\SS T\otimes_K V)\) is just a standard result about flat base change.
		Now, any simplicial vector space can be obtained as a transfinite composition of cobase changes
		...
		\end{equation}
		Moreover, in each dimension we can explicitly describe \(\ker d_i\) for \(\SS\Gamma\ol{K}[\mc{S}^n]\) as well as for \(\FF\Gamma\ol{K}[\mc{S}^n].\) Let \(x_n\) be the generator of \(\ol{K}[\mc{S}^n].\) By Hadamard's lemma \(\ker d_i^k\) is given by
	...
	\begin{definition}
		Denote by \(s\Tc\Alg_r\) the category  of simplicial objects in the category \(\Tc\Alg_r\) (see Definition \ref{def:res_TAlg}) with an additional assumption that the functor \(\pi_*\) yields finitely generated \(\Tc\)-algebras for all objects in this category. 
	\end{definition}
	\begin{construction}\label{con:cov_on_sTAlg}
		A collection \(\{f_i:B_i\to A\vert i\in I\}\) is a covering family of a simplicial \(\Tc\)-algebra \(A\) if 
		\begin{itemize}
	...
	\end{proposition}
	\begin{construction}
		The category \(s\Tc\Alg\) carries a model structure transferred from the category \(s\Vect_K\) along the functor \(\UU.\) We restrict \(s\Tc\Alg_r\) to give this category the structure of a \(\U\)-small model category. 
	\end{construction}
	\subsection{Commutative differential \(\Tc\)-algebras}
	Another way to construct affine derived \(\Tc\)-spaces is to consider them as duals of differential \(\Tc\)-algebras.
	\begin{definition}
	...
	\end{theorem}
	\begin{proof}
		This is the standard result about inclusion of the subcategory of cofibrant objects into a model category. See, for instance, \cite[Corollary 10.4]{dwyer2004homotopy}.
	\end{proof}
	\begin{proposition}\label{pro:DK_preserves_coverages}
		The functors \(\Qc,\N\) in the Dold-Kan correspondence preserve the coverages on categories \(s\Tc\Alg^{\op}_r\) and \(DG\Tc\Alg^{\op}_r.\) That is, if \(\{\B_i\to\A\}\) is a covering family in \(s\Tc\Alg^{\op}_r,\) then \(\{\N\B_i\to \N\A\}\) is a covering family in \(DG\Tc\Alg^{\op}_r\) and the analogous statement holds for \(\Qc.\)
	\end{proposition}
	\begin{proof}
		Observe that since homotopy groups of objects in \(s\Tc\Alg_r\) are computed at the level of underlying simplicial sets and all such sets are fibrant we have
		\[
		\pi_*(\A)=H_*(\N \A),\quad \A\in s\Tc\Alg_r.
		\]
		Similarly, for objects in \(DG\Tc\Alg_r\) we have
		\[
	...
	\end{proof}
	\begin{corollary}
		There is a Quillen adjunction between sheaf categories (see Definition \ref{def:sheaves}) induced by the adjunction of sites
	...
	In this section, we list several possible sites for derived stacks over the Fermat theory \(\CI\) (see Example \ref{ex:cinf}) following \cite[Appendix 2]{moerdijk2013models}. We then apply Theorem \ref{thm:site_eq_gen} to establish equivalences between sheaf topoi over some of those sites. It is customary to refer to algebras over the theory \(\CI\) as {\it \(\CI\)-rings}; we adhere to this convention. 
	\begin{notation}
	...
	\begin{construction}\label{con:cinf_sites}
		...
		\end{enumerate}
	\end{construction}
	All the categories listed above are defined to be closed under isomorphism. We have the following diagram of inclusion functors (see \cite[Discussion on p.364]{moerdijk2013models}). Single arrows mean that there is no right adjoint; all other inclusions are coreflective:
	...
	\begin{definition}\label{def:der_sites}
		...
		\par Then its simplicial derived version \(s\MM\) is the full subcategory of \(s\CI\Alg_r\) spanned by algebras weakly equivalent to ones with \(\pi_0\) belonging to \(M.\)
		\par Analogously we define DG derived version \(DG\MM\) of \(M\) by taking the full subcategory of \(DG\CI\Alg_r\)  spanned by algebras weakly equivalent to ones with \(H_0\) belonging to \(M.\)
	...
	\begin{theorem}\label{thm:eq_for_models}
		Derived simplicial, DG and semifree DG versions of any of the ten sites given by the above construction produce equivalent model topoi of stacks. 
	\end{theorem}
	\begin{proof}
		Since the categories of \(DG\MM\) and \(s\MM\) are mapped to one another by the pair \((\Qc,\N)\) and this pair of functors preserves the homotopical coverages, Theorem \ref{thm:site_eq_gen} applies, and we get the desired equivalence. For the inclusion of the semifree site, we use the standard technique involving the deformation of the identity functor via the cofibrant replacement \cite[\S 2]{riehl2014categorical} to see that there is an equivalence of homotopical categories. Subsequently we can apply \cite[Theorem 2.3.1]{toen2005homotopical}.
	\end{proof}
	\subsection{Deriving models for analysis}
	Obviously, all categories of Construction~\ref{con:cinf_sites} and coverages of Construction~\ref{con:cinf_top} can be defined for Examples \ref{ex:ag}, \ref{ex:hol} and even more generally for reduced Fermat theories (see \cite[Definition 2.28]{carchedi2012theories}). The difference between some of the categories can disappear. For example, for finitely generated algebras over \(\Com_K\) (\(K\) is a field) there is no difference between the categories \(\mb{V}\) and \(\mb{V}_\omega\) because all finitely generated \(\Com_K\)-algebras are Noetherian. It is also clear that Theorem \ref{thm:eq_for_models} can be adapted without any change to this setting if the Fermat theory in question is considered over a field of characteristic \(0.\)
	This idea can be formalized in the following result.
	\begin{theorem}\label{thm:eq_for_models_gen}
		Assume that we have a site \(\mb{M}\) consisting of finitely presented \(\Tc\)-algebras for a Fermat theory \(\Tc\). Then we can construct three derived sites \(DG\MM,\) \(SFDG\MM,\) and \(s\MM\) as in Definition \ref{def:der_sites}. If \(\Tc\) is a theory over a field of characteristic zero, these three sites produce equivalent model topoi of sheaves with the equivalence being induced by the pair \((\Qc,\N).\)
	\end{theorem}
	\begin{proof}
		The proof is essentially the same as the proof of Theorem \ref{thm:eq_for_models_gen}. We observe that \((\Qc,\N)\) map \(DG\MM\) and \(s\MM\) to one another and that homotopical coverages are preserved by construction. Hence Theorem \ref{thm:site_eq_gen} is applicable, and the result is proved. The equivalence with sheaves on the semifree site is proved verbatim as in Theorem~\ref{thm:eq_for_models}.
	\end{proof}
	...
	\begin{definition}[{\cite[Definition 1.15]{behrend2020derived}}]\label{def:g_pos_man}
		...
		\]
		and a covering of \(M\) by opens \(\{U_i\to M\}_{i\in I}\) such that 
		\[
		\As\vert_{U_i}\cong \CI(U_i)\otimes \SS V^\vee,\quad \text{for all } i\in I.
		\]
		as a sheaf of \(\CI(M)\)-algebras.
	\end{definition}
	We assume \(\M\) to finite-dimensional, i.e., both \(M\) and \(V\) have finite dimension. The following definition is originally due to Schwarz \cite{schwarz1993semiclassical} and Kontsevich; it was then expanded upon by \AKSZ\cite{alexandrov1997geometry}.
	...
	\begin{proof}
		...
		\]
		Here \(l\) denotes the usual locus of an ordinary \(\CI\)-ring (see \cite[\S II.1]{moerdijk2013models}) and \(\chi_U\) is the function which is zero precisely on the complement of the open subset \(U.\)
	...
	\end{proof}
	Proposition \ref{pro:equiv_dg_Cart} allows us to transfer the structure of a relative site on \(d\Cart\) to \(\dgCart.\) For this structure we have the following equivalence result.
	\begin{proposition}
		...
		\[
		\CI_!:\Sh(\dgCart)\rightleftarrows\Sh(d\Cart):(\CI)^*.
		\]
	\end{proposition}
	\begin{proof}
	...
	\begin{definition}[{\cite[Definition 1.1]{behrend2020derived}}]
	...
	\end{definition}
	\begin{definition}[{\cite[Definition 1.2]{behrend2020derived}}]\label{def:CL}
	...
	\end{definition}
	\begin{definition}[{\cite[Definition 1.3]{behrend2020derived}}]
	...
	\end{definition}
	\begin{definition}[{\cite[Definition 1.9]{behrend2020derived}}]
		...
		\[
		T_{\lambda_0(P)}L_1=T_P M\oplus L^1\vert_P.
		\]
		...
		\]
	\end{definition}
	\begin{definition}[{\cite[Definition 1.12]{behrend2020derived}}]\label{def:we_dMan}
	...
	\end{definition}
	\begin{definition}[{\cite[Definition 1.21]{behrend2020derived}}]\label{def:fib_dMan}
	...
	\end{definition}
	\begin{construction}[{\cite[\S 1.2]{behrend2020derived}}]
	...
	\end{proof}
	\begin{notation}
		Denote by \(\dMan\) the category of derived manifolds with distinguished classes of weak equivalences and fibrations defined above. Denote by \(\dCart\) the full subcategory of \(\dMan\) spanned by derived manifolds with a Cartesian base.
	\end{notation}
	...
	\end{proof}
	By Behrend--Liao-Xu \cite[Theorem 2.15]{behrend2020derived} the category \(\dMan\) forms a category of fibrant objects in the sense of Goerss--Jardine \cite[\S I.9]{goerss2009simplicial} (originally due to Brown \cite{brown1973abstract}). The category \(\dCart\) is not stable under taking pullbacks, but still carries distinguished classes of weak equivalences and fibrations (Definitions \ref{def:we_dMan}, \ref{def:fib_dMan}).  
	...
	\begin{proof} 
		...
		\[
		T_P^*\dl(\A)=\Omega_{\R}^1(\A)=\ker P/(\ker P)^2.
		\]
		Since \(\A\) is cofibrant and \(\Omega^1_\R\) is a left Quillen functor, we have a quasi-isomorphism of cotangent complexes:
		\[
	...
	In this section, we show that the semifreeness of objects spanning the Cartesian site \(d\Cart\) allows us to easily define differential cohomology for derived differentiable stacks. For a general treatment of differential forms on derived algebraic stacks see Calaque--Pantev--To\"{e}n--Vaqui\'{e}--Vezzosi \cite[\S 1.3.2, 1.4.1]{calaque2017shifted}.
	\begin{definition}\label{def:dif_forms}
		Let \(\Rc=l(\A)\) be an object of \(d\Cart\) corresponding to \({\A\in SFDG\CI\Alg.}\) Assume that there is a fixed cellular structure on \(\A\) (this is equivalent to the existence of a fixed "origin" for \(l(\A_0)\)). Then we can define the algebra of differential forms on \(\Rc\) as 
		\[
		\Omega^*(\Rc)=\SS^g(\ol{\A}/\ol{\A}^2).
		\] 
		Here \(\ol{\A}\) stands for the canonical augmentation ideal of Definition \ref{def:aug_ideal_Fermat}, or equivalently for the kernel of the origin. This definition then immediately gives a presheaf on \(d\Cart.\)
	\end{definition}
	Clearly, Definition \ref{def:dif_forms} and its homotopy invariance allows us to define sheaves of differential forms on the whole higher sheaf topos \(\Sh(d\Cart).\) Moreover, \(\Omega^*\) is naturally a sheaf on \(d\Cart\) and hence taking the derived mapping space into \(\Omega^*\) defines a differential forms on \(\Sh(d\Cart)\). By abuse of notation we will now denote by \(\Omega^*\) this derived (i.e. sheafified) version of \(\Omega^*\).
	Now we define the Deligne complex. For a reference on the ordinary Deligne complex and its relation to classical differential cohomology, see~\cite[\S 7.3]{amabel2021differential}.
	\begin{definition}\label{def:dif_coh}
		Denote by \(\Z\) the locally constant sheaf of \(\Z\)-valued functions on \(d\Cart.\)
		For \(k\ge 1\) consider a complex of sheaves:
		\[
		...
		\]
		Then we define the {\it \(k\)-th differential cohomology group} \(\hat{H}^k(X)\) as 
		\[
		\hat{H}^k(X)=H^k([X,\D^k],d).
		\]
		Here \([-,-]\) is the set of homotopy classes and \(d\) is the differential induced from \(\D^k.\) 
	\end{definition}
	\begin{remark}
