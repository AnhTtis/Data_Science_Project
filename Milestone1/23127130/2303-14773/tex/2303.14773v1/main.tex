...
CoOp \cite{zhou2022coop}    & {\color[HTML]{E80F0F} \xmark} &L& \cellcolor[HTML]{C0C0C0}\textit{emb } & {\color[HTML]{E80F0F} \xmark} \\ 
CoCoOp \cite{zhou2022cocoop}  & {\color[HTML]{E80F0F} \xmark} &L& \cellcolor[HTML]{C0C0C0}\textit{emb } & {\color[HTML]{17C211}\cmark} \\
...
Following the protocol in \cite{zhou2022coop, zhou2022cocoop}, we conduct a few-shot evaluation for all datasets: 16-shot for the train set, 4-shot for the validation set, and the whole test set. We use the few-shot split by \cite{zhou2022coop} for each dataset those are also used in \cite{zhou2022coop}, while for Resisc45 and CLEVR, we randomly select the 16-shot and 4-shot samples for training and validation dataset, respectively.
