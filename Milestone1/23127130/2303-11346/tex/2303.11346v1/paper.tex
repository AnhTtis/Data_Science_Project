...
framework~\cite{Efthymiou_2021, qibojit_paper, modular_qibo}. Starting from a prior toy
...
\begin{figure*}
  ...
  QAML estimations in the lowest part of the figures. While representing the PDFs,
...
\begin{figure*}
  \centering
    ...
    sample: in red the law extracted via QAML using exact
...
\begin{equation}
    \rho(x; \alpha, \beta) = \frac{\beta x^{\alpha - 1} e^{\beta x}}{\Gamma(\alpha)}, \label{eq:gamma}
\end{equation}
...
sufficient statistics to achieve accuracies into the range 4-10\% for
the ratio values (lower half of the PDFs plots). This accuracy range is obtained
by calculating the ratio between the estimated sigma and the estimated PDFs
ratio value for the points contained into a target range around the peak of the
PDF; the minimum and the maximum values are used to define the range.
...
%
%\begin{itemize}
%  \item Train in simulation -> print pdf on hardware
%  \item Train everything on hardware
%\end{itemize}
...
adiabatic Hamiltonian in terms of a quantum circuit. The PDF is them obtained by
...
available in~\footnote{https://github.com/qiboteam/adiabatic-fit}.
%
