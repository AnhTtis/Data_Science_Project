...
For free bosons, the partition function on $AdS_2$ is not smooth  when the mass saturates the Britenholer-Freedman bound. 
...
It has been argued that 
 ...
 maps normalizable modes to non-normalizable modes. Therefore the  supersymmetric algebra does not close in the 
...
Therefore the resultant partition functions are not smooth as a function of this ratio.  
 ...
 saturates the Britenholer-Freedman bound \cite{Breitenlohner:1982bm,Breitenlohner:1982jf}. 
...
Britenholer-Freedman bound for fermions in $AdS$ \cite{Amsel:2008iz,Dias:2019fmz}.  
...
It also introduces monopole harmonics which is used to evaluate the partition functions on $AdS_2\times S^2$.
 ...
 deviation of the mass from the Britenholer-Freedman bound.. 
 We will demonstrate that the partition function is not a smooth function in $x$   at the
 Britenholer-Freedman bound, that is when $x=0$. 
...
Examining the integrand, we see that if sufficient derivatives in $x$ is taken, 
  there is a possibility of 
 at $x=0$.  This divergence arises at the IR, that is at $\lambda \sim 0$ of the integrand. 
 This can be seen if   we  take the derivatives inside the integral. One sees that, 
 ...
 the integral first and then differentiate with respect to $r$. 
...
From this simple analysis we observe that the partition  for bosons
 ...
 Brietenholer-Freedman bound on $AdS_2$, the 3rd derivative of the function is discontinuous. 
...
-\frac{1}{2} \left\langle  \int dr d\theta \sqrt{g}  \phi^2( r, \theta ) \right\rangle = - \frac{d}{d \tilde m^2}  F (x)  =
-  \frac{L^2}{ 2 x}\frac{d}{dx} F(x) \, .
 ...
 \end{equation}
 The result of 
 evaluating this numerically  figure \ref{fig2} . This expectation value clearly shows the 
...
we obtain 
\begin{equation}
\langle \phi^2 \rangle_{\mathbb{R}^2} = -\frac{1}{2\pi } \Big( \gamma -  \log 2 )\, . 
\end{equation}
...
m^2 = -\frac{1}{4 L^2} + \frac{ x^2}{L^2}\,,
\end{equation}
therefore the Britenholer-Freedman bound is reached at $x=0$. 
...
S_I=\int d^{2}x\sqrt{g}\,\bar\psi(i \slashed D+ m )\psi\,.
\end{equation}
while the second action is given by 
\be \label{act2}
S_{II}=\int d^{2}x\sqrt{g}\,\bar\psi(i \slashed D+ m\g_{t})\psi\,.
\ee
...
\begin{equation}
Z_{II}  = {\rm det} ( i \slashed D + m \gamma_t) \,.
\end{equation}
Here we write the partition function as 
\begin{equation} \label{partspin2}
Z_{II}   = \sqrt{ {\rm det} ( i \slashed D + m \gamma_t)^2}  =\sqrt{  {\rm det} (  - \slashed D^2 + m ^2 ) } \,.
\end{equation}
