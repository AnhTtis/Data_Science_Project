...
The comparison of error distribution is shown in Figure \ref{fgr:distL4in1}, distribution of errors with short cut-range is in Figure S1. The Kernal Density Estimate (KDE) plot of error distribution is in Figure \ref{fgr:kde2in1}.
	...
	\section{Conclusion}
	In order to improve the computational efficiency of the vibrational spectrum simulation, we present a new machine learning method based on DPRc model. Rendered DPRc and DP model were utilized to calculate \ch{C=O} stretching vibrational frequency shifts rather than potential.
