...
\def\@name{\emph{Pingchuan Ma$^{1}$,  Alexandros Haliassos$^{1}$, Adriana Fernandez-Lopez$^2$, Honglie Chen$^2$} \\ \emph{Stavros Petridis$^{1,2}$, Maja Pantic$^{1,2}$
    \thanks{Only non-Meta co-authors downloaded, accessed, and used the datasets. Only non-Meta authors conducted any of the dataset pre-processing (no dataset pre-processing took place on Meta’s servers or facilities). Code and trained models are available at: \url{https://github.com/mpc001/auto_avsr}
    }
    }
}
