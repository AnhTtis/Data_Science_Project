...
%\textsc{Abstract:}
\small{}%
This \chptr\  is concerned with the simultaneous solution of a very large number of optimization
problems. One notable aspect of the models studied is that the structure of the
optimization problems involved is not given in the outset and is dynamically agreed upon while a
large number of optimizers are acting in an orchestra.
Models of this type are instrumental in the domain of macroeconomics and have given rise
to the mathematical theory of mean field games. This \chptr\  shows that an alternative
mathematical formulation may be needed and provides such formulation.  
The approach it develops  was prompted by the surprising discovery that
the common strategy, adopted in a large body of macroeconomic research,
for producing time-invariant equilibrium in  the classical
Aiyagari-Bewley-Huggett model 
fails to achieve its objective in a widely cited benchmark study,
with the implication that, contrary to the common belief,
a central problem in macroeconomics has been without an adequate solution,
save for special cases. 
It~is shown that the intrinsic structure of what is known in economics as
``heterogeneous models''
exhibits connections across time that existing mathematical techniques
cannot capture.
By expanding the approach proposed by Dumas and Lyasoff \citel{DL12}
this \chptr\  develops 
a novel mathematical framework which, among other things, leads to
numerically verifiable 
equilibria in some widely researched, yet still unsolved,
concrete instances of heterogeneous models.
The \chptr\  provides new insights about the channels through which
the cross-sectional distribution of a large population of interacting
agents gets 
transported through time (see \ref{main-q} and \ref{n2} below)
and clarifies the scope of what is known as the approximate aggregation conjecture
(only the population mean matters)
of Krusell and Smith~\citel{KS98} (still an open problem in macroeconomics).
\medskip
...
inadequate even in very basic examples. Thus, an entirely different structure is needed to
explain the law of motion of the distribution of interacting agents (see \ref{main-q} and \ref{n2} below).
...
(d)~Even if all individual choices are affected only by the population mean
...
The main conclusion \nocite{ALL22}{ibid.}\ arrives at is described  in
\nocite{ALL22}{ibid.}\ as ``tentative.''
The present \chptr\  has the goal of developing an alternative mathematical
...
including models with common noise, as~the one introduced by Krusell and Smith~\cite{KS98}.
...
The reason given in \cite{AHLLM17} is that practically all theoretical results on MFG rely on
assumptions that are 
too restrictive to accommodate
the classical Aiyagari-Bewley-Huggett and other interesting models.
%especially when
...
Moreover, these connections may not be compatible with
the tacit assumption (not used in this \chptr) that the probability
distribution of the optimal private state is the same as the distribution of the population,
or compatible with the structure of a coupled MFG system, the coefficients of which must be fixed in
the outset and remain frozen.

The approach proposed in the present \chptr\  stems from the realization that the flow of the
...
at all times and in all possible private and aggregate states (also incorporated into the solver).
In particular, the amount that an agent can borrow is
constrained only by the amount that other agents 
are willing to lend (in conjunction with the requirement that all individual budgets remain balanced
at all times). One consequence from this observation is that
...
simultaneously, hence the need for a radically new approach.
%designed from the ground up with insights and
%intuition borrowed exclusively from the domain of economics.
%For all these reasons,
The main goal of the present \chptr\  to develop a
mathematical framework that results into a 
computable program (in discrete time and with  finite time horizon),
which incorporates the features outlined in the foregoing.
So~to speak, the object of study is the mathematics
of a ``flux'' that is ``self-aware.'' 
...
To~the best of this author's knowledge, the ideas on which this \chptr\  rests
have no predecessors in the literature (in both mathematics and economics)
except for the paper \cite{DL12}, which
all that follows below is meant to be a continuation~of. The main limitation of the method proposed
in \cite{DL12} is that
it is applicable only to models with a relatively small number of agents,%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{Or, which amounts to the same, small number of large groups of identical agents.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
in which case the notion
...
One is then led to conclude that, as it stands, the classical framework described above is incomplete, in the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The main takeaway from \cite{DL12} is that, without any reliance on the representative
...
The reader must be forewarned that, just as in the paper \cite{DL12}, in the present \chptr\ %
...
Ultimately, the method developed in this \chptr\  allows one to construct
...
following 
contributions to the study of the transport in models with common noise, in particular to 
...
The Julia code used in all examples presented in the \chptr\  can be retrieved from
the author's \href{https://github.com/AndrewLyasoff/heterogeneous-models}{Git
repository}.

The \chptr\  is organized as follows. Sec.~\ref{sec:gen-model} describes a general 
...
In~essence, \ref{main-q}-\eqref{zze666} is the counterpart of the Kolmogorov forward equation for
the law of motion of the cross-sectional distribution of a large population of interacting
agents~--
as opposed to the law of motion of the probability distribution of a single Markovian state. 
...
No borrowing constraints are imposed explicitly in the individual savings problems~-- such
constraints arise 
...
The idea again is to use those conditions as a tool for deciphering the population distribution
transfer.
...
\section{Equilibrium in Models Without Aggregate Risk}
...
The approach developed in the present \chptr\  shows that this is not the
...
finite time horizon methodology developed in the \chptr\  is consistent with previously known
results.
...
number of households with a large time horizon and the transports are very close to affine (in this
particular example).
