...
\usepackage{setspace} 
\setstretch{1.0}%\setstretch{1.05}

\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}  

\usepackage{newtxtext} %!!! % MOD1
\usepackage[varg]{newtxmath} %!!! MOD1
...
The \chptr\ is concerned with the simultaneous solution of a very large number of optimization
problems the structure of which is not given in the outset and is dynamically agreed upon while a
large number of optimizers work in an orchestra. The proposed new approach to such problems was
prompted by the surprising discovery that the common strategy, adopted in a large body of
research, for producing time-invariant equilibrium in the classical Aiy\-ag\-ari-Bew\-ley-Hug\-gett
model fails to achieve its objective in a widely cited benchmark study, with the implication that
a central problem in macroeconomics has been without an adequate solution, and despite recent
advances based on novel mathematical techniques borrowed from the theory of mean field games.
It is shown that the intrinsic structure of a generic heterogeneous agent incomplete market model
imposes connections across time that existing mathematical frameworks cannot capture. The new
technique is shown to provide numerically verifiable equilibria in some widely researched, yet
still unsolved, concrete instances of heterogeneous agent models. The scope of ``the approximate
aggregation conjecture'' of Krusell and Smith~\cite{KS98} (still an open problem in macroeconomics)
is clarified and a new computational strategy, which does not rely on simulation or the need to
postulate infinite time horizon, for models with common shocks is developed. New insights about the
fluctuations in the population distribution in such models are drawn and some novel closed-form
...
Another relevant observation is that, in one way or another, the mathematical frameworks of the
...
%\toshow{\includegraphics[width=7.1cm]{/home/sglass/lavoro/github/heterogeneous-models/W.pdf}}
...
\end{figure} }% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
The~parameter values ($\b>\nobreak 0$,~$w>0$, $\SSS\in\R^7$, $\PPP\in\R^{7\otimes7}$) and the ad hoc range
$[u,v]$ are borrowed
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
:
the large dot in Figure~\ref{fg2} corresponds to the same rate $r\approx 0.037$,
...
different from the distribution shown in solid lines on the right plot in Figure~\ref{fg4},
...
attached to every productivity state (assuming that the population distribution can be identified
with the probability distribution of a representative private state).
...
known.%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{See the extensive discussion and references in \cite{DS12}, \cite{Sun06}.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In the benchmark economy studied in the lanmark paper \cite{KS98}  the
...
a quantity becomes meaningful only after all individual savings problems have been solved and does
...
and~$\qq$,
i.e., for $c_{t,\tts x,\tts \csd,\tts u}$, $\q_{t,\tts x,\tts \csd,\tts u}$ and
...
$$}%
For the sake of simplicity of the notation, we shall assume this form from now on, and note that,
simplicity aside, it also makes the model invariant to any re-scaling the consumption variable. 
...
The program-like description of the strategy just
...
\hbox to0.3cm{\hfil} = \bigl({ {g_{t,\tts x,\tts A,\tts u}^{ y ,v}}} + { {h_{t,\tts x,\tts A,\tts u}^{ y ,v}}}\times c\bigr)
...
\Bigl(\rho_ y \bigl(K_{t}(x, A)\bigr) + 1-\dd \Bigr)
\times  Q(x, y )\times  P_{x,\tts y }( u ,v)\,,\quad u\in\EEE\,,\\
\noalign{\rlap{{and}}}
\label{Taylor-b}
\begin{multlined}
...
\Bigl(\rho_ y \bigl(K_{t}(x, A)\bigr) + 1-\dd \Bigr)\qquad\qquad\\
\qquad\qquad \times  Q(x, y )\times  P_{x,\tts y }( u ,v)\,,\quad u\in\EEE\,.
\end{multlined}
\end{gather}
}%
\end{subequations}
...
One possible arrangement that would still allow one to use the total population mean $A^*$ as an
endogenous state variable is to consider Taylor expansion around the abscissa ${1\over c}={1\over
A^*}$. One major objection to this approach is that it ignores the variations in the group-specific
mean values, which are illustrated in Figure~\ref{fgKS5} below.\qed
\end{nit}
...
{\abovedisplayskip=10pt plus 1.5pt minus 0.5pt\belowdisplayskip=10pt plus 1.5pt minus 0.5pt\belowdisplayshortskip=5pt plus 1.5pt minus 1.5pt
\begin{gather*}
...
%%
%%
It is clear from Figure~\ref{fgKS12} that, in this particular example,
the dependence
...
in Krusell-Smith's algorithm described in [\cited{KS98},~III]
the dependence of the transport on the departing aggregate state is ignored,  i.e., in their
approach there are only two 
regression lines instead of four.} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
While
all four lines in Figure~\ref{fgKS12} appear to be very close, there is no reason for this feature
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{Just as an example, if the value of the impatience parameter $\b$ is decreased to $0.96$
from its original value of $0.99$, with which Figure~\ref{fgKS12} is generated, while all other
parameter values are kept unchanged, then the difference between dotted and solid lines in
Figure~\ref{fgKS12} becomes much more pronounced. This feature is illustrated in the computer code
that accompanies this \chptr.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
see \ref{transport-K}~-- which is shown on Figure~\ref{fgKS13}.  
%%
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%     Fg: 13
...
%%
%%
As can be seen from Figure~\ref{fgKS13},
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{There is no intuition to suggest that the transport mappings shown in Figure~\ref{fgKS13}
...
would always be as accurate as they are in the benchmark case study from the paper \cite{KS98}.}%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
).
...
note that the data shown in Figure~\ref{fgKS5} is more dispersed than the fluctuations
...
state. In particular, the plot shows (by way of an example) that the pair composed of the aggregate
...
investment and consumption levels in the long run.
It~is also interesting to note that increased impatience leads
...
The representative agent point of view reviewed in Sec.~\ref{sec:Intro},
or the classical Markovian game theoretic formulation  
(see, for example, \cite{GKSS00}, \cite{KSS94},~\cite{KSS97})
or the MFG formulation (see, for example, \cite{AHLLM17}, \cite{ALL22}, \cite{CD18})
do not capture all of these connections across time and 
across a large population of households.
Such formulations are useful~-- indeed, preferable~-- in models where
the backward-forward-backward connections
noted in the 
foregoing are redundant, i.e.,
happen to be a consequence of the remaining components of the model and the equilibrium can be
identified without them.
As~we have seen in Sec.~\ref{sec:Intro} however,
such a redundancy may be absent even in a simple model of 
pure exchange economy with no aggregate shocks. 
...
First, the method is built around the notion of equilibrium introduced
in \cite{DL12} and does not
rely on the notion of Nash equilibrium (it was noted earlier that the latter is not meaningful in
...
The new method can be implemented in practice by way of functional programming
...
It can also replicate the well documented and widely discussed results of Krusell and Smith
...
%%
%%
{\abovedisplayskip=8pt plus 1.5pt minus 1.5pt\belowdisplayskip=10pt plus 1.5pt minus 1.5pt\belowdisplayshortskip=8pt plus 1.5pt minus 1.5pt
...
C'est l'hôtel qu'il vous faut choisir si vous ne pouvez séjourner que dans un seul hôtel au monde.
