...
equation). Hence the need for a new approach, designed from the ground up with insights and
intuition borrowed exclusively from the domain of economics. Unfortunately a full mathematical theory
(rigorous proofs of existence, uniqueness, convergence, and such) that incorporates the features
outlined above is yet to be developed and is
beyond the scope of the present paper. The main objective in what follows is to develop a 
procedure in discrete time and finite horizon,
which incorporates the intricacies and the dependencies outlined above, to demonstrate
its feasibility in concrete applications, and, above all, to demonstrate that it is capable
of answering at least some unanswered questions.
To~the best of this author's knowledge, the
program developed in the present paper has no predecessor other than \cite{DL12}, which
this paper is meant to be a continuation of. The main limitation of the method proposed
in \nocite{DL12}{ibid.}\ is that
...
standing desire to
remove that limitation. It~was prompted also by the unexpected
discovery of a previously undocumented feature of the classical Aiyagari-Bewley-Huggett model,
which calls for a further study and an eventual resolution. This surprising feature is detailed next. 
...
that such a joint limit may reveal connections that are lost when~\eqref{lmb-iter}
...
i.e., the private state is exiting in terms of wealth but entering in terms of employment.
...
is attained in the long run, i.e., the prices are kept unchanged for may periods.
...
distribution associated with every instance of $\pst\q_u(\cdot)$ and $\pdg\q_u(\cdot)$~-- an
enormous simplification, possible only if stationary distribution exists.\qed
\end{nit}
...
\ %  
\toshow{\includegraphics[width=7.0cm]{fg-KS-14L}}
...
\ %
\toshow{\includegraphics[width=7.0cm]{fg-KS-14R}}
...
\begin{figure}[!htbp]
\centering
\toshow{\includegraphics[width=7.5cm]{fg-KS-19}}
...
reveals fluctuations that are much more pronounced than the fluctuations in the exogenous productivity
...
\ %  
\toshow{\includegraphics[width=7.0cm]{fg-KS-20L}}
...
\ %
\toshow{\includegraphics[width=7.0cm]{fg-KS-20R}} 
...
The new method reveals a substantial disparity in the cross-section of the population of households,
