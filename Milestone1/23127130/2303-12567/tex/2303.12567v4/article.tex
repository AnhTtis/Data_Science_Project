...
\usepackage[cal=boondoxo, calscaled=1.0, scr=boondox, scrscaled=1.0]{mathalpha} %!!!
...
\usepackage[colorlinks,citecolor=blue,linkcolor=red,anchorcolor=blue,urlcolor=blue]{hyperref}
...
\title{Self-Aware Transport of Heterogeneous Agents\\
...
\titlespacing*{\section}{0pt}{1.0ex plus 0.25ex minus .125ex}{0.75ex  plus 0.25ex minus 0.125ex}
...
\fancyhead[CO]{\small SELF-AWARE TRANSPORT OF HETEROGENEOUS AGENTS}  
...
%
{\noindent%
The present paper was prompted by the surprising discovery that the common strategy,
adopted in a large body of research,
for producing time-invariant equilibrium
in  the classical  Aiyagari-Bewley-Huggett model
fails to achieve its objective in a widely cited benchmark study, with the implication that,
contrary to the common belief,
a central problem in macroeconomics has been without an adequate solution, save for 
special cases. 
It is shown that equilibria in such models can be obtained generically only
if prices, optimal policy,
and population distribution are allowed to adjust toward their steady-state regimes simultaneously,
which requires 
new modeling and computational tools. 
By expanding the approach proposed by Dumas and Lyasoff \citel{DL12} the paper provides such tools
and offers a new interpretation of problems commonly stated
as mean field games or transportation of mass problems.  
The scope of the approximate aggregation conjecture
of Krusell and Smith \citel{KS98} is clarified and a
new analytical technique for models with common
noise is
developed. The proposed new method does not resort on the representative agent framework,
does not involve simulation, does not postulate infinite time
horizon, and allows for disparity in the cross-section of households to be accounted for.
\medskip

\noindent% 
\textsc{Keywords:} transport problems, mean field games, 
...
\noindent
It was noted recently in  \citel{ALL22}  that  the landmark model of Krusell and Smith \citel{KS98}
was crucial for placing the celebrated master equation in the center
of the theory of mean field games (MFG).
The paper \citel{ALL22} develops  
a numerical framework for approximating the solution to a concrete master equation, with which the
authors provide a rigorous mathematical interpretation of Krusell-Smith's model in terms of~MFG.
The 
objective in \citel{ALL22} is to
test empirically the approximate aggregation conjecture of Krusell and Smith. %%%%%%%%%%
The main conclusion \nocite{ALL22}{ibid.}\ arrives at is described  as ``tentative.''
The present paper has the goal of developing an alternative mathematical
formulation~-- i.e., formulation not rooted in the principles of MFG~--
of general
equilibrium incomplete market models with a large number of heterogeneous economic agents,
and to eventually arrive at a somewhat more conclusive analysis of such models,
including models with common noise, as~the one due to Krusell and Smith~\cite{KS98}.
The purpose of this section is to clarify why such an alternative formulation is needed and to
outline the ideas on which the proposed new approach is based.

Among its other findings, the paper \cite{AHLLM17} concludes that 
...
The reason given in \cite{AHLLM17} is that practically all theoretical results on MFG rely on assumptions that are
too restrictive to accommodate  the Aiyagari-Bewley-Huggett and other interesting models, especially when
matters of existence and uniqueness are consi\-dered.
Another reason noted in \cite{AHLLM17} is the need to impose state constraints, which the canonical
MFG framework lacks. This problem was taken
up, for example, in \cite{CanCap18} and \cite{CCC21} (see also the discussion in \cite{AHLLM17} and
the extensive references in \cite{CCC21}).
The need to impose certain
constraints on certain state variables is not new. To say the least, the well known borrowing
constraint has been the subject of extensive research in macroeconomics for a very long time~--
see \cite{Hug93}, \cite{LjunSar00}, for example.
The authors of \cite{CCC21} recognize
the difficulties in interpreting the coupled MFG system~-- especially its second component, the
continuity equation~-- in the context of heterogeneous agent models. They attribute those
difficulties to the state constraints, which typically result into singularities in the population
distribution, and point to an alternative (Lagrangian) formulation, which leads to the so called
relaxed MFG equilibrium~-- see \cite{CanCap18} and the closely related papers \cite{BB00}
and \cite{BCS17}. In most part, these papers are based on insights and intuition drawn from fluid
mechanics. The underlying idea is to pin down the individual trajectories of a large collection of particles and
then identify the flow of probabilities  along those trajectories.

The approach proposed in the present paper stems from the realization that the flow of the
population distribution in heterogeneous agent models is radically different from the flow
of probabilities encountered in fluid mechanics~-- and the
difference is  too big to compensate for by means of exogenously imposed
state constraints. Indeed, the transport (alias: transfer) of the population, i.e., of its
distribution over the collection of private states, from one period to the next is the outcome from
...
Hence, the transport of the population is driven by expectations about its outcome. The
notion of competitive equilibrium then requires that the outcome from the transport~--
during every period~-- must be the same as the
anticipated one, which drives the transport. Figuratively speaking, the transport must be self-aware.
Moreover, rational expectations about the endogenous state variables in the future are conditioned to the
realized exogenous aggregate shocks in the future, i.e., there must be a different
transport attached to every future
realization of those shocks. The transport then depends on both present and future
aggregate shocks, i.e., it takes place in the random environment determined by the transitions in
the exogenous aggregate state, not by the exogenous aggregate state alone.
In~addition, the agents are constrained in their choices of consumption and investment, but
not in a way that can be made explicit and imposed in the outset. What constrains these choices
are the collectively agreed upon prices and the need for every private budget to remain balanced
at all times and in all possible private and aggregate states.
In particular, the amount that an agent can borrow is
constrained only by the amount that other agents 
are willing to lend. One consequence from this observation is that
the lowest borrowing level across the population becomes endogenous
in much the same way in which the prices are endogenous.
Finally, as~was noted in \cite{DL12}, the  structure of general equilibrium incomplete market models is
intrinsically backward and forward: the (endogenous)
prices today depend on the (endogenous) demand today, which
depends on the anticipated returns tomorrow, which depend on the prices today.
In other words, in such models time does not flow in quite the same way as it does in a common
run-of-the-mill optimal control problem. 


On the one hand the features outlined in the preceding paragraph are intrinsic for all incomplete
market models with a large number of heterogeneous economic agents, but on the other hand such
features are difficult to place in the canonical mathematical framework of optimal control (with
or without constraints) and the associated continuity equation. The main difficulty is that the
``flux'' in a typical heterogeneous agent model is no longer a unidirectional prescription for
displacement, as the one modeled  by
a vector field, a stochastic differential, or PDE of one form or another (e.g., continuity or master
equation). Figuratively speaking, the ``flux'' and the result of its action can only be determined
simultaneously, hence the need for a new approach, designed from the ground up with insights and
intuition borrowed exclusively from the domain of economics. For all these reasons,
the present paper aims to develop a computable program (in discrete time and with  finite time horizon)
that incorporates the features described in the foregoing.
So~to speak, the object of study is a program built  around the idea of a ``flux'' that is ``self-aware.'' 
In~terms of its general purpose and objectives, this program is
somewhat similar to the one encrypted in a, say, FBSDE system, or a coupled MFG system. 
However, because of its unusual backward-and-forward time structure
the proposed new method is yet to give rise
to a reasonably well formulated mathematical theory,%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{Meaning, rigorous proofs of existence, uniqueness, convergence, and an eventual passage
to the continuous time framework.} %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
similar to the theory of FBSDE or MFG, for example.
Thus, the main objective of what follows is to
establish evidence that the proposed new approach is not just feasible, but also capable of providing
new insights and closing certain gaps (see below), hence worth pursuing.
To~the best of this author's knowledge, the ideas on which this paper rests
have no predecessors in the literature except for the paper \cite{DL12}, which
all that follows below is meant to be a continuation~of. The main limitation of the method proposed
in \cite{DL12} is that
it is applicable only to models with a relatively small number of agents, in which case the notion
of transport of the population does not arise.
In~most part, the present project grew out of the long 
standing desire to
remove that limitation. It~was prompted also by the surprising
discovery of a previously undocumented gap in the common approach to the classical
Aiyagari-Bewley-Huggett model,
which gap is detailed next. 


Consider  the familiar savings problem outlined in Sec.~18.2 in 
...
boils  down to calculating the (time-invariant) optimal policy function
...
representative household that follows the optimal policy $a'=g_\infty(a,s)$, which policy obtains in the
...
$[u,v]$ are borrowed
from the first specification in Sec.~18.7 in \cite{LjunSar00} and so is also the program
...
Julia is the ability of the latter to handle large grid sizes with ease.}
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
\ %   
\toshow{\includegraphics[width=7.1cm]{fg3}}%
...
It would be instructive for what follows in this paper to identify the reasons for the phenomenon
...
certain monotonicity in the transition probabilities) for any number of idiosyncratic states is
...
but if this collection is not a singleton then equation~\eqref{lmb-iter} alone, in conjunction with
the market clearing \eqref{mclr}, would not
be sufficient for locating the equilibrium.
...
present paper. While one of these two stationary distributions 
...
One is then led to conclude that, as it stands, the classical model described above is incomplete, in the
sense that passing to the limit as $t\to\infty$ in  \eqref{lmb-iter} and \eqref{Bellman}, in conjunction
with \eqref{mclr}, yields an underdetermined system.
To~see where the missing conditions may come from, notice that if the economy is to converge to its
...
In particular, the representative household is in steady state but the cross-sectional distribution
...
that configuration happens to be a time-invariant solution~to~\eqref{lmb-iter}.
...
only then investigate the joint limit of all endogenous variables as~$T\to\infty$, without
any reliance on the representative agent framework. The hope is
...
As was noted in \cite{DL12}, the main difficulty to overcome is the intrinsic backward-forward structure
...
practical only in models with very few periods~-- see \cite{DL12} for details.
The main takeaway from \nocite{DL12}{ibid.}\ is that, without any reliance on the representative
...
The present paper follows the same
...
requires a substantial reworking of the method.
The reader must be forewarned that, just as in the paper \cite{DL12}, in the present paper
...
discussed in \cite{DL12} and essentially boil down to the benefits from identifying (through a
particular homeomorphism) consumption as both state and dual variable. Another (somewhat more
...
chains is well known, the method proposed in the present paper should be possible to utilize~--
...
\begin{subfigure}{.47\textwidth} 
  \centering
\leavevmode\raise0.3cm\hbox{\rotatebox{90}{\tiny distribution within emplpyment group}}% 
\ %   
...
\begin{subfigure}{.47\textwidth}  
  \centering
\leavevmode\raise0.3cm\hbox{\rotatebox{90}{\tiny distribution within employment group}}%  
\ % 
...
the large dot in Figure~\ref{fg2} corresponds to $r\approx 0.037$ (the actual equilibrium rate)
and expected demand of~$\approx 6.901$, which is very far from $10^{-3}$~-- all due to the fact that the
iterations leading to Figure~\ref{fg2} were initiated with another (arbitrarily chosen) distribution. 
...
terms of employment.%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{Such an approach is only applicable in the absence of aggregate shocks.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
developed later in this paper, or with the classical method described above~-- have no
points of accumulation. This finding is in contrast with the result in \cite{AHLLM17} (in continuous time
setting and with only two employment states),
in which the borrowing limit is imposed exogenously and
the population distribution does accumulate
around that limit.%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
the expected demand after solving the FP equation attached to the representative agent,
...
Perhaps the most interesting application of the new methodology proposed
in this paper is to models with production that is subjected to common productivity shocks.
...
population distribution, treated as a stochastic process, to be Markov, with time-invariant
transition (transfer) mechanism.
...
In particular, the paper by Den Haan in the same volume discusses the weakness (referred  to as ``fatal
...
Nevertheless, the results obtained by the simulation technique from
\cite{KS98} are quite accurate (at least with the model
parameters chosen as in \nocite{KS98}{ibid.})~-- see Sec.~\ref{sec:KS} below for additional details.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
including  \cite{KS98},
...
the persistence of i.i.d.\ prediction errors.
...
verifiable solution to the classical  Aiyagari-Bewley-Huggett model, the present paper makes the following
contributions to the study of Krusell-Smith's model, i.e., the general model with production and shared
risk associated with productivity shocks.
...
hypothesis holds.
Second, the paper shows that even if all aggregate variables and individual choices can
be described only in terms of the mean of the population distribution and the productivity shocks,
the exact form of the full distribution transport, not just the transport of the mean, can still be
identified; in particular, the population distribution cannot be chosen arbitrarily, subject to the
only requirement for its mean to follow a prescribed law of motion.
This feature reveals dispersion in the individual wealth across the population that is
substantially larger than the dispersion in the productivity shocks~-- see Figure~\ref{fgKS5}
below~-- and cannot be captured by a model that is confined to the population mean alone. 
Third, it is shown that the
transport of the population takes place in the random environment of the transition in
...
This structure is consistent with the way in which Krusell and Smith describe their general
model framework in [\cited{KS98}-II-B], where the general form of the transport (updating rule)
is written as $\G'=H(\G,z,z')$. However, the general computational strategy described in
[\cited{KS98}-II-C] assumes that the transport of
the mean has the form ${\bar k\tts}'=h(\bar k, z)$. This later form persists  
...
Lastly, the general methodology proposed in the present paper is meaningful for any, large or small
...
the present paper seeks to endogenize internally quantities that, traditionally, have not been endogenized~--
...
(nothing to say about uniqueness) of the equilibria is impossible to establish generically~-- at least not with
the tools that currently exist. The main reason is that most of the dependencies that the program
...
guaranteed. Although endless loops do not occur in the examples included in this paper and the
...
In particular, a continuous-time interpretation of the population transport established
in~\ref{main-q} below does not appear to be possible within the existing continuous-time framework. 
...
Nevertheless, one must acknowledge the existence of game theoretic interpretation of
similar macroeconomic models~-- see, for
example, \cite{GKSS00}, \cite{KSS94},~\cite{KSS97}. 


The Julia code used in all examples presented in the paper can be retrieved from
...
The rule that governs this transport 
...
In what follows consumption will be used as a state variable~-- note wealth. 
Thus, the collective
state of the population can be expressed as a distribution of unit mass over the product space
$\EEE\times\Rpp$ (the space of household characteristics, i.e., employment and consumption levels). 
...
agents who happen to be in employment state~$u$ and have chosen a consumption level that is not
...
matrices $Q$ and $P_{x,y}$ are assumed strictly positive. The pair of 
the common productivity state and the private employment state
of a given household follows a Markov chain on the state-space $\XXX\times\EEE$ with transition
...
distribution functions $(\csd^u\in\DB,\,u\in\EEE)$.%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\footnote{Note that the first component of the aggregate state
$(x,\csd)\in\XXX\times\DB^\EEE$ is exogenous, while the second one is endogenous.}%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\qed
\end{nit}
...
The transport of the population, i.e., of its distributed over the range of employment states
and consumption levels, from period~$t$ to period $t+1$ must be allowed to depend
on the period-$t$ productivity state $x\in\XXX$ and on the period-$(t+1)$ productivity state
$y\in\XXX$. It will be expressed as a collection of mappings
...
with the realized ones, i.e, $F_t^*=F$ and \smash{$F_{t+1}^*=\T_{t,x}^{y^*}(F)$} in every future
...
is the main goal in what~follows in this paper.\qed
\end{nit}
\nitskip
...
In a first step, we state the one-period
...
productivity shocks (encrypted in the matrices $P$ and $Q$) and, most important, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In particular, all global (shared) endogenous variables, namely the aggregate installed capital and
...
aggregate installed capital is $K=K_t(x,F)>0$, and the (one period) risk-free rate is $r=r_t(x,F)>-1$.
Consider a generic household that enters employment state $u\in\EEE$ 
with its personal wealth measuring $w/N$ units of the numéraire good,
...
%\end{gather}
%%
with the understanding that  $V_{t,\tts x,\tts F,\tts u}(w/N)$ is the constrained maximum attained in
...
No borrowing constraints are imposed explicitly in the individual savings problems~-- such constraints arise
endogenously from the notion of equilibrium, which demands that all private savings problems are
to be solved simultaneously, with all private budgets remaining balanced at all times,
while the aggregate investments across the population generate returns that all agents agree~on and
the aggregate demand for private lending is~$0$.
...
In what follows the main purpose of the systems (\ref{ze1}-\ref{ze2}) is to describe, in conjunction
with other relations, the
transport of the population, i.e.,
of its distribution over the collection of private states $\EEE\times\Rpp$.\qed
\end{nit}
\nitskip
...
The idea again is to use those conditions as a tool to decipher the population distribution
transfer.
...
$$}%
The dual variable (Lagrange multiplier) attached to each of these constraints we deliberately cast
in the factor form
{\abovedisplayskip=5pt plus 1pt minus 1pt\belowdisplayskip=5pt plus 1pt minus 1pt\belowdisplayshortskip=3pt plus 0.5pt minus 0.5pt
$$
\l_{y,\tts v}=\Lmp_{y,\tts v} \times {\b\over N}Q(x, y)P_{x, y}(u,v)\,,\ \  y\in\XXX\,,\  v \in\EEE\,,
$$}%
i.e., we will be working with $\Lmp_{y,\tts v}$ instead of the true dual variable $\l_{y,\tts v}$,
and  the dual variable attached to the last condition in \eqref{ze2} we deliberately cast in the form $\ff/N$,
i.e., we will be working with $\ff$ instead of the true dual variable~$\ff/N$.
...
\]}%
It is now easy to remove the dual variables from the KKT conditions
...
$$}%
This convention leads to the removal of the number of households, $N$,
...
$\Lmp_{y,v}$. In particular, the system \eqref{ze5} does not really exclude the dual
...
also on the selection of aggregate installed capital, risk-free rate, and population transport mappings. 
...
aggregation of all private capital investments may differ from the given
...
aggregate demand for the risk free private lending instrument is
...
trajectory in the state space followed by a given household. Indeed, the latter requires knowledge
about, at the very least, the employment history of the household,
which history is not part of the model studied here.

This is a good place to stress that the individual
trajectories followed by the states of the households, whether in the range of wealth or
consumption, never enter the model developed in this paper.
...
transport of the population (treated as a probability measure on $\abs{\EEE}\times\Rpp$)~-- see \ref{main-q}
...
which property is the same as $c\le\hat\pfc_{t,x,\csd}^{ y,v}(u,\a)$.%
\qed
\end{nit}
...
equation in MFG, or the FP equation in the classical approach to heterogeneous models. However, its
...
below and reflects the backward-forward connections noted earlier in the paper.
...
side). 
This feature may appear reminiscent to a FP equation with coefficients that depend (explicitly) on the
distribution that the equation drives, but we stress that, notwithstanding the self-awareness of
the transport, the dependence on $\csd$ in the right
...
of \ref{main-q}-\eqref{zze666}.\qed 
\end{nit}
...
Let $\SSS_t$, for $0\le t<T$, stand for the collection of
equations $\bigl\{\text{\eqref{zze5a}, \eqref{zze5xb}, \eqref{ze9}, \eqref{zze667}}\bigr\}$.
We call $\SSS_t$ the local (in time) bi-period master system, or simply the local master system, or
simply the master system, if ``local'' is understood from the context. 
Solving for the general equilibrium comes down to solving the global master system
$\{\SSS_t\colon 0\le t<T\}\cup\text{\eqref{zze5ab}}$, where \eqref{zze5ab} is the collection of all
...
which are to be solved for the unknowns $\bar c_u$, $u\in\EEE$.

The reason for organizing all conditions that define the equilibrium in such a way that the
master system $\SSS_t$ includes some equations associated with period $t$ and other equations associated
with period $t+1$ is to make it possible to seek a solution by solving, sequentially, the systems
$\SSS_{T-1}$, $\ldots$, $\SSS_0$, \eqref{zze5ab}. This process requires organizing and connecting accordingly
the unknowns that are being solved for, while keeping in mind that $\SSS_t$ is a system parameterized
by $x\in\XXX$, $F\in\DB^\EEE$, and $u\in\EEE$. 
To be a bit more precise, when solving  $\SSS_t$ the collection of functions 
$\q_{t+1, y,\tilde\csd,v}\phd$ and
$\qq_{t+1,y,\tilde\csd,v}\phd$, for all choices of $y\in\XXX$, $\tilde\csd\in\DB^\EEE$ and $v\in\EEE$, are
assumed given, while the unknowns are the functions (since $\SSS_t$ is parameterized by $x$, $F$,
...
unknowns are associated with period $t+1$. Since every local (in time) system $\SSS_t$ has components
that are forward-looking, the sequence
$\SSS_{T-1}$, $\ldots$, $\SSS_0$, cannot be interpreted as a ``backward equation''~--- at least not
in the way this term is commonly understood. 
In addition, it does not appear possible to split $\SSS_{T-1}$, $\ldots$, $\SSS_0$, into two
sequences that have the meaning of backward and forward equations, as in a coupled
MFG system, for example.\qed
\end{nit}
 
The recursive program for solving the global system
$\{\SSS_t\colon 0\le t<T\}\cup\text{\eqref{zze5ab}}$
...
decisions. The notion of equilibrium then requires that the outcome from the transport coincides
...
to (2) without changing $K_{t}(x,\csd)$ and $r_{t}(x,\csd)$; otherwise, stop
...
mapping, and market clearing conditions~-- not as a stand-alone equation. 
Note also that this
...
objects, we now
restate the global master system $\{\SSS_t\colon 0\le t<T\}\cup\text{\eqref{zze5ab}}$
from \ref{cross-sys} 
...
$\pst\pfc^v(u,c)=\pdg{\hat{H}}_v\bigl(\pst\q_u(c)\,\inc + {\ee\,v}\bigr)$  for all $v\in\EEE$.  
...
enormous simplification, possible only if stationary distribution of the population exists.
\qed
\end{nit}
...
metaprogram from \ref{IOU-proc} in the context of the benchmark Huggett economy
...
In contrast to the conclusions from \citel{AHLLM17}, which involve exogenously imposed
...
setup of the widely cited case study of \cite{KS98},
...
capture the dynamics of relative disparity among households~-- an important macroeconomic
...
The approach developed in the present paper shows that this is not the
...
finite time horizon, which must be developed first. The reason for this (admittedly, not very
...
finite time horizon methodology developed in the paper is consistent with previously known
results.
...
%
%
{\abovedisplayskip=8pt plus 3.0pt minus 1.5pt\belowdisplayskip=8pt plus 3.0pt minus 1.5pt\belowdisplayshortskip=6pt plus 3.0pt minus 1.5pt
\[
...
{\abovedisplayskip=4pt plus 1.0pt minus 1.5pt\belowdisplayskip=8pt plus 1.0pt minus 1.5pt\belowdisplayshortskip=4pt plus 1.0pt minus 1.5pt
\[
...
provided that an ansatz choice for the capital $K_{t}(x, A)$ is somehow
...
and, consequently, the aggregate installed capital $K_t(x,A)$ can be cast as $K_t(x,A^*)$.
...
methodology developed in this paper to widely cited methods and results, most of which are based on the
...
shocks even when the time horizon is pushed to $\infty$.\qed
\end{nit}
%\nitskip
...
i.e., if
written in terms of installed capital (the approach used in \cite{KS98})
...
strategy developed in this section can be cast in terms of capital instead of consumption as a
...
for any $t<T$ and any $x,y\in\XXX$ and without the need to restrict those mappings to a particular
type of functional dependence (e.g., log-linear).
...
are the collectively installed capital $K_{t}(x, A^*)$ and the intercepts %(see \eqref{lin-subst})
...
{(2)} Set $K_{t}(x, A^*)=K_{t+1}(x,A^*)$ (initial guess for the installed capital taken from the
...
traded. In~the
present setup this feature is tantamount to an agreement about the aggregate of all private capital
...
The method developed in the present paper cannot~-- and does not attempt to~-- address such matters.
...
R_{\infty,x,u}(A^*,c) \df \sum_{ y\ts \in\ts\XXX, v\ts\in\ts\EEE} \ 
{ \b\,\bigl(\rho_ y \bigl(K_{\infty}(x, A^*)\bigr) + 1-\dd\bigr)
\over g_{\infty,x, A^*,u}^{ y ,v}/c  + \b\,\bigl(\rho_ y \bigl(K_{\infty}(x, A^*)\bigr) + 1-\dd\bigr)}
Q(x, y )\,  P_{x, y }( u ,v)\,.
\end{equation*}}%
...
The plot in Figure~\ref{fgKS11} shows the installed capital $K_{\infty}(x, A^*)$ as a function of
$A^*$ for all $x\in\XXX$.
%%
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption{Installed capital in high state (solid lines)
and low state (doted lines) shown as a function of the population average $A^*$.
...
all four lines in Figure~\ref{fgKS12} are almost identical, it remains unclear whether this feature
persists with other choices for the model parameters~-- especially if the numbers of the exogenous aggregate
...
one to produce the transport of the population mean in terms of installed capital~--
...
number of households with a large time horizon.
...
that the mappings from \eqref{no-fp-2-0} are very close to linear. As a result, the
...
is the mean consumption level or the mean investment level across the entire population.
Of course, the simulated employment-specific mean values from the right plot Figure~\ref{fgKS5} can easily be
...
\caption{Installed capital in the last 10,000 periods in a simulated sample of 1.1 million periods.}
\label{fgKS-IK}  
\end{figure} }%
%%
%%
This plot shows the long-run range of the aggregate installed capital and, most important,
...
productivity state and the capital investment of the representative household does not have a
stationary long-run distribution. Indeed, if that would be the case, then the plot in
Figure~\ref{fgKS-IK} would be showing fluctuations between only two points (associated with the two
productivity states).  
...
to a greater dispersion in the relative disparity between unemployed and employed~-- in both
...
between the dual (shadow) variables associated with the individual savings problems.
...
across a large population of households~-- even if stated in terms of the maximum principle.
...
was proposed in \cite{DL12} and was further expanded in the present
paper.
This new approach~-- ``self-aware shadow programming'' of a sort~--
can be implemented in practice by way of functional programming, leading to a numerically
verifiable equilibrium in a well known case study  of
the classical Aiyagari-Bewley-Huggett model, which, so far,
has not been adequately resolved by other means.
The new approach can also replicate the well documented and widely discussed results of Krusell-Smith,
...
In addition, it provides a nearly complete answer to the question whether or not 
only-the-mean-matters point of view, put forward by Krusell and Smith \cite{KS98},
is reasonably accurate and under what restrictions.
Moreover, as was shown in the last section, even if  only-the-mean-matters point of view is reasonably
accurate, the law of motion of the full distribution, not just of the mean, is still unique and
possible to decipher with the new approach.
Specifically, the law of motion of the vector of conditional (to employment)
mean values was derived in the concrete example borrowed here from \cite{KS98}. 
This illustration reveals  a substantial disparity in the cross-section of the population of households,
which is not possible to detect in models confined to the population mean alone.  
