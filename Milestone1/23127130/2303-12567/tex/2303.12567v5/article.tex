...
in General Equilibrium Incomplete Market Models}%%% 
...
horizon, and allows for certain disparity in the cross-section of households to be accounted for.
\medskip
...
too restrictive to accommodate  the Aiyagari-Bewley-Huggett and other interesting models.
%especially when
...
then identify the flow of probabilities  along those trajectories. One must also note~--
see \ref{cross-sys} below~-- that the notion of equilibrium imposes connections between the
individual optimal choices (e.g., the need for all agents to agree on the prices) that have no
counterpart in the MFG formulation (whether boundary conditions are imposed or not).
Moreover, it remains unclear whether these connections are compatible with
the tacit assumption (not used in this paper) that the probability
distribution of the optimal private state is the same as the distribution of the population.  
...
are the collectively agreed upon prices, which agreement must be incorporated into the
solver, and the need for every private budget to remain balanced
...
the lowest borrowing level within the population becomes endogenous
...
for imposing the requirement that all households must agree on the risk-free rate set for the next period. }
%%%%%%%%%%%%%%%%
...
plays a crucial rôle in the calculation of the transport (and the equilibrium).
...
The transport of the population, i.e., of its distribution over the range of employment states
...
Since in (\ref{ze1}-\ref{ze2}) the entering wealth $w$ is treated as an exogenous parameter, the
optimization problem encoded in (\ref{ze1}-\ref{ze2}) is not the one associated with a
``representative household.'' 
In addition, since the agents are distinguished by their consumption levels, which vary from one
...
and  the costate variable attached to the last condition in \eqref{ze2} we deliberately cast in the form $\ff/N$,
i.e., we will be working with $\ff$ instead of the true costate variable~$\ff/N$.
...
We call $\M_t$ the local (in time) bi-period master system, or simply the local master system, or
