...
To train the recommender model, we aim to optimize the parameters $\{\theta_1, \theta_2\}$ by maximizing the generative probability of observed user interactions $\bm{x}_{1:T}$ in $T$ environments. Formally, we can maximize the log-likelihood $\log p(\bm{x}_{1:T})$ as follows:
\begin{equation}
\label{eqn:log_likelihood}
% \small
\begin{aligned}
\log p(\bm{x}_{1:T}) &= \sum_{t=1}^T \log p(\bm{x}_{t})
                        =\sum_{t=1}^T \log \int p(\bm{x}_{t}|\bm{e}_t)p(\bm{e}_t)d\bm{e}_t,
\end{aligned}
\end{equation}
where $p(\bm{x}_{t}|\bm{e}_t)$ corresponds to the generalization procedure in Eq. (\ref{eqn:all_prior}) and we will further factorize it with $\bm{z}_t$ in the decoding process (\ie Eq. (\ref{eqn:decoder})). 
Nevertheless, maximizing Eq. (\ref{eqn:log_likelihood}) is intractable because it involves the integral over unobserved $\bm{e}_t$. 
To solve the problem, we embrace \textit{variational inference}~\cite{liang2018variational} to approximate $\log p(\bm{x}_{1:T})$ by incorporating a variational distribution $q(\bm{e}_t|\cdot)$. Formally,
\begin{subequations}
\label{eqn:ELBO}
% \small
\begin{align}
\log p(\bm{x}_{t}) &= \log \int p(\bm{x}_{t}|\bm{e}_t)p(\bm{e}_t) \frac{q(\bm{e}_t|\cdot)}{q(\bm{e}_t|\cdot)}d\bm{e}_t \\
            &\geq \mathbb{E}_{q(\bm{e}_t|\cdot)} \left[\log \frac{p(\bm{x}_t|\bm{e}_t)p(\bm{e}_t)}{q(\bm{e}_t|\cdot)} \right] \quad (\text{ELBO})\\
            &= \mathbb{E}_{q(\bm{e}_t|\cdot)} \left[\log p(\bm{x}_t|\bm{e}_t)\right] - \text{KL}\left(q\left(\bm{e}_t|\cdot)\| p(\bm{e}_t\right)\right),
\end{align}
\end{subequations}
where variational inference introduces the Evidence Lower BOund (ELBO) of Eq. (\ref{eqn:ELBO}a) by using $q(\bm{e}_t|\cdot)$. Meanwhile, the first term in Eq. (\ref{eqn:ELBO}c) represents the probability of collecting observed $\bm{x}_t$ conditioned on $\bm{e}_t$ while the second term denotes the Kullback-Leibler (KL) divergence between the variational distribution $q(\bm{e}_t|\cdot)$ and the prior of $\bm{e}_t$. By maximizing the ELBO in Eq. (\ref{eqn:ELBO}c), we are able to increase the log-likelihood $\log p(\bm{x}_{t})$. 
Note that we avoid factorizing $p(\bm{x}_{t}|\bm{e}_t)$ with $\bm{z}_t$ in Eq. (\ref{eqn:ELBO}), and then we do not estimate the distribution of unobserved $\bm{z}_t$ and $\bm{e}_t$ simultaneously by variational inference. This is because we choose the alternative Monte Carlo (MC) sampling to approximate the posterior distribution of $\bm{z}_t$~\cite{chen2012monte}, which avoid unnecessary prior hypothesis over the mean and covariance of $\bm{z}_t$ (see Eq. (\ref{eqn:decoder})).

So far, the key of calculating the ELBO in Eq. (\ref{eqn:ELBO}c) lies in estimating $q(\bm{e}_t|\cdot)$ and $p(\bm{x}_t|\bm{e}_t)$, which can be obtained by the encoder and decoder networks, respectively. We present the intuitive illustration of CDR with the encoder and decoder networks in Figure \ref{fig:CDR}.
...
We factorize $p(\bm{x}_t|\bm{e}_t)$ by following the causal relations in the interaction generation process:
\begin{equation}
\label{eqn:decoder}
% \small
\begin{aligned}
...
\end{aligned}
\end{equation}
where $p(\bm{z}_t|\bm{e}_t, \bm{z}_{t-1})$ denotes the probability of obtaining the user preference $\bm{z}_t$ in the environment $t$; and $p(\bm{z}_{t-1}|\cdot)$ represents that of previous environment $t-1$. If $t=1$, $\bm{z}_{0}$ is set as the constant vector $\bm{0}$, otherwise $p(\bm{z}_{t-1}|\cdot)$ is estimated by $p(\bm{z}_{t-1}|\bm{e}_{t-1}, \bm{z}_{t-2})$. Furthermore, to approximate the distribution of $\bm{z}_{t}$ and $\bm{z}_{t-1}$, we use MC sampling~\cite{chen2012monte} to draw them from $p(\bm{z}_t|\bm{e}_t, \bm{z}_{t-1})$ and $p(\bm{z}_{t-1}|\bm{e}_{t-1}, \bm{z}_{t-2})$, respectively.
To iteratively calculate $p(\bm{z}_t|\bm{e}_t, \bm{z}_{t-1})$, we adopt an MLP model $f_{\theta_1}(\cdot)$ to output the $\bm{\mu}_{\theta_1}(\cdot)$ and $\bm{\sigma}_{\theta_1}(\cdot)$ of $\bm{z}_t$. Formally, we have $f_{\theta_1}(\bm{e}_t, \bm{z}_{t-1}) = \left[\bm{\mu}_{\theta_1}(\bm{e}_t, \bm{z}_{t-1}), \bm{\sigma}_{\theta_1}(\bm{e}_t, \bm{z}_{t-1})\right] \in \mathbb{R}^{2H}$. Thereafter, the remaining challenge is estimating $p(\bm{x}_t|\bm{z}_t)$ in Eq. (\ref{eqn:decoder}). 
...
Following~\cite{ma2019learning}, \edit{the user preference representation $\bm{z}_t\in \mathbb{R}^{H}$ can cover the preference over multiple item categories, and we disentangle  $\bm{z}_t$ into several category-specific preference representations.}
...
Besides, the softmax function $\pi(\cdot)$ is used to normalize $f_{\theta_2}(\cdot)$ and $\pi_i(f_{\theta_2}(\cdot))$ denotes the normalized prediction score for item $i$. Intuitively, the log-likelihood $\log p(\bm{x}_t|\bm{e}_t)$ estimates the probability of drawing observed $\bm{x}_t$ from the multinomial distribution by sampling $N_t$ times, where $N_t$ is the interaction number of user $u$ in the environment $t$.
		...
		\While{\textit{not converged}}
		    ...
		    \ForAll{$u \in \mathcal{U}$}
		        \ForAll{$t \in \{1,2,...,T\}$}
    		        ...
    		        \State Sample $\bm{z}_t$ by feeding $\bm{e}_t$ and $\bm{z}_{t-1}$ into $f_{\theta_1}(\bm{e}_t, \bm{z}_{t-1})$;
...
\small
\begin{aligned}
\mathcal{L}^u = -\sum_{t=1}^T \left( \mathbb{E}_{q_{\phi}(\bm{e}_t|\bm{x}_t)} \left[\log p_{\theta}(\bm{x}_t|\bm{e}_t)\right] - \lambda_1\cdot \text{KL}(q_{\phi}(\bm{e}_t|\bm{x}_t)\| p(\bm{e}_t)) \right),
\end{aligned}
\end{equation}
which summarizes the negative ELBO over the $T$ environments. 
