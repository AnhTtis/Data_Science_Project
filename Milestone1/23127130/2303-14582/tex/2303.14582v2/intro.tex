...
Multi-Task Learning (MTL) is an approach to combining several tasks together and learning one model for all tasks simultaneously \cite{caruana97cmu}.
...
%On the other hand, if the amount of labeled data in the target domain is small,  MTL is more likely to outperform STL.
...
%Our approach estimates a linear surrogate model 
...
Conceptually, $\theta_i$ is analogous to the importance score of a feature in random forests when hundreds of other features are available.
...
%If $S$ only contains tasks that are similar to the target task, then we expect $f(S)$ to be small.
...
%This score measures the relevance of source task $j$ to the target task .
We rigorously analyze the sample complexity of our approach in Theorem \ref{thm_converge}.
%Given $n = \textup{O}(k \log^2(k))$ randomly sampled subsets and their MTL performances, we show that the importance scores can be estimated accurately until convergence.
%See for the complete statement.
%This statement only requires $f$ to be bounded from above by a fixed constant, which is usually satisfied in practice.
...
%   Task modeling answers the above question with $80\%$ accuracy with up to four tasks in $S$ (\textbf{Bottom left}).
%   Compared with existing higher-order approximations that average first-order task affinity \cite{standley2020tasks,fifty2021efficiently}, task modeling consistently captures higher-order predictions more accurately (\textbf{Bottom right}).
%The selected source tasks are then used for multitask learning with the target task.

%We show that task modeling remains highly correlated with $f_t(S)$ as $\abs{S}$ grows.
%Additionally, task modeling accurately predicts whether a set of source tasks transfer positively to the target.

%How can we optimize a performance metric for a target task using multitask learning?
%Previous works have shown that effectively training multiple tasks together requires tackling the problem of negative transfer \cite{pan2009survey}.
%However, effectively using multitask learning requires addressing several challenges.]
%There are two prominent approaches to mitigate negative transfer in the existing literature.
%One approach is to reweight tasks during training.
%For example, negative interference could be identified through gradient dynamics \cite{}, suggesting which tasks to down weight.
%Another approach is to group related tasks together by searching through the task space using a computational framework \cite{,}.
%Scaling up these existing approaches to tens of tasks remains a challenge because of a lack of tools for extracting and modeling task relations.

%To estimate a task model, we randomly sample subsets of source tasks and get the prediction score for a target task trained with a set of source tasks.
%Then, we estimate a linear task model to fit the multitask prediction scores of each subset.
%With the above approach in mind, we first conduct a theoretical analysis to understand the working of surrogate models.

%the empirical variance of the training algorithm (equation \eqref{eq_var_fA}), for both the tabular data and the text data.
%We prove a generalization bound for our procedure, guaranteeing its convergence.
%Let $\cS$ be a distribution over a set of source tasks.
%Suppose we sample $n$ subsets $S_1, \dots, S_n$ independently from $\cS$.
%For each $i$, we use an algorithm to learn a hypothesis from a function class $\cF$, by combining the samples of all tasks in $S_i$ and a target task $t$.
%Let $f_{t}(S_i)$ denote the score of the hypothesis on a validation set of task $t$ of size $N$.
%Assume $f_{t}$ is bounded and Lipschitz-continuous.
%With probability $0.99$, the empirical risk of our estimated linear task model $\hat\theta_n$, denoted as $\hat\cL_n\big_{\theta}(\hat\theta_n\big)$, converges to the expected risk of the population risk minimizer $\theta^{\star}$, denoted as $\cL(\theta^{\star})$

%We apply our approach to  and show improved performance by applying our approach.
\smallskip
...
\begin{itemize}[leftmargin=20pt,topsep=0.0pt,itemsep=7.0pt,partopsep=0pt,parsep=0pt]
    \item The runtime for constructing surrogate models until convergence scales linearly in $k$, and the predicted performances accurately fit the true MTL performances of unseen subsets, measured by Spearman's correlation (0.8 averaged among 16 evaluations). %using only linearly many samples.
    ...
    %Lastly, we provide ablation studies to guide the parameter choices for constructing task models.
    %We show that the task selection criterion yields stable selection results under different random seeds. % and parameter choices of the surrogate model.
\end{itemize}
