...
\title{Mode-coupling theory of lattice dynamics for classical and quantum crystal}
...
\begin{abstract}
The dynamical properties of nuclei, carried by the concept of phonon quasiparticles, are central to the field of condensed matter.
...
The only inputs needed for the resulting set of equations are the static Kubo correlation functions, which can be computed using path-integral molecular dynamics or approximated with (classical or ab initio) molecular dynamics.
\end{abstract}
...
Despite its simplicity, the harmonic approximation has shown tremendous success in explaining a large number of phenomena observed in the solid state, including excitation spectra, the phase stability of various materials, elastic properties or even zero point motion~\cite{Dove1993,Fultz2010}.
...
In some cases these inaccuracies are only quantitative, but many phenomena cannot be described even qualitatively with a harmonic theory of lattice dynamics.
...
Two of the most striking examples of these limitations are probably the dynamical stabilisation of some structures~\cite{Grimvall2012}, and  the computation of thermal conductivity, which diverges in the harmonic approximation~\cite{Dove1993}.
...
Such features are incompatible with the increasing frequencies associated with a dynamical stabilisation, or the finite lifetime associated with a finite thermal conductivity.
...
Unfortunately, this approach is biased in the type of anharmonicity it includes\cite{Glensk2015}, and is often insufficient to accurately describe the frequency evolution, and is still unable by construction to incorporate a linewidth~\cite{Kim2020}.

Nevertheless, a quasiparticle picture seems to still be an adequate representation of the finite temperature vibrational excitations for most solids.
...
However, if perturbation theory can improve the agreement between harmonic phonons and more realistic vibrational QP, it is based on the assumption that the higher order anharmonic contributions will be of progressively lower importance, the harmonic contribution being dominant.
...
Moreover, a significant drawback of such approaches is that it cannot be applied at all in strongly anharmonic systems with unstable harmonic phonons.
...
However, the quadratic form of the effective Hamiltonian means that the average is computed on a multivariate gaussian distribution, which can differ substantially from the true distribution.
For instance, the renormalized phonons obtained as a result of the self-consistent equations still needs a correction to approximate the QP frequencies~\cite{Choquard1967,Bianco2017}.

Fortunately, there exists a clear way to remove the multivariate gaussian distribution approximation.
...
This feature hinders a clear vision of its limits and raises questions on the justification for the uses of perturbation theory with the effective Hamiltonian.
...
However, while the definition of classical correlation functions is unambiguous, this is not the case for a quantum system, where there exist an infinite number of ways to define one~\cite{Hele2017}.
...
Unfortunately, the freedom of choice to define quantum correlation functions is still an obstacle to a clear vision on how to apply TDEP.
...
However, this choice was justified with the goal of having a good description of the BOS, which unfortunately doesn't answer the remaining question on how to approximate the QP with the effective Hamiltonian.
...
With simple manipulations, the authors were able to show that QPs computed this way match the exact zeroth frequency of the Matsubara QP Green's function.
...
From this type of correlation functions, the missing step is how to obtain the QP dynamical properties, but the use of KCF points to the field of linear response theory, in which established theoretical tools exist to solve such problems.

With this work, we wish to build a foundation for an exact non-perturbative theory of lattice dynamics that can describe strongly anharmonic quantum systems.
...
This approach projects the dynamics on the slow variables of the system, resulting in a generalized Langevin equation (GLE) and thus splitting the equation of motion for the KCF in a conservative and in a memory dependent dissipative part.
...
Moreover, we show how our findings can justify a theory of temperature dependent phonons, thus giving an explanation of the success of renormalized phonons methods.
We also discuss the implications of the method, showing for instance how it can be used to approximate nuclear quantum effects on classical simulations all while alleviating all ambiguities related to the multiple definitions of quantum correlation functions.
By doing some comparisons we show that well installed methods such as perturbation theory or the SCHA can be though as approximations of our mode-coupling theory and that TDEP, used with (PI)MD, can be a direct application of our approach. 
An other interesting result of the present theory is that it shows the physical impossibility of the often used Lorentzian approximation of spectral lineshapes.
...
We also discuss a theory of temperature dependent phonons as well as the Lorentzian approximation of vibrational lineshapes.
...
\end{equation}
where $f_i = \liou^2 u_i / M_i$ is the force acting on atom $i$, as well as $\delta \vec{f}(t)$, which is often called the random force,
\begin{equation}
...
\end{equation}
where $A_\lambda(\qvec,t)$ are unitless quasiparticle operators with the property $\bbl A_\lambda(\qvec), A_{\lambda'}(\qvec') \bbr = \frac{2\kBT}{\hbar \Omega_\lambda(\vec{q})} \delta_{\lambda\lambda'}\delta_{\qvec,\qvec'}$\footnote{While this property can seem arbitrary, we use it to simplify comparison with other methods of anharmonic lattice dynamics} and $\rho_{\qvec,\lambda}$ is a prefactor determined below.
...
\label{eq:forces qp operator}
    f_i^\alpha = \sqrt{\frac{\hbar M_i}{2}} \sum_{\vec{q}\lambda} \frac{\epsilon_i^\lambda(\qvec)}{\sqrt{\Omega_{\lambda}(\qvec)}} \ddot{A}_{\lambda}(\qvec)
\end{equation}
...
From eq.~(\ref{eq:dyn susc Kubo}), we see that the imaginary part of the dynamical susceptibility, which corresponds to the dissipative part of the response, is simply proportional to the real part of $\widetilde{G}_{AB}(t)$.
...
\end{equation}
The only inputs are contained in the vertex $\Psi_2$.
...
\begin{equation}
\begin{split}
    \Psi_2(\lambda, \mu, \nu) =& \sqrt{\frac{\hbar}{2} \frac{\Omega_\lambda}{\Omega_\mu \Omega_\nu}} \sum_{i,j,k} \frac{\eigvec_i^\lambda \eigvec_i^\mu \eigvec_i^\nu}{\sqrt{M_i M_j M_k}} \Theta_{ijk} \\
    &\times e^{i(\vec{R}_i\vec{q}_\lambda + \vec{R}_j\vec{q}_\mu + \vec{R}_k\vec{q}_\nu)} \delta_{\vec{G}}(\vec{q}_\lambda + \vec{q}_\mu + \vec{q}_\nu)
%   \Psi_2(\lambda, \mu, \nu) =& \bigg(\frac{\hbar}{2}\bigg)^{\frac{3}{2}}\sum_{i,j,k} \frac{\eigvec_i^\lambda \eigvec_i^\mu \eigvec_i^\nu}{\sqrt{\Omega_\lambda\Omega_{\lambda'} \Omega_{\lambda''}}\sqrt{M_i M_j M_k}} \Theta_{ijk} \\
%   &\times e^{i(\vec{R}_i\vec{q}_\lambda + \vec{R}_j\vec{q}_\mu + \vec{R}_k\vec{q}_\nu)} \delta_{\vec{G}}(\vec{q}_\lambda + \vec{q}_\mu + \vec{q}_\nu)
\end{split}
\end{equation}
...
\end{split}
\end{equation}
where $P$ is the number of beads in the classical polymer, $\bar{A}(t) = \sum_p A_p(t)$ is the mean of the property $A$ over the classical polymer, and where the second line corresponds to the approximation with a finite number of configurations and beads from path integral molecular dynamics.
...
Looking at the main results in the preceding derivation, it would be easy to the miss the quantum character of the dynamics described, since most of the equations seem to lack the usual quantum-related prefactors.
...
Once $\Psi$ has been calculated, using eq.(\ref{eq:gamma lesser}) to process inputs from MD simulations is a simple zero-cost and legitimate way to introduce quantum effects without needing to do expensive PIMD simulations.
...
From the relation $\ddot{G}_{ij} = \kBT$, and using the relation eq.(\ref{eq:displ qp operator}) and (\ref{eq:forces qp operator}), we have $\ddot{G}_\lambda(\qvec) = \bbl A_\lambda(\qvec), \ddot{A}_\lambda(\qvec) \bbr = \Omega_\lambda^2(\qvec)$.
Using then the Lehmann representation of the KCF $\ddot{G}_\lambda(\qvec)$, we obtain a formulation of the frequencies in terms of a complete set of eigenstates of the Hamiltonian
\begin{equation}
    \Omega_\lambda(\qvec) = \sqrt{\sum_{jk} \omega_{jk}^2 \vert\bra{j} A_\lambda(\qvec) \ket{k}\vert^2 \big(p_k - p_j)}
\end{equation}
...
\caption{Summary of different approach of anharmonic lattice dynamics.}
\begin{ruledtabular}
...
\makecell{SSCHA~\cite{Monacelli2021} \\ Alamode~\cite{Tadano2015,Tadano2018} \\ TDEP~\cite{Shulumba2017} \\ qSCAILD~\cite{van_Roekeghem2021}} &
\makecell{TDEP~\cite{Hellman2011,Hellman2013a}} 
\end{tabular}
\end{ruledtabular}
\end{table*}
...
\end{equation}
which correspond to the $(2, 2)$-order memory kernel defined in real space, but with third-order force constants instead of the full vertex.
...
Doing a naive comparison, it would seem that the other lowest-order diagrams, known as the tadpole (Fig.\ref{fig:Diagrams} b)) and the loop (Fig.\ref{fig:Diagrams} c)), are not included in the mode-coupling theory.
...
Even if both approaches are quite similar, we should expect the mode-coupling theory to have a wider range of validity.
Indeed, being constructed on a truncated expansion of Hamiltonian with respect to displacements, perturbation theory is known to fail for systems with strong anharmonicity.
...
Such a construction should allow the mode-coupling theory to be valid even in strongly anharmonic systems.
...
Using Jensen's inequality, it can be shown that the Gibbs-Bogoliubov is a higher bound to the real free energy $\Fe < \Feff$, so that optimizing the parameters means minimizing $\Feff$.
...
On top of the quasiparticles computed from the free energy Hessian, the dynamical susceptibility can be obtained with a bubble-like contribution~\cite{Bianco2017,Monacelli2021} where the vertices are computed using the average of the third-order derivative of the potential energy
\begin{equation}
...
\end{split}
\end{equation}
where the second line can be derived with integration by parts.
...
In principle, this successive fitting can be continued to any order, though usual applications of the method truncate the potential at the third order.
...
In this case, the set of forces and displacements have to be constructed self-consistently, by refitting the second order potential from the set of the previous iteration.

It is important to differentiate the approaches.
On one hand, sTDEP is actually an implementation of the SCHA, since the self-consistent least-squares fit allows to minimize the Gibbs-Bogoliubov free energy in eq.(\ref{eq:gb free energy}).
...
with solutions
\begin{equation}
    \vec{G}(\qvec, t) = \cos(\vec{\Omega}(\qvec) t) e^{-\vec{\Gamma}^M(\qvec) t} 
\end{equation}
...
By truncating this expansion to lowest order, we were able obtain a set of self-consistent equations for the memory kernel.
Within this mode coupling approach, the only inputs needed are the static KCFs, and we showed how to compute them from path-integral or classical molecular dynamics simulations.
...
Moreover, the classical-like nature of the equations derived allowed us to justify the use of classical MD to approximate the quantum behavior of materials.
We also compared our mode-coupling scheme to the more standard perturbation theory, which allowed us to derive a scattering-like approximation to the memory kernel.
Our approach gives physical meaning to the temperature dependent phonons which appear in numerous recent works.
Moreover, we were able to relate the mode-coupling theory to other approaches of anharmonic lattice dynamics.
Notably, we showed that in the perturbative limit, the presented theory is equivalent to the usual diagrammatic formulation to lowest order, and that the SCHA can be seen as a Gaussian averaged approximation of it.
We also showed that the TDEP method used in conjunction with (PI)MD is a direct application of the mode-coupling theory, provided that static corrections to the frequencies are not applied, as they would double-count interactions.
These comparisons treat the main theories for anharmonic lattice dynamics in a unified formalism, and give a better vision of their strengths and limits.
Finally, we showed that while the commonly used Lorentzian approximation of lineshapes can be a useful approximation, it cannot be an exact description of the lattice dynamics.

The mode-coupling theory of lattice dynamics should apply to a more general class of strongly anharmonic systems, where standard methods are known to fail.
This framework can serve as a starting point for new formulations of related properties such as the thermal conductivity~\cite{Simoncelli2019,Isaeva2019,Simoncelli2022,Caldarelli2022,Fiorentino2022}.
%These derivations will be the focus of future work.

%   \begin{acknowledgments}
%   %Liege: computing time from CECI, Zenobe, LUMI, (discoverer?). funding from ARC+EOS projects. 
%   The authors acknowledge the Fonds de la Recherche Scientifique (FRS-FNRS Belgium) for PdR Grant No. T.0103.19 - ALPS, and 
%   ARC project DREAMS (G.A. 21/25-11) funded by Federation Wallonie Bruxelles and ULiege.
%   
%   % CPU
%   Simulation time was awarded by the Belgian share of EuroHPC in LUMI hosted by CSC in Finland,
%   PRACE (optospin project id. 2020225411) on MareNostrum at Barcelona Supercomputing Center
%   by the CECI (FRS-FNRS Belgium Grant No. 2.5020.11),
%   as well as the Zenobe Tier-1 of the F\'ed\'eration Wallonie-Bruxelles (Walloon Region grant agreement No. 1117545).
