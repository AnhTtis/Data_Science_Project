...
The study of Cohen-Macaulay modules over Cohen-Macaulay rings is one of the most important subjects in commutative algebra.
...
The most fundamental case where this is indeed possible is when $R$ is of {\it finite representation type}. Recall that it means there are only finitely many indecomposable Cohen-Macaulay modules up to isomorphism. 
...
Let us explain how this notion is motivated. Let $k$ be a field, and let $R$ be a commutative Noetherian $k$-algebra which we assume to be Gorenstein. Then the category $\CM R$ is naturally a Frobenius category, hence the stable category $\sCM R$ is triangulated. It is also canonically equivalent to the {\it singularity category} $\Db(\mod R)/\Kb(\proj R)$ of Buchweitz \cite{Bu86} and Orlov \cite{Or04}, which are actively studied from many branches such as representation theory, algebraic geometry, mirror symmetry, and so on. When $R$ is $\Z$-graded, we similarly have the graded singularity category $\sCM^\Z\!R$.
%\comment{relation with singularity theory, important in non-commutative methods}

Beyond finite representation type an effective way of studying Cohen-Macaulay representations is tilting theory. It aims at giving equivalences among various triangulated categories arising from different contexts, for example realizing the singularity categories as derived categories or cluster categories of finite dimensional algebras. Constructing such equivalences provides a mutual understanding of these categories and it is therefore an important subject in representation theory. %, a point of view as a generalization of Auslander's algebraic McKay correspondence.
...
for some finite dimensional algebra $A$, realizing the triangulated category $\sCM^\Z\!R$ as a canonical form of such, namely the derived category of an algebra. For the ungraded $\sCM R$, one has an equivalence \cite{haI}
\[ \sCM R\simeq (\Db(\mod A)/F)_\triangle \]
for the autoequivalence $F$ on $\Db(\mod A)$ corresponding to the degree shift functor on $\sCM^\Z\!R$. Here the symbol $(-)_\triangle$ means the triangulated hull \cite{Ke05}.%Now if $R$ is moreover a local isolated singularity of dimension $d$, then Auslander-Reiten duality implies that the stable category $\sCM R$ is a $(d-1)$-Calabi-Yau in the sense that $[d-1]$ is the Serre functor. On the other hand, we have a natural 

We intend to measure the complexity of the representation theory of $R$ in terms of the finite dimensional algebra $A$. The most fundamental class of such algebras is formed by {\it hereditary algebras}; recall that it means the algebra has global dimension at most $1$. Over an algebraically closed field, every such algebra is Morita equivalent to the path algebra of a quiver.
Among many perspectives in which they are distinguished, the first but essential property is that hereditary algebras have concise structure of their derived categories; it is well-known that every object is isomorphic to its cohomology. Such a behavior of derived categories of hereditary algebras enables us to gain better understanding of the relevant categories, and suggest the following definition.
...
\end{Def}
Thanks to Keller's theorem \cite{Ke05}, the orbit category $\Db(\mod H)/F$ is canonically triangulated when $H$ is hereditary (under a weak assumption on $F$), and therefore we do not have to the triangulated hull in the above definition.
As a first consequence of a Gorenstein ring being hereditary representation type, this theorem of Keller translates into gradability of Cohen-Macaulay modules (Theorem \ref{gradable}).
...
Let $R$ be a commutative ring which we assume to be a Gorenstein local isolated singularity of dimension $d+1$ over a field $k$. Then Auslander-Reiten duality implies that the stable category $\sCM R$ is a $d$-Calabi-Yau triangulated category in the sense that the $d$-th suspension functor $[d]$ is the Serre functor. Such triangulated categories are of substantial importance in numerous areas of mathematics. In representation theory and around, those endowed with cluster tilting objects, called the {\it cluster categories} \cite{BMRRT,Am09}, are of particular interest. They play an essential role in categorification of cluster algebras, and also provides a deep connection between singularity theory. We refer to Section \ref{NCCR} for more details which will be used in this paper.
Now Morita theorem for Calabi-Yau triangulated categories \cite{ha4} (see Theorem \ref{morita}) provides the canonical form of $d$-Calabi-Yau triangulated categories arising from hereditary algebras, which leads to the following definition.
...
is of hereditary representation type.
\end{Thm}
...
\end{aligned}
\]
It serves as a natural domain for higher dimensional Auslander--Reiten theory \cite{Iy07a,Iy07b}, which is actively studied in representation theory. %and it is characterized by the distinguished homological properties of $\End_R(M)$, called the Auslander algebra \cite{Iy07b}.
...
Such a variety of aspects of cluster tilting objects or NCCRs attracts numerous studies on the subject, from such viewpoints as representation theory, algebraic geometry, commutative algebra, and combinatorics. We refer to \cite{Br,BLV1,BIKR,DFI,Hara,HIMO,HN,Iy07b,SV17,SV20a,VdB04} and also surveys \cite{Le12,We16}, for some of the studies on NCCRs or cluster tilting modules.
...
Let $R$ be the completion of the Segre product, that is,
\[ R=\prod_{i\geq0}R^\prime_i\otimes_k R^\pprime_i. \]
...
Our main general result is the following construction of non-commutative crepant resolutions over the Segre product.
...
Trivially, if $S$ is a regular local ring, then $S\in\CM S$ is an NCCR. Applying Theorem \ref{iCT} to this very special case has the following general consequence.
\begin{Cor}[{Corollary \ref{poly}}]
Let $S_i=k[[x_{i,0},\ldots,x_{i,d_i}]]$, $1\leq i\leq n$ be the power series rings with $\deg x_{i,j}=a_{i,j}>0$. Suppose that $\sum_{j=0}^{d_i}a_{i,j}$ is common for all $i$. Then the complete Segre product $S_1\seg\cdots\seg S_n$ has an NCCR.
\end{Cor}
...
Note that Calabi-Yau algebras have intimate relationship between a class of finite dimensional algebras called higher representation infinite algebras. An independent proof (which is valid also for non-completed Segre products) of Theorem \ref{iCT} based on such algebras will appear in our forthcoming paper \cite{segre}. 
...
In this subsection we let $k$ be a commutative base ring. Every tensor product will be over $k$. For a (completely) $\Z$-graded $k$-algebra $\La$ and a subset $I\subset\Z$ we put $\proj^I\!\La:=\add\{\La(-i)\mid i\in I\}\subset\proj^\Z\!\La$, the full subcategory consisting of projective modules generated in degree $i$ with $i\in I$.
...
\begin{Setup}
Let $k$ be a field and let $R=\prod_{i\geq0}R_i$ be a completely graded commutative Noetherian $k$-algebra such that $R_0$ is finite dimensional.
\begin{enumerate}
	\item $\La^\prime$ and $\La^\pprime$ are completely positively graded module-finite graded $R$-algebras of finite global dimension $n^\prime$ and $n^\pprime$, respectively. Moreover either $\La^\prime/J^\prime$ or $\La^\pprime/J^\pprime$ is separable over $k$, where $J^\prime$ and $J^\pprime$ are the Jacobson radicals.
...
Now let
\[ \La=\Prod_{l\geq0}\Hom_{\La^\prime\cotimes\La^\pprime}^{\Z^2}(P,P(l,l)) \]
for $P=\bigoplus_{l=0}^{p-1}\La^\prime\cotimes\La^\pprime(l,0)$ the complete $p$-Segre product.
\begin{Prop}\label{gl.dim}
The complete $p$-Segre product $\La$ has global dimension $\leq m^\prime+m^\pprime-1$. If $m^\prime=n^\prime$ and $m^\pprime=n^\pprime$, the global dimension is precisely $n^\prime+n^\pprime-1$. Moreover, the projective resolution of the semisimple $\La$-modules $S^\prime(l)\otimes S^\pprime$ are computed as in Proposition \ref{resolution}.
\end{Prop}
\begin{proof}
	Let $P^\prime\to S^\prime$ and $P^\prime\to S^\pprime$ be the minimal projective resolutions in $\mod^\Z\!\La^\prime$ and $\mod^\Z\!\La^\pprime$, respectively. Then by our assumption on the location of the $\Ext$ groups, these resolutions satisfy the conditions on the projective resolution from the previous subsection. Then we get a projective resolution of $S^\prime(l)\otimes S^\pprime\in\mod^\Z\!\La$ in Proposition \ref{resolution}. Since these are all the simple $\La$-modules (up to degree shifts) we see that $\La/J_\La$ has projective dimension $\leq m^\prime+m^\pprime-1$. Since $\La$ is a module-finite $R$-algebra, this yields $\gd\La\leq m^\prime+m^\pprime-1$. If $m^\prime=n^\prime$ and $m^\pprime=n^\pprime$, the last terms of $P^\prime$ and $P^\pprime$ do not vanishi, hence so does the last term of the resolution of $S^\prime(l)\otimes S^\pprime$.
\end{proof}
...
Let $R$ be a complete Gorenstein local normal domain and $M\in\refl R$. Then the following are equivalent.
\begin{enumerate}
...
\begin{Ex}
Let $R$ be a regular local ring. Then $R\in\CM R$ is a CT module.
\end{Ex}
...
\begin{Lem}\label{a}
Let $R$ be a (completely) positively graded Noetherian normal domain, $M\in\refl R$, and $\Ga=\End_R(M)$.
\begin{enumerate}
...
\end{proof}
We are now ready to state and prove the result. Let $k$ be a perfect field, {let $R^\prime=\prod_{i\geq0}R^\prime_i$ and $R^\pprime=\prod_{i\geq0}R^\pprime_i$ be completely positively graded} commutative Gorenstein rings of dimension $\geq2$, such that $\bigoplus_{i\geq0}R_i^\prime$ and $\bigoplus_{i\geq0}R_i^\pprime$ are normal domains, and $R^\prime_0$ and $R^\pprime_0$ are finite dimensional local $k$-algebras.
%with $\dim R^\prime\geq2$ and $\dim R^\pprime\geq1$
\begin{Thm}\label{CT}
Suppose that $R^\prime$ and $R^\pprime$ have the same $a$-invariant $-p<0$, and let $R=R^\prime\seg R^\pprime$ the complete Segre product.
...
\begin{proof}
	...
	Indeed, we have $\Hom_R(M^\prime(i)\seg M^\pprime,M^\prime(j)\seg M^\pprime)=\Hom_{R^\prime}(M^\prime(i),M^\prime(j))\seg\Hom_{R^\pprime}(M^\pprime,M^\pprime)=\Ga^\prime(j-i)\seg\Ga^\pprime$ by Proposition \ref{five}, while the components of the $q$-Segre product of $\Ga^\prime$ and $\Ga^\pprime$ is $\Hom^\Z_{\Ga^\prime\cotimes\Ga^\pprime}(\Ga^\prime(i)\cotimes\Ga^\pprime,\Ga^\prime(j)\cotimes\Ga^\pprime)=(\Hom_{\Ga^\prime}(\Ga^\prime(i),\Ga^\prime(j))\cotimes\Hom_{\Ga^\pprime}(\Ga^\pprime,\Ga^\pprime))_0=(\Ga^\prime(j-i)\cotimes\Ga^\pprime)_0=\Ga^\prime(j-i)\seg\Ga^\pprime$. 
...
We can apply Theorem \ref{CT} to ${}^{q^\prime}\!R^\prime$ and ${}^{q^\pprime}\!R^{\pprime}$, both of which have $a$-invariant $-p$. Their Segre product is $\prod_{i\geq0}({}^{q^\prime}\!R^\prime)_i\otimes({}^{q^\pprime}\!R^\pprime)_i$, whose $i$-th component do not vanish only if $i$ is a multiple of both $q^\prime$ and $q^\pprime$. Now since they are relatively prime, we can write the Segre product as $\prod_{j\geq0}R^\prime_{q^\pprime j}\otimes R^\pprime_{q^\prime j}$, which is nothing but $(R^\prime)^{(q^\pprime)}\seg (R^\pprime)^{(q^\prime)}$.
\end{proof}
...
Let $k$ be an arbitrary field, and let $S_i=k[[x_{i,0},\ldots,x_{i,d_i}]]$ be the power series rings with $\deg x_{i,j}=a_{i,j}>0$. Suppose that $\sum_{j=0}^{d_i}a_{i,j}$ is common for all $1\leq i\leq n$. Then the complete Segre product $S_1\seg\cdots\seg S_n$ has a CT module.
\end{Cor}
\begin{proof}
	Note that we do not need that $k$ is perfect since it is only used in the proof of Theorem \ref{CT} to ensure that $R^\prime\cotimes R^\pprime$ is normal.
\end{proof}
...
\begin{Def}
	Let $H$ be a finite dimensional hereditary algebra, $1\neq d\in\Z$ and $a\neq0$. For a bimodule $U\in\per H^e$ such that $V^{\lotimes_Ha}\simeq\RHom_H(DH,H)[d]$ in $\rD(H^e)$, the {\it $a$-folded $d$-cluster category} is the orbit category
...
\begin{Def}\label{hered}
	...
	\end{enumerate}
\end{Def}
We also give a stronger version, in which we require that the automorphism $F$ should be the best choice.
...
for some finite dimensional hereditary algebra $H$, where $\tau^{-1/(d-1)}$ is an autoequivalence of $\Db(\mod H)$ given by a bimodule complex $U\in\per H^e$ satisfying the following.
\begin{itemize}
...
By Morita theorem \cite{ha4} (see Theorem \ref{morita} below), the right-hand-side is the canonical form of $d$-Calabi-Yau categories arising from hereditary algebras, which justifies our definition. While hereditary type is a direct ungraded analogue of the graded hereditary type, the notion of strict hereditary type is more closely related to cluster tilting theory and the notion of ``strict tilting'' which we shall introduce and study in a forthcoming work \cite{segre}.
...
Even though some parts of the following results can be seen from the classification in the setting of commutative rings, we state them since the structure theorems above holds in much greater generality. 
...
\subsection{Consequences of hereditary representation type}
...
\end{Thm}
\begin{proof}
	We know by Proposition \ref{sg's} that $\sCM R\simeq\sCM\widehat{R}$, so it suffices to show that the given triangle equivalence $\sCM^\Z\!R\simeq\Db(\mod H)$ induces $\sCM R\simeq\Db(\mod H)/F$ for the autoequivalence $F$ of $\Db(\mod H)$ corresponding to the degree shift $(1)$ on $\sCM^\Z\!R$. Let $M\in\sCM^\Z\!R$ the object corresponding to $H\in\Db(\mod H)$. We only have to verify the conditions in Theorem \ref{orbit}.
...
As the first example, we give an interpretation of a Gorenstein ring of countable representation type in this context.
\begin{Ex}
Consider the ring
\[ R=k[[x,y]]/(y^2), \]
...
%\comment{
%The second example is given by Geigle-Lenzing's weighted projective lines.
%\begin{Ex}
%Let $n\geq2$ and pick integers $p_1,\ldots,p_n\geq2$ and non-zero distinct scalars $\la_3,\ldots,\la_n\in k$. Let
%\[ R=k[x_1,\ldots,x_n]/(x_i^{p_i}-x_1^{p_1}-\la_ix_2^{p_2}\mid 3\leq i\leq n) \]
%be the complete intersection ring. It is graded by the abelian group $\LL=(\Z\vec{c}\oplus\bigoplus_{i=1}^n\vec{x_i})/(\vec{c}-p_i\vec{x_i}\mid 1\leq i\leq n)$.
%\end{Ex}
%}
...
One of the strongest results to give equivalences of categories is Morita-type theorem. Let us start with the following results which characterize cluster categories in terms of cluster tilting objects, and which we aim to apply. Recall that a finite dimensional algebra $H$ is {\it $1$-representation infinite} if every connected component of $H$ is a representation infinite hereditary algebra.
...
\[ \NN\times L\setminus S=\{(0,\la)\mid \la\neq1\}\cup \{(1,\la)\}. \]
...
\begin{proof}
	...
	which computes $\Tor_i^S(k,E)$. We have $k\lotimes_SE=k\lotimes_SDS=D\RHom_S(k,S)=D(k(n)[-d])=k(-n)[d]$, so that the above sequence is acyclic except at the leftmost term, where it has $1$-dimensional cohomology in degree $n$. We obtain the conclusion by taking the degree $-m$ part.
\end{proof}
...
So far we have been discussing {\it Gorenstein} rings of hereditary representation type. In this section, we give some {\it non-Gorenstein} examples whose Cohen-Macaulay representation can be controlled by quiver representations, thus should be regarded as being hereditary representation type. %(although we have not given the definition for general Cohen-Macaulay rings).
...
\begin{Prop}[{\cite[Theorem 3.2]{DL}}]\label{DL}
