...
%\pdfoutput=0 % if your are submitting a pdflatex (i.e. if you have
...
\newcommand{\ardian}[1]{[***\texttt{#1}***]}
\newcommand{\handhika}[1]{[*\textit{#1}*]}
\newcommand{\mc}[1]{\mathcal{#1}}
\usepackage[T1]{fontenc} % if needed

%\title{Generators for BPS Equations of The Generalized Maxwell-Higgs Model}
\title{First-order formalism for BPS Alice strings}
...
%\author[a,b,1]{A. N. Atmaja,\note{Corresponding author.}}
\author[a]{E. Acalapati}%\note{Present address after 01/10/2022: Department of Applied Mathematics and \mbox{Theoretical} Physics, University of Cambridge, Cambridge, CB3 0WA, UK}
%\author[d]{and E. da Hora}
\author[a,1]{and H.~S.~Ramadhan}\note{Corresponding author.}
% The "\note" macro will give a warning: "Ignoring empty anchor..."
% you can safely ignore it.

%\affiliation[a]{Quantum Science Centre, Department of Physics, Faculty of Science, University of Malaya, 50603 Kuala Lumpur, Malaysia.}
%\affiliation[b]{Research Center for Physics, Indonesian Institute of Sciences (LIPI), Kompleks PUSPIPTEK Serpong, Tangerang 15310, Indonesia.}
\affiliation[a]{Departemen Fisika, FMIPA, Universitas Indonesia, Depok, 16424, Indonesia.}
%\affiliation[d]{Departemento de F\'{i}sica, Universidade Federal do Maranh\~{a}o, 65080-805, S\~{a}o Lu\'{i}s, Maranh\~{a}o, Brazil.}

% e-mail addresses: one for each author, in the same order as the authors
%\emailAdd{ardian\_n\_a@um.edu.my}
%\emailAdd{muhammad.ezra81@ui.ac.id}
%\emailAdd{meam3@cam.ac.uk}
%\emailAdd{hramad@sci.ui.ac.id}
%\emailAdd{edahora.ufma@gmail.com}

%\abstract{\handhika{We modify the recent {\it on-shell}  Bogomolnyi method and apply it to construct generators for BPS equations in the generalized Abelian Higgs theory. Our result shows a surprisingly rich family of BPS equations, not revealed in the off-shell counterpart. Our formalism also enables us to generate variety of potentials that give rise to different vacuum topology. The resulting equations can be classified into topological, semi-topological, and non-topological; depending on the nontriviality of the corresponding vacuum manifolds.}}

\abstract{
We revisit the Bogomolny equations for Alice string in the $U(1)\times SO(3)$ model, and show that they can be obtained by means of the first-order formalism. We then apply the same formalism to obtain the Bogomolny-Prasad-sommerfield (BPS) Alice strings in two other models which recently gain important role in cosmology and particle physics: the $SU(2)\times U(1)_{PQ}$ Lazarides-Shafi-inspired model, and the $SU(1)\times U(1)$ Aharonov-Bohm defect model with additional doublet scalar field. We also present their corresponding numerical solutions. This is the first systematic study of obtaining the Bogomolny equations for various Alice string models.}

%The BPS equations are very useful when used for solving fields in a system with topological defects. A more systematic method of finding the equations is by using the First-Order Formalism. In our research, the First-Order Formalism is used for models containing Alice strings to obtain their BPS equations potentials required for the equations to exist. The models used are a model containing both $SU(2)$ and $U(1)$ gauge fields, a model with only one $U(1)$ gauge field and a model containing both $SU(2)$ and $U(1)$ gauge fields with an additional doublet scalar field.}
...
Four decades ago, Schwarz found a class of topological vortices in non-Abelian gauge theories, dubbed the {\it Alice strings}, where the unbroken symmetries become multi-valued when parallel-transported around it~\cite{Schwarz:1982ec, Benson:2004ue}. This happens because the unbroken group $H$ contains both the usual $U(1)$ electrodynamics (with charge operator $Q$) and an element $X$ of the disconnected component of $H$ such that 
...
In field theory, the Bogomolny (or the BPS, interchangeably) equations are a very convenient way of solving systems with topological defects, since they consist of first-order differential equations that satisfy the original Euler-Lagrange (which is of second-order). Since every reduction attempt to simpler equations are always appreciated. Physically, the BPS solutions represent the most stable defect configuration since it is the state with the lowest global energy. This BPS energy can be shown to be proportional to its topological charge. The original Bogomolny mechanism relies so much on ``trial-and-error" method~\cite{Bogomolny:1975de}. Although successful for the standard canonical defects, this method is of little help in the face of various non-canonical or exotic non-standard defects. To overcome this difficulty, several formalism have been proposed to build systematic algorithm for obtaining the BPS equations. Bazeia, Losano, and Menezes observed that stability in static field theory implies the {\it stressless condition}, $T_{ij}=0$~\cite{Bazeia:2008tj, Bazeia:2007df}. In~\cite{Adam:2013hza} the authors generalizes the notion of self-duality to construct the BPS equations and their corresponding topological charge. One of us (HSR) and collaborators showed that the BPS condition can be derived directly from the Euler-Lagrange equations via separation of variable without the need to appeal to the Hamiltonian; we dubbed the {\it on-shell method}~\cite{Atmaja:2014fha, Atmaja:2015lia}. In~\cite{Atmaja:2015umo}, Atmaja suggests a method for obtaining BPS equations by relying on the so-called {\it BPS energy function} $Q$. This method is suitable for BPS defects with non-canonical kinetic terms. In~\cite{Bazeia:2017nas} the authors developed the so-called {\bf first-order formalism} to obtain BPS equations for generalized vortices. Interestingly, they did not start from the stressless condition as in~\cite{Bazeia:2008tj, Bazeia:2007df}. Rather, by assuming axial symmetry of the energy-momentum tensor, they show that the stressless condition is the result of stability under energy rescaling.

In this work, we shall use the first-order formalism~\cite{Bazeia:2017nas} to obtain BPS Alice strings for several models, including the $SU(2)\times U(1)$ Chatterjee-Nitta model discussed above~\cite{Chatterjee:2017jsi}. The first-order formalism is able to obtain their BPS equations. The other models we consider are: (i) {\it the $SU(2)\times U(1)_{PQ}$ string in the Lazarides-Shafi mechanism}, and (ii) {\it the $SU(1)\times U(1)$ with additional doublet scalar field in the Aharonov-Bohm defects}.

 
\section{First-order Formalism for Non-Abelian Multi-field Vortices}
%\label{sec: model}
\label{sec: formalism}

Alice strings can only exist in non-Abelian configuration. Thus, in this section we shall discuss the first-order formalism for generalized non-Abelian multi-field vortices. We start from the most general Lagrangian containing multi Higgs and gauge fields. The gauge fields can either be: Maxwell and $(m-1)$ Yang-Mills, or pure $m$-Yang-Mills. The Lagrangian is as follows:
\begin{equation}
		\label{gen Lag}
...
\begin{equation}
\Phi\equiv\left(\phi_1\ \phi_2.... \phi_n\right)^T, \ \ A^{\mu}_j\equiv\left(A^{\mu}_1, A^{\mu}_2,...\right)\equiv\left(a^{\mu}, A^{a\mu},...\right), 
\end{equation}
...
\end{equation}
where no summation is implied on $j$, and $G^{\mu\nu}_j$ denotes the field tensor for either Maxwell $G^{\mu\nu}_{(j)}\equiv \partial^\mu a^\nu - \partial^\nu a^\mu$ or Yang-Mills $G^{\mu\nu}_{(j)}\equiv \partial^\mu A^\nu - \partial^\nu A^\mu - ig_j [A^\mu,A^\nu]$ with $g_j$ the coupling constant of the corresponding symmetry group. Details on how many fields are involved depend on the models considered. Here, $\mathcal{L_M}\equiv\partial\mathcal{L}/\partial\mathcal{M}$. The corresponding Euler-Lagrange equations and energy-momentum tensors are
\begin{eqnarray}
\label{EL}
...
The second fundamental assumption, {\it i.e.,} the stability under rescaling, is essentially the {\it Derrick's theorem} for static defects~\cite{Derrick:1964ww}. The argument is as follows. For static axially-symmetric vortices, the total energy is
\begin{equation}
...
\end{equation}
Derrick's theorem states that the condition for stability implies
\begin{equation}
...
\end{equation}
where $M\left(f_i,r\right)$ in general is an algebraic function of $f_i$ (and $r$) but not their derivative. Eq.~\eqref{eq:bps2} constitutes the {\bf second} Bogomolny eqiations.
...
\begin{equation}
W_{f_i}\equiv\frac{dW}{df_i}\equiv2r\mathcal{L}_Xf_i',\ \ \ W_{A_j}\equiv\frac{dW}{dA_j}\equiv\mathcal{L}_Y\frac{A_j'}{g_j^2r}.
\end{equation}
...
\end{eqnarray}
In the next section we shall apply these mechanisms to several models of Alice strings and obtaining their corresponding BPS configurations.
...
This is the Alice strings considered by Chatterjee and Nitta~\cite{Chatterjee:2017jsi}, where they found and solved the BPS equations. We shall show how to derive the same BPS equations using the first-order formalism. The Lagrangian is given by
\begin{equation}
\label{lag1}
\mathcal{L}=|\Tr D_{\mu}\Phi|^2-\frac{1}{2}\Tr F_{\mu\nu}F^{\mu\nu}-\frac{1}{4}f_{\mu\nu}f^{\mu\nu}+V,
\end{equation}
where $D_{\mu}\Phi\equiv\partial_{\mu}\Phi-iea_{\mu}\Phi-ig\left[A_{\mu},\Phi\right]$, $F_{\mu\nu}\equiv\partial_{\mu}A_{\nu}-\partial_{\nu}A_{\mu}-ig\left[A_{\mu},A_{\nu}\right]$, and $f_{\mu\nu}\equiv\partial_{\mu}a_{\nu}-\partial_{\nu}a_{\mu}$. The gauge fields $a_{\mu}$ and $A_{\mu}$ refers to the Maxwell and Yang-Mills fields, respectively. Here $V=V\left(\Phi, \Phi^{\dag}\right)$, and note that we do not specify the potential beforehand. Unlike the original Bogomolny's trick~\cite{Bogomolny:1975de}, various alternative methods for obtaining BPS equations~\cite{Bazeia:2008tj, Bazeia:2007df, Adam:2013hza, Atmaja:2014fha, Atmaja:2015lia, Atmaja:2015umo, Bazeia:2017nas} do not start from a given potential. Instead, the form of the potential is deduced such that it satisfies the Bogomolny equations and the corresponding non-trivial topology suitable for the existence of the defects.
...
\Phi=\Phi(r,\theta)&=&\begin{pmatrix}
			...
			f_2(r) & 0
		\end{pmatrix},\\
a_{\mu}=a_i(r,\theta)&=&-\epsilon_{ij} \frac{x_j}{er^2}\biggl(a(r)-\frac{n}{2}\biggr),\\
A_{\mu}=A_i(r,\theta)&=&-\epsilon_{ij} \frac{x_j}{gr^2}\biggl(A(r)-\frac{n}{2}\biggr) \frac{\sigma^3}{2}.
\end{eqnarray}
...
\begin{eqnarray}
\label{exp1}
\frac{a'+A'}{r}f_1 + \frac{1}{2}\mathcal{L}_{f_1} &=& 0,\\
\frac{a'-A'}{r}f_2 + \frac{1}{2}\mathcal{L}_{f_2} &=& 0.
\end{eqnarray}
Eqs.~\eqref{exp1} are the second Bogomolny equations in this model. With these conditions, \ref{scalc} becomes
	\begin{equation}
		\label{scal1}
		-y-Y-V = 0.
	\end{equation}
...
\centering
	...
	\hspace{-1.5cm}
\includegraphics[scale=0.6]{Case1_l1.pdf}
	\caption{$\ell=1$.}
	\label{fig:sol1a}
	\end{subfigure}
		\begin{subfigure}{.5\textwidth}
			\hspace{-1.5cm}
\includegraphics[scale=0.6]{Case1_l2.pdf}
			\caption{$\ell=2$.}
		\label{fig:sol2b}
		\end{subfigure}
	...
	% 	\begin{subfigure}{.5\textwidth}
	% 		\hspace*{-1cm}
	% 		\includegraphics[scale=0.5]{Case1_V1.pdf}
	% 		\caption{Potential term $V_1$.}
	% 		\label{fig:sol1V1}
	% 	\end{subfigure}%
	% 	\begin{subfigure}{.5\textwidth}
	% 		\hspace*{1cm}
	% 		\includegraphics[scale=0.5]{Case1_V2.pdf}
	% 		\caption{Potential term $V_2$.}
	% 		\label{fig:sol1V2}
	% 	\end{subfigure}
	% 	\begin{subfigure}{.5\textwidth}
	% 		\hspace*{0cm}
	% 		\includegraphics[scale=0.5]{Case1_V.pdf}
	% 		\caption{Total potential $V$.}
	% 		\label{fig:sol1Vt}
	% 	\end{subfigure}
	% 	\caption{Potential graphs.}
	% 	\label{fig:sol1V}
	% \end{figure}
	
\pagebreak
\subsection{The $SU(2)\times U(1)_{PQ}$ Strings in the Lazarides-Shafi Mechanism}
\label{case2}

In order to explain the axionic dark matter without the domain-wall abundance problem, Lazaridez and Shafi proposed an axion model based on spontaneously-broken Peccei-Quinn (PQ) {\it discrete} subgroup embedded in the continuous Spin$(10)$ symmetry~\cite{Lazarides:1982tw}. This model soon becomes the seminal mechanism that inspires axionic model buildings, among one of which is the Kim-Shifman-Vainshtein-Zakharov (KSVZ) axion model~\cite{Kim:1979if, Shifman:1979if}. It was realized that the KSZV model contains Alice string in its spectrum as the decay product of unstable axionic string~\cite{Sato:2018nqy}. When domain walls are formed in this model each of them will be attached by a single Alice string, and such configuration renders the wall network unstable due to its tension; thus evading the wall abundance problem~\cite{Chatterjee:2019rch}.

The non-Abelian vortex in the Lazarides-Shafi mechanism is as follows~\cite{Chatterjee:2019rch}
\begin{equation}
\label{lag2}
\mathcal{L}=-\frac{1}{2}\Tr F_{\mu\nu}F^{\mu\nu}+\Tr  D_\mu\Phi^\dagger D^\mu\Phi-V(\Phi)+\left(y\bar{Q}\Phi\bar{Q}+H.c.\right),
\end{equation}
where $\left(\bar{Q},Q\right)$ are pairs of extra heavy quarks in the $SU(3)_c$ gauge symmetry. The ansatz we use is\footnote{Note that our ansatz for the scalar field differs from the original one,
$\Phi(r,\theta)=\begin{pmatrix}
0 & f_1(r)e^{in\theta}\\
f_2(r) & 0
\end{pmatrix}$~\cite{Chatterjee:2019rch}, by switching off $f_2(r)$. This is because we have been unable to find the BPS equations in the original set up. Nevertheless, this modified ansatz with $n = 1$ still satisfies the requirement for non-Abelian string.}
	\begin{eqnarray}
		\label{ansatz2}
		\Phi(r,\theta) &=& f_1(r)
		\begin{pmatrix}
			...
			e^{-i\frac{n}{2}\theta} & 0
		\end{pmatrix},\\
		A_i(r,\theta) &=& -\epsilon_{ij} \frac{x_j}{gr^2}\bigg(A(r)-\frac{n}{2}\bigg) \frac{\sigma^3}{2},
	\end{eqnarray}
	with $X$ and $Y$ are expressed as
	\begin{eqnarray}
		\label{def explicit 2}
		X &=& -2\biggl(f_1'^2+\frac{A^2}{r^2}f_1^2\biggr),\\
		Y &=& -\frac{A'^2}{2g^2r^2}.
	\end{eqnarray}

%	Notice that there is no $y$ that represents the $U(1)$ gauge field.
	
Following the same formalism as in the previous model, we have the following equation
\begin{equation}
\label{sym2}
f_1'^2=\frac{A^2f_1^2}{r^2},
\end{equation}
while the expanded Euler-Lagrange equation is
\begin{equation}
\label{exp2}
\frac{A'}{r}f_1 + \frac{1}{2} \mathcal{L}_{f_1} = 0.
\end{equation}
We can also identify, following \eqref{scal1},
\begin{equation}
\label{scal2}
V=-Y\rightarrow\frac{A'}{r}=g\sqrt{2V}.
\end{equation}
Inserting Eq.~\eqref{exp2} into \eqref{scal2} we obtain
\begin{eqnarray}
\label{V2}
g\sqrt{2V} f_1 - \frac{1}{2} V_{f_1} = 0,
\end{eqnarray}
which yields
\begin{eqnarray}
V(\Phi) = \frac{g^2}{4} Tr~\Bigl(\Phi^\dagger\Phi - \xi^2\Bigr)^2.
\end{eqnarray}

The BPS equations are thus
\begin{eqnarray}
\label{BPS2}
f_1'&=&\frac{Af_1}{r},\\
\frac{A'}{r}&=&g^2\Bigl(f_1^2 - \xi^2\Bigr).
\end{eqnarray}
Imposing the following boundary conditions
\begin{eqnarray}
\label{bound2}
f_1(0)&=&0,\ \ \ \ \ f_1(\infty)= \xi,\nonumber\\
A(0) &=& \frac{n}{2},\ \ \ \ \ A(\infty)=0,
\end{eqnarray}
we solve the Eq.~\eqref{BPS2} numerically for an Alice string ($n = 1$) . The solution is shown in Fig.~\ref{fig:solLS}.
\begin{figure}[h]
\centering
\hspace{-0.5cm}
\includegraphics[scale=0.45]{Case2_g1r.pdf}
\caption{Numerical solutions for the fields with $g=1$ and $n=1$.}
\label{fig:solLS}
\end{figure}


\subsection{$SU(2)\times U(1)$ Aharonov-Bohm String}
\label{case3}

When a field that develops VEV receives a nontrivial Aharonov-Bohm (AB) phase, the corresponding defects appears is called the Aharonov-Bohm defects~\cite{Georgi:1985nv, Chatterjee:2018znk}. In~\cite{Chatterjee:2019zwx} the authors show how Alice string can be an AB defect. In this model, an additional doublet scalar field $\Psi$ is added to the system. The Lagrangian is as follows
\begin{equation}
\label{Lag3}
\mathcal{L} = -\frac{1}{2} F_{\mu\nu}F^{\mu\nu} - \frac{1}{4} f_{\mu\nu}f^{\mu\nu} + Tr~D_\mu\Phi^\dagger D^\mu\Phi + D_\mu\Psi^\dagger D^\mu \Psi - V.
\end{equation}
Note that it is almost identical to the Lagrangian~\eqref{lag1} with the exception of the appearance of the field $\Psi$. The Euler-Lagrange equations due to $\Psi$ are
\begin{eqnarray}
\label{EQ3}   
\frac{\partial\mathcal{L}}{\partial\Psi^\dag}&=&\frac{\mathcal{L}_{|\Psi|}}{2|\Psi|}\Psi+\mathcal{L}_{\chi}\left(-i\frac{e_{\psi}}{2}a_{\mu}D^{\mu}\Psi-iA_{\mu}D^{\mu}\Psi\right),\nonumber\\
\partial{\mu}\frac{\partial\mathcal{L}}{\partial\partial_{\mu}\Psi^\dagger}&=&\partial_{\mu}\left(\mathcal{L}_{\chi}D^{\mu}\Psi\right),\nonumber\\
D_{\mu}\left(\mathcal{L}_{\chi}D^{\mu}\Psi\right)&=&\frac{\mathcal{L}_{|\Psi|}}{2|\Psi|}\Psi,
\end{eqnarray}
where 
\begin{equation}
\label{chi}
\chi\equiv D_\mu\Psi^\dagger D^\mu\Psi.
\end{equation}
We employ the same ansatze~\eqref{ansatz} and
\begin{equation}
\label{ansatz3}
\Psi = \begin{pmatrix}
\psi(r) \\
...
\end{pmatrix}.
\end{equation}
The kinetic term \eqref{chi} is expressed as
\begin{eqnarray}
\label{var3}
\chi=-\psi'^2 - \frac{1}{r^2} \biggl(e_\psi\bigg( \frac{a-n/2}{2e} \bigg)+\frac{A-n/2}{2g}\biggr)^2 \psi^2.
\end{eqnarray}


Eq.~\eqref{tijprime} for $\Psi$ becomes
\begin{equation}
\label{EL3}
\frac{1}{r}\bigl(r\mathcal{L}_{\chi}\psi'\bigr)' - \frac{1}{r^2} \biggl(e_\psi\bigg( \frac{a-n/2}{2e} \bigg) + \frac{A-n/2}{2g}\biggr)^2\psi + \frac{\mathcal{L}_{\psi}}{2} = 0,
\end{equation}
which can be expanded as 
	\begin{equation}
		\label{exp3}
		\psi'' + \frac{\psi'}{r} + \frac{\mathcal{L}_{\psi}}{2} - \frac{1}{r^2} \biggl( e_\psi\bigg( \frac{a-n/2}{2e} \bigg) + \frac{A-n/2}{2g}\biggr)^2 \psi = 0.
	\end{equation}
By axial symmetry, on the other hand, the first Bogomolny equation for the second scalar field is
	\begin{equation}
		\label{eqn:axsym3}
		\psi' = \biggl(e_\psi\bigg( \frac{a-n/2}{2e} \bigg)+\frac{A-n/2}{2g}\biggr) \frac{\psi}{r}.
	\end{equation}
The equation for the gauge fields are given by the same expressions as in Eqs.\eqref{V121a}-\eqref{V121b}.
Inserting them all into Eq.~\eqref{exp3} yields the following constraint 
	\begin{equation}
		\label{3A}
		\Bigl(e_\psi \sqrt{2V_1} + \sqrt{2V_2}\Bigr) \psi - \frac{\partial V_1}{\partial \psi} - \frac{\partial V_2}{\partial \psi} = 0,
	\end{equation}
which in turn gives us
\begin{eqnarray}
\label{constraintV}
		\frac{\partial V_1}{\partial \psi} &=& e_\psi \sqrt{2V_1} \psi,\nonumber\\
		\frac{\partial V_2}{\partial \psi} &=& \sqrt{2V_2} \psi.
\end{eqnarray}


Solving Eqs.~\eqref{constraintV} we obtain
\begin{eqnarray}
\label{sqrV1}
\sqrt{2V_1}&=&\frac{e_{\psi}}{2}\left[\psi^2+C\left(f_1,f_2\right)\right],\nonumber\\
\sqrt{2V_2}&=&\frac{1}{2}\left[\psi^2+C\left(f_1,f_2\right)\right].
\end{eqnarray}
On the other hand, since this model satisfies Eqs.~\eqref{exp1}, the corresponding potential also contains the \eqref{V1f1} and \eqref{V2f2} terms, 
\begin{eqnarray}
\label{sqrV2}
\sqrt{2V_1}&=&e\left[f_1^2+f_2^2+B_1(\psi)\right]^2,\nonumber\\
\sqrt{2V_2}&=&g\left[f_1^2-f_2^2+B_2(\psi)\right]^2, 
\end{eqnarray}
where $B_1$ and $B_2$ are two integration constants which are functions of $\psi$ only. Eqs.~\eqref{sqrV1}-\eqref{sqrV2} can be combined as follows
%\begin{eqnarray}
%\sqrt{2V_1}&=&e\left(f_1^2+f_2^2-%2\xi^2\right)+\frac{e_{\psi}}%{2}\left(\psi^2-%C_1^2\right),\nonumber\\
%\sqrt{2V_2}&=&g\left(f_1^2-f_2^2-%C_2^2\right)+\frac{1}{2}\left(\psi^2-%C_3^2\right),
%\end{eqnarray}
%or
\begin{eqnarray}
V_1&=&\left[\frac{e}{\sqrt{2}}\left(f_1^2+f_2^2-2\xi^2\right)+\frac{e_{\psi}}{\sqrt{8}}\left(\psi^2-C_1\right)\right]^2,\nonumber\\
V_2&=&\left[\frac{g}{\sqrt{2}}\left(f_1^2-f_2^2-C_2\right)+\frac{1}{\sqrt{8}}\left(\psi^2-C_3\right)\right]^2,
\end{eqnarray}
Those equations can be combined as:
\begin{eqnarray}
		\label{Vr1}
		V(\Phi) &=& V_1(\Phi)+V_2(\Phi),\nonumber\\
		&=& \bigg[ \frac{e}{\sqrt{2}} \Big( f_1^2 + f_2^2 -2\xi^2 \Big) + \frac{e_\psi}{\sqrt{8}} \Big(\psi^2 - C_2^2\Big)  \bigg]^2 + \bigg[ \frac{g}{\sqrt{2}}\Big( f_1^2 - f_2^2 - C_1^2\Big) + \frac{1}{\sqrt{8}} \Big(\psi^2 - C_3^2\Big)  \bigg]^2\nonumber\\
        &=& \frac{e^2}{2} \Big( Tr \Phi^\dagger \Phi - 2\xi^2 \Big)^2 + \frac{g^2}{4} Tr[\Phi^\dagger,\Phi]^2 + \frac{e_\psi^2}{8} \Big(\Psi^\dagger\Psi - C_2^2\Big)^2 + \frac{1}{8} \Big(\Psi^\dagger \Psi - C_3^2\Big)^2\nonumber\\
        && + \frac{ee_\psi}{2} \Big( Tr \Phi^\dagger \Phi - 2\xi^2 \Big) \Big(\Psi^\dagger \Psi - C_2^2\Big) + \frac{g}{2} \sqrt{\frac{Tr[\Phi^\dagger,\Phi]^2}{2}} \Big(\Psi^\dagger \Psi - C_3^2\Big).
\end{eqnarray}
Without any loss of generality, we can set $C_1=0$ and $C_2=C_3=1$.
 
The BPS equations can now be expressed as
\begin{eqnarray}
		\label{BPS3}
		f_1' &=& \frac{\bigl(a+A\bigr)f_1}{r},\\
		f_2' &=& \frac{\bigl(a-A\bigr)f_2}{r},\\
		\psi' &=& \biggl(\frac{e_\psi}{2}\frac{a}{e}+\frac{A}{2g}\biggr)\frac{\psi}{r},\\
		a' &=& e^2r \Bigl(f_1^2+f_2^2-2\xi^2\Bigr) + \frac{ee_\psi}{2}r\Bigl(\psi^2 \Bigr),\\
		A' &=& g^2r \Bigl(f_1^2-f_2^2\Bigr) + \frac{g}{2}r\Bigl(\psi^2-1\Bigr).
\end{eqnarray}
Applying the following boundary conditions
\begin{eqnarray}
\label{bound3}
f_1(0) = f_2'(0) = \psi(0) &=& 0,\ \ f_1(\infty) = f_2(\infty)=\xi,\ \ \psi(\infty)=1,\nonumber\\
\nonumber\\
a(0)=A(0) &=& \frac{n}{2},\ \ a(\infty)=A(\infty)=0,
\end{eqnarray}
\begin{figure}[h]
\centering
\hspace*{0cm}
\includegraphics[scale=0.7]{Case3_l1.pdf}
\caption{BPS Aharonov-Bohm Alice string profiles for $e/g = 1$ and $n=1$.}
\label{fig:sol3}
\end{figure}
we solve them numerically with $n = 1$. The solutions are shown in Fig.~\ref{fig:sol3}. 	
	% \begin{figure}[h]
	% 	\centering
	% 	\begin{subfigure}{.5\textwidth}
	% 		\hspace*{-1cm}
	...
	% 	\begin{subfigure}{.5\textwidth}
	% 		\hspace*{0cm}
	% 		\includegraphics[scale=0.5]{Case3_V.pdf}
	% 		\caption{Total potential $V$.}
	% 		\label{fig:sol3Vt}
	% 	\end{subfigure}
	% 	\caption{Potential graphs.}
	...
	% 	\centering
	% 		\hspace*{0cm}
	% 		\includegraphics[scale=0.5]{Case3_Vpar.pdf}
	% 		\caption{$f_1$ vs. $f_2$.}
	% 		\label{fig:sol3Vpar}
	% \end{figure}
	% \pagebreak
...
In this work we revisit the BPS equations of one type of exotic defects called Alice string and try to systematize it. To be concrete, we consider three models of Alice strings: the $SU(2)\times U(1)$ Chatterjee-Nitta string~\cite{Chatterjee:2017jsi}, the $SU(2)$ string in the Lazarides-Shafi mechanism~\cite{Chatterjee:2019rch}, and the $SU(2)\times U(1)$ Aharonov-Bohm string~\cite{Chatterjee:2019zwx}. We obtain the BPS equations for each string using the first-order formalism developed by Bazeia \textit{et al.} \cite{Bazeia:2017nas}. This formalism relies on {\it axial symmetry} and {\it Derrick's theorem}. From the first assumption one can (rather) easily ``guess" the BPS equation for the scalar field. The second assumption implies the stressless condition, which implies the BPS equation for the gauge field. these two equations must be consistent in a sense that both should satisfy the constraint equation. In this formalism, the potential is not put by hand. Rather, we start from unspecified potential and move forward by applying the algorithm to obtain the suitable BPS equations. The potential is obtained from the equation(s) of constraint. 
...
For the first model, Chatterjee and Nitta derived the BPS equations for the $SU(2)\times U(1)$ Alice string using the usual Bogomolny's {\it smart trick}~\cite{Bogomolny:1975de}. Here we derive the same BPS equations using the first-order formalism, and obtain the same potential up to some constant. More interesting cases are the second and third models, whose BPS states, to the best of our knowledge, are not known. We obtain the Bogomolny equations for both the $SU(2)$ Lazarides-Shafi string and the $SU(2)\times U(1)$ Aharonov-Bohm string, as well present their numerical solutions. However, for the second model we modify the ansatz since under this formalism we cannot find the suitable ``guessing" for the original one. It is possible that  by relaxing the condition~\eqref{eq:bps1} we might get around this situation. In the third model, the Alice string can be shown to be an example of the Aharonov-Bohm (AB) defects. Here the system is coupled to an additional scalar field. Our result dictates that the appropriate potential term for the BPS' existence AB Alice takes the form of~\eqref{Vr1}. To the best of our knowledge, our result is the first BPS equations for such AB string.
...
For future research, the first-order formalism can be applied to other types of non-abelian strings. It can also be used for more generalised versions of Alice strings or non-Abelian strings. We also hope that our results would shed light for the applications of Alice strings in the field of phenomenology such as in the field of condensed matter and, more especially, in the application of Alice strings in superfluids \cite{Fujimoto:2020dsa}-\cite{Masaki:2021hmk}.
...
\acknowledgments
We thank Ardian Atmaja and Ilham Prasetyo for the fruitful discussion.
