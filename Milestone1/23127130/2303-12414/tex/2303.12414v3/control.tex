...
where $E_{\textrm{c,LocalAgg}}= \sum_{j\in \mathcal{S}_c} M\times Q\times p_{j}/R_j^{(t)}$ is the energy consumption of conducting local model aggregation at edge server $n_c$, where $M$ denotes the size of the model (i.e., number of model parameters), $Q$ denotes the number of bits used to represent each model parameter, which is dependent on the quantization level, $p_{j}$ denotes the transmit power of device $j\in\mathcal S_c$,  and $R_j^{(t)}=W\log_2 \left(1+ \frac{p_j \vert {h}^{(t)}_{j}\vert^2}{N^2} \right)$ is the transmission rate between device $j\in\mathcal S_c$ and its associated edge server $n_c$ at time $t$. The noise power is represented by $N^2=N_0 W$, with $N_0$ as the white noise power spectral density, $W$ as the bandwidth, and ${h}^{(t)}_{j}$ as the channel coefficient.
