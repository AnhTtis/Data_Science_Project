...
\begin{enumerate}
    \item $B$ \textit{duplicates} $A$ if $B$ is a disjoint set of urelements that is equinumerous with $A$.
    \item $B$ is a \textit{tail} of $A$ if $B$ is disjoint set of urelements and for every $C \subseteq \A$ disjoint from $A$, there is a $D \subseteq B$ that duplicates $C$.
...
\end{theorem}
\begin{proof}
(1) Assume Tail$^+$. Consider the well-ordered tail for $\emptyset$. Then every set of urelements can be injectively mapped to this set of urelements, so $\ACA$ holds. Then it follows from Lemma \ref{tail->collection} that Collection holds, so we can apply Theorem \ref{homo->RP}.
