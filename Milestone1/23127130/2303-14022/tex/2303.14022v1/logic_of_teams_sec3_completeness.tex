...
\psi \in \Gamma$ then there are labels $b$ and $c$ such that
$\set{b\of \phi,c\of \psi,a = b \lor c} \subseteq
\Gamma$. That $\Gamma$ is \emph{$\iland$-saturated} is defined similarly.
$\Gamma$ is \emph{saturated} if it is both $\ilor$-saturated and
$\iland$-saturated.
\end{defin}
...
\begin{proof}
    ...
    Assume now that $H$ is a homomorphism into $\P B$ where $B$ is finite
    with $n$ elements. Then, since the sequence $H(\phi_0), H(\phi_1), \ldots$
    is increasing and the number of elements of $\P B$ is $2^n$ we have
    $H(\phi_{2^n}) = H(\phi_{2^n+1})$.
