...
First, we employ a 2D object detector that estimates a \gls{RoI} in an input image. 
Then, we propose an asymmetric encoder-decoder architecture\footnote{The encoder part is relatively larger than the decoder part.}, called \method, that predicts 2D-3D correspondences between pixel space and object coordinates. 
The correspondences from the \gls{RoI} are transformed to the original image to directly estimate the relative camera-to-object transformation using the well-established \gls{PnP} algorithm~\cite{Lepetit_undated-le}.
...
Given both maps, one could directly estimate an object's 6D pose by applying a threshold on the confidence $\widehat{\bm{I}}_o$ and forwarding the corresponding 3D points of $\widehat{\bm{I}}_{\tilde{q}}$ with their corresponding 2D locations to the \gls{PnP} algorithm.
...
Satellites and many other objects can exhibit strong symmetries which can drastically impair the performance of pose estimation methods~\cite{Sundermeyer2018-fl}. 
...
The resulting segmentation map is one-hot encoded which leads to the training target $\bm{I}_r \in \mathbb{Z}_{2}^{h\text{x}w\text{x}n}$, as described in~\cite{Hodan2020-dz}. % with one channel per surface region. 
