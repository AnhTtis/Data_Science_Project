...
In addition, we rectified each sample and reduced the resolution from $1920 \times 1200$ to $640 \times 400$ pixels.
...
We trained the model for $1\mathrm{e}6$ steps, using the same optimizer as in~\ref{subsubsec:exp:training} with batch size 32, and a similar training regiment in regards to a warmup phase and learning rate schedule. 
...
\begin{table}%[b]
    \begin{center}
        \caption{Ablation Study on SPEED+. \method~trained with different targets and pose estimates with different $\varepsilon$.}\label{tab:ablation_error} 
...
Hence, we employ an adaptive error threshold $f_{\alpha}(\bm{I}, \widehat{\bm{I}}_e) = \epsilon$ with ${\epsilon \in [1.0, 0.5, 0.3, 0.1, 0.075, 0.05, 0.025]}$ which chooses the best threshold given an input image $\bm{I}$ and error map $\widehat{\bm{I}}_e$ statistics. 
...
At last, we see a difference between \texttt{aug4} and \texttt{aug5} which mainly differ in an additional logic when to apply which augmentation. 
...
In addition, model capacity plays an integral role in the selection and amount of data augmentations.
Hence, while this study might give some clues, future research is required to find definitive factors in data augmentations for orbital perception.
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{images/dragons.png}
    \caption{Illustration of the three models $q_1, q_2$, and $q_3$ that we use in the model quality ablation study. (left) the ground truth 3D CAD model; (middle) the ground truth model is deformed with Blender's \texttt{taper} function; (right) the model is replaced with geometric primitives.}
    \label{fig:tudl_dragons}
    \vspace{-4mm}
\end{figure}

\subsubsection{Multi-hypothesis Testing and Refinement}
Finally, we demonstrate the positive effect of the refinement step, testing multiple hypotheses, and test set augmentations. 
...
\begin{enumerate*}[label=(\roman*)]
    ...
    \item multi-hypothesis refinement of all available hypotheses with predicted, 
    ...
    \item multi-hypothesis refinement with predicted object segmentation and test set augmentations.
\end{enumerate*}
...
More importantly, we showcase how our additions in \method~ help to still make accurate predictions.
For this experiment, we use the TUD-L dataset from the BOP challenge~\cite{hodan_bop_2018} which -- similar to SPEED+ -- contains challenging lighting effects on the real test set. 
\begin{figure}
    \centering
