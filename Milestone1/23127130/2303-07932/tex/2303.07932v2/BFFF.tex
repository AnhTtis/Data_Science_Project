...
\label{sec:BFFF}
In this section, polynomial feedforward strategy for LPV systems as in \eqref{eq:Ftilde} is developed, by posing an alternative parameterization for the LPV motion systems in \eqref{eq:LPVdesc}, that includes dynamic dependency on the scheduling sequence, but simplifies the identification problem significantly.
...
%	\sum_{i=-n_i}^{n_a}a_i(\rho) y^{(i)}=\sum_{j=0}^{n_{\mathrm{b}}}b_j(\rho) \frac{d^{j}}{d t^{j}} w.
%\end{equation}
Similarly to LTI polynomial feedforward in \defRef{def:polFF} and \eqref{eq:FFPolLTI}, feedforward for LPV systems is parameterized by
