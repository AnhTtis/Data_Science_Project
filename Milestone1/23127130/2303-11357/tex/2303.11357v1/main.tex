...
\title{Towards a Comprehensive Catalog of Bona Fide Compact Symmetric Objects}
...
%\title{Definite Catalog of Extragalactic PRCJ1Compact Symmetric Objects}
...
\author{Sandra O'Neill}
\affiliation{Owens Valley Radio Observatory, California Institute of Technology,  Pasadena, CA 91125, USA}
\author{T. J. Pearson}
\affiliation{Owens Valley Radio Observatory, California Institute of Technology,  Pasadena, CA 91125, USA}
\author{Evan Sheldahl}
...
\begin{abstract}
   \acp{cso} are jetted \acp{agn} with overall projected size $<1\,\mathrm{kpc}$. The classification was introduced to distinguish these objects from the majority of known compact jetted-\acp{agn}, where the observed emission is relativistically boosted towards the observer. The original classification criteria for \acp{cso}  were: (i) evidence of emission on both sides of the center of activity, and (ii) overall size $<1\,\mathrm{kpc}$. However some relativistically boosted objects with jet axes close to the line of sight appear symmetric and have been mis-classified as \acp{cso} in the literature, thereby undermining the \ac{cso} classification. We introduce two new \ac{cso} classification criteria  based on (iii) flux density variability, and (iv) the apparent velocity of components moving along the jets. As a first step towards creating a comprehensive catalog of ``bona fide'' \acp{cso}, in this paper we identify 79~bona fide \acp{cso} that meet our expanded \ac{cso} selection criteria.
    This sample of bona fide \acp{cso} can be used for astrophysical studies of \acp{cso} without fear of contamination by objects incorrectly identified as \acp{cso}. We define three complete sub-samples of the 79~\acp{cso}, which are suitable for statistical tests, and show that  the fraction of \acp{cso} in flux density limited samples with  S$_{\rm 5\,GHz}>700\,\mathrm{mJy}$ is between 6.4\% and 8.2\%.
\end{abstract}
...
The discovery that most compact ($<1\,\mathrm{kpc}$) radio sources in cm wavelength surveys have asymmetric, one-sided jet morphology, with a strong flat spectrum core at one end of a steep spectrum jet \citep{1977Natur.269..764W,1978Natur.276..768R,1980IAUS...92..165R}, provided powerful support for the hypothesis that the observed radio emission regions in flat spectrum radio sources are strongly boosted by relativistic beaming \citep{1966Natur.211..468R,1967MNRAS.135..345R}. This quickly became a crucial pillar of our understanding of jetted-active galactic nuclei (jetted-\acp{agn}) \citep{1984RvMP...56..255B,2019ARA&A..57..467B}.
...
Our findings are reported in a set of three papers.  In this, the first paper, hereafter Paper~1 in this series, we describe a large and intensive CSO literature search that we have carried out, and the selection of 79~``bona fide'' \acp{cso}. In the second paper (Kiehlmann et al., submitted), hereafter Paper~2 in this series, we present what we consider compelling evidence that \acp{cso} form a distinct class of jetted-\acp{agn}.  In the third paper (Readhead et al., submitted), hereafter Paper~3 in this series, we introduce a 4-way sub-classification of \acp{cso} and discuss the origin and evolution of \acp{cso}, and we show that CSOs might be fuelled by the capture of single stars by  dormant spinning supermassive black holes in a the nuclei of elliptical galaxies.

\citet{2016MNRAS.459..820T} carried out the most extensive study of \acp{cso} to date. They found  two classes of \acp{cso} that are analogous to the \ac{fr} classes \citep{1974MNRAS.167P..31F} of the large double radio sources: in one class of \ac{cso} the leading (or outermost) edges of the sources are fainter than the central parts of the source, i.e., they have ``edge-dimmed'' morphology,  and this class also has low luminosity, which \citet{2016MNRAS.459..820T} designate as \ac{cso}~1 objects. The other class has edge-brightened morphology, which \citet{2016MNRAS.459..820T} designate as \ac{cso}~2 objects. We find strong confirming evidence of these two distinct classes of \acp{cso}, so we have classified our sample accordingly.  In \cref{fig:morphology} we show three typical examples of the morphologies of these CSO 1 and CSO 2 classes. In Paper~3 we show that \ac{cso}~2s can be subdivided into three sub-classes that follow an evolutionary path.   We base our  classification of CSOs into four sub-classes on morphology alone. 
...
\Cref{sec:csos-angular-sizes-and-spectra} focuses on the  angular sizes and spectra of \acp{cso} and \cref{sec:csos-linear-sizes} on the linear sizes.
We discuss other important aspects of CSOs in \cref{sec:discussion} and conclude this paper in \cref{sec:conclusions}.
...
The \ac{cso} class was originally defined by W94 as: ``(1) two or more components, separated by 10--1000\,pc, either straddling a central core or with other compelling morphological evidence for symmetry such as outer hot spots, and (2) no emission on scales greater than 1000\,pc or only very faint emission.''
%A succinct definition of the CSO class has been suggested by \citet{2016MNRAS.459..820T}: ``We define a CSO as a source less than 1~kpc in projected extent, where two well-defined hotspots (the working surface where the jets push against the surrounding medium) and/or lobes (cavities filled with back flowing material) are observed''.  It is important to emphasize the requirement for  {\it clear} evidence of symmetric jets, as revealed by jets themselves/a clear identification of a central nucleus/or hotspots, and/or lobes. 

Because their jets are closely aligned with the line of sight,   BL~Lac objects and \acp{fsrq}  can contaminate the  \ac{cso} class in one of two ways: (i) in some cases a relativistically boosted nucleus and a relativistically boosted jet component present the appearance of a \ac{cd}, (ii) in the case of some slightly curving jets, different parts of the approaching jet are projected on opposite sides of the nucleus.
...
\begin{enumerate}
    ...
    \item The source should not exhibit  flux-density variability greater than $\sim$10\% of the total flux density over periods of a few years.
    \item No superluminal motion in any jet component in excess of $v_\mathrm{app} = 2.5 \mathrm{c}$, with the proviso that an \ac{agn} should be assumed to pass this criterion until proven otherwise.
\end{enumerate}

Since AGN apparent jet speeds can often take years to measure, we propose the following rule in applying criterion \#4:  apply the first three criteria for acceptance into the \ac{cso} class and use the speed criterion only to reject \acp{agn} from the \ac{cso} class if this criterion is ever violated.

We discuss these criteria further in \crefrange{sec:criteria-size}{sec:criteria-speed}. 
Examples of \acp{cso}~1s and \acp{cso}~2s that pass  our vetting criteria are shown in \cref{fig:morphology}. 
...
\begin{figure*}[!t]
    ...
    \caption{
        Three examples of bona fide \acp{cso} that pass our revised selection criteria. (a) \ac{vlba} map of  J1220+2916 (NGC~4278) at 4.845\,GHz \citep{2007ApJ...658..203H} -- a \ac{cso}~1 object showing edge-dimmed morphology. The  peak flux density is $0.1208\,\mathrm{Jy}\; \mathrm{beam}^{-1}$.
...
\citet{1995AA...302..317F} and R96 divided jetted-\acp{agn} into three classes: \acp{cso} with projected sizes less than 1\,kpc, \acp{mso} with sizes between 1\,kpc and 20\,kpc, and \acp{lso} with sizes greater than 20\,kpc.  These limits were chosen to correspond roughly to the boundary between the  region dominated by the supermassive black hole and that dominated by the galaxy, and the boundary between the region dominated by the galaxy and that  dominated by the extragalactic environment.

Some \acp{cso} show clear evidence of a previous epoch of activity \citep{1990AA...232...19B}, in which a new epoch of activity creating a \ac{cso} has started while vestiges of a previous epoch of activity are still visible. In these \acp{agn} the radio surface brightness either drops off significantly or is completely absent in the region between the ($<1$\,kpc-scale) \ac{cso} and the outer structures.  In such cases the symmetric structure  on the  sub-kpc scale is used to define the size required for inclusion in the \ac{cso} class.
...
The symmetry \ac{cso} criterion includes objects which show {\it any} emission on both sides of the center of activity, or nucleus. The center of activity is identified either by a compact flat spectrum component, or clear symmetry of the lobes. It is not necessary that flux densities of the components straddling the nucleus be comparable. For example, an \ac{agn} in which the lobes straddling the nucleus have a 10:1 flux density ratio, which passes the other CSO criteria, should be classified as a bona fide \ac{cso} according to our definition of a CSO.
...
Variability is used to disqualify an \ac{agn} from the \ac{cso} class if it shows evidence of strong relativistic beaming, for example through large fractional flux density variations on a timescale that is significantly shorter than the light travel time across the emission region. 
This criterion is not easily quantifiable and  we applied different approaches based on the available data.
...
The mean separation speeds of the hot spots are found to  be $\sim 0.36\,{c}$ \citep{2000ApJ...541..112T}, \citep{2002evn..conf..139P}, and Paper~2; but some jet components in \acp{cso} have speeds close to, or exceeding, $c$.
...
We therefore undertook an extensive literature search for sources that had been identified as \acp{cso} or potential \ac{cso} candidates. We augmented our initial list of candidates with other \acp{agn} that we thought might well be \acp{cso}, as well as all \acp{agn} from several complete flux density-limited samples. We then examined all of these sources in the light of our new \ac{cso} selection criteria.

It is not our intention in this paper to present a large complete catalogue of \acp{cso}.  We hope that the catalog of bona fide \acp{cso} we present will be added to and developed by the community into a complete catalog of bona fide \acp{cso}.
...
% complete samples:
We augmented the \ac{cso} candidate source list with all sources from the following statistically complete radio \acp{agn} samples: the \ac{pr} sample \citep{1988ApJ...328..114P}, the \ac{cj1} sample \citep{1995ApJS...98....1P}, 
the 171~sources from the \ac{pw} sample \citep{1981MNRAS.194..331P,1985MNRAS.216..173W} with flux density S$_{\rm 2.7 \,\,GHz} \geq 1.5\,\mathrm{Jy}$,
the \ac{cjf} sample \citep{1996ApJS..107...37T}, and the \ac{vips} sample \citep{2007ApJ...658..203H}. Note that the original PW sample \citep{1981MNRAS.194..331P} consisted of 168 objects, to which three more (DA~240, 0945+73 = 4C~73.08, and NGC~6251)
...
We classified all 3,175~sources as follows:
...
The remaining candidates were examined independently by at least two of our co-authors, at least one of whom had significant experience in \ac{vlbi} and the study of relativistic jets. Most of the rejected sources were obvious asymmetric, one-sided ``core-jet'' sources that could theregore be rejected immediately on the basis of their morphology.
...
A complete list of the 79~bona fide \acp{cso} is given in \cref{tab:csos} in the Appendix.
...
Of the 79~bona fide \acp{cso} we have identified, only 54~bona  fide CSOs have published spectroscopic redshifts. These  are  shown in \cref{fig:redshiftdist}. The peak at low redshift is due to (edge-dimmed) \ac{cso}~1s, which are predominantly nearby, low luminosity \acp{agn}.  The strong cutoff  with increasing redshift is primarily caused by several selection biases that are discussed in \cref{sec:bias-size}.
...
\begin{figure}
    ...
    \caption{Redshift distribution of the 54~bona fide \acp{cso} with spectroscopic redshifts. 25~\acp{cso} with unknown redshifts are omitted.  }
...
\ac{ovro} 40\,m light curves are available for ten of the 79~bona fide \acp{cso}.
Eight of these sources show long-term trends in their 15\,GHz flux densities, with fractional variability from 10\% to 60\% on time scales from 5--12~years.
As slow variability is not in violation of criterion~3 we accept these sources into the class of \acp{cso}. The variability of \acp{cso} is an area of great interest.
...
Spectral index limits can strongly bias the selection of \acp{cso} because  many CSOs have peaked spectra. Thus spectral index limits can exclude some CSOs.  In the CJF and VIPS samples, based on the results from our complete samples, we estimate that $\approx 50\%$ of the CSOs above the flux density limit are missed due to the spectral index limit $\alpha \geq -0.5$.
...
We searched for bona fide \acp{cso} in five statistically complete radio selected samples. The results are listed in \cref{tab:samples}.
The bona fide \ac{cso} detection fractions for the complete samples range
...
Our angular size measurements are listed in \cref{tab:csos} in the Appendix, and the  angular size distribution of the 79~\acp{cso} in our sample is shown in \cref{fig:angsizedist}. The two outlier CSOs at 250 ams and 280 mas are J1602+5243 and JJ1508+3423, respectively.  The distribution shows a dip below $\sim 10\,\mathrm{mas}$,  a peak from 10--50\,mas, and a steady decline in numbers at larger sizes. Both the strong cutoffs at small angular size and at large angular sizes are certain to be  strongly affected by the selection effects described in \cref{sec:bias-size} and hence cannot be be used for statistical tests.
...
The emission is optically thin at frequencies above the peak and optically thick below the peak due either to free-free absorption or synchrotron self-absorption. These must be due to the compactness of \acp{cso}.  In the case of free-free absorption, this would be a result of the  \ac{cso} emission regions being located in the dense central regions of their host galaxies.  In the case of synchrotron self-absorption it would arise because of the high surface brightness of the compact \ac{cso} emission regions \citep{1968ARAA...6..321S}.  In either case, the implied sizes of the  emission regions would range from a few tenths of a milliarcsecond for \acp{cso} with spectral peaks at a few GHz up to $\sim 10$\,mas for \acp{cso} with spectral peaks around 100\,MHz.
...
in \cref{fig:turnoverfreqvsangularsize} we plot the turnover frequency ($\nu_m$) versus largest angular size. The values are listed in \cref{tab:csos} in the Appendix. The $\nu_m$ values were taken from the literature, or estimated by us based on multi-epoch radio spectra compiled on the \ac{rfc} website. The arrows indicate upper limits on $\nu_m$ due to a lack of low-frequency flux density measurements. 

\Cref{fig:turnoverfluxvsangularsize} shows a lower envelope on the spectral peak flux density that increases with angular size.  This is likely affected by the flux density limit bias described in \cref{sec:bias-flux}, as well as the angular size biases described in \cref{sec:bias-size}.

Similarly, \cref{fig:turnoverfreqvsangularsize} shows a clear  decrease in turnover frequency with increase in angular size. This is likely affected by the spectral index bias described in \cref{sec:bias-spectral}
...
We give the projected physical sizes for the 54~\acp{cso} for which we have spectroscopic redshifts  in \cref{tab:csos} in the Appendix, and show the  linear size distribution  in \cref{fig:linsizedist}~(a).
...
\label{sec:discussion-highenergy}
...
Three bona fide \acp{cso} have been detected in $\gamma$-rays with \textit{Fermi}-LAT: PKS~1718$-$649 \citep{2016ApJ...821L..31M}, TXS~0128+554 (J0131+5545) \citep{2020ApJ...899..141L} and NGC~3894 (J1148+5924) \citep{2020AA...635A.185P}. Given the \acp{cso}’ radio morphology  $\gamma$-rays are predicted to originate in the radio lobes
...
\citep{2021arXiv211102611S}.
However, the radio emission from the core of TXS~0128+554 has a flat spectrum, more typical of the radio-loud quasars, so the core could be the main source of the gamma-rays \citep{2020ApJ...899..141L}. %Recent modeling of the PKS~1718$-$649 spectral energy distribution 
...
Through an extensive literature search we compiled a list of 3,175~candidate \acp{cso} and identified a sample of 79~bona fide \acp{cso}. In addition we identified 167~\ac{cso} A-class candidates that we are currently observing with the \ac{vlba}, in order to make a definitive decision on whether or not they are bona fide \acp{cso}.  The follow-up of the 1166~\ac{cso} B-class candidates will take much longer since there are so many of them.  

Our sample of 79~bona fide \acp{cso} includes five complete samples, in which  the fraction of \acp{cso}  ranges from 2.9\% to 9.4\%  The lower fractions occur in the flat-spectrum \ac{cjf} and \ac{vips} samples. These are  missing many steep-spectrum \acp{cso} due to their spectral selection filter.  Based on the complete \ac{pr}, \ac{cj1} and \ac{pw} samples, which have no spectral index  limits, the \ac{cso} fraction is $(6.4\pm 1.5)\% \rightarrow (8.2 \pm 1.7)\%$.  These 79~bona fide \acp{cso}, are suitable for some, but certainly not all statistical studies. Any statistical studies should consider  the selection effects we have described.  

In Appendix~A we present an on-line catalog of \acp{cso} which we anticipate will grow over time as new \acp{cso} are discovered. We hope that this effort, including our new criteria to eliminate highly beamed sources, will help to promote the study of the phenomenology of this distinctive class of jetted-\acp{agn}.

In two further papers, Paper~2 and Paper~3, we present compelling evidence that \acp{cso} comprise a distinct and separate class of jetted-\acp{agn}, and that they go through their whole lifespan, from early-, mid-, to late-life, as \acp{cso}, and we discuss the origin of \acp{cso}.  

\acp{cso} provide a unique window on relativistic jets and the central engines that drive them, that, with the uncontaminated elucidation of their multi-wavelength phenomenology,  are poised to address, in completely new ways, the origins of relativistic jets and the basic properties of their central engines. 
...
\appendix
%------------------------------------------------------------------------------
%\section{CSO\lowercase{cat} and List of Bona Fide CSO\lowercase{s}}

\label{app:csocat}

The \ac{cso} catalog website is available under \url{https://www.cv.nrao.edu/MOJAVE/cso/}.
The website lists all sources discussed in this paper. Each source has a unique ID assigned. The front page lists the ID, coordinates, the classification made in this publication, redshift, and alias names. Each ID links to a source page that shows radio maps and source parameters gathered from the literature.
...
\bibitem[{{Ahumada} {et~al.}(2020){Ahumada}, {Prieto}, {Almeida}, {Anders},
  {Anderson}, {Andrews}, {Anguiano}, {Arcodia}, {Armengaud}, {Aubert}, \&
  et~al.}]{2020ApJS..249....3A}
{Ahumada}, R., {Prieto}, C.~A., {Almeida}, A., {et~al.} 2020, \apjs, 249, 3,
  \dodoi{10.3847/1538-4365/ab929e}
...
{Conway}, J.~E. 1997, in High-Sensitivity Radio Astronomy, 153
...
\bibitem[{{Fanti} {et~al.}(2000){Fanti}, {Pozzi}, {Fanti}, {Baum}, {O'Dea},
  ...
  {Stanghellini}}]{2000A&A...358..499F}
{Fanti}, C., {Pozzi}, F., {Fanti}, R., {et~al.} 2000, \aap, 358, 499.
\newblock \doarXiv{astro-ph/0005035}
...
\bibitem[{{Ginsburg} {et~al.}(2019){Ginsburg}, {Sip{\H o}cz}, {Brasseur},
  ...
  {Woillez}, {The Astroquery collaboration}, \& {a subset of the astropy
...
\bibitem[{{Sobolewska} {et~al.}(2021){Sobolewska}, {Migliori}, {Ostorero},
  {Siemiginowska}, {Stawarz}, {Guainazzi}, \&
  {Hardcastle}}]{2021arXiv211102611S}
{Sobolewska}, M., {Migliori}, G., {Ostorero}, L., {et~al.} 2021, arXiv
  e-prints, arXiv:2111.02611.
\newblock \doarXiv{2111.02611}

\bibitem[{{Sobolewska} {et~al.}(2019){Sobolewska}, {Siemiginowska},
  {Guainazzi}, {Hardcastle}, {Migliori}, {Ostorero}, \&
  {Stawarz}}]{2019ApJ...884..166S}
{Sobolewska}, M., {Siemiginowska}, A., {Guainazzi}, M., {et~al.} 2019, \apj,
  884, 166, \dodoi{10.3847/1538-4357/ab3ec3}
...
\newblock \doarXiv{astro-ph/0007068}
