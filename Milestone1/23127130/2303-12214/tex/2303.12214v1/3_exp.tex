...
We cropped 224 $\times$ 224 sized patches in all our experiments and used ViT-Tiny (ViT-T/16)~\cite{vit} for feature extraction.
...
% We fine-tuned our model and vanilla MIL model for 40 epochs, and for the full fine-tuning experiments, extra 10 epochs were added to ensure full convergence.
% For TCGA-CRC, we also used the AdamW optimizer with a learning rate of $5e-4$; the training epochs were 40 for full fine-tuning and 30 otherwise.
% For Bright, we used the Adam~\cite{adam} optimizer with a learning rate of $1e-4$, a weight decay rate of $5e-2$ and trained for 40 epochs.
% For all full fine-tuning experiments, we used previous mentioned learning rate in for the classier and set the learning rate for the feature model to be 1/10 of the classifier.
