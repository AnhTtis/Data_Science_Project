...
In particular, this hidden valley may be a strongly coupled hidden sector with multiple species of dark quarks charged under a new, confining dark force carried by dark gluons, which form dark hadrons.
...
Assuming collider production of the hidden valley mediator and prompt decays of the unstable dark hadrons, the final state will include ``semivisible'' jets containing both visible SM and invisible DM particles~\cite{Cohen:2015toa}.
...
\caption{Representative Feynman diagrams for leading-order production of a \PZprime boson (left), a single \Pbifun boson (center), and a pair of \Pbifun bosons (right).
}
\label{fig:diagrams}
\end{figure}

In addition to \PZprime bosons and other $s$-channel mediators, another notable possibility is a bifundamental scalar particle \Pbifun, charged under both SM QCD and dark QCD.
...
Therefore, we apply a looser selection $\RT > 0.1$, which provides a level of rejection similar to the realistic analysis.
...
we emphasize here that the EVN was trained only on signal events and has no explicit knowledge of QCD background events.
...
Because the improvement is minor, searches may still prefer to use the simpler \MT variable, but in the event of a discovery, the \PZprime mass could be measured with better resolution using the artificial variable.
