...
G.P. was partially supported by the PANTOMIME project, the Paris Ile-de-France region, and NSF grants DMS-1760448 and DMS-1853650.}}}
\author{
...
\headers{Exact and optimal quadratization of nonlinear non-autonomous ODEs}{A. Bychkov, O. Issan, G. Pogudin and B. Kramer}
...
Quadratic model structure, as a special case, has seen the broad interest due to the ease of working with quadratic models. 
%
...
%
We take a different approach in~\cite{Bychkov2021} where the quadratization problem for a polynomial system is framed as a tree exploration.  The quadratization is obtained by building up the tree structure rather than pruning it as in~\cite{lifeware1}. 
...
In particular, dynamical systems models of engineering systems share common features that the previous version of \Qbee cannot handle:
%
\ben
  %
...
%
Fourth, we present a numerical study that shows how these lifting transformations can create better coordinate systems to learn ROMs from data, a key use case of quadratization.
...
Finally, \cref{sec:conclusions} offers conclusions and an outlook towards future work. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We start in \cref{ss:notation} with defining notation and then review known results about the existence of quadratizations in \cref{ss:QuadPolyODEs}. 
These results also provide an upper bound for the order of quadratization, i.e., the  maximum required number of variables needed for achieving a lifted quadratic model. 
...
and $\otimes^{\prime}$ is the compact Kronecker product, which removes redundant terms (only one term here, $x_1x_2 = x_2x_1$) in the standard Kronecker product, e.g., $[x_1 \ x_2]^\top \otimes^{\prime} [y_1 \ y_2]^\top = [x_1 y_1 \  x_1 y_2 \  x_2 y_2]^\top$.
%
...
\begin{definition}[Quadratization]\label{def:quadr}
  ...
  \end{equation}
  for every $\bx$ solving~\eqref{eq:sys_main}.
  The dimension $\ell$ of the vector $\bw$ is called the \emph{order of quadratization}.
  A quadratization of the smallest possible order is called an \emph{optimal quadratization}.
\end{definition}

\begin{example}\label{ex:quadr}
    Consider a two-dimensional system
    \begin{equation}\label{eq:quadr_ex}
    \dot{x}_1 = (x_1 + 1)^3 + x_2, \qquad \dot{x}_2 = x_1 + x_2
    \end{equation}
    We claim that a new variable $w(\bx) = (x_1 + 1)^2$ is a (polynomial) quadratization.
    Indeed, for the original states $x_1$ and $x_2$ we have
    \[
    \dot{x}_1 = (x_1 + 1)w + x_2, \qquad \dot{x}_2 = x_1 + x_2,
    \]
    so we take $\bq_1(\bx, w) = [(x_1 + 1)w + x_2, \; x_1 + x_2]^\top$.
    Furthermore,
    \[
    \dot{w} = 2(x_1 + 1)\dot{x}_1 = 2(x_1 + 1)^4 + 2(x_1 + 1)x_2 = 2w^2 + 2(x_1 + 1)x_2,
    \]
    so $q_2(\bx, w) = 2w^2 + 2(x_1 + 1)x_2$.
    Since this quadratization is of order one, it is optimal.
\end{example}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Monomial quadratization} 
This section considers the case where the new variables are restricted to be of monomial form. Those are useful in applications in synthetic biology~\cite{lifeware1} and are comparatively easier to find than polynomial terms because the search space is discrete and the problem of finding an optimal quadratization can be phrased as a combinatorial optimization problem.
...
\begin{example}[Continuation of~\Cref{ex:quadr}]
    The quadratization $w(\bx) = (x_1 + 1)^2$ introduced in~\Cref{ex:quadr} is not monomial.
    We next show that $w(\bx) = x_1^2$ is a monomial quadratization for the same system~\eqref{eq:quadr_ex}.
    For $x_1$ and $x_2$, we have
    \[
      \dot{x}_1 = x_1^3 + 3x_1^2 + 3x_1 + 1 + x_2 = x_1w + 3 w + 3x_1 + 1 + x_2,\quad \dot{x}_2 = x_1 + x_2,
    \]
    so $\bq_1(\bx, w) = [x_1 w + 3 w + 3x_1 + 1 + x_2,\; x_1 + x_2]^\top$.
    Furthermore,
    \[
    \dot{w} = 2(x_1 + 1)\dot{x_1} = 2x_1^4 + 8x_1^3 + 12 x_1^2 + 8 x_1 + 2 + (x_1 + 1)x_2 = 2w^2 + 8 x_1w + 12 w + 8 x_1 + 2 + (x_1 + 1)x_2, 
    \]
    so $q_2(\bx, w) = 2w^2 + 8 x_1w + 12 w + 8 x_1 + 2 + (x_1 + 1)x_2$. Therefore, $w(\bx) = x_1^2$ is an optimal monomial quadratization of~\eqref{eq:quadr_ex}.
    Here, both the monomial quadratization and the polynomial quadratization (cf.~\Cref{ex:arb_poly,ex:arb_poly2}) are optimal, yet the dynamics of the auxiliary state $w$ are a bit more involved in the monomial case.
\end{example}
...
Despite the appeal of monomial quadratizations in certain communities, it is natural to ask whether one may be able to obtain a substantially more concise quadratization if \emph{arbitrary polynomials} are allowed for the new variables (as in \cref{def:quadr}). 
Although, in the examples above, allowing arbitrary polynomials did not result in a lower-order quadratization, the following example from~\cite[Section 4]{Alauddin} shows that this is not always the case, and the difference between optimal monomial and arbitrary quadratizations may be significant. 
...
\begin{example}[Lower-order quadratization using arbitrary polynomials]\label{ex:arb_poly}
  ...
  %
  In contrast, it is always possible to quadratize the ODE by adding one new variable $w(x) := (x + 1)^{k - 1}$ so that
  \begin{align*}
      \dot{x} & = (x + 1)w,\\
...
\begin{example}[Lower-order quadratization using arbitrary polynomials, continued]\label{ex:arb_poly2}
...
Since we aim at the QB form~\eqref{eq:qb_general}, our definitions are different from the ones in~\cite{carravetta2020solution} where the input variables are ignored when counting the degree.
...
\begin{definition}[Quadratization of non-autonomous polynomial ODEs]\label{def:quadr_input}
 ...
 is said to be a \emph{quadratization} of~\eqref{eq:sys_main_input} if there exist vectors $\bq_1(\bx, \bw, \bu, \dot{\bu})$ and $\bq_2(\bx, \bw, \bu, \dot{\bu})$ of dimensions $\nn$ and $\ell$, respectively, with the entries being polynomials of total degree at most two such that 
  ...
  \end{equation}
  for every $\bx$ solving~\eqref{eq:sys_main_input}.
...
\begin{example}\label{ex:input}
    Consider a non-autonomous scalar ODE $\dot{x} = x + x^2u$.
    We claim that $w(x, u) = xu$ is a quadratization.
    Indeed, the corresponding quadratic system is
    \[
      \dot{x} = x + xw, \quad \dot{w} = xu' + (x + x^2u)u = xu' + xu + w^2,
    \]
    so $q_1(x, w, u, \dot{u}) = x + xw$ and $q_2(x, w, u, \dot{u}) = xu' + xu + w^2$.
    This quadratization is optimal and monomial.
\end{example}
...
\begin{remark}
  ...
  For example, the quadratization from~\Cref{ex:input} is not input-free (and in fact the model does not admit an input-free quadratization, see also~\Cref{ex:infinite}).
\end{remark}

\begin{example}
    Consider a two-dimensional system describing the Duffing oscillator:
    \[
    \dot{x}_1 = x_2, \quad \dot{x}_2 = -\alpha x_1 - \delta x_2 - \beta x_1^3 + u.
    \]
    We claim that $w(\bx) = x_1^2$ is an input-free (monomial and optimal) quadratization.
    Indeed, the corresponding quadratic system is
    \[
    \dot{x}_1 = x_2, \quad \dot{x}_2 = -\alpha x_1 - \delta x_2 - \beta x_1w + u, \quad \dot{w} = 2x_1x_2,
    \]
    so $\bq_1(\bx, w, u) = [x_2, -\alpha x_1 - \delta x_2 - \beta x_1w + u]^\top$ and $q_2(\bx, w, u) = 2x_1x_2$.
    %
    The quadratic system above does not involve any derivatives of $u$ and, thus, does not add any additional constraints on the input function.
    %
\end{example}

One nice feature of input-free quadratizations is that they produce the quadratic-bilinear structure as the following lemma shows.
...
\begin{comment}
\begin{example} \label{ex:PDE_quadratization}
Consider a special case of a dimensionless scalar PDE used in traffic flow modeling~\cite[\S 12.6]{haberman2019applied}, where 
\[
  \frac{\partial \rho}{\partial t}(t, \xi) = \rho(t, \xi) + \rho^2(t, \xi) \frac{\partial \rho(t, \xi)}{\partial \xi}, \qquad \rho(t, 0) = 0, \quad \frac{\partial \rho}{\partial \xi}(t, 1) = 0\; ,
\]
and $\rho(t,\xi)$ is a scaled traffic density 
%
%
and where $t>0$ and the spatial variable $\xi \in [0, 1]$. A square-integrable initial condition is  given as $\rho(0, \xi) = \varphi(\xi)$.
In a spatially uniform discretization of this PDE, let $x_{i}^{[\nx]}(t) = \rho(t, i/\nx)$ 
%
for $1\leq i \leq \nx$ be the finite-dimensional vector of nodal values of the (to be approximated) solution. This defines the $N$-dimensional state vector $\bx^{[\nx]}(t) = [x_{1}^{[\nx]}(t), \ldots, x_{\nx}^{[\nx]}(t)]^\top$. We approximate the spatial derivative by a forward Euler finite difference as
\[
  \frac{\partial \rho}{\partial \xi}(t, i/\nx) \approx \frac{\rho(t, i/\nx) - \rho(t, (i - 1)/\nx)}{\nx},
\]
which then yields the $\nx$-dimensional system of ODEs
\begin{equation}\label{eq:ex_discr}
  \dot{\bx}^{[\nx]} = \bx^{[\nx]} + (\bx^{[\nx]})^2 \odot (\bD\bx^{[\nx]}),
...
\begin{equation}\label{eq:ex_Dmatrix}
  \bD = \frac{1}{\nx} \left[
    ...
    \right]\in\mathbb{R}^{\nx \times \nx},
\end{equation}
where the last row is due to the Neumann boundary condition.
%
%
Our goal is to quadratize~\eqref{eq:ex_discr} for arbitrary size of the discretization, $\nx$. 
%
Consider the vectors $\bw^{[\nx]}_1 = (\bx^{[\nx]})^2$ and $\bw^{[\nx]}_2 = \bx^{[\nx]} \odot (\bS \bx^{[\nx]})$, where $\bS$ is the lower shift matrix (having ones on the subdiagonal and zeros elsewhere) which shifts the vector entries as $\bS\bx^{[\nx]} = [0, \; x_1^{[\nx]}, \;\ldots, \; x_{N - 1}^{[\nx]}]^\top$\OI{Gleb, we found a mistake in the derivative matrix $\mathbf{D}$ and changed its last row to match the boundary conditions. Can you make sure that the lifting variables remain the same in the algorithm? I suspect $\mathbf{w}_{2}$ might change. }.
%
We claim that the nonzero entries of $\bw_1^{[\nx]}$ and $\bw_2^{[\nx]}$ are a quadratization of~\eqref{eq:ex_discr} for any $\nx$.
We fix $\nx$ and omit the superscript $[\nx]$ for brevity in the rest of the argument.
The original system can be now written as:
\begin{equation}\label{eq:pde_discr}
  \dot{\bx} = \bx + \bw_1 \odot (\bD \bx).
\end{equation}
...
\begin{align}\label{eq:pde_ex_quadratized}
\begin{split}
  \dot{\bw}_1 &= 2\bx(\bx + \bx^2 \odot (\bD\bx)) = 2\bw_1 + \frac{2}{N}\bw_1(\bw_1 - \bw_2),\\
  \dot{\bw}_2 &= (\bx + \bx^2 \odot \frac{1}{N}(\bx - \bS\bx))(\bS \bx) + \bx (\bS \bx + (\bS\bx)^2\odot \frac{1}{N}(\bS \bx - \bS^2 \bx)) = \\
  &= 2\bw_2 + \frac{1}{N}(\bw_1\odot \bw_2 - \bw_2^2 + \bw_2\odot(\bS \bw_1) - \bw_2\odot(\bS \bw_2)) 
\end{split}
\end{align}
Therefore, $\bw^{[\nx]}_1$ and $\bw^{[\nx]}_2$ is a quadratization of~\eqref{eq:ex_discr} for every $\nx$, which we refer to as a \emph{dimension-agnostic} quadratization (see~\cref{def:da} for a formal definition). 
%
...
\begin{example}[Vectors $\bw_1$ and $\bw_2$ for~\eqref{eq:ex_discr}]\label{ex:da_quadr_basic}
    ...
    \]
    Furthermore, it is possible to show that $\cM(\bw_1, \bw_2; \bD)$ is a quadratization of~\eqref{eq:ex_discr} for any complex-valued matrix $\bD$ (see~\Cref{ex:use_agnostic}).
\end{example}
...
\begin{theorem}\label{thm:existencEgnostic}
  Every family of linearly coupled ODEs admits a monomial dimension-agnostic quadratization; this can be chosen such that $\deg_{\tilde{\bx}} \bw_{2}(\bx, \tilde{\bx}) = 1$.
...
It turns out that it is sufficient to consider the case $n = 4$ with particular matrices $\bD_i$. The intuition behind this is that $n = 4$ is sufficient to exhibit all important combinatorial types of coupling, and, thus, a general matrix will always look locally as one of the fragments of the $4\times 4$ matrix from the proposition. The proof of the next proposition formalizes this.
...
%
A Jupyter notebook with the examples from~\crefrange{sec:Qbee}{sec:heliospheric} is available online\footnote{Jupyter notebook: \url{https://github.com/AndreyBychkov/QBee/blob/master/examples/Examples_BOPK2023.ipynb}}
...
Furthermore, to keep the computation manageable, \Qbee implements additional pruning rules that allow the algorithm to decide that the branch can be safely skipped even if $|\mathcal{S}| < \ell_0$~\cite[Section~5]{Bychkov2021}.
%
...
\begin{comment}
We introduce the new `state' variables: $\bu^{(0)}=\bu$ and $\bu^{(1)} = \dot{\bu}$, as the zeroth and first derivatives of $\bu$, respectively. This allows us to formally write an \textit{autonomous} ODE system 
\begin{equation}\label{eq:input_alg_transformed}
\begin{split}
    \dot{\bx} & = \bp(\bx, \bu^{(0)}) \\
    \dot{\bu}^{(0)} &= \bu^{(1)} \\
    \dot{\bu}^{(1)} &= \mathbf{0}.
...
\end{equation}
 % 
The last dummy equation $\dot{\bu}^{(1)} = \mathbf{0}$ is merely added for syntactic compliance, as \Cref{alg:qbee_outline} requires a full polynomial ODE as an input---we do not claim the equivalence of \eqref{eq:input_alg_transformed} to~\eqref{eq:input_algo_start}.

Since by \cref{def:quadr_input} the quadratizations of the non-autonomous system~\eqref{eq:input_algo_start} involve only $\bx$ and $\bu$ but not derivatives of $\bu$, there is a bijection between the quadratizations of~\eqref{eq:input_algo_start} and  the quadratizations of the autonomous system~\eqref{eq:input_alg_transformed} involving $\bx$ and $\bu^{(0)}$ but not $\bu^{(1)}$.
%
Thus, the quadratizations of~\eqref{eq:input_alg_transformed} can be found by~\cref{alg:qbee_outline} if we restrict the extensions considered at~\ref{step:generation} to ones not containing $\bu^{(1)}$.
\cref{thm:existence_inputs} guarantees that even under this restriction a quadratization exists and, thus, \Qbee is finding it.

To compute an input-free quadratization (for cases when $\bu(t)$ is not differentiable), the approach above is modified as follows.
First, we set $\ell_0 = \infty$.
Second, we further restrict~\ref{step:generation} of~\cref{alg:qbee_outline} by allowing only the extensions not involving either $\bu^{(0)}$ or $\bu^{(1)}$.
%
We note that, in general, the search for input-free quadratizations is not guaranteed to terminate, as there may not be input-free quadratizations. 
We refer to~\cref{sec:input-free} for the criterion (\cref{prop:inp_general}) and special cases (\cref{lem:input_bilinear,lem:triangular}) where we expect the algorithm to terminate.
%
...
\begin{example}[Use of \Qbee for dimension agnostic quadratization]\label{ex:use_agnostic}
...
This work focuses on the task of quadratizing polynomial ODEs. 
However, many models in engineering and science are non-polynomial. The problem of optimal quadratization of non-polynomial systems remains open, but practical polynomialization algorithms can nevertheless be devised. 
%
We design and implement a polynomialization algorithm that can be used as a preprocessing step before quadratization with \cref{alg:qbee_outline} and~\cref{alg:qbee_input}.
%
...
\begin{itemize}
    ...
    \item For the polynomialization algorithm (\Cref{sec:polynomialization}), the algorithm for computing dimension-agnostic quadratizations (\Cref{alg:qbee_da}), as well as \Cref{alg:qbee_outline,alg:qbee_input} when allowing for Laurent polynomials in the model form, we do not guarantee optimality.
    %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This section applies the \Qbee algorithm to quadratize a wide range of nonlinear systems of ODEs.  
Here, we revisit examples from the literature and demonstrate that we can improve the order of quadratization (i.e., we need fewer variables to do so). \Cref{sec:tubularReactor} considers two different tubular reactor models, both with variable dimension and external inputs
In \cref{sec:gems} we consider a two-step rocket combustion process which is nonpolynomial. 
...
%
We consider two cases of different complexity.
Both cases are non-autonomous and of variable dimension (depending on the size of discretization).
...
\begin{comment}
We consider the polynomial ODE in $2N$ dimensions:
\begin{equation}
\label{eq:main_poly_reaction}
\begin{split}
  \bm{\dot{\psi}}  &= \mathbf{A}_\psi \bm{\psi} + \mathbf{b}_\psi - \mathcal{D}\bm{\psi} \odot (\mathbf{c}_0 + \mathbf{c}_1 \odot\bm{\theta} + \mathbf{c}_2 \odot\bm{\theta}^2 + \mathbf{c}_3 \odot\bm{\theta}^3), \\
...
\end{equation}
%
where $u=u(t)$ is a scalar input function, $\mathbf{A}_{\psi}$ and $\mathbf{A}_{\theta}$ are $N\times N$ constant matrices, $\mathbf{b}, \mathbf{b}_\psi,$ and $\mathbf{b}_\theta$ are $N$-dimensional constant vectors. 
The Damk{\"o}hler number $\mathcal{D}$ and $\mathcal{B}$ are the known constants. 
%
This ODE system has a polynomial reaction model, i.e., the chemical reaction term is $f(\bm{\psi}, \bm{\theta}; \gamma) = \psi (c_0 + c_1\bm{\theta} + c_2\bm{\theta}^2 + c_3\bm{\theta}^3)$ following~\cite[Eqs. (17)-(18)]{KW2019_balanced_truncation_lifted_QB}.  
%
In~\cite{KW2019_balanced_truncation_lifted_QB} this model was quadratized with $5N$ additional variables, so the lifted system dimension was $7N$.

We use \Qbee to search for a lower-dimensional quadratization of~\eqref{eq:main_poly_reaction} with new variables coming in vectors of the form $\bm{\psi}^a \odot \bm{\theta}^b$ with $a, b \in \mathbb{Z}_{\geqslant 0}$.
To achieve this, we treat~\eqref{eq:main_poly_reaction} as a two-dimensional ODE model (since $N=2$ and set $n=1$) in variables $\bm{\psi}$ and $\bm{\theta}$. We define $\mathbf{A}_{\psi}\bm{\psi}, \mathbf{A}_\theta \bm{\theta}$, and $u$ as inputs and look for input-free quadratization using the new functionality for non-autonomous systems in \Qbee as described in~\Cref{sec:Qbee}. With this algorithmic trick, we avoid having $\mathbf{A}_{\psi}\bm{\psi}$ and $\mathbf{A}_\theta \bm{\theta}$ in the new variables.
%
The output of \Qbee is the following $4n$ new variables which quadratize~\eqref{eq:main_poly_reaction} for any $n$:
\begin{equation}\label{eq:quadr_poly_reaction}
    \bm{\bw}_1 := \bm{\theta}^2, \quad \bm{\bw}_2 := \bm{\theta}^3, \quad \bm{\bw}_3 := \bm{\psi}\odot \bm{\theta}, \quad \bm{\bw}_4 := \bm{\psi}\odot \bm{\theta}^2.
\end{equation}
%
%
The corresponding quadratization produced by \Qbee is
...
\end{equation}
%
The condition that all the new variables come in vectors of the form $\bm{\psi}^a \odot \bm{\theta}^b$ may sound restrictive. However, we used \Qbee to analyze the system~\eqref{eq:main_poly_reaction} for $n = 1, 2, 3$, and observed that the optimal monomial quadratization is always the one from~\eqref{eq:quadr_poly_reaction} with terms $\bm{\psi}^a \odot \bm{\theta}^b$.\OI{Gleb, why do we vary $n$ here?}
Thus, we expect that \eqref{eq:quadr_poly_reaction} yields an optimal monomial quadratization for arbitrary $n$.
%
In sum, \Qbee produced a lower-dimensional quadratization ($4n$ extra variables) than has been previously reported in the literature ($5n$ extra variables).
...
With \Qbee, we improve on these results. First, we manually introduce only one variable $\bw_1 := e^{\gamma - \gamma / \btheta}$ and obtain a $3\nx$-dimensional system with Laurent polynomials in the right-hand side
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{comment}
\subsubsection{Polynomialization}
Explicit computation with \Qbee shows that the system can be polynomialized by introducing the four new variables
%
\begin{equation}\label{eq:poly}
    w_1 := x_1^{-0.8}, \quad w_2 := x_2^{-0.7}, \quad w_3 = \ffrac{1}{u}, \quad w_4 = \exp \Big( \ffrac{E}{R u} \Big).
\end{equation}
%
Those variables are all well defined as the species and temperature inputs are strictly positive. We write the equation for $\dot x_1$ as
\[
  \dot x_1 = -A x_1^{(1-0.8)}\ x_2^{(2-0.7)}\ w_4 = -A x_1 x_2^2 w_1 w_2 w_4,
\]
%
and add the dynamical evolution for the new variables~\eqref{eq:poly} to the existing system:

\begin{equation}
    \label{eq:polynomialized_gems}
    \begin{split}
  \dot x_1 &= -A x_1x_2^2 w_{1} w_{2} w_{4} \\
  \dot x_2 &= 2 \dot x_1 \\
  \dot x_3 &= - \dot x_1 \\
  \dot x_4 &= -2 \dot x_1 \\
  \dot w_{1} &= 0.8 A x_2^2 w_{1}^2 w_{2} w_{4} \\
  \dot w_{2} &= 1.4 A x_1 x_2 w_{1} w_{2}^2 w_{4} \\
  \dot w_{3} &= -\dot{u} w_{3}^2 \\
  \dot w_{4} &= \frac{E}{R} \dot{u} w_{3}^2 w_{4}.
  \end{split}
\end{equation}
This is a polynomialization with four extra variables (versus six new variables in~\cite[App.~A]{SKHW2020_learning_ROMs_combustor}); we therefore save two extra variables compared to existing results reported in the literature.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Quadratization} \label{sec:Species_Quadratization}

We now use \Qbee to find a quadratization by introducing as few additional variables as possible. The quadratization found by \Qbee is 
%
\begin{equation}
    \label{eq:quadr_c1c2}
    w_5 = \dot{u} w_3, \quad w_6 = \dot{u} w_3^2, \quad w_7 = x_2^2 w_1 w_2 w_4, \quad w_8 = x_1 x_2 w_1 w_2 w_4, \quad w_9 = w_3^2
\end{equation}
with the resulting quadratic system  
\begin{equation}
\label{eq:quadratized}
\begin{split}
    \dot x_1 &= -A x_2 w_8 \\
    \dot x_2 &= 2 \dot x_1 \\
    \dot x_3 &= -\dot x_1 \\
    \dot x_4 &= -2 \dot x_1 \\
    \dot w_1 &= 0.8 A w_1 w_7 \\
    \dot w_2 &= 1.4 A w_2 w_8 \\
    \dot w_3 &= -\dot{u} w_9 \\
    \dot w_4 &= \frac{E}{R} w_4 w_6 \\
    \dot w_5 &= \ddot{u} w_3 -\dot{u}(t) w_6\\
    \dot w_6 &= \ddot{u} w_9 - 2 w_5 w_6 \\
    \dot w_7 &= 0.8 A w_7^2 - 2.6 A w_{7} w_{8} + \frac{E}{R} w_{6} w_{7} \\
    \dot w_8 &= -0.2 A w_{7} w_{8} - 0.6 A w_{8}^2 + \frac{E}{R} w_{6} w_{8} \\
    \dot w_9 &= -2 w_5 w_9
\end{split}
\end{equation}

In total, we need to add nine new variables to the original system~\eqref{eq:main_gems} to obtain a quadratic ODE system.  The authors in \cite{SKHW2020_learning_ROMs_combustor} did not find a quadratized form for this system by hand due to the complexity of finding such quadratizations. This illustrates a significant advantage of the automated quadratization implemented in \Qbee. 


We note that it is not possible to find a quadratization without the appearance of $\ddot{u}(t)$ in the resulting system.
This can be deduced using~\Cref{prop:inp_general} but we also give the following direct proof.
Indeed, the equation for $\dot w_3$ in~\eqref{eq:polynomialized_gems} can not be quadratized without $\dot{u}(t)$ as a part of a new variable. To see this, consider the replacement $z_1 = w_3^2 \Rightarrow \dot z_1 = 2w_3\dot w_3 = -2 \dot{u} w_3^3$, and now we need to introduce another variable, say $z_2 = w_3^3 \Rightarrow \dot z_2 = 3w_3^2 \dot w_3 = -3 \dot{u} w_3^4$. This recursion continues infinitely, so we cannot find a quadratization without $\ddot{u}$.
\end{comment}
...
%
We set to answer this question by comparing the numerical results of a quadratic ROM learned from trajectories of the original states, $\bv$ from Eq.~\eqref{shifted-dynamics-ode-hux}, to a quadratic ROM learned from the lifted state variables, $\bw_{1, 1}, \bw_{2, 1}, \bw_3$ in Eq.~\eqref{eq:HUX_Final_QB}.
...
\begin{figure}
\centering
    \includegraphics[width=1\linewidth]{lifted_quadratic_rank_6.png}
    \includegraphics[width=1\linewidth]{quadratic_rank_6.png}
...
\begin{figure}
\centering
    \includegraphics[width=1\linewidth]{70_training_quadratic_results_rel_error_rank_6.png}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We thank the anonymous referee for the valuable suggestion to add more examples to the manuscript.
