...
The resolution proposed in \cite{Bender:2022eze} is to make use of the asymptotic behaviours of 
...
but it seems a nontrivial problem to deduce the large $n$ behaviours of the Green's functions at $D>0$. 

{\it Can we resolve the indeterminacy of the Dyson-Schwinger equations in a different approach?}
...
\<{\d S[\phi]}/{\d\phi(x)}\>=\<J(x)\>\,.
\ee
The infinite set of DS equations can be derived by taking the functional derivatives with respective to the classical source field $J$ and then setting $J$ to zero.

\section{Null state condition}
In this paper, we propose a new approach based on null states. 
We refer to \cite{Li:2022prn} for the use of null state conditions in the bootstrap context, 
which shares a similar spirit with the present work. 
The null states should be orthogonal to arbitrary states. 
In practice, we consider approximate null states 
and study their orthogonality to a subset of test states 
\be
...
They are labelled by $K$ and $L$, denoting the numbers of basis operators involved. 
For a given $K$, there is a proper $L$ that leads to a determined system. 
...
such as the $\mathcal {PT}$ symmetric solutions of non-Hermitian theories \cite{r5}. 
...
k=-1,0,1,\dots.
\ee
The general solution of the Green's function reads
\be
...
Therefore, the infinite system of the DS equations is underdetermined.  
Indeterminacy is an intrinsic feature of the DS equations, 
...
For $K=3$, the solutions of $G_2$ are $\{-1,\pm 1/\sqrt 3\}$. 
The latter solutions are the same as the ones from setting the connected part of 4-point function to zero, 
whose error is -14.6\% compared to the exact value. 
As shown in \cite{Bender:2022eze}, the results do not converge to the exact values by setting higher-point function to zero. 
On the contrary, our higher $K$ results from the null state condition \eqref{null-condition} converge to the exact values rapidly. 
...
For $K=7$, the solutions are \{$\pm$0.538\dots, $\pm$0.675\dots, $\pm$0.678\dots, $\pm$1.098\dots\}. 
...
In general, the solutions of $G_2$ correspond to the roots of some high-degree polynomial equations. 
...
also reminds us of the critical behaviours of the Yang-Lee edge singularities, 
described by the $i\phi^3$ theory. 

The second example is the non-Hermitian cubic theory with $\mathcal L(\phi)=\frac i 3\phi^3$. 
In the general solution of the DS equations, $G_1$ is the only free parameter. 
Its exact value is
\be
...
Then we introduce the null state operators as in the quartic case \eqref{n-t-operators} with $K=L$. 
The solutions of the null state condition again exhibit the root accumulation phenomena near the exact values. 
For $K=2$, the solutions of $G_1$ are $-i 2^{-1/3}e^{\frac{i2k\pi}{3}}$.  
The same results can be obtained 
by setting the connected part of $G_3$ to zero. 
...
The remaining nine solutions form three groups of nearby roots related by $e^{\frac{i2k\pi}{3}}$. 
One group consists of purely imaginary solutions around $-073i$, which is supposed to be $\mathcal {PT}$ symmetric.  
Then the median gives an estimate $-0.729011134i$, 
...
The null state condition with $L=K+1$ leads to a determined systems. 
...
then we consider the more complicated cases with unequal time. 
...
In the equal-time limit, one needs to be careful about the ordering of operators. 
...
In addition, the time-translational invariance of $2$-point functions implies
\be
...
we can express the equal-time limit of the Green's functions in terms of
\be
...
In the end, we reconstruct the physical observables from these solutions. 
...
The odd case $F_{2m+1}$ do not appear because they are not independent. For instance, we have
\be
F_1=\frac 1 2\,,\quad
...
\ee
Therefore, all the 1-point functions of composite operators are encoded in the two-point Green's function $G_2(\t_1,\t_2)$ in the equal-time limit. 
It turns out we should set $L=3K-1$ in the null state condition to obtain an determined system. 
Let us examine the simplest approximation $K=1$, which gives
\be
...
whose real root at 0.2991\dots is already close to the exact value $\<\phi^2\>=0.3058136507\dots$. 
...
which is close to the exact mass gap $E_\text{gap}=E_1-E_0=1.628230531\dots$ 
in the Hamiltonian formulation. 
For higher $K$, the number of solutions grows rapidly. 
If we impose the null state condition on the unequal time 2-point function, 
then the solution reads
\be
\<\O|\phi(\t_1)\phi(\t_2)|\O\>_{\t_1\geq\t_2}= \sum_{m=1}^K c_m\, e^{-\D E_m(\t_1-\t_2)}\,,
\label{sol-2pt}
\ee
...
\ee
They can be interpreted as the energy difference of an intermediate state and the reference state $|\O\>$. 
If $|\O\>$ is the ground state $|0\>$ with the lowest energy and the energy spectrum is real, 
then all the roots of \eqref{null-polynomial} should be positive for a gapped system. 
For each $K$ examined, it turns out that there is only one solution of the null state condition 
with all positive roots! 
In this way, we are able to a make definite selection from 
a large number of solutions of the null state condition. 
...
For comparison, the exact energy differences between the lowest parity-even state and the parity-odd state are
$\D E$=\{1.628230531\dots, 5.882226\dots, 10.9525\dots, 16.624\dots, 22.8\dots, 29.4\dots\}. 
...
One can further determine $c_i$ in \eqref{sol-2pt} using the solutions of $F_n$, 
which are also consistent with the exact squared matrix elements $|\<n|\phi|0\>|^2$. 
...
$\<2|\phi|0\>=0.021994761\dots$. 

It is straightforward to translate the results to the case of Lorentzian spacetime. 
...
\caption{The real part of the real-time 2-point function of the quartic theory for $K=1,2,3$ (dashed, blue/orange/green) and the exact function (solid, black). } 
\label{figure-2pt-Re}
\end{center}
\end{figure}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=8.5cm]{Abs-2pt.pdf}
...
the average of a small set of nearby roots. 
...
$\<\mathcal O^{(K)}_\text{null}\>=\<\text{const.}+a_0\phi\>=0$. 
The simplest approximation $K=1, L=2$ then gives 
\be
...
The estimate of the mass gap $E_\text{gap}=(9/2)^{1/5}=1.35\dots$ is the same as a truncation result in \cite{Bender:1999ek},  
which is close to the exact value $1.4764808747\dots$. 
At higher $K$, we also find 5 groups of roots related by 5-fold symmetry. 
The case of $K=6$ is presented in Fig. \ref{figure-root-cubic}. 
For $K=6$, we obtain $\<\phi\>=-0.590072522\dots i$ by assuming the real part of the solutions of the null polynomials are positive. 
...
by extracting the median of the group of roots around $\<\phi\>=-0.6$. 
...
\caption{The solutions of the 1D non-Hermitian $i\phi^3$ theory.  
The red point indicates the exact value at $G_1=-0.590072533\dots i$.  
We find 90 roots of distance less than 0.05 to this exact value, 
while 39 of them are of distance less than 0.005. 
The root accumulation points exhibit 5-fold symmetry. 
  } 
...
We would like to extend our results to higher-point functions and the case of $D>1$.

At $D=1$, we also propose another method for extracting the best solutions. 
The null polynomial associated with the null state solution encodes the spectral information 
of the low-lying intermediate states, 
so a bounded-from-below spectrum should have only roots with positive real part. 
...
the work of Belavin-Polyakov-Zamolodchikov 
on 2D minimal model conformal field theory (CFT) \cite{Belavin:1984vu}. 
...
It would be interesting to revisit the case of higher-dimensional CFTs 
\footnote{We refer to \cite{Poland:2018epd} for a comprehensive review on the conformal bootstrap. }, 
especially the truncation approach initiated by Gliozzi \cite{Gliozzi:2013ysa}. 
The complex solutions in the CFT context may also have important implications on 
gauge theory, statistical and condensed matter physics \cite{Gorbenko:2018ncu}. 

For more systematic investigations and improved numerical stability, it may be useful to introduce 
the tools of computational algebraic geometry or other advanced techniques to handle high degree polynomial equations. 
