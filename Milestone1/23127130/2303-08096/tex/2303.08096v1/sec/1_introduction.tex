...
Neural radiance fields (NeRFs)~\cite{Mildenhall20eccv_nerf} represent radiance with a neural field that reproduces the geometric structure and appearance of a scene, allowing the use of backpropagation to reconstruct a set of input images.
...
Our method simultaneously trains a CNN encoder that maps images to camera pose in $\sothree$, and a neural radiance field of the scene.
...
To cope with the presence of local minima in the low-dimensional latent space $\sothree$, we introduce a novel \textit{\lossname} that replicates the encoder pose output and only backpropagates the view with the lowest photometric error.
We formalize this replication with an equivalence relation $\relation$ defined in the space of camera parameters, and show this enables the encoder to operate in the quotient set of the camera space for the relation $\relation$, enhancing convergence.
