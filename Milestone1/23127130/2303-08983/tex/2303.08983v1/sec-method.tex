...
approach is related to prior work such as Fast Knowledge Distillation 
...
(\cref{sec:good_teacher})  then find generalizable reinforcements on ImageNet 
(\cref{sec:aug_difficulty}). To demonstrate the generality of our strategy and 
findings, we further reinforce CIFAR-100, Flowers-102, and Food-101 
...
ImageNet-22K and fine-tuned on ImageNet-1K~\citep{liu2022convnet}, DeiT-3 
pretrained on the ImageNet-21K and fine-tuned on ImageNet-1K, IG-ResNext 
...
pretrained with and without ImageNet-22K and fine-tuned on 
ImageNet-1K~\citep{liu2022swin}.
...
\subsection{ImageNet+: What is the best combination of 
...
\begin{table*}[t!]
    ...
    \resizebox{1.8\columnwidth}{!}{
        \begin{tabular}{lcccc}
            ...
            \\
            \midrule[1.25pt]
            \textbf{ImageNet+ variant} & All & All & +RA/RE, +M*+R* & +Mixing, M*+R*\\
    ...
    \vspace{-2mm}
    \caption{\textbf{Additional storage in ImageNet+ variants}.  Total 
    additional storage for ImageNet+ (\RRCRARE{}) is 61GB.}
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[b]{2\columnwidth}
        ...
        \begin{tabular}{cccc}
            ...
            & \hspace{3mm} \scalebox{0.85}{\textbf{ImageNet+ Variants}}  \\[-1mm]
            %
    ...
    \end{subfigure}
    \vspace{-2mm}
    \caption{\textbf{Improvements across architectures with ImageNet+ variants 
    compared with ImageNet.}.  Top-1 accuracy of different models on the 
    ImageNet validation set consistently improves when trained with the 
    proposed datasets as compared to the standard ImageNet training set 
    (Epochs=150).  Our proposed dataset variant with \RRCRARE{}, 
    \textbf{ImageNet+}, provides balanced improvements of 1-4\% across 
    architectures. Further improvements with longer training (300-1000 epochs) 
    in \cref{tab:long_train_effect}.
    %
    %
    %
...
In this section, we introduce ImageNet+, a reinforcement of ImageNet.  We 
create ImageNet+ using the IG-ResNext ensemble (\cref{sec:good_teacher}).  
Following \citep{shen2021fast}, we store top $10$ sparse probabilities for 
$400$ augmentations per training sample in the ImageNet dataset 
...
ImageNet+ variant in \cref{tab:imagenet_plus_specs}.
%
...
specific to each model on the ImageNet. We perform \textbf{no hyperparameter 
tuning specific to ImageNet+} and achieve improvements with the same setup as 
ImageNet for all models.
...
trained using ImageNet and ImageNet+ datasets.  
...
we use \RRCRARE{} as our reinforced dataset and call it \textbf{ImageNet+}. In 
...
epochs,
2) Additional augmentations on top of ImageNet+ are not useful.
...
\subsection{CIFAR-100+, Flowers-102+, Food-101+: How to reinforce other 
datasets?}
\label{sec:other_datasets}

We reinforced ImageNet due to its popularity and effectiveness as a pretraining 
dataset for other tasks (e.g., object detection).  Our findings on ImageNet are 
...
%
We build a teacher for each dataset by fine-tuning ImageNet+ pretrained 
...
4xResNet-152.
Next we generate reinforcements using similar augmentations to ImageNet+, that 
...
models on ImageNet/ImageNet+ and fine-tune on each dataset for varying epochs 
up to 1000, 10000, and 1000 (for CIFAR-100, Flowers-102, and Food-101, 
...
\begin{table}[t!]
    ...
    \resizebox{0.95\columnwidth}{!}{
        \begin{tabular}{lcc|cc|cc}
            ...
            & \multicolumn{2}{c}{\textbf{Food-101}}  \\
            \cmidrule[1.25pt]{2-7}
             & \textbf{Orig.} & \textbf{+}
             & \textbf{Orig.} & \textbf{+}
             & \textbf{Orig.} & \textbf{+}\\
    ...
    On Food-101, 86.1\% is improved to 89.5\%, demonstrating composition of 
    reinforced datasets.}
