...
\section{Full results of training on ImageNet and ImageNet+, compared with 
...
ImageNet+ compared with ImageNet and Knowledge Distillation (KD). We choose 
\RRCRARE{} that provides a balanced trade-off across architectures and training 
durations, and call it ImageNet+.
...
library~\citep{mehta2022cvnets}. We use the exact same ImageNet+ variant and 
...
\begin{itemize}[leftmargin=*]
    ...
    \item Higher resolution training: EfficientNets, ViT-Base, Swin-Base. We 
        observe that ImageNet+ reinforcements are resolution independent and 
...
\begin{table*}[thb!]
    ...
    distillation and different ImageNet/ImageNet+ datasets. Subscripts show the 
    improvement on top of the ImageNet accuracy. We highlight the best accuracy 
...
\begin{table*}[thb!]
    ...
    in CVNets library.} Subscripts show the improvement on top of the ImageNet 
...
\subsection{Aggregated improvements of ImageNet+ across models}
To better demonstrate the scale of accuracy improvements, we plot the results 
of training on ImageNet+ (\RRCRARE{}) from \cref{tab:imagenet_plus_full} in 
...
\begin{figure*}[thb!]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Maximum of 4 ImageNet+ variants}
        \label{fig:figs_delta_max}
    \end{subfigure}
    \vspace{-4mm}
    \centering
    \caption{\textbf{ImageNet+ training improves validation accuracy} compared 
        with ImageNet training (Epochs=$300$).
    To train models using the ImageNet+ dataset, we use the same publicly-available ImageNet training recipes with \emph{no hyperparameter tuning} on ImageNet+.
    We use the same hyperparameters 
    tuned for ImageNet with \emph{no hyperparameter tuning} on ImageNet+.  
    ImageNet+ provides a balanced tradeoff with more improvements for 
...
\newpage
\clearpage
\subsection{Super ensembles on ImageNet}\label{sec:super_ensembles}
...
super ensembles on CIFAR-100 and in \cref{fig:imagenet_superensemble} we 
present results on ImageNet. On CIFAR-100 we create super ensembles by training 
...
We also consider super ensembles for ImageNet using dataset reinforcement.  
Knowledge distillation with super ensembles of larger than 10 members on 
ImageNet becomes challenging and resource demanding.
...
training with the reinforced ImageNet dataset using the super ensemble.
...
\caption{Distillation on CIFAR-100 with super diverse ensembles.}
...
\caption{Accuracy of super diverse ensembles on CIFAR-100.}
...
\end{center}
    \vspace{-0.5cm}
    \caption{ImageNet accuracy of super ensembles as the size of the ensemble 
...
\end{center}
    \vspace{-0.5cm}
    \caption{ImageNet super ensemble distillation accuracy for ResNet-50 
...
ImageNet}\label{sec:reinforce_imagenet_sup}
...
\centering
    ...
    ImageNet validation accuracy of three representative architectures trained 
    on reinforcements of ImageNet. We use ConvNext-Base-IN22FT1K as the teacher 
    and train for 150 epochs.  The x-axis is the number of augmentations stored 
    per original sample in the ImageNet training set.
...
\end{center}
    ...
    accuracy for models trained with ImageNet+ as we vary the library size.  
...
alternatives provide a better tradeoff compared with \RRCRARE{}. Therefore, we use \RRCRARE{} in our paper and call it ImageNet+.
...
\end{center}
    ...
    ImageNet. We use ConvNext-Base-IN22FT1K as the teacher and train for 150 
    epochs. The x-axis is the number of augmentations stored per original 
    sample in the ImageNet training set. Augmentations used are Fixed 
...
\end{center}
    \vspace{-0.5cm}
    \caption{\textbf{Optimal augmentation sample selection.} ImageNet+ accuracy 
...
We train models for 100, 400, 1000 epochs on CIFAR-100, Food-101 and 1000, 
...
\cref{fig:val_calib_error}. We see observe that validation ECE of ImageNet+ 
pretrained models is lower than ImageNet pretrained models.
...
&ImageNet               &  300    &      0.1503 &    0.0727 &    0.0776 &        0.0934 &      0.2509 &      0.1575 \\
...
&ImageNet               &  300    &      0.1938 &    0.1513 &    0.0425 &        0.1239 &      0.2122 &      0.0883 \\
...
&ImageNet               &  300    &      0.1084 &    0.0663 &    0.0421 &        0.0734 &      0.1910 &      0.1176 \\
...
For CIFAR-100, Flowers-102, and Food-101, the reinforcement took 90, 40, and 
120 minutes respectively. With pretrained teachers and extrapolating our 
ImageNet+ observations, we can reinforce any new dataset and the cost is 
...
%
%
We provide storage cost analysis for ImageNet+ in 
...
augmentations for a pair. The proposed ImageNet+ variant, \RRCRARE{}, does not 
have that doubling cost. Also note that the storage can be further reduced 
using compression methods. For example, ImageNet+ \RRCRARE{} with the 
...
The storage cost for CIFAR-100+, Flowers-102+, and Food-101+ uses the same set 
...
ImageNet, the reinforcement overhead is much smaller relative to the original 
    ...
    \resizebox{0.45\textwidth}{!}{
        \begin{tabular}{llccc}
            ...
            & & \textbf{150} & \textbf{300} & \textbf{1000} \\
             %
             \midrule[1.25pt]
             \multirow{2}{*}{MobileNetV3-Large} & ImageNet & $1.00\times$ & $1.00\times$ & $1.00\times$ \\
             & ImageNet+ (Ours) & $1.13\times$ & $1.12\times$ & $1.12\times$ \\
             \midrule
             \multirow{2}{*}{ResNet-50} & ImageNet & $1.00\times$ & $1.00\times$ & $1.00\times$ \\
             & ImageNet+ (Ours) & $1.04 \times$ & $1.02 \times$ & $0.97\times$ \\
             \midrule
             \multirow{2}{*}{SwinTransformer-Tiny} & ImageNet & $1.00\times$ & $1.00\times$ & $1.00\times$ \\
              & ImageNet+ (Ours) & $0.99\times$ & $0.99\times$ & $0.99\times$ \\
            \bottomrule[1.5pt]
        \end{tabular}
    }
    \vspace{-2mm}
    \caption{\textbf{Training time for different models using ImageNet+ is 
    similar to ImageNet dataset}. Full results in 
...
hyperparameters on ImageNet and ImageNet+ with the exception of the data 
augmentations that are removed from the training on ImageNet+ based on our 
...
In \cref{tab:train_hparams_cvnets} we provide hyperparameters for training with CVNets. For higher resolution and variable resolution training, we use the same metadata in ImageNet+ to create a random crop then resize it to the target resolution instead of the base resolution of 224.
In \cref{tab:det_seg_hparams} we provide hyperparameters for training Detection/Segmentation models.
In \cref{tab:transfer_hparams} we provide hyperparameters for transfer learning on CIFAR-100/Flowers-102/Food-101 datasets.
    ...
    \resizebox{\columnwidth}{!}{
        \begin{tabular}{ll cccccc cccc}
           ...
           \midrule[1.25pt]
            ...
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & \xmark & \xmark & \xmark & \xmark \\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & \xmark & \xmark & \xmark & \xmark \\
            & KD        & SGD+Mom=0.9 & 1024 & 0.8 & 3 & $4.0\mathrm{e}{-6}$ & \xmark & \xmark & \xmark & \xmark & \xmark \\
            \midrule
            \multirow{3}{*}{\bfseries MobileNetV2}
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.4 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & \xmark & \xmark & \xmark & \xmark \\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.4 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & \xmark & \xmark & \xmark & \xmark \\
            ...
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $1.0\mathrm{e}{-4}$ & \cmark & \cmark & \cmark (0.25) & \cmark (0.2) & \cmark (1.0) \\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $1.0\mathrm{e}{-4}$ & \cmark & \xmark & \xmark & \xmark & \xmark \\
            & KD        & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $1.0\mathrm{e}{-5}$ & \xmark & \cmark & \cmark (0.25) & \cmark (1.0) & \cmark (1.0) \\
            \midrule
            \multirow{3}{*}{\bfseries EfficientNet-B2}
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $4.0\mathrm{e}{-5}$ & \cmark & \cmark & \cmark (0.25) & \cmark (0.2) & \cmark (1.0) \\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $4.0\mathrm{e}{-5}$ & \cmark & \xmark & \xmark & \xmark & \xmark \\
            ...
            & ImageNet  & AdamW (0.9, 0.999) & 1024 & 0.001 & 5 & 0.05 & \cmark & \cmark & \cmark (0.25) & \cmark (0.2) & \cmark (1.0) \\
            & ImageNet+ & AdamW (0.9, 0.999) & 1024 & 0.001 & 5 & 0.05 & \cmark & \xmark & \xmark & \xmark & \xmark \\
    ...
    \resizebox{\columnwidth}{!}{
        \begin{tabular}{ll cccccccc c}
           ...
           \midrule[1.25pt]
            ...
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & 224 & \xmark & LS+RRC+HF\\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & 224 & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries MobileNetV2}
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.4 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & 224 & \xmark & LS+RRC+HF\\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.4 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & 224 & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries MobileNetV3}
            & ImageNet  & SGD+Mom=0.9 & 2048 & 0.4 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & 224 & \xmark & LS+RRC+HF\\
            & ImageNet+ & SGD+Mom=0.9 & 2048 & 0.4 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & 224 & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries MobileNetViT}
            & ImageNet  & AdamW (0.9, 0.999) & 1024 & 0.002 & 20 & $0.01$ & \cmark & VBS(160, 320, 256) & \xmark & LS+RRC+HF\\
            & ImageNet+ & AdamW (0.9, 0.999) & 1024 & 0.002 & 20 & $0.01$ & \cmark & VBS(160, 320, 256) & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries ResNet}
            & ImageNet  & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $1.0\mathrm{e}{-4}$ & \cmark & 224 & \xmark & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & SGD+Mom=0.9 & 1024 & 0.4 & 5 & $1.0\mathrm{e}{-4}$ & \cmark & 224 & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries EfficientNet-B2}
            & ImageNet  & SGD+Mom=0.9 & 2048 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & VBS(144, 432, 288) & \xmark & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & SGD+Mom=0.9 & 2048 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & VBS(144, 432, 288) & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries EfficientNet-B3}
            & ImageNet  & SGD+Mom=0.9 & 2048 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & VBS(150, 450, 300) & \xmark & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & SGD+Mom=0.9 & 2048 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & VBS(150, 450, 300) & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries EfficientNet-B4}
            & ImageNet  & SGD+Mom=0.9 & 2048 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & VBS(190, 570, 380) & \xmark & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & SGD+Mom=0.9 & 2048 & 0.8 & 3 & $4.0\mathrm{e}{-5}$ & \cmark & VBS(190, 570, 380) & \xmark & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries ViT-Tiny}
            & ImageNet  & AdamW (0.9, 0.999) & 2048 & 0.002 & 10 & 0.05 & \cmark & 224 & \cmark (1.0) & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & AdamW (0.9, 0.999) & 2048 & 0.002 & 10 & 0.05 & \cmark & 224 & \cmark (1.0) & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries ViT-Small/Base}
            & ImageNet  & AdamW (0.9, 0.999) & 2048 & 0.002 & 10 & 0.2 & \cmark & 224 & \cmark (1.0) & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & AdamW (0.9, 0.999) & 2048 & 0.002 & 10 & 0.2 & \cmark & 224 & \cmark (1.0) & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries ViT-Base $\uparrow$384}
            & ImageNet  & AdamW (0.9, 0.999) & 2048 & 0.002 & 20 & 0.2 & \cmark & VBS(192, 576, 384) & \cmark (1.0) & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & AdamW (0.9, 0.999) & 2048 & 0.002 & 20 & 0.2 & \cmark & VBS(192, 576, 384) & \cmark (1.0) & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries SwinTransformer}
            & ImageNet  & AdamW (0.9, 0.999) & 1024 & 0.001 & 20 & 0.05 & \cmark & 224 & \cmark (5.0) & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & AdamW (0.9, 0.999) & 1024 & 0.001 & 20 & 0.05 & \cmark & 224 & \cmark (5.0) & \xmark\\
            \midrule
            \multirow{2}{*}{\bfseries SwinTransformer-Base $\uparrow$384}
            & ImageNet  & AdamW (0.9, 0.999) & 1024 & 0.001 & 20 & 0.05 & \cmark & VBS(192, 576, 384) & \cmark (5.0) & LS+RRC+HF+RA+RE+MU+CM\\
            & ImageNet+ & AdamW (0.9, 0.999) & 1024 & 0.001 & 20 & 0.05 & \cmark & VBS(192, 576, 384) & \cmark (5.0) & \xmark\\
           \bottomrule[1.5pt]
        \end{tabular}
    }
...
\begin{table}[h!]
    ...
    \caption{\textbf{Hyperparameters used for CIFAR-100/Flowers-102/Food-101.} We use cosine learning rate schedule to zero. We resize the inputs for all datasets to 224 including CIFAR-100 where we pad the input by 16. We also use label smoothing.}
