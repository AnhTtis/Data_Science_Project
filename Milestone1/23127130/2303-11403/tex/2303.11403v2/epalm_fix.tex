...
\begin{table}[h]
 ...
 \setlength\tabcolsep{4pt}
	\resizebox{0.95\linewidth}{!}{%
	\begin{tabular}	{lc@{\hspace{15pt}}c@{\hspace{10pt}}c@{\hspace{10pt}}c@{\hspace{10pt}}c@{\hspace{10pt}}c@{\hspace{10pt}}c}
		\toprule	 	
...
\begin{table}[h]
 ...
 \setlength\tabcolsep{1pt}
	...
	\begin{tabular}	{l@{\hspace{25pt}}c@{\hspace{25pt}}c@{\hspace{25pt}}c}
		\toprule	 	

Dataset (Metric)	& SoTA (ZS)  &  eP-ALM (FT)  & SoTA (FT)  \\
  \midrule
 
AudioCaps (CIDEr) &  -- &  \underline{63.6} & \textbf{66.7} (Liu et al. \cite{liu2022leveraging})  \\ 
MSRVTT-QA (Acc)   & 17.4 (Flamingo80B \cite{alayrac2022flamingo}) & \underline{36.7} &  \textbf{44.1} (OmniVL \cite{wang2022omnivl}) \\
MSR-VTT (CIDEr)  &  -- & \underline{50.7} & \textbf{60} (MV-GPT \cite{seo2022endmvgpt})  \\
COCO (CIDEr)   & 84.3 (Flamingo80B \cite{alayrac2022flamingo}) & \underline{107.0} & \textbf{145.3} (OFA \cite{wang2022unifyingofa}) \\
VQAv2 (Acc)  & \underline{56.3} (Flamingo80B \cite{alayrac2022flamingo}) & 53.3 & \textbf{84.3} (PaLI \cite{chen2022pali})   \\
GQA (Acc)   & 29.3 (FewVLM \cite{jin-etal-2022-good_fewvlm}) & \underline{42.7} & \textbf{60.8} (VL-T5 \cite{cho2021unifyingvlt5}) \\
	\bottomrule
 
	\end{tabular}
	}
 \caption{Comparison of eP-ALM with text generation-based SoTA that train significant number of parameters, including methods with large-scale pretraining. Best and next best scores are bolded and underlined respectively. FT: Finetuning. ZS: Zero-shot.}
 \label{tab:sotas}
\end{table}
...
\paragraph{Comparison between different text generation methods.} We find the approach sensitive to the text decoding strategy. In Tab. \ref{tab:decoding}, we compare with different text decoding methods; multinomial/random sampling \cite{fan2018hierarchicalmultinomial} and greedy decoding with beam-search (1 to 5 beams). Greedy decoding significantly outperform multinomial sampling, and increasing the number of beams leads to additional improvements, to the detriment of increasing inference cost.
...
\begin{table}[h]
 ...
 \setlength\tabcolsep{4pt}
	...
	\begin{tabular}	{lccccccccc}
		\toprule	 	
	   \multirow{2}{*}{Decoding Method}  & \multirow{2}{*}{\# of beams} & VQA v2  & GQA & \multicolumn{2}{c}{COCO} & MSVD-QA  & MSRVTT-QA & \multicolumn{2}{c@{\hspace{10pt}}}{MSRVTT}\\

	 &  & Test & Test & B@4 &  CIDEr & Test & Test & B@4 &  CIDEr \\
 ...
 \bottomrule
	...
	{
	\footnotesize	
		Comparison with different text generation mechanisms using eP-ALM-L.
		}
	\label{tab:decoding}
