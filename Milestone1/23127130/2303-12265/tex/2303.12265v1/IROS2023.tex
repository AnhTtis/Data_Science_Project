...
\usepackage[unicode=true,
 ...
 {hyperref}
\hypersetup{pdftitle={Autonomous Robotic Drilling System for Mice Cranial Window Creation: An Evaluation with an Egg Model},
 pdfauthor={Murilo M. Marinho},
...
Consider the setup shown in Fig.~ \ref{fig:robot_system}, using
one of the robotic arms of our robot platform for scientific exploration
\cite{marques2022design}. Let $R1$ be the robotic arm (CVR038,
Densowave, Japan) with joint values $\quat q_{\text{R1}}\in\mathbb{R}^{6}$
holding the micro drill (MD1200, Braintree Scientific, USA) . The
micro drill is used to drill the egg fixed by a clamping device. Images
are obtained through a 4K microscope system (STC-HD853HDMI, Omron-Sentech,
Japan) from above.
...
\textbullet{} The robots are velocity-controlled by the signals $\quat u_{\text{R1}}\triangleq\dot{\quat q}_{\text{R1}}\in\mathbb{R}^{6}$
(Premise I).
...
in the trajectory points $\quat p_{i}\left(t\right)\triangleq\begin{bmatrix}p_{x,i} & p_{y,i} & p_{z,i}\left(t\right)\end{bmatrix}^{T}\in\mathbb{R}^{3}$,
...
Fig.~ \eqref{fig:curve}).  Instead, we propose the use of constrained
...
Consider any two contiguous trajectory points $\quat p_{i}\triangleq\begin{bmatrix}p_{x,i} & p_{y,i} & p_{z,i}\end{bmatrix}^{T}$
and $\quat p_{i+1}\triangleq\begin{bmatrix}p_{x,i+1} & p_{y,i+1} & p_{z,i+1}\end{bmatrix}^{T}$,
...
\begin{equation}
\quat s_{i}\left(u\right)=\begin{bmatrix}s_{x,i}\left(u\right) & s_{y,i}\left(u\right) & s_{z,i}\left(u\right)\end{bmatrix}^{T}=\quat a_{i}^{T}+\quat b_{i}^{T}u+\quat c_{i}^{T}u^{2}+\quat d_{i}^{T}u^{3}.\label{eq:cons1}
\end{equation}
...
\[
\begin{array}{c}
\quat a_{i}^{T}=\begin{bmatrix}a_{x,i} & a_{y,i} & a_{z,i}\end{bmatrix}^{T}\\
\quat b_{i}^{T}=\begin{bmatrix}b_{x,i} & b_{y,i} & b_{z,i}\end{bmatrix}^{T}\\
\quat c_{i}^{T}=\begin{bmatrix}c_{x,i} & c_{y,i} & c_{z,i}\end{bmatrix}^{T}\\
\quat d_{i}^{T}=\begin{bmatrix}d_{x,i} & d_{y,i} & d_{z,i}\end{bmatrix}^{T}
\end{array}.
\]
...
\begin{equation}
\quat s_{i}^{\prime}\left(u\right)=\begin{bmatrix}s_{x,i}^{\prime} & s_{y,i}^{\prime} & s_{z,i}^{\prime}\left(u\right)\end{bmatrix}^{T}=\quat b_{i}^{T}+2\quat c_{i}^{T}u+3\quat d_{i}^{T}u^{2}.\label{eq:cons2}
\end{equation}

\begin{equation}
\quat s_{i}^{\prime\prime}\left(u\right)=\begin{bmatrix}s_{x,i}^{\prime\prime} & s_{y,i}^{\prime\prime} & s_{z,i}^{\prime\prime}\left(u\right)\end{bmatrix}^{T}=2\quat c_{i}^{T}+6\quat d_{i}^{T}u.\label{eq:cons3}
\end{equation}
...
\begin{equation}
\begin{array}{c}
\quat s_{i}\left(0\right)=\begin{bmatrix}p_{x,i} & p_{y,i} & p_{z,i}\end{bmatrix}^{T}\\
\quat s_{i}\left(1\right)=\begin{bmatrix}p_{x,i+1} & p_{y,i+1} & p_{z,i+1}\end{bmatrix}^{T}
...
\begin{equation}
\begin{array}{c}
\quat s_{i}^{\prime}\left(0\right)=\begin{bmatrix}p_{x,i}^{\prime} & p_{y,i}^{\prime} & p_{z,i}^{\prime}\end{bmatrix}^{T}\\
\quat s_{i}^{\prime}\left(1\right)=\begin{bmatrix}p_{x,i+1}^{\prime} & p_{y,i+1}^{\prime} & p_{z,i+1}^{\prime}\end{bmatrix}^{T}
...
\[
\quat a_{i}^{T}=\left[\begin{array}{c}
p_{x,i}\\
p_{y,i}\\
p_{z,i}
\end{array}\right],\,\quat b_{i}^{T}=\left[\begin{array}{c}
p_{x,i}^{\prime}\\
p_{y,i}^{\prime}\\
p_{z,i}^{\prime}
\end{array}\right],
...
\[
\quat c_{i}^{T}=\left[\begin{array}{c}
...
\[
\quat d_{i}^{T}=\left[\begin{array}{c}
...
to 100 Considering the complexity of this approach, we propose a more
treatable annotation strategy. Given a image of eggshell drilling,
we first annotate the image with 6 classes: drill, $0\,\%$ drilled
(background), $25\,\mathrm{\%}$ penetrated, $50\,\%$ penetrated,
$75\,\%$ penetrated, and $100\,\%$ penetrated. Second, we separate
the annotated image into completion level map channel, including 5
classes with the grayscale values of 0, 25, 50, 75 and 100 that reflect
the completion level in five quanta, and drill channel, including
the class of background with a grayscale value of 0 and the class
of drill with the grayscale value of 255. Lastly, taking advantage
of the expected continuity in driller regions, a Gaussian filter is
applied to smooth the completion level map.
...
box detection; (b) The completion level map; (c) The heapmap of completion
...
The network is able to output a bounding box for drilling area (Fig.~
\ref{fig:result}-(a)) and the completion level map for the area (Fig.~
\ref{fig:result}-(b)). We can visualize the completion level map
using heatmap (Fig.~ \ref{fig:result}-(c)) where color is red if
the completion level is higher and the color is blue if the completion
level is blue. By adding and normalizing the according pixel value,
we can easily calculate a value between 0 and 1 for each angle along
a circle and generate a drilling progress bar (Fig.~ \ref{fig:result}-(d))
based on the completion level map to show the drilling progress. By
setting $n$ discrete points evenly along the circle on the progress
bar, the value $c_{i}$ of point $i$, which is the completion level
at point $i$, is used to calculate the lowering velocity of drill
by \eqref{eq:vp}.
...
The system is set up as shown in Fig.~ \ref{fig:robot_system}. An
egg is fixed stably using a clamping device. The micro drill is held
by a robot arm and a 4K camera is set vertically above the drill and
egg to observe the drilling procedure. An air compressor with a pipe
is applied to blow away the shell dust emitted from drilling the surface
of the eggshell. Communication with the robot was enabled by the SmartArmStack\footnote{https://github.com/SmartArmStack}.
The quaternion algebra and robot kinematics were implemented using
DQ Robotics \cite{adorno2021dqrobotics} with Python3. We planned
the drilling trajectory with the method mentioned in Sec.~ \ref{subsec:Trajectory-planning-for}.
Ideally, when the completion level of one point along the circular
path is predicted as just thoroughly penetrated, the z position of
the point remains constant and the system will stop autonomously after
most of the points along the circular path are predicted as thoroughly
penetrated and the drilling is considered completed.
...
\begin{table}
\centering{}\caption{\label{tab:parameter-choice}The selected values during the experiment
of hyperparameters}
\begin{tabular}{cccc}
\hline 
...
failures accounted for 2 of the 4 total cases of failure. Even though
the $80\,\%$ success rate shows the robustness of the autonomous
robotic drilling system toward the eggshell thickness topology, the
adaptive capability for the initial egg pose should be improved in
the future.
...
Future works include the application of the system to drilling mouse
skulls and increasing the drilling speed and accuracy.
