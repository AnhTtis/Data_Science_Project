...
\title[Spin-induced offset stream self-crossing shocks in TDEs]{\textit{Spin-induced offset stream self-crossing shocks in tidal disruption events}}

% The list of authors, and the short list which is used in the headers.
% If you need two or more lines of authors, add an extra line using \newauthor
\author[T. Jankovič, C. Bonnerot, A. Gomboc]{T. Jankovič$^{1}${\thanks{Contact e-mail: \href{mailto:taj.jankovic@ung.si}{taj.jankovic@ung.si}}, C. Bonnerot$^{2,\, 3}$, A. Gomboc$^{1}$
}%
%, C. Bonnerot$^{2}$%
...
\date{Last updated 2021 November 14; in original form 2021 November 14}
...
\noindent In this section we present the properties of the outflow from the self-crossing shock produced by colliding streams. The collision is simulated in a local co-moving frame  according to the procedure described in Section \ref{sec2}, considering different offsets between the streams.\footnote{Movies made from the simulations are available online at \href{https://github.com/tajjankovic/Spin-induced-offset-stream-self-crossing-shocks-in-TDEs/tree/main/Movies}{https://github.com/tajjankovic/Spin-induced-offset-stream-self-crossing-shocks-in-TDEs/tree/main/Movies}.} We determine various properties of the outflow in the co-moving and in the black hole's reference frames.
...
As long as its evolution is dominated by the black hole's tidal force, the stream is confined between two orbital planes, inclined by an angle $\alpha$, that intersect along a line, which we refer to as $L_\mathrm{H}$. During its first approach, the in-falling stream is in hydrostatic equilibrium until the tidal force takes over, which we assume happens at the semi-minor axis of the most bound debris $b_\mathrm{orb}=\ell_\star \left( 2\Delta\epsilon  \right )^{-1/2}$,  where the stream width is $H_0$. This implies (see \citealt{Bonnerot_2022}) that $L_\mathrm{H}$ is along the major axis of the initial orbit, and that $\alpha=H_0/b_\mathrm{orb}$. We calculate the width of colliding streams $H_\mathrm{int}$ from the projected distance between the intersection point and $L_\mathrm{H}$, which yields
...
%\begin{equation} \label{eq:dz_hint}
%    \frac{\Delta z}{H_\mathrm{int}} = \xi \pi a \sin i \left ( \frac{R_\mathrm{g}}{R_\mathrm{p}} \right )^{3/2}  \frac{\  \ell_\star  }{\sqrt{\Delta \epsilon }H_0}  \approx \xi a \sin i\left ( \frac{M_\mathrm{bh}}{10^6\,\mathrm{M_\odot}}\right)^{3/2}\left ( \frac{H_\mathrm{0}}{5\, \mathrm{R_\odot}}\right)^{-1},
%\end{equation}
...
We conclude that the relationship between $\Delta z /H_\mathrm{int}$, $\gamma$ and $a$ is highly complicated. When $a\approx 0$, we find that stream collisions typically have low offsets, meaning that most collisions are strong. However, when $a\gtrsim 0.1$, we discover that there can be both strong and grazing collisions for all values of $a$ depending on the exact value of $\gamma$, as given by the projection of the black hole spin vector on the in-falling stream's orbital plane. The trends with other parameters $i$, $H_0$, and $M_\mathrm{bh}$ are however more tractable as shown with Equation (\ref{eq:dz_hint}).
...
%\centering
%        		\includegraphics[width=\linewidth]{images/resolution_test_dphi=360.0_philim=270.0}
%\captionsetup{font=small}
%\captionsetup[figure]{labelfont=bf,labelsep=period}
%	\captionof{figure}{ The outflow rate $\dot{M}$ for particle masses $m_\mathrm{p}=10^{-4},\, 10^{-3},\, 10^{-2}$ and $\Delta z/H=0.5$ through a surface at $50H$ for azimuthal angle $\phi = 270^\circ \pm 0.5^\circ$.}
%	\label{fig:res_test}
%\end{minipage}%
...
In order to quantify this effect we evaluate the trapping radius $R_\mathrm{tr}$, defined as the distance at which photons decouple from the gas to start diffusing away. $R_\mathrm{tr}$ is calculated by equating the dynamical timescale of the gas $t_\mathrm{dyn}\approx R_\mathrm{}/v(R_\mathrm{})$ to the diffusion timescale of photons $t_\mathrm{diff}\approx \tau R_\mathrm{}/c$, where $R$ is the radial distance from the point of self-crossing, $v(R_\mathrm{})$ is the velocity of the gas and $\tau$ is the optical depth, integrated from $R$ to infinity. Therefore, $R_\mathrm{tr}$ can be expressed from

\begin{equation} \label{eq:Rtr}
c/v(R_\mathrm{tr}) = \int_{R_\mathrm{tr}}^{\infty} \rho \kappa_\mathrm{s} \mathrm{d} R,
\end{equation}
where $\kappa_\mathrm{s}=0.34\, \mathrm{cm^2 g^{-1}}$ is the electron scattering opacity and $\rho$ is the gas density. The density is taken from the simulation with $\Delta z/H=1.2$, and its value in physical units is obtained by assuming $H=10\,\mathrm{R_\odot}$, $v_\mathrm{r}=0.01c$ and $\dot{M}=0.7\,\mathrm{M_\odot\,yr^{-1}}$. $R_\mathrm{tr}$ is evaluated for different values of $\theta$ and $\phi$, by computing the integral of Equation (\ref{eq:Rtr}) from outward in until its value reaches the local ratio of the speed of light to the gas velocity. In Figure \ref{fig:Rtr} we show the density of gas contained in a slice in the $zy$ plane at $x=0$ for $\Delta z/H=1.2$, where we overplot the trapping surface with a green line. Dashed blue lines indicate values of $\tau$ obtained by integration from $R=15H$ (dashed grey circle) to infinity along several directions. We see that the trapping surface follows the gas density, resulting in lower trapping radii along the equatorial plane of the colliding streams. This indicates that the radiation produced at the self-crossing shock may emerge closer to the intersection point for an offset collision than an aligned one, potentially leading to a more luminous signal as the radiation suffers fewer adiabatic losses. A more careful determination of the emerging radiation requires radiation-hydrodynamics simulations, which we defer to future research.


\begin{figure}
	\centering

					\includegraphics[width=\linewidth]{images/dz1.2_Rtr_new.pdf}

	\caption{A density slice of the outflow (in the local reference frame) in the $zy$ plane at $x=0$ at time $t=150$ for $\Delta z=1.2H$. Length units are expressed in units of the stream widths $H=10\, R_\odot$. Green solid line represents the location of the trapping surface defined in Equation (\ref{eq:Rtr}). We show values of the optical depth $\tau$, integrated from $R=15H$ (grey dashed circle) to infinity, along several directions marked by blue dashed lines.}
	\label{fig:Rtr}
\end{figure}


In the later stages of a TDE, when the accretion disc has formed, X-ray radiation from the disc can be absorbed and then reprocessed to the optical band by the outflow \citep{Roth_2016, Lu_2019}. For more offset collisions, we expect the solid angle covered by the outflow to decrease due to a deviation from sphericity (see Figure \ref{fig:3dtraj}), such that X-ray photons may only promptly escape along specific viewing angles while being efficiently reprocessed along others. This effect could explain the variety of optical to X-ray luminosity ratios found observationally (e.g. \citealt{van_Velzen_2021}).
...
\item The radiation produced at the self-crossing shock may emerge closer to the intersection point for an offset collision than an aligned one, potentially leading to a more luminous signal as the radiation suffers fewer adiabatic losses. Similarly, deviation from outflow sphericity may allow X-ray radiation generated by the accretion disc to emerge along specific directions without being absorbed.
\end{enumerate}
