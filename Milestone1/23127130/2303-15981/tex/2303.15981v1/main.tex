...
\author{Elia Fioravanti}
	...
	\email{fioravanti@mpim-bonn.mpg.de}
	\thanks{Fioravanti thanks the Max Planck Institute for Mathematics in Bonn for their hospitality and financial support while this work was being completed}
...
Theorem~\ref{thm:main_intro} also provides the first examples of groups with high-dimensional Morse boundary, not witnessed by an ``obvious'' high-dimensional hyperbolic subgroup. For instance, for $n\geq 4$, it is still an open problem whether the fundamental group of a (cusped) hyperbolic $n$--manifold always admits codimension--$1$ subgroups.
...
\item If $i<k$ and $c\sq A(p;r_1,r_2)$ for a parabolic point $p\in\mc{S}$ and radii $\delta\leq r_1<r_2\leq r_p/K$, then we can additionally ensure that $d\sq A(p;r_1/K,Kr_2)$.
\item If instead $i\leq k$ and $c\sq B(p,r_2)$ for a parabolic point $p\in\mc{S}$ and a radius $0<r_2\leq r_p/K$, then we can additionally ensure that $d\sq B(p,Kr_2)$.
\end{enumerate}
\end{prop}
...
As in Section~\ref{sect:Bowditch_filling}, let $(G,\mc{P})$ be a relatively hyperbolic pair where $G$ is finitely generated and $\mc{P}$ consists of finitely many finitely generated, virtually nilpotent subgroups. We assume that the Bowditch boundary $\mc{S}$ is homeomorphic to a sphere $S^{k+1}$ for some $k\geq 1$. This is except in Proposition \ref{prop:Morse_Bow} and Theorem \ref{thm:path_conn}, which we state under more general conditions.
...
\begin{proof}
This is proven for fundamental groups of finite volume hyperbolic manifolds in \cite[Proposition 6.4]{charney2019complete}, but the argument works more generally replacing $\mathbb H^3$ with the cusped space for $G$.
\end{proof}
