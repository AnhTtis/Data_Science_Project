...
This work is organized as follows. In Section \ref{sec:pressure} we present the perturbative setup and a few details on the calculation of the pressure and chiral condensate to next-to-leading order, as well as the running of the coupling and strange quark masses. In Section \ref{sec:results} we discuss our results and compare some of them to what has been obtained recently on the lattice. Section \ref{sec:outlook} contains our summary and outlook.
...
In this section we compute the pressure and chiral condensate in the lowest Landau level approximation up to next-to-leading order in perturbative QCD. We assume that the system is embedded in a uniform, {\it very} large magnetic field ${\bf B}=B \hat{\bf z}$, where the field strength $B$ is much larger than the temperature and all masses.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Leading-order contribution to the pressure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Let us start with the leading-order (LO), or one-loop (free), contribution to the pressure of thermal QCD in the presence of high magnetic fields. The LO contribution coming from the quark sector is given by the following renormalized expression (subtracting the pure vacuum term) \cite{Fraga:2012rr,Kharzeev:2013jha,Andersen:2014xxa,Miransky:2015ava}:
...
The LO contribution from the gluons has the usual Stefan-Boltzmann form \cite{Kapusta:2006pm}
%
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Next-to-leading-order contribution to the pressure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The next-to-leading-order (NLO) contribution from the quark sector to the pressure of thermal QCD in the presence of high magnetic fields was computed in Ref. \cite{Blaizot:2012sd}. For numerical purposes, however, it is convenient to recast the result found in that reference in a different fashion. 

Let us start with the two loop (exchange) pressure, in the LLL approximation extracted from Ref. \cite{Blaizot:2012sd}, 
 %
 \begin{align}
...
\item First evaluate the momentum integrals and then carry out the Matsubara sums numerically at $\mu =0$. This produces a term that depends on temperature and magnetic field which can not be separated into vacuum and medium contributions. 
\end{itemize}
%
...
This expression has the advantage of being numerically simple. Its downside, however, is that it only holds for $\mu=0$ and can not be used for cold and dense QCD. 



Taking into account the NLO contribution from the gluons, given by the well-known formula \cite{Kapusta:2006pm}:
%
\begin{equation}
P_{\rm 2}^G=-N_c (N_c^2-1)\frac{g^2 T^4}{144} \,,
\end{equation}
%
the total NLO pressure can be written as:
%
\begin{equation}
P_{\rm NLO}= P_{\rm free}^G + P_{\rm 2}^G + P_{\rm free}^{\rm LLL} +  P_{\rm exch}^{\rm LLL}
\,.
\label{PNLO}
\end{equation}
...
&\times\bigg \{\frac{m_f^3\left[\mathcal{E}_{n_2}\left|\mathcal{E}_l-\mathcal{E}_{n_2}\right|-\mathcal{E}_{n_1}\left(\mathcal{E}_l-\mathcal{E}_{n2}\right)\right]}{\mathcal{E}_l \mathcal{E}_{n_1}^2 \mathcal{E}_{n_2}^2 \left(\mathcal{E}_l-\mathcal{E}_{n_2}\right) \left(\left|\mathcal{E}_l-\mathcal{E}_{n_2}\right|+\mathcal{E}_{n_1}\right)^2}\\
 &-\frac{m_f\left(\mathcal{E}_l-\mathcal{E}_{n_2}\right)\left[2\omega_p^2\omega_q^2+m_f^2\left(\omega_p^2+\omega_q^2\right)\right]}{\mathcal{E}_l \mathcal{E}_{n_1}^3 \mathcal{E}_{n_2}^3 \left|\mathcal{E}_l-\mathcal{E}_{n_2}\right| \left(\left|\mathcal{E}_l-\mathcal{E}_{n_2}\right|+\mathcal{E}_{n_1}\right)} \bigg \} \,,
\end{split}
\end{align}
%
...
%
\begin{equation}
\chi_s=\frac{1}{T^2}\frac{\partial^2 P} {\partial\mu_s^2} \,.
\label{chi-s}
\end{equation}
%
...
\item[(iii)] Same as the previous one, but with $\bar{\Lambda}=2\pi T$.
\item[(iv)] $\alpha_s$ given by Eq. (\ref{eq:alphas}) and $\bar{\Lambda}=2\pi T$. This corresponds to the usual thermal QCD choice, and ignores the possible effect of the magnetic field on the scale $\bar{\Lambda}$. This choice has been adopted, e.g., in Ref. \cite{Bandyopadhyay:2017cle,Karmakar:2019tdp}.
...
\begin{subfigure}
 ...
 \end{subfigure}
\caption{$P_s^{\rm free}$ (top-left), $P_s^{\rm exch}$ (top-right), $P_s^{\rm exch}/P_s^{\rm free}$ (bottom-left), and $P_s$ (bottom-right) as functions of the temperature for $eB=9$ $\rm{GeV}^2$.}
\label{fig:Ps_B9}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We can now discuss our perturbative results for the pressure, chiral condensate and strange quark number susceptibility to next-to-leading order for very large magnetic fields. We show results for the different running schemes discussed in the literature, and compare them to what has been obtained recently on the lattice. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Pressure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In what follows, we present results for the pressure as a function of the temperature for the highest value of magnetic field attained in present lattice simulations ($eB=9$ GeV$^2$), Fig. \ref{fig:Ps_B9}, and for an even larger field ($eB=50$ GeV$^2$), Fig. \ref{fig:Ps_B50}. We also present results for the pressure as a function of the the magnetic field for $T=0.6$ GeV (Fig. \ref{fig:Ps_T6_bands}). In these figures, we show a panel with the free pressure, $P_s^{\rm free}$, the exchange diagram contribution, $P_s^{\rm exch}$, the ratio $P_s^{\rm exch}/P_s^{\rm free}$, and the full strange pressure, $P_s$. We show results for the contribution from the strange quark because mass effects are more relevant in this case. The ratio $P_s^{\rm exch}/P_s^{\rm free}$ provides a certain measure of the reliability of perturbation theory, since it seems to be more well behaved than the case in the absence of a large magnetic field \cite{Blaizot:2012sd}.
...
\begin{subfigure}
 ...
 \end{subfigure}
\caption{$P_s^{\rm free}$ (top-left), $P_s^{\rm exch}$ (top-right), $P_s^{\rm exch}/P_s^{\rm free}$ (bottom-left), and $P_s$ (bottom-right) as functions of the temperature for $eB=50$ $\rm{GeV}^2$.}
\label{fig:Ps_B50}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{subfigure}
 ...
 \end{subfigure}
\caption{$P_s^{\rm free}$ (top-left), $P_s^{\rm exch}$ (top-right), $P_s^{\rm exch}/P_s^{\rm free}$ (bottom-left), and $P_s$ (bottom-right) as functions of the magnetic field at $T=0.6$ $\rm{GeV}$. The bands correspond to changes in the central scale by a factor of $2$.}
...
% \begin{subfigure}
 ...
 %\end{subfigure}
\caption{$P_s^{exch}/P_s^{free}$ as a function of the magnetic field for $T=0.3$ $\rm{GeV}$.}
...
In this paper we computed the pressure, chiral condensate and strange quark number susceptibility  within perturbative QCD at finite temperature and very high magnetic fields up to next-to-leading order and physical quark masses. Since we adopt the lowest-Landau level approximation in order to obtain analytic results and more control on qualitative aspects, the region of validity for our framework is restricted to $m_s \ll T \ll \sqrt{eB}$, where $m_s$ is the strange quark mass, $e$ is the fundamental electric charge, $T$ is the temperature, and $B$ is the magnetic field strength. 

Since the literature in the field exhibits several possibilities for the running scheme, we study the convergence of the perturbative series for the pressure using the most commonly adopted choices for the scale and functional form of the running coupling, $\alpha_s (T,B)$. Our findings seem to indicate that cases (ii) and (iii) are inconsistent from the point of view of the convergence of the perturbative series, while cases (iv) and (v) pass this criterion, case (v) being the most well behaved. 
...
Another important aspect of QCD matter in the presence of external magnetic fields is the the phenomenon of inverse magnetic catalysis (IMC), which is predicted by LQCD \cite{Bali:2011qj,Bali:2012zg,Endrodi:2015oba}. It have been argued, that the IMC  phenomenon can be explained by introducing thermomagnetic corrections to the strong coupling. This approach have been made in the context of effective models \cite{Farias:2014eca,Ferreira:2014kpa,Ayala:2014iba,Ayala:2014gwa,Farias:2016gmy,Endrodi:2019whh,Moreira:2020wau,Tavares:2021fik} and in QCD from the behavior of the quark gluon vertex \cite{Ayala:2014uua,Ayala:2015bgv} and more recently using renormalization group (RG) equation at the 1L level \cite{Ayala:2018wux}. In the latest,  the authors found that at zero temperature, the coupling increases with the magnetic. At finite temperatures, they found that the coupling increases when the magnetic field is larger than the squared temperature and the opposite behaviour is found if the magnetic fields is lower than the squared temperature.

In fig \ref{Cond_Sig_B4_9} the renormalized quark condensates are plotted and compared with LQCD data from Ref. \cite{DElia:2021yvk}. 
This equation leads to a positive quark condensate. To see this more clearly we plot the pressure at $T=0$ e $B=100 m_{\pi}^2$ (using Eqs.~(\ref{Pfree}) and (\ref{P_T0}) in Fig \ref{PT0vsm}. We can see that the pressure has a negative slope, this means that we get a positive condensate. That is why we get  negatives values in Fig \ref{Cond_Sig_B4_9}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The pressure for $T=\mu=0$ can be read off from the results of Ref. \cite{Blaizot:2012sd} and has the form 
%
\begin{align}
\begin{split}
 \frac{P_{\rm exch}^{\rm LLL}}{N_c}=&-\frac{1}{2}g^2 \left(\frac{N_c^2-1}{2N_c}\right) \sum_f m_f^2\left(\frac{q_fB}{2\pi}\right)\int\frac{dm_k}{2\pi}m_ke^{-\frac{m_k^2}{2 q_f B}}
 %\\
 %&\times
 \int \frac{dp_zdq_zdk_z}{(2\pi)^3}(2\pi)\delta(k_z-p_z+q_z)\frac{1}{\omega E_p E_q}\left(\frac{1}{E_{+}+\omega}\right) \, ,
\end{split}\label{P_T0}
\end{align}
%
where $E_p=\sqrt{p_z^2+m_f^2}$, $\omega=\sqrt{k_1^2+k_2^2+k_3^2}$, and $E_+=E_p + E_q$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Finally, let us show the change of the up down quark condensate, which is define as \cite{Ding:2022tqn}
\begin{align}
 \Delta \Sigma_{ud}=\frac{m_u+m_d}{2m_{\pi}f_{\pi}}\sum_{f=u,d}\left(\left\langle\bar\psi_f\psi_f\right\rangle_{B,T}-\left\langle\bar\psi_f\psi_f\right\rangle_{0,T}\right)
\end{align}
which is shown in Fig \ref{figSigT100} as a function of $eB$ at $T=100$ MeV.
\begin{figure*}[!ht]
\begin{subfigure}
 \centering
 \includegraphics[width=0.45\textwidth]{Psfree_T6.pdf} 
 \end{subfigure}
 \begin{subfigure}
  \centering
 \includegraphics[width=0.45\textwidth]{Psexch_T6.pdf}
 \end{subfigure}
\begin{subfigure}
  \centering
 \includegraphics[width=0.45\textwidth]{Psexch_over_Psfree_T6.pdf}
 \end{subfigure}
 \begin{subfigure}
 \centering
 \includegraphics[width=0.45\textwidth]{Ps_T6.pdf} 
 \end{subfigure}
\caption{$P_s^{\rm free}$ (top-left), $P_s^{\rm exch}$ (top-right), $P_s^{\rm exch}/P_s^{\rm free}$ (bottom-left), and $P_s$ (bottom-right) as functions of the magnetic field for $T=0.6$ GeV.
}
\label{fig:Ps_T6}
\end{figure*}
...
\begin{subfigure}
 \centering
 \includegraphics[width=0.45\textwidth]{Psfree_T3.pdf} 
 \end{subfigure}
 \begin{subfigure}
  \centering
 \includegraphics[width=0.45\textwidth]{Psexch_T3.pdf}
 \end{subfigure}
\begin{subfigure}
  \centering
 \includegraphics[width=0.45\textwidth]{Psexch_over_Psfree_T3.pdf}
 \end{subfigure}
  \begin{subfigure}
 \centering
 \includegraphics[width=0.45\textwidth]{Ps_T3.pdf} 
 \end{subfigure}
\caption{$P_s^{\rm free}$ (top-left), $P_s^{\rm exch}$ (top-right), $P_s^{\rm exch}/P_s^{\rm free}$ (bottom-left), and $P_s$ (bottom-right) as functions of the magnetic field for $T=0.3$ GeV.
}
\label{fig:Ps_T3}
\end{figure*}
...
\begin{subfigure}
 \centering
 \includegraphics[width=0.45\textwidth]{Cond_up_vsT_B325.pdf} 
 \end{subfigure}
 \begin{subfigure}
  \centering
 \includegraphics[width=0.45\textwidth]{Sigma_up_vsT_B325.pdf}
 \end{subfigure}

\caption{left:Quark up condensate as a function of temperature for $eB=3.25$ GeV. Right: Quark up self energy as a function of temperature for $eB=3.25$ $\rm{GeV}^2$}
\label{Cond_sig_B325}
\end{figure}


{\color{blue} \bf At this point, it would be good to discuss the importance of the vacuum subtraction, which we do not have, since our vacuum value only will change in the 2L order via the coupling constant (in case it runs with $T$ or $B$). Also important to think about is that first term the free energy, which is a pure magnetic term, comes form decoupling the magnetic dependency from the vacuum. This can also be done at the two loop order? If the answer is yes, then we must get this pure magnetic 2L term to be consistent with what we have at leading order.} 