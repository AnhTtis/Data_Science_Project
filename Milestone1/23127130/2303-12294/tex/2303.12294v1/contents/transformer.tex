...
% We added different features in the training data to investigate the if labeling the phonetic radical and regularity improve the performance 
% could be improved with phonetic radical and regularity labeled.
We further examined whether identifying the phonetic radical before generating the syllable onset and final would improve the model's performance.  %\Lingyuin{I use a less strong reason here.}, 
...
% However, since many human speakers can not identify the phonetic radical accurately,
...
% In addition, we proposed another set of models adding frequency label \dallf model with frequency label as an input feature and trained on full dataset (\dall). We added two conditions in training all models: Shuffling and Adding tone. We shuffled the position of consonant and vowel in model output to explore the impact of the generated syllable order, and added tone before the `End' token in generation to see whether it would improve the model performance. Examples of input and output of the conditions are shown in Table \ref{input_example}.
% Since the high and mid frequency datasets have training data,
Since previous studies suggested that the regularity and consistency effects are more prominent for the characters with low frequency than high frequency  \cite[e.g.,][]{ziegler2000phonology,chen2009homophone}, the frequency of the known characters might also influence how participants predict the unknown characters. We further added the frequency label as an input feature in the full training data as the \dallf model. The characters  were categorized into four categories based on their frequency: `rare' (frequency = 1), `low' (1 < frequency $\leq$ 50\% percentile), `mid' (50\% percentile < frequency $\leq$ 75\% percentile) and `high' (frequency > 75\% percentile).%\Lingyuin{Not sure whether we need to stress that this is different from MID, HIGH datasets. Maybe it's okay as it is.} 
...
% Previous studies on English generation have shown that [shuffling enhances the performance or something]. \Xiaomengin{add why we need to do shuffle}
...
% \textsc{freq} & Begin, l, uo, high, End \\
\midrule\midrule
...
%\toprule
%dev acc (\%) & \multicolumn{5}{c}{No %Shuffle} & \multicolumn{5}{c}{Shuffle} \\
% & \base & \labm & \labmr & \labs & \multicolumn{1}{l|}{\labsr} & \base & \labm & \labmr & \labs & \labsr \\ \midrule
%All data (4281) & \textbf{58.03} & 55.06 & 56.88 & 55.61 & \multicolumn{1}{l|}{57.53} & 56.13 & 55.79 & 56.69 & 55.96 & 56.6 \\
%High frequent (1381) & 56.94 & 55.98 & 56.17 & 56.72 & \multicolumn{1}{l|}{54.23} & 56.7 & 56.24 & 57.54 & 56.35 & 55.06 \\ \bottomrule
%\end{tabular}
%\end{table*}
...
%\begin{tabular}{lllllllllll}
%\toprule
%test acc (\%)& \multicolumn{5}{c}{No Shuffle} & \multicolumn{5}{c}{Shuffle} \\
% & \base & \labm & \labmr & \labs & \multicolumn{1}{l|}{\labsr} & \base & \labm & \labmr & \labs & \labsr \\ \midrule
%All data & 53.06 & 49.28 & \textbf{53.56} & 50.67 & \multicolumn{1}{l|}{53.22} & 51.22 & 50.83 & 49.67 & 49.89 & 51.44 \\
%High frequent & 52.89 & 51 & 51.67 & 50.06 & \multicolumn{1}{l|}{50} & 52.33 & 50.67 & 51.61 & 50.22 & 49.06 \\ \bottomrule
%\end{tabular}
%\end{table*}

% \Lingyuin{We vary the learning rate and copy their hyperparameters, e.g., 4000 warm-up steps, so we need to point out the paper here. We could add the formula if necessary.}
%\footnote{The test set is constructed with rare words, which has a different distribution than the train and dev set. Our goal is to compare with human behaviors, so we choose to use more training data here instead of getting higher test accuracy with another test split.}, 
% and reported the average accuracy.

%When we add frequency lables to model input, the models have the labels in train/dev/test set. However, when we add se to model output, the models are only provided with the extra information during training, and are asked to predict these labels (i.e., phonetic radical position, regularity type, tone) on dev/test set.


%\begin{table}[!t]
%\small
%\centering
%\begin{tabular}{p{0.12\linewidth}p{0.18\linewidth}p{0.08\linewidth}p{0.09\linewidth}p{0.08\linewidth}p{0.09\linewidth}}\toprule
%Acc(\%) & Model & {-}T{-}S &{+}T{-}S& {+}T{+}S & {+}T{+}S \\\midrule
% \multirow{4}{*}{syllable} 
% & \base & 50.3 & 48.3 & 51.0 & 45.7 \\
% & \labm & 49.7 & 48.0 & 50.0 & 46.7 \\
% & \labs & 48.7 & 47.0 & 48.0 & 49.7 \\
% & \labmr & \textbf{52.3} & 50.7 & \textbf{52.0} & \textbf{50.7} \\
% & \labsr & 52.0 & \textbf{52.7} & \textbf{52.0} & 48.7 \\\midrule
%\multirow{4}{*}{cons.}
%& \base & 57.0 & 56.7 & 57.3 & 53.3 \\
%& \labm & 56.3 & 57.0 & 54.7 & 54.3 \\
%& \labs & 55.3 & 55.0 & 56.0 & 56.7 \\
%& \labmr & \textbf{59.7} & 57.7 & \textbf{58.7} & \textbf{59.3} \\
%& \labsr & 58.7 & \textbf{58.3} & \textbf{58.7} & 56.3 \\\midrule
%\multirow{4}{*}{vowel}
%&  \base & 67.0 & 66.0 & 70.3 & 67.3 \\
%&  \labm & 68.3 & 67.0 & \textbf{71.0} & 68.0 \\
% & \labs & 68.3 & 66.3 & \textbf{71.0} & 70.0 \\
% & \labmr & 69.0 & 68.0 & \textbf{71.0} & 66.3 \\
% & \labsr & \textbf{70.7} & \textbf{71.0} & 70.7 & \textbf{70.3}\\
% \bottomrule
%\end{tabular}
%\caption{\label{tab:dall}The average accuracy of consonants and vowels on test set for models trained on the full set (\dall). }
%\end{table}
...
\end{table}
% For the 80 models with different labels and conditions\footnote{The detailed accuracy for each model averaged over 5 random seed is shown in Table \ref{app_tab: test_acc} in Appendix.}, the average test accuracy is 0.47, which is significantly higher than human's average accuracy (0.45). \textbf{The impact of frequency:}\textbf{The impact of phonetic radical:} Models with \labsr has the best average accuracy (0.49). However, the average accuracy of different phonetic radical and regularity labels are not significantly different from each other. Therefore, knowing the phonetic radical's position and regularity might not improve the model's performance. \textbf{The impact of Shuffling and Deleting Tone}: Shuffling and Deleting the tone did not significantly change the average accuracy, thus these two conditions might not affect the model's performance. The summary of the average test accuracy is listed in Table \ref{tab:freq}. 
% \begin{table}[!h]
% \small
% \centering
% \begin{tabular}{ll}
...
% \midrule
% High Frequency & 0.42 \scriptsize{(0.04)} \\
% Mid Frequency & 0.47 \scriptsize{(0.04)} \\
% All Data & 0.51* \scriptsize{(0.04)} \\
% All + \textsc{freq} label & 0.50 \scriptsize{(0.04)} \\
% \hline
% %\multicolumn{2}{l}{\scriptsize{* indicates significantly higher}}\\
% \midrule
% Phonetic Radical Labels & Test Acc \\
% \midrule
% \base & 0.46 \scriptsize{(0.05)} \\
% \labm & 0.46 \scriptsize{(0.05)} \\
% \labs & 0.47 \scriptsize{(0.06)} \\
% \labmr & 0.47 \scriptsize{(0.05)} \\
% \labsr & 0.49 \scriptsize{(0.05)} \\
% \hline
% \midrule
% Conditions & Test Acc \\
% \midrule
% {[}-Shuffle{]} & 0.48 \scriptsize{(0.05)} \\
% {[}+Shuffle{]} & 0.47 \scriptsize{(0.05)} \\
% {[}-Tone{]}  & 0.48 \scriptsize{(0.05)} \\
% {[}+Tone{]}  & 0.47 \scriptsize{(0.05)} \\
% \bottomrule
% \end{tabular}
% \caption{The average test accuracy of models trained on different datasets with and without \textsc{freq} label, with different phonetic radical and regularity labels and Shuffling and Deleting tone conditions. The standard deviation is in apprentices.}
% \label{tab:freq}
% \end{table}
