...
\def\figureautorefname~#1\null{{\textcolor{black}{Fig.}~}#1\null}
\def\sectionautorefname~#1\null{{\textcolor{black}{Sec.}~}#1\null}

\providecommand*{\equationautorefnameplural}{Eqs.~}

\def\equationautorefname~#1\null{{\textcolor{black}{Eq.}~}(#1)\null}
...
\end{equation}
This formula was derived under an assumption of chaoticity and locality of the Hamiltonian. It is remarkable that Eq.~\eqref{eq:w=0} is exactly what one obtains for random pure states in the presence of $U(1)$ if one sums over the contributions of all magnetization sectors, with a weight that is determined by their Hilbert-space sizes~\cite{bianchi_hackl_22}. That those two independent results agree is remarkable because no Hamiltonian was invoked in Ref.~\cite{bianchi_hackl_22}, only a splitting of the Hilbert space in fixed magnetization sectors.

Following on the results in Ref.~\cite{huang_19}, and on numerical results reported in Ref.~\cite{Haque_Khaymovich_2022}, Huang conjectured (and provided some numerical evidence) that the average entanglement entropy over midspectrum eigenstates of (local) quantum-chaotic interacting Hamiltonians at $f=1/2$ has the form~\cite{huang_21}
...
\caption{Statistics of the level spacing ratio, see Eq.~\eqref{chaos:eq1}. (a),(b) Average level spacing ratio $\bar r$ for $50\%$ of midspectrum eigenstates in the XYZ ($L=18$) and XXZ ($L=20$) models, respectively. The solid (dashed) horizontal line denotes the GOE (Poisson distribution) prediction $\bar r_{\rm GOE} = 0.5307$ ($\bar r_{\rm P} = 0.3867$). (c),(d) Corresponding distributions $P(r)$ for four sets of Hamiltonian parameters. The solid [dashed] line denotes the GOE [Poisson distribution] prediction $P_{\rm GOE}(r) = \frac{27}{4}(r+r^2)/(1+r+r^2)^{5/2}$ [$P_{\rm P}(r) = 2/(1+r)^2$].}
\label{fig:levelspac}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The main panels of Fig.~\ref{fig:coefficients} show the PDFs $P(z)$ for the XYZ [Fig.~\ref{fig:coefficients}(a)] and XXZ [Fig.~\ref{fig:coefficients}(b)] models. We take $J_2=1.0$, for which the average level spacing ratio $\bar r$ in Fig.~\ref{fig:levelspac} matches the GOE prediction for the model parameters under investigation. The results in Fig.~\ref{fig:coefficients} show that the distribution of eigenstate coefficients is a more sensitive probe of many-body quantum chaos than $r$. Specifically, all the PDFs in Fig.~\ref{fig:coefficients} show deviations from the Gaussian distribution $\bar P(z)$, which are most visible in the tails of the distributions. Several works~\cite{luitz_barlev_16, beugeling_baecker_18, baecker_haque_19, Haque_Khaymovich_2022} have studied distributions of eigenstate coefficients and also observed deviations from the Gaussian distribution.
...
In Fig.~\ref{fig:entropyscalXYZ}(a) and its inset, for $\Lambda=100$ midspectrum eigenstates, we find indications that with increasing system size the difference between the average over random pure states and the average over Hamiltonian eigenstates saturates at a small $O(1)$ number that is smaller than, but likely of the order of, $0.1$. This $O(1)$ number appears to be slightly smaller than the one predicted by Eq.~\eqref{eq:nu}, which, as mentioned in the introduction, is identical to the result for the average over random pure states with fixed magnetization $s^z=0$ at $f=1/2$ [$n=1/2$ and $f=1/2$ in Eq.~\eqref{eq:nleading}].
...
\end{equation}
The average entanglement entropy of random pure states in a systems with $L$ sites and $N$ spinless fermions, after tracing out $L-L_B$ sites, takes the form~\cite{bianchi_dona_19, bianchi_hackl_22}
...
When considering a fixed number $\Lambda$ of midspectrum Hamiltonian eigenstates in the average ($\Lambda=100$, i.e., the fraction of Hamiltonian eigenstates $\nu=0^+$), we found indications that the average eigenstate entanglement entropy differs by a small $O(1)$ number from the prediction for the average over random states in the thermodynamic limit. We found the magnitude of the difference to be $\lesssim 0.1$ both for the XYZ [does not have $U(1)$ symmetry] and the XXZ [has $U(1)$ symmetry] chains. We emphasize two points that we find remarkable about those findings. The first one is that if there is a deviation in the thermodynamic limit, of which we find evidence that we cannot rule out it is a consequence of finite-size effects, the deviation is likely to be very small ($\lesssim 0.1$). The second point is that the deviation is almost the same independently of whether the model exhibits or not $U(1)$ symmetry. Here it is important to emphasize that the average over random states exhibits an $O(1)$ term that {\it does depend} on whether the average is carried out over states in which the magnetization is fixed or not~\cite{bianchi_hackl_22}. We also found indications that the average over eigenstates of the XYZ model differs from Huang's prediction (the former is greater) in the thermodynamic limit. Huang's $O(1)$ correction in the $\nu=0^+$ limit is identical to the $O(1)$ ``mean-field'' correction derived in Ref.~\cite{vidmar_rigol_17} for the average over random pure states at fixed particle number (fixed magnetization in the spin language). The fact that our numerical results appear to differ from Huang's prediction in the absence of $U(1)$ symmetry motivated us to conjecture that the $O(1)$ deviation (if present) is not universal. In order to explore this conjecture, we plan to study the average entanglement entropy over eigenstates of different kinds of random matrices (SYK models being specific examples).

When considering fixed nonvanishing fractions of midspectrum Hamiltonian eigenstates in the average, $\nu\in(0,1]$, we found stronger indications that the average eigenstate entanglement entropy differs by an $O(1)$ number from the prediction for the average over random states in the thermodynamic limit. For this case, we provided a simple analytical expression for the $O(1)$ deviation expected from the maximal result at the middle of the spectrum as a function of $\nu$. This phenomenological result is based on the concave functional form of the eigenstate entanglement entropy, which exhibits a maximum in the middle of the spectrum of local quantum-chaotic Hamiltonians, and the known Gaussian form of the density of states in such models. Our analytical expression provides an excellent description of the numerical results for the average entanglement entropy, after numerically computing one parameter ($\bar S_A^{(\Lambda)}$) and fitting a second one ($c$) using the numerical results for the eigenstate entanglement entropy vs the eigenenergies. We also found stronger indications that the average over eigenstates of the XYZ model differs from Huang's prediction (the average over eigenstates is greater) in the thermodynamic limit. Huang's $O(1)$ correction in the $\nu=1$ limit is identical to the $O(1)$ term derived in Ref.~\cite{bianchi_hackl_22} for the average over random pure states with fixed particle number (fixed magnetization in the spin language) when averaging over all particle-number sectors with a weight that is determined by the number of states in each sector.

If the $O(1)$ term in the average entanglement entropy of midspectrum Hamiltonian eigenstates of quantum-chaotic Hamiltonians is not universal as conjectured here, a fascinating question to be explored in the future is what such an $O(1)$ correction can tell us about the Hamiltonian. For the (integrable) XY model in a transverse field, in Refs.~\cite{vidmar_hackl_18,hackl_vidmar_19} is was shown that there is an $O(1)$ correction at the critical line ($h = J$) for $f>0$, and no $O(1)$ correction otherwise, so that such a correction can be used to identity the critical line. 
...
\caption{Distribution of the absolute values of eigenstate coefficients collected from 100 midspectrum eigenstates of each symmetry block from the: (a),(b) $k=0,\,\pi$ (``real'') and (c),(d) all other ($k\neq 0,\,\pi$; ``complex'') quasimomentum sectors. The results were obtained taking $J_2 = 1.0$, for: (a),(c) the XYZ model ($L=18$) and (b),(d) the XXZ model ($L=20$). The dashed lines show $P(z)$ from Eq.~\eqref{coeff:eq1} in (a),(b) and from Eq.~\eqref{app:coeff:eq7} in (c),(d). The insets show the ratio $\Gamma_\psi$ as a function of $h^z$ in (a),(c) and of $\Delta_1$ in (b),(d). The horizontal dashed lines are the predictions for random states [see discussion in Sec.~\ref{sec:coeff} and Eq.~\eqref{eq:gamma_psi_2}].}
...
\caption{Scaling of the mean energy $\bar {\mathrm E}$ in all symmetry blocks vs $1/L$ for (a),(c) XYZ and (b),(d) XXZ chains with (a),(b) periodic and (c),(d) open boundary conditions, for the same Hamiltonian parameters used in Sec.~\ref{sec:entro}. The empty red squares show the average energy over the entire Hilbert space $\bar E$, and the horizontal dashed lines mark the theoretical prediction for $\bar E$ in the thermodynamic limit: (a),(c) $\bar E^\text{XYZ}_\infty=0$ and (b),(d) $\bar E^\text{XXZ}_\infty=-\frac{1}{4}\qty(J_1\Delta_1+J_2\Delta_2)=-0.2$. In (b),(d), the dotted lines show fits of the numerical results for $\bar E$ in finite XXZ chains to the function $c_1+c_2/L$, which returned (b) $c_1=-0.199$ and $c_2=-0.227$, and (d) $c_1=-0.201$ and $c_2=0.171$. In (a),(b), we use empty (filled) symbols to report $\bar {\mathrm E}$ in the $k=0,\,\pi$ ($k\neq 0,\,\pi$) quasimomentum blocks.}
...
\begin{figure}[!t]
\includegraphics[width=0.98\linewidth]{figpdf/Fig_scaling_params_xxz.pdf}\vspace{-0.2cm}
