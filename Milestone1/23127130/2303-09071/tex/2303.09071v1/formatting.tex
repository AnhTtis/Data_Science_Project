...
%%%%%%%%% TITLE
\title{Joint Multi-scale Tone-Mapping and Denoising for Single HDR Image Enhancement}
...
{\tt\small chenhuaijin@sensebrain.site}
}
...
%%%%%%%%% ABSTRACT
\begin{abstract}
   An image processing unit (IPU), or image signal processor (ISP) for high dynamic range (HDR) imaging usually consists of demosaicing, white balancing, lens shading correction, color correction, denoising, and tone-mapping. Besides noise from the imaging sensors, almost every step in the ISP introduces noise in different ways, and denoising operators are designed for reducing these noise. Designed for dynamic range compressing, tone-mapping operators within an ISP can significantly amplify the noise level, especially for images captured in low-light conditions, making denoising very difficult. We propose a joint multi-scale denoising and tone-mapping that are designed with both operations in mind for HDR images. Our joint network is trained in an end-to-end manner that optimizes both operators together, in order to prevent the tone-mapping operator from overwhelming the denoising operator. Our model outperform existing HDR denoising and tone-mapping operators both quantitatively and qualitatively on the HDR+\ref{hdr+} dataset. 
\end{abstract}

%%%%%%%%% BODY TEXT
\section{Introduction}

In a typical ISP for HDR images,
