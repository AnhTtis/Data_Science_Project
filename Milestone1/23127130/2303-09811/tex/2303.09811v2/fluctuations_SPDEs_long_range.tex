...
subject, our focus in this article is on the case where the noise exhibits heavy-tailed correlations that are
in particular non-integrable at infinity. We will restrict ourselves to the high-dimensional case, which in 
this context means $d \geq 3$. While it is rather straightforward (see the proof of Theorem~\ref{thm:stat_field} below) 
to show that \eqref{eq:basic_mSHE} admits a space-time stationary solution $Z$, the main purpose of this
article is to study its large-scale fluctuations, giving rise to a type of central limit theorem.


The SHE, while significant in its own right \cite{molchanov1991ideas}, has garnered considerable interest due to its connection, via the Cole--Hopf transform \cite{Bertini-Giacomin97}, to the KPZ (Kardar--Parisi--Zhang) equation \dash a model for surface growth  \cite{kardar1986dynamic}. 
The solution of the linear SHE where $\sigma(u)=u$ describes the partition function of a system of directed polymers 
in a quenched random potential, thus establishing a connection to statistical mechanics \cite{Bertini-Cancrini-95}.

The question of large-scale fluctuations is of particular interest.   Recent studies, \cite{gu18edwards, magnen2018scaling, dunlap2020_KPZ_fluctuations, cosco2020law, Lygkonis-Zygouras20, gu20_nlmSHE,gu_komorowski21_kpz_torus_fluct,dunlap2018homo, comets2018fluctuation}, have demonstrated that, provided the correlations of the driving noise have compact support or decay sufficiently fast, both the SHE and KPZ equation exhibit Gaussian fluctuations at large scales in the weak disorder phase, notably when the interaction strength $\beta$ is small. In this case the fluctuations converge to the Edwards--Wilkinson model, 
with effective variance dependent on the noise's spatial mollification (cf. Remark~\ref{rem:effective_var}).


Assuming the noise correlation decays at a polynomial rate of order $\kappa \in (2, d)$, we demonstrate that spatial correlation in the noise persists in the scaling limit. This results in fluctuations described by the stochastic heat equation with additive, spatially coloured noise $\dot W^\kappa$. Our result diverges from the compactly supported 
noise case in two key aspects. First, the scaling factor is of order $\epsilon^{2-\kappa}$, suggesting that the 
decay order of the correlations plays the role of an effective dimension. Second, the formula for the effective 
variance of the limit $\cU$ is different from that appearing in the integrable case. (This is of course to be
expected since the latter diverges in that case so wouldn't be meaningful.)


The natural question whether our theorem could inform the fluctuation analysis of KPZ equation or be applicable to the critical dimension reveals substantial challenges. The Cole-Hopf transform, $h=\log(u)$, is a singular mapping, complicating direct application. These issues will be addressed in \cite{GHL24_KPZ_prep}. 
The critical case $\kappa=2$ in this setting, alongside the exploration of polymer connections in a long-range dependent setting,  remains unexplored and the subject of ongoing research.  In the compactly supported scenario, this has been already satisfactorily addressed for the \textit{anisotropic} KPZ equation \cite{AKPZ2D1,AKPZ2D2,MR4499841,AKPZ2D3} and 
...
it is at present unclear whether a similar behaviour is present in the long-range correlated setting.
 ...
 While extensive research has explored the law of large numbers, invariant measures, and intermittency for the stochastic heat equation \cite{Chen-Eisenberg24, Kohatsu-Higa-Nualart21, Conus-Joseph-Khoshnevisan13, Carmona-Molchanov94, Huang-Nualart-Viitasaari-Zheng20, Mueller91, Bertini-Cancrini-95, Bertini-Giacomin99, Bertini-Giacomin97, Carmona-Viens98, Gaertner88, Krug-Spohn92}, the challenges posed by noise with long-range dependence remain an open area for further investigation.
...
%	the solution of  the stochastic heat equation with additive correlated Gaussian noise:
	\begin{equ}[e:limitEW]
...
Let $\alpha<0$, denote $ \lceil -\alpha \rceil \geq -\alpha$ the smallest (positive) integer bigger than $-\alpha$.
