...
\author[2]{\fnm{Prasanta} \sur{Gorai}}\email{prasanta.astro@gmail.com}
\equalcont{These authors contributed equally to this work.}

\author[2,3]{\fnm{Jonathan C.} \sur{Tan}} %\email{iiiauthor@gmail.com}
%\equalcont{These authors contributed equally to this work.}

\affil*[1]{\orgdiv{Division of Science}, \orgname{National Astronomical Observatory of Japan}, \orgaddress{\street{2-21-1 Osawa, Mitaka}, \city{Tokyo}, \postcode{181-8588}, \state{State}, \country{Japan}}}

\affil[2]{\orgdiv{Department of Space, Earth and Environment}, \orgname{Chalmers University of Technology}, \orgaddress{\street{SE-412 96}, \city{Gothenburg}, \country{Sweeden}}} %\postcode{10587}, \state{State}, 

\affil[3]{\orgdiv{Department of Astronomy}, \orgname{University of Virginia}, \orgaddress{\city{Charlottesville}, \postcode{22904-4325}, \state{Virginia}, \country{USA}}}
...
Larger species of polycyclic aromatic hydrocarbons (PAHs) and fullerenes (C$_{60}$ and C$_{70}$) have been detected in circumstellar envelopes around carbon-rich Asymptotic Giant Branch (AGB) stars and planetary nebulae, while PAHs are also known to be a widespread component of the ISM in most galaxies. 
...
Fig. \ref{fig:route} shows the carbon-chain growth and formation pathways of nitrogen- and sulfur-bearing species (HC$_3$N, HC$_5$N, and CCS), which have been frequently detected in low-mass starless cores. 
...
Around the same time, C$_3$N, C$_4$H, and CH$_3$CCH were found toward dark clouds, including TMC-1 \citep{Friberg80,guelin78c4h,Irvine81}. 
...
It was noted that while many carbon-chain molecules are found to be abundant in the starless core TMC-1, some other quiescent starless cores in the Taurus region are deficient in these species. More generally, carbon-chain molecules have low abundances in diffuse clouds, translucent clouds, and dense hot gas around protostars, namely hot molecular cores (HMCs). 
...
Regarding the present astronomical detection status provided in Table \ref{tab:quant}, we have organized the data into three columns. The first column contains information on carbon-chain species detected in both the ISM and CSEs.
...
More than 300 individual molecular species have been identified in the ISM and CSEs by astronomical observations to date (CDMS\footnote{\url{https://cdms.astro.uni-koeln.de/classic/molecules}}, \citet{mcgu22}). 
...
Fig. \ref{fig:detectionsummary} shows the cumulative plot of carbon-chain detection together with the histogram plot in each year starting from 1971, the first carbon-chain detection year, until 2022. 
...
Following the discovery of HC$_3$N in 1971 \citep{Turner71}, new carbon-chain species continued to be found at a rate of just over one species per year for the next 50 years. A steep increase is seen in 2021 and 2022 in Fig. \ref{fig:detectionsummary}, thanks to the two deep line survey projects toward TMC-1 CP (section \ref{sec:3_11}).

Fig. \ref{fig:pichart} illustrates the detection statistics of all known carbon-chain species per decade starting from the 1970s. 
Fig. \ref{fig:pichart} basically summarizes the information of Fig. \ref{fig:detectionsummary}, but summing up each decade.
...
The point to be emphasized here is that enormous progress has been made in just three years of the current decade and discoveries are continuing at a rapid pace. 
...
\includegraphics[bb = 0 0 550 300, width=\columnwidth]{fig_cumulative_rev.pdf}
...
\begin{figure}
\begin{center}
\includegraphics[bb = 0 0 300 300, scale = 0.5]{fig_pi_rev.pdf}
\end{center}
...
Its ``Cyanopolyyne Peak'' (hereafter TMC-1 CP) is the famous site where carbon-chain molecules are particularly abundant (Fig. \ref{fig:tmc1}).
...
\citet{takano1998} observed the three $^{13}$C isotopologues of HC$_3$N (H$^{13}$CCCN, HC$^{13}$CCN, and HCC$^{13}$CN) using the Nobeyama 45m radio telescope.
...
\end{tabular}
\end{center}
{Note- ``Expected fractionation pattern'' means the [H$^{13}$CCCN] : [HC$^{13}$CCN] : [HCC$^{13}$CN] ratio. Here $x$, $y$, and $z$ are arbitrary values.}
%\end{tabnote}
\end{table*}
...
\end{table}
%\begin{tabnote}
% \footnotetext{Errors indicate the standard deviation. 
...
Thus, the suggested tentative correlation between the gas phase and ice mantle supports the scenario of WCCC; sublimation of CH$_4$ is a trigger of carbon-chain formation in lukewarm gas \citet[e.g.,][]{hassel2008}.

\citet{higuchi2018} conducted survey observations of CCH, $c$-C$_3$H$_2$, and CH$_3$OH toward 36 Class 0/I protostars in the Perseus molecular cloud using the IRAM 30m and Nobeyama 45m radio telescopes. 
...
Similar trends were suggested by \citet{lefloch2018} with data taken in the IRAM Large Program ``Astrochemical Surveys At IRAM (ASAI)''.
The ASAI program is an unbiased line survey from 80 to 272 GHz toward 10 sources with various evolutionary stages.
\citet{lefloch2018} found a difference in environmental conditions between hot corino and WCCC sources: i.e., inside and outside dense filamentary cloud regions, respectively.
...
\citet{pineda2020} found a streamer-like structure toward IRAS\,03292+3039 in the Perseus star-forming region with NOEMA. Such a streamer may be well traced by carbon-chain molecules such as CCS and HC$_3$N, if it is considered to be chemically young.
...
The detection rates of HC$_3$N, HC$_5$N, $c$-C$_3$H$_2$, CCS are 93\%, 50\%, 68\%, and 46\%, respectively, in high-mass star-forming regions \citep{tani2018survey,tani2019survey}.
...
\begin{figure*}[hbt]
\begin{center}
\includegraphics[bb = 0 10 700 270, scale=0.5]{fig_chemical_diversity.pdf}
\end{center}
...
\hline
Species & Cepheus E & L1206 & HH288 & I\,00420 & I\,20343 & I\,00259 & I\,05380 & I\,20293 & I\,21307 & I\,22198 & I\,23385  \\
\hline
HC$_3$N          & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ \\
HC$_5$N          & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ &         & $\surd$ & $\surd$ \\
C$_3$H           &         & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ &         &         &         & $\surd$ &         \\
C$_4$H           & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ & $\surd$ & $\surd$ \\
$l$-H$_2$CCC     & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         &         &         &         & $\surd$ & $\surd$ \\
$c$-C$_3$H$_2$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ &         & $\surd$ & $\surd$ \\
CCS                & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ & $\surd$ \\
C$_3$S           & $\surd$ & $\surd$ & $\surd$ &         &         &         &         &         &         & $\surd$ &         \\
CH$_3$CCH        &         & $\surd$ & $\surd$ &         & $\surd$ &         &         & $\surd$ &         & $\surd$ &         \\
CH$_3$OH         & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ & $\surd$ & $\surd$ & $\surd$ \\
CH$_3$CHO        & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ &         & $\surd$ &         \\
H$_2$CCO         & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         &         &         & $\surd$ &         \\
HNCO               & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ &         & $\surd$ &         \\
CH$_3$CN         & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         &         &         & $\surd$ &         \\
...
H$_2$CS          & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ & $\surd$ &         & $\surd$ & $\surd$ & $\surd$ & $\surd$ \\
\hline
\end{tabular}
\end{center}
%\begin{tabnote}
{Note: The ``$\surd$'' mark indicates detection with S/N ratios above 5. \\ ``I'' in the source name is an abbreviation of IRAS.}
%\end{tabnote}
\end{sidewaystable*}
%\end{table*}
%\end{landscape}
...
Thus, the addition method of BE may lead to large uncertainties to estimate BEs of carbon-chain molecules, especially longer ones. If BE of a carbon atom is $\ge$10,000 K is correct, the addition method of BE may not be valid, because long carbon-chain species will have huge BE values. A very recent experimental study suggests that the BE of carbon atoms is $\sim 2900$ K \citep{tsuge2023} in shallower binding sites.  A lower value ($<$5000 K) of carbon atom BE has also been reported in \cite{das18}. Nonetheless, to overcome this issue, dedicated quantum chemical calculations and/or experiments are required to estimate the BE values of carbon-chain species with higher accuracy. 
...
\item Recent new approaches with the machine learning will provide us with predictions of possible detectable species in the ISM, which is expected to help accelerate the discoveries of new interstellar molecules.
\end{enumerate}
...
\end{center}
%\begin{tabnote}
{Taken from the KIDA (https://kida.astrochem-tools.org/), and also see \citet{wake17}, \citet{pent17}, \citet{das18}.}
