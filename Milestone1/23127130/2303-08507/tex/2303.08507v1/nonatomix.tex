...
\author{David Auger\thanks{Thanks for something to someone}\\
Université Paris-Saclay, UVSQ, DAVID\\  78035, Versailles, France\\ 
david.auger{@}uvsq.fr 
\and Johanne Cohen\thanks{Thanks for something else to someone else}\\
...
Thus stated, the model of NBG is quite general, so we shall consider special cases and use graph-theoretic terminology to model this notion of proximity between resources and infer results from the graph topology.
...
As will be seen, our framework is more straightforward than the previous ones in many cases, but a lot of examples already manifest a great depth of complexity.
...
Moreover, the \textit{price of stability}~\cite{schulz2003performance} (ratio between the best equilibria and the optimal solution) is another measure when the game admits several equilibria.  Since nonatomic congestion games have a unique equilibrium, their prices of anarchy and stability are identical. This is why it has been studied only for atomic congestion games; the price of stability is known for congestion games with linear cost functions \cite{caragiannis2006tight, christodoulou2005price} and is upper-bounded \cite{christodoulou2015price} for  congestion games with polynomial cost
functions. 
...
In this section, we present and discuss our model, starting from the general case and refining special cases and properties that will be needed later.
...
Since for all $(x_1,x_2) \in \Delta_1(2)$ we have $x_1+x_2=1$, we can rewrite these costs as functions of $x_1$ only i.e. $C_1(x_1)= 4x_1(1-x_1)$ ans $C_2(x_1)=x_1$; these are depicted on Figure~\ref{fig:ex1}. As can be seen, costs are equal when $x_1=0$ and $x_1=\frac{3}{4}$. When $x_1 < \frac{3}{4}$, vertex $2$ has a lower, hence better cost. Hence the mass on vertex $1$ might want to move to $2$, decreasing $x_1$ in return. When $x_1 > \frac{3}{4}$, the cost is better on vertex $1$, hence the mass on $2$ might want to move to $1$, increasing $x_1$. Thus we can suspect that if we let the mass continuously move, starting from $x_1 \neq \frac{3}{4}$, we would obtain stabilization at $x_1=0$ or $x_1=1$ depending on the case. If $x_1$ is exactly $\frac{3}{4}$, the two costs are equal, and we can interpret this as a case where no player can improve its cost by switching from one vertex to the other.
...
\subsubsection{Special case where there are two ressources ($n=2$)}
...
the same cost, while uncharged vertices have a greater cost.
...
Here we study the structure of equilibria in one special case, namely normal linear NBGs. This enables us to derive complexity results for the problem of finding an equilibrium in a given NBG.
...
\begin{proof}
 First, consider a kernel $K$ of $G$ of size $k$ and define $\x^*$
...
In general, checking if a distribution is a strong equilibrium depends on the model used to define functions. But we still have that:
...
This gives $C_1(\x)=2-x_1$, $C_2(\x)=2$, and $\Phi(\x)=x_1+(x_2+\frac{1}{2}x_2^2)+x_1x_2=(3-x_1^2)/2$ (this potential function is given by Equation~\eqref{eq:formulePhi} in the upcoming Proposition~\ref{prop:potentialFormula}; we can  easily check that this is indeed a potential. Otherwise, other potentials are equal to this one up to an additive constant; also note that a potential can be expressed with $x_1$ alone, with $x_2$ alone, or with both $x_1$ and $x_2$). There are two equilibria, one in $x_1=1$ which is a minimum of the potential function, and one
...
\begin{aligned} \label{ALeqAL1}
 x_1 + \alpha x_2 &= c \\
 \forall \, 1 < i < n, \; \; \alpha x_{i-1} + x_i + x_{i+1} &= c \\
 \alpha x_{n-1} + x_n &=c \\
 \sum_{i\in [n]} x_i &= 1 \\
...
If we denote by $M_{n,\alpha}$ the $(n+1)\times (n+1)$ matrix
\[
 \begin{pmatrix}
  1 & \alpha &  & & & & & -1 \\
  \alpha & 1 & \alpha & & & & & -1 \\
