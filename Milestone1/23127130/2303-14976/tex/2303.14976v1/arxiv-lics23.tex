...
\IEEEauthorblockA{\IEEEauthorrefmark{4}
	\textit{Universidad Nacional Autónoma de México (UNAM)}, Mexico D.F., Mexico
...
These results assumed a finite, fixed set of agents, whose local states appear in every world.
...
We show the very important, and surprising fact that every semi-simplicial set model is bisimilar to a simplicial complex model (leading to Theorem \ref{thm:complete-pure}).
...
We define various interesting sub-classes of these models in \cref{sec:subclasses}. In \cref{sec:axiomatization}, we prove $\ECn$ is sound and complete with respect to these models, Theorem \ref{thm:complete-simple}, and give  sound and complete axiomatizations of interesting sub-classes, Theorems \ref{thm:complete-max}, \ref{thm:complete-min}, \ref{thm:complete-pure}. Finally, we prove that every simplicial set based model is bisimulation equivalent to a simplicial complex model, bringing interesting questions about the topological nature of bisimulation, Theorem \ref{thm:complete-standard}.
...
\begin{example}\label{ex:frame}
    Below is an example of a generalized epistemic frame.
    ...
    \end{center}
\end{example}
%\todo[inline]{Add a sensor network example?}
...
But the three sensors together can distinguish $w_1$ from all other worlds. Examining all other intersections in pairs of worlds gives the following epistemic frame: 
...
The cset in the picture below is composed of seven vertices (colored with three agents), eight edges, two triangles, and two $(-1)$-simplices.
%
%\begin{figure}[h]
%    \centering
\begin{center}
...
In the rest, by a \emph{(chromatic) simplicial complex} we mean a cset in which if two simplices share the same set of vertices, then they coincide.
...
The idea is that, lying above the simplicial set~$B$ representing the geometry of the model, we have a discrete simplicial set~$E$ representing the worlds. A morphism $f: E \to B$ then maps each world to its representation in the geometric model.
...
We say that~$X$ is \emph{discrete} when for every simplex $s\in X$, there is a unique maximal simplex $\uparrow\!s$, 
such that $s$ is a subsimplex of $\uparrow\!s$.

\begin{definition}
    An \emph{epistemic covering} is a morphism $f:E\to B$ in $\Simp$ such that $E$ is discrete and $f: E \to B$ is surjective.
    Equivalently, a morphism $f:E\to B$ is an epistemic covering if $E$ is discrete and every maximal simplex of $B$ has a preimage.
...
\subsection{Equivalence between epistemic frames and coverings}
...
Then, we need a discrete cset~$E$, which can be thought of as an ``exploded view'' of~$B$, depicted below.
...
by mimicking what we did for generalized epistemic models.
%
\[\begin{array}{lcl}
...
In the picture below, the top left covering has trivial empty-group knowledge as there is only one $(-1)$-simplex in the base space (one dashed region), but it has one maximal $(-1)$-simplex in the top space, so it has empty worlds. The top right covering has no empty worlds as it has two maximal simplices of dimension $1$ and one $0$-simplex in the top space. It, however, does not have trivial empty-group knowledge as it has two $(-1)$-simplices in the base space. It is not proper neither. The bottom covering is proper, as every simplex is annotated with at most one world.
%
\begin{center}
...
To demonstrate how to prove soundness and completeness with those axioms, we prove the following theorem.
...
\begin{tikzcd}[column sep=tiny, row sep=small]
        & {\mathrm{+Max +NE}} && {\textcolor{black}{\mathrm{+Max + NE+ P}}} \\
        && {\mathrm{+Max + P}} \\
        {\mathrm{+Max}} & {\mathrm{+NE}} && {\mathrm{+NE + P}} \\
        && {+\mathrm{P}} \\
        {\textcolor{black}{\mathrm{KB_4 + Mono + Union}}} & {+\mathrm{Min+ NE}} && {\mathrm{+Min + NE + P}} && {\textcolor{black}{\mathrm{+Pure + NE + P + Pure}}} \\
        && {+\mathrm{Min + P}} \\
        {\mathrm{+Min}}
...
M.~Herlihy, D.~Kozlov, and S.~Rajsbaum, \emph{Distributed Computing Through
  ...
  Francisco, CA, USA: Morgan Kaufmann, 2013.
...
H.~v. Ditmarsch, {\'E}.~Goubault, J.~Ledent, and S.~Rajsbaum, ``Knowledge and
  ...
  0.4em\relax Cham: Springer International Publishing, 2022, p. 1–50.
...
H.~van Ditmarsch, ``Wanted dead or alive: Epistemic logic for impure simplicial
  ...
  International Workshop, WoLLIC 2021, Proceedings}, ser. Lecture Notes in
...
J.~Gerbrandy, ``Distributed knowledge,'' in \emph{Formal Semantics and
  Pragmatics of Dialogue (Twendial)}, S.~Hulstijn and A.~Nijholt, Eds.,
...
A.~Baltag and S.~Smets, ``Learning what others know,'' in \emph{{LPAR} 2020:
  ...
  Intelligence and Reasoning, 2020}, ser. EPiC Series in Computing, E.~Albert
  and L.~Kov{\'{a}}cs, Eds., vol.~73, 2020, pp. 90--119.
...
P.~Goerss and J.~Jardine, \emph{Simplicial Homotopy Theory}, ser. Modern
  ...
  Birkh{\"a}user Basel, 2009. [Online]. Available:
...
G.~Friedman, ``An elementary illustrated introduction to simplicial sets,''
  2008, V7 25 May 2021. [Online]. Available:
...
H.~Sahlqvist, ``Completeness and {{Correspondence}} in the {{First}} and
  ...
  the {{Third Scandinavian Logic Symposium}}, S.~Kanger, Ed.\hskip 1em plus
  0.5em minus 0.4em\relax {Elsevier}, vol.~82, pp. 110--143. [Online].
  Available:
...
A.~Joyal, M.~Nielsen, and G.~Winskel, ``Bisimulation from {{Open Maps}},'' vol.
  127, no.~2, pp. 164--185. [Online]. Available:
...
P.~Gabriel and M.~Zisman, \emph{Calculus of {{Fractions}} and {{Homotopy
  Theory}}}, ser. Ergebnisse Der {{Mathematik}} Und Ihrer {{Grenzgebiete}}. 2.
...
{\'{E}}.~Goubault, M.~Lazic, J.~Ledent, and S.~Rajsbaum, ``Wait-free
  ...
  Budapest, Hungary}, ser. LIPIcs, J.~Suomela, Ed., vol. 146.\hskip 1em plus
