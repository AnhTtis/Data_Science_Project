...
\author[1]{Linfeng Shi}
\ead{20211249579@nuist.edu.cn}

\author[1]{Yan Li \corref{cor1}}
...
\begin{figure}[htbp]
	...
	\subfigure[]{
		\begin{minipage}[t]{0.5\linewidth}
			\centering
			\includegraphics[width=1.4in]{Fig_6_1.pdf}
		\end{minipage}%
	}%
	\subfigure[]{
		\begin{minipage}[t]{0.5\linewidth}
			\centering
			\includegraphics[width=1.5in]{Fig_6_3.pdf}
			%\caption{fig2}
		\end{minipage}%
	}%
	...
	\subfigure[]{
		\begin{minipage}[t]{0.5\linewidth}
			\centering
			\includegraphics[width=1.4in]{Fig_6_2.pdf}
			%\caption{fig2}
		\end{minipage}%
	}%
	\subfigure[]{
		\begin{minipage}[t]{0.5\linewidth}
			\centering
			\includegraphics[width=1.5in]{Fig_6_4.pdf}
			%\caption{fig2}
		\end{minipage}%
	}%
	\centering
	\caption{(a) For different n values, the relationship between the deviation between traditional IoU and JIoU and different angle differences. (b) For different object aspect ratios, the relationship between JIoU and different angle differences. (c)Test results of different n values}\label{fig7}
\end{figure}

Figure \ref{fig7}d shows the PR curves of JIoU and SmoothL1, It can also be seen from the figure that after using JIoU, the model recall rate has significantly improved, which shows that the JIoU proposed in this paper as a loss function has better performance in directional object detection tasks. From the accuracy of a single object, it can be seen that JIoU has significantly improved the detection accuracy of cars. From the detection effect of the boundary box in the results, two reasons can be found: first, although cars are smaller than aircraft, However, its aspect ratio is larger, which results in more sensitive results to angle changes when calculating IoU. Secondly, the angle has periodicity and the unit of expression is different from the length and width. In case of traditional SmoothL1, it is inconsistent in the optimization rate of angle and length and width. That leads to excessive optimization of local parameters of the model, ultimately leading to poor generalization of the model. However, JIoU calculates the extreme value of discrete ellipse through angle and jointly optimizes it, which effectively solves the above problems caused by angle, allowing the model to possess better generalization performance.

The sensitivity of the calculation method of JIoU to the aspect ratio of different objects is different. When the aspect ratio is small, the ellipse tends to be circular. At this time, the sensitivity of JIoU to the boundary box parameters will decrease, which can be seen in Figure \ref{fig7}a. Figure \ref{fig7}b shows the influence of different n values on JIoU. When the value of n increases gradually, the range of the deviation curve between traditional IoU and JIoU becomes smaller and smaller. At this time, the accuracy of the calculation of IoU is further improved.

At the same time, we use different n values to train on the UCAS-AOD data set, and test. As can be seen from Figure \ref{fig7}c, when $n$ values are small, the test results show randomness. With the increase of n, the accuracy also gradually increases, and the improvement is also smaller and smaller, and finally tends to be stable, showing a nonlinear relationship as a whole.
...
Thanks are due to Xi Zhu for providing language assistance and to Yan Li for valuable discussion.
