...
\begin{equation}\label{mesoscopic CLT - Intro}
L_{N}(f_\eta)- \EE L_{N}(f_\eta) \to \mathcal{N} (0, v_f)\,, \qquad v_f = \frac{1}{2\pi^2\beta} \int_{-\infty}^\infty\int_{-\infty}^\infty \bigg( \frac{f(x)-f(y)}{x-y}\bigg)^2 dx dy\,,
\end{equation}
...
Versions of the mesoscopic CLT \eqref{mesoscopic CLT - Intro} have been established for a wide class of random matrix models. These include the classical Gaussian ensembles \cite{BoutKhor99a,FyodKhorSimm16},  Wigner matrices, for which the CLT has been shown covering an increasing range of mesoscopic regimes \cite{BoutKhor99b,HeKnow17,LodhSimm15,Riab}, invariant ensembles  \cite{BekeLodh18,Lamb18}, deformed Wigner matrices \cite{LiSchnXu21}, Dyson Brownian motion \cite{DuitJoha18,HuanLand19}, band matrices \cite{ErdoKnow15b}, and free sums \cite{BaoSchnXu22}.
...
\begin{equation}\label{LMP - Intro}
...
\begin{proof}
  ...
  \begin{equation}
    \label{eq:60}
    \EE\Big[\efr(t) \big(1-\EE\big)\big[\Wb \Gb(z)\big]\Big]
    =
    -\EE\Big[\efr(t)\big(1-\EE\big)\big[\Scc[\Gb(z)]\Gb(z)\big] \Big] + \EE\Big[ \widetilde{\Wb}  \Gb(z) \nabla_{\widetilde{\Wb}} \big(\efr(t)\big)\Big]
     + \errC(z)
  \end{equation}
   ...
   \end{equation}
   and $\errJ_{1},\errJ_{2}$ from \eqref{eq:41}.
  We rewrite the first term on the right-hand side in the above equality as
  \begin{align}
    \label{eq:61}
    &-\EE\Big[\efr(t)\big(1-\EE\big)\big[\Scc[\Gb(z)]\Gb(z)\big] \Big]
      \\
    & \qquad
      =
      -\EE\Big[\efr(t)\big(1-\EE\big)\big[\Scc[\Mb(z)]\Gb(z)\big] \Big]
     -\EE\Big[\efr(t)\Scc \big[\big(1-\EE\big)[\Gb(z)]\big]\Mb(z) \Big]
      + \errH (z)
    \\ \label{eq:62}
    & \qquad
      =
      \EE\Big[\efr(t)\Big(\frac{1}{\Mb(z)} + z\Ib_{nN} - \Kb_{0}\Big) \big(1-\EE\big)\big[\Gb(z)\big] \Big]
      -\EE\Big[\efr(t)\big(1-\EE\big)\big[\Scc[\Gb(z)]\Mb(z)\big] \Big]
      + \errH (z)
      ,
  \end{align}
  where we used that $\Mb(z)$ satisfies \eqref{eq:46}, and introduced the error term
  \begin{equation}
    \label{eq:63}
    \errH (z)
    :=
    ...
    .
  \end{equation}
  The above identities \eqref{eq:60} and \eqref{eq:62} substituted into \eqref{eq:55} after an elementary cancellation give
  \begin{equation}
    \label{eq:64}
    \frac{1}{\Mb(z)}\EE\Big[\efr(t)\big(1-\EE\big)\big[\Gb(z)\big] \Big]
    =
    \EE\Big[\efr(t)\big(1-\EE\big)\big[\Scc[\Gb(z)]\Mb(z)\big] \Big]
  ...
  \begin{equation}
    \label{eq:65}
    \Bcc_z \, \EE\Big[\efr(t)\big(1-\EE\big)\big[\Gb(z)\big] \Big]
    =
    -\EE\Big[ \widetilde{\Wb}  \Gb(z) \nabla_{\widetilde{\Wb}} \big(\efr(t)\big)\Big] \frac{1}{\Mb(z) }
    - \Big(\errC(z) + \errH (z) \Big) \frac{1}{\Mb(z) }
    ,
  \end{equation}
  ...
  \begin{equation}
    \label{eq:66}
    (\Id_n\otimes \Tr_N)\EE\Big[\efr(t)\big(1-\EE\big)\big[\Gb(z)\big] \Big]
    =
    -\Bc_{z}^{-1} (\Id_n\otimes \Tr_N) \Big[\EE\big[ \widetilde{\Wb}  \Gb(z) \nabla_{\widetilde{\Wb}} \big(\efr(t)\big)\big]  \frac{1}{\Mb(z) } \Big] + \errL (z)
    ,
  \end{equation}
  where
  \begin{equation}
  ...
  \begin{align}
    ...
    &=
      ...
      .
  \end{align}
  Using the trivial relation $\frac{\partial}{\partial \zeta} \Gb(\zeta) = \Gb^2(\zeta)$, we get from \eqref{eq:56} and the cyclicity of the trace that
  \begin{equation}
    \label{eq:70}
  ...
  \begin{multline}
    ...
    \\=
    \frac{\imu t}{\pi} \int_{\Omega}   \frac{\partial \ft (\zeta)}{\partial \overline{\zeta}} \cdot  \frac{\partial}{\partial \zeta} \EE\Big[ \Tr \Big( \Bc_{z}^{-1} (\Id_n\otimes \Tr_N) \Big[  \widetilde{\Wb} \Gb(z) \frac{1}{\Mb(z) } \Big]\Big) \Tr \Big(\widetilde{\Wb} \Gb(\zeta) \Big)  \efr(t) \Big] \dzeta + \Tr \errL (z)
    .
  \end{multline}
  ...
  \begin{align}
    ...
    &\frac{d}{dt} \EE[\efr(t)]
      =
      \frac{\imu}{\pi} \int\displaylimits_{\Omega} \frac{\partial \ft(z)}{\partial \overline{z}}  \EE \big[ \efr(t) (1-\EE)[\Tr \Gb(z) ] \big] \dz
    \\ \nonumber
    &=
      - \frac{t}{\pi^2} \int\displaylimits_{\Omega \times \Omega}  \frac{\partial \ft(z)}{\partial \overline{z}}  \frac{\partial \ft(\zeta)}{\partial \overline{\zeta}} \frac{\partial}{\partial \zeta} \EE\bigg[ \Tr \Big(  \Bc_{z}^{-1} (\Id_n\otimes \Tr_N)\Big[ \widetilde{\Wb} \Gb(z) \frac{1}{\Mb(z)}\Big] \Big) \Tr \Big(\widetilde{\Wb} \Gb(\zeta) \Big)  \efr(t) \bigg] \dzeta \dz
      +
      \errK
      ,
  \end{align}
  ...
  Now the expectation in \eqref{eq:73} takes the form
  \begin{align}
    \label{eq:77}
    ...
    &\qquad
      =
      \EE\Big[ \Tr \Big(\Gb(z)  \frac{1}{\Mb(z)} \big((\Bcc_{z}^{*})^{-1}[\Ib_{nN} ]\big)^*  \widetilde{\Wb} \Big) \Tr \Big(\widetilde{\Wb} \Gb(\zeta) \Big)  \efr(t) \Big]
    \\
    &\qquad
      =\EE\Big[ \sum_{i,j=1}^{n}\sum_{p,q=1}^{N} \Tr \Big((E_{ji}\otimes \Eb_{q p}) \Gb(z)  \frac{1}{\Mb(z)} \big((\Bcc_{z}^{*})^{-1}[\Ib_{nN} ]\big)^*  \widetilde{\Wb}(E_{ij}\otimes \Eb_{p q}) \widetilde{\Wb} \Gb(\zeta) \Big)  \efr(t) \Big]
    \\ \label{eq:78}
    &\qquad
      ...
      ,
  \end{align}
  where $\{E_{ij}\otimes \Eb_{p q}\, : \, 1 \leq i,j \leq n, 1 \leq p, q \leq N\}$ is the standard basis in $\CC^{nN\times nN} = \CC^{n\times n} \otimes \CC^{N \times N}$, and in the last step we took the partial expectation with respect to $\widetilde{\Wb}$ (see \eqref{eq:43}).
  ...
  \begin{equation}
    ...
    \right.
  \end{equation}
  Plugging \eqref{eq:79} and \eqref{eq:76} into \eqref{eq:78} leads to the simplified expression of \eqref{eq:77},
  \begin{multline}
    \label{eq:80}
    \EE\Big[ \Tr \Big( \Bcc_{z}^{-1}\Big[  \widetilde{\Wb} \Gb(z) \frac{1}{\Mb(z)} \Big] \Big) \Tr \Big(\widetilde{\Wb} \Gb(\zeta) \Big)  \efr(t) \Big]
    \\ 
  ...
  Using this, the bound on $\errC(z)$ from \eqref{eq:bound on errC} and \eqref{eq:83} we have that the estimate
  \begin{equation}
    \label{eq:84}
    \| \errL (z) \|
    \prec
  ...
  \begin{equation}
    ...
    .
  \end{equation}
  Using now \eqref{eq:85} and the analyticity of $\Tr \errL(z)$ on $\Omega$, we rewrite the error term $\errK$ defined in \eqref{eq:74} as 
  \begin{equation}
    \label{eq:86}
    \errK
    =
...
The derivation is reminiscent of the proofs used in some earlier works on the mesoscopic CLT for random matrices with independent entries (e.g., \cite[Proposition~5.1 and Lemma~5.2]{LandLopaSoso}). 
\begin{lem}
  \label{lem:3}
...
\end{lem}
\begin{proof}
  Similarly as in \eqref{eq:30}, we define the (left) matrix coefficients $\{W_{ij}\}_{1\leq i,j\leq N}$ via
  \begin{equation}
    \label{eq:92}
    \Wb
    =
    \sum_{i,j=1}^{N} W_{ij} \otimes \Eb_{ij}
    , \quad
    W_{ij} \in \CC^{n\times n}
    .
  \end{equation}
  ...
  \begin{align}
    ...
    .
  \end{align}
  By the local law the first term is of order $O_{\prec}(N^{-1}\etaH^{\,-2})$.
...
The expectation of the summands in \eqref{eq:116} vanishes, i.e.,
  ...
  All the functions $M$, $G_{ij}$, $M^{-1}$, $G_{ii}^{-1}$ used in the above proof are analytic on $\Omega$.
  By collecting the error terms in \eqref{eq:114} ($O_{\prec}(N^{-1/2}\etaH^{\,-3/2})$ and $D_k$), taking the average over $k\in \{1,\ldots, N\}$ (see \eqref{eq:113}, denoting the resulting error by $\errE^{B}(z,\zeta)$  and using the equivalence of norms on $\CC^{n\times n}$, we obtain \eqref{eq:91}, which finishes the proof.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    .
  \end{equation}
  For convenience, introduce the centered variables $w:=z - E_{0} \in \CC_{-}$, $\xi:= \zeta - E_{0} \in \CC_{+}$, so that
  \begin{equation}
    \label{eq:129}
    \Bc_{z,\zeta}
    =
  ...
  \begin{equation}
    ...
    ,
  \end{equation}
  where we used that the adjoint operator to $\Bc_{E_{0},E_{0}}$ with respect to the scalar product \eqref{eq:51} is given by
  \begin{equation}
    \label{eq:131}
    \Bc_{E_{0},E_{0}}^{*}
    =
  ...
  \begin{align}
    ...
    \la \Im M \ra
    &=
      \Big\la \Im M \frac{1}{M} M' \frac{1}{M} \Big\ra - \Big\la  \Im M \frac{1}{M} M' \frac{1}{M^*} \Big\ra
      =
...
Therefore, we need the follwoing improved estimate of $G^{B}(z,\zeta)$ in the case when $z$ and $\zeta$ belong to different complex half-planes.
...
We split the domain of integration in \eqref{eq:181} into four regions determined by the signs of $\Im z$ and $\Im \zeta$, namely,
  ...
  By using Stokes' theorem \eqref{eq:85} (twice) and the definition of $\ft$ in \eqref{eq:26}, we  rewrite the integral in \eqref{eq:187}
  as
  \begin{equation}
    \label{eq:190}
    \frac{1}{\pi^2}\int\displaylimits_{\Omega^{+} \times \Omega^{+}}  \frac{\partial \ft(z)}{\partial \overline{z}}  \frac{\partial \ft(\zeta)}{\partial \overline{\zeta}} \frac{\partial}{\partial \zeta} \,h_{1}(z,\zeta) \, \dz \dzeta 
  ...
  \begin{multline}
    ...
    \\
    =
    \EE\big[  \efr(t) \big] \frac{1}{\pi^2 (z - \zeta )}  \sum_{i,j=1}^{n}  \Tr \Big(E_{ji} \, \frac{2\imu }{\la \Im M(E_{0}) \ra} \Im M(E_{0}) \la \Im M(E_{0}),\, B_{ij}(E_0) \, \ra  \Big)  + h_2(z,\zeta) 
    ,
  \end{multline}
  ...
  \begin{align} \label{eq:201}
    &\frac{1}{\pi^2}\int_{\Omega^{+} \times \Omega^{-}}\frac{\partial \ft(z)}{\partial \overline{z}}  \frac{\partial \ft(\zeta)}{\partial \overline{\zeta}} \frac{\partial}{\partial \zeta} \Big( \frac{\phi^{\,(+,-)}}{z-\zeta} \Big) \dz \dzeta
      =
      -\frac{1}{\pi^2}\int_{\Omega^{+} \times \Omega^{-}}\frac{\partial \ft(z)}{\partial \overline{z}}  \frac{\partial \ft(\zeta)}{\partial \overline{\zeta}} \frac{1}{(z-\zeta)^2}  \dz \dzeta
    \\ \label{eq:202}
    & \qquad
      =
      \frac{1}{2 \pi^2}\int_{\Omega^{+} \times \Omega^{-}}\frac{\partial }{\partial \overline{z}}  \frac{\partial }{\partial \overline{\zeta}} \frac{(\ft(z)- \ft(\zeta))^2}{(z-\zeta)^2}  \dz \dzeta
      =
      -\frac{1}{8 \pi^2}\int_{\partial \Omega^{+} \times \partial \Omega^{-}} \frac{(\ft(z)- \ft(\zeta))^2}{(z-\zeta)^2}  dz d\zeta
      .
  \end{align}
  The function $\ft$ vanishes everywhere on $\partial \Omega^{+} \times \partial \Omega^{-}$ except the part that intersects with the lines $\Im z = N^{-\tau} \eta_0$ and $\Im \zeta = N^{-\tau} \eta_0$, therefore, the last integral in \eqref{eq:202} can be written as
  \begin{multline}
    -\frac{1}{8 \pi^2}\int_{\partial \Omega^{+} \times \partial \Omega^{-}} \frac{(\ft(z)- \ft(\zeta))^2}{(z-\zeta)^2}  dz d\zeta
    \\ \label{eq:203}
    =
...
\begin{proof}
  ...
  \begin{equation}
    ...
    .
  \end{equation}
  Notice that for all $k,l \in \{1,\ldots, N\}$ the symmetry of $\Wb$ implies that $W_{l k}^{\, t} = W_{kl}$, $\big(G_{ l k}(z) \big)^{t} = G_{ k l }(z)$ and thus
  \begin{equation}
    \label{eq:231}
    G_{lk}(z) \otimes (G_{lk}(\zeta))^{t} = G_{lk}(z) \otimes G_{kl}(\zeta)
    .
  \end{equation}
 ...
 To show the invertibility of $\Bch_{z,\zeta}$ we define a linear operator $\Phi: \CC^{n\times n}\otimes \CC^{n\times n} \to \CC^{n\times n}\otimes \CC^{n\times n}$, acting on the basis vectors $E_{ij} \otimes E_{kl}$ as
  \begin{equation}
    \label{eq:239}
  ...
  For any vectors $a,b,c,d \in \CC^{n}$ it satisfies
  \begin{equation}
    \label{eq:240}
    \Phi\big[ a b^{\,t} \otimes c d^{\,t} \big]
    =
    a d^{\,t} \otimes  c b^{\,t}
    ,
  \end{equation}
  which in composition with $\Bch_{z,\zeta}$ gives
  \begin{align}
    \label{eq:241}
    \Phi \circ \Bch_{z,\zeta}\big[ E_{ij} \otimes E_{kl} \big]
    &=
    ...
    &
      =
      \big( \Bc_{z,\zeta} \otimes \Id \big)\circ \Phi\Big[ E_{ij} \otimes E_{kl}\Big]
      .
  \end{align}
  We see that
  \begin{equation}
    ...
    \Bch_{z,\zeta}
    =
    \Phi \circ \big( \Bc_{z,\zeta} \otimes \Id \big)\circ \Phi
    ,
  \end{equation}
  which means that the invertibility of $\Bch_{z,\zeta}$ is equivalent to the invertibility of $\Bc_{z,\zeta}$.
  By Lemma~\ref{lem:4}, the operator $\Bch_{z,\zeta}$ is invertible for $(z,\zeta) \in \Omega \times \Omega$ and
  \begin{equation}
    \label{eq:243}
    \Bch_{z,\zeta}^{-1}
    =
    \Phi \circ \big( \Bc_{z,\zeta}^{-1} \otimes \Id \big)\circ \Phi
    .
  \end{equation}
  ...
  \begin{equation}
    ...
    =
    \Phi \circ \big( \Bc_{z,\zeta}^{-1} \otimes \Id \big)\circ \Phi \Big[I_n \otimes I_n\Big] +       \Phi \circ \big( \Bc_{z,\zeta}^{-1} \otimes \Id \big)\circ \Phi \Big[\widetilde{\errE} (z,\zeta)\Big]
    .
  \end{equation}
...
\begin{equation}
  ...
  \widehat{\Qc}
  =
  \Phi \circ \big(\Qc \otimes \Id_n \big) \circ \Phi
  ,
\end{equation}
...
\begin{equation}
  ...
  =
  \Phi \circ \big((\Id_n - \Qc) \otimes \Id_n \big) \circ \Phi
  ,
\end{equation}
...
\begin{equation}
  ...
  =
  \Phi \circ \big( \Jc_{z,\zeta}\otimes \Id_n \big) \circ \Phi
  .
\end{equation}
...
\begin{align}
  ...
  \frac{1}{N}\sum_{k,l = 1}^{N} G_{l k}(z) \otimes G_{kl}(\zeta)
    =
    \widehat{\Qc}\,\Big[ \frac{1}{N}\sum_{k,l = 1}^{N} G_{l k}(z) \otimes G_{kl}(\zeta) \Big] + \Phi \circ \big( \Jc_{z,\zeta}\otimes \Id_n \big) \circ \Phi \,\Big[ I_n \otimes I_n + \widetilde{\errE} (z,\zeta) \Big]
    .
\end{align}
...
\begin{equation}
  ...
  =
   \frac{1}{N}\sum_{k,l = 1}^{N} \varphi^{-1}\bigg(\widehat{\Qc}\,\Big[ G_{l k}(z) \otimes G_{kl}(\zeta) \Big] \varphi\big(B \big) \bigg)
  .
\end{equation}
Denote $M_0 := \lim_{y \downarrow 0} M(E_0 + \imu y)$ for brevity, and denote by $\{\ell_{k}, 1 \leq k \leq n \}$ the collection of (non-normalized) eigenvectors of $\Im M_0$, so that
  \begin{equation}
    \label{eq:250}
    \Im M_0
    =
    \sum_{k = 1}^{n} \ell_k \ell_k^{t}
    .
  \end{equation}
  Here we used that $   \Im M_0 $ is real symmetric in the case $\beta=1$. This follows from $\Gamma[R]^t = \Gamma[R^t]$ for any $R \in \CC^{n \times n}$ and therefore $M(z) = M(z)^t$, because $M(z)^t$ also  satisfies the  Dyson equation. 
  For any $S = (s_{ij})\in \CC^{n\times n}$ and $T = (t_{\alpha \beta})\in \CC^{n\times n}$ we have
\begin{align}
  \widehat{\Qc}\,\Big[ S \otimes T \Big]
  & =
    \sum_{i,j,\alpha,\beta = 1}^{n} s_{ij} t_{\alpha \beta} \, \Phi \circ \big(\Qc \otimes \Id_n \big) \circ \Phi\,\Big[ E_{ij} \otimes E_{\alpha \beta} \Big]
  \\
  & =
    \sum_{i,j,\alpha,\beta = 1}^{n} s_{ij} t_{\alpha \beta} \, \Phi \circ \big(\Qc \otimes \Id_n \big)\,\Big[ E_{i \beta} \otimes E_{\alpha j} \Big]
  \\
  & =
  ...
  \\
  & =
   \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \sum_{i,j,\alpha,\beta,m} s_{ij} t_{\alpha \beta} \,  \Big(\ell_m e_j^{t}\otimes e_{\alpha} e_{\beta}^t\Im M_0 \, e_{i}  \, \ell_m^{t}  \Big)
  \\
  & =
   \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \sum_{i,j,m} s_{ij}  \,  \Big(\ell_m e_j^{t}\otimes T \Im M_0 \, e_{i}  \, \ell_m^{t}  \Big)
    .
\end{align}
...
\begin{align}
  ...
  & =
    \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \sum_{i,j,m = 1}^{n}  s_{ij}  \, \varphi^{-1}\bigg(  \,  \Big(\ell_m e_j^{t}\otimes T \Im M_0 e_{i}  \, \ell_m^{t}  \Big) \varphi\big(B \big) \bigg)
  \\
  & =
    \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \sum_{i,j,m = 1}^{n}  s_{ij}  \, \ell_m e_j^{t} B   \ell_m e_{i}^{t} \Im M_0 T^t
  \\
  & =
   \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \sum_{i,j,m = 1}^{n}  s_{ij}  \, \ell_m  \ell_m^{t} B^{t} e_j   e_{i}^{t} \Im M_0 T^t
  \\
  &=
    \frac{\Im M_0 \, B^t \, S^t \, \Im M_0 \,T^t}{\| \Im M_0 \|_{\textrm{HS}}^2} 
    ,
\end{align}
where in the second step we used the definition of $\varphi$ in \eqref{eq:229}.

Now we apply the above formula with $S = G_{lk}(z)$ and $T = G_{kl}(\zeta)$, and use that $(G_{kl})^{t} = G_{lk}$, to rewrite \eqref{eq:249} as
\begin{align}
  \label{eq:251}
  \varphi^{-1}\bigg(\widehat{\Qc}\,\Big[ \frac{1}{N}\sum_{k,l = 1}^{N} G_{l k}(z) \otimes G_{kl}(\zeta) \Big] \varphi\big(B \big) \bigg)
  & =
    \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \Im M_0 \,  B^{t} \frac{1}{N}\sum_{k,l = 1}^{N} G_{kl}(z) \Im M_0  G_{lk}(\zeta)
  \\
  & =
    \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \Im M_0 \,  B^{t} \,  G^{\Im M_0} (z,\zeta) 
  .
\end{align}
...
\begin{equation}
  ...
  \frac{1}{\| \Im M_0 \|_{\textrm{HS}}^2} \Im M_0 \,  B^{t} \,  G^{\Im M_0} (z,\zeta)
  =
  \vartheta \,\frac{2\imu }{z - \zeta } \frac{\Im M_0 B^{\,t} \Im M_{0} }{\la \Im M_{0} \ra}    + \errR^{B}(z,\zeta)
  ,
\end{equation}
...
\begin{lem}
  ...
  \begin{equation}
    ...
    \frac{d}{dt} \EE[\efr(t)]
    =
    - \frac{t}{\pi^2} \int_{\Omega \times \Omega}  \frac{\partial \ft(z)}{\partial \overline{z}}  \frac{\partial \ft(\zeta)}{\partial \overline{\zeta}} \frac{\partial}{\partial \zeta} \EE\Big[  \sum_{i,j=1}^{n}  \Tr \Big(E_{ji} \,S_{ij}(z,\zeta) + E_{ji}\ST_{ij}(z,\zeta) \Big)  \efr(t) \Big] \dzeta  \dz + \errT
  \end{equation}
  where $|\errT |\prec N^{-\tau}$,
  \begin{equation}
    \label{eq:254}
    S_{ij}(z,\zeta)
    :=
...
\begin{proof}
  ...
  Then  we apply the decomposition \eqref{eq:256} on the right-hand side of \eqref{eq:61}-\eqref{eq:62} and treat the part containing $\Scct$ as an additional error term.
  ...
  \begin{align}
    ...
    .
  \end{align}
  After using \eqref{eq:59} (with $\Scc^{(1)}$ instead of $\Scc$) and \eqref{eq:55} we obtain the same equation as in \eqref{eq:64} with an additional error term
  \begin{equation}
    \label{eq:259}
    \frac{1}{\Mb(z)}\EE\Big[\efr(t)\big(1-\EE\big)\big[\Gb(z)\big] \Big]
    =
    ...
    .
  \end{equation}
  We see that the operator $\Scct$ is not explicitly appearing in leading expressions of \eqref{eq:259} anymore and the terms have exactly the same form as in \eqref{eq:64}.
  ...
  \begin{align}
    ...
    \\
    & \qquad
      = \EE\Big[ \efr(t) \frac{1}{N} \sum_{i,j=1}^{n}\sum_{p,q=1}^{N} \Tr \Big((E_{ji}\otimes \Eb_{q p}) \Gb(z)  \frac{1}{\Mb(z)} \big((\Bcc_{z}^{*})^{-1}[\Ib_{nN} ]\big)^*  \big(\,\Gammat[E_{ij}]\otimes \Eb_{qp}\big) \,\Gb(\zeta) \Big)  \Big]
    \\
    & \qquad
      =
      \EE\Big[ \efr(t) \frac{1}{N} \sum_{i,j=1}^{n} \sum_{k,l = 1}^{N}  \Tr \Big(E_{ji} G_{l k}(z)  \frac{1}{M(z)} \Bc_{z}^{-1}[I_n ]   \Gammat [E_{ij} ]   G_{lk}(\zeta) \Big)  \Big]
      ,
  \end{align}
...
\begin{equation}
  ...
  \tilde{h}_1(z,\zeta)
    :=
    \EE\Big[  \sum_{i,j=1}^{n}  \Tr \Big(E_{ji} \, \Gt^{\BT_{ij}(z)}(z,\zeta) \Big)  \efr(t) \Big]
    .
  \end{equation}
  ...
  \begin{equation}
    ...
    .
  \end{equation}
  If, moreover, we have that $\gamma \in (0,1)$ and  $\tau \in (0, \min \{(1-\gamma)/7, \gamma/2 \})$, then
  \begin{equation}
    \label{eq:277}
    \big|\Vc^{\,(+,+)} \big| + \big|\Vc^{\,(-,-)} \big|
    \prec
  ...
  \begin{align}
    ...
    \\ \label{eq:281}
      \tilde{\phi}^{\,(+,-)}
    &=
      \frac{2\imu }{\la \Im M_{0} \ra}  \sum_{i,j=1}^{n} \Tr \Big(E_{ji} \Im M_{0} \, \Big(\frac{1}{M_0} M_0'  \, \Gammat [E_{ij} ] \Big)^{\, t} \Im M_{0}\Big)
      ,
    \\ \label{eq:282}
        \phi^{\,(-,+)}
    &=
...
It has been shown in \eqref{eq:198}-\eqref{eq:200} that
\begin{equation}
  \label{eq:284}
  \phi^{\,(+,-)}
    =
...
\begin{align}
  \label{eq:285}
  &\sum_{i,j=1}^{n}  \Tr \Big(E_{ji} \, \Im M_{0} \Big(\frac{1}{M_{0}} M'_{0} \, \Gammat [E_{ij} ] \Big)^{t} \Im M_{0}   \Big)
  \\
  &\qquad
    =
  ...
  & \qquad
    =
    \sum_{i,j=1}^{n} \sum_{\alpha = 1}^{d} \Big\la E_{ji} \Im M_0  L_{\alpha}E_{ji} L_{\alpha} \Big( \Im M_0  \frac{1}{M_0} M_0' \Big)^t  \Big\ra
  \\
  & \qquad \qquad \qquad\qquad \qquad \qquad
    + \Big\la E_{ji} \Im M_0 ( L_{\alpha})^{t}E_{ji} (L_{\alpha})^{t} \Big( \Im M_0  \frac{1}{M_0} M_0' \Big)^t  \Big\ra 
  \\
  & \qquad
    =
    \sum_{i,j=1}^{n} \sum_{\alpha = 1}^{d} \Big\la E_{ji} \Im M_0  L_{\alpha}E_{ji} \Big( \Im M_0  \frac{1}{M_0} M_0' (L_{\alpha} )^t \Big)^t  \Big\ra
  \\
  & \qquad \qquad \qquad\qquad \qquad \qquad
    + \Big\la E_{ji} \Im M_0 ( L_{\alpha})^{t}E_{ji} \Big( \Im M_0  \frac{1}{M_0} M_0' L_{\alpha} \Big)^t  \Big\ra 
  \\
  & \qquad
    =
...
\begin{pr}[Properties of the solution to the MDE]
  ...
  \item[(iv)] \textbf{Regularity of $M(z)$ and the density of states}:
    The continuous extension of $M(z)$ to  $\CC_{+}\cup \RR$ exists and satisfies
    \begin{equation}
      \label{eq:294}
      \|M(z_1) - M(z_2)\|
      \lesssim
      | z_1 - z_2|^{1/3}
    \end{equation}
    uniformly for all $z_1, z_2 \in \CC_{+}\cup \RR$.
...
By taking the imaginary part of the Dyson equation \eqref{eq:7} and multiplying it by $M^{*}$ from the left and $M$ from the right we get
 ...
 If we denote by $\mathrm{v}\in \RR^{n}$ the right Perron-Frobenius eigenvector of $T$, then after comparing $\mathrm{w}^{t} \mathrm{v}$ and $\mathrm{w}^{t} T \mathrm{v} = \|T \| \mathrm{w}^{t} \mathrm{v}$, and using \eqref{eq:300} we find that the spectral norm of $T$ satisfies the bound
 \begin{equation}
   \label{eq:301}
   \|T\|
   \lesssim
...
The bounds \eqref{eq:302} and \eqref{eq:303} imply that $M^*M \sim I_n$, from which we have
 ...
 \begin{equation}
   ...
   \rho(z)
 \end{equation}
 for all $1\leq j \leq n$, and from \eqref{eq:297} and \eqref{eq:302} we get
 \begin{equation}
   \label{eq:311}
   \Im M(z)
   \gtrsim
 ...
 \end{equation}
 uniformly on $\{z \in \CC_{+} \, : \, |z| \leq C\}$.
 To estimate $ \Im M(z)$ from above, we notice that since the dimension of the equation \eqref{eq:7} is fixed, we trivially have
 \begin{equation}
   \label{eq:312}
   \Gamma[\Im M(z)]
   \lesssim
...
Therefore, by taking again the imaginary part of the MDE \eqref{eq:7} we get
 \begin{equation}
   ...
   \Im M(z)
   =
   \Im z M^*(z)M(z) + M^*(z) \Gamma [\Im M(z)] M(z)
   \lesssim
   (\Im z + \rho(z)) M^*(z)M(z)
   .
 \end{equation}
 On the other hand, from the imaginary part of the Stieltjes transform representation \eqref{eq:9} we see that $\Im M(z) \gtrsim \Im z I_n$ in the regime $|z| \leq C$, which together with \eqref{eq:313} and \eqref{eq:303} gives
 \begin{equation}
   \label{eq:314}
   \Im M(z)
   \lesssim
...
Given the existence of the spectral gap for $\Fc$ in \eqref{eq:320}, the linear stability bound \eqref{eq:293} follows by repeating the argument presented in \cite[Section~4.2]{AjanErdoKrug19}.
 ...
 With \eqref{eq:303}, the lower bound in Eq. (4.45) in \cite{AjanErdoKrug19} becomes
 \begin{equation}
   \label{eq:323}
   1 - \| \Fc\|
   \gtrsim
   \mathrm{dist}(z, \mathrm{supp} \, \rho)^{2}
   ,
 \end{equation}
 and the last inequality in the proof of Lemma~4.4 in \cite{AjanErdoKrug19} gives
 \begin{equation}
   \label{eq:324}
   |1 - \la F , \Cc_{U}[F] \ra |
   \gtrsim
   ...
   ,
 \end{equation}
 which together with \eqref{eq:321} and \eqref{eq:322} establish \eqref{eq:293}.
...
\begin{equation}
  ...
  K_{m+1,s}^{\nabla}[\theta,\xib]
  :=
  m! \int_0^1 \dots \int_0^1\nabla_{\xib}(\mathbbm{1}_{s\le t_{m-1}}-\EE)\nabla_{\xib}(\mathbbm{1}_{t_{m-1}\le t_{m-2}}-\EE) \dots (\mathbbm{1}_{t_{2}\le t_{1}}-\EE)\nabla_{\xib}(\mathbbm{1}_{t_{1}\le 1}-\EE) \theta  dt_1\dots d t_{m-1}  
\end{equation}
...
\begin{proof}[Proof of Lemma~\ref{lem:b1}]
 For $\star \in \{1,2\}$ we have
  ...
  \begin{align}
    ...
    \\ \nonumber
    & \quad +
      \sum_{\sigma_{1},\sigma_{2}\in\{il,li\}}\int_{0}^{1}\int_{0}^{1}\nabla^{\sigma_{1}}_{\Wt_{\sigma_{1}}}(\mathbbm{1}_{s\le t_{1}}-\EE_{\Wbt}) \nabla^{\sigma_{2}}_{\Wt_{\sigma_{2}}}(\mathbbm{1}_{t_{1}\le 1}-\EE_{\Wbt}) \Wt_{il} d_{t_1}ds \,F_{\star,lj} (W_{il},W_{li})
    \\ \label{eq:335}
    & \quad +
  ...
  From the direct computation of the first term we see that
  \begin{align}
    \label{eq:337}
    \EE \bigg[\int_{0}^{1}\big(\nabla^{il}_{\Wt_{il}} + \nabla^{li}_{\Wt_{li}}\big)(\mathbbm{1}_{s\le 1}-\EE_{\Wbt}) \Wt_{il} ds \, F_{\star,lj} (W_{il},W_{li})\bigg]
    =
    \EE \bigg[ \Wt_{il} \big(\nabla^{il}_{\Wt_{il}} + \nabla^{li}_{\Wt_{li}}\big) \, F_{\star,lj} (W_{il},W_{li})\bigg]
    .
  \end{align}
  Taking the sum of the above expression for $l \in \{1,\ldots, N\}$ and using the independence of $\Wt_{il}$ and $\Wt_{ab}$ for $ab \notin \{il,li\}$ gives $\EE\big[ \big( \Wbt \nabla_{\Wbt}\Fcc_{\star}(\Wb) \big)_{ij} \big]$.
  Together with \eqref{eq:333} we recover the first term in \eqref{eq:41}.

  Similar computations for the second term give
  \begin{align}
    \nonumber 
    &\EE\bigg[\sum_{l=1}^{N} \sum_{\sigma_{1},\sigma_{2}\in\{il,li\}}\int_{0}^{1}\int_{0}^{1}\nabla^{\sigma_{1}}_{\Wt_{\sigma_{1}}}(\mathbbm{1}_{s\le t_{1}}-\EE_{\Wbt}) \nabla^{\sigma_{2}}_{\Wt_{\sigma_{2}}}(\mathbbm{1}_{t_{1}\le 1}-\EE_{\Wbt}) \Wt_{il} d_{t_1}ds \,F_{\star,lj} (W_{il},W_{li}) \bigg]
    \\ \label{eq:338}
  ...
  =
  (-1)^{k} \sum_{\tau \in S_k}G_{li_{\tau(1)}} V_{\tau(1)} G_{j_{\tau(2)} i_{\tau(2)}} V_{\tau(2)} \dots G_{j_{\tau(k-1)}i_{\tau(k)}}V_{\tau(k)} G_{j_{\tau(k)}j}\,,
\end{equation}
for any $V_{1},\ldots, V_{k}\in \CC^{n\times n}$ and double indices $\sigma_p =i_pj_p$. 
   ...
   \begin{equation}
     ...
     \left\{
       \begin{array}{ll}
         \displaystyle{O_{\prec}\Big( \frac{1}{N^{3/2}} \Big)}, & l=j\, , 
         \\[10pt]
         \displaystyle{O_{\prec}\bigg( \frac{1}{\sqrt{N |\Im z|}}\, \frac{1}{N^{3/2}} \bigg),} &
                                                                                l\neq j .
       \end{array}
       \right.
   \end{equation}
   ...
   To analyze the case $\Fcc_{2}(\Wb) = \Gb \efr(t)$, $F_{2,lj}(\Wb) = G_{lj}(z)\efr(t)$, we need to derive convenient formulas for the repeated directional derivatives of $\efr(t)$, similar to \eqref{eq:340}.
For this, we define 
\begin{equation}
  \label{eq:343}
\begin{split}
\Ic([\sigma,V]) &:= \sum_{\tau \in S_k}(-1)^k \frac{\imu t}{\pi} \int_\Omega \frac{\partial \tilde{f}(\zeta)}{\partial \overline{\zeta}}\Tr \Big[\sum_{l=1}^N G_{li_{\tau(1)}}(\zeta) V_{\tau(1)}G_{j_{\tau(1)}i_{\tau(2)}}(\zeta) V_{\tau(2)} \dots V_{\tau(k)}G_{j_{\tau(k)}l}(\zeta)\Big]
\\
&=
-\frac{(-1)^k}{k} \frac{\imu t}{\pi} \sum_{\tau \in S_k}\int_\Omega \frac{\partial \tilde{f}(\zeta)}{\partial \overline{\zeta}}\frac{\partial}{\partial \zeta}\Tr \Big[ G_{j_{\tau(k)}i_{\tau(1)}}(\zeta) V_{\tau(1)}G_{j_{\tau(1)}i_{\tau(2)}}(\zeta) V_{\tau(2)} \dots G_{j_{\tau(k-1)}i_{\tau(k)}}(\zeta)V_{\tau(k)}\Big]\,.
\end{split}
\end{equation}
...
\begin{equation}
  \label{eq:345}
\nabla^{\sigma_1}_{V_1} \efr(t) = \efr(t) \Ic\big([(\sigma_{1},V_{1})]\big)  
\end{equation}
...
\begin{equation}
  ...
  \left\{
    \begin{array}{ll}
      \displaystyle{O_{\prec}\Big( \frac{1}{N^{3/2}} \Big)}, & l=j\, ,
      \\[10pt]
      \displaystyle{O_{\prec}\bigg( \frac{N^{\tau/2}}{\sqrt{N \eta_{0}}}\, \frac{1}{N^{3/2}} \bigg)}, & l\neq j \, ,
    \end{array}
    \right.
\end{equation}
   ...
   \begin{equation}
     \label{eq:350}
     \Ic\big([(i_{2}j_{2},A)]\big) =  \frac{\imu t}{\pi} \int_{\Omega} \frac{\partial \ft(\zeta)}{\partial \overline{\zeta}} \frac{\partial}{\partial \zeta} \Tr \Big[ G_{j_{2}i_{2}}(\zeta) A \Big] \dzeta
     =
     \left\{
       \begin{array}{ll}
         O_{\prec}\Big(N^{\tau}|t|  \|A\|\Big),&\quad l=i,
         \\[10pt]
         \displaystyle{O_{\prec}\bigg(\frac{N^{3\tau/2}|t| \|A\|}{\sqrt{N\eta_0}}\bigg)},&\quad l\neq i,
       \end{array}
       \right.
   \end{equation}
    ...
    \begin{equation}
      ...
      \left\{
        \begin{array}{ll}
          \displaystyle{O_{\prec}\bigg(\frac{N^{\tau}|t|}{N^{3/2}}\bigg)},& l = i
          \\[10pt]
          \displaystyle{O_{\prec}\bigg(\frac{1}{N^{3/2}} \, \frac{N^{3\tau/2}|t|}{\sqrt{N\eta_{0}}} \bigg)}, & l \neq i \, .
        \end{array}
        \right.
      \end{equation}

      Finally, using \eqref{eq:346}, \eqref{eq:343} and applying Stoke's theorem again  we find that for all $\sigma_{1},\sigma_{2} \in \{il, li\}$
      \begin{equation}
        \label{eq:352}
        ...
        .
      \end{equation}
      Together with \eqref{eq:350}, \eqref{eq:346} and \eqref{eq:347} this gives the bound for the second order derivatives of $\efr(t)$
      \begin{align}
        \label{eq:353}
      ...
      where we also used that $\tau < 1-\gamma$ to absorb all three terms in \eqref{eq:353} into one.
        ...
        \left\{
          \begin{array}{ll}
            \displaystyle{O_{\prec}\bigg(\frac{N^{2\tau}(|t|+|t|^2)}{N^{3/2}} \bigg)}\, , & l = j \, ,
            \\[10pt]
            \displaystyle{O_{\prec}\bigg(\frac{N^{2\tau}(|t|+|t|^2)}{N^{3/2}} \, \frac{N^{\tau/2}}{\sqrt{N \eta_{0}}} \bigg)}\, , & l \neq j \, .
          \end{array}
          \right.
        \end{equation}
        ...
        Using the same argument as in the case $\star = 1$ to extend the bound \eqref{eq:356} to its expectation we conclude that
        \begin{equation}
          \label{eq:357}
...
It remains to estimate the last term in  \eqref{eq:335}.
With the local law bound $\|\Gb\|_{\max} \prec 1$ we have
\begin{equation}
  \label{eq:358}
  |\nabla^{\sigma_1}_{V_1} \dots \nabla^{\sigma_k}_{V_k}  \efr(t)|
  \prec
  ...
  .
\end{equation}
If we now use $\|W_{ab}\|\prec N^{-1/2}$ following from \eqref{eq:2}-\eqref{eq:4}, we see that the term in the remainder \eqref{eq:335} satisfies the bound
\begin{equation}
  \label{eq:359}
...
After plugging this bound into \eqref{eq:335}, taking the sum for $l\in \{1,\ldots, N\}$ in \eqref{eq:333} and using \eqref{eq:339} we get the following expansion formula
\begin{align}
\label{eq:360}
  \EE \Big[ \Wb \Fcc_{\star}(\Wb)\Big]
  =
