...
% \usepackage[colorlinks=red,linkcolor=red]{hyperref}
...
\title{\Pascal{}: A Learning-aided Cooperative Bandwidth Control Policy for Hierarchical Storage Systems}
...
\affiliation{%
  ...
  \streetaddress{Equal contribution}
  \city{Hefei}
  \state{Anhui}
  \country{China}}
...
% Hierarchical Storage System (HSS) is considered as an ideal model to meet the cost-performance demand. Recent research suggests that the bandwidth control has significant impact on performance. Few previous work achieves a good bandwidth control in HSS since it is hard to control so many bandwidth tasks simultaneously. Thus, we propose a novelty bandwidth control policy, named \Pascal{}, which cooperatively controls many bandwidth tasks and utilizes the learning techniques to improve the accuracy and robustness of control. Compared with three strong baselines, it shows that \Pascal{} can decrease 1.95X tail latency and 2X throughput jitter and keep high throughput.
