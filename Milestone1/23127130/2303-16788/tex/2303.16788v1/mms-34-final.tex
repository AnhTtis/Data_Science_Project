...
\and Setareh Taki\textsuperscript{\textdagger}\\\emailfont{staki2@illinois.edu}
}
...
\item A tight example of the Garg-Taki's and our algorithms, which shows the limits of this approach in obtaining a better bound of $\frac{3}{4}+O(1)$.  Interestingly, our example only utilizes identical valuations, for which MMS allocations are known to exist.
\end{itemize}
...
We now describe an algorithm for converting a fair division instance to a normalized instance.
...
We first reduce the problem of finding a $3/4$-MMS allocation to the special case
where the instance is Ordered, Normalized, and totally-$3/4$-Irreducible (ONI).
We then run an algorithm called $\bagFill$ to get a $3/4$-MMS allocation.

Our algorithm $\approxMMS$ (\cref{algo:gt})
is almost the same as that of Garg and Taki \shortcite{garg2021improved}.
The only difference is that, unlike them, we modify the instance to ensure that it is normalized.
...
\label{algo:gt}
\begin{algorithmic}[1]
\State $\Icalhat = \toOrdered(\normalize(\reduce_{\alpha}(\toOrdered(\Ical))))$
\State $\Ahat = \bagFill(\Icalhat, \alpha)$.
...
Let $\Icalhat \defeq \toOrdered(\normalize(\reduce_{3/4}(\toOrdered(\Ical))))$.
...
since the application of reductions rules preserves orderedness.
...
After this transformation, the rest of their algorithm is the same as ours.
...
Our algorithm, called $\bagFill(\Ical, \alpha)$, creates $n$ bags,
...
every agent gets a bag, and hence, $\bagFill$'s output is $3/4$-MMS.
...
The details can be found in \cref{sec:better}.
...
See \cref{sec:imp:fix-R4} for a proof.
...
to instances with dummy goods. See \cref{sec:imp:mods} for details.
...
and allocates the remaining goods using the algorithm $\bagFill$.
In \cref{sec:imp:analysis}, we show that when $\alpha \le \frac{3}{4} + \imp$,
...
(\cref{thm:B:n2-agents} in \cref{sec:better})
...
$S_4 \defeq \{1, 2n+1\}$.
Consider a fair division algorithm that either outputs $\bagFill(\Ical, \alpha)$ for some $\alpha$,
...
We also modify the algorithm $\reduce_{\alpha}$ to use the new $R_4(\alpha)$.

\subsection{Algorithm}
\label{sec:imp:analysis}

The high-level algorithm $\approxMMS$ (\cref{algo:gt}) is the same as that in \cref{sec:simple}.
The only changes are that we modify $\toOrdered$, $\normalize$, and $\reduce_{\alpha}$
as described in \cref{sec:imp:mods}.
$\bagFill$ discards all dummy goods and then proceeds as before.
