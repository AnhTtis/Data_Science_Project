...
\affiliation[a]{Department of Mathematical Sciences, University of Liverpool, L69 7ZL Liverpool,\\ United Kingdom}
...
%Email Address: a.sokolov@liverpool.ac.uk

%A. Ringwald\\
%Deutsches Elektronen-Synchrotron DESY, Notkestr. 85, 22607 Hamburg, Germany \\
%Email Address: andreas.ringwald@desy.de

%\end{affiliations}

\emailAdd{a.sokolov@liverpool.ac.uk}
...
\begin{eqnarray}\label{elect}
	\partial \wedge A = F + (n\! \cdot \! \partial)^{-1} (n\wedge j_m)^d \, , \\[6pt] \label{magnet}
...
Let us use use an antisymmetric representation for the kernel of the $(n\! \cdot \! \partial)^{-1}$ operator~\cite{Zwanziger:1968rs, PhysRevD.3.880}:
...
%\begin{eqnarray}
 %   &&\left\langle x \left| \, \exp \left\lbrace \frac{i}{2} \, \tau \left[ \left( \fsl{p} - ie\fsl{A} - ig\fsl{B} \right)^2 +  \gamma_5\, \fsl{\partial} a -a^2 -m^2 \right] \right\rbrace  \; - \; \exp \left\lbrace \frac{i}{2} \, \tau \left[ \fsl{p}^2 -m^2 \right] \right\rbrace \right| x \right\rangle \; = \nonumber \\
 %   && 
%\end{eqnarray}

%Remembering that the last expression is to be introduced into the functional integral under the exponent sign due to Eq.~\eqref{psitake},

%First, let us analyze how a constant axion field $a=\theta$ affects low energy classical dynamics of the charged particles. Our motivation is to see whether there exists an analogue of the Witten effect~\cite{Witten:1979ey} in the model under consideration, i.e. whether a constant axion field $\theta$ gives extra non-quantized electric or magnetic charges to the charged particles of the model. Assuming that the axion field is constant, one sees that the only change to the denominator of the integrand in the right-hand side of Eq.~\eqref{ratio} is an extra contribution to the mass $\mu$ of the heavy PQ-charged fermion. Such contribution is not unexpected since we defined the axion through linear decomposition of the PQ field, so that its value partly contributes to the radial mode of $\Phi$ that determines the mass of $\psi$, see Eq.~\eqref{1}. It is therefore clear that the $\theta$-dependence in the denominator of Eq.~\eqref{ratio} cannot directly contribute to changing the charges of particles. Moreover, the Witten effect should be linear in $\theta$. We are thus left with the following expression for the relevant part of the generating functional:
%\begin{eqnarray}
 %   \mathcal{Z}(\tilde{a}_{\mu},\tilde{b}_{\mu}) \; = \;  \mathcal{N} \int \prod_{ \mu} \mathcal{D} A_{\mu} \mathcal{D} B_{\mu} \exp \left\lbrace i \int d^4 x \, \left( \mathcal{L}_Z + \mathcal{L}_G + \mathcal{L}_{\theta} + j_e \!\cdot \! \tilde{a} + j_m \!\cdot \! \tilde{b} \right) \right\rbrace  \, ,
%\end{eqnarray}
%where
%\begin{eqnarray}
 %   &&\mathcal{L}_{\theta}\; = \; - \frac{y\theta}{\sqrt{2}}\, \text{tr}_{\gamma}\, \gamma_5\! \int\limits_{m}^{\infty} d\mu \, \left\langle x \left| \, \frac{1}{\fsl{D}^2 + \mu^2 + i \epsilon} \, \right| x \right\rangle \, = \nonumber \\[6pt]
%    && \qquad \qquad \qquad \qquad \qquad \qquad - \frac{y\theta}{\sqrt{2}}\, \text{tr}_{\gamma}\, \gamma_5\! \int\limits_{m}^{\infty} d\mu \, \int\limits_0^{\infty} ds\, \left\langle x \left| \exp \left\lbrace is \left( \fsl{D}^2 + \mu^2 + i \epsilon \right) \right\rbrace \right| x \right\rangle \, .
%\end{eqnarray}
