...
Let $(K,\val)$ be an algebraically closed valued field. A type-definable subgroup $H\subseteq G$ of an algebraic group $G$ over $K$ is said to be a connected generically stable group if there exists a generically stable type $p$, concentrated on $H$, satisfying that $gp=p$ for any $g\in H$ (so $p$ is the unique generic type of $H$). We recall that a global invariant type $p$ concentrated on $G$ is generically stable if for any open affine subvariety $V\subseteq G$, on which $p$ is concentrated and $L\succ K$, $\val(f(c))\in \Gamma_L$ for any $f\in L[V]$ and $c\models p|L$.
...
\item Assuming $\mathrm{char}(\bk_F)\neq 2,3$, there exists some $\gamma_\infty\in \Gamma_{F^{alg}}$ satisfying:
\begin{enumerate}
	\item  For any $\gamma_1\leq \gamma_2\leq \gamma_\infty$, there are smooth integral separated $\cO_{F^{alg}}$-group schemes $\cE_{\gamma_1},\, \cE_{\gamma_2}$ of finite type over $\cO_{F^{alg}}$ with integral fibers and $(\cE_{\gamma_1})_{F^{alg}}\cong(\cE_{\gamma_2})_{F^{alg}}\cong E_{F^{alg}}$, satisfying that $\cE_{\gamma_1}(\cO_{F^{alg}})\subseteq \cE_{\gamma_2}(\cO_{F^{alg}})$ and both are generically stable subgroups of $E(F^{alg})$.
	\item Conversely, for any integral group scheme $\cG$ of finite type $\cO_{F^{alg}}$ with integral special fiber and $\cG_{F^{alg}}\cong E_{F^{alg}}$, there exists some $\gamma\leq \gamma_\infty$ with $\cG(\cO_{F^{alg}})=\cE_\gamma(\cO_{F^{alg}})$.
\end{enumerate}
\end{enumerate}
\end{theorem*}
...
Let $(V,p)\in \aGVar[F]$.
If there exists a finite morphsim $f:V\to W$, with $W\subseteq  \mathbb{A}^n_F$ an open suvariety, satisfying that $f_*p=p_{\cO}^{\otimes n}$ and for $c\models p|F$, $(F(c)/F(f(c)),\val)$ is a henselian  extension then there exists an open subvariety $U\subseteq V$ such that $\Phi(U,p)$ is of finite type over $\cO_F$.
\end{proposition}
%\begin{remark}
...
Since $(F,\val)$ is a defectless valued field, by the generalized stability theorem \cite[Theorem 1.1]{defectFVK}, $(F(c),\val)$ is also defectless. So $F(c)/F(f(c))$ is finite defectless henselian, and hence by, e.g. \cite[Lemma 11.15]{kuhlmann} this extension has a separating basis (as a valued vector space). I.e., since $p$ is strictly based on $F$, $\Gamma_{F(c)}=\Gamma_F$, there are basis elements $g_1,\dots,g_d\in F(V)$, with $\val(g_i(c))=0$, such that for any $a_1,\dots,a_d\in F(f(c))$,
\[\val(\sum_i a_ig_i(c))=\min_i\{\val(a_i)\}.\]

Let $U\subseteq V$ be an open subvariety for which  $g_i\in F[U]$ for all $1\leq i\leq d$. 

Let $s\in F[U]^p$; by finiteness of $f:V\to W$ there exist $a_1,\dots,a_d\in F[f_1,\dots, f_n]_{h}$ such that $s=\sum_{i=1}^d a_ig_i$. Write each $a_i$ as $b_i/h^{n_i}$ for $b_i\in F[f_1,\dots, f_n]$. Each $b_i$ can be written as $\sum_{\nu\in I} t_{i,\nu} f^\nu$, with $t_{i,\nu}\in F$.

As $\val(s(c))\geq 0$, by the choice of the $g_i$ we get that $\val(b_i(c))=\val(b_i(c)/h(c)^{n_i})\geq 0$ for $i=1,\dots, d$. So $\val(t_{i,\nu})\geq 0$ for all the $t_{i,\nu}$'s.

As a conclusion, $1/h,\, g_i,\, f_j$, for $1\leq i\leq d,\, 1\leq j\leq n$ generate $F[U]^p$ as an $\cO_F$-algebra.
\end{proof}
...
By Noether normalization for projective varieties \cite[Corollary 2.29]{mumford}, there exists a linear subspace $\overline{L}\subseteq \mathbb{P}_{\bk_K}^m(\bk_K)$ of codimension $n+1$ disjoint from $\mathcal{V}_{\bk_K}(\bk_K)$. This linear subspace is defined by $n+1$ linearly independent vectors (forms) $\overline{v}_1,\dots, \overline{v_{n+1}}\in \bk_K^{m+1}$. Let $v_1,\dots,v_{n+1}\in \cO_K^{m+1}$ be lifts of these vectors. They are linearly independent over $K$ and thus define a linear subspace of $L\subseteq \mathbb{P}_K^m(K)$ of codimension $n+1$. By properness of $\cV$ over $\cO_K$, every $K$-point of $\cV$ is also an $\cO_K$-point, so $L$ is also disjoint from $\cV_K(K)$.
...
\end{lemma}
\begin{proof}
The result will follow once we show that $p^2=p$; indeed, for then $p$ is the unique generic type of $\mathrm{Stab}(p)$. The latter is definable since $p$ is strongly stable dominated. 


We show that $p^2=p$. By base-changing everything to a maximally complete ACVF containing $K$ and since the conclusion descends, there is no harm in assuming $K$ is maximally complete.
...
x_3=\frac{(Ax_2+2B)\cdot 1+(x_2^2+A)\cdot x_1+(-2y_2)\cdot y_1+x_2\cdot x_1^2}{x_2^2-2x_2x_1+x_1^2}.
\end{equation}
...
\begin{notation}
For any type $q$ concentrated on $V$ let $q_x$ be the type $\pi_*q$, where $\pi:V\to K$ is the projection on the first coordinate. 
\end{notation}
...
\begin{lemma}\label{L:existence of p_a}
	...
	For any gracious valued field $L\supseteq F$ and $d=(d_x,x_y)\models (p_a)|L$, \[\left\{\left(\frac{d_x}{a}\right)^i\cdot \left(\frac{d_y}{a^{3/2}}\right)^j\right\}_{i\geq 0,\, j=0,1}\] constitutes a separating  basis of $L[\frac{d_x}{a},\frac{d_y}{a^{3/2}}]$ over $L$.
	
	As a result, $p_a$ satisfies $(\dagger)$ of Corollary \ref{C: dagger implies defectlesshenselian} and $p_a^{\otimes n}$ is strongly based on $F$, for any $n\geq 1$.
\end{lemma}
\begin{remark}
Naturally, $a^{3/2}$ is not well-defined for an element $a\in K^\times$. However, any choice of square-root would work here. 
\end{remark}
\begin{proof}
We apply Lemma \ref{L:mmp in elliptic} to the Weierstrass equation $y^2=x^3+\frac{A}{a^2}x+\frac{B}{a^3}$; note that by the assumptions, $\val\left(\frac{A}{a^2}\right)\geq 0$ and $\val\left(\frac{B}{a^3}\right)\geq 0$ and that its discriminant is non-zero. Hence, its projective closure defines an elliptic curve. Let $\widehat p_a$ the unique generically stable $L$-definable type we are supplied with and let $p_a$ be type of $(a\widehat d_x,a^{3/2}\widehat d_y)$ over $\mathbb{K}$, for $\widehat d=(\widehat d_x,\widehat d_y)\models \widehat p_a$.
...
The following lemma gives that every generic of a connected generically stable definable subgroup of $E$ of the form $p_\gamma$ for some $\gamma$.
...
If $q$ is $F$-definale and strictly based on $F$ then $\val(d_x)\in \Gamma_F$ so we may choose $a\in F^\times$ with $q=p_a$./
\end{proof}
...
We now assume that $\gamma_1=\val(a_1)<\val(a_2)=\gamma_2$. Since $E$ is Abelian, and by using the above, $(p_{a_1}p_{a_2})^2=p_{a_1}p_{a_2}$. By Lemma \ref{L: classifying generics} there is some $a_3\in K^\times$ with $p_{a_1}p_{a_2}=p_{a_3}$. To show that $p_{a_3}=p_{a_2}$, we will show that $\val(a_3)=\val(a_2)$, for then $(p_{a_1}p_{a_2})_x=(p_{a_2})_x$.
...
Let $E$ be an elliptic curve over $F$. There exists a  smooth integral separated $\cO_F$-group scheme $\cE$ of finite type over $\cO_F$ with geometrically integral fibers, satisfying $E\cong \cE_F$ such that under this isomorphism $\mathrm{Stab}(p)=\cE(\cO)$, for which the following are equivalent:
\begin{enumerate}
...
\begin{enumerate}
\item For any $\gamma_1,\gamma_2\in \Gamma_F$ with $\gamma_1\leq \gamma_2\leq \gamma_\infty$, we have $\cE_{\gamma_1}(\cO)\subseteq \cE_{\gamma_2}(\cO)$.
\item Applying the above for $F=K$, for any integral group scheme $\cG$ of finite type over $\cO_K$ with integral special fiber and satisfying $\cG_K\cong E_K$, we have $\cG(\cO)=\cE_{\gamma}(\cO)$ for some $\gamma\in \Gamma_K$ with $\gamma\leq \gamma_\infty$. 
\end{enumerate}
\end{theorem}
\begin{proof}
...
(1) By Lemma \ref{L: product of generics}, if $\gamma_1\leq\gamma_2\leq \gamma_\infty$ then $\cE_{\gamma_1}(\cO)=\mathrm{Stab}(p_{\gamma_1})\subseteq \mathrm{Stab}(p_{\gamma_2})=\mathcal{E}_{\gamma_2}(\cO)$.
