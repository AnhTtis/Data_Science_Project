...
\input{_icml2023/icml_stuff.tex}

\icmltitlerunning{~ \hfill \ourmethod: Conformal Prediction for Time Series with Modern Hopfield Networks \hfill \thepage}

\begin{document}

\twocolumn[
% Conformal Prediction for Time Series with Hopfield Networks
% Conformal Prediction for Time Series with Modern Hopfield Networks
% Conformal Time Series Prediction with Modern Hopfield Networks
\icmltitle{Conformal Prediction for Time Series with Modern Hopfield Networks}
% {Time Series Conformal Prediction with Modern Hopfield Networks
\icmlsetsymbol{equal}{*}

\begin{icmlauthorlist}
\icmlauthor{Andreas Auer}{iml}
\icmlauthor{Martin Gauch}{iml,google}
\icmlauthor{Daniel Klotz}{iml}
\icmlauthor{Sepp Hochreiter}{iml,iarai}
\end{icmlauthorlist}

\icmlaffiliation{iml}{ELLIS Unit Linz and LIT AI Lab, Institute for Machine Learning, Johannes Kepler University, Linz, Austria}
\icmlaffiliation{google}{Google Research, Linz, Austria}
\icmlaffiliation{iarai}{Institute of Advanced Research in Artificial Intelligence (IARAI), Vienna, Austria}


\icmlcorrespondingauthor{Andreas Auer}{auer@ml.jku.at}

% You may provide any keywords that you
% find helpful for describing your paper; these are used to populate
% the "keywords" metadata in the PDF but will not be shown in the document
\icmlkeywords{Machine Learning, ICML, Conformal Prediction, Time Series, Hopfield, Modern Hopfield Networks}
...
%However, conformal prediction for time series remains 
%challenging as the autocorrelative structure of time series 
%violates basic assumptions required by conformal prediction.
%We propose a novel conformal prediction approach for time series
...
%In experiments, 
%we demonstrate on simulated and real-world data that our new approach
...
%Uncertainty estimates are imperative to make actionable predictions for complex time-dependent systems \citep[e.g.,][]{gneiting2014probforecasting, kendall2017uncertainties, zhu2017deep}. 
%This is particularly evident for phenomena with environmental aspects --- such as energy demand/supply estimation \citep[e.g.,][]{wang2019probabilistic}, weather forecasting \citep[e.g.,][]{murphy2019probability}, or flood prediction \citep[e.g.,][]{krzysztofowicz2001case} --- since the resulting time series usually exhibit pronounced seasonality.
%Conformal Prediction \citep[CP,][]{vovk1999machine} is a framework that provides uncertainty estimates based on prediction intervals. 
%CP achieves marginal coverage with almost no distributional assumptions except that the data are exchangeable \citep{vovk2005algorithmic, vovk2012conditional}. 
%The application of CP to time series is not straightforward because the autocorrelative structure generally inhibits exchangeability.
...
CP achieves finite-sample marginal coverage with almost no distributional assumptions, except that the data is exchangeable \citep{vovk2005algorithmic, vovk2012conditional}. 
However, CP for time series is not trivial because temporal dependencies generally violate the exchangeability assumption.

\begin{figure}[t]
\includegraphics[width=5.05cm]{graphics/tscp-figure1.pdf}
\centering
\caption{Schematic illustration of \ourmethod. The Modern Hopfield Network (MHN) identifies regimes similar to the current one in the time series and weights them (indicated by the colored lines). The weighted information enriches the conformal prediction procedure so that prediction intervals can be derived.}
\label{fig:figure1}
\end{figure}

\paragraph{\ourmethod.} 
Time series models often exhibit dynamical errors with locally specific behavior. 
\ourmethod uses continuous Modern Hopfield Networks (MHNs) to learn a weighting of similar situations for CP.
The CP procedure is then able to use these situations to produce strong uncertainty estimates that exhibit approximately the desired coverage level, and achieve new state-of-the-art efficiency, even with non-exchangeable data.
We designed our method for being applicable to large datasets. 

\paragraph{Our main contributions are:}
\begin{enumerate}
    \item We propose \ourmethod, a CP approach designed for time series prediction, a domain where CP methods struggle with non-exchangeable data.
    \item \ourmethod uses MHN to introduce the novel technique of similarity-based sample reweighting for time series CP.
    In contrast to existing approaches, \ourmethod can learn from larger datasets and predict intervals at arbitrary coverage levels without retraining.
...
%    that it is valid for non-exchangeable data.
    ...
    \item We are the first to provide formal guarantees for uncertainty estimation in  streamflow prediction --- a domain where uncertainty plays a key role in tasks such as flood forecasting and hydropower management.
\end{enumerate}
...
was already observed at previous time points. 
The error at these time points is therefore assumed to 
have the same distribution as the current error.
Following \citet{quandt1958estimation} and \citet{hamilton1990analysis}, 
we call the sets of time points with similar 
environmental condition \textit{regimes}.
...
For more details, we refer to Appendix~\ref{sec:appendix-hopfield}.

\subsection{Setting}\label{sec:setting}

Our setting consists of a multivariate time series $\{(\Bx_t, y_t)\}$, $t=1,\dotsc,T$, with a feature vector $\Bx_t \in \dR^m$, a target variable $y_t \in \dR$, and a given black-box prediction model $\mu: \dR^m \mapsto \dR$ that generates a point prediction $\hat{y}_t = \mu(\BX_t)$.
The input feature matrix $\BX_{t+1}$ can include all previous and current features $\{\Bx_i\}^{t+1}_{i=1}$, as well as all previous targets $\{y_i\}^{t}_{i=1}$.
Given the features $\BZ_{t+1}$, our goal is to construct a corresponding prediction interval $\hat{C}_{\alpha}(\BZ_{t+1})$ --- a set that includes $y_{t+1}$ at the specified probability $1 - \alpha$.
...
\begin{equation}
\PR\left\{Y_{t+1} \in \hat{C}_{\alpha}\left(\BZ_{t+1}\right)\right\} \geq 1-\alpha,
\label{eq:cp_prop}
\end{equation} 
%Sepp: if \PR is a functional (then capital roman) or 
% where random variables come with a distribution and
...
Thus, CP aims to minimize the width of the prediction interval $\hat{C}_{\alpha}$, while preserving the coverage.
A smaller prediction interval is called a more \textit{efficient} interval \citep{vovk2005algorithmic}.
We use the mean of the interval width over the prediction period (\textit{PI-Width}) as a metric to evaluate the efficiency.

Standard split conformal prediction takes a calibration set of size $n$ which has not been used to train the prediction model $\mu$.
For each data sample, it calculates the so-called conformal score \citep{vovk2005algorithmic}.
...
The prediction interval is then calculated based on the empirical $1-\alpha$ quantile $\mathcal{Q}_{1-\alpha}$ of the calibration scores:
\vspace{10pt}
\begin{equation}
    \hat{C}_{\alpha}(\BZ_{t+1}) = \mu(\BX_{t+1}) \pm \mathcal{Q}_{1-\alpha} (\{|y_i - \mu(\BX_i)|\}_{i=1}^n).
\end{equation}

If the data is exchangeable and $\mu$ treats the data points symmetrically, the errors on the test set follow the distribution from the calibration.
...
If the specified \textit{miscoverage} $\alpha$ differs from the actual marginal miscoverage $\alpha^\star$ in the evaluation, we denote the difference as the \textit{coverage gap}

\begin{equation}
    \Delta \ \text{Cov} = \alpha - \alpha^\star.
\end{equation}
...
\ourmethod combines the conformal-style quantile selection of errors with a learned similarity-based retrieval using MHN.
...
%
\citet{barber2022conformal} introduced CP with weighted quantiles.
...
\begin{equation}\label{eq:cov-bound-interval}
    \hat{C}_{\alpha}(\BX_{t+1}) = \mu(\BX_{t+1}) \pm \mathcal{Q}_{1-\alpha} \left(\sum_{i=1}^t a_i \delta_{\epsilon_i} + a_{t+1} \delta_{+\infty} \right),
\end{equation}
...
Then, $\Delta \ \text{Cov}$ can be bounded from below by the weighted sum of the total variation distances $d_\text{TV}$ between these permutations in the following way \citep{barber2022conformal}: 
\begin{equation}\label{eq:cov-bound}
    \Delta \ \text{Cov} \geq - \sum_{i=1}^t a_i \cdot d_\text{TV}(D,D^i)
\end{equation}
...
Note that lower values for $\omega_i$ would lead to a tighter bound for $\Delta \ \text{Cov}$ (Equations~\ref{eq:weighted-a} and \ref{eq:cov-bound}) but also to an increase in the interval width (Equation~\ref{eq:cov-bound-interval}).
% Case 1 - Mix of "alternating" regimes -> high distance if exchanged with "wrong" regime, low distance if exchanged with "right" regime
% Case 2 - Distribution shift over time (= switch from one to other regimes) -> high distance if exchange with "old" points, low distance if exchanged with "new" points
In a similar but more flexible fashion, \ourmethod does not fix the weights a priori.
The MHN association resembles direct estimates of $a_i$ --- dynamically assigning high values to samples from similar regimes.
...
The CP component of \ourmethod follows \citet{xu2021conformal}.
Like them, we use individual quantiles for the upper and lower bound of the prediction interval, calculated from the errors themselves.
\ourmethod computes the prediction interval $\hat{C}_{\alpha}$ for time step $t+1$ in the following way:
\vspace{10pt}
\begin{equation}\label{eq:myconfbounds}
\begin{aligned}
\hat{C}_{\alpha}(\BZ_{t+1}) = %
   \Bigl[ & \mu(\BX_{t+1}) + q(\frac{\alpha}{2}, \BZ_{t+1}), \\
     & \mu(\BX_{t+1}) + q(1-\frac{\alpha}{2}, \BZ_{t+1}) \Bigr],
\end{aligned}
\end{equation}

where $q(\tau, \BZ_{t+1}) = \mathcal{Q}_{\tau} \left(E_{t+1} \right)$ and $E_{t+1}$ is a multiset created by drawing $n$ times from $[\epsilon_i]_{i=1}^t$ with corresponding probabilities $[a_{t+1,i}]_{i=1}^t$.
Standard CP excludes the highest absolute errors from the set that defines the prediction interval \citep{vovk2005algorithmic}; Equation~\ref{eq:myconfbounds} instead removes the lowest and highest non-absolute errors from the prediction interval.
This can produce more efficient prediction intervals when $\mathbb{E}(\epsilon_i) \neq 0$ in certain error distribution regimes.

\subsection{Encoding Network}\label{sec:encoding-network}

We embed the raw time series features using a 2-layer fully connected network $m^L$ with ReLU activations and enhance the representation by temporal encoding features $z_{T,t}^{\text{time}}$.
...
However, training MHN with quantiles is difficult, which is why we use an auxiliary task:
...
\begin{equation}\label{eq:train-loss}
    \mathcal{L} = \frac{1}{T} * \lVert(|\bm{\epsilon}_{1:T}| - \BA_{1:T,1:T}  |\bm{\epsilon}_{1:T}|)^2\rVert_1.
\end{equation}
...
Section~\ref{sec:results-discuss} and Appendix~\opoint{\ref{sec:appendix-full-tables}} provide evidence that this holds empirically.
...
The following synthetic example illustrates the advantages of the \ourmethod association mechanism for CP.
...
$x_t$ is constant within a regime (that is, $x=3$ and $x=21$).
...
We use a ridge regression model as prediction model $\mu$.
...
\spic and \nexcp cannot properly select the relevant time steps, either.
...
\begin{figure}[t]
\includegraphics[width=8.0cm]{graphics/toy_result_facet.pdf}
\centering
\caption{Different approaches for our synthetic example. \ourmethod has the smallest width of the prediction intervals (PI-width), while maintaining a coverage close to the specified one (i.e., a $\Delta \ \text{Cov}$ that is positive and close to zero).}
\label{fig:toy-example}
\end{figure}


\section{Experiments}\label{sec:experiments} 
This section provides a comparative evaluation of \ourmethod and a qualitative analysis of its association mechanism.
...
The smallest one consists of 8 time series, each from a different location, over a period of 84 days.
...
a random forest, a LightGBM, a ridge regression model, and a Long Short-Term Memory (LSTM) model.
...
For the streamflow dataset, we deviate from this scheme and instead only use the state-of-the-art model, which is an LSTM network \citep[][see Appendix~\ref{sec:dataset-details}]{Hochreiter:97, kratzert2020note}.

\setTableCaption{Performance of the evaluated CP algorithms for the \solarThreeY, \airTen, \sapflux, and \hydro datasets. The specified miscoverage level is $\alpha=0.1$ for all experiments. The column FC specifies the prediction algorithm used for the experiment (Forest: Random Forest, LGBM: LightGBM, Ridge: Ridge Regression, LSTM: LSTM neural network). Bold numbers correspond to the best result for the respective metric in the experiment (PI-Width and Winkler score). The error term represents the standard deviation over repeated runs with different seeds (results without an error term are from deterministic models).}
\input{results_tables_new/main_results_alpha_0.1.tex}
...
Appendix~\ref{sec:hyperparam} describes the hyperparameter search that we conducted for each method. 
For \spic, an adaption of the original algorithm was necessary to provide scalability to larger datasets.
Appendix~\ref{sec:spic-retrained} provides more details and an empirical justification.
In Appendix~\ref{sec:adaptive-cp} we additionally evaluate the addition of \adaptiveci \citep{gibbs2021adaptive} as an enhancement to \ourmethod and the other time series CP methods.
...
\paragraph{Metrics.} In our analyses, we compute $\Delta \ \text{Cov}$, PI-Width (Section~\ref{sec:setting}), and the Winkler score \citep{winkler1972decision} per time series and miscoverage level.
The Winkler score jointly elicitates miscoverage and interval width in a single metric:
\begin{multline}\label{eq:winkler-score}
    \text{WS}_{\alpha}(\BZ_t, y_t) = \\
    \begin{cases}
        \text{IW}_{\alpha}(\BZ_t) + \frac{2}{\alpha} (\hat{C}_{\alpha}^l(\BZ_t) - y_t) & \text{if} \ y_t <  \hat{C}_{\alpha}^l(\BZ_t),\\
        \text{IW}_{\alpha}(\BZ_t) + \frac{2}{\alpha} (y_t - \hat{C}_{\alpha}^u(\BZ_t)) & \text{if} \ y_t > \hat{C}_{\alpha}^u(\BZ_t),\\
        \text{IW}_{\alpha}(\BZ_t) &  \text{else}.
    \end{cases}
\end{multline}

The score is calculated per time step $t$ and miscoverage level $\alpha$.
It corresponds to the interval width $\text{IW}_{\alpha}=\hat{C}_{\alpha}^u - \hat{C}_{\alpha}^l$ whenever the observed value $y_t$ is between the upper bound $\hat{C}_{\alpha}^u(\BZ_t)$ and the lower bound $\hat{C}_{\alpha}^l(\BZ_t)$ of $\hat{C}_{\alpha}(\BZ_t)$.
If $y_t$ is outside these bounds, a penalty is added to the interval width.
...
Notably, these results reflect the increasing requirements posed by each method on the data (Section~\ref{sec:related-work}).

Further, \ourmethod outperforms the other methods regarding both Winkler score and PI-Width when we evaluate over additional datasets and at different miscoverage levels (see Appendix~\ref{sec:appendix-full-tables}).
\ourmethod is the best-performing approach in all cases, except for the smallest of all datasets.
The limited amount of training data appears to hinder the MHN from learning generalizable retrieval patterns.
We argue that this is not a limitation of similarity-based CP but rather an artifact of dataset size.
In fact, a simplified and almost parameter-free variation of \ourmethod, which replaces the MHN with \knn retrieval, performs best on this small dataset, as we show in Appendix~\ref{sec:knn-comparison}.

All approaches report a delta coverage close to zero --- in other words, they approximately achieve the specified marginal coverage level.
...
Performance gaps between the approaches differ considerably across datasets, while they are generally consistent across prediction models.
The biggest differences between the best and worst methods exist in \solarThreeY and \sapflux, likely due to the strongly distinctive regimes in these datasets.
The smallest (but still significant) differences are visible in the \hydro data.  
On this dataset, we also evaluated the state-of-the-art non-CP uncertainty model in the domain \citep[][see Appendix~\ref{sec:appendix-full-tables}]{klotz2022uncertainty} and found that \ourmethod outperforms it with respect to both PI-Width and Winkler score.   


\begin{figure*}[t]
\includegraphics[width=\textwidth]{graphics/association.pdf}
\centering
\caption{Exemplary visualization of the 30 highest association weights that the MHN places on previous time steps. \ourmethod retrieves similar peak values when estimating at a peak, and it retrieves similar small values when estimating at a small value.}
...
\input{other_tables/hyperparameter.tex}
...
We chose this number so that the loss and model selection metric curves are already converged.
...
AdamW \citep{loshchilov2019adamw} with standard parameters ($\beta_1=0.9$, $\beta_2=0.999$, $\delta=0.01$) is used as optimizer.
...
Since \spic applies a random forest on top of the window, one can assume that it is capable to find the relevant parts of the window.
...
To check our reasoning, we evaluated the performance of \spic with window length 25 on all but the largest two datasets (Table~\ref{tab:spic-length-comparsion}).
...
\setTableCaption{Performance of \spic with an input window  of length 100 and length 25.
...
\input{results_tables/spic_length_comparsion.tex}
...
To nevertheless allow a comparison against \spic on the larger dataset, we modified the algorithm so that the retraining is skipped.
...
In the experiments (Section~\ref{sec:experiments} and Appendix~\ref{sec:appendix-full-tables}), standard CP achieves good coverage with only one exception (we look at this exception in detail in Section~\ref{sec:experiments}).
...
Despite the fact that CMAL is not based on the CP paradigm, it achieves good coverage results, however, at the cost of wide prediction intervals and high Winkler scores (see Table~\ref{tab:hydro-531-basin-list-overall-v2}).

\begin{figure}[t]
\includegraphics[width=8cm]{graphics/eps_drift.pdf}
\centering
...
While developing our approach we tested several configurations and adaptions. In the following, we briefly describe those that did not work (so that potential future research can avoid these paths):
...
The runtime differs greatly between different dataset sizes. We report the approximate run times for a single experiment (i.e., one seed, all evaluated coverage levels, not including training the prediction model $\mu$):
\vspace{-10pt}
\begin{enumerate}
    \itemsep0em 
...
The details are summarized in the following paragraphs.
A quantitative overview is given in Table~\ref{tab:dataset-summary}.
...
The datasets provide two target variables (10PM and 25PM measurements) which we use separately in our experiments (the variables are used mutually exclusively in the datasets, e.g., when predicting 10PM we do not use 25PM as a feature). 
...
\paragraph{Sap flow.}
The \sapflux dataset is a subset of the Sapflux \cite{poyatos2021global} data project.
...
(d) From the resulting set of sequences we remove all that have less than 15,000 or more than 20,000 time steps.

\paragraph{Streamflow.}
For our experiments on the streamflow data, we use the Catchment Attributes and Meteorology for Large-sample Studies (CAMELS) dataset \cite{newman2015development,addor2017camels}.
...
\begin{equation}
    q(\tau, \BZ_{t+1}) = \mathcal{Q}_{\tau} \left(\sum_{i=1}^t a_{t+1, i} \delta_{\epsilon_{i}} \right),
\end{equation}

where $\delta{\epsilon_{i}}$ is a point mass at $|\epsilon_i|$.
...
Overall, the results are slightly better, as the Winkler score (which considers both width and coverage) slightly increases in  most experiments.
...
%TK
The following arguments are adopted from \citet{furst2021cloob} and \citet{Ramsauer:21}.
...
The code and data to reproduce all of our experiments are available at \url{https://github.com/ml-jku/HopCPT}.
...
\input{results_tables_new/nsdb-60m-2018-20_overall_v2.tex}
...
\input{results_tables_new/nsdb-60m-2019_overall_v2.tex}
...
\input{results_tables_new/solar_overall_v2.tex}
...
\input{results_tables_new/air-10_overall_v2.tex}
...
\input{results_tables_new/air-25_overall_v2.tex}
...
\input{results_tables_new/sapflux-solo3-large_overall_v2.tex}
...
\input{results_tables_new/hydro-531_basin_list_overall_v2.tex}
