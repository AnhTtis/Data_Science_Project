...
\begin{definition}
Consider the discrete-time  dynamical system in \eqref{eqn:Pdelta}.  A function $V : B_{\rho}(x^*) \to \R_{\geq 0}$, for $B_{\rho}(x^*)$ as in \eqref{eqn:extendedpoincare}, is a \textbf{robust Lyapunov function} if:  
\begin{align}
\label{eqn:lyap1}
...
\begin{remark}
Note that the condition \eqref{eqn:lyapimplication} can be equivalently restated as: 
\begin{eqnarray}
...
This theorem is, overall, a variation on the Lemma 3.5 in \cite{jiang2001input}.  The proof here follows a similar overall arc, although there are key differences made necessary by the fact that $\P$ is only a partial function. 
