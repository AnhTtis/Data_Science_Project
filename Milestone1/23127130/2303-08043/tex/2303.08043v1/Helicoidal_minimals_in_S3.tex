...
\thanks{Research partially supported by Project PID2020-117868GB-I00 and Maria de Maeztu Excellence Unit IMAG
...
Minimal surfaces locally minimize area, that is, they are critical points of the area functional
and are characterized as surfaces whose mean curvature vanishes everywhere.
%by the vanishing of their mean curvature.
...
%In 1741, Euler discovered that when a catenary $x = \cosh z$ is rotated around the $z$-axis, one obtains a surface, called the {\em catenoid}, which minimizes area among surfaces of revolution after prescribing boundary values for the generating curves.
...
%Together with the plane, the catenoid is the only minimal surface of revolution (Bonnet, 1860).
...
It is pointed out for minimal surfaces in Euclidean $3$-space the beautiful property of existence of a $1$-parameter family of minimal isometric surfaces (the so called {\em associated} surfaces) connecting the catenoid and the helicoid, which are said to be {\em conjugate} each other. The associated surfaces  are precisely 
the minimal helicoidal surfaces studied by Scherk in 1834.
%
%Looking at their Weierstrass representation formula, they vary depending on their Bonnet angle and represent, in some way, a complex rotation of the classical (right) helicoid.
Viewing this family through their Weierstrass representation formula, one may see that they vary depending on their Bonnet angle and represent, in some way, a complex rotation of the classical (right) helicoid.
They are also Bj\"{o}rling surfaces associated to a circular helix. For the catenoid, the helix becomes a circle and in the case of the helicoid, it becomes the axis (cf.\ \cite{F93}).

The first aim of this paper is to analyze in depth the spherical version of the possible existence of an isometric family of minimal surfaces connecting the catenoid to the helicoid. To do so, we will first have to determine which surfaces play the role of helicoids and catenoids in the 3-sphere.

%\underline{About \textit{CLOSED MINIMAL} surfaces in $\s^3$ (survey of Brendle):}

Differently as happen in $\R^3$, %there do exist
interesting examples of {\em closed} minimal surfaces exist in $\s^3= \{ (x_1,x_2,x_3,x_4)\in \R^4 : x_1^2+x_2^2+x_3^2+x_4^2=1 \}$, which are characterized by remarkable uniqueness theorems (see e.g.\ \cite{Br13b} and references therein): 
The simplest is the totally geodesic {\em equatorial 2-sphere}, 
whose principal curvatures are both equal to zero.
%
Using Hopf differential arguments, Almgren proved in 1966 that it is the only immersed minimal surface of genus $0$ in $\s^3$.
%
Another important example of a closed minimal surface in $\s^3$ is the {\em Clifford torus}, given by 
$x_1^2+x_2^2=x_3^2+x_4^2=1/2$, whose principal curvatures are $1$ and $-1$.
Brendle proved in \cite{Br13a} that %the Clifford torus
...
Brendle technique was pioneered by Huisken and involves an application of the maximum principle to a two-point function.
In addition, we remark the following local rigidity result (\cite{La69}) concerning these two main examples: the equatorial $2$-sphere and the Clifford torus are the only minimal surfaces of constant Gauss curvature ($1$ and $0$, respectively) in $\s^3$. Moreover, these are the only minimal surfaces of cohomogeneity zero on $\s^3$.

In 1970 Lawson constructed in \cite[Theorem 3]{La70} an explicit infinite family $\tau_{m,k}$, with $m,k$ positive integers such that $(m,k)=1$, of immersed minimal tori
in $\s^3$ which fail to be embedded. 
They can be explicitly described by the doubly periodic immersions $\Psi_h: \R^2 \rightarrow \s^3$
given by
$$
\Psi_h (s,t)=\left( \cos s \cos ht, \cos s \sin ht, \sin s \cos t, \sin s \sin t \right)
$$
by taking $h=m/k$.
We recall (see \cite[Proposition 7.2]{La70}) that $\Psi_h$ are the only geodesically ruled minimal surfaces in $\s^3$ and so it is natural that they are  referred to as {\em Lawson spherical helicoids}.
...
Do Carmo and Dajczer, in their classical paper \cite{dCD83} on rotation hypersurfaces in spaces of constant curvature, coin the term "catenoids" for those minimal rotational surfaces both in the hyperbolic space $\H^3$ (where the catenoids are of three different types) and in the sphere $\s ^3$, similar to what happens in Euclidean space $\R^3$ where they recover the differential equation to be satisfied by the catenary as generatrix curve of the corresponding surface of revolution. The hyperbolic case is studied in depth in \cite{dCD83}, while for the spherical case the expected analogous results are discussed in several remarks, without going into too much detail. For example, in \cite[Remark (3.34)]{dCD83} it is commented that a conjugate correspondence can be established between the catenoids in $\s^3$  and the Lawson spherical helicoids. And in \cite[Remark (3.35)]{dCD83} it is stated that it is possible to determine explicitly the associated family of the catenoids in $\s^3$  but the authors will not go into that in the paper.
%
%->doCD: (3.34) REMARK. A similar correspondence can easily be established between the catenoids of S3(1) given by (3.24) and the geodesically-ruled surfaces (3.28) (as expected, the minimal Clifford torus appears as a self-conjugate surface). That they are conjugate has already been established by Lawson [7] by using certain reflection properties of the conjugate surface.
%
%->doCD: (3.35) REMARK. It is possible to determine explicitly the associated family fo of the catenoids in M3(c) but we will not go into that here.

On the other hand, we must mention that  $SO(2)$-invariant minimal surfaces in $\s^3$ were also determined in a slightly different and quite implicit way  in \cite{Ot70}. In fact, Otsuki studied in \cite{Ot70} minimal hypersurfaces of $\s^{n+1}$ with two distinct principal curvatures. When restricted to the case $n = 2$, the constructed compact surfaces are indeed tori, and
they were named {\em Otsuki tori} in \cite{Pn13}, where they were re-defined as a part of a general construction by Hsiang and Lawson of minimal submanifolds of low cohomogeneity proposed in \cite{HL71}. 
Most interestingly, Penskoi \cite{Pn13} showed that the Otsuki tori provide an extremal metric for the invariant spectral functional.
We highlight the rather more explicit description of them in \cite{HS12}. Lawson himself remarked in \cite{La70} that the tori $\tau_{m,k}$ are not the ones known to T.\ Otsuki \cite{Ot70} or E.\ Calabi \cite{Ca67} because the self-congruence groups are inequivalent.

Moreover, an analytical description of the minimal rotational surfaces of $\s^3$ is given in \cite[Theorem A]{Ri89} in the spirit of \cite{dCD83}, determining a one parameter family of compact minimal rotational surfaces.

But we must also pay attention to a more recent family of immersed minimal tori in $\s^3$ which are rotationally symmetric (see \cite[Theorem 1.4]{Br13b}, pointed out by Robert Kusner, according to the author). These surfaces are not embedded, but they turn out to be immersed in the sense of Alexandrov,
providing a large family of Alexandrov immersed minimal surfaces in $\s^3$. We will call them {\em Brendle-Kusner tori}.
Brendle  extended his argument used to prove Lawson's Conjecture showing in \cite{Br13c} that any minimal torus which is immersed in the sense of Alexandrov must be rotationally symmetric. 

Therefore, our second goal in this article will be to fully justify which surfaces of $\s ^3$ and why deserve the appellation of {\em spherical catenoids}, by clearly and concisely unifying the different examples of rotational minimal surfaces in $\s^3$ that one can find in the literature, which will come to be the conjugate surfaces to the geodesically ruled minimal surfaces in $\s^3$, that is, the Lawson spherical helicoids. In other words, we will show in detail that the well-known conjugation between the belicoid and the catenoid in Euclidean three-space extends to the spherical case. In fact, we will determine explicitly their associated family.

In order to achieve this objective, we deal in Section \ref{Sect:DefHel} with helicoidal surfaces in the unit $3$-sphere $\mathbb S^3$, i.e.\ surfaces invariant under the action of the helicoidal $1$-parameter group of isometries given by the composition of a translation and a rotation in $\mathbb S^3$. This class includes the rotational ones. 
As a key tool, we introduce in Section \ref{Sect:Momentum} the notion of {\em spherical angular momentum} of a spherical curve. 
It is a smooth function associated to any spherical curve, which completely determines it (up to a family of distinguished isometries) in relation with its relative position with respect to a fixed geodesic (see Theorem \ref{K determines}).
We compute it for great and small circles and for the classical spherical catenaries studied by Bobillier and Gudermann in nineteenth century. Then we introduce a one-parameter family of \textit{spherical catenoids} in Definition \ref{def:catenoid} just as the rotational surfaces generated by the above mentioned classical spherical catenaries. They turn to be the minimal rotational surfaces in $\s^3$ considered in \cite{dCD83} and \cite{Ri89}.
...
As a first application, in Theorem \ref{Th:Rotmin} we perform in an easy direct way the local classification of the minimal rotational surfaces of $\s^3$ in terms of the spherical catenoids, together with the totally geodesic sphere and the Clifford torus that appear as limiting cases. Moreover, in Theorem \ref{Th:Rotmin}, we also identify both Otsuki and Brendle-Kusner tori with the \textit{compact} spherical catenoids, resulting the only  Alexandrov immersed minimal tori in $\s^3$.  

In Theorem \ref{Th:Cat to Hel}, we make a first approach to the assertion of \cite[Remark (3.34)]{dCD83} by showing explicitly that each spherical catenoid is locally isometric to two, and only two, spherical Lawson helicoids and, as expected, the Clifford torus seems to appear as a self-conjugate surface (see Remarks \ref{h1 Clifford} and \ref{h1Cli}).

Finally, on the basis of our rather explicit description of all the minimal helicoidal surfaces in $\mathbb S^3$ in terms of their spherical angular momenta, we prove in Theorem \ref{Th:minimal associated} that they are the {\em associated} surfaces  (in the sense of \cite{La70}) to the spherical catenoids and confirm the conjugation between spherical catenoids and Lawson spherical helicoids suggested in Theorem \ref{Th:Cat to Hel}. In this way, we carry out the statement of \cite[Remark (3.35)]{dCD83} in detail.
...
%The existence and global behaviour of compact minimal submanifolds of a
%homogeneous Riemannian manifold is, in full generality, a difficult area of
%study. The nonlinearity of the problem makes even the construction of explicit
%examples reasonably difficult and, at the same time, makes such examples indispensable
%guidelines for research. Thus, it is natural to try to reduce the
%complexities of the situation by means of some isometry group G of low cohomogeneity
%and, in particular, to look for the existence and general behavior
%of G'-invariant, minimal submanifolds.
%
%\medskip
...
%\underline{About \textit{CLOSED CMC} surfaces in $\s^3$ (survey of Brendle):}
%
%Almgren adapted: genus zero $\implies$ geodesic spheres (totally umbilical)
%
%Andrews \& Li, Perdomo: embedded genus one are rotationally symmetric
%
%Brendle: Any Alexandrov-type immersed is rotationally symmetric
%
%
%Brendle also proved in \cite{Br13c} that in the setting of CMC tori, the thesis of rotationally symmetric can be also deduced by imposing another extension condition to the immersion.
%
%Andrews and Li proved in \cite{AL15} that any CMC (or $H_0$-surface) embedded torus in the 3-sphere is axially symmetric (as conjectured by Pinkall and Sterling) and, using this, they gave a classification of such surfaces for any given value of $H_0$, by completing a classification of rotationally symmetric CMC surfaces of Perdomo \cite{Pr10}. It appears an unexpected rigidity, similar of the Clifford torus, for certain standard tori just when $H_0=\pm 1/\sqrt 3$.

%Our aim: prescribed $H$ in helicoidal (in particular, rotational) surfaces. Focus on CMC.
%
%** A new family of CMC helicoidal surfaces in $\s^3$, playing perhaps the role of minimal Lawson helicoids in the CMC setting. 
%
%** Recover rotational CMC surfaces in $\s^3$, interpreting as spherical nodoids, undoloids, etc.
%
%\medskip
...
A {\em helicoidal surface} in $\s^3$ is a surface invariant under the action of the helicoidal 1-parameter group of isometries given by the composition of a translation and a rotation in $\s^3$. Concretely, for any $a\in \R$, let $\psi_a(t)$ be the translations along the great circle $c_0=\{ (x_1,x_2,0,0)\in \s^3 \}$ given by
$$
\psi_a(t)= \left(
\begin{array}{cccc}
\cos at & -\sin at & 0 & 0 \\
...
\right),
$$
and for any $b\neq 0$, let $\phi_b(t)$ be the rotations around $c_0$ given by
$$
\phi_b(t)= \left(
\begin{array}{cccc}
1 & 0 & 0 & 0 \\
...
\right).
$$
We remark that the group $\phi_b(t)$ fixes the great circle $c_0$ and the orbits are circles centred on $c_0$.

Hence, a helicoidal surface (along the great circle $c_0$) is invariant under $\psi_a(t)\circ \phi_b(t)$, for fixed {\em translation velocity} $a\in \R$  and {\em rotation velocity} $b\neq 0$, and can be locally parametrized by 
...
\begin{remark}\label{ab}
	...
	there is no restriction if we consider $a\geq 0$, $b>0$; that is, $h\geq 0$.
\end{remark}
...
We now show how the spherical angular momentum $\mathcal K = \mathcal K (z) $ determines the spherical curve $\xi =(x,y,z)$. For this purpose, we use geographical coordinates in $\s^2$ and write 
...
we point out that we can determine by quadratures in a constructive explicit way the spherical curves such that $\kappa=\kappa(z)$, in the spirit of Theorem 3.1 in \cite{S99}.
Concretely, if we prescribe a continuous function $\kappa=\kappa(z)$ as curvature, the proof of Theorem~\ref{K determines} clearly implies the computation of three quadratures, following the sequence:
\begin{enumerate}
...
with $|c|<\sqrt{1+k_0^2}$. 
But the computation of $\lambda$ is far from trivial and depends on the values of $c$. After a long computation, we deduce:
\begin{itemize}
	\item If $|c|\neq k_0$:
...
%In this section we are interested in critical points of the functional
%$$ \mathcal F^\mu (\xi):=\int_\xi \sqrt{\kappa+\mu} \, ds, \, \mu \in \R. $$
%When $\mu =0$, the above functional $\int_\xi \kappa^{1/2}\, ds$ was considered in Section 5 of \cite{AGM03} acting on the space of convex ($\kappa >0$) closed curves in $\s^2$, motivated by the study of ($r=1/2$)-generalized elastic curves in $\s^2$. We aim to connect the critical points of $\mathcal F^\mu$ with the classical catenaries curves in $\s^2$ and some possible generalization of them.
%
%Taking into account Remark -, we are devoted to study the spherical curves with curvature 
%\begin{equation}
%\label{k mu delta cate}
%\kappa (z) = \frac{\delta^2}{4z^2}-\mu, \ \mu \in \R, \delta \neq 0. 
%\end{equation}
%We start considering the case $\mu= 0$.
%
%\underline{Case $\mu =0$: spherical catenaries}

The spherical catenaries are the equilibrium lines of an inelastic flexible homogeneous infinitely thin massive wire included in a sphere, placed in a uniform gravitational field. Like any catenaries, their centres of gravity have the minimal altitude among all the curves with given length passing by two given points. They were studied by Bobillier in 1829 and by Gudermann in 1846. %See Figure \ref{catenaries}.
%\begin{figure}[h!]
%\begin{center}
%\includegraphics[height=4cm]{Catenaries.pdf}
%\caption{Spherical catenaries (Source: \cite{F93}).}
%\label{catenaries}
%\end{center}
%\end{figure}
...
\kappa(z)=c /z^2, \  c>0.
\end{equation}
%So they correspond  to the election $\mu=0$ and $\delta =2\sqrt a$.
%For any $c>0$, it is easy to prove that there exists an unique angle $\varphi_0 \in (0,\pi/2)$ such that $c=\tan\varphi_0 \sin^2 \varphi_0$. Thus the parallel $z=\sin \varphi_0$ is a constant solution to (\ref{k=a/z2}).
...
\end{equation}
which implies that $c<1/2$ and $1-\sqrt{1-4c^2}<2z^2<1+\sqrt{1-4c^2}$, and it is not difficult to get:  
...
%However, the explicit computation of $\lambda$ in terms of the arc parameter $s$ requires elliptic integrals of the first and third kind.
%As a summary, we have proved the following uniqueness result.
%\begin{corollary}\label{cor:catenary}
%The spherical catenaries \eqref{eq catenaries} are the only spherical curves (up to rotations around the $z$-axis) with spherical angular momentum $\mathcal K(z)=-a/z$ (and curvature $\kappa (z)=a/z^2$), $0<a<1/2$.
%\end{corollary}
%Combining (\ref{k=a/z2}) and (\ref{z_catenary}) we have that the intrinsic equation of the spherical catenaries is given by
%$$ \kappa (s)=\frac{2a}{1+\sqrt{1-4a^2}\sin 2s}, \ 0<a<1/2. $$
...
\end{equation}
Notice that $\lambda_\beta$ is an increasing function. Further, since the function $z_{\beta}$ is $\pi$-periodic, the catenary $\mathcal C_{\beta}$ will be a closed curve if and only if  $\lambda_{\beta}(s+m\pi)=\lambda_{\beta}(s)+2k\pi$, with $m,k\in\Z$. As $\lambda_{\beta}'$ is also a $\pi$-periodic function,  $\lambda_{\beta}(s+m\pi)=\lambda_{\beta}(s)+m\lambda_{\beta}(\pi)$. Thus $\mathcal C_{\beta}$ is a closed curve if and only if
\begin{equation}\label{eq:Tcat}
T(\beta):=\frac{\lambda_{\beta}(\pi)}{2\pi}\in \Q.
\end{equation}
So the problem of being $\mathcal C_\beta$ a closed curve can be solved analysing the function $T=T(\beta)$, $\beta \in (0,\pi/2)$, given in \eqref{eq:Tcat}. Using the same arguments as in \cite{Ot70}, \cite{AL15} or \cite{Pr16},
...
In conclusion, as a consequence of Theorem \ref{K determines}, we have proved the following uniqueness result.
\begin{corollary}\label{cor:catenary}
The spherical catenaries $\mathcal C_\beta$, $0<\beta <\pi/2$,  are the only spherical curves (up to rotations around the $z$-axis) with spherical angular momentum $\mathcal K(z)=-\sin\beta/(2z)$. In addition, the closed catenaries  are non-embedded, posses dihedral symmetry and can be parametrized as $\mathcal C_{\beta_q}$, with $q\in  \Q\cap(\frac{1}{2},\frac{\sqrt{2}}{2}) $. 
\end{corollary}
...
%\section{Catenary-type curves on the sphere}
%
%In this section we are interested in critical points of the functional
%$$ \mathcal F^\mu (\xi):=\int_\xi \sqrt{\kappa+\mu} \, ds, \, \mu \in \R. $$
%When $\mu =0$, the above functional $\int_\xi \kappa^{1/2}\, ds$ was considered in Section 5 of \cite{AGM03} acting on the space of convex ($\kappa >0$) closed curves in $\s^2$, motivated by the study of ($r=1/2$)-generalized elastic curves in $\s^2$. We aim to connect the critical points of $\mathcal F^\mu$ with the classical catenaries curves in $\s^2$ and some possible generalization of them.
%
%Taking into account Remark \ref{z P(k)}, we are devoted to study the spherical curves with curvature 
%\begin{equation}
%\label{k mu delta cate}
%\kappa (z) = \frac{\delta^2}{4z^2}-\mu, \ \mu \in \R, \delta \neq 0. 
%\end{equation}
%We start considering the case $\mu= 0$.
%
%\subsection{Case $\mu =0$: spherical catenaries}
%The spherical catenaries are the equilibrium lines of an inelastic flexible homogeneous infinitely thin massive wire included in a sphere, placed in a uniform gravitational field. Like any catenaries, their centres of gravity have the minimal altitude among all the curves with given length passing by two given points. They were studied by Bobillier in 1829 and by Gudermann in 1846 (cf.\ \cite{F93}). See Figure \ref{catenaries}.
%%\begin{figure}[h!]
%%\begin{center}
%%\includegraphics[height=6cm]{Catenaries.pdf}
%%\caption{Spherical catenaries (Source: \cite{F93}).}
%%\label{catenaries}
%%\end{center}
%%\end{figure}
% 
%Using cylindrical coordinates $(r,\theta,z)$ in $\R^3$, they can be described analytically by the following first integral of the corresponding ordinary differential equation:
%\begin{equation}\label{catenary}
%(z-z_0) \, r^2 \, \frac{d\theta}{ds}= {\rm constant}.
%\end{equation}
%
%We study in this section spherical curves satisfying the condition
%\begin{equation}\label{k=a/z2}
%\kappa(z)=a /z^2, \  a>0.
%\end{equation}
%So they correspond in \eqref{k mu delta cate} to the election $\mu=0$ and $\delta =2\sqrt a$.
%For any $a>0$, it is easy to prove that there exists an unique angle $\varphi_0 \in (0,\pi/2)$ such that $a=\tan\varphi_0 \sin^2 \varphi_0$. Thus the parallel $z=\sin \varphi_0$ is a constant solution to (\ref{k=a/z2}).
%
%We now apply Theorem~\ref{quadratures} and Remark \ref{c} considering (\ref{k=a/z2}) and
%$$
%\mathcal K (z)=-\frac{a}{z}, \ a>0.
%$$
%Then we have:
%\begin{equation*}
%s=s(z)=\int \frac{z\,dz}{\sqrt{z^2(1-z^2)-a^2}},
%\end{equation*}
%which implies that $a<1/2$ and $1-\sqrt{1-4a^2}<2z^2<1+\sqrt{1-4a^2}$, and it is not difficult to get:  
%\begin{equation}\label{z_catenary}
%z(s)=\sqrt{\frac{1+\sqrt{1-4a^2}\sin 2s}{2}}.
%\end{equation}
%In addition, we have that
%\begin{equation}
%\label{eq catenaries}
%d\lambda=\frac{a}{z(1-z^2)}\,ds
%\end{equation}
%
%Looking at (\ref{catenary}), taking into account that $r^2+z^2=1$ and $\theta = \lambda$, we deduce from (\ref{eq catenaries}) that we get a spherical catenary (with $z_0=0$ and constant $a\in (0,1/2)$). 
%However, the explicit computation of $\lambda$ in terms of the arc parameter $s$ requires elliptic integrals of the first and third kind.
%As a summary, we have proved the following uniqueness result.
%\begin{corollary}\label{cor:catenary}
%The spherical catenaries \eqref{eq catenaries} are the only spherical curves (up to rotations around the $z$-axis) with spherical angular momentum $\mathcal K(z)=-a/z$ (and curvature $\kappa (z)=a/z^2$), $0<a<1/2$.
...
%\begin{equation}\label{eq:paramXhel}
%\begin{array}{c}
%X(s,t)=\left( 
%x(s) \cos ht  - y(s) \sin ht , \right. \vspace{0.1cm } \\ 
%\left. x(s) \sin ht + y(s) \cos ht, 
...
%\begin{equation}\label{eq: mom-gauss}
%\mathcal K(z)
%%=\pm \sqrt{\dfrac{b^2 z^2-a^2B(z)-(b^2-a^2)z^2B(z)}{b^2 z^2-a^2 B(z)}}
%=\pm\sqrt{1-\dfrac{(b^2-a^2)z^2B(z)}{b^2 z^2-a^2 B(z)}},
%\end{equation}
...
Using \eqref{eq:K hel min}, it is not difficult to check that this condition is equivalent to $z^4+c^2<z^2$. Writing $z=\sin \varphi$, we have that $c^2<z^2(1-z^2)=\sin^2\varphi\cos^2\varphi=\frac14\sin^2 2\varphi <\frac14 $. In addition, from \eqref{eq:K hel min} it is clear that $\mathcal K^h_{-c}=-\mathcal K^h_c$ and then it is enough to consider $c\geq 0$. Hence, we take $c\in [0, 1/2)$. We point out that, up to a change of orientation, we can choose the sign $\pm$ we want.
...
The previous study done in Section \ref{ex:Kinvz} allows us to provide the following local classification theorem, where
...
%\[
%\big(\sqrt{1-h(\theta)^2-h'(\theta)^2}\cos t,
%\sqrt{1-h(\theta)^2-h'(\theta)^2}\sin t,
%h(\theta)\sin\theta+h'(\theta)\cos\theta,
%h'(\theta)\sin\theta-h(\theta)\cos\theta\big),
%\]
%\[
...
%which is the spherical angular momentum of catenoids $\mathrm{Cat}_{\beta}$ for $2c=\sin \beta$, 
%see Subsection \ref{ex:Kinvz}.
%In this case the change needed is the following
%$$z(s)^2=1-h(\theta)^2-h'(\theta)^2,\quad ds=(h''+h)\sqrt{\frac{1-h^2}{1-h^2-h'^2}}d\theta.$$
\end{proof}
...
To introduce in practice the family of associated surfaces, we proceed as described  also in \cite{dCD83}:
{\it
...
%For this purpose, we make use of \cite[Theorem 8]{La70}  (taking, in its notation, the particular values $H=1=c$, $H'=0$) and conclude:
%then $K_{\text G}<1$ and hypothesis automatically satisfied):

% $\mathrm{Cat}_{\tilde  c}\equiv \text{Cat}_\beta $, $\tilde  c = \frac12 \sin \beta$, $\tilde  c \in (0,1/2)$ and $\beta \in (0,\pi/2)$.


We are now able to prove in detail the following result commented in \cite[Remark 3.34]{dCD83}.

%****************
%The Lawson correspondence \cite{La70} associates isometric surfaces $M$ and $\hat M$ of
%distinct constant mean curvature $H$ and $\hat H$, immersed in space forms of constant
%sectional curvature $k$ and $\hat k$ respectively, such that $k+H^2 = \hat k + \hat H^2$. Whenever
%$M$ is a minimal surface, then $M$ is also referred to as a CMC cousin of $M$.
%
%To each complete minimal surface in $\s ^3$ there is associated a 1-parameter family of complete, locally isometric surfaces of constant mean curvature in each of the simply-connected, 3-dimensional space forms of curvature $\leq 1$.
%
%For this purpose, we make use of \cite[Theorem 8]{La70}  (taking, in its notation, the particular values $H=1=c$, $H'=0$) and conclude:
%%then $K_{\text G}<1$ and hypothesis automatically satisfied):
%{\it For any minimal immersion $X\colon S \rightarrow \s^3$ of a simply connected surface $S$, there exists a differentiable,
%	$2\pi$--periodic family of minimal isometric immersions $\mathrm{Asc}^\theta( X)\colon S \rightarrow \s^3$. Moreover, 
%	up to congruences, the maps $\mathrm{Asc}^\theta( X)$, $0\leq \theta \leq \pi$, represent (extensions of) all local, isometric, minimal immersions of S into $\s^3$.} The surface $\mathrm{Asc}^{\pi/2}( X)$ is called the {\em conjugate surface} of $X$.
%******************
...
%\begin{equation}\label{eq:deform min}
%	\tilde c=\pm \frac{\sqrt{(1-h^2)^2 c^2+h^2}}{1+h^2},\quad\text{and}\quad  \cos \theta =  \frac{ c(h^2-1)}{\tilde  c (1+h^2)}.
%\end{equation}
...
\end{theorem}
%
%{\color{red}Note: The surfaces are not congruent. Rigidity of Clifford Tours and other examples.}
%


\begin{remark}\label{coher}
{\rm 
Bearing in mind Theorem \ref{Th:Cat to Hel}, it is easy to check that $h^2/(1+h^2)^2=\frac14 \sin^2 \beta $ if, and only if, $h=\tan \beta/2$ or $h=\cot \beta/2$. In Figure \ref{graph deform}, we draw in the region  $0\leq c <1/2$, $h\geq 0$, the curves $c_\beta$ given by
...
%Fix ${\rm Cat}_{\hat c}$, $\hat c \in (0,1/2)$. Its first fundamental form $\hat I$  in coordinates $(\hat z,\hat t)$
%is given by
%\begin{equation}\label{eq:I Cat bis}
%\hat I=\frac{\hat z^2 d\hat z^2}{\hat z^2(1-\hat z^2)-\sin^2\beta /4}+\hat z^2 d\hat t^2
%\end{equation}
%
%The first fundamental form $I$ of the minimal helicoidal surfaces $X^h (\xi_c)$, $|c|<1/2$, in coordinates $(z,t)$, may be expressed as
%\begin{equation}\label{eq:I Hel min}
%I=\frac{(z^4+h^2c^2)}{z^2(z^2-z^4-c^2)}\ dz^2
%- \frac{2hc\sqrt{(1-h^2)z^2+h^2}}{z\sqrt{z^2-z^4-c^2}}\ dz dt
%+\left((1-h^2)z^2+h^2\right) \ dt^2
%\end{equation}
%
%Define locally:
%
%$$ \hat z = \sqrt{\frac{(1-h^2)z^2+h^2}{1+h^2}} $$
%
%$$
%\hat t = f(z) + \sqrt{1+h^2} \, t, \ f'(z)=-\frac{hc\sqrt{1+h^2}}{z \,\sqrt{(1-h^2)z^2+h^2}\, \sqrt{z^2-z^4-c^2}}
%$$
%
%$$
%\frac{(1-h^2)^2 c^2+h^2}{(1+h^2)^2}=\frac{\sin ^2 \beta}{4}
%$$
%See Figure \ref{graph deform}.
%
...
%	By personal communication from Pablo Mira, using arguments based on the Hopf differential associated to any minimal surface in $\s^3$, it can be ensured that the  associated family to a minimal surface invariant by a uniparametric group of isometries of the ambient space is also invariant by that group.
...
%The Gauss map of a minimal surface in $\s^3$ again describes a minimal surface in $\s^3$ with singularities occurring precisely at the points where $K_{\text G}= 1$. The curvature of the Gaussian image surface is $K_{\text G}^\star =-K_{\text G}/(1 - K_{\text G})$. Hence, to each minimal surface in $\s^3$ the Gauss map associates a second generalized minimal surface called the {\em polar variety}. It is easy to see that taking the Gauss map a second time produces the original surface, i.e.\ $X^{\star \star} =X$. 
%In addition, the polar variety is non-singular for a torus or a Klein bottle and the polar variety of the Lawson spherical helicoid $\tau_{m,k}$ is $\tau_{k,m}\cong \tau_{m,k}$ (cf.\ Section 10 of \cite{La70})
%
%\begin{theorem}\label{Th:polar index}
%The polar variety of an Otsuki spherical catenoid is itself. 
%
%%Estimation of the Morse index ...
%
%\end{theorem}
%
%\begin{proof}
%If $X=X^0(\xi)$, then $\nu = X^0 (N)$, where $N=\xi \times \dot \xi$.
%
%If $X$ minimal, then $X^\star = \nu$ minimal and $K_{\rm ext}^\star =1/ K_{\rm ext}$.
%
%$X={\rm Cat}_\beta$, $0<\beta <\pi/2$. 
%
%$K_{\rm ext}(z)=-\frac{\sin ^2 \beta }{4 z^4}$.
%
%
%\end{proof}
%
%{\Large INDEX} Kapouleas-Ros
...
%
%\bibitem[AGM04]{AGM04} J.~Arroyo, O.J.~Garay and J.J.~Menc\'{\i}a.
%\newblock {\em Extremals of curvature energy actions on
%spherical closed curves}
%\newblock J.\ Geom.\ Phys.\ {\bf 51} (2004), 101--125.
%
%\bibitem[AGM06]{AGM06} J.~Arroyo, O.J.~Garay and J.J.~Menc\'{\i}a.
%\newblock {\em Elastic circles in 2-spheres.}
%\newblock J.\ Phys.\ A: Math.\ Gen.\ {\bf 39} (2006), 2307--2324.
%
%\bibitem[AGP18]{AGP18} J.~Arroyo, O.J.~Garay and A.~P\'{a}mpano.
%\newblock {\em Constant mean curvature invariant surfaces and extremals of curvature energies.}
%\newblock J.\ Math.\ Anal.\ Appl.\ {\bf 462} (2018), 1644--1688.
%
%\bibitem[BC94]{BC94} G.~Brunnett and P.E.~Crouch.
%\newblock {\em Elastic curves on the sphere}
%\newblock Adv.\ Comp.\ Math.\ {\bf 2} (1994), 23--40.
...
%\bibitem[Br14]{Br14}  S.~Brendle,
%{\em Embedded Weingarten tori in $\s^3$},
%Adv.\ Math.\ {\bf 257} (2014), 462–-475.

%\bibitem[BF71]{BF71} P.F.~Byrd and M.D.~Friedman,
%{\em Handbook of elliptic integrals for engineers and physicists}, 2nd edition,
...
%\bibitem[CCIs17]{CCIs17} I.~Castro, I.~Castro-Infantes and J.~Castro-Infantes.
%\newblock {\em New plane curves with curvature depending on distance from the origin.}
%\newblock Mediterr.\ J.\ Math.\ (2017) {\bf 14}: 108.
%
...
%
%\bibitem[CCIs18b]{CCIs18b} I.~Castro, I.~Castro-Infantes and J.~Castro-Infantes.
% {\em Curves in Lorentz- Minkowski plane with curvature depending on their position.}
% Preprint, arXiv: 1806.09187.
%
%\bibitem[CLU06]{CLU06} I.~Castro, H.~Li and F.~Urbano. 
%\newblock {\em Hamiltonian minimal Lagrangian submanifolds in complex space forms.}
%\newblock Pacific J.\  Math.\ {\bf 227} (2006), 43--65.
%
%\bibitem[CMU01]{CMU01} I.~Castro, C.R.~Montealegre and F.~Urbano. 
%\newblock {\em Closed conformal vector fields and Lagrangian submanifolds in complex space forms.}
%\newblock Pacific J.\  Math.\ {\bf 199} (2001), 269--302.
%
%\bibitem[E00]{E00} P.~Erd\"{o}s.
%\newblock {\em Spiraling the Earth with C.\ G.\ J.\ Jacobi.}
%\newblock  Amer.\ J.\ Phys.\ {\bf 68} (2000), 888--895.

%\bibitem[dCD82]{dCD82} M.P.~do Carmo and M.~Dajczer,
%{\em Helicoidal surfaces with constant mean curvature},
%Tohoku Math.\ J.\  {\bf 34} (1982), 425--435.
...
%\bibitem[ES15]{ES15} N.~Edelen and B.~Solomon,
%{\em Constant mean curvature, flux conservation, and symmetry},
%Pacific J.\ Math.  {\bf 274} (2015), 53--72.
...
%\bibitem[J95]{J95} V.~Jurdjevic.
%\newblock {\em Non-Euclidean elastica.}
%\newblock  Amer.\ J.\ Math.\ {\bf 117} (1995), 93--124.
%
%\bibitem[LS84]{LS84} J.~Langer and D.~Singer.
%\newblock {\em The total squared curvature of closed curves.}
%\newblock  J.\ Diff.\ Geom.\ {\bf 20} (1984), 1--22.
%
%\bibitem[LS96]{LS96} J.~Langer and D.~Singer.
%\newblock {\em Lagrangian aspects of the Kirchhoff elastic rod.}
%\newblock SIAM Rev.\ {\bf 38} (1996), 1--17.

%\bibitem[Ga09]{Ga09} J.A.~G\'alvez,
%{\em Surfaces of constant curvature in 3-dimensional space forms},
%Mat.\ Contemp.\ {\bf 37} (2009), 1--42.
...
%\bibitem[HPM09]{HPM09} R.~Hynd, S-H.~Park and J.~McCuan,
%{\em Symmetric surfaces of constant mean curvature in $\s^3$},
%Pacific J.\ Math. {\bf 241} (2009), 63--115.
...
%\bibitem[LP20a]{LP20a} R.~L\'opez and A.~P\'ampano,
%{\em Rotational surfaces of constant astigmatism in space forms},
%J.\ Math.\ Anal.\ Appl.\ {\bf 483} (2020), 123602.
%
%
%\bibitem[LP20b]{LP20b} R.~L\'opez and A.~P\'ampano,
%{\em Classification of rotational surfaces with constant skew curvature in 3-space forms},
%J.\ Math.\ Anal.\ Appl.\ {\bf 489} (2020), 124195.
...
%\bibitem[Pi85]{Pi85} U.~Pinkall.
%\newblock {\em Hopf tori in $\mathbb S^3$.}
%\newblock Invent.\ Math.\ {\bf 81} (1985), 379--386.
...
%\bibitem[PP15]{PP15}  B.~Palmer and O.M.~Perdomo,
%{\em Rotating drops with helicoidal symmetry},
%Pacific J.\ Math.\  {\bf 273} (2015), 413--441.
...
%
%\bibitem[S08]{S08} D.~Singer.
%\newblock {\em Lectures on elastic curves and rods.}
%\newblock Curvature and Variational Modelling in Physics and Biophysics,
