...
For every original image $\bfX_i$, we define a probability distribution $\bfpi_i$, such that each object $\bfX_j$ gets sampled with probability $\bfpi_{i, j}$. For example, during the augmentation process, we will flip $\bfX_i$ to get $\bfX_j$ with probability $1/9$, then $\bfpi_{i, j}=1/9$. 
