...
\newcommand{\bpf}{\widetilde{\mathcal{M}}_{prob}^1}
\newcommand{\cc}{(c+1)}
\newcommand{\wxi}{\widetilde{\xi}}
\newcommand{\wPhi}{\widetilde{\Phi}}
\newcommand{\wY}{\widetilde{Y}}
\newcommand{\otau}{\overline{\tau}}
\newcommand{\opr}{\overline{\mathbb{P}}}
\newcommand{\oew}{\overline{\mathbb{E}}}
...
\newcommand{\B}{\mathcal{B}(X)}
\newcommand{\ms}{\mathcal{M}_{prob}(X)}
\newcommand{\mr}{\mathcal{M}_1(X)}
\newcommand{\ew}{\mathbb{E}}
\newcommand{\bb}{\mathbb{B}}
\newcommand{\bbh}{\widehat{\mathbb{B}}}
\newcommand{\uu}{\mathbf{u}}
\newcommand{\zz}{\mathbf{z}}
\newcommand{\uuh}{\widehat{\mathbf{u}}}
\newcommand{\zzh}{\widehat{\mathbf{z}}}
\newcommand{\UU}{\mathbf{U}}
\newcommand{\UUh}{\widehat{\mathbf{U}}}
...
\newcommand{\bew}{\overline{\mathbb{E}}}
\newcommand{\bpr}{\overline{\mathbb{P}}}
...
\textbf{Keywords:} Piecewise deterministic Markov process; Switching semiflows; State-dependent jump intensity; Invariant measure; Iterated function system; One-to-one correspondence; Polish space. \\
}
...
%Secondly, an essential step in the proof of \hbox{\cite[Theorem 4.4]{b:czapla_erg}} \hbox{(cf. also \cite{b:benaim1})}, on which the argument in \cite{b:czapla_exp_erg} relies, involves expressing the transition law~$P$ of the chain~$\Phi$ as the composition of two specific stochastic kernels $G$ and~$W$, so that $P=GW$. If $\lambda$ is constant, the operator on bounded continuous functions $f$ induced by~$G/\lambda$ coincides with the resolvent of the weak generator $A_Q$ of the semigroup $\{Q_t\}_{t\in\mathbb{R}_+}$ defined by $$Q_t f (y,i):=f(S_i(t,y),i)\quad\text{for}\quad (y,i)\in X.$$ Consequently, according to \cite[Theorem 1.7]{b:dynkin_1965}, $G/\lambda$ is then invertible and its inverse is the operator $\lambda\operatorname{id}-A_Q$ (defined on the domain of $A_Q$), which plays a key role in the proofs given in~\cite{b:czapla_erg} and~\cite{b:benaim1}. Unfortunately, this version of the theorem fails to hold
...
%is a discrete-time Markov chain such that
%\begin{gather}
%\begin{split}
%\pr(Y_{n+1}\in B\,|\,Y(\tau_{n+1}-)=y)=J(y,B)\\
%\pr(\xi_{n+1}=j\,|\,\xi_n=i)=\pi_{ij}(y)
%\end{split}
%\end{gather}
...
%In the case where $K$ is a stochastic kernel, also the set $\mathcal{M}_1(X)$ is invariant under the operator given by \eqref{e:mK}.
...
%While considering stochastic kernels and the corresponding Markov operators, one usually uses the symbol $P$ in place of~$K$.  
...
$$\pr(\Psi(t)\in A\,|\, \Psi(0)=x)=P_t(x,A) \quad\text{for all}\quad x\in E,\; A\in\mathcal{B}(E),\;t\geq 0.$$
...
It is easily seen that $GW=\tG\tW=P$, where $P$ is the stochastic kernel given by \eqref{def:P}.
\end{remark}
...
%Right-hand sides of \eqref{e:m_psi} and \eqref{e:m_phi} are well-defined probability measures since, due to \eqref{e:lambda_bounds}, for every $\mu\in\mathcal{M}_1(X)$, we have
%\begin{gather*}
%\mu\tG(X) = \<\tG\mathbbm{1}_X, \mu\>=\<G\left(\mathbbm{1}_X/\hlambda\right),\mu \>\geq \olambda^{-1}\<G\mathbbm{1}_X, \mu\>=\olambda^{-1}>0\\
%\mu\tW(X) = \<\tW\mathbbm{1}_X, \mu\>=  \<\hlambda W\mathbbm{1}_X, \mu\>\geq \ulambda  \<W\mathbbm{1}_X, \mu\>=\ulambda>0.
%\end{gather*}
\end{remark}

\begin{remark}\label{rem:feller_gw} The kernels $G$ and $\widetilde{G}$ are Feller. Moreover, if $J$ is Feller then so are the kernels $W$ and~$\widetilde{W}$.
\end{remark}
...
The remainder of the paper is devoted to proving Theorem \ref{thm:main}. In the first part of this section, we establish certain properties of the Markov semigroup under consideration. They will be used in the second one to prove two lemmas on the weak generators of $\{P_t\}_{t\in\mathbb{R}_+}$ and some auxiliary semigroup, which are crucial in the core of the proof, presented in the last part.
...
%As has been already mentioned in the introduction, $\{\eta(t)\}_{t\in\mathbb{R}_+}$ is not a Poisson process in general, since $\Delta \tau_n$ is not exponentially distributed, unless $\lambda$ is constant. 
%
%The distribution of $\eta(t)$ can be, however, estimated from above by the distribution of a such a process (with intensity $\olambda$) at time $t$, which we demonstrate in the following two lemmas.
...
\begin{equation}
\label{def:gh}
g_T(u,j,s):=\mathbbm{1}_{[0,T]}(t)f(S_j(T-s,u),j)\quad\text{and}\quad
...
$\boldsymbol{(\Rightarrow)}$: To prove that \ref{cnd:a} implies \ref{cnd:b}, suppose that $\mu_*^{\Phi}\in\mathcal{M}_1(X)$ is invariant for $P$, and let $\mu_*^{\Psi}$ be given by \eqref{e:m_psi}. 
...
$\boldsymbol{(\Leftarrow)}$: We now proceed to the proof of the opposite implication, i.e., \ref{cnd:b}$\Rightarrow$\ref{cnd:a}. For this aim, suppose that $\mu_*^{\Psi}\in\mathcal{M}_1(X)$ is invariant for $\{P_t\}_{t\in\mathbb{R}_+}$, and let $\mu_*^{\Phi}$ be the measure defined by \eqref{e:m_phi}.
...
%\begin{proof}
%The inequality is obvious for $n=0$. Let $\bar{x}=(y,i,s)\in\bar{X}$. We will show inductively that, for every $n\in\n$,
%\begin{equation}
%\label{as_ind}
%\ew_{\bar{x}}\left[e^{\ulambda \tau_n}\mathbbm{1}_{\{\tau_n\leq T\}} \right]\leq   \frac{(\olambda(T-s))^n}{n!}e^{\ulambda s}\mathbbm{1}_{[s,\infty)}(T)\quad\text{whenever}\quad T\in\mathbb{R}_+.
%\end{equation}
%Obviously, according to \eqref{e:dist_tau}, for any $n\in\n$, the conditional probability density function of $\tau_n$ given $\bar{\Phi}_{n-1}=\bar{x}$ is of the form
%$$f_{\tau_n|\bar{\Phi}_{n-1}}(t|\bar{x})=\lambda(S_i(t-s,y)) e^{-\Lambda(y,i,t-s)}\mathbbm{1}_{[s,\infty)}(t)\quad\text{for}\quad t\in\mathbb{R}_+.$$
%Hence, for $n=1$ we get
%
%\begin{align}
%\begin{split}
%\label{base_step}
%\ew_{\bar{x}}\left[e^{\ulambda \tau_1}\mathbbm{1}_{\{\tau_1\leq T\}} \right]&=\int_0^T e^{\ulambda t} f_{\tau_1|\bar{\Phi}_{0}}(t|\bar{x})\,dt\leq  \int_0^T e^{\ulambda t}\,\olambda e^{-\ulambda(t-s)}\mathbbm{1}_{[s,\infty)}(t)\,dt\\
%&=e^{\ulambda s} \olambda(T-s)\mathbbm{1}_{[s,\infty)}(T)\quad\text{for every}\quad T\in\mathbb{R}_+.
%\end{split}
%\end{align}
%Now, suppose that \eqref{as_ind} holds for some arbitrarily fixed $n\in\n$. Then
%\begin{align*}
%\ew_{\bar{x}}\left[e^{\ulambda \tau_{n+1}}\mathbbm{1}_{\{\tau_{n+1}\leq T\}}\,|\,\bar{\Phi}_n \right]&=\int_0^T e^{\ulambda t} f_{\tau_{n+1}|\bar{\Phi}_{n}}(t|\bar{\Phi}_n)\,dt\leq \int_0^T e^{\ulambda t}\, \olambda e^{-\ulambda(t-\tau_n)}\mathbbm{1}_{[\tau_n,\infty)}(t)\,dt\\
%&=\olambda\int_0^T e^{\ulambda\tau_n}\mathbbm{1}_{\{\tau_n\leq t\}}\,dt.
%\end{align*}
%Taking the expectation of both sides of this inequality and, further, using the inductive hypothesis we obtain
%
%\begin{align*}
%\ew_{\bar{x}}\left[e^{\ulambda \tau_{n+1}}\mathbbm{1}_{\{\tau_{n+1}\leq T\}}\right]
%&\leq \olambda\int_0^T \ew_{\bar{x}}\left[e^{\ulambda\tau_n}\mathbbm{1}_{\{\tau_n\leq t\}}\right] dt 
%\leq \olambda\int_0^T \frac{(\olambda(t-s))^n}{n!}e^{\ulambda s}\mathbbm{1}_{[s,\infty)}(t)
%\,dt \\
%&\leq \frac{\olambda^{n+1}}{n!} e^{\ulambda s}\int_s^T (t-s)^n\,dt=\frac{\olambda^{n+1}}{n!} e^{\ulambda s}\frac{(T-s)^{n+1}}{n+1}
%=\frac{\olambda(T-s)}{(n+1)!}e^{\ulambda s}
%\end{align*}
%\end{proof}