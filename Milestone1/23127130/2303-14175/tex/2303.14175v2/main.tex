\documentclass[final]{midl} % Include author names
...
\Name{Jie Yang\nametag{$^{1}$}} \Email{jieyang5@link.cuhk.edu.cn} \\
\Name{Si-Qi Liu\nametag{$^{1}$}} \Email{siqiliu@sribd.cn} \\
\Name{Ruimao Zhang\nametag{$^{1,*}$}} \Email{ruimao.zhang@ieee.org} \\
% \Name{Ruimao Zhang \midlotherjointauthor\nametag{$^{1}$}} \Email{xyz@sample.edu}\\
...
\addr $^{1}$ Shenzhen Research Institute of Big Data, The Chinese Univeristy of Hong Kong(Shenzhen), China \\
...
%
%In this paper, we propose a novel Inherent Consistent Learning (ICL) method to progressively learn the semantic consistency of category representations from labeled and unlabeled data. 
In this paper, we propose a novel Inherent Consistent Learning (ICL) method, which aims to learn robust semantic category representations through the semantic consistency guidance of labeled and unlabeled data to help segmentation.
%
%
%that aims to  learn the structured context information to help segmentation with the semantic consistency guidance of labeled and unlabeled data. 
%
In practice, we introduce two external modules namely Supervised Semantic Proxy Adaptor (SSPA) and Unsupervised Semantic Consistent Learner (USCL) that based on the attention mechanism to align the semantic category representations of labeled and unlabeled data, as well as update the global semantic representations over the entire training set. 
%
%Once the model is optimized, these external modules are discarded to maintain the simplicity of the main architecture.
%
...
%
%e.g., CNN-based v.s. Transformer-based, or 2D-based v.s. 3D-based.
%
...
%Anatomical organ or tumor segmentation has aroused extensive attention in recent years, for their pixel/voxel-wise visual guidance towards various medical imaging analysis tasks like disease diagnosis and radiation therapy \cite{deepigeos, sahiner2019deep}. 
Anatomical organ or tumor segmentation has aroused extensive attention in recent years due to the helpful pixel/voxel-wise visual guidance in various medical imaging analysis tasks like disease diagnosis and radiation therapy~\cite{deepigeos, sahiner2019deep}. With the evolution of deep learning techniques, popular networks have also been applied in this field and achieved huge success~\cite{segmenter, transunet, swinunetr}. However, these approaches are highly dependent on large-scale annotated data due to the data-driven nature of deep networks. % and annotation accuracy. 
Different from natural images, the annotation costs of medical images are much larger since it must be done by well-trained doctors~\cite{medsurvey, amos22}. 

To mitigate the need of large amount of annotated data, Semi-Supervised Learning (SSL) has been introduced %and widely explored 
to learn from scarce annotated data and become a crucial and challenging problem in the medical imaging analysis community. 
%
%While non-professionals can annotate general image datasets, the labeling of the medical imaging data must be done by 2\textasciitilde3 middle- and senior-level professional imaging doctors to ensure the labeling quality \cite{medsurvey, amos22}. Therefore, Semi-Supervised Learning (SSL) has been introduced and widely explored to learn from scarce annotated data, becoming a crucial and challenging problem in medical imaging analysis community.
%
%To this end, with the theoretical evolution of deep learning, various method and backbone networks have been proposed for medical segmentation tasks, among which the Vision-Transformer-based approaches \cite{segmenter, transunet, swinunetr} have attracted widespread attention for their strong generalization and great performance. However, these approaches are highly dependent on the large annotated data and annotation accuracy. While non-professionals can annotate general image datasets, the labeling of the medical imaging data must be done by 2\textasciitilde3 middle- and senior-level professional imaging doctors to ensure the labeling quality \cite{medsurvey, amos22}. Therefore, Semi-Supervised Learning (SSL) has been introduced and widely explored to learn from scarce annotated data, becoming a crucial and challenging problem in medical imaging analysis community.
%
%
%\noindent\textbf{Semi-superivsed Medical Image Segmentation.}
%Recently, many semi-supervised segmentation methods have been proposed to alleviate the dependency on the labeled data of general and medical images.
%
...
%
Self-training strategy that relies on generated pseudo labels as data expansion was first proposed by \cite{selftraining} and later followed by other variants \cite{pseudoseg, cps}; %its main idea is to find a way to refine and use the generated pseudo labels as data expansion for iterative training. 
%
Co-training methods \cite{selftraining, multiplanarcotrain, deepcotraining, selfpace, crosscnntrans} assume that different models can learn multiple independent and complementary views from the same data. By combining information from multiple perspectives %of different perspectives 
...
%believed that after applying a perturbation to an input, its intermediate representations should be consistent. Such strategies can make the model robust to perturbations, generalizing better while learning the inherent invariance of the data.
%
However, most of them follow the standard solution that relies on low-level features for consistency regularization or co-training while ignoring the global high-level features and their rich semantic information. 
%
For instance, \cite{crosscnntrans} leverage different learning paradigms of CNN and Transfomer to perform consistency regularization on the output-level pseudo labels. 
%
Although the method achieved the current SOTA performance, experimental results show its limitation on the discrimination of some specific categories, especially when the background has a similar appearance to the foreground categories.


%experimental results show limited discrimination of some specific categories, especially when the background has similar and ambiguous representations as the foreground categories.

%

%In this work, we start from the basic medical knowledge that the relative position of organs is fixed for different subjects (\eg, liver is on the right side of the spleen). Being aware of neighbor organs around could be helpful for the segmentation of the target organ.
%Therefore, we want to learn the structured context information to help segmentation with the semantic consistency guidance of labeled and unlabeled data. 
...
%
%
%A simple yet effective semantic consistent scheme called Inherent Consistent Learning (ICL) is proposed.
%
%
In practice, we introduce two external modules named Supervised Semantic Proxy Adaptor (SSPA) and Unsupervised Semantic Consistent Learner (USCL) that align the semantic representations of labeled and unlabeled data and update the global category representations.
%
In practice, the global category representations (\ie, semantic-aware proxy) are initialized as the learnable parameters of SSPA and updated at multiple scales by using the feature maps of the labeled data. 
%
%multi-scale semantic-aware proxies are constructed where they are first trained in the SSPA module with labeled data 
%
%
...
%
In the test phase, both the above two modules can be dropped out to maintain the model’s simplicity.
%
%Then multi-scale consistent constraints are applied between SSPA and USCL on the segmentation map to provide detailed guidance on the entire learning process. 
%
%
%During training, the labeled data are used for supervised learning and to explicitly form the multi-scale semantic-aware proxies in our newly proposed Supervised Semantic Proxy Adaptor (SSPA) module. 
%Such semantic-aware proxies can be later used to interact with the unlabeled data in the Unsupervised Semantic Consistent Learner (USCL) module, instructively generating the semantic-guided segmentation maps. These segmentation maps can be utilized to optimize the model through multi-scale consistent supervision.
%
The main contributions are three-fold:
%
%(1) An effective and novel Inherent Consistent Learning framework is proposed to progressively learn structured context information with the semantic consistency of category representations from labeled and unlabeled data.
(1) An effective and novel Inherent Consistent Learning framework is proposed to progressively learn robust category representations with the semantic consistency between labeled and unlabeled data. 
% robust category representations
%
(2) Two plug-and-play external modules named Supervised Semantic Proxy Adaptor (SSPA) and Unsupervised Semantic Consistent Learner (USCL) to effectively leverage limited labeled data and massive unlabeled data only in the training phase. 
%
...
\centering 
\includegraphics[width=1\textwidth]{figure/overview5.png} 
...
% which align the semantic representations of labeled and unlabeled data and update the global category representations.}}
\label{Fig.main} 
...
%For the semi-supervised learning setting, the training set usually contains a small portion of labeled set $D_{l} =\left\{\left(x_{i}^{l}, y_{i}^{l} \right)\right\}_{i=1}^{n}$ and a much larger unlabeled set $D_{u} =\left\{x_{i}^{u}\right\}_{i=1}^{m}$ (n $>>$ m). The goal is to train a segmentation network by mining information from a small amount of labeled data and efficiently utilizing large amounts of unlabeled data. 
...
%The goal is to train a segmentation network by mining information from a $D_{u}$ and efficiently utilizing large amounts of unlabeled data. 
...
Specifically, based on the attention mechanism we construct a set of semantic-aware proxies that aim to preserve the discriminability towards different categories. These semantic-aware proxies are first trained with $D_{l}$ and then implicitly serve as a classifier to reveal the semantic-related areas within the unlabeled images at different representation levels. %and encourage the network to learn the structured context information 

%that can implicitly serve as a classifier to reveal the semantic-related areas within the unlabeled images and encourage the network to learn the structured context information at different representation levels.
%that have strong discriminability towards different categories representation. 
%we explicitly from the semantic-aware proxies that have strong discriminability towards different categories representation. 

%Such semantic-aware proxies can implicitly serve as a classifier to reveal the semantic-related areas within the unlabeled images and encourage the network to learn the structured context information at different representation levels. By utilizing this strategy, our model can leverage massive unlabeled images to boost the final segmentation performance of the network.

%the proposed Inherent Consistent Learning (IRL) framework gives a new perspective for semi-supervised segmentation. \textcolor{blue}{Through limited labeled images and attention mechanism, we explicitly from the semantic-aware proxies that have strong discriminability towards different categories representation. Such semantic-aware proxies can implicitly serve as a classifier to reveal the semantic-related areas within the unlabeled images and encourage the network to learn the structured context information at different representation levels. By utilizing this strategy, our model can leverage massive unlabeled images to boost the final segmentation performance of the network.} 
%

As illustrated in Fig.~\ref{Fig.main}, our framework consists of a single encoder-decoder model with a segmentation head, two newly proposed external modules named Supervised Semantic Proxy Adaptor (SSPA) and Unsupervised Semantic Consistent Learner (USCL), and an initialized semantic-aware proxy $Q \in \mathbb{R} ^{Z \times 4C}$ that aims at transferring the well-learned semantic representation at different sacles from limited labeled data. $Z$ and $C$ indicate the number of categories (including the background) and the tokenized feature dimension of the last scale. $4C$ is the dimension of the current proxy due to the scale factor. In training, both $x^{l}$ and $x^{u}$ are fed to the encoder-decoder backbone, generating multi-scale intermediate features $\left \{F^{l}, F^{u}  \right \}$ for the SSPA and USCL modules to perform inherent consistent learning scheme by interacting with the initialized semantic-aware proxy. It is worth noting that such a strategy is suitable for both 2D and 3D networks, and these external modules are discarded during the inference phase. 
%
%后面的内容要不要直接省略掉了？感觉和2.1和2.2有重复，或许这里简单带过，然后说details in section 2.1 and 2.2%
% \textcolor{red}{The labeled features $f_{l}$ are then sent to the SSPA module with the initialized semantic-aware proxy.
% %
% Supervised by ground truth, the SSPA can explicitly extract the inherent semantic information through the proxy and the generated multi-scale semantic-aware segmentation maps. 
% %
% By transferring the learned proxy as guidance to the USCL module along with the $f_{u}$, semantic-guided segmentation maps can be generated and later used to perform consistency regularization with the predictions of the unlabeled data. In addition, the unlabeled data can also go through the SSPA module to produce more accurate semantic-aware maps to better form the USCL module with the consistency learning. It is worth noting that such strategy are suitable for both 2D and 3D networks, and both of the external modules are discarded during the inference phase.}
%And then they are sent to 
%the SSPA and USCL modules to perform inherent consistent learning scheme by interacting with the initialized semantic-aware proxy. It is worth noting that such a strategy is suitable for both 2D and 3D networks, and these external modules are discarded during the inference phase. 
...
\label{SSPA}
We first define a learnable semantic-aware proxy as query that aims to learn the statistics of organ categories~\cite{xie2021segmenting}, and after that we utilize the proxy to calculate the attention maps with the output features from the decoder. With a small convolutional head applied to attention maps, final predictions can be obtained and supervised by the ground truth. % to form the learnable semantic-aware proxy and later use it for guided segmentation with unlabeled data in Unsupervised Semantic Consistent Learner module. 
Later the well-trained proxy will be used to guide the learning of USCL module.

%
For simplicity 2D network is used for illustration. %We first feed $\left \{ x^{l}, x^{u}\right \} \in \mathbb{R} ^{ 1\times H\times W}$ to the network and obtain 
As depicted in Fig.~\ref{Fig.main}B, a set of multi-scale intermediate features $\left \{ F^{l}_\lambda , F^{l}_\lambda\right \}$ (scale $\lambda \in (1, 2, 3)$) are tokenized as  $\left \{ T^{l}_\lambda, T^{u}_\lambda\right \}$
%
and sent to a series of Cross-Attention Blocks within SSPA module as input Key and Value. 
%decoder outputs a set of multi-scale intermediate features $\left \{ F^{l}_\lambda , F^{l}_\lambda\right \}$ from both labeled and unlabeled images, where scale $\lambda \in (1, 2, 3)$. 
%$\left \{ F^{l}_\lambda, F^{l}_\lambda\right \}$ are then tokenized and sent to a series of Cross-Attention Blocks within SSPA module as input Key and Value $\left \{ T^{l}_\lambda, T^{u}_\lambda\right \} $. and the input Query is the initialized proxy $Q_0 \in \mathbb{R} ^{Z \times 4C}$.
%
...
\end{equation}
%
where $\lambda \in (1, 2, 3)$ represents three scales, $\sigma$(·) denotes the softmax operation, and $\mathbb{I}$(·) is the bilinear interpolation to upsample the predictions to the size of label $y^{l}$.

% 
%\textcolor{blue}{The semantic-aware proxy $Q_{\lambda } \in \mathbb{R} ^{Z\times \frac{4C}{\lambda } } $ will be sent to the USCL module. By providing structured context information through these proxies, massive unlabeled data can be leveraged to train the network and boost the final segmentation performance. 
%The above multi-scale semantic-aware segmentation maps $M^{u}_\lambda$ from unlabeled data will also be adopted to performer consistency learning in the USCL module}. %Details in section \textbf{2.2}}.

% In addition, at each scale, the semantic-aware proxy  and the detached semantic-aware segmentation maps $M^{u}_\lambda \in \mathbb{R}  ^{ Z\times (\frac{\lambda H}{16}\times  \frac{\lambda W}{16} )}$ 


\subsection{Unsupervised Semantic Consistent Learner}
As depicted in Fig.~\ref{Fig.main}B(right), USCL module receives semantic-aware proxies $ Q_{\lambda } \in \mathbb{R} ^{Z\times \frac{4C}{\lambda }}$ with rich category semantic information from SSPA. They are utilized to reveal the semantic-related areas of the unlabeled intermediate features at different representation levels, instructively guiding the USCL module to generate the semantic-guided attention maps $\bar{A} ^{u}_\lambda \in \mathbb{R}  ^{ Z\times (\frac{\lambda H}{16}\times  \frac{\lambda W}{16} )}$ at different scale $\lambda$. Similar to SSPA module, these attention maps are later used to get the guided segmentation maps $G^{u}_\lambda  \in \mathbb{R}  ^{ Z\times \frac{\lambda H}{16} \times \frac{\lambda W}{16}}$, but the difference is the USCL module only process data from unlabeled source. By applying dice loss between the prediction $p^{u} $ of the network and $G^{u}_\lambda$, we can efficiently leverage the large amount of unlabeled data to boost the final segmentation performance of the main network:
%
\begin{equation}
...
%
%Furthermore, we detach the semantic-aware segmentation maps $M^{u}_\lambda \in \mathbb{R}  ^{ Z\times \frac{\lambda H}{16} \times \frac{\lambda W}{16}}$ from SSPA model to perform consistency supervision as metioned in Sec.\ref{Sec.2.1}. \textcolor{red}{Todo:}Since these segmentation maps are generated through the well-supervised SSPA module, they can provide more reliable consistency regularization to better form the USCL module using the mean squared error (MSE) loss:
The learned proxies, which are used to guide USCL, are updated via SSPA with a new batch of samples.
Considering the perturbation of the proxies introduced in this update process, %learned proxies that are used to guide the USCL and SSPA are different, 
inconsistency may exist in the segmentation maps of these two modules given the same unlabeled input sample. 
%
Different from the training strategy of SSPA, there is no ground-truth segmentation mask to directly supervise USCL. %Since SSPA has been well-trained with labeled data before,
%
We leverage the above inconsistency to learn some discriminative features from the unlabeled data. 
%
...
%We want to further enhance the robustness of USCL in detail by comparing $G^{u}_\lambda$ with $M^{u}_\lambda$ at different scales. Mean squared error (MSE) loss is adopted to conduct such consistency regularization as follows:
%
\begin{equation}
...
\end{equation}
Note that here the semantic-aware segmentation maps $M^{u}_\lambda \in \mathbb{R}  ^{ Z\times \frac{\lambda H}{16} \times \frac{\lambda W}{16}}$ is detached to avoid disturbance of the well-trained SSPA.  %\TODO{Or, here the SSPA will not be updated since it is well trained before?}
%
...
\textbf{ACDC} dataset contains 200 cardiac cine-MR images with three segmentation targets, the left ventricle (LV), the myocardium (Myo) and the right ventricle (RV). For fair comparison, we perform 2D segmentation and use the same data split, pre-processing and augmentations following \cite{crosscnntrans}. Specifically, We use 2D slices from 3 and 7 cases (6/140 and 14/140 scans) for training and validate on the remaining 60 scans.
%


%

\textbf{AMOS} dataset provides 500 CT and 100 MRI scans, each with voxel-level annotations of 15 abdominal organs such as Spleen, Left and Right kidney, etc. In this work, we only use 300 CT scans from the AMOS 2022 official grand challenge \cite{amos22}, including 200 scans for training and 100 scans for validation. In detail, we divide the original validation set in a ratio of 3:7 to obtain the validation set and test set. Furthermore, 15/200 and 30/200 scans in the training set are used as two different semi-supervised settings.
%

During validation phase, we perform slice-to-slice inference for 2D segmentation and stack them into a 3D prediction volume. For 3D segmentation, sliding window inference is applied to get the 3D predictions. To evaluate the segmentation results, we use the metrics 1) Dice Coefficient (DSC) and 2) 95\% Hausdorff Distance (HD95).


\subsection{Experimental details}
\textbf{Network architectures.} Since we propose a plug-and-play scheme, we investigate the performance based on various different networks, including CNN-Based networks for 2D and 3D segmentation namely 2D- and 3D-UNet \cite{2dunet}. (More experiments on Transformer-Based (Trans-Based) networks of SwinUNet \cite{swinunet} and SwinUNETR \cite{swinunetr} in Sec.~\ref{add_exp_acdc}, ~\ref{exp_brats}.)
%

\noindent\textbf{Comparison with baselines and existing methods.} We compare our proposed framework with four baselines and some recent SOTA semi-supervised segmentation methods, including Entropy Minimization (Ent-Mini) \cite{entromini}, Cross Consistency Training (CCT) \cite{crossconsistency}, FixMatch \cite{fixmatch}, Regularized Dropout (R-Drop) \cite{r-drop}, Cross Pseudo Supervision (CPS) \cite{cps}, Uncertainty Rectified Pyramid Consistency (URPC) \cite{urpc} and Cross Teaching between CNN and Transformer (CTCT) \cite{crosscnntrans}. For a fair comparison, all implementations of these methods are consistent with our framework under the same task. During validation, all the auxiliary training modules are discarded and only the trained backbones are used to produce final predictions. The above methods are open available \cite{ssl4mis}.

%\section{Main Results}

\subsection{Results of 2D segmentation on ACDC dataset}
\textbf{Improvements over baselines.} 
Tab.\ref{table:1} presents the results of all methods on ACDC dataset using CNN-Based models (UNet). As the results show, all methods have improvement over the baseline under 3 and 7 labeled cases. Specifically, our proposed method surpasses the baseline by a large margin, with an average improvement of 28\% in DSC and 43mm in HD95 under 3 labeled cases, 11.5\% in DSC and 6.6mm in HD95 under 7 labeled cases. Especially for the category RV, our method improves the DSC from 41.26\% to 81.75\% and reduces the HD95 from 79.78mm to 3.56mm using only 3 labeled cases. 
% our method still has promising improvements in DSC and HD95 when using Trans-Based backbone, yet some of the compared approaches suffer from declined performance.

%
\noindent\textbf{Comparison with SOTA.}
Compared with some SOTA methods, our approach outperforms the best under all partition protocols, especially when the number of labeled scans is extremely limited. In detail, Tab.\ref{table:1} shows that our method significantly suppasses the current SOTA method CTCT \cite{crosscnntrans} by 8.21\% in DSC and 14mm in HD95 under 3 labeled cases. And we also achieve great improvement in DSC and HD95 (2.2\% larger and 3.16mm smaller than CTCT) using only 7 labeled cases. The first three columns in Fig.~\ref{Fig.vis_total}(left) present visualization of the segmentation performance of different methods. Compared with others, our method presents more stable results for hard-to-segment categories like the RV, and even stronger discrimination for the background and the foreground categories when they have similar and ambiguous representations.
% our method still has promising improvements in DSC and HD95 when using Trans-Based backbone, yet some of the compared approaches suffer from declined performance.
% In the lower half of Tab.\ref{table:1}, we surpasses CTCT using the same Trans backbone by 2.1\% DSC and 2.44mm in HD95 under 7 labeled scans. We can also notice that our method has significant effects on the metric HD95, where we achieve 1.91mm and the fully supervised baseline is 1.56mm.
...
\textbf{Improvements over baselines.} 
%
Tab.\ref{table:2} reports partial quantitative comparison results of our method and others under 15/200 and 30/200 labeled settings (Complete results in Tab.~\ref{table:add_amos_tab}). As presented in the Tab.\ref{table:2}, the results show that our method exceeds the baselines by a large margin, especially under 15/200 protocol, achieving the tremendous improvement of over 30\% in mean DSC and 33.5mm in mean HD95. When using 30 labeled scans, there is still an obvious improvement over the baseline, with a gain of 6.3\% in mean DSC and 3.6mm in mean HD95. Although all methods show enhancement under 15 labeled scans, Ent-Mini gets degraded results when using 30 labeled scans.

\noindent\textbf{Comparison with SOTA.}
%
Here we conduct comparative experiments on two existing methods considering the training cost and resource limitation. As presented in Tab.\ref{table:2}, our proposed method attains mean DSC and HD95 of 61.44\% and 17.85mm under 15 labeled scans, 74.81\% and 9.87mm under 30 labeled scans, which outperforms the SOTA methods Ent-Mini and CTCT by a large margin. Particularly, under the least labeled data setting (15/200), our method surpasses the Ent-Mini and CTCT with the improvements of 11.7\% and 20.6\% in DSC, 8.1mm and 46.1mm in HD95, respectively. We also notice that our proposed method has remarkable performance when segmenting medium and small size organs that are hard to be distinguished especially with limited labeled training data. The visual comparison base on 30 labeled data is presented in the last four columns in Fig.~\ref{Fig.vis_total}(left) and shows that our method can generate better results with fewer false-positive regions, especially on medium size and small size organs.
...
\noindent\textbf{Analysis of the discrimination ability towards the semantic-aware proxy.} In this section, we produce qualitative analysis to demonstrate the semantic-aware proxy can well distinguish the semantic representation of different categories. In details, we utilize the well-learned proxy to interact with all labeled data successively in the first scale of SSPA module. By regarding each updated proxy as a sample, we apply the t-SNE \cite{tsne} to visualize the proxy distribution of different categories in ACDC and AMOS datasets. In Fig.\ref{Fig.vis_total}(right), different categories are clearly separated with a larger metric distance, proving the discriminability of our semantic-aware proxy.

\noindent\textbf{Analysis of the Supervised Semantic Proxy Adaptor.}
In this section, we conduct experiments on ACDC dataset using 7 labeled cases and without unlabeled data to further investigate the effectiveness of the Supervised Semantic Proxy Adaptor. Particularly, we utilize UNet and SwinUNet as our backbone modules. As presented in the Tab.~\ref{table:Adaptor-ACDC-main}, our proposed SSPA module brings promising gains to the segmentation results over all categories on the CNN-Based and Trans-Based network. Specifically, the SSPA module improves 5\% in the mean DSC for Unet, and boosts not only 2.5\% in the mean DSC but also reduces 3.7mm in HD95 for SwinUNet. More experiments can be found in Sec.~\ref{Adaptor}.
...
No.2022A1515011524, by Shenzhen Science and Technology Program ZDSYS20211021111415025, and by the Guangdong Provincial Key Laboratory of Big Data Computing, The Chinese Univeristy of Hong Kong (Shenzhen). }
...
%
%\textit{i.e.} $\frac{H}{4P} \times \frac{W}{4P}$.
%
%
...
\textbf{BraTS} dataset includes 335 MR scans from  four different modalities (FLARE, T1, T1ce and T2). Here we conduct 3D segmentation using the whole tumors from FLARE (250/335) following \cite{urpc}, utilizing the same data split, augmentations and pre-processing strategy. For semi-supervised learning setting, we use 25/250 and 50/250 scans as different portions of the training sets, and 85 scans as validation set.
\subsection{AMOS dataset}
\subsubsection{Category details.} AMOS dataset aims to promote the abdominal multi-organ segmentation under diverse, complex and clinical scenarios. To get a clearer vision on the effectiveness of our proposed method, we objectively divide the abdominal organs into three different size as follows:
...
\subsubsection{Data splits.} In AMOS dataset, the collected 300 CT scans are derived from three distinct scanners (domains) to ensure the data diversity including 200 scans for training and 100 scans for validation. In particular, we randomly selected one sample of each domain from the training set and repeat 5 times, obtaining 15 labeled scans for the first semi-supervised setting. To get the other semi-supervised setting, we repeated the selection 5 more times and get 30 labeled scans with the former acquired 15 labeled scans. Furthermore, we perform the similar strategy to choose 30 scans to form the validation set to validate and select the best model during training, and the remaining 70 scans are used for later testing.


\subsection{Training details.} \label{training_details}
Our experiments are implemented in Pytorch \cite{pytorch} using an NVIDIA A100 GPU. All of the networks including 2D and 3D are trained by an SGD optimizer with the polylearning rate strategy. Specifically, during training, a batch of data consists of half labeled data
and half unlabeled data.
%

\textbf{2D networks.}
For the ACDC dataset, we train the 2D models for 30000 iterations with an initial learning rate (lr) 0.01 and a batch size of 16 and validate the models every 200 iterations finding the best models to make the final comparison.  

%
\textbf{3D networks.}
For the BraTS dataset, we set the batch size to 4 with an initial lr 0.01 and train all 3D models for 30000 iterations. Considering that the AMOS22 dataset with multiple categories may take a longer time for the models to converge, we train the models on AMOS dataset for 60000 iterations with a batch size of 4 and an initial lr of 0.02. To find the best model of each model, we validate the models every 200 (on BraTS) and 1200 (on AMOS) iterations.
%
...
In Tab.~\ref{table:computational-cost}, we report the total training time and per scan inference time of different SOTA methods and our proposed method under 7 labeled cases in ACDC dataset using the same device. It can be found that our method needs longer training cost due to the attention-mechanism-based modules that we proposed. But all the methods have the similar time since we utilize the same backbone network to perform inference.

\begin{table}[h]
    \centering
    \caption{Comparison of computational cost of the different SOTA methods and our proposed method under 7 labeled cases on ACDC dataset. TTimes (hours) denotes the total training time and ITimes (seconds) denotes the inference time per scans.}
    \scalebox{0.95}{
    %``Params'' indicates the parameter size of backbone. ``Fold X'' means per cross-validation split settings. ``Mean'' is the average result of all settings.}
...
To investigate the generalization ability of our proposed method, we further conduct additional experiments adopting Trans-Based models on ACDC and both Cnn-Based and Trans-Based for BraTS dataset. And these additional experimental results further demonstrate the generality of our proposed method. Besides, we presents the full results of all categories in AMOS dataset based on CNN-Based as a complement in Tab.~\ref{table:add_amos_tab}. 
...
When comparing with the current SOTA method using Trans-Based model, we surpass the CTCT by 3.1\% in mean DSC and 4.5mm in mean HD95 under 3 labeled cases, 2\% in mean DSC and 2.4mm in mean HD95. We can also notice that our method has significant effects on the metric HD95, where we achieve 1.91mm extremely close to the fully supervised baseline performance (1.56mm).

\begin{table}[h]
    \centering
    \caption{Comparisons of the SOTA methods on ACDC using Trans-Based models. Mean and standard variance (in parentheses) are presented in the table.}
    \scalebox{0.56}{
    %``Params'' indicates the parameter size of backbone. ``Fold X'' means per cross-validation split settings. ``Mean'' is the average result of all settings.}
...
\begin{table}[h]
    \centering
    \caption{Effects of Supervised Semantic Proxy Adaptor on ACDC dataset. Mean and standard variance (in parentheses) are presented in the table.}
    \scalebox{0.57}{
    %``Params'' indicates the parameter size of backbone. ``Fold X'' means per cross-validation split settings. ``Mean'' is the average result of all settings.}
...
\begin{table}[t]
    \centering
    \caption{Effects of Supervised Semantic Proxy Adaptor on AMOS dataset. Mean and standard variance (in parentheses) are presented in the table.} 
...
For semi-supervised learning, the selection of the labeled data can greatly affect the model's performance. Therefore, to further study the influence of different limited labeled data on our method, we randomly selected three subsets of data based on the experimental settings of the ACDC dataset with 3 labeled cases and compared the experimental results with Baseline UNet \cite{2dunet}, the current SOTA methohd CTCT \cite{crosscnntrans}. Specifically, three selected subsets have different number of 2D slices, which is 58, 56 and 58 slices, respectively. And the original split of 3 labeled cases contains 68 slices. As shown by the experimental results in Tab.~\ref{table:variablity_data}, different subset selections influence the model's performance more or less. Compared with the training results from the original split, both UNet and CTCT have larger fluctuations on both DSC and HD95. Although the performance of our proposed method has a slight decline (fewer 2D slices were used), the overall result is more stable and remains superior to the SOTA method. Experimental results further demonstrate the effectiveness of our proposed method.

\begin{table}[h]
    \centering
    \caption{Comparisons of the SOTA method on ACDC using CNN-Based models under 3 different labeled subsets (not overlapped). (DSC, HD95) from the original split, UNet:(54.34, 47.25), CTCT:(74.16, 18.27), Ours: (82.37, 4.28)}
    \scalebox{0.58}{
    %``Params'' indicates the parameter size of backbone. ``Fold X'' means per cross-validation split settings. ``Mean'' is the average result of all settings.}
...
Normally, the amount of unlabeled data is very large in most datasets. Therefore, most semi-supervised learning methods utilize massive unlabeled data to assist training. However, in scenarios where the amount of unlabeled data is limited, the sensitivity and robustness of the model to the number of unlabeled data is particularly important. Therefore, it is significant to explore the impact of the amount of unlabeled data on the model's performance. In this section, we conduct experiments on ACDC dataset under 3 and 7 labeled cases using different partition of unlabeled data. Specifically, we use 1/32, 1/16, 1/8, 1/4 and 1/2 partition protocols. When using 1/32 unlabeled data, the number of scans is 2, and 1/16 is 4 scans. Detailed experimental results can be found in Tab.~\ref{table:unlabeled_num}.

Surprisingly, our model can achieve the best performance under the 3 labeled cases settings with only 1/2 unlabeled data. Also, using only 1/16 and 1/8 of the unlabeled data (8 volumes and 16 volumes) degrades the model slightly. And the model performance will drop considerably when using 1/32 of the unlabeled data (DSC from 82.37 to 71.24 and HD95 from 4.28 to 14.7). Except for 1/32 partition, our model is better than the current SOTA method CTCT under the other five settings. 

When trained with 7 labeled cases, the number of unlabeled data has less impact on the performance of our proposed model. When only 1/32 of the unlabeled data is used, our model obtains comparable results to the current SOTA method CTCT, and it is even better on HD95 (from 5.43 to 3.7). In summary, when the amount of labeled data is larger, the impact of the amount of unlabeled data on the model is smaller. Besides, our model only requires a small amount of unlabeled data to assist training and still can achieve satisfactory performance, which further illustrate the effective of our proposed method.


\begin{table}[h]
    \centering
    \caption{Effects of the number of unlabeled data used on ACDC dataset. Mean and standard variance (in parentheses) are presented in the table.}
    \scalebox{0.73}{
    %``Params'' indicates the parameter size of backbone. ``Fold X'' means per cross-validation split settings. ``Mean'' is the average result of all settings.}
...
\begin{sidewaystable}
    \centering
    \caption{Comparisons of the SOTA methods on AMOS using CNN-Based models (Complete results with all 15 categories). Mean and standard variance (in parentheses) are presented in the table.}
    \scalebox{0.43}{
    %``Params'' indicates the parameter size of backbone. ``Fold X'' means per cross-validation split settings. ``Mean'' is the average result of all settings.}
...
Here we visualized more visual results of the test datta from ACDC, BraTS and AMOS to further indicate the effectiveness of our proposed method. As shown in Fig.~\ref{Fig.acdc-3}, Fig.~\ref{Fig.acdc-7}, Fig.~\ref{Fig.brats-25} and Fig.~\ref{Fig.amos-25}, multiple results has proven the discriminability of our method towards different categories, especially for the hard-to-segment and medium, small size organs. Such as the right ventricle (RV) in ACDC datasets and adrenal glands (RAG, LAG), gallbladder (GBL) in AMOS dataset. 

\begin{figure}[h] %H为当前位置，!htb为忽略美学标准，htbp为浮动图形
\centering 
\includegraphics[width=0.9\textwidth]{figure/ACDC_samples_2.png} 
\caption{Visualization of ACDC dataset under 3 labaled cases.} 
\label{Fig.acdc-3} 
\end{figure}
...
\caption{Visualization of ACDC dataset under 7 labaled cases.} 
\label{Fig.acdc-7} 
\end{figure}
...
\caption{Visualization of BraTS dataset under 25 labaled scans.} 
\label{Fig.brats-25} 
\end{figure}
...
\caption{Visualization of AMOS dataset under 30 labaled scans.} 
\label{Fig.amos-25} 
\end{figure}
