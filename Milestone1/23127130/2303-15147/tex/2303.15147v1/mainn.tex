...
\begin{itemize}
  ...
  \item We empirically show that the proposed method outperforms the current state-of-the-art semi-supervised 3D hand pose estimation methods.% by large margins.
\end{itemize}
...
%$\Phi: x\in{\mathbb{R}^{d}} \rightarrow{x^{\prime}\in{\mathbb{R}^{d}}}$
\par
...
%The associated algorithm is shown in Alg. 1.

%one  Let $\theta_T$ and $\theta_S$ respectively be their corresponding parameters.The overview of the %proposed framework is illustrated in Fig .1.
\begin{figure}[t]
\centering
...
%because it is able to effectively extract and subsequently fuse hand features at every scale.

%The CNN provides a 2K channel output with K channels for 2D localization heatmaps H2D and K channels for
%depth maps Hzˆ
...
%transferred to the view $\Phi(x_u)$ in order to be used by its own as the ground-truth labels for the second view. 
%In practice, at each training iteration, we compute the supervised loss on a mini-batch of size $B$ of labeled examples and compute the consistency loss on a mini-batch of size $\mu B$ of unlabeled examples. We control the ratio of unlabeled examples batch size to the labeled examples batch size by $\mu$.% In our experiments, we set $\mu = 2$.

%Image classification methods traditionally take the maximum of the output probability of the model as an approximate of the model's prediction confidence on a given sample \cite{sohn2020fixmatch,xie2019unsupervised,zhang2021flexmatch}.  However, this approach is not applicable to the proposed method due to the different nature of its output.
...
%giving non-zero weights to pseudo-labels that are rejected ($m_r \neq 0$) 
...
the pseudo-labels that are rejected in the training ($m_r \neq 0$) consistently leads to performance improvement in the proposed method. %In our experiments, we set $m_a = 1$ and $m_r = 10^{-1}$.   
The L1 distance $D$ in $\mathcal{L}_{u}$ in the Eq. \ref{eqn:teacher} is replaced by the following weighted average:
...
\end{equation}
% and is set $\eta = 0.1$ in our experiments
...
%\textbf{Training Signal adjustment.} A common scenario in semi-supervised learning is where there is a huge gap between the amount of unlabeled data and that of labeled data. Under such scenarios, the model tend to quickly overfit the limited amount of labeled data while still underfitting the unlabeled data. To tackle this issue,  
%in The proposed method uses a weighting factor $\lambda$ to balance the impacts of $\mathcal{L}_{s}$ and $\mathcal{L}_{u}$ on the final training signal for the teacher network. We empirically found that adjusting $\lambda$ according to the current state of training leads to performance improvements. For example, in the beginning of training when the pseudo-labels are not accurate, $\lambda$ is assigned a low value. This value increase as the model accuracy improves over the course of training. $\lambda$ is increased from the initial value $\lambda_{start}$ to reach its final value $\lambda_{end}$ using a cosine schedule strategy similar to Eq. \ref{eqn:rhoo}.
%\par

\subsection{Student Network Training}
The proposed method trains the student network using the unlabeled samples and their corresponding pseudo-labels generated by the teacher network. However, using pseudo-labels generated directly by the teacher network can lead to a potential problem. The teacher itself is constantly updated in the training, which could cause performance degradation and training instability in the student network since it has to learn to approximate a highly non-stationary function. %Secondly, using batch normalization (BN) \cite{ioffe2015batch} by the teacher network leads to cross-sample dependency of pseudo-labels, where the model's estimation on a given sample is dependent on all other samples in the same batch \cite{cai2021exponential}. 
An alternative is to use the exponential moving average (EMA) of the teacher network's parameters to generate pseudo-labels \cite{tarvainen2017mean}. However, this approach could lead to a potential mismatch between the EMA parameters and the batch normalization (BN) statistics in the parameter space  \cite{cai2021exponential} because the EMA parameters are averaged from the previous iterations, but the batch-wise BN statistics are instantly collected at the current iteration. We use the recently proposed fix for this issue called EMAN \cite{cai2021exponential}, where the batch-wise statistics are exponentially averaged from the previous iterations as well. The student network is trained by solving the following optimization problem:
...
%EMAN greatly improves the stability of learning by constraining the target values for the student network to change more slowly.
% To constrain the target values for the student network to change more slowly and hence improve the learning stability, we use the exponentially moving averaged  parameters of the teacher network for generating the pseudo-labels. Specifically, we use the recently proposed fix for EMA called EMAN \cite{cai2021exponential}, where the batch-wise statistics are exponentially averaged from the previous iterations as well to avoid a potential model parameter mismatch problem.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% We train 8 epochs for the ICVL training set and 58 epochs for the other training sets. 
%on a machine with 4 GPUs. 
%we use an initial learning rate of 0.03 with a cosine learning rate decay schedule \cite{loshchilov2016sgdr}.
%Hyper parameters to mentioned here: mu
%%he EMAN momentum m = 0.999.
%For fine-tuning, we use the learning rate $10^{-4}$and a batch size of 128.
%${\rho}_{end} and {\rho}_{start}$
%and batch size of 32 is used to optimize all models.
...
%For the evaluation of the proposed method, we use the two most commonly used metrics: the mean distance error (in mm) and the success rate. The mean distance error represents the average Euclidean distance between the estimated and the ground-truth joint locations computed over the entire testing set. The success rate is defined as the percentage of the frames for which the mean distance error is less than a certain distance threshold.
...
%requires the most efficient use of unlabeled data. We use the NYU dataset for performing ablation study.
\par
...
%\textbf{Effectiveness of Dynamic Thresholding.} We examine the effectiveness of the the proposed dynamic thresholding strategy in our framework. This strategy not only eliminates the need for hyper-parameter optimization for joint thresholds but also to a large degree avoids the issue of class-imbalance by enforcing a roughly equal probability for each joint pseudo-label of being accepted for training in each epoch. As can be seen in Table X,  this strategy leads to a performance improvement of 0.3~mm. %We also conduct a comparison between different schedules for the adjustment of ${\rho}_t$. Based on the results, the best strategy is to increase ${\rho}_t$ according to a cosine scheduling strategy.
\par
%%Mean Sampling. Mean sampling that assigns an equal probability for each class to be chosen has
%been shown to be better than random sampling when facing skewed data distribution [28, 22]. This
%is because in such situations, especially when the imbalanced ratio $\lambda$ is extremely large, random
%sampling will cause the tail classes to be sampled too few thus the model underfits on them. Instead,
%mean sampling which gives the same probability $\mu_j = k$ for all classes apparently has a better
%performance on a balanced test set. 

%\textbf{Effectiveness of training signal Annealing.}
%\par
...
%\textbf{Impact of Using a Separate Network as the Student.} Empirical evidence demonstrate that using a separate network as the student leads to further improvements (see Table X). We conjuncture that this improvement primarily stems from the fact the the student network is trained using a more stationary data distribution (only the pairs of unlabeled examples and their pseudo-labels provided by the EMAN of the teacher network's parameters), as opposed to the teacher network that is trained using a combination of distributions (pairs of labeled examples and their ground-truth labels and pairs of unlabeled examples and their corresponding pseudo-labels). %Note that this approach comes at no additional cost at test time as the teacher network is discarded after training and only the student network is used for inference.
\par
%\textbf{Impact of Post-training Fine-tuning of the Student.} Since the student network is trained only using the unlabeled samples and their corresponding pseudo-labels, it is fine-tuned on the available labeled data after the training. To verify the effectiveness of fine-tuning, we compare the performance of the student network with and without fine-tuning. As can be seen in Table X, fine-tuning leads to further performance improvement. 
%\par

\textbf{Impact of Using EMAN for Generating Pseudo-Labels.} To analyse the impact of using EMAN of the teacher network's parameters (instead of the parameters themselves) for generating the training pseudo-labels for the student network, we compare the performance of the student network in three cases in terms of the parameters used for generating pseudo-labels for its training: 1) the teacher network's parameters, 2) EMA-teacher \cite{tarvainen2017mean} and 3) EMAN-teacher \cite{cai2021exponential}. As can be seen in Table~\ref{tab:params}, the best performing case is when we use EMAN. EMAN greatly improves the stability of learning by constraining the target values for the student network to change more slowly.%, leading to a performance improvement of the student network.
%More stable semantic meaning
%As can be seen in Table X, using EMA renders the method prone to divergence due to the model parameter mismatch problem \cite{cai2021exponential}.
%Note that although the student is still cross-sample dependent, this is a less serious issue than the cross-sample dependency in the teacher. 

%More specifically, adding the remainder of $75\%$ ground-truth annotations leads to only modest improvements of 0.1 mm and 0.2mm on NYU and ICVL respectively. 
\begin{table}[t]
...
%Note that to mitigate the requirement for large amounts of annotated training data, 
