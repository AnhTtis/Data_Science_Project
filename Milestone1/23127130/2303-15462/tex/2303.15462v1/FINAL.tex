...
\tilde{\alpha}_{ab,c}&=\alpha_{ab,c}(f)+\mathrm{i}\alpha_{ab,c}(g)-\mathrm{i}[\alpha^\prime_{ab,c}(f)+\alpha^\prime_{ab,c}(g)], \label{alphamu}
\end{align}
...
For the static electric field, we consider a field strength of $\mathcal{E}_z=1.00\,\mathrm{kV}\,\mathrm{mm}^{-1}$, readily achievable using a high voltage source. For the standing wave, we consider field strengths of $\mathcal{E}_y=\mathcal{E}_x=1.00\,\mathrm{kV}\,\mathrm{mm}^{-1}$ (intensity $I=\epsilon_0c\mathcal{E}_y\mathcal{E}_x=2.65\times 10^5\,\mathrm{W}\,\mathrm{cm}^{-2}$) and a near-infrared wavelength of $2\pi/k=1.064\,\mu\mathrm{m}$, readily achievable using an Nd:YAG laser. Much stronger fields are possible, giving rise to even stronger chiral optical forces than those shown below. To calculate these forces reliably, however, a non-peturbative treatment of our chiral optical force is required, which is beyond the scope of this paper.
...
\begin{align}
\frac{\Delta U}{k_B}&=-41.6\,\mu\mathrm{K}. \nonumber
\end{align}
Our chiral optical force is negative for some rotational states and positive for others, however there are $908$ rotational states with $F_z<0$ and only $862$ rotational states with $F_z>0$, giving an overall preference for $F_z<0$. We emphasise here than for a given configuration of the electric field, our chiral optical force has opposite signs for opposite enantiomers in the same rotational state.
...
\begin{align}
\frac{\Delta U}{k_B}&=-26.4\,\mu\mathrm{K}. \nonumber
\end{align}
...
For the helicity lattice, we consider an angular separation of $2\vartheta=60.0^\circ$ and field strengths of $\mathcal{E}_\|=\mathcal{E}_\bot=1.00\,\mathrm{kV}\,\mathrm{mm}^{-1}$, matching the field strengths $\mathcal{E}_y=\mathcal{E}_x=1.00\,\mathrm{kV}\,\mathrm{mm}^{-1}$ of the standing wave to give a fair comparison.
...
\mathcal{R}&\sim 10^7 \nonumber
\end{align}
or higher are possible. That is to say, a disparity in strength of \textit{seven} orders of magnitude or more. Even higher ratios can be found at lower frequencies, as $\mathcal{R}\propto 1/\omega$ far off resonance. 
...
Such comparisons demonstrate that potential applications identified elsewhere for other chiral optical forces \cite{Kucirka96a, Canaguier-Durand13a, Cameron14b, Cameron14c, Bradshaw15a, Rukhlenko16a, Marichez19a, Fang21a, Kakkanattu21a, Stickler21a, Isaule22a, Genet22a, Forbes22a} should be enormously more robust and easier to implement using our chiral optical force instead, as claimed in Sec. \ref{Introduction}. 
...
Taking our lead from nature, we have identified a chiral optical force for small chiral molecules that can be several orders of magnitude stronger than other chiral optical forces proposed to date. Our chiral optical force should render potential applications enormously more robust and easier to implement than previously though possible. 
...
% First published 28th of November 2012.
...
% THESE ARE THE OTHER ELECTRIC-DIPOLE REFERENCES
...
% THIS IS THE END OF THE OTHER ELECTRIC-DIPOLE REFERENCES
...
\end{align}
where $k\in\{-J,\dots,J\}$ is the molecule-fixed rotational angular momentum projection quantum number, the corresponding wavefunctions $\langle\theta,\phi,\chi|J,k,m\rangle^{(0)}$ being given by
\begin{align}
\langle \theta,\phi,\chi|J,k,m\rangle^{(0)}&=\sqrt{\frac{(J+m)!(J-m)!(J+k)!(J-k)!(2J+1)}{8\pi^2}} \nonumber \\
&
\times\sum_{\sigma=\mathrm{max}(0,k-m)}^{\mathrm{min}(J-m,J+k)}(-1)^\sigma\frac{(\cos\frac{1}{2}\theta)^{2J+k-m-2\sigma}(-\sin\frac{1}{2}\theta)^{m-k+2\sigma}}{\sigma!(J-m-\sigma)!(m-k+\sigma)!(J+k-\sigma)!}\mathrm{e}^{\mathrm{i}m\phi}\mathrm{e}^{\mathrm{i}k\chi}. \nonumber
\end{align}
 The $|J,k,m\rangle^{(0)}$ render the unperturbed rotational Hamiltonian $H^{(0)}$ block diagonal in $J$, as
\begin{align}
