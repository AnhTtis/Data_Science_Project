...
1+ \delta(x,\tau) = \bigg|\frac{\d^3 x}{\d^3 q}\bigg| = \frac{1}{J(q,\tau)}\,,
\end{eqnarray}
...
The critical point of  these equations  (i.e $ {\d S}/{\d s}\big|_{s=0} =0$ gives the geodesic equation  $u^c \nabla_{c} u^a = 0$. 
...
In the Newtonian limit, $E_{ij} \approx \partial_{i} \partial_{j} \Phi$, where   $\Phi$ is the external gravitational potential and $E_{ij} = \partial_{\<i} \partial_{j\>} \Phi =  \partial_{(i} \partial_{j)} \Phi -\partial^2 \Phi/3$ and ${H}_{ab}$ vanishes in the Newtonian limit. 
...
+ \frac{1}{2} R_{\<ab\>}
\end{eqnarray}
Finally, from equation \eqref{eq:expansion} we can obtain the time-like geodesic version of the focussing theorem~cite{HawkingandEllis:1973lsss.book,Poisson:2003nc}%\cite{Poisson:2003nc}.
...
$ \xi^{i}(\tau,{\x}) = J_{ij}(\tau,{\x}) \xi^{j}(\tau_{\rm{ini}},{\q})\,,$
 ...
 \end{eqnarray}
  Although the relationships between various components of $ J_{i}$ and $K_{i k } $ are important(see \cite{Clarkson:2011br} in the case null geodesics),  our interest at the moment  is on the determinant which is given by
...
\end{eqnarray}
where ${\rm{det}[J]}$ is the determinant of the Jacobian.   Integrating equation \eqref{eq:volumeelement} gives
...
\end{eqnarray}
 At  $ \Theta= 0$,  the Jacobian becomes ${\rm{det}}[J](\tau, {\x}_{\rm{dp}}) = {\rm{det}}[J](\tau_{\rm{ini}}, {\x}_{\rm{dp}})$, where ${\x}_{\rm{dp}}$ defines  a spatial location where  $ \Theta = 0 $.  At this location, the property of the fluid element changes; it becomes an incompressible fluid~\cite{Fodor:1998jp,cushman2011introduction}.
...
Within this region, the one-parameter family of geodesics are converging to a singularity/caustic according to the focusing theorem. The locations where $\Theta_{L} <\Theta_H$ (e.g. void, vacuum) are expanding and will continue to expand.  These are the expanding regions. 
...
\\
 ...
 \end{eqnarray}
 Applying these to the Jacobi formula (equation \eqref{eq:volumeelement}) and imposing  $ \Theta(\tau',{\x}_{\rm{dp}})= 0$,  leads to 
 \begin{eqnarray}
 \frac{1}{ {\rm{det}[J](\tau,{\x}_{\rm{dp}})} }   \frac{\d {\rm{det} [J](\tau,{\x}_{\rm{dp}})}}{\d \tau}\bigg|_{\tau = \tau'}&=&
 - \left[ {\sigma}_{ab}{\sigma}^{ab}  + {R}_{ab} {u}^a {u}^b\right]\Delta \tau + \mathcal{O}(\Delta \tau)^2\,.
 \label{eq:translation2}
\end{eqnarray}
We made use of equation \eqref{eq:expansion} to replace $\dot{\Theta}$. 
 Solving for $ {\rm{det}[J]}(\tau, {\x}_{\rm{dp}}) $ by integrating both sides gives 
 \begin{eqnarray}\label{eq:volumeelement2}
 {\rm{det}}[J](\tau,{\x}_{\rm{dp}}) = {\rm{det}[J]}(\tau_{\tau_{\rm{ini}}},{\x}_{\rm{dp}}) \exp\left\{-\bigg[ \<R_{ab}  u^{a} u^{b}\>_{\tau}+  \<{\sigma}_{ab}{\sigma}^{ab} \>_{\tau}\bigg]   \int_{\tau_{\rm{ini}}}^{\tau} \d \tau' \Delta \tau'\right\}\,,
 \end{eqnarray}
  where we have defined the time-weighted average of the energy density and shear scalar
 \begin{eqnarray}
 \<R_{ab}  u^{a} u^{b}\>_{\tau}  &=& \frac{1}{ \int_{\tau_{\tau{\rm{ini}}}}^{\tau} \d \tau' \Delta \tau'} \int_{\tau{\rm{ini}}}^{\tau} \d \tau ' R_{ab}  u^{a} u^{b} \Delta \tau'\,,~~~~~
% \\
 \<{\sigma}_{ab}{\sigma}^{ab} \>_{\tau}=  \frac{1}{ \int_{\tau_{\rm{ini}}}^{\tau} \d \tau' \Delta \tau'} \int_{\tau_{\rm{ini}}}^{\tau_c} \d \tau' {\sigma}_{ab}{\sigma}^{ab}  \Delta \tau'\,.
 \end{eqnarray}
For  $ \int_{\tau_{\rm{ini}}}^{\tau_c} \d \tau' \Delta \tau'  > 0$ and assuming that the weak energy condition holds $ \<R_{\alpha\beta}  u^{\alpha} u^{\beta}\>_{\tau}  \ge 0$ and ${\sigma}_{ab}{\sigma}^{ab}$ is positive definite therefore $  \<{\sigma}_{ab}{\sigma}^{ab} \>_{\tau}  \ge0$, this implies that  $ {\rm{det}[J]}(\tau')  \to 0$ in a finite proper time of the same geodesic.
This shows that the global Hubble expansion breaks down at ${\x}_{\rm{dp}}$ and any attempt to extend the same one-parameter family of geodesics beyond the causal horizon will end in a singularity or a caustic or a conjugate point. 
There is another crucial piece of information which we have mentioned earlier that at $\Theta(\tau,{r}_{\rm{dp}})  = 0$, the determinant of the Jacobian ($ {\rm{det}}[J](\tau, {\x})$) is a non-zero constant. By the inverse function theorem, this indicates that we can always find another more suitable one-parameter family of geodesics or coordinates in the immediate neighbourhood of the causal horizon.  We show in section \ref{sec:vorticit_generation} that a change of coordinates in the neighbourhood of the causal horizon leads to a generation of vorticity which then alters the key assumption of the focussing theorem, thereby avoiding the singularity. 
...
Following the work of \cite{Clifton:2020oqx}, it is important to decompose the  expanding metric, $g_{+}^{ab}$ in Poisson gauge and for the collapsing metric, $g_{-}^{ab}$ in Conformal Fermi coordinates~\cite{Dai:2015jaa}. This is very essential as we shall see in the subsequent section that the vorticity cannot be generated in comoving synchronous coordinates. The Eulerian coordinate system is essential. 
...
\subsection{Dynamics of geodesics in both regions and their junction conditions }\label{sec:junction_conditions}
  ...
  We parameterise the geodesics by a translated time parameter $t   = \tau -\tau_{\rm{ini}}(r_{\rm{dp}})$, where $\tau_{\rm{ini}}(r_{\rm{dp}})$ is the cut-time. The boundary hypersurface $\Sigma$ is located at $t =0$ and   $t >0$ for  $ \tau >\tau_{\rm{ini}}(r_{\rm{dp}})$ and $t <0$ for  $ \tau <\tau_{\rm{ini}}(r_{\rm{dp}})$. We define the 4-velocity in each region as follows:
  \begin{eqnarray}
   u^a_{-}   = \frac{\d x^a_{-}}{\d t}  ~~~~{\rm{and}}~~~~~~~u^a_{+}
 =\frac{\d x^a_{+}}{\d t}  \,.
  \end{eqnarray}
  Note that these vectors are time-like $u^a_{\pm} u_{a}^{\pm} = -1$. 
  ...
  This setup is better understood using the language of the distribution function.  The Heaside function $\mathbb{H}( \ell)$ is used to constrain evolutionary history in both manifolds. The Heaside function is normalised such that it is equal to $+1$ if $ t >0$, $0$ if $ t<0$ and indeterminate if $ \ell = 0$, with the following properties 
\begin{eqnarray}
\mathbb{H}^2( t) = \mathbb{H}( t)\,, \qquad \mathbb{H}( t) \mathbb{H}(- t) = 0\,, \qquad \frac{\d }{\d  \ell} \mathbb{H}( t) = \delta ( t)  \,,
\end{eqnarray}
where $\delta( t)$ is the Dirac distribution. The metric of the ambient spacetime is related to metrics in both regions as 
\begin{eqnarray}
g_{ab} = \mathbb{H}(- t)g_{- ab}   +  \mathbb{H}( t)\ g_{+ab} + \delta(t) \delta g_{ab}\,,
\end{eqnarray}
where  last term $\delta g_{ab}$ denotes the metric at the boundary. To reduce clutter, we drop $ \mathbb{H}$ in the subsequent discussion. 
We define the action of the curve between point $p$ to point $q$ in the universe as a sum of the actions of the smooth curves in the two manifolds 
...
\end{eqnarray}
where $L_{-}$ is the Lagrangian for the smooth curves in the collapsing region and $L_{+}$ is the Lagrangian for smooth curves in the expanding region. We require that the curves join smoothly at $ t_{\rm{dp}}$. Prime indicates a derivative with respect to the argument.  For a smooth curve  $\gamma_{-}$ is geodesic within the range $[p,t_{\rm{dp}}]$ and  $\gamma_{+}$ is geodesic within the range $[t_{\rm{dp}},q]$. Equation \eqref{eq:oiecewise_action} indicates that we can describe the total action for the geodesic as a sum of piece-wise geodesics. 
...
\\
&=&\frac{\d }{\d s} \bigg|_{s = 0} \int_{p}^{\tau_{\rm{dp}}}L _{-}\left[\gamma_{-}( t),{{\gamma}'_{-}}( t)\right] \d  t  + 
...
\end{eqnarray}
Given the smoothness of the coordinates discussed in sub-section \ref{sec:Conformal_Killing}, we  impose that the curves are piecewise smooth, that is $\xi^i_{-}(t) = \xi_{+}^i (t)= \xi^i (t)$ at the boundary, therefore, the Euler Lagrange  equation holds in the separate spacetimes 
...
\end{eqnarray}
 ...
 The boundary conditions for the 4-vector for the  geodesic equation are obtained by plugging in the Lagrangian and can be obtained from  the geodesic equation given in equation \eqref{eq:length_functional}
\begin{eqnarray}\label{eq:four_velocity}
 \left[\frac{1}{L_{-}}\frac{\d x_{+}^a }{\d t}( t_{\rm{dp}}) - \frac{1}{L_{+}}  \frac{\d x_{+}^a }{\d t}( t_{\rm{dp}})  \right]\approx \left[u^a_{-}\bigg|_{\Sigma}- u^a_{+}\bigg|_{\Sigma}\right]\xi^i( t_{\rm{dp}}) = 0 \,.
\end{eqnarray}
...
Equations \eqref{eq:four_velocity} and \eqref{eq:Second_fundamental_form}  define the junction conditions that allow glueing spacetimes together across a boundary hypersurface $\Sigma  =  \partial M_{+} \cap \partial M_{-}$ via a thin shell.
...
We showed in the previous sub-section that the shell-crossing singularity at a finite distance is mild and may be avoided by a suitable change of coordinates. We derived the junction conditions that the 4-vector and the second fundamental form must satisfy.
%All of these derivations simply imposed general energy conditions on the matter content of the universe.  
We have not explicitly made use of any specific theory of gravity. What we have derived so far will apply to any theory that respects the principle of least action.  The Einstein general theory of  relativity is an example, it relates the geometry of spacetime  to the matter content of the universe:  
\begin{eqnarray}\label{eq:GR}
 G_{ab} = {R}_{ab} - \,\frac{1}{2}{R}\,{g}_{a b} =   {T}_{a b} \,,
\end{eqnarray}
 where $T^{ab}$  is the energy-momentum tensor and $R$ is the Ricci scalar.  We have so far derived the generalised junction conditions and equation of motion for the trajectory of massive particles on curved spacetime.  Equation \eqref{eq:GR} may be obtained from the Einstein-Hilbert action. For oriented manifolds such as those described in sub-section \ref{sec:junction_conditions},  the Einstein-Hilbert action has non-vanishing boundary terms~\cite{Gibbons:PhysRevD.15.2752}. 
...
\end{eqnarray}
Note that the violation of the $\doublesqbraket{ K_{ab}} = 0$ implies that the spacetime is not smooth at $\Sigma$. This has sound physical interpretation because it indicates that the surface layer has a non-vanishing stress-energy tensor at the hypersurface.  In cosmology for Gaussian initial conditions, one expects fluctuations of order $10^{-5}$ in the matter density field in large scales. On small scales, the fluctuation is much large.  
...
On the Giga-parces scales, for example, one could consider, galaxies as fluid parcels or fluid elements and assign each fluid parcel a geodesic. This is known as a fluid approximation. It is assumed that on Giga-parces scales, the internal dynamics of a galaxy are not important, hence, interactions within it are averaged over.  Even in the N-body simulation, this approximation is made but it is interpreted as mass resolution. In this case, a dark matter particle within the ensemble could have a mass of about $10^{12}$~\cite{Navarro:1995iw}. This approximation would break down when the internal dynamics of the cluster become important.

In our case with galaxies as a fluid element, the fluid approximation will break down at the causal limit. This is also the scale where the one-parameter family of geodesics breaks down(see sub-section \ref{subsec:separate_universe}).  Therefore, on the scale of clusters, we could consider stars as fluid elements and the fluid approximation in a different parameter will apply.  The conditions for the validity of fluid approximation are described in detail in figure \ref{fig:fluid_approximation}. The essential point is that fluid element before shell crossing is different from shell crossing after shell crossing. 
%
%
At the end of the day, the total energy-momentum tensor in the universe would include the stress-energy tensor at the boundary of the different fluid approximations
\begin{eqnarray}
T_{ab} = T^{+}_{ab} +  T^{-}_{ab} +  \mathcal{S}_{ab}\,,
\end{eqnarray}
...
T^{ab}_{+} = \rho_{m+} {u}^a_{+} {u}^b_{+}  %~~~~~~~{\rm{and}}~~~~~~~~~~~T^{ab}_{-} = \rho_{+} {u}^a_{+} {u}^b_{+}   
\end{eqnarray}
where ${\rho}_{m+} $ is the matter density field ${\rho}_{+}  ={T}_{{+} ab}{u}^a_{+}{u}^b_{+}\,$. We can write down a similar expression for $T^{-}_{ab}$.  Note that one can work with perfect fluid or even fluid with non-vanishing anisotropic stress.
...
\end{eqnarray}
Note that the extrinsic  curvature is related to the covariant derivative of $K_{ab}= \nabla_{b}{u}_{Sa}$
\begin{eqnarray}\label{eq:matter_decomp}
 \nabla_{b}{u}_{\mathcal{S}a}=-{u}_{\mathcal{S}b} {A}_{\mathcal{S}a}+\frac{1}{3}{\Theta}_{\mathcal{S}} {h}_{\mathcal{S}ab }
+{\sigma}_{\mathcal{S}ab} +{ \omega}_{\mathcal{S}ab}
\end{eqnarray}
where ${A}_{\mathcal{S}a} = {u}^b_{\mathcal{S}} \nabla_b \tilde{u}_{\mathcal{S}a}$ is the acceleration in the rest frame of ${u}_{Sa}$, ${\Theta}_{\mathcal{S}}$ is expansion of the  geodesics ${u}_{Sa}$, ${\sigma}_{\mathcal{S} ab} $ and ${ \omega}_{\mathcal{S} ab}$ are the corresponding shear and vorticity respectively. 
Furthermore,  it is instructive to interpret equation \eqref{eq:shell_crossing_EMT} as by decomposing $\mathcal{S}^{ab}$ into irreducible units with respect to $u^a_{\mathcal{S}}$ in a similar form as as equation \eqref{eq:emt_decomp} using equation \eqref{eq:matter_decomp}
\begin{eqnarray}\label{eq:Sab}
\mathcal{S}^{ab} = \rho_{\mathcal{S}} u^a_{\mathcal{S}} u^b_{\mathcal{S}} + P_{\mathcal{S}}h^{ab}_{\mathcal{S}}
  + q^{(a}_{\mathcal{S}} u^{b)}_{\mathcal{S}} + \pi^{\<ab\>}_{\mathcal{S}}
\end{eqnarray}
where $ \rho_{\mathcal{S}} $, $P_{\mathcal{S}}$, $q_{\mathcal{S} a}$ and $\pi^{\<ab\>}_{\mathcal{S}}$ are the corresponding energy density, pressure, energy flux vector and  stress-energy tensor respectively:
\begin{eqnarray}
{\rho}_{\mathcal{S}}  =\mathcal{S}_{ab}{u}^a_{\mathcal{S}} {u}^b_{\mathcal{S}}\,, \qquad P_{\mathcal{S}}  =\frac{1}{3} h_{\mathcal{S}ab} \mathcal{S}^{ab}\,, \qquad q_{\mathcal{S} \<a\>} = - \mathcal{S}_{\<a\>b} u^b_{\mathcal{S}}\,, \qquad  \pi_{\mathcal{S}\<ab\>} = \mathcal{S}_{\<ab\>} \,.
\end{eqnarray}
Note that $\mathcal{S} = \{+,-\}$.
Using equation \eqref{eq:matter_decomp}in equation \eqref{eq:shell_crossing_EMT}, we can calculate these observables
\begin{eqnarray}
{\rho}_{m+}  &=& 0\,, ~~~~~~~~~~~~~~~~~~~
P^{+} =  \frac{1} {2\pi}\frac{1}{3}\left[\Theta^{+}\right] \,, %=P_{+\mathcal{S}} -P_{-\mathcal{S}}\,,
\label{eq:pressure_def}%= \frac{\varepsilon} {8\pi} 
\\
q^{+ }_{\<a\>}  &=& -\frac{1} {8\pi} \left[A^{+}_{\<a\>}\right]\,,
~~~~~~~~
 \pi^{+}_{\<ab\>} =-\frac{1} {8\pi}\left[ \sigma^{+}_{ab} - \frac{2}{3}h^{+}_{ab}  \Theta^{+} \right] \,.
\end{eqnarray}
These fluid variables (i.e. $P_{\mathcal{S}}$, $q_{\mathcal{S} a}$ and $\pi^{\<ab\>}_{\mathcal{S}}$) are generated by the relative motion between two adjacent fluid elements. The relative motion induces
...
\begin{eqnarray}
\pi^{\<ab\>}_{+{\mathcal{S}} } = -\left[  \zeta_{+} \Theta_{+}  h^{ab}_{+}  +2\eta_{+} \sigma^{ab}_{+} \right]\approx -2\eta_{+} \sigma^{ab}_{+} \,,~~~~~~~\pi^{\<ab\>}_{- } = -\left[  \xi_{-} \Theta_{-}  h^{ab}_{-}  +2\eta_{-} \sigma^{ab}_{-} \right]\approx-2\eta_{-} \sigma^{ab}_{-}\,,
\end{eqnarray}
 where $\xi_{\pm} > 0$ and $\eta_{\pm} >0$ are bulk and shear viscosity respectively. In the second equality, we have approximated $\pi^{\<ab\>}_{\pm } $ with the shear viscosity only for simplicity. 
 It is straightforward, to include the additional contribution due to the bulk viscosity.
Finally, the  energy-momentum tensor that goes into the Einstein field equation is the sum of the energy-momentum tensor  of the fluid element decomposed in terms of the threading 4-vector and the  stress-energy tensor at the boundary
...
The energy-momentum tensor for each of the fluid specie is measured in its rest frame and not in the rest frame of a faraway observer.   
 \begin{eqnarray}\label{eq:effective_emt}
\tilde{T}_{+\rm{tot}}^{ab} = \tilde{\rho}_{m+} \tilde{u}^a_{+}\tilde{u}^b_{+} +  \tilde{P}_{+}\tilde{h}^{ab}_{+} + \tilde{q}^{(a}_{+} \tilde{u}^{b)}_{+}   - \xi_{+} \tilde{\Theta}_{+} \tilde{h}^{ab}_{+}- 2 \eta_{+} \tilde{\sigma}^{ab}_{+}\,,
\end{eqnarray}
where we made use of a tilde to indicate quantities measured in the rest frame of the fluid.  
The matter 4-velocity is related to the fundamental  4-velocity(4-velocity of the observer or the threading time-like unit vector)  according to 
...
\end{eqnarray}
where ${\rm{Div}} \sigma_{+a} = {\D}^b \sigma_{+ab}$ is the divergence of the rate of shear deformation tensor and $ {\D}^b$ is a spatial derivative on the corresponding hypersurface. Again, a similar equation exists for the collapsing region.  
...
The $\dot{P}_{+}$ comes from the time derivative of the first term in equation \eqref{eq:energyflux}
\begin{eqnarray}
...
$ {\partial P_{+}}/{\partial \rho_{+m}} = c_{s+}^2$:
 \begin{eqnarray}
\dot{P}_{+} = \frac{\partial P_{+}}{ \partial \rho_{m+}} \frac{\partial \rho_{m+}}{\partial t}  = c_{+s}^2 \dot{\rho}_{+} =  -c_{+s}^2 \left({\rho}_{m+}+{P}_{+}\right){\Theta_{+}} \longrightarrow  \frac{\dot{P}_{+}}{\left({\rho}_{m+}+{P}_{+}\right)} \tilde{\omega}^{ab}_{+} = - c_{+s}^2{\Theta}_{+} \tilde{\omega}^{ab}_{+}\,.
\end{eqnarray}
...
\begin{eqnarray}\label{eq:vorticity_eqn}
   \dot{\tilde{\omega}}_{+a}+\frac{1}{3}\tilde{\Theta}_{+}\tilde{\omega}_{+a}  =    c_{+s}^2\tilde{\Theta}_{+} \tilde{\omega}^{ab}_{+}+\eta_{+} {\D}_{b} {\D}^b \tilde{\omega}_{+a}  \,.
\end{eqnarray}
where $\tilde{\omega}_{+a} = \varepsilon_{+abc}\tilde{ \omega}^{[bc]}_{+}/2$.  The second term on the RHS is a vorticity diffusion term. This is due to the viscosity generated at the boundary between two fluid elements.  The first on the RHS is the acoustic dispersion term due to sound travelling at different speeds through different fluid elements in the neighbourhood of the boundary.  The two terms on the LHS are derivative terms, this becomes clear when the Lie derivative operator is used to bring them together $ \mathcal{L}_{\tilde{u}} \tilde{\omega}_{+a} =    \dot{\tilde{\omega}}_{+a}+{\Theta}_{+}\tilde{\omega}_{+a} /3$.



\subsection{Vorticity generation from rate of change of scalar circulation and the line of sight}\label{scalar_circulation}
...
The rate of viscous diffusion is determined by the conditions imposed on circulation at the boundary layer. We discussed in sub-section \ref{sec:fluid_element} how two different fluid elements emerge across the causal boundary.  Our target here is to use this idea to show how vorticity is generated at the boundary or thin shell between the expanding and collapsing regions of the universe.
...
\Gamma = \oint_{\partial S} v_{a} {\d} {\rm{l}}^a =\int_{S} \tilde{\omega}_{a} {\d} {\rm{S}}^a = \int_{S} \tilde{\omega}_{\parallel} {\d} S=\oint_{\partial S} v_{a}  t^a {\d} {\rm{s}}\,,
\end{eqnarray}
...
The radial component of the vorticity propagation equation becomes
 \begin{eqnarray}\label{eq:radial_vorticity}
  n^a \mathcal{L}_{u} \tilde{\omega}_a  &=&   \eta \nabla_{\bot b} \nabla_{\bot}^b \tilde{\omega}_{\parallel}  + \left[c_{s}^2 {\Theta} + \frac{1}{2} \eta_{\rm{f}}
\phi^2 \right] \tilde{\omega}_{\parallel}\,.
 \end{eqnarray}
...
+ \int_{ A}^{B}\left(\Sigma_{-} + \Sigma_{+}\right)\,{\d} s +\int_{ A}^{B} \left(\lambda_{-} +\lambda_{+}\right)\,{\d} s  
+{\frac {d}{dt}}\bigg|_{t=0}\int_{ A}^{B}  \gamma \,{\d} s \,,
\label{eq:introduc_vorticity_flux}
\end{eqnarray}
where $\Sigma_{-} = \eta_{-} t^a \nabla_{\bot a} \tilde{\omega}_{-\parallel}  $ and  $ \Sigma_{+} =  -\eta_{+} t^a \nabla_{\bot a} \tilde{\omega}_{+\parallel} $ is the diffusive vorticity flux of vorticity from the fluid in $-$ and $+$ respectively. While $\lambda_{-} = -\left[c_{s-}^2 {\Theta}_{-} + \frac{1}{2} \eta_{-}
\phi^2_{-} \right] {v_{-}}_{a}t^a$ and  $\lambda_{+} =  \left[c_{s+}^2 {\Theta}_{+} + \frac{1}{2} \eta_{+}\phi^2_{+} \right] {v_{+}}_{a}t^a$ are acoustic vorticity flux term due to the time rate of change of pressure and projection effects. 
 
 
 To evaluate the rate of change of the  junction condition, we have to, first of all, separate the terms 
  \begin{eqnarray}
 ...
 \end{eqnarray}
 Now we can make use of equation \eqref{eq:integral_formula} to evaluate the time rate of change of the integral. Note that to do this consistently we will need a $1+1+1+1$  decomposition equivalent of equation \eqref{eq:nabu112}. However, since we are considering only the linear order approximation, we can proceed as in equation \eqref{eq:Lie_inside-integral}
 \begin{eqnarray}
 ...
 +\eta\frac{2}{3} {\nabla}^{a}_{\bot} \tilde{\Theta} +c_{s}^2 {\Theta} v^a_{\bot} +\eta \varepsilon_{ab}\left[ \frac{1}{2} \phi\tilde{\omega}_{\bot}^b - \nabla_{\bot}^b \tilde{\omega}_{\parallel}\right] \,,
\end{eqnarray}
...
t_{a} {\mathcal {L}}_{u }{v_{\bot}}^{a}    &=& -t_{a}A^{a}_{\bot}
 ...
 +\eta  \frac{2}{3}t_{a} {\nabla}^{a}_{\bot} \tilde{\Theta} +\frac{1}{2} \eta\phi t_{a} \tilde{\omega}_{\bot}^a +c_{s}^2 {\Theta} t_{a}v^a_{\bot} -\eta t_{a}\nabla_{\bot}^a \tilde{\omega}_{\parallel}\,,
\end{eqnarray}
...
t_{a} {\mathcal {L}}_{u }{v_{\bot}}^{a}    &=& -t_{a}A^{a}_{\bot}
 ...
 +\phi \eta t^a \nabla_{\bot a} v_{\parallel} 
+ \left[ \frac{1}{2}\phi^2 \eta +c_{s}^2 {\Theta} \right] t_{a}v^a_{\bot} -\eta t_{a}\nabla_{\bot}^a \tilde{\omega}_{\parallel}\,.
\label{eq:massage_momentum}
\end{eqnarray}
Identifying the last two terms in equation \eqref{eq:massage_momentum} with the diffusive flux and the acoustic flux terms  introduced in equation \eqref{eq:introduc_vorticity_flux} and  making both terms the subject of the expression leads to
\begin{eqnarray}
\Sigma_{-} +\lambda_{-} &=&- t_{a} {\mathcal {L}}_{u }{v_{-}}_{\bot}^{a} +t_{a}{A_{-}}^{a}_{\bot}
...
\left(\Sigma_{-} + \Sigma_{+}\right) + \left(\lambda_{-} +\lambda_{+}\right) = - t_{a} \doublesqbraket{{\mathcal {L}}_{u }{v}_{\bot}^{a} }
-\doublesqbraket{ \frac{\partial \Phi }{\partial s} }
- \doublesqbraket{\frac{\partial }{\partial s}  \frac{P}{\left({\rho_{m}}+{P}\right)} } + \frac{2}{3}\doublesqbraket{{\eta} \frac{\partial \tilde{\Theta} }{\partial s}} + \doublesqbraket{  {\eta}{\phi}\frac{\partial v_{\parallel} }{\partial s}}\,,
\end{eqnarray}
...
-\doublesqbraket{ \frac{\partial \Phi }{\partial s} }
- \doublesqbraket{\frac{\partial }{\partial s}  \frac{P}{\left({\rho_{m}}+{P}\right)} } + \frac{2}{3}\doublesqbraket{{\eta} \frac{\partial \tilde{\Theta} }{\partial s}} + \doublesqbraket{ {\phi} {\eta}\frac{\partial v_{\parallel} }{\partial s}} \,.
\label{eq:vis_accen}
\end{eqnarray}
The jump in viscous acceleration is sourced by the angular gradients in the gravitational potential, pressure, expansion and the parallel component of the relative velocity (pair-wise velocity:).
...
\begin{eqnarray}\nonumber
  {\frac {d}{dt}}\bigg|_{t=0}\int_{ A}^{B}  \gamma \,{\d} s  &=&-\int_{ A}^{B} \left(\Sigma_{-} + \Sigma_{+}\right)  {\d} s  - \int_{ A}^{B}  \left(\lambda_{-} +\lambda_{+}\right)  {\d} s - \left( \doublesqbraket{ \Phi_{B} }- \doublesqbraket{\Phi_{A}}\right) -  \doublesqbraket{\frac{1}{(1+w)} \frac{P}{\rho_{m}} \bigg|_{B}} 
    \\  &&
  ...
  +  \doublesqbraket{ {\eta}{\phi}v_{\parallel B} }- \doublesqbraket{{\eta}_{\rm{f}}{\phi} v_{\parallel A}}
\end{eqnarray}
%$\frac{1}{(1+w)} \frac{P}{\rho} $
The circulation within the boundary is generated by the relative acceleration of fluid elements on both sides of the boundary which is sourced by the differences in gravitational potential, the sum of pressure and matter density, relative expansion and the radial component of the relative velocity.  If the geodesics and the geodesic deviation equation on both sides satisfy the junction conditions, the last two terms on the second line  and the second term on the first line vanish leading to 
\begin{eqnarray}\label{eq:circulation_condtions}
  ...
  - \left( \doublesqbraket{ \Phi_{B} }- \doublesqbraket{\Phi_{A}}\right) -  \doublesqbraket{\frac{1}{(1+w)} \frac{P}{\rho_{m}} \bigg|_{B}} 
     +\doublesqbraket{\frac{1}{(1+w)} \frac{P}{\rho_{m}} \bigg|_{A} }
\end{eqnarray}
In this limit,  circulation within the interface is generated by the differences in gravitational potential, the sum of pressure and matter density. Although there is a  flux  of  vorticity  term in equation \eqref{eq:circulation_condtions}, it does not contribute to the net circulation
\begin{eqnarray}\nonumber
{\frac {d}{dt}}\bigg|_{t=0} \Gamma &=&\oint_{\partial S_{-}}  \eta_{-} t^a \nabla_{\bot a} \tilde{\omega}_{-\parallel} {\d} {{s}} 
+  \oint_{\partial S_{-}} \left[c_{-s}^2 {\Theta}_{-} + \frac{1}{2} \eta_{-}\phi^2 \right] v_{-a}  t^a {\d} {{s}} 
...
\end{eqnarray}
Imposing the momentum junction conditions leads to 
\begin{eqnarray}%\nonumber 
{\frac {d}{dt}}\bigg|_{t=0} \Gamma &=&\oint_{\partial S_{-}}  \eta_{-} t^a \nabla_{\bot a} \tilde{\omega}_{-\parallel} {\d} {{s}} 
+  \oint_{\partial S_{-}} \left[c_{-s}^2 {\Theta}_{-} + \frac{1}{2} \eta_{-}\phi^2 \right] v_{-a}  t^a {\d} {{s}} 
+\oint_{\partial S_{+}}  \eta_{+} t^a \nabla_{\bot a} \tilde{\omega}_{+\parallel} {\d} {{s}} 
...
\\ \nonumber &&
 ...
 \,.
\end{eqnarray}
The flux diffusion and acoustic terms transport vorticity from the boundary to the interior of the fluid, they do not generate vorticity. The vorticity is generated by the difference in the gravitational and pressure across the boundary. 
...
Finally, if we further impose in addition to the momentum junction conditions and  that fluid is incompressible fluid at the boundary, we find a simple expression for the vorticity flux across the interface
\begin{eqnarray}\label{eq:viscous_accen2}
\left(\Sigma_{-} + \Sigma_{+}\right) = 
-\doublesqbraket{ \frac{\partial \Phi }{\partial s} }
- \doublesqbraket{\frac{\partial }{\partial s}  \frac{P}{\left({\rho}+{P}\right)} } \,.
\end{eqnarray}
Here the vorticity flux is sourced exclusively by the pressure and gravitational potential gradients, this is in agreement with \cite{bronsthompsonewekehourigan:2014,terringtonhouriganthompson:2020}.  %This is one of our key results. 
...
In the sub-section, we extend the treatment to the vorticity vector on the hypersurface. 
The basic building block of this is the curl theorem. It  relates vorticity at a given instant in time to the relative velocity at the boundary~\cite{terringtonhouriganthompson:2022}
...
\begin{eqnarray}
{\frac {d}{dt}}\bigg|_{t=0} \Gamma_a&=&   \int_{V}\left[ \eta  {\D}_{b} {\D}^b \tilde{\omega}_{a}  +c_{s}^2 {\Theta}\tilde{ \omega}^{a}\right]  \d V\,.
\end{eqnarray}
%\red{check the signs}
...
&=&    \int_{\partial V_{-}}\eta_{-} {n}_{b} {\D}^b \tilde{\omega}_{-a}  \d S_{-} +\int_{\partial V_{-}}c_{s{-}}^2 {\Theta} \varepsilon_{abc}n^c  {v}^b_{-} \d S_{-}  + \int_{\partial V_{+}}\eta_{+} {n}_{b} {\D}^b \tilde{\omega}_{+a}  \d S_{+} +\int_{\partial V_{+}}c_{s{+}}^2 {\Theta} \varepsilon_{abc}n^c  {v}^b_{+} \d S_{+}
\\  &&
+\int_{ I}  \left(\Sigma_{+a} +\Sigma_{{-}a}\right)   \d S +\int_{ I} \left(\lambda_{+a} + \lambda_{{-}a}\right) \d S
%\\ \nonumber  &&
...
For the integrals containing the viscous terms in equation \eqref{eq:vctor_cir_gamma}, we made use of the Divergence theorem to perform the integration and for the integrals containing the relative expansion term, we made use of equation \eqref{eq:3Dboundary_integration}
 To evaluate the rate of change of the vector circulation at the thin shell, we have to first of all separate  out the terms for clarity
 \begin{eqnarray}
 ...
 \end{eqnarray}
 where $A^c_{\bot}$ is a closed 2-sphere projected acceleration vector.  The directional  derivative of $n^a$  along $u^a$ \cite{Clarkson:2007yp}	
\begin{eqnarray}
...
Using the momentum constraint equation, we find that the Lie derivative of $v^a$ is given by
 ...
 +\frac{2}{3}\eta   \varepsilon_{ab} {\D}^{b} \tilde{\Theta} +c_{s}^2 {\Theta} \varepsilon_{ab} v^a-\eta n_{c}{\D}^c \tilde{\omega}^b \,.
...
\begin{eqnarray}
\Sigma_{{-}a} + \lambda_{{-}a} &=& -  \mathcal{L}_{u} v_{{-}\bot a} -A_{{-}\bot a}
 ...
 -\frac{2}{3}\eta_{{+}}  {\nabla}_{\bot a} \tilde{\Theta}_{+}\,,
\end{eqnarray}
 where we take the following  approximation $ \varepsilon_{ab}  \mathcal{L}_{u} v^b =   \mathcal{L}_{u} v_{\bot a}$. Combing these two equations leads to the expression of the diffusive flux vector and the acoustic flux vector in terms of the acceleration term, angular pressure gradient, gravitational potential gradients and the gradient in the expansion rates
 \begin{eqnarray}\label{eq:vorticity_flux_and_advective}
 \left( \Sigma_{+a}+ \Sigma_{{-}a} \right) + \left( \lambda_{+a} +  \lambda_{{-}a}\right) =  \doublesqbraket{  \mathcal{L}_{u} v_{\bot a}}
...
Putting equation \eqref{eq:junction_momentum_eqn} into equation \eqref{eq:simplified_junction_condition} and performing the angular integration gives
 \begin{eqnarray}\label{eq:full_gamma_exp}
  {\frac {d}{dt}}\bigg|_{t=0} \int_{ I}   \gamma_{a} \,{\d} S  &=&  \int_{ I}   \left( \Sigma_{+a}+ \Sigma_{{-}a} \right) + \left( \lambda_{+a} +  \lambda_{{-}a}\right)\,{\d} {S}
...
\begin{eqnarray}\nonumber
{\frac {d}{dt}}\bigg|_{t=0} \Gamma_a  &=&    \int_{\partial V_{-}}\eta_{-} {n}_{b} {\D}^b \tilde{\omega}_{-a}  \d S_{-} +\int_{\partial V_{-}}c_{s{-}}^2 {\Theta}_{-} \varepsilon_{ab}  {v}^b_{-} \d S_{-}+  \int_{\partial V_{+}}\eta_{+} {n}_{b} {\D}^b \tilde{\omega}_{+a}  \d S_{+} +\int_{\partial V_{+}}c_{s{+2}}^2 {\Theta}_{+} \varepsilon_{ab}  {v}^b_{+} \d S_{+}
\\  &&
...
For $ \tilde{\Theta} = 0$,  equation \eqref{eq:vctor_cir_gamma2} reduces to 
\begin{eqnarray}\nonumber
...
The key feature of the universe model described here is the existence of a ``causal limit" (boundary) for time-like geodesics. The causal limit divides the universe into two regions: expanding and collapsing regions. Using the principle of least action we derive the equations of motion and boundary conditions applicable in each case. With these equations and boundary conditions, we showed how the vorticity is sourced by the gradients in gravitational potential and pressure across the boundary.  The vorticity flux and acoustic flux generated at the boundary then diffuse away from the boundary. 
This is a unique model of the universe that does not introduce any new free parameter except for the parametrisation of the viscosity, rather it makes a prediction about the existence of a causal limit (a horizon) for massive particles in an expanding universe. 

In general,  causal limits are determined by the dynamics of time-like and light-like geodesics~\cite{HawkingandEllis:1973lsss.book}. 
Within the standard model of cosmology, the well-known causal limit is the particle horizon. It is determined by the dynamics of the light-like geodesic, it is important to note that the particle horizon indicates the maximum distance light from particles could have travelled to the observer in the age of the universe. 
 Ellis and Stoeger had earlier argued that there must exist a causal horizon in our universe that is determined by the dynamics of time-like geodesics~\cite{Ellis:2010fr}.  This causal limit is a unique feature of the universe model we describe here.  We showed how the causal limit could be determined given a halo model for a gravitational bound cluster. 
...
To see how the existence of vorticity is connected existence of the magnetic part of the Weyl tensor, we project the two free indices of the Ricci identity with $\varepsilon^{cdb}$. The further simplification of the results gives
...
\end{eqnarray}
A full treatment of this involves the use of the Einstein field equations and spatial averaging techniques~\cite{Buchert:2008zm,Umeh:2010pr,Clarkson:2011zq}, however, the point here is that the model we describe does not only describe a mechanism for the generation of vorticity in the universe, it also gives a natural explanation generating accelerated in the universe without the need for any assumption about the energy content of the universe. 
Here the acceleration of the ambient spacetime volume,  ${ \d^2 a_{W_{\Sigma_{3}}}}/{\d t^2} > 0$,  is easily be realised for known and standard matter source because the Hubble rate in the collapsing region has a sign opposite to the Hubble rate in the expanding region $H_{W_{\Sigma_{-}}} \propto -H_{W_{\Sigma_{+}}}$.
...
Our understanding of the evolution of large-scale structures in the universe is built on the geodesic equation of an expanding background spacetime. The solution to the geodesic equation is usually used to set the initial conditions for the growth of structures in the N-body simulations~\cite{Crocce:2005xz, Springel:2005mi,Adamek:2014xba}.  This practice is common in cosmology, however, less attention is paid to the validity range of geodesics in a universe like ours with over-dense and under-dense regions.
 It is well-known in the field of differential geometry/general relativity, especially in blackhole physics that a curve is not globally geodesic~\cite{HawkingandEllis:1973lsss.book}. A geodesic could cease to be a geodesic in a finite time~\cite{Penrose:1964wq}. 
 
 In this paper, we studied the validity range of time-like geodesics in a universe like ours which has over-dense and under-dense regions.  We show that shell-crossing or caustic formation in cosmology is equivalent to a conjugate point, i.e a point where a geodesic ceases to be geodesic.  We show that before a conjugate point is reached, the rate of expansion of the comoving one-parameter family of geodesics goes to zero (this is known as an apparent horizon for a family of null geodesics), we refer to this as the causal limit from following Ellis and Stoeger that discussed similar issues in~\cite{Ellis:2010fr}. The appearance of a causal limit before a conjugate point or caustics makes a change of coordinate possible since the  Jacobian determinant is non-zero at the causal limit. By the inverse function theorem, we showed that it is possible to introduce a more suitable set of coordinates in the neighbourhood of the causal limit. This is shown in detail sub-section \ref{sec:Conformal_Killing}. We showed that the coordinates system that matches smoothly with the expanding coordinates at the causal limit must satisfy the conformal Killing equation. This is consistent with the construction of the conformal Fermi coordinates used in the separate Universe approach to large-scale structure formation~\cite{Dai:2015rda,Dai:2015jaa}.


The existence of a causal horizon for time-like geodesic divides the universe into expanding and collapsing regions. The gravitationally bound systems like clusters are decoupled from the Hubble expansion of the universe. The dynamics within the decoupled region are more crucial for the formation of sub-structures such as galaxies~\cite{Ellis:2010fr}. We described how to build a more accurate model of the observed universe that captures the dynamics in both the expanding and collapsing regions in section \ref{sec:universe_model}. 

One of the key features of the model is that it provides a mechanism for the generation of vorticity in the universe. This is because the one-parameter family of geodesics in the expanding universe needs to match smoothly with another one-parameter family of geodesics in the collapsing region.  The jump in the junction conditions for the Riemann tensor in both regions of the universe is naturally interpreted in terms of the energy-momentum tensor at the boundary. The jump in the junction conditions has a natural physical interpretation in terms of the effective energy-momentum tensor of the universe. The components of the effective energy-momentum tensor include a non-vanishing stress tensor, isotropic pressure and the momentum flux vector. These observables exist even if the matter in the universe is purely dust. We then showed that the non-vanishing contribution of these observables leads to a non-vanishing scalar and vector circulation sourced by the gradients of gravitation potential, pressure and expansion. 
 This finding is very important because the vorticity has been observed around the filament, clusters, etc but its source cannot be found within the standard cosmological model on an expanding FLRW spacetime~\cite{Wang:2021axr}.  Also, some high-resolution N-body simulations have been able to measure vorticity in the outskirts of the large-scale structures but the mechanism for its formation has eluded a clear analytic understanding~\cite{Libeskind:2012ya,Jelic-Cizmek:2018gdp}.  Although there are earlier works based on the N-body simulations that established a link between vorticity generation and caustics formation or shell-crossing singularity in cosmology~\cite{Pichon:1999tk,Wang:2013gtn}, This is the first paper to describe in detail how it works. 
Furthermore, we showed that vorticity is generated at the boundary of the expanding and collapsing regions of spacetime.  The vorticity flux is sourced by the relative contributions of the gradients of the gravitational potential, pressure and rate of expansion or contraction in both regions. The vorticity flux generated at the boundary diffuses towards the fluid interior. 
...
Finally, we discussed other obvious predictions of the model in section \ref{sec:implications}. Some of these predictions include the existence of a causal limit or horizon for massive particles in the expanding universe and the non-zero magnetic part of the Weyl tensor. There is also a consequence for the generation of magnetic fields in clusters. We also discussed how the non-local variance in the Hubble rate between the collapsing and expanding regions could manifest as an apparent acceleration of the ambient domain. A comprehensive discussion of these obvious features of the model and many more would be provided elsewhere. 
...
%\bibliographystyle{JHEP}%{JHEP}%
%\bibliography{$HOME/Dropbox/Test_Equivalence_Principle/cosmoref.bib, $HOME/Dropbox/Exclusion/cosmoref.bib}
...
\bibitem{Smoot:1992td}
G.~F. Smoot, C.~Bennett, A.~Kogut, E.~Wright, J.~Aymon, et~al., {\it {Structure
  in the COBE differential microwave radiometer first year maps}},  {\em
  Astrophys.J.} {\bf 396} (1992) L1--L5.

\bibitem{Fixsen:1996nj}
D.~J. Fixsen, E.~S. Cheng, J.~M. Gales, J.~C. Mather, R.~A. Shafer, and E.~L.
  Wright, {\it {The Cosmic Microwave Background spectrum from the full COBE
  FIRAS data set}},  {\em Astrophys. J.} {\bf 473} (1996) 576,
  [\href{http://arxiv.org/abs/astro-ph/9605054}{{\tt astro-ph/9605054}}].

\bibitem{Kodama:1985bj}
H.~Kodama and M.~Sasaki, {\it {Cosmological Perturbation Theory}},  {\em Prog.
  Theor. Phys. Suppl.} {\bf 78} (1984) 1--166.

\bibitem{Mukhanov:1990me}
V.~F. Mukhanov, H.~A. Feldman, and R.~H. Brandenberger, {\it {Theory of
  cosmological perturbations. Part 1. Classical perturbations. Part 2. Quantum
  theory of perturbations. Part 3. Extensions}},  {\em Phys. Rept.} {\bf 215}
  (1992) 203--333.

\bibitem{Lu:2007cj}
T.~H.-C. Lu, K.~Ananda, and C.~Clarkson, {\it {Vector modes generated by
  primordial density fluctuations}},  {\em Phys. Rev.} {\bf D77} (2008) 043523,
  [\href{http://arxiv.org/abs/0709.1619}{{\tt arXiv:0709.1619}}].

\bibitem{jolicoeur:2018blf}
S.~Jolicoeur, A.~Allahyari, C.~Clarkson, J.~Larena, O.~Umeh, and R.~Maartens,
  {\it {Imprints of local lightcone projection effects on the galaxy bispectrum
  IV: second-order vector and tensor contributions}},  {\em JCAP} {\bf 03}
  (2019) 004, [\href{http://arxiv.org/abs/1811.05458}{{\tt arXiv:1811.05458}}].

\bibitem{Baumann:2007zm}
D.~Baumann, P.~J. Steinhardt, K.~Takahashi, and K.~Ichiki, {\it {Gravitational
  Wave Spectrum Induced by Primordial Scalar Perturbations}},  {\em Phys. Rev.}
  {\bf D76} (2007) 084019, [\href{http://arxiv.org/abs/hep-th/0703290}{{\tt
  hep-th/0703290}}].

\bibitem{Barrera-Hinojosa:2020gnx}
C.~Barrera-Hinojosa, B.~Li, M.~Bruni, and J.-h. He, {\it {Vector modes in
  $\Lambda$CDM: the gravitomagnetic potential in dark matter haloes from
  relativistic $N$-body simulations}},  {\em Mon. Not. Roy. Astron. Soc.} {\bf
  501} (2021), no.~4 5697--5713, [\href{http://arxiv.org/abs/2010.08257}{{\tt
  arXiv:2010.08257}}].

\bibitem{Barrera-Hinojosa:2021msx}
C.~Barrera-Hinojosa, B.~Li, and Y.-C. Cai, {\it {Looking for a twist: probing
  the cosmological gravitomagnetic effect via weak lensing-kSZ
  cross-correlations}},  {\em Mon. Not. Roy. Astron. Soc.} {\bf 510} (2022),
  no.~3 3589--3604, [\href{http://arxiv.org/abs/2109.02632}{{\tt
  arXiv:2109.02632}}].

\bibitem{Christopherson:2009bt}
A.~J. Christopherson, K.~A. Malik, and D.~R. Matravers, {\it {Vorticity
  generation at second order}},  {\em Phys. Rev.} {\bf D79} (2009) 123523,
  [\href{http://arxiv.org/abs/0904.0940}{{\tt arXiv:0904.0940}}].

\bibitem{Wang:2021axr}
P.~Wang, N.~I. Libeskind, E.~Tempel, X.~Kang, and Q.~Guo, {\it {Possible
  observational evidence for cosmic filament spin}},  {\em Nature Astron.} {\bf
  5} (2021), no.~8 839--845, [\href{http://arxiv.org/abs/2106.05989}{{\tt
  arXiv:2106.05989}}]. [Erratum: Nature Astron. 5, 1077 (2021)].

\bibitem{2019ApJ...872...78L}
J.~H. {Lee}, M.~{Pak}, H.-R. {Lee}, and H.~{Song}, {\it {Galaxy Rotation
  Coherent with the Motions of Neighbors: Discovery of Observational
  Evidence}},  {\em \apj} {\bf 872} (Feb., 2019) 78,
  [\href{http://arxiv.org/abs/1901.02151}{{\tt arXiv:1901.02151}}].

\bibitem{Krolewski:2019bfv}
A.~Krolewski, S.~Ho, Y.-C. Chen, P.~F. Chan, A.~Tenneti, D.~Bizyaev, and
  K.~Kraljic, {\it {Alignment between filaments and galaxy spins from the MaNGA
  integral-field survey}},  {\em Astrophys. J.} {\bf 876} (2019), no.~1 52,
  [\href{http://arxiv.org/abs/1902.09797}{{\tt arXiv:1902.09797}}].

\bibitem{cushman2011introduction}
B.~Cushman-Roisin and J.~Beckers, {\em Introduction to Geophysical Fluid
  Dynamics: Physical and Numerical Aspects}.
\newblock International Geophysics. Elsevier Science, 2011.

\bibitem{Jelic-Cizmek:2018gdp}
G.~Jelic-Cizmek, F.~Lepori, J.~Adamek, and R.~Durrer, {\it {The generation of
  vorticity in cosmological N-body simulations}},  {\em JCAP} {\bf 09} (2018)
  006, [\href{http://arxiv.org/abs/1806.05146}{{\tt arXiv:1806.05146}}].

\bibitem{Ivanov:2022mrd}
M.~M. Ivanov, {\it {Effective Field Theory for Large Scale Structure}},
  \href{http://arxiv.org/abs/2212.08488}{{\tt arXiv:2212.08488}}.

\bibitem{Adamek:2016zes}
J.~Adamek, D.~Daverio, R.~Durrer, and M.~Kunz, {\it {gevolution: a cosmological
  N-body code based on General Relativity}},  {\em JCAP} {\bf 07} (2016) 053,
  [\href{http://arxiv.org/abs/1604.06065}{{\tt arXiv:1604.06065}}].

\bibitem{Pueblas:2008uv}
S.~Pueblas and R.~Scoccimarro, {\it {Generation of Vorticity and Velocity
  Dispersion by Orbit Crossing}},  {\em Phys. Rev. D} {\bf 80} (2009) 043504,
  [\href{http://arxiv.org/abs/0809.4606}{{\tt arXiv:0809.4606}}].

\bibitem{Hahn:2014lca}
O.~Hahn, R.~E. Angulo, and T.~Abel, {\it {The Properties of Cosmic Velocity
  Fields}},  {\em Mon. Not. Roy. Astron. Soc.} {\bf 454} (2015), no.~4
  3920--3937, [\href{http://arxiv.org/abs/1404.2280}{{\tt arXiv:1404.2280}}].

\bibitem{Pichon:1999tk}
C.~Pichon and F.~Bernardeau, {\it {Vorticity generation in large scale
  structure caustics}},  {\em Astron. Astrophys.} {\bf 343} (1999) 663,
  [\href{http://arxiv.org/abs/astro-ph/9902142}{{\tt astro-ph/9902142}}].

\bibitem{Cusin:2016zvu}
G.~Cusin, V.~Tansella, and R.~Durrer, {\it {Vorticity generation in the
  Universe: A perturbative approach}},  {\em Phys. Rev. D} {\bf 95} (2017),
  no.~6 063527, [\href{http://arxiv.org/abs/1612.00783}{{\tt
  arXiv:1612.00783}}].

\bibitem{Ruggiero:2021lpf}
M.~L. Ruggiero, A.~Ortolan, and C.~C. Speake, {\it {Galactic dynamics in
  general relativity: the role of gravitomagnetism}},  {\em Class. Quant.
  Grav.} {\bf 39} (2022), no.~22 225015,
  [\href{http://arxiv.org/abs/2112.08290}{{\tt arXiv:2112.08290}}].

\bibitem{Astesiano:2022ozl}
D.~Astesiano and M.~L. Ruggiero, {\it {Galactic dark matter effects from purely
  geometrical aspects of general relativity}},  {\em Phys. Rev. D} {\bf 106}
  (2022), no.~4 044061, [\href{http://arxiv.org/abs/2205.03091}{{\tt
  arXiv:2205.03091}}].

\bibitem{Crocce:2005xz}
M.~Crocce and R.~Scoccimarro, {\it {Memory of Initial Conditions in
  Gravitational Clustering}},  {\em Phys. Rev.} {\bf D73} (2006) 063520,
  [\href{http://arxiv.org/abs/astro-ph/0509419}{{\tt astro-ph/0509419}}].

\bibitem{Springel:2005mi}
V.~Springel, {\it {The Cosmological simulation code GADGET-2}},  {\em
  Mon.Not.Roy.Astron.Soc.} {\bf 364} (2005) 1105--1134,
  [\href{http://arxiv.org/abs/astro-ph/0505010}{{\tt astro-ph/0505010}}].

\bibitem{Sokolowski:2014non}
L.~M. Soko\l{}owski and Z.~A. Golda, {\it {Jacobi fields, conjugate points and
  cut points on timelike geodesics in special spacetimes}},  {\em Acta Phys.
  Polon. B} {\bf 46} (2015) 773--787,
  [\href{http://arxiv.org/abs/1402.3976}{{\tt arXiv:1402.3976}}].

\bibitem{Zel'dovich:1970AandA}
Y.~B. {Zel'dovich}, {\it {Gravitational instability: An approximate theory for
  large density perturbations.}},  {\em \aap} {\bf 5} (Mar., 1970) 84--89.

\bibitem{Garrison:2016vvp}
L.~H. Garrison, D.~J. Eisenstein, D.~Ferrer, M.~V. Metchnik, and P.~A. Pinto,
  {\it {Improving Initial Conditions for Cosmological $N$-Body Simulations}},
  {\em Mon. Not. Roy. Astron. Soc.} {\bf 461} (2016), no.~4 4125--4145,
  [\href{http://arxiv.org/abs/1605.02333}{{\tt arXiv:1605.02333}}].

\bibitem{Bruni:2002sma}
M.~Bruni, L.~Gualtieri, and C.~F. Sopuerta, {\it {Two parameter nonlinear
  space-time perturbations: Gauge transformations and gauge invariance}},  {\em
  Class. Quant. Grav.} {\bf 20} (2003) 535--556,
  [\href{http://arxiv.org/abs/gr-qc/0207105}{{\tt gr-qc/0207105}}].

\bibitem{Nakamura:2003wk}
K.~Nakamura, {\it {Gauge invariant variables in two parameter nonlinear
  perturbations}},  {\em Prog.Theor.Phys.} {\bf 110} (2003) 723--755,
  [\href{http://arxiv.org/abs/gr-qc/0303090}{{\tt gr-qc/0303090}}].

\bibitem{Goldberg:2016lcq}
S.~R. Goldberg, T.~Clifton, and K.~A. Malik, {\it {Cosmology on all scales: a
  two-parameter perturbation expansion}},  {\em Phys. Rev. D} {\bf 95} (2017),
  no.~4 043503, [\href{http://arxiv.org/abs/1610.08882}{{\tt
  arXiv:1610.08882}}].

\bibitem{Rampf:2020hqh}
C.~Rampf and O.~Hahn, {\it {Shell-crossing in a $\Lambda$ CDM Universe}},  {\em
  Mon. Not. Roy. Astron. Soc.} {\bf 501} (2021), no.~1 L71--L75,
  [\href{http://arxiv.org/abs/2010.12584}{{\tt arXiv:2010.12584}}].

\bibitem{1980JAtS...37..630M}
W.~E. {Meador} and W.~R. {Weaver}, {\it {Two-stream approximations to radiative
  transfer in planetary atmospheres: a unified description of existing methods
  and a new improvement.}},  {\em Journal of Atmospheric Sciences} {\bf 37}
  (Mar., 1980) 630--643.

\bibitem{1987AmJPh..55..524B}
C.~F. {Bohren}, {\it {Multiple scattering of light and some of its observable
  consequences}},  {\em American Journal of Physics} {\bf 55} (June, 1987)
  524--533.

\bibitem{Gallagher:2019tcr}
C.~Gallagher, T.~Clifton, and C.~Clarkson, {\it {Multi-scale perturbation
  theory. Part I. Methodology and leading-order bispectrum corrections in the
  matter-dominated era}},  {\em JCAP} {\bf 03} (2020) 011,
  [\href{http://arxiv.org/abs/1910.04894}{{\tt arXiv:1910.04894}}].

\bibitem{Clifton:2020oqx}
T.~Clifton, C.~S. Gallagher, S.~Goldberg, and K.~A. Malik, {\it {Viable gauge
  choices in cosmologies with nonlinear structures}},  {\em Phys. Rev. D} {\bf
  101} (2020), no.~6 063530, [\href{http://arxiv.org/abs/2001.00394}{{\tt
  arXiv:2001.00394}}].

\bibitem{Gallagher:2018bdl}
C.~S. Gallagher and T.~Clifton, {\it {Relativistic Euler equations in
  cosmologies with nonlinear structures}},  {\em Phys. Rev. D} {\bf 98} (2018),
  no.~10 103516, [\href{http://arxiv.org/abs/1807.01655}{{\tt
  arXiv:1807.01655}}].

\bibitem{Goldberg:2017gsm}
S.~R. Goldberg, C.~Gallagher, and T.~Clifton, {\it {Perturbation theory for
  cosmologies with nonlinear structure}},  {\em Phys. Rev. D} {\bf 96} (2017),
  no.~10 103508, [\href{http://arxiv.org/abs/1707.01042}{{\tt
  arXiv:1707.01042}}].

\bibitem{Ellis:2010fr}
G.~F.~R. Ellis and W.~R. Stoeger, {\it {The Evolution of Our Local Cosmic
  Domain: Effective Causal Limits}},  {\em Mon. Not. Roy. Astron. Soc.} {\bf
  398} (2009) 1527--1536, [\href{http://arxiv.org/abs/1001.4572}{{\tt
  arXiv:1001.4572}}].

\bibitem{HawkingandEllis:1973lsss.book}
S.~W. {Hawking} and G.~F.~R. {Ellis}, {\em {The large-scale structure of
  space-time.}}
\newblock 1973.

\bibitem{Hwang:2005wb}
J.-C. Hwang and H.~Noh, {\it {Why Newton's gravity is practically reliable in
  the large-scale cosmological simulations}},  {\em Mon. Not. Roy. Astron.
  Soc.} {\bf 367} (2006) 1515,
  [\href{http://arxiv.org/abs/astro-ph/0507159}{{\tt astro-ph/0507159}}].

\bibitem{Ehlers:2009uv}
J.~Ehlers and T.~Buchert, {\it {On the Newtonian Limit of the Weyl Tensor}},
  {\em Gen. Rel. Grav.} {\bf 41} (2009) 2153--2158,
  [\href{http://arxiv.org/abs/0907.2645}{{\tt arXiv:0907.2645}}].

\bibitem{Clifton:2016mxx}
T.~Clifton, D.~Gregoris, and K.~Rosquist, {\it {The Magnetic Part of the Weyl
  Tensor, and the Expansion of Discrete Universes}},  {\em Gen. Rel. Grav.}
  {\bf 49} (2017), no.~2 30, [\href{http://arxiv.org/abs/1607.00775}{{\tt
  arXiv:1607.00775}}].

\bibitem{Ellis:1998ct}
G.~F.~R. Ellis and H.~van Elst, {\it {Cosmological models}},  {\em NATO Adv.
  Study Inst. Ser. C. Math. Phys. Sci.} {\bf 541} (1999) 1--116,
  [\href{http://arxiv.org/abs/gr-qc/9812046}{{\tt gr-qc/9812046}}].

\bibitem{Kumar:2021qrg}
S.~Kumar, {\em {Gravitational Memory Effect for Near-Horizon Asymptotic
  Symmetries}}.
\newblock PhD thesis, IIIT, Allahabad, 10, 2021.
\newblock \href{http://arxiv.org/abs/2203.08983}{{\tt arXiv:2203.08983}}.

\bibitem{Ellis:1990gi}
G.~F.~R. Ellis, M.~Bruni, and J.~Hwang, {\it {Density gradient-vorticity
  relation in perfect fluid Robertson-Walker perturbations}},  {\em Phys. Rev.}
  {\bf D42} (1990) 1035--1046.

\bibitem{2012reco.book.....E}
G.~F.~R. {Ellis}, R.~{Maartens}, and M.~A.~H. {MacCallum}, {\em {Relativistic
  Cosmology}}.
\newblock 2012.

\bibitem{Ellis1971grc..conf..104E}
G.~F.~R. {Ellis}, {\it {Relativistic cosmology.}},  in {\em General Relativity
  and Cosmology} (R.~K. {Sachs}, ed.), pp.~104--182, 1971.

\bibitem{Ellis2009}
G.~F.~R. Ellis, {\it Republication of: Relativistic cosmology},  {\em General
  Relativity and Gravitation} {\bf 41} (Mar, 2009) 581--660.

\bibitem{Senovilla:2006db}
J.~M.~M. Senovilla, {\it {Singularity theorems in general relativity:
  Achievements and open questions}},  {\em Einstein Stud.} {\bf 12} (2012)
  305--316, [\href{http://arxiv.org/abs/physics/0605007}{{\tt
  physics/0605007}}].

\bibitem{Aghanim:2018eyx}
{\bf Planck} Collaboration, N.~Aghanim et~al., {\it {Planck 2018 results. VI.
  Cosmological parameters}},  {\em Astron. Astrophys.} {\bf 641} (2020) A6,
  [\href{http://arxiv.org/abs/1807.06209}{{\tt arXiv:1807.06209}}].

\bibitem{Koyama:2018ttg}
K.~Koyama, O.~Umeh, R.~Maartens, and D.~Bertacca, {\it {The observed galaxy
  bispectrum from single-field inflation in the squeezed limit}},  {\em JCAP}
  {\bf 07} (2018) 050, [\href{http://arxiv.org/abs/1805.09189}{{\tt
  arXiv:1805.09189}}].

\bibitem{Maartens:2020jzf}
R.~Maartens, S.~Jolicoeur, O.~Umeh, E.~M. De~Weerd, and C.~Clarkson, {\it
  {Local primordial non-Gaussianity in the relativistic galaxy bispectrum}},
  {\em JCAP} {\bf 04} (2021) 013, [\href{http://arxiv.org/abs/2011.13660}{{\tt
  arXiv:2011.13660}}].

\bibitem{Verde:2009hy}
L.~Verde and S.~Matarrese, {\it {Detectability of the effect of Inflationary
  non-Gaussianity on halo bias}},  {\em Astrophys.J.} {\bf 706} (2009)
  L91--L95, [\href{http://arxiv.org/abs/0909.3224}{{\tt arXiv:0909.3224}}].

\bibitem{Dai:2015jaa}
L.~Dai, E.~Pajer, and F.~Schmidt, {\it {On Separate Universes}},  {\em JCAP}
  {\bf 1510} (2015), no.~10 059, [\href{http://arxiv.org/abs/1504.00351}{{\tt
  arXiv:1504.00351}}].

\bibitem{Clarkson:2011br}
C.~Clarkson, G.~F. Ellis, A.~Faltenbacher, R.~Maartens, O.~Umeh, et~al., {\it
  {(Mis-)Interpreting supernovae observations in a lumpy universe}},  {\em
  Mon.Not.Roy.Astron.Soc.} {\bf 426} (2012) 1121--1136,
  [\href{http://arxiv.org/abs/1109.2484}{{\tt arXiv:1109.2484}}].

\bibitem{Fodor:1998jp}
G.~Fodor, M.~Marklund, and Z.~Perjes, {\it {Axistationary perfect fluids: A
  Tetrad approach}},  {\em Class. Quant. Grav.} {\bf 16} (1999) 453--463,
  [\href{http://arxiv.org/abs/gr-qc/9805017}{{\tt gr-qc/9805017}}].

\bibitem{Karachentsev:2008st}
I.~D. Karachentsev, O.~G. Kashibadze, D.~I. Makarov, and R.~B. Tully, {\it {The
  Hubble flow around the Local Group}},  {\em Mon. Not. Roy. Astron. Soc.} {\bf
  393} (2009) 1265, [\href{http://arxiv.org/abs/0811.4610}{{\tt
  arXiv:0811.4610}}].

\bibitem{Anand:2019ApJ...880...52A}
G.~S. {Anand}, R.~B. {Tully}, L.~{Rizzi}, E.~J. {Shaya}, and I.~D.
  {Karachentsev}, {\it {Peculiar Velocities of Galaxies Just Beyond the Local
  Group}},  {\em \apj} {\bf 880} (July, 2019) 52,
  [\href{http://arxiv.org/abs/1905.11416}{{\tt arXiv:1905.11416}}].

\bibitem{Qin:2021tak}
F.~Qin, D.~Parkinson, C.~Howlett, and K.~Said, {\it {Cosmic Flow Measurement
  and Mock Sampling Algorithm of Cosmicflows-4 Tully-Fisher Catalogue}},
  \href{http://arxiv.org/abs/2109.14808}{{\tt arXiv:2109.14808}}.

\bibitem{2006Ap.....49....3K}
I.~D. {Karachentsev} and O.~G. {Kashibadze}, {\it {Masses of the local group
  and of the M81 group estimated from distortions in the local velocity
  field}},  {\em Astrophysics} {\bf 49} (Jan., 2006) 3--18.

\bibitem{1999AandARv...9..273V}
S.~{van den Bergh}, {\it {The local group of galaxies}},  {\em \aapr} {\bf 9}
  (Jan., 1999) 273--318.

\bibitem{Umeh:2022kqs}
O.~Umeh, {\it {Emergence of smooth distance and apparent magnitude in a lumpy
  Universe}},  \href{http://arxiv.org/abs/2202.08237}{{\tt arXiv:2202.08237}}.

\bibitem{Umeh:2022prn}
O.~Umeh, {\it {Consequences of using a smooth cosmic distance in a lumpy
  universe. I.}},  {\em Phys. Rev. D} {\bf 106} (2022), no.~2 023514,
  [\href{http://arxiv.org/abs/2202.08230}{{\tt arXiv:2202.08230}}].

\bibitem{Villa:2015ppa}
E.~Villa and C.~Rampf, {\it {Relativistic perturbations in $\Lambda$CDM:
  Eulerian \& Lagrangian approaches}},  {\em JCAP} {\bf 01} (2016) 030,
  [\href{http://arxiv.org/abs/1505.04782}{{\tt arXiv:1505.04782}}]. [Erratum:
  JCAP 05, E01 (2018)].

\bibitem{Cooray:2002dia}
A.~Cooray and R.~K. Sheth, {\it {Halo Models of Large Scale Structure}},  {\em
  Phys. Rept.} {\bf 372} (2002) 1--129,
  [\href{http://arxiv.org/abs/astro-ph/0206508}{{\tt astro-ph/0206508}}].

\bibitem{Clarkson:2010uz}
C.~Clarkson and R.~Maartens, {\it {Inhomogeneity and the foundations of
  concordance cosmology}},  {\em Class.Quant.Grav.} {\bf 27} (2010) 124008,
  [\href{http://arxiv.org/abs/1005.2165}{{\tt arXiv:1005.2165}}].

\bibitem{Diemer:2017bwl}
B.~Diemer, {\it {COLOSSUS: A python toolkit for cosmology, large-scale
  structure, and dark matter halos}},  {\em Astrophys. J. Suppl.} {\bf 239}
  (2018), no.~2 35, [\href{http://arxiv.org/abs/1712.04512}{{\tt
  arXiv:1712.04512}}].

\bibitem{Diemer:2017uwt}
B.~Diemer, P.~Mansfield, A.~V. Kravtsov, and S.~More, {\it {The splashback
  radius of halos from particle dynamics. II. Dependence on mass, accretion
  rate, redshift, and cosmology}},  {\em Astrophys. J.} {\bf 843} (2017), no.~2
  140, [\href{http://arxiv.org/abs/1703.09716}{{\tt arXiv:1703.09716}}].

\bibitem{Zuercher:2018prq}
D.~Z\"urcher and S.~More, {\it {The Splashback Radius of $Planck$-SZ
  clusters}},  {\em Astrophys. J.} {\bf 874} (2019), no.~2 184,
  [\href{http://arxiv.org/abs/1811.06511}{{\tt arXiv:1811.06511}}].

\bibitem{Murata:2020enz}
R.~Murata, T.~Sunayama, M.~Oguri, S.~More, A.~J. Nishizawa, T.~Nishimichi, and
  K.~Osato, {\it {The splashback radius of optically selected clusters with
  Subaru HSC Second Public Data Release}},  {\em Publ. Astron. Soc. Jap.} {\bf
  72} (2020), no.~4 Publications of the Astronomical Society of Japan, Volume
  72, Issue 4, August 2020, 64, https://doi.org/10.1093/pasj/psaa041,
  [\href{http://arxiv.org/abs/2001.01160}{{\tt arXiv:2001.01160}}].

\bibitem{Moon:2022nfg}
J.-S. Moon and J.~Lee, {\it {Radius-Dependent Spin Transition of Dark Matter
  Halos}},  \href{http://arxiv.org/abs/2210.15905}{{\tt arXiv:2210.15905}}.

\bibitem{Israel:1966rt}
W.~Israel, {\it {Singular hypersurfaces and thin shells in general
  relativity}},  {\em Nuovo Cim. B} {\bf 44S10} (1966) 1. [Erratum: Nuovo Cim.B
  48, 463 (1967)].

\bibitem{poisson_2004}
E.~Poisson, {\em Hypersurfaces}, p.~59–117.
\newblock Cambridge University Press, 2004.

\bibitem{levine1936}
J.~Levine, {\it {Groups of motions in conformally flat spaces}},  {\em Bulletin
  of the American Mathematical Society} {\bf 42} (1936), no.~6 418 -- 422.

\bibitem{Umeh:2019qyd}
O.~Umeh, K.~Koyama, R.~Maartens, F.~Schmidt, and C.~Clarkson, {\it {General
  relativistic effects in the galaxy bias at second order}},  {\em JCAP} {\bf
  05} (2019) 020, [\href{http://arxiv.org/abs/1901.07460}{{\tt
  arXiv:1901.07460}}].

\bibitem{Umeh:2019jqg}
O.~Umeh and K.~Koyama, {\it {The galaxy bias at second order in general
  relativity with Non-Gaussian initial conditions}},  {\em JCAP} {\bf 12}
  (2019) 048, [\href{http://arxiv.org/abs/1907.08094}{{\tt arXiv:1907.08094}}].

\bibitem{Gibbons:PhysRevD.15.2752}
G.~W. Gibbons and S.~W. Hawking, {\it Action integrals and partition functions
  in quantum gravity},  {\em Phys. Rev. D} {\bf 15} (May, 1977) 2752--2756.

\bibitem{Goldwirth:1994ru}
D.~S. Goldwirth and J.~Katz, {\it {A Comment on junction and energy conditions
  in thin shells}},  {\em Class. Quant. Grav.} {\bf 12} (1995) 769--778,
  [\href{http://arxiv.org/abs/gr-qc/9408034}{{\tt gr-qc/9408034}}].

\bibitem{Navarro:1995iw}
J.~F. Navarro, C.~S. Frenk, and S.~D.~M. White, {\it {The Structure of cold
  dark matter halos}},  {\em Astrophys. J.} {\bf 462} (1996) 563--575,
  [\href{http://arxiv.org/abs/astro-ph/9508025}{{\tt astro-ph/9508025}}].

\bibitem{Maartens:1998xg}
R.~Maartens, T.~Gebbie, and G.~F.~R. Ellis, {\it {Covariant cosmic microwave
  background anisotropies. 2. Nonlinear dynamics}},  {\em Phys. Rev. D} {\bf
  59} (1999) 083506, [\href{http://arxiv.org/abs/astro-ph/9808163}{{\tt
  astro-ph/9808163}}].

\bibitem{Ellisbook:2012}
G.~Ellis, R.~Maartens, and M.~MacCallum, {\em Relativistic cosmology}.
\newblock Cambridge University Press, United Kingdom, 2012.

\bibitem{wang1993simple}
B.~Wang and T.~Li, {\it A simple tropical atmosphere model of relevance to
  short-term climate variations},  {\em Journal of the atmospheric sciences}
  {\bf 50} (1993), no.~2 260--284.

\bibitem{Morton1984}
B.~R. Morton, {\it The generation and decay of vorticity},  {\em Geophysical \&
  Astrophysical Fluid Dynamics} {\bf 28} (1984), no.~3-4 277--308,
  [\href{http://arxiv.org/abs/https://doi.org/10.1080/03091928408230368}{{\tt
  https://doi.org/10.1080/03091928408230368}}].

\bibitem{Kaiser:1987qv}
N.~Kaiser, {\it {Clustering in real space and in redshift space}},  {\em
  Mon.Not.Roy.Astron.Soc.} {\bf 227}.

\bibitem{Clarkson:2007yp}
C.~Clarkson, {\it {A Covariant approach for perturbations of rotationally
  symmetric spacetimes}},  {\em Phys. Rev. D} {\bf 76} (2007) 104034,
  [\href{http://arxiv.org/abs/0708.1398}{{\tt arXiv:0708.1398}}].

\bibitem{terringtonhouriganthompson:2022}
S.~Terrington, K.~Hourigan, and M.~Thompson, {\it Vorticity generation and
  conservation on generalised interfaces in three-dimensional flows},  {\em
  Journal of Fluid Mechanics} {\bf 936} (2022) A44.

\bibitem{bronsthompsonewekehourigan:2014}
M.~Brøns, M.~C. Thompson, T.~Leweke, and K.~Hourigan, {\it Vorticity
  generation and conservation for two-dimensional interfaces and boundaries},
  {\em Journal of Fluid Mechanics} {\bf 758} (2014) 63–93.

\bibitem{terringtonhouriganthompson:2020}
S.~J. Terrington, K.~Hourigan, and M.~C. Thompson, {\it The generation and
  conservation of vorticity: deforming interfaces and boundaries in
  two-dimensional flows},  {\em Journal of Fluid Mechanics} {\bf 890} (2020)
  A5.

\bibitem{Libeskind:2012ya}
N.~I. Libeskind, Y.~Hoffman, M.~Steinmetz, S.~Gottloeber, A.~Knebe, and
  S.~Hess, {\it {Cosmic vorticity and the origin of halo spins}},  {\em
  Astrophys. J. Lett.} {\bf 766} (2013) L15,
  [\href{http://arxiv.org/abs/1212.1454}{{\tt arXiv:1212.1454}}].

\bibitem{Ellis:2020kry}
G.~F.~R. Ellis, {\it {The Causal Closure of Physics in Real World Contexts}},
  {\em Found. Phys.} {\bf 50} (2020), no.~10 1057--1097,
  [\href{http://arxiv.org/abs/2006.00972}{{\tt arXiv:2006.00972}}].

\bibitem{Dunsby:1997fyr}
P.~K.~S. Dunsby, B.~A. C.~C. Bassett, and G.~F.~R. Ellis, {\it {Covariant
  analysis of gravitational waves in a cosmological context}},  {\em Class.
  Quant. Grav.} {\bf 14} (1997) 1215--1222,
  [\href{http://arxiv.org/abs/gr-qc/9811092}{{\tt gr-qc/9811092}}].

\bibitem{Gasperini:2009mu}
M.~Gasperini, G.~Marozzi, and G.~Veneziano, {\it {A Covariant and gauge
  invariant formulation of the cosmological 'backreaction'}},  {\em JCAP} {\bf
  02} (2010) 009, [\href{http://arxiv.org/abs/0912.3244}{{\tt
  arXiv:0912.3244}}].

\bibitem{Heinesen:2018vjp}
A.~Heinesen, P.~Mourier, and T.~Buchert, {\it {On the covariance of scalar
  averaging and backreaction in relativistic inhomogeneous cosmology}},  {\em
  Class. Quant. Grav.} {\bf 36} (2019), no.~7 075001,
  [\href{http://arxiv.org/abs/1811.01374}{{\tt arXiv:1811.01374}}].

\bibitem{Anton:2023icm}
T.~Anton and T.~Clifton, {\it {Modelling the emergence of cosmic anisotropy
  from non-linear structures}},  \href{http://arxiv.org/abs/2302.05715}{{\tt
  arXiv:2302.05715}}.

\bibitem{Buchert:2008zm}
T.~Buchert and M.~Carfora, {\it {On the curvature of the present-day
  Universe}},  {\em Class. Quant. Grav.} {\bf 25} (2008) 195001,
  [\href{http://arxiv.org/abs/0803.1401}{{\tt arXiv:0803.1401}}].

\bibitem{Umeh:2010pr}
O.~Umeh, J.~Larena, and C.~Clarkson, {\it {The Hubble rate in averaged
  cosmology}},  {\em JCAP} {\bf 1103} (2011) 029,
  [\href{http://arxiv.org/abs/1011.3959}{{\tt arXiv:1011.3959}}].

\bibitem{Clarkson:2011zq}
C.~Clarkson, G.~Ellis, J.~Larena, and O.~Umeh, {\it {Does the growth of
  structure affect our dynamical models of the universe? The averaging,
  backreaction and fitting problems in cosmology}},  {\em Rept. Prog. Phys.}
  {\bf 74} (2011) 112901, [\href{http://arxiv.org/abs/1109.2314}{{\tt
  arXiv:1109.2314}}].

\bibitem{Adamek:2014xba}
J.~Adamek, R.~Durrer, and M.~Kunz, {\it {N-body methods for relativistic
  cosmology}},  {\em Class. Quant. Grav.} {\bf 31} (2014), no.~23 234006,
  [\href{http://arxiv.org/abs/1408.3352}{{\tt arXiv:1408.3352}}].

\bibitem{Penrose:1964wq}
R.~Penrose, {\it {Gravitational collapse and space-time singularities}},  {\em
  Phys. Rev. Lett.} {\bf 14} (1965) 57--59.

\bibitem{Dai:2015rda}
L.~Dai, E.~Pajer, and F.~Schmidt, {\it {Conformal Fermi Coordinates}},  {\em
  JCAP} {\bf 11} (2015) 043, [\href{http://arxiv.org/abs/1502.02011}{{\tt
  arXiv:1502.02011}}].

\bibitem{Wang:2013gtn}
X.~Wang, A.~Szalay, M.~A. Aragon-Calvo, M.~C. Neyrinck, and G.~L. Eyink, {\it
  {Kinematic Morphology of Large-scale Structure: Evolution from Potential to
  Rotational Flow}},  {\em Astrophys. J.} {\bf 793} (2014), no.~1 58,
  [\href{http://arxiv.org/abs/1309.5305}{{\tt arXiv:1309.5305}}].

\bibitem{Pitrou:2013hga}
C.~Pitrou, X.~Roy, and O.~Umeh, {\it {xPand: An algorithm for perturbing
  homogeneous cosmologies}},  {\em Class. Quant. Grav.} {\bf 30} (2013) 165002,
  [\href{http://arxiv.org/abs/1302.6174}{{\tt arXiv:1302.6174}}].

\bibitem{Brizuela:2008ra}
D.~Brizuela, J.~M. Martin-Garcia, and G.~A. Mena~Marugan, {\it {xPert: Computer
  algebra for metric perturbation theory}},  {\em Gen.Rel.Grav.} {\bf 41}
  (2009) 2415--2431, [\href{http://arxiv.org/abs/0807.0824}{{\tt
  arXiv:0807.0824}}].
