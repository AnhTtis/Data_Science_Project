...
%%%%% PREPRINT NUMBERS %%%%%%
{\small \noindent \mydate \hfill OU-HET-1169}
...
{\small \it $^e$Department of Physics, National Taiwan University,Taipei, Taiwan 10617, R.O.C.} \\
...
\noalign{\kern 5pt}
&\begin{pmatrix}  h^L_{Z^{(n)}} (z) \cr \mynoalign
 h^R_{Z^{(n)}} (z) \cr  \mynoalign
 \hat h_{Z^{(n)}} (z) \cr  \mynoalign
 h^B_{Z^{(n)}} (z)  \end{pmatrix}
= \frac{1}{\sqrt{ \, 2 \, r_{Z^{(n)}} }} \Bigg\{ 
\begin{pmatrix} (1 + \cos \theta_H) \,C(z, \lambda_{Z^{(n)}} ) \cr \mynoalign
(1 - \cos \theta_H) \,C(z, \lambda_{Z^{(n)}} ) \cr  \mynoalign
 - \sqrt{2} \sin \theta_H  \hat S (z, \lambda_{Z^{(n)}}  )  \cr  0\end{pmatrix}\cr
&\hskip 4.cm
 - 2 \sin \theta_W^0
 \begin{pmatrix} \sin \theta_W^0 \cr \mynoalign
 \sin \theta_W^0 \cr     0 \cr 
\sqrt{1 - 2 \sin^2 \theta_W^0} \end{pmatrix} C(z, \lambda_{Z^{(n)}} )   \Bigg\}
 ~~,\cr
\noalign{\kern 5pt}
...
the $Z$ couplings into the $U(1)_\EM$ part and the rest.   We define
\begin{align}
...
&=T^3_u \cos \theta_W^0  \sqrt{kL} \int_1^{z_L} dz \, \bigg\{ h^L_{Z^{(0)}}  f^{u^{(n)}*}_{L/R}  f^{u^{(m)}}_{L/R}
+ h^R_{Z^{(0)}}  g^{u^{(n)}*}_{L/R}  g^{u^{(m)}}_{L/R} \cr
\noalign{\kern 5pt}
&\hskip 3.5cm
+ \frac{i}{\sqrt{2}} \,  \hat h_{Z^{(0)}} \Big(  f^{u^{(n)}*}_{L/R}  g^{u^{(m)}}_{L/R} -  g^{u^{(n)}*}_{L/R} f^{u^{(m)}}_{L/R} \Big) \bigg\} , \cr
\noalign{\kern 5pt}
...
&=T^3_d \cos \theta_W^0  \sqrt{kL} \int_1^{z_L} dz \, \bigg\{ h^L_{Z^{(0)}}  f^{d^{(n)}*}_{L/R}  f^{d^{(m)}}_{L/R}
+ h^R_{Z^{(0)}}  g^{d^{(n)}*}_{L/R}  g^{d^{(m)}}_{L/R} \cr
\noalign{\kern 5pt}
&\hskip 3.5cm
+ \frac{i}{\sqrt{2}} \,  \hat h_{Z^{(0)}} \Big(  f^{d^{(n)}*}_{L/R}  g^{d^{(m)}}_{L/R} -  g^{d^{(n)}*}_{L/R} f^{d^{(m)}}_{L/R} \Big) \bigg\} , \cr
\noalign{\kern 5pt}
...
\begin{align}
\hat g^{Z\nu_{e ab}}_{L/R, nm} 
&=T^3_{\nu_e} \cos \theta_W^0  \sqrt{kL} \int_1^{z_L} dz \, \bigg\{ h^L_{Z^{(0)}}  f^{\nu_{e}^{a(n)}*}_{L/R}  f^{\nu_e^{b(m)}}_{L/R}
+ h^R_{Z^{(0)}}  g^{\nu_e^{a(n)}*}_{L/R}  g^{\nu_e^{b(m)}}_{L/R} \cr
\noalign{\kern 5pt}
&\hskip 3.5cm
+ \frac{i}{\sqrt{2}} \,  \hat h_{Z^{(0)}} \Big(  f^{\nu_e^{a(n)}*}_{L/R}  g^{\nu_e^{b(m)}}_{L/R} -  g^{\nu_e^{a(n)}*}_{L/R} f^{\nu_e^{b(m)}}_{L/R} \Big) \bigg\}  .
\label{Znu2}
\end{align}
