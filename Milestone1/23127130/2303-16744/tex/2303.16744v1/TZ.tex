...
\begin{abstract}
We point out that a modified temperature-redshift relation ($T$-$z$ relation) of the cosmic microwave background (CMB) can not be deduced by any observational method that appeals to an a priori thermalisation to the CMB temperature $T$ of the excited states in a probe environment of independently determined redshift $z$. For example, this applies to quasar-light absorption by a damped Lyman-alpha system due to atomic as well as ionic fine-splitting transitions or molecular rotational bands. Similarly, the thermal Sunyaev-Zel'dovich (thSZ) effect cannot be used to 
extract the CMB's $T$-$z$ relation. This is because the relative line strengths between ground and excited states in the former and the CMB spectral distortion in the latter case both depend, apart 
from environment specific normalisations, solely on the dimensionless spectral variable $x=\frac{h\nu}{k_B T}$. Since literature on extractions of the CMB's $T$-$z$ relation always assumes (i) $\nu(z)=(1+z)\nu(z=0)$ where $\nu(z=0)$ is the observed frequency in the heliocentric rest frame, 
the finding (ii) $T(z)=(1+z)T(z=0)$ just confirms the expected {\sl blackbody nature} of the interacting CMB at $z>0$. In contrast to emission of isolated, directed radiation, whose frequency-redshift relation ($\nu$-$z$ relation) {\sl is} subject to (i), 
a non-conventional $\nu$-$z$ relation $\nu(z)=f(z)\nu(z=0)$ of pure, isotropic blackbody radiation, subject to adiabatically slow cosmic expansion, necessarily has to follow that 
of the $T$-$z$ relation $T(z)=f(z)T(z=0)$ and vice versa. In general, the function $f(z)$ is determined by energy conservation of the CMB fluid in a Friedmann-Lemaitre-Robertson-Walker universe. If the pure CMB is subject to an SU(2) rather than a U(1) gauge principle, then $f(z)= \left({1/4}\right)^{1/3}(1+z)$ for $z\gg 1$, and $f(z)$ is nonlinear for $z\sim 1$. 
\end{abstract}
...
Cosmic Microwave Background (CMB) radiation \cite{1994ApJ...420..439M} are important probes for the extraction of 
cosmological parameters \cite{Aghanim:2018eyx}. Since the observed angular correlations chiefly are 
...
Therefore, they sensitively depend on the temperature ($T$) - redshift ($z$) relation ($T$-$z$ relation) assumed in expressing the CMB's energy density $\rho(T)$ 
in terms of $z$. If the CMB possesses no degrees of freedom other than ordinary thermal photons (subject to a quantum U(1) gauge theory) then, according 
to the Stefan-Boltzmann law and energy conservation in a Friedmann-Lemaitre-Robertson-Walker 
(FLRW) universe, the $T$-$z$ relation simply is $T(z)/T(z=0)=z+1$ where $T(z=0)$ is today's CMB temperature $T(z=0)=2.726\,$K \cite{1994ApJ...420..439M}. Such a U(1) $T$-$z$ relation is identical to the frequency - redshift relation ($\nu$-$z$ relation) 
$\nu(z)/\nu(z=0)=z+1$ describing electromagnetic waves emitted by compact astrophysical objects 
and travelling through an expanding FLRW universe towards the observer \cite{Weinberg:1972kfs}.\\

The thermodynamics underlying the CMB -- in its spectral bulk representing a quantum gas of photons -- or the thermodynamics of a dense gas of absorber-emitter particles, thermalising via electromagnetic waves facilitated by electronic transitions, may be richer than they appear such that the two situations need to be distinguished. That is, the conventional $T$-$z$ relation may not hold 
universally but, depending on how the above two extreme situations are mixed, is modified as 
$T(z)/T(z=0)=f(z)$ where the function $f(z)$ is specific to the generalising theory. 
Thermodynamics then immediately implies that also the $\nu$-$z$ relation 
is of the form $\nu(z)/\nu(z=0)=f(z)$. Such is the case, for example, 
if the thermal photons (and low-frequency waves) of the CMB are identified with the Cartan modes of a single SU(2) Yang-Mills theory 
in the deconfining phase \cite{Hofmann:2016cqh,Hahn:2017yei}.   
...
the rapidly attained Stefan-Boltzmann law $\propto T^4$ associated with thermal fluctuations \cite{Hofmann:2016cqh}.\\
...
inflicted by its photons scattering off hot electrons belonging to X-ray clusters along the line of sight, the thermal Sunyaev-Zel'dovich effect (thSZ). In Sec.\,\ref{OS} we discuss these two observational approaches in 
...
is used to extract a temperature $T(z)$. Note that $T(z)$ would not coincide with the present CMB's temperature 
$T(z=0)$ if it was not redshifted according to $T(z)/(z+1)=T(z=0)$. In other words, ignoring the value of a known transition frequency $\nu^*(z)$ of the system in using a different $\nu$-$z$ relation for $\nu(z)$, 
$\nu(z)=f(z)\nu(z=0)$, the extracted CMB temperature would only have redshifted to its present 
value under the use of $T(z)/f(z)=T(z=0)$. It thus appears that in a given absorber system, 
...
of the absorbing molecules. For deconfining SU(2) Yang-Mills thermodynamics $\nu(z) \rightarrow \nu^*(z)$ is an upward shift, see Sec.\,\ref{YM}.\\

In moving from a photon gas (the bulk of the pure CMB void of electronic charges) towards the center of the cloud (bound electrons facilitating electromagnetic transitions) the degrees of freedom invoked for thermalisation no longer are the same, thus causing frequency and temperature to react differently to cosmological expansion. 
As a result, $T^*(z)=(z+1)T(z=0)$ is interpreted as the 
CMB's $T$-$z$ relation, while actually it is $T(z)=f(z)T(z=0)$. In exploiting the thSZ effect for the CMB's $T$-$z$ relation extractions, 
we observe a similar situation. In Sec.\,\ref{YM} we review \cite{Hofmann:2016cqh,Hahn:2017yei} how a modified $T$-$z$ relation (and then the $\nu$-$z$ relation) 
arises if the CMB is subject to deconfining SU(2) rather than U(1) quantum thermodynamics and how the Yang-Mills scale of such an SU(2) model, in the following referred to as SU(2)$_{\rm CMB}$, is fixed by observation. Moreover, we discuss how the Cartan mode of another SU(2) model, SU(2)$_{\rm e}$ \cite{2017Entrp..19..575H,Hofmann:2020yhn,2016tqym.book.....H}, whose two stable solitonic 
excitations in the confining phase represent 
the first-family lepton doublet, mixes with that of SU(2)$_{\rm CMB}$.
Such a mixing depends,
up to temperatures $\sim 9.51\,$keV, on the degree of thermalisation prevailing in a local environment of electromagnetically interacting electronic charges within a certain range of frequencies and charge densities. 
The afore-mentioned upward shift of CMB frequency $\nu(z)$ to absorption line frequency $\nu^*(z)$, accompanied by a shift of CMB temperature $T(z)$ to cloud temperature $T^*(z)$, would then be a consequence of an incoherent mixture 
...
Finally, in Sec.\,\ref{SC} we summarise the results of this paper and mention an 
observational signature which is sensitive to the CMB's $T$-$z$ relation: the spectrum of ultrahigh-energy cosmic rays. 
...
The first class of probes comprises absorbing clouds of known redshifts (e.g. parts of damped Lyman-$\alpha$ systems) 
in the line of sight of a distant 
quasar or a bright galaxy. Here the assumed thermalisation with the CMB populates the fine-structure 
levels of the ground states of certain atoms or ions \cite{1994Natur.371...43S,1997ApJG,2000Natur.408..931S,2002A&A...381L..64M,2022Natur.602...58R} or excites 
rotational levels of certain molecules \cite{2000Natur.408..931S,2011A&A...526L...7N,2017A&A...597A..82N} whose 
population ratios can be obtained from the respective absorption-line profiles within the broad background spectra.\\ 

Modelling environmentally 
dependent contributions to level populations, such as particle collisions or pumping by UV radiation, 
the relative level populations yield (upper-limit) estimates of $T(z)$ at the redshift of the cloud, see, 
however, \cite{2017ApJ...847...65M} for a discussion of the limitations of this approach. Note that in \cite{2013A&A...551A.109M} 
a solution of the rotational excitations of various 
molecular species could be provided directly from their spectra.\\

The second class of probes refers to the observation of the CMB spectrum within certain frequency bands 
along the lines of sight of X-ray clusters of known redshifts. A characteristic spectral distortion, known as the thermal Sunyaev-Zel'dovich effect (thSZ) \cite{1969Ap&SS...4..301Z,1972CoASP...4..173S} and caused by inverse Compton scattering of CMB photons off free, thermal electrons of these clusters, is exploited to estimate $T(z)$.  
...
for the theoretical basis and \cite{2011A&A...526L...7N,2017A&A...597A..82N,1994Natur.371...43S,2000Natur.408..931S,2013A&A...551A.109M,2022Natur.602...58R} for applications. Since sources of level 
excitations other than the CMB (e.g. collisions, UV pumping) have to be modelled for 
the absorbers at study the extracted $T(z)$ is usually seen as an upper bound on the true CMB temperature.\\   
...
transitions in five diffuse interstellar clouds. The thus extracted value $T(z\,=\,0)$ agrees well with the spectral CMB fit by COBE \cite{1994ApJ...420..439M} of $T(z=0)=(2.726\pm 0.010)\,$K. 
So what about $z>0$?\\
...
of the two column densities $N_{\rm LTE}$ in dependence on $T_{\rm rot}$. 
...
assumed to be described by an unmixed SU(2) model, see Sec.\,\ref{YM}.\\
%
%(Typical molecular 
...
In Sec.\,\ref{APE} we will discuss in more detail a degree-of-thermalisation dependent mixing of Cartan excitations 
...
unconventional \cite{Hahn:2017yei}.


\subsection{The thermal Sunyaev-Zel'dovich effect\label{SZ}}

The thermal Sunyaev-Zel'dovich effect (thSZ) is a distortion of the blackbody shape of the CMB spectrum which is 
induced by inverse Compton scattering of CMB photons off thermalised electrons 
in the X-ray plasmas of a given cluster of galaxies \cite{1969Ap&SS...4..301Z,1972CoASP...4..173S}. Neglecting contributions from the weakly relativistic high-end part of 
...
$\Delta T$ with respect to CMB baseline temperature $T$ at the cluster's redshift $z$ as \cite{2014A&A...561A.143H}
...
x_0\sim 3.83=\frac{\nu_0}{T(z=0)}\,,
\eqe
where $T(z=0)=2.726\,$K \cite{1994ApJ...420..439M} denotes the CMB temperature 
...
$I_{z=0}(\nu)d\nu=16\pi^2\frac{\nu^3}{\exp\left(\frac{2\pi\nu}{T(z=0)}\right)-1}d\nu$ \cite{1994ApJ...420..439M}. If we assume a $T$-$z$ relation of 
...
modelled thSZ emission law to the observations w.r.t. X-ray clusters within a given redshift bin $\delta$. 
...
When the CMB gauge field represents the Cartan subalgebra of an SU(2) Yang-Mills theory, SU(2)$_{\rm CMB}$, it can be shown \cite{Hahn:2017yei} that $f(z)$ is different from $f(z)=1+z$ in the $T$-$z$ relation, see also Sec.\,\ref{TRRCMB}, and therefore also in the $\nu$-$z$ relation. 
...
regime \cite{2022arXiv220511450H} which is not targeted by Planck frequency bands. 
In Sec.\,\ref{YM} we review how this $T$-$z$ relation arises, and we discuss why an according non-conventional 
$\nu$-$z$ relation is to be expected from a thermalisation dependence of the mixing angle between the Cartan modes of two SU(2) 
gauge models subject to disparate Yang-Mills scales.  
...
To explain the (highly isotropic) CMB radio excess at frequencies 
below 1\,GHz (a pronounced spectral distortion 
deeply within the Rayleigh-Jeans part), see \cite{Fixsen:2009xn} and references therein, 
we have in \cite{Hofmann:2009yh} argued that the 
critical temperature $T(z=0)$ 
for the deconfining-preconfining phase transition 
of SU(2) Yang-Mills thermodynamics is 
very close to the present temperature of the CMB of 
$T(z=0)=2.726\,$K \cite{1994ApJ...420..439M}. At first sight, this may seem to be a 
somewhat fine-tuned situation. The difference with an 
ordinary tuning of parameters by hand is that, 
due to an extremely steep (that is, critical) rise of the dual gauge coupling 
with temperature decreasing into the 
preconfining phase, the smallness of the extracted thermal quasiparticle 
mass of the Cartan mode (excess in radiance explained by redistribution 
of thermal spectral power into nonthermal noise due to 
evanescence of low-frequency CMB waves \cite{Hofmann:2009yh}), when interpreted as a Meissner mass 
of $\sim 100\,$MHz, dictates $T(z=0)$ to be very close to 
$T_{\rm c}$. However, due to a presently incomplete understanding 
of supercooling of the deconfining into the preconfining phase and associated tunnelling there is, as of yet, a tolerance of 
$\sim 10\,\%$ in this dynamic tuning of the two temperatures, which corresponds to about 1\,Gy of cosmic 
evolution \cite{2007EPJC...50..635G}. The exact assignment $T(z=0)=T_{\rm c}$, re-adressed below,  
implies a Yang-Mills scale $\Lambda_{\rm CMB}=
1.064\times 10^{-4}\,$eV, and thus it is justified to 
refer to the SU(2) Yang-Mills model, whose deconfining thermodynamics is assumed to describe 
the CMB, as SU(2)$_{\rm CMB}$.
...
of deconfining SU(2)$_{\rm CMB}$ thermodynamics. As usual, redshift 
...
dependence of the $T$-$z$ relation, since the equation of state for ground-state pressure $P^{\rm gs}$ and energy density $\rho^{\rm gs}$ is $P^{\rm gs}=-\rho^{\rm gs}$ \cite{2016tqym.book.....H}.

In deconfining SU(2)$_{\rm CMB}$ thermodynamics, asymptotic freedom \cite{Gross:1973id,Politzer:1973fx} occurs nonperturbatively for $T\gg T(z=0)$ \cite{2016tqym.book.....H}. The Stefan-Boltzmann limit is then well saturated, and therefore 
...
\caption{\protect{\label{Fig-1}} Plot of function ${\cal S}(z)$ in Eq.\,(\ref{soltT0}). The curvature in ${\cal S}(z)$ at low $z$ indicates the breaking of conformal invariance in the deconfining SU(2) Yang-Mills plasma for $T\gtrsim T(z=0)$ with a rapid approach towards $\left({1}/{4}\right)^{1/3}$ as $z$ increases. The conventional $T$-$z$ relation of the CMB, as used in the cosmological standard model $\Lambda$CDM, associates with the dashed line ${\cal S}(z)\equiv 1$. Figure adapted from \protect\cite{Hahn:2018dih}.}
\end{figure}
%%************************
%***********************
\begin{figure}
...
Here the fact that dual gauge coupling critically diverges as one lowers the CMB temperature $T$ through $T_{\rm c}$ explains 
the observation of a so called radio background (line temperatures significantly higher than baseline temperature $T(z=0)$ for frequencies lower than 1\,GHz \cite{Fixsen:2009xn}) in terms of a small thermal Meissner mass of 100\,MHz \cite{Hofmann:2009yh}. 
For this to be valid, an exact coincidence of $T_{\rm c}$ and $T(z=0)$ is not required. Rather, $T(z=0)$ could well be lower than $T_{\rm c}$ by about 10$\%$, acknowledging that supercooling into the deconfining phase and associated tunnelling processes are 
not yet treated in full theoretical rigour.\\

In any case, $T(z=0)$ is excluded to be {\sl larger} than $T_{\rm c}$. This can be seen as follows: There is another contribution to the excess in line temperatures at low frequencies from the fact that the frequency of waves (populating the deep Rayleigh-Jeans spectrum up to const/$T^2$) and the frequency of photons (starting to represent the spectrum for frequencies larger than const/$T^2$) redshift differently. On the other hand, baseline temperature $T(z)$ redshifts like the frequency of photons. That is, wavelike modes redshift as $\nu(z)=(1+z)\nu(z=0)$ while temperature (photon frequency) redshifts weaker than that like 
...
Observationally \cite{Fixsen:2009xn}, the onset of this effect could be visible at $\nu\sim$\,1\,GHz which implies the wavelengths of low-frequency waves 
to be larger than 30\,cm, in turn, implying a critical temperature lower than 11.6\,K \cite{2016tqym.book.....H}. For the differential evolution of baseline temperature, we have 
\eqb
\label{riseT}
dT=T(z=0)\left(\frac{dS(z)}{dz}(1+z)+S(z)\right)\,dz\,.
\eqe
Since the present CMB’s line temperature rises steeply with a spectral index of $-2.6$ when lowering the frequency, see \cite{Fixsen:2009xn} and references on ground based radio-observations therein, the effect of Eq.\,(\ref{riseT}), which, modulo a mild stacking of low frequencies, is frequency independent 
...
The electron and its neutrino are associated 
with an onefold-selfintersecting, figure-eight like center-vortex loop and a single center-vortex loop, respectively, see \cite{2017Entrp..19..575H,Hofmann:2020yhn} and \cite{2016tqym.book.....H}. These excitations are immersed into the 
confining ground state of SU(2) Yang-Mills theory. A mass formula can be derived for the 
electron which equates the frequency of a breathing monopole \cite{PhysRevLett.92.151801,PhysRevLett.92.151802} 
(or the quantum selfenergy \cite{refId0,1964deBroglie}), contained 
within an extended blob associated with the region of vortex intersection, with the sum of the static monopole's rest mass 
and the energy content of deconfining SU(2) Yang-Mills 
thermodynamics of the blob at a 
temperature $T_{P=0}=1.315\,T_{\rm c}$ where the pressure vanishes. By invoking the value of the electron mass $m_{\rm e}=511\,$keV, this formula yields a value of the SU(2) Yang-Mills scale $\Lambda_{\rm e}=4.31\,$keV or a critical 
temperature $T_{c,\rm e}=9.51\,$keV for the 
deconfining-preconfining phase transition. In addition, one obtains a 
blob radius $r_0=0.2023\,$\AA. Note that such a value of radius $r_0$ is quite 
comparable to the Bohr radius $a_0=0.592\,$\AA. We refer to the SU(2) Yang-Mills theory thus 
made responsible for the emergence of the electron and its neutrino as SU(2)$_{\rm e}$.\\

If the global temperature of a photon gas is smaller than $T_{c,\rm e}$ then these 
photons must be thermalised w.r.t. SU(2)$_{\rm CMB}$ with 
$T_{c,\rm CMB}\sim 10^{-4}\,\mbox{eV}\ll 9.51\,\mbox{keV}\sim T_{c,\rm e}$. On the other hand, 
encountered then, as a nonthermal mode, it cannot be subject to the gauge group 
SU(2)$_{\rm CMB}$ but rather is described by SU(2) Yang-Mills theories of much larger 
Yang-Mills scales. The process of converting these isolated waves, emitted by the charge-carriers of a closed surface, into a thermal photon 
gas hence proceeds by chopping their coherent intensity distribution into grainy and short-lived energy-momentum packets 
by an increasing homogenisation and isotropisation of energy transport as more and more differently directed waves of varying oscillation frequencies superposition away from their emitting surfaces. This very process of thermalisation of the photon gas to 
the temperature of the emitting surface effectively can be understood as a rotation of SU(2) 
...
consider the interplay of gauge groups SU(2)$_{\rm CMB}$ and SU(2)$_{\rm e}$. The discussion in Sec.\,\ref{APE} can then be summarised as 
\eab
\label{rotationgf}
\bar{a}^{3,\rm e}_\mu&=&a^{3,\rm e}_\mu\,\cos\theta-a^{3,\rm CMB}_\mu\,\sin\theta\,,\nonumber\\ 
\bar{a}^{3,\rm CMB}_\mu&=&a^{3,\rm e}_\mu\,\sin\theta+a^{3,\rm CMB}_\mu\,\cos\theta\,,
\eae
where $\bar{a}^{3,\rm e}_\mu$ and $\bar{a}^{3,\rm CMB}_\mu$ refer to the rotated states 
reached from the initial states $a^{3,\rm e}$ and $a^{3,\rm CMB}$ (Cartan gauge fields in SU(2)$_{\rm e}$ 
and in SU(2)$_{\rm CMB}$ in unitary-Coulomb gauge \cite{2016tqym.book.....H}) 
upon a change of the state of thermalisation as expressed by the rotation angle 
$\theta=\theta(\eta,T^*,T)$. It is clear that mixing angle $\theta$ 
depends on the degree of thermalisation $\eta$ invoked by the initial states of electromagnetically interacting electrons of temperature $T^*$ 
and the temperature $T$ of pure radiation, maximum mixing corresponding to full thermalisation. 
Via the degree of thermalisation $\eta$, the mixing angle $\theta$ also 
depends on the range of frequencies $\nu_l\le\nu\le\nu_u$ of wavelike modes which mediate the interactions between the 
electrons. Due to the present CMB exhibiting the largest low-frequency interval of excitations 
being associated with (evanescent) waves throughout its cosmological history, see Sec.\,\ref{TRRCMB}, 
it is qualitatively understandable that the extraction of $T(z=0)$ from background-source-absorber-cloud systems 
in the Milky Way \cite{1993ApJ...413L..67R} yields the same value (full thermalisation) as obtained from the 
CMB blackbody spectral fit in \cite{1994ApJ...420..439M}. Due to the strong compression of this 
interval with a factor $\propto T^{-2}$ it is also plausible that temperature 
extracted from a background-source-absorber-cloud system at an earlier epoch {\sl differs} from the associated CMB temperature.\\

The quantitative computation of $\theta$ in a situation, 
where a system of interacting (bound) electrons is immersed into the CMB, is a complex task 
which we hope to gain more insight about in the future. 

\section{Summary and Outlook\label{SC}}

In this paper we have discussed two main approaches to extract CMB temperature at finite redshifts: the 
analysis of absorption line profiles originating from gases of atoms, ions, or molecules within 
the line of sight of a broad-spectrum and bright background source as well as the thermal Sunyaev-Zel'dovich effect (thSZ). In the literature the 
assumption of a conventional frequency-redshift relation ($\nu$-$z$ relation) for CMB photons, considered to thermalise with relevant transitions in the cloud systems in the former case or to represent CMB spectral distortions in the latter situation, yields a conventional temperature-redshift relation ($T$-$z$ relation) of the CMB. We have argued, based on the blackbody spectrum at all redshifts, that this is a consequence of thermodynamics. Whatever the assumed $\nu$-$z$ relation, observations necessarily produce the associated $T$-$z$ relation and vice versa. If the CMB is subject to an SU(2) rather than a U(1) quantum gauge principle, we have reviewed how the according $T$-$z$ relation changes. Consequently, the CMB's $\nu$-$z$ relation is changed. Finally, we have, on a qualitative level, given reasons why the temperature of a cloud of known redshift may differ from the temperature of a pure photon gas representing the CMB far away from the cloud. This is because thermalisation in the cloud, besides the interaction with CMB photons, also proceeds via emissions and absorptions of wavelike modes in association with electronic transitions.\\

Our conclusion about the extractions of CMB temperature using the thSZ effect as pursued in the literature is that they confirm the CMB blackbody spectrum at finite redshift. However, under the assumed conventional $\nu$-$z$ relation, the result of the $T$-$z$ relation extraction necessarily is conventional as well.\\

The extraction of the CMB's $T$-$z$ relation from an assumed thermalisation within absorber clouds, which also uses a conventional $\nu$-$z$ relation for the absorption lines in question, is not guaranteed to yield physical results since the two systems, (i) a cloud immersed into the CMB and 
(ii) a pure CMB,
are likely to thermalise to different temperatures at finite redshift.\\

One possibility to see the effect of the $T$-$z$ relation subject to SU(2)$_{\rm CMB}$ is to study the flux of ultra-high energy cosmic rays (UHECR). In particular, there is a sensitive region below the ankle, that is, for $1\times 10^9\,\mbox{GeV}\le E\le 6\times 10^9\,\mbox{GeV}$. Due to the reduced CMB photon density at one and the same finite redshift, there is a higher flux of UHECR under otherwise equal conditions for emission and propagation. Most prominently, the flux of protons is significantly increased in comparison to the use of the conventional $T$-$z$ relation in fits to UHECR data \cite{Meinert}.
...
This work was stimulated by an intense and fruitful exchange about a recent publication in Annalen der Physik between the two Referees and the current authors.\\

\noindent JM acknowledges insightful discussions with Professor Karl-Heinz Kampert and Dr. Alexander Sandrock.\\
