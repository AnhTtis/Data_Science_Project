...
PMG2000 is composed of 2,098 ultrasound images, and we randomly divide the entire dataset into a training part and a testing part at an 8:2 ratio.
%
\textbf{(b) HAM10000.}  
...
%
\textbf{(c) APTOS2019.}  In APTOS2019~\cite{aptos2019-blindness-detection}, 
%there are 3,662 labeled fundus images for grading diabetic retinopathy into five categories. 
A total of 3,662 fundus images have been labeled to classify diabetic retinopathy into five different categories.
%%
Following the same protocol in~\cite{gong2020distractor},  we split HAM10000 and APTOS2019 into a train part and a test part at a 7:3 ratio. 
...
%
%
Moreover, we introduce two widely-used metrics Accuracy and F1-score to quantitatively compare our framework against existing SOTA methods. 

\noindent \textbf{Implementation Details:}
Our framework is implemented with the PyTorch on one NVIDIA RTX 3090 GPU. 
%
We center-crop the image and then resize the spatial resolution of the cropped image to $224$$\times$$224$. Random flipping and rotation for data augmentation are implemented during the training processing.
In all experiments, we extract six $32$$\times$$32$ ROI patches from each image.
%%
%Random flipping and rotation are implemented for data augmentation in the training process. 
%
We trained our network end-to-end using the batch size of 32 and the Adam optimizer.
The initial learning rate for the denoising model U-Net is set as 1×10$^{-3}$, while for the DCG model (see Section~\ref{sec:dcg}) it is set to 2×10$^{-4}$ when training the entire network. 
%
...
\begin{table*}[!tbp]
\caption{Quantitative comparison to SOTA methods on three classification tasks. The best results are marked in bold font.}
...
Extensive experiments are conducted to evaluate the effectiveness of major modules of our network. To do so, we build three baseline networks from our method.  
%%
...
\noindent \textbf{Visualization of our Diffusion Procedure:} 
