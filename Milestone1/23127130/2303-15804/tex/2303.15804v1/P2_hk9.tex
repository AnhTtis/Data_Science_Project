...
\usepackage{comment}
\usepackage[numbers,sort&compress]{natbib} %for citation order in lists
...
%\thanks{Carolin Kleemann's and Johannes Heiny's research was partially supported by the Deutsche Forschungsgemeinschaft (DFG) via RTG 2131 High-dimensional Phenomena in Probability - Fluctuations and Discontinuity. We thank Peter Eichelsbacher for fruitful discussions. }
...
In \cite{decreusefond:schulte:thaele:2016}, Decreusefond, Schulte and Thäle provided an upper bound of the Kantorovich–Rubinstein distance between a PRM and the point process induced in the aforementioned way  by a Poisson or a binomial process on an abstract state space. Notice that convergence in Kantorovich-Rubinstein distance implies convergence in distribution. 
...
$a_n\sim b_n$ if $\lim_{n\to\infty} a_n/b_n=1$ and $a_n\lesssim b_n$ if $a_n$ is smaller than or equal to $b_n$ up to a positive universal constant. We further write $a\wedge b:=\max\{a,b\}$ for $a,b\in\R$ and for a set $A$ we denote $|A|$ as the number of elements in $A$.
...
\eeam
%{\red It might be slightly confusing for the reader. I suggest to use $n$ as subscript in all cases: $p_n, d_n, M_n$. The only quantity without subscript should be $p$ and we always write $\nto$. If we switch to $p \to \infty$, we confuse ourselves and the reader.}
...
Let $\x_1,\ldots, \x_p$ be $n$-dimensional, independent and identically distributed random vectors and $p=p_n$ is some sequence of positive integers tending to infinity as $\nto$. Additionally, let $g=g_n:\R^{mn}\to (v,w)$ be a measurable and symmetric function. Assume that there exists a function $\mu:(v,w)\to \R^+$ %with $\lim_{x \to v} \mu(x)=\infty$ and $\lim_{x \to w} \mu(x)=0$ 
...
Let $(U_i)_i$ be an iid sequence of random vectors uniformly distributed on $S_1$ and $\Gamma_i=E_1+\ldots+E_i$, where $(E_i)_i$ is an iid sequence of standard exponentially distributed random variables, independent of $(U_i)_i$. Additionally, we set
$N_{\Gamma}=\sum_{i=1}^\infty \varepsilon_{\Gamma_i}$ 
and write $\lambda_m$ for the $m$-dimensional Lebesgue measure. 
It is well--known that $N_\Gamma$ is a homogeneous Poisson process and hence it holds for every $A\subset(0,\infty)$ that $N_{\Gamma}(A)$ is Poisson distributed with parameter $\lambda_1(A)$ (see for example \cite[Example 5.1.10]{embrechts:kluppelberg:mikosch:1997}). For the mean measure $\eta$ of $M$ we get for a product of intervals $\otime{l=1}{m}(r_l,s_l]\subset S_1$
\begin{align*}
...
%\end{align*}
%\end{proof}
Setting $k=1$ we obtain the convergence in distribution of the maximum of the points $T_\iv$.
...
\end{align*}
where $\tilde{f}$ and $\tilde{g}$ are the right continuous extensions of $f$ and $g$ on $[0,1]$. Since $D[0,1]$ is separable with the Skorohod metric and complete with another metric, which generates the same Skorohod topology \cite[Theorem 12.2]{billingsley:1999}, these properties also hold for $D(0,1]$. Therefore, every probability measure on $D(0,1]$ is tight and convergence in distribution is equivalent to the convergence of the finite dimensional distributions.
...
\end{proposition}
Since $Y$ is a nondecreasing function, which is constant between isolated jumps, it has only countably many discontinuity points. Now let $(\tau_n)_n$ be the sequence of these discontinuity points of $Y$.
...
\begin{proof}
Part (1) is a direct consequence of the definitions of $\zeta$ and $L$. Part (2) follows by
\begin{align*}
...
Since
\begin{align*}
   \lim_{\nto} \P(p^{-1}(L(\zeta(p))-L(\zeta(p)-1))\leq x)\to  \P(\tau^{(1)}-\tau^{(2)}\leq x)
\end{align*}
...
\end{equation}
It is well--known that $\wt V_{ij}$ and $\wt U_{ij}$ are asymptotically standard normal, the following lemma provides a complementary large deviation result.
...
\end{equation}
where $\Gamma_i= E_1+\cdots+E_i$, $i\ge 1$, and $(E_i)$ is iid standard exponential, i.e., $N$ is a Poisson random measure with mean measure $\mu(x,\infty)=\e^{-x}$,
$x\in\R$. \\
...
\begin{enumerate}
\item[(1)] $\frac{p(p-1)}{2}\P(\tilde{V}_{12}>x_p)\to \e^{-x}$ as $n\to\infty$,
\item[(2)] $p^3\P(\tilde{V}_{12}>x_p, \tilde{V}_{13}>x_p)\to 0$ as $n\to\infty$,
\end{enumerate}
where $x_p=x/d_p+d_p$. We will begin with the proof of (1). Since $x_p\sim d_p=o(n^{1/6})$ we get by Lemma \ref{lem:tailUV}  
\begin{align*}
\frac{p(p-1)}{2}\P(\tilde{V}_{12}>x_p)=\frac{p(p-1)}{2}\bar{\Phi}(x_p)(1+o(1))
\end{align*}
...
\end{align*}
Regarding (2), we note that, by Lemma~\ref{lem:UVind}, $\tilde{V}_{12}$ and $\tilde{V}_{13}$ are independent. Thus, we get 
\begin{align*}
p^3\P(\tilde{V}_{12}>x_p, \tilde{V}_{13}>x_p)=p^3\P(\tilde{V}_{12}>x_p)^2=p^3(\bar{\Phi}(x_p)(1+o(1)))^2\to 0,\qquad n\to\infty,
\end{align*}
...
As in Theorem \ref{thm:rectime} we additionally can conclude point process convergence for the record times of the maxima of $V_{ij}$ and $U_{ij}$. 
Therefore, we take a look at the sequence $(\max_{1\leq i<j\leq k}U_{ij})_{k\geq 1}$. This sequence jumps at time $k$ if one of the random variables $U_{1k},\ldots, U_{k-1,k}$ is larger than every $U_{ij}$ for $1\leq i<j\leq k-1$. Between these jumps the sequence is constant.

Let $L^U$ be this sequence of jumps defined by
\begin{align*}
    L^U(1)&=1,\\
...
Let $\x_1,\ldots,\x_n$ be $p$-dimensional iid random vectors with $\x_i=(X_{1i},\ldots,X_{pi})$, where $\E[X_{ji}]=0$ for $1\leq j\leq p$ and $\bar{X}_j:=\frac{1}{n}\sum_{k=1}^n X_{jk}$. Denote $\Sigma=(\sigma_{i,j})_{1\leq i,j\leq p}$ as the covariance matrix  of the vector $\x_1$ and assume $\sigma_{ii}=1$ for $1\leq i\leq p$. The empirical covariance matrix $(\hat{\sigma}_{i,j})_{1\leq i,j\leq p}$ is given by
\begin{align*}
...
\end{itemize}
For technical reasons we consider (instead of $(M_{i,j})$) the transformed points $(W_{i,j})$ given by
\begin{align*}
...
Assume that $0<\Psi_4(0)<\infty$ and $\Var(X_{i1}X_{j})>0$ for all $i,j\in\Z$ and \\$|\operatorname{Cor}(X_{i1}X_{j1},X_{k1}X_{l1})|<1$ for all $i,j,k,l$, such that they are not all the same. Then, if either one of the conditions
\begin{enumerate}
...
The covariance matrix of $\x_1$ is given by $\operatorname{Cov}(\x_1)=A_nA_n^T$ with $(A_nA_n^T)_{ij}=\sum_{t=1}^p a_{it}a_{jt}$. Observe that the diagonal entries are equal to $1$. To satisfy assumption \eqref{assump} we have to assume that the entries except for the diagonal are smaller than $1$
\begin{align*}
...
%\end{align*}
\begin{align*}
h_{A_n}(k)=\sup_{1\leq i\leq p}\Big(\sum_{t=1}^{i-\lfloor k/2 \rfloor} a_{it}^2+\sum_{t=\lfloor k/2 \rfloor+i}^{p}a_{it}^2\Big)^{1/2}
\end{align*}
as a measure of how close the matrices $A_n$ are to diagonal matrices. 
For the point process convergence either (i) or (ii) has to be satisfied for $h_{A_n}$.
}\end{example}
...
$\lim\limits_{n\to\infty} \P(M_n(R)=0)=\e^{-\eta(R)}$.
 \end{enumerate}
Without loss of generality we can assume that the $A_k$'s are chosen disjointly.  First we will show (1). Set $T:=T_{(1,2,\ldots, m)}=g_n(\x_1,\x_2,\ldots, \x_m)$. If $q=1$ we get
\begin{align*}
...
&=|\P(M_n(R)=0)-P_n(0)|+o(1),
\end{align*}
where the last equality holds by (1). Therefore, we only have to estimate $|\P(M_n(R)=0)-P_n(0)|$. For this we employ the Stein-Chen method (see \cite{barbour:1992} for a discussion). The Stein equation for the Poisson distribution is given by
...
\begin{align*}
D&:=\{k:\,k=(k_1, k_2,\ldots,k_m), 1\leq k_1<k_2<\ldots<k_m\leq p\},\\
I_k&:=\sum_{i=1}^q\1_{A_i}(k/p)\1_{B_i}(T_k)\\
\eta_k&:=\E[I_k].
\end{align*}
For 
\begin{align*}
D_{1k}&:=\{l\in D:\, l_i\neq k_j,\, i,j=1,2,\ldots,m\}\quad\text{and}\\
D_{2k}&:=\{l\in D:\, l\neq k,\, l_i=k_j\,\,\text{for some}\,\, i,j=1,2,\ldots,m\}
\end{align*}
we have the disjoint union $D=D_{1k}\overset{.}{\cup} D_{2k}\overset{.}{\cup} \{k\}$, and therefore,
\begin{align*}
M_n(R)=\sum_{l\in D}I_l=\sum_{l\in D_{1k}}I_l+\Big(I_k+\sum_{l\in D_{2k}}I_l\Big)=:M^{(1)}_n(k)+M^{(2)}_n(k).
\end{align*}
Now, we bound \eqref{stein} by
\begin{align}
&\Big|\sum_{k\in D}\E[\eta_k x(M_n(R)+1)-I_k x(M_n(R))]\Big|\nonumber\\
&\leq \Big|\sum_{k\in D}\eta_k\E[x(M_n(R)+1))-x(M_n^{(1)}(k)+1)]\Big|+\Big|\sum_{k\in D}[\E[I_kx(M_n(R))]-\eta_k\E[x(M_n^{(1)}(k)+1)]]\Big|. \label{sum}
\end{align}
...
\begin{align*}
 \sum_{k\in D}\eta_k|\E[x(M_n^{(1)}(k)+M_n^{(2)}(k)+1))-x(M_n^{(1)}(k)+1)]|\leq\Delta x \sum_{k\in D}\eta_k\E[M_n^{(2)}(k)].
\end{align*}
Using the definitions of $\eta_k$ and $M_n^{(2)}(k)$, we get
%\begin{align*}
...
\begin{align}
&\sum_{k\in D}\eta_k\E[M_n^{(2)}(k)]\nonumber\\
&=\sum_{k\in D}\Big(\sum_{i=1}^q\1_{A_i}(\tfrac{k}{p})\P(T\in B_i)\Big)\Big(\sum_{l\in D}\sum_{j=1}^q\1_{A_j}\big(\tfrac{l}{p}\big)\P(T\in B_j)-\sum_{l\in D_{1k}}\sum_{j=1}^q\1_{A_j}(\tfrac{l}{p})\P(T\in B_j)\Big)\nonumber\\
&=\sum_{i=1}^q\sum_{j=1}^q\P(T\in B_i)\P(T\in B_j)\sum_{k\in D}\1_{A_i}(\tfrac{k}{p})\sum_{l\in D}\1_{A_j}(\tfrac{l}{p})\nonumber\\
&-\sum_{i=1}^q\sum_{j=1}^q\P(T\in B_i)\P(T\in B_j)\sum_{k\in D}\1_{A_i}(\tfrac{k}{p})\sum_{l\in D_{1k}}\1_{A_j}(\tfrac{l}{p})\,. \label{lan}
\end{align}
Since by assumption (A1) it holds that $p^m\P(T\in B_i)\to m!(\mu(r_i^{(m+1)})-\mu(s_i^{(m+1)}))$ as $n\to\infty$, and
\begin{align*}
\frac{1}{p^m}\sum_{k\in D}\1_{A_i}(\tfrac{k}{p})&\to\lambda_m(A_i),\qquad n\to\infty\\
\frac{1}{p^{2m}}\sum_{k\in D}\sum_{l\in D_{1k}}\1_{A_i}(\tfrac{k}{p})\1_{A_j}(\tfrac{l}{p})&\to\lambda_m(A_i)\lambda_m(A_j)\qquad n\to\infty,
\end{align*}
\eqref{lan} and thus the first term of \eqref{sum} tend to zero as $n\to\infty$. As $M_n^{(1)}(l)$ and $I_l$ are independent the second term of \eqref{sum} equals
\begin{align}
\Big|\sum_{k\in D}\E[I_k(x(M_n(R))-x(M_n^{(1)}(k)+1)]\Big|\leq \Delta x\sum_{k\in D}\E[I_k(M_n^{(2)}(k)-1)],\label{del}
\end{align}
where the last inequality follows from \eqref{zuwachs}. Since $I_k\leq 1$, the \rhs~in \eqref{del} is bounded above by
\begin{align}
&\Delta x\sum_{k\in D}\E[I_k\sum_{l\in D_{2k}}I_l]\nonumber\\
&=\Delta x \sum_{k\in D}\sum_{l\in D_{2k}}\E\Big[\Big(\sum_{i=1}^q\1_{A_i}(\tfrac{k}{p})\1_{B_i}(T_k) \Big)\Big(\sum_{j=1}^q\1_{A_j}(\tfrac{l}{p})\1_{B_i}(T_l)\Big)\Big]\nonumber\\
&=\Delta x \sum_{i=1}^q\sum_{j=1}^q\sum_{k\in D}\sum_{l\in D_{2k}}\1_{A_i}(\tfrac{k}{p})\1_{A_j}(\tfrac{l}{p})\P(T_k\in B_i,T_l\in B_j)\label{D2kr}
\end{align}
We set $D_{2k,r}:=\{l\in D:|\{l_1,\ldots, l_m,k_1,\ldots,k_m\}|=2m-r\}$. Therefore, \eqref{D2kr} is equal to
\begin{align*}
\Delta x \sum_{i=1}^q\sum_{j=1}^q\sum_{r=1}^{m-1}\sum_{k\in D}\sum_{l\in D_{2k,r}}\1_{A_i}(\tfrac{k}{p})\1_{A_j}(\tfrac{l}{p})\P(T_k\in B_i,T_l\in B_j).
\end{align*}
By assumption (A2), we have $p^{2m-r}\P(T_k\in B_i,T_l\in B_j)\to 0$ for $r=1, \ldots m-1$ as $n\to\infty$. Additionally, it holds that
\begin{align*}
\frac{1}{p^{2m-r}}\sum_{k\in D}\sum_{l\in D_{2k,r}}\1_{A_i}(\tfrac{k}{p})\1_{A_j}(\tfrac{l}{p})=O(1),\qquad n\to\infty.
\end{align*}
...
\begin{align*}
\delta< \frac{1}{2} \min\limits_{1\leq i,j\leq q}\Vert t_i-t_j\Vert
\end{align*}
small enough that the $\delta$-spheres of the distinct points of the set $\{(t_i,j_i)\}$ are disjoint and in $S_1\times [\beta,\infty)$, where $\Vert \cdot \Vert$ is the Euclidean norm. Pick $n$ so large that every $\delta$-sphere contains only one point of $\mathbf{m}_n$. Then set $\lambda_n:[0,1]\to [0,1]$ with $\lambda_n(0)=0$, $\lambda_n(1)=1$, $\lambda_n(t_{i,m})=t_{i,m}^{(n)}$ and $\lambda_n$ is linearly interpolated elsewhere on $[0,1]$. For this $\lambda_n$ it holds that
\begin{align*}
...
Our idea is to transfer the convergence of $N_n^X$ onto $N_n^Y$. To this end, it suffices to show that (see \cite[Theorem 4.2]{kallenberg:1983}) $N_n^Y-N_n^X \cip 0$ as $\nto$,
or equivalently that for any continuous function $f$ on $\R$ with compact support,
\begin{equation*}
