...
The goal is to output $0$ if $l\leq n$ and $1$ if $l>cn$ %, both with probability at least $9/10$.
(and otherwise the output can be either $0$ or $1$). %(i.e., the output does not matter).
\end{definition}
% \rnote{Please shorten this definition, otherwise readers will be bored to death already at this point. }
...
\begin{proof}
    ...
    % We show that the number of bits of communication is $\Omega(\log |C|)$.
    % For a random string $r$ representing the randomness of $A$, let $C_r\subset C$ be the set of all $s\in C$ for which Bob correctly learns $s$.
