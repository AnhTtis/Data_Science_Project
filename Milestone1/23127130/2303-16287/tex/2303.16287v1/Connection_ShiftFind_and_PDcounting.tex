...
% Denote Alice's input by $s^*\in \Sigma$;
% then Alice uses the shared randomness to execute the streaming algorithm $A'$,
% inserts $s^*$  to the stream, 
% and sends the algorithm's state (memory contents) to Bob.
...
to simulate algorithm $Q$ %on $F_{s^*}$ 
...
% Conditioned on a simulation of algorithm $Q$, 
