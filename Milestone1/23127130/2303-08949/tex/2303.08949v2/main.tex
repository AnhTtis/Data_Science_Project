...
The aim of this section and the following one is to provide a discussion of an extended example for the method we develop for computing quantum Steenrod operations for certain negative bundles over smooth projective varieties. The relevant geometry of the example is that of a local genus $0$ curve Calabi--Yau $3$-fold, namely $X = \mathrm{Tot}((\mathcal{O}(-1) \oplus \mathcal{O}(-1)) \to \mathbb{P}^1)$ the total space of the vector bundle $\mathcal{O}(-1)^{\oplus 2} \to \mathbb{P}^1$. 
...
\begin{defn}\label{defn:S1-quantum-product}
    The \emph{$S^1$-equivariant quantum product} $\ast_{S^1}$ is defined for $a, b \in H^*_{S^1}(X;\mathbb{F}_p)$ by menas of the Poincar\'e pairing $(\cdot, \cdot)_{S^1} : H^*_{S^1}(X;\mathbb{F}_p) \otimes H^*_{S^1}(X;\mathbb{F}_p) \to \mathbb{F}_p(\!(\teq)\!)$ as
    \begin{equation}
...
\end{prop}
\begin{proof}[Proof sketch]
    (ii) is a consequence of the observation that projecting to the $\teq$-constant term amounts to considering all cohomology classes $b \in H^*_{S^1}(X;\mathbb{F}_p)$ by their non-equivariant counterpart $b \in H^*(X;\mathbb{F}_p)$ and discarding for all $j>0$ the $(i, 2j)$th $(\mathbb{Z}/p \times S^1)$-equivariant moduli spaces with incidence constraints. The $(i,0)$th $(\mathbb{Z}/p \times S^1)$-equivariant moduli spaces exactly recover the $i$th $\mathbb{Z}/p$-equivariant moduli spaces.
...
\begin{proof}
    ...
    Now the integration formula \cref{prop:integration-formula-S1-QSt} gives
    \begin{align}
...
\begin{lem}\label{lem:QSt-structure-const-depends-on-remainder}
    ...
    \end{equation}
    only depends on the remainder of $d$ mod $p$.
\end{lem}
\begin{proof}
    ...
    \end{equation}
    (The division is taken in the Euclidean domain $\mathbb{F}_p(\!(t, \theta, \teq)\!) [\![H]\!]$.) The integral for the structure constant in degree $d$ is therefore the coefficient of $H^{2d+2p+1}$ in
    \begin{equation}
...
\begin{proof}[Proof of \cref{prop:QSt-annihilates-soln}]
    ...
    \begin{equation}
        \Sigma = \Sigma_\mu := Q\Sigma_b^{S^1}|_{\teq/ t = \mu}.
    \end{equation}
    ...
    \end{equation}
    the claim that $\Sigma I_\mu = 0$ is equivalent to $\Sigma I'_\mu = 0$. Below, without loss of generality we replace $I_\mu$ with $I'_\mu$. 
    
    Expand $\Sigma = \Sigma_\mu$ in power series, so that
    \begin{equation}
        \Sigma = \sum \Sigma_{d} q^d =  \Sigma_{0} + \Sigma_{1} \ q^1 + \Sigma_{2} \ q^2 + \cdots.
    \end{equation}
    By \cref{lem:QSt-structure-const-depends-on-remainder}, we have $\Sigma_{d} = \Sigma_{d+p}$ for all $d > 0$. Therefore the claim $\Sigma I_\mu = 0$ is equivalent to the claim that
    \begin{equation}
        q^p (\Sigma_{0} - \Sigma_{p}) I_\mu = (\Sigma_{0} + \Sigma_{1} q + \cdots + \Sigma_{p-1} q^{p-1}) I_{\mu}.
    \end{equation}
    Since $I_\mu$ has entries that are polynomials in $q$ bounded in degree $\le (p-1)$, this reduces the claim to a finite number of identities. Using the localization formula, one obtains the following simplified expression for the matrix $\Sigma = \sum \Sigma_d q^d$ in the stable envelope basis (see \cref{rem:QSt-simplifies}) for $0 \le d \le p-1$:
    ...
    \end{equation}
    From \eqref{eqn:ann-qst}, \eqref{eqn:ann-qst-0p}, and \eqref{eqn:ann-section}, it is now straightforward to verify that 
        \begin{equation}
        q^p (\Sigma_{0} - \Sigma_{p}) I_\mu = (\Sigma_{0} + \Sigma_{1} q + \cdots + \Sigma_{p-1} q^{p-1}) I_{\mu}.
    \end{equation}
...
\begin{equation}
    \prod_{k=0}^{p-1} ( x+ (\ell -k) t) = \prod_{k=0}^{p-1} (x - k  t) = x^{p-1} - t^{p-1};
\end{equation}
we have
\begin{align}
