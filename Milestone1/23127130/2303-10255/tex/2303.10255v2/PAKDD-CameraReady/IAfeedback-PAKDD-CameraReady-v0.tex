...
\newcommand{\ZX}[1]{{\bf \color{OliveGreen}{ZX: #1}}}
\newcommand{\KEV}[1]{{\bf \color{SkyBlue}{#1}}}
\newcommand{\DQS}[1]{{\bf \color{Peach}{DQS: #1}}}
\newcommand{\JL}[1]{{\bf \color{DarkOrchid}{JL: #1}}}
\newcommand{\YHZ}[1]{{\bf \color{Aquamarine}{YHZ: #1}}}
\newcommand{\SP}[1]{{\bf \color{CadetBlue}{SP: #1}}}
\newcommand{\HK}[1]{{\bf \color{Maroon}{HK: #1}}}
\usepackage{bbm}
...
\begin{document}
%
\title{How the Feedback Effect Shapes User Behavior with Intelligent Assistants}
...
\author{Zidi Xiu\textsuperscript{*}, Kai-Chen Cheng, David Q. Sun\textsuperscript{*}, Jiannan Lu, Hadas Kotek, Yuhan Zhang, Paul McCarthy,  Christopher Klein, Stephen Pulman, Jason D. Williams% All authors must be in the same font size and format. Use \Large and \textbf to achieve this result when breaking a line
}

\authorrunning{Z. Xiu, K. Cheng, D. Sun et al.}
\institute{Apple, One Apple Park Way, Cupertino, CA 95014, USA
\email{\{z\_xiu,dqs\}@apple.com}}
...
% \author{Zidi Xiu\textsuperscript{*}, Kai-Chen Cheng, David Q. Sun\textsuperscript{*}, Jiannan Lu, Hadas Kotek, Paul McCarthy,  Yuhan Zhang, Christopher Klein, Stephen Pulman, Jason D. Williams% All authors must be in the same font size and format. Use \Large and \textbf to achieve this result when breaking a line
% }
% \email{{z_xiu,dqs}@apple.com}
% \affiliation{%
%   \institution{\\Apple}
%   \country{USA}}
  
%
% \authorrunning{Z. Xiu et al.}
% \institute{Apple, USA
% \email{\{z\_xiu,dqs\}@apple.com}}
...
When building such an evaluation dataset on real-world usage, we inevitably introduce user behavior into the measurement. User interactions with IA are likely to be influenced by the their pre-existing perception of IA's capabilities and limitations, therefore introducing a bias in the distribution of ``chances of success'' in logged user interactions -- users are more likely to ask what they know the IA can handle. This hypothesis makes intuitive sense and has been partly suggested by an earlier study on vocabulary convergence in users learning to speak to an SDS \cite{levow2003learning}. 

In this context, we define \emph{feedback effect} as the behavior pattern changes of users of an interactive intelligent system (\EG, IA) that are attributable to their cumulative experiences with said system. To establish the existence of feedback effect and to understand its impact, we present two quantitative studies. First, we examine the causal link between IA system performance and immediate user engagement afterwards, with a randomly sampled and anonymized human-labeled IA dataset. We then conduct an exploratory study on the users' behavior changes over time. Our contributions can be summarized as follows. First, we establish a causal link between IA performance and immediate subsequent user activities, and quantify its impact on users of a real-world IA. Second, we identify distinct dynamics of behavior change for a cohort of new users over a set period of time, demonstrating how users first explore the IA's capabilities before eventually adapting or quitting.
...
%Comments from Ricardo
%you should probably note that you are to address observed confounding as accounting for unobserved confounding is much more difficult.

% In fact, at the end of 4.2, it will be important to say that you are assuming that there are not unobserved confounders given (1), (2) and (3), and though a reasonable assumption is a potential limitation of the study that may be addressed in the future with methods for unobserved confounding
 %(for detailed description for confounding variables, please check Supplemental Materials)
%  , as shown in Figure~\ref{fig:causalDiag}.
\begin{enumerate}
...
PS reflects the probability of assigned to treatment based on user's background attributes \cite{li2018balancing,rubin1974estimating}, $e(x)=P(Z_i=1|X_i=x)=\EE(\Zmat|\Xmat)$
% \vspace{-5pt}
...
% $= \EE[\nu(T_i(z);t)]$,
which is a population parameter. Without loss of generosity, we discuss the pseudo observation omitting the potential outcome notations.
...
while the dropout group has an average perplexity score of 10.6. This difference is statistically significant with $p$-value $<0.001$ with the t-test.
...
Crucially, as users adapt their behavior to the system  over time, we expect to observe fewer and fewer requests that may lead to unhelpful interactions with the IA---creating a \emph{feedback effect}. Consequently, we observe a bias which introduces a significant challenge to the meaningful offline evaluation of the IA system based on naive samples of the usage traffic. %We take a closer look at the impacts of this bias on evaluation metrics below.
...
\small{\bibliography{FeedbackEffect}}
