...
For over a decade, software tools have been available to assist with \textit{process execution}. The more sophisticated ones track and \textit{orchestrate} activities according to a BPMN model, register activity-related data and perform decision-making on further process evolution.

However, centralized orchestration introduces a trusted orchestrator party requirement when we move beyond single-entity processes. With the emergence of blockchain and distributed ledger technology, the potential of migrating various aspects of cross-organizational collaboration to a decentralized implementation has been recognized quite early.

%in general, and the orchestration of BPMN-expressed collaborations in particular,

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{img/examplemodel.pdf}
...
Consider the BPMN car leasing collaboration model in Figure~\ref{fig:example}\footnote{The model was created in the ''Digitisation, artificial intelligence and data age workgroup'' of the ongoing BME-MNB cooperation project. (MNB is the Hungarian National Bank.). In the interest of legibility, the process in Figure~\ref{fig:example} is slightly simplified; the full model is available in our project repository. }, where the internal processes of a car dealership, a leasing company and a financing bank have to be coordinated to accept a leasing application from a client. Individual executions of models are called \textit{instances} and have an evolving \textit{instance state}. The coloring in Figure \ref{fig:example} demonstrates a state: green denotes that the dealership has completed insurance processing and the leasing company and the bank would be able to begin processing. However, for the leasing company to proceed, the active downpayment checking (orange) has to be finished, then the downpayment filed, and a downpayment notification message sent and received.

Using blockchain-deployed smart contracts that track \textit{collaboration state}, the \textit{execution enablement} and \textit{execution obligation} of the activities of the various parties can be enforced without a single dedicated, trusted party. The transaction journal nature of blockchains can also ensure that the full trace is also stored in an immutable and irrepudiable way. While tracking the internal state of participant-internal processes on-chain is not always desirable, it is a valuable \textit{option}; e.g., when decisions have to be made in a way verifiable by the other collaborating parties.

Orchestrating and journaling messages and collaborative data handling are two additional aspects of collaborations that can be improved with ''blockchainification''. %These are a) facilitating message passing and securing the messages themselves; and b) reading and writing the data used collaboratively by the processes (expressed via ''data objects'' and ''data stores'' in BPMN). In both cases, mainly for performance and cost reasons, the dominant approach is to use smart contracts to store cryptographic (hash) commitments to externally handled messages and data modifications.
...
Tools and approaches exist to create orchestrator smart contracts from BPMN models (see Section~\ref{sec:procorch}). However, no systematic solution exists to protect sensitive collaboration state information in the smart contract state from parties who can read the blockchain but do not participate in the collaboration. In our example, a leasing company may wish that its competitors do not see how many open cases they have, how long it takes to perform key steps in the process, or what actual lease rates they apply.

Fulfilling such requirements is a privacy and confidentiality challenge that contradicts core blockchain design principles. Blockchain nodes must be able to \textit{validate} and \textit{execute} incoming transaction requests to reach consensus on the ledger update effects, be those changes of the balances of a natively tracked cryptocurrency or state changes of deployed smart contracts. If the transaction details are made ''incomprehensible'' to the nodes, e.g., by off-chain encryption, they can't validate the prerequisites for performing the transaction and compute state updates. %In this case, smart contracts can still serve as resilient storage for encrypted state updates or hash commitments but lose the ability to validate the request \textit{semantically}; in our case, whether the right party proposes the right state-advancement of the workflow. 
...
We assume full integrity (no successful attack on the consensus) and, for the sake of simplicity, deterministic finality for the underlying blockchain (accepted blocks do not get retracted). On the other hand, process external parties have full visibility of transactions requested from and accepted by the blockchain. We treat the blockchain as \textit{fair} -- any transaction submitted by a participant is included in a block in a reasonable time, irrespective of concurrent transaction request load.
...
\subsubsection{Security properties} we target the following integrity, availability and confidentiality guarantees. The integrity and availability properties are largely provided by prior art; our contributions lie in establishing group confidentiality despite using smart contracts. BPO-SC refers to a per-process instance Business Process Orchestrator smart contract.
