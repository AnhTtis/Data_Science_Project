...
Although several partially mitigative and corrective schemes can be built on this measure, we introduce the weakening assumption that the irrepudiable identifyability of participants halting execution this way is a sufficient disincentive.
...
Most aspects of the computational model are straightforward; we only expand on some important aspects of the BPMN model encoding and the logic for checking BPMN state change validity.
...
\label{subsec:encode}
The BPMN model logic is carried over into the zkWF program by a precomputed $P$ array (Section \ref{subsec:parray}). Additionally, to check whether the right paths are proposed for exclusive gateways, the expressions on the sequence flows after the gateways are also encoded in the program as assertions. Message passing and variable write permission checks are addressed similarly.
