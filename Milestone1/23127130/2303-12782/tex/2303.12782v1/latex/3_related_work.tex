...
% Universal Video Segmetnation Architectures.
...
%
Recent studies~\cite{VIS_TR,hwang2021video,seqformer,IDOL,TeViT} have extended DETR into VIS, proposing better ways to fuse different queries along the temporal dimension. 
%
...
%
STEP~\cite{STEP} proposes a metric named Segmentation and Tracking Quality (STQ) that decouples the segmentation and tracking error, along with long sequence VPS datasets. 
%
...
%
Recent works~\cite{park2022per} also perform clip-wise segmentation or tracking~\cite{cliptrack}. However, the former only focuses on single-object mask tracking, while the latter considers global tracking via clip-level matching. 
%
