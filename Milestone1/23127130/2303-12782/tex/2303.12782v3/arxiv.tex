...
\title{Tube-Link: A Flexible Cross Tube Framework for Universal Video Segmentation}
\author{
Xiangtai Li$^{1}$ \quad
Haobo Yuan$^{1}$ \quad
Wenwei Zhang$^{1,4}$ \quad \\
Guangliang Cheng$^{2,3}$ \quad
Jiangmiao Pang$^{4}$ \quad
Chen Change Loy$^{1 \textrm{\Letter}}$ 
\\[0.1cm]
\small $ ^1$ S-Lab, Nanyang Technological University \quad 
\small $ ^2$ University of Liverpool \quad $ ^3$ SenseTime Research \quad   \small $ ^4$ Shanghai AI Lab \\
...
\input{latex/1_abstract.tex}
\input{latex/2_introduction.tex}
\input{latex/3_related_work.tex}
\input{latex/4_method.tex}
\input{latex/5_experiment.tex}
\input{latex/6_conclusion.tex}
\input{latex/7_supplementary_file}
