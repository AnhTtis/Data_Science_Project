...
\author{\IEEEauthorblockN{Kyubihn Lee and Nam Yul Yu}
	...
	\thanks{This work was supported by the GIST Research Project grant funded by the GIST in 2023, and National Research Foundation of Korea~(NRF) grant funded by the Korea Government~(MSIT) (No. NRF-2022R1F1A1066143.)}}
...
In this paper, we propose a new end-to-end learning-based wireless image recognition scheme using the PyramidNet in edge intelligence. 
...
%Finally, we empirically examine the robustness of the proposed scheme against a network pruning strategy~\cite{l1_norm} to find out the possibility of further reducing on-device computation.
%Our main contributions are summarized as follows: 
%\begin{enumerate}
%	\item We propose a new E2E-WIR-P scheme with network splitting to reduce on-device computation of the IoT devices and improve the performance of image recognition in edge intelligence.
%	\item We apply SE block and network pruning to reduce the on-device computation and improve the performance of image recognition. 
%	\item We show the performance of proposed E2E-WIR-P compared to other E2E-WIR-P schemes, and analyze the on-device computation and the number of parameters of the network.
%\end{enumerate}
...
%As illustrated in Fig. \ref{Fig1}(b), the proposed E2E-WIR-P consists of feature extractor, compression encoder and decoder, and classifier. 
...
%\begin{figure}[t]
%	\centering
%	\includegraphics[width=0.4\textwidth, height=0.10\textheight]{Fig2.png}%\label{1d}
%	\caption{Pyramidal bottleneck residual unit with SE block~(PBRU-SE).}
%	\label{buildingBlock}
%\end{figure}
...
In Sec. IV-B, we will investigate the recognition accuracy over on-device computation by varying the splitting point of the SE-PyramidNet.
...
%\begin{figure}[t]
%	\centering
%	\subfigure[]{\includegraphics[width=0.145\textwidth, height=0.071\textheight]{Fig3a.png}}
%	\hfil
%	\subfigure[]{\includegraphics[width=0.185\textwidth, height=0.071\textheight]{Fig3b.png}}
%	\caption{Proposed autoencoder architecture for compression encoder and decoder. (a) compression encoder, (b) compression decoder.}
%	%(a) architecture of E2E-WIR-P (b) building block of SE-PyramidNet (c) compression encoder (d) compression decoder.}
%	\label{Fig3} 
%\end{figure} 
...
\begin{figure*}[th!]
	\centering
	\subfigure[]{\includegraphics[width=0.33\textwidth, height=0.22\textheight]{Fig4_SNR.eps}}\label{SNR}
	\hfil
	\subfigure[]{\includegraphics[width=0.33\textwidth, height=0.22\textheight]{Fig4_Bandwidth.eps}}\label{Bandwidth}
...
\begin{figure*}[th!]
	\centering
	\centering
	\subfigure[]{\includegraphics[width=0.33\textwidth, height=0.22\textheight]{Fig3_Bandwidth.eps}}
	\hfil
	\subfigure[]{\includegraphics[width=0.33\textwidth, height=0.22\textheight]{Fig3_ondevice.eps}}
...
%We split the SE-PyramidNet with the splitting point 45 for the remaining experiments of E2E-WIR-P, where we observed that it achieves the accuracy of image recognition with less than 3\% loss from the SE-PyramidNet over the Rayleigh fading channel through simulations. 
%To evaluate the performance of image recognition, we measure the top1 and top5 accuracies with test dataset images. 

To evaluate the performance of image recognition, we measure top1 and top5 accuracies of the test dataset images.
The top-$k$ accuracy measures the proportion of cases that the correct label is present within the $k$ labels having the highest predicted probabilities by the model.
...
%To compute the number of parameters and on-device computation of an IoT device, we use the \emph{get\_model\_complexity\_info} function of \emph{ptflops} in the Pytorch framework. 
...
%Fig. \ref{Fig4}(b) reveals that as long as $B\ge128$, the accuracies of E2E-WIR-P are stable with at most 3\% and 0.5\% losses of top1 and top5 accuracies, respectively, from those of the SE-PyramidNet, which allows low communication overhead.
...
%The performance of E2E-WIR-P nearly achieving that of the SE-PyramidNet demonstrates that our compression encoder and decoder work with the SE-PyramidNet effectively.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.38\textwidth, height=0.22\textheight]{Figure5.eps}%\label{1b}
...
%Fig. \ref{Split} sketches the accuracies of image recognition, on-device computation, and the number of parameters over various splitting points in SE-PyramidNet, where SNR = 15dB in Rayleigh fading channel. 
%We evaluate the performance by changing the splitting point by 9 PBRU-SEs, where the number of PBRU-SEs in the SE-PyramidNet is 54. 
%In Fig. \ref{Split}, the accuracies of image recognition, on-device computation and the number of parameters increase almost linearly as the splitting point is set backward.
%Fig. \ref{Split} shows that if we set splitting point 45, it can achieve the accuracies of image recognition with less than 3\% degradation from the SE-PyramidNet. 
%With this splitting point, our E2E-WIR-P can reduce on-device computation and the number of parameters by 13\% and 35\%, respectively, from the SE-PyramidNet. 
%Therefore, we use the splitting point 45 for network splitting in the remaining experiments of E2E-WIR-P.

%\subsection{Network pruning}
%To reduce the on-device computation further in an IoT device, we apply the network pruning to feature extractor and compression encoder.
%We use the pruning algorithm in \cite{l1_norm} to prune the filter having small $l_1$-norm values from convolution layers, which may have small effects on the recognition accuracy.
%In particular, we only prune the convolution layers in the BRUB shown in Fig. \ref{Fig1}(b), not on the SE block, since the SE block consists of fully connected layer which is fragile to %network pruning. 
%In this letter, we find the optimal pruning ratio to balance the on-device computation the performance of image recognition through simulations. 
...
A further study will be fruitful for investigating the robustness of the proposed scheme in different wireless channel environments, e.g., multipath fading.
