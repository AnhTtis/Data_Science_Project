...
%% Title
\title{Robust incremental learning pipelines for temporal tabular datasets with distribution shifts
%%%% Cite as
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Deriving time series using the procedure defined in \ref{def:derivedts}. For the purpose of the temporal ranking problem, the time-series output cannot be used directly as the prediction of the ranking tasks and therefore this transformation is not the last step of the pipeline
    \end{itemize}   
...
The above transformations present multiple ways to transform data formats within our machine-learning pipeline. In particular, any transformations starting with tabular features and ending with tabular targets can be used as predictions.
...
There are various benefits to reusing model predictions. 

Firstly, it provides a hierarchical structure of models, in which each model can be interpreted as an improvement/adaption of the previous ones to distributional shifts in data and the prediction quality of each model can be inspected independently. 

Moreover, reusing model predictions introduced a feedback learning loop where model predictions correct themselves in an incremental manner. This can be considered a primitive example of reinforcement learning which assumes the machine learning model itself has made zero impact on the environment. 
...
A major difference between our incremental learning pipeline from MLP is that training is done in a single forward pass rather than by back-propagation. This allows us to train very complicated pipelines even with little resources, as there is no need to put the whole model in (distributed) memory to pass gradients between layers. As each model within a layer can be trained independently, training models \textbf{within} a layer becomes an embarrassingly parallel task when multiple GPUs can be used.  The program code of the pipeline is also easier to maintain as there is no need to use specialised software packages to distribute data between GPUs.
...
Simple statistical rules can be applied on each feature time-series \textbf{independently} to summarise the history of feature performance. 
...
\end{equation*}
Similarly, the cumulative variance of the time series at time $t$ is defined as 
\begin{equation*}
...
Efficient algorithms \cite{WelfordB.P.1962NoaM} can be used to calculate the above quantities for the whole time series in $\mathcal{O}(N)$ time where $N$ is the length of the time series. The algorithm also only requires constant memory so that it can be used to process data streams that never end. 

In this paper, factor-timing models formed by the above two statistical rules (Cumulative Mean and Cumulative Sharpe) are used as benchmarks against other time-series prediction models. 

\begin{comment}
    There are many other valid statistical rules that can be proposed to summarise time series. Indeed, chart(technical) analysis that has been popular in finance can be also interpreted as a statistical rule under our framework. The biggest limitation of using statistical rules is that they have to be manually crafted and there is no solid mathematical theories backing their use of those. Therefore, we only pick two simple examples to illustrate the idea. 
\end{comment}
...
Random Fourier Transforms are then applied on the return series at each time step as in Algorithm \ref{alg:rft}. The key idea is to approximate a mixture model of Gaussian kernels with trigonometric functions \cite{Sutherland15}.  
...
Transformer \cite{Transformer17}, which is the state-of-the-art deep learning-based model for sequence modelling is used as a benchmark against other factor-timing models. The implementation in DARTS \cite{DARTS} is used. 

Standard parameters for the architectures of different deep learning models given in DARTS are used. The standard number of training epochs is fixed to a maximum of 100 epochs with early stopping applied on the \textbf{training set}, where the training process is stopped if Mean-Squared Error loss does not improve by $0.001$ for 20 rounds. A validation data set is not used. 

Similar to the feature engineering models above, a lookback window (which corresponds to the input sequence dimension) needs to be selected for each model along with the training size (which can be fixed or using all the valid feature/target pairs created since the start of time series). Again, using a fixed size of lookback and training sizes can limit the number of computational resources required for model training/inference and is therefore the recommended approach. 

\paragraph{Transformer}

Transformer \cite{Transformer17} uses self-attention mechanism within the encoder-decoder structure to create representations of the input and output sequences. The encoder maps the input sequence to a sequence of latent representations in which the decoder uses to generate the output sequence one prediction at a time. The encoder consists a fixed number of layers. Each layer consists of a multi-head self-attention mechanism followed by a fully connected feed-forward network. Residual connections are used. The decoder also consists of a fixed number of layers which each layer have similar structure to the encoder. Positional encodings are added at the end of the encoder and decoder stacks to learn the relative positions of values in the sequence. 

Scaled Dot-Product Attention is used as building blocks for multi-head attention. The formula for dot-product attention is given as follows. 
\begin{definition}[Scaled Dot-Product Attention] ~\\
    Given matrices Q(queries) of dimension $d_k$, K(keys) of dimension $d_k$ and V(Values) of dimension $d_v$, the output of scaled dot-product attention is given by 
    \begin{equation*}
        \textrm{Attention}(Q,K,V) = \textrm{softmax}\left( \frac{QK^T}{\sqrt{d_k}} \right) V 
    \end{equation*}
\end{definition}

Multi-head attention applies linear projection multiple times on the input data followed by scaled dot-product attention. 

\begin{definition}[Multi-head attention] ~\\
    Given projection matrices $W_i^Q, W_i^K, W_i^V, W^O$, multi-head attention is given by 
    \begin{align*}
...
Unlike neural networks, it is not strictly necessary to use early stopping in model training for GBDT models. For GBDT models, it is easy to extract model snapshots, defined as the model parameters captured at the different parts of the training process. This can be done without any additional memory costs. 

For example, for any GBDT model, model snapshots from a single trained model can be obtained as follows. Always starting with the first tree, the number of trees to be used (the ending tree) are set to be $10\%,20\%,\dots,100\%$ of the number of boosting rounds. This trivially gives 10 different GBDT models representing different model complexities while using the training time of a \textbf{single} model only. Therefore, instead of using hyper-parameter optimisation to select the number of trees, simply set the number of boosting rounds to a large number and then set a small learning rate, so that the learning process does not converge too early. 
...
However, for targets where linear effects are removed, feature neutralisation might not be able to improve model performances. 
...
Pearson correlation calculated on the whole \textbf{era} of target and predictions is used as the loss function at each training epoch. Feature neutralisation can be applied from the outputs of network architecture. The neutralised predictions are further standardised to zero mean and unit norm. The negative Pearson correlation of the standardised predictions and targets is then used as the loss function to train the network parameters. In particular, feature neutralisation is an integrated part of the training process, as the gradient updates and model parameters are based on neutralised predictions instead of raw predictions.  
...
Each factor-timing and tabular model is trained in an incremental manner as described in Section \ref{section:overview}, in which model parameters are updated as new data arrives. 
...
The incremental learning pipeline demonstrates self-similarity at various \textbf{scales}. In particular, the overall pipeline shared a similar structure with each of its components. 

The incremental learning pipeline can be interpreted as a gradient-boosted model with $\sum_{i=1}^L K_i$ base learners trained with $L$ boosting rounds, where $K_i$ base learners are trained in the $i$-th boosting round. Ideas from Bagging and Boosting are integrated within the pipeline. Each layer consists of multiple models trained in parallel as in bagging so that variance is reduced by combining predictions from different models within a layer. Boosting is achieved by adjusting the target based on predictions from previous layers for training in the following layers if needed. Therefore, if each building block is set to be a GBDT model, the overall pipeline would then apply gradient boosting at both the pipeline and component levels.

The incremental learning pipeline can also be interpreted as a neural network model where each node is now a machine learning model instead of a parameter. As predictions from previous layers can be used as features for the following layers, this simulates the residual connections in some neural network architecture. Each layer in the pipeline will refine the prediction as neural networks. Therefore, if each building is set to be a neural network, the overall pipeline would then be a neural network of neural networks. 
...
The incremental learning pipeline is a complete end-to-end autoML tool which transforms the given features into predictions. Different machine learning tasks, such as feature engineering and model stacking are integrated within the pipeline as follows. A key feature is that tabular models such as GBDT or MLP can be used repeatedly in different layers of the pipeline to achieve the purpose of different machine learning tasks, which were previously considered separate tasks. In particular, even with just two building blocks, GBDT and MLP models, very complicated and effective incremental learning pipelines can be built. 
...
A key issue that remained unresolved in the previous study \cite{ThomasW23} is that the degree of feature neutralisation and the size of the subset of features to be neutralised is given by useful heuristics. Clearly, these parameters should be learned from the dataset and adjusted according to data distribution shifts. Under the incremental learning pipeline, model predictions undergoing different feature neutralisation processes can be combined as a soft selection of model hyper-parameters. 
...
Here, Corr is the Pearson correlation function. The purpose of applying a power transformation is that emphasis the contribution from the highest and lowest predictions. 


\begin{comment}
    This paragraph is optional and can be taken away without impacting the rest of the paper 
\end{comment}

\paragraph{Hypothesis on the data generation process} 

The feature creation process of the Numerai dataset is proprietary and therefore there is no official documentation on interpreting what the features would be. In the following, a hypothesis is proposed to explain the correlation structure between the features identified above. 

Set A consists of 1445 features representing 289 firm-specific characteristics calculated with 5 different time-series smoothing techniques. Set B consists of 141 smart-beta factors which represent market and industry-wide effects, see \cite{JensenKellyPedersen2022} for an example of smart-beta features in the global stock market.

The data generation process is closely related to the investment aim of Numerai (and other Long/Short Equity Market Neutral hedge funds). The following table \ref{table:hedgefunds} provides a non-exhaustive overview of different sources of return for market participants in the stock market. 
...
For signature transforms and random Fourier transforms, the ridge regression coefficient is fixed to be 0.01 and 1.0 and the last 200 point-in-time observations in the time-series is used to fit the ridge regression model. Model complexity, which is defined as the ratio of the number of time-series features computed to the number of data points in the time-series is varied between 0.1 and 10 to obtain models from under-parameterised regime to over-parameterised regime. 

For transformer model, the last 200 point-in-time observations in the time-series is used for model training. All the default parameters are used for model training except the number of encoder and decoder layers are varied between 2-6.  

Table \ref{table:stats}, \ref{table:signatures}, \ref{table:fourier}, \ref{table:transformer} reported the model performances in the evaluation period for statistical rules, signatures transforms, random Fourier transforms and transformer models.  Models with features from Random Fourier transform had the worst performance compared to other factor timing models. Statistical rule based models performed the best. Models with features from Signature transforms performed better than transformer models with different number of layers, suggesting deep learning methods are not strictly necessary for time-series modelling. 

For both signature and random Fourier transforms models, models with a high complexity had a poorer performance than simpler models. Increasing the number of features does not always improve model performances. For models with a high complexity, the ridge regression coefficient needs to be set to the larger value (1.0) otherwise model performances would significantly drop. 

For transformer models, increasing model complexity by adding more layers does not always improve model performances. The optimal model performance is achieved when the number of encoder and decoder layers are set to 4. 
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|l|l|l|l|}
    \hline
        ~ & Mean Corr & Sharpe & Calmar \\ \hline
        Cumulative Sharpe & 0.0105  & 0.3932 & 0.0388 \\ \hline
        Cumulative Mean & 0.0109  & 0.3925 & 0.0388 \\ \hline
        Max Drawdown & 0.0085  & 0.3088 & 0.0311 \\ \hline
    \end{tabular}
    \caption{Statistical rule based models}
    \label{table:stats}
\end{table}
...
\begin{table}[!ht]
    ...
    \hline
        Model Complexity & Alpha & Mean Corr & Sharpe & Calmar\\ \hline
        0.1  & 0.01  & 0.0057  & 0.2056  & 0.0081 \\ \hline
        0.1  & 1.00  & 0.0059  & 0.2152  & 0.0083 \\ \hline
        0.5  & 0.01  & 0.0072  & 0.2714  & 0.0163 \\ \hline
        0.5  & 1.00  & 0.0059  & 0.2156  & 0.0083 \\ \hline
        1.0  & 0.01  & 0.0059  & 0.2109  & 0.0109 \\ \hline
        1.0  & 1.00  & 0.0059  & 0.2161  & 0.0083 \\ \hline
        5.0  & 0.01  & 0.0041  & 0.1467  & 0.0077 \\ \hline
        5.0  & 1.00  & 0.0061  & 0.2203  & 0.0087 \\ \hline
        10.0  & 0.01  & 0.0028  & 0.0961  & 0.0041 \\ \hline
        10.0  & 1.00  & 0.0062  & 0.2264  & 0.0094 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    ...
    \hline
        Model Complexity & Alpha & Mean Corr & Sharpe & Calmar\\ \hline
        0.1  & 0.01  & 0.0024  & 0.0994  & 0.0033 \\ \hline
        0.1  & 1.00  & 0.0042  & 0.1657  & 0.0055 \\ \hline
        0.5  & 0.01  & 0.0045  & 0.1692  & 0.0089 \\ \hline
        0.5  & 1.00  & 0.0047  & 0.1707  & 0.0072 \\ \hline
        1.0  & 0.01  & 0.0046  & 0.1711  & 0.0160 \\ \hline
        1.0  & 1.00  & 0.0060  & 0.2381  & 0.0165 \\ \hline
        5.0  & 0.01  & 0.0017  & 0.0692  & 0.0099 \\ \hline
        5.0  & 1.00  & 0.0010  & 0.0371  & 0.0022 \\ \hline
        10.0  & 0.01  & -0.0002  & -0.0090  & -0.0007 \\ \hline
        10.0  & 1.00  & 0.0035  & 0.1266  & 0.0093 \\ \hline
    \end{tabular}
    \caption{Fourier transform models}
    \label{table:fourier}
\end{table}
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|l|l|l|l|}
    \hline
        Layers & Mean Corr & Sharpe & Calmar \\ \hline
        2  & 0.0015  & 0.0641  & 0.0036 \\ \hline
        3  & 0.0058  & 0.2721  & 0.0199 \\ \hline
        4  & 0.0064  & 0.2949  & 0.0152 \\ \hline
        5  & 0.0035  & 0.1606  & 0.0064 \\ \hline
        6  & 0.0030  & 0.1461  & 0.0072 \\ \hline
    \end{tabular}
...
RMSE, the standard loss function for regression problems is used to train the XGBoost models in the grid search. Early stopping based on \textbf{Pearson} correlation in the validation set is applied to control the model complexity. 
...
The other hyper-parameters of the XGBoost models are fixed as follows in the grid search. 
...
%% Comparison of XGBoost performances over different parameers 
The performances of the 9 models in the grid search are grouped based on each criteria one at a time to calculate the marginal effects of a parameter. Table \ref{table:XGB1} compares performances of XGBoost models by different data subsample ratios and feature subsample ratios \textbf{without} feature neutralisation. 

Setting data sub-sampling to be $25\%$ significantly drops model performances. For models with $75\%$ data-sub-sampling, model performances with are less varied than models with $50\%$ data-sub-sampling. 

For the study of incremental learning in the next section, the data sub-sampling is set to $75\%$ and feature sub-sampling is set to $50\%$. Selection is based on \textbf{both} performances and robustness, namely the performance of models with perturbed hyper-parameters. Models trained with $75\%$ data sub-sampling and $50\%$ feature sub-sampling has the highest Calmar ratio, the second-highest Sharpe ratio and is also robust towards small changes in feature sub-sampling. 
...
\begin{table}[!ht]
    ...
    \hline
        ...
        0.75  & 0.75  & 0.0256  & 1.2778  & 0.6291 \\ \hline
        0.75  & 0.50  & 0.0254  & 1.3485  & 0.8276 \\ \hline
        0.75  & 0.25  & 0.0258  & 1.3244  & 0.5936 \\ \hline
        0.50  & 0.75  & 0.0233  & 1.2014  & 0.3148 \\ \hline
        0.50  & 0.50  & 0.0252  & 1.3646  & 0.6531 \\ \hline
        0.50  & 0.25  & 0.0265  & 1.3033  & 0.4010 \\ \hline
        0.25  & 0.75  & 0.0215  & 1.1593  & 0.2596 \\ \hline
        0.25  & 0.50  & 0.0216  & 1.0299  & 0.1330 \\ \hline
        0.25  & 0.25  & 0.0221  & 1.2158  & 0.4749 \\ \hline
    \end{tabular}
    \label{table:XGB1}
\end{table}
...
\begin{itemize}
    ...
    \item Number of Encoding Layers: 0,1,2
...
In Table \ref{table:MLP1} shows the performances of neural network models \textbf{without} feature neutralisation. When the number of funnel layers equals to one, the MLP model is equivalent to a (regularised) linear model and have the worst performances among other MLP models. The best performing model is the one without feature engineering layers and a standard MLP model with 2 linear layers. Model with and without feature engineering layers performed similarly. However, the most complex model considered (2 feature engineering layers and 3 Funnel layers) performed the second best in the grid search. 
...
\begin{table}[!ht]
    ...
    \hline
        Num Feature Eng Layers  & Num Funnel Layers  & Mean Corr & Sharpe  & Calmar \\ \hline
        0  & 1  & 0.0169  & 0.8322  & 0.0824 \\ \hline
        0  & 2  & 0.0236  & 1.1467  & 0.2677 \\ \hline
        0  & 3  & 0.0211  & 0.9713  & 0.2332 \\ \hline
        1  & 1  & 0.0218  & 1.0188  & 0.1943 \\ \hline
        1  & 2  & 0.0223  & 1.0365  & 0.2125 \\ \hline
        1  & 3  & 0.0216  & 1.0329  & 0.2292 \\ \hline
        2  & 1  & 0.0216  & 1.0134  & 0.1935 \\ \hline
        2  & 2  & 0.0225  & 1.0766  & 0.2367 \\ \hline
        2  & 3  & 0.0227  & 1.0510  & 0.2569 \\ \hline
    \end{tabular}
    \label{table:MLP1}
\end{table}


A different grid search is performed to study whether using \textbf{deep} neural networks can improve model performance. Other hyper-parameters of the neural network models are fixed as above. 

\begin{itemize}
    \item Degree of Feature Neutralisation: 0.0
    \item Number of Encoding Layers: 4,6,8
    \item Number of Funnel Layers: 0,2,4
\end{itemize}

In Table \ref{table:MLP2} shows the performances of deep neural network models \textbf{without} feature neutralisation. Model with 4 feature engineering layers and 2 funnel layers has a higher Mean Corr the standard MLP model with 2 layers, but the improvement is less than $2.5\%$. The most complex models with 8 feature engineering layers had a poorer performances than other simpler models. 

\begin{table}[!ht]
    \centering
    \caption{Deep neural network models without feature neutralisation}
    \begin{tabular}{|l|l|l|l|l|l|}
    \hline
        Num Feature Eng Layers  & Num Funnel Layers  & Mean Corr  & Sharpe  & Calmar \\ \hline
        4  & 0  & 0.0208  & 1.0004  & 0.2218 \\ \hline
        4  & 2  & 0.0241  & 1.1112  & 0.2567 \\ \hline
        4  & 4  & 0.0225  & 1.0840  & 0.2301 \\ \hline
        6  & 0  & 0.0215  & 0.9663  & 0.1447 \\ \hline
        6  & 2  & 0.0222  & 1.0572  & 0.1949 \\ \hline
        6  & 4  & 0.0216  & 1.0495  & 0.2342 \\ \hline
        8  & 0  & 0.0172  & 0.8304  & 0.1115 \\ \hline
        8  & 2  & 0.0217  & 1.0863  & 0.2441 \\ \hline
        8  & 4  & 0.0183  & 0.9817  & 0.1993 \\ \hline
    \end{tabular}
    \label{table:MLP2}
\end{table}


There is no clear relationship between the number of feature engineering layers and model performances. Using more complex model architectures (by increasing either the number of feature engineering layers or the number of funnel layers) does not always increase model performances. For a fixed number of feature engineering layers, the optimal number of funnel layers is 2. Adding more layers deteriorates model performances. 
...
Factor timing models, which is based on learning the dynamics of weights from the ridge regression model trained on each era under-performed other tabular models such as XGBoost and MLP. In fact, the feature engineering and deep learning based models performed worse than simple rule-based models. All the factor-timing models considered above are highly correlated in both performances and predictions. 

Numerai dataset demonstrates a very strong non-stationary nature such that the linear weights of regression model trained on each era cannot be effectively used to predict future weights. In other words, \textbf{linear} relationships between features and target is not stable such that forecast cannot be made accurately using historical values. It shows the dataset should be modelled with tabular machine learning models, with regular updates on model weights, rather than using time-series methods to predict the regression parameters for each era. 

%% XGBoost models performed better than MLP 
All XGBoost models considered in the grid search performed better than neural network models with different model complexity. Increasing the complexity of neural network models does not always improve model performances. A simple MLP with 2 layers is the second-best model in the grid searches. 
...
The train size of models are fixed to be 585 as above. The retrain frequency is set to be 400,200,100,50,25, respectively. Table \ref{table:XGBInc1} lists the performances of the XGBoost models trained with the same optimised hyper-parameters between Era 801 and Era 1050.  Retraining models more frequently improves model performances, as models retrained every 25th era has the highest Mean Corr and Sharpe ratio. 

\begin{table}[!ht]
    \centering
    \caption{XGBoost Models with different retrain frequencies, train size is fixed to 585}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Retrain Frequency  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        400  & 0.0186  & 0.9044  & 0.2062 \\ \hline
        200  & 0.0192  & 0.9575  & 0.2182 \\ \hline
        100  & 0.0184  & 0.8707  & 0.1771 \\ \hline
        50  & 0.0199  & 0.9588  & 0.1913 \\ \hline
        25  & 0.0208  & 1.0460  & 0.1859 \\ \hline
    \end{tabular}
    \label{table:XGBInc1}
\end{table}
...
Table \ref{table:XGBInc2} lists the performances of the XGBoost models trained with the same optimised hyper-parameters between Era 801 and Era 1050, with the train size increased to 785. There is no simple relationship between retrain frequencies and model performances as in the above where train size is set to 585. 

\begin{table}[!ht]
    \centering
    \caption{XGBoost Models with different retrain frequencies, train size is fixed to 785}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Retrain Frequency  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        400  & 0.0202  & 0.9901  & 0.1535 \\ \hline
        200  & 0.0204  & 1.0043  & 0.1549 \\ \hline
        100  & 0.0191  & 0.9234  & 0.1448 \\ \hline
        50  & 0.0205  & 1.0022  & 0.1559 \\ \hline
        25  & 0.0204  & 0.9800  & 0.1684 \\ \hline
    \end{tabular}
    \label{table:XGBInc2}
\end{table}
...
Tables \ref{table:XGBInc385}, \ref{table:XGBInc285}, \ref{table:XGBInc185}, \ref{table:XGBInc85} list the model performances when train size is set to 385,285,185,85 with different retrain frequencies. Model performances drops as train size is decreased. Increasing the number of model retrains have limited effects on model performances. All models with train sizes less than or equal to 385 have smaller Sharpe ratios than models with train sizes 585 or 785. 

\begin{table}[!ht]
    \centering
    \caption{XGBoost Models with different retrain frequencies, train size is fixed to 385}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Retrain Frequency  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        100  & 0.0136  & 0.6588  & 0.0736 \\ \hline
        50  & 0.0137  & 0.6705  & 0.0739 \\ \hline
        25  & 0.0158  & 0.7403  & 0.1010 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    \centering
    \caption{XGBoost Models with different retrain frequencies, train size is fixed to 285}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Retrain Frequency  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        100  & 0.0122  & 0.5502  & 0.0636 \\ \hline
        50  & 0.0120  & 0.5656  & 0.0624 \\ \hline
        25  & 0.0117  & 0.6133  & 0.0822 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    \centering
    \caption{XGBoost Models with different retrain frequencies, train size is fixed to 185}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Retrain Frequency  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        50  & 0.0083  & 0.4670  & 0.0550 \\ \hline
        10  & 0.0064  & 0.3695  & 0.0232 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    \centering
    \caption{XGBoost Models with different retrain frequencies, train size is fixed to 85}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Retrain Frequency  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        50  & 0.0054  & 0.3039  & 0.0290 \\ \hline
        10  & 0.0057  & 0.2958  & 0.0281 \\ \hline
    \end{tabular}
...
The optimal incremental learning parameters depends on the distribution shifts of data. These parameters are not directly related to model design and should be tuned independently. 
...
For the Numerai dataset, the recommended train size is 585 as this is the threshold where model performances saturates (with other model hyper-parameters unchanged), the retrain frequency can be set to 25. 
...
\begin{itemize}
    ...
    \item Feature subsample by tree: 0.5
    \item Feature subsample by level/node: 1
    \item L1 regularisation: 0.001
    \item L2 regularisation: 0.001 
\end{itemize}

Model snapshots are created by running the first $10\%,20\%,\dots,100\%$ of boosting rounds during inference. Performances of model snapshots from Era 801 to Era 1050 are reported in table \ref{table:XGBSnapshot}. 

\begin{table}[!ht]
    \centering
    \caption{Model Snapshots of XGBoost Models with different number of boosting rounds}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Snapshot  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        0.1  & 0.0220  & 1.0338  & 0.1773 \\ \hline
        0.2  & 0.0226  & 1.0951  & 0.2895 \\ \hline
        0.3  & 0.0225  & 1.1105  & 0.4112 \\ \hline
        0.4  & 0.0222  & 1.1166  & 0.4932 \\ \hline
        0.5  & 0.0218  & 1.1181  & 0.3896 \\ \hline
        0.6  & 0.0212  & 1.1087  & 0.3402 \\ \hline
        0.7  & 0.0208  & 1.0949  & 0.3274 \\ \hline
        0.8  & 0.0205  & 1.0929  & 0.3205 \\ \hline
        0.9  & 0.0201  & 1.0869  & 0.2954 \\ \hline
        1.0  & 0.0198  & 1.0832  & 0.2825 \\ \hline
    \end{tabular}
    \label{table:XGBSnapshot}
\end{table}

For the Numerai dataset, increasing the model complexity of XGBoost models by using a large number of boosting rounds without early stopping does not improve model performances. The optimal Mean Corr is attained using $20\%$ of boosting rounds. The optimal Sharpe and Calmar ratio is attained using between $40\%$ and $50\%$ of boosting rounds. 



\paragraph{Deep XGBoost ensembles with different random seeds}

20 XGBoost models with different random seeds are trained with the following hyperparameters. The train size is fixed to 585 with models retrained every 25 eras. Other hyper-parameters of the XGBoost models are set as follows. 
...
A deep XGBoost model is created by combining the 20 models incrementally. For every 10th era, a shallow XGBoost model with the following hyper-parameters are trained using model predictions from the most recent 185 eras (with suitable data lag). Monotonic constraints are imposed so that we do not assign negative weights on the model predictions. 

%% Layer 2 XGBoost 
\begin{itemize}
    \item Grow policy: Depth-wise
    \item Number of boosting rounds: 100
    \item Early Stopping 5
...
In table \ref{table:XGBDeep5000}, the performance from the deep XGBoost model (Deep) is compared with the \textbf{average} performances of the 20 XGBoost model (Average) in Era 801 to Era 1050. Deep model improves model performances by more than $10\%$ while maintaining similar Sharpe and Calmar ratios to the Average model. It suggests deep incremental learning offers unique value that cannot be replaced by standard model stacking techniques such as simple averaging. Deep incremental learning allows model weighting to be allocated \textbf{dynamically} and allows model performances to be improved without taking additional risks. 

\begin{table}[!ht]
    \centering
    \caption{Deep XGBoost models with 5000 boosting rounds each}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Method  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        Average  & 0.0204  & 1.1034  & 0.2588 \\ \hline
        Deep  & 0.0227  & 1.0936  & 0.3034 \\ \hline
    \end{tabular}
    \label{table:XGBDeep5000}
\end{table}


Repeating the above analysis with the number of boosting rounds changed from 5000 to 500/50000, tables \ref{table:XGBDeep500} and \ref{table:XGBDeep50000} compare the performance of the deep XGBoost model with the average model. 
...
For small XGBoost models (500 boosting rounds), deep incremental learning significantly improves Mean Corr with some trade-offs in Sharpe and Calmar ratios. Model performances are poorer than models trained with 5000 boosting rounds as expected. However, it demonstrates deep incremental learning is a very robust model ensemble method. Even if the base learners (small XGBoost models with 500 trees) are very weak predictors on their own (and is only $10\%$ size of the original trees), deep incremental learning can combine those to recover $90\%$ performance of the average XGBoost model with 5000 trees. 

For large XGBoost models (50000 boosting rounds), deep incremental learning improves Calmar ratio with Mean Corr and Sharpe ratio similar. Model performances are better than models trained with 50000 boosting rounds as expected. It demonstrates when the base learners are already strong predictors on their own, deep incremental learning can bring limited gains in model performances. 


\begin{table}[!ht]
    \centering
    \caption{Deep XGBoost models with 500 boosting rounds each}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Method  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        Average  & 0.0122  & 0.9953  & 0.2070 \\ \hline
        Deep  & 0.0185  & 0.8752  & 0.1731 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    \centering
    \caption{Deep XGBoost models with 50000 boosting rounds each}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Method  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        Average  & 0.0223  & 1.1393  & 0.2804 \\ \hline
        Deep  & 0.0225  & 1.1582  & 0.3840 \\ \hline
    \end{tabular}
...
Model performances obtained from the optimised XGBoost Incremental Learning pipeline are compared with the Meta Model, which is an aggregation of the predictions from participants. 

There are two different ways to measure model performances. The first way is to \textbf{directly} compare the performances of predictions obtained from the incremental learning pipeline to the Meta Model. The second way is to \textbf{indirectly} compare the performance of an \textbf{ensemble} consisting of $50\%$ incremental learning pipeline and $50\%$ Meta Model to the Meta Model. The weights here are given as an example only and can be adjusted depending on the assumptions made for the prediction task. 

The first way can identify models that are good as standalone models. The second way can identify models that are good within \textbf{ensemble}. These models might not have the best performances on a standalone basis but are still important for the Meta Model as they give orthogonal(uncorrelated) predictive signals to the known ones. 

Tables \ref{table:XGBEnsemble5000}, \ref{table:XGBEnsemble500},  \ref{table:XGBEnsemble50000} compares the performance of the above deep XGBoost models with the Meta Model from Era 888 to Era 1050 \footnote{The Numerai Meta Model data starts at Era 888}. All Deep XGBoost models have a higher Mean Corr than the Meta Model but a lower Calmar ratio. The equal weighted ensembles have a better performance in all metrics (Mean Corr, Sharpe and Calmar). The deep XGBoost models are both good models on a \textbf{standalone} and \textbf{ensemble} basis. 

Deep XGBoost models with 5000 and 50000 boosting rounds have similar performances on both standalone and ensemble basis. It suggests it is not necessary to train very large \textbf{single} XGBoost models to achieve good prediction performances. Deep XGBoost models with 500 boosting rounds have a lower Sharpe and Calmar ratio than the Meta Model but can still improve all metrics in the ensemble model. Small XGBoost models are still useful in the competition. 


\begin{table}[!ht]
    \centering
    \caption{Deep XGBoost models ensemble (5000 boosting rounds)}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Method  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        Base  & 0.0235  & 1.0524  & 0.3139 \\ \hline
        Meta Model  & 0.0198  & 0.9518  & 0.3338 \\ \hline
        Ensemble  & 0.0249  & 1.0957  & 0.4258 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    \centering
    \caption{Deep XGBoost models ensemble (500 boosting rounds)}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Method  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        Base  & 0.0202  & 0.8902  & 0.2070 \\ \hline
        Meta Model  & 0.0198  & 0.9518  & 0.3338 \\ \hline
        Ensemble  & 0.0229  & 0.9715  & 0.3262 \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    \centering
    \caption{Deep XGBoost models ensemble (50000 boosting rounds)}
    \begin{tabular}{|l|l|l|l|}
    \hline
        Method  & Mean Corr  & Sharpe  & Calmar  \\ \hline
        Base  & 0.0230  & 1.1042  & 0.3926 \\ \hline
        Meta Model  & 0.0198  & 0.9518  & 0.3338 \\ \hline
        Ensemble  & 0.0251  & 1.1355  & 0.4051 \\ \hline
    \end{tabular}
...
%% Summary
In this paper, both traditional tabular and factor-timing models are studied for the incremental learning problem on temporal tabular dataset from Numerai. Traditional tabular models, if retrained regularly can adapt to distribution shifts in data which cannot be modelled by factor-timing models effectively. 

Not all methods of increasing model complexities can improve model performances. For factor-timing models, increasing the size of random feature sets or increasing the number of layers in transformers layers are inefficient ways of data learning. For transformer models, there is no clear relationship between model complexities and performances, results from hyper-parameter optimisation are not robust and might not be useful for future predictions. For MLP models, increasing the number of layers does not always improve model performances. The optimal model complexity depends on both architecture and training process. For XGBoost models, increasing the complexity of a \textbf{single} model by disabling early stopping will result in over-fitting and a drop in model performances. However, increasing the complexity with a deep model using a multi-layer structure can increase Mean Corr without taking additional model risks. In general, increasing model complexity horizontally through different model ensemble methods such as bagging are more robust than training a single large model as the impact of random seeds can be reduced in bagging. Not all approaches of building complex models are equal and some approaches are more robust than others based on both theoretical and empirical findings. 

There is no simple rule to select whether using classic or over-parameterised machine learning models. The optimal model complexity depends on both the training process and datasets for a \textbf{static} machine learning problem. Within an incremental learning framework, training multiple models with different complexities and then apply soft hyper-parameter selection within a deep model framework is better than using cross-validation to select a \textbf{single} set of 'optimal' hyper-parameters. 

Incremental learning is an effective way to adapt to distribution shifts in data. Both the training size and model retrain/update frequency should be selected based on walk-forward prediction performances. The training size needs to be large enough to cover different data regimes in history and not too large to include old data that are no longer relevant. In general, increasing model retrain frequency can improve model performances but the requirements on computational resources also increases. Therefore, trade-offs between computational costs and the marginal gain in model performances are made for practical incremental learning systems. 

%% Further Work 
In most practical applications, \textbf{multiple} machine learning methods are used together to create an ensemble prediction. The incremental learning pipeline presented in this papers provides a comprehensive way to integrate different machine learning models in a consistent and systematic way to create point-in-time predictions. With a multi-layer structure and modularised design within each layer, the deep incremental learning pipeline can flexibly model datasets with different complexities and structures. Further work can be done by integrating different deep tabular models into the pipeline and benchmark different machine learning method under the incremental learning framework. 
...
%\subsection{Robustness of XGBoost models} 
...
Under the assumption where observations of the difference series is independent and followed a Gaussian distribution, then under the null hypothesis where the two models have the same mean performances, the cumulative performance of the delta series will follow a Brownian motion. The variance of this Brownian motion will be determined empirically from the data. 

Now consider at a fixed time-step $1 < t < T$ for a delta series with total length $T$. Under the null hypothesis the p-value of the observed value of delta series at time $t$ are calculated by comparing with a suitable Gaussian distribution. (The distribution used here can be replaced by any other valid distributions if fat-tailed behaviour is to be modelled). Doing so repeatedly at multiple time-steps will obtain \textbf{multiple} p-values determined incrementally which would then needs to be corrected by multiple test adjustments such as Bonferroni correction \cite{bonferroni1936teoria}. 
