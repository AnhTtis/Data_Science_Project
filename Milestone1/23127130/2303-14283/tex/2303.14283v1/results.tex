...
\begin{table}[t]
\caption{Point estimates for the full sequence of the Plaza1 dataset. The RMSE is presented by the mean $\pm$ standard deviation across 50 runs with different random seeds. The case `GAP. w/o reinit.' serves as an ablation study and is configured by disabling the non-Gaussian landmarks set and operations for re-initialization and sampling, which is essentially just GTSAM with the explicit regularization treatment (large-covariance priors). The RMSE in this case is computed on 29 out of 50 runs (i.e., $58\%$ recall) that were not ceased by errors from GTSAM.}
\label{table:rmse-recall}
\begin{tabularx}{\linewidth}{@{} l p{1.0cm} l l p{2.0cm} @{}}
\toprule
Metric  
& Odom. & GAPSLAM & RBPF-SOG & GAP. w/o reinit.\\ 
\midrule
RMSE (cm) & 639.6   & $\mathbf{34.4\pm 0.0}$ & $56.0\pm 5.4$ & $34.5\pm 0.0$ \\ 
Recall (\%) &- & $\mathbf{100}$     & $\mathbf{100}$        & 58  \\ 
\bottomrule
\end{tabularx}
\vspace{-5pt}
\end{table}
