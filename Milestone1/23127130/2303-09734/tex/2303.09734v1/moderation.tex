...
\usetikzlibrary{bayesnet, quotes, decorations.pathreplacing}
\usepackage{csquotes}
...
\title{Moderation in instant runoff voting}
...
\begin{abstract}
Instant runoff voting (IRV) has gained popularity in recent
years as an alternative to traditional
plurality voting. Advocates of IRV claim that one of its 
benefits relative to plurality voting is its tendency
toward {\em moderation}: that it produces more 
moderate winners than plurality and could therefore be a
useful tool for addressing polarization. However, there is little
theoretical backing for this claim, and existing evidence has focused
on simulations and case studies. In this work, we prove that IRV has a
moderating effect relative to traditional plurality voting in a specific sense,
developed in a 1-dimensional Euclidean model of voter preferences. 
Our results show that as long as
voters are symmetrically distributed and not too concentrated at the
extremes, IRV will not elect a candidate that is beyond a certain 
threshold in the tails of the distribution, 
while plurality can. For the uniform distribution, we provide an approach
for deriving the exact distributions of the plurality and IRV winner
positions, enabling further analysis. We also extend a classical
analysis of so-called stick-breaking processes
to derive the asymptotic winning plurality
vote share, which we use to prove that plurality can elect
arbitrarily extreme candidates even when there are many moderate
options.
...
horowitz2006strategy,fraenkel2006failure,horowitz2007have,
fraenkel2007immoderate}.
...
in a related voting system, two-round runoff~\cite{bordignon2016moderating}. In contrast, there has been almost no theoretical work on moderation in IRV. One interesting exception is the work of \citet{grofman2004if}, who showed that IRV is more likely than plurality
to elect the median candidate under single-peaked preferences and
four or fewer candidates---with no guarantees for five or more candidates.

There is clear value in mathematical analyses that identify general moderating tendencies.
At present, the arguments for IRV's moderating effects summarized above
...
Is it possible to identify a natural definition under which we could
isolate such a property?

\paragraph{Formalizing a moderating effect of IRV}
In this paper, we propose such definitions and use them to articulate
a natural sense in which IRV produces moderate winners in a way
...
closer to them on the line; this is often called the \textit{1-Euclidean} model. We typically assume the voters and candidates are drawn from the same distribution $F$, but some of our results hold for fixed candidate positions.
In addition to its role as one of the classical mathematical models
of voter preferences 
\cite{hotelling1929stability,downs1957economic,coombs1964theory,elkind2017structured}, where it is sometimes called the Hotelling model, 1-Euclidean preferences arise naturally from 
...
voters who are closest to them.
...
with respect to the uniform distribution, IRV has a moderating effect
...
when voters and candidates come from the uniform distribution on $[0,1]$,
...
$k$ goes to infinity. 
No such fact is true about plurality voting: for plurality, given
...
we provide a method for deriving the distribution of plurality and IRV winner positions and perform this derivation for $k=3$ candidates.
...
Starting from this point, we extend our results to consider arbitrary 
(non-uniform) 
distributions of voters and candidates.
We find that IRV continues to have a moderating effect (in the sense
of our formal definition) even for voter distributions that push probability
mass out toward the extremes of the unit interval, up to a specific
threshold beyond which the effect ceases to hold.
...
do so up until a certain level of polarization is reached.

As a final point, it is worth emphasizing two issues that are not
a focus of our work here.
First, we are not trying to characterize all possible voting systems
that have a moderating effect under our formal definition,
and if our goal were simply to design
a voting system that has a moderating effect, there are many 
additional systems not in wide use that exhibit this property (any Condorcet method, for instance, or the Coombs rule~\cite{coombs1964theory,grofman2004if}).
Our interest, instead, is in the fact that IRV and plurality are
both used extensively in practice, and that they differ in this
key property: IRV has a moderating effect and plurality does not.
Second, we do not analyze strategic choices by candidates about
where to position themselves on the unit interval
\cite{hotelling1929stability,downs1957economic},
...
to make optimal strategic positioning decisions in practice;
...
\quad
  \begin{tikzpicture}
\begin{axis}[height=5cm, width=6cm, xmin=0, xmax=1,ymin=-0.1, ymax=2, samples=500, axis y line=none,       axis x line=middle, yticklabels={..}, axis line style=thick]
  \addplot[black, thick, name path=A][domain=0.001:0.999] (x, {(x^(-0.3))*(1-x)^(-0.3)/2});
  \addplot[draw=none,name path=B] {0};     
  \addplot[fill={rgb,255:red,255; green,200; blue,200},fill opacity=1] fill between[of=A and B,soft clip={domain=0:0.25}];
  \addplot[fill={rgb,255:red,255; green,150; blue,150},fill opacity=1] fill between[of=A and B,soft clip={domain=0.25:0.35}];
  \addplot[fill={rgb,255:red,255; green,100; blue,100},fill opacity=1] fill between[of=A and B,soft clip={domain=0.35:0.625}];
...
We use $v(x_i)$ to denote this vote share for candidate $x_i$. 

We can specify the IRV rule using these vote shares $v(x_i)$.
Under IRV, the candidate $i$ with the smallest
$v(x_i)$ is eliminated and vote counts are recomputed without
...
sparser, granting more votes to any remaining moderates. Extreme
...
\begin{proof}
Suppose first there is only one candidate $x \in [1/6, 5/6]$ and all other candidates are $< 1/6$ or $> 5/6$. Suppose without loss of generality that $x \le 1/2$. The smallest vote share $x$ could have occurs when there are candidates at $1/6-\epsilon$ and $5/6+\epsilon$. In this case, $x$ gets $(x - 1/6 + \epsilon) / 2 + (5/6 + \epsilon - x)/2 = 1/3+ \epsilon$ votes. Meanwhile, the most votes any candidate $< 1/6$ could have (when $x$ is at $1/2$) is $1 / 6 - \epsilon + (1/2 - 1/6 + \epsilon) / 2 = 1/3 - \epsilon / 2$. Thus, every candidate $< 1/6$ will be eliminated before $x$. At this point, $x$ will win, since it is closer to $1/2$ than any of remaining candidates $> 5/6$ and therefore has a majority. 
...
Notice that the above argument still holds if we replace $1/6$ and $5/6$ with $c$ and $1-c$ for any $0 < c \le 1/6$: it only reduces the votes going towards candidates in $[0, c)$. Thus, if there is some candidate in $[c, 1-c]$, then the IRV winner is in $[c, 1-c]$. So, if there is no candidate in $[1/6/, 5/6]$, then let $c$ be the distance between the most moderate candidate and its closest edge. This candidate must be the IRV winner, since it is the only candidate in $[c, 1-c]$ (and $c < 1/6$). In this case, the IRV winner is the most moderate candidate as claimed. 
...
any number of candidate $k \geq 3$.
In the construction, 
there is one candidate at $1/2$, two
candidates at $c$ and $1-c$ for $c > 1/6$, one candidate at
$1-\epsilon$, and all remaining candidates in $(1-\epsilon, 1]$.
First, all candidates right of $1-\epsilon$ will be eliminated. Then,
the candidate at $1-\epsilon$ will be eliminated, since it has a
smaller vote share than the three candidates at $c, 1/2,$ and $1-c$
(if we make $\epsilon$ much smaller than $1/2-c$). At this point, the
candidate at $1/2$ has less than $2(1/2 - c)/2 = 1/2-c$ votes. Since
$c > 1/6$, this is less than $1/2-1/6 = 1/3$. Meanwhile, the
candidates at $c$ and $1-c$ have more than $c + (1/2-c)/2 = 1/4+c/2$
votes. Since $c > 1/6$, this is greater than $1/4+1/12 = 1/3$. Thus
the middle candidate is eliminated and the winner is outside of $(c,
1-c)$.
...
\end{theorem}
\begin{proof}
We show how to add candidates to the initial set $x_1, \dots, x_\kappa$ so that $x_1$ becomes the plurality winner (as long as $x_1 \notin \{0, 1\}$). First, add candidates at $x_0=0$ and $x_{\kappa+1} = 1$ to guarantee that $x_1$ is between two candidates. Let $x_\ell$ be the candidate to the left of $x_1$ and let $x_r$ be the candidate to the right of $x_1$. Let $v_\ell = (x_1 - x_\ell)/ 2$ be the number of votes $x_1$ gets on its left and let $v_r = (x_r - x_1)/2$ be the number of votes $x_1$ gets on its right. Add new candidates spaced by $\frac{1}{2} \min\{v_\ell, v_r\}$ in the intervals $[0, x_\ell]$ and $[x_r, 1]$. This causes every candidate in the intervals $[0, x_\ell)$ and $(x_r, 1]$ to have strictly less than $\frac{1}{2}\min\{v_\ell, v_r\}$ votes (whether they are part of the original $\kappa$ or new). Additionally, $x_\ell$ and $x_r$ have at most $\frac{1}{2} \min\{v_\ell, v_r\} + \max\{v_\ell, v_r\}$ votes. Meanwhile, $x_1$ has $v_\ell + v_r$ votes, so $x_1$ is the plurality winner in the new configuration.\end{proof}
...
\rightarrow \infty$. The plurality vote setting is slightly different,
since candidates get votes from half of the gap to their left plus
...
number of candidates grows large, the winning vote count $V_k$ with $k
...
candidate between these gaps gets votes from half of each gap (more
...
impossible to satisfy. In this hyper-polarized
...
then if there is at least one candidate in $[c, 1-c]$, the IRV winner must be in $[c, 1-c]$.
 \end{theorem}
\begin{proof}
  Suppose there is at least one candidate in $[0, c)$, at least one candidate in $(1-c, 1]$, and exactly one candidate $x$ in $[c, 1-c]$ (if there no candidates in the left or right extremes, then $x$ immediately wins by majority). Assume without loss of generality that $x \le 1/2$. Candidate $x$'s votes are minimized when there are candidates at $c-\epsilon$ and $1-c + \epsilon$. The number of votes $x$ has is then 
\begin{align*}
...
If Condition~(\ref{ineq:1/3-cond}) is satisfied, then
  ...
  \end{align*}
  Thus $x$ cannot be eliminated next, since there is a candidate with fewer votes than $x$. The IRV winner must therefore be in $[c, 1-c]$ by the same argument as in \Cref{thm:1/6}.
\end{proof}
...
\begin{theorem}\label{thm:polarized-voters-exclusion}
  (Polarized voter distribution.) Let $f$ be symmetric over $[0, 1]$ and non-increasing over $[0, 1/2]$. For any $c \le 2(F^{-1}(1/3) - 1/4)$, if there is a candidate in $[c, 1-c]$, then the IRV winner is in $[c, 1-c]$.
\end{theorem}



The uniform distribution is the unique distribution whose density function is both non-increasing and non-decreasing over $[0, 1/2]$. Indeed, for uniform $F(x) = x$, $1/6 = 2(F^{-1}(1/3) - 1/4) = F^{-1}(1/6)$. Note that for polarized voter distributions, if $F^{-1}(1/3) < 1/4$ (i.e., more than 1/3 over voters are left of 1/4), then \Cref{thm:polarized-voters-exclusion} vacuously says that the IRV winner will be in $[0, 1]$.  In such hyper-polarized cases, we can prove that IRV cannot elect moderates if both extremes are represented. 
...
\begin{figure}[t]\centering
  ...
  \caption{IRV (top) and plurality (bottom) winner positions with Beta$(\alpha, \alpha)$-distributed voters and candidates. The violin plots show empirical distributions from 100,000 simulation trials with $k=30$ candidates at each $\alpha$ value, with whiskers marking extrema. The dashed lines for $\alpha \ge 1$ show the bound from \Cref{thm:moderate-voters-exclusion}. The dashed lines for $1/2 \le \alpha\le 1$ show the bound from \Cref{thm:polarized-voters-exclusion}. Finally, the dashed lines for $0 \le \alpha \le 1/2$ show the bound from \Cref{thm:very-polarized-voters-exclusion}. As long as voters are not too polarized, IRV prevents extreme candidates from winning. Plurality, on the other hand, allows arbitrarily extreme candidates to win for $\alpha = 1$. }\label{fig:beta-sweep}
\end{figure}

\begin{theorem}\label{thm:very-polarized-voters-exclusion}
  (Hyper-polarized voter distribution.) Let $f$ be symmetric over $[0, 1]$. For any $c \ge 2 F^{-1}(1/3)$, if there is at least one candidate in $[0, c]$ and at least one candidate in $[1-c, 1]$, then the IRV winner must be in $[0, c]$ or $[1-c, 1]$. 
\end{theorem}

Note that \Cref{thm:very-polarized-voters-exclusion} is only nontrivial when $F^{-1}(1/3) < 1/4$; otherwise, the claim vacuously states that the IRV winner will be in $[0, 1]$. 
...
strategic analyses (e.g., of Nash equilibria), and were instead
motivated by bounded rationality and a need to better understand the
...
selecting the Condorcet winner whenever one exists (this holds for the
Copeland, Black, and Dodgson
methods~\cite{black1958theory,richelson1975comparative}); it is also true for
...
Dodgson's method is NP-hard to compute and the Coombs rule is very
...
\section{Quotes about IRV Moderation}
...
Consider the asymptotic CDF of the leftmost candidate's vote count scaled by $n \ge 1$:
\begin{align*}
...
Thus, the asymptotic distributions of $n$ times the vote counts equal the distributions of the corresponding sums of exponentials RVs. This is the same idea used in proving the asymptotic distribution of the maximum gap size~\cite{holst1980lengths}. Now consider the distribution of the maximum vote count. Let $V_k$ be the maximum vote count with $k\ge 3$ candidates (and therefore $n=k+1$ gaps between candidates) and let $M_k$ be the maximum corresponding exponential RV sum. As above, by LLN, $\lim_{n\rightarrow\infty}\Pr(nV_k \le z) = \Pr(M_k \le z)$. Let $L_n = X_1 + X_2 / 2$, $C_{i} = (X_i + X_{i+1})/2$, and $R_n = X_{n-1}/ 2 + X_n$.
...
\begin{align*}
  ...
  &=\lim_{n\rightarrow \infty} (n-4)\log\left(1-e^{-2x}n^{-2}\log^{-2}n - 2e^{-x} n^{-1}\log^{-1}n (\log n + \log \log n + x)\right)\\
  &=\lim_{n\rightarrow \infty} -(n-4)^2 \frac{d}{dn}\log\left(1-e^{-2x}n^{-2}\log^{-2}n - 2e^{-x} n^{-1}\log^{-1}n (\log n + \log \log n + x)\right)\\
  &= \lim_{n\rightarrow \infty} -(n-4)^2 \frac{2e^{x}n\log n (\log n + 1) (\log n + \log \log n + x -1) + 2\log n + 2}{ e^{x}n^2\log^2 n(e^{x}n\log n - 2\log n - 2 \log \log n -2x) - n \log n}\\
    &= \lim_{n\rightarrow \infty} \frac{-2e^{x}n^3\log^3 n - O(n^3\log^2 n)}{e^{2x} n^3 \log^3n - O(n^2\log^3 n)}\\
    &= -2e^{-x}.
\end{align*}
Plugging $(\log n + \log \log n+ x) / 2 $ into the second term:
\begin{align*}
  ...
  &= \lim_{n\rightarrow \infty}  \frac{n^2\log^2 n + O(n^2 \log n \log \log n)}{-e^{x} n^2\log n + O(n \log^2 n)}\\
  &= -e^{-x},
\end{align*}
...
\end{align*}
We will establish a lower bound on the right hand side for a carefully chosen $x$. We can add additional events to this intersection and only lower the probability. Let $E_k$ be the event that the largest vote count in $[\ell, r]$ does not belong to either the leftmost or rightmost candidate in the interval and that the largest vote count outside of $[\ell, r]$ does not belong to the two candidates bordering the interval. Additionally, let $N_k$ be the number of candidates in the interval $[\ell, r]$. 
...
\begin{proof}[Proof of \Cref{thm:moderate-voters-exclusion}]
  Begin the same way as in the proof of \Cref{thm:general-exclusion}, minimizing $x$'s vote counts with candidates at $c- \epsilon$ and $1-c + \epsilon$. We thus have
\begin{align*}
v(x) = F\left(\frac{x + 1-c + \epsilon}{2}\right) - F\left(\frac{c - \epsilon + x}{2}\right).
  \end{align*}
  
  If $f$ is monotonic and non-decreasing over $[0, 1/2]$, then $x$ has the smallest vote share when $x=c$. At this edge of the interval, $x$'s vote count is at least
...
\begin{proof}[Proof of \Cref{thm:polarized-voters-exclusion}]
 As in \Cref{thm:general-exclusion}, minimize $x$'s vote counts with candidates at $c- \epsilon$ and $1-c + \epsilon$. If $f$ is monotonic and non-increasing over $[0, 1/2]$, then $x$ has the smallest vote share when $x=1/2$: 
...
\begin{proof}[Proof of \Cref{thm:very-polarized-voters-exclusion}]
 Suppose there is exactly one candidate $\ell \in [0, c]$ and at least one candidate each in $(c, 1-c)$ and $[c, 1]$ (if there are no candidates in $(c, 1-c)$, the claim is vacuously true). The smallest vote share $\ell$ could have occurs when $\ell = 0$ and there is a candidate at $c + \epsilon$. In this case, $\ell$'s vote count is
\begin{align*}
...
\begin{proof}[Proof of \Cref{thm:no-exclusion}]
 ...
 Now we show how to add candidates to the initial set $x_1, \dots, x_\kappa$ so that $x_1$ becomes the plurality winner if $x_1 \notin \{0, 1\}$. Since $f$ is continuous and $f(x_1) > 0$, there must exist some $\delta > 0$ such that $|f(x_1) - f(x_1+t)|  < f(x_1)/ 4$ for $t \in [-\delta, \delta]$. This argument still holds if we make $\delta$ smaller, so we ensure than $\delta < \min\{x_1, 1-x_1\}$ (both are positive since $x_1 \notin\{0, 1\}$). Now add candidates $\ell = x_1 - \delta$ and $r = x_1 + \delta$. Then the number of votes $\ell$ has on its right is less than $\frac{5}{8}\delta f(x_1)$:
   ...
   \end{align*}
   The same argument shows the number of votes $r$ has on its left is less than $\frac{5}{8}\delta f(x_1)$. Meanwhile, $x_1$ has more than $\frac{3}{4}\delta f(x_1)$ votes:
   \begin{align*}
   ...
   \end{align*}
   Thus $x_1$ has more votes than $\ell$ gets on its right and than $r$ gets on its left. Now, we repeatedly add candidates to the left of $\ell$ adjacent to to the candidates with the maximum vote counts in $[0, \ell]$. We can make the maximum vote count in $[0, \ell]$ arbitrarily small (except $\ell$'s) by adding enough candidates in this way---in particular, we can make it smaller than $v(x_1)$. We can also make $\ell$'s total vote total smaller than $v(x_1)$, since it gets strictly fewer than $v(x_1)$ votes on its right. Doing the same in $[r, 1]$ then ensures $x_1$ is the plurality winner.
 \end{proof}
...
\begin{enumerate}
  ...
  \begin{enumerate}
  \item Candidate 2 is eliminated first. Since 2 has fewer votes than the winner,
 \begin{align*}
  ...
  Since 2 has fewer votes than 3,
  \begin{align*}
...
\end{center}
  ...
  \item Candidate 3 is eliminated first. Since candidate 3 has fewer votes than candidate 2:
  \begin{align*}
    &(x_3 - x_2) / 2 + 1-x_3 < (x_3 - x_2) / 2 + (x_2 - w) / 2\\
    \Leftrightarrow \quad &  1-x_3 <  (x_2 - w) / 2\\
    \Leftrightarrow \quad &  x_3 > 1-  (x_2 - w) / 2
  \end{align*} 
  Since candidate 3 has fewer votes than the winner:
    \begin{align*}
...
\begin{enumerate}
\item Candidate 2 is eliminated first.  Since candidate 2 has fewer votes than the winner,
  \begin{align*}
  ...
  Since candidate 2 has fewer votes than candidate 3,
    \begin{align*}
...
\item Candidate 3 is eliminated first. Since candidate 3 has fewer votes than the winner, 
\begin{align*}
...
\end{align*}
which is always true. Since candidate 3 has fewer votes than candidate 2,
\begin{align*}
