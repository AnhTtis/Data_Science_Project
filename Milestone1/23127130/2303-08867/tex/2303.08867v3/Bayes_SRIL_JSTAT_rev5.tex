...
For long times, when the sequence of orders is over, the price reverts back towards its original value. The law of impact decay, e.g. whether a meta-order leaves a permanent impact or not, appear to have a lower degree of universality~\cite{farmer2013efficiency,brokmann2015slow,bucci2018slow,zarinelli2015beyond}, also due to the scarcity of data for long time-scales. 
...
The market maker does not know $G$ but he observes the past values $F_{<t}=(f_{t-1},\ldots,F_0)$ of $F_t$. At time $t$, he fixes the (ask) price $a_t$ at which he will sell and the (bid) price $b_t$ at which he will buy, in a competitive manner\footnote{Traders will be attracted by other market makers if the market maker's expected gain is positive and, if it is negative, he will be driven out of the market.} based on his expectation of the value $W_t$ of the asset, given $F_{<t}$ and the observed sequences of orders $x_{<t}$. Hence he will announce the ask price ${a_{t} = \E{W_t|F_{<t},x_{<t},x_{t}=1}}$ and the bid price ${b_{t} = \E{W_t|F_{<t},x_{<t},x_{t}=-1}}$ for buy orders ($x_t=1$) and sell orders ($x_t=-1$), respectively. The realised price at time $t$ is given by
\begin{equation}
...
\end{equation}
with $\alpha>0$ a constant of order one. We remark that this setting assumes that the market maker knows both $\nu$ and $\theta$.
...
Hence, the market maker should set prices as in Eq.~(\ref{ptnu}) irrespective of whether the order flow originates from informed traders or from the execution of a meta-order. A meta-order is defined by a direction $G=\pm 1$ (buy or sell), a volume $Q$ and a time horizon $T$ to execute it which is equal to the traded volume $V_T=T$ over time $T$. The parameter $\nu=Q/V_T$, in this interpretations, becomes the frequency with which child orders are submitted, and is called participation rate in the market microstructure literature (see e.g~\cite{bucci2018slow,zarinelli2015beyond}). For the same meta-order size, $\nu$ is inversely proportional to $V_T$, hence $\nu$ is also a (inverse) measure of liquidity. 
In this interpretation of the model, the parameter $\theta$, which is the offset of the value of the asset perceived by the informed trader in the original GMM, tunes the strength of market response to the persistent order flow. 
...
The true value of $\nu$ becomes statistically detectable when $q\sim \nu^{-1}$, which is also the time when Eq.~(\ref{Epterf}) predicts that the true value of $G$ is revealed. 
...
Eqs.~(\ref{ptxi}) and~(\ref{Epterf}) assume that $\phi(v)$ has a finite limit for $v\to 0$.  Fig.~\ref{fig:SRILprior} shows that the leading behaviour in Eq.~(\ref{Epterf})  holds generally for any prior with $\phi(v)\sim v^{k-1}$ with $k>0$, as shown in Appendix~\ref{app:prior}\footnote{Zarinelli {\em et al}~\cite{zarinelli2015beyond} estimate that the distribution of the participation rate $\nu$ has a power law distribution with exponent $\kappa\simeq 0.864$.}. The square-root law indeed holds when the volume of individual orders is drawn from a distribution with finite second moment and/or in the presence of short range auto-correlations in the orders of noise traders (see Appendix~\ref{app:vol}). 
