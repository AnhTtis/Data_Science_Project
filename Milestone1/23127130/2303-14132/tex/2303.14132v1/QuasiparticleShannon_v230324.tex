...
The probabilities of finding different states in this basis are called formation probabilities \cite{Najafi:2015rma}. These probabilities, especially the empty formation probability, have been the subject of extensive research %
...
\begin{figure}[t]
  ...
  \includegraphics[height=0.25\textwidth]{FigureCC.pdf}\\
...
The aim of this paper is to investigate whether there exist universal properties for the total system Shannon entropy, subsystem Shannon entropy, and subsystem Shannon mutual information in excited states of quasiparticles, akin to those observed for entanglement entropy. In a state that is pure, the quantum correlation between a subsystem and its complement is usually described by the entanglement entropy, which refers to the von Neumann entropy of the reduced density matrix. Depending on the particular quantum state of a given many-body system, the entanglement entropy exhibits various behaviors in the scaling limit \cite{Amico:2007ag,Eisert:2008ur,Calabrese:2009bph,Laflorencie:2015eck,Witten:2018lha}.
In specific scenarios, the entanglement entropy in quasiparticle states of integrable quantum spin chains displays intriguing universal features in the scaling limit \cite{Pizorn:2012aut,Berkovits:2013mii,Molter:2014qsb,Castro-Alvaredo:2018dja,Castro-Alvaredo:2018bij,Castro-Alvaredo:2019irt,%
...
The reduced density matrix of subsystem $A$ can be denoted as $\rho_A$, and the entanglement entropy can be calculated as $S_{A,K}=-\tr_A(\rho_A\log\rho_A)$.
...
In this paper, we present analytical formulas for the total and subsystem Shannon entropy and mutual information in single- and double-quasiparticle excited states of free bosonic and fermionic chains, as well as in single- and double-magnon excited states of the ferromagnetic phase of the spin-1/2 XXX chain. In the scaling limit where $L$ and $\ell$ approach infinity with a fixed ratio between subsystem size and total system size $x={\ell}/{L}$, both the total system Shannon entropy and subsystem Shannon entropy follow a logarithmic law, while the subsystem mutual information is a finite function of the ratio $x$.
...
In a double-particle state with a large momentum difference, we derive universal formulas that apply to all three types of quantum chains but cannot be reproduced semiclassically.
...
In section~\ref{sectionXXX}, we consider magnon excited states in the spin-1/2 XXX chain. The paper concludes with discussions in section~\ref{sectionConclusion}.
...
\be
I_k^\bos(\ell) = - x \log x - (1-x) \log (1-x).
\ee
...
\begin{tabular}{|c|c|c|c|} \hline
   ...
   $|G\rag_A$   & $p_0 = 1 - x$    & -                & 1       \\ \hline
   $|j\rag_A$   & $p_j = \f{1}{L}$ & $j \in [1,\ell]$ & $\ell$  \\ \hline
\end{tabular}
  \caption{The local states and probabilities of the subsystem $A$ in the single particle state $|k\rag$ of the free bosonic chain.}
  \label{tablebosksub}
\end{table}
...
\ee
There are $\f{L(L+1)}{2}$ possible local states, and the probabilities are shown in Table~\ref{tableboskhat2tot}, which are the same as the probabilities of the configuration of two identical classical soft-core in Table~\ref{tablesoft2indtot}.
In the limit $L\to+\inf$, we get the Shannon entropy
\be
...
\be
H^\bos_{k^2}(\ell) = 2x \log L - x(2-x) \log2 -2(1-x)\log(1-x)% + O\Big(\frac{\log L}{L}\Big)
,
\ee
and mutual information
\be
I_{k^2}^\bos(\ell)=-x^2\log x^2- 2x(1-x)\log [2x(1-x)]-(1-x)^2\log (1-x)^2% + O\Big(\frac{\log L}{L}\Big)
.
\ee
...
\begin{tabular}{|c|c|c|c|} \hline
   local states    & probabilities           & ranges                       & numbers \\ \hline
   $|G\rag_A$      & $p_0 = (1 - x)^2$       & -                            & 1       \\ \hline
   $|j\rag_A$      & $p_j = \f{2(1-x)}{L}$   & $j \in [1,\ell]$             & $\ell$  \\ \hline
   $|j^2\rag_A$    & $p_{j^2} = \f{1}{L^2}$  & $j \in [1,\ell]$             & $\ell$     \\ \hline
   $|j_1j_2\rag_A$ & $p_{j_1j_2}=\f{2}{L^2}$ & $1 \leq j_1 < j_2 \leq \ell$ & $\f{\ell(\ell-1)}{2}$ \\ \hline
\end{tabular}
...
For $|k_{12}|=\f{mL}{n} \leq \f{L}2$ with the integer $n=2,3,4,\cdots$ being a divider of $L$ and the integer $m$ being coprime with $n$, we get the total system Shannon entropy
...
Interestingly, in further $n\to+\inf$ limit, the exceptional formula (\ref{Hbosk1k2Lk12eqmLn}) approaches the universal formula (\ref{Hunivk1k2L}).

We have obtained three formulas for the total system Shannon entropy, i.e.\ the exact formula (\ref{Hbosk1k2Lgeneral}) written in terms of a summation which is valid for general $L$ and $k_{12}$, the formula (\ref{Hbosk1k2Lk12llL}), i.e.\ the universal formula (\ref{Hunivk1k2L}), which is valid for $|k_{12}| \ll L$ in the scaling limit, and the exceptional formula (\ref{Hbosk1k2Lk12eqmLn}) which is valid for exceptional values of the momentum difference $|k_{12}|=\f{mL}{n} \leq \f{L}2$ with coprime integers $m,n$ in the scaling limit.
We show these results in Figure~\ref{FigureHbosk1k2L}.
In the left panel of Figure~\ref{FigureHbosk1k2L}, we see that for most values of momentum difference $|k_{12}|$, not only for $|k_{12}|\ll L$, the Shannon entropy of the total system is $(\ref{Hbosk1k2Lk12llL})$, i.e.\ the universal formula (\ref{Hunivk1k2L}).
...
In the right panel of Figure~\ref{FigureHbosk1k2L}, it is shown that the $n\to+\inf$ limit of exceptional result (\ref{Hbosk1k2Lk12eqmLn}) leads to the universal result (\ref{Hunivk1k2L}).
...
\begin{figure}[t]
  ...
  Left: The red empty diamonds connected with thin lines are the exact bosonic result (\ref{Hbosk1k2Lgeneral}) with $L=840$.
  The dark blue dashed line is the result (\ref{Hbosk1k2Lk12llL}), i.e.\ the universal (univ) formula (\ref{Hunivk1k2L}), which is valid for $|k_{12}| \ll L$.
  The other dashed lines are the exceptional result (\ref{Hbosk1k2Lk12eqmLn}) which is valid for exceptional values of the momentum difference $|k_{12}|=\f{mL}{n} \leq \f{L}{2}$ with coprime integers $m,n$.
  Right: The large $n$ limit of exceptional bosonic result (\ref{Hbosk1k2Lk12eqmLn}) leads to the universal result (\ref{Hunivk1k2L}).}
  \label{FigureHbosk1k2L}
\end{figure}

\subsubsection{Subsystem Shannon entropy}

For the subsystem $A$, there are $\f{(\ell+1)(\ell+2)}{2}$ possible local states and the probabilities are shown in Table~\ref{tablebosk1k2sub}, where we have
\bea
&& p_0 = (1 - x)^2+\f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}}, \label{bosonp0} \\
%\eea%\bea
&& p_j = \f{2(1-x)}{L} - \f{2\sin(\pi k_{12} x)\cos\f{2\pi k_{12} (j-\f{\ell+1}{2})}{L}}{L^2\sin\f{\pi k_{12}}{L}}. \label{bosonpj}
\eea
...
\ee
which is the same as neither the result for two identical classical particles $H^\cl_{1^2}(\ell)$ (\ref{Hcl1hat2ell}) nor the result for two distinguishable classical particles $H^\cl_{12}(\ell)$ (\ref{Hcl12ell}).
...
\begin{tabular}{|c|c|c|c|} \hline
   local states    & probabilities           & ranges                        & numbers \\ \hline
   $|G\rag_A$      & $p_0$ (\ref{bosonp0})   & -                             & 1       \\ \hline
   $|j\rag_A$      & $p_j$ (\ref{bosonpj})   & $j \in [1,\ell]$              & $\ell$  \\ \hline
   $|j^2\rag_A$    & $p_{j^2} = \f{2}{L^2}$  & $j \in [1,\ell]$              & $\ell$  \\ \hline
   $|j_1j_2\rag_A$ & $p_{j_1j_2} = \f{4}{L^2}\cos^2\f{\pi j_{12} k_{12}}{L}$ & $1 \leq j_1 < j_2 \leq \ell$ & $\f{\ell(\ell-1)}{2}$ \\ \hline
\end{tabular}
...
For exceptional values of the momentum difference $|k_{12}|=\f{mL}{n} \leq \f{L}{2}$ with coprime integers $m,n$ in the scaling limit $L\to+\inf$, $\ell\to+\inf$ with fixed $x={\ell}/{L}$, we get exceptional value of the subsystem Shannon entropy
...
We show the subsystem Shannon mutual information in Figure~\ref{FigureIbosk1k2ell}.

\begin{figure}[t]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[height=0.28\textwidth]{FigureIbosk1k2ell.pdf}\\
  ...
  \label{FigureIbosk1k2ell}
\end{figure}


\section{Free fermionic chain} \label{sectionFFC}

This section deals with the single-particle state $|k\rag$ and double-particle state $|k_1k_2\rag$ in the free fermionic chain. The case of the free fermionic chain resembles that of the free bosonic chain, so we will keep it brief in this section.
...
\ee
which is the same as the bosonic result $H^\bos_{k_1k_2}(L)$ (\ref{Hbosk1k2Lk12llL}) and the universal result $H^\univ_{k_1k_2}(L)$ (\ref{Hunivk1k2L}).
...
\Big\},
\eea
which are generally different from the exceptional bosonic results (\ref{EBR}).
...
\begin{figure}[t]
  ...
  Left: The red empty diamonds connected with thin lines are the exact result (\ref{Hferk1k2Lgeneral}) with $L=840$.
  The dark blue dashed line is the result (\ref{Hferk1k2Lk12llL}), i.e.\ the universal formula (\ref{Hunivk1k2L}), which is valid for $|k_{12}| \ll L$.
  The other dashed lines are the exceptional result (\ref{Hferk1k2Lk12eqmLn}) which is valid for exceptional values of the momentum difference $|k_{12}|=\f{mL}{n} \leq \f{L}{2}$ with coprime integers $m,n$.
  Right: The large $n$ limit of the exceptional result (\ref{Hferk1k2Lk12eqmLn}) leads to the universal result (\ref{Hunivk1k2L}).}
  \label{FigureHferk1k2L}
\end{figure}

\subsubsection{Subsystem Shannon entropy}

For the subsystem $A$, there are $\f{\ell(\ell+1)}{2}+1$ possible local states and the probabilities are shown in Table~\ref{tableferk1k2sub}, where we have
\bea
&& p_0 = (1 - x)^2 - \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}},\label{fermionp0} \\
%\ee
%\be
&& p_j = \f{2(1-x)}{L} + \f{2\sin(\pi k_{12} x)\cos\f{2\pi k_{12} (j-\f{\ell+1}{2})}{L}}{L^2\sin\f{\pi k_{12}}{L}}.\label{fermionpj}
\eea
For general $L$, $\ell$, and $k_{12}$, we get the exact subsystem Shannon entropy
...
\begin{tabular}{|c|c|c|c|} \hline
   local states    & probabilities           & ranges                        & numbers \\ \hline
   $|G\rag_A$      & $p_0$ (\ref{fermionp0})   & -                             & 1       \\ \hline
   $|j\rag_A$      & $p_j$ (\ref{fermionpj})   & $j \in [1,\ell]$              & $\ell$  \\ \hline
   $|j_1j_2\rag_A$ & $p_{j_1j_2} = \f{4}{L^2}\sin^2\f{\pi j_{12} k_{12}}{L}$ & $1 \leq j_1 < j_2 \leq \ell$ & $\f{\ell(\ell-1)}{2}$ \\ \hline
\end{tabular}
...
We show the subsystem mutual information in Figure~\ref{FigureIferk1k2ell}.

\begin{figure}[t]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[height=0.28\textwidth]{FigureIferk1k2ell.pdf}\\
...
We consider the spin-1/2 XXX chain
...
\ee
The low-lying eigenstates are magnon excited states in the ferromagnetic phase and can be obtained from the coordinate Bethe ansatz \cite{Gaudin:1983kpuCaux:2014uuq,Karbach:1998abi}.
...
Unlike for the free bosonic and fermionic chains, there is no natural local basis at each site for the XXX chain.
We choose the eigenstates of $\s_j^z$ as the local basis at each site in this paper.%
...
For one case II solution $|I_1I_2\rag$, there are
...
\bea
&& \hspace{-12mm} p^\II_0 = \f{1}{\cN} \Big[ (L-\ell)(L-\ell-1)
                       +\f{(L-\ell)\cos(p_{12}-\th)-(L-\ell-1)\cos\th-\cos[(L-\ell)p_{12}-\th]}{1-\cos p_{12}} \Big], \\
%\ee
%\be
&& \hspace{-12mm} p^\II_j = \f{2}{\cN} \Big[ L-\ell + \f{\sin\f{p_{12}(L-\ell)}{2}\cos[p_{12}(j-\f{L+\ell+1}{2})+\th]}{\sin\f{p_{12}}{2}} \Big],
~ j \in [1,\ell], \\
%\ee
%\be
&& \hspace{-12mm} p^\II_{j_1j_2} = \f{2}{\cN} [ 1+ \cos ( j_{12} p_{12} + \th ) ], ~ 1 \leq j_1 < j_2 \leq \ell.
\eea
...
\bea
&& H^\II_{I_1I_2}(L) = - \sum_{1 \leq j_1 < j_2 \leq L} p^\II_{j_1j_2} \log p^\II_{j_1j_2},\label{HIII1I2L} \\
&& H^\II_{I_1I_2}(\ell) = - p^\II_0 \log p^\II_0
                        - \sum_{j=1}^\ell p^\II_j \log p^\II_j
                        - \sum_{1 \leq j_1 < j_2 \leq \ell} p^\II_{j_1j_2} \log p^\II_{j_1j_2},\label{HIII1I2ell} \\
...
\ee
with $H^\univ_{k_1k_2}(L)$ (\ref{Hunivk1k2L}), $H^\univ_{k_1k_2}(\ell)$ (\ref{Hunivk1k2ell}) and $M^\univ_{k_1k_2}(\ell)$ (\ref{Iunivk1k2ell}).
...
For a case IIIa solution, there are Bethe numbers
\be
I_1=\f{I-1}{2}, ~
I_2=\f{I+1}{2},
\ee
...
For the subsystem $A$, the probabilities of the subsystem local states $|G\rag_A$, $|j\rag_A$, and $|j_1j_2\rag_A$ are, respectively,
\bea
&& p^\IIIa_0 = \f{1}{\cN} \Big[ (L-\ell) \f{\sinh[v(L-1)]}{\sinh v}
                           - \f{\sinh(v\ell)\sinh[v(L-\ell)]}{\sinh^2v}
                           - (L-\ell)(L-\ell-1) \Big], \\
&& p^\IIIa_j = \f{2}{\cN} \Big[ \f{\sinh[v(L-\ell)]\cosh[2v(j-\f{\ell+1}{2})]}{\sinh v}
                           - (L-\ell) \Big], ~
            j \in [1,\ell], \\
&& p^\IIIa_{j_1j_2} = \f{4}{\cN} \sinh^2 \Big[ v \Big( j_{12} + \f{L}{2} \Big) \Big], ~ 1 \leq j_1 < j_2 \leq \ell.
\eea
...
\bea
&& H^\IIIa_{I_1I_2}(L) = - \sum_{1 \leq j_1 < j_2 \leq L} p^\IIIa_{j_1j_2} \log p^\IIIa_{j_1j_2}, \label{HIIIaI1I2L} \\
&& H^\IIIa_{I_1I_2}(\ell) = - p^\IIIa_0 \log p^\IIIa_0
                        - \sum_{j=1}^\ell p^\IIIa_j \log p^\IIIa_j
                        - \sum_{1 \leq j_1 < j_2 \leq \ell} p^\IIIa_{j_1j_2} \log p^\IIIa_{j_1j_2}, \label{HIIIaI1I2ell} \\
&& M^\IIIa_{I_1I_2}(\ell) = H^\IIIa_{I_1I_2}(\ell) + H^\IIIa_{I_1I_2} (L-\ell) -H^\IIIa_{I_1I_2}(L). \label{IIIIaI1I2ell}
\eea

For finite $v$, we have in the limit $L\to+\inf$
\be
...
\eea
We obtain the tightly bound limit of the Shannon entropies and mutual information
\bea
...
For a case IIIb solution, there are Bethe numbers
\be
I_1=I_2=\f{I}{2},
\ee
with the total Bethe number
...
For the subsystem $A$, the probabilities of the subsystem local states $|G\rag_A$, $|j\rag_A$, and $|j_1j_2\rag_A$ are, respectively,
\bea
&& p^\IIIb_0 = \f{1}{\cN} \Big[ (L-\ell) \f{\sinh[v(L-1)]}{\sinh v}
                           - \f{\sinh(v\ell)\sinh[v(L-\ell)]}{\sinh^2v}
                           + (L-\ell)(L-\ell-1) \Big], \\
%\ee
%\be
&& p^\IIIb_j = \f{2}{\cN} \Big[ \f{\sinh[v(L-\ell)]\cosh[2v(j-\f{\ell+1}{2})]}{\sinh v}
                           + (L-\ell) \Big], ~
            j \in [1,\ell], \\
%\ee
%\be
&& p^\IIIb_{j_1j_2} = \f{4}{\cN} \cosh^2 \Big[ v \Big( j_{12} + \f{L}{2} \Big) \Big], ~ 1 \leq j_1 < j_2 \leq \ell.
\eea
...
%\ee
%\be
&& H^\IIIb_{I_1I_2}(\ell) = - p^\IIIb_0 \log p^\IIIb_0
                        - \sum_{j=1}^\ell p^\IIIb_j \log p^\IIIb_j
                        - \sum_{1 \leq j_1 < j_2 \leq \ell} p^\IIIb_{j_1j_2} \log p^\IIIb_{j_1j_2}, \label{HIIIbI1I2ell}\\
%\ee
%\be
...
Additionally, we found that even when two quasiparticles have a large momentum difference, their contributions to the Shannon entropy do not decouple, unlike the entanglement entropy.
Furthermore, we discovered universal formulas for the total system Shannon entropy, subsystem Shannon entropy, and mutual information in the limit of large momentum differences, which are valid for free bosonic and fermionic chains, as well as the XXX chain, but differ from any results obtained from classical particles.
In other words, we did not observe any semiclassical quasiparticle picture that could reproduce the universal results of the Shannon entropy in quantum spin chains.

The existence of universal formulas in various quantum chains that differ from any classical results is a puzzle. It has been found that the semiclassical quasiparticle picture explains the existence of universal entanglement entropy in quantum chains \cite{Castro-Alvaredo:2018dja,Castro-Alvaredo:2018bij}. However, no such explanation exists for the local basis Shannon entropy.
In the main text of the paper, we have calculated the Shannon entropy using a particular basis of local states. However, it is essential to note that the definition of the Shannon entropy is dependent on the chosen basis. If one selects the eigenstates of the density matrix as the basis, the Shannon entropy will become equivalent to the von Neumann entropy for both pure and mixed quantum states of the total system or its subsystem.
For quantum spin chains, the eigenstates of the subsystem particle number operator are also a natural basis.
...
In summary, we have found Shannon entropy in local bases for which no semiclassical quasiparticle picture applies.
...
We anticipate that the semiclassical picture applies to course-grained properties but not to fine-grained properties.
...
In this paper, our primary focus has been on one- and double-particle states. However, we believe that exploring states with multiple quasiparticles would be a worthwhile endeavor. We anticipate that the properties described in our study will still hold true for such states.
...
For example, by measuring the subsystem Shannon entropy in a double-particle state with a small momentum difference, one could tell whether the excitations are bosonic or fermionic.
...
%We first consider soft-core classical particles, for which there is no limit for the number of particles at one site, i.e.\ that at one site there may be zero, one, or more particles.
...
\begin{tabular}{|c|c|c|c|} \hline
   ...
   no particle in $A$              & $p_0 = 1 - x$    & -           & 1       \\ \hline
   one particle in $A$ at $j$ & $p_j = \f{1}{L}$ & $j \in [1,\ell]$ & $\ell$  \\ \hline
\end{tabular}
...
\begin{tabular}{|c|c|c|c|} \hline
   microscopic configurations                           & probabilities           & ranges                       & numbers \\ \hline
   no particle in $A$                                   & $p_0 = (1 - x)^2$       & -                            & 1       \\ \hline
   only one particle in $A$ at $j$                           & $p_j = \f{2(1-x)}{L}$   & $j \in [1,\ell]$             & $\ell$  \\ \hline
   both in $A$ at $j$                                   & $p_{j^2} = \f{1}{L^2}$  & $j \in [1,\ell]$             & $\ell$  \\ \hline
   both in $A$ with one at $j_1$ and the other at $j_2$ & $p_{j_1j_2}=\f{2}{L^2}$ & $1 \leq j_1 < j_2 \leq \ell$ & $\f{\ell(\ell-1)}{2}$ \\ \hline
\end{tabular}
...
\begin{tabular}{|c|c|c|c|} \hline
   microscopic configurations                   & probabilities                 & ranges                                   & numbers \\ \hline
   no particle in $A$                           & $p_0 = (1 - x)^2$             & -                                        & 1       \\ \hline
   only red particle in $A$ at $j$                   & $p_j^{\rm red} = \f{1-x}{L}$  & $j \in [1,\ell]$                         & $\ell$  \\ \hline
   only blue particle in $A$ at $j$                  & $p_j^{\rm blue} = \f{1-x}{L}$ & $j \in [1,\ell]$                         & $\ell$  \\ \hline
   both in $A$ at $j$                           & $p_{j^2} = \f{1}{L^2}$        & $j =1,2,\cdots,L$                        & $\ell$  \\ \hline
   both in $A$ with red at $j_1$ and blue at $j_2$ & $p_{j_1j_2}=\f{1}{L^2}$       & $j_1,j_2\in[1,\ell]$, $j_1 \neq j_2$ & ${\ell(\ell-1)}$ \\ \hline
\end{tabular}
...
\begin{tabular}{|c|c|c|c|} \hline
   microscopic configurations       & probabilities                          & ranges                      & numbers \\ \hline
   no particle in $A$               & $p_0 = \f{(L-\ell)(L-\ell-1)}{L(L-1)}$ & -                           & 1       \\ \hline
   only one particle in $A$ at $j$       & $p_j = \f{2(L-\ell)}{L(L-1)}$          & $j \in [1,\ell]$            & $\ell$  \\ \hline
   both in $A$ with at $j_1$, $j_2$ & $p_{j_1j_2}=\f{2}{L(L-1)}$             & $1\leq j_1 < j_2 \leq \ell$ & $\f{\ell(\ell-1)}{2}$ \\ \hline
\end{tabular}
\end{center}
...
\begin{tabular}{|c|c|c|c|} \hline
   microscopic configurations                   & probabilities           & ranges                                   & numbers \\ \hline
   no particle in $A$                           & $p_0 = \f{(L-\ell)(L-\ell-1)}{L(L-1)}$ & -                                        & 1       \\ \hline
  only red particle in $A$ at $j$                   & $p_j^{\rm red} = \f{L-\ell}{L(L-1)}$   & $j \in [1,\ell]$                     & $\ell$  \\ \hline
  only blue particle in $A$ at $j$                  & $p_j^{\rm blue} = \f{L-\ell}{L(L-1)}$  & $j \in [1,\ell]$                     & $\ell$  \\ \hline
   both in $A$ with red at $j_1$ and blue at $j_2$ & $p_{j_1j_2}=\f{1}{L(L-1)}$ & $j_1,j_2\in[1,\ell]$, $j_1 \neq j_2$ & ${\ell(\ell-1)}$ \\ \hline
\end{tabular}
\end{center}
...
&& H^\cl_{1^{r_1}2^{r_2}\cdots s^{r_s}}(\ell) = \sum_{i=1}^s H^\cl_{1^{r_i}}(\ell), \\
...
In this section, we determine the Shannon entropy for the probability distribution of subsystem particle numbers in free bosonic and fermionic chains. The semiclassical quasiparticle picture applies to the Shannon entropy of particle number probability distribution in the large momentum difference limit.
...
In the double-particle state $|k_1k_2\rag$, the probability of finding zero, one, and two particles in the subsystem $A=[1,\ell]$ are respectively
\bea
&& p^\bos_0 = (1 - x)^2 + \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}}, \nn\\
&& p^\bos_1 = 2 \Big[ x (1 - x) - \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}} \Big], \nn\\
&& p^\bos_2 = x^2 + \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}}.
\eea
The corresponding Shannon entropy is just
\be
H^\bos_{k_1k_2}(\ell) = - p^\bos_0 \log p^\bos_0 - p^\bos_1 \log p^\bos_1 - p^\bos_2 \log p^\bos_2.
\ee
In the large momentum limit $|k_{12}| \gg 1$, the result approaches that of two classical particles
\be
...
\bea
&& p^\fer_0 = (1 - x)^2 - \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}}, \nn\\
&& p^\fer_1 = 2 \Big[ x (1 - x) + \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}} \Big], \nn\\
&& p^\fer_2 = x^2 - \f{\sin^2(\pi k_{12} x)}{L^2\sin^2\f{\pi k_{12}}{L}}.
\eea
The corresponding Shannon entropy is
\be
H^\fer_{k_1k_2}(\ell) = - p^\fer_0 \log p^\fer_0 - p^\fer_1 \log p^\fer_1 - p^\fer_2 \log p^\fer_2,
\ee
and in the large momentum limit $|k_{12}| \gg 1$ it also approaches that of two classical particles
\be
...
\begin{figure}[t]
  ...
  \caption{The $\sigma_j^x$ basis total system Shannon entropy $H^\XXX_I(L)$ (\ref{XBtotRE}) in the single-magnon state of the XXX chain.}
  \label{FigureXBtotRE}
\end{figure}
...
\ee
with the $L$-independent constant $C_{0}=C_{L/2}\approx0.730$
...
For general $I$, the evaluations of the $\sigma_j^x$ basis total system Shannon entropy and the subsystem Shannon entropy and mutual information are exponentially difficult problems, and we could only numerically calculate them for not so large $L$, say $L\leq28$.
It is easy to see that both the total system and subsystem Shannon entropies $H^\XXX_I(L)$ (\ref{XBtotRE}) and $H^\XXX_I(\ell)$ (\ref{XBsubRE}) are invariant under the changes $I \to L-I$ and $I \to \f{L}{2}-I$, and so without loss of generality we only need to consider $I$ in the range $[1,\f{L}{4}]$.
...
\ee
with $C_I$ being a constant that depends on the relative values of $I$ and $L$. We anticipate that for most values of $I$ there is the universal constant $C_I\approx0.404$, and for a few exception values of $I$ the constant $C_I$ may take some exceptional values, like the cases $I=0$ and $I=L/2$ discussed above.
...
%\bibitem{Gaudin:1983kpu}
%M.~Gaudin, \textit{La Fonction d'Onde de Bethe}.
%\newblock Masson, 1983.

\bibitem{Gaudin:1983kpuCaux:2014uuq}
