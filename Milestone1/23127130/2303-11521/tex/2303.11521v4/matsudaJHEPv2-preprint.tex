...
\abstract{We studied the Stokes phenomena of the Schwinger and the 
Unruh effects in detail. 
...
We hope that such analytical methods will help to unravel many mysteries
of physics.}
\begin{document}
...
long enough time in quantum terms, this approach seems quite artificial
 ...
 possible.\footnote{
The theme of this paper is to investigate the stationary Stokes
phenomenon, which represents stationary particle production.
The introduction of an explicit time dependency to alleviate the problem is not
considered in this paper.}

The situation is even more difficult in Hawking's original
paper\cite{Hawking:1975vcx}.\footnote{To avoid confusion, it
is important to emphasize again that the theme of this paper is ``Solving
stationary particle production by the Stokes phenomena''.
There are many papers on Hawking radiation, but I will carefully
...
gauge degree of freedom to show why the particle production is stationary.
Note that introducing an explicit time dependency alleviates this
problem.
...
particle.\footnote{Note that $k_z\ne 0$ shifts the position
of the Stokes lines. 
