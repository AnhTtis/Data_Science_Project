...
\begin{gather*}
E(R) := \frac 1 {\lvert B_{R}R^p\rvert}\int_{(B_{R}\times \R^d)\cup (\R^d\times B_{R})}c(x-y)\d\pi \\
...
-\Div \nabla c^\ast(\D\phi)=\mu-\lambda\quad& \text{ in } B_R\\
...
\leq& \int_\Omega c(x-y)\d\pi - \int_{B_R}c(\nabla ^\ast c(\D\phi))\d x - \int_\Omega \int_\sigma^\tau \langle \dot X(t)-\nabla c^\ast(\D \phi(X(t)),\D\phi(X(t))\rangle \d t\d\pi \\
...
\bibliography{OptimalTransport2}
\end{document}