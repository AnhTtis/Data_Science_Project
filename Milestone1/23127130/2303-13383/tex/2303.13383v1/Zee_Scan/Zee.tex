...
The Zee model under consideration here is capable of ameliorating this CDF anomaly~\cite{Chowdhury:2022moc,Barman:2021xeq} and we will study the relevant parameter space below.
...
%~~~~~~~~~~~[Ti]& $4.3 \times 10^{-12}$~\cite{SINDRUMII:1993gxf} & $\sim 10^{-18}$~\cite{Kuno:2006}\\ 
...
%: two of the diagonal entries of $Y$ could be zero ($Y_{ee}$ and one of the other two) and one off-diagonal entry could be zero if $M^\nu$ has another texture zero.
%With at most one vanishing off-diagonal entry of $Y$ we unavoidably have LFV through $H_2$ once we try to get $\Delta a_\mu$.
%%%%%
\item \textbf{TX-F13:} Similarly, setting $f_{e\mu}=f_{\mu\tau}=0$ eliminates $\eta^+$ LFV and generates $M^\nu_{\mu\mu}=0$. 
%$Y$ can again have two vanishing diagonal entries ($Y_{\mu\mu}$ and one other one) and one more vanishing off-diagonal entry if $M^\nu$ has a second texture zero.
%%%%%%
\item \textbf{TX-F12:} Lastly, setting $f_{e\tau}=f_{\mu\tau}=0$ eliminates $\eta^+$ LFV and generates $M^\nu_{\tau\tau}=0$. 
%$Y$ can again have two vanishing diagonal entries ($Y_{\tau\tau}$ and one other one) and one more vanishing off-diagonal entry if $M^\nu$ has a second texture zero.
\end{itemize}
...
\nonumber\\   %weird hack to keep the matrices apart
Q &\equiv \matrixx{
...
Red and pink data points are currently valid and can be probed in future experiments (defined through the last column in Tab.~\ref{tab:201}), and correspond to different neutrino mass orderings. These include the textures recently put forward in Refs.~\cite{Chowdhury:2022moc,Barman:2021xeq}.
...
%\JH{3 TeV sounds high without chiral enhancement. Or is that for the gray/red/pink points? Maybe good to check what the masses are for the blue/green points.} \AT{$Y_{\mu\mu}$ has chiral contribution as well as non chiral part. This is how one obtaines maxium of 3.3 TeV. The chiral part has $2 \log m_\mu/m_\phi$ which is factor of 10 for 100 GeV mass and factor of 20 for 3 TeV.}
...
\AT{In addition to the texture given in Eq.~\eqref{eq:muontx1} the following texture also eliminates the muonic LFV by setting $M^\nu_{\mu\mu} =0$. 
\begin{align}
\kappa Y =   \matrixx{ 0 & 0 &  -\frac{M^\nu_{\tau\tau}}{2 f_{e\tau} m_e}\\
   %%
    0 & \frac{2 f_{\mu\tau} M^\nu_{e\tau} - 2 f_{e\tau} M^\nu_{\mu\tau} + f_{e\mu} M^\nu_{\tau\tau}}{2 f_{e\tau} f_{\mu\tau} m_\mu} & 0\\
    %%
    \frac{M^\nu_{ee}}{2f_{e\tau} m_\tau} & \frac{M^\nu_{\mu\mu}}{2 f_{\mu\tau} m_\tau} &  \frac{ M^\nu_{e\tau}}{2 f_{e\tau}m_\tau}} .
    \label{eq:muontx2}
\end{align}
The texture above can be found by solving $q_j$ that give the $0$ in the Yukawa matrix of Eq.~\eqref{eq:Yuk}. Moreover, the following texture also eliminate muonic LFV by setting $M^\nu_{\mu\mu} =0$ and $M^\nu_{\tau\tau} =0$.
\begin{align}
\kappa Y =   \matrixx{ \frac{2 f_{\mu\tau} M^\nu_{e\tau} + f_{e\mu} M^\nu_{\tau\tau}}{2 f_{e\tau} f_{\mu\tau} m_e} & 0 &  0\\
   %%
    0 & - \frac{ M^\nu_{\mu\tau}}{f_{\mu\tau}m_\mu} & - \frac{ M^\nu_{\tau\tau}}{f_{\mu\tau}m_\mu} \\
    %%
    \frac{M^\nu_{ee}}{2f_{e\tau} m_\tau} & \frac{M^\nu_{\mu\mu}}{2 f_{\mu\tau} m_\tau} &  0} .
    \label{eq:muontx2}
\end{align}
Here $q_{1,3} = 0$. 
}
...
\begin{equation}
    \mathcal{H}_{eff} = \frac{G_{M\bar{M}}}{\sqrt{2}}\ [\bar{\mu} \gamma_\mu (1+\gamma_5) e] [\bar{\mu} \gamma^\mu (1-\gamma_5) e] \, .
    \label{eq:hamil}
\end{equation}
The PSI limit translates into~\cite{Conlin:2020veq,Fukuyama:2021iyw}
\begin{align}
|Y_{e\mu} Y_{\mu e}| \leq 1.17 \times 10^{-7} \left( \frac{\text{GeV}}{m_H} + \frac{\text{GeV}}{m_A} \right)^{-2} \,.
\end{align}
}
\JH{Do this more carefully. Absolute values, $m_A$ as well etc. Might be several equations. \AT{there are other two operator, but the author in \cite{Fukuyama:2021iyw} claim that they vanish in alignment limit, which I don't understand. }



Moreover, the current limit on muonium-antimuonium oscillations \cite{Willmann:1998gd} translates to the bound on the Yukawa couplings as $|Y_{e\mu} Y_{\mu e}| \lesssim 1.17 \times 10^{-7} (m_H/{\rm GeV})^2$. However, the ratio of these couplings is fixed as aforementioned. Thus one has to deviate from these textures, which unavoidably leads to LFV.
