...
% \author{Xiaobao Guo*$^1$, Nithish Muthuchamy Selvaraj*$^1$, Zitong Yu$\dag^1$, Adams Kong$^1$, \\Bingquan Shen$^2$, and Alex Kot $^1$\\  $^1$Nanyang Technological University, Singapore\\  $^2$DSO National Laboratories, Singapore \\}

% \def\thefootnote{$\ast$}\footnotetext{Equal contributors}
% \def\thefootnote{$\dag$}\footnotetext{corresponding authors}
% \address{
...
% Experimental results indicate that 1) mainstream deep models trained on DOLOS generalize better than those on the existing gameshow deception dataset under cross-dataset testings; 2) PECL improves deception detection performance over conventional approaches; and 3) multi-task learning can further improve the performance of PECL. 


%The proposed method can be applied to other deceptive conversational content, and the dataset created will encourage more researchers to tackle this challenging problem.
\end{abstract}
...
% It shocked the whole world when Elizabeth Holmes, the founder of Theranos, was exposed as a fraud. If her lies had been discerned, many big-name investors would not have lost billions of dollars~\footnote{The founder of the failed blood testing start-up Theranos, was charged with fraud by the SEC in 2018 and found guilty in 2022.}.
...
Effective deception detection has crucial implications in the area of border security, anti-fraud, business negotiations and etc.~\cite{graciarena2006combining,fornaciari2013automatic,abouelenien2016detecting,gogate2017deep,ding2019face,stathopoulos2020deception}. Deep learning algorithms have achieved comparable or even better performance than human in many complex tasks~\cite{alphafold,alphazero,go,gpt-3,clip}. One may expect AI models can also bring significant breakthroughs in deception detection. Albeit the fruitful progress in computer vision~\cite{vaswani2017attention,dosovitskiy2020vit, liu2021swin,tolstikhin2021mlp} and audio representation learning~\cite{baevski2020wav2vec,hsu2021hubert,kong2020panns}, it remains a significant challenge to efficiently explore AI ability to process multimodal information in perceiving and predicting human deceptive behaviors.


% One of the critical issues in improving the performance of AI models on deception detection is how to collect authentic yet effective deception samples in the real world.
...
% These impede the enhancement of real-world deception detection performance. 
...
% \footnote{The DOLOS dataset and the source code will be released after the paper is accepted.}. 
...
%The novelty of this architecture lies in its aaa, bbb and ccc.
...
Early works in deception detection research investigated the psychological premises of deception \cite{depaulo2003cues,hirschberg2005distinguishing,warren2009detecting,levine2014theorizing,vrij2012eliciting} and identified potential cues, such as physiological, visual, vocal, verbal and behavioral cues. 
% that people elicit when they lie and based on these cues several methodologies and datasets were proposed for deception detection. 
The pioneering work on multimodal deception detection emerged with the introduction of the Real-Life Trials (RLT) dataset~\cite{perez2015deception} collected from courtroom trials.
% , where the veracity of the statements (uttered by witnesses, accused etc.) are verified by investigations. 
The courtroom setting provides a valuable and authentic scenario of people lying, but RLT is small and its performance bottleneck was quickly reached. P{\'e}rez-Rosas~\emph{et al.}~\cite{perez2015verbal} and Kamboj~\emph{et al.}~\cite{kamboj2020multimodal} later proposed another two new datasets collected from street interviews and political speeches. However, it is difficult to verify the veracity of the responses of the subjects in these datasets. Several lab-based multimodal deception detection datasets were proposed to overcome these shortcomings. The settings of these datasets were typically in %an environment or 
situations that speakers were allowed to lie, such as describing an object~\cite{gupta2019bag},  a personality~\cite{lloyd2019miami},
% a mock crime scenario~\cite{karpova2020you} or 
and face-to-face interviews with the subjects~\cite{speth2021deception}. 
% These datasets also provide additional modalities from specialized sensors, like EEG \cite{gupta2019bag} and pulse \cite{speth2021deception}. 
% However, in these lab-based datasets, there is no incentive for the participants to lie and even monetary incentives are less effective \cite{depaulo2003cues}. Thus, low stakes scenario can result in poor deceptive cues. 
However, the incentive to deceive may be low, leading to fewer deceptive cues captured.

Reality TV shows based on truth or lie games  provide an avenue to collect examples of conversational deception. They also offer a sweet spot among incentive, veracity, and sample size requirements. Often, the participants lie to each other to win the game, and the truth is revealed at the end of the round. Large amounts of gameshow footage are available online. 
% a) In these gameshows, the participants typically play in a competing environment which increases the stakes, b) the veracity of the statements are known at the end of each round c) %these gameshows usually proceed as a series with multiple seasons and episodes allowing for data collection from a larger pool of subjects. Moreover, 
The first gameshow-based deception detection dataset was proposed in \cite{soldner2019box} and it primarily focused on two modalities: text and manually annotated visual features. However, directly applying this dataset for multimodal deception detection by using audio and visual modalities faces several challenges, which we will describe in Sec.~\ref{sec:formatting}. 
% To address this, we introduce the first large scale gameshow based multimodal deception detection dataset, especially focusing on the facial-speech modalities with the largest number of subjects. 



\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{Figures/game_example.png}
\vspace{-0.3em}
  \caption{\small{
...
Previous works on multimodal deception detection include using unimodal and multimodal features and exploring effective fusion mechanism~\cite{mathur2020introducing, karimi2018toward, karnati2021lienet, avola2019automatic, ding2019face, wu2018deception, gogate2017deep}. Gogate \emph{et al.}~\cite{gogate2017deep} proposed a deep %learning
model that %first 
incorporated the audio cues with visual and text modalities to improve the accuracy of deception prediction. 
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.7\linewidth]{Figures/New_Data_Distribution.pdf}
  \caption{\small{DOLOS dataset for multimodal deception detection.}
  }
...
% In each round, one speaker from a team would read a card containing a statement about the speaker's personal life. The statement can either be a truth or a lie, which is known only to the speaker but not by any other members. The opposing team would then ask a series of questions and the speaker has to defend the statement. The speaker tries to give convincing replies and the opposing team tries to predict the veracity of the statement solely by questioning the speaker and analyzing the speaker's responses. If the opposing team predicts the statement's veracity correctly, they win the round; otherwise, the speaker's team wins. The veracity of the statement is only known at the end of each round. The game continues for multiple rounds until all the participants have played. If the statement is true, then the speaker's responses will be deemed as truthful video clips and vice versa. 


\subsection{Data Collection and Annotation Procedure \label{DOLOS anno method}}

% The gameshow episodes are downloaded from YouTube and for each round,
...
\vspace{-0.5em}
\end{itemize}
% looks directly/almost directly into the camera – more precisely, the face must be
...
%      &  &  & \multicolumn{1}{c}{\textbf{Total}} & \multicolumn{1}{c}{\textbf{Deceptive}} & \multicolumn{1}{c}{\textbf{Truthful}} & \textbf{Deception/Truth Ratio} &  \\ \hline
...
%     Real Life Deception\cite{perez2015verbal} & _ & - & \multicolumn{1}{c}{56} & \multicolumn{1}{c}{31} & \multicolumn{1}{c}{25} & 1.24 & Street interviews \\
%     Political Deception\cite{kamboj2020multimodal} & _ & 88 & \multicolumn{1}{c}{180} & \multicolumn{1}{c}{-} & \multicolumn{1}{c}{-} & - & Political speech \\ \hline
%     Bag of Lies\cite{gupta2019bag} & _ & 35 & \multicolumn{1}{c}{325} & \multicolumn{1}{c}{162} & \multicolumn{1}{c}{163} & 0.99 & Lab \\
%     MU3D\cite{lloyd2019miami} & _ & 80 & \multicolumn{1}{c}{320} & \multicolumn{1}{c}{160} & \multicolumn{1}{c}{160} & 1 & Lab \\
%     Deception Detection and remote PPG\cite{speth2021deception} & _ & 70 & \multicolumn{1}{c}{1680} & \multicolumn{1}{c}{630} & \multicolumn{1}{c}{1050} & 0.6 & Lab \\ \hline
%     Box of Lies\cite{soldner2019box} & \checkmark & 26 & \multicolumn{1}{c}{1049} & \multicolumn{1}{c}{862} & \multicolumn{1}{c}{187} & 4.61 & Gameshows \\
%     \rowcolor{Gainsboro!60} \textbf{DOLOS (Ours)} & \checkmark & 145 & \multicolumn{1}{c}{1183} & \multicolumn{1}{c}{682} & \multicolumn{1}{c}{501} & 1.36 & Gameshows \\
%     \bottomrule
%     \end{tabular}
% }
% \caption{\textcolor{blue}{ change numbers here...}Comparison of multimodal deception detection datasets}
% \label{table: dataset comparison table}
% \end{table*}
\begin{table*}[]
\centering
...
% The BOL dataset is the closest dataset to compare with as it is also based on a gameshow. 
...
\vspace{-0.5em}
\item Compared to the Box of Lies gameshow dataset, Dolo contains a larger number of video clips and is more well-balanced in terms of the proportion of deceptive and truthful samples.
\vspace{-0.5em}
...
\vspace{-0.5em}
%We empirically show that multi-task learning can significantly improve the performance of the multimodal deception detector.
% \item Deep models trained on DOLOS are more generalizable to other deceptive content than that of Box of Lies due to its large number of samples and subjects, fine-grained manual annotations, and various conversational speech contents.
% \vspace{-0.5em}

% The video clips are distributed across a substantially large population, which prevents overfitting and improves generalization. Results on cross dataset deception detection show that our dataset generalizes better.
\end{itemize}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.7\linewidth]{Figures/PECL.png}
...
%\section{Multimodal Deception Detection}
\section{Methodology}
\vspace{0.2em}
% To examine the quality of DOLOS and to overcome the weaknesses of the previous deception detection methods in a multimodal setting, \textcolor{blue}{we propose Parameter-Efficient Crossmodal Learning (PECL) based on a pre-training and fine-tuning scheme,} which utilizes both unimodal learning and audio-visual fusion for deception detection. UT-Adapter is designed to capture both visual and audio temporal attention (Sec.~\ref{UT-adapter}). PAVF is introduced to extract complementary information between modalities (Sec.~\ref{PAVF}). In addition, we use the MUMIN features (Fig~\ref{fig:distribution} (b)) to perform multi-task learning for enhancing the overall performance (Sec.~\ref{multitask}).\textbf{this part to be concise and talking more about motivation of PECL. to show parameter efficient, compare the W size  $W >> W_i$}


% We adopt W2V2~\cite{baevski2020wav2vec} as the backbone network for audio modality and ViT~\cite{dosovitskiy2020vit} for visual modality. 
...
% Fig.~\ref{fig:main} (b) and (d) depict the audio and visual encoders. Fig.~\ref{fig:main} (c) and (e) provide a detailed presentation of the UT-Adapter and PAVF module structures, respectively


\subsection{Uniform Temporal Adapter (UT-Adapter) \label{UT-adapter}}

% To reduce the computational cost and alleviate the overfitting issue,
...
\begin{equation}
\label{eq7}
    \boldsymbol{P^i} = \boldsymbol{X_v^i}^{\intercal}\boldsymbol{W_P^i}\boldsymbol{X_a^i}.
\end{equation}
...
% \cellcolor{Gainsboro!60}  cross testing full table in suppl.

% % Cross-Testing table- previous one
% \begin{table}[]
% \centering
% \resizebox{0.45\textwidth}{!}{%
% \begin{tabular}{ccc|cc}
%     \toprule
%     \multirow{2}{*}{Modalities} & \multicolumn{2}{c}{Train Data - DOLOS} & \multicolumn{2}{c}{Train Data - Box of Lies}  \\
%      & Test BgOL & Test RLT & Test BgOL & Test RLT \\
%     \midrule
%     Visual & 56.92 (+4.3) & 55.32 (+2.55) & 52.62 & 52.77 \\
%     Audio & 55.39 (+4.0) & 59.15 (+4.68) & 51.39 & 54.47 \\ \hline
%     \rowcolor{Gainsboro!60} Fusion (PAVF) & 57.54 (+4.31) & 56.17 (+2.98) & 53.23 & 53.19 \\
%     \bottomrule
% \end{tabular}
% }
% \caption{\textcolor{blue}{updating table}Cross-testing on Bag-of-Lies (BgOL) and  Real-Life-trials (RLT) datasets. The metric is ACC (\%).}
% % \vspace{-1.0em}
% \label{table:cross-testing}
% \end{table}
...
The performance of PECL model on DOLOS is presented in Table~\ref{table:intra-testing}. The average results were calculated from the 3 folds defined in the train-test protocol. Duration and gender results were calculated based on long-short and male-female protocols. For unimodal results, the visual modality showed slightly better performance than the audio across all the protocols. To study the performance of a simple feature-level fusion, we concatenated the last output embeddings from visual and audio encoders to perform prediction. The concatenation usually draws the average of unimodal predictors. %as it cannot effectively explore the complementary information between visual and audio modalities. 
To overcome this issue, the proposed PAVF module learned the correlation between visual and audio features from different encoder layers and improved the performance.
%explored the complementary information. 
% Therefore, our fusion method achieved better performance than unimodal results. 
The multi-task learning further boosted the accuracy across all the protocols. However, by comparing the overall performances, we observed that the duration and gender factors showed an adverse effect on performance. This experimental comparison pointed out the importance of duration and gender factors in deception detection.
...
\begin{tabular}{cccc}
    ...
    \multirow{3}{*}{A} & MFCC~\cite{wu2018deception} & MLP & 56.86 \\
     ...
     & \cellcolor{Gainsboro!60} Speech Audio~\cite{karnati2021lienet,ding2019face,krishnamurthy2018deep,karimi2018toward} & \cellcolor{Gainsboro!60}Ours (Audio) & \cellcolor{Gainsboro!60} \textbf{59.19} \\ \hline
    ...
    \multirow{5}{*}{V+A} & Face + Open SMILE~\cite{gogate2017deep,krishnamurthy2018deep,yang2021multimodal} & Score-Fusion & 60.00 \\
     & \cellcolor{Gainsboro!60} Face + Speech~\cite{karnati2021lienet,karimi2018toward,ding2019face} & \cellcolor{Gainsboro!60}Ours(Concat) &\cellcolor{Gainsboro!60} \textbf{61.62} \\
     & \cellcolor{Gainsboro!60} Face + Speech &\cellcolor{Gainsboro!60} Ours(PAVF) &\cellcolor{Gainsboro!60} \textbf{64.75}\\
    \bottomrule
\end{tabular}%
\end{adjustbox}
%\end{adjustbox}
...
\vspace{-0.4em}
We benchmark DOLOS in Table~\ref{table:benchmark}, following the deception detection literature. It is worth noting that the existing literature on deception detection is not standardized with protocols, data processing, and feature extraction methodologies. 
% Since the previous research works did not release their source codes,
To highlight and cover the most representative works in deception detection literature, we benchmarked DOLOS on the popular visual and audio features used before for deception detection. The Open Face features (2D facial landmarks, eye gaze, and head pose) and Action Units (AU) features were extracted using the OpenFace toolkit~\cite{amos2016openface}. The facial affect (emotion) features were extracted using the Affectnet~\cite{mollahosseini2017affectnet} model. For audio, the Mel Cepstral Frequency Coefficients (MFCC) and open SMILE features were extracted by using the openSMILE toolkit~\cite{eyben2010opensmile}.

\vspace{0.3em}
\noindent\textbf{Performance.} For visual modality, the RGB face images gave the best performance, since they contained more deceptive cues. This was followed by manually annotated MUMIN features, affect features, and AU features, which all represented high-level human expressions. The low-level facial landmark features from Open Face did not deliver good performance.
For audio modality, raw speech audio with rich deceptive cues performed the best. The openSMILE features performed better than MFCC, since it extracted nearly three times the audio features compared with MFCC. We also performed simple score-level fusion for the best performing features in the two modalities.
We can observe that the proposed transformer-based deception detector achieved the highest performance in both unimodal learning and multimodal setting.
...
\vspace{-0.5em}
Here we conducted detailed ablation studies on different components of PECL model. The analysis was based on the train-test protocol. 
% ablation figure
\begin{figure}[t]
\centering
% \includegraphics[width=8.5cm,height=5cm]{Figures/ablation.drawio.pdf}
...
% Kernel Size & 3 & 5 & 7 & 9 \\
% \midrule
% ACC (\%) & \textbf{64.75} & 64.4 & 64.31 & 64.14 \\
% \bottomrule
% \end{tabular}%
% }
% \end{adjustbox}
% % \vspace{-0.8em}
% \caption{Ablation study on UT-Adapter kernel size.}
% \label{table:adapter kernel ablation}
% \vspace{-1.2em}
% \end{table}
...
\vspace{0.3em}
\noindent\textbf{Impact of encoder depth and adapter types.} Fig.~\ref{fig:ablation} (a) indicated that the proposed PECL performed best with four encoder layers, with performance decreasing when additional encoder layers were added, which may due to overfitting. Fig.~\ref{fig:ablation} (b) showed that UT-Adapter outperformed the NLP adapter~\cite{houlsby2019parameter} and full fine-tuning.

\vspace{0.3em}
\noindent\textbf{Comparison of parameter amount.} Table~\ref{table:params numbers} demonstrated that our proposed method achieved the best performance while utilizing the least amount of trainable parameters when compared to both the NLP adapter and full fine-tuning approaches.

\vspace{0.3em}
\noindent\textbf{Impact of adapter position.} The ablation study for UT-Adapter positions inside the transformer encoders is presented in Table \ref{table:adapter position ablation}. The results showed that the UT-Adapter worked the best when parallel to both MHSA and FFN layers~\cite{jie2022convolutional}, as they captured the temporal attention for both MHSA and MLP. The performance dropped when parallel to only either layer. The performance was also close for UT-Adapter placed inbetween MHSA and FFN layers.
% The performance is closely trailed by UT-Adapter inbetween MHSA and FFN, which is similar to the conformer\cite{49414} architecture.

\vspace{0.3em}
% \noindent\textbf{Impact of kernel size.} The effect of convolutional kernel size in UT-Adapter is presented in Table \ref{table:adapter kernel ablation}. Kernel size of 3 delivered the best results with fewer parameters and a lower computation budget. For larger kernels, the performance dropped marginally and also at the expense of more parameters.


\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{Figures/fig5.pdf}
% \vspace{-0.9em}
