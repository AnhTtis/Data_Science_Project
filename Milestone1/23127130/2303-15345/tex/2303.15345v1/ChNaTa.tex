...
\begin{figure}[h!]
\centering
\begin{tikzpicture}[fill opacity=0.9,draw,scale=2.5]
...
\end{tikzpicture}\quad\begin{tikzpicture}[fill opacity=0.9,draw,scale=2.5]
...
In this work, we study the spectrum of a Laplace operator with mixed boundary conditions in a chamfered quarter of layer characterized by two parameters $\kappa_1$, $\kappa_2\in\bbR$ gathered in some vector $\kappa=(\kappa_1,\kappa_2)$. To describe the geometry, referring to carpentry and locksmith tools, we first define the ``blade'' 
\begin{equation}\label{00}
\mathscr{B}^{\kappa_1} := \big\{ x=(x_1,x_2,x_3)\in \bbR^3\,|\,x_1 > \kappa_1 x_3,\,x_2 \in \bbR,\,x_3\in (0,1) \big\}
\end{equation}
...
\end{array}
\end{equation}
where $\partial_\nu$ is the outward normal derivative on $\partial\Omega^\kappa$. Denote by $\mH_0^1(\Omega^\kappa ; \Sigma^\kappa)$ the Sobolev space of functions of $\mH^1(\Omega^\kappa)$ vanishing on $\Sigma^\kappa$. Classically (see e.g. \cite{Lad,LiMa68}), the variational formulation  of Problem \ef{MainProblem} writes
...
\end{array}
\end{equation}
where for a domain $\Xi$, $(\cdot,\cdot)_\Xi$ stands for the inner product of the Lebesgue spaces $\mL^2(\Xi)$ or $(\mL^2(\Xi))^3$ according to the case. Using the homogeneous Dirichlet condition on $\Sigma^\kappa$ for the functions in $\mH_0^1(\Omega^\kappa ; \Sigma^\kappa)$, one can prove that there holds the Friedrichs inequality
\[
\Vert u ; \mL^2(\Omega^\kappa) \Vert^2 \leq c_\kappa  \Vert \nabla_x u ; 
\mL^2(\Omega^\kappa) \Vert^2\qquad\forall u\in \mH_0^1(\Omega^\kappa ; \Sigma^\kappa).
\]
Classically (see e.g \cite[\S10.1]{BiSo87}, \cite[Ch.\,VIII.6]{RS78}), the variational problem (\ref{FV}) gives rise to the unbounded operator $A^\kappa$ of $\mL^2(\Omega^\kappa)$ such that
\[
\begin{array}{rccl}
...
\end{array}
\]
with $\mathcal{D}(A^\kappa):=\{u\in\mH_0^1(\Omega^\kappa ; \Sigma^\kappa)\,|\,\Delta u\in\mL^2(\Omega^\kappa)\mbox{ and }\partial_\nu u=0\mbox{ on }\Gamma_1^\kappa\cup\Gamma_2^\kappa\}$. The operator $A^\kappa$ is positive definite and selfadjoint. Since $\Omega^\kappa$ is unbounded, the embedding $\mH_0^1(\Omega^\kappa ; \Sigma^\kappa)\subset \mL^2(\Omega^\kappa)$ is not compact and $A^\kappa$ has a non-empty essential component $\sigma_{\mrm{ess}}(A^\kappa)$ (\cite[Thm.\,10.1.5]{BiSo87}). Let us mention that the case
\[
\kappa_1 = \kappa_2 = 1
\]
plays a particular role. Indeed in this situation, if $u$ is an eigenfunction associated with an eigenvalue of $A^\kappa$, by extending $u$ via even reflections with respect to the faces $\Gamma_1^\kappa$, $\Gamma_2^\kappa$, one gets an eigenvalue of the Dirichlet Laplacian in the  Fichera layer 
\[
\mathscr{F} = \bigcup_{j=1,2,3} \big\{ x\,|\,x_j \in (0,1),\, x_k > 0,\,k \not=j \big\}
\]
represented in Figure \ref{Figure3D} right (see \cite{Fich75} for the original article that gave rise to the name). This latter problem has been studied in \cite{DaLO18,BaNa21}. More precisely, in \cite{DaLO18} the authors give a characterization of the essential spectrum of the Dirichlet Laplacian and show that the discrete spectrum has at most a finite number of eigenvalues. The existence of discrete spectrum is proved in \cite[Thm.\,2]{BaNa21}.\\
\newline
The goal of this paper is to get similar information for the operator $A^\kappa$ with respect to the parameter $\kappa$. Observe that by exchanging the axes or  modifying their orientations, there is no loss of generality to restrict our study to the cases
\begin{equation}\label{01}
\kappa_1 \ge 0,\qquad |\kappa_2| \leq \kappa_1.
\end{equation}
Note that we consider mixed boundary conditions in (\ref{02}) because we have in mind to study the spectrum of the Dirichlet Laplacian in geometries similar to the Fichera layer of Figure \ref{Figure3D}. Indeed, playing with symmetries, we can reduce the analysis of certain of these problems to the one of (\ref{02}) (see Figure \ref{FigCubicalStructure} for an example). On the other hand, let us mention that the spectrum of the Dirichlet Laplacian in $\Om^\kappa$ has a rather simple structure with only essential spectrum and no discrete spectrum. This is a consequence of Proposition \ref{DirichletLaplacianApp} in Appendix.\\
\newline
This note is organized as follows. In Section \ref{sec2}, we describe the essential spectrum of $A^\kappa$ (Theorem \ref{T11}). Then in Section \ref{sec2bis}, we state the results for the discrete spectrum of $A^\kappa$ (the main outcome of the present work is Theorem \ref{T24}). The next four sections contain the proof of the different items of Theorem \ref{T24}. In Section \ref{SectionNum}, we illustrate the theory with some numericals results. Finally we establish the above mentioned result related to the Dirichlet Laplacian in $\Om^\kappa$ in the Appendix.
...
\newline
$\textit{2)}$ There exists $h(\kappa_1)>0$ such that:\\[3pt]
...
\in (0,\kappa_1)$. The rest of the present note is dedicated to the proof of the above statements.
...
\draw[-] (3.5,-0.1) -- (3.5,0.1);
\draw (-3.5,-0.2) node[anchor=north]{$-\kappa_2$};
\draw (3.5,-0.2) node[anchor=north]{$\kappa_2$};
\draw (4.5,0) node[anchor=west]{$\kappa_1$};
...
\draw[->] (-4,0) -- (4.4,0);
\draw[->] (0,-0.5) -- (0,3);
\draw[dashed] (1.8,-0.1) -- (1.8,1.8);
\draw (1.8,-0.1) node[anchor=north]{$h(\kappa_1)$};
\draw[domain=1.8:3.5, smooth, variable=\x, blue,line width=0.5mm,dotted] (0,1.9)--(1.8,1.9) -- plot ({\x}, {2.22-0.1*\x*\x});
\draw[domain=-3.5:0, smooth, variable=\x, blue,line width=0.5mm,dotted] plot ({\x}, {1.9-0.07*\x*\x});
\draw[domain=-3.5:3.5, smooth, variable=\x, red,line width=0.5mm] plot ({\x}, {2-0.1});
\end{tikzpicture}
\caption{Picture of the behaviour of $\lambda_1^\kappa$, the smallest eigenvalue of $\sigma_{\rm d}(A^\kappa)$, for a given $\kappa_1>0$ and $\kappa_2\in[-\kappa_1,0)\cup(h(\kappa_1),\kappa_1]$.}\label{BehaviourEigen}
\end{figure}
...
\Omega_-^\kappa := \{ x \in \Omega^\kappa \,|\, x_2 < 0\},\qquad\quad\Omega_+^\kappa := \{ x \in \Omega^\kappa \,|\, x_2 > 0\} = \Omega^{(0,\kappa_2)}
\end{equation}
(see Figure \ref{Figure3DDecoupage}). Then for $\eps>0$ small, define $\psi^\varepsilon$ such that
\begin{equation}\label{12}
\psi^\varepsilon(x) = \begin{array}{|ll}
v(x_1,x_3)&\mbox{ in }\Omega_-^\kappa\\[3pt]
e^{-\varepsilon x_2} v(x_1,x_3) &\mbox{ in }\Omega_+^\kappa
\end{array}
\end{equation}
where $v$ is an eigenfunction of the 2D problem (\ref{pb2D}) associated with $\mu_1^{\alpha_1}$, the smallest eigenvalue, and $\alpha_1 = \arctan\kappa_1$. To set ideas, we choose $v$ such that $\Vert  v;\mL^2( \Pi^{\alpha_1}) \Vert=1$. Note that $\psi^\varepsilon$ satisfies the homogeneous Dirichlet condition on $\Sigma^\kappa$ and decays exponentially at infinity. Using (\ref{p5}), we obtain
\begin{equation}\label{Terme1}
\begin{array}{ll}
 & \Vert \nabla_x \psi^\varepsilon;\mL^2( \Omega_+^\kappa) \Vert^2 
- \lambda_\dagger^\kappa\,\Vert \psi^\varepsilon;\mL^2( \Omega_+^\kappa) \Vert^2 \\[6pt]
=&\dsp\big( \Vert \nabla_\xi v;\mL^2( \Pi^{\alpha_1}) \Vert^2 
+ (\varepsilon^2 - \mu_1^{\alpha_1}) 
\Vert  v;\mL^2( \Pi^{\alpha_1}) \Vert^2  \big)\int_0^\infty e^{-2 \varepsilon x_2} dx_2 
 = \frac{\varepsilon}{2}\,.
\end{array}
\end{equation}
...
\begin{array}{ll}
\Vert \nabla_x \psi^\varepsilon;\mL^2( \Omega_-^\kappa) \Vert^2 
- \lambda_\dagger^\kappa\,\Vert \psi^\varepsilon;\mL^2( \Omega_-^\kappa) \Vert^2 
...
\bea\label{17}
\Vert \nabla_x \psi ;\mL^2(\Omega_{1-}^\kappa) \Vert^2 \geq (\lambda_\dagger^\kappa + \kappa_1 \varrho)
...
is empty for all $\kappa=(\kappa_1,0)$ with $\kappa_1\ge0$. This proves in particular the result of Theorem \ref{T23}.
\ENR
\noindent The remaining part of the proof is dedicated to establishing \ef{17}. Consider the mixed boundary-value problem
...
\eeas
and the first (smallest) eigenvalue of the discrete spectrum, which is simple, admits the expansion 
\bea\label{22}
...
\psi^\varepsilon(x) = \begin{array}{|ll}
e^{-\varepsilon x_1} v(x_2,x_3) & \mbox{ in }\Omega_\wedge^\kappa \\[3pt]
e^{-\varepsilon x_2} v(x_1,x_3) & \mbox{ in }\Omega_<^\kappa 
\end{array}
\end{equation}
where $v$ is as in \ef{12}. Since $\psi^\varepsilon$ is continuous on $\Upsilon^\kappa$
...
\begin{array}{ll}
 &\Vert \nabla_x \psi ;\mL^2(\Omega_\wedge^\kappa) \Vert^2 - \mu_1^{\alpha_1}
\Vert \psi ;\mL^2(\Omega_\wedge^\kappa) \Vert^2 \\[10pt]
=& - \dsp\int_{\Omega_\wedge^\kappa}\hspace{-0.2cm} e^{- 2 \varepsilon x_1} v(x_2,x_3) 
\big( \Delta_x v(x_2,x_3) + (\mu_1^{\alpha_1} + \varepsilon^2) v(x_2,x_3)\big)\,dx \dsp\\[10pt]
 & +\dsp\int_{\Upsilon^\kappa}\hspace{-0.2cm}e^{-  \varepsilon x_1} v(x_2,x_3) 
\partial_{\nu} \big( e^{-\varepsilon x_1}   v(x_2,x_3) \big)\,ds =: I_{\Omega^{\kappa}}^\eps + I_{\Upsilon^{\kappa}}^\eps. 
\end{array}
\end{equation}
Using that $v$ solves \ef{pb2D}, we get $I_{\Omega^{\kappa}}^\eps = O(\varepsilon)$. Now consider the integral $I_{\Upsilon^{\kappa}}^\eps$. Define the new orthonormal basis $(\hat{e}_1,\hat{e}_2,\hat{e}_3)$ with
...
\begin{array}{rcl}
I_{\Upsilon^{\kappa}}^\eps &=& \dsp\int_{\Upsilon^\kappa}e^{-  \varepsilon \sqrt{2}(\hat{x}_1-\hat{x}_2)/2} v\,\cfrac{\partial }{\partial\hat{x}_2} \big( e^{-\varepsilon \sqrt{2}(\hat{x}_1-\hat{x}_2)/2} v \big)\,d\hat{x}_1d\hat{x}_3 \\[10pt]
& = & \dsp\int_{\Upsilon^\kappa}\cfrac{\eps\sqrt{2}}{2}\,e^{-  \varepsilon \sqrt{2}(\hat{x}_1-\hat{x}_2)} v^2\,d\hat{x}_1d\hat{x}_3+\dsp\int_{\Upsilon^\kappa}\cfrac{1}{2}\,e^{-  \varepsilon \sqrt{2}(\hat{x}_1-\hat{x}_2)}\,\cfrac{\partial(v^2) }{\partial\hat{x}_2}\,d\hat{x}_1d\hat{x}_3.
\end{array}
\end{equation}
...
\partial \hat{x}_2}.
\]
Remarking also that $\hat{x}_1=0$ on $\Upsilon^\kappa$, this gives 
\begin{equation}\label{Terme3Bis}
\begin{array}{ll}
&\dsp\int_{\Upsilon^\kappa}\cfrac{1}{2}\,e^{-  \varepsilon \sqrt{2}(\hat{x}_1-\hat{x}_2)}\,\cfrac{\partial(v^2) }{\partial\hat{x}_2}\,d\hat{x}_1d\hat{x}_3=\dsp\int_{\Upsilon^\kappa}\cfrac{1}{2}\,e^{-  \varepsilon \sqrt{2}\,\hat{x}_1}\,\cfrac{\partial(v^2) }{\partial\hat{x}_1}\,d\hat{x}_1d\hat{x}_3 \\[10pt]
=&\dsp\int_{\Upsilon^\kappa}\cfrac{\eps\sqrt{2}}{2}\,e^{-  \varepsilon \sqrt{2}\,\hat{x}_1}\,v^2\,d\hat{x}_1d\hat{x}_3+\int_{L^\kappa} \cfrac{1}{2}\,e^{-  \varepsilon \sqrt{2}\,\hat{x}_1}\,v^2\,\nu\cdot\hat{e}_1\,d\ell
\end{array}
\end{equation}
...
\[
\Vert \nabla_x \psi ;\mL^2(\Omega_\wedge^\kappa) \Vert^2 - \mu_1^{\alpha_1}
\Vert \psi ;\mL^2(\Omega_\wedge^\kappa) \Vert^2<0
\]
for $\eps$ small enough. Then by symmetry, we obtain
\[
\Vert \nabla_x \psi ;\mL^2(\Omega^\kappa) \Vert^2 - \mu_1^{\alpha_1}
\Vert \psi ;\mL^2(\Omega^\kappa) \Vert^2=2\Vert \nabla_x \psi ;\mL^2(\Omega_\wedge^\kappa) \Vert^2 - 2\mu_1^{\alpha_1}
\Vert \psi ;\mL^2(\Omega_\wedge^\kappa) \Vert^2<0.
\]
...
\bea\label{28}
\lambda_1^{\kappa^\varepsilon}<\lambda_1^\kappa . 
\eea
\ENP
\begin{proof}
Using again the minimum principle, we can write
\begin{equation}\label{defQuotient}
\lambda_1^{\kappa^\varepsilon} = \min_{\psi^\varepsilon \in \mH_0^1( \Omega^{\kappa^\varepsilon}; \Sigma^{\kappa^\varepsilon}
) \setminus \{ 0 \}}
\frac{\big\Vert \nabla_x \psi^\varepsilon ;\mL^2(\Omega^{\kappa^\varepsilon}) \big\Vert^2 
}{
\big\Vert \psi^\varepsilon;\mL^2(\Omega^{\kappa^\varepsilon}) \big\Vert^2}\,.
\end{equation}
Now define the function $\psi^\varepsilon$ such that
\beas
\psi^\varepsilon(x) = u^\kappa \Big( x_1,\frac{\kappa_2 x_2}{\kappa_2 + \varepsilon},x_3 \Big),
\eeas
where $u^\kappa \in\mH_0^1(\Omega^\kappa ; \Sigma^\kappa)$ is an eigenfunction associated with the first eigenvalue of $\sigma_{d}(A^{\kappa})$. Clearly $\psi^\varepsilon$ is a non zero element of $\mH_0^1( \Omega^{\kappa^\varepsilon})$. Besides, we find 
\[
\big\Vert \psi^\varepsilon;\mL^2(\Omega^{\kappa^\varepsilon}) \big\Vert^2=\cfrac{\kappa_2+\eps}{\kappa_2}\,\big\Vert u^\kappa;\mL^2(\Omega^{\kappa}) \big\Vert^2
\]
\[
\mbox{ and }\qquad\big\Vert \nabla_x\psi^\varepsilon;\mL^2(\Omega^{\kappa^\varepsilon}) \big\Vert^2=\frac{\kappa_2}{\kappa_2 + \varepsilon} 
...
\beas
\lambda_1^{\kappa^\varepsilon} &\leq& \Vert u^\kappa ;\mL^2(\Omega^\kappa ) \Vert^{-2}
\bigg( 
\frac{\kappa_2^2}{(\kappa_2 + \varepsilon)^2} 
\,\Big\Vert \frac{\partial u^\kappa}{ \partial x_2} ;\mL^2(\Omega^\kappa ) \Big\Vert^2 
...
\end{proof}
\noindent According to relation \ef{28}, the function $\kappa_2\mapsto \lambda_1^\kappa$ is strictly decreasing on $(h(\kappa_1),\kappa_1)$. Besides, Proposition \ref{P28} ensures that $\sigma_{\rm d}(A^\kappa)$ cannot be non-empty for some $\tilde{h}(\kappa_1)\in(0,h(\kappa_1))$ otherwise  $\sigma_{\rm d}(A^\kappa)$ would be non-empty for all $\kappa_2\in(\tilde{h}(\kappa_1),\kappa_1]$ which contradicts the definition of $h(\kappa_1)$. This completes the proof of the item $\textit{2)}$ of Theorem \ref{T24}. 

\begin{remark}
For $\kappa_2\in[- \kappa_1,0)$, we have seen in Section \ref{sec3} that $\sigma_{\rm d}(A^\kappa)$ is non-empty. Let $\lambda_1^\kappa$ denote the smallest eigenvalue of $\sigma_{\rm d}(A^\kappa)$. By adapting the proof of Proposition \ref{P28}, one establishes that the map $\kappa_2\mapsto \lambda_1^\kappa$ is strictly increasing on $[- \kappa_1,0)$. Together with Proposition \ref{P28}, this shows the item $\textit{3)}$ of Theorem \ref{T24}.
\end{remark}
...
In this section, we illustrate some of the results above. In Figure \ref{BoundEssentialSpectrum}, we represent an approximation of the first eigenvalue of the 2D problem (\ref{pb2D}) set in the pointed strip $\Pi^{\alpha_1}$ with respect to $\alpha_1\in(0;9\pi/20)$. We use a rather crude method which consists in truncating the domain at $\xi_2=12$ (see the picture of Figure \ref{FigureBottomView} left) and imposing homogeneous Dirichlet boundary condition on the artificial boundary. Then we compute the spectrum in this bounded geometry by using a classical P2 finite element method. To proceed, we use the library \texttt{Freefem++} \cite{Hech12} and display the results with \texttt{Matlab}\footnote{\texttt{Matlab}, \url{http://www.mathworks.com/}.} and \texttt{Paraview}\footnote{\texttt{Paraview}, \url{http://www.paraview.org/}.}. The values we get are coherent with the ones recalled in (\ref{defAlpha1}).\\
\newline
In Figure \ref{Figk1m1}--\ref{Figk11}, we fix $\kappa_1=1$ (equivalently $\alpha_1=\pi/4$) and compute the first eigenvalue of $\sigma_{\rm d}(A^\kappa)$ for different $\kappa_2$. For $\kappa_1=1$, the bound of the essential spectrum of $A^\kappa$ is $\lambda^\kappa_\dagger\approx0.929\pi^2$ (see Figure  \ref{BoundEssentialSpectrum} as well as \cite{DaLO18}). For each of the three $\kappa_2$, in agreement with Theorem \ref{T24}, we find an eigenvalue below the essential spectrum.   Actually, in each situation our numerical experiments seem to indicate that there is only one eigenvalue in the discrete spectrum, which is a result that we have not proved. Interestingly, for $\kappa_2=-0.1$ (Figure \ref{Figk1m01}), the eigenfunction is not particularly localized at the intersection of the obliques sides. We emphasize that here we simply compute the spectrum of the Laplace operator with mixed boundary conditions in the bounded domain $\{x\in\Om^\kappa\,|x_1<6\mbox{ and }x_2<6\}$. At $x_1=6$ and $x_2=6$, we impose homogeneous Neumann boundary condition. Admittedly, this is a very naive approximation, especially in the case of Figure \ref{Figk1m01} where the eigenfunction has a poor decay at infinity. In the latter situation, the corresponding eigenvalue is very close to $\lambda^\kappa_\dagger$ and we do not give its value because our approximation probably lacks precision.\\
\newline
...
\newline
As mentioned in the introduction, the fact that the discrete spectrum of $A^\kappa$ for $\kappa=(1,1)$ is not empty ensures that the Dirichlet Laplacian in the so-called Fichera layer $\mathscr{F}$ of Figure \ref{Figure3D} right admits an eigenvalue below the essential spectrum. This can be proved by playing with symmetries and reconstructing $\mathscr{F}$ from three versions of $\Om^{(1,1)}$. Now, gluing six domains $\Om^{(1,-1)}$, we can create the cubical structure pictured in Figure \ref{FigCubicalStructure}. Then, from Theorem \ref{T24} which guarantees that $\sigma_{\rm d}(A^\kappa)$ contains at least one eigenvalue, we deduce that the Dirichlet Laplacian in this geometry has at least one eigenvalue. 
...
\begin{figure}[!ht]
\centering
\includegraphics[width=7cm]{k1m1View1.png}\qquad
\includegraphics[width=8cm]{k1m1View2.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=7cm]{k1m01.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=7cm]{k11.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=7cm]{k3m3View1.png}\qquad
\includegraphics[width=8cm]{k3m3View2.png}
