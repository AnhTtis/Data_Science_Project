...
If $\Phi(I) \supseteq [l,u]$, then there is nothing to show, so suppose $\Phi(I) \not\supseteq [l,u]$. If $y \in \Phi(I)^c \cap [l,u]$, then, since $\Phi(I) \cap [l,u]$ is closed and $l,u \in \Phi(I)$, we have $[y-\eta,y+\eta] \cap \Phi(I) = \emptyset$ for some $\eta > 0$ with $[y-\eta,y+\eta] \subset [l,u]$. Therefore, the function $\LSC (\Gamma\circ\Phi^{-})$ is constant on the interval $[y-\eta/2,y+\eta/2]$, implying in particular that
\begin{equation*}
    y^{\star}  = \max \argmax_{y' \in [l,u]} \left\{ty' -\LSC (\Gamma\circ\Phi^{-})(y') \right\} \neq y. \qed
\end{equation*}
...
$\G(v)$ is a Gaussian process with $\E[\G(v)]=\mu(v)$ for all $v\in \R$ and $\Cov[\G(v),\G(v')] = C \min\{|v|,|v'|\}\1(\sign(v)=\sign(v')) =: \mathcal{K}(v,v')$ for some positive constant $C$ and for all $v,v' \in \R$. The form of the covariance kernel $\mathcal{K}$ implies that for every $\tau> 0$ and every $v,v' \in \R$, 
\begin{equation*}
    \mathcal{K}(v \tau,v' \tau) = \mathcal{K}(v,v') \tau
\end{equation*}
and
\[\mathcal{K}(v + v',v + v') - 2 \mathcal{K}(v + v',v') + \mathcal{K}(v',v') = \mathcal{K}(v,v).\]
In addition, $\mathcal{K}(1,1) > 0$ and $\lim_{v \downarrow 0}\mathcal{K}(1,v) / \sqrt{v} = 0$.\footnote{As a side note, if the covariance kernel satisfies the two displays, then the Gaussian process is two-sided Brownian motion, up to mean shift and scaling by a positive constant.}

We begin by adapting the arguments of \citet[pp. 196-198]{Kim-Pollard_1990_AoS} to show that a maximizer of $\G(v)$ over $v \in \R$ exists and is unique with probability one. Let $\G^{\mu}(v) = \G(v) - \mu(v)$ be the centered process and suppose that, for the same $c>1/2$ as in the hypothesis,
\begin{equation}\label{Appendix eq: BC lemma}
    \P\left[\limsup_{|v|\to\infty}\frac{\G^{\mu}(v)}{|v|^{c}} > \eta \right] = 0 \qquad \text{for any } \eta > 0.
\end{equation}
...
\begin{align*}
    \sum_{k = 2}^{\infty} \P \left[ \sup_{k - 1 \leq |v| \leq k} \frac{\G^{\mu}(v)}{|v|^c} > \eta \right] 
    &\leq \sum_{k = 2}^{\infty} \P \left[ \sup_{|v| \leq k} \G^{\mu}(v) > (k / 2)^c \eta \right] \\
    &= \sum_{k = 2}^{\infty} \P \left[ \sup_{|v| \leq 1} \G^{\mu}(v) > k^{c - 1/2} 2^{-c} \eta \right] \\
    &\leq \frac{2^{4c/(2c-1)} \E \left[ \sup_{|v| \leq 1} |\G^{\mu}(v)|^{4c/(2c-1)} \right]} {\eta^{4c/(2c-1)}} \sum_{k = 2}^{\infty}k^{-2} < \infty,
\end{align*}
...
Fix $x \in \R$ and define $\widetilde{\G}_x(0) = 0$ and
\begin{equation*}
    \widetilde{\G}_x(v) = \frac{\G(x+v) - \G(x) }{\sqrt{\mathcal{K}(v,v)}}, \qquad v \neq 0.
\end{equation*}
Then $\max_{v \in \R}\widetilde{\G}_x(v) \geq 0$ and, for any set $\mathcal{V} \subset \R$,
\begin{equation}\label{Appendix eq: probability bound with subset}
    \P \left[ \argmax_{v \in \R}\{\G(v)\} = x \right] = \P \left[ \max_{v \in \R} \widetilde{\G}_x(v) \leq 0 \right] \leq \P \left[ \max_{v\in\mathcal{V}} \widetilde{\G}_x(v) \leq 0\right].
\end{equation}
...
In particular, for $\varepsilon \in (0,1)$, we construct $v_{\varepsilon} > 0$ such that
\begin{equation}\label{Appendix eq: mean_bound}
    \E \left[ \widetilde{\G}_x (v_{\varepsilon}^i) \right] \geq - \varepsilon \quad \text{ for every } i \geq 1,
\end{equation}
and
\begin{equation}\label{Appendix eq: cov_bound}
    \left| \Cov \left( \widetilde{\G}_x(v_{\varepsilon}^i),\widetilde{\G}_x(v_{\varepsilon}^j) \right) \right| \leq \varepsilon \quad \text{ for every } j > i \geq 1.
\end{equation}
For any $N \in \N$, defining $\mathcal{V}_{\varepsilon,N} = \{v_{\varepsilon},\dots,v_{\varepsilon}^N\}$, we therefore have
\begin{equation*}
    \P \left[ \max_{v \in \R} \widetilde{\G}_x(v) \leq 0 \right] \leq \liminf_{\varepsilon \downarrow 0} \P \left[ \max_{v \in \mathcal{V}_{\varepsilon,N}} \widetilde{\G}_x(v) \leq 0 \right] \leq \left| \int_{-\infty}^{0} \frac{\exp(-x^2/2)}{\sqrt{2\pi}}dx \right| ^N = 2^{-N},
\end{equation*}
where the second inequality uses the fact that convergence of means and covariances of normal random vectors implies convergence in distribution. Since $N$ is arbitrary, the left-hand side in the preceding display is zero. Letting $\varepsilon \in (0,1)$ be given, the proof can therefore be completed by exhibiting $v_{\varepsilon}$ satisfying \eqref{Appendix eq: mean_bound} and \eqref{Appendix eq: cov_bound}.
...
\begin{equation*}
    \E \left[ \widetilde{\G}_x(\tau) \right] = \frac{\mu(x + \tau) - \mu(x)}{\sqrt{\mathcal{K}(\tau,\tau)}}  =  \frac{[\mu(x + \tau ) -\mu(x)] / \sqrt{\tau}}{\sqrt{\mathcal{K}(1,1)}} \geq -\varepsilon
\end{equation*}
...
\begin{equation*}
    \Cov\left(\widetilde{\G}_x(\tau_i),\widetilde{\G}_x( \tau_j)\right) = \frac{\mathcal{K}(\tau_i,\tau_j)}{\sqrt{\mathcal{K}(\tau_i,\tau_i)\mathcal{K}(\tau_j,\tau_j)}} = \frac{\mathcal{K}(1,\tau_j/\tau_i)/\sqrt{\tau_j/\tau_i} }{\mathcal{K}(1,1)} \in [-\varepsilon,\varepsilon]
\end{equation*}
for all $\tau_i,\tau_j > 0$ with $\tau_j/\tau_i \leq \bar{\tau}_\varepsilon^{''}$. If $v_{\varepsilon} \in (0, \bar{\tau}_\varepsilon^{'} \land \bar{\tau}_\varepsilon^{''})$, then \eqref{Appendix eq: mean_bound} and \eqref{Appendix eq: cov_bound} are satisfied.\qed
...
\begin{equation*}
    |\LSC_{\Phi} (\Gamma_0)(x) - \Gamma_0(x)| \leq 2 \left( \sup_{x' \in I} |\theta_0 (x')| \right) \sup_{x' \in I} |\Phi(x') - \Phi_0 (x')|.
\end{equation*}
\end{lemma}
...
\begin{align*}
    ...
    &\leq 2 \sup_{x' \in I}|\Phi(x') - \Phi_0 (x')|.    
\end{align*}
Also,
\begin{equation*}
    |\Phi_0(\Phi^-(\Phi(x)+)) - \Phi_0(x)| \leq 2 \sup_{x' \in I} |\Phi(x') - \Phi_0 (x')|
\end{equation*}
because, for every $\eta > 0$,
\begin{align*}
    0
    &\leq \Phi_0(\Phi^-(\Phi(x)+)) - \Phi_0(x) \leq \Phi_0(\Phi^-(\Phi(x) + \eta)) - \Phi_0(x) \\
...
\begin{equation*}
    |\LSC_{\Phi}(\theta \Phi)(x) - \theta \Phi(x)| \leq |\theta| \sup_{x' \in I} |\Phi(x') - \Phi(x'-)|.
\end{equation*}
\end{lemma}
...
\begin{equation*}
    \LSC_{\Phi}(\theta \Phi)(x) = \theta \Phi(\Phi^-(\Phi(x))-) \leq \theta \Phi(\Phi^-(\Phi(x))) = \theta \Phi(x),
\end{equation*}
where
\begin{equation*}
...
\begin{equation*}
    \Phi_0(x) - \Phi_0(x - \epsilon) \geq \epsilon \inf_{x' \in I_{\x}^{\delta + \epsilon}} \partial \Phi_0(x') > 2 \sup_{x' \in I_{\x}^{\delta + \epsilon}}|\Phi(x') - \Phi_0(x')|,
\end{equation*}
whereas
\begin{equation*}
    \left| \left[ \Phi(x - \epsilon) - \Phi_0(x - \epsilon) \right] - \left[ \Phi(x) - \Phi_0(x) \right] \right| \leq 2 \sup_{x' \in I_{\x}^{\delta + \epsilon}} |\Phi(x') - \Phi_0(x')|.
\end{equation*}
In other words, $x \not \in I_{\x}^{\delta}$.
...
\begin{equation*}
    |\LSC_{\Phi} (\theta \Phi) (x) - \theta \Phi(x)| \leq |\theta| \sup_{x' \in I_{\x}^{\delta + \epsilon}}|\Phi(x') - \Phi(x'-)|.
\end{equation*}
\end{lemma}
...
\begin{align*}
    ...
    &= \P \left[ \max\argmax_{x \in \widehat{\Phi}_n^{-}([0,\widehat{u}_n])} \left\{ [\theta_0(\x) + t r_n^{-1}] \widehat{\Phi}_n(x) - \LSC_{\widehat{\Phi}_n} (\widehat{\Gamma}_n) (x) \right\} < \widehat{\Phi}_n^{-} \circ \widehat{\Phi}_n (\x) \right] \\
...
\end{equation*}
\begin{equation*}
    \widehat{H}_{\x,n}^{\q}(v;t) = \LSC_{\widehat{L}_{\x,n}^{\q}} \left(\widehat{G}_{\x,n}^{\q} + M_{\x,n}^{\q} + r_n \theta_0(\x) \widehat{L}_{\x,n}^{\q} \right)(v)  - [r_n \theta_0(\x) + t] \widehat{L}_{\x,n}^{\q}(v),
\end{equation*}
and
\begin{equation*}
...
\begin{align*}
    ...
    M_{\x,n}^{\q}(v) 
    &= \sqrt{n a_n} \left[\Gamma_{0}(\x + v a_n^{-1}) - \Gamma_{0}(\x)\right] - \theta_{0}(\x) \sqrt{n a_n} \left[\Phi_{0}(\x + v a_n^{-1})-\Phi_{0}(\x)\right], \\
...
\begin{align*}
    \frac{\Gamma_0(\x+u) - \theta_0(\x) \Phi_0(\x + u) -\Gamma_0(\x) + \theta_0(\x) \Phi_0(\x)}{u^{\q+1}} 
    &\to \lim_{u \to 0}\frac{\left[\theta_0(\x + u)-\theta_0(\x)\right] \partial \Phi_0(\x + u)}{(\q+1) u^{\q}} \\
...
\begin{align*}
    0 
    &\geq \LSC_{\widehat{L}_{\x,n}^{\q}}\left(\widehat{G}_{\x,n}^{\q} + M_{\x,n}^{\q} + r_n \theta_0(\x) \widehat{L}_{\x,n}^{\q}\right) - \left(\widehat{G}_{\x,n}^{\q} + M_{\x,n}^{\q} + r_n \theta_0(\x) \widehat{L}_{\x,n}^{\q}\right) \\
...
\begin{align*}
    ...
    &\leq \sup_{x \in I} \left|\widehat{\Gamma}_n(x) - \Gamma_0(x)\right| + 2 \left(\sup_{x \in I}|\theta_0 (x)|\right) \sup_{x \in I}|\widehat{\Phi}_n(x) - \Phi_0(x)| = o_{\P}(1).
\end{align*}
...
\begin{align*}
    \limsup_{n \to \infty}\P \left[ |\widehat{v}_n(t)| > 2^K \right]
    &\leq \limsup_{n \to \infty}\sum_{j \geq K : 2^j \leq a_n \delta'} \P \left[ 2^j < |\widehat{v}_n(t)| \leq 2^{j+1} \cap \mathcal{A}_n' \right] \\
...
\begin{align*}
    \widehat{H}_{\x,n}^{\q}(v;t) - \widehat{H}_{\x,n}^{\q}(0;t)
    &\geq \LSC_{\widehat{L}_{\x,n}^{\q}}\left(\widehat{G}_{\x,n}^{\q}\right)(v) + \LSC_{\widehat{L}_{\x,n}^{\q}}\left(M_{\x,n}^{\q}\right)(v) - t \widehat{L}_{\x,n}^{\q}(v) \\
...
\begin{align*}
    ...
    &\leq \frac{\E \left[ \sup_{v \in V_2(j)} \left| \widehat{G}_{\x,n}^{\q}(v) \right| \1_{\mathcal{A}_n'} \right]}{\inf_{v \in V_1(j)} \left[C_M v^{\q+1} - C_L |v| - C_0\right]} \qquad \text{for every } j \geq 2 \text{ with } 2^j \leq a_n \delta',
\end{align*}
where, by \ref{M-Assumption B: moment bound}, we may assume that, for some $C_G$,
\begin{equation*}
    \E \left[ \sup_{v \in V_2(j)} \left| \widehat{G}_{\x,n}^{\q}(v) \right| \1_{\mathcal{A}_n'} \right] \leq \sum_{j-1 \leq j' \leq j+1} \E \left[ \sup_{v \in V_1(j')} \left| \widehat{G}_{\x,n}^{\q}(v) \right| \1_{\mathcal{A}_n'} \right] \leq C_G 2^{j \beta},
\end{equation*}
...
\begin{align*}
    ...
    &= \P_n^* \left[ \max\argmax_{x \in \widehat{\Phi}_n^{*-}([0,\widehat{u}_n^*])} \left\{[\widehat{\theta}_n(\x) + t r_n^{-1}] \widehat{\Phi}_n^*(x) - \LSC_{\widehat{\Phi}_n^*} (\widetilde{\Gamma}_n^*)(x) \right\} < \widehat{\Phi}_n^{*-} \circ \widehat{\Phi}_n^*(\x) \right] \\
    &= \P_n^* \left[ \max\argmin_{v \in \widehat{V}_{\x,n}^{\q,*}} \widehat{H}_{\x,n}^{\q,*}(v;t) < \widehat{Z}_{\x,n}^{\q,*} \right],
\end{align*}
where
\begin{equation*}
    \widehat{V}_{\x,n}^{\q,*} = \left\{a_n (x - \x) : x \in \widehat{\Phi}_{n}^{*-}([0,\widehat{u}_{n}^*])\right\},
\end{equation*}
\begin{equation*}
    \widehat{H}_{\x,n}^{\q,*}(v;t) = \LSC_{\widehat{L}_{\x,n}^{\q,*}} \left( \widehat{G}_{\x,n}^{\q,*} + \widetilde{M}_{\x,n}^{\q} + r_n \widehat{\theta}_n(\x) \widehat{L}_{\x,n}^{\q,*} \right) (v)  - [r_n \widehat{\theta}_n(\x) + t] \widehat{L}_{\x,n}^{\q,*}(v)
\end{equation*}
and
\begin{equation*}
    \widehat{Z}_{\x,n}^{\q,*} = a_n \left[ \widehat{\Phi}_{n}^{*-} \circ \widehat{\Phi}_{n}^*(\x) - \x \right],
...
\begin{align*}
    \widehat{G}_{\x,n}^{\q,*}(v)
    &= \sqrt{n a_n} \left[\widehat{\Gamma}_n^*(\x + v a_n^{-1}) - \widehat{\Gamma}_n^*(\x) - \widehat{\Gamma}_n(\x + v a_n^{-1}) + \widehat{\Gamma}_n(\x) \right] \\
    ...
    \widetilde{M}_{\x,n}^{\q}(v) 
    &= \sqrt{n a_n} \widetilde{M}_{\x,n}(v a_n^{-1}), \qquad \widehat{L}_{\x,n}^{\q,*}(v) = a_n \left[ \widehat{\Phi}_{n}^*(\x + v a_n^{-1}) - \widehat{\Phi}_{n}^*(\x) \right].
\end{align*}
By \ref{M-Assumption B: uniform convergence Phi} and Lemma \ref{[Appendix Lemma] Phi- circ Phi}, $\widehat{Z}_{\x,n}^{\q,*} = o_{\P}(1)$. Suppose also that
...
\begin{align*}
    0
    &\geq \LSC_{\widehat{L}_{\x,n}^{\q,*}} \left( \widehat{G}_{\x,n}^{\q,*} + \widetilde{M}_{\x,n}^{\q} + r_n \widehat{\theta}_n(\x) \widehat{L}_{\x,n}^{\q,*} \right) - \left( \widehat{G}_{\x,n}^{\q,*} + \widetilde{M}_{\x,n}^{\q} + r_n \widehat{\theta}_n(\x) ,\widehat{L}_{\x,n}^{\q,*} \right) \\
...
\begin{align*}
    ...
    &\geq \widehat{\theta}_n(\x;t) \widehat{\Phi}_n^*(\x) - \widetilde{\Gamma}_n^*(\widehat{\x}_n^*) \\
    &= \widehat{\theta}_n(\x;t) \widehat{\Phi}_n^*(\x) - \widehat{\theta}_n(\x) \widehat{\Phi}_n(\widehat{\x}_n^*) - \widehat{\Gamma}_n^*(\widehat{\x}_n^*) + \widehat{\Gamma}_n(\widehat{\x}_n^*) - \widetilde{M}_{\x,n}^\q(\widehat{Z}_{\x,n}^{\q,*}) / \sqrt{n a_n} = o_{\P}(1),  
\end{align*}
where the last equality uses \ref{M-Assumption B: uniform consistency Gamma}, $\widehat{Z}_{\x,n}^{\q,*} = o_{\P}(1)$, and Assumption \ref{M-Assumption C}. Also, using \ref{M-Assumption B: uniform consistency Gamma}, \ref{M-Assumption B: uniform convergence Phi}, and Assumption \ref{M-Assumption C}, we have, uniformly in $x \not \in I_\x^\delta$ and for some $c > 0$,
\begin{align*}
    \widetilde{\Gamma}_n^*(x) - \widehat{\theta}_n(\x) \widehat{\Phi}_n^*(x)
    &= \widehat{\Gamma}_n^*(x) - \widehat{\Gamma}_n(x) - \widehat{\theta}_n(\x) \left(\widehat{\Phi}_n^*(x) - \widehat{\Phi}_n(x)\right) + \widetilde{M}_n(x - \x) \\
...
\begin{align*}
    \widehat{H}_{\x,n}^{\q,*}(v;t) - \widehat{H}_{\x,n}^{\q,*}(0;t) 
    &\geq \LSC_{\widehat{L}_{\x,n}^{\q,*}} \left( \widehat{G}_{\x,n}^{\q,*} \right) (v) + \LSC_{\widehat{L}_{\x,n}^{\q,*}} \left( \widetilde{M}_{\x,n}^{\q} \right) (v) - t \widehat{L}_{\x,n}^{\q,*}(v) \\
...
\begin{align*}
    \widetilde{\mathcal{D}}_{\q,n}^{\mathtt{MA}}(\x) 
    &= \epsilon_n^{-(\q + 1)}[\Gamma_0(\x + \epsilon_n) - \Gamma_0(\x) - \theta_0(\x) \{\Phi_0(\x + \epsilon_n) - \Phi_0(\x)\} ] \\
...
\begin{align*}
    \widetilde{\mathcal{D}}_{\q,n}^{\mathtt{FD}}(\x) 
    &= \epsilon_n^{-(\q+1)}\sum_{k = 1}^{\q + 1} (-1)^{k + \q + 1} \binom{\q + 1}{k} [\Upsilon_0(\x + k \epsilon_n) - \Upsilon_0(\x) ] \\
...
\end{equation*}
Finally, the third term in the stochastic expansion of $\widetilde{\mathcal{D}}_{j,n}^{\mathtt{BR}}(\x)$ is asymptotically negligible under the condition that $\widehat{R}_{\x,n}(1;\eta_n) = O_{\P}(a_n^{-1/2})$ for $a_n^{-1}\eta_n^{-1}=O(1)$, while the fourth term exhibits only a higher-order dependence on $\epsilon_n$ (relative to the dependence exhibited by the first two terms).
...
\begin{equation*}
    \sqrt{n a_n} \sup_{| v|\leq K}\left|\widehat{\Gamma}_n(\x+va_n^{-1}) -\widehat{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x)\right| =  o_{\P}(1), 
\end{equation*}
\begin{equation*}
    \sqrt{n a_n} \sup_{| v|\leq K}\left|\widehat{\Phi}_n(\x+va_n^{-1}) -\widehat{\Phi}_n(\x) - \bar{\Phi}_n(\x+va_n^{-1}) + \bar{\Phi}_n(\x)\right| = o_{\P}(1), 
\end{equation*}
for each $K>0$, and thus, defining $\bar{\psi}_{\x,n}(v;\mathbf{Z}_i) = \sqrt{a_n}\psi_{\x}(va_n^{-1};\mathbf{Z}_i)$,
\begin{equation*}
    \sup_{| v|\leq K}\left|\widehat{G}_{\x,n}^{\q}(v) - \frac{1}{\sqrt{n}} \sum_{i=1}^n \left\{\bar{\psi}_{\x,n}(v;\mathbf{Z}_i) - \E[\bar{\psi}_{\x,n}(v;\mathbf{Z})]  \right\}\right|  = o_{\P}(1).
\end{equation*}
Next, we prove weak convergence of the empirical process indexed by $\{\bar{\psi}_{\x,n}(v;\cdot):|v|\leq K\}$ by verifying finite-dimensional weak convergence and stochastic equicontinuity. 
...
\end{equation*}
Also, convergence of the covariance kernel is imposed in Assumption \ref{M-Assumption E: covariance kernel}. Thus, the Lyapunov central limit theorem implies finite-dimensional weak convergence. 

For stochastic equicontinuity, proceeding as in \citet[Lemma 4.6]{Kim-Pollard_1990_AoS} and using $a_n\E[\bar{D}_{\gamma}^{\eta_n} (\mathbf{Z})^2 +   \bar{D}_{\phi}^{\eta_n} (\mathbf{Z})^2]=O(1)$, it suffices to show that
\begin{equation*}
    \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{n}\sum_{i=1}^n |\bar{\psi}_{\x,n}(v;\mathbf{Z}_i)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i)|^2=o_{\P}(1)
\end{equation*}
for any $\epsilon_n=o(1)$. For any $M>0$, using $|\bar{\psi}_{\x,n}(v;\mathbf{z})|\leq \sqrt{a_n}(1+\theta_0(\x))(\bar{D}_{\gamma}^{\eta_n} (\mathbf{z}) + \bar{D}_{\phi}^{\eta_n} (\mathbf{z}) )$
\begin{align*}
    &\sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{n}\sum_{i=1}^n |\bar{\psi}_{\x,n}(v;\mathbf{Z}_i)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i)|^2 \\
    & \leq 4(1+|\theta_0(\x)|)^2\frac{a_n}{n}\sum_{i=1}^n  [\bar{D}_{\gamma}^{\eta_n} (\mathbf{Z}_i) + \bar{D}_{\phi}^{\eta_n} (\mathbf{Z}_i)]^2 \1(\bar{D}_{\gamma}^{\eta_n} (\mathbf{Z}_i) +   \bar{D}_{\phi}^{\eta_n} (\mathbf{Z}_i)> M) \\
    & + \bar{M} \sqrt{a_n} \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \E \left[|\bar{\psi}_{\x,n}(v;\mathbf{Z}_i)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i)| \right] \\
    & + \bar{M}\sqrt{a_n} \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}}\frac{1}{n}\sum_{i=1}^n  \left\{|\bar{\psi}_{\x,n}(v;\mathbf{Z}_i) - \bar{\psi}_{\x,n}(v';\mathbf{Z}_i)| -\E \left[|\bar{\psi}_{\x,n}(v;\mathbf{Z}_i) - \bar{\psi}_{\x,n}(v';\mathbf{Z}_i)| \right]\right\}
\end{align*}
where $\bar{M}=2M(1+|\theta_0(\x)|)$, the expectation of the first term after the inequality can be made arbitrarily small by making $M$ large using $a_n \E[ \bar{D}_{\gamma}^{\eta_n} (\mathbf{Z})^4 + \bar{D}_{\phi}^{\eta_n} (\mathbf{Z})^4] = O(1)$, the second term is $o(1)$ by Assumption \ref{M-Assumption E: covariance kernel}, and the third term is $O_{\P}( \sqrt{a_n/n} )$ by Theorem 4.2 of \cite{Pollard_1989_SS}. 

\paragraph*{Bootstrap weak convergence}
We next prove $\widehat{G}_{\x,n}^{\q}\rightsquigarrow_{\P}\mathcal{G}_{\x}$. As shown below, we have
\begin{align}\nonumber
...
\end{align}
for each $K>0$. Therefore, using $\widehat{\theta}_n(\x)\to_{\P}\theta_0(\x)$ and the fact that, uniformly over $| v| \leq K$,
\begin{equation*}
    \sqrt{\frac{a_n}{n}} \sum_{i=1}^nW_{i,n} \{\phi_0(\x+va_n^{-1};\mathbf{Z}_i) - \phi_0(\x;\mathbf{Z}_i) - \frac{1}{n}\sum_{j=1}^n[\phi_0(\x+va_n^{-1};\mathbf{Z}_j) - \phi_0(\x;\mathbf{Z}_j)]\} = O_{\P}(1),
\end{equation*}
we obtain
\begin{equation*}
    \sup_{| v|\leq K}\left|\widehat{G}_{\x,n}^{\q,*}(v)-\frac{1}{\sqrt{n}} \sum_{i=1}^n W_{i,n} \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i)\right| = o_{\P}(1),
\end{equation*}
where $\widehat{\psi}_{\x,n}(v;\mathbf{Z}) = \bar{\psi}_{\x,n}(v;\mathbf{Z}) - n^{-1}\sum_{j=1}^n \bar{\psi}_{\x,n}(v;\mathbf{Z}_j)$.

To prove finite-dimensional weak convergence, we apply Lemma 3.6.15 of \cite{vanderVaart-Wellner_1996_Book}. Assumption \ref{M-Assumption E: bootstrap weights} implies that $n^{-1}\sum_{i=1}^n(W_{i,n}-1)^2\to_{\P}1$ and $n^{-1}\max_{1\leq i \leq n}W_{i,n}^2=o_{\P}(1)$. Since
\begin{align*}
    \frac{1}{n}\sum_{i=1}^n \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i)\widehat{\psi}_{\x,n}(u;\mathbf{Z}_i)
    &= \frac{1}{n}\sum_{i=1}^n \bar{\psi}_{\x,n}(v;\mathbf{Z}_i)\bar{\psi}_{\x,n}(u;\mathbf{Z}_i) \\
    &\qquad - \frac{1}{n}\sum_{i=1}^n \bar{\psi}_{\x,n}(v;\mathbf{Z}_i)\frac{1}{n}\sum_{i=1}^n \bar{\psi}_{\x,n}(u;\mathbf{Z}_i)
\end{align*}
and $\sup_{|v|\leq\eta } |\psi_{\x}(v;\mathbf{Z})|\leq \bar{D}_{\gamma}^{\eta}(\mathbf{Z}) + |\theta_0(\x)|\bar{D}_{\phi}^{\eta}(\mathbf{Z})$, for any $v,u\in\R$,
\begin{equation*}
    \frac{1}{n}\sum_{i=1}^n \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i)\widehat{\psi}_{\x,n}(u;\mathbf{Z}_i) - \E[\bar{\psi}_{\x,n}(v;\mathbf{Z})\bar{\psi}_{\x,n}(u;\mathbf{Z})] = o_{\P}(1).
\end{equation*}
Also, $n^{-1}\sum_{i=1}^n \widehat{\psi}_{\x,n}^4(v;\mathbf{Z}_i)=O_{\P}(1)$, verifying the hypothesis of the lemma.
...
\begin{align*}
    &\E\left[ \left. \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \left| \frac{1}{\sqrt{n}} \sum_{i=1}^n W_{i,n}\left[  \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i)-\widehat{\psi}_{\x,n}(v';\mathbf{Z}_i)\right] \right| \right| \{\mathbf{Z}_i\}_{i=1}^n\right] \\
    &\leq C \frac{\sqrt{a_n}}{n}\sum_{i=1}^n \left[\bar{D}_{\gamma}^{\eta_n}(\mathbf{Z}_i) + \bar{D}_{\phi}^{\eta_n}(\mathbf{Z}_i)\right] (n_0-1) \frac{1}{\sqrt{n}}\E\max_{1\leq i \leq n} |W_{i,n}| \\
    & + C \max_{n_0\leq k \leq n} \E\left[ \left. \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \left| \frac{1}{\sqrt{k}} \sum_{i=n_0}^k \left[ \widehat{\psi}_{\x,n}(v;\mathbf{Z}_{R_i}) -\widehat{\psi}_{\x,n}(v';\mathbf{Z}_{R_i}) \right] \right| \right| \{\mathbf{Z}_i\}_{i=1}^n \right]
\end{align*}
where $(R_1,\dots,R_n)$ is uniformly distributed on the set of all permutations of $\{1,\dots,n\}$, independent of $\{\mathbf{Z}_i\}_{i=1}^n$. Choose $n_0$ such that $n^{1/2-1/\mathfrak{r}}/n_0\to \infty$ and $n_0/a_n\to \infty$ (which is possible by $\mathfrak{r}>(4\q+2)/(2\q-1)$), and the first term after the inequality in the above display is $o_{\P}(1)$. For the second term, following the argument of \citet[Theorem 3.6.13]{vanderVaart-Wellner_1996_Book}, it suffices to bound
\begin{equation*}
    \max_{n_0\leq k \leq n} \E_n^* \left[ \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \left| \frac{1}{\sqrt{k}} \sum_{i=1}^k \left[ \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i^*)-\widehat{\psi}_{\x,n}(v';\mathbf{Z}_i^*)\right] \right| \right], 
\end{equation*}
where $\{\mathbf{Z}_i^*\}_{i=1}^k$ denotes a random sample from the empirical cdf and $\E_n^*$ is the expectation under this empirical bootstrap law. Following the argument of \citet[Lemma 4.6]{Kim-Pollard_1990_AoS}, it suffices to show that
\begin{equation*}
    \max_{n_0\leq k \leq n} \E_n^* \left[ \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{k} \sum_{i=1}^k \left| \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i^*)-\widehat{\psi}_{\x,n}(v';\mathbf{Z}_i^*)\right|^2 \right] = o_{\P}(1).
\end{equation*} 
For $k\in \{n_0,\dots,n\}$ and $M>0$,
\begin{align*}
    &\E_n^* \left[ \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{k} \sum_{i=1}^k \left| \widehat{\psi}_{\x,n}(v;\mathbf{Z}_i^*)-\widehat{\psi}_{\x,n}(v';\mathbf{Z}_i^*) \right| ^2 \right] \\
    &\leq 2\E_n^* \left[ \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{k} \sum_{i=1}^k \left| \bar{\psi}_{\x,n}(v;\mathbf{Z}_i^*)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i^*) \right| ^2 \right] \\
    &\quad + 2 \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \left\vert\frac{1}{n}\sum_{i=1}^n \bar{\psi}_{\x,n}(v;\mathbf{Z}_i)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i)\right\vert^2
\end{align*}
where the second term after the inequality is shown to be $o_{\P}(1)$ in the non-bootstrap case above. For the first term after the inequality,
\begin{align*}
    &\E_n^* \left[ \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{k} \sum_{i=1}^k \left| \bar{\psi}_{\x,n}(v;\mathbf{Z}_i^*)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i^*) \right| ^2 \right]\\
    &\leq 4(1+|\theta_0(\x)|)^2 \frac{a_n}{n}\sum_{i=1}^n \left[ \bar{D}_{\gamma}^{\eta_n}(\mathbf{Z}_i) + \bar{D}_{\phi}^{\eta_n}(\mathbf{Z}_i)\right]^2 \1(\bar{D}_{\gamma}^{\eta_n}(\mathbf{Z}_i) + \bar{D}_{\phi}^{\eta_n}(\mathbf{Z}_i) > M) \\
    &+ \bar{M} \sqrt{a_n} \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{n}\sum_{i=1}^n \left| \bar{\psi}_{\x,n}(v;\mathbf{Z}_i)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i) \right| \\
    & + \bar{M} \sqrt{a_n} \E_n^* \left[ \sup_{\substack{|v-v'|\leq \epsilon_n\\ |v|\lor |v'|\leq K}} \frac{1}{k} \sum_{i=1}^k \left| \bar{\psi}_{\x,n}(v;\mathbf{Z}_i^*)-\bar{\psi}_{\x,n}(v';\mathbf{Z}_i^*) \right| - \E_n^* \left[ \left| \bar{\psi}_{\x,n}(v;\mathbf{Z}^*) - \bar{\psi}_{\x,n}(v';\mathbf{Z}^*) \right| \right] \right]
\end{align*}
where $\bar{M}=2M(1+|\theta_0(\x)|)$.
...
\begin{equation*}
    \sqrt{\frac{a_n}{k}} \sqrt{\frac{a_n}{n} \sum_{i=1}^n \left[ \bar{D}_{\gamma}^{\eta_n}(\mathbf{Z}_i)^2 + \bar{D}_{\phi}^{\eta_n}(\mathbf{Z}_i)^2 \right] } = O_{\P}\left( \sqrt{a_n/k} \right),
\end{equation*}
...
\begin{align*}
    ...
    &= \frac{1}{n}\sum_{i=1}^n W_{i,n} \Breve{\gamma}_n(v;\mathbf{Z}) + \left[ \check{\Gamma}_n(\x+va_n^{-1}) - \check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x) \right]
\end{align*}
where 
\begin{align*}
    \Breve{\gamma}_n(v;\mathbf{Z}) &= \widehat{\gamma}_n(\x+va_n^{-1};\mathbf{Z}_i)-\widehat{\gamma}_n(\x;\mathbf{Z}_i) - \gamma_0(\x+va_n^{-1};\mathbf{Z}_i)+\gamma_0(\x;\mathbf{Z}_i)\\
    &\qquad - \check{\Gamma}_n(\x+va_n^{-1}) + \check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x).
\end{align*}
By Assumption \ref{M-Assumption E: Gamma hat},
\begin{equation*}
    \sqrt{n a_n} \sup_{|v|\leq K} | \check{\Gamma}_n(\x+va_n^{-1}) - \check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x) | = o_{\P}(1).
\end{equation*}
...
\begin{align*}
    &\sqrt{n a_n} \E\left[ \left. \sup_{ |v|\leq K}  \left| \frac{1}{n}\sum_{i=1}^n W_{i,n} \Breve{\gamma}_n(v;\mathbf{Z}_i) \right| \right| \{\mathbf{Z}_i\}_{i=1}^n \right] \\
    \label{Appendix eq: high-level bootstrap approximation remainder 1}
    &\leq C  \frac{\sqrt{a_n}}{n} \sum_{i=1}^n \sup_{ |v|\leq K} |\Breve{\gamma}_n(v;\mathbf{Z}_i) | \frac{n_0 n^{\mathfrak{r}}}{\sqrt{n}} +  C \sqrt{a_n}\max_{n_0\leq k\leq n} \E_n^* \left[ \sup_{ |v|\leq K} \left| \frac{1}{\sqrt{k}} \sum_{i=1}^k \Breve{\gamma}_n(v;\mathbf{Z}_i^*) \right| \right].
\end{align*}
By Assumption \ref{M-Assumption E: Gamma hat},
\begin{align*}
    & \frac{\sqrt{a_n}}{n} \sum_{i=1}^n \sup_{ |v|\leq K} |\Breve{\gamma}_n(v;\mathbf{Z}_i) |\\
    &\leq \frac{\sqrt{a_n}}{n} \sum_{i=1}^n \sup_{ |v|\leq K} |\widehat{\gamma}_n(\x+va_n^{-1};\mathbf{Z}_i)-\widehat{\gamma}_n(\x;\mathbf{Z}_i) -\gamma_0(\x+va_n^{-1};\mathbf{Z}_i) + \gamma_0(\x;\mathbf{Z}_i) | \\
    &\quad + \sqrt{a_n} \sup_{ |v|\leq K} \left|\check{\Gamma}_n(\x+va_n^{-1}) - \check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x) \right| = o_{\P}(1).
\end{align*}
Also, Corollary 4.3 of \cite{Pollard_1989_SS} implies that for some fixed $C>0$,
\begin{align*}
    &\E_n^*\left[ \sup_{ |v|\leq K} \left|\frac{1}{\sqrt{k}}\sum_{i=1}^k \Breve{\gamma}_n(v;\mathbf{Z}_i^*)  \right| \right]^2 \\
    &\leq C \frac{1}{n}\sum_{i=1}^n\sup_{ |v|\leq K} |\widehat{\gamma}_n(\x+va_n^{-1};\mathbf{Z}_i)-\widehat{\gamma}_n(\x;\mathbf{Z}_i) -\gamma_0(\x+va_n^{-1};\mathbf{Z}_i) + \gamma_0(\x;\mathbf{Z}_i) |^2,
\end{align*}
which is $o_{\P}(a_n^{-1})$ by  Assumption  \ref{M-Assumption E: Gamma hat}.

\subsubsection*{Verifying Assumption \ref{M-Assumption B: moment bound}}
Defining
\begin{align*}
    \bar{G}_{\x,n}^{\q}(v)
    &= \sqrt{n a_n} \left[ \bar{\Gamma}_n(\x +v a_n^{-1} ) - \bar{\Gamma}_n(\x) - \Gamma_0(\x+v a_n^{-1}) + \Gamma_0(\x) \right]  \\
    &\quad -\theta_0(\x) \sqrt{n a_n} \left[ \bar{\Phi}_n(\x +v a_n^{-1} ) - \bar{\Phi}_n(\x) - \Phi_0(\x+v a_n^{-1}) + \Phi_0(\x) \right],
\end{align*}
we have
\begin{align*}
    \widehat{G}_{\x,n}^{\q}(v) - \bar{G}_{\x,n}^{\q}(v)
    &= \sqrt{n a_n} \left[ \widehat{\Gamma}_n(\x+va_n^{-1}) -\widehat{\Gamma}_n(\x) - \bar{\Gamma}_n(\x +va_n^{-1} ) + \bar{\Gamma}_n(\x) \right] \\
    &\quad - \theta_0(\x) \sqrt{n a_n} \left[ \widehat{\Phi}_n(\x+va_n^{-1}) -\widehat{\Phi}_n(\x) - \bar{\Phi}_n(\x +va_n^{-1} ) + \bar{\Phi}_n(\x) \right].
\end{align*}
Assumptions \ref{M-Assumption E: Gamma hat} and \ref{M-Assumption E: Phi hat} imply that for $V \in [1,a_n \delta]$,
\begin{equation*}
    \sqrt{n a_n}\sup_{|v|\in [V,2V]} | \widehat{\Gamma}_n(\x+v a_n^{-1}) -\widehat{\Gamma}_n(\x) - \bar{\Gamma}_n(\x +v a_n^{-1}) + \bar{\Gamma}_n(\x) | \leq (2V)^{\beta} a_n^{-\beta}B_n+A_n
\end{equation*}
and
\begin{equation*}
    \sqrt{n a_n}\sup_{|v|\in [V,2V]} | \widehat{\Phi}_n(\x+v a_n^{-1}) -\widehat{\Phi}_n(\x) - \bar{\Phi}_n(\x +v a_n^{-1}) + \bar{\Phi}_n(\x) | \leq (2V)^{\beta} a_n^{-\beta}B_n+A_n,
\end{equation*}
where $\beta = \beta_\gamma \lor \beta_{\phi} , A_n = A_{\gamma,n} \lor A_{\phi,n} = o_{\P}(1)$, and $B_n = B_{\gamma,n} \lor B_{\phi,n} = o_{\P}(a_n^{\beta})$. As a consequence, there exists $\eta_n' = o(1)$ such that
\begin{equation*}
    \lim_{n\to\infty} \P\left[\bigcap_{V \in [1,a_n\delta]} \left\{ \sup_{| v|\in [V,2V]}|\widehat{G}_{\x,n}^{\q}(v) - \bar{G}_{\x,n}^{\q}(v) | \leq [V^{\beta}+1]\eta_n' \right\}\right] = 1;
\end{equation*}
we take the event in the display to be $\mathcal{A}_n$. Also,
\begin{equation*}
    \limsup_{\eta \downarrow 0}\E[\bar{D}_{\gamma}^{\eta}(\mathbf{Z})^2/\eta] + \E[\bar{D}_{\phi}^{\eta}(\mathbf{Z})^2/\eta] < \infty
\end{equation*}
and, using Corollary 4.3 of \cite{Pollard_1989_SS},
\begin{equation*}
    \E \left[ \sup_{| v|\leq V}|\bar{G}_{\x,n}^{\q}(v)| \right] \leq C \sqrt{V} \quad \text{for } V \in (0,a_n\delta].
\end{equation*}
Therefore,
\begin{equation*}
    \E \left[ \sup_{| v| \in [V,2V]}| \widehat{G}_{\x,n}^{\q}(v)|\1_{\mathcal{A}_n} \right] \leq C \sqrt{V} + [V^{\beta}+1]\eta_n',
\end{equation*}
...
\begin{align*}
    \bar{G}_{\x,n}^{\q,*}(v) &= \sqrt{n a_n}\left[ \bar{\Gamma}_n^*(\x +v a_n^{-1} ) - \bar{\Gamma}_n^*(\x) - \bar{\Gamma}_n(\x +v a_n^{-1} ) + \bar{\Gamma}_n(\x) \right]  \\
    &\quad  -\theta_0(\x) \sqrt{n a_n} \left[ \bar{\Phi}_n^*(\x +v a_n^{-1} ) - \bar{\Phi}_n^*(\x) - \bar{\Phi}_n(\x +v a_n^{-1} ) + \bar{\Phi}_n(\x) \right],
\end{align*}
we have
\begin{align}
\label{Appendix eq: verifying bootstrap rate of convergence}
    ...
    &= \sqrt{n a_n} \left[ \widehat{\Gamma}_n^*(\x+va_n^{-1}) -\widehat{\Gamma}_n^*(\x) - \bar{\Gamma}_n^*(\x +va_n^{-1}) + \bar{\Gamma}_n^*(\x) \right] \\
    \nonumber
    & - \widehat{\theta}_n(\x)\sqrt{n a_n} \left[ \widehat{\Phi}_n^*(\x+va_n^{-1}) -\widehat{\Phi}_n^*(\x) - \bar{\Phi}_n^*(\x +va_n^{-1}) + \bar{\Phi}_n^*(\x) \right] \\
    \nonumber
    & + \sqrt{n a_n} \left[ \bar{\Gamma}_n(\x +va_n^{-1}) - \bar{\Gamma}_n(\x) - \widehat{\Gamma}_n(\x+va_n^{-1}) + \widehat{\Gamma}_n(\x) \right] \\
    \nonumber
    & - \widehat{\theta}_n(\x)\sqrt{n a_n} \left[ \bar{\Phi}_n(\x +va_n^{-1}) - \bar{\Phi}_n(\x) - \widehat{\Phi}_n(\x+va_n^{-1}) + \widehat{\Phi}_n(\x) \right] \\
    \nonumber
    & +\sqrt{a_n}[\widehat{\theta}_n(\x) - \theta_0(\x)] \sqrt{n} \left[ \bar{\Phi}_n^*(\x + va_n^{-1}) - \bar{\Phi}_n^*(\x) - \bar{\Phi}_n(\x +va_n^{-1}) + \bar{\Phi}_n(\x) \right],
\end{align}
where bounds for the third and fourth terms were derived in the non-bootstrap case and where the last term is $o_{\P}(1)$ uniformly over $| v| \leq a_n \delta$ as $\sqrt{a_n}[\widehat{\theta}_n(\x) - \theta_0(\x)]=o_{\P}(1)$ and $\sqrt{n} \sup_{| v|\leq \delta}|\bar{\Phi}_n^*(\x + v) - \bar{\Phi}_n(\x +v ) |=O_{\P}(1)$. For the first term after the equality in \eqref{Appendix eq: verifying bootstrap rate of convergence},
\begin{align*}
    & \sqrt{na_n} \left[ \widehat{\Gamma}_n^*(\x+va_n^{-1}) -\widehat{\Gamma}_n^*(\x) - \bar{\Gamma}_n^*(\x +v a_n^{-1}) + \bar{\Gamma}_n^*(\x) \right] \\
    &= \frac{1}{\sqrt{n}}\sum_{i=1}^n W_{i,n}\Breve{\gamma}_n(va_n^{-1};\mathbf{Z}_i) + \sqrt{na_n} \left[ \check{\Gamma}_n(\x+va_n^{-1}) - \check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x) \right]
\end{align*}
where, by Assumption \ref{M-Assumption E: Gamma hat},
\begin{equation*}
    \sqrt{n a_n}\sup_{|v|\leq V } | \check{\Gamma}_n(\x+va_n^{-1}) - \check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+va_n^{-1}) + \bar{\Gamma}_n(\x)| =[1+ V^{\beta}]o_{\P}(1)
\end{equation*}
uniformly over $V\in [1,a_n\delta]$ and where, applying Lemma 3.6.7 of \cite{vanderVaart-Wellner_1996_Book}, we have, for any $n_0\in \{1,\dots, n\}$,
\begin{align*}
    &\E\left[ \left. \sup_{| v|\leq \delta} \left| \frac{1}{\sqrt{n}}\sum_{i=1}^nW_{i,n}\Breve{\gamma}_n(v;\mathbf{Z}_i) \right| \right| \{\mathbf{Z}_i\}_{i=1}^n \right] \\
    \label{Appendix eq: verifying bootstrap moment bound}
    &\leq C \frac{1}{n}\sum_{i=1}^n \sup_{| v| \leq \delta} |\Breve{\gamma}_n(v;\mathbf{Z}_i)| \frac{n_0}{\sqrt{n}} \E \left[ \max_{1\leq i\leq n} | W_{i,n}| \right] + C \max_{n_0\leq k\leq n}\E_n^* \left[\sup_{| v|\leq \delta} \left| \frac{1}{\sqrt{k}} \sum_{i=1}^k \Breve{\gamma}_n(v;\mathbf{Z}_i^*) \right| \right],
\end{align*}
...
\begin{equation*}
    C\frac{1}{n}\sum_{i=1}^n \sup_{| x-\x|\leq \delta} | \widehat{\gamma}_n(x;\mathbf{Z}_i) - \gamma_0(x;\mathbf{Z}_i) | + C\sup_{| v|\leq \delta } | \check{\Gamma}_n(\x+v) - \bar{\Gamma}_n(\x+v)  | =o_{\P}(1),
\end{equation*}
and, by Corollary 4.3 of \cite{Pollard_1989_SS}, the second term is bounded (up to a constant) by
\begin{equation*}
    \sqrt{a_n} \left| \frac{1}{n}\sum_{i=1}^n \sup_{x\in I_{\x}^{\delta} } | \widehat{\gamma}_n(x;\mathbf{Z}_i)- \gamma_0(x;\mathbf{Z}_i) |^2 \right| ^{1/2} 
\end{equation*}
which is $o_{\P}(1)$ by Assumption \ref{M-Assumption E: Gamma hat}.
As a consequence, there exists a sequence of random variables $A_n'=o_{\P}(1)$ such that for $V\in [1,a_n\delta]$,
\begin{equation*}
    \sqrt{n a_n}\sup_{|v|\leq [V,2V]} \left| \widehat{\Gamma}_n^*(\x+va_n^{-1}) -\widehat{\Gamma}_n^*(\x) - \bar{\Gamma}_n^*(\x +va_n^{-1}) + \bar{\Gamma}_n^*(\x) \right| \leq  [V^{\beta}+1]A_n'.
\end{equation*}
By identical arguments, an analogous bound holds for the second term after the inequality in \eqref{Appendix eq: verifying bootstrap rate of convergence}. Therefore, there exists $\eta_n'=o(1)$ and events $\mathcal{A}_n$ such that $\lim_{n\to\infty}\P[\mathcal{A}_n]=1$ and
\begin{equation*}
    \E\left[\sup_{|v|\in [V,2V]} \left| \widehat{G}_{\x,n}^{\q,*}(v) - \bar{G}_{\x,n}^{\q,*}(v) \right| \1_{\mathcal{A}_n} \right]\leq [V^{\beta}+1]\eta_n'
\end{equation*}
for any $V\in [1,a_n\delta]$. Finally, proceeding as in the proof of stochastic equicontinuity, we obtain the bound $\E[\sup_{| v|\in [V,2V] }|\bar{G}_{\x,n}^{\q,*}(v)|]\leq C\sqrt{V}$.
...
We have
\begin{equation*}
    \sup_{x\in I} \left| \widehat{\Gamma}_n(x)-\Gamma_0(x) \right| \leq \sup_{x\in I} \left| \widehat{\Gamma}_n(x)-\bar{\Gamma}_n(x) \right| + \sup_{x\in I} \left| \bar{\Gamma}_n(x)-\Gamma_0(x) \right|,
\end{equation*}
where the first term after the inequality is assumed to be $o_{\P}(1)$ and the second term is $o_{\P}(1)$ by standard arguments. Similarly, $\sup_{x\in I}|\widehat{\Phi}_n(x)-\Phi_0(x)|=o_{\P}(1)$. Also,
\begin{align*}
    \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n(x) - \Phi_0(x) \right| 
    &\leq \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n(x) - \widehat{\Phi}_n(\x) - \bar{\Phi}_n(x) +\bar{\Phi}_n(\x) \right| \\
    &\quad +\sup_{x\in I_{\x}^{\delta}} \left| \bar{\Phi}_n(x)-\Phi_0(x) \right| + \left| \widehat{\Phi}_n(\x) - \bar{\Phi}_n(\x) \right| 
\end{align*}
where the last two terms are $o_{\P}(a_n^{-1})$ and where Assumption \ref{M-Assumption E: Phi hat} implies
\begin{equation*}
    \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n(x) - \widehat{\Phi}_n(\x) - \bar{\Phi}_n(x) +\bar{\Phi}_n(\x) \right| \leq (na_n)^{-1/2} [ A_{\phi,n}+ \delta^{\beta_{\phi}} B_{\phi,n}]  = o_{\P}(a_n^{-1}),
\end{equation*}
the last equality using $B_{\phi,n}=o_{\P}(a_n^{\beta_{\phi}})$ and $\beta_{\phi}\leq \q$.

Now we look at the bootstrap objects. For $\widehat{\Gamma}_n^*$,
\begin{equation*}
    \sup_{x\in I} \left| \widehat{\Gamma}_n^*(x) - \widehat{\Gamma}_n(x) \right| \leq \sup_{x\in I} \left| \widehat{\Gamma}_n^*(x) - \bar{\Gamma}_n^*(x) \right| + \sup_{x\in I} \left| \bar{\Gamma}_n^*(x) -\bar{\Gamma}_n(x) \right| + \sup_{x\in I} \left| \bar{\Gamma}_n(x) - \widehat{\Gamma}_n(x) \right|
\end{equation*}
where the last term is assumed to be $o_{\P}(1)$ and where
\begin{align*}
    \sup_{x\in I} \left| \widehat{\Gamma}_n^*(x) - \bar{\Gamma}_n^*(x) \right|
    &\leq \frac{1}{n}\sum_{i=1}^n |W_{i,n}| \sup_{x\in I} \left| \widehat{\gamma}_n(x;\mathbf{Z}_i) - \gamma_0(x;\mathbf{Z}_i) \right| \\
    &\leq \sqrt{\frac{1}{n}\sum_{i=1}^n |W_{i,n}|^2 \frac{1}{n}\sum_{i=1}^n  \sup_{x\in I} \left| \widehat{\gamma}_n(x;\mathbf{Z}_i) - \gamma_0(x;\mathbf{Z}_i) \right|^2 } = o_{\P}(1).
\end{align*}
For $\bar{\Gamma}_n^* -\bar{\Gamma}_n$, using Lemma 3.6.7 of \cite{vanderVaart-Wellner_1996_Book} and the same argument as for verifying Assumption \ref{M-Assumption B: weak convergence}, it suffices to show that
\begin{equation*}
    n^{-1/2} \max_{\lfloor\sqrt{n}\rfloor\leq k\leq n} \E_n^* \left[ \sup_{x\in I} \left| \frac{1}{\sqrt{k}} \sum_{i=1}^k \bar{\gamma}_n(x;\mathbf{Z}_i^*) \right| \right] = o_{\P}(1),
\end{equation*}
as can be done using Corollary 4.3 of \cite{Pollard_1989_SS}.

For $\widehat{\Phi}_n^*$, $\sup_{x\in I}|\widehat{\Phi}_n^*(x)-\widehat{\Phi}_n(x)|=o_{\P}(1)$ follows from the same argument as for $\widehat{\Gamma}_n^*$. For $a_n \sup_{x\in I_{\x}^{\delta}}|\widehat{\Phi}_n^*(x) -\widehat{\Phi}_n(x)|=o_{\P}(1)$,
\begin{align*}
    \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n^*(x) -\widehat{\Phi}_n(x) \right| 
    &\leq \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n^*(x) -\bar{\Phi}_n^*(x) \right| +\sup_{x\in I_{\x}^{\delta}} \left| \bar{\Phi}_n^*(x) - \bar{\Phi}_n(x) \right| \\
    &\quad + \sup_{x\in I_{\x}^{\delta}} \left| \bar{\Phi}_n(x) - \Phi_0(x) \right| + \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n(x) - \Phi_0(x) \right|
\end{align*}
where the last term is $o_{\P}(a_n^{-1})$ as shown above and the second and third terms after the inequality are $O_{\P}(n^{-1/2})$ by standard arguments. For the remaining term,
\begin{align*}
    \sup_{x\in I_{\x}^{\delta}} \left| \widehat{\Phi}_n^*(x) -\bar{\Phi}_n^*(x) \right| 
    &\leq \sup_{x\in I_{\x}^{\delta}} \left| \frac{1}{n}\sum_{i=1}^n W_{i,n}\Breve{\phi}_n(x;\mathbf{Z}_i) \right| + \left| \check{\Phi}_n(\x) - \bar{\Phi}_n(\x) \right| \\
    &\quad + \sup_{x\in I_{\x}^{\delta}} \left| \check{\Phi}_n(x) - \check{\Phi}_n(\x) - \bar{\Phi}_n(x) + \bar{\Phi}_n(\x) \right|
\end{align*}
where $\Breve{\phi}_n(x;\mathbf{Z}) = \widehat{\phi}_n(x;\mathbf{Z})-\phi_0(x;\mathbf{Z}) - [\check{\Phi}_n(x)-\bar{\Phi}_n(x)]$. The last two terms are $o_{\P}(a_n^{-1})$ by Assumption \ref{M-Assumption E: Phi hat}. Using Lemma 3.6.7 of \cite{vanderVaart-Wellner_1996_Book} and the argument similar to above, the remaining term is $o_{\P}(a_n^{-1})$.
...
\begin{equation*}
    r_n \left( \widehat{\theta}_n^*(\x) - \widehat{\theta}_n(\x) \right) \leadsto_{\P} \frac{1}{\partial \Phi_0(\x)} \partial_{-}\GCM_{\R}\{\mathcal{G}_{\x}+\mathcal{M}_{\x}^{\q}\}(0) = Y.
\end{equation*}
Then, by Theorem 2.2 of \cite{Kosorok_2008_BookCh}, we have 
\begin{equation}\label{eqBootFail:contradiction}
    r_n \left( \widehat{\theta}_n^*(\x) - \theta_0(\x) \right) \leadsto Y_1 + Y_2 =_d \sqrt{2} Y
\end{equation}
...
Using the switching lemma, $\P \left[r_n \left( \widehat{\theta}_n^*(\x) - \theta_0(\x) \right)  > t \right]$ equals
\begin{equation*}
    \P \left[ \max \argmax_{x\in \widehat{\Phi}_n^{*-} ([0,\widehat{u}_n^*]) } \left\{ [\theta_0(\x) + r_n^{-1}t]\widehat{\Phi}_n^*(x) - \LSC_{\widehat{\Phi}_n^{*}} \left( \widehat{\Gamma}_n^* \right) (x) \right\} < \widehat{\Phi}_n^{*-} \circ \widehat{\Phi}_n^*(\x) \right].
\end{equation*}
...
where 
\begin{align*}
    \check{G}_{\x,n}^{\q,*}(v) &= \sqrt{na_n} \left[ \widehat{\Gamma}_n^*(\x+va_n^{-1})  - \widehat{\Gamma}_n^*(\x) - \check{\Gamma}_n (\x+va_n^{-1}) + \check{\Gamma}_n (\x) \right] \\
    &\quad - \theta_0(\x)\sqrt{na_n} \left[ \widehat{\Phi}_n^*(\x+va_n^{-1}) - \widehat{\Phi}_n^*(\x)-\check{\Phi}_n(\x+va_n^{-1}) + \check{\Phi}_n(\x) \right]
\end{align*}
and
\begin{equation*}
    \check{M}_{\x,n}^{\q}(v)=\sqrt{n a_n} \left[ \check{\Gamma}_n(\x+va_n^{-1}) -\check{\Gamma}_n(\x) - \theta_0(\x)\{ \check{\Phi}_n(\x+va_n^{-1}) -\check{\Phi}_n(\x)\} \right].
\end{equation*}
...
\begin{equation*}
    \P \left[ r_n \left( \widehat{\theta}_n^*(\x) - \theta_0(\x) \right) >t \right] \to \P \left[ \argmin_{v\in \R} \left\{ \mathcal{G}_{\x,1}(v) + \mathcal{G}_{\x,2}(v) + \mathcal{M}_{\x}^{\q}(v)  - t \partial\Phi_0(\x) v \right\} < 0 \right],
\end{equation*}
...
\begin{align*}
    &\P \left[ 2^{\frac{1}{2\q+1}} \argmin_{u\in \R} \left\{   \mathcal{G}_{\x}(u) + \mathcal{M}_{\x}^{\q}(u) -  2^{-\frac{\q}{2\q+1}} t \partial\Phi_0(\x)u \right\} < 0 \right] \\
    &= \P \left[ 2^{\frac{\q}{2\q+1}} (\partial\Phi_0(\x))^{-1}  \partial_{-} \GCM_{\R} \{\mathcal{G}_{\x}+\mathcal{M}_{\x}^{\q}\}(0) >  t \right].
\end{align*} 
As a consequence,
\begin{equation*}
    r_n \left( \widehat{\theta}_n^*(\x) - \theta_0(\x) \right) \leadsto 2^{\frac{\q}{2\q+1}} (\partial \Phi_0(\x))^{-1} \partial_{-}\GCM_{\R}\{\mathcal{G}_{\x}+\mathcal{M}_{\x}^{\q}\}(0),
\end{equation*}
...
Below we verify the hypothesis of Theorem \ref{M-[Theorem] Main result} for various examples. For this purpose, it suffices to verify Assumptions \ref{M-Assumption A}, \ref{M-Assumption B: uhat}-\ref{M-Assumption B: jump of Phi hat}, and \ref{M-Assumption E} since Assumption \ref{M-Assumption E} implies \ref{M-Assumption B: weak convergence}-\ref{M-Assumption B: uniform convergence Phi} by Lemma \ref{M-[Lemma] Bootstrapping}.
...
\newline
\ref{M-Assumption E: covariance kernel}: 
...
\begin{align*}
    ...
    &= f_0(\x) (|s| \land |t|)\1(\sign(s)=\sign(t)) + o(1).
\end{align*}
...
Let $\delta = \min\{\x, (u_0-\x)\}/4$, $R_{1n}(v)=| \widehat{F}_n(\x+v)-\widehat{F}_n(\x) - F_0(\x+v)+F_0(\x)|$, $R_{2n}=| \widehat{F}_n(\x)-F_0(\x)|$, $R_{3n}= \sup_{x\in I}| [\widehat{S}_n(x)\widehat{G}_n(x)]^{-1} - [S_0(x)G_0(x)]^{-1} |$, and $R_{4n}(x_1,x_2) = |\int_{x_1}^{x_2} \frac{\widehat{\Lambda}_n(du)}{\widehat{S}_n(u)\widehat{G}_n(u)} -\int_{x_1}^{x_2} \frac{\Lambda_0(du)}{S_0(u)G_0(u)}| $. For $|v|\leq \delta$,
\begin{align*}
    &\big|\check{\Gamma}_n(\x+v)-\check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+v) + \bar{\Gamma}_n(\x)\big| \\
    ...
    &+ R_{2n} \int_{\x-v}^{\x+v} \frac{\widehat{\Lambda}_n(du)}{\widehat{S}_n(u)\widehat{G}_n(u)} + \sup_{x\in I_{\x}^{\delta}} f_0(x)| v | \bigg[R_{3n} +  \frac{1}{n}\sum_{i=1}^n R_{4n}(0,\check{X}_i\land (\x+v) )   \bigg] \\
    & +  R_{3n} \frac{1}{n}\sum_{i=1}^n \1(|X_i-\x|\leq v ) + \frac{1}{n}\sum_{i=1}^n R_{4n}(\check{X}_i\land\x,\check{X}_i\land (\x+v) ).
\end{align*}
As noted above, $\sup_{|v|\leq \delta}R_{1n}(v)=o_{\P}( (na_n)^{-1/2})$, $R_{2n}=O_{\P}(n^{-1/2})$, and $R_{3n}=O_{\P}(n^{-1/2})$ by $S_0(u_0)G_0(u_0)>0$.
Also, uniformly over $V\in (0, 2\delta]$, $\sup_{| v|\leq V}| \frac{1}{n}\sum_{i=1}^n  (\1(X_i\leq \x +v) - \1(X_i\leq \x) )| \leq C V + O_{\P}(n^{-1/2})$. If 
\begin{equation}\label{Appendix: Example monotone density right-censoring R3 1}
    \sup_{|v|\leq \delta}\frac{1}{n}\sum_{i=1}^n R_{4n}(0,\check{X}_i\land (\x+v) )=O_{\P}(n^{-1/2})
\end{equation}
and 
\begin{equation}\label{Appendix: Example monotone density right-censoring R3 2}
    \frac{1}{n}\sum_{i=1}^n R_{4n}(\check{X}_i\land\x,\check{X}_i\land (\x+v) ) \leq |v| O_{\P}(n^{-1/2}) + o_{\P}\Big(( na_n)^{-1/2} \Big)
\end{equation}
uniformly over $|v|\leq 2\delta$, then there exist random variables $A_n=o_{\P}(1)$ and $B_n=O_{\P}(\sqrt{a_n})$ independent of $v$ such that for $V\in (0, 2\delta]$,
\begin{equation*}
    \sqrt{na_n} \sup_{|v|\leq V} \big|\check{\Gamma}_n(\x+v)-\check{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+v) + \bar{\Gamma}_n(\x)\big| \leq A_n + V B_n;
\end{equation*}
that is,\ $\beta_{\gamma}=1$ in the notation of \ref{M-Assumption E: Gamma hat}. To show \eqref{Appendix: Example monotone density right-censoring R3 1} and \eqref{Appendix: Example monotone density right-censoring R3 2}, for $x_1,x_2\in I$,
\begin{equation*}
...
\end{align*}
where the last equality uses continuity of $(S_0,G_0,f_0)$ at $\x$ i.e.,\ $\int_{\x_n}^{\x_n +\eta_n} [\frac{f_0(u)}{S_0(u)^2G_0(u)}-\frac{f_0(\x)}{S_0(\x)^2G_0(\x)}]du =o(1)\eta_n$.
...
As noted when verifying \ref{M-Assumption E: Gamma hat}, $\widehat{G}_n(1;\eta_n)=o_{\P}(1)$ for any $\eta_n=o(1)$ with $a_n^{-1}\eta_n^{-1}=O(1)$. $\widehat{\Phi}_n=\Phi_0$ is the identity map and the desired result holds.
...
\begin{align*}
   ...
   &= \sigma_0^2(\x)f_0(\x) (|s|\land |t|) \1(\sign(s)=\sign(t)),
\end{align*}
as desired.
...
\noindent \ref{M-Assumption B: Phi uhat}:
...
\noindent \ref{M-Assumption B: jump of Phi hat}:
With probability one, all $X_i$'s are distinct. If $x$ is one of $X_i$'s, $\widehat{\Phi}_n^*( x ) - \widehat{\Phi}_n^*( x- ) = n^{-1}W_{j,n}$ for some $j$, and $\widehat{\Phi}_n^*( x ) - \widehat{\Phi}_n^*( x- ) =0$ otherwise.

Given $\E|W_{1,n}|^{\mathfrak{r}}<\infty$, $n^{-1}\max_{1\leq i\leq n}|W_{i,n}|=o_{\P}(n^{-5/6})$, which implies $$\sqrt{na_n}\sup_{x\in I}|\widehat{\Phi}_n^*( x ) - \widehat{\Phi}_n^*( x- ) |=o_{\P}(1).$$ The argument for $\widehat{\Phi}_n$ is similar.
\smallskip
\newline
...
\noindent \ref{M-Assumption E: Gamma hat}:
...
\begin{equation*}
    \sqrt{na_n}\sup_{|v|\leq V}\big\vert\widehat{\Gamma}_n(\x+v)-\widehat{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+v)+\bar{\Gamma}_n(\x)\big\vert \leq o_{\P}(1) + O_{\P}(\sqrt{a_n}) V
\end{equation*}
...
\noindent \ref{M-Assumption B: uhat} \ref{M-Assumption B: Phi uhat} \ref{M-Assumption B: jump of Phi hat}:
Verifying these conditions is the same as in the classical monotone regression case.
...
\begin{equation*}
    \widehat{f}_{X|\mathbf{A},n}(x|\mathbf{a}) = \mathbf{e}_2' \argmin_{\mathbf{u}\in\R^{\mathfrak{p}_1+1}}\sum_{i=1}^n\Big(\widehat{F}_{X|\mathbf{A},n}(\tilde{X}_i|\mathbf{a}) - \mathbf{q}_1(\tilde{X}_i-x)'\mathbf{u}\Big)^2K_h(\tilde{X}_i-x) 
\end{equation*}
%\begin{equation*}
%    \widehat{f}_{X|\mathbf{A},n}(x|\mathbf{a}) = \mathbf{e}_2'\widehat{\boldsymbol{\beta}}_n(x|\mathbf{a}),\quad \widehat{\boldsymbol{\beta}}_n(x|\mathbf{a}) = \argmin_{\mathbf{u}\in\R^{\mathfrak{p}_1+1}}\sum_{i=1}^n\Big(\widehat{F}_{X|\mathbf{A},n}(\tilde{X}_i|\mathbf{a}) - \mathbf{q}_1(\tilde{X}_i-x)'\mathbf{u}\Big)^2K_h(\tilde{X}_i-x) 
%\end{equation*}
...
\begin{equation*}
    \widehat{F}_{X|\mathbf{A},n}(x|\mathbf{a}) =\mathbf{e}_1' \argmin_{\mathbf{v}\in\R^{k_{\mathfrak{p}_2}}}\sum_{i=1}^n \big(\1(\tilde{X}_i\leq x) - \mathbf{q}_2(\tilde{\mathbf{A}}_i-\mathbf{a})'\mathbf{v}\big)^2L_h(\tilde{\mathbf{A}}_i-\mathbf{a})
\end{equation*}
%\begin{equation*}
%    \widehat{F}_{X|\mathbf{A},n}(x|\mathbf{a}) =\mathbf{e}_1'\widehat{\boldsymbol{\gamma}}_n(x|\mathbf{a}),\quad \widehat{\boldsymbol{\gamma}}_n(x|\mathbf{a}) = \argmin_{\mathbf{v}\in\R^{k_{\mathfrak{p}_2}}}\sum_{i=1}^n \big(\1(\tilde{X}_i\leq x) - \mathbf{q}_2(\tilde{\mathbf{A}}_i-\mathbf{a})'\mathbf{v}\big)^2L_h(\tilde{\mathbf{A}}_i-\mathbf{a})
%\end{equation*}
...
\begin{equation*}
    \widehat{f}_n(x) =\mathbf{e}_2'\argmin_{\mathbf{w}\in\R^{\mathfrak{p}_1}+1} \sum_{i=1}^n\Big(\widehat{F}_n(\tilde{X}_i)-\mathbf{q}_1(\tilde{X}_i-x)'\mathbf{w}\Big)^2K_b(\tilde{X}_i-x)
\end{equation*}
...
\begin{lemma}
    ...
    \begin{equation}\label{Appendix eq: result - monotone regression with covariates primitive conditions}
        \sqrt{n} \sup_{|v|\leq V}\big\vert \widehat{\Gamma}_n(\x+v) - \widehat{\Gamma}_n(\x) - \bar{\Gamma}_n(\x+v) +\bar{\Gamma}_n(\x)\big\vert \leq V O_{\P}(1) + o_{\P}(a_n^{-1})
    \end{equation}
...
\end{proof}
\paragraph*{Remark}
Using \eqref{Appendix eq: result - monotone regression with covariates primitive conditions}, one can verify the first part of Assumption \ref{M-Assumption D} i.e.,\ $\widehat{G}_{\x,n}(1;\eta_n)=O_{\P}(1)$. The second part is easy to verify using standard empirical process theory arguments.
...
We consider the problem of estimating the density of a non-negative, continuously distributed random variable with censoring. We use the same notation as in Section \ref{M-[Subsection] Examples: Monotone Density Estimation} of the main paper. Relative to Section \ref{M-[Subsection] Examples: Monotone Density Estimation}, we consider the additional complication of censoring being informative about the ``survival time'' $X$ i.e.,\ $X\not\indep C$. With covariates $\mathbf{A}$, we consider the setting of censoring at random: $X \indep C|\mathbf{A}$. See \cite{vanderlaan-Robins_2003_Book,Zeng_2004_AoS} and references therein for existing analysis of this problem. We have
\begin{equation*}
...
\begin{enumerate}[label=\normalfont(\roman*),noitemsep,itemindent=*]
    ...
    There exist $\delta>0, \beta_{\gamma} \in [1/2,2)$ such that for $V \in (0,2\delta]$, $\sqrt{n a_n}\sup_{|v|\leq V}|\widehat{\Gamma}_n(\x+v)-\widehat{\Gamma}_n(\x)-\bar{\Gamma}_n(\x+v)+\bar{\Gamma}_n(\x)|\leq o_{\P}(1) +V^{\beta_{\gamma}}o_{\P}(a_n^{\beta_{\gamma}})$ where $o_{\P}$ terms do not depend on $V$.
    
    \item With probability approaching one, $\widehat{S}_n\in\mathfrak{S}_n$, $\widehat{G}_{n}\in\mathfrak{G}_n$, $\widehat{\Lambda}_n\in\mathfrak{L}_n$. For $(\widehat{h}_n,h_0) \in \{(\widehat{S}_n,S_0),(\widehat{G}_n,G_0),(\widehat{\Lambda}_n,\Lambda_0) \}$,
    \begin{equation*}
...
where $\widehat{F}_n=1-\widehat{S}_n$, 
    ...
    \end{equation*}
    \begin{equation*}
        \mathcal{C}_{\x}(s,t) = \E\Big[\frac{f_{X|\mathbf{A}}(\x|\mathbf{A})}{G_0(\x|\mathbf{A})}\Big] (|s| \land |t|) \1(\sign(s)=\sign(t)), \quad \mathcal{D}_{\q}(\x) = \frac{\partial^\q f_0(\x)}{(\q+1)!}. 
\end{equation*}
\end{corollary}
...
\newline 
\ref{M-Assumption E: Phi hat} trivially holds since $\widehat{\Phi}_n(x)=\widehat{\Phi}_n^*(x)=x$.
...
\end{align*}
and $\eta_n^{-1}\E[\psi_{\x_n}(s\eta_n;\mathbf{Z})\psi_{\x_n}(t\eta_n;\mathbf{Z})]$ converges to $\E[\frac{f_{X|\mathbf{A}}(\x|\mathbf{A})}{G_0(\x|\mathbf{A})}] (|s|\land|t|) \1(\sign(s)=\sign(t))$.
...
Let $X$ be a non-negative random variable, $f_0$ be its Lebesgue density, and $S_0(x)=\P[X>x]$ be its survival function. We consider the parameter of estimating the hazard function of $X$, $\theta_0(\x)=f_0(\x)/S_0(\x)$, with possible right-censoring as in the monotone density function example. Observations $\mathbf{Z}_1,\dots,\mathbf{Z}_n$ come from a random sample of $\mathbf{Z}=(\check{X},\Delta)'$ where $\check{X}=\min\{X,C\}$ and $\Delta=\1(X\leq C)$, $C$ being a random censoring time. As pointed out by \cite{Westling-Carone_2020_AoS}, with strictly increasing $\Phi_0$ with $\Phi_0(0)=0$, the function $\Gamma_0$ takes the form $\Gamma_0(x) = \int_0^x \frac{f_0(u)}{S_0(u)}\Phi_0(du)$, and by taking $\Phi_0(x)=\int_0^xS_0(u)du$, $\Gamma_0(x) =F_0(x)=\P[X\leq x]$. Since $\Gamma_0$ is identical to the monotone density case with the choice $\Phi_0=\int_0^xS_0(u)du$, we can leverage the analysis for the monotone density. The interval $I$ equals $[0,u_0^{\mathtt{MD}}]$ where $u_0^{\mathtt{MD}}$ is $u_0$ in the monotone density example. The $u_0$ for the monotone hazard function estimation is $u_0=\Phi_0(u_0^{\mathtt{MD}})$.

Consider the case of completely random censoring i.e.,\ $X\indep C$. As in the setup for Corollary \ref{M-[Corollary] monotone density independent right-censoring}, let $\widehat{S}_n(x)$ be the Kaplan-Meier estimator for $S_0(x)=1-F_0(x)=\P[X> x]$, $\widehat{F}_n=1-\widehat{S}_n$, and $\widehat{G}_n$ be the Kaplan-Meier estimator for $G_0(x)=\P[C>x]$. Also,
\begin{equation*}
...
\begin{corollary}\label{Appendix Corollary: Monotone Hazard Function Independent Right-Censoring}
    ...
    \end{equation*}
    \begin{equation*}
        \mathcal{C}_{\x}(s,t) = \frac{f_0(\x)}{G_0(\x)} (|s| \land |t|) \1(\sign(s)=\sign(t)),\quad \mathcal{D}_{\q}(\x) = \frac{S_0(\x)\partial^\q f_0(\x)}{(\q+1)!}.
\end{equation*}
\end{corollary}

\subsection{Proof of Corollary \ref{Appendix Corollary: Monotone Hazard Function Independent Right-Censoring}}

We use the same $\widehat{\gamma}_n$ function and assumptions as in the monotone density setting. Also, the covariance kernels are the same as in the monotone density case. We focus on \ref{M-Assumption E: Phi hat}-\ref{M-Assumption E: covariance kernel} and \ref{M-Assumption B: uhat}-\ref{M-Assumption B: jump of Phi hat}.
...
\begin{equation*}
    \sqrt{n a_n}\sup_{|v|\leq V} \big\vert  \widehat{\Phi}_n(\x + v) -\widehat{\Phi}_n(\x) - \bar{\Phi}_n(\x+v) + \bar{\Phi}_n(\x)\big\vert \leq o_{\P}(1) + V O_{\P}\big(\sqrt{a_n}\big)
\end{equation*}
uniformly over $V\in (0,2\delta]$.
...
\begin{corollary}\label{Appendix Corollary: Current Status Data Completely Independent Case}
    ...
    \end{equation*}
    \begin{equation*}
        \mathcal{C}_{\x}(s,t) = h_0(\x) \sigma_0^2(\x) (|s| \land |t|)\1(\sign(s)=\sign(t)),\qquad \mathcal{D}_{\q}(\x) = \frac{h_0(\x)\partial^{\q}F_0(\x)}{(\q+1)!}.
\end{equation*}
\end{corollary}
...
Define $F_0(C,\mathbf{A})=\E[\Delta|C,\mathbf{A}]$ and $g_0(C,\mathbf{A})=\frac{h_{C|\mathbf{A}}(C|\mathbf{A})}{h_0(C)}$ where $h_{C|\mathbf{A}}$ is a conditional Lebesgue density of $C$ given $\mathbf{A}$ and $h_0$ is a Lebesgue density of $C$. 
...
Let $\varepsilon=\Delta-\E[\Delta|C,\mathbf{A}]$, $\sigma_0^2(C,\mathbf{A})=\E[\varepsilon^2|C,\mathbf{A}]$, and $\delta >0$ be some fixed number.
\begin{enumerate}[label=\normalfont(\roman*),noitemsep,itemindent=*]
	\item $\E[\frac{\sigma_0^2(\x,\mathbf{A})}{g_0(\x,\mathbf{A})}]>0$.

    \item $h_{C|A}$ is bounded and $g_0$ is bounded away from zero.

    \item There exist random variables $A_n=o_{\P}(1)$ and $B_n=O_{\P}(a_n^{1/2})$ such that $$\sqrt{na_n}\sup_{|v|\leq V}|\widehat{\Gamma}_n(\x+v)-\widehat{\Gamma}_n(\x)-\Gamma_0(\x+v)+\Gamma_0(\x)|\leq A_n +B_nV,\qquad V\in (0,2\delta].$$ In addition, $$\frac{a_n}{n}\sum_{i=1}^n|\widehat{F}_n(C_i,\mathbf{A}_i)-F_0(C_i,\mathbf{A}_i)|^2=o_{\P}(1),$$ $$\frac{a_n}{n^2}\sum_{i=1}^n\sum_{j=1}^n|\widehat{F}_n(C_i,\mathbf{A}_j)-F_0(C_i,\mathbf{A}_j)|^2=o_{\P}(1),$$ and $$\frac{a_n}{n}\sum_{i=1}^n|\widehat{g}_n(C_i,\mathbf{A}_i)-g_0(C_i,\mathbf{A}_i)|^2=o_{\P}(1).$$

    \item $\E[\bar{F}(\mathbf{A})^2]<\infty$, where $$\bar{F}(\mathbf{A})=\sup_{|c-c'|\leq\delta}\frac{|F_0(c,\mathbf{A})-F_0(c',\mathbf{A})|}{|c-c'|}.$$ 

    \item $\E[\bar{\sigma}(\mathbf{A})^2]<\infty$, where, for some function $\omega$ with $\lim_{\eta\downarrow0}\omega(\eta)=0$, $$\left\vert\frac{\sigma_0^2(x,\mathbf{A})h_{0}(x)}{g_0(x,\mathbf{A})} -\frac{\sigma_0^2(\x,\mathbf{A})h_{0}(x)}{g_0(\x,\mathbf{A})}\right\vert \leq\omega(|x-\x|) \bar{\sigma}(\mathbf{A}),\qquad x\in I_{\x}^{\delta}.$$
\end{enumerate}
Note that $|\varepsilon|\leq 1$.

\begin{corollary}\label{Appendix Corollary: Current Status Data Conditionally Independence}
    Suppose that $\x$ is in the interior of $I$, $\theta_0$ satisfies \ref{M-Assumption A: theta0}, $\Phi_0$ satisfies \ref{M-Assumption A: Phi0}, and Assumption BW holds. If Assumption Assumption \thesubsection \ holds, Assumptions \ref{M-Assumption A} and \ref{M-Assumption B} hold with
    \begin{equation*}
        \widehat{\Gamma}_n(x)= \frac{1}{n}\sum_{i=1}^n \widehat{\gamma}_n(x;\mathbf{Z}_i),\quad  \widehat{\Gamma}_n^*(x)= \frac{1}{n}\sum_{i=1}^nW_{i,n} \widehat{\gamma}_n(x;\mathbf{Z}_i),
    \end{equation*}
    \begin{equation*}
...
\end{equation*}
\begin{equation*}
    \mathcal{C}_{\x}(s,t) = h_0(\x) \E\bigg[\frac{\sigma_0^2(\x,\mathbf{A})}{g_0(\x,\mathbf{A})}\bigg] (|s| \land |t|) \1(\sign(s)=\sign(t)),\qquad \mathcal{D}_{\q}(\x) = \frac{h_0(\x)\partial^{\q}F_0(\x)}{(\q+1)!}.
\end{equation*}
\end{corollary}
...
\end{equation*}
Then, letting $\mathsf{B}_j^{\mathtt{BR}}(\x) = \frac{\partial^{6}\Upsilon_0(\x)}{6!} \sum_{k=1}^{4}\lambda_j^{\mathtt{BR}}(k)c_k^{6}$, the MSE-optimal step size is
\begin{equation*}
    \epsilon_{j,n}^{\mathtt{BR}} = \bigg(\frac{(2j+1)\mathsf{V}_j^{\mathtt{BR}}(\x) }{2(5-j)\mathsf{B}_j^{\mathtt{BR}}(\x)^2}\bigg)^{1/11} n^{-1/11}.
\end{equation*}
The bias and variance constants depend on unknown features of the data generating process. Specifically, $\mathsf{B}_j^{\mathtt{BR}}(\x)$ depends on the regression function $\theta_0$, the Lebesgue density of $X$, and their derivatives at $X=\x$ while $\mathsf{V}_j^{\mathtt{BR}}(\x)$ is determined by the density of $X$ and the conditional variance of the regression error $\varepsilon=Y-\theta_0(X)$ at $X=\x$. To implement the construction of the step size, we posit a simple parametric model:
\begin{equation*}
