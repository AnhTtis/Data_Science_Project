...
\begin{document}
	...
	\author{
		Chao~Chen, 
		Haoyu~Geng,
		Nianzu~Yang,~\IEEEmembership{Member,~IEEE,}\\
		Xiaokang~Yang,~\IEEEmembership{Fellow,~IEEE},
		and Junchi~Yan,~\IEEEmembership{Senior Member,~IEEE}
		
		\IEEEcompsocitemizethanks{
			\IEEEcompsocthanksitem%X. Yang 
			C. Chen, H. Geng, N. Yang, X, Yang and J. Yan are with Department of Computer Science and Engineering and MoE Key Lab of Artificial Intelligence, Shanghai Jiao Tong University, Shanghai, 200240, China. J. Yan is also affiliated with Shanghai Artificial Intelligence Laboratory, Shanghai, 200240, China.
			E-mail: \{chao.chen, genghaoyu98, yangnianzu, yanjunchi\}@sjtu.edu.cn.
			\IEEEcompsocthanksitem   Correspondence author: Junchi Yan.
		}% <-this % stops an unwanted space xkyang
	}
	...
	\IEEEPARstart{D}ynamic graph representation learning (DGRL) in general aims to obtain node vector embedding over time $t$ (ideally in continuous time domain, i.e., $t\!\in\!\mathbb{R}$) on graph of which the topological structures and the node attributes are evolving as shown in Fig.~\ref{fig:dyg}. Under such a general paradigm, tailored models are often devised and trained according to specific (time-conditioned) predictive tasks, ranging from (dynamic) node classification, link prediction to traffic flow forecasting. These tasks have found widespread application in social networks, bioinformatics, transport, etc.

	In practise, a general DGRL pipeline should consist of the encoding framework of which inputs include the graph information and the observed events, learning scheme with the derived loss according to specific tasks at hand. Besides, interpretation of model outputs (e.g., model predictions) is often of high interest in real-world applications. Our following discussion further elaborates on these three aspects.
	\begin{figure}[tb!]
	...
	\begin{figure*}[tb!]
		...
		(\textbf{c}) Finally, perturbation-based analysis is performed to quantify the importance of frequency content that the model learns from the data (see Sec.~\ref{sec:interpret}) in a scalable way. In particular, it enables model-level interpretation instead of instance-level interpretation as done in most existing explainable graph learning works~\cite{fan2021gcn,pope2019explainability,yuan2021explainability,yuan2022explainability}.}
		\vskip -0.1in
	\end{figure*}
	...
	Earlier works, e.g., DCRNN\!\cite{li2018diffusion}, DySAT\!\cite{sankar2020dysat} and STGCN\!\cite{yu2018spatio}, often view the dynamic temporal graph as a sequence of snapshots with discrete time slots. To improve the model expressiveness and flexibility, continuous-time approaches are recently fast developed whereby the evolution of graph structures and node attributes as shown in Fig.~\ref{fig:dyg} are directly modeled.
	Beyond the works\cite{kumar2019predicting,nguyen2018continuous,xu2020inductive,li2020time} that encode time into embeddings independent of spatial information, the seminal works\cite{zuo2018embedding,trivedi2019dyrep,zuo2020transformer} explore the use of temporal point process (TPP) techniques for DGRL and introduce the MessagePassing-Intensity framework to capture the entangled spatiotemporal dynamics. Although these approaches have shown promising results, they encounter the common limitations: 
	1) the message-passing module takes node features as input, whereas the intensity module delivers edge intensity as output. This mismatch between the input and the output limits the MessagePassing-Intensity networks to be used as basic building block that can be stacked to learn high-level features in multi-layered architectures.
	2) these works focus on link prediction tasks, as the TPP intensity naturally specifies the occurrence of edge-addition events at any given time. Meanwhile, it is not easy to design intensity based loss for node regression tasks, e.g., traffic forecasting.
	
	With the immense success of attention networks, recent work\cite{cho2021learning} designs an Attention-Attention framework where the former attention models periodic patterns and the latter attention encodes the structural and temporal dependencies. However, we argue that the lack of the ability to model the evolving patterns in continuous time domain will hurt the performance. CADN\cite{chien2022learning} develops an ODERNN-Attention encoding framework where ODERNN, an extension of RNN with ordinary differential equation (ODE) solver, is adopted to learn continuous-time dynamics. However, RNN requires sequential computation and thus suffers from slow training. In this paper, we address the aforementioned issues by formalizing an attention-intensity-attention encoding framework, suitable for link-level and node-level applications.
	
	
	
	% and this paper instead adopts attention mechanism~\cite{vaswani2017attention} to achieve efficient and effective encoding and perhaps more importantly, we propose to introduce the intensity of a temporal point process (TPP) which is a time-varying function in the continuous time domain to model the attention mechanism. TPP has shown its flexibility and interpretability on various real-world event data from traditional pure statistical models~\cite{YanIJCAI13,YanIJCAI18} to neural TPP models~\cite{XiaoTNNLS19,XiaoNIPS17}.
	...
	In recent years, masked autoencoding has found versatile applications thanks to several groundbreaking practices, such as BERT\cite{devlin2018bert} in natural language processing (NLP) and the very recent MAE\cite{he2022masked} in computer vision (CV). The loss of masked autoencoding, which recovers masked parts of the input data, is natural and applicable in DGRL. Recent works\cite{sun2019bert4rec,hou2022graphmae} randomly mask a portion of the graph data by special tokens and learn to predict the removed content. However, we found that applying such masked training to attention networks for (node) traffic forecasting would hurt the performance (see the results in Table~\ref{tab:motiv}).	
	We conjecture the reason is that different queries at different timestamps that are replaced by special tokens will have the same query embedding, making the ranking of attention weights of the keys shared across these queries.
	This may induce a global ranking of influential nodes, which makes sense in domains such as recommender systems and social networks where popular products and top influencers dominate the market. However, a node predictive task might emphasize recently formed edges or certain past timestamps. 
	In this paper,	we propose a new correlation-adjusted masking (CaM) for task-aware learning, which can help the attention identify and exploit above patterns in predicting node labels, while enjoying the benefits of existing masking in link prediction.

	Despite the masking, the other pursuit for loss design is to make sure that the node embeddings learn the dynamics of the observed graph (e.g., the addition of edges in Fig.~\ref{fig:dyg}). In literature, temporal smoothness is often considered~\cite{zhu2016scalable,zhou2018dynamic} which suppresses sharp changes over time. However, we argue that this prior can be sometimes too restrictive to real-world scenarios (see Table \ref{tab:motiv}) where graph data evolves quickly. In this paper, we address the issue by introducing the TPP likelihood of events (TPPLE) as a principled way of explaining the graph changes in continuous time domain.
	
	
	
	\begin{table}[t!]
		\centering
		\caption{\label{tab:motiv}RMSE on the META-LA dataset for 15 minutes, 30 minutes and 45 minutes ahead traffic forecasting, where {\mname} w/ masking randomly replace the nodes with special tokens\cite{hou2022graphmae}, and {\mname} w/ temporal smoothness penalizes the $\ell_2$-norm distance of vector embeddings of a node at consecutive snapshots\cite{zhou2018dynamic}.}
		\vspace{-5pt}
		\begin{tabular}{lc c c}\toprule\toprule
			& \textbf{15 min} & \textbf{30 min} 
			& \textbf{45 min} 
			\\\midrule
			\textbf{{\mname} w/o CaM and TPPLE} &
			5.618 & 6.628 & 7.306
			\\\midrule
			\textbf{{\mname} w/ masking} &
			5.653 & 6.654 & 7.272 \\
			\textbf{{\mname} w/ CaM} &
			5.562 & 6.541 & 7.162
			\\\midrule
			\textbf{{\mname} w/ temporal smoothness} &
			5.908  & 6.634 & 7.423 \\
			\textbf{{\mname} w/ TPPLE} &
			5.598  & 6.584 & 7.211
	...
	Existing works on interpretable GRL often analyze the importance of neighboring nodes' features to a query node (mostly indicated by the learned attention weights\cite{fan2021gcn} and the gradient-based weights\cite{pope2019explainability}) as well as the importance of the (sub)graph topologies based on the Shapley value\cite{yuan2021explainability}. These works are mainly example-specific and explain why a model predicts what it predicts in the graph vertex domain.
	It often remains unclear that which frequency information the model is exploiting in making predictions. 
	We note that the quantitative analysis in the graph Fourier domain has a distinct advantage since it can provide the model-level explanations with respect to the intrinsic structures of the underlying graph. In general, model-level explanations are high-level and can summarize the main characteristics of the model (see Table~\ref{tbl:perturb} for example). To achieve this, there exist two main challenges: 
	1) the graph Fourier transform (GFT)\cite{ortega2018graph} provides a means to map the data (e.g., model predictions and node embeddings) residing on the nodes into the spectrum of frequency components, but it cannot quantify the predictive power of the learned frequency content. For example in Fig.~\ref{fig:vf_analysis}, applying GFT shows
	that dynamic graph model BERT4REC\cite{sun2019bert4rec} can learn high-frequency information that is not provided by static graph model GSIMC\cite{chen2023graph}, but it is unclear if BERT4REC outperforms GSIMC because of this high-frequency information.
	2) GFT requires to perform orthogonalized graph Laplacian decomposition, whose cost is up to $\mathcal{O}(N^3)$, cubic to the number of nodes $N$ in the graph. 
	Although {\nystrom} methods\cite{fowlkes2004spectral} can obtain the linear cost, their results do not fulfill the orthogonality condition (see more discussion in Sec.~\ref{sec:interpret_decomp}). 
	In this paper, we address the challenges by proposing appropriate perturbations in the graph Fourier domain, in company with a scalable and orthogonalized graph Laplacian decomposition algorithm. 
	
	
	\subsection{Approach Overview and Contributions}
	In light of such above discussions, we propose \textbf{\mframe}, a new encode, train and interpret pipeline for dynamic graph representation learning,	of which the goal is to achieve: 
	...
	This paper is a significant extension (almost new)\footnote{Compared with the proceeding, extensions can be summarized in three folds: 
	i) we extend to a general representation learning framework on dynamic graph from the task-specific method for link prediction; 
	ii) we propose a principled training scheme that consists of task-agnostic TPP posterior maximization and task-aware loss with a masking strategy designed for dynamic graph; 
	iii) we devise a spectral quantitative post-analyzing technique based on perturbations, where a scalable and orthogonalized graph Laplacian decomposition is proposed to support the analysis on very large graph data.} of the preliminary works\cite{chen2021learning}. Our contributions are as follows:
	...
	\begin{comment}
	\subsubsection{Understanding Vanilla Attention's Limitation from Signal Processing Perspective}
	In what follows, we provide a signal processing perspective on attention networks, and summarize the limitations when applying them on dynamic graphs.
	
	Consider a sensor network, in which we keep the track of sequential measurements $\{\mathbf{v}_i\}$ from multiple different sensors $\{\mathbf{k}_i\}$ under statistical noise $\{\eta_i\}$. We concern about the following stochastic filtering problem in the state-space form, where the system dynamics are constant:
	\begin{flalign}
	\mathbf{h}_{u}^{(t_i)} &= \mathbf{h}_{u}^{(t_{i-1})} \label{eq:ss_state}\\
	\mathbf{v}_i &= \mathbf{h}_u^{(t_i)} + \eta_i, \label{eq:ss_measu}
	\end{flalign}where Eq.~\eqref{eq:ss_state} and \eqref{eq:ss_measu} are called the state equation and measurement equation, respectively; $\mathbf{h}_u^{(t_i)}$ signifies the state vector which is constant with time, $\mathbf{v}_i$ is the measurement vector; $\eta_i$ signifies the noise $\mathcal{N}(0, \sigma^2_i\mathbf{I})$ in the measurement.
	
	Kalman filtering~\cite{kalman1960new} has been widely adopted in multi-sensor fusion, due to the merits that the linear estimator of hidden states is not only unbiased $\mathbf{E}[\hat{\mathbf{h}}_u^{(t)}]\!=\!\mathbf{E}[\mathbf{h}_u^{(t)}]$ but also is minimum-variance. Performing standard procedures in~\cite{kalman1960new} derives the following solution, with $n$ measurements:
	\begin{flalign}
	\hat{\mathbf{h}}^{(t_n)}_u &= \bar{\mathbf{h}}^{(t_n)}_u 
	+ \mathbf{K}_n(\mathbf{v}_n - \bar{\mathbf{h}}^{(t_n)}_u ) \nonumber\\
	\mathbf{P}_{n|n} &= 
	(\mathbf{I}-\mathbf{K}_n)\mathbf{P}_{n|n-1}(\mathbf{I}-\mathbf{K}_n)^\top + \mathbf{K}_n\mathbf{\Sigma}_n\mathbf{K}^\top_n \nonumber\\
	\mathbf{K}_n &= \mathbf{P}_{n|n-1}(\mathbf{P}_{n|n-1} + \mathbf{\Sigma}_n)^{-}, \nonumber
	\end{flalign}where $\hat{\mathbf{h}}^{(t_n)}_u$ and $\mathbf{P}_{n|n}$ are respectively the estimate state and the estimate covariance after taking the measurement $\mathbf{v}_n$; 
	$\bar{\mathbf{h}}^{(t_{n})}_u\!\!=\!\!\hat{\mathbf{h}}^{(t_{n-1})}_u$ and $\mathbf{P}_{n|n-1}$ are the extrapolated state and covariance made at time $t_{n-1}$;
	$\mathbf{K}_{n}$ is called the Kalman gain\cite{kalman1960new} and the term $\mathbf{v}_n\!-\bar{\mathbf{h}}_u^{(t_n)}$ is called the innovation\cite{kalman1960new} that contains new information.
	Let $\mathbf{P}_{0|0}=\sigma_0^2$ and $\hat{\mathbf{h}}_u^{(t_0)}=\mathbf{v}_0$, we conclude the estimate of state $\mathbf{h}_u^{(t_n)}$ in the closed form:
	\begin{flalign}
	\hat{\mathbf{h}}_u^{(t_n)} = \sum_{i\le{n}}     
	\Big(
	\frac{\sigma_{i}^{-2}}{\sum_{j\le{n}}\sigma_{j}^{-2}}
	\Big) \mathbf{v}_i. \nonumber
	\end{flalign}This bears a resemblance with attention networks as defined in Eq. \eqref{eq:att}. In fact, attention network is identical to a Kalman filter \yanr{when the embedding is constant over time}, when the scoring function $e_{u,i}$ learns the log-inverse of the covariance of Gaussian noise in the measurements i.e., $\ln\sigma_i^{-2}$.
	
	From this viewpoint, attention based learning approach is the \textit{optimal linear filter} in the constant system. However, we argue that these methods would probably be suboptimal in dynamic systems, when extrapolating the hidden states at a future point in time. Specifically, unknown states of interest continuously evolve over time in a dynamic system, such that certain past measurements would become irrelevant to hidden state $\mathbf{h}_u^{(t)}$ at the specified future time $t$. Furthermore, certain sensor $\mathbf{k}_l$ would likely observe the measurement $\mathbf{v}_l$ again or multiple times, over a period of time up to $t$. For example, in recommender systems, food and other daily necessities will be purchased repeatedly every 24 hours. When the relative time intervals conform such periodicity pattern, these items are more likely to be purchased and should be weighted higher than others accordingly.  However, existing attention networks \cite{brody2021attentive,liu2020kalman,vaswani2017attention,velivckovic2017graph} are not aware of such temporal dynamics on the evolving graph. This limits the model's fidelity and flexibility for real applications.
	
	To go beyond such limitations, we propose to introduce the well-established conditional intensity function of a TPP to capture the dynamics on the observed graph. This enables a continuous time domain attention. In the following, we briefly depict the basics of TPP.
	\end{comment}
	...
	\end{flalign}
	where the exponential term is called the survival function $S^\ast(t)$ that characterizes the conditional probability that no event happens during $[t_n, t)$, and $t_n$ is the last event occurred before time $t$. Using the chain rule, the likelihood for a point pattern $\{(t_i,k_i)|t_i<T)\}$ is given by
	\begin{flalign}\label{eq:tpp_mle}
		...
		\Bigg),
	\end{flalign}where the integral for all possible non-events is intractable to compute, and we describe two algorithms to approximate the integral in Sec.~\ref{ssec:treg}. 
	...
	Given a continuous-time graph $(\mathcal{G}, \mathcal{O})$, if node $v_u$ involves $n$ events at time $\{t_1,\dots,t_n\}$ and $\mathcal{X}\!\!=\!\!\{\mathbf{X}^{(1)},
	\dots,\mathbf{X}^{(n)}\}$ denotes dynamic node attributes, then we define its continuous-time representation at any future timestamp $t>t_n$ by $\mathbf{h}^{(t)}_{u}$:
	\begin{flalign}
		\mathbf{h}^{(t)}_{u} = 
		\text{GRL}(u,t;
	...
	%We mainly discuss related works and techniques on graph representation learning, from static graph, to discrete-time dynamic graph, and ultimately to continuous-time dynamic graph which is the focus of this paper. 
	
	%\subsection{Static Graph Methods} 
	%Representation learning approaches for static graphs mainly focus on encoding the high-order proximity into low dimensional representations. Early works, such as Node2Vec~\cite{grover2016node2vec}, DeepWalk~\cite{perozzi2014deepwalk} and LINE~\cite{tang2015line}, leverage the matrix or tensor decomposition techniques to learn the linear representation for each node, which is inherently transductive. To improve the expressiveness, the authors in~\cite{bruna2014spectral,defferrard2016convolutional,henaff2015deep,bianchi2021graph} introduce convolutional neural networks to capture sophisticated feature information but are generally less scalable. Hamilton~\etal\cite{hamilton2017inductive} propose an inductive learning framework to predict the embeddings for previously unseen nodes by aggregating the information over the neighborhood. Furthermore, GAT \cite{velivckovic2017graph} and its successor \cite{brody2021attentive} advance GraphSAGE \cite{hamilton2017inductive} by using non-uniform attention over neighborhood. However, most of these approaches can only model static graph or evolving graph without temporal information.
	
	
	%\subsection{Discrete-Time Dynamic Graph Methods} Preliminary approaches in dynamic representation learning have considered discrete time approach that approximates a temporally evolving graph as a sequence of snapshot graphs that are sampled at regularly-spaced times.
	
	%\textit{Tensor Decomposition Methods.} The spatiotemporal data is naturally a high dimensional tensor and it is straightforward to decompose the tensor for achieving node representations at each snapshot. Mao~\etal\cite{mao2018spatio} propose temporal smoothness penalty to stop the learned representation from having sharp changes. McNeil~\etal\cite{mcneil2021temporal} encode prior knowledge as dictionaries, then by using sparse coding methods to derive sparse representations. Fang~\etal\cite{fang2022bayesian} model the representation as a time-varying function to learn irregularly sampled data. However, most of these approaches are transductive and cannot predict the representations for unseen nodes.
	
	
	%\textit{Sequence Methods.} There exists a rich body of research on extending sequence models to graph data. Li~\etal\cite{li2018diffusion} and Zhang~\etal\cite{zhang2018gaan} exploit static graph methods to model the spatial dependency then capture the evolution of individual node’s representation independent of others, while Pareja~\etal\cite{pareja2020evolvegcn} focus on the evolution of the model parameters over time. However, these approaches suffer from slow training due to use of recurrent neural networks. Bai~\etal\cite{bai2020adaptive}, Lan~\etal\cite{lan2022dstagnn} and Yu~\etal\cite{yu2018spatio} adopt gated convolutional neural networks to accelerate the model training, while Sankar~\etal\cite{sankar2020dysat} resort to self-attention networks. 
	
	%It has been extensively studied in \cite{kazemi2020representation} that discrete-time approaches do not have the ability to capture fine-grained dynamics due to the loss of information between snapshots; Meanwhile, such approaches update the representation for each individual node only when a new observation has been made about the node and its relations. This incurs the risk of memory staleness if nodes are not active for a relatively long time; Additionally, we note that most of existing sequence methods either use multiple feed-forward networks \cite{bai2020adaptive,lan2022dstagnn} or build complex autoregressive models \cite{li2018diffusion,zhang2018gaan} to make multi-step predictions. The former cannot make predictions on irregular sampled data as discussed in \cite{jin2022neural}, and the latter increases the risk of error accumulation.
	
	
	
	In literature, discrete-time dynamic graph approaches have been extensively studied due to ease of implementation. It can be mainly categorized into tensor decomposition\cite{mao2018spatio,mcneil2021temporal,fang2022bayesian} and sequential modeling\cite{li2018diffusion,zhang2018gaan,pareja2020evolvegcn,bai2020adaptive,lan2022dstagnn,yu2018spatio,sankar2020dysat} two families. By contrast, continuous-time graph approaches are relatively less studied. In the following, we review the related works in two aspects: 1) spatio-temporal decoupled modeling, where continuous-valued timestamps are directly encoded into vectorized representations; 
	...
	\end{flalign}
	where Eq.~\eqref{eq:gcats1} holds due to Eq.~\eqref{eq:intensity} where $\lambda_i^\ast(t)\mathrm{d}t$ specifies the number of type-$i$ events, i.e., new edges on pair $(v_i,v_u)$ during $[t,t\!+\!\mathrm{d}t)$.
	...
	\end{equation}
% 	where we call the attention in Eq.~\eqref{eq:catf_point},  the joint  structural-temporal dynamic graph encoding procedure in {\mname}.
	
	% or the representation in certain time interval $[t_n, t)$ by
	% \begin{flalign}
	% \mathrm{Att}(\mathbf{h}^{(t-)}_u|\mathcal{N}_u, \mathbf{v})
	% = \mathbf{E}_{\mathbf{v}_i\sim{Q}}
	%     \Bigg(
	%     \int_{t_n}^t \lambda_i^\ast(s\big|\mathcal{H}_{t}) \mathrm{d}s \Bigg)
	%     \mathbf{v}_i, \label{eq:catf_area}
	% \end{flalign}where $t-$ denotes the time interval $[t_n,t)$, and as mentioned the integral is challenging to compute and proper approximations are needed which will be discussed later.
	...
	% In the literature, one common rationale behind methods for Continuous-times is that the set of observed events are the realizations of the non-linearly evolving dynamic processes that govern the changes in the graph structure.
	%To inductively generate representations on dynamic graphs, we should operate the embeddings in the function space. Therefore, it becomes straightforward to combine the intensity function $\lambda^\ast(t\big|\mathcal{H}_{t},\mathbf{v})$ that captures the temporal patterns over dynamic graphs, with the softmax distribution function $p(\mathbf{v}_i|\mathcal{N}_u)$ that captures the spatial patterns over static graphs. Fully expanded out, \textit{continuous-time attention} that is able to capture continuous-time fine-grained temporal dynamics, can be defined as: 
	...
	The encoding network consists of four key steps and follows an attention-intensity-attention scheme: 
	\begin{enumerate}[label=\textbf{Step \arabic*:}, topsep=0in,leftmargin=0em,wide=0em]
		...
		\item \textbf{Endogenous dynamic encoding (Attention).} Since the temporal and structural dynamics are entangled in domains like road networks, we propose a specific attention-intensity-attention design. We first employ an attention network to encode localized changes in the neighborhood (i.e., the endogenous dynamics). 
		When an event changes spatial graph structures or node attributes, the representation outputted from the attention will be changed accordingly.
		...
		\item \textbf{Intensity-attention modulating (Attention)}. As the learned intensity naturally offers us the ability to specify the occurrence of edge-addition events at a specific future time, a new attention mechanism is proposed by using the intensity to modulate the attention in Step 2. The resulting attention can yield continuous-time node embeddings. 
	\end{enumerate}

	\subsubsection{Clustering Structure Encoding} 
	To capture the group-level interactions, we add ``clustering structure encodings" to the input embeddings as an initial step, followed by linear transformation:
	\begin{flalign}
		\mathbf{H}^{(t)}_{0} &=
	...
	We next employ deep attention network to capture localized changes in graph structures and node attributes (namely, the endogenous dynamics) within the neighborhood. Let $v_u$ denote the query node and set $\mathcal{N}_u$ denotes its neighborhood, then we encode such dynamics into representation $\mathbf{s}_{u;l}^{(t)}$:
	\begin{flalign}
		\mathbf{s}_{u;l}^{(t)} =
		\sum_{i\in\mathcal{N}_u} 
		\left( 
		\frac{\exp(e^{(t)}_{u,i})}
		{\sum_{i'\in\mathcal{N}_u}\exp(e^{(t)}_{u,i'})}
		\right)\mathbf{v}^{(t)}_{i;l}, \label{eq:pp_att}
	\end{flalign}
	where $\mathbf{v}^{(t)}_{i;l}$ is the $i^\mathrm{th}$ column of $\mathbf{V}^{(t)}_{l}$, $e^{(t)}_{u,i}$ is the attention coefficient which can be implemented in various ways (e.g., SA\cite{vaswani2017attention}, GAT \cite{velivckovic2017graph} and GATv2 \cite{vaswani2017attention}).
	Note that $e^{(t)}_{u,i}$ essentially models the associations between clusters since it implicitly computes the interaction between cluster embeddings, analogous to the positional encoding \cite{vaswani2017attention}. 
	
	
	\subsubsection{Conditional Intensity Modeling}\label{sec:esta_ints}
	Recall that $\mathcal{H}_t\!=\!\{(t_i,k_i)|
	t_i<t\}$ denotes the history, where pair $(t_i,k_i)$ signifies the occurrence of type-$k_i$ event at time $t_i$ --- a new edge connecting $v_u$ and one node in group $k_i$ appears in graph at time $t_i$. To model the dynamics of events,	we combine the structure-aware endogenous dynamics $\mathbf{s}^{(l)}_{u;l}$ with the	time-dependent exogenous dynamics:
	\begin{equation}
		\begin{aligned} \label{eqn:att_g}
			\mathbf{g}^{(t)}_{k;l}
			= \sigma \Big(
			\underbrace{ %
	...
	We use TPP intensity to modulate the attention following Eq.~\eqref{eq:scatf_point}:
	\begin{gather}
	...
	%\subsection{{\mreg}: Continuous-time as Regularized Penalty}\label{ssec:treg}
	...
	Let $\mathbf{y}$ denote the ground-truth label, $\hat{\mathbf{y}}_\Theta$ represents the predicted result where $\Theta$ denotes model parameters, and function $\ell(\mathbf{y}, \hat{\mathbf{y}}_\Theta)$ measures the difference between the label and the prediction. Then, we optimize $\Theta$ by minimizing the following risk:
	\begin{equation}\label{eqn:ctr_loss}
		\min_\Theta 
		\mathbf{E}_\mathbf{y} \ell(\mathbf{y}, \hat{\mathbf{y}}_\Theta) 
		- \gamma 
		\mathbf{E}_{u}\,R(\Theta; u),
	\end{equation}where $\gamma$ is a hyper-parameter which controls the strength of the regularization; $R(\Theta;u)$ signifies our {\mreg} term which maximizes the log-likelihood of the whole observed events $\{(k_1,t_1),\dots,(k_n,t_n)\}$ for any given query node $v_u$:
	...
	In experiments, we have found that the approximation defined in Eq.~(\ref{eqn:nu}) performs comparable to that in Eq.~(\ref{eqn:mc}) while consuming less computational time. Hence we choose numerical integration in our main approach. %We conjecture the reason is that the softplus function is highly smoothed and the bias introduced by linear approximations is relatively small.
	...
	%\subsection{Benchmark Scenarios: Problem, Model, Learning}\label{sec:bench}
	%This section introduces our benchmark scenarios by describing the considered problems, detailed network architectures and learning schemes. 
	
% 	Fig.~\ref{fig:cam} shows the overview of our proposed approaches for link prediction, node classification and node traffic forecasting. Note that our conference version CTSMA is tailored to link prediction. While in this paper we propose masking-based scheme in bi-directions in an auto-encoding way instead of the auto-regressive way as done in~\cite{chen2021learning} for learning and also incorporate two further tasks.


    
	
	
	\subsubsection{Task I: Masked Learning for Dynamic Link Prediction}\label{sec:lpredc}
	...
	\subsubsection{Task II: Masked Learning for Dynamic Node Classification}\label{sec:nclassf}
	...
	\textbf{Event.} Likewise, we focus on the edge-addition events. For example, on the Elliptic dataset, a new edge can be viewed as a flow of bitcoins from one transaction (node) to the next.
	
	
	\textbf{Training Loss.}
	Fig.~\ref{fig:cam}(c) presents the details about model encoding and training for node classification. Given $\mathcal{N}_u$, we compute $\mathbf{h}^{(t_u)}_{u;L}$ for query $v_u$ by stacking $L$ layers to predict $\mathbf{y}_u^{(t_u)}$ at time $t_u$ (e.g., the time of next event on query $v_u$):
	\begin{flalign}%\label{eq:ncpredc}
		\hat{\mathbf{y}}_{u}^{(t_u)} = \operatorname{softmax}(\mathbf{h}^{(t_u)}_{u;L}\mathbf{W}^O) + \mathbf{b}^O, 
	...
	\begin{figure*}[t!]
		\centerline{\includegraphics[width=.98\textwidth]{pics/Recall_dict.pdf}}
% 		\vspace{-10pt}
	...
	\end{figure*}
	\subsubsection{Task III: Masked Learning for Node Traffic Forecasting}\label{sec:tforecast}
	...
	\textbf{Inference.}
	Analogous to the treatment for node classification, we switch off the masking and utilize original node features as the input at testing stage.
	...
	and 2) it is difficult to perform GFT on large graphs
	due to the $\mathcal{O}(N^3)$ cost of orthogonalized graph Laplacian decomposition.
	...
	\begin{table*}[tb!]
		...
		\begin{center}\resizebox{0.98\textwidth}{!}{
		    \begin{tabular}{l ccc c ccc c ccc} \toprule\toprule
					...
					0.23018 & 0.39772 & 0.48608 \\\midrule
					...
					\textbf{\mframe} (ours)  &
					\textbf{0.29285} & \textbf{0.39280} & \textbf{0.44005} && 
					\textbf{0.27657} & \textbf{0.42063} & \textbf{0.49317} && 
					\textbf{0.29098} & \textbf{0.47395} & \textbf{0.56178} \\
					\textbf{Rel. Gain} (over \cite{chen2021learning}) &
					6.6\%  & 11.4\% & 12.1\% && 
					3.2\%  & 3.5\% &  3.8\% && 
					14.5\% & 8.4\% & 6.7\% \\\bottomrule\bottomrule
				\end{tabular}
...
% 	spectrum interacting with each other.
	...
	\begin{algorithm}[t!]
		...
		\begin{algorithmic}[1]
			...
			\ENSURE eigenvalues
			$\widetilde{\mathbf{\Sigma}}$ and eigenvectors $\mathbf{\widetilde{U}}$.
	...
	\begin{table*}[tb!]
		...
		\begin{center}\resizebox{0.98\textwidth}{!}{
				\begin{tabular}{l ccc c ccc c ccc} \toprule\toprule
					...
					0.14615 & 0.18301 & 0.19734 \\\midrule
					...
					\textbf{\mframe} (ours) &
					\textbf{0.22627} & \textbf{0.24838} & \textbf{0.25629} && 
					\textbf{0.21511} & \textbf{0.23313} & \textbf{0.23973} && 
					\textbf{0.18667} & \textbf{0.22688} & \textbf{0.24116} \\
					\textbf{Rel. Gain} (over \cite{chen2021learning}) &
					7.4\%  & 9.1\%  & 9.5\% && 
					19.2\% & 10.8\% & 8.0\% && 
					14.7\% & 12.4\% & 11.4\% \\\bottomrule\bottomrule
				\end{tabular}
		}\end{center}
	\end{table*}	
	...
	It is shown that sequential models achieve better results than static graph models, and continuous-time graph models (i.e., DyREP, TimelyREC, CSTMA, TiSASREC, TGREC, TiSASREC, TGAT and {\mname}) are among the best on all three datasets. In particular, {\mname} achieves $6.7-14.5\%$ relative hit rate gains and $11.4-14.7\%$ relative NDCG gains over the best baseline on challenging Netflix.
	
	In addition, we emphasize {\mname}'s consistent performance boosting  over our preliminary conference version CTSMA~\cite{chen2021learning}. The key difference is that CTSMA is a unidirectional autoregressive model. By contrast, {\mname} is a bidirectional masked autoencoding model that incorporates the contexts from both left and right side. Perhaps more importantly, {\mname} enjoys the benefit of improved robustness and generalizability, due to masking strategy that introduces the randomness to the model and also generates diversified samples for training.
	
	\begin{table*}[t!]
		\caption{\label{tab:nclassify}Evaluation on the Elliptic dataset (bitcoin transactions) for dynamic node classification over time, where {\mname} performs best for all columns at different time steps among both static graph models (top half) as well as dynamic graph models (bottom half). 
			Note that DySAT~\cite{sankar2020dysat}, EvolveGCN~\cite{pareja2020evolvegcn}, JODIE~\cite{kumar2019predicting} and TGN~\cite{rossi2020temporal} require multiple (i.e. 5) historical graph snapshots (see the `input' column), whereas {\mname} only accesses the update-to-date snapshot for prediction. {\mname} outperforms the best baseline TGN in accuracy with 4X speedup for training.
			All the methods perform poorly starting at time step 43, because this time is when the bitcoin dark market shutdown occurred which changes the external environment of the graph yet such information cannot be readily encoded by the evaluated models neither in training nor testing. In this extreme case, {\mname} performs relatively better than baselines especially regarding the Micro-F1 score which is for the minority (illicit) class.}
		\vspace{-10pt}
		\begin{center}\resizebox{\textwidth}{!}{
				\begin{tabular}{l c ccc c ccc c ccc} \toprule\toprule
					...
					\textbf{APPNP}~\cite{steck2019markov}   & 1 &
					...
					0.525$\pm$0.011 & 0.025$\pm$0.030 & \cellcolor{gray90}{0.836}$\pm$0.009 && 
					0.544$\pm$0.008 & 0.000$\pm$0.000 & 0.873$\pm$0.011 \\
					\textbf{TGN}~\cite{rossi2020temporal} & 5 &
					\cellcolor{gray90}{0.719$\pm$0.016} & 0.542$\pm$0.023 & \cellcolor{gray90}{0.834$\pm$0.009} && 
					\cellcolor{gray90}{0.529$\pm$0.010} & 0.024$\pm$0.028 & 0.827$\pm$0.021 && 
					\cellcolor{gray90}{0.572$\pm$0.013} & \cellcolor{gray90}{0.071$\pm$0.011} & \cellcolor{gray90}{0.885$\pm$0.042} \\\midrule
					
					\textbf{\mframe}  & 1 &
					\textbf{0.730$\pm$0.002} & \textbf{0.566$\pm$0.012} & \textbf{0.845$\pm$0.003} && 
					\textbf{0.540$\pm$0.007} & \textbf{0.069$\pm$0.023} & \textbf{0.843$\pm$0.003} && 
					\textbf{0.575$\pm$0.003} & \textbf{0.110$\pm$0.009} & \textbf{0.885$\pm$0.006}  \\\bottomrule\bottomrule
				\end{tabular}
		}\end{center}
% 		\vskip -0.1in
	\end{table*}
    \begin{table*}[t!]
		...
		Evaluation on the META-LA dataset for 15 minutes (horizon 3), 30 minutes (horizon 6) and 45 minutes (horizon 9) ahead traffic forecasting, where the baselines include static graph models (top half) and dynamic graph models (bottom half). 
			Note that DCRNN\cite{li2018diffusion} , DySAT\cite{sankar2020dysat}, GaAN\cite{zhang2018gaan}, STGCN\cite{yu2018spatio} and DSTAGNN\cite{lan2022dstagnn} use 60 minutes as the historical time window, namely the most recent 12 graph snapshots (see the `input' column) are used to forecast traffic conditions in the next 15, 30 and 45 minutes, whereas {\mname} makes predictions only based on the current single graph snapshot. Specifically, {\mname} performs competitively with state-of-the-art DSTAGNN with 6X speedup for training.}
		\vspace{-10pt}
		\begin{center}\resizebox{\textwidth}{!}{
			\begin{tabular}{l c ccc c ccc c ccc} \toprule\toprule
					&  &
					...
					\cellcolor{gray90}{3.575$\pm$0.027} & 7.251$\pm$0.036 & 0.104$\pm$0.001 \\
					\textbf{DSTAGNN}~\cite{lan2022dstagnn}  & 12 &
					\cellcolor{gray90}{2.960$\pm$0.035} & \cellcolor{gray90}{5.611$\pm$0.052} & \cellcolor{gray90}{0.079$\pm$0.001} && 
					\cellcolor{gray90}{3.321$\pm$0.030} & \cellcolor{gray90}{6.683$\pm$0.056} & \cellcolor{gray90}{0.093$\pm$0.001} && 
					\cellcolor{gray90}{3.595$\pm$0.029} & \cellcolor{gray90}{7.249$\pm$0.058} & \cellcolor{gray90}{0.102$\pm$0.000} \\
					\midrule
					
					\textbf{\mframe}  & 1 &
					\textbf{2.879$\pm$0.019} & \textbf{5.594$\pm$0.040} & \textbf{0.076$\pm$0.001} && 
					\textbf{3.268$\pm$0.005} & \textbf{6.590$\pm$0.021} & \textbf{0.090$\pm$0.001} && 
					\textbf{3.527$\pm$0.026} & \textbf{7.220$\pm$0.044} & \textbf{0.100$\pm$0.001}    \\\bottomrule\bottomrule
				\end{tabular}
		}\end{center}
% 		\vspace{-10pt}
	\end{table*}
	...
	We report the results in terms of Macro-F1, Micro-F1 and accuracy on the test set for the model that achieves the best on the validation data set. We note that nearly $80\%$ of nodes are labeled as ``unknown", such that a model which labels every node as unknown can achieve the accuracy up to $0.8$. Because the illicit nodes are of more interest, we report the Micro-F1 for the minority (illicit) class. The results are the mean from five random trials.
	
		

	
	\textbf{Baseline.}
	We compare with state-of-the-art methods for dynamic node classification: (1) static graph approaches, i.e., APPNP~\cite{steck2019markov}, GAT~\cite{velivckovic2017graph}, SAGE~\cite{hamilton2017inductive}, ARMA~\cite{bianchi2021graph}, GCN~\cite{kipf2016semi} and ChebyNet~\cite{defferrard2016convolutional}; 
	(2) discrete-time approaches. \textbf{DySAT}\cite{sankar2020dysat} uses attention networks to model both temporal and structural information. Differently, \textbf{EvolveGCN}\cite{pareja2020evolvegcn} models the evolution of parameters in GCN over time; (3) continuous-time approaches. \textbf{JODIE}\cite{kumar2019predicting} and \textbf{TGAT}\cite{xu2020inductive} adopt linear and sinusoidal functions of the elapsed time to encode the temporal information, respectively. \textbf{TGN}\cite{rossi2020temporal} combined the merits of temporal modules in JODIE, TGAT and DyREP to achieve the performance improvement. We omit the results of CTDNE\cite{nguyen2018continuous} and DyREP\cite{trivedi2019dyrep}, since it is known (see \cite{rossi2020temporal,jin2022neural}) that their accuracies are lower than TGN.
	...
	In particular, all models perform poorly after time step $43$, when there was an external sudden change --- the shutdown of dark market, which results in the universal performance degradation. {\mname} achieves $68\%$ and $55\%$ relative Micro-F1 gains over the best baseline during time step $[43,46]$ and $[47,49]$, respectively. This proves the merit of {\mname} in effective adaptation under sharp changes.
	\begin{table*}[tb!]
...
% 		\vspace{-10pt}
		...
		\resizebox{.68\textwidth}{!}{
			\begin{tabular}{c cccccc}\toprule\toprule
				...
				0.22755 & 0.39623 & 0.48232 &
				0.14541 & 0.18219 & 0.19611 \\
				\textbf{{\mframe} \textit{perturb.} TGAT} & 
				0.24832 & 0.42487 & 0.50587 & 
				0.15649 & 0.19550 & 0.20865
				\\\bottomrule\bottomrule
		\end{tabular}}
		% \vskip -0.2in
	\end{table*}
	...
	\textbf{Protocol.}
	We follow the protocol used in\cite{li2018diffusion} to study the performance for traffic forecasting task, where we aggregate traffic speed readings at $5$-minute level and apply z-score normalization. We utilize first $70\%$ of data for model training, $10\%$ for validation while the remaining $20\%$ data for testing. Each of compared models is allowed to leverage the traffic conditions up to now for $15/30/45$-minutes ahead forecasting. We also adopt the mean absolute error (MAE), rooted mean squared error (RMSE) as well as mean absolute percentage error (MAPE) to evaluate the model accuracies. Detailed formulations of these metrics can be found in \cite{li2018diffusion}.
	
	\textbf{Baseline.}
	Existing traffic forecasting methods are mainly focused on discrete-time graphs, {as the traffic data is often collected in a constant rate (e.g., every 5 minutes)}. These approaches maintain a sliding window of most recent 60-minutes data (i.e., $12$ snapshots) to make $15/30/45$-minutes ahead forecasting. \textbf{DCRNN}\cite{li2018diffusion} leverages GCN to encode spatial information, while \textbf{GaAN}\cite{zhang2018gaan} uses GAT to focus on most relevant neighbors in spatial domain. These methods both exploit RNNs to capture the evolution of each node. To increase the scalability, \textbf{DySAT}\cite{sankar2020dysat} utilize attention networks to incorporate both structural and temporal dynamics while \textbf{STGCN}\cite{yu2018spatio} resorts to simple CNNs. \textbf{DSTAGNN}\cite{lan2022dstagnn} integrate the attention and gated CNN for improvements. For comparison completeness, we also report the results of static graph approaches, including APPNP\cite{steck2019markov}, ChebyNet\cite{defferrard2016convolutional}, GAT\cite{velivckovic2017graph}, SAGE\cite{hamilton2017inductive}, ARMA\cite{bianchi2021graph} and GCN\cite{kipf2016semi}.
	
	
	Following \cite{li2018diffusion,yu2018spatio}, the embedding size is set to $32$ and the sliding window size for discrete-time approaches, i.e., DCRNN, GaAN, DySAT, STGCN and DSTAGNN, is $12$. The hyperparameter is tuned by grid search analogous to previous tasks. Note that {\mname} and static graph models leverage only the most recent snapshot to make multi-step traffic forecasting, or namely the window size is one. Thus, the comparison is in favor of discrete-time approaches. 
	
	
	\textbf{Results.}
	Table \ref{tab:traffic} presents the results on the METR-LA dataset for $15/30/45$ minutes ahead forecasting. Dynamic approaches outperform static ones by a notable margin, and {\mname} achieves the best results among baselines. We note that the protocol is in favor of discrete-time approaches: they accept $12$ graph snapshots as input, while {\mname} accesses to only $1$ snapshot. Also, {\mname} takes 1.1 hour training time, 6X faster than recently published DSTAGNN.
			
	\begin{figure*}[tb!]
		\centerline{\includegraphics[width=.98\textwidth]{pics/ablation.pdf}}
% 		\vskip -0.1in
		\caption{\label{fig:abl}Ablation study on Netflix for \textbf{dynamic link prediction (left)}, on Elliptic for \textbf{dynamic node classification (middle)} during step $[43,46]$, on META-LA for 30 minutes ahead \textbf{traffic forecasting (right)}. The higher HR@10 and Macro-F1 mean are better, while lower RMSE is better. }
	%	\vskip -0.2in
	\end{figure*}
    \begin{figure*}[tb!]
...
% 		\vskip -0.1in
	...
	We see that {\mname} w/o EndoDE performs worse than {\mname} for all three tasks which suggests the necessity of modelling entangled spatiotemporal information; 
	...
	In Table~\ref{tbl:perturb}, we apply the intra-perturbation to BERT4REC in order to remove spectral content at frequencies greater than $\lambda_{500}$. We see that \textit{perturb.} BERT4REC exhibits a significant drop in performance, which demonstrates the necessity of high-frequency information. Meanwhile, we use BERT4REC to perturb the predictions of GSIMC so as to measure the importance of high frequencies for GSIMC. The fact that BERT4REC \textit{perturb.} GSIMC performs much better than GSIMC and comparable to BERT4REC, 
	indicating that BERT4REC beats GSIMC due to the ability of learning high-frequency information on dynamic graph.
	...
	We have presented a general pipeline for dynamic graph representation learning, where the TPP-modulated attention networks encode the continuous-time graph with its history, the principled family of loss functions accounts for both prediction accuracy and history fitting, and the quantitative analysis based on scalable spectral perturbations provides a global understanding of the learned models on dynamic graphs. Extensive empirical results have shown the universal applicability and superior performance of our pipeline, which is implemented by the DGL library which further endows our work's potential impact and applicability for real-world industry settings.  
