...
The late time cosmic acceleration may be due to exotic matter components in the Universe. Another possibility is that this cosmic speed-up might be caused within GR by the dark energy. The acceleration could be due to purely gravitational effects that emerge from MoG. It may also be the case that GR produces the observational consequences of dark energy through additions in the matter section beyond a cosmological constant \cite{CANTATA:2021ktz}. Alternatively, it may be that the dynamics of dark energy are a result of the additional dynamical behavior of modified theories of gravity. The aim of our work is to combine data from these different systems and to show that more robust constraints of modified theories of gravity can be obtained from combined systems. Although these systems are from different scales, the combined constraint yields a better bound on the parameters. Combining both the cosmological scale tests and the strong field tests of astrophysical systems allows us to obtain higher precision constraints on the free parameters of modified gravity systems.

%\textcolor{red}{Many MoG theories satistfies the solar systems constraint where only consistent $f(R)$ models which evade the solar system constraints have a chameleon mechanism gravity~\cite{Brax:2008hh,Capozziello:2007eu,Katsuragawa:2019uto}. In this paper we suggest a novel method also to test these theories vs. GR, by adding the combined $\chi^2$ despite being in different systems}
...
%\subsection{f(R) as an extension to GR}

A natural approach to MoG theories is to replace the Ricci scalar $R$ in the Einstein-Hilbert action with an arbitrary function of the Ricci scalar \cite{delaCruz-Dombriz:2006kob,Sotiriou:2008rp,Mohsenzadeh:2012ka,Nojiri:2017ncd,Nojiri:2010wj,Pogosian:2007sw,Nesseris:2015fqa}
%
\be 
    S=\frac{1}{16\pi G}\int d^{4}x \sqrt{-g}\,f(R) + S_{m}\,, \label{f_R_action} 
\ee
%
where $G$ is the Newtonian constant, and $S_{m}$ is the matter term in the action, and $g$ is the determinant of the metric. Any modified gravity model should fit the conventional standard cosmology as well as explain the current cosmic acceleration issue and the growing cosmological tensions crisis. Moreover, in order to be able to drive the late time cosmic acceleration, the effective dark energy should be $w_{de} = -1$ asymptotically. The cosmological dynamics in modified gravity is analyzed in many works~\cite{Clifton:2011jh,CANTATA:2021ktz} across the various flavors of MoG. The equivalent of the Einstein equation in $f(R)$ gravity reads
%
\be 
    f'(R) R_{\mu\nu}-\frac{1}{2}f(R) g_{\mu\nu} - \Box_{\mu\nu} f'(R) = 8\pi G T_{\mu\nu}\,, \label{gravi_eq_fR} 
\ee
%
where $\Box$ is the usual notation for the covariant D'Alembert operator $\Box\equiv\nabla_{\alpha}\nabla^{\alpha}$ where we interpret the effect of modified gravity as an extra stress-energy contribution which is possible at background level, and the operator $\Box_{\mu\nu} = \left(\nabla_{\mu}\nabla_{\nu}-g_{\mu\nu} \Box\right)$. Compared to GR, $f(R)$ gravity has one extra scalar degree of freedom, $f'(R)$. The dynamics of this degree of freedom is determined by the trace of the field equations in Eq.~(\ref{gravi_eq_fR}), which gives
%
\begin{equation}
...
%R- \frac{m^2 c_1}{c_2}+\frac{m^2 c_1/c_2}{1+c_2 (R/m^2)^n}
% = ,
\end{equation}
...
The Starobinsky model~\cite{Starobinsky:2007hu} is also an interesting model of this class of MoG in that it has an impact both in inflation as well as at later times of cosmic evolution. This model is defined as
...
\begin{equation}
    R = 6 H^2 + 12 \dot{H}\,,
\end{equation}
where $H = \dot{a}/a$ is the Hubble parameter. Based on the equations of motion for the $f(R)$ model, Refs.~\cite{Sultana:2022qzn,Basilakos:2013nfa,Abuter:2018drb,Sultana:2022qzn,Rusyda:2022ywg} show that for small values of the parameter $b$, one is always able to find an analytic approximation to the Hubble parameter that works to a level of accuracy better than $\sim 10^{-5}\%$ for small $\beta$. The model approximation gives a Friedmann equation
\begin{equation}
    \frac{H(z)^2}{H_0^2}= 1 - \Omega_m+(1+z)^3 \Omega _m + \beta \tilde{\alpha}(z) + \beta^2 \tilde{\beta}(z)\,,  \label{approx1}
\end{equation}
with the expansion of $H(z)$ defined in \cite{Abuter:2018drb,Sultana:2022qzn} from which the coefficients $\alpha$ and $\beta$ can be obtained. In the above expressions, we take $\Omega_{r} = 0$ to simplify the equations, since we use the late time data of the Universe, where the radiation density parameter is negligible. The corresponding expression fits for $\Lambda$CDM when $\beta\rightarrow 0$. The cosmological likelihood $\chi^2_{cosmology}$ is the same as in \cite{Staicova:2021ntm}, with the EOS defined as above.
...
\caption{\it{The posterior distributions for the additional parameter from the modified gravity model: \textbf{Upper:} the Hu-Sawicki model, \textbf{Lower:} the Starobinski model. }} \label{fig:HS_Star_compare}   
\end{figure}
...
release (DR16), which come as angular (DM) and radial (DH) measurements and their covariance. A description of the dataset can be found in Ref.~\cite{Staicova:2021ntm}. This choice of points allows us to integrate out the dependence on $H_0 r_d$ which allows us not to calibrate our cosmology with the early or the late Universe. The description of our marginalization approach can also be found in  \cite{Staicova:2021ntm}. 

\item \textbf{Pantheon} -- Type Ia Supernovae (SNeIa) distance moduli measurements from the Pantheon sample (SN) consisting of 1048 SNeIa in the range $0.01 < z < 2.3$ \cite{Scolnic:2017caz} divided into 40 bins. These measurements constrain the uncalibrated luminosity distance $H_0 d_L$, or in other words the slope of the late-time expansion rate. In this dataset, we marginalise $M_B$ and $H_0$. This is done again to avoid having these quantities as free parameters. 

\item \textbf{S2} - From the galactic center, our analysis uses publicly available astrometric and spectroscopic data that have been collected during the past thirty years. We use $145$ astrometric positions spanning a period from 1992.225 to 2016.38 from \cite{2017ApJ...837...30G}. The data come from speckle camera SHARP at the ESO New Technology Telescope \cite{1993Ap&SS.205....1H}, measurements were made using the Very Large Telescope (VLT) Adaptive Optics (AO). The data include the location and velocities but also the precession rate of the S2 star. 
We perform a Bayesian statistical analysis to constrain the additional parameter from the modified model. 
%A repetition of the paragraph above:
%{Our analysis uses publicly available astrometric and spectroscopic data that have been collected during the past thirty years. We use $145$ astrometric positions spanning a period from 1992.225 to 2016.38 from \cite{2017ApJ...837...30G}. The data come from speckle camera SHARP at the ESO New Technology Telescope \cite{1993Ap&SS.205....1H}, measurements were made using the Very Large Telescope (VLT) Adaptive Optics (AO). } 
\end{itemize}


We use an affine-invariant Markov Chain Monte Carlo (MCMC) sampler \cite{ForemanMackey:2012ig} for the minimization of our likelihoods via the implementation of the open-source package $\text{Polychord}$ \cite{Handley:2015fda}. We also use the $\text{GetDist}$ package \cite{Lewis:2019xzd} for the analysis and illustration of our results. For simplicity, we use the angles reported by the Gravity collaboration as a Gaussian prior. For the rest, we assume a uniform prior of the initial location and velocities. The priors on the parameters we use, thus are: $\beta\in[0,1]$, $x_0\in[1000,\,1500]$, $y_0\in[1300,\,1700]$, $v_{x0}\in[0,\,90]$, $v_{y0}\in[-150,\,-50]$, and $\Omega_m\in[0.1,0.5]$. Furthermore, we use the following Gaussian priors: $M_{Sgr*}=(4.261 \pm 0.012) 10^{6} M_{\odot}$ , $d=8.2467\pm 0.0093 \mathrm{kpc}$, $\Omega=228.171\pm 0.031$, $\omega=66.263\pm 0.031$, $i=134.567\pm 0.033$ The priors on  $\Lambda$ and $\delta$ are discussed in the next section.  
...
%\begin{figure*}
%    \centering
%\includegraphics[width=0.9\textwidth]{Triangle_S2.pdf}
    %\caption{\it{Inferred constraints on all the Hu-Sawicki model parameters with the S2 star data.}}
    %\label{fig:HS_full_S2}
%\end{figure*}
...
\begin{equation}
f(R) = R + \frac{\alpha}{6}R^2
\end{equation}



\begin{equation}
\delta = -\frac{\alpha  R_0}{3}, \quad \lambda^2 = \frac{2 \alpha }{\delta  +1}
\end{equation}


Old notations, to change in the jupyter file:
\begin{equation}
\delta = \alpha R_0, \quad \lambda^2 = \frac{\alpha}{1+\delta}
\end{equation}
...
\begin{equation}
\Phi (r) = -\frac{G M}{r}\frac{1 + \delta \exp\left(-r/\lambda\right)}{1+\delta}
\end{equation}
...
\begin{equation}
f(R) = R-2 \Lambda  \left(1-\exp \left(-\frac{R}{\beta }\right)\right)
\end{equation}



\begin{equation}
\delta = -\frac{2 \Lambda}{\beta } e^{R_0/\beta}, \quad \lambda^2 = \frac{6 \delta  }{\beta\left(  \delta + 1\right)}
\end{equation}
...
\begin{equation}
f(R) = R-2 \Lambda  \tanh \left(\frac{R}{\beta  \Lambda }\right)
\end{equation}
...
\begin{equation}
\delta = -\frac{2 \text{sech}^2\left(\frac{R}{\beta  \Lambda }\right)}{\beta }, \quad \lambda^4 = \frac{72 \delta^2 (\beta  \delta +2)}{\beta ^2 (\delta+1)^2 \Lambda ^2}
\end{equation}

\begin{widetext}
\begin{equation}
\tilde{\alpha} = \frac{6\left(1-\Omega _m\right)^2}{(1+z)^9}\frac{ 4 \left(-1+\Omega _m\right)^2+(1+z)^3 \left(-1+\Omega _m\right) \Omega _m-2
   (1+z)^6 \Omega _m^2}{ \left(\frac{4 \left(-1+\Omega _m\right)}{(1+z)^3}-\Omega _m\right)^3}
\end{equation}
\begin{equation}
\begin{split}
\tilde{\beta} = \frac{\left(-1+\Omega_m\right)^3}{(1+z)^{24}
   \left(-\frac{4 \left(-1+\Omega _m\right)}{(1+z)^3}+\Omega _m\right)^8}\times \tilde{\gamma}.
\end{split}
\end{equation}
\begin{equation}
\begin{split}
\tilde{\gamma} =  1024 \left(-1+\Omega_m\right)^6+9216 (1+z)^3 \left(-1+\Omega _m\right)^5 \Omega_m-22848 (1+z)^6 \left(-1+\Omega _m\right)^4 \Omega_m^2
\\+25408 (1+z)^9 \left(-1+\Omega _m\right)^3 \Omega_m^3- 7452 (1+z)^{12} \left(-1+\Omega _m\right)^2 \Omega_m^4-4656 (1+z)^{15} \left(-1+\Omega _m\right) \Omega_m^5+37 (1+z)^{18} \Omega_m^6.
\end{split}
\end{equation}
\end{widetext}
...
\hline
 Parameter &  SN+CC+BAO & SN+CC+BAO+S2 & SN+BAO+S2 & SN+BAO+S2+$M_B$ & SN+CC+BAO+S2+$M_B$\\
\hline
$\Omega_c$ & $0.1159\pm 0.0093$ & $0.1177^{+0.0091}_{-0.0074}$ & $0.0880^{+0.034}_{-0.051}$ & $0.1523\pm 0.0065$ & $0.1372\pm 0.0064$ \\

$H_0$ & $69.04\pm 0.99$ & $69.38^{+0.92}_{-0.81}$ & $68.60^{+1.9}_{-2.9}$ & $72.62^{+0.64}_{-0.50}$ & $71.33\pm 0.61$\\

$f_{R0}$ & $> -0.0241$ & $> -0.0281$ & $> -0.0862$ & $> -0.00647$ & $> -0.0122$\\

$M_{B}$ & $-19.404\pm 0.039$ & $-19.392^{+0.041}_{-0.027}$ & $-19.550^{+0.18}_{-0.22}$  & $-19.260^{+0.021}_{-0.018} $ & $-19.310^{+0.019}_{-0.017} $\\
\hline
$x_0$ & $-$ & $1209^{+110}_{-140}$ & $1124^{+49}_{-92}$  & $1243^{+200}_{-200}$ & $1157^{+200}_{-100}$\\

$y_0$ & $-$ & $1527^{+120}_{-85}$ & $1483\pm 95$ & $1504\pm 91$ & $1399^{+53}_{-81}$\\

$v_{x0}$ & $-$ & $48.0^{+5.3}_{-7.4}$ & $48.0\pm 6$  & $46.0\pm 7$ & $42.9\pm 2.2$\\

$v_{y0}$ & $-$ & $-98.4^{+5.2}_{-8.0}$ & $-92.8^{+5.0}_{-3.5}$  & $-99.6^{+8.2}_{-9.5}$ & $-92.6^{+8.9}_{-10} $\\

$M_{S}$ & $-$ & $4.47^{+0.80}_{-0.32}$ & $3.88\pm 0.52$ & $4.49^{+0.51}_{-0.66}$ & $3.61\pm 0.67$\\

$d$ & $-$ & $8.23^{+0.52}_{-0.19}$ & $7.85^{+0.40}_{-0.34}$  & $8.26^{+0.32}_{-0.37}$ & $7.67\pm 0.47$\\

$\delta$ & $-$ & $0.252^{+0.13}_{-0.069}$ & $0.180^{+0.096}_{-0.12}$  & $0.182^{+0.098}_{-0.14}$ & $0.232^{+0.13}_{-0.097}$\\

$Om$ & $-$ & $220.28\pm 0.22$ & $220.32\pm 0.18$  & $220.31^{+0.15}_{-0.14}$ & $220.35\pm 0.21$\\

$om$ & $-$ & $57.1\pm 3.3$ &  $56.0^{+2.2}_{-3.2}$ & $58.4^{+3.5}_{-5.4}$ & $58.4^{+1.7}_{-2.1}$\\

$i$ & $-$ & $134.60\pm 0.20$ & $134.53^{+0.19}_{-0.24}$  & $134.65^{+0.17}_{-0.14}$ & $134.61\pm 0.21$\\
\hline
$\Omega_m$ & $0.298\pm 0.013$ & $0.300\pm 0.012$ &  $0.255^{+0.056}_{-0.064}$  & $0.333\pm 0.009$ & $0.318\pm 0.011$\\
\hline
\end{tabular}
\caption{Derived model parameter constraints with the indicated cosmological and S2 data sets. The first and last (derived) set of parameters correspond to the Hu-Sawicki cosmological parameters, while the parameters in the second set correspond to the S2 star system.}
\end{table*}

%\begin{figure*}
%    \centering
%    \includegraphics[width=0.47\textwidth]{Dec_mock.pdf}
%    \includegraphics[width=0.47\textwidth]{RA_mock.pdf}
%    \includegraphics[width=0.9\textwidth]{RA_Dec_mock.pdf}
%    \caption{\it{Illustration of the current S2 star astrometric positions and the mock astrometric positions data set which have been used in our analyses. The dashed line indicates the latest time interval of the adopted publicly available astrometric positions.}}
%    \label{fig:mock_data}
%\end{figure*}
...
\begin{figure*}
    \centering
%\includegraphics[width=0.9\textwidth]{Triangle_compare_cosmo.pdf}
\includegraphics[width=0.9\textwidth]{Triangle_compare_cosmo_MB.pdf}
    \caption{\it{Inferred constraints on the Hu-Sawicki model with cosmological data as well as with the S2 star data.}}
    \label{fig:HS_compare}
\end{figure*}

\begin{figure}
    \centering
\includegraphics[width=0.49\textwidth]{Whisker_plot.pdf}
    \caption{\it{Whisker plot illustrating the inferred $f_{R0}$ constraints from the respective data sets.}}
    \label{fig:Whisker_plot}
\end{figure}

\begin{figure}[]
        \centering
\includegraphics[width=0.45\textwidth]{beta_gauss_HSf.pdf}
\includegraphics[width=0.45\textwidth]{beta_gauss_Star1xf.pdf}
\caption{\it{The posterior distributions for the additional parameter from the modified gravity model: \textbf{Upper:} the Hu-Sawicki model, \textbf{Lower:} Starobinski model \ds{(recovered $\lambda$)}. }}
        \label{fig:HS_Star_compare}
\begin{tabular}{|c|c|c|c|c|}
\hline
        System & $\beta$ & $\lambda$ & $\delta$& $\Omega_m$\\
        \hline
         \multicolumn{5}{|c|}{HS}\\ 
         \hline
        Cosmology & $0.176\pm 0.11$  & - & - & $0.291\pm 0.014$  \\
         Cosmology+S2 & $0.154\pm 0.109$ &  $0.456\pm 0.299$ &  - & $0.292\pm 0.014$ \\
         \hline
         \multicolumn{5}{|c|}{Starobinsky}\\ 
          \hline
          Cosmology & $0.353\pm 0.218$  & - & - & $0.302\pm 0.017$  \\
         Cosmology+S2 & $0.364\pm 0.220$ & $(5.1\pm 2.8) 10^{-4}$  & $0.512\pm 0.286$ & $0.301\pm 0.012$\\
       %$ Cosmology & $3.532E-01\pm 2.177E-01$  & - & - & $3.019E-01\pm 1.165E-02$  \\
        % Cosmology+S2 & $3.642E-01\pm 2.204E-01$ & $ 5.119E-04\pm 2.846E-04$ & $5.117E-01\pm 2.856E-01$ & $3.012E-01\pm 1.184E-02$\\
           \hline
    \end{tabular}
\end{figure}
...
\begin{table*}[]
    ...
    \begin{tabular}{|c|c|c|c|c|}
\hline
        System & $\beta$ & $\Lambda 10^{-4}$ & $\delta$ & $\Omega_m$\\
        \hline
         \multicolumn{5}{|c|}{HS}\\ 
         \hline
         Cosmology & $0.176 \pm 0.110$  & - & - & $0.292 \pm 0.014$  \\
         S2 & $0.182 \pm 0.222$  &  $ 0.142 \pm 0.21$ & -& -\\ 
         Cosmology+S2 & $ 0.154 \pm 0.108$ & $0.455 \pm 0.299$ &  - & $0.292 \pm 0.014$ \\
         \hline
         \multicolumn{5}{|c|}{Starobinsky}\\ 
          \hline
        Cosmology & $0.353 \pm 0.219$  & - & - & $0.302 \pm 0.012$  \\
         S2 & $0.177 \pm 0.160$  &  $ 4.5 \pm 3.4$& $0.455 \pm 0.335$ & -\\ 
         Cosmology+S2 & $0.275 \pm 0.211$ & $4.9 \pm 2.9$ & $0.508 \pm 0.287$ & $0.302 \pm 0.011$\\
           \hline
    \end{tabular}
...
\begin{figure}[]
        \centering
\includegraphics[width=0.45\textwidth]{beta_gauss_HSf_sm.pdf}
\includegraphics[width=0.45\textwidth]{beta_gauss_Star1xf.pdf}
\caption{\it{The posterior distributions for the additional parameter from the modified gravity model: \textbf{Upper:} the Hu-Sawicki model, \textbf{Lower:} Starobinski model. \ds{Both of them  $\beta \in (0,1), \Lambda \in (0,1e-3), \delta \in (0,1)$}  }}
        \label{fig:HS_Star_compare}   

\begin{tabular}{|c|c|c|c|c|}
\hline
         & $\beta$ & $\Lambda \left(10^{-4} \right)$ & $\delta$ & $\Omega_m$\\
        \hline
         \multicolumn{5}{|c|}{HS}\\ 
         \hline
         C & $  0.176\pm 0.110$  & - & - & $0.291\pm 0.014$  \\
         C+S2 & $ 0.182\pm 0.111$ &  $4.9\pm 2.8$ &  - & $0.289\pm 0.014$ \\
         \hline
         \multicolumn{5}{|c|}{Starobinsky}\\ 
          \hline
          C & $0.353\pm 0.218$  & - & - & $0.302\pm 0.017$  \\
         C+S2 & $0.364\pm 0.220$ & $ 5.12\pm 2.85$ & $0.512\pm 0.286$ & $0.301\pm 0.012$\\
           \hline
    \end{tabular}
\end{figure}