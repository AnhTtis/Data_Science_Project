...
\dots \vecsequence_{\numseqtimesteps}\} =\vecsequence \in \mathcal{\setstate}^{\numseqtimesteps}$, where $\numseqtimesteps$ is the length of the sequence. The LGE provides control over sequence generation with the help of user-defined conditioning signals that can take two forms: explicit
...
%In Sec.~\ref{sec:nerf_overview}, we review the fundamentals of NeRF models and detail how we combine multiple NeRFs to compose scenes with multiple objects. Sec.~\ref{sec:style_encoder} describes our style encoder, while Sec.~\ref{sec:canonical_volume} shows the employed canonical volume modeling techniques used to allow efficient rendering. In Sec.~\ref{sec:deformation_modeling} we describe deformation modeling for the representation of articulated objects, Sec.~\ref{sec:enhancer} illustrates our feature enhancer model and Sec.~\ref{sec:objs} explains modeling of specialized objects. Lastly, Sec.~\ref{sec:synthesis_training} describes the training procedure.

\subsubsection{Scene Composition with NeRFs}
\label{sec:nerf_overview}
Neural radiance fields represent a scene as a radiance field, a 5D function parametrized as a neural network mapping the current position $\vecpoint$ and viewing direction $\vecdirection$ to density $\vecdensity$ and radiance $\veccolor$. %Given such function and a desired camera pose it is possible to render an image of the scene. This can be achieved by casting a ray $\ray$ through each pixel and sampling 3D points along each ray. The color $\veccolor(\ray)$ associated with each pixel can be computed by integration over the ray:
%\begin{equation}
%    \veccolor(\ray) = \int_{p_n}^{p_f} e^{-\int_{p_n}^{p} \sigma(\ray(s)) ds}\vecdensity(\ray(p))\veccolor(\ray(p)) dp. \label{eq:nerf}
%\end{equation}
%The representation can be extended to the more general case where a field of features with arbitrary size is present by substituting the radiance $\veccolor$ with the desired features $\vecfieldfeature$.
...
%Each radiance field $\netcanonical_i$ is bounded by its associated 3D bounding box $\vecbboxthree_i$.
...
We train all the components of the synthesis model jointly using Adam \cite{kingma2014adam} for 300k steps with batch size 32. We set the learning rate to $1e-4$ and exponentially decrease it to $1e-5$ at the end of training.
 The framework is trained on videos with 1024x576px resolution. We present additional details in \apref{ap:implementation_details_synthesis} and in \apref{ap:training_details_synthesis}.


\subsection{Animation Model}
\label{sec:animation_model}

In this section, we describe the animation model (see Fig.~\ref{fig:architecture_animation_model}), whose task is that of generating sequences of states $\vecsequence \in \mathcal{\setstate}^{\numseqtimesteps}$ according to user inputs. The animation model allows users to specify conditioning signals in two forms. First, conditional signals can take the form of values that the user wants to impose on some object properties in the sequence, such as the player position at a certain time step.
