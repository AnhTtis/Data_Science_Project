...
\author{
{\rm Adam Caulfield$^1$, Norrathep Rattanavipanon$^2$, Ivan De Oliveira Nunes$^1$}\\
$^1$Rochester Institute of Technology, $^2$Prince of Songkla University, Phuket Campus
} % end author
...
\begin{abstract}
Low-end embedded devices are increasingly used in a wide range of ``smart'' applications and spaces.
...
A large portion of these devices are implemented using low-end and bare-metal microcontroller units (MCUs), specifically designed for energy, cost, and spatial efficiency. They are well-suited for and commonly used in safety-critical sensor-based applications such as medical devices~\cite{islam2019design}, vehicular sensors/actuators~\cite{noman2018design}, and sensor/alarm systems~\cite{bahrudin2013development}. Due to cost/energy budgets, MCUs often lack common hardware features used to secure higher-end systems (e.g., memory management units, strong privilege separation, and inter-process isolation). 
...
Unsurprisingly, the absence of these features makes them attractive targets to a wide range of attacks and integrity compromises.
~\cite{deogirikar2017security,stuxnet,giraldo2016integrity}.

In this context, Remote Attestation (\RA)~\cite{smart,vrased,tytan,trustlite,simple,hydra,rata,DAA,Sancus17,scraps,pistis,reserve,sacha} as well Proofs of Execution (\PoX)~\cite{apex, asap} techniques have been proposed as means for a verifier (\vrf) to ascertain the software state on remote MCU prover (\prv). 
...
While an \RA report ensures integrity of a software binary on \prv and \PoX guarantees its execution, they cannot detect runtime attacks that tamper with the program's control flow without modifying the code.
%
...
In possession of both the attested binary and the log of all control flow transfers, \vrf can check whether the reported control flow is valid and is even able to emulate the reported execution (if data inputs are provided, as in~\cite{dialed}).
...
While this satisfies the ability to detect compromises (in general, absence of an \RA report indicates \underline{something is wrong}), this does not ensure {\it auditing} of \log to pinpoint the source of compromises (i.e., to determine \underline{what is wrong}).
...
Furthermore, current techniques can not guarantee \prv remediation when a compromise is detected. % can not guarantee the remediation of compromises, when detected from \log.

In addition, current techniques manage \log in ways that introduce non-trivial challenges. A typical approach is to compute an in-order hash-chain of all entries in \log. While this reduces the required storage (only the latest hash value needs to be maintained by \prv), it requires \vrf to have {\it a priori} knowledge of all valid control flow paths. Without this knowledge, \vrf cannot compute the correct hash result during the verification of \CFA report. Similar to the CFI case, determining all valid control paths is non-trivial and often infeasible. An alternative approach is to store \log in its entirety and send it verbatim to \vrf~\cite{oat}, once the attested execution is over. This eases the verification process. However, as \log grows rapidly, it can quickly fill up \prv limited memory. Due to this limitation, some \CFA techniques (e.g., OAT~\cite{oat} and Tiny-CFA\cite{tinycfa}) are only envisioned for small and self-contained operations.
...
This paper proposes \acron: an Active Control Flow Attestation architecture. \acron addresses aforementioned limitations by composing concepts of \CFA and active RoTs (see Section~\ref{sec:garota}) to guarantee that \vrf always receives \log and is able to remotely remediate \prv state in case of compromise detection. \acron architecture is implemented as an inexpensive and open-source hardware/software co-design.
...
\begin{compactitem}
    \item We propose \acron, the first architecture to guarantee \vrf eventually receives \CFA reports (\log-s) containing the information about \prv execution. % (even during exploits).
    \acron also supports guaranteed healing of \prv when a compromise is detected. These features are obtained through a synergic combination of active RoTs, \CFA, and novel \CFA-specific non-maskable interrupts, realized as a hybrid (HW/SW) design.% (see below).
    ...
    \acron also demonstrates the feasibility of secure \log slicing (introduced in SCaRR~\cite{scarr}) in MCUs and leverages this feature to support fine-grained control flow auditing of large software operations.
...
This work focuses on simple MCUs and aims for minimality of hardware requirements.
We argue that a design that is cost-effective enough for
the lowest-end MCUs could also be adapted and potentially enriched for higher-end devices, with less strict hardware budgets (we discuss alternative designs in Appendix~\ref{sec:alternatives}). Adapting designs in the other direction is more challenging. The choice of a simple device also facilitates reasoning and presenting \acron concepts systematically.

Following these premises, we choose to present a design targeting low-end MCUs based on low-power single-core platforms with only a few kilobytes (KB) of program and data memory (such as Atmel AVR ATmega and TI MSP430). They feature $8$- and $16$-bit CPUs, typically running at $1$-$16$ MHz clock frequencies, with $\approx64$ KB of addressable memory. SRAM is used as data memory ($DMEM$) ranging in size between $4$ and $16$ KB, while the rest of the address space is available for program memory ($PMEM$). They run software on ``bare metal'', execute instructions in place (physically from $PMEM$), and have no memory management unit (MMU) to support virtual memory.
...
Nevertheless, we expect \acron design to generalize to other bare-metal MCUs (e.g., ARM Cortex-M).

\subsection{Remote Attestation (\RA)}\label{sec:bg_ra}

%%Ivan: do not use 's after acronyms or symbols, e.g., \prv's.

\RA is a challenge-response protocol between two parties: a verifier (\vrf) and a potentially compromised prover (\prv).
It allows \vrf to remotely assess \prv trustworthiness by measuring the content of \prv memory.
...
software-based (a.k.a. ``keyless''), hardware-based, or hybrid. Software-based architectures~\cite{KeJa03, SPD+04, SLS+05, SLP08} require no hardware support. However, \RA must be local (e.g., over a wired communication) and requires several strong assumptions about \adv capabilities, implementation optimality, and fixed communication delays, which are often infeasible in practice~\cite{SW-Attsnt-Attacks}. Hardware-based architectures~\cite{PFM+04, KKW+12, SWP08} rely on standalone cryptographic coprocessors (e.g., TPMs~\cite{tpm}) or complex support from the CPU instruction set architecture (e.g., Intel SGX~\cite{sgx}). Although these approaches provide strong security guarantees for \RA, their hardware cost is often too expensive and unrealistic for MCUs. Hybrid architectures~\cite{vrased, smart, tytan} focus on low-cost MCUs. They leverage minimal hardware support to store cryptographic secret(s) and to support secure execution for a software implementation of the integrity-ensuring function (MAC or signature) computed during the \RA protocol. Hybrid architectures aim to combine the low hardware cost of software-based approaches with the security guarantees offered by hardware-based approaches.

VRASED~\cite{vrased} is a formally verified hybrid \RA architecture.
VRASED implements the authenticated integrity-ensuring function in software while introducing small trusted hardware to enforce correctness of, and absence of \RA key leakage from, this software execution. In addition, VRASED guarantees that the attested memory is temporally consistent, i.e., not modifiable during the memory measurement. We further elaborate on \RA related work in Section~\ref{sec:relatedwork}.
As we discuss in Sections~\ref{sec:design_overview} and~\ref{sec:design}, \acron hybrid design leverages VRASED to replace costly hardware hash engines and to authenticate \CFA reports due to its low cost and public availability. %This approach leads to significantly less hardware overhead. Section~\ref{sec:eval} details \acron cost and compares it to prior work on \CFA.
...
To prevent replays, \chal must be a monotonically increasing counter, and the latest \chal used to successfully authenticate \vrf must be stored in \prv persistent and protected memory. In each \RA request, incoming \chal must be greater than the stored value. Once an \RA request is successfully authenticated, the stored value is updated accordingly. As discussed later in Section~\ref{sec:design}, \acron also uses this \vrased extension to authenticate remediation decisions made by \vrf based on \prv reported state. 

\subsection{Control Flow Attestation (\CFA)}

%%Ivan: Redundant!!! This was already said...
%runtime attacks that do not modify binaries cannot be detected by static \RA.
%For instance, vulnerabilities such as buffer overflows can provide an adversary the ability to hijack the program's control flow (or to launch ROP attacks). These code reuse attacks do not modify \prv's binary and hence do not affect static \RA measurements.
%For this reason,
...
However, it does not satisfy \vrf ability to
{\it securely auditing} the source of an exploit: the control flow violation leading to the exploit may never be received by \vrf; thus \vrf cannot easily pinpoint the vulnerability. One of \acron key contributions is to enable {\it secure runtime auditing}, i.e., guaranteeing delivery of \log, even when \prv is compromised by malware that prevents \prv from sending \CFA reports to \vrf. Naturally, this guarantee holds when \adv is unable to jam the network indefinitely. In the case where \adv has such capability and \vrf never receives reports, \acron can optionally halt execution on \prv.
...
Recently proposed \emph{active} RoTs~\cite{garota,proactive1,proactive2,proactive3,aion}, on the other hand, focus on availability guarantees under software compromise.
%
In particular, GAROTA~\cite{garota} is a generalized interrupt-based active RoT designed as a hardware monitor for low-end MCUs. Its goal is to support a secure association between a \trigger event (e.g., ``temperature exceeds a threshold'') and the correct execution of a software function responsible for a safety-critical action (e.g., ``sound the alarm'') whenever the \trigger event occurs. This guaranteed \trigger-action association must hold even when the MCU software is compromised.
...
Figure~\ref{fig:phases} illustrates \acron execution workflow alongside \acron HW module responsible for generating and protecting \log, and issuing the \trigger interrupt when a \CFA report must be sent to \vrf. 
...
%
% OAK: change "Read" CFlog to measure CFlog
%% Adam: done
\begin{figure}[t]
    \centering
    \includegraphics[width=0.79\columnwidth]{./phases4.png}
...
We highlight two important consequences of \acron design and execution workflow. Even compromised software on \prv is unable to preclude sending of \log to \vrf, as \trigger cannot be disabled due to the active RoT guarantees and the sending function is implemented within the (atomically executed) \tcb. Therefore, \vrf receives \log even in case of \prv compromise, 
enabling auditing of the exploit's control flow to identify the vulnerability source. Similarly, in case of compromise detection, the healing function \tcb-Heal is also implemented within \tcb and cannot be avoided by any external interrupt attempts originating from potentially compromised software on \prv.
...
An end-to-end implementation of such applications deployed using \acron is presented in Appendix~\ref{sec:end_to_end}.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\columnwidth]{./system-overview5.png}
    % \vspace{-1em}
...
In addition, \adv has a Dolev-Yao~\cite{dolev-yao} capability with respect to the network. Therefore, it may discard, inject, or attempt to modify messages between \prv and \vrf. % They may launch this attack to render the device unavailable or to continue running malicious software.
...
\er location and size are configurable. Therefore, \vrf can choose what should be attested: the execution of a code segment, a single function, multiple functions, or the entirety of $PMEM$.
...
%
    \item Finally, \vrf evaluates \prv reported execution trace based on \log and its size ($\ptr$), where $\ptr$ is located inside $METADATA$. % (i.e., the trace corresponds to the first $\ptr$ entries in \log)
...
% OAK: is the paragraph below more like P5 TCB integrity?? I commented it out.
...
% OAK: better way to phrase the sentence below??
% OAK2: sorry for being unclear, I meant a better way to phrase it in terms of security, not functionality. I kept the old one for now.
...
% OAK: unnecessary to mention below
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\columnwidth]{./hardware-overview.png}
    % \vspace{-1em}
...
We use of Xilinx Vivado tool-set to synthesize \acron hardware. \acron hardware is written in the Verilog hardware description language and implements each of \acron sub-modules according to the logic defined in Section~\ref{sec:design}.
In total, \acron hardware is implemented in 2042 lines of Verilog code. The \CFA module accounts for 982 lines, the Active RoT module (including VRASED) accounts for additional 927 lines
, and 123 lines tie the two modules together. 
\acron functionality was tested using the Vivado simulator to ensure its correctness. We then synthesized and deployed \acron on the Basys3 board, which features a commodity Artix-7 FPGA. Once a hardware design is synthesizable on an FPGA, the same design can be manufactured into an application specific integrated circuit (ASIC) for larger scale production.

The software \tcb implements functions, \tcb-Att, \tcb-Wait, and \tcb-Heal, as described in Section~\ref{sec:design}. All three functions are linked so that the entire \tcb code is located within a contiguous region of $PMEM$. This ensures that \tcb can be monitored and protected as intended by the \acron hardware.
%
...
As explained in Section~\ref{sec:design_overview}, \tcb-Heal is configurable and can vary depending on application domain needs. 
...
\begin{figure}[t]
    \centering
    \subfigure[MSP430-based implementations]{\label{fig:hw_msp430}\includegraphics[width=0.49\columnwidth]{./hw_msp430.png}}
    \subfigure[\CFA-specific architectures]{\label{fig:hw_hwb}\includegraphics[width=0.49\columnwidth]{./hw_vs_acfa.png}}
    % \vspace{-1em}
    \caption{Comparison of Hardware Cost}
    % \vspace{-1em}
    \label{fig:hwcost}
\end{figure}
...
Due to its hybrid design, \acron incurs significantly lower hardware overhead than hardware-based approaches such as SANCUS, as shown in Figure~\ref{fig:hw_msp430}.
...
Figure~\ref{fig:hw_hwb} compares \acron with hardware-based \CFA architectures, showing that a hybrid design for \CFA requires much less hardware.
% OAK: do you have any exact number?, i.e., how much are we better compared to the cheapest hw-based CFA.
...
The exact runtime overhead incurred by \tcb execution varies depending on factors such as communication delays, \vrf choice of remediation function, and time taken by \vrf to verify reports.
...
We discuss more details and timing results in Appendix~\ref{sec:end_to_end}.
...
We use Vivado to synthesize the hardware specification and generate reports regarding the anticipated energy consumption of \acron.
...
In our implementation, \vrf emulates execution of \prv binary~\cite{oat} using the received \log to determine the destinations of dynamically defined branching instructions. As a part of the abstract execution, it implements a virtual shadow stack to confirm the validity of return addresses reported in \log. It also generates the CFG of the binary to check if the control flow transfers reported in \log match a valid path in the CFG. Aside from this sample implementation, any CFI policy that would otherwise be implemented on the resource-constrained \prv can now be outsourced to the higher-end \vrf for faster and less intrusive runtime integrity verification.
Due to space constraints, we further detail our sample \vrf implementation in Appendix~\ref{sec:end_to_end}.
...
\acron implementation is systematically de-constructed into sub-modules which together support \acron required properties.
Based on this set of properties, we argue \acron security.
...
\section{End-to-End Sample Application}
\label{sec:end_to_end}

To demonstrate \acron practicality in on-demand sensing applications,
we fully implement both \prv and \vrf realizing the \acron workflow in real-time. The implementation and a video demonstration of this end-to-end example are also available at~\cite{acfarepo}.


{\bf \prv setup:}
% OAK: who would enter the password? If its \vrf, would it be stupid enough to enter the password leading to the exploit.
In this application, \prv contains a simple program that receives a password from a remote user and compares it with an expected password. 
We purposefully introduce a buffer-overflow vulnerability in this program by not performing an array bound check when storing the user password. % OAK: refer to pseudocode
As a result, \adv could potentially overflow the buffer storing the user password and thus overwrite the return address.
After the correct password is entered, \prv then records six ultrasonic-sensor readings and exits the program.
...
{\bf \vrf Offline Phase (perfomed once):}
...
We implement this task using the \emph{angr} binary analysis tool~\cite{shoshitaishvili2016state, stephens2016driller, shoshitaishvili2015firmalice}. % OAK: how to properly generate a COMPLETE CFG in angr?? Is it not possibe?
In addition, \vrf prepares to verify the \acron report (Step 4 in Figure~\ref{fig:prot}) by precomputing the hash of $AER$.
...
\ignore{
The first task includes computing the \prv program's expected \log offline.
We implement this task using the \emph{angr} binary analysis tool~\cite{shoshitaishvili2016state, stephens2016driller, shoshitaishvili2015firmalice}.
In particular, we leverage \emph{angr}'s Simulation Manager~\cite{angrdocs} to simulate \prv program's execution given a specific concrete input.
In the simulation, upon encountering a control flow transfer, we  extract the source and destination addresses of the instructions causing this transfer and record them into the expected \log.
To ensure accurate/secure returns from function calls, 
we also maintain a shadow stack throughout this simulation.
After executing each return instruction, we append the shadow stack's return address into the expected \log.
At the end of this task, \vrf produces \log of the expected/uninterrupted execution of \prv software, which will later be used to verify \log sent by \prv in the online phase.
}
\ignore{
which allows \vrf to step through the binary one CFG node at a time.
Before stepping to the next node, our \vrf implementation extracts the source and destination addresses of the instructions causing the transition,
i.e. builds a \log of the expected/uninterrupted execution of \prv software.
This simulated \log is used to verify \log slices sent by \prv during the online phase.}
...
\begin{compactitem}
    ...
    \item \textbf{First \acron report.} Recall that a first \acron report is valid if it is produced on \prv immediately after \er is invoked. Thus, in this phase, \vrf checks whether its first entry corresponds to a function call to \er, i.e., the destination address matches $\er_{min}$. The rest of \log entries must strictly follow a valid control flow path right after calling \er. Thus, to validate them, \vrf checks whether these \log entries constitute a valid control flow path originated from $\er_{min}$ using the pre-generated CFG.
    
    \item \textbf{Intermediate \acron reports.} Intermediate \acron reports are generated after \er is invoked and before \er finishes its execution, i.e., while \er is executing.
    In these reports, the first \log entry must correspond to a jump from $\tcb_{max}$ to \er after obtaining \vrf approval from the previous report while other entries must continue the valid control flow path. 
    Hence, \vrf checks the first entry by comparing its source address to $\tcb_{max}$ and its destination to \er region.
    To validate other entries, \vrf keeps traversing CFG based on the control flow path, obtained when validating the first report, and checks whether these entries follow a valid graph traversal.
    
    \item \textbf{Last \acron report.} A last report is created when \er execution completes, resulting in a \trigger to \tcb. The first \log entry in this report is checked the same way as in the intermediate report. \vrf validates the last \log entry by matching it with a control flow transfer caused by a jump from $\er_{max}$ to $\tcb_{min}$. For other \log entries, \vrf continues traversing CFG and matches them with the rest of the control flow path.
    
    \item \textbf{Single \acron report.} In the case that no intermediate reports are created, \vrf obtains a single \acron report that covers the entire \er execution trace. Thus, \vrf can simply check validity of this execution trace via CFG traversal.
\end{compactitem}

In addition, to detect attacks overwriting return addresses, 
we implement a shadow stack on \vrf in the online phase.
While traversing CFG, if \vrf encounters any function call, it pushes the expected return address to this shadow stack. 
...
In our end-to-end example, \vrf may select to disable or enable this shadow stack option from this phase.

\ignore{
\edit{During \acron execution, each received \log slice compared in real-time to the expected \log generated by traversing the CFG during the offline phase. 
The first report received by \vrf shows the entry into \tcb due to a \textbf{[T3]} \trigger at boot, thus the first report should contain one entry.
For all reports, the source for the transfer into \tcb is saved in order to verify exit from \tcb, which will be recorded as the first entry in the next report.}

\edit{For verifying all remaining \log slices, \vrf checks the validity of: 1) the first entry 2) all intermediate entries and 3) the last entry. 
The first entry can be verified by checking if the source is the dedicated \tcb-exit address and by checking if the destination is the same address where \tcb-entry was triggered (value saved from previous report).
Intermediate entries are verified by parsing through the received \log slice and comparing it to the simulated \log.}
% OAK: above, what about last entry?
%% Adam: Sorry, its now added here: 
\edit{Finally, \vrf checks the last entry. For every \log slice, the last entry's destination address should be the dedicated address for \tcb-entry. The last entry's source address should always be an address in $AER$. 
If \vrf has verified all contents of the simulated \log, the source address should be $AER_{max}$.
Otherwise, \vrf saves the source address for the next round of the protocol to verify a proper exit from \tcb occurred.}
%
\edit{In addition to inspecting the simulated \log to match generic branching instructions, \vrf maintains a shadow stack in order to verify each \texttt{ret} instruction returns to the expected call site. In our end-to-end example, \vrf may select to disable or enable a shadow stack while parsing \log for invalid entries.}
}
...
In our demo, \vrf detects an invalid \log in the second instance of the \acron protocol and thus successfully discovers the buffer-overflow attack on \prv. 
...
{\bf Timing Results: } We evaluate the end-to-end demo by timing the protocol for a varied size of $AER$. When the end-to-end demo runs with the benign input, four 256B \log are generated. Table~\ref{tab:demo_timing} presents the average runtime of each step in the protocol from Figure~\ref{fig:prot}. The overall runtime of the protocol increases as the size of the $AER$ increases since more time is required to attest a larger region of program memory. The remaining steps are completed in constant time as they do not depend on the size of $AER$ -- including the time to verify \acron report due to the pre-computation of \er hash in \vrf offline phase. Steps 1-3 and Step 6 require the most time due to the need for communication delays.

\begin{table}[]
\caption{Protocol Timing (in ms) for varying size of $AER$. Steps correspond to the protocol steps of Figure~\ref{fig:prot}}
...
Table~\ref{tab:demo_timing} also demonstrates the overhead incurred by \prv producing a MAC compared to a resource-rich \vrf. 
In Step 5, \vrf produces a MAC ($Auth$) with an average runtime of $\approx$0.07ms. Whereas, in Step 7, \prv produces a MAC ($out$) with an average runtime of $\approx$96.1ms. Since Step 8 (a call to \tcb-Heal or return to \textit{S}) requires negligible time in ms, \prv time to produce a MAC dominates the reported runtime for Steps 7-8 in Table~\ref{tab:demo_timing}.
...
it increments an internal counter $ctr$ to indicate the number of loop iterations that have occurred so far.
% OAK: >=, not >?
Only when $ctr$ is greater than one, the Loop Monitor sets $\mathit{loop_{detect}}$ to $1$. 
...
The Loop Monitor ensures that all instances of loops are detected and their iterations are counted accurately. Because of this, the Loop Monitor ensures that loops are logged into \log efficiently and correctly.

%%Ivan: what happens if the counter overflows?

\section{Alternative Policies \& Designs}\label{sec:alternatives}

In this section we consider potential alternatives in \acron design and policies, as well as their implications:
\begin{compactitem}
\item {\bf Hardware Hash Engines:} \acron design opts for a software implementation of the \RA integrity-ensuring function (from VRASED). This choice significantly reduces the hardware cost. However, it also increases the execution time of \tcb-Wait. We note that, if \prv is implemented with a less strict hardware budget, hardware-based hash engines can be utilized to reduce the software runtime overhead of \tcb-Att. In practice, trade-offs between hardware cost and real-time response should be considered when deciding for one approach over the other.

\item {\bf Strict vs. Relaxed \CFA Auditing and Remediation:} As discussed in Section~\ref{sec:design_overview}, we describe the strict version of \acron protocol, in which auditing software integrity is a first-class priority (e.g., consider an MCU deployed as a part of a nuclear facility). Therefore, whenever a \CFA report is sent to \vrf, \tcb in \prv waits for a response indefinitely (while re-transmitting the report periodically). In that case, to avoid detection, \adv might jam the network communication rendering \prv unavailable. We note that resuming the execution of the attested application in this case is entirely possible (i.e., by jumping from step 2 to step 9 in Figure~\ref{fig:prot} upon a timeout). While this may be desired in some applications domains, it remains unclear why one would aim to guarantee availability to a compromised application. Alternatively, \tcb could resume \er execution for a fixed finite period, issuing a subsequent timer \trigger to check if \vrf response was received. In that way, \prv does not busy-wait on \vrf response. The security implications of this idea clearly need to be considered more carefully.   
 
\item {\bf Non-Control Data Attestation:} Subtle software integrity attacks are still possible by compromising non-control data, without modifying a program's control flow path, to corrupt computation results. While vulnerabilities enabling this class of attacks (e.g., a ``write anywhere'' vulnerability) are less common, they are still possible. One approach to deal with this problem is append all data inputs (any memory read from outside the attested program's stack) to \log as proposed in~\cite{dialed}. In possession of both the executed control flow path and all data inputs, \vrf can abstractly execute~\cite{oat} the attested program locally and observe any such vulnerability exploitation.\\
