...
Here, LCB provides values $\underline{V}$ more close to the optimum and, 
...
We plot bounds under a single fixed strategy $\underline{\sigma}$ on the right. 
...
LCB is also favorable under limited sampling access, e.g., in
...
optimal values. To determine how far the learned strategies depart from the optimum, 
...
With eager scoping, the optimal action may not be in the scope anymore, but the optimal strategy
in the subsystem still performs reasonably well compared to the overall optimal strategy. 
