...
%--------------------
\begin{document}
	\title{Impact of decoherence on the route to equilibrium}
...
\be\label{eq-initial}
\rho(0)=\rho^{\text S}(0)\otimes \frac{\mathbbm{1}_B}{d},
\ee
where $\rho^{\text S}(0) = |\psi_{\text S}(0)\rangle\langle \psi_{\text S}(0)|$ and $d$ denotes the Hilbert space dimension of the bath. The initial state of the bath can be regarded as a state at infinite temperature.
...
Although the derivations of reduced equations of motion for system in the interaction picture using the modified Redfield theory are already known \cite{yang2002influence,seibt2017ultrafast}, it may be helpful to show the main steps here. More detailed derivations can be found in the Appendix.
As a guideline of this section, we would like to mention here, that the derivations given below are almost the same as the standard derivations of the quantum master equations based on the Born-Markov approach, but with a different unperturbed Hamiltonian given in Eq. \eqref{eq-H0}. 
...
\ee
Using the general method of projection operator technique, and keeping perturbation terms up to second order, for initial states satisfying ${\cal P}\rho^{\text S}_I(0) = \rho^{\text S}_I(0)$
one has 
\be\label{eq-drho0}
...
\ee $\Gamma_r$ indicates the relaxation rate, which can be written as
\begin{align}
\Gamma_{r}& =4|\lambda_{12}|^{2}\int_{0}^{\infty}\Re[F_{12}(\tau)]{\text d}\tau
\nonumber \\
...
\begin{align}
[\rho_{I}^{\text S}(t)]_{11}-[\rho_{I}^{\text S}(\text{eq})]_{11}&=e^{-\Gamma_{r}t}([\rho_{I}^{\text S}(t)]_{11}-[\rho_{I}^{\text S}(\text{eq})]_{11}), \nonumber \\
[\rho_{I}^{\text S}(t)]_{22}-[\rho_{I}^{\text S}(\text{eq})]_{22}&=e^{-\Gamma_{r}t}([\rho_{I}^{\text S}(t)]_{22}-[\rho_{I}^{\text S}(\text{eq})]_{22}), \nonumber \\
[\rho_{I}^{\text S}(t)]_{21}&=e^{-\frac{\Gamma_{r}}{2}t}[\rho_{I}^{\text S}(t)]_{21}.\label{eq-result-rdm}
\end{align}
...
Based on the idea of dynamical quantum typicality \cite{sugiura2012typ,elsayed2013typ,steinigeweg2014typ}, the dynamics of the system starting from the pure initial state employed in Eq. \eqref{eq-initial-pure} is almost the same as that of the mixed initial state in Eq. \eqref{eq-initial}, if the dimension of the bath Hilbert space is large enough.
...
\approx & \frac{1}{d}\text{Tr}_{\text B}\{e^{i\lambda_{11}H_{11}t}e^{-i\lambda_{22}H_{22}t}\}. \label{eq-L12-strong}
\end{align}
Making use of the eigenstates and eigenvalue of $H_{mm}$, denoted by $|E_k^{mm}\rangle$ and $E^{mm}_k$, respectively,  $L_{12}^\text{typ}(t)$ can be written as
\be
...
\Gamma_{r}= 4{\cal K}|\lambda_{12}|^{2}\int_{0}^{+\infty}L_{12}^{\text{typ}}(\tau){\text d}\tau.
\ee
...
One can see that the Hamiltonian we consider here is a special case of the Hamiltonian defined in Eq. \eqref{eq-HSimple},  where $\lambda_{22} = -\lambda_{11} = \lambda_d$, $\lambda_{12}=\lambda^*_{12}=\lambda_r$, and  \textit{EC} and \textit{EX} interactions are given by
\be
...
\rho_{11}^{\text S}(t)-\rho_{11}^{{\text S}}(\text{eq})=e^{-\frac{16\lambda_{r}^{2}}{3\pi\sqrt{1+\lambda_{d}^{2}}}t}(\rho_{11}^{\text S}(t)-\rho_{11}^{{\text S}}(\text{eq})).
\ee
...
Here, for example we can define $\tau_B$ and $\tau_R$ to be the time at which $\Re[F_{12}(\tau)]$ and $|\rho_{11}^{\text S}(t)-\rho_{11}^{\text S}(\text{eq})|$ decay to $1\%$ of its initial value and never exceed that value afterwards. Then with straightforward calculations one has
\be\label{eq-tauFR}
...
In the Schr\"{o}dinger picture, the time evolution of off-diagonal elements of RDM is given in Eq. \eqref{eq-rho12-le}. As $|[\rho_{I}^{\text S}(t)]_{21}|$ has already been given above, one only needs to study $L_{12}^\text{typ}(t)$ which characterizes the  decoherence process induced by the \textit{EC} interaction. In case of $\lambda_d \ll 1$, one has $L_{12}^\text{typ}(t) \approx e^{-\Gamma_L t}$. Defining $H_d = H_{11}-H_{22}$,  Eq. \eqref{eq-GammaL} becomes 
\be\label{eq-GammaL}
...
A simple relation between the bath correlation function and the decoherence process induced by the \textit{EC} interaction can also be seen.
...
\begin{align}
\frac{{\text{d}}\rho_{11}^{\text S}(t)}{{\text{d}}t}&=-4|\lambda_{12}|^{2}\int_{0}^{t}\Re[F_{12}(t-s)](\rho_{11}^{\text S}(s)-\rho_{11}^{\text S}(\text{eq})){\text{d}}s, \nonumber \\
\frac{\text{d}\rho_{22}^{{\text S}}(t)}{\text{d}t}&=-\frac{\text{d}\rho_{11}^{{\text S}}(t)}{\text{d}t}, 
\end{align}
as well as for off-diagonal elements,
\be
\frac{{\text d}\rho_{21}^{\text S}(t)}{{\text d}t}=-2|\lambda_{12}|^{2}\int_{0}^{t}\Re[F_{12}(t-s)]\rho_{21}^{\text S}(s){\text d}s,
\ee
where
\be
...
\begin{align}
\frac{\text{d}\rho_{11}^{\text S}(t)}{\text{d}t}&=-\Gamma_{r}(\rho_{11}^{\text S}(t)-\rho_{11}^{{\text S}}(\text{eq})),
\nonumber \\
\frac{\text{d}\rho_{22}^{\text S}(t)}{\text{d}t}& =-\Gamma_{r}(\rho_{22}^{\text S}(t)-\rho_{22}^{{\text S}}(\text{eq})), \nonumber \\
\frac{\text{d}\rho_{21}^{\text S}(t)}{\text{d}t}&=-\frac{\Gamma_{r}}{2}\rho_{21}^{\text S}(t),
\end{align}
which directly leads to
\begin{align}
\rho_{11}^{\text S}(t)-\rho_{11}^{\text S}(\text{eq}) &=e^{-\Gamma_{r}t}(\rho_{11}^{\text S}(0)-\rho_{11}^{\text S}(\text{eq}))\nonumber \\
\rho_{22}^{\text S}(t)-\rho_{22}^{\text S}(\text{eq})&=e^{-\Gamma_{r}t}(\rho_{22}^{\text S}(0)-\rho_{22}^{\text S}(\text{eq})), \nonumber \\
\rho_{21}^{\text S}(t)&=e^{-\frac{\Gamma_{r}}{2}t}\rho_{21}^{\text S}(0).
\end{align}
Here $\Gamma_r$ indicates the relaxation rate, which is defined as
\begin{align}
\Gamma_{r}&=4|\lambda_{12}|^{2}\int_{0}^{\infty}\Re[F_{12}(\tau)]{\text d}\tau \nonumber \\
&=2|\lambda_{12}|^{2}\int_{-\infty}^{\infty}F_{12}(\tau){\text d}\tau.\label{eq-Gamma1}
\end{align}


% \section{Time evolution of Off-diagonal elements of RDM in the GORM model in the interaction picture}

% After deriving the time evolution of the diagonal elements, we continue to study the off-diagonal elements. The discussion of off-diagonal elements are much more complicated compared to the diagonal elements. So in this section we will only consider the spin-GORM model where $H_d$, $H_r$ and $H_{\text B}$ are all random matrices drawn from GOE.
...
% \begin{align}
% \frac{d\rho_{21}(t)}{dt} = & -2\lambda_{r}^{2}\int_{0}^{t}F_{21}(t-s)\rho_{21}^{\text S}(s)ds \nonumber \\
% & -2\lambda_{r}^{2}\int_{0}^{t}G_{12}(t,s)\rho_{12}^{\text S}(s)ds.
% \end{align}
% As shown in Eq.\eqref{eq-F12-1} in the main text, in the spin-GORM model $F_{12}(t-s)$ can be written as
% \be
% F_{12}(t-s)=\frac{e^{-2i\omega(t-s)}}{4d^{2}}\text{Tr}_{\text B}(e^{iH_{\text B}^{(1)}(t-s)})\text{Tr}_{\text B}(e^{-iH_{\text B}^{(2)}(t-s)}).
% \ee
% Similarly, $G_{12}(t,s)$ can also be written as
% \be
% G_{12}(t,s)=\frac{e^{-2i\omega(t+s)}}{4d^{2}}\text{Tr}_{\text B}(e^{-iH_{\text B}^{(2)}t}e^{iH_{\text B}^{(1)}s})\text{Tr}_{\text B}(e^{-iH_{\text B}^{(2)}s}e^{iH_{\text B}^{(1)}t}).
% \ee
...
% \ee
% First we compare the two terms $\text{Tr}_{\text B}(e^{iH_{\text B}^{(1)}(t-s)})$ and $\text{Tr}_{\text B}(e^{-iH_{\text B}^{(2)}s}e^{iH_{\text B}^{(1)}t})$
...
% \be
% G_{12}(t,s)=e^{-2i\omega(t+s)}\text{Tr}_{\text B}\left[H_{r}e^{-iH_{\text B}^{(2)}t}e^{iH_{\text B}^{(1)}s}H_{r}e^{-iH_{\text B}^{(2)}s}e^{iH_{\text B}^{(1)}t}\right].
% \ee
...
% $H^{(1)}_{\text B} = H^{(2)}_{\text B} = H_{\text B}$, and then
% \be
% G_{12}(t,s)=e^{-2i\omega(t+s)}\text{Tr}_{\text B}\left[e^{iH_{\text B}(t-s)}H_{r}e^{-iH_{\text B}(t-s)}H_{r}\right],
% \ee
...
% \be
% {\cal G}_{12}(\tau)=e^{2i\omega\tau}\text{Tr}_{\text B}\left[e^{iH_{\text B}\tau}H_{r}e^{-iH_{\text B}\tau}H_{r}\right].
% \ee
% Employing Makovian approximation, one has
% \begin{align}
% \frac{d\rho_{21}(t)}{dt}= & -2\lambda_{r}^{2}\int_{0}^{\infty}F_{21}(\tau)d\tau\rho_{21}^{\text S}(t) \nonumber \\
% & -2\lambda_{r}^{2}\int_{0}^{\infty}e^{-4i\omega t}{\cal G}_{12}(\tau)d\tau\rho_{12}^{\text S}(t).\label{eq-Drho12-new} 
% \end{align}
...
% \be
% \frac{d|\rho_{12}^{\text S}(t)|}{dt}=-\Gamma_{d}|\rho_{12}^{\text S}(t)|,
% \ee
% where 
% \be
...
% \be
% F_{12}(t-s) = \frac{e^{-2i\omega(t-s)}}{4d}\text{Tr}_{\text B}(e^{-iH_{\text B}^{(2)}(t-s)})\text{Tr}_{\text B}(e^{-iH_{\text B}^{(1)}(t-s)}).
% \ee
