...
The \ac{ourmethod} method consists of four steps. 
The first step is the parameterization of the ``initial situation'' and the possible ``future situations''.
Second, based on the initial situation, we estimate the probability (density) for the possible future situations. 
The third step includes determining the probability of the specified event based on the initial and the future situations.
...
This vector contains all relevant aspects for determining the risk.
...
The remainder of this subsection describes how we estimate $\density{\situationinitial, \situationfuture}$ using \cstart  $\left\{(\situationinitialinstance{\situationindex},\situationfutureinstance{\situationindex})\right\}_{\situationindex=1}^{\situationnumberof}$\cend.
...
\cstart If the shape of the \ac{pdf} is known, a particular functional form can be fitted to the data, e.g., by estimating the parameters of a distribution by maximizing the likelihood.
...
If, however, the shape is unknown, fitting a particular parametric distribution may lead to very inaccurate results \autocite{chen2017tutorial}. \cend
...
\cstart In the remainder of this work, we assume that the shape of the \ac{pdf} $\density{\situationinitial, \situationfuture}$ is unknown a priori. \cend
...
Our proposal is to keep increasing $\numberofsimulations$ until the variance of $\probabilityestcond{\simulationresult \in \spacecollision}{\situationinitial}$ is below a threshold $\simulationthreshold>0$.
...
\end{equation}
Because $\probabilitycond{\simulationresult \in \spacecollision}{\situationinitial}$ is unknown, we use the estimated counterpart of \cref{eq:estimate probability of collision}.
...
Therefore, we propose to evaluate \cref{eq:estimate probability of collision} only for some fixed \cstart $\left\{\situationinitialinstance{\situationindexdesign}'\right\}_{\situationindexdesign=1}^{\numberofdesignpoints}$\cend.
Next, regression is used to estimate \cref{eq:estimate probability of collision}.
\cstart One option is to choose a parametric model, e.g., a logistic model, and estimate the parameters of the model using $\left\{\left(\situationinitialinstance{\situationindexdesign}',\probabilityestcond{\collision}{\situationinitialinstance{\situationindexdesign}'}\right)\right\}_{\situationindexdesign=1}^{\numberofdesignpoints}$.
Up to our knowledge, however, there is no good reason to assume a particular parametric model, so we use a non-parametric regression technique \cend to estimate \cref{eq:estimate probability of collision}.
More specifically, we use the \acf{nw} kernel estimator \autocite{wasserman2006nonparametric}, \cstart because it automatically smooths the data (as is demonstrated in \cref{sec:wang stamatiadis comparison}) and the approximation is guaranteed to give a number between 0 and 1, also when extrapolating the data.
...
\begin{equation}
	...
	\approx \frac{ \sum_{\situationindexdesign=1}^{\numberofdesignpoints}
		...
		\kernelfuncnormalized{\bandwidthnw}{\situationinitial - \situationinitialinstance{\situationindexdesign}'}}.
\end{equation}\cend
Here, \cstart $\probabilityestcond{\collision}{\situationinitialinstance{\situationindexdesign}'}$ \cend is based on \cref{eq:estimate probability of collision} and $\kernelfuncnormalized{\bandwidthnw}{\cdot}$ represents the Gaussian kernel given by \cref{eq:kernel initial future}.
Two important choices have to be made: The choice of \cstart $\left\{\situationinitialinstance{\situationindexdesign}'\right\}_{\situationindexdesign=1}^{\numberofdesignpoints}$ \cend for which to evaluate \cref{eq:estimate probability of collision} and the choice of the bandwidth matrix $\bandwidthnw$.
We suggest to base the design points \cstart $\left\{\situationinitialinstance{\situationindexdesign}'\right\}_{\situationindexdesign=1}^{\numberofdesignpoints}$ \cend on the data that is used to estimate $\densitycond{\situationfuture}{\situationinitial}$ in \cref{sec:estimate future}, i.e., $\situationinitialinstance{\situationindex}$, $\situationindex \in \{1,\ldots,\situationnumberof\}$, such that all $\situationinitialinstance{\situationindex}$ have at least one design point \cstart $\situationinitialinstance{\situationindexdesign}'$ \cend nearby.
In other words, \cstart $\left\{\situationinitialinstance{\situationindexdesign}'\right\}_{\situationindexdesign=1}^{\numberofdesignpoints}$ is \cend chosen such that \cstart
\begin{equation}
...
\cstart Note that if $\bandwidthnw$ is a diagonal matrix with positive values on the diagonal that are close to zero, then the \ac{nw} kernel estimation of \cref{eq:nadaraya watson} acts like nearest-neighbor interpolation. \cend
