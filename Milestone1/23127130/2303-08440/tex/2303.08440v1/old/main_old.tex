...
TPDM has been tested in various 3D medical imaging reconstruction problems such as MRI Z-axis (\ie slice direction) super-resolution (MR-ZSR), compressed sensing MRI (CS-MRI), and sparse view CT (SV-CT), and has produced the state-of-the-art results compared to existing methods. 
 It outperformed existing methods and, to the best of our knowledge, achieved the first successful attempt at MR-ZSR for 
...
Diffusion posterior sampling (DPS) is the state-of-the-art method to solve the general noisy inverse problem introduced by Chung \etal~\cite{chung2023diffusion} by using the diffusion model as a prior. 
...
\add{The MR-ZSR, CS-MRI, and the 3D volume voxel generation task used our in-house brain MRI image dataset} (\ie \verb|BMR-ZSR-1mm| and \verb|BMR-ZSR-5mm|). \add{All volumes are in the shape of a 256$\times$256$\times$256 cube and standard 3T T1-weighted images.} \verb|BMR-ZSR-1mm|, \add{which is used for training and retrospective evaluation, has a 1mm slice thickness. 923 volumes (236,288 2D images) were used as a training dataset, and 1 volume was used as a test dataset with the retrospective slice thickness degradation.} \verb|BMR-ZSR-5mm|, \add{which is a prospective dataset acquired at a slice thickness of 5mm, was used for the clinical evaluation (for subject information, see Appendix~\ref{mr-zsr-result-supp}) of MR-ZSR.}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.7\columnwidth]{mrzsr_pro_1_1.png}
\end{center}
\caption{\add{Comparison of the brain cortical thickness measurement result of paired ground truth 1mm volumes (1mm) and upscaled volumes from 5mm to 1mm with MR-ZSR.}}
...
\end{center}
\caption{\add{Relationship between mean cortical thickness, and age (a) and GCA scale (b) measured through brain MRI volume that upscaled from 5mm to 1mm with MR-ZSR.}}
\label{fig:mrzsr_pro_2}
\end{figure}

\add{The cortical mask measured in the reconstructed 1mm image was comparable to the mask estimated in the raw 1mm image by FreeSurfer (Appendix.~\ref{mr-zsr-result-supp}), with a mean difference of 0.06±0.11 (paired t-test, p=0.28), indicating that reconstructed T1 images by TDPM are reliably used for cortical thickness measurement that was not available in routine T1 image in clinical practice (Fig.~\ref{fig:mrzsr_pro_1_1}). When the cortical thickness was measured by ATROSCAN, the cortical mask in the reconstructed 1mm image was larger than the cortical mask in the raw 1mm image; the mean difference was 0.34±0.08 (paired t-test; p$<$0.001). Age-dependent cortical thickness decline was observed in ATROSCAN but not in Freesurfer in the reconstructed T1 images, as demonstrated by the dot plot (Fig.~\ref{fig:mrzsr_pro_2}A). In addition, cortical thickness by ATROSCAN was better correlated with the Global Cortical Atrophy scale than that by FreeSurfer (Fig.~\ref{fig:mrzsr_pro_2}B), albeit the difference between scales was not significant due to the small sample (p=0.82 and 0.22, respectively). These findings suggest that the TDPM model may facilitate cognitive decline research by vastly extending magnetic resonance images eligible for volumetric measurement. Additional investigations with large sample sizes and diverse populations are required to demonstrate the clinical implications of image reconstruction with TDPM.}
...
TPDM can be trained with a three-dimensional volume data set and Algorithm~\ref{alg:train_tpdm}.
...
When $K$ is a real number, select the main model and the auxiliary model in a stochastic way through sampling from the Bernoulli distribution with $p=1-\frac{1}{K}$ (Algorithm~\ref{alg:inference_tpdm_real_k}).
...
\State $\bm{X}_N \sim \mathcal{N}(\bm{0}, \sigma^2_1\bm{I}) \in \mathbb{N}^{d_1 \times d_2 \times d_3}$
\For{$i$ \textbf{in} $N-1:0$}
\State is\_main $\sim  Bernoulli(1-\frac{1}{K})$
\State $t \gets \frac{i}{N}$
\State $\bm{X}_i \gets$ torch.empty\_like($\bm{X}_N$)
\If{is\_main}
\For{$j$ \textbf{in} $1:d_{3}$}
...
TPDM's Unconditional sampling can be performed by removing the DPS step of the main model from the conditional sampling method (Algorithm~\ref{alg:uncond_tpdm}).
...
\subsection{MR-ZSR} \label{mr-zsr-result-supp}

\add{Additional results and subject information of the prospective clinical evaluation of MR-ZSR with a slice thickness of 5mm to 1mm are shown in Fig.~\ref{fig:mrzsr_pro_1_2}, Table~\ref{tab:mrzsr_pro_subject_info}, Table~\ref{tab:mrzsr_pro_cortical_result}. Table~\ref{tab:mrzsr_supp_result} shows the results of the retrospective quantitative evaluation of MR-ZSR with a slice thickness of 3mm to 1mm.}

\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{mrzsr_pro_1_2.png}
\end{center}
\caption{\add{Estimated cortical mask comparison between raw 1mm image and 5mm$\rightarrow$1mm ($\times$5) image of the prospective test volume.}}
...
\end{tabular}
\end{center}
\caption{\add{Subject information of the prospective clinical evaluation of MR-ZSR}}
...
\end{tabular}
\end{center}
\caption{\add{Result of the mean cortical thickness measurement of prospective ground truth 1mm MRI volume and upscaled 1mm MRI volume from 5mm by TPDM}}
...
We further attempted reconstruction on $\times$8 and $\times$24 accelerated Poisson sub-sampled CS-MRI volumes. The results are Fig.~\ref{tab:csmri_supp_result_acc8} and Fig.~\ref{tab:csmri_supp_result_acc24}, respectively. If the problem is easy ($\times$8 acceleration), each 2D image can be restored almost perfectly, so perfect results can be produced in the 3D space even with only the 2D solving method (DPS~\cite{chung2023diffusion}). However, when the problem gets tough ($\times$24, $\times$48), we can only get better results in 3D with the help of a 3D prior (TPDM).
