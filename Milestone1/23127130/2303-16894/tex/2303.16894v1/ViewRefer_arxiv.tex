...
%%%%%%%%% TITLE
\title{ViewRefer: Grasp the Multi-view Knowledge for 3D Visual Grounding\\with GPT and Prototype Guidance\vspace{-0.4cm}}

\author{
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
\and Ziyu Guo\textsuperscript{1,2*}
...
\and
Renrui Zhang\textsuperscript{1,2*}\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \vspace{-0.1cm}
\and
Dong Wang\textsuperscript{1}
\and
Zhigang Wang\textsuperscript{1}
\and
Bin Zhao\textsuperscript{1\Letter}
\and
Xuelong Li\textsuperscript{1}
\vspace{0.1cm}
\and
...
\and
{\tt\small \{guoziyu, tangyiwen, wangdong, zhaobin\}@pjlab.org.cn}
}
...
With our designed paradigm, ViewRefer achieves superior performance on three benchmarks and surpasses the second-best by \textbf{+2.8\%}, \textbf{+1.2\%}, and \textbf{+0.73\%} on Sr3D, Nr3D, and ScanRefer.
Code will be released at \url{https://github.com/ZiyuGuo99/ViewRefer3D}.
\end{abstract}
...
For 3D visual grounding, a natural multi-modality task in 3D, ViewRefer imports specific designs for both single-modal feature extraction and multi-modal fusion for precise cross-modal grounding.
...
\vspace{0.1cm}
\centering
    \includegraphics[width=0.97\linewidth]{score3.pdf}
    \vspace{0.1cm}
...
To further unleash LLMs' linguistic knowledge, we respectively rephrase the input sentence and its opposite-view synonym for $(M-2)/2$ times with a simple command template of
\begin{align}
\begin{split}
\label{template}
% \nonumber
...
\end{split}
\end{align}
where $\operatorname{\textcolor{gray}{[TEXT]}}$ is replaced by the input text or its opposite-view synonym. Through this expanding strategy, we enrich the initial single text into a set of $M$ LLM-expanded texts, which contain abundant multi-view cues and effectively alleviate the discrepancy issue.

\vspace{0.1cm}
\paragraph{View-Independent Text.}
The description in view-independent texts is irrelevant to speaker's viewpoint.
Thus, we adopt no opposite-view enrichment on these view-independent inputs, and directly rephrase them for $M-1$ times with Template~\ref{template}. The generated $M$ LLM-expanded texts also inherit the rich linguistic knowledge from LLMs, and diversify the text embedding space for visual grounding performance.
...
\centering
  ...
  % \makecell*[c]{\multirow{2}*{\shortstack{\vspace*{2.2pt}\\View\\\vspace*{0.3pt}\\Dep.}}}
         ...
         \cmidrule(lr){11-11} 
            ...
            \cmidrule(lr){1-11} 
            MVT$^*$\cite{huang2022multi} &64.4$\%$&67.3$\%$&57.1$\%$&55.7\%&64.6$\%$ &54.9$\%$&61.2$\%$&48.9$\%$&54.4$\%$&55.4$\%$\\
            \rowcolor{notecolor!8}\textbf{ViewRefer} &~\textbf{67.2\%} &~\textbf{69.7\%} &~\textbf{61.7\%} &~\textbf{56.9\%} &~\textbf{67.8\%} &~\textbf{56.1\%} &~\textbf{63.1\%} &~\textbf{50.0\%} &~\textbf{56.1\%} &~\textbf{56.9\%}\\
            \textit{Gain} &\textcolor{notecolor2}{+2.8\%} &\textcolor{notecolor2}{+2.4\%} &\textcolor{notecolor2}{+4.6\%} &\textcolor{notecolor2}{+1.2\%} &\textcolor{notecolor2}{+3.2\%} &\textcolor{notecolor2}{+1.2\%} &\textcolor{notecolor2}{+1.9\%} &\textcolor{notecolor2}{+1.1\%} &\textcolor{notecolor2}{+1.7\%} &\textcolor{notecolor2}{+1.5\%} \\
	\bottomrule
	\end{tabular}
 \tabcaption{\textbf{Performance of ViewRefer on Sr3D and Nr3D~\cite{achlioptas2020referit3d}}. We report the performance on the overall dataset and all its splits. `*' denotes our implementation results$^\dagger$.}
\vspace{0.1cm}
 \label{sr3d}
\end{figure*}
...
\centering
	...
	\toprule
		\makecell*[c]{\multirow{2}*{Method}} &\multicolumn{2}{c}{Unique} &\multicolumn{2}{c}{Multiple} &\multicolumn{2}{c}{Overall}\\
		 \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
		 &Acc.@0.25 &Acc.@0.50 &Acc.@0.25 &Acc.@0.50 &Acc.@0.25 &Acc.@0.50 \\
		 \cmidrule(lr){1-1} \cmidrule(lr){2-7}
            ReferIt3D~\cite{achlioptas2020referit3d} &53.75$\%$ &37.47$\%$ &21.03$\%$ &12.83$\%$ &26.44$\%$ &16.90$\%$ \\
		    ScanRefer~\cite{chen2020scanrefer} &65.00$\%$ &43.31$\%$ &30.63$\%$ &19.75$\%$ &37.30$\%$ &24.32$\%$ \\
            TGNN~\cite{tgnn} &64.50$\%$ &53.01$\%$ &27.01$\%$ &21.88$\%$ &34.29$\%$ &27.92$\%$ \\
            InstanceRefer~\cite{yuan2021instancerefer} &77.45$\%$ &66.83\% &31.27$\%$ &24.77$\%$ &40.23$\%$ &32.93$\%$ \\
            MVT~\cite{huang2022multi} &77.67\% &66.45$\%$ &31.92$\%$ &25.26$\%$ &40.80$\%$ &33.26$\%$ \\
            \cmidrule(lr){1-7} 
            MVT$^*$~\cite{huang2022multi} &75.18\% &63.84$\%$ &31.46$\%$ &24.95$\%$ &40.65$\%$ &32.96$\%$ \\
            \rowcolor{notecolor!5}\textbf{ViewRefer} &~\textbf{76.35\%} &~\textbf{64.27\%}  &~\textbf{33.08\%} &~\textbf{26.50\%} &~\textbf{41.35\%} &~\textbf{33.69\%}\\
            \textit{Gain} &\textcolor{notecolor2}{+1.17\%} &\textcolor{notecolor2}{+0.43\%} &\textcolor{notecolor2}{+1.62\%} &\textcolor{notecolor2}{+1.55\%} &\textcolor{notecolor2}{+0.70\%}&\textcolor{notecolor2}{+0.73\%} \\
	\bottomrule
	\end{tabular}
\tabcaption{\textbf{Performance of ViewRefer on ScanRefer~\cite{chen2020scanrefer}}. We report the Acc@0.25 and Acc@0.50 on the overall dataset and its two splits, ``Unique'' and ``Multiple''. `*' denotes our implementation results$^\dagger$.}
 \label{scanrefer}
    \vspace{0.1cm}
\end{figure*}
...
We first apply a max pooling operation to $F_{vt}$ along the dimension of intra-view objects, which aggregates the global features of different views. We then adopt linear layers to project the prototypes and multi-modal features into a unified embedding space.
Upon that, we calculate the multi-view scores by cosine similarities of each $N$-view pair as
\vspace{-0.1cm}
\begin{align}
\begin{split}
& S = \operatorname{Sim}\big(Proto,\ \operatorname{MaxPool}(F_{vt})\big),
\end{split}
\end{align}
...
% \vspace{-0.2cm}
\paragraph{ScanRefer.}
% \blfootnote{$^\dagger$We obtain the results by re-training the model using official-released code from scratch, with a single NVIDIA A100 GPU.}
% \blfootnote{$^\dagger$We reproduce the results based on the official open-source code~\cite{huang2022multi} on a single NVIDIA A100 GPU.}
...
\centering
	...
	\toprule
        ...
        \rowcolor{gray!6}\checkmark&-&-&-&-&-&61.5$\%$&64.2$\%$&55.2$\%$&51.6$\%$&61.9$\%$\\
        % \rowcolor{gray!12}
        \checkmark&\checkmark&-&-&-&-&64.4$\%$&67.0$\%$&57.6$\%$&54.5$\%$&64.6$\%$\\
        % \cmidrule(lr){1-7}
        \checkmark&\checkmark&\checkmark&-&-&-&65.1$\%$&67.2$\%$&59.9$\%$&54.7$\%$&65.7$\%$\\
        \checkmark&\checkmark&\checkmark&\checkmark&-&-&66.0$\%$&68.4$\%$&60.4$\%$&55.2$\%$&66.4$\%$\\
        \checkmark&\checkmark&\checkmark&\checkmark&\checkmark&-&66.5$\%$&68.7$\%$&60.8$\%$&55.3$\%$&66.9$\%$\\
        \rowcolor{notecolor!7}\checkmark&\checkmark&\checkmark&\checkmark&\checkmark&\checkmark&\textbf{67.2$\%$}&\textbf{69.7$\%$}&\textbf{61.7$\%$}&\textbf{56.9$\%$}&\textbf{67.8$\%$}\\
	\bottomrule
	\end{tabular}
\tabcaption{\textbf{Ablation Study on Different Components for Grasping Multi-view Knowledge on Sr3D~\cite{achlioptas2020referit3d}.} {`VG. Score.' and `VG. Cont.'} denote the view-guided scoring strategy and view-guided textual context, respectively.}
\label{component}
% \end{adjustbox}
\end{figure*}
...
In Table~\ref{sr3d}, we report the performance of ViewRefer on Sr3D dataset~\cite{achlioptas2020referit3d} for 3D visual grounding. Our ViewRefer outperforms prior works in all splits, surpassing the \textit{state-of-the-art} method~\cite{huang2022multi} on the overall accuracy by +2.8\%, on the ``view-dependent" split by +1.2\%, which demonstrates the effectiveness of the view knowledge grasped by ViewRefer.
Also, the significant amplification of +4.6\% on the challenging ``Hard" split shows the effectiveness of our ViewRefer in 3D scenes with complex spatial relations between objects, which further indicates our superior understanding of spatial relations.


\begin{figure*}[t!]
% \vspace{-0.1cm}
  % \centering
    \includegraphics[width=\textwidth]{./vis_score4.pdf}
...
In Table~\ref{sr3d}, we evaluate ViewRefer on Nr3D dataset, where ViewRefer exceeds best competitor~\cite{huang2022multi} by an overall improvement of +1.2\%, and +1.7\% on ``view-dependent" split, indicating the superiority of ViewRefer on data with intricate view-dependent natural referential utterances, which further demonstrates the effectiveness of our view-guided designs. 
...
\begin{tabular}{ccccccc}
    \toprule
    \multicolumn{3}{c}{Intra-view Aggregation} &\multicolumn{2}{c}{Context} &\makecell*[c]{\multirow{2}*{Overall}} &\makecell*[c]{\multirow{2}*{\shortstack{\vspace*{2.2pt}\\View\\\vspace*{0.3pt}\\Dep.}}} \\
    \cmidrule(lr){1-3} \cmidrule(lr){4-5}
      Avg &Max &Max+Avg &Each &Glo. &&\\
     \cmidrule(lr){1-1}  \cmidrule(lr){2-2}  \cmidrule(lr){3-3}  \cmidrule(lr){4-4}  \cmidrule(lr){5-5}  \cmidrule(lr){6-6}  \cmidrule(lr){7-7}
     \checkmark &-&- &\checkmark &- &64.9\%&52.7\% \\
     \checkmark &-&- &-&\checkmark  &65.4\%&54.0\% \\
     - &-&\checkmark &\checkmark &- &66.1\% &53.8\% \\
     - &-&\checkmark &-&\checkmark &66.2\%&54.1\% \\
     - &\checkmark&- &\checkmark &- &66.6\% & 55.0\% \\
     \rowcolor{notecolor!5} - &\checkmark&- &- &\checkmark &~\textbf{67.2\%} &~\textbf{56.9\%}\\
    \bottomrule
\end{tabular}
...
As reported, the import of each major component benefits the object location separately, which shows the effectiveness of each design in ViewRefer.
