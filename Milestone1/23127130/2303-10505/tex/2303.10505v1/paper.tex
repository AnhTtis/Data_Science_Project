...
The number of photons emitted by the atoms in the source is proportional to the emission cross section, $N \sim \sigma_{\rm em}$, which is itself proportional to the squared transition amplitude. From eq.~\eqref{eq_ratio_mq_amps} above, we have then $N_{\rm L}/N_{\rm T} \sim \eta^2$, where $N_{\rm L}$ ($N_{\rm T}$) denote the number of longitudinal (transverse) photons emitted. Finally, the amplitude of the electric field ($\sim A_{\rm 0i}$) is proportional to $\sqrt{N}$, implying that 
%
...
We thank J.A. Helay\"el-Neto and P. de Fabritiis for helpful comments, as well as P. Spagnolo for valuable information on the STAX project. PCM is indebted to Marina and Karoline Selbach for insightful discussions.
\end{acknowledgments}
