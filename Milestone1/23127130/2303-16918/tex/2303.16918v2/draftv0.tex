...
\begin{equation}
\hat{H}(k)=\hbar v_f\tau_x \otimes (\mathbf{k}\cdot \boldsymbol{\sigma}) + m\tau_z \otimes \sigma_0 + \boldsymbol{\beta}\cdot\boldsymbol{\mathbf{\sigma}},
\end{equation}
...
\caption{\label{fig:bandstructures} Chirality $\langle\gamma^5\rangle$ projected plane-wave band structures for superlattice period $a_s = 100$ nm and $ J = 0.25$ eV with differing exchange field textures. $\Delta k$ denotes the position of the $|L\rangle$ Weyl point in the uniformly magnetized case. (a) Uniformly magnetized MWSM shows $|L\rangle$-chiral Weyl node with band folding. (b) N\'eel DW superlattices show band flattening in $\hat{x} \text{ and } \hat{y}$, while (c) Bloch DW superlattices only show band flattening in $\hat{x}$.}
\end{figure}
...
For a constant magnetization, the exchange field $\mathbf{\beta} = J \hat{M}$ will split the $|L\rangle \text{ or } |R\rangle$ chiral Weyl points (which we can denote with $\eta \in \{-1, +1\}$) in momentum space as $\mathbf{k}_\eta= -\eta \Delta k \hat{M} = - \eta\sqrt{J^2 - m^2} /\left(\hbar v_f\right)\hat{M}$. Then a low-energy model for the Weyl cone dispersions can be taken as \cite{armitage_weyl_2018,araki_localized_2018,grushin_inhomogeneous_weyl_2016, ilan_pseudo-electromagnetic_2020}
...
\end{align}
We may notice $\hat{H}_N^{2}$ is diagonal in spin space, permitting us to motivate the wavefunctions for each spin $s\in{\pm 1}$.
\begin{align}
\hat{H}_N^2 = \left(\hbar v_f\right)^2 \begin{bmatrix}
             k_z^2 + \Omega\Omega^+ & 0 \\
            0 & k_z^2 + \Omega^+ \Omega \\
            \end{bmatrix}; \\
\left[ -\partial_x^2 + \frac{J\eta}{\hbar v_f} \left(2k_y\text{cos}(Gx)-sG\text{sin}(Gx)\right) +k_y^2 + \right. \nonumber \\ \left.k_z^2 + \left(\frac{J}{ \hbar v_f}\right)^2\frac{1+ \text{cos}(2 Gx)}{2}\right]|s\rangle = \frac{E^2}{\left(\hbar v_f\right)^2}|s\rangle.
\end{align}

\begin{figure}[]
\includegraphics[width=\linewidth]{figures/axialLLs.png}%
\caption{\label{fig:axialLLs} 
For a system with $a_s = 100$ nm and $J=0.25$ eV, (a) and (c) show the spectrum of $\hat{H}_N(\Gamma)$ and $\hat{H}_B(\Delta k \hat{y})$, respectively, with the localization condition Eq.~\ref{bandflat} shown in purple. The analytic axial LL spacings are shown with dashed lines, in magenta, red, or blue for the degenerate, $|L\rangle$, or $|R\rangle$ bands. (b) and (d) show the corresponding 3D energy surfaces across the Brillouin zone at $k_x = 0$. 
}
\end{figure}

For realistic physical values of $J > \hbar v_f G$, and for small enough $k_y$, the above expressions reduce to the Mathieu differential equation from the dominant $\text{cos}(2 Gx)$ term -- notably without general analytic solutions. However, we can note that two approximately-parabolic potential wells are created around regions of large $|\mathbf{B}^5|$, implying charge localization around $x = a_s(2\mathbb{Z}+1)/4$. Increasing small values of $k_y$ will tilt the potential wells, introducing anharmonicity away from $k = \Gamma$, but ultimately preserving the charge localization for small enough $k_y^2$.  Around $k = \Gamma$, we can locally understand LL generation in $\hat{H}_N$ by defining the operators
\begin{align}
...
We may take as a guess that the 0th LL may have 0 energy at $k_{||} = \left[k_y, k_z\right] =\Gamma$, and proceed to directly solve for the null space of $\hat{H}'_{N}(\Gamma)$, leaving us with two expressions for the $|\uparrow\rangle$ and $|\downarrow\rangle$ states in the $E = 0$ potential well:

\begin{eqnarray}
    \sqrt{2 \hbar v_f J G} \, \hat{g}|\downarrow\rangle = 0 \nonumber \\
    \sqrt{2 \hbar v_f J G} \, \hat{g}^+|\uparrow\rangle = 0 \nonumber. 
\end{eqnarray}

Finally, with a separation of variables, we can exactly solve for the 0-energy wavefunctions of each spin:

\begin{eqnarray}
    |s\rangle = e^{\frac{\eta s J}{v_f G}\text{sin}(Gx)}.
\end{eqnarray}
...
This implies that the Hamiltonian produces a continuum of parabolic potential wells per unique $\theta_k$, and that the solutions will take the form of slightly modified Mathieu special functions for $J > \hbar v_f G$, this time with doubled spatial period and a single parabolic well per superlattice unit cell. Unfortunately, the off-diagonal term immensely complicates our analysis, mixing the solutions of the diagonal Mathieu ($s=1$) and Mathieu-like ($s=-1$) ODES. For further insight into the spin structure and of $\hat{H}_B$, we can replace $z$ with $x$ (for consistency with $\hat{H}_N$ analysis), define $\Lambda = \left(\eta k_{r} e^{i \theta_{k}} +Je^{-iGx} \right)^{-1}$ and solve for the components of $(\hat{H}_B-E)|\Psi\rangle = 0$ by substitution(see appendix \footnote{See Supplemental Material at [URL will be inserted by publisher] for full derivation.}). This is not analytically tractable in general, but we find two solutions at $k_{r} = -\eta J / \left(\hbar v_f\right)$, $\theta_k = 0$, and $E = \eta\hbar v_f G/4$,

 \begin{align}
     |\Psi_\pm\rangle = \frac{1}{{\sqrt[4]{e^{i G x}}}} e^{\frac{2 J \left(1+e^{i G x}\right)}{G \hbar v_f \sqrt{e^{i G x}}}}|\downarrow\rangle 
     \mp \eta  \sqrt[4]{e^{i G x}} e^{-\frac{2 J \left(1+e^{i G x}\right)}{G \hbar v_f \sqrt{e^{i G x}}}}|\uparrow\rangle.
 \end{align}
...
\end{equation}
is indeed predictive at $|k_{r}| = J/\hbar v_f$. This implies the complex-modulated gaussian $E = \eta\hbar v_f G/4$ wavefunctions may indeed act as an exotic twisted 0th LL, however $H_B$ needs more analysis to determine a pair of operators which may obey the harmonic oscillator commutation relations. Interestingly, the dispersion for $k_{||} = |k_r| = \Gamma$  can be obtained analytically \footnote{See Supplemental Material at [URL will be inserted by publisher] for full derivation} and is given by:
...
\end{equation}
so that the lowest-energy $|R\rangle \text{ and }|L\rangle$-chiral bands are raised and lowered in energy with $\Delta E =\eta (\sqrt{J^2+\hbar^2 v_f^2 \pi^2/a_S^2} - \hbar v_f \pi/ a_S) \approx \eta J$ at the $\Gamma$ point for a magnetic texture with right-handed  spatial helicity.
...
Both cases lead to a infinite lattice of parabolic wells, which gives us a means to analyse the band flattening by considering  a minimal tight-binding model. For a localized LL state, 
\begin{equation}
    \langle \Delta x \rangle = l_B \sqrt{n+\frac{1}{2}},
\end{equation}
which provides a heuristic condition for band flattening as a function of axial LL index, supposing individual tightly-bound wavepackets may have negligible overlap. The Bloch and Neel wall cases provide 1 and 2 potential wells per unit cell respectively, thus: 
...
\begin{align} \label{bandflat}
    |E_N| < \sqrt{J\left(\frac{\pi J}{4} - \frac{\hbar v_f}{a}\right)},
    |E_B| < \sqrt{J\left(\pi J - \frac{\hbar v_f}{a}\right)},
\end{align}
...
Chirality-resolved real-space Fermi surfaces with chemical potential $\mu = 0.1$ eV and broadening parameter $\eta = 10^{-2.5}$ eV are shown with corresponding exchange field textures for $x_0$ fixed to 63 nm. Isosurfaces in the $\gamma^L$ and $\gamma^R$ projected spectral functions are resolved in red and blue, respectively. (a) and (b) show the Neel DW lattice, while (c) and (d) show the Bloch DW lattice.
}
\end{figure}
...
%\begin{figure*}[!ht]
%\includegraphics[width=\linewidth]{figures/transmissions.png}%
%\caption{\label{fig:tmaps}  Device conductance is modulated by injecting DWs between contacts via sweeping x0x_0. (a) Conductance modulation by orders of magnitude is shown for all combinations of MWSM and simple metallic electrodes with Bloch and N\'eel DW lattices. (b) A phenomenological broadening term η\eta is swept, showing discrete stairsteps in conductance, thus multi-weight behavior, as η\eta increases (i.e. carrier lifetime decreases). (c) and (d) show k-resolved transmission maps for the N\'eel and Bloch DW cases, respectively, sweeping x0x_0, over center of surface Brillouin zone. Hotspots in transmission T(k||)arevisiblewherethebulkstatesfromtheleftandrightelectrodesoverlap,connectedbytwistedFermiarcsofcharacterdeterminedbythemagnetictexture.arevisiblewherethebulkstatesfromtheleftandrightelectrodesoverlap,connectedbytwistedFermiarcsofcharacterdeterminedbythemagnetictexture.x0T(k_{||})arevisiblewherethebulkstatesfromtheleftandrightelectrodesoverlap,connectedbytwistedFermiarcsofcharacterdeterminedbythemagnetictexture. are visible where the bulk states from the left and right electrodes overlap, connected by twisted Fermi arcs of character determined by the magnetic texture. x_0 position is labeled in white for each sub-plot.}
%\end{figure*}
%\end{widetext}
...
\end{figure*}
 ...
 Of particular interest is the twisting of the Fermi surfaces in connection with the band structures of Fig.~\ref{fig:bandstructures}, which are useful to understand transport. Fig.~\ref{fig:fermisurfaces} shows the Fermi surface of the periodically magnetized racetrack devices for energies close to the Weyl node. N\'eel devices show a $k_y$-symmetric twisting of the L and R chiral states, corresponding to the degenerate states in Fig.~\ref{fig:fermisurfaces} (b). This can be understood from the profile of $\boldsymbol{\beta}_N$ for the N\'eel case which decides the position of two Weyl nodes in the $k_y-k_z$ plane. On the other hand, for the Bloch DW case, $\mathbf{B}_B^5$ rotates in $y-z$ leading to these twisted Fermi-surfaces which can also be understood as the Fermi contour of the zeroth LL shown in Fig.~\ref{fig:fermisurfaces} (d) where for an infinitesimal non-zero energy, $|L\rangle$ and $|R\rangle$ states are at a different radius in the $k_y-k_z$ plane. As a result, the $|L\rangle$ chiral states wrap around the expected position of the twisted Weyl cones, while the $|R\rangle$ chiral states wrap closely to $k_{||} = \Gamma$. This twisted Fermi-surface motivates analysis of the chiral anomaly and Weyl orbits with periodic real and axial magnetic fields (see recent insights in \cite{breitkreiz_fermi_arc_2023}).

For device modeling, we neglect the orbital effects of the magnetic field and consider transport in the Landauer limit of the NEGF quantum transport formalism \footnote{See \cite{camsari_nonequilibrium_2023}; $G^R_{k_{||}} = (E+i\eta - H_{k_{||}} - \Sigma_{R,k_{||}} - \Sigma_{L,k_{||}})^{-1}; \Gamma_i = i(\Sigma_i - \Sigma_i^\dagger); T = tr(\Gamma_L G^R \Gamma_R G^A); G^A = (G^R)^\dagger$} at a chemical potential $\mu$ = 0.1 eV to cut through the 0th axial LL. With a minimal carrier lifetime broadening of $\eta = 10^{-4}$ eV, in Fig.~\ref{fig:conductances} (a) we show that device conductance can be modulated by orders of magnitude via the injection of DWs into the active region of the device, and that this effect persists for both magnetization patterns, with both MWSM and simple metal electrodes. In Fig.~\ref{fig:conductances} (b), we sweep the broadening parameter $\eta\iffalse=ℏ2τ = \frac{\hbar}{2\tau}\fi$, corresponding to a phenomenological inelastic momentum-preserving scattering self-energy \cite{gilbert_TI_interconnects_2017}, and show that it 1) smooths out conductivity vs. $x_0$ and 2) forms stairsteps in conductance with respect to the number of DWs in the active region of the device--as one might expect with an increasing number of spin-dependent tunneling barriers. A similar behavior is shown for the Bloch wall case. 


Curiously, even a high-resolution 401 x 401 k-grid over the zoomed-in portion of the surface Brillouin zone (SBZ) is unable to converge the value of the Landauer conductance for small broadening parameters ($\eta = 10^{-3}, 10^{-4}$ eV), leading to unphysical spikes in conduction. To explain this, we consider the $k_{||}$-resolved transmission in Fig.~\ref{fig:tmaps} (a) and Fig.~\ref{fig:tmaps} (b) to resolve conductance behavior from the distorted Weyl cones in the periodic magnetization region. We see that the distorted Weyl cones form infinitesimally-thick curtains of conductance in the SBZ which are challenging to capture numerically: In Fig.~\ref{fig:tmaps} (a), the N\'eel DW lattice conduction is dominated by the distorted $|L\rangle$ and $|R\rangle$ Weyl cones in the periodic region of the device, especially where they overlap with the contact electrodes' bulk Weyl cones. In contrast, the Bloch DW device in Fig.~\ref{fig:tmaps} (b) has minimal conduction through the distorted Weyl cones which wrap around the bulk Weyl cone states from either contact. Thus, the majority of the conductance tunnels through the periodic region of the device from the Bulk states in both contacts, leading to oscillations in Fig.~$\ref{fig:conductances}$ (a). Interestingly, due to the broadening parameter providing finite-lifetime states from the contacts, in the magnetization with left-handed spatial chirality, conductance from the contact Weyl cone overlap decreases and the inner $|R\rangle$-chiral transmission ring will begin to dominate the total conductance. This can be motivated by Eq.~\ref{chiralityfilter}, where the larger $k_{||}$, $|L\rangle$-chiral states must tunnel through deeper potential wells, thus facilitating lower conductivity in the $|L\rangle$ channel and  giving rise to a texture-dependent chirality filtering mechanism to generate or reflect chiral currents -- again, with potential in emerging devices~\cite{kharzeev_chiralanomaly_2013}. 
