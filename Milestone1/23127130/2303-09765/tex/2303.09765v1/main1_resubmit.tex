...
Through an interleaved measurement with different magnetic fields, we determine the $g$-factor to be 1.463(2), consistent with our relativistic many-body calculation. 
...
%%%%  g-factor  %%%%%%%%%%%%%%%%%
In addition, we precisely determine the $g$-factor of the $4f^{13}5d6s^2 \: (J = 2)$ state as $g_J = 1.463(2)$ with the uncertainty determined by the propagation of the fitting error and the uncertainty of the magnetic field calibration.
...
This is compared with the theoretical calculations in Refs.~\cite{Dzuba2018}, \cite{Safronova2018}, and \cite{Tang2022}, which predict a lifetime of about 200~s, 60~s, and 190~s, respectively.
...
% Experimentally, we confirm that the lifetime is, in fact, not affected by the increased depth of the 1D optical lattice at 797.2~nm from $25E_\text{R}$ to $40E_\text{R}$.
% Here, $E_r = \hbar^2k_L^2/(2m)$ is the recoil energy for optical lattice, where $\hbar$ is the reduced Planck constant, $k_L$ is the wavevector for the lattice laser light, and $m$ is the mass of the Yb atom.
% The reason of the discrepancy with the theoretical predictions is not known at present.
% Note that the time scale of atom hopping in the optical lattice at the depth of 25$E_r$ is on the order of 1.0 s, which may lead to the inelastic collisional loss through a p-wave channel, for example.    
% Note that the hopping rate of the optical lattice at $\lambda_1$($\lambda_2$) is 1.2~s$^{-1}$(1.9~s$^{-1}$), which may lead to the inelastic collisional loss through a $p$-wave channel, for example.    
%to see if the lifetime is reduced due to photon scattering, but no such phenomenon was observed.
%In order to investigate whether this is the original lifetime of the excited state $\ket{e}$, we also measured the lifetime of the ground state $\ket{g}$.
%The measurement procedure is simple;
%After loading the lattice, we wait for the "hold time" and finally observe the number of atoms.
%The result is 2.8(2) s, which is roughly consistent with the measured value of 1.9(1) s for $\ket{e}$.
%Thus, we believe that the $\ket{e}$ measurements are limited by problems in our experimental system, such as collisions with background gas.

%%%%%%%%%%%% Excitation spectra in a 3D optical lattice %%%%%%%%%%%%
%\indent
%{\it Excitation spectra in a 3D optical lattice.{\bf \textemdash}}
%Finally, we report on spectroscopy in a 3D optical lattice
%So far in this paper, atoms in crossed FORT have been used for spectroscopy, but as shown in Fig. 2, their spectral linewidths are limited by the Doppler broadening.
%Therefore, we expect that spectroscopy in an optical lattice will suppress the motion of atoms and narrow the linewidth.
%We have trapped $^{174} \mbox{Yb} (m_{J'}=0)$ in a three-dimensional optical lattice and performed Doppler broadening-free spectroscopy.
%The optical grating arrangement is the same as in the lifetime measurement, with a 759 nm semiconductor laser on the $x$ and $z$ axes and a 797.2 nm Ti:Sa laser on the $y$ axis, which is the excitation axis.
%The optical grating depths of the $x$, $y$, and $z$ axes are 20, 50, and 30Er, respectively.
%The optical grating depth on the $y$ excitation axis was as large as possible to minimize the Doppler broadening and to increase the carrier and sideband separation by increasing the optical grating trap frequency.

%\begin{figure}
%\includegraphics{Fig5_230104.png}
%\caption{\label{Fig5}
%Excitation spectrum of $^{174}$Yb ($m_J=0$) in a 3D optical lattice.
%We observe two peaks consisting of the carrier and the blue sideband.
%The solid red curve is the fit with a double-peak Lorentzian function.
%The FWHM of the carrier spectrum is $12$ kHz.
%}
%\end{figure}

%Figure~\ref{Fig5} shows the spectrum.
%It can be seen that a double peak was obtained.
%The low frequency side is considered to be the carrier and the high frequency side is the blue sideband.
%The solid red line shows the fitting by the double-peak Lorenz function. The FWHM of the carrier is about 12 kHz, and the frequency difference between the carrier and blue sideband is about 18 kHz.
%Compared to the FWHM of 30 kHz in the crossed FORT, the FWHM has been narrowed to less than half.
%We believe that the current linewidth is limited by the linewidth of the excitation laser or the non-uniform broadening of the light shift due to the 759 nm optical grating light.
...
We also determine the $g$-factor to be $1.463(2)$, and compare the result with our relativistic many-body calculation with good agreement. 
...
%%%%%%%%%% bibtex %%%%%%%%%%
% \bibliography{main_resubmit}% Produces the bibliography via BibTeX.
