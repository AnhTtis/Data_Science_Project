...
\begin{minipage}[b]{0.3\linewidth}
  ...
  \begin{minipage}[b]{0.3\linewidth}
    ...
    \begin{tikzpicture}[xscale=0.7, yscale=0.8]
      ...
      \node at (-1.5, -1) (B) {$\VIEW[A]{V^{@B}_{R}}$} edge[-] (A);
      \node at (1.5, -1) (C) {$\VIEW[A]{V^{@C}_{ST}}$} edge[-] (A);
      \node at (0.5, -2) (D) {$\VIEW[C]{V^{@D}_{T}}$} edge[-] (C);
      \node at (2.5, -2) (E) {$\VIEW[A,C]{V^{@E}_{S}}$} edge[-] (C);
      
      \node at (-1.5, -2) {$\VIEW[A,B]{R}$} edge[-] (B);
      \node at (2.5, -3) {$\VIEW[A,C,E]{S}$} edge[-] (E);
      \node at (0.5, -3) {$\VIEW[C,D]{T}$} edge[-] (D);
...
In this section, we introduce a framework for static query evaluation  based on factorized computation and data rings. In the next section, we extend it to incremental maintenance. 
...
$\VIEW[C,D]{T}$.   
Figure~\ref{fig:example_payloads}  gives a variable order (left) for
the query.
...
\begin{tabular}{@{}c@{}c@{~~~}l}
  ...
  \midrule 
  \phantom{a} & $\VIEW{R}$\hspace*{0.5em} & \RETURN $\VIEW[\mathit{\sch(\VIEW{R})}]{R}$ \\
  \cmidrule{2-3} \\[-6pt] 
  &
...
\nop{
The algorithm works on {\em any} variable order that is valid for the input query. The variable orders valid for queries with free (group-by) variables~\cite{BKOZ:PVLDB:2013,FAQ:PODS:2016} have the free variables placed above the bound variables. This leads to view trees where marginalization of bound variables happens as early as possible in a bottom-up evaluation.
}
...
\begin{figure}[t]
  \begin{minipage}[b]{\linewidth}
    ...
    \begin{tikzpicture}[xscale=0.75, yscale=0.35]         
      % at node R
      \node [anchor=north west] at (1.2, 0.0) {
    \begin{tabular}{@{\,}l@{~~}l@{~$\to$~}l@{\,}}
      ...
      $a_3$ & $b_4$ & $p_4$\\\bottomrule
        \end{tabular}
            ...
            % at node S
      \node [anchor=north west] at (1.2, -5.6) {
    ...
    \end{tabular}
            ...
            % at node T
      \node [anchor=north west] at (1.2, -11.2) {
       \begin{tabular}{@{\,}l@{~~}l@{~$\to$~}l@{\,}}
    ...
    \end{tabular}
      ...
      \node [anchor=north west] at (1.2, -21.5) {
      ...
      % at node B
      \node [anchor=north west] at (6.5, 0.1) {
        \begin{tabular}{@{\,}l@{\,} @{\,}c@{\,}c@{\,}l@{\,}}
          %  
      ...
      % at node C
      \node [anchor=north west] at (1.2, -16.8) {
        \begin{tabular}{@{\,}l@{\,} @{\,}c@{\,}c@{\,}l@{\,}}
      ...
      % at node D
      \node [anchor=north west] at (6.5, -11.1) {
        \begin{tabular}{@{\,}l@{\,} @{\,}c@{\,}c@{\,}l@{\,}}
          %  
      ...
      % at node E
      \node [anchor=north west] at (6.5, -5.5) {
...
$i \in [12]$.}
\label{fig:count}\vspace*{-1em}
\end{figure}

\begin{example}
\label{ex:views_count}
Figure~\ref{fig:example_payloads} shows the view tree constructed by the function $\tau$ from Figure~\ref{fig:static_view_tree_algo} over the variable order $\omega$ and the empty set of free variables.
...
By default, the function $\tau$ in Figure~\ref{fig:static_view_tree_algo} constructs one view per variable in the variable order $\omega$. A wide relation (with many variables) leads to long branches in $\omega$ with variables that are only local to this relation. This is, for instance, the case of our retailer dataset used in Section~\ref{sec:experiments}. Such long branches create long chains of views, where each view marginalizes one bound variable over its child view in the chain. For practical reasons, we compose such long chains into a single view that marginalizes several variables at a time. 
