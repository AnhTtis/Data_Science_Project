\section{F-IVM for Special Query Classes}
\label{sec:query_classes}

This section shows that \DF recovers the complexity results for two well-known query classes: the class of {\em free-connex ($\alpha$-)acyclic} queries~\cite{DynYannakakis:SIGMOD:2017} and the class of {\em $q$-hierarchical} queries~\cite{Nicole:PODS:2017}. The analysis for these queries is refined into three components: (i) the preprocessing phase, where the view tree is constructed, (ii) the enumeration phase, where we present the query result one tuple at a time, and (iii) the update phase, where we update the view tree. The following complexity\footnote{We analyze the {\em data complexity}, which is a function of the database size only.} claims assume that the ring operations require constant time, otherwise the complexity results stated in this section have an extra multiplying factor to account for the complexity of the ring operations.

\begin{theorem}\label{th:special-cases}
  Let a query $\VIEW[]{Q}$ and a database of size $N$.

  F-IVM can maintain $\VIEW[]{Q}$ with $O(N)$ preprocessing, $O(1)$ enumeration delay, and $O(N)$ single-tuple updates in case $\VIEW[]{Q}$ is free-connex acyclic.
  
  F-IVM can maintain $\VIEW[]{Q}$ with $O(N)$ preprocessing, $O(1)$ enumeration delay, and $O(1)$ single-tuple updates in case $\VIEW[]{Q}$ is $q$-hierarchical.
\end{theorem}

\nop{
\begin{remark}
  F-IVM has a special treatment for cyclic que\-ries, which is not discussed in this article for lack of space.
  Whereas for an acyclic join query the size of each view is asymptotically upper-bounded by the size of the query result, for a cyclic query views may be larger in size than the query result. 
  In prior work~\cite{FIVM:SIGMOD:2018}, we show how to reduce the size of intermediate views for cyclic queries  by extending view trees with indicator projections~\cite{FAQ:PODS:2016}.  Such projections have no effect on the query result but can constrain view definitions (e.g., create cycles) and bring asymptotic savings in space and time.\punto  
\end{remark}
}
...
We first introduce the class of free-connex acyclic que\-ries and then explain how F-IVM recovers the complexity result stated in Theorem~\ref{th:special-cases}.

\begin{definition}[\cite{Yannakakis81,BraultPhD13}]
  A query is \emph{($\alpha$-)acyclic} if it admits a join tree, where each node is a relation and if any two nodes have variables in common, then all nodes along the path between them also have these variables. 
...
$\VIEW[]{Q'}$ turns the query into a cyclic query $\VIEW[]{Q''}$
that does not admit a join tree. Hence, $\VIEW[]{Q'}$ is not free-connex acyclic.     
\punto
\end{example}
...
We next detail how F-IVM recovers the complexity results for a free-connex acyclic query $\VIEW[]{Q}$. 
...
If $X$ only has one child, then the creation of the auxiliary view $\VIEW[]{H^{@X}_{rels}}$ is not needed for efficient updates or enumeration.
...
Hence, all views in the view tree can be computed in linear time. 
\punto
\end{example}
...
by the function $\Delta$ in Figure~\ref{fig:dynamic_view_tree_algo}.
...
\begin{example}
Continuing Example~\ref{ex:free-connex-viewtree}, we now consider a single-tuple update $\delta\VIEW{T}[c,d]$ to $\VIEW{T}$.
...
Consider a view tree $\tau$ constructed using the function from Figure~\ref{fig:static_view_tree_algo_free-connex} for a free-top variable order of a 
query $\VIEW[]{Q}$. 
...
We use the views $\VIEW[]{H^{@X_1}}, \ldots , \VIEW[]{H^{@X_n}}$ to enumerate the distinct tuples
in the result of $\VIEW[]{Q}$. We retrieve from $\VIEW[]{H^{@X_1}}$ the first $X_1$-value in the 
result. 
When we arrive at a view  $\VIEW[]{H^{@X_j}}$ with $j > 1$, we have already fixed 
the values of the variables above $X_j$ in the variable order. 
We retrieve from $\VIEW[]{H^{@X_j}}$ the first $X_j$-value paired with these values. 
Once the values over all free variables 
are fixed, we have a complete result tuple that we output.
Then, we iterate over the remaining distinct $X_n$-values in $\VIEW[]{H^{@X_n}}$
...
and restart the iteration of the matching $X_n$-values, and so on.      
...
\begin{remark}
  The enumeration procedure only needs the payloads from several views only. This means that the payloads from the views above these latter views need not be maintained beyond keeping track of the multiplicities of each of their tuples. The maintenance of multiplicities is important for correctness, as it tells whether a tuple is to be removed from a view or still has at least one possible derivation from the input relations. 
...
$\VIEW[A,B,C]{Q}$ 
from Example~\ref{ex:acyclic} using the view tree in Figure 
\ref{fig:free-connex_view_tree} (left). We iterate with constant delay 
...
We first define $q$-hierarchical queries and then show how F-IVM achieves constant-time update for them. 
...
\end{definition}
 ...
 Every hierarchical query admits a {\em canonical} variable order, where each root-to-leaf path consists of variables that represent precisely the schema of a relation in the query. 
$Q$-hierarchical queries admit canonical free-top variable orders, which are canonical variable orders where  no bound variable is above a free variable~\cite{KNOZ20}.
...
By extending the schema of $\VIEW[]{T}$ with $A$, we obtain the hierarchical query 
$\VIEW[A,B,C]{Q_h}$ $=$ 
$\VSUM_{D}\VSUM_{E}$ $\VIEW[A,B]{R} \VPROD \VIEW[A,C,E]{S} \VPROD \VIEW[A,C,D]{T}$
whose canonical variable order is given in Figure~\ref{fig:q-hierarchical_view_tree} (left).
 
 The query $\VIEW[]{Q_h}$ is $q$-hierarchical and admits the canonical free-top variable order from 
 Figure~\ref{fig:q-hierarchical_view_tree} (left). The variant of the query,  
 where the variable $A$ is bound, is not $q$-hierarchical because  
...
Figure~\ref{fig:q-hierarchical_delta_view_trees} visualizes the
delta view trees 
under single-tuple updates to the relations $\VIEW[]{R}$  and $\VIEW[]{T}$.
...
\begin{remark}
  ...
  Consider the view tree in Figure~\ref{fig:q-hierarchical_view_tree}. Enumeration proceeds top-down: We iterate over the $A$-values in the top view and for each such value $a$, we look up in $\VIEW[a]{V^{@B}_{R}}$ and $\VIEW[a]{V^{@C}_{ST}}$, then continue to look up in $\VIEW[a,B]{R}$ to enumerate over all the $B$-values paired with $a$, and also look up into $\VIEW[a,C]{H^{@C}_{ST}}$ to enumerate over all $C$-values paired with $a$. We continue similarly to get all $D$-values and $E$-values paired with $a$ and each $C$-value. All these look-ups require primary or secondary indices.  
...
Non-hierar\-chical queries may become hierarchical under functional dependencies (fds), as shown in prior work on probabilistic databases~\cite{OlteanuHK09}. We leverage this idea for dynamic query evaluation.
...
\begin{align*}
  \;
  \VIEW[\textsf{CLOSURE}_\Sigma(\calS)]{Q} = \VSUM_{\calB} &\VIEW[\textsf{CLOSURE}_\Sigma(\calS_1)]{R_1}\VPROD\cdots\VPROD\\
  &\VIEW[\textsf{CLOSURE}_\Sigma(\calS_n)]{R_n}
\end{align*}
\end{definition}
In other words, the $\Sigma$-reduct of a query is another query where the schema of each relation is extended to include all variables in the closure of this schema under the fds in $\Sigma$. Intuitively, the newly added variables do not add more information to the schema, as they are functionally determined by the variables already in the schema. So, we could extend these schemas and the underlying relations without increasing the number of tuples in the relations. For any database $D$ with fds $\Sigma$ and a query, the result of the query over $D$ is the same as the result of its $\Sigma$-reduct over the extended database.
The benefit of this rewriting in our case is that queries may admit free-connex acyclic or even $q$-hierarchical $\Sigma$-reducts. We do not want to physically extend the input database to reap this benefit. Instead, we use the $\Sigma$-reduct of a query $\VIEW[]{Q}$ to infer a free-top variable order or even a canonical free-top variable order \emph{for} $\VIEW[]{Q}$ in case the $\Sigma$-reduct is free-connex acyclic and $q$-hierarchical, respectively.
From such a variable order, we can then construct a view tree for $\VIEW[]{Q}$ that enjoys the preprocessing, update, and enumerate times as for its $\Sigma$-reduct.
...
\begin{theorem}\label{th:special-cases-fds}
  ...
  F-IVM can maintain $\VIEW[]{Q}$ with $O(N)$ preprocessing, $O(1)$ enumeration delay, and $O(N)$ single-tuple updates in case the $\Sigma$-reduct of $\VIEW[]{Q}$ is free-connex acyclic.
    
  F-IVM can maintain $\VIEW[]{Q}$ with $O(N)$ preprocessing, $O(1)$ enumeration delay, and $O(1)$ single-tuple updates in case the $\Sigma$-reduct of $\VIEW[]{Q}$ is $q$-hierarchical.
\end{theorem}
...
\end{tikzpicture}
    ...
    \end{minipage}
    \caption{Delta view trees derived from the view tree in Figure 
        \ref{fig:q-hierarchical_rewriting} for single-tuple updates to 
...
\begin{align*}
\VIEW[A,B,C,D]{Q'} =\VIEW[A,B,C,D]{R} \VPROD \VIEW[B,C,D]{S} \VPROD \VIEW[C,D]{T}.
\end{align*}
