...
\begin{theorem}\label{thm:ext_space}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item $\{\eta(x) \mid x \in \mathcal{X}_-\}$ is a basis of $\Ext^1_{\Pi(A_n,[2,n-1]_\mathbb{Z})}(X,Y)$.
            \item $\{\eta(x) \mid x \in \mathcal{X}\}$ is a basis of $\Ext^1_{\Pi(A_n)}(X,Y)$.
        \end{enumerate}
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    (2a) This result is essentially contained in \cite[Section~6.2]{BaH}, where it is deduced from \cite[Theorem~8.5]{BDMTY}. Indeed, in \cite{BDMTY}, the authors give explicit bases for the Hom- and Ext-spaces between indecomposable ``string modules'' over gentle algebras. Their basis elements of $\Hom_{\Pi(A_n,[2,n-1]_\mathbb{Z})}(Y,X)$ are called ``graph maps''. The image of a graph map $f: Y \rightarrow X$ is always a brick\footnote{Over an arbitrary gentle algebra, there may be graph maps whose images are not bricks, but over $\Pi(A_n,[2,n-1]_\mathbb{Z})$ this will never be the case.}, and $f$ is called ``two-sided'' if there exists $i < j < k \in [n] \setminus\{0\}$ such that (a) $X(i) \oplus Y(i) \neq 0$ and $\mathrm{im}(f)(i) = 0$, (b) $\mathrm{im}(f)(j) \neq 0$, and (c) $X(k) \oplus Y(k) \neq 0$ and $\mathrm{im}(f)(k) = 0$. As is implicit in \cite[Theorem~6.2.5]{BaH}, the two-sided graph maps from $Y$ to $X$ are those corresponding (in the proof of Proposition~\ref{prop:hom_space}) to the nontrivial crossings directed from $\sigma(Y)$ to $\sigma(X)$. It then follows from \cite[Theorem~6.5]{BDMTY} that the nonsplit sequences $\{\eta(x) \mid x \in \mathcal{X}_-\}$ form a basis of $\Ext^1_{\Pi(A_n,[2,n-1]_\mathbb{Z})}(X,Y)$.
...
We conclude this section with two additional consequences of Theorem~\ref{thm:ext_space}. The first will be critical in the next two sections. See also \cite[Theorem~1.2]{IRRT} and \cite[Proposition~3.7]{mizuno2}
...
\begin{remark}
    Our proof of Theorem~\ref{thm:hereditary} highlights that assuming the underlying arcs form a forest is not necessary in \cite[Definition~14]{GIMO}. Indeed, it is shown in the proof of \cite[Theorem~12]{GIMO} that the arcs corresponding to any exceptional sequence do form a forest, but the forest condition is not used in showing that the modules corresponding to any strand diagram form an exceptional sequence.
\end{remark}
