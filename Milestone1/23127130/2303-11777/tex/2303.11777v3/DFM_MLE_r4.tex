...
Section \ref{sec:dynamic} heavily draws on \citet{BLqml} to which the reader is referred for the original results and their proof.\\
...
(see also \citealp[Chapter 4]{lawleymaxwell71}, \citealp[Chapter 9.3]{mardia1979multivariate}, \citealp[Chapter 7.2]{jolliffe2002principal}). This is also the definition adopted also by \citet{FGLR09}. In particular, 
...
\eeq
Although this result is derived under the alternative identifying condition $N^{-1}\bm\Lambda^\prime(\bm\Sigma^\xi)^{-1}\bm\Lambda$ being diagonal (IC3 in the original papers), it is in principle possible to show that it is applicable also under our identifying conditions. Moreover, in \citet[Theorem 3]{MBQMLPCA} it is shown that, under the assumptions of this paper, the following holds:
\beq\label{eq:AE3}
...
\upshape{An alternative intuitive argument for the equivalence between QML and OLS is the following. Consider the decomposition of the log-likelihood \eqref{eq:LL0iid}:
\beq
...
Namely, \citet[Theorem 4]{MBQMLPCA} proves that:
\[
...
The same argument applies to  the idiosyncratic variances. An equivalent argument, which, however, holds only asymptotically, can be made starting from Remark \ref{rem:BTintuition} which exploits the equivalence with the unfeasible OLS estimator.


It is then common practice to consider the output of the EM algorithm $\wh{\bm\lambda}_i^{\text{\tiny EM}}$ as equivalent to the QML estimators $\wh{\bm\lambda}_i^{\text{\tiny QML,D}}$ or $\wh{\bm\lambda}_i^{\text{\tiny QML,S}}$. This, however, is not always true, because there are two additional sources of errors: (i) the local maximum might not be the global maximum, and (ii) the EM algorithm runs for a finite number of iterations $k^*$.
...
From \eqref{eq:AEBL1} and \eqref{eq:AEBL2}, jointly with Proposition \ref{prop:LBAILI16}, it follows that.
...
\end{align}
Notice, that, unless the factors and the idiosyncratic components are Gaussian, these expressions are correct only asymptotically since, as $N,T\to\infty$, the Kalman smoother $\mbf F_{t|T}^{(k)}$ is a consistent estimator of the true factors at any given iteration $k\ge 0$, and any non-linearity is in $\E[\bm\xi_t|\bm{\mathcal X}]$. Moreover, since we treat the idiosyncratic components as cross-sectionally uncorrelated then $\mbf P_{t|T}^{(k^*)}$ is just an approximation of the true MSE of the Kalman smoother. However, since, given $\wh{\bm\varphi}^*$, both matrices are $O_{\mathrm P}(N^{-1})$ (\citealp[Proposition 1]{BLqml}) also this approximation error is negligible.
...
\begin{align}
x_{it}&={\bm\ell}_{i}^\prime {\bm f}_t + \phi_i \xi_{it}, \quad {\bm f}_t ={\bm A}  {\bm f}_{t-1} + \mbf u_t,\quad \xi_{it}=\alpha_i\xi_{it-1}+e_{it},\nn
\end{align}
where ${\bm\ell}_{i}$ and ${\bm f}_t$ are $r$-dimensional vectors, and 
\begin{inparaenum}
	\item []  ${\ell}_{ij}\stackrel{iid}{\sim}\mathcal{N}(1,1)$; 
	\item [] ${\bm A}=0.9 \check{\bm A} \{\nu^{(1)}({\check{\bm A}})\}^{-1}$, where $[\check{\bm A}]_{jj}\sim U[0.5,0.8]$, while $[\check{\bm A}]_{jk}\sim U[0,0.3]$;
	\item [] $u_{jt}\stackrel{iid}{\sim}\mathcal N(0,1)$, with $\Cov(u_{it},u_{jt})=0$, for $i\ne j$;
	\item [] $e_{it}\stackrel{iid}{\sim}\mathcal N(0,\sigma_{ei}^2)$, with $\sigma_{ei}^2\sim U[0.5, 1.5]$ and $\Cov(e_{it},e_{jt})=\tau^{\vert i-j\vert }$ with $\tau\in\{0,0.5\}$ if $\vert i-j\vert \le 10$, and $\Cov(e_{it},e_{jt})=0$ otherwise;
	\item  [] $\alpha_i=\{0,\delta_i\}$, where $\delta_i\stackrel{iid}{\sim}\mathcal{U}(0,\delta)$, and $\delta\in\{0,0.5\}$;
	\item [] $\phi_i=\sqrt{\theta_i\frac{\wh{\Var}(\chi_{it})}{\wh{\Var}(\xi_{it})}}$, where $\wh{\Var}(\cdot)$ denotes the sample variance, and 
...
Finally, in order to satisfy Assumptions \ref{ass:ident}(a) and \ref{ass:ident}(b) we proceed as follows. Given the common components is generated as $\chi_{it}={\bm\ell}_{i}^\prime {\bm f}_t$, let $\bm\chi_t=(\chi_{1t}\cdots \chi_{nt})^\prime$ and compute $\wh{\bm\Gamma}^\chi=\frac 1T\sum_{t=1}^T \bm\chi_t\bm\chi_t^\prime$. Collect its $r$ non-zero eigenvalues into the $r\times r$ diagonal matrix $\mbf M^{\chi}$ and the corresponding normalized eigenvectors as the columns of the $n\times r$ matrix $\mbf V^{\chi}$, with sign fixed such that it has non-negative entries in the first row. The loadings are then simulated as $\bm \Lambda=\mbf V^{\chi} (\mbf M^{\chi})^{1/2}$. 

We simulate the model described above $B=500$ times, and at each replication, $b=1,\ldots, B$, we estimate the loadings by means of: (i) the  OLS using the true factors, (ii) PC as in \citet{MBPCA}, (iii) QML implemented via the EM algorithm in \citet{baili12,baili16}, and (iv) EM algorithm as in \citet{DGRqml} and \citet{BLqml}. Both the QML and EM estimators are obtained by using the PC estimator to initialize the iterations. We estimate the factors in the following ways:(i)  OLS using the true loadings, (ii) PC using the PC estimator of the loadings, (iii) LP and WLS using the QML estimators of the loadings and  idiosyncratic variances, and (iv)  Kalman smoother  using the EM estimators of the loadings and  idiosyncratic variances.
