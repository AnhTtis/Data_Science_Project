...
Section \ref{sec:dynamic} heavily draws on \citet{BLqml} to which the reader is referred for the original results.\\
...
Factor analysis is one of the earliest proposed multivariate statistical techniques: it dates back to the studies of \citet{spearman04} in experimental psychology. The idea is to decompose a vector of $N$ random variables into two components: (i) a common component driven by $r<N$ latent factors, and (ii) a component which is idiosyncratic, in the sense that it can be due either to measurement errors or to specific features of a single, or a sub-group of variables. As such we can retrosepectively consider factor analysis as a pioneering technique in the filed of unsupervised statistical learning.

Over the years, two main strands of applications have emerged: first, in psychometrics \citep{thomson36,bartlett37,bartlett38,thomson51,lawleymaxwell71,bartholomew2011latent}, 
...
Lemma \ref{lem:egap} shows that the spectrum of the population covariance matrix of $\{\mbf x_t\}$ has an eigen-gap which widens as $N$ grows. This is  a necessary condition for identification of the model, and, indeed, virtually all existing methods for estimating the number of factors, $r$, are based on such property (\citealp{baing02}, \citealp{ABC10}, \citealp{onatski10}, \citealp{ahnhorenstein13}, \citealp{trapani2018randomized}). Notice, however, that even if an eigen-gap is displayed by an observed data set, this is not sufficient to say that the underlying data generating process is a factor model. Of course, in practice, requiring $N\to\infty$ is not sufficient for identification, since, as shown later, in order to consistently estimate all quantities we also need $T\to\infty$.
...
$\,$
\begin{compactenum}
\item [(a)] The roots of $\det(\mbf I_r -\mbf Az)= 0$ are $z_j^*\in \mathbb C$, $j=1,\ldots, r$  such that $\vert z_j^*\vert \ge M_A$ for some finite positive real $M_A > 1$ independent of $j$.
