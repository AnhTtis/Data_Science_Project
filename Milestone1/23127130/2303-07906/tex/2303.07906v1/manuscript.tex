...
%%is a widely used encoding method by which each feature is encoded in one qubit, and
...
%
%%A third register is initialized in 0 and is used to implement the quantum feature. The application of Ry(x)  on this register with its argument being xj can be done by performing the rotation conditioned on the first register with classical values xj, i.e., a conditional rotation c?Ry(x).
%
%
...
%\end{aligned}
%\end{equation}
%% apply a unitary transform consisting of p layers of interleaved operations
%
...
%\end{aligned}
%\end{equation}
%where $N_1$ and $N_2$ represent the odd and even subsets of $[0, N-1]$. For $l_1$-layer structure, the ansatz can be written as
%$W(\theta)=\prod_{k=1}^{l_1}W^k(\theta)$.
%
...
%
%%quantum metric learning needs to select the dimensions of output states according to subsequent operations. QCNN adopts a intersecting circuit structure to extract the features of samples, where pooling layers reduce the dimension of the output state while preserving information about relevant data features.
%
%
%
%%引入非线性
...
%%The expectation computation converts the output into classical data.
...
%\end{equation}
%
% %Repeat multiple iterations to optimize the loss function $L_l(\theta, x^a_i, x^p_{i}, x^n_{i})$ until satisfying stopping criteria, the model gets the optimal parameter $\theta^*$.
%
%
...
%%, which is helpful for creating the superposition state of multiple triplet sets.
%%It is possible to embed multiple triplet sets into one quantum state.
%%The precise context in which the term simultaneous is used here will become clear later in the paper.
