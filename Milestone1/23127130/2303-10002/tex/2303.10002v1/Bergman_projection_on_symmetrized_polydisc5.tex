...
\[p_j(w_1,w_2,\dots,w_n)=\sum_{k_1<k_2<\cdots<k_j}w_{k_1}w_{k_2}\cdots w_{k_j}.\]
 The symmetrized polydisc $\mathbb G^n$ is the image of $\mathbb D^n$ under $\Phi_n$:
	\[\mathbb G^n:=\{(p_1(w),\dots,p_n(w)):w\in \mathbb D^n\}.\]	
When $n=2$, the symmetrized bidisc $\mathbb G:=\mathbb G^2$ serves as an interesting example in several complex variables. It is a first known example of many interesting phenomenons. We list some of them here below: 
\begin{itemize}
	\item the Lempert theorem may hold on bounded pseudoconvex domains that are not biholomorphically equivalent to any convex domain. \cite{AY} 
	\item bounded $\mathbb C$-convex domains are not necessarily boholomorphically equivalent to convex ones. \cite{Nikolov_Pflug_Zwonek_2008}
\end{itemize} 
Partially due to $\mathbb G^n$'s interesting geometric properties, the $L^p$ regularity of $P_{\mathbb G^n}$ has also attracted attentions in recent years. In \cite{Liwei19}, Chen, Krantz, and Yuan showed that $P_{\mathbb G^n}$ is $L^p$ bounded for $p\in (1+\frac{n-1}{\sqrt{n^2-1}},1+\frac{\sqrt{n^2-1}}{n-1})$. Later, Chen, Jin, and Yuan \cite{CJY} improved the $L^p$ regular range of  $P_{\mathbb G}$ to $({4}/{3},4)$ and established the Sobolev estimates for $P_{\mathbb G}$. While preparing this paper, the authors were informed of a discrepancy between the arXiv version of \cite{CJY} and the version those authors submitted to a journal for publication. In a recent update of \cite{CJY} posted to the arXiv, the range of $L^p$ regularity for the symmetrized polydisc is at least $(\frac{2n}{n+1},\frac{2n}{n-1})$, see \cite[Remark 1.5]{CJY23}. 

The main idea in the proof of these results is to use  Bell's transformation formula to reformulate the $L^p$ regularity problem of $P_{\mathbb G^n}$  into a weighted $L^p$ regularity problem of $P_{\mathbb D^n}$ over a weight $L^p$ space of anti-symmetric functions. There are mainly two challenges for the problem: 1. the cancellation caused by integrating anti-symmetric functions creates obstacles to finding  examples for irregularity;  2. the complexity of  the Jacobian of $(p_1,\dots,p_n)$ for large $n$ dimension also makes estimations hard to compute. Due to these issues, the sharp $L^p$ regular range for $P_{\mathbb G^n}$ was not known.

In this paper, we give the precise $L^p$ regular ranges for $P_{\mathbb G^n}$ and $P^+_{\mathbb G^n}$:
...
\begin{enumerate}
	...
	\item We prove in details the $L^p$ boundedness results for $p\in(\frac{2n}{n+1},\frac{2n}{n-1})$ using known weighted estimates on the polydisc. (See Theorem \ref{main10}, Section 3, and \cite[Remark 1.5]{CJY23}).
...
\[P^+_{\mathbb D^n}(|J_{\mathbb C}\Phi_n|\cdot(h\circ \Phi_n))=|J_{\mathbb C}\Phi_n|\cdot (P^+_{\mathbb G^n}(h)\circ \Phi_n).\] Then the estimate  
...
\end{proof}
By Lemma \ref{2.1} and the fact that $|P_{\mathbb G^n}(f)(z)|\leq P^+_{\mathbb G^n}(|f|)(z)$, the next two theorems are sufficient to yield Theorem \ref{main}.
...
$P_{\mathbb D^n}$ is unbounded on $L^p_{\text{anti}}(\mathbb D^n,|J_{\mathbb C}\Phi_n|^{2-p})$ for all $p=\frac{2n}{n-1}$.
\end{thm}
...
\section{Proof of Theorem \ref{main10}} While the paper \cite[Remark 1.5]{CJY23} sketches the proof of Theorem 2.3, here we provide all the relevant details to make the paper self-contained. The proof for Theorem \ref{main10} uses the weighted norm estimates of the positive Bergman operator $P^+_{\mathbb D}$ over weighted $L^p$ spaces. 
On the unit disc $\mathbb D$, the boundedness of $P_{\mathbb D}$ and $P^+_{\mathbb D}$ on weighted $L^p$ spaces is closely related to the Bekoll\'e-Bonami constant of the weight.
...
\begin{thm}[Bekoll\'e-Bonami {\cite{BB78}}]
	...
	\end{enumerate} 
\end{thm}
Using dyadic harmonic analysis technique, various authors established quantitative weighted $L^p$ norm estimates of the Bergman projection. See \cite{Pott,Rahm,HW,HWW2}. 
...
If $|a-z|\geq 3(1-|z|)$, then $1-|z|\leq |a-w|\leq 5(1-|z|)$ for all $w\in T_z$ and hence
\begin{align*}
	&\frac{\int_{T_z}u_p(w) dV(w)}{\int_{T_z}dV(w)}\left(\frac{\int_{T_z}u_p^{-\frac{1}{p-1}} (w)dV(w)}{\int_{T_z}dV(w)}\right)^{p-1}\\\lesssim& 	\frac{(1-|z|)^{2-p}\int_{T_z} dV(w)}{\int_{T_z}dV(w)}\left(\frac{(1-|z|)^{(p-2)/(p-1)}\int_{T_z} dV(w)}{\int_{T_z}dV(w)}\right)^{p-1}\\=&1.
\end{align*}
...
Now we turn to the case of weight $v_p(w)=\prod_{j=1}^{m}|a_j-w|^{2-p}$. By a similar proof as above, $B_p(|a-w|^{(2-p)m})\lesssim1$ for any $p\in(\frac{2m+2}{m+2},\frac{2m+2}{m})$ where the upper bound is independent of $a$. Using the arithmetic-geometric mean inequality and H\"older's inequality, we obtain for any $z\in \mathbb D$
\begin{align*}
	&\frac{\int_{T_z}v_p(w) dV(w)}{\int_{T_z}dV(w)}\left(\frac{\int_{T_z}v_p^{-\frac{1}{p-1}} (w)dV(w)}{\int_{T_z}dV(w)}\right)^{p-1}\\\lesssim& 	\frac{\sum_{j=1}^{2m}\int_{T_z} |a_j-w|^{m(2-p)}dV(w)}{\int_{T_z}dV(w)}\left(\frac{\sum_{j=1}^m\int_{T_z} |a_j-w|^{m(p-2)/(p-1)}dV(w)}{\int_{T_z}dV(w)}\right)^{p-1}\\\lesssim&\left(\sum_{j=1}^{m}\left(\frac{\int_{T_z} |a_j-w|^{m(2-p)}dV(w)}{\int_{T_z}dV(w)}\right)^{\frac{1}{p}}\left(\frac{\int_{T_z} |a_j-w|^{m(p-2)/(p-1)}dV(w)}{\int_{T_z}dV(w)}\right)^{1-\frac{1}{p}}\right)^p\\\lesssim&\left(\sum_{j=1}^{m}B_p(|a_j-w|^{m(2-p)})\right)^p\lesssim 1.
\end{align*}
...
Since $|P_{\mathbb D^n}(h)(z)|\leq P^+_{\mathbb D^n}(|h|)(z)$ for any $h\in L^p(\mathbb D^2,|J_{\mathbb C}\Phi_n|^{2-p})$, it suffices to show the boundedness for $P^+_{\mathbb D^n}$.
Note that $J_{\mathbb C}\Phi_n(w)$ consists of $n-1$ many factors of each variable $w_j$. When integrating with respect to the single variable $w_j$, only these  $n-1$  factors matter in $J_{\mathbb C}\Phi_n(w)$. Thus the boundedness of $P^+_{\mathbb D^n}$ on $L^p(\mathbb D^2,|J_{\mathbb C}\Phi_n|^{2-p})$ for $p\in\left(\frac{2n}{n+1}, \frac{2n}{n-1}\right)$ follows from  Fubini and Lemma \ref{lem2.5} with $m=n-1$.\end{proof}
...
we set $$h_s(w)=\frac{1}{\pi(1-sw_1)^2}+\frac{1}{\pi(1-sw_2)^2}.$$
	Then
	\begin{align*}
		\|h_s\|^4_{L^4_{\text{sym}}(\mathbb D^2,|J_{\mathbb C}\Phi_2|^{2})}&=\int_{\mathbb D^2}\left|\frac{1}{\pi(1-sw_1)^2}+\frac{1}{\pi(1-sw_2)^2}\right|^4| w_1- w_2|^2dV(w)\\&\lesssim\int_{\mathbb D}\frac{1}{\pi^p|1-sw_1|^{8}}\int_{\mathbb D}| w_1- w_2|^{2}dV(w_2)dV(w_1)
		\\&\approx (1-s)^{-6},
	\end{align*}
	...
	\begin{align*}&\int_{U^2(s)}\frac{|z_1-z_2|^2}{|1-z_1s|^8|1-z_2s|^4}dV(z)\\=&\int_{U^2(s)}\frac{1}{|1-z_1s|^6|1-z_2s|^2}\left|\frac{1}{s(1-z_1s)}-\frac{1}{s(1-z_2s)}\right|^2dV(z)
		\\=&\int_{U^2(s)}\frac{1}{|1-z_1s|^6|1-z_2s|^2}\left(\frac{1}{|1-z_1s|^2}+\frac{1}{|1-z_2s|^2}-2\text{Re} \frac{1}{(1-z_1s)(1-\bar z_2 s)}\right)dV(z)\\\geq&\int_{U^2(s)}\frac{1}{|1-z_1s|^8|1-z_2s|^2}+\frac{1}{|1-z_1s|^6|1-z_2s|^4}-2 \frac{1}{|1-z_1s|^7|1-z_2s|^3}dV(z).
	\end{align*}
...
\subsection{The case for general $n$} Like the case $n=2$, our proof for general $n$ also involves a procedure of decompositions of $P_{\mathbb D^n}$ into bounded operators $T^n_1$ and $T^n_2$. 
...
\begin{proof}%We only show the proof for $T^n_1$. $T^{n+}_1$ can be proved by a similar argument.
	...
	%\end{itemize} 
If $K(z;\bar w)$  is $(j,k)$-symmetric in $w$, then for any anti-symmetric $f\in L^p_{\text{anti}}(\mathbb D^n,|J_{\mathbb C}\Phi_n|^{2-p})$
...
Then $P_1=T^n_2$ and $P_n=P_{\mathbb D^n}$. 
	We claim that $P_{\mathbb D^n}=P_{l}$  on $L^p_{\text{anti}}(\mathbb D^n,|J_{\mathbb C}\Phi_n|^{2-p})$ for all $l$ which implies that $T^n_1=P_{\mathbb D^n}-P_1=0$ on $L^p_{\text{anti}}(\mathbb D^n,|J_{\mathbb C}\Phi_n|^{2-p})$.
...
\end{thm}
\begin{proof}The proof for the case $n>2$ follows a similar argument as in the proof of Theorem \ref{main2}. 
...
For variable $w_j$, let $\mathcal D_{w_j}$ denote the partial differential operator
...
\[(\mathcal D_{w_j})^kf(w)=\sum_{\alpha\in \mathbb N^n} c_{\alpha}(m+1)^kw^\alpha.\]
Since for each integer $k>2$, $$\frac{1}{(1-w_js)^k}=\sum_{m=0}^{\infty}(m+1)_{k-1}s^mw_j^m=\sum_{m=0}^{\infty}(m+2)_{k-2}((m+1)s^mw_j^m)$$ where the Pochhammer symbol  $(m+2)_{k-2}=(m+2)\dot(m+3)\dots(m+k-1)$ is a polynomial in $m$ of degree $k-2$, there exists a polynomial $q_{k-2}$ of degree $k-2$ such that 
...
\end{align}
Set $U_n(s)=\{w\in \mathbb D: \text{Arg}(1-ws)\in \left(-\frac{\pi}{6(n-1)},\frac{\pi}{6(n-1)}\right)\}$. Then for any $z\in (U_n(s))^n$ and $\tau\in\mathcal S_n$
...
For $j\in\{1,\dots,n\}$, we set \[U_n(s,j)=U_n(s)\bigcap\left\{z:(5n!)^{2j}(1-s)<\left|z-\frac{1}{s}\right|<1\right\},\]
and set
$\mathbf U(s)=U_n(s,1)\times U_n(s,2)\times\cdots\times U_n(s,n).$
When $s$ is close enough to 1, 
\[U_n(s,j)=\left\{z:(5n!)^{2j}(1-s)<\left|z-\frac{1}{s}\right|<1, \text{Arg}(1-zs)\in\left(-\frac{\pi}{6(n-1)},\frac{\pi}{6(n-1)}\right)\right\}\]
for all $j$, and 
\begin{align}\label{4.19}\int_{U_n(s,j)}\frac{dV(z)}{|1-zs|^k}=&s^{-k}\int_{U_n(s,j)}\frac{dV(z)}{|z-s^{-1}|^k}\nonumber\\=&s^{-k}\int_{-\frac{\pi}{6(n-1)}}^{\frac{\pi}{6(-1)}}\int_{(5n!)^{2j}(1-s)}^{1}r^{1-k}drd\theta\nonumber\\=&\begin{cases}
	\frac{\pi}{3s^k(k-2)(n-1)}((5n!)^{2j(2-k)}(1-s)^{2-k}-1)& k>2\\-\frac{\pi}{3s^2}(2j\log 5n!+\log(1-s))& k=2
\end{cases}.\end{align}
For $s\in (0,1)$ and functions $f(s)$ and $g(s)$, we write $f(s)\sim g(s)$ if
\[\lim_{s\to 1^-}\frac{f(s)}{g(s)}=1.\]
Then (\ref{4.19}) yields
\begin{align}\label{4.31}\int_{U_n(s,j)}\frac{dV(z)}{|1-zs|^k}\sim	\begin{cases}
\frac{\pi(5n!)^{2j(2-k)}(1-s)^{2-k}}{3s^k(k-2)(n-1)}& k>2\\ -\frac{\pi\log(1-s)}{3s^2}& k=2
\end{cases}.\end{align}
Recall that for $\tau\in \mathcal S_n$, we let $\tau(j)$ be the index satisfying $z_{\tau(j)}=\tau(z_j)$. For  $p=\frac{2n}{n-1}$,
\begin{align}
...
Since $1\leq m\leq n-1$, the  denominator factor $|1-z_ms|$ in (\ref{4.32}) has degree strictly greater than 2. The factor $|1-z_ns|$ has degree 2 only if $\tau^{-1}(n)=n$, or equivalently $\tau(z_n)=z_n$. By the Forelli-Rudin estimates (\ref{**}) and the fact that $\{\tau^{-1}(1),\dots,\tau^{-1}(n)\}=\{1,\dots,n\}$,
\begin{align}&\int_{U_n(s,n)}\frac{dV(z_n)}{|1-z_ns|^{2n+2-2\tau^{-1}(n)}}\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{dV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2\tau^{-1}(m)}}\approx\begin{cases}
...
\end{align}
Hence, for any permutation $\tau\in \mathcal S_{n-1}$ that is not the identity,
\begin{align}
\frac{\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{dV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2m}}}{\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{dV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2\tau^{-1}(m)}}}&\sim(5n!)^{4\sum_{m=1}^{n-1}(m^2-m\tau^{-1}(m))}\geq 4n!.
\end{align}
Here $\sum_{m=1}^{n-1}(m^2-m\tau^{-1}(m))\geq 1$ follows by Cauchy-Schwarz inequality and the fact that $\sum_{m=1}^{n-1}(m^2-m\tau^{-1}(m))$ is an integer.
Substituting these estimates into (\ref{4.34}), we finally obtain
\begin{align}
&\int_{U_n(s,n)}\frac{dV(z_n)}{|1-z_ns|^{2}}\left(\frac{1}{2}\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{dV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2m}}-\sum_{\tau\in\mathcal S_{n-1}}\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{ndV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2\tau^{-1}(m)}}\right)\nonumber\\\gtrsim& \int_{U_n(s,n)}\frac{dV(z_n)}{|1-z_ns|^{2}}\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{dV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2m}}\left(\frac{1}{2}-\sum_{\tau\in\mathcal S_{n-1}}\frac{n}{4n!}\right)
\nonumber\\=&\frac{1}{4} \int_{U_n(s,n)}\frac{dV(z_n)}{|1-z_ns|^{2}}\prod_{m=1}^{n-1}\int_{U_n(s,m)}\frac{dV(z_m)}{|1-z_ms|^{\frac{2n^2}{n-1}+2-2m}}\approx-(1-s)^{-n^2-n}\log(1-s),
\end{align}
