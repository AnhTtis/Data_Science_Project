...
This is in contrast to out setting where no projection onto a submanifold is needed. 
...
A quantum wave function is a square-integrable function $\psi$ on a space of configurations $\Omega$. This is typically $\Omega=\mathbb R^{3N}$ for a system of $N$ electrons.\footnote{The spin degrees of freedom are considered as classical and are expressed in the symmetry type of $\psi_\theta$.} For any scalar $\lambda\in\mathbb{R}$ and $\lambda\neq0$, $\lambda\psi$ represents the same physical state as $\psi$.
...
The gradient of the VMC energy functional takes the form (see e.g., \cite{pfau2020ab})
\begin{equation}
...
the perturbation of the sampling distribution as $\theta$ varies. For completeness, we include the derivation of \cref{eqn:gradient_L_VMC} in \ref{gradproofs}. The condition that $\psi$ be $C^\infty$ with respect to $\theta$ is typically satisfied by using the logistic sigmoid activation function.
...
We therefore consider the problem of minimizing the distance from the line $\{\lambda \psi|\lambda\in\mathbb R\}$ to a target function $\varphi\in\Lt^2(\Omega)$.  That is, %\LL{ where is this $\rho$-norm defined?}~\zd{Added below.}
\begin{equation}\label{supervisedC}
\mathcal{L}(\psi)=\min_{\lambda\in \mathbb R}\left\|\lambda \psi-\varphi\right\|_\rho^2\,,
\end{equation}
where $\left\|\phi\right\|_\rho=\int_{\Omega}\phi(x)\mathrm{d}\rho(x)$ is the norm in $\Lt^2$-sense on a probability space $(\Omega,\rho)$ with probability measure $\rho$. 
%\begin{rev}
...
Minimizing $\mathcal{L}(\psi_\theta)$ is equivalent to minimizing $-\ol{g}{\psi_\theta}_\rho/\|\psi_\theta\|_\rho$. 
...
in the denominator of \cref{gradient_L_supervised} and do not have access to an unbiased estimator for \cref{gradient_L_supervised}. However, we will show that as long as the scaling can be crudely estimated (up to a constant factor), we could obtain a \emph{directionally unbiased} gradient estimator $G$, which means that $\EE[G]$ is in the positive span of $\nabla_\theta L$. 
...
This bound shows that the convergence of Algorithm \ref{algo:VMC} is the same as the convergence rate of SGD to first-order stationary point for nonconvex functions.
...
\subsubsection{Scale-invariant supervised pre-training} 
...
G=\frac{1}{\tilde{Z}^3}\frac1{n-1}\sum_{j=1}^n a_j\nabla_\theta \psi_{\theta}(X_j)\,,
\end{equation}
...
\begin{breakablealgorithm}
  ...
  \STATE $G_{m}\gets \frac{1}{\widetilde{Z}_{m}^3}\frac1{n-1}\sum_{i=1}^n a_i\nabla_\theta \psi_{\theta_m}(X^m_i)$;
  \STATE $\theta_{m+1}\gets \theta_m-\eta_m G_m$;
  \ENDWHILE
    \STATE \textbf{Output:} $\theta_m$;
    \end{algorithmic}
...
where $\psi_j(\cdot)=(\phi_j(x_1),\ldots,\phi_j(x_n))^T$, that is,
\begin{equation}
...
To evaluate the performance of the modified pre-training procedure we plot the angle between the target Slater determinant-based wave function and the neural network wave function during each training procedure (\cref{fig:comp}). Note that the plotted loss is the same for both training runs.    
The system shown is the lithium atom (details in \ref{sec:appnumerics}). 
...
\textbf{Note.} 
We note that another recent (concurrent) study also proved the convergence of VMC~\cite{Wen_2023}. The work of~\cite{Wen_2023} proves a similar convergence result and also takes into account the effect of Markov chain Monte Carlo sampling. It does not consider the setting of supervised pre-training.
...
\begin{lemma}\label{lem:svsl_2} Under \cref{assum_g_scale_inva} there exists a constant $C'$ that only depends on $C_g$ such that for any $\theta,\widetilde{\theta}\in\mathbb{R}^d$,
...
L(\theta_{m+1})\leq L(\theta_m)-\eta_m\nabla_\theta L(\theta_m)\cdot G_m+\frac{C\eta_m^2}{2}|G_m|^2
\]
where we use \eqref{eqn:gradient_Lip_2}. Here $C$ is a constant that only depends on $C_g$. Because
