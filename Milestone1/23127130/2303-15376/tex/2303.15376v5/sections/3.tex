...
In this section, we aim to determine whether it is possible, at least at the population level, to infer a causal graph from a joint distribution under the assumptions outlined in the previous section. 
...
\end{enumerate}
\end{proposition}
\begin{hproof} While this proposition follows as a special case of Theorem~\ref{thmAssymetricMultivariatesufficient}, we outline the key ideas behind the proof. Intuitively, condition \eqref{eq000} serves to break the symmetry; in the Gaussian ANM case, identifiability holds if and only if the link function is nonlinear \citep{Zhang2009}. This nonlinearity condition ensures that \( F_{X_1, X_2} \) is not symmetric with respect to the \( y = a + bx \) axis. Condition \eqref{eq000} extends this intuition to more general distributions.
...
\textbf{1)} If the supports of $X_1$ and $X_2$ differ, then $X_1$ trivially can not be written as $X_1 = F^{-1}\big(\varepsilon_1; \tilde{\theta}(X_2)\big)$  since the support of any distribution in the exponential family is fixed. This immediately rules out non-identifiability in such cases.
...
\begin{equation}\label{eq69}
 ...
 \end{equation}
Since $F$ belongs to the exponential family, using notation from \hyperref[appendix_exponential_family]{Appendix} \ref{appendix_exponential_family}, we rewrite the conditional densities as:$$p_{X_2\mid {X_1}}(y\mid x) = h_{1}(y)h_{2}[\theta(x)]\exp[\theta(x)T(y)], \,\,\,\,\,\,p_{{X_1}\mid {X_2}}(x\mid y) = h_{1}(x)h_{2}[\tilde{\theta}(y)]\exp[\tilde{\theta}(y)T(x)].$$
