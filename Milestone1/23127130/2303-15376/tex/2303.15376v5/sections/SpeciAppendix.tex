\section{Extended Definitions and Excluded Concepts}
\label{Appendix_A}
...
In the case where $F$ is a location family distribution (such as a Gaussian distribution with fixed variance, where $Y = \theta(X) + \varepsilon$), property \eqref{gdert} reduces to classical notion of a weak universal consistency:
\(
\lim_{n\to\infty}\mathbb{E}[(\theta(X) - \hat{\theta}(X))^2] = 0.
\)
This result has been established for various estimators under appropriate smoothness assumptions on $\theta$. Examples of such estimators include:
\begin{itemize}
...
\end{itemize}
 ...
 In the case where $F$ is a location-scale Gaussian distribution, some consistency results as been established for various estimators under appropriate smoothness and regularity assumptions, see e.g. Theorem 2 in \cite{immer2022identifiability}, or \cite{Siegfried02102023, Le_Smola}. 

Consistency results for non-Gaussian families are less common in non-parametric settings. Theorem 1 in \cite{GAMSamworthConsistency} establishes the almost sure universal consistency of GAM estimators for a general exponential family of distributions for \( Y \mid X \), assuming certain smoothness and convexity/monotonicity conditions on \( \theta \). \cite{Wood2} discusses general framework for smoothing parameter estimation for models with regular likelihoods constructed in terms of unknown smooth functions of covariates.   \cite{mammen1997} discusses the consistency under partial linearity assumption.  GAMLSS \citep{GAMLSS} offers a broad class of estimators, with \cite{GAMLSS_webpage} presenting extensive empirical evidence on simulations and hundreds of real-data examples demonstrating consistency.
