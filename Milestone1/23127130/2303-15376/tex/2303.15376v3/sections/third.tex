...
In the remainder of the section, we answer the following question: Under what conditions is the causal graph identifiable? We begin with the $CPCM(F)$ case. 
...
Theorem \ref{normalidentifiability} indicates that the non-identifiability holds only in the ``special case,'' when $\frac{\mu(x)}{\sigma^2(x)}, \frac{-1}{2\sigma^2(x)}$ are linear and quadratic, respectively. Note that natural parameters of a Gaussian distribution are  $\frac{\mu}{\sigma^2}, \frac{-1}{2\sigma^2}$, and sufficient statistics of the Gaussian distribution have a linear and quadratic form (for the definition of the exponential family, natural parameter and sufficient statistic, see \hyperref[appendix]{Appendix} \ref{appendix_exponential_family}). We show that such connections between non-identifiability and sufficient statistics hold in the more general context of the exponential family.  
...
\begin{equation}\label{eq69}
 ...
 \end{equation}
Since $F$ lies in the exponential family of distributions, we use the notation from \hyperref[appendix]{Appendix} \ref{appendix_exponential_family} and rewrite this as $$p_{X_2\mid {X_1}}(y\mid x) = h_{1}(y)h_{2}[\theta(x)]\exp[\theta(x)T(y)]$$
...
$f(x) + g(y) = a_1T(x) + a_2T(y) +a_3$ for some constants $a_, a_2, a_3\in\mathbb{R}$. Therefore, fixing $y$ yields $f(x)=\log\bigg\{  \frac{ p_{X_1}(x)h_{2}[\theta(x)]}{h_{1}(x)}\bigg\} = a_1T(x) + const$. Rewriting this yields 
$
...
The following theorem reveals that the $CPCM(F_1, F_2)$ model is ``typically'' identifiable.  
...
The proof is provided in \hyperref[Proof of thmAssymetricMultivariatesufficient]{Appendix} \ref{Proof of thmAssymetricMultivariatesufficient}. Note that condition (\ref{eq158}) is sufficient, but not necessary for identifiability. Similar to that in the Gaussian  (Theorem \ref{Gaussian case}) or Pareto cases (Consequence \ref{paretoidentifiability}), in order to obtain a necessary condition, the distribution of the cause also needs to be restricted.
...
\end{itemize}
\end{consequence}
Details regarding Consequence \ref{consequenceprva}, the definitions of the distributions, and their sufficient statistics are provided in  \hyperref[consequence]{Appendix} \ref{consequence}. We emphasize that not all constants $a_i,b_i,c_i,d_i,e_i,f_i, i=1,2,$ yield a valid (non-identifiable) model.
