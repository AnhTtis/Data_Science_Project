...
Numerous important distributions lie in the exponential family of continuous distributions, such as Gaussian, Pareto (with fixed support), log-normal, Gamma, and Beta distributions, to name a few. 

Functions in (\ref{Exponential family of distributions}) are \textit{not} uniquely defined. For example, $T_i$ is unique up to a linear transformation. 

The support of $f$ (defined as $supp(f) = \{x\in\mathbb{R}: f(x;\theta)>0\}$) is fixed and does not depend on $\theta$. Potentially, $T_i, h_1$ do not have to be defined outside of this support; however, we typically overlook this fact (or possibly define $h_1(x) = T_i(x) = 0$ for $x$ where these functions are not defined). If we assume that $f$ is continuous (which is typically the case in this paper), we additionally assume that the support is nontrivial in the sense that it includes an open interval. 
...
\end{pmatrix} 
\end{equation}
has full rank. Moreover, $T_1, \dots, T_q$ are affinly independent in the following sense: there exists $y_0, y_1, \dots, y_n\in supp(f)$, such that a matrix 
...
Plots corresponding to the datasets from Simulations 2 and 3 are presented in Figures \ref{Simulations2_plots} and  \ref{Simulations3_plot}. 
Plots corresponding to the application are drawn in Figures \ref{Just_plots} and \ref{Just_histograms}. 

\subsection{Baselines implementations from Simulations 2}
As mentioned earlier, the experiments from Simulations 2 were inspired by \cite{Natasa_Tagasovska} and implementations of other baseline methods are also taken from \cite{Natasa_Tagasovska} and \cite{immer2022identifiability}. 
...
\caption{ An example of datasets generated from different models in Simulation 2.}
