...
$$X_i\mid \textbf{X}_{pa_i}\sim Poisson\big(\theta(\textbf{X}_{pa_i})\big),$$where $\theta$ is a measurable function describing the rate of certain phenomena. Such a model is common in applications when $X_i$ represents a number of events occurring in a certain time period. 
\end{example}
...
\label{Setup}
Consider a DAG (directed acyclic graph) $\mathcal{G}=(V,E)$ with a finite set of vertices (nodes) $V=\{1, \dots, d\}$ and a set of directed edges $E$. We adapt the usual notation of graphical models  (e.g., \citealp{PCalgorithm}); for example, we write $pa_i(\mathcal{G})$, $ch_i(\mathcal{G})$ and $an_i(\mathcal{G})$ for parents, children and ancestors of the node $i$, respectively. In addition, we say that the node $i\in V$ is a source node if $pa_j(\mathcal{G})=\emptyset$, notation $i\in Source(\mathcal{G})$.  We omit the argument $\mathcal{G}$ if evident from the context.  

We use capital $F$ for distributions and small $p$ for densities. A random variable $Z$ that is uniformly distributed on $(0,1)$ is denoted as $Z\sim U(0,1)$. Support of a random variable $Z$ is denoted as $supp(Z)$. Given a random vector $\textbf{X }= (X_i)_{i\in V}$ over some probability space with distribution $F_\textbf{X}$, we identify the vertices $j \in V$ with the variables $X_j$.  We denote $\textbf{X}_S = \{X_s{:}\,\, s\in S\}$ for $S\subseteq V$. We focus on the case in which $\textbf{X}$ has either a continuous joint density function $p_\textbf{X}$ with respect to the Lebesgue measure or a counting measure. 
...
A core concept in causal inference is the identifiability of the causal structure. Consider a SCM with conditional densities that satisfy (\ref{def987}). 
...
Without strong restrictions of class $\mathcal{H}$, we cannot obtain the identifiability of $\mathcal{G}$. Generally, we can only identify the Markov equivalence class \citep[Proposition 7.1.]{Elements_of_Causal_Inference}. 
...
\end{equation*}
with an invertible link function $g_1$.  Here, potentially $X_i = g_2(\textbf{X}_{pa_i})\cdot \varepsilon_i$ for the special choice $g_1(x)=e^x$.  
...
The structure of the paper is as follows.  We lay out the main definitions and motivation in Section \ref{Section2}.   Section \ref{Section_identifiability} provides the identifiability results of the causal structure in the bivariate case. 
In Section \ref{Section4}, we discuss the multivariate extension. 
In Section \ref{Section5}, we propose an algorithm for estimating the causal graph under assumption (\ref{11}). In Section \ref{simulations_section}, we proceed to a short simulation study, followed by an illustration of  our methodology on a dataset concerning income and expenditure of residents of Philippines. 
We provide three appendices: Appendix \ref{appendix_exponential_family} contains a detailed definition and discussion about the Exponential family of distributions. Appendix \ref{SectionProofs} contains proofs of the theorems and lemmas presented in the paper. Appendix \ref{Appendix_simulations} presents some details about the simulation study and the application. 
