...
\textbf{Second bullet-point:} For a clarity in the indexes, we use the notation $X=X_1, Y=X_2$ and we assume $X\to Y$. 
...
By simple algebra, we obtain
 ...
 $$
In other words, $p_X(x) = \frac{const.}{ [a\log(x)+b] x^{d+1} }$, which is what we wanted to prove (note that we re-named the variables in the end for clarity).

\textbf{First bullet-point: }Poisson distribution has one parameter and it can be written as $h_1(x) = 1/x!, h_2(x) = e^{-e^x}, T(x) = x$. Note that we do not use classical form of density function but its reparametrisation where $\theta(x) = \log(\lambda(x))$ where $\lambda$ is the classical rate parameter. Plugging this into Proposition~\ref{Necessary condition for identifiability}, we directly obtain (\ref{eq505}) with possible $a\in\mathbb{R}$. It is not hard to see that $ \frac{e^{e^{ax+b}+cx}}{x! }$ is integrable if and only if $a<0$. In such a case, the backward model exist and has a form $\tilde{\theta}(y) = a y+c$ and $P(X_2=y) \propto \frac{e^{e^{ay+c}+by}}{y! } $. 


\textbf{Third bullet-point:} We can always write a backward model for the Bernoulli distribution. Let $P(X_1=X_2=0) = p_0$,  $P(X_1=0, X_2 = 1) = p_{0,1}$, $P(X_1=1, X_2 = 0) = p_{1,0}$ and $P(X_1=X_2 = 1) = p_{1}$ for $p_0, p_{0,1}, p_{1,0}, p_1>0$ and $p_0 + p_{0,1} + p_{1,0} + p_1 = 1$. We can define $X_2\mid X_1\sim Bernoulli(\theta(X_1))$ as $\theta(0) = p_{0,1}$ and $\theta(1) = p_1$. On the other hand, we can define $X_1\mid X_2 \sim Bernoulli(\tilde{\theta}(X_2))$ as $\tilde{\theta}(0) = p_{1,0}$ and $\tilde{\theta}(1) = p_{1}$. Since both models produce the same joint distribution, the causal model is not identifiable for any values of $p_0, p_{0,1}, p_{1,0}, p_1$.  
\end{proof}
...
Let $n\in\mathbb{N}$ and $\mathcal{S}\subseteq \mathbb{R}$. Let $f_1, \dots, f_n, g_1, \dots, g_n$ be non-constant functions on $\mathcal{S}$, such that 
$
...
We assume $supp(X_1) = supp(F_1)$; if this equality does not hold, the causal graph is trivially identifiable. Moreover, assume that $\mathcal{S}$ contains at least two distinct values; otherwise, the statement is trivial. 
...
The density function of the \textbf{Gamma distribution} with parameters \(\theta = (\alpha, \beta)^\top\) is given by \(p(x) = \frac{\beta^\alpha}{\Gamma(\alpha)} x^{\alpha - 1} e^{-\beta x}\), \(x > 0\). The sufficient statistics are \([T_1(x), T_2(x)] = [\log(x), x]\).

The density function of the \textbf{Beta distribution} with parameters \(\theta = (\alpha, \beta)^\top\) is given by \(p(x) = \frac{1}{B(\alpha, \beta)} x^{\alpha - 1} (1 - x)^{\beta - 1}\). The sufficient statistics are \([T_1(x), T_2(x)] = [\log(x), \log(1 - x)]\).

The definitions and details about the sufficient statistics of the \textbf{Poisson distribution} can be found in the proof of Consequence~\ref{paretoidentifiability}.

The first and third points follow directly from Theorem \ref{thmAssymetricMultivariatesufficient}. In the second point, the forms of \(\lambda\) and \(\mu\) also follow directly from Theorem \ref{thmAssymetricMultivariatesufficient}. We provide the calculations for the forms of the marginal distributions. 

For clarity of notation, we denote \(X = X_1\) and \(Y = X_2\). Write
\begin{equation*}
\begin{split}
\log(p(x, y)) &= \log[p_X(x)] + \log[p_{Y \mid X}(y \mid x)] = \log[p_X(x)] + y \theta(x) - e^{\theta(x)} - \log(y!) \\
&= \log[p_Y(y)] + \log[p_{X \mid Y}(x \mid y)] = \log[p_Y(y)] - \log\sigma - \frac{1}{2} \frac{[x - \mu(y)]^2}{\sigma^2}
\end{split}
\end{equation*}
for all \(x \in \mathbb{R}\) and \(y \in \{0, 1, \dots\}\). Rearranging, we obtain
\begin{equation*}
\begin{split}
y \theta(x) - x \mu(y) / \sigma^2 &= \underbrace{e^{\theta(x)} - \log[p_X(x)] - \frac{1}{2} \frac{x^2}{\sigma^2}}_{f(x)} + \underbrace{\log[p_Y(y)] + \log(y!) + \frac{1}{2} \frac{\mu(y)^2}{\sigma^2} - \log\sigma}_{g(y)}.
\end{split}
\end{equation*}
By Lemma~\ref{PomocnaLemma1}, both \(\theta(x)\) and \(\mu(y)\) are linear functions.

Rearranging the previous equality, we obtain
\begin{equation*}
\begin{split}
\log[p_X(x)] &= e^{\theta(x)} - y[ax + b] - \frac{1}{2 \sigma^2} x^2 + \frac{\mu(y)}{\sigma^2} x \\
&\quad + \left( \log(y!) + \log[p_Y(y)] - \log\sigma - \frac{1}{2} \frac{\mu(y)^2}{\sigma^2} \right) \\
&= \lambda(x) + d x^2 + e x + f,
\end{split}
\end{equation*}
where we simply fixed \(y = 1\) and used \(\theta(x) = \log(\lambda(x)) = ax + b\). This expression is equivalent to \(p_{X_1}(x) \propto e^{\lambda(x) + c x^2 + d x}\). Similarly, for \(p_Y(y)\) we get
\begin{equation*}
\begin{split}
\log[p_Y(y)] &= \left( -e^{\theta(x)} + \frac{1}{2 \sigma^2} x^2 - \log[p_X(x)] + \log\sigma \right) + y[ax + b] - \mu(y) \frac{x}{\sigma^2} - \log(y!) + \frac{\mu(y)^2}{2 \sigma^2}.
\end{split}
\end{equation*}
Fixing \(x\) and using an exponential transformation gives us the desired result.
\end{proof}
