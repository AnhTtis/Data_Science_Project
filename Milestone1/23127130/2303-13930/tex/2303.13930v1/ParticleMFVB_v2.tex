...
Another, and often more efficient, class of MCMC methods is based on the Langevin dynamics (sec. \ref{subsec:LMC})
\begin{equation} \label{eq:Langevin dynamics}
    dX_t = -\frac12\nabla U\left(X_t\right)dt + dB_t
\end{equation}
...
and employs the theory of Wasserstein gradient flows to establish the theoretical guarantee. 
...
This important connection between Wasserstein gradient flows and the Langevin dynamics, and Stochastic Differential Equation (SDE) theory in general, provides the theoretical foundation for our PMFVB procedure.
...
that are moved by combining classical MFVB framework with the Langevin diffusion.
...
We then develop a variant of PMFVB for inference in Bayesian deep learning, in which ingredients are introduced to make PMFVB suitable for deep neural networks.
...
The most closely related work to ours is \cite{Yao.Yang:2022MFVB-WGL} who, by focusing on a particular MFVB framework for statistical models with local latent variables\footnote{In this setting, each observed data point $y_i$, $i=1,...,n$, conditional on a local latent $z_i$ and model parameters $\theta$, has a tractable distribution $p(y_i|z_i,\theta)$.},
...
Our PMFVB framework is more general than earlier works, which assumes only regular condition for the framework, that is, in principle our work can be applied to any statistical models including deep learning.
...
\cite{Lambert:2022VI_WGL} study convergence results of Gaussian Variational Inference (VI) using the theory of Bures-Wasserstein gradient flow and close the theoretical gap between Gaussian VI and MCMC. They use particles to realise the flow of Gaussian approximations, and also extend the theory to mixtures of Gaussian. 
...
For a vector-valued function $v$ defined on $\SetR^d$, $\nabla\cdot v(x)$ denotes its divergence, i.e. $\nabla\cdot v(x) = \sum_j\frac{\partial v_j(x)}{\partial x_j}$.
...
v^{(k)}(x) = \nabla\log\pi(x)-\nabla\log q^{(k)}(x). 
\eeq
It is proved that (see, e.g., \cite[Chapter 8]{Santambrogio:OTbook} and \cite[Chapter 10]{Ambrosio:OTbook}), as $\eps\to0$, the discrete-time solution $\{q^{(k)}\}_{k=0,1,...}$ from \eqref{eq: JKO} converges to the continuous-time
...
\eean
which justifies that $\{q_t\}_{t\geq0}$ is a {\it gradient flow} minimizing the KL functional $F(q)$.
...
Therefore the curve $\{q_t\}_{t\geq0}$ induced by the Langevin dynamics can be viewed as a gradient flow that minimises some sort of discrepancy between $q_t$ and $\pi$; see, \cite{Dalalyan:JRSSB2017} and \cite{Cheng:ALT2018}.

For a fixed $h>0$, consider the following Langevin Monte Carlo (LMC) diffusion which is a discretisation approximation of \eqref{eq: Langegin diffusion}
\beq\label{eq: Langegin MC diffusion}
\d X_t^h=\frac{1}{2}\nabla\log\pi(X_{\tau(t)}^h)\d t+\d B_t,\;\;\;t\geq0,
\eeq
where $\tau(t):=kh$ if $t\in[kh,(k+1)h)$. It implies from \eqref{eq: Langegin MC diffusion} that, 
...
\eeq
Denote by $\mu_t^h$ the distribution of $X_t^h$, $t\geq0$.
The following lemma is proved in \cite{Cheng:ALT2018}, which says that the functional $F(\cdot)$ is reduced along the curve $\{\mu_t^h\}_{t\geq0}$.
...
The Langevin dynamics in \eqref{eq: Langegin diffusion} converges to the invariant distribution $\pi(\d x)$,
however, its convergence rate is not optimal. Many studies have aimed to improve the speed of convergence. A simple yet effective method is to add a momentum term to the drift coefficient $\nabla\log\pi(x)$; see, e.g., \cite{Hwang:AAP2005} and \cite{Kim.et.al:2022}.
...
\eeq
We study in Section \ref{sec: MFVB propoerties} conditions under which the problem in \eqref{eq: MFVB 1} is well-defined and has the unique solution.
Define
...
the solutions in \eqref{eq: MFVB 2}-\eqref{eq: MFVB 3} are simply $q_x^*(x)$ and $q_y^*(y)$ respectively.
...
We propose in the next section a method for solving \eqref{eq: MFVB 1} without making those assumptions, and study in Section \ref{sec: MFVB propoerties} the theoretical properties of $F(q)$ and its minima $q^*$.
...
of \eqref{eq: MFVB 2} and \eqref{eq: MFVB 3} are not available in closed form,  
we use Langevin Monte Carlo diffusions to iteratively approximate them.
We assume below that both $q_x^*(x)$ and $q_y^*(y)$ are intractable; in MFVB applications where the variational distribution is factorized into $K$ blocks, $q=\prod_{k=1}^Kq_k$, one only needs to use Langevin Monte Carlo diffusions to approximate those optimal solutions $q_k^*$ that are intractable.
Lemma \ref{lem:Cheng and Bartlett} indicates that the KL functional $F(q)$ is reduced after each iteration,
together with the results that $F(q)$ has the unique solution (c.f. Corollary \ref{cor: unique solution}),
...
We note that, unlike in Section \ref{sec:Preliminaries} where $t$ denoted continuous time, in this section $t$ denotes the $t$th iteration in the PMFVB algorithm.
...
\State Input: number of particles $M$, tolerance $\epsilon>0$, initial distributions $q^{(0)}_{x},q^{(0)}_{y}$.
...
However, as the LMC diffusion already spreads the particles by adding a Gaussian noise to them (c.f. \eqref{eq: Langegin MC diffusion 2}), hence circumvent the convergence to a degenerate measure. At the $t$th iteration, given the $M$ particles $\{X_i^{(t)},Y_i^{(t)}\}_{i=1}^M$ approximating $q$, we suggest approximating the entropy $H(q)$ by $\log(M)$.
The lower bound term at iteration $t$th can then be approximated by
...
Under the assumptions in Theorem \ref{the: theorem 1}, $F(q)$ has an unique minima on $\Q$. 
\end{corollary}
...
data generating parameter exists. To study these questions, we first define some notation.
...
\eeqn
with the space of probability measures $\mathcal Q$ having the factorised form in Section \ref{sec:reviewMFVB}, $\mathcal Q=\P(\X)\otimes\P(\Y)$, $\Theta=\X\times\Y$. We note that we do not need to equip $\mathcal Q$ with the Wassterstein distance in this section.
...
It turns out that the variational family $\mathcal Q$ in our particle MFVB is general enough; using the results in \cite{ZhangGao:2019}, we will show that the PMFVB approximation $q_n^*$ shares the posterior consistency as the posterior $\pi_n(\d\theta)$ does.
...
\end{cases}
\eeqn
Assumptions (A1) and (A2) are standard assumptions in the Bayesian statistics literature \citep{ghosal2000,ZhangGao:2019}, that are used to characterize the convergence rate of the posterior distribution.
...
%-------------------------------------------------------%
Although Bayesian logistic regression is a benchmark model in statistics,
given the lack of a conjugate prior in this problem, it is not straightforward to use the classical MFVB method.
We demonstrate in this section that it is straightforward to use the PMFVB method for Bayesian logistic regression. 
Consider the model
\beq\label{eq:BLR 1}
...
The PMFVB algorithm \ref{Alg: particle MFVB} maintains a set of $M$ particles $\{\theta_{1,i}^{(t)},\theta_{2,i}^{(t)}\}_{i=1,...,M}$ over iterations $t\geq 0$. At iteration $t+1$, according to \eqref{eq: update X}, the first block of the particles is updated as
...
\end{figure}
Figure \ref{fig:blogistic_hist} plots the marginal posteriors using kernel density estimation based on the PMFVB particles and Hamiltonian MC, and we can see that the results are  similar. 
\begin{figure}[H]
\centering
...
However, to the best of our knowledge, MFVB has never been successfully used for SV.
...
The CPU time taken by PMFVB and SMC was 8.2 and 29.3 minutes, respectively.
The running time for SMC depends on many factors such as the number of particles used in the particle filter for estimating the likelihood and the number of Markov moves. We select these numbers following what is typically used in the literature; see, e.g., \cite{Gunawan:ES2022}.
...
\caption{The posterior density curves for the SV model parameters estimated by PMFVB (solid line) and SMC (dashed line). The fist  panel from the right shows the smoothed lower bound in PMFVB over a window of size 50.}\label{fig:SV}
\end{figure}
...
The SGLD algorithm is based on the discretisation of the Langevin diffusion \eqref{eq: Langegin diffusion}
\beq
...
%---------------------------------------------------------%
We introduce three ingredients to make the PMFVB approach computationally efficient in big-data and complex-model situations.

First, we propose to choose the updating block  randomly in each iteration $t$ and for each particle $i$. 
...
Second, we propose to approximate the integral $\E_{q_{\theta(\setminus\iota_i)}^{(t)}}\big[\nabla_{\theta(\iota_i)}\log\pi\big(\theta_i^{(t)}(\iota_i),\theta(\setminus\iota_i)\big)\big]$ in \eqref{eq:LMC components} by $\nabla_{\theta(\iota_i)}\log\pi\big(\theta_i^{(t)}(\iota_i),\bar\theta^{(t)}(\setminus\iota_i)\big)$, where $\bar\theta^{(t)}$ is the sample mean of the particles $\{\theta_i^{(t)},i=1,...,M\}$. This leads to a significant reduction in computational time, compared to an alternative that averages over a subset of particles as in \eqref{eq: update X}.
...
In Algorithm \ref{Alg: particle MFVB-NN}, $\oslash$ and $\odot$ denote the component-wise division and multiplication operator, respectively.
...
Neural networks are prone to overfitting and the Bayesian framework provides a principle way for dealing with this problem by using a regularization prior on the weights. We consider the following adaptive ridge-type regularization prior on the weights
...
\eeq
It can be seen that, the optimal MFVB distribution for $\tau$ is $q(\tau)=\prod_jq(\tau_j)$, where $q(\tau_j)$ has an Inverse-Gamma density with scale $\alpha_0+1/2$ and rate $\beta_0+\langle w_j^2 \rangle/2$. Here, as common in the MFVB literature, $\langle\cdot\rangle$ denotes the expectation with respect to the variational distribution $q$.  
...
\eeqn
where $\epsilon\sim\N(0,1)$, $(x_1,...,x_{20})^\top$ are generated from a multivariate normal distribution with mean zero and covariance matrix $(0.5^{|i-j|})_{i,j}$; the last ten variables are not in the regression. The training data has 100,000 observations, the validation and test dataset each has 10,000 observations.
...
We set the tuning parameters in the SGLD methods following the suggestion in the papers where these methods are proposed.
...
Table \ref{tab: simulation regression NN} summarizes the result,
which is based on 10 different runs for each algorithm.
...
We note, however, that the PMFVB is parallelisable and the running time can be greatly reduced if multiple-core computers are used. 
...
We apply the Bayesian neural network model to predict the Income, using 43 covariates many of which are dummy variables used to encode the categorical variables. The dataset was randomly divided into a training set of 14,010 observations, a validation and test dataset each of 1751 observations (training for fitting, validation for tuning, test for final performance evaluation). We use a neural network with three hidden layers, each having 50 units.
We use the same algorithm settings as in the previous simulation example.
...
\eean
where $\eta(x_i,w)$ is the output from a neural network with the input $x_i$ and weights $w=\{w_j,j=1,...,d_w\}$.
We use the same regularisation prior as in regression (\ref{eq:regpriorweights}). The model parameters are $\theta=\big(w,\tau=\{\tau_j,j=1,...,d_w\}\big)$. We consider the following factorization in the PMFVB
\[q(\theta)=q(w)q(\tau).\]
...
The census dataset conducted by the U.S. Census Bureau is available on the UCI Machine Learning Repository. 
The task is to classify whether a person’s income is over \$50K per year, based on 14 covariates including
age, work class, race, etc. Using dummy variables to represent the categorical variables, there are totally 103 input variables in our Bayesian neural network model.
...
Table \ref{tab: census data} summarizes the result
which shows that the PMFVB obtains the best predictive performance compared with the other methods.
...
In general, we can expect that the dependence between $x$ and $\wt y$ is reduced and much less than the dependence between $x$ and $y$. \cite{tan2021use} considers this reparametrisation approach in Gaussian Variational Bayes and documents significant improvement in posterior approximation accuracy. Coupling the reparametrisation approach with PMFVB could lead to an efficient technique for Bayesian inference. This research is in progress.
...
%==============================================%
\section*{Appendix B: Technical proof}
%==============================================%
...
\eeq
As $\log\pi(x,y)$ is continuous, and hence bounded, on the compact set $\X\times\Y$, by definition of weak convergence, we have that
\beq\label{eq: fact 3}
...
\eeq
which proves that $F(q)$ is l.s.c.

We now prove the convexity of $F(q)$. Consider two measures $q^{(1)}, q^{(2)}\in\Q$ and any $t\in(0,1)$.
...
By Corollary \ref{cor: unique solution}, $q^{(t)}$ must converge to the unique minima $q^*$ of $F(q)$.
\end{proof}
...
\eea
\eqref{eq:gamma 1},\eqref{eq:gamma evaluation 1} and \eqref{eq:gamma evaluation 2} imply that $\gamma_n^2=o(1)$.
