...
\begin{document}
\title{SCALES: Boost Binary Neural Network for Image Super-Resolution with Efficient Scalings}

\author{
    Renjie Wei$^{12}$, Zechun Liu$^{5}$, Yuchen Fan$^{5}$,
    Runsheng Wang$^{234}$, Ru Huang$^{234}$, and Meng Li$^{123\dag}$
\\
...
\textit{$^5$Meta Inc Menlo Park CA, USA}
...
\input{src/3-related_work}
\input{src/4-motivation}
\input{src/5-method}
\input{src/6-experiment}
