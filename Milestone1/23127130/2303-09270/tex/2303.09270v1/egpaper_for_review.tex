...
\def\iccvPaperID{6148} % *** Enter the ICCV Paper ID here
...
\begin{center}
    ...
    \captionof{figure}{Text-guided style transfer results using either a CLIP-based image-text similarity  or  SpectralCLIP. While using the standard CLIP space leads to the generation of visual  and textual artifacts, the proposed SpectralCLIP prevents the generation of both types of artifacts. Better zoom in to see details.
    }
    \label{fig:teaser}
    \vspace{-0.1cm}
\end{center}
...
\begin{abstract}
\vspace{-0.3cm}
Contrastive Language-Image Pre-Training (CLIP) has refreshed the state of the art for a broad range of vision-language cross-modal tasks. 
Particularly, it has created an intriguing research line of text-guided image style transfer, dispensing with the need for style reference images as in traditional style transfer methods. However, directly using CLIP to guide the transfer of style leads to undesirable artifacts (mainly written words and unrelated visual entities) spread over the image, partly due to the entanglement of visual and written concepts inherent in CLIP. Inspired by the use of spectral analysis in filtering linguistic information at different granular levels, we analyse the patch embeddings from the last layer of the CLIP vision encoder from the perspective of spectral analysis and find that the presence of undesirable artifacts is highly correlated to some certain frequency components. We propose SpectralCLIP, which implements a spectral filtering layer on top of the CLIP vision encoder, to alleviate the artifact issue. Experimental results show that SpectralCLIP prevents the generation of artifacts effectively in quantitative and qualitative terms, without impairing the stylisation quality. We further apply SpectralCLIP to text-conditioned image generation and show that it prevents written words in the generated images. Code is available at \href{https://github.com/zipengxuc/SpectralCLIP}{https://github.com/zipengxuc/SpectralCLIP}.

%Owing to the power of the large-scale vision-language model CLIP, the areas of image synthesis have been refreshed. Particularly, for style transfer, CLIP enables transferring more general and abstract styles without collecting the style images in advance, as the style can be easily described with natural language, then the result is optimized by minimizing the CLIP similarity between the textual description and the stylized image. However, directly using CLIP to guide style transfer has the problem of producing artifacts, spread over the generated image. In this paper, we propose SpectralCLIP, which is based on a spectral representation of the CLIP embedding sequence, where most of the common artifacts occupy specific frequencies. By masking the band including these frequencies, we can condition the generation process to adhere to the  target style properties (\eg, the color, the texture, the artistic trait, etc.) while excluding the generation of larger-scale structures corresponding to the  artifacts. SpectralCLIP is proven effective for improving style transfer quality and preventing artifacts according to the qualitative and quantitative experimental results. Our code will be publicly available.
\end{abstract}

%%%%%%%%% BODY TEXT
\section{Introduction}
\label{sec:intro}
The goal of the style transfer task is to transform the overall appearance of a given {\em content image} to adhere to a specific style while preserving its content. Starting from the pioneering paper of Gatys \etal. \cite{gatys2016image}, this task has attracted a growing interest in the scientific community because of its large application interest (\eg, in the e-commerce or the entertainment industry, etc.).
While most of the methods proposed so far extract the style information from a {\em reference image} \cite{gatys2016image, huang2017arbitrary, park2019arbitrary,Chen_2021_CVPR, Lin_2021_CVPR, Deng_2022_CVPR, deng2022stytr2, wang2022aesust, chen2021artistic, DBLP:conf/cvpr/LiuSCBZSL0N21, cheng2021style, yao2019attention, liu2021adaattn, ulyanov2017improved, wang2020diversified} or a set of reference images \cite{sanakoyeu2018styleaware, zhu2017unpaired, kotovenko2019content}, they are subject to a common limitation: they all require at least one image reflecting the desired style to be provided. This greatly limits the practical use of these methods, considering that a reference image is not always available.
The recent emergence of large-scale vision-language models, such as CLIP \cite{radford2021learning}, creates a promising line of research, which is text-guided style transfer. Kwon \etal proposed CLIPstyler \cite{kwon2022clipstyler}, which is the first work to utilise the power of CLIP for text-guided image style transfer, demonstrating a broader range of styles and higher transfer quality than previous work based on reference images. However, as pointed out in \cite{kwon2022clipstyler}, this method tends to generate images with over-specific artifacts. In this paper, we define two types of artifacts: textual and visual artifacts. 
...
Spectral analysis has been used to analyse temporal or spatial variations of a signal \cite{gonzalez2018digital}. 
Despite being a significant signal processing tool in the `pre-Deep Learning' era, in recent years, researchers have seen its comeback in the field of natural language processing (NLP), which has proven effective in capturing linguistic information at different granular levels \cite{Prism,muller-eberstein-etal-2022-spectral}. 
...
Despite its promising use in the language domain, 
%to analyse linguistic information at certain granular levels, 
the use of spectral analysis in the vision domain
%in analysing visual information at different levels across image batches 
is still under-explored. 
Intuitively, analogously to a sequence of linguistic tokens containing a hierarchy of semantic levels (\ie from word level up to the document level), the constituent patches of an image also contain information at different levels. In this paper, we analyse the last layer representations of the vision encoder of CLIP over the image patches from the perspective of spectral analysis. 
...
We conduct experiments to verify the following points: 
...
(ii) we conduct a user study of 30 participants to compare the visual quality in terms of the overall style and the artifact-free performance of generated images, and find that our generated images are preferred by 55.28\% and 74.44\% of the users in terms of overall quality and artifact-free performance, respectively (Sec. \ref{sec:forget-to-spell}); 
...
Additionally, we employ our method for text-guided image generation (Sec. \ref{image_generation}). 
Specifically, following VQGAN-CLIP \cite{crowson2022vqgan}, we replace CLIP with our SpectralCLIP to guide the generation of images. We show that compared to CLIP, SpectralCLIP effectively prevents written words on the generated images. 
The contributions of this paper are:

\begin{itemize}
    \item From the perspective of spectral analysis, we analyse the artifact patterns present in the state-of-the-art text-conditioned style transfer model CLIPstyler. 
    We propose SpectralCLIP to alleviate the artifact issue of CLIP in the context of style transfer.
    %We show that the presence of artifacts are closely related to some frequency components, and that filtering them out can prevent artifacts in the stylised images without hurting the quality of style.
    %We propose SpectralCLIP, which prevents the artifact generation in text-guided style transfer by filtering the spectral representation of the image patches. 

    \item We experimentally show that SpectralCLIP significantly reduces artifacts in the generated images in both quantitative and qualitative terms. 
    %Particularly, user studies show that stylised images generated by SpectralCLIP are significantly preferred, with 55.28\% and 74.44\% in terms of overall quality and artifact-free performance.
    %We show that SpectralCLIP can be easily plugged into existing text-conditioned style transfer methods, leading to higher quality generation results. 
...
\begin{figure*}[t]
  \centering
   ...
   \caption{Style transfer results using either the CLIP space or  SpectralCLIP to condition the generation of the CLIPstyler baseline (zoom in to see details).}
   \label{fig:exp1}
   \vspace{-0.3cm}
\end{figure*}
...
\begin{figure*}[t]
  \centering
   ...
   \caption{Comparisons among text-guided style transfer results, generated using CLIPstyler with (a) CLIP,   (b) forget-to-spell CLIP \cite{Torralba-Disentangling}, and (c) SpectralCLIP, respectively (zoom in to see details).}
   \label{fig:exp2}
  \vspace{-0.4cm}
\end{figure*}
...
\begin{figure*}[ht]
  \centering
   ...
   \caption{The effect of the patch-rejection threshold $\tau$ in the CLIPstyler patch-loss (zoom in to see details).}
   \label{fig:exp3}
  \vspace{-0.4cm}
\end{figure*}
...
\begin{figure}[t]
  \centering
   \includegraphics[width=.8\linewidth]{images/fig6n.pdf}
...
\textbf{Firstly}, we empirically analyse the artifact patterns present in a range of artistic styles, and mask out certain bands using one of the three general filters. However, the reason why a certain target style tends to produce certain scales of artifacts is still unclear. This may require a deeper understanding of how CLIP captures these artistic concepts while it was pre-trained. 
\textbf{Secondly}, this work defines three general band combinations that effectively produce cleaner stylised images. 
A more promising alternative for future work is to automatically select frequency bands that cater to a target style.
Recently, in the language domain, M{\"u}ller-Eberstein \etal promote \cite{Prism} and develop learnable filters rather than handcrafted ones as in \cite{Prism},
offering an intriguing direction to follow in our vision domain. 
To this end, for image style transfer, the gap of a widely recognised metric to measure the presence of artifacts needs to be filled, which is also our future endeavour.
...
%------------------------------------------------------------------------
\section{Conclusion}
Despite the wide success of  CLIP in different image generation tasks, directly using this semantic space for style transfer suffers from the over-specification problem, i.e., the generation of visual and textual  artifacts.
In this paper,  we propose SpectralCLIP, which, inspired by spectral filtering methods for noise removal, transforms the CLIP embedding sequence into the frequency domain, and filters those frequencies whose period corresponds to the artifact scales.  
Our experimental style transfer results show that SpectralCLIP significantly mitigates the artifact generation, thus improving the realistic degree and the quality of the generated images. Further application to text-conditioned image generation shows that it also alleviates the appearance of text strings in generated images.
...
\vspace{-0.2cm}
  \centering
  \includegraphics[width=\linewidth]{images/r3.pdf}
  \caption{Results of (a) CLIPstyler and (b) SpectralCLIP w. $c_3$.}
  \label{fig:r3}
  \vspace{-0.45cm}
\end{figure}
