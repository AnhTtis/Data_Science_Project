...
\definechangesauthor[name={Zheng}, color=magenta]{zh}
\definechangesauthor[name={Michael}, color=olive]{mi}
\definechangesauthor[name={Simon}, color=orange]{si}
\definechangesauthor[name={Kris}, color=red]{kr}
\definechangesauthor[name={Matteo}, color=blue]{mt}
...
%in this scheme the electron beam spin-polarization can be controlled by tuning the laser and plasma parameters. This 

%
%TO BE REMOVED
%\added[id=mt]{Please use only added[id=] or replaced[id=]\{a\}\{b\} and in case include the comment [comment: your comment] between square brackets because the boxes that come from the other commands do not make the text legible.}
\end{abstract}
...
%
%\begin{align} 
%\frac{d p_x}{d\phi} = & \frac{m_e^2 c^2}{p_{-}} \big\{a_0^2 \cos\phi \sin\phi + a_0a_1 \cos\phi \sin(\phi +2 k_0 x + \phi_1) \nonumber \\
% + & (1 + 2dx/d\phi) [a_0 a_1 \sin\phi \cos(\phi +2 k_0 x + \phi_1)  \nonumber \\
% + & a_1^2 \cos(\phi+2k_0 x + \phi_1) \sin(\phi + 2 k_0 x + \phi_1) ] \big\}, \label{eq:px_py_colliding_l2}
%\end{align}
%
...
%\begin{equation} \label{eq:trap_p_th2} 
%\frac{p_{th}^{-}}{m_ec} \approx \frac{\sqrt{1-\mathcal{S}}(\rho\tilde{\gamma}+\sqrt{\mathcal{S}})-\sqrt{\rho^2\tilde{\gamma}^2+2\rho\tilde{\gamma}\sqrt{\mathcal{S}}}}{1-\sqrt{1-\mathcal{S}}}.
%\end{equation}
%
