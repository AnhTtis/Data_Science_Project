...
\author[M.~Lalin]{Matilde Lalin\, \orcidlink{0000-0002-7155-5439}}
...
\address[Ray]{Chennai Mathematical Institute, H1, SIPCOT IT Park, Kelambakkam, Siruseri,
...
\begin{theorem}\label{main thm}
    Let $r\in \Z_{\geq 2}$ and set $k:=\lfloor \frac{r}{2}\rfloor$. Let $J=[a, b]$ be an interval contained in $[-r, r]$. Set $[c,d]\subseteq [-k, k]$ to denote the interval $[a/2, b/2]$ (resp. $[(a-1)/2, (b-1)/2]$) if $r$ is even (resp. $r$ is odd). Then, as $n\rightarrow \infty$,
    \[\begin{split} \frac{\#\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n, \alpha\in J\} }{\#\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n\}}  
    =& \int_{c}^d \delta^{(k)}(u) du+ O_r(n^{-1+\epsilon}),\end{split}\]
...
\par The above result motivates the study of eigenvalue distributions in more general families of $r$-regular Cayley graphs, ordered by the size of the underlying group. The authors expect that the study of such questions shall lead to many interesting developments in the future.

\subsection*{Acknowledgements} The authors would like to thank the anonymous referee for carefully checking the manuscript, and Daniel Valli\`eres for helpful discussions. This work is supported by  the Natural Sciences and Engineering Research Council of Canada, Discovery Grant 355412-2022, the Fonds de recherche du Qu\'ebec - Nature et technologies, Projet de recherche en \'equipe 300951,  the Simons Foundation, and the Centre de recherches math\'ematiques. 

\section*{Statements and Declarations}

\subsection*{Competing Interests} The authors report there are no competing interests to declare.
...
\end{cases}\]
In either case, we may then write $S\backslash\{0\}=T\cup \left(-T\right)$, where $T=\{a_1, \dots, a_k\}$ and $1\leq a_i\leq \left( \frac{n-1}{2} \right)$ for $i=1, \dots, k$. We shall assume without loss of generality that $1\leq a_1<a_2<\dots <a_k\leq \left( \frac{n-1}{2} \right)$. 

Let $S$ be a subset of $G=C_n$ such that $\#S=r$ and $S=-S$. We set $\gamma:=(G, S)$. %As $\chi$ ranges over $\widehat{G}$, the eigenvalues $\lambda_\chi(X_\gamma)$ are all distinct. 
Recall that the spectrum of $X_\gamma$ is given by $\op{Sp} (X_\gamma)=\{\lambda_{\chi}(X_\gamma)\mid \chi\in \widehat{G}\}$.
For an integer $m\in [0, n-1]$, let $\chi_m\in \widehat{C_{n}}$ be the character defined by $\chi_m(x):=\op{exp}\left(\frac{2\pi i m x}{n}\right)$. We set $\lambda_m(\gamma):=\lambda_{\chi_m}(X_\gamma)$ and we find that 
\begin{equation}\label{lambda_m equation}\lambda_m(\gamma)=\begin{cases}  \sum_{j=1}^k 2\op{cos}\left(\frac{2\pi  m a_j}{n}\right) &\text{ if }r\text{ is even};\\  1+\sum_{j=1}^k 2\op{cos}\left(\frac{2\pi  m a_j}{n}\right) &\text{ if } r \text{ is odd.}
...
\[\lambda_m(\vec{a})=\lambda_m(\gamma_{\vec{a}})=\begin{cases}
 2\tau_m(\vec{a}) &\text{ if }r\text{ is even};\\  1+ 2\tau_m(\vec{a}) &\text{ if } r \text{ is odd.}
\end{cases}\]
We find that $\tau_m(\vec{a})\in [-k, k]$, and that $\lambda_m(\vec{a})\in [-r, r]$. 

Let $\mathcal{S}(n,k)$ be the set of pairs $\left(\gamma_{\vec{a}}, \tau_m(\vec{a})\right)$, where $\vec{a}\in A_k(n)$, and $m\in [0, n-1]$. 
Let $\mathcal{S}(n,k,m)$ be the subset of $\mathcal{S}(n, k)$ where $m$ is fixed.  Given and interval $[e,f]\subset \mathbb{R}$, we set $[e,f]_{\Z}:=\Z\cap [e,f]$. We identify $\mathcal{S}(n,k)$ with $A_k(n)\times \left[0, n-1\right]_{\Z}$. Moreover, we identify $\mathcal{S}(n,k,m)$ with $A_k(n)$, upon identifying $\vec{a}\in A_k(n)$ with $(\vec{a},m)\in A_k(n)\times \left[0, n-1\right]_{\Z}$. We refer to $\mathcal{S}(n,k,m)$ as the \emph{$m$-slice} in $\mathcal{S}(n,k)$. Let $J=[a,b]\subseteq [-r, r]$ be a closed interval and $I=[c,d]\subseteq [-k, k]$ defined by
\[[c,d]=\begin{cases}
...
&\mathcal{S}(n,k)=\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n\}, \\
&\mathcal{S}_I(n,k)=\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n, \alpha\in J\}. \\
...
\begin{equation}\label{probI}\op{Prob}_I\left(n, k\right)=\frac{\#\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n,\text{ and }\alpha\in J\}}{\#\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X),h(X)=n\}}.\end{equation}
...
Let $\sigma\in S_k$, $m\in [1, n-1]_{\Z}$, and $\vec{a}\in \mathcal{S}'(n, k, m)$. Let $I$ be an interval contained in $[-k,k]$. Then, the point $P(\vec{a}, \sigma, m, n)$ defined above in \eqref{def of P} lies in $\Omega_{n_1}'$. Furthermore, the following are equivalent
\begin{enumerate}
...
\begin{proof}
  ...
  \end{split}\]
where we recall that for fixed $k$, $\op{Vol}(B_k(I))$ is bounded uniformly in $I$, which follows from Lemma \ref{lem:vol-computation}. Moreover,  by Lemma \ref{lem:vol-computation}, 
...
\begin{proof}[Proof of Theorem \ref{main thm}]
   ...
   \[\op{Prob}_I(n, k)= \frac{\#\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n, \alpha\in J\} }{\#\{(X, \alpha)\mid X\in \cF_r, \alpha\in \op{Sp}(X), h(X)=n\}}.\]
...
\newblock {\em Int. Math. Res. Not. IMRN}, (15):2914--2955, 2009.
...
\bibitem{Broughan:2001}
Kevin~A. Broughan.
\newblock The gcd-sum function.
\newblock {\em J. Integer Seq.}, 4(2):Article 01.2.2, 19, 2001.
...
\bibitem{bucur2016distribution}
Alina Bucur, Chantal David, Brooke Feigon, Nathan Kaplan, Matilde Lal\'{\i}n,
  Ekin Ozman, and Melanie~Matchett Wood.
\newblock The distribution of {$\Bbb{F}_q$}-points on cyclic {$\ell$}-covers of
  genus {$g$}.
\newblock {\em Int. Math. Res. Not. IMRN}, (14):4297--4340, 2016.

\bibitem{bucur2011biased}
Alina Bucur, Chantal David, Brooke Feigon, and Matilde Lal\'{\i}n.
\newblock Biased statistics for traces of cyclic {$p$}-fold covers over finite
  fields.
\newblock In {\em W{IN}---women in numbers}, volume~60 of {\em Fields Inst.
  Commun.}, pages 121--143. Amer. Math. Soc., Providence, RI, 2011.

\bibitem{bucur2016statistics}
Alina Bucur, Chantal David, Brooke Feigon, and Matilde Lal\'{\i}n.
\newblock Statistics for ordinary {A}rtin-{S}chreier covers and other
  {$p$}-rank strata.
\newblock {\em Trans. Amer. Math. Soc.}, 368(4):2371--2413, 2016.

\bibitem{cheong2015distribution}
GilYoung Cheong, Melanie~Matchett Wood, and Azeem Zaman.
\newblock The distribution of points on superelliptic curves over finite
  fields.
\newblock {\em Proc. Amer. Math. Soc.}, 143(4):1365--1375, 2015.
...
\newblock {\em J. Algebraic Combin.}, 42(3):701--723, 2015.

\bibitem{DLRV}
C\'{e}dric Dion, Antonio Lei, Anwesh Ray, and Daniel Valli\`eres.
\newblock On the distribution of {I}wasawa invariants associated to
  multigraphs.
\newblock {\em Nagoya Math. J.}, 253:48--90, 2024.

\bibitem{entin2012distribution}
Alexei Entin.
\newblock On the distribution of zeroes of {A}rtin-{S}chreier {L}-functions.
\newblock {\em Geom. Funct. Anal.}, 22(5):1322--1360, 2012.

\bibitem{Ihara:1966}
Yasutaka Ihara.
\newblock On discrete subgroups of the two by two projective linear group over
  {$\mathfrak{p}$}-adic fields.
\newblock {\em J. Math. Soc. Japan}, 18:219--235, 1966.

\bibitem{JMRR}
Dmitry Jakobson, Stephen~D. Miller, Igor Rivin, and Ze\'{e}v Rudnick.
\newblock Eigenvalue spacings for regular graphs.
\newblock In {\em Emerging applications of number theory ({M}inneapolis, {MN},
  1996)}, volume 109 of {\em IMA Vol. Math. Appl.}, pages 317--327. Springer,
  New York, 1999.

\bibitem{kurlberg2009fluctuations}
P\"{a}r Kurlberg and Ze\'{e}v Rudnick.
\newblock The fluctuations in the number of points on a hyperelliptic curve
  over a finite field.
\newblock {\em J. Number Theory}, 129(3):580--587, 2009.

\bibitem{Newland}
Derek~B. Newland.
\newblock {\em Kernels in the {S}elberg trace formula on the k-regular tree and
  zeros of the {I}hara zeta function}.
\newblock ProQuest LLC, Ann Arbor, MI, 2005.
\newblock Thesis (Ph.D.)--University of California, San Diego.

\bibitem{Pillai:1933}
S.~S. Pillai.
\newblock On an arithmetic function.
\newblock {\em J. Annamalai Univ.}, 2:243--248, 1933.

\bibitem{ray2022statistics}
Anwesh Ray.
\newblock Statistics for {$p$}-ranks of {A}rtin-{S}chreier covers.
\newblock {\em Acta Arith.}, 205(3):211--226, 2022.

\bibitem{rudnick2008traces}
Ze\'{e}v Rudnick.
\newblock Traces of high powers of the {F}robenius class in the hyperelliptic
  ensemble.
\newblock {\em Acta Arith.}, 143(1):81--99, 2010.

\bibitem{sankar}
Soumya Sankar.
\newblock Proportion of ordinarity in some families of curves over finite
  fields.
\newblock {\em arXiv preprint arXiv:1904.12173}, 2019.

\bibitem{Sunada:1986}
Toshikazu Sunada.
\newblock {$L$}-functions in geometry and some applications.
\newblock In {\em Curvature and topology of {R}iemannian manifolds ({K}atata,
  1985)}, volume 1201 of {\em Lecture Notes in Math.}, pages 266--284.
  Springer, Berlin, 1986.

\bibitem{Sunada:2013}
Toshikazu Sunada.
\newblock {\em Topological crystallography}, volume~6 of {\em Surveys and
  Tutorials in the Applied Mathematical Sciences}.
\newblock Springer, Tokyo, 2013.
\newblock With a view towards discrete geometric analysis.

\bibitem{Terras}
Audrey Terras.
\newblock {\em Zeta functions of graphs}, volume 128 of {\em Cambridge Studies
  in Advanced Mathematics}.
\newblock Cambridge University Press, Cambridge, 2011.
\newblock A stroll through the garden.

\bibitem{thorne2014distribution}
Frank Thorne and Maosheng Xiong.
\newblock Distribution of zeta zeroes for cyclic trigonal curves over a finite
  field.
\newblock {\em preprint}, 2014.

\bibitem{Valette}
Alain Valette.
\newblock Can one hear the shape of a group?
\newblock {\em Rend. Sem. Mat. Fis. Milano}, 64:31--44 (1996), 1994.

\bibitem{Wood}
Melanie~Matchett Wood.
\newblock The distribution of sandpile groups of random graphs.
\newblock {\em J. Amer. Math. Soc.}, 30(4):915--958, 2017.

\bibitem{xiong2010fluctuations}
Maosheng Xiong.
\newblock The fluctuations in the number of points on a family of curves over a
  finite field.
\newblock {\em J. Th\'{e}or. Nombres Bordeaux}, 22(3):755--769, 2010.
