...
\author{Shanghua Gao$^{1,2}$
\thanks{This work was done while Shanghua Gao was a research intern at Sea AI Lab.} \quad
Pan Zhou$^{1}$ \thanks{Pan Zhou and Ming-Ming Cheng are joint corresponding authors.} \quad
Ming-Ming Cheng$^{2 \ \dagger} $ \quad
Shuicheng Yan$^{1}$ \\
...
dataset, and has about 3$\times$ faster learning speed 
...
and indeed independently learn each semantic.  %
The reason behind this phenomenon is that DPMs maximize the log probability of real data by minimizing the per-pixel prediction loss, 
...
In the forward noising process, Gaussian noise $\epsilon \sim \mathcal{N}(0, \mathbf{I})$ is gradually added  
to the real sample $x_{0}$ via a discrete SDE of formulation  $q(x_t, x_0) = \sqrt{\bar{\alpha}_t} x_0 + \epsilon \sqrt{1-\bar{\alpha}_t}$,
where $\bar{\alpha}_t$ denotes the noise magnitude. If the time step $t$ is large, $x_t$ would be a Gaussian noise. Similarly, the reverse denoising process is a discrete SDE that gradually maps a Gaussian
noise into a sample. At each  time step, given $x_t$, it predicts the next reverse step
$p_{\theta}(x_{t-1}|x_t) = \mathcal{N}(x_{t-1};\mu_{\theta}(x_t, t), \Sigma_{\theta}(x_t, t))$ via a network.
The network is trained by
optimizing the variational lower-bound $L_{\text{vlb}}$  of 
$p_{\theta}(x_0)$ ~\cite{sohl2015deep}, where 
  $L_{\text{vlb}} = -\log p_{\theta}(x_0 | x_1) + \sum_{t} D_{KL}( q(x_{t-1}|x_t,x_0) || p_{\theta}(x_{t-1}|x_t))$.
%
Following~\cite{nichol2021improved,peebles2022scalable},
we obtain $\Sigma_{\theta}(x_t, t)$ by optimizing $L_{\text{vlb}}$,
and we reparameterize $\mu_{\theta}(x_t, t)$ as a noise prediction network $\epsilon_{\theta}(x_t, t)$
and train it with a simple mean-squared error loss, \ie $L_{\text{simple}}=||\epsilon - \epsilon_{\theta}(x_t, t)||^2$,
where $\epsilon$ is the ground truth Gaussian noise.
%
During inference, one can sample  a Gaussian noise
and then gradually reverses to a sample $x_0$.
%
 
Same as~\cite{nichol2021improved,peebles2022scalable},
we train the diffusion model conditioned with class label $c$,
\ie $p_{\theta}(x_{t-1}|x_t, c)$.
%
...
LDM employs a pre-trained VAE encoder $\textbf{E}$ to encode an image 
$v \in \Real^{3 \times H \times W}$
to a latent embedding $z = \textbf{E}(v) \in \Real^{c\times h \times w}$. 
...
During training, we first add Gaussian noise to the latent embedding $z$ of an image. 
Then following~\cite{peebles2022scalable}, we divide the noisy embedding $z$ into 
a sequence of $p\times p$-sized tokens, 
and concatenate them to a matrix $u \in \Real^{d\times N}$, 
where $d$ is the channel number and $N$ is the number of tokens.  
%
Next, we randomly mask tokens with a  ratio $\rho$
and concatenate the remaining tokens as  $\hat{u} \in \Real^{d\times \hat{N}}$,
where $\hat{N} = \rho N$.  
Accordingly, we can create a binary mask  $M \in \Real^{N}$ in which one (zero) denotes the masked (unmasked) tokens.   
Finally, we feed the  unmasked tokens $\hat{u} $ into our diffusion model for processing.  
We only use unmasked tokens $\hat{u}$, 
because 1)
...
2) It saves the training cost compared to processing all $N$ tokens.  
...
Specifically, the encoder and decoder tailor the standard DiT block via adding two kinds of token position information, and respectively contain $N_1$ and $N_2$ tailored blocks. 
...
\begin{equation*}
	\mbox{Attention}(Q, K, V) = \mbox{Softmax}\left(\frac{QK^{\top}}{\sqrt{d_k}} + B_{r}\right) V,
\end{equation*}
where $Q$, $K$, and $V$  respectively denote the query, key and value in self-attention module, $d_k$ is the  dimension of the key, 
and $B_{r} \in  \Real^{N \times N}$ is the relative positional bias
that is selected by the relative positional difference $\Delta $ between the $i$-th position  and other positions, \ie $B_{r}(i, \Delta)$. The learnable mapping 
$B_{r}$ is updated during training. 
...
the encoder only processes the  unmasked tokens $\hat{u}$.
While in the inference phase,  
the encoder handles all tokens $u$ due to the lack of masks. 
...
In the training phase, the encoder processes the unmasked tokens to obtain its output token embedding $\hat{q} \in \Real^{d\times \hat{N}}$.  
Then as shown in Fig.~\ref{fig:overall}, the side-interpolater first fills the masked positions,
indicated by the mask $M$ defined in Sec.~\ref{sec:diff_train}, by a shared learnable mask token, 
and also adds a learnable positional embedding  to obtain embedding $q \in \Real^{d\times N}$. 
Next, we use a basic block of the encoder to process $q$ to predict an interpolated embedding $\hat{k}$. 
The tokens in $\hat{k}$ denote the predicted tokens. 
Finally,  we use a masked shortcut connection to combine prediction $\hat{k}$ and $q$ as  $ k = M \cdot q + (1-M) \cdot \hat{k} $.  
In summary, for masked tokens, we use the prediction by side-interpolater;  for unmasked tokens, we still adopt the corresponding tokens in $q$. 
This can 
...
\subsection{Training}
During training, we feed both full latent embedding $u$ and the masked latent embedding $\hat{u}$ to the diffusion model,
...
is small.  This is also demonstrated by~\figref{fig:converge_comp} which shows that  MDT still achieves about  3$\times$ faster learning progress than previous SoTA DiT  in terms of total training hours. 
...
\begin{table}
    \centering
    \setlength{\tabcolsep}{0.19mm} % column spacing
    \small
    \begin{tabular}{lcccccc}
        ...
        \textbf{MDT} & 6500k$\times$256  & \textbf{6.23}  & 5.23 & \textbf{143.02} & 0.71 & 0.65 \\
        \midrule
        ...
        \textbf{MDT-G} & 3500k$\times$256  & 2.02  & \textbf{4.46} & 263.77 & 0.82 & 0.60 \\ 
        \textbf{MDT-G} & 6500k$\times$256  & \textbf{1.79}  & 4.57 & \textbf{283.01} & 0.81 & \textbf{0.61} \\         
        \bottomrule
    \end{tabular}
    \vspace{2pt}
    \caption{Comparison with existing methods on class-conditional image generation 
    with the ImageNet 256$\times$256 dataset. 
...
we also use a patch size $p$=2 by default,
...
The VAE encoder has a downsampling ratio of $1/8$, and a  feature channel dimension of 4, 
\eg~an image of size 256$\times$256$\times$3 is encoded into a latent embedding of size 32$\times$32$\times$4.
...
\begin{table}
\centering
\setlength{\tabcolsep}{2.2mm} % column spacing
\small
\begin{tabular}{lcccccc}
\toprule
...
MDT-S/2 & 256$\times$256 & 3500 & \textbf{41.37}  \\ 
\midrule
DiT-B/2 & 256$\times$256 & 400 & 43.47   \\ \hdashline
MDT-B/2 & 256$\times$256 & 400 & 34.33   \\
MDT-B/2 & 256$\times$256 & 3500 & \textbf{20.45}  \\
\midrule
...
MDT-XL/2 & 256$\times$256 & \textbf{1300} & \textbf{9.60}    \\
MDT-XL/2 & 256$\times$256 & 3500 & \textbf{6.65}    \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
\centering
\small
\setlength{\tabcolsep}{1.8mm} % column spacing
...
\begin{table}[!t]
    ...
    \begin{tabular}{ccccccc}
        \toprule
        Decoder pos.	 &  FID$\downarrow$ & sFID$\downarrow$ & IS$\uparrow$ & Precision$\uparrow$ & Recall$\uparrow$ \\	\midrule
...
We follow the network configurations described in DiT~\cite{peebles2022scalable} to set the total block number (\ie~$N_1+N_2$), 
...
\begin{table}[h]
  \centering
  \small
  \setlength{\tabcolsep}{1.6mm} % column spacing
  \begin{tabular}{lcccccc}
      \toprule
...
\begin{equation*}
  w_{t} = \frac{1 - \cos{\pi \left( \frac{t}{t_{\text{max}}} \right)^{s}}}{2} w,
\end{equation*}
where $t$ is the time step during sampling, 
$t_{\text{max}}$ is the maximum sampling step,
 $w$ is the maximum guidance scale,
and $s$ is a factor that controls the increasing speed of the guidance scale.
...
In this work, $s$ is set to 4, and the corresponding $w$ is set to 3.8
...
\begin{figure}[t]
  ...
  \begin{overpic}[width=0.97\linewidth]{figures/powercos.pdf}
    \put(-5,43){\large \rotatebox{0}{$\frac{w_t}{w}$}}
    \put(50,-3){\large $\frac{t}{t_{\text{max}}} $}
  \end{overpic}\vspace{10pt}
