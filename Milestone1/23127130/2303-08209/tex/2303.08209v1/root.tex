...
\textbf{Intrinsic} parameters $\bds{\theta}_i$ determine the structure of the behavior tree, the number of control nodes, the type of motion generator, etc. 
...
We evaluated the efficacy of our approach in simulation and also a transfer of the simulation results to a real \textit{KUKA iiwa} manipulator for two tasks: an obstacle avoidance task and a pushing task, each having its own challenges. For simulation, we utilized the \textit{DART} simulation toolkit \cite{lee18joss} and in both simulation and reality, the robot arm was controlled using a Cartesian impedance controller~\cite{mayr2022c++}.
...
\end{enumerate}
The rewards and feasibility measures for the tasks are defined separately in their respective sections.
...
In this case, we could optimize over both policy and task parameters to find a matching set of learned parameters.
