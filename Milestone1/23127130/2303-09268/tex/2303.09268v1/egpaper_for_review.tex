...
\begin{center}
    ...
    \includegraphics[width=.95\linewidth]{imgs/fig1.pdf}
    \captionof{figure}{With a large-scale pretrained vector-quantized tokenizer and CLIP, $\mathtt{Styler}$DALLE can transfer various types of styles  (indicated on the top), from abstract art styles to specific artist styles and more.
    }
    \label{fig:1}
    \vspace{-0.1cm}
\end{center}
...
% , where we use CLIP-based language supervision to ensure stylization and content preservation simultaneously.
...
\noindent\textbf{Reference Image-Based Style Transfer.}
...
\noindent\textbf{Language-Guided Style Transfer.}
...
\noindent\textbf{Large-scale  Text-to-Image Generation Models.}
Recently,  text-to-image models trained with large or very large scale datasets \cite{DALL-E, ramesh2022hierarchical, saharia2022photorealistic, yu2022scaling} have attracted tremendous attention because of their excellent performance in generating high-quality images starting from a textual query.
...
Specifically, we focus on transformer-based methods, which are potentially less time-consuming as compared to diffusion model-based methods.
In addition, we propose to use a non-autoregressive transformer that can generate tokens in parallel.


\section{Background}
\label{sec.Background}
\noindent\textbf{Vector-Quantzied Image Tokenizer.} 
...
\begin{figure*}[t]
    \begin{center}
    \includegraphics[width=.95\linewidth]{imgs/fig2.pdf}
    \end{center}
    % \vspace{-0.2cm}
...
\noindent\textbf{Non-Autoregressive Transformer (NAT).}
...
Hence, we use an RL approach and the 
 REINFORCE algorithm \cite{williams1992rl}, which updates the parameters  of $f_{\theta}(\cdot)$ using
 $r$ as the 
 reward. This leads to the gradient estimate:
\begin{equation}
    \label{eq.rl_update}
...
where $\theta|_d$ indicates the parameters of the decoder only.
...
between the source and the target sequence  is an important component in this translation process. 
...
To train the model, we use the MS-COCO~\cite{lin2014microsoft} train-set, which contains 83k images of common objects in daily scenes. In the self-supervised pretraining stage, we only use the images while in the style-specific fine-tuning stage we use both images and captions. We give more implementation details in Appendix~\ref{apd:details}.

\begin{figure*}[ht]
    \begin{center}
    \includegraphics[width=.97\linewidth]{imgs/fig3.pdf}
    \end{center}
    \vspace{-0.2cm}
    \caption{Qualitative results of $\mathtt{Styler}$DALLE-1 on various styles: (a) Monet Sunrise, (b) Picasso cubism, (c) Van Gogh blue color, (d) Van Gogh purple color, (e) warm and relaxing, (f) ukiyo-e print, (g) fauvism, (h) pixel art illustration.}
    \label{fig:qla}
\end{figure*}

\begin{figure*}[ht]
    \begin{center}
    \includegraphics[width=.97\linewidth]{imgs/fig3ru.pdf}
    \end{center}
    \vspace{-0.2cm}
    \caption{Qualitative results of $\mathtt{Styler}$DALLE-Ru on various styles: (a) Monet, (b) Monet Paris, (c) Monet Venice, (d) Van Gogh, (e) Van Gogh Irises, (f) Van Gogh Almond, (g) Van Gogh Starry Night, (h) Van Gogh Sunflowers.}
    \label{fig:qlar}
\end{figure*}
...
The ablation study, inference time comparison, and additional experimental results are shown in Appendix ~\ref{apd:ablation}, ~\ref{apd:infert}, and ~\ref{apd:results}, respectively.



\begin{figure*}[ht]
    \begin{center}
    \includegraphics[width=\linewidth]{imgs/fig4x.pdf}
    \end{center}
    % \vspace{-0.2cm}
    \caption{Comparisons with language-guided methods. The styles are (a) fauvism, (b) Monet, (c) Monet Impression Sunrise, (d) oil painting, (e) watercolor painting, (f) Van Gogh, (g) Van Gogh blue color.}
    \label{fig:crl}
\end{figure*}
...
\bf Ours &
  ...
  \bf 31.64 \\ \bottomrule
\end{tabular}
\caption{Quantitative comparisons with CLIPstyler~\cite{kwon2022clipstyler} on different types of styles.}
\label{tab:qat}
\vspace{-0.3cm}
\end{table*}
...
We evaluate the methods using the MS-COCO val-set, which contains 41k images.
In Tab.~\ref{tab:qat}, we present the results of $\mathtt{Styler}$DALLE-1 and CLIPstyler-Fast.
As shown, although the way we use CLIP is to compute rewards in Reinforcement Learning, instead of directly using CLIP-scores to optimize the network as CLIPstyler, we achieve comparable and even better quantitative results, indicating the effectiveness of our method.
...
\noindent\textbf{Comparisons with Reference Image-Based Methods.}
...
In particular, we utilize all the caption annotations to enhance the model robustness, as usually human annotates different captions of a single image.
...
We only fine-tune the decoder of the NAT model, keep the encoder frozen. We use Adam optimizer with a learning rate of 1e-6.
For CLIP model, we use the CLIP ViT-B/32 model\footnote{CLIP: \url{https://github.com/openai/CLIP}}.
...
For both training stages, the model is trained on single RTX-A6000 GPU for 24 hours.
...
%     \includegraphics[width=\linewidth]{imgs/usd1.pdf}
%     \caption{}
%     \label{fig:usd1}
% \end{subfigure}

% \begin{subfigure}[]{.7\linewidth}
%     \includegraphics[width=\linewidth]{imgs/usd2.pdf}
%     \caption{}
%     \label{fig:usd2}
%   \end{subfigure}
%   \caption{Hyper-parameters study.}
%   \label{fig:usd}
%   % \vspace{-0.3cm}
% \end{figure*}
