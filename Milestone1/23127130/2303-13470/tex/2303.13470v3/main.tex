...
{\small\noindent\textbf{Abstract:} Let $G$ be a group definable in an NIP theory. We prove that, if $G$ admits a global f-generic type, then $G$ is definably amenable, answering a question of Chernikov and Simon. As an application, we show that every dp-minimal group is definably amenable, answering a question of Kaplan, Levi, and Simon.  \newline %\textcolor{blue}{We prove moreover that such a type is never concentrated on a cut of an indiscernible sequence.} \newline

\noindent\textbf{Acknowledgements:} I would like to thank Artem Chernikov, Itay Kaplan, Krzysztof Krupiński, Anand Pillay, and Pierre Simon for much encouragement and feedback. I would like to thank Itay Kaplan in particular for pointing out a shorter proof of Lemma 3.1, and for pointing out that the argument in Lemma 4.2 needs only inp-minimality, not dp-minimality. \newline
...
\end{fact} In this paper we will need only the countable case, but we note that Fact 2.5 is true even without the countability hypothesis; one way to see this is via Theorem 3.12 in \citep{chernikov_simon}. However, some care is needed, due to a gap found by Krupiński in the proof of the uncountable case of Proposition 5.6 in \citep{hrushovski_pillay}. For extensive discussion of this issue, and a solution found by Krupiński and Pillay, we refer the reader to Proposition 3.16 in \citep{pillay_groups}.

%Fact 2.5 is true even without the countability hypothesis, but a different argument is needed; in this paper we will need only the countable case, but see the footnote below.\footnote{One approach is via Theorem 3.12 in \citep{chernikov_simon}. However, some care is required in the proof there, due to a gap noticed by Krupiński in the proof of the uncountable case of Proposition 5.6 of \citep{hrushovski_pillay}. For extensive discussion of this, and a solution found by Krupiński and Pillay, we refer the reader to Proposition 3.16 in \citep{pillay_groups}.}

% However, some care is needed around the issue of the behavior of strongly f-generic types in reducts. For extensive di

% as written there, the theorem employs  some care is needed, as noticed by Krupiński; in particular it is not clear whether a strongly f-generic type remains strongly f-generic in a reduct of the language.  Proposition 5.6

% \citep{hrushovski_pillay} that \textcolor{red}{what to write...}

% as in Proposition 3.16 of \citep{pillay_groups}. We refer the reader there also for discussion on why some care is needed in this issue; 

% care is needed to apply as noticed by Krzysztof Krupiński.

% as in Proposition 3.16 of \citep{pillay_groups}. However, we will need only the countable case in this paper.
...
\end{proof} By (the argument of) Corollary 3.5 in \citep{chernikov_simon}, one has the following consequence:

\begin{fact}
    $G$ admits a global f-generic type if and only if the non-f-generic definable subsets of $G$ form an ideal.
\end{fact}
...
\end{lemma}
\begin{proof} By Lemma 3.2, let $q(x)\in S_G(\mathfrak{C})$ be f-generic and $M$-invariant. As f-genericity is translation-invariant, it suffices to show that $E:=aD\wedge bD$ is f-generic. Let $N$ be any small model containing $(M,a,b)$ and let $g\models q|_N$; then by Fact 2.6 it suffices to show that $g^{-1}E=g^{-1}aD\wedge g^{-1}bD$ does not fork over $N$. Since $D$ is f-generic, $g^{-1}aD$ does not fork over $M$, and so is contained in some global $M$-invariant type $p(x)$. Moreover, $\tp(g/M,a,b)$ extends to the global $M$-invariant type $q$; since $a\equiv_M b$ this implies $a\equiv_{M,g}b$ and hence $g^{-1}a\equiv_M g^{-1}b$. But $p(x)\vdash x\in g^{-1}aD$, so (since $p$ is $M$-invariant) this implies $p(x)\vdash x\in g^{-1}bD$. So $p$ is concentrated on $g^{-1}E$, which hence does not fork over $M$, and in particular does not fork over $N$, as needed.
...
Recall that a theory is `inp-minimal' if there do not exist formulas $\phi(x,y)$ and $\psi(x,z)$, where $x$ is a singleton variable of the home sort, and indiscernible sequences $(b_i)_{i\in\omega}$ and $(c_i)_{i\in\omega}$, such that $\phi(x,b_i)\wedge\psi(x,c_j)$ is consistent for each $i,j\in\omega$ but $\{\phi(x,b_i):i\in\omega\}$ and $\{\psi(x,c_i):i\in\omega\}$ are each inconsistent. A theory is `dp-minimal' if it is both inp-minimal and NIP; see for example \citep{dolich_goodrick_lippel}. In Problem 3.13 of \citep{kaplan_levi_simon}, Kaplan, Levi, and Simon asked whether every dp-minimal group is definably amenable. We point out here that a positive answer follows quickly from Corollary 3.8. First we need Lemma 4.1, which is a slightly more general version of Fact 2.6 and is proved in the same way.

\begin{lemma}
    Suppose $D,E\subseteq G$ are $M$-definable and not f-generic, and that $D\vee E$ is f-generic. Then there are $d\in D$ and $e\in E$ such that $e^{-1}D$ and $d^{-1}E$ each fork over $M$.
\end{lemma}
\begin{proof}
    By symmetry, it suffices to find $e\in E$ such that $e^{-1}D$ forks over $M$. Since $D$ is not f-generic, there is some $M$-indiscernible sequence $(g_i)_{i\in\omega}$ such that $\bigwedge_{i\in\omega}g_iD=\varnothing$; let $k$ be such that the translates $g_iD$ are $k$-inconsistent. Since $D\vee E$ is f-generic and defined over $M$, we have $\bigwedge_{i\in\omega}g_i(D\vee E)\neq\varnothing$, which forces $\bigwedge_{i\in\omega}g_iE\neq\varnothing$; let $a\in\bigwedge_{i\in\omega}g_iE$. Now $e_i:=g_i^{-1}a\in E$ for every $i\in\omega$. On the other hand, the translates $e_i^{-1}D$ are $k$-inconsistent, since for any $s\subset\omega$ of size $k$ we have $\bigwedge_{i\in s}e_i^{-1}D=a^{-1}\bigwedge_{i\in s}g_iD=\varnothing$. By Ramsey and compactness, we may assume that $(e_i)_{i\in\omega}$ is $M$-indiscernible, and then $e_0^{-1}D$ divides over $M$, as needed.
\end{proof}
\begin{lemma}
    If $G$ is dp-minimal, then the non-f-generic definable subsets of $G$ form an ideal.
\end{lemma}
\begin{proof}
    Suppose otherwise. Then there are some $\mathfrak{C}$-definable $D,E\subseteq G$ that are not f-generic but such that $D\vee E$ is f-generic. Let $M$ be a small model over which $D,E$ are both defined; by Lemma 4.1, there are some $d\in D$ and $e\in E$ such that $e^{-1}D$ and $d^{-1}E$ each fork over $M$. Let $(d_i)_{i\in\omega}$ and $(e_i)_{i\in\omega}$ be $M$-indiscernible with $d_0=d$ and $e_0=e$ and such that each of the families of formulas $\{x\in e_i^{-1}D:i\in\omega\}$ and $\{x\in d_i^{-1}E:i\in\omega\}$ is inconsistent; then also $\{x\in E^{-1}d_i:i\in\omega\}$ is inconsistent. On the other hand, for any $i,j\in\omega$, we have $e_i\in E$ and $d_j\in D$, so that $e_i^{-1}D\wedge E^{-1}d_j$ contains $e_i^{-1}d_j$ and is hence consistent. This contradicts inp-minimality.
\end{proof}
Now by Corollary 3.8 and Lemma 4.2 we obtain the desired result:
\begin{corollary}
    If $G$ is dp-minimal then it is definably amenable.
\end{corollary}
...
\bibitem{dolich_goodrick_lippel}Alfred Dolich, John Goodrick, David Lippel. Dp-minimality: basic facts and examples. \textit{Notre Dame Journal of Formal Logic}, 2011.
...
\bibitem{kaplan_levi_simon}Itay Kaplan, Elad Levi, Pierre Simon. Some remarks on dp-minimal groups, in: Groups, Modules, and Model Theory, Surveys and Recent Developments. \textit{Springer}, 2017.
