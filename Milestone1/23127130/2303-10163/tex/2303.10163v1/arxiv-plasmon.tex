...
\altaffiliation{E-mail: ipsita.mandal@ifj.edu.pl}
\affiliation{Institute of Nuclear Physics, Polish Academy of Sciences, 31-342 Krak\'{o}w, Poland}


\begin{abstract}
We investigate the parameter regimes favourable for the emergence of plasmons in isotropic, anisotropic, and band-mass asymmetric Luttinger semimetals (LSMs). An LSM harbours a quadratic band-crossing point (QBCP) in its bandstructure, where the upper and lower branches of dispersion are doubly degenrate.
...
A nonzero value of $T$ or $\mu$ is thus a necessary condition for a plasmon to exist, as otherwise the zero density of states at the QBCP can never lead to the appearance of this collective mode. In addition to $T$ and $\mu$, we consider the effects of all possible parameters like cubic anisotropy, band-mass asymmetry, and a material-dependent parameter $X$, to numerically compute the quasiparticle damping rate $\tau$ resulting from screened Coulomb interactions. A well-defined sharp peak in the profile of the inelastic scattering rate $\tau^{-1}$
signals the appearance of a plasmon. From our results, we conclude that $X$ turns out to be a crucial tuning parameter, as higher values of $X$ assist in the emergence of plasmons. On the other hand, the features are broadly insensitive to cubic anisotropy and band-mass asymmetry.
\end{abstract}
...
gray tin ($\alpha$-Sn) \cite{paul,gray-tin}, and HgTe~\cite{Tsidilkovski1997Book}. Such systems are also known as Luttinger semimetals (LSMs), since the Luttinger Hamiltonian of inverted band-gap semiconductors describe the low-energy physics~\cite{Luttinger1956PR,abrikosov1996,Shuichi2004PRB}.
...
The possibility of the emergence of plasmons at finite temperature \cite{Mandal2019AP} or chemical potential has also been explored \cite{Tchoumakov-Krempa2019PRB,polini} in isotropic LSMs.
One may ask whether the fate of these plasmons are robust in the presence of anisotropies, which is a more natural possibility. To this end, in the paper, we compute the behaviour of plasmons for both isotropic and anisotropic LSMs, under the influence of both finite temperature ($T$) and doping (away from the QBCP).

We follow the strategy of treating the Coulomb interactions on the 3d QBCP within a random-phase-approximation (RPA), as is done in Ref.~\cite{Mandal2019AP}. In the first step, the real and imaginary parts of the bare polarization function (i.e. without any interaction line in the loop) are explicitly derived, and the results tell us if a plasmon mode can exist when we add Coulomb interactions to the system [cf. Eq.~(\ref{Eq_diele})].
For the sake of completeness, all external variables, including the frequencies and momenta are considered at the same footing. In addition to keeping $T$ and chemical potential ($\mu$) finite, a suitable material-dependent parameter $X$ is introduced, which can influence the emergence of plasmons \footnote{Plasmons cannot exist at $T=\mu=0$ because if the vanishing of density of states at the QBCP.}. Finally, the results for polarization bubble in various regimes are used to calculate the inelastic scattering rate of the LSM quasiparticles due to Coulomb interactions. We investigate various parameter regimes to figure out when it is possible for plasmons to exist, and this is captured by sharp peaks in the inelastic scattering rates.
...
We also compute the spectral function and the wavefunction renormalization (quasiparticle residue). All our computations encompass the isotropic, anisotropic, and band-mass asymmetric scenarios. Finally, we end with a summary and discussion in Sec.~\ref{Sec_Summary}.
...
Here, the set of five matrices $\lbrace \Gamma_a \rbrace $ are the rank-four irreducible representations of the Euclidean Clifford algebra, satisfying anticommutator relation $ \lbrace \Gamma_a, \Gamma_{a'}  \rbrace = 2\,\delta_{a a'} $. In 3d, the basis for a generic $4\times 4$ Hermitian matrix is constituted by the identity matrix (denoted by $\Gamma_0$), and the five $ \lbrace \Gamma_a \rbrace$ matrices with $a \in[1,5]$.
From the latter, we get ten more distinct matrices defined by $\Gamma_{ab} =\frac{1}{2\,i} [\Gamma_a, \Gamma_b]$.
The three components of original spin operator $\boldsymbol{\mathcal J}$ can be expressed in terms of the $\Gamma_{ab}$-matrices via linear relations \cite{Moon2013PRL}. The functions denoted by $d_a(\mathbf{p})$ are the
...
the Luttinger parameters~\cite{Herbut2016PRB}, affecting the various symmetries of a LSM.
...
Analytically continuing to real frequencies, and after some tedious algebra (see Appendix~\ref{Subsec_A_Pi}),
we eventually get
\begin{align}
\label{eqpo1}
...
Eq.~\eqref{Eq_iso-KF}. Here we have used the variables $\lbrace \tilde q,\Theta,\Phi \rbrace $ of the spherical polar coordinates, related to the Cartesian coordinates as
\begin{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\caption{
Behaviour of $\mathrm{Re}\, \mathcal{E}_{\mathrm{eff}}$ for anisotropic LSM with $\tilde T=0.1$ and $\tilde \mu=0.1$, and $\tilde \omega$ restricted to positive values: Subfigure (a) captures the dependence on the angular variables $\Theta$ and $\Phi$, for $\tilde \omega=0.025$, $\tilde q=0.15$, and $X =300$.
...
\end{align}
The imaginary part ${\mathrm {Im} } \,\mathcal{E}_{\mathrm{eff}}$ quantifies the decay rate of the plasmon mode.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\caption{Behaviour of the bare polarization function for an anisotropic LSM with $\tilde T=0.5 $, $\tilde \mu=0.1$, and $\tilde \omega$ restricted to positive values:  Subfigure (a) shows $\tilde m\,\Lambda_0^2 \,\mathrm{Re} \, \Pi^R$ as a function of $\Theta$ and $\Phi$, for $\alpha=0.9$, $\eta=0.1$, $\tilde \omega =  0.37$, and $\tilde q =0.7 $. Subfigure (b) shows $\tilde m\,\Lambda_0^2 \,\mathrm{Im} \, \Pi^R$ as a function of $\tilde \omega $ and $\tilde q$, for $\alpha=0.5$, $\eta=1$, $\Phi= 3\,\pi/5$, and $\Theta= 1\,\pi/20$.
\label{Fig_aniso-Pi}}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The real part of the dielectric function $\mathcal E$ follow the trends seen for $\Pi^R$
(cf. Fig.~\ref{Fig_aniso_Re_E}), as far as the dependence on
$\tilde T$, $\tilde \mu$, $\Theta $, and $\phi$ are concerned,
due to the relations in Eq.~\eqref{Fig_aniso_Re_E}.
More specifically, we note that the behaviour of $\mathrm{Re} \mathcal E$ is quite insensitive to these variables.
As a result, the key variable which can potentially affect the characteristics of $\mathrm{Re} \,\mathcal E$ drastically is the material-dependent parameter $X$. The plots in Fig.~\ref{Fig_aniso_Re_E}(b)--(d) show that while for small values of $X$, $  \mathrm{Re}\, \mathcal{E}_{\mathrm{eff}}(\omega, \mathbf q)$ remain positive (i.e., do not possess any zeros),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%555
\caption{Behaviour of $\mathrm{Re}\, \mathcal{E}_{\mathrm{eff}}$ for n anisotropic LSM
with $\tilde T=0. 5$, $\tilde \mu=0.1$, and $\tilde \omega$ restricted to positive values: Subfigure (a) captures the dependence on the angular variables $\Theta$ and $\Phi$, for $\alpha=0.1$, $\eta=1$, $\tilde \omega= 0.033$, $\tilde q=0.05$, and $X = 100$.
Subfigures (b), (c), and (d) show the dependence on $\tilde \omega$ and $\tilde q$, with $\Phi= 3\,\pi/ 5 $ and $\Theta= 11 \,\pi /20$, for $X= 1$, $X= 50$, and $X= 100$, respectively.
...
%
%
%\red{By comparing the related quantities in the isotropic scenario presented in Fig~\ref{Fig_iso_Pi}(c) and Fig.~\ref{Fig_iso_Re_E}(c),
%we figure out that both real parts of polarization function and dielectric function shown in Fig.~\ref{Fig_iso_Pi-E-lower}
%are more likey to obtain negative values, and consequently, zeros as well.
%This, in accordance with our criterion, suggests  that the lower band is more preferable to induce the plasmon mode
%than upper band in the isotropic case. Furthermore, we have verified that the basic results for an
%anisotropic LSM are analogous to their upper-band counterparts, and henceforth the numerical figures are not show hereby.}
%
...
The one-loop order correction to the self-energy, caused by the Coulomb interaction, is given by
\begin{align}
...
\end{align}
in the the Matsubara space, where $V(i  \, \Omega_m,\mathbf{q}) $ is the Matsubara space counterpart of the effective retarded interaction shown in Eq.~\eqref{eqveff}.
Analytically continuing to real frequencies, we obtain
\begin{align}
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig6-v2-fig2d_minus}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig6-v2-fig3b_minus}}\\
%%%%%%%%%%%
\caption{\label{Fig_iso_Pi-E-lower}
Behaviour of (a) $\tilde m\,\Lambda_0^2 \, \mathrm{Re} \, \Pi^R $
and (b) $\mathrm{Re}\, \mathcal{E}_{\mathrm{eff}}$ for an isotropic LSM with $\tilde T=0.1$, $\tilde \mu=0.1$, and $\tilde \omega$ restricted to negative values. The values of the other parameters are the same as used in Figs.~\ref{Fig_iso_Pi}(c) and~\ref{Fig_iso_Re_E}(c), respectively, and we have shown only two cases with the aim of comparing the corresponding behaviour shown in those plots (which represent the $\tilde \omega>0$ case). We have not shown the plots for $\tilde \omega<0$ that are the counterparts of the remaining subfigures of Figs.~\ref{Fig_iso_Pi} and~\ref{Fig_iso_Re_E}, because the basic tendency is similar.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5



Since the self-energy is a $4\times 4$ matrix, it is convenient to parametrize as
\begin{align}
...
%\begin{figure}
%\centering
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6a-e-p-o-p}}\hspace{1 cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6b-e-p-o-p}}\\
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6a-e-p-o-m}}\hspace{1cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6b-e-p-o-m}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
%\caption{Contourplot of $1/\tau$ as a function of $\tilde \varepsilon $ (where $\tilde \varepsilon$ has been restricted to positive values) and $\tilde q$ for an isotropic LSM, with the parameters $\tilde T=0. 1$, $\tilde \mu=0.1$, $\Phi= \pi/ 5 $, and
%$\Theta= \pi / 10$. Subfigures (a) and (b) capture the contributions from the upper bands,
%while subfigures (c) and (d) capture the contributions from the lower bands.
%We have used $X= 1$ and $X= 50$ for subfigures (a) and (c), and (b) and (d), respectively.
%\label{Fig_iso_tau-one-band1}
%}
%\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%\begin{figure}
%\centering
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6a-e-m-o-p}}\hspace{1cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6b-e-m-o-p}} 
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6a-e-m-o-m}}\hspace{1cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig7-6b-e-m-o-m}} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
%\caption{Contourplot of $1/\tau$ as a function of $\tilde \varepsilon $ (where $\tilde \varepsilon$ has been restricted to negative values) and $\tilde q$ for an isotropic LSM, with the parameters $\tilde T=0. 1$, $\tilde \mu=0.1$, $\Phi= \pi/ 5 $, and
%$\Theta= \pi / 10$. Subfigures (a) and (b) capture the contributions from the upper bands,
%while subfigures (c) and (d) capture the contributions from the lower bands. We have used $X= 1$ and $X= 50$ for subfigures (a) and (c), and (b) and (d), respectively.
%\label{Fig_iso_tau-one-band2}
%}
%\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%\begin{figure}
%\centering
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7a-e-p-o-p}}\hspace{1 cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7b-e-p-o-p}}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7a-e-p-o-m}}\hspace{1 cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7b-e-p-o-m}}
%\caption{
%Contourplot of $1/\tau$ as a function of $\tilde \varepsilon $ (where $\tilde \varepsilon$ has been restricted to positive values) and $\tilde q$ for an anisotropic LSM,
%with the parameters $\tilde T=0. 5$, $\tilde \mu=0.1$, and $X=100$. Subfigures (a) and (b) capture the contributions from the upper bands for $\alpha=0.1$, $\eta= 0.1$, $\Phi= \pi/ 5 $, $\Theta= \pi / 10$,
%while subfigures (c) and (d) capture the contributions from the lower bands.
%for $\alpha=0.5$, $\eta =1$, $\Phi= 6\pi/ 5 $ and $\Theta= \pi / 10$.
%%%%%%%
%\label{Fig_aniso_tau-one-band1}
%}
%\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\begin{figure}
%\centering
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7a-e-m-o-p}}\hspace{1 cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7b-e-m-o-p}}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7a-e-m-o-m}}\hspace{1 cm}
%\subfigure[]{\includegraphics[width=0.45\textwidth]{fig8-7b-e-m-o-m}}
%\caption{
%Contourplot of $1/\tau$ as a function of $\tilde \varepsilon $ (where $\tilde \varepsilon$ has been restricted to negative values) and $\tilde q$ for an anisotropic LSM,
%with the parameters $\tilde T=0. 5$, $\tilde \mu=0.1$, and $X=100$. Subfigures (a) and (b) capture the contributions from the upper bands for $\alpha=0.1$, $\eta= 0.1$, $\Phi= \pi/ 5 $, $\Theta= \pi / 10$,
%while subfigures (c) and (d) capture the contributions from the lower bands.
%for $\alpha=0.5$, $\eta =1$, $\Phi= 6\pi/ 5 $ and $\Theta= \pi / 10$.
%%%%%%%
%\label{Fig_aniso_tau-one-band2}
%}
%\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig9-6d}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig9-6a}}\\
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig9-6b}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig9-6c}} \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
\caption{%%%%%%%%
...
variables $\Theta$ and $\Phi$, for $\tilde \omega= \tilde q = 0.05$ and $X = 100 $.
Subfigures (b), (c), and (d) show the dependence on $\tilde \omega$ and $\tilde q$, with
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig10-7d}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig10-7a}}\\
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig10-7b}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig10-7c}}
\caption{Behaviour of  $1/\tau $ for a generic LSM
with $\tilde T=0.5$, $\tilde \mu=0.1$, $X=100$, and $\tilde \varepsilon \geq 0$: Subfigure (a) captures the dependenceon the angular variables $\Theta$ and $\Phi$, for $\tilde \omega= \tilde q = 0.05$, $\alpha=0.5$, and $\eta=0.1$. Subfigures (b), (c), and (d) show the dependence on $\tilde \omega$ and $\tilde q$, for $ \lbrace \alpha, \eta, \Phi , \Theta  \rbrace = 
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig11-v2-fig6d_minus}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig11-v2-fig6a_minus}}\\
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig11-v2-fig6b_minus}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig11-v2-fig6c_minus}} \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
...
for $\tilde \omega=-0.05$, $\tilde q = 0.05$, and $X = 100 $.
Subfigures (b), (c), and (d) show the dependence on $\tilde \omega$ and $\tilde q$, with
$\Phi= \pi/ 5 $ and $\Theta= \pi / 10$, for $X= 1$, $X= 50$, and $X= 100$, respectively.
...
\label{Eq_tau}
\end{align}
we compute the scattering rate. Due to band-mass anisotropy, in general, 
...
$\varepsilon>0$ case than that of $\varepsilon<0$ case.
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig12-A-iso-ud-100-P-T-I1-J1_p}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig12-A_iso-ud-X-1-K1-L1_p}}\\
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig12-A_iso-ud-X-50-K1-L1_p}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig12-A-iso-ud-X-100-K1-L1_p-1}}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig13-A-ani-ud-P-T-I1-J1_p}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig13-A_ani-ud-01-01-K1-L1_p}}\\
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig13-A_ani-ud-05-1-K1-L1_p}}\quad
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig13-A_ani-ud-09-100-K6-L6_p}}\\
\caption{Behaviour of the one-loop corrected spectral function $A$ for a generic LSM
with $\tilde T=0.5$, $\tilde \mu=0.1$, $X=100$, and $\varepsilon>0$: Subfigure (a) captures the dependence
...
In this section, we investigate the the behaviour of quasiparticle by computing the quasiparticle residue and spectral function upto one-loop order~\cite{Mahan2000Book}, in the presence of the Coulomb interactions.
...
show some representative plots for the isotropic and anisotropic cases, respectively, for $\varepsilon \geq0$. The parameter values for these plots are the same as those in Figs.~\ref{Fig_iso_tune-X} and~\ref{Fig_aniso-tau}.
...
saturated at the locations where peaks of $1/\tau$ are observed. This implies that quasipartices are not even not well-defined in these regimes where plasmon modes are induced. In addition, we notice that the distances of positions between the peaks of spectral function and scattering rates become more pronounced as we increase the value of $X$. For the anisotropic LSM, the basic tendencies are analogous, as seen from comparing Figs.~\ref{Fig_aniso-tau} and~\ref{Fig_aniso_A_u-p_T05-mu-01-plus}.
The results for the $\varepsilon<0$ are qualitatively similar, and hence we have not included the plots here.



The quasiparticle residue is captured by
\begin{align}
Z_f(\mathbf{q})
&=\frac{1}{1-\lim \limits_{\omega\rightarrow0}\frac{\partial}{\partial\omega}\mathrm { Re} \,\Sigma^R(\omega,\mathbf{q})}.
\end{align}
%%%%%%%%%
Fig.~\ref{Fig_iso_Z_f_u-p_T01-mu-01-q-05} is a representative plot for $Z_f$ for an isotropic LSM for $\varepsilon \geq 0$, using the same parameter values in Fig.~\ref{Fig_iso_tune-X}. Comparing Fig.~\ref{Fig_iso_Z_f_u-p_T01-mu-01-q-05} with Fig.~\ref{Fig_iso_tune-X}, we observe that as we approach the frequency where a plasmon exists, $Z_f$ clearly dips when $X>50$. This decrease in $Z_f$ also signals a reduction in the weight of the quasiparticle, corroborating the fact that the quasipartices are not well-defined while the plasmon mode emerges [cf. Fig.~\ref{Fig_iso_Z_f_u-p_T01-mu-01-q-05}].
The results for the $\varepsilon<0$ case are again qualitatively similar, and hence not explicitly shown here. 
...
To summarize, we investigated the parameter regimes for the emergence of plasmons for isotropic, anisotropic, and band-mass asymmetric Luttinger semimetals. A nonzero value of $T$ or $\mu$ is a necessity to get a plasmon, as otherwise the zero density of states at the QBCP can never lead to the appearance of this collective mode.
...
The conditions for emergence of plasmons in isotropic LSMs were studied earlier in  Refs.~\cite{Mandal2019AP,Tchoumakov-Krempa2019PRB,polini}.
...
\begin{figure}
\centering
\subfigure[]{\includegraphics[width=0.45\textwidth]{fig14_a_Z-A-u-d-I1-J1-q-05}}\quad
\subfigure[]{\includegraphics[width=0.416\textwidth]{fig14_b_Z-A-u-d-K1-L1-q-05}}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
