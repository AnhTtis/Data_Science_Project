...
\end{equation}
Complete derivations of Eq.~\ref{E_9} are available in Supplementary S1.2. Then, to achieve better classification performance, the cross entropy term ${\mathcal L}_m^{CE}$ is also introduced into Eq.~\ref{E_9} as below:
\begin{equation}
\label{E_12}
...
\end{array}
\end{equation}
\subsection*{1.2. Decomposition details of Eq.~\ref{E_9} }\label{APP_1}
...
\label{APP_E2}
\begin{array}{l}
{\mathcal L}_F^{NIG}{\rm{ =  log}}\Sigma_F  + \log \frac{{\Gamma \left( {\frac{v_F}{2}} \right)}}{{\Gamma \left( {\frac{{v_F + 1}}{2}} \right)}} + \log \sqrt {v_F\pi }  - \log {\left( {1 + \frac{{{{\left( {{y^i} - u_F} \right)}^2}}}{{v_F{\Sigma_F}}}} \right)^{ - \frac{1}{2}\left( {v_F + 1} \right)}}\\
