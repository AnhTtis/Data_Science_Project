...
\end{align}
where $N_{\rm D}$ is the number of dimmers and $N_{\rm D}=N/2$ for lattice with periodic boundary conditions.
...
$\sigma^x\rightarrow -\sigma^x$ and  $\sigma^y\rightarrow -\sigma^y$;  for $\forall \sigma \in \mathcal{A}$. The same result can be also obtained by setting the visible biases to $a_i = i\pi/2$ for $i\in \mathcal{A}$ and $a_i = 0$ for $i\in \mathcal{B}$ as this exactly reconstructs the Marshall sign factor (up to an overall constant factor). 
...
W_{ij} = \begin{cases}
\im\frac{\pi}{2} & \text{spin } i \in \text{dimmer }j\\
0 & \text{otherwise}
\end{cases}\,.
\end{equation}
