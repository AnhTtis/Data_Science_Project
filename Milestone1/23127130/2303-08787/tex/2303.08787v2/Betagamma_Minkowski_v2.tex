...
\end{equation}
where $\psi=(\psi_1+i\psi_2)/\sqrt{2}$. In the article \cite{deVega:1981ka} there was conducted perturbative analysis of the theory \eqref{Minkowskian_CsG_Lagrangian}. One-loop $S$-matrix for this Lagrangian \eqref{Minkowskian_CsG_Lagrangian} was calculated, however, the theory was shown to be not one-loop renormalizable. Upon supplementing the theory with the proper counterterm, it becomes one-loop renormalizable and its one-loop $S$-matrix starts to satisfy the Yang-Baxter equation, which is equivalent to the condition of reflectionless scattering in this case
%\com{Maybe, explanation of this is needed}
. Then, in \cite{Dorey:1994mg} there was studied the semiclassical spectrum and $S$-matrix of the theory \eqref{Minkowskian_CsG_Lagrangian}. There was put forward a hypothesis for the minimal exact $S$-matrix \cite{Zamolodchikov:1978xm} for specific values of coupling constant of such a theory consistent with the semiclassical spectrum of the theory.
...
The next step is to calculate the $S$-matrix of the continuation of the theory \eqref{Euclidean_betagamma_Lagrangian} to the Minkowski signature. This is done by applying the Lehmann-Symanczik-Zimmermann (LSZ) reduction formula \cite{Lehmann:1954rq} to the four-point correlation function of the fields $\gamma$ and $\bar{\gamma}$, which yields the desired amplitudes. We were able to show this correspondence at the tree and $1$-loop level, thus confirming our hypothesis at these orders%\com{M: Add about 2-loop renormalizability}
.
...
\begin{align}\label{group_a_diagrams}
    ...
    \\
    &=g^4\frac{k^2+m^2}{(k^2-m^2)^2}\left(J_0(k^2)-J_2^{+-}(k^2)\right) + \frac{4img^4}{(k^2-m^2)^2}\left(k^+J^-_1(k) + k^+J_3^{-+-}(k)\right)
    \,. \notag 
\end{align}
...
\begin{align}\label{group_b_diagrams}
    ...
    & =\frac{ig^4m(k^2+m^2)}{16\pi^2(k^2-m^2)^2}\left(\frac{1}{\epsilon}-\log\frac{m^2}{\mu^2}-\gamma_E+\log(4\pi)\right)\left(\frac{1}{\epsilon}-2-\gamma_E+\log(4\pi) - \log\frac{m^2}{\mu^2}\right) + \mathcal{O}(\epsilon^0)\,, \notag
\end{align}
...
\begin{align}\label{group_c_diagrams}
    ...
    \\
    &=\frac{ig^4m^3(3k^2+m^2)}{16\pi^2(k^2-m^2)^3}\left(\frac{1}{\epsilon}-\log\frac{m^2}{\mu^2}-\gamma_E+\log(4\pi)\right)^2+\mathcal{O}(\epsilon^0)\,.\notag
\end{align}
...
\begin{align}\label{2_loop_gammagammabar_counterterms}
    ...
    \\
    &= -\frac{ig^4m^3\left(3k^2+ m^2\right)}{16\pi^2(k^2-m^2)^3}\left(\frac{1}{\epsilon}-\log\frac{m^2}{\mu^2}-\gamma_E+\log(4\pi)\right)^2-\notag
    \\
    &-\frac{ig^4m\left(m^2+k^2\right)}{16\pi^2(k^2-m^2)^2}\left(\frac{1}{\epsilon}-2-\gamma_E+\log(4\pi) - \log\frac{m^2}{\mu^2}\right)\left(\frac{1}{\epsilon}-\log\frac{m^2}{\mu^2}-\gamma_E+\log(4\pi)\right)+\mathcal{O}(\epsilon^0)\,.\notag
\end{align}
...
Let us calculate the loop integral $I_3^{\pm}(k)$ corresponding to the loops formed by the dashed or continues line
...
\end{equation}
Comparing both expressions, we obtain an expression for the integral $I_3^{\pm}(k)$  through the integral $I_1(k^2)$
\begin{equation}
    I_3^{\pm}(k) = -\frac{2ik^{\pm}}{m}I_1(k^2) -I_3^{\pm}(k) \rightarrow I_3^{\pm}(k) = \frac{ik^{\pm}}{m}I_1(k^2)\,.
\end{equation}
Thus, we obtain the following expression for the loop integral $I_3^{\pm}(k)$
\begin{equation}
...
& =\int \frac{d^2p}{(2\pi)^2}\left(\frac{4p^{\mp}k^{\pm}}{(p^2-m^2+i\varepsilon)((k-p)^2-m^2+i\varepsilon)}-\frac{p^2}{(p^2-m^2+i\varepsilon)((k-p)^2-m^2+i\varepsilon)}\right) = I_{2a}^{+-}(k^2) - I_{2b}^{+-}(k^2)\,, \notag
\end{align}
where take into account that $p^2 = 4p^+p^{-}$.

Consider the contributions of terms to the $I_2^{+-}(k^2)$ separately. Let's consider the second integral (see calculation of $I_3(k)$)
\begin{equation}
...
Therefore, one loop integral $I_2^{+-}(k^2)$ takes the form
% \begin{align}
%     &I_2^{+-}(p) =\frac{1}{4\pi}
...
% Splitting the integral into two terms and and calculate them separately
\begin{align}
...
% \end{align}
% These two contributions \eqref{eq:I2++contr} enter to the integral $I_2^{\pm\pm}$ with different signs. Therefore, they vanish. 
Thus, the  integral $I_2^{\pm\pm}$ in Mikowski signature has the form
\begin{equation}
...
The integral $J_0(k^2)$ is similar to the sunset integral for $\phi^4$ theory \cite{Kleinert:2001ax}
\begin{align}
...
Consider individual contributions to the integral $J_3^{\pm\pm\mp}(k)$ integral. Let's calculate the following term
\begin{align}
...
\end{equation}
The integral over the loop momentum $p_{2E}$ is equal to the following expression
% Then, differentiating with respect to the external momentum component, we obtain
\begin{align}
...
% \begin{align}
%     &J_a = \int\frac{d^2k_{2E}}{(2\pi)^2}\frac{k_{2E}^+p_E^+-(k_{2E}^+)^2}{\left(k_{2E}^2 - 2(k_2,p)_Ey +p_E^2y + M^2(x,y)\right)^2} = 
%     \partial^2_{p_E^-}\frac{1}{16y^2}\int\frac{d^2k_{2E}}{(2\pi)^2}\log\left(k_{2E}^2 +p_E^2y(1-y) + M^2(x,y)\right) - 
%     \\
%     &-\int\frac{d^2k_{2E}}{(2\pi)^2}\frac{p_E^+(k_{2E}^+ + yp_E^+)}{(k_{2E}^2 +p_E^2y(1-y) + M^2(x,y))^2} + \int\frac{d^2k_{2E}}{(2\pi)^2}\frac{(p_E^+)^2}{(k_{2E}^2 +p_E^2y(1-y) + M^2(x,y))^2} = \notag
%     \\
%     &=\frac{(p_E^+)^2y(1-y)}{4\pi}\frac{1}{p_E^2y(1-y)+M^2(x,y)}\,. \notag
% \end{align}
...
Consider the following contribution to $J_3^{++-}(k)$
\begin{align}
...
\end{align}
% \begin{align}
%     &J_b(k) = \int\frac{d^2p_{1E}}{(2\pi)^2}\frac{d^2p_{2E}}{(2\pi)^2}\frac{2p_{1E}^2p_{2E}^+}{(p_{1E}^2 + m^2)(p_{2E}^2 + m^2)((k-p_1-p_2)_E^2+m^2)} = 
%     \\
%     &=\underbrace{\int\frac{d^2p_{1E}}{(2\pi)^2}\frac{d^2p_{2E}}{(2\pi)^2}\frac{2p_{2E}^+}{(p_{2E}^2 + m^2)((k-p_1-p_2)_E^2+m^2)}}_{0} -\notag
%     \\
%     &-\underbrace{\int\frac{d^2p_{1E}}{(2\pi)^2}\frac{d^2p_{2E}}{(2\pi)^2}\frac{2m^2p_{2E}^+}{(p_{1E}^2 + m^2)(p_{2E}^2 + m^2)((k-p_1-p_2)_E^2+m^2)}}_{-J_{1E}^+(k)} = J_{1E}^+(k)\,.\notag
% \end{align}
Taking into account the symmetry of $J_3^{++-}(k)$ with respect to the replacement $p_1 \leftrightarrow p_2$ given contribution $J_b(k)$ enters into $J_3^{++-}(k)$ twice.
...
% \begin{equation}
%     M^2(k^2) = g^4 \frac{1}{im} (k^2+m^2)(J_0 - J_2^{+-})   + 4g^4(k^+J_1^- + k^+J_3^{--+}) + g^4(k^2+m^2)\delta_m^{(2)}\,.
% \end{equation}
% \begin{align}
%     &J_0(m^2) = -\frac{im}{64}\,,\quad \frac{d}{dk^2}J_0 |_{k^2=m^2} = \frac{i(\pi^2-12)}{256m\pi^2}\,,
%     \\
%     &J_1(m^2) = -\frac{k^{\pm}}{96}|_{k^2=m^2}\,,\quad \frac{d}{dk^2}(k^{\mp}J_1^{\pm}) |_{k^2=m^2} =-\frac{\pi^2+4}{512\pi^2}
%     \\
%     &J_2(m^2) = -\frac{im}{196} - \frac{im}{8\pi}\left(\frac{1}{\epsilon}-\gamma_E + \log 4\pi + \log\left(\frac{\mu}{m}\right)^2\right)^2\,,\;\;\frac{d}{dk^2}J_2|_{k^2=m^2} = \frac{i(\pi^2-4)}{256m\pi^2}\,,
%     \\
%     &J_3(m^2)= \frac{k^{\pm}}{8\pi^2}\left(1-\frac{\pi^2}{12}\right) - \frac{k^{\pm}}{48}\,, \quad \frac{d}{dk^2}(k^{\pm}J_3^{\pm\mp\mp})|_{k^2=m^2} = -\frac{\pi^2+12}{512\pi^2}\,.
% \end{align}


% System of renormalization conditions
% \begin{align}
%     &2m^2\left(-\frac{1}{64}+\frac{1}{196}+\frac{1}{8\pi}\left(\frac{1}{\epsilon}-\gamma_E + \log 4\pi + \log\left(\frac{\mu}{m}\right)^2\right)^2\right)+\frac{m^2}{8\pi^2}\left(1-\frac{\pi^2}{12}\right)-\frac{m^2}{32}+2m^2\delta_m^{(2)} = 0\,,
%     \\
%     &\left(-\frac{1}{64}+\frac{1}{196}+\frac{1}{8\pi}\left(\frac{1}{\epsilon}-\gamma_E + \log 4\pi + \log\left(\frac{\mu}{m}\right)^2\right)^2\right) -\frac{1}{16\pi^2}- \frac{8+\pi^2}{64\pi^2}+\delta_m^{(2)} = 0\,.
% \end{align}

% \begin{align}
%     &\langle \tilde\gamma(k)\tilde{\bar\gamma}(-k)\rangle_a^{(2)} = \frac{img^4}{8\pi}\frac{k^2+m^2}{(k^2-m^2)^2}\left(\frac{1}{\epsilon^2} + \frac{2}{\epsilon}\bigg(-\gamma_E+\log4\pi + \log\left(\frac{\mu}{m}\right)^2\right)  + 
%     \\
%     & + \left(-\gamma_E+\log4\pi + \log\left(\frac{\mu}{m}\right)^2\right)^2\bigg) + g^4\frac{k^2+m^2}{(k^2-m^2)^2}\frac{k^2+3m^2}{6m^2}J_0(k^2)+\notag
%     \\
%     &+\frac{4img^4}{(k^2-m^2)^2}\left(k^+J^-_1(k) + k^+J_3^{-+-}(k)\right)\,.\notag
% \end{align}

% Let $k = k_1 + k_2$, $k_4 = 0$, $m=0$. Let us calculate two-loop integrals for 4-point Green function in Euclidean space.



% \begin{align}
%     &L_3=\int \frac{d^2p}{(2\pi)^2}\frac{d^2q}{(2\pi)^2}\,\frac{8imq^-p^+(k-p)^+}{p^2q^2(k-p)^2(p-q)^2} = \int \frac{d^2p}{(2\pi)^2}\frac{d^2q}{(2\pi)^2}\,\frac{8im(p-q)^-p^+(k-p)^+}{p^2q^2(k-p)^2(p-q)^2}\rightarrow
%     \\
%     &\rightarrow 2I = \int \frac{d^2p}{(2\pi)^2}\frac{d^2q}{(2\pi)^2}\,\frac{2im(k-p)^+}{q^2(k-p)^2(p-q)^2} = \frac{2ik^+}{m^3}L_0 -\int \frac{d^2p}{(2\pi)^2}\frac{d^2q}{(2\pi)^2}\,\frac{
%     2imp^+}{q^2(k-p)^2(p-q)^2} \notag
% \end{align}

% \begin{align}
%     \int \frac{d^2p}{(2\pi)^2}\frac{2imp^+}{(k-p)^2}\int\frac{(\mu^{2})^{-\epsilon}d^{2+2\epsilon}q}{(2\pi)^{2+2\epsilon}}\,\frac{
%     1}{q^2(p-q)^2} = -\frac{i(-\pi)^{3+3\epsilon}(\mu^{2})^{-\epsilon}}{(2\pi)^{2+2\epsilon}}\int \frac{d^2p}{(2\pi)^2}\frac{2imp^+}{(k-p)^2(p^2)^{1-\epsilon}} \frac{(\Gamma(\epsilon))^2\Gamma(1-\epsilon)}{\Gamma(2\epsilon)}
% \end{align}

% \begin{align}
%      &\int \frac{d^2p}{(2\pi)^2}\frac{p^+}{(k-p)^2p^2}=\int \frac{d^2p}{(2\pi)^2}\frac{(k-p)^+}{(k-p)^2p^2} \rightarrow \int \frac{d^2p}{(2\pi)^2}\frac{p^+}{(k-p)^2p^2}=\frac{1}{2}\int\frac{d^2p}{(2\pi)^2}\frac{k^+}{(k-p)^2p^2} =
%      \\
%      &=\frac{k^+}{2}\int\frac{\mu^{-2\epsilon}d^{2+2\epsilon}p}{(2\pi)^{2+2\epsilon}}\frac{1}{(k-p)^2p^2}=-\frac{ik^+\mu^{-2\epsilon}(-\pi)^{3+3\epsilon}}{2(2\pi)^{2+2\epsilon}}\frac{(\Gamma(\epsilon))^2\Gamma(1-\epsilon)}{\Gamma(2\epsilon)}\notag
% \end{align}

% Then
% \begin{align}
%     L_3 = \frac{2ik^+}{m^3}L_0+imk^+\frac{(\pi)^{2+2\epsilon}\mu^{-4\epsilon}}{16^{1+\epsilon}}\frac{(\Gamma(\epsilon))^4(\Gamma(1-\epsilon))^2}{(\Gamma(2\epsilon))^2}
% \end{align}


% \begin{align}
%     &L_4 = \int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{16p^+q^+(k-p)^-(p-q)^-}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} =
%     \\
%     &=-4\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{k^-q^+p^2}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} + \notag
%     \\
%     &+ 4\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{k^-p^+q^2}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)}+\notag
%     \\
%     &+ 4\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{p^2p^-q^+}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} +\notag
%     \\
%     &+\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{p^2q^2}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)}\,. \notag
% \end{align}
...
%     &\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{p^2q^2}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)}=
%     \\
%     &=\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{1}{((k-p)^2+m^2)((p-q)^2+m^2)} + L_0 - \notag
%     \\
%     &- \int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{m^2}{(p^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)}-  \notag
%     \\
%     &- \int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{m^2}{(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} \notag
% \end{align}

% \begin{align}
%     &\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{1}{((k-p)^2+m^2)((p-q)^2+m^2)}=\int \frac{d^2p}{(2\pi)^2}\frac{1}{((k-p)^2+m^2)}\int \frac{d^2q}{(2\pi)^2}\frac{1}{((p-q)^2+m^2)}=
%     \\
%     &=\left(\frac{\mu^{2\epsilon}\Gamma(\epsilon)}{(4\pi)^{1-\epsilon}m^{2\epsilon}}\right)^2 = \frac{1}{16\pi^2}\left(\frac{1}{\epsilon} -\gamma_E + \log4\pi + \log\left(\frac{\mu}{m}\right)^2\right)^2\,.\notag
% \end{align}

% \begin{align}
%     &\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{p^2p^-q^+}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)}=
%     \\
%     &=\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{p^-q^+}{(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} -\notag
%     \\
%     &-\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{m^2p^-q^+}{(p^2+m^2)(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} \notag \sim
%     \\
%     &\sim \int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{p^-q^+}{(q^2+m^2)((k-p)^2+m^2)((p-q)^2+m^2)} \notag
% \end{align}

% \begin{align}
%     &\int \frac{d^2p}{(2\pi)^2}\frac{p^-}{((k-p)^2+m^2)}\int\frac{d^2q}{(2\pi)^2}\frac{q^+}{(q^2+m^2)((p-q)^2+m^2)} = 
%     \\
%     &=-\frac{1}{8}\int \frac{d^2p}{(2\pi)^2}\frac{p^2}{((k-p)^2+m^2)}\int\frac{d^2q}{(2\pi)^2}\frac{1}{(q^2+m^2)((p-q)^2+m^2)} =\notag
%     \\
%     &=-\frac{1}{32\pi}\int dx\int \frac{d^2p}{(2\pi)^2}\frac{p^2}{((k-p)^2+m^2)}\frac{1}{(p^2x(1-x)+m^2)}=\notag
%     \\
%     &=-\frac{1}{32\pi}\int dx \frac{1}{x(1-x)}\int \frac{d^2p}{(2\pi)^2}\left(\frac{1}{((k-p)^2+m^2)} - \frac{m^2}{((k-p)^2+m^2)(p^2x(1-x)+m^2)}\right) \notag
% \end{align}

% \begin{align}
%     &\int dx\,\frac{1}{x^2(1-x)^2}\int \frac{d^2p}{(2\pi)^2}\frac{m^2}{((k-p)^2+m^2)(p^2+\frac{m^2}{x(1-x)})} = 
%     \\
%     &=\int dx\,\frac{1}{x^2(1-x)^2}\int dy\int \frac{d^2p}{(2\pi)^2}\frac{m^2}{\left(p^2 + k^2y(1-y) + m^2\left(y+\frac{(1-y)}{x(1-x)}\right)\right)^2}=\notag
%     \\
%     &=\frac{1}{4\pi}\int dx\,\frac{1}{x^2(1-x)^2}\int dy\frac{m^2}{\left( k^2y(1-y) + m^2\left(y+\frac{(1-y)}{x(1-x)}\right)\right)} \notag
% \end{align}
% This integral does not converge even on shell in Minkowski space ($k^2\rightarrow-k^2$).

% \begin{align}
%     &\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{k^-p^+}{p^2(k-p)^2(p-q)^2} =  \int \frac{d^2p}{(2\pi)^2}\frac{k^-p^+}{p^2(k-p)^2}\int\frac{d^2q}{(2\pi)^2}\frac{1}{(p-q)^2}\xrightarrow[]{Euclid}
%     \\
%     &\xrightarrow[]{Euclid}\int \frac{d^2p}{(2\pi)^2}\frac{k^-p^+}{p^2(k-p)^2} \frac{i\mu^{2\epsilon}\epsilon(p^2)^{-\epsilon}}{(4\pi)^{1-\epsilon}\Gamma(1-\epsilon)} = \frac{i\mu^{2\epsilon}\epsilon}{(4\pi)^{1-\epsilon}\Gamma(1-\epsilon)}\int \frac{d^2p}{(2\pi)^2}\frac{k^-p^+}{(p^2)^{1+\epsilon}(k-p)^2} =   \notag
%     \\
%     &=\frac{i}{4\pi}\int dx\int\frac{d^2p}{(2\pi)^2}\frac{-\frac{1}{4}k^2}{(p^2+k^2x(1-x))^2}=-\frac{i}{64\pi^2}\int dx \frac{1}{x(1-x)}\notag
% \end{align}

% \begin{align}
%     &\int \frac{d^2q}{(2\pi)^2}\frac{d^2p}{(2\pi)^2}\frac{1}{(k-p)^2} =\int \frac{d^2p}{(2\pi)^2}\frac{1}{(k-p)^2}\int \frac{\mu^{2\epsilon}d^{2-2\epsilon}q}{(2\pi)^{2-2\epsilon}}\frac{1}{(p-q)^2}\xrightarrow[]{Euclid}
%     \\
%     &\xrightarrow[]{Euclid}-\int \frac{d^2p}{(2\pi)^2}\frac{1}{(k-p)^2}\frac{\mu^{2\epsilon}}{\Gamma(1-\epsilon)}
% \end{align}
