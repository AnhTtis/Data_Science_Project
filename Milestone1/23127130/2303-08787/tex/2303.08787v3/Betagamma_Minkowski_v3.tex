...
\end{equation}
where $\psi=(\psi_1+i\psi_2)/\sqrt{2}$. In the article \cite{deVega:1981ka} there was conducted perturbative analysis of the theory \eqref{Minkowskian_CsG_Lagrangian}. One-loop $S$-matrix for this Lagrangian \eqref{Minkowskian_CsG_Lagrangian} was calculated, however, the theory was shown to be not one-loop renormalizable. Upon supplementing the theory with the proper counterterm, it becomes one-loop renormalizable and its one-loop $S$-matrix starts to satisfy the Yang-Baxter equation, which is equivalent to the condition of reflectionless scattering in this case. Then, in the work \cite{Dorey:1994mg} the semiclassical spectrum and the $S$-matrix of the theory \eqref{Minkowskian_CsG_Lagrangian} was studied. There was put forward a hypothesis for the minimal exact $S$-matrix \cite{Zamolodchikov:1978xm} for specific values of coupling constant of such a theory consistent with the semiclassical spectrum of the theory.
...
The article is organized as follows. In the Section~\ref{CsG_setting} we remind the reader known facts about the complex sine-Gordon theory regarding its spectrum and scattering. In the next Section~\ref{betagamma_system_continuation} we explain the way we use to continue the theory \eqref{Euclidean_betagamma_Lagrangian} to the Minkowski signature and setting the propagators and vertices of such a theory. In the Section~\ref{Two_point_functions} we calculate the two-point functions at one and two loops and also do some analysis of necessary counterterms. Then, in the Section~\ref{Four_point_functions} we turn to the four-point function of the fields $\gamma$ and $\bar{\gamma}$, from which we extract the $2 \rightarrow 2$ amplitude to compare with the $S$-matrix of the CSG theory \cite{Dorey:1994mg}. In the Section~\ref{Conclusions} we make concluding remarks and discuss further open questions.
...
M.A. and A.K. are especially grateful to Ben Hoare for inspiring ideas and participating in the work at various stages of the project. M.A. would to also thank Alexey Litvinov, Arkady Tseytlin, Mikhail Vasiliev and Dmitri Bykov for fruitful and stimulating discussions.
...
Consider the contribution to $J_3^{++-}(k)$
\begin{align}
    &J_b(k)=-\int\frac{d^2p_{1}}{(2\pi)^2}\frac{d^2p_{2}}{(2\pi)^2}\frac{2p_{1}^2p_{2}^+}{(p_{1}^2 - m^2+i\varepsilon)(p_{2}^2 - m^2+i\varepsilon)((k-p_1-p_2+i\varepsilon)^2-m^2)} =\\
    & = -\underbrace{\int\frac{d^2p_{1}}{(2\pi)^2}\frac{d^2p_{2}}{(2\pi)^2}\frac{2p_{2}^+}{(p_{2}^2 - m^2)((k-p_1-p_2)^2-m^2)}}_{0} + J_1^+(k)\,.\notag
\end{align}
Taking into account the symmetry of $J_3^{++-}(k)$ with respect to the replacement $p_1 \leftrightarrow p_2$ the given contribution $J_b(k)$ enters into $J_3^{++-}(k)$ twice.

Thus we obtain the following expression for integral $J_3^{++-}(k)$ in Minkowski signature
\begin{align}
