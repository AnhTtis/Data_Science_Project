...
In this work, we address the optimization of the radial mask for lensless imaging and the verification of the extended-DOF lensless imaging by optical experiments with a prototype camera. Our contributions are: (1) we propose a radial-shape-preserving optimization scheme, in order to systematically identify the best parameters considering modulation transfer function~(MTF) for a radial mask; (2) we show that our optimized radial mask achieves a more balanced frequency response considering MTF when compared to naive implementations of radial masks; (3) we quantitatively and qualitatively validated the extended DOF capabilities of the optimized radial coded mask through simulations; and (4) we built a prototype lensless camera and empirically validated the extended DOF of the radial coded mask.