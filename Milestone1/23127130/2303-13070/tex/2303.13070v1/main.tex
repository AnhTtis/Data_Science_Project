...
\author[a]{Shigeki Aoki,}
\author[b,c]{Akitaka Ariga,}
\author[d]{Tomoko Ariga,}
\author[e]{Nikolaos Charitonidis,}
\author[f]{Sergey Dmitrievsky,}
\author[g,l]{Radu Dobre,}
\author[g]{Elena Firu,}
\author[f]{Yury Gornushkin,}
\author[h]{Ali Murat Guler,}
\author[k]{Daiki Hayakawa,}
\author[i]{Koichi Kodama,}
\author[j]{Masahiro Komatsu,}
\author[k]{Umut Kose,}
\author[f,l]{Madalina Mihaela Miloi,}
\author[b]{Manato Miura,}
\author[j]{Mitsuhiro Nakamura,}
\author[m]{Toshiyuki Nakano,}
\author[g]{Alina-Tania Neagu,}
\author[b,*]{Toranosuke Okumura\note[*]{Corresponding author.},}
\author[h]{Canay Oz,}
\author[j]{Hiroki Rokujo,}
\author[j]{Osamu Sato,}
\author[f]{Svetlana Vasina,}
\author[m]{Junya Yoshida,}
\author[n]{Masahiro Yoshimoto,}
\author[h]{Emin Yuksel.}
...
\affiliation[b]{Chiba University, Yayoi 1-33, Inage, Chiba, 263-8522 Chiba, Japan}
...
The new target mover provided irradiation with a proton density of \SI{0.98}{cm^{-2}} and the density fluctuation of $2.0\pm 0.3$\% in the DsTau 2021 run.
}
...
The validation of the Standard Model (SM) and exploration of Beyond Standard Model (BSM) physics are considered to be a paramount mission in the particle physics.
...
The identification of $D_s\to \tau \to X$ decays will be performed by using topological information, thanks to the high spatial and angular resolution of the emulsion-based detectors
%to the high-position and high-angular resolution of the emulsion detector 
and nanometer-precision readout. 
...
The similar movable stages were used in the past experiments~\cite{Kodama:1990zy, Aoki:1988pm}.
...
\begin{figure}[tbp]
\centering
\includegraphics[width=.6\textwidth]{image/Target_Mover_CAD.png}
\caption{\label{fig:b} The schematic view of the Target Mover. The overall size of the %overall
structure is 1370 \si{mm} height, 1800 \si{mm} width, and 400 \si{mm} depth, and the range of the motion is 350 \si{mm} in the vertical direction and 450 \si{mm} in a horizontal one.}
\end{figure}
...
Two scintillation counters fixed behind the TM measure the beam intensity.
...
A lead brick with weight of 22.7 kg was mounted on the TM instead of the emulsion module, and a 2 \si{kBq} \si{^{90}Sr} source was used to emulate the SPS beam. 
...
Therefore, we employed only one scintillation counter and omit the pre-scaller. 
...
The speed of the TM stage was optimized using the 2018 pilot run data and calculated by Equation~\ref{eq:y}, based on the assumption that the beam profile is 2-dimensional Gaussian shape with the \SI{10}{mm} standard deviations.
%This beam profile parameter was chosen based on the 2018 pilot run beam profile.
In order to emulate the stage velocity similar to one of the real experiment, the required pseudo beam density
...
\caption{\label{fig:e}The emulated beam spill by the \si{^{90}Sr} source.}
\end{figure}

%左の図はビームシェイプを2次元ガウス関数、TargetMoverがLabＲｕｎと同じ動きをしたとしたときのトラックデンシティマッピングであり、右の図は密度分布のヒストグラムである。
...
\qquad
\includegraphics[width=.4\textwidth]{image/Densit_Distribution_Lab_Run.pdf}
\caption{\label{fig:g}Left: The emulated particle density map of the TM test. Right: %\Yuri{The reproduced track density distribution with Gaussian fit} 
...
\begin{figure}[tbp]
\centering
\includegraphics[width=.4\textwidth]{image/Target_Mover_Setting_Photo.png}
...
This network delay was negligible for the RSCS performance. 
...
\caption{\label{fig:beam_intensity_SPS}The SPS beam intensity time structure in 2021.}
\end{figure}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=.5\textwidth]{image/YProfile.pdf}
\caption{\label{fig:h}Y (vertical) profile of the SPS beam in 2021.}
\end{figure}
\begin{figure}[tbp]
\centering
...
Figure~\ref{fig:i} shows the measured proton track density map with the bin size of $\SI{2}{mm} \times \SI{2}{mm}$ and distribution of the proton density in this data sample.
...
The mean value of the density $\mu_\rho$ is \SI{0.98e5}{cm^{-2}}, and the standard deviation $\sigma_\rho$ of the fitted Gaussian function is $0.21 \times 10^4$~\si{cm^{-2}}.
The fluctuation of the proton density is $\sigma_\rho/\mu_\rho = 2.1\%$, which satisfies the requirement of $< 10\%$ fluctuation.

\begin{figure}[tbp]
\centering
\includegraphics[width=.4\textwidth]{image/Density_Map_All.pdf}
\qquad
\includegraphics[width=.4\textwidth]{image/Density_Distribution_All.pdf}
\caption{\label{fig:i}Left: Track density mapping on the emulsion of the DsTau 2021 physics run. The blue spot has less tracks. Right: The density distribution histogram with Gaussian fitting.}
\end{figure}
...
During the DsTau physics run in 2021 at the CERN-SPS H2 beamline, the TM and the RSCS worked successfully and allowed the data taking rate of \SI{200}{\kilo\hertz} and the proton density of $\SI{0.98e05}{cm^{-2}}$ with $ 2.1\pm 0.3 \%$ fluctuation, which exceeds the requirement of the DsTau experiment. 
