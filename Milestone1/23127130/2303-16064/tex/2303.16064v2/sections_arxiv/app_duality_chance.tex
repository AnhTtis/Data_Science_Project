...
%{\color{blue}Matteo's comment: It is indeed very good to write the primal with only equality constraints, and decision variables belonging to a cone. However, ideally, you also want to put everything that depends on decision variables on the left of the "$=$" symbol, and parameters on the right, in order to correctly identify $\mathbf{Z}$ (using my notations); I suspect that your current dual formulation is wrong, did you try to implement an SoS program based on it to numerically verify strong duality? Indeed, I may have forgotten to say it, but I believe that strong duality also holds at each step of the hierarchy  above a certain minimal degree (using the exact same arguments as in the infinite dimensional setting)} {\color{red}be careful: letter $\tau$ is already used to denote the stopping time, so you might want to use another symbol; and I believe that the same happens with $s$ which, at the very beginning of the paper, denotes a scalar random variable} {\color{blue} Eventually, given \eqref{eq:soc_begin}, you can recast \eqref{eq:soc_end_aff} as $s_1 + \tau = 2$, which is only 2D, while its current formulation involves many moments of $\mu_\tau$.\color{red}Maybe this dual formulation is correct. To be compared to my red comments in the CDC paper.}
%\begin{subequations}
...
In order to apply the strong duality results of Appendix \ref{app:duality_general} towards the chance-peak problem (Theorem \ref{thm:cdc_strong_dual}), we need to provide an \ac{SDP}-representation of the \ac{SOC} cone.  
\begin{lem} \label{lem:soc_lmi}
An element $(\kappa,\lambda) \in Q^n$ satisfies the \ac{LMI} \cite{alizadeh2003second}
\begin{align}
    \Phi = \begin{bmatrix}
    \lambda & \kappa^T \\ \kappa & \lambda I_n
...
\end{lem}
\begin{proof}
When $\lambda=0$, the containment $(\kappa, 0) \in Q^n$ requires that $\kappa = \0_n$. The matrix in $\Phi$ \eqref{eq:soc_embedding} is therefore the $\0_{n \times n}$ matrix which is \ac{PSD}. 
Now consider the case where $\lambda > 0$. A Schur complement of $\Phi$ yields the constraint $\lambda - \kappa^T \kappa / \lambda \geq 0$. Multiplying through by the positive $\lambda$ results in $\lambda^2 - \norm{\kappa}^2_2 \geq 0, \ \lambda > 0$, which is the definition of the \ac{SOC} cone $(\kappa, \lambda) \in Q^n$. Lemma \ref{lem:soc_lmi} is therefore proven.
...
Lemma \ref{lem:soc_lmi} ensures that problem \eqref{eq:var_meas_soc_ext} is an instance of the more generic \eqref{eq:pmsdp} with $M = 2$, $X_1 = X_2 = [0,T]\times X$, $\cY = C^2([0,T]\times X) \times \R^3$ and
$$\cX_0 = \left\{\begin{bmatrix}
    \lambda & \kappa^T \\ \kappa & \lambda I_3
    \end{bmatrix} \; \middle| \; \lambda \in \R, \kappa \in \R^3\right\}.$$
...
Letting $q=([q_1, q_2, q_3], q_4) \in Q^3$ be an \ac{SOC}-constrained variable, we define the matrix $\Phi$ from \eqref{eq:soc_embedding} as
\begin{align}
    \Phi = \begin{bmatrix}
    q_4 & q_1 & q_2 & q_3 \\
    q_1 & q_4 & 0 & 0 \\
...
\begin{enumerate}
\item[R1]There exists a feasible solution for $(\mu_\tau, \mu, q)$ from \eqref{eq:var_meas_soc_ext}.
...
We start with R1. Letting $x(t \mid x_0)$ be an \ac{SDE} trajectory from \eqref{eq:sde_sol} and $t^* \in (0, T]$ be a stopping time, we define $\mu$ as the occupation measure of $x(t \mid x_0)$ and $\mu_\tau$ as its time-$t^*$ state distribution $\mu_{t^*}$. Feasible choices for entries of the \ac{SOC}-constrained $q$  are (from Lemma \ref{lem:sqrt})
\begin{align}
    q_1 &= 1-\inp{p^2}{\mu_{t^*}}, & q_2 &= \sqrt{\inp{p^2}{\mu_{t^*}} - \inp{p}{\mu_{t^*}}^2}, & q_3 &= 2\inp{p}{\mu_{t^*}}, & q_4 &= 1+\inp{p^2}{\mu_{t^*}}.
\end{align}
...
\begin{align}
    \Tr{\Phi^2} &=  2q_1^2 + 2q_2^2 + 2q_3^2 +4 q_4^2 \\&
    = 2(1-\inp{p^2}{\mu_{t^*}})^2 + 2( \sqrt{\inp{p^2}{\mu_{t^*}} - \inp{p}{\mu_{t^*}}^2})^2 + 2(2\inp{p}{\mu_{t^*}})^2 +4(1+\inp{p^2}{\mu_{t^*}})^2\\
