...
Define $\tau_X$ as a stopping time (random variable) corresponding to the time at which the process \eqref{eq:sde} starting from $X_0$ touches the boundary $\partial X$ for the first time. A process of \eqref{eq:sde} starting from an initial condition $x(0) \in X_0$ in times $t \in [0, T]$ is
\begin{equation}
\label{eq:sde_sol}
...
The generator $\Lie$ associated with the \ac{SDE} is a linear operator with $\forall v(t,x) \in C^2([0, T] \times X)$ \cite{oksendal2003stochastic}:
