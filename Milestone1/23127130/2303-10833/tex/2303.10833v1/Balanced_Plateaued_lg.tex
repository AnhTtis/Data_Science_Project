...
\title{Linear Codes From Two Weakly Regular Plateaued Functions with index $ (p-1)/2 $ \tnoteref{mytitlenote}
	}
...
\author[mymainaddress]{Tonghui Zhang}
...
Throughout the paper, we always set $p$ to be an odd prime. We will use the symbol $ \mathbb{F}_{p} $ to denote the finite field with $p$ elements. A linear code $C$ over $\mathbb{F}_{p}$ with length $ n $, dimension $ k $ and minimum distance $ d $ is said to have parameters $ [n,k,d] $, which means that $ C $ is a $k$-dimensional linear subspace of $\mathbb{F}_{p}^n$
 ...
 Let us introduce an efficient way to construct linear codes, which is proposed by Ding \emph{et al.} \cite{ding2007}. Let $ q=p^m $ for an integer $ m $, and $D=\{d_1,d_2,\dots,d_n\} $ be a subset of $ \mathbb{F}_q^*$. Define
 \begin{align*}
 ...
 \end{align}
 where the set $ D $ is a subset of $ \mathbb{F}_q^2 \backslash \{(0,0)\} $, and it is also called a defining set. Based on this method, Wu \emph{et al.} \cite{wu2020} offered a new approach to linear codes using the defining set 
 ...
 In this paper, we will investigate the constructed linear codes $ C_{D_{f,g}} $ of \eqref{eq:code} and \eqref{def:D-fg}, and determine their parameters and the weight distributions using Walsh transform.
...
Their punctured codes are also determined. As we will show later, they are projective, and some of them are optimal since they meet the Griesmer bound.
 ...
 Section \ref{sec:math}, we present an introduction to the mathematical foundations, including cyclotomic fields and weakly regular plateaued functions.  
 Section 3 gives necessary results for our computation. The main results are proposed in Section 4, where we study the weight distributions and the parameters of our constructed codes and their punctured ones. 
...
$ (1) $ The ring of integers in $ K $ is $ \mathbb{Z}[\zeta_p]$, where $ \mathbb{Z} $ is the ring of integers, and $\{ \zeta_p^i:1\leqslant i \leqslant p-1\} $ is an integer basis of $ \mathbb{Z}[\zeta_p]$. 
...
\end{lemma}
By Lemma \ref{lem:cyclo}, it is easily proved that $\sigma_z(\zeta_p^a)=\zeta_p^{za}$ and $\sigma_z(\sqrt{p^*}^m)=\eta^m(z)\sqrt{p^*}^m$ for all $a \in \mathbb{F}_p$ and $z \in  \mathbb{F}_p^*$.  
 ...
 The quadratic Gauss sum for $\mathbb{F}_q $ is defined by
 \begin{align*}
...
Applying the Parseval identity, one gets the absolute Walsh distribution of plateaued functions.
 ...
 \begin{lemma}\label{lem:car}(Lemma 1, \cite{Mes2014})
 	Let $f $ be an $s$-plateaued function. Then for $ \beta \in \mathbb{F}_{q}$, $|\widehat{\chi}_f(\beta)|^2 $ takes $p^{m-s}$ times the value $p^{m+s}$ and $p^{m}-p^{m-s}$ times the value $0$.
 \end{lemma}
...
\end{corollary} 
 ...
 \begin{table}[htbp]
 	...
 	\begin{tabular}{ll}
 		...
 		& $  p^{\gamma-1}-  \varepsilon_f \varepsilon_g \sqrt{p}^{\gamma-2} $\\
