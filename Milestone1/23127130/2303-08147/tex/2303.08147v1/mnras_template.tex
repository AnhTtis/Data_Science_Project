...
\begin{abstract}
Strong magnetic fields in the cores of stars are expected to significantly modify the behavior of gravity waves, which is likely the origin of suppressed dipole modes observed in many red giants.
...
For non-axisymmetric modes ($m\neq0$), the interaction between the discrete $g$ mode spectrum and a continuum of Alfv\'en \nzrremove{resonances}\nzradd{waves} produces nearly discontinuous features in the fluid displacements at critical latitudes associated with a singularity in the fluid equations.
...
Stellar magnetism is a highly impactful, but often neglected, property of many main sequence stars \citep{ferrario2009origin,vidotto2014stellar}, red giants \citep{garcia2014study,stello2016prevalence,fuller2015asteroseismology}, white dwarfs \citep{angel1977magnetism,wickramasinghe2000magnetism,liebert2003true}, and neutron stars \citep{thompson1993neutron,kulkarni1998star,levin2006qpos} alike.
...
Interest in the influence of magnetic fields on nonradial stellar oscillations has been reignited in the past few years by the discovery of suppressed dipole ($\ell=1$) and quadrupole ($\ell=2$) oscillation modes in a family of red giants \citep{mosser2012characterization,garcia2014study,stello2016prevalence,mosser2017dipole}.
...
\nzrremove{At the same time}\nzradd{In parallel}, \citet{gang2022topology} have made the first-ever constraints on the interior magnetic field \textit{topology}---the recent development of such new powerful observational tools further demands proportionate advances in our theoretical understanding of internal magnetogravity waves.
...
\nzradd{Throughout this work, we use the term ``magnetogravity wave'' to refer to the general phenomenon of a gravity wave propagating through a highly conductive, magnetized fluid.
...
\nzradd{and the star's structural variation scale $1/H$. However, the horizontal wavenumber $k_h\simeq\sqrt{\ell(\ell+1)}/r$, so low-$\ell$ magnetogravity modes
...
Therefore, we have adopted a WKB approximation in the radial direction only (i.e., $\partial/\partial r \approx -i k_r$) such that $k_r$ is assumed to be larger than any \nzrremove{horizontal}\nzradd{structural} gradients.

We define the Alfv\'en frequency $\omega_A = \vec{k}\cdot\vec{v}_A$, where $\vec{v}_A = \vec{B}_0/\sqrt{4 \pi \rho_0}$ is the Alfv\'en \nzrremove{speed}\nzradd{velocity}.
Then \nzrremove{a WKB approximation}\nzradd{the assumption that $k_r\gg k_h$} entails \nzradd{that} $\omega_A\propto\left(\vec{B}\cdot\vec{k}\right)=B_rk_r+B_hk_h \simeq B_r k_r$, such that the horizontal component of the magnetic field is unimportant as long as $B_r$ and $B_h$ are comparable.
...
\noindent where \nzrremove{$v_A$}\nzradd{$v_A=|v_{A,r}|$} is the \nzradd{radial component of the Alfv\'en velocity} \citep{unno1989nonradial}.
...
\nzradd{In Equation \ref{genev}, $v_A$ is a measure of the the Alfv\'en velocity at a given radius. Although the magnetic field strength clearly varies as a function of $\theta$ and $\phi$, hereafter we use $v_A$ to denote its \textit{maximum} value at a given radius.}
...
\nzradd{It is possible that $a$ could reach a maximum at some finite radius which would admit a weakly magnetized inner region. In practice,
...
For this special case\nzradd{, and adopting a radial WKB approximation}, Equations \ref{magnetoboussinesq} can be written in spherical polar coordinates as
...
\noindent \nzradd{and $\vec{B}'=-iB_0 \mu k_r\vec{\xi}$.}
...
\nzrremove{In the zero-field limit, w}\nzradd{Here, w}hile $a$ and $b$ individually approach zero, the combination $\lambda=b^2/a^2$ ($=r^2k_r^2\omega^2/N^2$) approaches $\ell(\ell+1)$, matching the zero-field result that $k_h=\sqrt{\ell(\ell+1)}/r$).
...
The eigenfunctions of $\mathcal{L}^{m,\nu}_{\mathrm{rot}}$ are called Hough functions \citep{hough1898application,hough1898v}, and \nzrremove{its}\nzradd{their} properties have been widely studied, both analytically \citep{homer1990boundary,townsend2003asymptotic,townsend2020improved} and numerically \citep{bildsten1996ocean,lee1997low,chen2009calculation,fuller2014dynamical,wang2016computation}.
...
We note that, for $|\nu|>1$, the coefficients in the Laplace tidal operator $\mathcal{L}_{\mathrm{rot}}^{m,\nu}$ (Equation \ref{lto}) switch \nzradd{relative} signs on the domain, and Sturm--Liouville theory no longer guarantees that its eigenvalues are positive-definite (see Section \ref{sl}), and indeed there are an infinite number of $\lambda^\nu_{\ell0}<0$ branches occupying the range $|\nu|>1$ which diverge to negative infinity as $|\nu|=1$ is approached \citep[e.g.,][]{lee1997low}.
In the rotation problem, these negative $\lambda^\nu_{\ell0}$ branches correspond physically to \nzradd{oscillatory convective modes (e.g., Section \ref{g}).
Notably, in the retrograde case for $|m|\neq0$, some of these branches of eigenvalues actually rise above $0$ and physically correspond to Rossby waves \citep{lee1997low}.}\nzrremove{Rossby waves.}
In the magnetogravity problem, these negative eigenvalue branches are not directly relevant in radiative regions (see Section \ref{mequalszero} for a discussion of this), although it may imply magnetically stabilized $g$ modes in convective regions (see Section \ref{g}).
...
\nzrremove{Moreover, the requirement that $\lambda\in\textbf{R}$ implies that the \textit{value} of these eigenfunctions must vanish at $x=0$ (e.g., Boyd 1981).}
...
\end{equation}
The leading-order term is the indicial equation, and can be solved to yield \nzradd{$\alpha=0$}\nzrremove{$r=0$} and \nzradd{$\alpha=2$}\nzrremove{$r=2$}, implying either that the leading-order dependence of the eigenfunctions around the singularity must either be constant or quadratic.
Enforcing equality at the next two lowest orders for \nzradd{$\alpha=0$}\nzrremove{$r=0$} (the constant case) yields
\begin{subequations}
    \begin{gather}
...
\begin{figure}
    ...
    \caption{The inverse depth parameter \nzrremove{$1/a$}\nzradd{$a^{-1}$} plotted against $b$ for zonal ($m=0$) modes (Section \ref{mequalszero}).
    The quantities \nzrremove{$1/a$}\nzradd{$a^{-1}$} and $b$ have been roughly translated to $r$ and $k_r$ using \nzradd{constant values} $\omega=2\pi\times10^2\,\mu\mathrm{Hz}$, $N=10^2\omega$, $v_A=0.1\,\mathrm{km}\,\mathrm{s}^{-1}$, and $R=10R_\odot$, reasonable parameters near the hydrogen burning shell in a first-ascent red giant.
    Ingoing gravity waves of different $\ell$ follow the tracks to the right, such that they never propagate back towards the surface of the star, and are converted to \nzradd{slow magnetic}\nzrremove{Alfv\'en-like} waves with high radial wavenumber.}
...
\end{equation}
This is approximately equal to the critical magnetic field strength derived in \cite{fuller2015asteroseismology}\nzradd{, although conceptually different}.
For $\ell=1$, \nzradd{we find numerically that} the incoming wave approaches the cutoff height from above, and approaches infinite wavenumber before reaching a turning point (as can be seen in Figure \ref{fig:fig_evals_0}).
However, for all other values of $\ell$, \nzradd{we find that} the incoming wave first refracts outwards before approaching the cutoff height from below.

\nzrremove{For stronger magnetic fields with $B_0 > B_0^{(c)}$}\nzradd{In addition, for each mode, there is some critical field $B_c$ such that, for $B_0>B_c$} (\nzradd{or $a > a_c$)}, there is no solution for a real value of $b$.
Only complex values of $b$ allow for solutions, implying \nzradd{(for real $\omega$)} complex wavenumbers $k_r$ and evanescent waves similar to those discussed in \cite{fuller2015asteroseismology} and \cite{lecoanet2016conversion}.
...
\begin{tabular}{l@{\hspace{8pt}}l@{\hspace{8pt}}l@{\hspace{8pt}}l@{\hspace{8pt}}c}
    ...
    \hline
\end{tabular}
\caption{\nzradd{For the mode branches computed in Sections \ref{mequalszero}, \ref{mnotequalszero}, and \ref{numerical2}, values of $b=b_c$ and $|a_c|^{-1}$ at the critical depth (the wave turning point), as well as values of the cutoff height $|a_\infty|^{-1}$.
...
\nzradd{In Table \ref{tablecutoffs}, we report values of the critical depth $a_c^{-1}$ and cutoff depths $a_\infty^{-1}$ for these mode branches.}

\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/vgr.pdf}
    \caption{\nzradd{The group velocities $v_{{\rm g},r}$ for $1\leq\ell\leq5$ zonal ($m=0$) magnetogravity wave branches.
...
\nzradd{The radial components of the phase and group velocities $v_{{\rm p},r}$ and $v_{{\rm g},r}$ can be specified in terms of $a$ and $b$ as}
...
\begin{subequations}
    ...
    \end{gather}
\end{subequations}
\noindent \nzradd{where we have used $\omega=\sqrt{Nv_A/r}a^{-1/2}$ and $k_r=\sqrt{N/v_Ar}ba^{-1/2}$ (from Equations \ref{b} and \ref{a}).}

\nzradd{While $v_{{\rm p}, r}$ follows the motion of the wave pattern, $v_{{\rm g},r}$ tracks the transport of wave energy.
...
In Section \ref{tar}, we pointed out that the Laplace tidal operator $\mathcal{L}^{m,\nu}_{\mathrm{rot}}$ (defined in Equation \ref{lto}) has \nzradd{branches of mostly} negative eigenvalues for $\nu>1$\nzradd{, which manifest as Rossby waves on the segments of the branches which are positive}\nzrremove{, corresponding to Rossby waves}.
However, in the magnetic problem, these branches are irrelevant when $\omega$ and $N$ are real\nzradd{, since the eigenvalues on these branches are always negative}.
...
We first consider an eigenfunction with eigenvalue $\lambda$, and form a ``Rayleigh quotient'' (\nzrremove{compare}\nzradd{cf.} Equation \ref{rayleigh}), but only over the portion of the domain bounded between $\mu\in(-1/b,+1/b)$ with $b>0$:
...
The eigenvalues for \nzrremove{$\ell,m\leq5$}\nzradd{$\ell,|m|\leq5$} are shown in Figure \ref{fig:fig_evals_1}, and example eigenfunctions are shown in the bottom panels of Figures \ref{fig:eigenfunctions_1} (for $b<1$) and \ref{fig:eigenfunctions_0} (for $b>1$), respectively.
...
Therefore, we see that $a$ approaches a constant cutoff value \nzrremove{$a_{\ell m}^{(c)}$}\nzradd{$a_\infty^{\ell m}$} in the large $b$ limit---in other words, when approaching some \nzrremove{critical} \nzradd{cutoff} value \nzrremove{$a=a_{\ell m}^{(c)}$}\nzradd{$a=a_\infty^{\ell m}$} from either above or below, $b$ \nzrremove{will diverge}\nzradd{diverges}.
Moreover, since Equation \ref{restricted2} does not depend on $m$, \nzrremove{$a_{\ell m}^{(c)}$}\nzradd{$a_\infty^{\ell m}$} only depends on the specific solution of Equation \ref{restricted2} which is approached by a given branch.
Therefore, \nzrremove{$a=a_{\ell m}^{(c)}$}\nzradd{$a=a_\infty^{\ell m}$} is a function of $\ell-|m|$, which defines the number of nodes possessed by the generalized Legendre operator.
The cutoff values roughly lie between the $m=0$ cutoff values \nzrremove{$a_{\ell0}^{(c)}$}\nzradd{$a_\infty^{\ell0}$} (defined in Equation \ref{cutoffa}), which do not follow the same pattern (see Figure \ref{fig:fig_evals_1}).
\nzradd{Table \ref{tablecutoffs} reports the eigenvalues of Equation \ref{restricted2}, which give the cutoff depths $a_\infty^{-1}$ for these $m\neq0$ mode branches (as well as the critical depths $a_c^{-1}$).}
...
For example, since the leading-order dependence of the $p'$ eigenfunction near the singularity is quadratic \nzrremove{in general }(Section \ref{frob}), the discontinuity of $\mathrm{d}^2p'/\mathrm{d}\mu^2$ across the critical latitudes implies via Equation \ref{xitheta} that the \textit{value} of $\xi_\theta$ is discontinuous.
...
\begin{figure}
    ...
    \caption{The inverse depth parameter \nzrremove{$1/a$}\nzradd{$a^{-1}$} plotted against $b$ for tesseral/sectoral ($m\neq0$) modes for the singular eigenvalue problem described in Section \ref{mnotequalszero}.
    Both \nzrremove{$1/a$}\nzradd{$a^{-1}$} and $b$ have been translated to $r$ and $k_r$ as in Figure \ref{fig:fig_evals_0}. As in Figure \ref{fig:fig_evals_0}, ingoing gravity waves follow the lines to the right, and are converted to \nzrremove{Alfv\'en}\nzradd{slow magnetic} waves that do not propagate back to the surface.}
    \label{fig:fig_evals_1}
\end{figure}

Because of the singular denominator factors in $\mathcal{L}_{\mathrm{mag}}^{m,b}$ and implied discontinuous eigenfunctions, it is important to consider that even \nzrremove{miniscule}\nzradd{infinitesimally little} viscosity/\nzradd{Ohmic} diffusivity can induce finite damping as well as global changes to the eigenfunctions.
...
However, the values of \nzradd{the indicial root $\alpha$}\nzrremove{$r$} found in Section \ref{frob} \textit{are} separated by an integer, so a Frobenius expansion in $p'$ is not particularly helpful in the search for the other solution.
...
\noindent gives an indicial equation which has a double root at \nzradd{$\alpha=0$}\nzrremove{$r=0$}, consistent with the results of \citealt{goedbloed2004principles} on a similar magnetohydrodynamic problem (see their Section 7.4).
...
The choice of \nzradd{$\alpha=0$}\nzrremove{$r=0$} gives a single everywhere-finite solution which can be called $\mathcal{Z}_\theta(\mu)=\mathcal{Z}_1(\mu)=u(\mu)$.
...
In practice, because each field line has a discrete spectrum of Alfv\'en \nzrremove{resonances}\nzradd{waves} (which are analogous to oscillations on a closed loop), a real global mode resonates with the Alfv\'en spectrum at only a finite (but large) number of locations \citep{loi2017torsional}.
...
\nzradd{Because Alfv\'en waves are incompressible, they likely cannot directly be detected using standard asteroseismic methods, which rely on changes in brightness due to surface temperature perturbations \citep{houdek2015interaction}. It may be possible to observe their signature in surface velocity fluctuations, if the waves do not damp before reaching the surface.}
...
If dissipation, in the form of fluid viscosity and Ohmic diffusion, are included, \nzradd{the linearized horizontal momentum and induction equations are modified to}
...
\noindent \nzradd{where we continue to assume the hierarchy of variables described in Section \ref{lfe} (including taking $\nabla^2\approx-k_r^2$).
...
\nzradd{Equations \ref{mb_mtm2} and \ref{mb_b2} can be combined to obtain}
\begin{equation} \label{newp}
    \nabla_hp' = \rho_0\omega^2\left(1 - b^2\mu^2 - ic\right)\vec{\xi}_h
\end{equation}

\noindent \nzradd{where $c$ is given by}
...
%     \end{split}
% \end{equation}
\nzradd{In deriving Equations \ref{newp} and \ref{cccc}, we have assumed that $k_r^2\eta/\omega\ll1$.
...
\nzradd{Overall, }the operator $\mathcal{L}^{m,b}_{\mathrm{mag}}$\nzradd{then takes the new form}\nzrremove{ gets modified to}
...
\nzrremove{where we have assumed that $k_r^2\eta/\omega$ is small, and that the radial wavenumber dominates the dissipation.
...
\begin{figure}
    ...
    \caption{The inverse depth parameter \nzrremove{$1/a$}\nzradd{$a^{-1}$} plotted against $b$ for non-axisymmetric $m\neq0$ modes, with finite dissipation and real $\omega$ (Section \ref{numerical1}).
    The color represents the complex argument of $k_r$, with the lower branches representing spatially evanescent solutions.
    Both \nzrremove{$1/a$}\nzradd{$a^{-1}$} and $b$ have been translated to $r$ and $k_r$ as in Figure \ref{fig:fig_evals_0}. Unlike previous figures, gravity waves do not propagate into the colored portions of the lines, where they become strongly evanescent. Instead, they are refracted upwards onto a\nzrremove{n} \nzradd{slow magnetic} \nzrremove{Alfv\'enic} wave branch not shown here (see Figure \ref{fig:fig_evals_3}).}
...
Section \ref{numerical1} considers modes with real $\omega$ but complex $k_r$, i.e., possibly spatially evanescent modes, and Section \ref{numerical2} considers modes with \nzradd{real radial phase velocity $v_{{\rm p},r}=\omega/k_r$}\nzrremove{ possibly complex $k_r$ and $\omega$, but subject to the constraint that their ratio $k_r/\omega$ be real} (approximating the case of propagating waves).
...
Even in the limit of \nzrremove{small $c$}$c\rightarrow0$, the imaginary part of $b$ \nzrremove{never becomes exactly zero}\nzradd{does not correspondingly vanish}, although (as we discuss below) \nzrremove{it sometimes approaches a small and constant limit}\nzradd{its limiting value is sometimes quite small}.
...
For some branches of modes\nzrremove{ (usually those with larger $\ell$ and $|m|$)}, there is a range extending from $|b|=1$ to some intermediate value of $|b|$ where $\mathrm{Im}(b)$ is small when $c\approx0$.
...
\nzradd{It appears that the size of the intermediate range of $|b|$ for which $\mathrm{Im}(b)$ is small appears to increase with $|m|$ for fixed $\ell$.
...
In all branches, for large enough \nzrremove{$|b|\simeq\mathrm{few}$}\nzradd{$|b|\gtrsim\mathrm{few}$}, the imaginary part of $b$ found by the solver becomes large, and $a^{-1}$ dips as the solver follows an evanescent branch deeper into the star.
...
\nzradd{Note that, because the values of $b$ (and therefore $k_r$) for these equator-ward and pole-ward traveling solutions are complex conjugates of each other, they exponentially decay in radius in opposite directions, and it is generally not possible to superpose them to form a wavefunction which is a horizontal standing wave at all radii.}
...
\nzradd{Physically, evanescent waves indicate the presence of either total internal reflection or (in this case) refraction.
Such waves cannot transport wave energy arbitrarily deeply, and conservation of energy therefore enforces that, in the limit of small dissipation, all of the wave energy must be converted into an outgoing propagating wave.}
\nzrremove{U}\nzradd{Thus, u}pon evanescence, the wave energy is refracted onto propagating branches that extend back out towards the surface of the star, discussed in the following section.
...
Instead, we consider the case \nzradd{where the radial phase velocity $v_{{\rm p},r}=\omega/k_r$ is real, which is equivalent to taking}\nzrremove{where} \nzradd{$b=v_A/v_{{\rm p},r}$} \nzrremove{is}\nzradd{to be} real (placing the singularity as close to the real line as possible), but allowing $a\propto\omega^{-1}$ to be complex (i.e., so that $\mathrm{arg}(k_r)=\mathrm{arg}(\omega)\neq0$).
The eigenvalues for these calculations are shown in Figure \ref{fig:fig_evals_3}.

Interestingly, in this formulation, the eigenvalues have similar qualitative behavior to discontin\nzradd{u}ous case in Figure \ref{fig:fig_evals_1}.
They reach some maximum $|a|$ at $b\sim1$, at which point the waves turn and propagate outwards onto a slow \nzrremove{Alfv\'enic}\nzradd{magnetic} branch which asymptotes to a finite cutoff height at infinite wavenumber.
This corroborates the basic picture that propagating modes with real (or nearly real) $k_r$ and $\omega$ cannot exist in a strongly magnetized star, and that gravity waves are converted to \nzradd{slow} \nzrremove{Alfv\'enic}\nzradd{magnetic} waves by strong magnetic fields.
\nzradd{Table \ref{tablecutoffs} reports the critical and cutoff depths $|a_c|^{-1}$ and $|a_\infty|^{-1}$ for these solutions.}
...
The $m=0$ modes have \nzrremove{$a_c^{-1}=2\ell-1$}\nzradd{$a_\infty^{-1}=2\ell-1$} (Figure \ref{fig:fig_evals_0}), offset by $4$ \nzradd{(in inverse depth)} for the same values of $\ell-|m|$.
%offset in $\ell-|m|$ by $2$. 
This should not be too surprising\nzrremove{,} because\nzradd{,} at $b\gtrsim1$, the mode eigenfunctions are very different in each case.
...
For larger values of $b$, \nzradd{the complex arguments of $k_r$ and $\omega$ appear to decrease to roughly constant values of $\mathrm{arg}(\omega)=\mathrm{arg}(k_r)\sim10^{-2}$.
However, due to numerical difficulty, we are unable to confirm this behavior for $b\gtrsim2.5$ or much lower values of $c$.}
\nzrremove{the imaginary components of $k_r$ and $\omega$ appear to decrease to a roughly constant value of ${\rm arg}(\omega) \! \sim \! 10^{-1}$ that is nearly independent of viscosity.
...
\begin{figure}
    ...
    \caption{The inverse depth parameter \nzrremove{$1/a$}\nzradd{$a^{-1}$} plotted against $b$ for tesseral/sectoral ($m\neq0$) modes, with finite dissipation and real \nzrremove{$k_r/\omega$}\nzradd{$v_{{\rm p},r}=\omega/k_r$} (Section \ref{numerical2}).
    The color represents the complex argument of $\omega$, which is enforced to be equal to the complex argument of $k_r$.
    Both \nzrremove{$1/a$}\nzradd{$a^{-1}$} and $b$ have been translated to $r$ and $k_r$ as in Figure \ref{fig:fig_evals_0}. As in previous figures, waves follow these tracks to the right as they are converted from gravity waves to \nzradd{slow magnetic}\nzrremove{ Alfv\'enic} waves.}
...
The spike in $\mathrm{Im}(\omega)$ \nzradd{(which occurs on the slow magnetic branch)} coincides with a transition between these two regimes.
...
It is thus unsurprising that the cutoff depths \nzrremove{$a_c^{-1}$}\nzradd{$a_\infty^{-1}$} predicted by the non-dissipative solution (Section \ref{mnotequalszero}) do not coincide with those predicted by Equation \ref{cutoff2}.
...
\nzrremove{Once again w}\nzradd{W}e suspect this is due to the finite damping rates that persist at vanishing viscosities\nzradd{/diffusivities} for waves with these sorts of internal singularities, as discussed \nzrremove{above}\nzradd{earlier}.
...
\nzradd{The problem then reduces to a more tractable one-dimensional eigenproblem by making an assumption that the equilibrium field is axisymmetric (although some analytical insight is still available if this assumption is relaxed; see Section \ref{general}).}
...
\nzradd{While such waves may exist, they have large horizontal wavenumbers and very large radial wavenumbers, so they may be difficult to observe.}
...
\noindent where we have ignored the pres\nzradd{s}ure term (note that the Alfv\'en \nzrremove{resonances}\nzradd{waves} which cause the sharp features at the critical latitudes cannot be restored by pressure).

Keeping the dominant terms \nzradd{(and still assuming $k_r\gg k_h$, $B_r\sim B_h$)},
...
This \nzrremove{heuristically}\nzradd{physically} explains why sharp fluid features near critical latitudes do not appear in our $m=0$ solutions (Section \ref{mequalszero}), or earlier two-dimensional solutions \citep{lecoanet2016conversion}.
...
Physically, the Alfv\'en and magnetogravity waves, which are decoupled in the dispersion relation of Equation \ref{cartesian}, \nzradd{may} become strongly coupled in a narrow band \nzradd{due to additional small terms left out of Equation \ref{cartesian}.}
\nzradd{Because the Alfv\'en waves are expected to have angular scales}\nzrremove{of width} $\delta\theta \! \sim \! 1/\sqrt{k_rr}$ \nzradd{due to the effect of the horizontal field, coupling between Alfv\'en and magnetogravity waves should also occur within $\sim \! \delta\theta$ of a critical latitude (due to geometric overlap)}.
%\nzradd{In real stars, strong coupling between two modes occurs when their phase velocities overlap, and so resonances between Alfv\'en and magnetogravity modes should in principle occur at only (a large) countable number of discrete points due to mode discretization due to radial boundary conditions (for magnetogravity waves) and periodic boundary conditions along field lines (for Alfv\'en waves) viz. e.g., \citet{loi2017torsional}. This discretization of resonances is not captured in our analysis where we do not impose these boundary conditions.}
\nzrremove{This}\nzradd{This coupling} may allow a small amount of gravity wave power to be converted into outgoing Alfv\'en waves.
\nzradd{These Alfv\'en waves would then propagate along a closed field line, eventually curving back inwards to the critical latitude on the opposite hemisphere of the star. Here, they could be converted back into outgoing gravity waves, potentially allowing for some wave power to escape the core.}
...
\nzradd{We investigate this possibility further in Appendix \ref{tangling}, finding that it causes the wave function to have an Airy function horizontal dependence near the critical latitude.}
...
In those \nzrremove{``quasi''-modal}\nzradd{``quasi-modal''} solutions, the oscillation frequency depends on position, and hence the solutions are not separable in space and time.
...
Without loss of generality, we can rescale $\psi$\nzradd{ and $B_0$} so that the maximum of $|\psi|$ on the sphere is $1$.
...
\nzradd{Since we have not assumed a specific magnetic field geometry here, the form of the differential eigenproblem in Equation \ref{genev} is generic.
...
\nzradd{Standard mixing-length theory assumes a slight superadiabatic temperature gradient such that $N^2<0$ in convective zones.}
...
However, the same argument can be applied to the magnetogravity problem\nzradd{, and gives meaning to the negative branches of eigenvalues $\lambda$ implied by Equation \ref{mte}}.
...
\nzradd{In the case of no buoyancy ($N^2=0$) and relaxing the Boussinesq assumption, convective regions are expected to sustain standard magnetohydrodynamic waves \citep{shu1991physics}.}
\nzradd{On top of these modes, the aforementioned negative eigenvalue branches}\nzrremove{These negative eigenvalues} hint at the existence of buoyancy-\nzrremove{driven}\nzradd{restored} oscillations in convective regions which are stabilized by magnetic forces.
...
\nzradd{Note that this feature is not unique to the magnetogravity problem, and would also be true for the rotational problem considered by \citet{lee1997low}.}
This appears to violate a fundamental assumption of our analysis that $k_r/k_h\sim N/\omega$ is large\nzradd{, or at least implies that stable convective oscillations which can accurately be described by our formalism must be of very low frequency}.
...
\begin{enumerate}
\item Propagating zonal ($m=0$) magnetogravity modes merge at a finite field with a branch of slow \nzrremove{Alfv\'enic}\nzradd{magnetic} waves whose wavenumbers diverge at a finite cutoff radius. Their horizontal eigenfunctions are Hough functions for a dipolar magnetic field. Hence, ingoing gravity waves are converted into \nzradd{slow} \nzrremove{Alfv\'enic}\nzradd{magnetic} waves at a critical magnetic field strength similar to that derived in \cite{fuller2015asteroseismology}. Above this field strength, the modes become evanescent and cannot propagate.
%$a^{(c)}_{\ell0}=1/(2\ell-1)$.
This is in agreement with the results of \citealt{lecoanet2016conversion} in a similar geometry.

\item Propagating sectoral and tesseral ($m\neq0$) modes also merge with branches of slow \nzrremove{Alfv\'enic}\nzradd{magnetic} waves whose wavenumbers diverge at a cutoff radius within the star. Like $m=0$ modes, ingoing gravity waves cannot propagate above a critical magnetic field strength, and are instead converted to outgoing \nzradd{slow} \nzrremove{Alfv\'enic}\nzradd{magnetic} waves.
...
We speculate that such an analysis might reveal that an observable amount of wave power may be able to escape a strongly magnetized stellar core through \nzradd{coupling with} Alfv\'en waves (at critical latitudes) or extremely localized magnetogravity waves (near the equator).% can bring an observable amount of wave power out of a strongly magnetized stellar core.
...
Our analysis reinforces conclusions from earlier studies that strong magnetic fields should convert gravity waves into \nzradd{slow magnetic}\nzrremove{Alfv\'enic} waves that damp within stellar interiors, causing magnetic fields to suppress the amplitudes of gravity modes in red giant stars \citep{fuller2015asteroseismology,stello2016prevalence}.
...
We thank Daniel Lecoanet, Yuri Levin, Sterl Phinney, Janosz Dewberry\nzradd{, Joel Ong}, and Saul Teukolsky for their helpful advice and comments.
...
\nzradd{Adopting a WKB approximation in the radial direction, w}\nzrremove{W}e can define
...
\noindent where \nzrremove{$v_{A,0}=B_0(r)/\sqrt{4\pi\rho_0}$}\nzradd{$v_A=B_0(r)/\sqrt{4\pi\rho_0}$}.
...
\nzrremove{where this time $v_{A,0}$ is defined with respect to $B_0(r)$ in Equation \ref{monopole}.}
...
At small buoyancy or Alfv\'en frequencies, the two solutions for \nzradd{$b_{\mathrm{F}}^2$}\nzrremove{$b^2$} in Equation \ref{fuller} approach the usual internal gravity wave and Alfv\'en wave dispersion relations, and remain finite.
Equations \nzrremove{\ref{dipolegeo}}\nzradd{\ref{monopole}}, \ref{fullerab}, and \ref{fullerdisp} are analogous to Equations \nzrremove{\ref{monopole}}\nzradd{\ref{dipolegeo}}, \ref{b} and \ref{a}, and \ref{dispersionrelation} in the main text.
...
In the $m=0$ (Section \ref{mequalszero}) and $m\neq0,|b|\leq1$ (Section \ref{mnotequalszero}) cases without dissipation, \nzradd{little} special care is \nzrremove{not }required to solve the requisite two first-order differential equations.
...
Moreover, because $\mathcal{L}_{\mathrm{mag}}^{m,b}$ (Equation \ref{mto}) is even with respect to $\mu$, its eigenfunctions can be partitioned into even and odd parity, which is given by \nzradd{$(-1)^{\ell+m}$}\nzrremove{$(-1)^{k+m}$} for \nzradd{$p'$}\nzrremove{$\mathcal{P}$} and $(-1)^{\ell+m+1}$ for \nzradd{$\xi_\theta$}\nzrremove{$\mathcal{Z}_\theta$}.
This known parity \nzradd{greatly} simplifies the problem, allowing us to solve for the eigenfunctions for only $\mu\in[0,+1)$ rather than over the full domain.
...
\nzradd{In the following, we demonstrate in this narrow equatorial band the existence of self-consistent, propagating solutions with wavenumbers enhanced in magnitude by a factor $N/\omega$.
...
This \nzrremove{behavior}\nzradd{increased radial wavenumber} appears to be qualitatively consistent with numerical simulations conducted by \citet{lecoanet2016conversion}, which seem to exhibit such waves at locations where the radial magnetic field vanishes (see their Figure 6).
\nzradd{However, their simulation also seems to show outgoing equatorially confined evanescent waves, whose driving we cannot explain.
...
In the solutions throughout \nzrremove{this}\nzradd{the main} text, it has been assumed that the magnetic tension terms which appear in the momentum equations are dominated by the radial component (in, e.g., Equations \ref{subeq}).
...
For Alfv\'en \nzrremove{resonances}\nzradd{waves}, we examine the horizontal momentum equation:
...
To characterize the effect that this shear modulus term has on the Alfv\'en \nzrremove{resonances}\nzradd{waves}, we can focus on the magnetic tension term in the horizontal momentum equation, which is the only other term capable of restoring torsional mode components \citep{loi2017torsional}:
\begin{equation} \label{schrod}
...
As noted by \citet{hoven2011magnetar}, the effect of a small shear modulus is to transform the continuous spectrum of Alfv\'en \nzrremove{resonances}\nzradd{waves} into a discrete one (as in standard bound-state spectra of the Schr\"odinger equation), whose mode spacings decrease to zero in the limit where $c_s\rightarrow0$.
If a WKB approximation is adopted for Equation \ref{schrod} in the horizontal direction, the coupling of these discrete \nzrremove{resonances}\nzradd{waves} to a mode is similar to that of the continuum \nzrremove{resonances}\nzradd{waves} in the $c_s=0$ case.
In the region where $\omega^2\gg\omega_{Ar}^2$, the discrete Alfv\'en \nzrremove{resonance}\nzradd{wave} oscillates spatially very rapidly, and its overlap with a global-scale $g$ mode averages to zero.
Similarly, in the region where $\omega^2\ll\omega_{Ar}^2$, the \nzrremove{resonance}\nzradd{wave} decays very rapidly, and therefore is very close to zero.
...
\noindent which sets the scale at which an interaction with a mode and Alfv\'en \nzrremove{resonance}\nzradd{wave} will be ``smeared'' in the angular direction, due to shear stress.
Physically, the shear modulus-induced discretization of the Alfv\'en \nzrremove{resonances}\nzradd{waves} occurs because shear adds an isotropic contribution to the wave speed (so that it is not exactly zero perpendicular to the field lines), and thus couples fluid motions across field lines.
\nzrremove{We note, however, that t}\nzradd{T}his effect is likely to be small relative to similar effects associated \nzrremove{to}\nzradd{with} the horizontal component of the mean magnetic field (Section \ref{wkb}).
