...
%INTRODUCTION
In the fields of high-speed X-ray science and synchrotron radiation, the maximum achievable repetition rate of a dynamic target of interest is an important figure of merit when pursuing efficient use of available beamtime, which is typically limited at user facilities.
However, many of the common dynamic processes of most interest are necessarily destroyed during operation, requiring complete or partial reassembly of the target after each imaging event \cite{Ram2012,Dattelbaum2020ShockwaveStructures,Sechrest2020}, which can slow repetition rate to perhaps only a handful of events per day.
Therefore it would be beneficial to develop a target which requires minimal to no maintenance between imaging events, without compromising phenomena of interest such as high instantaneous power density (e.g. comparable to ICF), high mass density gradients, high pressure and temperature gradients, and supersonic behavior/shockwaves.
In this letter we present such a target, which features a pulsed power device submerged in ambient liquid heptane that can produce a well-timed nanosecond-pulsed spark discharge.
...
Of particular interest in this subset of imaging results is the presence of a visible cylindrically expanding shock front generated by the spark discharge event, which to the best of our knowledge represents the weakest shock front ever imaged using PCI.
Since a sufficiently strong shock would be easily visible using less sensitive imaging techniques due to its relatively high mass density ratio, the ability to see relatively weak shocks (Ma $\approx$ 1) is an informative metric for testing the sensitivity of a particular imaging method; the fact that such a weak shock front is still observable in this work highlights the superior sensitivity of this implementation of PCI to very subtle dynamic phenomena, while still revealing the limits of current techniques and the path forward for the next generation of ultrafast imaging.
The high repetition rate ($>$3 events/minute), low cost ($<$US\$100k), and portability of this 3D-printed imaging target makes it quite attractive to those fields interested in events of similar timescales and power densities (e.g. ICF, dynamic compression, shock physics), but which rely on apparatuses which are either immovable or have a prohibitively slow event repetition rate.
...
The pulsed power device and high-voltage circuit used in this work to generate the submerged spark discharge event is similar to those used in our prior work \cite{Campbell2021,Akhter2021}, see the supplementary material for further details.
As shown in Figure \ref{fig:setup}, the imaging scene presented by the target consists of two electrodes, between which a submerged spark discharge occurs that is triggered to be well timed relative to X-ray imaging diagnostics from APS.
...
This work investigated the performance of LYSO and LuAG scintillators, which have decay tails of 44ns and 70ns respectively (note that LuAG also has multiple secondary slower decay modes up to $9.9 \si{\micro\second}$) \cite{Rusiecka2021AProperties,Rutherford2016,Chewpraditkul2009}, however most of the results presented in this letter were generated using a LYSO scintillator which had an observed stronger sensitivity to the plasma-induced shock front featured in this work, presumably due to its shorter decay tail leading to minimal image ghosting compared to LuAG (Figure \ref{fig:LuAGvsLYSO}a and \ref{fig:LuAGvsLYSO}b).
...
\begin{figure}[!t] %selected event
    ...
    \caption{    
    Selected frames from a single spark discharge event in heptane, for which the plasma-induced shock is visible.
    Timestamps are measured relative to the instant of spark discharge between the pulsed high-voltage anode (left side of frame) and the grounded cathode (right side of frame), as determined via oscilloscope trace.
    The left and right columns show contrast-enhanced raw images and corresponding background-subtracted frames, respectively; the average of all 128 frames from the event was used as the background.
...
\begin{figure}[!t]
    ...
    \caption{    
    Frames from selected PCI heptane spark events in which a shock front was visible, sorted by frame time relative to spark initiation (see timestamps).
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{fig1_shockspeed_combined.png}
    \caption{
    ...
    Shock position measurement was performed manually; by having two individuals independently measure the apparent position at ten different points across the contour of the shock, for each of the 85 frames in which a shock was visible.
    ...
    %The resulting trend is strongly linear, consistent with the expected near-constant speed of a cylindrically expanding shock.
    The solid line (red) shows the least-squares linear fit to these points (1.45 km/s), and the two red dotted lines assume a shock speed 90\% and 110\% of that linear fit; this was done to roughly illustrate the magnitude of uncertainty inherent in this method of determining shock speed. 
    ...
    Blue circles indicate shock speed subestimates calculated by fitting to subsamples of the full set (within 100ns of that subestimate's position on the horizontal axis); based on these subestimates, it is apparent that the shock speed is generally decreasing with time.
    }
...
\begin{figure}[!t]
    ...
    The black solid line in represents the average cutline, the black dotted lines are the upper and lower quartiles of the distribution of cutlines, and the red line shows the best result of an optimization routine which minimizes the difference between the simulated cutline (generated using Equation \ref{eq:fresnel}) to experiment.
    ...
    In this particular case, a post-shock density of $\rho_2 = 0.799^{+0.116}_{-0.057} \si{g/cc}$ ($\rho_2/\rho_1 = 1.176^{+0.171}_{-0.084}$) was determined to best match the shock visible in this X-ray frame.
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{fig_thermomodel_annotated.png}
    \caption{
    Hugoniot states in the $\rho_2$--$v_\text{shock}$ space, showing how values estimated from this work's time-sorted PCI data and diffraction model (black) compare to normal shock thermodynamic relations in heptane (red), using the frames from Figure \ref{fig:sortedframes} for analysis.
    ...
    This results in an estimated shock velocity that is consistently decreasing with time over the course of the event, emphasized by the arrow annotation. 
    The dashed red portion of normal shock curve indicates extrapolation of the heptane equation of state.
    ...
    The blue datapoint corresponds with the particular X-ray diffraction model fit result shown in further detail in Figure \ref{fig:comparetomodel}.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{fig_LuAGvsLYSO.png}
    \caption{
        PCI images of the expanding cavitation bubble generated using LuAG (a) and LYSO (b) scintillators, cropped for comparison, and examples of plasma-induced shocks imaged using LuAG (c) and LYSO (d) scintillators, indicated with red arrows.
        Note the increased ghosting in (a) compared to (b), and the higher signal-to-noise ratio in (d) compared to (c).
        }
    \label{fig:LuAGvsLYSO}
\end{figure}


See Figure \ref{fig:singleevent} for selected sequential PCI frames from a single spark discharge event, which for clarity includes the unprocessed frames (left column) alongside a background-subtracted version (right column) in which the shock (red arrows) is more easily visible.
...
This estimated speed is shown in Figure \ref{fig:shockspeed} to be $1.45 \pm 0.13 \si{km/s}$ for this dataset corresponding to a Mach number in ambient heptane ($v_\text{sound} = 1.129 \si{km/s}$) of $1.28 \pm 0.13$.
...
This type of diffraction is the essence of PCI and is governed by the following Fresnel–Kirchhoff integral, modified for cylindrically symmetric geometries in Equation \ref{eq:fresnel} to ease computation:
\begin{flalign}
...
See Sections and SM.III for a more complete derivation of Equation \ref{eq:fresnel} and further explanation of how the computational model was implemented, and also refer to the similar model from our prior work \cite{Campbell2021}.
...
We can now compare PCI shock front images from experiment to these diffraction model solutions, as long as the portion of the shock front image being considered has sufficient symmetry along a particular axis, which need not be linear.
Furthermore, we can use the post-shock density as a optimization parameter to minimize the difference between the simulated and experimental shock front profiles, so that the routine can be used as a measurement technique to estimate post-shock density.
...
By assuming a value for the change in specific internal energy across the shock, the system can be solved implicitly, resulting in a unique relationship between $\rho_2$ and $v_\text{shock}$, shown in Figure \ref{fig:densityVSspeed} as a red curve.
However, the independently-measured values for $\rho_2$ (determined from the diffraction model fit to shock images from Figure \ref{fig:sortedframes}) and $v_\text{shock}$ (measured via a quadratic fit to the experimental data in Figure \ref{fig:shockspeed}) tend to not reside on this curve.
Rather, experimental data tends to suggest a higher-density post-shock region than what the thermodynamic model implies is possible due to the shock alone.
We attribute this to the fact that this thermodynamic model ignores the cylindrical cavitation bubble which follows the expanding shock, clearly visible in Figures \ref{fig:singleevent} and \ref{fig:sortedframes}.
...
This is shown with green asterisks in Figure \ref{fig:densityVSspeed}, generated by assuming that this cavitation bubble compression does not in turn affect the speed of the shock; although this still does not result in a model that sufficiently matches experiment, it represents the maximum post-shock density $\rho_2$ which could be justified by the data.
Similarly, the original thermodynamic model (red) serves as the minimum justifiable $\rho_2$, since it completely neglects this compression effect.
In reality, the higher $\rho_2$ caused by this compression effect is expected to increase the shock speed, though the full nature of this effect is not explored here; to fully investigate this effect, a multiphysics model would be necessary which couples together the dynamics of the cavitation bubble (Rayleigh-Plesset), the liquid region just outside the bubble (Navier–Stokes), and the jump conditions which govern the shock (Rankine-Hugoniot).
...
The similarity of this work to the submerged exploding wire PCI experiments by Yanuka \cite{Yanuka2018} at the European Synchrotron Radiation Facility (ESRF) allows us to directly compare figures of merit for the two different implementations of PCI and of cylindrically expanding shock events, of which the most dramatic difference is peak instantaneous power and total event energy; for Yanuka, these values were approximately 1GW and 300J, respectively (recall 1MW and 100mJ for this work).
The fact that the shock front images presented in this work are still visible with such a small event energy, over three orders of magnitude smaller than that of Yanuka, is a testament to the superior sensitivity and utility of PCI as a diagnostic for not only observing propagating shocks, but also quantitatively analyzing such shocks via comparison to numerical models.

%, specifically in terms of frame rate (6.5MHz), resolution ($2 \si{\micro\meter}$), and exposure time (33ps).

As a part of the same experimental campaign, the ability to image these same plasma-induced shocks with a LuAG scintillator was also investigated.
Note from this comparison that the shorter decay time of the LYSO scintillator compared to LuAG was especially apparent in the image of the cavitation bubble due to significantly fewer multi-exposed ``ghost'' images (Figure \ref{fig:LuAGvsLYSO}a and \ref{fig:LuAGvsLYSO}b), resulting the LYSO image of the shock having a higher-signal-to-noise ratio (Figure \ref{fig:LuAGvsLYSO}c and \ref{fig:LuAGvsLYSO}d).
Despite LYSO having a lower absolute X-ray sensitivity, for the above reason LYSO remains the better choice of scintillator for shock imaging. 
...
Also, the plasma-based method of shock generation used here exhibits an order of magnitude improvement in repetition rate (multiple events per minute) over conventional dynamic targets in similar experiments (e.g. exploding wire).
Since the target itself is self-healing with a reset time of $<$10 seconds, the overall repetition rate of the current experimental setup as installed at the APS is limited only by the write speed of the data storage media.
It is relatively straightforward to envision an improved version of this experiment with automated control of the event as well as optimized data storage, resulting in an even higher and more consistent repetition rate.
While the higher rate of data generation is appealing in and of itself, the ability to quickly generate very large datasets is also useful for machine learning applications.
The eighty-five manually tracked shock fronts cataloged in Figure \ref{fig:shockspeed} can conceivably be used as training data for a neural net that takes background-subtracted PCI frames as input and outputs shock parameters such as position and density ratio.
...
By exchanging the heptane for alternate discharge media such as (but not limited to) air, water, mineral oil, ice, plastics, or rock, shock propagation can be studied in these materials using this target without compromising repetition rate.
Future work will push the PCI sensitivity limit further, while at the same time continuing to develop a quantitative analysis toolkit for shock imaging as well as PCI in general; this work represents the next step on this research avenue (refer to prior work on PCI and quantitative analysis of nanosecond plasma initiation phenomena in water \cite{Campbell2021}, from which the X-ray diffraction model in this work was developed).
...
%20221216 comments from Jeph

Chris and all,


I have some suggestions for a revised title and abstract before full text editing.

The most significant results in my view are
    1. unprecedented shockwave rate observed using ultrafast X-ray PCI;
    2. material dependent of the shockwave generation rate;
    3. new implications to shockwave physics; RH shockwave model using data science? significance of this result? --> please clarify these 
    4. details of the shockwave geneation can be put in the appendix/supplementary materials.
    5. Fig. 1 can be replaced by a schematic, and the figure itself can be put in the supplementary materials section.
