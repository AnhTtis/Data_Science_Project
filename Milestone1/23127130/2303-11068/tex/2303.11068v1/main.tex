...
\author{Ivan Izmestiev,~~~~Roman Prosanov\thanks{The research of R.P. is funded by the Austrian Science Fund (FWF) ESPRIT grant ESP-12-N.},~~~~Tianqi Wu\thanks{The research of T.W. is funded by the U.S. National Science Foundation 1760471.}}
\date{}
...
The case of the conformal class for the standard 2-sphere is known as \emph{the Nirenberg problem} and remains to be unresolved in general despite many partial progress, we refer to~\cite{And} as one of the most recent advancements. 
...
Theorem~\ref{main} is proven by a standard continuity method popularized by Alexandrov, see~\cite{Ale}. We parametrize the discrete conformal class of $d$ by some domain $U(d) \subset \R^V$ and consider the curvature as a $C^1$-map $\kappa: U(d) \rightarrow (-\infty, 2\pi)^V$. Let $U_C(d) \subset U(d)$ be the subset of convex cone-metrics. We show that, when restricted to $U_C(d)$, the differential of $\kappa$ is non-degenerate, the map is proper and the domain is connected. Since the target is clearly simply connected, we obtain that $\kappa|_{U_C(d)}$ is a homeomorphism onto its image, which is the set of $\kappa\in(0,2\pi)^V$ satisfying (\ref{e1}) and (\ref{e2}).

The same method was used in the proofs of Theorems~\ref{pce} and~\ref{pch} in~\cite{GLSW, GGLSW}. In all works the non-degeneracy of the differential is based on the fact that it is equal to the Hessian of an auxiliary functional $H$. The main difference lies in the fact that in the hyperbolic and Euclidean settings the respective functional is strictly concave, which immediately implies the non-degeneracy. This no longer holds in the spherical case, which brings many complications and is responsible for some flexibility phenomena specific to this case. 

In order to obtain the non-degeneracy in our situation, we give $H$ a geometric interpretation in terms of ideal cone-polyhedra, similarly as it was done in~\cite{Pro1}. Then we use dimensional reduction arguments inspired by~\cite{BI, Izm}.
...
If there are actual examples of non-uniqueness not coming from M\"obius transformations, we conjecture, similarly to the smooth setting, that the number of such solutions is finite. Finally, we emphasize that nothing is currently known about discrete conformality of spherical cone-metrics on surfaces other than the 2-sphere. This is an interesting direction of future research.
...
\subsection{Discretely conformally equivalence}
...
\begin{proof}
Since $\mathfrak C_0(\Phi(d))$ is homeomorphic $\mathbb R^V$ by Theorem \ref{glsw} and $\Phi$ is continuous and injective, 
...
We will use some basic trigonometry of hyperbolic triangles having simultaneously ideal and non-ideal vertices. We consider ideal points in $\H^2$ or $\H^3$ always equipped with horocycles or horospheres respectively, called \emph{canonical}. By a distance between two ideal points we mean the signed distance between their canonical horocycles/horospheres. Similarly, the distance from an ideal point to a non-ideal one means the signed distance from the canonical horocycle/horosphere to the non-ideal point.
...
Let $BA_{12}$ be the perpendicular from $B$ to $A_1A_2$. By $b_{12}$ and $b_{21}$ we denote the distances $A_{12}A_1$ and $A_{12}A_2$ respectively, by $u_{12}$ denote the distance $A_1A_2$.
...
The construction above produces a continuous map $\Psi_d: U(d) \rightarrow \mathfrak C_+(d)$, which is injective due to Lemmas~\ref{delaunay} and~\ref{delaun}. It was also shown that it has a continuous injective inverse. Hence, $\Psi_d$ is a homeomorphism. One can show that it is $C^1$, but we will only use that the curvature function $\kappa \circ \Psi_d$ is $C^1$. We will obtain this in next subsection. For $u \in U(d)$ we denote the metric $\Psi_d(u)$ by $u*d$.
...
The solid angle at the vertex $A_1$ of $\bar T$ cuts off a Euclidean triangle from the canonical horosphere at $A_1$. It has edge lengths $\lambda_1$, $\rho_{12}$ and $\rho_{13}$, and angles $\omega_1$, $\alpha_3$ and $\alpha_2$ respectively. We have
...
Now we differentiate the cosine law for the semi-ideal triangle $BA_1A_2$ given in Lemma~\ref{coslaw}, use the sine law from Lemma~\ref{sinlaw} and get the derivatives
...
domain in $\mathbb R^V$. Then Theorem~\ref{main} is equivalent to the fact that the discrete curvature map $\kappa$ is a bijection from $U_C(d)$ to $K$.

It follows from Theorem~\ref{LT} that the image of $\kappa$ is in $K$. Then Theorem~\ref{main} is obtained from the following three lemmas.
...
By Lemmas~\ref{infrig} and~\ref{proper}, $\kappa$ is a proper local homeomorphism. Thus it is a covering map. Since $K$ is simply connected and by Lemma~\ref{connect} we get that $U_C(d)$ is connected, $\kappa$ is a global homeomorphism.
\end{proof}
...
We show the non-degeneracy of $d\kappa$ by a ``dimensional reduction'' argument appearing in~\cite{BI, Izm2}. To this purpose we need to introduce new objects.
...
If not, there exists a sequence $\{u^k\}$, $k=1,2, \ldots$ in $U_C(d)$ such that $\{u^{k}\}$ is leaving any compact subset of $U_C(d)$ and $\kappa^{k}:=\kappa(u^{k})$ converge to some point $\bar\kappa\in K$. Denote $d^{k}:=u^{k}*d$ and up to taking a subsequence assume that $u^{k}$ converge to some $\bar u\in[-\infty,+\infty]^V$.
