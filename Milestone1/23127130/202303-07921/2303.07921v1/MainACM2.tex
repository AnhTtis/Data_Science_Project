...
\newcommand{\LM}[1]{\textcolor{black}{#1}}
\newcommand{\strict}{}
\newcommand{\strictly}{}
...
\title{The stochastic renormalized mean curvature flow for \LM{planar} convex sets}
...
\LM{LM acknowledges fundings from the grants ANR-17-EURE-0010 and AFOSR-22IOE016}}
\maketitle
...
RMCF is the inverse gradient flow for logarithm of $\sigma/\lambda^2$ where $\sigma$ is the perimeter and $\lambda$ is the volume. 
...
We prove that along RMCF,  entropy $\mathcal{E}_t$ for curvature as well as $h_t:=\sigma_t/\lambda_t$ are non-increasing. We deduce  infinite lifetime and convergence to a disk after normalization.
For SRMCF the situation is more complicated. As $h_t$ is always a supermartingale,  for $\mathcal{E}_t$ to be a supermartingale, we need that the starting set is invariant by the isometry  group $G_n$  generated by  \LM{the} reflection with respect to the vertical line and \LM{the} rotation of angle $2\pi/n$, for some $n\ge 3$. But for proving infinite lifetime, we need invariance of the starting set by $G_n$ for some $n\ge 7$. We provide the first SRMCF with infinite lifetime which cannot be reduced to a finite dimensional flow. Gage inequality plays a major role in our study of the regularity of flows, as well as a careful investigation of morphological skeletons. We characterize symmetric convex sets with star shaped skeletons in terms of properties of their Gauss map. Finally, we establish a new isoperimetric estimate for these sets, of order $1/n^4$ where $n$ is the number of branches of the skeleton.
...
The evolution of simple closed curves in $\RR^2$ by mean curvature flow (MCF) has been investigated for a long time.  It can also be described as the inverse gradient flow for the perimeter, or as some kind of (nonlinear) geometrical heat equation.  The motivation for the study of mean curvature flow comes from physics. In 1986, Gage and Hamilton (\cite{Gage_Hamilton}) proved that starting from any convex smooth simple closed curve, the mean curvature flow converges in finite time to one point, and the form of the curve becomes circular. In 1987, Grayson (\cite{Grayson:87}) generalized this result to non necessarily convex starting curve. It is a remarkable fact that no intersection occurs during the evolution of the flow.

The renormalized mean curvature flow (RMCF) can roughly be defined as the solution to the evolution equation for curves by mean curvature, to which we add a constant normal field which prevents from implosion.  More precisely we will prove in Lemma \ref{gradfow} that RMCF is the inverse gradient flow for logarithm of ${\sigma(\partial D)}/{\lambda(D)^2}$ where the considered curve is the boundary $\partial D$ of a \LM{bounded} domain $D$, $\lambda(D)$ is the volume of the domain and $\sigma(\partial D)$ is the perimeter of the curve. For this flow, intersection can occur when the starting curve is not convex. But when the starting curve is convex, we will prove  in Theorem~\ref{TH_section2} that the lifetime of the flow $(\partial D_t)_{t\ge 0}$ is infinite and the curve converges to a circle. Two quantities will be investigated for the convergence: the ratio $ h_t\df{\sigma(\partial D_t)}/{\lambda(D_t)} $ and the entropy $\displaystyle {\rm Ent}_t\df\int_{\partial D_t}\rho_t\log \rho_t $, $\rho_t$ being the curvature at each point of $\partial D_t$. We will prove that these two quantities are non-increasing along the flow (Lemmas~\ref{eq_geo} and~\ref{ent_est}).

One of the main goals of this paper is the investigation of stochastic renormalized mean curvature flow (SRMCF) in $\RR^2$.  To define it, we add to the RMCF a one dimensional normal Brownian noise. The SRMCF can be considered as a noise model for the previous flow. The intensity of the noise is chosen so that the generator of the flow is intertwined with the one of \LM{the} Brownian motion, via a Markov kernel (see e.g.\ \cite{zbMATH07470497} and \cite{arnaudon:hal-03037469}).  Alternatively, very nice connections with Bessel~3 processes can be exhibited (\cite{zbMATH07470497}). In fact, when the intertwining can be realized with a pair of stochastic processes $(X_t,D_t)_{\LM{t\geq 0}}$ with  a Brownian motion \LM{$(X_t)_{t\geq 0}$ satisfying that at any time $t\geq 0$, $X_t$ is uniformly distributed}
inside $D_t$ conditionally to $(D_s)_{0\le s\le t}$, the construction is a generalization of Pitman "$2M-X$" theorem. An important object in the construction of the coupling $(X_t,D_t)$ is the inner skeleton $S_t$ of $D_t$, which is the singularity set of distance to boundary, inside $D_t$: the equation for $\partial D_t$ has a component of the drift which is proportional to the local time of $X_t$ at $S_t$ (\cite{arnaudon:hal-03037469}). A very nice fact about the skeleton is that even if $\partial D_t$ has a Brownian noise, $S_t$ has finite variation. { As we will see in the present paper, the inner skeleton $S_t$ also plays a role in the lifetime of $D_t$. We will prove that starting with a convex subset $D_0$ of $\RR^2$, the explosion occurs only when  $\partial D_t$ meets $S_t$ (Theorem~\ref{th_cores}).} We will also prove that similarly to the deterministic situation, the process $h_t$ is a supermartingale (Lemma~\ref{prop-h-sm}). For the entropy being a supermartingale we will need that $D_0$ is invariant by the linear group $G_n$ generated by the rotation of angle $2\pi/n$ with $n\ge 3$, and the symmetry with respect to an axis (we will choose the vertical one, \LM{see} Proposition~\ref{Ent}). $G_n$-invariance for any $n\ge 2$  will be proved to be preserved by the flow.  Finally we will prove that $G_n$-invariance of $D_0$ with $n\ge 7$ implies infinite lifetime for the stochastic renormalized mean curvature flow (Theorem~\ref{infinite-lifetime}).

In Section~\ref{conv-sym} we investigate some class of convex sets in $\R^2$ which are symmetric with respect to $G_n$, and with star-shaped skeletons. We prove (Proposition~\ref{prop_S}) that they are preserved by all our flows. The last section is devoted to the proof of a new isoperimetric inequality for these classes of convex sets (Proposition~\ref{isoperimetric}). A bound of order $1/n^4$ is observed. 
\par
\medskip
\begin{definition} \label{strictconv}
 A simple closed curve is said to be strictly \LM{convex} when its geodesic curvatures are positive.
\end{definition}
...
The  coordinate $\theta$ will make the equations of our flows simple to \LM{analyze}, in particular since operators $\partial_{\te} $ and $\partial_t $ will commute, contrarily to derivative with respect to curvilign abscissa $\partial_{s} $ and $\partial_t $, as shown in \eqref{eq_courbure}. 
...
\begin{equation} \label{cores}
 ...
 \end{equation}
  as in Lemma 4.1.1 of Gage and Hamilton \cite{Gage_Hamilton}. The function $ \rho$ is in fact the curvature of the curve, see also Section \ref{conv-sym}. 

  Using the stochastic evolution of curves, such as stochastic mean curvature flow (SMCF) \eqref{eq_sto_MCF}  and SRMCF \eqref{eq_sto}, we will derive the equation of the curvature, and we will show in Theorem~\ref{th_cores} that positivity of the  curvature is preserved by the flows. Also Equation~\eqref{cores} is conserved along these equations, and so we will get an alternative definition of the stochastic evolution of a convex curve in term of the solution of some stochastic partial differential equation (Theorem~\ref{th_cores}). 

\par\medskip
To fix some notations used throughout the paper, let us present some notions  associated to a simple  $\rC^2$ closed curve
$C\st \TT\ni u\mapsto C(u)\in\mathbb{R}^2 $, where $\TT\df\RR/(2\pi\ZZ)$.
In this paper, all curves will be closed and immersed. 
\par
The bounded domain whose boundary is $C$ is denoted $D$. The quantities $\ld (D) $ and $ \sg(C)$ respectively stand for   the volume of $ D$
...
For any $x\in C$, $\nu_C(x)$ is the outer unit normal vector of the curve $C$ at the point $x$ and $\rho_{C}(x)$ is the corresponding  mean curvature.\par

\section{The renormalized mean curvature flow (RMCF)} 
Let $ u\in \mathbb{T}\mapsto C_0(u) $ be a simple $C^{2 + \alpha} $ closed curve in $ \mathbb{R}^2$.
\begin{definition}
 Let  $ C (t,u) :  [0,T_c) \times \mathbb{T} \to \mathbb{R}^2 $ a family of simple close curve, we say that $C (t,\cdot) $ evolves under the renormalized mean curvature flow, shortly (RMCF), if it satisfy the following equation
\begin{equation} \left \{  \begin{array}{lcl}\label{eq_det}
 \partial_t C(t,u) & = & (-\rho_t(C(t,u)) + 2h(D(t)) )\nu_{C(t,.)}(C(t,u) ) \\
  C(0,u) &= & C_0(u) \\
 \end{array}  \right. \end{equation}
 \end{definition} 
  Sometimes we will omit the parameter $ D(t)$ or $ C(t,.)$ in the notation, and even simply write for example $ h(t) $ for $h(D(t)) $. 
 \begin{Remark} Since the symbol of the equation \eqref{eq_det} is same as in the mean curvature flow, we have the short time existence of the equation \eqref{eq_det}  (i.e. $T_c > 0$) for simple initial  closed $\mathcal{C}^{\infty} $ curve, see for example \cite{Gage_Hamilton} or \cite{Gage_area}. We need the simplicity of  the curve to have no ambiguity for   $ h $ (mainly for the interior volume, see Figure.\ref{dessin} example a) ) and to have well-defined outer unit normal vector.\\
An  alternative proof of the existence, using quasi-linear equations, can be found in Chapter 4  of \cite{zbMATH07470497}, Theorem 40 with $B_t =0$.  
 \end{Remark}
 \subsection{The main result of this section}
 As we will see later, the evolution of the curvature is  easier to analyze. Also in the Euclidean plane we have some geometrical inequalities concerning convex  closed curve: the Gage inequality \eqref{gage},  involves the non local term $h$, and permits to have some a priory estimate along the solution; also the usual isoperimetric inequality plays a important role in  the study of this flow. The principal result of this section is the following:

 \begin{thm}\label{TH_section2}
 If the starting  closed curve  $C(0,.) $ is strictly convex and simple, then the solution $C(t,.) $ of equation \eqref{eq_det}  is defined for all $t \in [0,\infty[  $, it remains strictly convex and simple  for all times and becomes circular, the isoperimetric ratio is \strictly decreasing (except for circular starting curves). After  renormalization and translation,  we have the convergence with respect to the Hausdorff metric
   $$ \frac{1}{\sqrt{6t}} [C(t,.) - \ci(t)] \overset{d_{\mathcal{H}}}{\to} \mathcal{C}(0,1) ,$$
to the circle of center $0$ and  radius $ 1$,  where for all $t$, $ \ci(t)$ is a center of an inscribed circle of $C(t,.) $ . 
 \end{thm}
 
 The rest of this section is devoted to the proof of Theorem~\ref{TH_section2}.
 
\subsection{Gradient flow formulation, and evolution of geometric quantities}  To a solution $(C(t,.))_{t\in [0,T_c[}$ of \eqref{eq_det}, associate 
 \bq
...
To prevent the domain of definition of~$s$ to depend on~$t$, we decide that~$s$ is defined in~$\R$ and periodic with period~$\sigma(C(t))$.  Let $T\df  \partial_s C(t,s) $, the tangent vector of the curve $C(t.)$ at the point $C(t,s) $. { Let $\nu(t,s)$ be the unit vector obtained by a rotation of $T(t,s)$ by an angle of $-\pi/2$.  To make a choice of orientation, we will always assume that $\nu$ is the outer normal of the curve.}  


 \begin{lemma}\label{gradfow}
 Equation \eqref{eq_det} is the {inverse} gradient flow of the functional 
  $$ \Psi : D \mapsto { \ln \frac{\s(\partial D)}{\lambda(D)^2}}. $$
 \end{lemma}
 \begin{proof}
 Let  $ C (t,u) :  [0,T) \times \mathbb{T}  \to \mathbb{R}^2 $ be  a family of simple closed curves, such that  $ \partial_t C(t,u) = X(t,u) $ for some smooth $X : [0,T)\times \TT \to \mathbb{R}^2$. Then 
   $$\frac{d}{dt} \lambda(t) = \int_{C(t,.)} \langle X , \nu \rangle  ds $$
and
  $$\frac{d}{dt} \s(t) = \int_{C(t,.)} \langle X , \nu \rangle \rho ds, $$ 
  so we have 
 \begin{equation*}   \begin{array}{lcl}
\frac{d}{dt} \frac{\s(t)}{\lambda(t)^2} &=& \frac{1}{\lambda(t)^2}  \left(
 \int_{C(t,.)} \langle X , \nu \rangle \rho ds - \frac{2\s(t)}{\lambda(t)} \int_{C(t,.)} \langle X , \nu \rangle  ds  \right)\\
 &=& \frac{1}{\lambda(t)^2}  \left(
 \int_{C(t,.)} \langle X , (\rho  - \frac{2\s(t)}{\lambda(t)})  \nu \rangle  ds  \right)\\
 &=& \frac{\s(t)}{\lambda(t)^2}  \left(
 \int_{C(t,.)} \langle X , (\rho  - \frac{2\s(t)}{\lambda(t)})  \nu \rangle  \frac{ds}{\s(t)}  \right).
 \end{array}   \end{equation*} 
 Hence $  (-\rho + 2h)\nu$ is  the { $\LL^2\left(\f1{\s(t)}ds\right)$ 
inverse} gradient flow of $\Psi .$  
\end{proof} 



\begin{proposition}
Under the RMCF, we have the following evolution of geometric quantities:
...
$$ \begin{aligned}
 \partial_t  \partial_u C &= \partial_u \partial_t C  \\
  &= -(\partial_u \rho) \nu + (-\rho +2h)\partial_u \nu .\\
\end{aligned} $$
We deduce:
$$ \begin{aligned}
...
\begin{proposition}
Under the RMCF, we have the following evolution of the curvature:
...
\begin{lemma}\label{eq_geo} If the family of curves $C(t,.) $,  solution of the flow \eqref{eq_det}, remain simple for all $ t \in[0,T_c[$, then we have for all $ t \in[0,T_c[$:
\begin{enumerate}
...
If the family of curves $C(t,.) $, solution of \eqref{eq_det}, remain convex for all $ t \in [0,T_c[ $, then the isoperimetric ratio is non-increasing, i.e. for all $ t \in [0,T_c[ $,
...
If the family of curves $C(t,.) $, remain simple along the flow \eqref{eq_det}, then  the deficit of isoperimetry is non-increasing, i.e.:
$$ \frac{d}{dt} (\sg_t^2 - 4\pi \ld_t ) \le 0 .$$
If moreover  the family of curves $C(t,.) $  remain convex for all $ t \in [0,T_c[$ then for all $ t \in [0,T_c[ $ we have:
\begin{enumerate}
...
&\le 0 .\\
\end{align*}
If moreover  the family of curves $C(t,.) $  remain convex, in the second line of the above computation, we can improve the inequality using \eqref{gage} instead of Gauss-Bonnet Theorem, and we get  for all $ t \in [0,T_c[ $:
 \begin{align*}
...
\end{proof}
We get the following corollary, which describes the shape of the curve $C(t,.)$ as~$t$ goes to infinity.  
\begin{corollary} \label{become_circle}
If  the  curves $C(t,.) $ are defined for all times $t\geq 0$ and  remain convex then 
$$ \lim_{t \to \infty } \frac{\sg_t^2}{\ld_t} = 4\pi .$$

After  renormalization the curve $\frac{1}{\sqrt{6t}} [C(t,.)- \ci(t)]$ converge to the circle of center~$0$ and radius~$1$ for Hausdorff metric, where $\ci(t)$ is a center of an inscribed circle of $C(t,.) $ . 
...
where $ \rao(t) , \rai(t)$ are respectively the outer and the inner radius of the curve $C(t,.) $,
we get  that $$ (\rao(t) - \rai(t) )^2 \le  \frac{\ld_t}{\pi^2} \left( \frac{\sg_t ^2 }{\ld_t} - 4\pi \right) . $$
Let $\ci(t)$ be a center of an inscribed circle of $ C(t,.)$, and $\co(t)$ be a center of a circumscribed circle  of $ C(t,.)$.
...
For two compact set $ A,B \subset \mathbb{R}^2$ and $ \epsilon \ge 0$  we define $$A_{\epsilon}\df  \{x \in \mathbb{R}^2, d(x,A) \le \epsilon \} $$
...
Similarly  we have that $\frac{1}{\sqrt{6t}} [C(t,.)- \ci(t)]$ converge to the circle of radius $ 1$  and center $0$ for the Hausdorff metric.
\end{proof}
\subsection{Preserving the convexity, and lower bound of the curvature}

We will now show that if the starting curve is strictly convex and simple then it remains convex and simple during the flow \eqref{eq_det} until the lifetime.
In a second time we will show that the lifetime is infinite using intensively some ideas developed in \cite{Gage_Hamilton}.

It is possible to parametrize a strictly convex curve in $ \mathbb{R}^2$ using the angle $\theta$ between
the tangent line and the $ x $ axis, i.e. $T = (\cos(\theta), \sin(\theta))$. Normally, the angle $\theta$ depends on $ u$ and $t$. Following \cite{Gage_area} and \cite{Gage_Hamilton}, after adding to the flow \eqref{eq_det}  a tangential perturbation, we do not perturb  the shape of  the curve, and it is possible    
to find a tangential intensity so that the parameter $\theta$ does not depend on the time.
This  change of coordinate will make the equation more simple to understand (since operators $\partial_{\te} $ and $\partial_t $ will commute, contrarily to $\partial_{s} $ and $\partial_t $ as shown in \eqref{eq_courbure}). 

Let us quickly recall this fact. We define (since the shape of the curve does not change we will keep the notation,  only the parametrisation changes):
...
$$ \partial_t T = \left(- \frac{\partial_u \rho}{v} - \alpha (u,t) \rho \right)\nu $$
and 
$$ \partial_t v = -v\rho^2 + 2h v \rho +  \partial_u \alpha .$$
To make $ \theta$ and hence $T $ independent of $t$ we take 
$\alpha = - \frac{\partial_u \rho}{v\rho} . $
Differentiating $\partial_t T  $ with respect to $ u$, we get that $ \partial_t (v\rho) = 0. $
Since $ \partial_{\theta} T = -\nu $,  we have by chain rule $\frac{\partial u}{\partial \theta } \frac{\partial T}{\partial u } = - \nu  $, hence $ \frac{\partial u}{\partial \theta } = \frac{1}{v\rho} $ and we deduce that
...
\end{align*}
{
We summarize the result in the next lemma.
\begin{lemma}\label{eqrmcf}
When the curvature remains positive, equation~\eqref{eq_det_pert} for the RMCF yields  equation 
...
A strictly convex simple curve $C(0,.) $, that evolves according to \eqref{eq_det_pert} remains strictly convex and simple during the existence of the flow. Moreover
...
Also by Lemma \ref{eq_geo} ((*)suppose for the moment that the curve remains simple),  $h$ is non-increasing, and
...
$\mathcal{Q}_{\inf}(t) :=\inf \{ \mathcal{Q}(\te,t ), \, 0\le \te \le 2\pi\} $. The proof is by contradiction,  suppose that there exist $ 0 < \eta <\mathcal{Q}_{\inf}(0) $ and $ t > 0$ such that $ \mathcal{Q}_{\inf}(t)= \eta$, let $ t_0$ be the first time such that $\mathcal{Q}_{\inf}(t_0)= \eta .$  This minimum is achieved at some point $ \te_0$, and at this point:\\
 ...
 so $\rho( \te, t) \ge \rho_{\inf} ( 0) e^{-\mu t} $ for all $\mu > h_0^2   $. Hence if $C(0,.) $ is strictly convex then $C(t,.) $ remains strictly convex at any time~$t$ up to which it is defined.
  
(*)Remark that in the above proof we have used  Lemma \ref{eq_geo}, all along the duration of the equation \eqref{eq_det_pert}, without showing that the curve remains simple. But the argument is the same, by contradiction , if the first time $T_s $ when the curve stops to be simple, occurs strictly before the maximal lifetime of  \eqref{eq_det_pert}, i.e. $ 0< T_{s} \le t* < T_c$ by the same computation as above the curvature $\rho $ is \strictly positive until $T_{s}$, $\rho$ and all its derivatives are bounded in $[0,t*] $, so there exist a limiting curve as $t$ goes to $ T_s$, that is smooth and has \strictly positive curvature (Arzel\`a-Ascoli theorem). Using Lemma 4.1.1, and Theorem 4.1.4 in \cite{Gage_Hamilton} we know that the positive curvature and the conservation of $$\int_0 ^{2\pi} \frac{\cos(\te)}{\rho} \,d\te = \int_0 ^{2\pi} \frac{\sin(\te)}{\rho} \,d\te = 0 $$ during the flow characterize the simple close strictly convex curve. So the limiting curve  is simple and strictly convex, this contradicts the non simplicity at this time.
   \end{proof}
 \begin{corollary}\label{conv_plus}
 A strictly convex simple curve  $C(0,.) $, that evolves according to \eqref{eq_det_pert} remains convex and simple during the existence of the flow. Moreover
...
and $$ \rho(t) \ge \frac{1}{\frac{1}{\rho_{\inf}(0)} + \frac{\sg_0^2}{6\pi^2 \ld_0} \sqrt{24 \pi^2 t + 4\pi\ld_0 }}.$$ 
 \end{proof} 
\subsection{Upper bound of the curvature and long time existence of the flow for strictly  convex simple initial curve}
We will now show that \eqref{eq_det} exists for all times if $C(0,\cdot) $ is a strictly convex simple curve, by first establishing a uniform bound (depending on time horizon) of the maximum of curvature.
...
If a convex closed plane curve encloses an area $ \ld$ and has length $\sg $ then $ \rho^{*}  \le \frac{\sg}{\ld}. $
\end{lemma}
Following the above lemma we have:
\begin{corolary}
If the starting curve $C(0,.) $ is simple and  strictly convex, and the solution to Equation \eqref{eq_det} exists for all $ t \in[0, T [ $ then for all $  t \in[0, T [ $,
 $$ \rho^{*} (t) \le h \le h_0. $$
\end{corolary}
\begin{proof}
We use Lemma \ref{conv} to get the convexity until $ T$, { Lemma~\ref{eq_geo} for $h$ non-increasing,  } and the above Lemma.
\end{proof}

\begin{lemma}[Entropic estimate]\label{ent_est}
If the starting curve $C(0,.) $ is strictly convex and simple, and  the solution of equation \eqref{eq_det} is defined for all $t \in [0,T[  $, then 
...
\bq-2\pi \left[\log( \tilde{C}_0 + \sqrt{ \tilde{C}_1t +\tilde{C}_2 })\right]&\le & \Ent(t)\ \le\   \Ent(0)  -\frac{2\pi}{C_1} \left[\log (\frac{C_1t+C_0}{C_0} )\right] . \eq
...
If the starting curve $C(0,.) $ is strictly convex and simple, and  the solution of equation \eqref{eq_det} is defined for all $t \in [0,T[  $, then there exists a constant $C_0$ that depends only on the initial curve such that:
$$
...
If the starting curve $C(0,.) $ is strictly convex and simple, and  the solution of equation \eqref{eq_det} is defined for all $t \in [0,T[  $ then if $T < \infty$ 
$$  M = \sup \{ \rho(\te, t),(\te, t) \in \mathbb{R}/2 \pi \mathbb{Z} \times [0,T[  \} $$
 is bounded .
\end{proposition}
\begin{proof}
For $ t < T$ let $$M_t = \sup \left\{ \rho(\te, s),(\te, s) \in \mathbb{R}/2 \pi \mathbb{Z} \times [0,t]  \right\}  .$$
There exists $ (\te_1 , t_1)  \in \mathbb{R}/2 \pi \mathbb{Z}\times [0,t]   $ such that $M_t = \rho(\te_1, t_1) $. 
Then for all $\te_2 \in \mathbb{R}/2 \pi \mathbb{Z} $ we have:
\begin{align*}
...
\end{align*}
where we have used Proposition \ref{inv_Poin} at the third line, and $\vert \te_1 - \te_2 \vert$ is the usual distance in $ \mathbb{R}/2 \pi \mathbb{Z} $.
We also have,
$$ M_t \ge  \rho_{sup} ( 0)  . $$
...
So $  M = \sup \left\{ \rho(\te, t),(\te, t) \in \mathbb{R}/2 \pi \mathbb{Z}\times [0,T[  \right\} $ is bounded for $ T < \infty$.
\end{proof}


\begin{proposition}\label{bound_derivative}
If the starting curve $C(0,.) $ is strictly convex, and  the solution of equation \eqref{eq_det} is defined for all $t \in [0,T[  $ then if $T < \infty$ then
 for all $ n \in \mathbb{N}$
$$  M^{(n)} = \sup \left\{ \frac{\partial^n \rho}{\partial^n \te} (\te, t),(\te, t) \in \mathbb{R}/2 \pi \mathbb{Z}\times [0,T[  \right\} $$
 is bounded.
\end{proposition}
 \begin{proof}
...
\subsection{Proof of  Theorem \ref{TH_section2}} 
 ...
 We  prove the long time existence of the flow. 
   Assume that  the starting curve $C(0,.) $ is simple and strictly convex, and the flow exists for all $t \in [0,T[ $. 
 Then by Lemma \ref{conv} we know that the solution of \eqref{eq_det_pert} remains convex and simple during the flow. 
  ...
  By Propositions \ref{bound-rho}  and \ref{bound_derivative} we know that $\rho$ and all spacial derivatives of $\rho$ are bounded in $[0,T[ $, if $T < \infty $, hence { the same} for time derivative of $ \rho$. So by Arzel\`a-Ascoli Theorem, $ \rho $ converges to a $C^{\infty} $ function $\rho(T,.) $ as $ t \to T $. Using equation \eqref{eq_det} there exists a limiting curve $C(T,.) $ and this limiting curve is associated to $ \rho (T, .)$ (in the sense of Lemma 4.1.1 in \cite{Gage_Hamilton}), so  $C(T,.) $ is  strictly convex and simple. By the small time existence if $T < \infty $, we can extend the time interval on which the solution is defined using the solution that starts at $C(T,.)$. This proves that the solution of \eqref{eq_det} starting with at a simple strictly convex  curve exists for all time. 
  ...
  $ \frac{1}{\sqrt{6t}} [C(t,.) - \ci(t)] $ converges to circle of center $0$ and  radius $ 1$ for Hausdorff metric, where $ \ci(t)$ is an center of a inscribed circle of $C(t,.)$. 
\par\hfill $\square$\par\medskip
\begin{Remark}
If the starting curve is convex and not simple, as in Figure \ref{dessin}(a), the flow is not well defined. And if the starting curve is simple but non convex, the existence in long time is not automatic as in Figure\ref{dessin} b).
\end{Remark}
...
\section{The stochastic renormalized mean curvature flow (SRMCF)~in~$\mathbb{\R}^2$}
...
Let $ u\in \mathbb{T}\mapsto C_0(u) $ be a simple $C^{2 + \alpha} $ closed curve in $ \mathbb{R}^2$.
\begin{definition}
 Let  $ C (t,u) :  \mathbb{R}^+ \times \mathbb{T} \to \mathbb{R}^2 $ be a family of simple closed curves. We say that $C (t,\cdot) $ evolves under the renormalized stochastic mean curvature flow, shortly (SRMCF), if it satisfies the following equation
 \begin{equation} \left \{  \begin{array}{lcl}\label{eq_sto}
 d_t C(t,u) & = & \lt([-\rho_t(C(t,u)) + 2h_t ]dt + \sqrt{2}dB_t \rt) \nu_t(C(t,u))  \\
  C(0,u) &= & C_0(u) \\
 \end{array}  \right. \end{equation}
 where  $h_t$, $\nu_t$ and $\rho_t$ standing respectively for $h(D(t))$, $\nu_{C(t)}$ and $\rho_{C(t)}$.
\end{definition} 

 When convenient and when there is no possible confusion, the index $t\geq 0$ will even be omitted. 
  It is an important goal of this paper to show that the above equation  admits a solution in the whole temporal interval $\RR_+$ under some assumptions.
 Until it is proven, when we consider a time $t\geq 0$, it will be implicitly assumed that $t$ is before the (random) time up to which a solution of \eqref{eq_sto} is constructed.
  \begin{Remark} For the short time existence of \eqref{eq_sto}, i.e.\ the existence of a stopping time $\tau > 0$ such that equation \eqref{eq_sto} is defined for all $t \in [0,\tau[ $,  we refer to Theorem~40  in \cite{zbMATH07470497}, where the authors use Doss-Sussman method and the theory of  quasi-linear equations, as well as inverse function theorem.
  In the sequel, any such positive stopping time $\tau$ will be called a \textbf{lifetime} for \eqref{eq_sto}.
  \par Concerning the regularity,  for any $0<\alpha < 1$,   the solution of \eqref{eq_sto} is  $\rC^{\alpha / 2, \infty}$ if the starting curve $C(0,.) $ is smooth,. In fact it is enough  to consider that  the starting curve $C(0,.) $ are $\rC^{\alpha  + n}$ , for $ n \ge 2$ and $0<\alpha < 1$, to get the $\rC^{\alpha / 2,\alpha+ n}$ regularity of the solution of \eqref{eq_sto}  (cf.\ Chapter 8 of Lunardi \cite{MR3012216} and \ Chapter 4 in \cite{zbMATH07470497}). So, to justify the existence of all the derivatives one may need, it is sufficient to take $ C_0$  regular enough, but we will not insist on the regularity of $C_0$ in the rest of the paper. 
 \end{Remark}
 \par
 To a solution $(C(t))_{t\geq 0}$ of \eqref{eq_sto}, as in the deterministic situation, associate 
 \bq
 \fo t\geq 0,\,\fo u\in\TT,\qquad v(t,u) &\df& \vert \partial_u C(t,u) \vert\eq
  and the arc-length parametrization  $\partial_s\df  \frac{1}{v} \partial_u $ (equivalently  $ ds = v du $).
 \par
 For any $t\geq 0$ and $u\in\TT$, 
 $T_t (u) $ will stand for the unit tangent vector of the curve $C(t)$ at $u$ (i.e. in $\mathbb{R}^2$ we have $\mathcal{R} (T ) = \nu$,  where $\mathcal{R} $ is the rotation of angle $ - \frac{\pi}{2} $).
 \par
...
\begin{lemma} \label{eq_courbure_sto} Let $(C(t))_{t\geq 0}$ be a solution of \eqref{eq_sto}. We have the following equations in the $(t,s)$-domain of validity:
...
For the last point, the mean curvature $ \rho_t $ satisfies:
$$
\begin{aligned}
...
\end{proof}
  ...
  \begin{Remark}
We want to stress that \eqref{this} is a SPDE with mobile barrier (recall the parameter $s$ lives in a time dependent  interval $[0, \s_t] $).
\end{Remark}

\begin{Remark} \label{retrouve}
Note also that after integration, we recover the equation satisfied by $\s_t $ and $ \lambda_t$ obtained by $\mathcal{L}$ diffusion technique (resp. Proposition 57 and equation (106) in \cite{zbMATH07470497})   in the case of a simple curve: 
$$
\begin{aligned}
d_t \s_t &= d_t \int_{0}^{2\pi} v_t(u) \, du\\
           &=  \int_{0}^{2\pi} v_t \rho_t  \left( (- \rho_t + 2h) dt + \sqrt{2} dB_t   \right) \, du\\ 
           &= \lt( -\int \rho_t^2 ds + 2h_t \int \rho_t ds\rt) dt + (\int \rho_t ds)\sqrt{2} dB_t \\
...
Let us recall that for a simple strictly convex closed curve in $ \mathbb{R}^2$ it is possible to parametrize the curve using the angle $\theta$ between
the tangent line and the $ x $ axis, i.e. $T = (\cos(\theta), \sin(\theta))$.
 Usually, the angle $\theta$ depends on $ u$ and $t$. Following \cite{Gage_area} and \cite{Gage_Hamilton}, after adding to the stochastic flow \eqref{eq_sto}  a tangential perturbation with finite variation, we do not perturb  the shape of  the curve, and as detailed below, it is possible    
to find a tangential intensity so that the parameter $\theta$ does not depend on the time.
This  change of coordinate will make the equation easier to understand (since operators $\partial_{\te} $ and $\partial_t $ will commute, contrary to $\partial_{s} $ and $\partial_t $ as shown in Lemma~\ref{eq_courbure_sto}). 

Let us quickly show this fact. Consider the following tangential perturbation of equation \eqref{eq_sto}:
\begin{equation}
 \left \{  \begin{array}{rcl} \label{eq_sto_mod}
 d_t {\tilde C}(t,u) & = &  \left( -\tilde{\rho_t}(u) + 2h(\tilde{D_t}) )dt + \sqrt{2}  dB_t  \right)\tilde{\nu}_t(u) + (\alpha_t(u) dt)\tilde{T}_t (u)  \\
  {\tilde C}(0,u) &= & C_0(u)
 \end{array}  \right. 
 \end{equation}
 
 Since only the parametrisation changes, the lifetime of \eqref{eq_sto_mod} is the same as the one of \eqref{eq_sto}, so for simplicity we will drop the tilde in the above equation, and sometimes the parameter $u$. 
With similar computation as we have done before in the proof of Lemma \ref{eq_courbure_sto} we get:

\begin{lemma} Letting $ C_t(u)$ be a solution of \eqref{eq_sto_mod},  we have:
\begin{enumerate}


\item $ \begin{aligned} 
 d_t \partial_u C_t &=  \left( (-\partial_u \rho_t - \rho_t v_t \alpha_t ) dt  \right) \nu_t \\
 &+  \left( v_t\rho_t  [(-\rho_t + 2h )dt + \sqrt{2} dB_t ]    + \partial_u \alpha_t dt \right) T_t .
 \end{aligned}$

\item $ dv_t =  v_t\rho_t   [(-\rho_t + 2h )dt +\sqrt{2} dB_t ]  \  + \partial_u \alpha_t dt .$

\item $ d_t T_t = -\frac{1}{v} (\partial_u \rho_t + \rho_t v_t \alpha_t )dt \nu_t. $
\end{enumerate}
...
\begin{aligned}
 d_t (v_t)^2 &= d_t  \vert \partial_u C(t,u) \vert^2 = 2 \langle d_t \partial_u C(t,u) , \partial_u C(t,u) \rangle +  \langle d_t \partial_u C(t,u),d_t \partial_u C(t,u) \rangle  \\
   &=  2 v_t  \left( v_t \rho_t  \left( (-\rho_t + 2h_t )dt + \sqrt{2}  dB_t  \right) + \partial_u \alpha_t dt  \right) + 2  v^2_t\rho_t^2 dt.\\
\end{aligned}$$ 
...
&=  \left( - \frac{\rho_t}{v_t}  \left( (- 3 \rho_t + 2h) dt + \sqrt{2} dB_t  \right) - \frac{\partial_u \alpha_t}{v_t^2} dt  \right) v_t T_t \\
&+ \frac{1}{v_t}  \left(  ( - \partial_u \rho_t -\rho_tv_t\alpha_t) \nu_t dt +  \left( v_t \rho_t ( (-\rho_t + 2h_t )dt + \sqrt{2}  dB_t ) + \partial_u\alpha_t dt  \right)T_t  \right)\\ 
&-2\rho_t^2T_t dt \\
&= - \frac{1}{v_t}(\partial_u \rho_t + \rho_tv_t\alpha_t)  dt \nu_t .
\end{aligned}
$$ 
\end{proof}

In the above lemma, if the curvature is positive and  if we take $$ \alpha_t = \frac{-\partial_u \rho_t}{v_t \rho_t}$$ 
we get that $T_t$ and $\nu_t $ become constant in time hence the angle $\theta $ becomes constant in time, as desired.



The following lemma will describe the evolution of the curvature in this system of coordinates. 

\begin{lemma} \label{eq_rho_theta}  If $C_0 $ is $ \rC^{\alpha + 4}$ and  strictly convex, the solution to 
$$ d_t\rho_t(\theta) = \rho^2_t(\theta) (\partial^2_{\theta} \rho_t(\theta)) dt + \rho^2_t(\theta)   \left( (3\rho_t(\theta) - 2h )dt - \sqrt{2} dB_t  \right), $$
is well-defined  for all 
 $ 0 \le t < \tau_0 \wedge  \tau $ where,  $ \tau_0 = \inf \{t \ge 0,\, s.t. \, \exists u \in [0 , 1] , \rho_t(u) = 0 \}$ and $\tau $ is the lifetime of \eqref{eq_sto_mod}. Also by  definition of $\tau$ all the quantities $ h, \rho, \partial_{\te}\rho, \partial^2_{\te}\rho $ are bounded until $\tau $.
 \end{lemma}
...
\begin{aligned}
  v_t d\rho_t &= - \rho_t d_t (v_t) -  dv_t d\rho_t \\
              &=  - \rho_t   \left( v_t\rho_t   [(-\rho_t + 2h )dt +\sqrt{2} dB_t ]  + \partial_u \alpha_t dt    \right) -   dv_t d\rho_t .  \\
\end{aligned} $$ 
...
$$
\begin{aligned}
   d\rho_t  &=  - \rho^2_t   \left(   (-\rho_t + 2h )dt + \sqrt{2} dB_t  \right)    -\frac{\rho_t}{v_t} \partial_u \alpha_t dt +2 \rho^3_t dt \\
   &=  - \rho^2_t   \left(   (-3\rho_t + 2h )dt + \sqrt{2} dB_t  \right)    -\frac{\rho_t}{v_t} \partial_u \alpha_t dt  \\
...
Let $( C_0(\theta))_{\theta\in \RR/(2\pi\ZZ)}$ be a strictly convex simple closed curve, \goodbreak\noindent parametrized by the angle $ \theta$  (in particular $\rho_0>0$). 
...
\begin{equation} \left \{  \begin{array}{lcl}\label{eq_rho}
 ...
 \end{array}  \right.
\end{equation}
with lifetime $  \tau_2$,  namely the solution has to be regular up to order $2$ at least for all times smaller than $ \tau_2$.

Then $\tau_0 \wedge \tau \le  \tau_2 $, and for all $ t<  \tau_2 $, $\rho_t(\theta)>0$ for all $\theta\in \RR/(2\pi\ZZ)$ and \eqref{eq_rho} provides  the solution of \eqref{eq_sto}  by :
...
\par
Thus by uniqueness of the solution of \eqref{eq_sto}, \eqref{eq_rho} admits a unique regular solution too, and $  \tau_2 \le \tau $.

\end{thm}

\begin{proof}
 By  lemma \ref{eq_rho_theta}, \eqref{eq_rho} admit a solution and $\tau_0 \wedge \tau \le  \tau_2 $.
...
which is positive.\par

 Recall  Lemma 4.1.1 in \cite{Gage_Hamilton}, or see the beginning of Section \ref{conv-sym}, that says a $2 \pi $ periodic positive function $\rho $ represents the curvature of a simple closed strictly convex plane curve if and only if 
 \begin{equation}
 \int_0 ^{2\pi} \frac{\cos(\te)}{\rho(\te)} \,d\te = \int_0 ^{2\pi} \frac{\sin(\te)}{\rho(\te)} \,d\te = 0 .
 \end{equation}
 Here this equation is satisfied by $ \rho_0(\te) $.
 So we have to check that this relation is conserved over time for $ \rho_t(\te) $ solution of \eqref{eq_rho}.
...
This is  \eqref{eq_sto_mod}  and so up a parametrization, this is a solution of \eqref{eq_sto}. 
Since a solution of \eqref{eq_rho} produces a solution of \eqref{eq_sto}, by uniqness of solution of  \eqref{eq_sto}, we get the uniqueness of the solution of \eqref{eq_rho}, and 
$ \tau_2 \le \tau $.
 
 \end{proof}

 We will show that Equation \eqref{eq_sto} preserves the positivity of the mean curvature.
 

\begin{lemma}\label{conserve_convex}
Consider the solution of \eqref{eq_sto}, if $C_0 $ is $ \rC^{\alpha + 4}$ and $ \rho_0 > 0 $ then  $\rho_t > 0$ for all $ t < \tau$, where $\tau  $ is any lifetime of \eqref{eq_sto}, moreover $\tau =  \tau_2 $.
...
that here $1/(v\rho)=\partial u/\partial \theta=1$,
...
\end{aligned} $$
\par
Also by similar computation as above, we have:
...
\begin{corollary}\label{cor1} If the starting curve is simple closed and strictly convex, there is a one to one correspondence  between the solutions of \eqref{eq_sto},  \eqref{eq_rho} and \eqref{eq_sto_mod}.
\end{corollary}
\begin{proof}
...
Consider the following stochastic mean curvature flow,
  \begin{equation} \left \{  \begin{array}{lcl}\label{eq_sto_MCF}
...
\begin{corollary}
Consider the solution of \eqref{eq_sto_MCF}, if $C_0 $ is $ \rC^{\alpha + 4}$ and $ \rho_0 > 0 $, then  $\rho_t > 0$ for all $ t < \tau$, where $\tau  $ is any lifetime of \eqref{eq_sto_MCF}.
\end{corollary}
\begin{proof}
With similar computation as in the above lemma, and since $\rho_0 >0$, we have:
 ...
 \begin{equation} \left \{  \begin{array}{lcl}\label{eq_sto_rho}
  ...
  \rho_0 = \rho_0
 \end{array}  \right. \end{equation}
 and the proof is similar to the proof of \ref{eq_rho_theta}, \ref{th_cores}, \ref{conserve_convex}.  
  
\end{proof}

\begin{corollary}\label{cor2} If the starting curve is simple closed and strictly convex, there is a one to one correspondence  between the solutions of \eqref{eq_sto_MCF} and \eqref{eq_sto_rho}.
\end{corollary}
\begin{proof}
The proof is similar to that of Theorem \ref{th_cores}, essentially remove all the $h_t$.
\end{proof}

\section{Long time existence}
\subsection{Evolution of geometric quantities along the stochastic flow  \eqref{eq_sto}}

 \begin{proposition} \label{prop-ev} Let $C_0$ be a strictly convex curve.
 Let $C_t$  be the solution of \eqref{eq_sto}, $\lambda_t $ the volume of $D_t$ and  $\s_t $ the perimeter of $C_t$.  Then we have the followings equations for  $ t < \tau $ (where $\tau$ is any lifetime of \eqref{eq_sto} ):
\begin{enumerate}[i)]
...
\end{aligned} $$ 
 ...
 \begin{Remark}
 Note that   $\displaystyle \int_0^{2 \pi} \frac{1}{\rho^2_t}\, d\te - 2 \ld_t \ge  \frac{1}{2 \pi} \s^2_t - 2 \ld_t \ge 0 $ where we use isoperimetric estimate for the last point. Hence
...
\begin{lemma}\label{prop-h-sm}
 $ h_t$ is a positive super martingale, and so it is almost surely bounded for all $t < \tau$.
 \end{lemma} 
 \begin{proof}
 ...
 \end{proof}
 \par
 In the sequel we will encounter random constants, they will be denoted under the form $c(\omega)$, where $\omega$ stands for the randomness associated to underlying Brownian motions. This is a generic notation and the exact value of $c(\omega)$ may change from line to line.
 \par
  \begin{proposition}\label{min-rho}
  Let $C_0$ be a strictly convex curve.
 Let $C_t$  be the solution of \eqref{eq_sto}, and $\tau$ be any lifetime of \eqref{eq_sto}.    
 ...
 Suppose that  there exists $ t_0 \in [0, \tau[ $ and $\te_0 $ such that $ J_{t_0} (\te_0) = \alpha > 0 $.
 Let $W_t\df  e^{-t}J_t $, then $W_{t_0}(\te_0) = e^{-t_0} \alpha >0  $ and $\sup_{\te} W_{t_0}  \ge  e^{-t_0} \alpha >0  $. Consider the time $t_* = \inf\{t \in [0, t_0], s.t. \quad \sup_{\te} W_t  = W_{t_0}(\te_0) \}   $, and let $\te_*$ such that $W_{t_*}(\te_*) = \sup_{\te} W_{t_*}  $.
 We have  $ t_* >0$ and
 $$  \partial_t W_t \le \lt( \frac{1}{ e^t W_t(\te) +  \sqrt{2} B_t  + 2\int_0^t h(s) \,ds + \frac{1}{\inf \rho_0} }   \rt)^2 \partial^2_{\te} W_t  - W_t   . $$
 Note that since $ 0 \le \partial_t W_t(\te_*)_{\vert t_*}$,  $\partial^2_{\te} W_{t_*}(\te)_{\vert \te_*} \le 0 $ and $W_{t_*}(\te_*) = e^{-t_0} \alpha >0 $ we get a contradiction.
...
The goal of this section is to find a necessary condition on the strictly  convex domain to guarantee  the existence of the solution of \eqref{eq_sto} for all times. We will see that the entropy will be a supermartingale if the initial domain has enough symmetries. This condition seems to be a technical hypothesis and  we think that it is not necessary, and perhaps there exists an entropy which is more adapted to our situation. From Lemma \ref{eq_rho_theta}, we deduce  the evolution of the entropy  (defined in \eqref{Ent2}, it also coincides with  the relative entropy of the curvature density with respect to the arc length Lebesgue measure, up to normalizations in terms  of the length of the curve): 
 \bq
  d\Ent_t
&=& d \int_0^{2\pi} \log (\rho_t(\te)) \, d\te\\
& =& -\int_0^{2\pi}  (\partial_{\theta} \rho_t )^2\, d\te dt + 2 \int_0^{2\pi} \rho_t(\theta)   (\rho_t(\theta) - h ) \, d\te dt \\
...
\begin{definition}
We will say that a domain $D$ have  $ n $ axes of symmetries, if  up  to a translation there exists a linear straight line $\Delta$ such that $D$ is symmetric with respect to $\Delta,R_{\pi/n}(\Delta)...,R_{(n-1)\pi/n}(\Delta) $, where $ R_{\te}$ is a rotation of angle $\te$.
\end{definition}
 ...
 If the boundary of the initial domain is strictly convex (recall Definition~\ref{strictconv}), and the domain has  $ n $ axes of symmetries, with $n\ge 3$ then the entropy is a  
 super-martingale. 
 \end{proposition}
 \begin{proof}
...
\end{Remark}
 ...
 \begin{proposition} \label{p_4.10}If $D_0$ is strictly convex then:
...
This observation leads us to investigate the evolution of $\int\rho^4$ itself:
   \begin{proposition} If $D_0$ is strictly convex and  has $n$ axes of symmetries, with $n \ge 7 $ then 
...
\par
 ...
 If $D_0$ is strictly convex and  has $n$ axes of symmetries  with $n \ge 7 $, then there exists a finite random variable $ c(\omega)$ such that on  the event $\tau < \infty $:
...
\par
 \begin{proposition} \label{prop_rho_bound}
  If $D_0$ is strictly convex and  has $n$ axes of symmetries,  with $n \ge 7 $ then there exists a  random variable $ c(\omega)$ such that on  the event $\tau < \infty $:
 ...
 We will need the following lemma which is a small refinement of Lemma 4.1.1 from \cite{Gage_Hamilton}.
 
 \begin{lemma}\label{ident}
  Let a $2 \pi $ periodic positive function $\rho \in C^{\alpha }(\mathbb {T}) $, with $\alpha\in(0,1)$, be  such that
 \begin{equation}
 \int_0 ^{2\pi} \frac{\cos(\te)}{\rho(\te)} \,d\te = \int_0 ^{2\pi} \frac{\sin(\te)}{\rho(\te)} \,d\te = 0 .
 \end{equation}
...
with respect to the horizontal axis, and whose curvature function is $\rho$. When $X$ is parametrized by its arc-length, it becomes
 ...
 \end{lemma}
 \begin{proof}
 Note that under the parametrization of  $X $ by $\theta $ the curve seems to be only of order $C^{1 + \alpha}. $ Let us check it is in fact $ C^{2+\alpha}$ under the arc-length parametrization. 
...
Furthermore, in the parameter  $s$, the curve $ \tilde{X}(s) \df X(\te(s))$ satisfies $ \partial_s \tilde{X} =(\cos(\te(s)),\sin(\te(s)) $,   so we get that $\tilde{X}$ is $ C^{2+\alpha}$.
 ...
 \begin{thm}\label{infinite-lifetime}
  If $D_0$ is strictly convex and  has $n$ axes of symmetries,  and $n \ge 7 $ then a.s. $\tau = \infty $, where $\tau  $ is the maximal  lifetime of \eqref{eq_sto}. 
 \end{thm}
 \begin{proof}
...
$$  \int _0^{t_n} (-\partial_{\te}\rho_u(0), \rho_u(0)-2h_x ) \,du - (0, \sqrt{2} B_{t_n}  ) \to A .$$ 
 ...
 \begin{corollary} Let $D_0$ be strictly convex and  have $n$ axes of symmetries,  with $n \ge 7 $, and consider $(D_t)_{t\geq 0}$ the solution of \eqref{eq_sto}.
...
where $B(0,1/\sqrt{\pi})$ is the Euclidean  ball centered at 0 of radius $1/\sqrt{\pi}$.
 ...
 \section{Symmetric convex in $\mathbb{R}^2$ with star shaped skeletons}
\label{conv-sym}
Let $\mathcal{C}$ be the set of smooth closed simple and strictly convex curves embedded in $\mathbb{R}^2$. 

Fix $n\ge 2$.
Let $\mathcal{T}_n$ be the set of closed curves  symmetric with respect to the vertical axis, \LM{denoted}  $\Delta$, and invariant by the rotation $R_{2\pi/n}$ of angle $2\pi/n$ (and thus invariant by the group $G_n$ generated by these two isometries).\par
  
  Let us describe the set  $\mathcal{C}$ in terms of its curvature. Let $C_0 \in \mathcal{C} $, and let $ C : \TT \to \mathbb{R}^2$ be \LM{the} parametrization of $C_0$ such that  $\theta$ is the angle between the tangent line and the $x$ axis at the point $C(\theta)$ i.e a tangent vector is $ (\cos(\theta),\sin(\theta))\in T_{C(\theta)}C$. Note that this parametrization is possible since $ \partial_s \theta = \rho(\theta) >0$ where $s$ is the arc-length parametrization (due to Fr\'enet equation). From now on, we will take this parametrization for   curves in $\mathcal{C}$.

\LM{Recall from Lemma 4.1.1 of  Gage and Hamilton \cite{Gage_Hamilton}}
...
\eq
 ...
 Let us describe the set  $\mathcal{C} \cap \mathcal{T}_n  $ in terms of its curvature. Let $C \in \mathcal{C} \cap \mathcal{T}_n  $. \LM{For any $\theta\in\TT$,} denote $S_\theta$ the symmetry with respect to $R_\theta(\Delta)$.
 Using the symmetry $S_0$ we have $C(-\theta)=S_0(C(\theta))$ implying that $C(0)=S_0(C(0))$ and
\begin{equation}\label{b}
 C(0) = (0, -b)\quad\hbox{for some}\quad  b\ge 0.
\end{equation}
  Using the symmetry   $S_{\pi/n}\LM{=R_{2\pi/n}S_0}$ (\LM{thus belonging} to $G_n$) we have:
...
\end{equation}
 ...
 \end{equation}
   ...
   So $ \partial_{\theta}\rho\left(\frac{k\pi}{n}\right)  = 0 $ for all $k\in\{0,\ldots, \LM{2}n-1\}$.
 ...
 let $(0,\Pi(\theta) )$ be the intersection of the line $ D_{\te}$  orthogonal to $C$ at the point $ C(\theta)$ and the vertical axis~$\LM{\Delta}$. Define 
...
We are only left to prove the assertion for the map $r$.
 Putting the two parametrizations together, since $\langle C'(\te), N(\te) \rangle = 0$ and   $\langle C'(\te), T(\te) \rangle = \frac{1}{\rho(\te)}$, from \eqref{vrho}, we \LM{deduce from \eqref{Ctheta} that for any $\theta\in(0,\pi/n]$},
...
$$
contains $\bar B((0,\Pi(\theta)), r(\theta))$ (check for instance that the curvature of its boundary is everywhere smaller than $1/r(\theta)$). But $H(\theta)\subset \bar D$ since $\bar D$ is \LM{left invariant by $G_n$ and} convex. As a conclusion, $\bar B((0,\Pi(\theta)), r(\theta))\subset \bar D$, so $(0,\Pi(\theta))\in S$.
\end{itemize}
...
\LM{Consider $\theta'\in[0,2\pi)$ such that $C(\theta')$ minimizes the distance between $ P$ and $C$.
...
The set of curve $\mathcal{S}_n^\downarrow $ is stable under the stochastic mean curvature flow namely \eqref{eq_sto_MCF}. It is also stable under the usual deterministic mean curvature flow.
\end{proposition}
\begin{proof}
...
Let $C_t $ be the solution of the stochastic mean curvature started at $C_0$, namely the solution of:
 \begin{equation*} \left \{  \begin{array}{lcl}
 d_t C(t,u) & = & (-\rho_t(C(t,u)) )\nu_{C(t,u)}dt + \sqrt{2} \nu_{C(t,u)} dB_t  \\
  C(0,u) &= & C_0(u). \\
 \end{array}  \right. \end{equation*}
 ...
 \begin{equation*} \left \{  \begin{array}{lcl}
  ...
  \rho(0,\cdot) = \rho_0.
 \end{array}  \right. \end{equation*}
 \LM{(see \eqref{eq_rho} with $h$ replaced by 0).}
 
 Using Lemme \ref{conserve_convex} we get that $\rho_t>0$ for $ t < \tau $ where $\tau  $ is any lifetime of the stochastic mean curvature flow. Using It\^o formula, we have for $ 0 \le t < \tau  $
...
Using the uniqueness of the stochastic mean curvature flow, we have 
...
\end{equation}
\par
\LM{Define $\alpha\df\sup_{\te \in [0, \frac{\pi}{n}]} W_{t^{*}}(\te) >0 $,} \bq t_0&\df &\inf \lt\{ t\le t^{*},\,s.t. \, \sup_{\te \in [0, \frac{\pi}{n}]}W_{t}(\te) = \alpha \rt\} \eq
and let $ \te^{*}$ be such that $W_{t_0}(\te^{*}) = \alpha$. From boundary conditions we have  $ \te^{*} \in ] 0, \frac{\pi}{n} [$.
At $(t_0,\te^{*}) $ we have 
\begin{align*}
...
$$ 0 \le \partial_{t}W_t(\te^{*})_{\vert _{t_0}}  \le   \alpha \frac{\mu}{2} <0 .$$
With a similar proof, we get  the second part, namely the conservation of the class $\mathcal{S}_n^\downarrow $ under  the usual deterministic mean curvature flow.    
\end{proof}


\begin{corollary}
The class of domain $\mathcal{S}_n^\downarrow $ is also stable under the normalized stochastic mean curvature flow \eqref{eq_sto}.
\end{corollary}
\begin{proof}
Since the solutions of \eqref{eq_sto} are obtained by a change of probability from the solutions of the stochastic mean curvature flow, the state space does not change, and the result follows from Proposition \ref{prop_S}. 
\end{proof}
 
 \section{\LM{A} new isoperimetric estimate} 
 
\LM{ Let us end our consideration of $\mathcal{S}_n$ by observing that its elements are quite round when $n^2$ is much larger that the length of their skeleton:}
 \begin{proposition}\label{isoperimetric}
 For any curve $C$ in the set $\mathcal{S}_n $ defined in~\eqref{Sn} (and in particular with   skeleton $ G_n\left(\{0\}\times \left[ -{L(\LM{{C}})}/{n},0\right]\right) $)
we have
\begin{align*} \pi^2 (\rao -\rai)^2 &\le \sigma^2(C) - 4\pi \LM{\vol}(D)\\&
   \le\ \frac{2\pi^2 }{n^2}  L(\LM{{C}})^2 \left(1-\frac{\sin\left(\frac{2\pi}{n}\right)}{\frac{2\pi}{n}}\right)\\&
\le \frac{4\pi^4}{3n^4}L(\LM{{C}})^2 ,
\end{align*}
 where  $ L(\LM{{C}})$ is the length of the skeleton of $C$.
 \end{proposition}
 \begin{proof}
 \LM{The lower bound on $\sigma^2(C) - 4\pi \vol(D)$ is just Bonnesen inequality \eqref{Bonne}. For the upper bound, let}
...
$$ p'(\te) = \Pi(\te) \sin(\te)   $$ 
 ...
 Also $  \LM{\vol}(D) = \frac12 \int_0^{2\pi} p(\te) (p(\te) + p''(\te)) d\te =  \pi a_0^2 + \frac{\pi}{2} \sum_{k \ge 2} a^2_k (1-n^2k^2  )$
 and $a_0 =\frac{1}{2\pi} \int  p(\te) d\te = \frac{1}{2\pi} \sigma (C).$ 
 Hence
 $$\begin{aligned} \sigma^2(C) - 4\pi \LM{\vol}(D)  &= 2 \pi^2 \sum_{k\ge 1}a^2_k(n^2k^2 -1  )\\
 &\le 2 \pi \int_0^{2\pi} p'(\te)^2 d \te \\
 &= 4n\pi \int_0^{\pi/n} \Pi^2(\te) \sin^2(\te) d\te \\
 & \le 4n \pi \left( \frac{ L(\LM{{C}})}{n}\right)^2 \int_0^{\pi/n} \sin^2(\te) d\te \\
 &= \frac{2\pi^2 }{n^2}  L(\LM{{C}})^2 \left(1-\frac{\sin\left(\frac{2\pi}{n}\right)}{\frac{2\pi}{n}}\right)\\
&\le  \frac{4\pi^4}{3n^4}L(\LM{{C}})^2
 \end{aligned}$$
  \LM{since $1-\sin(x)/x\leq x^2/6$ for any $x\in \RR$ (with the usual convention $\sin(0)/0=1$).}
  \end{proof}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fig  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{figure}[h]
%\centerline{
...
\newblock Preprint on the open repository \textit{Hal}, December 2020.
