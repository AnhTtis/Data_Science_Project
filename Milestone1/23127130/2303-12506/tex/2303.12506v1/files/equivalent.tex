...
When two problems satisfy this relation, it is clear that in order to obtain an optimal \emph{value}, a solver for one can be used as a solver for the other. 
...
Moreover, when it comes to approximation, even if we are only concerned about the value, an approximate solver for one can no longer be used for the other.
...
\end{align*}    
Both problems have the same optimal value $1$.
Now, assume that a multiplicative error of $0.1$ is acceptable.
An approximate solver for the problem $(E1)$ may return the value $0.9$, which is not a possible value of $(E2)$; similarly, an approximate solver for the problem $(E2)$ may return the value $0.95$, which is not a possible value of $(E2)$.
...
In this appendix, we present a new definition of equivalent optimization problems, which requires a stronger relationship between them.
...
\paragraph{Equivalent problems definition} We say that two (single-objective) optimization problems, $OP1 = (S_1,f_1)$ and $OP2 = (S_2,f_2)$, are \emph{equivalent} if there exists a bijection, $B \colon S_1 \to S_2$, mapping each solution of $OP1$, $x \in S_1$, to a unique solution of $OP2$, $B(x) \in S_2$, and they have the same objective value $f_1(x) = f_2(B(x))$.



% a solver, either exact or approximate, for one can be used as a solver, with the same level of accuracy, for the other.
\begin{lemma}\label{lemma:solver-equivalent-prob}
    Let $OP1 = (S_1,f_1)$ and $OP2 = (S_2,f_2)$ be equivalent optimization problems. Then, an approximate solver for one can be used as an approximate solver for the other with the same level of accuracy.
...
\begin{proof}
    ...
    Let $B: S_1\to S_2$ be a bijection as in the definition of equivalent problems.    
    ...
    \eden{should say that we prove for maximization problem but the same applies for minimization}
...
For clarity, the problem description is provided here as well (table \ref{table:prob-des}).
...
\subsubsection{Equivalence of The Problems \eqref{eq:sums-OP} and \eqref{eq:compact-OP}}
we prove that the \emph{identity function} is an appropriate bijection between \eqref{eq:sums-OP} and \eqref{eq:compact-OP} and therefore, they are equivalent.
...
\begin{proof}
    ...
    To show that it also satisfies constraints $(\Tilde{2})$ and $(\Tilde{2})$ of \eqref{eq:compact-OP}, we start by proving that for any $\ell \in [n]$ and any constant $c \in \mathbb{R}$:
...
\begin{proof}
    ...
    This means that, given an exact solver to \eqref{eq:sums-OP}, the algorithm will output a leximin optimal solution.
    However, we saw that \eqref{eq:sums-OP} and \eqref{eq:compact-OP} are equivalent.
    Therefore, a solver to \eqref{eq:sums-OP} can be used to obtain the same result.
\end{proof}

\eden{in the next version we can prove it in a more interesting way.. that when the constants $z_1, \ldots, z_{t-1}$ represent the optimal values of \eqref{eq:basic-OP} the program are equivalent}