...
the leximin criterium is usually mentioned in the context of fairness,
...
A common algorithm,
independently proposed many times, e.g.~\cite{airiau_portioning_2019,nace_max-min_2008,Ogryczak2004TelecommunicationsND, willson}, is based on the notion of \textit{saturation}, operates roughly  as follows.
...
One could perhaps define an objective as ''saturated'' if its maximum attainable value is close to the maximum $z_t$, but there is no guarantee that this would lead to a good approximation\footnote{An example will be given on request.}\eden{ (for more details see Appendix \ref{}\fullVer)}.
...
Lastly, we consider a more challenging case. 
We apply our results to the problem of \emph{stochastic allocations of indivisible goods}. 
