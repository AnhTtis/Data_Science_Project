...
Second, dividing all the constraints by $z_t$ (notice that $z_t > 0$ since $z_t \geq z_1$ for any $t \geq 1$ and  $z_1 >0$).
\eden{to myself: maybe to explain why $z_1>0$}
And third, replacing all the original variables and constants, except $z_t$, with new variables that are smaller by a factor $z_t$ (that is, $p'_A = p_d(A)/z_t$ for all $A \in \mathcal{A}$, $,y'_{\ell} = y_{\ell}/z_t,m'_{\ell,j} = m_{\ell,j}/z_t$ for $\ell \in [t]$ and $ j\in [n]$,  and $z'_i = z_i/z_t$ for $i \in [t-1]$). Notice that the $z'_i$'s are variables (while $z_i$ for $1\leq i\leq t-1$ are constants).
...
It can be easily verified that $p'_A = p_d(A)/z_t$ for $A \in \mathcal{A}$, $z_t$, $y'_{\ell} = y_{\ell}/z_t$ and $m'_{\ell,j} = m_{\ell,j}/z_t$ for all $\ell \in [t]$ and $ j\in [n]$, and $z'_i = z_i/z_t$ for $i \in [t-1]$ is a feasible solution to the program \eqref{eq:app-ver2-vsums-OP} with objective value $1/V$.

In addition, let $p'_A$ for $A \in \mathcal{A}$, $z_t$, $y'_{\ell}$ and $m'_{\ell,j} = $ for all $\ell \in [t]$ and $ j\in [n]$, and $z'_i$ for $i \in [t-1]$ be a feasible solution to the program \eqref{eq:app-ver2-vsums-OP} with objective value $1/V$.
It can also be easily verified that $ = p_d(A) = p'_A\cdot z_t$ for $A \in \mathcal{A}$, $z_t$, and $ y_{\ell} = y'_{\ell}\cdot z_t$ and $ m_{\ell,j} =m'_{\ell,j}\cdot z_t$ for all $\ell \in [t]$ and $ j\in [n]$ is a feasible solution to the program \eqref{eq:app-vsums-OP} with objective value $V$.
\end{proof}
...
Notice that the variable $z_t$ appears only in the objective function of \eqref{eq:app-ver2-vsums-OP}, $\min 1/z_t$, and in constraint (\progAppSecond.1.1), which says that $\sum_{A \in \mathcal{A}}p'_A = 1/z_t$.
