...
Specifically, we prove that a $\frac{1}{3}$-approximation\footnote{This section discusses only multiplicative approximations; so, for brevity, we use the term "$\multApprox$-approximation" to refer to "$(\multApprox,0)$-approximation".} can be obtained deterministically, whereas a $\frac{(e-1)^2}{e^2-e+1} \approx 0.52$-approximation can be obtained w.h.p.
As a reference point, it is worth noting that the problem of maximizing the egalitarian welfare in the same settings has been shown to be NP-hard to approximate to a (multiplicative) factor better than $1-\frac{1}{e} \approx 0.632$~\cite{kawase_max-min_2020}.
...
However, as an $\multApprox$-approximation to leximin is first and foremost an $\multApprox$-approximation to the egalitarian welfare\displayEcai{\footnote{See Observation \ref{obs:rppeox-lex-generalizes-approx-egal}.}}, the same hardness result applies to our problem as well.
...
\begin{theorem}
\label{th:app-main}
Given a (randomized) algorithm that returns a simple allocation that $\multApprox$-approximates the utilitarian welfare (with success probability $p$).
Then, Algorithm \ref{alg:basic-ordered-Outcomes} can be used to obtain a $ \frac{\multApprox^2}{1-\multApprox +\multApprox^2}$-leximin-approximation for the problem of stochastic allocations (with the same probability).
% \eden{previous version: Then, Algorithm \ref{alg:basic-ordered-Outcomes} can be used to obtain a stochastic allocation that approximates leximin with a multiplicative error of at most $\frac{\multError}{1-\multError +\multError^2}$ (with the same probability).}
\end{theorem}

A complete proof is given in Appendix \appendixName{\ref{sec:app-sec-proofs}}{C}\fullVer.
...
Theorem \ref{th:app-main} yields two immediate corollaries, using  known algorithms to approximate the utilitarian welfare when the agents' utility functions are monotone and submodular.

First, there are deterministic $\frac{1}{2}$-approximation algorithms~\cite{Buchbinder2019, Fisher1978}, and therefore:
\begin{corollary}
    Algorithm \ref{alg:basic-ordered-Outcomes} can be used to obtain a $\frac{0.5^2}{1-0.5 +0.5^2} = \frac{1}{3}$-leximin-approximation for the problem of stochastic allocations.
    % \eden{previous version: a stochastic allocation that approximates leximin with a multiplicative error at most     $\frac{0.5}{1-0.5+0.5^2} =     \frac{2}{3}$.}
\end{corollary}
\noindent Second, there is a randomized $(1-\frac{1}{e})$-approximation algorithm w.h.p~\cite{vondrak_optimal_2008}, and therefore:
\begin{corollary}
    Algorithm \ref{alg:basic-ordered-Outcomes} can be used to obtain $\frac{(1-1/e)^2}{1-(1-1/e) +(1-1/e)^2} = \frac{(e-1)^2}{e^2-e+1 }\approx 0.52$-leximin-approximation for the problem of stochastic allocations w.h.p.
    % \eden{previous version:     a stochastic allocation that approximates leximin with a multiplicative error at most $\frac{e}{e^2-e+1} \approx 0.48$ w.h.p.}
\end{corollary}

\eden{I couldn't find the reference to the thesis where the result for gross-substitutes utilities is proven, so I gave up on presenting this result}
% It should also be noted that when the utilities are gross-substitutes, there exists an exact polynomial-time algorithm for calculating utilitarian welfare~\cite{bertelsen2005substitutes,kawase_max-min_2020}. Together with Theorem \ref{th:main}, this implies that Algorithm \ref{alg:basic-ordered-Outcomes} can be used to find an exact leximin-optimal solution to the problem of stochastic allocation in this case.

% The proof of Theorem \ref{th:app-main} is provided in Appendix \ref{sec:app-sec-proofs}. 


\eden{should we say that in the case of additive and gross-substitute utilities this means that a leximin optimal solution can be found?}

% \eden{I'm not sure "with success probability" or "with \emph{a} success probability"?}
