...
\begin{lemma}\label{lemma:beta-vk}
    ...
    \begin{align*}
        \beta \valBy{k}{\retSol} \geq \biggl(\sum_{i=1}^k \valBy{i}{\retSol} - \sum_{i=1}^k z_i\biggl) -\beta \biggl(\sum_{i=1}^{k-1} \valBy{i}{\retSol} - \sum_{i=1}^{k-1} z_i\biggl)
    \end{align*}
\end{lemma}
...
\begin{proof}
    ...
    \begin{align*}
         &\sum_{i=1}^k \valBy{i}{\retSol} - \sum_{i=1}^{k-1} z_i \leq \frac{1}{1-\beta} z_k\\
...
\begin{lemma}\label{lemma:zk-to-beta-sums}
    Let $1 \leq k \leq n$. Then, $z_k \leq \sum_{i=1}^k \beta^{i-1} \valBy{k-i+1}{\retSol}$.
\end{lemma}

\begin{proof}
    Assume by contradiction that this is not true.
    This allows us to conclude that $\sum_{i=1}^k \valBy{i}{\retSol} < \sum_{i=1}^k z_i$:
    \begin{align*}
        &z_k > \sum_{i=1}^k \beta^{i-1} \valBy{k-i+1}{\retSol} = \valBy{k}{\retSol} +  \sum_{i=2}^k \beta^{i-1} \valBy{k-i+1}{\retSol}\\
        & = \valBy{k}{\retSol} +  \sum_{i=1}^{k-1} \beta^{i} \valBy{k-(i+1)+1}{\retSol} = \valBy{k}{\retSol} +  \sum_{i=1}^{k-1} \beta^{i} \valBy{(k-1)-i+1}{\retSol}\\
        &\geq \valBy{k}{\retSol} + \sum_{i=1}^{k-1} \valBy{i}{\retSol} - \sum_{i=1}^{k-1} z_i && \text{Lemma \ref{lemma:beta-sums}}
    \end{align*}
    However, the opposite, $\sum_{i=1}^k \valBy{i}{\retSol} \geq \sum_{i=1}^k z_i$, is a constraint of \eqref{eq:compact-OP} that was solved in iteration $n$ (when $\retSol$ was obtained) --- a contradiction.
\end{proof}

%---- part 2
\begin{lemma}\label{lemma:fk-to-zk-zk-1}
    Let $1 < k \leq n$. Then, $\valBy{k}{\retSol} \geq z_k - \frac{\beta}{1-\beta} z_{k-1}$.
\end{lemma}

\begin{proof}
    Since $\retSol$ was obtained in \eqref{eq:compact-OP} that was solved in the last iteration $n$, $\sum_{i=1}^k \valBy{i}{\retSol} \geq \sum_{i=1}^{k} z_i$ (by constraint $(\Tilde{2})$ if $k<n$ and $(\Tilde{3})$ otherwise).
    Accordingly, by isolating $\valBy{k}{\retSol}$, the claim can be concluded:
    \begin{align*}
        ...
        & \geq z_k + z_{k-1} - \frac{1}{1-\beta}z_{k-1}  && \text{Observation \eqref{obs:obj-xt-to-zt} for $(k-1)$}
        \\
...
\begin{lemma}\label{lemma:new-lower-bound}
    Let $1 < k \leq n$. Then:
    \begin{align*}
        z_k \leq \biggl(1 + \frac{\beta(1-\beta^k)}{(1-\beta)^2}\biggl)\valBy{k}{\retSol}
    \end{align*}
\end{lemma}

\begin{proof}
    First, notice that since $(k-1)-i+1 \leq k$ for any $1\leq i \leq k$ and as the function $\valBy{i}$ represents the $i$-th smallest objective value, also:
    \begin{align}\label{eq:increase-by-obj-size}
        \forall 1\leq i \leq k \colon \quad \valBy{(k-1)-i+1}{\retSol} \leq \valBy{k}{\retSol}
    \end{align}.
    ...
    \begin{align*}
        & z_k \leq \valBy{k}{\retSol} + \frac{\beta}{1-\beta} z_{k-1} && \text{Lemma \ref{lemma:fk-to-zk-zk-1}}\\
        & \leq \valBy{k}{\retSol} +  \frac{\beta}{1-\beta} \sum_{i=1}^{k-1} \beta^{i-1} \valBy{(k-1)-i+1}{\retSol} && \text{Lemma \ref{lemma:zk-to-beta-sums} (for $k-1$)}\\
        & \leq \valBy{k}{\retSol} +  \frac{\beta}{1-\beta} \sum_{i=1}^{k-1} \beta^{i-1} \cdot \valBy{k}{\retSol}  && \text{equation \ref{eq:increase-by-obj-size}}\\
        & \leq \valBy{k}{\retSol} +  \frac{\beta}{1-\beta} \frac{1-\beta^k}{1-\beta} \valBy{k}{\retSol} && \text{equation \ref{eq:geometric-series-beta}} \\
        & \leq \biggl(1 + \frac{\beta(1-\beta^k)}{(1-\beta)^2}\biggl) \valBy{k}{\retSol}
    \end{align*}
\end{proof}



\begin{lemma}\label{lemma:new-fk-lower-bound}
    Let $1 < k \leq n$. Then:
    \begin{align*}
        z_k \leq \valBy{k}{\retSol} +  \frac{\beta}{1-\beta} \frac{1-\beta^k}{1-\beta} \valBy{k}{\retSol}
    \end{align*}
    \begin{align*}
        z_k \leq \biggl(1 + \frac{\beta(1-\beta^k)}{(1-\beta)^2}\biggl)\valBy{k}{\retSol}
    \end{align*}
...
Now, suppose for contradiction that $\retSol$ is not a $(1 - \mu \beta)$-approximate Leximin solution.
    ...
    \begin{align*}
        ...
        &\geq \sum_{i=1}^{k-1} \valBy{i}{\retSol} - \sum_{i=1}^{k-1} z_i + \frac{1}{1-\beta} \valBy{k}{\retSol} + \frac{\beta^2}{1-\beta}\frac{1-\beta^k}{1-\beta} \valBy{k}{\retSol}\\
        &\geq \sum_{i=1}^{k-1} \valBy{i}{y} - \sum_{i=1}^{k-1} z_i + \frac{1}{1-\beta} \valBy{k}{\retSol} + \frac{\beta}{1-\beta}\biggl(\sum_{i=1}^{k-1} \beta^{i} \valBy{(k-1)-i+1}{\retSol}\biggl) && \text{Lemma \ref{lemma:beta-sums-to-diff}}\\
        &\geq \sum_{i=1}^{k-1} \valBy{i}{\retSol} - \sum_{i=1}^{k-1} z_i + \frac{1}{1-\beta} \valBy{k}{\retSol} + \frac{\beta}{1-\beta}\biggl(\sum_{i=1}^{k-1}\valBy{i}{\retSol} - \sum_{i=1}^{k-1}z_i\biggl)\\
        &=\sum_{i=1}^{k-1} \valBy{i}{\retSol} - \sum_{i=1}^{k-1} z_i + \frac{1}{1-\beta} \valBy{k}{\retSol} + \frac{\beta}{1-\beta}\sum_{i=1}^{k-1}\valBy{i}{\retSol} - \frac{\beta}{1-\beta}\sum_{i=1}^{k-1}z_i\\
        & > \frac{1}{1-\beta} \biggr[\sum_{i=1}^k \valBy{k}{\retSol} - \sum_{i=1}^{k-1}z_i\biggr]
    \end{align*}
\end{proof}
\iffalse
