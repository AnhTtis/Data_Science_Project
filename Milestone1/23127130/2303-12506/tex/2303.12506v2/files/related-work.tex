...
Another approach to leximin optimization is to
%approximate the optimality of a solution with a single number, that is, 
find an aggregation function, which takes a utility vector and returns a number such that a solution is leximin-preferred over another if and only if its aggregate number is higher.
