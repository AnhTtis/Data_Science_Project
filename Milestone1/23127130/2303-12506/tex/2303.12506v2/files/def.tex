...
\eden{to fix "strict" also in the appendix and footnote}
The first step is defining the following \textit{partial} order\footnote{A proof that the approximate leximin order is a strict partial order can be found in appendix \ref{sec:approx-order-is-strict-partial}.}:
%(a partial order allows two solutions with different utilities such that no one is preferred over the other).:
a solution $y$ is \emph{$(\DEFmultApprox,\DEFadditiveApprox)$-leximin-preferred} over a solution $x$, denoted $y \alphaBetaPreferred x$, if there exists an integer $1 \leq k \leq n$ such that the smallest $(k-1)$ objective values of $y$ are \emph{at least} those of $x$, and the $k$'th smallest objective value of $y$ is $(\DEFmultApprox,\DEFadditiveApprox)$-preferred over the $k$'th smallest objective value of $x$, that is:
\begin{align*}
...
% \eden{to do: to try to shorten it}
\paragraph{Example } To illustrate, consider a group of $3$ agents, that has to select one out of three options $x,y,z$, with utility
multisets $\allValues{x}=\{1,10,15\}, \allValues{y} =\{1,40,60\}, \allValues{z}=\{2,20,30\}$.
...
We say that a solution $x\in S$ is \emph{$(\DEFmultApprox,\DEFadditiveError)$-approximately leximin-optimal} if it is a maximum element of the order $\alphaBetaPreferred$. \displayEcai{ in $S$ for $\DEFmultApprox\in (0,1]$ and $\DEFadditiveError \geq 0$.
...
\displayComsoc{
\eden{Here, I removed the second table and change the explenation a bit.}
...
% (more solution are considered approximately optimal).
