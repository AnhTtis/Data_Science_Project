...
Lenstra~\cite{IPinFixedDim-Lenstra1983} used the then-recent lattice basis reduction algorithm to solve any $n$-variable integer program in time $2^{O(n^2)}$. This was later improved by Kannan~\cite{n-to-n-algos-for-SVP-CVP-Kannan-MOR1987} to $n^{O(n)}$ and then by Dadush~\cite{DadushThesis2012} and by Eisenbrand et al~\cite{FromApxToExactIP_DER_Arxiv2022} to $2^{O(n)}n^n$.
...
\begin{theorem} \label{thm:FindingSubspaceIn2ToN} %\rem{T: I think it should be $O(\log^8(n))$ here..?}
  ...
  \[
    \mu(\Lambda,K) \leq O(\log^4 (n)) \cdot \Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(K))}\Big)^{1/d}.
  \]
...
For any full rank lattice $\Lambda \subseteq \setR^n$ and any convex body $K \subseteq \setR^n$ one has \[\mu(\Lambda,K-K) \leq \mu(\Lambda,K) \leq O(\log^3(n)) \cdot \mu(\Lambda,K-K).\]
\end{theorem}
This in turn implies that the \emph{flatness constant} in dimension $n$ is bounded by $O(n\log^{4}(n))$,
...
for any convex body $K \subseteq \setR^n$ with $K \cap \setZ^n = \emptyset$, there is a direction $c \in \setZ^n \setminus \{ \bm{0}\}$ so that at most $O(n \log^{4}(n))$ many hyperplanes of the form $\langle c, x \rangle = \delta$ with $\delta \in \setZ$ intersect $K$. More generally, the following holds:
\begin{theorem} \label{thm:FlatnessConstant}
  ...
  \[
   \mu(\Lambda,K) \cdot \lambda_1(\Lambda^{*}, (K-K)^{\circ}) \leq O(n \log^{4}(n)).
  \]
\end{theorem}
...
\noindent {\bf Claim II.} \emph{For any $d$-dimensional subspace $W \subseteq \setR^n$ one has
  \[(\Vol_d(\Pi_W(K)))^{1/d} \lesssim \Big(\frac{n}{d}\Big)^{3} \cdot \frac{\ell_{K_{\mathrm{sym}}^{\circ}}}{d}.\]
  }
\noindent {\bf Proof of Claim II.} Using the volume estimate from Proposition~\ref{prop:VolumeOfProjectionVsSymmetrizer} with the assumption that the barycenter of $K^{\circ}$ lies at the origin, we obtain
  \begin{eqnarray*}
    (\Vol_d(\Pi_W(K)))^{1/d} &\stackrel{\textrm{Prop~\ref{prop:VolumeOfProjectionVsSymmetrizer}}}{\lesssim}& \Big(\frac{n}{d}\Big)^3 \cdot (\Vol_d(\Pi_W(K_{\textrm{sym}})))^{1/d} \\ &\stackrel{\textrm{Thm~\ref{thm:UrysohnInequality}}}{\lesssim}& \Big(\frac{n}{d}\Big)^3 \cdot \frac{w(\Pi_W(K_{\mathrm{sym}}))}{\sqrt{d}} \\
    &\stackrel{\textrm{Lem~\ref{lem:PropertiesOfPolarity}}}{\lesssim}& \Big(\frac{n}{d}\Big)^{3} \cdot \frac{\ell_{K_{\textrm{sym}}^{\circ} \cap W}}{d} \\ &\stackrel{\textrm{Lem~\ref{lem:MonotonicityLValue}}}{\leq}& \Big(\frac{n}{d}\Big)^{3} \cdot \frac{\ell_{K_{\textrm{sym}}^{\circ}}}{d}.
  \end{eqnarray*}
  Here we used the Urysohn inequality to relate volume to width as well as the fact that $\Vol_d(B_2^d)^{1/d} = \Theta(\frac{1}{\sqrt{d}})$ (see e.g. \cite{AsymptoticGeometricAnalysis-Book2015}) and $\Pi_W(K_{\textrm{sym}})^{\circ} = K_{\textrm{sym}}^{\circ} \cap W$. \qed

  In the following we abbreviate $d_{\geq i} := \sum_{j=i}^k d_j$. \\
\noindent {\bf Claim III.} \emph{For any $i \in \{ 1,\ldots,k\}$ one has $r_i \lesssim (\frac{n}{d_{\geq i}})^3 \cdot \frac{\ell_{K_{\mathrm{sym}}^{\circ}}}{d_{\geq i}} \cdot \mu_{KL}(\Lambda,K)$.} \\
\noindent {\bf Proof of Claim III.}  Let $W := \textrm{span}(\Lambda_{i-1})^{\perp}$ and let $d := \dim(W) = \rank(\Lambda/\Lambda_{i-1}) = d_{\geq i}$. Then  using that $r_i \leq r_{i+1} \leq \ldots \leq r_k$ one has
  \begin{eqnarray*}
   r_i &\leq& \Big( \prod_{j = i}^k r_j^{d_j}\Big)^{1/\sum_{j = i}^k d_j} \\
    &=& \det(\Lambda/ \Lambda_{i-1})^{1/d} \\
                  &=&  (\Vol_d(\Pi_W(K)))^{1/d} \cdot \underbrace{\Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(K))}\Big)^{1/d}}_{\leq \mu_{KL}(\Lambda,K)} \\
    &\stackrel{\textrm{Claim II}}{\lesssim}& \left(\frac{n}{d}\right)^{3} \cdot \frac{\ell_{K_{\textrm{sym}}^{\circ}}}{d} \cdot \mu_{KL}(\Lambda,K). \qed
 % &=& \left(\frac{n}{\sum_{j = i}^k d_j}\right)^{3} \cdot \frac{\ell_{K_{\textrm{sym}}^{\circ}}}{\sum_{j = i}^k d_j} \cdot \mu_{KL}(\Lambda,K). \qed
  \end{eqnarray*}
 
%Assuming these two claims we can prove the  main claim. %proposition:
%\begin{proof}[Proof of Prop~\ref{prop:MainArgument}]
  Now we approach the main claim. First we have
  \begin{eqnarray*}
  ...
  \end{eqnarray*}
  as $r_1 \leq \ldots \leq r_{i^*}$ and $r_{i} \leq \frac{1}{2}r_{i+2}$ for all $i$. Thus we conclude
 \begin{eqnarray*}
  \mu(\Lambda \cap U, K_{\textrm{sym}} \cap U) &\lesssim& \log(n) \cdot \ell_{K_{\textrm{sym}}} \cdot r_{i^*} \\
                                      &\stackrel{\textrm{Claim III}}{\lesssim}& \log(n) \cdot \underbrace{\ell_{K_{\textrm{sym}}} \cdot \ell_{K_{\textrm{sym}}^{\circ}}}_{\lesssim n \log(n)} \cdot \Big(\frac{n}{d_{\geq i^*}}\Big)^3 \cdot \frac{1}{d_{\geq i^*}} \cdot \mu_{KL}(\Lambda,K) \\
                                               &\lesssim& \log^2(n) \cdot \Big(\frac{n}{d_{\geq i^*}}\Big)^4 \cdot \mu_{KL}(\Lambda,K) \\
  &\stackrel{(*)}{\lesssim}& \log^2(n) \cdot \mu_{KL}(\Lambda,K).
\end{eqnarray*}
In $(*)$ we use that $i^*$ is the \emph{minimal} index with $\textrm{rank}(\Lambda_{i^*}) \geq \frac{n}{2}$, so $d_{\geq i^*} = \sum_{j=i^*}^k d_j \geq \frac{n}{2}$.
\end{proof}
...
\begin{proof}[Proof of Theorem~\ref{thm:KLConj}]
  ...
  \[
  \mu(\Lambda,K) \leq C_0\log^3(n) \cdot \mu_{KL}(\Lambda,K),
\]
...
%  Following Proposition~\ref{prop:MainArgument}, consider the 2-approximate canonical filtration $\{ \bm{0} \} = \Lambda_{0} \subset \ldots \subset \Lambda_k = \Lambda$
  ...
  \begin{eqnarray*}
    ...
    &\stackrel{K \supseteq K_{\textrm{sym}}}{\leq}& \mu(\Lambda \cap W,K_{\textrm{sym}} \cap W) + \mu(\Pi_{W^{\perp}}(\Lambda),\Pi_{W^{\perp}}(K)) \\
                   &\stackrel{\textrm{Prop~\ref{prop:MainArgument}}+\textrm{induction}}{\leq}& C_0\log^2(n) \cdot \mu_{KL}(\Lambda,K) + C_0\log^3(\underbrace{\dim(W^{\perp})}_{\leq n/2}) \cdot \underbrace{\mu_{KL}(\Pi_{W^{\perp}}(\Lambda),\Pi_{W^{\perp}}(K))}_{\leq \mu_{KL}(\Lambda,K)} \\
    &\stackrel{\textrm{Lem~\ref{lem:MonotonicityOfMuKL}}}{\leq}& C_0\underbrace{\log^2(n) \cdot \Big(1+\log\Big(\frac{n}{2}\Big)\Big)}_{=\log^3(n)} \cdot \mu_{KL}(\Lambda,K). \qedhere
  \end{eqnarray*}
\end{proof}
...
\[\mu(\Lambda, K) \lesssim \log^4 (n) \cdot \Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(K))}\Big)^{1/d}. \]
\end{theorem}
...
\item[(10)] Return $W := T^{-1} W'$ where $W' := \underset{W \in \{W_{i^*}, W_\Pi\}}{\mathrm{argmin}} \Big\{ \frac{\det(\Pi_{W}(\Lambda'))}{\Vol_{\dim(W)} (\Pi_{W} (K'))} \Big\}$.
\end{enumerate*}
\end{minipage}}
\end{center}
...
a factor $\rho(n) := \Theta(\log^{4} (n))$. %\rem{T: Changed to $\rho = \Theta(\log^8(n))$.}
...
\end{proof}
We are now ready to state the complete algorithm. As mentioned earlier, we denote $\rho(n) := \Theta(\log^4(n))$ % \rem{T: Check $\rho(n)$!}
...
Let $W$ denote the subspace attaining $\mu_{KL}(\Lambda, K)$ with $\dim W = d$. We can use Theorem~\ref{thm:KLConj} to upper bound 
 \begin{eqnarray*}
\mu(\Lambda, K) \leq O(\log^{3} (n)) \cdot \mu_{KL} (\Lambda, K) & = & O(\log^{3} (n)) \cdot \Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(K))}\Big)^{1/d} \\ & \stackrel{\textrm{Lem~\ref{lem:RSineq}}}{\le} & O(\log^{3} (n)) \cdot 4 \cdot \Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(K-K))}\Big)^{1/d} \\ & \le & O(\log^{3} (n)) \cdot \mu_{KL}(\Lambda, K-K)\\ & \le & O(\log^{3} (n)) \cdot \mu(\Lambda, K-K). \qedhere
 \end{eqnarray*}
\end{proof}
...
\begin{proof}
  ...
  $\mu(\Lambda,K) \cdot \lambda_1(\Lambda^{*}, (K-K)^{\circ}) \leq O(\log^3 n) \cdot \mu(\Lambda,K-K) \cdot \lambda_1(\Lambda^{*}, (K-K)^{\circ}) \leq O(n\log^{4}(n))$ by Theorem~\ref{thm:CoveringRadiusKvsKminusK}.
