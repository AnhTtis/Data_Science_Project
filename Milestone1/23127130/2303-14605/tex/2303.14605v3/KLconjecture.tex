...
$w(T(Q)) \cdot w(T(Q)^{\circ}) \leq O(\log(2n))$. Moreover one can prove that for any symmetric convex body $Q$ one has $w(Q) \cdot w(Q^{\circ}) \gtrsim w(B_2^n)^2 \gtrsim 1$. Then one can interpret Theorem~\ref{thm:PisierRescaling} that
every symmetric convex body can be linearly transformed so that in terms of mean width and
...
\[
 \mu(\Lambda,K) \stackrel{K \supseteq K_{\textrm{sym}}}{\leq} \mu(\Lambda,K_{\textrm{sym}}) \stackrel{\textrm{Lem~\ref{lem:TriangleIneqForFiltration}}}{\leq} \sum_{i=1}^k \mu(\Lambda_{i}/\Lambda_{i-1},K_i) \stackrel{\textrm{Prop~\ref{prop:CovRadiusOf2StableLattice}}}{\lesssim} \log(2n) \sum_{i=1}^k r_i \ell_{K_i} \leq \log(2n) \cdot r_k \ell_K
\]
...
set $W := \textrm{span}(\Lambda')$. Then
  \begin{eqnarray*}
    ...
    &\stackrel{\textrm{Lem~\ref{lem:MonotonicityOfMuKL}}}{\leq}& C_0\underbrace{\log^2(2n) \cdot \Big(1+\log(2n)\Big)}_{=\log^3(2n)} \cdot \mu_{KL}(\Lambda,K). \qedhere
  \end{eqnarray*}
\end{proof}
...
\begin{proof}
 ...
 \begin{eqnarray*}
\mu(\Lambda, K) \lesssim \log^{3} (2n) \cdot \mu_{KL} (\Lambda, K) & = & \log^{3} (2n) \cdot \Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(K))}\Big)^{1/d} \\ & \stackrel{\textrm{Lem~\ref{lem:RSineq}}}{\lesssim} & \log^{3} (2n) \cdot 4 \cdot \Big(\frac{\det(\Pi_W(\Lambda))}{\Vol_d(\Pi_W(Q))}\Big)^{1/d} \\ & \stackrel{\textrm{Lem~\ref{thm:BSBM}}}{\asymp} & \log^{3} (2n) \cdot \Big(\frac{d \cdot \Vol_d(Q^\circ \cap W)}{\det(\Lambda^* \cap W)}\Big)^{1/d} \\ & \stackrel{\textrm{Thm~\ref{thm:Minkowski}}}{\lesssim} & n\log^{3} (2n) \cdot \frac{2}{\lambda_1(\Lambda^* \cap W, Q^\circ \cap W)}.
 \end{eqnarray*}
