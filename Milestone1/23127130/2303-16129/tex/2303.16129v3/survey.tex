...
GANs is a fundamental framework for AIGC, comprising a generative model and a discriminative model~\cite{goodfellow2020generative}. The generative network aims to generate data that is as realistic and similar to the original data as possible to deceive the discriminative model based on the data in the original dataset. Conversely, the discriminant model's task is to differentiate between real and fake instances. During the GAN training process, the two networks continually enhance their performance by competing against each other until they reach a stable equilibrium. 
...
This section introduces creative applications and advantages of AIGC services in the mobile edge network. Then, we provide four use cases of AIGC applications at mobile AIGC networks. Some examples of {\color{black} generative AI models} are shown in Fig.~\ref{fig:pumpkin}. The applications elaborated in this section are summarized in Table \ref{table:applications}.
...
\centering
    {\color{black} {\small \begin{tabular}{m{1.6cm}|m{8cm}|m{7cm}}
        ...
        \hline
        \cite{du2023enabling} & Large-scale deployment of AaaS with 20 AIGC service providers (ASPs) and 1000 edge users & Deep reinforcement learning (DRL)-enabled solution to maximize utility function \\
        \hline
        \cite{du2023ai} & Users sharing information through full-duplex device-to-device semantic communications. & AI-generated incentive mechanism to maximize the users' utilities \\
        \hline
        \cite{liu2023blockchain} & AIGC lifecycle management framework with three ESPs and three producers, supported by Draw Things application & Blockchain technology to protect the ownership and copyright of AIGC, along with a reputation-based service provider selection strategy \\
        \hline
        ...
        \hline
        \cite{du2023exploring} & Distributed diffusion model where user 1 transmits the results after the shared denoising step to user 2. & A collaborative distributed diffusion-based AIGC framework\\
        \hline
        \cite{du2023spear} & A user requests a specific number of images from a service provider that is attacked by data poisoning, while diffusion models provide the defense. & Generative AI-aided optimization to identify the optimal diffusion steps to minimize the total energy cost \\
        \hline
        \cite{du2023generative} & Selection of AIGC service providers (ASPs) capable of effectively executing user tasks. & AI-generated optimal decision algorithm to generate the optimal ASP selection decisions \\
        \hline
        ...
        \hline
        \cite{lin2023unified} & An integrated semantic communication and AIGC framework for Metaverse services. & Diffusion model-based joint resource allocation \\
        \hline
        \cite{du2023yolo} & A semantic communication framework based on You Only Look Once (YOLO) to construct a virtual apple orchard. & Semantic communications with generative AI-aided resource optimization \\
        \hline
        \cite{xu2023joint} & A foundation model caching and inference framework to balance the tradeoff among inference latency, accuracy, and resource consumption. & Manage cached foundation models and user requests during the provisioning of generative AI services \\
        \hline
        ...
        \hline
        \cite{xu2023generative} & An autonomous driving architecture, where generative AI is leveraged to synthesize conditioned traffic and driving data in simulations. & A multi-task digital twin offloading model and  a multi-task enhanced auction-based mechanism \\
        \hline
    \bottomrule[1pt]
    \end{tabular}}}
...
With the advancement of blockchain technologies \cite{li2020blockchain}, decentralized mobile AIGC networks can be realized based on distributed data storage, the convergence of computing and networking, and proof-of-ownership of data~\cite{xu2022quantum}. Such a decentralized network structure, enabled by digital identities and smart contracts, can protect AIGC users' privacy and data security. Furthermore, based on blockchain technologies, mobile AIGC networks can achieve decentralized management of the entire lifecycle of AIGC services. Therefore, future research should investigate specific consensus mechanisms~\cite{li2020blockchain, liu2021proof}, off-chain storage systems, and token structures for the deployment of decentralized mobile AIGC networks.
