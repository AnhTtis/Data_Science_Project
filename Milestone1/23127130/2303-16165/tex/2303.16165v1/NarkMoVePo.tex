...
%\usepackage{dsfont}     % to get mathds for cool R, I and C
...
%\usepackage{amsmath,amssymb,url,color,amsfonts}
\usepackage{amsthm}
\usepackage{epsfig}
\usepackage{cite}
\usepackage{graphicx}
...
This paper proposes, and rigorously analyzes, a \emph{modular} framework, within which established planning and control design methods can be integrated in a \emph{provably} stable way to ensure that the suggested plan is within the robot's capabilities; see Fig.~\ref{fig:overview}. We focus on robotic systems that move through their environment via dynamic rhythmic interactions, adopting legged robots---in particular, dynamically stable walking bipeds~\cite{mcgeer1990passive, Collins2005Science, Hobbelen2007}---as an example. However, the tools developed in this paper can, in principle, be used to safely plan motions for aerial~\cite{Ramezani2017Science} and underwater~\cite{Georgiades2009OE} vehicles that use oscillating appendages---e.g., flapping wings or flippers---for locomotion.

At the core of our approach is the notion of a \ac{DMP}~\cite{ijspeert2013dynamical}, which in the case of dynamic walkers takes the form of a stride-to-stride map with a locally asymptotically stable fixed point representing a desired periodic locomotion behavior (Fig.~\ref{fig:overview}). Thus, a \ac{DMP} captures the \emph{local dynamics} around a nominal locomotion behavior at the stride level. Execution of each \ac{DMP} results in a displacement in the robot's workspace; these displacements form \emph{actions} available for planning (Fig.~\ref{fig:overview}). To derive conditions under which the planner's suggestions are compatible with the platform's stability limitations, this paper formulates the planning process as a \ac{SSME} in discrete time~\cite{Veer2020TAC} (Fig.~\ref{fig:overview}), and proves that its solution can be confined in an \emph{explicitly characterized} safe region provided that a bound on the frequency of \ac{DMP} switching is imposed. This bound effectively \emph{compresses} the information relevant to stable switching of \ac{DMP}s to a single number, which can be easily communicated to the planner. Besides stability, these results enable the effective approximation of the DMPs in a form that drastically reduces computational effort, thus allowing online locomotion planning in the presence of moving obstacles.  

This paper extends our preliminary work~\cite{motahar2016composing, Veer2017IROS} in three directions. First, it characterizes the dynamic properties of 3D bipeds that enable the application of the proposed approach. Second, it applies the notion of practical stability~\cite[p. 121]{la1961stability} and recent theoretical developments in \ac{SSME}s~\cite{Veer2020TAC} to derive explicit conditions for stable reactive planning with dynamic walking gaits. Third, it demonstrates how the \emph{a priori} known stability conditions can be exploited to formulate a \emph{primitive-based} sequential \ac{MPC} scheme for online planning that allows dynamically walking bipeds to react to moving obstacles. 
...
$\mathbb{R}$ and $\mathbb{Z}$ denote the real and integer numbers, and $\mathbb{R}_+$, $\mathbb{Z}_+$ the non-negative reals and integers, respectively. The index $k\in\mathbb{Z}_+$ represents discrete time. Norms are denoted by $\|\cdot\|$. Let $\mathcal{S}\subseteq \mathbb{R}^n$, then $\accentset{\circ}{\mathcal{S}}$ denotes the interior of $\mathcal{S}$. We use $\mathsf{SO}(n)$ to denote the special orthogonal group of dimension $n$. Finally, a function $\alpha:\mathbb{R}_+ \to \mathbb{R}_+$ is of class $\mathcal{K}_\infty$ if it is continuous, strictly increasing, $\alpha(0)=0$, and $\lim_{s\to\infty} \alpha(s)=\infty$; see~\cite[p. 162]{haddad2008nonlinear}.
...
%
Our approach exploits the ``triangular'' structure of the augmented stride map \eqref{eq:Phat_map} to separate gait generation from navigation. Implicit in \eqref{eq:Phat_map} is the assumption that the stride-to-stride evolution of the system's state $x \in \mathcal{S}$ and the net change $\delta \mathsf{g} = \mathsf{g}_{k+1} - \mathsf{g}_k$ of the position of the system in $\mathsf{G}$ given by \eqref{eq:P_map} and \eqref{eq:H_map} respectively, do \emph{not} depend on the pose $\mathsf{g} \in \mathsf{G}$ of the system at the beginning of the stride. In Section~\ref{sec:model} we show that under mild assumptions, a large class of 3D bipedal walking models possesses augmented stride maps that have the structure of \eqref{eq:Phat_map}.
...
\end{equation}
%eq
Clearly, for any $x \in \mathcal{M}(\kappa)$, $V_p(x) \leq \omega({\kappa})$ for all $p \in \mathcal{P}$. Thus, as illustrated in Fig.~\ref{fig:set-construction-local},  $\omega$ effectively ``inflates'' the sets $\mathcal{M}_p(\kappa)$ to the sets $\mathcal{M}_p(\omega(\kappa))$, so that 
%eq
...
\end{align}
%eq
Note that the interchangeability of $p$ and $r$ implies $\mu(\kappa)\geq 1$.
...
This section describes a primitive-based sequential \ac{MPC} approach for planning limit-cycle walking motions amidst static and moving obstacles. It will be assumed here that a set $\mathbb{A}$ of reduced-order actions that leads to a practically stable gait switching system \eqref{eq:switch-system-zd} with respect to sets $\Omega_0$ and $\Omega$ under $N_\mathrm{d}=1$ is available. Then, approximate reduced-order actions $\tilde{\mathbb{A}}$ will be extracted from $\mathbb{A}$ offline as in Section~\ref{subsec:example-actions} and used for planning online.
...
%
Assuming that a \ac{SWC} $\{ \mathcal{H}_{i}~|~i = 1,..., M \}$ and a collection of waypoints $\{ \mathrm{w}_i~|~i = 1,...,M-1 \}$ are available, we formulate a corresponding sequence $\{\text{MPC}(i)~|~i=1,...,M\}$ of primitive-based \ac{MPC} programs that safely drives the robot to the goal while keeping its stride-to-stride evolution bounded within the \ac{SWC} and avoiding any moving obstacles. 

This is done by exploiting the \emph{predictive} nature of the available actions $\tilde{\mathbb{A}}$: Given the robot's pose $\mathsf{g}_k = [\mathrm{p}^\mathsf{T}_k ~ \psi_k]^\mathsf{T}$, (reduced) state $z_k$, and primitive index $\sigma(k)$ at the beginning of the $k$-th stride, applying the corresponding action $\mathcal{A}_{\sigma(k)}$ provides a \emph{prediction} of the pose and state of the robot at the beginning of the $(k+1)$ stride. This prediction is taken as initial condition for the \ac{MPC}, and is denoted by $\mathsf{g}_{0|k}$, and $z_{0|k}$; see Fig.~\ref{fig:mpc_algorithm}. Subsequent application of actions in $\tilde{\mathbb{A}}$ results in the corresponding predictions $\mathsf{g}_{\ell|k}=[\mathrm{p}^\mathsf{T}_{\ell|k} ~ \psi_{\ell|k}]^\mathsf{T}$ and $z_{\ell|k}$ for the $\ell=1,...,N$ strides that constitute the horizon $N$ of the \ac{MPC}. With this notation, the objective of \ac{MPC}$(i)$ is to minimize the distance from the $i$-th waypoint $\mathrm{w}_i$ while keeping the predicted robot positions $\mathrm{p}_{\ell|k}$ within $\mathcal{H}_i$. To take moving obstacles into account, we assume that the area occupied by the $\nu$-th moving obstacle can be captured by an ellipse, the position $\mathrm{p}_{\nu,\ell|k}^{\rm obs}$ and orientation $\mathsf{E}_{\nu,\ell|k}$ of which are known over the horizon\footnote{It is assumed that the motion of the moving obstacles is known or can be predicted using a Kalman filter~\cite{Mora_2019_RAL}. Only moving obstacles that are within a distance of $5\mathrm{m}$ from the robot's predicted position are taken into account.} Then, for each moving obstacle $\nu=1,...,n_\mathrm{d}$, the following constraint is incorporated in the \ac{MPC} programs 
%eq
\begin{equation}\nonumber
...
\caption{Snapshots of a walking robot (red circle with radius $0.2\mathrm{m}$) avoiding a moving obstacle (purple circle of radius $0.4\mathrm{m}$) in a space cluttered with static obstacles (brown rectangles). The \ac{SWC} is represented a sequence of pairwise intersecting polytopes (shown in blue) with a set of waypoints (green dots). }
\label{fig:mpc}
\end{figure*}
%fig
...
The paper also addressed various implementation aspects towards reducing the computational cost of the method. At the core of our approach lies the idea of dimensional reduction through the enforcement of suitably designed (virtual) holonomic constraint using feedback. This allows the restriction of the planning system on lower-dimensional surfaces and drastically simplifies computations. However, this reduction comes at the cost of providing stability guarantees for the reduced-order planning system. Our future work will focus on extending these guarantees to the full-order system; see~\cite[Section 6.1.4]{Veer2018PhDThesis} for a preliminary result. Finally, it is worth mentioning that the tools developed in this paper are relevant to other robots which---like legged robots---move through their environment via periodic interactions. 
