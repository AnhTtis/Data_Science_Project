...
% ACB ------ >>>>>>>>
\usepackage{adjustbox} % Adicione este pacote ao preâmbulo
\pdfminorversion=4 
% ACB ------ <<<<<<<<
...
\thanks{DOI: 10.1109/CDC49753.2023.10383966}
}
...
\begin{enumerate}[1)]
% \vspace{2pt} 
\begin{samepage} % ACB: Adicionado samepage para que fique dentro das margens do documento
...
\begin{align}
B_w(\theta) & = \begin{bmatrix} \eta_u B_u(\theta) & \eta_d B_d(\theta) \end{bmatrix}  , \ w = \begin{bmatrix} u_2/\eta_u \\ d/\eta_d  \end{bmatrix} , \notag \\
...
K(x) = K_0 + K_1 \Pi(x)    
\end{equation}
where $\Pi(x)$ is as in \eqref{eqn:Pi}, and $K_0 \in \mathbb{R}^{n_u\times n_x}$ and $K_1 \in \bbr^{n_u \times n_mn_x}$ are matrices to be determined. 
...
\end{equation}
% \vspace{-12pt} % ACB: Adicionado espaçamento para que fique dentro das margens do documento
\begin{multline}
\hspace{-4mm} 
    \begin{bmatrix}
...
(x,\theta) \in \mcx \times \Theta 
\end{equation}
with $G_d = \diag\{G,I_{n_w},I_{n_x}\}$ and
\begin{equation*}
\Psi(x,\theta) \!=\! 
\begin{bmatrix}
       (1-\mu)Q^{-1} & \star & \star \\
...
Given the network input $\upsilon[k]$ at time $k$,  the goal of the inverse model is to learn the plant control input $u_2[k-\delta]$, at time $k-\delta$, that drove the system to the current output $y[k]$.
...
\begin{figure}[htbp]
% \vspace{6pt} % ACB: Adicionado espaçamento para que a figura fique dentro das margens do documento
   %\begin{center}
  ...
  \begin{center}
  \begin{adjustbox}{max width=\textwidth} % Adicione esta linha
  \begin{tikzpicture}
    ...
    % more arrows here
\end{tikzpicture}
\end{adjustbox} % Adicione esta linha
\end{center}
...
\begin{equation*}
    u_2 = \frac{1}{\sqrt{2} \ \! \eta_u} \tanh{(\sqrt{2} \ \! \eta_u\overline{u}_2)}
\end{equation*}
