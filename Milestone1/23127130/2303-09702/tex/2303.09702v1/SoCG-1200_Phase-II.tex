...
The subpolygons we work with are
\newchanged{
...
\remove{
\defn{$3$-geodesic cells} defined as follows.
...
We only recurse on \defn{simple} $3$-geodesic cells. 
...
To apply $\epsilon$-net techniques
we define a
\defn{$3$-geodesic range space} as follows.
...
incomplete.
They subdivide using vertical chords,
but their algorithm does not add enough vertical chords. 
...
To show that $\epsilon$-nets of size $O(1)$ exist we need the following result.
...
bound becomes %
$372$. 
\newchanged{A $4$-cell is a special case of a $4$-anchor hull (Claim~\ref{claim:4-cell}), so}
...
\remove{Define an \defn{interior anchor}
to be a face, edge, or vertex of 
$A({\cal K)}$, and define a \defn{boundary anchor} to be a 
...
\begin{figure}
  ...
  \caption{
  A simple 3-anchor hull $Q$ with anchors $v_1, v_2$ and the polygon chain $c_3$. 
...
\changed{In Appendix~\ref{appendix:epsilon-nets} we prove that the $3$-anchor range space has a subspace oracle, i.e., that there is a deterministic algorithm that, given a subset ${\cal K}' \subseteq {\cal K}$ with $|{\cal K}'| = m$, computes the set
...
and find, for each 
chord $K \in {\cal K}'$, which of the $O(m^2)$ expanded anchors in $A({\cal K}')$ intersect each side of $K$, and then, for each of the $O(m^6)$ expanded $3$-anchor hulls, eliminate the chords that have all three expanded anchors to one side, leaving the chords that cross the hull. 
}
