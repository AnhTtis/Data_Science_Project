...
\begin{example}
    ...
    However if $X$ has parameters $(sq^{\frac12h^\vee},u,\rho)$ then $X$ has wavefront set $A_4+A_1$ which is the special orbit directly below $d(A_3+A_2+A_1)$.
...
Let $\mathcal B(\bfG,K)$ denote the (enlarged) Bruhat-Tits for $\bfG(K)$.
...
If $J\subsetneq S^a$, denote by $\widetilde W_J$ the normaliser in $\widetilde W$ of $J$. Let $\CH_J(\Psi,z)$ the subalgebra of $\CH^{L}(\Psi,z)$ generated by $T_w$, $w\in \widetilde W_J$. This is a finite Hecke algebra with parameters (possibly extended by diagram automorphims).
...
\begin{theorem}[{\cite{Lu-graded},\cite{BM2}}]\label{t:2.3} The graded Hecke algebra
...
\begin{align}
&t_w\cdot t_{w'}=t_{ww'},\ w,w'\in W,\\
&\sum_{\sigma\in \CO}E_{\sigma}=1, \quad
  E_{\sigma'}E_{\sigma''}=\delta_{\sigma',\sigma''} E_{\sigma'},\quad E_{\sigma}t_{s_\al}=t_{s_\al}E_{s_\al\sigma},\label{eq:2.3.3}\\
&\omega\cdot t_{s_\al}-t_{s_\al}\cdot s_\al(\omega)=r 
  g(\al)\langle\omega,\check\al \rangle, \text{ where }\al\in \Pi,
  \omega\in \ft^*,\\
& g(\al)=\sum_{\sigma\in\CO}
  E_{\sigma}\mu_{\sigma}(\al), \text{ and }\label{eq:2.3.5}\\
&\mu_{\sigma}(\al)=\left\{ \begin{matrix} 0, &\text{ if } s_\al\sigma\neq
    \sigma,\\
                                  2\lambda(\al), &\text{ if
...
n\},$ and from Proposition \ref{p:2.3}, 
...
\begin{theorem}[{\cite[Section 8]{Lu-graded}}]\label{t:lusztig-graded}
...
$\widehat{\bC[r]},\ \widehat{\mathscr{S}},$ and $\widehat{\bC[r]}\otimes\widehat{\mathscr{S}}$ 
...
\begin{theorem}[{\cite[Section 5.2]{Lu-graded} and \cite[Theorem 3.5]{BM2}}]
...
\widehat\bA/(\bA\cdot\widehat{\CI}_{\overline\chi})=\widehat\bA_{\overline\chi},\quad
...
\end{equation}
for some integers $m_{\mu'}$. Consequently,
\begin{equation}
