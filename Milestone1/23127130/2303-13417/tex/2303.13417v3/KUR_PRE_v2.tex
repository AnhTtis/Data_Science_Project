...
by Hasegawa [\rd{Nat. Commun. \textbf{14}, 2828 (2023)}].  
...
\rd{For open quantum systems described by the Gorini-Kossakowski-Sudarshan-Lindblad (GKSL)  equation, Ref.\cite{QTUR1} derived a TUR using
the large deviation statistics. 
For the nonequilibrium steady states, Ref.\cite{QTUR2} derived a TUR by analyzing the total system. 
Reference \cite{QTUR5} studied periodically driven heat engines described by the quantum master equation and 
derived a TUR in the slow driving. 
For systems described by the GKSL equation 
with time-independent Hamiltonian and jump operators, Hesegawa \cite{QTUR3, QTUR4, QKUR1} derived a quantum KUR
\bea{
\f{\tau^2(\p_\tau \bra \Phi \ket)^2}{\RM{var}[\Phi]} \le \mI \la{KUR_H}
}
...
$\mI$ is the symmetric logarithmic derivative (SLD) Fisher information:
\bea{
...
\rd{Using this, Hasegawa estimated $\mI$ in the long-time region \cite{QTUR3}. 
For same system,} Vu and Saito \cite{Vu22} derived a quantum KUR
\bea{
\f{\tau^2(\p_\tau \bra \Phi \ket)^2}{\RM{var}[\Phi]} \le F. \la{KUR_Vu}
}
Here, $F$ is the Fisher information of probability of quantum trajectory with a time-rescaling parameter (see \re{def_theta}).
...
Vu and Saito demonstrated that $F$ \rd{is upper bounded by $\mI$}.
...
\rd{For closed quantum systems, since 1945, the Mandelstam-Tamm relation \cite{Mandelstam, review_MT} $\int_0^\tau dt \ \Dl E \ge D$ has been known 
(In this paper, we set $\hbar=1$). 
 $\Dl E$ is the energy fluctuation and $D$ is the Bures angle (see \re{def_Bures}) between the initial and final states.
Recently, even in classical systems, it turns out that there exist speed limits expressed in terms of the distance between states \cite{Shiraishi18}. 
Shiraishi \textit{et al}. \cite{Shiraishi18} demonstrated that 
\bea{
\sqrt{\f{A(\tau)\sig}{2}}\aeqge \half l(p(0),p(\tau)) \la{SFS}
}
for a system described by a classical master equation 
$\f{d}{dt}p_n(t) =\sum_m W_{nm}p_m(t)$ satisfying the local detailed balance condition.
...
A similar relation 
\bea{
A(\tau) \ge \half l(p(0),p(\tau)) \la{SL_A} 
}
have been known \cite{Vo}. 
Quantum extensions of \re{SFS} for the open quantum systems described by the GKSL equation have been researched 
\cite{Funo19, Vu21, Vu21a, Nakajima22, Vu23}.
However, the quantum extension of \re{SL_A} has been less investigated}. 
For such open quantum systems, Hasegawa \cite{H22} derived a KUR by exploiting the Mandelstam-Tamm speed limit. 
...
\rd{Our speed limit can be regarded as a quantum extension of \re{SL_A}.} 
...
In Appendix \ref{A_D}, we derive \re{dot_Q} \rd{and \re{dot_Q_-}}.
...
Here, $[A,B]\defe AB-BA$,  $\{A, B \}\defe AB+BA$, and $\bu$ is an arbitrary linear operator of the system.
...
\rd{which is the discrete version of $F$. 
In $N \to \infty$ limit, $\tl F$ becomes $F$}.  
...
&\spa \times \p_\theta^2 \ln P^\theta(\al, \{m_i \}) \Big\vert_{\theta=0}. \la{def_tl_F^pr}
}
...
\subsection{Long time approximation}

In \rd{Refs.\cite{QTUR3} and} \cite{Vu22}, the SLD Fisher information $\mI$ is calculated in the limit of the long-time
when $H_S$ and $L_k$ are time-independent. 
\rd{The SLD Fisher information $\mI$ can be rewritten as 
\bea{
\mI \aeq 4\p_{\theta_1}\p_{\theta_2}\ln \tr_S \rho^{\theta_1,\theta_2}(\tau)\bv{\theta_1=0=\theta_2} .
}
If $H_S$ and $L_k$ are time-independent, $\mI$ becomes
\bea{
\mI\aeq 4\tau \p_{\theta_1}\p_{\theta_2}\lm(\theta_1, \theta_2)\bv{\theta_1=0=\theta_2}+O(1) \la{mI_in_long}
}
in the limit of the long-time.
Here, $\lm(\theta_1, \theta_2)$ is the eigenvalue of $\mL^{\theta_1,\theta_2}$ which satisfies $\lm(0,0)=0$. 
Based on this, Refs.\cite{QTUR3} and \cite{Vu22} have derived 
\bea{
\mI \aeqap \tau(\dot{B}_\RM{ss}+\dot{Q}_+) ,\\
\dot{B}_\RM{ss} \aeqd \sum_k \tr_S[L_k \dg L_k \rho^\RM{ss}]  ,\\
\dot{Q}_+ \aeqd -4\Big( \tr_S\Big[\mL_2\mR \mL_1 \rho^\RM{ss}\Big]
 +\tr_S\Big[\mL_1\mR \mL_2\rho^\RM{ss}\Big] \Big) \la{dot_Q_+_long}
} 
with
\bea{
...
Here, $\rho^\RM{ss}$ is the steady state and $\mR$ is the pseudo-inverse of the Liouvillian (see \re{def_R_0})}.


\subsection{Our result}

Here, \rd{when $H_S$ and $L_k$ dependent on time,} we derive an expression of $\mI$ for arbitrary times \rd{based on \re{def_FI}}.
...
In Appendix \ref{A_correction}, we check $Q_+$ in the long-time region reproduces \rd{\re{dot_Q_+_long}}. 
...
After the state relaxes to the steady state, \rd{$Q_\pm$} increases with a slope of \rd{$\dot{Q}_\pm$}.
Here, 
$\rd{\dot{Q}_\pm}\defe \lim_{\tau \to \infty} \rd{Q_\pm}/\tau$ \rd{are} given by \cite{Vu22} (Appendix \ref{A_D})
\begin{widetext}
\bea{
\dot{Q}_+ \aeq \f{8\fA}{\ga y^3[4(\Dl^2+2\Om^2)+\ga^2y^2]^3} ,\la{dot_Q}\\
\rd{\dot{Q}_-} \aeq \rd{\f{2\ga \fB}{y^3[4(\Dl^2+2\Om^2)+\ga^2y^2]^3} } ,\la{dot_Q_-} \\
...
\rd{\fB} \aeqd \rd{-x(4\Dl^2+\ga^2y^2)^3+16\Om^2x(-16\Dl^4+\ga^4y^4)+16\Om^4 y^2(-4\Dl^2+3\ga^2y^2)}
}
\end{widetext}
with $x\defe n(n+1)$ and $y\defe 2n+1$.  
\rd{$\dot{Q}_+$ is non-negative. 
In general, $Q_\pm$ saturates when $\dot{Q}_\pm = 0$. }
...
\begin{center}
  \begin{minipage}[b]{0.45\linewidth}
    \centering
    \includegraphics[keepaspectratio, width = 1 \columnwidth]{Q_2_1_2}
    \subcaption{$\ga t \le 6$}
  \end{minipage}
  \begin{minipage}[b]{0.45\linewidth}
    \centering
    \includegraphics[keepaspectratio, width = 1 \columnwidth]{Q_2_2_2}
    \subcaption{$\ga t \le 2$}
  \end{minipage}
  \caption{\label{Fig_Q}$\mQ$, $Q_+$, $Q_-$, and $B$ for (a) $\ga t \le 6$ and (b) $\ga t \le 2$.
...
\bea{
D(\rho, \sig) \aeqd \cos^{-1}F(\rho, \sig) ,\la{def_Bures}\\
...
\rd{Naive extensions of \re{SFS} and \re{SL_A} to the quantum regime may be,} 
\bea{\sqrt{\f{B(\tau)\sig}{2}}\aeqge T(\tl \rho(0), \tl \rho(\tau)) ,\\
...
However\rd{,} the quantum extension of \re{SL_A} has been less investigated. 
We speculate 
\bea{
...
Thus, \rd{$\tl F$ defined by \re{F2} is given by}
\bea{
...
The associated Fisher information \rd{defined by \re{def_tl_F^pr}} is given by
\bea{
...
The first derivative \rd{leads} to
\bea{
...
\bea{
\mL\dke{\rho_n} \aeq \lm_n \dke{\rho_n } ,\la{eigen_R}\\
\dbr{l_n}\mL  \aeq \lm_n \dbr{l_n} \la{eigen_L}  . 
}
...
where 
\bea{
\mR \aeqd \sum_{n\ne 0}\f{1}{\lm_n}\dke{\rho_n} \dbr{l_n}=-\int_0^\infty dt \ e^{t\mL}\mP \la{def_R_0}
}
...
The expression of $Q_+$ in the long-time approximation is consistent with \rd{\re{dot_Q_+_long}}.
...
\section{Derivation\rd{s} of \re{dot_Q} \rd{and \re{dot_Q_-}} } \la{A_D}

\rd{We start from \re{mI_in_long}}. 
...
\bea{
\dot{Q}_+\aeq 4\lm_{12}-\rd{\dot{B}_\RM{ss}} .\la{dot_Q_general}
}
...
\rd{$Q_-$ can be rewritten as 
\bea{
Q_-(t) \aeq \f{\p^2}{\p \theta^2} \ln \tr_S \rho^\theta(t)\Bv{\theta=0}. \la{SLD_like}
}
Here, $\rho^\theta(t)$ is defined by
\bea{
\f{d}{dt}\rho^\theta(t) \aeq \mL^\theta \rho^\theta(t) ,\\
\mL^\theta \bu \aeqd (1+i \theta)\hat{\ga}\bu + \sum_k L_k \bu L_k\dg
}
with $\rho^\theta(0)=\rho(0)$. 
From \re{SLD_like}, we obtain 
\bea{
Q_-(\tau) \aeq \tau \f{\p^2}{\p \theta^2} \Lm(\theta)\Bv{\theta=0}+O(1)
}
where $\Lm(\theta)$ is the eigenvalue of $\mL^{\theta}$ which satisfies $\Lm(0)=0$. 
$\dot{Q}_-=\p^2\Lm(\theta)/\p \theta^2 \vert_{\theta=0} $ is calculated in the similar way as $\lm_{12}$. 
Then, we obtain \re{dot_Q_-}.
}
...
\re{S58} is consistent with \rd{\re{KUR_H}}.
...
\bibitem{H22}Y. Hasegawa, \rd{ \co{Unifying speed limit, thermodynamic uncertainty relation and Heisenberg principle via bulk-boundary correspondence}, 
Nat. Commun. \textbf{14}, 2828 (2023).}
