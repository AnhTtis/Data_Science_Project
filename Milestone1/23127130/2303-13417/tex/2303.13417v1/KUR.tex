\documentclass[showpacs, preprintnumbers, amsmath, amssymb, floatfix]{revtex4-2}
\usepackage{graphicx}
...
In a quantum kinetic uncertainty relation derived by Vu-Saito [Phys. Rev. Lett. \textbf{128}, 140602 (2022)], 
...
For a finite time and arbitrary initial state, we give concise coupled first-order ordinary differential equations 
to calculate the SLD Fisher information given by a double integral concerning time. 
We also derive a simple lower bound of the Fisher information of quantum trajectory. 
The SLD Fisher information also appears in the speed limit based on the Mandelstam-Tamm relation 
[Hasegawa, arXiv:2203.12421v4].  
When the jump operators connect eigenstates of the system Hamiltonian, we show that 
 the Bures angle is upper bounded by the square root of the dynamical activity at short times, which contrasts with the classical counterpart.
...
In recent years, studies of universal relations that characterize the fluctuations of nonequilibrium systems have been intensively investigated.
...
with time-independent Hamiltonian and jump operators, Vu-Saito \cite{Vu22} derived a quantum KUR
\bea{
...
The calculation of $F$ is not light since one has to perform a sum over a huge number of trajectories. 
$F$ is bounded by the symmetric logarithmic derivative (SLD) Fisher information:
\bea{
\mI \aeqd 4[\p_{\theta_1}\p_{\theta_2}\mC(\theta_1,\theta_2)
-\p_{\theta_1}\mC(\theta_1,\theta_2)\p_{\theta_2}\mC(\theta_1,\theta_2) \Big]\Bv{\theta_1=0=\theta_2} \la{def_FI} ,\\
\mC(\theta_1,\theta_2) \aeqd \tr_S \rho^{\theta_1,\theta_2}(\tau).
}
Here,  $\rho^{\theta_1,\theta_2}(t)$ is the solution of the two-sided GKSL equation \cite{SLD}. 
$\tr_S$ is the trace of the system. 
Using this, Vu-Saito estimated an upper bound of $\mQ$ only in the long-time region. 
...
Another class of inequalities, the speed limit of state transformation, is also studied actively. 
...
For such open quantum systems, Hasegawa \cite{H22} derived a KUR from the Mandelstam-Tamm relation \cite{Mandelstam, review_MT}. 
In this KUR,  a dynamical activity-like quantity appeared instead of $F$.
We recognize that this quantity equals the SLD Fisher information when Hamiltonian and jump operators are time-independent. 
Using our expressions, we derive a speed limit described by the dynamical activity. 


The structure of the paper is as follows. 
First, we explain the Fisher information $F$ (\res{s_Vu}).
In \res{s_Fisher}, we analyze the SLD Fisher information $\mI$ and show that $\mI$ is a sum of the dynamical activity and a quantum correction, 
...
\mL(t)\bu \aeqd  -i[H_S(t), \bu]+\sum_k \Big[L_k(t) \bu L_k(t) \dg - \half \{  L_k(t) \dg L_k(t) ,\bu \} \Big].
}
...
\bea{
F \aeqd -\bra \p_\theta^2 \ln \mP^\theta(\Ga) \big \vert_{\theta=0} \ket ,
}
$\bra \bu \ket $ denotes the expected value for the probability distribution $\mP^\theta(\Ga)\vert_{\theta=0}$. 
...
Here, 
\bea{
\mL^{\theta_1,\theta_2}(t) \bu \aeqd -iH_S^{\theta_1} \bu +\bu i H_S^{\theta_2} 
+\sum_k \Big\{ L_k^{\theta_1} \bu (L_k^{\theta_2})\dg
-\half \Big[ (L_k^{\theta_1})\dg L_k^{\theta_1}\bu + \bu  (L_k^{\theta_2})\dg L_k^{\theta_2} \Big] \Big\}. \la{two_2L}
}

$F$ can be calculated only numerically.
First, we discretize time and introduce
\bea{
\Om_0^\theta \defe 1+\Big(-i H_S^\theta -\half  \sum_{m=1}^M  (L_m^\theta)\dg L_m^\theta \Big)\Dl t  \com
\Om_m^\theta \defe L_m^\theta \sqrt{\Dl t}  \ \ (m=1,\cdots, M).\la{Om_mu}
}
...
\bea{
P^\theta(\{m_i \}) \defe \tr_S[\Om_{m_{N-1}}^\theta \cdots \Om_{m_0}^\theta \rho(0)(\Om_{m_0}^\theta)\dg \cdots (\Om_{m_{N-1}}^\theta)\dg ] \la{pro_Nu}
}
in $N \to \infty$.
...
We explain the original Fisher information of Vu-Saito \cite{Vu22}.
...
\bea{
P^\theta(\al, \{m_i \}) \defe p_\al \tr_S[\Om_{m_{N-1}}^\theta \cdots \Om_{m_0}^\theta 
\ke{\al}\br{\al}(\Om_{m_0}^\theta)\dg \cdots (\Om_{m_{N-1}}^\theta)\dg ].
}
The associated Fisher information is given by
\bea{
\tl F^\pr \defe -\sum_\al  \sum_{m_0, \cdots,m_{N-1}=0}^M P^\theta(\al, \{m_i \})
 \p_\theta^2 \ln P^\theta(\al, \{m_i \}) \Big\vert_{\theta=0}.
}
Vu-Saito's original Fisher information $F^\pr$ is $N \to \infty$ limit of $\tl F^\pr$. 
$F^\pr \le \mI $ holds (Appendix \ref{A_SLD}). 
However, in general, $F^\pr$ does not coincide with $F$ and 
...
In Ref.\cite{Vu22}, the SLD Fisher information $\mI$ is calculated in the limit of the log-time.
Here, we derive an expression for arbitrary times.
The first term of \re{def_FI} is reduced to the sum of the dynamical activity and two double integrals (Appendix \ref{A_B}):
\bea{
\p_{\theta_1}\p_{\theta_2}\mC(\theta_1,\theta_2) \bv{\theta_1=0=\theta_2} \aeq \f{1}{4}B(\tau)+I_1+I_2 ,\la{def_I}\\
B(t) \aeqd \int_0^t ds \ \sum_k \tr_S [L_k(s)\rho(s) \dg L_k(s)] ,\\
...
with
\bea{
\mL_{i}(s) \defe \p_{\theta_i}\mL^{\theta_1,\theta_2}(s) \bv{\theta_1=0=\theta_2}.
}
Here, $B(t)$ is the dynamical activity. 
$\mU(s,u)$ is defined by 
\bea{
\f{\p \mU(s,u)}{\p s} \aeq \mL(s)\mU(s,u) 
}
with $\mU(u,u)=1$. 
The second term of \re{def_FI} is given by
\bea{
-\p_{\theta_1}\mC(\theta_1,\theta_2)\p_{\theta_2}\mC(\theta_1,\theta_2) \bv{\theta_1=0=\theta_2}
= -\prod_{i=1}^2\int_0^\tau ds \ \tr_S\Big[\mL_{i}(s)  \rho(s)\Big]=:I_3. \la{def_I_3}
}
$\mL_i$ is given by
\bea{
\mL_1 \bu \aeq -iH_S\bu +\half\sum_k \Big[ L_k\bu L_k \dg - L_k\dg L_k \bu \Big],\\
\mL_2 \bu \aeq \bu iH_S+ \half \sum_k \Big[ L_k \bu  L_k \dg -\bu L_k\dg L_k\Big] .
}
Using
\bea{
...
we obtain 
\bea{
I_3 \aeq -\Big(\int_0^\tau ds \ \tr_S\Big[H_S(s)\rho(s)\Big] \Big)^2.
}
...
Here, $Q_+$ is the quantum correction of the dynamical activity and an upper bound of $\mQ \defe F-B$. 

$Q_+(t)$ can be written as
\bea{
Q_+(t) \aeq Q_a(t)+Q_b(t)\la{Q_ex} ,\\
Q_a(t) \aeqd 4\int_0^t ds \int_0^s du \ \tr_S\Big[\mL_2(t) \mU(s,u) \mP(u)\mL_1(u)\rho(u)\Big] ,\\
Q_b(t) \aeqd 4\int_0^t ds \int_0^s du \ \tr_S\Big[\mL_1(t) \mU(s,u) \mP(u)\mL_2(u)\rho(u)\Big] 
 }
with 
\bea{
\mP(u)\bu \defe \bu-\rho(u)\tr_S[\bu].
}
Because $Q_b=Q_a^\ast$ (Appendix \ref{A_B}), 
\bea{
Q_+=2\RM{Re}(Q_a) \la{Q_+_re}
}
holds.
Here, $\RM{Re}(x)$ is the real part of $x$. 
$Q_+(t)$ can be calculated by a double integral concerning time. 
The double integral for $Q_a$ can be calculated from the following coupled differential equations,
\bea{
...
The lower bound of $\mQ$ is also given by a double integral concerning time (Appendix \ref{s_Lower}):
\bea{
\mQ \ge Q_- \defe B(\tau)^2-2\int_0^\tau ds\int_0^s du \ \tr_S[\hat{\ga}(s) \mU(s,u)\hat{\ga}(u) \rho(u)]=:B(\tau)^2 +Q_c(\tau). \la{Q_-}
}
Here, 
\bea{
\hat{\ga}(t) \bu \defe \Big(-i H_S -\half  \sum_k  L_k\dg L_k\Big) \bu +  \bu \Big(i H_S -\half  \sum_k  L_k\dg L_k\Big). \la{def_ga}
}
...
Figure \ref{Fig_Q} shows the time dependence of the dynamical activity $B$ and the quantum correction $\mQ(=F-B)$
and its under and lower bounds ($Q_+$ and $Q_-$). 
$F$ was calculated by using \re{Om_mu}, \re{pro_Nu} and \re{F2}
with the quantum jump method \cite{QJ}. 
In Fig.\ref{Fig_Q}, we set $\ga \Dl t = 0.001$ and used $10^6$ trajectories. 
$Q_-\le \mQ \le Q_+$ holds.
Figure \ref{Fig_Q} (a) shows that the dynamical activity $B$ and the quantum correction $\mQ$ are in the same order. 
At short times, $\mQ$ also takes negative values as Fig.\ref{Fig_Q} (b) shows. 
...
In this section, we study a quantum speed limit derived by Hasegawa \cite{H22}:
\bea{
...
with 
\bea{
V_{\tau}(s;t) \defe  \RM{T} \exp \Big[\int_0^s du \ \Big\{-i\f{t}{\tau} H_S\Big(\f{t}{\tau}u\Big)
+ \sqrt{\f{t}{\tau}} \sum_k [L_k\Big(\f{t}{\tau}u\Big)\otimes \phi_k\dg (u)
-L_k\Big(\f{t}{\tau}u\Big)\dg \otimes \phi_k (u)] \Big\} \Big]. 
}
...
$\mJ(t)/4$ is the energy variance and $\mJ(t)$ is the SLD Fisher information (see \re{SLD_pre}):
\bea{
...
holds. 
Remark that the trace distance $T(\rho_1,\rho_2) \defe \half \tr_S \sqrt{(\rho_1-\rho_2)^2}$ 
...
In this case, the quantum master equation for $\tl \rho(t)$ is given by
\bea{
\f{d\tl \rho}{dt} \aeq \sum_k \Big[L_k \tl \rho(t) L_k \dg - \half \{  L_k \dg L_k ,\tl \rho(t) \} \Big].
}
Repeating the arguments from \re{MTR} to \re{MTR3}, we obtain a quantum speed limit of the system:
\bea{
...
with $H_S=\ep a\dg a$ and $\mD[X](\bu) \defe X\bu X\dg-\half \{X\dg X, \bu \}$. 
...
Figure \ref{Fig_1}(a) shows the angle $D$, the trace distance $T$, and geometric length $l$ as functions of final time $t_2$. 
In Fig.\ref{Fig_1}, we set the initial time $\ga t_1=2$. 
Figure \ref{Fig_1}(b) shows that the bound achievement rate $D(\tl \rho(t_1), \tl \rho(t_2))/l(t_1.t_2)$ becomes greater than 0.8 around $\ga t_2=2$.  
In Fig.\ref{Fig_2}, we showed the results when the initial time $\ga t_1=0$. 
Around $\ga t=0$, because of $B(t) \propto t$, $l(0,t) \propto \sqrt{t}$ and $D(\tl \rho(0), \tl \rho(t))/l(0,t)  \propto \sqrt{t}$ hold. 
This time dependence spoils the upper bound. 
...
\sqrt{\f{A(\tau)\sig}{2}}\aeqge  \half (p(0),p(\tau)) \la{SFS}
}
...
The naive extensions
\bea{
B(\tau) \aeqge T(\tl \rho(0), \tl \rho(\tau))  ,\\
\sqrt{\f{B(\tau)\sig}{2}}\aeqge T(\tl \rho(0), \tl \rho(\tau)) 
}
do not hold even in the spinless quantum dot. 
These are quantum effects. 
The quantum extensions of \re{SFS} are known \cite{Funo19, Vu23, Nakajima22}, whereas the quantum extension of \re{SL_A} is not. 
We speculate 
\bea{
\sqrt{B(\tau)} \aeqge D(\tl \rho(0), \tl \rho(\tau)) 
}
from the discussion of Fig. \ref{Fig_2}. 
We cannot derived it from \re{main_3}, but the looser 
\bea{
\sqrt{b_\RM{max}(\tau)\tau} \ge D(\tl \rho(0), \tl \rho(\tau))  \la{b_max}
}
can be derived because $\sqrt{b_\RM{max}(\tau)\tau}  \ge l(0, \tau)$. 
Here,  $b_\RM{max}(\tau) \defe \max_{0\le t \le \tau} \f{1}{t}B(t)$. 
Derivation of a simple quantum speed limit at zero initial time is a future work. 


\section{Summary} \la{s_summary}

We investigated the symmetric logarithmic derivative (SLD) Fisher information which plays an essential role in the 
kinetic uncertainty relations (KURs) for open quantum systems described by the GKSL equation with and without the detailed balance condition. 
Vu and Saito \cite{Vu22} introduced the SLD Fisher information as an upper bound of the Fisher information of quantum trajectory. 
For a finite time and arbitrary initial state, we derived a concise expression of the SLD Fishder information using a double integral concerning time, 
which can be calculated by numerically solving coupled first-order ordinary differential equations.  
We also derived a simple lower bound of the Fisher information of quantum trajectory.
We also pointed out that for time-independent system, the SLD Fisher information divided time squared is identical to the SLD Fisher information 
appeared in the Mandelstam-Tamm theory of speed limit by Hasegawa \cite{H22}.
Based on this observation, we studied the speed limit derived from the Mandelstam-Tamm relation and showed that 
the Bures angle between system states in the interaction picture is bounded by the dynamical activity. 
...
\ke{\tl \psi(0)}\otimes \ke{m_{N-1}, \cdots, m_1, m_0}
}
...
We suppose that $\Om_m^\theta$ are the same as \re{Om_mu}.
...
\bea{
F \le \mI = 4  \Big[\f{\p^2}{\p \theta_1 \p \theta_2} \bra \psi^{\theta_2} \ke{ \psi^{\theta_1}}
- \Big(\f{\p}{ \p \theta_2} \bra \psi^{\theta_2}  \ke{\psi^{\theta_1}} \Big) 
\Big(\f{\p}{ \p \theta_1}\bra \psi^{\theta_2} \ke{\psi^{\theta_1}} \Big) \Big]
\Big \vert_{\theta_1=0=\theta_2} .
}
Here, 
\bea{
\bra \psi^{\theta_2} \ke{ \psi^{\theta_1}} =\tr_S \rho^{\theta_1,\theta_2}(\tau) \com 
\rho^{\theta_1,\theta_2}(\tau) \defe \tr_{AE}[\ke{\psi^{\theta_1}}\br{\psi^{\theta_2}}].
}
The time evolution equation of $\rho^{\theta_1,\theta_2}(t)$ is given by \re{two_2} \cite{SLD}.
Then, we obtain \re{I_J_relation}.


One of the purifications is given by
\bea{
\ke{\tl \psi(0)} = \sum_\al \sqrt{p_\al} \ke{\al} \otimes \ke{\varphi_\al}_A
}
with ${}_A\bra\varphi_\al \ke{\varphi_\be}_A=\dl_{\al\be}$. 
For POVM 
...
and 
\bea{
\int_0^\tau ds \int_0^s du \  \tr_S\Big[\mL_{i^\pr}(s) \mU(s,u) \mL_{i}(u) \rho(u)\Big]=I_i \ \ (i=1,2)
}
...
\bea{
Q_+(\tau) = 4 \Big( \int_0^\tau ds \int_0^s du \ \tr_S\Big[\mL_2 e^{(s-u)\mL} \mP \mL_1 \rho^\RM{ss}\Big] +
\int_0^\tau ds \int_0^s du  \ \tr_S\Big[\mL_1 e^{(s-u)\mL} \mP \mL_2\rho^\RM{ss} \Big] \Big)
}
with 
\bea{
...
we obtain
\bea{
\int_0^\tau ds \int_0^s du \ e^{(s-u)\mL} \mP 
...
Thus, we obtain
\bea{
Q_+(\tau) \approx -4\tau \Big( \tr_S\Big[\mL_2\mR \mL_1 \rho^\RM{ss}\Big]
...
\ga_n(\{m_i \}) \aeqd \f{1}{P^0(\{m_i \})} \tr_S[\hat{\Om}_{m_{N-1}} \cdots \hat{\ga} \cdots  \hat{\Om}_{m_0} \rho(0)] .
}
...
$b(\{m_i \})^2$ and $b(\{m_i \})d(\{m_i \})$ are given by
\bea{
b(\{m_i \})^2 \aeq b(\{m_i \}) +2\sum_{n=1}^{N-1}\sum_{l=0}^{n-1}\dl_{m_n\ne 0}\dl_{m_l\ne 0} ,\la{b^2}\\
b(\{m_i \})d(\{m_i \}) \aeq \sum_{n=1}^{N-1}\sum_{l=0}^{n-1}\Big(\dl_{m_n\ne 0} \ga_l(\{m_i \})\dl_{m_l,0} \Dl t+ \ga_n(\{m_i \})\dl_{m_n,0} \Dl t  \dl_{m_l\ne 0}\Big).
}
...
\bea{
\bra b(\{m_i \})^2 \ket \aeq B(\tau)+2\int_0^\tau ds\int_0^s du \ \tr_S[\hat{\Ga}(s) \mU(s,u)\hat{\Ga}(u) \rho(u)] ,\\
\bra b(\{m_i \})d(\{m_i \}) \ket  \aeq \int_0^\tau ds\int_0^s du \ \Big(\tr_S[\hat{\Ga}(s) \mU(s,u)\hat{\ga}(u) \rho(u)] 
+\tr_S[\hat{\ga}(s) \mU(s,u)\hat{\Ga}(u) \rho(u)]  \Big)
}
where
\bea{
...
Thus, we obtain
\bea{
\bra b(\{m_i \})^2 \ket +2 \bra b(\{m_i \})d(\{m_i \}) \ket \aeq B(\tau)-2\int_0^\tau ds\int_0^s du \ \tr_S[\hat{\ga}(s) \mU(s,u)\hat{\ga}(u) \rho(u)]
}
and 
\bea{
\mQ \aeq \bra d(\{m_i \})^2 \ket -2\int_0^\tau ds\int_0^s du \ \tr_S[\hat{\ga}(s) \mU(s,u)\hat{\ga}(u) \rho(u)] .
}
Here, $\bra d(\{m_i \})^2 \ket$ cannot be written in the form of a double integral because 
$\ga_n(\{m_i \})$ in $d(\{m_i \}) $ depends on the whole trajectory.
...
\bea{
a^{(0)}_i + a^{(1)}\lm_i \aeq 0 \ \ (i=1,2) ,\la{lm_1}\\
a^{(0)}_{12}+a^{(1)}\lm_{12}+a^{(1)}_1\lm_2  +a^{(1)}_2\lm_1 +2a^{(2)}\lm_1 \lm_2 \aeq 0.\la{lm_2}
}
Here,
\bea{
a^{(n)}\defe a^{(n)}(0,0) \com X_i \defe \p_{\theta_i} X \vert_{\theta_1=0=\theta_2} 
\com X_{12} \defe \p_{\theta_1}\p_{\theta_2} X \vert_{\theta_1=0=\theta_2}.
}
$\lm_{12}$ is calculated from \re{lm_1} and \re{lm_2}. 
$\dot{Q}_+\defe \lim_{\tau \to \infty} Q_+/\tau$ is given by
\bea{
\dot{Q}_+\aeq 4\lm_{12}-\dot{B} ,\la{dot_Q_general}\\
\dot{B} \aeqd \sum_k \tr_S[L_k \dg L_k \rho^\RM{ss}].
}
...
U(t) \aeq \RM{T} \exp \Big[\int_0^t ds \ \Big\{-iH_S(s)+ \sum_k [L_k(s)\otimes \phi_k\dg (s)-L_k(s)\dg \otimes \phi_k (s)] \Big\} \Big].
}
...
\bea{
d(\ke{\Phi(t)} \br{\Phi(t)})=d\ke{\Phi(t)} \br{\Phi(t)} +\ke{\Phi(t)} d\br{\Phi(t)} +d\ke{\Phi(t)} d\br{\Phi(t)} ,
}
and 
\bea{
d\ke{\Phi(t)} \aeq \Big(\big( -iH_S-\f{1}{2}\sum_k L_k\dg L_k\big) dt 
+ \sum_k L_k d\phi_k\dg 
+\half \sum_{k,l} L_kL_l d\phi_k\dg d\phi_l\dg\Big)\ke{\Phi(t)}, \la{dPhi}
}
...
with 
\bea{
\mL_\tau(s;t_1,t_2) \bu \aeq -i\f{t_1}{\tau} H_S\Big(\f{t_1}{\tau}s\Big) \bu +\bu i \f{t_2}{\tau} H_S\Big(\f{t_2}{\tau}s\Big) \no\\
...
+\bu \f{t_2}{\tau}L_k\Big(\f{t_2}{\tau}s\Big)\dg L_k\Big(\f{t_2}{\tau}s\Big) \Big]. \la{two_1L}
}
...
\bea{
\mJ(t) \aeq  4[\p_{t_1}\p_{t_2}C(t_1,t_2)-\p_{t_1}C(t_1,t_2)\p_{t_2}C(t_1,t_2) \Big]\Bv{t_1=t=t_2} \la{def_mI} ,\\
C(t_1,t_2) \aeqd \tr_S \rho_\tau(\tau;t_1,t_2) .
}
...
\f{d}{dt}y = -\half \ga y \com
\f{d}{dt}z=-\ga (z-[1-2f(\ep)]).
}
...
This leads to
\bea{
F(\tl \rho(t_1), \tl \rho(t_2)) \aeq \f{1}{\sqrt{2}}\sqrt{1+\bm{r}(t_1)\cdot\bm{r}(t_2)+\sqrt{[1-\bm{r}(t_1)^2][1-\bm{r}(t_2)^2]}}.
}
The Bures angle is given by $D(\tl \rho(t_1), \tl \rho(t_2))=\cos^{-1}F(\tl \rho(t_1), \tl \rho(t_2))$.
The trace distance is given by
\bea{
T(\tl \rho(t_1), \tl \rho(t_2)) \aeq \half \abs{\bm{r}(t_1)-\bm{r}(t_2)}. 
}
