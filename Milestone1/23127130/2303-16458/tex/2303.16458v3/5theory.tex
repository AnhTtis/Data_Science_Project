...
Given a graph $G=(V,E)$ ($V$ and $E$ are node set and edge set), graph motifs are substructure  $F=(V^\prime,E^\prime)$ that recur significantly in statistics, where $V^{\prime} \subset V, E^{\prime} \subset E$ and $\left|V^{\prime}\right| \ll|V|$.
\end{definition}
...
\end{equation}
where $|F_a|$ represents the number of nodes in motif $F_a$, and $||\cdot||_\square$ denotes the cut norm.
\end{theorem}
...
The homomorphism density of graph motif $F$ in $\mathbb{G}$ can be considered approximately equal to that in $f( \{\alpha_i\}, \{B_i\})$ with high probability, which can be represented as
\begin{equation}
