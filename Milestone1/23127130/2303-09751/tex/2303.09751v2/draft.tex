...
hadronic contributions to muon $g-2$, see 
...
Currently, there are discrepancies between the data-driven approaches and the lattice simulations~(see Ref.~\cite{ParticleDataGroup:2022pth} and references therein).
...
In the normal ordering case, we also fix the parameters as\footnote{%
Here, we consider a large value of $h_{e \mu}$, whose effect was not discussed in the previous section.
Since we investigate non-thermal leptogenesis, the right-handed neutrinos are not in the thermal bath.
They decay promptly after being generated from the inflatons, and hence we expect that they do not alter the potential of $\sigma$.
Even if finite density effects of the right-handed neutrinos exist, such effects are negligible because their number density is much smaller than that of other particles in the thermal bath.
}
%%
\begin{gather}
